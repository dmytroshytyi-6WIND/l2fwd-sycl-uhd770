...
static inline void *iova2kva(struct kni_dev *kni, void *iova) {
  return phys_to_virt(iova_to_phys(kni->usr_tsk, (unsigned long)iova));
}

static inline void *iova2data_kva(struct kni_dev *kni, struct rte_kni_mbuf *m) {
  return phys_to_virt(iova_to_phys(kni->usr_tsk, m->buf_iova) + m->data_off);
}
#endif

/* physical address to kernel virtual address */
static void *pa2kva(void *pa) { return phys_to_virt((unsigned long)pa); }

/* physical address to virtual address */
static void *pa2va(void *pa, struct rte_kni_mbuf *m) {
  void *va;

  va = (void *)((unsigned long)pa + (unsigned long)m->buf_addr -
                (unsigned long)m->buf_iova);
  return va;
}

/* mbuf data kernel virtual address from mbuf kernel virtual address */
static void *kva2data_kva(struct rte_kni_mbuf *m) {
  return phys_to_virt(m->buf_iova + m->data_off);
}

static inline void *get_kva(struct kni_dev *kni, void *pa) {
#ifdef HAVE_IOVA_TO_KVA_MAPPING_SUPPORT
  if (kni->iova_mode == 1)
...
static inline void *get_data_kva(struct kni_dev *kni, void *pkt_kva) {
#ifdef HAVE_IOVA_TO_KVA_MAPPING_SUPPORT
  if (kni->iova_mode == 1)
 ...
 */
static int kni_net_process_request(struct net_device *dev,
...
static int kni_net_release(struct net_device *dev) {
  int ret;
  struct rte_kni_request req;
...
static void kni_fifo_trans_pa2va(struct kni_dev *kni,
                                 struct rte_kni_fifo *src_pa,
                                 struct rte_kni_fifo *dst_va) {
 ...
 */
static int kni_net_config(struct net_device *dev, struct ifmap *map) {
...
static void kni_net_rx_normal(struct kni_dev *kni) {
  ...
  for (i = 0; i < num_rx; i++) {
    ...
    if (kva->nb_segs == 1) {
      ...
      for (nb_segs = 0; nb_segs < kva_nb_segs; nb_segs++) {
        memcpy(skb_put(skb, kva->data_len), data_kva, kva->data_len);
...
static void kni_net_tx_timeout(struct net_device *dev, unsigned int txqueue)
#else
...
static int kni_net_change_mtu(struct net_device *dev, int new_mtu) {
  int ret;
  struct rte_kni_request req;
...
static void kni_net_change_rx_flags(struct net_device *netdev, int flags) {
  struct rte_kni_request req;
 ...
 *  Fill the eth header
 */
static int kni_net_header(struct sk_buff *skb, struct net_device *dev,
...
static int kni_net_rebuild_header(struct sk_buff *skb) {
