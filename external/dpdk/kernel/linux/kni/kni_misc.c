...
static int kni_check_param(struct kni_dev *kni,
                           struct rte_kni_device_info *dev) {
  if (!kni || !dev)
    return -1;
...
static int kni_run_thread(struct kni_net *knet, struct kni_dev *kni,
                          uint8_t force_bind) {
  /**
  ...
  if (multiple_kthread_on) {
    kni->pthread =
        kthread_create(kni_thread_multiple, (void *)kni, "kni_%s", kni->name);
    ...
    if (knet->kni_kthread == NULL) {
      knet->kni_kthread =
          kthread_create(kni_thread_single, (void *)knet, "kni_single");
...
static int kni_ioctl_create(struct net *net, uint32_t ioctl_num,
...
static int kni_ioctl_release(struct net *net, uint32_t ioctl_num,
                             unsigned long ioctl_param) {
  struct kni_net *knet = net_generic(net, kni_net_id);
  int ret = -EINVAL;
  struct kni_dev *dev, *n;
...
static long kni_ioctl(struct file *file, unsigned int ioctl_num,
                      unsigned long ioctl_param) {
  long ret = -EINVAL;
  struct net *net = current->nsproxy->net_ns;
...
static long kni_compat_ioctl(struct file *file, unsigned int ioctl_num,
                             unsigned long ioctl_param) {
...
static int __init kni_parse_carrier_state(void) {
  if (!carrier) {
    kni_dflt_carrier = 0;
    return 0;
  }
...
static int __init kni_parse_bifurcated_support(void) {
...
static void __exit kni_exit(void) {
