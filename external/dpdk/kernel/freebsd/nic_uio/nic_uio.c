...
static device_method_t nic_uio_methods[] = {
    ...
    DEVMETHOD(device_detach, nic_uio_detach), DEVMETHOD_END};
...
DEFINE_CLASS_0(nic_uio, nic_uio_driver, nic_uio_methods,
               sizeof(struct nic_uio_softc));
DRIVER_MODULE(nic_uio, pci, nic_uio_driver, nic_uio_devclass, nic_uio_modevent,
              0);

static int nic_uio_mmap(struct cdev *cdev, vm_ooffset_t offset,
                        vm_paddr_t *paddr, int prot, vm_memattr_t *memattr) {
  *paddr = offset;
  return 0;
}

static int nic_uio_mmap_single(struct cdev *cdev, vm_ooffset_t *offset,
...
int nic_uio_open(struct cdev *dev, int oflags, int devtype, struct thread *td) {
  return 0;
}

int nic_uio_close(struct cdev *dev, int fflag, int devtype, struct thread *td) {
  return 0;
}
...
static int nic_uio_probe(device_t dev) {
  ...
  while (1) {
    ...
    if (bus == pci_get_bus(dev) && device == pci_get_slot(dev) &&
      ...
      if (num_detached < MAX_DETACHED_DEVICES) {
        ...
        printf(
            "%s: reached MAX_DETACHED_DEVICES=%d. dev=%p won't be reattached\n",
            __func__, MAX_DETACHED_DEVICES, dev);
        break;
      }
    }
  }
...
static int nic_uio_attach(device_t dev) {
  ...
  sc->dev_t = dev;
  sc->my_cdev = make_dev(&uio_cdevsw, device_get_unit(dev), UID_ROOT, GID_WHEEL,
                         0600, "uio@pci:%u:%u:%u", pci_get_bus(dev),
...
static void nic_uio_unload(void) {
  int i;
...
static int nic_uio_shutdown(void) { return 0; }
