...
moduledata_t contigmem_mod = {"contigmem", (modeventhand_t)contigmem_modevent,
                              0};
...
static int contigmem_load() {
  ...
  for (i = 0; i < contigmem_num_buffers; i++) {
    addr = contigmalloc(contigmem_buffer_size, M_CONTIGMEM, M_ZERO, 0,
    ...
    snprintf(description, sizeof(description), "phys addr for buffer %d", i);
    SYSCTL_ADD_PROC(NULL, &SYSCTL_NODE_CHILDREN(_hw_contigmem, physaddr),
                    OID_AUTO, index_string, CTLTYPE_U64 | CTLFLAG_RD,
...
error:
  for (i = 0; i < contigmem_num_buffers; i++) {
    if (contigmem_buffers[i].addr != NULL) {
      contigfree(contigmem_buffers[i].addr, contigmem_buffer_size, M_CONTIGMEM);
...
static int contigmem_open(struct cdev *cdev, int fflags, int devtype,
                          struct thread *td) {
...
static int contigmem_close(struct cdev *cdev, int fflags, int devtype,
                           struct thread *td) {
...
static int contigmem_cdev_pager_ctor(void *handle, vm_ooffset_t size,
...
static void contigmem_cdev_pager_dtor(void *handle) {
  struct contigmem_vm_handle *vmh = handle;
  struct contigmem_buffer *buf;
...
static int contigmem_cdev_pager_fault(vm_object_t object, vm_ooffset_t offset,
...
static int contigmem_mmap_single(struct cdev *cdev, vm_ooffset_t *offset,
                                 vm_size_t size, struct vm_object **obj,
                                 int nprot) {
  ...
  *obj = cdev_pager_allocate(vmh, OBJT_DEVICE, &contigmem_cdev_pager_ops, size,
