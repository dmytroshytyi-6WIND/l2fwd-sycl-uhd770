...
struct qat_qp_hw_spec_funcs *qat_qp_hw_spec[QAT_N_GENS];

static int qat_qp_check_queue_alignment(uint64_t phys_addr,
                                        uint32_t queue_size_bytes);
static void qat_queue_delete(struct qat_queue *queue);
static int qat_queue_create(struct qat_pci_device *qat_dev,
                            struct qat_queue *queue, struct qat_qp_config *,
                            uint8_t dir);
...
static int adf_queue_arb_enable(struct qat_pci_device *qat_dev,
                                struct qat_queue *txq, void *base_addr,
                                rte_spinlock_t *lock);
static int adf_queue_arb_disable(enum qat_device_gen qat_dev_gen,
                                 struct qat_queue *txq, void *base_addr,
                                 rte_spinlock_t *lock);
static int qat_qp_build_ring_base(struct qat_pci_device *qat_dev, void *io_addr,
...
int qat_qp_setup(struct qat_pci_device *qat_dev, struct qat_qp **qp_addr,
                 uint16_t queue_pair_id, struct qat_qp_config *qat_qp_conf) {
  struct qat_qp *qp = NULL;
  struct rte_pci_device *pci_dev = qat_pci_devs[qat_dev->qat_dev_id].pci_dev;
  char op_cookie_pool_name[RTE_RING_NAMESIZE];
  struct qat_dev_hw_spec_funcs *ops_hw = qat_dev_hw_spec[qat_dev->qat_dev_gen];
  void *io_addr;
  uint32_t i;

  QAT_LOG(DEBUG, "Setup qp %u on qat pci device %d gen %d", queue_pair_id,
          qat_dev->qat_dev_id, qat_dev->qat_dev_gen);
  ...
  qp = rte_zmalloc_socket("qat PMD qp metadata", sizeof(*qp),
  ...
  qp->op_cookies =
      rte_zmalloc_socket("qat PMD op cookie pointer",
  ...
  if (qat_queue_create(qat_dev, &(qp->tx_q), qat_qp_conf, ADF_RING_DIR_TX) !=
      0) {
    QAT_LOG(ERR,
  ...
  qp->max_inflights = ADF_MAX_INFLIGHTS(
      qp->tx_q.queue_size, ADF_BYTES_TO_MSG_SIZE(qp->tx_q.msg_size));
  ...
  if (qat_queue_create(qat_dev, &(qp->rx_q), qat_qp_conf, ADF_RING_DIR_RX) !=
      0) {
    QAT_LOG(ERR,
  ...
  snprintf(op_cookie_pool_name, RTE_RING_NAMESIZE, "%s%d_cookies_%s_qp%hu",
  ...
  if (qp->op_cookie_pool == NULL)
    qp->op_cookie_pool = rte_mempool_create(
        op_cookie_pool_name, qp->nb_descriptors, qat_qp_conf->cookie_size, 64,
        0, NULL, NULL, NULL, NULL, pci_dev->device.numa_node, 0);
  ...
  QAT_LOG(DEBUG, "QP setup complete: id: %d, cookiepool: %s", queue_pair_id,
          op_cookie_pool_name);
...
static int qat_queue_create(struct qat_pci_device *qat_dev,
  ...
  struct rte_pci_device *pci_dev = qat_pci_devs[qat_dev->qat_dev_id].pci_dev;
  int ret = 0;
  uint16_t desc_size = (dir == ADF_RING_DIR_TX ? qp_conf->hw->tx_msg_size
  ...
  queue->hw_queue_number = (dir == ADF_RING_DIR_TX ? qp_conf->hw->tx_ring_num
   ...
   */
  snprintf(queue->memz_name, sizeof(queue->memz_name), "%s_%d_%s_%s_%d_%d",
           pci_dev->driver->driver.name, qat_dev->qat_dev_id,
           qp_conf->service_str, "qp_mem", queue->hw_bundle_number,
  ...
  QAT_LOG(DEBUG,
          "RING: Name:%s, size in CSR: %u, in bytes %u,"
          " nb msgs %u, msg_size %u, modulo mask %u",
          queue->memz_name, queue->queue_size, queue_size_bytes,
          qp_conf->nb_descriptors, desc_size, queue->modulo_mask);
...
static const struct rte_memzone *queue_dma_zone_reserve(const char *queue_name,
  ...
  if (mz != 0) {
    if (((size_t)queue_size <= mz->len) &&
        ((socket_id == SOCKET_ID_ANY) || (socket_id == mz->socket_id))) {
      QAT_LOG(DEBUG,
    ...
    QAT_LOG(ERR,
            ...
            queue_name, (uint32_t)mz->len, mz->socket_id, queue_size,
            socket_id);
    return NULL;
  }

  QAT_LOG(DEBUG, "Allocate memzone for %s, size %u on socket %u", queue_name,
...
int qat_qp_release(enum qat_device_gen qat_dev_gen, struct qat_qp **qp_addr) {
  ...
  QAT_LOG(DEBUG, "Free qp on qat_pci device %d", qp->qat_dev->qat_dev_id);
...
static void qat_queue_delete(struct qat_queue *queue) {
  ...
  QAT_LOG(DEBUG, "Free ring %d, memzone: %s", queue->hw_queue_number,
          queue->memz_name);
  ...
  if (mz != NULL) {
    ...
    if (status != 0)
      QAT_LOG(ERR, "Error %d on freeing queue %s", status, queue->memz_name);
  } else {
    QAT_LOG(DEBUG, "queue %s doesn't exist", queue->memz_name);
  }
}

static int __rte_unused adf_queue_arb_enable(struct qat_pci_device *qat_dev,
                                             struct qat_queue *txq,
                                             void *base_addr,
                                             rte_spinlock_t *lock) {
  struct qat_qp_hw_spec_funcs *ops = qat_qp_hw_spec[qat_dev->qat_dev_gen];

  RTE_FUNC_PTR_OR_ERR_RET(ops->qat_qp_adf_arb_enable, -ENOTSUP);
...
static int adf_queue_arb_disable(enum qat_device_gen qat_dev_gen,
                                 struct qat_queue *txq, void *base_addr,
                                 rte_spinlock_t *lock) {
  struct qat_qp_hw_spec_funcs *ops = qat_qp_hw_spec[qat_dev_gen];

  RTE_FUNC_PTR_OR_ERR_RET(ops->qat_qp_adf_arb_disable, -ENOTSUP);
  ops->qat_qp_adf_arb_disable(txq, base_addr, lock);
  return 0;
}

static int __rte_unused qat_qp_build_ring_base(struct qat_pci_device *qat_dev,
                                               void *io_addr,
                                               struct qat_queue *queue) {
  struct qat_qp_hw_spec_funcs *ops = qat_qp_hw_spec[qat_dev->qat_dev_gen];

  RTE_FUNC_PTR_OR_ERR_RET(ops->qat_qp_build_ring_base, -ENOTSUP);
...
int qat_qps_per_service(struct qat_pci_device *qat_dev,
  ...
  RTE_FUNC_PTR_OR_ERR_RET(ops->qat_qp_rings_per_service, -ENOTSUP);
  return ops->qat_qp_rings_per_service(qat_dev, service);
}

const struct qat_qp_hw_data *qat_qp_get_hw_data(struct qat_pci_device *qat_dev,
                                                enum qat_service_type service,
                                                uint16_t qp_id) {
  struct qat_qp_hw_spec_funcs *ops = qat_qp_hw_spec[qat_dev->qat_dev_gen];
...
int qat_read_qp_config(struct qat_pci_device *qat_dev) {
  struct qat_dev_hw_spec_funcs *ops_hw = qat_dev_hw_spec[qat_dev->qat_dev_gen];

  RTE_FUNC_PTR_OR_ERR_RET(ops_hw->qat_dev_read_config, -ENOTSUP);
  return ops_hw->qat_dev_read_config(qat_dev);
}

static int __rte_unused adf_configure_queues(struct qat_qp *qp,
                                             enum qat_device_gen qat_dev_gen) {
  struct qat_qp_hw_spec_funcs *ops = qat_qp_hw_spec[qat_dev_gen];

  RTE_FUNC_PTR_OR_ERR_RET(ops->qat_qp_adf_configure_queues, -ENOTSUP);
  ops->qat_qp_adf_configure_queues(qp);
  return 0;
}

static inline void txq_write_tail(enum qat_device_gen qat_dev_gen,
...
static inline void qat_qp_csr_write_head(enum qat_device_gen qat_dev_gen,
                                         struct qat_qp *qp, struct qat_queue *q,
                                         uint32_t new_head) {
  struct qat_qp_hw_spec_funcs *ops = qat_qp_hw_spec[qat_dev_gen];
...
static int qat_qp_csr_setup(struct qat_pci_device *qat_dev, void *io_addr,
  ...
  RTE_FUNC_PTR_OR_ERR_RET(ops->qat_qp_csr_setup, -ENOTSUP);
  ops->qat_qp_csr_setup(qat_dev, io_addr, qp);
  return 0;
}

static inline void rxq_free_desc(enum qat_device_gen qat_dev_gen,
                                 struct qat_qp *qp, struct qat_queue *q) {
...
static int adf_verify_queue_size(uint32_t msg_size, uint32_t msg_num,
  ...
  for (; i <= ADF_MAX_RING_SIZE; i++)
    if ((msg_size * msg_num) == (uint32_t)ADF_SIZE_TO_RING_SIZE_IN_BYTES(i)) {
...
uint16_t qat_enqueue_op_burst(void *qp, qat_op_build_request_t op_build_request,
  ...
  /* Find how many can actually fit on the ring */
  {
     ...
     */
    uint32_t inflights = tmp_qp->enqueued - tmp_qp->dequeued;
     ...
     */
    if (tmp_qp->min_enq_burst_threshold &&
        inflights > QAT_QP_MIN_INFL_THRESHOLD &&
        nb_ops_possible < tmp_qp->min_enq_burst_threshold) {
 ...
 */
uint16_t qat_enqueue_comp_op_burst(void *qp, void **ops, uint16_t nb_ops) {
  register struct qat_queue *queue;
  struct qat_qp *tmp_qp = (struct qat_qp *)qp;
  register uint32_t nb_ops_sent = 0;
  ...
  nb_remaining_descriptors =
      nb_ops_possible + ((overflow >= 0) ? 0 : overflow * (-1));
  QAT_DP_LOG(DEBUG, "Nb ops requested %d, nb descriptors remaining %d", nb_ops,
             nb_remaining_descriptors);

  while (nb_ops_sent != nb_ops_possible && nb_remaining_descriptors > 0) {
    ...
    nb_desc_to_build = qat_comp_build_request(*ops, base_addr + tail, cookie,
                                              tmp_qp->qat_dev_gen);
    QAT_DP_LOG(DEBUG,
               "%d descriptors built, %d remaining, "
               "%d ops sent, %d descriptors needed",
               total_descriptors_built, nb_remaining_descriptors, nb_ops_sent,
               nb_desc_to_build);
    ...
    if (unlikely(nb_desc_to_build < 0)) {
      ...
      QAT_DP_LOG(DEBUG, "Build %d descriptors for this op", nb_desc_to_build);
      ...
      if (nb_remaining_descriptors >= 0) {
         ...
         * so retry
         */
        int ret2 = qat_comp_build_multiple_requests(*ops, tmp_qp, tail,
                                                    nb_desc_to_build);

        if (unlikely(ret2 < 1)) {
          QAT_DP_LOG(DEBUG, "Failed to build (%d) descriptors, status %d",
                     nb_desc_to_build, ret2);

          qat_comp_free_split_op_memzones(cookie, nb_desc_to_build - 1);
          ...
          total_descriptors_built += descriptors_built;
          nb_remaining_descriptors -= descriptors_built;
          QAT_DP_LOG(DEBUG, "Multiple descriptors (%d) built ok",
                     descriptors_built);
        }
      } else {
        QAT_DP_LOG(ERR,
                   "For the current op, number of requested descriptors (%d) "
...
uint16_t qat_dequeue_op_burst(void *qp, void **ops,
  ...
  while (*(uint32_t *)resp_msg != ADF_RING_EMPTY_SIG &&
    ...
    head = adf_modulo(head + rx_queue->msg_size, rx_queue->modulo_mask);
 ...
 */
static uint8_t qat_cq_dequeue_response(struct qat_qp *qp, void *out_data) {
  uint8_t result = 0;
  uint8_t retries = 0;
  struct qat_queue *queue = &(qp->rx_q);
  struct icp_qat_fw_comn_resp *resp_msg =
      (struct icp_qat_fw_comn_resp *)((uint8_t *)queue->base_addr +
                                      queue->head);
  ...
  if (*(uint32_t *)resp_msg != ADF_RING_EMPTY_SIG) {
    ...
    /* check status flag */
    if (ICP_QAT_FW_COMN_RESP_CRYPTO_STAT_GET(resp_msg->comn_hdr.comn_status) ==
    ...
    queue->head = adf_modulo(queue->head + queue->msg_size, queue->modulo_mask);
 ...
 */
int qat_cq_get_fw_version(struct qat_qp *qp) {
  ...
  queue->tail = adf_modulo(queue->tail + queue->msg_size, queue->modulo_mask);
...
#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
    QAT_DP_HEXDUMP_LOG(DEBUG, "NULL response:", &response, sizeof(response));
#endif
