...
const char *qat_service_get_str(enum qat_service_type type) {
  switch (type) {
...
int qat_sgl_fill_array(struct rte_mbuf *buf, int64_t offset, void *list_in,
                       uint32_t data_len, const uint16_t max_segs) {
  int res = -EINVAL;
  uint32_t buf_len, nr;
  ...
  if (unlikely(res != 0)) {
    if (nr == max_segs) {
      QAT_DP_LOG(ERR, "Exceeded max segments in QAT SGL (%u)", max_segs);
    } else {
    ...
    for (nr = 0; nr < list->num_bufs; nr++) {
      QAT_DP_LOG(INFO, "QAT SGL buf %d, len = %d, iova = 0x%012" PRIx64, nr,
                 list->buffers[nr].len, list->buffers[nr].addr);
      QAT_DP_HEXDUMP_LOG(DEBUG, "qat SGL", virt_addr[nr],
                         list->buffers[nr].len);
    }
#endif
  }
...
void qat_stats_get(struct qat_pci_device *dev, struct qat_common_stats *stats,
                   enum qat_service_type service) {
  int i;
  struct qat_qp **qp;

  if (stats == NULL || dev == NULL || service >= QAT_SERVICE_INVALID) {
    QAT_LOG(ERR, "invalid param: stats %p, dev %p, service %d", stats, dev,
            service);
    return;
  }
  ...
  for (i = 0; i < ADF_MAX_QPS_ON_ANY_SERVICE; i++) {
    if (qp[i] == NULL) {
      QAT_LOG(DEBUG, "Service %d Uninitialised qp %d", service, i);
      continue;
    }
