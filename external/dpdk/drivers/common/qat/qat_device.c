...
static int qat_pci_get_extra_size(enum qat_device_gen qat_dev_gen) {
  struct qat_dev_hw_spec_funcs *ops_hw = qat_dev_hw_spec[qat_dev_gen];
  RTE_FUNC_PTR_OR_ERR_RET(ops_hw->qat_dev_get_extra_size, -ENOTSUP);
  return ops_hw->qat_dev_get_extra_size();
}

static struct qat_pci_device *qat_pci_get_named_dev(const char *name) {
  unsigned int i;
  ...
  for (i = 0; i < RTE_PMD_QAT_MAX_PCI_DEVICES; i++) {
    if (qat_pci_devs[i].mz &&
        (strcmp(((struct qat_pci_device *)qat_pci_devs[i].mz->addr)->name,
                name) == 0))
      return (struct qat_pci_device *)qat_pci_devs[i].mz->addr;
  }

  return NULL;
}

static uint8_t qat_pci_find_free_device_index(void) {
  uint8_t dev_id;
...
static void qat_dev_parse_cmd(const char *str,
...
qat_pci_device_allocate(struct rte_pci_device *pci_dev,
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ...
    if (mz == NULL) {
      QAT_LOG(ERR, "Secondary can't find %s mz, did primary create device?",
              name);
      return NULL;
    }
    qat_dev = mz->addr;
  ...
  if (qat_pci_get_named_dev(name) != NULL) {
    QAT_LOG(ERR, "QAT device with name %s already allocated!", name);
    return NULL;
  }
  ...
  if (extra_size < 0) {
    QAT_LOG(ERR, "QAT internal error: no pci pointer for gen %d", qat_dev_gen);
    return NULL;
  }

  qat_dev_size = sizeof(struct qat_pci_device) + extra_size;
  qat_dev_mz = rte_memzone_reserve(name, qat_dev_size, rte_socket_id(), 0);

  if (qat_dev_mz == NULL) {
    QAT_LOG(ERR, "Error when allocating memzone for QAT_%d", qat_dev_id);
    return NULL;
  }
  ...
  if (qat_read_qp_config(qat_dev)) {
    QAT_LOG(ERR, "Cannot acquire ring configuration for QAT_%d", qat_dev_id);
    rte_memzone_free(qat_dev_mz);
    return NULL;
  }
...
static int qat_pci_device_release(struct rte_pci_device *pci_dev) {
  struct qat_pci_device *qat_dev;
  char name[QAT_DEV_NAME_MAX_LEN];
  int busy = 0;
  ...
  if (qat_dev != NULL) {
    ...
    struct qat_device_info *inst = &qat_pci_devs[qat_dev->qat_dev_id];
    ...
    qat_nb_pci_devices--;
    QAT_LOG(DEBUG, "QAT device %s released, total QATs %d", name,
            qat_nb_pci_devices);
  }
  return 0;
}

static int qat_pci_dev_destroy(struct qat_pci_device *qat_pci_dev,
...
static int qat_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  QAT_LOG(DEBUG, "Found QAT device at %02x:%02x.%x", pci_dev->addr.bus,
  ...
  RTE_FUNC_PTR_OR_ERR_RET(ops_hw->qat_dev_reset_ring_pairs, -ENOTSUP);
  ...
  if (sym_ret == 0) {
    ...
    QAT_LOG(WARNING, "Failed to create QAT SYM PMD on device %s",
            qat_pci_dev->name);
  ...
  if (comp_ret == 0)
    ...
    QAT_LOG(WARNING, "Failed to create QAT COMP PMD on device %s",
            qat_pci_dev->name);
  ...
  if (asym_ret == 0)
    ...
    QAT_LOG(WARNING, "Failed to create QAT ASYM PMD on device %s",
            qat_pci_dev->name);
...
static struct rte_pci_driver rte_qat_pmd = {.id_table = pci_id_qat_map,
                                            .drv_flags =
...
RTE_PMD_REGISTER_KMOD_DEP(QAT_PCI_NAME,
