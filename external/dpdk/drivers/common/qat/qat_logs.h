...
#define QAT_LOG(level, fmt, args...)                                           \
  rte_log(RTE_LOG_##level, qat_gen_logtype, "%s(): " fmt "\n", __func__, ##args)

#define QAT_DP_LOG(level, fmt, args...)                                        \
  rte_log(RTE_LOG_##level, qat_dp_logtype, "%s(): " fmt "\n", __func__, ##args)
 ...
 */
int qat_hexdump_log(uint32_t level, uint32_t logtype, const char *title,
