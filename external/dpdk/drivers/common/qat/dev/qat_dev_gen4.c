...
static int qat_query_svc_gen4(struct qat_pci_device *qat_dev, uint8_t *val) {
...
static int qat_select_valid_queue_gen4(struct qat_pci_device *qat_dev,
                                       int qp_id,
  ...
  for (; i < QAT_GEN4_BUNDLE_NUM; i++) {
    if (dev_extra->qp_gen4_data[i][0].service_type == service_type) {
      if (valid_qps == qp_id)
        return i;
      ++valid_qps;
    }
...
qat_qp_get_hw_data_gen4(struct qat_pci_device *qat_dev,
  ...
  struct qat_dev_gen4_extra *dev_extra = qat_dev->dev_private;
  int ring_pair = qat_select_valid_queue_gen4(qat_dev, qp_id, service_type);
...
static int qat_qp_rings_per_service_gen4(struct qat_pci_device *qat_dev,
...
static enum qat_service_type gen4_pick_service(uint8_t hw_service) {
  switch (hw_service) {
  case QAT_SVC_SYM:
...
static int qat_dev_read_config_gen4(struct qat_pci_device *qat_dev) {
  int i = 0;
  uint16_t svc = 0;
  struct qat_dev_gen4_extra *dev_extra = qat_dev->dev_private;
  ...
  for (; i < QAT_GEN4_BUNDLE_NUM; i++) {
    ...
    if (service_type == QAT_SERVICE_INVALID) {
      QAT_LOG(ERR, "Unrecognized service on bundle %d", i);
      return -ENOTSUP;
    }
...
static void qat_qp_build_ring_base_gen4(void *io_addr,
...
static void qat_qp_adf_arb_enable_gen4(const struct qat_queue *txq,
                                       void *base_addr, rte_spinlock_t *lock) {
  uint32_t arb_csr_offset = 0, value;

  rte_spinlock_lock(lock);
  arb_csr_offset = ADF_ARB_RINGSRVARBEN_OFFSET +
                   (ADF_RING_BUNDLE_SIZE_GEN4 * txq->hw_bundle_number);
  value =
      ADF_CSR_RD(base_addr + ADF_RING_CSR_ADDR_OFFSET_GEN4VF, arb_csr_offset);
  value |= (0x01 << txq->hw_queue_number);
  ADF_CSR_WR(base_addr, arb_csr_offset, value);
  rte_spinlock_unlock(lock);
}

static void qat_qp_adf_arb_disable_gen4(const struct qat_queue *txq,
                                        void *base_addr, rte_spinlock_t *lock) {
  uint32_t arb_csr_offset = 0, value;
...
static void qat_qp_adf_configure_queues_gen4(struct qat_qp *qp) {
  ...
  q_resp_config = BUILD_RESP_RING_CONFIG(
      q_rx->queue_size, ADF_RING_NEAR_WATERMARK_512, ADF_RING_NEAR_WATERMARK_0);

  WRITE_CSR_RING_CONFIG_GEN4VF(qp->mmap_bar_addr, q_tx->hw_bundle_number,
                               q_tx->hw_queue_number, q_tx_config);
  WRITE_CSR_RING_CONFIG_GEN4VF(qp->mmap_bar_addr, q_rx->hw_bundle_number,
                               q_rx->hw_queue_number, q_resp_config);
}

static void qat_qp_csr_write_tail_gen4(struct qat_qp *qp, struct qat_queue *q) {
  WRITE_CSR_RING_TAIL_GEN4VF(qp->mmap_bar_addr, q->hw_bundle_number,
                             q->hw_queue_number, q->tail);
}

static void qat_qp_csr_write_head_gen4(struct qat_qp *qp, struct qat_queue *q,
                                       uint32_t new_head) {
  WRITE_CSR_RING_HEAD_GEN4VF(qp->mmap_bar_addr, q->hw_bundle_number,
                             q->hw_queue_number, new_head);
}

static void qat_qp_csr_setup_gen4(struct qat_pci_device *qat_dev, void *io_addr,
                                  struct qat_qp *qp) {
...
static int qat_reset_ring_pairs_gen4(struct qat_pci_device *qat_pci_dev) {
  int ret = 0, i;
  uint8_t data[4];
  struct qat_pf2vf_msg pf2vf_msg;
  ...
  for (i = 0; i < QAT_GEN4_BUNDLE_NUM; i++) {
    ...
    if (ret) {
      QAT_LOG(ERR, "QAT error when reset bundle no %d", i);
      return ret;
    }
  }
...
static int qat_dev_get_misc_bar_gen4(struct rte_mem_resource **mem_resource,
                                     struct rte_pci_device *pci_dev) {
  *mem_resource = &pci_dev->mem_resource[2];
  return 0;
}

static int qat_dev_get_extra_size_gen4(void) {
