...
static void sfc_efx_mcdi_timeout(struct sfc_efx_mcdi *mcdi) {
...
static void sfc_efx_mcdi_poll(struct sfc_efx_mcdi *mcdi, boolean_t proxy) {
  efx_nic_t *enp;
  ...
  do {
    ...
    poll_completed = (proxy) ? sfc_efx_mcdi_proxy_event_available(mcdi)
    ...
    RTE_BUILD_BUG_ON(SFC_EFX_MCDI_POLL_INTERVAL_MAX_US > UINT_MAX / 2);
    delay_us *= 2;
...
static void sfc_efx_mcdi_execute(void *arg, efx_mcdi_req_t *emrp) {
  ...
  if (efx_mcdi_get_proxy_handle(mcdi->nic, emrp, &proxy_handle) == 0) {
    ...
    if ((mcdi->proxy_handle != 0) && (mcdi->proxy_handle != proxy_handle)) {
      ...
      sfc_efx_mcdi_err(
          mcdi, "MCDI proxy authorization failed (handle=%08x, result=%d)",
...
static void sfc_efx_mcdi_ev_cpl(void *arg) {
  struct sfc_efx_mcdi *mcdi = (struct sfc_efx_mcdi *)arg;
...
static void sfc_efx_mcdi_exception(void *arg, efx_mcdi_exception_t eme) {
  struct sfc_efx_mcdi *mcdi = (struct sfc_efx_mcdi *)arg;

  sfc_efx_mcdi_warn(mcdi, "MC %s",
                    (eme == EFX_MCDI_EXCEPTION_MC_REBOOT)      ? "REBOOT"
                    : (eme == EFX_MCDI_EXCEPTION_MC_BADASSERT) ? "BADASSERT"
                                                               : "UNKNOWN");
...
static size_t sfc_efx_mcdi_do_log(const struct sfc_efx_mcdi *mcdi, char *buffer,
                                  void *data, size_t data_size, size_t pfxsize,
                                  size_t position) {
  uint32_t *words = data;
  ...
  for (i = 0; i < data_size; i += sizeof(*words)) {
    if (position + word_str_space >= SFC_MCDI_LOG_BUF_SIZE) {
    ...
    position += snprintf(buffer + position, SFC_MCDI_LOG_BUF_SIZE - position,
                         " %08x", *words);
    words++;
  }
  return position;
...
static void sfc_efx_mcdi_logger(void *arg, efx_log_msg_t type, void *header,
  ...
  pfxsize = snprintf(buffer, sizeof(buffer), "MCDI RPC %s:",
                     ...
                     : type == EFX_LOG_MCDI_RESPONSE ? "RESP"
                                                     : "???");
  start =
      sfc_efx_mcdi_do_log(mcdi, buffer, header, header_size, pfxsize, pfxsize);
  start = sfc_efx_mcdi_do_log(mcdi, buffer, data, data_size, pfxsize, start);
  if (start != pfxsize) {
    buffer[start] = '\0';
    sfc_efx_log_mcdi(mcdi, "%s", buffer);
  }
...
static void sfc_efx_mcdi_ev_proxy_response(void *arg, uint32_t handle,
                                           efx_rc_t result) {
  struct sfc_efx_mcdi *mcdi = (struct sfc_efx_mcdi *)arg;
...
int sfc_efx_mcdi_init(struct sfc_efx_mcdi *mcdi, uint32_t logtype,
                      const char *log_prefix, efx_nic_t *nic,
