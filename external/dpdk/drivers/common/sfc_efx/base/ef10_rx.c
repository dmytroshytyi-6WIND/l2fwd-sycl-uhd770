...
static __checkReturn efx_rc_t efx_mcdi_rss_context_alloc(
  ...
  if (encp->enc_rx_scale_tbl_entry_count_is_selectable != B_FALSE) {
    MCDI_IN_SET_DWORD(req, RSS_CONTEXT_ALLOC_V2_IN_INDIRECTION_TABLE_SIZE,
                      table_nentries);
  }
...
#if EFSYS_OPT_RX_SCALE
static efx_rc_t efx_mcdi_rss_context_free(__in efx_nic_t *enp,
...
#if EFSYS_OPT_RX_SCALE
static efx_rc_t efx_mcdi_rss_context_set_flags(__in efx_nic_t *enp,
                                               __in uint32_t rss_context,
  ...
  MCDI_IN_SET_DWORD(req, RSS_CONTEXT_SET_FLAGS_IN_RSS_CONTEXT_ID, rss_context);
  ...
  MCDI_IN_POPULATE_DWORD_10(
      ...
      (type >> EFX_RX_CLASS_IPV4_TCP_LBN) & EFX_MASK32(EFX_RX_CLASS_IPV4_TCP),
      RSS_CONTEXT_SET_FLAGS_IN_UDP_IPV4_RSS_MODE,
      (type >> EFX_RX_CLASS_IPV4_UDP_LBN) & EFX_MASK32(EFX_RX_CLASS_IPV4_UDP),
      ...
      (type >> EFX_RX_CLASS_IPV6_TCP_LBN) & EFX_MASK32(EFX_RX_CLASS_IPV6_TCP),
      RSS_CONTEXT_SET_FLAGS_IN_UDP_IPV6_RSS_MODE,
      (type >> EFX_RX_CLASS_IPV6_UDP_LBN) & EFX_MASK32(EFX_RX_CLASS_IPV6_UDP),
...
#if EFSYS_OPT_RX_SCALE
static efx_rc_t efx_mcdi_rss_context_set_key(__in efx_nic_t *enp,
                                             __in uint32_t rss_context,
  ...
  MCDI_IN_SET_DWORD(req, RSS_CONTEXT_SET_KEY_IN_RSS_CONTEXT_ID, rss_context);
  ...
  memcpy(MCDI_IN2(req, uint8_t, RSS_CONTEXT_SET_KEY_IN_TOEPLITZ_KEY), key, n);
...
#if EFSYS_OPT_RX_SCALE
static efx_rc_t
efx_mcdi_rss_context_set_table(__in efx_nic_t *enp, __in uint32_t rss_context,
  ...
  MCDI_IN_SET_DWORD(req, RSS_CONTEXT_SET_TABLE_IN_RSS_CONTEXT_ID, rss_context);

  req_table =
      MCDI_IN2(req, uint8_t, RSS_CONTEXT_SET_TABLE_IN_INDIRECTION_TABLE);

  for (i = 0; i < MC_CMD_RSS_CONTEXT_SET_TABLE_IN_INDIRECTION_TABLE_LEN; i++) {
    req_table[i] = (nentries > 0) ? (uint8_t)table[i % nentries] : 0;
  }
...
#if EFSYS_OPT_RX_SCALE
static __checkReturn efx_rc_t efx_mcdi_rss_context_write_table(
    __in efx_nic_t *enp, __in uint32_t context, __in unsigned int start_idx,
    __in_ecount(nentries) unsigned int *table, __in unsigned int nentries) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_RSS_CONTEXT_WRITE_TABLE_IN_LENMAX_MCDI2,
                       MC_CMD_RSS_CONTEXT_WRITE_TABLE_OUT_LEN);
  unsigned int i;
  int rc;

  if (nentries > MC_CMD_RSS_CONTEXT_WRITE_TABLE_IN_ENTRIES_MAXNUM_MCDI2) {
    rc = EINVAL;
    goto fail1;
  }

  if (start_idx + nentries > encp->enc_rx_scale_tbl_max_nentries) {
    rc = EINVAL;
    goto fail2;
  }
  ...
  MCDI_IN_SET_DWORD(req, RSS_CONTEXT_WRITE_TABLE_IN_RSS_CONTEXT_ID, context);
  ...
  for (i = 0; i < nentries; ++i) {
    ...
    MCDI_IN_POPULATE_INDEXED_DWORD_2(
        req, RSS_CONTEXT_WRITE_TABLE_IN_ENTRIES, i,
...
__checkReturn efx_rc_t ef10_rx_init(__in efx_nic_t *enp) {
#if EFSYS_OPT_RX_SCALE
...
__checkReturn efx_rc_t ef10_rx_scatter_enable(__in efx_nic_t *enp,
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t ef10_rx_scale_context_alloc(
    __in efx_nic_t *enp, __in efx_rx_scale_context_type_t type,
    __in uint32_t num_queues, __in uint32_t table_nentries,
    __out uint32_t *rss_contextp) {
  efx_rc_t rc;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t ef10_rx_scale_context_free(__in efx_nic_t *enp,
                                                  __in uint32_t rss_context) {
  efx_rc_t rc;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t ef10_rx_scale_mode_set(__in efx_nic_t *enp,
                                              __in uint32_t rss_context,
  ...
  if ((rc = efx_mcdi_rss_context_set_flags(enp, rss_context, type)) != 0)
    goto fail3;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t ef10_rx_scale_key_set(__in efx_nic_t *enp,
                                             __in uint32_t rss_context,
                                             __in_ecount(n) uint8_t *key,
                                             __in size_t n) {
  efx_rc_t rc;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t ef10_rx_scale_tbl_set(
  ...
  if (encp->enc_rx_scale_tbl_entry_count_is_selectable != B_FALSE) {
    ...
    for (remain = nentries; remain > 0; remain -= batch) {
      ...
      rc = efx_mcdi_rss_context_write_table(enp, rss_context, index,
                                            &table[index], batch);
      if (rc != 0)
        goto fail2;
    ...
    rc = efx_mcdi_rss_context_set_table(enp, rss_context, table, nentries);
    if (rc != 0)
      goto fail3;
  }
...
static const efx_rx_prefix_layout_t ef10_default_rx_prefix_layout = {
    ...
    .erpl_length = 14,
    .erpl_fields = {
        [EFX_RX_PREFIX_FIELD_RSS_HASH] = {0, 32, B_FALSE},
        [EFX_RX_PREFIX_FIELD_VLAN_STRIP_TCI] = {32, 16, B_TRUE},
        [EFX_RX_PREFIX_FIELD_INNER_VLAN_STRIP_TCI] = {48, 16, B_TRUE},
        [EFX_RX_PREFIX_FIELD_LENGTH] = {64, 16, B_FALSE},
        [EFX_RX_PREFIX_FIELD_PARTIAL_TSTAMP] = {80, 32, B_FALSE},
    }};
...
__checkReturn efx_rc_t ef10_rx_prefix_pktlen(__in efx_nic_t *enp,
                                             __in uint8_t *buffer,
...
#if EFSYS_OPT_RX_SCALE
__checkReturn uint32_t ef10_rx_prefix_hash(__in efx_nic_t *enp,
...
void ef10_rx_qpost(__in efx_rxq_t *erp,
  ...
  for (i = 0; i < ndescs; i++) {
    ...
    EFX_POPULATE_QWORD_3(
        qword, ESF_DZ_RX_KER_BYTE_CNT, (uint32_t)(size),
        ESF_DZ_RX_KER_BUF_ADDR_DW0, (uint32_t)(addrp[i] & 0xffffffff),
        ESF_DZ_RX_KER_BUF_ADDR_DW1, (uint32_t)(addrp[i] >> 32));
...
void ef10_rx_qpush(__in efx_rxq_t *erp, __in unsigned int added,
  ...
  *pushedp = wptr;
  ...
  EFX_DMA_SYNC_QUEUE_FOR_DEVICE(erp->er_esmp, erp->er_mask + 1,
                                EF10_RXQ_DESC_SIZE, wptr,
                                pushed & erp->er_mask);
  EFSYS_PIO_WRITE_BARRIER();
  EFX_BAR_VI_WRITED(enp, ER_DZ_RX_DESC_UPD_REG, erp->er_index, &dword, B_FALSE);
}

#if EFSYS_OPT_RX_PACKED_STREAM

void ef10_rx_qpush_ps_credits(__in efx_rxq_t *erp) {
  efx_nic_t *enp = erp->er_enp;
  efx_dword_t dword;
  ...
  EFX_POPULATE_DWORD_3(
      dword, ERF_DZ_RX_DESC_MAGIC_DOORBELL, 1, ERF_DZ_RX_DESC_MAGIC_CMD,
      ERE_DZ_RX_DESC_MAGIC_CMD_PS_CREDITS, ERF_DZ_RX_DESC_MAGIC_DATA, credits);
  EFX_BAR_VI_WRITED(enp, ER_DZ_RX_DESC_UPD_REG, erp->er_index, &dword, B_FALSE);
 ...
 */
__checkReturn uint8_t *ef10_rx_qps_packet_info(
  ...
  *lengthp = EFX_QWORD_FIELD(*qwordp, ES_DZ_PS_RX_PREFIX_ORIG_LEN);
  ...
  buf_len =
      EFX_P2ROUNDUP(uint16_t, buf_len + EFX_RX_PACKED_STREAM_RX_PREFIX_SIZE,
                    EFX_RX_PACKED_STREAM_ALIGNMENT);
  *next_offsetp = current_offset + buf_len + EFX_RX_PACKED_STREAM_ALIGNMENT;

  EFSYS_ASSERT3U(*next_offsetp, <=, buffer_length);
  EFSYS_ASSERT3U(current_offset + *lengthp, <, *next_offsetp);

  if ((*next_offsetp ^ current_offset) & EFX_RX_PACKED_STREAM_MEM_PER_CREDIT)
...
__checkReturn efx_rc_t ef10_rx_qflush(__in efx_rxq_t *erp) {
  efx_nic_t *enp = erp->er_enp;
  efx_rc_t rc;
...
void ef10_rx_qenable(__in efx_rxq_t *erp){
    /* FIXME */
    _NOTE(ARGUNUSED(erp))
    /* FIXME */
}
...
__checkReturn efx_rc_t
    ef10_rx_qcreate(__in efx_nic_t *enp, __in unsigned int index,
                    __in unsigned int label, __in efx_rxq_type_t type,
                    __in_opt const efx_rxq_type_data_t *type_data,
                    __in efsys_mem_t *esmp, __in size_t ndescs,
                    __in uint32_t id, __in unsigned int flags,
  ...
  case EFX_RXQ_TYPE_ES_SUPER_BUFFER:
    ...
    params.es_max_dma_len = type_data->ertd_es_super_buffer.eessb_max_dma_len;
  ...
  if ((rc = efx_mcdi_init_rxq(enp, ndescs, eep, label, index, esmp, &params)) !=
      0)
    goto fail17;
...
void ef10_rx_qdestroy(__in efx_rxq_t *erp) {
...
void ef10_rx_fini(__in efx_nic_t *enp) {
#if EFSYS_OPT_RX_SCALE
  if (enp->en_rss_context_type != EFX_RX_SCALE_UNAVAILABLE)
