...
__checkReturn efx_rc_t efx_vpd_init(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t efx_vpd_size(__in efx_nic_t *enp, __out size_t *sizep) {
...
__checkReturn efx_rc_t efx_vpd_read(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_vpd_verify(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_vpd_reinit(__in efx_nic_t *enp,
                                      __in_bcount(size) caddr_t data,
                                      __in size_t size) {
  const efx_vpd_ops_t *evpdop = enp->en_evpdop;
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_vpd_get(__in efx_nic_t *enp,
                                   __in_bcount(size) caddr_t data,
                                   __in size_t size,
...
__checkReturn efx_rc_t efx_vpd_set(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_vpd_next(__in efx_nic_t *enp,
                                    __inout_bcount(size) caddr_t data,
                                    __in size_t size,
...
__checkReturn efx_rc_t efx_vpd_write(__in efx_nic_t *enp,
                                     __in_bcount(size) caddr_t data,
                                     __in size_t size) {
  const efx_vpd_ops_t *evpdop = enp->en_evpdop;
  efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_vpd_next_tag(__in caddr_t data,
                                               __in size_t size,
  ...
  case TAG_TYPE_LARGE_ITEM_DECODE:
    ...
    EFX_POPULATE_WORD_2(word, EFX_BYTE_0, data[*offsetp + 1], EFX_BYTE_1,
                        data[*offsetp + 2]);
...
static __checkReturn efx_rc_t efx_vpd_next_keyword(
    __in_bcount(size) caddr_t tag, __in size_t size, __in unsigned int pos,
    __out efx_vpd_keyword_t *keywordp, __out uint8_t *lengthp) {
...
__checkReturn efx_rc_t efx_vpd_hunk_length(__in_bcount(size) caddr_t data,
                                           __in size_t size,
  ...
  while (1) {
    if ((rc = efx_vpd_next_tag(data, size, &offset, &tag, &taglen)) != 0)
      goto fail1;
    offset += taglen;
    if (tag == EFX_VPD_END)
      break;
...
__checkReturn efx_rc_t efx_vpd_hunk_verify(__in_bcount(size) caddr_t data,
                                           __in size_t size,
  ...
  while (1) {
    ...
    for (pos = 0; pos != taglen; pos += 3 + keylen) {
      ...
      if ((rc = efx_vpd_next_keyword(data + offset, taglen, pos, &keyword,
                                     &keylen)) != 0)
        goto fail3;
...
__checkReturn efx_rc_t efx_vpd_hunk_reinit(__in_bcount(size) caddr_t data,
                                           __in size_t size,
  ...
  if (wantpid) {
    memcpy(data + offset, __efx_vpd_blank_pid, sizeof(__efx_vpd_blank_pid));
...
__checkReturn efx_rc_t efx_vpd_hunk_next(
    __in_bcount(size) caddr_t data, __in size_t size, __out efx_vpd_tag_t *tagp,
    __out efx_vpd_keyword_t *keywordp, __out_opt unsigned int *payloadp,
...
__checkReturn efx_rc_t efx_vpd_hunk_get(__in_bcount(size) caddr_t data,
                                        __in size_t size,
  ...
  _NOTE(CONSTANTCONDITION)
  while (1) {
    if ((rc = efx_vpd_next_tag(data, size, &offset, &itag, &taglen)) != 0)
      goto fail1;
    if (itag == EFX_VPD_END)
      break;
    ...
    if (itag == tag) {
      ...
      for (pos = 0; pos != taglen; pos += 3 + keylen) {
        if ((rc = efx_vpd_next_keyword(data + offset, taglen, pos, &ikeyword,
                                       &keylen)) != 0)
          goto fail2;
...
__checkReturn efx_rc_t efx_vpd_hunk_set(__in_bcount(size) caddr_t data,
                                        __in size_t size,
                                        __in efx_vpd_value_t *evvp) {
  efx_word_t word;
  efx_vpd_tag_t tag;
  ...
  while (1) {
    ...
    for (pos = 0; pos != taglen; pos += 3 + keylen) {
      ...
      if (keyword == evvp->evv_keyword && evvp->evv_length == 0) {
  ...
  check_space:
    ...
    /* Copy contents */
    memcpy(data + dest - evvp->evv_length, evvp->evv_value, evvp->evv_length);

    /* Insert new keyword header if required */
    if (tag != EFX_VPD_ID && evvp->evv_length > 0) {
      EFX_POPULATE_WORD_1(word, EFX_WORD_0, evvp->evv_keyword);
      data[offset + pos + 0] = EFX_WORD_FIELD(word, EFX_BYTE_0);
      data[offset + pos + 1] = EFX_WORD_FIELD(word, EFX_BYTE_1);
...
void efx_vpd_fini(__in efx_nic_t *enp) {
  const efx_vpd_ops_t *evpdop = enp->en_evpdop;
