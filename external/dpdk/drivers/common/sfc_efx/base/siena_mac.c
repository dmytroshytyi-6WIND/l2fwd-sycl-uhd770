...
__checkReturn efx_rc_t siena_mac_poll(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t siena_mac_up(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t siena_mac_reconfigure(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
  efx_oword_t multicast_hash[2];
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(
      payload, MAX(MC_CMD_SET_MAC_IN_LEN, MC_CMD_SET_MCAST_HASH_IN_LEN),
  ...
  MCDI_IN_POPULATE_DWORD_2(req, SET_MAC_IN_REJECT, SET_MAC_IN_REJECT_UNCST,
                           !epp->ep_all_unicst, SET_MAC_IN_REJECT_BRDCST,
                           !epp->ep_brdcst);
  ...
  if (epp->ep_fcntl_autoneg)
    ...
    fcntl = (epp->ep_fcntl & EFX_FCNTL_GENERATE) ? MC_CMD_FCNTL_BIDIR
...
__checkReturn efx_rc_t siena_mac_stats_get_mask(__in efx_nic_t *enp,
...
siena_mac_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
  ...
  SIENA_MAC_STAT_READ(esmp, (encp->enc_mac_stats_nstats - 1), &generation_end);
  ...
  SIENA_MAC_STAT_READ(esmp, MC_CMD_MAC_TX_MULTIPLE_COLLISION_PKTS, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_TX_MULT_COL_PKTS]), &value);

  SIENA_MAC_STAT_READ(esmp, MC_CMD_MAC_TX_EXCESSIVE_COLLISION_PKTS, &value);
  ...
  SIENA_MAC_STAT_READ(esmp, MC_CMD_MAC_TX_EXCESSIVE_DEFERRED_PKTS, &value);
  ...
  EFSYS_DMA_SYNC_FOR_KERNEL(esmp, 0, EFSYS_MEM_SIZE(esmp));
  EFSYS_MEM_READ_BARRIER();
  SIENA_MAC_STAT_READ(esmp, MC_CMD_MAC_GENERATION_START, &generation_start);
...
__checkReturn efx_rc_t siena_mac_pdu_get(__in efx_nic_t *enp,
                                         __out size_t *pdu) {
  return (ENOTSUP);
}
