...
static __checkReturn efx_rc_t siena_rx_init(__in efx_nic_t *enp);

static void siena_rx_fini(__in efx_nic_t *enp);

#if EFSYS_OPT_RX_SCATTER
static __checkReturn efx_rc_t
siena_rx_scatter_enable(__in efx_nic_t *enp, __in unsigned int buf_size);
#endif /* EFSYS_OPT_RX_SCATTER */

#if EFSYS_OPT_RX_SCALE
static __checkReturn efx_rc_t siena_rx_scale_mode_set(
...
static __checkReturn efx_rc_t siena_rx_scale_key_set(__in efx_nic_t *enp,
                                                     __in uint32_t rss_context,
                                                     __in_ecount(n)
                                                         uint8_t *key,
                                                     __in size_t n);

static __checkReturn efx_rc_t siena_rx_scale_tbl_set(
    __in efx_nic_t *enp, __in uint32_t rss_context,
    __in_ecount(nentries) unsigned int *table, __in size_t nentries);

static __checkReturn uint32_t siena_rx_prefix_hash(__in efx_nic_t *enp,
                                                   __in efx_rx_hash_alg_t func,
                                                   __in uint8_t *buffer);

#endif /* EFSYS_OPT_RX_SCALE */

static __checkReturn efx_rc_t siena_rx_prefix_pktlen(__in efx_nic_t *enp,
                                                     __in uint8_t *buffer,
                                                     __out uint16_t *lengthp);

static void siena_rx_qpost(__in efx_rxq_t *erp,
                           __in_ecount(ndescs) efsys_dma_addr_t *addrp,
                           __in size_t size, __in unsigned int ndescs,
                           __in unsigned int completed,
                           __in unsigned int added);

static void siena_rx_qpush(__in efx_rxq_t *erp, __in unsigned int added,
                           __inout unsigned int *pushedp);

#if EFSYS_OPT_RX_PACKED_STREAM
static void siena_rx_qpush_ps_credits(__in efx_rxq_t *erp);

static __checkReturn uint8_t *siena_rx_qps_packet_info(
...
static __checkReturn efx_rc_t siena_rx_qflush(__in efx_rxq_t *erp);

static void siena_rx_qenable(__in efx_rxq_t *erp);

static __checkReturn efx_rc_t siena_rx_qcreate(
    __in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
    __in efx_rxq_type_t type, __in_opt const efx_rxq_type_data_t *type_data,
    __in efsys_mem_t *esmp, __in size_t ndescs, __in uint32_t id,
    __in unsigned int flags, __in efx_evq_t *eep, __in efx_rxq_t *erp);

static void siena_rx_qdestroy(__in efx_rxq_t *erp);
...
__checkReturn efx_rc_t efx_rx_init(__inout efx_nic_t *enp) {
...
void efx_rx_fini(__in efx_nic_t *enp) {
...
#if EFSYS_OPT_RX_SCATTER
__checkReturn efx_rc_t efx_rx_scatter_enable(__in efx_nic_t *enp,
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_hash_flags_get(
...
__checkReturn efx_rc_t efx_rx_scale_default_support_get(
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_context_alloc(
  ...
  if ((rc = erxop->erxo_scale_context_alloc(
           enp, type, num_queues, table_nentries, rss_contextp)) != 0) {
    goto fail2;
  }
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_context_alloc_v2(
    __in efx_nic_t *enp, __in efx_rx_scale_context_type_t type,
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_context_free(__in efx_nic_t *enp,
                                                 __in uint32_t rss_context) {
  const efx_rx_ops_t *erxop = enp->en_erxop;
  efx_rc_t rc;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_mode_set(__in efx_nic_t *enp,
                                             __in uint32_t rss_context,
   ...
   */
  if ((type & EFX_RX_HASH_LEGACY_MASK) && (type & ~EFX_RX_HASH_LEGACY_MASK)) {
    rc = EINVAL;
    goto fail1;
  }
   ...
   */
  if (encp->enc_rx_scale_additional_modes_supported == B_FALSE) {
    efx_rx_hash_type_t t_ipv4 =
        EFX_RX_HASH(IPV4, 2TUPLE) | EFX_RX_HASH(IPV4_TCP, 2TUPLE);
    efx_rx_hash_type_t t_ipv6 =
        EFX_RX_HASH(IPV6, 2TUPLE) | EFX_RX_HASH(IPV6_TCP, 2TUPLE);
  ...
  if (erxop->erxo_scale_mode_set != NULL) {
    if ((rc = erxop->erxo_scale_mode_set(enp, rss_context, alg, type,
                                         insert)) != 0)
      goto fail4;
  }
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_key_set(__in efx_nic_t *enp,
                                            __in uint32_t rss_context,
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_tbl_set(
    __in efx_nic_t *enp, __in uint32_t rss_context,
    __in_ecount(nentries) unsigned int *table, __in size_t nentries) {
  const efx_rx_ops_t *erxop = enp->en_erxop;
  efx_rc_t rc;
  ...
  if ((rc = erxop->erxo_scale_tbl_set(enp, rss_context, table, nentries)) != 0)
    goto fail1;
...
void efx_rx_qpost(__in efx_rxq_t *erp,
                  __in_ecount(ndescs) efsys_dma_addr_t *addrp, __in size_t size,
                  __in unsigned int ndescs, __in unsigned int completed,
                  __in unsigned int added) {
...
void efx_rx_qpush_ps_credits(__in efx_rxq_t *erp) {
  efx_nic_t *enp = erp->er_enp;
  const efx_rx_ops_t *erxop = enp->en_erxop;
...
__checkReturn uint8_t *efx_rx_qps_packet_info(
    __in efx_rxq_t *erp, __in uint8_t *buffer, __in uint32_t buffer_length,
    __in uint32_t current_offset, __out uint16_t *lengthp,
    __out uint32_t *next_offsetp, __out uint32_t *timestamp) {
  efx_nic_t *enp = erp->er_enp;
  ...
  return (erxop->erxo_qps_packet_info(erp, buffer, buffer_length,
                                      current_offset, lengthp, next_offsetp,
                                      timestamp));
}

#endif /* EFSYS_OPT_RX_PACKED_STREAM */

void efx_rx_qpush(__in efx_rxq_t *erp, __in unsigned int added,
                  __inout unsigned int *pushedp) {
  efx_nic_t *enp = erp->er_enp;
  const efx_rx_ops_t *erxop = enp->en_erxop;
...
__checkReturn efx_rc_t efx_rx_qflush(__in efx_rxq_t *erp) {
  efx_nic_t *enp = erp->er_enp;
  const efx_rx_ops_t *erxop = enp->en_erxop;
  efx_rc_t rc;
...
__checkReturn size_t efx_rxq_size(__in const efx_nic_t *enp,
...
__checkReturn unsigned int efx_rxq_nbufs(__in const efx_nic_t *enp,
                                         __in unsigned int ndescs) {
  return (EFX_DIV_ROUND_UP(efx_rxq_size(enp, ndescs), EFX_BUF_SIZE));
}

void efx_rx_qenable(__in efx_rxq_t *erp) {
  efx_nic_t *enp = erp->er_enp;
  const efx_rx_ops_t *erxop = enp->en_erxop;
...
static __checkReturn efx_rc_t efx_rx_qcreate_internal(
    __in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
    __in efx_rxq_type_t type, __in_opt const efx_rxq_type_data_t *type_data,
    __in efsys_mem_t *esmp, __in size_t ndescs, __in uint32_t id,
    __in unsigned int flags, __in efx_evq_t *eep,
  ...
  if (!ISP2(ndescs) || ndescs < encp->enc_rxq_min_ndescs ||
  ...
  if (flags & EFX_RXQ_FLAG_RSS_HASH) {
    ...
    rss_hash_field = &erplp->erpl_fields[EFX_RX_PREFIX_FIELD_RSS_HASH];
...
__checkReturn efx_rc_t efx_rx_qcreate(
    __in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
  ...
  return efx_rx_qcreate_internal(enp, index, label, type, &type_data, esmp,
                                 ndescs, id, flags, eep, erpp);
}

#if EFSYS_OPT_RX_PACKED_STREAM

__checkReturn efx_rc_t efx_rx_qcreate_packed_stream(
    __in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
  ...
  return efx_rx_qcreate_internal(
      enp, index, label, EFX_RXQ_TYPE_PACKED_STREAM, &type_data, esmp, ndescs,
...
__checkReturn efx_rc_t efx_rx_qcreate_es_super_buffer(
    __in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
  ...
  type_data.ertd_es_super_buffer.eessb_hol_block_timeout = hol_block_timeout;

  rc = efx_rx_qcreate_internal(enp, index, label, EFX_RXQ_TYPE_ES_SUPER_BUFFER,
                               &type_data, esmp, ndescs,
...
void efx_rx_qdestroy(__in efx_rxq_t *erp) {
  efx_nic_t *enp = erp->er_enp;
  const efx_rx_ops_t *erxop = enp->en_erxop;
...
__checkReturn efx_rc_t efx_pseudo_hdr_pkt_length_get(__in efx_rxq_t *erp,
                                                     __in uint8_t *buffer,
                                                     __out uint16_t *lengthp) {
  efx_nic_t *enp = erp->er_enp;
  const efx_rx_ops_t *erxop = enp->en_erxop;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn uint32_t efx_pseudo_hdr_hash_get(__in efx_rxq_t *erp,
                                               __in efx_rx_hash_alg_t func,
                                               __in uint8_t *buffer) {
  efx_nic_t *enp = erp->er_enp;
  const efx_rx_ops_t *erxop = enp->en_erxop;
...
__checkReturn efx_rc_t efx_rx_prefix_get_layout(
...
static __checkReturn efx_rc_t siena_rx_init(__in efx_nic_t *enp) {
  ...
  for (index = 0; index < FR_BZ_RX_INDIRECTION_TBL_ROWS; index++) {
    ...
    EFX_BAR_TBL_WRITEO(enp, FR_BZ_RX_INDIRECTION_TBL, index, &oword, B_TRUE);
  }
...
siena_rx_scatter_enable(__in efx_nic_t *enp, __in unsigned int buf_size) {
  ...
  nbuf32 = buf_size / 32;
  if ((nbuf32 == 0) || (nbuf32 >= (1 << FRF_BZ_RX_USR_BUF_SIZE_WIDTH)) ||
      ((buf_size % 32) != 0)) {
    rc = EINVAL;
    goto fail1;
  }
...
#define EFX_RX_LFSR_HASH(_enp, _insert)                                        \
  do {                                                                         \
    ...
    EFX_SET_OWORD_FIELD(oword, FRF_BZ_RX_IP_HASH, 0);                          \
    EFX_SET_OWORD_FIELD(oword, FRF_BZ_RX_TCP_SUP, 0);                          \
    EFX_SET_OWORD_FIELD(oword, FRF_BZ_RX_HASH_INSRT_HDR, (_insert) ? 1 : 0);   \
    ...
    if ((_enp)->en_family == EFX_FAMILY_SIENA) {                               \
      ...
      EFX_SET_OWORD_FIELD(oword, FRF_CZ_RX_RSS_IPV6_THASH_ENABLE, 0);          \
...
#define EFX_RX_TOEPLITZ_IPV4_HASH(_enp, _insert, _ip, _tcp)                    \
  do {                                                                         \
    ...
    EFX_SET_OWORD_FIELD(oword, FRF_BZ_RX_IP_HASH, (_ip) ? 1 : 0);              \
    EFX_SET_OWORD_FIELD(oword, FRF_BZ_RX_TCP_SUP, (_tcp) ? 0 : 1);             \
    EFX_SET_OWORD_FIELD(oword, FRF_BZ_RX_HASH_INSRT_HDR, (_insert) ? 1 : 0);   \
    EFX_BAR_WRITEO((_enp), FR_AZ_RX_CFG_REG, &oword);                          \
                                                                               \
    _NOTE(CONSTANTCONDITION)                                                   \
...
#define EFX_RX_TOEPLITZ_IPV6_HASH(_enp, _ip, _tcp, _rc)                        \
  do {                                                                         \
    efx_oword_t oword;                                                         \
                                                                               \
    EFX_BAR_READO((_enp), FR_CZ_RX_RSS_IPV6_REG3, &oword);                     \
    EFX_SET_OWORD_FIELD(oword, FRF_CZ_RX_RSS_IPV6_THASH_ENABLE, 1);            \
    EFX_SET_OWORD_FIELD(oword, FRF_CZ_RX_RSS_IPV6_IP_THASH_ENABLE,             \
                        (_ip) ? 1 : 0);                                        \
    EFX_SET_OWORD_FIELD(oword, FRF_CZ_RX_RSS_IPV6_TCP_SUPPRESS,                \
                        (_tcp) ? 0 : 1);                                       \
    EFX_BAR_WRITEO((_enp), FR_CZ_RX_RSS_IPV6_REG3, &oword);                    \
                                                                               \
    (_rc) = 0;                                                                 \
...
static __checkReturn efx_rc_t siena_rx_scale_mode_set(
  ...
  case EFX_RX_HASHALG_TOEPLITZ:
    ...
    EFX_RX_TOEPLITZ_IPV6_HASH(enp, (type & EFX_RX_HASH_IPV6) ? B_TRUE : B_FALSE,
...
static __checkReturn efx_rc_t siena_rx_scale_key_set(__in efx_nic_t *enp,
  ...
  EFX_ZERO_OWORD(oword);
  for (offset =
           (FRF_CZ_RX_RSS_IPV6_TKEY_LO_LBN + FRF_CZ_RX_RSS_IPV6_TKEY_LO_WIDTH) /
           8;
       offset > 0 && byte < n; --offset)
    oword.eo_u8[offset - 1] = key[byte++];
...
static __checkReturn efx_rc_t siena_rx_scale_tbl_set(
  ...
  for (index = FR_BZ_RX_INDIRECTION_TBL_ROWS - 1; index >= 0; --index) {
    ...
    /* Read the table */
    EFX_BAR_TBL_READO(enp, FR_BZ_RX_INDIRECTION_TBL, index, &oword, B_TRUE);
...
static __checkReturn uint32_t siena_rx_prefix_hash(__in efx_nic_t *enp,
  ...
  switch (func) {
  case EFX_RX_HASHALG_TOEPLITZ:
    return ((buffer[12] << 24) | (buffer[13] << 16) | (buffer[14] << 8) |
            buffer[15]);
...
static void siena_rx_qpost(__in efx_rxq_t *erp,
  ...
  for (i = 0; i < ndescs; i++) {
    EFSYS_PROBE4(rx_post, unsigned int, erp->er_index, unsigned int, id,
                 efsys_dma_addr_t, addrp[i], size_t, size);

    EFX_POPULATE_QWORD_3(
        qword, FSF_AZ_RX_KER_BUF_SIZE, (uint32_t)(size),
        FSF_AZ_RX_KER_BUF_ADDR_DW0, (uint32_t)(addrp[i] & 0xffffffff),
        FSF_AZ_RX_KER_BUF_ADDR_DW1, (uint32_t)(addrp[i] >> 32));
...
static void siena_rx_qpush(__in efx_rxq_t *erp, __in unsigned int added,
  ...
  *pushedp = added;
  ...
  EFX_POPULATE_DWORD_1(dword, EFX_DWORD_0, EFX_OWORD_FIELD(oword, EFX_DWORD_3));

  /* Guarantee ordering of memory (descriptors) and PIO (doorbell) */
  EFX_DMA_SYNC_QUEUE_FOR_DEVICE(erp->er_esmp, erp->er_mask + 1,
                                SIENA_RXQ_DESC_SIZE, wptr,
                                pushed & erp->er_mask);
  EFSYS_PIO_WRITE_BARRIER();
  EFX_BAR_TBL_WRITED3(enp, FR_BZ_RX_DESC_UPD_REGP0, erp->er_index, &dword,
                      B_FALSE);
}
...
static uint8_t *siena_rx_qps_packet_info(
    __in efx_rxq_t *erp, __in uint8_t *buffer, __in uint32_t buffer_length,
    __in uint32_t current_offset, __out uint16_t *lengthp,
    __out uint32_t *next_offsetp, __out uint32_t *timestamp) {
  /* Not supported by Siena hardware */
...
static void siena_rx_qenable(__in efx_rxq_t *erp) {
  ...
  EFX_BAR_TBL_READO(enp, FR_AZ_RX_DESC_PTR_TBL, erp->er_index, &oword, B_TRUE);
  ...
  EFX_BAR_TBL_WRITEO(enp, FR_AZ_RX_DESC_PTR_TBL, erp->er_index, &oword, B_TRUE);
}
...
#undef SUPPORTED_RXQ_FLAGS
  ...
  EFX_POPULATE_OWORD_7(oword, FRF_AZ_RX_DESCQ_BUF_BASE_ID, id,
                       FRF_AZ_RX_DESCQ_EVQ_ID, eep->ee_index,
                       FRF_AZ_RX_DESCQ_OWNER_ID, 0, FRF_AZ_RX_DESCQ_LABEL,
                       label, FRF_AZ_RX_DESCQ_SIZE, size, FRF_AZ_RX_DESCQ_TYPE,
                       0, FRF_AZ_RX_DESCQ_JUMBO, jumbo);
...
static void siena_rx_fini(__in efx_nic_t *enp) { _NOTE(ARGUNUSED(enp)) }

#endif /* EFSYS_OPT_SIENA */

static __checkReturn boolean_t efx_rx_prefix_layout_fields_match(
...
efx_rx_prefix_layout_check(__in const efx_rx_prefix_layout_t *available,
  ...
  for (i = 0; i < EFX_RX_PREFIX_NFIELDS; ++i) {
    ...
    if (efx_rx_prefix_layout_fields_match(&available->erpl_fields[i],
