...
ef10_filter_entry_spec(__in const ef10_filter_table_t *eftp,
                       __in unsigned int index) {
  return ((efx_filter_spec_t *)(EFE_SPEC(eftp, index) &
                                ~(uintptr_t)EFX_EF10_FILTER_FLAGS));
}

static boolean_t ef10_filter_entry_is_busy(__in const ef10_filter_table_t *eftp,
                                           __in unsigned int index) {
...
static boolean_t
ef10_filter_entry_is_auto_old(__in const ef10_filter_table_t *eftp,
                              __in unsigned int index) {
...
static void ef10_filter_set_entry(__inout ef10_filter_table_t *eftp,
                                  __in unsigned int index,
                                  __in_opt const efx_filter_spec_t *efsp) {
  EFE_SPEC(eftp, index) = (uintptr_t)efsp;
}

static void ef10_filter_set_entry_busy(__inout ef10_filter_table_t *eftp,
                                       __in unsigned int index) {
  EFE_SPEC(eftp, index) |= (uintptr_t)EFX_EF10_FILTER_FLAG_BUSY;
}

static void ef10_filter_set_entry_not_busy(__inout ef10_filter_table_t *eftp,
                                           __in unsigned int index) {
  EFE_SPEC(eftp, index) &= ~(uintptr_t)EFX_EF10_FILTER_FLAG_BUSY;
}

static void ef10_filter_set_entry_auto_old(__inout ef10_filter_table_t *eftp,
                                           __in unsigned int index) {
...
static void
ef10_filter_set_entry_not_auto_old(__inout ef10_filter_table_t *eftp,
                                   __in unsigned int index) {
  EFE_SPEC(eftp, index) &= ~(uintptr_t)EFX_EF10_FILTER_FLAG_AUTO_OLD;
  EFSYS_ASSERT(ef10_filter_entry_spec(eftp, index) != NULL);
}

__checkReturn efx_rc_t ef10_filter_init(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
#define MATCH_MASK(match) (EFX_MASK32(match) << EFX_LOW_BIT(match))
  ...
  EFX_STATIC_ASSERT(
      (uint32_t)EFX_FILTER_MATCH_UNKNOWN_UCAST_DST ==
...
void ef10_filter_fini(__in efx_nic_t *enp) {
  EFSYS_ASSERT(EFX_FAMILY_IS_EF100(enp) || EFX_FAMILY_IS_EF10(enp));
...
static __checkReturn efx_rc_t efx_mcdi_filter_op_add(
    __in efx_nic_t *enp, __in efx_filter_spec_t *spec,
    __in unsigned int filter_op, __inout ef10_filter_handle_t *handle) {
  efx_mcdi_req_t req;
  ...
  if (spec->efs_dmaq_id == EFX_FILTER_SPEC_RX_DMAQ_ID_DROP) {
    ...
    MCDI_IN_SET_DWORD(req, FILTER_OP_EXT_IN_RX_QUEUE, spec->efs_dmaq_id);
  }
  ...
  if (spec->efs_flags & EFX_FILTER_FLAG_RX_RSS) {
    ...
    MCDI_IN_SET_DWORD(req, FILTER_OP_EXT_IN_RX_CONTEXT, rss_context);
  }
#endif
  ...
  if (filter_op != MC_CMD_FILTER_OP_IN_OP_REPLACE) {
     ...
     */
    memcpy(MCDI_IN2(req, uint8_t, FILTER_OP_EXT_IN_SRC_MAC), spec->efs_rem_mac,
           EFX_MAC_ADDR_LEN);
    memcpy(MCDI_IN2(req, uint8_t, FILTER_OP_EXT_IN_DST_MAC), spec->efs_loc_mac,
           EFX_MAC_ADDR_LEN);
    ...
    MCDI_IN_SET_BYTE(req, FILTER_OP_EXT_IN_IP_PROTO, spec->efs_ip_proto);
    ...
    memcpy(MCDI_IN2(req, uint8_t, FILTER_OP_EXT_IN_SRC_IP),
           &spec->efs_rem_host.eo_byte[0], MC_CMD_FILTER_OP_EXT_IN_SRC_IP_LEN);
    memcpy(MCDI_IN2(req, uint8_t, FILTER_OP_EXT_IN_DST_IP),
           &spec->efs_loc_host.eo_byte[0], MC_CMD_FILTER_OP_EXT_IN_DST_IP_LEN);
    ...
    case EFX_TUNNEL_PROTOCOL_GENEVE:
      MCDI_IN_POPULATE_DWORD_1(req, FILTER_OP_EXT_IN_VNI_OR_VSID,
  ...
  if (spec->efs_flags & EFX_FILTER_FLAG_ACTION_MARK) {
    ...
    MCDI_IN_SET_DWORD(req, FILTER_OP_V3_IN_MATCH_MARK_VALUE, spec->efs_mark);
...
efx_mcdi_filter_op_delete(__in efx_nic_t *enp, __in unsigned int filter_op,
...
static __checkReturn boolean_t ef10_filter_equal(
    __in const efx_filter_spec_t *left, __in const efx_filter_spec_t *right) {
  ...
  if (memcmp(left->efs_vni_or_vsid, right->efs_vni_or_vsid,
             EFX_VNI_OR_VSID_LEN))
    return (B_FALSE);
  if (memcmp(left->efs_ifrm_loc_mac, right->efs_ifrm_loc_mac, EFX_MAC_ADDR_LEN))
    return (B_FALSE);

  return (B_TRUE);
}

static __checkReturn boolean_t ef10_filter_same_dest(
    __in const efx_filter_spec_t *left, __in const efx_filter_spec_t *right) {
...
static __checkReturn uint32_t ef10_filter_hash(__in efx_filter_spec_t *spec) {
  EFX_STATIC_ASSERT((sizeof(efx_filter_spec_t) % sizeof(uint32_t)) == 0);
...
ef10_filter_is_exclusive(__in efx_filter_spec_t *spec) {
...
__checkReturn efx_rc_t ef10_filter_restore(__in efx_nic_t *enp) {
  int tbl_id;
  ...
  for (tbl_id = 0; tbl_id < EFX_EF10_FILTER_TBL_ROWS; tbl_id++) {
    ...
    if (ef10_filter_is_exclusive(spec)) {
      rc = efx_mcdi_filter_op_add(enp, spec, MC_CMD_FILTER_OP_IN_OP_INSERT,
                                  &eftp->eft_entry[tbl_id].efe_handle);
    } else {
      rc = efx_mcdi_filter_op_add(enp, spec, MC_CMD_FILTER_OP_IN_OP_SUBSCRIBE,
                                  &eftp->eft_entry[tbl_id].efe_handle);
    }
...
static __checkReturn efx_rc_t ef10_filter_add_lookup_equal_spec(
    __in efx_filter_spec_t *spec, __in efx_filter_spec_t *probe_spec,
    __in efx_filter_replacement_policy_t policy, __out boolean_t *found) {
  efx_rc_t rc;
...
static void
ef10_filter_add_select_action(__in efx_filter_spec_t *saved_spec,
...
static __checkReturn efx_rc_t ef10_filter_add_execute_action(
    __in efx_nic_t *enp, __in efx_filter_spec_t *saved_spec,
    __in efx_filter_spec_t *spec, __in efx_filter_spec_t *overridden_spec,
    __in enum ef10_filter_add_action_e action, __in int ins_index) {
  ef10_filter_table_t *eftp = enp->en_filter.ef_ef10_filter_table;
  efsys_lock_state_t state;
  efx_rc_t rc;
  ...
  case EF10_FILTER_ADD_REPLACE:
     ...
     */
    rc = efx_mcdi_filter_op_add(enp, spec, MC_CMD_FILTER_OP_IN_OP_REPLACE,
...
static __checkReturn efx_rc_t
ef10_filter_add_internal(__in efx_nic_t *enp, __inout efx_filter_spec_t *spec,
...
retry:
  ...
  for (depth = 1; depth <= EF10_FILTER_SEARCH_LIMIT; depth++) {
    ...
    if (probe_spec == NULL) {
      ...
      rc = ef10_filter_add_lookup_equal_spec(spec, probe_spec, policy, &found);
      if (rc != 0)
        goto fail1;
  ...
  ef10_filter_add_select_action(saved_spec, spec, &action, &overridden_spec);
  ...
  rc = ef10_filter_add_execute_action(enp, saved_spec, spec, overridden_spec,
                                      action, ins_index);
  if (rc != 0)
    goto fail4;
 ...
 */
static __checkReturn efx_rc_t
ef10_filter_delete_internal(__in efx_nic_t *enp, __in uint32_t filter_id,
  ...
  if (spec->efs_priority > priority) {
    ...
    if (spec->efs_overridden_spec != NULL) {
      EFSYS_ASSERT(spec->efs_overridden_spec->efs_overridden_spec == NULL);
      EFSYS_KMEM_FREE(enp->en_esip, sizeof(*spec), spec->efs_overridden_spec);
     ...
     */
    if (spec->efs_overridden_spec != NULL) {
      rc = efx_mcdi_filter_op_add(enp, spec->efs_overridden_spec,
                                  MC_CMD_FILTER_OP_IN_OP_REPLACE,
                                  &table->eft_entry[filter_idx].efe_handle);
    } else if (ef10_filter_is_exclusive(spec)) {
      rc = efx_mcdi_filter_op_delete(enp, MC_CMD_FILTER_OP_IN_OP_REMOVE,
                                     &table->eft_entry[filter_idx].efe_handle);
    } else {
      rc = efx_mcdi_filter_op_delete(enp, MC_CMD_FILTER_OP_IN_OP_UNSUBSCRIBE,
                                     &table->eft_entry[filter_idx].efe_handle);
    }
    ...
    ef10_filter_set_entry(table, filter_idx, spec->efs_overridden_spec);
    EFSYS_UNLOCK(enp->en_eslp, state);
...
static void ef10_filter_delete_auto(__in efx_nic_t *enp,
  ...
  (void)ef10_filter_delete_internal(enp, filter_idx, EFX_FILTER_PRI_AUTO);
}

__checkReturn efx_rc_t ef10_filter_delete(__in efx_nic_t *enp,
...
static __checkReturn efx_rc_t efx_mcdi_get_parser_disp_info(
    __in efx_nic_t *enp, __out_ecount(buffer_length) uint32_t *buffer,
    __in size_t buffer_length, __in boolean_t encap,
  ...
  MCDI_IN_SET_DWORD(
      req, GET_PARSER_DISP_INFO_OUT_OP,
      encap ? MC_CMD_GET_PARSER_DISP_INFO_IN_OP_GET_SUPPORTED_ENCAP_RX_MATCHES
  ...
  matches_count =
      MCDI_OUT_DWORD(req, GET_PARSER_DISP_INFO_OUT_NUM_SUPPORTED_MATCHES);
  ...
  *list_lengthp = matches_count;
  ...
  list_size =
      matches_count * MC_CMD_GET_PARSER_DISP_INFO_OUT_SUPPORTED_MATCHES_LEN;
  memcpy(buffer,
         MCDI_OUT2(req, uint32_t, GET_PARSER_DISP_INFO_OUT_SUPPORTED_MATCHES),
         list_size);
...
__checkReturn efx_rc_t ef10_filter_supported_filters(
    __in efx_nic_t *enp, __out_ecount(buffer_length) uint32_t *buffer,
  ...
  efx_filter_match_flags_t all_filter_flags =
      (EFX_FILTER_MATCH_REM_HOST | EFX_FILTER_MATCH_LOC_HOST |
       ...
       EFX_FILTER_MATCH_VNI_OR_VSID | EFX_FILTER_MATCH_IFRM_LOC_MAC |
       EFX_FILTER_MATCH_IFRM_UNKNOWN_MCAST_DST |
       EFX_FILTER_MATCH_IFRM_UNKNOWN_UCAST_DST | EFX_FILTER_MATCH_ENCAP_TYPE |
       EFX_FILTER_MATCH_UNKNOWN_MCAST_DST | EFX_FILTER_MATCH_UNKNOWN_UCAST_DST);
  ...
  if (encp->enc_tunnel_encapsulations_supported != 0) {
    rc = efx_mcdi_get_parser_disp_info(enp, &buffer[next_buf_idx],
                                       next_buf_length, B_TRUE,
                                       &mcdi_encap_list_length);
    if (rc != 0) {
      if (rc == ENOSPC) {
        no_space = B_TRUE;
      } else if (rc == EINVAL) {
      ...
      for (i = next_buf_idx; i < next_buf_idx + mcdi_encap_list_length; i++)
   ...
   * lowest priority.
   */
  EFSYS_ASSERT(mcdi_list_length + mcdi_encap_list_length <= buffer_length);
  list_length = 0;
...
static __checkReturn efx_rc_t ef10_filter_insert_unicast(
    __in efx_nic_t *enp, __in_ecount(6) uint8_t const *addr,
  ...
  efx_filter_spec_init_rx(&spec, EFX_FILTER_PRI_AUTO, filter_flags,
                          eftp->eft_default_rxq);
  rc = efx_filter_spec_set_eth_local(&spec, EFX_FILTER_SPEC_VID_UNSPEC, addr);
  if (rc != 0)
    goto fail1;

  rc = ef10_filter_add_internal(
      enp, &spec, EFX_FILTER_REPLACEMENT_NEVER,
...
static __checkReturn efx_rc_t ef10_filter_insert_all_unicast(
    __in efx_nic_t *enp, __in efx_filter_flags_t filter_flags) {
  ef10_filter_table_t *eftp = enp->en_filter.ef_ef10_filter_table;
  efx_filter_spec_t spec;
  efx_rc_t rc;
...
static __checkReturn efx_rc_t ef10_filter_insert_multicast_list(
    __in efx_nic_t *enp, __in boolean_t mulcst, __in boolean_t brdcst,
    __in_ecount(6 * count) uint8_t const *addrs, __in uint32_t count,
    __in efx_filter_flags_t filter_flags, __in boolean_t rollback) {
  ef10_filter_table_t *eftp = enp->en_filter.ef_ef10_filter_table;
  efx_filter_spec_t spec;
  uint8_t addr[6];
  uint32_t i;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (rc == 0) {
      eftp->eft_mulcst_filter_indexes[filter_count] = filter_index;
      filter_count++;
...
rollback:
  ...
  i = filter_count;
  while (i--) {
    ef10_filter_delete_auto(enp, eftp->eft_mulcst_filter_indexes[i]);
  }
...
static __checkReturn efx_rc_t ef10_filter_insert_all_multicast(
    __in efx_nic_t *enp, __in efx_filter_flags_t filter_flags) {
  ef10_filter_table_t *eftp = enp->en_filter.ef_ef10_filter_table;
  efx_filter_spec_t spec;
  efx_rc_t rc;
...
static __checkReturn efx_rc_t
ef10_filter_insert_encap_filters(__in efx_nic_t *enp, __in boolean_t mulcst,
                                 __in efx_filter_flags_t filter_flags) {
  ef10_filter_table_t *table = enp->en_filter.ef_ef10_filter_table;
  uint32_t i;
  efx_rc_t rc;
  ...
  for (i = 0; i < EFX_ARRAY_SIZE(ef10_filter_encap_list); i++) {
    efx_filter_spec_t spec;
    ef10_filter_encap_entry_t *encap_filter = &ef10_filter_encap_list[i];

    /*
     * Skip multicast filters if we've not been asked for
     * any multicast traffic.
     */
    if ((mulcst == B_FALSE) && (encap_filter->inner_frame_match ==
    ...
    rc = efx_filter_spec_set_encap_type(&spec, encap_filter->encap_type,
    ...
    rc = ef10_filter_add_internal(
        enp, &spec, EFX_FILTER_REPLACEMENT_NEVER,
        &table->eft_encap_filter_indexes[table->eft_encap_filter_count]);
    if (rc != 0) {
      if (rc != EACCES)
        goto fail2;
    } else {
...
static void ef10_filter_remove_old(__in efx_nic_t *enp) {
  ef10_filter_table_t *table = enp->en_filter.ef_ef10_filter_table;
  uint32_t i;
...
static __checkReturn efx_rc_t ef10_filter_get_workarounds(__in efx_nic_t *enp) {
  efx_nic_cfg_t *encp = &enp->en_nic_cfg;
...
static void ef10_filter_remove_all_existing_filters(__in efx_nic_t *enp) {
  ef10_filter_table_t *table = enp->en_filter.ef_ef10_filter_table;
  efx_port_t *epp = &(enp->en_port);
  unsigned int i;

  for (i = 0; i < table->eft_unicst_filter_count; i++) {
    ef10_filter_delete_auto(enp, table->eft_unicst_filter_indexes[i]);
  }
  table->eft_unicst_filter_count = 0;

  for (i = 0; i < table->eft_mulcst_filter_count; i++) {
    ef10_filter_delete_auto(enp, table->eft_mulcst_filter_indexes[i]);
  }
  table->eft_mulcst_filter_count = 0;

  for (i = 0; i < table->eft_encap_filter_count; i++) {
    ef10_filter_delete_auto(enp, table->eft_encap_filter_indexes[i]);
  }
  table->eft_encap_filter_count = 0;
...
static void ef10_filter_mark_old_filters(__in efx_nic_t *enp) {
  ef10_filter_table_t *table = enp->en_filter.ef_ef10_filter_table;
  unsigned int i;

  for (i = 0; i < table->eft_unicst_filter_count; i++) {
    ef10_filter_set_entry_auto_old(table, table->eft_unicst_filter_indexes[i]);
  }
  for (i = 0; i < table->eft_mulcst_filter_count; i++) {
    ef10_filter_set_entry_auto_old(table, table->eft_mulcst_filter_indexes[i]);
  }
  for (i = 0; i < table->eft_encap_filter_count; i++) {
    ef10_filter_set_entry_auto_old(table, table->eft_encap_filter_indexes[i]);
  }
}

static __checkReturn efx_rc_t ef10_filter_insert_renew_unicst_filters(
    __in efx_nic_t *enp, __in_ecount(6) uint8_t const *mac_addr,
    __in boolean_t all_unicst, __in efx_filter_flags_t filter_flags,
  ...
  *all_unicst_inserted = B_FALSE;
  if (all_unicst || (rc != 0)) {
    ...
    all_unicst_rc = ef10_filter_insert_all_unicast(enp, filter_flags);
    if (all_unicst_rc == 0) {
...
static __checkReturn efx_rc_t ef10_filter_insert_renew_mulcst_filters(
    __in efx_nic_t *enp, __in boolean_t mulcst, __in boolean_t all_mulcst,
    __in boolean_t brdcst, __in_ecount(6 * count) uint8_t const *addrs,
    __in uint32_t count, __in efx_filter_flags_t filter_flags,
    __in boolean_t all_unicst_inserted, __out boolean_t *all_mulcst_inserted) {
  ef10_filter_table_t *table = enp->en_filter.ef_ef10_filter_table;
  efx_nic_cfg_t *encp = &enp->en_nic_cfg;
  efx_port_t *epp = &(enp->en_port);
  efx_rc_t rc;
  ...
  *all_mulcst_inserted = B_FALSE;
  ...
  if (all_mulcst == B_TRUE) {
     ...
     * failure).
     */
    all_mulcst_rc = ef10_filter_insert_all_multicast(enp, filter_flags);
    if (all_mulcst_rc == 0) {
      epp->ep_all_mulcst_inserted = B_TRUE;
      *all_mulcst_inserted = B_TRUE;
    } else {
      rc = ef10_filter_insert_multicast_list(enp, B_TRUE, brdcst, addrs, count,
                                             filter_flags, B_FALSE);
      if (rc != 0)
        goto fail1;
    }
  } else {
     ...
     */
    rc = ef10_filter_insert_multicast_list(enp, mulcst, brdcst, addrs, count,
                                           filter_flags, B_TRUE);
    if (rc != 0) {
      ...
      rc = ef10_filter_insert_all_multicast(enp, filter_flags);
      if (rc == 0) {
        epp->ep_all_mulcst_inserted = B_TRUE;
        *all_mulcst_inserted = B_TRUE;
      } else {
        rc = ef10_filter_insert_multicast_list(enp, mulcst, brdcst, addrs,
                                               count, filter_flags, B_FALSE);
        if (rc != 0)
          goto fail2;
      }
    }
 ...
 */
__checkReturn efx_rc_t ef10_filter_reconfigure(
    __in efx_nic_t *enp, __in_ecount(6) uint8_t const *mac_addr,
  ...
  rc = ef10_filter_insert_renew_unicst_filters(
      enp, mac_addr, all_unicst, filter_flags, &all_unicst_inserted);
  if (rc != 0)
    goto fail1;
  ...
  rc = ef10_filter_insert_renew_mulcst_filters(
      enp, mulcst, all_mulcst, brdcst, addrs, count, filter_flags,
      all_unicst_inserted, &all_mulcst_inserted);
  if (rc != 0)
    goto fail3;

  if (encp->enc_tunnel_encapsulations_supported != 0) {
    /* Try to insert filters for encapsulated packets. */
    (void)ef10_filter_insert_encap_filters(enp, mulcst || all_mulcst || brdcst,
                                           filter_flags);
  }
...
void ef10_filter_get_default_rxq(__in efx_nic_t *enp, __out efx_rxq_t **erpp,
...
void ef10_filter_default_rxq_set(__in efx_nic_t *enp, __in efx_rxq_t *erp,
...
void ef10_filter_default_rxq_clear(__in efx_nic_t *enp) {
  ef10_filter_table_t *table = enp->en_filter.ef_ef10_filter_table;
