...
__checkReturn efx_rc_t efx_intr_init(__in efx_nic_t *enp,
                                     __in efx_intr_type_t type,
...
void efx_intr_fini(__in efx_nic_t *enp) {
  efx_intr_t *eip = &(enp->en_intr);
...
void efx_intr_enable(__in efx_nic_t *enp) {
  efx_intr_t *eip = &(enp->en_intr);
  const efx_intr_ops_t *eiop = eip->ei_eiop;
...
void efx_intr_disable(__in efx_nic_t *enp) {
  efx_intr_t *eip = &(enp->en_intr);
  const efx_intr_ops_t *eiop = eip->ei_eiop;
...
void efx_intr_disable_unlocked(__in efx_nic_t *enp) {
  efx_intr_t *eip = &(enp->en_intr);
  const efx_intr_ops_t *eiop = eip->ei_eiop;
...
__checkReturn efx_rc_t efx_intr_trigger(__in efx_nic_t *enp,
                                        __in unsigned int level) {
  efx_intr_t *eip = &(enp->en_intr);
  const efx_intr_ops_t *eiop = eip->ei_eiop;
...
void efx_intr_status_line(__in efx_nic_t *enp, __out boolean_t *fatalp,
                          __out uint32_t *qmaskp) {
  efx_intr_t *eip = &(enp->en_intr);
  const efx_intr_ops_t *eiop = eip->ei_eiop;
...
void efx_intr_status_message(__in efx_nic_t *enp, __in unsigned int message,
                             __out boolean_t *fatalp) {
  efx_intr_t *eip = &(enp->en_intr);
  const efx_intr_ops_t *eiop = eip->ei_eiop;
...
void efx_intr_fatal(__in efx_nic_t *enp) {
  efx_intr_t *eip = &(enp->en_intr);
  const efx_intr_ops_t *eiop = eip->ei_eiop;
...
static __checkReturn efx_rc_t siena_intr_init(__in efx_nic_t *enp,
  ...
  EFX_POPULATE_OWORD_3(
      oword, FRF_AZ_NORM_INT_VEC_DIS_KER, (type == EFX_INTR_MESSAGE) ? 1 : 0,
...
static void siena_intr_disable_unlocked(__in efx_nic_t *enp) {
  ...
  EFSYS_BAR_READO(enp->en_esbp, FR_AZ_INT_EN_REG_KER_OFST, &oword, B_FALSE);
  EFX_SET_OWORD_FIELD(oword, FRF_AZ_DRV_INT_EN_KER, 0);
  EFSYS_BAR_WRITEO(enp->en_esbp, FR_AZ_INT_EN_REG_KER_OFST, &oword, B_FALSE);
}
