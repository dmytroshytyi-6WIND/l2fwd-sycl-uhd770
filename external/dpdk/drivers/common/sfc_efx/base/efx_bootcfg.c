...
#define DHCP_NEXT_TAG(hdr)                                                     \
  ((efx_dhcp_tag_hdr_t *)(((uint8_t *)(hdr)) + DHCP_FULL_TAG_LENGTH((hdr))))
...
__checkReturn efx_rc_t efx_bootcfg_sector_info(
...
__checkReturn uint8_t efx_dhcp_csum(__in_bcount(size) uint8_t const *data,
                                    __in size_t size) {
  unsigned int pos;
  uint8_t checksum = 0;
...
__checkReturn efx_rc_t efx_dhcp_verify(__in_bcount(size) uint8_t const *data,
                                       __in size_t size,
 ...
 * ENOSPC.
 */
static efx_rc_t efx_dhcp_walk_tags(__deref_inout uint8_t **tagpp,
  ...
  if (DHCP_IS_ENCAP_OPT(opt)) {
     ...
     * skipped.
     */
    rc = efx_dhcp_walk_tags(tagpp, buffer_sizep, DHCP_ENCAPSULATOR(opt));
    if (rc == 0) {
  ...
  while (rc == 0) {
    ...
    if ((**tagpp) == EFX_DHCP_END) {
      ...
      size = DHCP_FULL_TAG_LENGTH((efx_dhcp_tag_hdr_t *)*tagpp);
    }
 ...
 */
__checkReturn efx_rc_t efx_dhcp_find_tag(__in_bcount(buffer_length)
                                             uint8_t *bufferp,
 ...
 */
__checkReturn efx_rc_t efx_dhcp_find_end(__in_bcount(buffer_length)
                                             uint8_t *bufferp,
                                         __in size_t buffer_length,
 ...
 * necessary.
 */
__checkReturn efx_rc_t efx_dhcp_delete_tag(__inout_bcount(buffer_length)
                                               uint8_t *bufferp,
                                           __in size_t buffer_length,
                                           __in uint16_t opt) {
  efx_rc_t rc;
  ...
  if (DHCP_IS_ENCAP_OPT(opt)) {
    ...
    if (encap_hdrp->length > tag_length) {
      encap_hdrp->length = (uint8_t)((size_t)encap_hdrp->length - tag_length);
    } else {
 ...
 */
static void efx_dhcp_write_tag(__in uint8_t *write_pointp, __in uint16_t opt,
 ...
 * necessary.
 */
__checkReturn efx_rc_t efx_dhcp_add_tag(
  ...
  if (DHCP_IS_ENCAP_OPT(opt)) {
    rc = efx_dhcp_walk_tags(&searchp, &search_size, DHCP_ENCAPSULATOR(opt));
    if (rc == 0) {
      ...
      if (((size_t)encap_hdrp->length + DHCP_CALC_TAG_LENGTH(value_length)) >
          DHCP_MAX_VALUE) {
        rc = E2BIG;
        goto fail6;
      }
  ...
  if (DHCP_IS_ENCAP_OPT(opt)) {
    ...
    encap_hdrp->length += ((uint8_t)DHCP_CALC_TAG_LENGTH(value_length));
 ...
 */
__checkReturn efx_rc_t efx_dhcp_update_tag(
    __inout_bcount(buffer_length) uint8_t *bufferp, __in size_t buffer_length,
  ...
  if (DHCP_IS_ENCAP_OPT(opt)) {
    ...
    rc = efx_dhcp_walk_tags(&encapp, &following_encap, DHCP_ENCAPSULATOR(opt));
    if (rc != 0)
      goto fail6;

    encap_hdrp = (efx_dhcp_tag_hdr_t *)encapp;

    new_length = ((size_t)encap_hdrp->length + value_length - old_length);
    /* Check encapsulator will not overflow */
   ...
   */
  if (old_length != value_length) {
    uint8_t *destp = (uint8_t *)DHCP_NEXT_TAG(hdrp) + value_length - old_length;
    size_t count = &bufferp[buffer_length] - (uint8_t *)DHCP_NEXT_TAG(hdrp);
 ...
 */
efx_rc_t efx_bootcfg_copy_sector(__in efx_nic_t *enp,
                                 __inout_bcount(sector_length) uint8_t *sector,
  ...
  if (!handle_format_errors) {
    ...
    if ((used_bytes < 2) || (sector[used_bytes - 1] != EFX_DHCP_END)) {
...
efx_rc_t efx_bootcfg_read(__in efx_nic_t *enp, __out_bcount(size) uint8_t *data,
                          __in size_t size) {
  ...
  if ((rc = efx_nvram_read_chunk(enp, EFX_NVRAM_BOOTROM_CFG, sector_offset,
                                 (caddr_t)payload, sector_length)) != 0) {
...
efx_rc_t efx_bootcfg_write(__in efx_nic_t *enp, __in_bcount(size) uint8_t *data,
                           __in size_t size) {
  uint8_t *partn_data;
  uint8_t checksum;
  size_t partn_length;
   ...
   * character
   */
  if ((used_bytes < 2) || ((uint8_t)data[used_bytes - 1] != EFX_DHCP_END)) {
    /* Block too short or EFX_DHCP_END missing */
    rc = ENOENT;
    goto fail5;
  }
  ...
  rc = efx_nvram_read_chunk(enp, EFX_NVRAM_BOOTROM_CFG, 0, (caddr_t)partn_data,
                            partn_length);
  if (rc != 0)
    goto fail9;
