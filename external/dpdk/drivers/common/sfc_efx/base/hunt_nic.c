...
static __checkReturn efx_rc_t hunt_nic_get_required_pcie_bandwidth(
  ...
  if (port_modes & (1U << TLV_PORT_MODE_40G_40G)) {
     ...
     */
    if ((rc = efx_nic_calculate_pcie_link_bandwidth(8, EFX_PCIE_LINK_SPEED_GEN3,
                                                    &bandwidth)) != 0)
      goto fail1;
  } else {
...
__checkReturn efx_rc_t hunt_board_cfg(__in efx_nic_t *enp) {
   ...
   */
  rc = efx_mcdi_set_workaround(enp, MC_CMD_WORKAROUND_BUG35388, B_TRUE, NULL);
  if ((rc == 0) || (rc == EACCES))
   ...
   */
  rc = efx_mcdi_set_workaround(enp, MC_CMD_WORKAROUND_BUG41750, B_TRUE, NULL);
  if (rc == 0) {
  ...
  if (encp->enc_bug35388_workaround) {
    encp->enc_evq_timer_max_us =
        (encp->enc_evq_timer_quantum_ns << ERF_DD_EVQ_IND_TIMER_VAL_WIDTH) /
        1000;
  } else {
    encp->enc_evq_timer_max_us =
