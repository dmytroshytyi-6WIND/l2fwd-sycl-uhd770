...
__checkReturn efx_rc_t efx_mcdi_get_port_assignment(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mcdi_get_port_modes(
   ...
   */
  if (req.emr_out_length_used < MC_CMD_GET_PORT_MODES_OUT_CURRENT_MODE_OFST) {
    rc = EMSGSIZE;
    goto fail2;
  }
  ...
  *modesp = MCDI_OUT_DWORD(req, GET_PORT_MODES_OUT_MODES);
  ...
  if (current_modep != NULL) {
    *current_modep = MCDI_OUT_DWORD(req, GET_PORT_MODES_OUT_CURRENT_MODE);
  }

  if (default_modep != NULL) {
    *default_modep = MCDI_OUT_DWORD(req, GET_PORT_MODES_OUT_DEFAULT_MODE);
  }
...
__checkReturn efx_rc_t ef10_nic_get_port_mode_bandwidth(
  ...
  if ((rc = efx_mcdi_get_port_modes(enp, &port_modes, &current_mode, NULL)) !=
      0) {
...
__checkReturn efx_rc_t efx_mcdi_vadaptor_alloc(__in efx_nic_t *enp,
  ...
  MCDI_IN_POPULATE_DWORD_1(
      req, VADAPTOR_ALLOC_IN_FLAGS,
...
__checkReturn efx_rc_t efx_mcdi_vadaptor_free(__in efx_nic_t *enp,
                                              __in uint32_t port_id) {
  efx_mcdi_req_t req;
...
__checkReturn efx_rc_t efx_mcdi_get_mac_address_pf(__in efx_nic_t *enp,
                                                   __out_ecount_opt(6)
                                                       uint8_t mac_addrp[6]) {
  efx_mcdi_req_t req;
  ...
  if (mac_addrp != NULL) {
    ...
    addrp = MCDI_OUT2(req, uint8_t, GET_MAC_ADDRESSES_OUT_MAC_ADDR_BASE);
...
__checkReturn efx_rc_t efx_mcdi_get_mac_address_vf(__in efx_nic_t *enp,
                                                   __out_ecount_opt(6)
                                                       uint8_t mac_addrp[6]) {
  efx_mcdi_req_t req;
  ...
  if (req.emr_out_length_used < MC_CMD_VPORT_GET_MAC_ADDRESSES_OUT_LENMIN) {
    rc = EMSGSIZE;
    goto fail2;
  }

  if (MCDI_OUT_DWORD(req, VPORT_GET_MAC_ADDRESSES_OUT_MACADDR_COUNT) < 1) {
    rc = ENOENT;
    goto fail3;
  }
  ...
  if (mac_addrp != NULL) {
    ...
    addrp = MCDI_OUT2(req, uint8_t, VPORT_GET_MAC_ADDRESSES_OUT_MACADDR);
...
__checkReturn efx_rc_t efx_mcdi_get_clock(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mcdi_get_rxdp_config(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mcdi_get_vector_cfg(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mcdi_alloc_vis(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mcdi_free_vis(__in efx_nic_t *enp) {
  efx_mcdi_req_t req;
  efx_rc_t rc;
...
efx_mcdi_alloc_piobuf(__in efx_nic_t *enp, __out efx_piobuf_handle_t *handlep) {
  efx_mcdi_req_t req;
...
static __checkReturn efx_rc_t
efx_mcdi_free_piobuf(__in efx_nic_t *enp, __in efx_piobuf_handle_t handle) {
  efx_mcdi_req_t req;
...
static __checkReturn efx_rc_t
efx_mcdi_link_piobuf(__in efx_nic_t *enp, __in uint32_t vi_index,
...
static __checkReturn efx_rc_t efx_mcdi_unlink_piobuf(__in efx_nic_t *enp,
...
static void ef10_nic_alloc_piobufs(__in efx_nic_t *enp,
...
static void ef10_nic_free_piobufs(__in efx_nic_t *enp) {
  efx_piobuf_handle_t *handlep;
  unsigned int i;
...
__checkReturn efx_rc_t ef10_nic_pio_alloc(__inout efx_nic_t *enp,
...
__checkReturn efx_rc_t ef10_nic_pio_free(__inout efx_nic_t *enp,
...
__checkReturn efx_rc_t ef10_nic_pio_link(__inout efx_nic_t *enp,
                                         __in uint32_t vi_index,
                                         __in efx_piobuf_handle_t handle) {
  return (efx_mcdi_link_piobuf(enp, vi_index, handle));
}

__checkReturn efx_rc_t ef10_nic_pio_unlink(__inout efx_nic_t *enp,
                                           __in uint32_t vi_index) {
...
static __checkReturn efx_rc_t
ef10_mcdi_get_pf_count(__in efx_nic_t *enp, __out uint32_t *pf_countp) {
  efx_mcdi_req_t req;
  ...
  *pf_countp = *MCDI_OUT(req, uint8_t, MC_CMD_GET_PF_COUNT_OUT_PF_COUNT_OFST);
...
static __checkReturn efx_rc_t ef10_get_datapath_caps(__in efx_nic_t *enp) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
  efx_mcdi_req_t req;
...
#define CAP_FLAGS3(_req, _flag)                                                \
  ...
  if (CAP_FLAGS1(req, VXLAN_NVGRE)) {
    encp->enc_tunnel_encapsulations_supported =
        (1u << EFX_TUNNEL_PROTOCOL_VXLAN) | (1u << EFX_TUNNEL_PROTOCOL_GENEVE) |
  ...
  if (req.emr_out_length_used >= MC_CMD_GET_CAPABILITIES_V3_OUT_LEN) {
    uint8_t mode = MCDI_OUT_BYTE(req, GET_CAPABILITIES_V3_OUT_VI_WINDOW_MODE);
  ...
  if (req.emr_out_length_used >= MC_CMD_GET_CAPABILITIES_V4_OUT_LEN) {
    ...
    encp->enc_mac_stats_nstats =
        MCDI_OUT_WORD(req, GET_CAPABILITIES_V4_OUT_MAC_STATS_NUM_STATS);
  } else {
  ...
  if (CAP_FLAGS1(req, RX_RSS_LIMITED)) {
    ...
    case EFX_FAMILY_HUNTINGTON:
       ...
       */
      encp->enc_rx_scale_hash_alg_mask = (1U << EFX_RX_HASHALG_PACKED_STREAM);
      break;
  ...
  if (req.emr_out_length_used >= MC_CMD_GET_CAPABILITIES_V9_OUT_LEN) {
    encp->enc_rx_scale_indirection_max_nqueues =
        MCDI_OUT_DWORD(req, GET_CAPABILITIES_V9_OUT_RSS_MAX_INDIRECTION_QUEUES);
    encp->enc_rx_scale_tbl_min_nentries = MCDI_OUT_DWORD(
        req, GET_CAPABILITIES_V9_OUT_RSS_MIN_INDIRECTION_TABLE_SIZE);
    encp->enc_rx_scale_tbl_max_nentries = MCDI_OUT_DWORD(
        req, GET_CAPABILITIES_V9_OUT_RSS_MAX_INDIRECTION_TABLE_SIZE);
...
#define EF10_LEGACY_PF_PRIVILEGE_MASK                                          \
  (MC_CMD_PRIVILEGE_MASK_IN_GRP_ADMIN | MC_CMD_PRIVILEGE_MASK_IN_GRP_LINK |    \
   MC_CMD_PRIVILEGE_MASK_IN_GRP_ONLOAD | MC_CMD_PRIVILEGE_MASK_IN_GRP_PTP |    \
...
__checkReturn efx_rc_t ef10_get_privilege_mask(__in efx_nic_t *enp,
                                               __out uint32_t *maskp) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
  uint32_t mask;
  efx_rc_t rc;

  if ((rc = efx_mcdi_privilege_mask(enp, encp->enc_pf, encp->enc_vf, &mask)) !=
      0) {
    if (rc != ENOTSUP)
      goto fail1;
...
static __checkReturn efx_rc_t ef10_external_port_mapping(
    __in efx_nic_t *enp, __in uint32_t port, __out uint8_t *external_portp) {
  efx_rc_t rc;
  int i;
  uint32_t port_modes;
  uint32_t matches;
  ...
  if ((rc = efx_mcdi_get_port_modes(enp, &port_modes, &current, NULL)) != 0) {
    /*
  ...
  for (i = 0; i < EFX_ARRAY_SIZE(__ef10_external_port_mappings); ++i) {
    struct ef10_external_port_map_s *eepmp = &__ef10_external_port_mappings[i];
...
static __checkReturn efx_rc_t efx_mcdi_get_nic_addr_caps(__in efx_nic_t *enp) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
  ...
  case MC_CMD_GET_DESC_ADDR_INFO_OUT_MAPPING_REGIONED:
    ...
    rc =
        efx_mcdi_get_nic_addr_regions(enp, &enp->en_dma.end_u.endu_region_info);
    if (rc != 0)
      goto fail2;
    break;
  default:
...
__checkReturn efx_rc_t efx_mcdi_nic_board_cfg(__in efx_nic_t *enp) {
  ...
  if ((rc = ef10_external_port_mapping(enp, port, &encp->enc_external_port)) !=
      0)
    goto fail2;
  ...
  if (epp->ep_phy_cap_mask & (1u << EFX_PHY_CAP_BASER_FEC))
    epp->ep_phy_cap_mask |= (1u << EFX_PHY_CAP_BASER_FEC_REQUESTED);
  if (epp->ep_phy_cap_mask & (1u << EFX_PHY_CAP_RS_FEC))
    epp->ep_phy_cap_mask |= (1u << EFX_PHY_CAP_RS_FEC_REQUESTED);
  if (epp->ep_phy_cap_mask & (1u << EFX_PHY_CAP_25G_BASER_FEC))
    epp->ep_phy_cap_mask |= (1u << EFX_PHY_CAP_25G_BASER_FEC_REQUESTED);
...
__checkReturn efx_rc_t efx_mcdi_entity_reset(__in efx_nic_t *enp) {
  efx_mcdi_req_t req;
...
static __checkReturn efx_rc_t
ef10_set_workaround_bug26807(__in efx_nic_t *enp) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
  uint32_t flags;
  efx_rc_t rc;
   ...
   */
  rc = efx_mcdi_set_workaround(enp, MC_CMD_WORKAROUND_BUG26807, B_TRUE, &flags);
  if (rc == 0) {
...
static __checkReturn efx_rc_t ef10_nic_board_cfg(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t ef10_nic_probe(__in efx_nic_t *enp) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
  efx_drv_cfg_t *edcp = &(enp->en_drv_cfg);
  efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_nic_set_drv_limits(__inout efx_nic_t *enp,
   ...
   */
  if ((encp->enc_piobuf_size == 0) || (encp->enc_piobuf_limit == 0) ||
    ...
    blk_size =
        MAX(edlp->edl_min_pio_alloc_size, encp->enc_piobuf_min_alloc_size);
...
__checkReturn efx_rc_t ef10_nic_reset(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_upstream_port_vadaptor_alloc(__in efx_nic_t *enp) {
  uint32_t retry;
  uint32_t delay_us;
  efx_rc_t rc;
  ...
  while ((rc = efx_mcdi_vadaptor_alloc(enp, EVB_PORT_ID_ASSIGNED)) != 0) {
    if (EFX_PCI_FUNCTION_IS_PF(&enp->en_nic_cfg) || (rc != ENOENT)) {
      /*
...
__checkReturn efx_rc_t ef10_nic_init(__in efx_nic_t *enp) {
  efx_drv_cfg_t *edcp = &(enp->en_drv_cfg);
  ...
  max_vi_count = edcp->edc_max_vi_count + enp->en_arch.ef10.ena_piobuf_count;
   ...
   */
  vi_count = 0;
  if ((rc = efx_mcdi_alloc_vis(enp, min_vi_count, max_vi_count, &vi_base,
                               &vi_count, &vi_shift)) != 0)
    goto fail3;
  ...
  if (enp->en_arch.ef10.ena_piobuf_count > 0) {
    enp->en_arch.ef10.ena_uc_mem_map_size =
        (vi_window_size * enp->en_arch.ef10.ena_pio_write_vi_base);
  } else {
    enp->en_arch.ef10.ena_uc_mem_map_size =
  ...
  enp->en_arch.ef10.ena_wc_mem_map_size =
      (vi_window_size * enp->en_arch.ef10.ena_piobuf_count);

  /* Link piobufs to extra VIs in WC mapping */
  if (enp->en_arch.ef10.ena_piobuf_count > 0) {
    for (i = 0; i < enp->en_arch.ef10.ena_piobuf_count; i++) {
      rc =
          efx_mcdi_link_piobuf(enp, enp->en_arch.ef10.ena_pio_write_vi_base + i,
...
__checkReturn efx_rc_t ef10_nic_get_vi_pool(__in efx_nic_t *enp,
                                            __out uint32_t *vi_countp) {
  EFSYS_ASSERT(EFX_FAMILY_IS_EF10(enp));
...
__checkReturn efx_rc_t ef10_nic_get_bar_region(__in efx_nic_t *enp,
...
__checkReturn boolean_t ef10_nic_hw_unavailable(__in efx_nic_t *enp) {
  efx_dword_t dword;
...
void ef10_nic_set_hw_unavailable(__in efx_nic_t *enp) {
  EFSYS_PROBE(hw_unavail);
  enp->en_reset_flags |= EFX_RESET_HW_UNAVAIL;
}

void ef10_nic_fini(__in efx_nic_t *enp) {
  uint32_t i;
  efx_rc_t rc;
...
void ef10_nic_unprobe(__in efx_nic_t *enp) {
#if EFSYS_OPT_MON_STATS
...
__checkReturn efx_rc_t ef10_nic_register_test(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_get_nic_global(__in efx_nic_t *enp,
                                               __in uint32_t key,
...
__checkReturn efx_rc_t efx_mcdi_set_nic_global(__in efx_nic_t *enp,
                                               __in uint32_t key,
                                               __in uint32_t value) {
  efx_mcdi_req_t req;
