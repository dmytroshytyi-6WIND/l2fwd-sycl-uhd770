...
__checkReturn efx_rc_t efx_evb_init(__in efx_nic_t *enp) {
...
void efx_evb_fini(__in efx_nic_t *enp) {
...
__checkReturn boolean_t efx_is_zero_eth_addr(__in_bcount(EFX_MAC_ADDR_LEN)
                                                 const uint8_t *addrp) {
  return (!(addrp[0] | addrp[1] | addrp[2] | addrp[3] | addrp[4] | addrp[5]));
}

static void efx_evb_free_vport(__in efx_nic_t *enp,
  ...
  if (EFX_VPORT_PCI_FUNCTION_IS_PF(configp)) {
    ...
    if ((configp->evc_vport_id != EFX_VPORT_ID_INVALID) &&
        (eeop->eeo_vadaptor_free(enp, vswitch_id, configp->evc_vport_id) !=
         0)) {
      EFSYS_PROBE2(eeo_vadaptor_free, uint16_t, configp->evc_function, uint32_t,
                   configp->evc_vport_id);
    }
  } else {
    if (configp->evc_vport_assigned == B_TRUE) {
      if (eeop->eeo_vport_assign(enp, vswitch_id, EVB_PORT_ID_NULL,
                                 configp->evc_function) != 0) {
        EFSYS_PROBE1(eeo_vport_assign, uint16_t, configp->evc_function);
      }
  ...
  if (!efx_is_zero_eth_addr(configp->evc_mac_addr)) {
    if (eeop->eeo_vport_mac_addr_del(enp, vswitch_id, configp->evc_vport_id,
                                     configp->evc_mac_addr) != 0) {
      EFSYS_PROBE1(eeo_vport_mac_addr_del, uint16_t, configp->evc_function);
    }
    memset(configp->evc_mac_addr, 0x00, EFX_MAC_ADDR_LEN);
  }

  if (configp->evc_vport_id != EFX_VPORT_ID_INVALID) {
    if (eeop->eeo_vport_free(enp, vswitch_id, configp->evc_vport_id) != 0) {
      EFSYS_PROBE1(eeo_vport_free, uint16_t, configp->evc_function);
    }
    configp->evc_vport_id = EFX_VPORT_ID_INVALID;
  }
}

static void efx_evb_free_vports(__in efx_nic_t *enp,
                                __in efx_vswitch_id_t vswitch_id,
                                __in uint32_t num_vports,
                                __inout_ecount(num_vports)
                                    efx_vport_config_t *vport_configp) {
...
static __checkReturn efx_rc_t efx_evb_configure_vport(
    __in efx_nic_t *enp, __in efx_vswitch_id_t vswitch_id,
    __in const efx_evb_ops_t *eeop, __inout efx_vport_config_t *configp) {
  efx_rc_t rc;
  efx_vport_id_t vport_id;

  if ((rc = eeop->eeo_vport_alloc(enp, vswitch_id, EFX_VPORT_TYPE_NORMAL,
                                  configp->evc_vid, configp->evc_vlan_restrict,
                                  &vport_id)) != 0)
    goto fail1;

  configp->evc_vport_id = vport_id;

  if ((rc = eeop->eeo_vport_mac_addr_add(enp, vswitch_id, configp->evc_vport_id,
                                         configp->evc_mac_addr)) != 0)
    goto fail2;
  ...
  if (EFX_VPORT_PCI_FUNCTION_IS_PF(configp)) {
    ...
    if ((rc = eeop->eeo_vport_assign(enp, vswitch_id, configp->evc_vport_id,
                                     configp->evc_function)) != 0)
      goto fail4;
...
__checkReturn efx_rc_t efx_evb_vswitch_create(
   ...
   */
  EFSYS_ASSERT((num_vports != 0) && (vport_configp != NULL) && (evpp != NULL));
  EFSYS_ASSERT(enp->en_mod_flags & EFX_MOD_EVB);
  EFSYS_ASSERT(!(enp->en_mod_flags & EFX_MOD_NIC));

  if ((eeop->eeo_vswitch_alloc == NULL) || (eeop->eeo_vport_alloc == NULL) ||
      (eeop->eeo_vport_free == NULL) ||
      (eeop->eeo_vport_mac_addr_add == NULL) ||
      (eeop->eeo_vport_mac_addr_del == NULL) ||
      (eeop->eeo_vadaptor_alloc == NULL) || (eeop->eeo_vadaptor_free == NULL) ||
      (eeop->eeo_vport_assign == NULL) || (eeop->eeo_vswitch_free == NULL)) {
    rc = ENOTSUP;
    goto fail1;
  }
  ...
  for (i = 0; i < num_vports; i++) {
    ...
    if ((rc = efx_evb_configure_vport(enp, vswitch_id, eeop, configp)) != 0)
      goto fail4;
  }
...
__checkReturn efx_rc_t efx_evb_vport_mac_set(__in efx_nic_t *enp,
                                             __in efx_vswitch_t *evp,
                                             __in efx_vport_id_t vport_id,
                                             __in_bcount(EFX_MAC_ADDR_LEN)
                                                 uint8_t *addrp) {
  const efx_evb_ops_t *eeop = enp->en_eeop;
  efx_rc_t rc;
  ...
  rc = eeop->eeo_vport_reconfigure(enp, evp->ev_vswitch_id, vport_id, NULL,
                                   addrp, NULL);
  if (rc != 0)
    goto fail3;
...
__checkReturn efx_rc_t efx_evb_vport_vlan_set(__in efx_nic_t *enp,
                                              __in efx_vswitch_t *evp,
                                              __in efx_vport_id_t vport_id,
                                              __in uint16_t vid) {
  const efx_evb_ops_t *eeop = enp->en_eeop;
  ...
  rc = eeop->eeo_vport_reconfigure(enp, evp->ev_vswitch_id, vport_id, &vid,
                                   NULL, NULL);
  if (rc != 0)
    goto fail2;
...
__checkReturn efx_rc_t efx_evb_vport_reset(
...
__checkReturn efx_rc_t efx_evb_vswitch_destroy(__in efx_nic_t *enp,
                                               __in efx_vswitch_t *evp) {
  const efx_evb_ops_t *eeop = enp->en_eeop;
  efx_vswitch_id_t vswitch_id;
  efx_rc_t rc;
  ...
  if ((eeop->eeo_vport_mac_addr_del == NULL) ||
      (eeop->eeo_vadaptor_free == NULL) || (eeop->eeo_vport_assign == NULL) ||
      (eeop->eeo_vport_free == NULL) || (eeop->eeo_vswitch_free == NULL)) {
    rc = ENOTSUP;
    goto fail1;
  }

  vswitch_id = evp->ev_vswitch_id;
  efx_evb_free_vports(enp, vswitch_id, evp->ev_num_vports, evp->ev_evcp);
...
__checkReturn efx_rc_t efx_evb_vport_stats(__in efx_nic_t *enp,
                                           __in efx_vswitch_t *evp,
                                           __in efx_vport_id_t vport_id,
  ...
  rc = eeop->eeo_vport_stats(enp, evp->ev_vswitch_id, vport_id, stats_bufferp);
  if (rc != 0)
    goto fail3;
