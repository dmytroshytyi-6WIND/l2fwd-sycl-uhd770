...
__checkReturn efx_rc_t efx_mac_pdu_set(__in efx_nic_t *enp, __in size_t pdu) {
...
__checkReturn efx_rc_t efx_mac_pdu_get(__in efx_nic_t *enp, __out size_t *pdu) {
  efx_port_t *epp = &(enp->en_port);
  const efx_mac_ops_t *emop = epp->ep_emop;
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mac_addr_set(__in efx_nic_t *enp,
                                        __in uint8_t *addr) {
  efx_port_t *epp = &(enp->en_port);
  const efx_mac_ops_t *emop = epp->ep_emop;
  uint8_t old_addr[6];
...
__checkReturn efx_rc_t efx_mac_filter_set(__in efx_nic_t *enp,
...
void efx_mac_filter_get_all_ucast_mcast(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mac_drain(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mac_up(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mac_fcntl_set(__in efx_nic_t *enp,
  ...
  if (fcntl & EFX_FCNTL_RESPOND)
    epp->ep_adv_cap_mask |= (1 << EFX_PHY_CAP_PAUSE | 1 << EFX_PHY_CAP_ASYM);
  else
    epp->ep_adv_cap_mask &= ~(1 << EFX_PHY_CAP_PAUSE | 1 << EFX_PHY_CAP_ASYM);
...
void efx_mac_fcntl_get(__in efx_nic_t *enp, __out unsigned int *fcntl_wantedp,
...
__checkReturn efx_rc_t efx_mac_multicast_list_set(__in efx_nic_t *enp,
                                                  __in_ecount(6 * count)
                                                      uint8_t const *addrs,
                                                  __in int count) {
  efx_port_t *epp = &(enp->en_port);
  ...
  if (old_mulcst_addr_count > 0) {
    ...
    EFSYS_KMEM_ALLOC(enp->en_esip, old_mulcst_addr_count * EFX_MAC_ADDR_LEN,
  ...
  if (old_mulcst_addr_count > 0) {
    EFSYS_KMEM_FREE(enp->en_esip, old_mulcst_addr_count * EFX_MAC_ADDR_LEN,
                    old_mulcst_addr_list);
  }
...
__checkReturn efx_rc_t efx_mac_filter_default_rxq_set(
...
void efx_mac_filter_default_rxq_clear(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
  const efx_mac_ops_t *emop = epp->ep_emop;
...
__checkReturn const char *efx_mac_stat_name(__in efx_nic_t *enp,
...
efx_mac_stats_mask_add_range(__inout_bcount(mask_size) uint32_t *maskp,
  ...
  for (el = 0; el < mask_npages; ++el) {
    ...
    if ((unsigned int)rngp->first > el_max || (unsigned int)rngp->last < el_min)
      continue;
...
efx_rc_t efx_mac_stats_mask_add_ranges(
  ...
  for (i = 0; i < rng_count; ++i) {
    if ((rc = efx_mac_stats_mask_add_range(maskp, mask_size, &rngp[i])) != 0)
      goto fail1;
  }
...
__checkReturn efx_rc_t efx_mac_stats_get_mask(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mac_stats_clear(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
  const efx_mac_ops_t *emop = epp->ep_emop;
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mac_stats_upload(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mac_stats_periodic(__in efx_nic_t *enp,
                                              __in efsys_mem_t *esmp,
...
__checkReturn efx_rc_t efx_mac_stats_update(__in efx_nic_t *enp,
                                            __in efsys_mem_t *esmp,
                                            __inout_ecount(EFX_MAC_NSTATS)
                                                efsys_stat_t *essp,
...
__checkReturn efx_rc_t efx_mac_select(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
...
static void siena_mac_multicast_hash_compute(__in_ecount(6 * count)
                                                 uint8_t const *addrs,
                                             __in int count,
...
siena_mac_multicast_list_set(__in efx_nic_t *enp) {
  ...
  siena_mac_multicast_hash_compute(
      epp->ep_mulcst_addr_list, epp->ep_mulcst_addr_count,
      &epp->ep_multicst_hash[0], &epp->ep_multicst_hash[1]);
