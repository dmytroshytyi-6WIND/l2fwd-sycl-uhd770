...
__checkReturn efx_rc_t ef10_proxy_auth_init(__in efx_nic_t *enp) {
  EFSYS_ASSERT(EFX_FAMILY_IS_EF10(enp));

  return (0);
}

void ef10_proxy_auth_fini(__in efx_nic_t *enp) {
  EFSYS_ASSERT(EFX_FAMILY_IS_EF10(enp));
}

static __checkReturn efx_rc_t efx_mcdi_proxy_configure(
    __in efx_nic_t *enp, __in boolean_t disable_proxy,
    __in uint64_t req_buffer_addr, __in uint64_t resp_buffer_addr,
    __in uint64_t stat_buffer_addr, __in size_t req_size, __in size_t resp_size,
    __in uint32_t block_cnt, __in uint8_t *op_maskp, __in size_t op_mask_size) {
  ...
  if (!disable_proxy) {
    ...
    MCDI_IN_SET_DWORD(req, PROXY_CONFIGURE_IN_STATUS_BUFF_ADDR_HI,
                      stat_buffer_addr >> 32);
    MCDI_IN_SET_DWORD(req, PROXY_CONFIGURE_IN_REQUEST_BLOCK_SIZE, req_size);
    MCDI_IN_SET_DWORD(req, PROXY_CONFIGURE_IN_REPLY_BLOCK_SIZE, resp_size);
    MCDI_IN_SET_DWORD(req, PROXY_CONFIGURE_IN_STATUS_BLOCK_SIZE,
                      MC_PROXY_STATUS_BUFFER_LEN);
    MCDI_IN_SET_DWORD(req, PROXY_CONFIGURE_IN_NUM_BLOCKS, block_cnt);
    memcpy(MCDI_IN2(req, efx_byte_t, PROXY_CONFIGURE_IN_ALLOWED_MCDI_MASK),
...
static __checkReturn efx_rc_t efx_mcdi_privilege_modify(
  ...
  if ((fn_group == MC_CMD_PRIVILEGE_MODIFY_IN_ONE) ||
    ...
    MCDI_IN_POPULATE_DWORD_2(req, PRIVILEGE_MODIFY_IN_FUNCTION,
                             PRIVILEGE_MODIFY_IN_FUNCTION_PF, pf_index,
                             PRIVILEGE_MODIFY_IN_FUNCTION_VF, vf_index);
  }

  MCDI_IN_SET_DWORD(req, PRIVILEGE_MODIFY_IN_ADD_MASK, add_privileges_mask);
...
static __checkReturn efx_rc_t efx_proxy_auth_fill_op_mask(
    __in_ecount(op_count) uint32_t *op_listp, __in size_t op_count,
    __out_ecount(op_mask_size) uint32_t *op_maskp, __in size_t op_mask_size) {
  efx_rc_t rc;
  uint32_t op;
...
__checkReturn efx_rc_t ef10_proxy_auth_mc_config(
    __in efx_nic_t *enp, __in_ecount(block_cnt) efsys_mem_t *request_bufferp,
    __in_ecount(block_cnt) efsys_mem_t *response_bufferp,
    __in_ecount(block_cnt) efsys_mem_t *status_bufferp, __in uint32_t block_cnt,
    __in_ecount(op_count) uint32_t *op_listp, __in size_t op_count) {
#define PROXY_OPS_MASK_SIZE                                                    \
  (EFX_DIV_ROUND_UP(MC_CMD_PROXY_CONFIGURE_IN_ALLOWED_MCDI_MASK_LEN,           \
                    sizeof(uint32_t)))
  ...
  if ((rc = efx_proxy_auth_fill_op_mask(op_listp, op_count, op_mask,
                                        PROXY_OPS_MASK_SIZE) != 0))
    goto fail1;

  if ((rc = efx_mcdi_proxy_configure(
           enp, B_FALSE, EFSYS_MEM_ADDR(request_bufferp),
           EFSYS_MEM_ADDR(response_bufferp), EFSYS_MEM_ADDR(status_bufferp),
           EFSYS_MEM_SIZE(request_bufferp) / block_cnt,
           EFSYS_MEM_SIZE(response_bufferp) / block_cnt, block_cnt,
           (uint8_t *)&op_mask, sizeof(op_mask))) != 0)
    goto fail2;
...
__checkReturn efx_rc_t ef10_proxy_auth_disable(__in efx_nic_t *enp) {
  efx_rc_t rc;

  if ((rc = efx_mcdi_proxy_configure(enp, B_TRUE, 0, 0, 0, 0, 0, 0, NULL, 0) !=
            0))
    goto fail1;
...
__checkReturn efx_rc_t ef10_proxy_auth_privilege_modify(
    __in efx_nic_t *enp, __in uint32_t fn_group, __in uint32_t pf_index,
    __in uint32_t vf_index, __in uint32_t add_privileges_mask,
    __in uint32_t remove_privileges_mask) {
...
efx_mcdi_privilege_mask_set(__in efx_nic_t *enp, __in uint32_t vf_index,
...
ef10_proxy_auth_set_privilege_mask(__in efx_nic_t *enp, __in uint32_t vf_index,
                                   __in uint32_t mask, __in uint32_t value) {
  return (efx_mcdi_privilege_mask_set(enp, vf_index, mask, value));
}

static __checkReturn efx_rc_t
efx_mcdi_proxy_complete(__in efx_nic_t *enp, __in uint32_t fn_index,
                        __in uint32_t proxy_result, __in uint32_t handle) {
...
__checkReturn efx_rc_t ef10_proxy_auth_complete_request(
    __in efx_nic_t *enp, __in uint32_t fn_index, __in uint32_t proxy_result,
    __in uint32_t handle) {
  return (efx_mcdi_proxy_complete(enp, fn_index, proxy_result, handle));
}

static __checkReturn efx_rc_t efx_mcdi_proxy_cmd(
    ...
    __in size_t response_size, __out_opt size_t *response_size_actualp) {
  efx_dword_t *inbufp;
  efx_mcdi_req_t req;
  efx_rc_t rc;
  ...
  MCDI_IN_POPULATE_DWORD_2(req, PROXY_CMD_IN_TARGET, PROXY_CMD_IN_TARGET_PF,
                           pf_index, PROXY_CMD_IN_TARGET_VF, vf_index);

  /* Proxied command should be located just after PROXY_CMD */
  memcpy(&inbufp[MC_CMD_PROXY_CMD_IN_LEN / sizeof(*inbufp)], request_bufferp,
         request_size);
...
__checkReturn efx_rc_t ef10_proxy_auth_get_privilege_mask(
    __in efx_nic_t *enp, __in uint32_t pf_index, __in uint32_t vf_index,
    __out uint32_t *maskp) {
  return (efx_mcdi_privilege_mask(enp, pf_index, vf_index, maskp));
}

__checkReturn efx_rc_t ef10_proxy_auth_exec_cmd(
