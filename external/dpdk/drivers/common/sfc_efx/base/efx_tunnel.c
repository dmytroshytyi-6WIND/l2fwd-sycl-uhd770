...
static void ef10_tunnel_fini(__in efx_nic_t *enp);
...
ef10_entry_staged(__in efx_tunnel_udp_entry_t *entry) {
...
static __checkReturn efx_rc_t efx_mcdi_set_tunnel_encap_udp_ports(
    __in efx_nic_t *enp, __in efx_tunnel_cfg_t *etcp, __in boolean_t unloading,
    __out boolean_t *resetting) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_SET_TUNNEL_ENCAP_UDP_PORTS_IN_LENMAX,
  ...
  if (etcp != NULL) {
    for (i = 0; i < etcp->etc_udp_entries_num; i++) {
      if (ef10_entry_staged(&etcp->etc_udp_entries[i]) != B_FALSE) {
        entries_num++;
      }
    }
  }
  ...
  req.emr_in_length = MC_CMD_SET_TUNNEL_ENCAP_UDP_PORTS_IN_LEN(entries_num);
  req.emr_out_buf = payload;
  req.emr_out_length = MC_CMD_SET_TUNNEL_ENCAP_UDP_PORTS_OUT_LEN;

  EFX_POPULATE_WORD_1(flags, MC_CMD_SET_TUNNEL_ENCAP_UDP_PORTS_IN_UNLOADING,
                      (unloading == B_TRUE) ? 1 : 0);
  MCDI_IN_SET_WORD(req, SET_TUNNEL_ENCAP_UDP_PORTS_IN_FLAGS,
                   EFX_WORD_FIELD(flags, EFX_WORD_0));

  MCDI_IN_SET_WORD(req, SET_TUNNEL_ENCAP_UDP_PORTS_IN_NUM_ENTRIES, entries_num);
  ...
  for (i = 0, entry = 0; entry < entries_num; ++entry, ++i) {
     ...
     */
    EFX_STATIC_ASSERT(sizeof(efx_dword_t) == TUNNEL_ENCAP_UDP_PORT_ENTRY_LEN);
    EFX_POPULATE_DWORD_2(MCDI_IN2(req, efx_dword_t,
  ...
  if (req.emr_out_length_used != MC_CMD_SET_TUNNEL_ENCAP_UDP_PORTS_OUT_LEN) {
    rc = EMSGSIZE;
    goto fail3;
  }

  *resetting = MCDI_OUT_WORD_FIELD(req, SET_TUNNEL_ENCAP_UDP_PORTS_OUT_FLAGS,
...
__checkReturn efx_rc_t efx_tunnel_init(__in efx_nic_t *enp) {
...
void efx_tunnel_fini(__in efx_nic_t *enp) {
  EFSYS_ASSERT3U(enp->en_magic, ==, EFX_NIC_MAGIC);
  EFSYS_ASSERT3U(enp->en_mod_flags, &, EFX_MOD_PROBE);
...
static __checkReturn efx_rc_t efx_tunnel_config_find_udp_tunnel_entry(
    __in efx_tunnel_cfg_t *etcp, __in uint16_t port,
  ...
  for (i = 0; i < etcp->etc_udp_entries_num; ++i) {
    ...
    if (p->etue_port == port && p->etue_state != EFX_TUNNEL_UDP_ENTRY_REMOVED) {
      *entryp = i;
      return (0);
    }
  }
...
__checkReturn efx_rc_t efx_tunnel_config_udp_add(
  ...
  if ((encp->enc_tunnel_encapsulations_supported & (1u << protocol)) == 0) {
    rc = ENOTSUP;
    goto fail2;
  }
  ...
  if (etcp->etc_udp_entries_num == encp->enc_tunnel_config_udp_entries_max) {
    rc = ENOSPC;
    goto fail4;
  }

  etcp->etc_udp_entries[etcp->etc_udp_entries_num].etue_port = port;
  etcp->etc_udp_entries[etcp->etc_udp_entries_num].etue_protocol = protocol;
 ...
 */
static unsigned int efx_tunnel_config_udp_do_remove(__in efx_tunnel_cfg_t *etcp,
                                                    __in unsigned int entry) {
  EFSYS_ASSERT3U(etcp->etc_udp_entries_num, >, 0);
  etcp->etc_udp_entries_num--;

  if (entry < etcp->etc_udp_entries_num) {
    memmove(&etcp->etc_udp_entries[entry], &etcp->etc_udp_entries[entry + 1],
 ...
 */
static unsigned int
efx_tunnel_config_udp_remove_prepare(__in efx_tunnel_cfg_t *etcp,
                                     __in unsigned int entry) {
  ...
  case EFX_TUNNEL_UDP_ENTRY_REMOVED:
    break;
  case EFX_TUNNEL_UDP_ENTRY_APPLIED:
    etcp->etc_udp_entries[entry].etue_state = EFX_TUNNEL_UDP_ENTRY_REMOVED;
    break;
  default:
    EFSYS_ASSERT(0);
    break;
...
__checkReturn efx_rc_t efx_tunnel_config_udp_remove(
    __in efx_nic_t *enp, __in uint16_t port /* host/cpu-endian */,
    __in efx_tunnel_protocol_t protocol) {
  efx_tunnel_cfg_t *etcp = &enp->en_tunnel_cfg;
  efsys_lock_state_t state;
...
static boolean_t efx_tunnel_table_all_available(__in efx_tunnel_cfg_t *etcp) {
  unsigned int i;
...
__checkReturn efx_rc_t efx_tunnel_config_clear(__in efx_nic_t *enp) {
  efx_tunnel_cfg_t *etcp = &enp->en_tunnel_cfg;
  efsys_lock_state_t state;
  unsigned int i;
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_tunnel_reconfigure(__in efx_nic_t *enp) {
  ...
  while (i < etcp->etc_udp_entries_num) {
    ...
    if (entry->etue_busy != B_FALSE) {
      ...
      case EFX_TUNNEL_UDP_ENTRY_APPLIED:
        break;
      case EFX_TUNNEL_UDP_ENTRY_ADDED:
        entry->etue_state = EFX_TUNNEL_UDP_ENTRY_APPLIED;
        break;
      case EFX_TUNNEL_UDP_ENTRY_REMOVED:
...
static __checkReturn boolean_t ef10_udp_encap_supported(__in efx_nic_t *enp) {
  ...
  return ((encp->enc_tunnel_encapsulations_supported & udp_tunnels_mask) == 0
              ? B_FALSE
              : B_TRUE);
}
...
static __checkReturn efx_rc_t ef10_tunnel_reconfigure(__in efx_nic_t *enp) {
  ...
  if (ef10_udp_encap_supported(enp) == B_FALSE) {
     ...
     */
    rc = efx_mcdi_set_tunnel_encap_udp_ports(enp, &etc, B_FALSE, &resetting);
    if (rc != 0) {
      /*
...
static void ef10_tunnel_fini(__in efx_nic_t *enp) {
  ...
  if (ef10_udp_encap_supported(enp) != B_FALSE) {
     ...
     */
    (void)efx_mcdi_set_tunnel_encap_udp_ports(enp, NULL, B_TRUE, &resetting);
  }
}
#endif /* EFSYS_OPT_MEDFORD || EFSYS_OPT_MEDFORD2 */
