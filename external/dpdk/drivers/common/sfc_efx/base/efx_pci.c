...
__checkReturn efx_rc_t efx_pci_config_next_ext_cap(
  ...
  if (*offsetp == 0) {
    *offsetp = ESE_GZ_PCI_BASE_CONFIG_SPACE_SIZE;
    ...
    rc = epop->epo_config_readd(
        espcp, *offsetp + (EFX_LOW_BIT(ESF_GZ_PCI_EXPRESS_XCAP_ID) / 8), &hdr);
    if (rc != 0) {
      rc = EIO;
      goto fail2;
    }
...
__checkReturn efx_rc_t efx_pci_config_find_next_ext_cap(
    __in efsys_pci_config_t *espcp, __in const efx_pci_ops_t *epop,
  ...
  while (1) {
    ...
    rc = epop->epo_config_readd(
        espcp, position + (EFX_LOW_BIT(ESF_GZ_PCI_EXPRESS_XCAP_ID) / 8), &hdr);
    if (rc != 0) {
      rc = EIO;
      goto fail3;
    }

    if (EFX_DWORD_FIELD(hdr, ESF_GZ_PCI_EXPRESS_XCAP_ID) == cap_id) {
      *offsetp = position;
      rc = 0;
      break;
    }
...
__checkReturn efx_rc_t efx_pci_find_next_xilinx_cap_table(
    __in efsys_pci_config_t *espcp, __in const efx_pci_ops_t *epop,
  ...
  while (1) {
    ...
    rc = efx_pci_config_find_next_ext_cap(
        espcp, epop, ESE_GZ_PCI_EXPRESS_XCAP_ID_VNDR, &cap_offset);
    if (rc != 0) {
      if (rc == ENOENT)
        break;
      else
     ...
     * not the locator.
     */
    rc = efx_pci_read_ext_cap_xilinx_table(espcp, epop, cap_offset, &tbl_bar,
                                           &tbl_offset);
    if (rc == 0) {
...
__checkReturn efx_rc_t efx_pci_read_ext_cap_xilinx_table(
    __in efsys_pci_config_t *espcp, __in const efx_pci_ops_t *epop,
  ...
  rc = epop->epo_config_readd(
      espcp, cap_offset + (EFX_LOW_BIT(ESF_GZ_PCI_EXPRESS_XCAP_ID) / 8),
      &cap_hdr);
  if (rc != 0) {
    rc = EIO;
    goto fail1;
  ...
  rc = epop->epo_config_readd(espcp,
                              vsec_offset + (EFX_LOW_BIT(ESF_GZ_VSEC_ID) / 8),
                              &vsec.eo_dword[0]);
  if (rc != 0) {
    rc = EIO;
    goto fail3;
  ...
  rc = epop->epo_config_readd(
      espcp, vsec_offset + (EFX_LOW_BIT(ESF_GZ_VSEC_TBL_BAR) / 8),
      &vsec.eo_dword[1]);
  if (rc != 0) {
    rc = EIO;
    goto fail6;
  ...
  if (vsec_len >= ESE_GZ_VSEC_LEN_HIGH_OFFT) {
    rc = epop->epo_config_readd(
        espcp, vsec_offset + (EFX_LOW_BIT(ESF_GZ_VSEC_TBL_OFF_HI) / 8),
        &vsec.eo_dword[2]);
    if (rc != 0) {
      rc = EIO;
      goto fail7;
...
__checkReturn efx_rc_t efx_pci_xilinx_cap_tbl_find(
  ...
  do {
     ...
     */
    EFSYS_BAR_READD(esbp,
                    offset + (EFX_LOW_BIT(ESF_GZ_CFGBAR_ENTRY_FORMAT) / 8),
