...
__checkReturn efx_rc_t siena_nvram_partn_size(__in efx_nic_t *enp,
                                              __in uint32_t partn,
                                              __out size_t *sizep) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nvram_partn_info(__in efx_nic_t *enp,
                                              __in uint32_t partn,
...
__checkReturn efx_rc_t siena_nvram_partn_lock(__in efx_nic_t *enp,
                                              __in uint32_t partn) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nvram_partn_read(__in efx_nic_t *enp,
                                              __in uint32_t partn,
...
__checkReturn efx_rc_t siena_nvram_partn_erase(__in efx_nic_t *enp,
                                               __in uint32_t partn,
                                               __in unsigned int offset,
                                               __in size_t size) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nvram_partn_write(__in efx_nic_t *enp,
                                               __in uint32_t partn,
                                               __in unsigned int offset,
                                               __out_bcount(size) caddr_t data,
                                               __in size_t size) {
...
siena_nvram_partn_unlock(__in efx_nic_t *enp, __in uint32_t partn,
  ...
  rc = efx_mcdi_nvram_update_finish(enp, partn, reboot, flags, verify_resultp);
  if (rc != 0)
    goto fail1;
...
__checkReturn efx_rc_t siena_nvram_type_to_partn(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t siena_nvram_test(__in efx_nic_t *enp) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
...
#define SIENA_DYNAMIC_CFG_SIZE(_nitems)                                        \
  (sizeof(siena_mc_dynamic_config_hdr_t) +                                     \
   ((_nitems) *                                                                \
    sizeof(((siena_mc_dynamic_config_hdr_t *)NULL)->fw_version[0])))

__checkReturn efx_rc_t siena_nvram_get_dynamic_cfg(
  ...
  if ((rc = siena_nvram_partn_read(enp, partn, 0, (caddr_t)dcfg,
                                   SIENA_NVRAM_CHUNK)) != 0)
    goto fail4;
...
invalid1:
   ...
   */
  EFX_POPULATE_DWORD_1(dcfg->magic, EFX_DWORD_0, SIENA_MC_DYNAMIC_CONFIG_MAGIC);
  ...
  EFX_POPULATE_DWORD_1(dcfg->dynamic_vpd_offset, EFX_DWORD_0, sizeof(*dcfg));
...
__checkReturn efx_rc_t siena_nvram_get_subtype(__in efx_nic_t *enp,
                                               __in uint32_t partn,
  ...
  if (req.emr_out_length_used < MC_CMD_GET_BOARD_CFG_OUT_FW_SUBTYPE_LIST_OFST +
                                    (partn + 1) * sizeof(efx_word_t)) {
    rc = ENOENT;
    goto fail3;
  }

  fw_list = MCDI_OUT2(req, efx_word_t, GET_BOARD_CFG_OUT_FW_SUBTYPE_LIST);
...
__checkReturn efx_rc_t siena_nvram_partn_get_version(__in efx_nic_t *enp,
                                                     __in uint32_t partn,
                                                     __out uint32_t *subtypep,
                                                     __out_ecount(4)
                                                         uint16_t version[4]) {
  ...
  for (i = 0; i < EFX_ARRAY_SIZE(siena_parttbl); i++) {
    ...
    dcfg_partn = (entry->port == 1) ? MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT0
...
__checkReturn efx_rc_t siena_nvram_partn_rw_start(__in efx_nic_t *enp,
                                                  __in uint32_t partn,
...
__checkReturn efx_rc_t siena_nvram_partn_set_version(__in efx_nic_t *enp,
                                                     __in uint32_t partn,
                                                     __in_ecount(4)
                                                         uint16_t version[4]) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
  ...
  dcfg_partn = (emip->emi_port == 1) ? MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT0
                                     : MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT1;
  ...
  if (hdr_length < required_hdr_length) {
    ...
    EFX_POPULATE_WORD_1(dcfg->length, EFX_WORD_0, hdr_length);
  }
  ...
  if (nitems < partn + 1) {
    nitems = partn + 1;
    EFX_POPULATE_DWORD_1(dcfg->num_fw_version_items, EFX_DWORD_0, nitems);
  }
