...
__checkReturn efx_rc_t ef10_intr_init(__in efx_nic_t *enp,
...
void ef10_intr_enable(__in efx_nic_t *enp) { _NOTE(ARGUNUSED(enp)) }

void ef10_intr_disable(__in efx_nic_t *enp) { _NOTE(ARGUNUSED(enp)) }

void ef10_intr_disable_unlocked(__in efx_nic_t *enp) { _NOTE(ARGUNUSED(enp)) }
...
__checkReturn efx_rc_t ef10_intr_trigger(__in efx_nic_t *enp,
...
void ef10_intr_status_line(__in efx_nic_t *enp, __out boolean_t *fatalp,
...
void ef10_intr_status_message(__in efx_nic_t *enp, __in unsigned int message,
...
void ef10_intr_fatal(__in efx_nic_t *enp) {
  /* EF10 fatal errors are reported via events */
  _NOTE(ARGUNUSED(enp))
}

void ef10_intr_fini(__in efx_nic_t *enp) { _NOTE(ARGUNUSED(enp)) }
