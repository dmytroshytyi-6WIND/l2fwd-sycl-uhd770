...
#define MCDI_STATIC_SENSOR_ASSERT(_field)                                      \
  EFX_STATIC_ASSERT(MC_CMD_SENSOR_STATE_##_field == EFX_MON_STAT_STATE_##_field)

static void mcdi_mon_decode_stats(
    __in efx_nic_t *enp, __in_bcount(sensor_mask_size) uint32_t *sensor_mask,
  ...
  for (sensor = 0; sensor < sensor_max; ++sensor) {
    ...
    if ((sensor % (MC_CMD_SENSOR_PAGE0_NEXT + 1)) == MC_CMD_SENSOR_PAGE0_NEXT) {
    ...
    if (stat != NULL && esmp != NULL && !EFSYS_MEM_IS_NULL(esmp)) {
      ...
      stat[id].emsv_value = (uint16_t)EFX_DWORD_FIELD(
          dword, MC_CMD_SENSOR_VALUE_ENTRY_TYPEDEF_VALUE);

      stat[id].emsv_state = (uint16_t)EFX_DWORD_FIELD(
          dword, MC_CMD_SENSOR_VALUE_ENTRY_TYPEDEF_STATE);

      stat[id].emsv_unit = efx_mon_get_stat_unit(id, &stat_unit)
                               ? stat_unit
...
__checkReturn efx_rc_t mcdi_mon_ev(__in efx_nic_t *enp, __in efx_qword_t *eqp,
  ...
  EFSYS_ASSERT3U(sensor, <, (8 * enp->en_nic_cfg.enc_mcdi_sensor_mask_size));
  ...
  EFSYS_ASSERT(
      (enp->en_nic_cfg
           .enc_mcdi_sensor_maskp[sensor / (MC_CMD_SENSOR_PAGE0_NEXT + 1)] &
...
static __checkReturn efx_rc_t efx_mcdi_read_sensors(__in efx_nic_t *enp,
...
static __checkReturn efx_rc_t efx_mcdi_sensor_info(__in efx_nic_t *enp,
                                                   __out_ecount(npages)
...
static __checkReturn efx_rc_t efx_mcdi_sensor_info_page(
    __in efx_nic_t *enp, __in uint32_t page, __out uint32_t *mask_part,
  ...
  EFSYS_ASSERT(sizeof(*limit_info) == MC_CMD_SENSOR_INFO_ENTRY_TYPEDEF_LEN);
  ...
  *mask_part = maskp->ed_u32[0];
  ...
  while (mask_copy) {
    ...
    if (mask_copy == (1U << MC_CMD_SENSOR_PAGE0_NEXT)) {
      ...
      limits->emlv_warning_min =
          EFX_QWORD_FIELD(*limit_info, MC_CMD_SENSOR_INFO_ENTRY_TYPEDEF_MIN1);

      limits->emlv_warning_max =
          EFX_QWORD_FIELD(*limit_info, MC_CMD_SENSOR_INFO_ENTRY_TYPEDEF_MAX1);

      limits->emlv_fatal_min =
          EFX_QWORD_FIELD(*limit_info, MC_CMD_SENSOR_INFO_ENTRY_TYPEDEF_MIN2);

      limits->emlv_fatal_max =
          EFX_QWORD_FIELD(*limit_info, MC_CMD_SENSOR_INFO_ENTRY_TYPEDEF_MAX2);
...
__checkReturn efx_rc_t mcdi_mon_stats_update(__in efx_nic_t *enp,
                                             __in efsys_mem_t *esmp,
                                             __inout_ecount(EFX_MON_NSTATS)
                                                 efx_mon_stat_value_t *values) {
...
static void lowest_set_bit(__in uint32_t input_mask,
...
mcdi_mon_limits_update(__in efx_nic_t *enp, __inout_ecount(EFX_MON_NSTATS)
  ...
  do {
    ...
    while (page_mask) {
      ...
      mcdi_index = page_index + (sizeof(page_mask) * 8 * page);
...
__checkReturn efx_rc_t mcdi_mon_cfg_build(__in efx_nic_t *enp) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
  uint32_t npages;
  efx_rc_t rc;
  ...
  /* Read mc sensor mask */
  if ((rc = efx_mcdi_sensor_info(enp, encp->enc_mcdi_sensor_maskp, npages)) !=
      0)
    goto fail4;

  /* Build monitor statistics mask */
  mcdi_mon_decode_stats(enp, encp->enc_mcdi_sensor_maskp,
                        encp->enc_mcdi_sensor_mask_size, NULL,
                        encp->enc_mon_stat_mask, NULL);
...
void mcdi_mon_cfg_free(__in efx_nic_t *enp) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
