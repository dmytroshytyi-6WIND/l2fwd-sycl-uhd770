...
typedef struct siena_mc_boot_hdr_s { /* GENERATED BY scripts/genfwdef */
  ...
  efx_byte_t
      generation; /* MC (Medford and later): MC partition generation when */
                  /* written to NVRAM. */
  ...
  efx_byte_t
      spi_phase_adj; /* SPI SDO/SCL phase adjustment, 0 is default (no adj) */
  efx_word_t xpm_sector; /* XPM (MEDFORD and later): The sector that contains */
  /* the key, or 0xffff if unsigned. (Otherwise set to 0) */
  efx_byte_t
      mumfw_subtype; /* MUM & SUC images: subtype. (Otherwise set to 0) */
  efx_byte_t reserved_b[3];   /* (set to 0) */
  efx_dword_t security_level; /* This number increases every time a serious
                                 security flaw */
...
#define SIENA_MC_EXPROM_COMBO_V2_MAGIC                                         \
  (0xB0070103) /* little-endian uint32_t                                       \
                */

typedef struct siena_mc_combo_rom_hdr_s { /* GENERATED BY scripts/genfwdef */
  efx_dword_t magic;                      /* = SIENA_MC_EXPROM_COMBO_MAGIC or
                                             SIENA_MC_EXPROM_COMBO_V2_MAGIC */
  union {
    struct {
      ...
      efx_byte_t
          infoblk_len; /* length of space reserved for one infoblk structure */
