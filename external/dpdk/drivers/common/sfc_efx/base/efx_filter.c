...
static __checkReturn efx_rc_t siena_filter_init(__in efx_nic_t *enp);

static void siena_filter_fini(__in efx_nic_t *enp);

static __checkReturn efx_rc_t siena_filter_restore(__in efx_nic_t *enp);

static __checkReturn efx_rc_t
siena_filter_add(__in efx_nic_t *enp, __inout efx_filter_spec_t *spec,
                 __in efx_filter_replacement_policy_t policy);

static __checkReturn efx_rc_t
siena_filter_delete(__in efx_nic_t *enp, __inout efx_filter_spec_t *spec);

static __checkReturn efx_rc_t siena_filter_supported_filters(
...
__checkReturn efx_rc_t efx_filter_insert(__in efx_nic_t *enp,
  ...
  return (efop->efo_add(enp, spec, EFX_FILTER_REPLACEMENT_HIGHER_PRIORITY));
...
__checkReturn efx_rc_t efx_filter_remove(__in efx_nic_t *enp,
                                         __inout efx_filter_spec_t *spec) {
  const efx_filter_ops_t *efop = enp->en_efop;
...
__checkReturn efx_rc_t efx_filter_restore(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_filter_init(__in efx_nic_t *enp) {
...
void efx_filter_fini(__in efx_nic_t *enp) {
  EFSYS_ASSERT3U(enp->en_magic, ==, EFX_NIC_MAGIC);
  EFSYS_ASSERT3U(enp->en_mod_flags, &, EFX_MOD_PROBE);
  EFSYS_ASSERT3U(enp->en_mod_flags, &, EFX_MOD_FILTER);
 ...
 */
__checkReturn efx_rc_t efx_filter_supported_filters(
    __in efx_nic_t *enp, __out_ecount(buffer_length) uint32_t *buffer,
    __in size_t buffer_length, __out size_t *list_lengthp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_filter_reconfigure(
  ...
  if (enp->en_efop->efo_reconfigure != NULL) {
    if ((rc = enp->en_efop->efo_reconfigure(enp, mac_addr, all_unicst, mulcst,
                                            all_mulcst, brdcst, addrs,
                                            count)) != 0)
      goto fail1;
  }
...
void efx_filter_spec_init_rx(__out efx_filter_spec_t *spec,
  ...
  EFSYS_ASSERT(
      (flags & ~(EFX_FILTER_FLAG_RX_RSS | EFX_FILTER_FLAG_RX_SCATTER)) == 0);
...
void efx_filter_spec_init_tx(__out efx_filter_spec_t *spec,
                             __in efx_txq_t *etp) {
  EFSYS_ASSERT3P(spec, !=, NULL);
 ...
 */
__checkReturn efx_rc_t efx_filter_spec_set_ipv4_local(
 ...
 */
__checkReturn efx_rc_t efx_filter_spec_set_ipv4_full(
...
void efx_filter_spec_set_ether_type(__inout efx_filter_spec_t *spec,
...
__checkReturn efx_rc_t efx_filter_spec_set_encap_type(
 ...
 * specification.
 */
static __checkReturn efx_rc_t efx_filter_spec_set_tunnel(
    __inout efx_filter_spec_t *spec, __in efx_tunnel_protocol_t encap_type,
    __in const uint8_t *vni_or_vsid, __in const uint8_t *inner_addr,
 ...
 * specification.
 */
__checkReturn efx_rc_t efx_filter_spec_set_vxlan(
    __inout efx_filter_spec_t *spec, __in const uint8_t *vni,
    __in const uint8_t *inner_addr, __in const uint8_t *outer_addr) {
  return efx_filter_spec_set_tunnel(spec, EFX_TUNNEL_PROTOCOL_VXLAN, vni,
 ...
 * specification.
 */
__checkReturn efx_rc_t efx_filter_spec_set_geneve(
    __inout efx_filter_spec_t *spec, __in const uint8_t *vni,
    __in const uint8_t *inner_addr, __in const uint8_t *outer_addr) {
  return efx_filter_spec_set_tunnel(spec, EFX_TUNNEL_PROTOCOL_GENEVE, vni,
                                    inner_addr, outer_addr);
}
 ...
 * specification.
 */
__checkReturn efx_rc_t efx_filter_spec_set_nvgre(
    __inout efx_filter_spec_t *spec, __in const uint8_t *vsid,
    __in const uint8_t *inner_addr, __in const uint8_t *outer_addr) {
  return efx_filter_spec_set_tunnel(spec, EFX_TUNNEL_PROTOCOL_NVGRE, vsid,
                                    inner_addr, outer_addr);
}
...
static __checkReturn efx_rc_t siena_filter_spec_from_gen_spec(
    __out siena_filter_spec_t *sf_spec, __in efx_filter_spec_t *gen_spec) {
  efx_rc_t rc;
  ...
  case EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_IP_PROTO |
    ...
    case EFX_IPPROTO_TCP:
      if (gen_spec->efs_flags & EFX_FILTER_FLAG_TX) {
        sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_TX_TCP_FULL
                                     : EFX_SIENA_FILTER_TX_TCP_WILD);
      } else {
        sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_RX_TCP_FULL
                                     : EFX_SIENA_FILTER_RX_TCP_WILD);
      }
      break;
    case EFX_IPPROTO_UDP:
      if (gen_spec->efs_flags & EFX_FILTER_FLAG_TX) {
        sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_TX_UDP_FULL
                                     : EFX_SIENA_FILTER_TX_UDP_WILD);
      } else {
        sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_RX_UDP_FULL
    ...
    if (gen_spec->efs_flags & EFX_FILTER_FLAG_TX) {
      ...
      if (sf_spec->sfs_type == EFX_SIENA_FILTER_RX_UDP_WILD) {
        port1 = gen_spec->efs_loc_port;
        port2 = rport;
      } else {
        port1 = rport;
  ...
  case EFX_FILTER_MATCH_LOC_MAC:
    if (gen_spec->efs_flags & EFX_FILTER_FLAG_TX) {
      sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_TX_MAC_FULL
    ...
    sf_spec->sfs_dword[1] =
        gen_spec->efs_loc_mac[2] << 24 | gen_spec->efs_loc_mac[3] << 16 |
        gen_spec->efs_loc_mac[4] << 8 | gen_spec->efs_loc_mac[5];
    sf_spec->sfs_dword[2] =
        gen_spec->efs_loc_mac[0] << 8 | gen_spec->efs_loc_mac[1];
    break;
 ...
 */
static uint16_t siena_filter_tbl_hash(__in uint32_t key) {
  uint16_t tmp;
 ...
 */
static uint16_t siena_filter_tbl_increment(__in uint32_t key) {
  return ((uint16_t)(key * 2 - 1));
}

static __checkReturn boolean_t
siena_filter_test_used(__in siena_filter_tbl_t *sftp, __in unsigned int index) {
  EFSYS_ASSERT3P(sftp->sft_bitmap, !=, NULL);
  return ((sftp->sft_bitmap[index / 32] & (1 << (index % 32))) != 0);
}

static void siena_filter_set_used(__in siena_filter_tbl_t *sftp,
...
static void siena_filter_clear_used(__in siena_filter_tbl_t *sftp,
                                    __in unsigned int index) {
  EFSYS_ASSERT3P(sftp->sft_bitmap, !=, NULL);
  sftp->sft_bitmap[index / 32] &= ~(1 << (index % 32));
...
siena_filter_tbl_id(__in siena_filter_type_t type) {
...
static void siena_filter_reset_search_depth(__inout siena_filter_t *sfp,
...
static void siena_filter_push_rx_limits(__in efx_nic_t *enp) {
  ...
  if (sfp->sf_tbl[EFX_SIENA_FILTER_TBL_RX_MAC].sft_size) {
    EFX_SET_OWORD_FIELD(oword, FRF_CZ_ETHERNET_FULL_SEARCH_LIMIT,
                        sfp->sf_depth[EFX_SIENA_FILTER_RX_MAC_FULL] +
                            FILTER_CTL_SRCH_FUDGE_FULL);
    EFX_SET_OWORD_FIELD(oword, FRF_CZ_ETHERNET_WILDCARD_SEARCH_LIMIT,
...
static void siena_filter_push_tx_limits(__in efx_nic_t *enp) {
  siena_filter_t *sfp = enp->en_filter.ef_siena_filter;
  efx_oword_t oword;
  ...
  if (sfp->sf_tbl[EFX_SIENA_FILTER_TBL_TX_MAC].sft_size != 0) {
    EFX_SET_OWORD_FIELD(oword, FRF_CZ_TX_ETH_FILTER_FULL_SEARCH_RANGE,
                        sfp->sf_depth[EFX_SIENA_FILTER_TX_MAC_FULL] +
                            FILTER_CTL_SRCH_FUDGE_FULL);
    EFX_SET_OWORD_FIELD(oword, FRF_CZ_TX_ETH_FILTER_WILD_SEARCH_RANGE,
...
siena_filter_build(__out efx_oword_t *filter, __in siena_filter_spec_t *spec) {
  uint32_t dword3;
  uint32_t key;
  ...
  case EFX_SIENA_FILTER_TBL_RX_IP: {
    ...
    EFX_POPULATE_OWORD_7(
        *filter, FRF_BZ_RSS_EN, (flags & EFX_FILTER_FLAG_RX_RSS) ? 1 : 0,
        FRF_BZ_SCATTER_EN, (flags & EFX_FILTER_FLAG_RX_SCATTER) ? 1 : 0,
        FRF_AZ_TCP_UDP, is_udp, FRF_AZ_RXQ_ID, spec->sfs_dmaq_id, EFX_DWORD_2,
        spec->sfs_dword[2], EFX_DWORD_1, spec->sfs_dword[1], EFX_DWORD_0,
        spec->sfs_dword[0]);
    dword3 = is_udp;
    break;
  }
  ...
  case EFX_SIENA_FILTER_TBL_RX_MAC: {
    ...
    EFX_POPULATE_OWORD_7(
        *filter, FRF_CZ_RMFT_RSS_EN, (flags & EFX_FILTER_FLAG_RX_RSS) ? 1 : 0,
        FRF_CZ_RMFT_SCATTER_EN, (flags & EFX_FILTER_FLAG_RX_SCATTER) ? 1 : 0,
        FRF_CZ_RMFT_RXQ_ID, spec->sfs_dmaq_id, FRF_CZ_RMFT_WILDCARD_MATCH,
        is_wild, FRF_CZ_RMFT_DEST_MAC_DW1, spec->sfs_dword[2],
        FRF_CZ_RMFT_DEST_MAC_DW0, spec->sfs_dword[1], FRF_CZ_RMFT_VLAN_ID,
        spec->sfs_dword[0]);
    dword3 = is_wild;
    break;
  }
  ...
  case EFX_SIENA_FILTER_TBL_TX_IP: {
    ...
    EFX_POPULATE_OWORD_5(*filter, FRF_CZ_TIFT_TCP_UDP, is_udp,
                         FRF_CZ_TIFT_TXQ_ID, spec->sfs_dmaq_id, EFX_DWORD_2,
                         spec->sfs_dword[2], EFX_DWORD_1, spec->sfs_dword[1],
  ...
  case EFX_SIENA_FILTER_TBL_TX_MAC: {
    ...
    EFX_POPULATE_OWORD_5(*filter, FRF_CZ_TMFT_TXQ_ID, spec->sfs_dmaq_id,
  ...
  key = spec->sfs_dword[0] ^ spec->sfs_dword[1] ^ spec->sfs_dword[2] ^ dword3;

  return (key);
}

static __checkReturn efx_rc_t
siena_filter_push_entry(__inout efx_nic_t *enp, __in siena_filter_type_t type,
                        __in int index, __in efx_oword_t *eop) {
  efx_rc_t rc;
  ...
  case EFX_SIENA_FILTER_RX_UDP_FULL:
  case EFX_SIENA_FILTER_RX_UDP_WILD:
    EFX_BAR_TBL_WRITEO(enp, FR_AZ_RX_FILTER_TBL0, index, eop, B_TRUE);
    break;

  case EFX_SIENA_FILTER_RX_MAC_FULL:
  case EFX_SIENA_FILTER_RX_MAC_WILD:
    EFX_BAR_TBL_WRITEO(enp, FR_CZ_RX_MAC_FILTER_TBL0, index, eop, B_TRUE);
    break;
  ...
  case EFX_SIENA_FILTER_TX_UDP_FULL:
  case EFX_SIENA_FILTER_TX_UDP_WILD:
    EFX_BAR_TBL_WRITEO(enp, FR_CZ_TX_FILTER_TBL0, index, eop, B_TRUE);
    break;

  case EFX_SIENA_FILTER_TX_MAC_FULL:
  case EFX_SIENA_FILTER_TX_MAC_WILD:
    EFX_BAR_TBL_WRITEO(enp, FR_CZ_TX_MAC_FILTER_TBL0, index, eop, B_TRUE);
    break;
...
static __checkReturn boolean_t
siena_filter_equal(__in const siena_filter_spec_t *left,
  ...
  if (memcmp(left->sfs_dword, right->sfs_dword, sizeof(left->sfs_dword)))
    return (B_FALSE);
...
static __checkReturn efx_rc_t siena_filter_search(
    __in siena_filter_tbl_t *sftp, __in siena_filter_spec_t *spec,
    __in uint32_t key, __in boolean_t for_insert, __out int *filter_index,
  ...
  for (;;) {
    ...
    if (siena_filter_test_used(sftp, filter_idx)
            ? siena_filter_equal(spec, &sftp->sft_spec[filter_idx])
            : for_insert) {
...
static void siena_filter_clear_entry(__in efx_nic_t *enp,
  ...
  if (siena_filter_test_used(sftp, index)) {
    ...
    EFX_ZERO_OWORD(filter);
    siena_filter_push_entry(enp, sftp->sft_spec[index].sfs_type, index,
                            &filter);

    memset(&sftp->sft_spec[index], 0, sizeof(sftp->sft_spec[0]));
  }
}

void siena_filter_tbl_clear(__in efx_nic_t *enp,
                            __in siena_filter_tbl_id_t tbl_id) {
  siena_filter_t *sfp = enp->en_filter.ef_siena_filter;
...
static __checkReturn efx_rc_t siena_filter_init(__in efx_nic_t *enp) {
  ...
  for (tbl_id = 0; tbl_id < EFX_SIENA_FILTER_NTBLS; tbl_id++) {
    ...
    EFX_STATIC_ASSERT(sizeof(sftp->sft_bitmap[0]) == sizeof(uint32_t));
    bitmap_size = (sftp->sft_size + (sizeof(uint32_t) * 8) - 1) / 8;
    ...
    EFSYS_KMEM_ALLOC(enp->en_esip, sftp->sft_size * sizeof(*sftp->sft_spec),
                     sftp->sft_spec);
    if (!sftp->sft_spec) {
      rc = ENOMEM;
      goto fail4;
    }
    memset(sftp->sft_spec, 0, sftp->sft_size * sizeof(*sftp->sft_spec));
  }
...
static void siena_filter_fini(__in efx_nic_t *enp) {
  ...
  for (tbl_id = 0; tbl_id < EFX_SIENA_FILTER_NTBLS; tbl_id++) {
    ...
    if (sftp->sft_bitmap != NULL) {
      EFSYS_KMEM_FREE(enp->en_esip, bitmap_size, sftp->sft_bitmap);
      sftp->sft_bitmap = NULL;
    }

    if (sftp->sft_spec != NULL) {
      EFSYS_KMEM_FREE(enp->en_esip, sftp->sft_size * sizeof(*sftp->sft_spec),
                      sftp->sft_spec);
      sftp->sft_spec = NULL;
    }
  }
...
static __checkReturn efx_rc_t siena_filter_restore(__in efx_nic_t *enp) {
  ...
  for (tbl_id = 0; tbl_id < EFX_SIENA_FILTER_NTBLS; tbl_id++) {
    sftp = &sfp->sf_tbl[tbl_id];
    for (filter_idx = 0; filter_idx < sftp->sft_size; filter_idx++) {
      if (!siena_filter_test_used(sftp, filter_idx))
        continue;
      ...
      if ((rc = siena_filter_push_entry(enp, spec->sfs_type, filter_idx,
                                        &filter)) != 0)
        goto fail2;
    }
  }
...
siena_filter_add(__in efx_nic_t *enp, __inout efx_filter_spec_t *spec,
  ...
  rc = siena_filter_search(sftp, &sf_spec, key, B_TRUE, &filter_idx, &depth);
  if (rc != 0)
    goto fail3;
  ...
  *saved_sf_spec = sf_spec;
  ...
  siena_filter_push_entry(enp, sf_spec.sfs_type, filter_idx, &filter);
...
siena_filter_delete(__in efx_nic_t *enp, __inout efx_filter_spec_t *spec) {
  ...
  rc = siena_filter_search(sftp, &sf_spec, key, B_FALSE, &filter_idx, &depth);
  if (rc != 0)
    goto fail2;
...
static __checkReturn efx_rc_t siena_filter_supported_filters(
  ...
  rx_matches[index++] = EFX_FILTER_MATCH_ETHER_TYPE |
                        EFX_FILTER_MATCH_IP_PROTO | EFX_FILTER_MATCH_LOC_HOST |
                        EFX_FILTER_MATCH_LOC_PORT | EFX_FILTER_MATCH_REM_HOST |
  ...
  if (enp->en_features & EFX_FEATURE_MAC_HEADER_FILTERS) {
    rx_matches[index++] = EFX_FILTER_MATCH_OUTER_VID | EFX_FILTER_MATCH_LOC_MAC;
