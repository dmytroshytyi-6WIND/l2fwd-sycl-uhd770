...
__checkReturn efx_rc_t ef10_mcdi_init(__in efx_nic_t *enp,
...
void ef10_mcdi_fini(__in efx_nic_t *enp) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
...
void ef10_mcdi_get_timeout(__in efx_nic_t *enp, __in efx_mcdi_req_t *emrp,
...
void ef10_mcdi_send_request(__in efx_nic_t *enp,
  ...
  EFX_POPULATE_DWORD_1(dword, EFX_DWORD_0, EFSYS_MEM_ADDR(esmp) & 0xffffffff);
  switch (enp->en_family) {
#if EFSYS_OPT_RIVERHEAD
  case EFX_FAMILY_RIVERHEAD:
    EFX_BAR_FCW_WRITED(enp, ER_GZ_MC_DB_HWRD_REG, &dword);
...
__checkReturn boolean_t ef10_mcdi_poll_response(__in efx_nic_t *enp) {
  const efx_mcdi_transport_t *emtp = enp->en_mcdi.em_emtp;
  efsys_mem_t *esmp = emtp->emt_dma_mem;
  efx_dword_t hdr;
...
void ef10_mcdi_read_response(__in efx_nic_t *enp,
...
efx_rc_t ef10_mcdi_poll_reboot(__in efx_nic_t *enp) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
  efx_dword_t dword;
  uint32_t old_status;
  uint32_t new_status;
...
ef10_mcdi_feature_supported(__in efx_nic_t *enp, __in efx_mcdi_feature_id_t id,
  ...
  case EFX_MCDI_FEATURE_MACADDR_CHANGE:
     ...
     */
    *supportedp = EFX_MCDI_HAVE_PRIVILEGE(privilege_mask, CHANGE_MAC) ||
  ...
  case EFX_MCDI_FEATURE_MAC_SPOOFING:
     ...
     */
    *supportedp = EFX_MCDI_HAVE_PRIVILEGE(privilege_mask, MAC_SPOOFING_TX) ||
                  EFX_MCDI_HAVE_PRIVILEGE(privilege_mask, MAC_SPOOFING) ||
                  EFX_MCDI_HAVE_PRIVILEGE(privilege_mask, ADMIN);
    break;
  default:
