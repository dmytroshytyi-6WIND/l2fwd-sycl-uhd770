...
static __checkReturn efx_rc_t efx_mcdi_set_evq_tmr(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t ef10_ev_init(__in efx_nic_t *enp) {
  _NOTE(ARGUNUSED(enp))
  return (0);
}

void ef10_ev_fini(__in efx_nic_t *enp){_NOTE(ARGUNUSED(enp))}
...
void ef10_ev_qdestroy(__in efx_evq_t *eep) {
...
__checkReturn efx_rc_t ef10_ev_qprime(__in efx_evq_t *eep,
  ...
  if (enp->en_nic_cfg.enc_bug35388_workaround) {
    EFX_STATIC_ASSERT(EF10_EVQ_MINNEVS > (1 << ERF_DD_EVQ_IND_RPTR_WIDTH));
    EFX_STATIC_ASSERT(EF10_EVQ_MAXNEVS < (1 << 2 * ERF_DD_EVQ_IND_RPTR_WIDTH));
    ...
    EFX_BAR_VI_WRITED(enp, ER_DD_EVQ_INDIRECT, eep->ee_index, &dword, B_FALSE);
    ...
    EFX_BAR_VI_WRITED(enp, ER_DZ_EVQ_RPTR_REG, eep->ee_index, &dword, B_FALSE);
  }

  return (0);
}

static __checkReturn efx_rc_t efx_mcdi_driver_event(__in efx_nic_t *enp,
                                                    __in uint32_t evq,
...
void ef10_ev_qpost(__in efx_evq_t *eep, __in uint16_t data) {
  efx_nic_t *enp = eep->ee_enp;
  efx_qword_t event;

  EFX_POPULATE_QWORD_3(event, ESF_DZ_DRV_CODE, ESE_DZ_EV_CODE_DRV_GEN_EV,
                       ESF_DZ_DRV_SUB_CODE, 0, ESF_DZ_DRV_SUB_DATA_DW0,
                       (uint32_t)data);

  (void)efx_mcdi_driver_event(enp, eep->ee_index, event);
}

__checkReturn efx_rc_t ef10_ev_qmoderate(__in efx_evq_t *eep,
  ...
  if (encp->enc_bug61265_workaround) {
    ...
    if (encp->enc_bug35388_workaround) {
      EFX_POPULATE_DWORD_3(
          dword, ERF_DD_EVQ_IND_TIMER_FLAGS, EFE_DD_EVQ_IND_TIMER_FLAGS,
          ERF_DD_EVQ_IND_TIMER_MODE, mode, ERF_DD_EVQ_IND_TIMER_VAL, ticks);
      EFX_BAR_VI_WRITED(enp, ER_DD_EVQ_INDIRECT, eep->ee_index, &dword, 0);
    } else {
      /*
      ...
      EFX_POPULATE_DWORD_3(dword, ERF_DZ_TC_TIMER_MODE, mode,
                           ...
                           ticks);
      EFX_BAR_VI_WRITED(enp, ER_DZ_EVQ_TMR_REG, eep->ee_index, &dword, 0);
    }
  }
...
#if EFSYS_OPT_QSTATS
void ef10_ev_qstats_update(__in efx_evq_t *eep,
...
static __checkReturn boolean_t ef10_ev_rx_packed_stream(
...
deliver:
  ...
  should_abort = eecp->eec_rx_ps(arg, label, current_id, pkt_count, flags);
...
__checkReturn boolean_t ef10_ev_mcdi(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                     __in const efx_ev_callbacks_t *eecp,
                                     __in_opt void *arg) {
  efx_nic_t *enp = eep->ee_enp;
  unsigned int code;
  ...
  case MCDI_EVENT_CODE_PROXY_RESPONSE:
     ...
     */
    efx_mcdi_ev_proxy_response(enp, MCDI_EV_FIELD(eqp, PROXY_RESPONSE_HANDLE),
  ...
  case MCDI_EVENT_CODE_TX_ERR: {
    ...
    EFSYS_PROBE2(tx_descq_err, uint32_t, EFX_QWORD_FIELD(*eqp, EFX_DWORD_1),
  ...
  case MCDI_EVENT_CODE_RX_ERR: {
    ...
    EFSYS_PROBE2(rx_descq_err, uint32_t, EFX_QWORD_FIELD(*eqp, EFX_DWORD_1),
                 uint32_t, EFX_QWORD_FIELD(*eqp, EFX_DWORD_0));
...
void ef10_ev_rxlabel_init(__in efx_evq_t *eep, __in efx_rxq_t *erp,
...
void ef10_ev_rxlabel_fini(__in efx_evq_t *eep, __in unsigned int label) {
  efx_evq_rxq_state_t *eersp;
