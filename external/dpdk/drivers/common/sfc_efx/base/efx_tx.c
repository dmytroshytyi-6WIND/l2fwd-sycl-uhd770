...
static __checkReturn efx_rc_t siena_tx_init(__in efx_nic_t *enp);

static void siena_tx_fini(__in efx_nic_t *enp);

static __checkReturn efx_rc_t siena_tx_qcreate(
...
static void siena_tx_qdestroy(__in efx_txq_t *etp);

static __checkReturn efx_rc_t siena_tx_qpost(__in efx_txq_t *etp,
                                             __in_ecount(ndescs)
                                                 efx_buffer_t *eb,
...
static void siena_tx_qpush(__in efx_txq_t *etp, __in unsigned int added,
                           __in unsigned int pushed);

static __checkReturn efx_rc_t siena_tx_qpace(__in efx_txq_t *etp,
                                             __in unsigned int ns);

static __checkReturn efx_rc_t siena_tx_qflush(__in efx_txq_t *etp);

static void siena_tx_qenable(__in efx_txq_t *etp);

__checkReturn efx_rc_t siena_tx_qdesc_post(__in efx_txq_t *etp,
                                           __in_ecount(ndescs) efx_desc_t *ed,
                                           __in unsigned int ndescs,
                                           __in unsigned int completed,
                                           __inout unsigned int *addedp);

void siena_tx_qdesc_dma_create(__in efx_txq_t *etp, __in efsys_dma_addr_t addr,
                               __in size_t size, __in boolean_t eop,
                               __out efx_desc_t *edp);

#if EFSYS_OPT_QSTATS
static void siena_tx_qstats_update(__in efx_txq_t *etp,
...
__checkReturn efx_rc_t efx_tx_init(__in efx_nic_t *enp) {
...
void efx_tx_fini(__in efx_nic_t *enp) {
...
__checkReturn size_t efx_txq_size(__in const efx_nic_t *enp,
                                  __in unsigned int ndescs) {
...
__checkReturn unsigned int efx_txq_nbufs(__in const efx_nic_t *enp,
                                         __in unsigned int ndescs) {
  return (EFX_DIV_ROUND_UP(efx_txq_size(enp, ndescs), EFX_BUF_SIZE));
}

__checkReturn efx_rc_t efx_tx_qcreate(
    __in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
    __in efsys_mem_t *esmp, __in size_t ndescs, __in uint32_t id,
    __in uint16_t flags, __in efx_evq_t *eep, __deref_out efx_txq_t **etpp,
    __out unsigned int *addedp) {
  const efx_tx_ops_t *etxop = enp->en_etxop;
  efx_txq_t *etp;
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  ...
  EFSYS_ASSERT3U(enp->en_tx_qcount + 1, <, enp->en_nic_cfg.enc_txq_limit);

  EFSYS_ASSERT(ISP2(encp->enc_txq_max_ndescs));
  EFSYS_ASSERT(ISP2(encp->enc_txq_min_ndescs));

  if (!ISP2(ndescs) || ndescs < encp->enc_txq_min_ndescs ||
  ...
  *addedp = 0;
  ...
  if ((rc = etxop->etxo_qcreate(enp, index, label, esmp, ndescs, id, flags, eep,
                                etp, addedp)) != 0)
    goto fail3;
...
void efx_tx_qdestroy(__in efx_txq_t *etp) {
...
__checkReturn efx_rc_t efx_tx_qpost(__in efx_txq_t *etp,
...
void efx_tx_qpush(__in efx_txq_t *etp, __in unsigned int added,
                  __in unsigned int pushed) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
...
__checkReturn efx_rc_t efx_tx_qpace(__in efx_txq_t *etp, __in unsigned int ns) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_tx_qflush(__in efx_txq_t *etp) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
  efx_rc_t rc;
...
void efx_tx_qenable(__in efx_txq_t *etp) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
...
__checkReturn efx_rc_t efx_tx_qpio_enable(__in efx_txq_t *etp) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
  efx_rc_t rc;
...
void efx_tx_qpio_disable(__in efx_txq_t *etp) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
...
__checkReturn efx_rc_t efx_tx_qpio_write(__in efx_txq_t *etp,
...
__checkReturn efx_rc_t efx_tx_qpio_post(__in efx_txq_t *etp,
  ...
  if (etxop->etxo_qpio_post != NULL) {
    if ((rc = etxop->etxo_qpio_post(etp, pkt_length, completed, addedp)) != 0)
      goto fail1;
    return (0);
  }
...
__checkReturn efx_rc_t efx_tx_qdesc_post(__in efx_txq_t *etp,
                                         __in_ecount(ndescs) efx_desc_t *ed,
                                         __in unsigned int ndescs,
                                         __in unsigned int completed,
                                         __inout unsigned int *addedp) {
...
void efx_tx_qdesc_dma_create(__in efx_txq_t *etp, __in efsys_dma_addr_t addr,
                             __in size_t size, __in boolean_t eop,
                             __out efx_desc_t *edp) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
...
void efx_tx_qdesc_tso_create(__in efx_txq_t *etp, __in uint16_t ipv4_id,
                             __in uint32_t tcp_seq, __in uint8_t tcp_flags,
                             __out efx_desc_t *edp) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
...
void efx_tx_qdesc_tso2_create(__in efx_txq_t *etp, __in uint16_t ipv4_id,
                              __in uint16_t outer_ipv4_id,
                              __in uint32_t tcp_seq, __in uint16_t mss,
  ...
  etxop->etxo_qdesc_tso2_create(etp, ipv4_id, outer_ipv4_id, tcp_seq, mss, edp,
                                count);
}

void efx_tx_qdesc_vlantci_create(__in efx_txq_t *etp, __in uint16_t tci,
                                 __out efx_desc_t *edp) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
...
void efx_tx_qdesc_checksum_create(__in efx_txq_t *etp, __in uint16_t flags,
                                  __out efx_desc_t *edp) {
  efx_nic_t *enp = etp->et_enp;
  const efx_tx_ops_t *etxop = enp->en_etxop;
...
#if EFSYS_OPT_QSTATS
void efx_tx_qstats_update(__in efx_txq_t *etp,
...
#define EFX_TX_DESC(_etp, _addr, _size, _eop, _added)                          \
  do {                                                                         \
    ...
    offset = id * sizeof(efx_qword_t);                                         \
                                                                               \
    EFSYS_PROBE5(tx_post, unsigned int, (_etp)->et_index, unsigned int, id,    \
                 efsys_dma_addr_t, (_addr), size_t, (_size), boolean_t,        \
                 (_eop));                                                      \
                                                                               \
    EFX_POPULATE_QWORD_4(                                                      \
        qword, FSF_AZ_TX_KER_CONT, (_eop) ? 0 : 1, FSF_AZ_TX_KER_BYTE_COUNT,   \
        (uint32_t)(_size), FSF_AZ_TX_KER_BUF_ADDR_DW0,                         \
        (uint32_t)((_addr)&0xffffffff), FSF_AZ_TX_KER_BUF_ADDR_DW1,            \
        (uint32_t)((_addr) >> 32));                                            \
...
static __checkReturn efx_rc_t siena_tx_qpost(__in efx_txq_t *etp,
  ...
  for (i = 0; i < ndescs; i++) {
     ...
     */
    EFSYS_ASSERT(
        EFX_P2ROUNDUP(efsys_dma_addr_t, start + 1,
...
static void siena_tx_qpush(__in efx_txq_t *etp, __in unsigned int added,
  ...
  EFX_DMA_SYNC_QUEUE_FOR_DEVICE(etp->et_esmp, etp->et_mask + 1,
                                SIENA_TXQ_DESC_SIZE, wptr,
                                pushed & etp->et_mask);
  EFSYS_PIO_WRITE_BARRIER();
  EFX_BAR_TBL_WRITED3(enp, FR_BZ_TX_DESC_UPD_REGP0, etp->et_index, &dword,
                      B_FALSE);
}
...
static __checkReturn efx_rc_t siena_tx_qpace(__in efx_txq_t *etp,
  ...
  EFX_BAR_TBL_WRITEO(enp, FR_AZ_TX_PACE_TBL, etp->et_index, &oword, B_TRUE);
...
static void siena_tx_qenable(__in efx_txq_t *etp) {
  ...
  EFX_BAR_TBL_READO(enp, FR_AZ_TX_DESC_PTR_TBL, etp->et_index, &oword, B_TRUE);

  EFSYS_PROBE5(tx_descq_ptr, unsigned int, etp->et_index, uint32_t,
               EFX_OWORD_FIELD(oword, EFX_DWORD_3), uint32_t,
               EFX_OWORD_FIELD(oword, EFX_DWORD_2), uint32_t,
               EFX_OWORD_FIELD(oword, EFX_DWORD_1), uint32_t,
               EFX_OWORD_FIELD(oword, EFX_DWORD_0));
  ...
  EFX_BAR_TBL_WRITEO(enp, FR_AZ_TX_DESC_PTR_TBL, etp->et_index, &oword, B_TRUE);
}
...
static __checkReturn efx_rc_t siena_tx_qcreate(
  ...
  *addedp = 0;
  ...
  EFX_POPULATE_OWORD_6(
      oword, FRF_AZ_TX_DESCQ_BUF_BASE_ID, id, FRF_AZ_TX_DESCQ_EVQ_ID,
      eep->ee_index, FRF_AZ_TX_DESCQ_OWNER_ID, 0, FRF_AZ_TX_DESCQ_LABEL, label,
      FRF_AZ_TX_DESCQ_SIZE, size, FRF_AZ_TX_DESCQ_TYPE, 0);
...
__checkReturn efx_rc_t siena_tx_qdesc_post(__in efx_txq_t *etp,
  ...
  EFSYS_PROBE3(tx_desc_post, unsigned int, etp->et_index, unsigned int, added,
               unsigned int, ndescs);
...
void siena_tx_qdesc_dma_create(__in efx_txq_t *etp, __in efsys_dma_addr_t addr,
   ...
   */
  EFSYS_ASSERT(
      EFX_P2ROUNDUP(efsys_dma_addr_t, addr + 1,
                    etp->et_enp->en_nic_cfg.enc_tx_dma_desc_boundary) >=
      addr + size);

  EFSYS_PROBE4(tx_desc_dma_create, unsigned int, etp->et_index,
               efsys_dma_addr_t, addr, size_t, size, boolean_t, eop);

  EFX_POPULATE_QWORD_4(
      edp->ed_eq, FSF_AZ_TX_KER_CONT, eop ? 0 : 1, FSF_AZ_TX_KER_BYTE_COUNT,
      (uint32_t)size, FSF_AZ_TX_KER_BUF_ADDR_DW0, (uint32_t)(addr & 0xffffffff),
      FSF_AZ_TX_KER_BUF_ADDR_DW1, (uint32_t)(addr >> 32));
}
...
const char *efx_tx_qstat_name(__in efx_nic_t *enp, __in unsigned int id) {
  _NOTE(ARGUNUSED(enp))
  EFSYS_ASSERT3U(enp->en_magic, ==, EFX_NIC_MAGIC);
...
static void siena_tx_fini(__in efx_nic_t *enp) { _NOTE(ARGUNUSED(enp)) }
