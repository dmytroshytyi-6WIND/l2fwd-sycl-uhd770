...
__checkReturn efx_rc_t rhead_rx_init(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
void rhead_rx_fini(__in efx_nic_t *enp) { ef10_rx_fini(enp); }

#if EFSYS_OPT_RX_SCATTER
__checkReturn efx_rc_t rhead_rx_scatter_enable(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t rhead_rx_scale_context_alloc(
...
__checkReturn efx_rc_t rhead_rx_scale_context_free(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t rhead_rx_scale_mode_set(__in efx_nic_t *enp,
                                               __in uint32_t rss_context,
...
__checkReturn efx_rc_t rhead_rx_scale_key_set(__in efx_nic_t *enp,
                                              __in uint32_t rss_context,
...
__checkReturn efx_rc_t rhead_rx_scale_tbl_set(
...
__checkReturn uint32_t rhead_rx_prefix_hash(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t rhead_rx_prefix_pktlen(__in efx_nic_t *enp,
                                              __in uint8_t *buffer,
...
void rhead_rx_qpost(__in efx_rxq_t *erp,
...
void rhead_rx_qpush(__in efx_rxq_t *erp, __in unsigned int added,
...
__checkReturn efx_rc_t rhead_rx_qflush(__in efx_rxq_t *erp) {
...
void rhead_rx_qenable(__in efx_rxq_t *erp) { _NOTE(ARGUNUSED(erp)) }

static __checkReturn efx_rc_t
efx_mcdi_get_rx_prefix_ids(__in efx_nic_t *enp, __in uint32_t mcdi_fields_mask,
                           __in unsigned int max_ids, __out unsigned int *nids,
...
efx_rx_prefix_field_map_to_mcdi(__in efx_rx_prefix_field_t field) {
...
static __checkReturn efx_rc_t efx_rx_prefix_fields_mask_to_mcdi(
    __in uint32_t fields_mask, __out uint32_t *mcdi_fields_maskp) {
...
static __checkReturn efx_rc_t
efx_mcdi_query_rx_prefix_id(__in efx_nic_t *enp, __in uint32_t prefix_id,
  ...
  response_len =
      req.emr_out_length_used - MC_CMD_QUERY_RX_PREFIX_ID_OUT_RESPONSE_OFST;
...
static __checkReturn efx_rc_t
rhead_rx_choose_prefix_id(__in efx_nic_t *enp, __in uint32_t fields_mask,
                          __out efx_rx_prefix_layout_t *erplp) {
...
__checkReturn efx_rc_t rhead_rx_qcreate(
    __in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
    __in efx_rxq_type_t type, __in const efx_rxq_type_data_t *type_data,
  ...
  EFX_STATIC_ASSERT(EFX_EV_RX_NLABELS <= (1 << ESF_GZ_EV_RXPKTS_Q_LABEL_WIDTH));
...
void rhead_rx_qdestroy(__in efx_rxq_t *erp) {
  _NOTE(ARGUNUSED(erp))
  /* Nothing to do here */
}
