...
__checkReturn efx_rc_t rhead_board_cfg(__in efx_nic_t *enp) {
  ...
  encp->enc_tunnel_encapsulations_supported =
      (1u << EFX_TUNNEL_PROTOCOL_VXLAN) | (1u << EFX_TUNNEL_PROTOCOL_NVGRE);
  ...
  encp->enc_tx_tso_max_header_length = ESE_EF100_DP_GZ_TSO_MAX_HDR_LEN_DEFAULT;
  ...
  encp->enc_tx_tso_max_nframes = ESE_EF100_DP_GZ_TSO_MAX_NUM_FRAMES_DEFAULT;
...
__checkReturn efx_rc_t rhead_nic_probe(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t rhead_nic_set_drv_limits(__inout efx_nic_t *enp,
...
__checkReturn efx_rc_t rhead_nic_reset(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t rhead_nic_init(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t rhead_nic_get_vi_pool(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t rhead_nic_get_bar_region(__in efx_nic_t *enp,
...
__checkReturn boolean_t rhead_nic_hw_unavailable(__in efx_nic_t *enp) {
  efx_dword_t dword;
...
void rhead_nic_set_hw_unavailable(__in efx_nic_t *enp) {
  EFSYS_PROBE(hw_unavail);
  enp->en_reset_flags |= EFX_RESET_HW_UNAVAIL;
}

void rhead_nic_fini(__in efx_nic_t *enp) {
...
void rhead_nic_unprobe(__in efx_nic_t *enp) {
  (void)efx_mcdi_drv_attach(enp, B_FALSE);
}

#if EFSYS_OPT_DIAG

__checkReturn efx_rc_t rhead_nic_register_test(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t rhead_nic_xilinx_cap_tbl_read_ef100_locator(
   ...
   */
  EFSYS_BAR_READD(esbp, offset + (EFX_LOW_BIT(ESF_GZ_CFGBAR_ENTRY_FORMAT) / 8),
                  &entry.eo_dword[0], B_FALSE);
  EFSYS_BAR_READD(esbp, offset + (EFX_LOW_BIT(ESF_GZ_CFGBAR_ENTRY_SIZE) / 8),
  ...
  EFSYS_BAR_READD(esbp, offset + (EFX_LOW_BIT(ESF_GZ_CFGBAR_EF100_BAR) / 8),
                  &entry.eo_dword[2], B_FALSE);

  ebrp->ebr_index = EFX_OWORD_FIELD32(entry, ESF_GZ_CFGBAR_EF100_BAR);
  ebrp->ebr_offset =
      EFX_OWORD_FIELD32(entry, ESF_GZ_CFGBAR_EF100_FUNC_CTL_WIN_OFF)
