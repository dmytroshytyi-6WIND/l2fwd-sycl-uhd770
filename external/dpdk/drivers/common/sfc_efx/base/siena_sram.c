...
void siena_sram_init(__in efx_nic_t *enp) {
  ...
  tx_base =
      rx_base + (encp->enc_rxq_limit * EFX_RXQ_DC_NDESCS(EFX_RXQ_DC_SIZE));
...
__checkReturn efx_rc_t siena_sram_test(__in efx_nic_t *enp,
  ...
  for (wptr = 0, rptr = 0; wptr < rows; ++wptr) {
    ...
    for (; rptr <= wptr; ++rptr) {
      ...
      EFX_BAR_TBL_READQ(enp, FR_AZ_BUF_HALF_TBL, rptr, &verify);
