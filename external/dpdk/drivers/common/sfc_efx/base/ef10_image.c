...
static __checkReturn efx_rc_t efx_asn1_parse_header_match_tag(
    __inout efx_asn1_cursor_t *cursor, __in uint8_t tag) {
  efx_rc_t rc;
  ...
  if ((cursor->buffer[1] & 0x80) == 0) {
    ...
    for (offset = 2; offset < cursor->hdr_size; offset++) {
      cursor->val_size = (cursor->val_size << 8) | cursor->buffer[offset];
    }
  }
...
static const uint8_t PKCS7_SignedData[] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                           0x0D, 0x01, 0x07, 0x02};

/* OID 1.2.840.113549.1.7.1 */
static const uint8_t PKCS7_Data[] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                     0x0D, 0x01, 0x07, 0x01};
 ...
 */
static __checkReturn efx_rc_t efx_check_signed_image_header(
    __in void *bufferp, __in uint32_t buffer_size,
  ...
  rc = efx_asn1_match_tag_value(&cursor, ASN1_TAG_OBJ_ID, PKCS7_SignedData,
                                sizeof(PKCS7_SignedData));
  if (rc != 0)
    goto fail3;
  ...
  /* SignedData.version */
  rc = efx_asn1_match_tag_value(&cursor, ASN1_TAG_INTEGER, SignedData_Version,
                                sizeof(SignedData_Version));
  if (rc != 0)
    goto fail6;
  ...
  valp = NULL;
  val_size = 0;
  rc = efx_asn1_get_tag_value(&cursor, ASN1_TAG_OCTET_STRING, &valp, &val_size);
  if (rc != 0)
    goto fail11;
...
efx_check_unsigned_image(__in void *bufferp, __in uint32_t buffer_size,
  ...
  if (buffer_size <
      (headerp->eih_size + headerp->eih_code_size + EFX_IMAGE_TRAILER_SIZE)) {
    rc = ENOSPC;
    goto fail2;
  }

  trailerp = (efx_image_trailer_t *)((uint8_t *)headerp + headerp->eih_size +
                                     headerp->eih_code_size);
...
__checkReturn efx_rc_t efx_check_reflash_image(__in void *bufferp,
...
__checkReturn efx_rc_t efx_build_signed_image_write_buffer(
  ...
  if ((code.size > buffer_size) || (code.offset > (buffer_size - code.size))) {
    rc = EINVAL;
    goto fail5;
  }
  ...
  memcpy(bufferp + chunk_hdr.offset, infop->eii_imagep + cms_header.offset,
         cms_header.size);
  ...
  memcpy(bufferp + chunk_hdr.offset, infop->eii_imagep + image_header.offset,
         image_header.size);
