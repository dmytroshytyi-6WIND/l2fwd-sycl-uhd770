...
static void siena_phy_decode_cap(__in uint32_t mcdi_cap,
...
static void siena_phy_decode_link_mode(__in efx_nic_t *enp,
...
void siena_phy_link_ev(__in efx_nic_t *enp, __in efx_qword_t *eqp,
                       __out efx_link_mode_t *link_modep) {
  ...
  siena_phy_decode_link_mode(enp, link_flags, speed,
                             MCDI_EV_FIELD(eqp, LINKCHANGE_FCNTL), &link_mode,
                             &fcntl);
  siena_phy_decode_cap(MCDI_EV_FIELD(eqp, LINKCHANGE_LP_CAP), &lp_cap_mask);
...
__checkReturn efx_rc_t siena_phy_power(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t siena_phy_get_link(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t siena_phy_reconfigure(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
  efx_mcdi_req_t req;
...
#if EFSYS_OPT_LOOPBACK
  MCDI_IN_SET_DWORD(req, SET_LINK_IN_LOOPBACK_MODE, epp->ep_loopback_type);
...
__checkReturn efx_rc_t siena_phy_verify(__in efx_nic_t *enp) {
  efx_mcdi_req_t req;
...
__checkReturn efx_rc_t siena_phy_oui_get(__in efx_nic_t *enp,
                                         __out uint32_t *ouip) {
...
#define SIENA_SIMPLE_STAT_SET(_vmask, _esmp, _smask, _stat, _mc_record,        \
  ...
  if ((_vmask) & (1ULL << (_mc_record))) {                                     \
    ...
    if ((_stat) != NULL && !EFSYS_MEM_IS_NULL(_esmp)) {                        \
      ...
      (_stat)[_efx_record] = EFX_DWORD_FIELD(dword, EFX_DWORD_0);              \
    }                                                                          \
  }
...
void siena_phy_decode_stats(__in efx_nic_t *enp, __in uint32_t vmask,
  ...
  if (vmask & (1 << MC_CMD_PMA_PMD_SIGNAL)) {
    ...
    if (stat != NULL && esmp != NULL && !EFSYS_MEM_IS_NULL(esmp)) {
      efx_dword_t dword;
      uint32_t sig;
      EFSYS_MEM_READD(esmp, 4 * MC_CMD_PMA_PMD_SIGNAL, &dword);
  ...
  if (vmask & (1 << MC_CMD_PHYXS_SYNC)) {
    smask |=
        ((1 << EFX_PHY_STAT_PHY_XS_SYNC_A) | (1 << EFX_PHY_STAT_PHY_XS_SYNC_B) |
         (1 << EFX_PHY_STAT_PHY_XS_SYNC_C) | (1 << EFX_PHY_STAT_PHY_XS_SYNC_D));
...
__checkReturn efx_rc_t siena_phy_stats_update(__in efx_nic_t *enp,
  ...
  MCDI_IN_SET_DWORD(req, PHY_STATS_IN_DMA_ADDR_HI, EFSYS_MEM_ADDR(esmp) >> 32);
...
__checkReturn efx_rc_t siena_phy_bist_start(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t siena_phy_bist_poll(
    ...
    __in size_t count) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_POLL_BIST_IN_LEN, MCDI_CTL_SDU_LEN_MAX);
  ...
  if (result == MC_CMD_POLL_BIST_PASSED &&
    ...
    if (count > EFX_BIST_PHY_CABLE_LENGTH_A) {
      if (valuesp != NULL)
        valuesp[EFX_BIST_PHY_CABLE_LENGTH_A] =
            MCDI_OUT_DWORD(req, POLL_BIST_OUT_SFT9001_CABLE_LENGTH_A);
    ...
    if (count > EFX_BIST_PHY_CABLE_LENGTH_B) {
      if (valuesp != NULL)
        valuesp[EFX_BIST_PHY_CABLE_LENGTH_B] =
            MCDI_OUT_DWORD(req, POLL_BIST_OUT_SFT9001_CABLE_LENGTH_B);
    ...
    if (count > EFX_BIST_PHY_CABLE_LENGTH_C) {
      if (valuesp != NULL)
        valuesp[EFX_BIST_PHY_CABLE_LENGTH_C] =
            MCDI_OUT_DWORD(req, POLL_BIST_OUT_SFT9001_CABLE_LENGTH_C);
    ...
    if (count > EFX_BIST_PHY_CABLE_LENGTH_D) {
      if (valuesp != NULL)
        valuesp[EFX_BIST_PHY_CABLE_LENGTH_D] =
            MCDI_OUT_DWORD(req, POLL_BIST_OUT_SFT9001_CABLE_LENGTH_D);
      value_mask |= (1 << EFX_BIST_PHY_CABLE_LENGTH_D);
    }

    if (count > EFX_BIST_PHY_CABLE_STATUS_A) {
      if (valuesp != NULL) {
        word = MCDI_OUT_WORD(req, POLL_BIST_OUT_SFT9001_CABLE_STATUS_A);
    ...
    if (count > EFX_BIST_PHY_CABLE_STATUS_B) {
      if (valuesp != NULL) {
        word = MCDI_OUT_WORD(req, POLL_BIST_OUT_SFT9001_CABLE_STATUS_B);
    ...
    if (count > EFX_BIST_PHY_CABLE_STATUS_C) {
      if (valuesp != NULL) {
        word = MCDI_OUT_WORD(req, POLL_BIST_OUT_SFT9001_CABLE_STATUS_C);
    ...
    if (count > EFX_BIST_PHY_CABLE_STATUS_D) {
      if (valuesp != NULL) {
        word = MCDI_OUT_WORD(req, POLL_BIST_OUT_SFT9001_CABLE_STATUS_D);
...
void siena_phy_bist_stop(__in efx_nic_t *enp, __in efx_bist_type_t type) {
