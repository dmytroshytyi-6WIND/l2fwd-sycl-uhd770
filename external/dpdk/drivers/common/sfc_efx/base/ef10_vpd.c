...
__checkReturn efx_rc_t ef10_vpd_init(__in efx_nic_t *enp) {
  caddr_t svpd;
  size_t svpd_size;
  uint32_t pci_pf;
  uint32_t tag;
  ...
  svpd = NULL;
  svpd_size = 0;
  rc = ef10_nvram_partn_read_tlv(enp, NVRAM_PARTITION_TYPE_STATIC_CONFIG, tag,
                                 &svpd, &svpd_size);
  if (rc != 0) {
    if (rc == EACCES) {
...
__checkReturn efx_rc_t ef10_vpd_size(__in efx_nic_t *enp, __out size_t *sizep) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_vpd_read(__in efx_nic_t *enp,
  ...
  if ((rc = ef10_nvram_partn_read_tlv(enp, NVRAM_PARTITION_TYPE_DYNAMIC_CONFIG,
...
__checkReturn efx_rc_t ef10_vpd_verify(__in efx_nic_t *enp,
  ...
  while (1) {
    if ((rc = efx_vpd_hunk_next(data, size, &dtag, &dkey, NULL, NULL,
                                &dcont)) != 0)
      goto fail2;
    if (dcont == 0)
      break;
...
__checkReturn efx_rc_t ef10_vpd_reinit(__in efx_nic_t *enp,
                                       __in_bcount(size) caddr_t data,
                                       __in size_t size) {
  boolean_t wantpid;
  efx_rc_t rc;
  ...
  if (enp->en_arch.ef10.ena_svpd_length == 0)
    ...
    rc = efx_vpd_hunk_get(enp->en_arch.ef10.ena_svpd,
                          enp->en_arch.ef10.ena_svpd_length, EFX_VPD_ID, 0,
                          &offset, &length);
    if (rc == 0)
      wantpid = B_FALSE;
    else if (rc == ENOENT)
...
__checkReturn efx_rc_t ef10_vpd_get(__in efx_nic_t *enp,
                                    __in_bcount(size) caddr_t data,
                                    __in size_t size,
  ...
  if (enp->en_arch.ef10.ena_svpd_length > 0) {
    if ((rc = efx_vpd_hunk_get(enp->en_arch.ef10.ena_svpd,
      ...
      memcpy(evvp->evv_value, enp->en_arch.ef10.ena_svpd + offset, length);
      return (0);
    } else if (rc != ENOENT)
      goto fail1;
  }
...
__checkReturn efx_rc_t ef10_vpd_set(__in efx_nic_t *enp,
                                    __in_bcount(size) caddr_t data,
                                    __in size_t size,
...
__checkReturn efx_rc_t ef10_vpd_next(__in efx_nic_t *enp,
                                     __in_bcount(size) caddr_t data,
                                     __in size_t size,
...
__checkReturn efx_rc_t ef10_vpd_write(__in efx_nic_t *enp,
                                      __in_bcount(size) caddr_t data,
                                      __in size_t size) {
  size_t vpd_length;
  uint32_t pci_pf;
  ...
  if ((rc = ef10_nvram_partn_write_segment_tlv(
           enp, NVRAM_PARTITION_TYPE_DYNAMIC_CONFIG, tag, data, vpd_length,
           B_TRUE)) != 0) {
    goto fail2;
  }
...
void ef10_vpd_fini(__in efx_nic_t *enp) {
  EFSYS_ASSERT(EFX_FAMILY_IS_EF10(enp));
