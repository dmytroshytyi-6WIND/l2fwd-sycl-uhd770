...
__checkReturn uint32_t efx_hash_dwords(__in_ecount(count) uint32_t const *input,
                                       __in size_t count, __in uint32_t init) {
  uint32_t a;
  uint32_t b;
  uint32_t c;
  ...
  a = b = c =
      EFX_HASH_INITIAL_VALUE + (((uint32_t)count) * sizeof(uint32_t)) + init;
...
__checkReturn uint32_t efx_hash_bytes(__in_ecount(length) uint8_t const *input,
                                      __in size_t length, __in uint32_t init) {
  uint32_t a;
  uint32_t b;
  uint32_t c;
