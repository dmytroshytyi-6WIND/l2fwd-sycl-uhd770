...
static __checkReturn efx_rc_t efx_mae_get_capabilities(__in efx_nic_t *enp) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_GET_CAPS_IN_LEN,
  ...
  if (MCDI_OUT_DWORD_FIELD(req, MAE_GET_CAPS_OUT_ENCAP_TYPES_SUPPORTED,
    ...
    maep->em_encap_types_supported |= (1U << EFX_TUNNEL_PROTOCOL_VXLAN);
  }

  if (MCDI_OUT_DWORD_FIELD(req, MAE_GET_CAPS_OUT_ENCAP_TYPES_SUPPORTED,
                           MAE_GET_CAPS_OUT_ENCAP_TYPE_GENEVE) != 0) {
    maep->em_encap_types_supported |= (1U << EFX_TUNNEL_PROTOCOL_GENEVE);
  }

  if (MCDI_OUT_DWORD_FIELD(req, MAE_GET_CAPS_OUT_ENCAP_TYPES_SUPPORTED,
                           MAE_GET_CAPS_OUT_ENCAP_TYPE_NVGRE) != 0) {
    maep->em_encap_types_supported |= (1U << EFX_TUNNEL_PROTOCOL_NVGRE);
  }

  maep->em_max_nfields =
      MCDI_OUT_DWORD(req, MAE_GET_CAPS_OUT_MATCH_FIELD_COUNT);

  maep->em_max_ncounters = MCDI_OUT_DWORD(req, MAE_GET_CAPS_OUT_COUNTERS);
...
static __checkReturn efx_rc_t efx_mae_get_outer_rule_caps(
    __in efx_nic_t *enp, __in unsigned int field_ncaps,
    __out_ecount(field_ncaps) efx_mae_field_cap_t *field_caps) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_GET_OR_CAPS_IN_LEN,
  ...
  for (i = 0; i < mcdi_field_ncaps; ++i) {
    ...
    field_caps[i].emfc_support =
        MCDI_OUT_INDEXED_DWORD_FIELD(req, MAE_GET_OR_CAPS_OUT_FIELD_FLAGS, i,
    ...
    field_caps[i].emfc_mask_affects_class = (mask_flag != 0) ? B_TRUE : B_FALSE;
  }
...
static __checkReturn efx_rc_t efx_mae_get_action_rule_caps(
    __in efx_nic_t *enp, __in unsigned int field_ncaps,
    __out_ecount(field_ncaps) efx_mae_field_cap_t *field_caps) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_GET_AR_CAPS_IN_LEN,
  ...
  for (i = 0; i < mcdi_field_ncaps; ++i) {
    ...
    field_caps[i].emfc_support =
        MCDI_OUT_INDEXED_DWORD_FIELD(req, MAE_GET_AR_CAPS_OUT_FIELD_FLAGS, i,
                                     MAE_FIELD_FLAGS_SUPPORT_STATUS);
...
__checkReturn efx_rc_t efx_mae_init(__in efx_nic_t *enp) {
...
void efx_mae_fini(__in efx_nic_t *enp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
...
__checkReturn efx_rc_t efx_mae_get_limits(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mae_match_spec_init(
    __in efx_nic_t *enp, __in efx_mae_rule_type_t type, __in uint32_t prio,
...
void efx_mae_match_spec_fini(__in efx_nic_t *enp,
...
#define EFX_MAE_MV_DESC(_name, _endianness)                                    \
  [EFX_MAE_FIELD_##_name] = {EFX_MAE_FIELD_ID_##_name,                         \
...
__checkReturn efx_rc_t efx_mae_mport_invalid(__out efx_mport_sel_t *mportp) {
  efx_dword_t dword;
  efx_rc_t rc;
  ...
  EFX_POPULATE_DWORD_1(dword, MAE_MPORT_SELECTOR_TYPE,
                       MAE_MPORT_SELECTOR_TYPE_INVALID);
...
__checkReturn efx_rc_t efx_mae_mport_by_phy_port(
  ...
  EFX_POPULATE_DWORD_2(dword, MAE_MPORT_SELECTOR_TYPE,
...
__checkReturn efx_rc_t efx_mae_mport_by_pcie_function(
    __in uint32_t pf, __in uint32_t vf, __out efx_mport_sel_t *mportp) {
  efx_dword_t dword;
  efx_rc_t rc;

  rc = efx_mae_mport_by_pcie_mh_function(EFX_PCIE_INTERFACE_CALLER, pf, vf,
                                         mportp);
  if (rc != 0)
    goto fail1;
...
static __checkReturn efx_rc_t efx_mae_intf_to_selector(
    __in efx_pcie_interface_t intf, __out uint32_t *selector_intfp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mae_mport_by_pcie_mh_function(
  ...
  EFX_STATIC_ASSERT(EFX_PCI_VF_INVALID == MAE_MPORT_SELECTOR_FUNC_VF_ID_NULL);
  ...
  EFX_POPULATE_DWORD_4(
      dword, MAE_MPORT_SELECTOR_TYPE, MAE_MPORT_SELECTOR_TYPE_MH_FUNC,
      MAE_MPORT_SELECTOR_FUNC_INTF_ID, selector_intf,
      MAE_MPORT_SELECTOR_FUNC_MH_PF_ID, pf, MAE_MPORT_SELECTOR_FUNC_VF_ID, vf);
...
static __checkReturn efx_rc_t efx_mcdi_mae_mport_lookup(
    __in efx_nic_t *enp, __in const efx_mport_sel_t *mport_selectorp,
    __out efx_mport_id_t *mport_idp) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_MPORT_LOOKUP_IN_LEN,
...
__checkReturn efx_rc_t efx_mae_mport_id_by_selector(
    __in efx_nic_t *enp, __in const efx_mport_sel_t *mport_selectorp,
    __out efx_mport_id_t *mport_idp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mae_match_spec_recirc_id_set(
  ...
  rc = efx_mae_match_spec_field_set(spec, EFX_MAE_FIELD_RECIRC_ID,
                                    sizeof(recirc_id), vp, sizeof(full_mask),
                                    mp);
  if (rc != 0)
    goto fail1;
...
__checkReturn efx_rc_t efx_mae_mport_by_id(__in const efx_mport_id_t *mport_idp,
                                           __out efx_mport_sel_t *mportp) {
  efx_dword_t dword;
...
__checkReturn efx_rc_t efx_mae_match_spec_field_set(
  ...
  case EFX_MAE_RULE_OUTER:
    desc_set_nentries = EFX_ARRAY_SIZE(__efx_mae_outer_rule_mv_desc_set);
    descp = &__efx_mae_outer_rule_mv_desc_set[field_id];
    mvp = spec->emms_mask_value_pairs.outer;
    break;
  case EFX_MAE_RULE_ACTION:
    desc_set_nentries = EFX_ARRAY_SIZE(__efx_mae_action_rule_mv_desc_set);
  ...
  if (descp->emmd_endianness == EFX_MAE_FIELD_BE) {
    ...
    switch (value_size) {
    case 4:
      EFX_POPULATE_DWORD_1(dword, EFX_DWORD_0, *(const uint32_t *)value);

      memcpy(mvp + descp->emmd_value_offset, &dword, sizeof(dword));
      break;
    case 1:
      memcpy(mvp + descp->emmd_value_offset, value, 1);
      break;
    default:
    ...
    switch (mask_size) {
    case 4:
      EFX_POPULATE_DWORD_1(dword, EFX_DWORD_0, *(const uint32_t *)mask);

      memcpy(mvp + descp->emmd_mask_offset, &dword, sizeof(dword));
      break;
    case 1:
      memcpy(mvp + descp->emmd_mask_offset, mask, 1);
      break;
    default:
      EFSYS_ASSERT(B_FALSE);
    }
...
__checkReturn efx_rc_t efx_mae_match_spec_bit_set(
    __in efx_mae_match_spec_t *spec, __in efx_mae_field_id_t field_id,
...
__checkReturn efx_rc_t efx_mae_match_spec_mport_set(
  ...
  rc = efx_mae_match_spec_field_set(spec, EFX_MAE_FIELD_INGRESS_MPORT_SELECTOR,
                                    sizeof(valuep->sel), vp, sizeof(maskp->sel),
                                    mp);
  if (rc != 0)
    goto fail2;
...
static boolean_t efx_mask_is_prefix(__in size_t mask_nbytes,
                                    __in_bcount(mask_nbytes)
                                        const uint8_t *maskp) {
...
static boolean_t efx_mask_is_all_ones(__in size_t mask_nbytes,
                                      __in_bcount(mask_nbytes)
                                          const uint8_t *maskp) {
  unsigned int i;
  uint8_t t = ~0;
...
static boolean_t efx_mask_is_all_zeros(__in size_t mask_nbytes,
                                       __in_bcount(mask_nbytes)
                                           const uint8_t *maskp) {
  unsigned int i;
  uint8_t t = 0;
...
__checkReturn boolean_t efx_mae_match_spec_is_valid(
  ...
  for (field_id = 0; (unsigned int)field_id < desc_set_nentries; ++field_id) {
    ...
    case MAE_FIELD_SUPPORTED_MATCH_ALWAYS:
      ...
      if ((is_valid == B_FALSE) && (alt_m_size != 0)) {
         ...
         */
        is_valid = efx_mask_is_all_ones(alt_m_size, alt_m_buf);
      }
      break;
  ...
  for (field_id = 0; (unsigned int)field_id < bit_desc_set_nentries;
       ++field_id) {
    const efx_mae_mv_bit_desc_t *bit_descp = &bit_desc_setp[field_id];
    unsigned int byte_idx =
        bit_descp->emmbd_mask_ofst + bit_descp->emmbd_mask_lbn / 8;
    unsigned int bit_idx = bit_descp->emmbd_mask_lbn % 8;
    efx_mae_field_cap_id_t bit_cap_id = bit_descp->emmbd_bit_cap_id;
...
__checkReturn efx_rc_t efx_mae_action_set_spec_init(
...
void efx_mae_action_set_spec_fini(__in efx_nic_t *enp,
                                  __in efx_mae_actions_t *spec) {
  EFSYS_KMEM_FREE(enp->en_esip, sizeof(*spec), spec);
}

static __checkReturn efx_rc_t
efx_mae_action_set_no_op(__in efx_mae_actions_t *spec, __in size_t arg_size,
...
static __checkReturn efx_rc_t efx_mae_action_set_add_vlan_pop(
    __in efx_mae_actions_t *spec, __in size_t arg_size,
    __in_bcount(arg_size) const uint8_t *arg) {
  efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mae_action_set_add_vlan_push(
    __in efx_mae_actions_t *spec, __in size_t arg_size,
    __in_bcount(arg_size) const uint8_t *arg) {
...
static __checkReturn efx_rc_t
efx_mae_action_set_add_count(__in efx_mae_actions_t *spec, __in size_t arg_size,
                             __in_bcount(arg_size) const uint8_t *arg) {
  efx_rc_t rc;
...
static __checkReturn efx_rc_t
efx_mae_action_set_add_mark(__in efx_mae_actions_t *spec, __in size_t arg_size,
                            __in_bcount(arg_size) const uint8_t *arg) {
  efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mae_action_set_add_deliver(
    __in efx_mae_actions_t *spec, __in size_t arg_size,
    __in_bcount(arg_size) const uint8_t *arg) {
  efx_rc_t rc;
...
typedef struct efx_mae_action_desc_s {
  ...
  efx_rc_t (*emad_add)(efx_mae_actions_t *, size_t, const uint8_t *);
} efx_mae_action_desc_t;

static const efx_mae_action_desc_t efx_mae_actions[EFX_MAE_NACTIONS] = {
    [EFX_MAE_ACTION_DECAP] = {.emad_add = efx_mae_action_set_no_op},
    [EFX_MAE_ACTION_VLAN_POP] = {.emad_add = efx_mae_action_set_add_vlan_pop},
    [EFX_MAE_ACTION_SET_DST_MAC] = {.emad_add = efx_mae_action_set_no_op},
    [EFX_MAE_ACTION_SET_SRC_MAC] = {.emad_add = efx_mae_action_set_no_op},
    [EFX_MAE_ACTION_DECR_IP_TTL] = {.emad_add = efx_mae_action_set_no_op},
    [EFX_MAE_ACTION_VLAN_PUSH] = {.emad_add = efx_mae_action_set_add_vlan_push},
    [EFX_MAE_ACTION_ENCAP] = {.emad_add = efx_mae_action_set_no_op},
    [EFX_MAE_ACTION_COUNT] = {.emad_add = efx_mae_action_set_add_count},
    [EFX_MAE_ACTION_FLAG] = {.emad_add = efx_mae_action_set_no_op},
    [EFX_MAE_ACTION_MARK] = {.emad_add = efx_mae_action_set_add_mark},
    [EFX_MAE_ACTION_DELIVER] = {.emad_add = efx_mae_action_set_add_deliver}};

static const uint32_t efx_mae_action_ordered_map =
    (1U << EFX_MAE_ACTION_DECAP) | (1U << EFX_MAE_ACTION_VLAN_POP) |
    (1U << EFX_MAE_ACTION_SET_DST_MAC) | (1U << EFX_MAE_ACTION_SET_SRC_MAC) |
    (1U << EFX_MAE_ACTION_DECR_IP_TTL) | (1U << EFX_MAE_ACTION_VLAN_PUSH) |
    /*
     * HW will conduct action COUNT after
     * the matching packet has been modified by
     * length-affecting actions except for ENCAP.
     */
    (1U << EFX_MAE_ACTION_COUNT) | (1U << EFX_MAE_ACTION_ENCAP) |
    (1U << EFX_MAE_ACTION_FLAG) | (1U << EFX_MAE_ACTION_MARK) |
...
static const uint32_t efx_mae_action_nonstrict_map =
    (1U << EFX_MAE_ACTION_COUNT) | (1U << EFX_MAE_ACTION_FLAG) |
    (1U << EFX_MAE_ACTION_MARK);

static const uint32_t efx_mae_action_repeat_map =
    (1U << EFX_MAE_ACTION_VLAN_POP) | (1U << EFX_MAE_ACTION_VLAN_PUSH) |
 ...
 */
static __checkReturn efx_rc_t efx_mae_action_set_spec_populate(
    __in efx_mae_actions_t *spec, __in efx_mae_action_t type,
    __in size_t arg_size, __in_bcount(arg_size) const uint8_t *arg) {
...
efx_mae_action_set_populate_decap(__in efx_mae_actions_t *spec) {
  return (
      efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_DECAP, 0, NULL));
}

__checkReturn efx_rc_t
efx_mae_action_set_populate_vlan_pop(__in efx_mae_actions_t *spec) {
  return (
      efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_VLAN_POP, 0, NULL));
}
...
efx_mae_action_set_populate_set_dst_mac(__in efx_mae_actions_t *spec) {
  ...
  return (efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_SET_DST_MAC, 0,
                                           NULL));
...
efx_mae_action_set_populate_set_src_mac(__in efx_mae_actions_t *spec) {
  ...
  return (efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_SET_SRC_MAC, 0,
                                           NULL));
...
efx_mae_action_set_populate_decr_ip_ttl(__in efx_mae_actions_t *spec) {
  ...
  return (efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_DECR_IP_TTL, 0,
                                           NULL));
...
__checkReturn efx_rc_t efx_mae_action_set_populate_vlan_push(
...
efx_mae_action_set_populate_encap(__in efx_mae_actions_t *spec) {
   ...
   */
  return (
      efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_ENCAP, 0, NULL));
}
...
efx_mae_action_set_populate_count(__in efx_mae_actions_t *spec) {
   ...
   */
  return (
      efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_COUNT, 0, NULL));
}

__checkReturn efx_rc_t
efx_mae_action_set_populate_flag(__in efx_mae_actions_t *spec) {
  return (efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_FLAG, 0, NULL));
}

__checkReturn efx_rc_t efx_mae_action_set_populate_mark(
    __in efx_mae_actions_t *spec, __in uint32_t mark_value) {
  const uint8_t *arg = (const uint8_t *)&mark_value;

  return (efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_MARK,
                                           sizeof(mark_value), arg));
}

__checkReturn efx_rc_t efx_mae_action_set_populate_deliver(
...
efx_mae_action_set_populate_drop(__in efx_mae_actions_t *spec) {
  ...
  EFX_POPULATE_DWORD_1(dword, MAE_MPORT_SELECTOR_FLAT, MAE_MPORT_SELECTOR_NULL);
  ...
  return (efx_mae_action_set_spec_populate(spec, EFX_MAE_ACTION_DELIVER,
                                           sizeof(mport.sel), arg));
}

__checkReturn boolean_t efx_mae_action_set_specs_equal(
...
__checkReturn efx_rc_t efx_mae_match_specs_class_cmp(
...
__checkReturn efx_rc_t efx_mae_outer_rule_recirc_id_set(
    __in efx_mae_match_spec_t *spec, __in uint8_t recirc_id) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mae_outer_rule_insert(
    __in efx_nic_t *enp, __in const efx_mae_match_spec_t *spec,
    __in efx_tunnel_protocol_t encap_type, __out efx_mae_rule_id_t *or_idp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_OUTER_RULE_INSERT_IN_LENMAX_MCDI2,
  ...
  MCDI_IN_SET_DWORD(req, MAE_OUTER_RULE_INSERT_IN_ENCAP_TYPE, encap_type_mcdi);
...
__checkReturn efx_rc_t efx_mae_outer_rule_remove(
    __in efx_nic_t *enp, __in const efx_mae_rule_id_t *or_idp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_OUTER_RULE_REMOVE_IN_LEN(1),
...
__checkReturn efx_rc_t efx_mae_match_spec_outer_rule_id_set(
  ...
  rc = efx_mae_match_spec_field_set(
      spec, EFX_MAE_FIELD_OUTER_RULE_ID, sizeof(or_idp->id),
      (const uint8_t *)&or_idp->id, sizeof(full_mask),
...
__checkReturn efx_rc_t efx_mae_mac_addr_alloc(
    __in efx_nic_t *enp, __in uint8_t addr_bytes[EFX_MAC_ADDR_LEN],
    __out efx_mae_mac_id_t *mac_idp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_MAC_ADDR_ALLOC_IN_LEN,
  ...
  memcpy(payload + MC_CMD_MAE_MAC_ADDR_ALLOC_IN_MAC_ADDR_OFST, addr_bytes,
         EFX_MAC_ADDR_LEN);
...
__checkReturn efx_rc_t efx_mae_mac_addr_free(
    __in efx_nic_t *enp, __in const efx_mae_mac_id_t *mac_idp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_MAC_ADDR_FREE_IN_LEN(1),
  ...
  if (MCDI_OUT_DWORD(req, MAE_MAC_ADDR_FREE_OUT_FREED_MAC_ID) != mac_idp->id) {
...
__checkReturn efx_rc_t efx_mae_action_set_fill_in_dst_mac_id(
...
__checkReturn efx_rc_t efx_mae_action_set_fill_in_src_mac_id(
    __in efx_mae_actions_t *spec, __in const efx_mae_mac_id_t *mac_idp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mae_encap_header_alloc(
  ...
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_ENCAP_HEADER_ALLOC_IN_LENMAX_MCDI2,
  ...
  if (header_size > MC_CMD_MAE_ENCAP_HEADER_ALLOC_IN_HDR_DATA_MAXNUM_MCDI2) {
    rc = EINVAL;
    goto fail3;
  }
  ...
  MCDI_IN_SET_DWORD(req, MAE_ENCAP_HEADER_ALLOC_IN_ENCAP_TYPE, encap_type_mcdi);

  memcpy(payload + MC_CMD_MAE_ENCAP_HEADER_ALLOC_IN_HDR_DATA_OFST, header_data,
         header_size);
  ...
  eh_id.id = MCDI_OUT_DWORD(req, MAE_ENCAP_HEADER_ALLOC_OUT_ENCAP_HEADER_ID);
...
__checkReturn efx_rc_t efx_mae_encap_header_free(
    __in efx_nic_t *enp, __in const efx_mae_eh_id_t *eh_idp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_ENCAP_HEADER_FREE_IN_LEN(1),
...
__checkReturn efx_rc_t efx_mae_action_set_fill_in_eh_id(
...
__checkReturn efx_rc_t efx_mae_action_set_alloc(
  ...
  if (spec->ema_n_vlan_tags_to_push > 0) {
    ...
    if (spec->ema_n_vlan_tags_to_push == EFX_MAE_VLAN_PUSH_MAX_NTAGS) {
      MCDI_IN_SET_WORD(req, MAE_ACTION_SET_ALLOC_IN_VLAN1_PROTO_BE,
                       spec->ema_vlan_push_descs[0].emavp_tpid_be);
      MCDI_IN_SET_WORD(req, MAE_ACTION_SET_ALLOC_IN_VLAN1_TCI_BE,
  ...
  if ((spec->ema_actions & (1U << EFX_MAE_ACTION_MARK)) != 0) {
    ...
    MCDI_IN_SET_DWORD(req, MAE_ACTION_SET_ALLOC_IN_MARK_VALUE,
                      spec->ema_mark_value);
  }

  MCDI_IN_SET_DWORD(req, MAE_ACTION_SET_ALLOC_IN_DELIVER,
                    spec->ema_deliver_mport.sel);
...
__checkReturn efx_rc_t efx_mae_action_set_fill_in_counter_id(
...
__checkReturn efx_rc_t efx_mae_counters_alloc(__in efx_nic_t *enp,
                                              __in uint32_t n_counters,
                                              __out uint32_t *n_allocatedp,
                                              __out_ecount(n_counters)
                                                  efx_counter_t *countersp,
  ...
  MCDI_IN_SET_DWORD(req, MAE_COUNTER_ALLOC_IN_REQUESTED_COUNT, n_counters);
  ...
  n_allocated = MCDI_OUT_DWORD(req, MAE_COUNTER_ALLOC_OUT_COUNTER_ID_COUNT);
  ...
  for (i = 0; i < n_allocated; i++) {
    countersp[i].id =
        MCDI_OUT_INDEXED_DWORD(req, MAE_COUNTER_ALLOC_OUT_COUNTER_ID, i);
  }

  if (gen_countp != NULL) {
    *gen_countp = MCDI_OUT_DWORD(req, MAE_COUNTER_ALLOC_OUT_GENERATION_COUNT);
  }
...
__checkReturn efx_rc_t efx_mae_counters_free(__in efx_nic_t *enp,
                                             __in uint32_t n_counters,
                                             __out uint32_t *n_freedp,
                                             __in_ecount(n_counters)
                                                 const efx_counter_t *countersp,
                                             __out_opt uint32_t *gen_countp) {
  ...
  if (n_counters > maep->em_max_ncounters ||
      ...
      n_counters > MC_CMD_MAE_COUNTER_FREE_IN_FREE_COUNTER_ID_MAXNUM_MCDI2) {
    rc = EINVAL;
    goto fail1;
  }
  ...
  for (i = 0; i < n_counters; i++) {
    MCDI_IN_SET_INDEXED_DWORD(req, MAE_COUNTER_FREE_IN_FREE_COUNTER_ID, i,
                              countersp[i].id);
  }
  MCDI_IN_SET_DWORD(req, MAE_COUNTER_FREE_IN_COUNTER_ID_COUNT, n_counters);
  ...
  if (gen_countp != NULL) {
    *gen_countp = MCDI_OUT_DWORD(req, MAE_COUNTER_FREE_OUT_GENERATION_COUNT);
  }
...
__checkReturn efx_rc_t efx_mae_counters_stream_start(
  ...
  EFX_STATIC_ASSERT(
      EFX_MAE_COUNTERS_STREAM_IN_ZERO_SQUASH_DISABLE ==
  ...
  MCDI_IN_SET_WORD(req, MAE_COUNTERS_STREAM_START_IN_PACKET_SIZE, packet_size);
  ...
  if (req.emr_out_length_used < MC_CMD_MAE_COUNTERS_STREAM_START_OUT_LEN) {
    rc = EMSGSIZE;
    goto fail2;
  }
...
__checkReturn efx_rc_t efx_mae_counters_stream_stop(
  ...
  if (req.emr_out_length_used < MC_CMD_MAE_COUNTERS_STREAM_STOP_OUT_LEN) {
    rc = EMSGSIZE;
    goto fail2;
  }
  ...
  if (gen_countp != NULL) {
    *gen_countp =
        MCDI_OUT_DWORD(req, MAE_COUNTERS_STREAM_STOP_OUT_GENERATION_COUNT);
  }
...
__checkReturn efx_rc_t efx_mae_counters_stream_give_credits(
    __in efx_nic_t *enp, __in uint32_t n_credits) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_COUNTERS_STREAM_GIVE_CREDITS_IN_LEN,
...
__checkReturn efx_rc_t efx_mae_action_set_free(
    __in efx_nic_t *enp, __in const efx_mae_aset_id_t *aset_idp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_ACTION_SET_FREE_IN_LEN(1),
...
__checkReturn efx_rc_t efx_mae_action_rule_insert(
    __in efx_nic_t *enp, __in const efx_mae_match_spec_t *spec,
    __in const efx_mae_aset_list_id_t *asl_idp,
    __in const efx_mae_aset_id_t *as_idp, __out efx_mae_rule_id_t *ar_idp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_ACTION_RULE_INSERT_IN_LENMAX_MCDI2,
  ...
  EFX_POPULATE_OWORD_3(
      *rule_response, MAE_ACTION_RULE_RESPONSE_ASL_ID,
...
__checkReturn efx_rc_t efx_mae_action_rule_remove(
    __in efx_nic_t *enp, __in const efx_mae_rule_id_t *ar_idp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_ACTION_RULE_DELETE_IN_LEN(1),
  ...
  if (req.emr_out_length_used < MC_CMD_MAE_ACTION_RULE_DELETE_OUT_LENMIN) {
    rc = EMSGSIZE;
    goto fail3;
  }
...
__checkReturn efx_rc_t efx_mcdi_mport_alloc_alias(__in efx_nic_t *enp,
                                                  __out efx_mport_id_t *mportp,
                                                  __out_opt uint32_t *labelp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_MPORT_ALLOC_ALIAS_IN_LEN,
...
__checkReturn efx_rc_t efx_mae_mport_free(__in efx_nic_t *enp,
                                          __in const efx_mport_id_t *mportp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_MPORT_FREE_IN_LEN,
...
static __checkReturn efx_rc_t efx_mae_read_mport_journal_single(
    __in uint8_t *entry_buf, __out efx_mport_desc_t *desc) {
  uint32_t pcie_intf;
  efx_rc_t rc;

  memset(desc, 0, sizeof(*desc));

  desc->emd_id.id = MCDI_STRUCT_DWORD(entry_buf, MAE_MPORT_DESC_V2_MPORT_ID);

  desc->emd_can_receive_on = MCDI_STRUCT_DWORD_FIELD(
      entry_buf, MAE_MPORT_DESC_V2_FLAGS, MAE_MPORT_DESC_V2_CAN_RECEIVE_ON);

  desc->emd_can_deliver_to = MCDI_STRUCT_DWORD_FIELD(
      entry_buf, MAE_MPORT_DESC_V2_FLAGS, MAE_MPORT_DESC_V2_CAN_DELIVER_TO);

  desc->emd_can_delete = MCDI_STRUCT_DWORD_FIELD(
      entry_buf, MAE_MPORT_DESC_V2_FLAGS, MAE_MPORT_DESC_V2_CAN_DELETE);

  desc->emd_zombie = MCDI_STRUCT_DWORD_FIELD(entry_buf, MAE_MPORT_DESC_V2_FLAGS,
                                             MAE_MPORT_DESC_V2_IS_ZOMBIE);

  desc->emd_type = MCDI_STRUCT_DWORD(entry_buf, MAE_MPORT_DESC_V2_MPORT_TYPE);
  ...
  case EFX_MPORT_TYPE_NET_PORT:
    desc->emd_net_port.ep_index =
        MCDI_STRUCT_DWORD(entry_buf, MAE_MPORT_DESC_V2_NET_PORT_IDX);
    break;
  case EFX_MPORT_TYPE_ALIAS:
    desc->emd_alias.ea_target_mport_id.id =
        MCDI_STRUCT_DWORD(entry_buf, MAE_MPORT_DESC_V2_ALIAS_DELIVER_MPORT_ID);
    break;
  case EFX_MPORT_TYPE_VNIC:
    desc->emd_vnic.ev_client_type =
        MCDI_STRUCT_DWORD(entry_buf, MAE_MPORT_DESC_V2_VNIC_CLIENT_TYPE);
    if (desc->emd_vnic.ev_client_type != EFX_MPORT_VNIC_CLIENT_FUNCTION)
      break;

    pcie_intf =
        MCDI_STRUCT_DWORD(entry_buf, MAE_MPORT_DESC_V2_VNIC_FUNCTION_INTERFACE);
    rc = efx_mcdi_intf_from_pcie(pcie_intf, &desc->emd_vnic.ev_intf);
    if (rc != 0)
      goto fail1;

    desc->emd_vnic.ev_pf =
        MCDI_STRUCT_WORD(entry_buf, MAE_MPORT_DESC_V2_VNIC_FUNCTION_PF_IDX);
    desc->emd_vnic.ev_vf =
        MCDI_STRUCT_WORD(entry_buf, MAE_MPORT_DESC_V2_VNIC_FUNCTION_VF_IDX);
    desc->emd_vnic.ev_handle =
        MCDI_STRUCT_DWORD(entry_buf, MAE_MPORT_DESC_V2_VNIC_CLIENT_HANDLE);
    break;
  default:
    rc = EINVAL;
    goto fail2;
...
static __checkReturn efx_rc_t efx_mae_read_mport_journal_batch(
    __in efx_nic_t *enp, __in efx_mae_read_mport_journal_cb *cbp,
    __in void *cb_datap, __out uint32_t *morep) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_MAE_MPORT_READ_JOURNAL_IN_LEN,
  ...
  EFX_STATIC_ASSERT(EFX_MPORT_TYPE_ALIAS == MAE_MPORT_DESC_V2_MPORT_TYPE_ALIAS);
  EFX_STATIC_ASSERT(EFX_MPORT_TYPE_VNIC == MAE_MPORT_DESC_V2_MPORT_TYPE_VNIC);
  ...
  if (req.emr_out_length_used < MC_CMD_MAE_MPORT_READ_JOURNAL_OUT_LENMIN) {
    rc = EMSGSIZE;
    goto fail3;
  }

  if (morep != NULL) {
    *morep = MCDI_OUT_DWORD_FIELD(req, MAE_MPORT_READ_JOURNAL_OUT_FLAGS,
                                  MAE_MPORT_READ_JOURNAL_OUT_MORE);
  }
  n_entries = MCDI_OUT_DWORD(req, MAE_MPORT_READ_JOURNAL_OUT_MPORT_DESC_COUNT);
  entry_sz = MCDI_OUT_DWORD(req, MAE_MPORT_READ_JOURNAL_OUT_SIZEOF_MPORT_DESC);
  entry_buf =
      MCDI_OUT2(req, uint8_t, MAE_MPORT_READ_JOURNAL_OUT_MPORT_DESC_DATA);
...
__checkReturn efx_rc_t efx_mae_read_mport_journal(
    __in efx_nic_t *enp, __in efx_mae_read_mport_journal_cb *cbp,
    __in void *cb_datap) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  uint32_t more = 0;
  ...
  do {
    rc = efx_mae_read_mport_journal_batch(enp, cbp, cb_datap, &more);
    if (rc != 0)
      goto fail2;
  } while (more != 0);
