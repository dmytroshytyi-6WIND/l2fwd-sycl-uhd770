 ...
 */
static __checkReturn efx_rc_t rhead_xilinx_cap_tbl_find_ef100_locator(
    __in efsys_bar_t *esbp, __in efsys_dma_addr_t tbl_offset,
    __out efx_bar_region_t *ef100_ebrp) {
  efx_rc_t rc;
  efsys_dma_addr_t entry_offset = tbl_offset;

  rc = efx_pci_xilinx_cap_tbl_find(esbp, ESE_GZ_CFGBAR_ENTRY_EF100, B_FALSE,
                                   &entry_offset);
  if (rc != 0) {
...
__checkReturn efx_rc_t rhead_pci_nic_membar_lookup(
   ...
   */
  while (1) {
    rc = efx_pci_find_next_xilinx_cap_table(
        espcp, epop, &pci_capa_offset, &xilinx_tbl_bar, &xilinx_tbl_offset);
    if (rc != 0) {
      /*
    ...
    rc = rhead_xilinx_cap_tbl_find_ef100_locator(&xil_eb, xilinx_tbl_offset,
                                                 ebrp);
    if (rc == 0) {
  ...
  EFSYS_BAR_READD(&nic_eb, ebrp->ebr_offset + ER_GZ_NIC_MAGIC_OFST, &magic_ed,
                  B_FALSE);
