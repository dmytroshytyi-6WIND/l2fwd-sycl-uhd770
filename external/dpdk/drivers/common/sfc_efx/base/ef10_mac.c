...
__checkReturn efx_rc_t ef10_mac_poll(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t ef10_mac_up(__in efx_nic_t *enp,
 ...
 */
static __checkReturn efx_rc_t efx_mcdi_vadapter_set_mac(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
  efx_mcdi_req_t req;
...
__checkReturn efx_rc_t ef10_mac_addr_set(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mcdi_mtu_set(__in efx_nic_t *enp,
                                               __in uint32_t mtu) {
  efx_mcdi_req_t req;
  ...
  MCDI_IN_POPULATE_DWORD_1(req, SET_MAC_EXT_IN_CONTROL, SET_MAC_EXT_IN_CFG_MTU,
                           1);
...
static __checkReturn efx_rc_t efx_mcdi_mtu_get(__in efx_nic_t *enp,
                                               __out size_t *mtu) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_SET_MAC_EXT_IN_LEN,
...
__checkReturn efx_rc_t ef10_mac_pdu_set(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
...
__checkReturn efx_rc_t ef10_mac_pdu_get(__in efx_nic_t *enp,
                                        __out size_t *pdu) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_mac_reconfigure(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_SET_MAC_IN_LEN, MC_CMD_SET_MAC_OUT_LEN);
  efx_rc_t rc;
  ...
  EFX_MAC_ADDR_COPY(MCDI_IN2(req, uint8_t, SET_MAC_IN_ADDR), epp->ep_mac_addr);
   ...
   */
  MCDI_IN_POPULATE_DWORD_2(req, SET_MAC_IN_REJECT, SET_MAC_IN_REJECT_UNCST, 0,
  ...
  MCDI_IN_POPULATE_DWORD_1(req, SET_MAC_IN_FLAGS, SET_MAC_IN_FLAG_INCLUDE_FCS,
                           0);
   ...
   */
  rc = efx_filter_reconfigure(enp, epp->ep_mac_addr, epp->ep_all_unicst,
                              epp->ep_mulcst, epp->ep_all_mulcst,
                              epp->ep_brdcst, epp->ep_mulcst_addr_list,
...
__checkReturn efx_rc_t ef10_mac_multicast_list_set(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
...
__checkReturn efx_rc_t ef10_mac_filter_default_rxq_set(
...
void ef10_mac_filter_default_rxq_clear(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);

  ef10_filter_default_rxq_clear(enp);

  (void)efx_filter_reconfigure(enp, epp->ep_mac_addr, epp->ep_all_unicst,
                               epp->ep_mulcst, epp->ep_all_mulcst,
                               epp->ep_brdcst, epp->ep_mulcst_addr_list,
                               epp->ep_mulcst_addr_count);
}
...
__checkReturn efx_rc_t ef10_mac_stats_get_mask(__in efx_nic_t *enp,
  ...
  if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_common,
                                          EFX_ARRAY_SIZE(ef10_common))) != 0)
    goto fail1;
  ...
  if (epp->ep_phy_cap_mask & (1 << MC_CMD_PHY_CAP_40000FDX_LBN)) {
    ...
    if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_40g_extra,
                                            EFX_ARRAY_SIZE(ef10_40g_extra))) !=
        0)
      goto fail2;

    if (encp->enc_mac_stats_40g_tx_size_bins) {
      if ((rc = efx_mac_stats_mask_add_ranges(
               maskp, mask_size, ef10_tx_size_bins,
  ...
  if (encp->enc_pm_and_rxdp_counters) {
    ...
    if ((rc = efx_mac_stats_mask_add_ranges(
             maskp, mask_size, ef10_pm_and_rxdp,
             EFX_ARRAY_SIZE(ef10_pm_and_rxdp))) != 0)
      goto fail5;
  }

  if (encp->enc_datapath_cap_evb) {
    const struct efx_mac_stats_range ef10_vadaptor[] = {
        {EFX_MAC_VADAPTER_RX_UNICAST_PACKETS, EFX_MAC_VADAPTER_TX_OVERFLOW},
    };

    if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_vadaptor,
                                            EFX_ARRAY_SIZE(ef10_vadaptor))) !=
        0)
      goto fail6;
  }
  ...
  if (encp->enc_fec_counters) {
    const struct efx_mac_stats_range ef10_fec[] = {
        {EFX_MAC_FEC_UNCORRECTED_ERRORS, EFX_MAC_FEC_CORRECTED_SYMBOLS_LANE3},
    };
    if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_fec,
                                            EFX_ARRAY_SIZE(ef10_fec))) != 0)
      goto fail7;
  }
  ...
  if (encp->enc_mac_stats_nstats >= MC_CMD_MAC_NSTATS_V4) {
    ...
    if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_rxdp_sdt,
                                            EFX_ARRAY_SIZE(ef10_rxdp_sdt))) !=
        0)
      goto fail8;
  }
  ...
  if (encp->enc_hlb_counters) {
    ...
    if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_hlb,
                                            EFX_ARRAY_SIZE(ef10_hlb))) != 0)
      goto fail9;
  }
...
ef10_mac_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
  ...
  EF10_MAC_STAT_READ(esmp, (encp->enc_mac_stats_nstats - 1), &generation_end);
  ...
  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_TX_MULTIPLE_COLLISION_PKTS, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_TX_MULT_COL_PKTS]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_TX_EXCESSIVE_COLLISION_PKTS, &value);
  ...
  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_TX_EXCESSIVE_DEFERRED_PKTS, &value);
  ...
  /* VADAPTER RX */
  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_RX_UNICAST_PACKETS, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_RX_UNICAST_PACKETS]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_RX_UNICAST_BYTES, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_RX_UNICAST_BYTES]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_RX_MULTICAST_PACKETS, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_RX_MULTICAST_PACKETS]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_RX_MULTICAST_BYTES, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_RX_MULTICAST_BYTES]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_RX_BROADCAST_PACKETS, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_RX_BROADCAST_PACKETS]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_RX_BROADCAST_BYTES, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_RX_BROADCAST_BYTES]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_RX_BAD_PACKETS, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_RX_BAD_PACKETS]), &value);
  ...
  /* VADAPTER TX */
  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_TX_UNICAST_PACKETS, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_TX_UNICAST_PACKETS]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_TX_UNICAST_BYTES, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_TX_UNICAST_BYTES]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_TX_MULTICAST_PACKETS, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_TX_MULTICAST_PACKETS]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_TX_MULTICAST_BYTES, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_TX_MULTICAST_BYTES]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_TX_BROADCAST_PACKETS, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_TX_BROADCAST_PACKETS]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_VADAPTER_TX_BROADCAST_BYTES, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_VADAPTER_TX_BROADCAST_BYTES]), &value);
  ...
  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_FEC_CORRECTED_SYMBOLS_LANE0, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_FEC_CORRECTED_SYMBOLS_LANE0]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_FEC_CORRECTED_SYMBOLS_LANE1, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_FEC_CORRECTED_SYMBOLS_LANE1]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_FEC_CORRECTED_SYMBOLS_LANE2, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_FEC_CORRECTED_SYMBOLS_LANE2]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_FEC_CORRECTED_SYMBOLS_LANE3, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_FEC_CORRECTED_SYMBOLS_LANE3]), &value);
  ...
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_CTPIO_VI_CLOBBER_FALLBACK]), &value);

  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_CTPIO_UNQUALIFIED_FALLBACK, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_CTPIO_UNQUALIFIED_FALLBACK]), &value);
  ...
  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_RXDP_SCATTER_DISABLED_TRUNC, &value);
  EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_RXDP_SCATTER_DISABLED_TRUNC]), &value);
  ...
  EFSYS_DMA_SYNC_FOR_KERNEL(esmp, 0, EFSYS_MEM_SIZE(esmp));
  EFSYS_MEM_READ_BARRIER();
  EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_GENERATION_START, &generation_start);

  /* Check that we didn't read the stats in the middle of a DMA */
  /* Not a good enough check ? */
  if (memcmp(&generation_start, &generation_end, sizeof(generation_start)))
    return (EAGAIN);
