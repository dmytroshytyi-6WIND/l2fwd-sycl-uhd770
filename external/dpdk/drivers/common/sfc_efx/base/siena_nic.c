...
static __checkReturn efx_rc_t siena_board_cfg(__in efx_nic_t *enp) {
  ...
  if (EFX_DWORD_FIELD(capabilities, MC_CMD_CAPABILITIES_TURBO)) {
    ...
    if (EFX_DWORD_FIELD(capabilities, MC_CMD_CAPABILITIES_TURBO_ACTIVE)) {
      encp->enc_clk_mult = 2;
    }
  }
  ...
  encp->enc_evq_timer_max_us =
      (encp->enc_evq_timer_quantum_ns << FRF_CZ_TC_TIMER_VAL_WIDTH) / 1000;
...
static __checkReturn efx_rc_t siena_phy_cfg(__in efx_nic_t *enp) {
...
static __checkReturn efx_rc_t siena_nic_biu_test(__in efx_nic_t *enp) {
  efx_oword_t oword;
  efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nic_probe(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t siena_nic_reset(__in efx_nic_t *enp) {
  efx_mcdi_req_t req;
  efx_rc_t rc;
...
static void siena_nic_rx_cfg(__in efx_nic_t *enp) {
  efx_oword_t oword;
...
static void siena_nic_usrev_dis(__in efx_nic_t *enp) {
  efx_oword_t oword;
...
__checkReturn efx_rc_t siena_nic_init(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
void siena_nic_fini(__in efx_nic_t *enp) { _NOTE(ARGUNUSED(enp)) }

void siena_nic_unprobe(__in efx_nic_t *enp) {
#if EFSYS_OPT_MON_STATS
...
static siena_register_set_t __siena_registers[] = {
    {FR_AZ_ADR_REGION_REG_OFST, 0, 1},   {FR_CZ_USR_EV_CFG_OFST, 0, 1},
    {FR_AZ_RX_CFG_REG_OFST, 0, 1},       {FR_AZ_TX_CFG_REG_OFST, 0, 1},
    {FR_AZ_TX_RESERVED_REG_OFST, 0, 1},  {FR_AZ_SRM_TX_DC_CFG_REG_OFST, 0, 1},
    {FR_AZ_RX_DC_CFG_REG_OFST, 0, 1},    {FR_AZ_RX_DC_PF_WM_REG_OFST, 0, 1},
    {FR_AZ_DP_CTRL_REG_OFST, 0, 1},      {FR_BZ_RX_RSS_TKEY_REG_OFST, 0, 1},
    {FR_CZ_RX_RSS_IPV6_REG1_OFST, 0, 1}, {FR_CZ_RX_RSS_IPV6_REG2_OFST, 0, 1},
    {FR_CZ_RX_RSS_IPV6_REG3_OFST, 0, 1}};

static const uint32_t __siena_register_masks[] = {
    0x0003FFFF, 0x0003FFFF, 0x0003FFFF, 0x0003FFFF, 0x000103FF, 0x00000000,
    0x00000000, 0x00000000, 0xFFFFFFFE, 0xFFFFFFFF, 0x0003FFFF, 0x00000000,
    0x7FFF0037, 0xFFFF8000, 0xFFFFFFFF, 0x03FFFFFF, 0xFFFEFE80, 0x1FFFFFFF,
    0x020000FE, 0x007FFFFF, 0x001FFFFF, 0x00000000, 0x00000000, 0x00000000,
    0x00000003, 0x00000000, 0x00000000, 0x00000000, 0x000003FF, 0x00000000,
    0x00000000, 0x00000000, 0x00000FFF, 0x00000000, 0x00000000, 0x00000000,
...
static const uint32_t __siena_table_masks[] = {
    0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x000003FF, 0xFFFF0FFF, 0xFFFFFFFF,
    0x00000E7F, 0x00000000, 0xFFFFFFFE, 0x0FFFFFFF, 0x01800000, 0x00000000,
    0xFFFFFFFE, 0x0FFFFFFF, 0x0C000000, 0x00000000, 0x3FFFFFFF, 0x00000000,
    0x00000000, 0x00000000, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x000013FF,
    0xFFFF07FF, 0xFFFFFFFF, 0x0000007F, 0x00000000,
};

__checkReturn efx_rc_t siena_nic_test_registers(__in efx_nic_t *enp,
  ...
  while (count > 0) {
    ...
    for (bit = 0; bit < 128; bit++) {
      ...
      EFSYS_BAR_WRITEO(enp->en_esbp, rsp->address, &reg, B_TRUE);
      EFSYS_BAR_READO(enp->en_esbp, rsp->address, &buf, B_TRUE);
...
__checkReturn efx_rc_t siena_nic_test_tables(__in efx_nic_t *enp,
                                             __in siena_register_set_t *rsp,
...
__checkReturn efx_rc_t siena_nic_register_test(__in efx_nic_t *enp) {
  ...
  if ((rc =
           siena_nic_test_tables(enp, __siena_tables, EFX_PATTERN_BYTE_CHANGING,
                                 EFX_ARRAY_SIZE(__siena_tables))) != 0)
    goto fail3;

  if ((rc = siena_nic_test_tables(enp, __siena_tables, EFX_PATTERN_BIT_SWEEP,
                                  EFX_ARRAY_SIZE(__siena_tables))) != 0)
    goto fail4;
