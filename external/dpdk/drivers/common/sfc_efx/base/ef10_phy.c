...
static void mcdi_phy_decode_cap(__in uint32_t mcdi_cap, __out uint32_t *maskp) {
  uint32_t mask;
...
mcdi_phy_decode_link_mode(__in efx_nic_t *enp, __in uint32_t link_flags,
                          ...
                          __in uint32_t fec, __out efx_link_mode_t *link_modep,
                          __out unsigned int *fcntlp,
                          __out efx_phy_fec_type_t *fecp) {
  boolean_t fd = !!(link_flags & (1 << MC_CMD_GET_LINK_OUT_FULL_DUPLEX_LBN));
...
void ef10_phy_link_ev(__in efx_nic_t *enp, __in efx_qword_t *eqp,
  ...
  mcdi_phy_decode_link_mode(enp, link_flags, speed,
                            ...
                            MC_CMD_FEC_NONE, &link_mode, &fcntl, &fec);
  mcdi_phy_decode_cap(MCDI_EV_FIELD(eqp, LINKCHANGE_LP_CAP), &lp_cap_mask);
...
__checkReturn efx_rc_t ef10_phy_power(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t ef10_phy_get_link(__in efx_nic_t *enp,
...
static __checkReturn efx_rc_t efx_mcdi_phy_set_link(
    __in efx_nic_t *enp, __in uint32_t cap_mask,
    __in efx_loopback_type_t loopback_type,
    __in efx_link_mode_t loopback_link_mode, __in uint32_t phy_flags) {
  efx_mcdi_req_t req;
  ...
  MCDI_IN_POPULATE_DWORD_10(
      req, SET_LINK_IN_CAP, PHY_CAP_10HDX,
  ...
  MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_BASER_FEC_REQUESTED,
  ...
  MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_25G_BASER_FEC_REQUESTED,
                          (cap_mask >> EFX_PHY_CAP_25G_BASER_FEC_REQUESTED) &
                              0x1);
...
static __checkReturn efx_rc_t efx_mcdi_phy_set_led(
    __in efx_nic_t *enp, __in efx_phy_led_mode_t phy_led_mode) {
  efx_mcdi_req_t req;
...
__checkReturn efx_rc_t ef10_phy_reconfigure(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
  ...
  rc = efx_mcdi_phy_set_link(enp, epp->ep_adv_cap_mask, loopback_type,
                             loopback_link_mode, phy_flags);
  if (rc != 0)
    goto fail2;
...
__checkReturn efx_rc_t ef10_phy_verify(__in efx_nic_t *enp) {
  efx_mcdi_req_t req;
...
__checkReturn efx_rc_t ef10_phy_oui_get(__in efx_nic_t *enp,
                                        __out uint32_t *ouip) {
...
__checkReturn efx_rc_t ef10_phy_link_state_get(
    __in efx_nic_t *enp, __out efx_phy_link_state_t *eplsp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_phy_stats_update(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t ef10_bist_enable_offline(__in efx_nic_t *enp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_bist_start(__in efx_nic_t *enp,
...
ef10_bist_poll(__in efx_nic_t *enp, __in efx_bist_type_t type,
  ...
  EFX_STATIC_ASSERT(MC_CMD_POLL_BIST_OUT_SFT9001_LEN <=
                    MCDI_CTL_SDU_LEN_MAX_V1);
  EFX_STATIC_ASSERT(MC_CMD_POLL_BIST_OUT_MRSFP_LEN <= MCDI_CTL_SDU_LEN_MAX_V1);
  EFX_STATIC_ASSERT(MC_CMD_POLL_BIST_OUT_MEM_LEN <= MCDI_CTL_SDU_LEN_MAX_V1);
  ...
  if (result == MC_CMD_POLL_BIST_FAILED &&
    ...
    if (valuesp != NULL) {
      valuesp[EFX_BIST_MEM_TEST] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MEM_TEST);
      valuesp[EFX_BIST_MEM_ADDR] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MEM_ADDR);
      valuesp[EFX_BIST_MEM_BUS] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MEM_BUS);
      ...
      valuesp[EFX_BIST_MEM_ECC] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MEM_ECC);
    ...
    value_mask |= (1 << EFX_BIST_MEM_TEST) | (1 << EFX_BIST_MEM_ADDR) |
                  (1 << EFX_BIST_MEM_BUS) | (1 << EFX_BIST_MEM_EXPECT) |
                  (1 << EFX_BIST_MEM_ACTUAL) | (1 << EFX_BIST_MEM_ECC) |
...
void ef10_bist_stop(__in efx_nic_t *enp, __in efx_bist_type_t type) {
