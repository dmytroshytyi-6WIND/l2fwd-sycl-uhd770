...
struct tlv_start {
  uint32_t tag;
  uint32_t length;
  /* Length of the TLV structure following this tag - includes length of all
   * tags within the TLV layout starting with this TLV_TAG_START. Includes
   * TLV_TAG_END. Does not include TLV_TAG_START
   */
...
struct tlv_partition_header {
  ...
  uint32_t length;
  uint16_t type_id;
  /* 0 indicates the default segment (always located at offset 0), while other
   * values are for RFID-selectable presets that should immediately follow the
   * default segment. The default segment may also have preset > 0, which means
   * that it is a preset selected through an RFID command and copied by FW to
   * the location at offset 0. */
  uint16_t preset;
  uint32_t generation;
 ...
 * TLV_TAG_GLOBAL_PCIE_CONFIG can and it should be used in place of that tag in
 * future (but compatibility with the old tag will be left in the firmware
 * indefinitely).  */
 ...
 *  - quad lane ports can do 100G/40G/50G/25G/10G/1G (with fallback to 2 or 1
 lanes)
...
#define TLV_PORT_MODE_1x1_1x1                                                  \
  (2) /* Single 10G/25G on mdi0, single 10G/25G on mdi1 */
#define TLV_PORT_MODE_1x4_1x4 (3) /* Single 40G on mdi0, single 40G on mdi1 */
#define TLV_PORT_MODE_2x1_2x1                                                  \
  (5) /* Dual 10G/25G on mdi0, dual 10G/25G on mdi1 */
#define TLV_PORT_MODE_4x1_NA (4) /* Quad 10G/25G on mdi0 */
#define TLV_PORT_MODE_NA_4x1 (8) /* Quad 10G/25G on mdi1 */
#define TLV_PORT_MODE_1x4_2x1                                                  \
  (6) /* Single 40G on mdi0, dual 10G/25G on mdi1                              \
       */
#define TLV_PORT_MODE_2x1_1x4                                                  \
  (7) /* Dual 10G/25G on mdi0, single 40G on mdi1                              \
       */
#define TLV_PORT_MODE_1x2_1x2                                                  \
  (12)                            /* Single 50G on mdi0, single 50G on mdi1    \
                                   */
#define TLV_PORT_MODE_2x2_NA (13) /* Dual 50G on mdi0 */
#define TLV_PORT_MODE_NA_2x2 (14) /* Dual 50G on mdi1 */
#define TLV_PORT_MODE_1x4_1x2                                                  \
  (15) /* Single 40G on mdi0, single 50G on mdi1                               \
        */
#define TLV_PORT_MODE_1x2_1x4                                                  \
  (16) /* Single 50G on mdi0, single 40G on mdi1                               \
        */
#define TLV_PORT_MODE_1x2_2x1                                                  \
  (17) /* Single 50G on mdi0, dual 10G/25G on mdi1 */
#define TLV_PORT_MODE_2x1_1x2                                                  \
  (18) /* Dual 10G/25G on mdi0, single 50G on mdi1 */
 ...
 * care about LLPCS.
 */
#define TLV_PORT_MODE_2x1_2x1_LL                                               \
  (19) /* Dual 10G/25G on mdi0, dual 10G/25G on mdi1, low-latency PCS */
#define TLV_PORT_MODE_4x1_NA_LL                                                \
  (20) /* Quad 10G/25G on mdi0, low-latency PCS                                \
        */
#define TLV_PORT_MODE_NA_4x1_LL                                                \
  (21) /* Quad 10G/25G on mdi1, low-latency PCS                                \
        */
#define TLV_PORT_MODE_1x1_NA_LL                                                \
  (23) /* Single 10G/25G on mdi0, low-latency PCS */
#define TLV_PORT_MODE_1x1_1x1_LL                                               \
  (24) /* Single 10G/25G on mdi0, single 10G/25G on mdi1, low-latency PCS */
...
#define TLV_VSWITCH_TYPE_DEFAULT                                               \
  (0xffffffff) /* Firmware default; equivalent to no TLV present for a given   \
                  port */
...
struct tlv_vport_vlan_tag {
  ...
  uint32_t length;
  uint32_t vlan_tag;
#define TLV_VPORT_NO_VLAN_TAG                                                  \
  (0xFFFFFFFF) /* Default in the absence of TLV for a given PF */
};
...
/* A privilege mask given on reset to all non-admin PCIe functions (that is
 * other than first-PF-per-port). The meaning of particular bits is defined in
 * mcdi_ef10.yml under MC_CMD_PRIVILEGE_MASK, see also bug 44583.
 * TLV_TAG_PRIVILEGE_MASK_ADD specifies bits that should be added (ORed) to
 * firmware default while TLV_TAG_PRIVILEGE_MASK_REM specifies bits that should
 * be removed (ANDed) from firmware default: Initial_privilege_mask =
 * (firmware_default_mask | privilege_mask_add) & ~privilege_mask_rem */
 ...
 * This tag only takes effect if TLV_TAG_VSWITCH_TYPE is missing or set to
 * DEFAULT. */
...
struct tlv_biu_vi_window_mode {
  ...
  uint32_t length;
  uint8_t mode;
#define TLV_BIU_VI_WINDOW_MODE_8K                                              \
  0 /*  8k per VI, CTPIO not mapped, medford/hunt compatible */
#define TLV_BIU_VI_WINDOW_MODE_16K 1 /* 16k per VI, CTPIO mapped */
#define TLV_BIU_VI_WINDOW_MODE_64K                                             \
  2 /* 64k per VI, CTPIO mapped, POWER-friendly */
};
...
#define TLV_FASTPD_MODE_FAST_SUPPORTED                                         \
  2 /* Supported packet types to the FastPD; everything else to the SoftPD  */
};
