...
__checkReturn efx_rc_t efx_mcdi_init(__in efx_nic_t *enp,
...
void efx_mcdi_fini(__in efx_nic_t *enp) {
...
void efx_mcdi_new_epoch(__in efx_nic_t *enp) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
...
static void efx_mcdi_send_request(__in efx_nic_t *enp, __in void *hdrp,
                                  __in size_t hdr_len, __in void *sdup,
                                  __in size_t sdu_len) {
  const efx_mcdi_ops_t *emcop = enp->en_mcdi.em_emcop;
...
static efx_rc_t efx_mcdi_poll_reboot(__in efx_nic_t *enp) {
  const efx_mcdi_ops_t *emcop = enp->en_mcdi.em_emcop;
  efx_rc_t rc;
...
static boolean_t efx_mcdi_poll_response(__in efx_nic_t *enp) {
  const efx_mcdi_ops_t *emcop = enp->en_mcdi.em_emcop;
  boolean_t available;
...
static void efx_mcdi_read_response(__in efx_nic_t *enp, __out void *bufferp,
...
void efx_mcdi_request_start(__in efx_nic_t *enp, __in efx_mcdi_req_t *emrp,
  ...
  if ((max_version >= 2) &&
    ...
    hdr_len = sizeof(hdr);
    EFX_POPULATE_DWORD_8(hdr[0], MCDI_HEADER_CODE, MC_CMD_V2_EXTN,
                         MCDI_HEADER_RESYNC, 1, MCDI_HEADER_DATALEN, 0,
                         MCDI_HEADER_SEQ, seq, MCDI_HEADER_NOT_EPOCH,
                         new_epoch ? 0 : 1, MCDI_HEADER_ERROR, 0,
                         MCDI_HEADER_RESPONSE, 0, MCDI_HEADER_XFLAGS, xflags);

    EFX_POPULATE_DWORD_2(hdr[1], MC_CMD_V2_EXTN_IN_EXTENDED_CMD, emrp->emr_cmd,
                         MC_CMD_V2_EXTN_IN_ACTUAL_LEN, emrp->emr_in_length);
  } else {
    /* Construct MCDI v1 header */
    hdr_len = sizeof(hdr[0]);
    EFX_POPULATE_DWORD_8(
        hdr[0], MCDI_HEADER_CODE, emrp->emr_cmd, MCDI_HEADER_RESYNC, 1,
        MCDI_HEADER_DATALEN, emrp->emr_in_length, MCDI_HEADER_SEQ, seq,
        MCDI_HEADER_NOT_EPOCH, new_epoch ? 0 : 1, MCDI_HEADER_ERROR, 0,
        MCDI_HEADER_RESPONSE, 0, MCDI_HEADER_XFLAGS, xflags);
  }

#if EFSYS_OPT_MCDI_LOGGING
  if (emtp->emt_logger != NULL) {
    emtp->emt_logger(emtp->emt_context, EFX_LOG_MCDI_REQUEST, &hdr[0], hdr_len,
                     emrp->emr_in_buf, emrp->emr_in_length);
  }
#endif /* EFSYS_OPT_MCDI_LOGGING */

  efx_mcdi_send_request(enp, &hdr[0], hdr_len, emrp->emr_in_buf,
                        emrp->emr_in_length);
}

static void efx_mcdi_read_response_header(__in efx_nic_t *enp,
  ...
  if (cmd != MC_CMD_V2_EXTN) {
    ...
    data_len = EFX_DWORD_FIELD(hdr[1], MC_CMD_V2_EXTN_IN_ACTUAL_LEN);
  }
  ...
  if (error) {
    ...
    if ((err_code == MC_CMD_ERR_PROXY_PENDING) && (err_len == sizeof(err))) {
      /*
...
#if EFSYS_OPT_MCDI_LOGGING
    if (emtp->emt_logger != NULL) {
      emtp->emt_logger(emtp->emt_context, EFX_LOG_MCDI_RESPONSE, &hdr[0],
                       hdr_len, &err[0], err_len);
    }
#endif /* EFSYS_OPT_MCDI_LOGGING */

    if (!emrp->emr_quiet) {
      EFSYS_PROBE3(mcdi_err_arg, int, emrp->emr_cmd, int, err_code, int,
                   err_arg);
    }
...
static void efx_mcdi_finish_response(__in efx_nic_t *enp,
  ...
  if (EFX_DWORD_FIELD(hdr[0], MCDI_HEADER_CODE) == MC_CMD_V2_EXTN) {
    ...
    proxied_cmd_resp =
        ((emrp->emr_cmd == MC_CMD_PROXY_CMD) && (resp_cmd != MC_CMD_PROXY_CMD));
    if (proxied_cmd_resp) {
      resp_off = 0;
...
#if EFSYS_OPT_MCDI_LOGGING
  if (emtp->emt_logger != NULL) {
    emtp->emt_logger(emtp->emt_context, EFX_LOG_MCDI_RESPONSE, &hdr[0], hdr_len,
                     emrp->emr_out_buf, bytes);
  }
#endif /* EFSYS_OPT_MCDI_LOGGING */
}

__checkReturn boolean_t efx_mcdi_request_poll(__in efx_nic_t *enp) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
  efx_mcdi_req_t *emrp;
  efsys_lock_state_t state;
  efx_rc_t rc;
...
__checkReturn boolean_t efx_mcdi_request_abort(__in efx_nic_t *enp) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
  efx_mcdi_req_t *emrp;
  boolean_t aborted;
  efsys_lock_state_t state;
...
__checkReturn efx_rc_t efx_mcdi_get_client_handle(
    __in efx_nic_t *enp, __in efx_pcie_interface_t intf, __in uint16_t pf,
    __in uint16_t vf, __out uint32_t *handle) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_CLIENT_HANDLE_IN_LEN,
...
__checkReturn efx_rc_t efx_mcdi_get_own_client_handle(__in efx_nic_t *enp,
                                                      __out uint32_t *handle) {
  efx_rc_t rc;

  rc = efx_mcdi_get_client_handle(enp, PCIE_INTERFACE_CALLER,
                                  PCIE_FUNCTION_PF_NULL, PCIE_FUNCTION_VF_NULL,
                                  handle);
  if (rc != 0)
    goto fail1;
...
void efx_mcdi_get_timeout(__in efx_nic_t *enp, __in efx_mcdi_req_t *emrp,
...
__checkReturn efx_rc_t efx_mcdi_request_errcode(__in unsigned int err) {
...
void efx_mcdi_raise_exception(__in efx_nic_t *enp,
                              __in_opt efx_mcdi_req_t *emrp, __in int rc) {
  const efx_mcdi_transport_t *emtp = enp->en_mcdi.em_emtp;
  ...
  exception = (rc == EIO) ? EFX_MCDI_EXCEPTION_MC_REBOOT
                          : EFX_MCDI_EXCEPTION_MC_BADASSERT;

  emtp->emt_exception(emtp->emt_context, exception);
}

void efx_mcdi_execute(__in efx_nic_t *enp, __inout efx_mcdi_req_t *emrp) {
  const efx_mcdi_transport_t *emtp = enp->en_mcdi.em_emtp;
...
void efx_mcdi_execute_quiet(__in efx_nic_t *enp, __inout efx_mcdi_req_t *emrp) {
  const efx_mcdi_transport_t *emtp = enp->en_mcdi.em_emtp;
...
void efx_mcdi_ev_cpl(__in efx_nic_t *enp, __in unsigned int seq,
                     __in unsigned int outlen, __in int errcode) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
  const efx_mcdi_transport_t *emtp = enp->en_mcdi.em_emtp;
  efx_mcdi_req_t *emrp;
  efsys_lock_state_t state;
...
__checkReturn efx_rc_t efx_mcdi_get_proxy_handle(__in efx_nic_t *enp,
                                                 __in efx_mcdi_req_t *emrp,
  ...
  if ((emrp->emr_rc != 0) && (emrp->emr_err_code == MC_CMD_ERR_PROXY_PENDING)) {
...
void efx_mcdi_ev_proxy_response(__in efx_nic_t *enp, __in unsigned int handle,
...
#if EFSYS_OPT_MCDI_PROXY_AUTH_SERVER
void efx_mcdi_ev_proxy_request(__in efx_nic_t *enp, __in unsigned int index) {
  const efx_mcdi_transport_t *emtp = enp->en_mcdi.em_emtp;
...
#endif /* EFSYS_OPT_MCDI_PROXY_AUTH_SERVER */
void efx_mcdi_ev_death(__in efx_nic_t *enp, __in int rc) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
  const efx_mcdi_transport_t *emtp = enp->en_mcdi.em_emtp;
...
__checkReturn efx_rc_t efx_mcdi_get_version(__in efx_nic_t *enp,
                                            __in uint32_t flags,
                                            __out efx_mcdi_version_t *verp) {
  efx_nic_board_info_t *board_infop = &verp->emv_board_info;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_VERSION_EXT_IN_LEN,
...
efx_mcdi_get_boot_status(__in efx_nic_t *enp, __out efx_mcdi_boot_t *statusp) {
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_BOOT_STATUS_IN_LEN,
...
__checkReturn efx_rc_t efx_mcdi_version(__in efx_nic_t *enp,
                                        __out_ecount_opt(4)
                                            uint16_t versionp[4],
...
__checkReturn efx_rc_t efx_mcdi_get_capabilities(
    __in efx_nic_t *enp, __out_opt uint32_t *flagsp,
    __out_opt uint16_t *rx_dpcpu_fw_idp, __out_opt uint16_t *tx_dpcpu_fw_idp,
    __out_opt uint32_t *flags2p, __out_opt uint32_t *tso2ncp) {
  efx_mcdi_req_t req;
  ...
  if (rx_dpcpu_fw_idp != NULL)
    *rx_dpcpu_fw_idp = MCDI_OUT_WORD(req, GET_CAPABILITIES_OUT_RX_DPCPU_FW_ID);

  if (tx_dpcpu_fw_idp != NULL)
    *tx_dpcpu_fw_idp = MCDI_OUT_WORD(req, GET_CAPABILITIES_OUT_TX_DPCPU_FW_ID);
  ...
  if (flags2p != NULL) {
    *flags2p =
        (v2_capable) ? MCDI_OUT_DWORD(req, GET_CAPABILITIES_V2_OUT_FLAGS2) : 0;
  }

  if (tso2ncp != NULL) {
    *tso2ncp =
        (v2_capable)
            ? MCDI_OUT_WORD(req, GET_CAPABILITIES_V2_OUT_TX_TSO_V2_N_CONTEXTS)
            : 0;
  }
...
static __checkReturn efx_rc_t
efx_mcdi_do_reboot(__in efx_nic_t *enp, __in boolean_t after_assertion) {
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_REBOOT_IN_LEN, MC_CMD_REBOOT_OUT_LEN);
  efx_mcdi_req_t req;
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_reboot(__in efx_nic_t *enp) {
  return (efx_mcdi_do_reboot(enp, B_FALSE));
}

__checkReturn efx_rc_t efx_mcdi_exit_assertion_handler(__in efx_nic_t *enp) {
  return (efx_mcdi_do_reboot(enp, B_TRUE));
}

__checkReturn efx_rc_t efx_mcdi_read_assertion(__in efx_nic_t *enp) {
  efx_mcdi_req_t req;
  ...
  reason =
      (flags == MC_CMD_GET_ASSERTS_FLAGS_SYS_FAIL)   ? "system-level assertion"
      : (flags == MC_CMD_GET_ASSERTS_FLAGS_THR_FAIL) ? "thread-level assertion"
      : (flags == MC_CMD_GET_ASSERTS_FLAGS_WDOG_FIRED) ? "watchdog reset"
      : (flags == MC_CMD_GET_ASSERTS_FLAGS_ADDR_TRAP)  ? "illegal address trap"
                                                       : "unknown assertion";
  EFSYS_PROBE3(mcpu_assertion, const char *, reason, unsigned int,
               MCDI_OUT_DWORD(req, GET_ASSERTS_OUT_SAVED_PC_OFFS), unsigned int,
               MCDI_OUT_DWORD(req, GET_ASSERTS_OUT_THREAD_OFFS));

  /* Print out the registers (r1 ... r31) */
  ofst = MC_CMD_GET_ASSERTS_OUT_GP_REGS_OFFS_OFST;
  for (index = 1; index < 1 + MC_CMD_GET_ASSERTS_OUT_GP_REGS_OFFS_NUM;
       index++) {
    EFSYS_PROBE2(
        mcpu_register, unsigned int, index, unsigned int,
        EFX_DWORD_FIELD(*MCDI_OUT(req, efx_dword_t, ofst), EFX_DWORD_0));
...
__checkReturn efx_rc_t efx_mcdi_drv_attach(__in efx_nic_t *enp,
   ...
   */
  MCDI_IN_POPULATE_DWORD_2(req, DRV_ATTACH_IN_NEW_STATE, DRV_ATTACH_IN_ATTACH,
                           attach ? 1 : 0, DRV_ATTACH_IN_SUBVARIANT_AWARE,
...
efx_mcdi_get_board_cfg(__in efx_nic_t *enp, __out_opt uint32_t *board_typep,
  ...
  if (mac_addrp != NULL) {
    ...
    if (emip->emi_port == 1) {
      addrp = MCDI_OUT2(req, uint8_t, GET_BOARD_CFG_OUT_MAC_ADDR_BASE_PORT0);
    } else if (emip->emi_port == 2) {
      addrp = MCDI_OUT2(req, uint8_t, GET_BOARD_CFG_OUT_MAC_ADDR_BASE_PORT1);
    } else {
      rc = EINVAL;
      goto fail3;
    }
  ...
  if (capabilitiesp != NULL) {
    if (emip->emi_port == 1) {
      *capabilitiesp =
          *MCDI_OUT2(req, efx_dword_t, GET_BOARD_CFG_OUT_CAPABILITIES_PORT0);
    } else if (emip->emi_port == 2) {
      *capabilitiesp =
  ...
  if (board_typep != NULL) {
    *board_typep = MCDI_OUT_DWORD(req, GET_BOARD_CFG_OUT_BOARD_TYPE);
  }
...
__checkReturn efx_rc_t efx_mcdi_get_resource_limits(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mcdi_get_phy_cfg(__in efx_nic_t *enp) {
  ...
  (void)memset(encp->enc_phy_name, 0, sizeof(encp->enc_phy_name));
  memcpy(encp->enc_phy_name, namep, namelen);
#endif /* EFSYS_OPT_NAMES */
  (void)memset(encp->enc_phy_revision, 0, sizeof(encp->enc_phy_revision));
  memcpy(encp->enc_phy_revision, MCDI_OUT2(req, char, GET_PHY_CFG_OUT_REVISION),
         MIN(sizeof(encp->enc_phy_revision) - 1,
             MC_CMD_GET_PHY_CFG_OUT_REVISION_LEN));
#if EFSYS_OPT_PHY_LED_CONTROL
  encp->enc_led_mask = ((1 << EFX_PHY_LED_DEFAULT) | (1 << EFX_PHY_LED_OFF) |
                        (1 << EFX_PHY_LED_ON));
  ...
  epp->ep_phy_cap_mask = MCDI_OUT_DWORD(req, GET_PHY_CFG_OUT_SUPPORTED_CAP);
  ...
  if (MCDI_OUT_DWORD_FIELD(req, GET_PHY_CFG_OUT_FLAGS, GET_PHY_CFG_OUT_BIST))
...
__checkReturn efx_rc_t efx_mcdi_firmware_update_supported(
    __in efx_nic_t *enp, __out boolean_t *supportedp) {
  const efx_mcdi_ops_t *emcop = enp->en_mcdi.em_emcop;
  efx_rc_t rc;

  if (emcop != NULL) {
    if ((rc = emcop->emco_feature_supported(enp, EFX_MCDI_FEATURE_FW_UPDATE,
                                            supportedp)) != 0)
      goto fail1;
  } else {
...
__checkReturn efx_rc_t efx_mcdi_macaddr_change_supported(
    __in efx_nic_t *enp, __out boolean_t *supportedp) {
  const efx_mcdi_ops_t *emcop = enp->en_mcdi.em_emcop;
  efx_rc_t rc;

  if (emcop != NULL) {
    if ((rc = emcop->emco_feature_supported(
             enp, EFX_MCDI_FEATURE_MACADDR_CHANGE, supportedp)) != 0)
      goto fail1;
  } else {
...
__checkReturn efx_rc_t efx_mcdi_link_control_supported(
    __in efx_nic_t *enp, __out boolean_t *supportedp) {
  const efx_mcdi_ops_t *emcop = enp->en_mcdi.em_emcop;
  efx_rc_t rc;

  if (emcop != NULL) {
    if ((rc = emcop->emco_feature_supported(enp, EFX_MCDI_FEATURE_LINK_CONTROL,
                                            supportedp)) != 0)
      goto fail1;
  } else {
...
__checkReturn efx_rc_t efx_mcdi_mac_spoofing_supported(
    __in efx_nic_t *enp, __out boolean_t *supportedp) {
  const efx_mcdi_ops_t *emcop = enp->en_mcdi.em_emcop;
  efx_rc_t rc;

  if (emcop != NULL) {
    if ((rc = emcop->emco_feature_supported(enp, EFX_MCDI_FEATURE_MAC_SPOOFING,
                                            supportedp)) != 0)
      goto fail1;
  } else {
 ...
 */
__checkReturn efx_rc_t efx_mcdi_bist_enable_offline(__in efx_nic_t *enp) {
  efx_mcdi_req_t req;
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_bist_start(__in efx_nic_t *enp,
  ...
  case EFX_BIST_TYPE_PHY_CABLE_SHORT:
    MCDI_IN_SET_DWORD(req, START_BIST_IN_TYPE, MC_CMD_PHY_BIST_CABLE_SHORT);
    break;
  case EFX_BIST_TYPE_PHY_CABLE_LONG:
    MCDI_IN_SET_DWORD(req, START_BIST_IN_TYPE, MC_CMD_PHY_BIST_CABLE_LONG);
    break;
  case EFX_BIST_TYPE_MC_MEM:
    MCDI_IN_SET_DWORD(req, START_BIST_IN_TYPE, MC_CMD_MC_MEM_BIST);
    break;
  case EFX_BIST_TYPE_SAT_MEM:
    MCDI_IN_SET_DWORD(req, START_BIST_IN_TYPE, MC_CMD_PORT_MEM_BIST);
    break;
  case EFX_BIST_TYPE_REG:
    MCDI_IN_SET_DWORD(req, START_BIST_IN_TYPE, MC_CMD_REG_BIST);
    break;
  default:
    EFSYS_ASSERT(0);
  }
...
__checkReturn efx_rc_t efx_mcdi_log_ctrl(__in efx_nic_t *enp) {
  efx_mcdi_req_t req;
...
__checkReturn efx_rc_t efx_mcdi_mac_stats(__in efx_nic_t *enp,
  ...
  MCDI_IN_POPULATE_DWORD_6(
      req, MAC_STATS_IN_CMD, MAC_STATS_IN_DMA, upload, MAC_STATS_IN_CLEAR,
      clear, MAC_STATS_IN_PERIODIC_CHANGE, enable | events | disable,
      MAC_STATS_IN_PERIODIC_ENABLE, enable | events,
      MAC_STATS_IN_PERIODIC_NOEVENT, !events, MAC_STATS_IN_PERIOD_MS,
...
__checkReturn efx_rc_t efx_mcdi_mac_stats_clear(__in efx_nic_t *enp) {
  efx_rc_t rc;

  if ((rc = efx_mcdi_mac_stats(enp, enp->en_vport_id, NULL, EFX_STATS_CLEAR,
                               0)) != 0)
    goto fail1;
...
__checkReturn efx_rc_t efx_mcdi_mac_stats_upload(__in efx_nic_t *enp,
   ...
   */
  if ((rc = efx_mcdi_mac_stats(enp, enp->en_vport_id, esmp, EFX_STATS_UPLOAD,
                               0)) != 0)
    goto fail1;
...
__checkReturn efx_rc_t efx_mcdi_mac_stats_periodic(__in efx_nic_t *enp,
                                                   __in efsys_mem_t *esmp,
                                                   __in uint16_t period_ms,
   ...
   */
  if (period_ms == 0)
    rc = efx_mcdi_mac_stats(enp, enp->en_vport_id, NULL, EFX_STATS_DISABLE, 0);
  else if (events)
...
__checkReturn efx_rc_t efx_mcdi_intf_from_pcie(
 ...
 */
__checkReturn efx_rc_t efx_mcdi_get_function_info(
  ...
  *pfp = MCDI_OUT_DWORD(req, GET_FUNCTION_INFO_OUT_PF);
  ...
  if (req.emr_out_length < MC_CMD_GET_FUNCTION_INFO_OUT_V2_LEN) {
    ...
    pcie_intf = MCDI_OUT_DWORD(req, GET_FUNCTION_INFO_OUT_V2_INTF);
...
__checkReturn efx_rc_t efx_mcdi_privilege_mask(__in efx_nic_t *enp,
                                               __in uint32_t pf,
                                               __in uint32_t vf,
...
__checkReturn efx_rc_t efx_mcdi_set_workaround(__in efx_nic_t *enp,
                                               __in uint32_t type,
...
efx_mcdi_get_workarounds(__in efx_nic_t *enp, __out_opt uint32_t *implementedp,
  ...
  if (implementedp != NULL) {
    *implementedp = MCDI_OUT_DWORD(req, GET_WORKAROUNDS_OUT_IMPLEMENTED);
  }
...
static __checkReturn efx_rc_t efx_mcdi_get_phy_media_info(
    __in efx_nic_t *enp, __in uint32_t mcdi_page, __in uint8_t offset,
    __in uint8_t len, __out_bcount(len) uint8_t *data) {
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(
      payload, MC_CMD_GET_PHY_MEDIA_INFO_IN_LEN,
  ...
  memcpy(data, MCDI_OUT2(req, uint8_t, GET_PHY_MEDIA_INFO_OUT_DATA) + offset,
         len);
...
__checkReturn efx_rc_t efx_mcdi_phy_module_get_info(
  ...
  if (offset < EFX_PHY_MEDIA_INFO_PAGE_SIZE) {
    size_t read_len = MIN(len, EFX_PHY_MEDIA_INFO_PAGE_SIZE - offset);

    rc = efx_mcdi_get_phy_media_info(enp, mcdi_lower_page, (uint8_t)offset,
                                     (uint8_t)read_len, data);
    if (rc != 0)
      goto fail4;
  ...
  if (len > 0) {
    ...
    rc = efx_mcdi_get_phy_media_info(enp, mcdi_upper_page, (uint8_t)offset,
                                     (uint8_t)len, data);
    if (rc != 0)
      goto fail5;
  }
...
__checkReturn efx_rc_t efx_mcdi_init_evq(
  ...
  efx_mcdi_req_t req;
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_INIT_EVQ_V2_IN_LEN(INIT_EVQ_MAXNBUFS),
  ...
  interrupting =
      ((flags & EFX_EVQ_FLAGS_NOTIFY_MASK) == EFX_EVQ_FLAGS_NOTIFY_INTERRUPT);
  ...
  MCDI_IN_POPULATE_DWORD_8(
      req, INIT_EVQ_V2_IN_FLAGS, INIT_EVQ_V2_IN_FLAG_INTERRUPTING, interrupting,
      INIT_EVQ_V2_IN_FLAG_RPTR_DOS, 0, INIT_EVQ_V2_IN_FLAG_INT_ARMD, 0,
      INIT_EVQ_V2_IN_FLAG_CUT_THRU, ev_cut_through,
      INIT_EVQ_V2_IN_FLAG_RX_MERGE, ev_merge, INIT_EVQ_V2_IN_FLAG_TX_MERGE,
      ev_merge, INIT_EVQ_V2_IN_FLAG_TYPE, evq_type,
  ...
  for (i = 0; i < npages; i++) {
    EFX_POPULATE_QWORD_2(*dma_addr, EFX_DWORD_1, (uint32_t)(addr >> 32),
...
__checkReturn efx_rc_t efx_mcdi_fini_evq(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_mcdi_init_rxq(
  ...
  if ((esmp == NULL) || (EFSYS_MEM_SIZE(esmp) < efx_rxq_size(enp, ndescs))) {
    rc = EINVAL;
    goto fail1;
  }
  ...
  MCDI_IN_POPULATE_DWORD_10(
      req, INIT_RXQ_EXT_IN_FLAGS, INIT_RXQ_EXT_IN_FLAG_BUFF_MODE, 0,
      INIT_RXQ_EXT_IN_FLAG_HDR_SPLIT, 0, INIT_RXQ_EXT_IN_FLAG_TIMESTAMP, 0,
      INIT_RXQ_EXT_IN_CRC_MODE, 0, INIT_RXQ_EXT_IN_FLAG_PREFIX, 1,
  ...
  if (params->es_bufs_per_desc > 0) {
    MCDI_IN_SET_DWORD(req, INIT_RXQ_V3_IN_ES_PACKET_BUFFERS_PER_BUCKET,
                      params->es_bufs_per_desc);
    MCDI_IN_SET_DWORD(req, INIT_RXQ_V3_IN_ES_MAX_DMA_LEN,
                      params->es_max_dma_len);
    MCDI_IN_SET_DWORD(req, INIT_RXQ_V3_IN_ES_PACKET_STRIDE,
                      params->es_buf_stride);
    MCDI_IN_SET_DWORD(req, INIT_RXQ_V3_IN_ES_HEAD_OF_LINE_BLOCK_TIMEOUT,
                      params->hol_block_timeout);
  }

  if (encp->enc_init_rxq_with_buffer_size)
    MCDI_IN_SET_DWORD(req, INIT_RXQ_V4_IN_BUFFER_SIZE_BYTES, params->buf_size);
...
__checkReturn efx_rc_t efx_mcdi_fini_rxq(__in efx_nic_t *enp,
                                         __in uint32_t instance) {
  efx_mcdi_req_t req;
...
__checkReturn efx_rc_t efx_mcdi_init_txq(
  ...
  if ((esmp == NULL) || (EFSYS_MEM_SIZE(esmp) < efx_txq_size(enp, ndescs))) {
    rc = EINVAL;
    goto fail1;
  }
  ...
  MCDI_IN_POPULATE_DWORD_9(
      req, INIT_TXQ_IN_FLAGS, INIT_TXQ_IN_FLAG_BUFF_MODE, 0,
      INIT_TXQ_IN_FLAG_IP_CSUM_DIS, (flags & EFX_TXQ_CKSUM_IPV4) ? 0 : 1,
      INIT_TXQ_IN_FLAG_TCP_CSUM_DIS, (flags & EFX_TXQ_CKSUM_TCPUDP) ? 0 : 1,
      ...
      INIT_TXQ_IN_FLAG_TCP_UDP_ONLY, 0, INIT_TXQ_IN_CRC_MODE, 0,
...
__checkReturn efx_rc_t efx_mcdi_fini_txq(__in efx_nic_t *enp,
                                         __in uint32_t instance) {
  efx_mcdi_req_t req;
...
__checkReturn efx_rc_t
efx_mcdi_get_nic_addr_info(__in efx_nic_t *enp, __out uint32_t *mapping_typep) {
  ...
  *mapping_typep = MCDI_OUT_DWORD(req, GET_DESC_ADDR_INFO_OUT_MAPPING_TYPE);
...
__checkReturn efx_rc_t efx_mcdi_get_nic_addr_regions(
    __in efx_nic_t *enp, __out efx_nic_dma_region_info_t *endrip) {
  ...
  nregions =
      MC_CMD_GET_DESC_ADDR_REGIONS_OUT_REGIONS_NUM(req.emr_out_length_used);

  EFX_STATIC_ASSERT(sizeof(*regions) == DESC_ADDR_REGION_LEN);
  regions = MCDI_OUT2(req, efx_xword_t, GET_DESC_ADDR_REGIONS_OUT_REGIONS);
  ...
  EFSYS_KMEM_ALLOC(enp->en_esip, alloc_size, endrip->endri_regions);
  ...
  for (i = 0; i < nregions; ++i) {
    ...
    region_info->endr_nic_base =
        MCDI_OUT_INDEXED_MEMBER_QWORD(req, GET_DESC_ADDR_REGIONS_OUT_REGIONS, i,
    ...
    region_info->endr_window_log2 =
        MCDI_OUT_INDEXED_MEMBER_DWORD(req, GET_DESC_ADDR_REGIONS_OUT_REGIONS, i,
...
__checkReturn efx_rc_t efx_mcdi_set_nic_addr_regions(
    __in efx_nic_t *enp, __in const efx_nic_dma_region_info_t *endrip) {
  EFX_MCDI_DECLARE_BUF(payload, MC_CMD_SET_DESC_ADDR_REGIONS_IN_LENMAX_MCDI2,
  ...
  req.emr_in_buf = payload;
  req.emr_in_length = MC_CMD_SET_DESC_ADDR_REGIONS_IN_LEN(endrip->endri_count);
  req.emr_out_buf = payload;
  ...
  trgt_addr_base =
      MCDI_OUT2(req, efx_qword_t, SET_DESC_ADDR_REGIONS_IN_TRGT_ADDR_BASE);
  ...
  for (i = 0; i < endrip->endri_count; ++i) {
    ...
    EFX_STATIC_ASSERT(sizeof(1U) * 8 >=
                      MC_CMD_SET_DESC_ADDR_REGIONS_IN_TRGT_ADDR_BASE_MAXNUM);
    MCDI_IN_SET_DWORD(req, SET_DESC_ADDR_REGIONS_IN_SET_REGION_MASK, 1U << i);

    MCDI_IN_SET_INDEXED_QWORD(req, SET_DESC_ADDR_REGIONS_IN_TRGT_ADDR_BASE, i,
                              region_info->endr_trgt_base);
  }
