...
__checkReturn efx_rc_t ef10_evb_init(__in efx_nic_t *enp) {
  EFSYS_ASSERT(EFX_FAMILY_IS_EF100(enp) || EFX_FAMILY_IS_EF10(enp));

  return (0);
}

void ef10_evb_fini(__in efx_nic_t *enp) {
  EFSYS_ASSERT(EFX_FAMILY_IS_EF100(enp) || EFX_FAMILY_IS_EF10(enp));
}

static __checkReturn efx_rc_t
efx_mcdi_vswitch_alloc(__in efx_nic_t *enp, __in efx_vport_id_t vport_id,
...
static __checkReturn efx_rc_t efx_mcdi_vswitch_free(__in efx_nic_t *enp) {
...
static __checkReturn efx_rc_t efx_mcdi_vport_alloc(
    __in efx_nic_t *enp, __in efx_vport_type_t vport_type, __in uint16_t vid,
...
static __checkReturn efx_rc_t
efx_mcdi_vport_free(__in efx_nic_t *enp, __in efx_vport_id_t vport_id) {
...
static __checkReturn efx_rc_t
efx_mcdi_vport_mac_addr_add(__in efx_nic_t *enp, __in efx_vport_id_t vport_id,
  ...
  EFX_MAC_ADDR_COPY(MCDI_IN2(req, uint8_t, VPORT_ADD_MAC_ADDRESS_IN_MACADDR),
                    addrp);
...
static __checkReturn efx_rc_t
efx_mcdi_vport_mac_addr_del(__in efx_nic_t *enp, __in efx_vport_id_t vport_id,
                            __in_bcount(EFX_MAC_ADDR_LEN) uint8_t *addrp) {
  ...
  EFX_MAC_ADDR_COPY(MCDI_IN2(req, uint8_t, VPORT_DEL_MAC_ADDRESS_IN_MACADDR),
                    addrp);
...
static __checkReturn efx_rc_t efx_mcdi_port_assign(__in efx_nic_t *enp,
...
static __checkReturn efx_rc_t efx_mcdi_vport_reconfigure(
    __in efx_nic_t *enp, __in efx_vport_id_t vport_id, __in_opt uint16_t *vidp,
  ...
  if (vidp != NULL) {
    ...
    if (*vidp != EFX_FILTER_VID_UNSPEC) {
      MCDI_IN_SET_DWORD(req, VPORT_RECONFIGURE_IN_NUM_VLAN_TAGS, 1);
      MCDI_IN_POPULATE_DWORD_1(req, VPORT_RECONFIGURE_IN_VLAN_TAGS,
  ...
  if ((addrp != NULL) && (efx_is_zero_eth_addr(addrp) == B_FALSE)) {
    ...
    EFX_MAC_ADDR_COPY(MCDI_IN2(req, uint8_t, VPORT_RECONFIGURE_IN_MACADDRS),
                      addrp);
  }
...
__checkReturn efx_rc_t ef10_evb_vswitch_alloc(
...
__checkReturn efx_rc_t ef10_evb_vswitch_free(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t ef10_evb_vport_alloc(__in efx_nic_t *enp,
                                            __in efx_vswitch_id_t vswitch_id,
  ...
  return (efx_mcdi_vport_alloc(enp, vport_type, vid, vlan_restrict, vport_idp));
}

__checkReturn efx_rc_t ef10_evb_vport_free(__in efx_nic_t *enp,
                                           __in efx_vswitch_id_t vswitch_id,
                                           __in efx_vport_id_t vport_id) {
  _NOTE(ARGUNUSED(vswitch_id))
...
__checkReturn efx_rc_t ef10_evb_vport_mac_addr_add(
...
__checkReturn efx_rc_t ef10_evb_vport_mac_addr_del(
    __in efx_nic_t *enp, __in efx_vswitch_id_t vswitch_id,
    __in efx_vport_id_t vport_id,
    __in_bcount(EFX_MAC_ADDR_LEN) uint8_t *addrp) {
  _NOTE(ARGUNUSED(vswitch_id))
...
__checkReturn efx_rc_t ef10_evb_vadaptor_alloc(__in efx_nic_t *enp,
                                               __in efx_vswitch_id_t vswitch_id,
                                               __in efx_vport_id_t vport_id) {
  _NOTE(ARGUNUSED(vswitch_id))
...
__checkReturn efx_rc_t ef10_evb_vadaptor_free(__in efx_nic_t *enp,
                                              __in efx_vswitch_id_t vswitch_id,
                                              __in efx_vport_id_t vport_id) {
  _NOTE(ARGUNUSED(vswitch_id))
...
__checkReturn efx_rc_t ef10_evb_vport_assign(__in efx_nic_t *enp,
                                             __in efx_vswitch_id_t vswitch_id,
                                             __in efx_vport_id_t vport_id,
                                             __in uint32_t vf_index) {
  _NOTE(ARGUNUSED(vswitch_id))
...
__checkReturn efx_rc_t ef10_evb_vport_reconfigure(
    __in efx_nic_t *enp, __in efx_vswitch_id_t vswitch_id,
  ...
  return (efx_mcdi_vport_reconfigure(enp, vport_id, vidp, addrp, fn_resetp));
}

__checkReturn efx_rc_t ef10_evb_vport_stats(__in efx_nic_t *enp,
                                            __in efx_vswitch_id_t vswitch_id,
                                            __in efx_vport_id_t vport_id,
                                            __in efsys_mem_t *esmp) {
  _NOTE(ARGUNUSED(vswitch_id))

  return (efx_mcdi_mac_stats(enp, vport_id, esmp, EFX_STATS_UPLOAD, 0));
}
