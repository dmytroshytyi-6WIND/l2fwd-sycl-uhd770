...

{
  ...
  MCDI_IN_SET_BYTE(req, VIRTIO_INIT_QUEUE_REQ_QUEUE_TYPE, evvcp->evvc_type);
  MCDI_IN_SET_WORD(req, VIRTIO_INIT_QUEUE_REQ_TARGET_VF, evvcp->evvc_target_vf);
...
__checkReturn efx_rc_t rhead_virtio_qstop(
...
__checkReturn efx_rc_t rhead_virtio_get_doorbell_offset(
  ...
  case MC_CMD_VIRTIO_GET_FEATURES_IN_NET:
    ...
    if (evvp->evv_type == EFX_VIRTIO_VQ_TYPE_NET_RXQ) {
      *offsetp = MCDI_OUT_DWORD(
          req, VIRTIO_GET_NET_DOORBELL_OFFSET_RESP_RX_DBL_OFFSET);
    } else if (evvp->evv_type == EFX_VIRTIO_VQ_TYPE_NET_TXQ) {
      *offsetp = MCDI_OUT_DWORD(
          req, VIRTIO_GET_NET_DOORBELL_OFFSET_RESP_TX_DBL_OFFSET);
    }
    break;
  case MC_CMD_VIRTIO_GET_FEATURES_IN_BLOCK:
    ...
    *offsetp =
        MCDI_OUT_DWORD(req, VIRTIO_GET_BLOCK_DOORBELL_OFFSET_RESP_DBL_OFFSET);
    break;
  default:
    EFSYS_ASSERT(0);
    rc = EINVAL;
...
__checkReturn efx_rc_t rhead_virtio_get_features(
...
__checkReturn efx_rc_t rhead_virtio_verify_features(
    __in efx_nic_t *enp, __in efx_virtio_device_type_t type,
