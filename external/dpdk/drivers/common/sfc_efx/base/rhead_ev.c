...
static boolean_t rhead_ev_dispatch(__in efx_evq_t *eep,
                                   __in efx_qword_t *eventp,
                                   __in const efx_ev_callbacks_t *eecp,
                                   __in_opt void *arg);

static __checkReturn boolean_t
rhead_ev_rx_packets(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                    __in const efx_ev_callbacks_t *eecp, __in_opt void *arg);

static __checkReturn boolean_t
rhead_ev_tx_completion(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                       __in const efx_ev_callbacks_t *eecp, __in_opt void *arg);

static __checkReturn boolean_t
rhead_ev_mcdi(__in efx_evq_t *eep, __in efx_qword_t *eqp,
              __in const efx_ev_callbacks_t *eecp, __in_opt void *arg);

#if EFSYS_OPT_EV_EXTENDED_WIDTH
static boolean_t rhead_ev_ew_dispatch(__in efx_evq_t *eep,
                                      __in efx_xword_t *eventp,
                                      __in const efx_ev_callbacks_t *eecp,
                                      __in_opt void *arg);

static void rhead_ev_ew_qpoll(__in efx_evq_t *eep, __inout unsigned int *countp,
                              __in const efx_ev_callbacks_t *eecp,
                              __in_opt void *arg);

#if EFSYS_OPT_DESC_PROXY
static boolean_t rhead_ev_ew_txq_desc(__in efx_evq_t *eep,
                                      __in efx_xword_t *eventp,
                                      __in const efx_ev_callbacks_t *eecp,
                                      __in_opt void *arg);

static boolean_t rhead_ev_ew_virtq_desc(__in efx_evq_t *eep,
                                        __in efx_xword_t *eventp,
                                        __in const efx_ev_callbacks_t *eecp,
                                        __in_opt void *arg);
...
__checkReturn efx_rc_t rhead_ev_init(__in efx_nic_t *enp) {
  _NOTE(ARGUNUSED(enp))

  return (0);
}

void rhead_ev_fini(__in efx_nic_t *enp){_NOTE(ARGUNUSED(enp))}

__checkReturn efx_rc_t
    rhead_ev_qcreate(__in efx_nic_t *enp, __in unsigned int index,
  ...
  if ((flags & EFX_EVQ_FLAGS_NOTIFY_MASK) == EFX_EVQ_FLAGS_NOTIFY_INTERRUPT) {
   ...
   */
  rc = efx_mcdi_init_evq(enp, index, esmp, ndescs, irq, target_evq, us, flags,
                         B_FALSE);
  if (rc != 0)
    goto fail2;
...
void rhead_ev_qdestroy(__in efx_evq_t *eep) {
...
__checkReturn efx_rc_t rhead_ev_qprime(__in efx_evq_t *eep,
  ...
  EFX_POPULATE_DWORD_2(dword, ERF_GZ_EVQ_ID, eep->ee_index, ERF_GZ_IDX, rptr);
...
void rhead_ev_qpost(__in efx_evq_t *eep, __in uint16_t data) {
...
static __checkReturn boolean_t
rhead_ev_dispatch(__in efx_evq_t *eep, __in efx_qword_t *eventp,
                  __in const efx_ev_callbacks_t *eecp, __in_opt void *arg) {
  ...
  default:
    EFSYS_PROBE3(bad_event, unsigned int, eep->ee_index, uint32_t,
                 EFX_QWORD_FIELD(*eventp, EFX_DWORD_1), uint32_t,
                 EFX_QWORD_FIELD(*eventp, EFX_DWORD_0));
...
void rhead_ev_qpoll(__in efx_evq_t *eep, __inout unsigned int *countp,
                    __in const efx_ev_callbacks_t *eecp, __in_opt void *arg) {
  ...
  do {
    ...
    for (index = 0; index < total; ++index) {
      ...
      should_abort = rhead_ev_dispatch(eep, &(ev[index]), eecp, arg);
...
static boolean_t rhead_ev_ew_dispatch(__in efx_evq_t *eep,
  ...
  case ESE_GZ_EF100_EVEW_64BIT:
    ...
    should_abort = rhead_ev_dispatch(eep, &eventp->ex_qword[0], eecp, arg);
    break;
  ...
  default:
    ...
    EFSYS_PROBE7(ew_bad_event, unsigned int, eep->ee_index, uint32_t,
                 EFX_XWORD_FIELD(*eventp, EFX_DWORD_7), uint32_t,
                 EFX_XWORD_FIELD(*eventp, EFX_DWORD_4), uint32_t,
                 EFX_XWORD_FIELD(*eventp, EFX_DWORD_3), uint32_t,
                 EFX_XWORD_FIELD(*eventp, EFX_DWORD_2), uint32_t,
                 EFX_XWORD_FIELD(*eventp, EFX_DWORD_1), uint32_t,
                 EFX_XWORD_FIELD(*eventp, EFX_DWORD_0));
...
static void rhead_ev_ew_qpoll(__in efx_evq_t *eep, __inout unsigned int *countp,
  ...
  do {
    ...
    for (total = 0; total < batch; ++total) {
      ...
      EFSYS_PROBE7(ew_event, unsigned int, eep->ee_index, uint32_t,
                   EFX_XWORD_FIELD(ev[total], EFX_DWORD_7), uint32_t,
                   EFX_XWORD_FIELD(ev[total], EFX_DWORD_4), uint32_t,
                   EFX_XWORD_FIELD(ev[total], EFX_DWORD_3), uint32_t,
                   EFX_XWORD_FIELD(ev[total], EFX_DWORD_2), uint32_t,
                   EFX_XWORD_FIELD(ev[total], EFX_DWORD_1), uint32_t,
                   EFX_XWORD_FIELD(ev[total], EFX_DWORD_0));
    ...
    for (index = 0; index < total; ++index) {
      ...
      should_abort = rhead_ev_ew_dispatch(eep, &(ev[index]), eecp, arg);
...
__checkReturn efx_rc_t rhead_ev_qmoderate(__in efx_evq_t *eep,
...
#if EFSYS_OPT_QSTATS
void rhead_ev_qstats_update(__in efx_evq_t *eep,
...
rhead_ev_rx_packets(__in efx_evq_t *eep, __in efx_qword_t *eqp,
  ...
  should_abort =
      eecp->eec_rx_packets(arg, label, num_packets, EFX_PKT_PREFIX_LEN);

  return (should_abort);
}

static __checkReturn boolean_t rhead_ev_tx_completion(
    __in efx_evq_t *eep, __in efx_qword_t *eqp,
    __in const efx_ev_callbacks_t *eecp, __in_opt void *arg) {
  efx_nic_t *enp = eep->ee_enp;
  uint32_t num_descs;
  uint32_t label;
...
static boolean_t rhead_ev_ew_virtq_desc(__in efx_evq_t *eep,
  ...
  should_abort = eecp->eec_desc_proxy_virtq_desc(arg, vi_id, avail, virtq_desc);
