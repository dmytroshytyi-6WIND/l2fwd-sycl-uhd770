...
__checkReturn efx_rc_t efx_proxy_auth_init(__in efx_nic_t *enp) {
...
void efx_proxy_auth_fini(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t efx_proxy_auth_configure(
  ...
  if ((configp == NULL) || (configp->request_bufferp == NULL) ||
      (configp->response_bufferp == NULL) ||
      (configp->status_bufferp == NULL) || (configp->op_listp == NULL) ||
      (configp->block_cnt == 0)) {
    rc = EINVAL;
    goto fail1;
  }

  if ((epop->epo_mc_config == NULL) || (epop->epo_privilege_modify == NULL)) {
    rc = ENOTSUP;
    goto fail2;
  }
  ...
  rc = epop->epo_privilege_modify(enp, MC_CMD_PRIVILEGE_MODIFY_IN_ALL, 0, 0, 0,
                                  configp->handled_privileges);
  if (rc != 0)
    goto fail4;
...
efx_proxy_auth_destroy(__in efx_nic_t *enp, __in uint32_t handled_privileges) {
  ...
  if ((epop->epo_disable == NULL) || (epop->epo_privilege_modify == NULL)) {
    rc = ENOTSUP;
    goto fail1;
  }

  rc = epop->epo_privilege_modify(enp, MC_CMD_PRIVILEGE_MODIFY_IN_ALL, 0, 0,
                                  handled_privileges, 0);
  if (rc != 0)
    goto fail2;
...
__checkReturn efx_rc_t efx_proxy_auth_complete_request(
...
__checkReturn efx_rc_t efx_proxy_auth_exec_cmd(
...
__checkReturn efx_rc_t efx_proxy_auth_set_privilege_mask(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_proxy_auth_privilege_mask_get(
...
__checkReturn efx_rc_t efx_proxy_auth_privilege_modify(
    __in efx_nic_t *enp, __in uint32_t pf_index, __in uint32_t vf_index,
  ...
  rc = epop->epo_privilege_modify(enp, MC_CMD_PRIVILEGE_MODIFY_IN_ONE, pf_index,
                                  vf_index, add_privileges_mask,
