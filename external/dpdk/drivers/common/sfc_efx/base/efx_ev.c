...
static __checkReturn efx_rc_t siena_ev_qcreate(
...
static void siena_ef10_ev_qpoll(__in efx_evq_t *eep,
...
__checkReturn efx_rc_t efx_ev_init(__in efx_nic_t *enp) {
...
__checkReturn size_t efx_evq_size(__in const efx_nic_t *enp,
...
__checkReturn unsigned int efx_evq_nbufs(__in const efx_nic_t *enp,
                                         __in unsigned int ndescs,
                                         __in uint32_t flags) {
  size_t size;
...
__checkReturn efx_rc_t efx_ev_qcreate_irq(
    __in efx_nic_t *enp, __in unsigned int index, __in efsys_mem_t *esmp,
    __in size_t ndescs, __in uint32_t id, __in uint32_t us, __in uint32_t flags,
  ...
  if (!ISP2(ndescs) || ndescs < encp->enc_evq_min_nevs ||
  ...
  *eepp = eep;
  ...
  if ((rc = eevop->eevo_qcreate(enp, index, esmp, ndescs, id, us, flags, irq,
                                eep)) != 0)
    goto fail9;
...
__checkReturn efx_rc_t efx_ev_qcreate(__in efx_nic_t *enp,
                                      __in unsigned int index,
                                      __in efsys_mem_t *esmp,
                                      __in size_t ndescs, __in uint32_t id,
                                      __in uint32_t us, __in uint32_t flags,
                                      __deref_out efx_evq_t **eepp) {
  uint32_t irq = index;

  return (
      efx_ev_qcreate_irq(enp, index, esmp, ndescs, id, us, flags, irq, eepp));
}

void efx_ev_qdestroy(__in efx_evq_t *eep) {
...
__checkReturn efx_rc_t efx_ev_qprime(__in efx_evq_t *eep,
                                     __in unsigned int count) {
  efx_nic_t *enp = eep->ee_enp;
  const efx_ev_ops_t *eevop = enp->en_eevop;
  efx_rc_t rc;
...
__checkReturn boolean_t efx_ev_qpending(__in efx_evq_t *eep,
                                        __in unsigned int count) {
  size_t offset;
  efx_qword_t qword;
...
void efx_ev_qprefetch(__in efx_evq_t *eep, __in unsigned int count) {
  unsigned int offset;
 ...
 */
void efx_ev_qcreate_check_init_done(__in efx_evq_t *eep,
                                    __in const efx_ev_callbacks_t *eecp,
                                    __in_opt void *arg) {
...
void efx_ev_qpoll(__in efx_evq_t *eep, __inout unsigned int *countp,
  ...
  EFSYS_ASSERT(eevop != NULL && eevop->eevo_qpoll != NULL);

  eevop->eevo_qpoll(eep, countp, eecp, arg);
}

void efx_ev_qpost(__in efx_evq_t *eep, __in uint16_t data) {
  efx_nic_t *enp = eep->ee_enp;
  const efx_ev_ops_t *eevop = enp->en_eevop;
...
__checkReturn efx_rc_t efx_ev_usecs_to_ticks(__in efx_nic_t *enp,
                                             __in unsigned int us,
...
__checkReturn efx_rc_t efx_ev_qmoderate(__in efx_evq_t *eep,
                                        __in unsigned int us) {
  efx_nic_t *enp = eep->ee_enp;
  const efx_ev_ops_t *eevop = enp->en_eevop;
  efx_rc_t rc;
...
#if EFSYS_OPT_QSTATS
void efx_ev_qstats_update(__in efx_evq_t *eep,
...
static __checkReturn boolean_t siena_ev_rx_not_ok(__in efx_evq_t *eep,
...
static __checkReturn boolean_t siena_ev_rx(__in efx_evq_t *eep,
                                           __in efx_qword_t *eqp,
                                           __in const efx_ev_callbacks_t *eecp,
                                           __in_opt void *arg) {
  uint32_t id;
...
static __checkReturn boolean_t siena_ev_tx(__in efx_evq_t *eep,
                                           __in efx_qword_t *eqp,
                                           __in const efx_ev_callbacks_t *eecp,
                                           __in_opt void *arg) {
  uint32_t id;
...
siena_ev_driver(__in efx_evq_t *eep, __in efx_qword_t *eqp,
  ...
  case FSE_AZ_RX_DSC_ERROR_EV:
    ...
    should_abort = eecp->eec_exception(arg, EFX_EXCEPTION_RX_DSC_ERROR, 0);
  ...
  case FSE_AZ_TX_DSC_ERROR_EV:
    ...
    EFSYS_ASSERT(eecp->eec_exception != NULL);
    should_abort = eecp->eec_exception(arg, EFX_EXCEPTION_TX_DSC_ERROR, 0);
...
siena_ev_mcdi(__in efx_evq_t *eep, __in efx_qword_t *eqp,
  ...
  case MCDI_EVENT_CODE_SENSOREVT: {
    ...
    if ((rc = mcdi_mon_ev(enp, eqp, &id, &value)) == 0)
      ...
      should_abort = eecp->eec_exception(arg, EFX_EXCEPTION_UNKNOWN_SENSOREVT,
  ...
  case MCDI_EVENT_CODE_MAC_STATS_DMA:
    ...
    if (eecp->eec_mac_stats != NULL) {
      eecp->eec_mac_stats(arg, MCDI_EV_FIELD(eqp, MAC_STATS_DMA_GENERATION));
    }
#endif
    break;
  ...
  case MCDI_EVENT_CODE_FWALERT: {
    ...
    if (reason == MCDI_EVENT_FWALERT_REASON_SRAM_ACCESS)
      ...
      should_abort = eecp->eec_exception(arg, EFX_EXCEPTION_UNKNOWN_FWALERT,
                                         MCDI_EV_FIELD(eqp, DATA));
    break;
  }
...
static __checkReturn efx_rc_t siena_ev_qprime(__in efx_evq_t *eep,
  ...
  EFX_BAR_TBL_WRITED(enp, FR_AZ_EVQ_RPTR_REG, eep->ee_index, &dword, B_FALSE);
...
static void siena_ev_qpost(__in efx_evq_t *eep, __in uint16_t data) {
  ...
  EFX_POPULATE_OWORD_3(oword, FRF_AZ_DRV_EV_QID, eep->ee_index, EFX_DWORD_0,
                       EFX_QWORD_FIELD(ev, EFX_DWORD_0), EFX_DWORD_1,
...
static __checkReturn efx_rc_t siena_ev_qmoderate(__in efx_evq_t *eep,
  ...
  if (us == 0) {
    EFX_POPULATE_DWORD_2(dword, FRF_CZ_TC_TIMER_MODE, FFE_CZ_TIMER_MODE_DIS,
  ...
  EFX_BAR_TBL_WRITED(enp, FR_BZ_TIMER_COMMAND_REGP0, eep->ee_index, &dword,
                     locked);
...
static __checkReturn efx_rc_t siena_ev_qcreate(
  ...
  if (enp->en_intr.ei_type == EFX_INTR_LINE && index >= EFX_MAXRSS_LEGACY) {
    rc = EINVAL;
    goto fail2;
  }
#endif
  for (size = 0;
       (1U << size) <= encp->enc_evq_max_nevs / encp->enc_evq_min_nevs; size++)
  ...
  EFX_POPULATE_OWORD_3(oword, FRF_CZ_TIMER_Q_EN, 1, FRF_CZ_HOST_NOTIFY_MODE,
                       notify_mode, FRF_CZ_TIMER_MODE, FFE_CZ_TIMER_MODE_DIS);
...
const char *efx_ev_qstat_name(__in efx_nic_t *enp, __in unsigned int id) {
  _NOTE(ARGUNUSED(enp))
...
static void siena_ev_qdestroy(__in efx_evq_t *eep) {
  ...
  EFX_BAR_TBL_WRITEO(enp, FR_AZ_EVQ_PTR_TBL, eep->ee_index, &oword, B_TRUE);

  EFX_ZERO_OWORD(oword);
  EFX_BAR_TBL_WRITEO(enp, FR_AZ_TIMER_TBL, eep->ee_index, &oword, B_TRUE);
}

static void siena_ev_fini(__in efx_nic_t *enp) { _NOTE(ARGUNUSED(enp)) }
...
static void siena_ef10_ev_qpoll(__in efx_evq_t *eep,
  ...
  EFX_STATIC_ASSERT(ESE_DZ_EV_CODE_DRV_GEN_EV == FSE_AZ_EV_CODE_DRV_GEN_EV);
#if EFSYS_OPT_MCDI
  EFX_STATIC_ASSERT(ESE_DZ_EV_CODE_MCDI_EV == FSE_AZ_EV_CODE_MCDI_EVRESPONSE);
#endif
  ...
  do {
    ...
    for (total = 0; total < batch; ++total) {
      ...
      EFSYS_PROBE3(event, unsigned int, eep->ee_index, uint32_t,
                   EFX_QWORD_FIELD(ev[total], EFX_DWORD_1), uint32_t,
                   EFX_QWORD_FIELD(ev[total], EFX_DWORD_0));
    ...
    for (index = 0; index < total; ++index) {
      ...
      if (total == batch && index + EFSYS_OPT_EV_PREFETCH_PERIOD == total) {
      ...
      case FSE_AZ_EV_CODE_RX_EV:
        should_abort = eep->ee_rx(eep, &(ev[index]), eecp, arg);
        break;
      case FSE_AZ_EV_CODE_TX_EV:
        should_abort = eep->ee_tx(eep, &(ev[index]), eecp, arg);
        break;
      case FSE_AZ_EV_CODE_DRIVER_EV:
        should_abort = eep->ee_driver(eep, &(ev[index]), eecp, arg);
        break;
      case FSE_AZ_EV_CODE_DRV_GEN_EV:
        should_abort = eep->ee_drv_gen(eep, &(ev[index]), eecp, arg);
        break;
#if EFSYS_OPT_MCDI
      case FSE_AZ_EV_CODE_MCDI_EVRESPONSE:
        should_abort = eep->ee_mcdi(eep, &(ev[index]), eecp, arg);
        break;
#endif
      case FSE_AZ_EV_CODE_GLOBAL_EV:
        if (eep->ee_global) {
          should_abort = eep->ee_global(eep, &(ev[index]), eecp, arg);
          break;
        }
        /* else fallthrough */
      default:
        EFSYS_PROBE3(bad_event, unsigned int, eep->ee_index, uint32_t,
                     EFX_QWORD_FIELD(ev[index], EFX_DWORD_1), uint32_t,
