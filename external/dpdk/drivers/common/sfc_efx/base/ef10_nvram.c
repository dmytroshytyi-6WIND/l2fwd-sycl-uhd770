...
static __checkReturn efx_rc_t tlv_validate_state(__inout tlv_cursor_t *cursor);

static void tlv_init_block(__out uint32_t *block) {
  *block = __CPU_TO_LE_32(TLV_TAG_END);
}

static uint32_t tlv_tag(__in tlv_cursor_t *cursor) {
  uint32_t dword, tag;
...
static size_t tlv_length(__in tlv_cursor_t *cursor) {
...
static uint8_t *tlv_value(__in tlv_cursor_t *cursor) {
  if (tlv_tag(cursor) == TLV_TAG_END)
    return (NULL);
...
static uint8_t *tlv_item(__in tlv_cursor_t *cursor) {
  if (tlv_tag(cursor) == TLV_TAG_END)
    return (NULL);
...
static uint32_t *tlv_next_item_ptr(__in tlv_cursor_t *cursor) {
  uint32_t length;
...
static __checkReturn efx_rc_t tlv_advance(__inout tlv_cursor_t *cursor) {
  efx_rc_t rc;
...
static efx_rc_t tlv_rewind(__in tlv_cursor_t *cursor) {
  efx_rc_t rc;
...
static efx_rc_t tlv_find(__inout tlv_cursor_t *cursor, __in uint32_t tag) {
  efx_rc_t rc;
...
static efx_rc_t tlv_init_cursor(__out tlv_cursor_t *cursor,
                                __in uint32_t *block, __in uint32_t *limit,
...
tlv_init_cursor_from_size(__out tlv_cursor_t *cursor,
                          __in_bcount(size) uint8_t *block, __in size_t size) {
  uint32_t *limit;
  limit = (uint32_t *)(block + size - sizeof(uint32_t));
  return (tlv_init_cursor(cursor, (uint32_t *)block, limit, (uint32_t *)block));
}

static __checkReturn efx_rc_t tlv_init_cursor_at_offset(
    __out tlv_cursor_t *cursor, __in_bcount(size) uint8_t *block,
    __in size_t size, __in size_t offset) {
  uint32_t *limit;
  uint32_t *current;
  limit = (uint32_t *)(block + size - sizeof(uint32_t));
...
static __checkReturn efx_rc_t tlv_require_end(__inout tlv_cursor_t *cursor) {
  uint32_t *pos;
  efx_rc_t rc;
...
static size_t tlv_block_length_used(__inout tlv_cursor_t *cursor) {
  efx_rc_t rc;
...
static uint32_t *tlv_last_segment_end(__in tlv_cursor_t *cursor) {
   ...
   */
  for (;;) {
    if (tlv_init_cursor(&segment_cursor, segment_start, cursor->limit,
                        segment_start) != 0)
      break;
...
static uint32_t *tlv_write(__in tlv_cursor_t *cursor, __in uint32_t tag,
                           __in_bcount(size) uint8_t *data, __in size_t size) {
  uint32_t len = size;
  uint32_t *ptr;
  ...
  *ptr++ = __CPU_TO_LE_32(len);
  ...
  if (len > 0) {
    ...
    memcpy(ptr, data, len);
    ptr += EFX_P2ROUNDUP(uint32_t, len, sizeof(uint32_t)) / sizeof(*ptr);
  }

  return (ptr);
}

static __checkReturn efx_rc_t tlv_insert(__inout tlv_cursor_t *cursor,
                                         __in uint32_t tag,
...
static __checkReturn efx_rc_t tlv_delete(__inout tlv_cursor_t *cursor) {
  unsigned int delta;
  uint32_t *last_segment_end;
  efx_rc_t rc;
...
static __checkReturn efx_rc_t tlv_modify(__inout tlv_cursor_t *cursor,
                                         __in uint32_t tag,
                                         __in_bcount(size) uint8_t *data,
                                         __in size_t size) {
  uint32_t *pos;
  ...
  if (new_ndwords > old_ndwords) {
    ...
    memmove(pos + delta, pos, (last_segment_end + 1 - pos) * sizeof(uint32_t));
    ...
    memmove(pos, pos + delta, (last_segment_end + 1 - pos) * sizeof(uint32_t));
...
static uint32_t checksum_tlv_partition(__in nvram_partition_t *partition) {
  tlv_cursor_t *cursor;
  uint32_t *ptr;
  uint32_t *end;
  uint32_t csum;
...
static __checkReturn efx_rc_t
tlv_update_partition_len_and_cks(__in tlv_cursor_t *cursor) {
  efx_rc_t rc;
  ...
  header->generation = __CPU_TO_LE_32(__LE_TO_CPU_32(header->generation) + 1);

  trailer =
      (struct tlv_partition_trailer *)((uint8_t *)header + new_len -
                                       sizeof(*trailer) - sizeof(uint32_t));
  trailer->generation = header->generation;
  trailer->checksum = __CPU_TO_LE_32(__LE_TO_CPU_32(trailer->checksum) -
...
__checkReturn efx_rc_t ef10_nvram_buffer_validate(__in uint32_t partn,
...
void ef10_nvram_buffer_init(__out_bcount(buffer_size) caddr_t bufferp,
...
__checkReturn efx_rc_t ef10_nvram_buffer_create(__in uint32_t partn_type,
  ...
  if ((rc = tlv_init_cursor(&cursor, buf,
                            (uint32_t *)((uint8_t *)buf + partn_size), buf)) !=
      0) {
    goto fail2;
  }
  ...
  if ((rc = tlv_insert(&cursor, TLV_TAG_PARTITION_HEADER,
  ...
  if ((rc = tlv_insert(&cursor, TLV_TAG_PARTITION_TRAILER,
                       (uint8_t *)&trailer.generation, sizeof(trailer) - 8)) !=
      0)
    goto fail5;
  ...
  if ((rc = ef10_nvram_buffer_validate(partn_type, partn_data, partn_size)) !=
      0)
    goto fail7;
...
static uint32_t byte_offset(__in uint32_t *position, __in uint32_t *base) {
  return (uint32_t)((uint8_t *)position - (uint8_t *)base);
}

__checkReturn efx_rc_t ef10_nvram_buffer_find_item_start(
...
__checkReturn efx_rc_t ef10_nvram_buffer_find_end(__in_bcount(buffer_size)
                                                      caddr_t bufferp,
                                                  __in size_t buffer_size,
...
__checkReturn __success(return != B_FALSE) boolean_t
  ...
  if (tlv_init_cursor_at_offset(&cursor, (uint8_t *)bufferp, buffer_size,
                                offset) != 0) {
    return (B_FALSE);
  }
  ...
  while ((key = tlv_item(&cursor)) != NULL) {
    tag = tlv_tag(&cursor);
    if (tag == TLV_TAG_PARTITION_HEADER || tag == TLV_TAG_PARTITION_TRAILER) {
    ...
    *startp = byte_offset(cursor.current, cursor.block);
    *lengthp = byte_offset(tlv_next_item_ptr(&cursor), cursor.current);
    return (B_TRUE);
  }
...
__checkReturn efx_rc_t ef10_nvram_buffer_peek_item(
    __in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
  ...
  if ((rc = tlv_init_cursor_at_offset(&cursor, (uint8_t *)bufferp, buffer_size,
                                      offset)) != 0) {
    goto fail1;
  }
  ...
  *tagp = tag;
  if (tag == TLV_TAG_END) {
    ...
    *lengthp = byte_offset(tlv_next_item_ptr(&cursor), cursor.current);
    *value_offsetp =
        byte_offset((uint32_t *)tlv_value(&cursor), cursor.current);
  }
  return (0);
...
__checkReturn efx_rc_t ef10_nvram_buffer_get_item(
    __in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
...
__checkReturn efx_rc_t ef10_nvram_buffer_insert_item(
    __in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
...
__checkReturn efx_rc_t ef10_nvram_buffer_modify_item(
    __in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
    __in uint32_t offset, __in uint32_t tag, __in_bcount(length) caddr_t valuep,
    __in uint32_t length, __out uint32_t *lengthp) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_nvram_buffer_delete_item(
    __in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
...
__checkReturn efx_rc_t ef10_nvram_buffer_finish(__in_bcount(buffer_size)
                                                    caddr_t bufferp,
                                                __in size_t buffer_size) {
  efx_rc_t rc;
  tlv_cursor_t cursor;
 ...
 */
static __checkReturn efx_rc_t ef10_nvram_read_tlv_segment(
    __in efx_nic_t *enp, __in uint32_t partn, __in size_t seg_offset,
    __in_bcount(max_seg_size) caddr_t seg_data, __in size_t max_seg_size) {
  tlv_cursor_t cursor;
  struct tlv_partition_header *header;
  struct tlv_partition_trailer *trailer;
  size_t total_length;
  ...
  if ((rc = ef10_nvram_partn_read_mode(
           enp, partn, seg_offset, seg_data, EF10_NVRAM_CHUNK,
  ...
  if (total_length > EF10_NVRAM_CHUNK) {
    if ((rc = ef10_nvram_partn_read_mode(
             enp, partn, seg_offset + EF10_NVRAM_CHUNK,
             seg_data + EF10_NVRAM_CHUNK, total_length - EF10_NVRAM_CHUNK,
             MC_CMD_NVRAM_READ_IN_V2_TARGET_CURRENT)) != 0)
      goto fail6;
  }
 ...
 */
__checkReturn efx_rc_t ef10_nvram_buf_read_tlv(
...
__checkReturn efx_rc_t ef10_nvram_partn_read_tlv(
  ...
  retry = 10;
  do {
    if ((rc = ef10_nvram_read_tlv_segment(enp, partn, 0, seg_data,
                                          partn_size)) != 0)
      --retry;
  ...
  if ((rc = ef10_nvram_buf_read_tlv(enp, seg_data, partn_size, tag, &data,
                                    &length)) != 0)
    goto fail5;
...
static __checkReturn efx_rc_t ef10_nvram_buf_segment_size(
    __in caddr_t seg_data, __in size_t max_seg_size, __out size_t *seg_sizep) {
  efx_rc_t rc;
  tlv_cursor_t cursor;
  struct tlv_partition_header *header;
  uint32_t cksum;
 ...
 */
__checkReturn efx_rc_t ef10_nvram_buf_write_tlv(
    __inout_bcount(max_seg_size) caddr_t seg_data, __in size_t max_seg_size,
  ...
  if ((rc = tlv_find(&cursor, tag)) == 0) {
    ...
    if ((rc = tlv_modify(&cursor, tag, (uint8_t *)tag_data, tag_size)) != 0)
      goto fail3;
  } else {
    ...
    if ((rc = tlv_insert(&cursor, tag, (uint8_t *)tag_data, tag_size)) != 0) {
      rc = EINVAL;
      goto fail5;
    }
  }
 ...
 * configuration.
 */
__checkReturn efx_rc_t ef10_nvram_partn_write_tlv(
    __in efx_nic_t *enp, __in uint32_t partn, __in uint32_t tag,
    __in_bcount(size) caddr_t data, __in size_t size) {
  return ef10_nvram_partn_write_segment_tlv(enp, partn, tag, data, size,
                                            B_FALSE);
}
 ...
 */
static __checkReturn efx_rc_t ef10_nvram_segment_write_tlv(
    __in efx_nic_t *enp, __in uint32_t partn, __in uint32_t tag,
    __in_bcount(size) caddr_t data, __in size_t size,
    __inout caddr_t *seg_datap, __inout size_t *partn_offsetp,
    __inout size_t *src_remain_lenp, __inout size_t *dest_remain_lenp,
   ...
   */
  status = ef10_nvram_read_tlv_segment(enp, partn, *partn_offsetp, *seg_datap,
                                       *src_remain_lenp);
  if (status != 0) {
    rc = EINVAL;
    goto fail1;
  ...
  status = ef10_nvram_buf_segment_size(*seg_datap, *src_remain_lenp,
  ...
  if (write) {
    ...
    if ((rc = ef10_nvram_buf_write_tlv(*seg_datap, *dest_remain_lenp, tag, data,
                                       size, &modified_segment_size)) != 0) {
      goto fail3;
    }
 ...
 * invalidate them.
 */
__checkReturn efx_rc_t ef10_nvram_partn_write_segment_tlv(
    __in efx_nic_t *enp, __in uint32_t partn, __in uint32_t tag,
    __in_bcount(size) caddr_t data, __in size_t size,
  ...
  do {
    ...
    rc = ef10_nvram_segment_write_tlv(
        enp, partn, tag, data, size, &segment_data, &current_offset,
        &remaining_original_length, &remaining_modified_length, write);
    if (rc != 0) {
      if (current_offset == 0) {
        /*
  ...
  if ((rc = ef10_nvram_partn_write(enp, partn, 0, partn_data, total_length)) !=
      0)
    goto fail7;
 ...
 */
__checkReturn efx_rc_t ef10_nvram_partn_size(__in efx_nic_t *enp,
                                             __in uint32_t partn,
                                             __out size_t *sizep) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_nvram_partn_info(__in efx_nic_t *enp,
                                             __in uint32_t partn,
...
__checkReturn efx_rc_t ef10_nvram_partn_lock(__in efx_nic_t *enp,
                                             __in uint32_t partn) {
  efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_nvram_partn_read_mode(
  ...
  while (size > 0) {
    ...
    if ((rc = efx_mcdi_nvram_read(enp, partn, offset, data, chunk, mode)) !=
        0) {
      goto fail1;
    }
...
__checkReturn efx_rc_t ef10_nvram_partn_read(__in efx_nic_t *enp,
                                             __in uint32_t partn,
...
__checkReturn efx_rc_t ef10_nvram_partn_read_backup(
    __in efx_nic_t *enp, __in uint32_t partn, __in unsigned int offset,
...
__checkReturn efx_rc_t ef10_nvram_partn_erase(__in efx_nic_t *enp,
                                              __in uint32_t partn,
                                              __in unsigned int offset,
                                              __in size_t size) {
  efx_rc_t rc;
  ...
  if (erase_size == 0) {
    ...
    while (size > 0) {
      if ((rc = efx_mcdi_nvram_erase(enp, partn, offset, erase_size)) != 0)
        goto fail4;
      offset += erase_size;
      size -= erase_size;
    }
...
__checkReturn efx_rc_t ef10_nvram_partn_write(__in efx_nic_t *enp,
                                              __in uint32_t partn,
                                              __in unsigned int offset,
  ...
  while (size > 0) {
    ...
    if ((rc = efx_mcdi_nvram_write(enp, partn, offset, data, chunk)) != 0) {
      goto fail3;
    }
...
ef10_nvram_partn_unlock(__in efx_nic_t *enp, __in uint32_t partn,
  ...
  rc = efx_mcdi_nvram_update_finish(
      enp, partn, reboot, EFX_NVRAM_UPDATE_FLAGS_BACKGROUND, &verify_result);
  ...
  while (verify_result == MC_CMD_NVRAM_VERIFY_RC_PENDING) {
    ...
    rc = efx_mcdi_nvram_update_finish(
        enp, partn, reboot, EFX_NVRAM_UPDATE_FLAGS_POLL, &verify_result);
    if (rc != 0) {
...
__checkReturn efx_rc_t ef10_nvram_partn_set_version(__in efx_nic_t *enp,
                                                    __in uint32_t partn,
                                                    __in_ecount(4)
                                                        uint16_t version[4]) {
  ...
  if ((rc = ef10_nvram_partn_write_segment_tlv(
           enp, NVRAM_PARTITION_TYPE_DYNAMIC_CONFIG,
           TLV_TAG_PARTITION_VERSION(partn), (caddr_t)&partn_version.version_w,
           size, B_TRUE)) != 0)
    goto fail1;
...
static __checkReturn efx_rc_t
ef10_parttbl_get(__in efx_nic_t *enp, __out ef10_parttbl_entry_t **parttblp,
...
__checkReturn efx_rc_t ef10_nvram_type_to_partn(__in efx_nic_t *enp,
...
static __checkReturn efx_rc_t ef10_nvram_partn_to_type(
    __in efx_nic_t *enp, __in uint32_t partn, __out efx_nvram_type_t *typep) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
  ef10_parttbl_entry_t *parttbl = NULL;
  size_t parttbl_rows = 0;
  unsigned int i;
...
__checkReturn efx_rc_t ef10_nvram_test(__in efx_nic_t *enp) {
  ...
  if ((rc = efx_mcdi_nvram_partitions(enp, (caddr_t)partns, size, &npartns)) !=
      0) {
    goto fail2;
  }
...
__checkReturn efx_rc_t ef10_nvram_partn_get_version(__in efx_nic_t *enp,
                                                    __in uint32_t partn,
  ...
  if ((rc = efx_mcdi_nvram_metadata(enp, partn, subtypep, version, NULL, 0)) !=
      0)
    goto fail1;
...
__checkReturn efx_rc_t ef10_nvram_partn_rw_start(__in efx_nic_t *enp,
                                                 __in uint32_t partn,
