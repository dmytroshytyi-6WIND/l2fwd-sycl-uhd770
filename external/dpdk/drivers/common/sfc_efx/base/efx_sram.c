...
__checkReturn efx_rc_t efx_sram_buf_tbl_set(__in efx_nic_t *enp,
                                            __in uint32_t id,
  ...
  EFX_POPULATE_OWORD_2(oword, FRF_AZ_BUF_UPD_CMD, 1, FRF_AZ_BUF_CLR_CMD, 0);
...
fail2:
  ...
  EFX_POPULATE_OWORD_4(oword, FRF_AZ_BUF_UPD_CMD, 0, FRF_AZ_BUF_CLR_CMD, 1,
                       FRF_AZ_BUF_CLR_END_ID, id - 1, FRF_AZ_BUF_CLR_START_ID,
                       start);
  EFX_BAR_WRITEO(enp, FR_AZ_BUF_TBL_UPD_REG, &oword);
...
void efx_sram_buf_tbl_clear(__in efx_nic_t *enp, __in uint32_t id,
                            __in size_t n) {
  efx_oword_t oword;
  uint32_t start = id;
  uint32_t stop = start + n;
  ...
  EFX_POPULATE_OWORD_4(oword, FRF_AZ_BUF_UPD_CMD, 0, FRF_AZ_BUF_CLR_CMD, 1,
                       FRF_AZ_BUF_CLR_END_ID, stop - 1, FRF_AZ_BUF_CLR_START_ID,
                       start);
  EFX_BAR_WRITEO(enp, FR_AZ_BUF_TBL_UPD_REG, &oword);
}
...
static void efx_sram_byte_increment_set(__in size_t row, __in boolean_t negate,
...
static void efx_sram_all_the_same_set(__in size_t row, __in boolean_t negate,
                                      __out efx_qword_t *eqp) {
  _NOTE(ARGUNUSED(row))
...
static void efx_sram_bit_alternate_set(__in size_t row, __in boolean_t negate,
                                       __out efx_qword_t *eqp) {
  _NOTE(ARGUNUSED(row))

  EFX_POPULATE_QWORD_2(*eqp, EFX_DWORD_0, (negate) ? 0x55555555 : 0xaaaaaaaa,
                       EFX_DWORD_1, (negate) ? 0x55555555 : 0xaaaaaaaa);
}

static void efx_sram_byte_alternate_set(__in size_t row, __in boolean_t negate,
                                        __out efx_qword_t *eqp) {
  _NOTE(ARGUNUSED(row))

  EFX_POPULATE_QWORD_2(*eqp, EFX_DWORD_0, (negate) ? 0x00ff00ff : 0xff00ff00,
                       EFX_DWORD_1, (negate) ? 0x00ff00ff : 0xff00ff00);
}

static void efx_sram_byte_changing_set(__in size_t row, __in boolean_t negate,
                                       __out efx_qword_t *eqp) {
  size_t offset = row * FR_AZ_SRM_DBG_REG_STEP;
  unsigned int index;
...
static void efx_sram_bit_sweep_set(__in size_t row, __in boolean_t negate,
                                   __out efx_qword_t *eqp) {
  size_t offset = row * FR_AZ_SRM_DBG_REG_STEP;
...
efx_sram_pattern_fn_t __efx_sram_pattern_fns[] = {
    efx_sram_byte_increment_set, efx_sram_all_the_same_set,
    efx_sram_bit_alternate_set,  efx_sram_byte_alternate_set,
    efx_sram_byte_changing_set,  efx_sram_bit_sweep_set};

__checkReturn efx_rc_t efx_sram_test(__in efx_nic_t *enp,
