...
__checkReturn efx_rc_t efx_family(__in uint16_t venid, __in uint16_t devid,
...
__checkReturn efx_rc_t efx_family_probe_bar(__in uint16_t venid,
                                            __in uint16_t devid,
...
__checkReturn efx_rc_t efx_nic_create(__in efx_family_t family,
  ...
  case EFX_FAMILY_SIENA:
    ...
    enp->en_features =
        EFX_FEATURE_IPV6 | EFX_FEATURE_LFSR_HASH_INSERT |
        EFX_FEATURE_LINK_EVENTS | EFX_FEATURE_PERIODIC_MAC_STATS |
        EFX_FEATURE_MCDI | EFX_FEATURE_LOOKAHEAD_SPLIT |
  ...
  case EFX_FAMILY_MEDFORD:
     ...
     */
    enp->en_features =
        EFX_FEATURE_IPV6 | EFX_FEATURE_LINK_EVENTS |
        EFX_FEATURE_PERIODIC_MAC_STATS | EFX_FEATURE_MCDI |
        EFX_FEATURE_MAC_HEADER_FILTERS | EFX_FEATURE_MCDI_DMA |
...
__checkReturn efx_rc_t efx_nic_probe(__in efx_nic_t *enp,
  ...
  EFX_STATIC_ASSERT(EFX_FW_VARIANT_FULL_FEATURED == MC_CMD_FW_FULL_FEATURED);
  EFX_STATIC_ASSERT(EFX_FW_VARIANT_LOW_LATENCY == MC_CMD_FW_LOW_LATENCY);
  EFX_STATIC_ASSERT(EFX_FW_VARIANT_PACKED_STREAM == MC_CMD_FW_PACKED_STREAM);
  ...
  EFX_STATIC_ASSERT(EFX_FW_VARIANT_RULES_ENGINE == MC_CMD_FW_RULES_ENGINE);
...
__checkReturn efx_rc_t efx_nic_set_drv_limits(__inout efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_nic_set_drv_version(
  ...
  (void)memset(enp->en_drv_version, 0, sizeof(enp->en_drv_version));
...
__checkReturn efx_rc_t efx_nic_get_bar_region(__in efx_nic_t *enp,
  ...
  if ((rc = (enop->eno_get_bar_region)(enp, region, offsetp, sizep)) != 0) {
    goto fail2;
  }
...
__checkReturn efx_rc_t efx_nic_get_vi_pool(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_nic_init(__in efx_nic_t *enp) {
  const efx_nic_ops_t *enop = enp->en_enop;
  efx_rc_t rc;
...
void efx_nic_fini(__in efx_nic_t *enp) {
  const efx_nic_ops_t *enop = enp->en_enop;
...
void efx_nic_unprobe(__in efx_nic_t *enp) {
  const efx_nic_ops_t *enop = enp->en_enop;
...
void efx_nic_destroy(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t efx_nic_reset(__in efx_nic_t *enp) {
  const efx_nic_ops_t *enop = enp->en_enop;
  ...
  mod_flags &= ~(EFX_MOD_MCDI | EFX_MOD_PROBE | EFX_MOD_NVRAM | EFX_MOD_VPD |
                 EFX_MOD_MON);
#if EFSYS_OPT_TUNNEL
...
const efx_nic_cfg_t *efx_nic_cfg_get(__in const efx_nic_t *enp) {
  EFSYS_ASSERT3U(enp->en_magic, ==, EFX_NIC_MAGIC);
  EFSYS_ASSERT3U(enp->en_mod_flags, &, EFX_MOD_PROBE);
...
__checkReturn efx_rc_t efx_nic_get_fw_version(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_nic_get_board_info(
...
__checkReturn boolean_t efx_nic_hw_unavailable(__in efx_nic_t *enp) {
  const efx_nic_ops_t *enop = enp->en_enop;
...
void efx_nic_set_hw_unavailable(__in efx_nic_t *enp) {
  const efx_nic_ops_t *enop = enp->en_enop;
...
__checkReturn efx_rc_t efx_nic_register_test(__in efx_nic_t *enp) {
  const efx_nic_ops_t *enop = enp->en_enop;
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_get_loopback_modes(__in efx_nic_t *enp) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
  efx_mcdi_req_t req;
  ...
  if (req.emr_out_length_used >= MC_CMD_GET_LOOPBACK_MODES_OUT_40G_OFST +
    ...
    modes = *MCDI_OUT2(req, efx_qword_t, GET_LOOPBACK_MODES_OUT_40G);
    EFX_AND_QWORD(modes, mask);
    encp->enc_loopback_types[EFX_LINK_40000FDX] = modes;
  }

  if (req.emr_out_length_used >= MC_CMD_GET_LOOPBACK_MODES_OUT_V2_25G_OFST +
                                     MC_CMD_GET_LOOPBACK_MODES_OUT_V2_25G_LEN) {
    /* Response includes 25G loopback modes */
    modes = *MCDI_OUT2(req, efx_qword_t, GET_LOOPBACK_MODES_OUT_V2_25G);
    EFX_AND_QWORD(modes, mask);
    encp->enc_loopback_types[EFX_LINK_25000FDX] = modes;
  }

  if (req.emr_out_length_used >= MC_CMD_GET_LOOPBACK_MODES_OUT_V2_50G_OFST +
                                     MC_CMD_GET_LOOPBACK_MODES_OUT_V2_50G_LEN) {
    /* Response includes 50G loopback modes */
    modes = *MCDI_OUT2(req, efx_qword_t, GET_LOOPBACK_MODES_OUT_V2_50G);
    EFX_AND_QWORD(modes, mask);
  ...
  if (req.emr_out_length_used >=
    ...
    modes = *MCDI_OUT2(req, efx_qword_t, GET_LOOPBACK_MODES_OUT_V2_100G);
    EFX_AND_QWORD(modes, mask);
...
__checkReturn efx_rc_t efx_nic_calculate_pcie_link_bandwidth(
...
__checkReturn efx_rc_t efx_nic_get_fw_subvariant(
    __in efx_nic_t *enp, __out efx_nic_fw_subvariant_t *subvariantp) {
  efx_rc_t rc;
  uint32_t value;

  rc = efx_mcdi_get_nic_global(
      enp, MC_CMD_SET_NIC_GLOBAL_IN_FIRMWARE_SUBVARIANT, &value);
  if (rc != 0)
    goto fail1;
...
__checkReturn efx_rc_t efx_nic_set_fw_subvariant(
  ...
  rc = efx_mcdi_set_nic_global(
      enp, MC_CMD_SET_NIC_GLOBAL_IN_FIRMWARE_SUBVARIANT, subvariant);
  if (rc != 0)
    goto fail2;
...
__checkReturn efx_rc_t efx_nic_check_pcie_link_speed(
  ...
  if ((encp->enc_required_pcie_bandwidth_mbps == 0) || (pcie_link_width == 0) ||
      (pcie_link_width == 32) || (pcie_link_gen == 0)) {
    /*
  ...
  rc = efx_nic_calculate_pcie_link_bandwidth(pcie_link_width, pcie_link_gen,
                                             &bandwidth);
  if (rc != 0)
    goto fail1;
...
static __checkReturn efx_rc_t efx_nic_dma_config_regioned_find_region(
    __in const efx_nic_t *enp, __in efsys_dma_addr_t trgt_addr, __in size_t len,
  ...
  if (efx_nic_cfg_get(enp)->enc_dma_mapping != EFX_NIC_DMA_MAPPING_REGIONED) {
    rc = EINVAL;
    goto fail1;
  }
...
static __checkReturn efx_rc_t efx_nic_dma_config_regioned_add_region(
...
static __checkReturn efx_rc_t efx_nic_dma_config_regioned_add(
    __in efx_nic_t *enp, __in efsys_dma_addr_t trgt_addr, __in size_t len,
  ...
  rc = efx_nic_dma_config_regioned_find_region(enp, trgt_addr, len, &region);
  switch (rc) {
  case 0:
  ...
  case ENOENT:
    ...
    rc = efx_nic_dma_config_regioned_add_region(enp, trgt_addr, len, &region);
    if (rc != 0)
      goto fail1;
    break;
  default:
...
__checkReturn efx_rc_t efx_nic_dma_config_add(
    __in efx_nic_t *enp, __in efsys_dma_addr_t trgt_addr, __in size_t len,
    __out_opt efsys_dma_addr_t *nic_basep,
    __out_opt efsys_dma_addr_t *trgt_basep, __out_opt size_t *map_lenp) {
  ...
  case EFX_NIC_DMA_MAPPING_REGIONED:
    rc = efx_nic_dma_config_regioned_add(enp, trgt_addr, len, nic_basep,
                                         trgt_basep, map_lenp);
    if (rc != 0)
      goto fail1;
    break;
...
efx_nic_dma_reconfigure_regioned(__in efx_nic_t *enp) {
  efx_rc_t rc;

  rc = efx_mcdi_set_nic_addr_regions(enp, &enp->en_dma.end_u.endu_region_info);
  if (rc != 0)
    goto fail1;
...
__checkReturn efx_rc_t efx_nic_dma_reconfigure(__in efx_nic_t *enp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);
  efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_nic_dma_unknown_map(
    __in efx_nic_t *enp, __in efx_nic_dma_addr_type_t addr_type,
    __in efsys_dma_addr_t trgt_addr, __in size_t len,
...
static __checkReturn efx_rc_t efx_nic_dma_flat_map(
    __in efx_nic_t *enp, __in efx_nic_dma_addr_type_t addr_type,
    __in efsys_dma_addr_t trgt_addr, __in size_t len,
    __out efsys_dma_addr_t *nic_addrp) {
...
static __checkReturn efx_rc_t efx_nic_dma_regioned_map(
    __in efx_nic_t *enp, __in efx_nic_dma_addr_type_t addr_type,
    __in efsys_dma_addr_t trgt_addr, __in size_t len,
    __out efsys_dma_addr_t *nic_addrp) {
  const efx_nic_dma_region_t *region;
  ...
  *nic_addrp = region->endr_nic_base + (trgt_addr - region->endr_trgt_base);
...
__checkReturn efx_rc_t efx_nic_dma_map(__in efx_nic_t *enp,
  ...
  switch (mapping) {
  case EFX_NIC_DMA_MAPPING_UNKNOWN:
    rc = efx_nic_dma_unknown_map(enp, addr_type, trgt_addr, len, nic_addrp);
    if (rc != 0)
      goto fail1;
    break;
  case EFX_NIC_DMA_MAPPING_FLAT:
    rc = efx_nic_dma_flat_map(enp, addr_type, trgt_addr, len, nic_addrp);
    if (rc != 0)
      goto fail2;
    break;
  case EFX_NIC_DMA_MAPPING_REGIONED:
    rc = efx_nic_dma_regioned_map(enp, addr_type, trgt_addr, len, nic_addrp);
    if (rc != 0)
      goto fail3;
    break;
  default:
