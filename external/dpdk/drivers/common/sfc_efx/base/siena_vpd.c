...
static __checkReturn efx_rc_t siena_vpd_get_static(
    __in efx_nic_t *enp, __in uint32_t partn,
...
__checkReturn efx_rc_t siena_vpd_init(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t siena_vpd_size(__in efx_nic_t *enp,
                                      __out size_t *sizep) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
  uint32_t partn;
  efx_rc_t rc;
...
__checkReturn efx_rc_t siena_vpd_read(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t siena_vpd_verify(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t siena_vpd_reinit(__in efx_nic_t *enp,
                                        __in_bcount(size) caddr_t data,
                                        __in size_t size) {
  boolean_t wantpid;
  efx_rc_t rc;
  ...
  if (enp->en_u.siena.enu_svpd_length == 0)
    ...
    rc = efx_vpd_hunk_get(enp->en_u.siena.enu_svpd,
                          enp->en_u.siena.enu_svpd_length, EFX_VPD_ID, 0,
                          &offset, &length);
    if (rc == 0)
      wantpid = B_FALSE;
    else if (rc == ENOENT)
...
__checkReturn efx_rc_t siena_vpd_get(__in efx_nic_t *enp,
                                     __in_bcount(size) caddr_t data,
                                     __in size_t size,
  ...
  if (enp->en_u.siena.enu_svpd_length > 0) {
    if ((rc = efx_vpd_hunk_get(enp->en_u.siena.enu_svpd,
      ...
      memcpy(evvp->evv_value, enp->en_u.siena.enu_svpd + offset, length);
      return (0);
    } else if (rc != ENOENT)
      goto fail1;
  }
...
__checkReturn efx_rc_t siena_vpd_set(__in efx_nic_t *enp,
                                     __in_bcount(size) caddr_t data,
                                     __in size_t size,
...
__checkReturn efx_rc_t siena_vpd_next(__in efx_nic_t *enp,
                                      __in_bcount(size) caddr_t data,
                                      __in size_t size,
...
__checkReturn efx_rc_t siena_vpd_write(__in efx_nic_t *enp,
                                       __in_bcount(size) caddr_t data,
                                       __in size_t size) {
  efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
  siena_mc_dynamic_config_hdr_t *dcfg = NULL;
...
void siena_vpd_fini(__in efx_nic_t *enp) {
  EFSYS_ASSERT(enp->en_family == EFX_FAMILY_SIENA);
