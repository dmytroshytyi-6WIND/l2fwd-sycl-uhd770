...
const char *efx_mon_name(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t efx_mon_init(__in efx_nic_t *enp) {
  efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
...
const char *efx_mon_stat_name(__in efx_nic_t *enp, __in efx_mon_stat_t id) {
  _NOTE(ARGUNUSED(enp))
  EFSYS_ASSERT3U(enp->en_magic, ==, EFX_NIC_MAGIC);
...
const char *efx_mon_stat_description(__in efx_nic_t *enp,
...
__checkReturn boolean_t efx_mon_mcdi_to_efx_stat(__in int mcdi_index,
...
__checkReturn boolean_t efx_mon_get_stat_unit(
...
__checkReturn boolean_t efx_mon_get_stat_portmap(
...
__checkReturn efx_rc_t efx_mon_stats_update(__in efx_nic_t *enp,
...
void efx_mon_fini(__in efx_nic_t *enp) {
  efx_mon_t *emp = &(enp->en_mon);
