...
void siena_mcdi_send_request(__in efx_nic_t *enp,
  ...
  for (pos = 0; pos < sdu_len; pos += sizeof(efx_dword_t)) {
    ...
    EFX_BAR_TBL_WRITED(enp, FR_CZ_MC_TREG_SMEM, pdur + 1 + (pos >> 2), &dword,
                       B_FALSE);
  }
...
efx_rc_t siena_mcdi_poll_reboot(__in efx_nic_t *enp) {
#if 1
  /*
...
void siena_mcdi_read_response(__in efx_nic_t *enp,
  ...
  while (remaining > 0) {
    ...
    EFX_BAR_TBL_READD(enp, FR_CZ_MC_TREG_SMEM, pdur + ((offset + pos) >> 2),
                      &data, B_FALSE);
...
__checkReturn efx_rc_t siena_mcdi_init(__in efx_nic_t *enp,
...
void siena_mcdi_fini(__in efx_nic_t *enp){_NOTE(ARGUNUSED(enp))}

__checkReturn efx_rc_t
    siena_mcdi_feature_supported(__in efx_nic_t *enp,
...
void siena_mcdi_get_timeout(__in efx_nic_t *enp, __in efx_mcdi_req_t *emrp,
