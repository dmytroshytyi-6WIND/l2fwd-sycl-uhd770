...
__checkReturn efx_rc_t efx_nvram_init(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t efx_nvram_test(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t efx_nvram_size(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_nvram_get_version(__in efx_nic_t *enp,
                                             __in efx_nvram_type_t type,
  ...
  if ((rc = envop->envo_partn_get_version(enp, partn, subtypep, version)) != 0)
    goto fail2;
...
__checkReturn efx_rc_t efx_nvram_rw_start(__in efx_nic_t *enp,
                                          __in efx_nvram_type_t type,
...
__checkReturn efx_rc_t efx_nvram_read_chunk(__in efx_nic_t *enp,
                                            __in efx_nvram_type_t type,
 ...
 */
__checkReturn efx_rc_t efx_nvram_read_backup(__in efx_nic_t *enp,
                                             __in efx_nvram_type_t type,
                                             __in unsigned int offset,
                                             __out_bcount(size) caddr_t data,
                                             __in size_t size) {
  ...
  if ((rc = envop->envo_partn_read_backup(enp, partn, offset, data, size)) != 0)
    goto fail2;
...
__checkReturn efx_rc_t efx_nvram_erase(__in efx_nic_t *enp,
                                       __in efx_nvram_type_t type) {
  const efx_nvram_ops_t *envop = enp->en_envop;
  ...
  if ((rc = envop->envo_partn_erase(enp, partn, offset, eni.eni_partn_size)) !=
      0)
    goto fail3;
...
__checkReturn efx_rc_t efx_nvram_write_chunk(__in efx_nic_t *enp,
                                             __in efx_nvram_type_t type,
                                             __in unsigned int offset,
...
__checkReturn efx_rc_t efx_nvram_rw_finish(__in efx_nic_t *enp,
                                           __in efx_nvram_type_t type,
...
__checkReturn efx_rc_t efx_nvram_set_version(__in efx_nic_t *enp,
                                             __in efx_nvram_type_t type,
                                             __in_ecount(4)
                                                 uint16_t version[4]) {
  const efx_nvram_ops_t *envop = enp->en_envop;
...
__checkReturn efx_rc_t efx_nvram_validate(__in efx_nic_t *enp,
                                          __in efx_nvram_type_t type,
  ...
  if (envop->envo_buffer_validate != NULL) {
    if ((rc = envop->envo_buffer_validate(partn, partn_data, partn_size)) != 0)
      goto fail2;
  }
...
void efx_nvram_fini(__in efx_nic_t *enp) {
  EFSYS_ASSERT3U(enp->en_magic, ==, EFX_NIC_MAGIC);
  EFSYS_ASSERT3U(enp->en_mod_flags, &, EFX_MOD_PROBE);
  EFSYS_ASSERT3U(enp->en_mod_flags, &, EFX_MOD_NVRAM);
...
__checkReturn efx_rc_t efx_mcdi_nvram_partitions(__in efx_nic_t *enp,
                                                 __out_bcount(size)
                                                     caddr_t data,
                                                 __in size_t size,
  ...
  *npartnp = npartn;
  ...
  memcpy(data, MCDI_OUT2(req, uint32_t, NVRAM_PARTITIONS_OUT_TYPE_ID),
...
__checkReturn efx_rc_t efx_mcdi_nvram_metadata(
  ...
  if (MCDI_OUT_DWORD_FIELD(req, NVRAM_METADATA_OUT_FLAGS,
    ...
    if ((descp != NULL) && (size > 0)) {
      ...
      descp[0] = '\0';
      desclen = (req.emr_out_length_used - MC_CMD_NVRAM_METADATA_OUT_LEN(0));

      EFSYS_ASSERT3U(desclen, <=, MC_CMD_NVRAM_METADATA_OUT_DESCRIPTION_MAXNUM);
      ...
      memcpy(descp, MCDI_OUT2(req, char, NVRAM_METADATA_OUT_DESCRIPTION),
             desclen);
...
__checkReturn efx_rc_t efx_mcdi_nvram_info(__in efx_nic_t *enp,
                                           __in uint32_t partn,
                                           __out efx_nvram_info_t *enip) {
  ...
  enip->eni_write_size =
      (req.emr_out_length_used < MC_CMD_NVRAM_INFO_V2_OUT_LEN)
          ? 0
          : MCDI_OUT_DWORD(req, NVRAM_INFO_V2_OUT_WRITESIZE);

  enip->eni_flags = 0;

  if (MCDI_OUT_DWORD_FIELD(req, NVRAM_INFO_OUT_FLAGS, NVRAM_INFO_OUT_PROTECTED))
    enip->eni_flags |= EFX_NVRAM_FLAG_READ_ONLY;

  if (MCDI_OUT_DWORD_FIELD(req, NVRAM_INFO_OUT_FLAGS, NVRAM_INFO_OUT_READ_ONLY))
    enip->eni_flags |= EFX_NVRAM_FLAG_READ_ONLY;
 ...
 */
__checkReturn efx_rc_t efx_mcdi_nvram_update_start(__in efx_nic_t *enp,
                                                   __in uint32_t partn) {
...
__checkReturn efx_rc_t efx_mcdi_nvram_read(
  ...
  memcpy(data, MCDI_OUT2(req, uint8_t, NVRAM_READ_OUT_READ_BUFFER), size);
...
__checkReturn efx_rc_t efx_mcdi_nvram_erase(__in efx_nic_t *enp,
                                            __in uint32_t partn,
 ...
 */
__checkReturn efx_rc_t efx_mcdi_nvram_write(__in efx_nic_t *enp,
                                            __in uint32_t partn,
                                            __in uint32_t offset,
                                            __in_bcount(size) caddr_t data,
                                            __in size_t size) {
  ...
  memcpy(MCDI_IN2(req, uint8_t, NVRAM_WRITE_IN_WRITE_BUFFER), data, size);
 ...
 * NVRAM updates. Older firmware will ignore the flags field in the request.
 */
__checkReturn efx_rc_t efx_mcdi_nvram_update_finish(
  ...
  MCDI_IN_POPULATE_DWORD_3(req, NVRAM_UPDATE_FINISH_V2_IN_FLAGS,
                           ...
                           1, NVRAM_UPDATE_FINISH_V2_IN_FLAG_RUN_IN_BACKGROUND,
  ...
  if (req.emr_out_length_used < MC_CMD_NVRAM_UPDATE_FINISH_V2_OUT_LEN) {
    ...
    verify_result = MCDI_OUT_DWORD(req, NVRAM_UPDATE_FINISH_V2_OUT_RESULT_CODE);
  }
...
__checkReturn efx_rc_t efx_mcdi_nvram_test(__in efx_nic_t *enp,
                                           __in uint32_t partn) {
  efx_mcdi_req_t req;
