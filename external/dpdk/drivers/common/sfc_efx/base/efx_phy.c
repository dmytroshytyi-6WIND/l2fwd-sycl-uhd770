...
__checkReturn efx_rc_t efx_phy_probe(__in efx_nic_t *enp) {
...
__checkReturn efx_rc_t efx_phy_verify(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
...
__checkReturn efx_rc_t efx_phy_led_set(__in efx_nic_t *enp,
...
void efx_phy_adv_cap_get(__in efx_nic_t *enp, __in uint32_t flag,
...
__checkReturn efx_rc_t efx_phy_adv_cap_set(__in efx_nic_t *enp,
                                           __in uint32_t mask) {
  efx_port_t *epp = &(enp->en_port);
  const efx_phy_ops_t *epop = epp->ep_epop;
  uint32_t old_mask;
...
void efx_phy_lp_cap_get(__in efx_nic_t *enp, __out uint32_t *maskp) {
  efx_port_t *epp = &(enp->en_port);
...
__checkReturn efx_rc_t efx_phy_oui_get(__in efx_nic_t *enp,
                                       __out uint32_t *ouip) {
  efx_port_t *epp = &(enp->en_port);
  const efx_phy_ops_t *epop = epp->ep_epop;
...
void efx_phy_media_type_get(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_phy_module_get_info(
  ...
  if ((rc = efx_mcdi_phy_module_get_info(enp, dev_addr, offset, len, data)) !=
      0)
    goto fail2;
...
__checkReturn efx_rc_t efx_phy_fec_type_get(__in efx_nic_t *enp,
...
const char *efx_phy_stat_name(__in efx_nic_t *enp, __in efx_phy_stat_t type) {
  _NOTE(ARGUNUSED(enp))
  EFSYS_ASSERT3U(enp->en_magic, ==, EFX_NIC_MAGIC);
...
__checkReturn efx_rc_t efx_phy_stats_update(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_bist_enable_offline(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
  const efx_phy_ops_t *epop = epp->ep_epop;
  efx_rc_t rc;
...
__checkReturn efx_rc_t efx_bist_start(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_bist_poll(
...
void efx_bist_stop(__in efx_nic_t *enp, __in efx_bist_type_t type) {
  efx_port_t *epp = &(enp->en_port);
  const efx_phy_ops_t *epop = epp->ep_epop;
...
#endif /* EFSYS_OPT_BIST */
void efx_phy_unprobe(__in efx_nic_t *enp) {
  efx_port_t *epp = &(enp->en_port);
