...
static void mlx5_cq_init(struct mlx5_devx_cq *cq_obj, uint16_t cq_size) {
...
int mlx5_devx_cq_create(void *ctx, struct mlx5_devx_cq *cq_obj,
  ...
  umem_buf =
      mlx5_malloc(MLX5_MEM_RTE | MLX5_MEM_ZERO, umem_size, alignment, socket);
  if (!umem_buf) {
 ...
 */
static void mlx5_devx_wq_res_destroy(struct mlx5_devx_wq_res *rq_res) {
  if (rq_res->umem_obj)
 ...
 *   DevX RMP to destroy.
 */
static void mlx5_devx_rmp_destroy(struct mlx5_devx_rmp *rmp) {
...
int mlx5_devx_qp_create(void *ctx, struct mlx5_devx_qp *qp_obj,
                        uint32_t queue_size, struct mlx5_devx_qp_attr *attr,
                        int socket) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_devx_wq_init(void *ctx, uint32_t wqe_size, uint16_t log_wqbb_n,
                             int socket, struct mlx5_devx_wq_attr *wq_attr,
  ...
  /* Register allocated buffer in user space with DevX. */
  umem_obj = mlx5_os_umem_reg(ctx, (void *)(uintptr_t)umem_buf, umem_size, 0);
  if (!umem_obj) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_devx_rq_std_create(void *ctx, struct mlx5_devx_rq *rq_obj,
  ...
  ret = mlx5_devx_wq_init(ctx, wqe_size, log_wqbb_n, socket, &attr->wq_attr,
                          &rq_obj->wq);
  if (ret != 0)
    return ret;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_devx_rmp_create(void *ctx, struct mlx5_devx_rmp *rmp_obj,
                                uint32_t wqe_size, uint16_t log_wqbb_n,
  ...
  ret = mlx5_devx_wq_init(ctx, wqe_size, log_wqbb_n, socket, &rmp_attr.wq_attr,
                          &rmp_obj->wq);
  if (ret != 0)
    return ret;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_devx_rq_shared_create(void *ctx, struct mlx5_devx_rq *rq_obj,
                                      uint32_t wqe_size, uint16_t log_wqbb_n,
                                      struct mlx5_devx_create_rq_attr *attr,
                                      int socket) {
  struct mlx5_devx_obj *rq;
...
int mlx5_devx_rq_create(void *ctx, struct mlx5_devx_rq *rq_obj,
  ...
  if (rq_obj->rmp == NULL)
    return mlx5_devx_rq_std_create(ctx, rq_obj, wqe_size, log_wqbb_n, attr,
                                   socket);
  return mlx5_devx_rq_shared_create(ctx, rq_obj, wqe_size, log_wqbb_n, attr,
                                    socket);
}
...
int mlx5_devx_qp2rts(struct mlx5_devx_qp *qp, uint32_t remote_qp_id) {
  if (mlx5_devx_cmd_modify_qp_state(qp->qp, MLX5_CMD_OP_RST2INIT_QP,
                                    remote_qp_id)) {
    DRV_LOG(ERR, "Failed to modify QP to INIT state(%u).", rte_errno);
    return -1;
  }
  if (mlx5_devx_cmd_modify_qp_state(qp->qp, MLX5_CMD_OP_INIT2RTR_QP,
                                    remote_qp_id)) {
    DRV_LOG(ERR, "Failed to modify QP to RTR state(%u).", rte_errno);
    return -1;
  }
  if (mlx5_devx_cmd_modify_qp_state(qp->qp, MLX5_CMD_OP_RTR2RTS_QP,
                                    remote_qp_id)) {
    DRV_LOG(ERR, "Failed to modify QP to RTS state(%u).", rte_errno);
    return -1;
  }
  return 0;
}
