 ...
 * Device name.
 */
static void *mlx5_glue_devx_get_device_list(int *num_devices) {
...
static void mlx5_glue_devx_free_device_list(void *list) {
  if (!list) {
    errno = EINVAL;
    return;
  }
...
static int mlx5_glue_devx_close_device(void *ctx) {
...
static void *mlx5_glue_devx_open_device(void *device) {
...
static int mlx5_glue_devx_query_device(void *device_bdf, void *dev_inf) {
...
static void *mlx5_glue_devx_query_hca_iseg_mapping(void *ctx,
                                                   uint32_t *cb_iseg) {
  struct mlx5_context *mlx5_ctx;
  void *pv_iseg;
  int err;
  ...
  err = devx_query_hca_iseg_mapping(mlx5_ctx->devx_ctx, cb_iseg, &pv_iseg);
  if (err) {
    errno = err;
    return NULL;
  }
...
static void *mlx5_glue_devx_obj_create(void *ctx, void *in, size_t inlen,
  ...
  devx_obj->obj = devx_obj_create(devx_obj->devx_ctx, in, inlen, out, outlen);
...
static int mlx5_glue_devx_obj_destroy(void *obj) {
  mlx5_devx_obj_st *devx_obj;
...
static int mlx5_glue_devx_general_cmd(void *ctx, void *in, size_t inlen,
                                      void *out, size_t outlen) {
  if (!ctx)
    return -EINVAL;
...
static int mlx5_glue_devx_obj_query(void *obj, void *in, size_t inlen,
                                    void *out, size_t outlen) {
  return devx_cmd(GET_OBJ_CTX(obj), in, inlen, out, outlen);
}

static int mlx5_glue_devx_obj_modify(void *obj, void *in, size_t inlen,
                                     void *out, size_t outlen) {
  return devx_cmd(GET_OBJ_CTX(obj), in, inlen, out, outlen);
}

static int mlx5_glue_devx_umem_dereg(void *pumem) {
...
static void *mlx5_glue_devx_umem_reg(void *ctx, void *addr, size_t size,
  ...
  umem_hdl = devx_umem_reg(GET_DEVX_CTX(ctx), addr, size, w_access, id);
...
static void *mlx5_glue_devx_alloc_uar(void *ctx, uint32_t flags) {
  devx_uar_handle *uar;
...
static int mlx5_glue_devx_query_eqn(void *ctx, uint32_t cpus, uint32_t *eqn) {
  if (!ctx)
    return -EINVAL;
  return devx_query_eqn(GET_DEVX_CTX(ctx), cpus, eqn);
}

static void mlx5_glue_devx_free_uar(void *uar) {
...
static void *mlx5_glue_devx_fs_rule_add(void *ctx, void *in, uint32_t inlen)
...
static int mlx5_glue_devx_fs_rule_del(void *flow) {
  return devx_fs_rule_del(flow);
}

static int mlx5_glue_query_rt_values(void *ctx, void *devx_clock) {
  struct mlx5_context *mlx5_ctx;
  ...
  err = devx_hca_clock_query(mlx5_ctx->devx_ctx, &clock->p_iseg_internal_timer,
...
static int mlx5_glue_devx_init_showdown_event(void *ctx) {
  struct mlx5_context *mlx5_ctx;
  int err;
...
alignas(RTE_CACHE_LINE_SIZE) const
    struct mlx5_glue *mlx5_glue = &(const struct mlx5_glue){
