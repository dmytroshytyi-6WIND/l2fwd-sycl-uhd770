 ...
 */
static int mlx5_match_devx_bdf_to_addr(struct devx_device_bdf *devx_bdf,
                                       struct rte_pci_addr *addr) {
  if (addr->domain != (devx_bdf->bus_id >> 8) ||
      addr->bus != (devx_bdf->bus_id & 0xff) ||
      addr->devid != devx_bdf->dev_id || addr->function != devx_bdf->fnc_id) {
    return 0;
  }
  return 1;
}
 ...
 */
static int mlx5_match_devx_devices_to_addr(struct devx_device_bdf *devx_bdf,
                                           struct rte_pci_addr *addr) {
  int err;
...
int mlx5_os_open_device(struct mlx5_common_device *cdev, uint32_t classes) {
  ...
  if (classes != MLX5_CLASS_ETH && classes != MLX5_CLASS_CRYPTO) {
    DRV_LOG(ERR, "The chosen classes are not supported on Windows.");
    rte_errno = ENOTSUP;
    return -rte_errno;
  }
  errno = 0;
...
void *mlx5_os_umem_reg(void *ctx, void *addr, size_t size, uint32_t access) {
  ...
  umem = mlx5_malloc(MLX5_MEM_ZERO, (sizeof(*umem)), 0, SOCKET_ID_ANY);
  if (!umem) {
    errno = ENOMEM;
    return NULL;
  }
  umem->umem_hdl =
      mlx5_glue->devx_umem_reg(ctx, addr, size, access, &umem->umem_id);
  if (!umem->umem_hdl) {
    mlx5_free(umem);
    return NULL;
  }
 ...
 */
static int mlx5_os_reg_mr(void *pd, void *addr, size_t length,
  ...
  if (mlx5_devx_cmd_query_hca_attr(mlx5_pd->devx_ctx, &attr))
    return -1;
  obj =
      mlx5_os_umem_reg(mlx5_pd->devx_ctx, addr, length, IBV_ACCESS_LOCAL_WRITE);
  if (!obj)
    return -1;
 ...
 */
static void mlx5_os_dereg_mr(struct mlx5_pmd_mr *pmd_mr) {
  if (!pmd_mr)
    return;
  if (pmd_mr->mkey)
...
int mlx5_os_wrapped_mkey_create(void *ctx, void *pd, uint32_t pdn, void *addr,
                                size_t length,
                                struct mlx5_pmd_wrapped_mr *wpmd_mr) {
...
void mlx5_os_wrapped_mkey_destroy(struct mlx5_pmd_wrapped_mr *wpmd_mr) {
  struct mlx5_pmd_mr pmd_mr;
