...
static int mlx5_glue_fork_init(void) { return ibv_fork_init(); }

static struct ibv_pd *mlx5_glue_alloc_pd(struct ibv_context *context) {
  return ibv_alloc_pd(context);
}

static int mlx5_glue_dealloc_pd(struct ibv_pd *pd) {
  return ibv_dealloc_pd(pd);
}

static struct ibv_pd *mlx5_glue_import_pd(struct ibv_context *context,
                                          uint32_t pd_handle) {
...
static int mlx5_glue_unimport_pd(struct ibv_pd *pd) {
#ifdef HAVE_MLX5_IBV_IMPORT_CTX_PD_AND_MR
  ibv_unimport_pd(pd);
  return 0;
#else
...
static struct ibv_device **mlx5_glue_get_device_list(int *num_devices) {
  return ibv_get_device_list(num_devices);
}

static void mlx5_glue_free_device_list(struct ibv_device **list) {
  ibv_free_device_list(list);
}

static struct ibv_context *mlx5_glue_open_device(struct ibv_device *device) {
  return ibv_open_device(device);
}

static struct ibv_context *mlx5_glue_import_device(int cmd_fd) {
#ifdef HAVE_MLX5_IBV_IMPORT_CTX_PD_AND_MR
...
static int mlx5_glue_close_device(struct ibv_context *context) {
  return ibv_close_device(context);
}

static int mlx5_glue_query_device(struct ibv_context *context,
...
static int mlx5_glue_query_rt_values_ex(struct ibv_context *context,
                                        struct ibv_values_ex *values) {
  return ibv_query_rt_values_ex(context, values);
}

static int mlx5_glue_query_port(struct ibv_context *context, uint8_t port_num,
...
static int mlx5_glue_destroy_comp_channel(struct ibv_comp_channel *channel) {
  return ibv_destroy_comp_channel(channel);
}

static struct ibv_cq *mlx5_glue_create_cq(struct ibv_context *context, int cqe,
                                          void *cq_context,
...
static int mlx5_glue_destroy_cq(struct ibv_cq *cq) {
  return ibv_destroy_cq(cq);
}

static int mlx5_glue_get_cq_event(struct ibv_comp_channel *channel,
                                  struct ibv_cq **cq, void **cq_context) {
  return ibv_get_cq_event(channel, cq, cq_context);
}

static void mlx5_glue_ack_cq_events(struct ibv_cq *cq, unsigned int nevents) {
...
static int mlx5_glue_destroy_wq(struct ibv_wq *wq) {
  return ibv_destroy_wq(wq);
}
static int mlx5_glue_modify_wq(struct ibv_wq *wq, struct ibv_wq_attr *wq_attr) {
  return ibv_modify_wq(wq, wq_attr);
}

static struct ibv_flow *mlx5_glue_create_flow(struct ibv_qp *qp,
                                              struct ibv_flow_attr *flow) {
  return ibv_create_flow(qp, flow);
}

static int mlx5_glue_destroy_flow(struct ibv_flow *flow_id) {
  return ibv_destroy_flow(flow_id);
}

static int mlx5_glue_destroy_flow_action(void *action) {
...
static int mlx5_glue_destroy_qp(struct ibv_qp *qp) {
  return ibv_destroy_qp(qp);
}

static int mlx5_glue_modify_qp(struct ibv_qp *qp, struct ibv_qp_attr *attr,
                               int attr_mask) {
  return ibv_modify_qp(qp, attr, attr_mask);
}

static struct ibv_mr *mlx5_glue_reg_mr(struct ibv_pd *pd, void *addr,
                                       size_t length, int access) {
  return ibv_reg_mr(pd, addr, length, access);
}

static struct ibv_mr *mlx5_glue_reg_mr_iova(struct ibv_pd *pd, void *addr,
...
static struct ibv_mr *mlx5_glue_alloc_null_mr(struct ibv_pd *pd) {
...
static int mlx5_glue_dereg_mr(struct ibv_mr *mr) { return ibv_dereg_mr(mr); }
...
static int mlx5_glue_destroy_counter_set(struct ibv_counter_set *cs) {
#ifndef HAVE_IBV_DEVICE_COUNTERS_SET_V42
  (void)cs;
  return -ENOTSUP;
#else
...
static int mlx5_glue_attach_counters(struct ibv_counters *counters,
...
static int mlx5_glue_query_counters(struct ibv_counters *counters,
...
static void mlx5_glue_ack_async_event(struct ibv_async_event *event) {
  ibv_ack_async_event(event);
}

static int mlx5_glue_get_async_event(struct ibv_context *context,
                                     struct ibv_async_event *event) {
  return ibv_get_async_event(context, event);
}

static const char *mlx5_glue_port_state_str(enum ibv_port_state port_state) {
  return ibv_port_state_str(port_state);
}

static struct ibv_cq *mlx5_glue_cq_ex_to_cq(struct ibv_cq_ex *cq) {
  return ibv_cq_ex_to_cq(cq);
}

static void *mlx5_glue_dr_create_flow_action_dest_flow_tbl(void *tbl) {
#ifdef HAVE_MLX5DV_DR
...
static void *mlx5_glue_dr_create_flow_action_dest_port(void *domain,
                                                       uint32_t port) {
...
static void *mlx5_glue_dr_create_flow_action_drop(void) {
#ifdef HAVE_MLX5DV_DR_ESWITCH
...
static void *mlx5_glue_dr_create_flow_action_pop_vlan(void) {
#ifdef HAVE_MLX5DV_DR_VLAN
...
static void *mlx5_glue_dr_create_flow_tbl(void *domain, uint32_t level) {
#ifdef HAVE_MLX5DV_DR
...
static int mlx5_glue_dr_destroy_flow_tbl(void *tbl) {
#ifdef HAVE_MLX5DV_DR
...
static void *mlx5_glue_dr_create_domain(struct ibv_context *ctx,
...
static int mlx5_glue_dr_destroy_domain(void *domain) {
#ifdef HAVE_MLX5DV_DR
...
static int mlx5_glue_dr_sync_domain(void *domain, uint32_t flags) {
#ifdef HAVE_MLX5DV_DR
...
static int mlx5_glue_dv_query_device(struct ibv_context *ctx,
                                     struct mlx5dv_context *attrs_out) {
  return mlx5dv_query_device(ctx, attrs_out);
}

static int mlx5_glue_dv_set_context_attr(struct ibv_context *ibv_ctx,
                                         enum mlx5dv_set_ctx_attr_type type,
                                         void *attr) {
  return mlx5dv_set_context_attr(ibv_ctx, type, attr);
}

static int mlx5_glue_dv_init_obj(struct mlx5dv_obj *obj, uint64_t obj_type) {
...
static void *mlx5_glue_dv_create_flow(void *matcher, void *match_value,
  ...
  for (i = 0; i < num_actions; i++)
    actions_attr[i] = *((struct mlx5dv_flow_action_attr *)(actions[i]));
  return mlx5dv_create_flow(matcher, match_value, num_actions, actions_attr);
#endif
#else
  (void)matcher;
  (void)match_value;
...
static void *mlx5_glue_dv_create_flow_action_counter(void *counter_obj,
                                                     uint32_t offset) {
#ifdef HAVE_IBV_FLOW_DV_SUPPORT
#ifdef HAVE_MLX5DV_DR
...
static void *mlx5_glue_dv_create_flow_action_dest_ibv_qp(void *qp) {
#ifdef HAVE_IBV_FLOW_DV_SUPPORT
#ifdef HAVE_MLX5DV_DR
...
static void *mlx5_glue_dv_create_flow_action_dest_devx_tir(void *tir) {
...
static void *mlx5_glue_dv_create_flow_action_modify_header(
    struct ibv_context *ctx, enum mlx5dv_flow_table_type ft_type, void *domain,
    uint64_t flags, size_t actions_sz, uint64_t actions[]) {
#ifdef HAVE_IBV_FLOW_DV_SUPPORT
#ifdef HAVE_MLX5DV_DR
  (void)ctx;
  (void)ft_type;
  ...
  action->action = mlx5dv_create_flow_action_modify_header(ctx, actions_sz,
                                                           actions, ft_type);
  return action;
#endif
#else
...
static void *mlx5_glue_dv_create_flow_action_packet_reformat(
    struct ibv_context *ctx,
    enum mlx5dv_flow_action_packet_reformat_type reformat_type,
    enum mlx5dv_flow_table_type ft_type, struct mlx5dv_dr_domain *domain,
    uint32_t flags, size_t data_sz, void *data) {
#ifdef HAVE_IBV_FLOW_DV_SUPPORT
#ifdef HAVE_MLX5DV_DR
  (void)ctx;
  (void)ft_type;
  return mlx5dv_dr_action_create_packet_reformat(domain, flags, reformat_type,
                                                 data_sz, data);
#else
  (void)domain;
  (void)flags;
...
static void *mlx5_glue_dv_create_flow_action_tag(uint32_t tag) {
#ifdef HAVE_IBV_FLOW_DV_SUPPORT
#ifdef HAVE_MLX5DV_DR
...
mlx5_glue_dv_create_flow_action_aso(struct mlx5dv_dr_domain *domain,
                                    void *aso_obj, uint32_t offset,
                                    uint32_t flags, uint8_t return_reg_c) {
#if defined(HAVE_MLX5DV_DR) && defined(HAVE_MLX5_DR_CREATE_ACTION_ASO)
  return mlx5dv_dr_action_create_aso(domain, aso_obj, offset, flags,
                                     return_reg_c);
#else
  (void)domain;
  (void)aso_obj;
...
static void *mlx5_glue_dr_create_flow_action_default_miss(void) {
...
static int mlx5_glue_dv_destroy_flow(void *flow_id) {
#ifdef HAVE_MLX5DV_DR
...
static int mlx5_glue_dv_destroy_flow_matcher(void *matcher) {
#ifdef HAVE_IBV_FLOW_DV_SUPPORT
#ifdef HAVE_MLX5DV_DR
...
static struct ibv_context *mlx5_glue_dv_open_device(struct ibv_device *device) {
#ifdef HAVE_IBV_DEVX_OBJ
  return mlx5dv_open_device(device, &(struct mlx5dv_context_attr){
...
mlx5_glue_devx_obj_create(struct ibv_context *ctx, const void *in, size_t inlen,
...
static int mlx5_glue_devx_obj_destroy(struct mlx5dv_devx_obj *obj) {
#ifdef HAVE_IBV_DEVX_OBJ
...
static int mlx5_glue_devx_obj_query(struct mlx5dv_devx_obj *obj, const void *in,
...
static int mlx5_glue_devx_obj_modify(struct mlx5dv_devx_obj *obj,
...
static int mlx5_glue_devx_general_cmd(struct ibv_context *ctx, const void *in,
                                      size_t inlen, void *out, size_t outlen) {
#ifdef HAVE_IBV_DEVX_OBJ
...
#ifdef HAVE_IBV_DEVX_ASYNC
  return mlx5dv_devx_obj_query_async(obj, in, inlen, outlen, wr_id, cmd_comp);
#else
  (void)obj;
  (void)in;
  (void)inlen;
...
mlx5_glue_devx_get_async_cmd_comp(struct mlx5dv_devx_cmd_comp *cmd_comp,
                                  ...
                                  size_t cmd_resp_len) {
#ifdef HAVE_IBV_DEVX_ASYNC
  return mlx5dv_devx_get_async_cmd_comp(cmd_comp, cmd_resp, cmd_resp_len);
#else
  (void)cmd_comp;
  (void)cmd_resp;
  (void)cmd_resp_len;
...
static int mlx5_glue_devx_umem_dereg(struct mlx5dv_devx_umem *dv_devx_umem) {
#ifdef HAVE_IBV_DEVX_OBJ
...
static int mlx5_glue_devx_qp_query(struct ibv_qp *qp, const void *in,
                                   size_t inlen, void *out, size_t outlen) {
...
static int mlx5_glue_devx_wq_query(struct ibv_wq *wq, const void *in,
                                   size_t inlen, void *out, size_t outlen) {
#ifdef HAVE_IBV_DEVX_QP
...
static int mlx5_glue_devx_port_query(struct ibv_context *ctx, uint32_t port_num,
  ...
  struct mlx5dv_devx_port devx_port = {
      .comp_mask = MLX5DV_DEVX_PORT_VPORT | MLX5DV_DEVX_PORT_MATCH_REG_C_0};
...
static int mlx5_glue_dr_dump_single_rule(FILE *file, void *rule) {
...
static int mlx5_glue_dr_dump_domain(FILE *file, void *domain) {
...
static void *mlx5_glue_dr_create_flow_action_sampler(
...
static void *mlx5_glue_dr_action_create_dest_array(
    void *domain, size_t num_dest, struct mlx5dv_dr_action_dest_attr *dests[]) {
#ifdef HAVE_MLX5_DR_CREATE_ACTION_DEST_ARRAY
  return mlx5dv_dr_action_create_dest_array(domain, num_dest, dests);
#else
  (void)domain;
  (void)num_dest;
  (void)dests;
...
static int mlx5_glue_devx_query_eqn(struct ibv_context *ctx, uint32_t cpus,
                                    uint32_t *eqn) {
#ifdef HAVE_IBV_DEVX_OBJ
...
static int mlx5_glue_devx_subscribe_devx_event(
    struct mlx5dv_devx_event_channel *eventc, struct mlx5dv_devx_obj *obj,
    uint16_t events_sz, uint16_t events_num[], uint64_t cookie) {
#ifdef HAVE_IBV_DEVX_EVENT
  return mlx5dv_devx_subscribe_devx_event(eventc, obj, events_sz, events_num,
                                          cookie);
#else
  (void)eventc;
  (void)obj;
...
static void mlx5_glue_devx_free_uar(struct mlx5dv_devx_uar *devx_uar) {
#ifdef HAVE_IBV_DEVX_OBJ
...
static struct mlx5dv_var *mlx5_glue_dv_alloc_var(struct ibv_context *context,
                                                 uint32_t flags) {
#ifdef HAVE_IBV_VAR
...
static void mlx5_glue_dv_free_var(struct mlx5dv_var *var) {
#ifdef HAVE_IBV_VAR
  mlx5dv_free_var(var);
#else
  (void)var;
...
static void mlx5_glue_dr_reclaim_domain_memory(void *domain, uint32_t enable) {
...
static struct mlx5dv_pp *mlx5_glue_dv_alloc_pp(struct ibv_context *context,
...
static void mlx5_glue_dr_allow_duplicate_rules(void *domain, uint32_t allow) {
...
static void mlx5_glue_dv_free_pp(struct mlx5dv_pp *pp) {
#ifdef HAVE_MLX5DV_PP_ALLOC
  mlx5dv_pp_free(pp);
#else
  RTE_SET_USED(pp);
...
__rte_cache_aligned const struct mlx5_glue *mlx5_glue = &(
    ...
    .dr_create_flow_action_drop = mlx5_glue_dr_create_flow_action_drop,
    .dr_create_flow_action_push_vlan =
        mlx5_glue_dr_create_flow_action_push_vlan,
    .dr_create_flow_action_pop_vlan = mlx5_glue_dr_create_flow_action_pop_vlan,
    ...
    .dv_create_flow_action_counter = mlx5_glue_dv_create_flow_action_counter,
    ...
    .dr_create_flow_action_sampler = mlx5_glue_dr_create_flow_action_sampler,
    .dr_create_flow_action_dest_array = mlx5_glue_dr_action_create_dest_array,
