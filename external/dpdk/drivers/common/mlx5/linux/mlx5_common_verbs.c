 ...
 */
static void *mlx5_alloc_verbs_buf(size_t size, void *data) {
 ...
 *   A pointer to the callback data.
 */
static void mlx5_free_verbs_buf(void *ptr, void *data __rte_unused) {
  MLX5_ASSERT(data != NULL);
  mlx5_free(ptr);
}
 ...
 */
int mlx5_common_verbs_reg_mr(void *pd, void *addr, size_t length,
                             struct mlx5_pmd_mr *pmd_mr) {
  struct ibv_mr *ibv_mr;

  ibv_mr = mlx5_glue->reg_mr(
      pd, addr, length,
 ...
 *
 */
void mlx5_common_verbs_dereg_mr(struct mlx5_pmd_mr *pmd_mr) {
