...
static int mlx5_auxiliary_get_pci_path(const struct rte_auxiliary_device *dev,
...
static int mlx5_auxiliary_get_numa(const struct rte_auxiliary_device *dev) {
  unsigned long numa;
...
mlx5_get_aux_ibv_device(const struct rte_auxiliary_device *dev) {
  ...
  if (mlx5_auxiliary_get_child_name(dev->name, "/infiniband", ib_name,
                                    sizeof(ib_name)) != 0)
    goto out;
  while (n-- > 0) {
...
static bool mlx5_common_auxiliary_match(const char *name) {
  return strncmp(name, MLX5_AUXILIARY_PREFIX, strlen(MLX5_AUXILIARY_PREFIX)) ==
         0;
}
...
mlx5_common_auxiliary_dma_unmap(struct rte_auxiliary_device *auxiliary_dev,
                                void *addr, uint64_t iova, size_t len) {
  return mlx5_common_dev_dma_unmap(&auxiliary_dev->device, addr, iova, len);
}
