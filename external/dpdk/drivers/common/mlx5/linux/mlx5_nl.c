...
int mlx5_nl_init(int protocol, int groups) {
  ...
  if (buf_size < MLX5_SEND_BUF_SIZE) {
    ret = setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &buf_size, sizeof(buf_size));
    if (ret == -1) {
      rte_errno = errno;
      goto error;
    }
  ...
  if (buf_size < MLX5_RECV_BUF_SIZE) {
    ret = setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &buf_size, sizeof(buf_size));
    if (ret == -1) {
      rte_errno = errno;
      goto error;
    }
 ...
 *   rte_errno is set.
 */
static int mlx5_nl_request(int nlsk_fd, struct nlmsghdr *nh, uint32_t sn,
                           void *req, int len) {
 ...
 *   rte_errno is set.
 */
static int mlx5_nl_send(int nlsk_fd, struct nlmsghdr *nh, uint32_t sn) {
  struct sockaddr_nl sa = {
      .nl_family = AF_NETLINK,
  };
  struct iovec iov = {
 ...
 */
static int mlx5_nl_recv(int nlsk_fd, uint32_t sn,
  ...
  do {
    ...
    do {
      ...
      iov.iov_base = NULL;
      iov.iov_len = 0;
      recv_bytes = recvmsg(nlsk_fd, &msg, MSG_PEEK | MSG_TRUNC);
      if (recv_bytes < 0) {
        rte_errno = errno;
        ret = -rte_errno;
        goto exit;
    ...
    for (; NLMSG_OK(nh, (unsigned int)recv_bytes);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_mac_addr_cb(struct nlmsghdr *nh, void *arg) {
  ...
  for (attribute = MLX5_NDA_RTA(r); RTA_OK(attribute, len);
       attribute = RTA_NEXT(attribute, len)) {
    if (attribute->rta_type == NDA_LLADDR) {
      if (data->mac_n == MLX5_MAX_MAC_ADDRESSES) {
        DRV_LOG(WARNING, "not enough room to finalize the"
                         " request");
        rte_errno = ENOMEM;
        return -rte_errno;
      }
      ...
      rte_ether_format_addr(m, RTE_ETHER_ADDR_FMT_SIZE, RTA_DATA(attribute));
      DRV_LOG(DEBUG, "bridge MAC address %s", m);
#endif
      memcpy(&(*data->mac)[data->mac_n++], RTA_DATA(attribute),
             RTE_ETHER_ADDR_LEN);
    }
  }
  return 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_mac_addr_list(int nlsk_fd, unsigned int iface_idx,
  ...
  *mac_n = data.mac_n;
  return 0;
error:
  DRV_LOG(DEBUG, "Interface %u cannot retrieve MAC address list %s", iface_idx,
          strerror(rte_errno));
  return -rte_errno;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_mac_addr_modify(int nlsk_fd, unsigned int iface_idx,
  ...
  req.hdr.nlmsg_len =
      NLMSG_ALIGN(req.hdr.nlmsg_len) + RTA_ALIGN(req.rta.rta_len);
...
#ifdef RTE_LIBRTE_MLX5_DEBUG
{
  ...
  DRV_LOG(DEBUG, "Interface %u cannot %s MAC address %s %s", iface_idx,
...
int mlx5_nl_vf_mac_addr_modify(int nlsk_fd, unsigned int iface_idx,
  ...
  req.hdr.nlmsg_len =
      NLMSG_ALIGN(req.hdr.nlmsg_len) + RTA_ALIGN(req.vf_list_rta.rta_len) +
      RTA_ALIGN(req.vf_info_rta.rta_len) + RTA_ALIGN(req.vf_mac_rta.rta_len);
  req.vf_list_rta.rta_len =
      RTE_PTR_DIFF(NLMSG_TAIL(&req.hdr), &req.vf_list_rta);
  req.vf_info_rta.rta_len =
      RTE_PTR_DIFF(NLMSG_TAIL(&req.hdr), &req.vf_info_rta);
...
error:
  DRV_LOG(ERR,
          "representor %u cannot set VF MAC address " RTE_ETHER_ADDR_PRT_FMT
          " : %s",
          vf_index, RTE_ETHER_ADDR_BYTES(mac), strerror(rte_errno));
  return -rte_errno;
}
...
void mlx5_nl_mac_addr_flush(int nlsk_fd, unsigned int iface_idx,
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    if (BITFIELD_ISSET(mac_own, i))
      mlx5_nl_mac_addr_remove(nlsk_fd, iface_idx, mac_own, m, i);
  }
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_device_flags(int nlsk_fd, unsigned int iface_idx,
...
int mlx5_nl_promisc(int nlsk_fd, unsigned int iface_idx, int enable) {
  ...
  if (ret)
    DRV_LOG(DEBUG, "Interface %u cannot %s promisc mode: Netlink error %s",
            iface_idx, enable ? "enable" : "disable", strerror(rte_errno));
  return ret;
}
...
int mlx5_nl_allmulti(int nlsk_fd, unsigned int iface_idx, int enable) {
  int ret = mlx5_nl_device_flags(nlsk_fd, iface_idx, IFF_ALLMULTI, enable);

  if (ret)
    DRV_LOG(DEBUG, "Interface %u cannot %s allmulti : Netlink error %s",
            iface_idx, enable ? "enable" : "disable", strerror(rte_errno));
  return ret;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_cmdget_cb(struct nlmsghdr *nh, void *arg) {
  ...
  if (nh->nlmsg_type != RDMA_NL_GET_TYPE(RDMA_NL_NLDEV, RDMA_NLDEV_CMD_GET) &&
      nh->nlmsg_type !=
 ...
 */
static int mlx5_nl_port_info(int nl, uint32_t pindex,
                             struct mlx5_nl_port_info *data) {
  union {
    struct nlmsghdr nh;
    uint8_t buf[NLMSG_HDRLEN + NLA_HDRLEN + NLA_ALIGN(sizeof(data->ibindex)) +
                NLA_HDRLEN + NLA_ALIGN(sizeof(pindex))];
  } req = {
      .nh =
          {
              .nlmsg_len = NLMSG_LENGTH(0),
              .nlmsg_type = RDMA_NL_GET_TYPE(RDMA_NL_NLDEV, RDMA_NLDEV_CMD_GET),
  ...
  req.nh.nlmsg_type = RDMA_NL_GET_TYPE(RDMA_NL_NLDEV, RDMA_NLDEV_CMD_PORT_GET);
  ...
  if (!(data->flags & MLX5_NL_CMD_GET_IB_NAME) ||
      !(data->flags & MLX5_NL_CMD_GET_IB_INDEX) ||
      !(data->flags & MLX5_NL_CMD_GET_NET_INDEX) || !data->ifindex)
    goto error;
  return 1;
error:
  rte_errno = ENODEV;
 ...
 */
static void mlx5_nl_check_switch_info(bool num_vf_set,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_switch_info_cb(struct nlmsghdr *nh, void *arg) {
  ...
  while (off < nh->nlmsg_len) {
    ...
    case IFLA_PHYS_PORT_NAME:
      ...
      if (len > 0 && len < MLX5_PHYS_PORT_NAME_MAX) {
        ...
        info.name_type = MLX5_PHYS_PORT_NAME_TYPE_UNKNOWN;
      }
      break;
...
int mlx5_nl_switch_info(int nl, unsigned int ifindex,
  ...
  struct {
    ...
    uint32_t extmask;
      ...
      .nh =
          {
              .nlmsg_len =
                  NLMSG_LENGTH(sizeof(req.info) + RTA_LENGTH(sizeof(uint32_t))),
  ...
  if (info->master && info->representor) {
    DRV_LOG(ERR,
            "ifindex %u device is recognized as master"
            " and as representor",
            ifindex);
    rte_errno = ENODEV;
    ret = -rte_errno;
...
void mlx5_nl_vlan_vmwa_delete(struct mlx5_nl_vlan_vmwa_context *vmwa,
  ...
  if (ifindex) {
    ...
    if (ret < 0)
      DRV_LOG(WARNING,
              "netlink: error deleting VLAN WA"
              " ifindex %u, %d",
              ifindex, ret);
  }
}
...
static struct nlattr *nl_msg_tail(struct nlmsghdr *nlh) {
  return (struct nlattr *)(((uint8_t *)nlh) + NLMSG_ALIGN(nlh->nlmsg_len));
}

static void nl_attr_put(struct nlmsghdr *nlh, int type, const void *data,
                        int alen) {
...
static struct nlattr *nl_attr_nest_start(struct nlmsghdr *nlh, int type) {
...
static void nl_attr_nest_end(struct nlmsghdr *nlh, struct nlattr *nest) {
...
uint32_t mlx5_nl_vlan_vmwa_create(struct mlx5_nl_vlan_vmwa_context *vmwa,
  ...
  __rte_cache_aligned uint8_t
      buf[NLMSG_ALIGN(sizeof(struct nlmsghdr)) +
          NLMSG_ALIGN(sizeof(struct ifinfomsg)) +
          NLMSG_ALIGN(sizeof(struct nlattr)) * 8 +
          NLMSG_ALIGN(sizeof(uint32_t)) + NLMSG_ALIGN(sizeof(name)) +
          NLMSG_ALIGN(sizeof("vlan")) + NLMSG_ALIGN(sizeof(uint32_t)) +
  ...
  ret = snprintf(name, sizeof(name), "%s.%u.%u", MLX5_VMWA_VLAN_DEVICE_PFX,
                 ifindex, tag);
  ...
  if (ret >= 0)
    ret = mlx5_nl_recv(vmwa->nl_socket, sn, NULL, NULL);
  if (ret < 0) {
    DRV_LOG(WARNING, "netlink: VLAN %s create failure (%d)", name, ret);
  }
  /* Try to get ifindex of created or pre-existing device. */
  ret = if_nametoindex(name);
  if (!ret) {
    DRV_LOG(WARNING, "VLAN %s failed to get index (%d)", name, errno);
    return 0;
  }
  return ret;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_family_id_cb(struct nlmsghdr *nh, void *arg) {

  struct nlattr *tail = RTE_PTR_ADD(nh, nh->nlmsg_len);
  struct nlattr *nla = RTE_PTR_ADD(
      nh, NLMSG_ALIGN(sizeof(*nh)) + NLMSG_ALIGN(sizeof(struct genlmsghdr)));
 ...
 */
static int mlx5_nl_generic_family_id_get(int nlsk_fd, const char *name) {
  struct nlmsghdr *nlh;
  ...
  if (ret < 0) {
    DRV_LOG(DEBUG, "Failed to get Netlink %s family ID: %d.", name, ret);
    return ret;
  }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_roce_cb(struct nlmsghdr *nh, void *arg) {
...
int mlx5_nl_enable_roce_get(int nlsk_fd, int family_id, const char *pci_addr,
  ...
  if (ret < 0) {
    DRV_LOG(DEBUG, "Failed to get ROCE enable on device %s: %d.", pci_addr,
            ret);
    return ret;
  }
  *enable = cur_en;
  DRV_LOG(DEBUG, "ROCE is %sabled for device \"%s\".", cur_en ? "en" : "dis",
          pci_addr);
  return ret;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_driver_reload(int nlsk_fd, int family_id,
                                 const char *pci_addr) {
  struct nlmsghdr *nlh;
  struct genlmsghdr *genl;
  uint32_t sn = MLX5_NL_SN_GENERATE;
  ...
  if (ret >= 0)
    ret = mlx5_nl_recv(nlsk_fd, sn, NULL, NULL);
  if (ret < 0) {
    DRV_LOG(DEBUG, "Failed to reload %s device by Netlink - %d", pci_addr, ret);
    return ret;
  }
...
int mlx5_nl_enable_roce_set(int nlsk_fd, int family_id, const char *pci_addr,
  ...
  if (ret < 0) {
    DRV_LOG(DEBUG,
            "Failed to %sable ROCE for device %s by Netlink:"
            " %d.",
            enable ? "en" : "dis", pci_addr, ret);
    return ret;
  }
...
int mlx5_nl_read_events(int nlsk_fd, mlx5_nl_event_cb *cb, void *cb_arg) {
  ...
  while (1) {
    ...
    if (size < 0) {
      ...
      if (errno == EINTR)
        continue;
      DRV_LOG(DEBUG, "Failed to receive netlink message: %s", strerror(errno));
      rte_errno = errno;
      return -rte_errno;
    }
