 ...
 */
static inline const char *mlx5_os_get_dev_device_name(void *dev) {
  if (!dev)
    return NULL;
 ...
 */
static inline const char *mlx5_os_get_ctx_device_name(void *ctx) {
  if (!ctx)
    return NULL;
...
static inline const char *mlx5_os_get_ctx_device_path(void *ctx) {
  if (!ctx)
    return NULL;
 ...
 */
static inline uint32_t mlx5_os_get_umem_id(void *umem) {
  if (!umem)
    return 0;
 ...
 */
static inline int mlx5_os_get_devx_channel_fd(void *channel) {
  if (!channel)
    return 0;
 ...
 */
static inline off_t mlx5_os_get_devx_uar_mmap_offset(void *uar) {
 ...
 */
static inline void *mlx5_os_get_devx_uar_base_addr(void *uar) {
#ifdef HAVE_MLX5DV_DEVX_UAR_OFFSET
  if (!uar)
    return NULL;
 ...
 */
static inline void *mlx5_os_get_devx_uar_reg_addr(void *uar) {
#ifdef HAVE_MLX5DV_DEVX_UAR_OFFSET
  if (!uar)
    return NULL;
 ...
 */
static inline uint32_t mlx5_os_get_devx_uar_page_id(void *uar) {
#ifdef HAVE_MLX5DV_DEVX_UAR_OFFSET
  if (!uar)
    return 0;
...
__rte_internal static inline void *
mlx5_os_umem_reg(void *ctx, void *addr, size_t size, uint32_t access) {
  return mlx5_glue->devx_umem_reg(ctx, addr, size, access);
}

__rte_internal static inline int mlx5_os_umem_dereg(void *pumem) {
  return mlx5_glue->devx_umem_dereg(pumem);
}

static inline void *mlx5_os_devx_create_event_channel(void *ctx, int flags) {
  return mlx5_glue->devx_create_event_channel(ctx, flags);
}

static inline void mlx5_os_devx_destroy_event_channel(void *eventc) {
  mlx5_glue->devx_destroy_event_channel(eventc);
}

static inline int mlx5_os_devx_subscribe_devx_event(void *eventc, void *obj,
                                                    uint16_t events_sz,
                                                    uint16_t events_num[],
                                                    uint64_t cookie) {
 ...
 */
static inline void *mlx5_os_malloc(size_t align, size_t size) {
  void *buf;
 ...
 *
 */
static inline void mlx5_os_free(void *addr) { free(addr); }

void mlx5_set_context_attr(struct rte_device *dev, struct ibv_context *ctx);
 ...
 *  >0 if success.
 */
__rte_internal int mlx5_get_device_guid(const struct rte_pci_addr *dev,
                                        uint8_t *guid, size_t len);
