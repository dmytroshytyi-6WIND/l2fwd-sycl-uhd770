...
void mlx5_translate_port_name(const char *port_name_in,
  ...
  sc_items = sscanf(port_name_in, "%c%d", &ctrl, &port_info_out->ctrl_num);
  if (sc_items == 2 && ctrl == 'c') {
    port_name_in++; /* 'c' */
    port_name_in += snprintf(NULL, 0, "%d", port_info_out->ctrl_num);
  }
  /* Check for port-name as a string of the form pf0vf0 or pf0sf0 */
  sc_items = sscanf(port_name_in, "%c%c%d%c%c%d%c", &pf_c1, &pf_c2,
                    &port_info_out->pf_num, &vf_c1, &vf_c2,
                    &port_info_out->port_name, &eol);
  if (sc_items == 6 && pf_c1 == 'p' && pf_c2 == 'f') {
    ...
    if (vf_c1 == 's' && vf_c2 == 'f') {
      ...
      port_info_out->name_type = MLX5_PHYS_PORT_NAME_TYPE_PFSF;
      return;
    }
  }
  /*
   ...
   */
  sc_items =
      sscanf(port_name_in, "%c%d%c", &pf_c1, &port_info_out->port_name, &eol);
   ...
   */
  sc_items = sscanf(port_name_in, "%c%c%d%c", &pf_c1, &pf_c2,
                    &port_info_out->pf_num, &eol);
  ...
  if (!errno && (size_t)(end - port_name_in) == strlen(port_name_in)) {
...
int mlx5_get_ifname_sysfs(const char *ibdev_path, char *ifname) {
  ...
  while ((dent = readdir(dir)) != NULL) {
    ...
    MKSTR(path, "%s/device/net/%s/%s", ibdev_path, name,
 ...
 */
static char *mlx5_glue_path(char *buf, size_t size) {
...
static int mlx5_glue_dlopen(void) {
  ...
  char const *path[] = {
       ...
       */
      (geteuid() == getuid() && getegid() == getgid() ? getenv("MLX5_GLUE_PATH")
                                                      : NULL),
      /*
       ...
       * own.
       */
      (*RTE_EAL_PMD_PATH ? mlx5_glue_path(glue_path, sizeof(glue_path)) : ""),
  };
  unsigned int i = 0;
  void **sym;
  const char *dlmsg;
  ...
  while (!handle && i != RTE_DIM(path)) {
    ...
    do {
      ...
      ret = snprintf(name, sizeof(name), "%.*s%s" MLX5_GLUE, (int)len, path[i],
                     (!len || *(end - 1) == '/') ? "" : "/");
      if (ret == -1)
        break;
...
int mlx5_os_remote_pd_and_ctx_validate(struct mlx5_common_dev_config *config) {
  ...
  DRV_LOG(DEBUG,
          "Remote PD and CTX is supported: (cmd_fd=%d, "
          "pd_handle=%d).",
          device_fd, pd_handle);
#else
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int mlx5_os_pd_create(struct mlx5_common_device *cdev) {
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int mlx5_os_pd_import(struct mlx5_common_device *cdev) {
...
static int mlx5_nl_roce_disable(const char *addr) {
  ...
  if (devlink_id < 0) {
    ret = devlink_id;
    DRV_LOG(DEBUG, "Failed to get devlink id for ROCE operations by Netlink.");
    goto close;
  }
  ret = mlx5_nl_enable_roce_get(nlsk_fd, devlink_id, addr, &enable);
  if (ret) {
    DRV_LOG(DEBUG, "Failed to get ROCE enable by Netlink: %d.", ret);
    goto close;
  } else if (!enable) {
...
static int mlx5_sys_roce_disable(const char *addr) {
  FILE *file_o;
  int enable;
  int ret;
...
static int mlx5_roce_disable(const struct rte_device *dev) {
...
static struct ibv_device *mlx5_os_get_ibv_dev(const struct rte_device *dev) {
...
static struct ibv_device *mlx5_vdpa_get_ibv_dev(const struct rte_device *dev) {
  struct ibv_device *ibv;
  int retry;

  if (mlx5_roce_disable(dev) != 0) {
    DRV_LOG(WARNING, "Failed to disable ROCE for \"%s\".", dev->name);
    return NULL;
  }
...
static int mlx5_config_doorbell_mapping_env(int dbnc) {
  char *env;
  int value;
  ...
  if (dbnc == MLX5_ARG_UNSET)
    ...
    setenv(MLX5_SHUT_UP_BF, dbnc == MLX5_SQ_DB_NCACHED ? "1" : "0", 1);
  return value;
}

static void mlx5_restore_doorbell_mapping_env(int value) {
  MLX5_ASSERT(rte_eal_process_type() == RTE_PROC_PRIMARY);
 ...
 */
static struct ibv_context *mlx5_open_device(struct mlx5_common_device *cdev,
                                            uint32_t classes) {
  struct ibv_device *ibv;
 ...
 *   Pointer to ibv_context on success, NULL otherwise and rte_errno is set.
 */
static struct ibv_context *mlx5_import_device(struct mlx5_common_device *cdev) {
  struct ibv_context *ctx = NULL;
...
int mlx5_get_device_guid(const struct rte_pci_addr *dev, uint8_t *guid,
                         size_t len) {
  char tmp[512];
  ...
  closedir(dir);
  snprintf(tmp + strlen(tmp), sizeof(tmp) - strlen(tmp), "/%s/phys_switch_id",
           cur_ifname);
 ...
 */
int mlx5_os_wrapped_mkey_create(void *ctx, void *pd, uint32_t pdn, void *addr,
                                size_t length,
                                struct mlx5_pmd_wrapped_mr *pmd_mr) {
  struct mlx5_klm klm = {
      .byte_count = length,
  ...
  struct ibv_mr *ibv_mr = mlx5_glue->reg_mr(
      pd, addr, length,
      IBV_ACCESS_LOCAL_WRITE |
          (haswell_broadwell_cpu ? 0 : IBV_ACCESS_RELAXED_ORDERING));
...
void mlx5_os_wrapped_mkey_destroy(struct mlx5_pmd_wrapped_mr *pmd_mr) {
  if (!pmd_mr)
    return;
  if (pmd_mr->imkey)
