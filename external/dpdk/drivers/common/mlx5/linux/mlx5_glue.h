...
struct mlx5_glue {
  ...
  struct ibv_pd *(*import_pd)(struct ibv_context *context, uint32_t pd_handle);
  ...
  struct ibv_mr *(*reg_mr)(struct ibv_pd *pd, void *addr, size_t length,
                           int access);
  struct ibv_mr *(*reg_mr_iova)(struct ibv_pd *pd, void *addr, size_t length,
                                uint64_t iova, int access);
  struct ibv_mr *(*alloc_null_mr)(struct ibv_pd *pd);
  int (*dereg_mr)(struct ibv_mr *mr);
  struct ibv_counter_set *(*create_counter_set)(
      struct ibv_context *context, struct ibv_counter_set_init_attr *init_attr);
  ...
  int (*query_counters)(struct ibv_counters *counters, uint64_t *counters_value,
  ...
  void *(*dr_create_flow_action_dest_port)(void *domain, uint32_t port);
  void *(*dr_create_flow_action_drop)(void);
  void *(*dr_create_flow_action_push_vlan)(struct mlx5dv_dr_domain *domain,
                                           rte_be32_t vlan_tag);
  ...
  struct ibv_cq_ex *(*dv_create_cq)(struct ibv_context *context,
                                    struct ibv_cq_init_attr_ex *cq_attr,
                                    struct mlx5dv_cq_init_attr *mlx5_cq_attr);
  struct ibv_wq *(*dv_create_wq)(struct ibv_context *context,
  ...
  struct ibv_qp *(*dv_create_qp)(struct ibv_context *context,
                                 struct ibv_qp_init_attr_ex *qp_init_attr_ex,
                                 struct mlx5dv_qp_init_attr *dv_qp_init_attr);
  void *(*dv_create_flow_matcher)(struct ibv_context *context,
                                  struct mlx5dv_flow_matcher_attr *matcher_attr,
                                  void *tbl);
  void *(*dv_create_flow)(void *matcher, void *match_value, size_t num_actions,
                          void *actions[]);
  ...
  void *(*dv_create_flow_action_modify_header)(
      ...
      void *domain, uint64_t flags, size_t actions_sz, uint64_t actions[]);
  ...
  void *(*dv_create_flow_action_meter)(struct mlx5dv_dr_flow_meter_attr *attr);
  ...
  struct mlx5dv_devx_obj *(*devx_obj_create)(struct ibv_context *ctx,
  ...
  int (*devx_wq_query)(struct ibv_wq *wq, const void *in, size_t inlen,
                       void *out, size_t outlen);
  int (*devx_port_query)(struct ibv_context *ctx, uint32_t port_num,
  ...
  int (*devx_subscribe_devx_event)(
      struct mlx5dv_devx_event_channel *event_channel,
      struct mlx5dv_devx_obj *obj, uint16_t events_sz, uint16_t events_num[],
      uint64_t cookie);
  int (*devx_subscribe_devx_event_fd)(
      struct mlx5dv_devx_event_channel *event_channel, int fd,
      struct mlx5dv_devx_obj *obj, uint16_t event_num);
  ...
  void *(*dr_create_flow_action_dest_array)(
      void *domain, size_t num_dest,
      struct mlx5dv_dr_action_dest_attr *dests[]);
  void *(*dv_create_flow_action_aso)(struct mlx5dv_dr_domain *domain,
