...
static inline const char *pmd_drv_log_basename(const char *s) {
  const char *n = s;
...
#define PMD_DRV_LOG_(level, type, name, s, ...)                                \
  PMD_DRV_LOG__(level, type, name,                                             \
                s "\n" PMD_DRV_LOG_COMMA pmd_drv_log_basename(__FILE__)        \
                    PMD_DRV_LOG_COMMA __LINE__ PMD_DRV_LOG_COMMA __func__,     \
                __VA_ARGS__)
...
check_cqe(volatile struct mlx5_cqe *cqe, const uint16_t cqes_n,
  ...
  rte_io_rmb();
  if (unlikely(op_code == MLX5_CQE_RESP_ERR || op_code == MLX5_CQE_REQ_ERR))
 ...
 *   - Negative value and rte_errno is set otherwise.
 */
__rte_internal int mlx5_get_pci_addr(const char *dev_path,
 ...
 *   - Negative value and rte_errno is set otherwise.
 */
__rte_internal int mlx5_get_ifname_sysfs(const char *ibdev_path, char *ifname);

__rte_internal int mlx5_auxiliary_get_child_name(const char *dev,
                                                 const char *node, char *child,
                                                 size_t size);
 ...
 *   - Negative on error
 */
__rte_internal int mlx5_kvargs_process(struct mlx5_kvargs_ctrl *mkvlist,
 ...
 */
static __rte_always_inline void mlx5_doorbell_ring(struct mlx5_uar_data *uar,
 ...
 */
static inline uint16_t mlx5_db_map_type_get(off_t uar_mmap_offset,
                                            size_t page_size) {
...
__rte_internal void mlx5_common_init(void);
 ...
 */
static inline bool mlx5_imported_pd_and_ctx(struct mlx5_common_device *cdev) {
 ...
 *   - False otherwise.
 */
__rte_internal bool mlx5_dev_is_pci(const struct rte_device *dev);
 ...
 *   - False otherwise.
 */
__rte_internal bool mlx5_dev_is_vf_pci(struct rte_pci_device *pci_dev);

__rte_internal int mlx5_dev_mempool_subscribe(struct mlx5_common_device *cdev);

__rte_internal void mlx5_dev_mempool_unregister(struct mlx5_common_device *cdev,
                                                struct rte_mempool *mp);

__rte_internal int mlx5_devx_uar_prepare(struct mlx5_common_device *cdev,
                                         struct mlx5_uar *uar);

__rte_internal void mlx5_devx_uar_release(struct mlx5_uar *uar);
