...
void mlx5_mprq_buf_free_cb(void *addr __rte_unused, void *opaque) {
  struct mlx5_mprq_buf *buf = opaque;

  if (__atomic_load_n(&buf->refcnt, __ATOMIC_RELAXED) == 1) {
    rte_mempool_put(buf->mp, buf);
  } else if (unlikely(__atomic_sub_fetch(&buf->refcnt, 1, __ATOMIC_RELAXED) ==
                      0)) {
 ...
 */
static int mr_btree_expand(struct mlx5_mr_btree *bt, int n) {
  void *mem;
  int ret = 0;
  ...
  if (mem == NULL) {
    ...
    DRV_LOG(WARNING, "failed to expand MR B-tree (%p) table", (void *)bt);
    ret = -1;
  } else {
 ...
 */
static uint32_t mr_btree_lookup(struct mlx5_mr_btree *bt, uint16_t *idx,
                                uintptr_t addr) {
  ...
  MLX5_ASSERT(bt->len > 0 ||
              (lkp_tbl[0].start == 0 && lkp_tbl[0].lkey == UINT32_MAX));
  /* Binary search. */
  do {
 ...
 *   0 on success, -1 on failure.
 */
static int mr_btree_insert(struct mlx5_mr_btree *bt,
 ...
 */
static int mlx5_mr_btree_init(struct mlx5_mr_btree *bt, int n, int socket) {
  if (bt == NULL) {
    rte_errno = EINVAL;
    return -rte_errno;
  }
  ...
  bt->table = mlx5_malloc(MLX5_MEM_RTE | MLX5_MEM_ZERO,
                          sizeof(struct mr_cache_entry) * n, 0, socket);
  if (bt->table == NULL) {
    rte_errno = ENOMEM;
    DRV_LOG(DEBUG,
  ...
  DRV_LOG(DEBUG, "initialized B-tree %p with table %p", (void *)bt,
          (void *)bt->table);
  return 0;
}
...
void mlx5_mr_btree_free(struct mlx5_mr_btree *bt) {
  if (bt == NULL)
    return;
  DRV_LOG(DEBUG, "freeing B-tree %p with table %p", (void *)bt,
          (void *)bt->table);
  mlx5_free(bt->table);
  memset(bt, 0, sizeof(*bt));
}
...
int mlx5_mr_ctrl_init(struct mlx5_mr_ctrl *mr_ctrl, uint32_t *dev_gen_ptr,
  ...
  return mlx5_mr_btree_init(&mr_ctrl->cache_bh, MLX5_MR_BTREE_CACHE_N, socket);
}
 ...
 */
static int mr_find_next_chunk(struct mlx5_mr *mr, struct mr_cache_entry *entry,
                              int base_idx) {
  uintptr_t start = 0;
  uintptr_t end = 0;
  uint32_t idx = 0;
  ...
  for (idx = base_idx; idx < mr->ms_bmp_n; ++idx) {
    if (rte_bitmap_get(mr->ms_bmp, idx)) {
      ...
      msl = mr->msl;
      ms = rte_fbarray_get(&msl->memseg_arr, mr->ms_base_idx + idx);
...
int mlx5_mr_insert_cache(struct mlx5_mr_share_cache *share_cache,
  ...
  DRV_LOG(DEBUG, "Inserting MR(%p) to global cache(%p)", (void *)mr,
          (void *)share_cache);
 ...
 */
struct mlx5_mr *mlx5_mr_lookup_list(struct mlx5_mr_share_cache *share_cache,
                                    struct mr_cache_entry *entry,
                                    uintptr_t addr) {
  struct mlx5_mr *mr;
 ...
 */
static uint32_t mlx5_mr_lookup_cache(struct mlx5_mr_share_cache *share_cache,
                                     struct mr_cache_entry *entry,
                                     uintptr_t addr) {
  uint16_t idx;
 ...
 *   Pointer to a global shared MR cache.
 */
static void mlx5_mr_garbage_collect(struct mlx5_mr_share_cache *share_cache) {
...
mlx5_mr_create_secondary(struct mlx5_common_device *cdev,
  ...
  DRV_LOG(DEBUG,
          "MR CREATED by primary process for %p:\n"
          "  [0x%" PRIxPTR ", 0x%" PRIxPTR "), lkey=0x%x",
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
static uint32_t mlx5_mr_create_primary(void *pd,
                                       struct mlx5_mr_share_cache *share_cache,
                                       struct mr_cache_entry *entry,
                                       uintptr_t addr,
...
alloc_resources:
  ...
  DRV_LOG(DEBUG,
          "Extending %p to [0x%" PRIxPTR ", 0x%" PRIxPTR "),"
  ...
  mr = mlx5_malloc(MLX5_MEM_RTE | MLX5_MEM_ZERO,
                   RTE_ALIGN_CEIL(sizeof(*mr), RTE_CACHE_LINE_SIZE) + bmp_size,
                   RTE_CACHE_LINE_SIZE, msl->socket_id);
  if (mr == NULL) {
    DRV_LOG(DEBUG,
            "Unable to allocate memory for a new MR of"
            " address (%p).",
            (void *)addr);
    rte_errno = ENOMEM;
    goto err_nolock;
  ...
  if (mr->ms_bmp == NULL) {
    DRV_LOG(DEBUG,
            "Unable to initialize bitmap for a new MR of"
            " address (%p).",
            (void *)addr);
    rte_errno = EINVAL;
    goto err_nolock;
  ...
  if (mlx5_mr_lookup_cache(share_cache, entry, addr) != UINT32_MAX) {
    ...
    DRV_LOG(DEBUG, "Found MR for %p on final lookup, abort", (void *)addr);
    rte_rwlock_write_unlock(&share_cache->rwlock);
    rte_mcfg_mem_read_unlock();
    /*
  ...
  if (mr->pmd_mr.obj == NULL) {
    DRV_LOG(DEBUG, "Fail to create an MR for address (%p)", (void *)addr);
    rte_errno = EINVAL;
    goto err_mrlock;
  }
  ...
  DRV_LOG(DEBUG,
          ...
          rte_cpu_to_be_32(mr->pmd_mr.lkey), mr->ms_base_idx, mr->ms_n,
          mr->ms_bmp_n);
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
uint32_t mlx5_mr_create(struct mlx5_common_device *cdev,
                        struct mlx5_mr_share_cache *share_cache,
                        struct mr_cache_entry *entry, uintptr_t addr) {
  uint32_t ret = 0;
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mr_lookup_caches(struct mlx5_mr_ctrl *mr_ctrl,
                                 struct mr_cache_entry *entry, uintptr_t addr) {
  struct mlx5_mr_share_cache *share_cache =
      container_of(mr_ctrl->dev_gen_ptr, struct mlx5_mr_share_cache, dev_gen);
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mlx5_mr_addr2mr_bh(struct mlx5_mr_ctrl *mr_ctrl,
                                   uintptr_t addr) {
  uint32_t lkey;
  uint16_t bh_idx = 0;
 ...
 *   Pointer to a global shared MR cache.
 */
void mlx5_mr_release_cache(struct mlx5_mr_share_cache *share_cache) {
  struct mlx5_mr *mr_next;
...
int mlx5_mr_create_cache(struct mlx5_mr_share_cache *share_cache, int socket) {
  ...
  return mlx5_mr_btree_init(&share_cache->cache, MLX5_MR_BTREE_CACHE_N * 2,
                            socket);
}
...
void mlx5_mr_flush_local_cache(struct mlx5_mr_ctrl *mr_ctrl) {
  ...
  DRV_LOG(DEBUG, "mr_ctrl(%p): flushed, cur_gen=%d", (void *)mr_ctrl,
          mr_ctrl->cur_gen);
}
 ...
 */
struct mlx5_mr *mlx5_create_mr_ext(void *pd, uintptr_t addr, size_t len,
                                   int socket_id, mlx5_reg_mr_t reg_mr_cb) {
  struct mlx5_mr *mr = NULL;
  ...
  if (mr->pmd_mr.obj == NULL) {
    DRV_LOG(WARNING, "Fail to create MR for address (%p)", (void *)addr);
    mlx5_free(mr);
    return NULL;
  }
  ...
  DRV_LOG(DEBUG,
          ...
          "  [0x%" PRIxPTR ", 0x%" PRIxPTR "),"
          " lkey=0x%x base_idx=%u ms_n=%u, ms_bmp_n=%u",
          (void *)mr, (void *)addr, addr, addr + len,
          rte_cpu_to_be_32(mr->pmd_mr.lkey), mr->ms_base_idx, mr->ms_n,
          mr->ms_bmp_n);
  return mr;
}
...
void mlx5_free_mr_by_addr(struct mlx5_mr_share_cache *share_cache,
                          const char *ibdev_name, const void *addr,
                          size_t len) {
  const struct rte_memseg_list *msl;
  struct mlx5_mr *mr;
  int ms_n;
  ...
  DRV_LOG(DEBUG, "device %s free callback: addr=%p, len=%zu", ibdev_name, addr,
          len);
  msl = rte_mem_virt2memseg_list(addr);
  /* addr and len must be page-aligned. */
  MLX5_ASSERT((uintptr_t)addr == RTE_ALIGN((uintptr_t)addr, msl->page_sz));
  ...
  for (i = 0; i < ms_n; ++i) {
    ...
    DRV_LOG(DEBUG, "device %s MR(%p): clear bitmap[%u] for addr %p", ibdev_name,
            (void *)mr, pos, (void *)start);
    rte_bitmap_clear(mr->ms_bmp, pos);
    if (--mr->ms_n == 0) {
      LIST_REMOVE(mr, mr);
      LIST_INSERT_HEAD(&share_cache->mr_free_list, mr, mr);
      DRV_LOG(DEBUG, "device %s remove MR(%p) from list", ibdev_name,
              (void *)mr);
    }
    /*
...
void mlx5_mr_dump_cache(struct mlx5_mr_share_cache *share_cache __rte_unused) {
  ...
  LIST_FOREACH(mr, &share_cache->mr_list, mr) {
    ...
    DRV_LOG(DEBUG, "MR[%u], LKey = 0x%x, ms_n = %u, ms_bmp_n = %u", mr_n++,
            rte_cpu_to_be_32(mr->pmd_mr.lkey), mr->ms_n, mr->ms_bmp_n);
    if (mr->ms_n == 0)
      continue;
    for (n = 0; n < mr->ms_bmp_n;) {
      struct mr_cache_entry ret = {
      ...
      if (!ret.end)
        break;
      DRV_LOG(DEBUG, "  chunk[%u], [0x%" PRIxPTR ", 0x%" PRIxPTR ")", chunk_n++,
              ret.start, ret.end);
    }
  }
...
static int mlx5_range_compare_start(const void *lhs, const void *rhs) {
...
static void mlx5_range_from_mempool_chunk(struct rte_mempool *mp, void *opaque,
 ...
 */
static int mlx5_mempool_get_chunks(struct rte_mempool *mp,
                                   struct mlx5_range **out,
                                   unsigned int *out_n) {
  unsigned int n;
...
static void mlx5_mempool_get_extmem_cb(struct rte_mempool *mp, void *opaque,
  ...
  qsort(data->heap, data->heap_size, sizeof(heap[0]), mlx5_range_compare_start);
}
 ...
 */
static int mlx5_mempool_get_extmem(struct rte_mempool *mp,
                                   struct mlx5_range **out,
                                   unsigned int *out_n) {
  struct mlx5_mempool_get_extmem_data data;

  DRV_LOG(DEBUG, "Recovering external pinned pages of mempool %s", mp->name);
 ...
 *   0 on success, (-1) on failure.
 */
static int mlx5_get_mempool_ranges(struct rte_mempool *mp, bool is_extmem,
                                   struct mlx5_range **out,
                                   unsigned int *out_n) {
 ...
 *   0 on success, (-1) on failure.
 */
static int mlx5_mempool_reg_analyze(struct rte_mempool *mp, bool is_extmem,
                                    struct mlx5_range **out,
  ...
  *share_hugepage = false;
  ...
  if (msl != NULL) {
    ...
    if (i == ranges_n) {
      ...
      uintptr_t hugepage_start = RTE_ALIGN_FLOOR(reg_start, hugepage_sz);
...
static struct mlx5_mempool_reg *mlx5_mempool_reg_create(struct rte_mempool *mp,
                                                        unsigned int mrs_n,
                                                        bool is_extmem) {
  struct mlx5_mempool_reg *mpr = NULL;

  mpr =
      mlx5_malloc(MLX5_MEM_RTE | MLX5_MEM_ZERO, sizeof(struct mlx5_mempool_reg),
                  RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
  if (mpr == NULL) {
    DRV_LOG(ERR, "Cannot allocate mempool %s registration object", mp->name);
    return NULL;
  }
  mpr->mrs = mlx5_malloc(MLX5_MEM_RTE | MLX5_MEM_ZERO,
                         ...
                         RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
  if (!mpr->mrs) {
    DRV_LOG(ERR, "Cannot allocate mempool %s registration MRs", mp->name);
    mlx5_free(mpr);
    return NULL;
  }
  mpr->mp = mp;
 ...
 */
static void mlx5_mempool_reg_destroy(struct mlx5_mr_share_cache *share_cache,
...
static void mlx5_mempool_reg_attach(struct mlx5_mempool_reg *mpr) {
  unsigned int i;
 ...
 */
static bool mlx5_mempool_reg_detach(struct mlx5_mempool_reg *mpr) {
  unsigned int i;
  bool ret = false;

  for (i = 0; i < mpr->mrs_n; i++)
    ret |= __atomic_sub_fetch(&mpr->mrs[i].refcnt, 1, __ATOMIC_RELAXED) == 0;
  return ret;
}
...
mlx5_mr_mempool_register_primary(struct mlx5_mr_share_cache *share_cache,
  ...
  if (old_mpr != NULL && (!is_extmem || old_mpr->is_extmem)) {
    DRV_LOG(DEBUG, "Mempool %s is already registered for PD %p", mp->name, pd);
    rte_errno = EEXIST;
    goto exit;
  }
  ...
  if (new_mpr == NULL) {
    DRV_LOG(ERR, "Cannot create a registration object for mempool %s in PD %p",
            mp->name, pd);
    rte_errno = ENOMEM;
    goto exit;
  }
  ...
  if (share_hugepage) {
    ...
    if (mpr != NULL) {
      DRV_LOG(DEBUG, "Shared MR %#x in PD %p for mempool %s with mempool %s",
              mpr->mrs[0].pmd_mr.lkey, pd, mp->name, mpr->mp->name);
      ret = 0;
      goto exit;
    }
  }
  for (i = 0; i < ranges_n; i++) {
    ...
    if (share_cache->reg_mr_cb(pd, (void *)range->start, len, &mr->pmd_mr) <
        0) {
      DRV_LOG(ERR,
    ...
    DRV_LOG(DEBUG,
            ...
            "[0x%" PRIxPTR ", 0x%" PRIxPTR "] (%zu bytes) for mempool %s",
            mr->pmd_mr.lkey, pd, range->start, range->end, len, mp->name);
  }
  if (i != ranges_n) {
  ...
  if (mpr != NULL) {
    ...
    DRV_LOG(DEBUG,
            "Mempool %s registration for PD %p updated for external memory",
            mp->name, pd);
...
static int mlx5_mr_mempool_register_secondary(struct mlx5_common_device *cdev,
                                              struct rte_mempool *mp,
                                              bool is_extmem) {
 ...
 */
int mlx5_mr_mempool_register(struct mlx5_common_device *cdev,
                             struct rte_mempool *mp, bool is_extmem) {
  if (mp->flags & RTE_MEMPOOL_F_NON_IO)
    return 0;
  switch (rte_eal_process_type()) {
  case RTE_PROC_PRIMARY:
    return mlx5_mr_mempool_register_primary(&cdev->mr_scache, cdev->pd, mp,
                                            is_extmem);
  case RTE_PROC_SECONDARY:
...
static int mlx5_mr_mempool_unregister_secondary(struct mlx5_common_device *cdev,
                                                struct rte_mempool *mp) {
 ...
 *   0 on success, (-1) on failure and rte_errno is set.
 */
int mlx5_mr_mempool_unregister(struct mlx5_common_device *cdev,
                               struct rte_mempool *mp) {
  if (mp->flags & RTE_MEMPOOL_F_NON_IO)
    return 0;
  switch (rte_eal_process_type()) {
 ...
 */
static uint32_t mlx5_mempool_reg_addr2mr(struct mlx5_mempool_reg *mpr,
                                         uintptr_t addr,
                                         struct mr_cache_entry *entry) {
  uint32_t lkey = UINT32_MAX;
  unsigned int i;
 ...
 */
static uint32_t mlx5_lookup_mempool_regs(struct mlx5_mr_ctrl *mr_ctrl,
                                         struct mr_cache_entry *entry,
                                         struct rte_mempool *mp,
                                         uintptr_t addr) {
  struct mlx5_mr_share_cache *share_cache =
 ...
 *  0 on success, (-1) on failure and rte_errno is set.
 */
int mlx5_mr_mempool_populate_cache(struct mlx5_mr_ctrl *mr_ctrl,
                                   struct rte_mempool *mp) {
  struct mlx5_mr_share_cache *share_cache =
      container_of(mr_ctrl->dev_gen_ptr, struct mlx5_mr_share_cache, dev_gen);
  struct mlx5_mr_btree *bt = &mr_ctrl->cache_bh;
  ...
  for (i = 0; i < mpr->mrs_n; i++) {
    ...
    if (mr_btree_insert(bt, &entry) < 0) {
      DRV_LOG(ERR, "Cannot insert cache entry for mempool %s MR %08x", mp->name,
              entry.lkey);
      rte_errno = EINVAL;
      return -1;
    }
...
uint32_t mlx5_mr_mb2mr_bh(struct mlx5_mr_ctrl *mr_ctrl, struct rte_mbuf *mb) {
  struct rte_mempool *mp;
  ...
  if (external && mb->shinfo->free_cb == mlx5_mprq_buf_free_cb) {
    ...
    mprq = false;
    mp = mlx5_mb2mp(mb);
    pinned = rte_pktmbuf_priv_flags(mp) & RTE_PKTMBUF_POOL_F_PINNED_EXT_BUF;
  }
