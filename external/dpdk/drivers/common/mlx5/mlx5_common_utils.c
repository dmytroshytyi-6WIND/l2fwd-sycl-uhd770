...
static int mlx5_list_init(struct mlx5_list_inconst *l_inconst,
...
mlx5_list_create(const char *name, void *ctx, bool lcores_share,
  ...
  if (!cb_match || !cb_create || !cb_remove || !cb_clone || !cb_clone_free) {
    rte_errno = EINVAL;
    return NULL;
  }
  list = mlx5_malloc(MLX5_MEM_ZERO,
                     sizeof(*list) + (lcores_share ? sizeof(*gc) : 0), 0,
                     SOCKET_ID_ANY);

  if (!list)
    return NULL;
  if (name)
    snprintf(list->l_const.name, sizeof(list->l_const.name), "%s", name);
...
__list_lookup(struct mlx5_list_inconst *l_inconst,
              struct mlx5_list_const *l_const, int lcore_index, void *ctx,
              bool reuse) {
  struct mlx5_list_entry *entry = LIST_FIRST(&l_inconst->cache[lcore_index]->h);
  uint32_t ret;

  while (entry != NULL) {
    if (l_const->cb_match(l_const->ctx, entry, ctx) == 0) {
      if (reuse) {
        ret = __atomic_add_fetch(&entry->ref_cnt, 1, __ATOMIC_RELAXED) - 1;
        DRV_LOG(DEBUG, "mlx5 list %s entry %p ref: %u.", l_const->name,
                (void *)entry, entry->ref_cnt);
      } else if (lcore_index < MLX5_LIST_GLOBAL) {
        ret = __atomic_load_n(&entry->ref_cnt, __ATOMIC_RELAXED);
      }
...
struct mlx5_list_entry *mlx5_list_lookup(struct mlx5_list *list, void *ctx) {
...
mlx5_list_cache_insert(struct mlx5_list_inconst *l_inconst,
  ...
  struct mlx5_list_entry *lentry = l_const->cb_clone(l_const->ctx, gentry, ctx);
...
static void __list_cache_clean(struct mlx5_list_inconst *l_inconst,
                               struct mlx5_list_const *l_const,
                               int lcore_index) {
  struct mlx5_list_cache *c = l_inconst->cache[lcore_index];
  struct mlx5_list_entry *entry = LIST_FIRST(&c->h);
  uint32_t inv_cnt = __atomic_exchange_n(&c->inv_cnt, 0, __ATOMIC_RELAXED);
...
_mlx5_list_register(struct mlx5_list_inconst *l_inconst,
  ...
  if (unlikely(!l_inconst->cache[lcore_index])) {
    l_inconst->cache[lcore_index] = mlx5_malloc(
        0, sizeof(struct mlx5_list_cache), RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
  ...
  if (l_const->lcores_share) {
    ...
    rte_rwlock_read_lock(&l_inconst->lock);
    entry = __list_lookup(l_inconst, l_const, MLX5_LIST_GLOBAL, ctx, true);
    if (likely(entry)) {
      rte_rwlock_read_unlock(&l_inconst->lock);
      return mlx5_list_cache_insert(l_inconst, l_const, lcore_index, entry,
                                    ctx);
    }
  ...
  if (!l_const->lcores_share) {
    ...
    LIST_INSERT_HEAD(&l_inconst->cache[lcore_index]->h, entry, next);
    __atomic_add_fetch(&l_inconst->count, 1, __ATOMIC_RELAXED);
    DRV_LOG(DEBUG, "MLX5 list %s c%d entry %p new: %u.", l_const->name,
            lcore_index, (void *)entry, entry->ref_cnt);
    return entry;
  }
  ...
  if (unlikely(prev_gen_cnt != l_inconst->gen_cnt)) {
    struct mlx5_list_entry *oentry =
        __list_lookup(l_inconst, l_const, MLX5_LIST_GLOBAL, ctx, true);
    ...
    if (unlikely(oentry)) {
      ...
      return mlx5_list_cache_insert(l_inconst, l_const, lcore_index, oentry,
                                    ctx);
    }
  }
  /* 5. Update lists. */
  ...
  __atomic_add_fetch(&l_inconst->count, 1, __ATOMIC_RELAXED);
  DRV_LOG(DEBUG, "mlx5 list %s entry %p new: %u.", l_const->name, (void *)entry,
          entry->ref_cnt);
  return local_entry;
}

struct mlx5_list_entry *mlx5_list_register(struct mlx5_list *list, void *ctx) {
  ...
  entry =
      _mlx5_list_register(&list->l_inconst, &list->l_const, ctx, lcore_index);
  if (unlikely(lcore_index == MLX5_LIST_NLCORE))
    rte_spinlock_unlock(&list->l_const.lcore_lock);
  return entry;
}

static inline int _mlx5_list_unregister(struct mlx5_list_inconst *l_inconst,
                                        struct mlx5_list_const *l_const,
                                        struct mlx5_list_entry *entry,
                                        int lcore_idx) {
  ...
  if (entry->lcore_idx == (uint32_t)lcore_idx) {
    ...
    __atomic_add_fetch(&l_inconst->cache[entry->lcore_idx]->inv_cnt, 1,
                       __ATOMIC_RELAXED);
  } else {
    return 0;
  }
  if (!l_const->lcores_share) {
    ...
    DRV_LOG(DEBUG, "mlx5 list %s entry %p removed.", l_const->name,
            (void *)entry);
    return 0;
  }
...
static void mlx5_list_uninit(struct mlx5_list_inconst *l_inconst,
                             struct mlx5_list_const *l_const) {
  struct mlx5_list_entry *entry;
  int i;
...
uint32_t mlx5_list_get_entry_num(struct mlx5_list *list) {
  MLX5_ASSERT(list);
...
struct mlx5_hlist *mlx5_hlist_create(const char *name, uint32_t size,
  ...
  if (!rte_is_power_of_2(size)) {
    ...
    DRV_LOG(WARNING,
            "Size 0x%" PRIX32 " is not power of 2, will "
  ...
  alloc_size =
      sizeof(struct mlx5_hlist) + sizeof(struct mlx5_hlist_bucket) * act_size;
  if (lcores_share)
  ...
  h = mlx5_malloc(MLX5_MEM_ZERO, alloc_size, RTE_CACHE_LINE_SIZE,
                  SOCKET_ID_ANY);
  if (!h) {
    DRV_LOG(ERR, "No memory for hash list %s creation", name ? name : "None");
    return NULL;
  }
  if (name)
  ...
  DRV_LOG(DEBUG, "Hash list %s with size 0x%" PRIX32 " was created.", name,
          act_size);
  return h;
}
...
struct mlx5_list_entry *mlx5_hlist_register(struct mlx5_hlist *h, uint64_t key,
  ...
  entry =
      _mlx5_list_register(&h->buckets[idx].l, &h->l_const, ctx, lcore_index);
  if (likely(entry)) {
...
int mlx5_hlist_unregister(struct mlx5_hlist *h, struct mlx5_list_entry *entry) {
  ...
  int ret;
  uint32_t idx =
      h->l_const.lcores_share ? entry->gentry->bucket_idx : entry->bucket_idx;
  if (unlikely(lcore_index == -1)) {
    lcore_index = MLX5_LIST_NLCORE;
    rte_spinlock_lock(&h->l_const.lcore_lock);
  }
