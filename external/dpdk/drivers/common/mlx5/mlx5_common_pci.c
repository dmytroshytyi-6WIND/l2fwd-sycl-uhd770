...
static int pci_id_table_size_get(const struct rte_pci_id *id_table) {
  int table_size = 0;
...
static bool pci_id_exists(const struct rte_pci_id *id,
                          const struct rte_pci_id *table, int next_idx) {
...
static void pci_id_insert(struct rte_pci_id *new_table, int *next_idx,
...
static int pci_ids_table_update(const struct rte_pci_id *driver_id_table) {
  ...
  if (old_table == NULL) {
    ...
    for (id_iter = driver_id_table; id_iter->vendor_id != 0; id_iter++, i++)
      updated_table[i] = *id_iter;
  } else {
    /* First copy existing table entries. */
    for (id_iter = old_table; id_iter->vendor_id != 0; id_iter++, i++)
      updated_table[i] = *id_iter;
...
bool mlx5_dev_is_pci(const struct rte_device *dev) {
  return strcmp(dev->bus->name, "pci") == 0;
}

bool mlx5_dev_is_vf_pci(struct rte_pci_device *pci_dev) {
...
static int mlx5_common_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                                 struct rte_pci_device *pci_dev) {
  return mlx5_common_dev_probe(&pci_dev->device);
}

static int mlx5_common_pci_remove(struct rte_pci_device *pci_dev) {
  return mlx5_common_dev_remove(&pci_dev->device);
}

static int mlx5_common_pci_dma_map(struct rte_pci_device *pci_dev, void *addr,
                                   uint64_t iova, size_t len) {
  return mlx5_common_dev_dma_map(&pci_dev->device, addr, iova, len);
}

static int mlx5_common_pci_dma_unmap(struct rte_pci_device *pci_dev, void *addr,
                                     uint64_t iova, size_t len) {
