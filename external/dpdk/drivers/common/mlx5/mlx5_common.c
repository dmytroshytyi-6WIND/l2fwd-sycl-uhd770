 ...
 */
static inline void mlx5_cpu_id(unsigned int level, unsigned int *eax,
...
static TAILQ_HEAD(mlx5_drivers, mlx5_class_driver)
    drivers_list = TAILQ_HEAD_INITIALIZER(drivers_list);

/* Head of devices. */
static TAILQ_HEAD(mlx5_devices, mlx5_common_device)
    devices_list = TAILQ_HEAD_INITIALIZER(devices_list);
...
static int class_name_to_value(const char *class_name) {
  unsigned int i;
...
static struct mlx5_class_driver *driver_get(uint32_t class) {
...
int mlx5_kvargs_process(struct mlx5_kvargs_ctrl *mkvlist,
                        const char *const keys[], arg_handler_t handler,
                        void *opaque_arg) {
 ...
 */
static int mlx5_kvargs_prepare(struct mlx5_kvargs_ctrl *mkvlist,
  ...
  DRV_LOG(DEBUG, "Parse successfully %u devargs.", mkvlist->kvlist->count);
  return 0;
}
 ...
 *   Pointer to mlx5 kvargs control.
 */
static void mlx5_kvargs_release(struct mlx5_kvargs_ctrl *mkvlist) {
  if (mkvlist == NULL)
    return;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_kvargs_validate(struct mlx5_kvargs_ctrl *mkvlist) {
  uint32_t i;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_common_args_check_handler(const char *key, const char *val,
                                          void *opaque) {
  ...
  if (strcmp(key, MLX5_TX_DB_NC) == 0)
    DRV_LOG(WARNING, "%s: deprecated parameter, converted to queue_db_nc", key);
  if (strcmp(key, MLX5_SQ_DB_NC) == 0 || strcmp(key, MLX5_TX_DB_NC) == 0) {
    if (tmp != MLX5_SQ_DB_CACHED && tmp != MLX5_SQ_DB_NCACHED &&
        tmp != MLX5_SQ_DB_HEURISTIC) {
      DRV_LOG(ERR, "Invalid Send Queue doorbell mapping parameter.");
      rte_errno = EINVAL;
      return -rte_errno;
    }
    config->dbnc = tmp;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_common_config_get(struct mlx5_kvargs_ctrl *mkvlist,
                                  struct mlx5_common_dev_config *config) {
  const char **params = (const char *[]){
      RTE_DEVARGS_KEY_CLASS,  MLX5_DRIVER_KEY,
      MLX5_TX_DB_NC,          MLX5_SQ_DB_NC,
      MLX5_MR_EXT_MEMSEG_EN,  MLX5_SYS_MEM_EN,
      MLX5_MR_MEMPOOL_REG_EN, MLX5_DEVICE_FD,
      MLX5_PD_HANDLE,         NULL,
  };
  int ret = 0;
  ...
  ret = mlx5_kvargs_process(mkvlist, params, mlx5_common_args_check_handler,
                            config);
  if (ret) {
    rte_errno = EINVAL;
    return -rte_errno;
...
static int devargs_class_handler(__rte_unused const char *key,
...
static int parse_class_options(const struct rte_devargs *devargs,
...
static int is_valid_class_combination(uint32_t user_classes) {
  unsigned int i;
...
static bool mlx5_bus_match(const struct mlx5_class_driver *drv,
...
int mlx5_dev_to_pci_str(const struct rte_device *dev, char *addr, size_t size) {
  ...
  return mlx5_auxiliary_get_pci_str(RTE_DEV_TO_AUXILIARY_CONST(dev), addr,
                                    size);
#else
  rte_errno = ENODEV;
  return -rte_errno;
 ...
 */
static int mlx5_dev_mempool_register(struct mlx5_common_device *cdev,
 ...
 */
void mlx5_dev_mempool_unregister(struct mlx5_common_device *cdev,
                                 struct rte_mempool *mp) {
  if (mlx5_mr_mempool_unregister(cdev, mp) < 0)
    DRV_LOG(WARNING, "Failed to unregister mempool %s for PD %p: %s", mp->name,
            cdev->pd, rte_strerror(rte_errno));
}
 ...
 */
static void mlx5_dev_mempool_register_cb(struct rte_mempool *mp, void *arg) {
 ...
 *   Pointer to the device shared context.
 */
static void mlx5_dev_mempool_unregister_cb(struct rte_mempool *mp, void *arg) {
 ...
 */
static void mlx5_dev_mempool_event_cb(enum rte_mempool_event event,
  ...
  case RTE_MEMPOOL_EVENT_READY:
    if (mlx5_dev_mempool_register(cdev, mp, false) < 0)
      DRV_LOG(ERR, "Failed to register new mempool %s for PD %p: %s", mp->name,
              cdev->pd, rte_strerror(rte_errno));
    break;
...
int mlx5_dev_mempool_subscribe(struct mlx5_common_device *cdev) {
  int ret = 0;
  ...
  ret = rte_mempool_event_callback_register(mlx5_dev_mempool_event_cb, cdev);
  if (ret != 0 && rte_errno != EEXIST)
    goto exit;
...
static void mlx5_dev_mempool_unsubscribe(struct mlx5_common_device *cdev) {
  int ret;

  if (!cdev->mr_scache.mp_cb_registered || !cdev->config.mr_mempool_reg_en)
    return;
  /* Stop watching for mempool events and unregister all mempools. */
  ret = rte_mempool_event_callback_unregister(mlx5_dev_mempool_event_cb, cdev);
  if (ret == 0)
 ...
 *   Size of memory.
 */
static void mlx5_mr_mem_event_cb(enum rte_mem_event event_type,
  ...
  case RTE_MEM_EVENT_FREE:
    ...
    mlx5_free_mr_by_addr(&cdev->mr_scache,
                         mlx5_os_get_ctx_device_name(cdev->ctx), addr, len);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static void mlx5_dev_hw_global_release(struct mlx5_common_device *cdev) {
  if (cdev->pd != NULL) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_dev_hw_global_prepare(struct mlx5_common_device *cdev,
                                      uint32_t classes) {
  int ret;
...
static void mlx5_common_dev_release(struct mlx5_common_device *cdev) {
  pthread_mutex_lock(&devices_list_lock);
...
mlx5_common_dev_create(struct rte_device *eal_dev, uint32_t classes,
  ...
  if (ret < 0) {
    DRV_LOG(ERR, "Failed to process device arguments: %s", strerror(rte_errno));
    rte_free(cdev);
    return NULL;
  }
  ...
  if (TAILQ_EMPTY(&devices_list))
    rte_mem_event_callback_register("MLX5_MEM_EVENT_CB", mlx5_mr_mem_event_cb,
                                    NULL);
exit:
  pthread_mutex_lock(&devices_list_lock);
...
static int drivers_remove(struct mlx5_common_device *cdev,
...
static int drivers_probe(struct mlx5_common_device *cdev, uint32_t user_classes,
                         struct mlx5_kvargs_ctrl *mkvlist) {
  struct mlx5_class_driver *driver;
  ...
  TAILQ_FOREACH(driver, &drivers_list, next) {
    ...
    if (already_loaded && driver->probe_again == 0) {
      DRV_LOG(ERR, "Device %s is already probed", cdev->dev->name);
      ret = -EEXIST;
      goto probe_err;
    }
    ret = driver->probe(cdev, mkvlist);
    if (ret < 0) {
      DRV_LOG(ERR, "Failed to load driver %s", driver->name);
      goto probe_err;
    }
...
int mlx5_common_dev_probe(struct rte_device *eal_dev) {
  ...
  if (ret < 0) {
    DRV_LOG(ERR, "Unsupported device arguments: %s", eal_dev->devargs->args);
    return ret;
  }
  ret = parse_class_options(eal_dev->devargs, mkvlist_p);
  if (ret < 0) {
    DRV_LOG(ERR, "Unsupported mlx5 class type: %s", eal_dev->devargs->args);
    goto class_err;
  }
  classes = ret;
  if (classes == 0)
  ...
  if (!cdev) {
    ...
    if (ret) {
      DRV_LOG(ERR, "Probe again parameters aren't compatible : %s",
              strerror(rte_errno));
      goto class_err;
    }
  }
 ...
 */
int mlx5_common_dev_dma_map(struct rte_device *rte_dev, void *addr,
  ...
  if (!dev) {
    DRV_LOG(WARNING, "Unable to find matching mlx5 device to device %s",
            rte_dev->name);
    rte_errno = ENODEV;
    return -1;
  }
  mr = mlx5_create_mr_ext(dev->pd, (uintptr_t)addr, len, SOCKET_ID_ANY,
 ...
 *   0 on success, negative value on error.
 */
int mlx5_common_dev_dma_unmap(struct rte_device *rte_dev, void *addr,
                              uint64_t iova __rte_unused,
...
void mlx5_class_driver_register(struct mlx5_class_driver *driver) {
 ...
 */
static void *mlx5_devx_alloc_uar(struct mlx5_common_device *cdev) {
  void *uar;
...
void mlx5_devx_uar_release(struct mlx5_uar *uar) {
  if (uar->obj != NULL)
    mlx5_glue->devx_free_uar(uar->obj);
  memset(uar, 0, sizeof(*uar));
}

int mlx5_devx_uar_prepare(struct mlx5_common_device *cdev,
                          struct mlx5_uar *uar) {
