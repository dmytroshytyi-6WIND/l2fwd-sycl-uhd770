...
static void *mlx5_devx_get_hca_cap(void *ctx, uint32_t *in, uint32_t *out,
  ...
  if (rc) {
    DRV_LOG(ERR, "Failed to query devx HCA capabilities func %#02x",
            flags >> 1);
    if (err)
      *err = rc > 0 ? -rc : rc;
    return NULL;
  ...
  if (status) {
    DRV_LOG(ERR,
            "Failed to query devx HCA capabilities func %#02x status %x, "
            "syndrome = %x",
 ...
 */
int mlx5_devx_cmd_register_read(void *ctx, uint16_t reg_id, uint32_t arg,
  ...
  MLX5_SET(access_register_in, in, opcode, MLX5_CMD_OP_ACCESS_REGISTER_USER);
  MLX5_SET(access_register_in, in, op_mod, MLX5_ACCESS_REGISTER_IN_OP_MOD_READ);
  ...
  if (status) {
    ...
    DRV_LOG(DEBUG,
            "Failed to read access NIC register 0x%X, "
            "status %x, syndrome = %x",
 ...
 *   0 on success, a negative value otherwise.
 */
int mlx5_devx_cmd_register_write(void *ctx, uint16_t reg_id, uint32_t arg,
                                 uint32_t *data, uint32_t dw_cnt) {
  ...
  rc = mlx5_glue->devx_general_cmd(
      ctx, in, MLX5_ST_SZ_BYTES(access_register_in) + dw_cnt * sizeof(uint32_t),
      out, sizeof(out));
  if (rc)
    goto error;
  status = MLX5_GET(access_register_out, out, status);
  if (status) {
    ...
    DRV_LOG(DEBUG,
            "Failed to write access NIC register 0x%X, "
            "status %x, syndrome = %x",
            reg_id, status, syndrome);
    return -1;
  }
...
struct mlx5_devx_obj *mlx5_devx_cmd_flow_counter_alloc(void *ctx,
  ...
  struct mlx5_devx_obj *dcs =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*dcs), 0, SOCKET_ID_ANY);
 ...
 *   0 on success, a negative value otherwise.
 */
int mlx5_devx_cmd_flow_counter_query(struct mlx5_devx_obj *dcs, int clear,
                                     uint32_t n_counters, uint64_t *pkts,
                                     uint64_t *bytes, uint32_t mkey, void *addr,
                                     void *cmd_comp, uint64_t async_id) {
  ...
  if (n_counters) {
    MLX5_SET(query_flow_counter_in, in, num_of_counters, n_counters);
    MLX5_SET(query_flow_counter_in, in, dump_to_memory, 1);
    MLX5_SET(query_flow_counter_in, in, mkey, mkey);
    MLX5_SET64(query_flow_counter_in, in, address, (uint64_t)(uintptr_t)addr);
  }
  if (!cmd_comp)
    rc = mlx5_glue->devx_obj_query(dcs->obj, in, sizeof(in), out, out_len);
  else
    rc = mlx5_glue->devx_obj_query_async(dcs->obj, in, sizeof(in), out_len,
                                         async_id, cmd_comp);
  if (rc) {
...
mlx5_devx_cmd_mkey_create(void *ctx, struct mlx5_devx_mkey_attr *attr) {
  ...
  struct mlx5_devx_obj *mkey =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*mkey), 0, SOCKET_ID_ANY);
  size_t pgsize;
  ...
  if (klm_num > 0) {
    ...
    MLX5_SET(mkc, mkc, access_mode_1_0,
             attr->log_entity_size ? MLX5_MKC_ACCESS_MODE_KLM_FBS
  ...
  MLX5_SET(mkc, mkc, relaxed_ordering_write, attr->relaxed_ordering_write);
  ...
  mkey->obj =
      mlx5_glue->devx_obj_create(ctx, in, in_size_dw * 4, out, sizeof(out));
  if (!mkey->obj) {
...
int mlx5_devx_get_out_command_status(void *out) {
  ...
  if (status) {
    ...
    DRV_LOG(ERR, "Bad DevX status %x, syndrome = %x", status, syndrome);
  }
  return status;
}
 ...
 *   0 on success, a negative value otherwise.
 */
static int mlx5_devx_cmd_query_nic_vport_context(void *ctx, unsigned int vport,
  ...
  if (status) {
    DRV_LOG(DEBUG,
            "Failed to query NIC vport context, "
            "status %x, syndrome = %x",
            status, syndrome);
    return -1;
  }
  vctx = MLX5_ADDR_OF(query_nic_vport_context_out, out, nic_vport_context);
  attr->vport_inline_mode =
      MLX5_GET(nic_vport_context, vctx, min_wqe_inline_mode);
  return 0;
error:
  rc = (rc > 0) ? -rc : rc;
  return rc;
...
mlx5_devx_cmd_query_hca_vdpa_attr(void *ctx,
  ...
  if (!hcattr) {
    ...
    vdpa_attr->desc_tunnel_offload_type =
        MLX5_GET(virtio_emulation_cap, hcattr, desc_tunnel_offload_type);
    vdpa_attr->eth_frame_offload_type =
        MLX5_GET(virtio_emulation_cap, hcattr, eth_frame_offload_type);
    vdpa_attr->virtio_version_1_0 =
        MLX5_GET(virtio_emulation_cap, hcattr, virtio_version_1_0);
    vdpa_attr->tso_ipv4 = MLX5_GET(virtio_emulation_cap, hcattr, tso_ipv4);
    vdpa_attr->tso_ipv6 = MLX5_GET(virtio_emulation_cap, hcattr, tso_ipv6);
    vdpa_attr->tx_csum = MLX5_GET(virtio_emulation_cap, hcattr, tx_csum);
    vdpa_attr->rx_csum = MLX5_GET(virtio_emulation_cap, hcattr, rx_csum);
    vdpa_attr->event_mode = MLX5_GET(virtio_emulation_cap, hcattr, event_mode);
    vdpa_attr->virtio_queue_type =
        MLX5_GET(virtio_emulation_cap, hcattr, virtio_queue_type);
    vdpa_attr->log_doorbell_stride =
        MLX5_GET(virtio_emulation_cap, hcattr, log_doorbell_stride);
    vdpa_attr->log_doorbell_bar_size =
        MLX5_GET(virtio_emulation_cap, hcattr, log_doorbell_bar_size);
    vdpa_attr->doorbell_bar_offset =
        MLX5_GET64(virtio_emulation_cap, hcattr, doorbell_bar_offset);
    vdpa_attr->max_num_virtio_queues =
        MLX5_GET(virtio_emulation_cap, hcattr, max_num_virtio_queues);
    vdpa_attr->umems[0].a =
        MLX5_GET(virtio_emulation_cap, hcattr, umem_1_buffer_param_a);
    vdpa_attr->umems[0].b =
        MLX5_GET(virtio_emulation_cap, hcattr, umem_1_buffer_param_b);
    vdpa_attr->umems[1].a =
        MLX5_GET(virtio_emulation_cap, hcattr, umem_2_buffer_param_a);
    vdpa_attr->umems[1].b =
        MLX5_GET(virtio_emulation_cap, hcattr, umem_2_buffer_param_b);
    vdpa_attr->umems[2].a =
        MLX5_GET(virtio_emulation_cap, hcattr, umem_3_buffer_param_a);
    vdpa_attr->umems[2].b =
        MLX5_GET(virtio_emulation_cap, hcattr, umem_3_buffer_param_b);
  }
}
...
int mlx5_devx_cmd_query_parse_samples(struct mlx5_devx_obj *flex_obj,
  ...
  ret = mlx5_glue->devx_obj_query(flex_obj->obj, in, sizeof(in), out,
                                  sizeof(out));
  if (ret) {
    rte_errno = ret;
  ...
  for (i = 0; i < MLX5_GRAPH_NODE_SAMPLE_NUM; i++) {
    void *s_off = (void *)((char *)sample +
                           i * MLX5_ST_SZ_BYTES(parse_graph_flow_match_sample));
    uint32_t en;

    en = MLX5_GET(parse_graph_flow_match_sample, s_off, flow_match_sample_en);
    if (!en)
      continue;
...
mlx5_devx_cmd_create_flex_parser(void *ctx,
  ...
  struct mlx5_devx_obj *parse_flex_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*parse_flex_obj), 0, SOCKET_ID_ANY);
  uint32_t i;
  ...
  for (i = 0; i < MLX5_GRAPH_NODE_SAMPLE_NUM; i++) {
    ...
    if (!s->flow_match_sample_en)
      continue;
    MLX5_SET(parse_graph_flow_match_sample, s_off, flow_match_sample_en,
             !!s->flow_match_sample_en);
    MLX5_SET(parse_graph_flow_match_sample, s_off,
             flow_match_sample_field_offset, s->flow_match_sample_field_offset);
    MLX5_SET(parse_graph_flow_match_sample, s_off,
             flow_match_sample_offset_mode, s->flow_match_sample_offset_mode);
    MLX5_SET(parse_graph_flow_match_sample, s_off,
    ...
    MLX5_SET(parse_graph_flow_match_sample, s_off,
             flow_match_sample_tunnel_mode, s->flow_match_sample_tunnel_mode);
  }
  for (i = 0; i < MLX5_GRAPH_NODE_ARC_NUM; i++) {
    ...
    void *in_off =
        (void *)((char *)in_arc + i * MLX5_ST_SZ_BYTES(parse_graph_arc));
    void *out_off =
        (void *)((char *)out_arc + i * MLX5_ST_SZ_BYTES(parse_graph_arc));

    if (ia->arc_parse_graph_node != 0) {
      MLX5_SET(parse_graph_arc, in_off, compare_condition_value,
    ...
    if (oa->arc_parse_graph_node != 0) {
      MLX5_SET(parse_graph_arc, out_off, compare_condition_value,
  ...
  parse_flex_obj->obj =
      mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
...
static int
mlx5_devx_cmd_query_hca_parse_graph_node_cap(void *ctx,
  ...
  attr->header_length_mode =
      MLX5_GET(parse_graph_node_cap, hcattr, header_length_mode);
  attr->sample_offset_mode =
      MLX5_GET(parse_graph_node_cap, hcattr, sample_offset_mode);
  attr->max_num_arc_in = MLX5_GET(parse_graph_node_cap, hcattr, max_num_arc_in);
  attr->max_num_arc_out =
      MLX5_GET(parse_graph_node_cap, hcattr, max_num_arc_out);
  attr->max_num_sample = MLX5_GET(parse_graph_node_cap, hcattr, max_num_sample);
  attr->sample_id_in_out =
      MLX5_GET(parse_graph_node_cap, hcattr, sample_id_in_out);
  attr->max_base_header_length =
      MLX5_GET(parse_graph_node_cap, hcattr, max_base_header_length);
  attr->max_sample_base_offset =
      MLX5_GET(parse_graph_node_cap, hcattr, max_sample_base_offset);
  attr->max_next_header_offset =
      MLX5_GET(parse_graph_node_cap, hcattr, max_next_header_offset);
  attr->header_length_mask_width =
      MLX5_GET(parse_graph_node_cap, hcattr, header_length_mask_width);
...
static int mlx5_devx_query_pkt_integrity_match(void *hcattr) {
 ...
 *   0 on success, a negative value otherwise.
 */
int mlx5_devx_cmd_query_hca_attr(void *ctx, struct mlx5_hca_attr *attr) {
  ...
  attr->flow_counters_dump = MLX5_GET(cmd_hca_cap, hcattr, flow_counters_dump);
  attr->log_max_rmp = MLX5_GET(cmd_hca_cap, hcattr, log_max_rmp);
  attr->mem_rq_rmp = MLX5_GET(cmd_hca_cap, hcattr, mem_rq_rmp);
  attr->log_max_rqt_size = MLX5_GET(cmd_hca_cap, hcattr, log_max_rqt_size);
  attr->eswitch_manager = MLX5_GET(cmd_hca_cap, hcattr, eswitch_manager);
  attr->hairpin = MLX5_GET(cmd_hca_cap, hcattr, hairpin);
  attr->log_max_hairpin_queues =
      MLX5_GET(cmd_hca_cap, hcattr, log_max_hairpin_queues);
  attr->log_max_hairpin_wq_data_sz =
      MLX5_GET(cmd_hca_cap, hcattr, log_max_hairpin_wq_data_sz);
  attr->log_max_hairpin_num_packets =
      MLX5_GET(cmd_hca_cap, hcattr, log_min_hairpin_wq_data_sz);
  attr->vhca_id = MLX5_GET(cmd_hca_cap, hcattr, vhca_id);
  attr->relaxed_ordering_write =
      MLX5_GET(cmd_hca_cap, hcattr, relaxed_ordering_write);
  attr->relaxed_ordering_read =
      MLX5_GET(cmd_hca_cap, hcattr, relaxed_ordering_read);
  attr->access_register_user =
      MLX5_GET(cmd_hca_cap, hcattr, access_register_user);
  attr->eth_net_offloads = MLX5_GET(cmd_hca_cap, hcattr, eth_net_offloads);
  attr->eth_virt = MLX5_GET(cmd_hca_cap, hcattr, eth_virt);
  attr->flex_parser_protocols =
      MLX5_GET(cmd_hca_cap, hcattr, flex_parser_protocols);
  attr->max_geneve_tlv_options =
      MLX5_GET(cmd_hca_cap, hcattr, max_geneve_tlv_options);
  attr->max_geneve_tlv_option_data_len =
      MLX5_GET(cmd_hca_cap, hcattr, max_geneve_tlv_option_data_len);
  attr->qos.sup = MLX5_GET(cmd_hca_cap, hcattr, qos);
  attr->qos.flow_meter_aso_sup =
      !!(MLX5_GET64(cmd_hca_cap, hcattr, general_obj_types) &
         MLX5_GENERAL_OBJ_TYPES_CAP_FLOW_METER_ASO);
  attr->vdpa.valid = !!(MLX5_GET64(cmd_hca_cap, hcattr, general_obj_types) &
  ...
  attr->log_max_static_sq_wq =
      MLX5_GET(cmd_hca_cap, hcattr, log_max_static_sq_wq);
  attr->num_lag_ports = MLX5_GET(cmd_hca_cap, hcattr, num_lag_ports);
  attr->dev_freq_khz = MLX5_GET(cmd_hca_cap, hcattr, device_frequency_khz);
  ...
  attr->regexp_num_of_engines =
      MLX5_GET(cmd_hca_cap, hcattr, regexp_num_of_engines);
  /* Read the general_obj_types bitmap and extract the relevant bits. */
  general_obj_types_supported =
      MLX5_GET64(cmd_hca_cap, hcattr, general_obj_types);
  attr->vdpa.valid =
      !!(general_obj_types_supported & MLX5_GENERAL_OBJ_TYPES_CAP_VIRTQ_NET_Q);
  attr->vdpa.queue_counters_valid =
      !!(general_obj_types_supported &
         MLX5_GENERAL_OBJ_TYPES_CAP_VIRTIO_Q_COUNTERS);
  attr->parse_graph_flex_node =
      !!(general_obj_types_supported &
         MLX5_GENERAL_OBJ_TYPES_CAP_PARSE_GRAPH_FLEX_NODE);
  attr->flow_hit_aso =
      !!(general_obj_types_supported & MLX5_GENERAL_OBJ_TYPES_CAP_FLOW_HIT_ASO);
  ...
  attr->import_kek =
      !!(general_obj_types_supported & MLX5_GENERAL_OBJ_TYPES_CAP_IMPORT_KEK);
  attr->credential =
      !!(general_obj_types_supported & MLX5_GENERAL_OBJ_TYPES_CAP_CREDENTIAL);
  attr->crypto_login =
      !!(general_obj_types_supported & MLX5_GENERAL_OBJ_TYPES_CAP_CRYPTO_LOGIN);
  ...
  attr->reg_c_preserve = MLX5_GET(cmd_hca_cap, hcattr, reg_c_preserve);
  ...
  attr->compress_min_block_size =
      MLX5_GET(cmd_hca_cap, hcattr, compress_min_block_size);
  attr->log_max_mmo_dma = MLX5_GET(cmd_hca_cap, hcattr, log_dma_mmo_size);
  attr->log_max_mmo_compress =
      MLX5_GET(cmd_hca_cap, hcattr, log_compress_mmo_size);
  attr->log_max_mmo_decompress =
      MLX5_GET(cmd_hca_cap, hcattr, log_decompress_mmo_size);
  attr->cqe_compression = MLX5_GET(cmd_hca_cap, hcattr, cqe_compression);
  attr->mini_cqe_resp_flow_tag =
      MLX5_GET(cmd_hca_cap, hcattr, mini_cqe_resp_flow_tag);
  attr->mini_cqe_resp_l3_l4_tag =
      MLX5_GET(cmd_hca_cap, hcattr, mini_cqe_resp_l3_l4_tag);
  ...
  attr->ct_offload = !!(MLX5_GET64(cmd_hca_cap, hcattr, general_obj_types) &
  ...
  if (hca_cap_2_sup) {
    hcattr = mlx5_devx_get_hca_cap(ctx, in, out, &rc,
                                   MLX5_GET_HCA_CAP_OP_MOD_GENERAL_DEVICE_2 |
                                       MLX5_HCA_CAP_OPMOD_GET_CUR);
    if (!hcattr) {
      DRV_LOG(DEBUG, "Failed to query DevX HCA capabilities 2.");
      return rc;
    }
    attr->log_min_stride_wqe_sz =
        MLX5_GET(cmd_hca_cap_2, hcattr, log_min_stride_wqe_sz);
  }
  ...
  if (attr->qos.sup) {
    ...
    attr->qos.flow_meter_old = MLX5_GET(qos_cap, hcattr, flow_meter_old);
    ...
    attr->qos.flow_meter = MLX5_GET(qos_cap, hcattr, flow_meter);
    attr->qos.packet_pacing = MLX5_GET(qos_cap, hcattr, packet_pacing);
    attr->qos.wqe_rate_pp = MLX5_GET(qos_cap, hcattr, wqe_rate_pp);
    if (attr->qos.flow_meter_aso_sup) {
      attr->qos.log_meter_aso_granularity =
          MLX5_GET(qos_cap, hcattr, log_meter_aso_granularity);
      attr->qos.log_meter_aso_max_alloc =
          MLX5_GET(qos_cap, hcattr, log_meter_aso_max_alloc);
      attr->qos.log_max_num_meter_aso =
          MLX5_GET(qos_cap, hcattr, log_max_num_meter_aso);
    }
  }
  /*
  ...
  attr->flow.tunnel_header_0_1 =
      MLX5_GET(flow_table_nic_cap, hcattr,
               ft_field_support_2_nic_receive.tunnel_header_0_1);
  attr->flow.tunnel_header_2_3 =
      MLX5_GET(flow_table_nic_cap, hcattr,
  ...
  attr->wqe_vlan_insert =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, wqe_vlan_insert);
  attr->csum_cap =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, csum_cap);
  attr->vlan_cap =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, vlan_cap);
  attr->lro_cap =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, lro_cap);
  attr->max_lso_cap =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, max_lso_cap);
  attr->scatter_fcs =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, scatter_fcs);
  attr->tunnel_lro_gre =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, tunnel_lro_gre);
  attr->tunnel_lro_vxlan =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, tunnel_lro_vxlan);
  attr->swp = MLX5_GET(per_protocol_networking_offload_caps, hcattr, swp);
  attr->tunnel_stateless_gre = MLX5_GET(per_protocol_networking_offload_caps,
                                        hcattr, tunnel_stateless_gre);
  attr->tunnel_stateless_vxlan = MLX5_GET(per_protocol_networking_offload_caps,
                                          hcattr, tunnel_stateless_vxlan);
  attr->swp_csum =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, swp_csum);
  attr->swp_lso =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, swp_lso);
  attr->lro_max_msg_sz_mode = MLX5_GET(per_protocol_networking_offload_caps,
  ...
  attr->lro_min_mss_size =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, lro_min_mss_size);
  attr->tunnel_stateless_geneve_rx = MLX5_GET(
      per_protocol_networking_offload_caps, hcattr, tunnel_stateless_geneve_rx);
  attr->geneve_max_opt_len = MLX5_GET(per_protocol_networking_offload_caps,
                                      hcattr, max_geneve_opt_len);
  attr->wqe_inline_mode =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, wqe_inline_mode);
  attr->tunnel_stateless_gtp = MLX5_GET(per_protocol_networking_offload_caps,
                                        hcattr, tunnel_stateless_gtp);
  attr->rss_ind_tbl_cap =
      MLX5_GET(per_protocol_networking_offload_caps, hcattr, rss_ind_tbl_cap);
  ...
  if (attr->roce) {
    hcattr = mlx5_devx_get_hca_cap(ctx, in, out, &rc,
                                   MLX5_GET_HCA_CAP_OP_MOD_ROCE |
                                       MLX5_HCA_CAP_OPMOD_GET_CUR);
    if (!hcattr) {
      DRV_LOG(DEBUG, "Failed to query devx HCA ROCE capabilities");
      return rc;
    }
  ...
  if (attr->eswitch_manager) {
    ...
    attr->esw_mgr_vport_id_valid =
        MLX5_GET(esw_cap, hcattr, esw_manager_vport_number_valid);
 ...
 */
static void devx_cmd_fill_wq_data(void *wq_ctx,
 ...
 */
struct mlx5_devx_obj *
mlx5_devx_cmd_create_rq(void *ctx, struct mlx5_devx_create_rq_attr *rq_attr,
                        int socket) {
  ...
  rq->obj = mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
  if (!rq->obj) {
...
int mlx5_devx_cmd_modify_rq(struct mlx5_devx_obj *rq,
  ...
  MLX5_SET(rqc, rq_ctx, state, rq_attr->state);
  if (rq_attr->modify_bitmask & MLX5_MODIFY_RQ_IN_MODIFY_BITMASK_SCATTER_FCS)
    MLX5_SET(rqc, rq_ctx, scatter_fcs, rq_attr->scatter_fcs);
  ...
  ret = mlx5_glue->devx_obj_modify(rq->obj, in, sizeof(in), out, sizeof(out));
  if (ret) {
 ...
 */
struct mlx5_devx_obj *
mlx5_devx_cmd_create_rmp(void *ctx, struct mlx5_devx_create_rmp_attr *rmp_attr,
                         int socket) {
  ...
  devx_cmd_fill_wq_data(wq_ctx, wq_attr);
  rmp->obj = mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
  if (!rmp->obj) {
 ...
 */
struct mlx5_devx_obj *
mlx5_devx_cmd_create_tir(void *ctx, struct mlx5_devx_tir_attr *tir_attr) {
  ...
  MLX5_SET(tirc, tir_ctx, tunneled_offload_en, tir_attr->tunneled_offload_en);
  ...
  tir->obj = mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
  if (!tir->obj) {
...
int mlx5_devx_cmd_modify_tir(
  ...
  if (modify_tir_attr->modify_bitmask & MLX5_MODIFY_TIR_IN_MODIFY_BITMASK_LRO) {
    MLX5_SET(tirc, tir_ctx, lro_timeout_period_usecs,
             tir_attr->lro_timeout_period_usecs);
    MLX5_SET(tirc, tir_ctx, lro_enable_mask, tir_attr->lro_enable_mask);
    MLX5_SET(tirc, tir_ctx, lro_max_msg_sz, tir_attr->lro_max_msg_sz);
  ...
  ret = mlx5_glue->devx_obj_modify(tir->obj, in, sizeof(in), out, sizeof(out));
  if (ret) {
 ...
 */
struct mlx5_devx_obj *
mlx5_devx_cmd_create_rqt(void *ctx, struct mlx5_devx_rqt_attr *rqt_attr) {
  uint32_t *in = NULL;
 ...
 **/
struct mlx5_devx_obj *
mlx5_devx_cmd_create_sq(void *ctx, struct mlx5_devx_create_sq_attr *sq_attr) {
  ...
  MLX5_SET(sqc, sq_ctx, min_wqe_inline_mode, sq_attr->min_wqe_inline_mode);
  ...
  devx_cmd_fill_wq_data(wq_ctx, wq_attr);
  sq->obj = mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
  if (!sq->obj) {
...
int mlx5_devx_cmd_modify_sq(struct mlx5_devx_obj *sq,
  ...
  ret = mlx5_glue->devx_obj_modify(sq->obj, in, sizeof(in), out, sizeof(out));
  if (ret) {
 ...
 */
struct mlx5_devx_obj *
mlx5_devx_cmd_create_tis(void *ctx, struct mlx5_devx_tis_attr *tis_attr) {
  ...
  MLX5_SET(tisc, tis_ctx, lag_tx_port_affinity, tis_attr->lag_tx_port_affinity);
  MLX5_SET(tisc, tis_ctx, prio, tis_attr->prio);
  MLX5_SET(tisc, tis_ctx, transport_domain, tis_attr->transport_domain);
  tis->obj = mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
  if (!tis->obj) {
...
struct mlx5_devx_obj *mlx5_devx_cmd_create_td(void *ctx) {
  ...
  td->obj = mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
  if (!td->obj) {
    DRV_LOG(ERR, "Failed to create TIS using DevX");
    rte_errno = errno;
    mlx5_free(td);
  ...
  td->id = MLX5_GET(alloc_transport_domain_out, out, transport_domain);
  return td;
}
 ...
 *   0 on success, a negative value otherwise.
 */
int mlx5_devx_cmd_flow_dump(void *fdb_domain __rte_unused,
                            void *rx_domain __rte_unused,
                            void *tx_domain __rte_unused,
...
int mlx5_devx_cmd_flow_single_dump(void *rule_info __rte_unused,
                                   FILE *file __rte_unused) {
  int ret = 0;
...
struct mlx5_devx_obj *mlx5_devx_cmd_create_cq(void *ctx,
  ...
  struct mlx5_devx_obj *cq_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*cq_obj), 0, SOCKET_ID_ANY);
  ...
  MLX5_SET(cqc, cqctx, cqe_sz,
           (RTE_CACHE_LINE_SIZE == 128) ? MLX5_CQE_SIZE_128B
                                        : MLX5_CQE_SIZE_64B);
  ...
  MLX5_SET(cqc, cqctx, mini_cqe_res_format_ext, attr->mini_cqe_res_format_ext);
  if (attr->q_umem_valid) {
    ...
    MLX5_SET64(create_cq_in, in, cq_umem_offset, attr->q_umem_offset);
  }
  cq_obj->obj =
      mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
  if (!cq_obj->obj) {
 ...
 */
struct mlx5_devx_obj *
mlx5_devx_cmd_create_virtq(void *ctx, struct mlx5_devx_virtq_attr *attr) {
  uint32_t in[MLX5_ST_SZ_DW(create_virtq_in)] = {0};
  uint32_t out[MLX5_ST_SZ_DW(general_obj_out_cmd_hdr)] = {0};
  struct mlx5_devx_obj *virtq_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*virtq_obj), 0, SOCKET_ID_ANY);
  ...
  MLX5_SET(general_obj_in_cmd_hdr, hdr, opcode,
           MLX5_CMD_OP_CREATE_GENERAL_OBJECT);
  MLX5_SET(general_obj_in_cmd_hdr, hdr, obj_type, MLX5_GENERAL_OBJ_TYPE_VIRTQ);
  MLX5_SET16(virtio_net_q, virtq, hw_available_index, attr->hw_available_index);
  ...
  MLX5_SET16(virtio_q, virtctx, virtio_version_1_0, attr->virtio_version_1_0);
...
int mlx5_devx_cmd_modify_virtq(struct mlx5_devx_obj *virtq_obj,
  ...
  case MLX5_VIRTQ_MODIFY_TYPE_DIRTY_BITMAP_PARAMS:
    MLX5_SET(virtio_net_q, virtq, dirty_bitmap_mkey, attr->dirty_bitmap_mkey);
    MLX5_SET64(virtio_net_q, virtq, dirty_bitmap_addr, attr->dirty_bitmap_addr);
    MLX5_SET(virtio_net_q, virtq, dirty_bitmap_size, attr->dirty_bitmap_size);
    break;
  ...
  ret = mlx5_glue->devx_obj_modify(virtq_obj->obj, in, sizeof(in), out,
                                   sizeof(out));
  if (ret) {
...
int mlx5_devx_cmd_query_virtq(struct mlx5_devx_obj *virtq_obj,
  ...
  MLX5_SET(general_obj_in_cmd_hdr, hdr, obj_type, MLX5_GENERAL_OBJ_TYPE_VIRTQ);
  MLX5_SET(general_obj_in_cmd_hdr, hdr, obj_id, virtq_obj->id);
  ret = mlx5_glue->devx_obj_query(virtq_obj->obj, in, sizeof(in), out,
                                  sizeof(out));
  if (ret) {
    DRV_LOG(ERR, "Failed to modify VIRTQ using DevX.");
    rte_errno = errno;
...
struct mlx5_devx_obj *mlx5_devx_cmd_create_qp(void *ctx,
  ...
  struct mlx5_devx_obj *qp_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*qp_obj), 0, SOCKET_ID_ANY);
  ...
  if (attr->uar_index) {
    if (attr->mmo) {
      void *qpc_ext_and_pas_list =
          MLX5_ADDR_OF(create_qp_in, in, qpc_extension_and_pas_list);
    ...
    if (attr->num_of_receive_wqes) {
      ...
      MLX5_SET(qpc, qpc, log_rq_stride,
               attr->log_rq_stride - MLX5_LOG_RQ_STRIDE_SHIFT);
...
int mlx5_devx_cmd_modify_qp_state(struct mlx5_devx_obj *qp,
  ...
  case MLX5_CMD_OP_RTR2RTS_QP:
    ...
    break;
  default:
    DRV_LOG(ERR, "Invalid or unsupported QP modify op %u.", qp_st_mod_op);
    rte_errno = EINVAL;
    return -rte_errno;
  }
...
struct mlx5_devx_obj *mlx5_devx_cmd_create_virtio_q_counters(void *ctx) {
  ...
  uint32_t out[MLX5_ST_SZ_DW(general_obj_out_cmd_hdr)] = {0};
  struct mlx5_devx_obj *couners_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*couners_obj), 0, SOCKET_ID_ANY);
...
int mlx5_devx_cmd_query_virtio_q_counters(
  ...
  void *virtio_q_counters =
      MLX5_ADDR_OF(query_virtio_q_counters_out, out, virtio_q_counters);
  int ret;
  ...
  attr->received_desc =
      MLX5_GET64(virtio_q_counters, virtio_q_counters, received_desc);
  attr->completed_desc =
      MLX5_GET64(virtio_q_counters, virtio_q_counters, completed_desc);
  attr->error_cqes = MLX5_GET(virtio_q_counters, virtio_q_counters, error_cqes);
  attr->bad_desc_errors =
      MLX5_GET(virtio_q_counters, virtio_q_counters, bad_desc_errors);
  attr->exceed_max_chain =
      MLX5_GET(virtio_q_counters, virtio_q_counters, exceed_max_chain);
  attr->invalid_buffer =
      MLX5_GET(virtio_q_counters, virtio_q_counters, invalid_buffer);
  return ret;
}
...
struct mlx5_devx_obj *mlx5_devx_cmd_create_flow_hit_aso_obj(void *ctx,
  ...
  flow_hit_aso_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*flow_hit_aso_obj), 0, SOCKET_ID_ANY);
  if (!flow_hit_aso_obj) {
...
struct mlx5_devx_obj *mlx5_devx_cmd_alloc_pd(void *ctx) {
  ...
  ppd->obj = mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
  if (!ppd->obj) {
    mlx5_free(ppd);
...
mlx5_devx_cmd_create_flow_meter_aso_obj(void *ctx, uint32_t pd,
  ...
  flow_meter_aso_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*flow_meter_aso_obj), 0, SOCKET_ID_ANY);
  ...
  flow_meter_aso_obj->id = MLX5_GET(general_obj_out_cmd_hdr, out, obj_id);
...
mlx5_devx_cmd_create_conn_track_offload_obj(void *ctx, uint32_t pd,
  ...
  ct_aso_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*ct_aso_obj), 0, SOCKET_ID_ANY);
  if (!ct_aso_obj) {
 ...
 *   The DevX object created, NULL otherwise and rte_errno is set.
 */
struct mlx5_devx_obj *mlx5_devx_cmd_create_geneve_tlv_option(void *ctx,
                                                             uint16_t class,
                                                             uint8_t type,
                                                             uint8_t len) {
  ...
  struct mlx5_devx_obj *geneve_tlv_opt_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*geneve_tlv_opt_obj), 0, SOCKET_ID_ANY);
  ...
  void *opt = MLX5_ADDR_OF(create_geneve_tlv_option_in, in, geneve_tlv_opt);
  MLX5_SET(general_obj_in_cmd_hdr, hdr, opcode,
           MLX5_CMD_OP_CREATE_GENERAL_OBJECT);
  MLX5_SET(general_obj_in_cmd_hdr, hdr, obj_type,
           MLX5_GENERAL_OBJ_TYPE_GENEVE_TLV_OPT);
  MLX5_SET(geneve_tlv_option, opt, option_class, rte_be_to_cpu_16(class));
  MLX5_SET(geneve_tlv_option, opt, option_type, type);
  MLX5_SET(geneve_tlv_option, opt, option_data_length, len);
  geneve_tlv_opt_obj->obj =
      mlx5_glue->devx_obj_create(ctx, in, sizeof(in), out, sizeof(out));
...
int mlx5_devx_cmd_wq_query(void *wq, uint32_t *counter_set_id) {
  ...
  if (rc) {
    rte_errno = errno;
    DRV_LOG(ERR,
            "Failed to query WQ counter set ID using DevX - "
            "rc = %d, errno = %d.",
            rc, errno);
    return -rc;
  };
...
struct mlx5_devx_obj *mlx5_devx_cmd_queue_counter_alloc(void *ctx) {
  ...
  if (!dcs->obj) {
    DRV_LOG(DEBUG,
            "Can't allocate q counter set by DevX - error "
            "%d.",
            errno);
    rte_errno = errno;
    mlx5_free(dcs);
...
mlx5_devx_cmd_create_dek_obj(void *ctx, struct mlx5_devx_dek_attr *attr) {
  ...
  dek_obj = mlx5_malloc(MLX5_MEM_ZERO, sizeof(*dek_obj), 0, SOCKET_ID_ANY);
  if (dek_obj == NULL) {
  ...
  MLX5_SET(general_obj_in_cmd_hdr, ptr, opcode,
           MLX5_CMD_OP_CREATE_GENERAL_OBJECT);
  MLX5_SET(general_obj_in_cmd_hdr, ptr, obj_type, MLX5_GENERAL_OBJ_TYPE_DEK);
...
mlx5_devx_cmd_create_import_kek_obj(void *ctx,
  ...
  import_kek_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*import_kek_obj), 0, SOCKET_ID_ANY);
...
mlx5_devx_cmd_create_credential_obj(void *ctx,
  ...
  credential_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*credential_obj), 0, SOCKET_ID_ANY);
 ...
 *   The DevX object created, NULL otherwise and rte_errno is set.
 */
struct mlx5_devx_obj *mlx5_devx_cmd_create_crypto_login_obj(
    void *ctx, struct mlx5_devx_crypto_login_attr *attr) {
  ...
  crypto_login_obj =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*crypto_login_obj), 0, SOCKET_ID_ANY);
  ...
  MLX5_SET(crypto_login, ptr, credential_pointer, attr->credential_pointer);
 ...
 *   0 on success, a negative value otherwise.
 */
int mlx5_devx_cmd_query_lag(void *ctx, struct mlx5_devx_lag_context *lag_ctx) {
  ...
  lag_ctx->fdb_selection_mode = MLX5_GET(lag_context, lctx, fdb_selection_mode);
  lag_ctx->port_select_mode = MLX5_GET(lag_context, lctx, port_select_mode);
  lag_ctx->lag_state = MLX5_GET(lag_context, lctx, lag_state);
  lag_ctx->tx_remap_affinity_2 =
      MLX5_GET(lag_context, lctx, tx_remap_affinity_2);
  lag_ctx->tx_remap_affinity_1 =
      MLX5_GET(lag_context, lctx, tx_remap_affinity_1);
  return 0;
error:
  rc = (rc > 0) ? -rc : rc;
  return rc;
