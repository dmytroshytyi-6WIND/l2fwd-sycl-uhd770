 ...
 * segments.
 */
#define MLX5_EMPW_DEF_INLINE_LEN                                               \
  (4u * MLX5_WQE_SIZE + MLX5_DSEG_MIN_INLINE_SIZE)
/*
 * Maximal inline data length sent with enhanced MPW.
 * Is based on maximal WQE size.
 */
#define MLX5_EMPW_MAX_INLINE_LEN                                               \
  (MLX5_WQE_SIZE_MAX - MLX5_WQE_CSEG_SIZE - MLX5_WQE_ESEG_SIZE -               \
   MLX5_WQE_DSEG_SIZE + MLX5_DSEG_MIN_INLINE_SIZE)
/*
 ...
 * CPU cycles.
 */
#define MLX5_SEND_DEF_INLINE_LEN                                               \
  (5U * MLX5_WQE_SIZE + MLX5_ESEG_MIN_INLINE_SIZE - MLX5_WQE_CSEG_SIZE -       \
   MLX5_WQE_ESEG_SIZE - MLX5_WQE_DSEG_SIZE)
/*
 * Maximal inline data length sent with ordinary SEND.
 * Is based on maximal WQE size.
 */
#define MLX5_SEND_MAX_INLINE_LEN                                               \
  (MLX5_WQE_SIZE_MAX - MLX5_WQE_CSEG_SIZE - MLX5_WQE_ESEG_SIZE -               \
   MLX5_WQE_DSEG_SIZE + MLX5_ESEG_MIN_INLINE_SIZE)
...
#define __mlx5_bit_off(typ, fld)                                               \
  ((unsigned int)(uintptr_t)(&(__mlx5_nullp(typ)->fld)))
...
#define __mlx5_16_bit_off(typ, fld)                                            \
  (16 - __mlx5_bit_sz(typ, fld) - (__mlx5_bit_off(typ, fld) & 0xf))
#define __mlx5_mask16(typ, fld) ((u16)((1ull << __mlx5_bit_sz(typ, fld)) - 1))
#define __mlx5_16_mask(typ, fld)                                               \
  (__mlx5_mask16(typ, fld) << __mlx5_16_bit_off(typ, fld))
...
#define MLX5_SET(typ, p, fld, v)                                               \
  do {                                                                         \
    u32 _v = v;                                                                \
    *((rte_be32_t *)(p) + __mlx5_dw_off(typ, fld)) = rte_cpu_to_be_32(         \
        (rte_be_to_cpu_32(*((u32 *)(p) + __mlx5_dw_off(typ, fld))) &           \
         (~__mlx5_dw_mask(typ, fld))) |                                        \
        (((_v)&__mlx5_mask(typ, fld)) << __mlx5_dw_bit_off(typ, fld)));        \
  } while (0)

#define MLX5_SET64(typ, p, fld, v)                                             \
  do {                                                                         \
    MLX5_ASSERT(__mlx5_bit_sz(typ, fld) == 64);                                \
    *((rte_be64_t *)(p) + __mlx5_64_off(typ, fld)) = rte_cpu_to_be_64(v);      \
  } while (0)

#define MLX5_SET16(typ, p, fld, v)                                             \
  do {                                                                         \
    u16 _v = v;                                                                \
    *((rte_be16_t *)(p) + __mlx5_16_off(typ, fld)) = rte_cpu_to_be_16(         \
        (rte_be_to_cpu_16(*((rte_be16_t *)(p) + __mlx5_16_off(typ, fld))) &    \
         (~__mlx5_16_mask(typ, fld))) |                                        \
        (((_v)&__mlx5_mask16(typ, fld)) << __mlx5_16_bit_off(typ, fld)));      \
  } while (0)

#define MLX5_GET_VOLATILE(typ, p, fld)                                         \
  ((rte_be_to_cpu_32(*((volatile __be32 *)(p) + __mlx5_dw_off(typ, fld))) >>   \
    __mlx5_dw_bit_off(typ, fld)) &                                             \
   __mlx5_mask(typ, fld))
#define MLX5_GET(typ, p, fld)                                                  \
  ((rte_be_to_cpu_32(*((rte_be32_t *)(p) + __mlx5_dw_off(typ, fld))) >>        \
    __mlx5_dw_bit_off(typ, fld)) &                                             \
   __mlx5_mask(typ, fld))
...
#define MLX5_GET64(typ, p, fld)                                                \
  rte_be_to_cpu_64(*((rte_be64_t *)(p) + __mlx5_64_off(typ, fld)))
...
#define MLX5_GENERAL_OBJ_TYPES_CAP_DEK (1ULL << MLX5_GENERAL_OBJ_TYPE_DEK)
...
struct mlx5_ifc_flow_table_nic_cap_bits {
  ...
  struct mlx5_ifc_ft_fields_support_2_bits ft_field_support_2_nic_receive;
};
 ...
 */
#define MLX5_IFC_LAG_MAP_TIS_AFFINITY(index, num)                              \
  ((num) ? (index) % (num) + 1 : 0)
 ...
 */
static inline uint32_t mlx5_flow_mark_set(uint32_t val) {
  uint32_t ret;
 ...
 *   Converted mark value.
 */
static inline uint32_t mlx5_flow_mark_get(uint32_t val) {
  /*
 ...
 */
static inline uint32_t mlx5_ts_format_conv(uint32_t ts_format) {
