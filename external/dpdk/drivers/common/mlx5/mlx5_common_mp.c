...
int mlx5_mp_req_mempool_reg(struct mlx5_common_device *cdev,
  ...
  MLX5_ASSERT(rte_eal_process_type() == RTE_PROC_SECONDARY);
  type = reg ? MLX5_MP_REQ_MEMPOOL_REGISTER : MLX5_MP_REQ_MEMPOOL_UNREGISTER;
  ...
  ret = rte_mp_request_sync(&mp_req, &mp_rep, &ts);
  if (ret) {
    DRV_LOG(ERR, "Mempool %sregister request to primary process failed.",
            reg ? "" : "un");
    return -rte_errno;
  }
  MLX5_ASSERT(mp_rep.nb_received == 1);
...
int mlx5_mp_req_queue_state_modify(struct mlx5_mp_id *mp_id,
  ...
  ret = rte_mp_request_sync(&mp_req, &mp_rep, &ts);
  if (ret) {
    DRV_LOG(ERR, "port %u request to primary process failed", mp_id->port_id);
    return -rte_errno;
  }
  MLX5_ASSERT(mp_rep.nb_received == 1);
  mp_res = &mp_rep.msgs[0];
...
int mlx5_mp_req_verbs_cmd_fd(struct mlx5_mp_id *mp_id) {
  ...
  if (res->result) {
    ...
    DRV_LOG(ERR, "port %u failed to get command FD from primary process",
            mp_id->port_id);
    ret = -rte_errno;
    goto exit;
  }
  ...
  ret = mp_res->fds[0];
  DRV_LOG(DEBUG, "port %u command FD from primary is %d", mp_id->port_id, ret);
exit:
  mlx5_free(mp_rep.msgs);
  return ret;
}
