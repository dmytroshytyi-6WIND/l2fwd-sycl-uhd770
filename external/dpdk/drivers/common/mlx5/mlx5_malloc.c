 ...
 */
static bool mlx5_mem_check_msl(void *addr, struct rte_memseg_list *msl) {
  void *start, *end;
 ...
 *   Memory address.
 */
static void mlx5_mem_update_msl(void *addr) {
  /*
   * Update the cache msl if the new addr comes from the new msl
   * different with the cached msl.
   */
  if (addr && !mlx5_mem_check_msl(
                  addr, (struct rte_memseg_list *)__atomic_load_n(
                            &mlx5_sys_mem.last_msl, __ATOMIC_RELAXED))) {
    __atomic_store_n(&mlx5_sys_mem.last_msl, rte_mem_virt2memseg_list(addr),
                     __ATOMIC_RELAXED);
#ifdef RTE_LIBRTE_MLX5_DEBUG
    __atomic_add_fetch(&mlx5_sys_mem.msl_update, 1, __ATOMIC_RELAXED);
#endif
  }
}
 ...
 */
static void *mlx5_alloc_align(size_t size, unsigned int align,
                              unsigned int zero) {
  void *buf;

  buf = mlx5_os_malloc(align, size);
  if (!buf) {
    DRV_LOG(ERR, "Couldn't allocate buf size=%zu align=%u.", size, align);
    return NULL;
  }
  if (zero)
    memset(buf, 0, size);
...
#ifdef RTE_LIBRTE_MLX5_DEBUG
    if (addr)
      __atomic_add_fetch(&mlx5_sys_mem.malloc_rte, 1, __ATOMIC_RELAXED);
#endif
    return addr;
  }
...
#ifdef RTE_LIBRTE_MLX5_DEBUG
  if (addr)
    __atomic_add_fetch(&mlx5_sys_mem.malloc_sys, 1, __ATOMIC_RELAXED);
#endif
  return addr;
}
...
#ifdef RTE_LIBRTE_MLX5_DEBUG
    if (new_addr)
      __atomic_add_fetch(&mlx5_sys_mem.realloc_rte, 1, __ATOMIC_RELAXED);
#endif
    return new_addr;
  }
...
#ifdef RTE_LIBRTE_MLX5_DEBUG
  if (new_addr)
    __atomic_add_fetch(&mlx5_sys_mem.realloc_sys, 1, __ATOMIC_RELAXED);
#endif
  return new_addr;
}
...
#ifdef RTE_LIBRTE_MLX5_DEBUG
    __atomic_add_fetch(&mlx5_sys_mem.free_sys, 1, __ATOMIC_RELAXED);
#endif
    mlx5_os_free(addr);
  } else {
#ifdef RTE_LIBRTE_MLX5_DEBUG
    __atomic_add_fetch(&mlx5_sys_mem.free_rte, 1, __ATOMIC_RELAXED);
#endif
    rte_free(addr);
  }
}
...
#ifdef RTE_LIBRTE_MLX5_DEBUG
  DRV_LOG(INFO,
          "System memory malloc:%" PRIi64 ", realloc:%" PRIi64 ","
