...
static inline void mlx5_mp_id_init(struct mlx5_mp_id *mp_id, uint16_t port_id) {
 ...
 *   Message type.
 */
static inline void mp_init_msg(struct mlx5_mp_id *mp_id, struct rte_mp_msg *msg,
                               enum mlx5_mp_req_type type) {
 ...
 *   Message type.
 */
static inline void mp_init_port_agnostic_msg(struct rte_mp_msg *msg,
                                             enum mlx5_mp_req_type type) {
  struct mlx5_mp_param *param = (struct mlx5_mp_param *)msg->param;
...
__rte_internal int mlx5_mp_init_primary(const char *name,
                                        const rte_mp_t primary_action);
__rte_internal void mlx5_mp_uninit_primary(const char *name);
__rte_internal int mlx5_mp_init_secondary(const char *name,
                                          const rte_mp_t secondary_action);
__rte_internal void mlx5_mp_uninit_secondary(const char *name);
__rte_internal int mlx5_mp_req_mr_create(struct mlx5_common_device *cdev,
                                         uintptr_t addr);
__rte_internal int mlx5_mp_req_mempool_reg(struct mlx5_common_device *cdev,
                                           struct rte_mempool *mempool,
                                           bool reg, bool is_extmem);
__rte_internal int
mlx5_mp_req_queue_state_modify(struct mlx5_mp_id *mp_id,
                               struct mlx5_mp_arg_queue_state_modify *sm);
__rte_internal int mlx5_mp_req_verbs_cmd_fd(struct mlx5_mp_id *mp_id);
