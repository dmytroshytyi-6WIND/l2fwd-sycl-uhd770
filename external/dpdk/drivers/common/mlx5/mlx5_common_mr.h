...
__rte_internal void mlx5_mprq_buf_free_cb(void *addr, void *opaque);
 ...
 */
static inline struct rte_mempool *mlx5_mb2mp(struct rte_mbuf *buf) {
...
mlx5_mr_lookup_lkey(struct mr_cache_entry *lkp_tbl, uint16_t *cached_idx,
  ...
  for (idx = 0; idx < n && lkp_tbl[idx].start != 0; ++idx) {
    if (addr >= lkp_tbl[idx].start && addr < lkp_tbl[idx].end) {
      /* Found. */
      *cached_idx = idx;
...
__rte_internal void mlx5_mr_flush_local_cache(struct mlx5_mr_ctrl *mr_ctrl);
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
__rte_internal uint32_t mlx5_mr_mb2mr_bh(struct mlx5_mr_ctrl *mr_ctrl,
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static __rte_always_inline uint32_t mlx5_mr_mb2mr(struct mlx5_mr_ctrl *mr_ctrl,
                                                  struct rte_mbuf *mbuf) {
  uint32_t lkey;
  ...
  lkey = mlx5_mr_lookup_lkey(mr_ctrl->cache, &mr_ctrl->mru, MLX5_MR_CACHE_N,
...
__rte_internal int mlx5_mr_ctrl_init(struct mlx5_mr_ctrl *mr_ctrl,
                                     uint32_t *dev_gen_ptr, int socket);
__rte_internal void mlx5_mr_btree_free(struct mlx5_mr_btree *bt);
void mlx5_mr_btree_dump(struct mlx5_mr_btree *bt __rte_unused);
__rte_internal uint32_t mlx5_mr_mempool2mr_bh(struct mlx5_mr_ctrl *mr_ctrl,
                                              struct rte_mempool *mp,
                                              uintptr_t addr);
...
__rte_internal uint32_t mlx5_mr_create(struct mlx5_common_device *cdev,
...
__rte_internal int mlx5_common_verbs_reg_mr(void *pd, void *addr, size_t length,
                                            struct mlx5_pmd_mr *pmd_mr);
__rte_internal void mlx5_common_verbs_dereg_mr(struct mlx5_pmd_mr *pmd_mr);

void mlx5_os_set_reg_mr_cb(mlx5_reg_mr_t *reg_mr_cb,
                           mlx5_dereg_mr_t *dereg_mr_cb);

__rte_internal int mlx5_mr_mempool_register(struct mlx5_common_device *cdev,
                                            struct rte_mempool *mp,
                                            bool is_extmem);
__rte_internal int mlx5_mr_mempool_unregister(struct mlx5_common_device *cdev,
                                              struct rte_mempool *mp);

__rte_internal int mlx5_mr_mempool_populate_cache(struct mlx5_mr_ctrl *mr_ctrl,
                                                  struct rte_mempool *mp);
