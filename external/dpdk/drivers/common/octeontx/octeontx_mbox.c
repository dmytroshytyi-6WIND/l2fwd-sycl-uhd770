...
static inline void mbox_msgcpy(volatile uint8_t *d, volatile const uint8_t *s,
                               uint16_t size) {
  uint16_t i;
...
static inline void mbox_send_request(struct mbox *m,
...
static inline int mbox_wait_response(struct mbox *m,
                                     struct octeontx_mbox_hdr *hdr, void *rxmsg,
                                     uint16_t rxsize) {
  int res = 0, wait;
  uint16_t len;
...
static inline int mbox_send(struct mbox *m, struct octeontx_mbox_hdr *hdr,
                            const void *txmsg, uint16_t txsize, void *rxmsg,
                            uint16_t rxsize) {
  int res = -EINVAL;

  if (m->init_once == 0 || hdr == NULL || txsize > MAX_RAM_MBOX_LEN ||
      rxsize > MAX_RAM_MBOX_LEN) {
    mbox_log_err("Invalid init_once=%d hdr=%p txsz=%d rxsz=%d", m->init_once,
                 hdr, txsize, rxsize);
    return res;
  }
...
int octeontx_mbox_send(struct octeontx_mbox_hdr *hdr, void *txdata,
...
static int octeontx_start_domain(void) {
...
static int octeontx_check_mbox_version(struct mbox_intf_ver *app_intf_ver,
  ...
  result = octeontx_mbox_send(&hdr, app_intf_ver, sizeof(struct mbox_intf_ver),
...
int octeontx_mbox_init(void) {
  struct mbox_intf_ver MBOX_INTERFACE_VERSION = {
      .platform = 0x01, .major = 0x01, .minor = 0x03};
  ...
  ret = octeontx_check_mbox_version(&MBOX_INTERFACE_VERSION, &rm_intf_ver);
  if (ret < 0) {
    mbox_log_err("MBOX version: Kernel(%d.%d.%d) != DPDK(%d.%d.%d)",
                 rm_intf_ver.platform, rm_intf_ver.major, rm_intf_ver.minor,
