...
static __rte_always_inline void gen_key_snow3g(const uint8_t *ck,
                                               uint32_t *keyx) {
  int i, base;
...
static __rte_always_inline void cpt_fc_salt_update(struct cpt_ctx *cpt_ctx,
                                                   uint8_t *salt) {
  mc_fc_context_t *fctx = &cpt_ctx->mc_ctx.fctx;
  memcpy(fctx->enc.encr_iv, salt, 4);
}

static __rte_always_inline int cpt_fc_ciph_validate_key_aes(uint16_t key_len) {
  switch (key_len) {
  case 16:
  case 24:
  case 32:
...
static __rte_always_inline int cpt_fc_ciph_set_type(cipher_type_t type,
                                                    struct cpt_ctx *ctx,
                                                    uint16_t key_len) {
  int fc_type = 0;
  switch (type) {
...
cpt_fc_ciph_set_key_passthrough(struct cpt_ctx *cpt_ctx,
...
static __rte_always_inline uint32_t fill_sg_comp(sg_comp_t *list, uint32_t i,
...
static __rte_always_inline uint32_t fill_sg_comp_from_buf(sg_comp_t *list,
                                                          uint32_t i,
                                                          buf_ptr_t *from) {
  sg_comp_t *to = &list[i >> 2];
...
static __rte_always_inline uint32_t fill_sg_comp_from_buf_min(sg_comp_t *list,
                                                              uint32_t i,
                                                              buf_ptr_t *from,
                                                              uint32_t *psize) {
  sg_comp_t *to = &list[i >> 2];
 ...
 */
static __rte_always_inline uint32_t fill_sg_comp_from_iov(
    sg_comp_t *list, uint32_t i, iov_ptr_t *from, uint32_t from_offset,
    uint32_t *psize, buf_ptr_t *extra_buf, uint32_t extra_offset) {
  int32_t j;
  ...
  for (j = 0; (j < from->buf_cnt) && size; j++) {
    ...
    if (extra_len && (e_len >= extra_offset)) {
      ...
      if (extra_len) {
        ...
        to->ptr[i % 4] = rte_cpu_to_be_64(extra_buf->dma_addr);
        size -= extra_len;
      }
...
static __rte_always_inline void cpt_digest_gen_prep(uint32_t flags,
                                                    uint64_t d_lens,
                                                    digest_params_t *params,
                                                    void *op, void **prep_req) {
  ...
  size = (uint8_t *)RTE_PTR_ALIGN((uint8_t *)m_vaddr, 16) - (uint8_t *)m_vaddr;
  ...
  if (ctx->hmac) {
    ...
    /* Key */
    i = fill_sg_comp(gather_comp, i, k_dma, RTE_ALIGN_CEIL(key_len, 8));
  }
  ...
  if (flags & VALID_MAC_BUF) {
    ...
    size = mac_len;
    i = fill_sg_comp_from_buf_min(scatter_comp, i, &params->mac_buf, &size);
  } else {
    size = mac_len;
    i = fill_sg_comp_from_iov(scatter_comp, i, params->src_iov, data_len, &size,
                              NULL, 0);
    if (unlikely(size)) {
...
cpt_enc_hmac_prep(uint32_t flags, uint64_t d_offs, uint64_t d_lens,
                  fc_params_t *fc_params, void *op, void **prep_req) {
  ...
  size = (uint8_t *)RTE_PTR_ALIGN(m_vaddr, 8) - (uint8_t *)m_vaddr;
  ...
  vq_cmd_w0.s.opcode.minor |= (cpt_ctx->auth_enc << CPT_HMAC_FIRST_BIT_POS);
  ...
  if (unlikely(encr_data_len & 0xf)) {
    ...
    else if (likely((cipher_type == AES_CBC) || (cipher_type == AES_ECB)))
      enc_dlen = RTE_ALIGN_CEIL(encr_data_len, 8) + encr_offset;
  }
  ...
  if (likely((flags & SINGLE_BUF_INPLACE) &&
    ...
    req->alternate_caddr =
        (uint64_t *)((uint8_t *)dm_vaddr + outputlen - iv_len);

    vq_cmd_w0.s.dlen = inputlen + OFF_CTRL_LEN;

    if (likely(iv_len)) {
      uint64_t *dest = (uint64_t *)((uint8_t *)offset_vaddr + OFF_CTRL_LEN);
    ...
    *(uint64_t *)offset_vaddr =
    ...
    if (likely(size)) {
      ...
      if (unlikely(flags & SINGLE_BUF_INPLACE)) {
        i = fill_sg_comp_from_buf_min(gather_comp, i, fc_params->bufs, &size);
      } else {
        i = fill_sg_comp_from_iov(gather_comp, i, fc_params->src_iov, 0, &size,
                                  aad_buf, aad_offset);
      }
    ...
    /* Add IV */
    if (likely(iv_len)) {
      i = fill_sg_comp(scatter_comp, i, offset_dma + OFF_CTRL_LEN, iv_len);
    }
    ...
    if (unlikely(flags & VALID_MAC_BUF)) {
      ...
      if (size) {
        ...
        if (unlikely(flags & SINGLE_BUF_INPLACE)) {
          i = fill_sg_comp_from_buf_min(scatter_comp, i, fc_params->bufs,
                                        &size);
        } else {
          i = fill_sg_comp_from_iov(scatter_comp, i, fc_params->dst_iov, 0,
...
cpt_dec_hmac_prep(uint32_t flags, uint64_t d_offs, uint64_t d_lens,
  ...
  vq_cmd_w0.s.opcode.minor |= (cpt_ctx->dec_auth << CPT_HMAC_FIRST_BIT_POS);
    ...
    *(uint64_t *)offset_vaddr =
    ...
    if (flags & VALID_MAC_BUF) {
      ...
      if (size) {
        ...
        if (unlikely(flags & SINGLE_BUF_INPLACE)) {
          ...
          i = fill_sg_comp_from_iov(gather_comp, i, fc_params->src_iov, 0,
                                    &size, aad_buf, aad_offset);
        }
        if (unlikely(size)) {
          CPT_LOG_DP_ERR("Insufficient buffer"
      ...
      /* mac data */
      if (mac_len) {
        i = fill_sg_comp_from_buf(gather_comp, i, &fc_params->mac_buf);
      }
    } else {
      /* input data + mac */
      size = inputlen - iv_len;
    ...
    if (size) {
      if (unlikely(flags & SINGLE_BUF_INPLACE)) {
        ...
        i = fill_sg_comp_from_buf_min(scatter_comp, i, fc_params->bufs, &size);
      } else {
        uint32_t aad_offset = aad_len ? passthrough_len : 0;
        ...
        i = fill_sg_comp_from_iov(scatter_comp, i, fc_params->dst_iov, 0, &size,
                                  aad_buf, aad_offset);
      }
...
cpt_zuc_snow3g_enc_prep(uint32_t req_flags, uint64_t d_offs, uint64_t d_lens,
                        fc_params_t *params, void *op, void **prep_req) {
  uint32_t size;
  ...
  vq_cmd_w0.s.opcode.minor =
      ((1 << 7) | (snow3g << 5) | (0 << 4) | (0 << 3) | (flags & 0x7));
  ...
  if (likely((req_flags & SINGLE_BUF_INPLACE) &&
    ...
    if (likely(iv_len)) {
      uint32_t *iv_d = (uint32_t *)((uint8_t *)offset_vaddr + OFF_CTRL_LEN);
      memcpy(iv_d, iv, 16);
    }
    ...
    *offset_vaddr = offset_ctrl;
    ...
    if (req_flags & VALID_MAC_BUF) {
      size = outputlen - iv_len - mac_len;
      if (size) {
        i = fill_sg_comp_from_iov(scatter_comp, i, params->dst_iov, 0, &size,
                                  NULL, 0);
...
cpt_kasumi_enc_prep(uint32_t req_flags, uint64_t d_offs, uint64_t d_lens,
  ...
  vq_cmd_w0.s.opcode.minor = ((1 << 6) | (cpt_ctx->k_ecb << 5) | (dir << 4) |
                              (0 << 3) | (flags & 0x7));
...
static __rte_always_inline void cpt_kasumi_dec_prep(uint64_t d_offs,
                                                    uint64_t d_lens,
                                                    fc_params_t *params,
                                                    void *op, void **prep_req) {
  uint32_t size;
  ...
  *offset_vaddr = rte_cpu_to_be_64((uint64_t)encr_offset << 16);
  ...
  /* IV */
  memcpy((uint8_t *)offset_vaddr + OFF_CTRL_LEN, params->iv_buf, iv_len);
...
cpt_fc_dec_hmac_prep(uint32_t flags, uint64_t d_offs, uint64_t d_lens,
  ...
  if (likely(fc_type == FC_GEN)) {
    cpt_dec_hmac_prep(flags, d_offs, d_lens, fc_params, op, &prep_req);
  } else if (fc_type == ZUC_SNOW3G) {
    cpt_zuc_snow3g_dec_prep(flags, d_offs, d_lens, fc_params, op, &prep_req);
...
cpt_fc_enc_hmac_prep(uint32_t flags, uint64_t d_offs, uint64_t d_lens,
  ...
  if (likely(fc_type == FC_GEN)) {
    cpt_enc_hmac_prep(flags, d_offs, d_lens, fc_params, op, &prep_req);
  } else if (fc_type == ZUC_SNOW3G) {
    cpt_zuc_snow3g_enc_prep(flags, d_offs, d_lens, fc_params, op, &prep_req);
  } else if (fc_type == KASUMI) {
    cpt_kasumi_enc_prep(flags, d_offs, d_lens, fc_params, op, &prep_req);
...
static __rte_always_inline int
fill_sess_aead(struct rte_crypto_sym_xform *xform, struct cpt_sess_misc *sess) {
  ...
  case RTE_CRYPTO_AEAD_AES_CCM:
    CPT_LOG_DP_ERR("Crypto: Unsupported cipher algo %u", aead_form->algo);
    return -1;
...
fill_sess_cipher(struct rte_crypto_sym_xform *xform,
  ...
  case RTE_CRYPTO_CIPHER_ARC4:
    CPT_LOG_DP_ERR("Crypto: Unsupported cipher algo %u", c_form->algo);
    return -1;
  default:
    CPT_LOG_DP_ERR("Crypto: Undefined cipher algo %u specified", c_form->algo);
    return -1;
  }
  ...
  if (unlikely(cpt_fc_ciph_set_key(SESS_PRIV(sess), enc_type, c_form->key.data,
...
static __rte_always_inline int
fill_sess_auth(struct rte_crypto_sym_xform *xform, struct cpt_sess_misc *sess) {
  struct cpt_ctx *ctx = SESS_PRIV(sess);
  ...
  case RTE_CRYPTO_AUTH_AES_CBC_MAC:
    CPT_LOG_DP_ERR("Crypto: Unsupported hash algo %u", a_form->algo);
    return -1;
  default:
    CPT_LOG_DP_ERR("Crypto: Undefined Hash algo %u specified", a_form->algo);
    return -1;
  }
  ...
  if (unlikely(cpt_fc_auth_set_key(SESS_PRIV(sess), auth_type, a_form->key.data,
...
static __rte_always_inline int
fill_sess_gmac(struct rte_crypto_sym_xform *xform, struct cpt_sess_misc *sess) {
  struct rte_crypto_auth_xform *a_form;
  cipher_type_t enc_type = 0; /* NULL Cipher type */
  auth_type_t auth_type = 0;  /* NULL Auth type */
  void *ctx = SESS_PRIV(sess);
  ...
  case RTE_CRYPTO_AUTH_AES_GMAC:
    ...
    break;
  default:
    CPT_LOG_DP_ERR("Crypto: Undefined cipher algo %u specified", a_form->algo);
    return -1;
  }
...
static __rte_always_inline void *
alloc_op_meta(struct rte_mbuf *m_src, buf_ptr_t *buf, int32_t len,
...
static __rte_always_inline uint32_t prepare_iov_from_pkt(
    struct rte_mbuf *pkt, iov_ptr_t *iovec, uint32_t start_offset) {
  uint16_t index = 0;
  void *seg_data = NULL;
...
static __rte_always_inline uint32_t prepare_iov_from_pkt_inplace(
    struct rte_mbuf *pkt, fc_params_t *param, uint32_t *flags) {
  uint16_t index = 0;
  void *seg_data = NULL;
  phys_addr_t seg_phys;
  uint32_t seg_size = 0;
  ...
  if (likely(!pkt->next)) {
    ...
    *flags |= SINGLE_BUF_INPLACE;
    ...
    if (likely((headroom >= 24) && (tailroom >= 8))) {
...
static __rte_always_inline int fill_fc_params(struct rte_crypto_op *cop,
  ...
  if (likely(sess_misc->iv_length)) {
    flags |= VALID_IV_BUF;
    fc_params.iv_buf =
        rte_crypto_op_ctod_offset(cop, uint8_t *, sess_misc->iv_offset);
    if (sess_misc->aes_ctr && unlikely(sess_misc->iv_length != 16)) {
  ...
  if (sess_misc->zsk_flag) {
    fc_params.auth_iv_buf =
        rte_crypto_op_ctod_offset(cop, uint8_t *, sess_misc->auth_iv_offset);
  ...
  if (sess_misc->aes_gcm || sess_misc->chacha_poly) {
    ...
    mc_hash_off = sym_op->aead.data.offset + sym_op->aead.data.length;
    ...
    if (likely(sess_misc->mac_len)) {
      struct rte_mbuf *m = (cpt_op & CPT_OP_ENCODE) ? m_dst : m_src;
      ...
      if (unlikely(rte_pktmbuf_mtod(m, uint8_t *) + mc_hash_off !=
        ...
        fc_params.mac_buf.dma_addr = sym_op->aead.digest.phys_addr;
        inplace = 0;
      }
    }
  } else {
    ...
    mc_hash_off = sym_op->cipher.data.offset + sym_op->cipher.data.length;
    d_offs = (d_offs << 16) | sym_op->auth.data.offset;
    d_lens = (d_lens << 32) | sym_op->auth.data.length;

    if (mc_hash_off < (sym_op->auth.data.offset + sym_op->auth.data.length)) {
      mc_hash_off = (sym_op->auth.data.offset + sym_op->auth.data.length);
    }
    ...
    if (likely(sess_misc->mac_len)) {
      ...
      if (!ctx->dec_auth && !ctx->auth_enc &&
        ...
        fc_params.mac_buf.dma_addr = sym_op->auth.digest.phys_addr;
        inplace = 0;
      }
    }
  }
  ...
  if (!ctx->dec_auth &&
      unlikely(sess_misc->is_null || sess_misc->cpt_op == CPT_OP_DECODE))
    inplace = 0;
  ...
  if (likely(flags & SINGLE_BUF_HEADTAILROOM))
    mdata = alloc_op_meta(m_src, &fc_params.meta_buf, m_info->lb_mlen,
                          m_info->pool);
  else
    mdata =
        alloc_op_meta(NULL, &fc_params.meta_buf, m_info->sg_mlen, m_info->pool);
  ...
  if (cpt_op & CPT_OP_ENCODE)
    *prep_req = cpt_fc_enc_hmac_prep(flags, d_offs, d_lens, &fc_params, op);
  else
    *prep_req = cpt_fc_dec_hmac_prep(flags, d_offs, d_lens, &fc_params, op);
...
static __rte_always_inline void compl_auth_verify(struct rte_crypto_op *op,
                                                  uint8_t *gen_mac,
                                                  uint64_t mac_len) {
  uint8_t *mac;
...
find_kasumif9_direction_and_length(uint8_t *src, uint32_t counter_num_bytes,
  ...
  while (!found && counter_num_bytes > 0) {
    ...
    if (pos == 7) {
      ...
      *addr_length_in_bits = counter_num_bytes * 8 + (8 - (pos + 2));
    }
    found = 1;
  }
}
 ...
 */
static __rte_always_inline int
fill_digest_params(struct rte_crypto_op *cop, struct cpt_sess_misc *sess,
                   struct cpt_qp_meta_info *m_info, void **mdata_ptr,
                   void **prep_req) {
  uint32_t space = 0;
  struct rte_crypto_sym_op *sym_op = cop->sym;
  void *mdata;
  ...
  mdata = alloc_op_meta(NULL, &params.meta_buf, m_info->sg_mlen, m_info->pool);
  if (mdata == NULL) {
    ret = -ENOMEM;
    goto err_exit;
  }
  ...
  if (unlikely(sess->zsk_flag)) {
    ...
    if (sess->zsk_flag == K_F9) {
      ...
      memcpy(iv_buf, rte_pktmbuf_mtod(cop->sym->m_src, uint8_t *), 8);
      /*
      ...
      find_kasumif9_direction_and_length(src, num_bytes, &length_in_bits,
                                         &direction);
      length_in_bits -= 64;
  ...
  if (auth_op == CPT_OP_AUTH_GENERATE) {
    if (sym_op->auth.digest.data) {
      ...
      params.mac_buf.dma_addr = sym_op->auth.digest.phys_addr;
    } else {
      uint32_t off = sym_op->auth.data.offset + sym_op->auth.data.length;
      int32_t dlen, space;
      ...
      params.mac_buf.vaddr = rte_pktmbuf_mtod_offset(m_dst, void *, off);
      params.mac_buf.dma_addr = rte_pktmbuf_iova_offset(m_dst, off);
