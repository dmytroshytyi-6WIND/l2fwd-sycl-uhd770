...
static __rte_always_inline void cpt_modex_param_normalize(uint8_t **data,
                                                          size_t *len) {
  size_t i;
...
cpt_fill_modex_params(struct cpt_asym_sess_misc *sess,
  ...
  if (unlikely(exp_len > mod_len)) {
    CPT_LOG_DP_ERR(
        "Exponent length greater than modulus length is not supported");
    return -ENOTSUP;
  }
...
cpt_fill_req_comp_addr(struct cpt_request_info *req, buf_ptr_t addr) {
  ...
  req->comp_baddr = addr.dma_addr + RTE_PTR_DIFF(completion_addr, addr.vaddr);
...
static __rte_always_inline void cpt_rsa_prep(struct asym_op_params *rsa_params,
                                             struct rte_crypto_rsa_xform *rsa,
  ...
  if (rsa_op.pad == RTE_CRYPTO_RSA_PADDING_NONE) {
    ...
    if (rsa_op.op_type == RTE_CRYPTO_ASYM_OP_ENCRYPT) {
      ...
      vq_cmd_w0.s.param2 = CPT_BLOCK_TYPE2 | ((uint16_t)(exp_len) << 1);
...
cpt_enqueue_rsa_op(struct rte_crypto_op *op, struct asym_op_params *params,
...
static const struct cpt_ec_group ec_grp[CPT_EC_ID_PMAX] = {
    {
        ...
        .consta = {.data = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                   ...
                   .length = 24},
        .constb = {.data = {0x64, 0x21, 0x05, 0x19, 0xE5, 0x9C, 0x80, 0xE7,
    ...
    {
        .prime = {.data = {0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x01,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
                           0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
                  .length = 32},
        .order = {.data = {0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00,
                           0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                           0xBC, 0xE6, 0xFA, 0xAD, 0xA7, 0x17, 0x9E, 0x84,
                           0xF3, 0xB9, 0xCA, 0xC2, 0xFC, 0x63, 0x25, 0x51},
                  .length = 32},
        .consta = {.data = {0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x01,
                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                            0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
                            0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFC},
                   .length = 32},
        .constb = {.data = {0x5A, 0xC6, 0x35, 0xD8, 0xAA, 0x3A, 0x93, 0xE7,
    ...
    {.prime = {.data = {0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                        ...
                        0xFF, 0xFF, 0xFF},
               .length = 66},
     .order = {.data = {0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFA, 0x51, 0x86,
                        0x87, 0x83, 0xBF, 0x2F, 0x96, 0x6B, 0x7F, 0xCC, 0x01,
                        0x48, 0xF7, 0x09, 0xA5, 0xD0, 0x3B, 0xB5, 0xC9, 0xB8,
                        0x89, 0x9C, 0x47, 0xAE, 0xBB, 0x6F, 0xB7, 0x1E, 0x91,
                        0x38, 0x64, 0x09},
               .length = 66},
     .consta = {.data = {0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                         0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                         0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                         0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                         0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                         ...
                         0xFF, 0xFF, 0xFC},
                .length = 66},
     .constb = {.data = {0x00, 0x51, 0x95, 0x3E, 0xB9, 0x61, 0x8E, 0x1C, 0x9A,
                         0x1F, 0x92, 0x9A, 0x21, 0xA0, 0xB6, 0x85, 0x40, 0xEE,
                         0xA2, 0xDA, 0x72, 0x5B, 0x99, 0xB3, 0x15, 0xF3, 0xB8,
                         0xB4, 0x89, 0x91, 0x8E, 0xF1, 0x09, 0xE1, 0x56, 0x19,
                         0x39, 0x51, 0xEC, 0x7E, 0x93, 0x7B, 0x16, 0x52, 0xC0,
                         0xBD, 0x3B, 0xB1, 0xBF, 0x07, 0x35, 0x73, 0xDF, 0x88,
                         0x3D, 0x2C, 0x34, 0xF1, 0xEF, 0x45, 0x1F, 0xD4, 0x6B,
                         0x50, 0x3F, 0x00},
                .length = 66}}};
...
static __rte_always_inline int __rte_hot
cpt_enqueue_ecdsa_op(struct rte_crypto_op *op, struct asym_op_params *params,
                     struct cpt_asym_sess_misc *sess, uint64_t *fpm_iova) {
  ...
  if (ecdsa->op_type == RTE_CRYPTO_ASYM_OP_SIGN)
    ...
    cpt_ecdsa_verify_prep(ecdsa, params, fpm_iova[curveid], curveid);
  else {
    op->status = RTE_CRYPTO_OP_STATUS_INVALID_ARGS;
    return -EINVAL;
  }
...
cpt_ecpm_prep(struct rte_crypto_ecpm_op_param *ecpm,
              struct asym_op_params *asym_params, uint8_t curveid) {
  ...
  memcpy(dptr, ec_grp[curveid].consta.data, ec_grp[curveid].consta.length);
  dptr += p_align;

  memcpy(dptr, ec_grp[curveid].constb.data, ec_grp[curveid].constb.length);
  dptr += p_align;
