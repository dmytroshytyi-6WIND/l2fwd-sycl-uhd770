...
int32_t cpt_pmd_ops_helper_get_mlen_direct_mode(void) {
  uint32_t len = 0;
...
int cpt_pmd_ops_helper_get_mlen_sg_mode(void) {
  uint32_t len = 0;
  ...
  len += RTE_ALIGN_CEIL(
      (SG_LIST_HDR_SIZE +
       (RTE_ALIGN_CEIL(CPT_MAX_SG_IN_OUT_CNT, 4) >> 2) * SG_ENTRY_SIZE),
      8);
...
int cpt_pmd_ops_helper_asym_get_mlen(void) {
  uint32_t len;
