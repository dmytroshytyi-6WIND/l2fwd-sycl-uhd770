...
static __rte_always_inline void pending_queue_push(struct pending_queue *q,
                                                   void *rid, unsigned int off,
                                                   const int qsize) {
  /* NOTE: no free space check, but it is expected that one is made */
  q->rid_queue[(q->tail + off) & (qsize - 1)] = rid;
}

static __rte_always_inline void pending_queue_commit(struct pending_queue *q,
                                                     unsigned int cnt,
...
static __rte_always_inline void pending_queue_pop(struct pending_queue *q,
                                                  const int qsize) {
...
static __rte_always_inline void pending_queue_peek(struct pending_queue *q,
                                                   void **rid, const int qsize,
                                                   int prefetch_next) {
  void *next_rid;
