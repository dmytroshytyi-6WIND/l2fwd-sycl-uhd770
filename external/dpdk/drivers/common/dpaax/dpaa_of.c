...
static int of_open_dir(const char *relative_path, struct dirent ***d) {
  int ret;
  ...
  if (ret < 0)
    DPAAX_LOG(ERR, "Failed to open directory %s", full_path);
  return ret;
}

static void of_close_dir(struct dirent **d, int num) {
  while (num--)
    free(d[num]);
  free(d);
}

static int of_open_file(const char *relative_path) {
  int ret;
  char full_path[PATH_MAX];
...
static const struct dt_dir *node2dir(const struct device_node *n) {
  struct dt_node *dn =
      container_of((struct device_node *)n, struct dt_node, node);
...
static int iterate_dir(struct dirent **d, int num, struct dt_dir *dt) {
  ...
  for (loop = 0; loop < num; loop++) {
    ...
    case DT_DIR:
      ...
      strlcpy(subdir->node.node.name, d[loop]->d_name, NAME_MAX);
...
static void linear_dir(struct dt_dir *d) {
  ...
  list_for_each_entry(f, &d->files, node.list) {
    if (!strcmp(f->node.node.name, "compatible")) {
      if (d->compatible)
        DPAAX_LOG(DEBUG,
                  "Duplicate compatible in"
                  " %s",
                  d->node.node.full_name);
      d->compatible = f;
    } else if (!strcmp(f->node.node.name, "status")) {
      if (d->status)
        DPAAX_LOG(DEBUG, "Duplicate status in %s", d->node.node.full_name);
      d->status = f;
    } else if (!strcmp(f->node.node.name, "linux,phandle")) {
      if (d->lphandle)
        DPAAX_LOG(DEBUG, "Duplicate lphandle in %s", d->node.node.full_name);
      d->lphandle = f;
      ...
      if (d->a_cells)
        DPAAX_LOG(DEBUG, "Duplicate a_cells in %s", d->node.node.full_name);
      d->a_cells = f;
    } else if (!strcmp(f->node.node.name, "#size-cells")) {
      if (d->s_cells)
        DPAAX_LOG(DEBUG, "Duplicate s_cells in %s", d->node.node.full_name);
      d->s_cells = f;
    } else if (!strcmp(f->node.node.name, "reg")) {
      if (d->reg)
        DPAAX_LOG(DEBUG, "Duplicate reg in %s", d->node.node.full_name);
      d->reg = f;
    }
  }
...
of_find_compatible_node(const struct device_node *from,
                        const char *type __rte_unused, const char *compatible) {
  ...
  if (!from)
    ...
    d = node2dir(from);
  for (d = next_linear(d);
       d && (!d->compatible || !check_compatible(d->compatible, compatible));
       d = next_linear(d))
    ;
  if (d)
    return &d->node.node;
...
const void *of_get_property(const struct device_node *from, const char *name,
  ...
  d = node2dir(from);
  list_for_each_entry(f, &d->files,
                      node.list) if (!strcmp(f->node.node.name, name)) {
    if (lenp)
      *lenp = f->len;
    return f->buf;
  }
...
const struct device_node *of_find_node_by_phandle(uint64_t ph) {
  ...
  DPAAX_HWWARN(!alive, "Device-tree driver not initialised!");
  list_for_each_entry(d, &linear, linear) if (d->lphandle &&
                                              (d->lphandle->len == 4) &&
                                              !memcmp(d->lphandle->buf, &ph,
                                                      4)) return &d->node.node;
  return NULL;
}
...
uint32_t of_n_addr_cells(const struct device_node *dev_node) {
  ...
  while ((d = d->parent))
    if (d->a_cells) {
      unsigned char *buf = (unsigned char *)&d->a_cells->buf[0];
...
uint32_t of_n_size_cells(const struct device_node *dev_node) {
  ...
  while ((d = d->parent))
    if (d->s_cells) {
      unsigned char *buf = (unsigned char *)&d->s_cells->buf[0];
...
const uint32_t *of_get_address(const struct device_node *dev_node, size_t idx,
  ...
  if (size)
    for (*size = 0; ns > 0; ns--, na++)
      *size =
          (*size << 32) +
          (((uint32_t)buf[4 * na] << 24) | ((uint32_t)buf[4 * na + 1] << 16) |
           ((uint32_t)buf[4 * na + 2] << 8) | (uint32_t)buf[4 * na + 3]);
