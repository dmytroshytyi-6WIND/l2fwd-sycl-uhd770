...
__rte_internal const struct device_node *
...
__rte_internal const void *of_get_property(const struct device_node *from,
                                           const char *name, size_t *lenp)
    __attribute__((nonnull(2)));
__rte_internal bool of_device_is_available(const struct device_node *dev_node);

__rte_internal const struct device_node *of_find_node_by_phandle(uint64_t ph);
...
__rte_internal const void *of_get_mac_address(const struct device_node *np);

#define for_each_child_node(parent, child)                                     \
  for (child = of_get_next_child(parent, NULL); child != NULL;                 \
       child = of_get_next_child(parent, child))

__rte_internal uint32_t of_n_addr_cells(const struct device_node *dev_node);
uint32_t of_n_size_cells(const struct device_node *dev_node);

__rte_internal const uint32_t *
of_get_address(const struct device_node *dev_node, size_t idx, uint64_t *size,
               uint32_t *flags);

__rte_internal uint64_t of_translate_address(const struct device_node *dev_node,
                                             const uint32_t *addr)
    __attribute__((nonnull));

__rte_internal bool of_device_is_compatible(const struct device_node *dev_node,
 ...
 */
__rte_internal int of_init_path(const char *dt_path);
...
static inline int of_init(void) { return of_init_path(OF_INIT_DEFAULT_PATH); }
