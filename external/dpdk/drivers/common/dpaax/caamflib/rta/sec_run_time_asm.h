 ...
 */
enum rta_jump_cond { ALL_TRUE, ALL_FALSE, ANY_TRUE, ANY_FALSE };
 ...
 */
enum rta_share_type { SHR_NEVER, SHR_WAIT, SHR_SERIAL, SHR_ALWAYS, SHR_DEFER };
 ...
 */
enum rta_data_type { RTA_DATA_PTR = 1, RTA_DATA_IMM, RTA_DATA_IMM_DMA };
...
#define COPY                                                                   \
  BIT(31) /* command param is pointer (not immediate)                          \
...
#define EKT                                                                    \
  BIT(15) /* AES CCM Encryption (default is                                    \
           * AES ECB Encryption)                                               \
           */
#define TK                                                                     \
  BIT(16)           /* Trusted Descriptor Key (default is                      \
                     * Job Descriptor Key)                                     \
                     */
...
static inline int rta_program_finalize(struct program *program) {
  ...
  if (program->shrhdr != NULL)
    *program->shrhdr |=
        program->bswap ? swab32(program->current_pc) : program->current_pc;
...
static inline void __rta_out32(struct program *program, uint32_t val) {
  program->buffer[program->current_pc] = program->bswap ? swab32(val) : val;
  program->current_pc++;
}
...
static inline void __rta_out64(struct program *program, bool is_ext,
  ...
  if (is_ext) {
    ...
    __rta_out32(program,
                program->bswap ? lower_32_bits(val) : upper_32_bits(val));

    __rta_out32(program,
                program->bswap ? upper_32_bits(val) : lower_32_bits(val));
#else
    __rta_out32(program,
                program->bswap ? upper_32_bits(val) : lower_32_bits(val));
...
static inline int rta_patch_header(struct program *program, int line,
  ...
  if (rta_sec_era >= RTA_SEC_ERA_10) {
    ...
    opcode |= (new_ref << HDR_START_IDX_SHIFT) & HDR_START_IDX_MASK_ERA10;
  } else {
...
static inline int rta_patch_load(struct program *program, int line,
  ...
  if (opcode & (LDST_SRCDST_WORD_DESCBUF | LDST_CLASS_DECO))
    ...
    opcode |= (new_ref << (LDST_OFFSET_SHIFT + 2)) & LDST_OFFSET_MASK;
