...
static inline int __rta_alg_aai_aes(uint16_t aai) {
  ...
  if (aai & OP_ALG_AAI_C2K) {
    if (rta_sec_era < RTA_SEC_ERA_5)
      return -1;
    if ((aes_mode != OP_ALG_AAI_CCM) && (aes_mode != OP_ALG_AAI_GCM))
      return -EINVAL;
  }
...
static inline int __rta_alg_aai_rng(uint16_t aai) {
  ...
  if ((rta_sec_era < RTA_SEC_ERA_5) &&
      (aai & (OP_ALG_AAI_RNG4_PS | OP_ALG_AAI_RNG4_AI | OP_ALG_AAI_RNG4_SK)))
    return -EINVAL;
 ...
 */
static const unsigned int alg_table_sz[] = {14, 15, 15, 15, 17,
                                            17, 11, 17, 17, 17};
...
static inline int rta_operation(struct program *program, uint32_t cipher_algo,
  ...
  for (i = 0; i < alg_table_sz[rta_sec_era]; i++) {
    if (alg_table[i].chipher_algo == cipher_algo) {
      if ((aai == OP_ALG_AAI_XCBC_MAC) || (aai == OP_ALG_AAI_CBC_XCBCMAC))
...
static inline int rta_operation2(struct program *program, uint32_t cipher_algo,
                                 uint16_t aai, uint8_t algo_state,
                                 int icv_checking, int enc) {
  uint32_t opcode = CMD_OPERATION;
  unsigned int i, found = 0;
...
static inline int rta_pkha_operation(struct program *program,
                                     uint32_t op_pkha) {
