...
static const uint32_t shr_header_flags[] = {DNR | SC | PD,
                                            DNR | SC | PD | CIF,
                                            DNR | SC | PD | CIF,
                                            DNR | SC | PD | CIF | RIF,
                                            DNR | SC | PD | CIF | RIF,
...
static inline int rta_shr_header(struct program *program,
                                 ...
                                 unsigned int start_idx, uint32_t flags) {
  ...
  if (rta_sec_era >= RTA_SEC_ERA_10)
    opcode |= (start_idx << HDR_START_IDX_SHIFT) & HDR_START_IDX_MASK_ERA10;
  else
...
static inline int rta_job_header(struct program *program,
                                 enum rta_share_type share,
                                 unsigned int start_idx, uint64_t shr_desc,
  ...
  if ((flags & TD) && (flags & REO)) {
    pr_err("JOB_DESC: REO flag not supported for trusted descriptors. SEC "
           "Program Line: %d\n",
           program->current_pc);
    goto err;
  }
  ...
  if ((rta_sec_era < RTA_SEC_ERA_7) && (flags & MTD) && !(flags & TD)) {
    pr_err("JOB_DESC: Trying to MTD a descriptor that is not a TD. SEC Program "
           "Line: %d\n",
           program->current_pc);
    goto err;
  }
  ...
  if ((flags & EXT) && !(flags & SHR) && (start_idx < 2)) {
    pr_err("JOB_DESC: Start index must be >= 2 in case of no SHR and EXT. SEC "
           "Program Line: %d\n",
           program->current_pc);
    goto err;
  }
  ...
  if (flags & EXT) {
    ...
    if (ext_flags & FTD) {
      if (rta_sec_era <= RTA_SEC_ERA_5) {
        pr_err(
            "JOB_DESC: Fake trusted descriptor not supported by SEC Era %d\n",
            USER_SEC_ERA(rta_sec_era));
        goto err;
      }
