 ...
 * supported.
 */
static const unsigned int fifo_load_table_sz[] = {22, 22, 23, 23, 23,
                                                  23, 23, 23, 23, 23};
...
static inline int rta_fifo_load(struct program *program, uint32_t src,
  ...
  ret = __rta_map_opcode(src, fifo_load_table, fifo_load_table_sz[rta_sec_era],
                         &val);
  if (ret < 0) {
 ...
 * supported.
 */
static const unsigned int fifo_store_table_sz[] = {21, 21, 21, 21, 22,
                                                   22, 22, 23, 23, 23};
...
static inline int rta_fifo_store(struct program *program, uint32_t src,
  ...
  if (is_seq_cmd) {
    ...
    if (((src == RNGOFIFO) && ((dst) || (flags & EXT))) || (src == METADATA)) {
