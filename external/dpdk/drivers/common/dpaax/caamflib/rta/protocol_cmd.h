...
static inline int __rta_dlc_proto(uint16_t protoinfo) {
  if ((rta_sec_era < RTA_SEC_ERA_2) &&
      (protoinfo &
       (OP_PCL_PKPROT_DSA_MSG | OP_PCL_PKPROT_HASH_MASK | OP_PCL_PKPROT_EKT_Z |
        OP_PCL_PKPROT_DECRYPT_Z | OP_PCL_PKPROT_DECRYPT_PRI)))
    return -EINVAL;
...
static inline int __rta_rsa_enc_proto(uint16_t protoinfo) {
  ...
  case OP_PCL_RSAPROT_OP_ENC_F_IN:
    if ((protoinfo & OP_PCL_RSAPROT_FFF_MASK) != OP_PCL_RSAPROT_FFF_RED)
      return -EINVAL;
    break;
 ...
 */
static const uint8_t key_in_out[4][4] = {
...
static const struct proto_map proto_table[] = {
    ...
    /*29*/
    {OP_TYPE_DECAP_PROTOCOL, OP_PCLID_LTE_PDCP_CTRL, __rta_lte_pdcp_proto},
    ...
    /*38*/
    {OP_TYPE_DECAP_PROTOCOL, OP_PCLID_LTE_PDCP_CTRL_MIXED,
     __rta_lte_pdcp_mixed_proto},
    {OP_TYPE_DECAP_PROTOCOL, OP_PCLID_IPSEC_NEW, __rta_ipsec_proto},
    /*40*/
    {OP_TYPE_DECAP_PROTOCOL, OP_PCLID_LTE_PDCP_USER_RN,
     __rta_lte_pdcp_mixed_proto},
};
 ...
 */
static const unsigned int proto_table_sz[] = {21, 29, 29, 29, 29,
                                              35, 37, 40, 40, 40};
  ...
  for (i = 0; i < proto_table_sz[rta_sec_era]; i++) {
    ...
    if (optype_tmp == proto_table[i].optype) {
      if (proto_table[i].protid == protid) {
        ...
        /* check protoinfo */
        ret = (*proto_table[i].protoinfo_func)(protoinfo);
        if (ret < 0) {
...
static inline int rta_dkp_proto(struct program *program, uint32_t protid,
  ...
  if ((key_src == OP_PCL_DKP_SRC_PTR) || (key_src == OP_PCL_DKP_SRC_SGF)) {
  ...
  if ((key_dst == OP_PCL_DKP_DST_PTR) || (key_dst == OP_PCL_DKP_DST_SGF)) {
    out_words = in_words;
