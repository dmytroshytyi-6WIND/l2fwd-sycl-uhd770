 ...
 */
static const unsigned int math_op1_sz[] = {10, 10, 12, 12, 12,
                                           12, 12, 12, 12, 12};
 ...
 */
static const unsigned int math_op2_sz[] = {8,  9,  13, 13, 13,
                                           13, 13, 13, 13, 13};
 ...
 * supported.
 */
static const unsigned int math_result_sz[] = {9,  9,  10, 10, 10,
                                              10, 10, 10, 10, 10};
...
static inline int rta_math(struct program *program, uint64_t operand1,
  ...
  if (((op == MATH_FUN_BSWAP) && (rta_sec_era < RTA_SEC_ERA_4)) ||
    ...
    pr_err(
        "MATH: operation not supported by SEC Era %d. SEC PC: %d; Instr: %d\n",
  ...
  if (options & SWP) {
    if (rta_sec_era < RTA_SEC_ERA_7) {
      pr_err("MATH: operation not supported by SEC Era %d. SEC PC: %d; Instr: "
             "%d\n",
             USER_SEC_ERA(rta_sec_era), program->current_pc,
             program->current_instruction);
      goto err;
    ...
    if ((options & IFB) || (!(options & IMMED) && !(options & IMMED2)) ||
   ...
   */
  if ((op != MATH_FUN_SHLD) && ((operand1 == NONE) || (operand2 == SEQINSZ))) {
   ...
   */
  if (((op == MATH_FUN_ZBYT) || (op == MATH_FUN_BSWAP)) && (operand2 != NONE)) {
  ...
  /* Write result field */
  ret =
      __rta_map_opcode(result, math_result, math_result_sz[rta_sec_era], &val);
  if (ret < 0) {
  ...
  if ((options & IMMED) && !(options & IMMED2)) {
    __rta_out64(program, (length > 4) && !(options & IFB), operand1);
  } else if ((options & IMMED2) && !(options & IMMED)) {
    __rta_out64(program, (length > 4) && !(options & IFB), operand2);
...
static inline int rta_mathi(struct program *program, uint64_t operand,
  ...
  if (rta_sec_era < RTA_SEC_ERA_6) {
    pr_err(
        "MATHI: Command not supported by SEC Era %d. SEC PC: %d; Instr: %d\n",
        USER_SEC_ERA(rta_sec_era), program->current_pc,
        program->current_instruction);
    goto err;
  }
  ...
  if (((op == MATH_FUN_FBYT) && (options & SSEL))) {
    pr_err(
        "MATHI: Illegal combination - FBYT and SSEL. SEC PC: %d; Instr: %d\n",
        program->current_pc, program->current_instruction);
    goto err;
  }
