 ...
 */
static const unsigned int nfifo_flags_sz[] = {12, 14, 14, 14, 14,
                                              14, 14, 14, 14, 14};

static const uint32_t nfifo_pad_flags[][2] = {
    {BM, NFIFOENTRY_BM}, {PS, NFIFOENTRY_PS}, {PR, NFIFOENTRY_PR}};
...
static inline int rta_nfifo_load(struct program *program, uint32_t src,
  ...
  int ret = -EINVAL;
  uint32_t load_cmd =
      CMD_LOAD | LDST_IMM | LDST_CLASS_IND_CCB | LDST_SRCDST_WORD_INFO_FIFO;
  ...
  /* write flags */
  __rta_map_flags(flags, nfifo_flags, nfifo_flags_sz[rta_sec_era], &opcode);

  /* in case of padding, check the destination */
  if (src == PAD)
    __rta_map_flags(flags, nfifo_pad_flags, nfifo_pad_flags_sz[rta_sec_era],
                    &opcode);
