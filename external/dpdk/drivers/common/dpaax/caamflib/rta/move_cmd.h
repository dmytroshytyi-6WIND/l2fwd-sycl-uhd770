 ...
 * supported.
 */
static const unsigned int move_src_table_sz[] = {9,  11, 14, 14, 14,
                                                 14, 14, 14, 14, 14};
 ...
 * supported.
 */
static const unsigned int move_dst_table_sz[] = {13, 14, 14, 15, 15,
                                                 15, 15, 15, 15, 15};
...
static inline int rta_move(struct program *program, int cmd_type, uint64_t src,
  ...
  if ((rta_sec_era < RTA_SEC_ERA_7) && (cmd_type != __MOVE)) {
    pr_err("MOVE: MOVEB / MOVEDW not supported by SEC Era %d. SEC PC: %d; "
           "Instr: %d\n",
  ...
  if (cmd_type == __MOVEB) {
    ...
    if (rta_sec_era < RTA_SEC_ERA_3) {
      pr_err(
          "MOVE: MOVE_LEN not supported by SEC Era %d. SEC PC: %d; Instr: %d\n",
          USER_SEC_ERA(rta_sec_era), program->current_pc,
          program->current_instruction);
      goto err;
    }

    if ((length != MATH0) && (length != MATH1) && (length != MATH2) &&
        (length != MATH3)) {
   ...
   */
  ret =
      set_move_offset(program, src, src_offset, dst, dst_offset, &offset, &opt);
  if (ret < 0)
    goto err;
  ...
  if (opt == MOVE_SET_AUX_SRC) {
    ...
    if (rta_sec_era < RTA_SEC_ERA_6) {
      if (offset)
        pr_debug(
            "MOVE: Offset not supported by SEC Era %d. SEC PC: %d; Instr: %d\n",
            USER_SEC_ERA(rta_sec_era), program->current_pc,
            program->current_instruction);
...
static inline int set_move_offset(struct program *program __maybe_unused,
  ...
  case (CONTEXT2):
    if (dst == DESCBUF) {
      ...
      *offset = dst_offset;
      ...
      if ((dst == MATH0) || (dst == MATH1) || (dst == MATH2) ||
          (dst == MATH3)) {
        *opt = MOVE_SET_AUX_MATH_DST;
      } else if (((dst == OFIFO) || (dst == ALTSOURCE)) && (src_offset % 4)) {
  ...
  case (OFIFO):
    ...
    if (((dst == IFIFOAB1) || (dst == IFIFOAB2) || (dst == IFIFO) ||
         (dst == PKA)) &&
  ...
  case (DESCBUF):
    ...
    } else if ((dst == MATH0) || (dst == MATH1) || (dst == MATH2) ||
               (dst == MATH3)) {
      *opt = MOVE_SET_AUX_MATH_DST;
    } else if (dst == DESCBUF) {
      pr_err("MOVE: Invalid DST. SEC PC: %d; Instr: %d\n", program->current_pc,
  ...
  case (MATH3):
    if ((dst == OFIFO) || (dst == ALTSOURCE)) {
      ...
      *offset = src_offset;
    } else if ((dst == IFIFOAB1) || (dst == IFIFOAB2) || (dst == IFIFO) ||
               (dst == PKA)) {
      *offset = src_offset;
    } else {
      *offset = dst_offset;
