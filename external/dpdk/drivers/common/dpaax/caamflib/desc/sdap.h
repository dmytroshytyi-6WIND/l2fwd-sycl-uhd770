 ...
 */
static inline int rta_inline_pdcp_sdap_query(enum auth_type_pdcp auth_alg,
  ...
  if ((rta_sec_era == RTA_SEC_ERA_8) && hfn_ovd &&
      ((sn_size == PDCP_SN_SIZE_12) || (sn_size == PDCP_SN_SIZE_18)) &&
      (cipher_alg != PDCP_CIPHER_TYPE_NULL) &&
      ((auth_alg == PDCP_AUTH_TYPE_SNOW) || (auth_alg == PDCP_AUTH_TYPE_ZUC))) {
...
static inline void key_loading_opti(struct program *p,
   ...
   *    already loaded
   */
  ref_skip_key_loading_jump = JUMP(p, lbl_skip_key_loading_jump, LOCAL_JUMP,
                                   ALL_TRUE, SHRD | SELF | BOTH);
...
static inline int pdcp_sdap_get_sn_parameters(enum pdcp_sn_size sn_size,
  ...
  case PDCP_SN_SIZE_7:
    *offset = 7;
    *length = 1;
    *sn_mask = (swap == false) ? PDCP_7BIT_SN_MASK : PDCP_7BIT_SN_MASK_BE;
    break;
...
static inline int pdcp_sdap_insert_no_int_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    unsigned int dir, enum pdcp_sn_size sn_size, enum pdb_type_e pdb_type) {
  int op;
  uint32_t sn_mask = 0;
  uint32_t length = 0;
  uint32_t offset = 0;
  ...
  if (pdcp_sdap_get_sn_parameters(sn_size, swap, &offset, &length, &sn_mask))
    return -ENOTSUP;
...
#ifdef SDAP_SUPPORT
  rta_mathi(p, MATH0, ((swap == true) ? MATH_FUN_RSHIFT : MATH_FUN_LSHIFT),
  ...
  MOVEB(p, DESCBUF, hfn_bearer_dir_offset_in_descbuf, MATH2, 0, 8,
        WAITCOMP | IMMED);
...
static inline int pdcp_sdap_insert_enc_only_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata __maybe_unused, unsigned int dir,
    enum pdcp_sn_size sn_size, enum pdb_type_e pdb_type) {
...
pdcp_sdap_insert_snoop_op(struct program *p, bool swap __maybe_unused,
  ...
  if (authdata->algtype == PDCP_AUTH_TYPE_ZUC) {
    ...
    if (swap == false) {
      ...
      MATHB(p, MATH1, AND, lower_32_bits(PDCP_BEARER_MASK_BE), MATH2, 4,
            IMMED2);
...
static inline int pdcp_sdap_insert_no_snoop_op(
  ...
  MATHB(p, MATH1, SHLD, MATH1, MATH1, 8, 0);
  MOVEB(p, DESCBUF, hfn_bearer_dir_offset_in_descbuf, MATH2, 0, 0x08,
        WAITCOMP | IMMED);
  MATHB(p, MATH1, OR, MATH2, MATH2, 8, 0);
  ...
  if (dir == OP_TYPE_ENCAP_PROTOCOL) {
    ...
    ALG_OPERATION(p, cipher_alg_op, cipher_alg_aai, OP_ALG_AS_INITFINAL,
...
static inline int pdcp_sdap_insert_cplane_null_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size,
...
static inline int pdcp_sdap_insert_cplane_int_only_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size,
    enum pdb_type_e pdb_type __maybe_unused) {
  return pdcp_insert_cplane_int_only_op(p, swap, cipherdata, authdata, dir,
                                        sn_size);
}
...
static int pdcp_sdap_insert_with_int_op(
  ...
  err = pdcp_cp_fp[cipherdata->algtype][authdata->algtype](
      p, swap, cipherdata, authdata, dir, sn_size, pdb_type);
  if (err)
    return err;
...
static inline int cnstr_shdsc_pdcp_sdap_u_plane(
    ...
    struct alginfo *authdata, uint32_t caps_mode) {
  struct program prg;
  ...
  if (!authdata) {
    if (cipherdata->algtype == PDCP_CIPHER_TYPE_NULL) {
      ...
      err = pdcp_sdap_insert_no_int_op(p, swap, cipherdata, caps_mode, sn_size,
                                       pdb_type);
      if (err) {
    ...
    err = pdcp_sdap_insert_with_int_op(p, swap, cipherdata, authdata, sn_size,
                                       caps_mode, pdb_type);
    if (err) {
 ...
 */
static inline int cnstr_shdsc_pdcp_sdap_u_plane_encap(
    uint32_t *descbuf, bool ps, bool swap, enum pdcp_sn_size sn_size,
    uint32_t hfn, unsigned short bearer, unsigned short direction,
    uint32_t hfn_threshold, struct alginfo *cipherdata,
    struct alginfo *authdata) {
  return cnstr_shdsc_pdcp_sdap_u_plane(descbuf, ps, swap, sn_size, hfn, bearer,
                                       direction, hfn_threshold, cipherdata,
 ...
 * bytes used, the user can reuse the remaining buffer space for other purposes.
 */
static inline int cnstr_shdsc_pdcp_sdap_u_plane_decap(
    uint32_t *descbuf, bool ps, bool swap, enum pdcp_sn_size sn_size,
    uint32_t hfn, unsigned short bearer, unsigned short direction,
    uint32_t hfn_threshold, struct alginfo *cipherdata,
    struct alginfo *authdata) {
