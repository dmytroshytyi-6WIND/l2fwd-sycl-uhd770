 ...
 */
enum pdcp_dir { PDCP_DIR_UPLINK = 0, PDCP_DIR_DOWNLINK = 1, PDCP_DIR_INVALID };
 ...
 */
enum pdcp_plane { PDCP_CONTROL_PLANE, PDCP_DATA_PLANE, PDCP_SHORT_MAC };
 ...
 */
static inline int rta_inline_pdcp_query(enum auth_type_pdcp auth_alg,
 ...
 */
static inline int
pdcp_insert_cplane_null_op(struct program *p, bool swap __maybe_unused,
  ...
  MATHB(p, VSEQINSZ, SUB, PDCP_NULL_MAX_FRAME_LEN, NONE, 4, IMMED2);
...
insert_copy_frame_op(struct program *p,
  ...
  MATHB(p, SEQINSZ, SUB, PDCP_NULL_MAX_FRAME_LEN, NONE, 4, IFB | IMMED2);
  JUMP(p, PDCP_MAX_FRAME_LEN_STATUS, HALT_STATUS, ALL_FALSE, MATH_N);
...
static inline int
pdcp_insert_cplane_int_only_op(struct program *p, bool swap __maybe_unused,
                               struct alginfo *cipherdata __maybe_unused,
  ...
  if (sn_size == PDCP_SN_SIZE_12) {
    ...
    PROTOCOL(p, dir, OP_PCLID_LTE_PDCP_USER_RN, (uint16_t)authdata->algtype);
    return 0;
  }
  ...
  case PDCP_SN_SIZE_5:
    offset = 7;
    length = 1;
    sn_mask = (swap == false) ? PDCP_C_PLANE_SN_MASK : PDCP_C_PLANE_SN_MASK_BE;
    break;
  ...
  case PDCP_AUTH_TYPE_SNOW:
    ...
    if (swap == false) {
      ...
      MATHB(p, MATH2, AND, PDCP_BEARER_MASK, MATH2, 8, IMMED2);
      ...
      MATHB(p, MATH2, AND, PDCP_BEARER_MASK_BE, MATH2, 8, IMMED2);

      MOVE(p, DESCBUF, 0x0C, MATH3, 0, 4, WAITCOMP | IMMED);
      MATHB(p, MATH3, AND, PDCP_DIR_MASK_BE, MATH3, 8, IMMED2);
      MATHB(p, MATH1, OR, MATH2, MATH2, 8, 0);
    ...
    SEQFIFOSTORE(p, MSG, 0, 0, VLF);
    ALG_OPERATION(p, OP_ALG_ALGSEL_SNOW_F9, OP_ALG_AAI_F9, OP_ALG_AS_INITFINAL,
  ...
  case PDCP_AUTH_TYPE_AES:
    ...
    SEQFIFOSTORE(p, MSG, 0, 0, VLF);
    ALG_OPERATION(p, OP_ALG_ALGSEL_AES, OP_ALG_AAI_CMAC, OP_ALG_AS_INITFINAL,
                  dir == OP_TYPE_ENCAP_PROTOCOL ? ICV_CHECK_DISABLE
                                                : ICV_CHECK_ENABLE,
                  DIR_ENC);
  ...
  case PDCP_AUTH_TYPE_ZUC:
    ...
    MATHB(p, MATH1, SUB, ZERO, VSEQOUTSZ, 4, 0);
    SEQFIFOSTORE(p, MSG, 0, 0, VLF);
    ALG_OPERATION(p, OP_ALG_ALGSEL_ZUCA, OP_ALG_AAI_F9, OP_ALG_AS_INITFINAL,
                  dir == OP_TYPE_ENCAP_PROTOCOL ? ICV_CHECK_DISABLE
                                                : ICV_CHECK_ENABLE,
                  DIR_ENC);
    SEQFIFOLOAD(p, MSGINSNOOP, 0, VLF | LAST1 | LAST2 | FLUSH1);
...
static inline int
pdcp_insert_cplane_enc_only_op(struct program *p, bool swap __maybe_unused,
                               struct alginfo *cipherdata,
                               struct alginfo *authdata __maybe_unused,
                               unsigned int dir, enum pdcp_sn_size sn_size) {
  uint32_t offset = 0, length = 0, sn_mask = 0;
  /* Insert Cipher Key */
  KEY(p, KEY1, cipherdata->key_enc_flags, cipherdata->key, cipherdata->keylen,
      INLINE_KEY(cipherdata));

  if ((sn_size != PDCP_SN_SIZE_18 &&
       !(rta_sec_era == RTA_SEC_ERA_8 && authdata->algtype == 0)) ||
  ...
  case PDCP_SN_SIZE_12:
    offset = 6;
    length = 2;
    sn_mask = (swap == false) ? PDCP_12BIT_SN_MASK : PDCP_12BIT_SN_MASK_BE;
    break;
  case PDCP_SN_SIZE_7:
  case PDCP_SN_SIZE_15:
    pr_err("Invalid sn_size for %s\n", __func__);
  ...
  case PDCP_CIPHER_TYPE_SNOW:
    ...
    ALG_OPERATION(p, OP_ALG_ALGSEL_SNOW_F8, OP_ALG_AAI_F8, OP_ALG_AS_INITFINAL,
                  ICV_CHECK_DISABLE,
  ...
  case PDCP_CIPHER_TYPE_AES:
    ...
    SEQFIFOSTORE(p, MSG, 0, 0, VLF | CONT);
    ALG_OPERATION(p, OP_ALG_ALGSEL_AES, OP_ALG_AAI_CTR, OP_ALG_AS_INITFINAL,
                  ICV_CHECK_DISABLE,
                  dir == OP_TYPE_ENCAP_PROTOCOL ? DIR_ENC : DIR_DEC);
    break;
  ...
  case PDCP_CIPHER_TYPE_ZUC:
    ...
    SEQFIFOSTORE(p, MSG, 0, 0, VLF | CONT);
    ALG_OPERATION(p, OP_ALG_ALGSEL_ZUCE, OP_ALG_AAI_F8, OP_ALG_AS_INITFINAL,
                  ICV_CHECK_DISABLE,
                  dir == OP_TYPE_ENCAP_PROTOCOL ? DIR_ENC : DIR_DEC);
    break;
  ...
  if (dir == OP_TYPE_ENCAP_PROTOCOL) {
    ...
    FIFOLOAD(p, MSG1, PDCP_NULL_INT_MAC_I_VAL, 4, LAST1 | FLUSH1 | IMMED);
  } else {
    SEQFIFOLOAD(p, MSG1, 0, VLF | LAST1 | FLUSH1);
    MOVE(p, OFIFO, 0, MATH1, 4, PDCP_MAC_I_LEN, WAITCOMP | IMMED);
    MATHB(p, MATH1, XOR, PDCP_NULL_INT_MAC_I_VAL, NONE, 4, IMMED2);
    JUMP(p, PDCP_NULL_INT_ICV_CHECK_FAILED_STATUS, HALT_STATUS, ALL_FALSE,
         MATH_Z);
  }
...
static inline int pdcp_insert_uplane_snow_snow_op(
  ...
  if (swap == false) {
    MATHB(p, MATH1, AND, upper_32_bits(PDCP_BEARER_MASK), MATH2, 4, IMMED2);
    MATHB(p, MATH1, AND, lower_32_bits(PDCP_DIR_MASK), MATH3, 4, IMMED2);
  } else {
    MATHB(p, MATH1, AND, lower_32_bits(PDCP_BEARER_MASK_BE), MATH2, 4, IMMED2);
    MATHB(p, MATH1, AND, upper_32_bits(PDCP_DIR_MASK_BE), MATH3, 4, IMMED2);
  }
...
static inline int pdcp_insert_uplane_zuc_zuc_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size) {
  uint32_t offset = 0, length = 0, sn_mask = 0;
  ...
  if (dir == OP_TYPE_ENCAP_PROTOCOL) {
    ...
    LOAD(p,
         NFIFOENTRY_STYPE_ALTSOURCE | NFIFOENTRY_DEST_CLASS2 |
             NFIFOENTRY_DTYPE_ICV | NFIFOENTRY_LC2 | 4,
         NFIFO_SZL, 0, 4, IMMED);
...
static inline int pdcp_insert_uplane_aes_aes_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size) {
  uint32_t offset = 0, length = 0, sn_mask = 0;
  ...
  if (dir == OP_TYPE_ENCAP_PROTOCOL) {
    ...
    LOAD(p,
         CLRW_RESET_CLS1_CHA | CLRW_CLR_C1KEY | CLRW_CLR_C1CTX |
             CLRW_CLR_C1ICV | CLRW_CLR_C1DATAS | CLRW_CLR_C1MODE,
         CLRW, 0, 4, IMMED);
    ...
    LOAD(p,
         NFIFOENTRY_STYPE_ALTSOURCE | NFIFOENTRY_DEST_CLASS1 |
             NFIFOENTRY_DTYPE_ICV | NFIFOENTRY_LC1 | NFIFOENTRY_FC1 | 4,
         NFIFO_SZL, 0, 4, IMMED);
...
static inline int
pdcp_insert_cplane_acc_op(struct program *p, bool swap __maybe_unused,
...
static inline int pdcp_insert_cplane_snow_aes_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size) {
  uint32_t offset = 0, length = 0, sn_mask = 0;
  ...
  if ((sn_size != PDCP_SN_SIZE_18) || (rta_sec_era == RTA_SEC_ERA_10)) {
    KEY(p, KEY1, cipherdata->key_enc_flags, cipherdata->key, cipherdata->keylen,
        INLINE_KEY(cipherdata));
    KEY(p, KEY2, authdata->key_enc_flags, authdata->key, authdata->keylen,
        INLINE_KEY(authdata));
  ...
  if (dir == OP_TYPE_ENCAP_PROTOCOL) {
    ...
    MATHB(p, SEQINSZ, SUB, ZERO, VSEQINSZ, 4, 0);
    MATHB(p, SEQINSZ, SUB, ZERO, MATH1, 4, 0);
    MATHB(p, VSEQINSZ, ADD, PDCP_MAC_I_LEN - 1, VSEQOUTSZ, 4, IMMED2);
    ...
    seqin_ptr_write = MOVE(p, MATH1, 0, DESCBUF, 0, 8, WAITCOMP | IMMED);
    ALG_OPERATION(p, OP_ALG_ALGSEL_AES, OP_ALG_AAI_CMAC, OP_ALG_AS_INITFINAL,
                  ICV_CHECK_DISABLE, DIR_DEC);
    SEQFIFOLOAD(p, MSG1, 0, VLF | LAST1 | FLUSH1);
    MOVEB(p, CONTEXT1, 0, MATH3, 0, 4, WAITCOMP | IMMED);
     ...
     */
    if (p->ps)
      seq_out_read = MOVE(p, DESCBUF, 0, MATH1, 0, 20, WAITCOMP | IMMED);
    else
      seq_out_read = MOVE(p, DESCBUF, 0, MATH1, 0, 16, WAITCOMP | IMMED);

    MATHB(p, MATH1, XOR, CMD_SEQ_IN_PTR ^ CMD_SEQ_OUT_PTR, MATH1, 4, IMMED2);
...
static inline int pdcp_insert_cplane_aes_snow_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size) {
  uint32_t offset = 0, length = 0, sn_mask = 0;
...
static inline int pdcp_insert_cplane_snow_zuc_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size) {
  uint32_t offset = 0, length = 0, sn_mask = 0;
...
static inline int pdcp_insert_cplane_aes_zuc_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size) {
  uint32_t offset = 0, length = 0, sn_mask = 0;
  LABEL(keyjump);
...
static inline int pdcp_insert_cplane_zuc_snow_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size) {
  uint32_t offset = 0, length = 0, sn_mask = 0;
  LABEL(keyjump);
...
static inline int pdcp_insert_cplane_zuc_aes_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, unsigned int dir, enum pdcp_sn_size sn_size) {
  uint32_t offset = 0, length = 0, sn_mask = 0;
...
static inline int pdcp_insert_uplane_no_int_op(struct program *p,
  ...
  if ((sn_size == PDCP_SN_SIZE_15) || (rta_sec_era >= RTA_SEC_ERA_10)) {
    PROTOCOL(p, dir, OP_PCLID_LTE_PDCP_USER, (uint16_t)cipherdata->algtype);
    return 0;
  }
 ...
 */
static inline int insert_hfn_ov_op(struct program *p, uint32_t shift,
...
cnstr_pdcp_c_plane_pdb(struct program *p, uint32_t hfn,
                       enum pdcp_sn_size sn_size, unsigned char bearer,
                       unsigned char direction, uint32_t hfn_threshold) {
  struct pdcp_pdb pdb;
  ...
  if (sn_size == PDCP_SN_SIZE_12) {
    ...
    pdb.bearer_dir_res = (uint32_t)((bearer << PDCP_U_PLANE_PDB_BEARER_SHIFT) |
                                    (direction << PDCP_U_PLANE_PDB_DIR_SHIFT));

    pdb.hfn_thr_res = hfn_threshold << PDCP_U_PLANE_PDB_LONG_SN_HFN_THR_SHIFT;
    ...
    pdb.bearer_dir_res = (uint32_t)((bearer << PDCP_C_PLANE_PDB_BEARER_SHIFT) |
                                    (direction << PDCP_C_PLANE_PDB_DIR_SHIFT));
    pdb.hfn_thr_res = hfn_threshold << PDCP_C_PLANE_PDB_HFN_THR_SHIFT;
  }
 ...
 */
static inline enum pdb_type_e
cnstr_pdcp_u_plane_pdb(struct program *p, enum pdcp_sn_size sn_size,
                       uint32_t hfn, unsigned short bearer,
                       unsigned short direction, uint32_t hfn_threshold,
                       struct alginfo *cipherdata, struct alginfo *authdata) {
  struct pdcp_pdb pdb;
  enum pdb_type_e pdb_type = PDCP_PDB_TYPE_FULL_PDB;
  enum pdb_type_e pdb_mask[PDCP_CIPHER_TYPE_INVALID][PDCP_AUTH_TYPE_INVALID] = {
      {
          /* NULL */
  ...
  case PDCP_SN_SIZE_7:
    ...
    pdb.hfn_thr_res = hfn_threshold << PDCP_U_PLANE_PDB_SHORT_SN_HFN_THR_SHIFT;
    break;
  ...
  case PDCP_SN_SIZE_15:
    ...
    pdb.hfn_thr_res = hfn_threshold << PDCP_U_PLANE_PDB_15BIT_SN_HFN_THR_SHIFT;
    break;

  case PDCP_SN_SIZE_18:
    pdb.opt_res.opt = (uint32_t)(PDCP_U_PLANE_PDB_OPT_18B_SN);
    pdb.hfn_res = hfn << PDCP_U_PLANE_PDB_18BIT_SN_HFN_SHIFT;
    pdb.hfn_thr_res = hfn_threshold << PDCP_U_PLANE_PDB_18BIT_SN_HFN_THR_SHIFT;

    if (rta_sec_era == RTA_SEC_ERA_8) {
      if (cipherdata && authdata)
        pdb_type = pdb_mask[cipherdata->algtype][authdata->algtype];
    }
    break;
 ...
 */
static inline int cnstr_shdsc_pdcp_c_plane_encap(
    uint32_t *descbuf, bool ps, bool swap, uint32_t hfn,
    enum pdcp_sn_size sn_size, unsigned char bearer, unsigned char direction,
    uint32_t hfn_threshold, struct alginfo *cipherdata,
  ...
  static int (*pdcp_cp_fp[PDCP_CIPHER_TYPE_INVALID][PDCP_AUTH_TYPE_INVALID])(
      ...
      unsigned int dir, enum pdcp_sn_size) = {
      {
          /* NULL */
  ...
  static enum rta_share_type desc_share[PDCP_CIPHER_TYPE_INVALID]
                                       [PDCP_AUTH_TYPE_INVALID] = {
                                           {
                                               /* NULL */
                                               SHR_WAIT,   /* NULL */
  ...
  pdb_type =
      cnstr_pdcp_c_plane_pdb(p, hfn, sn_size, bearer, direction, hfn_threshold);
  ...
  err = pdcp_cp_fp[cipherdata->algtype][authdata->algtype](
      p, swap, cipherdata, authdata, OP_TYPE_ENCAP_PROTOCOL, sn_size);
  if (err)
    return err;
 ...
 * bytes used, the user can reuse the remaining buffer space for other purposes.
 */
static inline int cnstr_shdsc_pdcp_c_plane_decap(
    uint32_t *descbuf, bool ps, bool swap, uint32_t hfn,
    enum pdcp_sn_size sn_size, unsigned char bearer, unsigned char direction,
    uint32_t hfn_threshold, struct alginfo *cipherdata,
    struct alginfo *authdata) {
  ...
  err = pdcp_cp_fp[cipherdata->algtype][authdata->algtype](
      p, swap, cipherdata, authdata, OP_TYPE_DECAP_PROTOCOL, sn_size);
  if (err)
    return err;
...
static int pdcp_insert_uplane_with_int_op(
    struct program *p, bool swap __maybe_unused, struct alginfo *cipherdata,
    struct alginfo *authdata, enum pdcp_sn_size sn_size, unsigned int dir) {
  static int (*pdcp_cp_fp[PDCP_CIPHER_TYPE_INVALID][PDCP_AUTH_TYPE_INVALID])(
      struct program *, bool swap, struct alginfo *, struct alginfo *,
      unsigned int dir, enum pdcp_sn_size) = {
      {
  ...
  err = pdcp_cp_fp[cipherdata->algtype][authdata->algtype](
      p, swap, cipherdata, authdata, dir, sn_size);
  if (err)
    return err;
 ...
 * bytes used, the user can reuse the remaining buffer space for other purposes.
 */
static inline int cnstr_shdsc_pdcp_u_plane_encap(
    uint32_t *descbuf, bool ps, bool swap, enum pdcp_sn_size sn_size,
    uint32_t hfn, unsigned short bearer, unsigned short direction,
    uint32_t hfn_threshold, struct alginfo *cipherdata,
    struct alginfo *authdata) {
  struct program prg;
  struct program *p = &prg;
  ...
  case PDCP_SN_SIZE_12:
    ...
    case PDCP_CIPHER_TYPE_NULL:
      if (rta_sec_era == RTA_SEC_ERA_8 && authdata && authdata->algtype == 0) {
        err = pdcp_insert_uplane_with_int_op(p, swap, cipherdata, authdata,
                                             sn_size, OP_TYPE_ENCAP_PROTOCOL);
        if (err)
          return err;
        break;
      }
      ...
      if (authdata && authdata->algtype) {
        KEY(p, KEY2, authdata->key_enc_flags, (uint64_t)authdata->key,
            authdata->keylen, INLINE_KEY(authdata));
      }
      /* Insert Cipher Key */
      KEY(p, KEY1, cipherdata->key_enc_flags, (uint64_t)cipherdata->key,
          cipherdata->keylen, INLINE_KEY(cipherdata));

      if (authdata)
        PROTOCOL(p, OP_TYPE_ENCAP_PROTOCOL, OP_PCLID_LTE_PDCP_USER_RN,
                 ((uint16_t)cipherdata->algtype << 8) |
                     (uint16_t)authdata->algtype);
      else
    ...
    default:
      pr_err("%s: Invalid encrypt algorithm selected: %d\n",
             "cnstr_pcl_shdsc_pdcp_u_plane_decap", cipherdata->algtype);
      return -EINVAL;
    }
    break;
  ...
  case PDCP_SN_SIZE_18:
    ...
    switch (cipherdata->algtype) {
    case PDCP_CIPHER_TYPE_NULL:
      insert_copy_frame_op(p, cipherdata, OP_TYPE_ENCAP_PROTOCOL);
      break;
 ...
 * bytes used, the user can reuse the remaining buffer space for other purposes.
 */
static inline int cnstr_shdsc_pdcp_u_plane_decap(
    uint32_t *descbuf, bool ps, bool swap, enum pdcp_sn_size sn_size,
    uint32_t hfn, unsigned short bearer, unsigned short direction,
    uint32_t hfn_threshold, struct alginfo *cipherdata,
    struct alginfo *authdata) {
  ...
  case PDCP_SN_SIZE_12:
    ...
    case PDCP_CIPHER_TYPE_NULL:
      ...
      if (authdata && authdata->algtype)
        ...
        err = pdcp_insert_uplane_with_int_op(p, swap, cipherdata, authdata,
                                             sn_size, OP_TYPE_DECAP_PROTOCOL);
        if (err)
          return err;
        break;
      }
      ...
      KEY(p, KEY1, cipherdata->key_enc_flags, cipherdata->key,
          cipherdata->keylen, INLINE_KEY(cipherdata));
      if (authdata)
        PROTOCOL(p, OP_TYPE_DECAP_PROTOCOL, OP_PCLID_LTE_PDCP_USER_RN,
                 ((uint16_t)cipherdata->algtype << 8) |
                     (uint16_t)authdata->algtype);
      else
  ...
  case PDCP_SN_SIZE_18:
    ...
    switch (cipherdata->algtype) {
    case PDCP_CIPHER_TYPE_NULL:
      insert_copy_frame_op(p, cipherdata, OP_TYPE_DECAP_PROTOCOL);
      break;
 ...
 * bytes used, the user can reuse the remaining buffer space for other purposes.
 */
static inline int cnstr_shdsc_pdcp_short_mac(uint32_t *descbuf, bool ps,
                                             bool swap,
                                             struct alginfo *authdata) {
  struct program prg;
  struct program *p = &prg;
