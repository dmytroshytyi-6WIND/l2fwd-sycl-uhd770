 ...
 */
static inline int cnstr_shdsc_zuce(uint32_t *descbuf, bool ps, bool swap,
  ...
  ALG_OPERATION(p, OP_ALG_ALGSEL_ZUCE, OP_ALG_AAI_F8, OP_ALG_AS_INITFINAL, 0,
                dir);
 ...
 * Return: size of descriptor written in words or negative number on error
 */
static inline int cnstr_shdsc_zuca(uint32_t *descbuf, bool ps, bool swap,
...
static inline int cnstr_shdsc_snow_f8(uint32_t *descbuf, bool ps, bool swap,
  ...
  MATHB(p, SEQINSZ, SUB, MATH2, VSEQINSZ, 4, 0);
  MATHB(p, SEQINSZ, SUB, MATH2, VSEQOUTSZ, 4, 0);
  ALG_OPERATION(p, OP_ALG_ALGSEL_SNOW_F8, OP_ALG_AAI_F8, OP_ALG_AS_INITFINAL, 0,
                dir);
  SEQFIFOLOAD(p, MSG1, 0, VLF | LAST1);
  SEQFIFOSTORE(p, MSG, 0, 0, VLF);
...
static inline int cnstr_shdsc_hmac(uint32_t *descbuf, bool ps, bool swap,
  ...
  /* Do operation */
  ALG_OPERATION(p, authdata->algtype, OP_ALG_AAI_HMAC, OP_ALG_AS_INITFINAL,
                opicv, dir);
 ...
 * Return: size of descriptor written in words or negative number on error
 */
static inline int cnstr_shdsc_hash(uint32_t *descbuf, bool ps, bool swap,
                                   enum rta_share_type share,
                                   struct alginfo *authdata, uint8_t do_icv,
                                   uint8_t trunc_len) {
  struct program prg;
  ...
  ALG_OPERATION(p, authdata->algtype, OP_ALG_AAI_HASH, OP_ALG_AS_INITFINAL,
                opicv, dir);
  SEQFIFOLOAD(p, MSG2, 0, VLF | LAST2);
...
static inline int cnstr_shdsc_kasumi_f8(uint32_t *descbuf, bool ps, bool swap,
  ...
  MATHB(p, SEQINSZ, SUB, MATH2, VSEQINSZ, 4, 0);
  MATHB(p, SEQINSZ, SUB, MATH2, VSEQOUTSZ, 4, 0);
  ALG_OPERATION(p, OP_ALG_ALGSEL_KASUMI, OP_ALG_AAI_F8, OP_ALG_AS_INITFINAL, 0,
                dir);
  SEQFIFOLOAD(p, MSG1, 0, VLF | LAST1);
  SEQFIFOSTORE(p, MSG, 0, 0, VLF);
...
static inline int cnstr_shdsc_kasumi_f9(uint32_t *descbuf, bool ps, bool swap,
                                        ...
                                        uint8_t chk_icv, uint32_t authlen) {
  struct program prg;
  struct program *p = &prg;
  int dir = chk_icv ? DIR_DEC : DIR_ENC;
  ...
  ALG_OPERATION(p, OP_ALG_ALGSEL_KASUMI, OP_ALG_AAI_F9, OP_ALG_AS_INITFINAL,
                chk_icv, dir);
...
static inline int cnstr_shdsc_crc(uint32_t *descbuf, bool swap) {
  ...
  MATHB(p, SEQINSZ, SUB, MATH2, VSEQINSZ, 4, 0);
  ALG_OPERATION(p, OP_ALG_ALGSEL_CRC, OP_ALG_AAI_802 | OP_ALG_AAI_DOC,
 ...
 * Return: size of descriptor written in words or negative number on error
 */
static inline int cnstr_shdsc_aes_mac(uint32_t *descbuf, bool ps, bool swap,
                                      enum rta_share_type share,
                                      struct alginfo *authdata, uint8_t do_icv,
                                      uint8_t trunc_len) {
  struct program prg;
  ...
  /* Do operation */
  ALG_OPERATION_NP(p, authdata->algtype, authdata->algmode, OP_ALG_AS_INITFINAL,
                   opicv, dir);
