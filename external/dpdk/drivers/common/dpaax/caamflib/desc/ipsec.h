...
__rta_copy_ipsec_encap_pdb(struct program *program, struct ipsec_encap_pdb *pdb,
                           uint32_t algtype) {
...
static inline unsigned int
__rta_copy_ipsec_decap_pdb(struct program *program, struct ipsec_decap_pdb *pdb,
                           uint32_t algtype) {
  unsigned int start_pc = program->current_pc;
  unsigned int i, ars;
  uint8_t mask;
 ...
 */
enum ipsec_icv_size { IPSEC_ICV_MD5_SIZE = 16, IPSEC_ICV_MD5_TRUNC_SIZE = 12 };
...
static inline void __gen_auth_key(struct program *program,
  ...
  case OP_PCL_IPSEC_HMAC_SHA2_512_256:
    ...
    break;
  default:
    KEY(program, KEY2, authdata->key_enc_flags, authdata->key, authdata->keylen,
        INLINE_KEY(authdata));
    return;
  }

  if (authdata->key_type == RTA_DATA_PTR)
    DKP_PROTOCOL(program, dkp_protid, OP_PCL_DKP_SRC_PTR, OP_PCL_DKP_DST_PTR,
                 (uint16_t)authdata->keylen, authdata->key, authdata->key_type);
  else
    DKP_PROTOCOL(program, dkp_protid, OP_PCL_DKP_SRC_IMM, OP_PCL_DKP_DST_IMM,
                 (uint16_t)authdata->keylen, authdata->key, authdata->key_type);
}
 ...
 * Return: size of descriptor written in words or negative number on error
 */
static inline int cnstr_shdsc_ipsec_encap_des_aes_xcbc(
    uint32_t *descbuf, enum rta_share_type share, struct ipsec_encap_pdb *pdb,
  ...
  SET_LABEL(p, keyjump);
  LOAD(p,
       LDST_SRCDST_WORD_CLRW | CLRW_CLR_C1MODE | CLRW_CLR_C1DATAS |
  ...
  MATHB(p, MATH1, AND, ~(CMD_SEQ_IN_PTR ^ CMD_SEQ_OUT_PTR), MATH1, 8,
        IFB | IMMED2);
  /*
  ...
  write_swapped_seqin_ptr = MOVE(p, MATH1, 0, DESCBUF, 0, 20, WAITCOMP | IMMED);
  swapped_seqin_ptr_jump = JUMP(p, swapped_seqin_ptr, LOCAL_JUMP, ALL_TRUE, 0);
  LOAD(p,
       LDST_SRCDST_WORD_CLRW | CLRW_CLR_C1MODE | CLRW_CLR_C1DATAS |
           CLRW_CLR_C1CTX | CLRW_CLR_C1KEY | CLRW_RESET_CLS1_CHA,
       CLRW, 0, 4, 0);
  ...
  MATHB(p, MATH0, SUB, (uint64_t)(pdb->ip_hdr_len + IPSEC_ICV_MD5_TRUNC_SIZE),
        VSEQINSZ, 4, IMMED2);
  MATHB(p, MATH0, SUB, IPSEC_ICV_MD5_TRUNC_SIZE, VSEQOUTSZ, 4, IMMED2);
  KEY(p, KEY1, authdata->key_enc_flags, authdata->key, authdata->keylen, 0);
  ALG_OPERATION(p, OP_ALG_ALGSEL_AES, OP_ALG_AAI_XCBC_MAC, OP_ALG_AS_INITFINAL,
  ...
  /*
   * TODO: RTA currently doesn't support adding labels in or after Job
   * Descriptor. To be changed when proper support is added in RTA.
   */
 ...
 * Return: size of descriptor written in words or negative number on error
 */
static inline int cnstr_shdsc_ipsec_decap_des_aes_xcbc(
    uint32_t *descbuf, enum rta_share_type share, struct ipsec_decap_pdb *pdb,
    struct alginfo *cipherdata, struct alginfo *authdata) {
  struct program prg;
  struct program *p = &prg;
  uint32_t ip_hdr_len =
      (pdb->options & PDBHDRLEN_MASK) >> PDBHDRLEN_ESP_DECAP_SHIFT;
  ...
  KEY(p, KEY1, authdata->key_enc_flags, authdata->key, authdata->keylen,
      INLINE_KEY(authdata));
  MATHB(p, SEQINSZ, SUB, (uint64_t)(ip_hdr_len + IPSEC_ICV_MD5_TRUNC_SIZE),
        MATH0, 4, IMMED2);
  ...
  MATHB(p, MATH1, OR, CMD_SEQ_IN_PTR ^ CMD_SEQ_OUT_PTR, MATH1, 8, IFB | IMMED2);
  /*
   * TODO: RTA currently doesn't support creating a LOAD command
   * with another command as IMM.
   * To be changed when proper support is added in RTA.
  ...
  MATHB(p, MATH3, SHLD, MATH3, MATH3, 8, 0);
  write_swapped_seqout_ptr =
      MOVE(p, MATH1, 0, DESCBUF, 0, 20, WAITCOMP | IMMED);
  swapped_seqout_ptr_jump =
      JUMP(p, swapped_seqout_ptr, LOCAL_JUMP, ALL_TRUE, 0);
  /*
  ...
  LOAD(p,
       ...
       CLRW, 0, 4, 0);
  SEQINPTR(p, 0, 65535, RTO);
  MATHB(p, MATH0, ADD, (uint64_t)(ip_hdr_len + IPSEC_ICV_MD5_TRUNC_SIZE),
        SEQINSZ, 4, IMMED2);
  KEY(p, KEY1, cipherdata->key_enc_flags, cipherdata->key, cipherdata->keylen,
      INLINE_KEY(cipherdata));
  PROTOCOL(p, OP_TYPE_DECAP_PROTOCOL, OP_PCLID_IPSEC,
 ...
 */
#define IPSEC_NEW_ENC_BASE_DESC_LEN                                            \
  (12 * CAAM_CMD_SZ + sizeof(struct ipsec_encap_pdb))
 ...
 * not. To be used as first parameter of rta_inline_query().
 */
#define IPSEC_NEW_NULL_ENC_BASE_DESC_LEN                                       \
  (11 * CAAM_CMD_SZ + sizeof(struct ipsec_encap_pdb))
 ...
 * Return: size of descriptor written in words or negative number on error
 */
static inline int cnstr_shdsc_ipsec_new_encap(
    uint32_t *descbuf, bool ps, bool swap, enum rta_share_type share,
    struct ipsec_encap_pdb *pdb, uint8_t *opt_ip_hdr,
    struct alginfo *cipherdata, struct alginfo *authdata) {
  struct program prg;
  struct program *p = &prg;
  ...
  MATHI(p, DPOVRD, RSHIFT, IPSEC_N_ENCAP_DPOVRD_L2_LEN_SHIFT, VSEQOUTSZ, 1, 0);
  MATHB(p, DPOVRD, AND, ~IPSEC_N_ENCAP_DPOVRD_L2_LEN_MASK, DPOVRD, 4, IMMED2);
  ...
  if (cipherdata->keylen)
    KEY(p, KEY1, cipherdata->key_enc_flags, cipherdata->key, cipherdata->keylen,
        INLINE_KEY(cipherdata));
  SET_LABEL(p, keyjmp);
  PROTOCOL(p, OP_TYPE_ENCAP_PROTOCOL, OP_PCLID_IPSEC_NEW,
           (uint16_t)(cipherdata->algtype | authdata->algtype));
 ...
 */
#define IPSEC_NEW_DEC_BASE_DESC_LEN                                            \
  (5 * CAAM_CMD_SZ + sizeof(struct ipsec_decap_pdb))
 ...
 * parameter of rta_inline_query().
 */
#define IPSEC_NEW_NULL_DEC_BASE_DESC_LEN                                       \
  (4 * CAAM_CMD_SZ + sizeof(struct ipsec_decap_pdb))
...
static inline int cnstr_shdsc_ipsec_new_decap(uint32_t *descbuf, bool ps,
  ...
  if (cipherdata->keylen)
    KEY(p, KEY1, cipherdata->key_enc_flags, cipherdata->key, cipherdata->keylen,
        INLINE_KEY(cipherdata));
  SET_LABEL(p, keyjmp);
  PROTOCOL(p, OP_TYPE_DECAP_PROTOCOL, OP_PCLID_IPSEC_NEW,
           (uint16_t)(cipherdata->algtype | authdata->algtype));
  PATCH_JUMP(p, pkeyjmp, keyjmp);
  PATCH_HDR(p, phdr, hdr);
  return PROGRAM_FINALIZE(p);
 ...
 * parameter of rta_inline_query().
 */
#define IPSEC_AUTH_AES_DEC_BASE_DESC_LEN                                       \
  (IPSEC_AUTH_BASE_DESC_LEN + CAAM_CMD_SZ)
...
static inline int cnstr_shdsc_authenc(uint32_t *descbuf, bool ps, bool swap,
                                      enum rta_share_type share,
                                      struct alginfo *cipherdata,
                                      struct alginfo *authdata, uint16_t ivlen,
  ...
  /* Do operation */
  ALG_OPERATION(p, authdata->algtype, authdata->algmode, OP_ALG_AS_INITFINAL,
                dir == DIR_ENC ? ICV_CHECK_DISABLE : ICV_CHECK_ENABLE, dir);
  ...
  if (need_dk) {
    ALG_OPERATION(p, OP_ALG_ALGSEL_AES, cipherdata->algmode | OP_ALG_AAI_DK,
                  OP_ALG_AS_INITFINAL, ICV_CHECK_DISABLE, dir);
