 ...
 */
static void rotate_8(unsigned char *arr) {
  uint32_t temp;
  uint32_t *first_half;
 ...
 */
static struct reg_node *read_memory_node(unsigned int *count) {
  int fd, ret, i;
  unsigned int j;
  glob_t result = {0};
  ...
  *count = 0;
  ...
  if (ret != 0) {
    DPAAX_DEBUG("Unable to glob device-tree memory node (err: %d)", ret);
    goto out;
  }
  ...
  DPAAX_DEBUG("Opening and parsing device-tree node: (%s)", result.gl_pathv[0]);
  ...
  ret = read(fd, file_data,
             statbuf.st_size > MEM_NODE_FILE_LEN ? MEM_NODE_FILE_LEN
                                                 : statbuf.st_size);
  if (ret <= 0) {
    DPAAX_DEBUG("Unable to read device-tree memory node: (%d)", ret);
    goto cleanup;
  }
  ...
  *count = (statbuf.st_size / 16);
  ...
  do {
    DPAAX_DEBUG("    %08" PRIx64 " %08zu", nodes[j].addr, nodes[j].len);
  } while (--j);
...
int dpaax_iova_table_populate(void) {
  ...
  for (i = 0; i < node_count; i++) {
    ...
    if (i > 0)
      entry[i].pages =
          entry[i - 1].pages + ((entry[i - 1].len / DPAAX_MEM_SPLIT));
    else
      entry[i].pages =
          (uint64_t *)((unsigned char *)entry +
                       (sizeof(struct dpaax_iovat_element) * node_count));

    DPAAX_DEBUG("\t(%u),(%8" PRIx64 "),(%8zu),(%8p)", i, entry[i].start,
                entry[i].len, entry[i].pages);
  }
...
int dpaax_iova_table_update(phys_addr_t paddr, void *vaddr, size_t length) {
  ...
  for (i = 0; i < dpaax_iova_table_p->count; i++) {
    if (align_paddr < entry[i].start) {
      ...
      DPAAX_ERR("Add: Incorrect entry for PA->VA Table"
                "(%" PRIu64 ")",
                paddr);
      DPAAX_ERR("Add: Lowest address: %" PRIu64 "", entry[i].start);
      return -1;
    }
    ...
    do {
      ...
      DPAAX_DEBUG("Added: vaddr=%zu for Phy:%" PRIu64 " at %zu"
                  " remaining len %zu",
                  align_vaddr, align_paddr, e_offset, req_length);
#endif
  ...
  DPAAX_DEBUG("Add: Found slot at (%" PRIu64 ")[(%zu)] for vaddr:(%p),"
              " phy(%" PRIu64 "), len(%zu)",
              entry[i].start, e_offset, vaddr, paddr, length);
#endif
  return 0;
}
...
void dpaax_iova_table_dump(void) {
  ...
  entry = dpaax_iova_table_p->entries;
  for (i = 0; i < dpaax_iova_table_p->count; i++) {
    DPAAX_DEBUG("\t(%16i),(%16" PRIu64 "),(%16zu),(%16p)", i, entry[i].start,
                entry[i].len, entry[i].pages);
    DPAAX_DEBUG("\t\t          (PA),          (VA)");
    for (j = 0; j < (entry->len / DPAAX_MEM_SPLIT); j++) {
      ...
      DPAAX_DEBUG("\t\t(%16" PRIx64 "),(%16" PRIx64 ")",
                  (entry[i].start + (j * sizeof(uint64_t))), entry[i].pages[j]);
    }
  }
  DPAAX_DEBUG(" === End of PA->VA Translation Table ===");
}

static void dpaax_memevent_cb(enum rte_mem_event type, const void *addr,
  ...
  while (cur_len < len) {
    ...
    DPAAX_DEBUG("Request for %s, va=%p, virt_addr=%p,"
                ...
                virt_addr, phys_addr, map_len);
#endif
    if (type == RTE_MEM_EVENT_ALLOC)
      ret = dpaax_iova_table_update(phys_addr, virt_addr, map_len);
    else
...
dpaax_memevent_walk_memsegs(const struct rte_memseg_list *msl __rte_unused,
  ...
  DPAAX_DEBUG("Walking for %p (pa=%" PRIu64 ") and len %zu", ms->addr, ms->iova,
              len);
