...
static void sso_hwgrp_irq(void *param) {
...
static int sso_hwgrp_register_irq(struct plt_intr_handle *handle,
                                  uint16_t ggrp_msixoff,
...
static void sso_hws_irq(void *param) {
  struct sso_rsrc *rsrc = param;
  uint64_t intr;
...
static int sso_hws_register_irq(struct plt_intr_handle *handle,
                                uint16_t hws_msixoff, struct sso_rsrc *rsrc) {
  int rc, vec;
...
int sso_register_irqs_priv(struct roc_sso *roc_sso,
  ...
  for (i = 0; i < nb_hws; i++) {
    if (sso->hws_msix_offset[i] == MSIX_VECTOR_INVALID) {
      plt_err("Invalid SSO HWS MSIX offset[%d] vector 0x%x", i,
              sso->hws_msix_offset[i]);
      goto fail;
    }
  }
  ...
  for (i = 0; i < nb_hwgrp; i++) {
    if (sso->hwgrp_msix_offset[i] == MSIX_VECTOR_INVALID) {
      plt_err("Invalid SSO HWGRP MSIX offset[%d] vector 0x%x", i,
              sso->hwgrp_msix_offset[i]);
      goto fail;
    }
  }

  for (i = 0; i < nb_hws; i++) {
    uintptr_t base = dev->bar2 + (RVU_BLOCK_ADDR_SSOW << 20 | i << 12);
  ...
  for (i = 0; i < nb_hwgrp; i++) {
    uintptr_t base = dev->bar2 + (RVU_BLOCK_ADDR_SSO << 20 | i << 12);
...
static void sso_hwgrp_unregister_irq(struct plt_intr_handle *handle,
                                     uint16_t ggrp_msixoff,
                                     struct sso_rsrc *rsrc) {
  int vec;
...
static void sso_hws_unregister_irq(struct plt_intr_handle *handle,
                                   uint16_t gws_msixoff,
                                   struct sso_rsrc *rsrc) {
  int vec;
...
void sso_unregister_irqs_priv(struct roc_sso *roc_sso,
  ...
  for (i = 0; i < nb_hws; i++)
    sso_hws_unregister_irq(handle, sso->hws_msix_offset[i], &sso->hws_rsrc[i]);
}
