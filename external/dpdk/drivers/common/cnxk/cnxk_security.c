...
static void ipsec_hmac_opad_ipad_gen(struct rte_crypto_sym_xform *auth_xform,
...
static int ot_ipsec_sa_common_param_fill(
    union roc_ot_ipsec_sa_word2 *w2, uint8_t *cipher_key, uint8_t *salt_key,
    uint8_t *hmac_opad_ipad, struct rte_security_ipsec_xform *ipsec_xfrm,
...
static size_t ot_ipsec_inb_ctx_size(struct roc_ot_ipsec_inb_sa *sa) {
  size_t size;
...
int cnxk_ot_ipsec_inb_sa_fill(struct roc_ot_ipsec_inb_sa *sa,
                              struct rte_security_ipsec_xform *ipsec_xfrm,
                              struct rte_crypto_sym_xform *crypto_xfrm,
                              bool is_inline) {
...
int cnxk_ot_ipsec_outb_sa_fill(struct roc_ot_ipsec_outb_sa *sa,
                               struct rte_security_ipsec_xform *ipsec_xfrm,
                               struct rte_crypto_sym_xform *crypto_xfrm) {
...
skip_tunnel_info:
  ...
  sa->w0.s.ctx_size =
      (PLT_ALIGN_CEIL(offset, ROC_CTX_UNIT_128B) / ROC_CTX_UNIT_128B) - 1;
...
bool cnxk_ot_ipsec_inb_sa_valid(struct roc_ot_ipsec_inb_sa *sa) {
  return !!sa->w2.s.valid;
}

bool cnxk_ot_ipsec_outb_sa_valid(struct roc_ot_ipsec_outb_sa *sa) {
  return !!sa->w2.s.valid;
}

static inline int ipsec_xfrm_verify(struct rte_security_ipsec_xform *ipsec_xfrm,
                                    struct rte_crypto_sym_xform *crypto_xfrm) {
...
int cnxk_onf_ipsec_inb_sa_fill(struct roc_onf_ipsec_inb_sa *sa,
                               struct rte_security_ipsec_xform *ipsec_xfrm,
                               struct rte_crypto_sym_xform *crypto_xfrm) {
  ...
  rc = onf_ipsec_sa_common_param_fill(ctl, sa->nonce, sa->cipher_key,
                                      sa->hmac_key, ipsec_xfrm, crypto_xfrm);
  if (rc)
    return rc;
...
int cnxk_onf_ipsec_outb_sa_fill(struct roc_onf_ipsec_outb_sa *sa,
                                struct rte_security_ipsec_xform *ipsec_xfrm,
                                struct rte_crypto_sym_xform *crypto_xfrm) {
  struct rte_security_ipsec_tunnel_param *tunnel = &ipsec_xfrm->tunnel;
  struct roc_ie_onf_sa_ctl *ctl = &sa->ctl;
  ...
  switch (tunnel->type) {
  case RTE_SECURITY_IPSEC_TUNNEL_IPV4:
    memcpy(&sa->ip_src, &tunnel->ipv4.src_ip, sizeof(struct in_addr));
    memcpy(&sa->ip_dst, &tunnel->ipv4.dst_ip, sizeof(struct in_addr));
    break;
  case RTE_SECURITY_IPSEC_TUNNEL_IPV6:
    return -ENOTSUP;
  default:
...
bool cnxk_onf_ipsec_inb_sa_valid(struct roc_onf_ipsec_inb_sa *sa) {
  return !!sa->ctl.valid;
}

bool cnxk_onf_ipsec_outb_sa_valid(struct roc_onf_ipsec_outb_sa *sa) {
  return !!sa->ctl.valid;
}

uint8_t cnxk_ipsec_ivlen_get(enum rte_crypto_cipher_algorithm c_algo,
...
uint8_t cnxk_ipsec_icvlen_get(enum rte_crypto_cipher_algorithm c_algo,
                              enum rte_crypto_auth_algorithm a_algo,
                              enum rte_crypto_aead_algorithm aead_algo) {
  uint8_t icv = 0;
...
uint8_t cnxk_ipsec_outb_roundup_byte(enum rte_crypto_cipher_algorithm c_algo,
                                     enum rte_crypto_aead_algorithm aead_algo) {
...
int cnxk_ipsec_outb_rlens_get(struct cnxk_ipsec_outb_rlens *rlens,
                              struct rte_security_ipsec_xform *ipsec_xfrm,
                              struct rte_crypto_sym_xform *crypto_xfrm) {
  struct rte_security_ipsec_tunnel_param *tunnel = &ipsec_xfrm->tunnel;
  ...
  if (crypto_xfrm->type == RTE_CRYPTO_SYM_XFORM_AEAD) {
    ...
    if (crypto_xfrm->next) {
      if (crypto_xfrm->next->type == RTE_CRYPTO_SYM_XFORM_CIPHER)
