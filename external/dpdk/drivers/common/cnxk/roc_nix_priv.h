...
#define NIX_TM_MARK_EN_MASK                                                    \
  (NIX_TM_MARK_IP_DSCP_EN | NIX_TM_MARK_IP_ECN_EN | NIX_TM_MARK_VLAN_DEI_EN)
...
static inline struct nix *roc_nix_to_nix_priv(struct roc_nix *roc_nix) {
  return (struct nix *)&roc_nix->reserved[0];
}

static inline struct roc_nix *nix_priv_to_roc_nix(struct nix *nix) {
  return (struct roc_nix *)((char *)nix - offsetof(struct roc_nix, reserved));
}
...
#define NIX_TM_TREE_MASK_ALL                                                   \
  (BIT(ROC_NIX_TM_DEFAULT) | BIT(ROC_NIX_TM_RLIMIT) | BIT(ROC_NIX_TM_USER))
...
static inline uint64_t nix_tm_weight_to_rr_quantum(uint64_t weight) {
...
static inline bool nix_tm_have_tl1_access(struct nix *nix) {
  return !!(nix->tm_flags & NIX_TM_TL1_ACCESS);
}

static inline bool nix_tm_is_leaf(struct nix *nix, int lvl) {
...
static inline const char *nix_tm_hwlvl2str(uint32_t hw_lvl) {
  switch (hw_lvl) {
...
static inline const char *nix_tm_tree2str(enum roc_nix_tm_tree tree) {
