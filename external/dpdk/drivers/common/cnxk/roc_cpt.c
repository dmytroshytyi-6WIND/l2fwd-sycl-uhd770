...
static void cpt_lf_misc_intr_enb_dis(struct roc_cpt_lf *lf, bool enb) {
  /* Enable all cpt lf error irqs except RQ_DISABLED and CQ_DISABLED */
  if (enb)
    plt_write64(
        (BIT_ULL(6) | BIT_ULL(5) | BIT_ULL(3) | BIT_ULL(2) | BIT_ULL(1)),
...
static void cpt_lf_misc_irq(void *param) {
...
static int cpt_lf_register_misc_irq(struct roc_cpt_lf *lf) {
...
static void cpt_lf_unregister_misc_irq(struct roc_cpt_lf *lf) {
  struct plt_pci_device *pci_dev = lf->pci_dev;
  struct plt_intr_handle *handle;
  int vec;
...
static void cpt_lf_done_intr_enb_dis(struct roc_cpt_lf *lf, bool enb) {
  if (enb)
...
static void cpt_lf_done_irq(void *param) {
...
static int cpt_lf_register_done_irq(struct roc_cpt_lf *lf) {
  struct plt_pci_device *pci_dev = lf->pci_dev;
  struct plt_intr_handle *handle;
  int rc, vec;
...
static void cpt_lf_unregister_done_irq(struct roc_cpt_lf *lf) {
  struct plt_pci_device *pci_dev = lf->pci_dev;
  struct plt_intr_handle *handle;
  int vec;
...
static int cpt_lf_register_irqs(struct roc_cpt_lf *lf) {
  int rc;

  if (lf->msixoff == MSIX_VECTOR_INVALID) {
    plt_err("Invalid CPTLF MSIX vector offset vector: 0x%x", lf->msixoff);
    return -EINVAL;
  }
...
static void cpt_lf_unregister_irqs(struct roc_cpt_lf *lf) {
  cpt_lf_unregister_misc_irq(lf);
  cpt_lf_unregister_done_irq(lf);
}

static void cpt_lf_dump(struct roc_cpt_lf *lf) {
  plt_cpt_dbg("CPT LF");
...
int roc_cpt_inline_ipsec_cfg(struct dev *cpt_dev, uint8_t lf_id,
...
int roc_cpt_inline_ipsec_inb_cfg(struct roc_cpt *roc_cpt, uint16_t param1,
                                 uint16_t param2) {
...
int roc_cpt_rxc_time_cfg(struct roc_cpt *roc_cpt,
...
static int cpt_available_lfs_get(struct dev *dev, uint16_t *nb_lf) {
  struct mbox *mbox = dev->mbox;
...
int cpt_lfs_alloc(struct dev *dev, uint8_t eng_grpmsk, uint8_t blkaddr,
                  bool inl_dev_sso) {
...
static int cpt_hardware_caps_get(struct dev *dev, struct roc_cpt *roc_cpt) {
...
static uint32_t cpt_lf_iq_mem_calc(uint32_t nb_desc) {
  uint32_t len;
  ...
  len += PLT_ALIGN(CPT_IQ_NB_DESC_SIZE_DIV40(nb_desc) *
                       CPT_IQ_NB_DESC_MULTIPLIER * sizeof(struct cpt_inst_s),
                   ROC_ALIGN);

  return len;
}

static inline void cpt_iq_init(struct roc_cpt_lf *lf) {
...
int roc_cpt_dev_configure(struct roc_cpt *roc_cpt, int nb_lf) {
  struct cpt *cpt = roc_cpt_to_cpt_priv(roc_cpt);
  ...
  for (i = 0; i < nb_lf; i++)
    cpt->lf_msix_off[i] = (cpt->lf_blkaddr[i] == RVU_BLOCK_ADDR_CPT1)
...
int roc_cpt_lf_init(struct roc_cpt *roc_cpt, struct roc_cpt_lf *lf) {
  struct cpt *cpt = roc_cpt_to_cpt_priv(roc_cpt);
  int rc;
...
int roc_cpt_dev_init(struct roc_cpt *roc_cpt) {
...
int roc_cpt_lf_ctx_flush(struct roc_cpt_lf *lf, void *cptr, bool inval) {
...
int roc_cpt_lf_ctx_reload(struct roc_cpt_lf *lf, void *cptr) {
...
void roc_cpt_lf_fini(struct roc_cpt_lf *lf) {
  if (lf == NULL)
    return;
...
int roc_cpt_dev_fini(struct roc_cpt *roc_cpt) {
  struct cpt *cpt = roc_cpt_to_cpt_priv(roc_cpt);
...
void roc_cpt_dev_clear(struct roc_cpt *roc_cpt) {
  struct cpt *cpt = roc_cpt_to_cpt_priv(roc_cpt);
  int i;
...
int roc_cpt_eng_grp_add(struct roc_cpt *roc_cpt, enum cpt_eng_type eng_type) {
  struct cpt *cpt = roc_cpt_to_cpt_priv(roc_cpt);
  struct dev *dev = &cpt->dev;
...
void roc_cpt_iq_disable(struct roc_cpt_lf *lf) {
...
void roc_cpt_iq_enable(struct roc_cpt_lf *lf) {
...
int roc_cpt_lmtline_init(struct roc_cpt *roc_cpt,
...
int roc_cpt_ctx_write(struct roc_cpt_lf *lf, void *sa_dptr, void *sa_cptr,
                      uint16_t sa_len) {
