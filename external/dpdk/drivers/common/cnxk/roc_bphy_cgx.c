...
static uint64_t roc_bphy_cgx_read(struct roc_bphy_cgx *roc_cgx, uint64_t lmac,
                                  uint64_t offset) {
...
static void roc_bphy_cgx_write(struct roc_bphy_cgx *roc_cgx, uint64_t lmac,
...
static void roc_bphy_cgx_ack(struct roc_bphy_cgx *roc_cgx, unsigned int lmac,
                             uint64_t *scr0) {
  uint64_t val;
...
static int roc_bphy_cgx_wait_for_ownership(struct roc_bphy_cgx *roc_cgx,
                                           unsigned int lmac, uint64_t *scr0) {
  int tries = 5000;
  uint64_t scr1;
...
static int roc_bphy_cgx_wait_for_ack(struct roc_bphy_cgx *roc_cgx,
                                     unsigned int lmac, uint64_t *scr0) {
  int tries = 5000;
  uint64_t scr1;
...
static int roc_bphy_cgx_intf_req(struct roc_bphy_cgx *roc_cgx,
                                 unsigned int lmac, uint64_t scr1,
                                 uint64_t *scr0) {
...
static unsigned int roc_bphy_cgx_dev_id(struct roc_bphy_cgx *roc_cgx) {
  uint64_t cgx_id =
      roc_model_is_cn10k() ? GENMASK_ULL(26, 24) : GENMASK_ULL(25, 24);

  return FIELD_GET(cgx_id, roc_cgx->bar0_pa);
}

int roc_bphy_cgx_dev_init(struct roc_bphy_cgx *roc_cgx) {
  uint64_t val;
  int ret;
...
int roc_bphy_cgx_dev_fini(struct roc_bphy_cgx *roc_cgx) {
  if (!roc_cgx)
    return -EINVAL;
...
static bool roc_bphy_cgx_lmac_exists(struct roc_bphy_cgx *roc_cgx,
                                     unsigned int lmac) {
  return (lmac < MAX_LMACS_PER_CGX) && (roc_cgx->lmac_bmap & BIT_ULL(lmac));
}

static int roc_bphy_cgx_start_stop_rxtx(struct roc_bphy_cgx *roc_cgx,
                                        unsigned int lmac, bool start) {
  uint64_t val;
  ...
  val &= ~(CGX_CMRX_CONFIG_DATA_PKT_RX_EN | CGX_CMRX_CONFIG_DATA_PKT_TX_EN);
...
static int roc_bphy_cgx_intlbk_ena_dis(struct roc_bphy_cgx *roc_cgx,
                                       unsigned int lmac, bool enable) {
  uint64_t scr1, scr0;
...
static int roc_bphy_cgx_ptp_rx_ena_dis(struct roc_bphy_cgx *roc_cgx,
                                       unsigned int lmac, bool enable) {
  uint64_t scr1, scr0;
...
int roc_bphy_cgx_start_rxtx(struct roc_bphy_cgx *roc_cgx, unsigned int lmac) {
  return roc_bphy_cgx_start_stop_rxtx(roc_cgx, lmac, true);
}

int roc_bphy_cgx_stop_rxtx(struct roc_bphy_cgx *roc_cgx, unsigned int lmac) {
  return roc_bphy_cgx_start_stop_rxtx(roc_cgx, lmac, false);
}

int roc_bphy_cgx_set_link_state(struct roc_bphy_cgx *roc_cgx, unsigned int lmac,
                                bool state) {
  uint64_t scr1, scr0;
...
int roc_bphy_cgx_get_linkinfo(struct roc_bphy_cgx *roc_cgx, unsigned int lmac,
...
int roc_bphy_cgx_set_link_mode(struct roc_bphy_cgx *roc_cgx, unsigned int lmac,
...
int roc_bphy_cgx_intlbk_enable(struct roc_bphy_cgx *roc_cgx,
                               unsigned int lmac) {
  return roc_bphy_cgx_intlbk_ena_dis(roc_cgx, lmac, true);
}

int roc_bphy_cgx_intlbk_disable(struct roc_bphy_cgx *roc_cgx,
                                unsigned int lmac) {
  return roc_bphy_cgx_intlbk_ena_dis(roc_cgx, lmac, false);
}

int roc_bphy_cgx_ptp_rx_enable(struct roc_bphy_cgx *roc_cgx,
                               unsigned int lmac) {
  return roc_bphy_cgx_ptp_rx_ena_dis(roc_cgx, lmac, true);
}

int roc_bphy_cgx_ptp_rx_disable(struct roc_bphy_cgx *roc_cgx,
                                unsigned int lmac) {
  return roc_bphy_cgx_ptp_rx_ena_dis(roc_cgx, lmac, false);
}

int roc_bphy_cgx_fec_set(struct roc_bphy_cgx *roc_cgx, unsigned int lmac,
...
int roc_bphy_cgx_fec_supported_get(struct roc_bphy_cgx *roc_cgx,
                                   unsigned int lmac,
                                   enum roc_bphy_cgx_eth_link_fec *fec) {
  uint64_t scr1, scr0;
  int ret;
