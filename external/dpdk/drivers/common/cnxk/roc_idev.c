...
uint16_t idev_sso_pffunc_get(void) {
  ...
  sso_pf_func = 0;
  if (idev != NULL)
    sso_pf_func = __atomic_load_n(&idev->sso_pf_func, __ATOMIC_ACQUIRE);
...
void idev_sso_pffunc_set(uint16_t sso_pf_func) {
  ...
  idev = idev_get_cfg();
  if (idev != NULL)
    __atomic_store_n(&idev->sso_pf_func, sso_pf_func, __ATOMIC_RELEASE);
}
...
uint32_t roc_idev_npa_maxpools_get(void) {
  struct idev_cfg *idev;
  uint32_t max_pools;
...
void roc_idev_npa_maxpools_set(uint32_t max_pools) {
  struct idev_cfg *idev;
...
uint64_t roc_idev_lmt_base_addr_get(void) {
...
uint16_t roc_idev_num_lmtlines_get(void) {
  struct idev_cfg *idev;
...
struct roc_cpt *roc_idev_cpt_get(void) {
...
uint64_t *roc_nix_inl_outb_ring_base_get(struct roc_nix *roc_nix) {
...
void roc_idev_cpt_set(struct roc_cpt *cpt) {
  struct idev_cfg *idev = idev_get_cfg();
...
struct roc_nix *roc_idev_npa_nix_get(void) {
