...
static inline struct mbox *get_mbox(struct roc_nix *roc_nix) {
...
static inline uint64_t meter_rate_to_nix(uint64_t value, uint64_t *exponent_p,
                                         uint64_t *mantissa_p,
                                         uint64_t *div_exp_p,
                                         uint32_t timeunit_p) {
...
static inline uint64_t meter_burst_to_nix(uint64_t value, uint64_t *exponent_p,
                                          uint64_t *mantissa_p) {
...
static inline void nix_lf_bpf_dump(__io struct nix_band_prof_s *bpf) {
  ...
  plt_dump("W6: red_pkt_pass \t\t\t0x%" PRIx64 "", (uint64_t)bpf->red_pkt_pass);
...
static inline void nix_precolor_conv_table_write(struct roc_nix *roc_nix,
...
nix_precolor_inner_dscp_table_update(struct roc_nix *roc_nix,
  ...
  nix_precolor_conv_table_write(roc_nix, val_lo, NIX_LF_RX_IIP_COLOR_CONV_LO);
  nix_precolor_conv_table_write(roc_nix, val_hi, NIX_LF_RX_IIP_COLOR_CONV_HI);
...
nix_precolor_outer_dscp_table_update(struct roc_nix *roc_nix,
  ...
  nix_precolor_conv_table_write(roc_nix, val_lo, NIX_LF_RX_OIP_COLOR_CONV_LO);
  nix_precolor_conv_table_write(roc_nix, val_hi, NIX_LF_RX_OIP_COLOR_CONV_HI);

  return false;
}

static uint8_t nix_precolor_gen_table_update(struct roc_nix *roc_nix,
                                             struct roc_nix_bpf_precolor *tbl) {
  uint64_t val = 0, i;
  uint8_t tn_ena;
  uint32_t off;
...
uint8_t roc_nix_bpf_level_to_idx(enum roc_nix_bpf_level_flag level_f) {
  uint8_t idx;
...
uint8_t roc_nix_bpf_stats_to_idx(enum roc_nix_bpf_stats level_f) {
  uint8_t idx;
...
int roc_nix_bpf_timeunit_get(struct roc_nix *roc_nix, uint32_t *time_unit) {
...
int roc_nix_bpf_count_get(struct roc_nix *roc_nix, uint8_t lvl_mask,
...
int roc_nix_bpf_alloc(struct roc_nix *roc_nix, uint8_t lvl_mask,
  ...
  if (leaf_idx != ROC_NIX_BPF_LEVEL_IDX_INVALID) {
    req->prof_count[sw_to_hw_lvl_map[leaf_idx]] = per_lvl_cnt[leaf_idx];
  }

  if (mid_idx != ROC_NIX_BPF_LEVEL_IDX_INVALID) {
    req->prof_count[sw_to_hw_lvl_map[mid_idx]] = per_lvl_cnt[mid_idx];
  }

  if (top_idx != ROC_NIX_BPF_LEVEL_IDX_INVALID) {
    req->prof_count[sw_to_hw_lvl_map[top_idx]] = per_lvl_cnt[top_idx];
  }
  ...
  if (leaf_idx != ROC_NIX_BPF_LEVEL_IDX_INVALID) {
    ...
    profs[leaf_idx].count = rsp->prof_count[sw_to_hw_lvl_map[leaf_idx]];
    for (i = 0; i < profs[leaf_idx].count; i++) {
      profs[leaf_idx].ids[i] = rsp->prof_idx[sw_to_hw_lvl_map[leaf_idx]][i];
    }
  }

  if (mid_idx != ROC_NIX_BPF_LEVEL_IDX_INVALID) {
    profs[mid_idx].level = mid_idx;
    profs[mid_idx].count = rsp->prof_count[sw_to_hw_lvl_map[mid_idx]];
    for (i = 0; i < profs[mid_idx].count; i++) {
      profs[mid_idx].ids[i] = rsp->prof_idx[sw_to_hw_lvl_map[mid_idx]][i];
    }
  }

  if (top_idx != ROC_NIX_BPF_LEVEL_IDX_INVALID) {
    profs[top_idx].level = top_idx;
    profs[top_idx].count = rsp->prof_count[sw_to_hw_lvl_map[top_idx]];
    for (i = 0; i < profs[top_idx].count; i++) {
      profs[top_idx].ids[i] = rsp->prof_idx[sw_to_hw_lvl_map[top_idx]][i];
    }
  }
...
int roc_nix_bpf_free_all(struct roc_nix *roc_nix) {
  struct mbox *mbox = get_mbox(roc_nix);
  struct nix_bandprof_free_req *req;
...
int roc_nix_bpf_config(struct roc_nix *roc_nix, uint16_t id,
  ...
  case ROC_NIX_BPF_ALGO_2697:
    meter_rate_to_nix(cfg->algo2697.cir, &exponent_p, &mantissa_p, &div_exp_p,
                      policer_timeunit);
  ...
  case ROC_NIX_BPF_ALGO_2698:
    meter_rate_to_nix(cfg->algo2698.cir, &exponent_p, &mantissa_p, &div_exp_p,
                      policer_timeunit);
    aq->prof.cir_mantissa = mantissa_p;
    aq->prof.cir_exponent = exponent_p;

    meter_rate_to_nix(cfg->algo2698.pir, &exponent_p, &mantissa_p, &div_exp_p,
                      policer_timeunit);
  ...
  case ROC_NIX_BPF_ALGO_4115:
    meter_rate_to_nix(cfg->algo4115.cir, &exponent_p, &mantissa_p, &div_exp_p,
                      policer_timeunit);
    aq->prof.cir_mantissa = mantissa_p;
    aq->prof.cir_exponent = exponent_p;

    meter_rate_to_nix(cfg->algo4115.eir, &exponent_p, &mantissa_p, &div_exp_p,
                      policer_timeunit);
    aq->prof.peir_mantissa = mantissa_p;
    aq->prof.peir_exponent = exponent_p;
...
int roc_nix_bpf_ena_dis(struct roc_nix *roc_nix, uint16_t id,
...
int roc_nix_bpf_dump(struct roc_nix *roc_nix, uint16_t id,
                     enum roc_nix_bpf_level_flag lvl_flag) {
  struct mbox *mbox = get_mbox(roc_nix);
...
int roc_nix_bpf_pre_color_tbl_setup(struct roc_nix *roc_nix, uint16_t id,
                                    enum roc_nix_bpf_level_flag lvl_flag,
                                    struct roc_nix_bpf_precolor *tbl) {
  struct mbox *mbox = get_mbox(roc_nix);
  struct nix_cn10k_aq_enq_req *aq;
  ...
  case ROC_NIX_BPF_PC_MODE_VLAN_OUTER:
    if (tbl->count != NIX_BPF_PRECOLOR_VLAN_TABLE_SIZE) {
      plt_err("Table size must be %d", NIX_BPF_PRECOLOR_VLAN_TABLE_SIZE);
      rc = NIX_ERR_PARAM;
      goto exit;
    }
  ...
  case ROC_NIX_BPF_PC_MODE_DSCP_INNER:
    if (tbl->count != NIX_BPF_PRECOLOR_DSCP_TABLE_SIZE) {
      plt_err("Table size must be %d", NIX_BPF_PRECOLOR_DSCP_TABLE_SIZE);
      rc = NIX_ERR_PARAM;
      goto exit;
    }
  ...
  case ROC_NIX_BPF_PC_MODE_GEN_OUTER:
    if (tbl->count != NIX_BPF_PRECOLOR_GEN_TABLE_SIZE) {
      plt_err("Table size must be %d", NIX_BPF_PRECOLOR_GEN_TABLE_SIZE);
      rc = NIX_ERR_PARAM;
      goto exit;
    }
...
int roc_nix_bpf_connect(struct roc_nix *roc_nix,
...
int roc_nix_bpf_stats_read(struct roc_nix *roc_nix, uint16_t id, uint64_t mask,
                           enum roc_nix_bpf_level_flag lvl_flag,
  ...
  red_pkt_pass = roc_nix_bpf_stats_to_idx(mask & ROC_NIX_BPF_RED_PKT_F_PASS);
  red_octs_pass = roc_nix_bpf_stats_to_idx(mask & ROC_NIX_BPF_RED_OCTS_F_PASS);
  red_pkt_drop = roc_nix_bpf_stats_to_idx(mask & ROC_NIX_BPF_RED_PKT_F_DROP);
  red_octs_drop = roc_nix_bpf_stats_to_idx(mask & ROC_NIX_BPF_RED_OCTS_F_DROP);
...
int roc_nix_bpf_stats_reset(struct roc_nix *roc_nix, uint16_t id, uint64_t mask,
                            enum roc_nix_bpf_level_flag lvl_flag) {
  struct mbox *mbox = get_mbox(roc_nix);
  struct nix_cn10k_aq_enq_req *aq;
  uint8_t level_idx;
  ...
  if (mask & ROC_NIX_BPF_GREEN_OCTS_F_PASS) {
    ...
    aq->prof_mask.green_octs_pass = ~(aq->prof_mask.green_octs_pass);
  }
  ...
  if (mask & ROC_NIX_BPF_GREEN_OCTS_F_DROP) {
    ...
    aq->prof_mask.green_octs_drop = ~(aq->prof_mask.green_octs_drop);
  }
  if (mask & ROC_NIX_BPF_YELLOW_PKT_F_PASS) {
    aq->prof.yellow_pkt_pass = 0;
    aq->prof_mask.yellow_pkt_pass = ~(aq->prof_mask.yellow_pkt_pass);
  }
  if (mask & ROC_NIX_BPF_YELLOW_OCTS_F_PASS) {
    aq->prof.yellow_octs_pass = 0;
    aq->prof_mask.yellow_octs_pass = ~(aq->prof_mask.yellow_octs_pass);
  }
  if (mask & ROC_NIX_BPF_YELLOW_PKT_F_DROP) {
    aq->prof.yellow_pkt_drop = 0;
    aq->prof_mask.yellow_pkt_drop = ~(aq->prof_mask.yellow_pkt_drop);
  }
  if (mask & ROC_NIX_BPF_YELLOW_OCTS_F_DROP) {
    aq->prof.yellow_octs_drop = 0;
    aq->prof_mask.yellow_octs_drop = ~(aq->prof_mask.yellow_octs_drop);
  }
...
int roc_nix_bpf_lf_stats_read(struct roc_nix *roc_nix, uint64_t mask,
                              uint64_t stats[ROC_NIX_BPF_STATS_MAX]) {
  uint8_t yellow_pkt_pass, yellow_octs_pass, yellow_pkt_drop;
  uint8_t green_octs_drop, yellow_octs_drop, red_octs_drop;
  uint8_t green_pkt_pass, green_octs_pass, green_pkt_drop;
  ...
  if (green_pkt_pass != ROC_NIX_BPF_STATS_MAX) {
    stats[green_pkt_pass] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_GC_OCTS_PASSED);
  }

  if (green_octs_pass != ROC_NIX_BPF_STATS_MAX) {
    stats[green_octs_pass] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_YC_PKTS_PASSED);
  }

  if (green_pkt_drop != ROC_NIX_BPF_STATS_MAX) {
    stats[green_pkt_drop] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_GC_OCTS_DROP);
  }

  if (green_octs_drop != ROC_NIX_BPF_STATS_MAX) {
    stats[green_octs_drop] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_YC_PKTS_DROP);
  }

  if (yellow_pkt_pass != ROC_NIX_BPF_STATS_MAX) {
    stats[yellow_pkt_pass] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_GC_PKTS_PASSED);
  }

  if (yellow_octs_pass != ROC_NIX_BPF_STATS_MAX) {
    stats[yellow_octs_pass] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_RC_OCTS_PASSED);
  }

  if (yellow_pkt_drop != ROC_NIX_BPF_STATS_MAX) {
    stats[yellow_pkt_drop] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_GC_PKTS_DROP);
  }

  if (yellow_octs_drop != ROC_NIX_BPF_STATS_MAX) {
    stats[yellow_octs_drop] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_RC_OCTS_DROP);
  }

  if (red_pkt_pass != ROC_NIX_BPF_STATS_MAX) {
    stats[red_pkt_pass] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_YC_OCTS_PASSED);
  }

  if (red_octs_pass != ROC_NIX_BPF_STATS_MAX) {
    stats[red_octs_pass] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_RC_PKTS_PASSED);
  }

  if (red_pkt_drop != ROC_NIX_BPF_STATS_MAX) {
    stats[red_pkt_drop] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_YC_OCTS_DROP);
  }

  if (red_octs_drop != ROC_NIX_BPF_STATS_MAX) {
    stats[red_octs_drop] = NIX_RD_STATS(NIX_STAT_LF_RX_RX_RC_PKTS_DROP);
  }

  return 0;
}

int roc_nix_bpf_lf_stats_reset(struct roc_nix *roc_nix, uint64_t mask) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
