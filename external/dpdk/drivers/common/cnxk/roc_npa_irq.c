...
static void npa_err_irq(void *param) {
...
static int npa_register_err_irq(struct npa_lf *lf) {
...
static void npa_unregister_err_irq(struct npa_lf *lf) {
  struct plt_intr_handle *handle = lf->intr_handle;
  int vec;
...
static void npa_ras_irq(void *param) {
  struct npa_lf *lf = (struct npa_lf *)param;
  uint64_t intr;
...
static int npa_register_ras_irq(struct npa_lf *lf) {
  struct plt_intr_handle *handle = lf->intr_handle;
  int rc, vec;
...
static void npa_unregister_ras_irq(struct npa_lf *lf) {
  int vec;
  struct plt_intr_handle *handle = lf->intr_handle;
...
static inline uint8_t npa_q_irq_get_and_clear(struct npa_lf *lf, uint32_t q,
...
static inline uint8_t npa_pool_irq_get_and_clear(struct npa_lf *lf,
                                                 uint32_t p) {
  return npa_q_irq_get_and_clear(lf, p, NPA_LF_POOL_OP_INT, ~0xff00);
}

static inline uint8_t npa_aura_irq_get_and_clear(struct npa_lf *lf,
                                                 uint32_t a) {
  return npa_q_irq_get_and_clear(lf, a, NPA_LF_AURA_OP_INT, ~0xff00);
}

static void npa_q_irq(void *param) {
...
static int npa_register_queue_irqs(struct npa_lf *lf) {
  struct plt_intr_handle *handle = lf->intr_handle;
  int vec, q, qs, rc = 0;
...
static void npa_unregister_queue_irqs(struct npa_lf *lf) {
  struct plt_intr_handle *handle = lf->intr_handle;
  int vec, q, qs;
...
int npa_register_irqs(struct npa_lf *lf) {
  ...
  if (lf->npa_msixoff == MSIX_VECTOR_INVALID) {
    plt_err("Invalid NPALF MSIX vector offset vector: 0x%x", lf->npa_msixoff);
