...
static int npc_mcam_alloc_counter(struct npc *npc, uint16_t *ctr) {
...
static int npc_supp_key_len(uint32_t supp_mask) {
  int nib_count = 0;
 ...
 */
static int npc_lid_lt_in_kex(struct npc *npc, uint8_t lid, uint8_t lt) {
...
static void npc_construct_ldata_mask(struct npc *npc, struct plt_bitmap *bmap,
                                     uint8_t lid, uint8_t lt, uint8_t ld) {
 ...
 */
static bool npc_is_kex_enabled(struct npc *npc, uint8_t lid, uint8_t lt,
                               int offset, int len) {
...
uint64_t npc_get_kex_capability(struct npc *npc) {
  ...
  kex_cap.bit.ethtype_0 =
      npc_is_kex_enabled(npc, NPC_LID_LA, NPC_LT_LA_ETHER, 12 * 8, 2 * 8);
  ...
  kex_cap.bit.vlan_id_0 =
      npc_is_kex_enabled(npc, NPC_LID_LB, NPC_LT_LB_CTAG, 2 * 8, 2 * 8);
  ...
  kex_cap.bit.dmac =
      npc_is_kex_enabled(npc, NPC_LID_LA, NPC_LT_LA_ETHER, 0 * 8, 6 * 8);
  ...
  kex_cap.bit.udp_dport =
      npc_is_kex_enabled(npc, NPC_LID_LD, NPC_LT_LD_UDP, 2 * 8, 2 * 8);
  /* UDP sport: offset 0B, len 2B */
  kex_cap.bit.udp_sport =
      npc_is_kex_enabled(npc, NPC_LID_LD, NPC_LT_LD_UDP, 0 * 8, 2 * 8);
  /* TCP dport: offset 2B, len 2B */
  kex_cap.bit.tcp_dport =
      npc_is_kex_enabled(npc, NPC_LID_LD, NPC_LT_LD_TCP, 2 * 8, 2 * 8);
  /* TCP sport: offset 0B, len 2B */
  kex_cap.bit.tcp_sport =
      npc_is_kex_enabled(npc, NPC_LID_LD, NPC_LT_LD_TCP, 0 * 8, 2 * 8);
  /* IP SIP: offset 12B, len 4B */
  kex_cap.bit.sip_addr =
      npc_is_kex_enabled(npc, NPC_LID_LC, NPC_LT_LC_IP, 12 * 8, 4 * 8);
  /* IP DIP: offset 14B, len 4B */
  kex_cap.bit.dip_addr =
      npc_is_kex_enabled(npc, NPC_LID_LC, NPC_LT_LC_IP, 14 * 8, 4 * 8);
  /* IP6 SIP: offset 8B, len 16B */
  kex_cap.bit.sip6_addr =
      npc_is_kex_enabled(npc, NPC_LID_LC, NPC_LT_LC_IP6, 8 * 8, 16 * 8);
  /* IP6 DIP: offset 24B, len 16B */
  kex_cap.bit.dip6_addr =
      npc_is_kex_enabled(npc, NPC_LID_LC, NPC_LT_LC_IP6, 24 * 8, 16 * 8);
  /* ESP SPI: offset 0B, len 4B */
  kex_cap.bit.ipsec_spi =
      npc_is_kex_enabled(npc, NPC_LID_LE, NPC_LT_LE_ESP, 0 * 8, 4 * 8);
  /* VXLAN VNI: offset 4B, len 3B */
  kex_cap.bit.ld_vni =
      npc_is_kex_enabled(npc, NPC_LID_LE, NPC_LT_LE_VXLAN, 0 * 8, 3 * 8);
  ...
  kex_cap.bit.sctp_sport =
      npc_is_kex_enabled(npc, NPC_LID_LD, NPC_LT_LD_SCTP, 0 * 8, 2 * 8);
  /* SCTP dport : offset 2B, len 2B */
  kex_cap.bit.sctp_dport =
      npc_is_kex_enabled(npc, NPC_LID_LD, NPC_LT_LD_SCTP, 2 * 8, 2 * 8);
  /* ICMP type : offset 0B, len 1B */
  kex_cap.bit.icmp_type =
      npc_is_kex_enabled(npc, NPC_LID_LD, NPC_LT_LD_ICMP, 0 * 8, 1 * 8);
  /* ICMP code : offset 1B, len 1B */
  kex_cap.bit.icmp_code =
      npc_is_kex_enabled(npc, NPC_LID_LD, NPC_LT_LD_ICMP, 1 * 8, 1 * 8);
  /* ICMP id : offset 4B, len 2B */
  kex_cap.bit.icmp_id =
      npc_is_kex_enabled(npc, NPC_LID_LD, NPC_LT_LD_ICMP, 4 * 8, 2 * 8);
  ...
  kex_cap.bit.gtpu_teid =
      npc_is_kex_enabled(npc, NPC_LID_LE, NPC_LT_LE_GTPU, 4 * 8, 4 * 8);
  return kex_cap.all_bits;
}

#define BYTESM1_SHIFT 16
#define HDR_OFF_SHIFT 8
static void npc_update_kex_info(struct npc_xtract_info *xtract_info,
                                uint64_t val) {
...
static void npc_mcam_process_mkex_cfg(struct npc *npc,
...
int npc_mcam_alloc_and_write(struct npc *npc, struct roc_npc_flow *flow,
  ...
  if (flow->nix_intf == NIX_INTF_RX) {
    if (inl_dev && inl_dev->is_multi_channel &&
      ...
      req->entry_data.kw_mask[0] |= (uint64_t)inl_dev->chan_mask;
      ...
      req->entry_data.kw_mask[0] |= (uint64_t)npc->sdp_channel_mask;
...
static void npc_set_vlan_ltype(struct npc_parse_state *pst) {
  uint64_t val, mask;
  uint8_t lb_offset;
...
static void npc_set_ipv6ext_ltype_mask(struct npc_parse_state *pst) {
