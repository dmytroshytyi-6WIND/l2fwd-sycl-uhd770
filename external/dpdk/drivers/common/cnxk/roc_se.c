...
static inline void cpt_snow3g_key_gen(const uint8_t *ck, uint32_t *keyx) {
  int i, base;
...
static inline int cpt_ciph_aes_key_validate(uint16_t key_len) {
  switch (key_len) {
  case 16:
  case 24:
  case 32:
...
static inline int cpt_ciph_type_set(roc_se_cipher_type type,
                                    struct roc_se_ctx *ctx, uint16_t key_len) {
  int fc_type = 0;
...
static inline void cpt_ciph_aes_key_type_set(struct roc_se_context *fctx,
                                             uint16_t key_len) {
...
static int cpt_pdcp_key_type_set(struct roc_se_zuc_snow3g_ctx *zs_ctx,
                                 uint16_t key_len) {
...
static int cpt_pdcp_mac_len_set(struct roc_se_zuc_snow3g_ctx *zs_ctx,
                                uint16_t mac_len) {
...
static void cpt_pdcp_update_zuc_const(uint8_t *zuc_const, int key_len,
                                      int mac_len) {
  if (key_len == 16) {
...
int roc_se_auth_key_set(struct roc_se_ctx *se_ctx, roc_se_auth_type type,
  ...
  if ((type >= ROC_SE_ZUC_EIA3) && (type <= ROC_SE_KASUMI_F9_ECB)) {
    ...
    case ROC_SE_SNOW3G_UIA2:
      ...
      zs_ctx->zuc.otk_ctx.w0.s.mac_len = ROC_SE_PDCP_MAC_LEN_32_BIT;
    ...
    case ROC_SE_AES_CMAC_EIA2:
      zs_ctx->zuc.otk_ctx.w0.s.alg_type = ROC_SE_PDCP_ALG_TYPE_AES_CTR;
      zs_ctx->zuc.otk_ctx.w0.s.mac_len = ROC_SE_PDCP_MAC_LEN_32_BIT;
...
success:
  ...
  if (se_ctx->fc_type == ROC_SE_PDCP) {
    if (roc_model_is_cn9k())
      se_ctx->template_w4.s.opcode_minor =
          ((1 << 7) | (se_ctx->pdcp_alg_type << 5) | (se_ctx->zsk_flags & 0x7));
    else
...
void roc_se_ctx_swap(struct roc_se_ctx *se_ctx) {
  struct roc_se_zuc_snow3g_ctx *zs_ctx = &se_ctx->se_ctx.zs_ctx;
