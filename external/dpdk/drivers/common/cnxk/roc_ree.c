...
static int roc_ree_available_queues_get(struct roc_ree_vf *vf,
                                        uint16_t *nb_queues) {
...
static int roc_ree_max_matches_get(struct roc_ree_vf *vf,
                                   uint8_t *max_matches) {
  uint64_t val;
  int ret;
...
int roc_ree_queues_attach(struct roc_ree_vf *vf, uint8_t nb_queues) {
...
int roc_ree_queues_detach(struct roc_ree_vf *vf) {
...
int roc_ree_msix_offsets_get(struct roc_ree_vf *vf) {
...
static int ree_send_mbox_msg(struct roc_ree_vf *vf) {
...
int roc_ree_config_lf(struct roc_ree_vf *vf, uint8_t lf, uint8_t pri,
                      uint32_t size) {
...
int roc_ree_af_reg_read(struct roc_ree_vf *vf, uint64_t reg, uint64_t *val) {
  ...
  mbox = vf->dev->mbox;
  mdev = &mbox->dev[0];
  msg = (struct ree_rd_wr_reg_msg *)mbox_alloc_msg_rsp(mbox, 0, sizeof(*msg),
                                                       sizeof(*msg));
  if (msg == NULL) {
    plt_err("Could not allocate mailbox message");
    return -EFAULT;
  ...
  off = mbox->rx_start + RTE_ALIGN(sizeof(struct mbox_hdr), MBOX_MSG_ALIGN);
...
int roc_ree_af_reg_write(struct roc_ree_vf *vf, uint64_t reg, uint64_t val) {
  struct ree_rd_wr_reg_msg *msg;
  struct mbox *mbox;
...
int roc_ree_rule_db_get(struct roc_ree_vf *vf, char *rule_db,
...
int roc_ree_rule_db_len_get(struct roc_ree_vf *vf, uint32_t *rule_db_len,
                            uint32_t *rule_dbi_len) {
...
static int ree_db_msg(struct roc_ree_vf *vf, const char *db, uint32_t db_len,
                      int inc, int dbi) {
...
int roc_ree_rule_db_prog(struct roc_ree_vf *vf, const char *rule_db,
...
static int ree_get_blkaddr(struct dev *dev) {
  int pf;
...
uintptr_t roc_ree_qp_get_base(struct roc_ree_vf *vf, uint16_t qp_id) {
  return REE_LF_BAR2(vf, qp_id);
}

static void roc_ree_lf_err_intr_handler(void *param) {
...
static void roc_ree_lf_err_intr_unregister(struct roc_ree_vf *vf,
...
void roc_ree_err_intr_unregister(struct roc_ree_vf *vf) {
  uintptr_t base;
  uint32_t i;
...
static int roc_ree_lf_err_intr_register(struct roc_ree_vf *vf,
                                        uint16_t msix_off, uintptr_t base) {
  struct rte_pci_device *pci_dev = vf->pci_dev;
  int ret;
  ...
  ret = dev_irq_register(pci_dev->intr_handle, roc_ree_lf_err_intr_handler,
                         (void *)base, msix_off);
  if (ret)
    return ret;
...
int roc_ree_err_intr_register(struct roc_ree_vf *vf) {
  uint32_t i, j, ret;
  uintptr_t base;

  for (i = 0; i < vf->nb_queues; i++) {
    if (vf->lf_msixoff[i] == MSIX_VECTOR_INVALID) {
      plt_err("Invalid REE LF MSI-X offset: 0x%x", vf->lf_msixoff[i]);
      return -EINVAL;
    }
  }
...
int roc_ree_iq_enable(struct roc_ree_vf *vf, const struct roc_ree_qp *qp,
...
void roc_ree_iq_disable(struct roc_ree_qp *qp) {
  uint64_t val;
...
int roc_ree_dev_init(struct roc_ree_vf *vf) {
...
int roc_ree_dev_fini(struct roc_ree_vf *vf) {
  if (vf == NULL)
    return -EINVAL;
