...
int roc_nix_ptp_rx_ena_dis(struct roc_nix *roc_nix, int enable) {
...
int roc_nix_ptp_tx_ena_dis(struct roc_nix *roc_nix, int enable) {
  struct mbox *mbox = get_mbox(roc_nix);
...
int roc_nix_ptp_clock_read(struct roc_nix *roc_nix, uint64_t *clock,
...
int roc_nix_ptp_sync_time_adjust(struct roc_nix *roc_nix, int64_t delta) {
  struct mbox *mbox = get_mbox(roc_nix);
  struct ptp_req *req;
  struct ptp_rsp *rsp;
  int rc = -ENOSPC;
...
int roc_nix_ptp_info_cb_register(struct roc_nix *roc_nix,
...
void roc_nix_ptp_info_cb_unregister(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct dev *dev = &nix->dev;
...
bool roc_nix_ptp_is_enable(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
