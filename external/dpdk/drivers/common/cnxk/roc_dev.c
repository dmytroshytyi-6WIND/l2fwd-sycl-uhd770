...
static void *mbox_mem_map(off_t off, size_t size) {
  void *va = MAP_FAILED;
  int mem_fd;
  ...
  va = plt_mmap(NULL, size, PLT_PROT_READ | PLT_PROT_WRITE, PLT_MAP_SHARED,
                mem_fd, off);
  close(mem_fd);
...
static void mbox_mem_unmap(void *va, size_t size) {
  if (va)
    munmap(va, size);
}

static int pf_af_sync_msg(struct dev *dev, struct mbox_msghdr **rsp) {
...
static int af_pf_wait_msg(struct dev *dev, uint16_t vf, int num_msg) {
  uint32_t timeout = 0, sleep = 1;
  struct mbox *mbox = dev->mbox;
  struct mbox_dev *mdev = &mbox->dev[0];
  ...
  for (i = 0; i < req_hdr->num_msgs; i++) {
    ...
    if (msg->id == MBOX_MSG_NIX_LF_START_RX) {
      ...
      sz = PLT_ALIGN(mbox_id2size(MBOX_MSG_CGX_LINK_EVENT), MBOX_MSG_ALIGN);
      /* Prepare the message to be sent */
      vf_msg = mbox_alloc_msg(&dev->mbox_vfpf_up, vf, sz);
      if (vf_msg) {
        mbox_req_init(MBOX_MSG_CGX_LINK_EVENT, vf_msg);
        memcpy((uint8_t *)vf_msg + sizeof(struct mbox_msghdr), &linfo,
...
static int vf_pf_process_msgs(struct dev *dev, uint16_t vf) {
  ...
  if (routed > 0) {
    plt_base_dbg("pf:%d routed %d messages from vf:%d to AF", dev->pf, routed,
                 vf);
  ...
  if (mdev->num_msgs) {
    plt_base_dbg("pf:%d reply %d messages to vf:%d", dev->pf, mdev->num_msgs,
                 vf);
...
static int vf_pf_process_up_msgs(struct dev *dev, uint16_t vf) {
  ...
  for (i = 0; i < req_hdr->num_msgs; i++) {
    ...
    case MBOX_MSG_CGX_LINK_EVENT:
      plt_base_dbg("PF: Msg 0x%x (%s) fn:0x%x (pf:%d,vf:%d)", msg->id,
...
static void roc_vf_pf_mbox_handle_msg(void *param) {
  ...
  for (vf = 0; vf < max_vf; vf++) {
    if (dev->intr.bits[vf / max_bits] & BIT_ULL(vf % max_bits)) {
      plt_base_dbg("Process vf:%d request (pf:%d, vf:%d)", vf, dev->pf,
                   dev->vf);
...
static void roc_vf_pf_mbox_irq(void *param) {
  struct dev *dev = param;
  ...
  for (vfpf = 0; vfpf < MAX_VFPF_DWORD_BITS; ++vfpf) {
    ...
    plt_base_dbg("vfpf: %d intr: 0x%" PRIx64 " (pf:%d, vf:%d)", vfpf, intr,
                 dev->pf, dev->vf);
  ...
  if (!dev->timer_set && alarm_set) {
    ...
    plt_alarm_set(VF_PF_MBOX_TIMER_MS, roc_vf_pf_mbox_handle_msg, dev);
  }
}

static void process_msgs(struct dev *dev, struct mbox *mbox) {
  struct mbox_dev *mdev = &mbox->dev[0];
  struct mbox_hdr *req_hdr;
  struct mbox_msghdr *msg;
  int msgs_acked = 0;
  ...
  for (i = 0; i < req_hdr->num_msgs; i++) {
    ...
    default:
      if (msg->rc)
        plt_err("Message (%s) response has err=%d", mbox_id2name(msg->id),
                msg->rc);
      break;
    }
    offset = mbox->rx_start + msg->next_msgoff;
...
static void pf_vf_mbox_send_up_msg(struct dev *dev, void *rec_msg) {
  ...
  for (vf = 0; vf < vf_mbox->ndevs; vf++) {
    ...
    plt_base_dbg("(%s) size: %zx to VF: %d", mbox_id2name(msg->hdr.id), size,
                 vf);
...
static int mbox_up_handler_cgx_link_event(struct dev *dev,
...
static int mbox_up_handler_cgx_ptp_rx_info(struct dev *dev,
...
static int mbox_process_msgs_up(struct dev *dev, struct mbox_msghdr *req) {
...
#define M(_name, _id, _fn_name, _req_type, _rsp_type)                          \
  case _id: {                                                                  \
    ...
    int err;                                                                   \
    rsp = (struct _rsp_type *)mbox_alloc_msg(&dev->mbox_up, 0,                 \
    ...
    rsp->hdr.rc = 0;                                                           \
    err = mbox_up_handler_##_fn_name(dev, (struct _req_type *)req, rsp);       \
    return err;                                                                \
  }
...
static void process_msgs_up(struct dev *dev, struct mbox *mbox) {
  struct mbox_dev *mdev = &mbox->dev[0];
  struct mbox_hdr *req_hdr;
  struct mbox_msghdr *msg;
  int i, err, offset;
...
static void roc_pf_vf_mbox_irq(void *param) {
  struct dev *dev = param;
  uint64_t intr;
...
static void roc_af_pf_mbox_irq(void *param) {
  struct dev *dev = param;
  uint64_t intr;
...
static int mbox_register_pf_irq(struct plt_pci_device *pci_dev,
                                struct dev *dev) {
  ...
  for (i = 0; i < MAX_VFPF_DWORD_BITS; ++i)
    plt_write64(~0ull, dev->bar2 + RVU_PF_VFPF_MBOX_INT_ENA_W1CX(i));
  ...
  /* HW enable intr */
  for (i = 0; i < MAX_VFPF_DWORD_BITS; ++i)
    plt_write64(~0ull, dev->bar2 + RVU_PF_VFPF_MBOX_INT_ENA_W1SX(i));
...
static int mbox_register_vf_irq(struct plt_pci_device *pci_dev,
                                struct dev *dev) {
  struct plt_intr_handle *intr_handle = pci_dev->intr_handle;
  int rc;
...
static int mbox_register_irq(struct plt_pci_device *pci_dev, struct dev *dev) {
  if (dev_is_vf(dev))
...
static void mbox_unregister_pf_irq(struct plt_pci_device *pci_dev,
                                   struct dev *dev) {
  struct plt_intr_handle *intr_handle = pci_dev->intr_handle;
  int i;
...
static void mbox_unregister_vf_irq(struct plt_pci_device *pci_dev,
                                   struct dev *dev) {
  struct plt_intr_handle *intr_handle = pci_dev->intr_handle;
...
static void mbox_unregister_irq(struct plt_pci_device *pci_dev,
                                struct dev *dev) {
  if (dev_is_vf(dev))
...
static int vf_flr_send_msg(struct dev *dev, uint16_t vf) {
  struct mbox *mbox = dev->mbox;
  struct msg_req *req;
  int rc;
...
static void roc_pf_vf_flr_irq(void *param) {
  ...
  for (i = 0; i < MAX_VFPF_DWORD_BITS; ++i) {
    ...
    for (vf = 0; vf < max_vf; vf++) {
      ...
      plt_base_dbg("FLR: i :%d intr: 0x%" PRIx64 ", vf-%d", i, intr,
                   (64 * i + vf));
      /* Clear interrupt */
      plt_write64(BIT_ULL(vf), bar2 + RVU_PF_VFFLR_INTX(i));
      /* Disable the interrupt */
      plt_write64(BIT_ULL(vf), bar2 + RVU_PF_VFFLR_INT_ENA_W1CX(i));
...
static int vf_flr_unregister_irqs(struct plt_pci_device *pci_dev,
                                  struct dev *dev) {
  struct plt_intr_handle *intr_handle = pci_dev->intr_handle;
  int i;
...
static int vf_flr_register_irqs(struct plt_pci_device *pci_dev,
                                struct dev *dev) {
  ...
  rc = dev_irq_register(handle, roc_pf_vf_flr_irq, dev, RVU_PF_INT_VEC_VFFLR0);
  if (rc)
    plt_err("Failed to init RVU_PF_INT_VEC_VFFLR0 rc=%d", rc);

  rc = dev_irq_register(handle, roc_pf_vf_flr_irq, dev, RVU_PF_INT_VEC_VFFLR1);
  if (rc)
...
static void clear_rvum_interrupts(struct dev *dev) {
  uint64_t intr;
  int i;
  ...
  if (dev_is_vf(dev)) {
    ...
    for (i = 0; i < MAX_VFPF_DWORD_BITS; ++i) {
      ...
      if (intr)
        plt_write64(intr, dev->bar2 + RVU_PF_VFPF_MBOX_INTX(i));
      /* Clear VF FLR interrupts */
      intr = plt_read64(dev->bar2 + RVU_PF_VFFLR_INTX(i));
      if (intr)
        plt_write64(intr, dev->bar2 + RVU_PF_VFFLR_INTX(i));
    }
  }
}
...
static void dev_vf_hwcap_update(struct plt_pci_device *pci_dev,
                                struct dev *dev) {
...
static uintptr_t dev_vf_mbase_get(struct plt_pci_device *pci_dev,
                                  struct dev *dev) {
  void *vf_mbase = NULL;
  uintptr_t pa;
  ...
  if (vf_mbase == MAP_FAILED) {
    plt_err("Failed to mmap vf mbase at pa 0x%lx, rc=%d", pa, errno);
    return 0;
  }
  return (uintptr_t)vf_mbase;
}

static void dev_vf_mbase_put(struct plt_pci_device *pci_dev,
                             uintptr_t vf_mbase) {
...
static int dev_setup_shared_lmt_region(struct mbox *mbox, bool valid_iova,
                                       uint64_t iova) {
...
static int dev_lmt_setup(struct dev *dev) {
