...
struct roc_bphy_irq_chip *roc_bphy_intr_init(void) {
...
void roc_bphy_intr_fini(struct roc_bphy_irq_chip *irq_chip) {
  if (irq_chip == NULL)
    return;
...
static void roc_bphy_irq_stack_remove(int cpu) {
  ...
  if (curr_stack->inuse == 0) {
    STAILQ_REMOVE(&irq_stacks, curr_stack, roc_bphy_irq_stack, entries);
...
static void *roc_bphy_irq_stack_get(int cpu) {
...
void roc_bphy_intr_handler(unsigned int irq_num) {
  struct roc_bphy_irq_chip *irq_chip;
  ...
  if (irq_chip->irq_vecs[irq_num].handler != NULL)
    irq_chip->irq_vecs[irq_num].handler((int)irq_num,
                                        irq_chip->irq_vecs[irq_num].isr_data);

  roc_atf_ret();
}

static int roc_bphy_irq_handler_set(struct roc_bphy_irq_chip *chip, int irq_num,
  ...
  if (mz == NULL) {
    ...
    mz = plt_memzone_reserve_cache_align(chip->mz_name, sizeof(chip));
    if (mz == NULL)
      return -ENOMEM;
  }
  ...
  rc =
      pthread_getaffinity_np(pthread_self(), sizeof(orig_cpuset), &orig_cpuset);
  if (rc < 0) {
  ...
  rc =
      pthread_setaffinity_np(pthread_self(), sizeof(intr_cpuset), &intr_cpuset);
  if (rc < 0) {
  ...
  *((struct roc_bphy_irq_chip **)(mz->addr)) = chip;
  ...
  rc =
      pthread_setaffinity_np(pthread_self(), sizeof(orig_cpuset), &orig_cpuset);
  if (rc < 0)
...
bool roc_bphy_intr_available(struct roc_bphy_irq_chip *irq_chip, int irq_num) {
...
uint64_t roc_bphy_intr_max_get(struct roc_bphy_irq_chip *irq_chip) {
  return irq_chip->max_irq;
}

int roc_bphy_intr_clear(struct roc_bphy_irq_chip *chip, int irq_num) {
  roc_cpuset_t orig_cpuset, intr_cpuset;
  const struct plt_memzone *mz;
  int retval;
  ...
  if (retval == 0) {
    ...
    if (chip->n_handlers == 0) {
      ...
      if (retval < 0)
        plt_err("Failed to free memzone: irq %d", irq_num);
    }
  } else {
...
int roc_bphy_intr_register(struct roc_bphy_irq_chip *irq_chip,
  ...
  ret = roc_bphy_irq_handler_set(irq_chip, intr->irq_num, intr->intr_handler,
                                 intr->isr_data);
