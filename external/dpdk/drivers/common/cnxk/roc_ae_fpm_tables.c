...
const struct ae_fpm_entry ae_fpm_tbl_scalar[ROC_AE_EC_ID_PMAX] = {
    {.data = ae_fpm_tbl_p192, .len = sizeof(ae_fpm_tbl_p192)},
    {.data = ae_fpm_tbl_p224, .len = sizeof(ae_fpm_tbl_p224)},
    {.data = ae_fpm_tbl_p256, .len = sizeof(ae_fpm_tbl_p256)},
    {.data = ae_fpm_tbl_p384, .len = sizeof(ae_fpm_tbl_p384)},
    {.data = ae_fpm_tbl_p521, .len = sizeof(ae_fpm_tbl_p521)}};

int roc_ae_fpm_get(uint64_t *tbl) {
  ...
  if (mz == NULL) {
    ...
    mz = plt_memzone_reserve_cache_align(name, sizeof(struct ae_fpm_tbl) + len);
    if (mz == NULL)
      return -ENOMEM;
  }
  ...
  for (i = ROC_AE_EC_ID_P192; i < ROC_AE_EC_ID_PMAX; i++) {
    memcpy(data, ae_fpm_tbl_scalar[i].data, ae_fpm_tbl_scalar[i].len);
