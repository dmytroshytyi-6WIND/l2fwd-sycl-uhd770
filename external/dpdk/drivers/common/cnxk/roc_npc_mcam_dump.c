...
static uint16_t npc_get_nibbles(struct roc_npc_flow *flow, uint16_t size,
                                uint32_t bit_offset) {
...
static void npc_flow_print_parse_nibbles(FILE *file, struct roc_npc_flow *flow,
...
static void npc_flow_print_xtractinfo(FILE *file,
                                      struct npc_xtract_info *lfinfo,
                                      struct roc_npc_flow *flow, int lid,
                                      int lt) {
  ...
  fprintf(file, "\t%s, hdr offset:%#X, len:%#X, key offset:%#X, ",
          ltype_str[lid][lt], lfinfo->hdr_off, lfinfo->len, lfinfo->key_off);
...
static void npc_flow_print_item(FILE *file, struct npc *npc,
                                struct npc_xtract_info *xinfo,
                                struct roc_npc_flow *flow, int intf, int lid,
                                int lt, int ld) {
...
static void npc_flow_dump_patterns(FILE *file, struct npc *npc,
...
static void npc_flow_dump_tx_action(FILE *file, uint64_t npc_action) {
  ...
  case NIX_TX_ACTIONOP_UCAST_CHAN:
    ...
    plt_strlcpy(index_name, "Transmit Channel:", NPC_MAX_FIELD_NAME_SIZE);
    break;
  case NIX_TX_ACTIONOP_MCAST:
    fprintf(file, "NIX_TX_ACTIONOP_MCAST (%" PRIu64 ")\n",
            (uint64_t)NIX_TX_ACTIONOP_MCAST);
    plt_strlcpy(index_name, "Multicast Table Index:", NPC_MAX_FIELD_NAME_SIZE);
    break;
  ...
  index = ((npc_action & NPC_TX_ACTION_INDEX_MASK) >> 12) & GENMASK(19, 0);

  fprintf(file, "\t%s:%#05X\n", index_name, index);

  match_id = ((npc_action & NPC_TX_ACTION_MATCH_MASK) >> 32) & GENMASK(15, 0);

  fprintf(file, "\tMatch Id:%#04X\n", match_id);
}

static void npc_flow_dump_rx_action(FILE *file, uint64_t npc_action) {
  ...
  case NIX_RX_ACTIONOP_RSS:
    ...
    plt_strlcpy(index_name, "RSS Group Index", NPC_MAX_FIELD_NAME_SIZE);
    break;
  ...
  pf_func = ((npc_action & NPC_RX_ACTION_PFFUNC_MASK) >> 4) & GENMASK(15, 0);

  fprintf(file, "\tPF_FUNC: %#04X\n", pf_func);

  index = ((npc_action & NPC_RX_ACTION_INDEX_MASK) >> 20) & GENMASK(19, 0);

  fprintf(file, "\t%s:%#05X\n", index_name, index);

  match_id = ((npc_action & NPC_RX_ACTION_MATCH_MASK) >> 40) & GENMASK(15, 0);

  fprintf(file, "\tMatch Id:%#04X\n", match_id);

  flowkey_alg =
      ((npc_action & NPC_RX_ACTION_FLOWKEY_MASK) >> 56) & GENMASK(4, 0);

  fprintf(file, "\tFlow Key Alg:%#X\n", flowkey_alg);
}

static void npc_flow_dump_parsed_action(FILE *file, uint64_t npc_action,
                                        bool is_rx) {
  if (is_rx) {
...
static void npc_flow_dump_rx_vtag_action(FILE *file, uint64_t vtag_action) {
  ...
  if (vtag_action & NIX_RX_VTAGACT_VTAG0_VALID_MASK) {
    ...
    lid = ((vtag_action & NIX_RX_VTAGACT_VTAG0_LID_MASK) >> 8) & GENMASK(2, 0);
    type =
        ((vtag_action & NIX_RX_VTAGACT_VTAG0_TYPE_MASK) >> 12) & GENMASK(2, 0);
  ...
  if (vtag_action & NIX_RX_VTAGACT_VTAG1_VALID_MASK) {
    relptr = ((vtag_action & NIX_RX_VTAGACT_VTAG1_RELPTR_MASK) >> 32) &
             GENMASK(7, 0);
    lid = ((vtag_action & NIX_RX_VTAGACT_VTAG1_LID_MASK) >> 40) & GENMASK(2, 0);
    type =
        ((vtag_action & NIX_RX_VTAGACT_VTAG1_TYPE_MASK) >> 44) & GENMASK(2, 0);
...
static void npc_get_vtag_opname(uint32_t op, char *opname, int len) {
  switch (op) {
  case 0x0:
...
static void npc_flow_dump_tx_vtag_action(FILE *file, uint64_t vtag_action) {
  uint32_t relptr, lid, op, vtag_def;
  char opname[10];

  relptr = vtag_action & NIX_TX_VTAGACT_VTAG0_RELPTR_MASK;
  lid = ((vtag_action & NIX_TX_VTAGACT_VTAG0_LID_MASK) >> 8) & GENMASK(2, 0);
  op = ((vtag_action & NIX_TX_VTAGACT_VTAG0_OP_MASK) >> 12) & GENMASK(1, 0);
  vtag_def =
      ((vtag_action & NIX_TX_VTAGACT_VTAG0_DEF_MASK) >> 16) & GENMASK(9, 0);
  ...
  relptr =
      ((vtag_action & NIX_TX_VTAGACT_VTAG1_RELPTR_MASK) >> 32) & GENMASK(7, 0);
  lid = ((vtag_action & NIX_TX_VTAGACT_VTAG1_LID_MASK) >> 40) & GENMASK(2, 0);
  op = ((vtag_action & NIX_TX_VTAGACT_VTAG1_OP_MASK) >> 44) & GENMASK(1, 0);
  vtag_def =
      ((vtag_action & NIX_TX_VTAGACT_VTAG1_DEF_MASK) >> 48) & GENMASK(9, 0);
...
static void npc_flow_dump_vtag_action(FILE *file, uint64_t vtag_action,
                                      bool is_rx) {
  if (is_rx) {
...
void roc_npc_flow_mcam_dump(FILE *file, struct roc_npc *roc_npc,
                            struct roc_npc_flow *flow) {
