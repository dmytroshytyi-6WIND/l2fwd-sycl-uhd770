 ...
 */
static inline uint64_t roc_npa_aura_handle_gen(uint32_t aura_id,
                                               uintptr_t addr) {
  uint64_t val;
...
static inline uint64_t roc_npa_aura_handle_to_aura(uint64_t aura_handle) {
  return aura_handle & ROC_AURA_ID_MASK;
}

static inline uintptr_t roc_npa_aura_handle_to_base(uint64_t aura_handle) {
  return (uintptr_t)(aura_handle & ~ROC_AURA_ID_MASK);
}

static inline uint64_t roc_npa_aura_op_alloc(uint64_t aura_handle,
                                             const int drop) {
...
static inline void roc_npa_aura_op_free(uint64_t aura_handle, const int fabs,
                                        uint64_t iova) {
...
static inline uint64_t roc_npa_aura_op_cnt_get(uint64_t aura_handle) {
  uint64_t wdata;
  int64_t *addr;
  uint64_t reg;
...
static inline void roc_npa_aura_op_cnt_set(uint64_t aura_handle, const int sign,
                                           uint64_t count) {
  ...
  plt_write64(reg,
              roc_npa_aura_handle_to_base(aura_handle) + NPA_LF_AURA_OP_CNT);
}

static inline uint64_t roc_npa_aura_op_limit_get(uint64_t aura_handle) {
  uint64_t wdata;
  int64_t *addr;
  uint64_t reg;
...
static inline void roc_npa_aura_op_limit_set(uint64_t aura_handle,
                                             uint64_t limit) {
  ...
  plt_write64(reg,
              roc_npa_aura_handle_to_base(aura_handle) + NPA_LF_AURA_OP_LIMIT);
}

static inline uint64_t roc_npa_aura_op_available(uint64_t aura_handle) {
  uint64_t wdata;
  uint64_t reg;
  int64_t *addr;
 ...
 */
static inline uint32_t roc_npa_aura_op_available_wait(uint64_t aura_handle,
                                                      uint32_t count,
                                                      uint32_t tmo_ms) {
...
static inline uint64_t roc_npa_pool_op_performance_counter(uint64_t aura_handle,
                                                           const int drop) {
  union {
    uint64_t u;
  ...
  addr =
      (int64_t *)(roc_npa_aura_handle_to_base(aura_handle) + NPA_LF_POOL_OP_PC);
...
static inline int roc_npa_aura_batch_alloc_issue(uint64_t aura_handle,
                                                 uint64_t *buf,
                                                 unsigned int num,
                                                 const int dis_wait,
                                                 const int drop) {
...
static inline void roc_npa_batch_alloc_wait(uint64_t *cache_line) {
...
static inline unsigned int roc_npa_aura_batch_alloc_count(uint64_t *aligned_buf,
                                                          unsigned int num) {
...
roc_npa_aura_batch_alloc_extract(uint64_t *buf, uint64_t *aligned_buf,
  ...
  for (i = 0; i < num; i += (ROC_ALIGN >> 3)) {
     ...
     * lines.
     */
    memmove(&buf[count], &aligned_buf[i], line_count * sizeof(uint64_t));
...
static inline void roc_npa_aura_op_bulk_free(uint64_t aura_handle,
                                             uint64_t const *buf,
...
static inline unsigned int roc_npa_aura_op_batch_alloc(
    uint64_t aura_handle, uint64_t *buf, uint64_t *aligned_buf,
    unsigned int num, const int dis_wait, const int drop, const int partial) {
  ...
  while (num) {
    ...
    if (roc_npa_aura_batch_alloc_issue(aura_handle, aligned_buf, chunk,
                                       dis_wait, drop))
      break;

    num_alloc = roc_npa_aura_batch_alloc_extract(buf, aligned_buf, chunk);
...
static inline void roc_npa_aura_batch_free(uint64_t aura_handle,
                                           uint64_t const *buf,
                                           unsigned int num, const int fabs,
  ...
  addr = roc_npa_aura_handle_to_base(aura_handle) + NPA_LF_AURA_BATCH_FREE0;
...
static inline void roc_npa_aura_op_batch_free(uint64_t aura_handle,
                                              uint64_t const *buf,
                                              unsigned int num, const int fabs,
                                              uint64_t lmt_addr,
                                              uint64_t lmt_id) {
  ...
  max_burst =
      ROC_CN10K_NPA_BATCH_FREE_MAX_PTRS * ROC_CN10K_NPA_BATCH_FREE_BURST_MAX;
  ...
  while (bnum) {
    ...
    roc_npa_aura_batch_free_burst(aura_handle, buf, chunk, fabs, lmt_addr,
                                  lmt_id);
    buf += chunk;
    bnum -= chunk;
  }
  ...
  if (num)
    roc_npa_aura_batch_free(aura_handle, buf, num, fabs, lmt_addr, lmt_id);
}

static inline unsigned int roc_npa_aura_bulk_alloc(uint64_t aura_handle,
                                                   uint64_t *buf,
                                                   unsigned int num,
                                                   const int drop) {
  ...
  addr = roc_npa_aura_handle_to_base(aura_handle) + NPA_LF_AURA_OP_ALLOCX(0);
  ...
  case 30:
    asm volatile(".arch_extension lse\n"
                 ...
                 : "memory", "x0", "x1", "x2", "x3", "x4", "x5", "x6", "x7",
                   ...
                   "x17", "x18", "x19", "x20", "x21", "x22", "x23", "v18",
                   "v19", "v20", "v21", "v22");
    break;
  case 16:
    asm volatile(".arch_extension lse\n"
...
static inline unsigned int roc_npa_aura_op_bulk_alloc(uint64_t aura_handle,
                                                      uint64_t *buf,
                                                      unsigned int num,
                                                      const int drop,
                                                      const int partial) {
  ...
  while (num) {
    ...
    num_alloc = roc_npa_aura_bulk_alloc(aura_handle, buf, chunk, drop);
