...
static int nix_inl_inb_sa_tbl_setup(struct roc_nix *roc_nix) {
  ...
  nix->inb_sa_base = plt_zmalloc(inb_sa_sz * max_sa, ROC_NIX_INL_SA_BASE_ALIGN);
  if (!nix->inb_sa_base) {
...
static int nix_inl_sa_tbl_release(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  int rc;
...
struct roc_cpt_lf *roc_nix_inl_outb_lf_base_get(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
uintptr_t roc_nix_inl_outb_sa_base_get(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return (uintptr_t)nix->outb_sa_base;
}

uintptr_t roc_nix_inl_inb_sa_base_get(struct roc_nix *roc_nix,
                                      bool inb_inl_dev) {
...
uint32_t roc_nix_inl_inb_spi_range(struct roc_nix *roc_nix, bool inb_inl_dev,
...
uint32_t roc_nix_inl_inb_sa_sz(struct roc_nix *roc_nix, bool inl_dev_sa) {
  struct idev_cfg *idev = idev_get_cfg();
  struct nix_inl_dev *inl_dev;
  struct nix *nix;
...
uintptr_t roc_nix_inl_inb_sa_get(struct roc_nix *roc_nix, bool inb_inl_dev,
                                 uint32_t spi) {
  ...
  mask = roc_nix_inl_inb_spi_range(roc_nix, inb_inl_dev, &min_spi, &max_spi);
  if (spi > max_spi || spi < min_spi)
    plt_warn("Inbound SA SPI %u not in range (%u..%u)", spi, min_spi, max_spi);
...
int roc_nix_reassembly_configure(uint32_t max_wait_time, uint16_t max_frags) {
  struct idev_cfg *idev = idev_get_cfg();
...
int roc_nix_inl_inb_init(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct idev_cfg *idev = idev_get_cfg();
  struct roc_cpt *roc_cpt;
  uint16_t param1;
...
int roc_nix_inl_inb_fini(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
int roc_nix_inl_outb_init(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct idev_cfg *idev = idev_get_cfg();
  ...
  /* Alloc CPT LF */
  eng_grpmask =
      (1ULL << ROC_CPT_DFLT_ENG_GRP_SE | 1ULL << ROC_CPT_DFLT_ENG_GRP_SE_IE |
       1ULL << ROC_CPT_DFLT_ENG_GRP_AE);
  rc = cpt_lfs_alloc(dev, eng_grpmask, blkaddr, !roc_nix->ipsec_out_sso_pffunc);
  if (rc) {
  ...
  for (i = 0; i < nb_lf; i++) {
    ...
    rc = cpt_lf_outb_cfg(dev, sso_pffunc, nix->dev.pf_func, i, true);
    if (rc) {
      plt_err("Failed to setup CPT LF->(NIX,SSO) link, rc=%d", rc);
      goto lf_fini;
    }
  ...
  sa_base =
      plt_zmalloc(sa_sz * roc_nix->ipsec_out_max_sa, ROC_NIX_INL_SA_BASE_ALIGN);
  if (!sa_base) {
...
skip_sa_alloc:
  ...
  for (i = 0; i < nix->outb_se_ring_cnt; i++) {
    ...
    if (!ring_base[nix->outb_se_ring_base + i]) {
      ...
      while (i--)
        plt_free(PLT_PTR_CAST(ring_base[nix->outb_se_ring_base + i]));
      rc = -ENOMEM;
      goto lf_fini;
    }
  }
...
int roc_nix_inl_outb_fini(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
bool roc_nix_inl_dev_is_probed(void) {
  struct idev_cfg *idev = idev_get_cfg();
...
bool roc_nix_inl_inb_is_enabled(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix->inl_inb_ena;
}

bool roc_nix_inl_outb_is_enabled(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix->inl_outb_ena;
}

int roc_nix_inl_dev_rq_get(struct roc_nix_rq *rq) {
  struct idev_cfg *idev = idev_get_cfg();
  struct nix_inl_dev *inl_dev;
  ...
  if (!roc_model_is_cn9k()) {
    uint64_t aura_limit = roc_npa_aura_op_limit_get(inl_rq->aura_handle);
  ...
  if (inl_rq->spb_ena) {
    uint64_t aura_limit = roc_npa_aura_op_limit_get(inl_rq->spb_aura_handle);
    uint64_t aura_shift = plt_log2_u32(aura_limit);
    uint64_t aura_drop, drop_pc;
...
int roc_nix_inl_dev_rq_put(struct roc_nix_rq *rq) {
  struct idev_cfg *idev = idev_get_cfg();
  struct nix_inl_dev *inl_dev;
  struct roc_nix_rq *inl_rq;
  struct dev *dev;
...
uint64_t roc_nix_inl_dev_rq_limit_get(void) {
  struct idev_cfg *idev = idev_get_cfg();
  struct nix_inl_dev *inl_dev;
  struct roc_nix_rq *inl_rq;
...
void roc_nix_inb_mode_set(struct roc_nix *roc_nix, bool use_inl_dev) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
int roc_nix_inl_outb_soft_exp_poll_switch(struct roc_nix *roc_nix, bool poll) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct idev_cfg *idev = idev_get_cfg();
  struct nix_inl_dev *inl_dev;
  uint16_t ring_idx, i;
...
bool roc_nix_inb_is_with_inl_dev(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix->inb_inl_dev;
}

struct roc_nix_rq *roc_nix_inl_dev_rq(void) {
  struct idev_cfg *idev = idev_get_cfg();
  struct nix_inl_dev *inl_dev;
...
int roc_nix_inl_cb_register(roc_nix_inl_sso_work_cb_t cb, void *args) {
  struct idev_cfg *idev = idev_get_cfg();
  struct nix_inl_dev *inl_dev;
...
int roc_nix_inl_cb_unregister(roc_nix_inl_sso_work_cb_t cb, void *args) {
  struct idev_cfg *idev = idev_get_cfg();
  struct nix_inl_dev *inl_dev;
...
int roc_nix_inl_inb_tag_update(struct roc_nix *roc_nix, uint32_t tag_const,
                               uint8_t tt) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct roc_nix_ipsec_cfg cfg;
...
int roc_nix_inl_sa_sync(struct roc_nix *roc_nix, void *sa, bool inb,
...
int roc_nix_inl_ctx_write(struct roc_nix *roc_nix, void *sa_dptr, void *sa_cptr,
...
void roc_nix_inl_dev_lock(void) {
  struct idev_cfg *idev = idev_get_cfg();
...
void roc_nix_inl_dev_unlock(void) {
  struct idev_cfg *idev = idev_get_cfg();
