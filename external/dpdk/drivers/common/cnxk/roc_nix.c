...
bool roc_nix_is_lbk(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix->lbk_link;
}

int roc_nix_get_base_chan(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix->rx_chan_base;
}

uint16_t roc_nix_get_vwqe_interval(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix->vwqe_interval;
}

bool roc_nix_is_sdp(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix->sdp_link;
}

bool roc_nix_is_pf(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return !dev_is_vf(&nix->dev);
}

int roc_nix_get_pf(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
int roc_nix_get_vf(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct dev *dev = &nix->dev;
...
bool roc_nix_is_vf_or_sdp(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return (dev_is_vf(&nix->dev) != 0) || roc_nix_is_sdp(roc_nix);
}

uint16_t roc_nix_get_pf_func(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct dev *dev = &nix->dev;
...
int roc_nix_lf_inl_ipsec_cfg(struct roc_nix *roc_nix,
...
int roc_nix_cpt_ctx_cache_sync(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
  struct msg_req *req;
...
int roc_nix_max_pkt_len(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
int roc_nix_lf_alloc(struct roc_nix *roc_nix, uint32_t nb_rxq, uint32_t nb_txq,
                     uint64_t rx_cfg) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
  ...
  req->rx_cfg = rx_cfg;
  if (roc_nix_is_lbk(roc_nix) && roc_nix->enable_loop && roc_model_is_cn98xx())
...
int roc_nix_lf_free(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
...
static inline int nix_lf_attach(struct dev *dev) {
...
static inline int nix_lf_get_msix_offset(struct dev *dev, struct nix *nix) {
...
static inline int nix_lf_detach(struct nix *nix) {
  struct mbox *mbox = (&nix->dev)->mbox;
...
static int roc_nix_get_hw_info(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
...
static void sdp_lbk_id_update(struct plt_pci_device *pci_dev, struct nix *nix) {
  nix->sdp_link = false;
  nix->lbk_link = false;
...
int roc_nix_dev_init(struct roc_nix *roc_nix) {
...
int roc_nix_dev_fini(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  int rc = 0;
