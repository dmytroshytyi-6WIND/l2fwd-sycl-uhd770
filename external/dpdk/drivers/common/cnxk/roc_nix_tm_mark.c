...
static uint8_t prepare_tm_shaper_red_algo(struct nix_tm_node *tm_node,
  ...
  plt_tm_dbg("Shaper read alg node %s(%u) lvl %u id %u, red_alg %x (%p)",
             nix_tm_hwlvl2str(tm_node->hw_lvl), schq, tm_node->lvl, tm_node->id,
             tm_node->red_algo, tm_node);
...
static int nix_tm_update_red_algo(struct nix *nix, bool red_send) {
...
static bool nix_tm_update_markfmt(struct nix *nix, enum roc_nix_tm_mark type,
...
int roc_nix_tm_mark_config(struct roc_nix *roc_nix, enum roc_nix_tm_mark type,
                           int mark_yellow, int mark_red) {
...
uint64_t roc_nix_tm_mark_format_get(struct roc_nix *roc_nix, uint64_t *flags) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
