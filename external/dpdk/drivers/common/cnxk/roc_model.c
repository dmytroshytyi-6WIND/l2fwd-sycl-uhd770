...
static const struct model_db {
  ...
  char name[ROC_MODEL_STR_LEN_MAX];
    ...
    {VENDOR_CAVIUM, PART_95xxMM, 0, 0, ROC_MODEL_CNF95xxMM_A0, "cnf95xxmm_a0"}};

static uint32_t cn10k_part_get(void) {
  uint32_t soc = 0x0;
  char buf[BUFSIZ];
  char *ptr;
  FILE *fd;
...
static bool populate_model(struct roc_model *model, uint32_t midr) {
  ...
  for (i = 0; i < PLT_DIM(model_db); i++)
    if (model_db[i].impl == impl && model_db[i].part == part &&
      ...
      strncpy(model->name, model_db[i].name, ROC_MODEL_STR_LEN_MAX - 1);
      found = true;
      break;
    }
...
static void detect_invalid_config(void) {
...
static uint64_t env_lookup_flag(const char *name) {
  unsigned int i;
  struct {
    const char *name;
    uint64_t flag;
...
static void of_env_get(struct roc_model *model) {
