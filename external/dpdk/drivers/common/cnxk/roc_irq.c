...
static int irq_get_info(struct plt_intr_handle *intr_handle) {
...
static int irq_config(struct plt_intr_handle *intr_handle, unsigned int vec) {
...
static int irq_init(struct plt_intr_handle *intr_handle) {
  char irq_set_buf[MSIX_IRQ_SET_BUF_LEN];
  struct vfio_irq_set *irq_set;
  int len, rc, vfio_dev_fd;
  int32_t *fd_ptr;
