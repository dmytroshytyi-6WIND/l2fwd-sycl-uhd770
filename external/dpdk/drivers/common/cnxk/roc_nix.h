...
/* NIX TM Inlines */
static inline uint64_t roc_nix_tm_max_sched_wt_get(void) {
...
static inline uint64_t roc_nix_tm_max_shaper_burst_get(void) {
  if (roc_model_is_cn9k())
...
void __roc_api roc_nix_tm_shaper_default_red_algo(
    struct roc_nix_tm_node *node, struct roc_nix_tm_shaper_profile *profile);
...
int __roc_api roc_nix_bpf_pre_color_tbl_setup(
    struct roc_nix *roc_nix, uint16_t id, enum roc_nix_bpf_level_flag lvl_flag,
