...
int npc_parse_lb(struct npc_parse_state *pst) {
  ...
  if (pst->pattern->type == ROC_NPC_ITEM_TYPE_VLAN) {
    ...
    } else if (pst->npc->switch_header_type == ROC_PRIV_FLAGS_EXDSA) {
      lt = NPC_LT_LB_EXDSA;
    } else {
      return -EINVAL;
    }
...
static int npc_parse_mpls_label_stack(struct npc_parse_state *pst, int *flag) {
...
static int npc_parse_mpls(struct npc_parse_state *pst, int lid) {
...
static inline void npc_check_lc_ip_tunnel(struct npc_parse_state *pst) {
