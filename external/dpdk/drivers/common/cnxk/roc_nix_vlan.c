...
int roc_nix_vlan_mcam_entry_read(struct roc_nix *roc_nix, uint32_t index,
...
int roc_nix_vlan_mcam_entry_write(struct roc_nix *roc_nix, uint32_t index,
...
int roc_nix_vlan_mcam_entry_alloc_and_write(struct roc_nix *roc_nix,
...
int roc_nix_vlan_mcam_entry_free(struct roc_nix *roc_nix, uint32_t index) {
  struct mbox *mbox = get_mbox(roc_nix);
...
int roc_nix_vlan_mcam_entry_ena_dis(struct roc_nix *roc_nix, uint32_t index,
                                    const int enable) {
...
int roc_nix_vlan_strip_vtag_ena_dis(struct roc_nix *roc_nix, bool enable) {
  struct mbox *mbox = get_mbox(roc_nix);
...
int roc_nix_vlan_insert_ena_dis(struct roc_nix *roc_nix,
  ...
  if (enable)
    *mcam_index = (((uint64_t)rsp->vtag1_idx << 32) | rsp->vtag0_idx);

  return 0;
}

int roc_nix_vlan_tpid_set(struct roc_nix *roc_nix, uint32_t type,
                          uint16_t tpid) {
  struct mbox *mbox = get_mbox(roc_nix);
