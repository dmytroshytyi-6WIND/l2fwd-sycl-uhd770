...
static inline uint32_t cnxk_on_anti_replay_get_seqh(uint32_t winsz,
                                                    uint32_t seql,
                                                    uint32_t esn_hi,
                                                    uint32_t esn_low) {
...
static inline int cnxk_on_anti_replay_check(uint64_t seq,
...
slow_shift:
  if (likely(seq > base)) {
    ...
    shiftwords =
        ((wint + shift - 1) >> WORD_SHIFT) - ((wint - 1) >> WORD_SHIFT);
