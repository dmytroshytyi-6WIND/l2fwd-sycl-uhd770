 ...
 */
void roc_hash_sha1_gen(uint8_t *msg, uint32_t *hash) {
 ...
 * Based on implementation from RFC 3174
 */
void roc_hash_sha256_gen(uint8_t *msg, uint32_t *hash) {
  const uint32_t _K[] = {
  ...
  for (i = 16; i < 64; i++) {
    S0 = rrot32(7, W[i - 15]) ^ rrot32(18, W[i - 15]) ^ (W[i - 15] >> 3);
    S1 = rrot32(17, W[i - 2]) ^ rrot32(19, W[i - 2]) ^ (W[i - 2] >> 10);
 ...
 * Based on implementation from RFC 3174
 */
void roc_hash_sha512_gen(uint8_t *msg, uint64_t *hash, int hash_size) {
  const uint64_t _K[] = {
  ...
  for (i = 16; i < 80; i++) {
    S0 = rrot64(1, W[i - 15]) ^ rrot64(8, W[i - 15]) ^ (W[i - 15] >> 7);
    S1 = rrot64(19, W[i - 2]) ^ rrot64(61, W[i - 2]) ^ (W[i - 2] >> 6);
    W[i] = W[i - 16] + S0 + W[i - 7] + S1;
  }
