...
static inline uint64_t nix_tm_shaper2regval(struct nix_tm_shaper_data *shaper) {
  uint64_t regval;
...
static uint8_t nix_tm_relchan_get(struct nix *nix) {
  return nix->tx_chan_base & 0xff;
}

static int nix_tm_find_prio_anchor(struct nix *nix, uint32_t node_id,
...
static uint64_t nix_tm_shaper_rate_conv_floor(uint64_t value,
                                              uint64_t *exponent_p,
                                              uint64_t *mantissa_p,
                                              uint64_t *div_exp_p) {
  ...
  if (value <= NIX_TM_SHAPER_RATE(0, 0, 0)) {
    ...
    while (value <=
           ((NIX_TM_SHAPER_RATE_CONST * ((256 + mantissa) << exponent)) / 256))
      mantissa -= 1;
  }
...
static uint64_t nix_tm_shaper_rate_conv_exact(uint64_t value,
                                              uint64_t *exponent_p,
                                              uint64_t *mantissa_p,
                                              uint64_t *div_exp_p) {
  uint64_t div_exp, exponent, mantissa;
...
uint64_t nix_tm_shaper_rate_conv(uint64_t value, uint64_t *exponent_p,
                                 ...
                                 int8_t accuracy) {
  if (!accuracy)
    return nix_tm_shaper_rate_conv_exact(value, exponent_p, mantissa_p,
                                         div_exp_p);
...
static void nix_tm_shaper_conf_get(struct nix_tm_shaper_profile *profile,
  ...
  if (profile->commit.rate)
    cir->rate = nix_tm_shaper_rate_conv(profile->commit.rate, &cir->exponent,
                                        &cir->mantissa, &cir->div_exp,
                                        profile->accuracy);

  /* Calculate PIR exponent and mantissa */
  if (profile->peak.rate)
    pir->rate = nix_tm_shaper_rate_conv(profile->peak.rate, &pir->exponent,
                                        &pir->mantissa, &pir->div_exp,
                                        profile->accuracy);

  /* Calculate CIR burst exponent and mantissa */
  if (profile->commit.size)
    cir->burst = nix_tm_shaper_burst_conv(
        profile->commit.size, &cir->burst_exponent, &cir->burst_mantissa);

  /* Calculate PIR burst exponent and mantissa */
  if (profile->peak.size)
    pir->burst = nix_tm_shaper_burst_conv(
        profile->peak.size, &pir->burst_exponent, &pir->burst_mantissa);
}
...
static uint16_t nix_tm_max_prio(struct nix *nix, uint16_t hw_lvl) {
...
uint8_t nix_tm_topology_reg_prep(struct nix *nix, struct nix_tm_node *node,
  ...
  case NIX_TXSCH_LVL_SMQ:
    ...
    regval[k] = (BIT_ULL(50) | NIX_MIN_HW_FRS | ((nix->mtu & 0xFFFF) << 8));
    /* Maximum Vtag insertion size as a multiple of four bytes */
    if (roc_nix->hw_vlan_ins)
      regval[k] |= (0x2ULL << 36);
    regval_mask[k] = ~(BIT_ULL(50) | GENMASK_ULL(6, 0) | GENMASK_ULL(23, 8) |
                       GENMASK_ULL(38, 36));
    k++;
  ...
  case NIX_TXSCH_LVL_TL3:
    ...
    /* Link configuration */
    if (!nix->sdp_link && nix->tm_link_cfg_lvl == NIX_TXSCH_LVL_TL3) {
  ...
  case NIX_TXSCH_LVL_TL2:
    ...
    /* Link configuration */
    if (!nix->sdp_link && nix->tm_link_cfg_lvl == NIX_TXSCH_LVL_TL2) {
      reg[k] = NIX_AF_TL3_TL2X_LINKX_CFG(schq, link);
      regval[k] = BIT_ULL(12) | relchan;
      /* Enable BP if node is BP capable and rx_pause is set
       */
...
uint8_t nix_tm_shaper_reg_prep(struct nix_tm_node *node,
  ...
  plt_tm_dbg("Shaper config node %s(%u) lvl %u id %u, "
             ...
             nix_tm_hwlvl2str(node->hw_lvl), schq, node->lvl, node->id,
             pir.rate, pir.burst, cir.rate, cir.burst, adjust, node->pkt_mode,
             node);
  ...
  case NIX_TXSCH_LVL_SMQ:
    ...
    regval[k] = (pir.rate && pir.burst) ? (nix_tm_shaper2regval(&pir) | 1) : 0;
    k++;

    reg[k] = NIX_AF_MDQX_CIR(schq);
    regval[k] = (cir.rate && cir.burst) ? (nix_tm_shaper2regval(&cir) | 1) : 0;
    k++;
...
uint8_t nix_tm_sw_xoff_prep(struct nix_tm_node *node, bool enable,
  ...
  plt_tm_dbg("sw xoff config node %s(%u) lvl %u id %u, enable %u (%p)",
             nix_tm_hwlvl2str(hw_lvl), schq, node->lvl, node->id, enable, node);
...
uint16_t nix_tm_resource_estimate(struct nix *nix, uint16_t *schq_contig,
  ...
  for (hw_lvl = 0; hw_lvl < NIX_TXSCH_LVL_CNT; hw_lvl++) {
    ...
    plt_tm_dbg("Estimate resources needed for %s: dis %u cont %u",
               nix_tm_hwlvl2str(hw_lvl), schq[hw_lvl], schq_contig[hw_lvl]);
  }

  return cnt;
}

uint16_t roc_nix_tm_leaf_cnt(struct roc_nix *roc_nix) {
...
int roc_nix_tm_node_lvl(struct roc_nix *roc_nix, uint32_t node_id) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct nix_tm_node *node;
...
struct roc_nix_tm_node *roc_nix_tm_node_get(struct roc_nix *roc_nix,
                                            uint32_t node_id) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct nix_tm_node *node;
...
struct roc_nix_tm_node *roc_nix_tm_node_next(struct roc_nix *roc_nix,
                                             struct roc_nix_tm_node *__prev) {
...
int roc_nix_tm_node_stats_get(struct roc_nix *roc_nix, uint32_t node_id,
                              bool clear,
...
bool roc_nix_tm_is_user_hierarchy_enabled(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
int roc_nix_tm_tree_type_get(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix->tm_tree;
}

int roc_nix_tm_max_prio(struct roc_nix *roc_nix, int lvl) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
int roc_nix_tm_lvl_is_leaf(struct roc_nix *roc_nix, int lvl) {
  return nix_tm_is_leaf(roc_nix_to_nix_priv(roc_nix), lvl);
}

void roc_nix_tm_shaper_default_red_algo(
    struct roc_nix_tm_node *node, struct roc_nix_tm_shaper_profile *roc_prof) {
...
int roc_nix_tm_lvl_cnt_get(struct roc_nix *roc_nix) {
...
int roc_nix_tm_lvl_have_link_access(struct roc_nix *roc_nix, int lvl) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
