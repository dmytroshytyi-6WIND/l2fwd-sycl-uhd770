...
static void npc_prep_mcam_ldata(uint8_t *ptr, const uint8_t *data, int len) {
  int idx;
...
static int npc_check_copysz(size_t size, size_t len) {
  if (len <= size)
    return len;
  return NPC_ERR_PARAM;
}

static inline int npc_mem_is_zero(const void *mem, int len) {
  const char *m = mem;
  int i;
...
static void npc_set_hw_mask(struct npc_parse_item_info *info,
                            struct npc_xtract_info *xinfo, char *hw_mask) {
  int max_off, offset;
  int j;
...
static inline int npc_mask_is_supported(const char *mask, const char *hw_mask,
                                        int len) {
  /*
...
static int npc_update_extraction_data(struct npc_parse_state *pst,
                                      struct npc_parse_item_info *info,
  ...
  len = npc_check_copysz((ROC_NPC_MAX_MCAM_WIDTH_DWORDS * 8) - x->key_off, len);
  if (len < 0)
    return NPC_ERR_INVALID_SIZE;
   ...
   * arrays
   */
  npc_prep_mcam_ldata(int_info, (const uint8_t *)info->spec + hdr_off, x->len);
  npc_prep_mcam_ldata(int_info_mask, (const uint8_t *)info->mask + hdr_off,
                      x->len);
...
static int npc_initialise_mcam_entry(struct npc *npc, struct roc_npc_flow *flow,
                                     int mcam_id) {
...
static int npc_shift_mcam_entry(struct mbox *mbox, uint16_t old_ent,
                                uint16_t new_ent) {
...
static int npc_slide_mcam_entries(struct mbox *mbox, struct npc *npc, int prio,
  ...
  while (curr) {
    ...
    if ((dir == SLIDE_ENTRIES_TO_HIGHER_INDEX && from_mcam_id < to_mcam_id) ||
        (dir == SLIDE_ENTRIES_TO_LOWER_INDEX && from_mcam_id > to_mcam_id)) {
       ...
       * this mcam.
       */
      rc = npc_initialise_mcam_entry(npc, curr->flow, to_mcam_id);
      if (rc)
        return rc;
      rc = npc_shift_mcam_entry(mbox, from_mcam_id, to_mcam_id);
      if (rc)
        return rc;
...
static int npc_sort_mcams_by_user_prio_level(
    struct mbox *mbox, struct npc_prio_flow_entry *flow_list_entry,
    struct npc *npc, struct npc_mcam_alloc_entry_rsp *rsp) {
  ...
  while (prio_idx <= npc->flow_max_priority - 1) {
    ...
    if ((requested_prio > prio_idx) && tail) {
      ...
      if (free_mcam < tail->flow->mcam_id) {
        rc = npc_slide_mcam_entries(mbox, npc, prio_idx, &free_mcam,
                                    SLIDE_ENTRIES_TO_LOWER_INDEX);
        if (rc)
          return rc;
...
static void npc_insert_into_flow_list(struct npc *npc,
                                      struct npc_prio_flow_entry *entry) {
  struct npc_prio_flow_list_head *list;
...
static int npc_allocate_mcam_entry(struct mbox *mbox, int prio,
...
static void npc_find_mcam_ref_entry(struct roc_npc_flow *flow, struct npc *npc,
                                    int *prio, int *ref_entry, int dir) {
  struct npc_prio_flow_entry *head, *tail;
  struct npc_prio_flow_list_head *list;
...
retry:
  ...
  if (rc && !retry_done) {
    plt_info("npc: Failed to allocate lower priority entry. Retrying for "
             "higher priority");
