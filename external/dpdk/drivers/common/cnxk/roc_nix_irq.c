...
static void nix_err_intr_enb_dis(struct nix *nix, bool enb) {
...
static void nix_ras_intr_enb_dis(struct nix *nix, bool enb) {
  if (enb)
...
void roc_nix_rx_queue_intr_enable(struct roc_nix *roc_nix,
...
void roc_nix_rx_queue_intr_disable(struct roc_nix *roc_nix,
                                   uint16_t rx_queue_id) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
void roc_nix_err_intr_ena_dis(struct roc_nix *roc_nix, bool enb) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix_err_intr_enb_dis(nix, enb);
}

void roc_nix_ras_intr_ena_dis(struct roc_nix *roc_nix, bool enb) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);

  return nix_ras_intr_enb_dis(nix, enb);
}

static void nix_lf_err_irq(void *param) {
...
static int nix_lf_register_err_irq(struct nix *nix) {
...
static void nix_lf_unregister_err_irq(struct nix *nix) {
  struct plt_intr_handle *handle = nix->pci_dev->intr_handle;
  int vec;
...
static void nix_lf_ras_irq(void *param) {
  struct nix *nix = (struct nix *)param;
  struct dev *dev = &nix->dev;
  uint64_t intr;
...
static int nix_lf_register_ras_irq(struct nix *nix) {
  struct plt_intr_handle *handle = nix->pci_dev->intr_handle;
  int rc, vec;
...
static void nix_lf_unregister_ras_irq(struct nix *nix) {
  struct plt_intr_handle *handle = nix->pci_dev->intr_handle;
  int vec;
...
static inline uint8_t nix_lf_q_irq_get_and_clear(struct nix *nix, uint16_t q,
...
static inline uint8_t nix_lf_rq_irq_get_and_clear(struct nix *nix,
                                                  uint16_t rq) {
  return nix_lf_q_irq_get_and_clear(nix, rq, NIX_LF_RQ_OP_INT, ~0xff00);
}

static inline uint8_t nix_lf_cq_irq_get_and_clear(struct nix *nix,
                                                  uint16_t cq) {
  return nix_lf_q_irq_get_and_clear(nix, cq, NIX_LF_CQ_OP_INT, ~0xff00);
}

static inline uint8_t nix_lf_sq_irq_get_and_clear(struct nix *nix,
                                                  uint16_t sq) {
  return nix_lf_q_irq_get_and_clear(nix, sq, NIX_LF_SQ_OP_INT, ~0x1ff00);
}

static inline bool nix_lf_is_sqb_null(struct dev *dev, int q) {
  ...
  if (rc) {
    ...
    is_sqb_null =
        roc_model_is_cn9k()
            ? (((__io struct nix_sq_ctx_s *)ctx)->next_sqb == 0)
            : (((__io struct nix_cn10k_sq_ctx_s *)ctx)->next_sqb == 0);
  }

  return is_sqb_null;
}

static inline uint8_t nix_lf_sq_debug_reg(struct nix *nix, uint32_t off) {
  uint8_t err = 0;
  uint64_t reg;
...
static void nix_lf_cq_irq(void *param) {
...
static void nix_lf_q_irq(void *param) {
  ...
  for (q = 0; q < nix->nb_tx_queues; q++) {
    ...
    if (irq & BIT_ULL(NIX_SQINT_SQB_ALLOC_FAIL) || nix_lf_is_sqb_null(dev, q))
...
int roc_nix_register_queue_irqs(struct roc_nix *roc_nix) {
  ...
  for (q = 0; q < qs; q++) {
    ...
    rc = dev_irq_register(handle, nix_lf_q_irq, &nix->qints_mem[q], vec);
    if (rc)
      break;
...
void roc_nix_unregister_queue_irqs(struct roc_nix *roc_nix) {
  struct plt_intr_handle *handle;
  struct nix *nix;
  int vec, q;
  ...
  for (q = 0; q < nix->configured_qints; q++) {
    ...
    dev_irq_unregister(handle, nix_lf_q_irq, &nix->qints_mem[q], vec);
  }
...
int roc_nix_register_cq_irqs(struct roc_nix *roc_nix) {
  struct plt_intr_handle *handle;
  uint8_t rc = 0, vec, q;
  struct nix *nix;
  ...
  for (q = 0; q < nix->configured_cints; q++) {
    ...
    /* Register queue irq vector */
    rc = dev_irq_register(handle, nix_lf_cq_irq, &nix->cints_mem[q], vec);
    if (rc) {
      plt_err("Fail to register CQ irq, rc=%d", rc);
      return rc;
    }

    rc = plt_intr_vec_list_alloc(handle, "cnxk", nix->configured_cints);
    if (rc) {
     ...
     */
    if (plt_intr_vec_list_index_set(handle, q, PLT_INTR_VEC_RXTX_OFFSET + vec))
      return -1;

    /* Configure CQE interrupt coalescing parameters */
    plt_write64(((CQ_CQE_THRESH_DEFAULT) | (CQ_CQE_THRESH_DEFAULT << 32) |
...
void roc_nix_unregister_cq_irqs(struct roc_nix *roc_nix) {
  struct plt_intr_handle *handle;
  struct nix *nix;
  int vec, q;
  ...
  for (q = 0; q < nix->configured_cints; q++) {
    ...
    /* Unregister queue irq vector */
    dev_irq_unregister(handle, nix_lf_cq_irq, &nix->cints_mem[q], vec);
  }
...
int nix_register_irqs(struct nix *nix) {
  ...
  if (nix->msixoff == MSIX_VECTOR_INVALID) {
    plt_err("Invalid NIXLF MSIX vector offset vector: 0x%x", nix->msixoff);
