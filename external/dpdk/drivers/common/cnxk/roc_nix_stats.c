...
int roc_nix_num_xstats_get(struct roc_nix *roc_nix) {
...
int roc_nix_stats_get(struct roc_nix *roc_nix, struct roc_nix_stats *stats) {
...
int roc_nix_stats_reset(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
static int queue_is_valid(struct nix *nix, uint16_t qid, bool is_rx) {
  uint16_t nb_queues;
...
static uint64_t qstat_read(struct nix *nix, uint16_t qid, uint32_t off) {
  uint64_t reg, val;
  int64_t *addr;
...
static void nix_stat_rx_queue_get(struct nix *nix, uint16_t qid,
...
static void nix_stat_tx_queue_get(struct nix *nix, uint16_t qid,
                                  struct roc_nix_stats_queue *qstats) {
...
static int nix_stat_rx_queue_reset(struct nix *nix, uint16_t qid) {
  struct mbox *mbox = (&nix->dev)->mbox;
  int rc;
...
static int nix_stat_tx_queue_reset(struct nix *nix, uint16_t qid) {
  struct mbox *mbox = (&nix->dev)->mbox;
  int rc;
...
int roc_nix_stats_queue_get(struct roc_nix *roc_nix, uint16_t qid, bool is_rx,
                            struct roc_nix_stats_queue *qstats) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  int rc;
...
int roc_nix_stats_queue_reset(struct roc_nix *roc_nix, uint16_t qid,
                              bool is_rx) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  int rc;
...
int roc_nix_xstats_get(struct roc_nix *roc_nix, struct roc_nix_xstat *xstats,
                       unsigned int n) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
  ...
  for (i = 0; i < nix->nb_rx_queues; i++)
    xstats[count].value += qstat_read(nix, i, nix_q_xstats[0].offset);
  ...
  if (roc_model_is_cn9k()) {
    ...
    for (i = 0; i < roc_nix_num_rx_xstats(); i++) {
      xstats[count].value = cgx_resp->rx_stats[nix_rx_xstats_cgx[i].offset];
      xstats[count].id = count;
      count++;
    }

    for (i = 0; i < roc_nix_num_tx_xstats(); i++) {
      xstats[count].value = cgx_resp->tx_stats[nix_tx_xstats_cgx[i].offset];
      xstats[count].id = count;
      count++;
    }
  } else {
    ...
    for (i = 0; i < roc_nix_num_rx_xstats(); i++) {
      xstats[count].value = rpm_resp->rx_stats[nix_rx_xstats_rpm[i].offset];
      xstats[count].id = count;
      count++;
    }

    for (i = 0; i < roc_nix_num_tx_xstats(); i++) {
      xstats[count].value = rpm_resp->tx_stats[nix_tx_xstats_rpm[i].offset];
      xstats[count].id = count;
      count++;
    }

    for (i = 0; i < CNXK_NIX_NUM_CN10K_RX_XSTATS; i++) {
      xstats[count].value = NIX_RX_STATS(nix_cn10k_rx_xstats[i].offset);
      xstats[count].id = count;
      count++;
    }
  }
...
int roc_nix_xstats_names_get(struct roc_nix *roc_nix,
  ...
  if (xstats_names) {
    ...
    if (roc_model_is_cn9k()) {
      ...
      for (i = 0; i < roc_nix_num_rx_xstats(); i++) {
        snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
                 "%s", nix_rx_xstats_rpm[i].name);
        count++;
      }

      for (i = 0; i < roc_nix_num_tx_xstats(); i++) {
        snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
                 "%s", nix_tx_xstats_rpm[i].name);
        count++;
      }

      for (i = 0; i < CNXK_NIX_NUM_CN10K_RX_XSTATS; i++) {
        snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
                 "%s", nix_cn10k_rx_xstats[i].name);
        count++;
      }
    }
  }
