...
int roc_nix_tm_sq_aura_fc(struct roc_nix_sq *sq, bool enable) {
...
int roc_nix_tm_free_resources(struct roc_nix *roc_nix, bool hw_only) {
...
nix_tm_adjust_shaper_pps_rate(struct nix_tm_shaper_profile *profile) {
  ...
  if (profile->peak.rate && (profile->peak.rate < NIX_TM_MIN_SHAPER_PPS_RATE ||
...
static int nix_tm_shaper_profile_add(struct roc_nix *roc_nix,
  ...
  plt_tm_dbg("Added TM shaper profile %u, "
             " pir %" PRIu64 " , pbs %" PRIu64 ", cir %" PRIu64 ", cbs %" PRIu64
             " , adj %u, pkt_mode %u",
             id, profile->peak.rate, profile->peak.size, profile->commit.rate,
             profile->commit.size, profile->pkt_len_adj, profile->pkt_mode);
...
int roc_nix_tm_shaper_profile_add(
    struct roc_nix *roc_nix, struct roc_nix_tm_shaper_profile *roc_profile) {
  struct nix_tm_shaper_profile *profile;
...
int roc_nix_tm_shaper_profile_update(
    struct roc_nix *roc_nix, struct roc_nix_tm_shaper_profile *roc_profile) {
  struct nix_tm_shaper_profile *profile;
...
int roc_nix_tm_shaper_profile_delete(struct roc_nix *roc_nix, uint32_t id) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct nix_tm_shaper_profile *profile;
...
int roc_nix_tm_node_add(struct roc_nix *roc_nix,
...
int roc_nix_tm_node_pkt_mode_update(struct roc_nix *roc_nix, uint32_t node_id,
                                    bool pkt_mode) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
int roc_nix_tm_node_name_get(struct roc_nix *roc_nix, uint32_t node_id,
  ...
  if (node->hw_lvl == NIX_TXSCH_LVL_CNT)
    ...
    snprintf(buf, buflen, "%s_%d", nix_tm_hwlvl2str(node->hw_lvl), node->hw_id);
  return 0;
}

int roc_nix_tm_node_delete(struct roc_nix *roc_nix, uint32_t node_id,
                           bool free) {
  return nix_tm_node_delete(roc_nix, node_id, ROC_NIX_TM_USER, free);
}

int roc_nix_smq_flush(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct nix_tm_node_list *list;
  ...
  TAILQ_FOREACH(node, list, node) {
    ...
    if (rc) {
      plt_err("Failed to enable smq %u, rc=%d", node->hw_id, rc);
      goto exit;
    }
  }
...
int roc_nix_tm_hierarchy_disable(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
int roc_nix_tm_hierarchy_enable(struct roc_nix *roc_nix,
                                enum roc_nix_tm_tree tree, bool xmit_enable) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct nix_tm_node_list *list;
  struct nix_tm_node *node;
  struct roc_nix_sq *sq;
  ...
  TAILQ_FOREACH(node, list, node) {
    ...
    if (rc) {
      plt_err("SQ %u sched update failed, rc=%d", node->id, rc);
      return rc;
    }
  }
  ...
  TAILQ_FOREACH(node, list, node) {
    ...
    if (rc) {
      plt_err("TM sw xon failed on SQ %u, rc=%d", node->id, rc);
      return rc;
    }
    node->flags |= NIX_TM_NODE_ENABLED;
  }
...
int roc_nix_tm_node_suspend_resume(struct roc_nix *roc_nix, uint32_t node_id,
                                   bool suspend) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
  ...
  flags = node->flags;
  flags =
      suspend ? (flags & ~NIX_TM_NODE_ENABLED) : (flags | NIX_TM_NODE_ENABLED);
  ...
  req->lvl = node->hw_lvl;
  req->num_regs = nix_tm_sw_xoff_prep(node, suspend, req->reg, req->regval);
  rc = mbox_process(mbox);
  if (!rc)
    node->flags = flags;
  return rc;
...
int roc_nix_tm_prealloc_res(struct roc_nix *roc_nix, uint8_t lvl,
...
int roc_nix_tm_node_shaper_update(struct roc_nix *roc_nix, uint32_t node_id,
  ...
  if ((profile_id == node->shaper_profile_id) && !force_update) {
    ...
    old = nix_tm_shaper_profile_search(nix, node->shaper_profile_id);
    if (old)
      old->ref_cnt--;
...
int roc_nix_tm_node_parent_update(struct roc_nix *roc_nix, uint32_t node_id,
  ...
  if (node->hw_lvl != nix->tm_root_lvl) {
    new_parent = nix_tm_node_search(nix, new_parent_id, ROC_NIX_TM_USER);
    if (!new_parent)
  ...
  if (nix_tm_is_leaf(nix, node->lvl)) {
    ...
    req->num_regs =
        nix_tm_sw_xoff_prep(node->parent, true, req->reg, req->regval);
    rc = mbox_process(mbox);
    if (rc)
      return rc;
    ...
    req->lvl = node->parent->hw_lvl;
    req->num_regs =
        nix_tm_sw_xoff_prep(node->parent, false, req->reg, req->regval);
    rc = mbox_process(mbox);
    if (rc)
      return rc;
  }
...
int roc_nix_tm_init(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  uint32_t tree_mask;
  int rc;
...
int roc_nix_tm_rlimit_sq(struct roc_nix *roc_nix, uint16_t qid, uint64_t rate) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct nix_tm_shaper_profile profile;
  struct mbox *mbox = (&nix->dev)->mbox;
...
void roc_nix_tm_fini(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
...
int roc_nix_tm_rsrc_count(struct roc_nix *roc_nix,
  ...
  for (i = 0; i < ROC_TM_LVL_MAX; i++) {
    ...
    schq[i] = (nix->is_nix1 ? rsp->schq_nix1[hw_lvl] : rsp->schq[hw_lvl]);
  }

  return 0;
}

void roc_nix_tm_rsrc_max(bool pf, uint16_t schq[ROC_TM_LVL_MAX]) {
  uint8_t hw_lvl, i;
  uint16_t max;

  for (i = 0; i < ROC_TM_LVL_MAX; i++) {
    hw_lvl = pf ? nix_tm_lvl2nix_tl1_root(i) : nix_tm_lvl2nix_tl2_root(i);
...
bool roc_nix_tm_root_has_sp(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
