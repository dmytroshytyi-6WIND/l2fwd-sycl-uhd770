...
static void nix_lso_tcp(struct nix_lso_format_cfg *req, bool v4) {
...
static void nix_lso_udp_tun_tcp(struct nix_lso_format_cfg *req, bool outer_v4,
                                bool inner_v4) {
  __io struct nix_lso_format *field;
...
static void nix_lso_tun_tcp(struct nix_lso_format_cfg *req, bool outer_v4,
                            bool inner_v4) {
  __io struct nix_lso_format *field;
...
int roc_nix_lso_custom_fmt_setup(struct roc_nix *roc_nix,
  ...
  req->field_mask = NIX_LSO_FIELD_MASK;
  mbox_memcpy(req->fields, fields, sizeof(struct nix_lso_format) * nb_fields);
...
int roc_nix_lso_fmt_setup(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = get_mbox(roc_nix);
  struct nix_lso_format_cfg_rsp *rsp;
  struct nix_lso_format_cfg *req;
...
int roc_nix_lso_fmt_get(struct roc_nix *roc_nix,
...
int roc_nix_switch_hdr_set(struct roc_nix *roc_nix, uint64_t switch_header_type,
...
int roc_nix_eeprom_info_get(struct roc_nix *roc_nix,
...
int roc_nix_rx_drop_re_set(struct roc_nix *roc_nix, bool ena) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = get_mbox(roc_nix);
