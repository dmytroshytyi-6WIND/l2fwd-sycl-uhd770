...
static struct nix_tel_node *nix_tel_node_get(struct roc_nix *roc_nix) {
...
int nix_tel_node_add(struct roc_nix *roc_nix) {
  ...
  if (node) {
    if (nix->nb_rx_queues == node->n_rq && nix->nb_tx_queues == node->n_sq)
      return 0;
  ...
  node->nix = roc_nix;
  node->rqs = plt_zmalloc(nix->nb_rx_queues * sizeof(struct roc_nix_rq *), 0);
  node->cqs = plt_zmalloc(nix->nb_rx_queues * sizeof(struct roc_nix_cq *), 0);
  node->sqs = plt_zmalloc(nix->nb_tx_queues * sizeof(struct roc_nix_sq *), 0);
...
static struct nix_tel_node *nix_tel_node_get_by_pcidev_name(const char *name) {
  struct nix_tel_node *node, *roc_node = NULL;

  TAILQ_FOREACH(node, &nix_list, node) {
    if (!strncmp(node->nix->pci_dev->name, name, PCI_PRI_STR_SIZE)) {
      roc_node = node;
      break;
    }
  }
...
static int cnxk_tel_nix(struct roc_nix *roc_nix, struct plt_tel_data *d) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
static int cnxk_tel_nix_rq(struct roc_nix_rq *rq, struct plt_tel_data *d) {
...
static int cnxk_tel_nix_cq(struct roc_nix_cq *cq, struct plt_tel_data *d) {
...
static int cnxk_tel_nix_sq(struct roc_nix_sq *sq, struct plt_tel_data *d) {
...
static void nix_rq_ctx_cn9k(volatile void *qctx, struct plt_tel_data *d) {
...
static void nix_rq_ctx(volatile void *qctx, struct plt_tel_data *d) {
...
static int cnxk_tel_nix_rq_ctx(struct roc_nix *roc_nix, uint8_t n,
...
static int cnxk_tel_nix_cq_ctx(struct roc_nix *roc_nix, uint8_t n,
                               struct plt_tel_data *d) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct dev *dev = &nix->dev;
  struct npa_lf *npa_lf;
...
static void nix_sq_ctx_cn9k(volatile void *qctx, struct plt_tel_data *d) {
...
static void nix_sq_ctx(volatile void *qctx, struct plt_tel_data *d) {
...
static int cnxk_tel_nix_sq_ctx(struct roc_nix *roc_nix, uint8_t n,
                               struct plt_tel_data *d) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct dev *dev = &nix->dev;
  struct npa_lf *npa_lf;
...
static int cnxk_nix_tel_handle_list(const char *cmd __plt_unused,
...
static int cnxk_nix_tel_handle_info(const char *cmd __plt_unused,
                                    const char *params,
                                    struct plt_tel_data *d) {
...
static int cnxk_nix_tel_handle_info_x(const char *cmd, const char *params,
                                      struct plt_tel_data *d) {
  struct nix_tel_node *node;
  char *name, *param;
  char buf[1024];
...
PLT_INIT(cnxk_telemetry_nix_init) {
  ...
  plt_telemetry_register_cmd("/cnxk/nix/info", cnxk_nix_tel_handle_info,
