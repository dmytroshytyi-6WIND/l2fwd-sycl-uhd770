...
static int tim_fill_msix(struct roc_tim *roc_tim, uint16_t nb_ring) {
...
static void tim_err_desc(int rc) {
  switch (rc) {
...
int roc_tim_lf_enable(struct roc_tim *roc_tim, uint8_t ring_id,
...
int roc_tim_lf_disable(struct roc_tim *roc_tim, uint8_t ring_id) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_tim->roc_sso)->dev;
  struct tim_ring_req *req;
  int rc = -ENOSPC;
...
uintptr_t roc_tim_lf_base_get(struct roc_tim *roc_tim, uint8_t ring_id) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_tim->roc_sso)->dev;

  return dev->bar2 + (RVU_BLOCK_ADDR_TIM << 20 | ring_id << 12);
}

int roc_tim_lf_config(struct roc_tim *roc_tim, uint8_t ring_id,
...
int roc_tim_lf_interval(struct roc_tim *roc_tim, enum roc_tim_clk_src clk_src,
...
int roc_tim_lf_alloc(struct roc_tim *roc_tim, uint8_t ring_id, uint64_t *clk) {
...
int roc_tim_lf_free(struct roc_tim *roc_tim, uint8_t ring_id) {
  struct sso *sso = roc_sso_to_sso_priv(roc_tim->roc_sso);
  struct tim *tim = roc_tim_to_tim_priv(roc_tim);
  struct dev *dev = &sso->dev;
  struct tim_ring_req *req;
...
void roc_tim_fini(struct roc_tim *roc_tim) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_tim->roc_sso)->dev;
  struct rsrc_detach_req *detach_req;
