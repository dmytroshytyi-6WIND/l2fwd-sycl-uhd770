...
#define plt_dbg(subsystem, fmt, args...)                                       \
  rte_log(RTE_LOG_DEBUG, cnxk_logtype_##subsystem, "[%s] %s():%u " fmt "\n",   \
...
#define CNXK_PCI_ID(subsystem_dev, dev)                                        \
  {                                                                            \
    .class_id = RTE_CLASS_ANY_ID, .vendor_id = PCI_VENDOR_ID_CAVIUM,           \
    .device_id = (dev), .subsystem_vendor_id = RTE_PCI_ANY_ID,                 \
    .subsystem_device_id = (subsystem_dev),                                    \
  }
#endif

__rte_internal int roc_plt_init(void);
...
plt_lmt_region_reserve_aligned(const char *name, size_t len, uint32_t align) {
   ...
   */
  return rte_memzone_reserve_bounded(
      name, len, SOCKET_ID_ANY, RTE_MEMZONE_IOVA_CONTIG, align, RTE_PGSIZE_2M);
}
