...
static inline int bitmap_ctzll(uint64_t slab) {
  if (slab == 0)
    return 0;
...
void nix_tm_clear_shaper_profiles(struct nix *nix) {
  ...
  PLT_TAILQ_FOREACH_SAFE(shaper_profile, list, shaper, tmp) {
    if (shaper_profile->ref_cnt)
      plt_warn("Shaper profile %u has non zero references", shaper_profile->id);
    TAILQ_REMOVE(&nix->shaper_profile_list, shaper_profile, shaper);
    nix_tm_shaper_profile_free(shaper_profile);
  }
}

static int nix_tm_node_reg_conf(struct nix *nix, struct nix_tm_node *node) {
  ...
  if (!nix_tm_have_tl1_access(nix) && hw_lvl == NIX_TXSCH_LVL_TL2) {
    ...
    k = nix_tm_tl1_default_prep(node->parent_hw_id, req->reg, req->regval);
    req->num_regs = k;
...
int nix_tm_txsch_reg_config(struct nix *nix, enum roc_nix_tm_tree tree) {
  ...
  for (hw_lvl = 0; hw_lvl <= nix->tm_root_lvl; hw_lvl++) {
    TAILQ_FOREACH(node, list, node) {
       ...
       */
      node->bp_capa = 0;
      if (is_pf_or_lbk && !skip_bp && node->hw_lvl == nix->tm_link_cfg_lvl) {
        node->bp_capa = 1;
        skip_bp = false;
      }
...
int nix_tm_update_parent_info(struct nix *nix, enum roc_nix_tm_tree tree) {
  ...
  TAILQ_FOREACH(parent, list, node) {
    ...
    rr_num = nix_tm_check_rr(nix, parent->id, tree, &rr_prio, &max_prio);
...
static int nix_tm_root_node_get(struct nix *nix, int tree) {
...
int nix_tm_node_add(struct roc_nix *roc_nix, struct nix_tm_node *node) {
  ...
  plt_tm_dbg("Add node %s lvl %u id %u, prio 0x%x weight 0x%x "
             ...
             nix_tm_hwlvl2str(nix_tm_lvl2nix(nix, lvl)), lvl, node_id, priority,
             node->weight, parent_id, profile_id, tree);
  ...
  plt_tm_dbg("Added node %s lvl %u id %u (%p)", nix_tm_hwlvl2str(node->hw_lvl),
             lvl, node_id, node);
  return 0;
}
...
int nix_tm_clear_path_xoff(struct nix *nix, struct nix_tm_node *node) {
  ...
  if (!nix_tm_is_leaf(nix, node->lvl))
    ...
    p = node->parent;
  while (p) {
    if (!(p->flags & NIX_TM_NODE_ENABLED) && (p->flags & NIX_TM_NODE_HWRES)) {
      req = mbox_alloc_msg_nix_txschq_cfg(mbox);
      req->lvl = p->hw_lvl;
      req->num_regs = nix_tm_sw_xoff_prep(p, false, req->reg, req->regval);
      rc = mbox_process(mbox);
      if (rc)
        return rc;
...
int nix_tm_smq_xoff(struct nix *nix, struct nix_tm_node *node, bool enable) {
  ...
  req->regval_mask[0] = enable ? ~(BIT_ULL(50) | BIT_ULL(49)) : ~BIT_ULL(50);
...
int roc_nix_tm_sq_flush_spin(struct roc_nix_sq *sq) {
...
int nix_tm_sq_flush_pre(struct roc_nix_sq *sq) {
  ...
  rc = nix_tm_smq_xoff(nix, node->parent, false);
  if (rc) {
    plt_err("Failed to enable smq %u, rc=%d", node->parent->hw_id, rc);
    return rc;
  }
  ...
  if (rc) {
    plt_err("Failed to disable smq %u, rc=%d", node->parent->hw_id, rc);
    goto cleanup;
  }
cleanup:
  /* Restore cgx state */
...
int nix_tm_sq_sched_conf(struct nix *nix, struct nix_tm_node *node,
  ...
  if (rr_quantum_only)
    plt_tm_dbg("Update sq(%u) rr_quantum 0x%" PRIx64, qid, rr_quantum);
  else
    plt_tm_dbg("Enabling sq(%u)->smq(%u), rr_quantum 0x%" PRIx64, qid, smq,
               rr_quantum);
...
int nix_tm_release_resources(struct nix *nix, uint8_t hw_lvl, bool contig,
  ...
  thresh = contig ? nix->contig_rsvd[hw_lvl] : nix->discontig_rsvd[hw_lvl];
  ...
  while (to_free) {
    ...
    rc = mbox_process(mbox);
    if (rc) {
      plt_err("failed to release hwres %s(%u) rc %d", nix_tm_hwlvl2str(hw_lvl),
              schq, rc);
      return rc;
    }

    plt_tm_dbg("Released hwres %s(%u)", nix_tm_hwlvl2str(hw_lvl), schq);
    plt_bitmap_clear(bmp, schq);
    to_free--;
  }

  if (to_free) {
    plt_err("resource inconsistency for %s(%u)", nix_tm_hwlvl2str(hw_lvl),
            contig);
    return -EFAULT;
  }
  return 0;
...
int nix_tm_free_node_resource(struct nix *nix, struct nix_tm_node *node) {
  ...
  plt_tm_dbg("Free hwres %s(%u) lvl %u id %u (%p)",
             nix_tm_hwlvl2str(node->hw_lvl), hw_id, node->lvl, node->id, node);

  avail = nix_tm_resource_avail(nix, hw_lvl, false);
  /* Always for now free to discontiguous queue when avail
   * is not sufficient.
   */
  if (nix->discontig_rsvd[hw_lvl] && avail < nix->discontig_rsvd[hw_lvl]) {
  ...
  node->hw_id = NIX_TM_HW_ID_INVALID;
  node->flags &= ~NIX_TM_NODE_HWRES;
  plt_tm_dbg("Released hwres %s(%u) to af", nix_tm_hwlvl2str(node->hw_lvl),
             hw_id);
  return 0;
}
...
static int nix_tm_assign_hw_id(struct nix *nix, struct nix_tm_node *parent,
  ...
  TAILQ_FOREACH(child, list, node) {
    ...
    if (child->flags & NIX_TM_NODE_HWRES) {
      plt_err("Resource exists for child (%s)%u, id %u (%p)",
              nix_tm_hwlvl2str(child->hw_lvl), child->hw_id, child->id, child);
      return -EFAULT;
    }
    ...
    plt_tm_dbg("Resource %s(%u), for lvl %u id %u(%p)",
               nix_tm_hwlvl2str(child->hw_lvl), schq, child->lvl, child->id,
               child);
...
int nix_tm_assign_resources(struct nix *nix, enum roc_nix_tm_tree tree) {
  ...
  for (hw_lvl = NIX_TXSCH_LVL_TL1; hw_lvl > 0; hw_lvl--) {
    TAILQ_FOREACH(parent, list, node) {
      ...
      rc = nix_tm_assign_hw_id(nix, parent, &contig_id, &cnt, list);
      if (cnt || rc) {
        plt_err("Unexpected err, contig res alloc, "
                "parent %u, of %s, rc=%d, cnt=%d",
                parent->id, nix_tm_hwlvl2str(hw_lvl), rc, cnt);
        return -EFAULT;
      }
...
void nix_tm_copy_rsp_to_nix(struct nix *nix, struct nix_txsch_alloc_rsp *rsp) {
  ...
  for (lvl = 0; lvl < NIX_TXSCH_LVL_CNT; lvl++) {
    for (i = 0; i < rsp->schq[lvl]; i++)
      plt_bitmap_set(nix->schq_bmp[lvl], rsp->schq_list[lvl][i]);

    for (i = 0; i < rsp->schq_contig[lvl]; i++)
      plt_bitmap_set(nix->schq_contig_bmp[lvl], rsp->schq_contig_list[lvl][i]);
  }
}
...
int nix_tm_alloc_txschq(struct nix *nix, enum roc_nix_tm_tree tree) {
  ...
  do {
    ...
    mbox_memcpy(req->schq_contig, schq_contig, sizeof(req->schq_contig));
...
int nix_tm_prepare_default_tree(struct roc_nix *roc_nix) {
  ...
  lvl_end = (nix_tm_have_tl1_access(nix) ? ROC_TM_LVL_SCH4 : ROC_TM_LVL_SCH3);
...
int roc_nix_tm_prepare_rate_limited_tree(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  uint32_t nonleaf_id = nix->nb_tx_queues;
  struct nix_tm_node *node = NULL;
  uint8_t leaf_lvl, lvl, lvl_end;
  ...
  parent = ROC_NIX_TM_NODE_ID_INVALID;
  lvl_end = (nix_tm_have_tl1_access(nix) ? ROC_TM_LVL_SCH3 : ROC_TM_LVL_SCH2);
...
int roc_nix_tm_pfc_prepare_tree(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  uint32_t nonleaf_id = nix->nb_tx_queues;
  struct nix_tm_node *node = NULL;
  uint8_t leaf_lvl, lvl, lvl_end;
...
int nix_tm_free_resources(struct roc_nix *roc_nix, uint32_t tree_mask,
  ...
  for (tree = 0; tree < ROC_NIX_TM_TREE_MAX; tree++) {
    ...
    while (next_node) {
      ...
      if (!nix_tm_is_leaf(nix, node->lvl) && node->flags & NIX_TM_NODE_HWRES) {
    ...
    while (next_node) {
      ...
      plt_tm_dbg("Free node lvl %u id %u (%p)", node->lvl, node->id, node);
