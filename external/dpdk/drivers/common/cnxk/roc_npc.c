...
int roc_npc_vtag_actions_get(struct roc_npc *roc_npc) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);

  return npc->vtag_strip_actions;
}

int roc_npc_vtag_actions_sub_return(struct roc_npc *roc_npc, uint32_t count) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);
...
int roc_npc_mcam_free_counter(struct roc_npc *roc_npc, uint16_t ctr_id) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);

  return npc_mcam_free_counter(npc, ctr_id);
}

int roc_npc_mcam_read_counter(struct roc_npc *roc_npc, uint32_t ctr_id,
                              uint64_t *count) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);
...
int roc_npc_mcam_clear_counter(struct roc_npc *roc_npc, uint32_t ctr_id) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);

  return npc_mcam_clear_counter(npc, ctr_id);
}

int roc_npc_mcam_free_entry(struct roc_npc *roc_npc, uint32_t entry) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);

  return npc_mcam_free_entry(npc, entry);
}

int roc_npc_mcam_free_all_resources(struct roc_npc *roc_npc) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);

  return npc_flow_free_all_resources(npc);
}

int roc_npc_mcam_alloc_entries(struct roc_npc *roc_npc, int ref_entry,
...
int roc_npc_mcam_enable_all_entries(struct roc_npc *roc_npc, bool enable) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);

  return npc_flow_enable_all_entries(npc, enable);
}

int roc_npc_mcam_alloc_entry(struct roc_npc *roc_npc, struct roc_npc_flow *mcam,
...
int roc_npc_mcam_ena_dis_entry(struct roc_npc *roc_npc,
                               struct roc_npc_flow *mcam, bool enable) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);

  return npc_mcam_ena_dis_entry(npc, mcam, enable);
}

int roc_npc_mcam_write_entry(struct roc_npc *roc_npc,
...
int roc_npc_get_low_priority_mcam(struct roc_npc *roc_npc) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);
...
static int npc_mcam_tot_entries(void) {
...
const char *roc_npc_profile_name_get(struct roc_npc *roc_npc) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);

  return (char *)npc->profile_name;
}

int roc_npc_init(struct roc_npc *roc_npc) {
...
int roc_npc_fini(struct roc_npc *roc_npc) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);
  int rc;
...
int roc_npc_validate_portid_action(struct roc_npc *roc_npc_src,
...
static int npc_parse_actions(struct roc_npc *roc_npc,
  ...
  for (; actions->type != ROC_NPC_ACTION_TYPE_END; actions++) {
    ...
    case ROC_NPC_ACTION_TYPE_MARK:
      act_mark = (const struct roc_npc_action_mark *)actions->conf;
    ...
    case ROC_NPC_ACTION_TYPE_PORT_ID:
      act_portid = (const struct roc_npc_action_port_id *)actions->conf;
      pf_func &= (0xfc00);
    ...
    case ROC_NPC_ACTION_TYPE_METER:
      act_mtr = (const struct roc_npc_action_meter *)actions->conf;
  ...
  if (attr->egress) {
    ...
    if (vlan_insert_action && (req_act & ROC_NPC_ACTION_TYPE_DROP)) {
...
static int npc_parse_pattern(struct npc *npc,
...
static int npc_parse_attr(struct npc *npc, const struct roc_npc_attr *attr,
                          struct roc_npc_flow *flow) {
  if (attr == NULL)
    return NPC_ERR_PARAM;
  else if (attr->priority >= npc->flow_max_priority)
    return NPC_ERR_PARAM;
  else if ((!attr->egress && !attr->ingress) || (attr->egress && attr->ingress))
    return NPC_ERR_PARAM;
...
static int npc_parse_rule(struct roc_npc *roc_npc,
                          const struct roc_npc_attr *attr,
                          const struct roc_npc_item_info pattern[],
                          const struct roc_npc_action actions[],
                          struct roc_npc_flow *flow,
...
int roc_npc_flow_parse(struct roc_npc *roc_npc, const struct roc_npc_attr *attr,
                       const struct roc_npc_item_info pattern[],
                       const struct roc_npc_action actions[],
                       struct roc_npc_flow *flow) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);
...
int npc_rss_action_configure(struct roc_npc *roc_npc,
  ...
  *rss_grp = rss_grp_idx;
  ...
  rc = roc_nix_rss_flowkey_set(roc_nix, &flowkey_algx, flowkey_cfg, *rss_grp,
                               mcam_id);
  if (rc) {
...
int npc_rss_action_program(struct roc_npc *roc_npc,
  ...
  for (; actions->type != ROC_NPC_ACTION_TYPE_END; actions++) {
    if (actions->type == ROC_NPC_ACTION_TYPE_RSS) {
      ...
      rc = npc_rss_action_configure(roc_npc, rss, &alg_idx, &rss_grp,
                                    flow->mcam_id);
      if (rc)
        return rc;
      ...
      flow->npc_action |= ((uint64_t)(alg_idx & NPC_RSS_ACT_ALG_MASK)
...
static int npc_vtag_cfg_delete(struct roc_npc *roc_npc,
                               struct roc_npc_flow *flow) {
  struct roc_nix *roc_nix = roc_npc->roc_nix;
...
static int npc_vtag_insert_action_parse(const struct roc_npc_action actions[],
                                        struct roc_npc_flow *flow,
  ...
  for (; count < NPC_ACTION_MAX_VLAN_PARAMS; count++, actions++) {
    if (actions->type == ROC_NPC_ACTION_TYPE_VLAN_INSERT) {
      ...
      const struct roc_npc_action_of_set_vlan_vid *vtag =
          (const struct roc_npc_action_of_set_vlan_vid *)actions->conf;
      ...
      (*parsed_cnt)++;
      vlan_id_found = true;
    } else if (actions->type == ROC_NPC_ACTION_TYPE_VLAN_ETHTYPE_INSERT) {
      if (ethtype_found)
        return 0;

      const struct roc_npc_action_of_push_vlan *ethtype =
          (const struct roc_npc_action_of_push_vlan *)actions->conf;
      vlan_info->vlan_ethtype = plt_be_to_cpu_16(ethtype->ethertype);
      ...
      (*parsed_cnt)++;
      ethtype_found = true;
    } else if (actions->type == ROC_NPC_ACTION_TYPE_VLAN_PCP_INSERT) {
      if (pcp_found)
        return 0;
      const struct roc_npc_action_of_set_vlan_pcp *pcp =
          (const struct roc_npc_action_of_set_vlan_pcp *)actions->conf;
...
npc_vtag_insert_action_configure(struct mbox *mbox, struct roc_npc_flow *flow,
  ...
  if (flow->vtag_insert_count == 2) {
     ...
     */
    tx_vtag_action.act.vtag1_relptr = NIX_TX_VTAGACTION_VTAG0_RELPTR;
  }
...
static int npc_vtag_action_program(struct roc_npc *roc_npc,
                                   const struct roc_npc_action actions[],
                                   struct roc_npc_flow *flow) {
  ...
  for (; actions->type != ROC_NPC_ACTION_TYPE_END; actions++) {
    if (actions->type == ROC_NPC_ACTION_TYPE_VLAN_STRIP) {
      ...
      rc = npc_vtag_strip_action_configure(mbox, actions, flow, &strip_cnt);
      if (rc)
        return rc;
      ...
      vlan_strip_parsed = true;
               ...
               actions->type == ROC_NPC_ACTION_TYPE_VLAN_ETHTYPE_INSERT ||
               actions->type == ROC_NPC_ACTION_TYPE_VLAN_PCP_INSERT) {
      ...
      for (i = 0; i < 2; i++) {
        rc = npc_vtag_insert_action_parse(insert_actions, flow, &vlan_info[i],
                                          &parsed_cnt);
...
int npc_rss_group_free(struct npc *npc, struct roc_npc_flow *flow) {
  ...
  if ((flow->npc_action & 0xF) == NIX_RX_ACTIONOP_RSS) {
    rss_grp =
        (flow->npc_action >> NPC_RSS_ACT_GRP_OFFSET) & NPC_RSS_ACT_GRP_MASK;
...
int roc_npc_flow_destroy(struct roc_npc *roc_npc, struct roc_npc_flow *flow) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);
  int rc;
...
void roc_npc_flow_dump(FILE *file, struct roc_npc *roc_npc) {
  struct npc *npc = roc_npc_to_npc_priv(roc_npc);
...
int roc_npc_mcam_merge_base_steering_rule(struct roc_npc *roc_npc,
                                          struct roc_npc_flow *flow) {
