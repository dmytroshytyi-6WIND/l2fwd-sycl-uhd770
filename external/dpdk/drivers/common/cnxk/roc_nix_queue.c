...
static inline uint32_t nix_qsize_to_val(enum nix_q_size qsize) {
  return (16UL << (qsize * 2));
}

static inline enum nix_q_size nix_qsize_clampup(uint32_t val) {
  int i = nix_q_size_16;
...
int roc_nix_rq_ena_dis(struct roc_nix_rq *rq, bool enable) {
...
int nix_rq_cfg(struct dev *dev, struct roc_nix_rq *rq, uint16_t qints, bool cfg,
  ...
  if (rq->spb_ena) {
    ...
    aq->rq.spb_ena = 1;
    aq->rq.spb_aura = roc_npa_aura_handle_to_aura(rq->spb_aura_handle);

    if (rq->spb_size & 0x7 || rq->spb_size > NIX_RQ_CN10K_SPB_MAX_SIZE)
      return -EINVAL;
  ...
  if (cfg) {
    if (rq->sso_ena) {
      ...
      if (rq->vwqe_ena) {
        ...
        aq->rq_mask.max_vsize_exp = ~aq->rq_mask.max_vsize_exp;
        aq->rq_mask.vtime_wait = ~aq->rq_mask.vtime_wait;
    ...
    if (rq->spb_ena) {
      ...
      aq->rq_mask.spb_high_sizem1 = ~aq->rq_mask.spb_high_sizem1;
    }
...
int roc_nix_rq_init(struct roc_nix *roc_nix, struct roc_nix_rq *rq, bool ena) {
...
int roc_nix_rq_modify(struct roc_nix *roc_nix, struct roc_nix_rq *rq,
                      bool ena) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
  bool is_cn9k = roc_model_is_cn9k();
...
int roc_nix_rq_fini(struct roc_nix_rq *rq) {
  /* Disabling RQ is sufficient */
  return roc_nix_rq_ena_dis(rq, false);
}

int roc_nix_cq_init(struct roc_nix *roc_nix, struct roc_nix_cq *cq) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
...
int roc_nix_cq_fini(struct roc_nix_cq *cq) {
  struct mbox *mbox;
  struct nix *nix;
  int rc;
...
static int sqb_pool_populate(struct roc_nix *roc_nix, struct roc_nix_sq *sq) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  ...
  sq->nb_sqb_bufs_adj = (sq->nb_sqb_bufs_adj * NIX_SQB_LOWER_THRESH) / 100;
  ...
  rc = roc_npa_pool_create(&sq->aura_handle, blk_sz, NIX_MAX_SQB, &aura, &pool);
  if (rc)
    goto fail;
...
static int sq_cn9k_init(struct nix *nix, struct roc_nix_sq *sq,
...
static int sq_cn9k_fini(struct nix *nix, struct roc_nix_sq *sq) {
  struct mbox *mbox = (&nix->dev)->mbox;
  ...
  while (count) {
    ...
    next_sqb = *(void **)((uintptr_t)sqb_buf +
                          (uint32_t)((sqes_per_sqb - 1) * sq->max_sqe_sz));
...
static int sq_init(struct nix *nix, struct roc_nix_sq *sq, uint32_t rr_quantum,
                   uint16_t smq) {
  struct mbox *mbox = (&nix->dev)->mbox;
  struct nix_cn10k_aq_enq_req *aq;
...
static int sq_fini(struct nix *nix, struct roc_nix_sq *sq) {
  struct mbox *mbox = (&nix->dev)->mbox;
...
int roc_nix_sq_init(struct roc_nix *roc_nix, struct roc_nix_sq *sq) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = (&nix->dev)->mbox;
  ...
  if (roc_model_is_cn9k()) {
    ...
    sq->lmt_addr =
        (void *)(nix->lmt_base + ((qid & RVU_CN9K_LMT_SLOT_MASK) << 12));
  }
...
int roc_nix_sq_fini(struct roc_nix_sq *sq) {
  struct nix *nix;
  struct mbox *mbox;
...
void roc_nix_cq_head_tail_get(struct roc_nix *roc_nix, uint16_t qid,
  ...
  if (val & (BIT_ULL(NIX_CQ_OP_STAT_OP_ERR) | BIT_ULL(NIX_CQ_OP_STAT_CQ_ERR)))
    val = 0;
...
void roc_nix_sq_head_tail_get(struct roc_nix *roc_nix, uint16_t qid,
                              uint32_t *head, uint32_t *tail) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
