...
static inline struct mbox_msghdr *mbox_alloc_msg(struct mbox *mbox, int devid,
                                                 int size) {
  return mbox_alloc_msg_rsp(mbox, devid, size, 0);
}

static inline void mbox_req_init(uint16_t mbox_id, void *msghdr) {
...
static inline void mbox_rsp_init(uint16_t mbox_id, void *msghdr) {
  struct mbox_msghdr *hdr = msghdr;
...
static inline bool mbox_nonempty(struct mbox *mbox, int devid) {
...
static inline int mbox_process(struct mbox *mbox) {
  mbox_msg_send(mbox, 0);
  return mbox_get_rsp(mbox, 0, NULL);
}

static inline int mbox_process_msg(struct mbox *mbox, void **msg) {
  mbox_msg_send(mbox, 0);
  return mbox_get_rsp(mbox, 0, msg);
}

static inline int mbox_process_tmo(struct mbox *mbox, uint32_t tmo) {
  mbox_msg_send(mbox, 0);
  return mbox_get_rsp_tmo(mbox, 0, NULL, tmo);
}

static inline int mbox_process_msg_tmo(struct mbox *mbox, void **msg,
                                       uint32_t tmo) {
  mbox_msg_send(mbox, 0);
...
#define M(_name, _id, _fn_name, _req_type, _rsp_type)                          \
  static inline struct _req_type *mbox_alloc_msg_##_fn_name(                   \
    ...
    req = (struct _req_type *)mbox_alloc_msg_rsp(                              \
        mbox, 0, sizeof(struct _req_type), sizeof(struct _rsp_type));          \
    if (!req)                                                                  \
      return NULL;                                                             \
 ...
 */
static inline volatile void *mbox_memcpy(volatile void *d,
                                         const volatile void *s, size_t l) {
 ...
 */
static inline void mbox_memset(volatile void *d, uint8_t val, size_t l) {
  volatile uint8_t *db;
  size_t i = 0;
