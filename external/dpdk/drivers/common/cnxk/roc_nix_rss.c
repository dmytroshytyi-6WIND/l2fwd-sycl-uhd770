...
void roc_nix_rss_key_default_fill(struct roc_nix *roc_nix,
                                  uint8_t key[ROC_NIX_RSS_KEY_LEN]) {
  PLT_SET_USED(roc_nix);
  const uint8_t default_key[ROC_NIX_RSS_KEY_LEN] = {
      0xFE, 0xED, 0x0B, 0xAD, 0xFE, 0xED, 0x0B, 0xAD, 0xFE, 0xED, 0x0B, 0xAD,
      0xFE, 0xED, 0x0B, 0xAD, 0xFE, 0xED, 0x0B, 0xAD, 0xFE, 0xED, 0x0B, 0xAD,
      0xFE, 0xED, 0x0B, 0xAD, 0xFE, 0xED, 0x0B, 0xAD, 0xFE, 0xED, 0x0B, 0xAD,
      0xFE, 0xED, 0x0B, 0xAD, 0xFE, 0xED, 0x0B, 0xAD, 0xFE, 0xED, 0x0B, 0xAD};
...
void roc_nix_rss_key_set(struct roc_nix *roc_nix,
...
void roc_nix_rss_key_get(struct roc_nix *roc_nix,
                         uint8_t key[ROC_NIX_RSS_KEY_LEN]) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
static int nix_cn9k_rss_reta_set(struct nix *nix, uint8_t group,
...
static int nix_rss_reta_set(struct nix *nix, uint8_t group,
                            uint16_t reta[ROC_NIX_RSS_RETA_MAX],
                            uint8_t lock_rx_ctx) {
  struct mbox *mbox = (&nix->dev)->mbox;
...
int roc_nix_rss_reta_set(struct roc_nix *roc_nix, uint8_t group,
                         uint16_t reta[ROC_NIX_RSS_RETA_MAX]) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  int rc;
...
int roc_nix_rss_reta_get(struct roc_nix *roc_nix, uint8_t group,
                         uint16_t reta[ROC_NIX_RSS_RETA_MAX]) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
int roc_nix_rss_flowkey_set(struct roc_nix *roc_nix, uint8_t *alg_idx,
...
int roc_nix_rss_default_setup(struct roc_nix *roc_nix, uint32_t flowkey) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
