...
#define roc_store_pair(val0, val1, addr)                                       \
  ({                                                                           \
    asm volatile("stp %x[x0], %x[x1], [%x[p1], #0]!" ::[x0] "r"(val0),         \
                 [x1] "r"(val1), [p1] "r"(addr));                              \
  })
...
static __plt_always_inline uint64_t roc_atomic64_cas(uint64_t compare,
                                                     uint64_t swap,
                                                     int64_t *ptr) {
  asm volatile(PLT_CPU_FEATURE_PREAMBLE "cas %[compare], %[swap], [%[ptr]]\n"
               : [compare] "+r"(compare)
...
static __plt_always_inline uint64_t roc_atomic64_casl(uint64_t compare,
                                                      uint64_t swap,
                                                      int64_t *ptr) {
  asm volatile(PLT_CPU_FEATURE_PREAMBLE "casl %[compare], %[swap], [%[ptr]]\n"
               : [compare] "+r"(compare)
               : [swap] "r"(swap), [ptr] "r"(ptr)
               : "memory");
...
static __plt_always_inline uint64_t roc_atomic64_add_nosync(int64_t incr,
                                                            int64_t *ptr) {
  uint64_t result;
...
static __plt_always_inline uint64_t roc_atomic64_add_sync(int64_t incr,
                                                          int64_t *ptr) {
  uint64_t result;
...
static __plt_always_inline void roc_lmt_submit_steor(uint64_t data,
                                                     plt_iova_t io_address) {
  asm volatile(PLT_CPU_FEATURE_PREAMBLE "steor %x[d], [%[rs]]" ::[d] "r"(data),
               [rs] "r"(io_address));
}

static __plt_always_inline void roc_lmt_submit_steorl(uint64_t data,
                                                      plt_iova_t io_address) {
  asm volatile(PLT_CPU_FEATURE_PREAMBLE "steorl %x[d], [%[rs]]" ::[d] "r"(data),
               [rs] "r"(io_address));
}

static __plt_always_inline void roc_lmt_mov(void *out, const void *in,
                                            const uint32_t lmtext) {
...
static __plt_always_inline void roc_lmt_mov64(void *out, const void *in) {
  volatile const __uint128_t *src128 = (const __uint128_t *)in;
  volatile __uint128_t *dst128 = (__uint128_t *)out;
...
static __plt_always_inline void roc_lmt_mov_nv(void *out, const void *in,
                                               const uint32_t lmtext) {
...
static __plt_always_inline void roc_lmt_mov_seg(void *out, const void *in,
...
static __plt_always_inline void roc_lmt_mov_one(void *out, const void *in) {
  volatile const __uint128_t *src128 = (const __uint128_t *)in;
  volatile __uint128_t *dst128 = (__uint128_t *)out;
...
static __plt_always_inline void roc_lmt_mov_seg_nv(void *out, const void *in,
                                                   const uint16_t segdw) {
  const __uint128_t *src128 = (const __uint128_t *)in;
  __uint128_t *dst128 = (__uint128_t *)out;
  uint8_t i;
...
static __plt_always_inline void roc_atf_ret(void) {
