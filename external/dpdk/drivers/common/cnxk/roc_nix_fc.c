...
static int nix_fc_rxchan_bpid_get(struct roc_nix *roc_nix,
...
static int nix_fc_rxchan_bpid_set(struct roc_nix *roc_nix, bool enable) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
static int nix_fc_cq_config_get(struct roc_nix *roc_nix,
                                struct roc_nix_fc_cfg *fc_cfg) {
  struct mbox *mbox = get_mbox(roc_nix);
...
static int nix_fc_cq_config_set(struct roc_nix *roc_nix,
                                struct roc_nix_fc_cfg *fc_cfg) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = get_mbox(roc_nix);
...
int roc_nix_fc_config_get(struct roc_nix *roc_nix,
                          struct roc_nix_fc_cfg *fc_cfg) {
...
int roc_nix_fc_config_set(struct roc_nix *roc_nix,
                          struct roc_nix_fc_cfg *fc_cfg) {
  if (roc_nix_is_vf_or_sdp(roc_nix) && !roc_nix_is_lbk(roc_nix))
    return 0;
  ...
  if (fc_cfg->type == ROC_NIX_FC_CQ_CFG)
    ...
    return nix_fc_rxchan_bpid_set(roc_nix, fc_cfg->rxchan_cfg.enable);
  else if (fc_cfg->type == ROC_NIX_FC_TM_CFG)
    return nix_tm_bp_config_set(roc_nix, fc_cfg->tm_cfg.sq, fc_cfg->tm_cfg.tc,
                                fc_cfg->tm_cfg.enable);

  return -EINVAL;
}

enum roc_nix_fc_mode roc_nix_fc_mode_get(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = get_mbox(roc_nix);
...
int roc_nix_fc_mode_set(struct roc_nix *roc_nix, enum roc_nix_fc_mode mode) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
  struct mbox *mbox = get_mbox(roc_nix);
...
void rox_nix_fc_npa_bp_cfg(struct roc_nix *roc_nix, uint64_t pool_id,
  ...
  if (rsp->aura.bp_ena) {
    ...
    if (((nix1 != nix->is_nix1) || (bpid != nix->bpid[0])) && !force) {
      req = mbox_alloc_msg_npa_aq_enq(mbox);
      if (req == NULL)
        return;
  ...
  if (ena) {
    ...
    req->aura.bp =
        NIX_RQ_AURA_THRESH(limit > 128 ? 256 : limit); /* 95% of size*/
...
int roc_nix_pfc_mode_set(struct roc_nix *roc_nix,
  ...
  rx_pause =
      (pfc_cfg->mode == ROC_NIX_FC_FULL) || (pfc_cfg->mode == ROC_NIX_FC_RX);
  tx_pause =
      (pfc_cfg->mode == ROC_NIX_FC_FULL) || (pfc_cfg->mode == ROC_NIX_FC_TX);
...
int roc_nix_pfc_mode_get(struct roc_nix *roc_nix,
                         struct roc_nix_pfc_cfg *pfc_cfg) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
...
uint16_t roc_nix_chan_count_get(struct roc_nix *roc_nix) {
  struct nix *nix = roc_nix_to_nix_priv(roc_nix);
