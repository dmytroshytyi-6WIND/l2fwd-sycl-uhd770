...
static void nix_inl_sso_work_cb(struct nix_inl_dev *inl_dev) {
...
again:
  ...
  if (work) {
    if (inl_dev->work_cb)
      ...
      plt_warn("Undelivered inl dev work gw0: %p gw1: %p", (void *)gw.u64[0],
               (void *)gw.u64[1]);
    cnt++;
    if (cnt < WORK_LIMIT)
      goto again;
...
static int nix_inl_nix_reg_dump(struct nix_inl_dev *inl_dev) {
...
static void nix_inl_sso_hwgrp_irq(void *param) {
...
static void nix_inl_sso_hws_irq(void *param) {
  struct nix_inl_dev *inl_dev = (struct nix_inl_dev *)param;
...
int nix_inl_sso_register_irqs(struct nix_inl_dev *inl_dev) {
  ...
  if (sso_msixoff == MSIX_VECTOR_INVALID ||
    ...
    plt_err("Invalid SSO/SSOW MSIX offsets (0x%x, 0x%x)", sso_msixoff,
            ssow_msixoff);
    return -EINVAL;
  }
...
static void nix_inl_nix_q_irq(void *param) {
  struct nix_inl_dev *inl_dev = (struct nix_inl_dev *)param;
  uintptr_t nix_base = inl_dev->nix_base;
  ...
  reg = roc_atomic64_add_nosync(0, (int64_t *)(nix_base + NIX_LF_RQ_OP_INT));
...
static void nix_inl_nix_ras_irq(void *param) {
  struct nix_inl_dev *inl_dev = (struct nix_inl_dev *)param;
  uintptr_t nix_base = inl_dev->nix_base;
  struct dev *dev = &inl_dev->dev;
  volatile void *ctx;
...
static void nix_inl_nix_err_irq(void *param) {
  struct nix_inl_dev *inl_dev = (struct nix_inl_dev *)param;
  uintptr_t nix_base = inl_dev->nix_base;
  struct dev *dev = &inl_dev->dev;
  volatile void *ctx;
...
int nix_inl_nix_register_irqs(struct nix_inl_dev *inl_dev) {
  ...
  plt_write64(~(BIT_ULL(11) | BIT_ULL(24)), nix_base + NIX_LF_ERR_INT_ENA_W1S);
