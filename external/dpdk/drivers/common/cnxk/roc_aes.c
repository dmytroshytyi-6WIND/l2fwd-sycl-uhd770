...
static uint32_t sub_word(uint32_t word) {
  word = (Sbox[(word >> 24) & 0xFF] << 24) | (Sbox[(word >> 16) & 0xFF] << 16) |
 ...
 * polynomial
 */
static uint8_t Xtime(uint8_t byte, uint8_t pow) {
  uint32_t w = byte;
 ...
 */
static uint8_t GF8mul(uint8_t byte, uint32_t mp) {
  uint8_t pow, mul = 0;
...
static void aes_key_expand(const uint8_t *key, uint32_t *ks) {
  unsigned int i = 4;
  uint32_t temp;
...
static void shift_word(uint8_t *sRc, uint8_t c, int count) {
...
static void mix_columns(uint8_t *sRc) {
  uint8_t new_st[4];
  int i;

  for (i = 0; i < 4; i++)
    new_st[i] = GF8mul(sRc[i], 0x2) ^ GF8mul(sRc[(i + 1) & 0x3], 0x3) ^
                sRc[(i + 2) & 0x3] ^ sRc[(i + 3) & 0x3];
  for (i = 0; i < 4; i++)
    sRc[i] = new_st[i];
}

static void cipher(uint8_t *in, uint8_t *out, uint32_t *ks) {
