...
static inline uint16_t msgs_offset(void) {
...
struct mbox_msghdr *mbox_alloc_msg_rsp(struct mbox *mbox, int devid, int size,
  ...
  msghdr = (struct mbox_msghdr *)(((uintptr_t)mdev->mbase + mbox->tx_start +
 ...
 */
static int mbox_poll(struct mbox *mbox, uint32_t wait) {
...
static int mbox_wait(struct mbox *mbox, int devid, uint32_t rst_timo) {
  ...
  while (mdev->num_msgs > mdev->msgs_acked) {
    ...
    if (timeout >= rst_timo) {
      ...
      plt_err("MBOX[devid: %d] message wait timeout %d, "
              ...
              "rsp_size: %d",
              devid, timeout, mdev->num_msgs, mdev->msgs_acked,
              tx_hdr->num_msgs, rx_hdr->num_msgs, mdev->msg_size,
              mdev->rsp_size);
...
int send_ready_msg(struct mbox *mbox, uint16_t *pcifunc) {
...
int reply_invalid_msg(struct mbox *mbox, int devid, uint16_t pcifunc,
                      uint16_t id) {
  struct msg_rsp *rsp;
