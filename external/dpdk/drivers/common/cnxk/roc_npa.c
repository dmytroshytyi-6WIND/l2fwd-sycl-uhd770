...
int roc_npa_lf_init_cb_register(roc_npa_lf_init_cb_t cb) {
  if (lf_init_cb != NULL)
    return -EEXIST;
...
void roc_npa_aura_op_range_set(uint64_t aura_handle, uint64_t start_iova,
                               uint64_t end_iova) {
  const uint64_t start =
      roc_npa_aura_handle_to_base(aura_handle) + NPA_LF_POOL_OP_PTR_START0;
  const uint64_t end =
      roc_npa_aura_handle_to_base(aura_handle) + NPA_LF_POOL_OP_PTR_END0;
...
static int npa_aura_pool_init(struct mbox *mbox, uint32_t aura_id,
...
static int npa_aura_pool_fini(struct mbox *mbox, uint32_t aura_id,
...
int roc_npa_pool_op_pc_reset(uint64_t aura_handle) {
  struct npa_lf *lf = idev_npa_obj_get();
...
int roc_npa_aura_drop_set(uint64_t aura_handle, uint64_t limit, bool ena) {
  ...
  aura_req->aura_mask.aura_drop_ena = ~(aura_req->aura_mask.aura_drop_ena);
...
static inline char *npa_stack_memzone_name(struct npa_lf *lf, int pool_id,
                                           char *name) {
...
static inline int npa_stack_dma_free(struct npa_lf *lf, char *name,
                                     int pool_id) {
...
npa_aura_pool_pair_alloc(struct npa_lf *lf, const uint32_t block_size,
  ...
  /* Sanity check */
  if (!lf || !block_size || !block_count || !pool || !aura || !aura_handle)
    return NPA_ERR_PARAM;
...
int roc_npa_pool_create(uint64_t *aura_handle, uint32_t block_size,
  ...
  plt_npa_dbg("lf=%p block_sz=%d block_count=%d aura_handle=0x%" PRIx64, lf,
...
int roc_npa_aura_limit_modify(uint64_t aura_handle, uint16_t aura_limit) {
  struct npa_aq_enq_req *aura_req;
  struct npa_lf *lf;
  int rc;
...
static int npa_aura_pool_pair_free(struct npa_lf *lf, uint64_t aura_handle) {
  char name[PLT_MEMZONE_NAMESIZE];
...
int roc_npa_pool_destroy(uint64_t aura_handle) {
  struct npa_lf *lf = idev_npa_obj_get();
  int rc = 0;
...
int roc_npa_pool_range_update_check(uint64_t aura_handle) {
...
static inline int npa_attach(struct mbox *mbox) {
...
static inline int npa_detach(struct mbox *mbox) {
...
static inline int npa_get_msix_offset(struct mbox *mbox,
...
static inline int npa_lf_alloc(struct npa_lf *lf) {
...
static int npa_lf_free(struct mbox *mbox) {
  mbox_alloc_msg_npa_lf_free(mbox);
  return mbox_process(mbox);
}

static inline uint32_t aura_size_to_u32(uint8_t val) {
...
static inline void pool_count_aura_sz_get(uint32_t *nr_pools,
                                          uint8_t *aura_sz) {
  uint32_t val;
...
static int npa_dev_init(struct npa_lf *lf, uintptr_t base, struct mbox *mbox) {
...
static int npa_dev_fini(struct npa_lf *lf) {
  if (!lf)
    return NPA_ERR_PARAM;
...
int npa_lf_init(struct dev *dev, struct plt_pci_device *pci_dev) {
  ...
  lf = &dev->npa;
  rc = npa_dev_init(lf, dev->bar2 + (RVU_BLOCK_ADDR_NPA << 20), dev->mbox);
  if (rc)
    goto npa_detach;
...
int roc_npa_dev_init(struct roc_npa *roc_npa) {
...
int roc_npa_dev_fini(struct roc_npa *roc_npa) {
