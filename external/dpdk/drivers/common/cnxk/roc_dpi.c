...
static inline int send_msg_to_pf(struct plt_pci_addr *pci_addr,
                                 const char *value, int size) {
  char buf[255] = {0};
  int res, fd;

  res = snprintf(buf, sizeof(buf), "/sys/bus/pci/devices/" PCI_PRI_FMT "/%s",
...
int roc_dpi_enable(struct roc_dpi *dpi) {
  plt_write64(0x1, dpi->rbase + DPI_VDMA_EN);
  return 0;
}

int roc_dpi_disable(struct roc_dpi *dpi) {
  plt_write64(0x0, dpi->rbase + DPI_VDMA_EN);
  return 0;
}

int roc_dpi_configure(struct roc_dpi *roc_dpi) {
  ...
  rc = roc_npa_pool_create(&aura_handle, DPI_CMD_QUEUE_SIZE, DPI_CMD_QUEUE_BUFS,
                           &aura, &pool);
  if (rc) {
  ...
  if (rc < 0) {
    plt_err("Failed to send mbox message %d to DPI PF, err %d", mbox_msg.s.cmd,
            rc);
    goto err2;
  }
...
int roc_dpi_dev_init(struct roc_dpi *roc_dpi) {
...
int roc_dpi_dev_fini(struct roc_dpi *roc_dpi) {
  struct plt_pci_device *pci_dev = roc_dpi->pci_dev;
  dpi_mbox_msg_t mbox_msg;
  uint64_t reg;
  int rc;
