...
static void nix_inl_selftest_work_cb(uint64_t *gw, void *args,
...
static int nix_inl_selftest(void) {
  struct idev_cfg *idev = idev_get_cfg();
...
#define WORK_MAGIC2 0xdeadbeef0
  ...
  if (work_arr[0] != WORK_MAGIC1 || work_arr[1] != WORK_MAGIC2) {
    plt_err("Failed to get expected work, [0]=%p [1]=%p", (void *)work_arr[0],
            (void *)work_arr[1]);
    rc = -EFAULT;
    goto exit;
  }

  plt_info("Work, [0]=%p [1]=%p", (void *)work_arr[0], (void *)work_arr[1]);
...
static int nix_inl_cpt_ctx_cache_sync(struct nix_inl_dev *inl_dev) {
...
static int nix_inl_nix_ipsec_cfg(struct nix_inl_dev *inl_dev, bool ena) {
  ...
  if (ena) {
    ...
    lf_cfg->ipsec_cfg0.sa_pow2_size = plt_log2_u32(inl_dev->inb_sa_sz);
...
static int nix_inl_cpt_setup(struct nix_inl_dev *inl_dev) {
...
static int nix_inl_cpt_release(struct nix_inl_dev *inl_dev) {
  struct roc_cpt_lf *lf = &inl_dev->cpt_lf;
  struct dev *dev = &inl_dev->dev;
  int rc, ret = 0;
...
static int nix_inl_sso_setup(struct nix_inl_dev *inl_dev) {
  ...
  rc = sso_hwgrp_init_xaq_aura(dev, &inl_dev->xaq, inl_dev->nb_xae,
                               inl_dev->xae_waes, inl_dev->xaq_buf_size, 1);
  if (rc) {
...
static int nix_inl_sso_release(struct nix_inl_dev *inl_dev) {
  uint16_t hwgrp[1] = {0};
...
static int nix_inl_nix_setup(struct nix_inl_dev *inl_dev) {
...
static int nix_inl_nix_release(struct nix_inl_dev *inl_dev) {
  struct dev *dev = &inl_dev->dev;
  struct mbox *mbox = dev->mbox;
...
static int nix_inl_lf_attach(struct nix_inl_dev *inl_dev) {
...
static int nix_inl_lf_detach(struct nix_inl_dev *inl_dev) {
  struct dev *dev = &inl_dev->dev;
  struct mbox *mbox = dev->mbox;
...
static int nix_inl_dev_wait_for_sso_empty(struct nix_inl_dev *inl_dev) {
...
int roc_nix_inl_dev_xaq_realloc(uint64_t aura_handle) {
  struct idev_cfg *idev = idev_get_cfg();
  struct nix_inl_dev *inl_dev;
  int rc, i;
...
no_pool:
  ...
  inl_dev->pkt_pools = plt_realloc(
      inl_dev->pkt_pools, sizeof(uint64_t *) * inl_dev->pkt_pools_cnt, 0);
  ...
  /* Realloc XAQ aura */
  rc = sso_hwgrp_init_xaq_aura(&inl_dev->dev, &inl_dev->xaq, inl_dev->nb_xae,
                               inl_dev->xae_waes, inl_dev->xaq_buf_size, 1);
  if (rc) {
...
static void inl_outb_soft_exp_poll(struct nix_inl_dev *inl_dev,
                                   uint32_t ring_idx) {
  ...
  while (tail_l != head_l) {
    ...
    while (poll_counter++ < ROC_NIX_INL_SA_SOFT_EXP_ERR_MAX_POLL_COUNT) {
      plt_delay_us(20);
      entry.u64 = __atomic_load_n(ring_base + tail_l + 1, __ATOMIC_ACQUIRE);
      if (likely(entry.u64))
        break;
    }

    entry.u64 = plt_be_to_cpu_64(entry.u64);
    sa = (struct roc_ot_ipsec_outb_sa *)(((uint64_t)entry.s.data1 << 51) |
                                         (entry.s.data0 << 7));
    ...
    if (sa != NULL) {
      ...
      __atomic_add_fetch((uint32_t *)ring_base, 1, __ATOMIC_ACQ_REL);
    } else
      plt_err("Invalid SA");
...
static void *nix_inl_outb_poll_thread(void *args) {
  ...
  while (!soft_exp_poll_thread_exit) {
    if (soft_exp_consumer_cnt) {
      for (i = 0; i < ROC_NIX_INL_MAX_SOFT_EXP_RNGS; i++) {
        bit = plt_bitmap_get(inl_dev->soft_exp_ring_bmap, i);
        if (bit)
...
static int nix_inl_outb_poll_thread_setup(struct nix_inl_dev *inl_dev) {
   ...
   */
  bmap_sz = plt_bitmap_get_memory_footprint(ROC_NIX_INL_MAX_SOFT_EXP_RNGS);
  ...
  inl_dev->sa_soft_exp_ring =
      plt_zmalloc(ROC_NIX_INL_MAX_SOFT_EXP_RNGS * sizeof(uint64_t), 0);
...
int roc_nix_inl_dev_init(struct roc_nix_inl_dev *roc_inl_dev) {
  ...
  if (idev->nix_inl_dev) {
    plt_info("Skipping device %s, inline device already probed", pci_dev->name);
    return -EEXIST;
  }
...
int roc_nix_inl_dev_fini(struct roc_nix_inl_dev *roc_inl_dev) {
  struct plt_pci_device *pci_dev;
  struct nix_inl_dev *inl_dev;
  struct idev_cfg *idev;
  int rc;
