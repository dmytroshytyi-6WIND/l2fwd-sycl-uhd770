...
static int sso_rsrc_attach(struct roc_sso *roc_sso, enum sso_lf_type lf_type,
                           uint16_t nb_lf) {
...
static int sso_rsrc_detach(struct roc_sso *roc_sso, enum sso_lf_type lf_type) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;
...
static int sso_rsrc_get(struct roc_sso *roc_sso) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;
...
void sso_hws_link_modify(uint8_t hws, uintptr_t base, struct plt_bitmap *bmp,
  ...
  while (n) {
    ...
    for (j = 0; j < k; j++) {
      ...
      plt_sso_dbg("HWS %d Linked to HWGRP %d", hws, hwgrp[i + j]);
    }
...
static int sso_msix_fill(struct roc_sso *roc_sso, uint16_t nb_hws,
                         uint16_t nb_hwgrp) {
...
/* Public Functions. */
uintptr_t roc_sso_hws_base_get(struct roc_sso *roc_sso, uint8_t hws) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;

  return dev->bar2 + (RVU_BLOCK_ADDR_SSOW << 20 | hws << 12);
}

uintptr_t roc_sso_hwgrp_base_get(struct roc_sso *roc_sso, uint16_t hwgrp) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;

  return dev->bar2 + (RVU_BLOCK_ADDR_SSO << 20 | hwgrp << 12);
}

uint64_t roc_sso_ns_to_gw(struct roc_sso *roc_sso, uint64_t ns) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;
...
int roc_sso_hws_link(struct roc_sso *roc_sso, uint8_t hws, uint16_t hwgrp[],
                     uint16_t nb_hwgrp) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;
  struct sso *sso;
  uintptr_t base;
...
int roc_sso_hws_unlink(struct roc_sso *roc_sso, uint8_t hws, uint16_t hwgrp[],
                       uint16_t nb_hwgrp) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;
  struct sso *sso;
  uintptr_t base;
...
int roc_sso_hws_stats_get(struct roc_sso *roc_sso, uint8_t hws,
  ...
  req_rsp = (struct sso_hws_stats *)mbox_alloc_msg_sso_hws_get_stats(dev->mbox);
  if (req_rsp == NULL) {
    rc = mbox_process(dev->mbox);
    if (rc < 0)
      return rc;
    req_rsp =
        (struct sso_hws_stats *)mbox_alloc_msg_sso_hws_get_stats(dev->mbox);
    if (req_rsp == NULL)
      return -ENOSPC;
  }
  req_rsp->hws = hws;
...
int roc_sso_hwgrp_stats_get(struct roc_sso *roc_sso, uint8_t hwgrp,
  ...
  req_rsp = (struct sso_grp_stats *)mbox_alloc_msg_sso_grp_get_stats(dev->mbox);
  if (req_rsp == NULL) {
    rc = mbox_process(dev->mbox);
    if (rc < 0)
      return rc;
    req_rsp =
        (struct sso_grp_stats *)mbox_alloc_msg_sso_grp_get_stats(dev->mbox);
    if (req_rsp == NULL)
      return -ENOSPC;
  }
  req_rsp->grp = hwgrp;
...
int roc_sso_hwgrp_hws_link_status(struct roc_sso *roc_sso, uint8_t hws,
                                  uint16_t hwgrp) {
  struct sso *sso;
...
int roc_sso_hwgrp_qos_config(struct roc_sso *roc_sso,
                             struct roc_sso_hwgrp_qos *qos, uint8_t nb_qos,
                             uint32_t nb_xaq) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;
  ...
  for (i = 0; i < nb_qos; i++) {
    ...
    req->taq_thr =
        (SSO_HWGRP_IAQ_MAX_THR_MASK * (iaq_prcnt ? iaq_prcnt : 100)) / 100;
    req->iaq_thr =
        (SSO_HWGRP_TAQ_MAX_THR_MASK * (taq_prcnt ? taq_prcnt : 100)) / 100;
  }
...
int sso_hwgrp_init_xaq_aura(struct dev *dev, struct roc_sso_xaq_data *xaq,
  ...
  rc = roc_npa_pool_create(&xaq->aura_handle, xaq_buf_size, xaq->nb_xaq, &aura,
                           &pool);
  if (rc) {
...
int roc_sso_hwgrp_init_xaq_aura(struct roc_sso *roc_sso, uint32_t nb_xae) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;

  return sso_hwgrp_init_xaq_aura(dev, &roc_sso->xaq, nb_xae, roc_sso->xae_waes,
                                 roc_sso->xaq_buf_size, roc_sso->nb_hwgrp);
}
...
int sso_hwgrp_alloc_xaq(struct dev *dev, uint32_t npa_aura_id,
                        uint16_t hwgrps) {
...
int roc_sso_hwgrp_alloc_xaq(struct roc_sso *roc_sso, uint32_t npa_aura_id,
                            uint16_t hwgrps) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;
...
int roc_sso_hwgrp_release_xaq(struct roc_sso *roc_sso, uint16_t hwgrps) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;

  return sso_hwgrp_release_xaq(dev, hwgrps);
}

int roc_sso_hwgrp_set_priority(struct roc_sso *roc_sso, uint16_t hwgrp,
                               uint8_t weight, uint8_t affinity,
                               uint8_t priority) {
  struct dev *dev = &roc_sso_to_sso_priv(roc_sso)->dev;
...
int roc_sso_rsrc_init(struct roc_sso *roc_sso, uint8_t nb_hws,
                      uint16_t nb_hwgrp) {
  struct sso *sso = roc_sso_to_sso_priv(roc_sso);
  ...
  rc =
      sso_lf_alloc(&sso->dev, SSO_LF_TYPE_HWGRP, nb_hwgrp, (void **)&rsp_hwgrp);
  if (rc < 0) {
...
void roc_sso_rsrc_fini(struct roc_sso *roc_sso) {
  struct sso *sso = roc_sso_to_sso_priv(roc_sso);

  if (!roc_sso->nb_hws && !roc_sso->nb_hwgrp)
    return;

  sso_unregister_irqs_priv(roc_sso, sso->pci_dev->intr_handle, roc_sso->nb_hws,
                           roc_sso->nb_hwgrp);
...
int roc_sso_dev_init(struct roc_sso *roc_sso) {
  ...
  for (i = 0; i < roc_sso->max_hws; i++) {
    sso->link_map[i] =
        plt_bitmap_init(roc_sso->max_hwgrp, link_mem, link_map_sz);
...
int roc_sso_dev_fini(struct roc_sso *roc_sso) {
  struct sso *sso;
