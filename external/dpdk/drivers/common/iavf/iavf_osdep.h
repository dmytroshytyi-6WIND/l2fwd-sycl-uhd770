...
#define iavf_debug(h, m, s, ...)                                               \
  do {                                                                         \
    if (((m) & (h)->debug_mask))                                               \
      rte_log(RTE_LOG_DEBUG, iavf_common_logger, "iavf %02x.%x " s,            \
              (h)->bus.device, (h)->bus.func, ##__VA_ARGS__);                  \
  } while (0)
...
static inline void iavf_acquire_spinlock_d(struct iavf_spinlock *sp) {
  rte_spinlock_lock(&sp->spinlock);
}

static inline void iavf_release_spinlock_d(struct iavf_spinlock *sp) {
