...
enum virtchnl_ops {
   ...
   * capabilities exchange and are not considered part of base mode feature set.
   */
...
#define VF_BASE_MODE_OFFLOADS                                                  \
  (VIRTCHNL_VF_OFFLOAD_L2 | VIRTCHNL_VF_OFFLOAD_VLAN |                         \
...
#define VIRTCHNL_DCF_VLAN_TYPE_M (0x1 << VIRTCHNL_DCF_VLAN_TYPE_S)
#define VIRTCHNL_DCF_VLAN_TYPE_INNER 0x0
#define VIRTCHNL_DCF_VLAN_TYPE_OUTER 0x1
#define VIRTCHNL_DCF_VLAN_INSERT_MODE_S 1
#define VIRTCHNL_DCF_VLAN_INSERT_MODE_M (0x7 << VIRTCHNL_DCF_VLAN_INSERT_MODE_S)
...
#define VIRTCHNL_DCF_VLAN_STRIP_MODE_M (0x7 << VIRTCHNL_DCF_VLAN_STRIP_MODE_S)
 ...
 *
 * If VIRTCHNL_VF_LARGE_NUM_QPAIRS was negotiated in
 * VIRTCHNL_OP_GET_VF_RESOURCES then VIRTCHNL_OP_ENABLE_QUEUES_V2 and
 * VIRTCHNL_OP_DISABLE_QUEUES_V2 are available.
 *
 ...
 *
 * If VIRTCHNL_VF_LARGE_NUM_QPAIRS was negotiated in
 * VIRTCHNL_OP_GET_VF_RESOURCES then only VIRTCHNL_OP_MAP_QUEUE_VECTOR is
 * available.
 *
 ...
 */
static inline int virtchnl_vc_validate_vf_msg(struct virtchnl_version_info *ver,
                                              u32 v_opcode, u8 *msg,
                                              u16 msglen) {
  ...
  case VIRTCHNL_OP_CONFIG_VSI_QUEUES:
    ...
    if (msglen >= valid_len) {
      ...
      if (vqc->num_queue_pairs == 0 ||
          vqc->num_queue_pairs > VIRTCHNL_OP_CONFIG_VSI_QUEUES_MAX) {
        err_msg_format = true;
        break;
      }

      valid_len +=
          (vqc->num_queue_pairs * sizeof(struct virtchnl_queue_pair_info));
    }
    break;
  case VIRTCHNL_OP_CONFIG_IRQ_MAP:
    ...
    if (msglen >= valid_len) {
      ...
      valid_len += (vimi->num_vectors * sizeof(struct virtchnl_vector_map));
    }
    break;
  case VIRTCHNL_OP_ENABLE_QUEUES:
  case VIRTCHNL_OP_DISABLE_QUEUES:
  ...
  case VIRTCHNL_OP_DEL_ETH_ADDR:
    ...
    if (msglen >= valid_len) {
      ...
      if (veal->num_elements == 0 ||
          veal->num_elements > VIRTCHNL_OP_ADD_DEL_ETH_ADDR_MAX) {
        err_msg_format = true;
        break;
      }

      valid_len += veal->num_elements * sizeof(struct virtchnl_ether_addr);
    }
    break;
  case VIRTCHNL_OP_ADD_VLAN:
  case VIRTCHNL_OP_DEL_VLAN:
  ...
  case VIRTCHNL_OP_ENABLE_CHANNELS:
    ...
    if (msglen >= valid_len) {
      ...
      if (vti->num_tc == 0 || vti->num_tc > VIRTCHNL_OP_ENABLE_CHANNELS_MAX) {
        err_msg_format = true;
        break;
      }

      valid_len += (vti->num_tc - 1) * sizeof(struct virtchnl_channel_info);
    }
    break;
  case VIRTCHNL_OP_DISABLE_CHANNELS:
    break;
  ...
  case VIRTCHNL_OP_DCF_CONFIG_BW:
    ...
    if (msglen >= valid_len) {
      ...
      valid_len +=
          (cfg_list->num_elem - 1) * sizeof(struct virtchnl_dcf_bw_cfg);
    }
    break;
  case VIRTCHNL_OP_GET_SUPPORTED_RXDIDS:
    break;
  ...
  case VIRTCHNL_OP_DEL_VLAN_V2:
    ...
    if (msglen >= valid_len) {
      ...
      if (vfl->num_elements == 0 ||
          vfl->num_elements > VIRTCHNL_OP_ADD_DEL_VLAN_V2_MAX) {
        err_msg_format = true;
        break;
      }

      valid_len +=
          (vfl->num_elements - 1) * sizeof(struct virtchnl_vlan_filter);
    }
    break;
  case VIRTCHNL_OP_ENABLE_VLAN_STRIPPING_V2:
  case VIRTCHNL_OP_DISABLE_VLAN_STRIPPING_V2:
  ...
  case VIRTCHNL_OP_DISABLE_QUEUES_V2:
    ...
    if (msglen >= valid_len) {
      ...
      valid_len +=
          (qs->chunks.num_chunks - 1) * sizeof(struct virtchnl_queue_chunk);
    }
    break;
  case VIRTCHNL_OP_MAP_QUEUE_VECTOR:
    ...
    if (msglen >= valid_len) {
      ...
      valid_len +=
          (v_qp->num_qv_maps - 1) * sizeof(struct virtchnl_queue_vector);
    }
    break;
