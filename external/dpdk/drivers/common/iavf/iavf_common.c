 ...
 **/
STATIC enum iavf_status iavf_aq_get_set_rss_lut(struct iavf_hw *hw, u16 vsi_id,
                                                bool pf_lut, u8 *lut,
  ...
  if (set)
    iavf_fill_default_direct_cmd_desc(&desc, iavf_aqc_opc_set_rss_lut);
  else
    iavf_fill_default_direct_cmd_desc(&desc, iavf_aqc_opc_get_rss_lut);
  ...
  cmd_resp->vsi_id =
      CPU_TO_LE16((u16)((vsi_id << IAVF_AQC_SET_RSS_LUT_VSI_ID_SHIFT) &
                        IAVF_AQC_SET_RSS_LUT_VSI_ID_MASK));
  cmd_resp->vsi_id |= CPU_TO_LE16((u16)IAVF_AQC_SET_RSS_LUT_VSI_VALID);

  if (pf_lut)
    cmd_resp->flags |=
        CPU_TO_LE16((u16)((IAVF_AQC_SET_RSS_LUT_TABLE_TYPE_PF
                           << IAVF_AQC_SET_RSS_LUT_TABLE_TYPE_SHIFT) &
                          IAVF_AQC_SET_RSS_LUT_TABLE_TYPE_MASK));
  else
    cmd_resp->flags |=
        CPU_TO_LE16((u16)((IAVF_AQC_SET_RSS_LUT_TABLE_TYPE_VSI
                           << IAVF_AQC_SET_RSS_LUT_TABLE_TYPE_SHIFT) &
                          IAVF_AQC_SET_RSS_LUT_TABLE_TYPE_MASK));
...
enum iavf_status iavf_aq_get_rss_lut(struct iavf_hw *hw, u16 vsi_id,
  ...
  return iavf_aq_get_set_rss_lut(hw, vsi_id, pf_lut, lut, lut_size, false);
}
...
iavf_aq_get_set_rss_key(struct iavf_hw *hw, u16 vsi_id,
  ...
  if (set)
    ...
    iavf_fill_default_direct_cmd_desc(&desc, iavf_aqc_opc_get_rss_key);
  ...
  cmd_resp->vsi_id =
      CPU_TO_LE16((u16)((vsi_id << IAVF_AQC_SET_RSS_KEY_VSI_ID_SHIFT) &
...
#define IAVF_PTT(PTYPE, OUTER_IP, OUTER_IP_VER, OUTER_FRAG, T, TE, TEF, I, PL) \
  {                                                                            \
    PTYPE, 1, IAVF_RX_PTYPE_OUTER_##OUTER_IP,                                  \
        IAVF_RX_PTYPE_OUTER_##OUTER_IP_VER, IAVF_RX_PTYPE_##OUTER_FRAG,        \
        IAVF_RX_PTYPE_TUNNEL_##T, IAVF_RX_PTYPE_TUNNEL_END_##TE,               \
        IAVF_RX_PTYPE_##TEF, IAVF_RX_PTYPE_INNER_PROT_##I,                     \
...
iavf_aq_send_msg_to_pf(struct iavf_hw *hw, enum virtchnl_ops v_opcode,
  ...
  if (msglen) {
    desc.flags |= CPU_TO_LE16((u16)(IAVF_AQ_FLAG_BUF | IAVF_AQ_FLAG_RD));
  ...
  status = iavf_asq_send_command(hw, (struct iavf_aq_desc *)&desc, msg, msglen,
                                 cmd_details);
  return status;
}
...
void iavf_vf_parse_hw_config(struct iavf_hw *hw,
  ...
  for (i = 0; i < msg->num_vsis; i++) {
    if (vsi_res->vsi_type == VIRTCHNL_VSI_SRIOV) {
      iavf_memcpy(hw->mac.perm_addr, vsi_res->default_mac_addr, ETH_ALEN,
                  IAVF_NONDMA_TO_NONDMA);
      iavf_memcpy(hw->mac.addr, vsi_res->default_mac_addr, ETH_ALEN,
                  IAVF_NONDMA_TO_NONDMA);
    }
    vsi_res++;
  }
...
enum iavf_status iavf_vf_reset(struct iavf_hw *hw) {
  return iavf_aq_send_msg_to_pf(hw, VIRTCHNL_OP_RESET_VF, IAVF_SUCCESS, NULL, 0,
                                NULL);
}
...
iavf_aq_clear_all_wol_filters(struct iavf_hw *hw,
  ...
  iavf_fill_default_direct_cmd_desc(&desc, iavf_aqc_opc_clear_all_wol_filters);
