...
enum iavf_status iavf_alloc_adminq_asq_ring(struct iavf_hw *hw) {
  ...
  ret_code = iavf_allocate_dma_mem(
      hw, &hw->aq.asq.desc_buf, iavf_mem_atq_ring,
      (hw->aq.num_asq_entries * sizeof(struct iavf_aq_desc)),
      IAVF_ADMINQ_DESC_ALIGNMENT);
  if (ret_code)
    return ret_code;

  ret_code = iavf_allocate_virt_mem(
...
enum iavf_status iavf_alloc_adminq_arq_ring(struct iavf_hw *hw) {
  ...
  ret_code = iavf_allocate_dma_mem(
      hw, &hw->aq.arq.desc_buf, iavf_mem_arq_ring,
...
STATIC enum iavf_status iavf_alloc_arq_bufs(struct iavf_hw *hw) {
  ...
  for (i = 0; i < hw->aq.num_arq_entries; i++) {
    ...
    ret_code =
        iavf_allocate_dma_mem(hw, bi, iavf_mem_arq_buf, hw->aq.arq_buf_size,
                              IAVF_ADMINQ_DESC_ALIGNMENT);
    if (ret_code)
...
STATIC enum iavf_status iavf_alloc_asq_bufs(struct iavf_hw *hw) {
  ...
  for (i = 0; i < hw->aq.num_asq_entries; i++) {
    ...
    ret_code =
        iavf_allocate_dma_mem(hw, bi, iavf_mem_asq_buf, hw->aq.asq_buf_size,
                              IAVF_ADMINQ_DESC_ALIGNMENT);
    if (ret_code)
...
u16 iavf_clean_asq(struct iavf_hw *hw) {
  ...
  while (rd32(hw, hw->aq.asq.head) != ntc) {
    ...
    if (details->callback) {
      IAVF_ADMINQ_CALLBACK cb_func = (IAVF_ADMINQ_CALLBACK)details->callback;
      iavf_memcpy(&desc_cb, desc, sizeof(struct iavf_aq_desc), IAVF_DMA_TO_DMA);
      cb_func(hw, &desc_cb);
    }
...
iavf_asq_send_command(struct iavf_hw *hw, struct iavf_aq_desc *desc,
  ...
  if (cmd_details) {
    iavf_memcpy(details, cmd_details, sizeof(struct iavf_asq_cmd_details),
     ...
     */
    if (details->cookie) {
      desc->cookie_high = CPU_TO_LE32(IAVF_HI_DWORD(details->cookie));
  ...
  if (buff_size > hw->aq.asq_buf_size) {
    iavf_debug(hw, IAVF_DEBUG_AQ_MESSAGE, "AQTX: Invalid buffer size: %d.\n",
               buff_size);
  ...
  if (iavf_clean_asq(hw) == 0) {
    iavf_debug(hw, IAVF_DEBUG_AQ_MESSAGE, "AQTX: Error queue is full.\n");
  ...
  if (iavf_asq_done(hw)) {
    iavf_memcpy(desc, desc_on_ring, sizeof(struct iavf_aq_desc),
                IAVF_DMA_TO_NONDMA);
    if (buff != NULL)
      iavf_memcpy(buff, dma_buff->va, buff_size, IAVF_DMA_TO_NONDMA);
  ...
  iavf_debug(hw, IAVF_DEBUG_AQ_MESSAGE, "AQTX: desc and buffer writeback:\n");
  iavf_debug_aq(hw, IAVF_DEBUG_AQ_COMMAND, (void *)desc, buff, buff_size);

  /* save writeback aq if requested */
  if (details->wb_desc)
    iavf_memcpy(details->wb_desc, desc_on_ring, sizeof(struct iavf_aq_desc),
                IAVF_DMA_TO_NONDMA);
...
void iavf_fill_default_direct_cmd_desc(struct iavf_aq_desc *desc, u16 opcode) {
  /* zero out the desc */
  iavf_memset((void *)desc, 0, sizeof(struct iavf_aq_desc), IAVF_NONDMA_MEM);
...
enum iavf_status iavf_clean_arq_element(struct iavf_hw *hw,
  ...
  hw->aq.arq_last_status = (enum iavf_admin_queue_err)LE16_TO_CPU(desc->retval);
  ...
  iavf_memcpy(&e->desc, desc, sizeof(struct iavf_aq_desc), IAVF_DMA_TO_NONDMA);
  datalen = LE16_TO_CPU(desc->datalen);
  e->msg_len = min(datalen, e->buf_len);
  if (e->msg_buf != NULL && (e->msg_len != 0))
    iavf_memcpy(e->msg_buf, hw->aq.arq.r.arq_bi[desc_idx].va, e->msg_len,
                IAVF_DMA_TO_NONDMA);
