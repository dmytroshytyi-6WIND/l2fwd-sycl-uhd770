...
static inline void print_ring_reg_debug_info(void *mmio_base, uint32_t offset) {
  ...
  rte_bbdev_log_debug(
      ...
      fpga_reg_read_64(mmio_base, offset + FPGA_5GNR_FEC_RING_HEAD_ADDR));
  rte_bbdev_log_debug(
      "RING_SIZE = 0x%04" PRIx16,
      fpga_reg_read_16(mmio_base, offset + FPGA_5GNR_FEC_RING_SIZE));
  rte_bbdev_log_debug(
      "RING_MISC = 0x%02" PRIx8,
      fpga_reg_read_8(mmio_base, offset + FPGA_5GNR_FEC_RING_MISC));
  rte_bbdev_log_debug(
      "RING_ENABLE = 0x%02" PRIx8,
      fpga_reg_read_8(mmio_base, offset + FPGA_5GNR_FEC_RING_ENABLE));
  rte_bbdev_log_debug(
      "RING_FLUSH_QUEUE_EN = 0x%02" PRIx8,
      fpga_reg_read_8(mmio_base, offset + FPGA_5GNR_FEC_RING_FLUSH_QUEUE_EN));
  rte_bbdev_log_debug(
      "RING_SHADOW_TAIL = 0x%04" PRIx16,
      fpga_reg_read_16(mmio_base, offset + FPGA_5GNR_FEC_RING_SHADOW_TAIL));
  rte_bbdev_log_debug(
      "RING_HEAD_POINT = 0x%04" PRIx16,
      fpga_reg_read_16(mmio_base, offset + FPGA_5GNR_FEC_RING_HEAD_POINT));
}

/* Read Static Register of FPGA 5GNR FEC device */
static inline void print_static_reg_debug_info(void *mmio_base) {
  uint16_t config = fpga_reg_read_16(mmio_base, FPGA_5GNR_FEC_CONFIGURATION);
  uint8_t qmap_done =
      fpga_reg_read_8(mmio_base, FPGA_5GNR_FEC_QUEUE_PF_VF_MAP_DONE);
  uint16_t lb_factor =
      fpga_reg_read_16(mmio_base, FPGA_5GNR_FEC_LOAD_BALANCE_FACTOR);
  uint16_t ring_desc_len =
      fpga_reg_read_16(mmio_base, FPGA_5GNR_FEC_RING_DESC_LEN);
  uint16_t flr_time_out =
      fpga_reg_read_16(mmio_base, FPGA_5GNR_FEC_FLR_TIME_OUT);
...
static void print_dma_dec_desc_debug_info(union fpga_dma_desc *desc) {
  rte_bbdev_log_debug(
      ...
      (uint32_t)desc->dec_req.et_pass, (uint32_t)desc->dec_req.crcb_pass,
      (uint32_t)desc->dec_req.error, (uint32_t)desc->dec_req.qm_idx,
      (uint32_t)desc->dec_req.max_iter, (uint32_t)desc->dec_req.bg_idx,
      (uint32_t)desc->dec_req.harqin_en, (uint32_t)desc->dec_req.zc,
      (uint32_t)desc->dec_req.hbstroe_offset, (uint32_t)desc->dec_req.num_null,
      (uint32_t)desc->dec_req.irq_en, (uint32_t)desc->dec_req.ncb,
      (uint32_t)desc->dec_req.desc_idx, (uint32_t)desc->dec_req.drop_crc24b,
      (uint32_t)desc->dec_req.rv, (uint32_t)desc->dec_req.crc24b_ind,
      (uint32_t)desc->dec_req.et_dis, (uint32_t)desc->dec_req.harq_input_length,
      (uint32_t)desc->dec_req.rm_e, (uint32_t)desc->dec_req.cbs_in_op,
...
static void print_dma_enc_desc_debug_info(union fpga_dma_desc *desc) {
  rte_bbdev_log_debug(
      "DMA response desc %p\n"
      "%" PRIu32 " %" PRIu32 "\n"
      ...
      desc, (uint32_t)desc->enc_req.done, (uint32_t)desc->enc_req.error,

      (uint32_t)desc->enc_req.k_, (uint32_t)desc->enc_req.rm_e,
      (uint32_t)desc->enc_req.desc_idx, (uint32_t)desc->enc_req.zc,

      (uint32_t)desc->enc_req.bg_idx, (uint32_t)desc->enc_req.qm_idx,
      (uint32_t)desc->enc_req.crc_en, (uint32_t)desc->enc_req.irq_en,

      (uint32_t)desc->enc_req.k0, (uint32_t)desc->enc_req.ncb,
...
static void fpga_dev_info_get(struct rte_bbdev *dev,
  ...
  uint32_t harq_buf_ready =
      fpga_reg_read_32(d->mmio_base, FPGA_5GNR_FEC_HARQ_BUF_SIZE_RDY_REGS);
  ...
  uint32_t harq_buf_size =
      fpga_reg_read_32(d->mmio_base, FPGA_5GNR_FEC_HARQ_BUF_SIZE_REGS);
  ...
  for (q_id = 0; q_id < FPGA_TOTAL_NUM_QUEUES; ++q_id) {
    uint32_t hw_q_id =
        fpga_reg_read_32(d->mmio_base, FPGA_5GNR_FEC_QUEUE_MAP + (q_id << 2));
    if (hw_q_id != FPGA_INVALID_HW_QUEUE_ID)
...
static int fpga_queue_release(struct rte_bbdev *dev, uint16_t queue_id) {
  ...
  if (q != NULL) {
    ...
    /* Disable queue */
    fpga_reg_write_8(d->mmio_base, offset + FPGA_5GNR_FEC_RING_ENABLE, 0x00);
...
static int fpga_queue_start(struct rte_bbdev *dev, uint16_t queue_id) {
  ...
  fpga_reg_write_16(d->mmio_base, offset + FPGA_5GNR_FEC_RING_HEAD_POINT, zero);
  fpga_reg_write_16(d->mmio_base, offset + FPGA_5GNR_FEC_RING_SHADOW_TAIL,
                    zero);

  /* Enable queue */
  fpga_reg_write_8(d->mmio_base, offset + FPGA_5GNR_FEC_RING_ENABLE, enable);
...
static int fpga_queue_stop(struct rte_bbdev *dev, uint16_t queue_id) {
  ...
  fpga_reg_write_8(d->mmio_base, offset + FPGA_5GNR_FEC_RING_FLUSH_QUEUE_EN,
                   payload);
  ...
  /* Disable queue */
  payload = 0x00;
  fpga_reg_write_8(d->mmio_base, offset + FPGA_5GNR_FEC_RING_ENABLE, payload);
...
fpga_dma_desc_te_fill(struct rte_bbdev_enc_op *op,
  ...
  desc->irq_en = 0;
  desc->k0 = get_k0(op->ldpc_enc.n_cb, op->ldpc_enc.z_c, op->ldpc_enc.basegraph,
 ...
 *   Number of CBs contained in one operation.
 */
static inline int fpga_dma_desc_ld_fill(
    struct rte_bbdev_dec_op *op, struct fpga_dma_dec_desc *desc,
    struct rte_mbuf *input, struct rte_mbuf *output, uint16_t harq_in_length,
    uint32_t in_offset, uint32_t out_offset, uint32_t harq_offset,
    uint16_t desc_offset, uint8_t cbs_in_op) {
  /* reset */
  desc->done = 0;
  desc->error = 0;
  ...
  desc->drop_crc24b =
      check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_CRC_TYPE_24B_DROP);
  desc->desc_idx = desc_offset;
...
static int validate_enc_op(struct rte_bbdev_enc_op *op __rte_unused) {
  ...
  if ((ldpc_enc->basegraph > 2) || (ldpc_enc->basegraph == 0)) {
    rte_bbdev_log(ERR, "basegraph (%u) is out of range 1 <= value <= 2",
                  ldpc_enc->basegraph);
    return -1;
  }
  if (ldpc_enc->code_block_mode > RTE_BBDEV_CODE_BLOCK) {
    rte_bbdev_log(ERR, "code_block_mode (%u) is out of range 0:Tb 1:CB",
  ...
  if (ldpc_enc->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
    ...
    if ((tb->ea < RTE_BBDEV_LDPC_MIN_CB_SIZE) && tb->r < tb->cab) {
      rte_bbdev_log(ERR, "ea (%u) is less than %u or it is not even", tb->ea,
                    RTE_BBDEV_LDPC_MIN_CB_SIZE);
      return -1;
    }
    if ((tb->eb < RTE_BBDEV_LDPC_MIN_CB_SIZE) && tb->c > tb->cab) {
      rte_bbdev_log(ERR, "eb (%u) is less than %u", tb->eb,
                    RTE_BBDEV_LDPC_MIN_CB_SIZE);
      return -1;
    }
    if (tb->r > (tb->c - 1)) {
...
static int validate_dec_op(struct rte_bbdev_dec_op *op __rte_unused) {
  ...
  if (ldpc_dec->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
    ...
    if (cb->e < RTE_BBDEV_LDPC_MIN_CB_SIZE) {
      rte_bbdev_log(ERR, "e (%u) is out of range %u <= value <= %u", cb->e,
                    RTE_BBDEV_LDPC_MIN_CB_SIZE, RTE_BBDEV_LDPC_MAX_CB_SIZE);
      return -1;
    }
  }
...
fpga_harq_write_loopback(struct fpga_5gnr_fec_device *fpga_dev,
   ...
   */
  reg_32 =
      fpga_reg_read_32(fpga_dev->mmio_base, FPGA_5GNR_FEC_HARQ_BUF_SIZE_REGS);
  ...
  while (left_length > 0) {
    if (fpga_reg_read_8(fpga_dev->mmio_base,
      ...
      fpga_reg_write_32(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_WR_ADDR_REGS,
                        out_offset);
      fpga_reg_write_64(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_WR_DATA_REGS,
                        input[increment]);
      left_length -= FPGA_5GNR_FEC_DDR_WR_DATA_LEN_IN_BYTES;
      out_offset += FPGA_5GNR_FEC_DDR_WR_DATA_LEN_IN_BYTES;
      increment++;
      fpga_reg_write_8(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_WR_DONE_REGS, 1);
    }
  }
  while (last_transaction > 0) {
    if (fpga_reg_read_8(fpga_dev->mmio_base,
      ...
      last_word &= (uint64_t)(1 << (last_transaction * 4)) - 1;
      fpga_reg_write_64(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_WR_DATA_REGS,
                        last_word);
      fpga_reg_write_8(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_WR_DONE_REGS, 1);
      last_transaction = 0;
...
static inline int fpga_harq_read_loopback(struct fpga_5gnr_fec_device *fpga_dev,
                                          struct rte_mbuf *harq_output,
  ...
  uint64_t *input = NULL;
  uint32_t last_transaction =
      harq_in_length % FPGA_5GNR_FEC_DDR_WR_DATA_LEN_IN_BYTES;

  if (last_transaction > 0)
    harq_in_length += (8 - last_transaction);

  reg = fpga_reg_read_32(fpga_dev->mmio_base, FPGA_5GNR_FEC_HARQ_BUF_SIZE_REGS);
  ...
  while (left_length > 0) {
    fpga_reg_write_32(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_RD_ADDR_REGS,
                      in_offset);
    fpga_reg_write_8(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_RD_DONE_REGS, 1);
    reg = fpga_reg_read_8(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_RD_RDY_REGS);
    while (reg != 1) {
      reg =
          fpga_reg_read_8(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_RD_RDY_REGS);
    ...
    input[increment] =
        fpga_reg_read_64(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_RD_DATA_REGS);
    left_length -= FPGA_5GNR_FEC_DDR_RD_DATA_LEN_IN_BYTES;
    in_offset += FPGA_5GNR_FEC_DDR_WR_DATA_LEN_IN_BYTES;
    increment++;
    fpga_reg_write_8(fpga_dev->mmio_base, FPGA_5GNR_FEC_DDR4_RD_DONE_REGS, 0);
  }
  return 1;
}

static inline int enqueue_ldpc_enc_one_op_cb(struct fpga_queue *q,
  ...
  if (total_left != in_length) {
    op->status |= 1 << RTE_BBDEV_DATA_ERROR;
    rte_bbdev_log(ERR, "Mismatch between mbuf length and included CBs sizes %d",
                  total_left);
  }
  ...
  ret = fpga_dma_desc_te_fill(op, &desc->enc_req, m_in, m_out, k_, e, in_offset,
                              out_offset, ring_offset, c);
  if (unlikely(ret < 0))
    return ret;
...
static inline int enqueue_ldpc_dec_one_op_cb(struct fpga_queue *q,
  ...
  if (check_bit(dec->op_flags, RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_LOOPBACK)) {
    ...
    if (check_bit(dec->op_flags,
                  RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_OUT_ENABLE)) {
      ret = fpga_harq_write_loopback(q->d, harq_in, harq_in_length,
                                     harq_in_offset, harq_out_offset);
    } else if (check_bit(dec->op_flags,
                         RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_IN_ENABLE)) {
      ret = fpga_harq_read_loopback(q->d, harq_out, harq_in_length,
                                    harq_in_offset, harq_out_offset);
  ...
  if (check_bit(dec->op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_IN_ENABLE)) {
    harq_in_length =
        RTE_MIN(dec->harq_combined_input.length, (uint32_t)dec->n_cb);
  }
  ...
  if (check_bit(dec->op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_OUT_ENABLE)) {
    ...
    if (k0 > parity_offset)
      ...
      l = k0 + e + dec->n_filler;
    harq_out_length =
        RTE_MIN(RTE_MAX(harq_in_length, l), dec->n_cb - dec->n_filler);
  ...
  ret =
      fpga_dma_desc_ld_fill(op, &desc->dec_req, m_in, m_out, harq_in_length,
                            in_offset, out_offset, harq_offset, ring_offset, c);
  if (unlikely(ret < 0))
    return ret;
  /* Update lengths */
...
static uint16_t fpga_enqueue_ldpc_enc(struct rte_bbdev_queue_data *q_data,
...
static uint16_t fpga_enqueue_ldpc_dec(struct rte_bbdev_queue_data *q_data,
  ...
  for (i = 0; i < num; ++i) {
    ...
    if (unlikely(avail - 1 < 0))
      break;
    avail -= 1;
    enqueued_cbs = enqueue_ldpc_dec_one_op_cb(q, ops[i], total_enqueued_cbs);
...
static inline int dequeue_ldpc_enc_one_op_cb(struct fpga_queue *q,
                                             struct rte_bbdev_enc_op **op,
                                             uint16_t desc_offset) {
  union fpga_dma_desc *desc;
  int desc_error;
...
static inline int dequeue_ldpc_dec_one_op_cb(struct fpga_queue *q,
                                             struct rte_bbdev_dec_op **op,
                                             uint16_t desc_offset) {
  union fpga_dma_desc *desc;
  int desc_error;
...
static uint16_t fpga_dequeue_ldpc_enc(struct rte_bbdev_queue_data *q_data,
                                      struct rte_bbdev_enc_op **ops,
                                      uint16_t num) {
  struct fpga_queue *q = q_data->queue_private;
...
static uint16_t fpga_dequeue_ldpc_dec(struct rte_bbdev_queue_data *q_data,
                                      struct rte_bbdev_dec_op **ops,
                                      uint16_t num) {
  struct fpga_queue *q = q_data->queue_private;
  uint32_t avail = (q->tail - q->head_free_desc) & q->sw_ring_wrap_mask;
...
static void fpga_5gnr_fec_init(struct rte_bbdev *dev,
...
static int fpga_5gnr_fec_probe(struct rte_pci_driver *pci_drv,
  ...
  bbdev->data->dev_private =
      rte_zmalloc_socket(dev_name, sizeof(struct fpga_5gnr_fec_device),
  ...
  uint32_t version_id =
      fpga_reg_read_32(d->mmio_base, FPGA_5GNR_FEC_VERSION_ID);
  rte_bbdev_log(INFO, "FEC FPGA RTL v%u.%u", ((uint16_t)(version_id >> 16)),
                ((uint16_t)version_id));

#ifdef RTE_LIBRTE_BBDEV_DEBUG
  if (!strcmp(pci_drv->driver.name, RTE_STR(FPGA_5GNR_FEC_PF_DRIVER_NAME)))
    print_static_reg_debug_info(d->mmio_base);
#endif
  return 0;
}

static int fpga_5gnr_fec_remove(struct rte_pci_device *pci_dev) {
...
int rte_fpga_5gnr_fec_configure(const char *dev_name,
  ...
  if (conf->pf_mode_en) {
    ...
    for (vf_id = 0; vf_id < FPGA_5GNR_FEC_NUM_VFS; ++vf_id) {
      for (q_id = 0; q_id < conf->vf_ul_queues_number[vf_id];
           ++q_id, ++total_ul_q_id) {
        address = (total_ul_q_id << 2) + FPGA_5GNR_FEC_QUEUE_MAP;
...
static struct rte_pci_id pci_id_fpga_5gnr_fec_pf_map[] = {
    {RTE_PCI_DEVICE(FPGA_5GNR_FEC_VENDOR_ID, FPGA_5GNR_FEC_PF_DEVICE_ID)},
    {.device_id = 0},
};
...
static struct rte_pci_id pci_id_fpga_5gnr_fec_vf_map[] = {
    {RTE_PCI_DEVICE(FPGA_5GNR_FEC_VENDOR_ID, FPGA_5GNR_FEC_VF_DEVICE_ID)},
    {.device_id = 0},
};
