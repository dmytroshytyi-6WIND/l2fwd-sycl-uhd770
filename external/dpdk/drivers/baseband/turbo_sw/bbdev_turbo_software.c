...
#define rte_bbdev_log_debug(fmt, ...)                                          \
  rte_bbdev_log(DEBUG, RTE_STR(__LINE__) ":%s() " fmt, __func__, ##__VA_ARGS__)
...
static const char *const turbo_sw_valid_params[] = {TURBO_SW_MAX_NB_QUEUES_ARG,
...
static inline int32_t compute_idx(uint16_t k) {
  int32_t result = 0;
...
/* Get device info */
static void info_get(struct rte_bbdev *dev,
  ...
  static const struct rte_bbdev_op_cap bbdev_capabilities[] = {
      ...
      {.type = RTE_BBDEV_OP_LDPC_DEC,
       .cap.ldpc_dec =
           {
               .capability_flags = RTE_BBDEV_LDPC_CRC_TYPE_16_CHECK |
...
/* Release queue */
static int q_release(struct rte_bbdev *dev, uint16_t q_id) {
  ...
  rte_bbdev_log_debug("released device queue %u:%u", dev->data->dev_id, q_id);
  return 0;
}

/* Setup a queue */
static int q_setup(struct rte_bbdev *dev, uint16_t q_id,
  ...
  q = rte_zmalloc_socket(RTE_STR(DRIVER_NAME), sizeof(*q), RTE_CACHE_LINE_SIZE,
                         queue_conf->socket);
  if (q == NULL) {
  ...
  if ((ret < 0) || (ret >= (int)RTE_RING_NAMESIZE)) {
    rte_bbdev_log(ERR, "Creating queue name for device %u queue %u failed",
                  dev->data->dev_id, q_id);
    ret = -ENAMETOOLONG;
    goto free_q;
  }
  q->enc_out = rte_zmalloc_socket(
      name, ((RTE_BBDEV_TURBO_MAX_TB_SIZE >> 3) + 3) * sizeof(*q->enc_out) * 3,
      RTE_CACHE_LINE_SIZE, queue_conf->socket);
  if (q->enc_out == NULL) {
    rte_bbdev_log(ERR, "Failed to allocate queue memory for %s", name);
    ret = -ENOMEM;
    goto free_q;
  }

  /* Allocate memory for rate matching output. */
  ret = snprintf(name, RTE_RING_NAMESIZE, RTE_STR(DRIVER_NAME) "_enc_i%u:%u",
                 dev->data->dev_id, q_id);
  if ((ret < 0) || (ret >= (int)RTE_RING_NAMESIZE)) {
    rte_bbdev_log(ERR, "Creating queue name for device %u queue %u failed",
                  dev->data->dev_id, q_id);
  ...
  q->enc_in = rte_zmalloc_socket(
      name, (RTE_BBDEV_LDPC_MAX_CB_SIZE >> 3) * sizeof(*q->enc_in),
      RTE_CACHE_LINE_SIZE, queue_conf->socket);
  if (q->enc_in == NULL) {
    rte_bbdev_log(ERR, "Failed to allocate queue memory for %s", name);
    ret = -ENOMEM;
  ...
  q->code_block = rte_zmalloc_socket(
      name, RTE_BBDEV_TURBO_MAX_CB_SIZE * sizeof(*q->code_block),
      RTE_CACHE_LINE_SIZE, queue_conf->socket);
  ...
  ret = snprintf(name, RTE_RING_NAMESIZE, RTE_STR(DRIVER_NAME) "_de_i%u:%u",
                 dev->data->dev_id, q_id);
  if ((ret < 0) || (ret >= (int)RTE_RING_NAMESIZE)) {
    rte_bbdev_log(ERR, "Creating queue name for device %u queue %u failed",
                  dev->data->dev_id, q_id);
  ...
  q->deint_input =
      rte_zmalloc_socket(name, DEINT_INPUT_BUF_SIZE * sizeof(*q->deint_input),
                         RTE_CACHE_LINE_SIZE, queue_conf->socket);
  ...
  ret = snprintf(name, RTE_RING_NAMESIZE, RTE_STR(DRIVER_NAME) "_de_o%u:%u",
                 dev->data->dev_id, q_id);
  if ((ret < 0) || (ret >= (int)RTE_RING_NAMESIZE)) {
    rte_bbdev_log(ERR, "Creating queue name for device %u queue %u failed",
                  dev->data->dev_id, q_id);
  ...
  q->deint_output =
      rte_zmalloc_socket(NULL, DEINT_OUTPUT_BUF_SIZE * sizeof(*q->deint_output),
                         RTE_CACHE_LINE_SIZE, queue_conf->socket);
  ...
  ret = snprintf(name, RTE_RING_NAMESIZE, RTE_STR(DRIVER_NAME) "_ada_o%u:%u",
                 dev->data->dev_id, q_id);
  if ((ret < 0) || (ret >= (int)RTE_RING_NAMESIZE)) {
    rte_bbdev_log(ERR, "Creating queue name for device %u queue %u failed",
                  dev->data->dev_id, q_id);
  ...
  q->adapter_output = rte_zmalloc_socket(
      NULL, ADAPTER_OUTPUT_BUF_SIZE * sizeof(*q->adapter_output),
      RTE_CACHE_LINE_SIZE, queue_conf->socket);
  ...
  q->processed_pkts =
      rte_ring_create(name, queue_conf->queue_size, queue_conf->socket,
...
static const struct rte_bbdev_ops pmd_ops = {
    .info_get = info_get, .queue_setup = q_setup, .queue_release = q_release};
 ...
 */
static inline int is_enc_input_valid(const uint16_t k, const int32_t k_idx,
  ...
  if (k > RTE_BBDEV_TURBO_MAX_CB_SIZE) {
    rte_bbdev_log(ERR, "CB size (%u) is too big, max: %d", k,
 ...
 * Returns 0 if it's valid, -1 otherwise.
 */
static inline int is_dec_input_valid(int32_t k_idx, int16_t kw,
                                     int16_t in_length) {
  if (k_idx < 0) {
    rte_bbdev_log(ERR, "K index is invalid");
    return -1;
  ...
  if (in_length < kw) {
    rte_bbdev_log(ERR, "Mismatch between input length (%u) and kw (%u)",
                  in_length, kw);
    return -1;
  }

  if (kw > RTE_BBDEV_TURBO_MAX_KW) {
    rte_bbdev_log(ERR, "Input length (%u) is too big, max: %d", kw,
...
static inline void
process_enc_cb(struct turbo_sw_queue *q, struct rte_bbdev_enc_op *op, uint8_t r,
               uint8_t c, uint16_t k, uint16_t ncb, uint32_t e,
               struct rte_mbuf *m_in, struct rte_mbuf *m_out_head,
  ...
  if (enc->op_flags & RTE_BBDEV_TURBO_RATE_MATCH) {
    ...
    out0 = (uint8_t *)mbuf_append(m_out_head, m_out, (k >> 3) * 3 + 2);
    if (out0 == NULL) {
      op->status |= 1 << RTE_BBDEV_DATA_ERROR;
    ...
    out1 = rte_pktmbuf_mtod_offset(m_out, uint8_t *, out_offset + (k >> 3) + 1);
...
process_ldpc_enc_cb(struct turbo_sw_queue *q, struct rte_bbdev_enc_op *op,
                    uint32_t e, struct rte_mbuf *m_in,
  ...
  ippsCopyBE_1u(((uint8_t *)ldpc_req.input[0]) + (puntBits / 8), puntBits % 8,
                q->adapter_output, 0, parity_offset);
...
static inline void enqueue_enc_one_op(struct turbo_sw_queue *q,
  ...
  if (mbuf_total_left > RTE_BBDEV_TURBO_MAX_TB_SIZE >> 3) {
    rte_bbdev_log(ERR, "TB size (%u) is too big, max: %d", mbuf_total_left,
  ...
  while (mbuf_total_left > 0 && r < c) {
    ...
    if (enc->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
      k = (r < enc->tb_params.c_neg) ? enc->tb_params.k_neg
                                     : enc->tb_params.k_pos;
      ncb = (r < enc->tb_params.c_neg) ? enc->tb_params.ncb_neg
                                       : enc->tb_params.ncb_pos;
      e = (r < enc->tb_params.cab) ? enc->tb_params.ea : enc->tb_params.eb;
    } else {
      k = enc->cb_params.k;
    ...
    process_enc_cb(q, op, r, c, k, ncb, e, m_in, m_out_head, m_out, in_offset,
                   out_offset, seg_total_left, queue_stats);
    /* Update total_left */
  ...
  if (mbuf_total_left != 0) {
    op->status |= 1 << RTE_BBDEV_DATA_ERROR;
    rte_bbdev_log(ERR, "Mismatch between mbuf length and included CBs sizes");
  }
}
...
enqueue_ldpc_enc_one_op(struct turbo_sw_queue *q, struct rte_bbdev_enc_op *op,
  ...
  while (mbuf_total_left > 0 && r < c) {
    ...
    process_ldpc_enc_cb(q, op, e, m_in, m_out_head, m_out, in_offset,
                        out_offset, seg_total_left, queue_stats);
    /* Update total_left */
...
enqueue_enc_all_ops(struct turbo_sw_queue *q, struct rte_bbdev_enc_op **ops,
  ...
  return rte_ring_enqueue_burst(q->processed_pkts, (void **)ops, nb_ops, NULL);
}
...
#ifdef RTE_BBDEV_SDK_AVX2
static inline void move_padding_bytes(const uint8_t *in, uint8_t *out,
                                      uint16_t k, uint16_t ncb) {
  uint16_t d = k + 4;
  uint16_t kpi = ncb / 3;
  uint16_t nd = kpi - d;
...
static inline void
process_dec_cb(struct turbo_sw_queue *q, struct rte_bbdev_dec_op *op, uint8_t c,
               uint16_t k, uint16_t kw, struct rte_mbuf *m_in,
               struct rte_mbuf *m_out_head, struct rte_mbuf *m_out,
  ...
  out = (uint8_t *)mbuf_append(m_out_head, m_out, ((k - crc24_overlap) >> 3));
  if (out == NULL) {
    op->status |= 1 << RTE_BBDEV_DATA_ERROR;
    rte_bbdev_log(ERR, "Too little space in output mbuf");
    return;
  ...
  turbo_req.early_term_disable =
      !check_bit(dec->op_flags, RTE_BBDEV_TURBO_EARLY_TERMINATION);
...
static inline void process_ldpc_dec_cb(
    struct turbo_sw_queue *q, struct rte_bbdev_dec_op *op, uint8_t c,
    uint16_t out_length, uint32_t e, struct rte_mbuf *m_in,
    struct rte_mbuf *m_out_head, struct rte_mbuf *m_out,
    struct rte_mbuf *m_harq_in, struct rte_mbuf *m_harq_out_head,
    struct rte_mbuf *m_harq_out, uint16_t in_offset, uint16_t out_offset,
    uint16_t harq_in_offset, uint16_t harq_out_offset, bool check_crc_24b,
    uint16_t crc24_overlap, uint16_t in_length,
    struct rte_bbdev_stats *q_stats) {
#ifdef RTE_BBDEV_SDK_AVX512
  RTE_SET_USED(in_length);
  ...
  if (check_bit(dec->op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_IN_ENABLE)) {
    ...
    harq_in = NULL;
    if (m_harq_in != NULL)
      harq_in = rte_pktmbuf_mtod_offset(m_harq_in, uint8_t *, harq_in_offset);
    if (harq_in == NULL) {
      op->status |= 1 << RTE_BBDEV_DATA_ERROR;
    ...
    memset(q->ag + harq_in_length, 0, dec->n_cb - harq_in_length);
  ...
  derm_req.isretx =
      check_bit(dec->op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_IN_ENABLE);
  ...
  deRmOutSize = RTE_MIN(
      derm_req.k0 + derm_req.e -
          ((derm_req.k0 < derm_req.start_null_index) ? 0 : dec->n_filler),
      dec->n_cb - dec->n_filler);
  if (m_harq_in != NULL)
    deRmOutSize = RTE_MAX(
        deRmOutSize, RTE_MIN(dec->n_cb - dec->n_filler, m_harq_in->data_len));
  numRows =
      ((deRmOutSize + dec->n_filler + dec->z_c - 1) / dec->z_c) - sys_cols + 2;
  ...
  if (out == NULL) {
    op->status |= 1 << RTE_BBDEV_DATA_ERROR;
    rte_bbdev_log(ERR, "Too little space in LDPC decoder output mbuf");
    return;
  }
  ...
  dec_req.enableEarlyTermination =
      check_bit(dec->op_flags, RTE_BBDEV_LDPC_ITERATION_STOP_ENABLE);
  ...
  dec->iter_count = RTE_MAX(dec_resp.iterationAtTermination, dec->iter_count);
  ...
  if (check_bit(dec->op_flags, RTE_BBDEV_LDPC_CRC_TYPE_24A_CHECK) ||
      check_bit(dec->op_flags, RTE_BBDEV_LDPC_CRC_TYPE_24B_CHECK)) {
  ...
  if (check_bit(dec->op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_OUT_ENABLE)) {
    ...
    if (m_harq_out != NULL) {
       ...
       */
      harq_out =
          (uint8_t *)mbuf_append(m_harq_out_head, m_harq_out, deRmOutSize);
    }
    if (harq_out == NULL) {
      op->status |= 1 << RTE_BBDEV_DATA_ERROR;
    ...
    harq_out = rte_pktmbuf_mtod_offset(m_harq_out, uint8_t *, harq_out_offset);
...
static inline void enqueue_dec_one_op(struct turbo_sw_queue *q,
  ...
  if ((c > 1) && !check_bit(dec->op_flags, RTE_BBDEV_TURBO_DEC_TB_CRC_24B_KEEP))
    crc24_overlap = 24;

  while (mbuf_total_left > 0) {
    if (dec->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK)
      k = (r < dec->tb_params.c_neg) ? dec->tb_params.k_neg
                                     : dec->tb_params.k_pos;
    ...
    process_dec_cb(q, op, c, k, kw, m_in, m_out_head, m_out, in_offset,
                   out_offset,
                   check_bit(dec->op_flags, RTE_BBDEV_TURBO_CRC_TYPE_24B),
                   crc24_overlap, seg_total_left, queue_stats);
...
enqueue_ldpc_dec_one_op(struct turbo_sw_queue *q, struct rte_bbdev_dec_op *op,
  ...
  while (mbuf_total_left > 0) {
    if (dec->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK)
      e = (r < dec->tb_params.cab) ? dec->tb_params.ea : dec->tb_params.eb;
    ...
    process_ldpc_dec_cb(
        q, op, c, out_length, e, m_in, m_out_head, m_out, m_harq_in,
        m_harq_out_head, m_harq_out, in_offset, out_offset, harq_in_offset,
        harq_out_offset,
        check_bit(dec->op_flags, RTE_BBDEV_LDPC_CRC_TYPE_24B_CHECK),
        crc24_overlap, seg_total_left, queue_stats);
...
/* Enqueue burst */
static uint16_t enqueue_enc_ops(struct rte_bbdev_queue_data *q_data,
...
/* Enqueue burst */
static uint16_t enqueue_ldpc_enc_ops(struct rte_bbdev_queue_data *q_data,
                                     struct rte_bbdev_enc_op **ops,
                                     uint16_t nb_ops) {
  void *queue = q_data->queue_private;
  struct turbo_sw_queue *q = queue;
  ...
  nb_enqueued = enqueue_ldpc_enc_all_ops(q, ops, nb_ops, &q_data->queue_stats);
...
/* Enqueue burst */
static uint16_t enqueue_dec_ops(struct rte_bbdev_queue_data *q_data,
...
/* Enqueue burst */
static uint16_t enqueue_ldpc_dec_ops(struct rte_bbdev_queue_data *q_data,
                                     struct rte_bbdev_dec_op **ops,
                                     uint16_t nb_ops) {
  void *queue = q_data->queue_private;
  struct turbo_sw_queue *q = queue;
  ...
  nb_enqueued = enqueue_ldpc_dec_all_ops(q, ops, nb_ops, &q_data->queue_stats);
...
static uint16_t dequeue_dec_ops(struct rte_bbdev_queue_data *q_data,
                                struct rte_bbdev_dec_op **ops,
                                uint16_t nb_ops) {
  struct turbo_sw_queue *q = q_data->queue_private;
  uint16_t nb_dequeued =
      rte_ring_dequeue_burst(q->processed_pkts, (void **)ops, nb_ops, NULL);
...
static uint16_t dequeue_enc_ops(struct rte_bbdev_queue_data *q_data,
                                struct rte_bbdev_enc_op **ops,
                                uint16_t nb_ops) {
  struct turbo_sw_queue *q = q_data->queue_private;
  uint16_t nb_dequeued =
...
static inline int parse_u16_arg(const char *key, const char *value,
                                void *extra_args) {
...
static int parse_turbo_sw_params(struct turbo_sw_params *params,
  ...
  if (input_args) {
    ...
    ret = rte_kvargs_process(kvlist, turbo_sw_valid_params[0], &parse_u16_arg,
                             &params->queues_num);
    if (ret < 0)
      goto exit;

    ret = rte_kvargs_process(kvlist, turbo_sw_valid_params[1], &parse_u16_arg,
                             &params->socket_id);
    if (ret < 0)
      goto exit;

    if (params->socket_id >= RTE_MAX_NUMA_NODES) {
      rte_bbdev_log(ERR, "Invalid socket, must be < %u", RTE_MAX_NUMA_NODES);
      goto exit;
    }
  }
...
/* Create device */
static int turbo_sw_bbdev_create(struct rte_vdev_device *vdev,
  ...
  bbdev->data->dev_private =
      rte_zmalloc_socket(name, sizeof(struct bbdev_private),
...
/* Initialise device */
static int turbo_sw_bbdev_probe(struct rte_vdev_device *vdev) {
  struct turbo_sw_params init_params = {rte_socket_id(),
...
/* Uninitialise device */
static int turbo_sw_bbdev_remove(struct rte_vdev_device *vdev) {
  struct rte_bbdev *bbdev;
  const char *name;
...
static struct rte_vdev_driver bbdev_turbo_sw_pmd_drv = {
    .probe = turbo_sw_bbdev_probe, .remove = turbo_sw_bbdev_remove};

RTE_PMD_REGISTER_VDEV(DRIVER_NAME, bbdev_turbo_sw_pmd_drv);
RTE_PMD_REGISTER_PARAM_STRING(DRIVER_NAME, TURBO_SW_MAX_NB_QUEUES_ARG
                              "=<int> " TURBO_SW_SOCKET_ID_ARG "=<int>");
