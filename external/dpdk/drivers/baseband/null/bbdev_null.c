...
static const char *const bbdev_null_valid_params[] = {
    BBDEV_NULL_MAX_NB_QUEUES_ARG, BBDEV_NULL_SOCKET_ID_ARG};
...
static int q_setup(struct rte_bbdev *dev, uint16_t q_id,
  ...
  q->processed_pkts =
      rte_ring_create(ring_name, queue_conf->queue_size, queue_conf->socket,
...
static uint16_t enqueue_dec_ops(struct rte_bbdev_queue_data *q_data,
  ...
  uint16_t nb_enqueued =
      rte_ring_enqueue_burst(q->processed_pkts, (void **)ops, nb_ops, NULL);
...
static int parse_bbdev_null_params(struct bbdev_null_params *params,
  ...
  if (input_args) {
    ...
    ret = rte_kvargs_process(kvlist, bbdev_null_valid_params[0], &parse_u16_arg,
                             &params->queues_num);
    if (ret < 0)
      goto exit;

    ret = rte_kvargs_process(kvlist, bbdev_null_valid_params[1], &parse_u16_arg,
                             &params->socket_id);
    if (ret < 0)
      goto exit;
...
/* Create device */
static int null_bbdev_create(struct rte_vdev_device *vdev,
...
/* Initialise device */
static int null_bbdev_probe(struct rte_vdev_device *vdev) {
  struct bbdev_null_params init_params = {rte_socket_id(),
  ...
  rte_bbdev_log_debug("Init %s on NUMA node %d with max queues: %d", name,
                      init_params.socket_id, init_params.queues_num);

  return null_bbdev_create(vdev, &init_params);
}

/* Uninitialise device */
static int null_bbdev_remove(struct rte_vdev_device *vdev) {
  struct rte_bbdev *bbdev;
  const char *name;
...
static struct rte_vdev_driver bbdev_null_pmd_drv = {
    .probe = null_bbdev_probe, .remove = null_bbdev_remove};

RTE_PMD_REGISTER_VDEV(DRIVER_NAME, bbdev_null_pmd_drv);
RTE_PMD_REGISTER_PARAM_STRING(DRIVER_NAME, BBDEV_NULL_MAX_NB_QUEUES_ARG
                              "=<int> " BBDEV_NULL_SOCKET_ID_ARG "=<int>");
