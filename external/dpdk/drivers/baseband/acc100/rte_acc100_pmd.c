...
static inline void mmio_write(void *addr, uint32_t value) {
  *((volatile uint32_t *)(addr)) = rte_cpu_to_le_32(value);
}

/* Write a register of a ACC100 device */
static inline void acc100_reg_write(struct acc100_device *d, uint32_t offset,
                                    uint32_t value) {
...
static inline uint32_t acc100_reg_read(struct acc100_device *d,
                                       uint32_t offset) {
...
static inline uint32_t log2_basic(uint32_t value) {
  return (value == 0) ? 0 : rte_bsf32(value);
}

/* Calculate memory alignment offset assuming alignment is 2^N */
static inline uint32_t calc_mem_alignment_offset(void *unaligned_virt_mem,
                                                 uint32_t alignment) {
  rte_iova_t unaligned_phy_mem = rte_malloc_virt2iova(unaligned_virt_mem);
  return (uint32_t)(alignment - (unaligned_phy_mem & (alignment - 1)));
}

/* Calculate the offset of the enqueue register */
static inline uint32_t queue_offset(bool pf_device, uint8_t vf_id,
                                    uint8_t qgrp_id, uint16_t aq_id) {
  if (pf_device)
    return ((vf_id << 12) + (qgrp_id << 7) + (aq_id << 3) + HWPfQmgrIngressAq);
  else
    return ((qgrp_id << 7) + (aq_id << 3) + HWVfQmgrIngressAq);
}

enum { UL_4G = 0, UL_5G, DL_4G, DL_5G, NUM_ACC };

/* Return the accelerator enum for a Queue Group Index */
static inline int accFromQgid(int qg_idx,
                              const struct rte_acc100_conf *acc100_conf) {
...
static inline void qtopFromAcc(struct rte_acc100_queue_topology **qtop,
                               int acc_enum,
...
static inline int aqDepth(int qg_idx, struct rte_acc100_conf *acc100_conf) {
...
/* Return the AQ depth for a Queue Group Index */
static inline int aqNum(int qg_idx, struct rte_acc100_conf *acc100_conf) {
  struct rte_acc100_queue_topology *q_top = NULL;
  int acc_enum = accFromQgid(qg_idx, acc100_conf);
  qtopFromAcc(&q_top, acc_enum, acc100_conf);
  if (unlikely(q_top == NULL))
...
static void initQTop(struct rte_acc100_conf *acc100_conf) {
...
static inline void updateQtop(uint8_t acc, uint8_t qg,
                              struct rte_acc100_conf *acc100_conf,
  ...
  if (q_top->first_qgroup_index == -1) {
    ...
    reg = acc100_reg_read(
        d, queue_offset(d->pf_device, 0, qg, ACC100_NUM_AQS - 1));
    ...
    for (aq = 0; aq < ACC100_NUM_AQS; aq++) {
      reg = acc100_reg_read(d, queue_offset(d->pf_device, 0, qg, aq));
      if (reg & ACC100_QUEUE_ENABLE)
...
static inline void fetch_acc100_config(struct rte_bbdev *dev) {
  ...
  for (qg = 0; qg < ACC100_NUM_QGRPS_PER_WORD; qg++) {
    reg_aq = acc100_reg_read(d, queue_offset(d->pf_device, 0, qg, 0));
  ...
  for (acc = 0; acc < NUM_ACC; acc++) {
    ...
    if (q_top->first_qgroup_index < ACC100_NUM_QGRPS_PER_WORD)
      ...
      q_top->aq_depth_log2 =
          (reg_len1 >>
           ((q_top->first_qgroup_index - ACC100_NUM_QGRPS_PER_WORD) * 4)) &
          0xF;
  }
  ...
  rte_bbdev_log_debug(
      "%s Config LLR SIGN IN/OUT %s %s QG %u %u %u %u AQ %u %u %u %u Len %u %u "
      "%u %u\n",
      (d->pf_device) ? "PF" : "VF",
      (acc100_conf->input_pos_llr_1_bit) ? "POS" : "NEG",
      (acc100_conf->output_pos_llr_1_bit) ? "POS" : "NEG",
      acc100_conf->q_ul_4g.num_qgroups, acc100_conf->q_dl_4g.num_qgroups,
      acc100_conf->q_ul_5g.num_qgroups, acc100_conf->q_dl_5g.num_qgroups,
      ...
      acc100_conf->q_ul_4g.aq_depth_log2, acc100_conf->q_dl_4g.aq_depth_log2,
      acc100_conf->q_ul_5g.aq_depth_log2, acc100_conf->q_dl_5g.aq_depth_log2);
}

static void free_base_addresses(void **base_addrs, int size) {
  int i;
  for (i = 0; i < size; i++)
    rte_free(base_addrs[i]);
}

static inline uint32_t get_desc_len(void) {
  return sizeof(union acc100_dma_desc);
}

/* Allocate the 2 * 64MB block for the sw rings */
static int alloc_2x64mb_sw_rings_mem(struct rte_bbdev *dev,
                                     struct acc100_device *d, int socket) {
  uint32_t sw_ring_size = ACC100_SIZE_64MBYTE;
  d->sw_rings_base = rte_zmalloc_socket(
      dev->device->driver->name, 2 * sw_ring_size, RTE_CACHE_LINE_SIZE, socket);
  ...
  d->sw_rings_iova =
      rte_malloc_virt2iova(d->sw_rings_base) + next_64mb_align_offset;
...
static void alloc_sw_rings_min_mem(struct rte_bbdev *dev,
                                   struct acc100_device *d, uint16_t num_queues,
                                   int socket) {
  ...
  while (i < ACC100_SW_RING_MEM_ALLOC_ATTEMPTS) {
    ...
    sw_rings_base = rte_zmalloc_socket(
        dev->device->driver->name, dev_sw_ring_size, q_sw_ring_size, socket);
    ...
    next_64mb_align_offset =
        calc_mem_alignment_offset(sw_rings_base, ACC100_SIZE_64MBYTE);
    next_64mb_align_addr_iova = sw_rings_base_iova + next_64mb_align_offset;
...
get_queue_id_from_ring_info(struct rte_bbdev_data *data,
  ...
  for (queue_id = 0; queue_id < data->num_queues; ++queue_id) {
    struct acc100_queue *acc100_q = data->queues[queue_id].queue_private;
...
static inline void acc100_check_ir(struct acc100_device *acc100_dev) {
  ...
  ring_data = acc100_dev->info_ring +
              (acc100_dev->info_ring_head & ACC100_INFO_RING_MASK);

  while (ring_data->valid) {
    if ((ring_data->int_nb < ACC100_PF_INT_DMA_DL_DESC_IRQ) ||
        (ring_data->int_nb > ACC100_PF_INT_DMA_DL5G_DESC_IRQ))
      rte_bbdev_log(WARNING, "InfoRing: ITR:%d Info:0x%x", ring_data->int_nb,
                    ring_data->detailed_info);
    /* Initialize Info Ring entry and move forward */
    ring_data->val = 0;
    info_ring_head++;
    ring_data =
        acc100_dev->info_ring + (info_ring_head & ACC100_INFO_RING_MASK);
  }
}

/* Checks PF Info Ring to find the interrupt cause and handles it accordingly */
static inline void acc100_pf_interrupt_handler(struct rte_bbdev *dev) {
  ...
  while (ring_data->valid) {
    ...
    rte_bbdev_log_debug("ACC100 PF Interrupt received, Info Ring data: 0x%x",
                        ring_data->val);
    ...
    case ACC100_PF_INT_DMA_DL5G_DESC_IRQ:
      deq_intr_det.queue_id =
          get_queue_id_from_ring_info(dev->data, *ring_data);
      if (deq_intr_det.queue_id == UINT16_MAX) {
        rte_bbdev_log(ERR,
                      "Couldn't find queue: aq_id: %u, qg_id: %u, vf_id: %u",
                      ring_data->aq_id, ring_data->qg_id, ring_data->vf_id);
        return;
      }
      rte_bbdev_pmd_callback_process(dev, RTE_BBDEV_EVENT_DEQUEUE,
                                     &deq_intr_det);
      break;
    default:
...
static inline void acc100_vf_interrupt_handler(struct rte_bbdev *dev) {
  struct acc100_device *acc100_dev = dev->data->dev_private;
  volatile union acc100_info_ring_data *ring_data;
  struct acc100_deq_intr_details deq_intr_det;
  ...
  while (ring_data->valid) {
    ...
    rte_bbdev_log_debug("ACC100 VF Interrupt received, Info Ring data: 0x%x",
                        ring_data->val);
    ...
    case ACC100_VF_INT_DMA_DL5G_DESC_IRQ:
      ...
      deq_intr_det.queue_id =
          get_queue_id_from_ring_info(dev->data, *ring_data);
      if (deq_intr_det.queue_id == UINT16_MAX) {
        rte_bbdev_log(ERR, "Couldn't find queue: aq_id: %u, qg_id: %u",
                      ring_data->aq_id, ring_data->qg_id);
        return;
      }
      rte_bbdev_pmd_callback_process(dev, RTE_BBDEV_EVENT_DEQUEUE,
                                     &deq_intr_det);
...
static void acc100_dev_interrupt_handler(void *cb_arg) {
...
static int allocate_info_ring(struct rte_bbdev *dev) {
  struct acc100_device *d = dev->data->dev_private;
  const struct acc100_registry_addr *reg_addr;
  ...
  if (d->pf_device)
    ...
    reg_addr = &vf_reg_addr;
  /* Allocate InfoRing */
  d->info_ring = rte_zmalloc_socket(
      "Info Ring", ACC100_INFO_RING_NUM_ENTRIES * sizeof(*d->info_ring),
      RTE_CACHE_LINE_SIZE, dev->data->socket_id);
  if (d->info_ring == NULL) {
    rte_bbdev_log(ERR, "Failed to allocate Info Ring for %s:%u",
                  dev->device->driver->name, dev->data->dev_id);
    return -ENOMEM;
  }
  ...
  d->info_ring_head = (acc100_reg_read(d, reg_addr->info_ring_ptr) & 0xFFF) /
                      sizeof(union acc100_info_ring_data);
  return 0;
}

/* Allocate 64MB memory used for all software rings */
static int acc100_setup_queues(struct rte_bbdev *dev, uint16_t num_queues,
                               int socket_id) {
  ...
  if (d->pf_device && !d->acc100_conf.pf_mode_en) {
    rte_bbdev_log(NOTICE, "%s has PF mode disabled. This PF can't be used.",
                  dev->data->name);
    return -ENODEV;
  }
  ...
  d->tail_ptrs =
      rte_zmalloc_socket(dev->device->driver->name,
  ...
  d->harq_layout = rte_zmalloc_socket(
      "HARQ Layout", ACC100_HARQ_LAYOUT * sizeof(*d->harq_layout),
      RTE_CACHE_LINE_SIZE, dev->data->socket_id);
  ...
  rte_bbdev_log_debug(
      "ACC100 (%s) configured  sw_rings = %p, sw_rings_iova = %#" PRIx64,
      dev->data->name, d->sw_rings, d->sw_rings_iova);

  return 0;
}

static int acc100_intr_enable(struct rte_bbdev *dev) {
  int ret;
  struct acc100_device *d = dev->data->dev_private;
  ...
  if (rte_intr_type_get(dev->intr_handle) == RTE_INTR_HANDLE_VFIO_MSI ||
    ...
    ret = allocate_info_ring(dev);
    if (ret < 0) {
      rte_bbdev_log(ERR, "Couldn't allocate info ring for device: %s",
                    dev->data->name);
      return ret;
    }
    ...
    if (ret < 0) {
      rte_bbdev_log(ERR, "Couldn't enable interrupts for device: %s",
                    dev->data->name);
    ...
    if (ret < 0) {
      rte_bbdev_log(ERR, "Couldn't register interrupt callback for device: %s",
                    dev->data->name);
      rte_free(d->info_ring);
      return ret;
    }
...
static int acc100_dev_close(struct rte_bbdev *dev) {
  struct acc100_device *d = dev->data->dev_private;
  acc100_check_ir(d);
  if (d->sw_rings_base != NULL) {
 ...
 */
static int acc100_find_free_queue_idx(struct rte_bbdev *dev,
  ...
  if (group_idx >= ACC100_NUM_QGRPS || conf->priority >= qtop->num_qgroups) {
    rte_bbdev_log(INFO, "Invalid Priority on %s, priority %u", dev->data->name,
                  conf->priority);
    return -1;
  }
  /* Find a free AQ_idx  */
...
/* Setup ACC100 queue */
static int acc100_queue_setup(struct rte_bbdev *dev, uint16_t queue_id,
                              const struct rte_bbdev_queue_conf *conf) {
  struct acc100_device *d = dev->data->dev_private;
  ...
  int fcw_len =
      (conf->op_type == RTE_BBDEV_OP_LDPC_ENC
           ? ACC100_FCW_LE_BLEN
           : (conf->op_type == RTE_BBDEV_OP_TURBO_DEC ? ACC100_FCW_TD_BLEN
                                                      : ACC100_FCW_LD_BLEN));
  ...
  q->lb_in = rte_zmalloc_socket(dev->device->driver->name, RTE_CACHE_LINE_SIZE,
                                RTE_CACHE_LINE_SIZE, conf->socket);
  if (q->lb_in == NULL) {
  ...
  q->lb_out = rte_zmalloc_socket(dev->device->driver->name, RTE_CACHE_LINE_SIZE,
                                 RTE_CACHE_LINE_SIZE, conf->socket);
  if (q->lb_out == NULL) {
  ...
  q->mmio_reg_enqueue = RTE_PTR_ADD(
      d->mmio_base, queue_offset(d->pf_device, q->vf_id, q->qgrp_id, q->aq_id));

  rte_bbdev_log_debug("Setup dev%u q%u: qgrp_id=%u, vf_id=%u, aq_id=%u, "
...
static int acc100_queue_release(struct rte_bbdev *dev, uint16_t q_id) {
  struct acc100_device *d = dev->data->dev_private;
  struct acc100_queue *q = dev->data->queues[q_id].queue_private;

  if (q != NULL) {
    /* Mark the Queue as un-assigned */
    d->q_assigned_bit_map[q->qgrp_id] &= (0xFFFFFFFF - (1 << q->aq_id));
    rte_free(q->lb_in);
    rte_free(q->lb_out);
    rte_free(q);
...
static void acc100_dev_info_get(struct rte_bbdev *dev,
  ...
  static const struct rte_bbdev_op_cap bbdev_capabilities[] = {
      {.type = RTE_BBDEV_OP_TURBO_DEC,
       .cap.turbo_dec =
           {
               .capability_flags = RTE_BBDEV_TURBO_SUBBLOCK_DEINTERLEAVE |
               ...
               .num_buffers_hard_out = RTE_BBDEV_TURBO_MAX_CODE_BLOCKS,
               .num_buffers_soft_out = RTE_BBDEV_TURBO_MAX_CODE_BLOCKS,
           }},
      {.type = RTE_BBDEV_OP_TURBO_ENC,
       .cap.turbo_enc =
           {
               .capability_flags = RTE_BBDEV_TURBO_CRC_24B_ATTACH |
      ...
      {.type = RTE_BBDEV_OP_LDPC_ENC,
       .cap.ldpc_enc =
           {
               .capability_flags = RTE_BBDEV_LDPC_RATE_MATCH |
                                   RTE_BBDEV_LDPC_CRC_24B_ATTACH |
                                   RTE_BBDEV_LDPC_INTERLEAVER_BYPASS |
                                   RTE_BBDEV_LDPC_ENC_INTERRUPTS,
               .num_buffers_src = RTE_BBDEV_LDPC_MAX_CODE_BLOCKS,
               .num_buffers_dst = RTE_BBDEV_LDPC_MAX_CODE_BLOCKS,
           }},
   ...
   * priorities
   */
  dev_info->max_num_queues = d->acc100_conf.q_dl_5g.num_aqs_per_groups *
  ...
  dev_info->max_dl_queue_priority = d->acc100_conf.q_dl_4g.num_qgroups - 1;
  dev_info->max_ul_queue_priority = d->acc100_conf.q_ul_4g.num_qgroups - 1;
...
static int acc100_queue_intr_enable(struct rte_bbdev *dev, uint16_t queue_id) {
...
static int acc100_queue_intr_disable(struct rte_bbdev *dev, uint16_t queue_id) {
  struct acc100_queue *q = dev->data->queues[queue_id].queue_private;
...
static inline char *mbuf_append(struct rte_mbuf *m_head, struct rte_mbuf *m,
                                uint16_t len) {
...
static inline void acc100_fcw_te_fill(const struct rte_bbdev_enc_op *op,
  ...
  fcw->bypass_rv_idx1 =
      check_bit(op->turbo_enc.op_flags, RTE_BBDEV_TURBO_RV_INDEX_BYPASS);
  fcw->code_block_crc =
      check_bit(op->turbo_enc.op_flags, RTE_BBDEV_TURBO_CRC_24B_ATTACH);
...
static inline uint16_t get_k0(uint16_t n_cb, uint16_t z_c, uint8_t bg,
  ...
  if (rv_index == 1)
    return (((bg == 1 ? ACC100_K0_1_1 : ACC100_K0_1_2) * n_cb) / n) * z_c;
  else if (rv_index == 2)
    return (((bg == 1 ? ACC100_K0_2_1 : ACC100_K0_2_2) * n_cb) / n) * z_c;
  else
    return (((bg == 1 ? ACC100_K0_3_1 : ACC100_K0_3_2) * n_cb) / n) * z_c;
}

/* Fill in a frame control word for LDPC encoding. */
static inline void acc100_fcw_le_fill(const struct rte_bbdev_enc_op *op,
  ...
  fcw->k0 =
      get_k0(fcw->ncb, fcw->Zc, op->ldpc_enc.basegraph, op->ldpc_enc.rv_index);
  fcw->rm_e = op->ldpc_enc.cb_params.e;
  fcw->crc_select =
      check_bit(op->ldpc_enc.op_flags, RTE_BBDEV_LDPC_CRC_24B_ATTACH);
  fcw->bypass_intlv =
      check_bit(op->ldpc_enc.op_flags, RTE_BBDEV_LDPC_INTERLEAVER_BYPASS);
  fcw->mcb_count = num_cb;
}

/* Fill in a frame control word for turbo decoding. */
static inline void acc100_fcw_td_fill(const struct rte_bbdev_dec_op *op,
  ...
  fcw->turbo_crc_type =
      check_bit(op->turbo_dec.op_flags, RTE_BBDEV_TURBO_CRC_TYPE_24B);
  fcw->bypass_sb_deint = 0;
  fcw->raw_decoder_input_on = 0;
  fcw->max_iter = op->turbo_dec.iter_max;
  fcw->half_iter_on =
      !check_bit(op->turbo_dec.op_flags, RTE_BBDEV_TURBO_HALF_ITERATION_EVEN);
}
...
acc100_fcw_ld_fill(const struct rte_bbdev_dec_op *op, struct acc100_fcw_ld *fcw,
  ...
  fcw->k0 =
      get_k0(fcw->ncb, fcw->Zc, op->ldpc_dec.basegraph, op->ldpc_dec.rv_index);
  if (op->ldpc_dec.code_block_mode == RTE_BBDEV_CODE_BLOCK)
    fcw->rm_e = op->ldpc_dec.cb_params.e;
  else
    fcw->rm_e = (op->ldpc_dec.tb_params.r < op->ldpc_dec.tb_params.cab)
                    ? op->ldpc_dec.tb_params.ea
                    : op->ldpc_dec.tb_params.eb;

  fcw->hcin_en =
      check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_IN_ENABLE);
  fcw->hcout_en =
      check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_OUT_ENABLE);
  fcw->crc_select =
      check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_CRC_TYPE_24B_CHECK);
  fcw->bypass_dec =
      check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_DECODE_BYPASS);
  fcw->bypass_intlv =
      check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_DEINTERLEAVER_BYPASS);
  if (op->ldpc_dec.q_m == 1) {
    fcw->bypass_intlv = 1;
    fcw->qm = 2;
  }
  fcw->hcin_decomp_mode =
  ...
  harq_index = op->ldpc_dec.harq_combined_output.offset / ACC100_HARQ_OFFSET;
#ifdef ACC100_EXT_MEM
  /* Limit cases when HARQ pruning is valid */
  harq_prun =
      ((op->ldpc_dec.harq_combined_output.offset % ACC100_HARQ_OFFSET) == 0) &&
      (op->ldpc_dec.harq_combined_output.offset <=
       UINT16_MAX * ACC100_HARQ_OFFSET);
#endif
  if (fcw->hcin_en > 0) {
  ...
  fcw->itstop =
      check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_ITERATION_STOP_ENABLE);
   ...
   */
  if (fcw->hcout_en > 0) {
    parity_offset = (op->ldpc_dec.basegraph == 1 ? 20 : 8) * op->ldpc_dec.z_c -
                    op->ldpc_dec.n_filler;
    k0_p =
        (fcw->k0 > parity_offset) ? fcw->k0 - op->ldpc_dec.n_filler : fcw->k0;
 ...
 *
 */
static inline int acc100_dma_fill_blk_type_in(struct acc100_dma_req_desc *desc,
                                              struct rte_mbuf **input,
                                              uint32_t *offset, uint32_t cb_len,
  ...
  *seg_total_left -= part_len;
  ...
  desc->data_ptrs[next_triplet].address = rte_pktmbuf_iova_offset(m, *offset);
  ...
  while (cb_len > 0) {
    if (next_triplet < ACC100_DMA_MAX_NUM_POINTERS_IN && m->next != NULL) {
      ...
      *seg_total_left = rte_pktmbuf_data_len(m);
      part_len = (*seg_total_left < cb_len) ? *seg_total_left : cb_len;
      desc->data_ptrs[next_triplet].address = rte_pktmbuf_iova_offset(m, 0);
      desc->data_ptrs[next_triplet].blen = part_len;
      desc->data_ptrs[next_triplet].blkid = ACC100_DMA_BLKID_IN;
      desc->data_ptrs[next_triplet].last = 0;
      desc->data_ptrs[next_triplet].dma_ext = 0;
 ...
 */
static inline int acc100_dma_fill_blk_type_out(struct acc100_dma_req_desc *desc,
                                               struct rte_mbuf *output,
                                               uint32_t out_offset,
                                               uint32_t output_len,
                                               int next_triplet, int blk_id) {
...
static inline int check_mbuf_total_left(uint32_t mbuf_total_left) {
...
static inline int acc100_dma_desc_te_fill(
    struct rte_bbdev_enc_op *op, struct acc100_dma_req_desc *desc,
    struct rte_mbuf **input, struct rte_mbuf *output, uint32_t *in_offset,
    uint32_t *out_offset, uint32_t *out_length, uint32_t *mbuf_total_left,
  ...
  if (unlikely((*mbuf_total_left == 0) || (*mbuf_total_left < length))) {
    rte_bbdev_log(ERR,
                  "Mismatch between mbuf length and included CB sizes: mbuf "
                  "len %u, cb len %u",
                  *mbuf_total_left, length);
    return -1;
  }

  next_triplet = acc100_dma_fill_blk_type_in(desc, input, in_offset, length,
  ...
  *mbuf_total_left -= length;
  ...
  next_triplet =
      acc100_dma_fill_blk_type_out(desc, output, *out_offset, *out_length,
                                   next_triplet, ACC100_DMA_BLKID_OUT_ENC);
  if (unlikely(next_triplet < 0)) {
    rte_bbdev_log(
        ERR,
        "Mismatch between data to process and mbuf data length in bbdev_op: %p",
...
static inline int acc100_dma_desc_le_fill(
    struct rte_bbdev_enc_op *op, struct acc100_dma_req_desc *desc,
    struct rte_mbuf **input, struct rte_mbuf *output, uint32_t *in_offset,
    uint32_t *out_offset, uint32_t *out_length, uint32_t *mbuf_total_left,
    uint32_t *seg_total_left) {
  ...
  next_triplet = acc100_dma_fill_blk_type_in(
      desc, input, in_offset, in_length_in_bytes, seg_total_left, next_triplet);
  if (unlikely(next_triplet < 0)) {
    rte_bbdev_log(
        ERR,
        "Mismatch between data to process and mbuf data length in bbdev_op: %p",
...
static inline int acc100_dma_desc_td_fill(
    struct rte_bbdev_dec_op *op, struct acc100_dma_req_desc *desc,
    struct rte_mbuf **input, struct rte_mbuf *h_output,
    struct rte_mbuf *s_output, uint32_t *in_offset, uint32_t *h_out_offset,
    uint32_t *s_out_offset, uint32_t *h_out_length, uint32_t *s_out_length,
  ...
  if (op->turbo_dec.code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
    k = (r < op->turbo_dec.tb_params.c_neg) ? op->turbo_dec.tb_params.k_neg
                                            : op->turbo_dec.tb_params.k_pos;
    e = (r < op->turbo_dec.tb_params.cab) ? op->turbo_dec.tb_params.ea
  ...
  if ((op->turbo_dec.code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) &&
      !check_bit(op->turbo_dec.op_flags, RTE_BBDEV_TURBO_DEC_TB_CRC_24B_KEEP))
    crc24_overlap = 24;
  if ((op->turbo_dec.code_block_mode == RTE_BBDEV_CODE_BLOCK) &&
      check_bit(op->turbo_dec.op_flags, RTE_BBDEV_TURBO_DEC_CRC_24B_DROP))
    crc24_overlap = 24;
  ...
  *mbuf_total_left -= kw;
  ...
  next_triplet = acc100_dma_fill_blk_type_out(
      desc, h_output, *h_out_offset, (k - crc24_overlap) >> 3, next_triplet,
  ...
  *h_out_offset += *h_out_length;
  ...
  if (check_bit(op->turbo_dec.op_flags, RTE_BBDEV_TURBO_SOFT_OUTPUT)) {
    if (check_bit(op->turbo_dec.op_flags, RTE_BBDEV_TURBO_EQUALIZER))
      *s_out_length = e;
    else
      *s_out_length = (k * 3) + 12;

    next_triplet = acc100_dma_fill_blk_type_out(desc, s_output, *s_out_offset,
                                                *s_out_length, next_triplet,
                                                ACC100_DMA_BLKID_OUT_SOFT);
    if (unlikely(next_triplet < 0)) {
      rte_bbdev_log(ERR,
                    "Mismatch between data to process and mbuf data length in "
                    "bbdev_op: %p",
                    op);
      return -1;
    }
...
static inline int acc100_dma_desc_ld_fill(
    struct rte_bbdev_dec_op *op, struct acc100_dma_req_desc *desc,
    struct rte_mbuf **input, struct rte_mbuf *h_output, uint32_t *in_offset,
    uint32_t *h_out_offset, uint32_t *h_out_length, uint32_t *mbuf_total_left,
    uint32_t *seg_total_left, struct acc100_fcw_ld *fcw) {
  ...
  next_triplet = acc100_dma_fill_blk_type_in(
      desc, input, in_offset, input_length, seg_total_left, next_triplet);
  ...
  if (check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_IN_ENABLE)) {
    ...
    desc->data_ptrs[next_triplet].address = dec->harq_combined_input.offset;
    ...
    acc100_dma_fill_blk_type_out(desc, op->ldpc_dec.harq_combined_input.data,
                                 op->ldpc_dec.harq_combined_input.offset,
                                 h_p_size, next_triplet,
  ...
  *mbuf_total_left -= input_length;
  ...
  next_triplet = acc100_dma_fill_blk_type_out(desc, h_output, *h_out_offset,
                                              output_length >> 3, next_triplet,
                                              ACC100_DMA_BLKID_OUT_HARD);
  ...
  if (check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_OUT_ENABLE)) {
    ...
    h_np_size =
        fcw->hcout_offset > 0 ? fcw->hcout_offset + fcw->hcout_size1 : h_p_size;
    if (h_comp) {
      h_np_size = (h_np_size * 3 + 3) / 4;
      h_p_size = (h_p_size * 3 + 3) / 4;
    }
    dec->harq_combined_output.length = h_np_size;
    desc->data_ptrs[next_triplet].address = dec->harq_combined_output.offset;
    desc->data_ptrs[next_triplet].blen = h_p_size;
    desc->data_ptrs[next_triplet].blkid = ACC100_DMA_BLKID_OUT_HARQ;
    desc->data_ptrs[next_triplet].dma_ext = 1;
#ifndef ACC100_EXT_MEM
    acc100_dma_fill_blk_type_out(desc, dec->harq_combined_output.data,
                                 dec->harq_combined_output.offset, h_p_size,
                                 next_triplet, ACC100_DMA_BLKID_OUT_HARQ);
#endif
    next_triplet++;
  }
...
static inline void acc100_dma_desc_ld_update(
    struct rte_bbdev_dec_op *op, struct acc100_dma_req_desc *desc,
    struct rte_mbuf *input, struct rte_mbuf *h_output, uint32_t *in_offset,
    uint32_t *h_out_offset, uint32_t *h_out_length,
    union acc100_harq_layout_data *harq_layout) {
  int next_triplet = 1; /* FCW already done */
  desc->data_ptrs[next_triplet].address =
  ...
  *h_out_length = desc->data_ptrs[next_triplet].blen;
  ...
  if (check_bit(op->ldpc_dec.op_flags, RTE_BBDEV_LDPC_HQ_COMBINE_OUT_ENABLE)) {
    ...
    int16_t hq_idx =
        op->ldpc_dec.harq_combined_output.offset / ACC100_HARQ_OFFSET;
    int16_t prev_hq_idx =
        prev_op->ldpc_dec.harq_combined_output.offset / ACC100_HARQ_OFFSET;
...
static inline void acc100_dma_enqueue(struct acc100_queue *q, uint16_t n,
  ...
  do {
    ...
    desc = q->ring_addr +
           ((q->sw_ring_head + enq_batch_size - 1) & q->sw_ring_wrap_mask);
...
#endif
    rte_bbdev_log_debug("Enqueue %u reqs (phys %#" PRIx64 ") to reg %p",
                        enq_batch_size, req_elem_addr,
...
static inline int validate_enc_op(struct rte_bbdev_enc_op *op) {
  ...
  if (turbo_enc->rv_index > 3) {
    rte_bbdev_log(ERR, "rv_index (%u) is out of range 0 <= value <= 3",
                  turbo_enc->rv_index);
    return -1;
  }
  if (turbo_enc->code_block_mode != RTE_BBDEV_TRANSPORT_BLOCK &&
      turbo_enc->code_block_mode != RTE_BBDEV_CODE_BLOCK) {
    rte_bbdev_log(ERR, "code_block_mode (%u) is out of range 0 <= value <= 1",
  ...
  if (turbo_enc->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
    ...
    if ((tb->k_neg < RTE_BBDEV_TURBO_MIN_CB_SIZE ||
        ...
        tb->c_neg > 0) {
      rte_bbdev_log(ERR, "k_neg (%u) is out of range %u <= value <= %u",
    ...
    if (tb->k_pos < RTE_BBDEV_TURBO_MIN_CB_SIZE ||
      ...
      rte_bbdev_log(ERR, "k_pos (%u) is out of range %u <= value <= %u",
                    tb->k_pos, RTE_BBDEV_TURBO_MIN_CB_SIZE,
                    RTE_BBDEV_TURBO_MAX_CB_SIZE);
      return -1;
    }
    if (tb->c_neg > (RTE_BBDEV_TURBO_MAX_CODE_BLOCKS - 1))
      rte_bbdev_log(ERR, "c_neg (%u) is out of range 0 <= value <= %u",
                    tb->c_neg, RTE_BBDEV_TURBO_MAX_CODE_BLOCKS - 1);
    if (tb->c < 1 || tb->c > RTE_BBDEV_TURBO_MAX_CODE_BLOCKS) {
      rte_bbdev_log(ERR, "c (%u) is out of range 1 <= value <= %u", tb->c,
                    RTE_BBDEV_TURBO_MAX_CODE_BLOCKS);
      return -1;
    }
    if (tb->cab > tb->c) {
      rte_bbdev_log(ERR, "cab (%u) is greater than c (%u)", tb->cab, tb->c);
      return -1;
    }
    if ((tb->ea < RTE_BBDEV_TURBO_MIN_CB_SIZE || (tb->ea % 2)) &&
        tb->r < tb->cab) {
      rte_bbdev_log(ERR, "ea (%u) is less than %u or it is not even", tb->ea,
                    RTE_BBDEV_TURBO_MIN_CB_SIZE);
      return -1;
    }
    if ((tb->eb < RTE_BBDEV_TURBO_MIN_CB_SIZE || (tb->eb % 2)) &&
        tb->c > tb->cab) {
      rte_bbdev_log(ERR, "eb (%u) is less than %u or it is not even", tb->eb,
                    RTE_BBDEV_TURBO_MIN_CB_SIZE);
      return -1;
    }

    kw_neg = 3 * RTE_ALIGN_CEIL(tb->k_neg + 4, RTE_BBDEV_TURBO_C_SUBBLOCK);
    ...
    kw_pos = 3 * RTE_ALIGN_CEIL(tb->k_pos + 4, RTE_BBDEV_TURBO_C_SUBBLOCK);
    ...
    if (tb->r > (tb->c - 1)) {
      rte_bbdev_log(ERR, "r (%u) is greater than c - 1 (%u)", tb->r, tb->c - 1);
      return -1;
    }
  } else {
    ...
    if (cb->k < RTE_BBDEV_TURBO_MIN_CB_SIZE ||
      ...
      rte_bbdev_log(ERR, "k (%u) is out of range %u <= value <= %u", cb->k,
                    RTE_BBDEV_TURBO_MIN_CB_SIZE, RTE_BBDEV_TURBO_MAX_CB_SIZE);
      return -1;
    }

    if (cb->e < RTE_BBDEV_TURBO_MIN_CB_SIZE || (cb->e % 2)) {
      rte_bbdev_log(ERR, "e (%u) is less than %u or it is not even", cb->e,
                    RTE_BBDEV_TURBO_MIN_CB_SIZE);
      return -1;
    }
    ...
    if (cb->ncb < cb->k || cb->ncb > kw) {
      rte_bbdev_log(ERR, "ncb (%u) is out of range (%u) k <= value <= (%u) kw",
                    cb->ncb, cb->k, kw);
      return -1;
    }
  }
...
static inline int validate_ldpc_enc_op(struct rte_bbdev_enc_op *op) {
  ...
  if (ldpc_enc->input.length > RTE_BBDEV_LDPC_MAX_CB_SIZE >> 3) {
    rte_bbdev_log(ERR, "CB size (%u) is too big, max: %d",
                  ldpc_enc->input.length, RTE_BBDEV_LDPC_MAX_CB_SIZE);
    return -1;
  }
  if ((ldpc_enc->basegraph > 2) || (ldpc_enc->basegraph == 0)) {
    rte_bbdev_log(ERR, "BG (%u) is out of range 1 <= value <= 2",
                  ldpc_enc->basegraph);
    return -1;
  }
  ...
  if (ldpc_enc->n_filler >= K) {
    rte_bbdev_log(ERR, "K and F are not compatible %u %u", K,
                  ldpc_enc->n_filler);
    return -1;
  }
  return 0;
...
static inline int validate_ldpc_dec_op(struct rte_bbdev_dec_op *op) {
...
static inline int enqueue_enc_one_op_cb(struct acc100_queue *q,
  ...
  ret = acc100_dma_desc_te_fill(op, &desc->req, &input, output, &in_offset,
                                &out_offset, &out_length, &mbuf_total_left,
                                &seg_total_left, 0);
...
#ifdef RTE_LIBRTE_BBDEV_DEBUG
  rte_memdump(stderr, "FCW", &desc->req.fcw_te, sizeof(desc->req.fcw_te) - 8);
...
/* Enqueue one encode operations for ACC100 device in CB mode */
static inline int enqueue_ldpc_enc_n_op_cb(struct acc100_queue *q,
  ...
  for (i = 0; i < num; i++) {
    ...
    next_triplet++;
    desc->req.data_ptrs[next_triplet].address =
        rte_pktmbuf_iova_offset(ops[i]->ldpc_enc.output.data, 0);
...
#ifdef RTE_LIBRTE_BBDEV_DEBUG
  rte_memdump(stderr, "FCW", &desc->req.fcw_le, sizeof(desc->req.fcw_le) - 8);
  rte_memdump(stderr, "Req Desc.", desc, sizeof(*desc));
#endif
...
/* Enqueue one encode operations for ACC100 device in CB mode */
static inline int enqueue_ldpc_enc_one_op_cb(struct acc100_queue *q,
                                             struct rte_bbdev_enc_op *op,
                                             uint16_t total_enqueued_cbs) {
  union acc100_dma_desc *desc = NULL;
  int ret;
  ...
  seg_total_left = rte_pktmbuf_data_len(op->ldpc_enc.input.data) - in_offset;

  ret = acc100_dma_desc_le_fill(op, &desc->req, &input, output, &in_offset,
                                &out_offset, &out_length, &mbuf_total_left,
                                &seg_total_left);
...
static inline int validate_dec_op(struct rte_bbdev_dec_op *op) {
  ...
  if (turbo_dec->iter_min < 1) {
    rte_bbdev_log(ERR, "iter_min (%u) is less than 1", turbo_dec->iter_min);
    return -1;
  }
  if (turbo_dec->iter_max <= 2) {
    rte_bbdev_log(ERR, "iter_max (%u) is less than or equal to 2",
                  turbo_dec->iter_max);
    return -1;
  }
  if (turbo_dec->iter_min > turbo_dec->iter_max) {
    rte_bbdev_log(ERR, "iter_min (%u) is greater than iter_max (%u)",
  ...
  if (turbo_dec->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
    ...
    if (check_bit(turbo_dec->op_flags, RTE_BBDEV_TURBO_EQUALIZER) &&
        (tb->ea < RTE_BBDEV_TURBO_MIN_CB_SIZE || (tb->ea % 2)) && tb->cab > 0) {
      rte_bbdev_log(ERR, "ea (%u) is less than %u or it is not even", tb->ea,
                    RTE_BBDEV_TURBO_MIN_CB_SIZE);
      return -1;
    }
    if (check_bit(turbo_dec->op_flags, RTE_BBDEV_TURBO_EQUALIZER) &&
        (tb->eb < RTE_BBDEV_TURBO_MIN_CB_SIZE || (tb->eb % 2)) &&
        tb->c > tb->cab) {
      rte_bbdev_log(ERR, "eb (%u) is less than %u or it is not even", tb->eb,
                    RTE_BBDEV_TURBO_MIN_CB_SIZE);
    }
    ...
    if (check_bit(turbo_dec->op_flags, RTE_BBDEV_TURBO_EQUALIZER) &&
        (cb->e < RTE_BBDEV_TURBO_MIN_CB_SIZE || (cb->e % 2))) {
      rte_bbdev_log(ERR, "e (%u) is less than %u or it is not even", cb->e,
                    RTE_BBDEV_TURBO_MIN_CB_SIZE);
      return -1;
    }
...
static inline int enqueue_dec_one_op_cb(struct acc100_queue *q,
  ...
  union acc100_dma_desc *desc = NULL;
  int ret;
  uint32_t in_offset, h_out_offset, s_out_offset, s_out_length, h_out_length,
      mbuf_total_left, seg_total_left;
  struct rte_mbuf *input, *h_output_head, *h_output, *s_output_head, *s_output;
  ...
  ret = acc100_dma_desc_td_fill(op, &desc->req, &input, h_output, s_output,
                                &in_offset, &h_out_offset, &s_out_offset,
...
#ifdef RTE_LIBRTE_BBDEV_DEBUG
  rte_memdump(stderr, "FCW", &desc->req.fcw_td, sizeof(desc->req.fcw_td) - 8);
  rte_memdump(stderr, "Req Desc.", desc, sizeof(*desc));
  if (check_mbuf_total_left(mbuf_total_left) != 0)
    return -EINVAL;
#endif
...
static inline int harq_loopback(struct acc100_queue *q,
                                struct rte_bbdev_dec_op *op,
                                uint16_t total_enqueued_cbs) {
  ...
  uint16_t harq_index =
      (ddr_mem_in ? op->ldpc_dec.harq_combined_input.offset
                  : op->ldpc_dec.harq_combined_output.offset) /
      ACC100_HARQ_OFFSET;
  ...
  rte_bbdev_log(DEBUG, "Loopback IN %d Index %d offset %d length %d %d\n",
                ddr_mem_in, harq_index, harq_layout[harq_index].offset,
                harq_in_length, harq_dma_length_in);
  ...
  rte_bbdev_log(DEBUG, "Loopback FCW Config %d %d %d\n", fcw->hcin_size0,
                fcw->hcin_offset, fcw->hcin_size1);
  ...
  desc->req.data_ptrs[next_triplet].address = q->lb_in_addr_iova;
  ...
  if (!ddr_mem_in) {
    next_triplet = acc100_dma_fill_blk_type_out(
        &desc->req, op->ldpc_dec.harq_combined_input.data,
        op->ldpc_dec.harq_combined_input.offset, harq_dma_length_in,
        next_triplet, ACC100_DMA_BLKID_IN_HARQ);
  } else {
    desc->req.data_ptrs[next_triplet].address =
        op->ldpc_dec.harq_combined_input.offset;
    desc->req.data_ptrs[next_triplet].blen = harq_dma_length_in;
    desc->req.data_ptrs[next_triplet].blkid = ACC100_DMA_BLKID_IN_HARQ;
  ...
  desc->req.data_ptrs[next_triplet].address = q->lb_out_addr_iova;
  ...
  if (check_bit(op->ldpc_dec.op_flags,
    ...
    desc->req.data_ptrs[next_triplet].address =
        op->ldpc_dec.harq_combined_output.offset;
    desc->req.data_ptrs[next_triplet].blen = harq_dma_length_out;
    desc->req.data_ptrs[next_triplet].blkid = ACC100_DMA_BLKID_OUT_HARQ;
    desc->req.data_ptrs[next_triplet].dma_ext = 1;
    next_triplet++;
  } else {
    ...
    next_triplet = acc100_dma_fill_blk_type_out(
        &desc->req, op->ldpc_dec.harq_combined_output.data,
        op->ldpc_dec.harq_combined_output.offset, harq_dma_length_out,
        next_triplet, ACC100_DMA_BLKID_OUT_HARQ);
    /* HARQ output */
    mbuf_append(hq_output_head, hq_output, harq_dma_length_out);
    op->ldpc_dec.harq_combined_output.length = harq_dma_length_out;
  }
  desc->req.data_ptrs[next_triplet - 1].last = 1;
...
static inline int enqueue_ldpc_dec_one_op_cb(struct acc100_queue *q,
  ...
  if (same_op) {
    ...
    desc_idx =
        ((q->sw_ring_head + total_enqueued_cbs - 1) & q->sw_ring_wrap_mask);
    ...
    rte_memcpy(new_ptr + ACC100_5GUL_OFFSET_0, prev_ptr + ACC100_5GUL_OFFSET_0,
               ACC100_5GUL_SIZE_1);
    desc->req.op_addr = prev_desc->req.op_addr;
    /* Copy FCW */
    rte_memcpy(new_ptr + ACC100_DESC_FCW_OFFSET,
               prev_ptr + ACC100_DESC_FCW_OFFSET, ACC100_FCW_LD_BLEN);
    acc100_dma_desc_ld_update(op, &desc->req, input, h_output, &in_offset,
                              &h_out_offset, &h_out_length, harq_layout);
  } else {
    struct acc100_fcw_ld *fcw;
    uint32_t seg_total_left;
    fcw = &desc->req.fcw_ld;
    ...
    ret = acc100_dma_desc_ld_fill(op, &desc->req, &input, h_output, &in_offset,
                                  &h_out_offset, &h_out_length,
                                  &mbuf_total_left, &seg_total_left, fcw);
    if (unlikely(ret < 0))
      return ret;
  }
...
#ifdef RTE_LIBRTE_BBDEV_DEBUG
  rte_memdump(stderr, "FCW", &desc->req.fcw_ld, sizeof(desc->req.fcw_ld) - 8);
  rte_memdump(stderr, "Req Desc.", desc, sizeof(*desc));
#endif
...
static inline int enqueue_ldpc_dec_one_op_tb(struct acc100_queue *q,
                                             struct rte_bbdev_dec_op *op,
                                             uint16_t total_enqueued_cbs,
                                             uint8_t cbs_in_tb) {
  union acc100_dma_desc *desc = NULL;
  ...
  while (mbuf_total_left > 0 && r < c) {
    ...
    ret = acc100_dma_desc_ld_fill(
        op, &desc->req, &input, h_output, &in_offset, &h_out_offset,
        &h_out_length, &mbuf_total_left, &seg_total_left, &desc->req.fcw_ld);
...
get_num_cbs_in_tb_ldpc_dec(struct rte_bbdev_op_ldpc_dec *ldpc_dec) {
  ...
  while (length > 0 && r < ldpc_dec->tb_params.c) {
    length -= (r < ldpc_dec->tb_params.cab) ? ldpc_dec->tb_params.ea
...
static uint16_t acc100_enqueue_enc_cb(struct rte_bbdev_queue_data *q_data,
                                      struct rte_bbdev_enc_op **ops,
                                      uint16_t num) {
...
static inline bool check_mux(struct rte_bbdev_enc_op **ops, uint16_t num) {
  uint16_t i;
  if (num <= 1)
    return false;
  for (i = 1; i < num; ++i) {
    ...
    if (memcmp((uint8_t *)(&ops[i]->ldpc_enc) + ACC100_ENC_OFFSET,
               (uint8_t *)(&ops[0]->ldpc_enc) + ACC100_ENC_OFFSET,
...
acc100_enqueue_ldpc_enc_cb(struct rte_bbdev_queue_data *q_data,
  ...
  /* Set SDone in last CB in enqueued ops for CB mode*/
  desc =
      q->ring_addr + ((q->sw_ring_head + desc_idx - 1) & q->sw_ring_wrap_mask);
  desc->req.sdone_enable = 1;
  desc->req.irq_enable = q->irq_enable;
...
static uint16_t acc100_enqueue_enc_tb(struct rte_bbdev_queue_data *q_data,
                                      struct rte_bbdev_enc_op **ops,
                                      uint16_t num) {
  struct acc100_queue *q = q_data->queue_private;
  int32_t avail = q->sw_ring_depth + q->sw_ring_tail - q->sw_ring_head;
...
static uint16_t acc100_enqueue_enc(struct rte_bbdev_queue_data *q_data,
                                   struct rte_bbdev_enc_op **ops,
                                   uint16_t num) {
  if (unlikely(num == 0))
    return 0;
...
/* Enqueue encode operations for ACC100 device. */
static uint16_t acc100_enqueue_ldpc_enc(struct rte_bbdev_queue_data *q_data,
                                        struct rte_bbdev_enc_op **ops,
                                        uint16_t num) {
  if (unlikely(num == 0))
    return 0;
...
static uint16_t acc100_enqueue_dec_cb(struct rte_bbdev_queue_data *q_data,
...
/* Check we can mux encode operations with common FCW */
static inline bool cmp_ldpc_dec_op(struct rte_bbdev_dec_op **ops) {
  /* Only mux compatible code blocks */
  if (memcmp((uint8_t *)(&ops[0]->ldpc_dec) + ACC100_DEC_OFFSET,
             (uint8_t *)(&ops[1]->ldpc_dec) + ACC100_DEC_OFFSET,
             ACC100_CMP_DEC_SIZE) != 0) {
    return false;
  } else
    return true;
}
...
static uint16_t acc100_enqueue_ldpc_dec_tb(struct rte_bbdev_queue_data *q_data,
                                           struct rte_bbdev_dec_op **ops,
                                           uint16_t num) {
  struct acc100_queue *q = q_data->queue_private;
  int32_t avail = q->sw_ring_depth + q->sw_ring_tail - q->sw_ring_head;
  ...
  for (i = 0; i < num; ++i) {
    ...
    ret = enqueue_ldpc_dec_one_op_tb(q, ops[i], enqueued_cbs, cbs_in_tb);
    if (ret < 0)
      break;
    enqueued_cbs += ret;
  }
...
/* Enqueue decode operations for ACC100 device in CB mode */
static uint16_t acc100_enqueue_ldpc_dec_cb(struct rte_bbdev_queue_data *q_data,
                                           struct rte_bbdev_dec_op **ops,
                                           uint16_t num) {
  struct acc100_queue *q = q_data->queue_private;
  int32_t avail = q->sw_ring_depth + q->sw_ring_tail - q->sw_ring_head;
  ...
  for (i = 0; i < num; ++i) {
    ...
    rte_bbdev_log(INFO, "Op %d %d %d %d %d %d %d %d %d %d %d %d\n", i,
                  ops[i]->ldpc_dec.op_flags, ops[i]->ldpc_dec.rv_index,
...
/* Enqueue decode operations for ACC100 device in TB mode */
static uint16_t acc100_enqueue_dec_tb(struct rte_bbdev_queue_data *q_data,
                                      struct rte_bbdev_dec_op **ops,
                                      uint16_t num) {
  struct acc100_queue *q = q_data->queue_private;
  int32_t avail = q->sw_ring_depth + q->sw_ring_tail - q->sw_ring_head;
...
static uint16_t acc100_enqueue_dec(struct rte_bbdev_queue_data *q_data,
                                   struct rte_bbdev_dec_op **ops,
                                   uint16_t num) {
  if (unlikely(num == 0))
    return 0;
...
/* Enqueue decode operations for ACC100 device. */
static uint16_t acc100_enqueue_ldpc_dec(struct rte_bbdev_queue_data *q_data,
                                        struct rte_bbdev_dec_op **ops,
                                        uint16_t num) {
  struct acc100_queue *q = q_data->queue_private;
  int32_t aq_avail = q->aq_depth + (q->aq_dequeued - q->aq_enqueued) / 128;
...
static inline int dequeue_enc_one_op_cb(struct acc100_queue *q,
                                        struct rte_bbdev_enc_op **ref_op,
                                        uint16_t total_dequeued_cbs,
                                        uint32_t *aq_dequeued) {
  ...
  desc = q->ring_addr +
         ((q->sw_ring_tail + total_dequeued_cbs) & q->sw_ring_wrap_mask);
...
static inline int dequeue_enc_one_op_tb(struct acc100_queue *q,
                                        struct rte_bbdev_enc_op **ref_op,
                                        uint16_t total_dequeued_cbs,
                                        uint32_t *aq_dequeued) {
  ...
  /* Get last CB */
  last_desc =
      q->ring_addr + ((q->sw_ring_tail + total_dequeued_cbs + cbs_in_tb - 1) &
                      q->sw_ring_wrap_mask);
...
static inline int dequeue_dec_one_op_cb(struct rte_bbdev_queue_data *q_data,
                                        struct acc100_queue *q,
                                        struct rte_bbdev_dec_op **ref_op,
                                        uint16_t dequeued_cbs,
                                        uint32_t *aq_dequeued) {
  union acc100_dma_desc *desc, atom_desc;
  union acc100_dma_rsp_desc rsp;
  struct rte_bbdev_dec_op *op;
  ...
  desc =
      q->ring_addr + ((q->sw_ring_tail + dequeued_cbs) & q->sw_ring_wrap_mask);
  atom_desc.atom_hdr = __atomic_load_n((uint64_t *)desc, __ATOMIC_RELAXED);
...
static inline int dequeue_dec_one_op_tb(struct acc100_queue *q,
                                        struct rte_bbdev_dec_op **ref_op,
                                        uint16_t dequeued_cbs,
                                        uint32_t *aq_dequeued) {
  union acc100_dma_desc *desc, *last_desc, atom_desc;
  ...
  while (cb_idx < cbs_in_tb) {
    desc = q->ring_addr +
           ((q->sw_ring_tail + dequeued_cbs) & q->sw_ring_wrap_mask);
    atom_desc.atom_hdr = __atomic_load_n((uint64_t *)desc, __ATOMIC_RELAXED);
    rsp.val = atom_desc.rsp.val;
    rte_bbdev_log_debug("Resp. desc %p: %x", desc, rsp.val);
    ...
    op->turbo_dec.iter_count =
        RTE_MAX((uint8_t)rsp.iter_cnt, op->turbo_dec.iter_count);
...
static uint16_t acc100_dequeue_enc(struct rte_bbdev_queue_data *q_data,
                                   struct rte_bbdev_enc_op **ops,
                                   uint16_t num) {
  struct acc100_queue *q = q_data->queue_private;
  ...
  for (i = 0; i < dequeue_num; ++i) {
    op = (q->ring_addr +
          ((q->sw_ring_tail + dequeued_cbs) & q->sw_ring_wrap_mask))
             ->req.op_addr;
    if (op->turbo_enc.code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK)
      ret = dequeue_enc_one_op_tb(q, &ops[i], dequeued_cbs, &aq_dequeued);
    else
      ret = dequeue_enc_one_op_cb(q, &ops[i], dequeued_cbs, &aq_dequeued);
...
static uint16_t acc100_dequeue_ldpc_enc(struct rte_bbdev_queue_data *q_data,
                                        struct rte_bbdev_enc_op **ops,
                                        uint16_t num) {
  struct acc100_queue *q = q_data->queue_private;
  uint32_t avail = q->sw_ring_head - q->sw_ring_tail;
  ...
  for (i = 0; i < dequeue_num; i++) {
    ret = dequeue_enc_one_op_cb(q, &ops[dequeued_cbs], dequeued_descs,
                                &aq_dequeued);
    if (ret < 0)
      break;
    dequeued_cbs += ret;
...
static uint16_t acc100_dequeue_dec(struct rte_bbdev_queue_data *q_data,
                                   struct rte_bbdev_dec_op **ops,
                                   uint16_t num) {
  struct acc100_queue *q = q_data->queue_private;
  uint16_t dequeue_num;
  ...
  for (i = 0; i < dequeue_num; ++i) {
    op = (q->ring_addr +
          ((q->sw_ring_tail + dequeued_cbs) & q->sw_ring_wrap_mask))
             ->req.op_addr;
    if (op->turbo_dec.code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK)
      ret = dequeue_dec_one_op_tb(q, &ops[i], dequeued_cbs, &aq_dequeued);
    else
      ret =
          dequeue_dec_one_op_cb(q_data, q, &ops[i], dequeued_cbs, &aq_dequeued);
...
/* Dequeue decode operations from ACC100 device. */
static uint16_t acc100_dequeue_ldpc_dec(struct rte_bbdev_queue_data *q_data,
                                        struct rte_bbdev_dec_op **ops,
                                        uint16_t num) {
  struct acc100_queue *q = q_data->queue_private;
  uint16_t dequeue_num;
  ...
  for (i = 0; i < dequeue_num; ++i) {
    ...
    if (op->ldpc_dec.code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK)
      ...
      ret = dequeue_ldpc_dec_one_op_cb(q_data, q, &ops[i], dequeued_cbs,
                                       &aq_dequeued);
...
static void acc100_bbdev_init(struct rte_bbdev *dev,
...
static int acc100_pci_probe(struct rte_pci_driver *pci_drv,
  ...
  bbdev->data->dev_private =
      rte_zmalloc_socket(dev_name, sizeof(struct acc100_device),
                         RTE_CACHE_LINE_SIZE, pci_dev->device.numa_node);

  if (bbdev->data->dev_private == NULL) {
    rte_bbdev_log(CRIT, "Allocate of %zu bytes for device \"%s\" failed",
  ...
  rte_bbdev_log_debug("Initialised bbdev %s (id = %u)", dev_name,
                      bbdev->data->dev_id);
  return 0;
}
...
static int acc100_pci_remove(struct rte_pci_device *pci_dev) {
  ...
  if (bbdev == NULL) {
    rte_bbdev_log(CRIT, "Couldn't find HW dev \"%s\" to uninitialise it",
                  pci_dev->device.name);
    return -ENODEV;
  }
  ...
  if (ret < 0)
    rte_bbdev_log(ERR, "Device %i failed to close during uninit: %i", dev_id,
                  ret);
 ...
 * defined.
 */
static void poweron_cleanup(struct rte_bbdev *bbdev, struct acc100_device *d,
  ...
  for (i = 0; i < ACC100_ENGINES_MAX; i++)
    acc100_reg_write(d, HWPfFecUl5gCntrlReg + ACC100_ENGINE_OFFSET * i,
                     ACC100_RESET_HI);
  usleep(ACC100_LONG_WAIT);
  for (i = 0; i < ACC100_ENGINES_MAX; i++)
    acc100_reg_write(d, HWPfFecUl5gCntrlReg + ACC100_ENGINE_OFFSET * i,
  ...
  uint32_t phys_low = (uint32_t)(d->sw_rings_iova & ~(ACC100_SIZE_64MBYTE - 1));
  ...
  union acc100_dma_desc *desc = NULL;
  desc = d->sw_rings;
  desc->req.data_ptrs[0].address = d->sw_rings_iova + ACC100_DESC_FCW_OFFSET;
  desc->req.data_ptrs[0].blen = ACC100_FCW_LD_BLEN;
  desc->req.data_ptrs[0].blkid = ACC100_DMA_BLKID_FCW;
  desc->req.data_ptrs[0].last = 0;
  desc->req.data_ptrs[0].dma_ext = 0;
  ...
  for (template_idx = ACC100_SIG_UL_5G; template_idx <= ACC100_SIG_UL_5G_LAST;
    ...
    address = HwPfFecUl5gIbDebugReg + ACC100_ENGINE_OFFSET * template_idx;
  ...
  for (i = 0; i < num_failed_engine; i++) {
    ...
    for (template_idx = ACC100_SIG_UL_5G; template_idx <= ACC100_SIG_UL_5G_LAST;
         template_idx++) {
      address =
          HWPfQmgrGrpTmplateReg4Indx + ACC100_BYTES_IN_WORD * template_idx;
  ...
  for (template_idx = ACC100_SIG_UL_5G; template_idx <= ACC100_SIG_UL_5G_LAST;
    ...
    address = HwPfFecUl5gIbDebugReg + ACC100_ENGINE_OFFSET * template_idx;
    status = (acc100_reg_read(d, address) >> 4) & 0xF;
    address = HWPfQmgrGrpTmplateReg4Indx + ACC100_BYTES_IN_WORD * template_idx;
    if (status == 1) {
      acc100_reg_write(d, address, value);
      numEngines++;
    } else
...
int rte_acc100_configure(const char *dev_name, struct rte_acc100_conf *conf) {
  ...
  for (i = 1; i < ACC100_GPEX_AXIMAP_NUM; i++)
    acc100_reg_write(d, HwPfPcieGpexAxiAddrMappingWindowPexBaseHigh + i * 16,
                     0);
  ...
  int totalQgs = conf->q_ul_4g.num_qgroups + conf->q_ul_5g.num_qgroups +
                 conf->q_dl_4g.num_qgroups + conf->q_dl_5g.num_qgroups;
  for (qg_idx = 0; qg_idx < totalQgs; qg_idx++) {
    address = HWPfQmgrDepthLog2Grp + ACC100_BYTES_IN_WORD * qg_idx;
    value = aqDepth(qg_idx, conf);
    acc100_reg_write(d, address, value);
    address = HWPfQmgrTholdGrp + ACC100_BYTES_IN_WORD * qg_idx;
  ...
  for (template_idx = 0; template_idx < ACC100_NUM_TMPL; template_idx++) {
    address =
        HWPfQmgrGrpTmplateReg0Indx + ACC100_BYTES_IN_WORD * (template_idx % 8);
    value = ACC100_TMPL_PRI_0;
    acc100_reg_write(d, address, value);
    address =
        HWPfQmgrGrpTmplateReg1Indx + ACC100_BYTES_IN_WORD * (template_idx % 8);
    value = ACC100_TMPL_PRI_1;
    acc100_reg_write(d, address, value);
    address =
        HWPfQmgrGrpTmplateReg2indx + ACC100_BYTES_IN_WORD * (template_idx % 8);
    value = ACC100_TMPL_PRI_2;
    acc100_reg_write(d, address, value);
    address =
        HWPfQmgrGrpTmplateReg3Indx + ACC100_BYTES_IN_WORD * (template_idx % 8);
  ...
  for (qg_idx = numQqsAcc; qg_idx < (numQgs + numQqsAcc); qg_idx++)
    value |= (1 << qg_idx);
  for (template_idx = ACC100_SIG_UL_4G; template_idx <= ACC100_SIG_UL_4G_LAST;
       template_idx++) {
    address = HWPfQmgrGrpTmplateReg4Indx + ACC100_BYTES_IN_WORD * template_idx;
    acc100_reg_write(d, address, value);
  }
  ...
  for (qg_idx = numQqsAcc; qg_idx < (numQgs + numQqsAcc); qg_idx++)
    value |= (1 << qg_idx);
  for (template_idx = ACC100_SIG_DL_4G; template_idx <= ACC100_SIG_DL_4G_LAST;
       template_idx++) {
    address = HWPfQmgrGrpTmplateReg4Indx + ACC100_BYTES_IN_WORD * template_idx;
    acc100_reg_write(d, address, value);
#if RTE_ACC100_SINGLE_FEC == 1
  ...
  for (qg_idx = numQqsAcc; qg_idx < (numQgs + numQqsAcc); qg_idx++)
    value |= (1 << qg_idx);
  for (template_idx = ACC100_SIG_DL_5G; template_idx <= ACC100_SIG_DL_5G_LAST;
       template_idx++) {
    address = HWPfQmgrGrpTmplateReg4Indx + ACC100_BYTES_IN_WORD * template_idx;
    acc100_reg_write(d, address, value);
#if RTE_ACC100_SINGLE_FEC == 1
  ...
  for (qg_idx = 0; qg_idx < totalQgs; qg_idx++) {
    address = HWPfQmgrArbQDepthGrp + ACC100_BYTES_IN_WORD * qg_idx;
    value = 0;
    acc100_reg_write(d, address, value);
  }
  ...
  for (vf_idx = 0; vf_idx < ACC100_NUM_VFS; vf_idx++) {
    for (qg_idx = 0; qg_idx < ACC100_NUM_QGRPS; qg_idx++) {
      ...
      address = HWPfQmgrAqEnableVf + vf_idx * ACC100_BYTES_IN_WORD;
      value += (qg_idx << 16);
      acc100_reg_write(d, address, value);
    }
  }
  ...
  for (qg_idx = 0; qg_idx < totalQgs; qg_idx++) {
    for (vf_idx = 0; vf_idx < conf->num_vf_bundles; vf_idx++) {
      address = HWPfQmgrVfBaseAddr + vf_idx * ACC100_BYTES_IN_WORD +
  ...
  for (vf_idx = 0; vf_idx < conf->num_vf_bundles; vf_idx++) {
    address = HWPfDmaVfDdrBaseRw + vf_idx * 0x10;
    value = ((vf_idx * (ddrSizeInMb / 64)) << 16) + (ddrSizeInMb - 1);
    acc100_reg_write(d, address, value);
  }
  usleep(ACC100_LONG_WAIT);
