...
static const struct rte_bbdev_op_cap bbdev_capabilities[] = {
    ...
    {.type = RTE_BBDEV_OP_LDPC_DEC,
     .cap.ldpc_dec =
         {
             .capability_flags = RTE_BBDEV_LDPC_CRC_TYPE_24A_CHECK |
...
/* Get device info */
static void la12xx_info_get(struct rte_bbdev *dev __rte_unused,
...
/* Release queue */
static int la12xx_queue_release(struct rte_bbdev *dev, uint16_t q_id) {
  RTE_SET_USED(dev);
  RTE_SET_USED(q_id);
...
static int ipc_queue_configure(uint32_t channel_id, ipc_t instance,
  ...
  rte_bbdev_log_debug("%x %p", ipc_instance->initialized, ipc_priv->instance);
  ch = &(ipc_instance->ch_list[channel_id]);

  rte_bbdev_log_debug("channel: %u, depth: %u, msg size: %u", channel_id,
                      q_priv->queue_size, msg_size);
  ...
  for (i = 0; i < q_priv->queue_size; i++) {
    ...
    /* Only offset now */
    ch->bd_h[i].modem_ptr = rte_cpu_to_be_32(HUGEPG_OFFSET(vaddr));
  ...
  ch->host_ipc_params = rte_cpu_to_be_32(HUGEPG_OFFSET(q_priv->host_params));

  rte_bbdev_log_debug("Channel configured");
  return IPC_SUCCESS;
}

static int la12xx_e200_queue_setup(struct rte_bbdev *dev,
                                   struct bbdev_la12xx_q_priv *q_priv) {
  ...
  if (ret) {
    rte_bbdev_log(ERR, "Unable to setup queue (%d) (err=%d)", q_priv->q_id,
                  ret);
    return ret;
  }
  ...
  case RTE_BBDEV_OP_LDPC_ENC:
    if (priv->num_ldpc_enc_queues >= LA12XX_MAX_LDPC_ENC_QUEUES) {
      rte_bbdev_log(ERR, "num_ldpc_enc_queues reached max value");
      return -1;
    }
    ch->la12xx_core_id = rte_cpu_to_be_32(LA12XX_LDPC_ENC_CORE);
    ch->feca_blk_id = rte_cpu_to_be_32(priv->num_ldpc_enc_queues++);
    break;
  case RTE_BBDEV_OP_LDPC_DEC:
    if (priv->num_ldpc_dec_queues >= LA12XX_MAX_LDPC_DEC_QUEUES) {
      rte_bbdev_log(ERR, "num_ldpc_dec_queues reached max value");
      return -1;
    }
    ch->la12xx_core_id = rte_cpu_to_be_32(LA12XX_LDPC_DEC_CORE);
...
/* Setup a queue */
static int la12xx_queue_setup(struct rte_bbdev *dev, uint16_t q_id,
  ...
  q_data->queue_private =
      rte_zmalloc(NULL, sizeof(struct bbdev_la12xx_q_priv), 0);
  ...
  if (ret) {
    rte_bbdev_log(ERR, "e200_queue_setup failed for qid: %d", q_id);
    return ret;
  }
...
static int la12xx_start(struct rte_bbdev *dev) {
  struct bbdev_la12xx_private *priv = dev->data->dev_private;
  ipc_userspace_t *ipc_priv = priv->ipc_priv;
  int ready = 0;
...
static const struct rte_bbdev_ops pmd_ops = {.info_get = la12xx_info_get,
                                             .queue_setup = la12xx_queue_setup,
                                             .queue_release =
                                                 la12xx_queue_release,
                                             .start = la12xx_start};

static inline int is_bd_ring_full(uint32_t ci, uint32_t ci_flag, uint32_t pi,
                                  uint32_t pi_flag) {
  if (pi == ci) {
    if (pi_flag != ci_flag)
...
prepare_ldpc_enc_op(struct rte_bbdev_enc_op *bbdev_enc_op,
  ...
  total_out_bits = (ldpc_enc->tb_params.cab * ldpc_enc->tb_params.ea) +
                   (ldpc_enc->tb_params.c - ldpc_enc->tb_params.cab) *
                       ldpc_enc->tb_params.eb;
...
prepare_ldpc_dec_op(struct rte_bbdev_dec_op *bbdev_dec_op,
  ...
  if (ldpc_dec->tb_params.c == 1) {
    total_out_bits = ((sys_cols * ldpc_dec->z_c) - ldpc_dec->n_filler);
    ...
    total_out_bits = (((sys_cols * ldpc_dec->z_c) - ldpc_dec->n_filler - 24) *
...
static int enqueue_single_op(struct bbdev_la12xx_q_priv *q_priv,
                             void *bbdev_op) {
  ...
  case RTE_BBDEV_OP_LDPC_ENC:
    ...
    ret = prepare_ldpc_enc_op(bbdev_op, q_priv, in_op_data, out_op_data);
    if (ret) {
      rte_bbdev_log(ERR, "process_ldpc_enc_op fail, ret: %d", ret);
      return ret;
    }
    break;
  ...
  case RTE_BBDEV_OP_LDPC_DEC:
    ...
    ret = prepare_ldpc_dec_op(bbdev_op, bbdev_ipc_op, q_priv, out_op_data);
    if (ret) {
      rte_bbdev_log(ERR, "process_ldpc_dec_op fail, ret: %d", ret);
      return ret;
    }
    break;
  ...
  if (in_op_data->data) {
    ...
    l1_pcie_addr =
        (uint32_t)GUL_USER_HUGE_PAGE_ADDR + data_ptr - huge_start_addr;
...
static void *dequeue_single_op(struct bbdev_la12xx_q_priv *q_priv, void *dst) {
  void *op;
  uint32_t ci, ci_flag;
  uint32_t temp_ci;
  ...
  rte_bbdev_dp_log(DEBUG, "ci: %u, ci_flag: %u, ring size: %u", ci, ci_flag,
                   q_priv->queue_size);
  ...
  rte_bbdev_dp_log(DEBUG, "exit: ci: %u, ci_flag: %u, ring size: %u", ci,
                   ci_flag, q_priv->queue_size);
...
static struct hugepage_info *get_hugepage_info(void) {
...
static int open_ipc_dev(int modem_id) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    if (!strncmp(dev_initials, entry->d_name, sizeof(dev_initials) - 1))
      break;
  }
...
static int setup_la12xx_dev(struct rte_bbdev *dev) {
  struct bbdev_la12xx_private *priv = dev->data->dev_private;
  ipc_userspace_t *ipc_priv = priv->ipc_priv;
  ...
  if (!ipc_priv) {
    ...
    rte_bbdev_log_debug("0x%" PRIx64 " %p 0x%" PRIx64, hp->paddr, hp->vaddr,
                        hp->len);

    ipc_priv = rte_zmalloc(0, sizeof(ipc_userspace_t), 0);
    if (ipc_priv == NULL) {
      rte_bbdev_log(ERR, "Unable to allocate memory for ipc priv");
      ret = -ENOMEM;
      goto err;
    }
    ...
    for (i = 0; i < IPC_MAX_CHANNEL_COUNT; i++) {
      ...
      if (ipc_priv_ch == NULL) {
        rte_bbdev_log(ERR, "Unable to allocate memory for channels");
        ret = -ENOMEM;
      }
    ...
    rte_bbdev_log_debug("hugepg input 0x%" PRIx64 "%p 0x%" PRIx64, hp->paddr,
                        hp->vaddr, hp->len);
  ...
  ret = ioctl(ipc_priv->dev_ipc, IOCTL_GUL_IPC_GET_SYS_MAP, &ipc_priv->sys_map);
  if (ret) {
    rte_bbdev_log(ERR, "IOCTL_GUL_IPC_GET_SYS_MAP ioctl failed");
    goto err;
  }
  ...
  ipc_priv->mhif_start.host_vaddr =
      (void *)((uintptr_t)(ipc_priv->mhif_start.host_vaddr) + phy_align);
  ...
  ipc_priv->peb_start.host_vaddr =
      (void *)((uintptr_t)(ipc_priv->peb_start.host_vaddr) + phy_align);
  ...
  ipc_priv->modem_ccsrbar.host_vaddr =
      (void *)((uintptr_t)(ipc_priv->modem_ccsrbar.host_vaddr) + phy_align);
  ...
  rte_bbdev_log(INFO, "peb 0x%" PRIx64 "%p 0x%" PRIx32,
                ipc_priv->peb_start.host_phys, ipc_priv->peb_start.host_vaddr,
                ipc_priv->peb_start.size);
  rte_bbdev_log(INFO, "hugepg 0x%" PRIx64 "%p 0x%" PRIx32,
                ipc_priv->hugepg_start.host_phys,
                ipc_priv->hugepg_start.host_vaddr, ipc_priv->hugepg_start.size);
  rte_bbdev_log(INFO, "mhif 0x%" PRIx64 "%p 0x%" PRIx32,
                ipc_priv->mhif_start.host_phys, ipc_priv->mhif_start.host_vaddr,
  ...
  if (sizeof(ipc_metadata_t) != mhif->ipc_regs.ipc_mdata_size) {
    ...
    rte_bbdev_log(ERR, "--> mhif->ipc_regs.ipc_mdata_offset= 0x%" PRIx32,
                  mhif->ipc_regs.ipc_mdata_offset);
  ...
  ipc_priv->instance =
      (ipc_instance_t *)(&ipc_md->instance_list[ipc_priv->instance_id]);
...
static inline int parse_u16_arg(const char *key, const char *value,
  ...
  if ((result >= (1 << 16)) || (errno != 0)) {
    rte_bbdev_log(ERR, "Invalid value %" PRIu64 " for %s", result, key);
    return -ERANGE;
  }
  *u16 = (uint16_t)result;
  return 0;
...
static int parse_integer_arg(const char *key __rte_unused, const char *value,
  ...
  if (*end != 0 || errno != 0 || i < 0 || i > LA12XX_MAX_MODEM) {
    rte_bbdev_log(ERR, "Supported Port IDS are 0 to %d", LA12XX_MAX_MODEM - 1);
    return -EINVAL;
  }
...
static int parse_bbdev_la12xx_params(struct bbdev_la12xx_params *params,
  ...
  if (params == NULL)
    return -EINVAL;
  if (input_args) {
    kvlist = rte_kvargs_parse(input_args, bbdev_la12xx_valid_params);
    if (kvlist == NULL)
      return -EFAULT;
    ...
    ret = rte_kvargs_process(kvlist, bbdev_la12xx_valid_params[1],
                             &parse_integer_arg, &params->modem_id);

    if (params->modem_id >= LA12XX_MAX_MODEM) {
      rte_bbdev_log(ERR, "Invalid modem id, must be < %u", LA12XX_MAX_MODEM);
      goto exit;
    }
  }
...
/* Create device */
static int la12xx_bbdev_create(struct rte_vdev_device *vdev,
  ...
  bbdev->data->dev_private = rte_zmalloc(
      name, sizeof(struct bbdev_la12xx_private), RTE_CACHE_LINE_SIZE);
  ...
  rte_bbdev_log(INFO, "Setting Up %s: DevId=%d, ModemId=%d", name,
                bbdev->data->dev_id, priv->modem_id);
...
/* Initialise device */
static int la12xx_bbdev_probe(struct rte_vdev_device *vdev) {
  struct bbdev_la12xx_params init_params = {
      8,
      -1,
  };
...
/* Uninitialise device */
static int la12xx_bbdev_remove(struct rte_vdev_device *vdev) {
  struct rte_bbdev *bbdev;
  const char *name;
...
static struct rte_vdev_driver bbdev_la12xx_pmd_drv = {
    .probe = la12xx_bbdev_probe, .remove = la12xx_bbdev_remove};

RTE_PMD_REGISTER_VDEV(DRIVER_NAME, bbdev_la12xx_pmd_drv);
RTE_PMD_REGISTER_PARAM_STRING(DRIVER_NAME, LA12XX_MAX_NB_QUEUES_ARG
                              "=<int>" LA12XX_VDEV_MODEM_ID_ARG "=<int> ");
