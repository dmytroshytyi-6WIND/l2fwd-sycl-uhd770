...
enum { SSO_SYNC_ORDERED, SSO_SYNC_ATOMIC, SSO_SYNC_UNTAGGED, SSO_SYNC_EMPTY };
...
ssovf_octeontx_wqe_xtract_mseg(octtx_wqe_t *wqe, struct rte_mbuf *mbuf) {
...
ssovf_octeontx_wqe_to_pkt(uint64_t work, uint16_t port_info,
  ...
  if (!!(flag & OCCTX_RX_OFFLOAD_CSUM_F))
    mbuf->ol_flags = ssovf_octeontx_rx_olflags_get(lookup_mem, wqe->w[2]);
  ...
  if (!!(flag & OCCTX_RX_VLAN_FLTR_F)) {
    if (likely(wqe->s.w2.vv)) {
      ...
      mbuf->vlan_tci =
          ntohs(*((uint16_t *)((char *)mbuf->buf_addr + mbuf->data_off +
                               wqe->s.w4.vlptr + 2)));
    }
  }
...
static __rte_always_inline void ssovf_octeontx_wqe_free(uint64_t work) {
  octtx_wqe_t *wqe = (octtx_wqe_t *)(uintptr_t)work;
...
static __rte_always_inline uint16_t ssows_get_work(struct ssows *ws,
                                                   struct rte_event *ev,
                                                   const uint16_t flag) {
  ...
  if (get_work1) {
    if (ev->event_type == RTE_EVENT_TYPE_ETHDEV)
      get_work1 = (uintptr_t)ssovf_octeontx_wqe_to_pkt(
          get_work1, (ev->event >> 20) & 0x7F, flag, ws->lookup_mem);
...
ssows_swtag_full(struct ssows *ws, const uint64_t event_ptr, const uint32_t tag,
  ...
  swtag_full0 = tag | ((uint64_t)(new_tt & 0x3) << 32) | ((uint64_t)grp << 34);
  ssovf_store_pair(swtag_full0, event_ptr,
                   (ws->base + SSOW_VHWS_OP_SWTAG_FULL0));
}

static __rte_always_inline void ssows_swtag_desched(struct ssows *ws,
                                                    uint32_t tag,
                                                    uint8_t new_tt,
                                                    uint8_t grp) {
  uint64_t val;
...
static __rte_always_inline void ssows_swtag_norm(struct ssows *ws, uint32_t tag,
                                                 uint8_t new_tt) {
  uint64_t val;
...
static __rte_always_inline void ssows_swtag_untag(struct ssows *ws) {
  ssovf_write64(0, ws->base + SSOW_VHWS_OP_SWTAG_UNTAG);
  ws->cur_tt = SSO_SYNC_UNTAGGED;
}

static __rte_always_inline void ssows_upd_wqp(struct ssows *ws, uint8_t grp,
                                              uint64_t event_ptr) {
  ssovf_store_pair((uint64_t)grp << 34, event_ptr,
                   (ws->base + SSOW_VHWS_OP_UPD_WQP_GRP0));
}

static __rte_always_inline void ssows_desched(struct ssows *ws) {
  ssovf_write64(0, ws->base + SSOW_VHWS_OP_DESCHED);
}

static __rte_always_inline void ssows_swtag_wait(struct ssows *ws) {
  /* Wait for the SWTAG/SWTAG_FULL operation */
  while (ssovf_read64(ws->base + SSOW_VHWS_SWTP))
    ;
}

static __rte_always_inline void ssows_head_wait(struct ssows *ws) {
