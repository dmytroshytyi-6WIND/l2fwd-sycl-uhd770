...
int ssovf_info(struct ssovf_info *info) {
  ...
  for (i = 0; i < sdev.total_ssovfs; i++) {
    ...
    if (sdev.grp[i].vfid != i || sdev.grp[i].bar0 == NULL ||
        sdev.grp[i].domain != domain) {
      mbox_log_err("GRP error, vfid=%d/%d domain=%d/%d %p", i, sdev.grp[i].vfid,
                   domain, sdev.grp[i].domain, sdev.grp[i].bar0);
      return -EINVAL;
    }
  }
  ...
  for (i = 0; i < sdev.total_ssowvfs; i++) {
    /* Check vfid's are contiguous and belong to same domain */
    if (sdev.hws[i].vfid != i || sdev.hws[i].bar0 == NULL ||
        sdev.hws[i].domain != domain) {
      mbox_log_err("HWS error, vfid=%d/%d domain=%d/%d %p", i, sdev.hws[i].vfid,
                   domain, sdev.hws[i].domain, sdev.hws[i].bar0);
      return -EINVAL;
    }
  }
...
void *ssovf_bar(enum ssovf_type type, uint8_t id, uint8_t bar) {
  if (rte_eal_process_type() != RTE_PROC_PRIMARY || type > OCTEONTX_SSO_HWS)
    return NULL;
...
static int ssowvf_probe(struct rte_pci_driver *pci_drv,
  ...
  if (pci_dev->mem_resource[0].addr == NULL ||
    ...
    mbox_log_err("Empty bars %p %p %p", pci_dev->mem_resource[0].addr,
                 pci_dev->mem_resource[2].addr, pci_dev->mem_resource[4].addr);
    return -ENODEV;
  }

  if (pci_dev->mem_resource[4].len != SSOW_BAR4_LEN) {
    mbox_log_err("Bar4 len mismatch %d != %d", SSOW_BAR4_LEN,
                 (int)pci_dev->mem_resource[4].len);
    return -EINVAL;
  }
  ...
  if (vfid == 0) {
    ...
    if (octeontx_mbox_set_ram_mbox_base(ram_mbox_base, res->domain)) {
  ...
  rte_wmb();
  mbox_log_dbg("Domain=%d hws=%d total_ssowvfs=%d", res->domain, res->vfid,
               sdev.total_ssowvfs);
  return 0;
}

static const struct rte_pci_id pci_ssowvf_map[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CAVIUM, PCI_DEVICE_ID_OCTEONTX_SSOWS_VF)},
    {
        .vendor_id = 0,
    },
};
...
static int ssovf_probe(struct rte_pci_driver *pci_drv,
                       struct rte_pci_device *pci_dev) {
  uint64_t val;
  uint16_t vfid;
  uint8_t *idreg;
  ...
  if (pci_dev->mem_resource[0].addr == NULL ||
      pci_dev->mem_resource[2].addr == NULL) {
    mbox_log_err("Empty bars %p %p", pci_dev->mem_resource[0].addr,
  ...
  rte_wmb();
  mbox_log_dbg("Domain=%d group=%d total_ssovfs=%d", res->domain, res->vfid,
               sdev.total_ssovfs);
  return 0;
}

static const struct rte_pci_id pci_ssovf_map[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CAVIUM, PCI_DEVICE_ID_OCTEONTX_SSOGRP_VF)},
    {
        .vendor_id = 0,
    },
};
