...
#define timvf_log(level, fmt, args...)                                         \
  rte_log(RTE_LOG_##level, otx_logtype_timvf, "[%s] %s() " fmt "\n",           \
...
typedef struct tim_mem_entry *(*refill_chunk)(struct tim_mem_bucket *const bkt,
...
static __rte_always_inline uint32_t bkt_mod(const uint32_t rel_bkt,
                                            const uint32_t nb_bkts) {
  return rel_bkt % nb_bkts;
}

static __rte_always_inline uint32_t bkt_and(uint32_t rel_bkt,
                                            uint32_t nb_bkts) {
