...
static inline void seqn_list_init(void) {
  RTE_BUILD_BUG_ON(NUM_PACKETS < MAX_EVENTS);
  memset(seqn_list, 0, sizeof(seqn_list));
  seqn_list_index = 0;
}

static inline int seqn_list_update(int val) {
...
static inline int seqn_list_check(int limit) {
  int i;
...
static int testsuite_setup(void) {
  ...
  if (evdev < 0) {
    ssovf_log_dbg("%d: Eventdev %s not found - creating.", __LINE__,
                  eventdev_name);
    if (rte_vdev_init(eventdev_name, NULL) < 0) {
      ssovf_log_dbg("Error creating eventdev %s", eventdev_name);
      return -1;
    }
    evdev = rte_event_dev_get_dev_id(eventdev_name);
    if (evdev < 0) {
...
static void testsuite_teardown(void) { rte_event_dev_close(evdev); }
...
static inline int _eventdev_setup(int mode) {
  int i, ret;
  ...
  eventdev_test_mempool =
      rte_pktmbuf_pool_create(pool_name, MAX_EVENTS, 0 /*MBUF_CACHE_SIZE*/, 0,
  ...
  if (mode == TEST_EVENTDEV_SETUP_PRIORITY) {
    if (queue_count > 8) {
      ssovf_log_dbg("test expects the unique priority per queue");
      return -ENOTSUP;
    }
     ...
     */
    uint8_t step = (RTE_EVENT_DEV_PRIORITY_LOWEST + 1) / queue_count;
    for (i = 0; i < (int)queue_count; i++) {
      struct rte_event_queue_conf queue_conf;

      ret = rte_event_queue_default_conf_get(evdev, i, &queue_conf);
      RTE_TEST_ASSERT_SUCCESS(ret, "Failed to get def_conf%d", i);
  ...
  RTE_TEST_ASSERT_SUCCESS(
      rte_event_dev_attr_get(evdev, RTE_EVENT_DEV_ATTR_PORT_COUNT, &port_count),
  ...
  for (i = 0; i < (int)port_count; i++) {
    ...
    RTE_TEST_ASSERT(ret >= 0, "Failed to link all queues port=%d", i);
  }
...
static inline int eventdev_setup_priority(void) {
  return _eventdev_setup(TEST_EVENTDEV_SETUP_PRIORITY);
}

static inline int eventdev_setup_dequeue_timeout(void) {
  return _eventdev_setup(TEST_EVENTDEV_SETUP_DEQUEUE_TIMEOUT);
}

static inline void eventdev_teardown(void) {
...
static inline int inject_events(uint32_t flow_id, uint8_t event_type,
                                uint8_t sub_event_type, uint8_t sched_type,
                                uint8_t queue, uint8_t port,
                                unsigned int events) {
  struct rte_mbuf *m;
...
static inline int check_excess_events(uint8_t port) {
  int i;
  ...
  for (i = 0; i < 32; i++) {
    ...
    RTE_TEST_ASSERT_SUCCESS(valid_event, "Unexpected valid event=%d",
                            *rte_event_pmd_selftest_seqn(ev.mbuf));
  }
  return 0;
}

static inline int generate_random_events(const unsigned int total_events) {
  struct rte_event_dev_info info;
  unsigned int i;
  int ret;
...
static inline int validate_event(struct rte_event *ev) {
  struct event_attr *attr;
  ...
  RTE_TEST_ASSERT_EQUAL(attr->event_type, ev->event_type,
                        "event_type mismatch enq=%d deq =%d", attr->event_type,
                        ev->event_type);
  ...
  RTE_TEST_ASSERT_EQUAL(attr->sched_type, ev->sched_type,
                        "sched_type mismatch enq=%d deq =%d", attr->sched_type,
                        ev->sched_type);
...
static inline int consume_events(uint8_t port, const uint32_t total_events,
  ...
  while (1) {
    ...
    if (fn != NULL) {
      ...
      RTE_TEST_ASSERT_SUCCESS(ret, "Failed to validate test specific event");
    }
...
static int validate_simple_enqdeq(uint32_t index, uint8_t port,
                                  struct rte_event *ev) {
  RTE_SET_USED(port);
...
static inline int test_simple_enqdeq(uint8_t sched_type) {
  int ret;
...
static int test_simple_enqdeq_ordered(void) {
  return test_simple_enqdeq(RTE_SCHED_TYPE_ORDERED);
}

static int test_simple_enqdeq_atomic(void) {
  return test_simple_enqdeq(RTE_SCHED_TYPE_ATOMIC);
}

static int test_simple_enqdeq_parallel(void) {
 ...
 * event attributes
 */
static int test_multi_queue_enq_single_port_deq(void) {
  int ret;
 ...
 */
static int validate_queue_priority(uint32_t index, uint8_t port,
                                   struct rte_event *ev) {
  uint32_t queue_count;
  RTE_TEST_ASSERT_SUCCESS(rte_event_dev_attr_get(evdev,
                                                 RTE_EVENT_DEV_ATTR_QUEUE_COUNT,
...
static int test_multi_queue_priority(void) {
  uint8_t queue;
  struct rte_mbuf *m;
  ...
  for (i = 0; i < max_evts_roundoff; i++) {
    ...
    *rte_event_pmd_selftest_seqn(m) = i;
    queue = i % queue_count;
    update_event_and_validation_attr(m, &ev, 0, RTE_EVENT_TYPE_CPU, 0,
                                     RTE_SCHED_TYPE_PARALLEL, queue, 0);
...
static int worker_multi_port_fn(void *arg) {
...
static inline int wait_workers_to_join(int lcore, const rte_atomic32_t *count) {
  ...
  while (rte_eal_get_lcore_state(lcore) != WAIT) {
    ...
    if (new_cycles - print_cycles > rte_get_timer_hz()) {
      ssovf_log_dbg("\r%s: events %d", __func__, rte_atomic32_read(count));
      print_cycles = new_cycles;
    }
    if (new_cycles - cycles > rte_get_timer_hz() * 10) {
      ssovf_log_dbg("%s: No schedules for seconds, deadlock (%d)", __func__,
...
static inline int launch_workers_and_wait(int (*main_worker)(void *),
                                          int (*worker)(void *),
  ...
  ret = rte_event_dequeue_timeout_ticks(evdev, rte_rand() % 10000000 /* 10ms */,
                                        &dequeue_tmo_ticks);
  if (ret) {
    free(param);
    return -1;
 ...
 * event attributes
 */
static int test_multi_queue_enq_multi_port_deq(void) {
  ...
  RTE_TEST_ASSERT_SUCCESS(
      rte_event_dev_attr_get(evdev, RTE_EVENT_DEV_ATTR_PORT_COUNT, &nr_ports),
      "Port count get failed");
  nr_ports = RTE_MIN(nr_ports, rte_lcore_count() - 1);

  if (!nr_ports) {
    ssovf_log_dbg("%s: Not enough ports=%d or workers=%d", __func__, nr_ports,
                  rte_lcore_count() - 1);
    return 0;
  }

  return launch_workers_and_wait(worker_multi_port_fn, worker_multi_port_fn,
                                 total_events, nr_ports, 0xff /* invalid */);
}

static void flush(uint8_t dev_id, struct rte_event event, void *arg) {
...
static int test_dev_stop_flush(void) {
  ...
  RTE_TEST_ASSERT_EQUAL(total_events, count,
                        "count mismatch total_events=%d count=%d", total_events,
                        count);
  return 0;
}

static int validate_queue_to_port_single_link(uint32_t index, uint8_t port,
                                              struct rte_event *ev) {
  RTE_SET_USED(index);
  RTE_TEST_ASSERT_EQUAL(port, ev->queue_id, "queue mismatch enq=%d deq =%d",
                        port, ev->queue_id);
  return 0;
}
 ...
 */
static int test_queue_to_port_single_link(void) {
  int i, nr_links, ret;
  ...
  for (i = 0; i < (int)port_count; i++) {
    ...
    RTE_TEST_ASSERT(ret >= 0, "Failed to unlink all queues port=%d", i);
  }
  ...
  for (i = 0; i < nr_links; i++) {
    ...
    ret = inject_events(0x100 /*flow_id */, RTE_EVENT_TYPE_CPU /* event_type */,
                        rte_rand() % 256 /* sub_event_type */,
                        rte_rand() % (RTE_SCHED_TYPE_PARALLEL + 1),
                        queue /* queue */, i /* port */,
                        total_events /* events */);
...
static int validate_queue_to_port_multi_link(uint32_t index, uint8_t port,
                                             struct rte_event *ev) {
  RTE_SET_USED(index);
 ...
 */
static int test_queue_to_port_multi_link(void) {
  ...
  RTE_TEST_ASSERT_SUCCESS(
      rte_event_dev_attr_get(evdev, RTE_EVENT_DEV_ATTR_QUEUE_COUNT, &nr_queues),
      "Queue count get failed");
  ...
  if (nr_ports < 2) {
    ssovf_log_dbg("%s: Not enough ports to test ports=%d", __func__, nr_ports);
    return 0;
  }
  ...
  for (port = 0; port < nr_ports; port++) {
    ...
    RTE_TEST_ASSERT(ret >= 0, "Failed to unlink all queues port=%d", port);
  }
  ...
  for (queue = 0; queue < nr_queues; queue++) {
    ...
    RTE_TEST_ASSERT(ret == 1, "Failed to link queue=%d to port=%d", queue,
                    port);
...
static int worker_flow_based_pipeline(void *arg) {
  struct test_core_param *param = arg;
  struct rte_event ev;
  uint16_t valid_event;
  uint8_t port = param->port;
  ...
  while (rte_atomic32_read(total_events) > 0) {
    valid_event =
        rte_event_dequeue_burst(evdev, port, &ev, 1, dequeue_tmo_ticks);
    if (!valid_event)
      continue;
    ...
    if (ev.sub_event_type == 0) {
      ...
      ssovf_log_dbg("Invalid ev.sub_event_type = %d", ev.sub_event_type);
      return -1;
    }
  }
  return 0;
...
static int test_multiport_flow_sched_type_test(uint8_t in_sched_type,
  ...
  ret = inject_events(0x1 /*flow_id */, RTE_EVENT_TYPE_CPU /* event_type */,
                      0 /* sub_event_type (stage 0) */, in_sched_type,
                      0 /* queue */, 0 /* port */, total_events /* events */);
  if (ret)
    return -1;
...
static int test_multi_port_flow_ordered_to_atomic(void) {
  /* Ingress event order test */
  return test_multiport_flow_sched_type_test(RTE_SCHED_TYPE_ORDERED,
                                             RTE_SCHED_TYPE_ATOMIC);
}

static int test_multi_port_flow_ordered_to_ordered(void) {
  return test_multiport_flow_sched_type_test(RTE_SCHED_TYPE_ORDERED,
                                             RTE_SCHED_TYPE_ORDERED);
}

static int test_multi_port_flow_ordered_to_parallel(void) {
  return test_multiport_flow_sched_type_test(RTE_SCHED_TYPE_ORDERED,
                                             RTE_SCHED_TYPE_PARALLEL);
}

static int test_multi_port_flow_atomic_to_atomic(void) {
  /* Ingress event order test */
  return test_multiport_flow_sched_type_test(RTE_SCHED_TYPE_ATOMIC,
                                             RTE_SCHED_TYPE_ATOMIC);
}

static int test_multi_port_flow_atomic_to_ordered(void) {
  return test_multiport_flow_sched_type_test(RTE_SCHED_TYPE_ATOMIC,
                                             RTE_SCHED_TYPE_ORDERED);
}

static int test_multi_port_flow_atomic_to_parallel(void) {
  return test_multiport_flow_sched_type_test(RTE_SCHED_TYPE_ATOMIC,
                                             RTE_SCHED_TYPE_PARALLEL);
}

static int test_multi_port_flow_parallel_to_atomic(void) {
  return test_multiport_flow_sched_type_test(RTE_SCHED_TYPE_PARALLEL,
                                             RTE_SCHED_TYPE_ATOMIC);
}

static int test_multi_port_flow_parallel_to_ordered(void) {
  return test_multiport_flow_sched_type_test(RTE_SCHED_TYPE_PARALLEL,
                                             RTE_SCHED_TYPE_ORDERED);
}

static int test_multi_port_flow_parallel_to_parallel(void) {
  return test_multiport_flow_sched_type_test(RTE_SCHED_TYPE_PARALLEL,
                                             RTE_SCHED_TYPE_PARALLEL);
}

static int worker_group_based_pipeline(void *arg) {
  struct test_core_param *param = arg;
  struct rte_event ev;
  uint16_t valid_event;
  uint8_t port = param->port;
...
static int test_multiport_queue_sched_type_test(uint8_t in_sched_type,
                                                uint8_t out_sched_type) {
  const unsigned int total_events = MAX_EVENTS;
  uint32_t nr_ports;
  int ret;
  ...
  if (queue_count < 2 || !nr_ports) {
    ssovf_log_dbg("%s: Not enough queues=%d ports=%d or workers=%d", __func__,
...
static int test_multi_port_queue_ordered_to_atomic(void) {
  /* Ingress event order test */
  return test_multiport_queue_sched_type_test(RTE_SCHED_TYPE_ORDERED,
                                              RTE_SCHED_TYPE_ATOMIC);
}

static int test_multi_port_queue_ordered_to_ordered(void) {
  return test_multiport_queue_sched_type_test(RTE_SCHED_TYPE_ORDERED,
                                              RTE_SCHED_TYPE_ORDERED);
}

static int test_multi_port_queue_ordered_to_parallel(void) {
  return test_multiport_queue_sched_type_test(RTE_SCHED_TYPE_ORDERED,
                                              RTE_SCHED_TYPE_PARALLEL);
}

static int test_multi_port_queue_atomic_to_atomic(void) {
  /* Ingress event order test */
  return test_multiport_queue_sched_type_test(RTE_SCHED_TYPE_ATOMIC,
                                              RTE_SCHED_TYPE_ATOMIC);
}

static int test_multi_port_queue_atomic_to_ordered(void) {
  return test_multiport_queue_sched_type_test(RTE_SCHED_TYPE_ATOMIC,
                                              RTE_SCHED_TYPE_ORDERED);
}

static int test_multi_port_queue_atomic_to_parallel(void) {
  return test_multiport_queue_sched_type_test(RTE_SCHED_TYPE_ATOMIC,
                                              RTE_SCHED_TYPE_PARALLEL);
}

static int test_multi_port_queue_parallel_to_atomic(void) {
  return test_multiport_queue_sched_type_test(RTE_SCHED_TYPE_PARALLEL,
                                              RTE_SCHED_TYPE_ATOMIC);
}

static int test_multi_port_queue_parallel_to_ordered(void) {
  return test_multiport_queue_sched_type_test(RTE_SCHED_TYPE_PARALLEL,
                                              RTE_SCHED_TYPE_ORDERED);
}

static int test_multi_port_queue_parallel_to_parallel(void) {
  return test_multiport_queue_sched_type_test(RTE_SCHED_TYPE_PARALLEL,
                                              RTE_SCHED_TYPE_PARALLEL);
}

static int worker_flow_based_pipeline_max_stages_rand_sched_type(void *arg) {
  struct test_core_param *param = arg;
  struct rte_event ev;
  uint16_t valid_event;
  uint8_t port = param->port;
  ...
  while (rte_atomic32_read(total_events) > 0) {
    ...
    if (ev.sub_event_type == 255) { /* last stage */
      ...
      ev.event_type = RTE_EVENT_TYPE_CPU;
      ev.sub_event_type++;
      ev.sched_type = rte_rand() % (RTE_SCHED_TYPE_PARALLEL + 1);
      ev.op = RTE_EVENT_OP_FORWARD;
      rte_event_enqueue_burst(evdev, port, &ev, 1);
    }
  }
...
static int launch_multi_port_max_stages_random_sched_type(int (*fn)(void *)) {
  uint32_t nr_ports;
  int ret;
  ...
  /* Injects events with a 0 sequence number to total_events */
  ret =
      inject_events(0x1 /*flow_id */, RTE_EVENT_TYPE_CPU /* event_type */,
...
static int test_multi_port_flow_max_stages_random_sched_type(void) {
  return launch_multi_port_max_stages_random_sched_type(
      worker_flow_based_pipeline_max_stages_rand_sched_type);
}

static int worker_queue_based_pipeline_max_stages_rand_sched_type(void *arg) {
  struct test_core_param *param = arg;
  struct rte_event ev;
  uint16_t valid_event;
  uint8_t port = param->port;
...
static int test_multi_port_queue_max_stages_random_sched_type(void) {
  return launch_multi_port_max_stages_random_sched_type(
      worker_queue_based_pipeline_max_stages_rand_sched_type);
}

static int worker_mixed_pipeline_max_stages_rand_sched_type(void *arg) {
  struct test_core_param *param = arg;
  struct rte_event ev;
  uint16_t valid_event;
  uint8_t port = param->port;
...
static int test_multi_port_mixed_max_stages_random_sched_type(void) {
  return launch_multi_port_max_stages_random_sched_type(
      worker_mixed_pipeline_max_stages_rand_sched_type);
}

static int worker_ordered_flow_producer(void *arg) {
  struct test_core_param *param = arg;
  uint8_t port = param->port;
  struct rte_mbuf *m;
  int counter = 0;
...
static inline int test_producer_consumer_ingress_order_test(int (*fn)(void *)) {
  uint32_t nr_ports;
...
static int test_flow_producer_consumer_ingress_order_test(void) {
  return test_producer_consumer_ingress_order_test(worker_flow_based_pipeline);
}

/* Queue based producer consumer ingress order test */
static int test_queue_producer_consumer_ingress_order_test(void) {
  return test_producer_consumer_ingress_order_test(worker_group_based_pipeline);
}
...
int test_eventdev_octeontx(void) {
  ...
  OCTEONTX_TEST_RUN(eventdev_setup, eventdev_teardown, test_dev_stop_flush);
  OCTEONTX_TEST_RUN(eventdev_setup, eventdev_teardown,
