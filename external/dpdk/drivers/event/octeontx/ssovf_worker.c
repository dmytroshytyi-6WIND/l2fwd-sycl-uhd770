...
static __rte_always_inline void ssows_new_event(struct ssows *ws,
...
static __rte_always_inline void ssows_fwd_swtag(struct ssows *ws,
                                                const struct rte_event *ev,
                                                const uint8_t grp) {
  ...
  if (unlikely(cur_tt == SSO_SYNC_UNTAGGED)) {
    if (new_tt != SSO_SYNC_UNTAGGED) {
      ssows_swtag_full(ws, ev->u64, tag, new_tt, grp);
    }
  } else {
...
static __rte_always_inline void ssows_fwd_group(struct ssows *ws,
                                                const struct rte_event *ev,
                                                const uint8_t grp) {
  const uint64_t event_ptr = ev->u64;
  const uint32_t tag = (uint32_t)ev->event;
...
static __rte_always_inline void ssows_release_event(struct ssows *ws) {
  if (likely(ws->cur_tt != SSO_SYNC_UNTAGGED))
    ssows_swtag_untag(ws);
}

#define R(name, f2, f1, f0, flags)                                             \
  static uint16_t __rte_noinline __rte_hot ssows_deq_##name(                   \
    ...
    if (ws->swtag_req) {                                                       \
      ...
      return ssows_get_work(ws, ev, flags);                                    \
                                                                               ...
                                                                               \
  static uint16_t __rte_hot ssows_deq_burst_##name(                            \
    ...
    return ssows_deq_##name(port, ev, timeout_ticks);                          \
                                                                               ...
                                                                               \
  static uint16_t __rte_hot ssows_deq_timeout_##name(                          \
      void *port, struct rte_event *ev, uint64_t timeout_ticks) {              \
    struct ssows *ws = port;                                                   \
    uint64_t iter;                                                             \
    uint16_t ret = 1;                                                          \
    ...
    return ret;                                                                \
                                                                               ...
                                                                               \
  static uint16_t __rte_hot ssows_deq_timeout_burst_##name(                    \
      void *port, struct rte_event ev[], uint16_t nb_events,                   \
      uint64_t timeout_ticks) {                                                \
    RTE_SET_USED(nb_events);                                                   \
                                                                               \
...
__rte_always_inline uint16_t __rte_hot ssows_enq(void *port,
                                                 const struct rte_event *ev) {
  struct ssows *ws = port;
  uint16_t ret = 1;
...
uint16_t __rte_hot ssows_enq_burst(void *port, const struct rte_event ev[],
                                   uint16_t nb_events) {
  RTE_SET_USED(nb_events);
  return ssows_enq(port, ev);
}

uint16_t __rte_hot ssows_enq_new_burst(void *port, const struct rte_event ev[],
                                       uint16_t nb_events) {
  uint16_t i;
  struct ssows *ws = port;
...
uint16_t __rte_hot ssows_enq_fwd_burst(void *port, const struct rte_event ev[],
                                       uint16_t nb_events) {
  struct ssows *ws = port;
  RTE_SET_USED(nb_events);
  ...
  while (aq_cnt || cq_ds_cnt) {
    ...
    if (get_work1 && ev.event_type == RTE_EVENT_TYPE_ETHDEV)
      ev.mbuf = ssovf_octeontx_wqe_to_pkt(
          get_work1, (ev.event >> 20) & 0x7F,
          OCCTX_RX_OFFLOAD_NONE | OCCTX_RX_MULTI_SEG_F, ws->lookup_mem);
    else
      ev.u64 = get_work1;
...
static __rte_always_inline uint16_t __sso_event_tx_adapter_enqueue(
    void *port, struct rte_event ev[], uint16_t nb_events, uint64_t *cmd,
    const uint16_t flag) {
  uint16_t port_id;
  uint16_t queue_id;
  struct rte_mbuf *m;
  ...
  case SSO_SYNC_UNTAGGED:
    ssows_swtag_full(ws, ev->u64, ev->event, SSO_SYNC_ATOMIC, ev->queue_id);
    rte_io_wmb();
    ssows_swtag_wait(ws);
    break;
...
#define T(name, f3, f2, f1, f0, sz, flags)                                     \
  static uint16_t __rte_noinline __rte_hot                                     \
      sso_event_tx_adapter_enqueue_##name(void *port, struct rte_event ev[],   \
                                          uint16_t nb_events) {                \
    uint64_t cmd[sz];                                                          \
    return __sso_event_tx_adapter_enqueue(port, ev, nb_events, cmd, flags);    \
  }

SSO_TX_ADPTR_ENQ_FASTPATH_FUNC
#undef T

static uint16_t __rte_hot ssow_crypto_adapter_enqueue(void *port,
                                                      struct rte_event ev[],
                                                      uint16_t nb_events) {
  RTE_SET_USED(nb_events);
...
#define R(name, f2, f1, f0, flags) [f2][f1][f0] = ssows_deq_##name,

      SSO_RX_ADPTR_ENQ_FASTPATH_FUNC
#undef R
  };

  dev->dequeue = ssow_deq[!!(edev->rx_offload_flags & OCCTX_RX_VLAN_FLTR_F)]
...
#define R(name, f2, f1, f0, flags) [f2][f1][f0] = ssows_deq_burst_##name,
...
#undef R
  ...
  dev->dequeue_burst =
      ssow_deq_burst[!!(edev->rx_offload_flags & OCCTX_RX_VLAN_FLTR_F)]
                    [!!(edev->rx_offload_flags & OCCTX_RX_OFFLOAD_CSUM_F)]
                    [!!(edev->rx_offload_flags & OCCTX_RX_MULTI_SEG_F)];

  if (edev->is_timeout_deq) {
    const event_dequeue_t ssow_deq_timeout[2][2][2] = {
#define R(name, f2, f1, f0, flags) [f2][f1][f0] = ssows_deq_timeout_##name,
...
#undef R
    ...
    dev->dequeue =
        ssow_deq_timeout[!!(edev->rx_offload_flags & OCCTX_RX_VLAN_FLTR_F)]
                        [!!(edev->rx_offload_flags & OCCTX_RX_OFFLOAD_CSUM_F)]
                        [!!(edev->rx_offload_flags & OCCTX_RX_MULTI_SEG_F)];
...
static void octeontx_create_rx_ol_flags_array(void *mem) {
...
void *octeontx_fastpath_lookup_mem_get(void) {
  ...
  mz = rte_memzone_reserve_aligned(name, LOOKUP_ARRAY_SZ, SOCKET_ID_ANY, 0,
                                   OCCTX_ALIGN);
  if (mz != NULL) {
    mem = mz->addr;
