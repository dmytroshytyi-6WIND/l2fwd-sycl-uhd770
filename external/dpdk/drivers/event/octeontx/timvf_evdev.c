...
static int timvf_mbox_dev_info_get(struct timvf_mbox_dev_info *info) {
...
static int timvf_ring_conf_set(struct timvf_ctrl_reg *rctl, uint8_t ring_id) {
  struct octeontx_mbox_hdr hdr = {0};
...
static int timvf_get_start_cyc(uint64_t *now, uint8_t ring_id) {
  struct octeontx_mbox_hdr hdr = {0};
...
static int optimize_bucket_parameters(struct timvf_ring *timr) {
  uint32_t hbkts;
  uint32_t lbkts;
  uint64_t tck_nsec;
  ...
  timr->nb_bkts =
      (hbkts - timr->nb_bkts) < (timr->nb_bkts - lbkts) ? hbkts : lbkts;
end:
  timr->get_target_bkt = bkt_and;
  timr->tck_nsec =
      RTE_ALIGN_MUL_CEIL((timr->max_tout / (timr->nb_bkts - 1)), 10);
  return 1;
}

static int timvf_ring_start(const struct rte_event_timer_adapter *adptr) {
  int ret;
  uint8_t use_fpa = 0;
  uint64_t interval;
  uintptr_t pool;
  ...
  case TIM_CLK_SRC_PTP:
    ...
    break;
  default:
    timvf_log_err("Unsupported clock source configured %d", timr->clk_src);
    return -EINVAL;
  }
  ...
  rctrl.rctrl1 = (uint64_t)(timr->clk_src) << 51 |
                 ...
                 1ull << 47 /* ENA */ | 1ull << 44 /* ENA_LDWB */ |
                 (timr->nb_bkts - 1);

  rctrl.rctrl2 = (uint64_t)(TIM_CHUNK_SIZE / 16) << 40;

  if (use_fpa) {
    pool = (uintptr_t)((struct rte_mempool *)timr->chunk_pool)->pool_id;
    ...
    timvf_write64((uint64_t)ret, (uint8_t *)timr->vbar0 + TIM_VRING_AURA);
  } else {
    rctrl.rctrl1 |= 1ull << 43 /* ENA_DFB (Enable don't free) */;
  }

  timvf_write64((uintptr_t)timr->bkt, (uint8_t *)timr->vbar0 + TIM_VRING_BASE);
  ...
  if (timvf_get_start_cyc(&timr->ring_start_cyc, timr->tim_ring_id) < 0) {
    ret = -EACCES;
    goto error;
  }
  ...
  timvf_log_info("nb_bkts %d min_ns %" PRIu64 " min_cyc %" PRIu64 ""
                 " maxtmo %" PRIu64 "\n",
                 timr->nb_bkts, timr->tck_nsec, interval, timr->max_tout);
...
static int timvf_ring_stop(const struct rte_event_timer_adapter *adptr) {
  struct timvf_ring *timr = adptr->data->adapter_priv;
...
static int timvf_ring_create(struct rte_event_timer_adapter *adptr) {
  ...
  timr = rte_zmalloc("octeontx_timvf_priv", sizeof(struct timvf_ring), 0);
  if (timr == NULL)
    return -ENOMEM;
  ...
  if ((rcfg->clk_src != RTE_EVENT_TIMER_ADAPTER_CPU_CLK) &&
      (!rcfg->timer_tick_ns || rcfg->timer_tick_ns < TIM_MIN_INTERVAL)) {
  ...
  if (free_idx == UINT16_MAX) {
    ...
    edev->tim_ring_ids = rte_realloc(edev->tim_ring_ids,
  ...
  timr->bkt = rte_zmalloc("octeontx_timvf_bucket",
                          (timr->nb_bkts) * sizeof(struct tim_mem_bucket), 0);
  if (timr->bkt == NULL)
    goto mem_err;

  snprintf(pool_name, sizeof(pool_name), "timvf_chunk_pool%d",
           timr->tim_ring_id);
  timr->chunk_pool = (void *)rte_mempool_create_empty(
      pool_name, timr->nb_chunks, TIM_CHUNK_SIZE, 0, 0, rte_socket_id(),
      mp_flags);
  ...
  ret = rte_mempool_set_ops_byname(timr->chunk_pool, mempool_ops, NULL);
...
static int timvf_ring_free(struct rte_event_timer_adapter *adptr) {
  struct timvf_ring *timr = adptr->data->adapter_priv;
  struct ssovf_evdev *edev;
  int i;
...
static int timvf_stats_get(const struct rte_event_timer_adapter *adapter,
  ...
  stats->adapter_tick_count =
      rte_reciprocal_divide_u64(bkt_cyc, &timr->fast_div);
  return 0;
}
...
int timvf_timer_adapter_caps_get(const struct rte_eventdev *dev, uint64_t flags,
  ...
  if (flags & RTE_EVENT_TIMER_ADAPTER_F_SP_PUT)
    timvf_ops.arm_burst = enable_stats ? timvf_timer_arm_burst_sp_stats
                                       : timvf_timer_arm_burst_sp;
  else
    timvf_ops.arm_burst = enable_stats ? timvf_timer_arm_burst_mp_stats
                                       : timvf_timer_arm_burst_mp;

  timvf_ops.arm_tmo_tick_burst =
      enable_stats ? timvf_timer_arm_tmo_brst_stats : timvf_timer_arm_tmo_brst;
...
void timvf_set_eventdevice(struct rte_eventdev *dev) { event_dev = dev; }
