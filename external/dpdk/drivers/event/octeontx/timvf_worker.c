...
static inline int timvf_timer_reg_checks(const struct timvf_ring *const timr,
  ...
  if (unlikely(!tim->timeout_ticks || tim->timeout_ticks >= timr->nb_bkts)) {
...
static inline void timvf_format_event(const struct rte_event_timer *const tim,
...
uint16_t timvf_timer_arm_burst_sp(const struct rte_event_timer_adapter *adptr,
  ...
  for (index = 0; index < nb_timers; index++) {
    ...
    ret =
        timvf_add_entry_sp(timr, tim[index]->timeout_ticks, tim[index], &entry);
    if (unlikely(ret)) {
      rte_errno = -ret;
      break;
    }
...
uint16_t timvf_timer_arm_burst_mp(const struct rte_event_timer_adapter *adptr,
  ...
  for (index = 0; index < nb_timers; index++) {
    ...
    timvf_format_event(tim[index], &entry);
    ret =
        timvf_add_entry_mp(timr, tim[index]->timeout_ticks, tim[index], &entry);
    if (unlikely(ret)) {
      rte_errno = -ret;
      break;
    }
...
uint16_t timvf_timer_arm_tmo_brst(const struct rte_event_timer_adapter *adptr,
  ...
  while (arr_idx < nb_timers) {
    ...
    ret =
        timvf_add_entry_brst(timr, timeout_tick, &tim[set_timers], entry, idx);
    set_timers += ret;
    if (ret != idx)
      break;
  }
...
uint16_t timvf_timer_arm_tmo_brst_stats(
  ...
  set_timers = timvf_timer_arm_tmo_brst(adptr, tim, timeout_tick, nb_timers);
