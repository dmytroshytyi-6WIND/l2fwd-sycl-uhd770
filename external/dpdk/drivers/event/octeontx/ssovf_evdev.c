...
static int ssovf_mbox_dev_info(struct ssovf_mbox_dev_info *info) {
...
static int ssovf_mbox_getwork_tmo_set(uint32_t timeout_ns) {
  struct octeontx_mbox_hdr hdr = {0};
...
static int ssovf_mbox_priority_set(uint8_t queue, uint8_t prio) {
  struct octeontx_mbox_hdr hdr = {0};
...
static int ssovf_mbox_timeout_ticks(uint64_t ns, uint64_t *tmo_ticks) {
  struct octeontx_mbox_hdr hdr = {0};
...
static void ssovf_info_get(struct rte_eventdev *dev,
  ...
  dev_info->event_dev_cap =
      RTE_EVENT_DEV_CAP_QUEUE_QOS | RTE_EVENT_DEV_CAP_DISTRIBUTED_SCHED |
      RTE_EVENT_DEV_CAP_QUEUE_ALL_TYPES | RTE_EVENT_DEV_CAP_RUNTIME_PORT_LINK |
...
static int ssovf_configure(const struct rte_eventdev *dev) {
...
static void ssovf_queue_def_conf(struct rte_eventdev *dev, uint8_t queue_id,
...
static void ssovf_queue_release(struct rte_eventdev *dev, uint8_t queue_id) {
  RTE_SET_USED(dev);
  RTE_SET_USED(queue_id);
}

static int ssovf_queue_setup(struct rte_eventdev *dev, uint8_t queue_id,
...
static void ssovf_port_def_conf(struct rte_eventdev *dev, uint8_t port_id,
...
static void ssovf_port_release(void *port) { rte_free(port); }

static int ssovf_port_setup(struct rte_eventdev *dev, uint8_t port_id,
  ...
  ws = rte_zmalloc_socket("eventdev ssows", sizeof(struct ssows),
...
static int ssovf_port_link(struct rte_eventdev *dev, void *port,
...
static int ssovf_port_unlink(struct rte_eventdev *dev, void *port,
...
static int ssovf_timeout_ticks(struct rte_eventdev *dev, uint64_t ns,
                               uint64_t *tmo_ticks) {
  RTE_SET_USED(dev);
...
static void ssows_dump(struct ssows *ws, FILE *f) {
  ...
  fprintf(f, "\tindex=%d valid=%d revlink=%d tail=%d head=%d grp=%d\n",
          (int)(val & 0x3ff), (int)(val >> 10) & 0x1, (int)(val >> 11) & 0x3ff,
          (int)(val >> 26) & 0x1, (int)(val >> 27) & 0x1,
          (int)(val >> 28) & 0x3ff);
...
static int ssovf_eth_rx_adapter_caps_get(const struct rte_eventdev *dev,
...
static int ssovf_eth_rx_adapter_queue_add(
  ...
  for (i = 0; i < edev->rxq_pools; i++) {
    if (edev->rxq_pool_array[i] == (uintptr_t)rxq->pool) {
      ...
      found = true;
      break;
    } else if (free_idx == UINT16_MAX && edev->rxq_pool_array[i] == 0) {
      free_idx = i;
    }
  }
  ...
  if (!found) {
    ...
    if (free_idx != UINT16_MAX) {
      ...
      edev->rxq_pools++;
      edev->rxq_pool_array = rte_realloc(edev->rxq_pool_array,
      ...
      edev->rxq_pool_rcnt = rte_realloc(edev->rxq_pool_rcnt,
  ...
  if (ret < 0)
    ssovf_log_err("failed to modify QOS, port=%d, q=%d", nic->port_id,
...
static int ssovf_eth_rx_adapter_queue_del(const struct rte_eventdev *dev,
                                          const struct rte_eth_dev *eth_dev,
                                          int32_t rx_queue_id) {
  const struct octeontx_nic *nic = eth_dev->data->dev_private;
  struct ssovf_evdev *edev = ssovf_pmd_priv(dev);
  ...
  if (ret < 0)
    ssovf_log_err("Failed to delete QOS port=%d, q=%d", nic->port_id,
                  rx_queue_id);
  return ret;
}

static int ssovf_eth_rx_adapter_start(const struct rte_eventdev *dev,
                                      const struct rte_eth_dev *eth_dev) {
  RTE_SET_USED(dev);
...
static int ssovf_eth_rx_adapter_stop(const struct rte_eventdev *dev,
                                     const struct rte_eth_dev *eth_dev) {
  RTE_SET_USED(dev);
  RTE_SET_USED(eth_dev);
...
static int ssovf_eth_tx_adapter_caps_get(const struct rte_eventdev *dev,
                                         const struct rte_eth_dev *eth_dev,
                                         uint32_t *caps) {
  int ret;
  RTE_SET_USED(dev);
...
static int ssovf_eth_tx_adapter_create(uint8_t id,
...
static int ssovf_eth_tx_adapter_free(uint8_t id,
                                     const struct rte_eventdev *dev) {
  RTE_SET_USED(id);
  RTE_SET_USED(dev);
  return 0;
...
static int ssovf_eth_tx_adapter_queue_add(uint8_t id,
                                          const struct rte_eventdev *dev,
                                          const struct rte_eth_dev *eth_dev,
                                          int32_t tx_queue_id) {
  RTE_SET_USED(id);
...
static int ssovf_eth_tx_adapter_queue_del(uint8_t id,
                                          const struct rte_eventdev *dev,
                                          const struct rte_eth_dev *eth_dev,
                                          int32_t tx_queue_id) {
  RTE_SET_USED(id);
...
static int ssovf_eth_tx_adapter_start(uint8_t id,
                                      const struct rte_eventdev *dev) {
  RTE_SET_USED(id);
  RTE_SET_USED(dev);
  return 0;
...
static int ssovf_eth_tx_adapter_stop(uint8_t id,
                                     const struct rte_eventdev *dev) {
  RTE_SET_USED(id);
  RTE_SET_USED(dev);
  return 0;
...
static void ssovf_dump(struct rte_eventdev *dev, FILE *f) {
  struct ssovf_evdev *edev = ssovf_pmd_priv(dev);
  uint8_t port;
...
static int ssovf_start(struct rte_eventdev *dev) {
  struct ssovf_evdev *edev = ssovf_pmd_priv(dev);
  struct ssows *ws;
  uint8_t *base;
  uint8_t i;
...
static void ssows_handle_event(void *arg, struct rte_event event) {
...
static void ssovf_stop(struct rte_eventdev *dev) {
  struct ssovf_evdev *edev = ssovf_pmd_priv(dev);
  struct ssows *ws;
  uint8_t *base;
  uint8_t i;
  ...
  for (i = 0; i < edev->nb_event_queues; i++) {
    /* Consume all the events through HWS0 */
    ssows_flush_events(dev->data->ports[0], i, ssows_handle_event, dev);
...
static int ssovf_close(struct rte_eventdev *dev) {
  struct ssovf_evdev *edev = ssovf_pmd_priv(dev);
...
static int ssovf_parsekv(const char *key __rte_unused, const char *value,
                         void *opaque) {
  int *flag = opaque;
  *flag = !!atoi(value);
  return 0;
...
static int ssovf_timvf_caps_get(const struct rte_eventdev *dev, uint64_t flags,
                                uint32_t *caps,
...
static int ssovf_crypto_adapter_caps_get(const struct rte_eventdev *dev,
...
static int ssovf_crypto_adapter_qp_add(const struct rte_eventdev *dev,
                                       const struct rte_cryptodev *cdev,
...
static int ssovf_crypto_adapter_qp_del(const struct rte_eventdev *dev,
                                       const struct rte_cryptodev *cdev,
                                       int32_t queue_pair_id) {
  struct cpt_instance *qp;
  uint8_t qp_id;
...
static int ssovf_vdev_probe(struct rte_vdev_device *vdev) {
  ...
  static const char *const args[] = {TIMVF_ENABLE_STATS_ARG, NULL};
  ...
  if (params != NULL && params[0] != '\0') {
    ...
    if (!kvlist) {
      ssovf_log_info("Ignoring unsupported params supplied '%s'", name);
    } else {
      ret = rte_kvargs_process(kvlist, TIMVF_ENABLE_STATS_ARG, ssovf_parsekv,
                               &timvf_enable_stats);
      if (ret != 0) {
  ...
  ssovf_log_info("Initializing %s domain=%d max_queues=%d max_ports=%d", name,
                 oinfo.domain, edev->max_event_queues, edev->max_event_ports);
...
static int ssovf_vdev_remove(struct rte_vdev_device *vdev) {
  const char *name;
...
static struct rte_vdev_driver vdev_ssovf_pmd = {.probe = ssovf_vdev_probe,
