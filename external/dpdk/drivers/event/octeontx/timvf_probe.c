...
void timvf_release_ring(uint8_t tim_ring_id) {
  uint16_t global_domain = octeontx_get_global_domain();
  int i;
...
void *timvf_bar(uint8_t vfid, uint8_t bar) {
  uint16_t global_domain = octeontx_get_global_domain();
...
static int timvf_probe(struct rte_pci_driver *pci_drv,
  ...
  if (pci_dev->mem_resource[0].addr == NULL ||
    ...
    timvf_log_err("Empty bars %p %p", pci_dev->mem_resource[0].addr,
  ...
  timvf_log_dbg("Domain=%d VFid=%d bar0 %p total_timvfs=%d", res->domain,
                res->vfid, pci_dev->mem_resource[0].addr, tdev.total_timvfs);
  return 0;
}

static const struct rte_pci_id pci_timvf_map[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CAVIUM, PCI_DEVICE_ID_OCTEONTX_TIM_VF)},
    {
        .vendor_id = 0,
    },
};
