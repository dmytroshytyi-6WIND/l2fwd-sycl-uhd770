...
static bool dsw_port_acquire_credits(struct dsw_evdev *dsw,
                                     struct dsw_port *port, int32_t credits) {
  ...
  total_on_loan = __atomic_load_n(&dsw->credits_on_loan, __ATOMIC_RELAXED);
   ...
   * allocation.
   */
  new_total_on_loan = __atomic_add_fetch(&dsw->credits_on_loan,
                                         acquired_credits, __ATOMIC_RELAXED);
...
static void dsw_port_return_credits(struct dsw_evdev *dsw,
                                    struct dsw_port *port, int32_t credits) {
  port->inflight_credits += credits;

  if (unlikely(port->inflight_credits > DSW_PORT_MAX_CREDITS)) {
    int32_t leave_credits = DSW_PORT_MIN_CREDITS;
    int32_t return_credits = port->inflight_credits - leave_credits;

    port->inflight_credits = leave_credits;

    __atomic_sub_fetch(&dsw->credits_on_loan, return_credits, __ATOMIC_RELAXED);

    DSW_LOG_DP_PORT(DEBUG, port->id, "Returned %d tokens to pool.\n",
                    return_credits);
  }
}

static void dsw_port_enqueue_stats(struct dsw_port *port, uint16_t num_new,
...
static void dsw_port_queue_enqueue_stats(struct dsw_port *source_port,
                                         uint8_t queue_id) {
  source_port->queue_enqueued[queue_id]++;
}

static void dsw_port_dequeue_stats(struct dsw_port *port, uint16_t num) {
  port->dequeued += num;
}

static void dsw_port_queue_dequeued_stats(struct dsw_port *source_port,
                                          uint8_t queue_id) {
  source_port->queue_dequeued[queue_id]++;
}

static void dsw_port_load_record(struct dsw_port *port, unsigned int dequeued) {
  if (dequeued > 0 && port->busy_start == 0)
    /* work period begins */
    port->busy_start = rte_get_timer_cycles();
  else if (dequeued == 0 && port->busy_start > 0) {
    /* work period ends */
    uint64_t work_period = rte_get_timer_cycles() - port->busy_start;
    port->busy_cycles += work_period;
    port->busy_start = 0;
  }
}

static int16_t dsw_port_load_close_period(struct dsw_port *port, uint64_t now) {
...
static void dsw_port_load_update(struct dsw_port *port, uint64_t now) {
  int16_t old_load;
  int16_t period_load;
  int16_t new_load;
...
static void dsw_port_consider_load_update(struct dsw_port *port, uint64_t now) {
...
static void dsw_port_ctl_enqueue(struct dsw_port *port,
...
static int dsw_port_ctl_dequeue(struct dsw_port *port,
                                struct dsw_ctl_msg *msg) {
  return rte_ring_dequeue_elem(port->ctl_in_ring, msg, sizeof(*msg));
}

static void dsw_port_ctl_broadcast(struct dsw_evdev *dsw,
                                   struct dsw_port *source_port, uint8_t type,
                                   struct dsw_queue_flow *qfs,
                                   uint8_t qfs_len) {
  uint16_t port_id;
  struct dsw_ctl_msg msg = {
      .type = type, .originating_port_id = source_port->id, .qfs_len = qfs_len};
...
dsw_is_queue_flow_in_ary(const struct dsw_queue_flow *qfs, uint16_t qfs_len,
  ...
  for (i = 0; i < qfs_len; i++)
    if (qfs[i].queue_id == queue_id && qfs[i].flow_hash == flow_hash)
      return true;

  return false;
}

static __rte_always_inline bool dsw_port_is_flow_paused(struct dsw_port *port,
                                                        uint8_t queue_id,
                                                        uint16_t flow_hash) {
  return dsw_is_queue_flow_in_ary(port->paused_flows, port->paused_flows_len,
                                  queue_id, flow_hash);
}

static void dsw_port_add_paused_flows(struct dsw_port *port,
                                      struct dsw_queue_flow *qfs,
                                      uint8_t qfs_len) {
  uint8_t i;
...
static void dsw_port_remove_paused_flow(struct dsw_port *port,
  ...
  for (i = 0; i < port->paused_flows_len; i++) {
    ...
    if (qf->queue_id == target_qf->queue_id &&
      ...
      if (i != last_idx)
        port->paused_flows[i] = port->paused_flows[last_idx];
      port->paused_flows_len--;
      break;
    }
  }
...
static void dsw_port_remove_paused_flows(struct dsw_port *port,
                                         struct dsw_queue_flow *qfs,
                                         uint8_t qfs_len) {
  uint8_t i;
...
static void dsw_port_flush_out_buffers(struct dsw_evdev *dsw,
                                       struct dsw_port *source_port);

static void dsw_port_handle_pause_flows(struct dsw_evdev *dsw,
                                        struct dsw_port *port,
                                        uint8_t originating_port_id,
                                        struct dsw_queue_flow *paused_qfs,
                                        uint8_t qfs_len) {
  struct dsw_ctl_msg cfm = {.type = DSW_CTL_CFM,
...
static inline int dsw_cmp_qf(const void *v_qf_a, const void *v_qf_b) {
...
static uint16_t dsw_sort_qfs_to_bursts(struct dsw_queue_flow *qfs,
                                       uint16_t qfs_len,
  ...
  for (i = 0; i < qfs_len; i++) {
    if (i == 0 || dsw_cmp_qf(&qfs[i], &current_burst->queue_flow) != 0) {
...
static bool dsw_retrieve_port_loads(struct dsw_evdev *dsw, int16_t *port_loads,
                                    int16_t load_limit) {
  ...
  for (i = 0; i < dsw->num_ports; i++) {
    ...
    int32_t immigration_load =
        __atomic_load_n(&dsw->ports[i].immigration_load, __ATOMIC_RELAXED);
...
static int16_t dsw_flow_load(uint16_t num_events, int16_t port_load) {
  return ((int32_t)port_load * (int32_t)num_events) / DSW_MAX_EVENTS_RECORDED;
}

static int16_t dsw_evaluate_migration(int16_t source_load, int16_t target_load,
                                      int16_t flow_load) {
...
static bool dsw_is_serving_port(struct dsw_evdev *dsw, uint8_t port_id,
                                uint8_t queue_id) {
...
static bool dsw_select_emigration_target(
    struct dsw_evdev *dsw, struct dsw_queue_flow_burst *bursts,
    uint16_t num_bursts, uint8_t source_port_id, int16_t *port_loads,
    uint16_t num_ports, uint8_t *target_port_ids,
    struct dsw_queue_flow *target_qfs, uint8_t *targets_len) {
  ...
  for (i = 0; i < num_bursts; i++) {
    ...
    if (dsw_is_queue_flow_in_ary(target_qfs, *targets_len, qf->queue_id,
                                 qf->flow_hash))
      continue;
    ...
    for (port_id = 0; port_id < num_ports; port_id++) {
      ...
      weight = dsw_evaluate_migration(source_port_load, port_loads[port_id],
                                      flow_load);
  ...
  DSW_LOG_DP_PORT(DEBUG, source_port_id,
                  "Selected queue_id %d "
                  "flow_hash %d (with flow load %d) for migration "
                  "to port %d.\n",
                  candidate_qf->queue_id, candidate_qf->flow_hash,
                  DSW_LOAD_TO_PERCENT(candidate_flow_load), candidate_port_id);
...
static void dsw_select_emigration_targets(struct dsw_evdev *dsw,
                                          struct dsw_port *source_port,
                                          struct dsw_queue_flow_burst *bursts,
                                          uint16_t num_bursts,
                                          int16_t *port_loads) {
  ...
  for (i = 0; i < DSW_MAX_FLOWS_PER_MIGRATION; i++) {
    ...
    found = dsw_select_emigration_target(
        dsw, bursts, num_bursts, source_port->id, port_loads, dsw->num_ports,
        target_port_ids, target_qfs, targets_len);
    if (!found)
      break;
  }
...
static uint8_t dsw_schedule(struct dsw_evdev *dsw, uint8_t queue_id,
                            uint16_t flow_hash) {
  struct dsw_queue *queue = &dsw->queues[queue_id];
  uint8_t port_id;
  ...
  DSW_LOG_DP(DEBUG,
             "Event with queue_id %d flow_hash %d is scheduled "
             "to port %d.\n",
...
static void dsw_port_transmit_buffered(struct dsw_evdev *dsw,
                                       struct dsw_port *source_port,
   ...
   */
  do {
    enqueued += rte_event_ring_enqueue_burst(
        dest_port->in_ring, buffer + enqueued, *buffer_len - enqueued, NULL);
  } while (unlikely(enqueued != *buffer_len));

  (*buffer_len) = 0;
}

static uint16_t dsw_port_get_parallel_flow_id(struct dsw_port *port) {
...
static void dsw_port_buffer_paused(struct dsw_port *port,
                                   const struct rte_event *paused_event) {
  port->paused_events[port->paused_events_len] = *paused_event;
  port->paused_events_len++;
}

static void dsw_port_buffer_non_paused(struct dsw_evdev *dsw,
                                       struct dsw_port *source_port,
                                       uint8_t dest_port_id,
...
static void dsw_port_buffer_parallel(struct dsw_evdev *dsw,
                                     struct dsw_port *source_port,
  ...
  dest_port_id =
      dsw_schedule(dsw, event.queue_id, dsw_flow_id_hash(event.flow_id));

  dsw_port_buffer_non_paused(dsw, source_port, dest_port_id, &event);
}

static void dsw_port_buffer_event(struct dsw_evdev *dsw,
                                  struct dsw_port *source_port,
                                  const struct rte_event *event) {
  uint16_t flow_hash;
  uint8_t dest_port_id;
  ...
  if (unlikely(
          dsw_port_is_flow_paused(source_port, event->queue_id, flow_hash))) {
...
static void dsw_port_flush_paused_events(struct dsw_evdev *dsw,
                                         struct dsw_port *source_port,
  ...
  for (i = 0; i < paused_events_len; i++) {
    ...
    if (event->queue_id == qf->queue_id && flow_hash == qf->flow_hash)
      dsw_port_buffer_non_paused(dsw, source_port, dest_port_id, event);
    else
      dsw_port_buffer_paused(source_port, event);
  }
...
static void dsw_port_emigration_stats(struct dsw_port *port, uint8_t finished) {
  uint64_t flow_migration_latency;

  flow_migration_latency = (rte_get_timer_cycles() - port->emigration_start);
  port->emigration_latency += (flow_migration_latency * finished);
  port->emigrations += finished;
}

static void dsw_port_end_emigration(struct dsw_evdev *dsw,
                                    struct dsw_port *port,
  ...
  for (i = 0; i < port->emigration_targets_len; i++) {
    ...
    uint8_t queue_schedule_type = dsw->queues[queue_id].schedule_type;
    uint16_t flow_hash = qf->flow_hash;

    if (queue_schedule_type != schedule_type) {
      left_port_ids[left_qfs_len] = port->emigration_target_port_ids[i];
...
static void dsw_port_move_parallel_flows(struct dsw_evdev *dsw,
                                         struct dsw_port *source_port) {
  uint8_t i;

  for (i = 0; i < source_port->emigration_targets_len; i++) {
    struct dsw_queue_flow *qf = &source_port->emigration_target_qfs[i];
    uint8_t queue_id = qf->queue_id;

    if (dsw->queues[queue_id].schedule_type == RTE_SCHED_TYPE_PARALLEL) {
      uint8_t dest_port_id = source_port->emigration_target_port_ids[i];
      uint16_t flow_hash = qf->flow_hash;

      /* Single byte-sized stores are always atomic. */
      dsw->queues[queue_id].flow_to_port_map[flow_hash] = dest_port_id;
    }
  }
...
static void dsw_port_consider_emigration(struct dsw_evdev *dsw,
                                         struct dsw_port *source_port,
                                         uint64_t now) {
   ...
   */
  source_port->next_emigration = now + source_port->migration_interval / 2 +
  ...
  source_port_load = __atomic_load_n(&source_port->load, __ATOMIC_RELAXED);
  if (source_port_load < DSW_MIN_SOURCE_LOAD_FOR_MIGRATION) {
    DSW_LOG_DP_PORT(DEBUG, source_port->id,
   ...
   */
  any_port_below_limit = dsw_retrieve_port_loads(
      dsw, port_loads, DSW_MAX_TARGET_LOAD_FOR_MIGRATION);
  ...
  num_bursts = dsw_sort_qfs_to_bursts(seen_events, seen_events_len, bursts);
  ...
  dsw_port_add_paused_flows(source_port, source_port->emigration_target_qfs,
...
static void dsw_port_handle_unpause_flows(struct dsw_evdev *dsw,
                                          struct dsw_port *port,
                                          uint8_t originating_port_id,
                                          struct dsw_queue_flow *paused_qfs,
                                          uint8_t qfs_len) {
...
static void dsw_port_forward_emigrated_flow(struct dsw_port *source_port,
  ...
  while (events_left > 0) {
    uint16_t in_burst_size = RTE_MIN(FORWARD_BURST_SIZE, events_left);
    struct rte_event in_burst[in_burst_size];
    uint16_t in_len;
    uint16_t i;

    in_len = rte_event_ring_dequeue_burst(source_port->in_ring, in_burst,
                                          in_burst_size, NULL);
    ...
    for (i = 0; i < in_len; i++) {
      ...
      if (e->queue_id == queue_id &&
        ...
        while (rte_event_ring_enqueue_burst(dest_ring, e, 1, NULL) != 1)
          rte_pause();
      } else {
...
static void dsw_port_move_emigrating_flows(struct dsw_evdev *dsw,
                                           struct dsw_port *source_port) {
  uint8_t i;
  ...
  for (i = 0; i < source_port->emigration_targets_len; i++) {
    ...
    dsw->queues[qf->queue_id].flow_to_port_map[qf->flow_hash] = dest_port_id;
...
static void dsw_port_handle_confirm(struct dsw_evdev *dsw,
                                    struct dsw_port *port) {
  port->cfm_cnt++;
  ...
  if (port->cfm_cnt == (dsw->num_ports - 1)) {
    ...
    case DSW_MIGRATION_STATE_UNPAUSING:
      dsw_port_end_emigration(dsw, port, RTE_SCHED_TYPE_ATOMIC);
      break;
    default:
      RTE_ASSERT(0);
      break;
...
static void dsw_port_ctl_process(struct dsw_evdev *dsw, struct dsw_port *port) {
  struct dsw_ctl_msg msg;

  if (dsw_port_ctl_dequeue(port, &msg) == 0) {
    switch (msg.type) {
    case DSW_CTL_PAUS_REQ:
      dsw_port_handle_pause_flows(dsw, port, msg.originating_port_id, msg.qfs,
                                  msg.qfs_len);
      break;
    case DSW_CTL_UNPAUS_REQ:
      dsw_port_handle_unpause_flows(dsw, port, msg.originating_port_id, msg.qfs,
                                    msg.qfs_len);
      break;
    case DSW_CTL_CFM:
...
static void dsw_port_note_op(struct dsw_port *port, uint16_t num_events) {
...
static void dsw_port_bg_process(struct dsw_evdev *dsw, struct dsw_port *port) {
...
uint16_t dsw_event_enqueue(void *port, const struct rte_event *ev) {
  return dsw_event_enqueue_burst(port, ev, unlikely(ev == NULL) ? 0 : 1);
}

static __rte_always_inline uint16_t dsw_event_enqueue_burst_generic(
    struct dsw_port *source_port, const struct rte_event events[],
    uint16_t events_len, bool op_types_known, uint16_t num_new,
    uint16_t num_release, uint16_t num_non_release) {
  struct dsw_evdev *dsw = source_port->dsw;
  bool enough_credits;
  uint16_t i;

  DSW_LOG_DP_PORT(DEBUG, source_port->id,
                  "Attempting to enqueue %d "
                  "events to port %d.\n",
  ...
  enough_credits = dsw_port_acquire_credits(dsw, source_port, num_non_release);
  if (unlikely(!enough_credits))
    return 0;

  source_port->pending_releases -= num_release;

  dsw_port_enqueue_stats(source_port, num_new, num_non_release - num_new,
                         num_release);

  for (i = 0; i < events_len; i++) {
    const struct rte_event *event = &events[i];

    if (likely(num_release == 0 || event->op != RTE_EVENT_OP_RELEASE))
      dsw_port_buffer_event(dsw, source_port, event);
    dsw_port_queue_enqueue_stats(source_port, event->queue_id);
  }

  DSW_LOG_DP_PORT(DEBUG, source_port->id,
                  "%d non-release events "
                  "accepted.\n",
                  num_non_release);
...
uint16_t dsw_event_enqueue_burst(void *port, const struct rte_event events[],
                                 uint16_t events_len) {
  ...
  return dsw_event_enqueue_burst_generic(source_port, events, events_len, false,
                                         0, 0, 0);
}
...
uint16_t dsw_event_enqueue_new_burst(void *port,
  ...
  return dsw_event_enqueue_burst_generic(source_port, events, events_len, true,
...
uint16_t dsw_event_dequeue(void *port, struct rte_event *events,
                           uint64_t wait) {
  return dsw_event_dequeue_burst(port, events, 1, wait);
}

static void dsw_port_record_seen_events(struct dsw_port *port,
  ...
  if (unlikely(port->seen_events_len != DSW_MAX_EVENTS_RECORDED))
    port->seen_events_len =
        RTE_MIN(port->seen_events_len + num, DSW_MAX_EVENTS_RECORDED);
}
...
static inline int dsw_cmp_event(const void *v_event_a, const void *v_event_b) {
...
static uint16_t dsw_port_dequeue_burst(struct dsw_port *port,
...
uint16_t dsw_event_dequeue_burst(void *port, struct rte_event *events,
                                 uint16_t num, uint64_t wait __rte_unused) {
  struct dsw_port *source_port = port;
  struct dsw_evdev *dsw = source_port->dsw;
  uint16_t dequeued;
