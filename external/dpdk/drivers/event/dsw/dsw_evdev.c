...
static int dsw_port_setup(struct rte_eventdev *dev, uint8_t port_id,
  ...
  *port = (struct dsw_port){.id = port_id,
                            .dsw = dsw,
  ...
  in_ring =
      rte_event_ring_create(ring_name, DSW_IN_RING_SIZE, dev->data->socket_id,
                            RING_F_SC_DEQ | RING_F_EXACT_SZ);

  if (in_ring == NULL)
    return -ENOMEM;

  snprintf(ring_name, sizeof(ring_name), "dswctl%d_p%u", dev->data->dev_id,
           port_id);

  ctl_in_ring = rte_ring_create_elem(ring_name, sizeof(struct dsw_ctl_msg),
                                     DSW_CTL_IN_RING_SIZE, dev->data->socket_id,
                                     RING_F_SC_DEQ | RING_F_EXACT_SZ);
...
static void dsw_port_def_conf(struct rte_eventdev *dev __rte_unused,
...
static void dsw_port_release(void *p) {
...
static int dsw_queue_setup(struct rte_eventdev *dev, uint8_t queue_id,
...
static void dsw_queue_def_conf(struct rte_eventdev *dev __rte_unused,
                               uint8_t queue_id __rte_unused,
                               struct rte_event_queue_conf *queue_conf) {
  *queue_conf =
      (struct rte_event_queue_conf){.nb_atomic_flows = 4096,
                                    .schedule_type = RTE_SCHED_TYPE_ATOMIC,
                                    .priority = RTE_EVENT_DEV_PRIORITY_NORMAL};
}

static void dsw_queue_release(struct rte_eventdev *dev __rte_unused,
                              uint8_t queue_id __rte_unused) {}

static void queue_add_port(struct dsw_queue *queue, uint16_t port_id) {
  queue->serving_ports[queue->num_serving_ports] = port_id;
  queue->num_serving_ports++;
}

static bool queue_remove_port(struct dsw_queue *queue, uint16_t port_id) {
  uint16_t i;
  ...
  for (i = 0; i < queue->num_serving_ports; i++)
    if (queue->serving_ports[i] == port_id) {
      ...
      if (i != last_idx)
        queue->serving_ports[i] = queue->serving_ports[last_idx];
      queue->num_serving_ports--;
      return true;
    }
  return false;
...
static int dsw_port_link_unlink(struct rte_eventdev *dev, void *port,
                                const uint8_t queues[], uint16_t num,
                                bool link) {
  struct dsw_evdev *dsw = dsw_pmd_priv(dev);
...
static int dsw_port_link(struct rte_eventdev *dev, void *port,
                         const uint8_t queues[],
                         const uint8_t priorities[] __rte_unused,
                         uint16_t num) {
  return dsw_port_link_unlink(dev, port, queues, num, true);
}

static int dsw_port_unlink(struct rte_eventdev *dev, void *port,
                           uint8_t queues[], uint16_t num) {
  return dsw_port_link_unlink(dev, port, queues, num, false);
}

static void dsw_info_get(struct rte_eventdev *dev __rte_unused,
...
static int dsw_configure(const struct rte_eventdev *dev) {
  struct dsw_evdev *dsw = dsw_pmd_priv(dev);
...
static void initial_flow_to_port_assignment(struct dsw_evdev *dsw) {
  uint8_t queue_id;
  for (queue_id = 0; queue_id < dsw->num_queues; queue_id++) {
    struct dsw_queue *queue = &dsw->queues[queue_id];
    uint16_t flow_hash;
    for (flow_hash = 0; flow_hash < DSW_MAX_FLOWS; flow_hash++) {
      uint8_t port_idx = rte_rand() % queue->num_serving_ports;
      uint8_t port_id = queue->serving_ports[port_idx];
      dsw->queues[queue_id].flow_to_port_map[flow_hash] = port_id;
    }
  }
}

static int dsw_start(struct rte_eventdev *dev) {
  struct dsw_evdev *dsw = dsw_pmd_priv(dev);
  uint16_t i;
  uint64_t now;
...
static void dsw_port_drain_buf(uint8_t dev_id, struct rte_event *buf,
                               uint16_t buf_len, eventdev_stop_flush_t flush,
                               void *flush_arg) {
  uint16_t i;
...
static void dsw_port_drain_paused(uint8_t dev_id, struct dsw_port *port,
...
static void dsw_port_drain_out(uint8_t dev_id, struct dsw_evdev *dsw,
                               struct dsw_port *port,
  ...
  for (dport_id = 0; dport_id < dsw->num_ports; dport_id++)
    if (dport_id != port->id)
      dsw_port_drain_buf(dev_id, port->out_buffer[dport_id],
                         port->out_buffer_len[dport_id], flush, flush_arg);
}

static void dsw_port_drain_in_ring(uint8_t dev_id, struct dsw_port *port,
                                   eventdev_stop_flush_t flush,
                                   void *flush_arg) {
  struct rte_event ev;
...
static void dsw_drain(uint8_t dev_id, struct dsw_evdev *dsw,
                      eventdev_stop_flush_t flush, void *flush_arg) {
  uint16_t port_id;
...
static void dsw_stop(struct rte_eventdev *dev) {
  struct dsw_evdev *dsw = dsw_pmd_priv(dev);
  uint8_t dev_id;
  eventdev_stop_flush_t flush;
  void *flush_arg;
...
static int dsw_close(struct rte_eventdev *dev) {
  struct dsw_evdev *dsw = dsw_pmd_priv(dev);
...
static int dsw_probe(struct rte_vdev_device *vdev) {
  const char *name;
  ...
  dev =
      rte_event_pmd_vdev_init(name, sizeof(struct dsw_evdev), rte_socket_id());
  if (dev == NULL)
    return -EFAULT;
...
static int dsw_remove(struct rte_vdev_device *vdev) {
  const char *name;
...
static struct rte_vdev_driver evdev_dsw_pmd_drv = {.probe = dsw_probe,
                                                   .remove = dsw_remove};
