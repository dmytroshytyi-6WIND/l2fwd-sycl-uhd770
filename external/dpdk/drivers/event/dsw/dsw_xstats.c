...
#define DSW_XSTATS_ID_GET_PARAM(id) ((id) >> DSW_XSTATS_ID_STAT_BITS)

#define DSW_XSTATS_ID_GET_STAT(id) ((id)&DSW_XSTATS_ID_STAT_MASK)

#define DSW_XSTATS_ID_CREATE(id, param_value)                                  \
  (((param_value) << DSW_XSTATS_ID_STAT_BITS) | id)

typedef uint64_t (*dsw_xstats_dev_get_value_fn)(struct dsw_evdev *dsw);
...
typedef uint64_t (*dsw_xstats_port_get_value_fn)(struct dsw_evdev *dsw,
                                                 uint8_t port_id,
                                                 uint8_t queue_id);
...
static uint64_t dsw_xstats_dev_credits_on_loan(struct dsw_evdev *dsw) {
...
#define DSW_GEN_PORT_ACCESS_FN(_variable)                                      \
  static uint64_t dsw_xstats_port_get_##_variable(                             \
      struct dsw_evdev *dsw, uint8_t port_id, uint8_t queue_id __rte_unused) { \
...
static uint64_t dsw_xstats_port_get_queue_enqueued(struct dsw_evdev *dsw,
                                                   uint8_t port_id,
                                                   uint8_t queue_id) {
...
static uint64_t dsw_xstats_port_get_queue_dequeued(struct dsw_evdev *dsw,
                                                   uint8_t port_id,
                                                   uint8_t queue_id) {
...
dsw_xstats_port_get_event_proc_latency(struct dsw_evdev *dsw, uint8_t port_id,
                                       uint8_t queue_id __rte_unused) {
  uint64_t total_busy_cycles = dsw->ports[port_id].total_busy_cycles;
  uint64_t dequeued = dsw->ports[port_id].dequeued;

  return dequeued > 0 ? total_busy_cycles / dequeued : 0;
}

static uint64_t dsw_xstats_port_get_busy_cycles(struct dsw_evdev *dsw,
                                                uint8_t port_id,
                                                uint8_t queue_id __rte_unused) {
...
static uint64_t dsw_xstats_port_get_load(struct dsw_evdev *dsw, uint8_t port_id,
                                         uint8_t queue_id __rte_unused) {
  int16_t load;
...
static struct dsw_xstats_port dsw_port_xstats[] = {
    {"port_%u_new_enqueued", dsw_xstats_port_get_new_enqueued, false},
    {"port_%u_forward_enqueued", dsw_xstats_port_get_forward_enqueued, false},
    {"port_%u_release_enqueued", dsw_xstats_port_get_release_enqueued, false},
    {"port_%u_queue_%u_enqueued", dsw_xstats_port_get_queue_enqueued, true},
    {"port_%u_dequeued", dsw_xstats_port_get_dequeued, false},
    {"port_%u_queue_%u_dequeued", dsw_xstats_port_get_queue_dequeued, true},
    {"port_%u_emigrations", dsw_xstats_port_get_emigrations, false},
    {"port_%u_migration_latency", dsw_xstats_port_get_migration_latency, false},
    {"port_%u_immigrations", dsw_xstats_port_get_immigrations, false},
    {"port_%u_event_proc_latency", dsw_xstats_port_get_event_proc_latency,
     false},
    {"port_%u_busy_cycles", dsw_xstats_port_get_busy_cycles, false},
    {"port_%u_inflight_credits", dsw_xstats_port_get_inflight_credits, false},
    {"port_%u_pending_releases", dsw_xstats_port_get_pending_releases, false},
    {"port_%u_load", dsw_xstats_port_get_load, false},
    {"port_%u_last_bg", dsw_xstats_port_get_last_bg, false}};

typedef void (*dsw_xstats_foreach_fn)(const char *xstats_name,
                                      enum rte_event_dev_xstats_mode mode,
                                      uint8_t queue_port_id,
                                      unsigned int xstats_id, void *data);

static void dsw_xstats_dev_foreach(dsw_xstats_foreach_fn fn, void *fn_data) {
  unsigned int i;

  for (i = 0; i < RTE_DIM(dsw_dev_xstats); i++)
    fn(dsw_dev_xstats[i].name, RTE_EVENT_DEV_XSTATS_DEVICE, 0, i, fn_data);
}

static void dsw_xstats_port_foreach(struct dsw_evdev *dsw, uint8_t port_id,
                                    dsw_xstats_foreach_fn fn, void *fn_data) {
  uint8_t queue_id;
  unsigned int stat_idx;

  for (stat_idx = 0, queue_id = 0; stat_idx < RTE_DIM(dsw_port_xstats);) {
    ...
    if (xstat->per_queue) {
      ...
      snprintf(xstats_name, sizeof(xstats_name),
               dsw_port_xstats[stat_idx].name_fmt, port_id, queue_id);
      queue_id++;
    } else {
      xstats_id = stat_idx;
      snprintf(xstats_name, sizeof(xstats_name),
    ...
    fn(xstats_name, RTE_EVENT_DEV_XSTATS_PORT, port_id, xstats_id, fn_data);
...
static void dsw_xstats_store_stat(const char *xstats_name,
                                  enum rte_event_dev_xstats_mode mode,
...
int dsw_xstats_get_names(const struct rte_eventdev *dev,
  ...
  struct store_ctx ctx = {
      .names = xstats_names, .ids = ids, .capacity = capacity};
  ...
  case RTE_EVENT_DEV_XSTATS_PORT:
    dsw_xstats_port_foreach(dsw, queue_port_id, dsw_xstats_store_stat, &ctx);
    return ctx.count;
...
static int dsw_xstats_dev_get(const struct rte_eventdev *dev,
...
static int dsw_xstats_port_get(const struct rte_eventdev *dev, uint8_t port_id,
                               const unsigned int ids[], uint64_t values[],
                               unsigned int n) {
  struct dsw_evdev *dsw = dsw_pmd_priv(dev);
  unsigned int i;
...
static void dsw_xstats_find_stat(const char *xstats_name,
                                 enum rte_event_dev_xstats_mode mode,
                                 uint8_t queue_port_id, unsigned int xstats_id,
                                 void *data) {
  ...
  if (strcmp(ctx->name, xstats_name) == 0) {
    if (ctx->id != NULL)
      *ctx->id = xstats_id;
    dsw_xstats_get(ctx->dev, mode, queue_port_id, &xstats_id, &ctx->value, 1);
  }
}
...
uint64_t dsw_xstats_get_by_name(const struct rte_eventdev *dev,
  ...
  struct find_ctx ctx = {.dev = dev, .name = name, .id = id, .value = -EINVAL};

  dsw_xstats_dev_foreach(dsw_xstats_find_stat, &ctx);

  for (port_id = 0; port_id < dsw->num_ports; port_id++)
    dsw_xstats_port_foreach(dsw, port_id, dsw_xstats_find_stat, &ctx);
