...
rte_dpaa2_create_dpcon_device(int dev_fd __rte_unused,
  ...
  if (!dpcon_node) {
    DPAA2_EVENTDEV_ERR("Memory allocation failed for dpcon device");
    return -1;
  }
  ...
  ret =
      dpcon_open(&dpcon_node->dpcon, CMD_PRI_LOW, dpcon_id, &dpcon_node->token);
  if (ret) {
    DPAA2_EVENTDEV_ERR("Unable to open dpcon device: err(%d)", ret);
    rte_free(dpcon_node);
    return -1;
  }

  /* Get the device attributes */
  ret = dpcon_get_attributes(&dpcon_node->dpcon, CMD_PRI_LOW, dpcon_node->token,
                             &attr);
  if (ret != 0) {
    DPAA2_EVENTDEV_ERR("dpcon attribute fetch failed: err(%d)", ret);
    rte_free(dpcon_node);
    return -1;
  }
