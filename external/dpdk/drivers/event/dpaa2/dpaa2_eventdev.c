...
static uint16_t dpaa2_eventdev_enqueue_burst(void *port,
  ...
  if (unlikely(!DPAA2_PER_LCORE_DPIO)) {
    ...
    if (ret < 0) {
      DPAA2_EVENTDEV_ERR("Failed to allocate IO portal, tid: %d\n",
                         rte_gettid());
      return 0;
    }
  }
  ...
  for (i = 0; i < DPAA2_EVENT_MAX_QUEUES; i++) {
    ...
    ret = dpio_add_static_dequeue_channel(
        dpio_dev->dpio, CMD_PRI_LOW, dpio_dev->token, evq_info->dpcon->dpcon_id,
        &channel_index);
    if (ret < 0) {
      DPAA2_EVENTDEV_ERR("Static dequeue config failed: err(%d)", ret);
      goto err;
    }
...
skip_linking:
  ...
  while (nb_events) {
    frames_to_send =
        (nb_events > dpaa2_eqcr_size) ? dpaa2_eqcr_size : nb_events;

    for (loop = 0; loop < frames_to_send; loop++) {
      const struct rte_event *event = &ev[num_tx + loop];

      if (event->sched_type != RTE_SCHED_TYPE_ATOMIC)
        fqid = evq_info->dpci->rx_queue[DPAA2_EVENT_DPCI_PARALLEL_QUEUE].fqid;
      else
        fqid = evq_info->dpci->rx_queue[DPAA2_EVENT_DPCI_ATOMIC_QUEUE].fqid;
      ...
      if (event->sched_type == RTE_SCHED_TYPE_ATOMIC &&
        ...
        uint8_t dqrr_index = *dpaa2_seqn(event->mbuf) - 1;
       ...
       * to avoid copy
       */
      struct rte_event *ev_temp = rte_malloc(NULL, sizeof(struct rte_event), 0);

      if (!ev_temp) {
        if (!loop)
          return num_tx;
        frames_to_send = loop;
        DPAA2_EVENTDEV_ERR("Unable to allocate event object");
        goto send_partial;
      }
...
err:
  for (n = 0; n < i; n++) {
    ...
    dpio_remove_static_dequeue_channel(dpio_dev->dpio, 0, dpio_dev->token,
                                       evq_info->dpcon->dpcon_id);
  }
  return 0;
}

static uint16_t dpaa2_eventdev_enqueue(void *port, const struct rte_event *ev) {
...
static void dpaa2_eventdev_dequeue_wait(uint64_t timeout_ticks) {
  ...
  epoll_wait(DPAA2_PER_LCORE_DPIO->epoll_fd, &epoll_ev, 1, timeout_ticks);
}
...
static void dpaa2_eventdev_process_parallel(struct qbman_swp *swp,
                                            ...
                                            struct rte_event *ev) {
  struct rte_event *ev_temp = (struct rte_event *)(size_t)DPAA2_GET_FD_ADDR(fd);
...
static uint16_t dpaa2_eventdev_dequeue_burst(void *port, struct rte_event ev[],
                                             uint16_t nb_events,
...
skip_linking:
  ...
  while (DPAA2_PER_LCORE_DQRR_SIZE) {
    if (DPAA2_PER_LCORE_DQRR_HELD & (1 << i)) {
      ...
      DPAA2_PER_LCORE_DQRR_SIZE--;
      *dpaa2_seqn(DPAA2_PER_LCORE_DQRR_MBUF(i)) = DPAA2_INVALID_MBUF_SEQN;
    }
    i++;
  }
...
static uint16_t dpaa2_eventdev_dequeue(void *port, struct rte_event *ev,
                                       uint64_t timeout_ticks) {
  return dpaa2_eventdev_dequeue_burst(port, ev, 1, timeout_ticks);
}

static void dpaa2_eventdev_info_get(struct rte_eventdev *dev,
  ...
  dev_info->min_dequeue_timeout_ns = DPAA2_EVENT_MIN_DEQUEUE_TIMEOUT;
  dev_info->max_dequeue_timeout_ns = DPAA2_EVENT_MAX_DEQUEUE_TIMEOUT;
  dev_info->dequeue_timeout_ns = DPAA2_EVENT_PORT_DEQUEUE_TIMEOUT_NS;
  dev_info->max_event_queues = priv->max_event_queues;
  dev_info->max_event_queue_flows = DPAA2_EVENT_MAX_QUEUE_FLOWS;
  dev_info->max_event_queue_priority_levels =
      DPAA2_EVENT_MAX_QUEUE_PRIORITY_LEVELS;
  dev_info->max_event_priority_levels = DPAA2_EVENT_MAX_EVENT_PRIORITY_LEVELS;
  ...
  dev_info->max_event_port_dequeue_depth = DPAA2_EVENT_MAX_PORT_DEQUEUE_DEPTH;
  dev_info->max_event_port_enqueue_depth = DPAA2_EVENT_MAX_PORT_ENQUEUE_DEPTH;
  ...
  dev_info->event_dev_cap =
      ...
      RTE_EVENT_DEV_CAP_QUEUE_ALL_TYPES | RTE_EVENT_DEV_CAP_CARRY_FLOW_ID |
      RTE_EVENT_DEV_CAP_MAINTENANCE_FREE;
}

static int dpaa2_eventdev_configure(const struct rte_eventdev *dev) {
  struct dpaa2_eventdev *priv = dev->data->dev_private;
  ...
  DPAA2_EVENTDEV_DEBUG("Configured eventdev devid=%d", dev->data->dev_id);
  return 0;
}

static int dpaa2_eventdev_start(struct rte_eventdev *dev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static void dpaa2_eventdev_stop(struct rte_eventdev *dev) {
  EVENTDEV_INIT_FUNC_TRACE();

  RTE_SET_USED(dev);
}

static int dpaa2_eventdev_close(struct rte_eventdev *dev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
dpaa2_eventdev_queue_def_conf(struct rte_eventdev *dev, uint8_t queue_id,
  ...
  queue_conf->nb_atomic_order_sequences = DPAA2_EVENT_QUEUE_ORDER_SEQUENCES;
...
static void dpaa2_eventdev_queue_release(struct rte_eventdev *dev,
                                         uint8_t queue_id) {
  EVENTDEV_INIT_FUNC_TRACE();
...
dpaa2_eventdev_port_def_conf(struct rte_eventdev *dev, uint8_t port_id,
  ...
  port_conf->dequeue_depth = DPAA2_EVENT_MAX_PORT_DEQUEUE_DEPTH;
...
static void dpaa2_eventdev_port_release(void *port) {
...
static int dpaa2_eventdev_port_link(struct rte_eventdev *dev, void *port,
...
static int dpaa2_eventdev_port_unlink(struct rte_eventdev *dev, void *port,
...
static int dpaa2_eventdev_timeout_ticks(struct rte_eventdev *dev, uint64_t ns,
                                        uint64_t *timeout_ticks) {
...
static void dpaa2_eventdev_dump(struct rte_eventdev *dev, FILE *f) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa2_eventdev_eth_caps_get(const struct rte_eventdev *dev,
...
static int dpaa2_eventdev_eth_queue_add_all(
  ...
  for (i = 0; i < eth_dev->data->nb_rx_queues; i++) {
    ret = dpaa2_eth_eventq_attach(eth_dev, i, dpcon, queue_conf);
    if (ret) {
      DPAA2_EVENTDEV_ERR("Event queue attach failed: err(%d)", ret);
      goto fail;
    }
  }
  return 0;
...
static int dpaa2_eventdev_eth_queue_add(
    const struct rte_eventdev *dev, const struct rte_eth_dev *eth_dev,
    int32_t rx_queue_id,
    const struct rte_event_eth_rx_adapter_queue_conf *queue_conf) {
  struct dpaa2_eventdev *priv = dev->data->dev_private;
  ...
  if (rx_queue_id == -1)
    return dpaa2_eventdev_eth_queue_add_all(dev, eth_dev, queue_conf);

  ret = dpaa2_eth_eventq_attach(eth_dev, rx_queue_id, dpcon, queue_conf);
  if (ret) {
    DPAA2_EVENTDEV_ERR("Event queue attach failed: err(%d)", ret);
    return ret;
  }
...
static int dpaa2_eventdev_eth_queue_del_all(const struct rte_eventdev *dev,
                                            const struct rte_eth_dev *eth_dev) {
  int i, ret;
  ...
  for (i = 0; i < eth_dev->data->nb_rx_queues; i++) {
    ...
    if (ret) {
      DPAA2_EVENTDEV_ERR("Event queue detach failed: err(%d)", ret);
      return ret;
    }
  }
...
static int dpaa2_eventdev_eth_queue_del(const struct rte_eventdev *dev,
                                        const struct rte_eth_dev *eth_dev,
                                        int32_t rx_queue_id) {
  int ret;
...
static int dpaa2_eventdev_eth_start(const struct rte_eventdev *dev,
                                    const struct rte_eth_dev *eth_dev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa2_eventdev_eth_stop(const struct rte_eventdev *dev,
                                   const struct rte_eth_dev *eth_dev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa2_eventdev_crypto_caps_get(const struct rte_eventdev *dev,
...
static int dpaa2_eventdev_crypto_queue_add(
    const struct rte_eventdev *dev, const struct rte_cryptodev *cryptodev,
    int32_t rx_queue_id, const struct rte_event *ev) {
  struct dpaa2_eventdev *priv = dev->data->dev_private;
  uint8_t ev_qid = ev->queue_id;
  struct dpaa2_dpcon_dev *dpcon = priv->evq_info[ev_qid].dpcon;
  int ret;
  ...
  if (rx_queue_id == -1)
    return dpaa2_eventdev_crypto_queue_add_all(dev, cryptodev, ev);

  ret = dpaa2_sec_eventq_attach(cryptodev, rx_queue_id, dpcon, ev);
  if (ret) {
    DPAA2_EVENTDEV_ERR("dpaa2_sec_eventq_attach failed: ret: %d\n", ret);
    return ret;
  }
...
static int dpaa2_eventdev_crypto_start(const struct rte_eventdev *dev,
                                       const struct rte_cryptodev *cryptodev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa2_eventdev_crypto_stop(const struct rte_eventdev *dev,
                                      const struct rte_cryptodev *cryptodev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa2_eventdev_tx_adapter_create(uint8_t id,
...
static int dpaa2_eventdev_tx_adapter_caps(const struct rte_eventdev *dev,
                                          const struct rte_eth_dev *eth_dev,
                                          uint32_t *caps) {
  RTE_SET_USED(dev);
  RTE_SET_USED(eth_dev);
...
static uint16_t dpaa2_eventdev_txa_enqueue_same_dest(void *port,
                                                     struct rte_event ev[],
                                                     uint16_t nb_events) {
...
static uint16_t dpaa2_eventdev_txa_enqueue(void *port, struct rte_event ev[],
                                           uint16_t nb_events) {
...
static int dpaa2_eventdev_setup_dpci(struct dpaa2_dpci_dev *dpci_dev,
                                     struct dpaa2_dpcon_dev *dpcon_dev) {
  struct dpci_rx_queue_cfg rx_queue_cfg;
  int ret, i;

  /*Do settings to get the frame on a DPCON object*/
  rx_queue_cfg.options = DPCI_QUEUE_OPT_DEST | DPCI_QUEUE_OPT_USER_CTX;
  ...
  for (i = 0; i < DPAA2_EVENT_DPCI_MAX_QUEUES; i++) {
    ...
    ret = dpci_set_rx_queue(&dpci_dev->dpci, CMD_PRI_LOW, dpci_dev->token, i,
                            &rx_queue_cfg);
    if (ret) {
      DPAA2_EVENTDEV_ERR("DPCI Rx queue setup failed: err(%d)", ret);
      return ret;
    }
  }
  return 0;
...
static int dpaa2_eventdev_create(const char *name) {
  ...
  eventdev = rte_event_pmd_vdev_init(name, sizeof(struct dpaa2_eventdev),
                                     rte_socket_id());
  if (eventdev == NULL) {
...
static int dpaa2_eventdev_destroy(const char *name) {
  struct rte_eventdev *eventdev;
  struct dpaa2_eventdev *priv;
  int i;
...
static int dpaa2_eventdev_probe(struct rte_vdev_device *vdev) {
  const char *name;
...
static int dpaa2_eventdev_remove(struct rte_vdev_device *vdev) {
  const char *name;
...
static struct rte_vdev_driver vdev_eventdev_dpaa2_pmd = {
    .probe = dpaa2_eventdev_probe, .remove = dpaa2_eventdev_remove};
