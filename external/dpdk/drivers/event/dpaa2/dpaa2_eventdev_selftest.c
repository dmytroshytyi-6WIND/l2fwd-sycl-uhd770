...
static int testsuite_setup(void) {
  ...
  if (evdev < 0) {
    dpaa2_evdev_dbg("%d: Eventdev %s not found - creating.", __LINE__,
                    eventdev_name);
    if (rte_vdev_init(eventdev_name, NULL) < 0) {
      dpaa2_evdev_err("Error creating eventdev %s", eventdev_name);
      return -1;
    }
    evdev = rte_event_dev_get_dev_id(eventdev_name);
    if (evdev < 0) {
...
static int _eventdev_setup(int mode) {
  int i, ret;
  ...
  if (mode == TEST_EVENTDEV_SETUP_PRIORITY) {
    if (queue_count > 8) {
      dpaa2_evdev_err("test expects the unique priority per queue");
      return -ENOTSUP;
    }
...
static int eventdev_setup(void) {
  return _eventdev_setup(TEST_EVENTDEV_SETUP_DEFAULT);
}

static void eventdev_teardown(void) {
...
static int inject_events(uint32_t flow_id, uint8_t event_type,
                         uint8_t sub_event_type, uint8_t sched_type,
                         uint8_t queue, uint8_t port, unsigned int events) {
  struct rte_mbuf *m;
  unsigned int i;
...
static int check_excess_events(uint8_t port) {
  int i;
...
static int generate_random_events(const unsigned int total_events) {
  struct rte_event_dev_info info;
  unsigned int i;
  int ret;
...
static int validate_event(struct rte_event *ev) {
  struct event_attr *attr;
...
static int consume_events(uint8_t port, const uint32_t total_events,
...
static int validate_simple_enqdeq(uint32_t index, uint8_t port,
  ...
  RTE_SET_USED(port);
  RTE_TEST_ASSERT_EQUAL(index, attr->seq, "index=%d != seqn=%d", index,
                        attr->seq);
  return 0;
}

static int test_simple_enqdeq(uint8_t sched_type) {
  int ret;
...
static int wait_workers_to_join(int lcore, const rte_atomic32_t *count) {
  ...
  while (rte_eal_get_lcore_state(lcore) != WAIT) {
    ...
    if (new_cycles - print_cycles > rte_get_timer_hz()) {
      dpaa2_evdev_dbg("\r%s: events %d", __func__, rte_atomic32_read(count));
      print_cycles = new_cycles;
    }
    if (new_cycles - cycles > rte_get_timer_hz() * 10) {
      dpaa2_evdev_info("%s: No schedules for seconds, deadlock (%d)", __func__,
...
static int launch_workers_and_wait(int (*main_worker)(void *),
                                   int (*workers)(void *),
                                   uint32_t total_events, uint8_t nb_workers,
                                   uint8_t sched_type) {
  uint8_t port = 0;
  int w_lcore;
  int ret;
...
static int test_multi_queue_enq_multi_port_deq(void) {
  ...
  if (!nr_ports) {
    dpaa2_evdev_err("%s: Not enough ports=%d or workers=%d", __func__, nr_ports,
                    rte_lcore_count() - 1);
    return 0;
  }
...
static int test_queue_to_port_multi_link(void) {
  ...
  if (nr_ports < 2) {
    dpaa2_evdev_err("%s: Not enough ports to test ports=%d", __func__,
                    nr_ports);
    return 0;
  }
...
int test_eventdev_dpaa2(void) {
  ...
  DPAA2_TEST_RUN(eventdev_setup, eventdev_teardown, test_simple_enqdeq_atomic);
  DPAA2_TEST_RUN(eventdev_setup, eventdev_teardown,
                 test_simple_enqdeq_parallel);
  DPAA2_TEST_RUN(eventdev_setup, eventdev_teardown,
                 test_multi_queue_enq_single_port_deq);
  DPAA2_TEST_RUN(eventdev_setup, eventdev_teardown, test_dev_stop_flush);
  DPAA2_TEST_RUN(eventdev_setup, eventdev_teardown,
