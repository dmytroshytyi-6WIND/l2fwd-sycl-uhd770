...
static int dpaa_event_dequeue_timeout_ticks(struct rte_eventdev *dev,
                                            uint64_t ns,
...
static int dpaa_event_dequeue_timeout_ticks_intr(struct rte_eventdev *dev,
                                                 uint64_t ns,
                                                 uint64_t *timeout_ticks) {
  RTE_SET_USED(dev);
...
static void dpaa_eventq_portal_add(u16 ch_id) {
  uint32_t sdqcr;
...
static uint16_t dpaa_event_enqueue_burst(void *port,
...
static uint16_t dpaa_event_enqueue(void *port, const struct rte_event *ev) {
...
static inline int dpaa_event_dequeue_wait(uint64_t timeout_ticks) {
  int fd_qman, nfds;
  int ret;
  fd_set readset;
   ...
   * potential nastiness.
   */
  struct timeval tv = {.tv_sec = timeout_ticks / 1000000,
...
static uint16_t dpaa_event_dequeue_burst(void *port, struct rte_event ev[],
                                         uint16_t nb_events,
                                         uint64_t timeout_ticks) {
  int ret;
  u16 ch_id;
...
static uint16_t dpaa_event_dequeue(void *port, struct rte_event *ev,
                                   uint64_t timeout_ticks) {
  return dpaa_event_dequeue_burst(port, ev, 1, timeout_ticks);
}

static uint16_t dpaa_event_dequeue_burst_intr(void *port, struct rte_event ev[],
                                              uint16_t nb_events,
                                              uint64_t timeout_ticks) {
  int ret;
  u16 ch_id;
...
static uint16_t dpaa_event_dequeue_intr(void *port, struct rte_event *ev,
                                        uint64_t timeout_ticks) {
  return dpaa_event_dequeue_burst_intr(port, ev, 1, timeout_ticks);
}

static void dpaa_event_dev_info_get(struct rte_eventdev *dev,
                                    struct rte_event_dev_info *dev_info) {
  EVENTDEV_INIT_FUNC_TRACE();

  RTE_SET_USED(dev);
  dev_info->driver_name = "event_dpaa1";
  dev_info->min_dequeue_timeout_ns = DPAA_EVENT_MIN_DEQUEUE_TIMEOUT;
  dev_info->max_dequeue_timeout_ns = DPAA_EVENT_MAX_DEQUEUE_TIMEOUT;
  dev_info->dequeue_timeout_ns = DPAA_EVENT_PORT_DEQUEUE_TIMEOUT_NS;
  dev_info->max_event_queues = DPAA_EVENT_MAX_QUEUES;
  dev_info->max_event_queue_flows = DPAA_EVENT_MAX_QUEUE_FLOWS;
  dev_info->max_event_queue_priority_levels =
      DPAA_EVENT_MAX_QUEUE_PRIORITY_LEVELS;
  dev_info->max_event_priority_levels = DPAA_EVENT_MAX_EVENT_PRIORITY_LEVELS;
  dev_info->max_event_ports = DPAA_EVENT_MAX_EVENT_PORT;
  dev_info->max_event_port_dequeue_depth = DPAA_EVENT_MAX_PORT_DEQUEUE_DEPTH;
  dev_info->max_event_port_enqueue_depth = DPAA_EVENT_MAX_PORT_ENQUEUE_DEPTH;
  /*
   * TODO: Need to find out that how to fetch this info
   * from kernel or somewhere else.
   */
  dev_info->max_num_events = DPAA_EVENT_MAX_NUM_EVENTS;
...
static int dpaa_event_dev_configure(const struct rte_eventdev *dev) {
  ...
  ch_id = rte_malloc("dpaa-channels", sizeof(uint32_t) * priv->nb_event_queues,
                     RTE_CACHE_LINE_SIZE);
  if (ch_id == NULL) {
  ...
  memset(&priv->ports[0], 0, sizeof(struct dpaa_port) * priv->nb_event_ports);
  ...
  for (i = 0; i < priv->nb_event_ports; i++) {
    if (priv->intr_mode) {
      ...
      priv->ports[i].timeout_us =
          (priv->dequeue_timeout_ns * cycles_per_second) / NS_PER_S;
    }
  }
...
static int dpaa_event_dev_start(struct rte_eventdev *dev) {
  EVENTDEV_INIT_FUNC_TRACE();
  RTE_SET_USED(dev);
...
static void dpaa_event_dev_stop(struct rte_eventdev *dev) {
  EVENTDEV_INIT_FUNC_TRACE();
  RTE_SET_USED(dev);
}

static int dpaa_event_dev_close(struct rte_eventdev *dev) {
  EVENTDEV_INIT_FUNC_TRACE();
  RTE_SET_USED(dev);
...
static void dpaa_event_queue_def_conf(struct rte_eventdev *dev,
                                      uint8_t queue_id,
...
static void dpaa_event_queue_release(struct rte_eventdev *dev,
                                     uint8_t queue_id) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa_event_port_setup(struct rte_eventdev *dev, uint8_t port_id,
...
static void dpaa_event_port_release(void *port) {
  EVENTDEV_INIT_FUNC_TRACE();

  RTE_SET_USED(port);
}

static int dpaa_event_port_link(struct rte_eventdev *dev, void *port,
  ...
  for (i = 0; i < nb_links; i++) {
    ...
    if ((event_queue->event_queue_cfg & RTE_EVENT_QUEUE_CFG_SINGLE_LINK) &&
...
static int dpaa_event_port_unlink(struct rte_eventdev *dev, void *port,
...
static int dpaa_event_eth_rx_adapter_caps_get(const struct rte_eventdev *dev,
...
static int dpaa_event_eth_rx_adapter_queue_add(
  ...
  if (rx_queue_id == -1) {
    for (i = 0; i < dpaa_intf->nb_rx_queues; i++) {
      ret = dpaa_eth_eventq_attach(eth_dev, i, ch_id, queue_conf);
      if (ret) {
        DPAA_EVENTDEV_ERR("Event Queue attach failed:%d\n", ret);
...
dpaa_event_eth_rx_adapter_queue_del(const struct rte_eventdev *dev,
  ...
  if (rx_queue_id == -1) {
    for (i = 0; i < dpaa_intf->nb_rx_queues; i++) {
      ...
      if (ret)
        DPAA_EVENTDEV_ERR("Event Queue detach failed:%d\n", ret);
    }
...
static int dpaa_event_eth_rx_adapter_start(const struct rte_eventdev *dev,
                                           const struct rte_eth_dev *eth_dev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa_event_eth_rx_adapter_stop(const struct rte_eventdev *dev,
                                          const struct rte_eth_dev *eth_dev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa_eventdev_crypto_caps_get(const struct rte_eventdev *dev,
...
dpaa_eventdev_crypto_queue_add_all(const struct rte_eventdev *dev,
  ...
  for (i = 0; i < cryptodev->data->nb_queue_pairs; i++) {
    ret = dpaa_sec_eventq_attach(cryptodev, i, ch_id, ev);
    if (ret) {
...
static int dpaa_eventdev_crypto_queue_add(const struct rte_eventdev *dev,
                                          const struct rte_cryptodev *cryptodev,
                                          int32_t rx_queue_id,
                                          const struct rte_event *ev) {
  struct dpaa_eventdev *priv = dev->data->dev_private;
  ...
  if (rx_queue_id == -1)
    return dpaa_eventdev_crypto_queue_add_all(dev, cryptodev, ev);

  ret = dpaa_sec_eventq_attach(cryptodev, rx_queue_id, ch_id, ev);
  if (ret) {
    DPAA_EVENTDEV_ERR("dpaa_sec_eventq_attach failed: ret: %d\n", ret);
    return ret;
  }
...
dpaa_eventdev_crypto_queue_del_all(const struct rte_eventdev *dev,
  ...
  for (i = 0; i < cdev->data->nb_queue_pairs; i++) {
    ...
    if (ret) {
      DPAA_EVENTDEV_ERR("dpaa_sec_eventq_detach failed:ret %d\n", ret);
      return ret;
    }
  }
...
static int dpaa_eventdev_crypto_queue_del(const struct rte_eventdev *dev,
                                          const struct rte_cryptodev *cryptodev,
                                          int32_t rx_queue_id) {
  int ret;
...
static int dpaa_eventdev_crypto_start(const struct rte_eventdev *dev,
                                      const struct rte_cryptodev *cryptodev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa_eventdev_crypto_stop(const struct rte_eventdev *dev,
                                     const struct rte_cryptodev *cryptodev) {
  EVENTDEV_INIT_FUNC_TRACE();
...
static int dpaa_eventdev_tx_adapter_create(uint8_t id,
...
static int dpaa_eventdev_tx_adapter_caps(const struct rte_eventdev *dev,
                                         const struct rte_eth_dev *eth_dev,
                                         uint32_t *caps) {
  RTE_SET_USED(dev);
  RTE_SET_USED(eth_dev);
...
static uint16_t dpaa_eventdev_txa_enqueue_same_dest(void *port,
                                                    struct rte_event ev[],
                                                    uint16_t nb_events) {
...
static uint16_t dpaa_eventdev_txa_enqueue(void *port, struct rte_event ev[],
                                          uint16_t nb_events) {
...
static int flag_check_handler(__rte_unused const char *key, const char *value,
...
static int dpaa_event_check_flags(const char *params) {
  ...
  if (rte_kvargs_process(kvlist, DISABLE_INTR_MODE, flag_check_handler, NULL) <
      0) {
    rte_kvargs_free(kvlist);
    return 0;
  }
...
static int dpaa_event_dev_create(const char *name, const char *params) {
  struct rte_eventdev *eventdev;
  struct dpaa_eventdev *priv;

  eventdev = rte_event_pmd_vdev_init(name, sizeof(struct dpaa_eventdev),
                                     rte_socket_id());
  if (eventdev == NULL) {
  ...
  if (dpaa_event_check_flags(params)) {
    ...
    priv->intr_mode = 1;
    eventdev->dev_ops->timeout_ticks = dpaa_event_dequeue_timeout_ticks_intr;
...
static int dpaa_event_dev_probe(struct rte_vdev_device *vdev) {
  const char *name;
  const char *params;
...
static int dpaa_event_dev_remove(struct rte_vdev_device *vdev) {
  const char *name;
...
static struct rte_vdev_driver vdev_eventdev_dpaa_pmd = {
    .probe = dpaa_event_dev_probe, .remove = dpaa_event_dev_remove};
