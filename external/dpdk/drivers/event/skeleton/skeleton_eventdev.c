...
static uint16_t skeleton_eventdev_enqueue(void *port,
...
static uint16_t skeleton_eventdev_enqueue_burst(void *port,
                                                const struct rte_event ev[],
                                                uint16_t nb_events) {
  struct skeleton_port *sp = port;
...
static uint16_t skeleton_eventdev_dequeue(void *port, struct rte_event *ev,
...
static uint16_t skeleton_eventdev_dequeue_burst(void *port,
                                                struct rte_event ev[],
                                                uint16_t nb_events,
                                                uint64_t timeout_ticks) {
  struct skeleton_port *sp = port;
...
static void skeleton_eventdev_info_get(struct rte_eventdev *dev,
  ...
  dev_info->event_dev_cap =
      ...
      RTE_EVENT_DEV_CAP_EVENT_QOS | RTE_EVENT_DEV_CAP_CARRY_FLOW_ID |
      RTE_EVENT_DEV_CAP_MAINTENANCE_FREE;
}

static int skeleton_eventdev_configure(const struct rte_eventdev *dev) {
...
static int skeleton_eventdev_start(struct rte_eventdev *dev) {
  struct skeleton_eventdev *skel = skeleton_pmd_priv(dev);
...
static void skeleton_eventdev_stop(struct rte_eventdev *dev) {
  struct skeleton_eventdev *skel = skeleton_pmd_priv(dev);
...
static int skeleton_eventdev_close(struct rte_eventdev *dev) {
  struct skeleton_eventdev *skel = skeleton_pmd_priv(dev);
...
static void skeleton_eventdev_queue_release(struct rte_eventdev *dev,
                                            uint8_t queue_id) {
  PMD_DRV_FUNC_TRACE();
...
static void skeleton_eventdev_port_release(void *port) {
  struct skeleton_port *sp = port;
  PMD_DRV_FUNC_TRACE();
...
skeleton_eventdev_port_setup(struct rte_eventdev *dev, uint8_t port_id,
  ...
  if (dev->data->ports[port_id] != NULL) {
    PMD_DRV_LOG(DEBUG, "Freeing memory prior to re-allocation %d", port_id);
    skeleton_eventdev_port_release(dev->data->ports[port_id]);
    dev->data->ports[port_id] = NULL;
  }

  /* Allocate event port memory */
  sp = rte_zmalloc_socket("eventdev port", sizeof(struct skeleton_port),
...
static int skeleton_eventdev_port_link(struct rte_eventdev *dev, void *port,
...
static int skeleton_eventdev_port_unlink(struct rte_eventdev *dev, void *port,
                                         uint8_t queues[],
                                         uint16_t nb_unlinks) {
  struct skeleton_port *sp = port;
  PMD_DRV_FUNC_TRACE();
...
static int skeleton_eventdev_timeout_ticks(struct rte_eventdev *dev,
                                           uint64_t ns,
                                           uint64_t *timeout_ticks) {
  struct skeleton_eventdev *skel = skeleton_pmd_priv(dev);
  uint32_t scale = 1;
...
static void skeleton_eventdev_dump(struct rte_eventdev *dev, FILE *f) {
  struct skeleton_eventdev *skel = skeleton_pmd_priv(dev);
...
static int skeleton_eventdev_init(struct rte_eventdev *eventdev) {
  ...
  PMD_DRV_LOG(DEBUG, "pci device (%x:%x) %u:%u:%u:%u", pci_dev->id.vendor_id,
              pci_dev->id.device_id, pci_dev->addr.domain, pci_dev->addr.bus,
              pci_dev->addr.devid, pci_dev->addr.function);

  PMD_DRV_LOG(INFO, "dev_id=%d socket_id=%d (%x:%x)", eventdev->data->dev_id,
              eventdev->data->socket_id, skel->vendor_id, skel->device_id);
...
static const struct rte_pci_id pci_id_skeleton_map[] = {
    {RTE_PCI_DEVICE(EVENTDEV_SKEL_VENDOR_ID, EVENTDEV_SKEL_PRODUCT_ID)},
    {
        .vendor_id = 0,
    },
};

static int event_skeleton_pci_probe(struct rte_pci_driver *pci_drv,
                                    struct rte_pci_device *pci_dev) {
  return rte_event_pmd_pci_probe(pci_drv, pci_dev,
                                 sizeof(struct skeleton_eventdev),
                                 skeleton_eventdev_init);
}

static int event_skeleton_pci_remove(struct rte_pci_device *pci_dev) {
...
static int skeleton_eventdev_create(const char *name, int socket_id) {
  struct rte_eventdev *eventdev;

  eventdev = rte_event_pmd_vdev_init(name, sizeof(struct skeleton_eventdev),
                                     socket_id);
  if (eventdev == NULL) {
...
static int skeleton_eventdev_probe(struct rte_vdev_device *vdev) {
  const char *name;
...
static int skeleton_eventdev_remove(struct rte_vdev_device *vdev) {
  const char *name;
...
static struct rte_vdev_driver vdev_eventdev_skeleton_pmd = {
    .probe = skeleton_eventdev_probe, .remove = skeleton_eventdev_remove};
