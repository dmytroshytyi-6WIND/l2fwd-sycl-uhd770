...
static struct rte_event_dev_info evdev_dlb2_default_info = {
    ...
    .event_dev_cap =
        (RTE_EVENT_DEV_CAP_QUEUE_QOS | RTE_EVENT_DEV_CAP_EVENT_QOS |
         RTE_EVENT_DEV_CAP_BURST_MODE | RTE_EVENT_DEV_CAP_DISTRIBUTED_SCHED |
         RTE_EVENT_DEV_CAP_IMPLICIT_RELEASE_DISABLE |
         RTE_EVENT_DEV_CAP_QUEUE_ALL_TYPES |
         RTE_EVENT_DEV_CAP_MAINTENANCE_FREE),
};

struct process_local_port_data dlb2_port[DLB2_MAX_NUM_PORTS_ALL]
                                        [DLB2_NUM_PORT_TYPES];

static void dlb2_free_qe_mem(struct dlb2_port *qm_port) {
  if (qm_port == NULL)
    return;
...
static void dlb2_init_queue_depth_thresholds(struct dlb2_eventdev *dlb2,
                                             int *qid_depth_thresholds) {
  int q;

  for (q = 0; q < DLB2_MAX_NUM_QUEUES(dlb2->version); q++) {
    if (qid_depth_thresholds[q] != 0)
      dlb2->ev_queues[q].depth_threshold = qid_depth_thresholds[q];
  }
}

static int dlb2_hw_query_resources(struct dlb2_eventdev *dlb2) {
  ...
  ret = dlb2_iface_get_num_resources(handle, &dlb2->hw_rsrc_query_results);
  if (ret) {
...
static int dlb2_string_to_int(int *result, const char *str) {
  long ret;
  char *endptr;
...
static int set_numa_node(const char *key __rte_unused, const char *value,
                         void *opaque) {
...
static int set_max_num_events(const char *key __rte_unused, const char *value,
                              void *opaque) {
  ...
  if (*max_num_events < 0 || *max_num_events > DLB2_MAX_NUM_LDB_CREDITS) {
...
static int set_num_dir_credits(const char *key __rte_unused, const char *value,
                               void *opaque) {
...
static int set_dev_id(const char *key __rte_unused, const char *value,
                      void *opaque) {
  int *dev_id = opaque;
  int ret;
...
static int set_cos(const char *key __rte_unused, const char *value,
                   void *opaque) {
  ...
  if (x != DLB2_COS_DEFAULT && (x < DLB2_COS_0 || x > DLB2_COS_3)) {
    DLB2_LOG_ERR("COS %d out of range, must be DLB2_COS_DEFAULT or 0-3\n", x);
    return -EINVAL;
  }
...
static int set_poll_interval(const char *key __rte_unused, const char *value,
                             void *opaque) {
...
static int set_sw_credit_quanta(const char *key __rte_unused, const char *value,
                                void *opaque) {
...
static int set_hw_credit_quanta(const char *key __rte_unused, const char *value,
                                void *opaque) {
...
static int set_default_depth_thresh(const char *key __rte_unused,
...
static int set_vector_opts_enab(const char *key __rte_unused, const char *value,
                                void *opaque) {
...
static int set_qid_depth_thresh(const char *key __rte_unused, const char *value,
                                void *opaque) {
  ...
  if (first > last || first < 0 || last >= DLB2_MAX_NUM_QUEUES(DLB2_HW_V2)) {
...
static int set_qid_depth_thresh_v2_5(const char *key __rte_unused,
                                     const char *value, void *opaque) {
  struct dlb2_qid_depth_thresholds *qid_thresh = opaque;
  int first, last, thresh, i;
  ...
  if (first > last || first < 0 || last >= DLB2_MAX_NUM_QUEUES(DLB2_HW_V2_5)) {
    DLB2_LOG_ERR("Error parsing qid depth devarg, invalid qid value\n");
    return -EINVAL;
  }
...
static void dlb2_eventdev_info_get(struct rte_eventdev *dev,
  ...
  if (dlb2->version == DLB2_HW_V2_5) {
    evdev_dlb2_default_info.max_num_events += dlb2->max_credits;
  } else {
    evdev_dlb2_default_info.max_num_events += dlb2->max_ldb_credits;
  }
  evdev_dlb2_default_info.max_event_queues = RTE_MIN(
      evdev_dlb2_default_info.max_event_queues, RTE_EVENT_MAX_QUEUES_PER_DEV);

  evdev_dlb2_default_info.max_num_events = RTE_MIN(
      evdev_dlb2_default_info.max_num_events, dlb2->max_num_events_override);
...
dlb2_hw_create_sched_domain(struct dlb2_hw_dev *handle,
  ...
  case DLB2_COS_0:
    ...
    cfg->num_cos_ldb_ports[0] = resources_asked->num_ldb_ports;
    break;
  case DLB2_COS_1:
    cfg->num_ldb_ports = 0; /* no don't care ports */
  ...
  case DLB2_COS_3:
    cfg->num_ldb_ports = 0; /* no don't care ports */
    cfg->num_cos_ldb_ports[3] = resources_asked->num_ldb_ports;
    break;
  case DLB2_COS_DEFAULT:
    /* all ldb ports are don't care ports from a cos perspective */
    cfg->num_ldb_ports = resources_asked->num_ldb_ports;
    break;
  }
  ...
  cfg->num_atomic_inflights =
      DLB2_NUM_ATOMIC_INFLIGHTS_PER_QUEUE * cfg->num_ldb_queues;

  cfg->num_hist_list_entries =
      resources_asked->num_ldb_ports * DLB2_NUM_HIST_LIST_ENTRIES_PER_LDB_PORT;

  if (device_version == DLB2_HW_V2_5) {
    DLB2_LOG_DBG("sched domain create - ldb_qs=%d, ldb_ports=%d, dir_ports=%d, "
                 "atomic_inflights=%d, hist_list_entries=%d, credits=%d\n",
                 cfg->num_ldb_queues, resources_asked->num_ldb_ports,
                 cfg->num_dir_ports, cfg->num_atomic_inflights,
                 cfg->num_hist_list_entries, cfg->num_credits);
  } else {
    DLB2_LOG_DBG("sched domain create - ldb_qs=%d, ldb_ports=%d, dir_ports=%d, "
                 "atomic_inflights=%d, hist_list_entries=%d, ldb_credits=%d, "
                 "dir_credits=%d\n",
                 cfg->num_ldb_queues, resources_asked->num_ldb_ports,
                 cfg->num_dir_ports, cfg->num_atomic_inflights,
                 cfg->num_hist_list_entries, cfg->num_ldb_credits,
                 cfg->num_dir_credits);
  }
...
static void dlb2_hw_reset_sched_domain(const struct rte_eventdev *dev,
                                       bool reconfig) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
   ...
   */
  config_state = (reconfig) ? DLB2_PREV_CONFIGURED : DLB2_NOT_CONFIGURED;
...
static int dlb2_eventdev_configure(const struct rte_eventdev *dev) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
  struct dlb2_hw_dev *handle = &dlb2->qm_instance;
  ...
  if (config->nb_event_queues > rsrcs->num_queues) {
    DLB2_LOG_ERR("nb_event_queues parameter (%d) exceeds the QM device's "
                 "capabilities (%d).\n",
                 config->nb_event_queues, rsrcs->num_queues);
    return -EINVAL;
  }
  if (config->nb_event_ports > (rsrcs->num_ldb_ports + rsrcs->num_dir_ports)) {
    DLB2_LOG_ERR("nb_event_ports parameter (%d) exceeds the QM device's "
                 "capabilities (%d).\n",
  ...
  if (config->nb_events_limit > rsrcs->nb_events_limit) {
    DLB2_LOG_ERR("nb_events_limit parameter (%d) exceeds the QM device's "
                 "capabilities (%d).\n",
                 config->nb_events_limit, rsrcs->nb_events_limit);
    return -EINVAL;
  }
  ...
  if (config->event_dev_cfg & RTE_EVENT_DEV_CFG_PER_DEQUEUE_TIMEOUT)
    ...
    dlb2->global_dequeue_wait_ticks = timeout32 * (rte_get_timer_hz() / 1E9);
  }
...
static void
dlb2_eventdev_port_default_conf_get(struct rte_eventdev *dev, uint8_t port_id,
...
static void
dlb2_eventdev_queue_default_conf_get(struct rte_eventdev *dev, uint8_t queue_id,
...
static int32_t dlb2_get_sn_allocation(struct dlb2_eventdev *dlb2, int group) {
  struct dlb2_hw_dev *handle = &dlb2->qm_instance;
...
static int dlb2_set_sn_allocation(struct dlb2_eventdev *dlb2, int group,
                                  int num) {
  struct dlb2_hw_dev *handle = &dlb2->qm_instance;
...
static int32_t dlb2_get_sn_occupancy(struct dlb2_eventdev *dlb2, int group) {
  struct dlb2_hw_dev *handle = &dlb2->qm_instance;
...
dlb2_program_sn_allocation(struct dlb2_eventdev *dlb2,
  ...
  for (i = 0; i < DLB2_NUM_SN_GROUPS; i++) {
     ...
     */
    if (grp_alloc[i] == sequence_numbers && grp_occupancy[i] < total_slots)
      return;
  }
  ...
  if (i == DLB2_NUM_SN_GROUPS) {
    DLB2_LOG_ERR("[%s()] No groups with %d sequence_numbers are available or "
                 "have free slots\n",
...
dlb2_hw_create_ldb_queue(struct dlb2_eventdev *dlb2,
  ...
  if (ret < 0) {
    DLB2_LOG_ERR(
        "dlb2: create LB event queue error, ret=%d (driver status: %s)\n", ret,
        dlb2_error_strings[cfg.response.status]);
    return -EINVAL;
  }
  ...
  DLB2_LOG_DBG("Created LB event queue %d, nb_inflights=%d, nb_seq=%d, qid "
               "inflights=%d\n",
               qm_qid, cfg.num_atomic_inflights, cfg.num_sequence_numbers,
...
static void dlb2_queue_link_teardown(struct dlb2_eventdev *dlb2,
                                     struct dlb2_eventdev_queue *ev_queue) {
  ...
  for (i = 0; i < dlb2->num_ports; i++) {
    ...
    for (j = 0; j < DLB2_MAX_NUM_QIDS_PER_LDB_CQ; j++) {
      if (!ev_port->link[j].valid || ev_port->link[j].queue_id != ev_queue->id)
        continue;
...
static int
dlb2_eventdev_queue_setup(struct rte_eventdev *dev, uint8_t ev_qid,
                          const struct rte_event_queue_conf *queue_conf) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
  struct dlb2_eventdev_queue *ev_queue;
  int ret;
...
static int dlb2_init_consume_qe(struct dlb2_port *qm_port, char *mz_name) {
  struct dlb2_cq_pop_qe *qe;

  qe = rte_zmalloc(mz_name,
                   DLB2_NUM_QES_PER_CACHE_LINE * sizeof(struct dlb2_cq_pop_qe),
                   RTE_CACHE_LINE_SIZE);
...
static int dlb2_init_int_arm_qe(struct dlb2_port *qm_port, char *mz_name) {
  struct dlb2_enqueue_qe *qe;

  qe = rte_zmalloc(mz_name,
                   DLB2_NUM_QES_PER_CACHE_LINE * sizeof(struct dlb2_enqueue_qe),
                   RTE_CACHE_LINE_SIZE);
...
static int dlb2_init_qe_mem(struct dlb2_port *qm_port, char *mz_name) {
  int ret, sz;
...
dlb2_event_enqueue_delayed(void *event_port, const struct rte_event events[]);

static inline uint16_t
dlb2_event_enqueue_burst_delayed(void *event_port,
                                 const struct rte_event events[], uint16_t num);

static inline uint16_t dlb2_event_enqueue_new_burst_delayed(
    void *event_port, const struct rte_event events[], uint16_t num);

static inline uint16_t dlb2_event_enqueue_forward_burst_delayed(
    void *event_port, const struct rte_event events[], uint16_t num);
 ...
 */
static void dlb2_hw_cq_bitmask_init(struct dlb2_port *qm_port,
                                    uint32_t cq_depth) {
...
static int dlb2_hw_create_ldb_port(struct dlb2_eventdev *dlb2,
                                   struct dlb2_eventdev_port *ev_port,
  ...
  if (dlb2->version == DLB2_HW_V2) {
    ...
    dir_credit_high_watermark =
        RTE_MIN(enqueue_depth, handle->cfg.num_dir_credits / dlb2->num_ports);
  } else
  ...
  if (ret < 0) {
    DLB2_LOG_ERR(
        "dlb2: dlb2_ldb_port_create error, ret=%d (driver status: %s)\n", ret,
        dlb2_error_strings[cfg.response.status]);
    goto error_exit;
  }
  ...
  snprintf(mz_name, sizeof(mz_name), "dlb2_ldb_port%d", ev_port->id);
  ...
  if (qm_port->token_pop_mode == DELAYED_POP) {
    ...
    dlb2->event_dev->enqueue_burst = dlb2_event_enqueue_burst_delayed;
    dlb2->event_dev->enqueue_new_burst = dlb2_event_enqueue_new_burst_delayed;
  ...
  if (dlb2->version == DLB2_HW_V2) {
    ...
    DLB2_LOG_DBG("dlb2: created ldb port %d, depth = %d, ldb credits=%d, dir "
                 "credits=%d\n",
                 qm_port_id, dequeue_depth, qm_port->ldb_credits,
                 qm_port->dir_credits);
  } else {
    ...
    DLB2_LOG_DBG("dlb2: created ldb port %d, depth = %d, credits=%d\n",
                 qm_port_id, dequeue_depth, qm_port->credits);
  }
...
#else
  if ((qm_port->cq_depth > 64) || (!rte_is_power_of_2(qm_port->cq_depth)) ||
...
static void dlb2_port_link_teardown(struct dlb2_eventdev *dlb2,
                                    struct dlb2_eventdev_port *ev_port) {
  struct dlb2_eventdev_queue *ev_queue;
  int i;
...
static int dlb2_hw_create_dir_port(struct dlb2_eventdev *dlb2,
                                   struct dlb2_eventdev_port *ev_port,
                                   uint32_t dequeue_depth,
                                   uint32_t enqueue_depth) {
  struct dlb2_hw_dev *handle = &dlb2->qm_instance;
  ...
  if (ret < 0) {
    DLB2_LOG_ERR(
        "dlb2: dlb2_dir_port_create error, ret=%d (driver status: %s)\n", ret,
        dlb2_error_strings[cfg.response.status]);
    goto error_exit;
  }
  ...
  snprintf(mz_name, sizeof(mz_name), "dlb2_dir_port%d", ev_port->id);
  ...
  if (dlb2->version == DLB2_HW_V2) {
    ...
    DLB2_LOG_DBG("dlb2: created dir port %d, depth = %d cr=%d,%d\n", qm_port_id,
                 dequeue_depth, dir_credit_high_watermark,
    ...
    DLB2_LOG_DBG("dlb2: created dir port %d, depth = %d cr=%d\n", qm_port_id,
                 dequeue_depth, credit_high_watermark);
  }
...
static int
dlb2_eventdev_port_setup(struct rte_eventdev *dev, uint8_t ev_port_id,
  ...
  if (!ev_port->qm_port.is_directed) {
    ret = dlb2_hw_create_ldb_port(dlb2, ev_port, port_conf->dequeue_depth,
                                  port_conf->enqueue_depth);
    if (ret < 0) {
    ...
    ret = dlb2_hw_create_dir_port(dlb2, ev_port, port_conf->dequeue_depth,
                                  port_conf->enqueue_depth);
    if (ret < 0) {
...
static int16_t dlb2_hw_map_ldb_qid_to_port(struct dlb2_hw_dev *handle,
                                           uint32_t qm_port_id, uint16_t qm_qid,
                                           uint8_t priority) {
  ...
  if (ret < 0) {
    ...
    DLB2_LOG_ERR("dlb2: grp=%d, qm_port=%d, qm_qid=%d prio=%d\n",
                 handle->domain_id, cfg.port_id, cfg.qid, cfg.priority);
  } else {
...
static int dlb2_event_queue_join_ldb(struct dlb2_eventdev *dlb2,
                                     struct dlb2_eventdev_port *ev_port,
                                     struct dlb2_eventdev_queue *ev_queue,
                                     uint8_t priority) {
  int first_avail = -1;
  ...
  ret = dlb2_hw_map_ldb_qid_to_port(&dlb2->qm_instance, ev_port->qm_port.id,
                                    ev_queue->qm_queue.id, priority);
...
static int32_t dlb2_hw_create_dir_queue(struct dlb2_eventdev *dlb2,
                                        struct dlb2_eventdev_queue *ev_queue,
                                        int32_t qm_port_id) {
  struct dlb2_hw_dev *handle = &dlb2->qm_instance;
  ...
  if (ret < 0) {
    DLB2_LOG_ERR(
        "dlb2: create DIR event queue error, ret=%d (driver status: %s)\n", ret,
        dlb2_error_strings[cfg.response.status]);
    return -EINVAL;
  }
...
static int dlb2_eventdev_dir_queue_setup(struct dlb2_eventdev *dlb2,
                                         struct dlb2_eventdev_queue *ev_queue,
                                         struct dlb2_eventdev_port *ev_port) {
  int32_t qm_qid;
...
static int dlb2_do_port_link(struct rte_eventdev *dev,
                             struct dlb2_eventdev_queue *ev_queue,
                             struct dlb2_eventdev_port *ev_port, uint8_t prio) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
  int err;
  ...
  if (err) {
    DLB2_LOG_ERR("port link failure for %s ev_q %d, ev_port %d\n",
                 ev_queue->qm_queue.is_directed ? "DIR" : "LDB", ev_queue->id,
                 ev_port->id);
...
static int dlb2_validate_port_link(struct dlb2_eventdev_port *ev_port,
                                   uint8_t queue_id, bool link_exists,
                                   int index) {
  ...
  if (ev_port->qm_port.is_directed && ev_port->num_links > 0 && !link_exists) {
...
static int dlb2_eventdev_port_link(struct rte_eventdev *dev, void *event_port,
...

{
  ...
  DLB2_LOG_DBG("Linking %u queues to %s port %d\n", nb_links,
               ev_port->qm_port.is_directed ? "DIR" : "LDB", ev_port->id);
...
static int16_t dlb2_hw_unmap_ldb_qid_from_port(struct dlb2_hw_dev *handle,
                                               uint32_t qm_port_id,
                                               uint16_t qm_qid) {
...
static int dlb2_event_queue_detach_ldb(struct dlb2_eventdev *dlb2,
                                       struct dlb2_eventdev_port *ev_port,
                                       struct dlb2_eventdev_queue *ev_queue) {
  int ret, i;
  ...
  for (i = 0; i < DLB2_MAX_NUM_QIDS_PER_LDB_CQ; i++) {
    if (ev_port->link[i].valid && ev_port->link[i].queue_id == ev_queue->id)
      break; /* found */
  }
  ...
  if (i == DLB2_MAX_NUM_QIDS_PER_LDB_CQ) {
    DLB2_LOG_DBG("dlb2: ignoring LB QID %d not mapped for qm_port %d.\n",
                 ev_queue->qm_queue.id, ev_port->qm_port.id);
    return 0;
  }

  ret = dlb2_hw_unmap_ldb_qid_from_port(&dlb2->qm_instance, ev_port->qm_port.id,
...
static int dlb2_eventdev_port_unlink(struct rte_eventdev *dev, void *event_port,
  ...
  for (i = 0; i < nb_unlinks; i++) {
    ...
    /* Does a link exist? */
    for (j = 0; j < DLB2_MAX_NUM_QIDS_PER_LDB_CQ; j++)
      if (ev_port->link[j].queue_id == queues[i] && ev_port->link[j].valid)
        break;
...
static int dlb2_eventdev_port_unlinks_in_progress(struct rte_eventdev *dev,
                                                  void *event_port) {
  struct dlb2_eventdev_port *ev_port = event_port;
  struct dlb2_eventdev *dlb2;
...
static int dlb2_eventdev_reapply_configuration(struct rte_eventdev *dev) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
  int ret, i;
  ...
  for (i = 0; i < dlb2->num_ports; i++) {
    ...
    if (ret < 0) {
      DLB2_LOG_ERR("dlb2: failed to reconfigure ev_port %d", i);
      return ret;
    }
  }
...
static int dlb2_eventdev_apply_port_links(struct rte_eventdev *dev) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
  int i;
...
static int dlb2_eventdev_start(struct rte_eventdev *dev) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
  struct dlb2_hw_dev *handle = &dlb2->qm_instance;
...
static inline uint32_t dlb2_port_credits_get(struct dlb2_port *qm_port,
  ...
  if (likely(credits &&
             __atomic_compare_exchange_n(qm_port->credit_pool[type], &credits,
                                         credits - batch_size, false,
...
dlb2_check_enqueue_sw_credits(struct dlb2_eventdev *dlb2,
  ...
  if (ev_port->inflight_credits < num) {
    ...
    if (sw_inflights + credit_update_quanta > dlb2->new_event_limit) {
      DLB2_INC_STAT(ev_port->stats.traffic.tx_nospc_new_event_limit, 1);
      rte_errno = -ENOSPC;
      return 1;
    }
    ...
    if (ev_port->inflight_credits < num) {
      DLB2_INC_STAT(ev_port->stats.traffic.tx_nospc_inflight_credits, 1);
      rte_errno = -ENOSPC;
      return 1;
    }
  }
...
static inline int dlb2_check_enqueue_hw_ldb_credits(struct dlb2_port *qm_port) {
  if (unlikely(qm_port->cached_ldb_credits == 0)) {
    qm_port->cached_ldb_credits =
        dlb2_port_credits_get(qm_port, DLB2_LDB_QUEUE);
    if (unlikely(qm_port->cached_ldb_credits == 0)) {
      DLB2_INC_STAT(qm_port->ev_port->stats.traffic.tx_nospc_ldb_hw_credits, 1);
...
static inline int dlb2_check_enqueue_hw_dir_credits(struct dlb2_port *qm_port) {
  if (unlikely(qm_port->cached_dir_credits == 0)) {
    qm_port->cached_dir_credits =
        dlb2_port_credits_get(qm_port, DLB2_DIR_QUEUE);
    if (unlikely(qm_port->cached_dir_credits == 0)) {
      DLB2_INC_STAT(qm_port->ev_port->stats.traffic.tx_nospc_dir_hw_credits, 1);
...
static inline int dlb2_check_enqueue_hw_credits(struct dlb2_port *qm_port) {
  if (unlikely(qm_port->cached_credits == 0)) {
    qm_port->cached_credits =
        dlb2_port_credits_get(qm_port, DLB2_COMBINED_POOL);
    if (unlikely(qm_port->cached_credits == 0)) {
      DLB2_INC_STAT(qm_port->ev_port->stats.traffic.tx_nospc_hw_credits, 1);
...
static inline int dlb2_consume_qe_immediate(struct dlb2_port *qm_port,
                                            int num) {
  struct process_local_port_data *port_data;
  struct dlb2_cq_pop_qe *qe;
...
static inline void
dlb2_hw_do_enqueue(struct dlb2_port *qm_port, bool do_sfence,
                   struct process_local_port_data *port_data) {
...
static inline void dlb2_construct_token_pop_qe(struct dlb2_port *qm_port,
                                               int idx) {
...
static inline void dlb2_event_build_hcws(struct dlb2_port *qm_port,
                                         const struct rte_event ev[], int num,
                                         uint8_t *sched_type,
                                         uint8_t *queue_id) {
  struct dlb2_enqueue_qe *qe;
...
#define DLB2_QE_CMD_BYTE 7
    sse_qe[0] =
        _mm_insert_epi8(sse_qe[0], cmd_byte_map[qm_port->is_directed][ev[0].op],
                        DLB2_QE_CMD_BYTE);
    sse_qe[0] =
        _mm_insert_epi8(sse_qe[0], cmd_byte_map[qm_port->is_directed][ev[1].op],
                        DLB2_QE_CMD_BYTE + 8);
    sse_qe[1] =
        _mm_insert_epi8(sse_qe[1], cmd_byte_map[qm_port->is_directed][ev[2].op],
                        DLB2_QE_CMD_BYTE);
    sse_qe[1] =
        _mm_insert_epi8(sse_qe[1], cmd_byte_map[qm_port->is_directed][ev[3].op],
                        DLB2_QE_CMD_BYTE + 8);
     ...
     */
    sched_word[0] = EV_TO_DLB2_PRIO(ev[0].priority) << 10 | sched_type[0] << 8 |
                    queue_id[0];
    sched_word[1] = EV_TO_DLB2_PRIO(ev[1].priority) << 10 | sched_type[1] << 8 |
                    queue_id[1];
    sched_word[2] = EV_TO_DLB2_PRIO(ev[2].priority) << 10 | sched_type[2] << 8 |
                    queue_id[2];
    sched_word[3] = EV_TO_DLB2_PRIO(ev[3].priority) << 10 | sched_type[3] << 8 |
                    queue_id[3];
...
#define DLB2_QE_QID_SCHED_WORD 1
    sse_qe[0] =
        _mm_insert_epi16(sse_qe[0], sched_word[0], DLB2_QE_QID_SCHED_WORD);
    sse_qe[0] =
        _mm_insert_epi16(sse_qe[0], sched_word[1], DLB2_QE_QID_SCHED_WORD + 4);
    sse_qe[1] =
        _mm_insert_epi16(sse_qe[1], sched_word[2], DLB2_QE_QID_SCHED_WORD);
    sse_qe[1] =
        _mm_insert_epi16(sse_qe[1], sched_word[3], DLB2_QE_QID_SCHED_WORD + 4);
...
#define DLB2_QE_LOCK_ID_WORD 2
    sse_qe[0] = _mm_insert_epi16(
        sse_qe[0],
        (sched_type[0] == DLB2_SCHED_DIRECTED) ? sched_word[0] : ev[0].flow_id,
        DLB2_QE_LOCK_ID_WORD);
    sse_qe[0] = _mm_insert_epi16(
        sse_qe[0],
        (sched_type[1] == DLB2_SCHED_DIRECTED) ? sched_word[1] : ev[1].flow_id,
        DLB2_QE_LOCK_ID_WORD + 4);
    sse_qe[1] = _mm_insert_epi16(
        sse_qe[1],
        (sched_type[2] == DLB2_SCHED_DIRECTED) ? sched_word[2] : ev[2].flow_id,
        DLB2_QE_LOCK_ID_WORD);
    sse_qe[1] = _mm_insert_epi16(
        sse_qe[1],
        (sched_type[3] == DLB2_SCHED_DIRECTED) ? sched_word[3] : ev[3].flow_id,
        DLB2_QE_LOCK_ID_WORD + 4);
...
#define DLB2_QE_EV_TYPE_WORD 0
    sse_qe[0] = _mm_insert_epi16(sse_qe[0],
                                 ev[0].sub_event_type << 8 | ev[0].event_type,
                                 DLB2_QE_EV_TYPE_WORD);
    sse_qe[0] = _mm_insert_epi16(sse_qe[0],
                                 ev[1].sub_event_type << 8 | ev[1].event_type,
                                 DLB2_QE_EV_TYPE_WORD + 4);
    sse_qe[1] = _mm_insert_epi16(sse_qe[1],
                                 ev[2].sub_event_type << 8 | ev[2].event_type,
                                 DLB2_QE_EV_TYPE_WORD);
    sse_qe[1] = _mm_insert_epi16(sse_qe[1],
                                 ev[3].sub_event_type << 8 | ev[3].event_type,
                                 DLB2_QE_EV_TYPE_WORD + 4);
  ...
  case 1:
    for (i = 0; i < num; i++) {
      ...
      if (sched_type[i] == DLB2_SCHED_DIRECTED) {
        struct dlb2_msg_info *info = (struct dlb2_msg_info *)&qe[i].lock_id;
...
static inline int dlb2_event_enqueue_prep(struct dlb2_eventdev_port *ev_port,
  ...
  if (!qm_queue->is_directed) {
    ...
    case RTE_SCHED_TYPE_ORDERED:
      ...
      if (qm_queue->sched_type != RTE_SCHED_TYPE_ORDERED) {
        DLB2_LOG_ERR(
            "dlb2: tried to send ordered event to unordered queue %d\n",
            *queue_id);
        rte_errno = -EINVAL;
        return 1;
      }
...
static inline uint16_t
__dlb2_event_enqueue_burst(void *event_port, const struct rte_event events[],
                           uint16_t num, bool use_delayed) {
  struct dlb2_eventdev_port *ev_port = event_port;
  ...
  while (i < num) {
    ...
    for (; j < DLB2_NUM_QES_PER_CACHE_LINE && (i + j) < num; j++) {
      ...
      if (use_delayed && qm_port->token_pop_mode == DELAYED_POP &&
          (ev->op == RTE_EVENT_OP_FORWARD || ev->op == RTE_EVENT_OP_RELEASE) &&
      ...
      if (dlb2_event_enqueue_prep(ev_port, qm_port, ev, &sched_types[j],
                                  &queue_ids[j]))
        break;
    }
    ...
    dlb2_event_build_hcws(qm_port, &events[i], j - pop_offs, sched_types,
                          queue_ids);
...
static uint16_t dlb2_event_enqueue_burst(void *event_port,
                                         const struct rte_event events[],
                                         uint16_t num) {
  return __dlb2_event_enqueue_burst(event_port, events, num, false);
}

static uint16_t dlb2_event_enqueue_burst_delayed(
    void *event_port, const struct rte_event events[], uint16_t num) {
  return __dlb2_event_enqueue_burst(event_port, events, num, true);
}

static inline uint16_t dlb2_event_enqueue(void *event_port,
                                          const struct rte_event events[]) {
...
static uint16_t dlb2_event_enqueue_new_burst(void *event_port,
                                             const struct rte_event events[],
                                             uint16_t num) {
  return __dlb2_event_enqueue_burst(event_port, events, num, false);
}

static uint16_t dlb2_event_enqueue_new_burst_delayed(
    void *event_port, const struct rte_event events[], uint16_t num) {
  return __dlb2_event_enqueue_burst(event_port, events, num, true);
}

static uint16_t dlb2_event_enqueue_forward_burst(
    void *event_port, const struct rte_event events[], uint16_t num) {
  return __dlb2_event_enqueue_burst(event_port, events, num, false);
}

static uint16_t dlb2_event_enqueue_forward_burst_delayed(
    void *event_port, const struct rte_event events[], uint16_t num) {
  return __dlb2_event_enqueue_burst(event_port, events, num, true);
}

static void dlb2_event_release(struct dlb2_eventdev *dlb2, uint8_t port_id,
                               int n) {
  struct process_local_port_data *port_data;
  struct dlb2_eventdev_port *ev_port;
  struct dlb2_port *qm_port;
...
static inline void dlb2_port_credits_inc(struct dlb2_port *qm_port, int num) {
  uint32_t batch_size = qm_port->hw_credit_quanta;
  ...
  if (!qm_port->is_directed) {
    if (qm_port->dlb2->version == DLB2_HW_V2) {
      ...
      if (qm_port->cached_ldb_credits >= 2 * batch_size) {
        __atomic_fetch_add(qm_port->credit_pool[DLB2_LDB_QUEUE], batch_size,
                           __ATOMIC_SEQ_CST);
      ...
      if (qm_port->cached_credits >= 2 * batch_size) {
        __atomic_fetch_add(qm_port->credit_pool[DLB2_COMBINED_POOL], batch_size,
                           __ATOMIC_SEQ_CST);
    ...
    if (qm_port->dlb2->version == DLB2_HW_V2) {
      ...
      if (qm_port->cached_dir_credits >= 2 * batch_size) {
        __atomic_fetch_add(qm_port->credit_pool[DLB2_DIR_QUEUE], batch_size,
                           __ATOMIC_SEQ_CST);
...
static inline int dlb2_dequeue_wait(struct dlb2_eventdev *dlb2,
                                    struct dlb2_eventdev_port *ev_port,
                                    struct dlb2_port *qm_port, uint64_t timeout,
  ...
  if (elapsed_ticks >= timeout) {
    ...
    monitor_addr =
        (volatile uint64_t *)(volatile void *)&cq_base[qm_port->cq_idx];
...
dlb2_process_dequeue_qes(struct dlb2_eventdev_port *ev_port,
                         struct dlb2_port *qm_port, struct rte_event *events,
                         struct dlb2_dequeue_qe *qes, int cnt) {
  ...
  for (i = 0, num = 0; i < cnt; i++) {
    ...
    DLB2_INC_STAT(ev_port->stats.queue[evq_id].qid_depth[qe->qid_depth], 1);
...
static inline int dlb2_process_dequeue_four_qes(
    struct dlb2_eventdev_port *ev_port, struct dlb2_port *qm_port,
    struct rte_event *events, struct dlb2_dequeue_qe *qes) {
  int sched_type_map[] = {
      [DLB2_SCHED_ATOMIC] = RTE_SCHED_TYPE_ATOMIC,
      [DLB2_SCHED_UNORDERED] = RTE_SCHED_TYPE_PARALLEL,
      [DLB2_SCHED_ORDERED] = RTE_SCHED_TYPE_ORDERED,
   ...
   * them one at a time.
   */
  if (unlikely(qes[0].error || qes[1].error || qes[2].error || qes[3].error))
    return dlb2_process_dequeue_qes(ev_port, qm_port, events, qes, num_events);
...
#define DLB_EVENT_QUEUE_ID_BYTE 5
  sse_evt[0] = _mm_insert_epi8(sse_evt[0], qid_mappings[qes[0].qid],
                               DLB_EVENT_QUEUE_ID_BYTE);
  sse_evt[0] = _mm_insert_epi8(sse_evt[0], qid_mappings[qes[1].qid],
                               DLB_EVENT_QUEUE_ID_BYTE + 8);
  sse_evt[1] = _mm_insert_epi8(sse_evt[1], qid_mappings[qes[2].qid],
                               DLB_EVENT_QUEUE_ID_BYTE);
  sse_evt[1] = _mm_insert_epi8(sse_evt[1], qid_mappings[qes[3].qid],
                               DLB_EVENT_QUEUE_ID_BYTE + 8);
...
#define DLB_BYTE_SHIFT 8
  sse_evt[0] = _mm_insert_epi16(sse_evt[0],
  ...
  sse_evt[1] = _mm_insert_epi16(sse_evt[1],
...
#define DLB_EVENT_SUB_EV_TYPE_SHIFT 20
  sse_evt[0] = _mm_insert_epi32(
      sse_evt[0],
      qes[0].flow_id | qes[0].u.event_type.major << DLB_EVENT_EV_TYPE_SHIFT |
          qes[0].u.event_type.sub << DLB_EVENT_SUB_EV_TYPE_SHIFT,
      DLB_EVENT_EV_TYPE_DW);
  sse_evt[0] = _mm_insert_epi32(
      sse_evt[0],
      qes[1].flow_id | qes[1].u.event_type.major << DLB_EVENT_EV_TYPE_SHIFT |
          qes[1].u.event_type.sub << DLB_EVENT_SUB_EV_TYPE_SHIFT,
      DLB_EVENT_EV_TYPE_DW + 2);
  sse_evt[1] = _mm_insert_epi32(
      sse_evt[1],
      qes[2].flow_id | qes[2].u.event_type.major << DLB_EVENT_EV_TYPE_SHIFT |
          qes[2].u.event_type.sub << DLB_EVENT_SUB_EV_TYPE_SHIFT,
      DLB_EVENT_EV_TYPE_DW);
  sse_evt[1] = _mm_insert_epi32(
      sse_evt[1],
      qes[3].flow_id | qes[3].u.event_type.major << DLB_EVENT_EV_TYPE_SHIFT |
...
#define DLB_EVENT_SCHED_TYPE_SHIFT 6
  sse_evt[0] = _mm_insert_epi8(sse_evt[0],
                               sched_type_map[qes[0].sched_type]
                                   << DLB_EVENT_SCHED_TYPE_SHIFT,
                               DLB_EVENT_SCHED_TYPE_BYTE);
  sse_evt[0] = _mm_insert_epi8(sse_evt[0],
                               sched_type_map[qes[1].sched_type]
                                   << DLB_EVENT_SCHED_TYPE_SHIFT,
                               DLB_EVENT_SCHED_TYPE_BYTE + 8);
  sse_evt[1] = _mm_insert_epi8(sse_evt[1],
                               sched_type_map[qes[2].sched_type]
                                   << DLB_EVENT_SCHED_TYPE_SHIFT,
                               DLB_EVENT_SCHED_TYPE_BYTE);
  sse_evt[1] = _mm_insert_epi8(sse_evt[1],
                               sched_type_map[qes[3].sched_type]
                                   << DLB_EVENT_SCHED_TYPE_SHIFT,
                               DLB_EVENT_SCHED_TYPE_BYTE + 8);
  ...
  DLB2_INC_STAT(
      ev_port->stats.queue[events[0].queue_id].qid_depth[qes[0].qid_depth], 1);
  DLB2_INC_STAT(
      ev_port->stats.queue[events[1].queue_id].qid_depth[qes[1].qid_depth], 1);
  DLB2_INC_STAT(
      ev_port->stats.queue[events[2].queue_id].qid_depth[qes[2].qid_depth], 1);
  DLB2_INC_STAT(
      ev_port->stats.queue[events[3].queue_id].qid_depth[qes[3].qid_depth], 1);
...
static __rte_always_inline int dlb2_recv_qe_sparse(struct dlb2_port *qm_port,
                                                   struct dlb2_dequeue_qe *qe) {
...
static inline void
_process_deq_qes_vec_impl(struct dlb2_port *qm_port, struct rte_event *events,
                          __m128i v_qe_3, __m128i v_qe_2, __m128i v_qe_1,
                          __m128i v_qe_0, __m128i v_qe_meta,
                          __m128i v_qe_status, uint32_t valid_events) {
  ...
  __m128i v_sched_bits;
  {
    ...
    const __m128i v_sched_map =
        _mm_loadu_si128((const __m128i *)sched_type_map);
    __m128i v_sched_mask = _mm_loadu_si128((const __m128i *)&sched_and_mask);
    v_sched_bits = _mm_and_si128(v_qe_meta, v_sched_mask);
    __m128i v_sched_remapped = _mm_shuffle_epi8(v_sched_map, v_sched_bits);
    __m128i v_preshift = _mm_and_si128(v_sched_remapped, v_sched_mask);
  ...
  __m128i v_prio_done;
  {
    static const uint8_t prio_mask[16] = {
        0x00, 0x00, 0x00, 0x07 << 5, 0x00, 0x00, 0x00, 0x07 << 5,
        0x00, 0x00, 0x00, 0x07 << 5, 0x00, 0x00, 0x00, 0x07 << 5,
    };
    __m128i v_prio_mask = _mm_loadu_si128((const __m128i *)prio_mask);
  ...
  __m128i v_types_done;
  {
    ...
    __m128i v_event_mask = _mm_loadu_si128((const __m128i *)event_mask);
    __m128i v_sub_event_mask = _mm_loadu_si128((const __m128i *)sub_event_mask);
    __m128i v_flow_mask = _mm_loadu_si128((const __m128i *)flow_mask);
  ...
  case 4:
    ...
    DLB2_INC_STAT(qm_port->ev_port->stats.rx_sched_cnt[hw_sched3], 1);
    /* fallthrough */
  case 3:
    v_ev_2 = _mm_unpacklo_epi64(v_unpk_ev_23, v_qe_2);
    _mm_storeu_si128((__m128i *)&events[2], v_ev_2);
    DLB2_INC_STAT(qm_port->ev_port->stats.rx_sched_cnt[hw_sched2], 1);
    /* fallthrough */
  case 2:
    ...
    DLB2_INC_STAT(qm_port->ev_port->stats.rx_sched_cnt[hw_sched1], 1);
    /* fallthrough */
  case 1:
    v_ev_0 = _mm_unpacklo_epi64(v_unpk_ev_01, v_qe_0);
    _mm_storeu_si128((__m128i *)&events[0], v_ev_0);
    DLB2_INC_STAT(qm_port->ev_port->stats.rx_sched_cnt[hw_sched0], 1);
  }
}
...
dlb2_recv_qe_sparse_vec(struct dlb2_port *qm_port, void *events,
  ...
  uintptr_t qe_ptr_3 = (uintptr_t)&cq_addr[(idx + 12) & qm_port->cq_depth_mask];
  uintptr_t qe_ptr_2 = (uintptr_t)&cq_addr[(idx + 8) & qm_port->cq_depth_mask];
  uintptr_t qe_ptr_1 = (uintptr_t)&cq_addr[(idx + 4) & qm_port->cq_depth_mask];
  uintptr_t qe_ptr_0 = (uintptr_t)&cq_addr[(idx + 0) & qm_port->cq_depth_mask];
  ...
  _process_deq_qes_vec_impl(qm_port, events, v_qe_3, v_qe_2, v_qe_1, v_qe_0,
                            v_qe_meta, v_qe_status, count_new);

  return count_new;
}

static inline void dlb2_inc_cq_idx(struct dlb2_port *qm_port, int cnt) {
...
static inline int16_t dlb2_hw_dequeue_sparse(struct dlb2_eventdev *dlb2,
                                             struct dlb2_eventdev_port *ev_port,
  ...
  while (num < max_num) {
    ...
    if (use_scalar) {
      ...
      if (num_avail == DLB2_NUM_QES_PER_CACHE_LINE)
        n_iter = dlb2_process_dequeue_four_qes(ev_port, qm_port, &events[num],
                                               &qes[0]);
      else if (num_avail)
        n_iter = dlb2_process_dequeue_qes(ev_port, qm_port, &events[num],
                                          &qes[0], num_avail);
      if (n_iter != 0) {
        num += n_iter;
        ...
        m2_lshift = qm_port->cq_rolling_mask_2 << (64 - n_iter);
        qm_port->cq_rolling_mask = (m_rshift | m2_lshift);
        qm_port->cq_rolling_mask_2 = (m2_rshift | m_lshift);
      }
    } else { /* !use_scalar */
      num_avail = dlb2_recv_qe_sparse_vec(qm_port, &events[num], max_num - num);
      dlb2_inc_cq_idx(qm_port, num_avail << 2);
      num += num_avail;
    ...
    if (!num_avail) {
      if ((timeout == 0) || (num > 0))
        ...
        break;
      else if (dlb2_dequeue_wait(dlb2, ev_port, qm_port, timeout, start_ticks))
        break;
    }
  }
...
static __rte_always_inline int dlb2_recv_qe(struct dlb2_port *qm_port,
                                            struct dlb2_dequeue_qe *qe,
                                            uint8_t *offset) {
...
static inline int16_t dlb2_hw_dequeue(struct dlb2_eventdev *dlb2,
                                      struct dlb2_eventdev_port *ev_port,
                                      struct rte_event *events,
                                      uint16_t max_num,
                                      uint64_t dequeue_timeout_ticks) {
  ...
  while (num < max_num) {
    ...
    if (num_avail == DLB2_NUM_QES_PER_CACHE_LINE)
      num += dlb2_process_dequeue_four_qes(ev_port, qm_port, &events[num],
                                           &qes[offset]);
    else if (num_avail)
      num += dlb2_process_dequeue_qes(ev_port, qm_port, &events[num],
                                      &qes[offset], num_avail);
...
static uint16_t dlb2_event_dequeue_burst(void *event_port, struct rte_event *ev,
...
static uint16_t dlb2_event_dequeue(void *event_port, struct rte_event *ev,
                                   uint64_t wait) {
  return dlb2_event_dequeue_burst(event_port, ev, 1, wait);
}

static uint16_t dlb2_event_dequeue_burst_sparse(void *event_port,
                                                struct rte_event *ev,
                                                uint16_t num, uint64_t wait) {
  struct dlb2_eventdev_port *ev_port = event_port;
  struct dlb2_port *qm_port = &ev_port->qm_port;
...
static uint16_t dlb2_event_dequeue_sparse(void *event_port,
                                          struct rte_event *ev, uint64_t wait) {
  return dlb2_event_dequeue_burst_sparse(event_port, ev, 1, wait);
}

static void dlb2_flush_port(struct rte_eventdev *dev, int port_id) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
...
static uint32_t dlb2_get_ldb_queue_depth(struct dlb2_eventdev *dlb2,
...
static uint32_t dlb2_get_dir_queue_depth(struct dlb2_eventdev *dlb2,
                                         struct dlb2_eventdev_queue *queue) {
  struct dlb2_hw_dev *handle = &dlb2->qm_instance;
...
static bool dlb2_queue_is_empty(struct dlb2_eventdev *dlb2,
                                struct dlb2_eventdev_queue *queue) {
  return dlb2_get_queue_depth(dlb2, queue) == 0;
}

static bool dlb2_linked_queues_empty(struct dlb2_eventdev *dlb2) {
  int i;
...
static bool dlb2_queues_empty(struct dlb2_eventdev *dlb2) {
  int i;
...
static void dlb2_drain(struct rte_eventdev *dev) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
...
static void dlb2_eventdev_stop(struct rte_eventdev *dev) {
  struct dlb2_eventdev *dlb2 = dlb2_pmd_priv(dev);
...
static int dlb2_eventdev_close(struct rte_eventdev *dev) {
  dlb2_hw_reset_sched_domain(dev, false);

  return 0;
}

static void dlb2_eventdev_queue_release(struct rte_eventdev *dev, uint8_t id) {
  RTE_SET_USED(dev);
  RTE_SET_USED(id);
...
static void dlb2_eventdev_port_release(void *port) {
...
static int dlb2_eventdev_timeout_ticks(struct rte_eventdev *dev, uint64_t ns,
...
static void dlb2_entry_points_init(struct rte_eventdev *dev) {
  struct dlb2_eventdev *dlb2;
  ...
  static struct eventdev_ops dlb2_eventdev_entry_ops = {
      ...
      .port_unlinks_in_progress = dlb2_eventdev_port_unlinks_in_progress,
...
int dlb2_primary_eventdev_probe(struct rte_eventdev *dev, const char *name,
  ...
  err = dlb2_iface_get_device_version(&dlb2->qm_instance, &dlb2->revision);
  if (err < 0) {
  ...
  dlb2_init_queue_depth_thresholds(dlb2, dlb2_args->qid_depth_thresholds.val);

  return 0;
}

int dlb2_secondary_eventdev_probe(struct rte_eventdev *dev, const char *name) {
  struct dlb2_eventdev *dlb2;
  int err;
...
int dlb2_parse_params(const char *params, const char *name,
                      struct dlb2_devargs *dlb2_args, uint8_t version) {
  int ret = 0;
  ...
  if (params != NULL && params[0] != '\0') {
    ...
    if (kvlist == NULL) {
      ...
      int ret = rte_kvargs_process(kvlist, NUMA_NODE_ARG, set_numa_node,
                                   &dlb2_args->socket_id);
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing numa node parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, DLB2_MAX_NUM_EVENTS, set_max_num_events,
                               &dlb2_args->max_num_events);
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing max_num_events parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      if (version == DLB2_HW_V2) {
        ret = rte_kvargs_process(kvlist, DLB2_NUM_DIR_CREDITS,
                                 set_num_dir_credits,
                                 &dlb2_args->num_dir_credits_override);
        if (ret != 0) {
          DLB2_LOG_ERR("%s: Error parsing num_dir_credits parameter", name);
          rte_kvargs_free(kvlist);
          return ret;
        }
      }
      ret = rte_kvargs_process(kvlist, DEV_ID_ARG, set_dev_id,
                               &dlb2_args->dev_id);
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing dev_id parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      if (version == DLB2_HW_V2) {
        ret = rte_kvargs_process(kvlist, DLB2_QID_DEPTH_THRESH_ARG,
      ...
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing qid_depth_thresh parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret =
          rte_kvargs_process(kvlist, DLB2_COS_ARG, set_cos, &dlb2_args->cos_id);
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing cos parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, DLB2_POLL_INTERVAL_ARG,
                               set_poll_interval, &dlb2_args->poll_interval);
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing poll interval parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, DLB2_SW_CREDIT_QUANTA_ARG,
                               set_sw_credit_quanta,
                               &dlb2_args->sw_credit_quanta);
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing sw credit quanta parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, DLB2_HW_CREDIT_QUANTA_ARG,
                               set_hw_credit_quanta,
                               &dlb2_args->hw_credit_quanta);
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing hw credit quanta parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }
      ...
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing set depth thresh parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, DLB2_VECTOR_OPTS_ENAB_ARG,
                               set_vector_opts_enab,
                               &dlb2_args->vector_opts_enabled);
      if (ret != 0) {
        DLB2_LOG_ERR("%s: Error parsing vector opts enabled", name);
        rte_kvargs_free(kvlist);
        return ret;
      }
