...
static int dlb2_pf_init_driver_state(struct dlb2_dev *dlb2_dev) {
...
static int dlb2_pf_wait_for_device_ready(struct dlb2_dev *dlb2_dev,
  ...
  for (retries = 0; retries < DLB2_READY_RETRY_LIMIT; retries++) {
    ...
    idle_dlb_func_idle =
        idle_val & DLB2_CM_CFG_DIAGNOSTIC_IDLE_STATUS_DLB_FUNC_IDLE;
...
struct dlb2_dev *dlb2_probe(struct rte_pci_device *pdev) {
  ...
  dlb2_dev =
      rte_malloc("DLB2_PF", sizeof(struct dlb2_dev), RTE_CACHE_LINE_SIZE);
  ...
  DLB2_INFO(dlb2_dev, "DLB2 FUNC VA=%p, PA=%p, len=%p\n",
            (void *)dlb2_dev->hw.func_kva, (void *)dlb2_dev->hw.func_phys_addr,
  ...
  DLB2_INFO(dlb2_dev, "DLB2 CSR VA=%p, PA=%p, len=%p\n",
            (void *)dlb2_dev->hw.csr_kva, (void *)dlb2_dev->hw.csr_phys_addr,
...
int dlb2_pf_reset(struct dlb2_dev *dlb2_dev) {
  ...
  if (pcie_cap_offset >= 0) {
    ...
    if (ret != 2) {
      DLB2_LOG_ERR(
          "[%s()] failed to write the pcie device control at offset %d\n",
          __func__, (int)off);
      return ret;
    }
    ...
    if (ret != 2) {
      DLB2_LOG_ERR(
          "[%s()] failed to write the pcie config space at offset %d\n",
          __func__, (int)off);
      return ret;
    }
  ...
  msix_cap_offset = dlb2_pci_find_capability(pdev, DLB2_PCI_CAP_ID_MSIX);
...
int dlb2_pf_create_sched_domain(struct dlb2_hw *hw,
  ...
  return dlb2_hw_create_sched_domain(hw, args, resp, NOT_VF_REQ, PF_ID_ZERO);
}

int dlb2_pf_reset_domain(struct dlb2_hw *hw, u32 id) {
  return dlb2_reset_domain(hw, id, NOT_VF_REQ, PF_ID_ZERO);
}

int dlb2_pf_create_ldb_queue(struct dlb2_hw *hw, u32 id,
                             struct dlb2_create_ldb_queue_args *args,
                             struct dlb2_cmd_response *resp) {
  return dlb2_hw_create_ldb_queue(hw, id, args, resp, NOT_VF_REQ, PF_ID_ZERO);
}

int dlb2_pf_create_ldb_port(struct dlb2_hw *hw, u32 id,
                            struct dlb2_create_ldb_port_args *args,
                            uintptr_t cq_dma_base,
                            struct dlb2_cmd_response *resp) {
  return dlb2_hw_create_ldb_port(hw, id, args, cq_dma_base, resp, NOT_VF_REQ,
                                 PF_ID_ZERO);
}

int dlb2_pf_create_dir_port(struct dlb2_hw *hw, u32 id,
                            struct dlb2_create_dir_port_args *args,
                            uintptr_t cq_dma_base,
                            struct dlb2_cmd_response *resp) {
  return dlb2_hw_create_dir_port(hw, id, args, cq_dma_base, resp, NOT_VF_REQ,
                                 PF_ID_ZERO);
}

int dlb2_pf_create_dir_queue(struct dlb2_hw *hw, u32 id,
                             struct dlb2_create_dir_queue_args *args,
                             struct dlb2_cmd_response *resp) {
  return dlb2_hw_create_dir_queue(hw, id, args, resp, NOT_VF_REQ, PF_ID_ZERO);
}

int dlb2_pf_start_domain(struct dlb2_hw *hw, u32 id,
                         struct dlb2_start_domain_args *args,
                         struct dlb2_cmd_response *resp) {
  return dlb2_hw_start_domain(hw, id, args, resp, NOT_VF_REQ, PF_ID_ZERO);
}
