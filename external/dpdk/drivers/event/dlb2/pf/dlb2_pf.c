...
static void dlb2_pf_low_level_io_init(void) {
  int i;
...
static int dlb2_pf_open(struct dlb2_hw_dev *handle, const char *name) {
  RTE_SET_USED(handle);
  RTE_SET_USED(name);
...
static int dlb2_pf_get_device_version(struct dlb2_hw_dev *handle,
                                      uint8_t *revision) {
...
static void dlb2_pf_hardware_init(struct dlb2_hw_dev *handle) {
  struct dlb2_dev *dlb2_dev = (struct dlb2_dev *)handle->pf_dev;
...
static int dlb2_pf_get_cq_poll_mode(struct dlb2_hw_dev *handle,
...
static void dlb2_pf_domain_reset(struct dlb2_eventdev *dlb2) {
...
static int dlb2_pf_ldb_queue_create(struct dlb2_hw_dev *handle,
  ...
  ret = dlb2_pf_create_ldb_queue(&dlb2_dev->hw, handle->domain_id, cfg,
                                 &response);
...
static int dlb2_pf_get_sn_occupancy(struct dlb2_hw_dev *handle,
                                    struct dlb2_get_sn_occupancy_args *args) {
  struct dlb2_dev *dlb2_dev = (struct dlb2_dev *)handle->pf_dev;
  struct dlb2_cmd_response response = {0};
  int ret;

  ret = dlb2_get_group_sequence_number_occupancy(&dlb2_dev->hw, args->group);
...
static int dlb2_pf_get_sn_allocation(struct dlb2_hw_dev *handle,
...
static int dlb2_pf_set_sn_allocation(struct dlb2_hw_dev *handle,
                                     struct dlb2_set_sn_allocation_args *args) {
  struct dlb2_dev *dlb2_dev = (struct dlb2_dev *)handle->pf_dev;
  struct dlb2_cmd_response response = {0};
  int ret;

  ret = dlb2_set_group_sequence_numbers(&dlb2_dev->hw, args->group, args->num);
...
static void *dlb2_alloc_coherent_aligned(const struct rte_memzone **mz,
...
static int dlb2_pf_ldb_port_create(struct dlb2_hw_dev *handle,
  ...
  port_base =
      dlb2_alloc_coherent_aligned(&mz, &cq_base, alloc_sz, rte_mem_page_size());
  if (port_base == NULL)
    return -ENOMEM;
  ...
  ret = dlb2_pf_create_ldb_port(&dlb2_dev->hw, handle->domain_id, cfg, cq_base,
                                &response);
  if (ret)
    goto create_port_err;
...
static int dlb2_pf_dir_port_create(struct dlb2_hw_dev *handle,
  ...
  ret = dlb2_pf_create_dir_port(&dlb2_dev->hw, handle->domain_id, cfg, cq_base,
                                &response);
  if (ret)
    goto create_port_err;
  ...
  dlb2_port[response.id][DLB2_DIR_PORT].cq_base = (void *)(port_base);
  memset(&port_memory, 0, sizeof(port_memory));
...
static int dlb2_pf_dir_queue_create(struct dlb2_hw_dev *handle,
  ...
  ret = dlb2_pf_create_dir_queue(&dlb2_dev->hw, handle->domain_id, cfg,
                                 &response);
...
static int dlb2_pf_map_qid(struct dlb2_hw_dev *handle,
  ...
  ret = dlb2_hw_map_qid(&dlb2_dev->hw, handle->domain_id, cfg, &response, false,
                        0);
...
static int dlb2_pf_unmap_qid(struct dlb2_hw_dev *handle,
  ...
  ret = dlb2_hw_unmap_qid(&dlb2_dev->hw, handle->domain_id, cfg, &response,
                          false, 0);
...
dlb2_pf_pending_port_unmaps(struct dlb2_hw_dev *handle,
  ...
  ret = dlb2_hw_pending_port_unmaps(&dlb2_dev->hw, handle->domain_id, args,
                                    &response, false, 0);
...
static int dlb2_pf_sched_domain_start(struct dlb2_hw_dev *handle,
  ...
  ret = dlb2_pf_start_domain(&dlb2_dev->hw, handle->domain_id, cfg, &response);
...
dlb2_pf_get_ldb_queue_depth(struct dlb2_hw_dev *handle,
  ...
  ret = dlb2_hw_get_ldb_queue_depth(&dlb2_dev->hw, handle->domain_id, args,
                                    &response, false, 0);
...
dlb2_pf_get_dir_queue_depth(struct dlb2_hw_dev *handle,
  ...
  ret = dlb2_hw_get_dir_queue_depth(&dlb2_dev->hw, handle->domain_id, args,
                                    &response, false, 0);
...
static void dlb2_pf_iface_fn_ptrs_init(void) {
...
/* PCI DEV HOOKS */
static int dlb2_eventdev_pci_init(struct rte_eventdev *eventdev) {
  int ret = 0;
  ...
  DLB2_LOG_DBG("Enter with dev_id=%d socket_id=%d", eventdev->data->dev_id,
               eventdev->data->socket_id);
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    ...
    if (pci_dev->device.devargs) {
      ret = dlb2_parse_params(pci_dev->device.devargs->args,
                              pci_dev->device.devargs->name, &dlb2_args,
                              dlb2->version);
      if (ret) {
    ...
    ret = dlb2_primary_eventdev_probe(eventdev, event_dlb2_pf_name, &dlb2_args);
  } else {
    dlb2 = dlb2_pmd_priv(eventdev);
    dlb2->version = DLB2_HW_DEVICE_FROM_PCI_ID(pci_dev);
    ret = dlb2_secondary_eventdev_probe(eventdev, event_dlb2_pf_name);
  }
  if (ret)
    goto dlb2_probe_failed;
...
static const struct rte_pci_id pci_id_dlb2_map[] = {
    {RTE_PCI_DEVICE(EVENTDEV_INTEL_VENDOR_ID, PCI_DEVICE_ID_INTEL_DLB2_PF)},
    {
        .vendor_id = 0,
    },
};
...
static const struct rte_pci_id pci_id_dlb2_5_map[] = {
    {RTE_PCI_DEVICE(EVENTDEV_INTEL_VENDOR_ID, PCI_DEVICE_ID_INTEL_DLB2_5_PF)},
    {
        .vendor_id = 0,
    },
};

static int event_dlb2_pci_probe(struct rte_pci_driver *pci_drv,
                                struct rte_pci_device *pci_dev) {
  int ret;

  ret = rte_event_pmd_pci_probe_named(
      pci_drv, pci_dev, sizeof(struct dlb2_eventdev), dlb2_eventdev_pci_init,
      event_dlb2_pf_name);
  if (ret) {
...
static int event_dlb2_pci_remove(struct rte_pci_device *pci_dev) {
  int ret;
...
static int event_dlb2_5_pci_probe(struct rte_pci_driver *pci_drv,
                                  struct rte_pci_device *pci_dev) {
  int ret;
...
static int event_dlb2_5_pci_remove(struct rte_pci_device *pci_dev) {
  int ret;
