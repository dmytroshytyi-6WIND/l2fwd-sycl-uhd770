...
#define DLB2_CSR_RD(hw, reg) DLB2_PCI_REG_READ(DLB2_CSR_REG_ADDR((hw), (reg)))
...
#define DLB2_FUNC_RD(hw, reg) DLB2_PCI_REG_READ(DLB2_FUNC_REG_ADDR((hw), (reg)))
#define DLB2_FUNC_WR(hw, reg, value)                                           \
  DLB2_PCI_REG_WRITE(DLB2_FUNC_REG_ADDR((hw), (reg)), (value))

/* Map to PMDs logging interface */
#define DLB2_ERR(dev, fmt, args...) DLB2_LOG_ERR(fmt, ##args)

#define DLB2_INFO(dev, fmt, args...) DLB2_LOG_INFO(fmt, ##args)

#define DLB2_DEBUG(dev, fmt, args...) DLB2_LOG_DBG(fmt, ##args)

/**
 * os_udelay() - busy-wait for a number of microseconds
 * @usecs: delay duration.
 */
static inline void os_udelay(int usecs) { rte_delay_us(usecs); }

/**
 * os_msleep() - sleep for a number of milliseconds
 * @usecs: delay duration.
 */
static inline void os_msleep(int msecs) { rte_delay_ms(msecs); }
 ...
 */
static inline void *os_map_producer_port(struct dlb2_hw *hw, u8 port_id,
                                         bool is_ldb) {
  uint64_t addr;
...
static inline void os_schedule_work(struct dlb2_hw *hw) {
  ...
  ret = rte_ctrl_thread_create(&complete_queue_map_unmap_thread,
                               "dlb_queue_unmap_waiter", NULL,
                               dlb2_complete_queue_map_unmap, dlb2_dev);
  if (ret)
    DLB2_ERR(dlb2_dev,
