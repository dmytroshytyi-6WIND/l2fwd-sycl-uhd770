...
#define TAILQ_FOREACH_ENTRY(ptr, head, name, iter)                             \
  for ((iter) = TAILQ_FIRST(&head);                                            \
       (iter) && (ptr = container_of(iter, typeof(*(ptr)), name));             \
 ...
 */
static inline void dlb2_list_add(struct dlb2_list_head *head,
...
#define DLB2_LIST_HEAD(head, type, name)                                       \
  (TAILQ_FIRST(&head) ? container_of(TAILQ_FIRST(&head), type, name) : NULL)
