...
static inline int dlb2_bitmap_alloc(struct dlb2_bitmap **bitmap,
  ...
  bm = rte_malloc("DLB2_PF", sizeof(struct dlb2_bitmap), RTE_CACHE_LINE_SIZE);
...
static inline int dlb2_bitmap_or(struct dlb2_bitmap *dest,
  ...
  if (dest == NULL || dest->map == NULL || src1 == NULL || src1->map == NULL ||
  ...
  for (i = 0; i != min; i++) {
    if (rte_bitmap_get(src1->map, i) || rte_bitmap_get(src2->map, i)) {
