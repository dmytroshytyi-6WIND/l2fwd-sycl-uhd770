...
#define DLB2_DOM_LIST_HEAD(head, type) DLB2_LIST_HEAD((head), type, domain_list)

#define DLB2_FUNC_LIST_HEAD(head, type) DLB2_LIST_HEAD((head), type, func_list)
...
int dlb2_resource_init(struct dlb2_hw *hw, enum dlb2_hw_ver ver) {
  ...
  for (i = 0; i < DLB2_MAX_NUM_LDB_PORTS; i++) {
    ...
    dlb2_list_add(&hw->pf.avail_ldb_ports[cos_id], &port->func_list);
  }
...
int dlb2_hw_get_num_resources(struct dlb2_hw *hw,
  ...
  arg->max_contiguous_hist_list_entries = dlb2_bitmap_longest_set_range(map);
...
static void dlb2_configure_domain_credits_v2(struct dlb2_hw *hw,
  ...
  DLB2_BITS_SET(reg, domain->num_ldb_credits, DLB2_CHP_CFG_LDB_VAS_CRD_COUNT);
  DLB2_CSR_WR(hw, DLB2_CHP_CFG_LDB_VAS_CRD(domain->id.phys_id), reg);

  reg = 0;
  DLB2_BITS_SET(reg, domain->num_dir_credits, DLB2_CHP_CFG_DIR_VAS_CRD_COUNT);
...
static int dlb2_attach_credits(struct dlb2_function_resources *rsrcs,
                               struct dlb2_hw_domain *domain, u32 num_credits,
...
dlb2_get_next_ldb_port(struct dlb2_hw *hw,
                       struct dlb2_function_resources *rsrcs, u32 domain_id,
                       u32 cos_id) {
  ...
  DLB2_FUNC_LIST_FOR(rsrcs->avail_ldb_ports[cos_id], port, iter) {
    ...
    if (!hw->rsrcs.ldb_ports[prev].owned && hw->rsrcs.ldb_ports[next].owned &&
        hw->rsrcs.ldb_ports[next].domain_id.phys_id != domain_id)
      return port;

    if (!hw->rsrcs.ldb_ports[next].owned && hw->rsrcs.ldb_ports[prev].owned &&
        hw->rsrcs.ldb_ports[prev].domain_id.phys_id != domain_id)
      return port;
  }
  ...
  return DLB2_FUNC_LIST_HEAD(rsrcs->avail_ldb_ports[cos_id], typeof(*port));
}

static int __dlb2_attach_ldb_ports(struct dlb2_hw *hw,
                                   struct dlb2_function_resources *rsrcs,
                                   struct dlb2_hw_domain *domain, u32 num_ports,
                                   u32 cos_id, struct dlb2_cmd_response *resp) {
  unsigned int i;
  ...
  for (i = 0; i < num_ports; i++) {
    ...
    port = dlb2_get_next_ldb_port(hw, rsrcs, domain->id.phys_id, cos_id);
    if (port == NULL) {
      DLB2_HW_ERR(hw, "[%s()] Internal error: domain validation failed\n",
                  __func__);
      return -EFAULT;
    }

    dlb2_list_del(&rsrcs->avail_ldb_ports[cos_id], &port->func_list);

    port->domain_id = domain->id;
    port->owned = true;

    dlb2_list_add(&domain->avail_ldb_ports[cos_id], &port->domain_list);
  }
...
static int dlb2_attach_ldb_ports(struct dlb2_hw *hw,
  ...
  if (args->cos_strict) {
    for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
      ...
      ret = __dlb2_attach_ldb_ports(hw, rsrcs, domain, num, i, resp);
      if (ret)
        return ret;
    }
  } else {
    ...
    for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
      for (j = 0; j < args->num_cos_ldb_ports[i]; j++) {
        for (k = 0; k < DLB2_NUM_COS_DOMAINS; k++) {
          ...
          ret = __dlb2_attach_ldb_ports(hw, rsrcs, domain, 1, cos_id, resp);
          if (ret == 0)
            break;
        }
  ...
  for (i = 0; i < args->num_ldb_ports; i++) {
    for (j = 0; j < DLB2_NUM_COS_DOMAINS; j++) {
      ret = __dlb2_attach_ldb_ports(hw, rsrcs, domain, 1, j, resp);
      if (ret == 0)
        break;
    }
...
static int dlb2_attach_dir_ports(struct dlb2_hw *hw,
  ...
  for (i = 0; i < num_ports; i++) {
    ...
    port = DLB2_FUNC_LIST_HEAD(rsrcs->avail_dir_pq_pairs, typeof(*port));
    if (port == NULL) {
      DLB2_HW_ERR(hw, "[%s()] Internal error: domain validation failed\n",
                  __func__);
      return -EFAULT;
...
static int dlb2_attach_domain_hist_list_entries(
    struct dlb2_function_resources *rsrcs, struct dlb2_hw_domain *domain,
    u32 num_hist_list_entries, struct dlb2_cmd_response *resp) {
  ...
  if (num_hist_list_entries) {
    ...
    base = dlb2_bitmap_find_set_bit_range(bitmap, num_hist_list_entries);
    if (base < 0)
      goto error;
...
static int dlb2_attach_ldb_queues(struct dlb2_hw *hw,
                                  struct dlb2_function_resources *rsrcs,
                                  struct dlb2_hw_domain *domain, u32 num_queues,
                                  struct dlb2_cmd_response *resp) {
  unsigned int i;
  ...
  for (i = 0; i < num_queues; i++) {
    ...
    queue = DLB2_FUNC_LIST_HEAD(rsrcs->avail_ldb_queues, typeof(*queue));
    if (queue == NULL) {
      DLB2_HW_ERR(hw, "[%s()] Internal error: domain validation failed\n",
                  __func__);
      return -EFAULT;
...
static int dlb2_domain_attach_resources(
    struct dlb2_hw *hw, struct dlb2_function_resources *rsrcs,
    struct dlb2_hw_domain *domain, struct dlb2_create_sched_domain_args *args,
    struct dlb2_cmd_response *resp) {
  int ret;

  ret = dlb2_attach_ldb_queues(hw, rsrcs, domain, args->num_ldb_queues, resp);
  if (ret)
    return ret;

  ret = dlb2_attach_ldb_ports(hw, rsrcs, domain, args, resp);
  if (ret)
    return ret;

  ret = dlb2_attach_dir_ports(hw, rsrcs, domain, args->num_dir_ports, resp);
  if (ret)
    return ret;

  if (hw->ver == DLB2_HW_V2) {
    ret = dlb2_attach_ldb_credits(rsrcs, domain, args->num_ldb_credits, resp);
    if (ret)
      return ret;

    ret = dlb2_attach_dir_credits(rsrcs, domain, args->num_dir_credits, resp);
    if (ret)
      return ret;
  } else { /* DLB 2.5 */
    ret = dlb2_attach_credits(rsrcs, domain, args->num_credits, resp);
    if (ret)
      return ret;
  }

  ret = dlb2_attach_domain_hist_list_entries(rsrcs, domain,
                                             args->num_hist_list_entries, resp);
  if (ret)
    return ret;

  ret = dlb2_attach_atomic_inflights(rsrcs, domain, args->num_atomic_inflights,
                                     resp);
  if (ret)
    return ret;
...
static int dlb2_verify_create_sched_dom_args(
    struct dlb2_function_resources *rsrcs,
    struct dlb2_create_sched_domain_args *args, struct dlb2_cmd_response *resp,
    struct dlb2_hw *hw, struct dlb2_hw_domain **out_domain) {
  ...
  for (i = 0; args->cos_strict && i < DLB2_NUM_COS_DOMAINS; i++) {
    if (args->num_cos_ldb_ports[i] > rsrcs->num_avail_ldb_ports[i]) {
      resp->status = DLB2_ST_LDB_PORTS_UNAVAILABLE;
      return -EINVAL;
    }
  }
...
int dlb2_hw_create_sched_domain(struct dlb2_hw *hw,
  ...
  if (ret) {
    DLB2_HW_ERR(hw, "[%s()] Internal error: failed to verify args.\n",
                __func__);
...
static u32 dlb2_dir_cq_token_count(struct dlb2_hw *hw,
  ...
  cnt = DLB2_CSR_RD(hw, DLB2_LSP_CQ_DIR_TKN_CNT(hw->ver, port->id.phys_id));
  ...
  return DLB2_BITS_GET(cnt, DLB2_LSP_CQ_DIR_TKN_CNT_COUNT) - port->init_tkn_cnt;
}
...
static u32 dlb2_dir_queue_depth(struct dlb2_hw *hw,
  ...
  cnt =
      DLB2_CSR_RD(hw, DLB2_LSP_QID_DIR_ENQUEUE_CNT(hw->ver, queue->id.phys_id));
...
static u32 dlb2_ldb_cq_inflight_count(struct dlb2_hw *hw,
  ...
  cnt = DLB2_CSR_RD(hw, DLB2_LSP_CQ_LDB_INFL_CNT(hw->ver, port->id.phys_id));
...
static u32 dlb2_ldb_cq_token_count(struct dlb2_hw *hw,
  ...
  cnt = DLB2_CSR_RD(hw, DLB2_LSP_CQ_LDB_TKN_CNT(hw->ver, port->id.phys_id));
...
static u32 dlb2_ldb_queue_depth(struct dlb2_hw *hw,
  ...
  aqed =
      DLB2_CSR_RD(hw, DLB2_LSP_QID_AQED_ACTIVE_CNT(hw->ver, queue->id.phys_id));
  ldb =
      DLB2_CSR_RD(hw, DLB2_LSP_QID_LDB_ENQUEUE_CNT(hw->ver, queue->id.phys_id));
  atm = DLB2_CSR_RD(hw, DLB2_LSP_QID_ATM_ACTIVE(hw->ver, queue->id.phys_id));
...
static int dlb2_domain_drain_mapped_queues(struct dlb2_hw *hw,
  ...
  if (domain->num_pending_removals > 0) {
    DLB2_HW_ERR(hw, "[%s()] Internal error: failed to unmap domain queues\n",
                __func__);
    return -EFAULT;
  }
...
static struct dlb2_ldb_queue *dlb2_get_ldb_queue_from_id(struct dlb2_hw *hw,
                                                         u32 id, bool vdev_req,
                                                         unsigned int vdev_id) {
...
static int dlb2_port_slot_state_transition(struct dlb2_hw *hw,
  ...
  if (domain == NULL) {
    DLB2_HW_ERR(hw, "[%s()] Internal error: unable to find domain %d\n",
                __func__, domain_id);
    return -EINVAL;
  }
...
error:
  DLB2_HW_ERR(hw,
              "[%s()] Internal error: invalid queue %d -> port %d state "
              "transition (%d -> %d)\n",
              __func__, queue->id.phys_id, port->id.phys_id, curr_state,
              new_state);
  return -EFAULT;
...
static bool dlb2_port_find_slot(struct dlb2_ldb_port *port,
                                enum dlb2_qid_map_state state, int *slot) {
  int i;
...
static bool dlb2_port_find_slot_queue(struct dlb2_ldb_port *port,
                                      ...
                                      struct dlb2_ldb_queue *queue, int *slot) {
  int i;
...
static int dlb2_ldb_port_map_qid_static(struct dlb2_hw *hw,
                                        ...
                                        struct dlb2_ldb_queue *q, u8 priority) {
  enum dlb2_qid_map_state state;
  u32 lsp_qid2cq2;
  u32 lsp_qid2cq;
  u32 atm_qid2cq;
  ...
  if (!dlb2_port_find_slot_queue(p, DLB2_QUEUE_MAP_IN_PROG, q, &i) &&
    ...
    DLB2_HW_ERR(
        hw, "[%s():%d] Internal error: CQ has no available QID mapping slots\n",
        __func__, __LINE__);
    return -EFAULT;
  }
  ...
  if (i < 4)
    cq2qid = DLB2_CSR_RD(hw, DLB2_LSP_CQ2QID0(hw->ver, p->id.phys_id));
  else
    cq2qid = DLB2_CSR_RD(hw, DLB2_LSP_CQ2QID1(hw->ver, p->id.phys_id));
  ...
  if (i < 4)
    DLB2_CSR_WR(hw, DLB2_LSP_CQ2QID0(hw->ver, p->id.phys_id), cq2qid);
  else
    DLB2_CSR_WR(hw, DLB2_LSP_CQ2QID1(hw->ver, p->id.phys_id), cq2qid);

  atm_qid2cq =
      DLB2_CSR_RD(hw, DLB2_ATM_QID2CQIDIX(q->id.phys_id, p->id.phys_id / 4));

  lsp_qid2cq = DLB2_CSR_RD(
      hw, DLB2_LSP_QID2CQIDIX(hw->ver, q->id.phys_id, p->id.phys_id / 4));

  lsp_qid2cq2 = DLB2_CSR_RD(
      hw, DLB2_LSP_QID2CQIDIX2(hw->ver, q->id.phys_id, p->id.phys_id / 4));

  switch (p->id.phys_id % 4) {
  case 0:
    DLB2_BIT_SET(atm_qid2cq, 1 << (i + DLB2_ATM_QID2CQIDIX_00_CQ_P0_LOC));
    DLB2_BIT_SET(lsp_qid2cq, 1 << (i + DLB2_LSP_QID2CQIDIX_00_CQ_P0_LOC));
    DLB2_BIT_SET(lsp_qid2cq2, 1 << (i + DLB2_LSP_QID2CQIDIX2_00_CQ_P0_LOC));
    break;

  case 1:
    DLB2_BIT_SET(atm_qid2cq, 1 << (i + DLB2_ATM_QID2CQIDIX_00_CQ_P1_LOC));
    DLB2_BIT_SET(lsp_qid2cq, 1 << (i + DLB2_LSP_QID2CQIDIX_00_CQ_P1_LOC));
    DLB2_BIT_SET(lsp_qid2cq2, 1 << (i + DLB2_LSP_QID2CQIDIX2_00_CQ_P1_LOC));
    break;

  case 2:
    DLB2_BIT_SET(atm_qid2cq, 1 << (i + DLB2_ATM_QID2CQIDIX_00_CQ_P2_LOC));
    DLB2_BIT_SET(lsp_qid2cq, 1 << (i + DLB2_LSP_QID2CQIDIX_00_CQ_P2_LOC));
    DLB2_BIT_SET(lsp_qid2cq2, 1 << (i + DLB2_LSP_QID2CQIDIX2_00_CQ_P2_LOC));
    break;

  case 3:
    DLB2_BIT_SET(atm_qid2cq, 1 << (i + DLB2_ATM_QID2CQIDIX_00_CQ_P3_LOC));
    DLB2_BIT_SET(lsp_qid2cq, 1 << (i + DLB2_LSP_QID2CQIDIX_00_CQ_P3_LOC));
    DLB2_BIT_SET(lsp_qid2cq2, 1 << (i + DLB2_LSP_QID2CQIDIX2_00_CQ_P3_LOC));
    break;
  }

  DLB2_CSR_WR(hw, DLB2_ATM_QID2CQIDIX(q->id.phys_id, p->id.phys_id / 4),
              atm_qid2cq);

  DLB2_CSR_WR(hw,
              DLB2_LSP_QID2CQIDIX(hw->ver, q->id.phys_id, p->id.phys_id / 4),
              lsp_qid2cq);

  DLB2_CSR_WR(hw,
              DLB2_LSP_QID2CQIDIX2(hw->ver, q->id.phys_id, p->id.phys_id / 4),
              lsp_qid2cq2);
...
static int dlb2_ldb_port_set_has_work_bits(struct dlb2_hw *hw,
  ...
  DLB2_BIT_SET(ctrl, DLB2_LSP_LDB_SCHED_CTRL_VALUE);
  DLB2_BITS_SET(ctrl,
                DLB2_BITS_GET(active, DLB2_LSP_QID_AQED_ACTIVE_CNT_COUNT) > 0,
  ...
  DLB2_BIT_SET(ctrl, DLB2_LSP_LDB_SCHED_CTRL_VALUE);
  DLB2_BITS_SET(ctrl,
                DLB2_BITS_GET(enq, DLB2_LSP_QID_LDB_ENQUEUE_CNT_COUNT) > 0,
...
static int dlb2_ldb_port_finish_map_qid_dynamic(struct dlb2_hw *hw,
  ...
  infl_cnt =
      DLB2_CSR_RD(hw, DLB2_LSP_QID_LDB_INFL_CNT(hw->ver, queue->id.phys_id));

  if (DLB2_BITS_GET(infl_cnt, DLB2_LSP_QID_LDB_INFL_CNT_COUNT)) {
    DLB2_HW_ERR(hw, "[%s()] Internal error: non-zero QID inflight count\n",
                __func__);
    return -EINVAL;
  }
...
static int dlb2_ldb_port_map_qid_dynamic(struct dlb2_hw *hw,
   ...
   * queue.
   */
  DLB2_CSR_WR(hw, DLB2_LSP_QID_LDB_INFL_LIM(hw->ver, queue->id.phys_id), 0);

  if (!dlb2_port_find_slot(port, DLB2_QUEUE_UNMAPPED, &slot)) {
    DLB2_HW_ERR(hw, "Internal error: No available unmapped slots\n");
    return -EFAULT;
  }
...
static void dlb2_domain_finish_map_port(struct dlb2_hw *hw,
  ...
  for (i = 0; i < DLB2_MAX_NUM_QIDS_PER_LDB_CQ; i++) {
    ...
    if (queue == NULL) {
      DLB2_HW_ERR(hw, "[%s()] Internal error: unable to find queue %d\n",
                  __func__, qid);
      continue;
    }

    infl_cnt = DLB2_CSR_RD(hw, DLB2_LSP_QID_LDB_INFL_CNT(hw->ver, qid));
...
static int dlb2_ldb_port_unmap_qid(struct dlb2_hw *hw,
  ...
  if (!dlb2_port_find_slot_queue(port, mapped, queue, &i) &&
    ...
    DLB2_HW_ERR(hw, "[%s():%d] Internal error: QID %d isn't mapped\n", __func__,
                __LINE__, queue->id.phys_id);
    return -EFAULT;
  }
  ...
  atm_qid2cq = DLB2_CSR_RD(hw, DLB2_ATM_QID2CQIDIX(queue_id, port_id / 4));

  lsp_qid2cq =
      DLB2_CSR_RD(hw, DLB2_LSP_QID2CQIDIX(hw->ver, queue_id, port_id / 4));

  lsp_qid2cq2 =
      DLB2_CSR_RD(hw, DLB2_LSP_QID2CQIDIX2(hw->ver, queue_id, port_id / 4));
...
static int dlb2_ldb_port_map_qid(struct dlb2_hw *hw,
                                 struct dlb2_hw_domain *domain,
                                 struct dlb2_ldb_port *port,
                                 struct dlb2_ldb_queue *queue, u8 prio) {
  if (domain->started)
...
static void dlb2_domain_finish_unmap_port_slot(struct dlb2_hw *hw,
                                               struct dlb2_hw_domain *domain,
                                               struct dlb2_ldb_port *port,
                                               int slot) {
  enum dlb2_qid_map_state state;
...
static bool dlb2_domain_finish_unmap_port(struct dlb2_hw *hw,
  ...
  for (i = 0; i < max_iters; i++) {
    infl_cnt =
        DLB2_CSR_RD(hw, DLB2_LSP_CQ_LDB_INFL_CNT(hw->ver, port->id.phys_id));
...
static void dlb2_log_reset_domain(struct dlb2_hw *hw, u32 domain_id,
                                  bool vdev_req, unsigned int vdev_id) {
...
dlb2_domain_disable_ldb_port_interrupts(struct dlb2_hw *hw,
  ...
  for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
    DLB2_DOM_LIST_FOR(domain->used_ldb_ports[i], port, iter) {
      DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ_INT_ENB(hw->ver, port->id.phys_id),
                  int_en);

      DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ_WD_ENB(hw->ver, port->id.phys_id), wd_en);
    }
  }
}
...
dlb2_domain_disable_dir_port_interrupts(struct dlb2_hw *hw,
  ...
  DLB2_DOM_LIST_FOR(domain->used_dir_pq_pairs, port, iter) {
    DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ_INT_ENB(hw->ver, port->id.phys_id), int_en);

    DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ_WD_ENB(hw->ver, port->id.phys_id), wd_en);
  }
}
...
dlb2_domain_disable_ldb_queue_write_perms(struct dlb2_hw *hw,
  ...
  DLB2_DOM_LIST_FOR(domain->used_ldb_queues, queue, iter) {
    ...
    if (queue->id.vdev_owned) {
      DLB2_CSR_WR(hw, DLB2_SYS_LDB_QID2VQID(queue->id.phys_id), 0);

      idx = queue->id.vdev_id * DLB2_MAX_NUM_LDB_QUEUES + queue->id.virt_id;
...
static void dlb2_domain_disable_ldb_seq_checks(struct dlb2_hw *hw,
  ...
  for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
    DLB2_DOM_LIST_FOR(domain->used_ldb_ports[i], port, iter) {
      DLB2_CSR_WR(hw, DLB2_CHP_SN_CHK_ENBL(hw->ver, port->id.phys_id), chk_en);
    }
  }
}

static int
dlb2_domain_wait_for_ldb_cqs_to_empty(struct dlb2_hw *hw,
                                      struct dlb2_hw_domain *domain) {
  struct dlb2_list_entry *iter;
  struct dlb2_ldb_port *port;
  int i;
  ...
  for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
    DLB2_DOM_LIST_FOR(domain->used_ldb_ports[i], port, iter) {
      ...
      if (j == DLB2_MAX_CQ_COMP_CHECK_LOOPS) {
        DLB2_HW_ERR(hw,
                    "[%s()] Internal error: failed to flush load-balanced port "
                    "%d's completions.\n",
...
dlb2_domain_disable_dir_producer_ports(struct dlb2_hw *hw,
  ...
  DLB2_DOM_LIST_FOR(domain->used_dir_pq_pairs, port, iter) {
    DLB2_CSR_WR(hw, DLB2_SYS_DIR_PP_V(port->id.phys_id), pp_v);
  }
}
...
dlb2_domain_disable_ldb_producer_ports(struct dlb2_hw *hw,
  ...
  for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
    DLB2_DOM_LIST_FOR(domain->used_ldb_ports[i], port, iter) {
      DLB2_CSR_WR(hw, DLB2_SYS_LDB_PP_V(port->id.phys_id), pp_v);
    }
  }
}
...
static int dlb2_domain_verify_reset_success(struct dlb2_hw *hw,
  ...
  DLB2_DOM_LIST_FOR(domain->used_ldb_queues, queue, iter) {
    if (!dlb2_ldb_queue_is_empty(hw, queue)) {
      DLB2_HW_ERR(hw, "[%s()] Internal error: failed to empty ldb queue %d\n",
  ...
  for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
    DLB2_DOM_LIST_FOR(domain->used_ldb_ports[i], ldb_port, iter) {
      if (dlb2_ldb_cq_inflight_count(hw, ldb_port) ||
        ...
        DLB2_HW_ERR(hw, "[%s()] Internal error: failed to empty ldb port %d\n",
  ...
  DLB2_DOM_LIST_FOR(domain->used_dir_pq_pairs, dir_port, iter) {
    if (!dlb2_dir_queue_is_empty(hw, dir_port)) {
      DLB2_HW_ERR(hw, "[%s()] Internal error: failed to empty dir queue %d\n",
                  __func__, dir_port->id.phys_id);
      return -EFAULT;
    }

    if (dlb2_dir_cq_token_count(hw, dir_port)) {
      DLB2_HW_ERR(hw, "[%s()] Internal error: failed to empty dir port %d\n",
                  __func__, dir_port->id.phys_id);
      return -EFAULT;
    }
  }
...
static void __dlb2_domain_reset_ldb_port_registers(struct dlb2_hw *hw,
                                                   struct dlb2_ldb_port *port) {
  DLB2_CSR_WR(hw, DLB2_SYS_LDB_PP2VAS(port->id.phys_id),
              DLB2_SYS_LDB_PP2VAS_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ2VAS(hw->ver, port->id.phys_id),
              DLB2_CHP_LDB_CQ2VAS_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_LDB_PP2VDEV(port->id.phys_id),
  ...
  if (port->id.vdev_owned) {
    ...
    DLB2_CSR_WR(hw, DLB2_SYS_VF_LDB_VPP2PP(offs), DLB2_SYS_VF_LDB_VPP2PP_RST);

    DLB2_CSR_WR(hw, DLB2_SYS_VF_LDB_VPP_V(offs), DLB2_SYS_VF_LDB_VPP_V_RST);
  }

  DLB2_CSR_WR(hw, DLB2_SYS_LDB_PP_V(port->id.phys_id), DLB2_SYS_LDB_PP_V_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_LDB_DSBL(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ_LDB_DSBL_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ_DEPTH(hw->ver, port->id.phys_id),
              DLB2_CHP_LDB_CQ_DEPTH_RST);

  if (hw->ver != DLB2_HW_V2)
    DLB2_CSR_WR(hw, DLB2_LSP_CFG_CQ_LDB_WU_LIMIT(port->id.phys_id),
  ...
  DLB2_CSR_WR(hw, DLB2_CHP_HIST_LIST_LIM(hw->ver, port->id.phys_id),
              DLB2_CHP_HIST_LIST_LIM_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_HIST_LIST_BASE(hw->ver, port->id.phys_id),
  ...
  DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ_INT_DEPTH_THRSH(hw->ver, port->id.phys_id),
              DLB2_CHP_LDB_CQ_INT_DEPTH_THRSH_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ_TMR_THRSH(hw->ver, port->id.phys_id),
              DLB2_CHP_LDB_CQ_TMR_THRSH_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ_INT_ENB(hw->ver, port->id.phys_id),
              DLB2_CHP_LDB_CQ_INT_ENB_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_LDB_CQ_ISR(port->id.phys_id),
              DLB2_SYS_LDB_CQ_ISR_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_LDB_TKN_DEPTH_SEL(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ_LDB_TKN_DEPTH_SEL_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ_TKN_DEPTH_SEL(hw->ver, port->id.phys_id),
              DLB2_CHP_LDB_CQ_TKN_DEPTH_SEL_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ_WPTR(hw->ver, port->id.phys_id),
              DLB2_CHP_LDB_CQ_WPTR_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_LDB_TKN_CNT(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ_LDB_TKN_CNT_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_LDB_CQ_ADDR_L(port->id.phys_id),
              DLB2_SYS_LDB_CQ_ADDR_L_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_LDB_CQ_ADDR_U(port->id.phys_id),
              DLB2_SYS_LDB_CQ_ADDR_U_RST);

  if (hw->ver == DLB2_HW_V2)
    DLB2_CSR_WR(hw, DLB2_SYS_LDB_CQ_AT(port->id.phys_id),
                DLB2_SYS_LDB_CQ_AT_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_LDB_CQ_PASID(hw->ver, port->id.phys_id),
              DLB2_SYS_LDB_CQ_PASID_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_LDB_CQ2VF_PF_RO(port->id.phys_id),
              DLB2_SYS_LDB_CQ2VF_PF_RO_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_LDB_TOT_SCH_CNTL(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ_LDB_TOT_SCH_CNTL_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_LDB_TOT_SCH_CNTH(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ_LDB_TOT_SCH_CNTH_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ2QID0(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ2QID0_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ2QID1(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ2QID1_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ2PRIOV(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ2PRIOV_RST);
}

static void
dlb2_domain_reset_ldb_port_registers(struct dlb2_hw *hw,
                                     struct dlb2_hw_domain *domain) {
  struct dlb2_list_entry *iter;
  struct dlb2_ldb_port *port;
  int i;
...
__dlb2_domain_reset_dir_port_registers(struct dlb2_hw *hw,
  ...
  DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ2VAS(hw->ver, port->id.phys_id),
              DLB2_CHP_DIR_CQ2VAS_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_DIR_DSBL(hw->ver, port->id.phys_id),
  ...
  if (hw->ver == DLB2_HW_V2)
    ...
    DLB2_CSR_WR(hw, DLB2_SYS_WB_DIR_CQ_STATE(port->id.phys_id), reg);

  DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ_DEPTH(hw->ver, port->id.phys_id),
              DLB2_CHP_DIR_CQ_DEPTH_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ_INT_DEPTH_THRSH(hw->ver, port->id.phys_id),
              DLB2_CHP_DIR_CQ_INT_DEPTH_THRSH_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ_TMR_THRSH(hw->ver, port->id.phys_id),
              DLB2_CHP_DIR_CQ_TMR_THRSH_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ_INT_ENB(hw->ver, port->id.phys_id),
              DLB2_CHP_DIR_CQ_INT_ENB_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_CQ_ISR(port->id.phys_id),
              DLB2_SYS_DIR_CQ_ISR_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_DIR_TKN_DEPTH_SEL_DSI(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ_DIR_TKN_DEPTH_SEL_DSI_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ_TKN_DEPTH_SEL(hw->ver, port->id.phys_id),
              DLB2_CHP_DIR_CQ_TKN_DEPTH_SEL_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ_WPTR(hw->ver, port->id.phys_id),
              DLB2_CHP_DIR_CQ_WPTR_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_DIR_TKN_CNT(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ_DIR_TKN_CNT_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_CQ_ADDR_L(port->id.phys_id),
              DLB2_SYS_DIR_CQ_ADDR_L_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_CQ_ADDR_U(port->id.phys_id),
              DLB2_SYS_DIR_CQ_ADDR_U_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_CQ_AT(port->id.phys_id), DLB2_SYS_DIR_CQ_AT_RST);

  if (hw->ver == DLB2_HW_V2)
    DLB2_CSR_WR(hw, DLB2_SYS_DIR_CQ_AT(port->id.phys_id),
                DLB2_SYS_DIR_CQ_AT_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_CQ_PASID(hw->ver, port->id.phys_id),
              DLB2_SYS_DIR_CQ_PASID_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_CQ_FMT(port->id.phys_id),
              DLB2_SYS_DIR_CQ_FMT_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_CQ2VF_PF_RO(port->id.phys_id),
              DLB2_SYS_DIR_CQ2VF_PF_RO_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_DIR_TOT_SCH_CNTL(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ_DIR_TOT_SCH_CNTL_RST);

  DLB2_CSR_WR(hw, DLB2_LSP_CQ_DIR_TOT_SCH_CNTH(hw->ver, port->id.phys_id),
              DLB2_LSP_CQ_DIR_TOT_SCH_CNTH_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_PP2VAS(port->id.phys_id),
              DLB2_SYS_DIR_PP2VAS_RST);

  DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ2VAS(hw->ver, port->id.phys_id),
              DLB2_CHP_DIR_CQ2VAS_RST);

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_PP2VDEV(port->id.phys_id),
  ...
  if (port->id.vdev_owned) {
    ...
    offs = port->id.vdev_id * DLB2_MAX_NUM_DIR_PORTS(hw->ver) + virt_id;

    DLB2_CSR_WR(hw, DLB2_SYS_VF_DIR_VPP2PP(offs), DLB2_SYS_VF_DIR_VPP2PP_RST);

    DLB2_CSR_WR(hw, DLB2_SYS_VF_DIR_VPP_V(offs), DLB2_SYS_VF_DIR_VPP_V_RST);
  }

  DLB2_CSR_WR(hw, DLB2_SYS_DIR_PP_V(port->id.phys_id), DLB2_SYS_DIR_PP_V_RST);
}

static void
dlb2_domain_reset_dir_port_registers(struct dlb2_hw *hw,
                                     struct dlb2_hw_domain *domain) {
  struct dlb2_list_entry *iter;
  struct dlb2_dir_pq_pair *port;
  RTE_SET_USED(iter);
...
static void
dlb2_domain_reset_ldb_queue_registers(struct dlb2_hw *hw,
                                      struct dlb2_hw_domain *domain) {
  struct dlb2_list_entry *iter;
  struct dlb2_ldb_queue *queue;
  RTE_SET_USED(iter);
  ...
  DLB2_DOM_LIST_FOR(domain->used_ldb_queues, queue, iter) {
    ...
    DLB2_CSR_WR(hw, DLB2_LSP_QID_NALDB_TOT_ENQ_CNTL(hw->ver, queue_id),
                DLB2_LSP_QID_NALDB_TOT_ENQ_CNTL_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_NALDB_TOT_ENQ_CNTH(hw->ver, queue_id),
                DLB2_LSP_QID_NALDB_TOT_ENQ_CNTH_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_ATM_TOT_ENQ_CNTL(hw->ver, queue_id),
                DLB2_LSP_QID_ATM_TOT_ENQ_CNTL_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_ATM_TOT_ENQ_CNTH(hw->ver, queue_id),
                DLB2_LSP_QID_ATM_TOT_ENQ_CNTH_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_NALDB_MAX_DEPTH(hw->ver, queue_id),
                DLB2_LSP_QID_NALDB_MAX_DEPTH_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_LDB_INFL_LIM(hw->ver, queue_id),
                DLB2_LSP_QID_LDB_INFL_LIM_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_AQED_ACTIVE_LIM(hw->ver, queue_id),
                DLB2_LSP_QID_AQED_ACTIVE_LIM_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_ATM_DEPTH_THRSH(hw->ver, queue_id),
                DLB2_LSP_QID_ATM_DEPTH_THRSH_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_NALDB_DEPTH_THRSH(hw->ver, queue_id),
                DLB2_LSP_QID_NALDB_DEPTH_THRSH_RST);

    DLB2_CSR_WR(hw, DLB2_SYS_LDB_QID_ITS(queue_id), DLB2_SYS_LDB_QID_ITS_RST);

    DLB2_CSR_WR(hw, DLB2_CHP_ORD_QID_SN(hw->ver, queue_id),
                DLB2_CHP_ORD_QID_SN_RST);

    DLB2_CSR_WR(hw, DLB2_CHP_ORD_QID_SN_MAP(hw->ver, queue_id),
                DLB2_CHP_ORD_QID_SN_MAP_RST);

    DLB2_CSR_WR(hw, DLB2_SYS_LDB_QID_V(queue_id), DLB2_SYS_LDB_QID_V_RST);

    DLB2_CSR_WR(hw, DLB2_SYS_LDB_QID_CFG_V(queue_id),
                DLB2_SYS_LDB_QID_CFG_V_RST);

    if (queue->sn_cfg_valid) {
      u32 offs[2];

      offs[0] = DLB2_RO_GRP_0_SLT_SHFT(hw->ver, queue->sn_slot);
      offs[1] = DLB2_RO_GRP_1_SLT_SHFT(hw->ver, queue->sn_slot);

      DLB2_CSR_WR(hw, offs[queue->sn_group], DLB2_RO_GRP_0_SLT_SHFT_RST);
    }

    for (i = 0; i < DLB2_LSP_QID2CQIDIX_NUM; i++) {
      DLB2_CSR_WR(hw, DLB2_LSP_QID2CQIDIX(hw->ver, queue_id, i),
                  DLB2_LSP_QID2CQIDIX_00_RST);

      DLB2_CSR_WR(hw, DLB2_LSP_QID2CQIDIX2(hw->ver, queue_id, i),
                  DLB2_LSP_QID2CQIDIX2_00_RST);

      DLB2_CSR_WR(hw, DLB2_ATM_QID2CQIDIX(queue_id, i),
...
static void
dlb2_domain_reset_dir_queue_registers(struct dlb2_hw *hw,
                                      struct dlb2_hw_domain *domain) {
  struct dlb2_list_entry *iter;
  struct dlb2_dir_pq_pair *queue;
  RTE_SET_USED(iter);
  ...
  DLB2_DOM_LIST_FOR(domain->used_dir_pq_pairs, queue, iter) {
    DLB2_CSR_WR(hw, DLB2_LSP_QID_DIR_MAX_DEPTH(hw->ver, queue->id.phys_id),
                DLB2_LSP_QID_DIR_MAX_DEPTH_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_DIR_TOT_ENQ_CNTL(hw->ver, queue->id.phys_id),
                DLB2_LSP_QID_DIR_TOT_ENQ_CNTL_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_DIR_TOT_ENQ_CNTH(hw->ver, queue->id.phys_id),
                DLB2_LSP_QID_DIR_TOT_ENQ_CNTH_RST);

    DLB2_CSR_WR(hw, DLB2_LSP_QID_DIR_DEPTH_THRSH(hw->ver, queue->id.phys_id),
                DLB2_LSP_QID_DIR_DEPTH_THRSH_RST);

    DLB2_CSR_WR(hw, DLB2_SYS_DIR_QID_ITS(queue->id.phys_id),
                DLB2_SYS_DIR_QID_ITS_RST);

    DLB2_CSR_WR(hw, DLB2_SYS_DIR_QID_V(queue->id.phys_id),
...
static void dlb2_domain_reset_registers(struct dlb2_hw *hw,
  ...
  if (hw->ver == DLB2_HW_V2) {
    DLB2_CSR_WR(hw, DLB2_CHP_CFG_LDB_VAS_CRD(domain->id.phys_id),
                DLB2_CHP_CFG_LDB_VAS_CRD_RST);

    DLB2_CSR_WR(hw, DLB2_CHP_CFG_DIR_VAS_CRD(domain->id.phys_id),
                DLB2_CHP_CFG_DIR_VAS_CRD_RST);
  } else
    DLB2_CSR_WR(hw, DLB2_CHP_CFG_VAS_CRD(domain->id.phys_id),
...
static int dlb2_domain_reset_software_state(struct dlb2_hw *hw,
  ...
  DLB2_DOM_LIST_FOR_SAFE(*list, ldb_queue, tmp_ldb_queue, iter1, iter2) {
    ...
    dlb2_list_del(&domain->used_ldb_queues, &ldb_queue->domain_list);
    dlb2_list_add(&rsrcs->avail_ldb_queues, &ldb_queue->func_list);
  ...
  DLB2_DOM_LIST_FOR_SAFE(*list, ldb_queue, tmp_ldb_queue, iter1, iter2) {
    ...
    dlb2_list_del(&domain->avail_ldb_queues, &ldb_queue->domain_list);
    dlb2_list_add(&rsrcs->avail_ldb_queues, &ldb_queue->func_list);
    rsrcs->num_avail_ldb_queues++;
  }
  ...
  for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
    ...
    DLB2_DOM_LIST_FOR_SAFE(*list, ldb_port, tmp_ldb_port, iter1, iter2) {
      int j;
      ...
      for (j = 0; j < DLB2_MAX_NUM_QIDS_PER_LDB_CQ; j++)
        ldb_port->qid_map[j].state = DLB2_QUEUE_UNMAPPED;

      dlb2_list_del(&domain->used_ldb_ports[i], &ldb_port->domain_list);
      dlb2_list_add(&rsrcs->avail_ldb_ports[i], &ldb_port->func_list);
    ...
    DLB2_DOM_LIST_FOR_SAFE(*list, ldb_port, tmp_ldb_port, iter1, iter2) {
      ...
      dlb2_list_del(&domain->avail_ldb_ports[i], &ldb_port->domain_list);
      dlb2_list_add(&rsrcs->avail_ldb_ports[i], &ldb_port->func_list);
      rsrcs->num_avail_ldb_ports[i]++;
    }
  }
  ...
  DLB2_DOM_LIST_FOR_SAFE(*list, dir_port, tmp_dir_port, iter1, iter2) {
    ...
    dlb2_list_del(&domain->used_dir_pq_pairs, &dir_port->domain_list);

    dlb2_list_add(&rsrcs->avail_dir_pq_pairs, &dir_port->func_list);
  ...
  DLB2_DOM_LIST_FOR_SAFE(*list, dir_port, tmp_dir_port, iter1, iter2) {
    ...
    dlb2_list_del(&domain->avail_dir_pq_pairs, &dir_port->domain_list);
  ...
  if (ret) {
    DLB2_HW_ERR(hw,
                "[%s()] Internal error: domain hist list base does not match "
                "the function's bitmap.\n",
                __func__);
    return ret;
  }
...
static int dlb2_domain_drain_unmapped_queue(struct dlb2_hw *hw,
  ...
  if (port == NULL) {
    DLB2_HW_ERR(hw, "[%s()] Internal error: No configured LDB ports\n",
                __func__);
    return -EFAULT;
  }
...
static int dlb2_domain_drain_unmapped_queues(struct dlb2_hw *hw,
   ...
   */
  DLB2_DOM_LIST_FOR(domain->used_ldb_queues, queue, iter) {
    if (queue->num_mappings != 0 || dlb2_ldb_queue_is_empty(hw, queue))
      continue;
 ...
 */
int dlb2_reset_domain(struct dlb2_hw *hw, u32 domain_id, bool vdev_req,
                      unsigned int vdev_id) {
  struct dlb2_hw_domain *domain;
  int ret;
...
static void
dlb2_log_create_ldb_queue_args(struct dlb2_hw *hw, u32 domain_id,
...
dlb2_ldb_queue_attach_to_sn_group(struct dlb2_hw *hw,
  ...
  for (i = 0; i < DLB2_MAX_NUM_SEQUENCE_NUMBER_GROUPS; i++) {
    ...
    if (group->sequence_numbers_per_queue == args->num_sequence_numbers &&
  ...
  if (slot == -1) {
    DLB2_HW_ERR(
        hw, "[%s():%d] Internal error: no sequence number slots available\n",
        __func__, __LINE__);
    return -EFAULT;
  }
...
static int dlb2_verify_create_ldb_queue_args(
    struct dlb2_hw *hw, u32 domain_id, struct dlb2_create_ldb_queue_args *args,
    struct dlb2_cmd_response *resp, bool vdev_req, unsigned int vdev_id,
    struct dlb2_hw_domain **out_domain, struct dlb2_ldb_queue **out_queue) {
  struct dlb2_hw_domain *domain;
  struct dlb2_ldb_queue *queue;
  int i;
  ...
  if (args->num_atomic_inflights && args->lock_id_comp_level != 0 &&
      args->lock_id_comp_level != 64 && args->lock_id_comp_level != 128 &&
      args->lock_id_comp_level != 256 && args->lock_id_comp_level != 512 &&
      args->lock_id_comp_level != 1024 && args->lock_id_comp_level != 2048 &&
      args->lock_id_comp_level != 4096 && args->lock_id_comp_level != 65536) {
...
static int dlb2_ldb_queue_attach_resources(
    struct dlb2_hw *hw, struct dlb2_hw_domain *domain,
    struct dlb2_ldb_queue *queue, struct dlb2_create_ldb_queue_args *args) {
  int ret;
...
static void dlb2_configure_ldb_queue(struct dlb2_hw *hw,
   ...
   * of sequence numbers.
   */
  DLB2_BITS_SET(reg, args->num_qid_inflights, DLB2_LSP_QID_LDB_INFL_LIM_LIMIT);
  DLB2_CSR_WR(hw, DLB2_LSP_QID_LDB_INFL_LIM(hw->ver, queue->id.phys_id), reg);
  ...
  DLB2_CSR_WR(hw, DLB2_LSP_QID_AQED_ACTIVE_LIM(hw->ver, queue->id.phys_id),
              reg);
  ...
  DLB2_CSR_WR(hw, DLB2_LSP_QID_ATM_DEPTH_THRSH(hw->ver, queue->id.phys_id),
              reg);

  reg = 0;
  DLB2_BITS_SET(reg, args->depth_threshold,
                DLB2_LSP_QID_NALDB_DEPTH_THRSH_THRESH);
  DLB2_CSR_WR(hw, DLB2_LSP_QID_NALDB_DEPTH_THRSH(hw->ver, queue->id.phys_id),
              reg);
  ...
  DLB2_CSR_WR(hw, DLB2_CHP_ORD_QID_SN_MAP(hw->ver, queue->id.phys_id), reg);
  ...
  if (vdev_req) {
    ...
    reg = 0;
    DLB2_BITS_SET(reg, queue->id.phys_id, DLB2_SYS_VF_LDB_VQID2QID_QID);
    DLB2_CSR_WR(hw, DLB2_SYS_VF_LDB_VQID2QID(offs), reg);

    reg = 0;
    DLB2_BITS_SET(reg, queue->id.virt_id, DLB2_SYS_LDB_QID2VQID_VQID);
 ...
 * EFAULT - Internal error (resp->status not set).
 */
int dlb2_hw_create_ldb_queue(struct dlb2_hw *hw, u32 domain_id,
                             struct dlb2_create_ldb_queue_args *args,
                             struct dlb2_cmd_response *resp, bool vdev_req,
                             unsigned int vdev_id) {
  struct dlb2_hw_domain *domain;
   ...
   * satisfy the request. This simplifies the error unwinding code.
   */
  ret = dlb2_verify_create_ldb_queue_args(hw, domain_id, args, resp, vdev_req,
                                          vdev_id, &domain, &queue);
  if (ret)
    return ret;
...
static int dlb2_ldb_port_configure_cq(struct dlb2_hw *hw,
  ...
  if (args->cq_depth <= 8) {
    ...
    DLB2_HW_ERR(hw, "[%s():%d] Internal error: invalid CQ depth\n", __func__,
                __LINE__);
    return -EFAULT;
  }

  reg = 0;
  DLB2_BITS_SET(reg, ds, DLB2_CHP_LDB_CQ_TKN_DEPTH_SEL_TOKEN_DEPTH_SELECT);
  DLB2_CSR_WR(hw, DLB2_CHP_LDB_CQ_TKN_DEPTH_SEL(hw->ver, port->id.phys_id),
              reg);
  ...
  if (args->cq_depth < 8) {
    ...
    DLB2_BITS_SET(reg, port->init_tkn_cnt, DLB2_LSP_CQ_LDB_TKN_CNT_TOKEN_COUNT);
    DLB2_CSR_WR(hw, DLB2_LSP_CQ_LDB_TKN_CNT(hw->ver, port->id.phys_id), reg);
  } else {
    DLB2_CSR_WR(hw, DLB2_LSP_CQ_LDB_TKN_CNT(hw->ver, port->id.phys_id),
                DLB2_LSP_CQ_LDB_TKN_CNT_RST);
  }
  ...
  reg = 0;
  DLB2_BITS_SET(reg, ds, DLB2_LSP_CQ_LDB_TKN_DEPTH_SEL_TOKEN_DEPTH_SELECT_V2);
  DLB2_CSR_WR(hw, DLB2_LSP_CQ_LDB_TKN_DEPTH_SEL(hw->ver, port->id.phys_id),
              reg);
  ...
  reg = 0;
  DLB2_BITS_SET(reg, port->hist_list_entry_limit - 1,
  ...
  DLB2_CSR_WR(hw, DLB2_CHP_HIST_LIST_BASE(hw->ver, port->id.phys_id), hl_base);
  ...
  DLB2_CSR_WR(hw, DLB2_LSP_CQ_LDB_INFL_LIM(hw->ver, port->id.phys_id), reg);

  reg = 0;
  DLB2_BITS_SET(reg, DLB2_BITS_GET(hl_base, DLB2_CHP_HIST_LIST_BASE_BASE),
                DLB2_CHP_HIST_LIST_PUSH_PTR_PUSH_PTR);
  DLB2_CSR_WR(hw, DLB2_CHP_HIST_LIST_PUSH_PTR(hw->ver, port->id.phys_id), reg);

  reg = 0;
  DLB2_BITS_SET(reg, DLB2_BITS_GET(hl_base, DLB2_CHP_HIST_LIST_BASE_BASE),
                DLB2_CHP_HIST_LIST_POP_PTR_POP_PTR);
  DLB2_CSR_WR(hw, DLB2_CHP_HIST_LIST_POP_PTR(hw->ver, port->id.phys_id), reg);
  ...
  if (vdev_req && hw->virt_mode == DLB2_VIRT_SIOV) {
    reg = 0;
    DLB2_BITS_SET(reg, hw->pasid[vdev_id], DLB2_SYS_LDB_CQ_PASID_PASID);
...
static bool dlb2_cq_depth_is_valid(u32 depth) {
  if (depth != 1 && depth != 2 && depth != 4 && depth != 8 && depth != 16 &&
      depth != 32 && depth != 64 && depth != 128 && depth != 256 &&
      depth != 512 && depth != 1024)
    return false;
...
static int dlb2_configure_ldb_port(struct dlb2_hw *hw,
  ...
  port->hist_list_entry_base =
      domain->hist_list_entry_base + domain->hist_list_entry_offset;
  port->hist_list_entry_limit =
      port->hist_list_entry_base + args->cq_history_list_size;

  domain->hist_list_entry_offset += args->cq_history_list_size;
  domain->avail_hist_list_entries -= args->cq_history_list_size;

  ret = dlb2_ldb_port_configure_cq(hw, domain, port, cq_dma_base, args,
                                   vdev_req, vdev_id);
  if (ret)
    return ret;

  dlb2_ldb_port_configure_pp(hw, domain, port, vdev_req, vdev_id);
...
static void
dlb2_log_create_ldb_port_args(struct dlb2_hw *hw, u32 domain_id,
                              uintptr_t cq_dma_base,
                              struct dlb2_create_ldb_port_args *args,
                              bool vdev_req, unsigned int vdev_id) {
...
static int dlb2_verify_create_ldb_port_args(
    struct dlb2_hw *hw, u32 domain_id, uintptr_t cq_dma_base,
    struct dlb2_create_ldb_port_args *args, struct dlb2_cmd_response *resp,
    bool vdev_req, unsigned int vdev_id, struct dlb2_hw_domain **out_domain,
    struct dlb2_ldb_port **out_port, int *out_cos_id) {
  struct dlb2_hw_domain *domain;
  struct dlb2_ldb_port *port;
  int i, id;
  ...
  if (args->cos_strict) {
    id = args->cos_id;
    port = DLB2_DOM_LIST_HEAD(domain->avail_ldb_ports[id], typeof(*port));
  } else {
    for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
 ...
 * EFAULT - Internal error (resp->status not set).
 */
int dlb2_hw_create_ldb_port(struct dlb2_hw *hw, u32 domain_id,
                            struct dlb2_create_ldb_port_args *args,
                            uintptr_t cq_dma_base,
                            struct dlb2_cmd_response *resp, bool vdev_req,
                            unsigned int vdev_id) {
  ...
  dlb2_log_create_ldb_port_args(hw, domain_id, cq_dma_base, args, vdev_req,
                                vdev_id);
   ...
   * satisfy the request. This simplifies the error unwinding code.
   */
  ret = dlb2_verify_create_ldb_port_args(hw, domain_id, cq_dma_base, args, resp,
                                         vdev_req, vdev_id, &domain, &port,
                                         &cos_id);
  if (ret)
    return ret;

  ret = dlb2_configure_ldb_port(hw, domain, port, cq_dma_base, args, vdev_req,
                                vdev_id);
  if (ret)
    return ret;
...
static void
dlb2_log_create_dir_port_args(struct dlb2_hw *hw, u32 domain_id,
                              uintptr_t cq_dma_base,
...
dlb2_get_domain_used_dir_pq(struct dlb2_hw *hw, u32 id, bool vdev_req,
                            struct dlb2_hw_domain *domain) {
  struct dlb2_list_entry *iter;
  struct dlb2_dir_pq_pair *port;
  RTE_SET_USED(iter);
...
static int dlb2_verify_create_dir_port_args(
    struct dlb2_hw *hw, u32 domain_id, uintptr_t cq_dma_base,
    struct dlb2_create_dir_port_args *args, struct dlb2_cmd_response *resp,
    bool vdev_req, unsigned int vdev_id, struct dlb2_hw_domain **out_domain,
  ...
  if (args->queue_id != -1) {
     ...
     * configured.
     */
    pq = dlb2_get_domain_used_dir_pq(hw, args->queue_id, vdev_req, domain);
     ...
     */
    pq = DLB2_DOM_LIST_HEAD(domain->avail_dir_pq_pairs, typeof(*pq));
    if (!pq) {
      resp->status = DLB2_ST_DIR_PORTS_UNAVAILABLE;
      return -EINVAL;
    }
...
static int dlb2_dir_port_configure_cq(struct dlb2_hw *hw,
  ...
  reg = 0;
  DLB2_BITS_SET(reg, ds, DLB2_CHP_DIR_CQ_TKN_DEPTH_SEL_TOKEN_DEPTH_SELECT);
  DLB2_CSR_WR(hw, DLB2_CHP_DIR_CQ_TKN_DEPTH_SEL(hw->ver, port->id.phys_id),
              reg);
  ...
  if (args->cq_depth < 8) {
    ...
    DLB2_BITS_SET(reg, port->init_tkn_cnt, DLB2_LSP_CQ_DIR_TKN_CNT_COUNT);
    DLB2_CSR_WR(hw, DLB2_LSP_CQ_DIR_TKN_CNT(hw->ver, port->id.phys_id), reg);
  } else {
    DLB2_CSR_WR(hw, DLB2_LSP_CQ_DIR_TKN_CNT(hw->ver, port->id.phys_id),
                DLB2_LSP_CQ_DIR_TKN_CNT_RST);
  }
  ...
  if (vdev_req && hw->virt_mode == DLB2_VIRT_SIOV) {
    DLB2_BITS_SET(reg, hw->pasid[vdev_id], DLB2_SYS_DIR_CQ_PASID_PASID);
...
static int dlb2_configure_dir_port(struct dlb2_hw *hw,
  ...
  ret = dlb2_dir_port_configure_cq(hw, domain, port, cq_dma_base, args,
                                   vdev_req, vdev_id);

  if (ret)
    return ret;

  dlb2_dir_port_configure_pp(hw, domain, port, vdev_req, vdev_id);
 ...
 * EFAULT - Internal error (resp->status not set).
 */
int dlb2_hw_create_dir_port(struct dlb2_hw *hw, u32 domain_id,
                            struct dlb2_create_dir_port_args *args,
                            uintptr_t cq_dma_base,
                            struct dlb2_cmd_response *resp, bool vdev_req,
                            unsigned int vdev_id) {
  ...
  dlb2_log_create_dir_port_args(hw, domain_id, cq_dma_base, args, vdev_req,
                                vdev_id);
   ...
   * satisfy the request. This simplifies the error unwinding code.
   */
  ret = dlb2_verify_create_dir_port_args(hw, domain_id, cq_dma_base, args, resp,
                                         vdev_req, vdev_id, &domain, &port);
  if (ret)
    return ret;

  ret = dlb2_configure_dir_port(hw, domain, port, cq_dma_base, args, vdev_req,
                                vdev_id);
  if (ret)
    return ret;
...
static void dlb2_configure_dir_queue(struct dlb2_hw *hw,
  ...
  /* QID write permissions are turned on when the domain is started */
  offs =
      domain->id.phys_id * DLB2_MAX_NUM_DIR_QUEUES(hw->ver) + queue->id.phys_id;
  ...
  if (vdev_req) {
    ...
    reg = 0;
    DLB2_BITS_SET(reg, queue->id.phys_id, DLB2_SYS_VF_DIR_VQID2QID_QID);
...
static void
dlb2_log_create_dir_queue_args(struct dlb2_hw *hw, u32 domain_id,
                               struct dlb2_create_dir_queue_args *args,
                               bool vdev_req, unsigned int vdev_id) {
...
static int dlb2_verify_create_dir_queue_args(
    struct dlb2_hw *hw, u32 domain_id, struct dlb2_create_dir_queue_args *args,
    struct dlb2_cmd_response *resp, bool vdev_req, unsigned int vdev_id,
    struct dlb2_hw_domain **out_domain, struct dlb2_dir_pq_pair **out_queue) {
  struct dlb2_hw_domain *domain;
  struct dlb2_dir_pq_pair *pq;
   ...
   */
  if (args->port_id != -1) {
    pq = dlb2_get_domain_used_dir_pq(hw, args->port_id, vdev_req, domain);
 ...
 * EFAULT - Internal error (resp->status not set).
 */
int dlb2_hw_create_dir_queue(struct dlb2_hw *hw, u32 domain_id,
                             struct dlb2_create_dir_queue_args *args,
                             struct dlb2_cmd_response *resp, bool vdev_req,
                             unsigned int vdev_id) {
  struct dlb2_dir_pq_pair *queue;
   ...
   * satisfy the request. This simplifies the error unwinding code.
   */
  ret = dlb2_verify_create_dir_queue_args(hw, domain_id, args, resp, vdev_req,
                                          vdev_id, &domain, &queue);
  if (ret)
    return ret;
   ...
   */
  if (args->port_id == -1) {
    dlb2_list_del(&domain->avail_dir_pq_pairs, &queue->domain_list);

    dlb2_list_add(&domain->used_dir_pq_pairs, &queue->domain_list);
  }
...
static bool dlb2_port_find_slot_with_pending_map_queue(
    struct dlb2_ldb_port *port, struct dlb2_ldb_queue *queue, int *slot) {
  int i;
...
dlb2_get_domain_ldb_queue(u32 id, bool vdev_req,
                          struct dlb2_hw_domain *domain) {
  struct dlb2_list_entry *iter;
  struct dlb2_ldb_queue *queue;
  RTE_SET_USED(iter);
...
static struct dlb2_ldb_port *
dlb2_get_domain_used_ldb_port(u32 id, bool vdev_req,
                              struct dlb2_hw_domain *domain) {
  struct dlb2_list_entry *iter;
  struct dlb2_ldb_port *port;
  int i;
...
static void dlb2_ldb_port_change_qid_priority(struct dlb2_hw *hw,
  ...
  /* Read-modify-write the priority and valid bit register */
  cq2priov = DLB2_CSR_RD(hw, DLB2_LSP_CQ2PRIOV(hw->ver, port->id.phys_id));

  cq2priov |= (1 << (slot + DLB2_LSP_CQ2PRIOV_V_LOC)) & DLB2_LSP_CQ2PRIOV_V;
  cq2priov |= ((args->priority & 0x7) << slot * 3) & DLB2_LSP_CQ2PRIOV_PRIO;
...
static int dlb2_verify_map_qid_args(struct dlb2_hw *hw, u32 domain_id,
                                    struct dlb2_map_qid_args *args,
                                    struct dlb2_cmd_response *resp,
                                    bool vdev_req, unsigned int vdev_id,
...
static void dlb2_log_map_qid(struct dlb2_hw *hw, u32 domain_id,
                             struct dlb2_map_qid_args *args, bool vdev_req,
                             unsigned int vdev_id) {
 ...
 */
int dlb2_hw_map_qid(struct dlb2_hw *hw, u32 domain_id,
                    struct dlb2_map_qid_args *args,
                    struct dlb2_cmd_response *resp, bool vdev_req,
                    unsigned int vdev_id) {
  struct dlb2_hw_domain *domain;
   ...
   * satisfy the request. This simplifies the error unwinding code.
   */
  ret = dlb2_verify_map_qid_args(hw, domain_id, args, resp, vdev_req, vdev_id,
                                 &domain, &port, &queue);
  if (ret)
    return ret;
  ...
  if (!dlb2_port_find_slot(port, DLB2_QUEUE_UNMAPPED, &i)) {
    if (dlb2_port_find_slot(port, DLB2_QUEUE_UNMAP_IN_PROG, &i)) {
      ...
      ret = dlb2_port_slot_state_transition(hw, port, queue, i, new_st);
      if (ret)
        return ret;
...
static void dlb2_log_unmap_qid(struct dlb2_hw *hw, u32 domain_id,
                               struct dlb2_unmap_qid_args *args, bool vdev_req,
                               unsigned int vdev_id) {
...
static int dlb2_verify_unmap_qid_args(struct dlb2_hw *hw, u32 domain_id,
 ...
 * EFAULT - Internal error (resp->status not set).
 */
int dlb2_hw_unmap_qid(struct dlb2_hw *hw, u32 domain_id,
                      struct dlb2_unmap_qid_args *args,
                      struct dlb2_cmd_response *resp, bool vdev_req,
                      unsigned int vdev_id) {
  struct dlb2_hw_domain *domain;
   ...
   * satisfy the request. This simplifies the error unwinding code.
   */
  ret = dlb2_verify_unmap_qid_args(hw, domain_id, args, resp, vdev_req, vdev_id,
                                   &domain, &port, &queue);
  if (ret)
    return ret;
  ...
  st = DLB2_QUEUE_MAPPED;
  if (!dlb2_port_find_slot_queue(port, st, queue, &i)) {
    DLB2_HW_ERR(hw, "[%s()] Internal error: no available CQ slots\n", __func__);
    return -EFAULT;
  }
 ...
 */
int dlb2_hw_pending_port_unmaps(struct dlb2_hw *hw, u32 domain_id,
                                struct dlb2_pending_port_unmaps_args *args,
                                struct dlb2_cmd_response *resp, bool vdev_req,
                                unsigned int vdev_id) {
  struct dlb2_hw_domain *domain;
...
static int dlb2_verify_start_domain_args(struct dlb2_hw *hw, u32 domain_id,
                                         struct dlb2_cmd_response *resp,
                                         bool vdev_req, unsigned int vdev_id,
                                         struct dlb2_hw_domain **out_domain) {
  struct dlb2_hw_domain *domain;
...
static void dlb2_log_start_domain(struct dlb2_hw *hw, u32 domain_id,
                                  bool vdev_req, unsigned int vdev_id) {
 ...
 */
int dlb2_hw_start_domain(struct dlb2_hw *hw, u32 domain_id,
  ...
  DLB2_DOM_LIST_FOR(domain->used_ldb_queues, ldb_queue, iter) {
    ...
    offs = domain->id.phys_id * DLB2_MAX_NUM_LDB_QUEUES + ldb_queue->id.phys_id;
...
static void dlb2_log_get_dir_queue_depth(struct dlb2_hw *hw, u32 domain_id,
 ...
 */
int dlb2_hw_get_dir_queue_depth(struct dlb2_hw *hw, u32 domain_id,
  ...
  dlb2_log_get_dir_queue_depth(hw, domain_id, args->queue_id, vdev_req,
                               vdev_id);
...
static void dlb2_log_get_ldb_queue_depth(struct dlb2_hw *hw, u32 domain_id,
                                         u32 queue_id, bool vdev_req,
                                         unsigned int vf_id) {
 ...
 * EINVAL - Invalid domain ID or queue ID.
 */
int dlb2_hw_get_ldb_queue_depth(struct dlb2_hw *hw, u32 domain_id,
  ...
  dlb2_log_get_ldb_queue_depth(hw, domain_id, args->queue_id, vdev_req,
                               vdev_id);
...
void dlb2_hw_enable_sparse_dir_cq_mode(struct dlb2_hw *hw) {
  ...
  DLB2_BIT_SET(ctrl, DLB2_CHP_CFG_CHP_CSR_CTRL_CFG_64BYTES_QE_DIR_CQ_MODE);
...
void dlb2_hw_enable_sparse_ldb_cq_mode(struct dlb2_hw *hw) {
  ...
  DLB2_BIT_SET(ctrl, DLB2_CHP_CFG_CHP_CSR_CTRL_CFG_64BYTES_QE_LDB_CQ_MODE);
 ...
 */
int dlb2_set_group_sequence_numbers(struct dlb2_hw *hw, u32 group_id, u32 val) {
