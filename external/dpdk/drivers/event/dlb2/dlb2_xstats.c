...
static uint64_t dlb2_device_traffic_stat_get(struct dlb2_eventdev *dlb2,
                                             int which_stat) {
  int i;
  uint64_t val = 0;
...
static uint64_t get_dev_stat(struct dlb2_eventdev *dlb2,
                             uint16_t obj_idx __rte_unused,
                             enum dlb2_xstats_type type,
...
static uint64_t get_port_stat(struct dlb2_eventdev *dlb2, uint16_t obj_idx,
                              enum dlb2_xstats_type type,
                              int extra_arg __rte_unused) {
  ...
  switch (type) {
  case rx_ok:
    return ev_port->stats.traffic.rx_ok;

  case rx_drop:
    return ev_port->stats.traffic.rx_drop;

  case rx_interrupt_wait:
    return ev_port->stats.traffic.rx_interrupt_wait;

  case rx_umonitor_umwait:
    return ev_port->stats.traffic.rx_umonitor_umwait;

  case tx_ok:
    return ev_port->stats.traffic.tx_ok;

  case total_polls:
    return ev_port->stats.traffic.total_polls;

  case zero_polls:
    return ev_port->stats.traffic.zero_polls;
  ...
  case is_configured:
    return ev_port->setup_done;

  case is_load_balanced:
    return !ev_port->qm_port.is_directed;

  case hw_id:
    return ev_port->qm_port.id;

  case tx_new:
    return ev_port->stats.tx_op_cnt[RTE_EVENT_OP_NEW];

  case tx_fwd:
    return ev_port->stats.tx_op_cnt[RTE_EVENT_OP_FORWARD];

  case tx_rel:
    return ev_port->stats.tx_op_cnt[RTE_EVENT_OP_RELEASE];

  case tx_implicit_rel:
    return ev_port->stats.tx_implicit_rel;
  ...
  case tx_invalid:
    return ev_port->stats.tx_invalid;

  case outstanding_releases:
    return ev_port->outstanding_releases;
  ...
  case rx_sched_invalid:
    return ev_port->stats.rx_sched_invalid;
...
static uint64_t dlb2_get_threshold_stat(struct dlb2_eventdev *dlb2, int qid,
                                        int stat) {
  int port = 0;
  uint64_t tally = 0;
...
static uint64_t dlb2_get_enq_ok_stat(struct dlb2_eventdev *dlb2, int qid) {
  int port = 0;
...
static uint64_t get_queue_stat(struct dlb2_eventdev *dlb2, uint16_t obj_idx,
                               enum dlb2_xstats_type type,
                               int extra_arg __rte_unused) {
  struct dlb2_eventdev_queue *ev_queue = &dlb2->ev_queues[obj_idx];

  switch (type) {
  case is_configured:
    return ev_queue->setup_done;

  case is_load_balanced:
    return !ev_queue->qm_queue.is_directed;

  case hw_id:
    return ev_queue->qm_queue.id;

  case num_links:
    return ev_queue->num_links;

  case sched_type:
    return ev_queue->qm_queue.sched_type;

  case enq_ok:
    return dlb2_get_enq_ok_stat(dlb2, obj_idx);

  case current_depth:
    return dlb2_get_queue_depth(dlb2, ev_queue);

  case depth_threshold:
    return ev_queue->depth_threshold;

  case depth_le50_threshold:
    return dlb2_get_threshold_stat(dlb2, ev_queue->id, DLB2_QID_DEPTH_LE50);
  ...
  case depth_gt100_threshold:
    return dlb2_get_threshold_stat(dlb2, ev_queue->id, DLB2_QID_DEPTH_GT100);
...
int dlb2_xstats_init(struct dlb2_eventdev *dlb2) {
  ...
  static const char *const port_stats[] = {"is_configured",
                                           "is_load_balanced",
                                           "hw_id",
                                           "rx_ok",
                                           "rx_drop",
                                           ...
                                           "rx_sched_directed",
                                           "rx_sched_invalid"};
  static const enum dlb2_xstats_type port_types[] = {is_configured,
                                                     is_load_balanced,
                                                     hw_id,
                                                     rx_ok,
                                                     rx_drop,
  ...
  dlb2->xstats = rte_zmalloc_socket(NULL, sizeof(dlb2->xstats[0]) * count, 0,
...
#define sname dlb2->xstats[stat_id].name.name
  ...
  for (port = 0; port < DLB2_MAX_NUM_PORTS(dlb2->version); port++) {
    ...
    for (i = 0; i < RTE_DIM(port_stats); i++, stat_id++) {
      ...
      snprintf(sname, sizeof(sname), "port_%u_%s", port, port_stats[i]);
    }
  ...
  for (qid = 0; qid < DLB2_MAX_NUM_QUEUES(dlb2->version); qid++) {
    ...
    for (i = 0; i < RTE_DIM(qid_stats); i++, stat_id++) {
      ...
      snprintf(sname, sizeof(sname), "qid_%u_%s", qid, qid_stats[i]);
    }

    dlb2->xstats_count_per_qid[qid] = stat_id - count_offset;
  }

  dlb2->xstats_count_mode_queue =
      stat_id - (dlb2->xstats_count_mode_dev + dlb2->xstats_count_mode_port);
#undef sname
...
int dlb2_eventdev_xstats_get_names(
    const struct rte_eventdev *dev, enum rte_event_dev_xstats_mode mode,
...
static int dlb2_xstats_update(struct dlb2_eventdev *dlb2,
                              enum rte_event_dev_xstats_mode mode,
  ...
  for (i = 0; i < n && xidx < xstats_mode_count; i++) {
    ...
    if (mode != RTE_EVENT_DEV_XSTATS_DEVICE && queue_port_id != xs->obj_idx)
      continue;
...
int dlb2_eventdev_xstats_get(const struct rte_eventdev *dev,
  ...
  return dlb2_xstats_update(dlb2, mode, queue_port_id, ids, values, n, reset);
}
...
uint64_t dlb2_eventdev_xstats_get_by_name(const struct rte_eventdev *dev,
  ...
  for (i = 0; i < dlb2->xstats_count; i++) {
    ...
    if (strncmp(xs->name.name, name, RTE_EVENT_DEV_XSTATS_NAME_SIZE) == 0) {
      if (id != NULL)
        *id = i;
      ...
      return fn(dlb2, xs->obj_idx, xs->stat, xs->extra_arg) - xs->reset_value;
    }
  }
  if (id != NULL)
    *id = (uint32_t)-1;
...
static void dlb2_xstats_reset_range(struct dlb2_eventdev *dlb2, uint32_t start,
                                    uint32_t num) {
  uint32_t i;
  dlb2_xstats_fn fn;
...
static int dlb2_xstats_reset_queue(struct dlb2_eventdev *dlb2, uint8_t queue_id,
                                   const uint32_t ids[], uint32_t nb_ids) {
  const uint32_t reset = 1;

  if (ids) {
    uint32_t nb_reset = dlb2_xstats_update(dlb2, RTE_EVENT_DEV_XSTATS_QUEUE,
                                           queue_id, ids, NULL, nb_ids, reset);
    return nb_reset == nb_ids ? 0 : -EINVAL;
  }

  if (ids == NULL)
    dlb2_xstats_reset_range(dlb2, dlb2->xstats_offset_for_qid[queue_id],
                            dlb2->xstats_count_per_qid[queue_id]);

  return 0;
}

static int dlb2_xstats_reset_port(struct dlb2_eventdev *dlb2, uint8_t port_id,
                                  const uint32_t ids[], uint32_t nb_ids) {
  const uint32_t reset = 1;
  ...
  if (ids) {
    uint32_t nb_reset = dlb2_xstats_update(dlb2, RTE_EVENT_DEV_XSTATS_PORT,
                                           port_id, ids, NULL, nb_ids, reset);
    return nb_reset == nb_ids ? 0 : -EINVAL;
  }
...
static int dlb2_xstats_reset_dev(struct dlb2_eventdev *dlb2,
                                 const uint32_t ids[], uint32_t nb_ids) {
  uint32_t i;
...
int dlb2_eventdev_xstats_reset(struct rte_eventdev *dev,
  ...
  case RTE_EVENT_DEV_XSTATS_PORT:
    if (queue_port_id == -1) {
      for (i = 0; i < DLB2_MAX_NUM_PORTS(dlb2->version); i++) {
        if (dlb2_xstats_reset_port(dlb2, i, ids, nb_ids))
          return -EINVAL;
      }
    } else if (queue_port_id < DLB2_MAX_NUM_PORTS(dlb2->version)) {
      if (dlb2_xstats_reset_port(dlb2, queue_port_id, ids, nb_ids))
        return -EINVAL;
    }
    break;
  case RTE_EVENT_DEV_XSTATS_QUEUE:
    if (queue_port_id == -1) {
      for (i = 0; i < DLB2_MAX_NUM_QUEUES(dlb2->version); i++) {
        if (dlb2_xstats_reset_queue(dlb2, i, ids, nb_ids))
          return -EINVAL;
      }
    } else if (queue_port_id < DLB2_MAX_NUM_QUEUES(dlb2->version)) {
      if (dlb2_xstats_reset_queue(dlb2, queue_port_id, ids, nb_ids))
        return -EINVAL;
    }
    break;
  };
...
void dlb2_eventdev_dump(struct rte_eventdev *dev, FILE *f) {
  ...
  fprintf(f, "domain ID=%u, socket_id=%u, evdev=%p\n", handle->domain_id,
          handle->info.socket_id, dlb2->event_dev);
  ...
  for (i = 0; i < dlb2->num_ports; i++) {
    ...
    for (j = 0; j < DLB2_MAX_NUM_QIDS_PER_LDB_CQ; j++) {
      if (p->link[j].valid)
        fprintf(f, "id=%u prio=%u ", p->link[j].queue_id, p->link[j].priority);
    }
    fprintf(f, "\n");
    ...
    fprintf(f, "\tgenbit=%d, cq_idx=%d, cq_depth=%d\n", p->qm_port.gen_bit,
            p->qm_port.cq_idx, p->qm_port.cq_depth);
  ...
  for (i = 0; i < dlb2->num_queues; i++) {
    ...
    for (j = 0; j < dlb2->num_ports; j++) {
      ...
      for (k = 0; k < DLB2_MAX_NUM_QIDS_PER_LDB_CQ; k++) {
        if (p->link[k].valid && p->link[k].queue_id == q->id)
          fprintf(f, "id=%u prio=%u ", p->id, p->link[k].priority);
      }
    }
    fprintf(f, "\n");
