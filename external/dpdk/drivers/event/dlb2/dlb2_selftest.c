...
static inline int create_ports(int num_ports) {
  int i;
...
static inline int enqueue_timeout(uint8_t port_id, struct rte_event *ev,
                                  uint64_t tmo_us) {
...
static void flush(uint8_t id __rte_unused, struct rte_event event,
...
test_stop_flush(struct test *t) /* test to check we can properly flush events */
{
  ...
  if (init(t, 2, 1) < 0 || create_ports(1) < 0 ||
  ...
  if (rte_event_port_attr_get(evdev, 0, RTE_EVENT_PORT_ATTR_DEQ_DEPTH,
                              &dequeue_depth)) {
...
static int test_single_link(void) {
  struct rte_event_dev_config config = {0};
...
static int test_reconfiguration_link(void) {
  struct rte_event_dev_config config = {0};
  struct rte_event_queue_conf queue_conf;
  struct rte_event_port_conf port_conf;
  struct rte_event_dev_info info;
  ...
  if (rte_event_port_setup(evdev, config.nb_event_ports - 1, &port_conf) < 0) {
    printf("%d: port 1 setup expected to succeed\n", __LINE__);
    goto err;
  }
...
static int test_load_balanced_traffic(void) {
  uint64_t timeout;
  struct rte_event_dev_config config = {0};
  struct rte_event_queue_conf queue_conf;
  struct rte_event_port_conf port_conf;
...
static int test_directed_traffic(void) {
  uint64_t timeout;
  struct rte_event_dev_config config = {0};
  struct rte_event_queue_conf queue_conf;
  struct rte_event_port_conf port_conf;
...
static int test_deferred_sched(void) {
  uint64_t timeout;
  struct rte_event_dev_config config = {0};
  struct rte_event_queue_conf queue_conf;
  struct rte_event_port_conf port_conf;
...
static int test_delayed_pop(void) {
  uint64_t timeout;
  struct rte_event_dev_config config = {0};
  struct rte_event_queue_conf queue_conf;
  struct rte_event_port_conf port_conf;
...
static int do_selftest(void) {
  ...
  if (!eventdev_func_mempool) {
    eventdev_func_mempool = rte_pktmbuf_pool_create(
        "EVENTDEV_DLB2_ST_POOL", (1 << 12), /* 4k buffers */
...
int test_dlb2_eventdev(void) {
  ...
  for (i = 0; found + skipped < num_evdevs && i < RTE_EVENT_MAX_DEVS; i++) {
