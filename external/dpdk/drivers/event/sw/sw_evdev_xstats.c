...
static uint64_t get_dev_stat(const struct sw_evdev *sw,
                             uint16_t obj_idx __rte_unused,
                             enum xstats_type type,
                             int extra_arg __rte_unused) {
  switch (type) {
  case rx:
    return sw->stats.rx_pkts;
  case tx:
    return sw->stats.tx_pkts;
  case dropped:
    return sw->stats.rx_dropped;
  case calls:
    return sw->sched_called;
  case no_iq_enq:
    return sw->sched_no_iq_enqueues;
  case no_cq_enq:
    return sw->sched_no_cq_enqueues;
  case sched_last_iter_bitmask:
    return sw->sched_last_iter_bitmask;
  case sched_progress_last_iter:
    return sw->sched_progress_last_iter;
...
static uint64_t get_port_stat(const struct sw_evdev *sw, uint16_t obj_idx,
                              enum xstats_type type,
                              int extra_arg __rte_unused) {
  ...
  switch (type) {
  case rx:
    return p->stats.rx_pkts;
  case tx:
    return p->stats.tx_pkts;
  case dropped:
    return p->stats.rx_dropped;
  case inflight:
    return p->inflights;
  case pkt_cycles:
    return p->avg_pkt_ticks;
  case calls:
    return p->total_polls;
  case credits:
    return p->inflight_credits;
  case poll_return:
    return p->zero_polls;
  case rx_used:
    return rte_event_ring_count(p->rx_worker_ring);
  case rx_free:
    return rte_event_ring_free_count(p->rx_worker_ring);
  case tx_used:
    return rte_event_ring_count(p->cq_worker_ring);
  case tx_free:
    return rte_event_ring_free_count(p->cq_worker_ring);
  default:
    return -1;
  }
}

static uint64_t get_port_bucket_stat(const struct sw_evdev *sw,
                                     uint16_t obj_idx, enum xstats_type type,
                                     int extra_arg) {
  const struct sw_port *p = &sw->ports[obj_idx];

  switch (type) {
  case poll_return:
    return p->poll_buckets[extra_arg];
  default:
    return -1;
  }
}

static uint64_t get_qid_stat(const struct sw_evdev *sw, uint16_t obj_idx,
                             enum xstats_type type,
                             int extra_arg __rte_unused) {
  ...
  switch (type) {
  case rx:
    return qid->stats.rx_pkts;
  case tx:
    return qid->stats.tx_pkts;
  case dropped:
    return qid->stats.rx_dropped;
  case inflight:
    do {
      uint64_t infl = 0;
      unsigned int i;
...
static uint64_t get_qid_iq_stat(const struct sw_evdev *sw, uint16_t obj_idx,
  ...
  switch (type) {
  case iq_used:
    return iq_count(&qid->iq[iq_idx]);
  default:
    return -1;
  }
}

static uint64_t get_qid_port_stat(const struct sw_evdev *sw, uint16_t obj_idx,
                                  enum xstats_type type, int extra_arg) {
  const struct sw_qid *qid = &sw->qids[obj_idx];
...
int sw_xstats_init(struct sw_evdev *sw) {
  /*
  ...
  static const enum xstats_type dev_types[] = {
      rx,
      tx,
      dropped,
      calls,
  ...
  static const char *const port_stats[] = {
      ...
      "cq_ring_free",
      "dequeue_calls",
      "dequeues_returning_0",
  };
  static const enum xstats_type port_types[] = {
      rx,      tx,      dropped, inflight, pkt_cycles, credits,
      rx_used, rx_free, tx_used, tx_free,  calls,      poll_return,
  };
  static const uint8_t port_reset_allowed[] = {
      1, 1, 1, 0, 1, 0, 0, 0, 0, 0, 1, 1,
  };

  static const char *const port_bucket_stats[] = {"dequeues_returning"};
  ...
  static const char *const qid_stats[] = {"rx", "tx", "drop", "inflight"};
  static const enum xstats_type qid_types[] = {rx, tx, dropped, inflight};
  static const uint8_t qid_reset_allowed[] = {1, 1, 1, 0};

  static const char *const qid_iq_stats[] = {"used"};
  static const enum xstats_type qid_iq_types[] = {iq_used};
  /* reset allowed */

  static const char *const qid_port_stats[] = {"pinned_flows", "packets"};
  ...
  const unsigned int count =
      RTE_DIM(dev_stats) + sw->port_count * RTE_DIM(port_stats) +
      sw->port_count * RTE_DIM(port_bucket_stats) * (cons_bkt_shift + 1) +
...
#define sname sw->xstats[stat].name.name
  ...
  for (port = 0; port < sw->port_count; port++) {
    ...
    for (bkt = 0;
         bkt < (rte_event_ring_get_capacity(sw->ports[port].cq_worker_ring) >>
      ...
      for (i = 0; i < RTE_DIM(port_bucket_stats); i++) {
        ...
        snprintf(sname, sizeof(sname), "port_%u_%s_%u-%u", port,
                 port_bucket_stats[i], (bkt << SW_DEQ_STAT_BUCKET_SHIFT) + 1,
  ...
  for (qid = 0; qid < sw->qid_count; qid++) {
    ...
    for (iq = 0; iq < SW_IQS_MAX; iq++)
      for (i = 0; i < RTE_DIM(qid_iq_stats); i++, stat++) {
        ...
        snprintf(sname, sizeof(sname), "qid_%u_iq_%u_%s", qid, iq,
                 qid_iq_stats[i]);
      }
    ...
    for (port = 0; port < sw->port_count; port++)
      for (i = 0; i < RTE_DIM(qid_port_stats); i++, stat++) {
        sw->xstats[stat] = (struct sw_xstats_entry){
            ...
            .mode = RTE_EVENT_DEV_XSTATS_QUEUE,
            .extra_arg = port,
            .reset_allowed = qid_port_reset_allowed[i],
        };
        snprintf(sname, sizeof(sname), "qid_%u_port_%u_%s", qid, port,
                 qid_port_stats[i]);
      }
  ...
  sw->xstats_count_mode_queue =
      stat - (sw->xstats_count_mode_dev + sw->xstats_count_mode_port);
#undef sname
...
int sw_xstats_uninit(struct sw_evdev *sw) {
  rte_free(sw->xstats);
  sw->xstats_count = 0;
  return 0;
}
...
static int sw_xstats_update(struct sw_evdev *sw,
                            enum rte_event_dev_xstats_mode mode,
  ...
  for (i = 0; i < n && xidx < xstats_mode_count; i++) {
    ...
    uint64_t val =
        xs->fn(sw, xs->obj_idx, xs->stat, xs->extra_arg) - xs->reset_value;
...
int sw_xstats_get(const struct rte_eventdev *dev,
  ...
  return sw_xstats_update(sw, mode, queue_port_id, ids, values, n, reset,
                          ret_n_lt_stats);
}

uint64_t sw_xstats_get_by_name(const struct rte_eventdev *dev, const char *name,
                               unsigned int *id) {
  const struct sw_evdev *sw = sw_pmd_priv_const(dev);
  unsigned int i;
  ...
  for (i = 0; i < sw->xstats_count; i++) {
    ...
    if (strncmp(xs->name.name, name, RTE_EVENT_DEV_XSTATS_NAME_SIZE) == 0) {
      if (id != NULL)
        *id = i;
      return xs->fn(sw, xs->obj_idx, xs->stat, xs->extra_arg) - xs->reset_value;
    }
  }
  if (id != NULL)
    *id = (uint32_t)-1;
...
static void sw_xstats_reset_range(struct sw_evdev *sw, uint32_t start,
                                  uint32_t num) {
  uint32_t i;
...
static int sw_xstats_reset_queue(struct sw_evdev *sw, uint8_t queue_id,
                                 const uint32_t ids[], uint32_t nb_ids) {
  const uint32_t reset = 1;
  const uint32_t ret_n_lt_stats = 0;
  if (ids) {
    uint32_t nb_reset =
        sw_xstats_update(sw, RTE_EVENT_DEV_XSTATS_QUEUE, queue_id, ids, NULL,
                         nb_ids, reset, ret_n_lt_stats);
...
static int sw_xstats_reset_port(struct sw_evdev *sw, uint8_t port_id,
                                const uint32_t ids[], uint32_t nb_ids) {
  const uint32_t reset = 1;
  const uint32_t ret_n_lt_stats = 0;
  ...
  if (ids) {
    uint32_t nb_reset =
        sw_xstats_update(sw, RTE_EVENT_DEV_XSTATS_PORT, port_id, ids, NULL,
                         nb_ids, reset, ret_n_lt_stats);
    return nb_reset == nb_ids ? 0 : -EINVAL;
  }
...
static int sw_xstats_reset_dev(struct sw_evdev *sw, const uint32_t ids[],
                               uint32_t nb_ids) {
  uint32_t i;
  if (ids) {
    for (i = 0; i < nb_ids; i++) {
