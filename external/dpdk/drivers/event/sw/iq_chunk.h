...
static __rte_always_inline bool iq_empty(struct sw_iq *iq) {
  return (iq->count == 0);
}

static __rte_always_inline uint16_t iq_count(const struct sw_iq *iq) {
  return iq->count;
}
...
static __rte_always_inline void iq_free_chunk(struct sw_evdev *sw,
                                              struct sw_queue_chunk *chunk) {
...
static __rte_always_inline void iq_init(struct sw_evdev *sw, struct sw_iq *iq) {
...
static __rte_always_inline void iq_pop(struct sw_evdev *sw, struct sw_iq *iq) {
  iq->head_idx++;
  iq->count--;
...
static __rte_always_inline const struct rte_event *iq_peek(struct sw_iq *iq) {
  return &iq->head->events[iq->head_idx];
}

/* Note: the caller must ensure that count <= iq_count() */
static __rte_always_inline uint16_t iq_dequeue_burst(struct sw_evdev *sw,
                                                     struct sw_iq *iq,
                                                     struct rte_event *ev,
                                                     uint16_t count) {
...
static __rte_always_inline void iq_put_back(struct sw_evdev *sw,
                                            struct sw_iq *iq,
                                            struct rte_event *ev,
                                            unsigned int count) {
