...
static inline counter_dynfield_t *counter_field(struct rte_mbuf *mbuf) {
  return RTE_MBUF_DYNFIELD(mbuf, counter_dynfield_offset, counter_dynfield_t *);
}

static struct rte_event release_ev;

static inline struct rte_mbuf *rte_gen_arp(int portid, struct rte_mempool *mp) {
  /*
   * len = 14 + 46
   * ARP, Request who-has 10.0.0.1 tell 10.0.0.2, length 46
   */
  static const uint8_t arp_request[] = {/*0x0000:*/ 0xff,
                                        0xff,
                                        0xff,
                                        0xff,
                                        0xff,
  ...
  memcpy((void *)((uintptr_t)m->buf_addr + m->data_off), arp_request, pkt_len);
...
static void xstats_print(void) {
  ...
  /* Device names / values */
  int ret = rte_event_dev_xstats_names_get(evdev, RTE_EVENT_DEV_XSTATS_DEVICE,
                                           0, xstats_names, ids, XSTATS_MAX);
  if (ret < 0) {
    printf("%d: xstats names get() returned error\n", __LINE__);
    return;
  }
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_DEVICE, 0, ids,
                                 values, ret);
  ...
  /* Port names / values */
  ret = rte_event_dev_xstats_names_get(evdev, RTE_EVENT_DEV_XSTATS_PORT, 0,
                                       xstats_names, ids, XSTATS_MAX);
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_PORT, 1, ids,
                                 values, ret);
  if (ret > (signed int)XSTATS_MAX)
    printf("%s %d: more xstats available than space\n", __func__, __LINE__);
  for (i = 0; (signed int)i < ret; i++) {
  ...
  /* Queue names / values */
  ret = rte_event_dev_xstats_names_get(evdev, RTE_EVENT_DEV_XSTATS_QUEUE, 0,
                                       xstats_names, ids, XSTATS_MAX);
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_QUEUE, 1, ids,
                                 values, ret);
  if (ret > (signed int)XSTATS_MAX)
    printf("%s %d: more xstats available than space\n", __func__, __LINE__);
  for (i = 0; (signed int)i < ret; i++) {
...
static inline int init(struct test *t, int nb_queues, int nb_ports) {
...
static inline int create_ports(struct test *t, int num_ports) {
  int i;
...
static inline int create_lb_qids(struct test *t, int num_qids, uint32_t flags) {
  int i;
...
static inline int create_atomic_qids(struct test *t, int num_qids) {
  return create_lb_qids(t, num_qids, RTE_SCHED_TYPE_ATOMIC);
}

static inline int create_ordered_qids(struct test *t, int num_qids) {
  return create_lb_qids(t, num_qids, RTE_SCHED_TYPE_ORDERED);
}

static inline int create_unordered_qids(struct test *t, int num_qids) {
  return create_lb_qids(t, num_qids, RTE_SCHED_TYPE_PARALLEL);
}

static inline int create_directed_qids(struct test *t, int num_qids,
                                       const uint8_t ports[]) {
  int i;
  ...
  for (i = t->nb_qids; i < t->nb_qids + num_qids; i++) {
    ...
    if (rte_event_port_link(evdev, ports[i - t->nb_qids], &t->qid[i], NULL,
                            1) != 1) {
...
/* destruction */
static inline int cleanup(struct test *t __rte_unused) {
...
static inline int test_event_dev_stats_get(int dev_id,
  ...
  stats->rx_pkts =
      rte_event_dev_xstats_by_name_get(dev_id, "dev_rx", &total_ids[0]);
  stats->rx_dropped =
      rte_event_dev_xstats_by_name_get(dev_id, "dev_drop", &total_ids[1]);
  stats->tx_pkts =
      rte_event_dev_xstats_by_name_get(dev_id, "dev_tx", &total_ids[2]);
  for (i = 0; i < MAX_PORTS; i++) {
    char name[32];
    snprintf(name, sizeof(name), "port_%u_rx", i);
    stats->port_rx_pkts[i] =
        rte_event_dev_xstats_by_name_get(dev_id, name, &port_rx_pkts_ids[i]);
    snprintf(name, sizeof(name), "port_%u_drop", i);
    stats->port_rx_dropped[i] =
        rte_event_dev_xstats_by_name_get(dev_id, name, &port_rx_dropped_ids[i]);
    snprintf(name, sizeof(name), "port_%u_inflight", i);
    stats->port_inflight[i] =
        rte_event_dev_xstats_by_name_get(dev_id, name, &port_inflight_ids[i]);
    snprintf(name, sizeof(name), "port_%u_tx", i);
    stats->port_tx_pkts[i] =
        rte_event_dev_xstats_by_name_get(dev_id, name, &port_tx_pkts_ids[i]);
  }
  for (i = 0; i < MAX_QIDS; i++) {
    char name[32];
    snprintf(name, sizeof(name), "qid_%u_rx", i);
    stats->qid_rx_pkts[i] =
        rte_event_dev_xstats_by_name_get(dev_id, name, &qid_rx_pkts_ids[i]);
    snprintf(name, sizeof(name), "qid_%u_drop", i);
    stats->qid_rx_dropped[i] =
        rte_event_dev_xstats_by_name_get(dev_id, name, &qid_rx_dropped_ids[i]);
    snprintf(name, sizeof(name), "qid_%u_tx", i);
    stats->qid_tx_pkts[i] =
        rte_event_dev_xstats_by_name_get(dev_id, name, &qid_tx_pkts_ids[i]);
  }
 ...
 */
static int run_prio_packet_test(struct test *t) {
  int err;
  const uint32_t MAGIC_SEQN[] = {4711, 1234};
  const uint32_t PRIORITY[] = {RTE_EVENT_DEV_PRIORITY_NORMAL,
  ...
  for (i = 0; i < RTE_DIM(MAGIC_SEQN); i++) {
    ...
    *rte_event_pmd_selftest_seqn(arp) = MAGIC_SEQN[i];
    ...
    ev = (struct rte_event){.priority = PRIORITY[i],
                            .op = RTE_EVENT_OP_NEW,
                            .queue_id = t->qid[0],
                            .mbuf = arp};
...
static int test_single_directed_packet(struct test *t) {
  const int rx_enq = 0;
  const int wrk_enq = 2;
  int err;

  /* Create instance with 3 directed QIDs going to 3 ports */
  if (init(t, 3, 3) < 0 || create_ports(t, 3) < 0 ||
  ...
  *rte_event_pmd_selftest_seqn(arp) = MAGIC_SEQN;
  ...
  err = test_event_dev_stats_get(evdev, &stats);
  if (stats.port_rx_pkts[wrk_enq] != 0 && stats.port_rx_pkts[wrk_enq] != 1) {
...
static int test_directed_forward_credits(struct test *t) {
  uint32_t i;
  int32_t err;

  if (init(t, 1, 1) < 0 || create_ports(t, 1) < 0 ||
...
static int test_priority_directed(struct test *t) {
  if (init(t, 1, 1) < 0 || create_ports(t, 1) < 0 ||
      create_directed_qids(t, 1, t->port) < 0) {
...
static int test_priority_atomic(struct test *t) {
  if (init(t, 1, 1) < 0 || create_ports(t, 1) < 0 ||
...
static int test_priority_ordered(struct test *t) {
  if (init(t, 1, 1) < 0 || create_ports(t, 1) < 0 ||
...
static int test_priority_unordered(struct test *t) {
  if (init(t, 1, 1) < 0 || create_ports(t, 1) < 0 ||
...
static int burst_packets(struct test *t) {
  /************** CONFIG ****************/
  uint32_t i;
  int err;
  int ret;
  ...
  if (init(t, 2, 2) < 0 || create_ports(t, 2) < 0 ||
...
static int abuse_inflights(struct test *t) {
  const int rx_enq = 0;
  const int wrk_enq = 2;
  int err;

  /* Create instance with 4 ports */
  if (init(t, 1, 4) < 0 || create_ports(t, 4) < 0 ||
      create_atomic_qids(t, 1) < 0) {
    printf("%d: Error initializing device\n", __LINE__);
    return -1;
  }
  ...
  if (stats.rx_pkts != 0 || stats.tx_pkts != 0 ||
...
static int xstats_tests(struct test *t) {
  const int wrk_enq = 2;
  int err;
  ...
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_PORT, 0, ids,
                                 values, ret);
  if (ret != 21) {
    printf("%d: expected 21 stats, got return %d\n", __LINE__, ret);
    return -1;
  ...
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_QUEUE, 0, ids,
                                 values, ret);
  if (ret != 16) {
    printf("%d: expected 16 stats, got return %d\n", __LINE__, ret);
    return -1;
  ...
  /* Device names / values */
  int num_stats = rte_event_dev_xstats_names_get(
      evdev, RTE_EVENT_DEV_XSTATS_DEVICE, 0, xstats_names, ids, XSTATS_MAX);
  if (num_stats < 0)
    goto fail;
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_DEVICE, 0, ids,
                                 values, num_stats);
  ...
  for (i = 0; (signed int)i < ret; i++) {
    if (expected[i] != values[i]) {
      printf("%d Error xstat %d (id %d) %s : %" PRIu64 ", expect %" PRIu64 "\n",
             __LINE__, i, ids[i], xstats_names[i].name, values[i], expected[i]);
      goto fail;
    }
  }

  ret = rte_event_dev_xstats_reset(evdev, RTE_EVENT_DEV_XSTATS_DEVICE, 0, NULL,
                                   0);
  ...
  for (i = 0; (signed int)i < ret; i++) {
    if (expected_zero[i] != values[i]) {
      printf("%d Error, xstat %d (id %d) %s : %" PRIu64 ", expect %" PRIu64
             "\n",
             __LINE__, i, ids[i], xstats_names[i].name, values[i],
             expected_zero[i]);
      goto fail;
    }
  }
  ...
  /* port reset checks */
  num_stats = rte_event_dev_xstats_names_get(evdev, RTE_EVENT_DEV_XSTATS_PORT,
                                             0, xstats_names, ids, XSTATS_MAX);
  if (num_stats < 0)
    goto fail;
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_PORT, 0, ids,
  ...
  if (ret != RTE_DIM(port_expected)) {
    printf("%s %d: wrong number of port stats (%d), expected %zu\n", __func__,
           __LINE__, ret, RTE_DIM(port_expected));
  }

  for (i = 0; (signed int)i < ret; i++) {
    if (port_expected[i] != values[i]) {
      printf("%s : %d: Error stat %s is %" PRIu64 ", expected %" PRIu64 "\n",
             __func__, __LINE__, xstats_names[i].name, values[i],
             port_expected[i]);
      goto fail;
    }
  }
  ...
  ret =
      rte_event_dev_xstats_reset(evdev, RTE_EVENT_DEV_XSTATS_PORT, 0, NULL, 0);
  ...
  for (i = 0; (signed int)i < ret; i++) {
    if (port_expected_zero[i] != values[i]) {
      printf("%d, Error, xstat %d (id %d) %s : %" PRIu64 ", expect %" PRIu64
             "\n",
             __LINE__, i, ids[i], xstats_names[i].name, values[i],
             port_expected_zero[i]);
      goto fail;
    }
  }

  /* QUEUE STATS TESTS */
  num_stats = rte_event_dev_xstats_names_get(evdev, RTE_EVENT_DEV_XSTATS_QUEUE,
                                             0, xstats_names, ids, XSTATS_MAX);
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_QUEUE, 0, ids,
                                 values, num_stats);
  if (ret < 0) {
  ...
  ret =
      rte_event_dev_xstats_reset(evdev, RTE_EVENT_DEV_XSTATS_QUEUE, 0, NULL, 0);
...
static int xstats_id_abuse_tests(struct test *t) {
  int err;
  const uint32_t XSTATS_MAX = 1024;
  ...
  int num_stats = rte_event_dev_xstats_names_get(
      evdev, RTE_EVENT_DEV_XSTATS_PORT, UINT8_MAX - 1, xstats_names, ids,
      XSTATS_MAX);
  if (num_stats != 0) {
    printf("%d: expected %d stats, got return %d\n", __LINE__, 0, num_stats);
    goto fail;
  }
...
static int port_reconfig_credits(struct test *t) {
  if (init(t, 1, 1) < 0) {
    printf("%d: Error initializing device\n", __LINE__);
    return -1;
  }
  ...
  for (i = 0; i < NUM_ITERS; i++) {
    ...
    int deq = rte_event_dequeue_burst(evdev, t->port[0], ev, NPKTS, 0);
    if (deq != 1)
...
static int port_single_lb_reconfig(struct test *t) {
  if (init(t, 2, 2) < 0) {
    printf("%d: Error initializing device\n", __LINE__);
    goto fail;
  }
...
static int xstats_brute_force(struct test *t) {
  uint32_t i;
  const uint32_t XSTATS_MAX = 1024;
  uint32_t ids[XSTATS_MAX];
  uint64_t values[XSTATS_MAX];
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0; j < UINT8_MAX; j++) {
      rte_event_dev_xstats_names_get(evdev, mode, j, xstats_names, ids,
                                     XSTATS_MAX);

      rte_event_dev_xstats_get(evdev, mode, j, ids, values, XSTATS_MAX);
    }
  }
...
static int xstats_id_reset_tests(struct test *t) {
  const int wrk_enq = 2;
  int err;
...
#define NPKTS 7
  ...
  static const char *const dev_names[] = {"dev_rx",
                                          "dev_tx",
                                          "dev_drop",
                                          "dev_sched_calls",
                                          "dev_sched_no_iq_enq",
  ...
  for (i = 0; (int)i < ret; i++) {
    unsigned int id;
    uint64_t val = rte_event_dev_xstats_by_name_get(evdev, dev_names[i], &id);
    if (id != i) {
    ...
    if (val != dev_expected[i]) {
      printf("%d: %s value incorrect, expected %" PRIu64 " got %" PRIu64 "\n",
             __LINE__, dev_names[i], dev_expected[i], val);
      goto fail;
    }
    /* reset to zero */
    int reset_ret = rte_event_dev_xstats_reset(
        evdev, RTE_EVENT_DEV_XSTATS_DEVICE, 0, &id, 1);
    if (reset_ret) {
...
/* the port to test. */
#define PORT 2
  num_stats = rte_event_dev_xstats_names_get(
      evdev, RTE_EVENT_DEV_XSTATS_PORT, PORT, xstats_names, ids, XSTATS_MAX);
  if (num_stats != NUM_PORT_STATS) {
    printf("%d: expected %d stats, got return %d\n", __LINE__, NUM_PORT_STATS,
           num_stats);
    goto fail;
  }
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_PORT, PORT, ids,
                                 values, num_stats);
  ...
  for (i = 0; (int)i < ret; i++) {
    unsigned int id;
    uint64_t val = rte_event_dev_xstats_by_name_get(evdev, port_names[i], &id);
    if (id != i + PORT_OFF) {
      printf("%d: %s id incorrect, expected %d got %d\n", __LINE__,
             port_names[i], i + PORT_OFF, id);
      failed = 1;
    }
    if (val != port_expected[i]) {
      printf("%d: %s value incorrect, expected %" PRIu64 " got %d\n", __LINE__,
             port_names[i], port_expected[i], id);
      failed = 1;
    }
    /* reset to zero */
    int reset_ret = rte_event_dev_xstats_reset(evdev, RTE_EVENT_DEV_XSTATS_PORT,
                                               PORT, &id, 1);
    if (reset_ret) {
      printf("%d: failed to reset successfully\n", __LINE__);
      failed = 1;
    ...
    if (val != port_expected_zero[i]) {
      printf("%d: %s value incorrect, expected %" PRIu64 " got %" PRIu64 "\n",
             __LINE__, port_names[i], port_expected_zero[i], val);
      failed = 1;
    }
  };
  if (failed)
...
#define QUEUE_OFF 92
  ...
  num_stats = rte_event_dev_xstats_names_get(
      evdev, RTE_EVENT_DEV_XSTATS_QUEUE, queue, xstats_names, ids, XSTATS_MAX);
  if (num_stats != NUM_Q_STATS) {
    printf("%d: expected %d stats, got return %d\n", __LINE__, NUM_Q_STATS,
           num_stats);
    goto fail;
  }
  ret = rte_event_dev_xstats_get(evdev, RTE_EVENT_DEV_XSTATS_QUEUE, queue, ids,
                                 values, num_stats);
  if (ret != NUM_Q_STATS) {
    printf("%d: expected 21 stats, got return %d\n", __LINE__, ret);
    goto fail;
  ...
  for (i = 0; (int)i < ret; i++) {
    unsigned int id;
    uint64_t val = rte_event_dev_xstats_by_name_get(evdev, queue_names[i], &id);
    if (id != i + QUEUE_OFF) {
      printf("%d: %s id incorrect, expected %d got %d\n", __LINE__,
             queue_names[i], i + QUEUE_OFF, id);
      failed = 1;
    }
    if (val != queue_expected[i]) {
      printf("%d: %d: %s value , expected %" PRIu64 " got %" PRIu64 "\n", i,
             __LINE__, queue_names[i], queue_expected[i], val);
      failed = 1;
    }
    /* reset to zero */
    int reset_ret = rte_event_dev_xstats_reset(
        evdev, RTE_EVENT_DEV_XSTATS_QUEUE, queue, &id, 1);
    if (reset_ret) {
      printf("%d: failed to reset successfully\n", __LINE__);
      failed = 1;
    }
    /* check value again */
    val = rte_event_dev_xstats_by_name_get(evdev, queue_names[i], 0);
    if (val != queue_expected_zero[i]) {
      printf("%d: %s value incorrect, expected %" PRIu64 " got %" PRIu64 "\n",
             __LINE__, queue_names[i], queue_expected_zero[i], val);
      failed = 1;
    }
  };
...
static int ordered_reconfigure(struct test *t) {
  if (init(t, 1, 1) < 0 || create_ports(t, 1) < 0) {
    printf("%d: Error initializing device\n", __LINE__);
    return -1;
  }
...
static int qid_priorities(struct test *t) {
  ...
  if (init(t, 3, 1) < 0 || create_ports(t, 1) < 0) {
    printf("%d: Error initializing device\n", __LINE__);
    return -1;
  }
...
static int unlink_in_progress(struct test *t) {
  ...
  unlinks_in_progress = rte_event_port_unlinks_in_progress(evdev, t->port[0]);
...
static int load_balancing(struct test *t) {
  const int rx_enq = 0;
  int err;
  uint32_t i;
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (rte_event_port_link(evdev, t->port[i + 1], &t->qid[0], NULL, 1) != 1) {
...
static int load_balancing_history(struct test *t) {
  ...
  for (i = 0; i < RTE_DIM(flows1); i++) {
    struct rte_mbuf *arp = rte_gen_arp(0, t->mbuf_pool);
    struct rte_event ev = {.flow_id = flows1[i],
                           .op = RTE_EVENT_OP_NEW,
                           .queue_id = t->qid[0],
  ...
  for (i = 0; i < RTE_DIM(flows2); i++) {
    struct rte_mbuf *arp = rte_gen_arp(0, t->mbuf_pool);
    struct rte_event ev = {.flow_id = flows2[i],
                           .op = RTE_EVENT_OP_NEW,
                           .queue_id = t->qid[0],
                           .event_type = RTE_EVENT_TYPE_CPU,
                           .priority = RTE_EVENT_DEV_PRIORITY_NORMAL,
...
static int invalid_qid(struct test *t) {
  struct test_event_dev_stats stats;
  const int rx_enq = 0;
  int err;
  uint32_t i;
...
static int single_packet(struct test *t) {
  ...
  *rte_event_pmd_selftest_seqn(arp) = MAGIC_SEQN;
  ...
  if (stats.rx_pkts != 1 || stats.tx_pkts != 1 ||
...
static int inflight_counts(struct test *t) {
  struct rte_event ev;
  struct test_event_dev_stats stats;
  const int rx_enq = 0;
  const int p1 = 1;
  ...
  /* Create instance with 4 ports */
  if (init(t, 2, 3) < 0 || create_ports(t, 3) < 0 ||
      create_atomic_qids(t, 2) < 0) {
    printf("%d: Error initializing device\n", __LINE__);
    return -1;
  }
...
#define QID2_NUM 3
  ...
  uint32_t deq_pkts =
      rte_event_dequeue_burst(evdev, t->port[p1], events, RTE_DIM(events), 0);
  ...
  /* port2 */
  deq_pkts =
      rte_event_dequeue_burst(evdev, t->port[p2], events, RTE_DIM(events), 0);
...
static int parallel_basic(struct test *t, int check_order) {
  ...
  /* Create instance with 4 ports */
  if (init(t, 2, tx_port + 1) < 0 || create_ports(t, tx_port + 1) < 0 ||
      (check_order ? create_ordered_qids(t, 1) : create_unordered_qids(t, 1)) <
          0 ||
  ...
  for (i = w1_port; i <= w3_port; i++) {
    deq_pkts = rte_event_dequeue_burst(evdev, t->port[i], &deq_ev[i], 1, 0);
    if (deq_pkts != 1) {
      printf("%d: Failed to deq\n", __LINE__);
      rte_event_dev_dump(evdev, stdout);
      return -1;
  ...
  if (check_order) {
    for (j = 0; j < deq_pkts; j++) {
      if (*rte_event_pmd_selftest_seqn(deq_ev[j].mbuf) != MAGIC_SEQN + j) {
        printf("%d: Incorrect sequence number(%d) from port %d\n", __LINE__,
               *rte_event_pmd_selftest_seqn(mbufs_out[j]), tx_port);
        return -1;
      }
    }
  }
...
static int ordered_basic(struct test *t) { return parallel_basic(t, 1); }

static int unordered_basic(struct test *t) { return parallel_basic(t, 0); }
...
holb(struct test *t) /* test to check we avoid basic head-of-line blocking */
{
  ...
  if (init(t, 1, 2) < 0 || create_ports(t, 2) < 0 ||
      create_atomic_qids(t, 1) < 0) {
    printf("%d: Error initializing device\n", __LINE__);
    return -1;
  }
  ...
  snprintf(rx_port_used_stat, sizeof(rx_port_used_stat), "port_%u_cq_ring_used",
           rx_port);
  snprintf(rx_port_free_stat, sizeof(rx_port_free_stat), "port_%u_cq_ring_free",
           rx_port);
  ...
  do {
    ...
    rte_service_run_iter_on_app_lcore(t->service_id, 1);
  } while (rte_event_dev_xstats_by_name_get(evdev, rx_port_free_stat, NULL) !=
           0);
...
static void flush(uint8_t dev_id __rte_unused, struct rte_event event,
                  void *arg) {
...
{
  const struct rte_event new_ev = {
      .op = RTE_EVENT_OP_NEW, .u64 = 0xCA11BACC, .queue_id = 0};
  struct rte_event ev = new_ev;
  uint8_t count = 0;
  int i;
...
static int worker_loopback_worker_fn(void *arg) {
  struct test *t = arg;
...
static int worker_loopback_producer_fn(void *arg) {
  struct test *t = arg;
...
static int worker_loopback(struct test *t, uint8_t disable_implicit_release) {
  ...
  counter_dynfield_offset = rte_mbuf_dynfield_register(&counter_dynfield_desc);
  if (counter_dynfield_offset < 0) {
    printf("Error registering mbuf field\n");
    return -rte_errno;
  }

  if (init(t, 8, 2) < 0 || create_atomic_qids(t, 8) < 0) {
    printf("%d: Error initializing device\n", __LINE__);
    return -1;
  }
  ...
  conf.event_port_cfg =
      disable_implicit_release ? RTE_EVENT_PORT_CFG_DISABLE_IMPL_REL : 0;
...
int test_sw_eventdev(void) {
  ...
  if (!eventdev_func_mempool) {
    eventdev_func_mempool = rte_pktmbuf_pool_create(
        "EVENTDEV_SW_SA_MBUF_POOL", (1 << 12), /* 4k buffers */
        32 /*MBUF_CACHE_SIZE*/, 0, 512,        /* use very small mbufs */
        rte_socket_id());
    if (!eventdev_func_mempool) {
