...
static inline void sw_event_release(struct sw_port *p, uint8_t index) {
  /*
...
uint16_t sw_event_enqueue_burst(void *port, const struct rte_event ev[],
  ...
  for (i = 0; i < num; i++) {
    ...
    p->inflight_credits += (op == RTE_EVENT_OP_RELEASE) * outstanding;
  ...
  uint32_t enq = enqueue_burst_with_ops(p->rx_worker_ring, ev, i, new_ops);
  if (p->outstanding_releases == 0 && p->last_dequeue_burst_sz != 0) {
    uint64_t burst_ticks = rte_get_timer_cycles() - p->last_dequeue_ticks;
