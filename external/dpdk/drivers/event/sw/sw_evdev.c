...
static void sw_info_get(struct rte_eventdev *dev,
                        struct rte_event_dev_info *info);

static int sw_port_link(struct rte_eventdev *dev, void *port,
...
static int sw_port_unlink(struct rte_eventdev *dev, void *port,
...
static int sw_port_unlinks_in_progress(struct rte_eventdev *dev, void *port) {
  RTE_SET_USED(dev);
  struct sw_port *p = port;
  return p->unlinks_in_progress;
}

static int sw_port_setup(struct rte_eventdev *dev, uint8_t port_id,
  ...
  *p = (struct sw_port){0}; /* zero entire structure */
   ...
   */
  snprintf(buf, sizeof(buf), "sw%d_p%u_%s", dev->data->dev_id, port_id,
           "rx_worker_ring");
  ...
  p->rx_worker_ring =
      rte_event_ring_create(buf, MAX_SW_PROD_Q_DEPTH, dev->data->socket_id,
  ...
  snprintf(buf, sizeof(buf), "sw%d_p%u, %s", dev->data->dev_id, port_id,
           "cq_worker_ring");
...
static void sw_port_release(void *port) {
...
static int32_t qid_init(struct sw_evdev *sw, unsigned int idx, int type,
  ...
  if (qid->type == RTE_SCHED_TYPE_ORDERED) {
     ...
     * be a power-of-2.
     */
    window_size = rte_align32pow2(queue_conf->nb_atomic_order_sequences);
    ...
    qid->reorder_buffer = rte_zmalloc_socket(
        buf, window_size * sizeof(qid->reorder_buffer[0]), 0, socket_id);
    if (!qid->reorder_buffer) {
      SW_LOG_DBG("reorder_buffer malloc failed\n");
      goto cleanup;
    }

    memset(&qid->reorder_buffer[0], 0,
           window_size * sizeof(qid->reorder_buffer[0]));

    qid->reorder_buffer_freelist = rob_ring_create(window_size, socket_id);
...
static void sw_queue_release(struct rte_eventdev *dev, uint8_t id) {
  struct sw_evdev *sw = sw_pmd_priv(dev);
...
static int sw_queue_setup(struct rte_eventdev *dev, uint8_t queue_id,
  ...
  } else if (RTE_EVENT_QUEUE_CFG_ALL_TYPES & conf->event_queue_cfg) {
...
static void sw_init_qid_iqs(struct sw_evdev *sw) {
  int i, j;
...
static int sw_qids_empty(struct sw_evdev *sw) {
  unsigned int i, j;
...
static int sw_ports_empty(struct sw_evdev *sw) {
  unsigned int i;
...
static void sw_drain_ports(struct rte_eventdev *dev) {
  struct sw_evdev *sw = sw_pmd_priv(dev);
...
static void sw_drain_queue(struct rte_eventdev *dev, struct sw_iq *iq) {
  struct sw_evdev *sw = sw_pmd_priv(dev);
  eventdev_stop_flush_t flush;
  uint8_t dev_id;
  void *arg;
...
static void sw_drain_queues(struct rte_eventdev *dev) {
  struct sw_evdev *sw = sw_pmd_priv(dev);
  unsigned int i, j;
...
static void sw_clean_qid_iqs(struct rte_eventdev *dev) {
  struct sw_evdev *sw = sw_pmd_priv(dev);
  int i, j;
...
static void sw_queue_def_conf(struct rte_eventdev *dev, uint8_t queue_id,
...
static void sw_port_def_conf(struct rte_eventdev *dev, uint8_t port_id,
...
static int sw_dev_configure(const struct rte_eventdev *dev) {
  struct sw_evdev *sw = sw_pmd_priv(dev);
  ...
  sw->chunks = rte_malloc_socket(
      NULL, sizeof(struct sw_queue_chunk) * num_chunks, 0, sw->data->socket_id);
  if (!sw->chunks)
    return -ENOMEM;
...
static int sw_eth_rx_adapter_caps_get(const struct rte_eventdev *dev,
...
static int sw_crypto_adapter_caps_get(const struct rte_eventdev *dev,
...
static void sw_info_get(struct rte_eventdev *dev,
  ...
  static const struct rte_event_dev_info evdev_sw_info = {
      ...
      .event_dev_cap =
          (RTE_EVENT_DEV_CAP_QUEUE_QOS | RTE_EVENT_DEV_CAP_BURST_MODE |
           RTE_EVENT_DEV_CAP_EVENT_QOS |
           RTE_EVENT_DEV_CAP_IMPLICIT_RELEASE_DISABLE |
           RTE_EVENT_DEV_CAP_RUNTIME_PORT_LINK |
           RTE_EVENT_DEV_CAP_MULTIPLE_QUEUE_PORT |
           RTE_EVENT_DEV_CAP_NONSEQ_MODE | RTE_EVENT_DEV_CAP_CARRY_FLOW_ID |
...
static void sw_dump(struct rte_eventdev *dev, FILE *f) {
  const struct sw_evdev *sw = sw_pmd_priv(dev);

  static const char *const q_type_strings[] = {"Ordered", "Atomic", "Parallel",
                                               "Directed"};
  uint32_t i;
...
#define COL_RESET "\x1b[0m"
  ...
  for (i = 0; i < sw->port_count; i++) {
    ...
    fprintf(f, "  Port %d %s\n", i, p->is_directed ? " (SingleCons)" : "");
    fprintf(f,
            "\trx   %" PRIu64 "\tdrop %" PRIu64 "\ttx   %" PRIu64
            "\t%sinflight %d%s\n",
            sw->ports[i].stats.rx_pkts, sw->ports[i].stats.rx_dropped,
            sw->ports[i].stats.tx_pkts,
            (p->inflights == p->inflight_max) ? COL_RED : COL_RESET,
            sw->ports[i].inflights, COL_RESET);

    fprintf(f,
            "\tMax New: %u"
            "\tAvg cycles PP: %" PRIu64 "\tCredits: %u\n",
            sw->ports[i].inflight_max, sw->ports[i].avg_pkt_ticks,
    ...
    for (j = 0; j <= max; j++) {
      if (p->poll_buckets[j] != 0) {
        float poll_pc = p->poll_buckets[j] * 100.0 / p->total_polls;
        fprintf(f, "%u-%u:%.02f%% ", ((j << SW_DEQ_STAT_BUCKET_SHIFT) + 1),
                ((j + 1) << SW_DEQ_STAT_BUCKET_SHIFT), poll_pc);
      }
    }
    fprintf(f, "\n");
    ...
    if (p->rx_worker_ring) {
      ...
      uint64_t space = rte_event_ring_free_count(p->rx_worker_ring);
      const char *col = (space == 0) ? COL_RED : COL_RESET;
      fprintf(f,
              "\t%srx ring used: %4" PRIu64 "\tfree: %4" PRIu64 COL_RESET "\n",
              col, used, space);
    } else
    ...
    if (p->cq_worker_ring) {
      ...
      const char *col = (space == 0) ? COL_RED : COL_RESET;
      fprintf(f,
              "\t%scq ring used: %4" PRIu64 "\tfree: %4" PRIu64 COL_RESET "\n",
              col, used, space);
    } else
  ...
  for (i = 0; i < sw->qid_count; i++) {
    ...
    fprintf(f, "\trx   %" PRIu64 "\tdrop %" PRIu64 "\ttx   %" PRIu64 "\n",
            qid->stats.rx_pkts, qid->stats.rx_dropped, qid->stats.tx_pkts);
    if (qid->type == RTE_SCHED_TYPE_ORDERED) {
      struct rob_ring *rob_buf_free = qid->reorder_buffer_freelist;
      if (rob_buf_free)
        fprintf(f, "\tReorder entries in use: %u\n",
                rob_ring_free_count(rob_buf_free));
      else
        fprintf(f, "\tReorder buffer not initialized\n");
    }
    ...
    for (port = 0; port < sw->port_count; port++) {
      fprintf(f, "\t  Port %d: Pkts: %" PRIu64, port, qid->to_port[port]);
    ...
    for (iq = 0; iq < SW_IQS_MAX; iq++) {
      ...
      if (used > 0) {
        fprintf(f, "\t%siq %d: Used %d" COL_RESET "\n", col, iq, used);
        iq_printed = 1;
      }
    }
    if (iq_printed == 0)
...
static int sw_start(struct rte_eventdev *dev) {
  unsigned int i, j;
  struct sw_evdev *sw = sw_pmd_priv(dev);
  ...
  for (i = 0; i < sw->qid_count; i++)
    if (!sw->qids[i].initialized || sw->qids[i].cq_num_mapped_cqs == 0) {
...
static void sw_stop(struct rte_eventdev *dev) {
  struct sw_evdev *sw = sw_pmd_priv(dev);
  int32_t runstate;
...
static int sw_close(struct rte_eventdev *dev) {
  struct sw_evdev *sw = sw_pmd_priv(dev);
  uint32_t i;
...
static int assign_numa_node(const char *key __rte_unused, const char *value,
                            void *opaque) {
...
static int set_sched_quanta(const char *key __rte_unused, const char *value,
                            void *opaque) {
  int *quanta = opaque;
  *quanta = atoi(value);
...
static int set_credit_quanta(const char *key __rte_unused, const char *value,
                             void *opaque) {
  int *credit = opaque;
  *credit = atoi(value);
...
static int set_deq_burst_sz(const char *key __rte_unused, const char *value,
                            void *opaque) {
...
static int set_min_burst_sz(const char *key __rte_unused, const char *value,
                            void *opaque) {
...
static int set_refill_once(const char *key __rte_unused, const char *value,
                           void *opaque) {
...
static int sw_probe(struct rte_vdev_device *vdev) {
  ...
  if (params != NULL && params[0] != '\0') {
    ...
    if (!kvlist) {
      SW_LOG_INFO("Ignoring unsupported parameters when creating device '%s'\n",
                  name);
    } else {
      int ret = rte_kvargs_process(kvlist, NUMA_NODE_ARG, assign_numa_node,
                                   &socket_id);
      if (ret != 0) {
        SW_LOG_ERR("%s: Error parsing numa node parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, SCHED_QUANTA_ARG, set_sched_quanta,
                               &sched_quanta);
      if (ret != 0) {
        SW_LOG_ERR("%s: Error parsing sched quanta parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, CREDIT_QUANTA_ARG, set_credit_quanta,
                               &credit_quanta);
      if (ret != 0) {
        SW_LOG_ERR("%s: Error parsing credit quanta parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, MIN_BURST_SIZE_ARG, set_min_burst_sz,
                               &min_burst_size);
      if (ret != 0) {
        SW_LOG_ERR("%s: Error parsing minimum burst size parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, DEQ_BURST_SIZE_ARG, set_deq_burst_sz,
                               &deq_burst_size);
      if (ret != 0) {
        SW_LOG_ERR("%s: Error parsing dequeue burst size parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, REFIL_ONCE_ARG, set_refill_once,
                               &refill_once);
      if (ret != 0) {
        SW_LOG_ERR("%s: Error parsing refill once per call switch", name);
        rte_kvargs_free(kvlist);
        return ret;
      }
  ...
  SW_LOG_INFO("Creating eventdev sw device %s, numa_node=%d, "
              "sched_quanta=%d, credit_quanta=%d "
              "min_burst=%d, deq_burst=%d, refill_once=%d\n",
              name, socket_id, sched_quanta, credit_quanta, min_burst_size,
              deq_burst_size, refill_once);

  dev = rte_event_pmd_vdev_init(name, sizeof(struct sw_evdev), socket_id);
  if (dev == NULL) {
  ...
  snprintf(sw->service_name, sizeof(sw->service_name), "%s_service", name);
...
static int sw_remove(struct rte_vdev_device *vdev) {
  const char *name;
...
static struct rte_vdev_driver evdev_sw_pmd_drv = {.probe = sw_probe,
                                                  .remove = sw_remove};

RTE_PMD_REGISTER_VDEV(EVENTDEV_NAME_SW_PMD, evdev_sw_pmd_drv);
RTE_PMD_REGISTER_PARAM_STRING(event_sw,
                              NUMA_NODE_ARG "=<int> " SCHED_QUANTA_ARG
                                            "=<int>" CREDIT_QUANTA_ARG
                                            "=<int>" MIN_BURST_SIZE_ARG
                                            "=<int>" DEQ_BURST_SIZE_ARG
                                            "=<int>" REFIL_ONCE_ARG "=<int>");
