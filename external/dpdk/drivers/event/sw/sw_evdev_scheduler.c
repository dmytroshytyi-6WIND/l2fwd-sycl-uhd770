 ...
 */
#define PKT_MASK_TO_IQ(pkts) (__builtin_ctz(pkts | (1 << SW_IQS_MAX)))
...
static inline uint32_t sw_schedule_atomic_to_cq(struct sw_evdev *sw,
                                                struct sw_qid *const qid,
                                                uint32_t iq_num,
                                                unsigned int count) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (sw->cq_ring_space[cq] == 0) {
      ...
      rte_event_ring_enqueue_burst(worker, p->cq_buf, p->cq_buf_count,
                                   &sw->cq_ring_space[cq]);
      p->cq_buf_count = 0;
    }
  }
...
static inline uint32_t sw_schedule_parallel_to_cq(struct sw_evdev *sw,
                                                  struct sw_qid *const qid,
                                                  uint32_t iq_num,
                                                  unsigned int count,
                                                  int keep_order) {
  ...
  if (keep_order)
    ...
    count = RTE_MIN(count, rob_ring_count(qid->reorder_buffer_freelist));
  ...
  for (i = 0; i < count; i++) {
    ...
    do {
      ...
      cq = qid->cq_map[cq_idx++];
             ...
             rte_event_ring_free_count(sw->ports[cq].cq_worker_ring) == 0);

    struct sw_port *p = &sw->ports[cq];
    if (sw->cq_ring_space[cq] == 0 || p->inflights == SW_PORT_HIST_LIST)
      break;
...
static uint32_t sw_schedule_dir_to_cq(struct sw_evdev *sw,
                                      struct sw_qid *const qid, uint32_t iq_num,
  ...
  uint32_t ret =
      iq_dequeue_burst(sw, iq, &port->cq_buf[port->cq_buf_count], count_free);
...
static uint32_t sw_schedule_qid_to_cq(struct sw_evdev *sw) {
  uint32_t pkts = 0;
  uint32_t qid_idx;
  ...
  for (qid_idx = 0; qid_idx < sw->qid_count; qid_idx++) {
    ...
    if (count >= sw->sched_min_burst) {
      if (type == SW_SCHED_TYPE_DIRECT)
        pkts_done += sw_schedule_dir_to_cq(sw, qid, iq_num, count);
      else if (type == RTE_SCHED_TYPE_ATOMIC)
        pkts_done += sw_schedule_atomic_to_cq(sw, qid, iq_num, count);
      else
        pkts_done += sw_schedule_parallel_to_cq(sw, qid, iq_num, count,
 ...
 */
static uint16_t sw_schedule_reorder(struct sw_evdev *sw, int qid_start,
                                    int qid_end) {
  ...
  for (; qid_start < qid_end; qid_start++) {
    ...
    num_entries_in_use = rob_ring_free_count(qid->reorder_buffer_freelist);
    ...
    for (i = 0; i < num_entries_in_use; i++) {
      ...
      if (!entry->ready) {
        ...
        rob_ring_enqueue(qid->reorder_buffer_freelist, entry);
...
static __rte_always_inline void sw_refill_pp_buf(struct sw_evdev *sw,
                                                 struct sw_port *port) {
  RTE_SET_USED(sw);
  struct rte_event_ring *worker = port->rx_worker_ring;
  port->pp_buf_start = 0;
  port->pp_buf_count = rte_event_ring_dequeue_burst(
      worker, port->pp_buf, sw->sched_deq_burst_size, NULL);
}

static __rte_always_inline uint32_t __pull_port_lb(struct sw_evdev *sw,
                                                   uint32_t port_id,
                                                   int allow_reorder) {
  ...
  while (port->pp_buf_count) {
    ...
    if ((flags & QE_FLAG_COMPLETE) && port->inflights > 0) {
      const uint32_t hist_tail = port->hist_tail & (SW_PORT_HIST_LIST - 1);
    ...
    if (flags & QE_FLAG_VALID) {
      ...
      if (allow_reorder && needs_reorder) {
        struct reorder_buffer_entry *rob_entry = hist_entry->rob_entry;
...
static uint32_t sw_schedule_pull_port_lb(struct sw_evdev *sw,
                                         uint32_t port_id) {
  return __pull_port_lb(sw, port_id, 1);
}

static uint32_t sw_schedule_pull_port_no_reorder(struct sw_evdev *sw,
                                                 uint32_t port_id) {
  return __pull_port_lb(sw, port_id, 0);
}

static uint32_t sw_schedule_pull_port_dir(struct sw_evdev *sw,
                                          uint32_t port_id) {
  uint32_t pkts_iter = 0;
  struct sw_port *port = &sw->ports[port_id];
...
void sw_event_schedule(struct rte_eventdev *dev) {
  ...
  do {
    ...
    do {
      ...
      in_pkts += sw_schedule_reorder(sw, 0, sw->qid_count);
      in_pkts_this_iteration += in_pkts;
    } while (in_pkts > 4 && (int)in_pkts_this_iteration < sched_quanta);
  ...
  for (i = 0; i < sw->port_count; i++) {
    ...
    if (port->cq_buf_count >= sw->sched_min_burst) {
      rte_event_ring_enqueue_burst(worker, port->cq_buf, port->cq_buf_count,
                                   &sw->cq_ring_space[i]);
      port->cq_buf_count = 0;
      no_enq = 0;
      ...
      sw->cq_ring_space[i] =
          rte_event_ring_free_count(worker) - port->cq_buf_count;
    }
  }
