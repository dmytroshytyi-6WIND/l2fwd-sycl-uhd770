...
static inline struct rob_ring *rob_ring_create(unsigned int size,
                                               unsigned int socket_id) {
  struct rob_ring *retval;
  const uint32_t ring_size = rte_align32pow2(size + 1);
  size_t memsize = sizeof(*retval) + (ring_size * sizeof(retval->ring[0]));
...
static inline void rob_ring_free(struct rob_ring *r) { rte_free(r); }
...
static __rte_always_inline unsigned int rob_ring_enqueue(struct rob_ring *r,
                                                         void *re) {
...
static __rte_always_inline unsigned int rob_ring_dequeue(struct rob_ring *r,
                                                         void **re) {
  const uint32_t mask = r->mask;
