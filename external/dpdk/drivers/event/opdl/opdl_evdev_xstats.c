...
static const char *const port_xstat_str[] = {
    ...
    "claim_pkts_requested", "claim_pkts_granted", "claim_non_empty",
    "claim_empty",          "total_cycles",
};
  ...
  for (i = 0; i < device->max_port_nb; i++) {
    ...
    for (j = 0; j < max_num_port_xstat; j++) {
      ...
      snprintf(device->port_xstat[index].stat.name,
               sizeof(device->port_xstat[index].stat.name), "port_%02u_%s", i,
               port_xstat_str[j]);
...
int opdl_xstats_uninit(struct rte_eventdev *dev) {
  ...
  memset(device->port_xstat, 0, sizeof(device->port_xstat));
...
int opdl_xstats_get_names(const struct rte_eventdev *dev,
  ...
  if (mode == RTE_EVENT_DEV_XSTATS_DEVICE || mode == RTE_EVENT_DEV_XSTATS_QUEUE)
    return -EINVAL;
  ...
  for (j = 0; j < max_num_port_xstat; j++) {
    ...
    strcpy(xstats_names[j].name, device->port_xstat[j + port_idx].stat.name);
...
uint64_t opdl_xstats_get_by_name(const struct rte_eventdev *dev,
  ...
  for (i = 0; i < max_index; i++) {
    ...
    if (strncmp(name, device->port_xstat[i].stat.name,
