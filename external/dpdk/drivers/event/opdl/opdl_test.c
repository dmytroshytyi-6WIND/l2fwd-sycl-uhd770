...
static __rte_always_inline int init(struct test *t, int nb_queues,
                                    int nb_ports) {
...
static __rte_always_inline int create_ports(struct test *t, int num_ports) {
  int i;
...
static __rte_always_inline int create_queues_type(struct test *t, int num_qids,
                                                  enum queue_type flags) {
  int i;
  uint8_t type;
  ...
  const struct rte_event_queue_conf conf = {
      .event_queue_cfg =
          (flags == OPDL_Q_TYPE_SINGLE_LINK ? RTE_EVENT_QUEUE_CFG_SINGLE_LINK
                                            : 0),
      .schedule_type = type,
...
/* destruction */
static __rte_always_inline int cleanup(struct test *t __rte_unused) {
...
static int ordered_basic(struct test *t) {
  ...
  err = rte_event_port_link(evdev, t->port[tx_port], &t->qid[1], NULL, 1);
  if (err != 1) {
...
static int atomic_basic(struct test *t) {
  const uint8_t rx_port = 0;
  const uint8_t w1_port = 1;
  const uint8_t w3_port = 3;
  const uint8_t tx_port = 4;
  ...
  for (i = w1_port; i <= w3_port; i++) {
    ...
    deq_pkts = rte_event_dequeue_burst(evdev, t->port[i], deq_ev, 3, 0);
    ...
    if (t->port[i] != 2) {
      ...
      err = rte_event_enqueue_burst(evdev, t->port[i], deq_ev, 3);

      if (err != 3) {
        PMD_DRV_LOG(ERR,
                    "port %d: Failed to enqueue pkt %u, "
                    "retval = %u\n",
                    t->port[i], 3, err);
        return -1;
      }
...
static __rte_always_inline int check_qid_stats(uint32_t id[], int index) {

  if (index == 0) {
    if (id[0] != 3 || id[1] != 3 || id[2] != 3)
      return -1;
  } else if (index == 1) {
    if (id[0] != 5 || id[1] != 5 || id[2] != 2)
      return -1;
  } else if (index == 2) {
    if (id[0] != 3 || id[1] != 1 || id[2] != 1)
      return -1;
  }
...
static int check_statistics(void) {
  ...
  for (i = 0; i < num_ports; i++) {
    ...
    num_stats = rte_event_dev_xstats_names_get(0, RTE_EVENT_DEV_XSTATS_PORT, i,
                                               NULL, NULL, 0);
    if (num_stats > 0) {
      ...
      num_stats_returned = rte_event_dev_xstats_names_get(
          0, RTE_EVENT_DEV_XSTATS_PORT, i, names, id, num_stats);

      if (num_stats == num_stats_returned) {
        num_stats_returned = rte_event_dev_xstats_get(
            0, RTE_EVENT_DEV_XSTATS_PORT, i, id, values, num_stats);
...
static int single_link_w_stats(struct test *t) {
  const uint8_t rx_port = 0;
  const uint8_t w1_port = 1;
  ...
  if (init(t, 2, tx_port + 1) < 0 || create_ports(t, 3) < 0 || /* 0,1,2 */
  ...
  err = rte_event_port_link(evdev, t->port[1], &t->qid[0], NULL, 1);
  if (err != 1) {
  ...
  err = rte_event_port_link(evdev, t->port[2], &t->qid[1], NULL, 1);
  if (err != 1) {
    PMD_DRV_LOG(ERR, "%d: error linking port:[%u] to queue:[%u]\n", __LINE__,
                t->port[2], t->qid[1]);
    cleanup(t);
  ...
  deq_pkts = rte_event_dequeue_burst(evdev, t->port[w1_port], deq_ev, 3, 0);
  ...
  deq_pkts =
      rte_event_enqueue_burst(evdev, t->port[w1_port], deq_ev, NEW_NUM_PACKETS);
...
static int single_link(struct test *t) {
  const uint8_t tx_port = 2;
  int err;
  struct rte_mbuf *mbufs[3];
  RTE_SET_USED(mbufs);
  ...
  err = rte_event_port_link(evdev, t->port[2], &t->qid[0], NULL, 1);
  if (err != 1) {
    PMD_DRV_LOG(ERR, "%d: error mapping lb qid\n", __LINE__);
    cleanup(t);
    return -1;
  ...
  if (rte_event_dev_start(evdev) == 0) {
    PMD_DRV_LOG(ERR,
                "%d: start DIDN'T FAIL with more than 1 "
                "SINGLE_LINK PORT\n",
                __LINE__);
    cleanup(t);
    return -1;
...
populate_event_burst(struct rte_event ev[], uint8_t qid, uint16_t num_events) {
  uint16_t i;
...
static int qid_basic(struct test *t) {
  int err = 0;
  ...
  for (i = 0; i < NUM_QUEUES; i++) {
    ...
    nb_linked = rte_event_port_link(evdev, i + 1, /* port = q_id + 1*/
                                    &q_id, NULL, 1);
  ...
  if (!err) {
    uint8_t t_qid = 0;
    if (rte_event_port_link(evdev, 1, &t_qid, NULL, 1) > 0) {
      PMD_DRV_LOG(ERR,
                  "%s:%d: Second call to port link on same port DID NOT fail\n",
                  __FILE__, __LINE__);
      err = -1;
    }
    ...
    if (!err) {
      test_num_events = rte_event_dequeue_burst(evdev, p_id, ev, BATCH_SIZE, 0);
      if (test_num_events != 0) {
        PMD_DRV_LOG(
            ERR,
            "%s:%d: Error dequeuing 0 packets from port %u on stopped device\n",
            __FILE__, __LINE__, p_id);
        err = -1;
      }
    }

    if (!err) {
      test_num_events = rte_event_enqueue_burst(evdev, p_id, ev, BATCH_SIZE);
      if (test_num_events != 0) {
        PMD_DRV_LOG(
            ERR,
            "%s:%d: Error enqueuing 0 packets to port %u on stopped device\n",
            __FILE__, __LINE__, p_id);
        err = -1;
      }
    }
  ...
  if (!err) {
    ...
    if (rte_event_port_link(evdev, 1, &t_qid, NULL, 1) > 0) {
      PMD_DRV_LOG(ERR,
                  "%s:%d: Call to port link on started device DID NOT fail\n",
                  __FILE__, __LINE__);
      err = -1;
    }
  }
  ...
  if (!err) {
    ...
    populate_event_burst(ev, q_id, BATCH_SIZE);

    num_events = rte_event_enqueue_burst(evdev, p_id, ev, BATCH_SIZE);
  ...
  if (!err) {
    while (++p_id < NUM_QUEUES) {
      ...
      num_events = rte_event_dequeue_burst(evdev, p_id, ev, BATCH_SIZE, 0);
      ...
      if (ev[0].queue_id != q_id) {
        PMD_DRV_LOG(ERR,
                    "%s:%d: Error event portid[%u] q_id:[%u] does not match "
                    "expected:[%u]\n",
                    __FILE__, __LINE__, p_id, ev[0].queue_id, q_id);
        err = -1;
        break;
      }
      ...
      if (num_events != BATCH_SIZE) {
        PMD_DRV_LOG(ERR,
                    "%s:%d: Error enqueuing packets from port:%u to queue:%u\n",
                    __FILE__, __LINE__, p_id, q_id);
        err = -1;
        break;
      }
    }
