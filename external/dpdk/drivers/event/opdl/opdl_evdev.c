...
static void opdl_info_get(struct rte_eventdev *dev,
...
static int opdl_port_link(struct rte_eventdev *dev, void *port,
  ...
  if (p->external_qid != OPDL_INVALID_QID) {
    PMD_DRV_LOG(ERR,
                ...
                dev->data->dev_id, p->id, p->external_qid, queues[0]);
    rte_errno = EINVAL;
    return 0;
  }
...
static int opdl_port_unlink(struct rte_eventdev *dev, void *port,
...
static int opdl_port_setup(struct rte_eventdev *dev, uint8_t port_id,
...
static void opdl_port_release(void *port) {
  struct opdl_port *p = (void *)port;

  if (p == NULL || p->opdl->data->dev_started) {
    return;
  }
...
static void opdl_port_def_conf(struct rte_eventdev *dev, uint8_t port_id,
...
static int opdl_queue_setup(struct rte_eventdev *dev, uint8_t queue_id,
  ...
  if (device->nb_q_md > device->max_queue_nb) {
    PMD_DRV_LOG(ERR,
                ...
                dev->data->dev_id, device->max_queue_nb, device->nb_q_md);
    return -EINVAL;
  }
  ...
  if (RTE_EVENT_QUEUE_CFG_ALL_TYPES & conf->event_queue_cfg) {
    PMD_DRV_LOG(ERR,
                ...
                dev->data->dev_id);
    return -ENOTSUP;
  } else if (RTE_EVENT_QUEUE_CFG_SINGLE_LINK & conf->event_queue_cfg) {
    ...
    default:
      PMD_DRV_LOG(ERR,
                  ...
                  dev->data->dev_id, conf->event_queue_cfg);
      return -EINVAL;
    }
  }
...
static void opdl_queue_release(struct rte_eventdev *dev, uint8_t queue_id) {
  struct opdl_evdev *device = opdl_pmd_priv(dev);
...
static void opdl_queue_def_conf(struct rte_eventdev *dev, uint8_t queue_id,
...
static int opdl_dev_configure(const struct rte_eventdev *dev) {
...
static void opdl_dump(struct rte_eventdev *dev, FILE *f) {
  struct opdl_evdev *device = opdl_pmd_priv(dev);
  ...
  for (i = 0; i < device->max_port_nb; i++) {
    ...
    if (port->initialized) {
      ...
      snprintf(queue_id, sizeof(queue_id), "%02u", port->external_qid);
      if (port->p_type == OPDL_REGULAR_PORT || port->p_type == OPDL_ASYNC_PORT)
        snprintf(total_cyc, sizeof(total_cyc), " %'16" PRIu64 "",
                 (cpg != 0 ? port->port_stat[total_cycles] / cpg : 0));
      else
      ...
      fprintf(f,
              ...
              p_type, i, port->id,
              (port->external_qid == OPDL_INVALID_QID ? "---" : queue_id),
              (cne != 0 ? port->port_stat[claim_pkts_requested] / cne : 0),
              (cne != 0 ? port->port_stat[claim_pkts_granted] / cne : 0),
              total_cyc, port->port_stat[claim_empty],
...
static void opdl_stop(struct rte_eventdev *dev) {
  struct opdl_evdev *device = opdl_pmd_priv(dev);
...
static int opdl_start(struct rte_eventdev *dev) {
  int err = 0;
  ...
  if (!err) {
    ...
    PMD_DRV_LOG(INFO,
                ...
                opdl_pmd_dev_id(device), device->nb_queues,
                (device->nb_queues - device->nb_opdls), device->nb_opdls,
                device->nb_opdls, device->nb_ports, device->queue[0].nb_ports);
  } else
    opdl_stop(dev);
...
static int opdl_close(struct rte_eventdev *dev) {
  struct opdl_evdev *device = opdl_pmd_priv(dev);
  uint32_t i;

  for (i = 0; i < device->max_port_nb; i++) {
    memset(&device->ports[i], 0, sizeof(struct opdl_port));
  }

  memset(&device->s_md, 0x0,
         sizeof(struct opdl_stage_meta_data) * OPDL_PORTS_MAX);

  memset(&device->q_md, 0xFF,
         sizeof(struct opdl_queue_meta_data) * OPDL_MAX_QUEUES);

  memset(device->q_map_ex_to_in, 0, sizeof(uint8_t) * OPDL_INVALID_QID);
...
static int set_do_validation(const char *key __rte_unused, const char *value,
                             void *opaque) {
  int *do_val = opaque;
  *do_val = atoi(value);
  if (*do_val != 0)
...
static int set_do_test(const char *key __rte_unused, const char *value,
                       void *opaque) {
  int *do_test = opaque;
...
static int opdl_probe(struct rte_vdev_device *vdev) {
  ...
  static const char *const args[] = {NUMA_NODE_ARG, DO_VALIDATION_ARG,
                                     DO_TEST_ARG, NULL};
  const char *name;
  const char *params;
  ...
  if (params != NULL && params[0] != '\0') {
    ...
    if (!kvlist) {
      ...
      int ret = rte_kvargs_process(kvlist, NUMA_NODE_ARG, assign_numa_node,
                                   &socket_id);
      if (ret != 0) {
        PMD_DRV_LOG(ERR, "%s: Error parsing numa node parameter", name);

        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, DO_VALIDATION_ARG, set_do_validation,
                               &do_validation);
      if (ret != 0) {
        PMD_DRV_LOG(ERR, "%s: Error parsing do validation parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }

      ret = rte_kvargs_process(kvlist, DO_TEST_ARG, set_do_test, &do_test);
      if (ret != 0) {
        PMD_DRV_LOG(ERR, "%s: Error parsing do test parameter", name);
        rte_kvargs_free(kvlist);
        return ret;
      }
  ...
  dev = rte_event_pmd_vdev_init(name, sizeof(struct opdl_evdev), socket_id);
  ...
  PMD_DRV_LOG(INFO,
              "DEV_ID:[%02d] : "
              "Success - creating eventdev device %s, numa_node:[%d], "
              "do_validation:[%s]"
              " , self_test:[%s]\n",
              dev->data->dev_id, name, socket_id,
              (do_validation ? "true" : "false"), (do_test ? "true" : "false"));
...
static int opdl_remove(struct rte_vdev_device *vdev) {
  const char *name;
...
static struct rte_vdev_driver evdev_opdl_pmd_drv = {.probe = opdl_probe,
                                                    .remove = opdl_remove};

RTE_LOG_REGISTER_SUFFIX(opdl_logtype_driver, driver, INFO);

RTE_PMD_REGISTER_VDEV(EVENTDEV_NAME_OPDL_PMD, evdev_opdl_pmd_drv);
RTE_PMD_REGISTER_PARAM_STRING(event_opdl,
                              NUMA_NODE_ARG "=<int>" DO_VALIDATION_ARG
                                            "=<int>" DO_TEST_ARG "=<int>");
