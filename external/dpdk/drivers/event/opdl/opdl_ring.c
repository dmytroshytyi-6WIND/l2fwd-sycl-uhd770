...
struct opdl_stage {
  ...
  struct claim_manager pending_disclaims[RTE_MAX_LCORE] __rte_cache_aligned;
...
static __rte_always_inline bool is_input_stage(const struct opdl_stage *s) {
  return s->index == 0;
}

/* Get slot pointer from sequence number */
static __rte_always_inline void *get_slot(const struct opdl_ring *t,
                                          uint32_t n) {
  return (void *)(uintptr_t)&t->slots[(n & t->mask) * t->slot_size];
}

/* Find how many entries are available for processing */
static __rte_always_inline uint32_t available(const struct opdl_stage *s) {
  if (s->threadsafe == true) {
    uint32_t n = __atomic_load_n(&s->shared.available_seq, __ATOMIC_ACQUIRE) -
                 __atomic_load_n(&s->shared.head, __ATOMIC_ACQUIRE);
...
static __rte_always_inline void update_available_seq(struct opdl_stage *s) {
  uint32_t i;
  ...
  if (is_input_stage(s)) {
    ...
    for (i = 1; i < s->num_deps; i++) {
      uint32_t seq = __atomic_load_n(&s->deps[i]->tail, __ATOMIC_ACQUIRE);
...
static __rte_always_inline void wait_for_available(struct opdl_stage *s,
                                                   uint32_t n) {
  while (available(s) < n) {
    rte_pause();
...
static __rte_always_inline uint32_t num_to_process(struct opdl_stage *s,
                                                   uint32_t n, bool block) {
  ...
  if (unlikely(n > s->num_slots)) {
    PMD_DRV_LOG(ERR, "%u entries is more than max (%u)", n, s->num_slots);
...
static __rte_always_inline void copy_entries_in(struct opdl_ring *t,
                                                uint32_t start,
                                                const void *entries,
  ...
  if (slot_index + num_entries <= t->num_slots) {
    rte_memcpy(get_slot(t, start), entries, num_entries * slot_size);
  } else {
    uint32_t split = t->num_slots - slot_index;

    rte_memcpy(get_slot(t, start), entries, split * slot_size);
    rte_memcpy(get_slot(t, 0), RTE_PTR_ADD(entries, split * slot_size),
               (num_entries - split) * slot_size);
  }
}

/* Copy entries out from slots with wrap-around */
static __rte_always_inline void copy_entries_out(struct opdl_ring *t,
                                                 uint32_t start, void *entries,
                                                 uint32_t num_entries) {
  uint32_t slot_size = t->slot_size;
  uint32_t slot_index = start & t->mask;

  if (slot_index + num_entries <= t->num_slots) {
    rte_memcpy(entries, get_slot(t, start), num_entries * slot_size);
  } else {
    uint32_t split = t->num_slots - slot_index;

    rte_memcpy(entries, get_slot(t, start), split * slot_size);
    rte_memcpy(RTE_PTR_ADD(entries, split * slot_size), get_slot(t, 0),
               (num_entries - split) * slot_size);
  }
}
...
static __rte_always_inline uint32_t claim_mgr_index(uint32_t n) {
  return n & (OPDL_DISCLAIMS_PER_LCORE - 1);
}

/* Check if there are available slots in claim_manager */
static __rte_always_inline bool claim_mgr_available(struct claim_manager *mgr) {
  return (mgr->mgr_head < (mgr->mgr_tail + OPDL_DISCLAIMS_PER_LCORE)) ? true
                                                                      : false;
}

/* Record a new claim. Only use after first checking an entry is available */
static __rte_always_inline void claim_mgr_add(struct claim_manager *mgr,
                                              uint32_t tail, uint32_t head) {
  if ((mgr->mgr_head != mgr->mgr_tail) &&
      (mgr->claims[claim_mgr_index(mgr->mgr_head - 1)].head == tail)) {
...
static __rte_always_inline bool claim_mgr_read(struct claim_manager *mgr,
                                               uint32_t *tail, uint32_t *head) {
  if (mgr->mgr_head == mgr->mgr_tail)
    return false;
...
static __rte_always_inline void claim_mgr_remove(struct claim_manager *mgr) {
...
static __rte_always_inline void claim_mgr_move_tail(struct claim_manager *mgr,
                                                    uint32_t num_entries) {
...
opdl_stage_disclaim_multithread_n(struct opdl_stage *s, uint32_t num_entries,
                                  bool block) {
  ...
  while (num_entries) {
     ...
     */
    if (__atomic_load_n(&s->shared.tail, __ATOMIC_ACQUIRE) == tail) {
      if (num_entries >= (head - tail)) {
        claim_mgr_remove(disclaims);
        __atomic_store_n(&s->shared.tail, head, __ATOMIC_RELEASE);
        num_entries -= (head - tail);
      } else {
        claim_mgr_move_tail(disclaims, num_entries);
        __atomic_store_n(&s->shared.tail, num_entries + tail, __ATOMIC_RELEASE);
        num_entries = 0;
      }
    } else if (block == false)
...
move_head_atomically(struct opdl_stage *s, uint32_t *num_entries,
  ...
  opdl_stage_disclaim_multithread_n(s, disclaims->num_to_disclaim, false);
  ...
  while (true) {
    ...
    *num_entries = num_to_process(s, orig_num_entries, block);
    ...
    success = __atomic_compare_exchange_n(
        &s->shared.head, old_head, *old_head + *num_entries,
...
opdl_ring_input_multithread(struct opdl_ring *t, const void *entries,
  ...
  __atomic_store_n(&s->shared.tail, old_head + num_entries, __ATOMIC_RELEASE);

  return num_entries;
}

static __rte_always_inline uint32_t opdl_first_entry_id(uint32_t start_seq,
                                                        uint8_t nb_p_lcores,
                                                        uint8_t this_lcore) {
  return ((nb_p_lcores <= 1)
              ? 0
...
static __rte_always_inline uint32_t opdl_stage_claim_singlethread(
    struct opdl_stage *s, void *entries, uint32_t num_entries, uint32_t *seq,
    bool block, bool atomic) {
  ...
  if (!atomic) {
    ...
    offset = opdl_first_entry_id(s->seq, s->nb_instance, s->instance_id);
    ...
    for (j = 0; j < num_entries; j++) {
      ...
      event = __atomic_load_n(&(ev->event), __ATOMIC_ACQUIRE);
...
opdl_stage_claim_multithread(struct opdl_stage *s, void *entries,
  ...
  for (; offset < num_entries; offset += s->nb_instance) {
    memcpy(entries_offset, get_slot(t, s->head + offset), t->slot_size);
    entries_offset += t->slot_size;
    i++;
  }
...
static __rte_always_inline uint32_t opdl_stage_claim_copy_singlethread(
    struct opdl_stage *s, void *entries, uint32_t num_entries, uint32_t *seq,
    bool block) {
  num_entries = num_to_process(s, num_entries, block);
  if (num_entries == 0)
...
static __rte_always_inline uint32_t opdl_stage_claim_copy_multithread(
    struct opdl_stage *s, void *entries, uint32_t num_entries, uint32_t *seq,
    bool block) {
  uint32_t old_head;
...
static __rte_always_inline void
opdl_stage_disclaim_singlethread_n(struct opdl_stage *s, uint32_t num_entries) {
  ...
  __atomic_store_n(&s->shared.tail, num_entries + old_tail, __ATOMIC_RELEASE);
}

uint32_t opdl_ring_input(struct opdl_ring *t, const void *entries,
                         uint32_t num_entries, bool block) {
  if (input_stage(t)->threadsafe == false)
    return opdl_ring_input_singlethread(t, entries, num_entries, block);
  else
    return opdl_ring_input_multithread(t, entries, num_entries, block);
}

uint32_t opdl_ring_copy_from_burst(struct opdl_ring *t, struct opdl_stage *s,
                                   const void *entries, uint32_t num_entries,
                                   bool block) {
  uint32_t head = s->head;
...
uint32_t opdl_ring_copy_to_burst(struct opdl_ring *t, struct opdl_stage *s,
                                 void *entries, uint32_t num_entries,
                                 bool block) {
  uint32_t head = s->head;
...
uint32_t opdl_stage_find_num_available(struct opdl_stage *s,
                                       uint32_t num_entries) {
...
uint32_t opdl_stage_claim(struct opdl_stage *s, void *entries,
                          uint32_t num_entries, uint32_t *seq, bool block,
                          bool atomic) {
  if (s->threadsafe == false)
    return opdl_stage_claim_singlethread(s, entries, num_entries, seq, block,
                                         atomic);
  else
    return opdl_stage_claim_multithread(s, entries, num_entries, seq, block);
}

uint32_t opdl_stage_claim_copy(struct opdl_stage *s, void *entries,
                               uint32_t num_entries, uint32_t *seq,
                               bool block) {
  if (s->threadsafe == false)
    return opdl_stage_claim_copy_singlethread(s, entries, num_entries, seq,
                                              block);
  else
    return opdl_stage_claim_copy_multithread(s, entries, num_entries, seq,
                                             block);
}

void opdl_stage_disclaim_n(struct opdl_stage *s, uint32_t num_entries,
                           bool block) {
...
int opdl_stage_disclaim(struct opdl_stage *s, uint32_t num_entries,
                        bool block) {
  ...
  if (s->threadsafe == false) {
    ...
    struct claim_manager *disclaims = &s->pending_disclaims[rte_lcore_id()];
    opdl_stage_disclaim_multithread_n(s, disclaims->num_claimed, block);
  }
  return num_entries;
}

uint32_t opdl_ring_available(struct opdl_ring *t) {
  return opdl_stage_available(&t->stages[0]);
}

uint32_t opdl_stage_available(struct opdl_stage *s) {
  update_available_seq(s);
  return available(s);
}

void opdl_ring_flush(struct opdl_ring *t) {
  struct opdl_stage *s = input_stage(t);
...
static int init_stage(struct opdl_ring *t, struct opdl_stage *s,
                      bool threadsafe, bool is_input) {
  ...
  s->dep_tracking = rte_zmalloc_socket(
      LIB_NAME, t->max_num_stages * sizeof(enum dep_type), 0, t->socket);
  if (s->dep_tracking == NULL)
    return -ENOMEM;

  s->deps = rte_zmalloc_socket(
      LIB_NAME, t->max_num_stages * sizeof(struct shared_state *), 0,
      t->socket);
  if (s->deps == NULL) {
...
static int add_dep(struct opdl_stage *dependent,
                   const struct opdl_stage *dependency, enum dep_type type) {
  struct opdl_ring *t = dependent->t;
  uint32_t i;

  /* Add new direct dependency */
  if ((type == DEP_DIRECT) &&
      (dependent->dep_tracking[dependency->index] == DEP_NONE)) {
    PMD_DRV_LOG(DEBUG, "%s:%u direct dependency on %u", t->name,
  ...
  if ((type == DEP_INDIRECT) &&
      ((dependent->dep_tracking[dependency->index] == DEP_NONE) ||
       (dependent->dep_tracking[dependency->index] == DEP_DIRECT))) {
    PMD_DRV_LOG(DEBUG, "%s:%u indirect dependency on %u", t->name,
                dependent->index, dependency->index);
  ...
  if ((dependent->dep_tracking[dependency->index] == DEP_SELF) &&
    ...
    PMD_DRV_LOG(ERR, "Loop in dependency graph %s:%u", t->name,
                dependent->index);
    return -EINVAL;
  }
  ...
  if (dependency != input_stage(t))
    for (i = 0; i < dependency->num_deps; i++) {
      int ret = add_dep(dependent, dependency->deps[i]->stage, DEP_INDIRECT);
    ...
    for (i = 0, dependent->num_deps = 0; i < t->num_stages; i++)
      if (dependent->dep_tracking[i] == DEP_DIRECT) {
        if ((i == 0) && (dependent->num_deps > 1))
          rte_panic("%s:%u depends on > input", t->name, dependent->index);
        dependent->deps[dependent->num_deps++] = &t->stages[i].shared;
      }

  return 0;
}

struct opdl_ring *opdl_ring_create(const char *name, uint32_t num_slots,
                                   uint32_t slot_size, uint32_t max_num_stages,
                                   int socket) {
  struct opdl_ring *t;
  ...
  size_t alloc_size =
      RTE_CACHE_LINE_ROUNDUP(sizeof(*t) + (num_slots * slot_size));

  /* Compile time checking */
  RTE_BUILD_BUG_ON((sizeof(struct shared_state) & RTE_CACHE_LINE_MASK) != 0);
  RTE_BUILD_BUG_ON(
      (offsetof(struct opdl_stage, shared) & RTE_CACHE_LINE_MASK) != 0);
  RTE_BUILD_BUG_ON((offsetof(struct opdl_ring, slots) & RTE_CACHE_LINE_MASK) !=
                   0);
  ...
  if (!rte_is_power_of_2(num_slots)) {
    PMD_DRV_LOG(ERR, "num_slots (%u) for %s is not power of 2", num_slots,
                name);
    return NULL;
  }

  /* Alloc memory for stages */
  st = rte_zmalloc_socket(LIB_NAME, max_num_stages * sizeof(struct opdl_stage),
...
void *opdl_ring_get_slot(const struct opdl_ring *t, uint32_t index) {
  return get_slot(t, index);
}

bool opdl_ring_cas_slot(struct opdl_stage *s, const struct rte_event *ev,
  ...
  if (!atomic) {
    ...
    for (i = s->pos; i < s->num_claimed; i++) {
      ...
      event = __atomic_load_n(&(ev_orig->event), __ATOMIC_ACQUIRE);
      ...
      if ((flow_id % s->nb_instance) == s->instance_id) {
        ...
        ev_update = (ev_update << OPDL_OPA_OFFSET) | ev->event;

        s->pos = i + 1;

        if ((event & OPDL_EVENT_MASK) != ev_temp) {
          __atomic_store_n(&(ev_orig->event), ev_update, __ATOMIC_RELEASE);
          ev_updated = true;
        }
        if (ev_orig->u64 != ev->u64) {
          ev_orig->u64 = ev->u64;
...
int opdl_ring_get_socket(const struct opdl_ring *t) { return t->socket; }

uint32_t opdl_ring_get_num_slots(const struct opdl_ring *t) {
  return t->num_slots;
}

const char *opdl_ring_get_name(const struct opdl_ring *t) { return t->name; }

/* Check dependency list is valid for a given opdl_ring */
static int check_deps(struct opdl_ring *t, struct opdl_stage *deps[],
                      uint32_t num_deps) {
  unsigned int i;
  ...
  for (i = 0; i < num_deps; ++i) {
    ...
    if (t != deps[i]->t) {
      PMD_DRV_LOG(ERR, "deps[%u] is in opdl_ring %s, not %s", i,
                  deps[i]->t->name, t->name);
      return -EINVAL;
    }
  }
...
struct opdl_stage *opdl_stage_add(struct opdl_ring *t, bool threadsafe,
                                  bool is_input) {
  struct opdl_stage *s;
  ...
  if (t->num_stages == t->max_num_stages) {
    PMD_DRV_LOG(ERR, "%s has max number of stages (%u)", t->name,
                t->max_num_stages);
    return NULL;
  }
...
uint32_t opdl_stage_deps_add(struct opdl_ring *t, struct opdl_stage *s,
...
struct opdl_stage *opdl_ring_get_input_stage(const struct opdl_ring *t) {
  return input_stage(t);
}

int opdl_stage_set_deps(struct opdl_stage *s, struct opdl_stage *deps[],
                        uint32_t num_deps) {
  unsigned int i;
  int ret;
...
struct opdl_ring *opdl_stage_get_opdl_ring(const struct opdl_stage *s) {
  return s->t;
}

void opdl_stage_set_queue_id(struct opdl_stage *s, uint32_t queue_id) {
  s->queue_id = queue_id;
}

void opdl_ring_dump(const struct opdl_ring *t, FILE *f) {
  uint32_t i;
  ...
  fprintf(f,
          "OPDL \"%s\": num_slots=%u; mask=%#x; slot_size=%u; num_stages=%u; "
          "socket=%i\n",
          t->name, t->num_slots, t->mask, t->slot_size, t->num_stages,
          t->socket);
  for (i = 0; i < t->num_stages; i++) {
    ...
    fprintf(
        ...
        (s->threadsafe) ? s->shared.available_seq : s->available_seq,
        s->shared.tail, (s->num_deps > 0) ? s->deps[0]->stage->index : 0);
...
void opdl_ring_free(struct opdl_ring *t) {
  uint32_t i;
...
struct opdl_ring *opdl_ring_lookup(const char *name) {
  const struct rte_memzone *mz;
  char mz_name[RTE_MEMZONE_NAMESIZE];
...
void opdl_ring_set_stage_threadsafe(struct opdl_stage *s, bool threadsafe) {
  s->threadsafe = threadsafe;
}
