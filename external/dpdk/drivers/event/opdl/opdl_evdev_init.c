...
static __rte_always_inline uint32_t enqueue_check(struct opdl_port *p,
  ...
  if (p->opdl->do_validation) {
    ...
    for (i = 0; i < num; i++) {
      if (ev[i].queue_id != p->next_external_qid) {
        PMD_DRV_LOG(ERR,
                    ...
                    " to enq to q_id[%u],"
                    " but should be [%u]",
                    opdl_pmd_dev_id(p->opdl), p->id, ev[i].queue_id,
                    p->next_external_qid);
        rte_errno = EINVAL;
        return 0;
      }
    ...
    /* Stats */
    if (p->p_type == OPDL_PURE_RX_PORT || p->p_type == OPDL_ASYNC_PORT) {
      /* Stats */
      if (num_events) {
      ...
      if (p->start_cycles) {
        ...
        p->port_stat[total_cycles] += end_cycles - p->start_cycles;
      }
    }
  } else {
    if (num > 0 && ev[0].queue_id != p->next_external_qid) {
      rte_errno = EINVAL;
      return 0;
    }
  }
...
static __rte_always_inline void update_on_dequeue(struct opdl_port *p,
                                                  struct rte_event ev[],
                                                  uint16_t num,
                                                  uint16_t num_events) {
  if (p->opdl->do_validation) {
    int16_t i;
    for (i = 0; i < num; i++)
      ev[i].queue_id = p->opdl->queue[p->queue_id].external_qid;
    ...
    if (num > 0)
      ev[0].queue_id = p->opdl->queue[p->queue_id].external_qid;
  }
}
...
static uint16_t opdl_rx_error_enqueue(struct opdl_port *p,
                                      const struct rte_event ev[],
                                      uint16_t num) {
  RTE_SET_USED(p);
  RTE_SET_USED(ev);
...
static uint16_t opdl_rx_enqueue(struct opdl_port *p,
                                const struct rte_event ev[], uint16_t num) {
  uint16_t enqueued = 0;

  enqueued = opdl_ring_input(opdl_stage_get_opdl_ring(p->enq_stage_inst), ev,
                             num, false);
...
static uint16_t opdl_tx_error_dequeue(struct opdl_port *p,
...
static uint16_t opdl_tx_dequeue_single_thread(struct opdl_port *p,
                                              struct rte_event ev[],
                                              uint16_t num) {
  uint16_t returned;
  ...
  returned = opdl_ring_copy_to_burst(ring, p->deq_stage_inst, ev, num, false);
...
static uint16_t opdl_tx_dequeue_multi_inst(struct opdl_port *p,
                                           struct rte_event ev[],
                                           uint16_t num) {
  uint32_t num_events = 0;

  num_events =
      opdl_stage_claim(p->deq_stage_inst, (void *)ev, num, NULL, false, false);
...
static uint16_t opdl_claim(struct opdl_port *p, struct rte_event ev[],
                           uint16_t num) {
  uint32_t num_events = 0;
  ...
  if (unlikely(num > MAX_OPDL_CONS_Q_DEPTH)) {
    PMD_DRV_LOG(ERR,
                ...
                opdl_pmd_dev_id(p->opdl), p->id);
    rte_errno = EINVAL;
    return 0;
  }

  num_events = opdl_stage_claim(p->deq_stage_inst, (void *)ev, num, NULL, false,
                                p->atomic_claim);
...
static uint16_t opdl_disclaim(struct opdl_port *p, const struct rte_event ev[],
                              uint16_t num) {
  uint16_t enqueued = 0;
  ...
  for (i = 0; i < num; i++)
    opdl_ring_cas_slot(p->enq_stage_inst, &ev[i], i, p->atomic_claim);

  enqueued = opdl_stage_disclaim(p->enq_stage_inst, num, false);
...
static int opdl_add_deps(struct opdl_evdev *device, int q_id, int deps_q_id) {
  unsigned int i, j;
  int status;
  struct opdl_ring *ring;
  ...
  for (i = 0; i < queue->nb_ports; i++) {
    struct opdl_ring *r = opdl_stage_get_opdl_ring(stage_for_port(queue, i));
    for (j = 0; j < queue_deps->nb_ports; j++) {
      struct opdl_ring *rj =
          opdl_stage_get_opdl_ring(stage_for_port(queue_deps, j));
      if (r != rj) {
        PMD_DRV_LOG(ERR,
                    "DEV_ID:[%02d] : "
        ...
        for (k = 0; k < device->nb_opdls; k++) {
          opdl_ring_dump(device->opdl[k], stdout);
        }
        return -EINVAL;
      }
    }
  ...
  for (i = 0; i < queue->nb_ports; i++) {
    ...
    status = opdl_stage_deps_add(
        ring, stage_for_port(queue, i), queue->ports[i]->num_instance,
        queue->ports[i]->instance_id, dep_stages, queue_deps->nb_ports);
    if (status < 0)
      return -EINVAL;
  }
    ...
    if (port->configured) {
      if (port->p_type == OPDL_PURE_RX_PORT) {
        ...
        PMD_DRV_LOG(ERR,
                    ...
                    opdl_pmd_dev_id(port->opdl), port->id);
        err = -EINVAL;
        break;
      }
      port->initialized = 1;
    ...
    if (queue->q_pos == OPDL_Q_POS_MIDDLE) {
      ...
      if (err < 0) {
        PMD_DRV_LOG(ERR,
                    ...
                    dev->data->dev_id, queue->external_qid);
        break;
      }
    }
  ...
  if (!err) {
    if ((i - nb_iq) != device->max_queue_nb) {
      PMD_DRV_LOG(ERR,
                  ...
                  dev->data->dev_id, i - nb_iq, device->max_queue_nb);
      err = -1;
    }
  }
  return err;
...
void destroy_queues_and_rings(struct rte_eventdev *dev) {
  ...
  memset(&device->queue, 0,
         sizeof(struct opdl_queue) * RTE_EVENT_MAX_QUEUES_PER_DEV);
}

#define OPDL_ID(d) (d->nb_opdls - 1)

static __rte_always_inline void
initialise_queue(struct opdl_evdev *device, enum queue_pos pos, int32_t i) {
...
static __rte_always_inline int create_opdl(struct opdl_evdev *device) {
  int err = 0;

  char name[RTE_MEMZONE_NAMESIZE];

  snprintf(name, RTE_MEMZONE_NAMESIZE, "%s_%u", device->service_name,
           device->nb_opdls);

  device->opdl[device->nb_opdls] =
      opdl_ring_create(name, device->nb_events_limit, sizeof(struct rte_event),
                       device->max_port_nb * 2, device->socket);

  if (!device->opdl[device->nb_opdls]) {
    PMD_DRV_LOG(ERR,
                "DEV_ID:[%02d] : "
                "opdl ring %u creation - FAILED",
                opdl_pmd_dev_id(device), device->nb_opdls);
    err = -EINVAL;
  } else {
    device->nb_opdls++;
  }
...
static __rte_always_inline int create_link_opdl(struct opdl_evdev *device,
                                                uint32_t index) {

  int err = 0;

  if (device->q_md[index + 1].type != OPDL_Q_TYPE_SINGLE_LINK) {
    ...
    if (!err) {
       ...
       */
      initialise_queue(device, OPDL_Q_POS_START, -1);
    } else {
      err = -EINVAL;
    }
  } else {
...
int create_queues_and_rings(struct rte_eventdev *dev) {
  ...
  if (device->nb_ports != device->max_port_nb) {
    PMD_DRV_LOG(ERR,
                ...
                device->nb_ports, device->max_port_nb);
    err = -1;
  }
  ...
  if (!err) {
    ...
    for (i = 0; i < device->nb_q_md; i++) {
      ...
      if (!device->q_md[i].setup) {
        ...
        err = -EINVAL;
        break;
      } else if (device->q_md[i].type != OPDL_Q_TYPE_SINGLE_LINK) {

        if (!device->q_md[i + 1].setup) {
          /* Create a simple ORDERED/ATOMIC
           * queue at the end
           */
          initialise_queue(device, OPDL_Q_POS_END, i);

        } else {
          /* Create a simple ORDERED/ATOMIC
           * queue in the middle
           */
          initialise_queue(device, OPDL_Q_POS_MIDDLE, i);
        }
      } else if (device->q_md[i].type == OPDL_Q_TYPE_SINGLE_LINK) {
...
int initialise_all_other_ports(struct rte_eventdev *dev) {
  ...
  for (i = 0; i < device->nb_ports; i++) {
    ...
    if (port->queue_id == 0) {
      ...
      if (queue->q_pos == OPDL_Q_POS_MIDDLE) {
        ...
        stage_inst = opdl_stage_add(device->opdl[queue->opdl_id], false, false);
        ...
        queue->nb_ports++;
        opdl_stage_set_queue_id(stage_inst, port->queue_id);
        ...
        PMD_DRV_LOG(ERR,
                    ...
                    opdl_pmd_dev_id(port->opdl), port->id, queue->q_pos);
        err = -EINVAL;
        break;
      }
      ...
      /* -- tx -- */
      stage_inst = opdl_stage_add(device->opdl[queue->opdl_id], false,
                                  false); /* First stage */
      port->deq_stage_inst = stage_inst;
      ...
      if (queue->nb_ports > 1) {
        PMD_DRV_LOG(ERR,
                    ...
                    opdl_pmd_dev_id(port->opdl), queue->external_qid);
        err = -EINVAL;
        break;
      }

      /* -- single instance rx for next opdl -- */
      uint8_t next_qid = device->q_map_ex_to_in[queue->external_qid] + 1;
      if (next_qid < RTE_EVENT_MAX_QUEUES_PER_DEV &&
        ...
        stage_inst = opdl_stage_add(device->opdl[queue->opdl_id], false, true);
        port->enq_stage_inst = stage_inst;
        ...
        if (queue->nb_ports > 1) {
          PMD_DRV_LOG(ERR,
                      ...
                      opdl_pmd_dev_id(port->opdl), next_qid, port->id);
          err = -EINVAL;
          break;
        }
  ...
  if (!err) {
    for (i = 0; i < device->nb_ports; i++) {
      ...
      if (port->configured && (port->queue_id != OPDL_INVALID_QID)) {
        if (queue->nb_ports == 0) {
          PMD_DRV_LOG(ERR,
                      "DEV_ID:[%02d] : "
...
int initialise_queue_zero_ports(struct rte_eventdev *dev) {
  ...
  /* Create the stage */
  stage_inst =
      opdl_stage_add(device->opdl[0], (mt_rx > 1 ? true : false), true);
  if (stage_inst) {
...
int assign_internal_queue_ids(struct rte_eventdev *dev) {
  ...
  for (i = 0; i < device->nb_ports; i++) {
    ...
    if (port->external_qid != OPDL_INVALID_QID) {
      port->queue_id = device->q_map_ex_to_in[port->external_qid];
