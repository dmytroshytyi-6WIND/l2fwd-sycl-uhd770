...
int cnxk_sso_xstats_get(const struct rte_eventdev *event_dev,
  ...
  case RTE_EVENT_DEV_XSTATS_PORT:
    ...
    rc = roc_sso_hws_stats_get(&dev->sso, queue_port_id, &hws_stats);
    if (rc < 0)
      goto invalid_value;
    rsp = &hws_stats;
    break;
  case RTE_EVENT_DEV_XSTATS_QUEUE:
    ...
    rc = roc_sso_hwgrp_stats_get(&dev->sso, queue_port_id, &hwgrp_stats);
    if (rc < 0)
      goto invalid_value;
    rsp = &hwgrp_stats;
...
int cnxk_sso_xstats_get_names(const struct rte_eventdev *event_dev,
  ...
  for (i = 0; i < CNXK_SSO_NUM_HWS_XSTATS; i++) {
    snprintf(xstats_names_copy[i].name, sizeof(xstats_names_copy[i].name), "%s",
