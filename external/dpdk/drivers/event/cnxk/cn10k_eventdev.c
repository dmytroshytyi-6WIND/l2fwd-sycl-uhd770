...
static uint32_t cn10k_sso_gw_mode_wdata(struct cnxk_sso_evdev *dev) {
  uint32_t wdata = 1;
...
static void *cn10k_sso_init_hws_mem(void *arg, uint8_t port_id) {
...
static int cn10k_sso_hws_link(void *arg, void *port, uint16_t *map,
                              uint16_t nb_link) {
  struct cnxk_sso_evdev *dev = arg;
...
static int cn10k_sso_hws_unlink(void *arg, void *port, uint16_t *map,
                                uint16_t nb_link) {
  struct cnxk_sso_evdev *dev = arg;
  struct cn10k_sso_hws *ws = port;
...
static void cn10k_sso_hws_setup(void *arg, void *hws, uintptr_t grp_base) {
  struct cnxk_sso_evdev *dev = arg;
...
static void cn10k_sso_hws_release(void *arg, void *hws) {
  struct cnxk_sso_evdev *dev = arg;
  struct cn10k_sso_hws *ws = hws;
  uint16_t i;
...
static void cn10k_sso_hws_flush_events(void *hws, uint8_t queue_id,
...
static void cn10k_sso_hws_reset(void *arg, void *hws) {
  struct cnxk_sso_evdev *dev = arg;
  struct cn10k_sso_hws *ws = hws;
  ...
  } while (pend_state & (BIT_ULL(63) | BIT_ULL(62) | BIT_ULL(58) | BIT_ULL(56) |
                         BIT_ULL(54)));
...
static void cn10k_sso_set_rsrc(void *arg) {
  struct cnxk_sso_evdev *dev = arg;
...
static int cn10k_sso_rsrc_init(void *arg, uint8_t hws, uint8_t hwgrp) {
  struct cnxk_sso_evdev *dev = arg;

  return roc_sso_rsrc_init(&dev->sso, hws, hwgrp);
}

static int cn10k_sso_updt_tx_adptr_data(const struct rte_eventdev *event_dev) {
...
static void cn10k_sso_fp_fns_set(struct rte_eventdev *event_dev) {
  struct cnxk_sso_evdev *dev = cnxk_sso_pmd_priv(event_dev);
...
#undef T
  ...
  if (dev->rx_offloads & NIX_RX_MULTI_SEG_F) {
    ...
    if (dev->is_timeout_deq) {
      CN10K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_deq_tmo_seg);
      CN10K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
    ...
    if (dev->is_timeout_deq && dev->is_ca_internal_port) {
      CN10K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_deq_tmo_ca_seg);
      CN10K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
...
static void cn10k_sso_info_get(struct rte_eventdev *event_dev,
...
static int cn10k_sso_dev_configure(const struct rte_eventdev *event_dev) {
  struct cnxk_sso_evdev *dev = cnxk_sso_pmd_priv(event_dev);
  int rc;
  ...
  rc = cn10k_sso_rsrc_init(dev, dev->nb_event_ports, dev->nb_event_queues);
  if (rc < 0) {
...
static int cn10k_sso_port_setup(struct rte_eventdev *event_dev, uint8_t port_id,
...
static void cn10k_sso_port_release(void *port) {
...
static int cn10k_sso_port_link(struct rte_eventdev *event_dev, void *port,
...
static int cn10k_sso_port_unlink(struct rte_eventdev *event_dev, void *port,
...
static int cn10k_sso_start(struct rte_eventdev *event_dev) {
  int rc;
...
static void cn10k_sso_stop(struct rte_eventdev *event_dev) {
  cnxk_sso_stop(event_dev, cn10k_sso_hws_reset, cn10k_sso_hws_flush_events);
}

static int cn10k_sso_close(struct rte_eventdev *event_dev) {
  return cnxk_sso_close(event_dev, cn10k_sso_hws_unlink);
}

static int cn10k_sso_selftest(void) {
  return cnxk_sso_selftest(RTE_STR(event_cn10k));
}

static int cn10k_sso_rx_adapter_caps_get(const struct rte_eventdev *event_dev,
...
static void cn10k_sso_set_priv_mem(const struct rte_eventdev *event_dev,
...
static int cn10k_sso_rx_adapter_queue_add(
...
static int cn10k_sso_rx_adapter_queue_del(const struct rte_eventdev *event_dev,
                                          const struct rte_eth_dev *eth_dev,
                                          int32_t rx_queue_id) {
  int rc;
...
static int cn10k_sso_rx_adapter_vector_limits(
...
static int cn10k_sso_tx_adapter_caps_get(const struct rte_eventdev *dev,
                                         const struct rte_eth_dev *eth_dev,
                                         uint32_t *caps) {
  int ret;
...
static void cn10k_sso_txq_fc_update(const struct rte_eth_dev *eth_dev,
                                    int32_t tx_queue_id) {
...
static int cn10k_sso_tx_adapter_queue_add(uint8_t id,
...
static int cn10k_sso_tx_adapter_queue_del(uint8_t id,
                                          const struct rte_eventdev *event_dev,
                                          const struct rte_eth_dev *eth_dev,
                                          int32_t tx_queue_id) {
  int rc;
...
static int cn10k_crypto_adapter_caps_get(const struct rte_eventdev *event_dev,
...
static int cn10k_crypto_adapter_qp_add(const struct rte_eventdev *event_dev,
                                       const struct rte_cryptodev *cdev,
...
static int cn10k_crypto_adapter_qp_del(const struct rte_eventdev *event_dev,
                                       const struct rte_cryptodev *cdev,
                                       int32_t queue_pair_id) {
  CNXK_VALID_DEV_OR_ERR_RET(event_dev->dev, "event_cn10k");
  CNXK_VALID_DEV_OR_ERR_RET(cdev->device, "crypto_cn10k");
...
static int cn10k_sso_init(struct rte_eventdev *event_dev) {
  struct cnxk_sso_evdev *dev = cnxk_sso_pmd_priv(event_dev);
  int rc;
...
static int cn10k_sso_probe(struct rte_pci_driver *pci_drv,
                           struct rte_pci_device *pci_dev) {
  return rte_event_pmd_pci_probe(pci_drv, pci_dev,
                                 sizeof(struct cnxk_sso_evdev), cn10k_sso_init);
}
...
RTE_PMD_REGISTER_PARAM_STRING(
    event_cn10k, CNXK_SSO_XAE_CNT
    "=<int>" CNXK_SSO_GGRP_QOS "=<string>" CNXK_SSO_FORCE_BP
    "=1" CN10K_SSO_GW_MODE "=<int>" CNXK_TIM_DISABLE_NPA
    "=1" CNXK_TIM_CHNK_SLOTS "=<int>" CNXK_TIM_RINGS_LMT
    "=<int>" CNXK_TIM_STATS_ENA "=1" CNXK_TIM_EXT_CLK "=<string>");
