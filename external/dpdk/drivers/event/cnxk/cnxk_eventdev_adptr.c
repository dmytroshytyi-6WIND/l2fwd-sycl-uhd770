...
void cnxk_sso_updt_xae_cnt(struct cnxk_sso_evdev *dev, void *data,
  ...
  case RTE_EVENT_TYPE_ETHDEV: {
    ...
    dev->rx_adptr_pools = rte_realloc(
        dev->rx_adptr_pools, sizeof(uint64_t) * dev->rx_adptr_pool_cnt, 0);
    ...
    dev->rx_adptr_pools[dev->rx_adptr_pool_cnt - 1] = (uint64_t)rxq->qconf.mp;
  ...
  case RTE_EVENT_TYPE_ETHDEV_VECTOR: {
    ...
    dev->vec_pools =
        rte_realloc(dev->vec_pools, sizeof(uint64_t) * dev->vec_pool_cnt, 0);
  ...
  case RTE_EVENT_TYPE_TIMER: {
    ...
    dev->timer_adptr_rings = rte_realloc(
        dev->timer_adptr_rings, sizeof(uint16_t) * dev->tim_adptr_ring_cnt, 0);
    ...
    dev->timer_adptr_sz = rte_realloc(
        dev->timer_adptr_sz, sizeof(uint64_t) * dev->tim_adptr_ring_cnt, 0);
    ...
    dev->timer_adptr_rings[dev->tim_adptr_ring_cnt - 1] = timr->ring_id;
    dev->timer_adptr_sz[dev->tim_adptr_ring_cnt - 1] = timr->nb_timers;
...
static int cnxk_sso_rxq_enable(struct cnxk_eth_dev *cnxk_eth_dev,
                               uint16_t rq_id, uint16_t port_id,
  ...
  rq->tag_mask |= (((port_id >> 4) & 0xF) | (RTE_EVENT_TYPE_ETHDEV << 4)) << 24;
  ...
  if (rq_id == 0 && roc_nix_inl_inb_is_enabled(nix)) {
    ...
    rc = roc_nix_inl_inb_tag_update(nix, sec_tag_const, ev->sched_type);
    if (rc)
...
static int cnxk_sso_rxq_disable(struct cnxk_eth_dev *cnxk_eth_dev,
                                uint16_t rq_id) {
  struct roc_nix_rq *rq;
...
static int cnxk_sso_rx_adapter_vwqe_enable(struct cnxk_eth_dev *cnxk_eth_dev,
                                           uint16_t port_id, uint16_t rq_id,
                                           uint16_t sz, uint64_t tmo_ns,
                                           struct rte_mempool *vmp) {
  struct roc_nix_rq *rq;
  ...
  rq->vwqe_wait_tmo =
      tmo_ns / ((roc_nix_get_vwqe_interval(&cnxk_eth_dev->nix) + 1) * 100);
  rq->tag_mask = (port_id & 0xF) << 20;
  rq->tag_mask |= (((port_id >> 4) & 0xF) | (RTE_EVENT_TYPE_ETHDEV_VECTOR << 4))
                  << 24;
...
int cnxk_sso_rx_adapter_queue_add(
  ...
  if (rx_queue_id < 0) {
    for (i = 0; i < eth_dev->data->nb_rx_queues; i++)
      rc |= cnxk_sso_rx_adapter_queue_add(event_dev, eth_dev, i, queue_conf);
  } else {
    ...
    rc |= cnxk_sso_rxq_enable(cnxk_eth_dev, (uint16_t)rx_queue_id, port,
                              &queue_conf->ev,
    ...
    if (queue_conf->rx_queue_flags &
      ...
      rc |= cnxk_sso_rx_adapter_vwqe_enable(
          cnxk_eth_dev, port, rx_queue_id, queue_conf->vector_sz,
          queue_conf->vector_timeout_ns, queue_conf->vector_mp);

      if (cnxk_eth_dev->vec_drop_re_dis)
        rc |= roc_nix_rx_drop_re_set(&cnxk_eth_dev->nix, false);
    }
    rox_nix_fc_npa_bp_cfg(&cnxk_eth_dev->nix, rxq_sp->qconf.mp->pool_id, true,
                          dev->force_ena_bp);
...
int cnxk_sso_rx_adapter_queue_del(const struct rte_eventdev *event_dev,
  ...
  if (rx_queue_id < 0) {
    ...
    rox_nix_fc_npa_bp_cfg(&cnxk_eth_dev->nix, rxq_sp->qconf.mp->pool_id, false,
                          dev->force_ena_bp);
    cnxk_eth_dev->nb_rxq_sso--;
...
static int cnxk_sso_sqb_aura_limit_edit(struct roc_nix_sq *sq,
                                        uint16_t nb_sqb_bufs) {
  int rc;

  if (sq->nb_sqb_bufs != nb_sqb_bufs) {
    rc = roc_npa_aura_limit_modify(sq->aura_handle,
...
static void cnxk_sso_tx_queue_data_init(struct cnxk_sso_evdev *dev,
                                        uint64_t *txq_data,
...
static void cnxk_sso_tx_queue_data_cpy(struct cnxk_sso_evdev *dev,
                                       uint64_t *txq_data, uint64_t *otxq_data,
                                       uint16_t eth_port_id) {
  uint64_t offset = 0;
  int i, j;
  ...
  for (i = 1; i < eth_port_id; i++) {
    offset += (dev->max_queue_id[i - 1] + 1);
    txq_data[i] |= offset << 48;
    for (j = 0; (i < dev->max_port_id) && (j < dev->max_queue_id[i] + 1); j++)
      txq_data[offset + j] = otxq_data[(otxq_data[i] >> 48) + j];
  }
}
...
cnxk_sso_tx_queue_data_rewrite(struct cnxk_sso_evdev *dev, uint64_t *txq_data,
  ...
  if (eth_port_id > max_port_id) {
    ...
    cnxk_sso_tx_queue_data_cpy(dev, txq_data, otxq_data, eth_port_id);
    dev->max_queue_id[eth_port_id] =
        RTE_MAX(dev->max_queue_id[eth_port_id], tx_queue_id);
    ...
    cnxk_sso_tx_queue_data_cpy_max(dev, txq_data, otxq_data, eth_port_id,
                                   max_port_id, max_queue_id);
  }
}
...
static int cnxk_sso_updt_tx_queue_data(const struct rte_eventdev *event_dev,
                                       uint16_t eth_port_id,
                                       uint16_t tx_queue_id, void *txq) {
  struct cnxk_sso_evdev *dev = cnxk_sso_pmd_priv(event_dev);
  ...
  if (size) {
    ...
    if (otxq_data != NULL)
      cnxk_sso_tx_queue_data_rewrite(dev, txq_data, eth_port_id, tx_queue_id,
                                     otxq_data, max_port_id, max_queue_id);
    else
      cnxk_sso_tx_queue_data_init(dev, txq_data, eth_port_id, tx_queue_id);
...
int cnxk_sso_tx_adapter_queue_add(const struct rte_eventdev *event_dev,
  ...
  if (tx_queue_id < 0) {
    for (i = 0; i < eth_dev->data->nb_tx_queues; i++)
      ret |= cnxk_sso_tx_adapter_queue_add(event_dev, eth_dev, i);
  } else {
    txq = eth_dev->data->tx_queues[tx_queue_id];
    sq = &cnxk_eth_dev->sqs[tx_queue_id];
    cnxk_sso_sqb_aura_limit_edit(sq, sq->nb_sqb_bufs);
    ret = cnxk_sso_updt_tx_queue_data(event_dev, eth_dev->data->port_id,
                                      tx_queue_id, txq);
    if (ret < 0)
      return ret;
  }
...
int cnxk_sso_tx_adapter_queue_del(const struct rte_eventdev *event_dev,
  ...
  if (tx_queue_id < 0) {
    for (i = 0; i < eth_dev->data->nb_tx_queues; i++)
      ret |= cnxk_sso_tx_adapter_queue_del(event_dev, eth_dev, i);
  } else {
    sq = &cnxk_eth_dev->sqs[tx_queue_id];
    cnxk_sso_sqb_aura_limit_edit(sq, sq->nb_sqb_bufs);
    ret = cnxk_sso_updt_tx_queue_data(event_dev, eth_dev->data->port_id,
