...
cnxk_tim_arm_checks(const struct cnxk_tim_ring *const tim_ring,
  ...
  if (unlikely(!tim->timeout_ticks || tim->timeout_ticks > tim_ring->nb_bkts)) {
...
cnxk_tim_format_event(const struct rte_event_timer *const tim,
  ...
  entry->w0 =
      (tim->ev.event & 0xFFC000000000) >> 6 | (tim->ev.event & 0xFFFFFFFFF);
  entry->wqe = tim->ev.u64;
}

static inline void cnxk_tim_sync_start_cyc(struct cnxk_tim_ring *tim_ring) {
  ...
  if (cur_cyc - tim_ring->last_updt_cyc > tim_ring->tot_int) {
    ...
    tim_ring->ring_start_cyc = cur_cyc - (real_bkt * tim_ring->tck_int);
    tim_ring->last_updt_cyc = cur_cyc;
  }
}

static __rte_always_inline uint16_t cnxk_tim_timer_arm_burst(
    const struct rte_event_timer_adapter *adptr, struct rte_event_timer **tim,
  ...
  for (index = 0; index < nb_timers; index++) {
    ...
    if (flags & CNXK_TIM_SP)
      ret = cnxk_tim_add_entry_sp(tim_ring, tim[index]->timeout_ticks,
                                  tim[index], &entry, flags);
    if (flags & CNXK_TIM_MP)
      ret = cnxk_tim_add_entry_mp(tim_ring, tim[index]->timeout_ticks,
                                  tim[index], &entry, flags);
...
#define FP(_name, _f3, _f2, _f1, _flags)                                       \
  uint16_t __rte_noinline cnxk_tim_arm_burst_##_name(                          \
    ...
    return cnxk_tim_timer_arm_burst(adptr, tim, nb_timers, _flags);            \
  }
TIM_ARM_FASTPATH_MODES
#undef FP

static __rte_always_inline uint16_t cnxk_tim_timer_arm_tmo_brst(
    const struct rte_event_timer_adapter *adptr, struct rte_event_timer **tim,
  ...
  while (arr_idx < nb_timers) {
    ...
    ret = cnxk_tim_add_entry_brst(tim_ring, timeout_tick, &tim[set_timers],
                                  entry, idx, flags);
    set_timers += ret;
    if (ret != idx)
      break;
  ...
  if (flags & CNXK_TIM_ENA_STATS)
    __atomic_fetch_add(&tim_ring->arm_cnt, set_timers, __ATOMIC_RELAXED);
...
#define FP(_name, _f2, _f1, _flags)                                            \
  uint16_t __rte_noinline cnxk_tim_arm_tmo_tick_burst_##_name(                 \
    ...
    return cnxk_tim_timer_arm_tmo_brst(adptr, tim, timeout_tick, nb_timers,    \
                                       _flags);                                \
  }
