...
cn9k_sso_hws_fwd_swtag(uint64_t base, const struct rte_event *ev) {
  const uint32_t tag = (uint32_t)ev->event;
  const uint8_t new_tt = ev->sched_type;
  const uint8_t cur_tt = CNXK_TT_FROM_TAG(plt_read64(base + SSOW_LF_GWS_TAG));
  ...
  if (new_tt == SSO_TT_UNTAGGED) {
    ...
    cnxk_sso_hws_swtag_norm(tag, new_tt, base + SSOW_LF_GWS_OP_SWTAG_NORM);
  }
}

static __rte_always_inline void
cn9k_sso_hws_new_event_wait(struct cn9k_sso_hws *ws,
...
static __rte_always_inline void
cn9k_sso_hws_forward_event(struct cn9k_sso_hws *ws,
                           const struct rte_event *ev) {
...
static __rte_always_inline uint8_t cn9k_sso_hws_dual_new_event(
    struct cn9k_sso_hws_dual *dws, const struct rte_event *ev) {
  const uint32_t tag = (uint32_t)ev->event;
  const uint8_t new_tt = ev->sched_type;
  const uint64_t event_ptr = ev->u64;
  const uint16_t grp = ev->queue_id;
  ...
  cnxk_sso_hws_add_work(event_ptr, tag, new_tt, dws->grp_base + (grp << 12));
  return 1;
}
...
static __rte_always_inline uint16_t cn9k_sso_hws_dual_get_work(
    uint64_t base, uint64_t pair_base, struct rte_event *ev,
    const uint32_t flags, struct cn9k_sso_hws_dual *dws) {
  union {
  ...
  asm volatile(
      ...
      "		sub %[mbuf], %[wqp], #0x80	\n"
      "		prfm pldl1keep, [%[mbuf]]	\n"
      : [tag] "=&r"(gw.u64[0]), [wqp] "=&r"(gw.u64[1]), [mbuf] "=&r"(mbuf)
  ...
  if (CNXK_TT_FROM_EVENT(gw.u64[0]) != SSO_TT_EMPTY) {
    if ((flags & CPT_RX_WQE_F) &&
        (CNXK_EVENT_TYPE_FROM_TAG(gw.u64[0]) == RTE_EVENT_TYPE_CRYPTODEV)) {
      gw.u64[1] = cn9k_cpt_crypto_adapter_dequeue(gw.u64[1]);
    } else if (CNXK_EVENT_TYPE_FROM_TAG(gw.u64[0]) == RTE_EVENT_TYPE_ETHDEV) {
      uint8_t port = CNXK_SUB_EVENT_FROM_TAG(gw.u64[0]);

      gw.u64[0] = CNXK_CLR_SUB_EVENT(gw.u64[0]);
      cn9k_wqe_to_mbuf(gw.u64[1], mbuf, port, gw.u64[0] & 0xFFFFF, flags,
                       dws->lookup_mem);
      /* Extracting tstamp, if PTP enabled*/
      tstamp_ptr = *(uint64_t *)(((struct nix_wqe_hdr_s *)gw.u64[1]) +
                                 CNXK_SSO_WQE_SG_PTR);
      cn9k_nix_mbuf_to_tstamp((struct rte_mbuf *)mbuf, dws->tstamp,
...
cn9k_sso_hws_get_work(struct cn9k_sso_hws *ws, struct rte_event *ev,
  ...
  asm volatile(
      ...
      "		prfm pldl1keep, [%[mbuf]]	\n"
      : [tag] "=&r"(gw.u64[0]), [wqp] "=&r"(gw.u64[1]), [mbuf] "=&r"(mbuf)
      : [tag_loc] "r"(ws->base + SSOW_LF_GWS_TAG), [wqp_loc] "r"(
                                                       ws->base +
                                                       SSOW_LF_GWS_WQP));
#else
  ...
  if (CNXK_TT_FROM_EVENT(gw.u64[0]) != SSO_TT_EMPTY) {
    if ((flags & CPT_RX_WQE_F) &&
      ...
      tstamp_ptr = *(uint64_t *)(((struct nix_wqe_hdr_s *)gw.u64[1]) +
                                 CNXK_SSO_WQE_SG_PTR);
      cn9k_nix_mbuf_to_tstamp((struct rte_mbuf *)mbuf, ws->tstamp,
                              flags & NIX_RX_OFFLOAD_TSTAMP_F,
                              (uint64_t *)tstamp_ptr);
      gw.u64[1] = mbuf;
    }
...
cn9k_sso_hws_get_work_empty(uint64_t base, struct rte_event *ev) {
  ...
  asm volatile(
      ...
      "		sub %[mbuf], %[wqp], #0x80	\n"
      : [tag] "=&r"(gw.u64[0]), [wqp] "=&r"(gw.u64[1]), [mbuf] "=&r"(mbuf)
      : [tag_loc] "r"(base + SSOW_LF_GWS_TAG), [wqp_loc] "r"(base +
                                                             SSOW_LF_GWS_WQP));
#else
  gw.u64[0] = plt_read64(base + SSOW_LF_GWS_TAG);
  while ((BIT_ULL(63)) & gw.u64[0])
  ...
  if (CNXK_TT_FROM_EVENT(gw.u64[0]) != SSO_TT_EMPTY) {
    if (CNXK_EVENT_TYPE_FROM_TAG(gw.u64[0]) == RTE_EVENT_TYPE_ETHDEV) {
      uint8_t port = CNXK_SUB_EVENT_FROM_TAG(gw.u64[0]);

      gw.u64[0] = CNXK_CLR_SUB_EVENT(gw.u64[0]);
      cn9k_wqe_to_mbuf(gw.u64[1], mbuf, port, gw.u64[0] & 0xFFFFF, 0, NULL);
      gw.u64[1] = mbuf;
    }
  }
...
#define R(name, flags)                                                         \
  uint16_t __rte_hot cn9k_sso_hws_deq_##name(void *port, struct rte_event *ev, \
...
#define SSO_DUAL_DEQ(fn, flags)                                                \
  uint16_t __rte_hot fn(void *port, struct rte_event *ev,                      \
    ...
    gw = cn9k_sso_hws_dual_get_work(dws->base[dws->vws], dws->base[!dws->vws], \
                                    ev, flags, dws);                           \
    dws->vws = !dws->vws;                                                      \
    return gw;                                                                 \
  }
...
cn9k_sso_hws_xtract_meta(struct rte_mbuf *m, uint64_t *txq_data) {
  return (struct cn9k_eth_txq *)(txq_data[(txq_data[m->port] >> 48) +
...
cn9k_sso_hws_xmit_sec_one(const struct cn9k_eth_txq *txq, uint64_t base,
                          struct rte_mbuf *m, uint64_t *cmd, uint32_t flags) {
  ...
  pkt_len -= l2_len;
  pkt_len +=
      sizeof(struct roc_onf_ipsec_outb_hdr) + ROC_ONF_IPSEC_OUTB_MAX_L2_INFO_SZ;
...
static inline void cn9k_sso_hws_xmit_sec_one(const struct cn9k_eth_txq *txq,
                                             uint64_t base, struct rte_mbuf *m,
                                             uint64_t *cmd, uint32_t flags) {
  RTE_SET_USED(txq);
  RTE_SET_USED(base);
  RTE_SET_USED(m);
  RTE_SET_USED(cmd);
...
cn9k_sso_hws_event_tx(uint64_t base, struct rte_event *ev, uint64_t *cmd,
  ...
  if (flags & NIX_TX_OFFLOAD_SECURITY_F) {
    ...
    if (ol_flags & RTE_MBUF_F_TX_SEC_OFFLOAD) {
      ...
      cn9k_sso_hws_xmit_sec_one(txq, ssow_base, m, cmd, flags);
      goto done;
    }
  ...
  if (flags & NIX_TX_MULTI_SEG_F) {
    ...
    cn9k_nix_xmit_prepare_tstamp(txq, cmd, m->ol_flags, segdw, flags);
    if (!CNXK_TT_FROM_EVENT(ev->event)) {
      ...
      if (cn9k_nix_xmit_submit_lmt(txq->io_addr) == 0)
        cn9k_nix_xmit_mseg_one(cmd, txq->lmt_addr, txq->io_addr, segdw);
    } else {
      cn9k_nix_xmit_mseg_one(cmd, txq->lmt_addr, txq->io_addr, segdw);
    }
  } else {
    ...
    if (!CNXK_TT_FROM_EVENT(ev->event)) {
      ...
      cn9k_sso_txq_fc_wait(txq);
      if (cn9k_nix_xmit_submit_lmt(txq->io_addr) == 0)
        cn9k_nix_xmit_one(cmd, txq->lmt_addr, txq->io_addr, flags);
    } else {
      cn9k_nix_xmit_one(cmd, txq->lmt_addr, txq->io_addr, flags);
    }
  }
...
#define SSO_TX(fn, sz, flags)                                                  \
  uint16_t __rte_hot fn(void *port, struct rte_event ev[],                     \
    ...
    return cn9k_sso_hws_event_tx(ws->base, &ev[0], cmd,                        \
                                 (uint64_t *)ws->tx_adptr_data, flags);        \
  }
