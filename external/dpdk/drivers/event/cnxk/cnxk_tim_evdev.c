...
cnxk_tim_chnk_pool_create(struct cnxk_tim_ring *tim_ring,
  ...
  if (!tim_ring->disable_npa) {
    ...
    tim_ring->chunk_pool = rte_mempool_create(
        pool_name, tim_ring->nb_chunks, tim_ring->chunk_sz, cache_sz, 0, NULL,
        NULL, NULL, NULL, rte_socket_id(), mp_flags);
    if (tim_ring->chunk_pool == NULL) {
      plt_err("Unable to create chunkpool.");
      return -ENOMEM;
    }
...
static void cnxk_tim_set_fp_ops(struct cnxk_tim_ring *tim_ring) {
...
cnxk_tim_ring_info_get(const struct rte_event_timer_adapter *adptr,
  ...
  adptr_info->min_resolution_ns =
      tim_ring->ena_periodic ? tim_ring->max_tout : tim_ring->tck_nsec;
  rte_memcpy(&adptr_info->conf, &adptr->data->conf,
             sizeof(struct rte_event_timer_adapter_conf));
}

static inline void sort_multi_array(double ref_arr[], uint64_t arr1[],
                                    uint64_t arr2[], uint64_t arr3[],
                                    uint8_t sz) {
  int x;
...
static inline void populate_sample(uint64_t tck[], uint64_t ns[], double diff[],
                                   uint64_t dst[], uint64_t req_tck,
                                   uint64_t clk_freq, double tck_ns, uint8_t sz,
                                   bool mov_fwd) {
  int i;
  ...
  for (i = 0; i < sz; i++) {
    ...
    do {
      ...
      if (round((double)tck[i] * tck_ns) > ((double)tck[i] * tck_ns))
        continue;
...
static void tim_adjust_resolution(uint64_t *req_ns, uint64_t *req_tck,
                                  double tck_ns, uint64_t clk_freq,
                                  uint64_t max_tmo, uint64_t m_tck) {
#define MAX_SAMPLES 5
  ...
  for (i = 0; i < MAX_SAMPLES; i++) {
    if (min_dst[i] < max_dst[i] && min_tck[i] > m_tck &&
        (max_tmo / min_ns[i]) <= (TIM_MAX_BUCKET_SIZE - TIM_MIN_BUCKET_SIZE)) {
      *req_tck = min_tck[i];
      *req_ns = min_ns[i];
      break;
...
static int cnxk_tim_ring_create(struct rte_event_timer_adapter *adptr) {
  ...
  tim_ring->tck_nsec = round(RTE_ALIGN_MUL_NEAR(
      (long double)rcfg->timer_tick_ns, cnxk_tim_ns_per_tck(clk_freq)));
  if (log10(clk_freq) - floor(log10(clk_freq)) != 0.0) {
    ...
    tim_adjust_resolution(&req_ns, &req_tck, tck_ns, clk_freq, rcfg->max_tmo_ns,
                          min_intvl_cyc);
  ...
  tim_ring->tck_int =
      round((long double)tim_ring->tck_nsec / cnxk_tim_ns_per_tck(clk_freq));
  tim_ring->tck_nsec = ceil(tim_ring->tck_int * cnxk_tim_ns_per_tck(clk_freq));
  ...
  if (tim_ring->disable_npa) {
    tim_ring->nb_chunks =
        tim_ring->nb_timers / CNXK_TIM_NB_CHUNK_SLOTS(tim_ring->chunk_sz);
  ...
  /* Create buckets. */
  tim_ring->bkt = rte_zmalloc("cnxk_tim_bucket",
  ...
  plt_tim_dbg("Total memory used %" PRIu64 "MB\n",
...
static int cnxk_tim_ring_free(struct rte_event_timer_adapter *adptr) {
  struct cnxk_tim_ring *tim_ring = adptr->data->adapter_priv;
  struct cnxk_tim_evdev *dev = cnxk_tim_priv_get();
...
static void cnxk_tim_calibrate_start_tsc(struct cnxk_tim_ring *tim_ring) {
#define CNXK_TIM_CALIB_ITER 1E6
  ...
  for (icount = 0; icount < CNXK_TIM_CALIB_ITER; icount++) {
    ...
    bucket = (bkt_cyc - tim_ring->ring_start_cyc) / tim_ring->tck_int;
    bucket = bucket % (tim_ring->nb_bkts);
    tim_ring->ring_start_cyc = bkt_cyc - (real_bkt * tim_ring->tck_int);
    if (bucket != real_bkt)
      ecount++;
  }
...
static int cnxk_tim_ring_start(const struct rte_event_timer_adapter *adptr) {
  struct cnxk_tim_ring *tim_ring = adptr->data->adapter_priv;
  struct cnxk_tim_evdev *dev = cnxk_tim_priv_get();
  int rc;
...
static int cnxk_tim_ring_stop(const struct rte_event_timer_adapter *adptr) {
  struct cnxk_tim_ring *tim_ring = adptr->data->adapter_priv;
  struct cnxk_tim_evdev *dev = cnxk_tim_priv_get();
  int rc;
...
static int cnxk_tim_stats_get(const struct rte_event_timer_adapter *adapter,
...
static int cnxk_tim_stats_reset(const struct rte_event_timer_adapter *adapter) {
  struct cnxk_tim_ring *tim_ring = adapter->data->adapter_priv;
...
int cnxk_tim_caps_get(const struct rte_eventdev *evdev, uint64_t flags,
                      uint32_t *caps,
...
static void cnxk_tim_parse_ring_param(char *value, void *opaque) {
  ...
  dev->ring_ctl_data = rte_realloc(
      dev->ring_ctl_data, sizeof(struct cnxk_tim_ctl) * dev->ring_ctl_cnt, 0);
...
static void cnxk_tim_parse_ring_ctl_list(const char *value, void *opaque) {
  char *s = strdup(value);
  char *start = NULL;
  char *end = NULL;
  char *f = s;
...
static int cnxk_tim_parse_kvargs_dict(const char *key, const char *value,
                                      void *opaque) {
  RTE_SET_USED(key);
...
static void cnxk_tim_parse_clk_list(const char *value, void *opaque) {
  enum roc_tim_clk_src src[] = {ROC_TIM_CLK_SRC_GPIO, ROC_TIM_CLK_SRC_PTP,
                                ROC_TIM_CLK_SRC_SYNCE, ROC_TIM_CLK_SRC_INVALID};
  struct cnxk_tim_evdev *dev = opaque;
...
static int cnxk_tim_parse_kvargs_dsv(const char *key, const char *value,
                                     void *opaque) {
  RTE_SET_USED(key);
...
static void cnxk_tim_parse_devargs(struct rte_devargs *devargs,
  ...
  rte_kvargs_process(kvlist, CNXK_TIM_RINGS_LMT, &parse_kvargs_value,
                     &dev->min_ring_cnt);
  rte_kvargs_process(kvlist, CNXK_TIM_RING_CTL, &cnxk_tim_parse_kvargs_dict,
                     &dev);
  rte_kvargs_process(kvlist, CNXK_TIM_EXT_CLK, &cnxk_tim_parse_kvargs_dsv, dev);
...
void cnxk_tim_init(struct roc_sso *sso) {
  ...
  if (dev->chunk_slots && dev->chunk_slots <= CNXK_TIM_MAX_CHUNK_SLOTS &&
    ...
    dev->chunk_sz = (dev->chunk_slots + 1) * CNXK_TIM_CHUNK_ALIGNMENT;
  } else {
