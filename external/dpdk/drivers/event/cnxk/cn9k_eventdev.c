...
static int cn9k_sso_hws_link(void *arg, void *port, uint16_t *map,
                             uint16_t nb_link) {
  ...
  if (dev->dual_ws) {
    dws = port;
    rc = roc_sso_hws_link(&dev->sso, CN9K_DUAL_WS_PAIR_ID(dws->hws_id, 0), map,
                          nb_link);
...
static int cn9k_sso_hws_unlink(void *arg, void *port, uint16_t *map,
                               uint16_t nb_link) {
  struct cnxk_sso_evdev *dev = arg;
  struct cn9k_sso_hws_dual *dws;
  struct cn9k_sso_hws *ws;
  ...
  if (dev->dual_ws) {
    dws = port;
    rc = roc_sso_hws_unlink(&dev->sso, CN9K_DUAL_WS_PAIR_ID(dws->hws_id, 0),
                            map, nb_link);
    rc |= roc_sso_hws_unlink(&dev->sso, CN9K_DUAL_WS_PAIR_ID(dws->hws_id, 1),
                             map, nb_link);
  } else {
    ws = port;
...
static void cn9k_sso_hws_setup(void *arg, void *hws, uintptr_t grp_base) {
  struct cnxk_sso_evdev *dev = arg;
  struct cn9k_sso_hws_dual *dws;
  struct cn9k_sso_hws *ws;
  uint64_t val;
...
static void cn9k_sso_hws_release(void *arg, void *hws) {
  struct cnxk_sso_evdev *dev = arg;
  struct cn9k_sso_hws_dual *dws;
  struct cn9k_sso_hws *ws;
  uint16_t i;
  ...
  if (dev->dual_ws) {
    ...
    for (i = 0; i < dev->nb_event_queues; i++) {
      roc_sso_hws_unlink(&dev->sso, CN9K_DUAL_WS_PAIR_ID(dws->hws_id, 0), &i,
                         1);
      roc_sso_hws_unlink(&dev->sso, CN9K_DUAL_WS_PAIR_ID(dws->hws_id, 1), &i,
                         1);
    }
...
static void cn9k_sso_hws_flush_events(void *hws, uint8_t queue_id,
                                      uintptr_t base, cnxk_handle_event_t fn,
                                      void *arg) {
  ...
  while (aq_cnt || cq_ds_cnt || ds_cnt) {
    ...
    if (ev.sched_type != SSO_TT_EMPTY)
      cnxk_sso_hws_swtag_flush(ws_base + SSOW_LF_GWS_TAG,
...
static void cn9k_sso_hws_reset(void *arg, void *hws) {
  struct cnxk_sso_evdev *dev = arg;
  struct cn9k_sso_hws_dual *dws;
  struct cn9k_sso_hws *ws;
  uint64_t pend_state;
...
static int cn9k_sso_rsrc_init(void *arg, uint8_t hws, uint8_t hwgrp) {
  struct cnxk_sso_evdev *dev = arg;
...
static int cn9k_sso_updt_tx_adptr_data(const struct rte_eventdev *event_dev) {
  ...
  for (i = 0; i < dev->nb_event_ports; i++) {
    if (dev->dual_ws) {
      struct cn9k_sso_hws_dual *dws = event_dev->data->ports[i];
      void *ws_cookie;
      ...
      if (ws_cookie == NULL)
        return -ENOMEM;
      dws = RTE_PTR_ADD(ws_cookie, sizeof(struct cnxk_sso_hws_cookie));
      memcpy(&dws->tx_adptr_data, dev->tx_adptr_data, dev->tx_adptr_data_sz);
...
static void cn9k_sso_fp_fns_set(struct rte_eventdev *event_dev) {
  struct cnxk_sso_evdev *dev = cnxk_sso_pmd_priv(event_dev);
  /* Single WS modes */
...
#undef T
  ...
  if (dev->rx_offloads & NIX_RX_MULTI_SEG_F) {
    ...
    CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst, sso_hws_deq_seg_burst);
    if (dev->is_timeout_deq) {
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_deq_tmo_seg);
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
                            sso_hws_deq_tmo_seg_burst);
    }
    if (dev->is_ca_internal_port) {
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_deq_ca_seg);
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
                            sso_hws_deq_ca_seg_burst);
    }

    if (dev->is_ca_internal_port && dev->is_timeout_deq) {
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_deq_tmo_ca_seg);
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
                            sso_hws_deq_tmo_ca_seg_burst);
    }
  } else {
    CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_deq);
    CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst, sso_hws_deq_burst);
    if (dev->is_timeout_deq) {
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_deq_tmo);
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
                            sso_hws_deq_tmo_burst);
    }
    if (dev->is_ca_internal_port) {
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_deq_ca);
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
                            sso_hws_deq_ca_burst);
    }

    if (dev->is_ca_internal_port && dev->is_timeout_deq) {
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_deq_tmo_ca);
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
  ...
  if (dev->tx_offloads & NIX_TX_MULTI_SEG_F)
    ...
    CN9K_SET_EVDEV_ENQ_OP(dev, event_dev->txa_enqueue, sso_hws_tx_adptr_enq);
  ...
  if (dev->dual_ws) {
    ...
    if (dev->rx_offloads & NIX_RX_MULTI_SEG_F) {
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_dual_deq_seg);
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
      ...
      if (dev->is_ca_internal_port) {
        CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_dual_deq_ca_seg);
        CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
      ...
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_dual_deq);
      CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
                            sso_hws_dual_deq_burst);
      if (dev->is_timeout_deq) {
        CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_dual_deq_tmo);
        CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
                              sso_hws_dual_deq_tmo_burst);
      }
      if (dev->is_ca_internal_port) {
        CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_dual_deq_ca);
        CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
                              sso_hws_dual_deq_ca_burst);
      }
      if (dev->is_ca_internal_port && dev->is_timeout_deq) {
        CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue, sso_hws_dual_deq_tmo_ca);
        CN9K_SET_EVDEV_DEQ_OP(dev, event_dev->dequeue_burst,
...
static void *cn9k_sso_init_hws_mem(void *arg, uint8_t port_id) {
  struct cnxk_sso_evdev *dev = arg;
  struct cn9k_sso_hws_dual *dws;
  struct cn9k_sso_hws *ws;
  void *data;
  ...
  if (dev->dual_ws) {
    dws = rte_zmalloc("cn9k_dual_ws",
                      sizeof(struct cn9k_sso_hws_dual) + RTE_CACHE_LINE_SIZE,
                      RTE_CACHE_LINE_SIZE);
    if (dws == NULL) {
    ...
    dws->base[0] =
        roc_sso_hws_base_get(&dev->sso, CN9K_DUAL_WS_PAIR_ID(port_id, 0));
    dws->base[1] =
        roc_sso_hws_base_get(&dev->sso, CN9K_DUAL_WS_PAIR_ID(port_id, 1));
    dws->hws_id = port_id;
    dws->swtag_req = 0;
    dws->vws = 0;
    if (dev->deq_tmo_ns)
    ...
    ws = rte_zmalloc("cn9k_ws",
                     sizeof(struct cn9k_sso_hws) + RTE_CACHE_LINE_SIZE,
                     RTE_CACHE_LINE_SIZE);
    if (ws == NULL) {
      plt_err("Failed to alloc memory for port=%d", port_id);
      return NULL;
...
static void cn9k_sso_info_get(struct rte_eventdev *event_dev,
...
static int cn9k_sso_dev_configure(const struct rte_eventdev *event_dev) {
  struct cnxk_sso_evdev *dev = cnxk_sso_pmd_priv(event_dev);
  int rc;
...
static int cn9k_sso_port_setup(struct rte_eventdev *event_dev, uint8_t port_id,
...
static void cn9k_sso_port_release(void *port) {
...
static int cn9k_sso_port_link(struct rte_eventdev *event_dev, void *port,
...
static int cn9k_sso_port_unlink(struct rte_eventdev *event_dev, void *port,
...
static int cn9k_sso_start(struct rte_eventdev *event_dev) {
  int rc;
  ...
  rc = cnxk_sso_start(event_dev, cn9k_sso_hws_reset, cn9k_sso_hws_flush_events);
  if (rc < 0)
    return rc;
...
static void cn9k_sso_stop(struct rte_eventdev *event_dev) {
  cnxk_sso_stop(event_dev, cn9k_sso_hws_reset, cn9k_sso_hws_flush_events);
}

static int cn9k_sso_close(struct rte_eventdev *event_dev) {
  return cnxk_sso_close(event_dev, cn9k_sso_hws_unlink);
}

static int cn9k_sso_selftest(void) {
  return cnxk_sso_selftest(RTE_STR(event_cn9k));
}

static int cn9k_sso_rx_adapter_caps_get(const struct rte_eventdev *event_dev,
...
static void cn9k_sso_set_priv_mem(const struct rte_eventdev *event_dev,
                                  void *lookup_mem, void *tstmp_info) {
  struct cnxk_sso_evdev *dev = cnxk_sso_pmd_priv(event_dev);
  int i;
...
static int cn9k_sso_rx_adapter_queue_add(
...
static int cn9k_sso_rx_adapter_queue_del(const struct rte_eventdev *event_dev,
                                         const struct rte_eth_dev *eth_dev,
                                         int32_t rx_queue_id) {
  int rc;
...
static int cn9k_sso_tx_adapter_caps_get(const struct rte_eventdev *dev,
                                        const struct rte_eth_dev *eth_dev,
                                        uint32_t *caps) {
  int ret;
...
static void cn9k_sso_txq_fc_update(const struct rte_eth_dev *eth_dev,
                                   int32_t tx_queue_id) {
  ...
  if (tx_queue_id < 0) {
    ...
    sq->nb_sqb_bufs_adj =
        sq->nb_sqb_bufs -
        RTE_ALIGN_MUL_CEIL(sq->nb_sqb_bufs, sqes_per_sqb) / sqes_per_sqb;
    if (cnxk_eth_dev->tx_offloads & RTE_ETH_TX_OFFLOAD_SECURITY)
      sq->nb_sqb_bufs_adj -= (cnxk_eth_dev->outb.nb_desc / (sqes_per_sqb - 1));
    txq->nb_sqb_bufs_adj = sq->nb_sqb_bufs_adj;
    txq->nb_sqb_bufs_adj = (70 * txq->nb_sqb_bufs_adj) / 100;
  }
}

static int cn9k_sso_tx_adapter_queue_add(uint8_t id,
...
static int cn9k_sso_tx_adapter_queue_del(uint8_t id,
                                         const struct rte_eventdev *event_dev,
                                         const struct rte_eth_dev *eth_dev,
                                         int32_t tx_queue_id) {
  int rc;
...
static int cn9k_crypto_adapter_caps_get(const struct rte_eventdev *event_dev,
...
static int cn9k_crypto_adapter_qp_add(const struct rte_eventdev *event_dev,
                                      const struct rte_cryptodev *cdev,
...
static int cn9k_crypto_adapter_qp_del(const struct rte_eventdev *event_dev,
                                      const struct rte_cryptodev *cdev,
                                      int32_t queue_pair_id) {
  CNXK_VALID_DEV_OR_ERR_RET(event_dev->dev, "event_cn9k");
  CNXK_VALID_DEV_OR_ERR_RET(cdev->device, "crypto_cn9k");
...
static int cn9k_sso_init(struct rte_eventdev *event_dev) {
  struct cnxk_sso_evdev *dev = cnxk_sso_pmd_priv(event_dev);
  int rc;
...
static int cn9k_sso_probe(struct rte_pci_driver *pci_drv,
                          struct rte_pci_device *pci_dev) {
  return rte_event_pmd_pci_probe(pci_drv, pci_dev,
                                 sizeof(struct cnxk_sso_evdev), cn9k_sso_init);
}
...
RTE_PMD_REGISTER_PARAM_STRING(event_cn9k, CNXK_SSO_XAE_CNT
                              "=<int>" CNXK_SSO_GGRP_QOS
                              "=<string>" CNXK_SSO_FORCE_BP
                              "=1" CN9K_SSO_SINGLE_WS "=1" CNXK_TIM_DISABLE_NPA
                              "=1" CNXK_TIM_CHNK_SLOTS
                              "=<int>" CNXK_TIM_RINGS_LMT
                              "=<int>" CNXK_TIM_STATS_ENA "=1");
