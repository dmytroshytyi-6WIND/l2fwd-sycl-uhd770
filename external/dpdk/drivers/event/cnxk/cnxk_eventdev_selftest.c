...
static int testsuite_setup(const char *eventdev_name) {
...
static inline int _eventdev_setup(int mode) {
  ...
  eventdev_test_mempool = rte_pktmbuf_pool_create(pool_name, MAX_EVENTS, 0, 0,
                                                  512, rte_socket_id());
...
static inline int wait_workers_to_join(const uint32_t *count) {
  ...
  while (__atomic_load_n(count, __ATOMIC_RELAXED)) {
    ...
    if (new_cycles - print_cycles > rte_get_timer_hz()) {
      plt_info("Events %d", __atomic_load_n(count, __ATOMIC_RELAXED));
...
static inline int launch_workers_and_wait(int (*main_thread)(void *),
...
static int worker_flow_based_pipeline(void *arg) {
  ...
  while (__atomic_load_n(total_events, __ATOMIC_RELAXED) > 0) {
    ...
    if (ev.sub_event_type == 0) {
      ...
      plt_err("Invalid ev.sub_event_type = %d", ev.sub_event_type);
      return -1;
    }
  }
  return 0;
...
static int test_multiport_queue_sched_type_test(uint8_t in_sched_type,
  ...
  if (queue_count < 2 || !nr_ports) {
    plt_err("Not enough queues=%d ports=%d or workers=%d", queue_count,
...
static void cnxk_test_run(int (*setup)(void), void (*tdown)(void),
...
static int cnxk_sso_testsuite_run(const char *dev_name) {
  int rc;

  testsuite_setup(dev_name);

  CNXK_TEST_RUN(eventdev_setup, eventdev_teardown, test_simple_enqdeq_ordered);
  CNXK_TEST_RUN(eventdev_setup, eventdev_teardown, test_simple_enqdeq_atomic);
  CNXK_TEST_RUN(eventdev_setup, eventdev_teardown, test_simple_enqdeq_parallel);
