...
#define CNXK_TIM_MAX_BURST (RTE_CACHE_LINE_SIZE / CNXK_TIM_CHUNK_ALIGNMENT)
...
static inline struct cnxk_tim_evdev *cnxk_tim_priv_get(void) {
...
static inline long double cnxk_tim_ns_per_tck(uint64_t freq) {
  return (long double)NSECPERSEC / freq;
}

#ifdef RTE_ARCH_ARM64
static inline uint64_t cnxk_tim_cntvct(void) {
  uint64_t tsc;
...
static inline uint64_t cnxk_tim_cntfrq(void) {
  uint64_t freq;
...
#else
static inline uint64_t cnxk_tim_cntvct(void) { return 0; }

static inline uint64_t cnxk_tim_cntfrq(void) { return 0; }
#endif
...
static inline int cnxk_tim_get_clk_freq(struct cnxk_tim_evdev *dev,
                                        enum roc_tim_clk_src clk_src,
                                        uint64_t *freq) {
  if (freq == NULL)
    return -EINVAL;
...
#define TIM_ARM_FASTPATH_MODES                                                 \
  ...
  FP(stats_sp, 1, 0, 0, CNXK_TIM_ENA_STATS | CNXK_TIM_ENA_DFB | CNXK_TIM_SP)   \
  FP(stats_mp, 1, 0, 1, CNXK_TIM_ENA_STATS | CNXK_TIM_ENA_DFB | CNXK_TIM_MP)   \
  FP(stats_fb_sp, 1, 1, 0, CNXK_TIM_ENA_STATS | CNXK_TIM_ENA_FB | CNXK_TIM_SP) \
  FP(stats_fb_mp, 1, 1, 1, CNXK_TIM_ENA_STATS | CNXK_TIM_ENA_FB | CNXK_TIM_MP)
