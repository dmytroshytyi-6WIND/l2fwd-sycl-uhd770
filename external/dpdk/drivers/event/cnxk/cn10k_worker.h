  ...
  while (non_vec) {
    ...
    if (flags & NIX_RX_OFFLOAD_SECURITY_F) {
      ...
      mbuf = nix_sec_meta_to_mbuf_sc(cq_w1, cq_w5, sa_base, laddr, &loff, mbuf,
                                     d_off, flags, mbuf_init);
    }

    cn10k_nix_cqe_to_mbuf(cqe, cqe->tag, mbuf, lookup_mem, mbuf_init, flags);
    /* Extracting tstamp, if PTP enabled*/
    tstamp_ptr =
        *(uint64_t *)(((struct nix_wqe_hdr_s *)cqe) + CNXK_SSO_WQE_SG_PTR);
...
cn10k_sso_hws_get_work(struct cn10k_sso_hws *ws, struct rte_event *ev,
  ...
  if (gw.u64[1]) {
    ...
    if ((flags & CPT_RX_WQE_F) &&
      ...
      cn10k_wqe_to_mbuf(gw.u64[1], mbuf, port, gw.u64[0] & 0xFFFFF, flags,
                        lookup_mem);
      /* Extracting tstamp, if PTP enabled*/
      ...
      *(uint64_t *)gw.u64[1] = (uint64_t)vwqe_hdr;
      cn10k_process_vwqe(gw.u64[1], port, flags, lookup_mem, ws->tstamp,
                         ws->lmt_base);
      /* Mark vector mempool object as get */
      RTE_MEMPOOL_CHECK_COOKIES(rte_mempool_from_obj((void *)gw.u64[1]),
                                (void **)&gw.u64[1], 1, 1);
    }
  }
...
cn10k_sso_hws_get_work_empty(struct cn10k_sso_hws *ws, struct rte_event *ev) {
  ...
  if (CNXK_TT_FROM_EVENT(gw.u64[0]) != SSO_TT_EMPTY) {
    if (CNXK_EVENT_TYPE_FROM_TAG(gw.u64[0]) == RTE_EVENT_TYPE_ETHDEV) {
      ...
      gw.u64[0] = CNXK_CLR_SUB_EVENT(gw.u64[0]);
      cn10k_wqe_to_mbuf(gw.u64[1], mbuf, port, gw.u64[0] & 0xFFFFF, 0, NULL);
      gw.u64[1] = mbuf;
    }
  }
...
cn10k_sso_hws_xtract_meta(struct rte_mbuf *m, const uint64_t *txq_data) {
  return (
      struct cn10k_eth_txq *)(txq_data[(txq_data[m->port] >> 48) +
...
cn10k_sso_tx_one(struct cn10k_sso_hws *ws, struct rte_mbuf *m, uint64_t *cmd,
  ...
  cn10k_nix_xmit_prepare(m, cmd, flags, txq->lso_tun_fmt, &sec, txq->mark_flag,
                         txq->mark_fmt);
  ...
  if (flags & NIX_TX_OFFLOAD_SECURITY_F && sec)
    cn10k_nix_prep_sec(m, cmd, &laddr, lmt_addr, &lnum, &loff, &shft,
                       txq->sa_base, flags);
...
cn10k_sso_vwqe_split_tx(struct cn10k_sso_hws *ws, struct rte_mbuf **mbufs,
  ...
  for (i = 0; i < pkts; i += NIX_DESCS_PER_LOOP) {
    ...
    if (((port[0] ^ port[1]) & (port[2] ^ port[3])) ||
      ...
      for (j = 0; j < 4; j++)
        cn10k_sso_tx_one(ws, mbufs[i + j], cmd, lmt_id, lmt_addr, sched_type,
                         txq_data, flags);
    } else {
      txq = (struct cn10k_eth_txq
                 *)(txq_data[(txq_data[port[0]] >> 48) + queue[0]] &
                    (BIT_ULL(48) - 1));
      cn10k_nix_xmit_pkts_vector(txq, (uint64_t *)ws, &mbufs[i], 4, cmd,
                                 flags | NIX_TX_VWQE_F);
    }
  }
  ...
  for (i = 0; i < scalar; i++) {
    cn10k_sso_tx_one(ws, mbufs[i], cmd, lmt_id, lmt_addr, sched_type, txq_data,
                     flags);
  }
}

static __rte_always_inline uint16_t cn10k_sso_hws_event_tx(
    struct cn10k_sso_hws *ws, struct rte_event *ev, uint64_t *cmd,
    const uint64_t *txq_data, const uint32_t flags) {
  struct cn10k_eth_txq *txq;
  struct rte_mbuf *m;
  uintptr_t lmt_addr;
  ...
  if (ev->event_type & RTE_EVENT_TYPE_VECTOR) {
    ...
    if (meta & BIT(31)) {
      ...
      cn10k_nix_xmit_pkts_vector(txq, (uint64_t *)ws, mbufs, meta & 0xFFFF, cmd,
                                 flags | NIX_TX_VWQE_F);
    } else {
      cn10k_sso_vwqe_split_tx(ws, mbufs, meta & 0xFFFF, cmd, lmt_id, lmt_addr,
...
#define SSO_TX(fn, sz, flags)                                                  \
  uint16_t __rte_hot fn(void *port, struct rte_event ev[],                     \
    ...
    uint64_t cmd[sz];                                                          \
    RTE_SET_USED(nb_events);                                                   \
    return cn10k_sso_hws_event_tx(ws, &ev[0], cmd,                             \
