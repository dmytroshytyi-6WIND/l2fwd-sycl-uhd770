...
static int crypto_adapter_qp_setup(const struct rte_cryptodev *cdev,
                                   struct cnxk_cpt_qp *qp) {
  char name[RTE_MEMPOOL_NAMESIZE];
  uint32_t cache_size, nb_req;
  unsigned int req_size;

  snprintf(name, RTE_MEMPOOL_NAMESIZE, "cnxk_ca_req_%u:%u", cdev->data->dev_id,
           qp->lf.lf_id);
  ...
  qp->ca.req_mp =
      rte_mempool_create(name, nb_req, req_size, cache_size, 0, NULL, NULL,
...
static int crypto_adapter_qp_free(struct cnxk_cpt_qp *qp) {
...
int cnxk_sso_xaq_allocate(struct cnxk_sso_evdev *dev) {
  ...
  return roc_sso_hwgrp_alloc_xaq(
      &dev->sso, roc_npa_aura_handle_to_aura(dev->sso.xaq.aura_handle),
      dev->nb_event_queues);
}
...
int cnxk_setup_event_ports(const struct rte_eventdev *event_dev,
  ...
  for (i = 0; i < dev->nb_event_ports; i++) {
    ...
    cnxk_sso_port_setup((struct rte_eventdev *)(uintptr_t)event_dev, i,
                        setup_hws_fn);
  }
...
static void cnxk_handle_event(void *arg, struct rte_event event) {
  struct rte_eventdev *event_dev = arg;

  if (event_dev->dev_ops->dev_stop_flush != NULL)
    event_dev->dev_ops->dev_stop_flush(event_dev->data->dev_id, event,
                                       event_dev->data->dev_stop_flush_arg);
}

static void cnxk_sso_cleanup(struct rte_eventdev *event_dev,
...
int cnxk_sso_close(struct rte_eventdev *event_dev,
...
static void parse_queue_param(char *value, void *opaque) {
  ...
  dev->qos_parse_data = rte_realloc(
      dev->qos_parse_data, sizeof(struct cnxk_sso_qos) * dev->qos_queue_cnt, 0);
...
static void parse_qos_list(const char *value, void *opaque) {
  char *s = strdup(value);
  char *start = NULL;
  char *end = NULL;
  char *f = s;
...
static int parse_sso_kvargs_dict(const char *key, const char *value,
                                 void *opaque) {
  RTE_SET_USED(key);
...
static void cnxk_sso_parse_devargs(struct cnxk_sso_evdev *dev,
  ...
  rte_kvargs_process(kvlist, CNXK_SSO_XAE_CNT, &parse_kvargs_value,
                     &dev->xae_cnt);
  rte_kvargs_process(kvlist, CNXK_SSO_GGRP_QOS, &parse_sso_kvargs_dict, dev);
...
int cnxk_sso_init(struct rte_eventdev *event_dev) {
  ...
  mz =
      rte_memzone_reserve(CNXK_SSO_MZ_NAME, sizeof(uint64_t), SOCKET_ID_ANY, 0);
  if (mz == NULL) {
