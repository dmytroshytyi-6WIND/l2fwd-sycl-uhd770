...
static CUresult CUDAAPI (*sym_cuGetProcAddress)(const char *symbol, void **pfn,
                                                int cudaVersion,
                                                uint64_t flags);
...
#define INTEL_GPU_UHD_13900K_DEVICE_ID (0xa780)
...
#define NVIDIA_GPU_RTX_4090_DEVICE_ID (0x2684)
...
static const struct rte_pci_id pci_id_cuda_map[] = {
    {RTE_PCI_DEVICE(INTEL_GPU_VENDOR_ID, INTEL_GPU_UHD_13900K_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_RTX_4090_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_A100_40GB_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_A100_80GB_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_A100_80GB_DPU_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_A30_24GB_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_A10_24GB_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_V100_32GB_SXM_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_V100_32GB_PCIE_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_V100_16GB_DEVICE_ID)},
    {RTE_PCI_DEVICE(NVIDIA_GPU_VENDOR_ID, NVIDIA_GPU_T4_16GB_DEVICE_ID)},
    {.device_id = 0}};
...
static int cuda_sym_func_loader(void) {
  if (cudalib == NULL)
    return -1;
  ...
  //	rte_cuda_log(ERR, "Failed to load CUDA missing symbol
  // cuGetProcAddress"); 	return -1;
  //}

  return 0;
}

static int cuda_pfn_func_loader(void) {
  CUresult res;

  res =
      sym_cuGetProcAddress("cuGetErrorString", (void **)(&pfn_cuGetErrorString),
  ...
  res = sym_cuGetProcAddress("cuGetErrorName", (void **)(&pfn_cuGetErrorName),
                             cuda_driver_version, 0);
  if (res != 0) {
  ...
  res = sym_cuGetProcAddress("cuPointerSetAttribute",
                             (void **)(&pfn_cuPointerSetAttribute),
                             cuda_driver_version, 0);
  if (res != 0) {
  ...
  res = sym_cuGetProcAddress("cuDeviceGetByPCIBusId",
                             (void **)(&pfn_cuDeviceGetByPCIBusId),
                             cuda_driver_version, 0);
  if (res != 0) {
  ...
  res = sym_cuGetProcAddress("cuDeviceGetName", (void **)(&pfn_cuDeviceGetName),
                             cuda_driver_version, 0);
  if (res != 0) {
  ...
  res = sym_cuGetProcAddress("cuDevicePrimaryCtxRetain",
                             (void **)(&pfn_cuDevicePrimaryCtxRetain),
                             cuda_driver_version, 0);
  if (res != 0) {
    rte_cuda_log(ERR, "Retrieve pfn_cuDevicePrimaryCtxRetain failed with %d",
                 res);
    return -1;
  }
  ...
  res = sym_cuGetProcAddress("cuDevicePrimaryCtxRelease",
                             ...
                             cuda_driver_version, 0);
  if (res != 0) {
    rte_cuda_log(ERR, "Retrieve pfn_cuDevicePrimaryCtxRelease failed with %d",
                 res);
    return -1;
  }

  res =
      sym_cuGetProcAddress("cuDeviceTotalMem", (void **)(&pfn_cuDeviceTotalMem),
                           cuda_driver_version, 0);
  if (res != 0) {
  ...
  res = sym_cuGetProcAddress("cuCtxGetDevice", (void **)(&pfn_cuCtxGetDevice),
                             cuda_driver_version, 0);
  if (res != 0) {
  ...
  res = sym_cuGetProcAddress("cuCtxSetCurrent", (void **)(&pfn_cuCtxSetCurrent),
                             cuda_driver_version, 0);
  if (res != 0) {
  ...
  res = sym_cuGetProcAddress("cuCtxGetCurrent", (void **)(&pfn_cuCtxGetCurrent),
                             cuda_driver_version, 0);
  if (res != 0) {
  ...
  res = sym_cuGetProcAddress("cuMemHostGetDevicePointer",
                             (void **)(&pfn_cuMemHostGetDevicePointer),
                             cuda_driver_version, 0);
  if (res != 0) {
    rte_cuda_log(ERR, "Retrieve pfn_cuMemHostGetDevicePointer failed with %d",
                 res);
    return -1;
  }

  res = sym_cuGetProcAddress("cuFlushGPUDirectRDMAWrites",
                             (void **)(&pfn_cuFlushGPUDirectRDMAWrites),
                             cuda_driver_version, 0);
  if (res != 0) {
    rte_cuda_log(ERR, "Retrieve cuFlushGPUDirectRDMAWrites failed with %d",
                 res);
    return -1;
  }
...
static cuda_ptr_key get_hash_from_ptr(void *ptr) { return (uintptr_t)ptr; }

static uint32_t mem_list_count_item(void) { return mem_alloc_list_last_elem; }

/* Initiate list of memory allocations if not done yet */
static struct mem_entry *mem_list_add_item(void) {
  /* Initiate list of memory allocations if not done yet */
  if (mem_alloc_list_head == NULL) {
    mem_alloc_list_head =
        rte_zmalloc(NULL, sizeof(struct mem_entry), RTE_CACHE_LINE_SIZE);
    if (mem_alloc_list_head == NULL) {
    ...
    struct mem_entry *mem_alloc_list_cur =
        rte_zmalloc(NULL, sizeof(struct mem_entry), RTE_CACHE_LINE_SIZE);
...
static struct mem_entry *mem_list_find_item(cuda_ptr_key pk) {
...
static int mem_list_del_item(cuda_ptr_key pk) {
  struct mem_entry *mem_alloc_list_cur = NULL;
...
static int cuda_dev_info_get(struct rte_gpu *dev, struct rte_gpu_info *info) {
  int ret = 0;
  CUresult res;
  ...
  if (dev->mpshared->info.parent != RTE_GPU_ID_NONE &&
    ...
    if (res != 0) {
      pfn_cuGetErrorString(res, &(err_string));
      rte_cuda_log(ERR, "cuCtxSetCurrent input failed with %s", err_string);
      rte_errno = EPERM;
      return -rte_errno;
    }
    ...
    /* MPS compatible */
    res =
        pfn_cuCtxGetExecAffinity(&affinityPrm, CU_EXEC_AFFINITY_TYPE_SM_COUNT);
    if (res != 0) {
      pfn_cuGetErrorString(res, &(err_string));
     ...
     */
    dev->mpshared->dev_private =
        rte_zmalloc(NULL, sizeof(struct cuda_info), RTE_CACHE_LINE_SIZE);
  ...
  private
    = (struct cuda_info *)dev->mpshared->dev_private;
    ...
    res = pfn_cuDeviceGetName(private->gpu_name, RTE_DEV_NAME_MAX_LEN,
                              private->cu_dev);
    if (res != 0) {
      pfn_cuGetErrorString(res, &(err_string));
    ...
    if (res != 0) {
      pfn_cuGetErrorString(res, &(err_string));
      rte_cuda_log(ERR, "cuCtxSetCurrent current failed with %s", err_string);
      rte_errno = EPERM;
      return -rte_errno;
    }
  }
...
static int cuda_mem_alloc(struct rte_gpu *dev, size_t size, unsigned int align,
                          void **ptr) {
  CUresult res;
  const char *err_string;
  CUcontext current_ctx;
  ...
  if (align && ((uintptr_t)mem_alloc_list_tail->ptr_d) % align)
    mem_alloc_list_tail->ptr_d +=
        (align - (((uintptr_t)mem_alloc_list_tail->ptr_d) % align));

  /* GPUDirect RDMA attribute required */
  res = pfn_cuPointerSetAttribute(&flag, CU_POINTER_ATTRIBUTE_SYNC_MEMOPS,
                                  mem_alloc_list_tail->ptr_d);
  if (res != 0) {
    rte_cuda_log(ERR,
                 "Could not set SYNC MEMOP attribute for "
  ...
  mem_alloc_list_tail->pkey =
      get_hash_from_ptr((void *)mem_alloc_list_tail->ptr_d);
...
static int cuda_mem_register(struct rte_gpu *dev, size_t size, void *ptr) {
  CUresult res;
  const char *err_string;
  CUcontext current_ctx;
  CUcontext input_ctx;
  ...
  res = pfn_cuMemHostRegister(
      mem_alloc_list_tail->ptr_h, mem_alloc_list_tail->size,
  ...
  res = pfn_cuDeviceGetAttribute(
      &(use_ptr_h), CU_DEVICE_ATTRIBUTE_CAN_USE_HOST_POINTER_FOR_REGISTERED_MEM,
      ((struct cuda_info *)(dev->mpshared->dev_private))->cu_dev);
  if (res != 0) {
    pfn_cuGetErrorString(res, &(err_string));
    rte_cuda_log(ERR, "cuDeviceGetAttribute failed with %s", err_string);
    rte_errno = EPERM;
    return -rte_errno;
  }
  ...
  if (use_ptr_h == 0) {
    ...
    if (res != 0) {
      pfn_cuGetErrorString(res, &(err_string));
      rte_cuda_log(ERR, "cuMemHostGetDevicePointer failed with %s", err_string);
      rte_errno = EPERM;
      return -rte_errno;
    }
    ...
    if ((uintptr_t)mem_alloc_list_tail->ptr_d !=
      ...
      rte_cuda_log(
          ERR, "Host input pointer is different wrt GPU registered pointer");
      rte_errno = ENOTSUP;
      return -rte_errno;
    }
  } else {
  ...
  if (res != 0) {
    rte_cuda_log(ERR,
                 "Could not set SYNC MEMOP attribute for GPU memory at %" PRIu32
                 ", err %d",
                 (uint32_t)mem_alloc_list_tail->ptr_d, res);
    rte_errno = EPERM;
    return -rte_errno;
  ...
  mem_alloc_list_tail->pkey =
      get_hash_from_ptr((void *)mem_alloc_list_tail->ptr_h);
  mem_alloc_list_tail->size = size;
  mem_alloc_list_tail->dev = dev;
  mem_alloc_list_tail->ctx =
      (CUcontext)((uintptr_t)dev->mpshared->info.context);
...
static int cuda_mem_cpu_map(struct rte_gpu *dev, __rte_unused size_t size,
                            void *ptr_in, void **ptr_out) {
  ...
  if (mem_item == NULL) {
    rte_cuda_log(ERR, "Memory address 0x%p not found in driver memory.",
                 ptr_in);
    rte_errno = EPERM;
    return -rte_errno;
  }
  ...
  if (mem_item->size != size)
    rte_cuda_log(WARNING,
                 "Can't expose memory area with size (%zd) different from "
                 "original size (%zd).",
                 size, mem_item->size);
...
static int cuda_mem_free(struct rte_gpu *dev, void *ptr) {
  CUresult res;
  struct mem_entry *mem_item;
  const char *err_string;
  cuda_ptr_key hk;
  ...
  if (mem_item->mtype == GPU_MEM) {
    ...
    if (res != 0) {
      pfn_cuGetErrorString(res, &(err_string));
      rte_cuda_log(ERR, "cuMemFree current failed with %s", err_string);
      rte_errno = EPERM;
      return -rte_errno;
    }
...
static int cuda_mem_unregister(struct rte_gpu *dev, void *ptr) {
  CUresult res;
  struct mem_entry *mem_item;
  const char *err_string;
  cuda_ptr_key hk;
...
static int cuda_mem_cpu_unmap(struct rte_gpu *dev, void *ptr_in) {
  struct mem_entry *mem_item;
  cuda_ptr_key hk;
...
static int cuda_dev_close(struct rte_gpu *dev) {
  if (dev == NULL)
    return -EINVAL;
...
static int cuda_wmb(struct rte_gpu *dev) {
  CUresult res;
  const char *err_string;
  CUcontext current_ctx;
  CUcontext input_ctx;
...
static int cuda_gpu_probe(__rte_unused struct rte_pci_driver *pci_drv,
  ...
  /* Processor count */
  res = pfn_cuDeviceGetAttribute(
      &(processor_count), CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT, cu_dev_id);
  if (res != 0) {
    pfn_cuGetErrorString(res, &(err_string));
    rte_cuda_log(ERR, "cuDeviceGetAttribute failed with %s", err_string);
    rte_errno = EPERM;
  ...
  if (res != 0) {
    pfn_cuGetErrorString(res, &(err_string));
    rte_cuda_log(ERR, "cuDeviceTotalMem failed with %s", err_string);
    rte_errno = EPERM;
    return -rte_errno;
  }
...
private
  ->cu_dev = cu_dev_id;
  res = pfn_cuDeviceGetName(private->gpu_name, RTE_DEV_NAME_MAX_LEN, cu_dev_id);
  if (res != 0) {
    pfn_cuGetErrorString(res, &(err_string));
    rte_cuda_log(ERR, "cuDeviceGetName failed with %s", err_string);
    rte_errno = EPERM;
  ...
  res = pfn_cuDeviceGetAttribute(
      &(private->gdr_write_ordering),
      CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_WRITES_ORDERING, cu_dev_id);
  if (res != 0) {
    pfn_cuGetErrorString(res, &(err_string));
    rte_cuda_log(ERR, "cuDeviceGetAttribute failed with %s", err_string);
    rte_errno = EPERM;
  ...
  if (private->gdr_write_ordering == CU_GPU_DIRECT_RDMA_WRITES_ORDERING_NONE) {
    res = pfn_cuDeviceGetAttribute(
        &(private->gdr_flush_type),
        CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_FLUSH_WRITES_OPTIONS, cu_dev_id);
    if (res != 0) {
      pfn_cuGetErrorString(res, &(err_string));
      rte_cuda_log(ERR, "cuDeviceGetAttribute failed with %s", err_string);
      rte_errno = EPERM;
  ...
  rte_cuda_debug("dev id = %u name = %s", dev->mpshared->info.dev_id,
                 private->gpu_name);

  return 0;
}

static int cuda_gpu_remove(struct rte_pci_device *pci_dev) {
  struct rte_gpu *dev;
  int ret;
  uint8_t gpu_id;
