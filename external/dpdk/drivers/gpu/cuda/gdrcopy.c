...
static int gdrcopy_open(gdr_t *g) {
  gdr_t g_;
...
int gdrcopy_pin(__rte_unused gdr_t *gdrc_h, __rte_unused gdr_mh_t *mh,
  ...
  return 0;
#else
  rte_cuda_log(ERR, "GDRCopy headers not provided at DPDK building time. Can't "
                    "CPU map GPU memory.");
  return -ENOTSUP;
#endif
}
