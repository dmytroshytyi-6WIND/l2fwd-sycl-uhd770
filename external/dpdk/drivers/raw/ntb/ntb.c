...
static inline void ntb_link_cleanup(struct rte_rawdev *dev) {
  struct ntb_hw *hw = dev->dev_private;
  int status, i;

  if (hw->ntb_ops->spad_write == NULL || hw->ntb_ops->mw_set_trans == NULL) {
...
static inline int ntb_handshake_work(const struct rte_rawdev *dev) {
  struct ntb_hw *hw = dev->dev_private;
  uint32_t val;
  int ret, i;
  ...
  for (i = 0; i < hw->mw_cnt; i++) {
    NTB_LOG(INFO, "Local %u mw size: 0x%" PRIx64 "", i, hw->mw_size[i]);
    val = hw->mw_size[i] >> 32;
    ret = (*hw->ntb_ops->spad_write)(dev, SPAD_MW0_SZ_H + 2 * i, 1, val);
    if (ret < 0)
      return ret;
    val = hw->mw_size[i];
    ret = (*hw->ntb_ops->spad_write)(dev, SPAD_MW0_SZ_L + 2 * i, 1, val);
    if (ret < 0)
      return ret;
  }
  ...
  if (ret < 0)
    return ret;
  ret = (*hw->ntb_ops->spad_write)(dev, SPAD_NUM_QPS, 1, hw->queue_pairs);
  if (ret < 0)
    return ret;
  ret = (*hw->ntb_ops->spad_write)(dev, SPAD_USED_MWS, 1, hw->used_mw_num);
  if (ret < 0)
    return ret;
  for (i = 0; i < hw->used_mw_num; i++) {
    val = (uint64_t)(size_t)(hw->mz[i]->addr) >> 32;
    ret = (*hw->ntb_ops->spad_write)(dev, SPAD_MW0_BA_H + 2 * i, 1, val);
    if (ret < 0)
      return ret;
    val = (uint64_t)(size_t)(hw->mz[i]->addr);
    ret = (*hw->ntb_ops->spad_write)(dev, SPAD_MW0_BA_L + 2 * i, 1, val);
    if (ret < 0)
      return ret;
  }

  for (i = 0; i < hw->used_mw_num; i++) {
    ret = (*hw->ntb_ops->mw_set_trans)(dev, i, hw->mz[i]->iova, hw->mz[i]->len);
    if (ret < 0)
      return ret;
  }
...
static void ntb_dev_intr_handler(void *param) {
  ...
  if (hw->ntb_ops->db_read == NULL || hw->ntb_ops->db_clear == NULL ||
  ...
  if (db_bits & 1) {
    ...
    for (i = 0; i < hw->mw_cnt; i++) {
      val_h = (*hw->ntb_ops->spad_read)(dev, SPAD_MW0_SZ_H + 2 * i, 0);
      val_l = (*hw->ntb_ops->spad_read)(dev, SPAD_MW0_SZ_L + 2 * i, 0);
      peer_mw_size = ((uint64_t)val_h << 32) | val_l;
      NTB_LOG(DEBUG, "Peer %u mw size: 0x%" PRIx64 "", i, peer_mw_size);
    ...
    NTB_LOG(INFO, "Link is up. Link speed: %u. Link width: %u", hw->link_speed,
            hw->link_width);
    return;
  }
...
static int ntb_queue_conf_get(struct rte_rawdev *dev, uint16_t queue_id,
...
static void ntb_rxq_release_mbufs(struct ntb_rx_queue *q) {
  int i;
...
static void ntb_rxq_release(struct ntb_rx_queue *rxq) {
  if (!rxq) {
...
static int ntb_rxq_setup(struct rte_rawdev *dev, uint16_t qp_id,
                         rte_rawdev_obj_t queue_conf, size_t conf_size) {
  ...
  rxq = rte_zmalloc_socket("ntb rx queue", sizeof(struct ntb_rx_queue),
  ...
  rxq->sw_ring = rte_zmalloc_socket(
      "ntb rx sw ring", sizeof(struct ntb_rx_entry) * rxq->nb_rx_desc,
      RTE_CACHE_LINE_SIZE, dev->socket_id);
  if (!rxq->sw_ring) {
    ntb_rxq_release(rxq);
    rxq = NULL;
...
static void ntb_txq_release_mbufs(struct ntb_tx_queue *q) {
  int i;
...
static void ntb_txq_release(struct ntb_tx_queue *txq) {
  if (!txq) {
...
static int ntb_txq_setup(struct rte_rawdev *dev, uint16_t qp_id,
                         rte_rawdev_obj_t queue_conf, size_t conf_size) {
  ...
  txq = rte_zmalloc_socket("ntb tx queue", sizeof(struct ntb_tx_queue),
                           RTE_CACHE_LINE_SIZE, dev->socket_id);
  if (!txq) {
    NTB_LOG(ERR, "Failed to allocate memory for "
                 "tx queue structure");
  ...
  txq->sw_ring = rte_zmalloc_socket(
      "ntb tx sw ring", sizeof(struct ntb_tx_entry) * txq->nb_tx_desc,
      RTE_CACHE_LINE_SIZE, dev->socket_id);
  if (!txq->sw_ring) {
    ntb_txq_release(txq);
    txq = NULL;
  ...
  txq->tx_free_thresh = txq_conf->tx_free_thresh ? txq_conf->tx_free_thresh
                                                 : NTB_DFLT_TX_FREE_THRESH;
  if (txq->tx_free_thresh >= txq->nb_tx_desc - 3) {
    NTB_LOG(ERR,
            "tx_free_thresh must be less than nb_desc - 3. "
            "(tx_free_thresh=%u qp_id=%u)",
            txq->tx_free_thresh, qp_id);
    return -EINVAL;
  }
...
static int ntb_queue_setup(struct rte_rawdev *dev, uint16_t queue_id,
                           rte_rawdev_obj_t queue_conf, size_t conf_size) {
  struct ntb_hw *hw = dev->dev_private;
  int ret;
...
static int ntb_queue_release(struct rte_rawdev *dev, uint16_t queue_id) {
  struct ntb_hw *hw = dev->dev_private;
...
static uint16_t ntb_queue_count(struct rte_rawdev *dev) {
  struct ntb_hw *hw = dev->dev_private;
  return hw->queue_pairs;
}

static int ntb_queue_init(struct rte_rawdev *dev, uint16_t qp_id) {
  struct ntb_hw *hw = dev->dev_private;
  ...
  local_hdr =
      (volatile struct ntb_header *)((size_t)hw->mz[0]->addr + hdr_offset);
  bar_addr = (*hw->ntb_ops->get_peer_mw_addr)(dev, 0);
  if (bar_addr == NULL)
    return -EINVAL;
  remote_hdr = (struct ntb_header *)((size_t)bar_addr + hdr_offset);

  /* rxq init. */
  rxq->rx_desc_ring = (struct ntb_desc *)(&remote_hdr->desc_ring);
  rxq->rx_used_ring =
      (volatile struct ntb_used *)(&local_hdr->desc_ring[q_size]);
  ...
  *rxq->avail_cnt = rxq->nb_rx_desc - 1;
  ...
  /* txq init */
  txq->tx_desc_ring = (volatile struct ntb_desc *)(&local_hdr->desc_ring);
  txq->tx_used_ring = (struct ntb_used *)(&remote_hdr->desc_ring[q_size]);
...
static inline void ntb_enqueue_cleanup(struct ntb_tx_queue *txq) {
  ...
  nb_to_clean = (*txq->avail_cnt - txq->last_avail + 1 + txq->nb_tx_desc) &
                (txq->nb_tx_desc - 1);
...
static int ntb_enqueue_bufs(struct rte_rawdev *dev,
  ...
  for (nb_tx = 0; nb_tx < count; nb_tx++) {
    ...
    tx_last = (txq->last_used + txm->nb_segs - 1) & (txq->nb_tx_desc - 1);
    nb_segs = txm->nb_segs;
    for (i = 0; i < nb_segs; i++) {
      ...
      if (txm->data_len > tx_item->len) {
        NTB_LOG(ERR,
                "Data length exceeds buf length."
      ...
      rte_memcpy(buf_addr, rte_pktmbuf_mtod(txm, void *), txm->data_len);

      tx_used[nb_mbufs].len = txm->data_len;
      tx_used[nb_mbufs++].flags =
          (txq->last_used == tx_last) ? NTB_FLAG_EOP : 0;
      ...
      sw_ring[txq->last_used].next_id =
          (txq->last_used + 1) & (txq->nb_tx_desc - 1);
      sw_ring[txq->last_used].last_id = tx_last;
      txq->last_used = (txq->last_used + 1) & (txq->nb_tx_desc - 1);
    }
...
end_of_tx:
  if (nb_tx) {
    ...
    rte_memcpy(txq->tx_used_ring, tx_used + nb1, sizeof(struct ntb_used) * nb2);
    rte_wmb();
...
static int ntb_dequeue_bufs(struct rte_rawdev *dev,
                            struct rte_rawdev_buf **buffers, unsigned int count,
                            rte_rawdev_obj_t context) {
  struct ntb_hw *hw = dev->dev_private;
  ...
  for (nb_rx = 0; nb_rx < count; nb_rx++) {
    ...
    while (true) {
      ...
      rxq->last_used = (rxq->last_used + 1) & (rxq->nb_rx_desc - 1);
      ...
      /* fill new desc */
      rx_desc[nb_mbufs].addr = rte_pktmbuf_mtod(rxm_t, size_t);
      rx_desc[nb_mbufs++].len = rxm_t->buf_len - RTE_PKTMBUF_HEADROOM;
      rxq->last_avail = (rxq->last_avail + 1) & (rxq->nb_rx_desc - 1);
...
end_of_rx:
  if (nb_rx) {
    ...
    rte_memcpy(rxq->rx_desc_ring, rx_desc + nb1, sizeof(struct ntb_desc) * nb2);
    rte_wmb();
...
static int ntb_dev_info_get(struct rte_rawdev *dev, rte_rawdev_obj_t dev_info,
   ...
   */
  info->mw_size_align =
      (uint8_t)(hw->pci_dev->id.vendor_id == NTB_INTEL_VENDOR_ID);
  ...
  hw->hdr_size_per_queue = RTE_ALIGN(
      sizeof(struct ntb_header) + hw->queue_size * sizeof(struct ntb_desc) +
...
static int ntb_dev_configure(const struct rte_rawdev *dev,
  ...
  hw->mz = conf->mz_list;
  hw->rx_queues = rte_zmalloc(
      "ntb_rx_queues", sizeof(struct ntb_rx_queue *) * hw->queue_pairs, 0);
  hw->tx_queues = rte_zmalloc(
      "ntb_tx_queues", sizeof(struct ntb_tx_queue *) * hw->queue_pairs, 0);
  /* First total stats, then per queue stats. */
  xstats_num = (hw->queue_pairs + 1) * NTB_XSTATS_NUM;
  hw->ntb_xstats = rte_zmalloc("ntb_xstats", xstats_num * sizeof(uint64_t), 0);
  hw->ntb_xstats_off =
      rte_zmalloc("ntb_xstats_off", xstats_num * sizeof(uint64_t), 0);
...
static int ntb_dev_start(struct rte_rawdev *dev) {
  struct ntb_hw *hw = dev->dev_private;
  ...
  hw->peer_mw_base =
      rte_zmalloc("ntb_peer_mw_base", hw->mw_cnt * sizeof(uint64_t), 0);
  ...
  if (peer_val != hw->queue_pairs) {
    NTB_LOG(ERR,
            "Inconsistent number of queues! (local: %u peer:"
            " %u)",
  ...
  for (i = 0; i < hw->peer_used_mws; i++) {
    peer_base_h = (*hw->ntb_ops->spad_read)(dev, SPAD_MW0_BA_H + 2 * i, 0);
    peer_base_l = (*hw->ntb_ops->spad_read)(dev, SPAD_MW0_BA_L + 2 * i, 0);
...
static void ntb_dev_stop(struct rte_rawdev *dev) {
  struct ntb_hw *hw = dev->dev_private;
  uint32_t time_out;
  int status, i;
...
clean:
  ...
  status = (*hw->ntb_ops->db_set_mask)(dev, (((uint64_t)1 << hw->db_cnt) - 1));
  if (status)
...
static int ntb_dev_close(struct rte_rawdev *dev) {
  struct ntb_hw *hw = dev->dev_private;
  ...
  rte_intr_callback_unregister(intr_handle, ntb_dev_intr_handler, dev);

  return 0;
}

static int ntb_dev_reset(struct rte_rawdev *rawdev __rte_unused) { return 0; }

static int ntb_attr_set(struct rte_rawdev *dev, const char *attr_name,
                        uint64_t attr_value) {
  struct ntb_hw *hw;
  int index;
  ...
  if (!strncmp(attr_name, NTB_SPAD_USER, NTB_SPAD_USER_LEN)) {
    ...
    (*hw->ntb_ops->spad_write)(dev, hw->spad_user_list[index], 1, attr_value);
    NTB_LOG(DEBUG, "Set attribute (%s) Value (%" PRIu64 ")", attr_name,
            attr_value);
    return 0;
  }
...
static int ntb_attr_get(struct rte_rawdev *dev, const char *attr_name,
                        uint64_t *attr_value) {
  struct ntb_hw *hw;
  int index;
  ...
  if (!strncmp(attr_name, NTB_TOPO_NAME, NTB_ATTR_NAME_LEN)) {
    *attr_value = hw->topo;
    NTB_LOG(DEBUG, "Attribute (%s) Value (%" PRIu64 ")", attr_name,
            *attr_value);
    return 0;
  }
  ...
  if (!strncmp(attr_name, NTB_SPAD_USER, NTB_SPAD_USER_LEN)) {
    ...
    *attr_value = (*hw->ntb_ops->spad_read)(dev, hw->spad_user_list[index], 0);
    NTB_LOG(DEBUG, "Attribute (%s) Value (%" PRIu64 ")", attr_name,
            *attr_value);
    return 0;
  }
...
static inline uint64_t ntb_stats_update(uint64_t offset, uint64_t stat) {
  if (stat >= offset)
    return (stat - offset);
  else
...
static int ntb_xstats_get(const struct rte_rawdev *dev,
                          const unsigned int ids[], uint64_t values[],
                          unsigned int n) {
  struct ntb_hw *hw = dev->dev_private;
  ...
  for (i = 0; i < NTB_XSTATS_NUM; i++) {
    ...
    for (j = 0; j < hw->queue_pairs; j++) {
      ...
      hw->ntb_xstats[i] +=
          ntb_stats_update(hw->ntb_xstats_off[off], hw->ntb_xstats[off]);
    }
  }
  ...
  for (i = 0; i < n && ids[i] < xstats_num; i++) {
    if (ids[i] < NTB_XSTATS_NUM)
      ...
      values[i] =
          ntb_stats_update(hw->ntb_xstats_off[ids[i]], hw->ntb_xstats[ids[i]]);
  }

  return i;
}

static int ntb_xstats_get_names(const struct rte_rawdev *dev,
  ...
  for (i = 0; i < hw->queue_pairs; i++) {
    for (j = 0; j < NTB_XSTATS_NUM; j++) {
      ...
      snprintf(xstats_names[off].name, sizeof(xstats_names[0].name), "%s_q%u",
               ntb_xstats_names[j].name, i);
    }
  }
...
static uint64_t ntb_xstats_get_by_name(const struct rte_rawdev *dev,
  ...
  xstats_num = NTB_XSTATS_NUM * (hw->queue_pairs + 1);
  xstats_names = rte_zmalloc(
      "ntb_stats_name", sizeof(struct rte_rawdev_xstats_name) * xstats_num, 0);
  ...
  for (i = 0; i < xstats_num; i++) {
    if (!strncmp(name, xstats_names[i].name, RTE_RAW_DEV_XSTATS_NAME_SIZE)) {
      *id = i;
      rte_free(xstats_names);
      if (i < NTB_XSTATS_NUM)
        return hw->ntb_xstats[i];
      else
        return ntb_stats_update(hw->ntb_xstats_off[i], hw->ntb_xstats[i]);
    }
  }
...
static int ntb_xstats_reset(struct rte_rawdev *dev, const uint32_t ids[],
                            uint32_t nb_ids) {
  struct ntb_hw *hw = dev->dev_private;
  uint32_t i, j, off, xstats_num;
...
static int ntb_init_hw(struct rte_rawdev *dev, struct rte_pci_device *pci_dev) {
  struct ntb_hw *hw = dev->dev_private;
  struct rte_intr_handle *intr_handle;
  int ret, i;
  ...
  rte_intr_callback_register(intr_handle, ntb_dev_intr_handler, dev);
  ...
  if (hw->ntb_ops->db_set_mask == NULL || hw->ntb_ops->peer_db_set == NULL) {
    NTB_LOG(ERR, "Doorbell is not supported.");
    return -ENOTSUP;
  }
  hw->db_mask = 0;
...
static int ntb_create(struct rte_pci_device *pci_dev, int socket_id) {
  ...
  snprintf(name, RTE_RAWDEV_NAME_MAX_LEN, "NTB:%x:%02x.%x", pci_dev->addr.bus,
           pci_dev->addr.devid, pci_dev->addr.function);

  NTB_LOG(INFO, "Init %s on NUMA node %d", name, socket_id);

  /* Allocate device structure. */
  rawdev = rte_rawdev_pmd_allocate(name, sizeof(struct ntb_hw), socket_id);
  if (rawdev == NULL) {
...
static int ntb_destroy(struct rte_pci_device *pci_dev) {
  char name[RTE_RAWDEV_NAME_MAX_LEN];
...
static int ntb_probe(struct rte_pci_driver *pci_drv __rte_unused,
                     struct rte_pci_device *pci_dev) {
  return ntb_create(pci_dev, rte_socket_id());
}

static int ntb_remove(struct rte_pci_device *pci_dev) {
