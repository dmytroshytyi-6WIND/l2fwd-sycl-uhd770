...
struct ntb_dev_ops {
  ...
  int (*mw_set_trans)(const struct rte_rawdev *dev, int mw_idx, uint64_t addr,
                      uint64_t size);
  ...
  uint32_t (*spad_read)(const struct rte_rawdev *dev, int spad, bool peer);
  int (*spad_write)(const struct rte_rawdev *dev, int spad, bool peer,
                    uint32_t spad_v);
  ...
  int (*vector_bind)(const struct rte_rawdev *dev, uint8_t intr, uint8_t msix);
};
