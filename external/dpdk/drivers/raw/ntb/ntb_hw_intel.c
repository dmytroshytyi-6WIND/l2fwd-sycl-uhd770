...
static inline int is_gen3_ntb(const struct ntb_hw *hw) {
...
static inline int is_gen4_ntb(const struct ntb_hw *hw) {
...
static int intel_ntb3_check_ppd(struct ntb_hw *hw) {
  uint8_t reg_val;
  int ret;

  ret = rte_pci_read_config(hw->pci_dev, &reg_val, sizeof(reg_val),
                            XEON_PPD_OFFSET);
  if (ret < 0) {
...
static int intel_ntb4_check_ppd(struct ntb_hw *hw) {
  uint32_t reg_val;
...
static int intel_ntb_dev_init(const struct rte_rawdev *dev) {
  ...
  hw->mw_size = rte_zmalloc("ntb_mw_size", hw->mw_cnt * sizeof(uint64_t), 0);
  if (hw->mw_size == NULL) {
...
static void *intel_ntb_get_peer_mw_addr(const struct rte_rawdev *dev,
                                        int mw_idx) {
  struct ntb_hw *hw = dev->dev_private;
  uint8_t bar;
  ...
  if (mw_idx < 0 || mw_idx >= hw->mw_cnt) {
    NTB_LOG(ERR, "Invalid memory window index (0 - %u).", hw->mw_cnt - 1);
    return 0;
  }
...
static int intel_ntb_mw_set_trans(const struct rte_rawdev *dev, int mw_idx,
  ...
  if (is_gen3_ntb(hw)) {
    ...
    xlat_off =
        XEON_GEN4_IM1XBASEIDX_OFFSET + mw_idx * XEON_GEN4_XBASEIDX_INTERVAL;
    xlat_addr = hw->hw_addr + xlat_off;
    rte_write16(rte_log2_u64(size), xlat_addr);
  } else {
    NTB_LOG(ERR,
            "Cannot set translation of memory windows for unsupported device.");
...
static void *intel_ntb_ioremap(const struct rte_rawdev *dev, uint64_t addr) {
  struct ntb_hw *hw = dev->dev_private;
  void *mapped = NULL;
  void *base;
  int i;
  ...
  for (i = 0; i < hw->peer_used_mws; i++) {
    if (addr >= hw->peer_mw_base[i] &&
      ...
      mapped = (void *)(size_t)(addr - hw->peer_mw_base[i] + (size_t)base);
      break;
    }
  }
...
static int intel_ntb_get_link_status(const struct rte_rawdev *dev) {
  struct ntb_hw *hw = dev->dev_private;
  ...
  if (is_gen3_ntb(hw)) {
    ...
    ret = rte_pci_read_config(hw->pci_dev, &reg_val, sizeof(reg_val), reg_off);
    if (ret < 0) {
...
static int intel_ntb_gen3_set_link(const struct ntb_hw *hw, bool up) {
...
static int intel_ntb_gen4_set_link(const struct ntb_hw *hw, bool up) {
...
static int intel_ntb_set_link(const struct rte_rawdev *dev, bool up) {
  struct ntb_hw *hw = dev->dev_private;
  int ret = 0;
...
static uint32_t intel_ntb_spad_read(const struct rte_rawdev *dev, int spad,
                                    bool peer) {
  struct ntb_hw *hw = dev->dev_private;
  ...
  if (is_gen3_ntb(hw))
    reg_off = peer ? XEON_GEN3_B2B_SPAD_OFFSET : XEON_IM_SPAD_OFFSET;
  else if (is_gen4_ntb(hw))
    reg_off = peer ? XEON_GEN4_B2B_SPAD_OFFSET : XEON_IM_SPAD_OFFSET;
  else {
...
static int intel_ntb_spad_write(const struct rte_rawdev *dev, int spad,
...
static uint64_t intel_ntb_db_read(const struct rte_rawdev *dev) {
  struct ntb_hw *hw = dev->dev_private;
...
static int intel_ntb_db_clear(const struct rte_rawdev *dev, uint64_t db_bits) {
  struct ntb_hw *hw = dev->dev_private;
  uint64_t db_off;
  void *db_addr;
  ...
  if (is_gen4_ntb(hw))
    rte_write16(XEON_GEN4_SLOTSTS_DLLSCS, hw->hw_addr + XEON_GEN4_SLOTSTS);
  rte_write64(db_bits, db_addr);

  return 0;
}

static int intel_ntb_db_set_mask(const struct rte_rawdev *dev,
                                 uint64_t db_mask) {
  struct ntb_hw *hw = dev->dev_private;
  uint64_t db_m_off;
  void *db_m_addr;
...
static int intel_ntb_peer_db_set(const struct rte_rawdev *dev, uint8_t db_idx) {
  struct ntb_hw *hw = dev->dev_private;
  uint32_t db_off;
  void *db_addr;
...
static int intel_ntb_vector_bind(const struct rte_rawdev *dev, uint8_t intr,
                                 uint8_t msix) {
  struct ntb_hw *hw = dev->dev_private;
  uint8_t reg_off;
  void *reg_addr;
