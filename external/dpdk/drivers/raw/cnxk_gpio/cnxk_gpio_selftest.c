...
#define OTX_IOC_TRIGGER_GPIO_HANDLER _IO(OTX_IOC_MAGIC, 3)

static int fd;

static int cnxk_gpio_attr_exists(const char *attr) {
  struct stat st;
...
#define CNXK_GPIO_ERR_STR(err, str, ...)                                       \
  do {                                                                         \
    if (err) {                                                                 \
...
static int cnxk_gpio_validate_attr(char *attr, const char *expected) {
...
static int cnxk_gpio_test_input(uint16_t dev_id, int base, int gpio) {
  char buf[CNXK_GPIO_BUFSZ];
  int ret, n;
...
static int cnxk_gpio_trigger_irq(int gpio) {
  int ret;
...
static void cnxk_gpio_irq_handler(int gpio, void *data) { *(int *)data = gpio; }

static int cnxk_gpio_test_irq(uint16_t dev_id, int gpio) {
  int irq_data, ret;
...
static int cnxk_gpio_test_output(uint16_t dev_id, int base, int gpio) {
  char buf[CNXK_GPIO_BUFSZ];
  int ret, val, n;
...
int cnxk_gpio_selftest(uint16_t dev_id) {
  ...
  for (i = 0; i < queues; i++) {
    ...
    if (ret) {
      RTE_LOG(ERR, PMD, "failed to read queue configuration (%d)\n", ret);
      goto out;
    }
