...
#define CNXK_GPIO_ARG_ALLOWLIST "allowlist"
    CNXK_GPIO_ARG_ALLOWLIST, NULL};

static char *allowlist;

static void cnxk_gpio_format_name(char *name, size_t len) {
  snprintf(name, len, "cnxk_gpio");
}

static int cnxk_gpio_filter_gpiochip(const struct dirent *dirent) {
  const char *pattern = "gpiochip";

  return !strncmp(dirent->d_name, pattern, strlen(pattern));
}

static void cnxk_gpio_set_defaults(struct cnxk_gpiochip *gpiochip) {
...
static int cnxk_gpio_parse_arg_gpiochip(const char *key __rte_unused,
...
static int cnxk_gpio_parse_arg_allowlist(const char *key __rte_unused,
                                         const char *value,
...
static int cnxk_gpio_parse_args(struct cnxk_gpiochip *gpiochip,
                                const char *args) {
  ...
  if (ret == 1) {
    ret = rte_kvargs_process(kvlist, CNXK_GPIO_ARG_GPIOCHIP,
                             cnxk_gpio_parse_arg_gpiochip, &gpiochip->num);
    if (ret)
      goto out;
  }
...
static int cnxk_gpio_parse_allowlist(struct cnxk_gpiochip *gpiochip) {
...
static int cnxk_gpio_read_attr(char *attr, char *val) {
  int ret, ret2;
  FILE *fp;
...
static int cnxk_gpio_read_attr_int(char *attr, int *val) {
...
static int cnxk_gpio_write_attr(const char *attr, const char *val) {
  FILE *fp;
  int ret;
...
static int cnxk_gpio_write_attr_int(const char *attr, int val) {
  char buf[CNXK_GPIO_BUFSZ];
...
static bool cnxk_gpio_queue_valid(struct cnxk_gpiochip *gpiochip,
                                  uint16_t queue) {
  return queue < gpiochip->num_queues;
}

static int cnxk_queue_to_gpio(struct cnxk_gpiochip *gpiochip, uint16_t queue) {
  return gpiochip->allowlist ? gpiochip->allowlist[queue] : queue;
}

static struct cnxk_gpio *cnxk_gpio_lookup(struct cnxk_gpiochip *gpiochip,
                                          uint16_t queue) {
...
static int cnxk_gpio_queue_setup(struct rte_rawdev *dev, uint16_t queue_id,
...
static int cnxk_gpio_queue_release(struct rte_rawdev *dev, uint16_t queue_id) {
  struct cnxk_gpiochip *gpiochip = dev->dev_private;
  char buf[CNXK_GPIO_BUFSZ];
  struct cnxk_gpio *gpio;
  int num, ret;
...
static int cnxk_gpio_queue_def_conf(struct rte_rawdev *dev, uint16_t queue_id,
                                    rte_rawdev_obj_t queue_conf,
                                    size_t queue_conf_size) {
  struct cnxk_gpiochip *gpiochip = dev->dev_private;
...
static uint16_t cnxk_gpio_queue_count(struct rte_rawdev *dev) {
  struct cnxk_gpiochip *gpiochip = dev->dev_private;
...
static const char *cnxk_gpio_edge_to_name(enum cnxk_gpio_pin_edge edge) {
  unsigned int i;
...
static enum cnxk_gpio_pin_edge cnxk_gpio_name_to_edge(const char *name) {
  unsigned int i;
...
static const char *cnxk_gpio_dir_to_name(enum cnxk_gpio_pin_dir dir) {
  unsigned int i;
...
static enum cnxk_gpio_pin_dir cnxk_gpio_name_to_dir(const char *name) {
  unsigned int i;
...
static int cnxk_gpio_register_irq(struct cnxk_gpio *gpio,
...
static int cnxk_gpio_unregister_irq(struct cnxk_gpio *gpio) {
  return cnxk_gpio_irq_free(gpio->num - gpio->gpiochip->base);
}

static int cnxk_gpio_process_buf(struct cnxk_gpio *gpio,
                                 struct rte_rawdev_buf *rbuf) {
  ...
  n = snprintf(buf, sizeof(buf), "%s/gpio%d", CNXK_GPIO_CLASS_PATH, gpio->num);
...
static bool cnxk_gpio_valid(struct cnxk_gpiochip *gpiochip, int gpio) {
  return gpio < gpiochip->num_gpios && gpiochip->gpios[gpio];
}

static int cnxk_gpio_enqueue_bufs(struct rte_rawdev *dev,
...
static int cnxk_gpio_dequeue_bufs(struct rte_rawdev *dev,
                                  struct rte_rawdev_buf **buffers,
                                  unsigned int count,
                                  rte_rawdev_obj_t context) {
  struct cnxk_gpiochip *gpiochip = dev->dev_private;
...
static int cnxk_gpio_dev_close(struct rte_rawdev *dev) {
  RTE_SET_USED(dev);
...
static int cnxk_gpio_probe(struct rte_vdev_device *dev) {
  ...
  gpiochip->gpios =
      rte_calloc(NULL, gpiochip->num_gpios, sizeof(struct cnxk_gpio *), 0);
  if (!gpiochip->gpios) {
...
static int cnxk_gpio_remove(struct rte_vdev_device *dev) {
  char name[RTE_RAWDEV_NAME_MAX_LEN];
  struct cnxk_gpiochip *gpiochip;
  struct rte_rawdev *rawdev;
  struct cnxk_gpio *gpio;
