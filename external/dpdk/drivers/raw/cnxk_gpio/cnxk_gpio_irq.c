...
#define OTX_IOC_CLR_GPIO_HANDLER _IO(OTX_IOC_MAGIC, 2)
...
static void cnxk_gpio_irq_stack_free(int cpu) {
...
static void *cnxk_gpio_irq_stack_alloc(int cpu) {
...
#define IRQ_STACK_SIZE 0x200000
  ...
  stack->sp_buffer = rte_zmalloc(NULL, IRQ_STACK_SIZE * 2, ARM_STACK_ALIGNMENT);
  if (!stack->sp_buffer) {
    rte_free(stack);
    return NULL;
  }
...
static void cnxk_gpio_irq_handler(int gpio_num) {
