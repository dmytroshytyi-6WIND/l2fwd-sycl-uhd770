...
static int cnxk_bphy_cgx_link_cond(uint16_t dev_id, unsigned int queue,
                                   int cond) {
...
int cnxk_bphy_cgx_dev_selftest(uint16_t dev_id) {
  ...
  for (i = 0; i < queues; i++) {
    ...
    ret = rte_rawdev_queue_conf_get(dev_id, i, &descs, sizeof(descs));
    if (ret)
      break;
    if (descs != 1) {
    ...
    if (ret != 0)
      RTE_LOG(ERR, PMD, "Timed out waiting for a link down\n");
