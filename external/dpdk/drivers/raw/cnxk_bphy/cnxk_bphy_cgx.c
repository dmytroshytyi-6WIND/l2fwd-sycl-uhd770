...
static void cnxk_bphy_cgx_format_name(char *name, unsigned int len,
                                      struct rte_pci_device *pci_dev) {
  snprintf(name, len, "BPHY_CGX:%02x:%02x.%x", pci_dev->addr.bus,
           pci_dev->addr.devid, pci_dev->addr.function);
}

static int cnxk_bphy_cgx_queue_def_conf(struct rte_rawdev *dev,
                                        uint16_t queue_id,
...
static int cnxk_bphy_cgx_process_buf(struct cnxk_bphy_cgx *cgx,
                                     unsigned int queue,
  ...
  case CNXK_BPHY_CGX_MSG_TYPE_GET_LINKINFO:
    ...
    link_info->speed = (enum cnxk_bphy_cgx_eth_link_speed)rlink_info.speed;
    link_info->autoneg = rlink_info.an;
    link_info->fec = (enum cnxk_bphy_cgx_eth_link_fec)rlink_info.fec;
    link_info->mode = (enum cnxk_bphy_cgx_eth_link_mode)rlink_info.mode;
    rsp = link_info;
    break;
  ...
  case CNXK_BPHY_CGX_MSG_TYPE_SET_LINK_MODE:
    ...
    rlink_mode.speed = (enum roc_bphy_cgx_eth_link_speed)link_mode->speed;
    rlink_mode.mode = (enum roc_bphy_cgx_eth_link_mode)link_mode->mode;
    ret = roc_bphy_cgx_set_link_mode(cgx->rcgx, lmac, &rlink_mode);
    break;
  case CNXK_BPHY_CGX_MSG_TYPE_SET_LINK_STATE:
    link_state = msg->data;
    ret = roc_bphy_cgx_set_link_state(cgx->rcgx, lmac, link_state->state);
    break;
...
static int cnxk_bphy_cgx_enqueue_bufs(struct rte_rawdev *dev,
...
static int cnxk_bphy_cgx_dequeue_bufs(struct rte_rawdev *dev,
                                      struct rte_rawdev_buf **buffers,
                                      unsigned int count,
                                      rte_rawdev_obj_t context) {
  struct cnxk_bphy_cgx *cgx = dev->dev_private;
...
static uint16_t cnxk_bphy_cgx_queue_count(struct rte_rawdev *dev) {
  struct cnxk_bphy_cgx *cgx = dev->dev_private;
...
static void cnxk_bphy_cgx_init_queues(struct cnxk_bphy_cgx *cgx) {
...
static void cnxk_bphy_cgx_fini_queues(struct cnxk_bphy_cgx *cgx) {
  unsigned int i;
...
static int cnxk_bphy_cgx_rawdev_probe(struct rte_pci_driver *pci_drv,
                                      struct rte_pci_device *pci_dev) {
...
static int cnxk_bphy_cgx_rawdev_remove(struct rte_pci_device *pci_dev) {
  char name[RTE_RAWDEV_NAME_MAX_LEN];
  struct rte_rawdev *rawdev;
  struct cnxk_bphy_cgx *cgx;
