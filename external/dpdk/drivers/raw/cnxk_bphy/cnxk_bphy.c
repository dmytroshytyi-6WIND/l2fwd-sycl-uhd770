...
static void bphy_test_handler_fn(int irq_num, void *isr_data) {
  test[irq_num].handled_intr = true;
  test[irq_num].handled_data = *((int *)isr_data);
}

static int bphy_rawdev_selftest(uint16_t dev_id) {
  ...
  for (i = 0; i < max_irq; i++) {
    ret = rte_pmd_bphy_intr_register(dev_id, test[i].irq_num, test[i].handler,
                                     test[i].data, 0);
    if (ret == -ENOTSUP) {
...
static void bphy_rawdev_get_name(char *name, struct rte_pci_device *pci_dev) {
  snprintf(name, RTE_RAWDEV_NAME_MAX_LEN, "BPHY:%02x:%02x.%x",
           pci_dev->addr.bus, pci_dev->addr.devid, pci_dev->addr.function);
}

static int cnxk_bphy_irq_enqueue_bufs(struct rte_rawdev *dev,
  ...
  case CNXK_BPHY_IRQ_MSG_TYPE_REGISTER:
    ...
    ret = cnxk_bphy_intr_register(dev->dev_id, info->irq_num, info->handler,
                                  info->data, info->cpu);
    if (ret)
      return ret;
    break;
...
static int cnxk_bphy_irq_dequeue_bufs(struct rte_rawdev *dev,
                                      struct rte_rawdev_buf **buffers,
                                      unsigned int count,
                                      rte_rawdev_obj_t context) {
  struct bphy_device *bphy_dev = (struct bphy_device *)dev->dev_private;
...
static uint16_t cnxk_bphy_irq_queue_count(struct rte_rawdev *dev) {
  struct bphy_device *bphy_dev = (struct bphy_device *)dev->dev_private;

  return RTE_DIM(bphy_dev->queues);
}

static int cnxk_bphy_irq_queue_def_conf(struct rte_rawdev *dev,
                                        uint16_t queue_id,
...
static int bphy_rawdev_probe(struct rte_pci_driver *pci_drv,
  ...
  bphy_rawdev =
      rte_rawdev_pmd_allocate(name, sizeof(*bphy_dev), rte_socket_id());
  if (bphy_rawdev == NULL) {
...
static int bphy_rawdev_remove(struct rte_pci_device *pci_dev) {
  char name[RTE_RAWDEV_NAME_MAX_LEN];
