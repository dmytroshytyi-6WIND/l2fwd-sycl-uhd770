...
static int ioat_dev_configure(const struct rte_rawdev *dev,
  ...
  ioat->desc_mz =
      rte_memzone_reserve(mz_name, (DESC_SZ + COMPLETION_SZ) * ioat->ring_size,
  ...
  for (i = 0; i < ioat->ring_size; i++) {
    ioat->desc_ring[i].next =
        ioat->ring_addr + (((i + 1) % ioat->ring_size) * DESC_SZ);
  }

  return 0;
}

static int ioat_dev_start(struct rte_rawdev *dev) {
  struct rte_ioat_rawdev *ioat = dev->dev_private;
...
static void ioat_dev_stop(struct rte_rawdev *dev) { RTE_SET_USED(dev); }

static int ioat_dev_info_get(struct rte_rawdev *dev, rte_rawdev_obj_t dev_info,
...
static int ioat_dev_close(struct rte_rawdev *dev __rte_unused) { return 0; }

static int ioat_rawdev_create(const char *name, struct rte_pci_device *dev) {
  ...
  ioat->ring_size = 0;
  ioat->desc_ring = NULL;
  ioat->status_addr = ioat->mz->iova + offsetof(struct rte_ioat_rawdev, status);
  ...
  while (ioat->regs->chancmd & RTE_IOAT_CHANCMD_RESET) {
    ...
    rte_delay_ms(1);
    if (++retry >= 200) {
      IOAT_PMD_ERR("%s: cannot reset device. CHANCMD=0x%" PRIx8
                   ", CHANSTS=0x%" PRIx64 ", CHANERR=0x%" PRIx32 "\n",
  ...
  ioat->regs->chanctrl =
      RTE_IOAT_CHANCTRL_ANY_ERR_ABORT_EN | RTE_IOAT_CHANCTRL_ERR_COMPLETION_EN;
...
static int ioat_rawdev_destroy(const char *name) {
  int ret;
...
static int ioat_rawdev_probe(struct rte_pci_driver *drv,
...
static int ioat_rawdev_remove(struct rte_pci_device *dev) {
  char name[32];
  int ret;
