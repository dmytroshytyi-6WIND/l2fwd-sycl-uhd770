...
static inline int idxd_pci_dev_command(struct idxd_rawdev *idxd,
...
static void idxd_pci_dev_stop(struct rte_rawdev *dev) {
  ...
  if (err_code || idxd_is_wq_enabled(idxd)) {
    IOAT_PMD_ERR("Failed disabling work queue %d, error code: %#x", idxd->qid,
                 err_code);
    return;
  }
...
static int idxd_pci_dev_start(struct rte_rawdev *dev) {
  struct idxd_rawdev *idxd = dev->dev_private;
  uint8_t err_code;
  ...
  if (err_code || !idxd_is_wq_enabled(idxd)) {
    IOAT_PMD_ERR("Failed enabling work queue %d, error code: %#x", idxd->qid,
                 err_code);
...
static int init_pci_device(struct rte_pci_device *dev, struct idxd_rawdev *idxd,
  ...
  IOAT_PMD_DEBUG("nb_groups = %u, nb_engines = %u, nb_wqs = %u", nb_groups,
                 nb_engines, nb_wqs);
  ...
  for (i = 0; i < nb_engines; i++) {
    IOAT_PMD_DEBUG("Assigning engine %u to group %u", i, i % nb_groups);
  ...
  for (i = 0; i < nb_wqs; i++) {
    ...
    IOAT_PMD_DEBUG("Assigning work queue %u to group %u", i, i % nb_groups);
...
static int idxd_rawdev_probe_pci(struct rte_pci_driver *drv,
  ...
  if (dev->device.devargs && dev->device.devargs->args[0] != '\0') {
    ...
    if (sscanf(dev->device.devargs->args, "max_queues=%u", &max_queues) != 1) {
      IOAT_PMD_ERR("Invalid device parameter: '%s'", dev->device.devargs->args);
      return -1;
    }
  }
  ...
  for (qid = 0; qid < nb_wqs; qid++) {
    ...
    idxd.qid = qid;
    idxd.public.portal =
        RTE_PTR_ADD(idxd.u.pci->portals, qid * IDXD_PORTAL_SIZE);
    if (idxd_is_wq_enabled(&idxd))
      IOAT_PMD_ERR("Error, WQ %u seems enabled", qid);
    ret = idxd_rawdev_create(qname, &dev->device, &idxd, &idxd_pci_ops);
    if (ret != 0) {
...
static int idxd_rawdev_destroy(const char *name) {
  int ret;
  uint8_t err_code;
...
static int idxd_rawdev_remove_pci(struct rte_pci_device *dev) {
  char name[PCI_PRI_STR_SIZE];
  int ret = 0;
