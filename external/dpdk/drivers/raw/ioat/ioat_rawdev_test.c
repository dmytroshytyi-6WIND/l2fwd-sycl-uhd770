...
static inline int __rte_format_printf(3, 4)
...
static int do_multi_copies(int dev_id, int split_batches,
                           int split_completions) {
  ...
  for (i = 0; i < RTE_DIM(srcs); i++) {
    ...
    if (rte_ioat_enqueue_copy(dev_id, srcs[i]->buf_iova + srcs[i]->data_off,
                              dsts[i]->buf_iova + dsts[i]->data_off, COPY_LEN,
                              (uintptr_t)srcs[i], (uintptr_t)dsts[i]) != 1) {
...
static int test_enqueue_copies(int dev_id) {
  unsigned int i;
  ...
  do {
    ...
    if (rte_ioat_enqueue_copy(dev_id, src->buf_iova + src->data_off,
    ...
    if (rte_ioat_completed_ops(dev_id, 1, NULL, NULL, (void *)&completed[0],
                               (void *)&completed[1]) != 0) {
      PRINT_ERR("Error: got unexpected returned handles from "
  ...
  do {
    ...
    if (rte_ioat_completed_ops(
            dev_id, max_completions, NULL, NULL, (void *)&completed[0],
  ...
  do_multi_copies(dev_id, 0,
                  1); /* enqueue 1 batch, then complete in two halves */
  return 0;
}

static int test_enqueue_fill(int dev_id) {
  ...
  for (i = 0; i < RTE_DIM(lengths); i++) {
    ...
    if (rte_ioat_enqueue_fill(dev_id, pattern, dst->buf_iova + dst->data_off,
                              lengths[i], (uintptr_t)dst) != 1) {
...
static int test_burst_capacity(int dev_id) {
#define BURST_SIZE 64
  ...
  for (iter = 0; iter < 2; iter++) {
    ...
    usleep(100);
    for (i = 0; i < ring_space / (2 * BURST_SIZE); i++) {
      if (rte_ioat_completed_ops(dev_id, BURST_SIZE, NULL, NULL, completions,
...
static int test_completion_status(int dev_id) {
#define COMP_BURST_SZ 16
  ...
  for (i = 0; i < RTE_DIM(fail_copy); i++) {
    ...
    for (j = 0; j < COMP_BURST_SZ; j++) {
      ...
      if (rte_ioat_enqueue_copy(dev_id,
                                ...
                                dsts[j]->buf_iova + dsts[j]->data_off, length,
                                (uintptr_t)srcs[j], (uintptr_t)dsts[j]) != 1) {
        PRINT_ERR("Error with rte_ioat_enqueue_copy for buffer %u\n", j);
        return -1;
      }
    }
  ...
  for (i = 0; i < RTE_DIM(fail_copy); i++) {
    ...
    if (fail_copy[i] >= COMP_BURST_SZ / 2 &&
        (not_ok != 1 ||
         status[fail_copy[i] - (COMP_BURST_SZ / 2)] == RTE_IOAT_OP_SUCCESS)) {
  ...
  for (i = 1; i < RTE_DIM(fail_copy); i++) {
    ...
    count =
        rte_ioat_completed_ops(dev_id, COMP_BURST_SZ * 2, status, &not_ok,
...
#define IOAT_TEST_RINGSIZE 512
  ...
  if (p.ring_size != expected_ring_size[dev_id]) {
    PRINT_ERR("Error, initial ring size is not as expected (Actual: %d, "
              "Expected: %d)\n",
