...
static const char *const xstat_names[] = {"failed_enqueues",
                                          "successful_enqueues",
...
int idxd_rawdev_close(struct rte_rawdev *dev __rte_unused) { return 0; }

int idxd_dev_dump(struct rte_rawdev *dev, FILE *f) {
...
int idxd_dev_info_get(struct rte_rawdev *dev, rte_rawdev_obj_t dev_info,
                      size_t info_size) {
...
int idxd_dev_configure(const struct rte_rawdev *dev, rte_rawdev_obj_t config,
                       size_t config_size) {
  struct idxd_rawdev *idxd = dev->dev_private;
  struct rte_idxd_rawdev *rte_idxd = &idxd->public;
  ...
  IOAT_PMD_DEBUG("Rawdev %u using %u descriptors", dev->dev_id, max_desc);
  ...
  rte_idxd->desc_ring =
      rte_zmalloc(NULL, sizeof(*rte_idxd->desc_ring) * max_desc * 2, 0);
  if (rte_idxd->desc_ring == NULL)
    return -ENOMEM;
  rte_idxd->desc_iova = rte_mem_virt2iova(rte_idxd->desc_ring);

  rte_idxd->hdl_ring =
      rte_zmalloc(NULL, sizeof(*rte_idxd->hdl_ring) * max_desc, 0);
  if (rte_idxd->hdl_ring == NULL) {
    rte_free(rte_idxd->desc_ring);
    rte_idxd->desc_ring = NULL;
    return -ENOMEM;
  }
  rte_idxd->hdl_ring_flags =
      rte_zmalloc(NULL, sizeof(*rte_idxd->hdl_ring_flags) * max_desc, 0);
...
int idxd_rawdev_create(const char *name, struct rte_device *dev,
  ...
  rawdev =
      rte_rawdev_pmd_allocate(name, sizeof(struct idxd_rawdev), dev->numa_node);
  if (rawdev == NULL) {
  ...
  mz = rte_memzone_reserve(mz_name, sizeof(struct idxd_rawdev), dev->numa_node,
...
public
  ->batch_idx_ring =
      rte_zmalloc(NULL, sizeof(uint16_t) * (idxd->max_batches + 1), 0);
