...
static void *idxd_vdev_mmap_wq(struct rte_dsa_device *dev) {
  void *addr;
  char path[PATH_MAX];
  int fd;
...
static int read_wq_string(struct rte_dsa_device *dev, const char *filename,
  ...
  if (fd < 0) {
    IOAT_PMD_ERR("%s(): opening file '%s' failed: %s", __func__, sysfs_node,
                 strerror(errno));
    return -1;
  }
  ...
  close(fd);
  if (len < 0) {
    IOAT_PMD_ERR("%s(): error reading file '%s': %s", __func__, sysfs_node,
                 strerror(errno));
    return -1;
  }
  value[len] = '\0';
...
static int idxd_rawdev_probe_dsa(struct rte_dsa_device *dev) {
  struct idxd_rawdev idxd = {{0}}; /* double {} to avoid error on BSD12 */
  int ret = 0;

  IOAT_PMD_INFO("Probing device %s on numa node %d", dev->wq_name,
...
static int is_for_this_process_use(const char *name) {
...
static int dsa_scan(void) {
  ...
  if (dev_dir == NULL) {
    ...
    IOAT_PMD_ERR("%s(): opendir '%s' failed: %s", __func__, path,
                 strerror(errno));
    return -1;
  }
  ...
  while ((wq = readdir(dev_dir)) != NULL) {
    ...
    if (strnlen(wq->d_name, sizeof(dev->wq_name)) == sizeof(dev->wq_name)) {
      IOAT_PMD_ERR("%s(): wq name too long: '%s', skipping", __func__,
                   wq->d_name);
      continue;
    }
