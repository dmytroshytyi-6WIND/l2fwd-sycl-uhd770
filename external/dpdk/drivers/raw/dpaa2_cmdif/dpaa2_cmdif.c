 ...
 */
static int dpaa2_cmdif_get_attr(struct rte_rawdev *dev, const char *attr_name,
                                uint64_t *attr_value) {
...
static int dpaa2_cmdif_enqueue_bufs(struct rte_rawdev *dev,
...
static int dpaa2_cmdif_dequeue_bufs(struct rte_rawdev *dev,
                                    struct rte_rawdev_buf **buffers,
                                    unsigned int count,
                                    rte_rawdev_obj_t context) {
  struct dpaa2_dpci_dev *cidev = dev->dev_private;
  ...
  buffers[0]->buf_addr = (void *)DPAA2_IOVA_TO_VADDR(DPAA2_GET_FD_ADDR(fd) +
...
static int dpaa2_cmdif_create(const char *name, struct rte_vdev_device *vdev,
                              int socket_id) {
  ...
  rawdev =
      rte_rawdev_pmd_allocate(name, sizeof(struct dpaa2_dpci_dev), socket_id);
  if (!rawdev) {
...
static int dpaa2_cmdif_destroy(const char *name) {
  int ret;
...
static int dpaa2_cmdif_probe(struct rte_vdev_device *vdev) {
  const char *name;
  int ret = 0;
...
static int dpaa2_cmdif_remove(struct rte_vdev_device *vdev) {
  const char *name;
  int ret;
...
static struct rte_vdev_driver dpaa2_cmdif_drv = {.probe = dpaa2_cmdif_probe,
