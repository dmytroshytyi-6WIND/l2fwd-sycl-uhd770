...
int rte_pmd_ifpga_get_dev_id(const char *pci_addr, uint16_t *dev_id) {
  ...
  snprintf(rdev_name, RTE_RAWDEV_NAME_MAX_LEN, IFPGA_RAWDEV_NAME_FMT, addr.bus,
           addr.devid, addr.function);
  rdev = rte_rawdev_pmd_get_named_dev(rdev_name);
  if (!rdev) {
    IFPGA_RAWDEV_PMD_DEBUG("%s is not probed by ifpga driver.", pci_addr);
    return -ENODEV;
  }
  *dev_id = rdev->dev_id;
...
static struct rte_rawdev *get_rte_rawdev(uint16_t dev_id) {
...
static struct opae_adapter *get_opae_adapter(uint16_t dev_id) {
  struct rte_rawdev *dev = NULL;
...
static opae_share_data *get_share_data(struct opae_adapter *adapter) {
...
static int ifpga_is_rebooting(struct opae_adapter *adapter) {
  opae_share_data *sd = NULL;
...
static int get_common_property(struct opae_adapter *adapter,
...
static int get_port_property(struct opae_adapter *adapter, uint16_t port,
  ...
  if (prop->type == 0) {
    ...
    if (ret) {
      IFPGA_RAWDEV_PMD_ERR("Failed to get port%u AFU ID.", port);
      return ret;
    }
...
const struct rte_pci_bus *rte_pmd_ifpga_get_pci_bus(void) {
  return ifpga_get_pci_bus();
}

int rte_pmd_ifpga_partial_reconfigure(uint16_t dev_id, int port,
                                      const char *file) {
  struct rte_rawdev *dev = NULL;
...
void rte_pmd_ifpga_cleanup(void) { ifpga_rawdev_cleanup(); }
