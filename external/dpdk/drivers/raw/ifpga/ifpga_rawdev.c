...
static struct ifpga_rawdev *ifpga_rawdev_allocate(struct rte_rawdev *rawdev);
static int set_surprise_link_check_aer(struct ifpga_rawdev *ifpga_rdev,
                                       int force_disable);
static int ifpga_pci_find_next_ext_capability(unsigned int fd, int start,
                                              uint32_t cap);
static int ifpga_pci_find_ext_capability(unsigned int fd, uint32_t cap);

struct ifpga_rawdev *ifpga_rawdev_get(const struct rte_rawdev *rawdev) {
...
static inline uint8_t ifpga_rawdev_find_free_device_index(void) {
  uint16_t dev_id;
...
static int ifpga_get_dev_vendor_id(const char *bdf, uint32_t *dev_id,
                                   uint32_t *vendor_id) {
  int fd;
  char path[1024];
  int ret;
...
static int ifpga_rawdev_fill_info(struct ifpga_rawdev *ifpga_dev) {
  ...
  while ((entry = readdir(dp)) != NULL) {
    ...
    if (sscanf(entry->d_name, "%x:%x:%x.%d", &dom, &bus, &dev, &func) < 4)
      ...
      strlcpy(sub_brg_bdf[i], entry->d_name, sizeof(sub_brg_bdf[i]));
      i++;
    }
  }
  closedir(dp);
  ...
  for (i = 0; i < 4; i++) {
    ...
    while ((entry = readdir(dp)) != NULL) {
      ...
      if (sscanf(entry->d_name, "%x:%x:%x.%d", &dom, &bus, &dev, &func) < 4)
        ...
        if (ifpga_get_dev_vendor_id(entry->d_name, &dev_id, &vendor_id))
          continue;
        if (vendor_id == 0x8086 &&
            (dev_id == 0x0CF8 || dev_id == 0x0D58 || dev_id == 0x1580)) {
          strlcpy(ifpga_dev->fvl_bdf[j], entry->d_name,
...
#define LOW_WARN(_sens, value)                                                 \
  (((_sens)->flags & OPAE_SENSOR_LOW_WARN_VALID) && (value > (_sens)->low_warn))

#define AUX_VOLTAGE_WARN 11400

static int ifpga_monitor_sensor(struct rte_rawdev *raw_dev, bool *gsd_start) {
  ...
  opae_mgr_for_each_sensor(mgr, sensor) {
    ...
    if (!strcmp(sensor->name, "Board Temperature") ||
      ...
      IFPGA_RAWDEV_PMD_DEBUG("read sensor %s %d %d %d\n", sensor->name, value,
                             sensor->high_warn, sensor->high_fatal);

      if (HIGH_WARN(sensor, value) || LOW_WARN(sensor, value)) {
    ...
    if (!strcmp(sensor->name, "12V AUX Voltage")) {
      if (value < AUX_VOLTAGE_WARN) {
        IFPGA_RAWDEV_PMD_INFO("%s reach threshold %d mV\n", sensor->name,
                              value);
        *gsd_start = true;
        break;
      }
...
static int set_surprise_link_check_aer(struct ifpga_rawdev *ifpga_rdev,
  ...
  if (enable || force_disable) {
    ...
    printf(">>>>>>Set AER %x,%x %x,%x\n", ifpga_rdev->aer_old[0],
           ifpga_rdev->aer_old[1], aer_new0, aer_new1);
...
static void *ifpga_rawdev_gsd_handle(__rte_unused void *param) {
  ...
  while (__atomic_load_n(&ifpga_monitor_refcnt, __ATOMIC_RELAXED)) {
    ...
    for (i = 0; i < IFPGA_RAWDEV_NUM; i++) {
      ...
      if (ifpga_rdev->poll_enabled) {
        ret = set_surprise_link_check_aer(ifpga_rdev, gsd_enable);
        if (ret == 1 && !gsd_enable) {
          gsd_enable = 1;
          i = -1;
        }
...
static int ifpga_monitor_start_func(struct ifpga_rawdev *dev) {
  int ret;
  ...
  if (!__atomic_fetch_add(&ifpga_monitor_refcnt, 1, __ATOMIC_RELAXED)) {
    ret = rte_ctrl_thread_create(&ifpga_monitor_start_thread, "ifpga-monitor",
                                 NULL, ifpga_rawdev_gsd_handle, NULL);
    if (ret != 0) {
      ifpga_monitor_start_thread = 0;
      IFPGA_RAWDEV_PMD_ERR("Fail to create ifpga monitor thread");
      return -1;
    }
  }
...
static int ifpga_monitor_stop_func(struct ifpga_rawdev *dev) {
  int ret;
...
static int ifpga_rawdev_info_get(struct rte_rawdev *dev,
  ...
  if (mgr) {
    ...
    if (opae_manager_get_eth_group_region_info(mgr, 0, &opae_lside_eth_info)) {
      return -ENOENT;
    }
    lside_bar_idx = opae_lside_eth_info.mem_idx;

    /* get NICSide BAR Index */
    if (opae_manager_get_eth_group_region_info(mgr, 1, &opae_nside_eth_info)) {
      return -ENOENT;
    }
    nside_bar_idx = opae_nside_eth_info.mem_idx;

    if (lside_bar_idx >= PCI_MAX_RESOURCE ||
        nside_bar_idx >= PCI_MAX_RESOURCE || lside_bar_idx == nside_bar_idx)
      return -ENOENT;

    /* fill LineSide BAR Index */
    afu_dev->mem_resource[lside_bar_idx].phys_addr =
        opae_lside_eth_info.phys_addr;
    afu_dev->mem_resource[lside_bar_idx].len = opae_lside_eth_info.len;
    afu_dev->mem_resource[lside_bar_idx].addr = opae_lside_eth_info.addr;

    /* fill NICSide BAR Index */
    afu_dev->mem_resource[nside_bar_idx].phys_addr =
        opae_nside_eth_info.phys_addr;
    afu_dev->mem_resource[nside_bar_idx].len = opae_nside_eth_info.len;
    afu_dev->mem_resource[nside_bar_idx].addr = opae_nside_eth_info.addr;
  }
  return 0;
}

static int ifpga_rawdev_configure(const struct rte_rawdev *dev,
...
static void ifpga_rawdev_stop(struct rte_rawdev *dev) { dev->started = 0; }
...
static int ifpga_rawdev_reset(struct rte_rawdev *dev) { return dev ? 0 : 1; }

static int fpga_pr(struct rte_rawdev *raw_dev, u32 port_id, const char *buffer,
                   u32 size, u64 *status) {
...
static int ifpga_rawdev_pr(struct rte_rawdev *dev, rte_rawdev_obj_t pr_conf) {
  struct opae_adapter *adapter;
  struct opae_manager *mgr;
  ...
  if (afu_pr_conf->pr_enable) {
    ret = rte_fpga_do_pr(dev, afu_pr_conf->afu_id.port, afu_pr_conf->bs_path);
    if (ret) {
  ...
  if (info->lightweight) {
    ...
    rte_memcpy(&afu_pr_conf->afu_id.uuid.uuid_high, uuid.b + 8, sizeof(u64));
...
static int ifpga_rawdev_get_attr(struct rte_rawdev *dev, const char *attr_name,
                                 uint64_t *attr_value) {
  struct opae_adapter *adapter;
  struct opae_manager *mgr;
...
#define MAX_PORT_PER_RETIMER 4
  ...
  if (!strcmp(attr_name, "LineSideMACType")) {
    ...
    if (opae_manager_get_eth_group_info(mgr, 0, &opae_eth_grp_info))
      return -1;
    switch (opae_eth_grp_info.speed) {
    case ETH_SPEED_10G:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_RETIMER_MAC_TYPE_10GE_XFI);
      break;
    case ETH_SPEED_25G:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_RETIMER_MAC_TYPE_25GE_25GAUI);
      break;
    default:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_RETIMER_MAC_TYPE_UNKNOWN);
      break;
    }
    return 0;
  }
  if (!strcmp(attr_name, "LineSideLinkSpeed")) {
    ...
    case MXD_1GB:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_LINK_SPEED_UNKNOWN);
      break;
    case MXD_2_5GB:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_LINK_SPEED_UNKNOWN);
      break;
    case MXD_5GB:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_LINK_SPEED_UNKNOWN);
      break;
    case MXD_10GB:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_LINK_SPEED_10GB);
      break;
    case MXD_25GB:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_LINK_SPEED_25GB);
      break;
    case MXD_40GB:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_LINK_SPEED_40GB);
      break;
    case MXD_100GB:
      *attr_value = (uint64_t)(IFPGA_RAWDEV_LINK_SPEED_UNKNOWN);
      break;
  ...
  if (!strcmp(attr_name, "LineSideBARIndex")) {
    /* eth_group 0 on FPGA connect to LineSide */
    if (opae_manager_get_eth_group_region_info(mgr, 0, &opae_eth_grp_reg_info))
      return -1;
  ...
  if (!strcmp(attr_name, "NICSideMACType")) {
    ...
    if (opae_manager_get_eth_group_info(mgr, 1, &opae_eth_grp_info))
      return -1;
  ...
  if (!strcmp(attr_name, "NICSideBARIndex")) {
    /* eth_group 1 on FPGA connect to NicSide */
    if (opae_manager_get_eth_group_region_info(mgr, 1, &opae_eth_grp_reg_info))
      return -1;
    *attr_value = (uint64_t)opae_eth_grp_reg_info.mem_idx;
    return 0;
  }
...
static int ifpga_get_fme_error_prop(struct opae_manager *mgr, u64 prop_id,
                                    u64 *val) {
...
static int ifpga_set_fme_error_prop(struct opae_manager *mgr, u64 prop_id,
                                    u64 val) {
  struct feature_prop prop;
...
static int fme_err_read_seu_emr(struct opae_manager *mgr) {
  u64 val;
  int ret;
...
static int fme_err_handle_error0(struct opae_manager *mgr) {
...
static int fme_err_handle_catfatal_error(struct opae_manager *mgr) {
...
static int fme_err_handle_nonfaterror(struct opae_manager *mgr) {
...
int ifpga_unregister_msix_irq(struct ifpga_rawdev *dev,
                              enum ifpga_irq_type type, int vec_start,
                              rte_intr_callback_fn handler, void *arg) {
...
int ifpga_register_msix_irq(struct ifpga_rawdev *dev, int port_id,
  ...
  *intr_handle = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_PRIVATE);
  ...
  if (rte_intr_fd_set(*intr_handle, rte_intr_efds_index_get(*intr_handle, 0)))
    return -rte_errno;
  ...
  if (type == IFPGA_FME_IRQ) {
    ...
    err_irq_set.evtfd = rte_intr_efds_index_get(*intr_handle, 0);
  ...
  ret = rte_intr_callback_register(*intr_handle, handler, (void *)arg);
  if (ret) {
    free(intr_efds);
    return -EINVAL;
  }
...
static int ifpga_rawdev_create(struct rte_pci_device *pci_dev, int socket_id) {
  int ret = 0;
  ...
  rawdev =
      rte_rawdev_pmd_allocate(name, sizeof(struct opae_adapter), socket_id);
  if (rawdev == NULL) {
...
static int ifpga_rawdev_destroy(struct rte_pci_device *pci_dev) {
  int ret;
  ...
  if (ifpga_unregister_msix_irq(dev, IFPGA_FME_IRQ, 0, fme_interrupt_handler,
                                mgr) < 0)
    return -EINVAL;
...
static int ifpga_rawdev_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
...
RTE_PMD_REGISTER_KMOD_DEP(ifpga_rawdev_pci_driver,
...
static int ifpga_cfg_probe(struct rte_vdev_device *dev) {
  ...
  if (rte_kvargs_count(kvlist, IFPGA_ARG_NAME) == 1) {
    if (rte_kvargs_process(kvlist, IFPGA_ARG_NAME, &ifpga_rawdev_get_string_arg,
                           &name) < 0) {
      IFPGA_RAWDEV_PMD_ERR("error to parse %s", IFPGA_ARG_NAME);
      goto end;
    }
  } else {
    IFPGA_RAWDEV_PMD_ERR("arg %s is mandatory for ifpga bus", IFPGA_ARG_NAME);
    goto end;
  }

  if (rte_kvargs_count(kvlist, IFPGA_ARG_PORT) == 1) {
    if (rte_kvargs_process(kvlist, IFPGA_ARG_PORT, &rte_ifpga_get_integer32_arg,
                           &port) < 0) {
      IFPGA_RAWDEV_PMD_ERR("error to parse %s", IFPGA_ARG_PORT);
      goto end;
    }
  } else {
    IFPGA_RAWDEV_PMD_ERR("arg %s is mandatory for ifpga bus", IFPGA_ARG_PORT);
    goto end;
  }
  ...
  memset(dev_name, 0, sizeof(dev_name));
  snprintf(dev_name, RTE_RAWDEV_NAME_MAX_LEN, "%d|%s", port, name);

  ret = rte_eal_hotplug_add(RTE_STR(IFPGA_BUS_NAME), dev_name, devargs->args);
end:
...
static int ifpga_cfg_remove(struct rte_vdev_device *vdev) {
  IFPGA_RAWDEV_PMD_INFO("Remove ifpga_cfg %p", vdev);
...
RTE_PMD_REGISTER_PARAM_STRING(ifpga_rawdev_cfg, "ifpga=<string> "
                                                "port=<int> "
                                                "afu_bts=<path>");
