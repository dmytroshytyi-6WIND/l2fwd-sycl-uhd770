...
static unsigned char xor_20(unsigned char val) { return val ^ 0x20; }

static void reorder_phy_data(u8 bits_per_word, void *buf, unsigned int len) {
...
#define RX_ALL_IDLE_DATA                                                       \
  (SPI_BYTE_IDLE << 24 | SPI_BYTE_IDLE << 16 | SPI_BYTE_IDLE << 8 |            \
   SPI_BYTE_IDLE)
...
static int do_spi_txrx(struct spi_transaction_dev *dev,
                       unsigned char *tx_buffer, unsigned int tx_len,
                       unsigned char *rx_buffer, unsigned int rx_len,
  ...
  timeout = rte_get_timer_cycles() + msecs_to_timer_cycles(2000);
    ...
    if (!eop_found) {
      ...
      if (eop) {
        if ((BPW + rxbuf - eop) > 2) {
          ...
          if (*ptr != SPI_BYTE_ESC && *ptr != SPI_PACKET_ESC)
            break;
        ...
        rx_len = min(rx_len, IFPGA_ALIGN(rx_cnt + add_byte, BPW));
        eop_found = true;
      }
    }
    rxbuf += BPW;
...
static int byte_to_core_convert(struct spi_transaction_dev *dev,
  ...
  ret = do_spi_txrx(dev, send_packet, tx_len, resp_packet, resp_max_len,
                    &actual_rx);
  if (ret)
    return ret;
...
static int packet_to_byte_conver(struct spi_transaction_dev *dev,
  ...
  *p++ = 0;
  ...
  ret = byte_to_core_convert(dev, p - send_packet, send_packet, resp_max_len,
                             resp_packet, &valid_resp_len);
  if (ret)
    return -EBUSY;
...
static int do_transaction(struct spi_transaction_dev *dev, unsigned int addr,
  ...
  case SPI_TRAN_NON_SEQ_WRITE:
    ...
    ret = packet_to_byte_conver(dev, size + HEADER_LEN, transaction,
                                RESPONSE_LEN, response, &valid_len);
    if (ret)
      return -EBUSY;
  ...
  case SPI_TRAN_NON_SEQ_READ:
    ret = packet_to_byte_conver(dev, HEADER_LEN, transaction, size, response,
                                &valid_len);
...
int spi_transaction_read(struct spi_transaction_dev *dev, unsigned int addr,
  ...
  ret = do_transaction(dev, addr, size, data,
                       (size > SPI_REG_BYTES) ? SPI_TRAN_SEQ_READ
...
int spi_transaction_write(struct spi_transaction_dev *dev, unsigned int addr,
  ...
  pthread_mutex_lock(dev->mutex);
  ret = do_transaction(dev, addr, size, data,
                       (size > SPI_REG_BYTES) ? SPI_TRAN_SEQ_WRITE
