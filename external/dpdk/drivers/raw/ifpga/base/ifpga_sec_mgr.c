...
static const char *const rsu_prog[] = {
    "IDLE",    "PREPARING",    "SLEEPING",        "READY", "AUTHENTICATING",
    "COPYING", "CANCELLATION", "PROGRAMMING_KEY", "DONE",  "PKVL_DONE"};
static const char *const rsu_statl[] = {
    "NORMAL",      "TIMEOUT",  "AUTH_FAIL",  "COPY_FAIL", "FATAL",
    "PKVL_REJECT", "NON_INCR", "ERASE_FAIL", "WEAROUT"};
static const char *const rsu_stath[] = {
    "NIOS_OK",      "USER_OK",        "FACTORY_OK",    "USER_FAIL",
    "FACTORY_FAIL", "NIOS_FLASH_ERR", "FPGA_FLASH_ERR"};
...
static int poll_timeout(struct intel_max10_device *dev, uint32_t offset,
                        bool (*cond)(uint32_t), uint32_t interval_ms,
                        uint32_t timeout_ms) {
  uint32_t val = 0;
  int ret = 0;
  ...
  for (;;) {
    ...
    if (ret < 0) {
      dev_err(dev, "Failed to read max10 register 0x%x [e:%d]\n", offset, ret);
      break;
    }
...
static int n3000_secure_update_start(struct intel_max10_device *dev) {
  ...
  ret = max10_sys_update_bits(dev, MAX10_DOORBELL, RSU_REQUEST | HOST_STATUS,
                              RSU_REQUEST);
  if (ret < 0) {
    dev_err(dev, "Failed to updt max10 doorbell register [e:%d]\n", ret);
    return ret;
  }
  ...
  if (ret < 0) {
    dev_err(dev, "Failed to poll max10 doorbell register [e:%d]\n", ret);
    return ret;
  }
...
static int n3000_bulk_write(struct intel_max10_device *dev, uint32_t addr,
  ...
  if (len & 0x3) {
    dev_err(dev, "Length of data block is not 4 bytes aligned [e:%u]\n", len);
    return -EINVAL;
  }
  ...
  for (i = 0; i < n; i++) {
    ...
    if (ret < 0) {
      dev_err(dev, "Failed to write to staging area 0x%08x [e:%d]\n", addr + p,
              ret);
      return ret;
    }
    usleep(1);
...
static int n3000_write_blk(struct ifpga_sec_mgr *smgr, char *buf,
  ...
  if (offset + len > dev->staging_area_size) {
    dev_err(dev, "Write position would be out of staging area [e:%u]\n",
...
static int n3000_write_done(struct ifpga_sec_mgr *smgr) {
  ...
  if (ret < 0) {
    dev_err(dev, "Failed to update max10 doorbell register [e:%d]\n", ret);
    return ret;
  }
...
static int n3000_reload_fpga(struct intel_max10_device *dev, int page) {
  ...
  if (dev->flags & MAX10_FLAGS_SECURE) {
    ret = max10_sys_update_bits(dev, FPGA_RECONF_REG, SFPGA_RP_LOAD, 0);
    if (ret < 0) {
      dev_err(dev, "Failed to update max10 reconfig register [e:%d]\n", ret);
      goto end;
    }
    ...
    if (ret < 0) {
      dev_err(dev, "Failed to update max10 rsu register [e:%d]\n", ret);
      goto end;
    }
    ret = max10_sys_update_bits(dev, RSU_REG, FPGA_RP_LOAD | FPGA_RECONF_PAGE,
...
static int n3000_reload_bmc(struct intel_max10_device *dev, int page) {
  ...
  if (dev->flags & MAX10_FLAGS_SECURE) {
    ret = max10_sys_update_bits(dev, MAX10_DOORBELL, CONFIG_SEL | REBOOT_REQ,
                                CONFIG_SEL_S(page) | REBOOT_REQ);
  } else {
    val = (page == 0) ? 0x1 : 0x3;
    ret = max10_reg_write(dev, IFPGA_DUAL_CFG_CTRL1, val);
    if (ret < 0) {
      dev_err(dev, "Failed to write to dual config1 register [e:%d]\n", ret);
      goto end;
    }
    ...
    if (ret < 0) {
      ...
      dev_err(dev, "Failed to write to dual config0 register [e:%d]\n", ret);
    }
  }
...
static uint64_t n3000_get_hw_errinfo(struct ifpga_sec_mgr *smgr) {
  ...
  if (ret < 0) {
    dev_err(dev, "Failed to read authenticate result register [e:%d]\n", ret);
    return -1;
  }
