...
static int write_flash_image(struct ifpga_sec_mgr *smgr, const char *image,
  ...
  if (fd < 0) {
    dev_err(smgr, "Failed to open \'%s\' for RD [e:%s]\n", image,
            strerror(errno));
    return -EIO;
  }
  ...
  do {
    to_transfer =
        (length > IFPGA_RSU_DATA_BLK_SIZE) ? IFPGA_RSU_DATA_BLK_SIZE : length;
    ...
    if ((uint32_t)read_size != to_transfer) {
      dev_err(smgr, "Read length %zd is not expected [e:%u]\n", read_size,
              to_transfer);
      ret = -EIO;
      goto end;
    }
    ...
    do {
      ...
      ret = smgr->ops->write_blk(smgr, buf, offset, to_transfer);
      if (ret == 0)
        break;
      sleep(1);
...
static int apply_flash_update(struct ifpga_sec_mgr *smgr) {
  ...
  if (smgr->copy_speed == 0) /* avoid zero divide fault */
    smgr->copy_speed = 1;
  one_percent_time = (one_percent + smgr->copy_speed - 1) / smgr->copy_speed;
...
int fpga_update_flash(struct ifpga_fme_hw *fme, const char *image,
  ...
  if (len < 0) {
    dev_err(smgr, "Failed to get file length of \'%s\' [e:%s]\n", image,
            strerror(errno));
    return -EIO;
  }
  if (len == 0) {
  ...
  if (smgr->max10_dev->staging_area_size < smgr->rsu_length) {
    dev_err(dev,
            "Size of staging area is small than image length "
            "[%u<%u]\n",
            smgr->max10_dev->staging_area_size, smgr->rsu_length);
    return -EINVAL;
  }
