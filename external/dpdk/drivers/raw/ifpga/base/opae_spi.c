...
static int spi_indirect_read(struct altera_spi_device *dev, u32 reg, u32 *val) {
  u64 tmp;
  u64 ctrl;
...
int spi_reg_write(struct altera_spi_device *dev, u32 reg, u32 value) {
...
int spi_command(struct altera_spi_device *dev, unsigned int chip_select,
                unsigned int wlen, void *wdata, unsigned int rlen,
                void *rdata) {
...
void altera_spi_init(struct altera_spi_device *spi_dev) {
  ...
  dev_info(spi_dev,
           "spi param: type=%d, data width:%d, endian:%d, clock_polarity=%d, "
           "clock=%dMHz, chips=%d, cpha=%d\n",
           spi_dev->spi_param.type, spi_dev->data_width, spi_dev->endian,
           spi_dev->spi_param.clock_polarity, spi_dev->spi_param.clock,
           spi_dev->num_chipselect, spi_dev->spi_param.clock_phase);
