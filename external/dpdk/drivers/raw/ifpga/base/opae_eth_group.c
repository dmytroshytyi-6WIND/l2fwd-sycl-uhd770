...
static int eth_group_get_select(struct eth_group_device *dev, u8 type, u8 index,
                                u8 *select) {
  /*
...
int eth_group_write_reg(struct eth_group_device *dev, u8 type, u8 index,
                        u16 addr, u32 data) {
  u8 dev_select = 0;
  u64 v = 0;
  int ret;
  ...
  dev_debug(dev, "%s type %s index %u addr 0x%x\n", __func__,
            eth_type_to_string(type), index, addr);
  ...
  v = CMD_WR << CTRL_CMD_SHIT | (u64)dev_select << CTRL_DS_SHIFT |
      (u64)addr << CTRL_ADDR_SHIFT | (data & CTRL_WR_DATA);
...
int eth_group_read_reg(struct eth_group_device *dev, u8 type, u8 index,
                       u16 addr, u32 *data) {
  u8 dev_select = 0;
  u64 v = 0;
  int ret;
  ...
  v = CMD_RD << CTRL_CMD_SHIT | (u64)dev_select << CTRL_DS_SHIFT |
  ...
  if (opae_readq_poll_timeout(dev->base + ETH_GROUP_STAT, v, v & STAT_DATA_VAL,
                              DATA_VAL_INVL, DATA_VAL_POLL_TIMEOUT))
    return -ETIMEDOUT;
...
static int eth_group_reset_mac(struct eth_group_device *dev, u8 index,
  ...
  ret = eth_group_read_reg(dev, ETH_GROUP_MAC, index, MAC_CONFIG, &val);
  if (ret) {
  ...
  ret = eth_group_write_reg(dev, ETH_GROUP_MAC, index, MAC_CONFIG, val);
  if (ret)
...
static int eth_group_reset_phy(struct eth_group_device *dev, u8 index,
  ...
  ret = eth_group_read_reg(dev, ETH_GROUP_PHY, index, ADD_PHY_CTRL, &val);
  if (ret) {
  ...
  ret = eth_group_write_reg(dev, ETH_GROUP_PHY, index, ADD_PHY_CTRL, val);
  if (ret)
...
struct eth_group_device *eth_group_probe(void *base) {
  ...
  dev_info(dev,
           "eth group device %d probe done: phy_num=mac_num:%d, speed=%d\n",
