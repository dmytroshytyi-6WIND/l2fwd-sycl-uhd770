 ...
 * Return: 0 on success, otherwise error code.
 */
int opae_acc_get_info(struct opae_accelerator *acc,
 ...
 * Return: 0 on success, otherwise error code.
 */
int opae_acc_set_irq(struct opae_accelerator *acc, u32 start, u32 count,
                     s32 evtfds[]) {
  if (!acc || !acc->data)
    return -EINVAL;
...
static int opae_shm_open(char *shm_name, u32 size, int *new_shm) {
  ...
  if (shm_id == -1) {
    ...
    *new_shm = 1;
    ...
    if (ret == -1) {
      dev_err(NULL, "failed to set shared memory size to %u\n", size);
...
static pthread_mutex_t *opae_adapter_mutex_open(struct opae_adapter *adapter) {
  ...
  if (shm_id == -1) {
    ...
    if (ptr) {
      dev_info(NULL, "shared memory %s address is %p\n", shm_name, ptr);
      if (new_shm)
...
static void *opae_adapter_shm_alloc(struct opae_adapter *adapter) {
  ...
  if (shm_id == -1) {
    ...
    adapter->shm.ptr =
        mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_id, 0);
    if (adapter->shm.ptr) {
      dev_info(NULL, "shared memory %s address is %p\n", shm_name,
               adapter->shm.ptr);
      if (new_shm)
 ...
 * Return: 0 on success.
 */
int opae_adapter_init(struct opae_adapter *adapter, const char *name,
                      void *data) {
  if (!adapter)
    return -ENOMEM;
 ...
 * destroy things allocated during adapter enumeration.
 */
struct opae_accelerator *opae_adapter_get_acc(struct opae_adapter *adapter,
                                              int acc_id) {
  ...
  opae_adapter_for_each_acc(adapter, acc) if (acc->index == acc_id) return acc;
...
int opae_manager_write_mac_rom(struct opae_manager *mgr, int port,
  ...
  if (mgr->network_ops && mgr->network_ops->write_mac_rom)
    return mgr->network_ops->write_mac_rom(
        mgr, port * sizeof(struct opae_ether_addr), addr,
 ...
 * Return: 0 on success, otherwise error code
 */
int opae_manager_get_eth_group_info(struct opae_manager *mgr, u8 group_id,
                                    struct opae_eth_group_info *info) {
  if (!mgr || !mgr->network_ops)
    return -EINVAL;

  if (mgr->network_ops->get_retimer_info)
    return mgr->network_ops->get_eth_group_info(mgr, group_id, info);
 ...
 * Return: 0 on success, otherwise error code.
 */
int opae_manager_get_eth_group_region_info(
    struct opae_manager *mgr, u8 group_id,
...
int opae_manager_eth_group_read_reg(struct opae_manager *mgr, u8 group_id,
  ...
  if (mgr->network_ops->eth_group_reg_read)
    return mgr->network_ops->eth_group_reg_read(mgr, group_id, type, index,
                                                addr, data);
...
int opae_manager_eth_group_write_reg(struct opae_manager *mgr, u8 group_id,
  ...
  if (mgr->network_ops->eth_group_reg_write)
    return mgr->network_ops->eth_group_reg_write(mgr, group_id, type, index,
                                                 addr, data);
...
int opae_manager_get_retimer_status(struct opae_manager *mgr,
  ...
  if (mgr->network_ops->get_retimer_status)
    return mgr->network_ops->get_retimer_status(mgr, status);
...
struct opae_sensor_info *opae_mgr_get_sensor_by_id(struct opae_manager *mgr,
  ...
  opae_mgr_for_each_sensor(mgr, sensor) if (sensor->id == id) return sensor;
...
struct opae_sensor_info *opae_mgr_get_sensor_by_name(struct opae_manager *mgr,
  ...
  opae_mgr_for_each_sensor(
      mgr, sensor) if (!strcmp(sensor->name, name)) return sensor;
 ...
 * Return: 0 on success, otherwise error code
 */
int opae_mgr_get_sensor_value_by_id(struct opae_manager *mgr, unsigned int id,
                                    unsigned int *value) {
  struct opae_sensor_info *sensor;
