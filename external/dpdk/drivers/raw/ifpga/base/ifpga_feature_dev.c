...
int __fpga_port_disable(struct ifpga_port_hw *port) {
  ...
  if (fpga_wait_register_field(port_sftrst_ack, control, &port_hdr->control,
                               RST_POLL_TIMEOUT, RST_POLL_INVL)) {
...
static struct feature_driver fme_feature_drvs[] = {
    {
        FEATURE_DRV(FME_FEATURE_ID_HEADER, FME_FEATURE_HEADER, &fme_hdr_ops),
    },
    {
...
#define MSIX_IRQ_SET_BUF_LEN                                                   \
  (sizeof(struct vfio_irq_set) + sizeof(int) * FPGA_MAX_MSIX_VEC_COUNT)
...
static int vfio_msix_enable_block(s32 vfio_dev_fd, unsigned int vec_start,
  ...
  irq_set->count =
      count
          ? (count > FPGA_MAX_MSIX_VEC_COUNT ? FPGA_MAX_MSIX_VEC_COUNT : count)
          : 1;
...
int fpga_msix_set_block(struct ifpga_feature *feature, unsigned int start,
  ...
  ret =
      vfio_msix_enable_block(feature->vfio_dev_fd, ctx[start].idx, count, fds);
  if (!ret) {
    for (i = 0; i < count; i++)
      ctx[i].eventfd = fds[i];
  }
