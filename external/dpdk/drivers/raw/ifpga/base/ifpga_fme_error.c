...
static int fme_err_get_errors(struct ifpga_fme_hw *fme, u64 *val) {
  struct feature_fme_err *fme_err =
      get_fme_feature_ioaddr_by_index(fme, FME_FEATURE_ID_GLOBAL_ERR);
...
static int fme_err_set_pcie0_errors(struct ifpga_fme_hw *fme, u64 val) {
  ...
  if (val != pcie0_err.csr)
    ...
    writeq(pcie0_err.csr & FME_PCIE0_ERROR_MASK, &fme_err->pcie0_err);
...
static int fme_err_set_pcie1_errors(struct ifpga_fme_hw *fme, u64 val) {
  ...
  if (val != pcie1_err.csr)
    ...
    writeq(pcie1_err.csr & FME_PCIE1_ERROR_MASK, &fme_err->pcie1_err);
...
static int fme_err_get_seu_emr(struct ifpga_fme_hw *fme, u64 *val, bool high) {
  struct feature_fme_err *fme_err =
      get_fme_feature_ioaddr_by_index(fme, FME_FEATURE_ID_GLOBAL_ERR);
...
static int fme_global_err_set_irq(struct ifpga_feature *feature,
                                  void *irq_set) {
