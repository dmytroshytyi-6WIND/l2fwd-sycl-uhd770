...
/* Accelerator APIs */
static int ifpga_acc_get_uuid(struct opae_accelerator *acc, struct uuid *uuid) {
...
static int ifpga_acc_set_irq(struct opae_accelerator *acc, u32 start, u32 count,
                             s32 evtfds[]) {
...
static int
ifpga_mgr_get_eth_group_region_info(struct opae_manager *mgr,
...
static int ifpga_mgr_get_eth_group_info(struct opae_manager *mgr, u8 group_id,
...
static int ifpga_mgr_eth_group_reg_read(struct opae_manager *mgr, u8 group_id,
                                        u8 type, u8 index, u16 addr,
                                        u32 *data) {
  struct ifpga_fme_hw *fme = mgr->data;

  return fme_mgr_eth_group_read_reg(fme, group_id, type, index, addr, data);
}
...
static int ifpga_mgr_eth_group_reg_write(struct opae_manager *mgr, u8 group_id,
  ...
  return fme_mgr_eth_group_write_reg(fme, group_id, type, index, addr, data);
}
...
int ifpga_set_irq(struct ifpga_hw *hw, u32 fiu_id, u32 port_id, u32 feature_id,
                  void *irq_set) {
  if (!hw || !irq_set)
    return -EINVAL;
