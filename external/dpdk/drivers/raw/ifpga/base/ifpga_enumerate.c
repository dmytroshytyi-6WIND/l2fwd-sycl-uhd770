...
static int build_info_add_sub_feature(struct build_feature_devs_info *binfo,
  ...
  feature->phys_addr = binfo->phys_addr + ((u8 *)start - (u8 *)binfo->ioaddr);
  ...
  if (binfo->current_type == FME_ID) {
    ...
    TAILQ_INSERT_TAIL(&hw->port[port_id].feature_list, feature, next);
  } else {
    return -EFAULT;
  }
  return ret;
...
static int create_feature_instance(struct build_feature_devs_info *binfo,
                                   void __iomem *start, u64 fid,
                                   unsigned int size, unsigned int vec_start,
                                   unsigned int vec_cnt) {
...
static int parse_feature_port_uafu(struct build_feature_devs_info *binfo,
  ...
  info->region[0].phys_addr =
      binfo->phys_addr + (uint8_t *)start - (uint8_t *)binfo->ioaddr;
...
static int build_info_commit_dev(struct build_feature_devs_info *binfo) {
  ...
  if (binfo->current_type == PORT_ID) {
    ...
    br = opae_bridge_alloc(hw->adapter->name, &ifpga_br_ops, binfo->fiu);
    if (!br)
      return -ENOMEM;
    ...
    feature = get_feature_by_id(&port->feature_list, PORT_FEATURE_ID_UINT);
    if (feature)
      info->num_irqs = feature->vec_cnt;

    acc = opae_accelerator_alloc(hw->adapter->name, &ifpga_acc_ops, info);
    if (!acc) {
      opae_bridge_free(br);
      return -ENOMEM;
    }
...
static int build_info_create_dev(struct build_feature_devs_info *binfo,
...
static int parse_feature_fiu(struct build_feature_devs_info *binfo,
  ...
  case FEATURE_FIU_ID_PORT:
    ...
    if (fiu_header.next_afu) {
      ...
      ret = parse_feature_afus(binfo, (struct feature_header *)start);
      if (ret)
        return ret;
    } else {
...
static int parse_feature_private(struct build_feature_devs_info *binfo,
  ...
  default:
    dev_err(binfo,
            "private feature %x belonging to AFU %d (unknown_type) is not "
            "supported yet.\n",
...
static int parse_feature_list(struct build_feature_devs_info *binfo,
  ...
  for (; start < end; start += header.next_header_offset) {
    ...
    dev_debug(binfo,
              "%s: address=0x%p, val=0x%llx, header.id=0x%x, "
              "header.next_offset=0x%x, header.eol=0x%x, header.type=0x%x\n",
              __func__, hdr, (unsigned long long)header.csr, header.id,
              header.next_header_offset, header.end_of_list, header.type);
...
static void ifpga_print_device_feature_list(struct ifpga_hw *hw) {
  ...
  ifpga_for_each_fme_feature(fme, feature) {
    ...
    dev_info(hw, "%12s:	%p - %p  - paddr: 0x%lx\n", feature->name,
             feature->addr, feature->addr + feature->size - 1,
