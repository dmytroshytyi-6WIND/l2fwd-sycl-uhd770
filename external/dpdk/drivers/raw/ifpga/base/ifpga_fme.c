...
static int fme_hdr_get_prop(struct ifpga_feature *feature,
...
static int fme_thermal_get_threshold1(struct ifpga_fme_hw *fme, u64 *thres1) {
  ...
  thermal = get_fme_feature_ioaddr_by_index(fme, FME_FEATURE_ID_THERMAL_MGMT);
...
static int fme_thermal_get_revision(struct ifpga_fme_hw *fme, u64 *revision) {
  struct feature_fme_thermal *fme_thermal =
      get_fme_feature_ioaddr_by_index(fme, FME_FEATURE_ID_THERMAL_MGMT);
  struct feature_header header;
...
static int fme_thermal_set_prop(struct ifpga_feature *feature,
                                struct feature_prop *prop) {
  struct ifpga_fme_hw *fme = feature->parent;
...
static int fme_thermal_get_prop(struct ifpga_feature *feature,
                                struct feature_prop *prop) {
  struct ifpga_fme_hw *fme = feature->parent;
...
static int fme_pwr_get_consumed(struct ifpga_fme_hw *fme, u64 *consumed) {
  struct feature_fme_power *fme_power =
      get_fme_feature_ioaddr_by_index(fme, FME_FEATURE_ID_POWER_MGMT);
...
static void fme_emif_uinit(struct ifpga_feature *feature) { UNUSED(feature); }
...
static int board_type_to_info(u32 type, struct opae_board_info *info) {
  switch (type) {
  case VC_8_10G:
...
static int fme_get_board_interface(struct ifpga_fme_hw *fme) {
  ...
  dev_info(fme, "found: PCI dev: %02x:%02x:%x board: %s type: %s\n",
           hw->pci_data->bus, hw->pci_data->devid, hw->pci_data->function,
  ...
  dev_info(fme,
           "get board info: nums_retimers %d ports_per_retimer %d nums_fvl %d "
           "ports_per_fvl %d\n",
           fme->board_info.nums_of_retimer, fme->board_info.ports_per_retimer,
           fme->board_info.nums_of_fvl, fme->board_info.ports_per_fvl);
  ...
  dev_info(fme, "max10 version 0x%x, nios fw version 0x%x\n",
           fme->board_info.max10_version, fme->board_info.nios_fw_version);
...
static int nios_spi_wait_init_done(struct altera_spi_device *dev) {
  u32 val = 0;
  unsigned long timeout = rte_get_timer_cycles() + msecs_to_timer_cycles(10000);
  unsigned long ticks;
  int major_version;
  ...
  major_version = (val & NIOS_VERSION_MAJOR) >> NIOS_VERSION_MAJOR_SHIFT;
...
static int i2c_mac_rom_test(struct altera_i2c_dev *dev) {
  ...
  ret =
      at24_eeprom_write(dev, AT24512_SLAVE_ADDR, 0, (u8 *)buf, strlen(string));
  if (ret < 0) {
    dev_err(NULL, "write i2c error:%d\n", ret);
    return ret;
  }

  ret = at24_eeprom_read(dev, AT24512_SLAVE_ADDR, 0, (u8 *)read_buf,
                         strlen(string));
  if (ret < 0) {
...
static int fme_eth_group_init(struct ifpga_feature *feature) {
  ...
  fme->eth_group_region[dev->group_id].addr = feature->addr;
  fme->eth_group_region[dev->group_id].phys_addr = feature->phys_addr;
  fme->eth_group_region[dev->group_id].len = feature->size;

  fme->nums_eth_dev++;

  dev_info(NULL, "FME PHY Group %d Init.\n", dev->group_id);
  dev_info(NULL, "found %d eth group, addr %p phys_addr 0x%llx len %u\n",
           dev->group_id, feature->addr, (unsigned long long)feature->phys_addr,
           feature->size);
...
int fme_mgr_read_mac_rom(struct ifpga_fme_hw *fme, int offset, void *buf,
                         int size) {
...
int fme_mgr_write_mac_rom(struct ifpga_fme_hw *fme, int offset, void *buf,
                          int size) {
  struct altera_i2c_dev *dev;
...
int fme_mgr_get_eth_group_info(struct ifpga_fme_hw *fme, u8 group_id,
...
int fme_mgr_eth_group_read_reg(struct ifpga_fme_hw *fme, u8 group_id, u8 type,
                               u8 index, u16 addr, u32 *data) {
  struct eth_group_device *dev;
...
int fme_mgr_eth_group_write_reg(struct ifpga_fme_hw *fme, u8 group_id, u8 type,
                                u8 index, u16 addr, u32 data) {
  struct eth_group_device *dev;
...
static int fme_get_eth_group_speed(struct ifpga_fme_hw *fme, u8 group_id) {
  struct eth_group_device *dev;
...
int fme_mgr_get_retimer_info(struct ifpga_fme_hw *fme,
  ...
  info->support_speed = fme_get_eth_group_speed(fme, LINE_SIDE_GROUP_ID);
...
int fme_mgr_get_retimer_status(struct ifpga_fme_hw *fme,
  ...
  /* The speed of PKVL is identical the eth group's speed */
  status->speed = fme_get_eth_group_speed(fme, LINE_SIDE_GROUP_ID);

  status->line_link_bitmap = val;

  dev_debug(dev, "get retimer status: speed:%d. line_link_bitmap:0x%x\n",
            status->speed, status->line_link_bitmap);
