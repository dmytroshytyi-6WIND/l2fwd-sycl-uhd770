...
static int i2c_transfer(struct altera_i2c_dev *dev, struct i2c_msg *msg,
                        int num) {
  int ret, try;
...
int i2c_read16(struct altera_i2c_dev *dev, unsigned int slave_addr, u32 offset,
               u8 *buf, u32 count) {
  return i2c_read(dev, I2C_FLAG_ADDR16, slave_addr, offset, buf, count);
}
...
int i2c_write16(struct altera_i2c_dev *dev, unsigned int slave_addr, u32 offset,
                u8 *buf, u32 count) {
  return i2c_write(dev, I2C_FLAG_ADDR16, slave_addr, offset, buf, count);
}

static void i2c_indirect_write(struct altera_i2c_dev *dev, u32 reg, u32 value) {
  u64 ctrl;
...
static int altera_i2c_wait_core_idle(struct altera_i2c_dev *dev) {
  ...
  while (i2c_indirect_read(dev, ALTERA_I2C_STATUS) & ALTERA_I2C_STAT_CORE) {
...
static void altera_i2c_enable_interrupt(struct altera_i2c_dev *dev, u32 mask,
                                        bool enable) {
  u32 status;
...
static int altera_i2c_fill_tx_fifo(struct altera_i2c_dev *dev) {
  ...
  tx_avail = dev->fifo_size - i2c_indirect_read(dev, ALTERA_I2C_TC_FIFO_LVL);
...
static int altera_i2c_wait_complete(struct altera_i2c_dev *dev, u32 *status) {
  int retry = 0;

  while (
      !((*status = i2c_indirect_read(dev, ALTERA_I2C_ISR)) & dev->isr_mask)) {
    if (retry++ > ALTERA_I2C_TIMEOUT_US)
      return -EBUSY;
...
static int altera_i2c_xfer_msg(struct altera_i2c_dev *dev,
                               struct i2c_msg *msg) {
  u32 int_mask = ALTERA_I2C_ISR_RXOF | ALTERA_I2C_ISR_ARB | ALTERA_I2C_ISR_NACK;
...
static int altera_i2c_xfer(struct altera_i2c_dev *dev, struct i2c_msg *msg,
                           int num) {
  int ret = 0;
  int i;
...
static void altera_i2c_hardware_init(struct altera_i2c_dev *dev) {
  ...
  dev_info(dev, "%s: rate=%uHz per_clk=%uMHz -> ratio=1:%u\n", __func__,
           dev->bus_clk_rate, clk_mhz, divisor);
