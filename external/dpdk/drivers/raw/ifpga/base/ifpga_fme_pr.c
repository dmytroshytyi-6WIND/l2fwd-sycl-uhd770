...
static u64 pr_err_handle(struct feature_fme_pr *fme_pr) {
...
static int fme_pr_write_init(struct ifpga_fme_hw *fme_dev,
  ...
  fme_pr = get_fme_feature_ioaddr_by_index(fme_dev, FME_FEATURE_ID_PR_MGMT);
  if (!fme_pr)
    return -EINVAL;
  ...
  if (fpga_wait_register_field(pr_reset_ack, fme_pr_ctl,
                               &fme_pr->ccip_fme_pr_control, PR_WAIT_TIMEOUT,
                               1)) {
  ...
  if (fpga_wait_register_field(pr_host_status, fme_pr_status,
                               &fme_pr->ccip_fme_pr_status, PR_WAIT_TIMEOUT,
                               1)) {
    dev_err(fme_dev, "maximum PR timeout\n");
    return -ETIMEDOUT;
  }
...
static int fme_pr_write(struct ifpga_fme_hw *fme_dev, int port_id,
  ...
  while (count > 0) {
    ...
    if (count >= fme_dev->pr_bandwidth) {
      ...
      case 4:
        ...
        writeq(fme_pr_data.csr, &fme_pr->ccip_fme_pr_data);
        break;
      default:
        ret = -EFAULT;
        goto done;
...
int do_pr(struct ifpga_hw *hw, u32 port_id, const char *buffer, u32 size,
  ...
  if (is_valid_bts(bts_hdr)) {
    ...
    header_size = sizeof(struct bts_header) + bts_hdr->metadata_len;
    if (size < header_size)
      return -EINVAL;
    size -= header_size;
