...
static int fme_dperf_get_clock(struct ifpga_fme_hw *fme, u64 *clock) {
  ...
  dperf = get_fme_feature_ioaddr_by_index(fme, FME_FEATURE_ID_GLOBAL_DPERF);
...
static u64 read_fabric_counter(struct ifpga_fme_hw *fme, u8 port_id,
  ...
  if (fpga_wait_register_field(event_code, ctr, &dperf->fab_ctr, DPERF_TIMEOUT,
                               1)) {
...
static int fme_dperf_get_fab_port_enable(struct ifpga_fme_hw *fme, u8 port_id,
                                         u64 *enable) {
  struct feature_fme_dperf *dperf;
  int status;
 ...
 */
static int fme_dperf_set_fab_port_enable(struct ifpga_fme_hw *fme, u8 port_id,
                                         u64 enable) {
  struct feature_fme_dfpmon_fab_ctl ctl;
  struct feature_fme_dperf *dperf;
  bool state;
...
static int fme_dperf_fab_get_prop(struct ifpga_feature *feature,
  ...
  case 0x3: /* PCIE0_WRITE */
    return fme_dperf_get_fab_port_pcie0_write(fme, sub, &prop->data);
  case 0x4: /* MMIO_READ */
