...
int max10_reg_read(struct intel_max10_device *dev, unsigned int reg,
                   unsigned int *val) {
  if (!dev)
    return -ENODEV;
  ...
  return spi_transaction_read(dev->spi_tran_dev, reg, 4, (unsigned char *)val);
}

int max10_reg_write(struct intel_max10_device *dev, unsigned int reg,
                    unsigned int val) {
  ...
  return spi_transaction_write(dev->spi_tran_dev, reg, 4,
                               (unsigned char *)&tmp);
}

int max10_sys_read(struct intel_max10_device *dev, unsigned int offset,
                   unsigned int *val) {
  if (!dev)
    return -ENODEV;
...
int max10_sys_write(struct intel_max10_device *dev, unsigned int offset,
                    unsigned int val) {
  if (!dev)
    return -ENODEV;
...
static struct max10_compatible_id *
max10_match_compatible(const char *fdt_root) {
...
static inline bool is_max10_pac_n3000(struct intel_max10_device *max10) {
  return max10->id && !strcmp(max10->id->compatible, MAX10_PAC_N3000);
}
...
static void max10_check_capability(struct intel_max10_device *max10) {
  ...
  if (is_max10_pac_n3000(max10)) {
    max10->flags |= MAX10_FLAGS_NO_I2C2 | MAX10_FLAGS_NO_BMCIMG_FLASH;
    dev_info(max10, "found %s card\n", max10->id->compatible);
  } else
    max10->flags |= MAX10_FLAGS_MAC_CACHE;
}

static int altera_nor_flash_read(struct intel_max10_device *dev, u32 offset,
                                 void *buffer, u32 len) {
  int word_len;
  int i;
  ...
  for (i = 0; i < word_len; i++) {
    ret = max10_reg_read(dev, offset + i * 4, &value);
    if (ret)
      return -EBUSY;
  ...
  if (dtb_magic != 0xEDFE0DD0) {
    ...
    if (spi->dtb) {
      if (*spi->dtb_sz_ptr < dt_size) {
        dev_warn(max10, "share memory for dtb is smaller than required %u\n",
                 dt_size);
      } else {
...
static int fdt_get_reg(const void *fdt, int node, unsigned int idx, u64 *start,
                       u64 *size) {
...
static bool sensor_reg_valid(struct sensor_reg *reg) { return !!reg->size; }

static int max10_add_sensor(struct intel_max10_device *dev,
                            struct raw_sensor_info *info,
...
static int max10_sensor_init(struct intel_max10_device *dev, int parent) {
  ...
  fdt_for_each_subnode(offset, fdt_root, parent) {
    ...
    for (i = SENSOR_REG_VALUE; i < SENSOR_REG_MAX; i++) {
      ret = fdt_get_named_reg(fdt_root, offset, sensor_reg_name[i], &start,
                              &size);
      if (ret) {
      ...
      if (parent)
        ...
        raw->regs[i].regoff = start - MAX10_BASE_ADDR;
    ...
    dev_debug(dev, "found sensor from DTB: %s: %s: %u: %u\n", raw->name,
              raw->type, raw->id, raw->multiplier);
...
static int check_max10_version(struct intel_max10_device *dev) {
  ...
  if (!max10_reg_read(dev, MAX10_SEC_BASE_ADDR + MAX10_BUILD_VER, &v)) {
    if (v != 0xffffffff) {
...
static int max10_staging_area_init(struct intel_max10_device *dev) {
  ...
  if (!fdt_root) {
    dev_debug(dev, "skip staging area init as not find Device Tree\n");
    return -ENODEV;
  }
...
static int max10_secure_hw_init(struct intel_max10_device *dev) {
...
static int max10_non_secure_hw_init(struct intel_max10_device *dev) {
  max10_check_capability(dev);
...
intel_max10_device_probe(struct altera_spi_device *spi, int chipselect) {
