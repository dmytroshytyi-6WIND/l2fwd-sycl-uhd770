...
struct opae_manager_ops {
  int (*flash)(struct opae_manager *mgr, int id, const char *buffer, u32 size,
               u64 *status);
  ...
  int (*get_sensor_value)(struct opae_manager *mgr,
                          struct opae_sensor_info *sensor, unsigned int *value);
  int (*get_board_info)(struct opae_manager *mgr,
                        struct opae_board_info **info);
  int (*get_uuid)(struct opae_manager *mgr, struct uuid *uuid);
  int (*update_flash)(struct opae_manager *mgr, const char *image, u64 *status);
...
struct opae_manager_networking_ops {
  ...
  int (*eth_group_reg_read)(struct opae_manager *mgr, u8 group_id, u8 type,
                            u8 index, u16 addr, u32 *data);
  int (*eth_group_reg_write)(struct opae_manager *mgr, u8 group_id, u8 type,
                             u8 index, u16 addr, u32 data);
...
struct opae_accelerator_ops {
  int (*read)(struct opae_accelerator *acc, unsigned int region_idx, u64 offset,
  ...
  int (*set_irq)(struct opae_accelerator *acc, u32 start, u32 count,
                 s32 evtfds[]);
