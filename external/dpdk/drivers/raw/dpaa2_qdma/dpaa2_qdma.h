 ...
 */
#define QDMA_FLE_SINGLE_POOL_SIZE                                              \
...
#define QDMA_FLE_FLE_OFFSET (QDMA_FLE_JOB_NB_OFFSET + sizeof(uint64_t))

#define QDMA_FLE_SDD_OFFSET                                                    \
  (QDMA_FLE_FLE_OFFSET + sizeof(struct qbman_fle) * DPAA2_QDMA_MAX_FLE)

#define QDMA_FLE_SG_ENTRY_OFFSET                                               \
  (QDMA_FLE_SDD_OFFSET + sizeof(struct qdma_sdd) * DPAA2_QDMA_MAX_SDD)
...
typedef int(qdma_set_fd_t)(struct qdma_virt_queue *qdma_vq, struct qbman_fd *fd,
                           struct rte_qdma_job **job, uint16_t nb_jobs);

typedef int(qdma_dequeue_multijob_t)(struct qdma_virt_queue *qdma_vq,
                                     uint16_t *vq_id, struct rte_qdma_job **job,
                                     uint16_t nb_jobs);

typedef int(qdma_enqueue_multijob_t)(struct qdma_virt_queue *qdma_vq,
...
static inline struct qdma_device *QDMA_DEV_OF_VQ(struct qdma_virt_queue *vq) {
