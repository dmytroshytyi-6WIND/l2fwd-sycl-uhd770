...
static inline int qdma_populate_fd_pci(phys_addr_t src, phys_addr_t dest,
...
static inline int qdma_populate_fd_ddr(phys_addr_t src, phys_addr_t dest,
                                       uint32_t len, struct qbman_fd *fd,
                                       int ser) {
...
static void dpaa2_qdma_populate_fle(struct qbman_fle *fle, uint64_t fle_iova,
                                    struct rte_qdma_rbp *rbp, uint64_t src,
                                    uint64_t dest, size_t len, uint32_t flags,
                                    uint32_t fmt) {
  struct qdma_sdd *sdd;
  uint64_t sdd_iova;

  sdd = (struct qdma_sdd *)((uintptr_t)(uint64_t)fle - QDMA_FLE_FLE_OFFSET +
                            QDMA_FLE_SDD_OFFSET);
...
static inline int dpdmai_dev_set_fd_us(struct qdma_virt_queue *qdma_vq,
                                       struct qbman_fd *fd,
  ...
  size_t iova;
  int ret = 0, loop;
  int ser = (qdma_vq->flags & RTE_QDMA_VQ_NO_RESPONSE) ? 0 : 1;
    ...
    *ppjob = job[loop];
    ...
    if ((rbp->drbp == 1) || (rbp->srbp == 1))
      ret = qdma_populate_fd_pci((phys_addr_t)job[loop]->src,
                                 (phys_addr_t)job[loop]->dest, job[loop]->len,
                                 &fd[loop], rbp, ser);
    else
...
static uint32_t qdma_populate_sg_entry(struct rte_qdma_job **jobs,
...
dpdmai_dev_set_multi_fd_lf_no_rsp(struct qdma_virt_queue *qdma_vq,
  ...
  for (i = 0; i < nb_jobs; i++) {
    ...
    ppjob = (struct rte_qdma_job **)((uintptr_t)(uint64_t)elem +
                                     QDMA_FLE_SINGLE_JOB_OFFSET);
    *ppjob = job[i];

    job[i]->vq_id = qdma_vq->vq_id;

    fle = (struct qbman_fle *)((uintptr_t)(uint64_t)elem + QDMA_FLE_FLE_OFFSET);
    ...
    dpaa2_qdma_populate_fle(fle, fle_iova, rbp, job[i]->src, job[i]->dest,
                            job[i]->len, job[i]->flags,
...
static inline int dpdmai_dev_set_multi_fd_lf(struct qdma_virt_queue *qdma_vq,
                                             struct qbman_fd *fd,
                                             struct rte_qdma_job **job,
                                             uint16_t nb_jobs) {
  struct rte_qdma_rbp *rbp = &qdma_vq->rbp;
  ...
  for (i = 0; i < nb_jobs; i++) {
    ...
    ppjob = (struct rte_qdma_job **)((uintptr_t)(uint64_t)elem[i] +
                                     QDMA_FLE_SINGLE_JOB_OFFSET);
    *ppjob = job[i];

    job[i]->vq_id = qdma_vq->vq_id;

    fle = (struct qbman_fle *)((uintptr_t)(uint64_t)elem[i] +
                               QDMA_FLE_FLE_OFFSET);
    fle_iova = elem_iova + QDMA_FLE_FLE_OFFSET;
...
static inline int dpdmai_dev_set_sg_fd_lf(struct qdma_virt_queue *qdma_vq,
                                          struct qbman_fd *fd,
                                          struct rte_qdma_job **job,
                                          uint16_t nb_jobs) {
  struct rte_qdma_rbp *rbp = &qdma_vq->rbp;
  ...
  /* Set the metadata */
  /* Save job context. */
  *((uint16_t *)((uintptr_t)(uint64_t)elem + QDMA_FLE_JOB_NB_OFFSET)) = nb_jobs;
  ppjob = (struct rte_qdma_job **)((uintptr_t)(uint64_t)elem +
                                   QDMA_FLE_SG_JOBS_OFFSET);
  for (i = 0; i < nb_jobs; i++)
    ppjob[i] = job[i];
  ...
  /* Populate FLE */
  if (likely(nb_jobs > 1)) {
    src_sge = (struct qdma_sg_entry *)((uintptr_t)(uint64_t)elem +
                                       QDMA_FLE_SG_ENTRY_OFFSET);
    dst_sge = src_sge + DPAA2_QDMA_MAX_SG_NB;
    src = elem_iova + QDMA_FLE_SG_ENTRY_OFFSET;
    dst = src + DPAA2_QDMA_MAX_SG_NB * sizeof(struct qdma_sg_entry);
  ...
  dpaa2_qdma_populate_fle(fle, fle_iova, rbp, src, dst, len, flags, fmt);
...
dpdmai_dev_get_single_job_lf(struct qdma_virt_queue *qdma_vq,
  ...
  *nb_jobs = 1;
  ppjob =
      (struct rte_qdma_job **)((uintptr_t)(uint64_t)fle - QDMA_FLE_FLE_OFFSET +
                               QDMA_FLE_SINGLE_JOB_OFFSET);
  ...
  *job = *ppjob;
  ...
  rte_mempool_put(qdma_vq->fle_pool,
                  (void *)((uintptr_t)(uint64_t)fle - QDMA_FLE_FLE_OFFSET));

  return (*job)->vq_id;
}

static inline uint16_t dpdmai_dev_get_sg_job_lf(struct qdma_virt_queue *qdma_vq,
                                                const struct qbman_fd *fd,
                                                struct rte_qdma_job **job,
                                                uint16_t *nb_jobs) {
  struct qbman_fle *fle;
   ...
   */
  fle = (struct qbman_fle *)DPAA2_IOVA_TO_VADDR(DPAA2_GET_FD_ADDR(fd));
  *nb_jobs = *((uint16_t *)((uintptr_t)(uint64_t)fle - QDMA_FLE_FLE_OFFSET +
                            QDMA_FLE_JOB_NB_OFFSET));
  ppjob =
      (struct rte_qdma_job **)((uintptr_t)(uint64_t)fle - QDMA_FLE_FLE_OFFSET +
                               QDMA_FLE_SG_JOBS_OFFSET);
  status = (DPAA2_GET_FD_ERR(fd) << 8) | (DPAA2_GET_FD_FRC(fd) & 0xFF);
...
static int dpdmai_dev_dequeue_multijob_prefetch(struct qdma_virt_queue *qdma_vq,
                                                uint16_t *vq_id,
                                                struct rte_qdma_job **job,
                                                uint16_t nb_jobs) {
  ...
  do {
    ...
    vqid = qdma_vq->get_job(qdma_vq, fd, &job[num_rx], &num_rx_ret);
    if (vq_id)
      vq_id[num_rx] = vqid;
...
static int dpdmai_dev_dequeue_multijob_no_prefetch(
    struct qdma_virt_queue *qdma_vq, uint16_t *vq_id, struct rte_qdma_job **job,
    uint16_t nb_jobs) {
  struct qdma_hw_queue *qdma_pq = qdma_vq->hw_queue;
  struct dpaa2_dpdmai_dev *dpdmai_dev = qdma_pq->dpdmai_dev;
  uint16_t rxq_id = qdma_pq->queue_id;
...
static int dpdmai_dev_enqueue_multi(struct qdma_virt_queue *qdma_vq,
                                    struct rte_qdma_job **job,
                                    uint16_t nb_jobs) {
  struct qdma_hw_queue *qdma_pq = qdma_vq->hw_queue;
  struct dpaa2_dpdmai_dev *dpdmai_dev = qdma_pq->dpdmai_dev;
  ...
  if (qdma_vq->flags & RTE_QDMA_VQ_FD_SG_FORMAT) {
    uint16_t fd_nb;
    uint16_t sg_entry_nb =
        nb_jobs > DPAA2_QDMA_MAX_SG_NB ? DPAA2_QDMA_MAX_SG_NB : nb_jobs;
    uint16_t job_idx = 0;
    uint16_t fd_sg_nb[8];
    ...
    for (loop = 0; loop < fd_nb; loop++) {
      ret = qdma_vq->set_fd(qdma_vq, &fd[loop], &job[job_idx], sg_entry_nb);
      if (unlikely(ret < 0))
        return 0;
    ...
    while (enqueue_loop < fd_nb) {
      ret = qbman_swp_enqueue_multiple(swp, &eqdesc, &fd[enqueue_loop], NULL,
                                       fd_nb - enqueue_loop);
      if (unlikely(ret < 0)) {
        retry_count++;
  ...
  while (nb_jobs > 0) {
    num_to_send = (nb_jobs > dpaa2_eqcr_size) ? dpaa2_eqcr_size : nb_jobs;

    ret = qdma_vq->set_fd(qdma_vq, &fd[num_tx], &job[num_tx], num_to_send);
    if (unlikely(ret < 0))
      break;
    ...
    while (enqueue_loop < loop) {
      ret = qbman_swp_enqueue_multiple(swp, &eqdesc, &fd[num_tx + enqueue_loop],
                                       NULL, loop - enqueue_loop);
      if (unlikely(ret < 0)) {
        retry_count++;
        if (retry_count > DPAA2_MAX_TX_RETRY_COUNT)
          return num_tx;
...
static struct qdma_hw_queue *alloc_hw_queue(uint32_t lcore_id) {
...
static void free_hw_queue(struct qdma_hw_queue *queue) {
  DPAA2_QDMA_FUNC_TRACE();

  queue->num_users--;
}

static struct qdma_hw_queue *get_hw_queue(struct qdma_device *qdma_dev,
                                          uint32_t lcore_id) {
...
static void put_hw_queue(struct qdma_hw_queue *queue) {
  struct qdma_per_core_info *core_info;
...
static int dpaa2_qdma_attr_get(struct rte_rawdev *rawdev,
...
static int dpaa2_qdma_reset(struct rte_rawdev *rawdev) {
  ...
  if (qdma_dev->state == 1) {
    DPAA2_QDMA_ERR("Device is in running state. Stop before reset.");
    return -EBUSY;
  }
  ...
  for (i = 0; i < qdma_dev->max_vqs; i++) {
    if (qdma_dev->vqs[i].in_use &&
        (qdma_dev->vqs[i].num_enqueues != qdma_dev->vqs[i].num_dequeues)) {
  ...
  /* Reset per core info */
  memset(&qdma_core_info, 0, sizeof(struct qdma_per_core_info) * RTE_MAX_LCORE);
...
static int dpaa2_qdma_configure(const struct rte_rawdev *rawdev,
  ...
  if (config_size != sizeof(*qdma_config)) {
    DPAA2_QDMA_ERR("Config size mismatch. Expected %" PRIu64 ", Got: %" PRIu64,
                   (uint64_t)(sizeof(*qdma_config)), (uint64_t)config_size);
    return -EINVAL;
  }

  /* In case QDMA device is not in stopped state, return -EBUSY */
  if (qdma_dev->state == 1) {
    DPAA2_QDMA_ERR("Device is in running state. Stop before config.");
    return -1;
  }
  ...
  qdma_dev->max_hw_queues_per_core = qdma_config->max_hw_queues_per_core;

  /* Allocate Virtual Queues */
  sprintf(name, "qdma_%d_vq", rawdev->dev_id);
  qdma_dev->vqs =
      rte_malloc(name, (sizeof(struct qdma_virt_queue) * qdma_config->max_vqs),
                 RTE_CACHE_LINE_SIZE);
  if (!qdma_dev->vqs) {
...
static int dpaa2_qdma_start(struct rte_rawdev *rawdev) {
  struct dpaa2_dpdmai_dev *dpdmai_dev = rawdev->dev_private;
  struct qdma_device *qdma_dev = dpdmai_dev->qdma_dev;
...
static int dpaa2_qdma_queue_setup(struct rte_rawdev *rawdev,
  ...
  if (conf_size != sizeof(*q_config)) {
    DPAA2_QDMA_ERR("Config size mismatch. Expected %" PRIu64 ", Got: %" PRIu64,
                   (uint64_t)(sizeof(*q_config)), (uint64_t)conf_size);
    return -EINVAL;
  }
  ...
  if (q_config->flags & RTE_QDMA_VQ_FD_SG_FORMAT) {
    if (!(q_config->flags & RTE_QDMA_VQ_EXCLUSIVE_PQ)) {
      DPAA2_QDMA_ERR("qDMA SG format only supports physical queue!");
      rte_spinlock_unlock(&qdma_dev->lock);
      return -ENODEV;
    }
    if (!(q_config->flags & RTE_QDMA_VQ_FD_LONG_FORMAT)) {
      DPAA2_QDMA_ERR("qDMA SG format only supports long FD format!");
      rte_spinlock_unlock(&qdma_dev->lock);
      return -ENODEV;
    }
  ...
  if (q_config->flags & RTE_QDMA_VQ_EXCLUSIVE_PQ) {
    ...
    qdma_dev->vqs[i].status_ring = rte_ring_create(
        ring_name, qdma_dev->fle_queue_pool_cnt, rte_socket_id(), 0);
    ...
    qdma_dev->vqs[i].hw_queue = get_hw_queue(qdma_dev, q_config->lcore_id);
  ...
  snprintf(pool_name, sizeof(pool_name), "qdma_fle_pool%u_queue%d", getpid(),
           i);
  qdma_dev->vqs[i].fle_pool =
      rte_mempool_create(pool_name, qdma_dev->fle_queue_pool_cnt, pool_size,
  ...
  if (q_config->flags & RTE_QDMA_VQ_FD_LONG_FORMAT) {
    if (q_config->flags & RTE_QDMA_VQ_FD_SG_FORMAT) {
      ...
      if (q_config->flags & RTE_QDMA_VQ_NO_RESPONSE)
        qdma_dev->vqs[i].set_fd = dpdmai_dev_set_multi_fd_lf_no_rsp;
      else
  ...
  if (dpaa2_get_devargs(rawdev->device->devargs, DPAA2_QDMA_NO_PREFETCH) ||
    ...
    qdma_dev->vqs[i].dequeue_job = dpdmai_dev_dequeue_multijob_no_prefetch;
    DPAA2_QDMA_INFO("No Prefetch RX Mode enabled");
  } else {
    qdma_dev->vqs[i].dequeue_job = dpdmai_dev_dequeue_multijob_prefetch;
  }

  qdma_dev->vqs[i].enqueue_job = dpdmai_dev_enqueue_multi;

  if (q_config->rbp != NULL)
    memcpy(&qdma_dev->vqs[i].rbp, q_config->rbp, sizeof(struct rte_qdma_rbp));
...
static int dpaa2_qdma_enqueue(struct rte_rawdev *rawdev,
                              __rte_unused struct rte_rawdev_buf **buffers,
                              unsigned int nb_jobs, rte_rawdev_obj_t context) {
  struct dpaa2_dpdmai_dev *dpdmai_dev = rawdev->dev_private;
...
static int dpaa2_qdma_dequeue(struct rte_rawdev *rawdev,
                              __rte_unused struct rte_rawdev_buf **buffers,
                              unsigned int nb_jobs, rte_rawdev_obj_t cntxt) {
  struct dpaa2_dpdmai_dev *dpdmai_dev = rawdev->dev_private;
  struct qdma_device *qdma_dev = dpdmai_dev->qdma_dev;
  ...
  if (qdma_vq->exclusive_hw_queue) {
    ...
    ret = qdma_vq->dequeue_job(qdma_vq, NULL, context->job, nb_jobs);
    if (ret < 0) {
      DPAA2_QDMA_ERR("Dequeue from DPDMAI device failed: %d", ret);
      return ret;
    }
    ...
    if (ring_count < nb_jobs) {
      ...
      ret = qdma_vq->dequeue_job(qdma_vq, temp_vq_id, context->job, nb_jobs);
      for (i = 0; i < ret; i++) {
        temp_qdma_vq = &qdma_dev->vqs[temp_vq_id[i]];
        rte_ring_enqueue(temp_qdma_vq->status_ring, (void *)(context->job[i]));
      }
      ring_count = rte_ring_count(qdma_vq->status_ring);
    }
    ...
    if (ring_count) {
       ...
       */
      ret = rte_ring_dequeue_bulk(qdma_vq->status_ring, (void **)context->job,
                                  ring_count, NULL);
      if (ret)
        qdma_vq->num_dequeues += ret;
    }
...
void rte_qdma_vq_stats(struct rte_rawdev *rawdev, uint16_t vq_id,
  ...
  if (qdma_vq->in_use) {
    ...
    vq_status->num_pending_jobs =
        vq_status->num_enqueues - vq_status->num_dequeues;
  }
}

static int dpaa2_qdma_queue_release(struct rte_rawdev *rawdev, uint16_t vq_id) {
  struct dpaa2_dpdmai_dev *dpdmai_dev = rawdev->dev_private;
  struct qdma_device *qdma_dev = dpdmai_dev->qdma_dev;
...
static void dpaa2_qdma_stop(struct rte_rawdev *rawdev) {
  struct dpaa2_dpdmai_dev *dpdmai_dev = rawdev->dev_private;
  struct qdma_device *qdma_dev = dpdmai_dev->qdma_dev;
...
static int dpaa2_qdma_close(struct rte_rawdev *rawdev) {
  DPAA2_QDMA_FUNC_TRACE();
...
static int add_hw_queues_to_list(struct dpaa2_dpdmai_dev *dpdmai_dev) {
  struct qdma_hw_queue *queue;
  int i;
  ...
  for (i = 0; i < dpdmai_dev->num_queues; i++) {
    ...
    if (!queue) {
      DPAA2_QDMA_ERR("Memory allocation failed for QDMA queue");
      return -ENOMEM;
    }
...
static void remove_hw_queues_from_list(struct dpaa2_dpdmai_dev *dpdmai_dev) {
  struct qdma_hw_queue *queue = NULL;
...
static int dpaa2_dpdmai_dev_uninit(struct rte_rawdev *rawdev) {
  struct dpaa2_dpdmai_dev *dpdmai_dev = rawdev->dev_private;
  int ret, i;
  ...
  ret = dpdmai_disable(&dpdmai_dev->dpdmai, CMD_PRI_LOW, dpdmai_dev->token);
  if (ret)
...
static int dpaa2_dpdmai_dev_init(struct rte_rawdev *rawdev, int dpdmai_id) {
  struct dpaa2_dpdmai_dev *dpdmai_dev = rawdev->dev_private;
  ...
  ret = dpdmai_open(&dpdmai_dev->dpdmai, CMD_PRI_LOW, dpdmai_dev->dpdmai_id,
                    &dpdmai_dev->token);
  if (ret) {
  ...
  if (ret) {
    DPAA2_QDMA_ERR("dpdmai get attributes failed with err: %d", ret);
    goto init_err;
  }
  ...
  for (i = 0; i < dpdmai_dev->num_queues; i++) {
    ...
    ret = dpdmai_set_rx_queue(&dpdmai_dev->dpdmai, CMD_PRI_LOW,
                              dpdmai_dev->token, i, 0, &rx_queue_cfg);
    if (ret) {
      DPAA2_QDMA_ERR("Setting Rx queue failed with err: %d", ret);
      goto init_err;
    }

    /* Allocate DQ storage for the DPDMAI Rx queues */
    rxq = &(dpdmai_dev->rx_queue[i]);
    rxq->q_storage = rte_malloc(
  ...
  for (i = 0; i < dpdmai_dev->num_queues; i++) {
    ...
    if (ret) {
      DPAA2_QDMA_ERR("Reading device failed with err: %d", ret);
      goto init_err;
    }
  ...
  /* Enable the device */
  ret = dpdmai_enable(&dpdmai_dev->dpdmai, CMD_PRI_LOW, dpdmai_dev->token);
  if (ret) {
  ...
  if (!dpaa2_coherent_no_alloc_cache) {
    if (dpaa2_svr_family == SVR_LX2160A) {
      dpaa2_coherent_no_alloc_cache = DPAA2_LX2_COHERENT_NO_ALLOCATE_CACHE;
      dpaa2_coherent_alloc_cache = DPAA2_LX2_COHERENT_ALLOCATE_CACHE;
    } else {
      dpaa2_coherent_no_alloc_cache = DPAA2_COHERENT_NO_ALLOCATE_CACHE;
      dpaa2_coherent_alloc_cache = DPAA2_COHERENT_ALLOCATE_CACHE;
    }
  }
...
static int rte_dpaa2_qdma_probe(struct rte_dpaa2_driver *dpaa2_drv,
  ...
  rawdev = rte_rawdev_pmd_allocate(
      dpaa2_dev->device.name, sizeof(struct dpaa2_dpdmai_dev), rte_socket_id());
  if (!rawdev) {
...
static int rte_dpaa2_qdma_remove(struct rte_dpaa2_device *dpaa2_dev) {
...
RTE_PMD_REGISTER_PARAM_STRING(dpaa2_qdma, "no_prefetch=<int> ");
