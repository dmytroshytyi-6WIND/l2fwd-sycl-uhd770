...
#define SKELDEV_TEST_INFO(fmt, args...) SKELDEV_LOGS(INFO, fmt, ##args)
#define SKELDEV_TEST_DEBUG(fmt, args...) SKELDEV_LOGS(DEBUG, fmt, ##args)
...
static void testsuite_teardown(void) { local_teardown(); }

static void local_teardown(void) { rte_vdev_uninit(TEST_DEV_NAME); }

static int test_rawdev_count(void) {
  uint8_t count;
  count = rte_rawdev_count();
...
static int test_rawdev_get_dev_id(void) {
  int ret;
  ret = rte_rawdev_get_dev_id("invalid_rawdev_device");
  RTE_TEST_ASSERT_FAIL(ret, "Expected <0 for invalid dev name ret=%d", ret);
  return TEST_SUCCESS;
}

static int test_rawdev_socket_id(void) {
  int socket_id;
  socket_id = rte_rawdev_socket_id(test_dev_id);
  RTE_TEST_ASSERT(socket_id != -EINVAL, "Failed to get socket_id %d",
                  socket_id);
  socket_id = rte_rawdev_socket_id(RTE_RAWDEV_MAX_DEVS);
  RTE_TEST_ASSERT(socket_id == -EINVAL, "Expected -EINVAL %d", socket_id);

  return TEST_SUCCESS;
}

static int test_rawdev_info_get(void) {
  int ret;
...
static int test_rawdev_configure(void) {
  int ret;
  struct rte_rawdev_info rdev_info = {0};
  ...
  RTE_TEST_ASSERT(ret == -EINVAL, "Null configure; Expected -EINVAL, got %d",
                  ret);

  /* Valid configuration test */
  rdev_conf_set.num_queues = 1;
  rdev_conf_set.capabilities = SKELETON_CAPA_FW_LOAD | SKELETON_CAPA_FW_RESET;

  rdev_info.dev_private = &rdev_conf_set;
  ret = rte_rawdev_configure(test_dev_id, (rte_rawdev_obj_t)&rdev_info,
  ...
  RTE_TEST_ASSERT_EQUAL(rdev_conf_set.num_queues, rdev_conf_get.num_queues,
                        "Configuration test failed; num_queues (%d)(%d)",
                        rdev_conf_set.num_queues, rdev_conf_get.num_queues);
  RTE_TEST_ASSERT_EQUAL(rdev_conf_set.capabilities, rdev_conf_get.capabilities,
                        "Configuration test failed; capabilities");

  return TEST_SUCCESS;
}

static int test_rawdev_queue_default_conf_get(void) {
  int ret, i;
  struct rte_rawdev_info rdev_info = {0};
  struct skeleton_rawdev_conf rdev_conf_get = {0};
  ...
  for (i = 0; i < rdev_conf_get.num_queues; i++) {
    ...
    RTE_TEST_ASSERT_EQUAL(q.depth, SKELETON_QUEUE_DEF_DEPTH,
                          "Invalid default depth of queue (%d)", q.depth);
    RTE_TEST_ASSERT_EQUAL(q.state, SKELETON_QUEUE_DETACH,
                          "Invalid default state of queue (%d)", q.state);
  }

  return TEST_SUCCESS;
}

static int test_rawdev_queue_count(void) {
...
static int test_rawdev_queue_setup(void) {
  int ret;
  struct rte_rawdev_info rdev_info = {0};
  struct skeleton_rawdev_conf rdev_conf_get = {0};
 ...
 */
static int test_rawdev_queue_release(void) {
  int ret;
  struct skeleton_rawdev_queue qget = {0};
...
static int test_rawdev_attr_set_get(void) {
  int ret;
...
static int test_rawdev_start_stop(void) {
  int ret;
  struct rte_rawdev_info rdev_info = {0};
  struct skeleton_rawdev_conf rdev_conf_get = {0};
...
static int test_rawdev_enqdeq(void) {
  int ret;
  ...
  ret = rte_rawdev_enqueue_buffers(
      test_dev_id, (struct rte_rawdev_buf **)&buffers, count, &queue_id);
  RTE_TEST_ASSERT_EQUAL((unsigned int)ret, count, "Unable to enqueue buffers");

  deq_buffers = malloc(sizeof(struct rte_rawdev_buf) * count);
  if (!deq_buffers)
    goto cleanup;

  ret = rte_rawdev_dequeue_buffers(
      test_dev_id, (struct rte_rawdev_buf **)&deq_buffers, count, &queue_id);
  RTE_TEST_ASSERT_EQUAL((unsigned int)ret, count, "Unable to dequeue buffers");
...
static void skeldev_test_run(int (*setup)(void), void (*teardown)(void),
...
int test_rawdev_skeldev(uint16_t dev_id) {
  ...
  SKELDEV_TEST_RUN(test_rawdev_queue_setup, NULL, test_rawdev_queue_release);
