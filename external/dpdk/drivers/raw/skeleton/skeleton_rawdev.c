...
static int skeleton_rawdev_start(struct rte_rawdev *dev) {
  ...
  if (fw_state == SKELETON_FW_LOADED && device_state == SKELETON_DEV_STOPPED) {
...
static void reset_queues(struct skeleton_rawdev *skeldev) {
  int i;
...
static void reset_attribute_table(struct skeleton_rawdev *skeldev) {
  int i;
...
static int skeleton_rawdev_queue_def_conf(struct rte_rawdev *dev,
  ...
  if (queue_id < SKELETON_MAX_QUEUES)
    rte_memcpy(queue_conf, skelq, sizeof(struct skeleton_rawdev_queue));
...
static int skeleton_rawdev_queue_setup(struct rte_rawdev *dev,
  ...
  if (skeldev->num_queues > queue_id && q->depth < SKELETON_QUEUE_MAX_DEPTH) {
    rte_memcpy(q, queue_conf, sizeof(struct skeleton_rawdev_queue));
  ...
  for (i = 0; i < SKELETON_MAX_ATTRIBUTES; i++) {
    ...
    if (!strncmp(skeldev->attr[i].name, attr_name,
      ...
      *attr_value = skeldev->attr[i].value;
      ...
      SKELETON_PMD_DEBUG("Attribute (%s) Value (%" PRIu64 ")", attr_name,
                         *attr_value);
      break;
    }
  }
...
static int skeleton_rawdev_firmware_version_get(struct rte_rawdev *dev,
...
static int skeleton_rawdev_create(const char *name,
                                  struct rte_vdev_device *vdev, int socket_id) {
  int ret = 0, i;
  ...
  rawdev =
      rte_rawdev_pmd_allocate(name, sizeof(struct skeleton_rawdev), socket_id);
  if (rawdev == NULL) {
...
static int skeleton_rawdev_destroy(const char *name) {
  int ret;
...
static int skeldev_get_selftest(const char *key __rte_unused, const char *value,
                                void *opaque) {
  int *flag = opaque;
  *flag = atoi(value);
  return 0;
...
static int skeldev_parse_vdev_args(struct rte_vdev_device *vdev) {
  int selftest = 0;
  const char *name;
  const char *params;

  static const char *const args[] = {SKELETON_SELFTEST_ARG, NULL};
  ...
  if (params != NULL && params[0] != '\0') {
    ...
    if (!kvlist) {
      SKELETON_PMD_INFO("Ignoring unsupported params supplied '%s'", name);
    } else {
      int ret = rte_kvargs_process(kvlist, SKELETON_SELFTEST_ARG,
                                   skeldev_get_selftest, &selftest);
      if (ret != 0 || (selftest < 0 || selftest > 1)) {
        SKELETON_PMD_ERR("%s: Error in parsing args", name);
...
static int skeleton_rawdev_probe(struct rte_vdev_device *vdev) {
  const char *name;
  ...
  if (skeldev_init_once) {
    SKELETON_PMD_ERR("Multiple instance not supported for %s", name);
    return -EINVAL;
  }
...
static int skeleton_rawdev_remove(struct rte_vdev_device *vdev) {
  const char *name;
  int ret;
...
static struct rte_vdev_driver skeleton_pmd_drv = {
    .probe = skeleton_rawdev_probe, .remove = skeleton_rawdev_remove};
