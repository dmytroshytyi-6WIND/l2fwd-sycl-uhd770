...
ifpga_find_afu_dev(const struct rte_rawdev *rdev,
  ...
  TAILQ_FOREACH(afu_dev, &ifpga_afu_dev_list, next) {
    if (afu_dev->rawdev == rdev && !ifpga_afu_id_cmp(&afu_dev->id, afu_id))
      return afu_dev;
  }
  return NULL;
}

struct rte_afu_device *rte_ifpga_find_afu_by_name(const char *name) {
  struct rte_afu_device *afu_dev = NULL;
 ...
 * list
 */
static struct rte_afu_device *ifpga_scan_one(struct rte_rawdev *rawdev,
  ...
  if (rte_kvargs_count(kvlist, IFPGA_ARG_PORT) == 1) {
    ...
    IFPGA_BUS_ERR("arg %s is mandatory for ifpga bus", IFPGA_ARG_PORT);
    goto end;
  }

  if (rte_kvargs_count(kvlist, IFPGA_AFU_BTS) == 1) {
    if (rte_kvargs_process(kvlist, IFPGA_AFU_BTS, &rte_ifpga_get_string_arg,
                           &path) < 0) {
  ...
  afu_dev->intr_handle = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_PRIVATE);
  ...
  if (rawdev->dev_ops && rawdev->dev_ops->dev_start &&
      rawdev->dev_ops->dev_start(rawdev))
    goto end;

  strlcpy(afu_pr_conf.bs_path, path, sizeof(afu_pr_conf.bs_path));
  if (rawdev->dev_ops && rawdev->dev_ops->firmware_load &&
      rawdev->dev_ops->firmware_load(rawdev, &afu_pr_conf)) {
...
static int ifpga_scan(void) {
  ...
  RTE_EAL_DEVARGS_FOREACH(IFPGA_ARG_NAME, devargs) {
    ...
    if (rte_kvargs_count(kvlist, IFPGA_ARG_NAME) == 1) {
      if (rte_kvargs_process(kvlist, IFPGA_ARG_NAME, &rte_ifpga_get_string_arg,
                             &name) < 0) {
        IFPGA_BUS_ERR("error to parse %s", IFPGA_ARG_NAME);
        goto end;
      }
    } else {
      IFPGA_BUS_ERR("arg %s is mandatory for ifpga bus", IFPGA_ARG_NAME);
      goto end;
    }
...
static int rte_afu_match(const struct rte_afu_driver *afu_drv,
  ...
  for (id_table = afu_drv->id_table;
       ((id_table->uuid_low != 0) && (id_table->uuid_high != 0)); id_table++) {
    /* check if device's identifiers match the driver's ones */
    if ((id_table->uuid_low != afu_dev->id.uuid.uuid_low) ||
        id_table->uuid_high != afu_dev->id.uuid.uuid_high)
      continue;
...
static int ifpga_probe_one_driver(struct rte_afu_driver *drv,
...
static int ifpga_probe_all_drivers(struct rte_afu_device *afu_dev) {
...
static int ifpga_plug(struct rte_device *dev) {
...
static int ifpga_unplug(struct rte_device *dev) {
  struct rte_afu_device *afu_dev = NULL;
  int ret;
...
static struct rte_device *ifpga_find_device(const struct rte_device *start,
                                            rte_dev_cmp_t cmp,
                                            const void *data) {
  struct rte_afu_device *afu_dev;
...
static int ifpga_parse(const char *name, void *addr) {
  int *out = addr;
  struct rte_rawdev *rawdev = NULL;
  ...
  if (str_port_len < 8 && str_port_len > 0) {
  ...
  if ((port < IFPGA_BUS_DEV_PORT_MAX) && rawdev && (addr != NULL))
    *out = port;

  if ((port < IFPGA_BUS_DEV_PORT_MAX) && rawdev)
    return 0;
  else
    return 1;
}
