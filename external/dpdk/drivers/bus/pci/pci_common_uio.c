...
static int pci_uio_map_secondary(struct rte_pci_device *dev) {
  int fd, i, j;
  ...
  TAILQ_FOREACH(uio_res, uio_res_list, next) {
    ...
    for (i = 0; i != uio_res->nb_maps; i++) {
      ...
      void *mapaddr = pci_map_resource(uio_res->maps[i].addr, fd,
      ...
      if (mapaddr != uio_res->maps[i].addr) {
        RTE_LOG(ERR, EAL,
                ...
                uio_res->maps[i].path, uio_res->maps[i].addr);
        if (mapaddr != NULL) {
          /* unmap addrs correctly mapped */
          for (j = 0; j < i; j++)
            pci_unmap_resource(uio_res->maps[j].addr,
                               (size_t)uio_res->maps[j].size);
          /* unmap addr wrongly mapped */
          pci_unmap_resource(mapaddr, (size_t)uio_res->maps[i].size);
        }
        return -1;
      }
...
int pci_uio_remap_resource(struct rte_pci_device *dev) {
  ...
  for (i = 0; i != PCI_MAX_RESOURCE; i++) {
    ...
    map_address = mmap(dev->mem_resource[i].addr,
                       (size_t)dev->mem_resource[i].len, PROT_READ | PROT_WRITE,
                       MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
    if (map_address == MAP_FAILED) {
      RTE_LOG(ERR, EAL, "Cannot remap resource for device %s\n", dev->name);
      return -1;
    }
    RTE_LOG(INFO, EAL, "Successful remap resource for device %s\n", dev->name);
  }
