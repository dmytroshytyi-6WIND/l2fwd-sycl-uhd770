...
int pci_uio_alloc_resource(struct rte_pci_device *dev,
  ...
  snprintf(devname, sizeof(devname), "/dev/uio@pci:%u:%u:%u", dev->addr.bus,
...
static int pci_scan_one(int dev_pci_fd, struct pci_conf *conf) {
  ...
  /* get class id */
  dev->id.class_id =
      (conf->pc_class << 16) | (conf->pc_subclass << 8) | (conf->pc_progif);
...
int rte_pci_read_config(const struct rte_pci_device *dev, void *buf, size_t len,
                        off_t offset) {
  int fd = -1;
  int size;
...
int rte_pci_write_config(const struct rte_pci_device *dev, const void *buf,
...
static void pci_uio_ioport_read(struct rte_pci_ioport *p, void *data,
                                size_t len, off_t offset) {
#if defined(RTE_ARCH_X86)
  uint8_t *d;
  int size;
...
static void pci_uio_ioport_write(struct rte_pci_ioport *p, const void *data,
                                 size_t len, off_t offset) {
#if defined(RTE_ARCH_X86)
  const uint8_t *s;
  int size;
