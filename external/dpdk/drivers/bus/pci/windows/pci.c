...
/* Unmap pci device */
void rte_pci_unmap_device(struct rte_pci_device *dev __rte_unused) {
...
int rte_pci_read_config(const struct rte_pci_device *dev __rte_unused,
...
int rte_pci_write_config(const struct rte_pci_device *dev __rte_unused,
...
int rte_pci_ioport_map(struct rte_pci_device *dev __rte_unused,
                       int bar __rte_unused,
                       struct rte_pci_ioport *p __rte_unused) {
  /* This function is not implemented on Windows.
   * We really should short-circuit the call to these functions by
   * clearing the RTE_PCI_DRV_NEED_MAPPING flag
   * in the rte_pci_driver flags.
...
void rte_pci_ioport_read(struct rte_pci_ioport *p __rte_unused,
...
int rte_pci_ioport_unmap(struct rte_pci_ioport *p __rte_unused) {
  /* This function is not implemented on Windows.
   * We really should short-circuit the call to these functions by
   * clearing the RTE_PCI_DRV_NEED_MAPPING flag
   * in the rte_pci_driver flags.
...
bool pci_device_iommu_support_va(
...
void rte_pci_ioport_write(struct rte_pci_ioport *p __rte_unused,
                          const void *data __rte_unused,
                          size_t len __rte_unused, off_t offset __rte_unused) {
  /* This function is not implemented on Windows.
   * We really should short-circuit the call to these functions by
   * clearing the RTE_PCI_DRV_NEED_MAPPING flag
   * in the rte_pci_driver flags.
...
int pci_uio_remap_resource(struct rte_pci_device *dev __rte_unused) {
  /* This function is not implemented on Windows.
   * We really should short-circuit the call to these functions by
   * clearing the RTE_PCI_DRV_NEED_MAPPING flag
   * in the rte_pci_driver flags.
...
static int get_device_pci_address(HDEVINFO dev_info,
  ...
  res = SetupDiGetDeviceRegistryProperty(dev_info, device_info_data,
                                         SPDRP_BUSNUMBER, NULL, (PBYTE)&bus_num,
                                         sizeof(bus_num), NULL);
  if (!res) {
    RTE_LOG_WIN32_ERR("SetupDiGetDeviceRegistryProperty(SPDRP_BUSNUMBER)");
    return -1;
  }

  res = SetupDiGetDeviceRegistryProperty(
      dev_info, device_info_data, SPDRP_ADDRESS, NULL, (PBYTE)&dev_and_func,
      sizeof(dev_and_func), NULL);
  if (!res) {
    RTE_LOG_WIN32_ERR("SetupDiGetDeviceRegistryProperty(SPDRP_ADDRESS)");
    return -1;
  }
...
static int get_device_resource_info(HDEVINFO dev_info,
  ...
  case RTE_PCI_KDRV_NET_UIO:
    ...
    if (ret != 0) {
      RTE_LOG(DEBUG, EAL,
              "Could not retrieve device info for PCI device " PCI_PRI_FMT,
  ...
  res = SetupDiGetDevicePropertyW(
      dev_info, dev_info_data, &DEVPKEY_Device_Numa_Node, &property_type,
 ...
 */
static int get_pci_hardware_id(HDEVINFO dev_info,
                               PSP_DEVINFO_DATA device_info_data,
                               char *pci_device_info,
                               size_t pci_device_info_len) {
  BOOL res;

  /* Retrieve PCI device IDs */
  res = SetupDiGetDeviceRegistryPropertyA(
      dev_info, device_info_data, SPDRP_HARDWAREID, NULL,
      (BYTE *)pci_device_info, pci_device_info_len, NULL);
  if (!res) {
    RTE_LOG_WIN32_ERR("SetupDiGetDeviceRegistryPropertyA(SPDRP_HARDWAREID)");
    return -1;
  }
 ...
 */
static int parse_pci_hardware_id(const char *buf, struct rte_pci_id *pci_id) {
  int ids = 0;
  uint16_t vendor_id, device_id;
  uint32_t subvendor_id = 0, class_id = 0;
  const char *cp;

  ids = sscanf_s(buf, "PCI\\VEN_%" PRIx16 "&DEV_%" PRIx16 "&SUBSYS_%" PRIx32,
...
static void set_kernel_driver_type(PSP_DEVINFO_DATA device_info_data,
                                   struct rte_pci_device *dev) {
...
static int pci_scan_one(HDEVINFO dev_info, PSP_DEVINFO_DATA device_info_data) {
  ...
  ret = get_pci_hardware_id(dev_info, device_info_data, pci_device_info,
...
int rte_pci_scan(void) {
  ...
  while (SetupDiEnumDeviceInfo(dev_info, device_index, &device_info_data)) {
    ...
    if (IsEqualGUID(&(device_info_data.ClassGuid), &GUID_DEVCLASS_NET) ||
        IsEqualGUID(&(device_info_data.ClassGuid), &GUID_DEVCLASS_NETUIO)) {
