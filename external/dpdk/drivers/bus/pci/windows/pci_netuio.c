...
static int send_ioctl(HANDLE f, DWORD ioctl, void *in_buf, DWORD in_buf_size,
                      void *out_buf, DWORD out_buf_size) {
  BOOL res;
  DWORD bytes_ret = 0;

  res = DeviceIoControl(f, ioctl, in_buf, in_buf_size, out_buf, out_buf_size,
                        &bytes_ret, NULL);
  if (!res) {
...
get_netuio_device_information_set(HDEVINFO dev_info,
  ...
  res = SetupDiGetDeviceInstanceId(dev_info, dev_info_data, dev_instance_id,
                                   sizeof(dev_instance_id), &required_size);
  if (!res) {
  ...
  di_set = SetupDiGetClassDevs(&GUID_DEVINTERFACE_NETUIO, dev_instance_id, NULL,
...
get_netuio_device_interface_detail(HDEVINFO di_set) {
  ...
  res = SetupDiEnumDeviceInterfaces(di_set, 0, &GUID_DEVINTERFACE_NETUIO, 0,
                                    &dev_ifx_data);
  if (!res) {
  ...
  res = SetupDiGetDeviceInterfaceDetail(di_set, &dev_ifx_data, dev_ifx_detail,
 ...
 */
int get_netuio_device_info(HDEVINFO dev_info, PSP_DEVINFO_DATA dev_info_data,
  ...
  netuio = CreateFile(dev_ifx_detail->DevicePath, GENERIC_READ | GENERIC_WRITE,
  ...
  if (send_ioctl(netuio, IOCTL_NETUIO_MAP_HW_INTO_USERSPACE, NULL, 0, &hw_info,
                 sizeof(hw_info)) != ERROR_SUCCESS) {
  ...
  for (idx = 0; idx < PCI_MAX_RESOURCE; idx++) {
    dev->mem_resource[idx].phys_addr = hw_info.hw[idx].phys_addr.QuadPart;
    dev->mem_resource[idx].addr = hw_info.hw[idx].user_mapped_virt_addr;
