...
int pci_uio_read_config(const struct rte_intr_handle *intr_handle, void *buf,
...
static int pci_get_uio_dev(struct rte_pci_device *dev, char *dstbuf,
  ...
  snprintf(dirname, sizeof(dirname), "%s/" PCI_PRI_FMT "/uio",
           rte_pci_get_sysfs_path(), loc->domain, loc->bus, loc->devid,
           loc->function);

  dir = opendir(dirname);
  if (dir == NULL) {
    /* retry with the parent directory */
    snprintf(dirname, sizeof(dirname), "%s/" PCI_PRI_FMT,
             rte_pci_get_sysfs_path(), loc->domain, loc->bus, loc->devid,
             loc->function);
    dir = opendir(dirname);
...
int pci_uio_alloc_resource(struct rte_pci_device *dev,
  ...
  snprintf(cfgname, sizeof(cfgname), "/sys/class/uio/uio%u/device/config",
           uio_num);
...
int pci_uio_map_resource_by_index(struct rte_pci_device *dev, int res_idx,
  ...
  if (wc_activate) {
    ...
    snprintf(devname, sizeof(devname), "%s/" PCI_PRI_FMT "/resource%d_wc",
             rte_pci_get_sysfs_path(), loc->domain, loc->bus, loc->devid,
...
int pci_uio_ioport_map(struct rte_pci_device *dev, int bar,
  ...
  if (pci_parse_one_sysfs_resource(buf, sizeof(buf), &phys_addr, &end_addr,
                                   &flags) < 0)
    goto error;
  ...
  if (rte_intr_type_get(dev->intr_handle) == RTE_INTR_HANDLE_UNKNOWN) {
...
int pci_uio_ioport_map(struct rte_pci_device *dev, int bar,
  ...
  addr = mmap(NULL, end_addr + 1, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  close(fd);
  if (addr == MAP_FAILED) {
...
static inline uint8_t ioread8(void *addr) {
  ...
  val = (uint64_t)(uintptr_t)addr >= PIO_MAX ? *(volatile uint8_t *)addr :
#ifdef __GLIBC__
...
static inline uint16_t ioread16(void *addr) {
  ...
  val = (uint64_t)(uintptr_t)addr >= PIO_MAX ? *(volatile uint16_t *)addr :
#ifdef __GLIBC__
...
static inline uint32_t ioread32(void *addr) {
  ...
  val = (uint64_t)(uintptr_t)addr >= PIO_MAX ? *(volatile uint32_t *)addr :
#ifdef __GLIBC__
...
static inline void iowrite8(uint8_t val, void *addr) {
  (uint64_t)(uintptr_t) addr >= PIO_MAX ? *(volatile uint8_t *)addr = val :
#ifdef __GLIBC__
...
static inline void iowrite16(uint16_t val, void *addr) {
  (uint64_t)(uintptr_t) addr >= PIO_MAX ? *(volatile uint16_t *)addr = val :
#ifdef __GLIBC__
...
static inline void iowrite32(uint32_t val, void *addr) {
  (uint64_t)(uintptr_t) addr >= PIO_MAX ? *(volatile uint32_t *)addr = val :
#ifdef __GLIBC__
...
static inline uint8_t ioread8(void *addr) { return *(volatile uint8_t *)addr; }
...
void pci_uio_ioport_read(struct rte_pci_ioport *p, void *data, size_t len,
                         off_t offset) {
  uint8_t *d;
  int size;
...
void pci_uio_ioport_write(struct rte_pci_ioport *p, const void *data,
                          size_t len, off_t offset) {
  const uint8_t *s;
  int size;
  uintptr_t reg = p->base + offset;
