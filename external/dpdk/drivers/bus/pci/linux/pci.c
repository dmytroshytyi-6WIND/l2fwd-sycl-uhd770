    ...
    TAILQ_FOREACH(dev2, &rte_pci_bus.device_list, next) {
      ...
      if (ret < 0) {
        ...
        if (!rte_dev_is_probed(&dev2->device)) {
           ...
           **/
          if (dev2->kdrv != dev->kdrv || dev2->max_vfs != dev->max_vfs ||
 ...
 */
static int parse_pci_addr_format(const char *buf, int bufsize,
...
int rte_pci_scan(void) {
  ...
  while ((e = readdir(dir)) != NULL) {
    ...
    snprintf(dirname, sizeof(dirname), "%s/%s", rte_pci_get_sysfs_path(),
             e->d_name);
...
bool pci_device_iommu_support_va(const struct rte_pci_device *dev) {
...
#define VTD_CAP_MGAW_MASK (0x3fULL << VTD_CAP_MGAW_SHIFT)
  ...
  snprintf(filename, sizeof(filename),
           "%s/" PCI_PRI_FMT "/iommu/intel-iommu/cap", rte_pci_get_sysfs_path(),
...
bool pci_device_iommu_support_va(
    __rte_unused const struct rte_pci_device *dev) {
  /*
...
enum rte_iova_mode pci_device_iova_mode(const struct rte_pci_driver *pdrv,
...
int rte_pci_read_config(const struct rte_pci_device *device, void *buf,
...
int rte_pci_write_config(const struct rte_pci_device *device, const void *buf,
                         size_t len, off_t offset) {
  char devname[RTE_DEV_NAME_MAX_LEN] = "";
  const struct rte_intr_handle *intr_handle = device->intr_handle;
...
void rte_pci_ioport_read(struct rte_pci_ioport *p, void *data, size_t len,
                         off_t offset) {
  switch (p->dev->kdrv) {
#ifdef VFIO_PRESENT
  case RTE_PCI_KDRV_VFIO:
...
void rte_pci_ioport_write(struct rte_pci_ioport *p, const void *data,
                          size_t len, off_t offset) {
  switch (p->dev->kdrv) {
#ifdef VFIO_PRESENT
  case RTE_PCI_KDRV_VFIO:
