...
int pci_vfio_read_config(const struct rte_intr_handle *intr_handle, void *buf,
                         size_t len, off_t offs) {
...
static int pci_vfio_get_msix_bar(int fd, struct pci_msix_table *msix_table) {
  int ret;
  uint32_t reg;
  uint16_t flags;
  ...
  while (cap_offset) {
    ...
    ret = pread64(fd, &reg, sizeof(reg),
                  VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) +
                      cap_offset);
    if (ret != sizeof(reg)) {
      RTE_LOG(ERR, EAL, "Cannot read capability ID from PCI config space!\n");
      return -1;
    }
    ...
    /* else, read table offset */
      ...
      ret = pread64(fd, &reg, sizeof(reg),
                    VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) +
                        cap_offset + 4);
      if (ret != sizeof(reg)) {
        RTE_LOG(ERR, EAL, "Cannot read table offset from PCI config space!\n");
        return -1;
      }
      ...
      ret = pread64(fd, &flags, sizeof(flags),
                    VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) +
                        cap_offset + 2);
      if (ret != sizeof(flags)) {
        RTE_LOG(ERR, EAL, "Cannot read table flags from PCI config space!\n");
        return -1;
      }
...
static int pci_vfio_enable_bus_memory(int dev_fd) {
  uint16_t cmd;
  int ret;

  ret =
      pread64(dev_fd, &cmd, sizeof(cmd),
...
static int pci_vfio_set_bus_master(int dev_fd, bool op) {
  uint16_t reg;
  int ret;

  ret =
      pread64(dev_fd, &reg, sizeof(reg),
              VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + PCI_COMMAND);
  if (ret != sizeof(reg)) {
    RTE_LOG(ERR, EAL, "Cannot read command from PCI config space!\n");
    return -1;
...
static int pci_vfio_setup_interrupts(struct rte_pci_device *dev,
                                     int vfio_dev_fd) {
  int i, ret, intr_idx;
  ...
  for (i = VFIO_PCI_MSIX_IRQ_INDEX; i >= 0; i--) {
    ...
    if (ret < 0) {
      RTE_LOG(ERR, EAL,
              "Cannot get VFIO IRQ info, error "
              "%i (%s)\n",
    ...
    case VFIO_PCI_MSIX_IRQ_INDEX:
      ...
      rte_intr_type_set(dev->intr_handle, RTE_INTR_HANDLE_VFIO_MSIX);
      break;
    case VFIO_PCI_MSI_IRQ_INDEX:
      intr_mode = RTE_INTR_MODE_MSI;
      rte_intr_type_set(dev->intr_handle, RTE_INTR_HANDLE_VFIO_MSI);
      break;
    case VFIO_PCI_INTX_IRQ_INDEX:
      intr_mode = RTE_INTR_MODE_LEGACY;
      rte_intr_type_set(dev->intr_handle, RTE_INTR_HANDLE_VFIO_LEGACY);
      break;
    default:
...
static void pci_vfio_req_handler(void *param) {
  struct rte_bus *bus;
  int ret;
  ...
  if (ret)
    RTE_LOG(ERR, EAL, "Can not handle hot-unplug for device (%s)\n",
            device->name);
handle_end:
...
static int pci_vfio_enable_notifier(struct rte_pci_device *dev,
                                    int vfio_dev_fd) {
  int ret;
  int fd = -1;
  ...
  ret = rte_intr_callback_register(dev->vfio_req_intr_handle,
                                   pci_vfio_req_handler, (void *)&dev->device);
  if (ret) {
  ...
  if (ret) {
    ...
    ret = rte_intr_callback_unregister(
        dev->vfio_req_intr_handle, pci_vfio_req_handler, (void *)&dev->device);
    if (ret < 0)
...
static int pci_vfio_disable_notifier(struct rte_pci_device *dev) {
  int ret;
  ...
  ret = rte_intr_callback_unregister_sync(
      dev->vfio_req_intr_handle, pci_vfio_req_handler, (void *)&dev->device);
  if (ret < 0) {
    RTE_LOG(ERR, EAL, "fail to unregister req notifier handler.\n");
    return -1;
...
static int pci_vfio_is_ioport_bar(int vfio_dev_fd, int bar_index) {
  uint32_t ioport_bar;
  int ret;
...
static int pci_rte_vfio_setup_device(struct rte_pci_device *dev,
                                     int vfio_dev_fd) {
...
static int pci_vfio_mmap_bar(int vfio_dev_fd,
                             struct mapped_pci_resource *vfio_res,
  ...
  bar_addr = mmap(bar->addr, bar->size, 0,
                  MAP_PRIVATE | MAP_ANONYMOUS | additional_flags, -1, 0);
  if (bar_addr != MAP_FAILED) {
    void *map_addr = NULL;
    if (memreg[0].size) {
      /* actual map of first part */
      map_addr = pci_map_resource(bar_addr, vfio_dev_fd, memreg[0].offset,
                                  memreg[0].size, RTE_MAP_FORCE_ADDRESS);
    }
    ...
    if ((map_addr != NULL || memreg[0].size == 0) && memreg[1].offset &&
        memreg[1].size) {
      void *second_addr =
          RTE_PTR_ADD(bar_addr, (uintptr_t)(memreg[1].offset - bar->offset));
      map_addr = pci_map_resource(second_addr, vfio_dev_fd, memreg[1].offset,
                                  memreg[1].size, RTE_MAP_FORCE_ADDRESS);
    }
...
static int pci_vfio_get_region_info(int vfio_dev_fd,
  ...
  ri = malloc(sizeof(*ri));
  if (ri == NULL) {
    RTE_LOG(ERR, EAL, "Cannot allocate memory for VFIO region info\n");
    return -1;
  }
again:
  memset(ri, 0, argsz);
  ...
  if (ri->argsz != argsz) {
    ...
    if (tmp == NULL) {
      ...
      free(ri);
      RTE_LOG(ERR, EAL, "Cannot reallocate memory for VFIO region info\n");
      return -1;
    }
    ri = tmp;
    goto again;
...
static int pci_vfio_msix_is_mappable(int vfio_dev_fd, int msix_region) {
  struct vfio_region_info *info;
  int ret;
...
static int pci_vfio_map_resource_primary(struct rte_pci_device *dev) {
  ...
  snprintf(pci_addr, sizeof(pci_addr), PCI_PRI_FMT, loc->domain, loc->bus,
  ...
  if (vfio_res == NULL) {
    RTE_LOG(ERR, EAL, "Cannot store VFIO mmap details\n");
  ...
  vfio_res->nb_maps =
      RTE_MIN((int)device_info.num_regions, VFIO_PCI_BAR5_REGION_INDEX + 1);
...
err_vfio_res:
  rte_free(vfio_res);
err_vfio_dev_fd:
  rte_vfio_release_device(rte_pci_get_sysfs_path(), pci_addr, vfio_dev_fd);
  return -1;
}

static int pci_vfio_map_resource_secondary(struct rte_pci_device *dev) {
  struct vfio_device_info device_info = {.argsz = sizeof(device_info)};
  char pci_addr[PATH_MAX] = {0};
  int vfio_dev_fd;
  struct rte_pci_addr *loc = &dev->addr;
...
find_and_unmap_vfio_resource(struct mapped_pci_res_list *vfio_res_list,
                             struct rte_pci_device *dev, const char *pci_addr) {
  struct mapped_pci_resource *vfio_res = NULL;
  struct pci_map *maps;
  int i;
...
static int pci_vfio_unmap_resource_primary(struct rte_pci_device *dev) {
  char pci_addr[PATH_MAX] = {0};
  struct rte_pci_addr *loc = &dev->addr;
  struct mapped_pci_resource *vfio_res = NULL;
  struct mapped_pci_res_list *vfio_res_list;
...
static int pci_vfio_unmap_resource_secondary(struct rte_pci_device *dev) {
  char pci_addr[PATH_MAX] = {0};
  struct rte_pci_addr *loc = &dev->addr;
  struct mapped_pci_resource *vfio_res = NULL;
  struct mapped_pci_res_list *vfio_res_list;
...
int pci_vfio_ioport_map(struct rte_pci_device *dev, int bar,
  ...
  if (bar < VFIO_PCI_BAR0_REGION_INDEX || bar > VFIO_PCI_BAR5_REGION_INDEX) {
...
void pci_vfio_ioport_read(struct rte_pci_ioport *p, void *data, size_t len,
                          off_t offset) {
  ...
  if (pread64(vfio_dev_fd, data, len, p->base + offset) <= 0)
    RTE_LOG(ERR, EAL, "Can't read from PCI bar (%" PRIu64 ") : offset (%x)\n",
            VFIO_GET_REGION_IDX(p->base), (int)offset);
}

void pci_vfio_ioport_write(struct rte_pci_ioport *p, const void *data,
  ...
  if (pwrite64(vfio_dev_fd, data, len, p->base + offset) <= 0)
    RTE_LOG(ERR, EAL, "Can't write to PCI bar (%" PRIu64 ") : offset (%x)\n",
            VFIO_GET_REGION_IDX(p->base), (int)offset);
}
...
int pci_vfio_is_enabled(void) { return rte_vfio_is_enabled("vfio_pci"); }
#endif
