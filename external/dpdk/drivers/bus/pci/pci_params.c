...
static int pci_addr_kv_cmp(const char *key __rte_unused, const char *value,
                           void *_addr2) {
...
static int pci_dev_match(const struct rte_device *dev, const void *_kvlist) {
  ...
  if (rte_kvargs_process(kvlist, pci_params_keys[RTE_PCI_PARAM_ADDR],
                         &pci_addr_kv_cmp, (void *)(intptr_t)&pdev->addr))
    return 1;
  return 0;
}
