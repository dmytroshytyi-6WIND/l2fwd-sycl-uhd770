...
void *pci_map_resource(void *requested_addr, int fd, off_t offset, size_t size,
  ...
  if (mapaddr == NULL) {
    RTE_LOG(ERR, EAL,
            ...
            __func__, fd, requested_addr, size, (unsigned long long)offset,
 ...
 */
int rte_pci_match(const struct rte_pci_driver *pci_drv,
...
static int rte_pci_probe_one_driver(struct rte_pci_driver *dr,
  ...
  if (dev->device.numa_node < 0) {
    if (rte_socket_count() > 1)
      RTE_LOG(INFO, EAL,
              "Device %s is not NUMA-aware, defaulting socket to 0\n",
              dev->name);
  ...
  if (!already_probed) {
    ...
    if (dev_iova_mode != RTE_IOVA_DC && dev_iova_mode != iova_mode) {
      RTE_LOG(ERR, EAL,
              "  Expecting '%s' IOVA mode but current mode is '%s', not "
              "initializing\n",
    ...
    if (dev->intr_handle == NULL) {
      RTE_LOG(ERR, EAL, "Failed to create interrupt instance for %s\n",
              dev->device.name);
      return -ENOMEM;
    }
    ...
    if (dev->vfio_req_intr_handle == NULL) {
      ...
      RTE_LOG(ERR, EAL, "Failed to create vfio req interrupt instance for %s\n",
              dev->device.name);
      return -ENOMEM;
    }
  ...
  RTE_LOG(INFO, EAL,
          "Probe PCI driver: %s (%x:%x) device: " PCI_PRI_FMT " (socket %i)\n",
          dr->driver.name, dev->id.vendor_id, dev->id.device_id, loc->domain,
          loc->bus, loc->devid, loc->function, dev->device.numa_node);
 ...
 * driver.
 */
static int rte_pci_detach_dev(struct rte_pci_device *dev) {
  struct rte_pci_addr *loc;
...
static int pci_probe(void) {
  ...
  FOREACH_DEVICE_ON_PCIBUS(dev) {
    ...
    if (ret < 0) {
      if (ret != -EEXIST) {
        RTE_LOG(ERR, EAL, "Requested device " PCI_PRI_FMT " cannot be used\n",
...
/* dump one device */
static int pci_dump_one_device(FILE *f, struct rte_pci_device *dev) {
  int i;

  fprintf(f, PCI_PRI_FMT, dev->addr.domain, dev->addr.bus, dev->addr.devid,
          dev->addr.function);
  fprintf(f, " - vendor:%x device:%x\n", dev->id.vendor_id, dev->id.device_id);

  for (i = 0; i != sizeof(dev->mem_resource) / sizeof(dev->mem_resource[0]);
       i++) {
    fprintf(f, "   %16.16" PRIx64 " %16.16" PRIx64 "\n",
            dev->mem_resource[i].phys_addr, dev->mem_resource[i].len);
  }
  return 0;
}
...
void rte_pci_dump(FILE *f) {
  ...
  FOREACH_DEVICE_ON_PCIBUS(dev) { pci_dump_one_device(f, dev); }
}
...
static void rte_pci_remove_device(struct rte_pci_device *pci_dev) {
  TAILQ_REMOVE(&rte_pci_bus.device_list, pci_dev, next);
}

static struct rte_device *pci_find_device(const struct rte_device *start,
...
static int pci_hot_unplug_handler(struct rte_device *dev) {
  struct rte_pci_device *pdev = NULL;
  int ret = 0;
  ...
  case RTE_PCI_KDRV_VFIO:
     ...
     */
    rte_dev_event_callback_process(dev->name, RTE_DEV_EVENT_REMOVE);
    break;
#endif
...
static int pci_sigbus_handler(const void *failure_addr) {
  struct rte_pci_device *pdev = NULL;
  int ret = 0;
  ...
  if (!pdev) {
    ...
    if (ret) {
      RTE_LOG(ERR, EAL, "Failed to handle hot-unplug for device %s",
              pdev->name);
      ret = -1;
    }
  }
...
static int pci_plug(struct rte_device *dev) {
  return pci_probe_all_drivers(RTE_DEV_TO_PCI(dev));
}

static int pci_unplug(struct rte_device *dev) {
  struct rte_pci_device *pdev;
  int ret;
...
static int pci_dma_map(struct rte_device *dev, void *addr, uint64_t iova,
                       size_t len) {
...
static int pci_dma_unmap(struct rte_device *dev, void *addr, uint64_t iova,
                         size_t len) {
  struct rte_pci_device *pdev = RTE_DEV_TO_PCI(dev);
...
enum rte_iova_mode rte_pci_get_iommu_class(void) {
  ...
  FOREACH_DEVICE_ON_PCIBUS(dev) {
     ...
     */
    if (iommu_no_va == -1)
      iommu_no_va = pci_device_iommu_support_va(dev) ? 0 : 1;

    if (dev->kdrv == RTE_PCI_KDRV_UNKNOWN || dev->kdrv == RTE_PCI_KDRV_NONE)
      continue;
    FOREACH_DRIVER_ON_PCIBUS(drv) {
      ...
      RTE_LOG(DEBUG, EAL,
              "PCI driver %s for device " PCI_PRI_FMT " wants IOVA as '%s'\n",
              drv->driver.name, dev->addr.domain, dev->addr.bus,
              dev->addr.devid, dev->addr.function,
              dev_iova_mode == RTE_IOVA_DC
                  ? "DC"
                  : (dev_iova_mode == RTE_IOVA_PA ? "PA" : "VA"));
  ...
  if (iommu_no_va == 1) {
    ...
    if (devices_want_va) {
      RTE_LOG(WARNING, EAL,
              "Some devices want 'VA' but IOMMU does not support 'VA'.\n");
    ...
    if (devices_want_va) {
      RTE_LOG(WARNING, EAL,
              "Some devices want 'VA' but forcing 'DC' because other devices "
              "want 'PA'.\n");
      RTE_LOG(WARNING, EAL,
              "Depending on the final decision by the EAL, not all devices may "
...
int rte_pci_set_bus_master(struct rte_pci_device *dev, bool enable) {
  ...
  if (rte_pci_read_config(dev, &old_cmd, sizeof(old_cmd), RTE_PCI_COMMAND) <
      0) {
