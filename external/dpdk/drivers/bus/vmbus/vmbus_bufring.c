...
static inline uint32_t vmbus_br_idxinc(uint32_t idx, uint32_t inc,
                                       uint32_t sz) {
  idx += inc;
  if (idx >= sz)
    idx -= sz;
 ...
 *   data have arrived.
 */
static inline bool vmbus_txbr_need_signal(const struct vmbus_bufring *vbr,
                                          uint32_t old_windex) {
  rte_smp_mb();
  if (vbr->imask)
    return false;
...
static inline uint32_t vmbus_txbr_copyto(const struct vmbus_br *tbr,
                                         uint32_t windex, const void *src0,
                                         uint32_t cplen) {
...
int vmbus_txbr_write(struct vmbus_br *tbr, const struct iovec iov[], int iovlen,
  ...
  windex = vmbus_txbr_copyto(tbr, windex, &save_windex, sizeof(save_windex));
...
static inline uint32_t vmbus_rxbr_copyfrom(const struct vmbus_br *rbr,
                                           uint32_t rindex, void *dst0,
                                           size_t cplen) {
 ...
 */
int vmbus_rxbr_read(struct vmbus_br *rbr, void *data, size_t dlen,
                    size_t skip) {
