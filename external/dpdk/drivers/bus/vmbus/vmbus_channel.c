...
static inline void vmbus_sync_set_bit(volatile uint32_t *addr, uint32_t mask) {
  /* Use GCC builtin which atomic does atomic OR operation */
  __sync_or_and_fetch(addr, mask);
}

static inline void vmbus_set_monitor(const struct rte_vmbus_device *dev,
                                     uint32_t monitor_id) {
...
static void vmbus_set_event(const struct rte_vmbus_device *dev,
...
void rte_vmbus_set_latency(const struct rte_vmbus_device *dev,
                           const struct vmbus_channel *chan, uint32_t latency) {
  ...
  if (trig_idx >= VMBUS_MONTRIGS_MAX) {
    VMBUS_LOG(ERR, "invalid monitor trigger %u", trig_idx);
    return;
  }
 ...
 */
void rte_vmbus_chan_signal_tx(const struct vmbus_channel *chan) {
...
int rte_vmbus_chan_send(struct vmbus_channel *chan, uint16_t type, void *data,
...
int rte_vmbus_chan_send_sglist(struct vmbus_channel *chan,
                               ...
                               void *data, uint32_t dlen, uint64_t xactid,
                               bool *need_sig) {
...
void rte_vmbus_chan_signal_read(struct vmbus_channel *chan,
                                uint32_t bytes_read) {
...
int rte_vmbus_chan_recv(struct vmbus_channel *chan, void *data, uint32_t *len,
  ...
  *len = 0;
  ...
  if (unlikely(pkt.hlen > pkt.tlen)) {
    VMBUS_LOG(ERR, "VMBUS recv,invalid hlen %u and tlen %u", pkt.hlen,
              pkt.tlen);
    return -EIO;
  }
...
int rte_vmbus_chan_recv_raw(struct vmbus_channel *chan, void *data,
                            uint32_t *len) {
  struct vmbus_chanpkt_hdr pkt;
...
int vmbus_chan_create(const struct rte_vmbus_device *device, uint16_t relid,
                      uint16_t subid, uint8_t monitor_id,
...
int rte_vmbus_chan_open(struct rte_vmbus_device *device,
  ...
  err =
      vmbus_chan_create(device, device->relid, 0, device->monitor_id, new_chan);
  if (!err)
