...
void *vmbus_map_resource(void *requested_addr, int fd, off_t offset,
  ...
  if (mapaddr == MAP_FAILED) {
    VMBUS_LOG(ERR, "mmap(%d, %p, %zu, %ld) failed: %s", fd, requested_addr,
              size, (long)offset, strerror(errno));
  }
  return mapaddr;
}
...
void vmbus_unmap_resource(void *requested_addr, size_t size) {
  ...
  if (munmap(requested_addr, size)) {
    VMBUS_LOG(ERR, "munmap(%p, 0x%lx) failed: %s", requested_addr,
              (unsigned long)size, strerror(errno));
  } else
    VMBUS_LOG(DEBUG, "  VMBUS memory unmapped at %p", requested_addr);
}
 ...
 */
static bool vmbus_match(const struct rte_vmbus_driver *dr,
 ...
 */
static int vmbus_probe_one_driver(struct rte_vmbus_driver *dr,
  ...
  VMBUS_LOG(INFO, "VMBUS device %s on NUMA socket %i", guid,
 ...
 */
static int vmbus_probe_all_drivers(struct rte_vmbus_device *dev) {
...
int rte_vmbus_probe(void) {
  ...
  FOREACH_DEVICE_ON_VMBUS(dev) {
    ...
    if (vmbus_probe_all_drivers(dev) < 0) {
      VMBUS_LOG(NOTICE, "Requested device %s cannot be used", ubuf);
      rte_errno = errno;
      failed++;
    }
  }
...
static int vmbus_parse(const char *name, void *addr) {
  rte_uuid_t guid;
  int ret;
 ...
 */
struct rte_devargs *vmbus_devargs_lookup(struct rte_vmbus_device *dev) {
...
void rte_vmbus_register(struct rte_vmbus_driver *driver) {
  VMBUS_LOG(DEBUG, "Registered driver %s", driver->driver.name);
...
void vmbus_remove_device(struct rte_vmbus_device *vmbus_dev) {
  TAILQ_REMOVE(&rte_vmbus_bus.device_list, vmbus_dev, next);
}

/* VMBUS doesn't support hotplug */
static struct rte_device *vmbus_find_device(const struct rte_device *start,
                                            rte_dev_cmp_t cmp,
                                            const void *data) {
  struct rte_vmbus_device *dev;
