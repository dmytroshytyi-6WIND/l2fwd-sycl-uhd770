...
static int vmbus_uio_map_secondary(struct rte_vmbus_device *dev) {
  struct mapped_vmbus_resource *uio_res;
  ...
  for (i = 0; i != uio_res->nb_maps; i++) {
    ...
    mapaddr = vmbus_map_resource(uio_res->maps[i].addr, fd, offset,
    ...
    if (mapaddr == MAP_FAILED)
      VMBUS_LOG(ERR, "mmap resource %d in secondary failed", i);
    else {
      VMBUS_LOG(ERR, "mmap resource %d address mismatch", i);
  ...
  if (vmbus_chan_create(dev, dev->relid, 0, dev->monitor_id, &dev->primary)) {
  ...
  for (i = 0; i < uio_res->nb_subchannels; i++) {
    if (rte_vmbus_subchan_open(dev->primary, &chan)) {
      VMBUS_LOG(ERR, "failed to create subchannel at index %d", i);
...
failed_primary:
  for (i = 0; i != uio_res->nb_maps; i++) {
    vmbus_unmap_resource(uio_res->maps[i].addr, uio_res->maps[i].size);
  }

  return -1;
}

static int vmbus_uio_map_primary(struct rte_vmbus_device *dev) {
  int i, ret;
...
error:
  while (--i >= 0) {
    vmbus_unmap_resource(uio_res->maps[i].addr, (size_t)uio_res->maps[i].size);
  }
...
static void vmbus_uio_unmap(struct mapped_vmbus_resource *uio_res) {
  int i;
