 ...
 */
static const rte_uuid_t vmbus_nic_uuid = {0xf8, 0x61, 0x51, 0x63, 0xdf, 0x3e,
                                          0x46, 0xc5, 0x91, 0x3f, 0xf2, 0xd2,
                                          0xf9, 0x65, 0xed, 0xe};

extern struct rte_vmbus_bus rte_vmbus_bus;

/* Read sysfs file to get UUID */
static int parse_sysfs_uuid(const char *filename, rte_uuid_t uu) {
  char buf[BUFSIZ];
  char *cp, *in = buf;
  FILE *f;
  ...
  f = fopen(filename, "r");
  if (f == NULL) {
    VMBUS_LOG(ERR, "cannot open sysfs value %s: %s", filename, strerror(errno));
    return -1;
  }

  if (fgets(buf, sizeof(buf), f) == NULL) {
    VMBUS_LOG(ERR, "cannot read sysfs value %s", filename);
    fclose(f);
    return -1;
  }
  fclose(f);
  ...
  if (rte_uuid_parse(in, uu) < 0) {
    VMBUS_LOG(ERR, "%s %s not a valid UUID", filename, buf);
    return -1;
  }
...
static int get_sysfs_string(const char *filename, char *buf, size_t buflen) {
  char *cp;
  FILE *f;
...
static int vmbus_get_uio_dev(const struct rte_vmbus_device *dev, char *dstbuf,
                             size_t buflen) {
  ...
  snprintf(dirname, sizeof(dirname), SYSFS_VMBUS_DEVICES "/%s/uio",
           dev->device.name);
...
static const char *map_names[VMBUS_MAX_RESOURCE] = {
    [HV_TXRX_RING_MAP] = "txrx_rings",  [HV_INT_PAGE_MAP] = "int_page",
    [HV_MON_PAGE_MAP] = "monitor_page", [HV_RECV_BUF_MAP] = "recv:",
...
int rte_vmbus_map_device(struct rte_vmbus_device *dev) {
  ...
  for (i = 0; i < VMBUS_MAX_RESOURCE; i++) {
    ...
    snprintf(dirname, sizeof(dirname), "%s/maps/map%d", uioname, i);

    snprintf(filename, sizeof(filename), "%s/name", dirname);
    ...
    if (strncmp(map_names[i], mapname, strlen(map_names[i])) != 0) {
      VMBUS_LOG(ERR, "unexpected resource %s (expected %s)", mapname,
                map_names[i]);
      return -1;
    }
...
static int vmbus_scan_one(const char *name) {
   ...
   */
  snprintf(dirname, sizeof(dirname), "%s/%s", SYSFS_VMBUS_DEVICES, name);
  ...
  TAILQ_FOREACH(dev2, &rte_vmbus_bus.device_list, next) {
    ...
    if (ret < 0) {
      ...
      VMBUS_LOG(NOTICE, "%s already registered", name);
      free(dev_name);
      free(dev);
    }
    return 0;
...
int rte_vmbus_scan(void) {
  ...
  if (dir == NULL) {
    ...
    VMBUS_LOG(ERR, "opendir %s failed: %s", SYSFS_VMBUS_DEVICES,
              strerror(errno));
    return -1;
  }
