...
void vmbus_uio_irq_control(struct rte_vmbus_device *dev, int32_t onoff) {
  if ((rte_intr_fd_get(dev->intr_handle) < 0) ||
      write(rte_intr_fd_get(dev->intr_handle), &onoff, sizeof(onoff)) < 0) {
    VMBUS_LOG(ERR, "cannot write to %d:%s", rte_intr_fd_get(dev->intr_handle),
              strerror(errno));
  }
}
...
int vmbus_uio_irq_read(struct rte_vmbus_device *dev) {
  ...
  cc = read(rte_intr_fd_get(dev->intr_handle), &count, sizeof(count));
  if (cc < (int)sizeof(count)) {
    if (cc < 0) {
      VMBUS_LOG(ERR, "IRQ read failed %s", strerror(errno));
      return -errno;
    }
...
int vmbus_uio_alloc_resource(struct rte_vmbus_device *dev,
  ...
  if (fd < 0) {
    VMBUS_LOG(ERR, "Cannot open %s: %s", devname, strerror(errno));
    goto error;
  }
...
static int find_max_end_va(const struct rte_memseg_list *msl, void *arg) {
 ...
 */
static void *vmbus_find_max_end_va(void) {
  void *va = NULL;
...
int vmbus_uio_map_resource_by_index(struct rte_vmbus_device *dev, int idx,
                                    struct mapped_vmbus_resource *uio_res,
                                    int flags) {
  ...
  if (fd < 0) {
    VMBUS_LOG(ERR, "Cannot open %s: %s", uio_res->path, strerror(errno));
    return -1;
  }
...
static int vmbus_uio_map_primary(struct vmbus_channel *chan, void **ring_buf,
                                 uint32_t *ring_size) {
  struct mapped_vmbus_resource *uio_res;
  ...
  if (uio_res->nb_maps < VMBUS_MAX_RESOURCE) {
    VMBUS_LOG(ERR, "VMBUS: only %u resources found!", uio_res->nb_maps);
    return -EINVAL;
  }
...
static int vmbus_uio_map_subchan(const struct rte_vmbus_device *dev,
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    if (uio_res->nb_subchannels >= UIO_MAX_SUBCHANNEL) {
      VMBUS_LOG(ERR, "exceeding max subchannels UIO_MAX_SUBCHANNEL(%d)",
                UIO_MAX_SUBCHANNEL);
      VMBUS_LOG(ERR, "Change UIO_MAX_SUBCHANNEL and recompile");
      return -ENOMEM;
    }
  } else {
    for (channel_idx = 0; channel_idx < uio_res->nb_subchannels; channel_idx++)
      if (uio_res->subchannel_maps[channel_idx].relid == chan->relid)
        break;
  ...
  snprintf(ring_path, sizeof(ring_path), "%s/%s/channels/%u/ring",
           SYSFS_VMBUS_DEVICES, dev->device.name, chan->relid);

  fd = open(ring_path, O_RDWR);
  if (fd < 0) {
    VMBUS_LOG(ERR, "Cannot open %s: %s", ring_path, strerror(errno));
    return -errno;
  }

  if (fstat(fd, &sb) < 0) {
    VMBUS_LOG(ERR, "Cannot state %s: %s", ring_path, strerror(errno));
    close(fd);
    return -errno;
  }
  ...
  if (file_size == 0 || (file_size & (rte_mem_page_size() - 1))) {
    VMBUS_LOG(ERR, "incorrect size %s: %zu", ring_path, file_size);

    close(fd);
    return -EINVAL;
  }

  mapaddr = vmbus_map_resource(vmbus_map_addr, fd, 0, file_size, 0);
  close(fd);
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    ...
    uio_res->subchannel_maps[uio_res->nb_subchannels].relid = chan->relid;
    uio_res->subchannel_maps[uio_res->nb_subchannels].addr = mapaddr;
    uio_res->subchannel_maps[uio_res->nb_subchannels].size = file_size;
    uio_res->nb_subchannels++;

    vmbus_map_addr = RTE_PTR_ADD(mapaddr, file_size);
  } else {
    if (mapaddr != vmbus_map_addr) {
      VMBUS_LOG(ERR, "failed to map channel %d to addr %p", chan->relid,
                mapaddr);
...
static int vmbus_uio_sysfs_read(const char *dir, const char *name,
  ...
  f = fopen(path, "r");
  if (!f) {
    VMBUS_LOG(ERR, "can't open %s:%s", path, strerror(errno));
    return -errno;
  }
...
int vmbus_uio_get_subchan(struct vmbus_channel *primary,
  ...
  snprintf(chan_path, sizeof(chan_path), "%s/%s/channels", SYSFS_VMBUS_DEVICES,
           dev->device.name);

  chan_dir = opendir(chan_path);
  if (!chan_dir) {
    VMBUS_LOG(ERR, "cannot open %s: %s", chan_path, strerror(errno));
    return -errno;
  }
  ...
  while ((ent = readdir(chan_dir))) {
    ...
    if (*endp || errno != 0 || relid > UINT16_MAX) {
      VMBUS_LOG(NOTICE, "not a valid channel relid: %s", ent->d_name);
      continue;
    }

    if (!vmbus_isnew_subchannel(primary, relid)) {
      VMBUS_LOG(DEBUG, "skip already found channel: %lu", relid);
      continue;
    }

    if (!vmbus_uio_ring_present(dev, relid)) {
      VMBUS_LOG(DEBUG, "ring mmap not found (yet) for: %lu", relid);
      continue;
    }

    snprintf(subchan_path, sizeof(subchan_path), "%s/%lu", chan_path, relid);
    err =
        vmbus_uio_sysfs_read(subchan_path, "subchannel_id", &subid, UINT16_MAX);
    if (err) {
      VMBUS_LOG(NOTICE, "no subchannel_id in %s:%s", subchan_path,
                strerror(-err));
      goto fail;
    }
    ...
    err = vmbus_uio_sysfs_read(subchan_path, "monitor_id", &monid, UINT8_MAX);
    if (err) {
      VMBUS_LOG(NOTICE, "no monitor_id in %s:%s", subchan_path, strerror(-err));
      goto fail;
    }
