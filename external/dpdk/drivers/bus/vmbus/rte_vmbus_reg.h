...
struct vmbus_mon_page {
  ...
  struct hyperv_mon_param param[VMBUS_MONTRIGS_MAX][VMBUS_MONTRIG_LEN];
  uint8_t rsvd4[1984];
} __rte_packed;
...
static inline uint32_t vmbus_chanpkt_getlen(uint16_t pktlen) {
...
vmbus_chanpkt_datalen(const struct vmbus_chanpkt_hdr *pkt) {
  return vmbus_chanpkt_getlen(pkt->tlen) - vmbus_chanpkt_getlen(pkt->hlen);
}
