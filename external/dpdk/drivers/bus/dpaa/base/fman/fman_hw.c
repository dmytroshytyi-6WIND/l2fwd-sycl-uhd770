...
#define ETH_ADDR_TO_UINT64(eth_addr)                                           \
  (uint64_t)(                                                                  \
      ((uint64_t)(eth_addr)[0] << 40) | ((uint64_t)(eth_addr)[1] << 32) |      \
      ((uint64_t)(eth_addr)[2] << 24) | ((uint64_t)(eth_addr)[3] << 16) |      \
      ((uint64_t)(eth_addr)[4] << 8) | ((uint64_t)(eth_addr)[5]))
...
void fman_if_clear_mac_addr(struct fman_if *p, uint8_t addr_num) {
  ...
  if (addr_num) {
    reg =
        &((struct memac_regs *)m->ccsr_map)->mac_addr[addr_num - 1].mac_addr_l;
    out_be32(reg, 0x0);
    reg =
        &((struct memac_regs *)m->ccsr_map)->mac_addr[addr_num - 1].mac_addr_u;
    out_be32(reg, 0x0);
  } else {
...
int fman_if_add_mac_addr(struct fman_if *p, uint8_t *eth, uint8_t addr_num) {
  ...
  val =
      (m->__if.mac_addr.addr_bytes[0] | (m->__if.mac_addr.addr_bytes[1] << 8) |
...
void fman_if_stats_get(struct fman_if *p, struct rte_eth_stats *stats) {
  ...
  stats->ipackets = (u64)in_be32(&regs->rfrm_l) | ((u64)in_be32(&regs->rfrm_u))
                                                      << 32;
  stats->ibytes = (u64)in_be32(&regs->roct_l) | ((u64)in_be32(&regs->roct_u))
                                                    << 32;
  stats->ierrors = (u64)in_be32(&regs->rerr_l) | ((u64)in_be32(&regs->rerr_u))
                                                     << 32;

  /* read xmited packet count */
  stats->opackets = (u64)in_be32(&regs->tfrm_l) | ((u64)in_be32(&regs->tfrm_u))
                                                      << 32;
  stats->obytes = (u64)in_be32(&regs->toct_l) | ((u64)in_be32(&regs->toct_u))
                                                    << 32;
  stats->oerrors = (u64)in_be32(&regs->terr_l) | ((u64)in_be32(&regs->terr_u))
                                                     << 32;
}
...
void fman_if_stats_get_all(struct fman_if *p, uint64_t *value, int n) {
  ...
  for (i = 0; i < n; i++)
    value[i] = (((u64)in_be32((char *)regs + base_offset + 8 * i) |
                 (u64)in_be32((char *)regs + base_offset + 8 * i + 4))
                << 32);
}
...
void fman_if_loopback_enable(struct fman_if *p) {
  ...
  if ((__if->__if.is_memac) && (__if->__if.is_rgmii)) {
    unsigned int *ifmode = &((struct memac_regs *)__if->ccsr_map)->if_mode;
...
void fman_if_set_bp(struct fman_if *fm_if, unsigned num __always_unused,
  ...
  fmbm_ebmpi = in_be32(&((struct rx_bmi_regs *)__if->bmi_map)->fmbm_ebmpi[0]);
  fmbm_ebmpi =
      ebmpi_val_ace | (fmbm_ebmpi & ebmpi_mask) | (bpid << 16) | (bufsize);

  out_be32(&((struct rx_bmi_regs *)__if->bmi_map)->fmbm_ebmpi[0], fmbm_ebmpi);
}
...
int fman_if_set_fc_threshold(struct fman_if *fm_if, u32 high_water,
...
int fman_if_get_ic_params(struct fman_if *fm_if,
  ...
  unsigned int *fmbm_ricp = &((struct rx_bmi_regs *)__if->bmi_map)->fmbm_ricp;
...
void fman_if_receive_rx_errors(struct fman_if *fm_if, unsigned int err_eq) {
  struct __fman_if *__if = container_of(fm_if, struct __fman_if, __if);
