...
void dump_netcfg(struct netcfg_info *cfg_ptr) {
  ...
  for (i = 0; i < cfg_ptr->num_ethports; i++) {
    ...
    printf("\n+ Fman %d, MAC %d (%s);\n", __if->fman_idx, __if->mac_idx,
           ...
           : (__if->mac_type == fman_mac_2_5g) ? "2.5G"
                                               : "10G");
    ...
    fman_if_for_each_bpool(bpool, __if)
        printf("\tbuffer pool: (bpid=%d, count=%" PRId64 " size=%" PRId64
               ", addr=0x%" PRIx64 ")\n",
               bpool->bpid, bpool->count, bpool->size, bpool->addr);
  }
}
...
struct netcfg_info *netcfg_acquire(void) {
  ...
  /* Number of MAC ports */
  list_for_each_entry(__if, fman_if_list, node) num_ports++;
  ...
  size = sizeof(*netcfg) + (num_ports * sizeof(struct fm_eth_port_cfg));
