...
static int fman_get_ip_rev(const struct device_node *fman_node) {
  ...
  fman_ip_rev =
      (ip_rev_1 & FMAN_IP_REV_1_MAJOR_MASK) >> FMAN_IP_REV_1_MAJOR_SHIFT;
...
static int fman_get_mac_index(uint64_t regs_addr_host, uint8_t *mac_idx) {
  int ret = 0;
...
static void fman_if_vsp_init(struct __fman_if *__if) {
  ...
  if (__if->__if.mac_type == fman_mac_1g) {
    for_each_compatible_node(dev, NULL, "fsl,fman-port-1g-rx-extended-args") {
      ...
      if (prop) {
        cell_index = of_read_number(&prop[0], lenp / sizeof(phandle));
        if (cell_index == mac_idx[__if->__if.mac_idx]) {
          prop = of_get_property(dev, "vsp-window", &lenp);
          if (prop) {
    ...
    for_each_compatible_node(dev, NULL, "fsl,fman-port-10g-rx-extended-args") {
      prop = of_get_property(dev, "cell-index", &lenp);
      if (prop) {
        cell_index = of_read_number(&prop[0], lenp / sizeof(phandle));
        if (cell_index == mac_idx[__if->__if.mac_idx]) {
...
static int fman_if_init(const struct device_node *dpa_node) {
  ...
  ports_phandle = of_get_property(mac_node, "fsl,port-handles", &lenp);
  if (!ports_phandle)
    ports_phandle = of_get_property(mac_node, "fsl,fman-ports", &lenp);
  if (!ports_phandle) {
  ...
  if (of_device_is_compatible(dpa_node, "fsl,dpa-ethernet")) {
    ...
    if (!port_cell_idx) {
      FMAN_ERR(-ENXIO, "%s: no cell-index for port\n", mname);
      return -ENXIO;
    }
    assert(lenp == sizeof(*port_cell_idx));
    port_cell_idx_val = of_read_number(port_cell_idx, lenp / sizeof(phandle));

    if (of_device_is_compatible(rx_node, "fsl,fman-port-1g-rx"))
      port_cell_idx_val -= 0x8;
    else if (of_device_is_compatible(rx_node, "fsl,fman-port-10g-rx"))
      port_cell_idx_val -= 0x10;

    parent_node_ext_args =
        of_find_compatible_node(NULL, NULL, "fsl,fman-extended-args");
    if (!parent_node_ext_args)
      return 0;

    for_each_child_node(parent_node_ext_args, ext_args_node) {
      ext_args_cell_idx = of_get_property(ext_args_node, "cell-index", &lenp);
      ...
      ext_args_cell_idx_val =
          of_read_number(ext_args_cell_idx, lenp / sizeof(phandle));

      if (port_cell_idx_val == ext_args_cell_idx_val) {
        if (of_device_is_compatible(ext_args_node,
                                    "fsl,fman-port-1g-rx-extended-args") &&
            of_device_is_compatible(rx_node, "fsl,fman-port-1g-rx")) {
          if (of_get_property(ext_args_node, "vsp-window", &lenp))
            is_shared = 1;
          break;
        }
        if (of_device_is_compatible(ext_args_node,
                                    "fsl,fman-port-10g-rx-extended-args") &&
            of_device_is_compatible(rx_node, "fsl,fman-port-10g-rx")) {
          if (of_get_property(ext_args_node, "vsp-window", &lenp))
            is_shared = 1;
          break;
        }
  ...
  __if->ccsr_map = mmap(NULL, __if->regs_size, PROT_READ | PROT_WRITE,
  ...
  if (fman_ip_rev >= FMAN_V3) {
    ...
    fman_dealloc_bufs_mask_hi =
        FMAN_V3_CONTEXTA_EN_A2V | FMAN_V3_CONTEXTA_EN_OVOM;
  ...
  if (of_device_is_compatible(mac_node, "fsl,fman-1g-mac"))
    ...
    char_prop = of_get_property(mac_node, "phy-connection-type", NULL);
    if (!char_prop) {
  ...
  if (_errno) {
    FMAN_ERR(-EINVAL, "Invalid register address: %" PRIx64, regs_addr_host);
    goto err;
  }

  /* Extract the MAC address for private and shared interfaces */
  mac_addr = of_get_property(mac_node, "local-mac-address", &lenp);
  if (!mac_addr) {
  ...
  tx_channel_id = of_get_property(tx_node, "fsl,qman-channel-id", &lenp);
  if (!tx_channel_id) {
  ...
  __if->bmi_map = mmap(NULL, __if->regs_size, PROT_READ | PROT_WRITE,
                       MAP_SHARED, fman_ccsr_map_fd, phys_addr);
  ...
  tx_phandle = of_get_property(dpa_node, "fsl,qman-frame-queues-tx", &lenp);
  if (!tx_phandle) {
  ...
  pools_phandle = of_get_property(dpa_node, "fsl,bman-buffer-pools", &lenp);
  if (!pools_phandle) {
  ...
  while (lenp) {
    ...
    if (!prop)
      prop = of_get_property(pool_node, "fsl,bpool-ethernet-cfg", &proplen);
    if (!prop) {
      ...
      bpool->count = ((uint64_t)bpool_host[0] << 32) | bpool_host[1];
      bpool->size = ((uint64_t)bpool_host[2] << 32) | bpool_host[3];
      bpool->addr = ((uint64_t)bpool_host[4] << 32) | bpool_host[5];
    }
...
void fman_finish(void) {
  ...
  list_for_each_entry_safe(__if, tmpif, &__ifs, __if.node) {
    ...
    if ((__if->__if.mac_type == fman_mac_1g) && (!__if->__if.is_memac))
      ...
      out_be32(__if->ccsr_map + 8, in_be32(__if->ccsr_map + 8) & ~(u32)3);
    /* release the mapping */
