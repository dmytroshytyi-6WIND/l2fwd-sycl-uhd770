...
int process_alloc(enum dpaa_id_type id_type, uint32_t *base, uint32_t num,
  ...
  struct dpaa_ioctl_id_alloc id = {
      .id_type = id_type, .num = num, .align = align, .partial = partial};
  int ret = check_fd();
...
void process_release(enum dpaa_id_type id_type, uint32_t base, uint32_t num) {
  struct dpaa_ioctl_id_release id = {
      .id_type = id_type, .base = base, .num = num};
  int ret = check_fd();
...
int process_portal_irq_unmap(int ifd) { return close(ifd); }
...
#define DPAA_IOCTL_GET_IOCTL_VERSION _IOR(DPAA_IOCTL_MAGIC, 0x14, int)
...
#define DPAA_IOCTL_RESTART_LINK_AUTONEG _IOW(DPAA_IOCTL_MAGIC, 0x13, char *)
