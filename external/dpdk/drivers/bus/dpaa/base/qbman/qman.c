...
struct qman_portal {
   ...
   */
  struct qm_dqrr_entry shadow_dqrr[QM_DQRR_SIZE] __rte_aligned(1024);
#endif
};
...
static irqreturn_t portal_isr(__always_unused int irq, void *ptr) {
   ...
   */
  u32 clear =
      QM_DQAVAIL_MASK | (p->irq_sources & ~(QM_PIRQ_CSCI | QM_PIRQ_CCSCI));
...
static inline int qm_eqcr_init(struct qm_portal *portal,
  ...
  eqcr->vbit = (qm_in(EQCR_PI_CINH) & QM_EQCR_SIZE) ? QM_EQCR_VERB_VBIT : 0;
  eqcr->available = QM_EQCR_SIZE - 1 - qm_cyc_diff(QM_EQCR_SIZE, eqcr->ci, pi);
...
static inline void qm_eqcr_finish(struct qm_portal *portal) {
   ...
   */
  cfg = (qm_in(CFG) & 0x0fffffff) | (1 << 28); /* QCSP_CFG: EST */
  qm_out(CFG, cfg);
...
static inline int
qm_dqrr_init(struct qm_portal *portal,
             __maybe_unused const struct qm_portal_config *config,
             enum qm_dqrr_dmode dmode, __maybe_unused enum qm_dqrr_pmode pmode,
  ...
  dqrr->vbit = (qm_in(DQRR_PI_CINH) & QM_DQRR_SIZE) ? QM_DQRR_VERB_VBIT : 0;
...
#ifdef RTE_LIBRTE_DPAA_HWDEBUG
  if ((dqrr->cmode != qm_dqrr_cdc) && (dqrr->ci != DQRR_PTR2IDX(dqrr->cursor)))
...
static inline int qm_mr_init(struct qm_portal *portal,
  ...
  mr->cmode = cmode;
#endif
  cfg = (qm_in(CFG) & 0xfffff0ff) | ((cmode & 1) << 8); /* QCSP_CFG:MM */
  qm_out(CFG, cfg);
  return 0;
}
...
struct qman_portal *qman_init_portal(struct qman_portal *portal,
   ...
   */
  if (qm_eqcr_init(p, qm_eqcr_pvb, portal->use_eqcr_ci_stashing, 1)) {
    pr_err("Qman EQCR initialisation failed\n");
    goto fail_eqcr;
  }
  if (qm_dqrr_init(p, c, qm_dqrr_dpush, qm_dqrr_pvb, qm_dqrr_cdc,
                   DQRR_MAXFILL)) {
...
static inline void qman_destroy_portal(struct qman_portal *qm) {
...
void qman_ern_register_cb(qman_cb_free_mbuf cb) { qman_free_mbuf_cb = cb; }
...
static u32 __poll_portal_slow(struct qman_portal *p, u32 is) {
  ...
  if (is & QM_PIRQ_CSCI) {
    ...
    qman_cgrs_and(&rr, (const struct qman_cgrs *)&mcr->querycongestion.state,
                  &p->cgrs[0]);
    ...
    /* Invoke callback */
    list_for_each_entry(cgr, &p->cgr_cbs,
                        node) if (cgr->cb && qman_cgrs_get(&c, cgr->cgrid))
...
unsigned int qman_portal_poll_rx(unsigned int poll_limit, void **bufs,
  ...
  do {
     ...
     * copied and the index stored within the copy
     */
    shadow[rx_number] = &p->shadow_dqrr[DQRR_PTR2IDX(dq[rx_number])];
    shadow[rx_number]->fd.opaque_addr = dq[rx_number]->fd.opaque_addr;
    shadow[rx_number]->fd.addr = be40_to_cpu(dq[rx_number]->fd.addr);
    shadow[rx_number]->fd.opaque = be32_to_cpu(dq[rx_number]->fd.opaque);
#else
...
#endif
    /* Now let the callback do its stuff */
    res = fq->cb.dqrr_dpdk_cb(&ev[rx_number], p, fq, dq, &bufs[rx_number]);
    rx_number++;
    /* Interpret 'dq' from a driver perspective. */
    /*
     * Parking isn't possible unless HELDACTIVE was set. NB,
...
void qman_dqrr_consume(struct qman_fq *fq, struct qm_dqrr_entry *dq) {
  struct qman_portal *p = get_affine_portal();
...
u32 qman_fq_fqid(struct qman_fq *fq) { return fq->fqid; }
...
int qman_init_fq(struct qman_fq *fq, u32 flags, struct qm_mcc_initfq *opts) {
  ...
  u8 res, myverb = (flags & QMAN_INITFQ_FLAG_SCHED) ? QM_MCC_VERB_INITFQ_SCHED
                                                    : QM_MCC_VERB_INITFQ_PARKED;

  if ((fq->state != qman_fq_state_oos) && (fq->state != qman_fq_state_parked))
    return -EINVAL;
#ifdef RTE_LIBRTE_DPAA_HWDEBUG
  ...
  fq->state = (flags & QMAN_INITFQ_FLAG_SCHED) ? qman_fq_state_sched
...
int qman_retire_fq(struct qman_fq *fq, u32 *flags) {
  ...
  if ((fq->state != qman_fq_state_parked) && (fq->state != qman_fq_state_sched))
    return -EINVAL;
#ifdef RTE_LIBRTE_DPAA_HWDEBUG
  if (unlikely(fq_isset(fq, QMAN_FQ_FLAG_NO_MODIFY)))
    return -EINVAL;
...
int qman_query_wq(u8 query_dedicated, struct qm_mcr_querywq *wq) {
  ...
  myverb =
      (query_dedicated) ? QM_MCR_VERB_QUERYWQ_DEDICATED : QM_MCR_VERB_QUERYWQ;
  mcc = qm_mc_start(&p->p);
...
int qman_query_cgr(struct qman_cgr *cgr, struct qm_mcr_querycgr *cgrd) {
  ...
  cgrd->cgr.wr_parm_g.word = be32_to_cpu(cgrd->cgr.wr_parm_g.word);
  cgrd->cgr.wr_parm_y.word = be32_to_cpu(cgrd->cgr.wr_parm_y.word);
  cgrd->cgr.wr_parm_r.word = be32_to_cpu(cgrd->cgr.wr_parm_r.word);
  ...
  for (i = 0; i < ARRAY_SIZE(cgrd->cscn_targ_swp); i++)
    cgrd->cscn_targ_swp[i] = be32_to_cpu(cgrd->cscn_targ_swp[i]);
  return 0;
}
...
int qman_query_congestion(struct qm_mcr_querycongestion *congestion) {
  ...
  while (!(mcr = qm_mc_result(&p->p)))
    cpu_relax();
  DPAA_ASSERT((mcr->verb & QM_MCR_VERB_MASK) == QM_MCC_VERB_QUERYCONGESTION);
  res = mcr->result;
  if (res == QM_MCR_RESULT_OK)
  ...
  for (i = 0; i < ARRAY_SIZE(congestion->state.state); i++)
    congestion->state.state[i] = be32_to_cpu(congestion->state.state[i]);
  return 0;
}
...
static inline struct qm_eqcr_entry *try_p_eq_start(struct qman_portal *p,
  ...
  if (flags & QMAN_ENQUEUE_FLAG_DCA)
    eq->dca = QM_EQCR_DCA_ENABLE |
              ((flags & QMAN_ENQUEUE_FLAG_DCA_PARK) ? QM_EQCR_DCA_PARK : 0) |
...
int qman_enqueue_multi(struct qman_fq *fq, const struct qm_fd *fd, u32 *flags,
  ...
  while (eqcr->available && frames_to_send--) {
    ...
    if (flags && (flags[i] & QMAN_ENQUEUE_FLAG_DCA)) {
      eq->dca = QM_EQCR_DCA_ENABLE | ((flags[i] >> 8) & QM_EQCR_DCA_IDXMASK);
    }
    i++;
  ...
  eq = eqcr->cursor;
  for (i = 0; i < sent; i++) {
    eq->__dont_write_directly__verb = QM_EQCR_VERB_CMD_ENQUEUE | eqcr->vbit;
    prev_eq = eq;
    eq = (void *)((unsigned long)(eq + 1) &
                  (~(unsigned long)(QM_EQCR_SIZE << 6)));
...
int qman_enqueue_multi_fq(struct qman_fq *fq[], const struct qm_fd *fd,
...
int qman_create_cgr_to_dcp(struct qman_cgr *cgr, u32 flags, u16 dcp_portal,
  ...
  if ((qman_ip_rev & 0xFF00) >= QMAN_REV30)
    local_opts.cgr.cscn_targ_upd_ctrl =
        QM_CGR_TARG_UDP_CTRL_WRITE_BIT | QM_CGR_TARG_UDP_CTRL_DCP | dcp_portal;
  else
    local_opts.cgr.cscn_targ =
        cgr_state.cgr.cscn_targ | TARG_DCP_MASK(dcp_portal);
  local_opts.we_mask |= QM_CGR_WE_CSCN_TARG;
...
int qman_delete_cgr(struct qman_cgr *cgr) {
   ...
   */
  list_for_each_entry(i, &p->cgr_cbs, node) if ((i->cgrid == cgr->cgrid) &&
                                                i->cb) goto release_lock;
  ret = qman_query_cgr(cgr, &cgr_state);
  if (ret) {
  ...
  case QM_MCR_NP_STATE_PARKED:
    ...
    if (result == QM_MCR_RESULT_PENDING) {
      ...
      if (channel >= qm_channel_pool1 &&
        ...
        dequeue_wq = (channel - qm_channel_pool1 + 1) << 4 | wq;
      ...
      if (channel < qm_channel_pool1)
        qm_dqrr_sdqcr_set(low_p,
                          QM_SDQCR_TYPE_ACTIVE | QM_SDQCR_CHANNELS_DEDICATED);
      else
        qm_dqrr_sdqcr_set(low_p, QM_SDQCR_TYPE_ACTIVE |
      ...
      while (!found_fqrn) {
        ...
        while (dqrr) {
          qm_dqrr_cdc_consume_1ptr(low_p, dqrr, 0);
          qm_dqrr_pvb_update(low_p);
          qm_dqrr_next(low_p);
          dqrr = qm_dqrr_current(low_p);
        }
        ...
        while (msg) {
          if ((msg->ern.verb & QM_MR_VERB_TYPE_MASK) == QM_MR_VERB_FQRN)
            found_fqrn = 1;
          qm_mr_next(low_p);
    ...
    if (result != QM_MCR_RESULT_OK && result != QM_MCR_RESULT_PENDING) {
      /* error */
    ...
    if (drain || mcr->alterfq.fqs & QM_MCR_FQS_NOTEMPTY) {
      ...
      do {
         ...
         */
        while (dqrr) {
          if (dqrr->fqid == fqid && dqrr->stat & QM_DQRR_STAT_FQ_EMPTY)
            fq_empty = 1;
          qm_dqrr_cdc_consume_1ptr(low_p, dqrr, 0);
          qm_dqrr_pvb_update(low_p);
          qm_dqrr_next(low_p);
    ...
    while (orl_empty == 0) {
      ...
      msg = qm_mr_current(low_p);
      while (msg) {
        if ((msg->ern.verb & QM_MR_VERB_TYPE_MASK) == QM_MR_VERB_FQRL)
          orl_empty = 1;
        qm_mr_next(low_p);
        qm_mr_cci_consume_to_current(low_p);
        qm_mr_pvb_update(low_p);
