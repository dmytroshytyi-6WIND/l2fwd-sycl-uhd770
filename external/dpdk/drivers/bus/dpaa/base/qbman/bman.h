...
#define __bm_out(bm, o, val) __raw_writel(cpu_to_be32(val), (bm)->ci + (o))
...
static inline int bm_rcr_init(struct bm_portal *portal, enum bm_rcr_pmode pmode,
  ...
  rcr->available = BM_RCR_SIZE - 1 - bm_cyc_diff(BM_RCR_SIZE, rcr->ci, pi);
...
bm_rcr_pend_and_next(struct bm_portal *portal, u8 myverb) {
  register struct bm_rcr *rcr = &portal->rcr;
...
static inline int bm_mc_init(struct bm_portal *portal) {
  ...
  mc->rridx =
      (__raw_readb(&mc->cr->__dont_write_directly__verb) & BM_MCC_VERB_VBIT)
          ? 0
          : 1;
