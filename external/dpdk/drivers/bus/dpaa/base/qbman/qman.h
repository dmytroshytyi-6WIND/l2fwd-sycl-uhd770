...
#define __qm_out(qm, o, val) __raw_writel((cpu_to_be32(val)), (qm)->ci + (o))
...
static inline struct qm_eqcr_entry *
qm_eqcr_start_no_stash(struct qm_portal *portal) {
...
static inline struct qm_eqcr_entry *
qm_eqcr_start_stash(struct qm_portal *portal) {
  register struct qm_eqcr *eqcr = &portal->eqcr;
  u8 diff, old_ci;
...
static inline struct qm_eqcr_entry *
qm_eqcr_pend_and_next(struct qm_portal *portal, u8 myverb) {
  register struct qm_eqcr *eqcr = &portal->eqcr;
...
static inline struct qm_dqrr_entry *DQRR_INC(const struct qm_dqrr_entry *e) {
  return DQRR_CARRYCLEAR(e + 1);
}

static inline void qm_dqrr_set_maxfill(struct qm_portal *portal, u8 mf) {
  qm_out(CFG, (qm_in(CFG) & 0xff0fffff) | ((mf & (QM_DQRR_SIZE - 1)) << 20));
}

static inline const struct qm_dqrr_entry *
qm_dqrr_current(struct qm_portal *portal) {
...
qm_mr_current(struct qm_portal *portal) {
  register struct qm_mr *mr = &portal->mr;
...
static inline int qm_mc_init(struct qm_portal *portal) {
  ...
  mc->rridx =
      (__raw_readb(&mc->cr->__dont_write_directly__verb) & QM_MCC_VERB_VBIT)
          ? 0
          : 1;
