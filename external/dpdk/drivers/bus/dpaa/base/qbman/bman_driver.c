...
static __thread struct dpaa_ioctl_portal_map map = {.type = dpaa_portal_bman};
...
static int fsl_bman_portal_init(uint32_t idx, int is_shared) {
  ...
  if (!portal) {
    pr_err("Bman portal initialisation failed (%d)", pcfg.cpu);
    process_portal_unmap(&map.addr);
    return -EBUSY;
  }
...
int bman_thread_fd(void) { return bmfd; }
...
int bman_thread_finish(void) { return fsl_bman_portal_finish(); }
...
int bman_global_init(void) {
  ...
  }
  {
    const struct device_node *dn =
        of_find_compatible_node(NULL, NULL, "fsl,bman");
    if (!dn)
