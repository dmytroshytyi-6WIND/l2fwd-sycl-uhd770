...
static inline struct bman_portal *
bman_create_portal(struct bman_portal *portal,
  ...
  if (request_irq(c->irq, NULL, 0, portal->irqname, portal)) {
...
static inline void bman_destroy_portal(struct bman_portal *bm) {
...
int bman_release(struct bman_pool *pool, const struct bm_buffer *bufs, u8 num,
   ...
   * with the valid-bit
   */
  r->bufs[0].opaque = cpu_to_be64(((u64)pool->params.bpid << 48) |
                                  (bufs[0].opaque & BMAN_BUF_MASK));
  if (i) {
    for (i = 1; i < num; i++)
      r->bufs[i].opaque = cpu_to_be64(bufs[i].opaque & BMAN_BUF_MASK);
  }
...
int bman_acquire(struct bman_pool *pool, struct bm_buffer *bufs, u8 num,
  ...
  bm_mc_commit(&p->p,
               BM_MCC_VERB_CMD_ACQUIRE | (num & BM_MCC_VERB_ACQUIRE_BUFCOUNT));
...
int bman_query_pools(struct bm_pool_state *state) {
  ...
  while (!(mcr = bm_mc_result(&p->p)))
    cpu_relax();
  DPAA_ASSERT((mcr->verb & BM_MCR_VERB_CMD_MASK) == BM_MCR_VERB_CMD_QUERY);
  *state = mcr->query;
