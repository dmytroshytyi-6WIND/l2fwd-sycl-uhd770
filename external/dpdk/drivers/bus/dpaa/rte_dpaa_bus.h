 ...
 */
__rte_internal static inline dpaa_seqn_t *dpaa_seqn(struct rte_mbuf *mbuf) {
  return RTE_MBUF_DYNFIELD(mbuf, dpaa_seqn_dynfield_offset, dpaa_seqn_t *);
}
...
enum rte_dpaa_type { FSL_DPAA_ETH = 1, FSL_DPAA_CRYPTO, FSL_DPAA_QDMA };
...
#define DPAA_PER_LCORE_PORTAL RTE_PER_LCORE(dpaa_io)
...
#define DPAA_PER_LCORE_RTE_CRYPTO_OP RTE_PER_LCORE(dpaa_io)->dpaa_sec_ops
#define DPAA_PER_LCORE_DPAA_SEC_OP_NB RTE_PER_LCORE(dpaa_io)->dpaa_sec_op_nb
...
static inline void *rte_dpaa_mem_ptov(phys_addr_t paddr) {
  ...
  TAILQ_FOREACH(ms, &rte_dpaa_memsegs, next) {
    if (paddr >= ms->iova && paddr < ms->iova + ms->len)
...
static inline rte_iova_t rte_dpaa_mem_vtop(void *vaddr) {
 ...
 *   to be registered.
 */
__rte_internal void rte_dpaa_driver_register(struct rte_dpaa_driver *driver);
 ...
 *	to be unregistered.
 */
__rte_internal void rte_dpaa_driver_unregister(struct rte_dpaa_driver *driver);
 ...
 */
__rte_internal int rte_dpaa_portal_init(void *arg);

__rte_internal int rte_dpaa_portal_fq_init(void *arg, struct qman_fq *fq);

__rte_internal int rte_dpaa_portal_fq_close(struct qman_fq *fq);
...
__rte_internal struct fm_eth_port_cfg *dpaa_get_eth_port_cfg(int dev_id);
