...
static inline uint64_t fman_crc64_update(uint64_t crc, void *data,
                                         unsigned int len) {
  uint8_t *p = data;
  while (len--)
    crc = fman_crc64_ecma_182.table[(crc ^ *(p++)) & 0xff] ^ (crc >> 8);
  return crc;
}
 ...
 */
static inline uint64_t fman_crc64_finish(uint64_t seed) { return ~seed; }
