...
__rte_internal u16 dpaa_get_qm_channel_caam(void);

__rte_internal u16 dpaa_get_qm_channel_pool(void);
 ...
 */
#define QM_PIRQ_SLOW                                                           \
  (QM_PIRQ_CSCI | QM_PIRQ_EQCI | QM_PIRQ_EQRI | QM_PIRQ_MRI | QM_PIRQ_CCSCI)
...
static inline u64 qm_fd_addr_get64(const struct qm_fd *fd) { return fd->addr; }
...
static inline u64 qm_fqd_context_a_get64(const struct qm_fqd *fqd) {
  return ((u64)fqd->context_a.hi << 32) | (u64)fqd->context_a.lo;
}
...
typedef enum qman_cb_dqrr_result (*qman_cb_dqrr)(
    struct qman_portal *qm, struct qman_fq *fq,
    const struct qm_dqrr_entry *dqrr);

typedef enum qman_cb_dqrr_result (*qman_dpdk_cb_dqrr)(
    void *event, struct qman_portal *qm, struct qman_fq *fq,
...
typedef void (*qman_dpdk_pull_cb_dqrr)(struct qman_fq **fq,
                                       struct qm_dqrr_entry **dqrr, void **bufs,
                                       int num_bufs);
 ...
 */
typedef void (*qman_cb_cgr)(struct qman_portal *qm, struct qman_cgr *cgr,
                            int congested);
...
#ifdef CONFIG_FSL_QMAN_FQ_LOOKUP
__rte_internal void qman_set_fq_lookup_table(void **table);
#endif
...
__rte_internal u32 qman_portal_dequeue(struct rte_event ev[],
                                       unsigned int poll_limit, void **bufs);
 ...
 * portal.
 */
__rte_internal int qman_fq_portal_irqsource_add(struct qman_portal *p,
                                                u32 bits);
 ...
 */
__rte_internal int qman_irqsource_remove(u32 bits);
 ...
 * portal.
 */
__rte_internal int qman_fq_portal_irqsource_remove(struct qman_portal *p,
                                                   u32 bits);
...
__rte_internal unsigned int qman_portal_poll_rx(unsigned int poll_limit,
                                                void **bufs,
                                                struct qman_portal *q);
 ...
 */
__rte_internal int qman_set_vdq(struct qman_fq *fq, u16 num,
 ...
 * the DQRR.
 */
__rte_internal struct qm_dqrr_entry *qman_dequeue(struct qman_fq *fq);
 ...
 * dequeue.
 */
__rte_internal void qman_dqrr_consume(struct qman_fq *fq,
 ...
 */
__rte_internal int qman_poll_dqrr(unsigned int limit);
 ...
 * access to.
 */
__rte_internal void qman_static_dequeue_add(u32 pools, struct qman_portal *qm);
 ...
 * entry in the first place.
 */
__rte_internal void qman_dca_index(u8 index, int park_request);
 ...
 * Out of Service.
 */
__rte_internal int qman_create_fq(u32 fqid, u32 flags, struct qman_fq *fq);
 ...
 */
__rte_internal u32 qman_fq_fqid(struct qman_fq *fq);
 ...
 * executes.
 */
__rte_internal void qman_fq_state(struct qman_fq *fq, enum qman_fq_state *state,
                                  u32 *flags);
 ...
 */
__rte_internal int qman_init_fq(struct qman_fq *fq, u32 flags,
 ...
 */
__rte_internal int qman_retire_fq(struct qman_fq *fq, u32 *flags);
 ...
 */
__rte_internal int qman_oos_fq(struct qman_fq *fq);
 ...
 */
__rte_internal int qman_query_fq_np(struct qman_fq *fq,
 ...
 */
__rte_internal int qman_query_fq_frm_cnt(struct qman_fq *fq, u32 *frm_cnt);
 ...
 */
__rte_internal int qman_volatile_dequeue(struct qman_fq *fq, u32 flags,
                                         u32 vdqcr);
 ...
 */
__rte_internal int qman_enqueue(struct qman_fq *fq, const struct qm_fd *fd,
                                u32 flags);

__rte_internal int qman_enqueue_multi(struct qman_fq *fq,
 ...
 */
__rte_internal void qman_ern_poll_free(void);

/**
 * qman_ern_register_cb - Register a callback function to free buffers.
 */
__rte_internal void qman_ern_register_cb(qman_cb_free_mbuf cb);
 ...
 */
__rte_internal int qman_enqueue_multi_fq(struct qman_fq *fq[],
                                         const struct qm_fd *fd, u32 *flags,
                                         int frames_to_send);
 ...
 */
__rte_internal int qman_alloc_fqid_range(u32 *result, u32 count, u32 align,
                                         int partial);
 ...
 * @count: the number of FQIDs in the range
 */
__rte_internal int qman_reserve_fqid_range(u32 fqid, unsigned int count);
 ...
 */
__rte_internal int qman_alloc_pool_range(u32 *result, u32 count, u32 align,
                                         int partial);
...
static inline void qman_release_pool(u32 id) { qman_release_pool_range(id, 1); }
 ...
 */
__rte_internal int qman_create_cgr(struct qman_cgr *cgr, u32 flags,
 ...
 * created.
 */
__rte_internal int qman_delete_cgr(struct qman_cgr *cgr);
 ...
 */
__rte_internal int qman_modify_cgr(struct qman_cgr *cgr, u32 flags,
                                   struct qm_mcc_initcgr *opts);
 ...
 * the return value will either be 'count' or negative.
 */
__rte_internal int qman_alloc_cgrid_range(u32 *result, u32 count, u32 align,
                                          int partial);
 ...
 */
__rte_internal void qman_release_cgrid_range(u32 id, unsigned int count);
