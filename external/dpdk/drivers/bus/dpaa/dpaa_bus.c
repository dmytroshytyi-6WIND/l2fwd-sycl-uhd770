...
static int compare_dpaa_devices(struct rte_dpaa_device *dev1,
...
static inline void dpaa_add_to_device_list(struct rte_dpaa_device *newdev) {
  int comp, inserted = 0;
...
static inline int dpaa_sec_available(void) {
  ...
  for_each_compatible_node(caam_node, NULL, "fsl,sec-v4.0") { return 0; }
...
static struct rte_devargs *dpaa_devargs_lookup(struct rte_dpaa_device *dev) {
...
static int dpaa_create_device_list(void) {
  int i;
  int ret;
...
int rte_dpaa_portal_init(void *arg) {
  ...
  if (dpaa_seqn_dynfield_offset < 0) {
    DPAA_BUS_LOG(ERR,
                 "Failed to register mbuf field for dpaa sequence number\n");
    return -rte_errno;
  }
  ...
  if (ret) {
    DPAA_BUS_LOG(ERR,
                 "bman_thread_init failed on core %u"
                 " (lcore=%u) with ret: %d",
                 cpu, lcore, ret);
    return ret;
  }

  DPAA_BUS_LOG(DEBUG, "BMAN thread initialized - CPU=%d lcore=%d", cpu, lcore);

  /* Initialise qman thread portals */
  ret = qman_thread_init();
  if (ret) {
    DPAA_BUS_LOG(ERR,
                 "qman_thread_init failed on core %u"
                 " (lcore=%u) with ret: %d",
                 cpu, lcore, ret);
    bman_thread_finish();
    return ret;
  ...
  DPAA_BUS_LOG(DEBUG, "QMAN thread initialized - CPU=%d lcore=%d", cpu, lcore);

  DPAA_PER_LCORE_PORTAL =
      rte_malloc(NULL, sizeof(struct dpaa_portal), RTE_CACHE_LINE_SIZE);
  ...
  ret = pthread_setspecific(dpaa_portal_key, (void *)DPAA_PER_LCORE_PORTAL);
  if (ret) {
    DPAA_BUS_LOG(ERR,
                 "pthread_setspecific failed on core %u"
                 " (lcore=%u) with ret: %d",
                 cpu, lcore, ret);
...
static int rte_dpaa_bus_parse(const char *name, void *out) {
  unsigned int i, j;
  size_t delta;
  size_t max_name_len;
  ...
  if (strncmp("dpaa_sec", &name[delta], 8) == 0) {
    if (sscanf(&name[delta], "dpaa_sec-%u", &i) != 1 || i < 1 || i > 4)
      return -EINVAL;
  ...
  if (out != NULL) {
    ...
    if ((strlen(out_name) == max_name_len) && out_name[max_name_len] == ',')
...
static int rte_dpaa_bus_scan(void) {
  int ret;
...
static int rte_dpaa_device_match(struct rte_dpaa_driver *drv,
                                 struct rte_dpaa_device *dev) {
  if (!drv || !dev) {
...
static int rte_dpaa_bus_dev_build(void) {
  int ret;
  ...
  if (!dpaa_netcfg) {
    DPAA_BUS_LOG(ERR, "netcfg failed: /dev/fsl_usdpaa device not available");
...
static int rte_dpaa_setup_intr(struct rte_intr_handle *intr_handle) {
  ...
  if (fd < 0) {
    DPAA_BUS_ERR("Cannot set up eventfd, error %i (%s)", errno,
                 strerror(errno));
    return errno;
  }
...
static int rte_dpaa_bus_probe(void) {
  int ret = -1;
  struct rte_dpaa_device *dev;
  ...
  TAILQ_FOREACH(dev, &rte_dpaa_bus.device_list, next) {
    TAILQ_FOREACH(drv, &rte_dpaa_bus.driver_list, next) {
      ...
      if (!drv->probe || (dev->device.devargs &&
...
static struct rte_device *rte_dpaa_find_device(const struct rte_device *start,
                                               rte_dev_cmp_t cmp,
                                               const void *data) {
  struct rte_dpaa_device *dev;
 ...
 */
static enum rte_iova_mode rte_dpaa_get_iommu_class(void) {
  if ((access(DPAA_DEV_PATH1, F_OK) != 0) &&
      (access(DPAA_DEV_PATH2, F_OK) != 0)) {
    return RTE_IOVA_DC;
  }
...
static int dpaa_bus_plug(struct rte_device *dev __rte_unused) {
  /* No operation is performed while plugging the device */
  return 0;
}

static int dpaa_bus_unplug(struct rte_device *dev __rte_unused) {
