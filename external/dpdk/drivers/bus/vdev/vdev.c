...
int rte_vdev_add_custom_scan(rte_vdev_scan_callback callback, void *user_arg) {
  ...
  TAILQ_FOREACH(custom_scan, &vdev_custom_scans, next) {
    if (custom_scan->callback == callback && custom_scan->user_arg == user_arg)
      break;
  }
...
int rte_vdev_remove_custom_scan(rte_vdev_scan_callback callback,
  ...
  rte_spinlock_lock(&vdev_custom_scan_lock);
  RTE_TAILQ_FOREACH_SAFE(custom_scan, &vdev_custom_scans, next, tmp_scan) {
...
static int vdev_parse(const char *name, void *addr) {
  struct rte_vdev_driver **out = addr;
  struct rte_vdev_driver *driver = NULL;

  TAILQ_FOREACH(driver, &vdev_driver_list, next) {
    if (strncmp(driver->driver.name, name, strlen(driver->driver.name)) == 0)
      break;
    if (driver->driver.alias &&
        strncmp(driver->driver.alias, name, strlen(driver->driver.alias)) == 0)
      break;
  }
  if (driver != NULL && addr != NULL)
    *out = driver;
  return driver == NULL;
}

static int vdev_dma_map(struct rte_device *dev, void *addr, uint64_t iova,
                        size_t len) {
  ...
  driver =
      container_of(vdev->device.driver, const struct rte_vdev_driver, driver);
...
static int vdev_dma_unmap(struct rte_device *dev, void *addr, uint64_t iova,
                          size_t len) {
  struct rte_vdev_device *vdev = RTE_DEV_TO_VDEV(dev);
  const struct rte_vdev_driver *driver;
...
static int vdev_probe_all_drivers(struct rte_vdev_device *dev) {
  ...
  if ((driver->drv_flags & RTE_VDEV_DRV_NEED_IOVA_AS_VA) &&
    ...
    VDEV_LOG(
        ERR,
        "%s requires VA IOVA mode but current mode is PA, not initializing",
        name);
    return -1;
  }
...
static struct rte_vdev_device *find_vdev(const char *name) {
...
static struct rte_devargs *alloc_devargs(const char *name, const char *args) {
...
static int insert_vdev(const char *name, const char *args,
                       struct rte_vdev_device **p_dev, bool init) {
  struct rte_vdev_device *dev;
  struct rte_devargs *devargs;
  int ret;
...
static int vdev_remove_driver(struct rte_vdev_device *dev) {
  ...
  driver =
      container_of(dev->device.driver, const struct rte_vdev_driver, driver);
 ...
 * are sent.
 */
static int vdev_action(const struct rte_mp_msg *mp_msg, const void *peer) {
  struct rte_vdev_device *dev;
  ...
  case VDEV_SCAN_REQ:
    ...
    TAILQ_FOREACH(dev, &vdev_device_list, next) {
      ...
      if (rte_mp_sendmsg(&mp_resp) < 0)
        VDEV_LOG(ERR, "send vdev, %s, failed, %s", devname,
                 strerror(rte_errno));
      num++;
    }
    rte_spinlock_recursive_unlock(&vdev_device_list_lock);
...
static int vdev_scan(void) {
  ...
  if (rte_mp_action_register(VDEV_MP_KEY, vdev_action) < 0 &&
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY && rte_errno == ENOTSUP)
      goto scan;
...
struct rte_device *rte_vdev_find_device(const struct rte_device *start,
...
static int vdev_unplug(struct rte_device *dev) {
  return rte_vdev_uninit(dev->name);
}

static enum rte_iova_mode vdev_get_iommu_class(void) {
  const char *name;
  struct rte_vdev_device *dev;
  struct rte_vdev_driver *driver;
