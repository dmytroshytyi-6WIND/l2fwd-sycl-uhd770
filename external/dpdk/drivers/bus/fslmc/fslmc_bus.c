...
static void cleanup_fslmc_device_list(void) {
...
static int compare_dpaa2_devname(struct rte_dpaa2_device *dev1,
...
static void insert_in_device_list(struct rte_dpaa2_device *newdev) {
  int comp, inserted = 0;
...
static struct rte_devargs *fslmc_devargs_lookup(struct rte_dpaa2_device *dev) {
...
static void dump_device_list(void) {
  struct rte_dpaa2_device *dev;
...
static int scan_one_fslmc_device(char *dev_name) {
...
static int rte_fslmc_parse(const char *name, void *addr) {
  uint16_t dev_id;
  char *t_ptr;
  const char *sep;
...
jump_out:
  ...
  if (strncmp("dpni", sep, 4) && strncmp("dpseci", sep, 6) &&
      strncmp("dpcon", sep, 5) && strncmp("dpbp", sep, 4) &&
      strncmp("dpio", sep, 4) && strncmp("dpci", sep, 4) &&
      strncmp("dpmcp", sep, 5) && strncmp("dpdmai", sep, 6) &&
...
static int rte_fslmc_scan(void) {
  int ret;
...
static int rte_fslmc_match(struct rte_dpaa2_driver *dpaa2_drv,
...
static int rte_fslmc_probe(void) {
  int ret = 0;
  int probe_all;
...
static struct rte_device *rte_fslmc_find_device(const struct rte_device *start,
                                                rte_dev_cmp_t cmp,
                                                const void *data) {
 ...
 */
static inline int fslmc_all_device_support_iova(void) {
  int ret = 0;
  struct rte_dpaa2_device *dev;
  struct rte_dpaa2_driver *drv;
 ...
 */
static enum rte_iova_mode rte_dpaa2_get_iommu_class(void) {
  ...
  is_vfio_noiommu_enabled =
      rte_vfio_noiommu_is_enabled() == true ? true : false;
#endif
...
static int fslmc_bus_plug(struct rte_device *dev __rte_unused) {
  /* No operation is performed while plugging the device */
  return 0;
}

static int fslmc_bus_unplug(struct rte_device *dev __rte_unused) {
