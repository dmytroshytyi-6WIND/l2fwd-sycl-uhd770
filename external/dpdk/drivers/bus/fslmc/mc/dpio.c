 ...
 */
int dpio_open(struct fsl_mc_io *mc_io, uint32_t cmd_flags, int dpio_id,
              uint16_t *token) {
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_OPEN, cmd_flags, 0);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpio_close(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_CLOSE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpio_create(struct fsl_mc_io *mc_io, uint16_t dprc_token,
                uint32_t cmd_flags, const struct dpio_cfg *cfg,
                uint32_t *obj_id) {
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_CREATE, cmd_flags, dprc_token);
  cmd_params = (struct dpio_cmd_create *)cmd.params;
  cmd_params->num_priorities = cfg->num_priorities;
  dpio_set_field(cmd_params->channel_mode, CHANNEL_MODE, cfg->channel_mode);
 ...
 * Return:	'0' on Success; Error code otherwise
 */
int dpio_destroy(struct fsl_mc_io *mc_io, uint16_t dprc_token,
                 uint32_t cmd_flags, uint32_t object_id) {
  struct dpio_cmd_destroy *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_DESTROY, cmd_flags, dprc_token);
 ...
 * Return:	'0' on Success; Error code otherwise
 */
int dpio_enable(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_ENABLE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise
 */
int dpio_disable(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_DISABLE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpio_is_enabled(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token,
                    int *en) {
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_IS_ENABLED, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpio_reset(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_RESET, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise
 */
int dpio_get_attributes(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                        uint16_t token, struct dpio_attr *attr) {
  struct dpio_rsp_get_attr *rsp_params;
  struct mc_command cmd = {0};
  int err;

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_GET_ATTR, cmd_flags, token);
  ...
  attr->channel_mode =
      dpio_get_field(rsp_params->channel_mode, ATTR_CHANNEL_MODE);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpio_set_stashing_destination(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                  uint16_t token, uint8_t sdest) {
  struct dpio_stashing_dest *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPIO_CMDID_SET_STASHING_DEST, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpio_get_stashing_destination(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                  uint16_t token, uint8_t *sdest) {
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPIO_CMDID_GET_STASHING_DEST, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpio_add_static_dequeue_channel(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                    uint16_t token, int dpcon_id,
...
int dpio_remove_static_dequeue_channel(struct fsl_mc_io *mc_io,
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPIO_CMDID_REMOVE_STATIC_DEQUEUE_CHANNEL,
                                    cmd_flags, token);
  cmd_params = (struct dpio_cmd_static_dequeue_channel *)cmd.params;
  cmd_params->dpcon_id = cpu_to_le32(dpcon_id);
 ...
 * Return:  '0' on Success; Error code otherwise.
 */
int dpio_get_api_version(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                         uint16_t *major_ver, uint16_t *minor_ver) {
  struct dpio_rsp_get_api_version *rsp_params;
  struct mc_command cmd = {0};
  int err;

  cmd.header = mc_encode_cmd_header(DPIO_CMDID_GET_API_VERSION, cmd_flags, 0);
