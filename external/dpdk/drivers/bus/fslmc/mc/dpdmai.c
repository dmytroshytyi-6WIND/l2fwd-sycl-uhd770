 ...
 */
int dpdmai_open(struct fsl_mc_io *mc_io, uint32_t cmd_flags, int dpdmai_id,
                uint16_t *token) {
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_close(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_create(struct fsl_mc_io *mc_io, uint16_t dprc_token,
                  uint32_t cmd_flags, const struct dpdmai_cfg *cfg,
                  uint32_t *obj_id) {
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMAI_CMDID_CREATE, cmd_flags, dprc_token);
 ...
 * Return:	'0' on Success; error code otherwise.
 */
int dpdmai_destroy(struct fsl_mc_io *mc_io, uint16_t dprc_token,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMAI_CMDID_DESTROY, cmd_flags, dprc_token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_enable(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_disable(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                   uint16_t token) {
  struct mc_command cmd = {0};
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_is_enabled(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMAI_CMDID_IS_ENABLED, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_reset(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_get_attributes(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                          uint16_t token, struct dpdmai_attr *attr) {
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_set_rx_queue(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                        uint16_t token, uint8_t queue_idx, uint8_t priority,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMAI_CMDID_SET_RX_QUEUE, cmd_flags, token);
  ...
  dpdmai_set_field(cmd_params->dest_type, DEST_TYPE, cfg->dest_cfg.dest_type);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_get_rx_queue(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                        uint16_t token, uint8_t queue_idx, uint8_t priority,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMAI_CMDID_GET_RX_QUEUE, cmd_flags, token);
  ...
  attr->dest_cfg.dest_type = dpdmai_get_field(rsp_params->dest_type, DEST_TYPE);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmai_get_tx_queue(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                        uint16_t token, uint8_t queue_idx, uint8_t priority,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMAI_CMDID_GET_TX_QUEUE, cmd_flags, token);
  cmd_params = (struct dpdmai_cmd_get_queue *)cmd.params;
  cmd_params->priority = priority;
  cmd_params->queue_idx = queue_idx;
