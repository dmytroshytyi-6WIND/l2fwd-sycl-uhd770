...
enum qbman_fd_format { qbman_fd_single = 0, qbman_fd_list, qbman_fd_sg };
...
#define DPAA2_SET_ONLY_FD_BPID(fd, bpid) ((fd)->simple.bpid_offset = bpid)
...
#define DPAA2_FLE_SAVE_CTXT(fle, addr)                                         \
  do {                                                                         \
...
#define DPAA2_FD_SET_FORMAT(fd, format)                                        \
  do {                                                                         \
...
#define DPAA2_SG_SET_FORMAT(sg, format)                                        \
  do {                                                                         \
    (sg)->fin_bpid_offset &= 0xCFFFFFFF;                                       \
    (sg)->fin_bpid_offset |= (uint32_t)format << 28;                           \
  } while (0)

#define DPAA2_SG_SET_FINAL(sg, fin)                                            \
  do {                                                                         \
...
static inline int check_swp_active_dqs(uint16_t dpio_index) {
  if (rte_global_active_dqs_list[dpio_index].global_active_dqs != NULL)
    return 1;
  return 0;
}

static inline void clear_swp_active_dqs(uint16_t dpio_index) {
  rte_global_active_dqs_list[dpio_index].global_active_dqs = NULL;
}

static inline struct qbman_result *get_swp_active_dqs(uint16_t dpio_index) {
  return rte_global_active_dqs_list[dpio_index].global_active_dqs;
}

static inline void set_swp_active_dqs(uint16_t dpio_index,
                                      struct qbman_result *dqs) {
  rte_global_active_dqs_list[dpio_index].global_active_dqs = dqs;
}

__rte_internal struct dpaa2_dpbp_dev *dpaa2_alloc_dpbp_dev(void);

__rte_internal void dpaa2_free_dpbp_dev(struct dpaa2_dpbp_dev *dpbp);

__rte_internal int dpaa2_dpbp_supported(void);

__rte_internal struct dpaa2_dpci_dev *rte_dpaa2_alloc_dpci_dev(void);

__rte_internal void rte_dpaa2_free_dpci_dev(struct dpaa2_dpci_dev *dpci);

/* Global MCP pointer */
__rte_internal void *dpaa2_get_mcp_ptr(int portal_idx);
