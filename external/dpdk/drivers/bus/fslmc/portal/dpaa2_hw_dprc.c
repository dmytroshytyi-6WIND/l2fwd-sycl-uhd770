...
rte_dpaa2_create_dprc_device(int vdev_fd __rte_unused,
  ...
  ret = dprc_open(&dprc_node->dprc, CMD_PRI_LOW, dprc_id, &dprc_node->token);
  if (ret) {
  ...
  RTE_TAILQ_FOREACH_SAFE(dev, &rte_fslmc_bus.device_list, next, dev_tmp) {
    if (dev->dev_type == DPAA2_ETH) {
      ...
      ret = dprc_get_connection(&dprc_node->dprc, CMD_PRI_LOW, dprc_node->token,
                                &endpoint1, &endpoint2, &link_state);
      if (ret) {
        DPAA2_BUS_ERR("dpni.%d connection failed!", dev->object_id);
        dprc_close(&dprc_node->dprc, CMD_PRI_LOW, dprc_node->token);
        rte_free(dprc_node);
        return ret;
      }
