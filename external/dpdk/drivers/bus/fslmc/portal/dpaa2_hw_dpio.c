...
static int dpaa2_get_core_id(void) {
  rte_cpuset_t cpuset;
  int i, ret, cpu_id = -1;
...
static int dpaa2_core_cluster_sdest(int cpu_id) {
...
#define COMMAND_LEN 50
  ...
  if (!token) {
    DPAA2_BUS_WARN("Failed to get interrupt id for dpio.%d", dpio_id);
    free(temp);
    fclose(file);
    return;
  }
  ...
  ret = system(command);
  if (ret < 0)
    DPAA2_BUS_DEBUG("Failed to affine interrupts on respective core");
  else
...
static int dpaa2_configure_stashing(struct dpaa2_dpio_dev *dpio_dev,
                                    int cpu_id) {
  int sdest, ret;
  ...
  DPAA2_BUS_DEBUG("Portal= %d  CPU= %u SDEST= %d", dpio_dev->index, cpu_id,
                  sdest);
...
static struct dpaa2_dpio_dev *dpaa2_get_qbman_swp(void) {
  ...
  DPAA2_BUS_DEBUG("New Portal %p (%d) affined thread - %u", dpio_dev,
                  dpio_dev->index, rte_gettid());
...
int dpaa2_affine_qbman_swp(void) {
  ...
  if (!RTE_PER_LCORE(_dpaa2_io).dpio_dev) {
    ...
    DPAA2_BUS_INFO("DPAA Portal=%p (%d) is affined to thread %" PRIu64,
...
int dpaa2_affine_qbman_ethrx_swp(void) {
  ...
  if (!RTE_PER_LCORE(_dpaa2_io).ethrx_dpio_dev) {
    ...
    DPAA2_BUS_INFO(
        "DPAA Portal=%p (%d) is affined for eth rx to thread %" PRIu64,
        dpio_dev, dpio_dev->index, tid);
  }
  return 0;
}
...
static int dpaa2_create_dpio_device(int vdev_fd,
  ...
  dpio_dev =
      rte_zmalloc(NULL, sizeof(struct dpaa2_dpio_dev), RTE_CACHE_LINE_SIZE);
  if (!dpio_dev) {
  ...
  dpio_dev->intr_handle = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_SHARED);
  ...
  dpio_dev->dpio =
      rte_zmalloc(NULL, sizeof(struct fsl_mc_io), RTE_CACHE_LINE_SIZE);
  if (!dpio_dev->dpio) {
  ...
  if (dpio_get_attributes(dpio_dev->dpio, CMD_PRI_LOW, dpio_dev->token,
                          &attr)) {
  ...
  dpio_dev->qbman_portal_ce_paddr =
      (size_t)mmap(NULL, reg_info.size, PROT_WRITE | PROT_READ, MAP_SHARED,
  ...
  dpio_dev->qbman_portal_ci_paddr =
      (size_t)mmap(NULL, reg_info.size, PROT_WRITE | PROT_READ, MAP_SHARED,
                   vdev_fd, reg_info.offset);
  ...
  if (rte_dpaa2_vfio_setup_intr(dpio_dev->intr_handle, vdev_fd, 1)) {
    DPAA2_BUS_ERR("Fail to setup interrupt for %d", dpio_dev->hw_id);
    goto err;
  }

  dpio_dev->eqresp =
      rte_zmalloc(NULL,
                  MAX_EQ_RESP_ENTRIES * (sizeof(struct qbman_result) +
  ...
  dpio_dev->eqresp_meta =
      (struct eqresp_metadata *)(dpio_dev->eqresp + MAX_EQ_RESP_ENTRIES);
  ...
  if (!dpaa2_portal_key) {
    ...
    if (ret) {
      DPAA2_BUS_DEBUG("Unable to create pthread key (%d)", ret);
      goto err;
    }
  }
...
int dpaa2_alloc_dq_storage(struct queue_storage_info_t *q_storage) {
  ...
  for (i = 0; i < NUM_DQS_PER_QUEUE; i++) {
    q_storage->dq_storage[i] =
        rte_malloc(NULL, dpaa2_dqrr_size * sizeof(struct qbman_result),
                   RTE_CACHE_LINE_SIZE);
...
uint32_t dpaa2_free_eq_descriptors(void) {
  ...
  while (dpio_dev->eqresp_ci != dpio_dev->eqresp_pi) {
    ...
    dpio_dev->eqresp_ci + 1 < MAX_EQ_RESP_ENTRIES ? dpio_dev->eqresp_ci++
                                                  : (dpio_dev->eqresp_ci = 0);
  }
  ...
  if (dpio_dev->eqresp_ci > dpio_dev->eqresp_pi)
    ...
    return dpio_dev->eqresp_ci - dpio_dev->eqresp_pi + MAX_EQ_RESP_ENTRIES - 1;
}
