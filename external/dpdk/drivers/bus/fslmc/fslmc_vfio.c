...
int fslmc_get_container_group(int *groupid) {
  ...
  fslmc_iommu_type = (rte_vfio_noiommu_is_enabled() == 1) ? RTE_VFIO_NOIOMMU
                                                          : VFIO_TYPE1_IOMMU;

  /* get group number */
  ret = rte_vfio_get_group_num(SYSFS_FSL_MC_DEVICES, fslmc_container, groupid);
  if (ret <= 0) {
    DPAA2_BUS_ERR("Unable to find %s IOMMU group", fslmc_container);
    return -1;
  }

  DPAA2_BUS_DEBUG("Container: %s has VFIO iommu group id = %d", fslmc_container,
                  *groupid);
...
static int vfio_connect_container(void) {
  ...
  if (!ioctl(vfio_group.fd, VFIO_GROUP_SET_CONTAINER, &vfio_container.fd)) {
    DPAA2_BUS_DEBUG("Container pre-exists with FD[0x%x] for this group",
                    vfio_container.fd);
...
static int vfio_map_irq_region(struct fslmc_vfio_group *group) {
  ...
  vaddr = (unsigned long *)mmap(NULL, 0x1000, PROT_WRITE | PROT_READ,
                                MAP_SHARED, container_device_fd, 0x6030000);
  if (vaddr == MAP_FAILED) {
...
static void fslmc_memevent_cb(enum rte_mem_event type, const void *addr,
  ...
  while (cur_len < len) {
    ...
    DPAA2_BUS_DEBUG("Request for %s, va=%p, "
                    ...
                    type == RTE_MEM_EVENT_ALLOC ? "alloc" : "dealloc", va,
                    virt_addr, iova_addr, map_len);
  ...
  if (type == RTE_MEM_EVENT_ALLOC)
    DPAA2_BUS_DEBUG("Total Mapped: addr=%p, len=%zu", addr, len);
  else
    DPAA2_BUS_DEBUG("Total Unmapped: addr=%p, len=%zu", addr, len);
}

static int fslmc_map_dma(uint64_t vaddr, rte_iova_t iovaddr __rte_unused,
                         size_t len) {
  ...
  if (ret) {
    DPAA2_BUS_ERR("VFIO_IOMMU_MAP_DMA API(errno = %d)", errno);
    return -1;
  }
...
static int fslmc_unmap_dma(uint64_t vaddr, uint64_t iovaddr __rte_unused,
                           size_t len) {
  struct fslmc_vfio_group *group;
  ...
  if (ret) {
    DPAA2_BUS_ERR("VFIO_IOMMU_UNMAP_DMA API(errno = %d)", errno);
    return -1;
  }
...
static int fslmc_dmamap_seg(const struct rte_memseg_list *msl __rte_unused,
  ...
  if (ret)
    DPAA2_BUS_ERR("Unable to VFIO map (addr=%p, len=%zu)", ms->addr, ms->len);
  else
    (*n_segs)++;
...
int rte_fslmc_vfio_dmamap(void) {
  ...
  ret = rte_mem_event_callback_register("fslmc_memevent_clb", fslmc_memevent_cb,
                                        NULL);
...
static int fslmc_vfio_setup_device(const char *sysfs_base, const char *dev_addr,
                                   int *vfio_dev_fd,
  ...
  *vfio_dev_fd = ioctl(vfio_group_fd, VFIO_GROUP_GET_DEVICE_FD, dev_addr);
  ...
  if (ret) {
    DPAA2_BUS_ERR("  %s cannot get device info, error %i (%s)", dev_addr, errno,
                  strerror(errno));
    close(*vfio_dev_fd);
    close(vfio_group_fd);
    close(vfio_container_fd);
...
static intptr_t vfio_map_mcp_obj(const char *mcp_obj) {
  ...
  fslmc_vfio_setup_device(SYSFS_FSL_MC_DEVICES, mcp_obj, &mc_fd, &d_info);
  ...
  v_addr = (size_t)mmap(NULL, reg_info.size, PROT_WRITE | PROT_READ, MAP_SHARED,
  ...
  *fd_ptr = rte_intr_fd_get(intr_handle);
  ...
  if (ret) {
    DPAA2_BUS_ERR("Error:dpaa2 SET IRQs fd=%d, err = %d(%s)",
                  rte_intr_fd_get(intr_handle), errno, strerror(errno));
    return ret;
  }
...
int rte_dpaa2_vfio_setup_intr(struct rte_intr_handle *intr_handle,
  ...
  for (i = 0; i < num_irqs; i++) {
    ...
    if (ret < 0) {
      DPAA2_BUS_ERR("Cannot get IRQ(%d) info, error %i (%s)", i, errno,
                    strerror(errno));
      return -1;
    }
    ...
    if (fd < 0) {
      DPAA2_BUS_ERR("Cannot set up eventfd, error %i (%s)", errno,
                    strerror(errno));
      return -1;
    }
 ...
 * EVENT) devices.
 */
static int fslmc_process_iodevices(struct rte_dpaa2_device *dev) {
  int dev_fd;
  struct vfio_device_info device_info = {.argsz = sizeof(device_info)};
  struct rte_dpaa2_object *object = NULL;

  fslmc_vfio_setup_device(SYSFS_FSL_MC_DEVICES, dev->device.name, &dev_fd,
                          &device_info);

  switch (dev->dev_type) {
  case DPAA2_ETH:
    rte_dpaa2_vfio_setup_intr(dev->intr_handle, dev_fd, device_info.num_irqs);
    break;
  case DPAA2_CON:
  case DPAA2_IO:
  case DPAA2_CI:
  ...
  case DPAA2_DPRC:
    TAILQ_FOREACH(object, &dpaa2_obj_list, next) {
      if (dev->dev_type == object->dev_type)
        object->create(dev_fd, &device_info, dev->object_id);
      else
        continue;
    }
    break;
  ...
  DPAA2_BUS_LOG(DEBUG, "Device (%s) abstracted from VFIO", dev->device.name);
  return 0;
}

static int fslmc_process_mcp(struct rte_dpaa2_device *dev) {
  int ret;
  intptr_t v_addr;
  ...
  if ((mc_ver_info.major != MC_VER_MAJOR) ||
    ...
    DPAA2_BUS_ERR("DPAA2 MC version not compatible!"
                  ...
                  MC_VER_MAJOR, MC_VER_MINOR, mc_ver_info.major,
...
int fslmc_vfio_process_group(void) {
  ...
  RTE_TAILQ_FOREACH_SAFE(dev, &rte_fslmc_bus.device_list, next, dev_temp) {
    if (dev->dev_type == DPAA2_MPORTAL) {
      ...
      if (rte_eal_process_type() == RTE_PROC_SECONDARY &&
        ...
        if (dpmcp_count == 1 || current_device != dpmcp_count) {
          TAILQ_REMOVE(&rte_fslmc_bus.device_list, dev, next);
          continue;
        }
      }
  ...
  RTE_TAILQ_FOREACH_SAFE(dev, &rte_fslmc_bus.device_list, next, dev_temp) {
    ...
    if (rte_eal_process_type() == RTE_PROC_SECONDARY &&
        dev->dev_type != DPAA2_ETH && dev->dev_type != DPAA2_CRYPTO &&
        dev->dev_type != DPAA2_QDMA && dev->dev_type != DPAA2_IO) {
      TAILQ_REMOVE(&rte_fslmc_bus.device_list, dev, next);
      continue;
    }
    switch (dev->dev_type) {
    ...
    case DPAA2_QDMA:
      ret = fslmc_process_iodevices(dev);
      if (ret) {
        DPAA2_BUS_DEBUG("Dev (%s) init failed", dev->device.name);
        return ret;
      }
      break;
    case DPAA2_CON:
    ...
    default:
      /* Unknown - ignore */
      DPAA2_BUS_DEBUG("Found unknown device (%s)", dev->device.name);
      TAILQ_REMOVE(&rte_fslmc_bus.device_list, dev, next);
      free(dev);
      dev = NULL;
    }
...
int fslmc_vfio_setup_group(void) {
  ...
  if (!(status.flags & VFIO_GROUP_FLAGS_CONTAINER_SET)) {
    ...
    if (ret) {
      DPAA2_BUS_ERR("Error connecting container with groupid %d", groupid);
      close(vfio_group.fd);
      rte_vfio_clear_group(vfio_group.fd);
      return ret;
    }
  ...
  if (ret < 0) {
    DPAA2_BUS_ERR("Error getting device %s fd from group %d", fslmc_container,
                  vfio_group.groupid);
    close(vfio_group.fd);
    rte_vfio_clear_group(vfio_group.fd);
    return ret;
  ...
  DPAA2_BUS_DEBUG("VFIO Container FD is [0x%X]", container_device_fd);
