...
enum qbman_sdqcr_fc { qbman_sdqcr_fc_one = 0, qbman_sdqcr_fc_up_to_3 = 1 };
...
static int qbman_swp_enqueue_multiple_cinh_read_direct(
    struct qbman_swp *s, const struct qbman_eq_desc *d,
    const struct qbman_fd *fd, uint32_t *flags, int num_frames);
static int qbman_swp_enqueue_multiple_cinh_direct(struct qbman_swp *s,
                                                  const struct qbman_eq_desc *d,
                                                  const struct qbman_fd *fd,
                                                  uint32_t *flags,
                                                  int num_frames);
...
static int qbman_swp_enqueue_multiple_fd_cinh_read_direct(
    struct qbman_swp *s, const struct qbman_eq_desc *d, struct qbman_fd **fd,
    uint32_t *flags, int num_frames);
...
static int qbman_swp_enqueue_multiple_desc_cinh_read_direct(
    struct qbman_swp *s, const struct qbman_eq_desc *d,
    const struct qbman_fd *fd, int num_frames);
static int qbman_swp_enqueue_multiple_desc_cinh_direct(
    struct qbman_swp *s, const struct qbman_eq_desc *d,
    const struct qbman_fd *fd, int num_frames);
static int qbman_swp_enqueue_multiple_desc_mem_back(
    struct qbman_swp *s, const struct qbman_eq_desc *d,
    const struct qbman_fd *fd, int num_frames);
...
/* Function pointers */
static int (*qbman_swp_enqueue_array_mode_ptr)(
    struct qbman_swp *s, const struct qbman_eq_desc *d,
    const struct qbman_fd *fd) = qbman_swp_enqueue_array_mode_direct;

static int (*qbman_swp_enqueue_ring_mode_ptr)(
    struct qbman_swp *s, const struct qbman_eq_desc *d,
    const struct qbman_fd *fd) = qbman_swp_enqueue_ring_mode_direct;

static int (*qbman_swp_enqueue_multiple_ptr)(
    struct qbman_swp *s, const struct qbman_eq_desc *d,
    const struct qbman_fd *fd, uint32_t *flags,
    int num_frames) = qbman_swp_enqueue_multiple_direct;

static int (*qbman_swp_enqueue_multiple_fd_ptr)(
    struct qbman_swp *s, const struct qbman_eq_desc *d, struct qbman_fd **fd,
    uint32_t *flags, int num_frames) = qbman_swp_enqueue_multiple_fd_direct;

static int (*qbman_swp_enqueue_multiple_desc_ptr)(
    struct qbman_swp *s, const struct qbman_eq_desc *d,
    const struct qbman_fd *fd,
    int num_frames) = qbman_swp_enqueue_multiple_desc_direct;

static int (*qbman_swp_pull_ptr)(
    struct qbman_swp *s, struct qbman_pull_desc *d) = qbman_swp_pull_direct;

const struct qbman_result *(*qbman_swp_dqrr_next_ptr)(struct qbman_swp *s) =
    qbman_swp_dqrr_next_direct;

static int (*qbman_swp_release_ptr)(
    struct qbman_swp *s, const struct qbman_release_desc *d,
    const uint64_t *buffers,
    unsigned int num_buffers) = qbman_swp_release_direct;
...
struct qbman_swp *qbman_swp_init(const struct qbman_swp_desc *d) {
  ...
  if ((qman_version & QMAN_REV_MASK) >= QMAN_REV_5000 &&
    ...
    qbman_swp_enqueue_multiple_fd_ptr = qbman_swp_enqueue_multiple_fd_mem_back;
  ...
  p->eqcr.ci =
      qbman_cinh_read(&p->sys, QBMAN_CINH_SWP_EQCR_CI) & p->eqcr.pi_ci_mask;
...
void qbman_swp_mc_submit_cinh(struct qbman_swp *p, void *cmd,
                              uint8_t cmd_verb) {
  uint8_t *v = cmd;
#ifdef QBMAN_CHECKING
  QBMAN_BUG_ON(!(p->mc.check != swp_mc_can_submit));
...
void *qbman_swp_mc_result(struct qbman_swp *p) {
  ...
  if ((p->desc.qman_version & QMAN_REV_MASK) >= QMAN_REV_5000 &&
    ...
    qbman_cena_invalidate_prefetch(&p->sys, QBMAN_CENA_SWP_RR(p->mc.valid_bit));
...
void *qbman_swp_mc_result_cinh(struct qbman_swp *p) {
  ...
  QBMAN_BUG_ON(p->mc.check != swp_mc_can_poll);
#endif
  ret = qbman_cinh_read_shadow(&p->sys, QBMAN_CENA_SWP_RR(p->mc.valid_bit));
  /* Remove the valid-bit -
   * command completed iff the rest is non-zero
   */
  verb = ret[0] & ~QB_VALID_BIT;
...
void qbman_eq_desc_clear(struct qbman_eq_desc *d) { memset(d, 0, sizeof(*d)); }
...
static inline void qbman_write_eqcr_am_rt_register(struct qbman_swp *p,
  ...
  if (idx < 16)
    ...
    qbman_cinh_write(&p->sys, QBMAN_CINH_SWP_EQCR_AM_RT2 + (idx - 16) * 4,
                     QMAN_RT_MODE);
}
...
static int qbman_swp_enqueue_ring_mode_direct(struct qbman_swp *s,
  ...
  if (!s->eqcr.available) {
    eqcr_ci = s->eqcr.ci;
    s->eqcr.ci =
        qbman_cena_read_reg(&s->sys, QBMAN_CENA_SWP_EQCR_CI) & full_mask;
    s->eqcr.available = qm_cyc_diff(s->eqcr.pi_ring_size, eqcr_ci, s->eqcr.ci);
    if (!s->eqcr.available)
      return -EBUSY;
  }

  p = qbman_cena_write_start_wo_shadow(
  ...
  qbman_cena_write_complete_wo_shadow(
      &s->sys, QBMAN_CENA_SWP_EQCR(s->eqcr.pi & half_mask));
  s->eqcr.pi++;
  s->eqcr.pi &= full_mask;
  s->eqcr.available--;
...
qbman_swp_enqueue_ring_mode_cinh_read_direct(struct qbman_swp *s,
  ...
  if (!s->eqcr.available) {
    eqcr_ci = s->eqcr.ci;
    s->eqcr.ci = qbman_cinh_read(&s->sys, QBMAN_CINH_SWP_EQCR_CI) & full_mask;
    s->eqcr.available = qm_cyc_diff(s->eqcr.pi_ring_size, eqcr_ci, s->eqcr.ci);
    if (!s->eqcr.available)
      return -EBUSY;
  }

  p = qbman_cinh_write_start_wo_shadow(
      &s->sys, QBMAN_CENA_SWP_EQCR(s->eqcr.pi & half_mask));
  memcpy(&p[1], &cl[1], 28);
  memcpy(&p[8], fd, sizeof(*fd));
  lwsync();
...
static int qbman_swp_enqueue_ring_mode_mem_back(struct qbman_swp *s,
  ...
  if (!s->eqcr.available) {
    eqcr_ci = s->eqcr.ci;
    s->eqcr.ci = qbman_cena_read_reg(&s->sys, QBMAN_CENA_SWP_EQCR_CI_MEMBACK) &
                 full_mask;
    s->eqcr.available = qm_cyc_diff(s->eqcr.pi_ring_size, eqcr_ci, s->eqcr.ci);
    if (!s->eqcr.available)
      return -EBUSY;
...
static int qbman_swp_enqueue_multiple_direct(struct qbman_swp *s,
  ...
  eqcr_pi = s->eqcr.pi;
  num_enqueued =
      (s->eqcr.available < num_frames) ? s->eqcr.available : num_frames;
  s->eqcr.available -= num_enqueued;
  /* Fill in the EQCR ring */
  for (i = 0; i < num_enqueued; i++) {
    p = qbman_cena_write_start_wo_shadow(
        &s->sys, QBMAN_CENA_SWP_EQCR(eqcr_pi & half_mask));
    memcpy(&p[1], &cl[1], 28);
  ...
  for (i = 0; i < num_enqueued; i++) {
    dcbf((uintptr_t)(addr_cena + QBMAN_CENA_SWP_EQCR(eqcr_pi & half_mask)));
    eqcr_pi++;
  }
...
int qbman_swp_enqueue_multiple(struct qbman_swp *s,
  ...
  if (!s->stash_off)
    ...
    return qbman_swp_enqueue_multiple_cinh_direct(s, d, fd, flags, num_frames);
}
...
int qbman_swp_enqueue_multiple_fd(struct qbman_swp *s,
                                  const struct qbman_eq_desc *d,
                                  struct qbman_fd **fd, uint32_t *flags,
                                  int num_frames) {
  if (!s->stash_off)
    return qbman_swp_enqueue_multiple_fd_ptr(s, d, fd, flags, num_frames);
  else
    return qbman_swp_enqueue_multiple_fd_cinh_direct(s, d, fd, flags,
                                                     num_frames);
}
...
int qbman_swp_enqueue_multiple_desc(struct qbman_swp *s,
  ...
  if (!s->stash_off)
    ...
    return qbman_swp_enqueue_multiple_desc_cinh_direct(s, d, fd, num_frames);
}
...
void qbman_swp_push_get(struct qbman_swp *s, uint8_t channel_idx,
                        int *enabled) {
...
const struct qbman_result *qbman_swp_dqrr_next_direct(struct qbman_swp *s) {
  ...
  if (s->dqrr.reset_bug) {
     ...
     */
    uint8_t pi =
        qbman_cinh_read(&s->sys, QBMAN_CINH_SWP_DQPI) & QMAN_DQRR_PI_MASK;
  ...
  if ((response_verb == QBMAN_RESULT_DQ) && (flags & QBMAN_DQ_STAT_VOLATILE) &&
...
qbman_swp_dqrr_next_cinh_direct(struct qbman_swp *s) {
  uint32_t verb;
  uint32_t response_verb;
  uint32_t flags;
  const struct qbman_result *p;
  ...
  p = qbman_cinh_read_wo_shadow(&s->sys, QBMAN_CENA_SWP_DQRR(s->dqrr.next_idx));
...
int qbman_result_has_new_result(struct qbman_swp *s, struct qbman_result *dq) {
  if (dq->dq.tok == 0)
    return 0;
...
static int qbman_swp_release_mem_back(struct qbman_swp *s,
  ...
  p[0] = cl[0] | RAR_VB(rar) | num_buffers;
  lwsync();
  qbman_cinh_write(&s->sys, QBMAN_CINH_SWP_RCR_AM_RT + RAR_IDX(rar) * 4,
                   QMAN_RT_MODE);
...
int qbman_swp_release(struct qbman_swp *s, const struct qbman_release_desc *d,
  ...
  if (!s->stash_off)
    ...
    return qbman_swp_release_cinh_direct(s, d, buffers, num_buffers);
}
...
int qbman_swp_acquire(struct qbman_swp *s, uint16_t bpid, uint64_t *buffers,
  ...
  if (!s->stash_off)
    ...
    return qbman_swp_acquire_cinh_direct(s, bpid, buffers, num_buffers);
}
...
static int qbman_swp_CDAN_set(struct qbman_swp *s, uint16_t channelid,
                              uint8_t we_mask, uint8_t cdan_en, uint64_t ctx) {
...
int qbman_swp_CDAN_set_context(struct qbman_swp *s, uint16_t channelid,
                               uint64_t ctx) {
  return qbman_swp_CDAN_set(s, channelid, CODE_CDAN_WE_CTX, 0, ctx);
}

int qbman_swp_CDAN_enable(struct qbman_swp *s, uint16_t channelid) {
  return qbman_swp_CDAN_set(s, channelid, CODE_CDAN_WE_EN, 1, 0);
}

int qbman_swp_CDAN_disable(struct qbman_swp *s, uint16_t channelid) {
  return qbman_swp_CDAN_set(s, channelid, CODE_CDAN_WE_EN, 0, 0);
}

int qbman_swp_CDAN_set_context_enable(struct qbman_swp *s, uint16_t channelid,
                                      uint64_t ctx) {
  return qbman_swp_CDAN_set(s, channelid, CODE_CDAN_WE_EN | CODE_CDAN_WE_CTX, 1,
                            ctx);
}
