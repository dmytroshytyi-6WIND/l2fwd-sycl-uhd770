...
int qbman_bp_get_bdi(struct qbman_bp_query_rslt *r) { return r->bdi & 1; }
...
uint16_t qbman_bp_get_vbpid(struct qbman_bp_query_rslt *r) { return r->vbpid; }
...
uint32_t qbman_bp_get_hdptr(struct qbman_bp_query_rslt *r) { return r->hdptr; }

uint32_t qbman_bp_get_sdcnt(struct qbman_bp_query_rslt *r) { return r->sdcnt; }

uint32_t qbman_bp_get_hdcnt(struct qbman_bp_query_rslt *r) { return r->hdcnt; }

uint32_t qbman_bp_get_sscnt(struct qbman_bp_query_rslt *r) { return r->sscnt; }
...
static int qbman_cgr_statistics_query(struct qbman_swp *s, uint32_t cgid,
  ...
  if (command_type < 2)
    p->ct = command_type;
  query_verb = clear ? QBMAN_CGR_STAT_QUERY_CLR : QBMAN_CGR_STAT_QUERY;
  r = (struct qbman_cgr_statistics_query_rslt *)qbman_swp_mc_complete(
      s, p, query_verb);
  if (!r) {
...
int qbman_cgr_reject_statistics(struct qbman_swp *s, uint32_t cgid, int clear,
                                uint64_t *frame_cnt, uint64_t *byte_cnt) {
  return qbman_cgr_statistics_query(s, cgid, clear, 0xff, frame_cnt, byte_cnt);
}

int qbman_ccgr_reject_statistics(struct qbman_swp *s, uint32_t cgid, int clear,
                                 uint64_t *frame_cnt, uint64_t *byte_cnt) {
  return qbman_cgr_statistics_query(s, cgid, clear, 1, frame_cnt, byte_cnt);
}

int qbman_cq_dequeue_statistics(struct qbman_swp *s, uint32_t cgid, int clear,
                                uint64_t *frame_cnt, uint64_t *byte_cnt) {
  return qbman_cgr_statistics_query(s, cgid, clear, 0, frame_cnt, byte_cnt);
}
...
uint32_t qbman_wqchan_attr_get_wqlen(struct qbman_wqchan_query_rslt *r,
                                     int wq) {
