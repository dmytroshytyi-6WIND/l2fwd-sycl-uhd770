...
static inline void qbman_cinh_write_complete(struct qbman_swp_sys *s,
  ...
  for (loop = 15; loop >= 1; loop--)
    __raw_writel(shadow[loop], s->addr_cinh + offset + loop * 4);
  lwsync();
  __raw_writel(shadow[0], s->addr_cinh + offset);
}

static inline uint32_t qbman_cinh_read(struct qbman_swp_sys *s,
                                       uint32_t offset) {
...
static inline void *qbman_cinh_read_shadow(struct qbman_swp_sys *s,
  ...
  unsigned int loop;
#ifdef QBMAN_CINH_TRACE
  pr_info(" %s (%p:%d:0x%03x) %p\n", __func__, s->addr_cinh, s->idx, offset,
          shadow);
#endif

  for (loop = 0; loop < 16; loop++)
    shadow[loop] = __raw_readl(s->addr_cinh + offset + loop * 4);
#ifdef QBMAN_CINH_TRACE
  hexdump(shadow, 64);
#endif
  return shadow;
...
#ifdef RTE_ARCH_64
  for (loop = 15; loop >= 1; loop--)
    __raw_writel(shadow[loop], s->addr_cena + offset + loop * 4);
  lwsync();
...
#ifdef RTE_ARCH_64
  for (loop = 0; loop < 16; loop++)
    shadow[loop] = __raw_readl(s->addr_cena + offset + loop * 4);
#else
  for (loop = 0; loop < 16; loop++)
    shadow[loop] = __raw_readl(s->addr_cinh + offset + loop * 4);
#endif
...
static inline uint32_t qbman_set_swp_cfg(uint8_t max_fill, uint8_t wn,
  ...
  reg = (max_fill << SWP_CFG_DQRR_MF_SHIFT | est << SWP_CFG_EST_SHIFT |
         wn << SWP_CFG_WN_SHIFT | rpm << SWP_CFG_RPM_SHIFT |
         dcm << SWP_CFG_DCM_SHIFT | epm << SWP_CFG_EPM_SHIFT |
         sd << SWP_CFG_SD_SHIFT | sp << SWP_CFG_SP_SHIFT |
         se << SWP_CFG_SE_SHIFT | dp << SWP_CFG_DP_SHIFT |
         de << SWP_CFG_DE_SHIFT | ep << SWP_CFG_EP_SHIFT);
...
static inline int qbman_swp_sys_init(struct qbman_swp_sys *s,
  ...
  if (s->eqcr_mode == qman_eqcr_vb_array) {
    reg = qbman_set_swp_cfg(dqrr_size, wn, 0, 3, 2, 3, 1, 1, 1, 1, 1, 1);
  } else {
    if ((d->qman_version & QMAN_REV_MASK) >= QMAN_REV_5000 &&
        (d->cena_access_mode == qman_cena_fastest_access))
      reg = qbman_set_swp_cfg(dqrr_size, wn, 1, 3, 2, 0, 1, 1, 1, 1, 1, 1);
    else
      reg = qbman_set_swp_cfg(dqrr_size, wn, est, 3, 2, 2, 1, 1, 1, 1, 1, 1);
  }
...
static inline int qbman_swp_sys_update(struct qbman_swp_sys *s,
                                       const struct qbman_swp_desc *d,
                                       uint8_t dqrr_size, int stash_off) {
  uint32_t reg;
  int i;
  int cena_region_size = 4 * 1024;
  uint8_t est = 1;
