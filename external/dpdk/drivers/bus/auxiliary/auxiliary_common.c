...
static struct rte_devargs *auxiliary_devargs_lookup(const char *name) {
 ...
 */
__rte_weak bool auxiliary_dev_exists(const char *name) {
  RTE_SET_USED(name);
  return false;
}
 ...
 * Stub for OS not supporting auxiliary bus.
 */
__rte_weak int auxiliary_scan(void) { return 0; }
 ...
 */
static int rte_auxiliary_probe_one_driver(struct rte_auxiliary_driver *drv,
  ...
  if ((drv->drv_flags & RTE_AUXILIARY_DRV_NEED_IOVA_AS_VA) > 0 &&
    ...
    AUXILIARY_LOG(ERR,
                  "Driver %s expecting VA IOVA mode but current mode is PA, "
                  "not initializing",
                  drv->driver.name);
    return -EINVAL;
  }
 ...
 */
static int rte_auxiliary_driver_remove_dev(struct rte_auxiliary_device *dev) {
 ...
 */
static int auxiliary_probe_all_drivers(struct rte_auxiliary_device *dev) {
  struct rte_auxiliary_driver *drv;
  int rc;
 ...
 */
static int auxiliary_probe(void) {
  ...
  FOREACH_DEVICE_ON_AUXILIARY_BUS(dev) {
    ...
    if (ret < 0) {
      if (ret != -EEXIST) {
        AUXILIARY_LOG(ERR, "Requested device %s cannot be used", dev->name);
        rte_errno = errno;
        failed++;
      }
      ret = 0;
...
static int auxiliary_parse(const char *name, void *addr) {
...
static struct rte_device *auxiliary_find_device(const struct rte_device *start,
                                                rte_dev_cmp_t cmp,
                                                const void *data) {
...
static int auxiliary_plug(struct rte_device *dev) {
  if (!auxiliary_dev_exists(dev->name))
    return -ENOENT;
  return auxiliary_probe_all_drivers(RTE_DEV_TO_AUXILIARY(dev));
}

static int auxiliary_unplug(struct rte_device *dev) {
  struct rte_auxiliary_device *adev;
  int ret;
...
static int auxiliary_dma_map(struct rte_device *dev, void *addr, uint64_t iova,
                             size_t len) {
...
static int auxiliary_dma_unmap(struct rte_device *dev, void *addr,
...
static enum rte_iova_mode auxiliary_get_iommu_class(void) {
