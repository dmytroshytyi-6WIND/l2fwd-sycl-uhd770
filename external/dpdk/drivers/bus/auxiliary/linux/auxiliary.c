...
static int auxiliary_scan_one(const char *dirname, const char *name) {
  ...
  snprintf(filename, sizeof(filename), "%s/%s/numa_node", dirname, name);
...
bool auxiliary_dev_exists(const char *name) {
  ...
  snprintf(dirname, sizeof(dirname), "%s/%s", AUXILIARY_SYSFS_PATH, name);
  dir = opendir(dirname);
  if (dir == NULL)
    return false;
  closedir(dir);
...
int auxiliary_scan(void) {
  ...
  while ((e = readdir(dir)) != NULL) {
    ...
    snprintf(dirname, sizeof(dirname), "%s/%s", AUXILIARY_SYSFS_PATH,
             e->d_name);
