...
static int skeldma_info_get(const struct rte_dma_dev *dev,
...
#define SKELDMA_MIN_DESC 32
  ...
  dev_info->dev_capa =
      RTE_DMA_CAPA_MEM_TO_MEM | RTE_DMA_CAPA_SVA | RTE_DMA_CAPA_OPS_COPY;
...
static int skeldma_configure(struct rte_dma_dev *dev,
...
static void *cpucopy_thread(void *param) {
...
static void fflush_ring(struct skeldma_hw *hw, struct rte_ring *ring) {
  struct skeldma_desc *desc = NULL;
...
static int skeldma_start(struct rte_dma_dev *dev) {
  struct skeldma_hw *hw = dev->data->dev_private;
  rte_cpuset_t cpuset;
  int ret;
  ...
  if (hw->lcore_id != -1) {
    ...
    ret = pthread_setaffinity_np(hw->thread, sizeof(cpuset), &cpuset);
    if (ret)
      SKELDMA_LOG(WARNING, "Set thread affinity lcore = %d fail!",
                  hw->lcore_id);
  }
...
static int skeldma_stop(struct rte_dma_dev *dev) {
  struct skeldma_hw *hw = dev->data->dev_private;
...
static int vchan_setup(struct skeldma_hw *hw, uint16_t nb_desc) {
  ...
  empty = rte_ring_create("dma_skeleton_desc_empty", nb_desc, hw->socket_id,
                          RING_F_SP_ENQ | RING_F_SC_DEQ);
  pending = rte_ring_create("dma_skeleton_desc_pending", nb_desc, hw->socket_id,
                            RING_F_SP_ENQ | RING_F_SC_DEQ);
  running = rte_ring_create("dma_skeleton_desc_running", nb_desc, hw->socket_id,
                            RING_F_SP_ENQ | RING_F_SC_DEQ);
...
static void vchan_release(struct skeldma_hw *hw) {
  if (hw->desc_mem == NULL)
    return;
...
static int skeldma_close(struct rte_dma_dev *dev) {
  /* The device already stopped */
  vchan_release(dev->data->dev_private);
  return 0;
}

static int skeldma_vchan_setup(struct rte_dma_dev *dev, uint16_t vchan,
...
static int skeldma_vchan_status(const struct rte_dma_dev *dev, uint16_t vchan,
  ...
  *status = RTE_DMA_VCHAN_IDLE;
  if (hw->submitted_count !=
          __atomic_load_n(&hw->completed_count, __ATOMIC_ACQUIRE) ||
      hw->zero_req_count == 0)
    *status = RTE_DMA_VCHAN_ACTIVE;
  return 0;
}

static int skeldma_stats_get(const struct rte_dma_dev *dev, uint16_t vchan,
...
static int skeldma_stats_reset(struct rte_dma_dev *dev, uint16_t vchan) {
  struct skeldma_hw *hw = dev->data->dev_private;
...
static int skeldma_dump(const struct rte_dma_dev *dev, FILE *f) {
#define GET_RING_COUNT(ring) ((ring) ? (rte_ring_count(ring)) : 0)
  ...
  (void)fprintf(f,
                ...
                hw->lcore_id, hw->socket_id, GET_RING_COUNT(hw->desc_empty),
...
static inline void submit(struct skeldma_hw *hw, struct skeldma_desc *desc) {
...
static int skeldma_copy(void *dev_private, uint16_t vchan, rte_iova_t src,
...
static int skeldma_submit(void *dev_private, uint16_t vchan) {
  struct skeldma_hw *hw = dev_private;
  RTE_SET_USED(vchan);
  submit(hw, NULL);
  return 0;
...
static uint16_t skeldma_completed(void *dev_private, uint16_t vchan,
...
static uint16_t skeldma_completed_status(void *dev_private, uint16_t vchan,
...
static uint16_t skeldma_burst_capacity(const void *dev_private,
                                       uint16_t vchan) {
...
static int skeldma_create(const char *name, struct rte_vdev_device *vdev,
                          int lcore_id) {
  ...
  socket_id =
      (lcore_id < 0) ? rte_socket_id() : rte_lcore_to_socket_id(lcore_id);
...
static int skeldma_destroy(const char *name) {
  return rte_dma_pmd_release(name);
}

static int skeldma_parse_lcore(const char *key __rte_unused, const char *value,
                               void *opaque) {
...
static void skeldma_parse_vdev_args(struct rte_vdev_device *vdev,
                                    int *lcore_id) {
  static const char *const args[] = {SKELDMA_ARG_LCORE, NULL};
  ...
  (void)rte_kvargs_process(kvlist, SKELDMA_ARG_LCORE, skeldma_parse_lcore,
                           lcore_id);
...
static int skeldma_probe(struct rte_vdev_device *vdev) {
  const char *name;
  int lcore_id = -1;
  int ret;
  ...
  if (skeldma_count > 0) {
    SKELDMA_LOG(ERR, "Multiple instance not supported for %s", name);
    return -EINVAL;
  }
  ...
  if (ret >= 0) {
    SKELDMA_LOG(INFO, "Create %s dmadev with lcore-id %d", name, lcore_id);
    skeldma_count = 1;
  }
...
static int skeldma_remove(struct rte_vdev_device *vdev) {
  const char *name;
  int ret;
...
RTE_PMD_REGISTER_PARAM_STRING(dma_skeleton, SKELDMA_ARG_LCORE "=<uint16> ");
