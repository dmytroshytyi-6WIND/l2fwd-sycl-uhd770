...
static int cnxk_dmadev_info_get(const struct rte_dma_dev *dev,
...
static int cnxk_dmadev_configure(struct rte_dma_dev *dev,
...
static int cnxk_dmadev_vchan_setup(struct rte_dma_dev *dev, uint16_t vchan,
...
static int cnxk_dmadev_start(struct rte_dma_dev *dev) {
  struct cnxk_dpi_vf_s *dpivf = dev->fp_obj->dev_private;
...
static inline int __dpi_queue_write(struct roc_dpi *dpi, uint64_t *cmds,
                                    int cmd_count) {
...
static int cnxk_dmadev_copy(void *dev_private, uint16_t vchan, rte_iova_t src,
...
static int cnxk_dmadev_copy_sg(void *dev_private, uint16_t vchan,
...
static uint16_t cnxk_dmadev_completed(void *dev_private, uint16_t vchan,
  ...
  for (cnt = 0; cnt < nb_cpls; cnt++) {
    struct cnxk_dpi_compl_s *comp_ptr = dpivf->conf.c_desc.compl_ptr[cnt];
...
static uint16_t cnxk_dmadev_completed_status(void *dev_private, uint16_t vchan,
                                             const uint16_t nb_cpls,
                                             uint16_t *last_idx,
...
static int cnxk_dmadev_submit(void *dev_private, uint16_t vchan __rte_unused) {
  struct cnxk_dpi_vf_s *dpivf = dev_private;
...
static int cnxk_stats_get(const struct rte_dma_dev *dev, uint16_t vchan,
...
static int cnxk_stats_reset(struct rte_dma_dev *dev,
...
static int cnxk_dmadev_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  dmadev =
      rte_dma_pmd_allocate(name, pci_dev->device.numa_node, sizeof(*dpivf));
  if (dmadev == NULL) {
...
static int cnxk_dmadev_remove(struct rte_pci_device *pci_dev) {
  char name[RTE_DEV_NAME_MAX_LEN];
