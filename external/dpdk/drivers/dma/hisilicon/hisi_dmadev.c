...
#define HISI_DMA_LOG(level, fmt, args...)                                      \
  rte_log(RTE_LOG_##level, hisi_dma_logtype, "%s(): " fmt "\n", __func__,      \
          ##args)
#define HISI_DMA_LOG_RAW(hw, level, fmt, args...)                              \
  rte_log(RTE_LOG_##level, hisi_dma_logtype, "%s %s(): " fmt "\n",             \
          (hw)->data->dev_name, __func__, ##args)
#define HISI_DMA_DEBUG(hw, fmt, args...)                                       \
  HISI_DMA_LOG_RAW(hw, DEBUG, fmt, ##args)
#define HISI_DMA_INFO(hw, fmt, args...) HISI_DMA_LOG_RAW(hw, INFO, fmt, ##args)
#define HISI_DMA_WARN(hw, fmt, args...)                                        \
  HISI_DMA_LOG_RAW(hw, WARNING, fmt, ##args)
#define HISI_DMA_ERR(hw, fmt, args...) HISI_DMA_LOG_RAW(hw, ERR, fmt, ##args)

static uint32_t hisi_dma_queue_base(struct hisi_dma_dev *hw) {
...
static volatile void *hisi_dma_queue_regaddr(struct hisi_dma_dev *hw,
                                             uint32_t qoff) {
...
static void hisi_dma_write_reg(void *base, uint32_t off, uint32_t val) {
  rte_write32(rte_cpu_to_le_32(val), (volatile void *)((char *)base + off));
}

static void hisi_dma_write_dev(struct hisi_dma_dev *hw, uint32_t off,
                               uint32_t val) {
  hisi_dma_write_reg(hw->io_base, off, val);
}

static void hisi_dma_write_queue(struct hisi_dma_dev *hw, uint32_t qoff,
                                 uint32_t val) {
  uint32_t off = hisi_dma_queue_base(hw) +
                 hw->queue_id * HISI_DMA_QUEUE_REGION_SIZE + qoff;
...
static uint32_t hisi_dma_read_reg(void *base, uint32_t off) {
  uint32_t val = rte_read32((volatile void *)((char *)base + off));
  return rte_le_to_cpu_32(val);
}

static uint32_t hisi_dma_read_dev(struct hisi_dma_dev *hw, uint32_t off) {
  return hisi_dma_read_reg(hw->io_base, off);
}

static uint32_t hisi_dma_read_queue(struct hisi_dma_dev *hw, uint32_t qoff) {
  uint32_t off = hisi_dma_queue_base(hw) +
                 hw->queue_id * HISI_DMA_QUEUE_REGION_SIZE + qoff;
  return hisi_dma_read_dev(hw, off);
}

static void hisi_dma_update_bit(struct hisi_dma_dev *hw, uint32_t off,
...
static void hisi_dma_update_queue_bit(struct hisi_dma_dev *hw, uint32_t qoff,
                                      uint32_t pos, bool set) {
...
static void hisi_dma_update_queue_mbit(struct hisi_dma_dev *hw, uint32_t qoff,
...
static int hisi_dma_reset_hw(struct hisi_dma_dev *hw) {
...
#define POLL_TIMEOUT_US 10000
  ...
  ret = hisi_dma_poll_hw_state(
      hw, tmp, FIELD_GET(HISI_DMA_QUEUE_FSM_STS_M, tmp) != HISI_DMA_STATE_RUN,
  ...
  ret = hisi_dma_poll_hw_state(
      hw, tmp, FIELD_GET(HISI_DMA_QUEUE_FSM_STS_M, tmp) == HISI_DMA_STATE_IDLE,
      POLL_SLEEP_US, POLL_TIMEOUT_US);
  if (ret) {
...
static void hisi_dma_init_common(struct hisi_dma_dev *hw) {
  ...
  hisi_dma_write_queue(hw, HISI_DMA_QUEUE_SQ_DEPTH_REG, hw->sq_depth_mask);
...
static void hisi_dma_init_hw(struct hisi_dma_dev *hw) {
  hisi_dma_init_common(hw);

  if (hw->reg_layout == HISI_DMA_REG_LAYOUT_HIP08) {
    hisi_dma_write_queue(hw, HISI_DMA_HIP08_QUEUE_ERR_INT_NUM3_REG, 0);
    hisi_dma_write_queue(hw, HISI_DMA_HIP08_QUEUE_ERR_INT_NUM4_REG, 0);
    hisi_dma_write_queue(hw, HISI_DMA_HIP08_QUEUE_ERR_INT_NUM5_REG, 0);
    hisi_dma_write_queue(hw, HISI_DMA_HIP08_QUEUE_ERR_INT_NUM6_REG, 0);
    hisi_dma_update_queue_bit(hw, HISI_DMA_QUEUE_CTRL0_REG,
    ...
    hisi_dma_update_queue_mbit(hw, HISI_DMA_QUEUE_INT_MASK_REG,
                               HISI_DMA_HIP09_QUEUE_INT_MASK_M, true);
    hisi_dma_update_queue_mbit(hw, HISI_DMA_HIP09_QUEUE_ERR_INT_STATUS_REG,
                               HISI_DMA_HIP09_QUEUE_ERR_INT_MASK_M, true);
    hisi_dma_update_queue_mbit(hw, HISI_DMA_HIP09_QUEUE_ERR_INT_MASK_REG,
                               HISI_DMA_HIP09_QUEUE_ERR_INT_MASK_M, true);
    hisi_dma_update_queue_bit(hw, HISI_DMA_QUEUE_CTRL1_REG,
                              HISI_DMA_HIP09_QUEUE_CTRL1_VA_ENABLE_B, true);
    hisi_dma_update_bit(hw, HISI_DMA_HIP09_QUEUE_CFG_REG(hw->queue_id),
                        HISI_DMA_HIP09_QUEUE_CFG_LINK_DOWN_MASK_B, true);
  }
}

static void hisi_dma_init_gbl(void *pci_bar, uint8_t revision) {
  ...
  if (revision == HISI_DMA_REVISION_HIP08B)
    hisi_dma_update_bit(&hw, HISI_DMA_HIP08_MODE_REG, HISI_DMA_HIP08_MODE_SEL_B,
                        true);
}

static uint8_t hisi_dma_reg_layout(uint8_t revision) {
  if (revision == HISI_DMA_REVISION_HIP08B)
...
static void hisi_dma_zero_iomem(struct hisi_dma_dev *hw) {
  memset(hw->iomz->addr, 0, hw->iomz_sz);
}

static int hisi_dma_alloc_iomem(struct hisi_dma_dev *hw, uint16_t ring_size,
                                const char *dev_name) {
  uint32_t sq_size = sizeof(struct hisi_dma_sqe) * ring_size;
  uint32_t cq_size =
      sizeof(struct hisi_dma_cqe) * (ring_size + HISI_DMA_CQ_RESERVED);
...
static void hisi_dma_free_iomem(struct hisi_dma_dev *hw) {
  if (hw->iomz != NULL)
...
static int hisi_dma_info_get(const struct rte_dma_dev *dev,
                             struct rte_dma_info *dev_info, uint32_t info_sz) {
  struct hisi_dma_dev *hw = dev->data->dev_private;
  RTE_SET_USED(info_sz);

  dev_info->dev_capa = RTE_DMA_CAPA_MEM_TO_MEM | RTE_DMA_CAPA_OPS_COPY;
...
static int hisi_dma_configure(struct rte_dma_dev *dev,
...
static int hisi_dma_vchan_setup(struct rte_dma_dev *dev, uint16_t vchan,
...
static int hisi_dma_start(struct rte_dma_dev *dev) {
  struct hisi_dma_dev *hw = dev->data->dev_private;
...
static int hisi_dma_stop(struct rte_dma_dev *dev) {
  return hisi_dma_reset_hw(dev->data->dev_private);
}

static int hisi_dma_close(struct rte_dma_dev *dev) {
...
static int hisi_dma_stats_get(const struct rte_dma_dev *dev, uint16_t vchan,
...
static int hisi_dma_stats_reset(struct rte_dma_dev *dev, uint16_t vchan) {
  struct hisi_dma_dev *hw = dev->data->dev_private;
...
static void hisi_dma_dump_range(struct hisi_dma_dev *hw, FILE *f,
...
static void hisi_dma_dump_common(struct hisi_dma_dev *hw, FILE *f) {
  struct {
    uint8_t reg_layout;
    uint32_t start;
    uint32_t end;
  } reg_info[] = {
      {HISI_DMA_REG_LAYOUT_HIP08, HISI_DMA_HIP08_DUMP_START_REG,
       HISI_DMA_HIP08_DUMP_END_REG},
      {HISI_DMA_REG_LAYOUT_HIP09, HISI_DMA_HIP09_DUMP_REGION_A_START_REG,
       HISI_DMA_HIP09_DUMP_REGION_A_END_REG},
      {HISI_DMA_REG_LAYOUT_HIP09, HISI_DMA_HIP09_DUMP_REGION_B_START_REG,
       HISI_DMA_HIP09_DUMP_REGION_B_END_REG},
      {HISI_DMA_REG_LAYOUT_HIP09, HISI_DMA_HIP09_DUMP_REGION_C_START_REG,
       HISI_DMA_HIP09_DUMP_REGION_C_END_REG},
      {HISI_DMA_REG_LAYOUT_HIP09, HISI_DMA_HIP09_DUMP_REGION_D_START_REG,
...
static void hisi_dma_dump_read_queue(struct hisi_dma_dev *hw, uint32_t qoff,
...
static void hisi_dma_dump_queue(struct hisi_dma_dev *hw, FILE *f) {
...
static int hisi_dma_dump(const struct rte_dma_dev *dev, FILE *f) {
  struct hisi_dma_dev *hw = dev->data->dev_private;
  ...
  (void)fprintf(f,
                ...
                " errors: %" PRIu64 " qfulls: %" PRIu64 "\n",
                hw->revision, hw->queue_id,
                hw->sq_depth_mask > 0 ? hw->sq_depth_mask + 1 : 0, hw->ridx,
                hw->cridx, hw->sq_head, hw->sq_tail, hw->cq_sq_head,
                hw->cq_head, hw->cqs_completed, hw->cqe_vld, hw->submitted,
                hw->completed, hw->errors, hw->qfulls);
...
static int hisi_dma_copy(void *dev_private, uint16_t vchan, rte_iova_t src,
...
static int hisi_dma_submit(void *dev_private, uint16_t vchan) {
  struct hisi_dma_dev *hw = dev_private;
...
static inline void hisi_dma_scan_cq(struct hisi_dma_dev *hw) {
  ...
  while (true) {
    ...
    if (unlikely(misc & CQE_STATUS_MASK))
      hw->status[csq_head] = FIELD_GET(CQE_STATUS_MASK, misc);
...
static inline uint16_t hisi_dma_calc_cpls(struct hisi_dma_dev *hw,
...
static uint16_t hisi_dma_completed(void *dev_private, uint16_t vchan,
...
static enum rte_dma_status_code hisi_dma_convert_status(uint16_t status) {
  switch (status) {
...
static uint16_t hisi_dma_completed_status(void *dev_private, uint16_t vchan,
                                          const uint16_t nb_cpls,
                                          uint16_t *last_idx,
...
static uint16_t hisi_dma_burst_capacity(const void *dev_private,
                                        uint16_t vchan) {
...
static void hisi_dma_gen_dev_name(const struct rte_pci_device *pci_dev,
                                  uint8_t queue_id, char *dev_name,
                                  size_t size) {
...
static int hisi_dma_create(struct rte_pci_device *pci_dev, uint8_t queue_id,
                           uint8_t revision) {
#define REG_PCI_BAR_INDEX 2
  ...
  dev = rte_dma_pmd_allocate(name, pci_dev->device.numa_node, sizeof(*hw));
  if (dev == NULL) {
...
static int hisi_dma_check_revision(struct rte_pci_device *pci_dev,
                                   const char *name, uint8_t *out_revision) {
  uint8_t revision;
  int ret;

  ret =
      rte_pci_read_config(pci_dev, &revision, 1, HISI_DMA_PCI_REVISION_ID_REG);
  if (ret != 1) {
    HISI_DMA_LOG(ERR, "%s read PCI revision failed!", name);
    return -EINVAL;
  }
  if (hisi_dma_reg_layout(revision) == HISI_DMA_REG_LAYOUT_INVALID) {
    HISI_DMA_LOG(ERR, "%s revision: 0x%x not supported!", name, revision);
    return -EINVAL;
  }
...
static int hisi_dma_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  for (i = 0; i < HISI_DMA_MAX_HW_QUEUES; i++) {
    ...
    if (ret) {
      HISI_DMA_LOG(ERR, "%s create dmadev %u failed!", name, i);
      break;
    }
  }
...
static int hisi_dma_remove(struct rte_pci_device *pci_dev) {
  char name[RTE_DEV_NAME_MAX_LEN];
  uint8_t i;
  int ret;
