...
/* Configure a device. */
static int ioat_dev_configure(struct rte_dma_dev *dev __rte_unused,
...
static int ioat_vchan_setup(struct rte_dma_dev *dev,
  ...
  if (!rte_is_power_of_2(max_desc)) {
    ...
    IOAT_PMD_DEBUG("DMA dev %u using %u descriptors", dev->data->dev_id,
                   max_desc);
  ...
  ioat->status_addr =
      rte_mem_virt2iova(ioat) + offsetof(struct ioat_dmadev, status);
  ...
  for (i = 0; i < ioat->qcfg.nb_desc; i++) {
    ioat->desc_ring[i].next =
        ioat->ring_addr + (((i + 1) % ioat->qcfg.nb_desc) * DESC_SZ);
  }
...
/* Recover IOAT device. */
static inline int __ioat_recover(struct ioat_dmadev *ioat) {
  ...
  ioat->regs->chanctrl =
      IOAT_CHANCTRL_ANY_ERR_ABORT_EN | IOAT_CHANCTRL_ERR_COMPLETION_EN;
...
static int ioat_dev_start(struct rte_dma_dev *dev) {
  struct ioat_dmadev *ioat = dev->fp_obj->dev_private;
...
static int ioat_dev_stop(struct rte_dma_dev *dev) {
  struct ioat_dmadev *ioat = dev->fp_obj->dev_private;
  uint32_t retry = 0;
...
static int ioat_dev_info_get(const struct rte_dma_dev *dev,
                             struct rte_dma_info *info, uint32_t size) {
  struct ioat_dmadev *ioat = dev->fp_obj->dev_private;
  if (size < sizeof(*info))
    return -EINVAL;
  info->dev_capa =
      RTE_DMA_CAPA_MEM_TO_MEM | RTE_DMA_CAPA_OPS_COPY | RTE_DMA_CAPA_OPS_FILL;
...
static int ioat_dev_close(struct rte_dma_dev *dev) {
...
static inline void __submit(struct ioat_dmadev *ioat) {
...
static int ioat_submit(void *dev_private, uint16_t qid __rte_unused) {
...
static inline int __write_desc(void *dev_private, uint32_t op, uint64_t src,
                               phys_addr_t dst, unsigned int length,
                               uint64_t flags) {
  struct ioat_dmadev *ioat = dev_private;
  uint16_t ret;
  ...
  desc->size = length;
  desc->u.control_raw =
      (uint32_t)((op << IOAT_CMD_OP_SHIFT) | (1 << IOAT_COMP_UPDATE_SHIFT));
...
static int ioat_enqueue_fill(void *dev_private, uint16_t qid __rte_unused,
...
static int ioat_enqueue_copy(void *dev_private, uint16_t qid __rte_unused,
...
static int __dev_dump(void *dev_private, FILE *f) {
  struct ioat_dmadev *ioat = dev_private;
  ...
  struct ioat_dma_hw_desc *desc_ring =
      &ioat->desc_ring[(ioat->next_write - 1) & mask];
  ...
  fprintf(f, "  }\n");
  fprintf(f,
          "  Key Stats { submitted: %" PRIu64 ", comp: %" PRIu64
          ", failed: %" PRIu64 " }\n",
...
static int ioat_dev_dump(const struct rte_dma_dev *dev, FILE *f) {
  return __dev_dump(dev->fp_obj->dev_private, f);
}

/* Returns the index of the last completed operation. */
static inline uint16_t __get_last_completed(const struct ioat_dmadev *ioat,
                                            int *state) {
...
static uint16_t ioat_completed(void *dev_private, uint16_t qid __rte_unused,
                               const uint16_t max_ops, uint16_t *last_idx,
                               bool *has_error) {
  struct ioat_dmadev *ioat = dev_private;
 ...
 * completed. */
static uint16_t ioat_completed_status(void *dev_private,
...
static uint16_t ioat_burst_capacity(const void *dev_private,
                                    uint16_t vchan __rte_unused) {
...
static int ioat_stats_get(const struct rte_dma_dev *dev,
                          uint16_t vchan __rte_unused,
                          struct rte_dma_stats *rte_stats, uint32_t size) {
  struct rte_dma_stats *stats =
      (&((struct ioat_dmadev *)dev->fp_obj->dev_private)->stats);
...
static int ioat_stats_reset(struct rte_dma_dev *dev,
                            uint16_t vchan __rte_unused) {
  struct ioat_dmadev *ioat = dev->fp_obj->dev_private;
...
static int ioat_vchan_status(const struct rte_dma_dev *dev,
                             uint16_t vchan __rte_unused,
...
/* Create a DMA device. */
static int ioat_dmadev_create(const char *name, struct rte_pci_device *dev) {
  ...
  while (ioat->regs->chancmd & IOAT_CHANCMD_RESET) {
    ...
    if (++retry >= 200) {
      IOAT_PMD_ERR("%s: cannot reset device. CHANCMD=%#" PRIx8
                   ...
                   __func__, ioat->regs->chancmd, ioat->regs->chansts,
                   ioat->regs->chanerr);
...
static int ioat_dmadev_destroy(const char *name) {
  int ret;
...
/* Probe DMA device. */
static int ioat_dmadev_probe(struct rte_pci_driver *drv,
...
/* Remove DMA device. */
static int ioat_dmadev_remove(struct rte_pci_device *dev) {
  char name[32];
...
RTE_PMD_REGISTER_KMOD_DEP(IOAT_PMD_NAME,
