...
static inline int idxd_pci_dev_command(struct idxd_dmadev *idxd,
                                       enum rte_idxd_cmds command) {
  uint32_t err_code;
...
static uint32_t *idxd_get_wq_cfg(struct idxd_pci_common *pci, uint8_t wq_idx) {
  return RTE_PTR_ADD(pci->wq_regs_base,
                     (uintptr_t)wq_idx << (5 + pci->wq_cfg_sz));
}

static int idxd_is_wq_enabled(struct idxd_dmadev *idxd) {
  uint32_t state = idxd_get_wq_cfg(idxd->u.pci, idxd->qid)[wq_state_idx];
  return ((state >> WQ_STATE_SHIFT) & WQ_STATE_MASK) == 0x1;
}

static int idxd_pci_dev_stop(struct rte_dma_dev *dev) {
  ...
  if (err_code || idxd_is_wq_enabled(idxd)) {
    IDXD_PMD_ERR("Failed disabling work queue %d, error code: %#x", idxd->qid,
                 err_code);
...
static int idxd_pci_dev_start(struct rte_dma_dev *dev) {
  struct idxd_dmadev *idxd = dev->fp_obj->dev_private;
  uint8_t err_code;
  ...
  if (err_code || !idxd_is_wq_enabled(idxd)) {
    IDXD_PMD_ERR("Failed enabling work queue %d, error code: %#x", idxd->qid,
                 err_code);
    return err_code == 0 ? -1 : -err_code;
  }
...
static int idxd_pci_dev_close(struct rte_dma_dev *dev) {
  struct idxd_dmadev *idxd = dev->fp_obj->dev_private;
  uint8_t err_code;
...
static int init_pci_device(struct rte_pci_device *dev, struct idxd_dmadev *idxd,
  ...
  IDXD_PMD_DEBUG("nb_groups = %u, nb_engines = %u, nb_wqs = %u", nb_groups,
                 nb_engines, nb_wqs);
  ...
  for (i = 0; i < nb_engines; i++) {
    IDXD_PMD_DEBUG("Assigning engine %u to group %u", i, i % nb_groups);
  ...
  for (i = 0; i < nb_wqs; i++) {
    ...
    IDXD_PMD_DEBUG("Assigning work queue %u to group %u", i, i % nb_groups);
    ...
    idxd_get_wq_cfg(pci, i)[wq_mode_idx] =
        (1 << WQ_PRIORITY_SHIFT) | WQ_MODE_DEDICATED;
    idxd_get_wq_cfg(pci, i)[wq_sizes_idx] =
...
static int idxd_dmadev_probe_pci(struct rte_pci_driver *drv,
  ...
  if (dev->device.devargs && dev->device.devargs->args[0] != '\0') {
    ...
    if (sscanf(dev->device.devargs->args, "max_queues=%u", &max_queues) != 1) {
      IDXD_PMD_ERR("Invalid device parameter: '%s'", dev->device.devargs->args);
      return -1;
    }
  }
  ...
  for (qid = 0; qid < nb_wqs; qid++) {
    ...
    idxd.qid = qid;
    idxd.portal = RTE_PTR_ADD(idxd.u.pci->portals, qid * IDXD_PORTAL_SIZE);
    if (idxd_is_wq_enabled(&idxd))
      IDXD_PMD_ERR("Error, WQ %u seems enabled", qid);
    ret = idxd_dmadev_create(qname, &dev->device, &idxd, &idxd_pci_ops);
    if (ret != 0) {
...
static int idxd_dmadev_destroy(const char *name) {
  int ret = 0;
...
static int idxd_dmadev_remove_pci(struct rte_pci_device *dev) {
  int i = 0;
  char name[PCI_PRI_STR_SIZE];
