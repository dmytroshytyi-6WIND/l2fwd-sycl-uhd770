...
static __rte_always_inline void __submit(struct idxd_dmadev *idxd) {
  ...
  rte_iova_t comp_addr = idxd->batch_iova + (idxd->batch_idx_write *
                                             sizeof(struct idxd_completion));
  ...
  if (idxd->batch_size == 1) {
    ...
    const struct idxd_hw_desc batch_desc = {
        ...
        .desc_addr =
            __desc_idx_to_iova(idxd, idxd->batch_start & idxd->desc_ring_mask),
...
__idxd_write_desc(struct idxd_dmadev *idxd, const uint32_t op_flags,
                  const rte_iova_t src, const rte_iova_t dst,
  ...
  /* we never wrap batches, so we only mask the start and allow start+size to
   * overflow */
  uint16_t write_idx = (idxd->batch_start & mask) + idxd->batch_size;

  /* first check batch ring space then desc ring space */
  if ((idxd->batch_idx_read == 0 &&
       idxd->batch_idx_write == idxd->max_batches) ||
   ...
   */
  const uint64_t op_flags64 =
      (uint64_t)(op_flags | IDXD_FLAG_COMPLETION_ADDR_VALID) << 32;
...
int idxd_enqueue_copy(void *dev_private, uint16_t qid __rte_unused,
                      rte_iova_t src, rte_iova_t dst, unsigned int length,
                      uint64_t flags) {
  /* we can take advantage of the fact that the fence flag in dmadev and DSA are
   * the same, but check it at compile time to be sure.
   */
  ...
  return __idxd_write_desc(dev_private, memmove, src, dst, length, flags);
}

int idxd_enqueue_fill(void *dev_private, uint16_t qid __rte_unused,
                      uint64_t pattern, rte_iova_t dst, unsigned int length,
                      uint64_t flags) {
  uint32_t fill = (idxd_op_fill << IDXD_CMD_OP_SHIFT) |
                  IDXD_FLAG_CACHE_CONTROL | (flags & IDXD_FLAG_FENCE);
  return __idxd_write_desc(dev_private, fill, pattern, dst, length, flags);
}

int idxd_submit(void *dev_private, uint16_t qid __rte_unused) {
  __submit(dev_private);
  return 0;
}

static enum rte_dma_status_code get_comp_status(struct idxd_completion *c) {
  uint8_t st = c->status;
  switch (st) {
...
int idxd_vchan_status(const struct rte_dma_dev *dev,
                      uint16_t vchan __rte_unused,
...
static __rte_always_inline int batch_ok(struct idxd_dmadev *idxd,
                                        uint16_t max_ops,
  ...
  if (bstatus == IDXD_COMP_STATUS_SUCCESS) {
    /* since the batch idx ring stores the start of each batch, pre-increment to
     * lookup start of next batch.
     */
...
static inline uint16_t batch_completed(struct idxd_dmadev *idxd,
                                       uint16_t max_ops, bool *has_error) {
  uint16_t i;
   ...
   * the batch ring indexes to make it look like the first job of the batch has
   * failed. Subsequent calls here will always return zero packets, and the
   * error must be cleared by calling the completed_status() function.
   */
  ...
  for (i = b_start; i < b_end; i++) {
    struct idxd_completion *c =
        (void *)&idxd->desc_ring[i & idxd->desc_ring_mask];
    if (c->status >
        IDXD_COMP_STATUS_SUCCESS) /* ignore incomplete(0) and success(1) */
      break;
  }
...
static uint16_t batch_completed_status(struct idxd_dmadev *idxd,
                                       uint16_t max_ops,
                                       enum rte_dma_status_code *status) {
  uint16_t next_batch;
   ...
   *   - adjust the batch descriptor to update where we stopped, with
   * appropriate bcount
   *   - if bcount is to be exactly 1, update the batch descriptor as it will be
   * treated as non-batch next time.
   */
  const uint16_t bcount =
      idxd->batch_comp_ring[idxd->batch_idx_read].completed_size;
  for (ret = 0; ret < b_len && ret < max_ops; ret++) {
    struct idxd_completion *c =
        (void *)&idxd->desc_ring[(b_start + ret) & idxd->desc_ring_mask];
    status[ret] =
        (ret < bcount) ? get_comp_status(c) : RTE_DMA_STATUS_NOT_ATTEMPTED;
  ...
  /* set up for next time, update existing batch descriptor & start idx at
   * batch_idx_read */
  idxd->batch_idx_ring[idxd->batch_idx_read] += ret;
  if (ret > bcount) {
    ...
    /* if there is only one descriptor left, job skipped so set flag
     * appropriately */
    if (b_len - ret == 1)
...
uint16_t idxd_completed(void *dev_private, uint16_t qid __rte_unused,
                        uint16_t max_ops, uint16_t *last_idx, bool *has_error) {
...
int idxd_stats_get(const struct rte_dma_dev *dev, uint16_t vchan __rte_unused,
...
int idxd_stats_reset(struct rte_dma_dev *dev, uint16_t vchan __rte_unused) {
  struct idxd_dmadev *idxd = dev->fp_obj->dev_private;
  idxd->stats = (struct rte_dma_stats){0};
  return 0;
}

int idxd_info_get(const struct rte_dma_dev *dev, struct rte_dma_info *info,
                  uint32_t size) {
  struct idxd_dmadev *idxd = dev->fp_obj->dev_private;
...
uint16_t idxd_burst_capacity(const void *dev_private,
                             uint16_t vchan __rte_unused) {
...
int idxd_configure(struct rte_dma_dev *dev __rte_unused,
                   const struct rte_dma_conf *dev_conf, uint32_t conf_sz) {
...
int idxd_vchan_setup(struct rte_dma_dev *dev, uint16_t vchan __rte_unused,
  ...
  IDXD_PMD_DEBUG("DMA dev %u using %u descriptors", dev->data->dev_id,
                 max_desc);
  ...
  idxd->desc_ring =
      rte_zmalloc(NULL, sizeof(*idxd->desc_ring) * max_desc * 2, 0);
  ...
  memset(idxd->batch_comp_ring, 0,
         sizeof(*idxd->batch_comp_ring) * (idxd->max_batches + 1));
  return 0;
}
...
int idxd_dmadev_create(const char *name, struct rte_device *dev,
  ...
  dmadev =
      rte_dma_pmd_allocate(name, dev->numa_node, sizeof(struct idxd_dmadev));
  if (dmadev == NULL) {
  ...
  *idxd = *base_idxd; /* copy over the main fields already passed in */
   ...
   */
  idxd->batch_comp_ring = rte_zmalloc_socket(
      NULL,
      (sizeof(idxd->batch_idx_ring[0]) + sizeof(idxd->batch_comp_ring[0])) *
          (idxd->max_batches + 1),
