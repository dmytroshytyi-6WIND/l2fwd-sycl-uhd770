...
static void *idxd_bus_mmap_wq(struct rte_dsa_device *dev) {
  void *addr;
  char path[PATH_MAX];
  int fd;
...
static int read_wq_string(struct rte_dsa_device *dev, const char *filename,
                          char *value, size_t valuelen) {
  char sysfs_node[PATH_MAX];
  int len;
  int fd;

  snprintf(sysfs_node, sizeof(sysfs_node), "%s/%s/%s", dsa_get_sysfs_path(),
           dev->wq_name, filename);
  fd = open(sysfs_node, O_RDONLY);
  if (fd < 0) {
    IDXD_PMD_ERR("%s(): opening file '%s' failed: %s", __func__, sysfs_node,
                 strerror(errno));
    return -1;
  }
  ...
  close(fd);
  if (len < 0) {
    IDXD_PMD_ERR("%s(): error reading file '%s': %s", __func__, sysfs_node,
                 strerror(errno));
    return -1;
  }
  value[len] = '\0';
...
static int read_wq_int(struct rte_dsa_device *dev, const char *filename,
                       int *value) {
  char sysfs_node[PATH_MAX];
  FILE *f;
  int ret = 0;
...
static int read_device_int(struct rte_dsa_device *dev, const char *filename,
                           int *value) {
  char sysfs_node[PATH_MAX];
  FILE *f;
  int ret = 0;
...
static int idxd_probe_dsa(struct rte_dsa_device *dev) {
  struct idxd_dmadev idxd = {0};
  int ret = 0;

  IDXD_PMD_INFO("Probing device %s on numa node %d", dev->wq_name,
...
static int is_for_this_process_use(struct rte_dsa_device *dev,
                                   const char *name) {
...
static int dsa_scan(void) {
  ...
  if (dev_dir == NULL) {
    ...
    IDXD_PMD_ERR("%s(): opendir '%s' failed: %s", __func__, path,
                 strerror(errno));
    return -1;
  }
  ...
  while ((wq = readdir(dev_dir)) != NULL) {
    ...
    if (strnlen(wq->d_name, sizeof(dev->wq_name)) == sizeof(dev->wq_name)) {
      IDXD_PMD_ERR("%s(): wq name too long: '%s', skipping", __func__,
                   wq->d_name);
      continue;
    }
