...
static inline u64 qdma_ccdf_get_queue(const struct fsl_qdma_format *ccdf) {
...
static u32 qdma_readl(void *addr) { return QDMA_IN(addr); }

static void qdma_writel(u32 val, void *addr) { QDMA_OUT(addr, val); }

static u32 qdma_readl_be(void *addr) { return QDMA_IN_BE(addr); }

static void qdma_writel_be(u32 val, void *addr) { QDMA_OUT_BE(addr, val); }
...
static void dma_pool_free(void *addr) { rte_free(addr); }
...
static void fsl_qdma_free_chan_resources(struct fsl_qdma_chan *fsl_chan) {
  ...
  id = (fsl_qdma->block_base - fsl_queue->block_base) / fsl_qdma->block_offset;

  while (rte_atomic32_read(&wait_task[id]) == 1)
    rte_delay_us(QDMA_DELAY);

  list_for_each_entry_safe(comp_temp, _comp_temp, &fsl_queue->comp_used, list) {
  ...
  list_for_each_entry_safe(comp_temp, _comp_temp, &fsl_queue->comp_free, list) {
    list_del(&comp_temp->list);
    dma_pool_free(comp_temp->virt_addr);
    dma_pool_free(comp_temp->desc_virt_addr);
    rte_free(comp_temp);
 ...
 */
static int
fsl_qdma_pre_request_enqueue_comp_sd_desc(struct fsl_qdma_queue *queue,
  ...
  for (i = 0; i < (int)(queue->n_cq + COMMAND_QUEUE_OVERFLOW); i++) {
    comp_temp = rte_zmalloc("qdma: comp temp", sizeof(*comp_temp), 0);
    if (!comp_temp)
      return -ENOMEM;

    comp_temp->virt_addr = dma_pool_alloc(size, aligned, &comp_temp->bus_addr);
    ...
    /* Descriptor Buffer */
    sdf->cmd =
        rte_cpu_to_le_32(FSL_QDMA_CMD_RWTTYPE << FSL_QDMA_CMD_RWTTYPE_OFFSET);
    ddf->cmd =
        rte_cpu_to_le_32(FSL_QDMA_CMD_RWTTYPE << FSL_QDMA_CMD_RWTTYPE_OFFSET);
    ddf->cmd |= rte_cpu_to_le_32(FSL_QDMA_CMD_LWC << FSL_QDMA_CMD_LWC_OFFSET);
...
fail:
  list_for_each_entry_safe(comp_temp, _comp_temp, &queue->comp_free, list) {
    list_del(&comp_temp->list);
    rte_free(comp_temp->virt_addr);
...
fsl_qdma_request_enqueue_desc(struct fsl_qdma_chan *fsl_chan) {
  ...
  if (!list_empty(&queue->comp_free)) {
    comp_temp = list_first_entry(&queue->comp_free, struct fsl_qdma_comp, list);
    list_del(&comp_temp->list);
    return comp_temp;
  }
...
fsl_qdma_alloc_queue_resources(struct fsl_qdma_engine *fsl_qdma) {
  ...
  for (j = 0; j < blocks; j++) {
    for (i = 0; i < queue_num; i++) {
      ...
      queue_temp->cq =
          dma_pool_alloc(sizeof(struct fsl_qdma_format) * queue_size[i],
                         sizeof(struct fsl_qdma_format) * queue_size[i],
                         &queue_temp->bus_addr);

      if (!queue_temp->cq)
        goto fail;

      memset(queue_temp->cq, 0x0,
             queue_size[i] * sizeof(struct fsl_qdma_format));

      queue_temp->block_base =
          fsl_qdma->block_base + FSL_QDMA_BLOCK_BASE_OFFSET(fsl_qdma, j);
...
static struct fsl_qdma_queue *fsl_qdma_prep_status_queue(void) {
   ...
   */
  status_head->cq = dma_pool_alloc(sizeof(struct fsl_qdma_format) * status_size,
                                   sizeof(struct fsl_qdma_format) * status_size,
  ...
  memset(status_head->cq, 0x0, status_size * sizeof(struct fsl_qdma_format));
...
static int fsl_qdma_halt(struct fsl_qdma_engine *fsl_qdma) {
  ...
  for (j = 0; j < fsl_qdma->num_blocks; j++) {
    block = fsl_qdma->block_base + FSL_QDMA_BLOCK_BASE_OFFSET(fsl_qdma, j);
...
static int fsl_qdma_queue_transfer_complete(struct fsl_qdma_engine *fsl_qdma,
  ...
  while (count < nb_cpls) {
    ...
    i = qdma_ccdf_get_queue(status_addr) + id * fsl_qdma->n_queues;
    temp_queue = fsl_queue + i;
    fsl_comp =
        list_first_entry(&temp_queue->comp_used, struct fsl_qdma_comp, list);
...
static int fsl_qdma_reg_init(struct fsl_qdma_engine *fsl_qdma) {
  ...
  for (j = 0; j < fsl_qdma->num_blocks; j++) {
    ...
    qdma_writel(FSL_QDMA_SQCCMR_ENTER_WM, block + FSL_QDMA_SQCCMR);
    ...
    qdma_writel(upper_32_bits(fsl_qdma->status[j]->bus_addr),
                block + FSL_QDMA_SQEEPAR);
    qdma_writel(lower_32_bits(fsl_qdma->status[j]->bus_addr),
...
static void *fsl_qdma_prep_memcpy(void *fsl_chan, dma_addr_t dst,
                                  dma_addr_t src, size_t len, void *call_back,
                                  void *param) {
  struct fsl_qdma_comp *fsl_comp;

  fsl_comp = fsl_qdma_request_enqueue_desc((struct fsl_qdma_chan *)fsl_chan);
  if (!fsl_comp)
    return NULL;
...
static int fsl_qdma_enqueue_desc(struct fsl_qdma_chan *fsl_chan,
   ...
   * to avoid bits swap
   */
  reg = qdma_readl_be(block + FSL_QDMA_BCQSR(fsl_queue->id));
...
static int fsl_qdma_alloc_chan_resources(struct fsl_qdma_chan *fsl_chan) {
  ...
  ret = fsl_qdma_pre_request_enqueue_comp_sd_desc(
      fsl_queue, FSL_QDMA_COMMAND_BUFFER_SIZE, 64);
  if (ret) {
    DPAA_QDMA_ERR("failed to alloc dma buffer for comp descriptor\n");
    goto exit;
  }
...
static int dpaa_info_get(const struct rte_dma_dev *dev,
...
#define DPAADMA_MIN_DESC 64
  ...
  dev_info->dev_capa = RTE_DMA_CAPA_MEM_TO_MEM | RTE_DMA_CAPA_MEM_TO_DEV |
                       RTE_DMA_CAPA_DEV_TO_DEV | RTE_DMA_CAPA_DEV_TO_MEM |
...
static int dpaa_get_channel(struct fsl_qdma_engine *fsl_qdma, uint16_t vchan) {
  u32 i, start, end;
  int ret;
...
static void dma_release(void *fsl_chan) {
  ((struct fsl_qdma_chan *)fsl_chan)->free = true;
  fsl_qdma_free_chan_resources((struct fsl_qdma_chan *)fsl_chan);
}

static int dpaa_qdma_configure(__rte_unused struct rte_dma_dev *dmadev,
                               __rte_unused const struct rte_dma_conf *dev_conf,
                               __rte_unused uint32_t conf_sz) {
  return 0;
}

static int dpaa_qdma_start(__rte_unused struct rte_dma_dev *dev) { return 0; }

static int dpaa_qdma_close(__rte_unused struct rte_dma_dev *dev) { return 0; }

static int
dpaa_qdma_queue_setup(struct rte_dma_dev *dmadev, uint16_t vchan,
...
static int dpaa_qdma_submit(void *dev_private, uint16_t vchan) {
  struct fsl_qdma_engine *fsl_qdma = (struct fsl_qdma_engine *)dev_private;
  struct fsl_qdma_chan *fsl_chan = &fsl_qdma->chans[fsl_qdma->vchan_map[vchan]];
  struct fsl_qdma_queue *fsl_queue = fsl_chan->queue;
  void *block = fsl_queue->block_base;
  u32 reg;
...
static int dpaa_qdma_enqueue(void *dev_private, uint16_t vchan, rte_iova_t src,
  ...
  fsl_comp = fsl_qdma_prep_memcpy(fsl_chan, (dma_addr_t)dst, (dma_addr_t)src,
                                  length, NULL, NULL);
  if (!fsl_comp) {
...
static uint16_t dpaa_qdma_dequeue_status(void *dev_private, uint16_t vchan,
                                         const uint16_t nb_cpls,
                                         uint16_t *last_idx,
  ...
  block = fsl_qdma->block_base + FSL_QDMA_BLOCK_BASE_OFFSET(fsl_qdma, id);
...
static uint16_t dpaa_qdma_dequeue(void *dev_private, uint16_t vchan,
...
static int dpaa_qdma_stats_get(const struct rte_dma_dev *dmadev, uint16_t vchan,
...
static int dpaa_qdma_stats_reset(struct rte_dma_dev *dmadev, uint16_t vchan) {
  struct fsl_qdma_engine *fsl_qdma = dmadev->data->dev_private;
  struct fsl_qdma_chan *fsl_chan = &fsl_qdma->chans[fsl_qdma->vchan_map[vchan]];
  struct fsl_qdma_queue *fsl_queue = fsl_chan->queue;
...
static int dpaa_qdma_init(struct rte_dma_dev *dmadev) {
  struct fsl_qdma_engine *fsl_qdma = dmadev->data->dev_private;
  ...
  fsl_qdma->ctrl_base = mmap(NULL, regs_size, PROT_READ | PROT_WRITE,
                             MAP_SHARED, ccsr_qdma_fd, phys_addr);

  close(ccsr_qdma_fd);
  if (fsl_qdma->ctrl_base == MAP_FAILED) {
    DPAA_QDMA_ERR("Can not map CCSR base qdma: Phys: %08" PRIx64 "size %d\n",
                  phys_addr, regs_size);
    goto err;
  }
  ...
  for (i = 0; i < fsl_qdma->n_chans; i++) {
    ...
    fsl_chan->queue =
        fsl_qdma->queue + i % (fsl_qdma->n_queues * fsl_qdma->num_blocks);
    fsl_chan->free = true;
  }
...
static int dpaa_qdma_probe(__rte_unused struct rte_dpaa_driver *dpaa_drv,
                           struct rte_dpaa_device *dpaa_dev) {
  struct rte_dma_dev *dmadev;
  int ret;

  dmadev = rte_dma_pmd_allocate(dpaa_dev->device.name, rte_socket_id(),
...
static int dpaa_qdma_remove(struct rte_dpaa_device *dpaa_dev) {
