...
int isal_comp_set_priv_xform_parameters(struct isal_priv_xform *priv_xform,
  ...
  if (xform->type == RTE_COMP_COMPRESS) {
    ...
    if (xform->compress.window_size == RTE_COMP_ISAL_WINDOW_SIZE)
      priv_xform->compress.window_size = RTE_COMP_ISAL_WINDOW_SIZE;
    else {
    ...
    case (RTE_COMP_HUFFMAN_DEFAULT):
      priv_xform->compress.deflate.huffman = RTE_COMP_HUFFMAN_DEFAULT;
      break;
    case (RTE_COMP_HUFFMAN_FIXED):
      priv_xform->compress.deflate.huffman = RTE_COMP_HUFFMAN_FIXED;
      break;
    case (RTE_COMP_HUFFMAN_DYNAMIC):
      priv_xform->compress.deflate.huffman = RTE_COMP_HUFFMAN_DYNAMIC;
      break;
    default:
    ...
    /* If using fixed huffman code, level must be 0 */
    else if (priv_xform->compress.deflate.huffman == RTE_COMP_HUFFMAN_FIXED) {
      ISAL_PMD_LOG(DEBUG, "ISA-L level 0 used due to a"
                          " fixed huffman code\n");
      priv_xform->compress.level = RTE_COMP_ISAL_LEVEL_ZERO;
      priv_xform->level_buffer_size = ISAL_DEF_LVL0_DEFAULT;
    } else {
      /* Mapping API levels to ISA-L levels 1,2 & 3 */
      switch (xform->compress.level) {
      case RTE_COMP_LEVEL_PMD_DEFAULT:
        /* Default is 1 if not using fixed huffman */
        priv_xform->compress.level = RTE_COMP_ISAL_LEVEL_ONE;
        priv_xform->level_buffer_size = ISAL_DEF_LVL1_DEFAULT;
        break;
      ...
      default:
        ...
        if (rte_cpu_get_flag_enabled(RTE_CPUFLAG_AVX512F)) {
          priv_xform->compress.level = RTE_COMP_ISAL_LEVEL_THREE;
  ...
  /* Set decompression private xform variables */
    ...
    if (xform->decompress.window_size == RTE_COMP_ISAL_WINDOW_SIZE)
      priv_xform->decompress.window_size = RTE_COMP_ISAL_WINDOW_SIZE;
    else {
      ISAL_PMD_LOG(ERR, "Window size not supported\n");
      return -ENOTSUP;
    }
...
static int chained_mbuf_compression(struct rte_comp_op *op,
  ...
  qp->stream->avail_in =
      RTE_MIN(src->data_len - remaining_offset, op->src.length);
  qp->stream->next_in =
      rte_pktmbuf_mtod_offset(src, uint8_t *, remaining_offset);
  ...
  qp->stream->next_out =
      rte_pktmbuf_mtod_offset(dst, uint8_t *, remaining_offset);
  ...
  while (qp->stream->internal_state.state != ZSTATE_END) {
    ...
    if (qp->stream->avail_in == 0 && qp->stream->total_in != op->src.length) {
      if (src->next != NULL) {
        src = src->next;
        qp->stream->next_in = rte_pktmbuf_mtod(src, uint8_t *);
        qp->stream->avail_in = RTE_MIN(remaining_data, src->data_len);
      } else {
        ISAL_PMD_LOG(ERR, "Not enough input buffer segments\n");
        op->status = RTE_COMP_OP_STATUS_INVALID_ARGS;
        return -1;
      }
    }
    ...
    if (qp->stream->avail_out == 0 &&
      ...
      if (dst->next != NULL) {
        dst = dst->next;
        qp->stream->next_out = rte_pktmbuf_mtod(dst, uint8_t *);
        qp->stream->avail_out = dst->data_len;
      } else {
        ISAL_PMD_LOG(ERR, "Not enough output buffer segments\n");
...
static int chained_mbuf_decompression(struct rte_comp_op *op,
                                      struct isal_comp_qp *qp) {
  int ret;
  ...
  qp->state->avail_in =
      RTE_MIN(src->data_len - src_remaining_offset, op->src.length);
  qp->state->next_in =
      rte_pktmbuf_mtod_offset(src, uint8_t *, src_remaining_offset);
  ...
  qp->state->next_out =
      rte_pktmbuf_mtod_offset(dst, uint8_t *, dst_remaining_offset);
  ...
  while (qp->state->block_state != ISAL_BLOCK_FINISH) {
    ...
    if (qp->state->avail_in == 0 && op->consumed != op->src.length) {
      op->consumed += consumed_data;
      remaining_data -= consumed_data;

      if (src->next != NULL) {
        src = src->next;
        qp->state->next_in = rte_pktmbuf_mtod(src, uint8_t *);
        qp->state->avail_in = RTE_MIN(remaining_data, src->data_len);
      }
    }

    if (ret == ISAL_OUT_OVERFLOW) {
      ISAL_PMD_LOG(ERR,
                   "Decompression operation ran "
    ...
    if (qp->state->avail_out == 0 &&
      ...
      if (dst->next != NULL) {
        dst = dst->next;
        qp->state->next_out = rte_pktmbuf_mtod(dst, uint8_t *);
...
static int process_isal_deflate(struct rte_comp_op *op, struct isal_comp_qp *qp,
  ...
  if (priv_xform->compress.deflate.huffman == RTE_COMP_HUFFMAN_FIXED)
    isal_deflate_set_hufftables(qp->stream, NULL, IGZIP_HUFFTABLE_STATIC);
  else if (priv_xform->compress.deflate.huffman == RTE_COMP_HUFFMAN_DEFAULT)
    isal_deflate_set_hufftables(qp->stream, NULL, IGZIP_HUFFTABLE_DEFAULT);
  /* Dynamically change the huffman code to suit the input data */
  else if (priv_xform->compress.deflate.huffman == RTE_COMP_HUFFMAN_DYNAMIC)
    isal_deflate_set_hufftables(qp->stream, NULL, IGZIP_HUFFTABLE_DEFAULT);
...
static int process_isal_inflate(struct rte_comp_op *op, struct isal_comp_qp *qp,
                                struct isal_priv_xform *priv_xform) {
  int ret = 0;
...
static int process_op(struct isal_comp_qp *qp, struct rte_comp_op *op,
                      struct isal_priv_xform *priv_xform) {
...
/* Enqueue burst */
static uint16_t isal_comp_pmd_enqueue_burst(void *queue_pair,
  ...
  for (i = 0; i < num_enq; i++) {
    ...
    if (unlikely(retval < 0) || ops[i]->status != RTE_COMP_OP_STATUS_SUCCESS) {
      qp->qp_stats.enqueue_err_count++;
    }
  }

  retval =
      rte_ring_enqueue_burst(qp->processed_pkts, (void *)ops, num_enq, NULL);
...
/* Dequeue burst */
static uint16_t isal_comp_pmd_dequeue_burst(void *queue_pair,
                                            struct rte_comp_op **ops,
                                            uint16_t nb_ops) {
  struct isal_comp_qp *qp = queue_pair;
  ...
  nb_dequeued =
      rte_ring_dequeue_burst(qp->processed_pkts, (void **)ops, nb_ops, NULL);
...
compdev_isal_create(const char *name, struct rte_vdev_device *vdev,
  ...
  dev = rte_compressdev_pmd_create(
      name, &vdev->device, sizeof(struct isal_comp_private), init_params);
  if (dev == NULL) {
...
static int compdev_isal_remove_dev(struct rte_vdev_device *vdev) {
...
static int compdev_isal_probe(struct rte_vdev_device *dev) {
  struct rte_compressdev_pmd_init_params init_params = {
      "",
      rte_socket_id(),
  };
  ...
  if (retval) {
    ISAL_PMD_LOG(ERR, "Failed to parse initialisation arguments[%s]\n", args);
    return -EINVAL;
  }
...
RTE_PMD_REGISTER_PARAM_STRING(COMPDEV_NAME_ISAL_PMD, "socket_id=<int>");
