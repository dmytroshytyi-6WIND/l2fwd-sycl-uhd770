...
static const struct rte_compressdev_capabilities isal_pmd_capabilities[] = {
    {
        ...
        .comp_feature_flags =
            RTE_COMP_FF_OOP_SGL_IN_SGL_OUT | RTE_COMP_FF_OOP_SGL_IN_LB_OUT |
            RTE_COMP_FF_OOP_LB_IN_SGL_OUT | RTE_COMP_FF_SHAREABLE_PRIV_XFORM |
...
/** Configure device */
static int isal_comp_pmd_config(struct rte_compressdev *dev,
  ...
  n = snprintf(mp_name, sizeof(mp_name), "compdev_%d_xform_mp",
               dev->data->dev_id);
  if (n > sizeof(mp_name)) {
    ISAL_PMD_LOG(ERR, "Unable to create unique name for xform mempool");
    return -ENOMEM;
  }
  ...
  if (internals->priv_xform_mp != NULL) {
    if (((internals->priv_xform_mp)->elt_size != elt_size) ||
        ((internals->priv_xform_mp)->size < config->max_nb_priv_xforms)) {

      ISAL_PMD_LOG(ERR,
                   "%s mempool already exists with different"
    ...
    internals->priv_xform_mp = rte_mempool_create(
        ...
        /* number of elements*/
        config->max_nb_priv_xforms, elt_size, /* element size*/
        0,                                    /* Cache size*/
        0,                                    /* private data size */
...
/** Start device */
static int isal_comp_pmd_start(__rte_unused struct rte_compressdev *dev) {
  return 0;
}

/** Stop device */
static void isal_comp_pmd_stop(__rte_unused struct rte_compressdev *dev) {}

/** Close device */
static int isal_comp_pmd_close(struct rte_compressdev *dev) {
  /* Free private data */
  struct isal_comp_private *internals = dev->data->dev_private;
...
static void isal_comp_pmd_stats_get(struct rte_compressdev *dev,
...
/** Get device info */
static void isal_comp_pmd_info_get(struct rte_compressdev *dev __rte_unused,
...
static void isal_comp_pmd_stats_reset(struct rte_compressdev *dev) {
  uint16_t qp_id;
...
/** Release queue pair */
static int isal_comp_pmd_qp_release(struct rte_compressdev *dev,
                                    uint16_t qp_id) {
  struct isal_comp_qp *qp = dev->data->queue_pairs[qp_id];
...
static struct rte_ring *isal_comp_pmd_qp_create_processed_pkts_ring(
    struct isal_comp_qp *qp, unsigned int ring_size, int socket_id) {
  struct rte_ring *r;

  r = rte_ring_lookup(qp->name);
  if (r) {
    if (rte_ring_get_size(r) >= ring_size) {
      ISAL_PMD_LOG(DEBUG, "Reusing existing ring %s for processed packets",
                   qp->name);
      return r;
    }
...
static int isal_comp_pmd_qp_set_unique_name(struct rte_compressdev *dev,
                                            struct isal_comp_qp *qp) {
  unsigned int n =
      snprintf(qp->name, sizeof(qp->name), "isal_comp_pmd_%u_qp_%u",
...
/* Setup a queue pair */
static int isal_comp_pmd_qp_setup(struct rte_compressdev *dev, uint16_t qp_id,
  ...
  qp->stream->level_buf =
      rte_zmalloc_socket("Isa-l compression lev_buf", ISAL_DEF_LVL3_DEFAULT,
                         RTE_CACHE_LINE_SIZE, socket_id);
  ...
  qp->processed_pkts = isal_comp_pmd_qp_create_processed_pkts_ring(
      qp, max_inflight_ops, socket_id);
...
static int isal_comp_pmd_priv_xform_create(struct rte_compressdev *dev,
  ...
  if (rte_mempool_get(internals->priv_xform_mp, priv_xform)) {
    ISAL_PMD_LOG(ERR, "Couldn't get object from private xform mempool");
    return -ENOMEM;
  }
...
static int isal_comp_pmd_priv_xform_free(struct rte_compressdev *dev,
                                         void *priv_xform) {
  struct isal_comp_private *internals = dev->data->dev_private;
