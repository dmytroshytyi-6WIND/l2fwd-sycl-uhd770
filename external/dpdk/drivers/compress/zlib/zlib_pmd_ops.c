...
static const struct rte_compressdev_capabilities zlib_pmd_capabilities[] = {
    {
        ...
        .comp_feature_flags =
            (RTE_COMP_FF_NONCOMPRESSED_BLOCKS | RTE_COMP_FF_HUFFMAN_FIXED |
             RTE_COMP_FF_HUFFMAN_DYNAMIC),
        .window_size = {.min = 8, .max = 15, .increment = 1},
    },
...
/** Configure device */
static int zlib_pmd_config(struct rte_compressdev *dev,
                           struct rte_compressdev_config *config) {
  struct rte_mempool *mp;
  char mp_name[RTE_MEMPOOL_NAMESIZE];
  struct zlib_private *internals = dev->data->dev_private;

  snprintf(mp_name, RTE_MEMPOOL_NAMESIZE, "stream_mp_%u", dev->data->dev_id);
  mp = internals->mp;
  if (mp == NULL) {
    mp = rte_mempool_create(mp_name,
                            config->max_nb_priv_xforms + config->max_nb_streams,
                            sizeof(struct zlib_priv_xform), 0, 0, NULL, NULL,
                            NULL, NULL, config->socket_id, 0);
    if (mp == NULL) {
...
/** Start device */
static int zlib_pmd_start(__rte_unused struct rte_compressdev *dev) {
  return 0;
}

/** Stop device */
static void zlib_pmd_stop(__rte_unused struct rte_compressdev *dev) {}

/** Close device */
static int zlib_pmd_close(struct rte_compressdev *dev) {
  struct zlib_private *internals = dev->data->dev_private;
...
static void zlib_pmd_stats_get(struct rte_compressdev *dev,
...
static void zlib_pmd_stats_reset(struct rte_compressdev *dev) {
  int qp_id;
...
/** Get device info */
static void zlib_pmd_info_get(struct rte_compressdev *dev,
...
/** Release queue pair */
static int zlib_pmd_qp_release(struct rte_compressdev *dev, uint16_t qp_id) {
  struct zlib_qp *qp = dev->data->queue_pairs[qp_id];
...
static int zlib_pmd_qp_set_unique_name(struct rte_compressdev *dev,
                                       struct zlib_qp *qp) {
  unsigned int n = snprintf(qp->name, sizeof(qp->name), "zlib_pmd_%u_qp_%u",
...
/** Setup a queue pair */
static int zlib_pmd_qp_setup(struct rte_compressdev *dev, uint16_t qp_id,
  ...
  qp->processed_pkts =
      zlib_pmd_qp_create_processed_pkts_ring(qp, max_inflight_ops, socket_id);
...
/** Configure stream */
static int zlib_pmd_stream_create(struct rte_compressdev *dev,
...
static int zlib_pmd_private_xform_create(struct rte_compressdev *dev,
                                         const struct rte_comp_xform *xform,
                                         void **private_xform) {
...
static int zlib_pmd_stream_free(__rte_unused struct rte_compressdev *dev,
                                void *zstream) {
...
/** Clear the memory of stream so it doesn't leave key material behind */
static int zlib_pmd_private_xform_free(struct rte_compressdev *dev,
                                       void *private_xform) {
