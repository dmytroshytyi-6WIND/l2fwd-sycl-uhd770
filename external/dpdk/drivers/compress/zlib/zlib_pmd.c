...
#define COMPUTE_BUF(mbuf, data, len)                                           \
  ((mbuf = mbuf->next) ? (data = rte_pktmbuf_mtod(mbuf, uint8_t *)),           \
   (len = rte_pktmbuf_data_len(mbuf)) : 0)

static void process_zlib_deflate(struct rte_comp_op *op, z_stream *strm) {
  ...
  strm->next_in = rte_pktmbuf_mtod_offset(mbuf_src, uint8_t *, op->src.offset);

  strm->avail_in = rte_pktmbuf_data_len(mbuf_src) - op->src.offset;

  strm->next_out = rte_pktmbuf_mtod_offset(mbuf_dst, uint8_t *, op->dst.offset);
...
static void process_zlib_inflate(struct rte_comp_op *op, z_stream *strm) {
  int ret, flush;
  struct rte_mbuf *mbuf_src = op->m_src;
  struct rte_mbuf *mbuf_dst = op->m_dst;
...
static inline int process_zlib_op(struct zlib_qp *qp, struct rte_comp_op *op) {
...
int zlib_set_stream_parameters(const struct rte_comp_xform *xform,
                               struct zlib_stream *stream) {
  ...
  case RTE_COMP_COMPRESS:
    ...
    if (deflateInit2(strm, level, Z_DEFLATED, wbits, DEF_MEM_LEVEL, strategy) !=
        Z_OK) {
...
static uint16_t zlib_pmd_enqueue_burst(void *queue_pair,
...
static uint16_t zlib_pmd_dequeue_burst(void *queue_pair,
                                       struct rte_comp_op **ops,
                                       uint16_t nb_ops) {
  struct zlib_qp *qp = queue_pair;
...
static int zlib_create(const char *name, struct rte_vdev_device *vdev,
...
static int zlib_probe(struct rte_vdev_device *vdev) {
  struct rte_compressdev_pmd_init_params init_params = {"", rte_socket_id()};
  const char *name;
  ...
  if (retval < 0) {
    ZLIB_PMD_LOG(ERR, "Failed to parse initialisation arguments[%s]\n",
                 input_args);
    return -EINVAL;
  }
...
static struct rte_vdev_driver zlib_pmd_drv = {.probe = zlib_probe,
                                              .remove = zlib_remove};
