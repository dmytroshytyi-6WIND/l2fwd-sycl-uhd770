...
TAILQ_HEAD(mlx5_compress_privs, mlx5_compress_priv)
mlx5_compress_priv_list = TAILQ_HEAD_INITIALIZER(mlx5_compress_priv_list);
...
static const struct rte_compressdev_capabilities mlx5_caps[] = {
    ...
    },
    {
        ...
        .comp_feature_flags =
            RTE_COMP_FF_ADLER32_CHECKSUM | RTE_COMP_FF_CRC32_CHECKSUM |
            RTE_COMP_FF_CRC32_ADLER32_CHECKSUM |
            RTE_COMP_FF_SHAREABLE_PRIV_XFORM | RTE_COMP_FF_HUFFMAN_FIXED |
...
static void mlx5_compress_dev_info_get(struct rte_compressdev *dev,
...
static int mlx5_compress_dev_configure(struct rte_compressdev *dev,
...
static int mlx5_compress_dev_close(struct rte_compressdev *dev) {
  RTE_SET_USED(dev);
  return 0;
}

static int mlx5_compress_qp_release(struct rte_compressdev *dev,
                                    uint16_t qp_id) {
...
static void mlx5_compress_init_qp(struct mlx5_compress_qp *qp) {
  ...
  const uint32_t flags = RTE_BE32(MLX5_COMP_ALWAYS << MLX5_COMP_MODE_OFFSET);
...
static int mlx5_compress_qp_setup(struct rte_compressdev *dev, uint16_t qp_id,
  ...
  qp->ops = (struct rte_comp_op **)RTE_ALIGN((uintptr_t)(qp + 1),
                                             RTE_CACHE_LINE_SIZE);
  if (mlx5_common_verbs_reg_mr(priv->cdev->pd, opaq_buf,
                               qp->entries_n *
                                   sizeof(struct mlx5_gga_compress_opaque),
                               &qp->opaque_mr) != 0) {
    rte_free(opaq_buf);
  ...
  ret = mlx5_devx_qp_create(priv->cdev->ctx, &qp->qp,
                            qp_attr.num_of_send_wqbbs * MLX5_WQE_SIZE, &qp_attr,
                            socket_id);
  if (ret != 0) {
  ...
  if (ret)
    goto err;
  DRV_LOG(INFO, "QP %u: SQN=0x%X CQN=0x%X entries num = %u", (uint32_t)qp_id,
          qp->qp.qp->id, qp->cq.cq->id, qp->entries_n);
  return 0;
err:
...
static int mlx5_compress_xform_free(struct rte_compressdev *dev, void *xform) {
  struct mlx5_compress_priv *priv = dev->data->dev_private;
...
static int mlx5_compress_xform_create(struct rte_compressdev *dev,
  ...
  case RTE_COMP_COMPRESS:
    if (xform->compress.algo == RTE_COMP_ALGO_NULL && !priv->mmo_dma_qp &&
        !priv->mmo_dma_sq) {
      DRV_LOG(ERR, "Not enough capabilities to support DMA operation, maybe "
                   "old FW/OFED version?");
      return -ENOTSUP;
  ...
  case RTE_COMP_DECOMPRESS:
    if (xform->decompress.algo == RTE_COMP_ALGO_NULL && !priv->mmo_dma_qp &&
        !priv->mmo_dma_sq) {
      DRV_LOG(ERR, "Not enough capabilities to support DMA operation, maybe "
                   "old FW/OFED version?");
      return -ENOTSUP;
    } else if (!priv->mmo_decomp_qp && !priv->mmo_decomp_sq) {
      DRV_LOG(ERR, "Not enough capabilities to support decompress operation, "
                   "maybe old FW/OFED version?");
      return -ENOTSUP;
    }
    if (xform->compress.hash_algo != RTE_COMP_HASH_ALGO_NONE) {
  ...
  case RTE_COMP_COMPRESS:
    ...
    case RTE_COMP_ALGO_NULL:
      xfrm->opcode += MLX5_OPC_MOD_MMO_DMA << WQE_CSEG_OPC_MOD_OFFSET;
      break;
    case RTE_COMP_ALGO_DEFLATE:
      size = 1 << xform->compress.window_size;
      size /= MLX5_GGA_COMP_WIN_SIZE_UNITS;
      xfrm->gga_ctrl1 +=
          RTE_MIN(rte_log2_u32(size), MLX5_COMP_MAX_WIN_SIZE_CONF)
          << WQE_GGA_COMP_WIN_SIZE_OFFSET;
      size = priv->log_block_sz;
      xfrm->gga_ctrl1 += size << WQE_GGA_COMP_BLOCK_SIZE_OFFSET;
      xfrm->opcode += MLX5_OPC_MOD_MMO_COMP << WQE_CSEG_OPC_MOD_OFFSET;
      size = xform->compress.deflate.huffman == RTE_COMP_HUFFMAN_DYNAMIC
                 ? MLX5_GGA_COMP_LOG_DYNAMIC_SIZE_MAX
                 : MLX5_GGA_COMP_LOG_DYNAMIC_SIZE_MIN;
      xfrm->gga_ctrl1 += size << WQE_GGA_COMP_DYNAMIC_SIZE_OFFSET;
      break;
    default:
      goto err;
    }
  ...
  case RTE_COMP_DECOMPRESS:
    ...
    case RTE_COMP_ALGO_NULL:
      ...
      break;
    case RTE_COMP_ALGO_DEFLATE:
      xfrm->opcode += MLX5_OPC_MOD_MMO_DECOMP << WQE_CSEG_OPC_MOD_OFFSET;
      break;
    default:
      goto err;
    }
  ...
  DRV_LOG(DEBUG,
          "New xform: gga ctrl1 = 0x%08X opcode = 0x%08X csum "
          "type = %d.",
          xfrm->gga_ctrl1, xfrm->opcode, xfrm->csum_type);
...
static void mlx5_compress_dev_stop(struct rte_compressdev *dev) {
  RTE_SET_USED(dev);
}

static int mlx5_compress_dev_start(struct rte_compressdev *dev) {
  struct mlx5_compress_priv *priv = dev->data->dev_private;

  return mlx5_dev_mempool_subscribe(priv->cdev);
}

static void mlx5_compress_stats_get(struct rte_compressdev *dev,
                                    struct rte_compressdev_stats *stats) {
  int qp_id;
...
static void mlx5_compress_stats_reset(struct rte_compressdev *dev) {
  int qp_id;
...
static __rte_always_inline uint32_t mlx5_compress_dseg_set(
    struct mlx5_compress_qp *qp, volatile struct mlx5_wqe_dseg *restrict dseg,
    struct rte_mbuf *restrict mbuf, uint32_t offset, uint32_t len) {
...
static uint16_t mlx5_compress_enqueue_burst(void *queue_pair,
                                            struct rte_comp_op **ops,
                                            uint16_t nb_ops) {
  struct mlx5_compress_qp *qp = queue_pair;
  volatile struct mlx5_gga_wqe *wqes =
                                   (volatile struct mlx5_gga_wqe *)qp->qp.wqes,
                               *wqe;
  ...
  do {
    ...
    if (unlikely(
            invalid ||
            (mlx5_compress_dseg_set(qp, &wqe->gather, op->m_src, op->src.offset,
      ...
      op->status =
          invalid ? RTE_COMP_OP_STATUS_INVALID_ARGS : RTE_COMP_OP_STATUS_ERROR;
      nb_ops -= remain;
  ...
  mlx5_doorbell_ring(&qp->priv->uar.bf_db, *(volatile uint64_t *)wqe, qp->pi,
                     &qp->qp.db_rec[MLX5_SND_DBR], !qp->priv->uar.dbnc);
  return nb_ops;
}

static void mlx5_compress_dump_err_objs(volatile uint32_t *cqe,
  ...
  for (i = 0; i < sizeof(struct mlx5_gga_compress_opaque) >> 2; i += 4)
    DRV_LOG(ERR, "%08X %08X %08X %08X", opaq[i], opaq[i + 1], opaq[i + 2],
            opaq[i + 3]);
}

static void mlx5_compress_cqe_err_handle(struct mlx5_compress_qp *qp,
                                         struct rte_comp_op *op) {
  const uint32_t idx = qp->ci & (qp->entries_n - 1);
  volatile struct mlx5_err_cqe *cqe =
      (volatile struct mlx5_err_cqe *)&qp->cq.cqes[idx];
  volatile struct mlx5_gga_wqe *wqes =
      (volatile struct mlx5_gga_wqe *)qp->qp.wqes;
  volatile struct mlx5_gga_compress_opaque *opaq = qp->opaque_mr.addr;

  volatile uint32_t *synd_word =
      RTE_PTR_ADD(cqe, MLX5_ERROR_CQE_SYNDROME_OFFSET);
  switch (*synd_word) {
  ...
  case MLX5_GGA_COMP_MISSING_BFINAL_SYNDROME_BE:
    DRV_LOG(DEBUG, "The last compressed block missed the B-final flag; maybe "
                   "the compressed data is not complete or garbaged?");
    /* fallthrough */
  default:
...
static uint16_t mlx5_compress_dequeue_burst(void *queue_pair,
                                            struct rte_comp_op **ops,
                                            uint16_t nb_ops) {
  struct mlx5_compress_qp *qp = queue_pair;
  ...
  do {
    ...
    if (unlikely(ret != MLX5_CQE_STATUS_SW_OWN)) {
      ...
      MLX5_ASSERT(cqe->byte_cnt == opaq[idx].scattered_length);
      switch (xform->csum_type) {
      case RTE_COMP_CHECKSUM_CRC32:
        op->output_chksum = (uint64_t)rte_be_to_cpu_32(opaq[idx].crc32);
        break;
      case RTE_COMP_CHECKSUM_ADLER32:
        op->output_chksum = (uint64_t)rte_be_to_cpu_32(opaq[idx].adler32) << 32;
        break;
      case RTE_COMP_CHECKSUM_CRC32_ADLER32:
        op->output_chksum =
            (uint64_t)rte_be_to_cpu_32(opaq[idx].crc32) |
            ((uint64_t)rte_be_to_cpu_32(opaq[idx].adler32) << 32);
        break;
      default:
        break;
      }
...
static int mlx5_compress_args_check_handler(const char *key, const char *val,
                                            void *opaque) {
...
mlx5_compress_handle_devargs(struct mlx5_kvargs_ctrl *mkvlist,
  ...
  if (mkvlist == NULL)
    return 0;
  if (mlx5_kvargs_process(mkvlist, params, mlx5_compress_args_check_handler,
                          devarg_prms) != 0) {
  ...
  if (devarg_prms->log_block_sz > MLX5_GGA_COMP_LOG_BLOCK_SIZE_MAX ||
    ...
    DRV_LOG(WARNING,
            "Log block size provided is out of range("
            "%u); default it to %u.",
            devarg_prms->log_block_sz, MLX5_GGA_COMP_LOG_BLOCK_SIZE_MAX);
    devarg_prms->log_block_sz = MLX5_GGA_COMP_LOG_BLOCK_SIZE_MAX;
  }
...
static int mlx5_compress_dev_probe(struct mlx5_common_device *cdev,
  ...
  if (!attr->mmo_decompress_qp_en && !attr->mmo_decompress_sq_en &&
    ...
    DRV_LOG(ERR, "Not enough capabilities to support compress operations, "
                 "maybe old FW/OFED version?");
    rte_errno = ENOTSUP;
    return -ENOTSUP;
  }
  mlx5_compress_handle_devargs(mkvlist, &devarg_prms, attr);
  compressdev = rte_compressdev_pmd_create(ibdev_name, cdev->dev, sizeof(*priv),
                                           &init_params);
  if (compressdev == NULL) {
    DRV_LOG(ERR, "Failed to create device \"%s\".", ibdev_name);
    return -ENODEV;
  }
  DRV_LOG(INFO, "Compress device %s was created successfully.", ibdev_name);
...
static int mlx5_compress_dev_remove(struct mlx5_common_device *cdev) {
...
RTE_PMD_REGISTER_KMOD_DEP(MLX5_COMPRESS_DRIVER_NAME,
