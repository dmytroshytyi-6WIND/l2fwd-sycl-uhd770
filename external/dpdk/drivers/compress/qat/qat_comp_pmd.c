...
qat_comp_get_capa_info(enum qat_device_gen qat_dev_gen,
  ...
  RTE_FUNC_PTR_OR_ERR_RET(
      qat_comp_gen_dev_ops[qat_dev_gen].qat_comp_get_capabilities, ret);
  return qat_comp_gen_dev_ops[qat_dev_gen].qat_comp_get_capabilities(qat_dev);
}

void qat_comp_stats_get(struct rte_compressdev *dev,
...
void qat_comp_stats_reset(struct rte_compressdev *dev) {
  struct qat_comp_dev_private *qat_priv;
...
int qat_comp_qp_release(struct rte_compressdev *dev, uint16_t queue_pair_id) {
  ...
  QAT_LOG(DEBUG, "Release comp qp %u on device %d", queue_pair_id,
          dev->data->dev_id);
  ...
  return qat_qp_release(
      qat_dev_gen, (struct qat_qp **)&(dev->data->queue_pairs[queue_pair_id]));
}

int qat_comp_qp_setup(struct rte_compressdev *dev, uint16_t qp_id,
                      uint32_t max_inflight_ops, int socket_id) {
  struct qat_qp_config qat_qp_conf = {0};
  struct qat_qp **qp_addr = (struct qat_qp **)&(dev->data->queue_pairs[qp_id]);
  struct qat_comp_dev_private *qat_private = dev->data->dev_private;
  ...
  if (qp_id >= qat_qps_per_service(qat_dev, QAT_SERVICE_COMPRESSION)) {
    QAT_LOG(ERR, "qp_id %u invalid for this device", qp_id);
    return -EINVAL;
  }

  qat_qp_conf.hw = qat_qp_get_hw_data(qat_dev, QAT_SERVICE_COMPRESSION, qp_id);
  ...
  qat_private->qat_dev->qps_in_use[QAT_SERVICE_COMPRESSION][qp_id] = *qp_addr;
  ...
  for (i = 0; i < qp->nb_descriptors; i++) {
    ...
    cookie->qat_sgl_src_d = rte_zmalloc_socket(
        NULL,
        sizeof(struct qat_sgl) +
            sizeof(struct qat_flat_buf) * QAT_PMD_COMP_SGL_DEF_SEGMENTS,
        64, dev->data->socket_id);

    cookie->qat_sgl_dst_d = rte_zmalloc_socket(
        NULL,
        sizeof(struct qat_sgl) +
            sizeof(struct qat_flat_buf) * QAT_PMD_COMP_SGL_DEF_SEGMENTS,
        64, dev->data->socket_id);

    if (cookie->qat_sgl_src_d == NULL || cookie->qat_sgl_dst_d == NULL) {
      QAT_LOG(ERR,
              "Can't allocate SGL"
              " for device %s",
    ...
    cookie->qat_sgl_src_phys_addr = rte_malloc_virt2iova(cookie->qat_sgl_src_d);

    cookie->qat_sgl_dst_phys_addr = rte_malloc_virt2iova(cookie->qat_sgl_dst_d);

    cookie->dst_nb_elems = cookie->src_nb_elems = QAT_PMD_COMP_SGL_DEF_SEGMENTS;
...
qat_comp_setup_inter_buffers(struct qat_comp_dev_private *comp_dev,
  ...
  int i;
  int num_im_sgls =
      qat_comp_get_num_im_bufs_required(comp_dev->qat_dev->qat_dev_gen);

  QAT_LOG(DEBUG, "QAT COMP device %s needs %d sgls", comp_dev->qat_dev->name,
          num_im_sgls);
  snprintf(inter_buff_mz_name, RTE_MEMZONE_NAMESIZE, "%s_inter_buff",
  ...
  full_size = offset_of_flat_buffs + buff_size * QAT_NUM_BUFS_IN_IM_SGL;

  memzone =
      rte_memzone_reserve_aligned(inter_buff_mz_name, size_of_ptr_array,
                                  comp_dev->compressdev->data->socket_id,
                                  RTE_MEMZONE_IOVA_CONTIG, QAT_64_BYTE_ALIGN);
  if (memzone == NULL) {
    QAT_LOG(ERR, "Can't allocate intermediate buffers for device %s",
            comp_dev->qat_dev->name);
    return NULL;
  }
  ...
  QAT_LOG(DEBUG,
          "Memzone %s: addr = %p, phys = 0x%" PRIx64
          ", size required %d, size created %zu",
          inter_buff_mz_name, mz_start, mz_start_phys, size_of_ptr_array,
          memzone->len);
  ...
  for (i = 0; i < num_im_sgls; i++) {
    ...
    snprintf(inter_buff_mz_name, RTE_MEMZONE_NAMESIZE, "%s_inter_buff_%d",
             comp_dev->qat_dev->name, i);
    mz = rte_memzone_lookup(inter_buff_mz_name);
    if (mz == NULL) {
      mz = rte_memzone_reserve_aligned(
          inter_buff_mz_name, full_size, comp_dev->compressdev->data->socket_id,
          RTE_MEMZONE_IOVA_CONTIG, QAT_64_BYTE_ALIGN);
      if (mz == NULL) {
        QAT_LOG(ERR, "Can't allocate intermediate buffers for device %s",
                comp_dev->qat_dev->name);
        while (--i >= 0) {
          snprintf(inter_buff_mz_name, RTE_MEMZONE_NAMESIZE, "%s_inter_buff_%d",
                   comp_dev->qat_dev->name, i);
          rte_memzone_free(rte_memzone_lookup(inter_buff_mz_name));
        }
    ...
    QAT_LOG(DEBUG,
            "Memzone %s: addr = %p, phys = 0x%" PRIx64
            ", size required %d, size created %zu",
            inter_buff_mz_name, mz->addr, mz->iova, full_size, mz->len);
    ...
    QAT_LOG(DEBUG,
            "  : phys addr of sgl[%i] in array_of_pointers"
            " = 0x%" PRIx64,
            i, array_of_pointers->pointer[i]);
    QAT_LOG(DEBUG, "  : virt address of sgl[%i] = %p", i, sgl);
#endif
    for (lb = 0; lb < QAT_NUM_BUFS_IN_IM_SGL; lb++) {
      sgl->buffers[lb].addr = mz->iova + offset_of_flat_buffs + lb * buff_size;
      sgl->buffers[lb].len = buff_size;
      sgl->buffers[lb].resrvd = 0;
#if QAT_IM_BUFFER_DEBUG
      QAT_LOG(DEBUG, "  : sgl->buffers[%d].addr = 0x%" PRIx64 ", len=%d", lb,
              sgl->buffers[lb].addr, sgl->buffers[lb].len);
#endif
    }
  }
#if QAT_IM_BUFFER_DEBUG
  QAT_DP_HEXDUMP_LOG(DEBUG, "IM buffer memzone start:", memzone->addr,
                     size_of_ptr_array);
#endif
  return memzone;
}
...
qat_comp_create_xform_pool(struct qat_comp_dev_private *comp_dev,
  ...
  if (mp == NULL)
    mp =
        rte_mempool_create(xform_pool_name, num_elements, qat_comp_xform_size(),
                           0, 0, NULL, NULL, NULL, NULL, config->socket_id, 0);
  if (mp == NULL) {
...
static void qat_comp_stream_init(struct rte_mempool *mp __rte_unused,
  ...
  if (memzone == NULL) {
    ...
    memzone = rte_memzone_reserve_aligned(
        mz_name,
        QAT_STATE_REGISTERS_MAX_SIZE + sizeof(struct qat_inter_sgl) +
            QAT_INFLATE_CONTEXT_SIZE,
        info->socket_id, RTE_MEMZONE_IOVA_CONTIG, QAT_64_BYTE_ALIGN);
    if (memzone == NULL) {
      QAT_LOG(ERR, "Can't allocate RAM banks for device %s, object %u",
              info->comp_dev->qat_dev->name, obj_idx);
      info->error = -ENOMEM;
      return;
    }
  ...
  ram_banks_desc = (struct qat_inter_sgl *)(((uint8_t *)memzone->addr) +
  ...
  stream->inflate_context =
      ((uint8_t *)memzone->addr) + QAT_STATE_REGISTERS_MAX_SIZE;
  stream->inflate_context_phys = memzone->iova + QAT_STATE_REGISTERS_MAX_SIZE;
}

static void qat_comp_stream_destroy(struct rte_mempool *mp __rte_unused,
...
qat_comp_create_stream_pool(struct qat_comp_dev_private *comp_dev,
                            int socket_id, uint32_t num_elements) {
  char stream_pool_name[RTE_MEMPOOL_NAMESIZE];
  struct rte_mempool *mp;

  snprintf(stream_pool_name, RTE_MEMPOOL_NAMESIZE, "%s_streams",
           comp_dev->qat_dev->name);
  ...
  if (mp == NULL) {
    struct stream_create_info info = {
        .comp_dev = comp_dev, .socket_id = socket_id, .error = 0};
    mp = rte_mempool_create(stream_pool_name, num_elements,
                            qat_comp_stream_size(), 0, 0, NULL, NULL,
                            qat_comp_stream_init, &info, socket_id, 0);
    if (mp == NULL) {
      QAT_LOG(ERR, "Err creating mempool %s w %d elements of size %d",
              stream_pool_name, num_elements, qat_comp_stream_size());
    } else if (info.error) {
      rte_mempool_obj_iter(mp, qat_comp_stream_destroy, NULL);
      QAT_LOG(
          ERR,
...
static void _qat_comp_dev_config_clear(struct qat_comp_dev_private *comp_dev) {
  /* Free intermediate buffers */
  if (comp_dev->interm_buff_mz) {
    char mz_name[RTE_MEMZONE_NAMESIZE];
    int i = qat_comp_get_num_im_bufs_required(comp_dev->qat_dev->qat_dev_gen);

    while (--i >= 0) {
      snprintf(mz_name, RTE_MEMZONE_NAMESIZE, "%s_inter_buff_%d",
               comp_dev->qat_dev->name, i);
  ...
  if (comp_dev->streampool) {
    rte_mempool_obj_iter(comp_dev->streampool, qat_comp_stream_destroy, NULL);
    rte_mempool_free(comp_dev->streampool);
    comp_dev->streampool = NULL;
  }
}

int qat_comp_dev_config(struct rte_compressdev *dev,
                        struct rte_compressdev_config *config) {
  struct qat_comp_dev_private *comp_dev = dev->data->dev_private;
  int ret = 0;

  if (config->max_nb_priv_xforms) {
    comp_dev->xformpool = qat_comp_create_xform_pool(
  ...
  if (config->max_nb_streams) {
    comp_dev->streampool = qat_comp_create_stream_pool(
        comp_dev, config->socket_id, config->max_nb_streams);
...
int qat_comp_dev_start(struct rte_compressdev *dev __rte_unused) { return 0; }

void qat_comp_dev_stop(struct rte_compressdev *dev __rte_unused) {}

int qat_comp_dev_close(struct rte_compressdev *dev) {
  int i;
  int ret = 0;
  struct qat_comp_dev_private *comp_dev = dev->data->dev_private;
...
void qat_comp_dev_info_get(struct rte_compressdev *dev,
  ...
  if (info != NULL) {
    info->max_nb_queue_pairs =
        qat_qps_per_service(qat_dev, QAT_SERVICE_COMPRESSION);
...
static uint16_t qat_comp_dequeue_burst(void *qp, struct rte_comp_op **ops,
                                       uint16_t nb_ops) {
  return qat_dequeue_op_burst(qp, (void **)ops, qat_comp_process_response,
                              nb_ops);
}

static uint16_t qat_comp_pmd_dequeue_first_op_burst(void *qp,
  ...
  if (ret) {
    if ((*ops)->debug_status == (uint64_t)ERR_CODE_QAT_COMP_WRONG_FW) {
      ...
      tmp_qp->qat_dev->comp_dev->compressdev->dev_ops = &compress_qat_dummy_ops;
      QAT_LOG(ERR, "This QAT hardware doesn't support compression operation");
...
static const struct rte_driver compdev_qat_driver = {
    .name = qat_comp_drv_name, .alias = qat_comp_drv_name};

int qat_comp_dev_create(struct qat_pci_device *qat_pci_dev,
  ...
  snprintf(name, RTE_COMPRESSDEV_NAME_MAX_LEN, "%s_%s", qat_pci_dev->name,
           "comp");
  ...
  compressdev = rte_compressdev_pmd_create(
      name, &(qat_dev_instance->comp_rte_dev),
      sizeof(struct qat_comp_dev_private), &init_params);
  ...
  compressdev->enqueue_burst =
      (compressdev_enqueue_pkt_burst_t)qat_enqueue_comp_op_burst;
  compressdev->dequeue_burst = qat_comp_pmd_dequeue_first_op_burst;
  compressdev->feature_flags = qat_comp_gen_ops->qat_comp_get_feature_flags();

  if (rte_eal_process_type() != RTE_PROC_PRIMARY)
    return 0;

  snprintf(capa_memz_name, RTE_COMPRESSDEV_NAME_MAX_LEN, "QAT_COMP_CAPA_GEN_%d",
  ...
  capabilities_info =
      qat_comp_get_capa_info(qat_pci_dev->qat_dev_gen, qat_pci_dev);

  if (capabilities_info.data == NULL) {
    QAT_LOG(DEBUG, "QAT gen %d capabilities unknown, default to GEN1",
            qat_pci_dev->qat_dev_gen);
    capabilities_info = qat_comp_get_capa_info(QAT_GEN1, qat_pci_dev);
  }
  ...
  QAT_LOG(DEBUG, "Created QAT COMP device %s as compressdev instance %d", name,
          compressdev->data->dev_id);
  return 0;
}

int qat_comp_dev_destroy(struct qat_pci_device *qat_pci_dev) {
  struct qat_comp_dev_private *comp_dev;
