...
static const struct rte_compressdev_capabilities qat_gen4_comp_capabilities[] =
    {{/* COMPRESSION - deflate */
      .algo = RTE_COMP_ALGO_DEFLATE,
      .comp_feature_flags =
          RTE_COMP_FF_MULTI_PKT_CHECKSUM | RTE_COMP_FF_CRC32_CHECKSUM |
          RTE_COMP_FF_ADLER32_CHECKSUM | RTE_COMP_FF_CRC32_ADLER32_CHECKSUM |
          RTE_COMP_FF_SHAREABLE_PRIV_XFORM | RTE_COMP_FF_HUFFMAN_FIXED |
          RTE_COMP_FF_HUFFMAN_DYNAMIC | RTE_COMP_FF_OOP_SGL_IN_SGL_OUT |
          RTE_COMP_FF_OOP_SGL_IN_LB_OUT | RTE_COMP_FF_OOP_LB_IN_SGL_OUT,
      .window_size = {.min = 15, .max = 15, .increment = 0}},
     {RTE_COMP_ALGO_LIST_END, 0, {0, 0, 0}}};

static int qat_comp_dev_config_gen4(struct rte_compressdev *dev,
...
static uint16_t qat_comp_get_ram_bank_flags_gen4(void) { return 0; }

static int qat_comp_set_slice_cfg_word_gen4(struct qat_comp_xform *qat_xform,
  ...
  if (qat_xform->qat_comp_request_type ==
    ...
    if (xform->compress.algo == RTE_COMP_ALGO_DEFLATE) {
      ...
      if (qat_xform->qat_comp_request_type ==
        ...
        hw_comp_lower_csr.algo = ICP_QAT_HW_COMP_20_HW_COMP_FORMAT_DEFLATE;
        hw_comp_upper_csr.scb_ctrl = ICP_QAT_HW_COMP_20_SCB_CONTROL_DISABLE;
      }

      if (op_type == RTE_COMP_OP_STATEFUL) {
        hw_comp_upper_csr.som_ctrl = ICP_QAT_HW_COMP_20_SOM_CONTROL_REPLAY_MODE;
      }
    } else {
...
static unsigned int qat_comp_get_num_im_bufs_required_gen4(void) {
  return QAT_NUM_INTERM_BUFS_GEN4;
}

RTE_INIT(qat_comp_pmd_gen4_init) {
  qat_comp_gen_dev_ops[QAT_GEN4].compressdev_ops = &qat_comp_ops_gen4;
