...
static int qat_comp_dev_config_gen1(struct rte_compressdev *dev,
                                    struct rte_compressdev_config *config) {
  struct qat_comp_dev_private *comp_dev = dev->data->dev_private;

  if (RTE_PMD_QAT_COMP_IM_BUFFER_SIZE == 0) {
    QAT_LOG(WARNING, "RTE_PMD_QAT_COMP_IM_BUFFER_SIZE = 0 in config file, so"
                     " QAT device can't be used for Dynamic Deflate.");
  } else {
    comp_dev->interm_buff_mz =
        qat_comp_setup_inter_buffers(comp_dev, RTE_PMD_QAT_COMP_IM_BUFFER_SIZE);
...
int qat_comp_set_slice_cfg_word_gen1(struct qat_comp_xform *qat_xform,
  ...
  if (qat_xform->qat_comp_request_type == QAT_COMP_REQUEST_DECOMPRESS) {
    ...
    if (xform->compress.level == RTE_COMP_LEVEL_PMD_DEFAULT)
      ...
      comp_level = ICP_QAT_HW_COMPRESSION_DEPTH_8;
    else if (xform->compress.level >= 4 && xform->compress.level <= 9)
  ...
  comp_slice_cfg_word[0] = ICP_QAT_HW_COMPRESSION_CONFIG_BUILD(
      direction,
...
static unsigned int qat_comp_get_num_im_bufs_required_gen1(void) {
...
RTE_INIT(qat_comp_pmd_gen1_init) {
  qat_comp_gen_dev_ops[QAT_GEN1].compressdev_ops = &qat_comp_ops_gen1;
