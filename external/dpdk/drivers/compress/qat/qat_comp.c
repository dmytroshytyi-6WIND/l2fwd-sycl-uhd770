...
static void qat_comp_fallback_to_fixed(struct icp_qat_fw_comp_req *comp_req) {
  QAT_DP_LOG(DEBUG, "QAT PMD: fallback to fixed compression!");

  comp_req->comn_hdr.service_cmd_id = ICP_QAT_FW_COMP_CMD_STATIC;

  ICP_QAT_FW_COMN_NEXT_ID_SET(&comp_req->comp_cd_ctrl,
                              ICP_QAT_FW_SLICE_DRAM_WR);

  ICP_QAT_FW_COMN_NEXT_ID_SET(&comp_req->u2.xlt_cd_ctrl, ICP_QAT_FW_SLICE_NULL);
  ICP_QAT_FW_COMN_CURR_ID_SET(&comp_req->u2.xlt_cd_ctrl, ICP_QAT_FW_SLICE_NULL);
}

void qat_comp_free_split_op_memzones(struct qat_comp_op_cookie *cookie,
...
qat_comp_allocate_split_op_memzones(struct qat_comp_op_cookie *cookie,
  ...
  cookie->dst_memzones = rte_zmalloc_socket(
      "qat PMD im buf mz pointers",
      (nb_descriptors_needed - 1) * sizeof(const struct rte_memzone *),
      RTE_CACHE_LINE_SIZE, cookie->socket_id);

  if (cookie->dst_memzones == NULL) {
    QAT_DP_LOG(ERR, "QAT PMD: failed to allocate im buf mz pointers");
    return -ENOMEM;
  }

  for (i = 0; i < nb_descriptors_needed - 1; i++) {
    snprintf(dst_memz_name, sizeof(dst_memz_name), "dst_%u_%u_%u_%u_%u",
             cookie->qp->qat_dev->qat_dev_id, txq->hw_bundle_number,
             txq->hw_queue_number, cookie->cookie_index, i);

    cookie->dst_memzones[i] = rte_memzone_reserve_aligned(
        dst_memz_name, RTE_PMD_QAT_COMP_IM_BUFFER_SIZE, cookie->socket_id,
        RTE_MEMZONE_IOVA_CONTIG, RTE_CACHE_LINE_SIZE);

    if (cookie->dst_memzones[i] == NULL) {
      QAT_DP_LOG(ERR, "QAT PMD: failed to allocate dst buffer memzone");
...
int qat_comp_build_request(void *in_op, uint8_t *out_msg, void *op_cookie,
                           enum qat_device_gen qat_dev_gen __rte_unused) {
  struct rte_comp_op *op = in_op;
  struct qat_comp_op_cookie *cookie = (struct qat_comp_op_cookie *)op_cookie;
  struct qat_comp_stream *stream;
  struct qat_comp_xform *qat_xform;
  const uint8_t *tmpl;
  struct icp_qat_fw_comp_req *comp_req = (struct icp_qat_fw_comp_req *)out_msg;
  ...
  if (op->op_type == RTE_COMP_OP_STATEFUL) {
    ...
    if (unlikely(stream->op_in_progress)) {
      QAT_DP_LOG(ERR, "QAT PMD does not support running multiple stateful "
  ...
  if (likely(qat_xform->qat_comp_request_type ==
    ...
    if (unlikely(op->src.length > QAT_FALLBACK_THLD)) {
      ...
      if (qat_xform->checksum_type != RTE_COMP_CHECKSUM_NONE) {
        ...
        if (qat_comp_allocate_split_op_memzones(cookie,
    ...
    comp_req->comp_pars.req_par_flags = ICP_QAT_FW_COMP_REQ_PARAM_FLAGS_BUILD(
        ICP_QAT_FW_COMP_SOP, ICP_QAT_FW_COMP_EOP,
        op->flush_flag == RTE_COMP_FLUSH_FINAL ? ICP_QAT_FW_COMP_BFINAL
                                               : ICP_QAT_FW_COMP_NOT_BFINAL,
        ICP_QAT_FW_COMP_CNV, ICP_QAT_FW_COMP_CNV_RECOVERY);

  } else if (op->op_type == RTE_COMP_OP_STATEFUL) {

    comp_req->comp_pars.req_par_flags = ICP_QAT_FW_COMP_REQ_PARAM_FLAGS_BUILD(
        (stream->start_of_packet) ? ICP_QAT_FW_COMP_SOP
                                  : ICP_QAT_FW_COMP_NOT_SOP,
        (op->flush_flag == RTE_COMP_FLUSH_FULL ||
         op->flush_flag == RTE_COMP_FLUSH_FINAL)
            ? ICP_QAT_FW_COMP_EOP
            : ICP_QAT_FW_COMP_NOT_EOP,
        ICP_QAT_FW_COMP_NOT_BFINAL, ICP_QAT_FW_COMP_CNV,
  ...
  comp_req->comp_pars.out_buffer_sz =
      rte_pktmbuf_pkt_len(op->m_dst) - op->dst.offset;
  ...
  if (op->m_src->next != NULL || op->m_dst->next != NULL) {
    ...
    ret = qat_sgl_fill_array(op->m_src, op->src.offset, cookie->qat_sgl_src_d,
                             op->src.length, cookie->src_nb_elems);
    if (ret) {
    ...
    ret = qat_sgl_fill_array(op->m_dst, op->dst.offset, cookie->qat_sgl_dst_d,
                             comp_req->comp_pars.out_buffer_sz,
                             cookie->dst_nb_elems);
    if (ret) {
    ...
    comp_req->comn_mid.src_data_addr = cookie->qat_sgl_src_phys_addr;
    comp_req->comn_mid.dest_data_addr = cookie->qat_sgl_dst_phys_addr;
    ...
    comp_req->comn_mid.dst_length = comp_req->comp_pars.out_buffer_sz;
  ...
  if (unlikely(rte_pktmbuf_pkt_len(op->m_dst) < QAT_MIN_OUT_BUF_SIZE)) {
     ...
     */
    QAT_DP_LOG(WARNING,
               "QAT destination buffer too small - resend with larger buffer");
...
static inline void qat_comp_mbuf_skip(struct rte_mbuf **mbuf, uint32_t *offset,
                                      uint32_t len) {
...
int qat_comp_build_multiple_requests(void *in_op, struct qat_qp *qp,
  ...
  struct qat_comp_op_cookie *parent_cookie =
      (struct qat_comp_op_cookie *)qp->op_cookies[parent_tail / txq->msg_size];
  struct qat_comp_op_cookie *child_cookie;
  uint16_t dst_data_size = RTE_MIN(RTE_PMD_QAT_COMP_IM_BUFFER_SIZE, 65535);
  ...
  QAT_DP_LOG(DEBUG, "parent op src len %u dst len %u", op->src.length,
             op->m_dst->pkt_len);

  ret = qat_comp_build_request(in_op, out_msg, parent_cookie, qp->qat_dev_gen);
  if (ret != 0) {
  ...
  while (data_to_enqueue) {
    ...
    uint32_t src_data_size = RTE_MIN(data_to_enqueue, QAT_FALLBACK_THLD);
    uint32_t cookie_index;

    /* update params for the next op */
    op->src.offset += QAT_FALLBACK_THLD;
    op->src.length = src_data_size;
    op->flush_flag = (src_data_size == data_to_enqueue) ? op_backup.flush_flag
                                                        : RTE_COMP_FLUSH_FULL;
    ...
    child_cookie = (struct qat_comp_op_cookie *)qp->op_cookies[cookie_index];
    ...
    QAT_DP_LOG(DEBUG, "cookie_index %u, child_cookie %p, comp_req %p",
               cookie_index, child_cookie, comp_req);
    QAT_DP_LOG(DEBUG, "data_to_enqueue %u, num_descriptors_built %d",
               data_to_enqueue, num_descriptors_built);
    QAT_DP_LOG(DEBUG, "child op src len %u dst len %u", op->src.length,
               op->m_dst->pkt_len);

    /* build the request */
    ret = qat_comp_build_request(op, (uint8_t *)comp_req, child_cookie,
                                 qp->qat_dev_gen);
    if (ret < 0) {
  ...
  if (nb_descr != num_descriptors_built)
    QAT_DP_LOG(ERR, "split op. expected %d, built %d", nb_descr,
               num_descriptors_built);
...
qat_comp_response_data_copy(struct qat_comp_op_cookie *cookie,
  ...
  void *op_dst_addr =
      rte_pktmbuf_mtod_offset(sgl_buf, uint8_t *, pc->dst_data_offset);

  /* number of bytes left in the current segment */
  uint32_t left_in_current =
      rte_pktmbuf_data_len(sgl_buf) - pc->dst_data_offset;

  uint32_t prod, sent;

  if (rx_op->produced <= left_in_current) {
    rte_memcpy(op_dst_addr, cookie->dest_buffer, rx_op->produced);
    ...
    rte_memcpy(op_dst_addr, cookie->dest_buffer, left_in_current);
    ...
    while (prod > rte_pktmbuf_data_len(sgl_buf)) {
      ...
      rte_memcpy(op_dst_addr, ((uint8_t *)cookie->dest_buffer) + sent,
    ...
    rte_memcpy(op_dst_addr, ((uint8_t *)cookie->dest_buffer) + sent, prod);
...
int qat_comp_process_response(void **op, uint8_t *resp, void *op_cookie,
                              uint64_t *dequeue_err_count) {
  struct icp_qat_fw_comp_resp *resp_msg = (struct icp_qat_fw_comp_resp *)resp;
  struct qat_comp_op_cookie *cookie = (struct qat_comp_op_cookie *)op_cookie;
  ...
  QAT_DP_LOG(DEBUG, "input counter = %u, output counter = %u",
             comp_resp1->input_byte_counter, comp_resp1->output_byte_counter);

  struct rte_comp_op *rx_op =
      (struct rte_comp_op *)(uintptr_t)(resp_msg->opaque_data);
  struct qat_comp_stream *stream;
  struct qat_comp_xform *qat_xform;
  ...
  if (likely(qat_xform->qat_comp_request_type != QAT_COMP_REQUEST_DECOMPRESS)) {
    if (unlikely(
            ICP_QAT_FW_COMN_HDR_CNV_FLAG_GET(resp_msg->comn_resp.hdr_flags) ==
  ...
  if (err) {
    ...
    int8_t cmp_err_code = (int8_t)resp_msg->comn_resp.comn_error.cmp_err_code;
    int8_t xlat_err_code = (int8_t)resp_msg->comn_resp.comn_error.xlat_err_code;
    ...
    if (cmp_err_code == ERR_CODE_OVERFLOW_ERROR && !xlat_err_code &&
      ...
      struct icp_qat_fw_resp_comp_pars *comp_resp = &resp_msg->comp_resp_pars;
      rx_op->status = RTE_COMP_OP_STATUS_OUT_OF_SPACE_RECOVERABLE;
      rx_op->consumed = comp_resp->input_byte_counter;
      rx_op->produced = comp_resp->output_byte_counter;
      stream->start_of_packet = 0;
    } else if ((cmp_err_code == ERR_CODE_OVERFLOW_ERROR && !xlat_err_code) ||
      ...
      if (comp_resp->input_byte_counter) {
        if ((qat_xform->qat_comp_request_type ==
          ...
          rx_op->status = RTE_COMP_OP_STATUS_OUT_OF_SPACE_TERMINATED;
      } else
        rx_op->status = RTE_COMP_OP_STATUS_OUT_OF_SPACE_TERMINATED;
    } else
      rx_op->status = RTE_COMP_OP_STATUS_ERROR;
    ...
    ++(*dequeue_err_count);
    rx_op->debug_status = *((uint16_t *)(&resp_msg->comn_resp.comn_error));
  } else {
    struct icp_qat_fw_resp_comp_pars *comp_resp =
        (struct icp_qat_fw_resp_comp_pars *)&resp_msg->comp_resp_pars;
    ...
    if (qat_xform->checksum_type != RTE_COMP_CHECKSUM_NONE) {
      ...
      else if (qat_xform->checksum_type == RTE_COMP_CHECKSUM_ADLER32)
    ...
    *op = NULL;
    ...
    if (cookie->nb_children > 0) {
      ...
      if (err) {
        ...
        qat_comp_mbuf_skip(&cookie->dst_data, &cookie->dst_data_offset,
                           rx_op->produced);
      }
    } else {
      ...
      if (pc->nb_child_responses == pc->nb_children) {
        ...
        *op = (void *)rx_op;
        ...
        qat_comp_free_split_op_memzones(pc, pc->nb_children);
...
unsigned int qat_comp_xform_size(void) {
  return RTE_ALIGN_CEIL(sizeof(struct qat_comp_xform), 8);
}

unsigned int qat_comp_stream_size(void) {
...
static int qat_comp_create_templates(struct qat_comp_xform *qat_xform,
  ...
  if (qat_xform->qat_comp_request_type == QAT_COMP_REQUEST_DECOMPRESS)
    req_par_flags = ICP_QAT_FW_COMP_REQ_PARAM_FLAGS_BUILD(
        ICP_QAT_FW_COMP_SOP, ICP_QAT_FW_COMP_EOP, ICP_QAT_FW_COMP_BFINAL,
        ICP_QAT_FW_COMP_CNV, ICP_QAT_FW_COMP_CNV_RECOVERY);
  else
    req_par_flags = ICP_QAT_FW_COMP_REQ_PARAM_FLAGS_BUILD(
        ICP_QAT_FW_COMP_SOP, ICP_QAT_FW_COMP_EOP, ICP_QAT_FW_COMP_BFINAL,
  ...
  if (op_type == RTE_COMP_OP_STATEFUL) {
    comp_req->comn_hdr.serv_specif_flags = ICP_QAT_FW_COMP_FLAGS_BUILD(
        ICP_QAT_FW_COMP_STATEFUL_SESSION, ICP_QAT_FW_COMP_NOT_AUTO_SELECT_BEST,
    ...
    comp_req->comp_cd_ctrl.ram_bank_flags =
        qat_comp_gen_dev_ops[qat_dev_gen].qat_comp_get_ram_bank_flags();

    comp_req->comp_cd_ctrl.ram_banks_addr = stream->inflate_context_phys;
  } else {
    comp_req->comn_hdr.serv_specif_flags = ICP_QAT_FW_COMP_FLAGS_BUILD(
        ICP_QAT_FW_COMP_STATELESS_SESSION, ICP_QAT_FW_COMP_NOT_AUTO_SELECT_BEST,
        ICP_QAT_FW_COMP_NOT_ENH_AUTO_SELECT_BEST,
        ICP_QAT_FW_COMP_NOT_DISABLE_TYPE0_ENH_AUTO_SELECT_BEST,
        ICP_QAT_FW_COMP_ENABLE_SECURE_RAM_USED_AS_INTMD_BUF);
  }
  ...
  res = qat_comp_gen_dev_ops[qat_dev_gen].qat_comp_set_slice_cfg_word(
      qat_xform, xform, op_type, comp_req->cd_pars.sl.comp_slice_cfg_word);
  if (res)
    return res;
  ...
  if (qat_xform->qat_comp_request_type ==
    ...
    ICP_QAT_FW_COMN_NEXT_ID_SET(&comp_req->comp_cd_ctrl,
                                ICP_QAT_FW_SLICE_DRAM_WR);
    ICP_QAT_FW_COMN_CURR_ID_SET(&comp_req->comp_cd_ctrl, ICP_QAT_FW_SLICE_COMP);
  } else if (qat_xform->qat_comp_request_type ==
             QAT_COMP_REQUEST_DYNAMIC_COMP_STATELESS) {

    ICP_QAT_FW_COMN_NEXT_ID_SET(&comp_req->comp_cd_ctrl, ICP_QAT_FW_SLICE_XLAT);
    ICP_QAT_FW_COMN_CURR_ID_SET(&comp_req->comp_cd_ctrl, ICP_QAT_FW_SLICE_COMP);
    ...
    comp_req->u1.xlt_pars.inter_buff_ptr =
        (qat_comp_get_num_im_bufs_required(qat_dev_gen) == 0)
            ? 0
            : interm_buff_mz->iova;
  }
 ...
 */
int qat_comp_private_xform_create(struct rte_compressdev *dev,
                                  const struct rte_comp_xform *xform,
                                  void **private_xform) {
  ...
  struct qat_comp_xform *qat_xform = (struct qat_comp_xform *)*private_xform;

  if (xform->type == RTE_COMP_COMPRESS) {

    if (xform->compress.deflate.huffman == RTE_COMP_HUFFMAN_FIXED ||
        ((xform->compress.deflate.huffman == RTE_COMP_HUFFMAN_DEFAULT) &&
         qat->interm_buff_mz == NULL && im_bufs > 0))
      qat_xform->qat_comp_request_type = QAT_COMP_REQUEST_FIXED_COMP_STATELESS;

    else if ((xform->compress.deflate.huffman == RTE_COMP_HUFFMAN_DYNAMIC ||
              xform->compress.deflate.huffman == RTE_COMP_HUFFMAN_DEFAULT) &&
             (qat->interm_buff_mz != NULL || im_bufs == 0))
  ...
  if (qat_comp_create_templates(qat_xform, qat->interm_buff_mz, xform, NULL,
                                RTE_COMP_OP_STATELESS, qat_dev_gen)) {
 ...
 *  - Returns -EINVAL if input parameters are invalid.
 */
int qat_comp_private_xform_free(struct rte_compressdev *dev __rte_unused,
                                void *private_xform) {
  struct qat_comp_xform *qat_xform = (struct qat_comp_xform *)private_xform;
 ...
 */
static void qat_comp_stream_reset(struct qat_comp_stream *stream) {
  if (stream) {
 ...
 */
int qat_comp_stream_create(struct rte_compressdev *dev,
  ...
  if (qat_comp_create_templates(&ptr->qat_xform, qat->interm_buff_mz, xform,
                                ptr, RTE_COMP_OP_STATEFUL,
 ...
 */
int qat_comp_stream_free(struct rte_compressdev *dev, void *stream) {
  if (stream) {
    struct qat_comp_dev_private *qat = dev->data->dev_private;
