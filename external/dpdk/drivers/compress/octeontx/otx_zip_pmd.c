...
static const struct rte_compressdev_capabilities octtx_zip_pmd_capabilities[] =
    {{
         .algo = RTE_COMP_ALGO_DEFLATE,
         /* Deflate */
         .comp_feature_flags =
             RTE_COMP_FF_HUFFMAN_FIXED | RTE_COMP_FF_HUFFMAN_DYNAMIC,
         /* Non sharable Priv XFORM and Stateless */
         .window_size =
             {
                 .min = 1, .max = 14, .increment = 1
...
int zip_process_op(struct rte_comp_op *op, struct zipvf_qp *qp,
...
static int zip_set_stream_parameters(struct rte_compressdev *dev,
  ...
  ret = rte_mempool_get_bulk(vf->zip_mp, z_stream->bufs, MAX_BUFS_PER_STREAM);
  if (ret < 0)
    return -1;
...
err:
  rte_mempool_put_bulk(vf->zip_mp, (void *)&(z_stream->bufs[0]),
                       MAX_BUFS_PER_STREAM);
...
/** Configure device */
static int zip_pmd_config(struct rte_compressdev *dev,
  ...
  zip_buf_mp = rte_mempool_create(res_pool, nb_streams * MAX_BUFS_PER_STREAM,
                                  ZIP_BUF_SIZE, 0, 0, NULL, NULL, NULL, NULL,
                                  SOCKET_ID_ANY, 0);

  if (zip_buf_mp == NULL) {
    ZIP_PMD_ERR("Failed to create buf mempool octtx_zip_res_pool%u",
                dev->data->dev_id);
    return -1;
  }
...
/** Start device */
static int zip_pmd_start(__rte_unused struct rte_compressdev *dev) { return 0; }

/** Stop device */
static void zip_pmd_stop(__rte_unused struct rte_compressdev *dev) {}

/** Close device */
static int zip_pmd_close(struct rte_compressdev *dev) {
  if (dev == NULL)
    return -1;
...
static void zip_pmd_stats_get(struct rte_compressdev *dev,
...
static void zip_pmd_stats_reset(struct rte_compressdev *dev) {
  int qp_id;
...
/** Get device info */
static void zip_pmd_info_get(struct rte_compressdev *dev,
...
/** Release queue pair */
static int zip_pmd_qp_release(struct rte_compressdev *dev, uint16_t qp_id) {
  struct zipvf_qp *qp = dev->data->queue_pairs[qp_id];
...
/** Setup a queue pair */
static int zip_pmd_qp_setup(struct rte_compressdev *dev, uint16_t qp_id,
  ...
  if (name == NULL)
    return (-ENOMEM);
  snprintf(name, RTE_COMPRESSDEV_NAME_MAX_LEN, "zip_pmd_%u_qp_%u",
           dev->data->dev_id, qp_id);

  /* Allocate the queue pair data structure. */
  qp = rte_zmalloc_socket(name, sizeof(*qp), RTE_CACHE_LINE_SIZE, socket_id);
  if (qp == NULL) {
    rte_free(name);
    return (-ENOMEM);
  }
  ...
  qp->processed_pkts =
      zip_pmd_qp_create_processed_pkts_ring(qp, max_inflight_ops, socket_id);
...
static int zip_pmd_stream_create(struct rte_compressdev *dev,
                                 const struct rte_comp_xform *xform,
                                 void **stream) {
  int ret;
  struct zip_stream *strm = NULL;

  strm = rte_malloc(NULL, sizeof(struct zip_stream), 0);
...
static int zip_pmd_stream_free(struct rte_compressdev *dev, void *stream) {
  struct zip_vf *vf = (struct zip_vf *)(dev->data->dev_private);
  struct zip_stream *z_stream;
...
static uint16_t zip_pmd_enqueue_burst_sync(void *queue_pair,
...
static uint16_t zip_pmd_dequeue_burst_sync(void *queue_pair,
                                           struct rte_comp_op **ops,
                                           uint16_t nb_ops) {
  struct zipvf_qp *qp = queue_pair;
...
static int zip_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  compressdev = rte_compressdev_pmd_create(compressdev_name, &pci_dev->device,
                                           sizeof(struct zip_vf), &init_params);
  if (compressdev == NULL) {
...
static int zip_pci_remove(struct rte_pci_device *pci_dev) {
  struct rte_compressdev *compressdev;
  char compressdev_name[RTE_COMPRESSDEV_NAME_MAX_LEN];
...
static struct rte_pci_id pci_id_octtx_zipvf_table[] = {
    {
        RTE_PCI_DEVICE(PCI_VENDOR_ID_CAVIUM, PCI_DEVICE_ID_OCTEONTX_ZIPVF),
    },
    {.device_id = 0},
};
