...
#define ZIP_MAX_NUM_CMDS                                                       \
  ((ZIP_MAX_CHUNK_SIZE - ZIP_MAX_NCBP_SIZE) / ZIP_CMD_SIZE) /* 63 */

/* size of instruct queue in bytes */
#define ZIP_MAX_CMDQ_SIZE                                                      \
  ((ZIP_MAX_NUM_CMDS * ZIP_CMD_SIZE) + ZIP_MAX_NCBP_SIZE) /* ~8072ull */
...
#define ZIP_PMD_LOG(level, fmt, args...)                                       \
  rte_log(RTE_LOG_##level, octtx_zip_logtype_driver, "%s(): " fmt "\n",        \
          __func__, ##args)

#define ZIP_PMD_INFO(fmt, args...) ZIP_PMD_LOG(INFO, fmt, ##args)
#define ZIP_PMD_ERR(fmt, args...) ZIP_PMD_LOG(ERR, fmt, ##args)
...
static inline void zipvf_prepare_in_buf(struct zip_stream *zstrm,
...
static inline void zipvf_prepare_out_buf(struct zip_stream *zstrm,
                                         struct rte_comp_op *op) {
  uint32_t offset;
...
static inline void zipvf_prepare_cmd_stateless(struct rte_comp_op *op,
                                               struct zip_stream *zstrm) {
  union zip_inst_s *inst = zstrm->inst;
...
static inline void zip_dump_instruction(void *inst) {
  ...
  printf("flush:%d speed:%d cc:%d\n", cmd83->s.sf, cmd83->s.ss, cmd83->s.cc);
  printf("eof:%d bof:%d op:%d dscatter:%d dgather:%d hgather:%d\n", cmd83->s.ef,
         cmd83->s.bf, cmd83->s.op, cmd83->s.ds, cmd83->s.dg, cmd83->s.hg);
...
int zipvf_create(struct rte_compressdev *compressdev);
...
void zipvf_push_command(struct zipvf_qp *qp, union zip_inst_s *zcmd);
