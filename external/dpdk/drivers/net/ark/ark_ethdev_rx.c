...
static uint32_t eth_ark_rx_jumbo(struct ark_rx_queue *queue,
                                 ...
                                 struct rte_mbuf *mbuf0, uint32_t cons_index);
...
/* ************************************************************************* */
static int eth_ark_rx_hw_setup(struct rte_eth_dev *dev,
  ...
  phys_addr_prod_index = queue_base + offsetof(struct ark_rx_queue, prod_index);
...
static inline void eth_ark_rx_update_cons_index(struct ark_rx_queue *queue,
                                                uint32_t cons_index) {
...
/* ************************************************************************* */
int eth_ark_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (rx_conf != NULL && warning1 == 0) {
    warning1 = 1;
    ARK_PMD_LOG(NOTICE, "Arkville ignores rte_eth_rxconf argument.\n");
  }
  ...
  queue = rte_zmalloc_socket("Ark_rxqueue", sizeof(struct ark_rx_queue), 64,
                             socket_id);
  if (queue == 0) {
  ...
  queue->dataroom = rte_pktmbuf_data_room_size(mb_pool) - RTE_PKTMBUF_HEADROOM;
  ...
  queue->reserve_q = rte_zmalloc_socket(
      "Ark_rx_queue mbuf", nb_desc * sizeof(struct rte_mbuf *), 512, socket_id);
  queue->paddress_q = rte_zmalloc_socket(
      "Ark_rx_queue paddr", nb_desc * sizeof(rte_iova_t), 512, socket_id);

  if (queue->reserve_q == 0 || queue->paddress_q == 0) {
    ARK_PMD_LOG(ERR, "Failed to allocate queue memory in %s\n", __func__);
  ...
  ark_udm_configure(queue->udm, RTE_PKTMBUF_HEADROOM, queue->dataroom,
  ...
  if (unlikely(status != 0)) {
    ...
    for (i = 0, mbuf = queue->reserve_q; i < queue->seed_index; ++i, mbuf++) {
...
uint16_t eth_ark_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (prod_index != cons_index) {
    ...
    if (ARK_DEBUG_CORE) { /* debug sanity checks */
      ...
      if ((meta->pkt_len > (1024 * 16)) || (meta->pkt_len == 0)) {
        ARK_PMD_LOG(
            DEBUG,
            "RX: Bad Meta Q: %u"
            " cons: %" PRIU32 " prod: %" PRIU32 " seed_index %" PRIU32 "\n",
            queue->phys_qid, cons_index, queue->prod_index, queue->seed_index);

        ARK_PMD_LOG(DEBUG,
                    "       :  UDM"
                    " prod: %" PRIU32 " len: %u\n",
                    queue->udm->rt_cfg.prod_idx, meta->pkt_len);
        ark_mpu_dump(queue->mpu, "    ", queue->phys_qid);
    ...
    if (unlikely(meta->pkt_len > queue->dataroom))
      cons_index = eth_ark_rx_jumbo(queue, meta, mbuf, cons_index + 1);
    else
      cons_index += 1;
...
static uint32_t eth_ark_rx_jumbo(struct ark_rx_queue *queue,
  ...
  while (remaining != 0) {
    data_len = RTE_MIN(remaining, queue->dataroom);
...
static void eth_ark_rx_queue_drain(struct ark_rx_queue *queue) {
...
static inline int eth_ark_rx_seed_mbufs(struct ark_rx_queue *queue) {
  uint32_t limit =
      (queue->cons_index & ~(ARK_RX_MPU_CHUNK - 1)) + queue->queue_size;
  ...
  if (unlikely(status != 0)) {
    ARK_PMD_LOG(NOTICE,
                ...
                nb, queue->queue_index, queue->seed_index - queue->cons_index);
    return -1;
  }

  if (ARK_DEBUG_CORE) { /* DEBUG */
    while (count != nb) {
      struct rte_mbuf *mbuf_init = queue->reserve_q[seed_m + count];

      memset(mbuf_init->buf_addr, -1, 512);
      *((uint32_t *)mbuf_init->buf_addr) = seed_index + count;
      *(uint16_t *)RTE_PTR_ADD(mbuf_init->buf_addr, 4) = queue->phys_qid;
      count++;
    }
    count = 0;
  } /* DEBUG */
  ...
  case 0:
    while (count != nb) {
      queue->paddress_q[seed_m++] = (*mbufs++)->buf_iova;
      count++;
      /* FALLTHROUGH */
    case 3:
      queue->paddress_q[seed_m++] = (*mbufs++)->buf_iova;
...
void eth_ark_dev_rx_queue_release(void *vqueue) {
  struct ark_rx_queue *queue;
  uint32_t i;
...
static void ark_ethdev_rx_dump(const char *name, struct ark_rx_queue *queue) {
  ...
  ARK_PMD_LOG(DEBUG, ARK_SU32 ARK_SU32 ARK_SU32 ARK_SU32 "\n", "queue_size",
              queue->queue_size, "seed_index", queue->seed_index, "prod_index",
              queue->prod_index, "cons_index", queue->cons_index);
