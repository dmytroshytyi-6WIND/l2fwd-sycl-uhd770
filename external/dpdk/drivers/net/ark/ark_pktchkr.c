...
ark_pkt_chkr_t ark_pktchkr_init(void *addr, int ord, int l2_mode) {
  struct ark_pkt_chkr_inst *inst =
      rte_malloc("ark_pkt_chkr_inst", sizeof(struct ark_pkt_chkr_inst), 0);
  if (inst == NULL) {
    ARK_PMD_LOG(ERR, "Failed to malloc ark_pkt_chkr_inst.\n");
    return inst;
  }
  inst->sregs = (struct ark_pkt_chkr_stat_regs *)addr;
  inst->cregs = (struct ark_pkt_chkr_ctl_regs *)(((uint8_t *)addr) + 0x100);
  inst->ordinal = ord;
  inst->l2_mode = l2_mode;
  return inst;
}

void ark_pktchkr_uninit(ark_pkt_chkr_t handle) { rte_free(handle); }
...
static void ark_pktchkr_set_pkt_ctrl(ark_pkt_chkr_t handle,
                                     uint32_t gen_forever, uint32_t vary_length,
                                     uint32_t incr_payload,
                                     uint32_t incr_first_byte,
                                     uint32_t ins_seq_num, uint32_t ins_udp_hdr,
                                     uint32_t en_resync, uint32_t tuser_err_val,
  ...
  if (!inst->l2_mode)
    ins_udp_hdr = 0;
  r = ((gen_forever << 24) | (vary_length << 16) | (incr_payload << 12) |
       (incr_first_byte << 8) | (ins_time_stamp << 5) | (ins_seq_num << 4) |
       ins_udp_hdr);
...
int ark_pktchkr_wait_done(ark_pkt_chkr_t handle) {
  ...
  while (!ark_pktchkr_stopped(handle) && (wait_cycle > 0)) {
    ...
    wait_cycle--;
    ARK_PMD_LOG(DEBUG,
                "Waiting for packet checker %d's"
...
static struct OPTIONS *options(const char *id) {
  ...
  ARK_PMD_LOG(ERR, "pktchkr: Could not find requested option!, option = %s\n",
              id);
  return NULL;
}
...
void ark_pktchkr_setup(ark_pkt_chkr_t handle) {
  ...
  if (!options("stop")->v.BOOL && options("configure")->v.BOOL) {
    ark_pktchkr_set_payload_byte(handle, options("payload_byte")->v.INT);
    ark_pktchkr_set_src_mac_addr(handle, options("src_mac_addr")->v.INT);
    ark_pktchkr_set_dst_mac_addr(handle, options("dst_mac_addr")->v.LONG);

    ark_pktchkr_set_eth_type(handle, options("eth_type")->v.INT);
    ...
    ark_pktchkr_set_num_pkts(handle, options("num_pkts")->v.INT);
    ark_pktchkr_set_pkt_size_min(handle, options("pkt_size_min")->v.INT);
    ark_pktchkr_set_pkt_size_max(handle, options("pkt_size_max")->v.INT);
    ark_pktchkr_set_pkt_size_incr(handle, options("pkt_size_incr")->v.INT);
    ark_pktchkr_set_pkt_ctrl(
        handle, options("gen_forever")->v.BOOL, options("vary_length")->v.BOOL,
        options("incr_payload")->v.BOOL, options("incr_first_byte")->v.BOOL,
        options("ins_seq_num")->v.INT, options("ins_udp_hdr")->v.BOOL,
        options("en_resync")->v.BOOL, options("tuser_err_val")->v.INT,
