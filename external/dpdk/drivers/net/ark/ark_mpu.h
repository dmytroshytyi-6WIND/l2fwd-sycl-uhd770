...
struct ark_mpu_t {
  ...
  uint8_t reserved0[(ARK_MPU_HW - ARK_MPU_ID) - sizeof(struct ark_mpu_id_t)];
  struct ark_mpu_hw_t hw;
  uint8_t reserved1[(ARK_MPU_CFG - ARK_MPU_HW) - sizeof(struct ark_mpu_hw_t)];
  struct ark_mpu_cfg_t cfg;
  uint8_t
      reserved2[(ARK_MPU_STATS - ARK_MPU_CFG) - sizeof(struct ark_mpu_cfg_t)];
...
uint16_t ark_api_num_queues_per_port(struct ark_mpu_t *mpu, uint16_t ark_ports);
...
static inline void ark_mpu_set_producer(struct ark_mpu_t *mpu, uint32_t idx) {
