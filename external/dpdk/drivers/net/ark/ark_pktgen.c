...
enum OPTYPE { OTINT, OTLONG, OTBOOL, OTSTRING };
...
ark_pkt_gen_t ark_pktgen_init(void *adr, int ord, int l2_mode) {
  struct ark_pkt_gen_inst *inst =
      rte_malloc("ark_pkt_gen_inst_pmd", sizeof(struct ark_pkt_gen_inst), 0);
  if (inst == NULL) {
...
void ark_pktgen_uninit(ark_pkt_gen_t handle) { rte_free(handle); }
...
void ark_pktgen_reset(ark_pkt_gen_t handle) {
  ...
  if (!ark_pktgen_is_running(handle) && !ark_pktgen_paused(handle)) {
    ARK_PMD_LOG(DEBUG,
                "Pktgen %d is not running"
  ...
  if (ark_pktgen_is_running(handle) && !ark_pktgen_paused(handle)) {
    ARK_PMD_LOG(DEBUG, "Pktgen %d is not paused. Pausing first.\n",
                inst->ordinal);
...
void ark_pktgen_wait_done(ark_pkt_gen_t handle) {
  ...
  while (!ark_pktgen_tx_done(handle) && (wait_cycle > 0)) {
    usleep(1000);
    wait_cycle--;
    ARK_PMD_LOG(DEBUG, "Waiting for pktgen %d to finish sending...\n",
                inst->ordinal);
  }
...
static struct OPTIONS *options(const char *id) {
  unsigned int i;
 ...
 ******/
void ark_pktgen_parse(char *args) {
  char *argv, *v;
  const char toks[] = " =\n\t\v\f \r";
...
static int32_t parse_ipv4_string(char const *ip_address) {
  ...
  if (sscanf(ip_address, "%u.%u.%u.%u", &ip[0], &ip[1], &ip[2], &ip[3]) != 4)
    return 0;
  return ip[3] + ip[2] * 0x100 + ip[1] * 0x10000ul + ip[0] * 0x1000000ul;
}

static void ark_pktgen_set_pkt_ctrl(ark_pkt_gen_t handle, uint32_t gen_forever,
                                    uint32_t en_slaved_start,
                                    uint32_t vary_length, uint32_t incr_payload,
                                    uint32_t incr_first_byte,
                                    uint32_t ins_seq_num, uint32_t ins_udp_hdr,
  ...
  r = ((gen_forever << 24) | (en_slaved_start << 20) | (vary_length << 16) |
       (incr_payload << 12) | (incr_first_byte << 8) | (ins_time_stamp << 5) |
       (ins_seq_num << 4) | ins_udp_hdr);
...
void ark_pktgen_setup(ark_pkt_gen_t handle) {
  ...
  if (!options("pause")->v.BOOL &&
      (!options("reset")->v.BOOL && (options("configure")->v.BOOL))) {
    ark_pktgen_set_payload_byte(handle, options("payload_byte")->v.INT);
    ark_pktgen_set_src_mac_addr(handle, options("src_mac_addr")->v.INT);
    ark_pktgen_set_dst_mac_addr(handle, options("dst_mac_addr")->v.LONG);
    ark_pktgen_set_eth_type(handle, options("eth_type")->v.INT);
    ...
    ark_pktgen_set_num_pkts(handle, options("num_pkts")->v.INT);
    ark_pktgen_set_pkt_size_min(handle, options("pkt_size_min")->v.INT);
    ark_pktgen_set_pkt_size_max(handle, options("pkt_size_max")->v.INT);
    ark_pktgen_set_pkt_size_incr(handle, options("pkt_size_incr")->v.INT);
    ark_pktgen_set_pkt_spacing(handle, options("pkt_spacing")->v.INT);
    ark_pktgen_set_start_offset(handle, options("start_offset")->v.INT);
    ark_pktgen_set_pkt_ctrl(
        handle, options("gen_forever")->v.BOOL,
        options("en_slaved_start")->v.BOOL, options("vary_length")->v.BOOL,
        options("incr_payload")->v.BOOL, options("incr_first_byte")->v.BOOL,
        options("ins_seq_num")->v.INT, options("ins_udp_hdr")->v.BOOL,
