...
static int eth_ark_tx_jumbo(struct ark_tx_queue *queue, struct rte_mbuf *mbuf,
...
static inline void ark_tx_hw_queue_stop(struct ark_tx_queue *queue) {
  ark_mpu_stop(queue->mpu);
}

/* ************************************************************************* */
static inline void eth_ark_tx_desc_fill(struct ark_tx_queue *queue,
                                        struct rte_mbuf *mbuf, uint8_t flags,
                                        uint32_t *user_meta,
...
/* ************************************************************************* */
uint16_t eth_ark_xmit_pkts(void *vtxq, struct rte_mbuf **tx_pkts,
                           uint16_t nb_pkts) {
  ...
  for (nb = 0; (nb < nb_pkts) && (prod_index_limit - queue->prod_index) > 0;
       ++nb) {
    mbuf = tx_pkts[nb];

    if (min_pkt_len && unlikely(rte_pktmbuf_pkt_len(mbuf) < min_pkt_len)) {
      /* this packet even if it is small can be split,
       * be sure to add to the end mbuf
       */
      uint16_t to_add = min_pkt_len - rte_pktmbuf_pkt_len(mbuf);
      char *appended = rte_pktmbuf_append(mbuf, to_add);
    ...
    if (tx_user_meta_hook)
      tx_user_meta_hook(mbuf, user_meta, &user_len, queue->ext_user_data);
    if (unlikely(mbuf->nb_segs != 1)) {
      stat = eth_ark_tx_jumbo(queue, mbuf, user_meta, user_len);
      if (unlikely(stat != 0))
        break; /* Queue is full */
    } else {
      eth_ark_tx_desc_fill(queue, mbuf, ARK_DDM_SOP | ARK_DDM_EOP, user_meta,
                           user_len);
    }
  }
  ...
  if (ARK_DEBUG_CORE && nb != nb_pkts) {
    ARK_PMD_LOG(DEBUG,
                "TX: Failure to send:"
                " req: %" PRIU32 " sent: %" PRIU32 " prod: %" PRIU32
                " cons: %" PRIU32 " free: %" PRIU32 "\n",
                nb_pkts, nb, queue->prod_index, queue->cons_index,
                queue->free_index);
...
static int eth_ark_tx_jumbo(struct ark_tx_queue *queue, struct rte_mbuf *mbuf,
  ...
  free_queue_space =
      queue->queue_mask - (queue->prod_index - queue->free_index);
...
int eth_ark_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  queue = rte_zmalloc_socket("Ark_txqueue", sizeof(struct ark_tx_queue), 64,
                             socket_id);
  if (queue == 0) {
    ARK_PMD_LOG(ERR,
  ...
  queue->meta_q = rte_zmalloc_socket(
      "Ark_txqueue meta", nb_desc * sizeof(union ark_tx_meta), 64, socket_id);
  queue->bufs = rte_zmalloc_socket(
      "Ark_txqueue bufs", nb_desc * sizeof(struct rte_mbuf *), 64, socket_id);
...
static int eth_ark_tx_hw_queue_config(struct ark_tx_queue *queue) {
  ...
  cons_index_addr = queue_base + offsetof(struct ark_tx_queue, cons_index);
