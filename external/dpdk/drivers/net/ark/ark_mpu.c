...
int ark_mpu_verify(struct ark_mpu_t *mpu, uint32_t obj_size) {
  ...
  if ((mpu->id.idnum != 0x2055504d) || (mpu->hw.obj_size != obj_size) ||
    ...
    ARK_PMD_LOG(ERR,
                ...
                " \"%c%c%c%c %c%c%c%c\"\n",
                mpu->id.idnum, mpu->id.id[0], mpu->id.id[1], mpu->id.id[2],
                mpu->id.id[3], mpu->id.ver[0], mpu->id.ver[1], mpu->id.ver[2],
                mpu->id.ver[3]);
    ARK_PMD_LOG(ERR,
...
void ark_mpu_stop(struct ark_mpu_t *mpu) { mpu->cfg.command = MPU_CMD_STOP; }

void ark_mpu_start(struct ark_mpu_t *mpu) { mpu->cfg.command = MPU_CMD_RUN; }
...
void ark_mpu_dump(struct ark_mpu_t *mpu, const char *code, uint16_t qid) {
  ...
  ARK_PMD_LOG(DEBUG,
              "MPU: %s state: %d count %d, reserved %d"
              "\n",
              code, mpu->debug.state, mpu->debug.count, mpu->debug.reserved);
}

void ark_mpu_dump_setup(struct ark_mpu_t *mpu, uint16_t q_id) {
  ARK_PMD_LOG(DEBUG, "MPU Setup Q: %u" ARK_SU64X "\n", q_id, "ring_base",
              mpu->cfg.ring_base);
}
