...
static_assert(sizeof(struct ark_rx_meta) == 32,
              "Unexpected struct size ark_rx_meta");

int ark_udm_verify(struct ark_udm_t *udm) {
  if (sizeof(struct ark_udm_t) != ARK_UDM_EXPECT_SIZE) {
    ARK_PMD_LOG(ERR, "ARK: UDM structure looks incorrect %d vs %zd\n",
                ARK_UDM_EXPECT_SIZE, sizeof(struct ark_udm_t));
    return -1;
  }

  if (udm->setup.const0 != ARK_UDM_CONST) {
    ARK_PMD_LOG(ERR, "ARK: UDM module not found as expected 0x%08x\n",
                udm->setup.const0);
    return -1;
  }
  return 0;
...
int ark_udm_stop(struct ark_udm_t *udm, const int wait) {
  int cnt = 0;
...
void ark_udm_configure(struct ark_udm_t *udm, uint32_t headroom,
                       uint32_t dataroom, uint32_t write_interval_ns) {
...
void ark_udm_dump_stats(struct ark_udm_t *udm, const char *msg) {
  ARK_PMD_LOG(INFO,
              "UDM Stats: %s" ARK_SU64 ARK_SU64 ARK_SU64 ARK_SU64 ARK_SU64 "\n",
              msg, "Pkts Received", udm->stats.rx_packet_count,
              "Pkts Finalized", udm->stats.rx_sent_packets, "Pkts Dropped",
              udm->tlp.pkt_drop, "Bytes Count", udm->stats.rx_byte_count,
...
void ark_udm_dump_queue_stats(struct ark_udm_t *udm, const char *msg,
  ...
  ARK_PMD_LOG(
      INFO,
      "UDM Queue %3u Stats: %s" ARK_SU64 ARK_SU64 ARK_SU64 ARK_SU64 ARK_SU64
      "\n",
      qid, msg, "Pkts Received", udm->qstats.q_packet_count, "Pkts Finalized",
      udm->qstats.q_ff_packet_count, "Pkts Dropped", udm->qstats.q_pkt_drop,
      "Bytes Count", udm->qstats.q_byte_count, "MBuf Count",
      udm->qstats.q_mbuf_count);
}
...
void ark_udm_dump_setup(struct ark_udm_t *udm, uint16_t q_id) {
  ARK_PMD_LOG(DEBUG, "UDM Setup Q: %u" ARK_SU64X ARK_SU32 "\n", q_id,
              "hw_prod_addr", udm->rt_cfg.hw_prod_addr, "prod_idx",
              udm->rt_cfg.prod_idx);
}
...
void ark_udm_dump_perf(struct ark_udm_t *udm, const char *msg) {
  ...
  ARK_PMD_LOG(
      INFO,
      "UDM Performance %s" ARK_SU32 ARK_SU32 ARK_SU32 ARK_SU32 ARK_SU32 ARK_SU32
      "\n",
      msg, "PCI Empty", bp->pci_empty, "PCI Q1", bp->pci_q1, "PCI Q2",
      bp->pci_q2, "PCI Q3", bp->pci_q3, "PCI Q4", bp->pci_q4, "PCI Full",
      bp->pci_full);
}
