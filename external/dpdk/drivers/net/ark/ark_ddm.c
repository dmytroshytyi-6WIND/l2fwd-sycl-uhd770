...
static_assert(sizeof(union ark_tx_meta) == 8,
              "Unexpected struct size ark_tx_meta");
...
int ark_ddm_verify(struct ark_ddm_t *ddm) {
  ...
  if (hw_const == ARK_DDM_CONST1) {
    ARK_PMD_LOG(ERR, "ARK: DDM module is version 1, "
                     "PMD expects version 2\n");
    return -1;
  }

  if (hw_const == ARK_DDM_CONST2) {
    ARK_PMD_LOG(ERR, "ARK: DDM module is version 2, "
                     "PMD expects version 3\n");
    return -1;
  }
  ARK_PMD_LOG(ERR, "ARK: DDM module not found as expected 0x%08x\n",
              ddm->cfg.const0);
  return -1;
}

void ark_ddm_start(struct ark_ddm_t *ddm) { ddm->cfg.command = 1; }
...
void ark_ddm_dump(struct ark_ddm_t *ddm, const char *msg) {
  ARK_PMD_LOG(DEBUG, "%s Stopped: %d\n", msg, ark_ddm_is_stopped(ddm));
}

void ark_ddm_dump_stats(struct ark_ddm_t *ddm, const char *msg) {
  struct ark_ddm_stats_t *stats = &ddm->stats;

  ARK_PMD_LOG(INFO, "DDM Stats: %s" ARK_SU64 ARK_SU64 ARK_SU64 "\n", msg,
              "Bytes:", stats->tx_byte_count, "Packets:", stats->tx_pkt_count,
