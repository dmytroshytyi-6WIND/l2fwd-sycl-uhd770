...
static const char *const valid_arguments[] = {ARK_PKTGEN_ARG, ARK_PKTCHKR_ARG,
                                              ARK_PKTDIR_ARG, NULL};
...
static const struct ark_dev_caps ark_device_caps[] = {
...
static int eth_ark_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
...
static int eth_ark_pci_remove(struct rte_pci_device *pci_dev) {
...
static int check_for_ext(struct ark_adapter *ark) {
  int found = 0;
  ...
  ark->user_ext.dev_init = (void *(*)(struct rte_eth_dev *, void *, int))dlsym(
      ark->d_handle, "rte_pmd_ark_dev_init");
  ARK_PMD_LOG(DEBUG, "device ext init pointer = %p\n", ark->user_ext.dev_init);
  ark->user_ext.dev_get_port_count =
      (int (*)(struct rte_eth_dev *, void *))dlsym(
          ark->d_handle, "rte_pmd_ark_dev_get_port_count");
  ark->user_ext.dev_uninit = (void (*)(struct rte_eth_dev *, void *))dlsym(
      ark->d_handle, "rte_pmd_ark_dev_uninit");
  ark->user_ext.dev_configure = (int (*)(struct rte_eth_dev *, void *))dlsym(
      ark->d_handle, "rte_pmd_ark_dev_configure");
  ark->user_ext.dev_start = (int (*)(struct rte_eth_dev *, void *))dlsym(
      ark->d_handle, "rte_pmd_ark_dev_start");
  ark->user_ext.dev_stop = (void (*)(struct rte_eth_dev *, void *))dlsym(
      ark->d_handle, "rte_pmd_ark_dev_stop");
  ark->user_ext.dev_close = (void (*)(struct rte_eth_dev *, void *))dlsym(
      ark->d_handle, "rte_pmd_ark_dev_close");
  ark->user_ext.link_update = (int (*)(struct rte_eth_dev *, int, void *))dlsym(
      ark->d_handle, "rte_pmd_ark_link_update");
  ark->user_ext.dev_set_link_up = (int (*)(struct rte_eth_dev *, void *))dlsym(
      ark->d_handle, "rte_pmd_ark_dev_set_link_up");
  ark->user_ext.dev_set_link_down =
      (int (*)(struct rte_eth_dev *, void *))dlsym(
          ark->d_handle, "rte_pmd_ark_dev_set_link_down");
  ark->user_ext.stats_get =
      (int (*)(struct rte_eth_dev *, struct rte_eth_stats *, void *))dlsym(
          ark->d_handle, "rte_pmd_ark_stats_get");
  ark->user_ext.stats_reset = (void (*)(struct rte_eth_dev *, void *))dlsym(
      ark->d_handle, "rte_pmd_ark_stats_reset");
  ark->user_ext.mac_addr_add =
      (void (*)(struct rte_eth_dev *, struct rte_ether_addr *, uint32_t,
                uint32_t, void *))dlsym(ark->d_handle,
                                        "rte_pmd_ark_mac_addr_add");
  ark->user_ext.mac_addr_remove =
      (void (*)(struct rte_eth_dev *, uint32_t, void *))dlsym(
          ark->d_handle, "rte_pmd_ark_mac_addr_remove");
  ark->user_ext.mac_addr_set =
      (void (*)(struct rte_eth_dev *, struct rte_ether_addr *, void *))dlsym(
          ark->d_handle, "rte_pmd_ark_mac_addr_set");
  ark->user_ext.set_mtu =
      (int (*)(struct rte_eth_dev *, uint16_t, void *))dlsym(
          ark->d_handle, "rte_pmd_ark_set_mtu");
  ark->user_ext.rx_user_meta_hook = (rx_user_meta_hook_fn)dlsym(
      ark->d_handle, "rte_pmd_ark_rx_user_meta_hook");
  ark->user_ext.tx_user_meta_hook = (tx_user_meta_hook_fn)dlsym(
      ark->d_handle, "rte_pmd_ark_tx_user_meta_hook");
...
static int eth_ark_dev_init(struct rte_eth_dev *dev) {
  ...
  if (!dev->data->mac_addrs) {
    ARK_PMD_LOG(ERR, "Failed to allocated memory for storing mac address");
  }
  ...
  if (ark->user_ext.dev_init) {
    ...
    if (!ark->user_data[dev->data->port_id]) {
      ARK_PMD_LOG(WARNING, "Failed to initialize PMD extension!"
  ...
  if (ark->user_ext.dev_get_port_count)
    port_count = ark->user_ext.dev_get_port_count(
        dev, ark->user_data[dev->data->port_id]);
  ...
  for (p = 0; p < port_count; p++) {
    ...
    if (!eth_dev) {
      ARK_PMD_LOG(ERR, "Could not allocate eth_dev for port %d\n", p);
      goto error;
    }
    ...
    if (!eth_dev->data->mac_addrs) {
      ARK_PMD_LOG(ERR, "Memory allocation for MAC failed!"
                       " Exiting.\n");
      goto error;
    }
...
static int eth_ark_dev_uninit(struct rte_eth_dev *dev) {
  struct ark_adapter *ark = dev->data->dev_private;
  ...
  if (ark->user_ext.dev_uninit)
    ark->user_ext.dev_uninit(dev, ark->user_data[dev->data->port_id]);
...
static int eth_ark_dev_configure(struct rte_eth_dev *dev) {
  ...
  if (ark->user_ext.dev_configure)
    return ark->user_ext.dev_configure(dev, ark->user_data[dev->data->port_id]);
  return 0;
}
...
static int eth_ark_dev_start(struct rte_eth_dev *dev) {
  ...
  if (ark->start_pg && !ark->pg_running) {
     ...
     */
    char tname[32];
    snprintf(tname, sizeof(tname), "ark-delay-pg-%d", dev->data->port_id);

    if (rte_ctrl_thread_create(&thread, tname, NULL, ark_pktgen_delay_start,
                               ark->pg)) {
  ...
  if (ark->user_ext.dev_start)
    ark->user_ext.dev_start(dev, ark->user_data[dev->data->port_id]);
...
static int eth_ark_dev_stop(struct rte_eth_dev *dev) {
  ...
  if (ark->user_ext.dev_stop)
    ark->user_ext.dev_stop(dev, ark->user_data[dev->data->port_id]);
...
static int eth_ark_dev_close(struct rte_eth_dev *dev) {
  ...
  if (ark->user_ext.dev_close)
    ark->user_ext.dev_close(dev, ark->user_data[dev->data->port_id]);
...
static int eth_ark_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_desc_lim =
      (struct rte_eth_desc_lim){.nb_max = ARK_RX_MAX_QUEUE,
                                .nb_min = ARK_RX_MIN_QUEUE,
                                .nb_align = ARK_RX_MIN_QUEUE}; /* power of 2 */

  dev_info->tx_desc_lim =
      (struct rte_eth_desc_lim){.nb_max = ARK_TX_MAX_QUEUE,
...
static int eth_ark_dev_stats_reset(struct rte_eth_dev *dev) {
  ...
  if (ark->user_ext.stats_reset)
    ark->user_ext.stats_reset(dev, ark->user_data[dev->data->port_id]);
...
static int eth_ark_macaddr_add(struct rte_eth_dev *dev,
  ...
  if (ark->user_ext.mac_addr_add) {
    ark->user_ext.mac_addr_add(dev, mac_addr, index, pool,
                               ark->user_data[dev->data->port_id]);
    return 0;
  }
  return -ENOTSUP;
...
static int eth_ark_set_mtu(struct rte_eth_dev *dev, uint16_t size) {
  ...
  if (ark->user_ext.set_mtu)
    return ark->user_ext.set_mtu(dev, size, ark->user_data[dev->data->port_id]);

  return -ENOTSUP;
}

static inline int process_pktdir_arg(const char *key, const char *value,
                                     void *extra_args) {
  ARK_PMD_LOG(DEBUG, "key = %s, value = %s\n", key, value);
  struct ark_adapter *ark = (struct ark_adapter *)extra_args;
...
static inline int process_file_args(const char *key, const char *value,
                                    void *extra_args) {
  ARK_PMD_LOG(DEBUG, "key = %s, value = %s\n", key, value);
...
static int eth_ark_check_args(struct ark_adapter *ark, const char *params) {
  ...
  if (rte_kvargs_process(kvlist, ARK_PKTDIR_ARG, &process_pktdir_arg, ark) !=
      0) {
    ARK_PMD_LOG(ERR, "Unable to parse arg %s\n", ARK_PKTDIR_ARG);
    goto free_kvlist;
  }

  if (rte_kvargs_process(kvlist, ARK_PKTGEN_ARG, &process_file_args,
                         ark->pkt_gen_args) != 0) {
    ARK_PMD_LOG(ERR, "Unable to parse arg %s\n", ARK_PKTGEN_ARG);
    goto free_kvlist;
  }

  if (rte_kvargs_process(kvlist, ARK_PKTCHKR_ARG, &process_file_args,
                         ark->pkt_chkr_args) != 0) {
    ARK_PMD_LOG(ERR, "Unable to parse arg %s\n", ARK_PKTCHKR_ARG);
    goto free_kvlist;
  }
...
RTE_PMD_REGISTER_PARAM_STRING(net_ark, ARK_PKTGEN_ARG
                              "=<filename> " ARK_PKTCHKR_ARG
                              "=<filename> " ARK_PKTDIR_ARG "=<bitmap>");
