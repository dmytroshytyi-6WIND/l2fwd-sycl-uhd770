 ...
 *  cache <cache_size>
 */
static void cmd_mempool(struct pmd_internals *softnic, char **tokens,
                        uint32_t n_tokens, char *out, size_t out_size) {
 ...
 */
static void cmd_link(struct pmd_internals *softnic, char **tokens,
                     uint32_t n_tokens, char *out, size_t out_size) {
 ...
 *  size <size>
 */
static void cmd_swq(struct pmd_internals *softnic, char **tokens,
                    uint32_t n_tokens, char *out, size_t out_size) {
 ...
 */
static void cmd_tmgr_shaper_profile(struct pmd_internals *softnic,
                                    char **tokens, uint32_t n_tokens, char *out,
                                    size_t out_size) {
 ...
 */
static void cmd_tmgr_shared_shaper(struct pmd_internals *softnic, char **tokens,
 ...
 */
static void cmd_tmgr_node(struct pmd_internals *softnic, char **tokens,
                          uint32_t n_tokens, char *out, size_t out_size) {
  struct rte_tm_error error;
  ...
  if (n_tokens >= 2 && (strcmp(tokens[0], "shaper") == 0) &&
  ...
  if (n_tokens >= 2 && (strcmp(tokens[0], "shared") == 0) &&
  ...
  if (n_tokens >= 2 && (strcmp(tokens[0], "nonleaf") == 0) &&
    ...
    if (softnic_parser_read_uint32(&np.nonleaf.n_sp_priorities, tokens[2]) !=
        0) {
  ...
  status = rte_tm_node_add(port_id, node_id, parent_node_id, priority, weight,
                           RTE_TM_NODE_LEVEL_ID_ANY, &np, &error);
  if (status != 0) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static uint32_t root_node_id(uint32_t n_spp, uint32_t n_pps) {
...
static uint32_t subport_node_id(uint32_t n_spp, uint32_t n_pps,
                                uint32_t subport_id) {
  uint32_t n_pipes = n_spp * n_pps;
...
static uint32_t pipe_node_id(uint32_t n_spp, uint32_t n_pps,
                             uint32_t subport_id, uint32_t pipe_id) {
  uint32_t n_pipes = n_spp * n_pps;
  uint32_t n_tc = n_pipes * RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE;
  uint32_t n_queues = n_pipes * RTE_SCHED_QUEUES_PER_PIPE;

  return n_queues + n_tc + pipe_id + subport_id * n_pps;
}

static uint32_t tc_node_id(uint32_t n_spp, uint32_t n_pps, uint32_t subport_id,
                           uint32_t pipe_id, uint32_t tc_id) {
  uint32_t n_pipes = n_spp * n_pps;
  uint32_t n_queues = n_pipes * RTE_SCHED_QUEUES_PER_PIPE;

  return n_queues + tc_id +
         (pipe_id + subport_id * n_pps) * RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE;
}

static uint32_t queue_node_id(uint32_t n_spp __rte_unused, uint32_t n_pps,
                              uint32_t subport_id, uint32_t pipe_id,
                              uint32_t tc_id, uint32_t queue_id) {
...
tmgr_hierarchy_default(struct pmd_internals *softnic,
  ...
  uint32_t *shared_shaper_id =
      (uint32_t *)calloc(RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE, sizeof(uint32_t));
  ...
  status = rte_tm_node_add(port_id, root_node_id(n_spp, n_pps),
                           RTE_TM_NODE_ID_NULL, 0, 1, RTE_TM_NODE_LEVEL_ID_ANY,
                           &root_node_params, &error);
  if (status)
    return -1;
  ...
  for (s = 0; s < params->n_spp; s++) {
    ...
    status = rte_tm_node_add(
        port_id, subport_node_id(n_spp, n_pps, s), root_node_id(n_spp, n_pps),
        0, 1, RTE_TM_NODE_LEVEL_ID_ANY, &subport_node_params, &error);
    if (status)
      return -1;
    ...
    for (p = 0; p < params->n_pps; p++) {
      ...
      status =
          rte_tm_node_add(port_id, pipe_node_id(n_spp, n_pps, s, p),
                          subport_node_id(n_spp, n_pps, s), 0, 1,
                          RTE_TM_NODE_LEVEL_ID_ANY, &pipe_node_params, &error);
      if (status)
        return -1;
      ...
      for (t = 0; t < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; t++) {
        ...
        status = rte_tm_node_add(port_id, tc_node_id(n_spp, n_pps, s, p, t),
                                 pipe_node_id(n_spp, n_pps, s, p), t, 1,
                                 RTE_TM_NODE_LEVEL_ID_ANY, &tc_node_params[t],
                                 &error);
        if (status)
          return -1;
        ...
        if (t < RTE_SCHED_TRAFFIC_CLASS_BE) {
          ...
          q = 0;
          status = rte_tm_node_add(
              port_id, queue_node_id(n_spp, n_pps, s, p, t, q),
              tc_node_id(n_spp, n_pps, s, p, t), 0, params->weight.queue[q],
              RTE_TM_NODE_LEVEL_ID_ANY, &queue_node_params, &error);
          if (status)
            return -1;
 ...
 *   queue  <q12> ... <q15>
 */
static void cmd_tmgr_hierarchy_default(struct pmd_internals *softnic,
  ...
  if (softnic_parser_read_uint32(&p.shaper_profile_id.subport, tokens[11]) !=
      0) {
  ...
  if (softnic_parser_read_uint32(&p.shaper_profile_id.tc[10], tokens[35]) !=
      0) {
  ...
  if (softnic_parser_read_uint32(&p.shaper_profile_id.tc[11], tokens[37]) !=
      0) {
  ...
  if (softnic_parser_read_uint32(&p.shaper_profile_id.tc[12], tokens[39]) !=
      0) {
  ...
  if (strcmp(tokens[43], "none") == 0)
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[0], tokens[43]) !=
        0) {
  ...
  if (strcmp(tokens[45], "none") == 0)
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[1], tokens[45]) !=
        0) {
  ...
  if (strcmp(tokens[47], "none") == 0)
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[2], tokens[47]) !=
        0) {
  ...
  if (strcmp(tokens[49], "none") == 0)
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[3], tokens[49]) !=
        0) {
  ...
  if (strcmp(tokens[51], "none") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[4], tokens[51]) !=
        0) {
  ...
  if (strcmp(tokens[53], "none") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[5], tokens[53]) !=
        0) {
  ...
  if (strcmp(tokens[55], "none") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[6], tokens[55]) !=
        0) {
  ...
  if (strcmp(tokens[57], "none") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[7], tokens[57]) !=
        0) {
  ...
  if (strcmp(tokens[59], "none") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[8], tokens[59]) !=
        0) {
  ...
  if (strcmp(tokens[61], "none") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[9], tokens[61]) !=
        0) {
  ...
  if (strcmp(tokens[63], "none") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[10], tokens[63]) !=
        0) {
  ...
  if (strcmp(tokens[65], "none") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[11], tokens[65]) !=
        0) {
  ...
  if (strcmp(tokens[67], "none") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.shared_shaper_id.tc[12], tokens[67]) !=
        0) {
 ...
 */
static void cmd_tmgr_hierarchy_commit(struct pmd_internals *softnic,
                                      char **tokens, uint32_t n_tokens,
                                      char *out, size_t out_size) {
  struct rte_tm_error error;
  uint16_t port_id;
 ...
 * tmgr <tmgr_name>
 */
static void cmd_tmgr(struct pmd_internals *softnic, char **tokens,
                     uint32_t n_tokens, char *out, size_t out_size) {
  char *name;
 ...
 * tap <tap_name>
 */
static void cmd_tap(struct pmd_internals *softnic, char **tokens,
                    uint32_t n_tokens, char *out, size_t out_size) {
  char *name;
...
static void cmd_cryptodev(struct pmd_internals *softnic, char **tokens,
                          uint32_t n_tokens, char *out, size_t out_size) {
  ...
  if (strcmp(tokens[4], "queue")) {
    snprintf(out, out_size, MSG_ARG_NOT_FOUND, "4");
    return;
  }
  ...
  if (softnic_parser_read_uint32(&params.session_pool_size, tokens[8]) < 0) {
    snprintf(out, out_size, MSG_ARG_INVALID, "q");
    return;
  }
 ...
 *  [filter match | mismatch offset <key_offset> mask <key_mask> key <key_value>
 * port <port_id>] [balance offset <key_offset> mask <key_mask> port <port_id0>
 * ... <port_id15>]
 */
static void cmd_port_in_action_profile(struct pmd_internals *softnic,
                                       char **tokens, uint32_t n_tokens,
                                       char *out, size_t out_size) {
  ...
  if (t0 < n_tokens && (strcmp(tokens[t0], "filter") == 0)) {
    ...
    if (softnic_parser_read_uint32(&p.fltr.key_offset, tokens[t0 + 3]) != 0) {
    ...
    if ((softnic_parse_hex_string(tokens[t0 + 5], p.fltr.key_mask, &size) !=
         0) ||
        size != RTE_PORT_IN_ACTION_FLTR_KEY_SIZE) {
    ...
    size = RTE_PORT_IN_ACTION_FLTR_KEY_SIZE;
    if ((softnic_parse_hex_string(tokens[t0 + 7], p.fltr.key, &size) != 0) ||
        size != RTE_PORT_IN_ACTION_FLTR_KEY_SIZE) {
    ...
    if (softnic_parser_read_uint32(&p.fltr.port_id, tokens[t0 + 9]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "port_id");
      return;
    }
  ...
  if (t0 < n_tokens && (strcmp(tokens[t0], "balance") == 0)) {
    ...
    if (softnic_parser_read_uint32(&p.lb.key_offset, tokens[t0 + 2]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "key_offset");
      return;
    }
    ...
    if (softnic_parse_hex_string(tokens[t0 + 4], p.lb.key_mask,
                                 &p.lb.key_size) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "key_mask");
      return;
    }
    ...
    for (i = 0; i < 16; i++)
      if (softnic_parser_read_uint32(&p.lb.port_id[i], tokens[t0 + 6 + i]) !=
          0) {
        snprintf(out, out_size, MSG_ARG_INVALID, "port_id");
        return;
      }
 ...
 *
 */
static void cmd_table_action_profile(struct pmd_internals *softnic,
                                     char **tokens, uint32_t n_tokens,
                                     char *out, size_t out_size) {
  ...
  if (softnic_parser_read_uint32(&p.common.ip_offset, tokens[6]) != 0) {
  ...
  if (t0 < n_tokens && (strcmp(tokens[t0], "balance") == 0)) {
    ...
    if (softnic_parser_read_uint32(&p.lb.out_offset, tokens[t0 + 6]) != 0) {
  ...
  if (t0 < n_tokens && (strcmp(tokens[t0], "meter") == 0)) {
    ...
    if (softnic_parser_read_uint32(&p.mtr.n_tc, tokens[t0 + 3]) != 0) {
  ...
  if (t0 < n_tokens && (strcmp(tokens[t0], "tm") == 0)) {
    ...
    if (softnic_parser_read_uint32(&p.tm.n_subports_per_port, tokens[t0 + 2]) !=
        0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_subports_per_port");
      return;
    }
    ...
    if (softnic_parser_read_uint32(&p.tm.n_pipes_per_subport, tokens[t0 + 4]) !=
        0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_pipes_per_subport");
      return;
    }
  ...
  if (t0 < n_tokens && (strcmp(tokens[t0], "sym_crypto") == 0)) {
    ...
    if (softnic_parser_read_uint32(&p.sym_crypto.op_offset, tokens[t0 + 4]) !=
        0) {
      snprintf(out, out_size, MSG_ARG_INVALID,
               "table action profile sym_crypto");
      return;
 ...
 */
static void cmd_pipeline(struct pmd_internals *softnic, char **tokens,
                         uint32_t n_tokens, char *out, size_t out_size) {
  ...
  if (softnic_parser_read_uint32(&p.timer_period_ms, tokens[3]) != 0) {
  ...
  if (softnic_parser_read_uint32(&p.offset_port_id, tokens[5]) != 0) {
 ...
 *  [disabled]
 */
static void cmd_pipeline_port_in(struct pmd_internals *softnic, char **tokens,
                                 uint32_t n_tokens, char *out,
                                 size_t out_size) {
  ...
  if (strcmp(tokens[t0], "link") == 0) {
    ...
    if (softnic_parser_read_uint16(&p.rxq.queue_id, tokens[t0 + 3]) != 0) {
    ...
    if (softnic_parser_read_uint32(&p.tap.mtu, tokens[t0 + 5]) != 0) {
    ...
    if (softnic_parser_read_uint32(&p.source.n_bytes_per_pkt, tokens[t0 + 6]) !=
        0) {
  ...
  status = softnic_pipeline_port_in_create(softnic, pipeline_name, &p, enabled);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
 ...
 */
static void cmd_pipeline_port_out(struct pmd_internals *softnic, char **tokens,
                                  uint32_t n_tokens, char *out,
                                  size_t out_size) {
  ...
  if (strcmp(tokens[6], "link") == 0) {
    ...
    if (softnic_parser_read_uint16(&p.txq.queue_id, tokens[9]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "queue_id");
      return;
    }
    ...
    if (n_tokens == 7) {
      ...
      if (softnic_parser_read_uint32(&p.sink.max_n_pkts, tokens[10]) != 0) {
    ...
    if (softnic_parser_read_uint16(&p.cryptodev.queue_id, tokens[9]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "queue_id");
      return;
    }
    ...
    if (softnic_parser_read_uint32(&p.cryptodev.op_offset, tokens[11]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "queue_id");
      return;
    }
  } else {
 ...
 */
static void cmd_pipeline_table(struct pmd_internals *softnic, char **tokens,
                               uint32_t n_tokens, char *out, size_t out_size) {
  ...
  if (strcmp(tokens[t0], "acl") == 0) {
    ...
    if (softnic_parser_read_uint32(&p.match.acl.n_rules, tokens[t0 + 5]) != 0) {
    ...
    if (softnic_parser_read_uint32(&p.match.array.key_offset, tokens[t0 + 2]) !=
        0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "key_offset");
      return;
    }
    ...
    if (softnic_parser_read_uint32(&p.match.array.n_keys, tokens[t0 + 4]) !=
        0) {
    ...
    if ((softnic_parser_read_uint32(&p.match.hash.key_size, tokens[t0 + 3]) !=
         0) ||
    ...
    if ((softnic_parse_hex_string(tokens[t0 + 5], p.match.hash.key_mask,
                                  &key_mask_size) != 0) ||
    ...
    if (softnic_parser_read_uint32(&p.match.hash.key_offset, tokens[t0 + 7]) !=
        0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "key_offset");
      return;
    }
    ...
    if (softnic_parser_read_uint32(&p.match.hash.n_buckets, tokens[t0 + 9]) !=
        0) {
    ...
    if (softnic_parser_read_uint32(&p.match.hash.n_keys, tokens[t0 + 11]) !=
        0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_keys");
      return;
    }
    ...
    if (softnic_parser_read_uint32(&p.match.lpm.key_offset, tokens[t0 + 3]) !=
        0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "key_offset");
      return;
    }
    ...
    if (softnic_parser_read_uint32(&p.match.lpm.n_rules, tokens[t0 + 5]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_rules");
      return;
    }
 ...
 */
static void cmd_pipeline_port_in_table(struct pmd_internals *softnic,
                                       char **tokens, uint32_t n_tokens,
                                       char *out, size_t out_size) {
  char *pipeline_name;
  ...
  status = softnic_pipeline_port_in_connect_to_table(softnic, pipeline_name,
                                                     port_id, table_id);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
...
static void cmd_pipeline_port_in_stats(struct pmd_internals *softnic,
                                       char **tokens, uint32_t n_tokens,
                                       char *out, size_t out_size) {
  ...
  status = softnic_pipeline_port_in_stats_read(softnic, pipeline_name, port_id,
                                               &stats, clear);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
 ...
 */
static void cmd_softnic_pipeline_port_in_enable(struct pmd_internals *softnic,
                                                char **tokens,
                                                uint32_t n_tokens, char *out,
                                                size_t out_size) {
  char *pipeline_name;
 ...
 */
static void cmd_softnic_pipeline_port_in_disable(struct pmd_internals *softnic,
                                                 char **tokens,
                                                 uint32_t n_tokens, char *out,
                                                 size_t out_size) {
  char *pipeline_name;
...
static void cmd_pipeline_port_out_stats(struct pmd_internals *softnic,
                                        char **tokens, uint32_t n_tokens,
                                        char *out, size_t out_size) {
  ...
  status = softnic_pipeline_port_out_stats_read(softnic, pipeline_name, port_id,
                                                &stats, clear);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
...
static void cmd_pipeline_table_stats(struct pmd_internals *softnic,
                                     char **tokens, uint32_t n_tokens,
                                     char *out, size_t out_size) {
  ...
  status = softnic_pipeline_table_stats_read(softnic, pipeline_name, table_id,
                                             &stats, clear);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
...
static uint32_t parse_match(char **tokens, uint32_t n_tokens, char *out,
  ...
  if (strcmp(tokens[1], "acl") == 0) {
    ...
    if (softnic_parser_read_uint32(&m->match.acl.priority, tokens[3]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "priority");
      return 0;
    }
    ...
    if (softnic_parser_read_uint32(&m->match.acl.sa_depth, tokens[6]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "sa_depth");
      return 0;
    }

    if (softnic_parser_read_uint32(&m->match.acl.da_depth, tokens[8]) != 0) {
  ...
  if (strcmp(tokens[1], "hash") == 0) {
    ...
    if (strcmp(tokens[2], "raw") == 0) {
      ...
      if (softnic_parse_hex_string(tokens[3], m->match.hash.key, &key_size) !=
          0) {
...
static uint32_t parse_table_action_fwd(char **tokens, uint32_t n_tokens,
  ...
  if (n_tokens && (strcmp(tokens[0], "port") == 0)) {
    ...
    if (n_tokens < 2 || softnic_parser_read_uint32(&id, tokens[1]))
      return 0;
...
static uint32_t
parse_table_action_balance(char **tokens, uint32_t n_tokens,
                           struct softnic_table_rule_action *a) {
  uint32_t i;
...
static uint32_t parse_table_action_tm(char **tokens, uint32_t n_tokens,
  ...
  if (n_tokens < 5 || strcmp(tokens[0], "tm") || strcmp(tokens[1], "subport") ||
...
static uint32_t parse_table_action_encap(char **tokens, uint32_t n_tokens,
  ...
  if (n_tokens && (strcmp(tokens[0], "vlan") == 0)) {
    ...
    if (n_tokens < 6 ||
        ...
        softnic_parser_read_uint32(&pcp, tokens[3]) || pcp > 0x7 ||
        softnic_parser_read_uint32(&dei, tokens[4]) || dei > 0x1 ||
        softnic_parser_read_uint32(&vid, tokens[5]) || vid > 0xFFF)
      return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "qinq") == 0)) {
    ...
    if (n_tokens < 9 ||
        ...
        softnic_parser_read_uint32(&svlan_pcp, tokens[3]) || svlan_pcp > 0x7 ||
        softnic_parser_read_uint32(&svlan_dei, tokens[4]) || svlan_dei > 0x1 ||
        softnic_parser_read_uint32(&svlan_vid, tokens[5]) ||
        svlan_vid > 0xFFF ||
        softnic_parser_read_uint32(&cvlan_pcp, tokens[6]) || cvlan_pcp > 0x7 ||
        softnic_parser_read_uint32(&cvlan_dei, tokens[7]) || cvlan_dei > 0x1 ||
        softnic_parser_read_uint32(&cvlan_vid, tokens[8]) || cvlan_vid > 0xFFF)
      return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "qinq_pppoe") == 0)) {
    ...
    if (n_tokens < 10 ||
        softnic_parse_mac_addr(tokens[1], &a->encap.qinq_pppoe.ether.da) ||
        softnic_parse_mac_addr(tokens[2], &a->encap.qinq_pppoe.ether.sa) ||
        softnic_parser_read_uint32(&svlan_pcp, tokens[3]) || svlan_pcp > 0x7 ||
        softnic_parser_read_uint32(&svlan_dei, tokens[4]) || svlan_dei > 0x1 ||
        softnic_parser_read_uint32(&svlan_vid, tokens[5]) ||
        svlan_vid > 0xFFF ||
  ...
  if (n_tokens && (strcmp(tokens[0], "mpls") == 0)) {
    ...
    if (softnic_parse_mac_addr(tokens[2], &a->encap.mpls.ether.da) ||
        ...
        softnic_parser_read_uint32(&label, tokens[5]) || label > 0xFFFFF ||
        softnic_parser_read_uint32(&tc, tokens[6]) || tc > 0x7 ||
        softnic_parser_read_uint32(&ttl, tokens[7]) || ttl > 0x3F)
      return 0;
    ...
    if (n_tokens < 4 || softnic_parser_read_uint32(&label, tokens[1]) ||
        label > 0xFFFFF || softnic_parser_read_uint32(&tc, tokens[2]) ||
        tc > 0x7 || softnic_parser_read_uint32(&ttl, tokens[3]) || ttl > 0x3F)
      return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "pppoe") == 0)) {
    if (n_tokens < 4 ||
        ...
        softnic_parser_read_uint16(&a->encap.pppoe.pppoe.session_id, tokens[3]))
      return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "vxlan") == 0)) {
    ...
    if (strcmp(tokens[0], "vlan") == 0) {
      ...
      if ((n_tokens < 4) || softnic_parser_read_uint32(&pcp, tokens[1]) ||
          (pcp > 7) || softnic_parser_read_uint32(&dei, tokens[2]) ||
          (dei > 1) || softnic_parser_read_uint32(&vid, tokens[3]) ||
          (vid > 0xFFF))
        return 0;
    ...
    if (strcmp(tokens[0], "ipv4") == 0) {
      ...
      if ((n_tokens < 5) || softnic_parse_ipv4_addr(tokens[1], &sa) ||
          softnic_parse_ipv4_addr(tokens[2], &da) ||
          softnic_parser_read_uint8(&dscp, tokens[3]) || (dscp > 64) ||
      ...
      if ((n_tokens < 6) || softnic_parse_ipv6_addr(tokens[1], &sa) ||
          softnic_parse_ipv6_addr(tokens[2], &da) ||
          softnic_parser_read_uint32(&flow_label, tokens[3]) ||
          softnic_parser_read_uint8(&dscp, tokens[4]) || (dscp > 64) ||
...
parse_table_action_cipher(struct rte_table_action_sym_crypto_params *p,
  ...
  status = softnic_parse_hex_string(tokens[6], p->cipher_auth.cipher_iv.val,
                                    (uint32_t *)&len);
  if (status < 0)
    goto error_exit;
...
parse_table_action_cipher_auth(struct rte_table_action_sym_crypto_params *p,
  ...
  status =
      softnic_parser_read_uint16(&xform_auth->auth.digest_length, tokens[12]);
  if (status < 0)
    goto error_exit;
...
parse_table_action_aead(struct rte_table_action_sym_crypto_params *p,
  ...
  status =
      softnic_parse_hex_string(tokens[6], p->aead.iv.val, (uint32_t *)&len);
  if (status < 0)
    goto error_exit;
  ...
  status =
      softnic_parse_hex_string(tokens[8], p->aead.aad.val, (uint32_t *)&len);
  if (status < 0)
    goto error_exit;
  ...
  /* digest_size */
  status =
      softnic_parser_read_uint16(&xform_aead->aead.digest_length, tokens[10]);
  if (status < 0)
    goto error_exit;
...
static uint32_t
parse_table_action_sym_crypto(char **tokens, uint32_t n_tokens,
                              struct softnic_table_rule_action *a) {
 ...
 *    action <table_action>
 */
static void cmd_softnic_pipeline_table_rule_add(struct pmd_internals *softnic,
                                                char **tokens,
                                                uint32_t n_tokens, char *out,
                                                size_t out_size) {
  struct softnic_table_rule_match m;
  ...
  status = softnic_pipeline_table_rule_add(softnic, pipeline_name, table_id, &m,
                                           &a, &data);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
...
cmd_softnic_pipeline_table_rule_add_default(struct pmd_internals *softnic,
  ...
  status = softnic_pipeline_table_rule_add_default(softnic, pipeline_name,
                                                   table_id, &action, &data);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
cmd_softnic_pipeline_table_rule_add_bulk(struct pmd_internals *softnic,
  ...
  if ((softnic_parser_read_uint32(&n_rules, tokens[8]) != 0) || n_rules == 0) {
    snprintf(out, out_size, MSG_ARG_INVALID, "n_rules");
    return;
  }
  ...
  status = cli_rule_file_process(file_name, 1024, match, action,
                                 &n_rules_parsed, &line_number, out, out_size);
  if (status) {
  ...
  status = softnic_pipeline_table_rule_add_bulk(
      softnic, pipeline_name, table_id, match, action, data, &n_rules);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    free(data);
    free(action);
...
cmd_softnic_pipeline_table_rule_delete(struct pmd_internals *softnic,
  ...
  status =
      softnic_pipeline_table_rule_delete(softnic, pipeline_name, table_id, &m);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
cmd_softnic_pipeline_table_rule_delete_default(struct pmd_internals *softnic,
  ...
  status = softnic_pipeline_table_rule_delete_default(softnic, pipeline_name,
                                                      table_id);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
 ...
 */
static void cmd_softnic_pipeline_table_rule_stats_read(
    struct pmd_internals *softnic __rte_unused, char **tokens,
    uint32_t n_tokens __rte_unused, char *out, size_t out_size) {
 ...
 */
static void cmd_pipeline_table_meter_profile_add(struct pmd_internals *softnic,
                                                 char **tokens,
                                                 uint32_t n_tokens, char *out,
                                                 size_t out_size) {
  ...
  status = softnic_pipeline_table_mtr_profile_add(
      softnic, pipeline_name, table_id, meter_profile_id, &p);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
cmd_pipeline_table_meter_profile_delete(struct pmd_internals *softnic,
  ...
  status = softnic_pipeline_table_mtr_profile_delete(
      softnic, pipeline_name, table_id, meter_profile_id);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static void cmd_pipeline_table_dscp(struct pmd_internals *softnic,
                                    char **tokens, uint32_t n_tokens, char *out,
                                    size_t out_size) {
  ...
  status = softnic_pipeline_table_dscp_table_update(
      softnic, pipeline_name, table_id, UINT64_MAX, &dscp_table);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
 ...
 */
static void cmd_softnic_pipeline_table_rule_ttl_read(
    struct pmd_internals *softnic __rte_unused, char **tokens,
    uint32_t n_tokens __rte_unused, char *out, size_t out_size) {
  snprintf(out, out_size, MSG_CMD_UNIMPLEM, tokens[0]);
}
 ...
 */
static void cmd_softnic_thread_pipeline_enable(struct pmd_internals *softnic,
                                               char **tokens, uint32_t n_tokens,
                                               char *out, size_t out_size) {
  char *pipeline_name;
  uint32_t thread_id;
 ...
 */
static void cmd_softnic_thread_pipeline_disable(struct pmd_internals *softnic,
                                                char **tokens,
                                                uint32_t n_tokens, char *out,
                                                size_t out_size) {
  char *pipeline_name;
 ...
 *  table <table_id>
 */
static void cmd_softnic_flowapi_map(struct pmd_internals *softnic,
                                    char **tokens, uint32_t n_tokens, char *out,
                                    size_t out_size) {
  char *pipeline_name;
  ...
  status =
      flow_attr_map_set(softnic, group_id, ingress, pipeline_name, table_id);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
void softnic_cli_process(char *in, char *out, size_t out_size, void *arg) {
  ...
  if (strcmp(tokens[0], "pipeline") == 0) {
    ...
    if (n_tokens >= 6 && (strcmp(tokens[2], "port") == 0) &&
      ...
      cmd_pipeline_port_in_table(softnic, tokens, n_tokens, out, out_size);
      return;
    }

    if (n_tokens >= 6 && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "in") == 0) && (strcmp(tokens[5], "stats") == 0)) {
      cmd_pipeline_port_in_stats(softnic, tokens, n_tokens, out, out_size);
      return;
    }

    if (n_tokens >= 6 && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "in") == 0) && (strcmp(tokens[5], "enable") == 0)) {
      cmd_softnic_pipeline_port_in_enable(softnic, tokens, n_tokens, out,
                                          out_size);
      return;
    }
    ...
    if (n_tokens >= 6 && (strcmp(tokens[2], "port") == 0) &&
      ...
      cmd_softnic_pipeline_port_in_disable(softnic, tokens, n_tokens, out,
                                           out_size);
      return;
    }
    ...
    if (n_tokens >= 6 && (strcmp(tokens[2], "port") == 0) &&
      ...
      cmd_pipeline_port_out_stats(softnic, tokens, n_tokens, out, out_size);
      return;
    }

    if (n_tokens >= 5 && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "stats") == 0)) {
      cmd_pipeline_table_stats(softnic, tokens, n_tokens, out, out_size);
      return;
    }
    ...
    if (n_tokens >= 7 && (strcmp(tokens[2], "table") == 0) &&
      ...
      if (n_tokens >= 8 && (strcmp(tokens[7], "default") == 0)) {
        cmd_softnic_pipeline_table_rule_add_default(softnic, tokens, n_tokens,
                                                    out, out_size);
        return;
      }

      cmd_softnic_pipeline_table_rule_add(softnic, tokens, n_tokens, out,
                                          out_size);
      return;
    }
    ...
    if (n_tokens >= 7 && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_softnic_pipeline_table_rule_add_bulk(softnic, tokens, n_tokens, out,
                                               out_size);
      return;
    }
    ...
    if (n_tokens >= 7 && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_softnic_pipeline_table_rule_delete(softnic, tokens, n_tokens, out,
                                             out_size);
      return;
    }
    ...
    if (n_tokens >= 7 && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_softnic_pipeline_table_rule_stats_read(softnic, tokens, n_tokens, out,
                                                 out_size);
      return;
    }
    ...
    if (n_tokens >= 8 && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_pipeline_table_meter_profile_add(softnic, tokens, n_tokens, out,
                                           out_size);
      return;
    }
    ...
    if (n_tokens >= 8 && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_pipeline_table_meter_profile_delete(softnic, tokens, n_tokens, out,
                                              out_size);
      return;
    }
    ...
    if (n_tokens >= 7 && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_pipeline_table_rule_meter_read(softnic, tokens, n_tokens, out,
                                         out_size);
      return;
    }
    ...
    if (n_tokens >= 5 && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_pipeline_table_dscp(softnic, tokens, n_tokens, out, out_size);
      return;
    }
    ...
    if (n_tokens >= 7 && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_softnic_pipeline_table_rule_ttl_read(softnic, tokens, n_tokens, out,
                                               out_size);
      return;
    }
  }
  ...
  if (strcmp(tokens[0], "thread") == 0) {
    if (n_tokens >= 5 && (strcmp(tokens[4], "enable") == 0)) {
      cmd_softnic_thread_pipeline_enable(softnic, tokens, n_tokens, out,
                                         out_size);
      return;
    }

    if (n_tokens >= 5 && (strcmp(tokens[4], "disable") == 0)) {
      cmd_softnic_thread_pipeline_disable(softnic, tokens, n_tokens, out,
                                          out_size);
      return;
    }
  }
  ...
  if (strcmp(tokens[0], "flowapi") == 0) {
    cmd_softnic_flowapi_map(softnic, tokens, n_tokens, out, out_size);
    return;
  }
...
int softnic_cli_script_process(struct pmd_internals *softnic,
                               const char *file_name, size_t msg_in_len_max,
  ...
  for (;;) {
    ...
    softnic_cli_process(msg_in, msg_out, msg_out_len_max, softnic);
...
static int cli_rule_file_process(const char *file_name, size_t line_len_max,
  ...
  for (line_id = 1, rule_id = 0; rule_id < *n_rules; line_id++) {
    ...
    /* Rule match. */
    n_tokens_parsed =
        parse_match(tokens + t0, n_tokens - t0, out, out_size, &m[rule_id]);
    if (n_tokens_parsed == 0) {
      status = -EINVAL;
      break;
    }
