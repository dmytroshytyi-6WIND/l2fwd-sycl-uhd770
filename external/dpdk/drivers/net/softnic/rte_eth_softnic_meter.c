...
static int meter_profile_check(struct rte_eth_dev *dev,
                               uint32_t meter_profile_id,
  ...
  if (profile->alg != RTE_MTR_TRTCM_RFC2698)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_PROFILE,
                              NULL, "Metering alg not supported");

  /* Not support packet mode, just support byte mode. */
  if (profile->packet_mode)
    return -rte_mtr_error_set(error, EINVAL,
                              RTE_MTR_ERROR_TYPE_METER_PROFILE_PACKET_MODE,
                              NULL, "Meter packet mode not supported");

  return 0;
}

/* MTR meter profile add */
static int pmd_mtr_meter_profile_add(struct rte_eth_dev *dev,
                                     uint32_t meter_profile_id,
                                     struct rte_mtr_meter_profile *profile,
                                     struct rte_mtr_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
...
static int pmd_mtr_meter_profile_delete(struct rte_eth_dev *dev,
                                        uint32_t meter_profile_id,
                                        struct rte_mtr_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct softnic_mtr_meter_profile *mp;
...
static int pmd_mtr_meter_policy_add(struct rte_eth_dev *dev,
                                    uint32_t meter_policy_id,
  ...
  if (policy == NULL)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_POLICY,
                              NULL, "Null meter policy invalid");

  /* Meter policy must not exist. */
  mp = softnic_mtr_meter_policy_find(p, meter_policy_id);
  if (mp != NULL)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_POLICY_ID,
  ...
  for (i = 0; i < RTE_COLORS; i++) {
    ...
    if (act->type == RTE_FLOW_ACTION_TYPE_METER_COLOR) {
      ...
      case RTE_COLOR_GREEN:
        mp->policer[i] = RTE_TABLE_ACTION_POLICER_COLOR_GREEN;
        break;
      case RTE_COLOR_YELLOW:
        mp->policer[i] = RTE_TABLE_ACTION_POLICER_COLOR_YELLOW;
        break;
      case RTE_COLOR_RED:
        mp->policer[i] = RTE_TABLE_ACTION_POLICER_COLOR_RED;
        break;
      default:
        break;
      }
...
static int pmd_mtr_meter_policy_delete(struct rte_eth_dev *dev,
                                       uint32_t meter_policy_id,
                                       struct rte_mtr_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct softnic_mtr_meter_policy *mp;
...
static int mtr_check(struct pmd_internals *p, uint32_t mtr_id,
  ...
  if (params == NULL)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_MTR_PARAMS,
                              NULL, "MTR object params null");

  /* Previous meter color not supported */
  if (params->use_prev_mtr_color)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_MTR_PARAMS,
                              NULL, "Previous meter color not supported");

  /* Shared MTR object not supported */
  if (shared)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_SHARED, NULL,
                              "Shared MTR object not supported");
...
/* MTR object create */
static int pmd_mtr_create(struct rte_eth_dev *dev, uint32_t mtr_id,
                          struct rte_mtr_params *params, int shared,
                          struct rte_mtr_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct softnic_mtr_list *ml = &p->mtr.mtrs;
...
/* MTR object destroy */
static int pmd_mtr_destroy(struct rte_eth_dev *dev, uint32_t mtr_id,
                           struct rte_mtr_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct softnic_mtr_list *ml = &p->mtr.mtrs;
  struct softnic_mtr_meter_profile *mp;
  ...
  if (policy == NULL)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_POLICY_ID,
                              NULL, "MTR object meter policy invalid");
...
static int pmd_mtr_meter_profile_update(struct rte_eth_dev *dev,
                                        uint32_t mtr_id,
                                        uint32_t meter_profile_id,
                                        struct rte_mtr_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  ...
  if (m->flow) {
    ...
    if (!softnic_pipeline_table_meter_profile_find(table, meter_profile_id)) {
      ...
      status = softnic_pipeline_table_mtr_profile_add(
          p, m->flow->pipeline->name, table_id, meter_profile_id, &profile);
      if (status)
        return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                                  NULL, "Table meter profile add failed");
    }
    ...
    /* Re-add rule */
    status = softnic_pipeline_table_rule_add(p, m->flow->pipeline->name,
                                             table_id, &m->flow->match, &action,
                                             &m->flow->data);
    if (status)
      return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Pipeline table rule add failed");
...
static int pmd_mtr_meter_dscp_table_update(struct rte_eth_dev *dev,
                                           uint32_t mtr_id,
  ...
  /* Update table */
  status = softnic_pipeline_table_dscp_table_update(p, pipeline->name, table_id,
                                                    UINT64_MAX, &dt);
  if (status)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Table action dscp table update failed");

  return 0;
}

/* MTR object policy update */
static int pmd_mtr_meter_policy_update(struct rte_eth_dev *dev, uint32_t mtr_id,
                                       uint32_t meter_policy_id,
                                       struct rte_mtr_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct softnic_mtr *m;
  ...
  if (m->flow) {
    ...
    /* Re-add the rule */
    status = softnic_pipeline_table_rule_add(p, pipeline->name,
                                             m->flow->table_id, &m->flow->match,
                                             &action, &m->flow->data);
    if (status)
      return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Pipeline table rule re-add failed");

    /* Flow: Update meter action */
    memcpy(&m->flow->action, &action, sizeof(m->flow->action));

    /* Reset the meter stats */
    rte_table_action_meter_read(table->a, m->flow->data, 1, NULL, 1);
  }

  mp_old = softnic_mtr_meter_policy_find(p, m->params.meter_policy_id);
  if (mp_old == NULL)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_POLICY_ID,
                              NULL, "Old meter policy id invalid");
...
#define MTR_STATS_PKTS_DEFAULT                                                 \
  (RTE_MTR_STATS_N_PKTS_GREEN | RTE_MTR_STATS_N_PKTS_YELLOW |                  \
   RTE_MTR_STATS_N_PKTS_RED | RTE_MTR_STATS_N_PKTS_DROPPED)

#define MTR_STATS_BYTES_DEFAULT                                                \
  (RTE_MTR_STATS_N_BYTES_GREEN | RTE_MTR_STATS_N_BYTES_YELLOW |                \
   RTE_MTR_STATS_N_BYTES_RED | RTE_MTR_STATS_N_BYTES_DROPPED)

/* MTR object stats read */
static void mtr_stats_convert(struct pmd_internals *p, struct softnic_mtr *m,
                              struct rte_table_action_mtr_counters_tc *in,
                              struct rte_mtr_stats *out, uint64_t *out_mask) {
  struct softnic_mtr_meter_policy *mp;
  ...
  *out_mask = 0;
  ...
  if (in->n_packets_valid) {
    ...
    for (i = 0; i < RTE_COLORS; i++) {
      if (mp->policer[i] == RTE_TABLE_ACTION_POLICER_COLOR_GREEN)
        out->n_pkts[RTE_COLOR_GREEN] += in->n_packets[i];

      if (mp->policer[i] == RTE_TABLE_ACTION_POLICER_COLOR_YELLOW)
        out->n_pkts[RTE_COLOR_YELLOW] += in->n_packets[i];

      if (mp->policer[i] == RTE_TABLE_ACTION_POLICER_COLOR_RED)
        out->n_pkts[RTE_COLOR_RED] += in->n_packets[i];

      if (mp->policer[i] == RTE_TABLE_ACTION_POLICER_DROP)
...
/* MTR object stats read */
static int pmd_mtr_stats_read(struct rte_eth_dev *dev, uint32_t mtr_id,
  ...
  if (m->flow == NULL) {
    ...
    if (stats_mask)
      *stats_mask = MTR_STATS_PKTS_DEFAULT | MTR_STATS_BYTES_DEFAULT;
  ...
  /* Meter stats read. */
  status =
      rte_table_action_meter_read(table->a, m->flow->data, 1, &counters, clear);
  if (status)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Meter stats read failed");
  ...
  if (stats || stats_mask) {
    ...
    mtr_stats_convert(p, m, &counters.stats[0], &s, &s_mask);
