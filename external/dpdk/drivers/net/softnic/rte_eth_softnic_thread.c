 ...
 */
void softnic_thread_free(struct pmd_internals *softnic) {
  uint32_t i;
...
int softnic_thread_init(struct pmd_internals *softnic) {
  uint32_t i;
  ...
  for (i = 0; i < RTE_MAX_LCORE; i++) {
    ...
    /* MSGQs */
    snprintf(ring_name, sizeof(ring_name), "%s-TH%u-REQ", softnic->params.name,
             i);

    msgq_req = rte_ring_create(ring_name, THREAD_MSGQ_SIZE, cpu_id,
    ...
    snprintf(ring_name, sizeof(ring_name), "%s-TH%u-RSP", softnic->params.name,
             i);

    msgq_rsp = rte_ring_create(ring_name, THREAD_MSGQ_SIZE, cpu_id,
                               RING_F_SP_ENQ | RING_F_SC_DEQ);
    ...
    t_data->timer_period = (rte_get_tsc_hz() * THREAD_TIMER_PERIOD_MS) / 1000;
...
static inline int thread_is_valid(struct pmd_internals *softnic,
                                  uint32_t thread_id) {
...
static int32_t rte_pmd_softnic_run_internal(void *arg);

static inline int thread_sc_service_up(struct pmd_internals *softnic,
                                       uint32_t thread_id) {
...
static inline void thread_sc_service_down(struct pmd_internals *softnic,
                                          uint32_t thread_id) {
  struct softnic_thread *t = &softnic->thread[thread_id];
 ...
 */
static inline int pipeline_is_running(struct pipeline *p) {
  if (p->enabled == 0)
    return 0;
...
static struct thread_msg_req *thread_msg_alloc(void) {
  size_t size =
      RTE_MAX(sizeof(struct thread_msg_req), sizeof(struct thread_msg_rsp));

  return calloc(1, size);
}

static void thread_msg_free(struct thread_msg_rsp *rsp) { free(rsp); }

static struct thread_msg_rsp *
thread_msg_send_recv(struct pmd_internals *softnic, uint32_t thread_id,
...
int softnic_thread_pipeline_enable(struct pmd_internals *softnic,
                                   uint32_t thread_id,
  ...
  if (!thread_is_valid(softnic, thread_id) || (p == NULL) ||
      (p->n_ports_in == 0) || (p->n_ports_out == 0) || (p->n_tables == 0) ||
      p->enabled)
    return -1;
  ...
  for (i = 0; i < p->n_tables; i++)
    req->pipeline_enable.table[i].a = p->table[i].a;
...
int softnic_thread_pipeline_disable(struct pmd_internals *softnic,
                                    uint32_t thread_id,
                                    const char *pipeline_name) {
  struct pipeline *p = softnic_pipeline_find(softnic, pipeline_name);
  struct thread_msg_req *req;
  ...
  if (!thread_is_running(thread_id)) {
    ...
    for (i = 0; i < td->n_pipelines; i++) {
      ...
      if (i < td->n_pipelines - 1) {
        struct rte_pipeline *pipeline_last = td->p[td->n_pipelines - 1];
...
static inline void thread_msg_send(struct rte_ring *msgq_rsp,
                                   struct thread_msg_rsp *rsp) {
  int status;
...
thread_msg_handle_pipeline_enable(struct softnic_thread_data *t,
  ...
  for (i = 0; i < req->pipeline_enable.n_tables; i++)
    p->table_data[i].a = req->pipeline_enable.table[i].a;
...
thread_msg_handle_pipeline_disable(struct softnic_thread_data *t,
  ...
  for (i = 0; i < n_pipelines; i++) {
    ...
    if (i < n_pipelines - 1) {
      struct rte_pipeline *pipeline_last = t->p[n_pipelines - 1];
...
static void thread_msg_handle(struct softnic_thread_data *t) {
  for (;;) {
    struct thread_msg_req *req;
    struct thread_msg_rsp *rsp;
 ...
 * Main thread
 */
static struct pipeline_msg_req *pipeline_msg_alloc(void) {
  size_t size =
      RTE_MAX(sizeof(struct pipeline_msg_req), sizeof(struct pipeline_msg_rsp));

  return calloc(1, size);
}

static void pipeline_msg_free(struct pipeline_msg_rsp *rsp) { free(rsp); }

static struct pipeline_msg_rsp *
pipeline_msg_send_recv(struct pipeline *p, struct pipeline_msg_req *req) {
...
int softnic_pipeline_port_in_stats_read(
    struct pmd_internals *softnic, const char *pipeline_name, uint32_t port_id,
  ...
  /* Check input params */
  if (pipeline_name == NULL || stats == NULL)
    return -1;

  p = softnic_pipeline_find(softnic, pipeline_name);
  if (p == NULL || port_id >= p->n_ports_in)
    return -1;

  if (!pipeline_is_running(p)) {
    status = rte_pipeline_port_in_stats_read(p->p, port_id, stats, clear);
...
int softnic_pipeline_port_in_enable(struct pmd_internals *softnic,
                                    const char *pipeline_name,
                                    uint32_t port_id) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
...
int softnic_pipeline_port_in_disable(struct pmd_internals *softnic,
                                     const char *pipeline_name,
                                     uint32_t port_id) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
...
int softnic_pipeline_port_out_stats_read(
    struct pmd_internals *softnic, const char *pipeline_name, uint32_t port_id,
  ...
  p = softnic_pipeline_find(softnic, pipeline_name);
  if (p == NULL || port_id >= p->n_ports_out)
    return -1;

  if (!pipeline_is_running(p)) {
    status = rte_pipeline_port_out_stats_read(p->p, port_id, stats, clear);
...
int softnic_pipeline_table_stats_read(struct pmd_internals *softnic,
                                      const char *pipeline_name,
                                      uint32_t table_id,
                                      struct rte_pipeline_table_stats *stats,
                                      int clear) {
  ...
  if (!pipeline_is_running(p)) {
    status = rte_pipeline_table_stats_read(p->p, table_id, stats, clear);
...
static int match_check(struct softnic_table_rule_match *match,
                       struct pipeline *p, uint32_t table_id) {
  struct softnic_table *table;

  if (match == NULL || p == NULL || table_id >= p->n_tables)
    return -1;
  ...
  case TABLE_ACL: {
    ...
    if (r->ip_version) {
      if (r->sa_depth > 32 || r->da_depth > 32)
        return -1;
    } else {
      if (r->sa_depth > 128 || r->da_depth > 128)
        return -1;
    }
    return 0;
  }
...
static int action_check(struct softnic_table_rule_action *action,
                        struct pipeline *p, uint32_t table_id) {
  struct softnic_table_action_profile *ap;

  if (action == NULL || p == NULL || table_id >= p->n_tables)
    return -1;
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_TM)) {
    uint32_t n_subports_per_port = ap->params.tm.n_subports_per_port;
    uint32_t n_pipes_per_subport = ap->params.tm.n_pipes_per_subport;
    uint32_t subport_id = action->tm.subport_id;
    uint32_t pipe_id = action->tm.pipe_id;

    if (subport_id >= n_subports_per_port || pipe_id >= n_pipes_per_subport)
      return -1;
  }
...
static int action_default_check(struct softnic_table_rule_action *action,
                                struct pipeline *p, uint32_t table_id) {
  if (action == NULL || action->action_mask != (1LLU << RTE_TABLE_ACTION_FWD) ||
      p == NULL || table_id >= p->n_tables)
    return -1;
...
static int match_convert(struct softnic_table_rule_match *mh,
                         union table_rule_match_low_level *ml, int add);

static int action_convert(struct rte_table_action *a,
                          struct softnic_table_rule_action *action,
                          struct rte_pipeline_table_entry *data);

int softnic_pipeline_table_rule_add(struct pmd_internals *softnic,
                                    const char *pipeline_name,
                                    uint32_t table_id,
                                    struct softnic_table_rule_match *match,
                                    struct softnic_table_rule_action *action,
  ...
  /* Check input params */
  if (pipeline_name == NULL || match == NULL || action == NULL || data == NULL)
    return -1;

  p = softnic_pipeline_find(softnic, pipeline_name);
  if (p == NULL || table_id >= p->n_tables || match_check(match, p, table_id) ||
  ...
  if (!pipeline_is_running(p)) {
    ...
    status = rte_pipeline_table_entry_add(p->p, table_id, &match_ll, data_in,
                                          &key_found, &data_out);
    if (status) {
      free(buffer);
      return -1;
...
int softnic_pipeline_table_rule_add_default(
    struct pmd_internals *softnic, const char *pipeline_name, uint32_t table_id,
    struct softnic_table_rule_action *action, void **data) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  int status;
  ...
  /* Check input params */
  if (pipeline_name == NULL || action == NULL || data == NULL)
    return -1;
  ...
  if (!pipeline_is_running(p)) {
    ...
    status = rte_pipeline_table_default_entry_add(p->p, table_id, data_in,
                                                  &data_out);
    if (status) {
      free(buffer);
      return -1;
...
int softnic_pipeline_table_rule_add_bulk(
    struct pmd_internals *softnic, const char *pipeline_name, uint32_t table_id,
    struct softnic_table_rule_match *match,
    struct softnic_table_rule_action *action, void **data, uint32_t *n_rules) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  uint32_t i;
  ...
  /* Check input params */
  if (pipeline_name == NULL || match == NULL || action == NULL ||
      data == NULL || n_rules == NULL || (*n_rules == 0))
    return -1;
  ...
  for (i = 0; i < *n_rules; i++)
    if (match_check(match, p, table_id) || action_check(action, p, table_id))
      return -1;
  ...
  if (!pipeline_is_running(p)) {
    ...
    struct rte_pipeline_table_entry **entries_ptr =
        (struct rte_pipeline_table_entry **)data;
    uint32_t bulk = (p->table[table_id].params.match_type == TABLE_ACL) ? 1 : 0;
    int *found;
    ...
    action_ll_ptr = calloc(*n_rules, sizeof(struct rte_pipeline_table_entry *));
    found = calloc(*n_rules, sizeof(int));

    if (match_ll == NULL || action_ll == NULL || match_ll_ptr == NULL ||
        action_ll_ptr == NULL || found == NULL)
      goto fail;
    ...
    /* Add rule (match, action) to table */
    if (bulk) {
      status = rte_pipeline_table_entry_add_bulk(p->p, table_id, match_ll_ptr,
                                                 action_ll_ptr, *n_rules, found,
                                                 entries_ptr);
      if (status)
        *n_rules = 0;
    } else {
      for (i = 0; i < *n_rules; i++) {
        status = rte_pipeline_table_entry_add(p->p, table_id, match_ll_ptr[i],
                                              action_ll_ptr[i], &found[i],
                                              &entries_ptr[i]);
        if (status) {
          *n_rules = i;
          break;
...
int softnic_pipeline_table_rule_delete(struct pmd_internals *softnic,
                                       const char *pipeline_name,
                                       uint32_t table_id,
                                       struct softnic_table_rule_match *match) {
  struct pipeline *p;
  ...
  /* Check input params */
  if (pipeline_name == NULL || match == NULL)
    return -1;
  ...
  if (!pipeline_is_running(p)) {
    ...
    status = rte_pipeline_table_entry_delete(p->p, table_id, &match_ll,
                                             &key_found, NULL);
...
int softnic_pipeline_table_rule_delete_default(struct pmd_internals *softnic,
                                               const char *pipeline_name,
                                               uint32_t table_id) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  ...
  if (!pipeline_is_running(p)) {
    status = rte_pipeline_table_default_entry_delete(p->p, table_id, NULL);
...
int softnic_pipeline_table_rule_stats_read(
    struct pmd_internals *softnic, const char *pipeline_name, uint32_t table_id,
    void *data, struct rte_table_action_stats_counters *stats, int clear) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  int status;
  ...
  /* Check input params */
  if (pipeline_name == NULL || data == NULL || stats == NULL)
    return -1;
  ...
  if (!pipeline_is_running(p)) {
    ...
    status = rte_table_action_stats_read(a, data, stats, clear);
...
int softnic_pipeline_table_mtr_profile_add(
    struct pmd_internals *softnic, const char *pipeline_name, uint32_t table_id,
    uint32_t meter_profile_id, struct rte_table_action_meter_profile *profile) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  struct softnic_table *table;
  ...
  /* Check input params */
  if (pipeline_name == NULL || profile == NULL)
    return -1;
  ...
  if (!pipeline_is_running(p)) {
    status =
        rte_table_action_meter_profile_add(table->a, meter_profile_id, profile);
    if (status) {
      free(mp);
      return status;
    }
...
int softnic_pipeline_table_mtr_profile_delete(struct pmd_internals *softnic,
                                              const char *pipeline_name,
                                              uint32_t table_id,
                                              uint32_t meter_profile_id) {
  struct pipeline *p;
  ...
  if (!pipeline_is_running(p)) {
    ...
    status = rte_table_action_meter_profile_delete(a, meter_profile_id);
...
int softnic_pipeline_table_rule_mtr_read(
    struct pmd_internals *softnic, const char *pipeline_name, uint32_t table_id,
    void *data, uint32_t tc_mask, struct rte_table_action_mtr_counters *stats,
    int clear) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  ...
  if (!pipeline_is_running(p)) {
    ...
    status = rte_table_action_meter_read(a, data, tc_mask, stats, clear);
...
int softnic_pipeline_table_dscp_table_update(
    struct pmd_internals *softnic, const char *pipeline_name, uint32_t table_id,
    uint64_t dscp_mask, struct rte_table_action_dscp_table *dscp_table) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  int status;
  ...
  /* Check input params */
  if (pipeline_name == NULL || dscp_table == NULL)
    return -1;
  ...
  if (!pipeline_is_running(p)) {
    ...
    status = rte_table_action_dscp_table_update(a, dscp_mask, dscp_table);
  ...
  memcpy(&req->table_dscp_table_update.dscp_table, dscp_table,
         sizeof(*dscp_table));
...
int softnic_pipeline_table_rule_ttl_read(
    struct pmd_internals *softnic, const char *pipeline_name, uint32_t table_id,
    void *data, struct rte_table_action_ttl_counters *stats, int clear) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  int status;
  ...
  if (!pipeline_is_running(p)) {
    ...
    status = rte_table_action_ttl_read(a, data, stats, clear);
...
static inline void pipeline_msg_send(struct rte_ring *msgq_rsp,
                                     struct pipeline_msg_rsp *rsp) {
  int status;
...
pipeline_msg_handle_port_in_stats_read(struct pipeline_data *p,
  ...
  rsp->status = rte_pipeline_port_in_stats_read(
      p->p, port_id, &rsp->port_in_stats_read.stats, clear);
...
pipeline_msg_handle_port_in_enable(struct pipeline_data *p,
  ...
  rsp->status = rte_pipeline_port_in_enable(p->p, port_id);
...
pipeline_msg_handle_port_in_disable(struct pipeline_data *p,
  ...
  rsp->status = rte_pipeline_port_in_disable(p->p, port_id);
...
pipeline_msg_handle_port_out_stats_read(struct pipeline_data *p,
  ...
  rsp->status = rte_pipeline_port_out_stats_read(
      p->p, port_id, &rsp->port_out_stats_read.stats, clear);
...
pipeline_msg_handle_table_stats_read(struct pipeline_data *p,
  ...
  rsp->status = rte_pipeline_table_stats_read(
      p->p, port_id, &rsp->table_stats_read.stats, clear);

  return rsp;
}

static int match_convert_ipv6_depth(uint32_t depth, uint32_t *depth32) {
  if (depth > 128)
    return -1;
...
static int match_convert(struct softnic_table_rule_match *mh,
  ...
  case TABLE_ACL:
    if (mh->match.acl.ip_version)
      if (add) {
        ml->acl_add.field_value[0].value.u8 = mh->match.acl.proto;
        ml->acl_add.field_value[0].mask_range.u8 = mh->match.acl.proto_mask;

        ml->acl_add.field_value[1].value.u32 = mh->match.acl.ipv4.sa;
        ml->acl_add.field_value[1].mask_range.u32 = mh->match.acl.sa_depth;

        ml->acl_add.field_value[2].value.u32 = mh->match.acl.ipv4.da;
        ml->acl_add.field_value[2].mask_range.u32 = mh->match.acl.da_depth;

        ml->acl_add.field_value[3].value.u16 = mh->match.acl.sp0;
        ml->acl_add.field_value[3].mask_range.u16 = mh->match.acl.sp1;

        ml->acl_add.field_value[4].value.u16 = mh->match.acl.dp0;
        ml->acl_add.field_value[4].mask_range.u16 = mh->match.acl.dp1;

        ml->acl_add.priority = (int32_t)mh->match.acl.priority;
      } else {
        ml->acl_delete.field_value[0].value.u8 = mh->match.acl.proto;
        ml->acl_delete.field_value[0].mask_range.u8 = mh->match.acl.proto_mask;

        ml->acl_delete.field_value[1].value.u32 = mh->match.acl.ipv4.sa;
        ml->acl_delete.field_value[1].mask_range.u32 = mh->match.acl.sa_depth;

        ml->acl_delete.field_value[2].value.u32 = mh->match.acl.ipv4.da;
        ml->acl_delete.field_value[2].mask_range.u32 = mh->match.acl.da_depth;

        ml->acl_delete.field_value[3].value.u16 = mh->match.acl.sp0;
        ml->acl_delete.field_value[3].mask_range.u16 = mh->match.acl.sp1;

        ml->acl_delete.field_value[4].value.u16 = mh->match.acl.dp0;
        ml->acl_delete.field_value[4].mask_range.u16 = mh->match.acl.dp1;
      }
    else if (add) {
      ...
      status = match_convert_ipv6_depth(mh->match.acl.sa_depth, sa32_depth);
      if (status)
        return status;

      status = match_convert_ipv6_depth(mh->match.acl.da_depth, da32_depth);
      if (status)
        return status;
      ...
      ml->acl_add.field_value[1].mask_range.u32 = sa32_depth[0];
      ml->acl_add.field_value[2].value.u32 = rte_be_to_cpu_32(sa32[1]);
      ml->acl_add.field_value[2].mask_range.u32 = sa32_depth[1];
      ml->acl_add.field_value[3].value.u32 = rte_be_to_cpu_32(sa32[2]);
      ml->acl_add.field_value[3].mask_range.u32 = sa32_depth[2];
      ml->acl_add.field_value[4].value.u32 = rte_be_to_cpu_32(sa32[3]);
      ml->acl_add.field_value[4].mask_range.u32 = sa32_depth[3];

      ml->acl_add.field_value[5].value.u32 = rte_be_to_cpu_32(da32[0]);
      ml->acl_add.field_value[5].mask_range.u32 = da32_depth[0];
      ml->acl_add.field_value[6].value.u32 = rte_be_to_cpu_32(da32[1]);
      ml->acl_add.field_value[6].mask_range.u32 = da32_depth[1];
      ml->acl_add.field_value[7].value.u32 = rte_be_to_cpu_32(da32[2]);
      ml->acl_add.field_value[7].mask_range.u32 = da32_depth[2];
      ml->acl_add.field_value[8].value.u32 = rte_be_to_cpu_32(da32[3]);
      ml->acl_add.field_value[8].mask_range.u32 = da32_depth[3];

      ml->acl_add.field_value[9].value.u16 = mh->match.acl.sp0;
      ml->acl_add.field_value[9].mask_range.u16 = mh->match.acl.sp1;

      ml->acl_add.field_value[10].value.u16 = mh->match.acl.dp0;
      ml->acl_add.field_value[10].mask_range.u16 = mh->match.acl.dp1;
      ...
      ml->acl_delete.field_value[1].value.u32 = rte_be_to_cpu_32(sa32[0]);
      ml->acl_delete.field_value[1].mask_range.u32 = sa32_depth[0];
      ml->acl_delete.field_value[2].value.u32 = rte_be_to_cpu_32(sa32[1]);
      ml->acl_delete.field_value[2].mask_range.u32 = sa32_depth[1];
      ml->acl_delete.field_value[3].value.u32 = rte_be_to_cpu_32(sa32[2]);
      ml->acl_delete.field_value[3].mask_range.u32 = sa32_depth[2];
      ml->acl_delete.field_value[4].value.u32 = rte_be_to_cpu_32(sa32[3]);
      ml->acl_delete.field_value[4].mask_range.u32 = sa32_depth[3];

      ml->acl_delete.field_value[5].value.u32 = rte_be_to_cpu_32(da32[0]);
      ml->acl_delete.field_value[5].mask_range.u32 = da32_depth[0];
      ml->acl_delete.field_value[6].value.u32 = rte_be_to_cpu_32(da32[1]);
      ml->acl_delete.field_value[6].mask_range.u32 = da32_depth[1];
      ml->acl_delete.field_value[7].value.u32 = rte_be_to_cpu_32(da32[2]);
      ml->acl_delete.field_value[7].mask_range.u32 = da32_depth[2];
      ml->acl_delete.field_value[8].value.u32 = rte_be_to_cpu_32(da32[3]);
      ml->acl_delete.field_value[8].mask_range.u32 = da32_depth[3];

      ml->acl_delete.field_value[9].value.u16 = mh->match.acl.sp0;
      ml->acl_delete.field_value[9].mask_range.u16 = mh->match.acl.sp1;

      ml->acl_delete.field_value[10].value.u16 = mh->match.acl.dp0;
      ml->acl_delete.field_value[10].mask_range.u16 = mh->match.acl.dp1;
    }
    return 0;
  ...
  case TABLE_LPM:
    if (mh->match.lpm.ip_version) {
      ...
      memcpy(ml->lpm_ipv6.ip, mh->match.lpm.ipv6, sizeof(ml->lpm_ipv6.ip));
...
static int action_convert(struct rte_table_action *a,
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_FWD)) {
    status =
        rte_table_action_apply(a, data, RTE_TABLE_ACTION_FWD, &action->fwd);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_LB)) {
    status = rte_table_action_apply(a, data, RTE_TABLE_ACTION_LB, &action->lb);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_MTR)) {
    status =
        rte_table_action_apply(a, data, RTE_TABLE_ACTION_MTR, &action->mtr);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_TM)) {
    status = rte_table_action_apply(a, data, RTE_TABLE_ACTION_TM, &action->tm);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_ENCAP)) {
    status =
        rte_table_action_apply(a, data, RTE_TABLE_ACTION_ENCAP, &action->encap);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_NAT)) {
    status =
        rte_table_action_apply(a, data, RTE_TABLE_ACTION_NAT, &action->nat);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_TTL)) {
    status =
        rte_table_action_apply(a, data, RTE_TABLE_ACTION_TTL, &action->ttl);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_STATS)) {
    status =
        rte_table_action_apply(a, data, RTE_TABLE_ACTION_STATS, &action->stats);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_TIME)) {
    status =
        rte_table_action_apply(a, data, RTE_TABLE_ACTION_TIME, &action->time);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_TAG)) {
    status =
        rte_table_action_apply(a, data, RTE_TABLE_ACTION_TAG, &action->tag);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_DECAP)) {
    status =
        rte_table_action_apply(a, data, RTE_TABLE_ACTION_DECAP, &action->decap);
  ...
  if (action->action_mask & (1LLU << RTE_TABLE_ACTION_SYM_CRYPTO)) {
    status = rte_table_action_apply(a, data, RTE_TABLE_ACTION_SYM_CRYPTO,
                                    &action->sym_crypto);
...
pipeline_msg_handle_table_rule_add_default(struct pipeline_data *p,
  ...
  /* Add default rule to table */
  status =
      rte_pipeline_table_default_entry_add(p->p, table_id, data_in, &data_out);
  if (status) {
    rsp->status = -1;
    return rsp;
  }
...
pipeline_msg_handle_table_rule_add_bulk(struct pipeline_data *p,
  ...
  /* Add rule (match, action) to table */
  if (bulk) {
    status = rte_pipeline_table_entry_add_bulk(
        p->p, table_id, match_ll_ptr, action_ll_ptr, n_rules, found, data);
    if (status)
      n_rules = 0;
  } else {
    for (i = 0; i < n_rules; i++) {
      status =
          rte_pipeline_table_entry_add(p->p, table_id, match_ll_ptr[i],
                                       action_ll_ptr[i], &found[i], &data[i]);
      if (status) {
        n_rules = i;
        break;
      }
...
pipeline_msg_handle_table_rule_delete(struct pipeline_data *p,
  ...
  rsp->status = rte_pipeline_table_entry_delete(p->p, table_id, &match_ll,
                                                &key_found, NULL);
...
pipeline_msg_handle_table_rule_delete_default(struct pipeline_data *p,
  ...
  rsp->status = rte_pipeline_table_default_entry_delete(p->p, table_id, NULL);
...
pipeline_msg_handle_table_rule_stats_read(struct pipeline_data *p,
  ...
  rsp->status = rte_table_action_stats_read(
      a, data, &rsp->table_rule_stats_read.stats, clear);
...
pipeline_msg_handle_table_mtr_profile_add(struct pipeline_data *p,
  ...
  rsp->status =
      rte_table_action_meter_profile_add(a, meter_profile_id, profile);
...
pipeline_msg_handle_table_mtr_profile_delete(struct pipeline_data *p,
  ...
  struct pipeline_msg_rsp *rsp = (struct pipeline_msg_rsp *)req;
  uint32_t table_id = req->id;
  uint32_t meter_profile_id = req->table_mtr_profile_delete.meter_profile_id;
  struct rte_table_action *a = p->table_data[table_id].a;

  rsp->status = rte_table_action_meter_profile_delete(a, meter_profile_id);
...
pipeline_msg_handle_table_rule_mtr_read(struct pipeline_data *p,
  ...
  rsp->status = rte_table_action_meter_read(
      a, data, tc_mask, &rsp->table_rule_mtr_read.stats, clear);
...
pipeline_msg_handle_table_dscp_table_update(struct pipeline_data *p,
  ...
  rsp->status = rte_table_action_dscp_table_update(a, dscp_mask, dscp_table);
...
pipeline_msg_handle_table_rule_ttl_read(struct pipeline_data *p,
  ...
  rsp->status = rte_table_action_ttl_read(
      a, data, &rsp->table_rule_ttl_read.stats, clear);

  return rsp;
}

static void pipeline_msg_handle(struct pipeline_data *p) {
  for (;;) {
    struct pipeline_msg_req *req;
    struct pipeline_msg_rsp *rsp;
...
static int32_t rte_pmd_softnic_run_internal(void *arg) {
  ...
  if ((t->iter & 0xFLLU) == 0) {
    ...
    for (j = 0; j < t->n_pipelines; j++) {
      struct pipeline_data *p = &t->pipeline_data[j];
...
int rte_pmd_softnic_run(uint16_t port_id) {
