...
static uint32_t get_hex_val(char c) {
  switch (c) {
  case '0':
  case '1':
  case '2':
...
int softnic_parse_tokenize_string(char *string, char *tokens[],
                                  uint32_t *n_tokens) {
  uint32_t i;
...
int softnic_parse_mpls_labels(char *string, uint32_t *labels,
                              uint32_t *n_labels) {
...
static struct rte_ether_addr *my_ether_aton(const char *a) {
  int i;
  char *end;
