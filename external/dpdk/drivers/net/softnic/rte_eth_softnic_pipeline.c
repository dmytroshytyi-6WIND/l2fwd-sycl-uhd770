...
static void softnic_pipeline_table_free(struct softnic_table *table) {
  for (;;) {
...
void softnic_pipeline_free(struct pmd_internals *p) {
  for (;;) {
    ...
    for (table_id = 0; table_id < pipeline->n_tables; table_id++) {
      struct softnic_table *table = &pipeline->table[table_id];
...
void softnic_pipeline_disable_all(struct pmd_internals *p) {
  ...
  if (pipeline->enabled)
    softnic_thread_pipeline_disable(p, pipeline->thread_id, pipeline->name);
}
...
struct pipeline *softnic_pipeline_create(struct pmd_internals *softnic,
                                         const char *name,
  ...
  /* Check input params */
  if (name == NULL || softnic_pipeline_find(softnic, name) || params == NULL ||
  ...
  msgq_req =
      rte_ring_create(resource_name, PIPELINE_MSGQ_SIZE, softnic->params.cpu_id,
  ...
  snprintf(resource_name, sizeof(resource_name), "%s_%s", softnic->params.name,
           name);
...
int softnic_pipeline_port_in_create(struct pmd_internals *softnic,
  ...
  /* Check input params */
  if (pipeline_name == NULL || params == NULL || params->burst_size == 0 ||
  ...
  if (ap) {
    action = rte_port_in_action_create(ap->ap, softnic->params.cpu_id);
    if (action == NULL)
      return -1;

    status = rte_port_in_action_params_get(action, &p);
    if (status) {
  ...
  status = rte_pipeline_port_in_create(pipeline->p, &p, &port_id);
  if (status) {
    rte_port_in_action_free(action);
    return -1;
  }
...
int softnic_pipeline_port_in_connect_to_table(struct pmd_internals *softnic,
                                              const char *pipeline_name,
                                              uint32_t port_id,
                                              uint32_t table_id) {
  struct pipeline *pipeline;
  ...
  pipeline = softnic_pipeline_find(softnic, pipeline_name);
  if (pipeline == NULL || port_id >= pipeline->n_ports_in ||
      table_id >= pipeline->n_tables)
    return -1;

  /* Resource */
  status =
      rte_pipeline_port_in_connect_to_table(pipeline->p, port_id, table_id);

  return status;
}

int softnic_pipeline_port_out_create(struct pmd_internals *softnic,
                                     const char *pipeline_name,
  ...
  case PORT_OUT_CRYPTODEV: {
    ...
    pp_nodrop.cryptodev.crypto_op_offset = params->cryptodev.op_offset;
  ...
  /* Resource create */
  status = rte_pipeline_port_out_create(pipeline->p, &p, &port_id);
...
int softnic_pipeline_table_create(struct pmd_internals *softnic,
                                  const char *pipeline_name,
  ...
  /* Check input params */
  if (pipeline_name == NULL || params == NULL)
    return -1;

  pipeline = softnic_pipeline_find(softnic, pipeline_name);
  if (pipeline == NULL || pipeline->n_tables >= RTE_PIPELINE_TABLE_MAX)
    return -1;

  ap = NULL;
  if (strlen(params->action_profile_name)) {
    ap =
        softnic_table_action_profile_find(softnic, params->action_profile_name);
    if (ap == NULL)
      return -1;
  }

  snprintf(name, NAME_MAX, "%s_%s_table%u", softnic->params.name, pipeline_name,
           pipeline->n_tables);

  switch (params->match_type) {
  case TABLE_ACL: {
    uint32_t ip_header_offset =
        params->match.acl.ip_header_offset -
    ...
    if (params->match.acl.ip_version) {
      memcpy(&pp.acl.field_format, &table_acl_field_format_ipv4,
             sizeof(table_acl_field_format_ipv4));
      pp.acl.n_rule_fields = RTE_DIM(table_acl_field_format_ipv4);
    } else {
      memcpy(&pp.acl.field_format, &table_acl_field_format_ipv6,
             sizeof(table_acl_field_format_ipv6));
      pp.acl.n_rule_fields = RTE_DIM(table_acl_field_format_ipv6);
    }
  ...
  case TABLE_LPM: {
    ...
    case 4: {
      ...
      pp.lpm.flags = 0;
      pp.lpm.entry_unique_size =
          p.action_data_size + sizeof(struct rte_pipeline_table_entry);
  ...
  if (ap) {
    action = rte_table_action_create(ap->ap, softnic->params.cpu_id);
    if (action == NULL)
      return -1;

    status = rte_table_action_table_params_get(action, &p);
    if (status ||
        ((p.action_data_size + sizeof(struct rte_pipeline_table_entry)) >
  ...
  status = rte_pipeline_table_create(pipeline->p, &p, &table_id);
  if (status) {
    rte_table_action_free(action);
    return -1;
  }
...
int softnic_pipeline_port_out_find(struct pmd_internals *softnic,
  ...
  if (softnic == NULL || pipeline_name == NULL || name == NULL ||
      port_id == NULL)
    return -1;
