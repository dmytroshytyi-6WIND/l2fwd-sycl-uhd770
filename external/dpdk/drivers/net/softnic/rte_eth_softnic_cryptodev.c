...
softnic_cryptodev_create(struct pmd_internals *p, const char *name,
  ...
  /* Check input params */
  if ((name == NULL) || softnic_cryptodev_find(p, name) ||
      (params->n_queues == 0) || (params->queue_size == 0) ||
  ...
  cache_size =
      (params->session_pool_size / 2 < SOFTNIC_CRYPTO_SESSION_CACHE_SIZE)
  ...
  cryptodev->mp_create = rte_cryptodev_sym_session_pool_create(
      mp_name, params->session_pool_size, 0, cache_size, 0, socket_id);
  if (!cryptodev->mp_create)
    goto error_exit;

  snprintf(mp_name, NAME_SIZE, "%s_priv_mp%u", name, dev_id);
  cryptodev->mp_init =
      rte_mempool_create(mp_name, params->session_pool_size,
