...
int flow_attr_map_set(struct pmd_internals *softnic, uint32_t group_id,
                      int ingress, const char *pipeline_name,
                      uint32_t table_id) {
  struct pipeline *pipeline;
  struct flow_attr_map *map;

  if (group_id >= SOFTNIC_FLOW_MAX_GROUPS || pipeline_name == NULL)
    return -1;
...
struct flow_attr_map *flow_attr_map_get(struct pmd_internals *softnic,
                                        uint32_t group_id, int ingress) {
...
static int flow_pipeline_table_get(struct pmd_internals *softnic,
  ...
  if (!attr->ingress && !attr->egress)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS,
                              attr, "Ingress/egress not specified");
...
static int flow_item_is_proto(enum rte_flow_item_type type, const void **mask,
                              size_t *size) {
  switch (type) {
...
static int flow_item_raw_preprocess(const struct rte_flow_item *item,
                                    union flow_item *item_spec,
                                    union flow_item *item_mask,
                                    size_t *item_size, int *item_disabled,
                                    struct rte_flow_error *error) {
  ...
  if (item_raw_spec->offset + item_raw_spec->length > TABLE_RULE_MATCH_SIZE_MAX)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item,
                              "RAW: Item too big");
...
static int flow_item_proto_preprocess(const struct rte_flow_item *item,
                                      union flow_item *item_spec,
                                      union flow_item *item_mask,
                                      size_t *item_size, int *item_disabled,
                                      struct rte_flow_error *error) {
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_RAW)
    return flow_item_raw_preprocess(item, item_spec, item_mask, item_size,
                                    item_disabled, error);
 ...
 */
static int flow_item_skip_disabled_protos(const struct rte_flow_item **item,
                                          uint64_t proto_mask, size_t *length,
                                          struct rte_flow_error *error) {
  size_t len = 0;
  ...
  for (; (*item)->type != RTE_FLOW_ITEM_TYPE_END; (*item)++) {
    ...
    status = flow_item_proto_preprocess(*item, &spec, &mask, &size, &disabled,
                                        error);
    if (status)
      return status;

    if ((proto_mask & (1LLU << (*item)->type)) || !disabled)
      break;
...
#define FLOW_ITEM_PROTO_IP                                                     \
  ((1LLU << RTE_FLOW_ITEM_TYPE_IPV4) | (1LLU << RTE_FLOW_ITEM_TYPE_IPV6))
...
static int mask_to_depth(uint64_t mask, uint32_t *depth) {
  uint64_t n;
...
static int ipv4_mask_to_depth(uint32_t mask, uint32_t *depth) {
  uint32_t d;
  int status;
...
static int ipv6_mask_to_depth(uint8_t *mask, uint32_t *depth) {
...
static int port_mask_to_range(uint16_t port, uint16_t port_mask,
...
static int flow_rule_match_acl_get(struct pmd_internals *softnic __rte_unused,
  ...
  status =
      flow_item_skip_disabled_protos(&item, FLOW_ITEM_PROTO_IP, &length, error);
  if (status)
    return status;

  /* IP only. */
  status =
      flow_item_proto_preprocess(item, &spec, &mask, &size, &disabled, error);
  if (status)
    return status;
  ...
  case RTE_FLOW_ITEM_TYPE_IPV4: {
    ...
    status = ipv4_mask_to_depth(rte_ntohl(mask.ipv4.hdr.src_addr), &sa_depth);
    if (status)
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                                "ACL: Illegal IPv4 header source address mask");

    status = ipv4_mask_to_depth(rte_ntohl(mask.ipv4.hdr.dst_addr), &da_depth);
    if (status)
      return rte_flow_error_set(
          error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
    ...
    rule_match->match.acl.ipv4.sa = rte_ntohl(spec.ipv4.hdr.src_addr);
    rule_match->match.acl.ipv4.da = rte_ntohl(spec.ipv4.hdr.dst_addr);
  ...
  case RTE_FLOW_ITEM_TYPE_IPV6: {
    ...
    memcpy(rule_match->match.acl.ipv6.sa, spec.ipv6.hdr.src_addr,
           sizeof(spec.ipv6.hdr.src_addr));
    memcpy(rule_match->match.acl.ipv6.da, spec.ipv6.hdr.dst_addr,
  ...
  case RTE_FLOW_ITEM_TYPE_TCP: {
    ...
    status = port_mask_to_range(rte_ntohs(spec.tcp.hdr.src_port),
                                rte_ntohs(mask.tcp.hdr.src_port), &sp0, &sp1);
    ...
    status = port_mask_to_range(rte_ntohs(spec.tcp.hdr.dst_port),
                                rte_ntohs(mask.tcp.hdr.dst_port), &dp0, &dp1);
  ...
  case RTE_FLOW_ITEM_TYPE_SCTP: {
    ...
    status = port_mask_to_range(rte_ntohs(spec.sctp.hdr.src_port),
                                rte_ntohs(mask.sctp.hdr.src_port), &sp0, &sp1);
    ...
    status = port_mask_to_range(rte_ntohs(spec.sctp.hdr.dst_port),
                                rte_ntohs(mask.sctp.hdr.dst_port), &dp0, &dp1);
    if (status)
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
 ...
 */
static int hash_key_mask_is_same(uint8_t *tmask, size_t toffset, size_t tsize,
                                 uint8_t *fmask, size_t foffset, size_t fsize,
                                 size_t *toffset_plus, size_t *foffset_plus) {
...
static int flow_rule_match_hash_get(
    struct pmd_internals *softnic __rte_unused,
    struct pipeline *pipeline __rte_unused, struct softnic_table *table,
    const struct rte_flow_attr *attr __rte_unused,
    const struct rte_flow_item *item,
    struct softnic_table_rule_match *rule_match, struct rte_flow_error *error) {
  ...
  if (!hash_key_mask_is_same(params->key_mask, params->key_offset,
                             params->key_size, key_mask.key, offset, length,
                             &tpos, &fpos))
    return rte_flow_error_set(
  ...
  memcpy(&rule_match->match.hash.key[tpos], &key.key[fpos],
         RTE_MIN(sizeof(rule_match->match.hash.key) - tpos, length - fpos));

  return 0;
}

static int flow_rule_match_get(struct pmd_internals *softnic,
                               struct pipeline *pipeline,
  ...
  case TABLE_ACL:
    return flow_rule_match_acl_get(softnic, pipeline, table, attr, item,
                                   rule_match, error);

    /* FALLTHROUGH */

  case TABLE_HASH:
    return flow_rule_match_hash_get(softnic, pipeline, table, attr, item,
                                    rule_match, error);
  ...
  default:
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Unsupported pipeline table match type");
  }
}

static int flow_rule_action_get(struct pmd_internals *softnic,
                                struct pipeline *pipeline,
                                struct softnic_table *table,
                                const struct rte_flow_attr *attr,
  ...
  profile = softnic_table_action_profile_find(
      softnic, table->params.action_profile_name);
  if (profile == NULL)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              action, "JUMP: Table action profile");
  ...
  for (; action->type != RTE_FLOW_ACTION_TYPE_END; action++) {
    ...
    case RTE_FLOW_ACTION_TYPE_JUMP: {
      ...
      if (conf == NULL)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "JUMP: Null configuration");
      ...
      if ((params->action_mask & (1LLU << RTE_TABLE_ACTION_FWD)) == 0)
      ...
      map = flow_attr_map_get(softnic, conf->group, attr->ingress);
      if (map == NULL || map->valid == 0)
        return rte_flow_error_set(error, EINVAL,
                                  RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
    ...
    case RTE_FLOW_ACTION_TYPE_QUEUE: {
      ...
      if (conf == NULL)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "QUEUE: Null configuration");
      ...
      dev = ETHDEV(softnic);
      if (dev == NULL || conf->index >= dev->data->nb_rx_queues)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "QUEUE: Invalid RX queue ID");

      snprintf(name, sizeof(name), "RXQ%u", (uint32_t)conf->index);

      status = softnic_pipeline_port_out_find(softnic, pipeline->name, name,
                                              &port_id);
      if (status)
        return rte_flow_error_set(
    ...
    case RTE_FLOW_ACTION_TYPE_RSS: {
      ...
      if (conf == NULL)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "RSS: Null configuration");

      if (!rte_is_power_of_2(conf->queue_num))
        return rte_flow_error_set(error, EINVAL,
                                  RTE_FLOW_ERROR_TYPE_ACTION_CONF, conf,
      ...
      if (n_jump_queue_rss_drop)
        return rte_flow_error_set(
            error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, action,
            "Only one termination action is allowed per flow");

      if (((params->action_mask & (1LLU << RTE_TABLE_ACTION_FWD)) == 0) ||
          ((params->action_mask & (1LLU << RTE_TABLE_ACTION_LB)) == 0))
        return rte_flow_error_set(error, ENOTSUP,
                                  RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                                  "RSS action not supported by this table");

      if (params->lb.out_offset != pipeline->params.offset_port_id)
        return rte_flow_error_set(error, EINVAL,
                                  RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
      ...
      for (i = 0; i < conf->queue_num; i++) {
        ...
        dev = ETHDEV(softnic);
        if (dev == NULL || conf->queue[i] >= dev->data->nb_rx_queues)
          return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                    action, "RSS: Invalid RX queue ID");

        snprintf(name, sizeof(name), "RXQ%u", (uint32_t)conf->queue[i]);
      ...
      for (; i < RTE_DIM(rule_action->lb.out); i++)
        rule_action->lb.out[i] = rule_action->lb.out[i % conf->queue_num];
    ...
    case RTE_FLOW_ACTION_TYPE_DROP: {
      ...
      if (conf != NULL)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "DROP: No configuration required");
    ...
    case RTE_FLOW_ACTION_TYPE_COUNT: {
      ...
      if (conf == NULL)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "COUNT: Null configuration");

      if (n_count)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "Only one COUNT action per flow");

      if ((params->action_mask & (1LLU << RTE_TABLE_ACTION_STATS)) == 0)
        return rte_flow_error_set(error, ENOTSUP,
                                  RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
    ...
    case RTE_FLOW_ACTION_TYPE_MARK: {
      ...
      if (conf == NULL)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "MARK: Null configuration");

      if (n_mark)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "Only one MARK action per flow");

      if ((params->action_mask & (1LLU << RTE_TABLE_ACTION_TAG)) == 0)
        return rte_flow_error_set(error, ENOTSUP,
                                  RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
    ...
    case RTE_FLOW_ACTION_TYPE_VXLAN_DECAP: {
      ...
      if ((params->action_mask & (1LLU << RTE_TABLE_ACTION_DECAP)) == 0)
        return rte_flow_error_set(
    ...
    case RTE_FLOW_ACTION_TYPE_METER: {
      ...
      if (conf == NULL)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "METER: Null configuration");
      ...
      if (!softnic_pipeline_table_meter_profile_find(table, meter_profile_id)) {
        ...
        status = softnic_pipeline_table_mtr_profile_add(
            softnic, pipeline->name, table_id, meter_profile_id, &profile);
        if (status) {
          rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                             NULL, "METER: Table meter profile add failed");
          return -1;
        }
      }
      /* Meter policy must exist */
      policy =
          softnic_mtr_meter_policy_find(softnic, m->params.meter_policy_id);
      if (policy == NULL) {
    ...
    case RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP: {
      ...
      if (conf == NULL)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  action, "VXLAN ENCAP: Null configuration");
      ...
      rte_ether_addr_copy(&spec.eth.dst, &rule_action->encap.vxlan.ether.da);
      rte_ether_addr_copy(&spec.eth.src, &rule_action->encap.vxlan.ether.sa);
      ...
      if (item->type == RTE_FLOW_ITEM_TYPE_VLAN) {
        ...
        flow_item_skip_void(&item);
        status = flow_item_proto_preprocess(item, &spec, &mask, &size,
                                            &disabled, error);
        if (status)
          return status;
      } else {
      ...
      switch (item->type) {
      case RTE_FLOW_ITEM_TYPE_IPV4: {
        rule_action->encap.vxlan.ipv4.sa = rte_ntohl(spec.ipv4.hdr.src_addr);
        rule_action->encap.vxlan.ipv4.da = rte_ntohl(spec.ipv4.hdr.dst_addr);
        rule_action->encap.vxlan.ipv4.dscp = spec.ipv4.hdr.type_of_service >> 2;
        rule_action->encap.vxlan.ipv4.ttl = spec.ipv4.hdr.time_to_live;
        break;
      }
      case RTE_FLOW_ITEM_TYPE_IPV6: {
        uint32_t vtc_flow;

        memcpy(&rule_action->encap.vxlan.ipv6.sa, &spec.ipv6.hdr.src_addr,
               sizeof(spec.ipv6.hdr.src_addr));
        memcpy(&rule_action->encap.vxlan.ipv6.da, &spec.ipv6.hdr.dst_addr,
               sizeof(spec.ipv6.hdr.dst_addr));
        ...
        rule_action->encap.vxlan.ipv6.hop_limit = spec.ipv6.hdr.hop_limits;
        break;
      }
      default:
        return rte_flow_error_set(
      ...
      rule_action->encap.vxlan.udp.sp = rte_ntohs(spec.udp.hdr.src_port);
      rule_action->encap.vxlan.udp.dp = rte_ntohs(spec.udp.hdr.dst_port);
      ...
      rule_action->encap.vxlan.vxlan.vni =
          (spec.vxlan.vni[0] << 16U | spec.vxlan.vni[1] << 8U |
           spec.vxlan.vni[2]);
...
static int pmd_flow_validate(struct rte_eth_dev *dev,
                             const struct rte_flow_attr *attr,
                             const struct rte_flow_item item[],
                             const struct rte_flow_action action[],
                             struct rte_flow_error *error) {
  ...
  status =
      flow_pipeline_table_get(softnic, attr, &pipeline_name, &table_id, error);
  if (status)
    return status;
  ...
  if (pipeline == NULL)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Invalid pipeline name");

  if (table_id >= pipeline->n_tables)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Invalid pipeline table ID");

  table = &pipeline->table[table_id];

  /* Rule match. */
  memset(&rule_match, 0, sizeof(rule_match));
  status = flow_rule_match_get(softnic, pipeline, table, attr, item,
                               &rule_match, error);
  if (status)
    return status;
  ...
  status = flow_rule_action_get(softnic, pipeline, table, attr, action,
                                &rule_action, error);
  if (status)
    return status;
...
static void flow_meter_owner_reset(struct pmd_internals *softnic,
                                   struct rte_flow *flow) {
...
static void flow_meter_owner_set(struct pmd_internals *softnic,
                                 struct rte_flow *flow,
...
static int is_meter_action_enable(struct pmd_internals *softnic,
                                  struct softnic_table *table) {
  struct softnic_table_action_profile *profile =
...
static struct rte_flow *pmd_flow_create(struct rte_eth_dev *dev,
                                        const struct rte_flow_attr *attr,
                                        const struct rte_flow_item item[],
                                        const struct rte_flow_action action[],
                                        struct rte_flow_error *error) {
  ...
  /* Rule add. */
  status = softnic_pipeline_table_rule_add(
      softnic, pipeline_name, table_id, &rule_match, &rule_action, &rule_data);
  if (status) {
    if (new_flow)
      free(flow);
...
static int pmd_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                            struct rte_flow_error *error) {
  struct pmd_internals *softnic = dev->data->dev_private;
  struct softnic_table *table;
  int status;
  ...
  /* Rule delete. */
  status = softnic_pipeline_table_rule_delete(softnic, flow->pipeline->name,
                                              flow->table_id, &flow->match);
  if (status)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Pipeline table rule delete failed");
...
static int pmd_flow_flush(struct rte_eth_dev *dev,
                          struct rte_flow_error *error) {
  struct pmd_internals *softnic = dev->data->dev_private;
  struct pipeline *pipeline;
  ...
  TAILQ_FOREACH(pipeline, &softnic->pipeline_list, node) {
    ...
    for (i = 0; i < pipeline->n_tables; i++) {
      ...
      RTE_TAILQ_FOREACH_SAFE(flow, &table->flows, node, temp) {
        /* Rule delete. */
        status = softnic_pipeline_table_rule_delete(softnic, pipeline->name, i,
                                                    &flow->match);
        if (status)
          fail_to_del_rule = 1;
        /* Update dependencies */
  ...
  if (fail_to_del_rule)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Some of the rules could not be deleted");

  return 0;
}

static int pmd_flow_query(struct rte_eth_dev *dev __rte_unused,
                          struct rte_flow *flow,
  ...
  /* Rule stats read. */
  status = rte_table_action_stats_read(table->a, flow->data, &stats,
                                       flow_stats->reset);
  if (status)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Pipeline table rule stats read failed");

  /* Fill in flow stats. */
  flow_stats->hits_set = (table->ap->params.stats.n_packets_enabled) ? 1 : 0;
  flow_stats->bytes_set = (table->ap->params.stats.n_bytes_enabled) ? 1 : 0;
