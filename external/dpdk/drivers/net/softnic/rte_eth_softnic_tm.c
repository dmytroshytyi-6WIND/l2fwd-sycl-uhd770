...
int softnic_tmgr_init(struct pmd_internals *p) {
  TAILQ_INIT(&p->tmgr_port_list);

  return 0;
}

void softnic_tmgr_free(struct pmd_internals *p) {
  for (;;) {
...
struct softnic_tmgr_port *softnic_tmgr_port_find(struct pmd_internals *p,
                                                 const char *name) {
  struct softnic_tmgr_port *tmgr_port;
...
struct softnic_tmgr_port *softnic_tmgr_port_create(struct pmd_internals *p,
                                                   const char *name) {
  struct softnic_tmgr_port *tmgr_port;
  ...
  /* Check input params */
  if (name == NULL || softnic_tmgr_port_find(p, name))
    return NULL;
  ...
  for (subport_id = 0; subport_id < n_subports; subport_id++) {
    ...
    status = rte_sched_subport_config(sched, subport_id,
    ...
    for (pipe_id = 0; pipe_id < n_pipes_per_subport; pipe_id++) {
      ...
      status = rte_sched_pipe_config(sched, subport_id, pipe_id, profile_id);
      if (status) {
        rte_sched_port_free(sched);
        return NULL;
      }
...
static struct rte_sched_port *SCHED(struct pmd_internals *p) {
  struct softnic_tmgr_port *tmgr_port;
...
void tm_hierarchy_init(struct pmd_internals *p) {
...
void tm_hierarchy_free(struct pmd_internals *p) {
  /* Remove all nodes*/
  for (;;) {
  ...
  for (;;) {
    ...
    TAILQ_REMOVE(&p->soft.tm.h.shaper_profiles, shaper_profile, node);
    free(shaper_profile);
  }
...
static struct tm_node *tm_node_search(struct rte_eth_dev *dev,
                                      uint32_t node_id) {
  struct pmd_internals *p = dev->data->dev_private;
...
static struct tm_node *tm_root_node_present(struct rte_eth_dev *dev) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_node_list *nl = &p->soft.tm.h.nodes;
  struct tm_node *n;
...
static uint32_t tm_node_subport_id(struct rte_eth_dev *dev,
...
static uint32_t tm_node_pipe_id(struct rte_eth_dev *dev,
                                struct tm_node *pipe_node) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_node_list *nl = &p->soft.tm.h.nodes;
  struct tm_node *np;
  uint32_t pipe_id;
...
static uint32_t tm_node_tc_id(struct rte_eth_dev *dev __rte_unused,
                              struct tm_node *tc_node) {
  return tc_node->priority;
}

static uint32_t tm_node_queue_id(struct rte_eth_dev *dev,
                                 struct tm_node *queue_node) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_node_list *nl = &p->soft.tm.h.nodes;
  struct tm_node *nq;
  uint32_t queue_id;
...
static uint32_t tm_level_get_max_nodes(struct rte_eth_dev *dev,
                                       enum tm_node_level level) {
  struct pmd_internals *p = dev->data->dev_private;
  uint32_t n_queues_max = p->params.tm.n_queues;
  uint32_t n_tc_max = (n_queues_max * RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE) /
...
static int pmd_tm_node_type_get(struct rte_eth_dev *dev, uint32_t node_id,
                                int *is_leaf, struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;

  if (is_leaf == NULL)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
                             rte_strerror(EINVAL));

  if (node_id == RTE_TM_NODE_ID_NULL || (tm_node_search(dev, node_id) == NULL))
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_NODE_ID, NULL,
                             rte_strerror(EINVAL));
...
#define STATS_MASK_DEFAULT                                                     \
  (RTE_TM_STATS_N_PKTS | RTE_TM_STATS_N_BYTES |                                \
   RTE_TM_STATS_N_PKTS_GREEN_DROPPED | RTE_TM_STATS_N_BYTES_GREEN_DROPPED)

#define STATS_MASK_QUEUE (STATS_MASK_DEFAULT | RTE_TM_STATS_N_PKTS_QUEUED)
...
static int pmd_tm_capabilities_get(struct rte_eth_dev *dev __rte_unused,
                                   struct rte_tm_capabilities *cap,
                                   struct rte_tm_error *error) {
  if (cap == NULL)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_CAPABILITIES,
  ...
  cap->shaper_n_max = cap->shaper_private_n_max + cap->shaper_shared_n_max;

  cap->shaper_shared_n_nodes_per_shaper_max =
      tm_level_get_max_nodes(dev, TM_NODE_LEVEL_PIPE);

  cap->sched_n_children_max =
      RTE_MAX(tm_level_get_max_nodes(dev, TM_NODE_LEVEL_PIPE),
            ...
            {.nonleaf =
                 {
                     ...
                     .sched_n_children_max = RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE,
            ...
            {.nonleaf =
                 {
                     ...
                     .sched_n_children_max = RTE_SCHED_BE_QUEUES_PER_PIPE,
                     .sched_sp_n_priorities_max = 1,
...
static int pmd_tm_level_capabilities_get(struct rte_eth_dev *dev __rte_unused,
                                         uint32_t level_id,
  ...
  if (level_id >= TM_NODE_LEVEL_MAX)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_LEVEL_ID, NULL,
                             rte_strerror(EINVAL));
  ...
  case TM_NODE_LEVEL_SUBPORT:
    cap->n_nodes_max = tm_level_get_max_nodes(dev, TM_NODE_LEVEL_SUBPORT);
  ...
  case TM_NODE_LEVEL_PIPE:
    cap->n_nodes_max = tm_level_get_max_nodes(dev, TM_NODE_LEVEL_PIPE);
    cap->n_nodes_nonleaf_max = cap->n_nodes_max;
    break;

  case TM_NODE_LEVEL_TC:
    cap->n_nodes_max = tm_level_get_max_nodes(dev, TM_NODE_LEVEL_TC);
    cap->n_nodes_nonleaf_max = cap->n_nodes_max;
    break;

  case TM_NODE_LEVEL_QUEUE:
  default:
    cap->n_nodes_max = tm_level_get_max_nodes(dev, TM_NODE_LEVEL_QUEUE);
...
static int pmd_tm_node_capabilities_get(struct rte_eth_dev *dev __rte_unused,
                                        uint32_t node_id,
...
static int shaper_profile_check(struct rte_eth_dev *dev,
  ...
  if (shaper_profile_id == RTE_TM_SHAPER_PROFILE_ID_NONE)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_SHAPER_PROFILE_ID,
                             NULL, rte_strerror(EINVAL));

  /* Shaper profile must not exist. */
  sp = tm_shaper_profile_search(dev, shaper_profile_id);
  if (sp)
    return -rte_tm_error_set(error, EEXIST, RTE_TM_ERROR_TYPE_SHAPER_PROFILE_ID,
                             NULL, rte_strerror(EEXIST));

  /* Profile must not be NULL. */
  if (profile == NULL)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_SHAPER_PROFILE,
                             NULL, rte_strerror(EINVAL));

  /* Peak rate: non-zero, 32-bit */
  if (profile->peak.rate == 0 || profile->peak.rate >= UINT32_MAX)
    return -rte_tm_error_set(error, EINVAL,
                             RTE_TM_ERROR_TYPE_SHAPER_PROFILE_PEAK_RATE, NULL,
                             rte_strerror(EINVAL));

  /* Peak size: non-zero, 32-bit */
  if (profile->peak.size == 0 || profile->peak.size >= UINT32_MAX)
    return -rte_tm_error_set(error, EINVAL,
                             RTE_TM_ERROR_TYPE_SHAPER_PROFILE_PEAK_SIZE, NULL,
                             rte_strerror(EINVAL));
  ...
  if (profile->packet_mode != 0)
    return -rte_tm_error_set(error, EINVAL,
                             RTE_TM_ERROR_TYPE_SHAPER_PROFILE_PACKET_MODE, NULL,
                             rte_strerror(EINVAL));
  return 0;
}

/* Traffic manager shaper profile add */
static int pmd_tm_shaper_profile_add(struct rte_eth_dev *dev,
                                     uint32_t shaper_profile_id,
                                     struct rte_tm_shaper_params *profile,
                                     struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  ...
  if (sp == NULL)
    return -rte_tm_error_set(error, ENOMEM, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
                             rte_strerror(ENOMEM));
...
static int pmd_tm_shaper_profile_delete(struct rte_eth_dev *dev,
                                        uint32_t shaper_profile_id,
                                        struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_shaper_profile *sp;
  ...
  /* Check unused */
  if (sp->n_users)
    return -rte_tm_error_set(error, EBUSY, RTE_TM_ERROR_TYPE_SHAPER_PROFILE_ID,
                             NULL, rte_strerror(EBUSY));
...
static struct tm_node *tm_shared_shaper_get_tc(struct rte_eth_dev *dev,
                                               struct tm_shared_shaper *ss) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_node_list *nl = &p->soft.tm.h.nodes;
  struct tm_node *n;
  ...
  TAILQ_FOREACH(n, nl, node) {
    if (n->level != TM_NODE_LEVEL_TC || n->params.n_shared_shapers == 0 ||
...
static int subport_profile_exists(struct rte_eth_dev *dev,
...
static int update_subport_tc_rate(struct rte_eth_dev *dev, struct tm_node *nt,
                                  struct tm_shared_shaper *ss,
  ...
  struct tm_params *t = &p->soft.tm.params;
  uint32_t subport_profile_id;
  struct tm_shaper_profile *sp_old =
      tm_shaper_profile_search(dev, ss->shaper_profile_id);
  ...
  if (rte_sched_subport_config(SCHED(p), subport_id, NULL, subport_profile_id))
    return -1;
  ...
  memcpy(&p->soft.tm.params.subport_profile[subport_profile_id],
         &subport_profile, sizeof(subport_profile));

  return 0;
}

/* Traffic manager shared shaper add/update */
static int pmd_tm_shared_shaper_add_update(struct rte_eth_dev *dev,
  ...
  if (ss == NULL) {
    ...
    if (p->soft.tm.hierarchy_frozen)
      return -rte_tm_error_set(error, EBUSY, RTE_TM_ERROR_TYPE_UNSPECIFIED,
                               NULL, rte_strerror(EBUSY));

    /* Memory allocation */
    ss = calloc(1, sizeof(struct tm_shared_shaper));
    if (ss == NULL)
      return -rte_tm_error_set(error, ENOMEM, RTE_TM_ERROR_TYPE_UNSPECIFIED,
  ...
  if (p->soft.tm.hierarchy_frozen == 0)
    return -rte_tm_error_set(error, EBUSY, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
                             rte_strerror(EBUSY));
...
static int pmd_tm_shared_shaper_delete(struct rte_eth_dev *dev,
                                       uint32_t shared_shaper_id,
                                       struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_shared_shaper *ss;
  ...
  ss = tm_shared_shaper_search(dev, shared_shaper_id);
  if (ss == NULL)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_SHARED_SHAPER_ID,
                             NULL, rte_strerror(EINVAL));

  /* Check unused */
  if (ss->n_users)
    return -rte_tm_error_set(error, EBUSY, RTE_TM_ERROR_TYPE_SHARED_SHAPER_ID,
                             NULL, rte_strerror(EBUSY));
...
static int wred_profile_check(struct rte_eth_dev *dev, uint32_t wred_profile_id,
  ...
  if (wred_profile_id == RTE_TM_WRED_PROFILE_ID_NONE)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_WRED_PROFILE_ID,
                             NULL, rte_strerror(EINVAL));

  /* WRED profile must not exist. */
  wp = tm_wred_profile_search(dev, wred_profile_id);
  if (wp)
    return -rte_tm_error_set(error, EEXIST, RTE_TM_ERROR_TYPE_WRED_PROFILE_ID,
                             NULL, rte_strerror(EEXIST));

  /* Profile must not be NULL. */
  if (profile == NULL)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_WRED_PROFILE,
                             NULL, rte_strerror(EINVAL));

  /* WRED profile should be in packet mode */
  if (profile->packet_mode == 0)
    return -rte_tm_error_set(error, ENOTSUP, RTE_TM_ERROR_TYPE_WRED_PROFILE,
  ...
  for (color = RTE_COLOR_GREEN; color < RTE_COLORS; color++) {
    ...
    if (min_th > max_th || max_th == 0 || min_th > UINT16_MAX ||
        max_th > UINT16_MAX)
      return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_WRED_PROFILE,
                               NULL, rte_strerror(EINVAL));
  }
...
static int pmd_tm_wred_profile_add(struct rte_eth_dev *dev,
...
static int pmd_tm_wred_profile_delete(struct rte_eth_dev *dev,
                                      uint32_t wred_profile_id,
                                      struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_wred_profile *wp;
  ...
  /* Check unused */
  if (wp->n_users)
    return -rte_tm_error_set(error, EBUSY, RTE_TM_ERROR_TYPE_WRED_PROFILE_ID,
                             NULL, rte_strerror(EBUSY));
...
static int node_add_check_port(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  /* Weight must be 1 */
  if (weight != 1)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_NODE_WEIGHT, NULL,
                             rte_strerror(EINVAL));

  /* Shaper must be valid */
  if (params->shaper_profile_id == RTE_TM_SHAPER_PROFILE_ID_NONE || sp == NULL)
    return -rte_tm_error_set(error, EINVAL,
  ...
  if (params->stats_mask & ~STATS_MASK_DEFAULT)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_NODE_PARAMS_STATS,
                             NULL, rte_strerror(EINVAL));

  return 0;
}

static int node_add_check_subport(struct rte_eth_dev *dev, uint32_t node_id,
                                  uint32_t parent_node_id __rte_unused,
                                  uint32_t priority, uint32_t weight,
                                  uint32_t level_id __rte_unused,
                                  struct rte_tm_node_params *params,
...
static int node_add_check_pipe(struct rte_eth_dev *dev, uint32_t node_id,
                               uint32_t parent_node_id __rte_unused,
                               uint32_t priority, uint32_t weight __rte_unused,
                               uint32_t level_id __rte_unused,
                               struct rte_tm_node_params *params,
                               struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  ...
  if (params->nonleaf.n_sp_priorities != RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE)
    return -rte_tm_error_set(error, EINVAL,
                             RTE_TM_ERROR_TYPE_NODE_PARAMS_N_SP_PRIORITIES,
                             NULL, rte_strerror(EINVAL));
...
static int node_add_check_tc(struct rte_eth_dev *dev, uint32_t node_id,
                             uint32_t parent_node_id __rte_unused,
                             uint32_t priority __rte_unused, uint32_t weight,
                             uint32_t level_id __rte_unused,
                             struct rte_tm_node_params *params,
                             struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
...
static int node_add_check_queue(struct rte_eth_dev *dev, uint32_t node_id,
                                uint32_t parent_node_id __rte_unused,
                                uint32_t priority, uint32_t weight __rte_unused,
                                uint32_t level_id __rte_unused,
                                struct rte_tm_node_params *params,
  ...
  if (params->leaf.cman == RTE_TM_CMAN_HEAD_DROP)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_NODE_PARAMS_CMAN,
                             NULL, rte_strerror(EINVAL));
  ...
  if (params->leaf.cman == RTE_TM_CMAN_WRED) {
    ...
    struct tm_wred_profile *wp = tm_wred_profile_search(dev, wred_profile_id);

    /* WRED profile (for private WRED context) must be valid */
    if (wred_profile_id == RTE_TM_WRED_PROFILE_ID_NONE || wp == NULL)
      return -rte_tm_error_set(error, EINVAL,
    ...
    if (params->leaf.wred.n_shared_wred_contexts != 0)
      return -rte_tm_error_set(
          error, EINVAL, RTE_TM_ERROR_TYPE_NODE_PARAMS_N_SHARED_WRED_CONTEXTS,
          NULL, rte_strerror(EINVAL));
  }
...
static int node_add_check(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  if (tm_node_search(dev, node_id))
    return -rte_tm_error_set(error, EEXIST, RTE_TM_ERROR_TYPE_NODE_ID, NULL,
                             rte_strerror(EEXIST));
  ...
  if (parent_node_id == RTE_TM_NODE_ID_NULL) {
    ...
    if (tm_root_node_present(dev))
      return -rte_tm_error_set(error, EEXIST,
                               RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID, NULL,
                               rte_strerror(EEXIST));
  } else {
  ...
  if (params == NULL)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_NODE_PARAMS, NULL,
                             rte_strerror(EINVAL));

  /* params: per level checks */
  switch (level) {
  case TM_NODE_LEVEL_PORT:
    status = node_add_check_port(dev, node_id, parent_node_id, priority, weight,
  ...
  case TM_NODE_LEVEL_SUBPORT:
    status = node_add_check_subport(dev, node_id, parent_node_id, priority,
  ...
  case TM_NODE_LEVEL_PIPE:
    status = node_add_check_pipe(dev, node_id, parent_node_id, priority, weight,
                                 level_id, params, error);
    if (status)
      return status;
    break;
  ...
  case TM_NODE_LEVEL_TC:
    status = node_add_check_tc(dev, node_id, parent_node_id, priority, weight,
                               level_id, params, error);
    if (status)
      return status;
    break;
  ...
  case TM_NODE_LEVEL_QUEUE:
    status = node_add_check_queue(dev, node_id, parent_node_id, priority,
                                  weight, level_id, params, error);
    if (status)
      return status;
    break;
...
static int pmd_tm_node_add(struct rte_eth_dev *dev, uint32_t node_id,
                           uint32_t parent_node_id, uint32_t priority,
                           uint32_t weight, uint32_t level_id,
                           struct rte_tm_node_params *params,
                           struct rte_tm_error *error) {
  ...
  if (n->level == TM_NODE_LEVEL_QUEUE && params->leaf.cman == RTE_TM_CMAN_WRED)
    n->wred_profile =
        tm_wred_profile_search(dev, params->leaf.wred.wred_profile_id);
...
static int pmd_tm_node_delete(struct rte_eth_dev *dev, uint32_t node_id,
                              struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_node *n;
  uint32_t i;
  ...
  /* Check unused */
  if (n->n_children)
    return -rte_tm_error_set(error, EBUSY, RTE_TM_ERROR_TYPE_NODE_ID, NULL,
                             rte_strerror(EBUSY));
  ...
  for (i = 0; i < n->params.n_shared_shapers; i++) {
    ...
    ss = tm_shared_shaper_search(dev, n->params.shared_shaper_id[i]);
    ss->n_users--;
  }
...
static void pipe_profile_build(struct rte_eth_dev *dev, struct tm_node *np,
  ...
  TAILQ_FOREACH(nt, nl, node) {
    ...
    if (nt->level != TM_NODE_LEVEL_TC || nt->parent_node_id != np->node_id)
      continue;

    pp->tc_rate[nt->priority] = nt->shaper_profile->params.peak.rate;

    /* Queue */
    TAILQ_FOREACH(nq, nl, node) {

      if (nq->level != TM_NODE_LEVEL_QUEUE || nq->parent_node_id != nt->node_id)
        continue;
...
static int pipe_profile_free_exists(struct rte_eth_dev *dev,
...
static int pipe_profile_exists(struct rte_eth_dev *dev,
                               struct rte_sched_pipe_params *pp,
                               uint32_t *pipe_profile_id) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_params *t = &p->soft.tm.params;
...
static void pipe_profile_install(struct rte_eth_dev *dev,
                                 struct rte_sched_pipe_params *pp,
                                 uint32_t pipe_profile_id) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_params *t = &p->soft.tm.params;
...
static void pipe_profile_mark(struct rte_eth_dev *dev, uint32_t subport_id,
                              uint32_t pipe_id, uint32_t pipe_profile_id) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_hierarchy *h = &p->soft.tm.h;
  struct tm_params *t = &p->soft.tm.params;
  ...
  n_pipes_per_subport =
      h->n_tm_nodes[TM_NODE_LEVEL_PIPE] / h->n_tm_nodes[TM_NODE_LEVEL_SUBPORT];
  pos = subport_id * n_pipes_per_subport + pipe_id;

  t->pipe_to_profile[pos] = pipe_profile_id;
}

static struct rte_sched_pipe_params *pipe_profile_get(struct rte_eth_dev *dev,
                                                      struct tm_node *np) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_hierarchy *h = &p->soft.tm.h;
  struct tm_params *t = &p->soft.tm.params;
...
static int pipe_profiles_generate(struct rte_eth_dev *dev) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_hierarchy *h = &p->soft.tm.h;
  struct tm_node_list *nl = &h->nodes;
  ...
  TAILQ_FOREACH(ns, nl, node) {
    ...
    TAILQ_FOREACH(np, nl, node) {
      ...
      if (np->level != TM_NODE_LEVEL_PIPE || np->parent_node_id != ns->node_id)
        continue;
...
static struct tm_wred_profile *tm_tc_wred_profile_get(struct rte_eth_dev *dev,
                                                      uint32_t tc_id) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_hierarchy *h = &p->soft.tm.h;
  struct tm_node_list *nl = &h->nodes;
  ...
  TAILQ_FOREACH(nq, nl, node) {
    if (nq->level != TM_NODE_LEVEL_QUEUE || nq->parent_node->priority != tc_id)
      continue;
...
static void wred_profiles_set(struct rte_eth_dev *dev, uint32_t subport_id) {
  struct pmd_internals *p = dev->data->dev_private;
  ...
  for (tc_id = 0; tc_id < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; tc_id++)
    for (color = RTE_COLOR_GREEN; color < RTE_COLORS; color++) {
      struct rte_red_params *dst = &pp->cman_params->red_params[tc_id][color];
      struct tm_wred_profile *src_wp = tm_tc_wred_profile_get(dev, tc_id);
      struct rte_tm_red_params *src = &src_wp->params.red_params[color];
...
tm_tc_shared_shaper_get(struct rte_eth_dev *dev, struct tm_node *tc_node) {
  return (tc_node->params.n_shared_shapers)
             ? tm_shared_shaper_search(dev, tc_node->params.shared_shaper_id[0])
             : NULL;
}

static struct tm_shared_shaper *
tm_subport_tc_shared_shaper_get(struct rte_eth_dev *dev,
                                struct tm_node *subport_node, uint32_t tc_id) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_node_list *nl = &p->soft.tm.h.nodes;
  struct tm_node *n;
  ...
  TAILQ_FOREACH(n, nl, node) {
    if (n->level != TM_NODE_LEVEL_TC ||
        n->parent_node->parent_node_id != subport_node->node_id ||
        n->priority != tc_id)
      continue;
...
static void subport_profile_mark(struct rte_eth_dev *dev, uint32_t subport_id,
                                 uint32_t subport_profile_id) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_params *t = &p->soft.tm.params;
...
static void subport_profile_install(struct rte_eth_dev *dev,
                                    struct rte_sched_subport_profile_params *sp,
                                    uint32_t subport_profile_id) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_params *t = &p->soft.tm.params;

  memcpy(&t->subport_profile[subport_profile_id], sp, sizeof(*sp));
  t->n_subport_profiles++;
}

static int subport_profile_free_exists(struct rte_eth_dev *dev,
                                       uint32_t *subport_profile_id) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_params *t = &p->soft.tm.params;
...
static void subport_profile_build(struct rte_eth_dev *dev, struct tm_node *np,
                                  struct rte_sched_subport_profile_params *sp) {
  uint32_t i;
  ...
  for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++) {
    ...
    ssp = (ss) ? tm_shaper_profile_search(dev, ss->shaper_profile_id)
               : np->shaper_profile;
...
static int subport_profiles_generate(struct rte_eth_dev *dev) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_hierarchy *h = &p->soft.tm.h;
  struct tm_node_list *nl = &h->nodes;
  struct tm_node *ns;
...
static int hierarchy_commit_check(struct rte_eth_dev *dev,
                                  struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_hierarchy *h = &p->soft.tm.h;
  struct tm_node_list *nl = &h->nodes;
  ...
  TAILQ_FOREACH(np, nl, node) {
    uint32_t mask = 0,
             mask_expected =
                 RTE_LEN2MASK(RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE, uint32_t);
  ...
  TAILQ_FOREACH(ns, nl, node) {
    ...
    TAILQ_FOREACH(nt, nl, node) {
      ...
      if (nt->level != TM_NODE_LEVEL_TC ||
          nt->parent_node->parent_node_id != ns->node_id)
        continue;
      ...
      if ((subport_ss == NULL && tc_ss != NULL) ||
          ...
          subport_ss->shared_shaper_id != tc_ss->shared_shaper_id)
        return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_UNSPECIFIED,
                                 NULL, rte_strerror(EINVAL));
    }
  }
  ...
  TAILQ_FOREACH(ss, ssl, node) {
    ...
    if (nt_any != NULL)
      TAILQ_FOREACH(nt, nl, node) {
        if (nt->level != TM_NODE_LEVEL_TC || nt->priority != nt_any->priority ||
  ...
  if (h->n_wred_profiles) {
    ...
    TAILQ_FOREACH(nq, nl, node) {
      ...
      if (nq->wred_profile == NULL ||
          nq->wred_profile->wred_profile_id != w[id]->wred_profile_id)
        return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_UNSPECIFIED,
                                 NULL, rte_strerror(EINVAL));
    }
  }
...
static void hierarchy_blueprints_create(struct rte_eth_dev *dev) {
  struct pmd_internals *p = dev->data->dev_private;
  struct tm_params *t = &p->soft.tm.params;
  struct tm_hierarchy *h = &p->soft.tm.h;
  ...
  t->port_params = (struct rte_sched_port_params){
      ...
      .mtu = dev->data->mtu,
      .frame_overhead = root->shaper_profile->params.pkt_length_adjust,
  ...
  TAILQ_FOREACH(n, nl, node) {
    ...
    t->subport_params[subport_id] = (struct rte_sched_subport_params){
        .n_pipes_per_subport_enabled = h->n_tm_nodes[TM_NODE_LEVEL_PIPE] /
...
static int pmd_tm_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
                                   struct rte_tm_error *error) {
  struct pmd_internals *p = dev->data->dev_private;
  int status;
...
static int update_pipe_weight(struct rte_eth_dev *dev, struct tm_node *np,
                              uint32_t weight) {
  struct pmd_internals *p = dev->data->dev_private;
  uint32_t pipe_id = tm_node_pipe_id(dev, np);
...
static int update_queue_weight(struct rte_eth_dev *dev, struct tm_node *nq,
                               uint32_t weight) {
  struct pmd_internals *p = dev->data->dev_private;
  ...
  uint32_t pipe_be_queue_id = queue_id - RTE_SCHED_TRAFFIC_CLASS_BE;
...
static int pmd_tm_node_parent_update(struct rte_eth_dev *dev, uint32_t node_id,
                                     uint32_t parent_node_id, uint32_t priority,
                                     uint32_t weight,
                                     struct rte_tm_error *error) {
  struct tm_node *n;
...
static int update_subport_rate(struct rte_eth_dev *dev, struct tm_node *ns,
                               struct tm_shaper_profile *sp) {
  struct pmd_internals *p = dev->data->dev_private;
  uint32_t subport_id = tm_node_subport_id(dev, ns);
  ...
  memcpy(&p->soft.tm.params.subport_profile[subport_profile_id], &profile1,
         sizeof(profile1));

  return 0;
}

static int update_pipe_rate(struct rte_eth_dev *dev, struct tm_node *np,
                            struct tm_shaper_profile *sp) {
  struct pmd_internals *p = dev->data->dev_private;
  uint32_t pipe_id = tm_node_pipe_id(dev, np);
...
static int update_tc_rate(struct rte_eth_dev *dev, struct tm_node *nt,
                          struct tm_shaper_profile *sp) {
  struct pmd_internals *p = dev->data->dev_private;
  uint32_t tc_id = tm_node_tc_id(dev, nt);
...
static int pmd_tm_node_shaper_update(struct rte_eth_dev *dev, uint32_t node_id,
                                     uint32_t shaper_profile_id,
                                     struct rte_tm_error *error) {
  struct tm_node *n;
  struct tm_shaper_profile *sp;
...
static inline uint32_t tm_port_queue_id(struct rte_eth_dev *dev,
...
static int read_port_stats(struct rte_eth_dev *dev, struct tm_node *nr,
  ...
  for (subport_id = 0; subport_id < n_subports_per_port; subport_id++) {
    ...
    /* Stats read */
    int status = rte_sched_subport_read_stats(SCHED(p), subport_id, &s, &tc_ov);
    if (status)
      return status;

    /* Stats accumulate */
    for (id = 0; id < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; id++) {
      nr->stats.n_pkts += s.n_pkts_tc[id] - s.n_pkts_tc_dropped[id];
      nr->stats.n_bytes += s.n_bytes_tc[id] - s.n_bytes_tc_dropped[id];
      nr->stats.leaf.n_pkts_dropped[RTE_COLOR_GREEN] += s.n_pkts_tc_dropped[id];
...
static int read_subport_stats(struct rte_eth_dev *dev, struct tm_node *ns,
                              struct rte_tm_node_stats *stats,
                              uint64_t *stats_mask, int clear) {
  struct pmd_internals *p = dev->data->dev_private;
  uint32_t subport_id = tm_node_subport_id(dev, ns);
  ...
  /* Stats accumulate */
  for (tc_id = 0; tc_id < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; tc_id++) {
    ns->stats.n_pkts += s.n_pkts_tc[tc_id] - s.n_pkts_tc_dropped[tc_id];
    ns->stats.n_bytes += s.n_bytes_tc[tc_id] - s.n_bytes_tc_dropped[tc_id];
...
static int read_pipe_stats(struct rte_eth_dev *dev, struct tm_node *np,
                           struct rte_tm_node_stats *stats,
                           uint64_t *stats_mask, int clear) {
  struct pmd_internals *p = dev->data->dev_private;
  ...
  for (i = 0; i < RTE_SCHED_QUEUES_PER_PIPE; i++) {
    ...
    uint32_t qid = tm_port_queue_id(dev, subport_id, pipe_id, tc_id, queue_id);

    int status = rte_sched_queue_read_stats(SCHED(p), qid, &s, &qlen);
    if (status)
      return status;
    ...
    np->stats.leaf.n_bytes_dropped[RTE_COLOR_GREEN] += s.n_bytes_dropped;
...
static int read_tc_stats(struct rte_eth_dev *dev, struct tm_node *nt,
  ...
  if (tc_id < RTE_SCHED_TRAFFIC_CLASS_BE) {
    qid = tm_port_queue_id(dev, subport_id, pipe_id, tc_id, 0);

    status = rte_sched_queue_read_stats(SCHED(p), qid, &s, &qlen);
    if (status)
      return status;
    ...
    nt->stats.leaf.n_bytes_dropped[RTE_COLOR_GREEN] += s.n_bytes_dropped;
    nt->stats.leaf.n_pkts_queued = qlen;
  } else {
    for (i = 0; i < RTE_SCHED_BE_QUEUES_PER_PIPE; i++) {
      qid = tm_port_queue_id(dev, subport_id, pipe_id, tc_id, i);
...
static int read_queue_stats(struct rte_eth_dev *dev, struct tm_node *nq,
                            struct rte_tm_node_stats *stats,
                            uint64_t *stats_mask, int clear) {
  struct pmd_internals *p = dev->data->dev_private;
  struct rte_sched_queue_stats s;
  ...
  nq->stats.leaf.n_bytes_dropped[RTE_COLOR_GREEN] += s.n_bytes_dropped;
...
static int pmd_tm_node_stats_read(struct rte_eth_dev *dev, uint32_t node_id,
                                  struct rte_tm_node_stats *stats,
                                  uint64_t *stats_mask, int clear,
                                  struct rte_tm_error *error) {
  struct tm_node *n;
