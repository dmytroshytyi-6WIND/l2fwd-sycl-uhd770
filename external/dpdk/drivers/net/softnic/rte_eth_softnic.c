...
static const char *const pmd_valid_args[] = {PMD_PARAM_FIRMWARE,
                                             PMD_PARAM_CONN_PORT,
                                             PMD_PARAM_CPU_ID,
                                             PMD_PARAM_SC,
...
#define PMD_LOG(level, fmt, args...)                                           \
  rte_log(RTE_LOG_##level, pmd_softnic_logtype, "%s(): " fmt "\n", __func__,   \
          ##args)

static int pmd_dev_infos_get(struct rte_eth_dev *dev __rte_unused,
...
static int pmd_dev_configure(struct rte_eth_dev *dev __rte_unused) { return 0; }

static int pmd_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
                              uint16_t nb_rx_desc,
  ...
  swq = softnic_swq_create(p, name, &params);
  if (swq == NULL)
    return -1;
...
static int
pmd_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id,
...
static int pmd_dev_start(struct rte_eth_dev *dev) {
  struct pmd_internals *p = dev->data->dev_private;
  int status;

  /* Firmware */
  status = softnic_cli_script_process(p, p->params.firmware,
...
static int pmd_dev_stop(struct rte_eth_dev *dev) {
  struct pmd_internals *p = dev->data->dev_private;
...
static void pmd_free(struct pmd_internals *p) {
  if (p == NULL)
    return;
...
static int pmd_dev_close(struct rte_eth_dev *dev) {
...
static int pmd_link_update(struct rte_eth_dev *dev __rte_unused,
                           int wait_to_complete __rte_unused) {
  return 0;
}

static int pmd_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
                            const struct rte_flow_ops **ops) {
  *ops = &pmd_flow_ops;
  return 0;
}

static int pmd_tm_ops_get(struct rte_eth_dev *dev __rte_unused, void *arg) {
  *(const struct rte_tm_ops **)arg = &pmd_tm_ops;

  return 0;
}

static int pmd_mtr_ops_get(struct rte_eth_dev *dev __rte_unused, void *arg) {
...
static uint16_t pmd_rx_pkt_burst(void *rxq, struct rte_mbuf **rx_pkts,
                                 uint16_t nb_pkts) {
  return (uint16_t)rte_ring_sc_dequeue_burst(rxq, (void **)rx_pkts, nb_pkts,
                                             NULL);
}

static uint16_t pmd_tx_pkt_burst(void *txq, struct rte_mbuf **tx_pkts,
                                 uint16_t nb_pkts) {
  return (uint16_t)rte_ring_sp_enqueue_burst(txq, (void **)tx_pkts, nb_pkts,
                                             NULL);
}

static void *pmd_init(struct pmd_params *params) {
  struct pmd_internals *p;
  int status;

  p = rte_zmalloc_socket(params->name, sizeof(struct pmd_internals), 0,
                         params->cpu_id);
  if (p == NULL)
    return NULL;
...
static int pmd_ethdev_register(struct rte_vdev_device *vdev,
                               struct pmd_params *params, void *dev_private) {
...
static int get_string(const char *key __rte_unused, const char *value,
                      void *extra_args) {
  if (!value || !extra_args)
    return -EINVAL;
...
static int get_uint32(const char *key __rte_unused, const char *value,
                      void *extra_args) {
  if (!value || !extra_args)
    return -EINVAL;
...
static int get_uint16(const char *key __rte_unused, const char *value,
                      void *extra_args) {
  if (!value || !extra_args)
    return -EINVAL;
...
static int pmd_parse_args(struct pmd_params *p, const char *params) {
  ...
  /* Set default values */
  memset(p, 0, sizeof(*p));
  if (rte_strscpy(p->firmware, SOFTNIC_FIRMWARE, sizeof(p->firmware)) < 0) {
    PMD_LOG(WARNING, "\"%s\": firmware path should be shorter than %zu",
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_FIRMWARE) == 1) {
    ret =
        rte_kvargs_process(kvlist, PMD_PARAM_FIRMWARE, &get_string, &firmware);
    if (ret < 0)
      goto out_free;

    if (rte_strscpy(p->firmware, firmware, sizeof(p->firmware)) < 0) {
      PMD_LOG(WARNING,
              "\"%s\": "
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_CONN_PORT) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_CONN_PORT, &get_uint16,
                             &p->conn_port);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_CPU_ID) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_CPU_ID, &get_uint32, &p->cpu_id);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_SC) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_SC, &get_uint32, &p->sc);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_N_QUEUES) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_N_QUEUES, &get_uint32,
                             &p->tm.n_queues);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE0) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE0, &get_uint32,
                             &p->tm.qsize[0]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE1) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE1, &get_uint32,
                             &p->tm.qsize[1]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE2) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE2, &get_uint32,
                             &p->tm.qsize[2]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE3) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE3, &get_uint32,
                             &p->tm.qsize[3]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE4) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE4, &get_uint32,
                             &p->tm.qsize[4]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE5) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE5, &get_uint32,
                             &p->tm.qsize[5]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE6) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE6, &get_uint32,
                             &p->tm.qsize[6]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE7) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE7, &get_uint32,
                             &p->tm.qsize[7]);
    if (ret < 0)
      goto out_free;
  }
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE8) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE8, &get_uint32,
                             &p->tm.qsize[8]);
    if (ret < 0)
      goto out_free;
  }
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE9) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE9, &get_uint32,
                             &p->tm.qsize[9]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE10) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE10, &get_uint32,
                             &p->tm.qsize[10]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE11) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE11, &get_uint32,
                             &p->tm.qsize[11]);
    if (ret < 0)
      goto out_free;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_PARAM_TM_QSIZE12) == 1) {
    ret = rte_kvargs_process(kvlist, PMD_PARAM_TM_QSIZE12, &get_uint32,
                             &p->tm.qsize[12]);
    if (ret < 0)
      goto out_free;
  }
...
static int pmd_probe(struct rte_vdev_device *vdev) {
  struct pmd_params p;
  const char *params;
  int status = 0;
  ...
  if (rte_strscpy(p.name, name, sizeof(p.name)) < 0) {
    PMD_LOG(WARNING, "\"%s\": device name should be shorter than %zu", name,
            sizeof(p.name));
    return -EINVAL;
  }
...
static int pmd_remove(struct rte_vdev_device *vdev) {
...
RTE_PMD_REGISTER_PARAM_STRING(
    net_softnic, PMD_PARAM_FIRMWARE
    "=<string> " PMD_PARAM_CONN_PORT "=<uint16> " PMD_PARAM_CPU_ID
    "=<uint32> " PMD_PARAM_TM_N_QUEUES "=<uint32> " PMD_PARAM_TM_QSIZE0
    "=<uint32> " PMD_PARAM_TM_QSIZE1 "=<uint32> " PMD_PARAM_TM_QSIZE2
    "=<uint32> " PMD_PARAM_TM_QSIZE3 "=<uint32>" PMD_PARAM_TM_QSIZE4
    "=<uint32> " PMD_PARAM_TM_QSIZE5 "=<uint32> " PMD_PARAM_TM_QSIZE6
    "=<uint32> " PMD_PARAM_TM_QSIZE7 "=<uint32> " PMD_PARAM_TM_QSIZE8
    "=<uint32> " PMD_PARAM_TM_QSIZE9 "=<uint32> " PMD_PARAM_TM_QSIZE10
    "=<uint32> " PMD_PARAM_TM_QSIZE11 "=<uint32>" PMD_PARAM_TM_QSIZE12
    "=<uint32>");

int rte_pmd_softnic_manage(uint16_t port_id) {
