...
struct softnic_swq *softnic_swq_find(struct pmd_internals *p,
                                     const char *name) {
  struct softnic_swq *swq;
...
struct softnic_swq *softnic_swq_create(struct pmd_internals *p,
  ...
  /* Check input params */
  if (name == NULL || softnic_swq_find(p, name) || params == NULL ||
      params->size == 0)
    return NULL;

  /* Resource create */
  snprintf(ring_name, sizeof(ring_name), "%s_%s", p->params.name, name);

  r = rte_ring_create(ring_name, params->size, p->params.cpu_id, flags);
