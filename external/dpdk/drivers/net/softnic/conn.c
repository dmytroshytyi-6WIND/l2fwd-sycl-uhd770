...
struct softnic_conn *softnic_conn_init(struct softnic_conn_params *p) {
  ...
  status = bind(fd_server, (struct sockaddr *)&server_address,
...
void softnic_conn_free(struct softnic_conn *conn) {
  if (conn == NULL)
    return;
...
int softnic_conn_poll_for_conn(struct softnic_conn *conn) {
  ...
  status = epoll_ctl(conn->fd_client_group, EPOLL_CTL_ADD, fd_client, &event);
  if (status == -1) {
    close(fd_client);
    return -1;
  }
  ...
  /* Client */
  status = write(fd_client, conn->welcome, strlen(conn->welcome));
  if (status == -1) {
    close(fd_client);
    return -1;
  }

  status = write(fd_client, conn->prompt, strlen(conn->prompt));
  if (status == -1) {
    close(fd_client);
    return -1;
  }
...
static int data_event_handle(struct softnic_conn *conn, int fd_client) {
  ...
  for (i = 0; i < len; i++) {
    if (conn->buf[i] == '\n') {
      ...
      if (n) {
        status = write(fd_client, conn->msg_out, n);
        if (status == -1)
          return status;
      }
      ...
      status = write(fd_client, MSG_CMD_TOO_LONG, strlen(MSG_CMD_TOO_LONG));
      if (status == -1)
        return status;
...
static int control_event_handle(struct softnic_conn *conn, int fd_client) {
  int status;

  status = epoll_ctl(conn->fd_client_group, EPOLL_CTL_DEL, fd_client, NULL);
  if (status == -1)
    return -1;
...
int softnic_conn_poll_for_msg(struct softnic_conn *conn) {
  struct epoll_event event;
  ...
  /* Client group */
  status = epoll_wait(conn->fd_client_group, &event, 1, 0);
  if (status == -1)
    return -1;
  if (status == 0)
    return 0;
