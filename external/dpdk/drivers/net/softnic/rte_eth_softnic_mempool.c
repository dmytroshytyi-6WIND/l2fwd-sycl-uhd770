...
softnic_mempool_create(struct pmd_internals *p, const char *name,
  ...
  /* Check input params */
  if (name == NULL || softnic_mempool_find(p, name) || params == NULL ||
      params->buffer_size < BUFFER_SIZE_MIN || params->pool_size == 0)
    return NULL;

  /* Resource create */
  snprintf(mempool_name, sizeof(mempool_name), "%s_%s", p->params.name, name);

  m = rte_pktmbuf_pool_create(
      mempool_name, params->pool_size, params->cache_size, 0,
      params->buffer_size - sizeof(struct rte_mbuf), p->params.cpu_id);
