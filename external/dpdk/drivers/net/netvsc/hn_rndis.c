...
#define HN_NDIS_TXCSUM_CAP_TCP6                                                \
  (NDIS_TXCSUM_CAP_TCP6 | NDIS_TXCSUM_CAP_TCP6OPT | NDIS_TXCSUM_CAP_IP6EXT)
...
static inline uint32_t hn_rndis_rid(struct hn_data *hv) {
  uint32_t rid;
...
void hn_rndis_dump(const void *buf) {
  ...
  case RNDIS_PACKET_MSG: {
    ...
    rte_log(
        RTE_LOG_DEBUG, hn_logtype_driver,
        "RNDIS_MSG_PACKET (len %u, data %u:%u, # oob %u %u:%u, pkt %u:%u)\n",
        rndis_msg->pkt.len, rndis_msg->pkt.dataoffset, rndis_msg->pkt.datalen,
        rndis_msg->pkt.oobdataelements, rndis_msg->pkt.oobdataoffset,
        rndis_msg->pkt.oobdatalen, rndis_msg->pkt.pktinfooffset,
        rndis_msg->pkt.pktinfolen);

    ppi = (const struct rndis_pktinfo *)((const char *)buf +
                                         RNDIS_PACKET_MSG_OFFSET_ABS(
    ...
    while (ppi_len > 0) {
      ...
      rte_log(RTE_LOG_DEBUG, hn_logtype_driver,
              ...
              ppi->type, ppi->offset, *(const uint32_t *)ppi_data);
      if (ppi->size == 0)
        break;
      ppi_len -= ppi->size;
      ppi = (const struct rndis_pktinfo *)((const char *)ppi + ppi->size);
    }
    break;
  }
  case RNDIS_INITIALIZE_MSG:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver,
            ...
            rndis_msg->init_request.len, rndis_msg->init_request.rid,
  ...
  case RNDIS_INITIALIZE_CMPLT:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver,
            ...
            rndis_msg->init_complete.len, rndis_msg->init_complete.rid,
            rndis_msg->init_complete.status, rndis_msg->init_complete.ver_major,
            rndis_msg->init_complete.ver_minor,
            rndis_msg->init_complete.devflags,
            rndis_msg->init_complete.pktmaxsz,
            rndis_msg->init_complete.pktmaxcnt, rndis_msg->init_complete.align);
    break;

  case RNDIS_HALT_MSG:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver, "RNDIS_HALT (len %u id %#x)\n",
  ...
  case RNDIS_QUERY_MSG:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver,
            ...
            rndis_msg->query_request.len, rndis_msg->query_request.rid,
            rndis_msg->query_request.oid, rndis_msg->query_request.infobuflen,
  ...
  case RNDIS_QUERY_CMPLT:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver,
            ...
            rndis_msg->query_complete.len, rndis_msg->query_complete.rid,
  ...
  case RNDIS_SET_MSG:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver,
            ...
            rndis_msg->set_request.len, rndis_msg->set_request.rid,
            rndis_msg->set_request.oid, rndis_msg->set_request.infobuflen,
  ...
  case RNDIS_SET_CMPLT:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver,
            ...
            rndis_msg->set_complete.len, rndis_msg->set_complete.rid,
  ...
  case RNDIS_INDICATE_STATUS_MSG:
    rte_log(
        ...
        rndis_msg->indicate_status.len, rndis_msg->indicate_status.status,
        rndis_msg->indicate_status.stbuflen,
        rndis_msg->indicate_status.stbufoffset);
    break;

  case RNDIS_RESET_MSG:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver, "RNDIS_RESET (len %u, id %#x)\n",
            rndis_msg->reset_request.len, rndis_msg->reset_request.rid);
    break;

  case RNDIS_RESET_CMPLT:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver,
            "RNDIS_RESET_C (len %u, status %#x address %#x)\n",
            rndis_msg->reset_complete.len, rndis_msg->reset_complete.status,
  ...
  case RNDIS_KEEPALIVE_MSG:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver,
            ...
            rndis_msg->keepalive_request.len, rndis_msg->keepalive_request.rid);
    break;
  ...
  default:
    rte_log(RTE_LOG_DEBUG, hn_logtype_driver, "RNDIS type %#x len %u\n",
...
static int hn_nvs_send_rndis_ctrl(struct vmbus_channel *chan, const void *req,
                                  uint32_t reqlen)

{
  struct hn_nvs_rndis nvs_rndis = {.type = NVS_TYPE_RNDIS,
  ...
  if (unlikely(reqlen > rte_mem_page_size())) {
    PMD_DRV_LOG(ERR, "RNDIS request %u greater than page size", reqlen);
    return -EINVAL;
  }
  ...
  return hn_nvs_send_sglist(chan, &sg, 1, &nvs_rndis, sizeof(nvs_rndis), 0U,
                            NULL);
}
...
void hn_rndis_link_status(struct rte_eth_dev *dev, const void *msg) {
  ...
  case RNDIS_STATUS_MEDIA_DISCONNECT:
    ...
    break;
  default:
    PMD_DRV_LOG(NOTICE, "unknown RNDIS indication: %#x", indicate->status);
  }
}

/* Callback from hn_process_events when response is visible */
void hn_rndis_receive_response(struct hn_data *hv, const void *data,
                               uint32_t len) {
  ...
  if (len < sizeof(3 * sizeof(uint32_t))) {
    PMD_DRV_LOG(ERR, "missing RNDIS header %u", len);
    return;
  }

  if (len < hdr->len) {
    PMD_DRV_LOG(ERR, "truncated RNDIS response %u", len);
    return;
  }

  if (len > sizeof(hv->rndis_resp)) {
    PMD_DRV_LOG(NOTICE, "RNDIS response exceeds buffer");
    len = sizeof(hv->rndis_resp);
  }

  if (hdr->rid == 0) {
    PMD_DRV_LOG(NOTICE, "RNDIS response id zero!");
  }
  ...
  if (rte_atomic32_cmpset(&hv->rndis_pending, hdr->rid, 0) == 0) {
    PMD_DRV_LOG(NOTICE, "received id %#x pending id %#x", hdr->rid,
                (uint32_t)hv->rndis_pending);
  }
}

/* Do request/response transaction */
static int hn_rndis_exec1(struct hn_data *hv, const void *req, uint32_t reqlen,
  ...
  if (comp_len > sizeof(hv->rndis_resp)) {
    PMD_DRV_LOG(ERR, "Expected completion size %u exceeds buffer %zu", comp_len,
                sizeof(hv->rndis_resp));
    return -EIO;
  }
  ...
  if (comp != NULL && rte_atomic32_cmpset(&hv->rndis_pending, 0, rid) == 0) {
    PMD_DRV_LOG(ERR, "Request already pending");
    return -EBUSY;
  }
  ...
  if (comp) {
    ...
    while (hv->rndis_pending == rid) {
      ...
      if (time(NULL) - start > RNDIS_TIMEOUT_SEC) {
        PMD_DRV_LOG(ERR, "RNDIS response timed out");
...
static int hn_rndis_execute(struct hn_data *hv, uint32_t rid, const void *req,
                            uint32_t reqlen, void *comp, uint32_t comp_len,
                            uint32_t comp_type) {
  ...
  if (unlikely(hdr->type != comp_type)) {
    PMD_DRV_LOG(ERR, "unexpected RNDIS response complete %#x expect %#x",
                hdr->type, comp_type);
...
static int hn_rndis_query(struct hn_data *hv, uint32_t oid, const void *idata,
                          uint32_t idlen, void *odata, uint32_t odlen) {
  ...
  error =
      hn_rndis_execute(hv, rid, req, reqlen, comp, comp_len, RNDIS_QUERY_CMPLT);

  if (error)
    goto done;

  if (comp->status != RNDIS_STATUS_SUCCESS) {
    PMD_DRV_LOG(ERR, "RNDIS query 0x%08x failed: status 0x%08x", oid,
                comp->status);
    error = -EINVAL;
    goto done;
  }
...
static int hn_rndis_halt(struct hn_data *hv) {
  struct rndis_halt_req *halt;
...
static int hn_rndis_query_hwcaps(struct hn_data *hv,
  ...
  error = hn_rndis_query(hv, OID_TCP_OFFLOAD_HARDWARE_CAPABILITIES, &in, size,
                         caps, caps_len);
  if (error)
    return error;
  ...
  if (caps->ndis_hdr.ndis_rev < NDIS_OFFLOAD_REV_1) {
    PMD_DRV_LOG(NOTICE, "invalid NDIS objrev 0x%02x", caps->ndis_hdr.ndis_rev);
    return -EINVAL;
  }
  if (caps->ndis_hdr.ndis_size > caps_len) {
    ...
    PMD_DRV_LOG(NOTICE, "invalid NDIS objsize %u", caps->ndis_hdr.ndis_size);
    return -EINVAL;
  }
...
int hn_rndis_query_rsscaps(struct hn_data *hv, unsigned int *rxr_cnt0) {
  ...
  *rxr_cnt0 = 0;
  ...
  error = hn_rndis_query(hv, OID_GEN_RECEIVE_SCALE_CAPABILITIES, &in,
                         NDIS_RSS_CAPS_SIZE, &caps, caps_len);
  if (error)
    return error;

  PMD_INIT_LOG(DEBUG, "RX rings %u indirect %u caps %#x", caps.ndis_nrxr,
               caps.ndis_nind, caps.ndis_caps);
  /*
   * Preliminary verification.
   */
  if (caps.ndis_hdr.ndis_type != NDIS_OBJTYPE_RSS_CAPS) {
    PMD_DRV_LOG(ERR, "invalid NDIS objtype 0x%02x", caps.ndis_hdr.ndis_type);
    return -EINVAL;
  }
  if (caps.ndis_hdr.ndis_rev < NDIS_RSS_CAPS_REV_1) {
    PMD_DRV_LOG(ERR, "invalid NDIS objrev 0x%02x", caps.ndis_hdr.ndis_rev);
    return -EINVAL;
  }
  if (caps.ndis_hdr.ndis_size > caps_len) {
    PMD_DRV_LOG(ERR, "invalid NDIS objsize %u, data size %u",
                caps.ndis_hdr.ndis_size, caps_len);
    return -EINVAL;
  } else if (caps.ndis_hdr.ndis_size < NDIS_RSS_CAPS_SIZE_6_0) {
    PMD_DRV_LOG(ERR, "invalid NDIS objsize %u", caps.ndis_hdr.ndis_size);
    return -EINVAL;
  }
  ...
  if (caps.ndis_hdr.ndis_size == NDIS_RSS_CAPS_SIZE &&
    ...
    if (caps.ndis_nind > NDIS_HASH_INDCNT) {
      PMD_DRV_LOG(ERR, "too many RSS indirect table entries %u",
                  caps.ndis_nind);
      return -EOPNOTSUPP;
    }
    if (!rte_is_power_of_2(caps.ndis_nind)) {
      PMD_DRV_LOG(ERR, "RSS indirect table size is not power-of-2 %u",
                  caps.ndis_nind);
    }
  ...
  if (indsz < rxr_cnt) {
    PMD_DRV_LOG(NOTICE, "# of RX rings (%d) > RSS indirect table size %d",
                rxr_cnt, indsz);
    rxr_cnt = indsz;
  }
  ...
  if (caps.ndis_caps & NDIS_RSS_CAP_IPV4)
    hv->rss_offloads |= RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_NONFRAG_IPV4_TCP |
                        RTE_ETH_RSS_NONFRAG_IPV4_UDP;
  if (caps.ndis_caps & NDIS_RSS_CAP_IPV6)
    hv->rss_offloads |= RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_NONFRAG_IPV6_TCP;
  if (caps.ndis_caps & NDIS_RSS_CAP_IPV6_EX)
    hv->rss_offloads |= RTE_ETH_RSS_IPV6_EX | RTE_ETH_RSS_IPV6_TCP_EX;
...
static int hn_rndis_set(struct hn_data *hv, uint32_t oid, const void *data,
                        uint32_t dlen) {
  ...
  error =
      hn_rndis_execute(hv, rid, req, reqlen, &comp, comp_len, RNDIS_SET_CMPLT);
  if (error) {
    PMD_DRV_LOG(ERR, "exec RNDIS set %#" PRIx32 " failed", oid);
    error = EIO;
    goto done;
  }

  if (comp.status != RNDIS_STATUS_SUCCESS) {
    PMD_DRV_LOG(ERR, "RNDIS set %#" PRIx32 " failed: status %#" PRIx32, oid,
                comp.status);
    error = EIO;
    goto done;
  }
...
int hn_rndis_conf_offload(struct hn_data *hv, uint64_t tx_offloads,
...
int hn_rndis_set_rxfilter(struct hn_data *hv, uint32_t filter) {
  ...
  error =
      hn_rndis_set(hv, OID_GEN_CURRENT_PACKET_FILTER, &filter, sizeof(filter));
  if (error) {
    PMD_DRV_LOG(ERR, "set RX filter %#" PRIx32 " failed: %d", filter, error);
  } else {
...
int hn_rndis_conf_rss(struct hn_data *hv, uint32_t flags) {
  ...
  error =
      hn_rndis_set(hv, OID_GEN_RECEIVE_SCALE_PARAMETERS, &rssp, sizeof(rssp));
  if (error != 0) {
    PMD_DRV_LOG(ERR, "RSS config num queues=%u failed: %d", hv->num_queues,
                error);
  }
  return error;
}
...
static int hn_rndis_init(struct hn_data *hv) {
  ...
  error = hn_rndis_execute(hv, rid, req, sizeof(*req), &comp, comp_len,
                           RNDIS_INITIALIZE_CMPLT);
  if (error)
    goto done;

  if (comp.status != RNDIS_STATUS_SUCCESS) {
    PMD_DRV_LOG(ERR, "RNDIS init failed: status 0x%08x", comp.status);
    error = -EIO;
    goto done;
  }
  ...
  if (hv->rndis_agg_align < sizeof(uint32_t)) {
     ...
     * too low.
     */
    PMD_DRV_LOG(NOTICE, "fixup RNDIS aggpkt align: %u -> %zu",
                hv->rndis_agg_align, sizeof(uint32_t));
    hv->rndis_agg_align = sizeof(uint32_t);
  }

  PMD_INIT_LOG(
      INFO, "RNDIS ver %u.%u, aggpkt size %u, aggpkt cnt %u, aggpkt align %u",
      comp.ver_major, comp.ver_minor, hv->rndis_agg_size, hv->rndis_agg_pkts,
      hv->rndis_agg_align);
  error = 0;
done:
  rte_free(req);
...
int hn_rndis_get_eaddr(struct hn_data *hv, uint8_t *eaddr) {
  ...
  error = hn_rndis_query(hv, OID_802_3_PERMANENT_ADDRESS, NULL, 0, eaddr,
                         eaddr_len);
  if (error)
    return error;

  PMD_DRV_LOG(INFO, "MAC address " RTE_ETHER_ADDR_PRT_FMT, eaddr[0], eaddr[1],
...
int hn_rndis_get_linkspeed(struct hn_data *hv) {
  return hn_rndis_query(hv, OID_GEN_LINK_SPEED, NULL, 0, &hv->link_speed,
                        sizeof(uint32_t));
}
