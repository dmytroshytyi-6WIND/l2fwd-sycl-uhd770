...
static inline struct vmbus_channel *hn_primary_chan(const struct hn_data *hv) {
 ...
 */
static inline struct rte_eth_dev *hn_get_vf_dev(const struct hn_data *hv) {
...
int hn_vf_tx_queue_status(struct hn_data *hv, uint16_t queue_id,
                          uint16_t offset);
