...
#define HN_RXINFO_ALL                                                          \
  (HN_RXINFO_VLAN | HN_RXINFO_CSUM | HN_RXINFO_HASHINF | HN_RXINFO_HASHVAL)
...
static void hn_update_packet_stats(struct hn_stats *stats,
...
static inline uint32_t hn_rndis_pktmsg_offset(uint32_t ofs) {
  return ofs - offsetof(struct rndis_packet_msg, dataoffset);
}

static void hn_txd_init(struct rte_mempool *mp __rte_unused, void *opaque,
...
int hn_chim_init(struct rte_eth_dev *dev) {
  ...
  hv->chim_bmem =
      rte_zmalloc("hn_chim_bitmap", chim_bmp_size, RTE_CACHE_LINE_SIZE);
...

{
  ...
  if (tx_free_thresh + 3 >= nb_desc) {
    PMD_INIT_LOG(
        ...
        nb_desc - 3, tx_free_thresh, dev->data->port_id, queue_idx);
    return -EINVAL;
  }
  ...
  snprintf(name, sizeof(name), "hn_txd_%u_%u", dev->data->port_id, queue_idx);

  PMD_INIT_LOG(DEBUG, "TX descriptor pool %s n=%u size=%zu", name, nb_desc,
               sizeof(struct hn_txdesc));

  txq->tx_rndis_mz = rte_memzone_reserve_aligned(
      name, nb_desc * HN_RNDIS_PKT_ALIGNED, rte_socket_id(),
  ...
  txq->txdesc_pool =
      rte_mempool_create(name, nb_desc, sizeof(struct hn_txdesc), 0, 0, NULL,
                         NULL, hn_txd_init, txq, dev->device->numa_node, 0);
  if (txq->txdesc_pool == NULL) {
    PMD_DRV_LOG(ERR, "mempool %s create failed: %d", name, rte_errno);
    goto error;
  }
  ...
  err = hn_vf_tx_queue_setup(dev, queue_idx, nb_desc, socket_id, tx_conf);
  if (err == 0) {
...
void hn_dev_tx_queue_info(struct rte_eth_dev *dev, uint16_t queue_id,
...
static void hn_nvs_send_completed(struct rte_eth_dev *dev, uint16_t queue_id,
                                  unsigned long xactid,
                                  const struct hn_nvs_rndis_ack *ack) {
  struct hn_data *hv = dev->data->dev_private;
  ...
  if (likely(ack->status == NVS_STATUS_OK)) {
    PMD_TX_LOG(DEBUG, "port %u:%u complete tx %u packets %u bytes %u",
               txq->port_id, txq->queue_id, txd->chim_index, txd->packets,
               txd->data_size);
...
static void hn_nvs_handle_comp(struct rte_eth_dev *dev, uint16_t queue_id,
  ...
  default:
    PMD_DRV_LOG(NOTICE, "unexpected send completion type %u", hdr->type);
  }
}

/* Parse per-packet info (meta data) */
static int hn_rndis_rxinfo(const void *info_data, unsigned int info_dlen,
  ...
  next:
    pi = (const struct rndis_pktinfo *)((const uint8_t *)pi + pi->size);
  }
...
static struct hn_rx_bufinfo *
hn_rx_buf_init(struct hn_rx_queue *rxq, const struct vmbus_chanpkt_rxbuf *pkt) {
...
static void hn_rxpkt(struct hn_rx_queue *rxq, struct hn_rx_bufinfo *rxb,
  ...
  if (hv->rx_extmbuf_enable && dlen > hv->rx_copybreak &&
    ...
    rte_pktmbuf_attach_extbuf(m, data, iova, dlen + headroom, shinfo);
    m->data_off = headroom;
    use_extbuf = true;
  } else {
    ...
    rte_memcpy(rte_pktmbuf_mtod(m, void *), data + headroom, dlen);
  }

  m->port = rxq->port_id;
  m->pkt_len = dlen;
  m->data_len = dlen;
  m->packet_type = rte_net_get_ptype(
      m, NULL, RTE_PTYPE_L2_MASK | RTE_PTYPE_L3_MASK | RTE_PTYPE_L4_MASK);
  ...
  if (info->csum_info != HN_NDIS_RXCSUM_INFO_INVALID) {
    ...
    if (info->csum_info &
        (NDIS_RXCSUM_INFO_UDPCS_OK | NDIS_RXCSUM_INFO_TCPCS_OK))
      m->ol_flags |= RTE_MBUF_F_RX_L4_CKSUM_GOOD;
    else if (info->csum_info &
             (NDIS_RXCSUM_INFO_TCPCS_FAILED | NDIS_RXCSUM_INFO_UDPCS_FAILED))
  ...
  PMD_RX_LOG(DEBUG,
             ...
             rxq->port_id, rxq->queue_id, rxb->xactid, m->pkt_len,
...
static void hn_rndis_rx_data(struct hn_rx_queue *rxq, struct hn_rx_bufinfo *rxb,
  ...
  if (unlikely(pkt->len < pkt->datalen + pkt->oobdatalen + pkt->pktinfolen))
    goto error;
  ...
  if (likely(pktinfo_len > 0)) {
    err =
        hn_rndis_rxinfo((const uint8_t *)pkt + pktinfo_off, pktinfo_len, &info);
    if (err)
      goto error;
  }
...
static void hn_rndis_receive(struct rte_eth_dev *dev, struct hn_rx_queue *rxq,
                             struct hn_rx_bufinfo *rxb, void *buf,
                             uint32_t len) {
  ...
  default:
    PMD_DRV_LOG(NOTICE, "unexpected RNDIS message (type %#x len %u)", hdr->type,
                len);
    break;
  }
}

static void hn_nvs_handle_rxbuf(struct rte_eth_dev *dev, struct hn_data *hv,
  ...
  if (unlikely(nvs_hdr->type != NVS_TYPE_RNDIS)) {
    PMD_RX_LOG(ERR, "nvs type %u, not RNDIS", nvs_hdr->type);
    return;
  }
  ...
  if (unlikely(pkt->rxbuf_id != NVS_RXBUF_SIG)) {
    PMD_RX_LOG(ERR, "invalid rxbuf_id 0x%08x", pkt->rxbuf_id);
    return;
  }

  count = pkt->rxbuf_cnt;
  if (unlikely(hlen < offsetof(struct vmbus_chanpkt_rxbuf, rxbuf[count]))) {
    PMD_RX_LOG(ERR, "invalid rxbuf_cnt %u", count);
    return;
  }

  if (pkt->hdr.xactid > hv->rxbuf_section_cnt) {
    PMD_RX_LOG(ERR, "invalid rxbuf section id %" PRIx64, pkt->hdr.xactid);
    return;
  }
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (unlikely(ofs + len > rxbuf_sz)) {
      PMD_RX_LOG(ERR, "%uth RNDIS msg overflow ofs %u, len %u", i, ofs, len);
      continue;
    }
    ...
    hn_rndis_receive(dev, rxq, rxb, rxbuf + ofs, len);
  }
...
static void hn_nvs_handle_notify(struct rte_eth_dev *dev,
  ...
  default:
    PMD_DRV_LOG(INFO, "got notify, nvs type %u", hdr->type);
  }
}

struct hn_rx_queue *hn_rx_queue_alloc(struct hn_data *hv, uint16_t queue_id,
                                      unsigned int socket_id) {
  struct hn_rx_queue *rxq;

  rxq = rte_zmalloc_socket("HN_RXQ", sizeof(*rxq), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (!rxq)
    return NULL;
  ...
  if (queue_id) {
    rxq->rxbuf_info = rte_calloc("HN_RXBUF_INFO", hv->rxbuf_section_cnt,
                                 sizeof(*rxq->rxbuf_info), RTE_CACHE_LINE_SIZE);

    if (!rxq->rxbuf_info) {
      PMD_DRV_LOG(ERR, "Could not allocate rxbuf info for queue %d\n",
                  queue_id);
...
int hn_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                          uint16_t nb_desc, unsigned int socket_id,
   ...
   */
  snprintf(ring_name, sizeof(ring_name), "hn_rx_%u_%u", dev->data->port_id,
           queue_idx);
  rxq->rx_ring =
      rte_ring_create(ring_name, rte_align32pow2(nb_desc), socket_id, 0);
  if (!rxq->rx_ring)
    goto fail;

  error = hn_vf_rx_queue_setup(dev, queue_idx, nb_desc, socket_id, rx_conf, mp);
  if (error)
    goto fail;
...
static void hn_rx_queue_free(struct hn_rx_queue *rxq, bool keep_primary) {
...
uint32_t hn_process_events(struct hn_data *hv, uint16_t queue_id,
  ...
  retry:
    ...
    if (unlikely(ret == -ENOBUFS)) {
      ...
      PMD_DRV_LOG(DEBUG, "event buffer expansion (need %u)", len);
      rxq->event_sz = len + len / 4;
      rxq->event_buf =
          rte_realloc(rxq->event_buf, rxq->event_sz, RTE_CACHE_LINE_SIZE);
      if (rxq->event_buf)
        goto retry;
    ...
    if (unlikely(ret <= 0)) {
      ...
      rte_exit(EXIT_FAILURE, "vmbus ring buffer error: %d", ret);
    }
...
static void hn_append_to_chim(struct hn_tx_queue *txq,
  ...
  for (; m; m = m->next) {
    ...
    rte_memcpy(buf + data_offs, rte_pktmbuf_mtod(m, const char *), len);
    data_offs += len;
  }
}
...

{
  ...
  PMD_TX_LOG(DEBUG, "port %u:%u tx %u size %u", txq->port_id, txq->queue_id,
             txd->chim_index, txd->chim_size);

  ret = hn_nvs_send(txq->chan, VMBUS_CHANPKT_FLAG_RC, &rndis, sizeof(rndis),
                    (uintptr_t)txd, need_sig);

  if (likely(ret == 0))
    hn_reset_txagg(txq);
  else if (ret == -EAGAIN) {
    PMD_TX_LOG(DEBUG, "port %u:%u channel full", txq->port_id, txq->queue_id);
 ...
 */
static void *hn_try_txagg(struct hn_data *hv, struct hn_tx_queue *txq,
  ...
  chim = (uint8_t *)hv->chim_res->addr + txd->chim_index * hv->chim_szmax;
...
static inline void *hn_rndis_pktinfo_append(struct rndis_packet_msg *pkt,
                                            uint32_t pi_dlen,
                                            uint32_t pi_type) {
...
static void hn_encap(struct rndis_packet_msg *pkt, uint16_t queue_id,
                     const struct rte_mbuf *m) {
  ...
  *pi_data = queue_id;
  ...
  if (m->ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    if (m->ol_flags & RTE_MBUF_F_TX_IPV6) {
      *pi_data = NDIS_LSO2_INFO_MAKEIPV6(hlen, m->tso_segsz);
    } else {
      *pi_data = NDIS_LSO2_INFO_MAKEIPV4(hlen, m->tso_segsz);
    }
  } else if (m->ol_flags & (RTE_MBUF_F_TX_TCP_CKSUM | RTE_MBUF_F_TX_UDP_CKSUM |
...
static unsigned int hn_get_slots(const struct rte_mbuf *m) {
  ...
  while (m) {
    ...
    slots += (offs + size + rte_mem_page_size() - 1) / rte_mem_page_size();
    m = m->next;
  }
...
static unsigned int hn_fill_sg(struct vmbus_gpa *sg, const struct rte_mbuf *m) {
  unsigned int segs = 0;
  ...
  while (m) {
    ...
    while (len > 0) {
      unsigned int bytes = RTE_MIN(len, rte_mem_page_size() - offset);
...
static int hn_xmit_sg(struct hn_tx_queue *txq, const struct hn_txdesc *txd,
                      const struct rte_mbuf *m, bool *need_sig) {
  ...
  addr = txq->tx_rndis_iova + ((char *)txd->rndis_pkt - (char *)txq->tx_rndis);
  ...
  PMD_TX_LOG(DEBUG, "port %u:%u tx %u segs %u size %u", txq->port_id,
             txq->queue_id, txd->chim_index, segs, nvs_rndis.chim_sz);

  return hn_nvs_send_sglist(txq->chan, sg, segs, &nvs_rndis, sizeof(nvs_rndis),
                            (uintptr_t)txd, need_sig);
}

uint16_t hn_xmit_pkts(void *ptxq, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (m->pkt_len <= hv->tx_copybreak && pkt_size <= txq->agg_szmax) {
      ...
      if (txq->agg_pktleft == 0 && hn_flush_txagg(txq, &need_sig))
        goto fail;
    } else {
...
static uint16_t hn_recv_vf(uint16_t vf_port, const struct hn_rx_queue *rxq,
...
uint16_t hn_recv_pkts(void *prxq, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
  ...
  nb_rcv =
      rte_ring_sc_dequeue_burst(rxq->rx_ring, (void **)rx_pkts, nb_pkts, NULL);
  ...
  if (hv->vf_ctx.vf_vsc_switched) {
    ...
    vf_dev = hn_get_vf_dev(hv);
    if (hv->vf_ctx.vf_vsc_switched && vf_dev && vf_dev->data->dev_started)
      nb_rcv += hn_recv_vf(vf_dev->data->port_id, rxq, rx_pkts + nb_rcv,
                           nb_pkts - nb_rcv);
