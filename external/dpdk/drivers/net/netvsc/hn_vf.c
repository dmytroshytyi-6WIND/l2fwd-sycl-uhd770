...
static int hn_vf_attach(struct rte_eth_dev *dev, struct hn_data *hv) {
  ...
  if (owner.id != RTE_ETH_DEV_NO_OWNER) {
    PMD_DRV_LOG(ERR, "Port %u already owned by other device %s", port,
                owner.name);
    return -EBUSY;
  }
...
static void hn_remove_delayed(void *args) {
  ...
  if (ret)
    PMD_DRV_LOG(ERR, "rte_eth_dev_owner_unset failed ret=%d", ret);
  ...
  if (ret)
    PMD_DRV_LOG(ERR, "rte_eth_dev_callback_unregister failed ret=%d", ret);

  /* Detach and release port_id from system */
  ret = rte_eth_dev_stop(port_id);
  if (ret)
    PMD_DRV_LOG(ERR, "rte_eth_dev_stop failed port_id=%u ret=%d", port_id, ret);

  ret = rte_eth_dev_close(port_id);
  if (ret)
    PMD_DRV_LOG(ERR, "rte_eth_dev_close failed port_id=%u ret=%d", port_id,
                ret);
...
static int hn_setup_vf_queues(int port, struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    if (ret) {
      PMD_DRV_LOG(ERR, "rte_eth_tx_queue_info_get failed ret=%d", ret);
      return ret;
    }

    ret = rte_eth_tx_queue_setup(port, i, txinfo.nb_desc, 0, &txinfo.conf);
    if (ret) {
      PMD_DRV_LOG(ERR, "rte_eth_tx_queue_setup failed ret=%d", ret);
      return ret;
    }
  }
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    if (ret) {
      PMD_DRV_LOG(ERR, "rte_eth_rx_queue_info_get failed ret=%d", ret);
      return ret;
    }

    rx_queue = dev->data->rx_queues[i];

    ret = rte_eth_rx_queue_setup(port, i, rxinfo.nb_desc, 0, &rxinfo.conf,
                                 rx_queue->mb_pool);
    if (ret) {
      PMD_DRV_LOG(ERR, "rte_eth_rx_queue_setup failed ret=%d", ret);
      return ret;
    }
  }
...
int hn_vf_add(struct rte_eth_dev *dev, struct hn_data *hv) {
  ...
  if (dev->data->dev_started) {
    ...
    if (ret) {
      PMD_DRV_LOG(ERR, "Failed to configure VF port %d", port);
      goto exit;
    }

    ret = hn_setup_vf_queues(port, dev);
    if (ret) {
      PMD_DRV_LOG(ERR, "Failed to configure VF queues port %d", port);
      goto exit;
    }
    ...
    if (ret) {
      PMD_DRV_LOG(ERR, "rte_eth_dev_start failed ret=%d", ret);
      goto exit;
    }
...
void hn_nvs_handle_vfassoc(struct rte_eth_dev *dev,
  ...
  PMD_DRV_LOG(DEBUG, "VF serial %u %s port %u", vf_assoc->serial,
...
static void hn_vf_merge_desc_lim(struct rte_eth_desc_lim *lim,
...
static int hn_vf_info_merge(struct rte_eth_dev *vf_dev,
  ...
  info->max_rx_queues = RTE_MIN(vf_info.max_rx_queues, info->max_rx_queues);
  ...
  info->max_tx_queues = RTE_MIN(vf_info.max_tx_queues, info->max_tx_queues);
  ...
  info->min_rx_bufsize = RTE_MAX(vf_info.min_rx_bufsize, info->min_rx_bufsize);
  info->max_rx_pktlen = RTE_MAX(vf_info.max_rx_pktlen, info->max_rx_pktlen);
...
int hn_vf_configure(struct rte_eth_dev *dev,
  ...
  if (hv->vf_ctx.vf_attached) {
    ret = rte_eth_dev_callback_register(hv->vf_ctx.vf_port,
                                        ...
                                        hn_eth_rmv_event_callback, hv);
    if (ret) {
      PMD_DRV_LOG(ERR, "Registering callback failed for vf port %d ret %d",
                  hv->vf_ctx.vf_port, ret);
      return ret;
    }

    ret = rte_eth_dev_configure(hv->vf_ctx.vf_port, dev->data->nb_rx_queues,
                                dev->data->nb_tx_queues, &vf_conf);
    if (ret) {
...
int hn_vf_close(struct rte_eth_dev *dev) {
  ...
  rte_rwlock_read_lock(&hv->vf_lock);
  if (hv->vf_ctx.vf_attached) {
    rte_eth_dev_callback_unregister(hv->vf_ctx.vf_port, RTE_ETH_EVENT_INTR_RMV,
                                    hn_eth_rmv_event_callback, hv);
...
int hn_vf_mc_addr_list(struct rte_eth_dev *dev,
  ...
  vf_dev = hn_get_vf_dev(hv);
  if (vf_dev)
    ret = rte_eth_dev_set_mc_addr_list(vf_dev->data->port_id, mc_addr_set,
                                       nb_mc_addr);
  rte_rwlock_read_unlock(&hv->vf_lock);
  return ret;
}

int hn_vf_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  vf_dev = hn_get_vf_dev(hv);
  if (vf_dev)
    ret = rte_eth_tx_queue_setup(vf_dev->data->port_id, queue_idx, nb_desc,
                                 socket_id, tx_conf);
  rte_rwlock_read_unlock(&hv->vf_lock);
  return ret;
}
...
int hn_vf_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                         uint16_t nb_desc, unsigned int socket_id,
  ...
  vf_dev = hn_get_vf_dev(hv);
  if (vf_dev)
    ret = rte_eth_rx_queue_setup(vf_dev->data->port_id, queue_idx, nb_desc,
...
int hn_vf_xstats_get_names(struct rte_eth_dev *dev,
  ...
  vf_dev = hn_get_vf_dev(hv);
  if (vf_dev)
    count = rte_eth_xstats_get_names(vf_dev->data->port_id, names, n);
  rte_rwlock_read_unlock(&hv->vf_lock);
...
int hn_vf_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats,
  ...
  if (vf_dev)
    count =
        rte_eth_xstats_get(vf_dev->data->port_id, xstats + offset, n - offset);
  rte_rwlock_read_unlock(&hv->vf_lock);
...
int hn_vf_reta_hash_update(struct rte_eth_dev *dev,
  ...
  if (vf_dev && vf_dev->dev_ops->reta_update)
    ret = vf_dev->dev_ops->reta_update(vf_dev, reta_conf, reta_size);
  rte_rwlock_read_unlock(&hv->vf_lock);
