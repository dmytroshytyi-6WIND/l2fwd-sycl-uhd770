...
static const uint32_t hn_nvs_version[] = {NVS_VERSION_61, NVS_VERSION_6,
                                          NVS_VERSION_5,  NVS_VERSION_4,
                                          NVS_VERSION_2,  NVS_VERSION_1};

static int hn_nvs_req_send(struct hn_data *hv, void *req, uint32_t reqlen) {
  return rte_vmbus_chan_send(hn_primary_chan(hv), VMBUS_CHANPKT_TYPE_INBAND,
                             req, reqlen, 0, VMBUS_CHANPKT_FLAG_NONE, NULL);
}

static int __hn_nvs_execute(struct hn_data *hv, void *req, uint32_t reqlen,
                            void *resp, uint32_t resplen, uint32_t type) {
  ...
  ret = rte_vmbus_chan_send(chan, VMBUS_CHANPKT_TYPE_INBAND, req, reqlen, 0,
...
retry:
  ...
  if (hdr->type != type) {
    PMD_DRV_LOG(ERR, "unexpected NVS resp %#x, expect %#x", hdr->type, type);
    return -EINVAL;
  }

  if (len < resplen) {
    PMD_DRV_LOG(ERR, "invalid NVS resp len %u (expect %u)", len, resplen);
    return -EINVAL;
  }
 ...
 */
static int hn_nvs_execute(struct hn_data *hv, void *req, uint32_t reqlen,
                          void *resp, uint32_t resplen, uint32_t type) {
...
static int hn_nvs_doinit(struct hn_data *hv, uint32_t nvs_ver) {
  ...
  error = hn_nvs_execute(hv, &init, sizeof(init), &resp, sizeof(resp),
                         NVS_TYPE_INIT_RESP);
  if (error)
    return error;
  ...
  if (status != NVS_STATUS_OK) {
    ...
    PMD_INIT_LOG(DEBUG, "nvs init failed for ver 0x%x", nvs_ver);
    return -EINVAL;
  }
...
static int hn_nvs_conn_rxbuf(struct hn_data *hv) {
  ...
  PMD_DRV_LOG(DEBUG, "connect rxbuff va=%p gpad=%#" PRIx64, hv->rxbuf_res->addr,
              hv->rxbuf_res->phys_addr);

  error = hn_nvs_execute(hv, &conn, sizeof(conn), &resp, sizeof(resp),
                         NVS_TYPE_RXBUF_CONNRESP);
  if (error) {
    PMD_DRV_LOG(ERR, "exec nvs rxbuf conn failed: %d", error);
    return error;
  }

  status = resp.status;
  if (status != NVS_STATUS_OK) {
    PMD_DRV_LOG(ERR, "nvs rxbuf conn failed: %x", status);
    return -EIO;
  }
  if (resp.nsect != 1) {
    PMD_DRV_LOG(ERR, "nvs rxbuf response num sections %u != 1", resp.nsect);
    return -EIO;
  }

  PMD_DRV_LOG(INFO, "receive buffer size %u count %u", resp.nvs_sect[0].slotsz,
  ...
  hv->primary->rxbuf_info =
      rte_calloc("HN_RXBUF_INFO", hv->rxbuf_section_cnt,
                 sizeof(*hv->primary->rxbuf_info), RTE_CACHE_LINE_SIZE);
  if (!hv->primary->rxbuf_info) {
    PMD_DRV_LOG(ERR, "could not allocate rxbuf info");
    return -ENOMEM;
  }
...
static void hn_nvs_disconn_rxbuf(struct hn_data *hv) {
  ...
  if (error) {
    PMD_DRV_LOG(ERR, "send nvs rxbuf disconn failed: %d", error);
  }
...
static void hn_nvs_disconn_chim(struct hn_data *hv) {
  int error;
  ...
  if (hv->chim_cnt != 0) {
    ...
    if (error) {
      PMD_DRV_LOG(ERR, "send nvs chim disconn failed: %d", error);
    }
...
static int hn_nvs_conn_chim(struct hn_data *hv) {
  ...
  error = hn_nvs_execute(hv, &chim, sizeof(chim), &resp, sizeof(resp),
  ...
  if (resp.status != NVS_STATUS_OK) {
    PMD_DRV_LOG(ERR, "nvs chim conn failed: %x", resp.status);
    return -EIO;
  }
  ...
  if (sectsz == 0 || sectsz & (sizeof(uint32_t) - 1)) {
    ...
    PMD_DRV_LOG(NOTICE, "invalid chimney sending buffer section size: %u",
                sectsz);
    error = -EINVAL;
    goto cleanup;
  }
  ...
  PMD_DRV_LOG(INFO, "send buffer %lu section size:%u, count:%u", len,
 ...
 */
static int hn_nvs_conf_ndis(struct hn_data *hv, unsigned int mtu) {
  ...
  if (error) {
    PMD_DRV_LOG(ERR, "send nvs ndis conf failed: %d", error);
    return error;
  }
...
static int hn_nvs_init_ndis(struct hn_data *hv) {
  ...
  if (error)
    PMD_DRV_LOG(ERR, "send nvs ndis init failed: %d", error);

  return error;
}

static int hn_nvs_init(struct hn_data *hv) {
  unsigned int i;
  int error;
  ...
  for (i = 0; i < RTE_DIM(hn_nvs_version); ++i) {
    ...
    if (error) {
      PMD_INIT_LOG(DEBUG, "version %#x error %d", hn_nvs_version[i], error);
      continue;
    }
    ...
    PMD_INIT_LOG(DEBUG, "NVS version %#x, NDIS version %u.%u", hv->nvs_ver,
                 NDIS_VERSION_MAJOR(hv->ndis_ver),
...
void hn_nvs_detach(struct hn_data *hv __rte_unused) {
  PMD_INIT_FUNC_TRACE();
...
again:
  error = rte_vmbus_chan_send(chan, VMBUS_CHANPKT_TYPE_COMP, &ack, sizeof(ack),
...
int hn_nvs_alloc_subchans(struct hn_data *hv, uint32_t *nsubch) {
  ...
  error = hn_nvs_execute(hv, &req, sizeof(req), &resp, sizeof(resp),
                         NVS_TYPE_SUBCH_RESP);
  if (error)
    return error;

  if (resp.status != NVS_STATUS_OK) {
    PMD_INIT_LOG(ERR, "nvs subch alloc failed: %#x", resp.status);
    return -EIO;
  }

  if (resp.nsubch > *nsubch) {
    PMD_INIT_LOG(NOTICE, "%u subchans are allocated, requested %u", resp.nsubch,
                 *nsubch);
  }
  *nsubch = resp.nsubch;
...
int hn_nvs_set_datapath(struct hn_data *hv, uint32_t path) {
  ...
  PMD_DRV_LOG(DEBUG, "set datapath %s", path ? "VF" : "Synthetic");
  ...
  if (error) {
    PMD_DRV_LOG(ERR, "send set datapath failed: %d", error);
  }
