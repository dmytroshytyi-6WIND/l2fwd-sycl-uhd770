...
#define HN_RX_OFFLOAD_CAPS                                                     \
  (RTE_ETH_RX_OFFLOAD_CHECKSUM | RTE_ETH_RX_OFFLOAD_VLAN_STRIP |               \
...
static struct rte_eth_dev *eth_dev_vmbus_allocate(struct rte_vmbus_device *dev,
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    ...
    if (private_data_size) {
      eth_dev->data->dev_private = rte_zmalloc_socket(
          name, private_data_size, RTE_CACHE_LINE_SIZE, dev->device.numa_node);
...
static void eth_dev_vmbus_release(struct rte_eth_dev *eth_dev) {
  /* free ether device */
  rte_eth_dev_release_port(eth_dev);
...
static int hn_set_parameter(const char *key, const char *value, void *opaque) {
  ...
  if (!strcmp(key, NETVSC_ARG_LATENCY)) {
    ...
    hv->rx_copybreak = v;
    PMD_DRV_LOG(DEBUG, "rx copy break set to %u", hv->rx_copybreak);
  } else if (!strcmp(key, NETVSC_ARG_TXBREAK)) {
    hv->tx_copybreak = v;
    PMD_DRV_LOG(DEBUG, "tx copy break set to %u", hv->tx_copybreak);
  } else if (!strcmp(key, NETVSC_ARG_RX_EXTMBUF_ENABLE)) {
    hv->rx_extmbuf_enable = v;
    PMD_DRV_LOG(DEBUG, "rx extmbuf enable set to %u", hv->rx_extmbuf_enable);
  }
...
static int hn_parse_args(const struct rte_eth_dev *dev) {
  ...
  static const char *const valid_keys[] = {
      NETVSC_ARG_LATENCY, NETVSC_ARG_RXBREAK, NETVSC_ARG_TXBREAK,
      NETVSC_ARG_RX_EXTMBUF_ENABLE, NULL};
  ...
  PMD_INIT_LOG(DEBUG, "device args %s %s", devargs->name, devargs->args);
 ...
 */
int hn_dev_link_update(struct rte_eth_dev *dev,
...
static int hn_rss_reta_update(struct rte_eth_dev *dev,
  ...
  if (err) {
    PMD_DRV_LOG(NOTICE, "rss disable failed");
    return err;
  }

  err = hn_rndis_conf_rss(hv, 0);
  if (err) {
    PMD_DRV_LOG(NOTICE, "reta reconfig failed");
    return err;
  }
...
static int hn_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_hf != 0) {
    err = hn_rndis_conf_rss(hv, 0);
    if (err) {
      PMD_DRV_LOG(NOTICE, "rss reconfig failed (RSS disabled)");
      return err;
    }
  }
...
static int hn_rss_hash_conf_get(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_key)
    memcpy(rss_conf->rss_key, hv->rss_key, NDIS_HASH_KEYSIZE_TOEPLITZ);
...
static int hn_dev_promiscuous_enable(struct rte_eth_dev *dev) {
  struct hn_data *hv = dev->data->dev_private;
...
static int hn_dev_promiscuous_disable(struct rte_eth_dev *dev) {
  struct hn_data *hv = dev->data->dev_private;
  uint32_t filter;
...
static int hn_dev_allmulticast_enable(struct rte_eth_dev *dev) {
  struct hn_data *hv = dev->data->dev_private;
...
static int hn_dev_allmulticast_disable(struct rte_eth_dev *dev) {
  struct hn_data *hv = dev->data->dev_private;
...
static int hn_dev_mc_addr_list(struct rte_eth_dev *dev,
...
static int hn_subchan_configure(struct hn_data *hv, uint32_t subchan) {
  struct vmbus_channel *primary = hn_primary_chan(hv);
  int err;
  unsigned int retry = 0;

  PMD_DRV_LOG(DEBUG, "open %u subchannels", subchan);
  ...
  while (subchan > 0) {
    ...
    if (err) {
      PMD_DRV_LOG(ERR, "open subchannel failed: %d", err);
      return err;
    }
    ...
    if (chn_index == 0 || chn_index > hv->max_queues) {
      PMD_DRV_LOG(ERR, "Invalid subchannel offermsg channel %u", chn_index);
      return -EIO;
    }
...
static void netvsc_hotplug_retry(void *args) {
  ...
  PMD_DRV_LOG(DEBUG, "%s: retry count %d", __func__, hv->eal_hot_plug_retry);
  ...
  while ((dir = readdir(di))) {
    ...
    if (s == -1) {
      PMD_DRV_LOG(ERR, "Failed to create socket errno %d", errno);
      break;
    }
    ...
    if (rte_is_same_ether_addr(&eth_addr, dev->data->mac_addrs)) {
      ...
      if (ret) {
        PMD_DRV_LOG(ERR, "Failed to add PCI device %s", d->name);
        break;
      }
    }
...
static void netvsc_hotadd_callback(const char *device_name,
                                   enum rte_dev_event_type type, void *arg) {
  struct hn_data *hv = arg;
  struct rte_devargs *d = &hv->devargs;
  int ret;

  PMD_DRV_LOG(INFO, "Device notification type=%d device_name=%s", type,
              device_name);
  ...
  case RTE_DEV_EVENT_ADD:
    ...
    if (ret) {
      PMD_DRV_LOG(ERR, "devargs parsing failed ret=%d", ret);
      return;
    }
...
static int hn_dev_configure(struct rte_eth_dev *dev) {
  ...
  if (unsupported) {
    PMD_DRV_LOG(NOTICE, "unsupported TX offload: %#" PRIx64, unsupported);
    return -EINVAL;
  }

  unsupported = rxmode->offloads & ~HN_RX_OFFLOAD_CAPS;
  if (unsupported) {
    PMD_DRV_LOG(NOTICE, "unsupported RX offload: %#" PRIx64, rxmode->offloads);
    return -EINVAL;
  }

  hv->vlan_strip = !!(rxmode->offloads & RTE_ETH_RX_OFFLOAD_VLAN_STRIP);

  err = hn_rndis_conf_offload(hv, txmode->offloads, rxmode->offloads);
  if (err) {
    PMD_DRV_LOG(NOTICE, "offload configure failed");
    return err;
  }

  hv->num_queues = RTE_MAX(dev->data->nb_rx_queues, dev->data->nb_tx_queues);
  ...
  if (subchan > 0) {
    ...
    if (err) {
      PMD_DRV_LOG(NOTICE, "subchannel configuration failed");
      return err;
    }
    ...
    if (rss_conf->rss_hf != 0) {
      err = hn_rndis_conf_rss(hv, 0);
      if (err) {
        PMD_DRV_LOG(NOTICE, "initial RSS config failed");
        return err;
      }
    }
  }
...
static int hn_dev_stats_reset(struct rte_eth_dev *dev) {
  unsigned int i;
...
static int hn_dev_xstats_reset(struct rte_eth_dev *dev) {
  int ret;
...
static int hn_dev_xstats_count(struct rte_eth_dev *dev) {
  int ret, count;
...
static int hn_dev_xstats_get_names(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    for (t = 0; t < RTE_DIM(hn_stat_strings); t++)
      snprintf(xstats_names[count++].name, RTE_ETH_XSTATS_NAME_SIZE,
               "rx_q%u_%s", i, hn_stat_strings[t].name);
  }

  ret = hn_vf_xstats_get_names(dev, xstats_names + count, limit - count);
  if (ret < 0)
    return ret;
...
static int hn_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    for (t = 0; t < RTE_DIM(hn_stat_strings); t++, count++) {
      ...
      xstats[count].value =
          *(const uint64_t *)(stats + hn_stat_strings[t].offset);
    }
  }
...
static int hn_dev_start(struct rte_eth_dev *dev) {
  struct hn_data *hv = dev->data->dev_private;
  int error;
  ...
  error = hn_rndis_set_rxfilter(hv, NDIS_PACKET_TYPE_BROADCAST |
                                        NDIS_PACKET_TYPE_ALL_MULTICAST |
...
static int hn_dev_stop(struct rte_eth_dev *dev) {
  struct hn_data *hv = dev->data->dev_private;
...
static int hn_dev_close(struct rte_eth_dev *dev) {
  int ret;
  struct hn_data *hv = dev->data->dev_private;
...
static int eth_hn_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  eth_dev->data->mac_addrs =
      rte_calloc("hv_mac", HN_MAX_MAC_ADDRS, sizeof(struct rte_ether_addr), 0);
  if (eth_dev->data->mac_addrs == NULL) {
    PMD_INIT_LOG(ERR, "Failed to allocate memory store MAC addresses");
    return -ENOMEM;
  }
  ...
  strlcpy(hv->owner.name, eth_dev->device->name, RTE_ETH_MAX_OWNER_NAME_LEN);
  ...
  hv->primary = hn_rx_queue_alloc(hv, 0, eth_dev->device->numa_node);
...
static int eth_hn_dev_uninit(struct rte_eth_dev *eth_dev) {
  struct hn_data *hv = eth_dev->data->dev_private;
  int ret, ret_stop;
...
RTE_PMD_REGISTER_PARAM_STRING(net_netvsc, NETVSC_ARG_LATENCY
                              "=<uint32> " NETVSC_ARG_RXBREAK
                              "=<uint32> " NETVSC_ARG_TXBREAK
                              "=<uint32> " NETVSC_ARG_RX_EXTMBUF_ENABLE
                              "=<0|1>");
