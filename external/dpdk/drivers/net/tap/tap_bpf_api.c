...
int tap_flow_bpf_cls_q(__u32 queue_idx) {
  ...
  return bpf_load(BPF_PROG_TYPE_SCHED_CLS, (struct bpf_insn *)cls_q_insns,
                  RTE_DIM(cls_q_insns), "Dual BSD/GPL");
}
...
int tap_flow_bpf_calc_l3_l4_hash(__u32 key_idx, int map_fd) {
  ...
  return bpf_load(BPF_PROG_TYPE_SCHED_ACT, (struct bpf_insn *)l3_l4_hash_insns,
                  RTE_DIM(l3_l4_hash_insns), "Dual BSD/GPL");
}
