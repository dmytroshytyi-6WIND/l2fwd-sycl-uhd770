 ...
 */
static int qdisc_del(int nlsk_fd, uint16_t ifindex, struct qdisc *qinfo) {
  struct nlmsg msg;
  int fd = 0;
  ...
  if (!nlsk_fd) {
    fd = tap_nl_init(0);
    if (fd < 0) {
      TAP_LOG(ERR, "Could not delete QDISC: null netlink socket");
      return -1;
    }
  } else {
    fd = nlsk_fd;
 ...
 *   0 on success, -1 otherwise with errno set.
 */
static int qdisc_del_cb(struct nlmsghdr *nh, void *arg) {
 ...
 *   0 on success, -1 otherwise with errno set.
 */
static int qdisc_iterate(int nlsk_fd, uint16_t ifindex,
                         int (*callback)(struct nlmsghdr *, void *),
                         void *arg) {
  struct nlmsg msg;
...
int qdisc_create_multiq(int nlsk_fd, uint16_t ifindex) {
  ...
  if (err < 0 && errno != -EEXIST) {
    TAP_LOG(ERR, "Could not add multiq qdisc (%d): %s", errno, strerror(errno));
    return -1;
  }
  return 0;
}
...
int qdisc_create_ingress(int nlsk_fd, uint16_t ifindex) {
  ...
  if (err < 0 && errno != -EEXIST) {
    TAP_LOG(ERR, "Could not add ingress qdisc (%d): %s", errno,
            strerror(errno));
    return -1;
  }
  return 0;
