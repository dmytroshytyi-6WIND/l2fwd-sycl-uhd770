...
#define TAP_GSO_MBUFS_NUM (TAP_GSO_MBUFS_PER_CORE * TAP_GSO_MBUF_CACHE_SIZE)
...
#define TAP_RX_OFFLOAD                                                         \
  (RTE_ETH_RX_OFFLOAD_SCATTER | RTE_ETH_RX_OFFLOAD_IPV4_CKSUM |                \
   RTE_ETH_RX_OFFLOAD_UDP_CKSUM | RTE_ETH_RX_OFFLOAD_TCP_CKSUM)

#define TAP_TX_OFFLOAD                                                         \
  (RTE_ETH_TX_OFFLOAD_MULTI_SEGS | RTE_ETH_TX_OFFLOAD_IPV4_CKSUM |             \
...
static const char *valid_arguments[] = {ETH_TAP_IFACE_ARG, ETH_TAP_REMOTE_ARG,
                                        ETH_TAP_MAC_ARG, NULL};
...
static void tap_trigger_cb(int sig __rte_unused) {
 ...
 */
static int tun_alloc(struct pmd_internals *pmd, int is_keepalive) {
  struct ifreq ifr;
   ...
   */
  ifr.ifr_flags =
      (pmd->type == ETH_TUNTAP_TYPE_TAP) ? IFF_TAP : IFF_TUN | IFF_POINTOPOINT;
  ...
  if (ioctl(fd, TUNSETIFF, (void *)&ifr) < 0) {
    TAP_LOG(WARNING, "Unable to set TUNSETIFF for %s: %s", ifr.ifr_name,
            strerror(errno));
    goto error;
  }
  ...
  if (is_keepalive) {
    ...
    if (ioctl(fd, TUNSETQUEUE, (void *)&ifr) < 0) {
      TAP_LOG(WARNING, "Unable to detach keep-alive queue for %s: %s",
  ...
  if (flags == -1) {
    TAP_LOG(WARNING, "Unable to get %s current flags\n", ifr.ifr_name);
    goto error;
  }
  ...
  if (fcntl(fd, F_SETFL, flags) < 0) {
    TAP_LOG(WARNING, "Unable to set %s to nonblocking: %s", ifr.ifr_name,
            strerror(errno));
    goto error;
  }
  ...
  for (signo = SIGRTMIN + 1; signo < SIGRTMAX; signo++) {
    ...
    if (sigaction(signo, NULL, &sa) == -1) {
      TAP_LOG(WARNING, "Unable to get current rt-signal %d handler", signo);
      goto error;
    }
    ...
    if (sa.sa_handler != SIG_DFL) {
      TAP_LOG(DEBUG, "Skipping used rt-signal %d", signo);
      continue;
    }
    ...
    if (sigaction(signo, &sa, NULL) == -1) {
      TAP_LOG(WARNING, "Unable to set rt-signal %d handler\n", signo);
      goto error;
    }

    /* Found a good signal to use */
    TAP_LOG(DEBUG, "Using rt-signal %d", signo);
    break;
  }
  ...
  if (signo == SIGRTMAX) {
    ...
    if (fcntl(fd, F_SETSIG, signo) < 0) {
      TAP_LOG(WARNING, "Unable to set signo %d for fd %d: %s", signo, fd,
              strerror(errno));
      goto error;
    }
    if (fcntl(fd, F_SETFL, flags | O_ASYNC) < 0) {
      TAP_LOG(WARNING, "Unable to set fcntl flags: %s", strerror(errno));
      goto error;
    }

    if (fcntl(fd, F_SETOWN, getpid()) < 0) {
      TAP_LOG(WARNING, "Unable to set fcntl owner: %s", strerror(errno));
      goto error;
    }
  }
  return fd;
...
static void tap_verify_csum(struct rte_mbuf *mbuf) {
  ...
  if (l3 == RTE_PTYPE_L3_IPV4 || l3 == RTE_PTYPE_L3_IPV4_EXT) {
    ...
    mbuf->ol_flags |=
        cksum ? RTE_MBUF_F_RX_IP_CKSUM_BAD : RTE_MBUF_F_RX_IP_CKSUM_GOOD;
  ...
  if (l4 == RTE_PTYPE_L4_UDP || l4 == RTE_PTYPE_L4_TCP) {
    ...
    mbuf->ol_flags |=
        cksum_ok ? RTE_MBUF_F_RX_L4_CKSUM_GOOD : RTE_MBUF_F_RX_L4_CKSUM_BAD;
  }
}
 ...
 */
static uint16_t pmd_rx_burst(void *queue, struct rte_mbuf **bufs,
                             uint16_t nb_pkts) {
  ...
  for (num_rx = 0; num_rx < nb_pkts;) {
    ...
    len = readv(process_private->rxq_fds[rxq->queue_id], *rxq->iovecs,
    ...
    while (1) {
      ...
      (*rxq->iovecs)[mbuf->nb_segs].iov_len = buf->buf_len - data_off;
      (*rxq->iovecs)[mbuf->nb_segs].iov_base = (char *)buf->buf_addr + data_off;
...
static void tap_tx_l4_cksum(uint16_t *l4_cksum, uint16_t l4_phdr_cksum,
...
static void tap_tx_l4_add_rcksum(char *l4_data, unsigned int l4_len,
                                 uint16_t *l4_cksum, uint32_t *l4_raw_cksum) {
  if (l4_cksum == NULL)
    return;
...
static void tap_tx_l3_cksum(char *packet, uint64_t ol_flags,
                            unsigned int l2_len, unsigned int l3_len,
                            unsigned int l4_len, uint16_t **l4_cksum,
...
static inline int tap_write_mbufs(struct tx_queue *txq, uint16_t num_mbufs,
  ...
  for (i = 0; i < num_mbufs; i++) {
    ...
    if (txq->type == ETH_TUNTAP_TYPE_TUN) {
      ...
      pi.proto = (proto == 0x40)
                     ...
                     : ((proto == 0x60) ? rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6)
                                        : 0x00);
    }
    ...
    if (txq->csum &&
       ...
       */
      rte_memcpy(m_copy, rte_pktmbuf_mtod(mbuf, void *), l234_hlen);
      tap_tx_l3_cksum(m_copy, mbuf->ol_flags, mbuf->l2_len, mbuf->l3_len,
                      mbuf->l4_len, &l4_cksum, &l4_phdr_cksum, &l4_raw_cksum);
      ...
      if (seg_len > l234_hlen) {
        ...
        iovecs[k].iov_base = rte_pktmbuf_mtod(seg, char *) + l234_hlen;
        tap_tx_l4_add_rcksum(iovecs[k].iov_base, iovecs[k].iov_len, l4_cksum,
                             &l4_raw_cksum);
        k++;
        nb_segs++;
      }
    ...
    for (j = k; j <= nb_segs; j++) {
      ...
      if (is_cksum)
        tap_tx_l4_add_rcksum(iovecs[j].iov_base, iovecs[j].iov_len, l4_cksum,
                             &l4_raw_cksum);
      seg = seg->next;
    }
...
static uint16_t pmd_tx_burst(void *queue, struct rte_mbuf **bufs,
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (tso) {
      ...
      if (unlikely(tso_segsz == hdrs_len) || tso_segsz > *txq->mtu) {
        txq->stats.errs++;
        break;
      }
      ...
      num_tso_mbufs =
          rte_gso_segment(mbuf_in, gso_ctx, /* gso control block */
    ...
    ret = tap_write_mbufs(txq, num_mbufs, mbuf, &num_packets, &num_tx_bytes);
    if (ret == -1) {
      txq->stats.errs++;
      /* free tso mbufs */
      if (num_tso_mbufs > 0)
...
static const char *tap_ioctl_req2str(unsigned long request) {
  switch (request) {
  case SIOCSIFFLAGS:
    return "SIOCSIFFLAGS";
  case SIOCGIFFLAGS:
...
static int tap_ioctl(struct pmd_internals *pmd, unsigned long request,
  ...
  int remote =
      pmd->remote_if_index && (mode == REMOTE_ONLY || mode == LOCAL_AND_REMOTE);
...
apply:
  ...
  case SIOCSIFMTU:
    break;
  default:
    TAP_LOG(WARNING, "%s: ioctl() called with wrong arg", pmd->name);
    return -EINVAL;
  }
...
static int tap_link_set_down(struct rte_eth_dev *dev) {
...
static int tap_link_set_up(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct ifreq ifr = {.ifr_flags = IFF_UP};
...
static int tap_dev_start(struct rte_eth_dev *dev) {
  int err, i;
...
static int tap_mp_req_start_rxtx(const struct rte_mp_msg *request,
                                 __rte_unused const void *peer) {
  ...
  if (!dev) {
    TAP_LOG(ERR, "Failed to get dev for %s", request_param->port_name);
    return -1;
  }
 ...
 */
static int tap_dev_stop(struct rte_eth_dev *dev) {
  int i;
...
static int tap_dev_configure(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;

  if (dev->data->nb_rx_queues > RTE_PMD_TAP_MAX_QUEUES) {
    TAP_LOG(ERR, "%s: number of rx queues %d exceeds max num of queues %d",
            dev->device->name, dev->data->nb_rx_queues, RTE_PMD_TAP_MAX_QUEUES);
    return -1;
  }
  if (dev->data->nb_tx_queues > RTE_PMD_TAP_MAX_QUEUES) {
    TAP_LOG(ERR, "%s: number of tx queues %d exceeds max num of queues %d",
  ...
  if (dev->data->nb_rx_queues != dev->data->nb_tx_queues) {
    TAP_LOG(
        ...
        dev->device->name, dev->data->nb_rx_queues, dev->data->nb_tx_queues);
    return -1;
  }

  TAP_LOG(INFO, "%s: %s: TX configured queues number: %u", dev->device->name,
          pmd->name, dev->data->nb_tx_queues);

  TAP_LOG(INFO, "%s: %s: RX configured queues number: %u", dev->device->name,
          pmd->name, dev->data->nb_rx_queues);

  return 0;
}

static uint32_t tap_dev_speed_capa(void) {
...
static int tap_dev_info(struct rte_eth_dev *dev,
...
static int tap_stats_get(struct rte_eth_dev *dev,
                         struct rte_eth_stats *tap_stats) {
  unsigned int i, imax;
...
static int tap_stats_reset(struct rte_eth_dev *dev) {
  int i;
  struct pmd_internals *pmd = dev->data->dev_private;
...
static int tap_dev_close(struct rte_eth_dev *dev) {
  int i;
  struct pmd_internals *internals = dev->data->dev_private;
  struct pmd_process_private *process_private = dev->process_private;
  struct rx_queue *rxq;
...
static void tap_rx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
...
static void tap_tx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
...
static int tap_link_update(struct rte_eth_dev *dev,
...
static int tap_promisc_enable(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
...
static int tap_promisc_disable(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct ifreq ifr = {.ifr_flags = IFF_PROMISC};
  int ret;
...
static int tap_allmulti_enable(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
...
static int tap_allmulti_disable(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct ifreq ifr = {.ifr_flags = IFF_ALLMULTI};
  int ret;
...
static int tap_mac_set(struct rte_eth_dev *dev,
  ...
  if (pmd->type == ETH_TUNTAP_TYPE_TUN) {
    TAP_LOG(ERR, "%s: can't MAC address for TUN", dev->device->name);
    return -ENOTSUP;
  }

  if (rte_is_zero_ether_addr(mac_addr)) {
    TAP_LOG(ERR, "%s: can't set an empty MAC address", dev->device->name);
    return -EINVAL;
  }
  ...
  if (pmd->remote_if_index && !pmd->flow_isolate) {
    ...
    if (ret < 0) {
      TAP_LOG(ERR, "%s: Couldn't delete MAC redirection rule",
              dev->device->name);
      return ret;
    }
    ret = tap_flow_implicit_create(pmd, TAP_REMOTE_LOCAL_MAC);
    if (ret < 0) {
      TAP_LOG(ERR, "%s: Couldn't add MAC redirection rule", dev->device->name);
      return ret;
    }
  }
...
static int tap_gso_ctx_setup(struct rte_gso_ctx *gso_ctx,
                             struct rte_eth_dev *dev) {
  uint32_t gso_types;
  char pool_name[64];
  struct pmd_internals *pmd = dev->data->dev_private;
  ...
  if (!pmd->gso_ctx_mp) {
     ...
     * indirect mbufs
     */
    ret = snprintf(pool_name, sizeof(pool_name), "mp_%s", dev->device->name);
    ...
    pmd->gso_ctx_mp = rte_pktmbuf_pool_create(
        pool_name, TAP_GSO_MBUFS_NUM, TAP_GSO_MBUF_CACHE_SIZE, 0,
        RTE_PKTMBUF_HEADROOM + TAP_GSO_MBUF_SEG_SIZE, SOCKET_ID_ANY);
    if (!pmd->gso_ctx_mp) {
...
static int tap_setup_queue(struct rte_eth_dev *dev,
                           struct pmd_internals *internals, uint16_t qid,
                           int is_rx) {
  int ret;
  int *fd;
  int *other_fd;
  ...
  if (*fd != -1) {
    ...
    TAP_LOG(DEBUG, "%s: fd %d for %s queue qid %d exists", pmd->name, *fd, dir,
            qid);
    gso_ctx = NULL;
  } else if (*other_fd != -1) {
    ...
    *fd = dup(*other_fd);
    ...
    TAP_LOG(DEBUG, "%s: dup fd %d for %s queue qid %d (%d)", pmd->name,
            *other_fd, dir, qid, *fd);
  } else {
    ...
    *fd = tun_alloc(pmd, 0);
    ...
    TAP_LOG(DEBUG, "%s: add %s queue for qid %d fd %d", pmd->name, dir, qid,
            *fd);
  }
...
static int tap_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
  ...
  if (iov_max <= 0) {
    TAP_LOG(WARNING, "_SC_IOV_MAX is not defined. Using %d as default",
            TAP_IOV_DEFAULT_MAX);
  ...
  if (rx_queue_id >= dev->data->nb_rx_queues || !mp) {
    TAP_LOG(WARNING, "nb_rx_queues %d too small or mempool NULL",
            dev->data->nb_rx_queues);
    return -1;
  }
  ...
  iovecs = rte_zmalloc_socket(dev->device->name, sizeof(*iovecs), 0, socket_id);
  if (!iovecs) {
    TAP_LOG(WARNING, "%s: Couldn't allocate %d RX descriptors",
  ...
  for (i = 1; i <= nb_desc; i++) {
    *tmp = rte_pktmbuf_alloc(rxq->mp);
    if (!*tmp) {
      TAP_LOG(WARNING, "%s: couldn't allocate memory for queue %d",
              dev->device->name, rx_queue_id);
      ret = -ENOMEM;
      goto error;
    }
    (*rxq->iovecs)[i].iov_len = (*tmp)->buf_len - data_off;
    (*rxq->iovecs)[i].iov_base = (char *)(*tmp)->buf_addr + data_off;
    data_off = 0;
    tmp = &(*tmp)->next;
  }

  TAP_LOG(DEBUG, "  RX TUNTAP device name %s, qid %d on fd %d", internals->name,
          rx_queue_id, process_private->rxq_fds[rx_queue_id]);
...
static int tap_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id,
  ...
  if (ret == -1)
    return -1;
  TAP_LOG(DEBUG, "  TX TUNTAP device name %s, qid %d on fd %d csum %s",
          internals->name, tx_queue_id, process_private->txq_fds[tx_queue_id],
          txq->csum ? "on" : "off");

  return 0;
}

static int tap_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct pmd_internals *pmd = dev->data->dev_private;
...
static int tap_nl_msg_handler(struct nlmsghdr *nh, void *arg) {
...
static void tap_dev_intr_handler(void *cb_arg) {
  ...
  if (rte_intr_fd_get(pmd->intr_handle) >= 0)
    tap_nl_recv(rte_intr_fd_get(pmd->intr_handle), tap_nl_msg_handler, dev);
}
...
static int tap_lsc_intr_handle_set(struct rte_eth_dev *dev, int set) {
  ...
  if (set) {
    ...
    if (unlikely(rte_intr_fd_get(pmd->intr_handle) == -1))
      return -EBADF;
    return rte_intr_callback_register(pmd->intr_handle, tap_dev_intr_handler,
                                      dev);
  }

clean:
  do {
    ret = rte_intr_callback_unregister(pmd->intr_handle, tap_dev_intr_handler,
                                       dev);
    if (ret >= 0) {
      break;
    } else if (ret == -EAGAIN) {
      ...
      TAP_LOG(ERR, "intr callback unregister failed: %d", ret);
      break;
    }
  } while (true);
...
static int tap_flow_ctrl_get(struct rte_eth_dev *dev __rte_unused,
                             struct rte_eth_fc_conf *fc_conf) {
  fc_conf->mode = RTE_ETH_FC_NONE;
  return 0;
}

static int tap_flow_ctrl_set(struct rte_eth_dev *dev __rte_unused,
                             struct rte_eth_fc_conf *fc_conf) {
 ...
 */
static int tap_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_key && rss_conf->rss_key_len) {
     ...
     */
    TAP_LOG(ERR, "port %u RSS key cannot be updated", dev->data->port_id);
    rte_errno = EINVAL;
    return -rte_errno;
  }
  return 0;
...
static int tap_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  dev->data->rx_queue_state[rx_queue_id] = RTE_ETH_QUEUE_STATE_STARTED;

  return 0;
}

static int tap_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  dev->data->tx_queue_state[tx_queue_id] = RTE_ETH_QUEUE_STATE_STARTED;

  return 0;
}

static int tap_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  dev->data->rx_queue_state[rx_queue_id] = RTE_ETH_QUEUE_STATE_STOPPED;

  return 0;
}

static int tap_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
...
static int eth_dev_tap_create(struct rte_vdev_device *vdev,
                              const char *tap_name, char *remote_iface,
                              struct rte_ether_addr *mac_addr,
  ...
  if (!dev) {
    TAP_LOG(ERR, "%s Unable to allocate device struct", tuntap_name);
    goto error_exit_nodev;
  }

  process_private = (struct pmd_process_private *)rte_zmalloc_socket(
      tap_name, sizeof(struct pmd_process_private), RTE_CACHE_LINE_SIZE,
  ...
  if (pmd->ioctl_sock == -1) {
    TAP_LOG(ERR, "%s Unable to get a socket for management: %s", tuntap_name,
            strerror(errno));
    goto error_exit;
  }
  ...
  data->dev_flags = RTE_ETH_DEV_INTR_LSC | RTE_ETH_DEV_AUTOFILL_QUEUE_XSTATS;
  ...
  if (pmd->type == ETH_TUNTAP_TYPE_TAP) {
    memset(&ifr, 0, sizeof(struct ifreq));
    ifr.ifr_hwaddr.sa_family = AF_LOCAL;
    rte_memcpy(ifr.ifr_hwaddr.sa_data, &pmd->eth_addr, RTE_ETHER_ADDR_LEN);
  ...
  if (pmd->nlsk_fd == -1) {
    TAP_LOG(WARNING, "%s: failed to create netlink socket.", pmd->name);
  ...
  if (qdisc_create_multiq(pmd->nlsk_fd, pmd->if_index) < 0) {
    TAP_LOG(ERR, "%s: failed to create multiq qdisc.", pmd->name);
    goto disable_rte_flow;
  }
  if (qdisc_create_ingress(pmd->nlsk_fd, pmd->if_index) < 0) {
    TAP_LOG(ERR, "%s: failed to create ingress qdisc.", pmd->name);
    goto disable_rte_flow;
  }
  LIST_INIT(&pmd->flows);
  ...
  if (strlen(remote_iface)) {
    ...
    if (!pmd->remote_if_index) {
      TAP_LOG(ERR, "%s: failed to get %s if_index.", pmd->name, remote_iface);
      goto error_remote;
    }
    ...
    if (tap_ioctl(pmd, SIOCGIFHWADDR, &ifr, 0, REMOTE_ONLY) < 0) {
      TAP_LOG(ERR, "%s: failed to get %s MAC address.", pmd->name,
              pmd->remote_iface);
      goto error_remote;
    }
    rte_memcpy(&pmd->eth_addr, ifr.ifr_hwaddr.sa_data, RTE_ETHER_ADDR_LEN);
    ...
    if (qdisc_create_ingress(pmd->nlsk_fd, pmd->remote_if_index) < 0) {
      TAP_LOG(ERR, "%s: failed to create ingress qdisc.", pmd->remote_iface);
      goto error_remote;
    }
    ...
    if (tap_flow_implicit_create(pmd, TAP_REMOTE_TX) < 0 ||
      ...
      TAP_LOG(ERR, "%s: failed to create implicit rules.", pmd->name);
      goto error_remote;
    }
  }
...
disable_rte_flow:
  TAP_LOG(ERR, " Disabling rte flow support: %s(%d)", strerror(errno), errno);
  if (strlen(remote_iface)) {
...
error_remote:
  TAP_LOG(ERR, " Can't set up remote feature: %s(%d)", strerror(errno), errno);
...
error_exit_nodev:
  TAP_LOG(ERR, "%s Unable to initialize %s", tuntap_name,
...
static int set_interface_name(const char *key __rte_unused, const char *value,
                              void *extra_args) {
  char *name = (char *)extra_args;

  if (value) {
    if (!is_valid_iface(value)) {
      TAP_LOG(ERR, "TAP invalid remote interface name (%s)", value);
      return -1;
    }
...
static int set_remote_iface(const char *key __rte_unused, const char *value,
                            void *extra_args) {
  char *name = (char *)extra_args;
...
static int parse_user_mac(struct rte_ether_addr *user_mac, const char *value) {
  ...
  while ((mac_byte != NULL) && (strlen(mac_byte) <= 2) &&
...
static int set_mac_type(const char *key __rte_unused, const char *value,
                        void *extra_args) {
...
error:
  TAP_LOG(ERR, "TAP user MAC (%s) is not in format (%s|%s)", value,
          ETH_TAP_MAC_FIXED, ETH_TAP_USR_MAC_FMT);
  return -1;
}
 ...
 */
static int rte_pmd_tun_probe(struct rte_vdev_device *dev) {
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY && strlen(params) == 0) {
  ...
  if (params && (params[0] != '\0')) {
    ...
    if (kvlist) {
      if (rte_kvargs_count(kvlist, ETH_TAP_IFACE_ARG) == 1) {
        ret = rte_kvargs_process(kvlist, ETH_TAP_IFACE_ARG, &set_interface_name,
                                 tun_name);
  ...
  ret = eth_dev_tap_create(dev, tun_name, remote_iface, 0, ETH_TUNTAP_TYPE_TUN);

leave:
  if (ret == -1) {
    TAP_LOG(ERR, "Failed to create pmd for %s as %s", name, tun_name);
  }
...
static int tap_mp_attach_queues(const char *port_name,
                                struct rte_eth_dev *dev) {
  int ret;
...
static int tap_mp_sync_queues(const struct rte_mp_msg *request,
                              const void *peer) {
  struct rte_eth_dev *dev;
  struct pmd_process_private *process_private;
 ...
 */
static int rte_pmd_tap_probe(struct rte_vdev_device *dev) {
  const char *name, *params;
  int ret;
  struct rte_kvargs *kvlist = NULL;
  int speed;
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ...
    if (!tap_devices_count) {
      ret =
          rte_mp_action_register(TAP_MP_REQ_START_RXTX, tap_mp_req_start_rxtx);
  ...
  if (params && (params[0] != '\0')) {
    ...
    if (kvlist) {
      ...
      if (rte_kvargs_count(kvlist, ETH_TAP_REMOTE_ARG) == 1) {
        ret = rte_kvargs_process(kvlist, ETH_TAP_REMOTE_ARG, &set_remote_iface,
                                 remote_iface);
        if (ret == -1)
          goto leave;
      }
      ...
      if (rte_kvargs_count(kvlist, ETH_TAP_MAC_ARG) == 1) {
        ret = rte_kvargs_process(kvlist, ETH_TAP_MAC_ARG, &set_mac_type,
                                 &user_mac);
        if (ret == -1)
          goto leave;
      }
...
leave:
  if (ret == -1) {
    TAP_LOG(ERR, "Failed to create pmd for %s as %s", name, tap_name);
 ...
 */
static int rte_pmd_tap_remove(struct rte_vdev_device *dev) {
...
RTE_PMD_REGISTER_PARAM_STRING(net_tun, ETH_TAP_IFACE_ARG "=<string> ");
RTE_PMD_REGISTER_PARAM_STRING(net_tap, ETH_TAP_IFACE_ARG
                              "=<string> " ETH_TAP_MAC_ARG
                              "=" ETH_TAP_MAC_ARG_FMT " " ETH_TAP_REMOTE_ARG
                              "=<string>");
