...
#define BUF_SIZE                                                               \
  (32 * 1024)             /* Size of the buffer to receive kernel messages     \
                           */
...
int tap_nl_final(int nlsk_fd) {
  if (close(nlsk_fd)) {
    TAP_LOG(ERR, "Failed to close netlink socket: %s (%d)", strerror(errno),
            errno);
    return -1;
  }
  return 0;
...
retry:
  ...
  if (send_bytes < 0) {
    ...
    TAP_LOG(ERR, "Failed to send netlink message: %s (%d)", strerror(errno),
            errno);
    return -1;
  }
  return send_bytes;
...
#ifdef NETLINK_EXT_ACK
static const struct nlattr *tap_nl_attr_first(const struct nlmsghdr *nh,
                                              size_t offset) {
  return (const struct nlattr *)((const char *)nh + NLMSG_SPACE(offset));
}

static const struct nlattr *tap_nl_attr_next(const struct nlattr *attr) {
  return (const struct nlattr *)((const char *)attr +
                                 NLMSG_ALIGN(attr->nla_len));
}

static bool tap_nl_attr_ok(const struct nlattr *attr, int len) {
...
static void tap_nl_dump_ext_ack(const struct nlmsghdr *nh,
  ...
  for (attr = tap_nl_attr_first(nh, hlen);
    ...
    if (type == NLMSGERR_ATTR_MSG) {
      const char *msg = (const char *)attr + NLMSG_ALIGN(sizeof(*attr));
 ...
 */
#define tap_nl_dump_ext_ack(nh, err)                                           \
  do {                                                                         \
  } while (0)
#endif
 ...
 */
int tap_nl_recv_ack(int nlsk_fd) { return tap_nl_recv(nlsk_fd, NULL, NULL); }
 ...
 *   0 on success, -1 otherwise with errno set.
 */
int tap_nl_recv(int nlsk_fd, int (*cb)(struct nlmsghdr *, void *arg),
                void *arg) {
  char buf[BUF_SIZE];
  int multipart = 0;
  int ret = 0;
  ...
  retry:
    ...
    for (nh = (struct nlmsghdr *)buf; NLMSG_OK(nh, (unsigned int)recv_bytes);
...
int tap_nlattr_nested_start(struct nlmsg *msg, uint16_t type) {
  ...
  if (!tail) {
    TAP_LOG(ERR, "Couldn't allocate memory for nested netlink attribute");
    return -1;
  }
