...
static int tap_flow_validate(struct rte_eth_dev *dev,
                             const struct rte_flow_attr *attr,
                             const struct rte_flow_item items[],
                             const struct rte_flow_action actions[],
                             struct rte_flow_error *error);

static struct rte_flow *tap_flow_create(struct rte_eth_dev *dev,
                                        const struct rte_flow_attr *attr,
                                        const struct rte_flow_item items[],
                                        const struct rte_flow_action actions[],
                                        struct rte_flow_error *error);

static void tap_flow_free(struct pmd_internals *pmd, struct rte_flow *flow);

static int tap_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                            struct rte_flow_error *error);

static int tap_flow_isolate(struct rte_eth_dev *dev, int set,
                            struct rte_flow_error *error);
...
#define ITEMS(...)                                                             \
  (const enum rte_flow_item_type[]) { __VA_ARGS__, RTE_FLOW_ITEM_TYPE_END, }
...
static const struct tap_flow_items tap_flow_items[] = {
    ...
    [RTE_FLOW_ITEM_TYPE_ETH] =
        {
            .items = ITEMS(RTE_FLOW_ITEM_TYPE_VLAN, RTE_FLOW_ITEM_TYPE_IPV4,
    ...
    [RTE_FLOW_ITEM_TYPE_VLAN] =
        {
            .items = ITEMS(RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_IPV6),
            .mask =
...
/* DEI matching is not supported */
    ...
    [RTE_FLOW_ITEM_TYPE_IPV4] =
        {
            .items = ITEMS(RTE_FLOW_ITEM_TYPE_UDP, RTE_FLOW_ITEM_TYPE_TCP),
            .mask =
...
static int tap_flow_create_eth(const struct rte_flow_item *item, void *data) {
  ...
  if (!rte_is_zero_ether_addr(&mask->dst)) {
    tap_nlattr_add(&msg->nh, TCA_FLOWER_KEY_ETH_DST, RTE_ETHER_ADDR_LEN,
                   &spec->dst.addr_bytes);
    tap_nlattr_add(&msg->nh, TCA_FLOWER_KEY_ETH_DST_MASK, RTE_ETHER_ADDR_LEN,
                   &mask->dst.addr_bytes);
  }
  if (!rte_is_zero_ether_addr(&mask->src)) {
    tap_nlattr_add(&msg->nh, TCA_FLOWER_KEY_ETH_SRC, RTE_ETHER_ADDR_LEN,
                   &spec->src.addr_bytes);
    tap_nlattr_add(&msg->nh, TCA_FLOWER_KEY_ETH_SRC_MASK, RTE_ETHER_ADDR_LEN,
                   &mask->src.addr_bytes);
  }
  return 0;
}
...
#define VLAN_ID(tci) ((tci)&0xfff)
  ...
  if (spec->tci) {
    ...
    if (prio)
      tap_nlattr_add8(&msg->nh, TCA_FLOWER_KEY_VLAN_PRIO, prio);
    if (vid)
      tap_nlattr_add16(&msg->nh, TCA_FLOWER_KEY_VLAN_ID, vid);
  }
  return 0;
}
...
static int tap_flow_create_ipv4(const struct rte_flow_item *item, void *data) {
  ...
  if (mask->hdr.dst_addr) {
    tap_nlattr_add32(&msg->nh, TCA_FLOWER_KEY_IPV4_DST, spec->hdr.dst_addr);
    tap_nlattr_add32(&msg->nh, TCA_FLOWER_KEY_IPV4_DST_MASK,
                     mask->hdr.dst_addr);
  }
  if (mask->hdr.src_addr) {
    tap_nlattr_add32(&msg->nh, TCA_FLOWER_KEY_IPV4_SRC, spec->hdr.src_addr);
...
static int tap_flow_create_ipv6(const struct rte_flow_item *item, void *data) {
  ...
  if (spec->hdr.proto)
    tap_nlattr_add8(&msg->nh, TCA_FLOWER_KEY_IP_PROTO, spec->hdr.proto);
  return 0;
}
...
static int tap_flow_create_udp(const struct rte_flow_item *item, void *data) {
  ...
  if (mask->hdr.dst_port)
    tap_nlattr_add16(&msg->nh, TCA_FLOWER_KEY_UDP_DST, spec->hdr.dst_port);
  if (mask->hdr.src_port)
    tap_nlattr_add16(&msg->nh, TCA_FLOWER_KEY_UDP_SRC, spec->hdr.src_port);
  return 0;
}
...
static int tap_flow_create_tcp(const struct rte_flow_item *item, void *data) {
  ...
  if (!spec)
    return 0;
  if (mask->hdr.dst_port)
    tap_nlattr_add16(&msg->nh, TCA_FLOWER_KEY_TCP_DST, spec->hdr.dst_port);
  if (mask->hdr.src_port)
    tap_nlattr_add16(&msg->nh, TCA_FLOWER_KEY_TCP_SRC, spec->hdr.src_port);
  return 0;
}
 ...
 *   0 on success.
 */
static int tap_flow_item_validate(const struct rte_flow_item *item,
                                  unsigned int size,
  ...
  if (item->spec && !item->mask) {
    ...
    for (i = 0; i < size; i++)
      if ((default_mask[i] | supported_mask[i]) != supported_mask[i])
        return -1;
  }
 ...
 */
static int add_action(struct rte_flow *flow, size_t *act_index,
  ...
  tap_nlattr_add(&msg->nh, TCA_ACT_KIND, strlen(adata->id) + 1, adata->id);
  if (tap_nlattr_nested_start(msg, TCA_ACT_OPTIONS) < 0)
    return -1;
  if (strcmp("gact", adata->id) == 0) {
    tap_nlattr_add(&msg->nh, TCA_GACT_PARMS, sizeof(adata->gact), &adata->gact);
    ...
    tap_nlattr_add(&msg->nh, TCA_MIRRED_PARMS, sizeof(adata->mirred),
                   &adata->mirred);
  } else if (strcmp("skbedit", adata->id) == 0) {
    tap_nlattr_add(&msg->nh, TCA_SKBEDIT_PARMS, sizeof(adata->skbedit.skbedit),
    ...
    tap_nlattr_add(&msg->nh, TCA_ACT_BPF_NAME,
                   strlen(adata->bpf.annotation) + 1, adata->bpf.annotation);
    tap_nlattr_add(&msg->nh, TCA_ACT_BPF_PARMS, sizeof(adata->bpf.bpf),
                   &adata->bpf.bpf);
  } else {
    return -1;
  }
 ...
 *   -1 on failure, 0 on success
 */
static int add_actions(struct rte_flow *flow, int nb_actions,
 ...
 */
static int priv_flow_process(struct pmd_internals *pmd,
                             const struct rte_flow_attr *attr,
                             const struct rte_flow_item items[],
                             const struct rte_flow_action actions[],
                             struct rte_flow_error *error,
                             struct rte_flow *flow, int mirred) {
  ...
  if (attr->group > MAX_GROUP) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_GROUP, NULL,
                       "group value too big: cannot exceed 15");
    return -rte_errno;
  }
  if (attr->priority > MAX_PRIORITY) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY, NULL,
                       "priority value too big");
    return -rte_errno;
  } else if (flow) {
    uint16_t group = attr->group << GROUP_SHIFT;
    uint16_t prio =
        group | (attr->priority + RSS_PRIORITY_OFFSET + PRIORITY_OFFSET);
    flow->msg.t.tcm_info = TC_H_MAKE(prio << 16, flow->msg.t.tcm_info);
  }
  if (flow) {
    if (mirred) {
      /*
  ...
  for (; items->type != RTE_FLOW_ITEM_TYPE_END; ++items) {
    ...
    if (items->type == RTE_FLOW_ITEM_TYPE_VOID)
      continue;
    for (i = 0; cur_item->items && cur_item->items[i] != RTE_FLOW_ITEM_TYPE_END;
         ++i) {
    ...
    if (!token)
      goto exit_item_not_supported;
    cur_item = token;
    err = tap_flow_item_validate(items, cur_item->mask_sz,
  ...
  if (flow) {
    if (data.vlan) {
      tap_nlattr_add16(&flow->msg.nh, TCA_FLOWER_KEY_ETH_TYPE,
                       htons(ETH_P_8021Q));
      tap_nlattr_add16(&flow->msg.nh, TCA_FLOWER_KEY_VLAN_ETH_TYPE,
                       data.eth_type ? data.eth_type : htons(ETH_P_ALL));
    } else if (data.eth_type) {
      tap_nlattr_add16(&flow->msg.nh, TCA_FLOWER_KEY_ETH_TYPE, data.eth_type);
    }
  }
  if (mirred && flow) {
    struct action_data adata = {
     ...
     */
    adata.mirred.ifindex = attr->ingress ? pmd->if_index : pmd->remote_if_index;
...
exit_item_not_supported:
  rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, items,
                     "item not supported");
  return -rte_errno;
 ...
 */
static void tap_flow_set_handle(struct rte_flow *flow) {
  union {
    struct rte_flow *flow;
    const void *key;
  } tmp;
...
static struct rte_flow *tap_flow_create(struct rte_eth_dev *dev,
  ...
  if (err < 0) {
    TAP_LOG(ERR, "Kernel refused TC filter rule creation (%d): %s", errno,
            strerror(errno));
    rte_flow_error_set(
        error, EEXIST, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
  ...
  if (pmd->remote_if_index) {
    ...
    tc_init_msg(msg, pmd->remote_if_index, RTM_NEWTFILTER,
                NLM_F_REQUEST | NLM_F_ACK | NLM_F_EXCL | NLM_F_CREATE);
    msg->t.tcm_info = TC_H_MAKE(0, htons(ETH_P_ALL));
    tap_flow_set_handle(remote_flow);
    if (priv_flow_process(pmd, attr, items, NULL, error, remote_flow,
                          TCA_EGRESS_REDIR)) {
      rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
 ...
 */
static int tap_flow_destroy_pmd(struct pmd_internals *pmd,
                                struct rte_flow *flow,
                                struct rte_flow_error *error) {
  ...
  if (ret < 0 && errno == ENOENT)
    ret = 0;
  if (ret < 0) {
    TAP_LOG(ERR, "Kernel refused TC filter rule deletion (%d): %s", errno,
            strerror(errno));
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
...
int tap_flow_implicit_create(struct pmd_internals *pmd,
  ...
  struct nlmsg *msg = NULL;
  int err = 0;
  struct rte_flow_item items_local[2] = {[0] =
                                             {
                                                 .type = items[0].type,
                                                 .spec = &eth_local,
                                                 .mask = items[0].mask,
  ...
  if (idx == TAP_REMOTE_TX) {
    ...
    isolate_actions[0].type = pmd->flow_isolate ? RTE_FLOW_ACTION_TYPE_DROP
  ...
  if (idx == TAP_ISOLATE)
    ...
    tap_flow_set_handle(remote_flow);
  if (priv_flow_process(pmd, attr, items, actions, NULL, remote_flow,
                        implicit_rte_flows[idx].mirred)) {
...
int tap_flow_implicit_destroy(struct pmd_internals *pmd,
  ...
  for (remote_flow = LIST_FIRST(&pmd->implicit_flows); remote_flow;
...
static int rss_enable(struct pmd_internals *pmd,
   ...
   */
  pmd->map_fd =
      tap_flow_bpf_rss_map_create(sizeof(__u32), /* key size */
                                  sizeof(struct rss_key), MAX_RSS_KEYS);
  if (pmd->map_fd < 0) {
    TAP_LOG(ERR, "Failed to create BPF map (%d): %s", errno, strerror(errno));
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
  ...
  for (i = 0; i < pmd->dev->data->nb_rx_queues; i++) {
    ...
    if (pmd->bpf_fd[i] < 0) {
      TAP_LOG(ERR, "Failed to load BPF section %s for queue %d", SEC_NAME_CLS_Q,
              i);
      rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
                         "Kernel too old or not configured "
    ...
    snprintf(annotation, sizeof(annotation), "[%s%d]", SEC_NAME_CLS_Q, i);
    tap_nlattr_add(&msg->nh, TCA_BPF_NAME, strlen(annotation) + 1, annotation);
    /* Actions */
    {
      struct action_data adata = {
          .id = "skbedit",
...
static int rss_add_actions(struct rte_flow *flow, struct pmd_internals *pmd,
  ...
  unsigned int i;
  int err;
  struct rss_key rss_entry = {.hash_fields = 0, .key_size = 0};
  ...
  err = tap_flow_bpf_update_rss_elem(pmd->map_fd, &flow->key_idx, &rss_entry);

  if (err) {
    TAP_LOG(ERR, "Failed to update BPF map entry #%u (%d): %s", flow->key_idx,
  ...
  if (flow->bpf_fd[SEC_L3_L4] < 0) {
    TAP_LOG(ERR, "Failed to load BPF section %s (%d): %s", sec_name[SEC_L3_L4],
            errno, strerror(errno));
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
                       "Kernel too old or not configured "
  ...
  /* Actions */
  {
    ...
    if (add_actions(flow, RTE_DIM(adata), adata, TCA_FLOWER_ACT) < 0)
      return -1;
  }
 ...
 */
int tap_dev_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
