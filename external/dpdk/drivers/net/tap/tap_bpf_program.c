...
/** Create IPv4 address */
#define IPv4(a, b, c, d)                                                       \
  ((__u32)(((a)&0xff) << 24) | (((b)&0xff) << 16) | (((c)&0xff) << 8) |        \
   ((d)&0xff))

#define PORT(a, b) ((__u16)(((a)&0xff) << 8) | ((b)&0xff))
...
struct bpf_elf_map __attribute__((section("maps"), used)) map_keys = {
...
__section("cls_q") int match_q(struct __sk_buff *skb) {
    ...
    for (i = 0; i < 32; i++) {
      if (input_tuple[j] & (1U << (31 - i))) {
        hash ^= ((const __u32 *)def_rss_key)[j] << i |
                (__u32)((uint64_t)(((const __u32 *)def_rss_key)[j + 1]) >>
                        (32 - i));
      }
    }
  }
...
static int __attribute__((always_inline)) rss_l3_l4(struct __sk_buff *skb) {
  ...
  if (skb->vlan_present && skb->vlan_proto == htons(ETH_P_8021AD)) {
    if (data + ETH_ALEN * 2 + sizeof(struct vlan_hdr) + sizeof(proto) >
        data_end)
      return TC_ACT_OK;
    proto = *(__u16 *)(data + ETH_ALEN * 2 + sizeof(struct vlan_hdr));
    off += sizeof(struct vlan_hdr);
  }

  if (proto == htons(ETH_P_IP)) {
    if (data + off + sizeof(struct iphdr) + sizeof(__u32) > data_end)
      return TC_ACT_OK;

    __u8 *src_dst_addr = data + off + offsetof(struct iphdr, saddr);
    __u8 *src_dst_port = data + off + sizeof(struct iphdr);
    struct ipv4_l3_l4_tuple v4_tuple = {
        .src_addr = IPv4(*(src_dst_addr + 0), *(src_dst_addr + 1),
                         *(src_dst_addr + 2), *(src_dst_addr + 3)),
        .dst_addr = IPv4(*(src_dst_addr + 4), *(src_dst_addr + 5),
                         *(src_dst_addr + 6), *(src_dst_addr + 7)),
        .sport = PORT(*(src_dst_port + 0), *(src_dst_port + 1)),
        .dport = PORT(*(src_dst_port + 2), *(src_dst_port + 3)),
    };
    ...
    if (data + off + sizeof(struct ipv6hdr) + sizeof(__u32) > data_end)
      return TC_ACT_OK;
    __u8 *src_dst_addr = data + off + offsetof(struct ipv6hdr, saddr);
    __u8 *src_dst_port = data + off + sizeof(struct ipv6hdr);
    struct ipv6_l3_l4_tuple v6_tuple;
    for (j = 0; j < 4; j++)
      *((uint32_t *)&v6_tuple.src_addr + j) =
          __builtin_bswap32(*((uint32_t *)src_dst_addr + j));
    for (j = 0; j < 4; j++)
      *((uint32_t *)&v6_tuple.dst_addr + j) =
          __builtin_bswap32(*((uint32_t *)src_dst_addr + 4 + j));
    v6_tuple.sport = PORT(*(src_dst_port + 0), *(src_dst_port + 1));
    v6_tuple.dport = PORT(*(src_dst_port + 2), *(src_dst_port + 3));
  ...
  queue = rsskey->queues[(hash % rsskey->nb_queues) & (TAP_MAX_QUEUES - 1)];
...
#define RSS(L)                                                                 \
  __section(#L) int L##_hash(struct __sk_buff *skb) { return rss_##L(skb); }
