 ...
 */
int nfb_eth_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id,
 ...
 *   0 on success, a negative errno value otherwise.
 */
int nfb_eth_tx_queue_init(struct nfb_device *nfb, uint16_t tx_queue_id,
 ...
 *   Receive queue index.
 */
void nfb_eth_tx_queue_release(struct rte_eth_dev *dev, uint16_t qid);
 ...
 *   0 on success, a negative errno value otherwise.
 */
int nfb_eth_tx_queue_start(struct rte_eth_dev *dev, uint16_t txq_id);
 ...
 *   0 on success, a negative errno value otherwise.
 */
int nfb_eth_tx_queue_stop(struct rte_eth_dev *dev, uint16_t txq_id);
 ...
 */
static __rte_always_inline uint16_t nfb_eth_ndp_tx(void *queue,
  ...
  for (i = 0; i < nb_pkts; ++i) {
    ...
    if (mbuf_segs == 1) {
       ...
       */
      rte_memcpy(packets[i].data, rte_pktmbuf_mtod(mbuf, const void *),
                 pkt_len);
    } else {
      ...
      while (m) {
        ...
        rte_memcpy(dst, rte_pktmbuf_mtod(m, const void *), m->data_len);
