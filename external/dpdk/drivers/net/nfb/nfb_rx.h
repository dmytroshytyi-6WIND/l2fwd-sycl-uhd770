...
nfb_timestamp_dynfield(struct rte_mbuf *mbuf) {
  return RTE_MBUF_DYNFIELD(mbuf, nfb_timestamp_dynfield_offset,
 ...
 */
int nfb_eth_rx_queue_init(struct nfb_device *nfb, uint16_t rx_queue_id,
                          uint16_t port_id, struct rte_mempool *mb_pool,
 ...
 *   0 on success, a negative errno value otherwise.
 */
int nfb_eth_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
 ...
 *   Receive queue index.
 */
void nfb_eth_rx_queue_release(struct rte_eth_dev *dev, uint16_t qid);
 ...
 *   0 on success, a negative errno value otherwise.
 */
int nfb_eth_rx_queue_start(struct rte_eth_dev *dev, uint16_t rxq_id);
 ...
 *   RX queue index.
 */
int nfb_eth_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rxq_id);
 ...
 */
static __rte_always_inline uint16_t nfb_eth_ndp_rx(void *queue,
  ...
  for (i = 0; i < nb_pkts; ++i) {
    ...
    if (likely(packet_size <= buf_size)) {
      ...
      rte_memcpy(rte_pktmbuf_mtod(mbuf, void *), packets[i].data, packet_size);
      ...
      if (nfb_timestamp_dynfield_offset >= 0) {
        ...
        /* nanoseconds */
        timestamp = rte_le_to_cpu_32(*((uint32_t *)(packets[i].header + 4)));
        timestamp <<= 32;
        /* seconds */
        timestamp |= rte_le_to_cpu_32(*((uint32_t *)(packets[i].header + 8)));
