 ...
 */
static void nfb_nc_rxmac_init(struct nfb_device *nfb,
 ...
 */
static void nfb_nc_txmac_init(struct nfb_device *nfb,
 ...
 */
static void nfb_nc_rxmac_deinit(struct nc_rxmac *rxmac[RTE_MAX_NC_RXMAC],
                                uint16_t max_rxmac) {
  uint16_t i;
 ...
 */
static void nfb_nc_txmac_deinit(struct nc_txmac *txmac[RTE_MAX_NC_TXMAC],
                                uint16_t max_txmac) {
  uint16_t i;
 ...
 */
static int nfb_eth_dev_start(struct rte_eth_dev *dev) {
  int ret;
  uint16_t i;
 ...
 *   Pointer to Ethernet device structure.
 */
static int nfb_eth_dev_stop(struct rte_eth_dev *dev) {
  uint16_t i;
  uint16_t nb_rx = dev->data->nb_rx_queues;
  uint16_t nb_tx = dev->data->nb_tx_queues;
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int nfb_eth_dev_configure(struct rte_eth_dev *dev __rte_unused) {
  int ret;
  struct pmd_internals *internals = dev->data->dev_private;
  struct rte_eth_conf *dev_conf = &dev->data->dev_conf;

  if (dev_conf->rxmode.offloads & RTE_ETH_RX_OFFLOAD_TIMESTAMP) {
    ret = rte_mbuf_dyn_rx_timestamp_register(&nfb_timestamp_dynfield_offset,
                                             &nfb_timestamp_rx_dynflag);
    if (ret != 0) {
      RTE_LOG(ERR, PMD,
              "Cannot register Rx timestamp"
              " field/flag %d\n",
              ret);
...
static uint32_t nfb_eth_get_max_mac_address_count(struct rte_eth_dev *dev) {
  uint16_t i;
  uint32_t c;
 ...
 */
static int nfb_eth_dev_info(struct rte_eth_dev *dev,
 ...
 *   Pointer to Ethernet device structure.
 */
static int nfb_eth_dev_close(struct rte_eth_dev *dev) {
  struct pmd_internals *internals = dev->data->dev_private;
  uint16_t i;
  uint16_t nb_rx = dev->data->nb_rx_queues;
  uint16_t nb_tx = dev->data->nb_tx_queues;
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int nfb_eth_link_update(struct rte_eth_dev *dev,
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int nfb_eth_dev_set_link_up(struct rte_eth_dev *dev) {
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int nfb_eth_dev_set_link_down(struct rte_eth_dev *dev) {
  struct pmd_internals *internals =
      (struct pmd_internals *)dev->data->dev_private;
...
static uint64_t nfb_eth_mac_addr_conv(struct rte_ether_addr *mac_addr) {
  int i;
  uint64_t res = 0;
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int nfb_eth_mac_addr_set(struct rte_eth_dev *dev,
                                struct rte_ether_addr *mac_addr) {
  unsigned int i;
  uint64_t mac;
  struct rte_eth_dev_data *data = dev->data;
  struct pmd_internals *internals = (struct pmd_internals *)data->dev_private;
...
static int nfb_eth_mac_addr_add(struct rte_eth_dev *dev,
...
static void nfb_eth_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
  unsigned int i;
  struct rte_eth_dev_data *data = dev->data;
  struct pmd_internals *internals = (struct pmd_internals *)data->dev_private;
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int nfb_eth_dev_init(struct rte_eth_dev *dev) {
  uint32_t mac_count;
  struct rte_eth_dev_data *data = dev->data;
  struct pmd_internals *internals = (struct pmd_internals *)data->dev_private;
  ...
  snprintf(internals->nfb_dev, PATH_MAX, "/dev/nfb/by-pci-slot/" PCI_PRI_FMT,
           pci_addr->domain, pci_addr->bus, pci_addr->devid,
           pci_addr->function);

  /* Check validity of device args */
  if (dev->device->devargs != NULL && dev->device->devargs->args != NULL &&
  ...
  if (internals->nfb == NULL) {
    RTE_LOG(ERR, PMD, "nfb_open(): failed to open %s", internals->nfb_dev);
    return -EINVAL;
  }
  ...
  nfb_nc_rxmac_init(internals->nfb, internals->rxmac, &internals->max_rxmac);
  nfb_nc_txmac_init(internals->nfb, internals->txmac, &internals->max_txmac);
  ...
  data->mac_addrs =
      rte_zmalloc(data->name, sizeof(struct rte_ether_addr) * mac_count,
                  RTE_CACHE_LINE_SIZE);
  ...
  RTE_LOG(INFO, PMD, "NFB device (" PCI_PRI_FMT ") successfully initialized\n",
          pci_addr->domain, pci_addr->bus, pci_addr->devid, pci_addr->function);
...
static int nfb_eth_dev_uninit(struct rte_eth_dev *dev) {
  ...
  RTE_LOG(INFO, PMD,
          "NFB device (" PCI_PRI_FMT ") successfully uninitialized\n",
          pci_addr->domain, pci_addr->bus, pci_addr->devid, pci_addr->function);
...
static int nfb_eth_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct pmd_internals),
                                       nfb_eth_dev_init);
}
