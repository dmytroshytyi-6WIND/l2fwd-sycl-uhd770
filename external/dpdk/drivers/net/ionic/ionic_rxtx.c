...
static __rte_always_inline void ionic_tx_flush(struct ionic_tx_qcq *txq) {
...
void __rte_cold ionic_dev_tx_queue_release(struct rte_eth_dev *dev,
                                           uint16_t qid) {
  ...
  IONIC_PRINT(DEBUG, "TX queue %u pkts %ju tso %ju", txq->qcq.q.index,
              stats->packets, stats->tso);
...
int __rte_cold ionic_dev_tx_queue_stop(struct rte_eth_dev *eth_dev,
...
int __rte_cold ionic_dev_tx_queue_setup(struct rte_eth_dev *eth_dev,
                                        uint16_t tx_queue_id, uint16_t nb_desc,
                                        uint32_t socket_id,
 ...
 */
int __rte_cold ionic_dev_tx_queue_start(struct rte_eth_dev *eth_dev,
                                        uint16_t tx_queue_id) {
  ...
  if (tx_queue_state[tx_queue_id] == RTE_ETH_QUEUE_STATE_STARTED) {
    IONIC_PRINT(DEBUG, "TX queue %u already started", tx_queue_id);
    return 0;
  }

  txq = eth_dev->data->tx_queues[tx_queue_id];

  IONIC_PRINT(DEBUG, "Starting TX queue %u, %u descs", tx_queue_id,
              txq->qcq.q.num_descs);
...
static void ionic_tx_tcp_pseudo_csum(struct rte_mbuf *txm) {
  struct ether_hdr *eth_hdr = rte_pktmbuf_mtod(txm, struct ether_hdr *);
  char *l3_hdr = ((char *)eth_hdr) + txm->l2_len;
  struct rte_tcp_hdr *tcp_hdr = (struct rte_tcp_hdr *)(l3_hdr + txm->l3_len);
...
static void ionic_tx_tcp_inner_pseudo_csum(struct rte_mbuf *txm) {
  struct ether_hdr *eth_hdr = rte_pktmbuf_mtod(txm, struct ether_hdr *);
  char *l3_hdr =
      ((char *)eth_hdr) + txm->outer_l2_len + txm->outer_l3_len + txm->l2_len;
  struct rte_tcp_hdr *tcp_hdr = (struct rte_tcp_hdr *)(l3_hdr + txm->l3_len);
...
static void ionic_tx_tso_post(struct ionic_queue *q,
                              struct ionic_txq_desc *desc, struct rte_mbuf *txm,
                              rte_iova_t addr, uint8_t nsge, uint16_t len,
                              uint32_t hdrlen, uint32_t mss, bool encap,
                              uint16_t vlan_tci, bool has_vlan, bool start,
                              bool done) {
  void **info;
  uint8_t flags = 0;
  ...
  desc->cmd = encode_txq_desc_cmd(IONIC_TXQ_DESC_OPCODE_TSO, flags, nsge, addr);
  desc->len = len;
...
static int ionic_tx_tso(struct ionic_tx_qcq *txq, struct rte_mbuf *txm) {
  struct ionic_queue *q = &txq->qcq.q;
  struct ionic_tx_stats *stats = &txq->stats;
  ...
  if (encap) {
    ...
    hdrlen = txm->outer_l2_len + txm->outer_l3_len + txm->l2_len + txm->l3_len +
             txm->l4_len;
  } else {
  ...
  while (left > 0) {
    ...
    ionic_tx_tso_post(q, desc, txm, desc_addr, desc_nsge, desc_len, hdrlen, mss,
                      encap, vlan_tci, has_vlan, start, done);
    desc = ionic_tx_tso_next(txq, &elem);
    start = false;
    seglen = mss;
  }
  ...
  while (txm_seg != NULL) {
    ...
    while (left > 0) {
      ...
      ionic_tx_tso_post(q, desc, txm_seg, desc_addr, desc_nsge, desc_len,
                        hdrlen, mss, encap, vlan_tci, has_vlan, start, done);
      desc = ionic_tx_tso_next(txq, &elem);
      start = false;
    }
...
static __rte_always_inline int ionic_tx(struct ionic_tx_qcq *txq,
                                        struct rte_mbuf *txm) {
  struct ionic_queue *q = &txq->qcq.q;
...
#define IONIC_TX_OFFLOAD_MASK                                                  \
  (RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IPV6 | RTE_MBUF_F_TX_VLAN |              \
   RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_L4_MASK)
...
static void __rte_cold ionic_rx_empty(struct ionic_rx_qcq *rxq) {
  struct ionic_queue *q = &rxq->qcq.q;
  struct rte_mbuf *mbuf;
  void **info;
...
void __rte_cold ionic_dev_rx_queue_release(struct rte_eth_dev *dev,
                                           uint16_t qid) {
  ...
  IONIC_PRINT(DEBUG, "RX queue %u pkts %ju mtod %ju", rxq->qcq.q.index,
              stats->packets, stats->mtods);
...
int __rte_cold ionic_dev_rx_queue_setup(struct rte_eth_dev *eth_dev,
                                        uint16_t rx_queue_id, uint16_t nb_desc,
                                        uint32_t socket_id,
  ...
  if (rx_queue_id >= lif->nrxqcqs) {
    IONIC_PRINT(ERR, "Queue index %u not available (max %u queues)",
  ...
  if (!rte_is_power_of_2(nb_desc) || nb_desc < IONIC_MIN_RING_DESC ||
    ...
    IONIC_PRINT(ERR, "Bad descriptor count (%u) for queue %u (min: %u)",
  ...
  err = ionic_rx_qcq_alloc(lif, socket_id, rx_queue_id, nb_desc, &rxq);
  if (err) {
...
static __rte_always_inline void ionic_rx_clean(struct ionic_rx_qcq *rxq,
  ...
  struct ionic_rx_service *recv_args =
      (struct ionic_rx_service *)service_cb_arg;
  uint32_t buf_size = (uint16_t)(rte_pktmbuf_data_room_size(rxq->mb_pool) -
  ...
  if (cq_desc->len > max_frame_size || cq_desc->len == 0) {
    stats->bad_len++;
    ionic_rx_recycle(q, q_desc_index, rxm);
    return;
  }
  ...
  if (cq_desc->csum_flags & IONIC_RXQ_COMP_CSUM_F_CALC) {
    ...
    else if ((cq_desc->csum_flags & IONIC_RXQ_COMP_CSUM_F_TCP_BAD) ||
             (cq_desc->csum_flags & IONIC_RXQ_COMP_CSUM_F_UDP_BAD))
  ...
  case IONIC_PKT_TYPE_IPV4_TCP:
    pkt_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP;
    break;
  case IONIC_PKT_TYPE_IPV6_TCP:
    pkt_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP;
    break;
  case IONIC_PKT_TYPE_IPV4_UDP:
    pkt_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP;
    break;
  case IONIC_PKT_TYPE_IPV6_UDP:
    pkt_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_UDP;
    break;
  default: {
    struct rte_ether_hdr *eth_h = rte_pktmbuf_mtod(rxm, struct rte_ether_hdr *);
...
static __rte_always_inline int ionic_rx_fill(struct ionic_rx_qcq *rxq,
                                             uint32_t len) {
  struct ionic_queue *q = &rxq->qcq.q;
  ...
  for (i = ionic_q_space_avail(q); i; i--) {
    ...
    size = buf_size;
    desc->opcode =
        (nsegs > 1) ? IONIC_RXQ_DESC_OPCODE_SG : IONIC_RXQ_DESC_OPCODE_SIMPLE;
    rxm->next = NULL;
    ...
    if (size < len)
      IONIC_PRINT(ERR, "Rx SG size is not sufficient (%d < %d)", size, len);
 ...
 */
int __rte_cold ionic_dev_rx_queue_start(struct rte_eth_dev *eth_dev,
  ...
  if (rx_queue_state[rx_queue_id] == RTE_ETH_QUEUE_STATE_STARTED) {
    IONIC_PRINT(DEBUG, "RX queue %u already started", rx_queue_id);
    return 0;
  }

  rxq = eth_dev->data->rx_queues[rx_queue_id];

  IONIC_PRINT(DEBUG, "Starting RX queue %u, %u descs (size: %u)", rx_queue_id,
              rxq->qcq.q.num_descs, frame_size);
  ...
  if (ionic_rx_fill(rxq, frame_size) != 0) {
    IONIC_PRINT(ERR, "Could not alloc mbuf for queue:%d", rx_queue_id);
    return -1;
  }
...
static __rte_always_inline void ionic_rxq_service(struct ionic_rx_qcq *rxq,
                                                  uint32_t work_to_do,
                                                  void *service_cb_arg) {
  struct ionic_cq *cq = &rxq->qcq.cq;
  struct ionic_queue *q = &rxq->qcq.q;
  ...
  while (color_match(cq_desc->pkt_type_color, cq->done_color)) {
    ...
    do {
      ...
      ionic_rx_clean(rxq, curr_q_tail_idx, curr_cq_tail_idx, service_cb_arg);
 ...
 */
int __rte_cold ionic_dev_rx_queue_stop(struct rte_eth_dev *eth_dev,
                                       uint16_t rx_queue_id) {
  struct ionic_rx_qcq *rxq;
...
uint16_t ionic_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  uint32_t frame_size = rxq->qcq.lif->eth_dev->data->mtu + RTE_ETHER_HDR_LEN;
  struct ionic_rx_service service_cb_arg;
