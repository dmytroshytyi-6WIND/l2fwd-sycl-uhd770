...
static void ionic_lif_get_abs_stats(const struct ionic_lif *lif,
                                    struct rte_eth_stats *stats) {
  struct ionic_lif_stats *ls = &lif->info->stats;
  uint32_t i;
  uint32_t num_rx_q_counters =
      RTE_MIN(lif->nrxqcqs, (uint32_t)RTE_ETHDEV_QUEUE_STAT_CNTRS);
  uint32_t num_tx_q_counters =
      RTE_MIN(lif->ntxqcqs, (uint32_t)RTE_ETHDEV_QUEUE_STAT_CNTRS);

  memset(stats, 0, sizeof(*stats));

  if (ls == NULL) {
    IONIC_PRINT(DEBUG, "Stats on port %u not yet initialized", lif->port_id);
    return;
  }
  ...
  stats->ipackets =
      ls->rx_ucast_packets + ls->rx_mcast_packets + ls->rx_bcast_packets;

  stats->ibytes = ls->rx_ucast_bytes + ls->rx_mcast_bytes + ls->rx_bcast_bytes;

  for (i = 0; i < lif->nrxqcqs; i++) {
    struct ionic_rx_stats *rx_stats = &lif->rxqcqs[i]->stats;
    stats->imissed += rx_stats->no_cb_arg + rx_stats->bad_cq_status +
                      rx_stats->no_room + rx_stats->bad_len;
  }

  stats->imissed += ls->rx_ucast_drop_packets + ls->rx_mcast_drop_packets +
                    ls->rx_bcast_drop_packets;

  stats->imissed += ls->rx_queue_empty + ls->rx_dma_error +
                    ls->rx_queue_disabled + ls->rx_desc_fetch_error +
  ...
  for (i = 0; i < num_rx_q_counters; i++) {
    ...
    stats->q_errors[i] = rx_stats->no_cb_arg + rx_stats->bad_cq_status +
                         rx_stats->no_room + rx_stats->bad_len;
  }
  ...
  stats->opackets =
      ls->tx_ucast_packets + ls->tx_mcast_packets + ls->tx_bcast_packets;

  stats->obytes = ls->tx_ucast_bytes + ls->tx_mcast_bytes + ls->tx_bcast_bytes;
  ...
  stats->oerrors += ls->tx_ucast_drop_packets + ls->tx_mcast_drop_packets +
                    ls->tx_bcast_drop_packets;

  stats->oerrors += ls->tx_dma_error + ls->tx_queue_disabled +
                    ls->tx_desc_fetch_error + ls->tx_desc_data_error;
...
void ionic_lif_reset_stats(struct ionic_lif *lif) {
  ...
  for (i = 0; i < lif->nrxqcqs; i++) {
    memset(&lif->rxqcqs[i]->stats, 0, sizeof(struct ionic_rx_stats));
    memset(&lif->txqcqs[i]->stats, 0, sizeof(struct ionic_tx_stats));
  }
...
void ionic_dev_remove_mac(struct rte_eth_dev *eth_dev, uint32_t index) {
  struct ionic_lif *lif = IONIC_ETH_DEV_TO_LIF(eth_dev);
  ...
  if (index >= adapter->max_mac_addrs) {
    IONIC_PRINT(WARNING, "Index %u is above MAC filter limit %u", index,
...
int ionic_dev_set_mac(struct rte_eth_dev *eth_dev,
  ...
  if (!rte_is_zero_ether_addr((struct rte_ether_addr *)lif->mac_addr)) {
    IONIC_PRINT(INFO, "Deleting mac addr %pM", lif->mac_addr);
...
static int ionic_vlan_rx_add_vid(struct ionic_lif *lif, uint16_t vid) {
  struct ionic_admin_ctx ctx = {
      .pending_work = true,
      .cmd.rx_filter_add =
          {
...
static int ionic_vlan_rx_kill_vid(struct ionic_lif *lif, uint16_t vid) {
  struct ionic_admin_ctx ctx = {
      .pending_work = true,
      .cmd.rx_filter_del =
          {
...
static void ionic_lif_rx_mode(struct ionic_lif *lif, uint32_t rx_mode) {
  struct ionic_admin_ctx ctx = {
      .pending_work = true,
      .cmd.rx_mode_set =
          {
...
static void ionic_set_rx_mode(struct ionic_lif *lif, uint32_t rx_mode) {
...
int ionic_dev_promiscuous_enable(struct rte_eth_dev *eth_dev) {
  struct ionic_lif *lif = IONIC_ETH_DEV_TO_LIF(eth_dev);
...
int ionic_dev_promiscuous_disable(struct rte_eth_dev *eth_dev) {
  struct ionic_lif *lif = IONIC_ETH_DEV_TO_LIF(eth_dev);
  uint32_t rx_mode = lif->rx_mode;
...
int ionic_dev_allmulticast_enable(struct rte_eth_dev *eth_dev) {
  struct ionic_lif *lif = IONIC_ETH_DEV_TO_LIF(eth_dev);
  uint32_t rx_mode = lif->rx_mode;
...
int ionic_dev_allmulticast_disable(struct rte_eth_dev *eth_dev) {
  struct ionic_lif *lif = IONIC_ETH_DEV_TO_LIF(eth_dev);
  uint32_t rx_mode = lif->rx_mode;
...
static int ionic_qcq_alloc(struct ionic_lif *lif, uint8_t type,
                           size_t struct_size, uint32_t socket_id,
                           uint32_t index, const char *type_name,
                           uint16_t flags, uint16_t num_descs,
                           uint16_t desc_size, uint16_t cq_desc_size,
                           uint16_t sg_desc_size, struct ionic_qcq **qcq) {
  ...
  *qcq = NULL;
  ...
  new->q.info = rte_calloc_socket("ionic", num_descs, sizeof(void *),
  ...
  new->base_z =
      rte_eth_dma_zone_reserve(lif->eth_dev, type_name, index /* queue_idx */,
  ...
  cq_base = (void *)RTE_ALIGN((uintptr_t)q_base + q_size, rte_mem_page_size());
  cq_base_pa = RTE_ALIGN(q_base_pa + q_size, rte_mem_page_size());

  if (flags & IONIC_QCQ_F_SG) {
    sg_base =
        (void *)RTE_ALIGN((uintptr_t)cq_base + cq_size, rte_mem_page_size());
    sg_base_pa = RTE_ALIGN(cq_base_pa + cq_size, rte_mem_page_size());
    ionic_q_sg_map(&new->q, sg_base, sg_base_pa);
  }

  IONIC_PRINT(DEBUG,
              "Q-Base-PA = %#jx CQ-Base-PA = %#jx "
              "SG-base-PA = %#jx",
...
int ionic_rx_qcq_alloc(struct ionic_lif *lif, uint32_t socket_id,
  ...
  err = ionic_qcq_alloc(
      lif, IONIC_QTYPE_RXQ, sizeof(struct ionic_rx_qcq), socket_id, index, "rx",
      flags, nrxq_descs, sizeof(struct ionic_rxq_desc),
      sizeof(struct ionic_rxq_comp), sizeof(struct ionic_rxq_sg_desc),
...
int ionic_tx_qcq_alloc(struct ionic_lif *lif, uint32_t socket_id,
  ...
  err = ionic_qcq_alloc(
      lif, IONIC_QTYPE_TXQ, sizeof(struct ionic_tx_qcq), socket_id, index, "tx",
      flags, ntxq_descs, sizeof(struct ionic_txq_desc),
      sizeof(struct ionic_txq_comp), sizeof(struct ionic_txq_sg_desc_v1),
...
static int ionic_admin_qcq_alloc(struct ionic_lif *lif) {
  uint16_t flags = 0;
  int err;

  err = ionic_qcq_alloc(
      lif, IONIC_QTYPE_ADMINQ, sizeof(struct ionic_admin_qcq), rte_socket_id(),
      0, "admin", flags, IONIC_ADMINQ_LENGTH, sizeof(struct ionic_admin_cmd),
      sizeof(struct ionic_admin_comp), 0, (struct ionic_qcq **)&lif->adminqcq);
  if (err)
    return err;
...
static int ionic_notify_qcq_alloc(struct ionic_lif *lif) {
  ...
  err = ionic_qcq_alloc(
      lif, IONIC_QTYPE_NOTIFYQ, sizeof(struct ionic_notify_qcq),
      rte_socket_id(), 0, "notify", flags, IONIC_NOTIFYQ_LENGTH,
      sizeof(struct ionic_notifyq_cmd), sizeof(union ionic_notifyq_comp), 0,
...
static void *ionic_bus_map_dbpage(struct ionic_adapter *adapter, int page_num) {
...
static void ionic_lif_queue_identify(struct ionic_lif *lif) {
  ...
  for (qtype = 0; qtype < RTE_DIM(ionic_qtype_vers); qtype++) {
    ...
    ionic_dev_cmd_queue_identify(idev, IONIC_LIF_TYPE_CLASSIC, qtype,
                                 ionic_qtype_vers[qtype]);
    err = ionic_dev_cmd_wait_check(idev, IONIC_DEVCMD_TIMEOUT);
    if (err == -EINVAL) {
    ...
    qti->sg_desc_stride = rte_le_to_cpu_16(q_ident->sg_desc_stride);

    IONIC_PRINT(DEBUG, " qtype[%d].version = %d", qtype, qti->version);
    IONIC_PRINT(DEBUG, " qtype[%d].supported = %#x", qtype, qti->supported);
    IONIC_PRINT(DEBUG, " qtype[%d].features = %#jx", qtype, qti->features);
    IONIC_PRINT(DEBUG, " qtype[%d].desc_sz = %d", qtype, qti->desc_sz);
    IONIC_PRINT(DEBUG, " qtype[%d].comp_sz = %d", qtype, qti->comp_sz);
    IONIC_PRINT(DEBUG, " qtype[%d].sg_desc_sz = %d", qtype, qti->sg_desc_sz);
    IONIC_PRINT(DEBUG, " qtype[%d].max_sg_elems = %d", qtype,
                qti->max_sg_elems);
    IONIC_PRINT(DEBUG, " qtype[%d].sg_desc_stride = %d", qtype,
                qti->sg_desc_stride);
  }
}
...
int ionic_lif_alloc(struct ionic_lif *lif) {
  ...
  lif->txqcqs = rte_zmalloc(
      "ionic", sizeof(*lif->txqcqs) * adapter->max_ntxqs_per_lif, 0);
  ...
  lif->rxqcqs = rte_zmalloc(
      "ionic", sizeof(*lif->rxqcqs) * adapter->max_nrxqs_per_lif, 0);
  ...
  lif->info_z =
      rte_eth_dma_zone_reserve(lif->eth_dev, "lif_info", 0 /* queue_idx*/,
...
int ionic_lif_rss_config(struct ionic_lif *lif, const uint16_t types,
  ...
  unsigned int i;
  uint16_t tbl_sz = rte_le_to_cpu_16(adapter->ident.lif.eth.rss_ind_tbl_sz);
...
static int ionic_lif_rss_setup(struct ionic_lif *lif) {
  struct ionic_adapter *adapter = lif->adapter;
  ...
  if (!lif->rss_ind_tbl_z) {
    lif->rss_ind_tbl_z = rte_eth_dma_zone_reserve(
        lif->eth_dev, "rss_ind_tbl", 0 /* queue_idx */,
        sizeof(*lif->rss_ind_tbl) * tbl_sz, IONIC_ALIGN, rte_socket_id());
    if (!lif->rss_ind_tbl_z) {
      IONIC_PRINT(ERR, "OOM");
      return -ENOMEM;
    }
...
static void ionic_lif_rss_teardown(struct ionic_lif *lif) {
  if (!lif->rss_ind_tbl)
    return;
...
static void ionic_lif_adminq_deinit(struct ionic_lif *lif) {
  lif->adminqcq->flags &= ~IONIC_QCQ_F_INITED;
}

static void ionic_lif_notifyq_deinit(struct ionic_lif *lif) {
  ...
  ionic_intr_mask(idev->intr_ctrl, nqcq->intr.index, IONIC_INTR_MASK_SET);
...
static void ionic_link_status_check(struct ionic_lif *lif) {
  ...
  if ((link_up && adapter->link_up) || (!link_up && !adapter->link_up))
    return;

  if (link_up) {
    adapter->link_speed = rte_le_to_cpu_32(lif->info->status.link_speed);
    IONIC_PRINT(DEBUG, "Link up - %d Gbps", adapter->link_speed);
  } else {
...
static void ionic_lif_handle_fw_down(struct ionic_lif *lif) {
  ...
  if (lif->state & IONIC_LIF_F_UP) {
    IONIC_PRINT(NOTICE, "Surprise FW stop, stopping %s\n", lif->name);
    ionic_lif_stop(lif);
  }
...
static bool ionic_notifyq_cb(struct ionic_cq *cq, uint16_t cq_desc_index,
                             void *cb_arg) {
  ...
  case IONIC_EVENT_LINK_CHANGE:
    IONIC_PRINT(DEBUG,
                ...
                lif->name, cq_desc->event.eid, cq_desc->link_change.link_status,
  ...
  case IONIC_EVENT_RESET:
    IONIC_PRINT(NOTICE,
                ...
                lif->name, cq_desc->event.eid, cq_desc->reset.reset_code,
                cq_desc->reset.state);
...
int ionic_notifyq_handler(struct ionic_lif *lif, int budget) {
  ...
  work_done = ionic_qcq_service(&nqcq->qcq, budget, ionic_notifyq_cb, lif);

  if (lif->state & IONIC_LIF_F_LINK_CHECK_NEEDED)
    ionic_link_status_check(lif);

  ionic_intr_credits(idev->intr_ctrl, nqcq->intr.index, work_done,
                     IONIC_INTR_CRED_RESET_COALESCE);

  ionic_intr_mask(idev->intr_ctrl, nqcq->intr.index, IONIC_INTR_MASK_CLEAR);
...
static int ionic_lif_notifyq_init(struct ionic_lif *lif) {
  ...
  struct ionic_admin_ctx ctx = {
      ...
      .cmd.q_init = {
          ...
          .flags = rte_cpu_to_le_16(IONIC_QINIT_F_IRQ | IONIC_QINIT_F_ENA),
  ...
  IONIC_PRINT(DEBUG, "notifyq_init.ring_size %d", ctx.cmd.q_init.ring_size);
      ...
      .cmd.q_init =
          {
              ...
              .ver = lif->qtype_info[q->type].version,
              .index = rte_cpu_to_le_32(q->index),
              .flags = rte_cpu_to_le_16(IONIC_QINIT_F_SG | IONIC_QINIT_F_ENA),
...
static void ionic_lif_set_name(struct ionic_lif *lif) {
  struct ionic_admin_ctx ctx = {
      .pending_work = true,
      .cmd.lif_setattr =
          {
...
void ionic_lif_configure(struct ionic_lif *lif) {
  ...
  uint32_t ntxqs_per_lif = rte_le_to_cpu_32(cfg->queue_count[IONIC_QTYPE_TXQ]);
  uint32_t nrxqs_per_lif = rte_le_to_cpu_32(cfg->queue_count[IONIC_QTYPE_RXQ]);
  ...
  IONIC_PRINT(DEBUG, "Configuring LIF on port %u", lif->port_id);
...
int ionic_lif_start(struct ionic_lif *lif) {
  ...
  if (!lif->rx_mode) {
    IONIC_PRINT(DEBUG, "Setting RX mode on %s", lif->name);
  ...
  IONIC_PRINT(DEBUG,
              "Starting %u RX queues and %u TX queues "
              "on port %u",
...
int ionic_lifs_size(struct ionic_adapter *adapter) {
  struct ionic_identity *ident = &adapter->ident;
  union ionic_lif_config *cfg = &ident->lif.eth.config;
  ...
  if (nintrs > dev_nintrs) {
    IONIC_PRINT(ERR, "At most %d intr supported, minimum req'd is %u",
                dev_nintrs, nintrs);
    return -ENOSPC;
  }
