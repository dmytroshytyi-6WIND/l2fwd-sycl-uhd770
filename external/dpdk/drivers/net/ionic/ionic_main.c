...
static int ionic_adminq_check_err(struct ionic_admin_ctx *ctx, bool timeout) {
  const char *name;
  const char *status;
  ...
  if (ctx->comp.comp.status || timeout) {
    ...
    IONIC_PRINT(ERR, "%s (%d) failed: %s (%d)", name, ctx->cmd.cmd.opcode,
...
static bool ionic_adminq_service(struct ionic_cq *cq, uint16_t cq_desc_index,
 ...
 */
static int ionic_adminq_post(struct ionic_lif *lif,
                             struct ionic_admin_ctx *ctx) {
...
static int ionic_adminq_wait_for_completion(struct ionic_lif *lif,
                                            struct ionic_admin_ctx *ctx,
  ...
  while (ctx->pending_work && elapsed_usec < max_wait_usec) {
    ...
    ionic_qcq_service(&lif->adminqcq->qcq, budget, ionic_adminq_service, NULL);
...
int ionic_adminq_post_wait(struct ionic_lif *lif, struct ionic_admin_ctx *ctx) {
  ...
  done = ionic_adminq_wait_for_completion(lif, ctx, IONIC_DEVCMD_TIMEOUT);
  ...
  do {
    ...
    if (done) {
      IONIC_PRINT(DEBUG, "DEVCMD %d done took %ld usecs",
                  ioread8(&idev->dev_cmd->cmd.cmd.opcode), elapsed_usec);
      return 0;
    }
...
int ionic_identify(struct ionic_adapter *adapter) {
  ...
  ident->drv.os_dist = 0;
  snprintf(ident->drv.os_dist_str, sizeof(ident->drv.os_dist_str), "Unknown");
  ident->drv.kernel_ver = 0;
  snprintf(ident->drv.kernel_ver_str, sizeof(ident->drv.kernel_ver_str),
           "DPDK");
...
int ionic_port_identify(struct ionic_adapter *adapter) {
  ...
  IONIC_PRINT(INFO, "speed %d", rte_le_to_cpu_32(ident->port.config.speed));
  IONIC_PRINT(INFO, "mtu %d", rte_le_to_cpu_32(ident->port.config.mtu));
  ...
  IONIC_PRINT(INFO, "loopback_mode %d", ident->port.config.loopback_mode);
...
int ionic_port_init(struct ionic_adapter *adapter) {
  ...
  idev->port_info_sz = RTE_ALIGN(sizeof(*idev->port_info), rte_mem_page_size());

  snprintf(z_name, sizeof(z_name), "%s_port_%s_info", IONIC_DRV_NAME,
           adapter->name);

  idev->port_info_z =
      ionic_memzone_reserve(z_name, idev->port_info_sz, SOCKET_ID_ANY);
  if (!idev->port_info_z) {
