...
int32_t ionic_set_mac_type(struct ionic_hw *hw) {
  ...
  if (hw->vendor_id != IONIC_PENSANDO_VENDOR_ID) {
    IONIC_PRINT(ERR, "Unsupported vendor id: %" PRIx32 "", hw->vendor_id);
    return -EINVAL;
  }
  ...
  default:
    err = -EINVAL;
    IONIC_PRINT(ERR, "Unsupported device id: %" PRIx32 "", hw->device_id);
    break;
  }

  IONIC_PRINT(INFO, "Mac: %d (%d)", hw->mac.type, err);
