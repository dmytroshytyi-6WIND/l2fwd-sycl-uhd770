...
#define DRV_LOG(level, ...)                                                    \
  rte_log(RTE_LOG_##level, vdev_netvsc_logtype,                                \
...
static LIST_HEAD(, vdev_netvsc_ctx)
    vdev_netvsc_ctx_list = LIST_HEAD_INITIALIZER(vdev_netvsc_ctx_list);
 ...
 *   Context to destroy.
 */
static void vdev_netvsc_ctx_destroy(struct vdev_netvsc_ctx *ctx) {
  if (ctx->pipe[0] != -1)
    close(ctx->pipe[0]);
  if (ctx->pipe[1] != -1)
    close(ctx->pipe[1]);
 ...
 */
static int vdev_netvsc_iface_is_netvsc(const struct if_nameindex *iface) {
 ...
 */
static int vdev_netvsc_foreach_iface(
    int (*func)(const struct if_nameindex *iface,
                const struct rte_ether_addr *eth_addr, va_list ap),
    int is_netvsc, ...) {
 ...
 *   0 otherwise.
 */
static int vdev_netvsc_has_route(const struct if_nameindex *iface,
  ...
  if (res == -1) {
    ret = -errno;
    DRV_LOG(ERR, "cannot send socket message: %s", rte_strerror(errno));
    goto close;
  }
  ...
  if (len == -1) {
    ret = -errno;
    DRV_LOG(ERR, "cannot receive socket message: %s", rte_strerror(errno));
    goto close;
  }
    ...
    if (retaddr->ifa_family == family &&
      ...
      while (RTA_OK(retrta, attlen)) {
        if (retrta->rta_type == IFA_ADDRESS) {
          ret = 1;
          DRV_LOG(DEBUG, "interface %s has IP", iface->if_name);
          goto close;
        }
 ...
 */
static int vdev_netvsc_sysfs_readlink(char *buf, size_t size,
                                      const char *if_name,
                                      const char *relpath) {
  ...
  ret = snprintf(in, sizeof(in), "/sys/class/net/%s/%s", if_name, relpath);
 ...
 *   error.
 */
static int vdev_netvsc_device_probe(const struct if_nameindex *iface,
  ...
  if (strcmp(addr, "pci"))
    return 0;
  ret = vdev_netvsc_sysfs_readlink(buf, sizeof(buf), iface->if_name, "device");
  if (ret)
    return 0;
  addr = strrchr(buf, '/');
  addr = addr ? addr + 1 : buf;
 ...
 *   Ignored.
 */
static void vdev_netvsc_alarm(__rte_unused void *arg) {
  struct vdev_netvsc_ctx *ctx;
  int ret;

  LIST_FOREACH(ctx, &vdev_netvsc_ctx_list, entry) {
    ret = vdev_netvsc_foreach_iface(vdev_netvsc_device_probe, 0, ctx);
    if (ret < 0)
      break;
  }
  if (!vdev_netvsc_ctx_count)
    return;
  ret = rte_eal_alarm_set(VDEV_NETVSC_PROBE_MS * 1000, vdev_netvsc_alarm, NULL);
  if (ret < 0) {
    DRV_LOG(ERR, "unable to reschedule alarm callback: %s", rte_strerror(-ret));
  }
}
 ...
 *   error.
 */
static int vdev_netvsc_netvsc_probe(const struct if_nameindex *iface,
                                    const struct rte_ether_addr *eth_addr,
                                    va_list ap) {
  ...
  if (vdev_netvsc_has_route(iface, AF_INET) ||
    ...
    if (!specified)
      return 0;
    DRV_LOG(WARNING,
            "probably using routed NetVSC interface \"%s\""
            " (index %u)",
            iface->if_name, iface->if_index);
  }
  ...
  if (pipe(ctx->pipe) == -1) {
    ret = -errno;
    DRV_LOG(ERR, "cannot allocate control pipe for interface \"%s\": %s",
  ...
  for (i = 0; i != RTE_DIM(ctx->pipe); ++i) {
    ...
    if (flf != -1 && fcntl(ctx->pipe[i], F_SETFL, flf | O_NONBLOCK) != -1)
      continue;
    ret = -errno;
    DRV_LOG(ERR,
  ...
  i = 0;
  ret = snprintf(ctx->name, sizeof(ctx->name), "%s_id%u", name, ctx->id);
  ...
  ret = snprintf(ctx->devargs, sizeof(ctx->devargs),
                 "fd(%d),dev(net_tap_vsc%u,remote=%s)", ctx->pipe[0], ctx->id,
                 ctx->if_name);
  ...
  DRV_LOG(DEBUG, "added NetVSC interface \"%s\" to context list", ctx->if_name);
  return 0;
error:
  if (ctx)
 ...
 */
static int vdev_netvsc_vdev_probe(struct rte_vdev_device *dev) {
  ...
  struct rte_kvargs *kvargs =
      rte_kvargs_parse(args ? args : "", vdev_netvsc_arg);
  ...
  /* Gather interfaces. */
  ret = vdev_netvsc_foreach_iface(vdev_netvsc_netvsc_probe, 1, name, kvargs,
                                  specified, &matched);
  if (ret < 0)
    goto error;
  if (specified && matched < specified) {
    ...
    if (vdev_netvsc_foreach_iface(vdev_netvsc_netvsc_probe, 0, name, kvargs,
                                  specified, &matched) < 0)
      goto error;
...
ignore:
  ...
  if (vdev_netvsc_ctx_count) {
    ret =
        rte_eal_alarm_set(VDEV_NETVSC_PROBE_MS * 1000, vdev_netvsc_alarm, NULL);
    if (ret < 0)
      DRV_LOG(ERR, "unable to schedule alarm callback: %s", rte_strerror(-ret));
  }
  return 0;
}
 ...
 *   Always 0.
 */
static int vdev_netvsc_vdev_remove(__rte_unused struct rte_vdev_device *dev) {
...
RTE_PMD_REGISTER_PARAM_STRING(net_vdev_netvsc, VDEV_NETVSC_ARG_IFACE
                              "=<string> " VDEV_NETVSC_ARG_MAC
                              "=<string> " VDEV_NETVSC_ARG_FORCE
                              "=<int> " VDEV_NETVSC_ARG_IGNORE "=<int>");

/** Compare function for vdev find device operation. */
static int vdev_netvsc_cmp_rte_device(const struct rte_device *dev1,
 ...
 * devargs list.
 */
static void vdev_netvsc_scan_callback(__rte_unused void *arg) {
