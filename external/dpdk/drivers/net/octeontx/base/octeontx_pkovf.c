...
#define PKO_IOMEM_NULL                                                         \
  (struct octeontx_pko_iomem) { 0, 0, 0 }
...
static void *octeontx_pko_dq_vf_bar0(uint16_t txq) {
  int vf_ix;
...
static int octeontx_pko_dq_gdq(uint16_t txq) { return txq % PKO_VF_NUM_DQ; }

/**
 * Open a PKO DQ.
 */
static inline int octeontx_pko_dq_open(uint16_t txq) {
  ...
  *(volatile int64_t *)(pko_vf_ctl.fc_ctl + txq) =
  ...
  octeontx_write64(PKO_DQ_FC_DEPTH_PAGES, vf_bar0 + PKO_VF_DQ_FC_STATUS(gdq));

  /* Set the register to return descriptor (packet) count as DEPTH */
  /* KIND=1, NCB_QUERY_RSP=0 */
  octeontx_write64(1ull << PKO_DQ_KIND_BIT, vf_bar0 + PKO_VF_DQ_WM_CTL(gdq));
 ...
 */
static inline int octeontx_pko_dq_close(uint16_t txq) {
  unsigned int reg_off;
  uint8_t *vf_bar0;
  uint64_t rtn;
  int res;
...
static inline int octeontx_pko_dq_drain(uint16_t txq) {
  unsigned int gdq;
  uint8_t *vf_bar0;
  uint64_t reg;
...
octeontx_pko_dq_range_lookup(struct octeontx_pko_vf_ctl_s *ctl, uint64_t chanid,
  ...
  while (dq < RTE_DIM(ctl->dq_map)) {
    dq_base = dq;
    dq_cnt = 0;
    while (ctl->dq_map[dq].chanid == ~chanid && dq < RTE_DIM(ctl->dq_map)) {
      dq_cnt++;
      if (dq_cnt == dq_num)
        return dq_base;
      dq++;
...
octeontx_pko_dq_range_assign(struct octeontx_pko_vf_ctl_s *ctl, uint64_t chanid,
  ...
  while (dq_cnt < dq_num) {
    ...
    octeontx_log_dbg("DQ# %u assigned to CHAN# %" PRIx64 "", dq, chanid);
...
static inline int octeontx_pko_dq_claim(struct octeontx_pko_vf_ctl_s *ctl,
                                        unsigned int dq_base,
...
static inline int octeontx_pko_dq_free(struct octeontx_pko_vf_ctl_s *ctl,
                                       uint64_t chanid) {
  const uint64_t null_chanid = ~0ull;
...
static inline int octeontx_pko_chan_start(struct octeontx_pko_vf_ctl_s *ctl,
                                          uint64_t chanid) {
  unsigned int dq_vf;
  unsigned int dq, dq_cnt;
...
static inline int octeontx_pko_chan_stop(struct octeontx_pko_vf_ctl_s *ctl,
                                         uint64_t chanid) {
  ...
  while (dq < RTE_DIM(ctl->dq_map)) {
    ...
    if (res > 0)
      octeontx_log_err("draining DQ%d, buffers left: %x", dq, res);
...
static inline int octeontx_pko_channel_query(struct octeontx_pko_vf_ctl_s *ctl,
                                             uint64_t chanid, void *out,
                                             size_t out_elem_size,
                                             size_t dq_num,
                                             octeontx_pko_dq_getter_t getter) {
  octeontx_dq_t curr;
  unsigned int dq_vf;
  unsigned int dq;
  ...
  curr.ioreg_va =
      (void *)((uintptr_t)ctl->pko[dq_vf].bar0 + PKO_VF_DQ_OP_SEND((dq), 0));
  curr.fc_status_va = ctl->fc_ctl + dq_num;

  octeontx_log_dbg("lmtline=%p ioreg_va=%p fc_status_va=%p", curr.lmtline_va,
                   curr.ioreg_va, curr.fc_status_va);

  getter(&curr, (void *)iter);
  return 0;
}

int octeontx_pko_channel_query_dqs(int chanid, void *out, size_t out_elem_size,
                                   size_t dq_num,
                                   octeontx_pko_dq_getter_t getter) {
  struct octeontx_pko_vf_ctl_s *ctl = &pko_vf_ctl;
  int dq_cnt;

  dq_cnt = octeontx_pko_channel_query(ctl, chanid, out, out_elem_size, dq_num,
                                      getter);
  if (dq_cnt < 0)
    return -1;
...
int octeontx_pko_send_mtu(int port, int mtu) {
  ...
  res = octeontx_mbox_send(&hdr, &cfg, sizeof(mbox_pko_mtu_cfg_t), NULL, 0);
  if (res < 0)
    return -EACCES;
...
int octeontx_pko_init_fc(const size_t pko_vf_count) {
  ...
  fc_mem_size =
      sizeof(struct octeontx_pko_fc_ctl_s) * pko_vf_count * PKO_VF_NUM_DQ;
  ...
  pko_vf_ctl.fc_iomem.iova =
      rte_malloc_virt2iova((void *)pko_vf_ctl.fc_iomem.va);
  pko_vf_ctl.fc_iomem.size = fc_mem_size;

  pko_vf_ctl.fc_ctl = (struct octeontx_pko_fc_ctl_s *)pko_vf_ctl.fc_iomem.va;
  ...
  for (vf_idx = 0; vf_idx < pko_vf_count; vf_idx++) {
    ...
    reg = (pko_vf_ctl.fc_iomem.iova +
           (sizeof(struct octeontx_pko_fc_ctl_s) * dq_ix)) &
          ~0x7F;
    reg |=           /* BASE */
        (0x2 << 3) | /* HYST_BITS */
    ...
    octeontx_log_dbg("PKO: bar0 %p VF_idx %d DQ_FC_CFG=%" PRIx64 "", vf_bar0,
                     (int)vf_idx, reg);
  }
  return 0;
}

void octeontx_pko_fc_free(void) { rte_free(pko_vf_ctl.fc_iomem.va); }

static void octeontx_pkovf_setup(void) {
...
/* PKOVF pcie device*/
static int pkovf_probe(struct rte_pci_driver *pci_drv,
  ...
  if (pci_dev->mem_resource[0].addr == NULL ||
    ...
    octeontx_log_err("Empty bars %p %p", pci_dev->mem_resource[0].addr,
...
static const struct rte_pci_id pci_pkovf_map[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CAVIUM, PCI_DEVICE_ID_OCTEONTX_PKO_VF)},
    {
        .vendor_id = 0,
    },
};
