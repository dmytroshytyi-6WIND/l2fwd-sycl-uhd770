...
int octeontx_pki_port_open(int port) {
  ...
  res = octeontx_mbox_send(&hdr, &port_type, sizeof(pki_port_type_t), NULL, 0);
  if (res < 0)
    return -EACCES;
  return res;
}
...
int octeontx_pki_port_vlan_fltr_config(
    int port, pki_port_vlan_filter_config_t *fltr_cfg) {
  struct octeontx_mbox_hdr hdr;
  int res;
...
int octeontx_pki_port_vlan_fltr_entry_config(
    int port, pki_port_vlan_filter_entry_config_t *e_cfg) {
  struct octeontx_mbox_hdr hdr;
  int res;
...
/* PKIVF pcie device */
static int pkivf_probe(struct rte_pci_driver *pci_drv,
  ...
  if (pci_dev->mem_resource[0].addr == NULL) {
    octeontx_log_err("PKI Empty bar[0] %p", pci_dev->mem_resource[0].addr);
    return -ENODEV;
  }
...
static const struct rte_pci_id pci_pkivf_map[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CAVIUM, PCI_DEVICE_ID_OCTEONTX_PKI_VF)},
    {
        .vendor_id = 0,
    },
};
