 ...
 */
static inline uint64_t octeontx_reg_ldadd_u64(void *addr, int64_t off) {
  uint64_t old_val;

  __asm__ volatile(__LSE_PREAMBLE " ldadd	%1, %0, [%2]\n"
                   : "=r"(old_val)
                   : "r"(off), "r"(addr)
                   : "memory");
 ...
 * @return N/A
 */
static inline void octeontx_reg_lmtst(void *lmtline_va, void *ioreg_va,
  ...
  do {
    ...
    __asm__ volatile(__LSE_PREAMBLE " ldeor	xzr, %0, [%1]\n"
                     : "=r"(result)
                     : "r"(ioreg_va)
                     : "memory");
  } while (!result);
