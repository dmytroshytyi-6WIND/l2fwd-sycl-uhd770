...
static int octeontx_vlan_hw_filter(struct octeontx_nic *nic, uint8_t flag) {
...
int octeontx_dev_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                 int on) {
  struct octeontx_nic *nic = octeontx_pmd_priv(dev);
  struct octeontx_vlan_info *vlan = &nic->vlan_info;
  ...
  if (on) {
    entry =
        rte_zmalloc("octeontx_nic_vlan_entry", sizeof(struct vlan_entry), 0);
    if (!entry) {
  ...
  rc = octeontx_pki_port_vlan_fltr_entry_config(nic->port_id, &fltr_entry);
  if (rc != 0) {
...
int octeontx_dev_vlan_offload_fini(struct rte_eth_dev *dev) {
  struct octeontx_nic *nic = octeontx_pmd_priv(dev);
  struct octeontx_vlan_info *vlan = &nic->vlan_info;
  pki_port_vlan_filter_entry_config_t fltr_entry;
...
int octeontx_dev_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  if (fc_conf->pause_time || fc_conf->mac_ctrl_frame_fwd || fc_conf->autoneg) {
  ...
  if (fc_conf->high_water == fc->high_water &&
      fc_conf->low_water == fc->low_water && fc_conf->mode == fc->mode)
    return 0;
