...
octeontx_tx_checksum_offload(uint64_t *cmd_buf, const uint16_t flags,
                             struct rte_mbuf *m) {
  struct octeontx_send_hdr_s *send_hdr = (struct octeontx_send_hdr_s *)cmd_buf;
   ...
   */
  const uint8_t csum =
      (!(((ol_flags ^ RTE_MBUF_F_TX_UDP_CKSUM) >> 52) & 0x3) +
       (!(((ol_flags ^ RTE_MBUF_F_TX_TCP_CKSUM) >> 52) & 0x3) * 2) +
       (!(((ol_flags ^ RTE_MBUF_F_TX_SCTP_CKSUM) >> 52) & 0x3) * 3));

  const uint8_t is_tunnel_parsed =
      (!!(ol_flags & RTE_MBUF_F_TX_TUNNEL_GTP) ||
...
static __rte_always_inline uint16_t __octeontx_xmit_prepare(
    struct rte_mbuf *tx_pkt, uint64_t *cmd_buf, const uint16_t flag) {
  ...
  if (flag & OCCTX_TX_OFFLOAD_MBUF_NOFF_F)
    cmd_buf[0] |= (octeontx_prefree_seg(tx_pkt, &m_tofree) << 58);

  /* Mark mempool object as "put" since it is freed by PKO */
  if (!(cmd_buf[0] & (1ULL << 58)))
    RTE_MEMPOOL_CHECK_COOKIES(m_tofree->pool, (void **)&m_tofree, 1, 0);
  /* Get the gaura Id */
  gaura_id = octeontx_fpa_bufpool_gaura((uintptr_t)m_tofree->pool->pool_id);

  /* Setup PKO_SEND_BUFLINK_S */
  cmd_buf[nb_desc++] = PKO_SEND_BUFLINK_SUBDC |
                       PKO_SEND_BUFLINK_LDTYPE(0x1ull) |
                       PKO_SEND_BUFLINK_GAUAR((long)gaura_id) | data_len;
  cmd_buf[nb_desc++] = iova;

  return nb_desc;
}

static __rte_always_inline uint16_t __octeontx_xmit_mseg_prepare(
    struct rte_mbuf *tx_pkt, uint64_t *cmd_buf, const uint16_t flag) {
  ...
  do {
     ...
     */
    if (flag & OCCTX_TX_OFFLOAD_MBUF_NOFF_F) {
      cmd_buf[nb_desc] |= (octeontx_prefree_seg(tx_pkt, &m_tofree) << 57);
    }
    ...
    /* Setup PKO_SEND_GATHER_S */
    cmd_buf[nb_desc] |= PKO_SEND_GATHER_SUBDC | PKO_SEND_GATHER_LDTYPE(0x1ull) |
                        PKO_SEND_GATHER_GAUAR((long)gaura_id) | data_len;
...
static __rte_always_inline uint16_t __octeontx_xmit_pkts(
    void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts,
    uint64_t *cmd_buf, const uint16_t flags) {
  ...
  while (count < nb_pkts) {
    ...
    if (flags & OCCTX_TX_MULTI_SEG_F) {
      nb_desc = __octeontx_xmit_mseg_prepare(tx_pkts[count], cmd_buf, flags);
    } else {
      nb_desc = __octeontx_xmit_prepare(tx_pkts[count], cmd_buf, flags);
    }

    octeontx_reg_lmtst(dq->lmtline_va, dq->ioreg_va, cmd_buf, nb_desc);
...
uint16_t octeontx_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                            uint16_t nb_pkts);
...
#define OCCTX_TX_FASTPATH_MODES                                                \
  T(no_offload, 0, 0, 0, 0, 4, OCCTX_TX_OFFLOAD_NONE)                          \
  T(mseg, 0, 0, 0, 1, 14, MULT_F)                                              \
  T(l3l4csum, 0, 0, 1, 0, 4, L3L4CSUM_F)                                       \
  T(l3l4csum_mseg, 0, 0, 1, 1, 14, L3L4CSUM_F | MULT_F)                        \
  T(ol3ol4csum, 0, 1, 0, 0, 4, OL3OL4CSUM_F)                                   \
  T(ol3l4csum_mseg, 0, 1, 0, 1, 14, OL3OL4CSUM_F | MULT_F)                     \
  T(ol3l4csum_l3l4csum, 0, 1, 1, 0, 4, OL3OL4CSUM_F | L3L4CSUM_F)              \
  T(ol3l4csum_l3l4csum_mseg, 0, 1, 1, 1, 14,                                   \
    OL3OL4CSUM_F | L3L4CSUM_F | MULT_F)                                        \
  T(noff, 1, 0, 0, 0, 4, NOFF_F)                                               \
  T(noff_mseg, 1, 0, 0, 1, 14, NOFF_F | MULT_F)                                \
  T(noff_l3l4csum, 1, 0, 1, 0, 4, NOFF_F | L3L4CSUM_F)                         \
  T(noff_l3l4csum_mseg, 1, 0, 1, 1, 14, NOFF_F | L3L4CSUM_F | MULT_F)          \
  T(noff_ol3ol4csum, 1, 1, 0, 0, 4, NOFF_F | OL3OL4CSUM_F)                     \
  T(noff_ol3ol4csum_mseg, 1, 1, 0, 1, 14, NOFF_F | OL3OL4CSUM_F | MULT_F)      \
  T(noff_ol3ol4csum_l3l4csum, 1, 1, 1, 0, 4,                                   \
    NOFF_F | OL3OL4CSUM_F | L3L4CSUM_F)                                        \
  T(noff_ol3ol4csum_l3l4csum_mseg, 1, 1, 1, 1, 14,                             \
    NOFF_F | OL3OL4CSUM_F | L3L4CSUM_F | MULT_F)
...
#define OCCTX_RX_FASTPATH_MODES                                                \
  ...
  R(vlan_csum_mseg, 1, 1, 1, CSUM_F | VLAN_FLTR_F | MULT_RX_F)
