...
uint16_t rte_octeontx_pchan_map[OCTEONTX_MAX_BGX_PORTS]
...
octeontx_parse_vdev_init_params(struct octeontx_vdev_init_params *params,
  ...
  static const char *const octeontx_vdev_valid_params[] = {
      OCTEONTX_VDEV_NR_PORT_ARG, NULL};
  ...
  if (input_args) {
    kvlist = rte_kvargs_parse(input_args, octeontx_vdev_valid_params);
    if (kvlist == NULL)
      return -1;

    ret = rte_kvargs_process(kvlist, OCTEONTX_VDEV_NR_PORT_ARG,
                             &parse_integer_arg, &params->nr_port);
    if (ret < 0)
      goto free_kvlist;
  }
...
static int octeontx_port_open(struct octeontx_nic *nic) {
...
static void octeontx_link_status_print(struct rte_eth_dev *eth_dev,
                                       struct rte_eth_link *link) {
  if (link && link->link_status)
    octeontx_log_info("Port %u: Link Up - speed %u Mbps - %s",
                      (eth_dev->data->port_id), link->link_speed,
                      link->link_duplex == RTE_ETH_LINK_FULL_DUPLEX
                          ? "full-duplex"
                          : "half-duplex");
  else
    octeontx_log_info("Port %d: Link Down", (int)(eth_dev->data->port_id));
}

static void octeontx_link_status_update(struct octeontx_nic *nic,
                                        struct rte_eth_link *link) {
...
static void octeontx_link_status_poll(void *arg) {
  ...
  if (res < 0) {
    octeontx_log_err("Failed to get port %d link status", nic->port_id);
  } else {
...
static void octeontx_port_close(struct octeontx_nic *nic) {
  PMD_INIT_FUNC_TRACE();
...
static int octeontx_port_start(struct octeontx_nic *nic) {
  PMD_INIT_FUNC_TRACE();

  return octeontx_bgx_port_start(nic->port_id);
}

static int octeontx_port_stop(struct octeontx_nic *nic) {
  PMD_INIT_FUNC_TRACE();

  return octeontx_bgx_port_stop(nic->port_id);
}

static int octeontx_port_promisc_set(struct octeontx_nic *nic, int en) {
  struct rte_eth_dev *dev;
  int res;
  ...
  if (res < 0) {
    octeontx_log_err("failed to set promiscuous mode %d", nic->port_id);
    return res;
  }
  ...
  octeontx_log_dbg("port %d : promiscuous mode %s", nic->port_id,
                   en ? "set" : "unset");

  return 0;
}

static int octeontx_port_stats(struct octeontx_nic *nic,
...
static int octeontx_port_stats_clr(struct octeontx_nic *nic) {
  PMD_INIT_FUNC_TRACE();
...
static uint16_t octeontx_tx_offload_flags(struct rte_eth_dev *eth_dev) {
...
static uint16_t octeontx_rx_offload_flags(struct rte_eth_dev *eth_dev) {
  struct octeontx_nic *nic = octeontx_pmd_priv(eth_dev);
  uint16_t flags = 0;
...
static int octeontx_dev_configure(struct rte_eth_dev *dev) {
  ...
  ret = octeontx_pko_channel_open(nic->pko_vfid * PKO_VF_NUM_DQ,
                                  nic->num_tx_queues, nic->base_ochan);
  if (ret) {
    octeontx_log_err("failed to open channel %d no-of-txq %d", nic->base_ochan,
                     nic->num_tx_queues);
    return -EFAULT;
  }
...
static int octeontx_dev_close(struct rte_eth_dev *dev) {
  ...
  if (ret < 0) {
    octeontx_log_err("failed to close channel %d VF%d %d %d", nic->base_ochan,
                     nic->port_id, nic->num_tx_queues, ret);
  }
...
static int octeontx_dev_mtu_set(struct rte_eth_dev *eth_dev, uint16_t mtu) {
...
static int octeontx_recheck_rx_offloads(struct octeontx_rxq *rxq) {
...
static int octeontx_dev_start(struct rte_eth_dev *dev) {
  struct octeontx_nic *nic = octeontx_pmd_priv(dev);
  ...
  if (ret < 0) {
    octeontx_log_err("fail to conf VF%d no. txq %d chan %d ret %d",
                     nic->port_id, nic->num_tx_queues, nic->base_ochan, ret);
    goto error;
  }
...
static int octeontx_dev_stop(struct rte_eth_dev *dev) {
  struct octeontx_nic *nic = octeontx_pmd_priv(dev);
  int ret;
  ...
  if (ret < 0) {
    octeontx_log_err("failed to req stop port %d res=%d", nic->port_id, ret);
    return ret;
  }

  ret = octeontx_pki_port_stop(nic->port_id);
  if (ret < 0) {
    octeontx_log_err("failed to stop pki port %d res=%d", nic->port_id, ret);
    return ret;
  }

  ret = octeontx_pko_channel_stop(nic->base_ochan);
  if (ret < 0) {
    octeontx_log_err("failed to stop channel %d VF%d %d %d", nic->base_ochan,
                     nic->port_id, nic->num_tx_queues, ret);
    return ret;
  }
...
static int octeontx_dev_promisc_enable(struct rte_eth_dev *dev) {
  struct octeontx_nic *nic = octeontx_pmd_priv(dev);
...
static int octeontx_dev_promisc_disable(struct rte_eth_dev *dev) {
  struct octeontx_nic *nic = octeontx_pmd_priv(dev);
...
static int octeontx_port_link_status(struct octeontx_nic *nic) {
  int res;
 ...
 */
static int octeontx_dev_link_update(struct rte_eth_dev *dev,
...
static int octeontx_dev_stats_get(struct rte_eth_dev *dev,
                                  struct rte_eth_stats *stats) {
  struct octeontx_nic *nic = octeontx_pmd_priv(dev);
...
static int octeontx_dev_stats_reset(struct rte_eth_dev *dev) {
  struct octeontx_nic *nic = octeontx_pmd_priv(dev);
...
static void octeontx_dev_mac_addr_del(struct rte_eth_dev *dev, uint32_t index) {
  struct octeontx_nic *nic = octeontx_pmd_priv(dev);
  int ret;
...
static int octeontx_dev_mac_addr_add(struct rte_eth_dev *dev,
  ...
  ret = octeontx_bgx_port_mac_add(nic->port_id, mac_addr->addr_bytes, index);
  if (ret < 0) {
...
static int octeontx_dev_default_mac_addr_set(struct rte_eth_dev *dev,
  ...
  if (ret == 0) {
    ...
    ret = octeontx_bgx_port_mac_add(nic->port_id, addr->addr_bytes, 0);
  }
  if (ret < 0) {
    octeontx_log_err("failed to set MAC address on port %d", nic->port_id);
  }

  return ret;
}

static int octeontx_dev_info(struct rte_eth_dev *dev,
  ...
  dev_info->max_mac_addrs = octeontx_bgx_port_mac_entries_get(nic->port_id);
...
static void octeontx_dq_info_getter(octeontx_dq_t *dq, void *out) {
...
static int octeontx_vf_start_tx_queue(struct rte_eth_dev *dev,
                                      struct octeontx_nic *nic, uint16_t qidx) {
  ...
  res = octeontx_pko_channel_query_dqs(nic->base_ochan, &txq->dq,
                                       sizeof(octeontx_dq_t), txq->queue_id,
...
static inline int octeontx_vf_stop_tx_queue(struct rte_eth_dev *dev,
                                            struct octeontx_nic *nic,
                                            uint16_t qidx) {
  int ret = 0;
...
static void octeontx_dev_tx_queue_release(struct rte_eth_dev *dev,
                                          uint16_t qid) {
  int res;
...
octeontx_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t qidx,
  ...
  if (socket_id != (unsigned int)SOCKET_ID_ANY &&
    ...
    PMD_TX_LOG(INFO, "socket_id expected %d, configured %d", socket_id,
               nic->node);

  /* Free memory prior to re-allocation if needed. */
  if (dev->data->tx_queues[qidx] != NULL) {
    PMD_TX_LOG(DEBUG, "freeing memory prior to re-allocation %d", qidx);
  ...
  PMD_TX_LOG(
      ...
      qidx, txq->queue_id, txq, nb_desc, txq->dq.lmtline_va, txq->dq.ioreg_va,
      txq->dq.fc_status_va);
...
static int octeontx_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t qidx,
                                       uint16_t nb_desc, unsigned int socket_id,
  ...
  if (socket_id != (unsigned int)SOCKET_ID_ANY &&
      socket_id != (unsigned int)nic->node)
    PMD_RX_LOG(INFO, "socket_id expected %d, configured %d", socket_id,
               nic->node);
  ...
  if (!nic->pki.initialized) {
    ...
    pktbuf_conf.mbuff_size =
        (mb_pool->elt_size - RTE_PKTMBUF_HEADROOM -
         rte_pktmbuf_priv_size(mb_pool) - sizeof(struct rte_mbuf));

    pktbuf_conf.cache_mode = PKI_OPC_MODE_STF2_STT;

    ret = octeontx_pki_port_pktbuf_config(port, &pktbuf_conf);
    if (ret != 0) {
      octeontx_log_err("fail to configure pktbuf for port %d", port);
      rte_free(rxq);
      return ret;
    }
    PMD_RX_LOG(DEBUG,
               ...
               "\tlater_skip:\t0x%0x\n"
               "\tcache_mode:\t%s\n",
               port, pktbuf_conf.mbuff_size, pktbuf_conf.wqe_skip,
               pktbuf_conf.first_skip, pktbuf_conf.later_skip,
               (pktbuf_conf.cache_mode == PKI_OPC_MODE_STT)   ? "STT"
               : (pktbuf_conf.cache_mode == PKI_OPC_MODE_STF) ? "STF"
               : (pktbuf_conf.cache_mode == PKI_OPC_MODE_STF1_STT)
                   ? "STF1_STT"
                   : "STF2_STT");
    ...
    if (ret < 0) {
      octeontx_log_err("failed to create QOS port=%d, q=%d", port, qidx);
      rte_free(rxq);
      return ret;
    }
...
static void octeontx_dev_rx_queue_release(struct rte_eth_dev *dev,
                                          uint16_t qid) {
...
octeontx_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {RTE_PTYPE_L3_IPV4, RTE_PTYPE_L3_IPV4_EXT,
                                    RTE_PTYPE_L3_IPV6, RTE_PTYPE_L3_IPV6_EXT,
                                    RTE_PTYPE_L4_TCP,  RTE_PTYPE_L4_UDP,
                                    RTE_PTYPE_L4_FRAG, RTE_PTYPE_UNKNOWN};
...
static int octeontx_pool_ops(struct rte_eth_dev *dev, const char *pool) {
  RTE_SET_USED(dev);
...
static int octeontx_create(struct rte_vdev_device *dev, int port, uint8_t evdev,
                           int socket_id) {
  int res;
  size_t pko_vfid;
  ...
  data->mac_addrs = rte_zmalloc_socket(
      octtx_name, max_entries * RTE_ETHER_ADDR_LEN, 0, socket_id);
  ...
  PMD_INIT_LOG(DEBUG, "port %d, port_ena %d ochan %d num_ochan %d tx_q %d",
               nic->port_id, nic->port_ena, nic->base_ochan, nic->num_ochans,
               nic->num_tx_queues);
...
static int octeontx_remove(struct rte_vdev_device *dev) {
  char octtx_name[OCTEONTX_MAX_NAME_LEN];
  struct rte_eth_dev *eth_dev = NULL;
  struct octeontx_nic *nic = NULL;
  int i;
...
static int octeontx_probe(struct rte_vdev_device *dev) {
  const char *dev_name;
  ...
  if (tx_vfcnt < init_params.nr_port) {
    octeontx_log_err("not enough PKO (%d) for port number (%d)", tx_vfcnt,
                     init_params.nr_port);
    return -EINVAL;
  }
  ...
  PMD_INIT_LOG(DEBUG, "max_queue %d max_port %d", info.max_event_queues,
  ...
  rte_event_dev_attr_get(evdev, RTE_EVENT_DEV_ATTR_QUEUE_COUNT,
                         (uint32_t *)&qnum);
  if (pnum < qnum) {
    octeontx_log_err("too few event ports (%d) for event_q(%d)", pnum, qnum);
    res = -EINVAL;
    goto parse_error;
  }
  ...
  for (i = 0; i < qnum; i++) {
    ...
    if (res < 0) {
      octeontx_log_err("failed to setup event_q(%d): res %d", i, res);
      goto parse_error;
    }
  }
  ...
  for (i = 0; i < pnum; i++) {
    ...
    if (res < 0) {
      res = -ENODEV;
      octeontx_log_err("failed to setup ev port(%d) res=%d", i, res);
      goto parse_error;
    }
  }
  ...
  for (i = 0; i < qnum; i++) {
    ...
    if (res < 0) {
      res = -ENODEV;
      octeontx_log_err("failed to link port (%d): res=%d", i, res);
      goto parse_error;
    }
  }
  ...
  for (i = 0; i < init_params.nr_port; i++) {
    ...
    if (port_id < 0) {
      octeontx_log_err("failed to create device %s", dev_name);
      res = -ENODEV;
      goto parse_error;
    }

    PMD_INIT_LOG(INFO, "created ethdev %s for port %d", dev_name, port_id);
  }
