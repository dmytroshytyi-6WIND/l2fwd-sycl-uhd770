...
int pfe_hif_init_buffers(struct pfe_hif *hif) {
  ...
  for (i = 0; i < hif->rx_ring_size; i++) {
     ...
     */
    hif->rx_buf_vaddr[i] =
        (void *)((size_t)mbuf->buf_addr + mbuf->data_off - PFE_PKT_HEADER_SZ);
    hif->rx_buf_addr[i] =
    ...
    writel(DDR_PHYS_TO_PFE(hif->rx_buf_addr[i]), &desc->data);
    writel(0, &desc->status);
    ...
    writel((BD_CTRL_PKT_INT_EN | BD_CTRL_LIFM | BD_CTRL_DIR | BD_CTRL_DESC_EN |
            BD_BUF_LEN(hif->rx_buf_len[i])),
           &desc->ctrl);
  ...
  first_desc_p = (struct hif_desc *)hif->descr_baseaddr_p + hif->rx_ring_size;
...
client_put_rxpacket(struct hif_rx_queue *queue, void *pkt, u32 len, u32 flags,
                    u32 client_ctrl, struct rte_mempool *pool, u32 *rem_len) {
...
int pfe_hif_rx_process(struct pfe *pfe, int budget) {
  ...
  while (rx_processed < budget) {
    ...
    if (!hif->started) {
      ...
      hif->client_ctrl =
          (hif_hdr.hdr.client_ctrl1 << 16) | hif_hdr.hdr.client_ctrl;
      flags = CL_DESC_FIRST;
    ...
    if (hif->client_id >= HIF_CLIENTS_MAX ||
        !(test_bit(hif->client_id, &hif->shm->g_client_status[0]))) {
      PFE_PMD_INFO("packet with invalid client id %d qnum %d", hif->client_id,
                   hif->qno);
    ...
    if (hif->client[hif->client_id].rx_qn <= hif->qno) {
      PFE_DP_LOG(DEBUG, "packet with invalid queue: %d", hif->qno);
      hif->qno = 0;
    }

  retry:
    mbuf = client_put_rxpacket(&hif->client[hif->client_id].rx_q[hif->qno],
                               (void *)pkt_hdr, len, flags, hif->client_ctrl,
                               hif->shm->pool, &buf_size);
...
static int client_ack_txpacket(struct pfe_hif *hif, unsigned int client_id,
  ...
  if (readl(&desc->ctrl) & CL_DESC_OWN) {
    ...
    PFE_PMD_ERR("%d %d %d %d %d %p %d", hif->txtosend, hif->txtoclean,
                hif->txavail, client_id, q_no, queue, queue->ack_idx);
    return 1;
  }
}
 ...
 */
void hif_xmit_pkt(struct pfe_hif *hif, unsigned int client_id,
                  unsigned int q_no, void *data, u32 len, unsigned int flags) {
  struct hif_desc *desc;
  struct hif_desc_sw *desc_sw;
  ...
  if ((!((flags & HIF_DATA_VALID) && (flags & HIF_LAST_BUFFER))))
    goto skip_tx;
  ...
  do {
    ...
    if (desc_sw->flags & HIF_LAST_BUFFER) {
      writel((BD_CTRL_LIFM | BD_CTRL_BRFETCH_DISABLE | BD_CTRL_RTFETCH_DISABLE |
              BD_CTRL_PARSE_DISABLE | BD_CTRL_DESC_EN |
              BD_BUF_LEN(desc_sw->len)),
             &desc->ctrl);
    } else {
      writel((BD_CTRL_DESC_EN | BD_BUF_LEN(desc_sw->len)), &desc->ctrl);
    }
...
void hif_process_client_req(struct pfe_hif *hif, int req, int data1,
                            __rte_unused int data2) {
  ...
  case REQUEST_CL_REGISTER:
    ...
    pfe_hif_client_register(
        hif, client_id, (struct hif_client_shm *)&hif->shm->client[client_id]);
    break;
...
send_dummy_pkt_to_hif(void) {
  ...
  static u32 dummy_pkt[] = {0x33221100, 0x2b785544, 0xd73093cb, 0x01000608,
  ...
  local_hdr.status =
      htonl((STATUS_IP_CHECKSUM_CORRECT | STATUS_UDP_CHECKSUM_CORRECT |
             STATUS_TCP_CHECKSUM_CORRECT | STATUS_UNICAST_HASH_MATCH |
             STATUS_CUMULATIVE_ARC_HIT));
  copy_to_lmem((u32 *)lmem_virt_addr, (u32 *)&local_hdr, sizeof(local_hdr));

  copy_to_lmem((u32 *)(lmem_virt_addr + LMEM_HDR_SIZE), (u32 *)dummy_pkt, 0x40);
