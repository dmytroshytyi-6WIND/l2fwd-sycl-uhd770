...
#define IS_PE_LMEM(addr, len)                                                  \
  ({                                                                           \
    typeof(addr) addr_ = (addr);                                               \
    ((unsigned long)(addr_) >= PE_LMEM_BASE_ADDR) &&                           \
        (((unsigned long)(addr_) + (len)) <= PE_LMEM_END);                     \
  })

#define IS_PFE_LMEM(addr, len)                                                 \
  ({                                                                           \
    typeof(addr) addr_ = (addr);                                               \
    ((unsigned long)(addr_) >= CBUS_VIRT_TO_PFE(LMEM_BASE_ADDR)) &&            \
        (((unsigned long)(addr_) + (len)) <= CBUS_VIRT_TO_PFE(LMEM_END));      \
  })

#define __IS_PHYS_DDR(addr, len)                                               \
  ({                                                                           \
    typeof(addr) addr_ = (addr);                                               \
    ((unsigned long)(addr_) >= DDR_PHYS_BASE_ADDR) &&                          \
        (((unsigned long)(addr_) + (len)) <= DDR_PHYS_END);                    \
  })
...
#define LS1012A_PFE_RESET_WA /*                                                \
                              ...
                              */
#define PFE_CBUS_PHYS_BASE_ADDR                                                \
  0xc0000000 /* CBUS physical base address                                     \
              * as seen by PE's.                                               \
              */
...
#define CBUS_PHYS_TO_PFE(p)                                                    \
  (((p)-PFE_CBUS_PHYS_BASE_ADDR) + PFE_CBUS_PHYS_BASE_ADDR_FROM_PFE)
...
#define CBUS_PFE_TO_VIRT(p)                                                    \
  (((unsigned long)(p)-PFE_CBUS_PHYS_BASE_ADDR) + CBUS_BASE_ADDR)

/* The below part of the code is used in QOS control driver from host */
#define TMU_APB_BASE_ADDR                                                      \
  0xc1000000 /* TMU base address seen by                                       \
              * pe's                                                           \
              */
...
#define CLASS_MASK                                                             \
  (BIT(CLASS0_ID) | BIT(CLASS1_ID) | BIT(CLASS2_ID) | BIT(CLASS3_ID) |         \
