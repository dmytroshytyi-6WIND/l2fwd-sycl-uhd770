...
void gemac_set_duplex(void *base, int duplex) {
  if (duplex == DUPLEX_HALF) {
    writel(readl(base + EMAC_TCNTRL_REG) & ~EMAC_TCNTRL_FDEN,
           base + EMAC_TCNTRL_REG);
    writel(readl(base + EMAC_RCNTRL_REG) | EMAC_RCNTRL_DRT,
...
void gemac_enable(void *base) {
  writel(readl(base + EMAC_ECNTRL_REG) | EMAC_ECNTRL_ETHER_EN,
         base + EMAC_ECNTRL_REG);
}
...
void gemac_tx_disable(void *base) {
  writel(readl(base + EMAC_TCNTRL_REG) | EMAC_TCNTRL_GTS,
         base + EMAC_TCNTRL_REG);
}
...
void gemac_set_laddrN(void *base, struct pfe_mac_addr *address,
  ...
  if (entry_index < 1) {
    ...
    writel(htonl(address->bottom),
           base + ((entry_index - 1) * 8) + EMAC_SMAC_0_0);
    writel((htonl(address->top) | 0x8808),
           base + ((entry_index - 1) * 8) + EMAC_SMAC_0_1);
  }
}
...
void gemac_set_loop(void *base, __rte_unused enum mac_loop gem_loop) {
  pr_info("%s()\n", __func__);
  writel(readl(base + EMAC_RCNTRL_REG) | EMAC_RCNTRL_LOOP,
         (base + EMAC_RCNTRL_REG));
}
...
void gemac_enable_copy_all(void *base) {
  writel(readl(base + EMAC_RCNTRL_REG) | EMAC_RCNTRL_PROM,
         (base + EMAC_RCNTRL_REG));
}
...
void gemac_allow_broadcast(void *base) {
  writel(readl(base + EMAC_RCNTRL_REG) & ~EMAC_RCNTRL_BC_REJ,
         base + EMAC_RCNTRL_REG);
}
 ...
 * @param[in] base	GPI base address
 */
void gpi_reset(void *base) { writel(CORE_SW_RESET, base + GPI_CTRL); }

/* Enables a GPI block.
 * @param[in] base	GPI base address
 */
void gpi_enable(void *base) { writel(CORE_ENABLE, base + GPI_CTRL); }

/* Disables a GPI block.
 * @param[in] base	GPI base address
 */
void gpi_disable(void *base) { writel(CORE_DISABLE, base + GPI_CTRL); }
