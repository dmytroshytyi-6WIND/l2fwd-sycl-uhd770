...
static uint64_t dev_rx_offloads_sup = RTE_ETH_RX_OFFLOAD_IPV4_CKSUM |
                                      RTE_ETH_RX_OFFLOAD_UDP_CKSUM |
                                      RTE_ETH_RX_OFFLOAD_TCP_CKSUM;

/* Supported Tx offloads */
static uint64_t dev_tx_offloads_sup = RTE_ETH_TX_OFFLOAD_IPV4_CKSUM |
...
static void pfe_soc_version_get(void) {
  FILE *svr_file = NULL;
...
static void pfe_eth_flush_txQ(struct pfe_eth_priv_s *priv, int tx_q_num,
                              int __rte_unused from_tx,
                              __rte_unused int n_desc) {
  struct rte_mbuf *mbuf;
  unsigned int flags;

  /* Clean HIF and client queue */
  while ((mbuf = hif_lib_tx_get_next_complete(&priv->client, tx_q_num, &flags,
                                              HIF_TX_DESC_NT))) {
    if (mbuf) {
      mbuf->next = NULL;
      mbuf->nb_segs = 1;
...
static uint16_t pfe_recv_pkts_on_intr(void *rxq, struct rte_mbuf **rx_pkts,
                                      uint16_t nb_pkts) {
...
#define RESET_STATUS (HIF_INT | HIF_RXPKT_INT)
  ...
  work_done =
      hif_lib_receive_pkt(rxq, priv->pfe->hif.shm->pool, rx_pkts, nb_pkts);
...
static uint16_t pfe_recv_pkts(void *rxq, struct rte_mbuf **rx_pkts,
                              uint16_t nb_pkts) {
  struct hif_client_rx_queue *queue = rxq;
  struct pfe_eth_priv_s *priv = queue->priv;
...
static uint16_t pfe_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                              uint16_t nb_pkts) {
  ...
  for (i = 0; i < nb_pkts; i++) {
    if (tx_pkts[i]->nb_segs > 1) {
      ...
      hif_lib_xmit_pkt(&priv->client, queue->queue_id,
                       ...
                       tx_pkts[i]->data_len, 0x0, HIF_FIRST_BUFFER, tx_pkts[i]);

      mbuf = tx_pkts[i]->next;
      for (j = 0; j < (tx_pkts[i]->nb_segs - 2); j++) {
        hif_lib_xmit_pkt(&priv->client, queue->queue_id,
                         (void *)(size_t)rte_pktmbuf_iova(mbuf),
                         mbuf->buf_addr + mbuf->data_off, mbuf->data_len, 0x0,
                         0x0, mbuf);
        mbuf = mbuf->next;
      }
      ...
      hif_lib_xmit_pkt(&priv->client, queue->queue_id,
                       (void *)(size_t)rte_pktmbuf_iova(mbuf),
                       mbuf->buf_addr + mbuf->data_off, mbuf->data_len, 0x0,
                       HIF_LAST_BUFFER | HIF_DATA_VALID, mbuf);
    } else {
      hif_lib_xmit_pkt(&priv->client, queue->queue_id,
                       (void *)(size_t)rte_pktmbuf_iova(tx_pkts[i]),
                       tx_pkts[i]->buf_addr + tx_pkts[i]->data_off,
...
static int pfe_eth_open_cdev(struct pfe_eth_priv_s *priv) {
  int pfe_cdev_fd;
...
static void pfe_eth_close_cdev(struct pfe_eth_priv_s *priv) {
  if (priv == NULL)
    return;
...
static int pfe_eth_stop(struct rte_eth_dev *dev /*, int wake*/) {
  struct pfe_eth_priv_s *priv = dev->data->dev_private;
...
static int pfe_eth_close(struct rte_eth_dev *dev) {
  int ret;
  PMD_INIT_FUNC_TRACE();
...
static int pfe_eth_configure(struct rte_eth_dev *dev __rte_unused) { return 0; }

static int pfe_eth_info(struct rte_eth_dev *dev,
...
static int
pfe_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
...
static const uint32_t *pfe_supported_ptypes_get(struct rte_eth_dev *dev) {
...
static inline int pfe_eth_atomic_read_link_status(struct rte_eth_dev *dev,
...
static inline int pfe_eth_atomic_write_link_status(struct rte_eth_dev *dev,
                                                   struct rte_eth_link *link) {
...
static int pfe_promiscuous_enable(struct rte_eth_dev *dev) {
  struct pfe_eth_priv_s *priv = dev->data->dev_private;
...
static int pfe_promiscuous_disable(struct rte_eth_dev *dev) {
  struct pfe_eth_priv_s *priv = dev->data->dev_private;
...
static int pfe_allmulticast_enable(struct rte_eth_dev *dev) {
  struct pfe_eth_priv_s *priv = dev->data->dev_private;
...
static int pfe_link_down(struct rte_eth_dev *dev) { return pfe_eth_stop(dev); }
...
static int pfe_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct pfe_eth_priv_s *priv = dev->data->dev_private;
...
static int pfe_dev_set_mac_addr(struct rte_eth_dev *dev,
  ...
  gemac_set_laddrN(priv->EMAC_baseaddr, (struct pfe_mac_addr *)&spec_addr, 1);
  rte_ether_addr_copy(addr, &dev->data->mac_addrs[0]);
  return 0;
}

static int pfe_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  struct pfe_eth_priv_s *priv = dev->data->dev_private;
...
#define HIF_GEMAC_TMUQ_BASE 6
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("mac_addr", ETHER_ADDR_LEN * PFE_MAX_MACS, 0);
  ...
  memcpy(addr.addr_bytes, priv->einfo->mac_addr, ETH_ALEN);
...
pfe_get_gemac_if_proprties(struct pfe *pfe,
  ...
  if (mac_addr) {
    memcpy(pdata->ls1012a_eth_pdata[port].mac_addr, mac_addr, ETH_ALEN);
  }
...
static int pfe_parse_vdev_init_params(struct pfe_vdev_init_params *params,
                                      struct rte_vdev_device *dev) {
  struct rte_kvargs *kvlist = NULL;
  int ret = 0;

  static const char *const pfe_vdev_valid_params[] = {PFE_VDEV_GEM_ID_ARG,
                                                      NULL};
  ...
  ret = rte_kvargs_process(kvlist, PFE_VDEV_GEM_ID_ARG, &parse_integer_arg,
                           &params->gem_id);
  rte_kvargs_free(kvlist);
  return ret;
}

static int pmd_pfe_probe(struct rte_vdev_device *vdev) {
  const u32 *prop;
  ...
  size_t size = 0;
  struct pfe_vdev_init_params init_params = {.gem_id = -1};
  ...
  PFE_PMD_LOG(INFO, "Initializing pmd_pfe for %s Given gem-id %d", name,
              init_params.gem_id);

  if (g_pfe) {
    if (g_pfe->nb_devs >= g_pfe->max_intf) {
      PFE_PMD_ERR("PFE %d dev already created Max is %d", g_pfe->nb_devs,
                  g_pfe->max_intf);
      return -EINVAL;
    }
    goto eth_init;
  ...
  g_pfe->cbus_baseaddr =
      mmap(NULL, cbus_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, cbus_addr);
  close(fd);
...
eth_init:
  ...
  PFE_PMD_LOG(INFO, "Init pmd_pfe for %s gem-id %d(given =%d)", name, gem_id,
              init_params.gem_id);
...
static int pmd_pfe_remove(struct rte_vdev_device *vdev) {
  const char *name;
  struct rte_eth_dev *eth_dev = NULL;
  int ret = 0;
