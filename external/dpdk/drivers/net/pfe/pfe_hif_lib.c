 ...
 */
static void hif_lib_indicate_hif(struct pfe_hif *hif, int req, int data1,
                                 int data2) {
  ...
  for (qno = 0; qno < client->rx_qn; qno++) {
    ...
    for (ii = 0; ii < client->rx_q[qno].size; ii++) {
      ...
      if (buf) {
         ...
         */
        buf = buf + PFE_PKT_HEADER_SZ - sizeof(struct rte_mbuf) -
              RTE_PKTMBUF_HEADROOM - mb_priv->mbuf_priv_size;
...
static int hif_lib_client_init_rx_buffers(struct hif_client_s *client,
  ...
  client->rx_qbase =
      rte_malloc(NULL, client->rx_qn * q_size * sizeof(struct rx_queue_desc),
                 RTE_CACHE_LINE_SIZE);
  if (!client->rx_qbase)
    goto err;
  ...
  for (qno = 0; qno < client->rx_qn; qno++) {
    ...
    queue->base =
        client->rx_qbase + qno * q_size * sizeof(struct rx_queue_desc);
    queue->size = q_size;
    queue->read_idx = 0;
    queue->write_idx = 0;
    queue->queue_id = 0;
...
static int hif_lib_client_init_tx_buffers(struct hif_client_s *client,
                                          int q_size) {
  struct hif_client_tx_queue *queue;
  unsigned int qno;

  client->tx_qbase =
      rte_malloc(NULL, client->tx_qn * q_size * sizeof(struct tx_queue_desc),
                 RTE_CACHE_LINE_SIZE);
  if (!client->tx_qbase)
    return 1;
  ...
  for (qno = 0; qno < client->tx_qn; qno++) {
    ...
    queue->base =
        client->tx_qbase + qno * q_size * sizeof(struct tx_queue_desc);
    queue->size = q_size;
    queue->read_idx = 0;
    queue->write_idx = 0;
    queue->tx_pending = 0;
    ...
    PFE_PMD_DEBUG("tx queue: %d, base: %p, size: %d", qno, queue->base,
                  queue->size);
  }
...
static int hif_lib_event_dummy(__rte_unused void *priv,
                               __rte_unused int event_type,
                               __rte_unused int qno) {
  return 0;
}
...
int hif_lib_client_register(struct hif_client_s *client) {
  ...
  PFE_PMD_DEBUG("client: %p, client_id: %d, tx_qsize: %d, rx_qsize: %d", client,
                client->id, client->tx_qsize, client->rx_qsize);
...
int hif_lib_event_handler_start(struct hif_client_s *client, int event,
  ...
  if (event >= HIF_EVENT_MAX || qno >= HIF_CLIENT_QUEUES_MAX) {
    PFE_PMD_WARN("Unsupported event : %d  queue number : %d", event, qno);
    return -1;
  }
  ...
  case EVENT_RX_PKT_IND:
    if (!(desc->ctrl & CL_DESC_OWN))
      hif_lib_indicate_client(client, EVENT_RX_PKT_IND, qno);
    break;
...
static inline void pfe_sw_parse_pkt(struct rte_mbuf *mbuf) {
...
int hif_lib_receive_pkt(struct hif_client_rx_queue *queue,
  ...
  for (i = 0; i < nb_pkts;) {
    do {
      ...
      mbuf = desc->data + PFE_PKT_HEADER_SZ - sizeof(struct rte_mbuf) -
             RTE_PKTMBUF_HEADROOM - mb_priv->mbuf_priv_size;
      mbuf->next = NULL;
      if (desc->ctrl & CL_DESC_FIRST) {
         ...
         */
        u16 size = 0;
        mbuf->pkt_len = CL_DESC_BUF_LEN(desc->ctrl) - PFE_PKT_HEADER_SZ - size;
        mbuf->data_len = mbuf->pkt_len;
        mbuf->port = queue->port_id;
#ifdef RTE_LIBRTE_PFE_SW_PARSE
        pfe_sw_parse_pkt(mbuf);
#else
        parse_res = (struct pfe_parse *)(desc->data + PFE_HIF_SIZE);
...
static inline void hif_hdr_write(struct hif_hdr *pkt_hdr,
                                 unsigned int client_id, unsigned int qno,
                                 u32 client_ctrl) {
...
void hif_lib_xmit_pkt(struct hif_client_s *client, unsigned int qno, void *data,
                      void *data1, unsigned int len, u32 client_ctrl,
...
void *hif_lib_tx_get_next_complete(struct hif_client_s *client, int qno,
                                   unsigned int *flags,
                                   __rte_unused int count) {
  struct hif_client_tx_queue *queue = &client->tx_q[qno];
  struct tx_queue_desc *desc = queue->base + queue->read_idx;

  PFE_DP_LOG(DEBUG, "qno : %d rd_indx: %d pending:%d", qno, queue->read_idx,
             queue->tx_pending);
  ...
  if (queue->nocpy_flag && !queue->done_tmu_tx_pkts) {
    ...
    if (queue->prev_tmu_tx_pkts > tmu_tx_pkts)
      queue->done_tmu_tx_pkts =
          UINT_MAX - queue->prev_tmu_tx_pkts + tmu_tx_pkts;
    else
...
void pfe_hif_lib_exit(__rte_unused struct pfe *pfe) { PMD_INIT_FUNC_TRACE(); }
