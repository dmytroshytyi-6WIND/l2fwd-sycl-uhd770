...
bnxt_consume_rx_buf(struct bnxt_rx_ring_info *rxr, uint16_t cons) {
...
static void bnxt_tpa_get_metadata(struct bnxt *bp,
  ...
  if (bp->vnic_cap_flags & BNXT_VNIC_CAP_RX_CMPL_V2) {
    ...
    struct rx_tpa_start_v2_cmpl_hi *v2_tpa_start1 = (void *)tpa_start1;

    if (v2_tpa_start->agg_id & RX_TPA_START_V2_CMPL_METADATA1_VALID) {
      tpa_info->vlan_valid = 1;
      tpa_info->vlan = rte_le_to_cpu_16(v2_tpa_start1->metadata0);
    }
...
static void bnxt_tpa_start(struct bnxt_rx_queue *rxq,
  ...
  rxr->rx_next_cons = RING_IDX(rxr->rx_ring_struct, RING_NEXT(data_cons));
}

static int bnxt_agg_bufs_valid(struct bnxt_cp_ring_info *cpr, uint8_t agg_bufs,
...
static int bnxt_rx_pages(struct bnxt_rx_queue *rxq, struct rte_mbuf *mbuf,
...
static inline struct rte_mbuf *
bnxt_tpa_end(struct bnxt_rx_queue *rxq, uint32_t *raw_cp_cons,
...
static void __rte_cold bnxt_init_ptype_table(void) {
...
static uint32_t bnxt_parse_pkt_type(struct rx_pkt_cmpl *rxcmp,
  ...
  index = BNXT_CMPL_ITYPE_TO_IDX(flags_type) |
          BNXT_CMPL_VLAN_TUN_TO_IDX(flags2) | BNXT_CMPL_IP_VER_TO_IDX(flags2);

  return bnxt_ptype_table[index];
}

static void __rte_cold bnxt_init_ol_flags_tables(struct bnxt_rx_queue *rxq) {
  struct bnxt_rx_ring_info *rxr = rxq->rx_ring;
...
static void bnxt_set_ol_flags(struct bnxt_rx_ring_info *rxr,
                              struct rx_pkt_cmpl *rxcmp,
                              struct rx_pkt_cmpl_hi *rxcmp1,
                              struct rte_mbuf *mbuf) {
  ...
  flags = rte_le_to_cpu_32(rxcmp1->flags2) &
          (RX_PKT_CMPL_FLAGS2_IP_CS_CALC | RX_PKT_CMPL_FLAGS2_L4_CS_CALC |
           RX_PKT_CMPL_FLAGS2_T_IP_CS_CALC | RX_PKT_CMPL_FLAGS2_T_L4_CS_CALC |
           RX_PKT_CMPL_FLAGS2_META_FORMAT_VLAN);

  flags |= (flags & RX_PKT_CMPL_FLAGS2_T_IP_CS_CALC) << 3;
  errors =
      rte_le_to_cpu_16(rxcmp1->errors_v2) &
      (RX_PKT_CMPL_ERRORS_IP_CS_ERROR | RX_PKT_CMPL_ERRORS_L4_CS_ERROR |
       RX_PKT_CMPL_ERRORS_T_IP_CS_ERROR | RX_PKT_CMPL_ERRORS_T_L4_CS_ERROR);
...
#ifdef RTE_LIBRTE_IEEE1588
static void bnxt_get_rx_ts_p5(struct bnxt *bp, uint32_t rx_ts_cmpl) {
...
static uint32_t bnxt_ulp_set_mark_in_mbuf(struct bnxt *bp,
                                          struct rx_pkt_cmpl_hi *rxcmp1,
                                          struct rte_mbuf *mbuf,
                                          uint32_t *vfr_flag) {
  uint32_t cfa_code;
   ...
   */
  meta_fmt = (flags2 & BNXT_CFA_META_FMT_MASK) >> BNXT_CFA_META_FMT_SHFT;
  ...
  rc = ulp_mark_db_mark_get(bp->ulp_ctx, gfid, cfa_code, vfr_flag, &mark_id);
  if (!rc) {
...
void bnxt_set_mark_in_mbuf(struct bnxt *bp, struct rx_pkt_cmpl_hi *rxcmp1,
                           struct rte_mbuf *mbuf) {
  uint32_t cfa_code = 0;
...
static int bnxt_rx_pkt(struct rte_mbuf **rx_pkt, struct bnxt_rx_queue *rxq,
                       uint32_t *raw_cons) {
  struct bnxt_cp_ring_info *cpr = rxq->cp_ring;
  struct bnxt_rx_ring_info *rxr = rxq->rx_ring;
  struct rx_pkt_cmpl *rxcmp;
  ...
  if (!bnxt_cpr_cmp_valid(rxcmp1, tmp_raw_cons, cpr->cp_ring_struct->ring_size))
    return -EBUSY;
  ...
  if (cmp_type == RX_TPA_START_CMPL_TYPE_RX_TPA_START ||
    ...
    mbuf = bnxt_tpa_end(rxq, &tmp_raw_cons, (struct rx_tpa_end_cmpl *)rxcmp,
...
#ifdef RTE_LIBRTE_IEEE1588
  if (unlikely((rte_le_to_cpu_16(rxcmp->flags_type) & RX_PKT_CMPL_FLAGS_MASK) ==
               RX_PKT_CMPL_FLAGS_ITYPE_PTP_W_TIMESTAMP))
  ...
  if (BNXT_TRUFLOW_EN(bp))
    mark_id = bnxt_ulp_set_mark_in_mbuf(rxq->bp, rxcmp1, mbuf, &vfr_flag);
  else
...
uint16_t bnxt_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (1) {
    ...
    if (!bnxt_cpr_cmp_valid(rxcmp, raw_cons, cpr->cp_ring_struct->ring_size))
      break;
    if (CMP_TYPE(rxcmp) == CMPL_BASE_TYPE_HWRM_DONE) {
      ...
      evt = bnxt_event_hwrm_resp_handler(rxq->bp, (struct cmpl_base *)rxcmp);
...
int bnxt_init_rx_ring_struct(struct bnxt_rx_queue *rxq,
  ...
  if (rxq->rx_ring != NULL) {
    ...
    rxr = rte_zmalloc_socket("bnxt_rx_ring", sizeof(struct bnxt_rx_ring_info),
  ...
  if (rxr->rx_ring_struct == NULL) {
    ring = rte_zmalloc_socket("bnxt_rx_ring_struct", sizeof(struct bnxt_ring),
                              RTE_CACHE_LINE_SIZE, socket_id);
    if (ring == NULL)
      return -ENOMEM;
  ...
  ring->ring_size = rte_align32pow2(rxq->nb_rx_desc * AGG_RING_SIZE_FACTOR);
  ring->ring_mask = ring->ring_size - 1;
...
int bnxt_init_one_rx_ring(struct bnxt_rx_queue *rxq) {
  ...
  for (i = 0; i < ring->ring_size; i++) {
    if (unlikely(!rxr->rx_buf_ring[i])) {
      if (bnxt_alloc_rx_data(rxq, rxr, raw_prod) != 0) {
        PMD_DRV_LOG(WARNING, "RxQ %d allocated %d of %d mbufs\n", rxq->queue_id,
                    i, ring->ring_size);
        return -ENOMEM;
      }
    }
  ...
  for (i = ring->ring_size; i < ring->ring_size + BNXT_RX_EXTRA_MBUF_ENTRIES;
       i++) {
  ...
  if (rxr->tpa_info) {
    ...
    for (i = 0; i < max_aggs; i++) {
      if (unlikely(!rxr->tpa_info[i].mbuf)) {
        rxr->tpa_info[i].mbuf = __bnxt_alloc_rx_data(rxq->mb_pool);
