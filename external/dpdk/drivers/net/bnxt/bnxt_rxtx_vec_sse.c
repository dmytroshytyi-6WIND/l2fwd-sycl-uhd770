...
static inline void descs_to_mbufs(__m128i mm_rxcmp[4], __m128i mm_rxcmp1[4],
  ...
  const __m128i flags_type_mask = _mm_set1_epi32(RX_PKT_CMPL_FLAGS_ITYPE_MASK);
  const __m128i flags2_mask1 = _mm_set1_epi32(CMPL_FLAGS2_VLAN_TUN_MSK);
  const __m128i flags2_mask2 = _mm_set1_epi32(RX_PKT_CMPL_FLAGS2_IP_TYPE);
  const __m128i rss_mask = _mm_set1_epi32(RX_PKT_CMPL_FLAGS_RSS_VALID);
  ...
  ptype_idx =
      _mm_srli_epi32(_mm_and_si128(flags_type, flags_type_mask),
...
static uint16_t recv_burst_vec_sse(void *rx_queue, struct rte_mbuf **rx_pkts,
                                   uint16_t nb_pkts) {
  ...
  int nb_rx_pkts = 0;
  const __m128i valid_target = _mm_set1_epi32(!!(raw_cons & cp_ring_size));
  int i;
  ...
  for (i = 0; i < nb_pkts; i += BNXT_RX_DESCS_PER_LOOP_VEC128,
    ...
    info3_v = _mm_and_si128(_mm_unpacklo_epi64(tmp0, tmp1), info3_v_mask);
    ...
    descs_to_mbufs(rxcmp, rxcmp1, mbuf_init, &rx_pkts[nb_rx_pkts], rxr);
...
uint16_t bnxt_recv_pkts_vec(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_pkts > RTE_BNXT_MAX_RX_BURST) {
    ...
    burst = recv_burst_vec_sse(rx_queue, rx_pkts + cnt, RTE_BNXT_MAX_RX_BURST);
...
static inline void bnxt_xmit_one(struct rte_mbuf *mbuf, struct tx_bd_long *txbd,
  ...
  *tx_buf = mbuf;
  ...
  desc =
      _mm_set_epi64x(mbuf->buf_iova + mbuf->data_off,
                     bnxt_xmit_flags_len(mbuf->data_len, TX_BD_FLAGS_NOCMPL));
  desc = _mm_blend_epi16(
      desc, _mm_set_epi16(0, 0, 0, 0, 0, 0, mbuf->data_len, 0), 0x02);
