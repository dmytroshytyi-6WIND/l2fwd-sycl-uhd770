...
int bnxt_alloc_ring_grps(struct bnxt *bp) {
  ...
  if (BNXT_HAS_RING_GRPS(bp)) {
    ...
    if (!bp->grp_info) {
      PMD_DRV_LOG(ERR, "Failed to alloc grp info tbl.\n");
      return -ENOMEM;
    }
...
int bnxt_alloc_rings(struct bnxt *bp, unsigned int socket_id, uint16_t qidx,
                     struct bnxt_tx_queue *txq, struct bnxt_rx_queue *rxq,
  ...
  int nq_vmem_len =
      nq_ring_info ? RTE_CACHE_LINE_ROUNDUP(cp_ring->vmem_size) : 0;
  ...
  if (bnxt_need_agg_ring(bp->eth_dev))
    ag_vmem_len =
        rx_ring_info && rx_ring_info->ag_ring_struct
  ...
  int cp_ring_len =
      RTE_CACHE_LINE_ROUNDUP(cp_ring->ring_size * sizeof(struct cmpl_base));
  ...
  snprintf(mz_name, RTE_MEMZONE_NAMESIZE, "bnxt_" PCI_PRI_FMT "-%04x_%s",
           pdev->addr.domain, pdev->addr.bus, pdev->addr.devid,
           pdev->addr.function, qidx, suffix);
  mz_name[RTE_MEMZONE_NAMESIZE - 1] = 0;
  mz = rte_memzone_lookup(mz_name);
  if (!mz) {
    mz = rte_memzone_reserve_aligned(
        mz_name, total_alloc_len, socket_id,
  ...
  if (tx_ring_info) {
    ...
    if (tx_ring->vmem_size) {
      tx_ring->vmem = (void **)((char *)mz->addr + tx_vmem_start);
      tx_ring_info->tx_buf_ring = (struct rte_mbuf **)tx_ring->vmem;
    }
  }
  ...
  if (rx_ring_info) {
    ...
    rx_ring_info->rx_desc_ring = (struct rx_prod_pkt_bd *)rx_ring->bd;
    ...
    if (rx_ring->vmem_size) {
      rx_ring->vmem = (void **)((char *)mz->addr + rx_vmem_start);
      rx_ring_info->rx_buf_ring = (struct rte_mbuf **)rx_ring->vmem;
    }
    ...
    if (bnxt_need_agg_ring(bp->eth_dev)) {
      ...
      rx_ring_info->ag_desc_ring = (struct rx_prod_pkt_bd *)rx_ring->bd;
      ...
      if (!rx_ring->bd)
        return -ENOMEM;
      if (rx_ring->vmem_size) {
        rx_ring->vmem = (void **)((char *)mz->addr + ag_vmem_start);
        rx_ring_info->ag_buf_ring = (struct rte_mbuf **)rx_ring->vmem;
      }

      rx_ring_info->ag_bitmap = rte_bitmap_init(
          rx_ring_info->rx_ring_struct->ring_size * AGG_RING_SIZE_FACTOR,
          (uint8_t *)mz->addr + ag_bitmap_start, ag_bitmap_len);

      /* TPA info */
      if (rx_offloads & RTE_ETH_RX_OFFLOAD_TCP_LRO)
        rx_ring_info->tpa_info =
            ((struct bnxt_tpa_info *)((char *)mz->addr + tpa_info_start));
    }
  }
...
static void bnxt_set_db(struct bnxt *bp, struct bnxt_db_info *db,
                        uint32_t ring_type, uint32_t map_idx, uint32_t fid,
                        uint32_t ring_mask) {
  if (BNXT_CHIP_P5(bp)) {
  ...
  if (BNXT_CHIP_SR2(bp)) {
    ...
    db->db_epoch_shift = DBR_EPOCH_SFT - rte_log2_u32(db->db_epoch_mask);
  }
}
...
static int bnxt_alloc_cmpl_ring(struct bnxt *bp, int queue_index,
  ...
  cpr->cp_raw_cons = 0;
  bnxt_set_db(bp, &cpr->cp_db, ring_type, cp_ring_index, cp_ring->fw_ring_id,
              cp_ring->ring_mask);
  bnxt_db_cq(cpr);
...
int bnxt_alloc_rxtx_nq_ring(struct bnxt *bp) {
  ...
  nqr = rte_zmalloc_socket("nqr", sizeof(struct bnxt_cp_ring_info),
                           RTE_CACHE_LINE_SIZE, bp->eth_dev->device->numa_node);
  if (nqr == NULL)
    return -ENOMEM;

  ring =
      rte_zmalloc_socket("bnxt_cp_ring_struct", sizeof(struct bnxt_ring),
                         RTE_CACHE_LINE_SIZE, bp->eth_dev->device->numa_node);
  if (ring == NULL) {
    rte_free(nqr);
    return -ENOMEM;
  ...
  rc = bnxt_alloc_rings(bp, bp->eth_dev->device->numa_node, 0, NULL, NULL, nqr,
                        NULL, "l2_nqr");
  if (rc) {
    rte_free(ring);
    rte_free(nqr);
  ...
  rc = bnxt_hwrm_ring_alloc(bp, ring, ring_type, ring_index, HWRM_NA_SIGNATURE,
                            HWRM_NA_SIGNATURE, 0);
  if (rc) {
    rte_free(ring);
    rte_free(nqr);
  ...
  bnxt_set_db(bp, &nqr->cp_db, ring_type, ring_index, ring->fw_ring_id,
              ring->ring_mask);
  bnxt_db_nq(nqr);
...
static int bnxt_alloc_rx_ring(struct bnxt *bp, int queue_index) {
  ...
  rc = bnxt_hwrm_ring_alloc(bp, ring, ring_type, queue_index,
                            cpr->hw_stats_ctx_id, cp_ring->fw_ring_id, 0);
  if (rc)
    return rc;
...
static int bnxt_alloc_rx_agg_ring(struct bnxt *bp, int queue_index) {
  ...
  rc = bnxt_hwrm_ring_alloc(bp, ring, ring_type, map_idx, hw_stats_ctx_id,
...
int bnxt_alloc_hwrm_rx_ring(struct bnxt *bp, int queue_index) {
  ...
  if (rxq->rx_started) {
    if (bnxt_init_one_rx_ring(rxq)) {
      PMD_DRV_LOG(ERR, "ring%d bnxt_init_one_rx_ring failed!\n", queue_index);
      rc = -ENOMEM;
      goto err_out;
    }
    bnxt_db_write(&rxr->rx_db, rxr->rx_raw_prod);
...
err_out:
  PMD_DRV_LOG(ERR, "Failed to allocate receive queue %d, rc %d.\n", queue_index,
              rc);
  return rc;
}
  ...
  for (i = 0; i < bp->rx_cp_nr_rings; i++) {
    ...
    if (bnxt_need_agg_ring(bp->eth_dev)) {
      ...
      if (ring == NULL) {
        ...
        rc = bnxt_alloc_rings(bp, soc_id, i, NULL, rxq, rxq->cp_ring, NULL,
                              "rxr");
        if (rc)
          goto err_out;
      }
...
int bnxt_alloc_async_cp_ring(struct bnxt *bp) {
  ...
  rc = bnxt_hwrm_ring_alloc(bp, cp_ring, ring_type, 0, HWRM_NA_SIGNATURE,
                            HWRM_NA_SIGNATURE, 0);
  ...
  cpr->cp_raw_cons = 0;
  bnxt_set_db(bp, &cpr->cp_db, ring_type, 0, cp_ring->fw_ring_id,
              cp_ring->ring_mask);
...
int bnxt_alloc_async_ring_struct(struct bnxt *bp) {
  ...
  cpr = rte_zmalloc_socket("cpr", sizeof(struct bnxt_cp_ring_info),
                           RTE_CACHE_LINE_SIZE, bp->eth_dev->device->numa_node);
  if (cpr == NULL)
    return -ENOMEM;
  ...
  return bnxt_alloc_rings(bp, bp->eth_dev->device->numa_node, 0, NULL, NULL,
                          bp->async_cp_ring, NULL, "def_cp");
}
...
int bnxt_alloc_hwrm_tx_ring(struct bnxt *bp, int queue_index) {
  ...
  rc = bnxt_hwrm_ring_alloc(bp, ring, HWRM_RING_ALLOC_INPUT_RING_TYPE_TX,
                            queue_index, cpr->hw_stats_ctx_id,
                            cp_ring->fw_ring_id, tx_cosq_id);
  if (rc)
    goto err_out;

  bnxt_set_db(bp, &txr->tx_db, HWRM_RING_ALLOC_INPUT_RING_TYPE_TX, queue_index,
