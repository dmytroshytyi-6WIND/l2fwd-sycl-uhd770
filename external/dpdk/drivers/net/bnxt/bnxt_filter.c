...
void bnxt_free_filter_mem(struct bnxt *bp) {
  ...
  for (i = 0; i < max_filters; i++) {
    ...
    if (filter->fw_ntuple_filter_id != ((uint64_t)-1) &&
      ...
      if (rc)
        PMD_DRV_LOG(ERR, "Cannot free ntuple filter: %d\n", rc);
    }
    filter->fw_ntuple_filter_id = UINT64_MAX;
    ...
    if (filter->fw_l2_filter_id != ((uint64_t)-1) &&
      ...
      if (rc)
        PMD_DRV_LOG(ERR, "Cannot free L2 filter: %d\n", rc);
    }
    filter->fw_l2_filter_id = UINT64_MAX;
  }
  STAILQ_INIT(&bp->free_filter_list);
  ...
  for (i = 0; i < bp->pf->max_vfs; i++) {
    STAILQ_FOREACH(filter, &bp->pf->vf_info[i].filter, next) {
      rte_free(filter);
      STAILQ_REMOVE(&bp->pf->vf_info[i].filter, filter, bnxt_filter_info, next);
    }
  }
}
...
int bnxt_alloc_filter_mem(struct bnxt *bp) {
  ...
  filter_mem = rte_zmalloc("bnxt_filter_info",
                           max_filters * sizeof(struct bnxt_filter_info), 0);
  if (filter_mem == NULL) {
    PMD_DRV_LOG(ERR, "Failed to alloc memory for %d filters", max_filters);
    return -ENOMEM;
  }
