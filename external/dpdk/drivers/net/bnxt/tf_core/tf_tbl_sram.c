...
static int tf_tbl_sram_get_info(struct tf_tbl_sram_get_info_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed to get hcapi_type %s, rc:%s\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->tbl_type),
                strerror(-rc));
    return rc;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed to get slice cnt %s, rc:%s\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->tbl_type),
                strerror(-rc));
    return rc;
  }
...
int tf_tbl_sram_bind(struct tf *tfp __rte_unused) {
  int rc = 0;
  ...
  TFP_DRV_LOG(INFO, "SRAM Table - initialized\n");

  return rc;
}

int tf_tbl_sram_unbind(struct tf *tfp __rte_unused) {
  int rc = 0;
  void *sram_handle = NULL;
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to get sram_handle from session, rc:%s\n",
                strerror(-rc));
    return rc;
  }
  if (sram_handle)
  ...
  TFP_DRV_LOG(INFO, "SRAM Table - deinitialized\n");
  return rc;
}
...
int tf_tbl_sram_alloc(struct tf *tfp, struct tf_tbl_alloc_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to get tbl_db from session, rc:%s\n",
                strerror(-rc));
    return rc;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed to get SRAM info %s\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type));
    return rc;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed to allocate SRAM table:%s\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type));
    return rc;
  }
  *parms->idx = idx;
...
int tf_tbl_sram_free(struct tf *tfp __rte_unused,
  ...
  rc = tf_tbl_sram_get_info(&iparms);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed to get table info:%s\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type));
    return rc;
  }
  ...
  if (rc || !allocated) {
    TFP_DRV_LOG(ERR, "%s: Free of invalid entry:%s idx(%d):(%s)\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                parms->idx, strerror(-rc));
    rc = -ENOMEM;
    return rc;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed to free entry:%s idx(%d)\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                parms->idx);
    return rc;
  }
...
int tf_tbl_sram_set(struct tf *tfp, struct tf_tbl_set_parms *parms) {
  ...
  if (tf_session_is_shared_session(tfs)) {
    ...
    if (dev->ops->tf_dev_get_tbl_info) {
      rc = dev->ops->tf_dev_get_tbl_info(tfp, tbl_db->tbl_db[parms->dir],
                                         parms->type, &base, &shift);
      if (rc) {
        TFP_DRV_LOG(ERR, "%s: Failed to get table info:%d\n",
    ...
    if (rallocated != TF_RM_ALLOCATED_ENTRY_IN_USE) {
      TFP_DRV_LOG(ERR, "%s, Invalid or not allocated index, type:%s, idx:%d\n",
                  tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                  parms->idx);
      return -EINVAL;
    }
    ...
    rc = tf_sram_mgr_is_allocated(sram_handle, &aparms);
    if (rc || !allocated) {
      TFP_DRV_LOG(ERR, "%s: Entry not allocated:%s idx(%d):(%s)\n",
                  tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                  parms->idx, strerror(-rc));
      rc = -ENOMEM;
      return rc;
  ...
  rc = tf_rm_get_hcapi_type(&hparms);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s, Failed type lookup, type:%s, rc:%s\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                strerror(-rc));
    return rc;
  }
  ...
  rc = tf_msg_set_tbl_entry(tfp, parms->dir, hcapi_type,
                            parms->data_sz_in_bytes, parms->data, parms->idx);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s, Set failed, type:%s, rc:%s\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                strerror(-rc));
    return rc;
  }
...
int tf_tbl_sram_get(struct tf *tfp, struct tf_tbl_get_parms *parms) {
  ...
  /* Get the entry */
  rc =
      tf_msg_get_tbl_entry(tfp, parms->dir, hcapi_type, parms->data_sz_in_bytes,
                           parms->data, parms->idx, clear_on_read);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s, Get failed, type:%s, rc:%s\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                strerror(-rc));
    return rc;
  }
...
int tf_tbl_sram_bulk_get(struct tf *tfp, struct tf_tbl_get_bulk_parms *parms) {
  ...
  rc = tf_sram_mgr_is_allocated(sram_handle, &aparms);
  if (rc || !allocated) {
    TFP_DRV_LOG(ERR, "%s: Entry not allocated:%s starting_idx(%d):(%s)\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                parms->starting_idx, strerror(-rc));
    rc = -ENOMEM;
    return rc;
  }
  ...
  rc = tf_sram_mgr_is_allocated(sram_handle, &aparms);
  if (rc || !allocated) {
    TFP_DRV_LOG(ERR, "%s: Entry not allocated:%s last_idx(%d):(%s)\n",
  ...
  /* Get the entries */
  rc = tf_msg_bulk_get_tbl_entry(
      tfp, parms->dir, hcapi_type, parms->starting_idx, parms->num_entries,
      parms->entry_sz_in_bytes, parms->physical_mem_addr, clear_on_read);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s, Bulk get failed, type:%s, rc:%s\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                strerror(-rc));
  }
  return rc;
