...
int tf_open_session(struct tf *tfp, struct tf_open_session_parms *parms) {
  ...
  if (parms->device_type != TF_DEVICE_TYPE_WH &&
    ...
    TFP_DRV_LOG(ERR, "Unsupported device type %d\n", parms->device_type);
    return -ENOTSUP;
  }
  ...
  TFP_DRV_LOG(INFO, "domain:%d, bus:%d, device:%u\n",
              parms->session_id.internal.domain, parms->session_id.internal.bus,
...
int tf_attach_session(struct tf *tfp, struct tf_attach_session_parms *parms) {
  ...
  rc = sscanf(parms->attach_chan_name, "%x:%x:%x.%u", &domain, &bus, &slot,
              &device);
  if (rc != 4) {
    TFP_DRV_LOG(ERR, "Failed to scan device attach_chan_name\n");
    return -EINVAL;
  }
  ...
  rc = tf_session_attach_session(tfp, &aparms);
  /* Logging handled by dev_bind */
  if (rc)
    return rc;

  TFP_DRV_LOG(INFO, "Attached to session, session_id:%d\n",
              parms->session_id.id);

  TFP_DRV_LOG(INFO, "domain:%d, bus:%d, device:%d, fw_session_id:%d\n",
              parms->session_id.internal.domain, parms->session_id.internal.bus,
              parms->session_id.internal.device,
...
int tf_close_session(struct tf *tfp) {
   ...
   */
  rc = tf_session_close_session(tfp, &cparms);
  /* Logging handled by tf_session_close_session */
  if (rc)
    return rc;

  TFP_DRV_LOG(INFO, "domain:%d, bus:%d, device:%d\n",
...
int tf_insert_em_entry(struct tf *tfp, struct tf_insert_em_entry_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: EM insert failed, rc:%s\n", tf_dir_2_str(parms->dir),
                strerror(-rc));
    return rc;
  }
...
int tf_delete_em_entry(struct tf *tfp, struct tf_delete_em_entry_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: EM delete failed, rc:%s\n", tf_dir_2_str(parms->dir),
                strerror(-rc));
    return rc;
  }
...
int tf_get_global_cfg(struct tf *tfp, struct tf_global_cfg_parms *parms) {
  ...
  if (parms->config == NULL || parms->config_sz_in_bytes == 0) {
  ...
  if (dev->ops->tf_dev_get_global_cfg == NULL) {
    rc = -EOPNOTSUPP;
    TFP_DRV_LOG(ERR, "%s: Operation not supported, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return -EOPNOTSUPP;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Global Cfg get failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_set_global_cfg(struct tf *tfp, struct tf_global_cfg_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Global Cfg set failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_alloc_identifier(struct tf *tfp,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Identifier allocation failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_free_identifier(struct tf *tfp, struct tf_free_identifier_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Identifier free failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_search_identifier(struct tf *tfp,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Identifier search failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_search_tcam_entry(struct tf *tfp,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: TCAM allocation failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_set_tcam_entry(struct tf *tfp, struct tf_set_tcam_entry_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: TCAM set failed, rc:%s\n", tf_dir_2_str(parms->dir),
                strerror(-rc));
    return rc;
  }
...
int tf_get_tcam_entry(struct tf *tfp __rte_unused,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: TCAM get failed, rc:%s\n", tf_dir_2_str(parms->dir),
                strerror(-rc));
    return rc;
  }
...
int tf_free_tcam_entry(struct tf *tfp, struct tf_free_tcam_entry_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: TCAM free failed, rc:%s\n", tf_dir_2_str(parms->dir),
                strerror(-rc));
    return rc;
  }
...
int tf_move_tcam_shared_entries(
    struct tf *tfp, struct tf_move_tcam_shared_entries_parms *parms) {
  int rc;
  struct tf_session *tfs;
  struct tf_dev_info *dev;
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: TCAM shared entries move failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_clear_tcam_shared_entries(
    struct tf *tfp, struct tf_clear_tcam_shared_entries_parms *parms) {
  int rc;
  struct tf_session *tfs;
  struct tf_dev_info *dev;
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: TCAM shared entries clear failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_alloc_tbl_entry(struct tf *tfp, struct tf_alloc_tbl_entry_parms *parms) {
  ...
  if (parms->type == TF_TBL_TYPE_EXT) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: External table allocation failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
      return rc;
    }
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: SRAM table allocation failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
      return rc;
    }
  } else {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: Table allocation failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
      return rc;
    }
  }
...
int tf_free_tbl_entry(struct tf *tfp, struct tf_free_tbl_entry_parms *parms) {
  ...
  if (parms->type == TF_TBL_TYPE_EXT) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: Table free failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
      return rc;
    }
  } else if (dev->ops->tf_dev_is_sram_managed(tfp, parms->type)) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: SRAM table free failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
      return rc;
    }
  } else {
...
int tf_set_tbl_entry(struct tf *tfp, struct tf_set_tbl_entry_parms *parms) {
  ...
  if (parms->type == TF_TBL_TYPE_EXT) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: Table set failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
      return rc;
    }
  } else if (dev->ops->tf_dev_is_sram_managed(tfp, parms->type)) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: SRAM table set failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
      return rc;
    }
  } else {
...
int tf_get_tbl_entry(struct tf *tfp, struct tf_get_tbl_entry_parms *parms) {
  ...
  if (dev->ops->tf_dev_is_sram_managed(tfp, parms->type)) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: SRAM table get failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
      return rc;
    }
  } else {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: Table get failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
      return rc;
    }
  }
...
int tf_bulk_get_tbl_entry(struct tf *tfp,
  ...
  if (parms->type == TF_TBL_TYPE_EXT) {
    /* Not supported, yet */
    rc = -EOPNOTSUPP;
    TFP_DRV_LOG(ERR, "%s, External table type not supported, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: SRAM table bulk get failed, rc:%s\n",
                  tf_dir_2_str(parms->dir), strerror(-rc));
    }
    return rc;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Table get bulk failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
  return rc;
...
int tf_get_shared_tbl_increment(
    struct tf *tfp, struct tf_get_shared_tbl_increment_parms *parms) {
  int rc = 0;
  struct tf_session *tfs;
  struct tf_dev_info *dev;
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Get table increment not supported, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_alloc_tbl_scope(struct tf *tfp, struct tf_alloc_tbl_scope_parms *parms) {
  struct tf_session *tfs;
  struct tf_dev_info *dev;
  int rc;
  ...
  if (dev->ops->tf_dev_alloc_tbl_scope != NULL) {
    ...
    TFP_DRV_LOG(ERR, "Alloc table scope not supported by device\n");
    return -EINVAL;
  }
...
int tf_map_tbl_scope(struct tf *tfp, struct tf_map_tbl_scope_parms *parms) {
  ...
  if (dev->ops->tf_dev_map_tbl_scope != NULL) {
    ...
    TFP_DRV_LOG(ERR, "Map table scope not supported by device\n");
    return -EINVAL;
  }
...
int tf_free_tbl_scope(struct tf *tfp, struct tf_free_tbl_scope_parms *parms) {
  ...
  if (dev->ops->tf_dev_free_tbl_scope) {
    ...
    TFP_DRV_LOG(ERR, "Free table scope not supported by device\n");
    return -EINVAL;
  }
...
int tf_set_if_tbl_entry(struct tf *tfp,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: If_tbl set failed, rc:%s\n", tf_dir_2_str(parms->dir),
                strerror(-rc));
    return rc;
  }
...
int tf_get_if_tbl_entry(struct tf *tfp,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: If_tbl get failed, rc:%s\n", tf_dir_2_str(parms->dir),
                strerror(-rc));
    return rc;
  }
...
int tf_get_session_info(struct tf *tfp,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Ident get resc info failed, rc:%s\n", strerror(-rc));
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Tbl get resc info failed, rc:%s\n", strerror(-rc));
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "TCAM get resc info failed, rc:%s\n", strerror(-rc));
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "EM get resc info failed, rc:%s\n", strerror(-rc));
  }
...
int tf_query_sram_resources(struct tf *tfp,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Get SRAM resc info failed, rc:%s\n", strerror(-rc));
    return rc;
  }
  ...
  rc = tf_msg_session_resc_qcaps(tfp, &dev, parms->dir, max_types, query,
  ...
  rc = dev.ops->tf_dev_get_sram_resources((void *)query, parms->bank_resc_count,
...
int tf_set_sram_policy(struct tf *tfp, struct tf_set_sram_policy_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: SRAM policy set failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_get_sram_policy(struct tf *tfp, struct tf_get_sram_policy_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: SRAM policy get failed, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
