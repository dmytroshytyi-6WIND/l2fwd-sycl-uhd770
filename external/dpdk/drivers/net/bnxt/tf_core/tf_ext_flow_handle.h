...
#define TF_FLOW_HANDLE_MASK                                                    \
  (TF_NUM_KEY_ENTRIES_FLOW_HANDLE_MASK | TF_FLOW_TYPE_FLOW_HANDLE_MASK |       \
   TF_FLAGS_FLOW_HANDLE_MASK | TF_INDEX_FLOW_HANDLE_MASK |                     \
   TF_ENTRY_NUM_FLOW_HANDLE_MASK | TF_HASH_TYPE_FLOW_HANDLE_MASK)

#define TF_GET_FIELDS_FROM_FLOW_HANDLE(flow_handle, num_key_entries,           \
                                       flow_type, flags, index, entry_num,     \
                                       hash_type)                              \
  do {                                                                         \
    (num_key_entries) =                                                        \
...
#define TF_SET_FIELDS_IN_FLOW_HANDLE(flow_handle, num_key_entries, flow_type,  \
                                     flags, index, entry_num, hash_type)       \
  do {                                                                         \
    ...
    (flow_handle) |=                                                           \
        (((flags) << TF_FLAGS_FLOW_HANDLE_SFT) & TF_FLAGS_FLOW_HANDLE_MASK);   \
...
#define TF_GET_INDEX_FROM_FLOW_HANDLE(flow_handle, index)                      \
  do {                                                                         \
    index = (((flow_handle)&TF_INDEX_FLOW_HANDLE_MASK) >>                      \
             TF_INDEX_FLOW_HANDLE_SFT);                                        \
  } while (0)

#define TF_GET_HASH_TYPE_FROM_FLOW_HANDLE(flow_handle, hash_type)              \
  do {                                                                         \
    hash_type = (((flow_handle)&TF_HASH_TYPE_FLOW_HANDLE_MASK) >>              \
                 TF_HASH_TYPE_FLOW_HANDLE_SFT);                                \
  } while (0)

#define TF_GET_NUM_KEY_ENTRIES_FROM_FLOW_HANDLE(flow_handle, num_key_entries)  \
  do {                                                                         \
    (num_key_entries = (((flow_handle)&TF_NUM_KEY_ENTRIES_FLOW_HANDLE_MASK) >> \
                        TF_NUM_KEY_ENTRIES_FLOW_HANDLE_SFT));                  \
  } while (0)

#define TF_GET_ENTRY_NUM_FROM_FLOW_HANDLE(flow_handle, entry_num)              \
  do {                                                                         \
    (entry_num = (((flow_handle)&TF_ENTRY_NUM_FLOW_HANDLE_MASK) >>             \
                  TF_ENTRY_NUM_FLOW_HANDLE_SFT));                              \
  } while (0)
...
#define TF_SET_FLOW_ID(flow_id, gfid, flag, dir)                               \
  do {                                                                         \
    (flow_id) &=                                                               \
        ~(TF_GFID_FLOW_ID_MASK | TF_FLAG_FLOW_ID_MASK | TF_DIR_FLOW_ID_MASK);  \
    (flow_id) |= (((gfid) << TF_GFID_FLOW_ID_SFT) & TF_GFID_FLOW_ID_MASK) |    \
                 (((flag) << TF_FLAG_FLOW_ID_SFT) & TF_FLAG_FLOW_ID_MASK) |    \
                 (((dir) << TF_DIR_FLOW_ID_SFT) & TF_DIR_FLOW_ID_MASK);        \
  } while (0)

#define TF_GET_GFID_FROM_FLOW_ID(flow_id, gfid)                                \
  do {                                                                         \
    gfid = (((flow_id)&TF_GFID_FLOW_ID_MASK) >> TF_GFID_FLOW_ID_SFT);          \
  } while (0)

#define TF_GET_DIR_FROM_FLOW_ID(flow_id, dir)                                  \
  do {                                                                         \
    dir = (((flow_id)&TF_DIR_FLOW_ID_MASK) >> TF_DIR_FLOW_ID_SFT);             \
  } while (0)

#define TF_GET_FLAG_FROM_FLOW_ID(flow_id, flag)                                \
  do {                                                                         \
    flag = (((flow_id)&TF_FLAG_FLOW_ID_MASK) >> TF_FLAG_FLOW_ID_SFT);          \
  } while (0)
...
#define TF_SET_GFID(gfid, index, type)                                         \
  do {                                                                         \
    gfid = (((index) << TF_HASH_INDEX_GFID_SFT) & TF_HASH_INDEX_GFID_MASK) |   \
           (((type) << TF_HASH_TYPE_GFID_SFT) & TF_HASH_TYPE_GFID_MASK);       \
  } while (0)

#define TF_GET_HASH_INDEX_FROM_GFID(gfid, index)                               \
  do {                                                                         \
    index = (((gfid)&TF_HASH_INDEX_GFID_MASK) >> TF_HASH_INDEX_GFID_SFT);      \
  } while (0)

#define TF_GET_HASH_TYPE_FROM_GFID(gfid, type)                                 \
  do {                                                                         \
    type = (((gfid)&TF_HASH_TYPE_GFID_MASK) >> TF_HASH_TYPE_GFID_SFT);         \
  } while (0)
