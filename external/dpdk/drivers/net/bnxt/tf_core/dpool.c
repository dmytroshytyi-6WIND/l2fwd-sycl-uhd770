...
int dpool_init(struct dpool *dpool, uint32_t start_index, uint32_t size,
               uint8_t max_alloc_size, void *user_data,
...
static int dpool_move(struct dpool *dpool, uint32_t dst_index,
                      uint32_t src_index) {
  uint32_t size;
  uint32_t i;
  if (DP_IS_FREE(dpool->entry[dst_index].flags)) {
    ...
    if (dpool->move_callback != NULL) {
      dpool->move_callback(dpool->user_data, dpool->entry[src_index].entry_data,
...
int dpool_defrag(struct dpool *dpool, uint32_t entry_size, uint8_t defrag) {
  ...
  while (1) {
     ...
     */
    if (defrag == DP_DEFRAG_TO_FIT && largest_free_size >= entry_size)
      goto done;
    ...
    for (size = largest_free_size; size > 0; size--) {
      for (i = 0; i < adj_list->size; i++) {
        if (adj_list->entry[i].size == size &&
            ((size + adj_list->entry[i].left + adj_list->entry[i].right) >
             max)) {
          max = size + adj_list->entry[i].left + adj_list->entry[i].right;
          max_size = size;
     ...
     */
    if (max) {
      rc = dpool_move(dpool, free_list->entry[largest_free_index].index,
                      max_index);
      if (rc) {
        tfp_free(free_list);
        tfp_free(adj_list);
...
uint32_t dpool_alloc(struct dpool *dpool, uint32_t size, uint8_t defrag) {
  uint32_t i;
  uint32_t j;
  uint32_t count = 0;
   ...
   */
  if (defrag != DP_DEFRAG_NONE && dpool->move_callback == NULL)
    return DP_INVALID_INDEX;
...
int dpool_free(struct dpool *dpool, uint32_t index) {
  uint32_t i;
...
int dpool_set_entry_data(struct dpool *dpool, uint32_t index,
                         uint64_t entry_data) {
  int start = (index - dpool->start_index);
