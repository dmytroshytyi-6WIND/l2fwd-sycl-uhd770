...
int tf_ident_bind(struct tf *tfp, struct tf_ident_cfg_parms *parms) {
  ...
  for (i = 0; i < TF_DIR_MAX; i++) {
    ...
    if (parms->shadow_copy) {
      shadow_cfg.alloc_cnt = parms->resources->ident_cnt[i].cnt;
      ...
      if (rc) {
        TFP_DRV_LOG(ERR, "%s: Ident shadow DB creation failed\n",
                    tf_dir_2_str(i));
  ...
  TFP_DRV_LOG(INFO, "Identifier - initialized\n");
...
int tf_ident_unbind(struct tf *tfp) {
  ...
  for (i = 0; i < TF_DIR_MAX; i++) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "rm free failed on unbind\n");
    }
    if (shadow_init) {
...
int tf_ident_alloc(struct tf *tfp __rte_unused,
  ...
  rc = tf_session_get_db(tfp, TF_MODULE_TYPE_IDENTIFIER, &ident_db_ptr);
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to get ident_db from session, rc:%s\n",
                strerror(-rc));
    return rc;
  }
  ident_db = (struct ident_rm_db *)ident_db_ptr;
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed allocate, type:%d\n", tf_dir_2_str(parms->dir),
                parms->type);
    return rc;
  }
  ...
  if (shadow_init) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: Failed insert shadow DB, type:%d\n",
...
int tf_ident_free(struct tf *tfp __rte_unused,
...
int tf_ident_search(struct tf *tfp __rte_unused,
  ...
  if (!shadow_init) {
    TFP_DRV_LOG(ERR, "%s: Identifier Shadow copy is not enabled\n",
                tf_dir_2_str(parms->dir));
    return -EINVAL;
  }
  ...
  if (allocated != TF_RM_ALLOCATED_ENTRY_IN_USE) {
    TFP_DRV_LOG(ERR, "%s: Entry not allocated, type:%d, index:%d\n",
                tf_dir_2_str(parms->dir), parms->type, parms->search_id);
    return -EINVAL;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed search shadow DB, type:%d\n",
                tf_dir_2_str(parms->dir), parms->type);
    return rc;
  }
