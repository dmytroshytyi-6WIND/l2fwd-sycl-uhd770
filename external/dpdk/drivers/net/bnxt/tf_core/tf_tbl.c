...
int tf_tbl_bind(struct tf *tfp, struct tf_tbl_cfg_parms *parms) {
  ...
  if (db_rc[TF_DIR_RX] && db_rc[TF_DIR_TX]) {
    TFP_DRV_LOG(ERR, "%s: No Table DB created\n", tf_dir_2_str(d));
    return db_rc[TF_DIR_RX];
  }

  TFP_DRV_LOG(INFO, "Table Type - initialized\n");
...
int tf_tbl_alloc(struct tf *tfp __rte_unused,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed allocate, type:%s\n", tf_dir_2_str(parms->dir),
...
int tf_tbl_free(struct tf *tfp __rte_unused, struct tf_tbl_free_parms *parms) {
  int rc;
  ...
  if (allocated != TF_RM_ALLOCATED_ENTRY_IN_USE) {
    TFP_DRV_LOG(ERR, "%s: Entry already free, type:%s, index:%d\n",
  ...
  if (parms->type == TF_TBL_TYPE_ACT_STATS_64) {
    ...
    /* Clear the counter
     */
    rc = tf_msg_set_tbl_entry(tfp, parms->dir, hcapi_type, sizeof(data), data,
                              parms->idx);
    if (rc) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Free failed, type:%s, index:%d\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                parms->idx);
    return rc;
  }
...
int tf_tbl_set(struct tf *tfp, struct tf_tbl_set_parms *parms) {
  ...
  if (allocated != TF_RM_ALLOCATED_ENTRY_IN_USE) {
    TFP_DRV_LOG(ERR, "%s, Invalid or not allocated, type:%s, idx:%d\n",
                tf_dir_2_str(parms->dir), tf_tbl_type_2_str(parms->type),
                parms->idx);
    return -EINVAL;
  }
...
int tf_tbl_bulk_get(struct tf *tfp, struct tf_tbl_get_bulk_parms *parms) {
  ...
  rc = tf_msg_bulk_get_tbl_entry(
      ...
      parms->entry_sz_in_bytes, parms->physical_mem_addr, false);
  if (rc) {
  ...
  for (d = 0; d < TF_DIR_MAX; d++) {
    ...
    if (dev->ops->tf_dev_get_tbl_info) {
      ...
      for (i = 0; i < TF_TBL_TYPE_MAX; i++) {
        ...
        rc = dev->ops->tf_dev_get_tbl_info(tfp, tbl_db->tbl_db[d], i, &base,
                                           &shift);
        if (rc) {
        ...
        if (dinfo[i].stride)
          TF_TBL_RM_TO_PTR(&dinfo[i].start, dinfo[i].start, base, shift);
      }
    }
  }
