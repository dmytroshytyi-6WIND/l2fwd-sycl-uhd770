...
uint16_t tf_sram_bank_2_base_offset[TF_SRAM_BANK_ID_MAX] = {0, 2048, 4096,
                                                            6144};
...
static void tf_sram_block_id_2_offset(enum tf_sram_bank_id bank_id,
                                      uint16_t block_id, uint16_t *offset) {
...
static void tf_sram_offset_2_block_id(enum tf_sram_bank_id bank_id,
                                      uint16_t offset, uint16_t *block_id,
                                      uint16_t *slice_offset) {
  *slice_offset = offset & 0x7;
  *block_id = ((offset & ~0x7) >> 3) - tf_sram_bank_2_base_offset[bank_id];
}
...
tf_sram_is_slice_allocated_in_block(struct tf_sram_block *block,
                                    enum tf_sram_slice_size slice_size,
                                    uint16_t slice_offset, bool *is_allocated) {
  int rc = 0;
  uint8_t shift;
  uint8_t slice_mask = 0;
 ...
 * Get the block count
 */
static uint32_t tf_sram_get_block_cnt(struct tf_sram_slice_list *slice_list) {
 ...
 */
static struct tf_sram_block *
tf_sram_alloc_block(struct tf_sram_slice_list *slice_list, uint16_t block_id) {
  struct tf_sram_block *block;
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "Failed to allocate block, rc:%s\n", strerror(-rc));
    return NULL;
  }
...
int tf_sram_mgr_bind(void **sram_handle) {
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "Failed to allocate SRAM mgmt data, rc:%s\n",
                strerror(-rc));
    return rc;
  }
...
int tf_sram_mgr_unbind(void *sram_handle) {
  ...
  for (dir = 0; dir < TF_DIR_MAX; dir++) {
    /* For each bank
     */
    for (bank_id = TF_SRAM_BANK_ID_0; bank_id < TF_SRAM_BANK_ID_MAX;
         bank_id++) {
      /* For each slice size
       */
      for (slice_size = TF_SRAM_SLICE_SIZE_8B;
           slice_size < TF_SRAM_SLICE_SIZE_MAX; slice_size++) {
        rc =
            tf_sram_get_slice_list(sram, &slice_list, slice_size, dir, bank_id);
        if (rc) {
          /* Log error */
          TFP_DRV_LOG(ERR, "No SRAM slice list, rc:%s\n", strerror(-rc));
          return rc;
        }
...
int tf_sram_mgr_alloc(void *sram_handle,
   ...
   */
  rc = tf_sram_get_slice_list(sram, &slice_list, parms->slice_size, parms->dir,
                              parms->bank_id);
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "No SRAM slice list, rc:%s\n", strerror(-rc));
  ...
  if (!slice_list->first_not_full_block) {
    ...
    /* Block exists
     */
    block = (struct tf_sram_block *)(slice_list->first_not_full_block);
  }
  rc = tf_sram_get_next_slice_in_block(block, parms->slice_size, &slice_offset,
                                       &block_is_full);

  /* Find the new first non-full block in the list
   */
  tf_sram_find_first_not_full_block(slice_list, parms->slice_size,
...
int tf_sram_mgr_free(void *sram_handle, struct tf_sram_mgr_free_parms *parms) {
   ...
   */
  rc = tf_sram_free_slice(parms->slice_size, slice_offset, block,
                          &block_is_empty);
  if (rc) {
  ...
  if (parms->tbl_type == TF_TBL_TYPE_ACT_STATS_64) {
    ...
    /* Clear the counter
     */
    rc = tf_msg_set_tbl_entry(parms->tfp, parms->dir, hcapi_type, sizeof(data),
                              data, parms->sram_offset);
    if (rc) {
...
int tf_sram_mgr_dump(void *sram_handle, struct tf_sram_mgr_dump_parms *parms) {
  ...
  if (slice_list->cnt || slice_list->first_not_full_block) {
    TFP_DRV_LOG(INFO, "\n********** %s: %s: %s ***********\n",
                tf_sram_bank_2_str(parms->bank_id), tf_dir_2_str(parms->dir),
...
int tf_sram_mgr_is_allocated(void *sram_handle,
  ...
  rc = tf_sram_is_slice_allocated_in_block(block, parms->slice_size,
                                           slice_offset, parms->is_allocated);
done:
  return rc;
}
