 ...
 */
static void tf_rm_count_hcapi_reservations(
    enum tf_dir dir, enum tf_module_type module, struct tf_rm_element_cfg *cfg,
    uint16_t *reservations, uint16_t count, uint16_t *valid_count) {
  int i;
  uint16_t cnt = 0;

  for (i = 0; i < count; i++) {
    if (cfg[i].cfg_type != TF_RM_ELEM_CFG_NULL && reservations[i] > 0)
      cnt++;
     ...
     * this type of check.
     */
    if (module != TF_MODULE_TYPE_EM && cfg[i].cfg_type == TF_RM_ELEM_CFG_NULL &&
        reservations[i] > 0) {
      TFP_DRV_LOG(ERR, "%s, %s, %s allocation of %d not supported\n",
                  tf_module_2_str(module), tf_dir_2_str(dir),
                  tf_module_subtype_2_str(module, i), reservations[i]);
    }
  }
...
static int tf_rm_adjust_index(struct tf_rm_element *db,
                              enum tf_rm_adjust_type action, uint32_t subtype,
                              uint32_t index, uint32_t *adj_index) {
  int rc = 0;
  uint32_t base_index;
...
static void tf_rm_log_residuals(enum tf_dir dir, enum tf_module_type module,
                                uint16_t count, uint16_t *residuals) {
  int i;
  ...
  for (i = 0; i < count; i++) {
    if (residuals[i] != 0)
      TFP_DRV_LOG(INFO, "%s, %s was not cleaned up, %d outstanding\n",
                  tf_dir_2_str(dir), tf_module_subtype_2_str(module, i),
                  residuals[i]);
  }
}
#endif /* TF_RM_DEBUG == 1 */
 ...
 *   - EOPNOTSUPP - Operation not supported
 */
static int tf_rm_check_residuals(struct tf_rm_new_db *rm_db,
                                 uint16_t *resv_size,
...
#if (TF_RM_DEBUG == 1)
  tf_rm_log_residuals(rm_db->dir, rm_db->module, rm_db->num_entries, residuals);
#endif
 ...
 */
static int
tf_rm_update_parent_reservations(struct tf *tfp, struct tf_dev_info *dev,
                                 struct tf_rm_element_cfg *cfg,
                                 uint16_t *alloc_cnt, uint16_t num_elements,
                                 uint16_t *req_cnt, bool shared_session) {
  int parent, child;
  const char *type_str;
  ...
  for (parent = 0; parent < num_elements; parent++) {
    ...
    if (cfg[parent].cfg_type == TF_RM_ELEM_CFG_HCAPI_BA_PARENT) {
      ...
      if (alloc_cnt[parent]) {
        dev->ops->tf_dev_get_resource_str(tfp, cfg[parent].hcapi_type,
                                          &type_str);
#if (TF_RM_DEBUG == 1)
        printf("%s:%s cnt(%d) slices(%d)\n", type_str,
               tf_tbl_type_2_str(parent), alloc_cnt[parent], p_slices);
#endif /* (TF_RM_DEBUG == 1) */
      }
      ...
      for (child = 0; child < num_elements; child++) {
        ...
        if (cfg[child].cfg_type == TF_RM_ELEM_CFG_HCAPI_BA_CHILD &&
            cfg[child].parent_subtype == parent && alloc_cnt[child]) {
          uint8_t c_slices = 1;
          uint16_t cnt = 0;
          ...
          dev->ops->tf_dev_get_resource_str(tfp, cfg[child].hcapi_type,
                                            &type_str);
#if (TF_RM_DEBUG == 1)
          printf("%s:%s cnt(%d) slices(%d)\n", type_str,
                 tf_tbl_type_2_str(child), alloc_cnt[child], c_slices);
#endif /* (TF_RM_DEBUG == 1) */
...
int tf_rm_create_db(struct tf *tfp, struct tf_rm_create_db_parms *parms) {
  ...
  tfp_memcpy(req_cnt, parms->alloc_cnt, parms->num_elements * sizeof(uint16_t));

  shared_session = tf_session_is_shared_session(tfs);

  /* Update the req_cnt based upon the element configuration
   */
  tf_rm_update_parent_reservations(tfp, dev, parms->cfg, parms->alloc_cnt,
                                   parms->num_elements, req_cnt,
                                   shared_session);
   ...
   */
  tf_rm_count_hcapi_reservations(parms->dir, parms->module, parms->cfg, req_cnt,
                                 parms->num_elements, &hcapi_items);

  if (hcapi_items == 0) {
#if (TF_RM_DEBUG == 1)
    TFP_DRV_LOG(INFO, "%s: module: %s Empty RM DB create request\n",
                tf_dir_2_str(parms->dir), tf_module_2_str(parms->module));
#endif
    parms->rm_db = NULL;
    return -ENOMEM;
  }
  ...
  for (i = 0, j = 0; i < parms->num_elements; i++) {
    ...
    if (cfg->cfg_type == TF_RM_ELEM_CFG_HCAPI ||
      ...
      if (req_cnt[i] <= query[hcapi_type].max) {
        ...
        dev->ops->tf_dev_get_resource_str(tfp, hcapi_type, &type_str);
        TFP_DRV_LOG(ERR, "Failure, %s:%d:%s req:%d avail:%d\n",
                    tf_dir_2_str(parms->dir), hcapi_type, type_str, req_cnt[i],
   ...
   */
  rc = tf_msg_session_resc_alloc(tfp, dev, parms->dir, hcapi_items, req, resv);
  if (rc)
    return rc;
  ...
  for (i = 0, j = 0; i < parms->num_elements; i++) {
    ...
    dev->ops->tf_dev_get_resource_str(tfp, cfg->hcapi_type, &type_str);
    ...
    if (req_cnt[i] == resv[j].stride) {
      ...
      if (cfg->cfg_type == TF_RM_ELEM_CFG_HCAPI_BA ||
          cfg->cfg_type == TF_RM_ELEM_CFG_HCAPI_BA_PARENT) {
        /* Create pool */
        pool_size = (BITALLOC_SIZEOF(resv[j].stride) / sizeof(struct bitalloc));
        /* Alloc request, alignment already set */
        cparms.nitems = pool_size;
        cparms.size = sizeof(struct bitalloc);
        rc = tfp_calloc(&cparms);
        if (rc) {
          TFP_DRV_LOG(ERR, "%s: Pool alloc failed, type:%d:%s\n",
                      tf_dir_2_str(parms->dir), cfg->hcapi_type, type_str);
          goto fail;
        }
        ...
        if (rc) {
          TFP_DRV_LOG(ERR, "%s: Pool init failed, type:%d:%s\n",
                      tf_dir_2_str(parms->dir), cfg->hcapi_type, type_str);
          goto fail;
        }
      }
       ...
       */
      TFP_DRV_LOG(ERR, "%s: Alloc failed %d:%s req:%d, alloc:%d\n",
                  tf_dir_2_str(parms->dir), cfg->hcapi_type, type_str,
                  req_cnt[i], resv[j].stride);
      goto fail;
    }
  }
...
int tf_rm_create_db_no_reservation(struct tf *tfp,
  ...
  if (hcapi_items == 0) {
    TFP_DRV_LOG(ERR, "%s: module:%s Empty RM DB create request\n",
                tf_dir_2_str(parms->dir), tf_module_2_str(parms->module));
   ...
   */
  rc = tf_msg_session_resc_info(tfp, dev, parms->dir, hcapi_items, req, resv);
  if (rc)
    return rc;
...
int tf_rm_free_db(struct tf *tfp, struct tf_rm_free_db_parms *parms) {
   ...
   */
  if (residuals_found) {
    rc = tf_msg_session_resc_flush(tfp, parms->dir, resv_size, resv);
    tfp_free((void *)resv);
    /* On failure we still have to cleanup so we can only
     * log that FW failed.
     */
    if (rc)
      TFP_DRV_LOG(ERR, "%s: Internal Flush error, module:%s\n",
                  tf_dir_2_str(parms->dir), tf_module_2_str(rm_db->module));
  }
 ...
 */
static int tf_rm_get_pool(struct tf_rm_new_db *rm_db, uint16_t subtype,
                          struct bitalloc **pool, uint16_t *new_subtype) {
  int rc = 0;
  ...
  *pool = rm_db->db[tmp_subtype].pool;
  ...
  if (rm_db->db[tmp_subtype].pool == NULL) {
    rc = -ENOTSUP;
    TFP_DRV_LOG(ERR, "%s: Invalid pool for this type:%d, rc:%s\n",
                tf_dir_2_str(rm_db->dir), tmp_subtype, strerror(-rc));
    return rc;
  }
...
int tf_rm_allocate(struct tf_rm_allocate_parms *parms) {
  ...
  rc =
      tf_rm_adjust_index(rm_db->db, TF_RM_ADJUST_ADD_BASE, subtype, id, &index);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Alloc adjust of base index failed, rc:%s\n",
                tf_dir_2_str(rm_db->dir), strerror(-rc));
    return -EINVAL;
  }
...
int tf_rm_free(struct tf_rm_free_parms *parms) {
  ...
  /* Adjust for any non zero start value */
  rc = tf_rm_adjust_index(rm_db->db, TF_RM_ADJUST_RM_BASE, subtype,
                          parms->index, &adj_index);
  if (rc)
    return rc;
...
int tf_rm_get_all_info(struct tf_rm_get_alloc_info_parms *parms, int size) {
  ...
  for (i = 0; i < size; i++) {
    ...
    memcpy(info, &rm_db->db[i].alloc, sizeof(struct tf_rm_alloc_info));
    info++;
  }
 ...
 */
int tf_rm_check_indexes_in_range(
    struct tf_rm_check_indexes_in_range_parms *parms) {
  struct tf_rm_new_db *rm_db;
  enum tf_rm_elem_cfg_type cfg_type;
  uint32_t base_index;
  uint32_t stride;
