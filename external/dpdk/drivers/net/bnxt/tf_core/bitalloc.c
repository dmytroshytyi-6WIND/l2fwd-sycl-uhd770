...
static int ba_fls(bitalloc_word_t v) {
  int c = 32;
...
static int ba_ffs(bitalloc_word_t v) {
...
int ba_init(struct bitalloc *pool, int size, bool free) {
  ...
  /* Zero structure */
  for (i = 0; i < (int)(BITALLOC_SIZEOF(size) / sizeof(bitalloc_word_t)); i++)
    mem[i] = 0;
...
static int ba_alloc_helper(struct bitalloc *pool, int offset, int words,
                           unsigned int size, int index, int *clear) {
  ...
  if (pool->size > size) {
    r = ba_alloc_helper(pool, offset + words + 1, storage[words], size * 32,
                        index * 32 + loc, clear);
  } else {
    r = index * 32 + loc;
    *clear = 1;
 ...
 *    -1 - Failure
 */
static int ba_alloc_reverse_helper(struct bitalloc *pool, int offset, int words,
                                   unsigned int size, int index, int *clear) {
  bitalloc_word_t *storage = &pool->storage[offset];
  ...
  if (pool->size > size) {
    r = ba_alloc_reverse_helper(pool, offset + words + 1, storage[words],
                                size * 32, index * 32 + loc, clear);
  } else {
    r = index * 32 + loc;
    *clear = 1;
    pool->free_count--;
...
static int ba_alloc_index_helper(struct bitalloc *pool, int offset, int words,
                                 unsigned int size, int *index, int *clear) {
  bitalloc_word_t *storage = &pool->storage[offset];
  int loc;
  int r;
  ...
  if (pool->size > size)
    r = ba_alloc_index_helper(pool, offset + words + 1, storage[words],
                              size * 32, index, clear);
  else
...
static int ba_inuse_helper(struct bitalloc *pool, int offset, int words,
  ...
  if (pool->size > size)
    r = ba_inuse_helper(pool, offset + words + 1, storage[words], size * 32,
                        index);
  else
    r = 1; /* Check if in use */
...
static int ba_free_helper(struct bitalloc *pool, int offset, int words,
                          unsigned int size, int *index) {
  bitalloc_word_t *storage = &pool->storage[offset];
  int loc;
  int r;
  ...
  if (pool->size > size)
    r = ba_free_helper(pool, offset + words + 1, storage[words], size * 32,
                       index);
  else
...
int ba_free_count(struct bitalloc *pool) { return (int)pool->free_count; }

int ba_inuse_count(struct bitalloc *pool) {
  return (int)(pool->size) - (int)(pool->free_count);
}

static int ba_find_next_helper(struct bitalloc *pool, int offset, int words,
                               unsigned int size, int *index, int free) {
  bitalloc_word_t *storage = &pool->storage[offset];
  int loc, r, bottom = 0;

  if (pool->size > size)
    r = ba_find_next_helper(pool, offset + words + 1, storage[words], size * 32,
                            index, free);
  else
...
int ba_find_next_inuse(struct bitalloc *pool, int index) {
  if (index < 0 || index >= (int)pool->size || pool->free_count == pool->size)
    return -1;

  return ba_find_next_helper(pool, 0, 1, 32, &index, 0);
}

int ba_find_next_inuse_free(struct bitalloc *pool, int index) {
  if (index < 0 || index >= (int)pool->size || pool->free_count == pool->size)
    return -1;
