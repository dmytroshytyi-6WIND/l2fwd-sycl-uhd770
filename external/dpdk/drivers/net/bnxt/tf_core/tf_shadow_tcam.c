...
#define TF_SHADOW_TCAM_HB_HANDLE_CREATE(idx, be)                               \
  ((1 << 31) | ((be) << 29) | (idx))

#define TF_SHADOW_TCAM_HB_HANDLE_BE_GET(hdl)                                   \
  (((hdl) >> 29) & (TF_SHADOW_TCAM_HB_NUM_ELEM - 1))

#define TF_SHADOW_TCAM_HB_HANDLE_HASH_GET(ctxt, hdl)                           \
  ((hdl) & (ctxt)->hash_ctxt.hid_mask)
...
#define TF_SHADOW_TCAM_IDX_TO_SHIDX(ctxt, idx)                                 \
  ((idx) - (ctxt)->shadow_ctxt.base_addr)

/* Convert the shadow index to a tcam index */
#define TF_SHADOW_TCAM_SHIDX_TO_IDX(ctxt, idx)                                 \
  ((idx) + (ctxt)->shadow_ctxt.base_addr)
 ...
 */
static int tf_shadow_tcam_key_cmp(struct tf_shadow_tcam_ctxt *ctxt,
                                  uint8_t *key, uint8_t *mask, uint16_t sh_idx,
                                  uint16_t size) {
 ...
 */
static void *tf_shadow_tcam_res_cpy(struct tf_shadow_tcam_ctxt *ctxt,
                                    uint8_t *result, uint16_t sh_idx,
                                    uint16_t size) {
  ...
  return memcpy(result, ctxt->shadow_ctxt.sh_res_tbl[sh_idx].result, size);
}
 ...
 */
static uint32_t tf_shadow_tcam_crc32_calc(uint8_t *key, uint32_t len) {
 ...
 */
static void tf_shadow_tcam_ctxt_delete(struct tf_shadow_tcam_ctxt *ctxt) {
  if (!ctxt)
    return;
 ...
 * the shadow db.
 */
static int tf_shadow_tcam_ctxt_create(struct tf_shadow_tcam_ctxt *ctxt,
                                      uint16_t num_entries,
                                      uint16_t base_addr) {
...
tf_shadow_tcam_ctxt_get(struct tf_shadow_tcam_db *shadow_db,
  ...
  if (type >= TF_TCAM_TBL_TYPE_MAX || !shadow_db || !shadow_db->ctxt[type])
    return NULL;
...
tf_shadow_tcam_set_hash_entry(struct tf_shadow_tcam_ctxt *ctxt,
                              uint32_t hb_handle, uint16_t sh_idx) {
 ...
 * shadow index.
 */
static void tf_shadow_tcam_clear_sh_entry(struct tf_shadow_tcam_ctxt *ctxt,
                                          uint16_t sh_idx) {
 ...
 * data.
 */
int tf_shadow_tcam_bind_index(struct tf_shadow_tcam_bind_index_parms *parms) {
  int rc;
  int i;
  uint16_t idx, klen;
  ...
  if (idx >= tf_shadow_tcam_sh_num_entries_get(ctxt) ||
    ...
    TFP_DRV_LOG(ERR, "%s:%s Invalid len (%d) > %d || oob idx %d\n",
                tf_dir_2_str(parms->dir), tf_tcam_tbl_2_str(parms->type), klen,
...
int tf_shadow_tcam_remove(struct tf_shadow_tcam_remove_parms *parms) {
  ...
  if (idx >= tf_shadow_tcam_sh_num_entries_get(ctxt)) {
    TFP_DRV_LOG(DEBUG, "%s %d >= %d\n", tf_tcam_tbl_2_str(fparms->type),
                fparms->idx, tf_shadow_tcam_sh_num_entries_get(ctxt));
    return 0;
  }
...
int tf_shadow_tcam_search(struct tf_shadow_tcam_search_parms *parms) {
  ...
  if (len > TF_SHADOW_TCAM_MAX_KEY_SZ || !sparms->key || !sparms->mask ||
      !len) {
    TFP_DRV_LOG(ERR, "%s:%s Invalid parms %d : %p : %p\n",
                tf_dir_2_str(sparms->dir), tf_tcam_tbl_2_str(sparms->type), len,
                sparms->key, sparms->mask);
    return -EINVAL;
  }
  ...
  for (i = 0; i < TF_SHADOW_TCAM_HB_NUM_ELEM; i++) {
    ...
    if (!tf_shadow_tcam_key_cmp(ctxt, sparms->key, sparms->mask, shtbl_key,
                                sparms->key_size)) {
      /*
      ...
      parms->hb_handle = TF_SHADOW_TCAM_HB_HANDLE_CREATE(hb_idx, i);
      sparms->idx = TF_SHADOW_TCAM_SHIDX_TO_IDX(ctxt, shtbl_key);
      sparms->ref_cnt = ctxt->shadow_ctxt.sh_res_tbl[shtbl_key].refcnt;

      /* copy the result, if caller wanted it. */
      if (rcopy && !tf_shadow_tcam_res_cpy(ctxt, sparms->result, shtbl_key,
                                           sparms->result_size)) {
        /*
  ...
  if (be_avail < TF_SHADOW_TCAM_HB_NUM_ELEM) {
    parms->hb_handle = TF_SHADOW_TCAM_HB_HANDLE_CREATE(hb_idx, be_avail);
    sparms->search_status = MISS;
    sparms->hit = 0;
    sparms->idx = 0;
  } else {
...
int tf_shadow_tcam_insert(struct tf_shadow_tcam_insert_parms *parms) {
  ...
  if (idx >= tf_shadow_tcam_sh_num_entries_get(ctxt)) {
    TFP_DRV_LOG(ERR, "%s:%s Invalid idx(0x%x)\n", tf_dir_2_str(sparms->dir),
                tf_tcam_tbl_2_str(sparms->type), sparms->idx);
    return -EINVAL;
  }
  ...
  if (sparms->result_size > TF_SHADOW_TCAM_MAX_RESULT_SZ) {
    TFP_DRV_LOG(ERR, "%s:%s Result length %d > %d\n", tf_dir_2_str(sparms->dir),
                tf_tcam_tbl_2_str(sparms->type), sparms->result_size,
...
int tf_shadow_tcam_create_db(struct tf_shadow_tcam_create_db_parms *parms) {
  ...
  *parms->shadow_db = (void *)shadow_db;
  ...
  TFP_DRV_LOG(INFO, "TF SHADOW TCAM - initialized\n");
