...
static int tf_session_create(struct tf *tfp,
  ...
  tf_dev_bind_ops(parms->open_cfg->device_type, &dev);

  /* Open FW session and get a new session_id */
  rc = tf_msg_session_open(parms->open_cfg->bp, parms->open_cfg->ctrl_chan_name,
                           &fw_session_id, &fw_session_client_id, &dev,
                           &shared_session_creator);
  if (rc) {
    /* Log error */
    if (rc == -EEXIST)
      TFP_DRV_LOG(ERR, "Session is already open, rc:%s\n", strerror(-rc));
    else
      TFP_DRV_LOG(ERR, "Open message send failed, rc:%s\n", strerror(-rc));
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "Failed to allocate session info, rc:%s\n", strerror(-rc));
    goto cleanup;
  }
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "Failed to allocate session data, rc:%s\n", strerror(-rc));
    goto cleanup;
  }
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "Failed to allocate session client, rc:%s\n",
                strerror(-rc));
    goto cleanup;
  }
  ...
  tfp_memcpy(client->ctrl_chan_name, parms->open_cfg->ctrl_chan_name,
             TF_SESSION_NAME_MAX);
  ...
  name_len = strnlen(parms->open_cfg->ctrl_chan_name, TF_SESSION_NAME_MAX);
  ...
  name =
      &parms->open_cfg->ctrl_chan_name[name_len - strlen("tf_shared-wc_tcam")];
  ...
  rc = tf_dev_bind(tfp, parms->open_cfg->device_type, session->shadow_copy,
                   &parms->open_cfg->resources, parms->open_cfg->wc_num_slices,
                   &session->dev);
  ...
  if (session->dev.ops->tf_dev_get_mailbox == NULL) {
    /* Log error */
    TFP_DRV_LOG(ERR, "No tf_dev_get_mailbox() defined for device\n");
    goto cleanup;
  }
...
cleanup:
  rc = tf_msg_session_close(tfp, fw_session_id, dev.ops->tf_dev_get_mailbox());
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "FW Session close failed, rc:%s\n", strerror(-rc));
  }
  if (tfp->session) {
...
tf_session_client_create(struct tf *tfp,
  ...
  client =
      tf_session_find_session_client_by_name(session, parms->ctrl_chan_name);
  if (client) {
    TFP_DRV_LOG(ERR, "Client %s, already registered with this session\n",
                parms->ctrl_chan_name);
    return -EOPNOTSUPP;
  }

  rc = tf_msg_session_client_register(
      tfp, session, parms->ctrl_chan_name,
      &session_client_id.internal.fw_session_client_id);
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to create client on session, rc:%s\n",
                strerror(-rc));
    return rc;
  }
  ...
  rc = tf_session_get_fw_session_id(tfp,
                                    &session_client_id.internal.fw_session_id);
  if (rc) {
    TFP_DRV_LOG(ERR, "Session Firmware id lookup failed, rc:%s\n",
                strerror(-rc));
    return rc;
  }

  tfp_memcpy(client->ctrl_chan_name, parms->ctrl_chan_name,
             TF_SESSION_NAME_MAX);
...
tf_session_client_destroy(struct tf *tfp,
  ...
  client = tf_session_get_session_client(tfs, parms->session_client_id);
  if (client == NULL) {
    TFP_DRV_LOG(ERR, "Client %d, not found within this session\n",
  ...
  rc = tf_msg_session_client_unregister(
      tfp, tfs, parms->session_client_id.internal.fw_session_client_id);
   ...
   */
  if (rc) {
    TFP_DRV_LOG(ERR, "Client destroy on FW Failed, rc:%s\n", strerror(-rc));
  }
...
int tf_session_open_session(struct tf *tfp,
  ...
  if (tfp->session == NULL) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "Failed to create session, ctrl_chan_name:%s, rc:%s\n",
                  parms->open_cfg->ctrl_chan_name, strerror(-rc));
      return rc;
    }
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "Failed to create client on session 0x%x, rc:%s\n",
                  parms->open_cfg->session_id.id, strerror(-rc));
      return rc;
    }

    TFP_DRV_LOG(INFO, "Session Client:%d registered on session:0x%8x\n",
...
int tf_session_attach_session(struct tf *tfp __rte_unused,
  ...
  TFP_DRV_LOG(ERR, "Attach not yet supported, rc:%s\n", strerror(-rc));
  return rc;
}
...
int tf_session_close_session(struct tf *tfp,
  ...
  rc = tf_session_get_session(tfp, &tfs);
  if (rc) {
    TFP_DRV_LOG(ERR, "Session lookup failed, rc:%s\n", strerror(-rc));
    return rc;
  }

  if (tfs->session_id.id == TF_SESSION_ID_INVALID) {
    rc = -EINVAL;
    TFP_DRV_LOG(ERR, "Invalid session id, unable to close, rc:%s\n",
                strerror(-rc));
    return rc;
  }
  ...
  client = tf_session_find_session_client_by_fid(tfs, fid);
  if (!client) {
    rc = -EINVAL;
  ...
  if (tfs->ref_count > 1) {
    ...
    memset(&scdparms, 0, sizeof(struct tf_session_client_destroy_parms));
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "Failed to unregister Client %d, rc:%s\n",
                  client->session_client_id.id, strerror(-rc));
      return rc;
    }

    TFP_DRV_LOG(INFO, "Closed session client, session_client_id:%d\n",
                client->session_client_id.id);

    TFP_DRV_LOG(INFO, "session_id:0x%08x, ref_count:%d\n", tfs->session_id.id,
                tfs->ref_count);
  ...
  *parms->session_id = tfs->session_id;
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Device lookup failed, rc:%s\n", strerror(-rc));
    return rc;
  }
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "Device unbind failed, rc:%s\n", strerror(-rc));
  }
  ...
  TFP_DRV_LOG(INFO, "Closed session, session_id:0x%08x, ref_count:%d\n",
...
bool tf_session_is_fid_supported(struct tf_session *tfs, uint16_t fid) {
  struct ll_entry *c_entry;
  struct tf_session_client *client;

  for (c_entry = tfs->client_ll.head; c_entry != NULL;
...
int tf_session_get_session(struct tf *tfp, struct tf_session **tfs) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Internal FID lookup\n, rc:%s\n", strerror(-rc));
    return rc;
  }

  supported = tf_session_is_fid_supported(*tfs, fw_fid);
  if (!supported) {
    TFP_DRV_LOG(ERR, "Ctrl channel not registered with session\n, rc:%s\n",
                strerror(-rc));
    return -EINVAL;
  }
...
tf_session_find_session_client_by_name(struct tf_session *tfs,
  ...
  for (c_entry = tfs->client_ll.head; c_entry != NULL;
       c_entry = c_entry->next) {
    client = (struct tf_session_client *)c_entry;
    if (strncmp(client->ctrl_chan_name, ctrl_chan_name, TF_SESSION_NAME_MAX) ==
        0)
      return client;
  }
...
struct tf_session_client *
tf_session_find_session_client_by_fid(struct tf_session *tfs, uint16_t fid) {
  struct ll_entry *c_entry;
  struct tf_session_client *client;
...
int tf_session_get_fw_session_id(struct tf *tfp, uint8_t *fw_session_id) {
  int rc;
  struct tf_session *tfs = NULL;
...
int tf_session_get_tcam_shared_db(struct tf *tfp,
...
int tf_session_set_tcam_shared_db(struct tf *tfp, void *tcam_shared_db_handle) {
  struct tf_session *tfs = NULL;
  int rc = 0;
