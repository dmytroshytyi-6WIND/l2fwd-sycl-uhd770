 ...
 */
int tfp_send_msg_direct(struct bnxt *bp, struct tfp_send_msg_parms *parms) {
  int rc = 0;
  ...
  rc = bnxt_hwrm_tf_message_direct(bp, use_kong_mb, parms->tf_type,
                                   parms->req_data, parms->req_size,
                                   parms->resp_data, parms->resp_size);
...
int tfp_calloc(struct tfp_calloc_parms *parms) {
  ...
  parms->mem_va =
      rte_zmalloc("tf", (parms->nitems * parms->size), parms->alignment);
 ...
 */
void tfp_free(void *addr) { rte_free(addr); }

/**
 * Copies n bytes from src memory to dest memory. The memory areas
 * must not overlap.
 */
void tfp_memcpy(void *dest, void *src, size_t n) { rte_memcpy(dest, src, n); }

/**
 * Used to initialize portable spin lock
 */
void tfp_spinlock_init(struct tfp_spinlock_parms *parms) {
 ...
 */
void tfp_spinlock_lock(struct tfp_spinlock_parms *parms) {
 ...
 */
void tfp_spinlock_unlock(struct tfp_spinlock_parms *parms) {
