 ...
 */
static int tf_dev_reservation_check(uint16_t count,
  ...
  for (i = 0; i < TF_DIR_MAX; i++) {
    ...
    for (j = 0; j < count; j++) {
      if ((cfg[j].cfg_type == TF_RM_ELEM_CFG_HCAPI ||
           ...
           cfg[j].cfg_type == TF_RM_ELEM_CFG_HCAPI_BA_PARENT ||
           cfg[j].cfg_type == TF_RM_ELEM_CFG_HCAPI_BA_CHILD) &&
          rm_num[j] > 0)
        cnt++;
    }
  }
 ...
 */
static int tf_dev_bind_p4(struct tf *tfp, bool shadow_copy,
  ...
  rsv_cnt = tf_dev_reservation_check(TF_IDENT_TYPE_MAX, tf_ident_p4,
  ...
  if (rsv_cnt) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "Identifier initialization failure\n");
      goto fail;
    }
  ...
  rsv_cnt = tf_dev_reservation_check(TF_TBL_TYPE_MAX, tf_tbl_p4[TF_DIR_RX],
  ...
  if (rsv_cnt) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "Table initialization failure\n");
      goto fail;
    }
  ...
  rsv_cnt = tf_dev_reservation_check(TF_TCAM_TBL_TYPE_MAX, tf_tcam_p4,
...
#endif
    if (rc) {
      TFP_DRV_LOG(ERR, "TCAM initialization failure\n");
      goto fail;
    }
    no_rsv_flag = false;
  }
  ...
  rsv_cnt = tf_dev_reservation_check(TF_EM_TBL_TYPE_MAX, em_cfg.cfg,
  ...
  if (rsv_cnt) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "EEM initialization failure\n");
      goto fail;
    }
    no_rsv_flag = false;
  }
   ...
   * EM
   */
  rsv_cnt = tf_dev_reservation_check(TF_EM_TBL_TYPE_MAX, tf_em_int_p4,
                                     (uint16_t *)resources->em_cnt);
  if (rsv_cnt) {
    em_cfg.num_elements = TF_EM_TBL_TYPE_MAX;
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "EM initialization failure\n");
      goto fail;
    }
    no_rsv_flag = false;
  }
   ...
   */
  if (no_rsv_flag) {
    TFP_DRV_LOG(ERR, "No rm reserved for any tables\n");
    return -ENOMEM;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "IF Table initialization failure\n");
    goto fail;
  }
  ...
  if (!tf_session_is_shared_session(tfs)) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "Global Cfg initialization failure\n");
      goto fail;
    }
  }
...
static int tf_dev_unbind_p4(struct tf *tfp) {
...
#endif /* TF_TCAM_SHARED */
  if (rc) {
    TFP_DRV_LOG(INFO, "Device unbind failed, TCAM\n");
    fail = true;
  }

  rc = tf_ident_unbind(tfp);
  if (rc) {
    TFP_DRV_LOG(INFO, "Device unbind failed, Identifier\n");
    fail = true;
  }

  rc = tf_tbl_unbind(tfp);
  if (rc) {
    TFP_DRV_LOG(INFO, "Device unbind failed, Table Type\n");
    fail = true;
  }

  rc = tf_em_ext_common_unbind(tfp);
  if (rc) {
    TFP_DRV_LOG(INFO, "Device unbind failed, EEM\n");
    fail = true;
  }

  rc = tf_em_int_unbind(tfp);
  if (rc) {
    TFP_DRV_LOG(INFO, "Device unbind failed, EM\n");
    fail = true;
  }
  ...
  if (!tf_session_is_shared_session(tfs)) {
    ...
    if (rc) {
      TFP_DRV_LOG(INFO, "Device unbind failed, IF Table Type\n");
      fail = true;
    }

    rc = tf_global_cfg_unbind(tfp);
    if (rc) {
      TFP_DRV_LOG(INFO, "Device unbind failed, Global Cfg Type\n");
      fail = true;
    }
  }
 ...
 *   - (-EINVAL) on parameter or internal failure.
 */
static int tf_dev_bind_p58(struct tf *tfp, bool shadow_copy,
                           struct tf_session_resources *resources,
                           struct tf_dev_info *dev_handle,
                           enum tf_wc_num_slice wc_num_slices) {
  int rc;
  ...
  rsv_cnt = tf_dev_reservation_check(TF_IDENT_TYPE_MAX, tf_ident_p58,
                                     (uint16_t *)resources->ident_cnt);
  if (rsv_cnt) {
    ident_cfg.num_elements = TF_IDENT_TYPE_MAX;
  ...
  rsv_cnt = tf_dev_reservation_check(TF_TBL_TYPE_MAX, tf_tbl_p58[TF_DIR_RX],
                                     (uint16_t *)resources->tbl_cnt);
  rsv_cnt += tf_dev_reservation_check(TF_TBL_TYPE_MAX, tf_tbl_p58[TF_DIR_TX],
                                      (uint16_t *)resources->tbl_cnt);
  if (rsv_cnt) {
    tbl_cfg.num_elements = TF_TBL_TYPE_MAX;
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "SRAM table initialization failure\n");
      goto fail;
    }
  }

  rsv_cnt = tf_dev_reservation_check(TF_TCAM_TBL_TYPE_MAX, tf_tcam_p58,
                                     (uint16_t *)resources->tcam_cnt);
  if (rsv_cnt) {
    tcam_cfg.num_elements = TF_TCAM_TBL_TYPE_MAX;
   ...
   * EM
   */
  rsv_cnt = tf_dev_reservation_check(TF_EM_TBL_TYPE_MAX, tf_em_int_p58,
                                     (uint16_t *)resources->em_cnt);
  if (rsv_cnt) {
    em_cfg.num_elements = TF_EM_TBL_TYPE_MAX;
...
static int tf_dev_unbind_p58(struct tf *tfp) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Device unbind failed, SRAM table\n");
    fail = true;
  }
  ...
  rc = tf_if_tbl_unbind(tfp);
  if (rc) {
    TFP_DRV_LOG(ERR, "Device unbind failed, IF Table Type\n");
    fail = true;
  }
  ...
  if (!tf_session_is_shared_session(tfs)) {
    rc = tf_global_cfg_unbind(tfp);
    if (rc) {
      TFP_DRV_LOG(ERR, "Device unbind failed, Global Cfg Type\n");
      fail = true;
    }
  }
...
int tf_dev_bind(struct tf *tfp __rte_unused, enum tf_device_type type,
                bool shadow_copy, struct tf_session_resources *resources,
                uint16_t wc_num_slices, struct tf_dev_info *dev_handle) {
  switch (type) {
  case TF_DEVICE_TYPE_WH:
  case TF_DEVICE_TYPE_SR:
    dev_handle->type = type;
    return tf_dev_bind_p4(tfp, shadow_copy, resources, dev_handle,
                          wc_num_slices);
  case TF_DEVICE_TYPE_THOR:
    dev_handle->type = type;
    return tf_dev_bind_p58(tfp, shadow_copy, resources, dev_handle,
                           wc_num_slices);
  default:
    TFP_DRV_LOG(ERR, "No such device\n");
    return -ENODEV;
  }
}
