...
int tf_create_tbl_pool_external(enum tf_dir dir,
                                ...
                                uint32_t num_entries, uint32_t entry_sz_bytes) {
...
void tf_destroy_tbl_pool_external(enum tf_dir dir,
                                  struct tf_tbl_scope_cb *tbl_scope_cb) {
  uint32_t *ext_act_pool_mem = tbl_scope_cb->ext_act_pool_mem[dir];
 ...
 */
struct tf_tbl_scope_cb *tf_em_ext_common_tbl_scope_find(struct tf *tfp,
  ...
  for (entry = ext_db->tbl_scope_ll.head; entry != NULL; entry = entry->next) {
 ...
 */
int tf_tbl_ext_alloc(struct tf *tfp, struct tf_tbl_alloc_parms *parms) {
  int rc;
  uint32_t index;
  struct tf_tbl_scope_cb *tbl_scope_cb;
  struct stack *pool;
  ...
  if (tbl_scope_cb == NULL) {
    TFP_DRV_LOG(ERR, "%s, table scope not allocated\n",
  ...
  if (rc != 0) {
    TFP_DRV_LOG(ERR, "%s, Allocation failed, type:%d\n",
 ...
 *  -EINVAL
 */
int tf_tbl_ext_free(struct tf *tfp, struct tf_tbl_free_parms *parms) {
  int rc = 0;
  uint32_t index;
  struct tf_tbl_scope_cb *tbl_scope_cb;
  struct stack *pool;
  ...
  tbl_scope_cb = tf_em_ext_common_tbl_scope_find(tfp, parms->tbl_scope_id);
  if (tbl_scope_cb == NULL) {
    TFP_DRV_LOG(ERR, "%s, table scope error\n", tf_dir_2_str(parms->dir));
    return -EINVAL;
  }
  pool = &tbl_scope_cb->ext_act_pool[parms->dir];
  ...
  if (rc != 0) {
    TFP_DRV_LOG(ERR, "%s, consistency error, stack full, type:%d, idx:%d\n",
                tf_dir_2_str(parms->dir), parms->type, index);
  }
  return rc;
}
 ...
 */
static uint32_t tf_em_page_tbl_pgcnt(uint32_t num_pages, uint32_t page_size) {
 ...
 *   EM page count
 */
static void tf_em_size_page_tbls(int max_lvl, uint64_t num_data_pages,
 ...
 */
static int tf_em_size_page_tbl_lvl(uint32_t page_size, uint32_t entry_size,
  ...
  *num_data_pages = 0;
  ...
  while (lvl_data_size < data_size) {
    ...
    if (lvl == TF_PT_LVL_1)
      lvl_data_size = (uint64_t)MAX_PAGE_PTRS(page_size) * page_size;
 ...
 *   - ENOMEM - Out of memory
 */
int tf_em_size_table(struct hcapi_cfa_em_table *tbl, uint32_t page_size) {
  uint64_t num_data_pages;
  uint32_t *page_cnt;
  int max_lvl;
  uint32_t num_entries;
  ...
  max_lvl = tf_em_size_page_tbl_lvl(page_size, tbl->entry_size,
                                    tbl->num_entries, &num_data_pages);
  if (max_lvl < 0) {
    TFP_DRV_LOG(WARNING, "EEM: Failed to size page table levels\n");
    TFP_DRV_LOG(WARNING, "table: %d data-sz: %016" PRIu64 " page-sz: %u\n",
                tbl->type, (uint64_t)num_entries * tbl->entry_size, page_size);
    return -ENOMEM;
  }
  ...
  tf_em_size_page_tbls(max_lvl, num_data_pages, page_size, page_cnt);
  ...
  TFP_DRV_LOG(INFO,
              ...
              page_cnt[TF_PT_LVL_0], page_cnt[TF_PT_LVL_1],
...
int tf_em_validate_num_entries(struct tf_tbl_scope_cb *tbl_scope_cb,
  ...
  if (parms->rx_mem_size_in_mb != 0) {
    ...
    uint32_t action_b = ((parms->rx_max_action_entry_sz_in_bits / 8) + 1);
    uint32_t num_entries =
        (parms->rx_mem_size_in_mb * TF_MEGABYTE) / (key_b + action_b);

    if (num_entries < TF_EM_MIN_ENTRIES) {
      TFP_DRV_LOG(ERR,
                  "EEM: Insufficient memory requested:"
                  "%uMB\n",
    ...
    if (cnt > TF_EM_MAX_ENTRIES) {
      TFP_DRV_LOG(ERR,
                  "EEM: Invalid number of Tx requested: "
                  "%u\n",
    ...
    if ((parms->rx_num_flows_in_k * TF_KILOBYTE) < TF_EM_MIN_ENTRIES ||
    ...
    if (cnt > TF_EM_MAX_ENTRIES) {
      TFP_DRV_LOG(ERR, "EEM: Invalid number of Rx requested: %u\n",
                  (parms->rx_num_flows_in_k * TF_KILOBYTE));
      return -EINVAL;
    }
  }
  ...
  if (parms->tx_mem_size_in_mb != 0) {
    ...
    uint32_t action_b = ((parms->tx_max_action_entry_sz_in_bits / 8) + 1);
    uint32_t num_entries =
        (parms->tx_mem_size_in_mb * (TF_KILOBYTE * TF_KILOBYTE)) /
        (key_b + action_b);

    if (num_entries < TF_EM_MIN_ENTRIES) {
      TFP_DRV_LOG(ERR, "EEM: Insufficient memory requested:%uMB\n",
                  parms->rx_mem_size_in_mb);
      return -EINVAL;
    }
    ...
    if (cnt > TF_EM_MAX_ENTRIES) {
      TFP_DRV_LOG(ERR, "EEM: Invalid number of Tx requested: %u\n",
                  (parms->tx_num_flows_in_k * TF_KILOBYTE));
      return -EINVAL;
    }
    ...
    if ((parms->tx_num_flows_in_k * TF_KILOBYTE) < TF_EM_MIN_ENTRIES ||
        (parms->tx_num_flows_in_k * TF_KILOBYTE) >
  ...
  if (parms->rx_num_flows_in_k != 0 && parms->rx_max_key_sz_in_bits / 8 == 0) {
    TFP_DRV_LOG(ERR, "EEM: Rx key size required: %u\n",
                (parms->rx_max_key_sz_in_bits));
    return -EINVAL;
  }

  if (parms->tx_num_flows_in_k != 0 && parms->tx_max_key_sz_in_bits / 8 == 0) {
    TFP_DRV_LOG(ERR, "EEM: Tx key size required: %u\n",
 ...
 */
static int tf_insert_eem_entry(struct tf_dev_info *dev,
                               struct tf_tbl_scope_cb *tbl_scope_cb,
  ...
  mask = tf_em_get_key_mask(tbl_scope_cb->em_ctx_info[parms->dir]
                                .em_tables[TF_KEY0_TABLE]
                                .num_entries);
  ...
  key_tbl.base0 = (uint8_t *)&tbl_scope_cb->em_ctx_info[parms->dir]
                      .em_tables[TF_KEY0_TABLE];
  ...
  rc = cfa_p4_devops.hcapi_cfa_key_hw_op(&op, &key_tbl, &key_obj, &key_loc);
  ...
  TF_SET_GFID(gfid, index, table_type);
  TF_SET_FLOW_ID(parms->flow_id, gfid, TF_GFID_TABLE_EXTERNAL, parms->dir);
  TF_SET_FIELDS_IN_FLOW_HANDLE(parms->flow_handle, 0, 0, 0, index, 0,
                               table_type);
...
static int tf_delete_eem_entry(struct tf_tbl_scope_cb *tbl_scope_cb,
  ...
  key_tbl.base0 =
      (uint8_t *)&tbl_scope_cb->em_ctx_info[parms->dir]
          .em_tables[(hash_type == 0 ? TF_KEY0_TABLE : TF_KEY1_TABLE)];
  key_tbl.page_size = TF_EM_PAGE_SIZE;
  key_obj.offset = index * TF_P4_EM_KEY_RECORD_SIZE;
  key_obj.data = NULL;
  key_obj.size = TF_P4_EM_KEY_RECORD_SIZE;
...
int tf_em_insert_ext_entry(struct tf *tfp,
  ...
  return tf_insert_eem_entry(dev, tbl_scope_cb, parms);
}
...
int tf_em_ext_common_bind(struct tf *tfp, struct tf_em_cfg_parms *parms) {
  int rc;
  int i;
  ...
  for (i = 0; i < TF_DIR_MAX; i++) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: EM Ext DB creation failed\n", tf_dir_2_str(i));
...
int tf_em_ext_common_unbind(struct tf *tfp) {
  ...
  rc = tf_session_get_em_ext_db(tfp, &ext_ptr);
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to get em_ext_db from session, rc:%s\n",
                strerror(-rc));
    return rc;
  }
  ext_db = (struct em_ext_db *)ext_ptr;
 ...
 *   - (-EINVAL) on failure.
 */
int tf_tbl_ext_common_set(struct tf *tfp, struct tf_tbl_set_parms *parms) {
  int rc = 0;
  struct tf_tbl_scope_cb *tbl_scope_cb;
  uint32_t tbl_scope_id;
  struct hcapi_cfa_hwop op;
  ...
  if (parms->data == NULL) {
    TFP_DRV_LOG(ERR, "%s, invalid parms->data\n", tf_dir_2_str(parms->dir));
    return -EINVAL;
  }
...
int tf_em_ext_map_tbl_scope(struct tf *tfp,
  ...
  if (dev->ops->tf_dev_map_tbl_scope == NULL) {
    rc = -EOPNOTSUPP;
    TFP_DRV_LOG(ERR, "Map table scope operation not supported, rc:%s\n",
                strerror(-rc));
    return rc;
  }
  ...
  rc =
      dev->ops->tf_dev_map_parif(tfp, parms->parif_bitmask, tbl_scope_cb->pf,
                                 (uint8_t *)data, (uint8_t *)mask, sz_in_bytes);

  if (rc) {
    TFP_DRV_LOG(ERR, "Map table scope config failure, rc:%s\n", strerror(-rc));
    goto cleaner;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Map tbl scope, set failed, rc:%s\n", strerror(-rc));
  }
cleaner:
  tfp_free(mask);
clean:
