 ...
 */
static const char *tf_pool_2_str(enum tf_tcam_shared_wc_pool_id id) {
  switch (id) {
 ...
 */
/* struct tf_tcam_shared_wc_pool
 * tcam_shared_wc[TF_DIR_MAX][TF_TCAM_SHARED_WC_POOL_MAX]; */

static int tf_tcam_shared_create_db(struct tf_tcam_shared_wc_pools **db) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "TCAM shared db allocation failed (%s)\n", strerror(-rc));
    return rc;
  }
  *db = cparms.mem_va;
 ...
 */
static int
tf_tcam_shared_create_wc_pool(int dir, enum tf_tcam_shared_wc_pool_id id,
  ...
  rc = tfp_calloc(&cparms);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: pool memory alloc failed %s:%s\n", tf_dir_2_str(dir),
                tf_pool_2_str(id), strerror(-rc));
    return rc;
  }
  tcam_shared_wc->db[dir][id].pool = (struct bitalloc *)cparms.mem_va;

  rc = ba_init(tcam_shared_wc->db[dir][id].pool, stride, free);

  if (rc) {
    TFP_DRV_LOG(ERR, "%s: pool bitalloc failed %s\n", tf_dir_2_str(dir),
                tf_pool_2_str(id));
    return rc;
  }
 ...
 */
static int
tf_tcam_shared_free_wc_pool(int dir, enum tf_tcam_shared_wc_pool_id id,
                            struct tf_tcam_shared_wc_pools *tcam_shared_wc) {
  int rc = 0;
 ...
 */
static int tf_tcam_shared_get_slices(struct tf *tfp, struct tf_dev_info *dev,
                                     uint16_t *num_slices) {
  int rc;

  if (dev->ops->tf_dev_get_tcam_slice_info == NULL) {
    rc = -EOPNOTSUPP;
    TFP_DRV_LOG(ERR, "Operation not supported, rc:%s\n", strerror(-rc));
    return rc;
  }
  rc = dev->ops->tf_dev_get_tcam_slice_info(tfp, TF_TCAM_TBL_TYPE_WC_TCAM, 0,
                                            num_slices);
  return rc;
}

static bool tf_tcam_db_valid(struct tf *tfp, enum tf_dir dir) {
...
static int tf_tcam_shared_get_rm_info(struct tf *tfp, enum tf_dir dir,
                                      uint16_t *hcapi_type,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Get RM hcapi type failed %s\n", tf_dir_2_str(dir),
                strerror(-rc));
    return rc;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: TCAM rm info get failed %s\n", tf_dir_2_str(dir),
                strerror(-rc));
    return rc;
  }
  return rc;
...
int tf_tcam_shared_bind(struct tf *tfp, struct tf_tcam_cfg_parms *parms) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Session access failure: %s\n", strerror(-rc));
    return rc;
  }
  if (tf_session_is_shared_session(tfs)) {
    ...
    for (dir = 0; dir < TF_DIR_MAX; dir++) {
      ...
      rc = tf_tcam_shared_get_rm_info(tfp, dir, &hcapi_type, &info);
      if (rc) {
        TFP_DRV_LOG(ERR, "%s: TCAM rm info get failed\n", tf_dir_2_str(dir));
        goto done;
      }
      ...
      tf_tcam_shared_create_wc_pool(dir, TF_TCAM_SHARED_WC_POOL_HI, start,
                                    stride, tcam_shared_wc);

      start += stride;
      tf_tcam_shared_create_wc_pool(dir, TF_TCAM_SHARED_WC_POOL_LO, start,
                                    stride, tcam_shared_wc);
    ...
    if (num_slices > 1) {
      TFP_DRV_LOG(ERR, "Only single slice supported\n");
      return -EOPNOTSUPP;
    }
  }
done:
...
int tf_tcam_shared_unbind(struct tf *tfp) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to get tcam_shared_db, rc:%s\n", strerror(-rc));
    return rc;
  }
  ...
  for (dir = 0; dir < TF_DIR_MAX; dir++) {
    ...
    rc = tf_tcam_shared_get_rm_info(tfp, dir, &hcapi_type, &info);
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: TCAM shared rm info get failed\n",
                  tf_dir_2_str(dir));
      return rc;
    }

    for (pool_id = TF_TCAM_SHARED_WC_POOL_HI;
         pool_id < TF_TCAM_SHARED_WC_POOL_MAX; pool_id++) {
      pool = tcam_shared_wc->db[dir][pool_id].pool;
      start = tcam_shared_wc->db[dir][pool_id].info.start;
      pool_cnt = ba_inuse_count(pool);

      if (pool_cnt) {
        TFP_DRV_LOG(INFO, "%s: %s: %d residuals found, freeing\n",
                    tf_dir_2_str(dir), tf_pool_2_str(pool_id), pool_cnt);
      }
      ...
      for (i = 0; i < pool_cnt; i++) {
        ...
        if (log_idx < 0) {
          TFP_DRV_LOG(ERR, "Expected a found %s entry %d\n",
                      tf_pool_2_str(pool_id), i);
        ...
        if (rc) {
          /* Log error */
          TFP_DRV_LOG(ERR, "%s: %s: %d free failed, rc:%s\n",
                      tf_dir_2_str(parms.dir), tf_tcam_tbl_2_str(parms.type),
                      phy_idx, strerror(-rc));
          return rc;
        }
      }
       ...
       * have been cleared
       */
      tf_tcam_shared_free_wc_pool(dir, pool_id, tcam_shared_wc);
    }
  }
done:
  rc = tf_tcam_unbind(tfp);
 ...
 */
int tf_tcam_shared_alloc(struct tf *tfp, struct tf_tcam_alloc_parms *parms) {
  int rc;
  struct tf_session *tfs;
  struct tf_dev_info *dev;
  int log_idx;
  ...
  if (!tf_tcam_db_valid(tfp, parms->dir)) {
    TFP_DRV_LOG(ERR, "%s: tcam shared pool doesn't exist\n",
  ...
  rc = tf_session_get_tcam_shared_db(tfp, (void *)&tcam_shared_db_ptr);
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to get tcam_shared_db from session, rc:%s\n",
                strerror(-rc));
    return rc;
  }
  tcam_shared_wc = (struct tf_tcam_shared_wc_pools *)tcam_shared_db_ptr;
...
int tf_tcam_shared_free(struct tf *tfp, struct tf_tcam_free_parms *parms) {
  int rc;
  struct tf_session *tfs;
  struct tf_dev_info *dev;
  int allocated = 0;
  ...
  rc = tf_tcam_shared_get_rm_info(tfp, parms->dir, &hcapi_type, &info);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: TCAM rm info get failed\n", tf_dir_2_str(parms->dir));
    return rc;
  }
  ...
  if (allocated != TF_RM_ALLOCATED_ENTRY_IN_USE) {
    TFP_DRV_LOG(ERR, "%s: Entry already free, type:%d, idx:%d\n",
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Free failed, type:%s, idx:%d\n",
                tf_dir_2_str(parms->dir), tf_tcam_tbl_2_str(parms->type),
                parms->idx);
    return rc;
  }
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "%s: %s: log%d free failed, rc:%s\n",
                tf_dir_2_str(nparms.dir), tf_tcam_tbl_2_str(nparms.type),
                phy_idx, strerror(-rc));
    return rc;
  }
  return 0;
...
int tf_tcam_shared_set(struct tf *tfp __rte_unused,
  ...
  if (allocated != TF_RM_ALLOCATED_ENTRY_IN_USE) {
    TFP_DRV_LOG(ERR, "%s: Entry is not allocated, type:%d, logid:%d\n",
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "%s: %s: phy entry %d set failed, rc:%s",
                tf_dir_2_str(parms->dir), tf_tcam_tbl_2_str(nparms.type),
                phy_idx, strerror(-rc));
    return rc;
  }
  return 0;
...
int tf_tcam_shared_get(struct tf *tfp __rte_unused,
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "%s: %s: Entry %d set failed, rc:%s",
                tf_dir_2_str(nparms.dir), tf_tcam_tbl_2_str(nparms.type),
                nparms.idx, strerror(-rc));
    return rc;
  }
  return 0;
}
 ...
 */
static int tf_tcam_shared_move_entry(struct tf *tfp, struct tf_dev_info *dev,
                                     uint16_t hcapi_type, enum tf_dir dir,
                                     int sphy_idx, int dphy_idx,
                                     int key_sz_bytes, int remap_sz_bytes,
                                     bool set_enable_bit) {
  int rc = 0;
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "%s: %s: phyid(%d) get failed, rc:%s\n",
                tf_tcam_tbl_2_str(gparms.type), tf_dir_2_str(dir), gparms.idx,
                strerror(-rc));
    return rc;
  }
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "%s: %s phyid(%d/0x%x) set failed, rc:%s\n",
                tf_tcam_tbl_2_str(sparms.type), tf_dir_2_str(dir), sparms.idx,
                sparms.idx, strerror(-rc));
    return rc;
  }
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "%s: %s: phyid(%d/0x%x) free failed, rc:%s\n",
                tf_dir_2_str(dir), tf_tcam_tbl_2_str(fparms.type), sphy_idx,
                sphy_idx, strerror(-rc));
    return rc;
  }
  return rc;
}
 ...
 */
static int tf_tcam_shared_move(struct tf *tfp,
  ...
  if (!tf_session_is_shared_session(tfs) ||
    ...
    TFP_DRV_LOG(ERR, "%s: Session must be shared with HI/LO type\n",
                tf_dir_2_str(parms->dir));
    return -EOPNOTSUPP;
  }
  ...
  for (i = 0; i < hi_cnt; i++) {
    ...
    if (ba_alloc_index(lo_pool, log_idx) < 0) {
      TFP_DRV_LOG(ERR, "Warning %s index %d already allocated\n",
                  tf_pool_2_str(lo_id), i);

      /* Since already allocated, continue with move
       */
    }

    rc = tf_tcam_shared_move_entry(
        tfp, dev, hcapi_type, parms->dir, hi_start + log_idx,
        lo_start + log_idx, key_sz_bytes, remap_sz_bytes, set_enable_bit);
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: Move error %s to %s index %d\n",
                  tf_dir_2_str(parms->dir), tf_pool_2_str(hi_id),
                  tf_pool_2_str(lo_id), i);
      goto done;
    }
...
int tf_tcam_shared_move_p4(struct tf *tfp,
                           struct tf_move_tcam_shared_entries_parms *parms) {
  int rc = 0;
  rc = tf_tcam_shared_move(tfp, parms, TF_TCAM_SHARED_KEY_SLICE_SZ_BYTES_P4,
...
int tf_tcam_shared_move_p58(struct tf *tfp,
                            struct tf_move_tcam_shared_entries_parms *parms) {
  int rc = 0;
  rc = tf_tcam_shared_move(tfp, parms, TF_TCAM_SHARED_KEY_SLICE_SZ_BYTES_P58,
...
int tf_tcam_shared_clear(struct tf *tfp,
  ...
  TFP_DRV_LOG(DEBUG, "%s: TCAM shared clear pool(%s)\n",
              tf_dir_2_str(nparms.dir), tf_pool_2_str(id));
  return 0;
}
