...
#define BITALLOC_SIZEOF(size)                                                  \
  (sizeof(struct bitalloc) *                                                   \
   (((sizeof(struct bitalloc) + sizeof(struct bitalloc) - 1 +                  \
      (sizeof(bitalloc_word_t) *                                               \
       ((BA_L0(size) - 1) + ((BA_L0(size) == 1) ? 0 : (BA_L1(size) + 1)) +     \
...
#define BITALLOC_INST(name, size)                                              \
  struct bitalloc name[(BITALLOC_SIZEOF(size) / sizeof(struct bitalloc))]
