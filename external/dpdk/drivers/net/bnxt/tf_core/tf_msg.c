 ...
 */
static int tf_msg_alloc_dma_buf(struct tf_msg_dma_buf *buf, int size) {
 ...
 *   Pointer to DMA buffer structure
 */
static void tf_msg_free_dma_buf(struct tf_msg_dma_buf *buf) {
...
int tf_msg_session_open(struct bnxt *bp, char *ctrl_chan_name,
                        uint8_t *fw_session_id, uint8_t *fw_session_client_id,
                        struct tf_dev_info *dev, bool *shared_session_creator) {
  int rc;
  ...
  if (!strncmp(tcam_session_name, "tf_shared-wc_tcam",
               strlen("tf_shared-wc_tcam")))
    tfp_memcpy(&req.session_name, tcam_session_name,
  ...
  rc = tfp_send_msg_direct(bp, &parms);
  if (rc)
    return rc;

  *fw_session_id = (uint8_t)tfp_le_to_cpu_32(resp.fw_session_id);
  *fw_session_client_id = (uint8_t)tfp_le_to_cpu_32(resp.fw_session_client_id);
  *shared_session_creator = (bool)tfp_le_to_cpu_32(
      resp.flags & HWRM_TF_SESSION_OPEN_OUTPUT_FLAGS_SHARED_SESSION_CREATOR);

  return rc;
}

int tf_msg_session_attach(struct tf *tfp __rte_unused,
                          char *ctrl_chan_name __rte_unused,
                          uint8_t tf_fw_session_id __rte_unused) {
  return -1;
}

int tf_msg_session_client_register(struct tf *tfp, struct tf_session *tfs,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to lookup device, rc:%s\n", strerror(-rc));
    return rc;
  }

  rc = tf_session_get_fw_session_id(tfp, &fw_session_id);
  if (rc) {
    TFP_DRV_LOG(ERR, "Unable to lookup FW id, rc:%s\n", strerror(-rc));
    return rc;
  }
  ...
  tcam_session_name =
      &ctrl_channel_name[name_len - strlen("tf_shared-wc_tcam")];
  if (!strncmp(tcam_session_name, "tf_shared-wc_tcam",
               strlen("tf_shared-wc_tcam")))
    tfp_memcpy(&req.session_client_name, tcam_session_name,
               TF_TCAM_SHARED_SESSION_NAME_LEN);
  else if (!strncmp(session_name, "tf_shared", strlen("tf_shared")))
    tfp_memcpy(&req.session_client_name, session_name,
               TF_SHARED_SESSION_NAME_LEN);
  else
    tfp_memcpy(&req.session_client_name, ctrl_channel_name,
               TF_SESSION_NAME_MAX);
...
int tf_msg_session_client_unregister(struct tf *tfp, struct tf_session *tfs,
                                     uint8_t fw_session_client_id) {
  int rc;
...
int tf_msg_session_close(struct tf *tfp, uint8_t fw_session_id, int mailbox) {
  int rc;
...
int tf_msg_session_qcfg(struct tf *tfp) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to lookup session, rc:%s\n", strerror(-rc));
    return rc;
  }
  ...
  parms.tf_type = HWRM_TF_SESSION_QCFG, parms.req_data = (uint32_t *)&req;
  parms.req_size = sizeof(req);
  parms.resp_data = (uint32_t *)&resp;
  parms.resp_size = sizeof(resp);
  parms.mailbox = dev->ops->tf_dev_get_mailbox();
...
int tf_msg_session_resc_qcaps(struct tf *tfp, struct tf_dev_info *dev,
                              enum tf_dir dir, uint16_t size,
  ...
  if (tfp_le_to_cpu_32(resp.size) != size) {
    TFP_DRV_LOG(
        ...
        tf_dir_2_str(dir), strerror(EINVAL), size, resp.size);
  }
...
int tf_msg_session_resc_alloc(struct tf *tfp, struct tf_dev_info *dev,
                              enum tf_dir dir, uint16_t size,
  ...
  rc = tf_session_get_fw_session_id(tfp, &fw_session_id);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Unable to lookup FW id, rc:%s\n", tf_dir_2_str(dir),
                strerror(-rc));
    return rc;
  }
   ...
   */
  if (tfp_le_to_cpu_32(resp.size) != size) {
    TFP_DRV_LOG(ERR, "%s: Alloc message size error, rc:%s\n", tf_dir_2_str(dir),
                strerror(EINVAL));
    rc = -EINVAL;
    goto cleanup;
  }
...
int tf_msg_session_resc_info(struct tf *tfp, struct tf_dev_info *dev,
                             enum tf_dir dir, uint16_t size,
                             struct tf_rm_resc_req_entry *request,
                             struct tf_rm_resc_entry *resv) {
  int rc;
...
int tf_msg_session_resc_flush(struct tf *tfp, enum tf_dir dir, uint16_t size,
                              struct tf_rm_resc_entry *resv) {
  int rc;
  int i;
  struct tfp_send_msg_parms parms = {0};
  ...
  rc = tf_session_get_session_internal(tfp, &tfs);
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed to lookup session, rc:%s\n", tf_dir_2_str(dir),
                strerror(-rc));
    return rc;
  }
...
int tf_msg_insert_em_internal_entry(struct tf *tfp,
                                    ...
                                    uint16_t *rptr_index, uint8_t *rptr_entry,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed to lookup session, rc:%s\n",
                tf_dir_2_str(em_parms->dir), strerror(-rc));
    return rc;
  }
  ...
  tfp_memcpy(req.em_key, em_parms->key, msg_key_size);

  flags = (em_parms->dir == TF_DIR_TX ? HWRM_TF_EM_INSERT_INPUT_FLAGS_DIR_TX
...
int tf_msg_hash_insert_em_internal_entry(
    struct tf *tfp, struct tf_insert_em_entry_parms *em_parms,
    uint32_t key0_hash, uint32_t key1_hash, uint16_t *rptr_index,
    uint8_t *rptr_entry, uint8_t *num_of_entries) {
  int rc;
  struct tfp_send_msg_parms parms = {0};
  ...
  tfp_memcpy((char *)req.em_record, em_parms->em_record, msg_record_size);
...
int tf_msg_delete_em_entry(struct tf *tfp,
  ...
  flags = (em_parms->dir == TF_DIR_TX ? HWRM_TF_EM_DELETE_INPUT_FLAGS_DIR_TX
...
int tf_msg_ext_em_ctxt_mem_alloc(struct tf *tfp, struct hcapi_cfa_em_table *tbl,
                                 uint64_t *dma_addr, uint32_t *page_lvl,
                                 uint32_t *page_size) {
  struct tfp_send_msg_parms parms = {0};
...
int tf_msg_ext_em_ctxt_mem_free(struct tf *tfp, uint32_t mem_size_k,
                                uint64_t dma_addr, uint8_t page_level,
                                uint8_t page_size) {
  struct tfp_send_msg_parms parms = {0};
...
int tf_msg_em_mem_rgtr(struct tf *tfp, int page_lvl, int page_size,
                       uint64_t dma_addr, uint16_t *ctx_id) {
  int rc;
...
int tf_msg_em_mem_unrgtr(struct tf *tfp, uint16_t *ctx_id) {
  int rc;
...
int tf_msg_em_qcaps(struct tf *tfp, int dir, struct tf_em_caps *em_caps) {
  int rc;
  ...
  em_caps->max_entries_supported = tfp_le_to_cpu_32(resp.max_entries_supported);
  em_caps->key_entry_size = tfp_le_to_cpu_16(resp.key_entry_size);
  em_caps->record_entry_size = tfp_le_to_cpu_16(resp.record_entry_size);
  em_caps->efc_entry_size = tfp_le_to_cpu_16(resp.efc_entry_size);

  return rc;
}

int tf_msg_em_cfg(struct tf *tfp, uint32_t num_entries, uint16_t key0_ctx_id,
                  uint16_t key1_ctx_id, uint16_t record_ctx_id,
                  uint16_t efc_ctx_id, uint8_t flush_interval, int dir) {
  int rc;
...
int tf_msg_ext_em_cfg(struct tf *tfp, struct tf_tbl_scope_cb *tbl_scope_cb,
                      uint32_t st_buckets, uint8_t flush_interval,
                      enum tf_dir dir) {
...
int tf_msg_em_op(struct tf *tfp, int dir, uint16_t op) {
  int rc;
...
int tf_msg_tcam_entry_set(struct tf *tfp, struct tf_dev_info *dev,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Unable to lookup FW id, rc:%s\n",
                tf_dir_2_str(parms->dir), strerror(-rc));
    return rc;
  }
  ...
  if (data_size <= TF_PCI_BUF_SIZE_MAX) {
    ...
    if (rc)
      goto cleanup;
    data = buf.va_addr;
    tfp_memcpy(&req.dev_data[0], &buf.pa_addr, sizeof(buf.pa_addr));
  }
  ...
  rc = tfp_send_msg_direct(tf_session_get_bp(tfp), &mparms);
...
int tf_msg_tcam_entry_get(struct tf *tfp, struct tf_dev_info *dev,
  ...
  if (parms->key_size < resp.key_size ||
    ...
    rc = -EINVAL;
    TFP_DRV_LOG(ERR, "%s: Key buffer(%d) is smaller than the key(%d), rc:%s\n",
                tf_dir_2_str(parms->dir), parms->key_size, resp.key_size,
                strerror(-rc));
    return rc;
  }
  ...
  tfp_memcpy(parms->result, &resp.dev_data[resp.result_offset],
             resp.result_size);

  return 0;
}

int tf_msg_tcam_entry_free(struct tf *tfp, struct tf_dev_info *dev,
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Unable to lookup FW id, rc:%s\n",
                tf_dir_2_str(in_parms->dir), strerror(-rc));
    return rc;
  }
...
int tf_msg_set_tbl_entry(struct tf *tfp, enum tf_dir dir, uint16_t hcapi_type,
                         uint16_t size, uint8_t *data, uint32_t index) {
  int rc;
...
int tf_msg_get_tbl_entry(struct tf *tfp, enum tf_dir dir, uint16_t hcapi_type,
                         uint16_t size, uint8_t *data, uint32_t index,
                         bool clear_on_read) {
  int rc;
  ...
  flags = (dir == TF_DIR_TX ? HWRM_TF_TBL_TYPE_GET_INPUT_FLAGS_DIR_TX
   ...
   */
  tfp_memcpy(data, &resp.data, size);
...
int tf_msg_get_global_cfg(struct tf *tfp, struct tf_global_cfg_parms *params) {
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "%s: Failed to lookup session, rc:%s\n",
                tf_dir_2_str(params->dir), strerror(-rc));
    return rc;
  }
  ...
  flags =
      (params->dir == TF_DIR_TX ? HWRM_TF_GLOBAL_CFG_GET_INPUT_FLAGS_DIR_TX
...
int tf_msg_set_global_cfg(struct tf *tfp, struct tf_global_cfg_parms *params) {
  ...
  flags =
      (params->dir == TF_DIR_TX ? HWRM_TF_GLOBAL_CFG_SET_INPUT_FLAGS_DIR_TX
  ...
  tfp_memcpy(req.data, params->config, params->config_sz_in_bytes);

  /* Only set mask if pointer is provided
   */
  if (params->config_mask) {
    tfp_memcpy(req.mask, params->config_mask, params->config_sz_in_bytes);
  }
...
int tf_msg_bulk_get_tbl_entry(struct tf *tfp, enum tf_dir dir,
                              uint16_t hcapi_type, uint32_t starting_idx,
                              uint16_t num_entries, uint16_t entry_sz_in_bytes,
                              uint64_t physical_mem_addr, bool clear_on_read) {
  int rc;
  struct tfp_send_msg_parms parms = {0};
  ...
  flags = (dir == TF_DIR_TX ? HWRM_TF_TBL_TYPE_BULK_GET_INPUT_FLAGS_DIR_TX
...
int tf_msg_get_if_tbl_entry(struct tf *tfp,
  ...
  flags = (params->dir == TF_DIR_TX ? HWRM_TF_IF_TBL_GET_INPUT_FLAGS_DIR_TX
                                    : HWRM_TF_IF_TBL_GET_INPUT_FLAGS_DIR_RX);

  /* Populate the request */
  req.fw_session_id = tfp_cpu_to_le_32(tfs->session_id.internal.fw_session_id);
  req.flags = flags;
...
int tf_msg_set_if_tbl_entry(struct tf *tfp,
  ...
  flags = (params->dir == TF_DIR_TX ? HWRM_TF_IF_TBL_SET_INPUT_FLAGS_DIR_TX
...

{
  ...
  /* Populate the request */
  parms.tf_type = HWRM_TF_VERSION_GET, parms.req_data = (uint32_t *)&req;
  parms.req_size = sizeof(req);
  parms.resp_data = (uint32_t *)&resp;
  parms.resp_size = sizeof(resp);
  parms.mailbox = dev->ops->tf_dev_get_mailbox();
  ...
  dev->ops->tf_dev_map_hcapi_caps(resp.dev_caps_cfg, &params->dev_ident_caps,
                                  &params->dev_tcam_caps, &params->dev_tbl_caps,
                                  &params->dev_em_caps);
