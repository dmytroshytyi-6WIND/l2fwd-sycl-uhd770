...
int tf_em_insert_int_entry(struct tf *tfp,
  ...
  if (index == DP_INVALID_INDEX) {
    PMD_DRV_LOG(ERR, "%s, EM entry index allocation failed\n",
                tf_dir_2_str(parms->dir));
    return -1;
  }

  rptr_index = index;
  rc = tf_msg_insert_em_internal_entry(tfp, parms, &rptr_index, &rptr_entry,
                                       &num_of_entries);
  if (rc) {
  ...
  TF_SET_GFID(gfid, ((rptr_index << TF_EM_INTERNAL_INDEX_SHIFT) | rptr_entry),
              0); /* N/A for internal table */

  TF_SET_FLOW_ID(parms->flow_id, gfid, TF_GFID_TABLE_INTERNAL, parms->dir);

  TF_SET_FIELDS_IN_FLOW_HANDLE(parms->flow_handle, (uint32_t)num_of_entries, 0,
                               TF_FLAGS_FLOW_HANDLE_INTERNAL, rptr_index,
                               rptr_entry, 0);
  return 0;
}
...
static int tf_em_move_callback(void *user_data, uint64_t entry_data,
                               uint32_t new_index) {
  int rc;
...
int tf_em_int_bind(struct tf *tfp, struct tf_em_cfg_parms *parms) {
  ...
  for (i = 0; i < TF_DIR_MAX; i++) {
    ...
    if (db_cfg.alloc_cnt[TF_EM_TBL_TYPE_EM_RECORD] % TF_SESSION_EM_ENTRY_SIZE !=
        0) {
      rc = -ENOMEM;
      TFP_DRV_LOG(ERR,
                  "%s, EM Allocation must be in blocks of %d, failure %s\n",
                  tf_dir_2_str(i), TF_SESSION_EM_ENTRY_SIZE, strerror(-rc));
  ...
  if (!tf_session_is_shared_session(tfs)) {
    for (i = 0; i < TF_DIR_MAX; i++) {
      ...
      if (rc) {
        TFP_DRV_LOG(ERR, "%s: EM DB get info failed\n", tf_dir_2_str(i));
        return rc;
      }
      ...
      if (rc) {
        TFP_DRV_LOG(ERR, "%s, EM stack allocation failure %s\n",
                    tf_dir_2_str(i), strerror(-rc));
        return rc;
      }

      tfs->em_pool[i] = (struct dpool *)cparms.mem_va;

      rc = dpool_init(tfs->em_pool[i], iparms.info->entry.start,
                      iparms.info->entry.stride, 7, (void *)tfp,
                      tf_em_move_callback);
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: EM pool init failed\n", tf_dir_2_str(i));
      return rc;
    }
  }
