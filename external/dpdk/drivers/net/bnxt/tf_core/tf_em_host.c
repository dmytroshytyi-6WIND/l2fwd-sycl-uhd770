 ...
 */
static void tf_em_free_pg_tbl(struct hcapi_cfa_em_page_tbl *tp) {
  uint32_t i;
 ...
 */
static void tf_em_free_page_table(struct hcapi_cfa_em_table *tbl) {
  struct hcapi_cfa_em_page_tbl *tp;
  int i;

  for (i = 0; i < tbl->num_lvl; i++) {
    tp = &tbl->pg_tbl[i];
    TFP_DRV_LOG(INFO, "EEM: Freeing page table: size %u lvl %d cnt %u\n",
                TF_EM_PAGE_SIZE, i, tp->pg_count);
 ...
 */
static int tf_em_alloc_pg_tbl(struct hcapi_cfa_em_page_tbl *tp,
 ...
 *   -ENOMEM - Out of memory
 */
static int tf_em_alloc_page_table(struct hcapi_cfa_em_table *tbl) {
  struct hcapi_cfa_em_page_tbl *tp;
  int rc = 0;
  int i;
  uint32_t j;
  ...
  for (i = 0; i < tbl->num_lvl; i++) {
    ...
    rc = tf_em_alloc_pg_tbl(tp, tbl->page_cnt[i], TF_EM_PAGE_SIZE);
    if (rc) {
      TFP_DRV_LOG(WARNING, "Failed to allocate page table: lvl: %d, rc:%s\n", i,
                  strerror(-rc));
      goto cleanup;
    }
 ...
 */
static void tf_em_link_page_table(struct hcapi_cfa_em_page_tbl *tp,
 ...
 */
static void tf_em_setup_page_table(struct hcapi_cfa_em_table *tbl) {
  struct hcapi_cfa_em_page_tbl *tp_next;
  struct hcapi_cfa_em_page_tbl *tp;
  bool set_pte_last = 0;
  int i;
 ...
 */
static void tf_em_ctx_unreg(struct tf *tfp,
                            struct tf_tbl_scope_cb *tbl_scope_cb, int dir) {
 ...
 *   -ENOMEM - Out of Memory
 */
static int tf_em_ctx_reg(struct tf *tfp, struct tf_tbl_scope_cb *tbl_scope_cb,
                         int dir) {
  struct hcapi_cfa_em_ctx_mem_info *ctxp = &tbl_scope_cb->em_ctx_info[dir];
  struct hcapi_cfa_em_table *tbl;
  int rc = 0;
  ...
  for (i = TF_KEY0_TABLE; i < TF_MAX_TABLE; i++) {
    ...
    if (tbl->num_entries && tbl->entry_size) {
      ...
      rc = tf_msg_em_mem_rgtr(tfp, tbl->num_lvl - 1, TF_EM_PAGE_SIZE_ENUM,
                              tbl->l0_dma_addr, &tbl->ctx_id);
      if (rc)
        goto cleanup;
    }
  }
...
int tf_em_ext_alloc(struct tf *tfp, struct tf_alloc_tbl_scope_parms *parms) {
  int rc;
  enum tf_dir dir;
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "EEM: PF query error rc:%s\n", strerror(-rc));
    goto cleanup;
  }
  ...
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to allocate table scope\n");
    goto cleanup;
  }
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "Failed to allocate session table scope, rc:%s\n",
                strerror(-rc));
    goto cleanup;
  }
  ...
  for (dir = 0; dir < TF_DIR_MAX; dir++) {
    rc = tf_msg_em_qcaps(tfp, dir, &tbl_scope_cb->em_caps[dir]);
    if (rc) {
      TFP_DRV_LOG(ERR,
  ...
  for (dir = 0; dir < TF_DIR_MAX; dir++) {
    ...
    rc = tf_msg_em_cfg(
        tfp, em_tables[TF_KEY0_TABLE].num_entries,
        em_tables[TF_KEY0_TABLE].ctx_id, em_tables[TF_KEY1_TABLE].ctx_id,
        em_tables[TF_RECORD_TABLE].ctx_id, em_tables[TF_EFC_TABLE].ctx_id,
        parms->hw_flow_cache_flush_timer, dir);
    if (rc) {
      TFP_DRV_LOG(ERR,
    ...
    rc = tf_msg_em_op(tfp, dir, HWRM_TF_EXT_EM_OP_INPUT_OP_EXT_EM_ENABLE);
     ...
     */
    rc = tf_create_tbl_pool_external(dir, tbl_scope_cb,
                                     em_tables[TF_RECORD_TABLE].num_entries,
                                     em_tables[TF_RECORD_TABLE].entry_size);
    if (rc) {
      TFP_DRV_LOG(ERR, "%s TBL: Unable to allocate idx pools %s\n",
...
int tf_em_ext_free(struct tf *tfp, struct tf_free_tbl_scope_parms *parms) {
  int rc = 0;
  enum tf_dir dir;
  struct tf_tbl_scope_cb *tbl_scope_cb;
  ...
  for (dir = 0; dir < TF_DIR_MAX; dir++) {
     ...
     */
    tf_destroy_tbl_pool_external(dir, tbl_scope_cb);
    tf_msg_em_op(tfp, dir, HWRM_TF_EXT_EM_OP_INPUT_OP_EXT_EM_DISABLE);
