...
int tf_tcam_bind(struct tf *tfp, struct tf_tcam_cfg_parms *parms) {
  ...
  rc = dev->ops->tf_dev_set_tcam_slice_info(tfp, num_slices);
  if (rc)
    return rc;
  ...
  if ((tcam_cnt[TF_DIR_RX].cnt[TF_TCAM_TBL_TYPE_WC_TCAM] % num_slices) ||
    ...
    TFP_DRV_LOG(ERR, "Requested num of WC TCAM entries has to be multiple %d\n",
                num_slices);
    return -EINVAL;
  }
  ...
  for (d = 0; d < TF_DIR_MAX; d++) {
    ...
    if (info.entry.start % num_slices != 0 ||
      ...
      TFP_DRV_LOG(ERR, "%s: TCAM reserved resource is not multiple of %d\n",
                  tf_dir_2_str(d), num_slices);
      rc = -EINVAL;
      goto error;
    }
  }
  ...
  if (parms->shadow_copy) {
    for (d = 0; d < TF_DIR_MAX; d++) {
      ...
      shadow_cfg.alloc_cnt = parms->resources->tcam_cnt[d].cnt;
  ...
  TFP_DRV_LOG(INFO, "TCAM - initialized\n");
...
int tf_tcam_alloc(struct tf *tfp, struct tf_tcam_alloc_parms *parms) {
  ...
  rc = dev->ops->tf_dev_get_tcam_slice_info(tfp, parms->type, parms->key_size,
                                            &num_slices);
  if (rc)
    return rc;
  ...
  rc = tf_session_get_db(tfp, TF_MODULE_TYPE_TCAM, &tcam_db_ptr);
  if (rc) {
    TFP_DRV_LOG(ERR, "Failed to get tcam_db from session, rc:%s\n",
                strerror(-rc));
    return rc;
  }
  tcam_db = (struct tcam_rm_db *)tcam_db_ptr;
  ...
  for (i = 0; i < num_slices; i++) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: Failed tcam, type:%d\n", tf_dir_2_str(parms->dir),
                  parms->type);
      return rc;
    }
...
int tf_tcam_free(struct tf *tfp, struct tf_tcam_free_parms *parms) {
  ...
  rc = dev->ops->tf_dev_get_tcam_slice_info(tfp, parms->type, 0, &num_slices);
  if (rc)
    return rc;
  ...
  if (allocated != TF_RM_ALLOCATED_ENTRY_IN_USE) {
    TFP_DRV_LOG(ERR, "%s: Entry already free, type:%d, index:%d\n",
  ...
  for (i = 0; i < num_slices; i++) {
    ...
    if (rc) {
      TFP_DRV_LOG(ERR, "%s: Free failed, type:%d, index:%d\n",
                  tf_dir_2_str(parms->dir), parms->type, parms->idx);
      return rc;
    }
  }
  ...
  if (rc) {
    /* Log error */
    TFP_DRV_LOG(ERR, "%s: %s: Entry %d free failed, rc:%s\n",
...
int tf_tcam_set(struct tf *tfp __rte_unused,
  ...
  if (allocated != TF_RM_ALLOCATED_ENTRY_IN_USE) {
    TFP_DRV_LOG(ERR, "%s: Entry is not allocated, type:%d, index:%d\n",
                tf_dir_2_str(parms->dir), parms->type, parms->idx);
    return -EINVAL;
  }
...
int tf_tcam_get(struct tf *tfp __rte_unused, struct tf_tcam_get_parms *parms) {
  int rc;
  struct tf_session *tfs;
  struct tf_dev_info *dev;
  struct tf_rm_is_allocated_parms aparms;
