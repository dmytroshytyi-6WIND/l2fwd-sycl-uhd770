...
static const char bnxt_version[] = "Broadcom NetXtreme driver " DRV_MODULE_NAME;

/*
 * The set of PCI devices this driver supports
 */
static const struct rte_pci_id bnxt_pci_id_map[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_BROADCOM, BROADCOM_DEV_ID_STRATUS_NIC_VF1)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_BROADCOM, BROADCOM_DEV_ID_STRATUS_NIC_VF2)},
...
static const char *const bnxt_dev_args[] = {BNXT_DEVARG_REPRESENTOR,
...
static uint16_t bnxt_rss_ctxts(const struct bnxt *bp) {
  unsigned int num_rss_rings = RTE_MIN(bp->rx_nr_rings, BNXT_RSS_TBL_SIZE_P5);

  if (!BNXT_CHIP_P5(bp))
    return 1;

  return RTE_ALIGN_MUL_CEIL(num_rss_rings, BNXT_RSS_ENTRIES_PER_CTX_P5) /
...
static int bnxt_alloc_parent_info(struct bnxt *bp) {
  bp->parent =
      rte_zmalloc("bnxt_parent_info", sizeof(struct bnxt_parent_info), 0);
  if (bp->parent == NULL)
    return -ENOMEM;
...
static int bnxt_alloc_leds_info(struct bnxt *bp) {
  ...
  bp->leds =
      rte_zmalloc("bnxt_leds", BNXT_MAX_LED * sizeof(struct bnxt_led_info), 0);
  if (bp->leds == NULL)
    return -ENOMEM;
...
static int bnxt_alloc_cos_queues(struct bnxt *bp) {
  bp->rx_cos_queue =
      rte_zmalloc("bnxt_rx_cosq",
                  BNXT_COS_QUEUE_COUNT * sizeof(struct bnxt_cos_queue_info), 0);
...
static int bnxt_alloc_flow_stats_info(struct bnxt *bp) {
  bp->flow_stat =
      rte_zmalloc("bnxt_flow_xstat", sizeof(struct bnxt_flow_stat_info), 0);
...
static int bnxt_setup_one_vnic(struct bnxt *bp, uint16_t vnic_id) {
  ...
  if (dev_conf->rxmode.mq_mode & RTE_ETH_MQ_RX_RSS) {
    ...
    if (rc) {
      PMD_DRV_LOG(ERR, "HWRM vnic %d ctx %d alloc failure rc: %x\n", vnic_id, j,
                  rc);
      goto err_out;
    }
  ...
  for (j = 0; j < bp->rx_num_qs_per_vnic; j++) {
    ...
    PMD_DRV_LOG(DEBUG, "rxq[%d]->vnic=%p vnic->fw_grp_ids=%p\n", j, rxq->vnic,
                rxq->vnic->fw_grp_ids);
  ...
  rc = bnxt_hwrm_vnic_tpa_cfg(
      bp, vnic, (rx_offloads & RTE_ETH_RX_OFFLOAD_TCP_LRO) ? true : false);
  if (rc)
    goto err_out;
...
static int bnxt_register_fc_ctx_mem(struct bnxt *bp) {
  ...
  memset(bp->flow_stat->rx_fc_out_tbl.va, 0, bp->flow_stat->rx_fc_out_tbl.size);
  rc = bnxt_hwrm_cfa_counter_cfg(
      bp, BNXT_DIR_RX, CFA_COUNTER_CFG_IN_COUNTER_TYPE_FC,
      bp->flow_stat->rx_fc_out_tbl.ctx_id, bp->flow_stat->max_fc, true);
  if (rc)
    return rc;

  memset(bp->flow_stat->tx_fc_out_tbl.va, 0, bp->flow_stat->tx_fc_out_tbl.size);
  rc = bnxt_hwrm_cfa_counter_cfg(
      bp, BNXT_DIR_TX, CFA_COUNTER_CFG_IN_COUNTER_TYPE_FC,
      bp->flow_stat->tx_fc_out_tbl.ctx_id, bp->flow_stat->max_fc, true);
...
static int bnxt_alloc_ctx_mem_buf(struct bnxt *bp, char *type, size_t size,
  ...
  ctx->va = rte_zmalloc_socket(type, size, 0, bp->eth_dev->device->numa_node);
  if (ctx->va == NULL)
    return -ENOMEM;
...
static int bnxt_init_fc_ctx_mem(struct bnxt *bp) {
  ...
  sprintf(type, "bnxt_rx_fc_in_" PCI_PRI_FMT, pdev->addr.domain, pdev->addr.bus,
          pdev->addr.devid, pdev->addr.function);
  /* 4 bytes for each counter-id */
  rc = bnxt_alloc_ctx_mem_buf(bp, type, max_fc * 4,
  ...
  rc = bnxt_alloc_ctx_mem_buf(bp, type, max_fc * 16,
                              &bp->flow_stat->rx_fc_out_tbl);
  if (rc)
    return rc;

  sprintf(type, "bnxt_tx_fc_in_" PCI_PRI_FMT, pdev->addr.domain, pdev->addr.bus,
          pdev->addr.devid, pdev->addr.function);
  /* 4 bytes for each counter-id */
  rc = bnxt_alloc_ctx_mem_buf(bp, type, max_fc * 4,
...
static int bnxt_init_ctx_mem(struct bnxt *bp) {
  ...
  if (!(bp->fw_cap & BNXT_FW_CAP_ADV_FLOW_COUNTERS) ||
      !(BNXT_PF(bp) || BNXT_VF_IS_TRUSTED(bp)) || !BNXT_FLOW_XSTATS_EN(bp))
    return 0;
...
static int bnxt_alloc_prev_ring_stats(struct bnxt *bp) {
  bp->prev_rx_ring_stats =
      rte_zmalloc("bnxt_prev_rx_ring_stats",
                  sizeof(struct bnxt_ring_stats) * bp->rx_cp_nr_rings, 0);
  if (bp->prev_rx_ring_stats == NULL)
    return -ENOMEM;

  bp->prev_tx_ring_stats =
      rte_zmalloc("bnxt_prev_tx_ring_stats",
                  sizeof(struct bnxt_ring_stats) * bp->tx_cp_nr_rings, 0);
...
static int bnxt_start_nic(struct bnxt *bp) {
  ...
  for (j = 0, i = 0; i < BNXT_COS_QUEUE_COUNT; i++) {
    if (bp->rx_cos_queue[i].id != 0xff) {
      ...
      if (!vnic) {
        PMD_DRV_LOG(ERR, "Num pools more than FW profile\n");
        rc = -EINVAL;
        goto err_out;
      }
...
skip_cosq_cfg:
  ...
  for (j = 0; j < bp->rx_nr_rings; j++) {
    ...
    if (!rxq->rx_deferred_start) {
      bp->eth_dev->data->rx_queue_state[j] = RTE_ETH_QUEUE_STATE_STARTED;
      rxq->rx_started = true;
    }
  }
  ...
  for (j = 0; j < bp->tx_nr_rings; j++) {
    ...
    if (!txq->tx_deferred_start) {
      bp->eth_dev->data->tx_queue_state[j] = RTE_ETH_QUEUE_STATE_STARTED;
      txq->tx_started = true;
    }
  }
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "HWRM cfa l2 rx mask failure rc: %x\n", rc);
    goto err_out;
  }
  ...
  if ((rte_intr_cap_multiple(intr_handle) ||
    ...
    if (intr_vector > bp->rx_cp_nr_rings) {
      PMD_DRV_LOG(ERR, "At most %d intr queues supported", bp->rx_cp_nr_rings);
      return -ENOTSUP;
    }
  ...
  if (rte_intr_dp_is_en(intr_handle)) {
    ...
    PMD_DRV_LOG(DEBUG,
                "intr_handle->nb_efd = %d "
                "intr_handle->max_intr = %d\n",
                rte_intr_nb_efd_get(intr_handle),
                rte_intr_max_intr_get(intr_handle));
    for (queue_id = 0; queue_id < bp->eth_dev->data->nb_rx_queues; queue_id++) {
      rte_intr_vec_list_index_set(intr_handle, queue_id,
...
static int bnxt_dev_info_get_op(struct rte_eth_dev *eth_dev,
  ...
  dev_info->max_mac_addrs =
      RTE_MIN(bp->max_l2_ctx, RTE_ETH_NUM_RECEIVE_MAC_ADDR);
  ...
  dev_info->tx_offload_capa =
      BNXT_DEV_TX_OFFLOAD_SUPPORT | dev_info->tx_queue_offload_capa;
  ...
  if (BNXT_PF(bp) || BNXT_VF_IS_TRUSTED(bp)) {
    ...
    dev_info->switch_info.port_id =
        BNXT_PF(bp) ? BNXT_SWITCH_PORT_ID_PF : BNXT_SWITCH_PORT_ID_TRUSTED_VF;
  }
...
resource_error:
  PMD_DRV_LOG(ERR, "Insufficient resources to support requested config\n");
  PMD_DRV_LOG(ERR, "Num Queues Requested: Tx %d, Rx %d\n",
              eth_dev->data->nb_tx_queues, eth_dev->data->nb_rx_queues);
  PMD_DRV_LOG(ERR, "MAX: TxQ %d, RxQ %d, CQ %d Stat %d, Grp %d, Vnic %d\n",
...
void bnxt_print_link_info(struct rte_eth_dev *eth_dev) {
  ...
  if (link->link_status)
    PMD_DRV_LOG(DEBUG, "Port %d Link Up - speed %u Mbps - %s\n",
                eth_dev->data->port_id, (uint32_t)link->link_speed,
...
static eth_rx_burst_t bnxt_receive_function(struct rte_eth_dev *eth_dev) {
  struct bnxt *bp = eth_dev->data->dev_private;
  ...
  if (rte_vect_get_max_simd_bitwidth() >= RTE_VECT_SIMD_256 &&
    ...
    PMD_DRV_LOG(INFO, "Using AVX2 vector mode receive for port %d\n",
                eth_dev->data->port_id);
    bp->flags |= BNXT_FLAG_RX_VECTOR_PKT_MODE;
  ...
  if (rte_vect_get_max_simd_bitwidth() >= RTE_VECT_SIMD_128) {
    PMD_DRV_LOG(INFO, "Using SSE vector mode receive for port %d\n",
                eth_dev->data->port_id);
    bp->flags |= BNXT_FLAG_RX_VECTOR_PKT_MODE;
...
use_scalar_rx:
  PMD_DRV_LOG(INFO, "Vector mode receive disabled for port %d\n",
              eth_dev->data->port_id);
  PMD_DRV_LOG(INFO, "Port %d scatter: %d rx offload: %" PRIX64 "\n",
...
static eth_tx_burst_t bnxt_transmit_function(struct rte_eth_dev *eth_dev) {
  struct bnxt *bp = eth_dev->data->dev_private;
  ...
  if (BNXT_CHIP_SR2(bp))
    ...
    defined(RTE_ARCH_ARM64) && !defined(RTE_LIBRTE_IEEE1588)
   ...
   */
  if (eth_dev->data->scattered_rx ||
      (offloads & ~RTE_ETH_TX_OFFLOAD_MBUF_FAST_FREE) || BNXT_TRUFLOW_EN(bp))
    goto use_scalar_tx;

#if defined(RTE_ARCH_X86) && defined(CC_AVX2_SUPPORT)
  if (rte_vect_get_max_simd_bitwidth() >= RTE_VECT_SIMD_256 &&
      rte_cpu_get_flag_enabled(RTE_CPUFLAG_AVX2) == 1) {
    PMD_DRV_LOG(INFO, "Using AVX2 vector mode transmit for port %d\n",
                eth_dev->data->port_id);
    return bnxt_xmit_pkts_vec_avx2;
  }
#endif
  if (rte_vect_get_max_simd_bitwidth() >= RTE_VECT_SIMD_128) {
    PMD_DRV_LOG(INFO, "Using SSE vector mode transmit for port %d\n",
                eth_dev->data->port_id);
...
use_scalar_tx:
  PMD_DRV_LOG(INFO, "Vector mode transmit disabled for port %d\n",
              eth_dev->data->port_id);
  PMD_DRV_LOG(INFO, "Port %d scatter: %d tx offload: %" PRIX64 "\n",
              eth_dev->data->port_id, eth_dev->data->scattered_rx, offloads);
#endif
  return bnxt_xmit_pkts;
}
...
static void bnxt_ptp_get_current_time(void *arg) {
  ...
  bnxt_hwrm_port_ts_query(bp, BNXT_PTP_FLAGS_CURRENT_TIME, &ptp->current_time);
...
int bnxt_dev_stop_op(struct rte_eth_dev *eth_dev) {
  ...
  if (bp->flags & BNXT_FLAG_FW_RESET) {
    PMD_DRV_LOG(ERR, "Adapter recovering from error..Please retry\n");
...
static void bnxt_uninit_locks(struct bnxt *bp) {
...
int bnxt_link_update_op(struct rte_eth_dev *eth_dev, int wait_to_complete) {
  ...
  struct bnxt *bp = eth_dev->data->dev_private;
  struct rte_eth_link new;
  int cnt = wait_to_complete ? BNXT_MAX_LINK_WAIT_CNT : BNXT_MIN_LINK_WAIT_CNT;
  ...
  do {
    ...
    if (rc) {
      ...
      PMD_DRV_LOG(ERR, "Failed to retrieve link rc = 0x%x!\n", rc);
      goto out;
    }
...
static int bnxt_reta_update_op(struct rte_eth_dev *eth_dev,
  ...
  for (i = 0; i < reta_size; i++) {
    ...
    if (BNXT_CHIP_P5(bp)) {
      vnic->rss_table[i * 2] = rxq->rx_ring->rx_ring_struct->fw_ring_id;
      vnic->rss_table[i * 2 + 1] = rxq->cp_ring->cp_ring_struct->fw_ring_id;
    } else {
...
static int bnxt_reta_query_op(struct rte_eth_dev *eth_dev,
  ...
  for (idx = 0, i = 0; i < reta_size; i++) {
    ...
    if (reta_conf[idx].mask & (1ULL << sft)) {
      ...
      if (BNXT_CHIP_P5(bp))
        qid = bnxt_rss_to_qid(bp, vnic->rss_table[i * 2]);
      else
...
static int bnxt_rss_hash_update_op(struct rte_eth_dev *eth_dev,
  ...
  vnic->hash_mode = bnxt_rte_to_hwrm_hash_level(
      bp, rss_conf->rss_hf, RTE_ETH_RSS_LEVEL(rss_conf->rss_hf));
  ...
  if (rss_conf->rss_key_len != HW_HASH_KEY_SIZE) {
    PMD_DRV_LOG(ERR, "Invalid hashkey length, should be %d bytes\n",
                HW_HASH_KEY_SIZE);
    return -EINVAL;
  }
...
static int bnxt_rss_hash_conf_get_op(struct rte_eth_dev *eth_dev,
  ...
  if (vnic && vnic->rss_hash_key) {
    if (rss_conf->rss_key) {
      len = rss_conf->rss_key_len <= HW_HASH_KEY_SIZE ? rss_conf->rss_key_len
                                                      : HW_HASH_KEY_SIZE;
    ...
    if (hash_types & HWRM_VNIC_RSS_CFG_INPUT_HASH_TYPE_TCP_IPV4) {
      ...
      hash_types &= ~HWRM_VNIC_RSS_CFG_INPUT_HASH_TYPE_TCP_IPV4;
    }
    ...
    rss_conf->rss_hf |= bnxt_hwrm_to_rte_rss_level(bp, vnic->hash_mode);

    if (hash_types) {
      PMD_DRV_LOG(ERR, "Unknown RSS config from firmware (%08x), RSS disabled",
                  vnic->hash_type);
      return -ENOTSUP;
    }
  } else {
...
static int bnxt_udp_tunnel_port_add_op(struct rte_eth_dev *eth_dev,
  ...
  case RTE_ETH_TUNNEL_TYPE_VXLAN:
    ...
    tunnel_type = HWRM_TUNNEL_DST_PORT_ALLOC_INPUT_TUNNEL_TYPE_VXLAN;
    break;
  case RTE_ETH_TUNNEL_TYPE_GENEVE:
    ...
    tunnel_type = HWRM_TUNNEL_DST_PORT_ALLOC_INPUT_TUNNEL_TYPE_GENEVE;
    break;
  default:
    PMD_DRV_LOG(ERR, "Tunnel type is not supported\n");
    return -ENOTSUP;
  }
  rc = bnxt_hwrm_tunnel_dst_port_alloc(bp, udp_tunnel->udp_port, tunnel_type);

  if (rc != 0)
    return rc;

  if (tunnel_type == HWRM_TUNNEL_DST_PORT_ALLOC_INPUT_TUNNEL_TYPE_VXLAN)
    bp->vxlan_port_cnt++;

  if (tunnel_type == HWRM_TUNNEL_DST_PORT_ALLOC_INPUT_TUNNEL_TYPE_GENEVE)
    bp->geneve_port_cnt++;

  return rc;
}

static int bnxt_udp_tunnel_port_del_op(struct rte_eth_dev *eth_dev,
                                       struct rte_eth_udp_tunnel *udp_tunnel) {
  struct bnxt *bp = eth_dev->data->dev_private;
  uint16_t tunnel_type = 0;
  uint16_t port = 0;
  ...
  case RTE_ETH_TUNNEL_TYPE_VXLAN:
    ...
    tunnel_type = HWRM_TUNNEL_DST_PORT_FREE_INPUT_TUNNEL_TYPE_VXLAN;
  ...
  case RTE_ETH_TUNNEL_TYPE_GENEVE:
    ...
    tunnel_type = HWRM_TUNNEL_DST_PORT_FREE_INPUT_TUNNEL_TYPE_GENEVE;
...
static int bnxt_del_vlan_filter(struct bnxt *bp, uint16_t vlan_id) {
  ...
  while (filter) {
    ...
    if (bnxt_vlan_filter_exists(bp, filter, chk, vlan_id)) {
      ...
      STAILQ_REMOVE(&vnic->filter, filter, bnxt_filter_info, next);
      bnxt_free_filter(bp, filter);
      PMD_DRV_LOG(INFO, "Deleted vlan filter for %d\n", vlan_id);
      return 0;
    }
...
static int bnxt_add_vlan_filter(struct bnxt *bp, uint16_t vlan_id) {
  ...
  filter = bnxt_alloc_filter(bp);
  if (!filter) {
    PMD_DRV_LOG(ERR, "MAC/VLAN filter alloc failed\n");
    return -ENOMEM;
  }
  /* MAC + VLAN ID filter */
  ...
  PMD_DRV_LOG(INFO, "Added Vlan filter for %d\n", vlan_id);
  return rc;
}
...
static int bnxt_config_vlan_hw_filter(struct bnxt *bp, uint64_t rx_offloads) {
  struct bnxt_vnic_info *vnic;
  unsigned int i;
  int rc;
...
static int bnxt_config_vlan_hw_stripping(struct bnxt *bp,
...
static int bnxt_vlan_tpid_set_op(struct rte_eth_dev *dev,
  ...
  if (!qinq) {
    PMD_DRV_LOG(ERR, "QinQ not enabled. Needs to be ON as we can "
  ...
  if (vlan_type == RTE_ETH_VLAN_TYPE_OUTER) {
    ...
    case RTE_ETHER_TYPE_QINQ:
      bp->outer_tpid_bd = TX_BD_LONG_CFA_META_VLAN_TPID_TPID88A8;
      break;
    case RTE_ETHER_TYPE_VLAN:
      bp->outer_tpid_bd = TX_BD_LONG_CFA_META_VLAN_TPID_TPID8100;
      break;
    case RTE_ETHER_TYPE_QINQ1:
      bp->outer_tpid_bd = TX_BD_LONG_CFA_META_VLAN_TPID_TPID9100;
      break;
    case RTE_ETHER_TYPE_QINQ2:
      bp->outer_tpid_bd = TX_BD_LONG_CFA_META_VLAN_TPID_TPID9200;
      break;
    case RTE_ETHER_TYPE_QINQ3:
      bp->outer_tpid_bd = TX_BD_LONG_CFA_META_VLAN_TPID_TPID9300;
      break;
    default:
...
static int bnxt_set_default_mac_addr_op(struct rte_eth_dev *dev,
...
static int bnxt_dev_set_mc_addr_list_op(struct rte_eth_dev *eth_dev,
...
static int bnxt_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
                               size_t fw_size) {
  struct bnxt *bp = dev->data->dev_private;
  ...
  ret = snprintf(fw_version, fw_size, "%d.%d.%d.%d", fw_major, fw_minor,
                 fw_updt, fw_rsvd);
  if (ret < 0)
    return -EINVAL;
...
static void bnxt_rxq_info_get_op(struct rte_eth_dev *dev, uint16_t queue_id,
...
static void bnxt_txq_info_get_op(struct rte_eth_dev *dev, uint16_t queue_id,
...
static int bnxt_rx_burst_mode_get(struct rte_eth_dev *dev,
...
static int bnxt_tx_burst_mode_get(struct rte_eth_dev *dev,
                                  __rte_unused uint16_t queue_id,
                                  struct rte_eth_burst_mode *mode) {
...
int bnxt_mtu_set_op(struct rte_eth_dev *eth_dev, uint16_t new_mtu) {
   ...
   */
  if (eth_dev->data->dev_started && !eth_dev->data->scattered_rx &&
      (new_pkt_size > eth_dev->data->min_rx_buf_size - RTE_PKTMBUF_HEADROOM)) {
    PMD_DRV_LOG(ERR, "MTU change would require scattered rx support. ");
...
static int bnxt_vlan_pvid_set_op(struct rte_eth_dev *dev, uint16_t pvid,
                                 int on) {
  struct bnxt *bp = dev->data->dev_private;
...
static int bnxt_dev_led_on_op(struct rte_eth_dev *dev) {
  struct bnxt *bp = dev->data->dev_private;
  int rc;
...
static int bnxt_dev_led_off_op(struct rte_eth_dev *dev) {
  struct bnxt *bp = dev->data->dev_private;
  int rc;
...
static uint32_t bnxt_rx_queue_count_op(void *rx_queue) {
  struct bnxt *bp;
...
static int bnxt_rx_descriptor_status_op(void *rx_queue, uint16_t offset) {
  ...
  if (bp->flags & BNXT_FLAG_RX_VECTOR_PKT_MODE) {
    ...
    raw_cons = cpr->cp_raw_cons + offset * CMP_LEN(CMPL_BASE_TYPE_RX_L2);
    cons = RING_CMP(cpr->cp_ring_struct, raw_cons);
    rxcmp = (struct rx_pkt_cmpl *)&cpr->cp_desc_ring[cons];
...
static int bnxt_tx_descriptor_status_op(void *tx_queue, uint16_t offset) {
...
bnxt_dev_supported_ptypes_get_op(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {RTE_PTYPE_L2_ETHER_VLAN,
...
static void bnxt_unmap_ptp_regs(struct bnxt *bp) {
  rte_write32(0, (uint8_t *)bp->bar0 + BNXT_GRCPF_REG_WINDOW_BASE_OUT + 16);
  rte_write32(0, (uint8_t *)bp->bar0 + BNXT_GRCPF_REG_WINDOW_BASE_OUT + 20);
}
...
static uint64_t bnxt_cc_read(struct bnxt *bp) {
  ...
  ns = rte_le_to_cpu_32(
      rte_read32((uint8_t *)bp->bar0 + BNXT_GRCPF_REG_SYNC_TIME));
  ns |= (uint64_t)(rte_le_to_cpu_32(
            rte_read32((uint8_t *)bp->bar0 + BNXT_GRCPF_REG_SYNC_TIME + 4)))
        << 32;
  return ns;
}
...
static int bnxt_get_tx_ts(struct bnxt *bp, uint64_t *ts) {
  ...
  fifo = rte_le_to_cpu_32(
      rte_read32((uint8_t *)bp->bar0 + ptp->tx_mapped_regs[BNXT_PTP_TX_FIFO]));
  ...
  fifo = rte_le_to_cpu_32(
      rte_read32((uint8_t *)bp->bar0 + ptp->tx_mapped_regs[BNXT_PTP_TX_FIFO]));
  *ts = rte_le_to_cpu_32(
      rte_read32((uint8_t *)bp->bar0 + ptp->tx_mapped_regs[BNXT_PTP_TX_TS_L]));
  *ts |= (uint64_t)rte_le_to_cpu_32(rte_read32(
             (uint8_t *)bp->bar0 + ptp->tx_mapped_regs[BNXT_PTP_TX_TS_H]))
         << 32;
...
static int bnxt_clr_rx_ts(struct bnxt *bp, uint64_t *last_ts) {
  ...
  port_id = pf->port_id;
  fifo = rte_le_to_cpu_32(
      rte_read32((uint8_t *)bp->bar0 + ptp->rx_mapped_regs[BNXT_PTP_RX_FIFO]));
  while ((fifo & BNXT_PTP_RX_FIFO_PENDING) && (i < BNXT_PTP_RX_PND_CNT)) {
    rte_write32(1 << port_id, (uint8_t *)bp->bar0 +
                                  ptp->rx_mapped_regs[BNXT_PTP_RX_FIFO_ADV]);
    fifo = rte_le_to_cpu_32(rte_read32((uint8_t *)bp->bar0 +
                                       ptp->rx_mapped_regs[BNXT_PTP_RX_FIFO]));
    *last_ts = rte_le_to_cpu_32(rte_read32(
        (uint8_t *)bp->bar0 + ptp->rx_mapped_regs[BNXT_PTP_RX_TS_L]));
    *last_ts |=
        (uint64_t)rte_le_to_cpu_32(rte_read32(
            (uint8_t *)bp->bar0 + ptp->rx_mapped_regs[BNXT_PTP_RX_TS_H]))
        << 32;
    i++;
  }
...
static int bnxt_get_rx_ts(struct bnxt *bp, uint64_t *ts) {
  ...
  port_id = pf->port_id;
  rte_write32(1 << port_id,
              (uint8_t *)bp->bar0 + ptp->rx_mapped_regs[BNXT_PTP_RX_FIFO_ADV]);
  ...
  *ts = rte_le_to_cpu_32(
      rte_read32((uint8_t *)bp->bar0 + ptp->rx_mapped_regs[BNXT_PTP_RX_TS_L]));
  *ts |= (uint64_t)rte_le_to_cpu_32(rte_read32(
             (uint8_t *)bp->bar0 + ptp->rx_mapped_regs[BNXT_PTP_RX_TS_H]))
         << 32;
...
static int bnxt_timesync_write_time(struct rte_eth_dev *dev,
...
static int bnxt_timesync_read_time(struct rte_eth_dev *dev,
                                   struct timespec *ts) {
  struct bnxt *bp = dev->data->dev_private;
  struct bnxt_ptp_cfg *ptp = bp->ptp_cfg;
...
static int bnxt_timesync_enable(struct rte_eth_dev *dev) {
  struct bnxt *bp = dev->data->dev_private;
  struct bnxt_ptp_cfg *ptp = bp->ptp_cfg;
  uint32_t shift = 0;
  int rc;
...
static int bnxt_timesync_disable(struct rte_eth_dev *dev) {
  struct bnxt *bp = dev->data->dev_private;
  struct bnxt_ptp_cfg *ptp = bp->ptp_cfg;
...
static int bnxt_timesync_read_rx_timestamp(struct rte_eth_dev *dev,
...
static int bnxt_timesync_read_tx_timestamp(struct rte_eth_dev *dev,
                                           struct timespec *timestamp) {
  struct bnxt *bp = dev->data->dev_private;
  struct bnxt_ptp_cfg *ptp = bp->ptp_cfg;
  ...
  if (BNXT_CHIP_P5(bp))
    rc = bnxt_hwrm_port_ts_query(bp, BNXT_PTP_FLAGS_PATH_TX, &tx_tstamp_cycles);
  else
...
static int bnxt_timesync_adjust_time(struct rte_eth_dev *dev, int64_t delta) {
  struct bnxt *bp = dev->data->dev_private;
  struct bnxt_ptp_cfg *ptp = bp->ptp_cfg;
...
static int bnxt_get_eeprom_length_op(struct rte_eth_dev *dev) {
  struct bnxt *bp = dev->data->dev_private;
  int rc;
...
static int bnxt_get_eeprom_op(struct rte_eth_dev *dev,
  ...
  PMD_DRV_LOG(INFO, PCI_PRI_FMT " in_eeprom->offset = %d len = %d\n",
              bp->pdev->addr.domain, bp->pdev->addr.bus, bp->pdev->addr.devid,
              bp->pdev->addr.function, in_eeprom->offset, in_eeprom->length);

  if (in_eeprom->offset == 0) /* special offset value to get directory */
    return bnxt_get_nvram_directory(bp, in_eeprom->length, in_eeprom->data);

  index = in_eeprom->offset >> 24;
  offset = in_eeprom->offset & 0xffffff;

  if (index != 0)
    return bnxt_hwrm_get_nvram_item(bp, index - 1, offset, in_eeprom->length,
                                    in_eeprom->data);
...
static int bnxt_set_eeprom_op(struct rte_eth_dev *dev,
                              struct rte_dev_eeprom_info *in_eeprom) {
  struct bnxt *bp = dev->data->dev_private;
  ...
  return bnxt_hwrm_flash_nvram(bp, type, ordinal, ext, attr, in_eeprom->data,
                               in_eeprom->length);
}
...
static int bnxt_get_module_info(struct rte_eth_dev *dev,
  ...
  if (bp->hwrm_spec_code < 0x10202) {
    PMD_DRV_LOG(NOTICE,
                "Port %u : Feature is not supported in older firmware\n",
                dev->data->port_id);
    return -ENOTSUP;
  }

  rc = bnxt_hwrm_read_sfp_module_eeprom_info(
      bp, I2C_DEV_ADDR_A0, 0, 0, SFF_DIAG_SUPPORT_OFFSET + 1, module_info);
...
static int bnxt_get_module_eeprom(struct rte_eth_dev *dev,
  ...
  case SFF_MODULE_ID_QSFP28:
    rc = bnxt_hwrm_read_sfp_module_eeprom_info(
        bp, I2C_DEV_ADDR_A0, 0, SFF8636_OPT_PAGES_OFFSET, 1, &opt_pages);
    if (rc)
      return rc;
  ...
  while (length && page < max_pages) {
    ...
    if (pg_addr[page]) {
      rc = bnxt_hwrm_read_sfp_module_eeprom_info(bp, pg_addr[page], raw_page,
                                                 offset, chunk, data);
      if (rc)
        return rc;
    }
...
static uint32_t bnxt_map_reset_regs(struct bnxt *bp, uint32_t reg) {
  ...
  rte_write32(reg & 0xfffff000,
              (uint8_t *)bp->bar0 + BNXT_GRCPF_REG_WINDOW_BASE_OUT + 8);
...
int bnxt_map_fw_health_status_regs(struct bnxt *bp) {
  ...
  for (i = 0; i < BNXT_FW_STATUS_REG_CNT; i++) {
     ...
     */
    info->mapped_status_regs[i] = BNXT_GRCP_WINDOW_2_BASE + (reg & 0xffc);
  }
  ...
  rte_write32(reg_base,
              (uint8_t *)bp->bar0 + BNXT_GRCPF_REG_WINDOW_BASE_OUT + 4);
...
static void bnxt_write_fw_reset_reg(struct bnxt *bp, uint32_t index) {
  ...
  case BNXT_FW_STATUS_REG_TYPE_CFG:
    ...
    if (ret < 0) {
      PMD_DRV_LOG(ERR, "Failed to write %#x at PCI offset %#x", val, offset);
      return;
    }
    break;
...
static int bnxt_check_fw_reset_done(struct bnxt *bp) {
...
static void bnxt_dev_recover(void *arg) {
  ...
  rc = bnxt_init_resources(bp, true);
  if (rc) {
    PMD_DRV_LOG(ERR, "Failed to initialize resources after reset\n");
    goto err;
  }
...
err:
  ...
  if (bp->eth_dev->data->dev_conf.intr_conf.rmv)
    rte_eth_dev_callback_process(bp->eth_dev, RTE_ETH_EVENT_INTR_RMV, NULL);
  pthread_mutex_unlock(&bp->err_recovery_lock);
...
uint32_t bnxt_read_fw_status_reg(struct bnxt *bp, uint32_t index) {
  ...
  case BNXT_FW_STATUS_REG_TYPE_CFG:
    ...
    if (ret < 0)
      PMD_DRV_LOG(ERR, "Failed to read PCI offset %#x", offset);
    break;
  case BNXT_FW_STATUS_REG_TYPE_GRC:
    offset = info->mapped_status_regs[index];
    /* FALLTHROUGH */
  case BNXT_FW_STATUS_REG_TYPE_BAR0:
    val = rte_le_to_cpu_32(rte_read32((uint8_t *)bp->bar0 + offset));
    break;
  }
...
static void bnxt_fw_reset_cb(void *arg) {
  ...
  if (bnxt_is_primary_func(bp) && bnxt_is_recovery_enabled(bp)) {
  ...
  if (info->flags & BNXT_FLAG_ERROR_RECOVERY_HOST) {
    ...
    rte_eal_alarm_set(US_PER_MS, bnxt_dev_reset_and_resume, (void *)bp);
  }
}
...
static void bnxt_check_fw_health(void *arg) {
  ...
  if (!info || !bnxt_is_recovery_enabled(bp) || is_bnxt_in_error(bp))
    return;
  ...
  rte_eal_alarm_set(US_PER_MS * info->driver_polling_freq, bnxt_check_fw_health,
                    (void *)bp);
...
reset:
  ...
  rte_eal_alarm_set(US_PER_MS * wait_msec, bnxt_fw_reset_cb, (void *)bp);
}
...
void bnxt_schedule_fw_health_check(struct bnxt *bp) {
  ...
  rte_eal_alarm_set(US_PER_MS * polling_freq, bnxt_check_fw_health, (void *)bp);
...
static int bnxt_alloc_ctx_mem_blk(struct bnxt *bp,
                                  ...
                                  uint32_t mem_size, const char *suffix,
                                  uint16_t idx) {
  ...
  if (rmem->nr_pages > 1) {
    snprintf(mz_name, RTE_MEMZONE_NAMESIZE, "bnxt_ctx_pg_tbl%s_%x_%d", suffix,
             idx, bp->eth_dev->data->port_id);
    mz_name[RTE_MEMZONE_NAMESIZE - 1] = 0;
    mz = rte_memzone_lookup(mz_name);
    if (!mz) {
      mz = rte_memzone_reserve_aligned(
          mz_name, rmem->nr_pages * 8, bp->eth_dev->device->numa_node,
  ...
  snprintf(mz_name, RTE_MEMZONE_NAMESIZE, "bnxt_ctx_%s_%x_%d", suffix, idx,
           bp->eth_dev->data->port_id);
  mz = rte_memzone_lookup(mz_name);
  if (!mz) {
    mz = rte_memzone_reserve_aligned(
        mz_name, mem_size, bp->eth_dev->device->numa_node,
        RTE_MEMZONE_1GB | RTE_MEMZONE_SIZE_HINT_ONLY | RTE_MEMZONE_IOVA_CONTIG,
        BNXT_PAGE_SIZE);
    if (mz == NULL)
      return -ENOMEM;
  }
  ...
  for (sz = 0, i = 0; sz < mem_size; sz += BNXT_PAGE_SIZE, i++) {
    ...
    if (rmem->nr_pages > 1) {
      if (i == rmem->nr_pages - 2 && (rmem->flags & BNXT_RMEM_RING_PTE_FLAG))
        valid_bits |= PTU_PTE_NEXT_TO_LAST;
      else if (i == rmem->nr_pages - 1 &&
               (rmem->flags & BNXT_RMEM_RING_PTE_FLAG))
        valid_bits |= PTU_PTE_LAST;

      rmem->pg_tbl[i] = rte_cpu_to_le_64(rmem->dma_arr[i] | valid_bits);
    }
  }
...
int bnxt_alloc_ctx_mem(struct bnxt *bp) {
  ...
  ctx_pg = &ctx->vnic_mem;
  ctx_pg->entries =
      ctx->vnic_max_vnic_entries + ctx->vnic_max_ring_table_entries;
  ...
  entries_sp = ctx->qp_max_l2_entries + ctx->vnic_max_vnic_entries +
  ...
  entries = clamp_t(uint32_t, entries, min, ctx->tqm_max_entries_per_ring);
  ...
  if (rc)
    PMD_DRV_LOG(ERR, "Failed to configure context mem: rc = %d\n", rc);
  else
...
static int bnxt_alloc_stats_mem(struct bnxt *bp) {
  ...
  snprintf(mz_name, RTE_MEMZONE_NAMESIZE, "bnxt_" PCI_PRI_FMT "-%s",
  ...
  total_alloc_len = RTE_CACHE_LINE_ROUNDUP(
      sizeof(struct rx_port_stats) + sizeof(struct rx_port_stats_ext) + 512);
  if (!mz) {
    mz = rte_memzone_reserve(mz_name, total_alloc_len, SOCKET_ID_ANY,
                             RTE_MEMZONE_2MB | RTE_MEMZONE_SIZE_HINT_ONLY |
                                 RTE_MEMZONE_IOVA_CONTIG);
    if (mz == NULL)
      return -ENOMEM;
  ...
  mz = rte_memzone_lookup(mz_name);
  total_alloc_len = RTE_CACHE_LINE_ROUNDUP(
      sizeof(struct tx_port_stats) + sizeof(struct tx_port_stats_ext) + 512);
  if (!mz) {
    mz = rte_memzone_reserve(mz_name, total_alloc_len, SOCKET_ID_ANY,
                             RTE_MEMZONE_2MB | RTE_MEMZONE_SIZE_HINT_ONLY |
                                 RTE_MEMZONE_IOVA_CONTIG);
  ...
  bp->hw_rx_port_stats_ext =
      (void *)((uint8_t *)bp->hw_rx_port_stats + sizeof(struct rx_port_stats));
  bp->hw_rx_port_stats_ext_map =
      bp->hw_rx_port_stats_map + sizeof(struct rx_port_stats);
  bp->flags |= BNXT_FLAG_EXT_RX_PORT_STATS;

  if (bp->hwrm_spec_code < HWRM_SPEC_CODE_1_9_2 ||
      bp->flags & BNXT_FLAG_EXT_STATS_SUPPORTED) {
    bp->hw_tx_port_stats_ext = (void *)((uint8_t *)bp->hw_tx_port_stats +
                                        sizeof(struct tx_port_stats));
    bp->hw_tx_port_stats_ext_map =
        bp->hw_tx_port_stats_map + sizeof(struct tx_port_stats);
...
static int bnxt_setup_mac_addr(struct rte_eth_dev *eth_dev) {
  ...
  if (bp->max_l2_ctx > RTE_ETH_NUM_RECEIVE_MAC_ADDR)
    PMD_DRV_LOG(
        INFO,
        "Max number of MAC addrs supported is %d, but will be limited to %d\n",
        bp->max_l2_ctx, RTE_ETH_NUM_RECEIVE_MAC_ADDR);

  eth_dev->data->mac_addrs =
      rte_zmalloc("bnxt_mac_addr_tbl", RTE_ETHER_ADDR_LEN * max_mac_addr, 0);
  ...
  if (!BNXT_HAS_DFLT_MAC_SET(bp)) {
    ...
    PMD_DRV_LOG(INFO, "Assign random MAC:" RTE_ETHER_ADDR_PRT_FMT "\n",
   ...
   */
  bp->mcast_addr_list =
      rte_zmalloc("bnxt_mcast_addr_tbl",
                  sizeof(struct rte_ether_addr) * BNXT_MAX_MC_ADDRS, 0);
...
static void bnxt_alloc_error_recovery_info(struct bnxt *bp) {
  ...
  info = rte_zmalloc("bnxt_hwrm_error_recovery_qcfg", sizeof(*info), 0);
  if (!info)
...
static void bnxt_check_fw_status(struct bnxt *bp) {
  ...
  if (!(bp->recovery_info && (bp->fw_cap & BNXT_FW_CAP_HCOMM_FW_STATUS)))
    return;
...
static int bnxt_map_hcomm_fw_status_reg(struct bnxt *bp) {
  ...
  rte_write32(HCOMM_STATUS_STRUCT_LOC,
              (uint8_t *)bp->bar0 + BNXT_GRCPF_REG_WINDOW_BASE_OUT + 4);
  sig_ver = rte_le_to_cpu_32(
      rte_read32((uint8_t *)bp->bar0 + BNXT_GRCP_WINDOW_2_BASE +
                 offsetof(struct hcomm_status, sig_ver)));
  /* If the signature is absent, then FW does not support this feature */
  if ((sig_ver & HCOMM_STATUS_SIGNATURE_MASK) != HCOMM_STATUS_SIGNATURE_VAL)
    return 0;
  ...
  status_loc = rte_le_to_cpu_32(
      rte_read32((uint8_t *)bp->bar0 + BNXT_GRCP_WINDOW_2_BASE +
                 offsetof(struct hcomm_status, fw_status_loc)));
  ...
  rte_write32((status_loc & BNXT_GRCP_BASE_MASK),
              (uint8_t *)bp->bar0 + BNXT_GRCPF_REG_WINDOW_BASE_OUT + 4);
...
static int bnxt_init_resources(struct bnxt *bp, bool reconfig_dev) {
  ...
  if (BNXT_PF(bp)) {
    if (bp->pdev->max_vfs) {
      ...
      if (rc) {
        PMD_DRV_LOG(ERR, "Failed to allocate PF resources");
        return rc;
      }
    }
  }
  ...
  if (!reconfig_dev) {
    bp->rss_conf.rss_key = rte_zmalloc("bnxt_rss_key", HW_HASH_KEY_SIZE, 0);
...
static int bnxt_parse_devarg_flow_xstat(__rte_unused const char *key,
  ...
  if (!value || !opaque_arg) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to flow_xstat devarg.\n");
    return -EINVAL;
  }
  ...
  if (BNXT_DEVARG_FLOW_XSTAT_INVALID(flow_xstat)) {
    PMD_DRV_LOG(ERR, "Invalid value passed to flow_xstat devarg.\n");
    return -EINVAL;
  }
...
static int bnxt_parse_devarg_max_num_kflows(__rte_unused const char *key,
                                            const char *value,
                                            void *opaque_arg) {
  struct bnxt *bp = opaque_arg;
  ...
  if (!value || !opaque_arg) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to max_num_kflows devarg.\n");
    return -EINVAL;
  }
  ...
  if (bnxt_devarg_max_num_kflow_invalid(max_num_kflows)) {
    PMD_DRV_LOG(ERR, "Invalid value passed to max_num_kflows devarg.\n");
    return -EINVAL;
  }
...
static int bnxt_parse_devarg_app_id(__rte_unused const char *key,
                                    const char *value, void *opaque_arg) {
  struct bnxt *bp = opaque_arg;
  ...
  if (!value || !opaque_arg) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to app-id "
                     "devargs.\n");
    return -EINVAL;
  }

  app_id = strtoul(value, &end, 10);
  if (end == NULL || *end != '\0' || (app_id == ULONG_MAX && errno == ERANGE)) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to app_id "
                     "devargs.\n");
    return -EINVAL;
  }
...
static int bnxt_parse_devarg_rep_is_pf(__rte_unused const char *key,
                                       const char *value, void *opaque_arg) {
  ...
  if (!value || !opaque_arg) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to rep_is_pf devargs.\n");
    return -EINVAL;
  }
  ...
  if (BNXT_DEVARG_REP_IS_PF_INVALID(rep_is_pf)) {
    PMD_DRV_LOG(ERR, "Invalid value passed to rep_is_pf devargs.\n");
    return -EINVAL;
  }
...
static int bnxt_parse_devarg_rep_based_pf(__rte_unused const char *key,
                                          const char *value, void *opaque_arg) {
  struct bnxt_representor *vfr_bp = opaque_arg;
  ...
  if (!value || !opaque_arg) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to rep_based_pf "
                     "devargs.\n");
    return -EINVAL;
  }
  ...
  if (BNXT_DEVARG_REP_BASED_PF_INVALID(rep_based_pf)) {
    PMD_DRV_LOG(ERR, "Invalid value passed to rep_based_pf devargs.\n");
    return -EINVAL;
  }
...
static int bnxt_parse_devarg_rep_q_r2f(__rte_unused const char *key,
                                       const char *value, void *opaque_arg) {
  struct bnxt_representor *vfr_bp = opaque_arg;
  ...
  if (!value || !opaque_arg) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to rep_q_r2f "
                     "devargs.\n");
    return -EINVAL;
  }
  ...
  if (BNXT_DEVARG_REP_Q_R2F_INVALID(rep_q_r2f)) {
    PMD_DRV_LOG(ERR, "Invalid value passed to rep_q_r2f devargs.\n");
    return -EINVAL;
  }
...
static int bnxt_parse_devarg_rep_q_f2r(__rte_unused const char *key,
                                       const char *value, void *opaque_arg) {
  struct bnxt_representor *vfr_bp = opaque_arg;
  ...
  if (!value || !opaque_arg) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to rep_q_f2r "
                     "devargs.\n");
    return -EINVAL;
  }
  ...
  if (BNXT_DEVARG_REP_Q_F2R_INVALID(rep_q_f2r)) {
    PMD_DRV_LOG(ERR, "Invalid value passed to rep_q_f2r devargs.\n");
    return -EINVAL;
  }
...
static int bnxt_parse_devarg_rep_fc_r2f(__rte_unused const char *key,
                                        const char *value, void *opaque_arg) {
  struct bnxt_representor *vfr_bp = opaque_arg;
  ...
  if (!value || !opaque_arg) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to rep_fc_r2f "
                     "devargs.\n");
    return -EINVAL;
  }
  ...
  if (BNXT_DEVARG_REP_FC_R2F_INVALID(rep_fc_r2f)) {
    PMD_DRV_LOG(ERR, "Invalid value passed to rep_fc_r2f devargs.\n");
    return -EINVAL;
  }
...
static int bnxt_parse_devarg_rep_fc_f2r(__rte_unused const char *key,
                                        const char *value, void *opaque_arg) {
  struct bnxt_representor *vfr_bp = opaque_arg;
  ...
  if (!value || !opaque_arg) {
    PMD_DRV_LOG(ERR, "Invalid parameter passed to rep_fc_f2r "
                     "devargs.\n");
    return -EINVAL;
  }
  ...
  if (BNXT_DEVARG_REP_FC_F2R_INVALID(rep_fc_f2r)) {
    PMD_DRV_LOG(ERR, "Invalid value passed to rep_fc_f2r devargs.\n");
    return -EINVAL;
  }
...
static int bnxt_parse_dev_args(struct bnxt *bp, struct rte_devargs *devargs) {
...
err:
   ...
   */
  rte_kvargs_process(kvlist, BNXT_DEVARG_APP_ID, bnxt_parse_devarg_app_id, bp);
...
static int bnxt_alloc_switch_domain(struct bnxt *bp) {
  ...
  if (BNXT_PF(bp) || BNXT_VF_IS_TRUSTED(bp)) {
    ...
    if (rc)
      PMD_DRV_LOG(ERR, "Failed to alloc switch domain: %d\n", rc);
    else
      PMD_DRV_LOG(INFO, "Switch domain allocated %d\n", bp->switch_domain_id);
  }
...
static int bnxt_drv_init(struct rte_eth_dev *eth_dev) {
  ...
  if (BNXT_TRUFLOW_EN(bp)) {
    ...
    if (bnxt_cfa_code_dynfield_offset < 0) {
      PMD_DRV_LOG(ERR, "Failed to register mbuf field for TruFlow mark\n");
      return -rte_errno;
    }
  }
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Failed to initialize board rc: %x\n", rc);
    return rc;
  }
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Failed to allocate response buffer rc: %x\n", rc);
    return rc;
  }
...
static int bnxt_dev_init(struct rte_eth_dev *eth_dev,
                         void *params __rte_unused) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  ...
  PMD_DRV_LOG(INFO, "Found %s device at mem %" PRIX64 ", node addr %pM\n",
              DRV_MODULE_NAME, pci_dev->mem_resource[0].phys_addr,
...
static void bnxt_unregister_fc_ctx_mem(struct bnxt *bp) {
  bnxt_hwrm_cfa_counter_cfg(bp, BNXT_DIR_RX, CFA_COUNTER_CFG_IN_COUNTER_TYPE_FC,
                            bp->flow_stat->rx_fc_out_tbl.ctx_id,
                            bp->flow_stat->max_fc, false);

  bnxt_hwrm_cfa_counter_cfg(bp, BNXT_DIR_TX, CFA_COUNTER_CFG_IN_COUNTER_TYPE_FC,
                            bp->flow_stat->tx_fc_out_tbl.ctx_id,
                            bp->flow_stat->max_fc, false);
...
static int bnxt_init_rep_info(struct bnxt *bp) {
  ...
  bp->rep_info = rte_zmalloc("bnxt_rep_info",
                             sizeof(bp->rep_info[0]) * BNXT_MAX_VF_REPS(bp), 0);
  if (!bp->rep_info) {
    PMD_DRV_LOG(ERR, "Failed to alloc memory for rep info\n");
    return -ENOMEM;
  }
  bp->cfa_code_map = rte_zmalloc(
      "bnxt_cfa_code_map", sizeof(*bp->cfa_code_map) * BNXT_MAX_CFA_CODE, 0);
  if (!bp->cfa_code_map) {
...
static int bnxt_rep_port_probe(struct rte_pci_device *pci_dev,
  ...
  if (num_rep >= RTE_MAX_ETHPORTS) {
    PMD_DRV_LOG(ERR, "nb_representor_ports = %d > %d MAX ETHPORTS\n", num_rep,
                RTE_MAX_ETHPORTS);
    return -EINVAL;
  }

  if (!(BNXT_PF(backing_bp) || BNXT_VF_IS_TRUSTED(backing_bp))) {
    PMD_DRV_LOG(ERR, "Not a PF or trusted VF. No Representor support\n");
  ...
  for (i = 0; i < num_rep; i++) {
    ...
    if (kvlist) {
      ...
      ret =
          rte_kvargs_process(kvlist, BNXT_DEVARG_REP_IS_PF,
                             bnxt_parse_devarg_rep_is_pf, (void *)&representor);
      if (ret) {
        ret = -EINVAL;
        goto err;
      }
       ...
       * rep-based-pf=<pf index> rep-is-pf=<VF=0 or PF=1>"
       */
      ret = rte_kvargs_process(kvlist, BNXT_DEVARG_REP_BASED_PF,
      ...
      ret =
          rte_kvargs_process(kvlist, BNXT_DEVARG_REP_Q_R2F,
                             bnxt_parse_devarg_rep_q_r2f, (void *)&representor);
      if (ret) {
        ret = -EINVAL;
        goto err;
      }
      ...
      ret =
          rte_kvargs_process(kvlist, BNXT_DEVARG_REP_Q_F2R,
                             bnxt_parse_devarg_rep_q_f2r, (void *)&representor);
      if (ret) {
        ret = -EINVAL;
        goto err;
      }
    ...
    ret = rte_eth_dev_create(&pci_dev->device, name,
                             sizeof(struct bnxt_representor), NULL, NULL,
                             bnxt_representor_init, &representor);
    if (ret) {
      PMD_DRV_LOG(ERR,
                  "failed to create bnxt vf "
                  "representor %s.",
                  name);
      goto err;
    }
    ...
    backing_bp->rep_info[representor.vf_id].vfr_eth_dev = vf_rep_eth_dev;
    backing_bp->num_reps++;
  }
...
static int bnxt_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  if (backing_eth_dev == NULL) {
    ret = rte_eth_dev_create(&pci_dev->device, pci_dev->device.name,
                             sizeof(struct bnxt), eth_dev_pci_specific_init,
                             pci_dev, bnxt_dev_init, NULL);
...
static int bnxt_pci_remove(struct rte_pci_device *pci_dev) {
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    if (eth_dev->data->dev_flags & RTE_ETH_DEV_REPRESENTOR)
      return rte_eth_dev_destroy(eth_dev, bnxt_representor_uninit);
    else
      return rte_eth_dev_destroy(eth_dev, bnxt_dev_uninit);
  } else {
