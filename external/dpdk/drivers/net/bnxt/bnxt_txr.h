...
static inline uint32_t bnxt_tx_avail(struct bnxt_tx_queue *txq) {
  ...
  return ((txq->tx_ring->tx_ring_struct->ring_size - bnxt_tx_bds_in_hw(txq)) -
          1);
}
...
#define PKT_TX_OIP_TCP_UDP_CKSUM                                               \
  (RTE_MBUF_F_TX_TCP_CKSUM | RTE_MBUF_F_TX_UDP_CKSUM |                         \
   RTE_MBUF_F_TX_OUTER_IP_CKSUM)
#define PKT_TX_OIP_UDP_CKSUM                                                   \
  (RTE_MBUF_F_TX_UDP_CKSUM | RTE_MBUF_F_TX_OUTER_IP_CKSUM)
#define PKT_TX_OIP_TCP_CKSUM                                                   \
  (RTE_MBUF_F_TX_TCP_CKSUM | RTE_MBUF_F_TX_OUTER_IP_CKSUM)
#define PKT_TX_OIP_IIP_CKSUM                                                   \
  (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_OUTER_IP_CKSUM)
#define PKT_TX_TCP_UDP_CKSUM (RTE_MBUF_F_TX_TCP_CKSUM | RTE_MBUF_F_TX_UDP_CKSUM)

#define TX_BD_FLG_TIP_IP_TCP_UDP_CHKSUM                                        \
  (TX_BD_LONG_LFLAGS_TCP_UDP_CHKSUM | TX_BD_LONG_LFLAGS_T_IP_CHKSUM |          \
   TX_BD_LONG_LFLAGS_IP_CHKSUM)
#define TX_BD_FLG_IP_TCP_UDP_CHKSUM                                            \
  (TX_BD_LONG_LFLAGS_TCP_UDP_CHKSUM | TX_BD_LONG_LFLAGS_IP_CHKSUM)
#define TX_BD_FLG_TIP_IP_CHKSUM                                                \
  (TX_BD_LONG_LFLAGS_T_IP_CHKSUM | TX_BD_LONG_LFLAGS_IP_CHKSUM)
#define TX_BD_FLG_TIP_TCP_UDP_CHKSUM                                           \
  (TX_BD_LONG_LFLAGS_TCP_UDP_CHKSUM | TX_BD_LONG_LFLAGS_T_IP_CHKSUM)
