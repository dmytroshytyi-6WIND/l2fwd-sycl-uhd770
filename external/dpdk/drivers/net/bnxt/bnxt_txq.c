...
int bnxt_tx_queue_setup_op(struct rte_eth_dev *eth_dev, uint16_t queue_idx,
  ...
  if (queue_idx >= bnxt_max_rings(bp)) {
    PMD_DRV_LOG(ERR, "Cannot create Tx ring %d. Only %d rings available\n",
  ...
  txq->free = rte_zmalloc_socket(NULL, sizeof(struct rte_mbuf *) * nb_desc,
                                 RTE_CACHE_LINE_SIZE, socket_id);
  if (!txq->free) {
  ...
  txq->offloads = eth_dev->data->dev_conf.txmode.offloads | tx_conf->offloads;
  ...
  if (bnxt_alloc_rings(bp, socket_id, queue_idx, txq, NULL, txq->cp_ring, NULL,
                       "txr")) {
