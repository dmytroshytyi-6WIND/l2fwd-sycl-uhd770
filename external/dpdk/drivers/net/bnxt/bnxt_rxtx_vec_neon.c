...
#define GET_DESC_FIELDS(rxcmp, rxcmp1, shuf_msk, ptype_idx, pkt_idx, ret)      \
  {                                                                            \
    ...
    r = vreinterpretq_u32_u8(                                                  \
        vqtbl1q_u8(vreinterpretq_u8_u32(rxcmp), (shuf_msk)));                  \
                                                                               \
    /* Set packet type. */                                                     \
    ...
    r = vreinterpretq_u32_u16(                                                 \
        vsetq_lane_u16(vlan_tci, vreinterpretq_u16_u32(r), 5));                \
    (ret) = r;                                                                 \
  }

static void descs_to_mbufs(uint32x4_t mm_rxcmp[4], uint32x4_t mm_rxcmp1[4],
  ...
  const uint32x4_t flags_type_mask = vdupq_n_u32(RX_PKT_CMPL_FLAGS_ITYPE_MASK);
  const uint32x4_t flags2_mask1 = vdupq_n_u32(CMPL_FLAGS2_VLAN_TUN_MSK);
  const uint32x4_t flags2_mask2 = vdupq_n_u32(RX_PKT_CMPL_FLAGS2_IP_TYPE);
  ...
  flags_type =
      vreinterpretq_u32_u64(vcombine_u64(vget_low_u64(t0), vget_low_u64(t1)));
  ptype_idx =
      vshrq_n_u32(vandq_u32(flags_type, flags_type_mask),
  ...
  ptype_idx =
      vorrq_u32(ptype_idx, vshrq_n_u32(vandq_u32(flags2, flags2_mask1),
                                       RX_PKT_CMPL_FLAGS2_META_FORMAT_SFT -
                                           BNXT_PTYPE_TBL_VLAN_SFT));
  ptype_idx = vorrq_u32(ptype_idx, vshrq_n_u32(vandq_u32(flags2, flags2_mask2),
...
static uint16_t recv_burst_vec_neon(void *rx_queue, struct rte_mbuf **rx_pkts,
                                    uint16_t nb_pkts) {
  ...
  const uint32x4_t valid_target = vdupq_n_u32(!!(raw_cons & cp_ring_size));
  int i;
  ...
  if (nb_pkts < BNXT_RX_DESCS_PER_LOOP_VEC128) {
    desc_valid_mask >>= 16 * (BNXT_RX_DESCS_PER_LOOP_VEC128 - nb_pkts);
  } else {
    nb_pkts = RTE_ALIGN_FLOOR(nb_pkts, BNXT_RX_DESCS_PER_LOOP_VEC128);
  }
  ...
  for (i = 0; i < nb_pkts; i += BNXT_RX_DESCS_PER_LOOP_VEC128,
     ...
     */
    valid = vget_lane_u64(vreinterpret_u64_u16(vqmovn_u32(info3_v)), 0);
    /*
    ...
    descs_to_mbufs(rxcmp, rxcmp1, mb_init, &rx_pkts[nb_rx_pkts], rxr);
...
uint16_t bnxt_recv_pkts_vec(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_pkts > RTE_BNXT_MAX_RX_BURST) {
    ...
    burst = recv_burst_vec_neon(rx_queue, rx_pkts + cnt, RTE_BNXT_MAX_RX_BURST);
...
static uint16_t bnxt_xmit_fixed_burst_vec(void *tx_queue,
  ...
  while (to_send) {
    ...
    *tx_buf = tx_mbuf;
    ...
    txbd->flags_type =
        bnxt_xmit_flags_len(tx_mbuf->data_len, TX_BD_FLAGS_NOCMPL);
...
uint16_t bnxt_xmit_pkts_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  while (nb_pkts) {
    ...
    ret = bnxt_xmit_fixed_burst_vec(tx_queue, &tx_pkts[nb_sent], num);
    nb_sent += ret;
    nb_pkts -= ret;
    if (ret < num)
      break;
...
int __rte_cold bnxt_rxq_vec_setup(struct bnxt_rx_queue *rxq) {
