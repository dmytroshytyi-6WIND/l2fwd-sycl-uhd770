...
int bnxt_free_int(struct bnxt *bp) {
  ...
  if (irq->requested) {
     ...
     */
    do {
      rc = rte_intr_callback_unregister(intr_handle, irq->handler, bp->eth_dev);
      if (rc >= 0) {
        irq->requested = 0;
        break;
      }
...
int bnxt_setup_int(struct bnxt *bp) {
  ...
  total_vecs = 1;
  bp->irq_tbl =
      rte_calloc("bnxt_irq_tbl", total_vecs, sizeof(struct bnxt_irq), 0);
  if (bp->irq_tbl) {
    for (i = 0; i < total_vecs; i++) {
      bp->irq_tbl[i].vector_idx = i;
      snprintf(bp->irq_tbl[i].name, len, "%s-%d", bp->eth_dev->device->name, i);
...
int bnxt_request_int(struct bnxt *bp) {
  ...
  if (!irq->requested) {
    rc = rte_intr_callback_register(intr_handle, irq->handler, bp->eth_dev);
    if (!rc)
      irq->requested = 1;
  }
