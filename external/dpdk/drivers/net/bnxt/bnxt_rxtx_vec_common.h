...
#define TX_BD_FLAGS_CMPL                                                       \
  ((1 << TX_BD_LONG_FLAGS_BD_CNT_SFT) | TX_BD_SHORT_FLAGS_COAL_NOW |           \
   TX_BD_SHORT_TYPE_TX_BD_SHORT | TX_BD_LONG_FLAGS_PACKET_END)

#define TX_BD_FLAGS_NOCMPL (TX_BD_FLAGS_CMPL | TX_BD_LONG_FLAGS_NO_CMPL)

static inline uint32_t bnxt_xmit_flags_len(uint16_t len, uint16_t flags) {
  switch (len >> 9) {
  case 0:
...
static inline int bnxt_rxq_vec_setup_common(struct bnxt_rx_queue *rxq) {
  uintptr_t p;
...
static inline void bnxt_rxq_rearm(struct bnxt_rx_queue *rxq,
   ...
   */
  nb = RTE_MIN(rxq->rxrearm_nb & ~0x3, rxq->nb_rx_desc - rxq->rxrearm_start);
 ...
 * is enabled.
 */
static inline void bnxt_tx_cmp_vec_fast(struct bnxt_tx_queue *txq,
                                        uint32_t nr_pkts) {
...
static inline void bnxt_tx_cmp_vec(struct bnxt_tx_queue *txq,
                                   uint32_t nr_pkts) {
  struct bnxt_tx_ring_info *txr = txq->tx_ring;
  uint16_t cons, raw_cons = txr->tx_raw_cons;
  ...
  while (nr_pkts) {
    ...
    if (blk) {
      rte_mempool_put_bulk(pool, (void **)&txr->tx_buf_ring[cons], blk);
      memset(&txr->tx_buf_ring[cons], 0, blk * sizeof(struct rte_mbuf *));
      raw_cons += blk;
      nr_pkts -= blk;
    }
    if (!mbuf) {
