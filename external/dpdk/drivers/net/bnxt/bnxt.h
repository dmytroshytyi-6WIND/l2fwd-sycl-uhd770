...
#define BNXT_MAX_PKT_LEN                                                       \
  (BNXT_MAX_MTU + RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN +                      \
...
#define BNXT_TPA_MAX_AGGS(bp)                                                  \
  (BNXT_CHIP_P5(bp) ? TPA_MAX_AGGS_TH : TPA_MAX_AGGS)

#define BNXT_TPA_MAX_SEGS(bp)                                                  \
  (BNXT_CHIP_P5(bp) ? TPA_MAX_SEGS_TH : TPA_MAX_SEGS)
...
#define BNXT_EVENT_ERROR_REPORT_TYPE(data1)                                         \
  (((data1)&HWRM_ASYNC_EVENT_CMPL_ERROR_REPORT_BASE_EVENT_DATA1_ERROR_TYPE_MASK) >> \
...
#define BNXT_LED_ALT_BLINK_CAP(x)                                              \
  ((x)&rte_cpu_to_le_16(HWRM_PORT_LED_QCFG_OUTPUT_LED0_STATE_BLINKALT))
...
#define BNXT_MAX_VF_REPS(bp)                                                   \
  (BNXT_CHIP_P5(bp) ? BNXT_MAX_VF_REPS_TH : BNXT_MAX_VF_REPS_WH)
#define BNXT_TOTAL_VFS(bp) ((bp)->pf->total_vfs)
#define BNXT_FIRST_VF_FID 128
#define BNXT_PF_RINGS_USED(bp) bnxt_get_num_queues(bp)
#define BNXT_PF_RINGS_AVAIL(bp)                                                \
  ((bp)->pf->max_cp_rings - BNXT_PF_RINGS_USED(bp))
  uint16_t port_id;
...
#define BNXT_PTP_MSG_EVENTS                                                    \
  (BNXT_PTP_MSG_SYNC | BNXT_PTP_MSG_DELAY_REQ | BNXT_PTP_MSG_PDELAY_REQ |      \
...
#define BNXT_MAX_TQM_RINGS (BNXT_MAX_TQM_SP_RINGS + BNXT_MAX_TQM_FP_RINGS)
...
#define BNXT_DEV_TX_OFFLOAD_SUPPORT                                            \
  (RTE_ETH_TX_OFFLOAD_IPV4_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_CKSUM |              \
   ...
   RTE_ETH_TX_OFFLOAD_GENEVE_TNL_TSO | RTE_ETH_TX_OFFLOAD_QINQ_INSERT |        \
   RTE_ETH_TX_OFFLOAD_MULTI_SEGS)

#define BNXT_DEV_RX_OFFLOAD_SUPPORT                                            \
  (RTE_ETH_RX_OFFLOAD_VLAN_FILTER | RTE_ETH_RX_OFFLOAD_IPV4_CKSUM |            \
   RTE_ETH_RX_OFFLOAD_UDP_CKSUM | RTE_ETH_RX_OFFLOAD_TCP_CKSUM |               \
   RTE_ETH_RX_OFFLOAD_OUTER_IPV4_CKSUM | RTE_ETH_RX_OFFLOAD_OUTER_UDP_CKSUM |  \
   RTE_ETH_RX_OFFLOAD_KEEP_CRC | RTE_ETH_RX_OFFLOAD_VLAN_EXTEND |              \
...
#define BNXT_MULTIROOT_EN(bp) ((bp)->flags2 & BNXT_FLAGS2_MULTIROOT_EN)
...
#define BNXT_RX_VLAN_STRIP_EN(bp)                                              \
...
static inline uint16_t bnxt_max_rings(struct bnxt *bp) {
   ...
   */
  if (BNXT_STINGRAY(bp)) {
    max_rx_rings = RTE_MIN(RTE_MIN(max_rx_rings / 2U, MAX_STINGRAY_RINGS),
                           bp->max_stat_ctx / 2U);
  } else {
    max_rx_rings = RTE_MIN(max_rx_rings / 2U, bp->max_stat_ctx / 2U);
  }
...
#define BNXT_REP_BASED_PF(vfr_bp) ((vfr_bp)->flags & BNXT_REP_BASED_PF_VALID)
...
#define bnxt_acquire_flow_lock(bp) pthread_mutex_lock(&(bp)->flow_lock)

#define bnxt_release_flow_lock(bp) pthread_mutex_unlock(&(bp)->flow_lock)

#define BNXT_VALID_VNIC_OR_RET(bp, vnic_id)                                    \
  do {                                                                         \
...
#define PMD_DRV_LOG_RAW(level, fmt, args...)                                   \
  rte_log(RTE_LOG_##level, bnxt_logtype_driver, "%s(): " fmt, __func__, ##args)

#define PMD_DRV_LOG(level, fmt, args...) PMD_DRV_LOG_RAW(level, fmt, ##args)
