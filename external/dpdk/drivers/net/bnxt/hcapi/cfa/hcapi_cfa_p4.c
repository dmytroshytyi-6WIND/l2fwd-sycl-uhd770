...
uint64_t hcapi_cfa_p4_key_hash(uint64_t *key_data, uint16_t bitlen) {
  ...
  key0_hash = hcapi_cfa_crc32_hash(((uint8_t *)key_data) + (bitlen / 8) - 1);
...
static int hcapi_cfa_p4_key_hw_op_add(struct hcapi_cfa_hwop *op,
   ...
   * Is entry free?
   */
  memcpy(&table_entry, (uint8_t *)(uintptr_t)op->hw.base_addr + key_obj->offset,
         key_obj->size);
...
static int hcapi_cfa_p4_key_hw_op_del(struct hcapi_cfa_hwop *op,
  ...
  if (table_entry.hdr.word1 & (1 << CFA_P4_EEM_ENTRY_VALID_SHIFT)) {
    ...
    if (key_obj->data != NULL) {
      if (memcmp(&table_entry, key_obj->data, key_obj->size) != 0)
        return -1;
    }
  } else {
    return -1;
   ...
   * Delete entry
   */
  memset((uint8_t *)(uintptr_t)op->hw.base_addr + key_obj->offset, 0,
         key_obj->size);
