...
static int bnxt_flow_args_validate(const struct rte_flow_attr *attr,
...
static int bnxt_filter_type_check(const struct rte_flow_item pattern[],
                                  struct rte_flow_error *error) {
...
static int bnxt_validate_and_parse_flow_type(
    struct bnxt *bp, const struct rte_flow_attr *attr,
  ...
  filter->filter_type =
      use_ntuple ? HWRM_CFA_NTUPLE_FILTER : HWRM_CFA_L2_FILTER;
  en_ethertype = use_ntuple ? NTUPLE_FLTR_ALLOC_INPUT_EN_ETHERTYPE
  ...
  while (item->type != RTE_FLOW_ITEM_TYPE_END) {
    ...
    switch (item->type) {
    case RTE_FLOW_ITEM_TYPE_ANY:
      inner = ((const struct rte_flow_item_any *)item->spec)->num > 3;
      if (inner)
    ...
    case RTE_FLOW_ITEM_TYPE_ETH:
      ...
      if (eth_mask->type && eth_mask->type != RTE_BE16(0xffff)) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
      ...
      if (rte_is_broadcast_ether_addr(&eth_mask->dst)) {
        ...
        if (!rte_is_valid_assigned_ether_addr(dst)) {
          rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                             "DMAC is invalid");
          PMD_DRV_LOG(ERR, "DMAC is invalid!\n");
          return -rte_errno;
        }
        rte_memcpy(filter->dst_macaddr, &eth_spec->dst, RTE_ETHER_ADDR_LEN);
        en |= use_ntuple ? NTUPLE_FLTR_ALLOC_INPUT_EN_DST_MACADDR
                         : EM_FLOW_ALLOC_INPUT_EN_DST_MACADDR;
        valid_flags |= inner ? BNXT_FLOW_L2_INNER_DST_VALID_FLAG
                             : BNXT_FLOW_L2_DST_VALID_FLAG;
        filter->priority = attr->priority;
        PMD_DRV_LOG(DEBUG, "Creating a priority flow\n");
      }
      if (rte_is_broadcast_ether_addr(&eth_mask->src)) {
        ...
        if (!rte_is_valid_assigned_ether_addr(src)) {
          rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                             "SMAC is invalid");
          PMD_DRV_LOG(ERR, "SMAC is invalid!\n");
          return -rte_errno;
        }
        rte_memcpy(filter->src_macaddr, &eth_spec->src, RTE_ETHER_ADDR_LEN);
        en |= use_ntuple ? NTUPLE_FLTR_ALLOC_INPUT_EN_SRC_MACADDR
                         : EM_FLOW_ALLOC_INPUT_EN_SRC_MACADDR;
        valid_flags |= inner ? BNXT_FLOW_L2_INNER_SRC_VALID_FLAG
         ...
         */
      if (eth_mask->type) {
        filter->ethertype = rte_be_to_cpu_16(eth_spec->type);
        en |= en_ethertype;
      }
      if (inner)
    ...
    case RTE_FLOW_ITEM_TYPE_VLAN:
      ...
      if (vlan_mask->tci && vlan_mask->tci == RTE_BE16(0x0fff)) {
        /* Only the VLAN ID can be matched. */
        filter->l2_ovlan = rte_be_to_cpu_16(vlan_spec->tci & RTE_BE16(0x0fff));
      ...
      if (vlan_mask->inner_type && vlan_mask->inner_type != RTE_BE16(0xffff)) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
      ...
      if (vlan_mask->inner_type) {
        filter->ethertype = rte_be_to_cpu_16(vlan_spec->inner_type);
        en |= en_ethertype;
      }
    ...
    case RTE_FLOW_ITEM_TYPE_IPV4:
      ...
      if (ipv4_mask->hdr.src_addr) {
        filter->src_ipaddr_mask[0] = ipv4_mask->hdr.src_addr;
        en |= !use_ntuple ? 0 : NTUPLE_FLTR_ALLOC_INPUT_EN_SRC_IPADDR_MASK;
      }

      if (ipv4_mask->hdr.dst_addr) {
        filter->dst_ipaddr_mask[0] = ipv4_mask->hdr.dst_addr;
        en |= !use_ntuple ? 0 : NTUPLE_FLTR_ALLOC_INPUT_EN_DST_IPADDR_MASK;
      }

      filter->ip_addr_type =
          use_ntuple ? HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_IP_ADDR_TYPE_IPV4
                     : HWRM_CFA_EM_FLOW_ALLOC_INPUT_IP_ADDR_TYPE_IPV4;

      if (ipv4_spec->hdr.next_proto_id) {
        filter->ip_protocol = ipv4_spec->hdr.next_proto_id;
        if (use_ntuple)
    ...
    case RTE_FLOW_ITEM_TYPE_IPV6:
      ...
      rte_memcpy(filter->src_ipaddr, ipv6_spec->hdr.src_addr, 16);
      rte_memcpy(filter->dst_ipaddr, ipv6_spec->hdr.dst_addr, 16);

      if (!bnxt_check_zero_bytes(ipv6_mask->hdr.src_addr, 16)) {
        rte_memcpy(filter->src_ipaddr_mask, ipv6_mask->hdr.src_addr, 16);
        en |= !use_ntuple ? 0 : NTUPLE_FLTR_ALLOC_INPUT_EN_SRC_IPADDR_MASK;
      }

      if (!bnxt_check_zero_bytes(ipv6_mask->hdr.dst_addr, 16)) {
        rte_memcpy(filter->dst_ipaddr_mask, ipv6_mask->hdr.dst_addr, 16);
        en |= !use_ntuple ? 0 : NTUPLE_FLTR_ALLOC_INPUT_EN_DST_IPADDR_MASK;
      }
    ...
    case RTE_FLOW_ITEM_TYPE_TCP:
      ...
      if (use_ntuple)
        ...
        en |= EM_FLOW_ALLOC_INPUT_EN_SRC_PORT | EM_FLOW_ALLOC_INPUT_EN_DST_PORT;

      if (tcp_mask->hdr.dst_port) {
        filter->dst_port_mask = tcp_mask->hdr.dst_port;
        en |= !use_ntuple ? 0 : NTUPLE_FLTR_ALLOC_INPUT_EN_DST_PORT_MASK;
      }

      if (tcp_mask->hdr.src_port) {
        filter->src_port_mask = tcp_mask->hdr.src_port;
        en |= !use_ntuple ? 0 : NTUPLE_FLTR_ALLOC_INPUT_EN_SRC_PORT_MASK;
      }
      break;
    case RTE_FLOW_ITEM_TYPE_UDP:
      udp_spec = item->spec;
    ...
    case RTE_FLOW_ITEM_TYPE_VXLAN:
      ...
      if (vxlan_spec->rsvd1 || vxlan_spec->rsvd0[0] || vxlan_spec->rsvd0[1] ||
          vxlan_spec->rsvd0[2] || vxlan_spec->flags != 0x8) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                           "Invalid VXLAN item");
        return -rte_errno;
      }
      ...
      if (vxlan_mask != NULL) {
        vni_masked = !!memcmp(vxlan_mask->vni, vni_mask, RTE_DIM(vni_mask));
        if (vni_masked) {
          rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                             "Invalid VNI mask");
          return -rte_errno;
        ...
        filter->vni = rte_be_to_cpu_32(tenant_id_be);
        filter->tunnel_type = CFA_NTUPLE_FILTER_ALLOC_REQ_TUNNEL_TYPE_VXLAN;
      }
      break;
    case RTE_FLOW_ITEM_TYPE_NVGRE:
      ...
      if (!nvgre_spec && !nvgre_mask) {
        filter->tunnel_type = CFA_NTUPLE_FILTER_ALLOC_REQ_TUNNEL_TYPE_NVGRE;
        break;
      }
    ...
    case RTE_FLOW_ITEM_TYPE_VF:
      ...
      filter->mirror_vnic_id = dflt_vnic =
          bnxt_hwrm_func_qcfg_vf_dflt_vnic_id(bp, vf);
      if (dflt_vnic < 0) {
...
/* Parse attributes */
static int bnxt_flow_parse_attr(const struct rte_flow_attr *attr,
                                struct rte_flow_error *error) {
...
bnxt_find_matching_l2_filter(struct bnxt *bp, struct bnxt_filter_info *nf) {
  ...
  for (i = bp->max_vnics - 1; i >= 0; i--) {
    ...
    STAILQ_FOREACH(mf, &vnic->filter, next) {
      ...
      if (mf->ethertype == nf->ethertype && mf->l2_ovlan == nf->l2_ovlan &&
          mf->l2_ovlan_mask == nf->l2_ovlan_mask &&
          mf->l2_ivlan == nf->l2_ivlan &&
          mf->l2_ivlan_mask == nf->l2_ivlan_mask &&
          !memcmp(mf->src_macaddr, nf->src_macaddr, RTE_ETHER_ADDR_LEN) &&
          !memcmp(mf->dst_macaddr, nf->dst_macaddr, RTE_ETHER_ADDR_LEN))
        return mf;
    }
  }
  return NULL;
...
bnxt_create_l2_filter(struct bnxt *bp, struct bnxt_filter_info *nf,
  ...
  if (nf->filter_type == HWRM_CFA_L2_FILTER &&
    ...
    filter1->flags |= HWRM_CFA_L2_FILTER_ALLOC_INPUT_FLAGS_SOURCE_VALID;
  ...
  if (nf->priority && (nf->valid_flags & BNXT_FLOW_L2_DST_VALID_FLAG ||
    ...
    if (nf->priority > 65535) {
      filter1->pri_hint = HWRM_CFA_L2_FILTER_ALLOC_INPUT_PRI_HINT_BELOW_FILTER;
  ...
  if (nf->valid_flags &
      (BNXT_FLOW_L2_DST_VALID_FLAG | BNXT_FLOW_L2_SRC_VALID_FLAG |
       BNXT_FLOW_L2_INNER_SRC_VALID_FLAG | BNXT_FLOW_L2_INNER_DST_VALID_FLAG)) {
  ...
  if (nf->valid_flags & BNXT_FLOW_L2_DROP_FLAG) {
    ...
    if (nf->ethertype == RTE_ETHER_TYPE_IPV4) {
      ...
      if (nf->valid_flags & BNXT_FLOW_PARSE_INNER_FLAG) {
        filter1->enables |= L2_FILTER_ALLOC_INPUT_EN_T_NUM_VLANS;
      } else {
        filter1->enables |= L2_FILTER_ALLOC_INPUT_EN_NUM_VLANS;
        filter1->flags |= HWRM_CFA_L2_FILTER_ALLOC_INPUT_FLAGS_OUTERMOST;
      }
    }
  }

  rc = bnxt_hwrm_set_l2_filter(bp, vnic->fw_vnic_id, filter1);
  if (rc) {
...
static int bnxt_vnic_prep(struct bnxt *bp, struct bnxt_vnic_info *vnic,
  ...
  if (bp->nr_vnics > bp->max_vnics - 1)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_GROUP,
                              NULL, "Group id is invalid");

  rc = bnxt_vnic_grp_alloc(bp, vnic);
  if (rc)
    return rte_flow_error_set(error, -rc, RTE_FLOW_ERROR_TYPE_ACTION, act,
                              "Failed to alloc VNIC group");

  rc = bnxt_hwrm_vnic_alloc(bp, vnic);
  if (rc) {
    rte_flow_error_set(error, -rc, RTE_FLOW_ERROR_TYPE_ACTION, act,
...
static int match_vnic_rss_cfg(struct bnxt *bp, struct bnxt_vnic_info *vnic,
  ...
  for (i = 0; i < vnic->rx_queue_cnt; i++) {
    ...
    for (j = 0; j < vnic->rx_queue_cnt; j++) {
      if (bp->grp_info[rss->queue[i]].fw_grp_id == vnic->fw_grp_ids[j])
        match++;
    }
  }

  if (match != vnic->rx_queue_cnt) {
    PMD_DRV_LOG(ERR, "VNIC queue count %d vs queues matched %d\n", match,
                vnic->rx_queue_cnt);
    return -EINVAL;
  }
...
static void bnxt_update_filter_flags_en(struct bnxt_filter_info *filter,
                                        struct bnxt_filter_info *filter1,
                                        int use_ntuple) {
  if (!use_ntuple &&
      !(filter->valid_flags &
        ~(BNXT_FLOW_L2_DST_VALID_FLAG | BNXT_FLOW_L2_SRC_VALID_FLAG |
          ...
          BNXT_FLOW_L2_INNER_DST_VALID_FLAG | BNXT_FLOW_L2_DROP_FLAG |
...
static int bnxt_validate_rss_action(const struct rte_flow_action actions[]) {
...
static int bnxt_get_vnic(struct bnxt *bp, uint32_t group) {
  int vnic_id = 0;
...
static int bnxt_vnic_rss_cfg_update(struct bnxt *bp,
                                    struct bnxt_vnic_info *vnic,
                                    const struct rte_flow_action *act,
                                    struct rte_flow_error *error) {
  const struct rte_flow_action_rss *rss;
  ...
  if (rss->func != RTE_ETH_HASH_FUNCTION_DEFAULT &&
    ...
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, act,
  ...
  vnic->hash_mode = bnxt_rte_to_hwrm_hash_level(bp, rss->types, rss->level);
  ...
  for (i = 0; i < rss->queue_num; i++) {
    ...
    if (rss->queue[i] >= bp->rx_nr_rings || !bp->rx_queues[rss->queue[i]]) {
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, act,
  ...
  for (rss_idx = 0; rss_idx < HW_HASH_INDEX_SIZE; rss_idx++) {
    ...
    if (BNXT_CHIP_P5(bp)) {
      ...
      vnic->rss_table[rss_idx * 2] = rxq->rx_ring->rx_ring_struct->fw_ring_id;
...
static int bnxt_validate_and_parse_flow(struct rte_eth_dev *dev,
                                        const struct rte_flow_item pattern[],
                                        const struct rte_flow_action actions[],
                                        const struct rte_flow_attr *attr,
                                        struct rte_flow_error *error,
                                        struct bnxt_filter_info *filter) {
  const struct rte_flow_action *act = bnxt_flow_non_void_action(actions);
  ...
  rc = bnxt_validate_and_parse_flow_type(bp, attr, pattern, error, filter);
  if (rc != 0)
    goto ret;
...
start:
  ...
  case RTE_FLOW_ACTION_TYPE_QUEUE:
    ...
    if (vnic->rx_queue_cnt) {
      if (vnic->start_grp_id != act_q->index) {
        PMD_DRV_LOG(ERR, "VNIC already in use\n");
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, act,
                           "VNIC already in use");
        rc = -rte_errno;
        goto ret;
    ...
    if (!rxq) {
      PMD_DRV_LOG(ERR, "Queue invalid or used with other VNIC\n");
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, act,
    ...
    PMD_DRV_LOG(DEBUG, "vnic[%d] = %p vnic->fw_grp_ids = %p\n", act_q->index,
                vnic, vnic->fw_grp_ids);

  use_vnic:
    vnic->ff_pool_idx = vnic_id;
    PMD_DRV_LOG(DEBUG, "Setting vnic ff_idx %d\n", vnic->ff_pool_idx);
    ...
    if (filter1 == NULL) {
      rte_flow_error_set(error, ENOSPC, RTE_FLOW_ERROR_TYPE_ACTION, act,
  ...
  case RTE_FLOW_ACTION_TYPE_DROP:
    ...
    if (filter->filter_type == HWRM_CFA_EM_FILTER)
      filter->flags = HWRM_CFA_EM_FLOW_ALLOC_INPUT_FLAGS_DROP;
  ...
  case RTE_FLOW_ACTION_TYPE_VF:
    ...
    if (filter->tunnel_type == CFA_NTUPLE_FILTER_ALLOC_REQ_TUNNEL_TYPE_VXLAN ||
        filter->tunnel_type == CFA_NTUPLE_FILTER_ALLOC_REQ_TUNNEL_TYPE_IPGRE) {
  ...
  case RTE_FLOW_ACTION_TYPE_RSS:
    ...
    if (vnic->rx_queue_cnt) {
      ...
      if (rc) {
        PMD_DRV_LOG(ERR, "VNIC and RSS config mismatch\n");
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, act,
    ...
    for (i = 0; i < rss->queue_num; i++) {
      ...
      if (!rss->queue[i] || rss->queue[i] >= bp->rx_nr_rings ||
      ...
      if (bp->vnic_info[0].fw_grp_ids[rss->queue[i]] != INVALID_HW_RING_ID) {
        PMD_DRV_LOG(ERR, "queue active with other VNIC\n");
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, act,
                           "Invalid queue ID for RSS");
        rc = -rte_errno;
        goto ret;
    ...
    vnic->ff_pool_idx = vnic_id;
    PMD_DRV_LOG(DEBUG, "Setting vnic ff_pool_idx %d\n", vnic->ff_pool_idx);

    /* This can be done only after vnic_grp_alloc is done. */
    for (i = 0; i < vnic->rx_queue_cnt; i++) {
      vnic->fw_grp_ids[i] = bp->grp_info[rss->queue[i]].fw_grp_id;
      /* Make sure vnic0 does not use these rings. */
      bp->vnic_info[0].fw_grp_ids[rss->queue[i]] = INVALID_HW_RING_ID;
    }

    for (rss_idx = 0; rss_idx < HW_HASH_INDEX_SIZE;) {
      for (i = 0; i < vnic->rx_queue_cnt; i++)
        vnic->rss_table[rss_idx++] = vnic->fw_grp_ids[i];
    }

    /* Configure RSS only if the queue count is > 1 */
    if (vnic->rx_queue_cnt > 1) {
      vnic->hash_type = bnxt_rte_to_hwrm_hash_types(rss->types);
      vnic->hash_mode = bnxt_rte_to_hwrm_hash_level(bp, rss->types, rss->level);
      ...
      if (!rss->key_len) {
        ...
        if (rss->key_len > HW_HASH_KEY_SIZE)
          memcpy(vnic->rss_hash_key, rss->key, HW_HASH_KEY_SIZE);
        else
          memcpy(vnic->rss_hash_key, rss->key, rss->key_len);
      }
  ...
  case RTE_FLOW_ACTION_TYPE_MARK:
    if (bp->mark_table == NULL) {
      rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_ACTION, act,
    ...
    if (bp->flags & BNXT_FLAG_RX_VECTOR_PKT_MODE) {
      PMD_DRV_LOG(DEBUG, "Disabling vector processing for mark\n");
      bp->eth_dev->rx_pkt_burst = bnxt_recv_pkts;
      bp->flags &= ~BNXT_FLAG_RX_VECTOR_PKT_MODE;
    }

    filter->valid_flags |= BNXT_FLOW_MARK_FLAG;
    filter->mark = ((const struct rte_flow_action_mark *)act->conf)->id;
...
find_matching_vnic(struct bnxt *bp, struct bnxt_filter_info *filter) {
...
static int bnxt_flow_validate(struct rte_eth_dev *dev,
                              const struct rte_flow_attr *attr,
                              const struct rte_flow_item pattern[],
                              const struct rte_flow_action actions[],
                              struct rte_flow_error *error) {
  ...
  if (filter == NULL) {
    rte_flow_error_set(error, ENOSPC, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
  ...
  ret =
      bnxt_validate_and_parse_flow(dev, pattern, actions, attr, error, filter);
  if (ret)
    goto exit;
...
static void bnxt_update_filter(struct bnxt *bp,
                               struct bnxt_filter_info *old_filter,
...
static int bnxt_match_filter(struct bnxt *bp, struct bnxt_filter_info *nf) {
  ...
  for (i = bp->max_vnics - 1; i >= 0; i--) {
    ...
    STAILQ_FOREACH(flow, &vnic->flow_list, next) {
      ...
      if (mf->filter_type == nf->filter_type && mf->flags == nf->flags &&
          ...
          mf->ethertype == nf->ethertype && mf->vni == nf->vni &&
          mf->tunnel_type == nf->tunnel_type && mf->l2_ovlan == nf->l2_ovlan &&
          mf->l2_ovlan_mask == nf->l2_ovlan_mask &&
          mf->l2_ivlan == nf->l2_ivlan &&
          mf->l2_ivlan_mask == nf->l2_ivlan_mask &&
          !memcmp(mf->l2_addr, nf->l2_addr, RTE_ETHER_ADDR_LEN) &&
          !memcmp(mf->l2_addr_mask, nf->l2_addr_mask, RTE_ETHER_ADDR_LEN) &&
          !memcmp(mf->src_macaddr, nf->src_macaddr, RTE_ETHER_ADDR_LEN) &&
          !memcmp(mf->dst_macaddr, nf->dst_macaddr, RTE_ETHER_ADDR_LEN) &&
          !memcmp(mf->src_ipaddr, nf->src_ipaddr, sizeof(nf->src_ipaddr)) &&
          !memcmp(mf->src_ipaddr_mask, nf->src_ipaddr_mask,
                  sizeof(nf->src_ipaddr_mask)) &&
          !memcmp(mf->dst_ipaddr, nf->dst_ipaddr, sizeof(nf->dst_ipaddr)) &&
        ...
        STAILQ_REMOVE(&vnic->filter, mf, bnxt_filter_info, next);
...
static void bnxt_setup_flow_counter(struct bnxt *bp) {
  if (bp->fw_cap & BNXT_FW_CAP_ADV_FLOW_COUNTERS &&
      !(bp->flags & BNXT_FLAG_FC_THREAD) && BNXT_FLOW_XSTATS_EN(bp)) {
    rte_eal_alarm_set(US_PER_S * BNXT_FC_TIMER, bnxt_flow_cnt_alarm_cb,
                      (void *)bp);
...
static struct rte_flow *bnxt_flow_create(struct rte_eth_dev *dev,
                                         const struct rte_flow_attr *attr,
                                         const struct rte_flow_item pattern[],
                                         const struct rte_flow_action actions[],
                                         struct rte_flow_error *error) {
  ...
  if (filter->filter_type == HWRM_CFA_TUNNEL_REDIRECT_FILTER &&
    ...
    if (tun_type == (1U << filter->tunnel_type)) {
      ...
      if (ret) {
        PMD_DRV_LOG(ERR, "Unable to free existing tunnel\n");
        rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
  ...
  if (filter->filter_type == HWRM_CFA_EM_FILTER) {
    filter->enables |= HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_L2_FILTER_ID;
  ...
  if (filter->filter_type == HWRM_CFA_NTUPLE_FILTER) {
    filter->enables |= HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_L2_FILTER_ID;
  ...
  if (!ret || update_flow) {
    ...
    if (filter->valid_flags & BNXT_FLOW_MARK_FLAG) {
      PMD_DRV_LOG(DEBUG, "Mark action: mark id 0x%x, flow id 0x%x\n",
...
static int bnxt_handle_tunnel_redirect_destroy(struct bnxt *bp,
  ...
  if (tun_type == (1U << filter->tunnel_type)) {
    ret = bnxt_hwrm_tunnel_redirect_info(bp, filter->tunnel_type, &tun_dst_fid);
    if (ret) {
      rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
    ...
    if (bp->fw_fid != (tun_dst_fid + bp->first_vf_id)) {
      PMD_DRV_LOG(ERR, "Tunnel does not belong to this VF, skip "
...
static int _bnxt_flow_destroy(struct bnxt *bp, struct rte_flow *flow,
                              struct rte_flow_error *error) {
  struct bnxt_filter_info *filter;
  struct bnxt_vnic_info *vnic;
  int ret = 0;
  ...
  if (filter->valid_flags & BNXT_FLOW_MARK_FLAG) {
    flow_id = filter->flow_id & BNXT_FLOW_ID_MASK;
    memset(&bp->mark_table[flow_id], 0, sizeof(bp->mark_table[flow_id]));
    filter->flow_id = 0;
  }
  ...
  if (!ret) {
     ...
     */
    if (vnic && !vnic->func_default && STAILQ_EMPTY(&vnic->flow_list)) {
      bnxt_vnic_cleanup(bp, vnic);
      bp->nr_vnics--;
    }
  } else {
...
static int bnxt_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                             struct rte_flow_error *error) {
  struct bnxt *bp = dev->data->dev_private;
  int ret = 0;
...
static int bnxt_flow_flush(struct rte_eth_dev *dev,
                           struct rte_flow_error *error) {
  struct bnxt *bp = dev->data->dev_private;
  struct bnxt_vnic_info *vnic;
  struct rte_flow *flow;
