 ...
 */
uint32_t ulp_regfile_init(struct ulp_regfile *regfile) {
 ...
 */
uint32_t ulp_regfile_read(struct ulp_regfile *regfile,
                          enum bnxt_ulp_rf_idx field, uint64_t *data) {
  /* validate the arguments */
 ...
 */
int32_t ulp_regfile_write(struct ulp_regfile *regfile,
                          enum bnxt_ulp_rf_idx field, uint64_t data) {
  /* validate the arguments */
  if (!regfile || field >= BNXT_ULP_RF_IDX_LAST) {
    BNXT_TF_DBG(ERR, "invalid argument\n");
...
static void ulp_bs_put_msb(uint8_t *bs, uint16_t bitpos, uint8_t bitlen,
                           uint8_t val) {
...
static void ulp_bs_put_lsb(uint8_t *bs, uint16_t bitpos, uint8_t bitlen,
                           uint8_t val) {
  uint8_t bitoffs = bitpos % 8;
  uint16_t index = bitpos / 8;
  uint8_t mask;
 ...
 */
uint32_t ulp_bs_push_lsb(uint8_t *bs, uint16_t pos, uint8_t len, uint8_t *val) {
  int i;
  int cnt = (len) / 8;
  int tlen = len;
 ...
 * returns the number of bits pushed.
 */
uint32_t ulp_bs_push_msb(uint8_t *bs, uint16_t pos, uint8_t len, uint8_t *val) {
  int i;
  int cnt = (len + 7) / 8;
 ...
 */
uint32_t ulp_blob_init(struct ulp_blob *blob, uint16_t bitlen,
...
uint32_t ulp_blob_push(struct ulp_blob *blob, uint8_t *data, uint32_t datalen) {
  uint32_t rc;
  ...
  if (blob->byte_order == BNXT_ULP_BYTE_ORDER_BE)
    rc = ulp_bs_push_msb(blob->data, blob->write_idx, datalen, data);
  else
    rc = ulp_bs_push_lsb(blob->data, blob->write_idx, datalen, data);
  if (!rc) {
 ...
 * NULL returned on error.
 */
uint32_t ulp_blob_insert(struct ulp_blob *blob, uint32_t offset, uint8_t *data,
                         uint32_t datalen) {
  uint32_t rc;
 ...
 */
uint8_t *ulp_blob_push_64(struct ulp_blob *blob, uint64_t *data,
                          uint32_t datalen) {
  ...
  if (!blob || !data || datalen > (uint32_t)(blob->bitlen - blob->write_idx)) {
    BNXT_TF_DBG(ERR, "invalid argument\n");
    return 0;
  }
 ...
 * NULL returned on error, pointer pushed value otherwise.
 */
uint8_t *ulp_blob_push_32(struct ulp_blob *blob, uint32_t *data,
                          uint32_t datalen) {
  uint8_t *val = (uint8_t *)data;
  uint32_t rc;
 ...
 * NULL returned on error, pointer pushed value otherwise.
 */
int32_t ulp_blob_push_encap(struct ulp_blob *blob, uint8_t *data,
                            uint32_t datalen) {
  uint8_t *val = (uint8_t *)data;
 ...
 */
int32_t ulp_blob_pad_push(struct ulp_blob *blob, uint32_t datalen) {
 ...
 * returns the number of pad bits added, -1 on failure
 */
int32_t ulp_blob_pad_align(struct ulp_blob *blob, uint32_t align) {
  int32_t pad = 0;
...
static void ulp_bs_get_lsb(uint8_t *src, uint16_t bitpos, uint8_t bitlen,
                           uint8_t *dst) {
 ...
 * returns None.
 */
void ulp_bs_pull_lsb(uint8_t *src, uint8_t *dst, uint32_t size, uint32_t offset,
                     uint32_t len) {
  uint32_t idx;
  ...
  for (idx = 0; idx < cnt; idx++) {
    ulp_bs_get_lsb(src, offset, ULP_BLOB_BYTE, &dst[size - 1 - idx]);
...
static void ulp_bs_get_msb(uint8_t *src, uint16_t bitpos, uint8_t bitlen,
                           uint8_t *dst) {
  uint8_t bitoffs = bitpos % ULP_BLOB_BYTE;
  uint16_t index = ULP_BITS_2_BYTE_NR(bitpos);
  uint8_t mask;
 ...
 * returns None.
 */
void ulp_bs_pull_msb(uint8_t *src, uint8_t *dst, uint32_t offset,
                     uint32_t len) {
  uint32_t idx;
  uint32_t cnt = ULP_BITS_2_BYTE_NR(len);
 ...
 */
int32_t ulp_blob_pull(struct ulp_blob *blob, uint8_t *data, uint32_t data_size,
 ...
 */
uint8_t *ulp_blob_data_get(struct ulp_blob *blob, uint16_t *datalen) {
  /* validate the arguments */
  if (!blob) {
    BNXT_TF_DBG(ERR, "invalid argument\n");
    return NULL; /* failure */
 ...
 */
uint16_t ulp_blob_data_len_get(struct ulp_blob *blob) {
  /* validate the arguments */
  if (!blob) {
    BNXT_TF_DBG(ERR, "invalid argument\n");
    return 0; /* failure */
 ...
 * returns void.
 */
void ulp_blob_encap_swap_idx_set(struct ulp_blob *blob) {
  /* validate the arguments */
  if (!blob) {
    BNXT_TF_DBG(ERR, "invalid argument\n");
    return; /* failure */
 ...
 * returns void.
 */
void ulp_blob_perform_encap_swap(struct ulp_blob *blob) {
 ...
 * returns void.
 */
void ulp_blob_perform_byte_reverse(struct ulp_blob *blob, uint32_t chunk_size) {
 ...
 * returns void.
 */
void ulp_blob_perform_64B_word_swap(struct ulp_blob *blob) {
  uint32_t i, j, num;
  uint8_t xchar;
 ...
 * returns void.
 */
void ulp_blob_perform_64B_byte_swap(struct ulp_blob *blob) {
  uint32_t i, j, num;
  uint8_t xchar;
...
static int32_t ulp_blob_msb_block_merge(struct ulp_blob *dst,
                                        struct ulp_blob *src,
  ...
  for (i = 0; i < num;) {
    if (((dst->write_idx % block_size) + (num - i)) > block_size)
      write_bytes = block_size - (dst->write_idx % block_size);
    else
      write_bytes = num - i;
    for (k = 0; k < ULP_BITS_2_BYTE_NR(write_bytes); k++) {
      ulp_bs_put_msb(dst->data, dst->write_idx, ULP_BLOB_BYTE, *src_buf);
      dst->write_idx += ULP_BLOB_BYTE;
      src_buf++;
    }
    remaining = write_bytes % ULP_BLOB_BYTE;
    if (remaining) {
      bluff = (*src_buf) & ((uint8_t)-1 << (ULP_BLOB_BYTE - remaining));
      ulp_bs_put_msb(dst->data, dst->write_idx, ULP_BLOB_BYTE, bluff);
    ...
    if (write_bytes != (num - i)) {
      ...
      if (remaining) {
        ulp_bs_put_msb(dst->data, dst->write_idx, ULP_BLOB_BYTE - remaining,
                       *src_buf);
 ...
 * returns 0 on success.
 */
int32_t ulp_blob_block_merge(struct ulp_blob *dst, struct ulp_blob *src,
                             uint32_t block_size, uint32_t pad) {
...
int32_t ulp_blob_append(struct ulp_blob *dst, struct ulp_blob *src,
  ...
  if (remaining) {
    bluff = src_buf[src_offset / ULP_BLOB_BYTE] &
            ((uint8_t)-1 >> (ULP_BLOB_BYTE - remaining));
    ulp_bs_put_msb(dst->data, dst->write_idx, ULP_BLOB_BYTE, bluff);
    dst->write_idx += remaining;
 ...
 * returns 0 on success.
 */
int32_t ulp_blob_buffer_copy(struct ulp_blob *dst, struct ulp_blob *src) {
  ...
  memcpy(&dst->data[ULP_BITS_2_BYTE_NR(dst->write_idx)], src->data,
         ULP_BITS_2_BYTE_NR(src->write_idx));
 ...
 */
uint16_t ulp_operand_read(uint8_t *operand, uint8_t *val, uint16_t bytes) {
  /* validate the arguments */
  if (!operand || !val) {
    BNXT_TF_DBG(ERR, "invalid argument\n");
    return 0; /* failure */
