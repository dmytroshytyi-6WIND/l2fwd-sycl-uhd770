...
int32_t ulp_flow_db_fid_free(struct bnxt_ulp_context *ulp_ctxt,
                             enum bnxt_ulp_fdb_type tbl_idx, uint32_t fid);
 ...
 * returns 0 on success or negative number on failure
 */
int32_t ulp_flow_db_flush_flows(struct bnxt_ulp_context *ulp_ctx, uint32_t idx);
 ...
 * returns zero on success and negative on failure.
 */
int32_t ulp_flow_db_parent_flow_idx_get(struct bnxt_ulp_context *ulp_ctxt,
                                        uint32_t parent_fid,
                                        uint32_t *parent_idx);
 ...
 */
int32_t ulp_flow_db_parent_child_flow_next_entry_get(
    struct bnxt_ulp_flow_db *flow_db, uint32_t parent_idx, uint32_t *child_fid);
