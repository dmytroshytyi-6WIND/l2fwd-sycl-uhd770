...
static int32_t ulp_bit_alloc_list_alloc(struct bit_alloc_list *blist,
                                        uint32_t *index) {
  uint64_t bentry;
...
static int32_t ulp_bit_alloc_list_dealloc(struct bit_alloc_list *blist,
                                          uint32_t index) {
  uint32_t idx = 0, jdx;
  uint32_t bsize_64 = blist->bsize / ULP_64B_IN_BYTES;
 ...
 */
int32_t ulp_gen_hash_tbl_list_init(struct ulp_hash_create_params *cparams,
  ...
  hash_tbl =
      rte_zmalloc("Generic hash table", sizeof(struct ulp_gen_hash_tbl), 0);
  if (!hash_tbl) {
  ...
  *hash_table = hash_tbl;
  ...
  hash_tbl->key_tbl.mem_size =
      cparams->key_size * (cparams->num_key_entries + 1);
  hash_tbl->key_tbl.key_data =
      rte_zmalloc("Generic hash keys", hash_tbl->key_tbl.mem_size, 0);
  ...
  hash_tbl->hash_list =
      rte_zmalloc("Generic hash table list", size, ULP_BUFFER_ALIGN_64_BYTE);
  ...
  hash_tbl->bit_list.bdata =
      rte_zmalloc("Generic hash bit alloc", size, ULP_BUFFER_ALIGN_64_BYTE);
 ...
 * returns 0 on success
 */
int32_t ulp_gen_hash_tbl_list_deinit(struct ulp_gen_hash_tbl *hash_tbl) {
  if (!hash_tbl)
    return -EINVAL;
...
ulp_gen_hash_tbl_list_key_search(struct ulp_gen_hash_tbl *hash_tbl,
  ...
  /* validate the arguments */
  if (!hash_tbl || !entry || !entry->key_data ||
      entry->key_length != hash_tbl->key_tbl.data_size) {
    BNXT_TF_DBG(ERR, "invalid arguments\n");
    return -EINVAL;
  }

  /* calculate the hash */
  hash_id = tf_hash_calc_crc32(entry->key_data, hash_tbl->key_tbl.data_size);
  ...
  for (idx = 0; idx < (hash_tbl->hash_bkt_num * ULP_HASH_BUCKET_ROW_SZ);
    ...
    if (ULP_HASH_BUCKET_INUSE(bucket)) {
      ...
      if (!memcmp(entry->key_data,
                  &hash_tbl->key_tbl
                       .key_data[key_idx * hash_tbl->key_tbl.data_size],
                  hash_tbl->key_tbl.data_size)) {
        /* Found the entry */
        entry->search_flag = ULP_GEN_HASH_SEARCH_FOUND;
        entry->hash_index = ULP_HASH_INDEX_CALC(hash_id, idx);
 ...
 *
 */
int32_t ulp_gen_hash_tbl_list_add(struct ulp_gen_hash_tbl *hash_tbl,
                                  struct ulp_gen_hash_entry_params *entry) {
  int32_t rc = 0;
  uint16_t *bucket;
 ...
 * returns 0 on success
 */
int32_t ulp_gen_hash_tbl_list_del(struct ulp_gen_hash_tbl *hash_tbl,
                                  struct ulp_gen_hash_entry_params *entry) {
  uint16_t *bucket;
  uint32_t idx, key_index;
  ...
  if (ulp_bit_alloc_list_dealloc(&hash_tbl->bit_list, key_index)) {
