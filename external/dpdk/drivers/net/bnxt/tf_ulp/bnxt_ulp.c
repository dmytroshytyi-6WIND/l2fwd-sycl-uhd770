...
STAILQ_HEAD(, bnxt_ulp_session_state)
bnxt_ulp_session_list = STAILQ_HEAD_INITIALIZER(bnxt_ulp_session_list);
 ...
 * created the session.
 */
bool ulp_ctx_deinit_allowed(struct bnxt_ulp_context *ulp_ctx) {
...
static int32_t bnxt_ulp_devid_get(struct bnxt *bp,
...
bnxt_ulp_named_resources_calc(struct bnxt_ulp_context *ulp_ctx,
                              ...
                              uint32_t num, struct tf_session_resources *res) {
...
static int32_t bnxt_ulp_unnamed_resources_calc(
    struct bnxt_ulp_context *ulp_ctx, struct bnxt_ulp_resource_resv_info *info,
    uint32_t num, struct tf_session_resources *res) {
...
static int32_t bnxt_ulp_tf_resources_get(struct bnxt_ulp_context *ulp_ctx,
...
int32_t bnxt_ulp_cntxt_app_caps_init(struct bnxt *bp, uint8_t app_id,
                                     uint32_t dev_id) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (info[i].flags & BNXT_ULP_APP_CAP_SHARED_EN)
      ulp_ctx->cfg_data->ulp_flags |= BNXT_ULP_SHARED_SESSION_ENABLED;
    ...
    if (info[i].flags & BNXT_ULP_APP_CAP_UNICAST_ONLY)
      ulp_ctx->cfg_data->ulp_flags |= BNXT_ULP_APP_UNICAST_ONLY;
    if (info[i].flags & BNXT_ULP_APP_CAP_SOCKET_DIRECT) {
      /* Enable socket direction only if MR is enabled in fw*/
      if (BNXT_MULTIROOT_EN(bp)) {
        ulp_ctx->cfg_data->ulp_flags |= BNXT_ULP_APP_SOCKET_DIRECT;
        BNXT_TF_DBG(DEBUG, "Socket Direct feature is enabled");
      }
    }
  }
  if (!found) {
...
ulp_ctx_shared_session_open(struct bnxt *bp,
  ...
  rc =
      rte_eth_dev_get_name_by_port(ethdev->data->port_id, parms.ctrl_chan_name);
  if (rc) {
   ...
   * terminator
   */
  copy_nbytes = sizeof(parms.ctrl_chan_name) - strlen(parms.ctrl_chan_name) - 1;
  ...
  if (bnxt_ulp_cntxt_ha_enabled(bp->ulp_ctx))
    strncat(parms.ctrl_chan_name, "-tf_shared-wc_tcam", copy_nbytes);
  else
 ...
 */
static int32_t ulp_ctx_session_open(struct bnxt *bp,
                                    struct bnxt_ulp_session_state *session) {
  struct rte_eth_dev *ethdev = bp->eth_dev;
  int32_t rc = 0;
  ...
  if (!session->session_opened) {
    ...
    session->g_tfp = rte_zmalloc("bnxt_ulp_session_tfp", sizeof(struct tf), 0);
...
static void bnxt_init_tbl_scope_parms(struct bnxt *bp,
  ...
  if (!dparms) {
    ...
    params->rx_max_action_entry_sz_in_bits = BNXT_ULP_DFLT_RX_MAX_ACTN_ENTRY;
    ...
    params->tx_max_action_entry_sz_in_bits = BNXT_ULP_DFLT_TX_MAX_ACTN_ENTRY;
    ...
    params->rx_max_action_entry_sz_in_bits = BNXT_ULP_DFLT_RX_MAX_ACTN_ENTRY;
    params->rx_mem_size_in_mb = BNXT_ULP_DFLT_RX_MEM;
    params->rx_num_flows_in_k = dparms->ext_flow_db_num_entries / 1024;

    params->tx_max_key_sz_in_bits = BNXT_ULP_DFLT_TX_MAX_KEY;
    params->tx_max_action_entry_sz_in_bits = BNXT_ULP_DFLT_TX_MAX_ACTN_ENTRY;
    params->tx_mem_size_in_mb = BNXT_ULP_DFLT_TX_MEM;
    params->tx_num_flows_in_k = dparms->ext_flow_db_num_entries / 1024;
  }
...
static int32_t ulp_eem_tbl_scope_init(struct bnxt *bp) {
...
static int32_t ulp_eem_tbl_scope_deinit(struct bnxt *bp,
...
static int32_t ulp_ctx_deinit(struct bnxt *bp,
                              struct bnxt_ulp_session_state *session) {
...
static int32_t ulp_ctx_init(struct bnxt *bp,
                            struct bnxt_ulp_session_state *session) {
  ...
  ulp_data = rte_zmalloc("bnxt_ulp_data", sizeof(struct bnxt_ulp_data), 0);
  if (!ulp_data) {
...
static int32_t ulp_dparms_init(struct bnxt *bp,
                               struct bnxt_ulp_context *ulp_ctx) {
  struct bnxt_ulp_device_params *dparms;
  ...
  if (!bp->max_num_kflows) {
    ...
    bnxt_ulp_cntxt_mem_type_set(ulp_ctx, BNXT_ULP_FLOW_MEM_TYPE_INT);
    return 0;
  }
...
static int32_t ulp_ctx_attach(struct bnxt *bp,
                              struct bnxt_ulp_session_state *session) {
  int32_t rc = 0;
...
static void ulp_ctx_detach(struct bnxt *bp) {
  if (bp->tfp.session) {
    tf_close_session(&bp->tfp);
    bp->tfp.session = NULL;
  }
 ...
 */
static void ulp_context_initialized(struct bnxt_ulp_session_state *session,
                                    bool *init) {
...
ulp_get_session(struct bnxt *bp, struct rte_pci_addr *pci_addr) {
  ...
  STAILQ_FOREACH(session, &bnxt_ulp_session_list, next) {
    if (session->pci_info.domain == pci_addr->domain &&
        (BNXT_MULTIROOT_EN(bp) || session->pci_info.bus == pci_addr->bus)) {
      return session;
    }
  }
  return NULL;
 ...
 */
static struct bnxt_ulp_session_state *ulp_session_init(struct bnxt *bp,
                                                       bool *init) {
  ...
  if (!session) {
    ...
    session = rte_zmalloc("bnxt_ulp_session",
                          sizeof(struct bnxt_ulp_session_state), 0);
    if (!session) {
      BNXT_TF_DBG(ERR, "Allocation failed for bnxt_ulp_session\n");
      ...
      STAILQ_INSERT_TAIL(&bnxt_ulp_session_list, session, next);
    }
  }
 ...
 * session list.
 */
static void ulp_session_deinit(struct bnxt_ulp_session_state *session) {
  if (!session)
    return;

  if (!session->cfg_data) {
    pthread_mutex_lock(&bnxt_ulp_global_mutex);
    STAILQ_REMOVE(&bnxt_ulp_session_list, session, bnxt_ulp_session_state,
                  next);
 ...
 */
static int32_t bnxt_ulp_global_cfg_update(struct bnxt *bp, enum tf_dir dir,
  ...
  parms.dir = dir, parms.type = type, parms.offset = offset,
...
static void bnxt_ulp_flush_port_flows(struct bnxt *bp) {
  uint16_t func_id;

  /* it is assumed that port is either TVF or PF */
  if (ulp_port_db_port_func_id_get(bp->ulp_ctx, bp->eth_dev->data->port_id,
                                   &func_id)) {
...
static void bnxt_ulp_destroy_vfr_default_rules(struct bnxt *bp, bool global) {
  ...
  for (port_id = 0; port_id < RTE_MAX_ETHPORTS; port_id++) {
    ...
    if (!global && info->parent_port_id != bp->eth_dev->data->port_id)
      continue;
 ...
 */
static void bnxt_ulp_deinit(struct bnxt *bp,
                            struct bnxt_ulp_session_state *session) {
  bool ha_enabled;
 ...
 * infrastructure associated with it.
 */
static int32_t bnxt_ulp_init(struct bnxt *bp,
                             struct bnxt_ulp_session_state *session) {
  int rc;
...
int32_t bnxt_ulp_port_init(struct bnxt *bp) {
  ...
  if (!BNXT_TRUFLOW_EN(bp)) {
    BNXT_TF_DBG(DEBUG, "Skip ulp init for port: %d, TF is not enabled\n",
                bp->eth_dev->data->port_id);
    return rc;
  }

  if (!BNXT_PF(bp) && !BNXT_VF_IS_TRUSTED(bp)) {
    BNXT_TF_DBG(DEBUG, "Skip ulp init for port: %d, not a TVF or PF\n",
                bp->eth_dev->data->port_id);
    return rc;
  }
  ...
  bp->ulp_ctx = rte_zmalloc("bnxt_ulp_ctx", sizeof(struct bnxt_ulp_context), 0);
  if (!bp->ulp_ctx) {
  ...
  if (initialized) {
    ...
    if (rc) {
      BNXT_TF_DBG(ERR, "Failed attach to shared session (%d)", rc);
      goto jump_to_error;
    }
  } else {
...
void bnxt_ulp_port_deinit(struct bnxt *bp) {
  ...
  if (!BNXT_TRUFLOW_EN(bp)) {
    BNXT_TF_DBG(DEBUG, "Skip ULP deinit for port:%d, TF is not enabled\n",
                bp->eth_dev->data->port_id);
    return;
  }

  if (!BNXT_PF(bp) && !BNXT_VF_IS_TRUSTED(bp)) {
    BNXT_TF_DBG(DEBUG, "Skip ULP deinit port:%d, not a TVF or PF\n",
                bp->eth_dev->data->port_id);
    return;
  }
...
int32_t bnxt_ulp_cntxt_ptr2_mark_db_set(struct bnxt_ulp_context *ulp_ctx,
...
int32_t bnxt_ulp_cntxt_app_id_set(struct bnxt_ulp_context *ulp_ctx,
                                  uint8_t app_id) {
  if (!ulp_ctx)
    return -EINVAL;
...
int32_t bnxt_ulp_cntxt_app_id_get(struct bnxt_ulp_context *ulp_ctx,
                                  uint8_t *app_id) {
...
int32_t bnxt_ulp_cntxt_dev_id_set(struct bnxt_ulp_context *ulp_ctx,
                                  uint32_t dev_id) {
  if (ulp_ctx && ulp_ctx->cfg_data) {
...
int32_t bnxt_ulp_cntxt_dev_id_get(struct bnxt_ulp_context *ulp_ctx,
                                  uint32_t *dev_id) {
  if (ulp_ctx && ulp_ctx->cfg_data) {
...
int32_t bnxt_ulp_cntxt_mem_type_set(struct bnxt_ulp_context *ulp_ctx,
...
int32_t bnxt_ulp_cntxt_mem_type_get(struct bnxt_ulp_context *ulp_ctx,
                                    enum bnxt_ulp_flow_mem_type *mem_type) {
  if (ulp_ctx && ulp_ctx->cfg_data) {
...
int32_t bnxt_ulp_cntxt_tbl_scope_id_get(struct bnxt_ulp_context *ulp_ctx,
...
int32_t bnxt_ulp_cntxt_tbl_scope_id_set(struct bnxt_ulp_context *ulp_ctx,
                                        uint32_t tbl_scope_id) {
  if (ulp_ctx && ulp_ctx->cfg_data) {
...
int32_t bnxt_ulp_cntxt_shared_tfp_set(struct bnxt_ulp_context *ulp,
                                      struct tf *tfp) {
  if (!ulp) {
...
struct tf *bnxt_ulp_cntxt_shared_tfp_get(struct bnxt_ulp_context *ulp) {
  if (!ulp) {
    BNXT_TF_DBG(ERR, "Invalid arguments\n");
    return NULL;
  }
...
uint8_t bnxt_ulp_cntxt_num_shared_clients_get(struct bnxt_ulp_context *ulp) {
...
int32_t bnxt_ulp_cntxt_tfp_set(struct bnxt_ulp_context *ulp, struct tf *tfp) {
  if (!ulp) {
    BNXT_TF_DBG(ERR, "Invalid arguments\n");
    return -EINVAL;
  }
 ...
 */
struct bnxt_ulp_device_params *bnxt_ulp_device_params_get(uint32_t dev_id) {
...
int32_t bnxt_ulp_cntxt_ptr2_flow_db_set(struct bnxt_ulp_context *ulp_ctx,
...
int32_t bnxt_ulp_cntxt_ptr2_mapper_data_set(struct bnxt_ulp_context *ulp_ctx,
                                            void *mapper_data) {
  if (!ulp_ctx || !ulp_ctx->cfg_data) {
    BNXT_TF_DBG(ERR, "Invalid ulp context data\n");
    return -EINVAL;
...
void *bnxt_ulp_cntxt_ptr2_mapper_data_get(struct bnxt_ulp_context *ulp_ctx) {
  if (!ulp_ctx || !ulp_ctx->cfg_data) {
    BNXT_TF_DBG(ERR, "Invalid ulp context data\n");
    return NULL;
  }
...
int32_t bnxt_ulp_cntxt_ptr2_port_db_set(struct bnxt_ulp_context *ulp_ctx,
...
int32_t bnxt_ulp_cntxt_ptr2_fc_info_set(struct bnxt_ulp_context *ulp_ctx,
...
int32_t bnxt_ulp_cntxt_ptr2_ulp_flags_get(struct bnxt_ulp_context *ulp_ctx,
                                          uint32_t *flags) {
  if (!ulp_ctx || !ulp_ctx->cfg_data)
    return -1;
...
int32_t bnxt_ulp_cntxt_acquire_fdb_lock(struct bnxt_ulp_context *ulp_ctx) {
  if (!ulp_ctx || !ulp_ctx->cfg_data)
    return -1;
...
void bnxt_ulp_cntxt_release_fdb_lock(struct bnxt_ulp_context *ulp_ctx) {
  if (!ulp_ctx || !ulp_ctx->cfg_data)
    return;
...
bool bnxt_ulp_cntxt_ha_enabled(struct bnxt_ulp_context *ulp_ctx) {
  if (ulp_ctx == NULL || ulp_ctx->cfg_data == NULL)
    return false;
...
struct bnxt_ulp_context *bnxt_ulp_cntxt_entry_acquire(void *arg) {
  struct ulp_context_list_entry *entry;
...
void bnxt_ulp_cntxt_entry_release(void) {
  rte_spinlock_unlock(&bnxt_ulp_ctxt_lock);
}
