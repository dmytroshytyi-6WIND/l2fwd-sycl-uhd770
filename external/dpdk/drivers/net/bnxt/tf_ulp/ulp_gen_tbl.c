...
ulp_mapper_generic_tbl_list_init(struct bnxt_ulp_mapper_data *mapper_data) {
  ...
  for (idx = 0; idx < BNXT_ULP_GEN_TBL_MAX_SZ; idx++) {
    ...
    if (tbl->result_num_entries != 0) {
      ...
      entry->mem_data =
          rte_zmalloc("ulp mapper gen tbl", entry->mem_data_size, 0);
      if (!entry->mem_data) {
        BNXT_TF_DBG(ERR, "%s:Failed to alloc gen table %d\n", tbl->name, idx);
        return -ENOMEM;
      }
      ...
      entry->container.ref_count = (uint32_t *)entry->mem_data;
    ...
    if (tbl->hash_tbl_entries) {
      ...
      if (ulp_gen_hash_tbl_list_init(&cparams, &entry->hash_tbl)) {
        BNXT_TF_DBG(ERR, "%s: Failed to alloc hash tbl %d\n", tbl->name, idx);
        return -ENOMEM;
      }
    }
  }
 ...
 * returns 0 on success.
 */
int32_t ulp_mapper_gen_tbl_entry_get(struct ulp_mapper_gen_tbl_list *tbl_list,
                                     uint32_t key,
  ...
  entry->byte_data =
      &tbl_list->container.byte_data[key * entry->byte_data_size];
 ...
 * returns None
 */
int32_t ulp_mapper_gen_tbl_idx_calculate(uint32_t res_sub_type, uint32_t dir) {
  int32_t tbl_idx;
 ...
 * returns 0 on success
 */
int32_t ulp_mapper_gen_tbl_entry_free(struct bnxt_ulp_context *ulp_ctx,
 ...
 * returns 0 on success
 */
int32_t ulp_mapper_gen_tbl_res_free(struct bnxt_ulp_context *ulp_ctx,
                                    struct ulp_flow_db_res_params *res) {
  ...
  tbl_idx =
      ulp_mapper_gen_tbl_idx_calculate(res->resource_sub_type, res->direction);
  if (tbl_idx < 0) {
  ...
  if (fid) {
    ...
    if (ulp_mapper_flow_destroy(ulp_ctx, BNXT_ULP_FDB_TYPE_RID, fid))
 ...
 * returns 0 on success.
 */
int32_t ulp_mapper_gen_tbl_hash_entry_add(
    struct ulp_mapper_gen_tbl_list *tbl_list,
    struct ulp_gen_hash_entry_params *hash_entry,
  ...
  case ULP_GEN_HASH_SEARCH_MISSED:
    ...
    gen_tbl_ent->byte_data_size = tbl_list->container.byte_data_size;
    gen_tbl_ent->byte_data =
