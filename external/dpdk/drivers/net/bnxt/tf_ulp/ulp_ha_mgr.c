...
static int32_t ulp_ha_mgr_region_set(struct bnxt_ulp_context *ulp_ctx,
                                     enum ulp_ha_mgr_region region);
static int32_t ulp_ha_mgr_state_set(struct bnxt_ulp_context *ulp_ctx,
                                    enum ulp_ha_mgr_state state);

static int32_t ulp_ha_mgr_tf_client_num_get(struct bnxt_ulp_context *ulp_ctx,
                                            uint32_t *cnt);
...
static void ulp_ha_mgr_timer_cb(void *arg) {
  ...
  if (myclient_cnt == 0) {
    BNXT_TF_DBG(ERR, "PANIC Client Count is zero kill timer\n.");
    return;
  }
  ...
  if (client_cnt == myclient_cnt) {
    if (curr_state == ULP_HA_STATE_PRIM_SEC_RUN &&
      ...
      if (rc) {
        BNXT_TF_DBG(ERR, "On HA CB:Failed(%d) to set state\n", rc);
        goto cb_restart;
      }

      cparms.dir = TF_DIR_RX;
      cparms.tcam_tbl_type = TF_TCAM_TBL_TYPE_WC_TCAM_HIGH;
      rc = tf_clear_tcam_shared_entries(tfp, &cparms);
      if (rc) {
        BNXT_TF_DBG(ERR, "On HA CB:Failed(%d) clear tcam\n", rc);
        goto cb_restart;
      }
       ...
       */
      rc = ulp_ha_mgr_state_set(ulp_ctx, ULP_HA_STATE_SEC_TIMER_COPY);
      if (rc) {
        BNXT_TF_DBG(ERR, "On HA CB:Failed(%d) to set state\n", rc);
        goto cb_restart;
      }
  ...
  rc = tf_clear_tcam_shared_entries(tfp, &cparms);
  if (rc) {
    BNXT_TF_DBG(ERR, "On HA CB:Failed(%d) clear tcam low\n", rc);
    goto unlock;
  }
...
static int32_t ulp_ha_mgr_timer_start(void *arg) {
  rte_eal_alarm_set(US_PER_S * ULP_HA_TIMER_SEC, ulp_ha_mgr_timer_cb, arg);
  return 0;
}
...
int32_t ulp_ha_mgr_init(struct bnxt_ulp_context *ulp_ctx) {
  struct bnxt_ulp_ha_mgr_info *ha_info;
  int32_t rc;
...
void ulp_ha_mgr_deinit(struct bnxt_ulp_context *ulp_ctx) {
  struct bnxt_ulp_ha_mgr_info *ha_info;
...
int32_t ulp_ha_mgr_app_type_get(struct bnxt_ulp_context *ulp_ctx,
                                enum ulp_ha_mgr_app_type *app_type) {
  struct bnxt_ulp_ha_mgr_info *ha_info;
...
int32_t ulp_ha_mgr_state_get(struct bnxt_ulp_context *ulp_ctx,
                             enum ulp_ha_mgr_state *state) {
  struct tf_get_if_tbl_entry_parms get_parms = {0};
  struct tf *tfp;
  uint32_t val = 0;
...
int32_t ulp_ha_mgr_open(struct bnxt_ulp_context *ulp_ctx) {
  enum ulp_ha_mgr_state curr_state;
  int32_t rc;
...
int32_t ulp_ha_mgr_close(struct bnxt_ulp_context *ulp_ctx) {
  ...
  if (curr_state == ULP_HA_STATE_PRIM_RUN && app_type == ULP_HA_APP_TYPE_PRIM) {
    /*
     ...
     */
    BNXT_TF_DBG(INFO, "On Close: PRIM[PRIM_SEC_RUN] flushing flows.\n");
     ...
     */
    BNXT_TF_DBG(INFO, "On Close: PRIM[PRIM_SEC_RUN] => [Copy], enter wait.\n");
    timeout = ULP_HA_WAIT_TIMEOUT;
    do {
      rte_delay_ms(ULP_HA_WAIT_TIME);
      rc = ulp_ha_mgr_state_get(ulp_ctx, &poll_state);
      if (rc) {
        BNXT_TF_DBG(ERR, "Failed to get HA state on Close (%d)\n", rc);
        goto cleanup;
      }
      timeout -= ULP_HA_WAIT_TIME;
      BNXT_TF_DBG(INFO, "On Close: Waiting %d ms for PRIM_RUN\n", timeout);
    ...
    if (rc) {
      BNXT_TF_DBG(ERR, "On Close: Failed to set state to INIT(%x)\n", rc);
      goto cleanup;
    }

    BNXT_TF_DBG(INFO, "On Close: SEC[COPY] => [INIT] after %d ms\n",
...
int32_t ulp_ha_mgr_region_get(struct bnxt_ulp_context *ulp_ctx,
                              enum ulp_ha_mgr_region *region) {
  struct bnxt_ulp_ha_mgr_info *ha_info;
