...
#define ULP_MARK_DB_ENTRY_SET_VALID(mark_info)                                 \
  ((mark_info)->flags |= BNXT_ULP_MARK_VALID)
#define ULP_MARK_DB_ENTRY_IS_INVALID(mark_info)                                \
  (!((mark_info)->flags & BNXT_ULP_MARK_VALID))
#define ULP_MARK_DB_ENTRY_SET_VFR_ID(mark_info)                                \
  ((mark_info)->flags |= BNXT_ULP_MARK_VFR_ID)
#define ULP_MARK_DB_ENTRY_IS_VFR_ID(mark_info)                                 \
  ((mark_info)->flags & BNXT_ULP_MARK_VFR_ID)
#define ULP_MARK_DB_ENTRY_IS_GLOBAL_HW_FID(mark_info)                          \
  ((mark_info)->flags & BNXT_ULP_MARK_GLOBAL_HW_FID)

static inline uint32_t ulp_mark_db_idx_get(bool is_gfid, uint32_t fid,
 ...
 *
 */
int32_t ulp_mark_db_init(struct bnxt_ulp_context *ctxt) {
  ...
  mark_tbl =
      rte_zmalloc("ulp_rx_mark_tbl_ptr", sizeof(struct bnxt_ulp_mark_tbl), 0);
  if (!mark_tbl)
    goto mem_error;

  /* Need to allocate 2 * Num flows to account for hash type bit.*/
  mark_tbl->lfid_num_entries = dparms->mark_db_lfid_entries;
  mark_tbl->lfid_tbl = rte_zmalloc(
      "ulp_rx_em_flow_mark_table",
      mark_tbl->lfid_num_entries * sizeof(struct bnxt_lfid_mark_info), 0);
  ...
  mark_tbl->gfid_tbl = rte_zmalloc(
      "ulp_rx_eem_flow_mark_table",
      mark_tbl->gfid_num_entries * sizeof(struct bnxt_gfid_mark_info), 0);
  ...
  BNXT_TF_DBG(DEBUG, "GFID Max = 0x%08x GFID MASK = 0x%08x\n",
              mark_tbl->gfid_num_entries - 1, mark_tbl->gfid_mask);
 ...
 *
 */
int32_t ulp_mark_db_deinit(struct bnxt_ulp_context *ctxt) {
  struct bnxt_ulp_mark_tbl *mtbl;
 ...
 *
 */
int32_t ulp_mark_db_mark_get(struct bnxt_ulp_context *ctxt, bool is_gfid,
                             uint32_t fid, uint32_t *vfr_flag, uint32_t *mark) {
  struct bnxt_ulp_mark_tbl *mtbl;
  uint32_t idx = 0;
 ...
 *
 */
int32_t ulp_mark_db_mark_add(struct bnxt_ulp_context *ctxt, uint32_t mark_flag,
                             uint32_t fid, uint32_t mark) {
  struct bnxt_ulp_mark_tbl *mtbl;
  uint32_t idx = 0;
  bool is_gfid;
 ...
 *
 */
int32_t ulp_mark_db_mark_del(struct bnxt_ulp_context *ctxt, uint32_t mark_flag,
                             uint32_t fid) {
  struct bnxt_ulp_mark_tbl *mtbl;
  uint32_t idx = 0;
  bool is_gfid;
  ...
  if (is_gfid) {
    ...
    memset(&mtbl->gfid_tbl[idx], 0, sizeof(struct bnxt_gfid_mark_info));
    ...
    memset(&mtbl->lfid_tbl[fid], 0, sizeof(struct bnxt_lfid_mark_info));
  }
