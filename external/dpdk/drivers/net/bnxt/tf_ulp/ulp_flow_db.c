...
#define ULP_FLOW_DB_RES_NXT_SET(dst, src)                                      \
  { (dst) |= ((src)&ULP_FLOW_DB_RES_NXT_MASK); }
 ...
 * returns none
 */
static void ulp_flow_db_active_flows_bit_set(struct bnxt_ulp_flow_db *flow_db,
                                             enum bnxt_ulp_fdb_type flow_type,
                                             uint32_t idx, uint32_t flag) {
  ...
  if (flag) {
    if (flow_type == BNXT_ULP_FDB_TYPE_REGULAR ||
      ...
      ULP_INDEX_BITMAP_SET(f_tbl->active_reg_flows[a_idx], idx);
    if (flow_type == BNXT_ULP_FDB_TYPE_DEFAULT ||
        flow_type == BNXT_ULP_FDB_TYPE_RID)
      ULP_INDEX_BITMAP_SET(f_tbl->active_dflt_flows[a_idx], idx);
  } else {
    if (flow_type == BNXT_ULP_FDB_TYPE_REGULAR ||
        flow_type == BNXT_ULP_FDB_TYPE_RID)
      ULP_INDEX_BITMAP_RESET(f_tbl->active_reg_flows[a_idx], idx);
    if (flow_type == BNXT_ULP_FDB_TYPE_DEFAULT ||
        flow_type == BNXT_ULP_FDB_TYPE_RID)
      ULP_INDEX_BITMAP_RESET(f_tbl->active_dflt_flows[a_idx], idx);
  }
}
 ...
 */
static int32_t ulp_flow_db_alloc_resource(struct bnxt_ulp_flow_db *flow_db) {
  uint32_t idx = 0;
  ...
  flow_tbl->flow_resources = rte_zmalloc("ulp_fdb_resource_info", size, 0);
  ...
  flow_tbl->active_reg_flows =
      rte_zmalloc("active reg flows", size, ULP_BUFFER_ALIGN_64_BYTE);
  if (!flow_tbl->active_reg_flows) {
    BNXT_TF_DBG(ERR, "Failed to alloc memory active reg flows\n");
    return -ENOMEM;
  }

  flow_tbl->active_dflt_flows =
      rte_zmalloc("active dflt flows", size, ULP_BUFFER_ALIGN_64_BYTE);
 ...
 * Returns none.
 */
static void ulp_flow_db_dealloc_resource(struct bnxt_ulp_flow_db *flow_db) {
 ...
 * returns none
 */
static void ulp_flow_db_func_id_set(struct bnxt_ulp_flow_db *flow_db,
                                    uint32_t flow_id, uint32_t func_id) {
 ...
 * Returns 0 on success or negative number on failure.
 */
static int32_t ulp_flow_db_parent_tbl_init(struct bnxt_ulp_flow_db *flow_db,
  ...
  p_db->child_bitset_size =
      (flow_db->flow_tbl.num_flows / sizeof(uint64_t)) + 1; /* size in bytes */
  p_db->child_bitset_size = ULP_BYTE_ROUND_OFF_8(p_db->child_bitset_size);
  p_db->entries_count = num_entries;

  /* allocate the memory */
  p_db->parent_flow_tbl =
      rte_zmalloc("fdb parent flow tbl",
                  sizeof(struct ulp_fdb_parent_info) * p_db->entries_count, 0);
  if (!p_db->parent_flow_tbl) {
    BNXT_TF_DBG(ERR, "Failed to allocate memory fdb parent flow tbl\n");
    return -ENOMEM;
  }
   ...
   */
  p_db->parent_flow_tbl_mem =
      rte_zmalloc("fdb parent flow tbl mem", size, ULP_BUFFER_ALIGN_64_BYTE);
  if (!p_db->parent_flow_tbl_mem) {
    BNXT_TF_DBG(ERR, "Failed to allocate memory fdb parent flow mem\n");
    return -ENOMEM;
  }
  ...
  for (idx = 0; idx < p_db->entries_count; idx++) {
    p_db->parent_flow_tbl[idx].child_fid_bitset =
        (uint64_t *)&p_db->parent_flow_tbl_mem[idx * p_db->child_bitset_size];
  }
  /* success */
  return 0;
}
 ...
 * Returns none
 */
static void ulp_flow_db_parent_tbl_deinit(struct bnxt_ulp_flow_db *flow_db) {
...
int32_t ulp_flow_db_init(struct bnxt_ulp_context *ulp_ctxt) {
  ...
  flow_db = rte_zmalloc("bnxt_ulp_flow_db", sizeof(struct bnxt_ulp_flow_db), 0);
  if (!flow_db) {
    BNXT_TF_DBG(ERR, "Failed to allocate memory for flow table ptr\n");
    return -ENOMEM;
  }
  ...
  flow_tbl->num_resources = ((num_flows + 1) * dparms->num_resources_per_flow);
  ...
  flow_db->func_id_tbl =
      rte_zmalloc("bnxt_ulp_flow_db_func_id_table",
                  flow_db->func_id_tbl_size * sizeof(uint16_t), 0);
  if (!flow_db->func_id_tbl) {
    BNXT_TF_DBG(ERR, "Failed to allocate mem for flow table func id\n");
    goto error_free;
  }
  /* initialize the parent child database */
  if (ulp_flow_db_parent_tbl_init(flow_db, dparms->fdb_parent_flow_entries)) {
    BNXT_TF_DBG(ERR, "Failed to allocate mem for parent child db\n");
    goto error_free;
  }
...
int32_t ulp_flow_db_resource_add(struct bnxt_ulp_context *ulp_ctxt,
                                 enum bnxt_ulp_fdb_type flow_type, uint32_t fid,
                                 struct ulp_flow_db_res_params *params) {
  struct bnxt_ulp_flow_db *flow_db;
  struct bnxt_ulp_flow_tbl *flow_tbl;
  ...
  if (!params->critical_resource) {
    ...
    ULP_FLOW_DB_RES_NXT_SET(fid_resource->nxt_resource_idx, idx);
  } else {
  ...
  if (params->resource_type == TF_TBL_TYPE_ACT_STATS_64 &&
    ...
    ulp_fc_mgr_cntr_set(ulp_ctxt, params->direction, params->resource_hndl);
...
int32_t ulp_flow_db_resource_del(struct bnxt_ulp_context *ulp_ctxt,
  ...
  if (!params->critical_resource) {
    ...
    ULP_FLOW_DB_RES_NXT_SET(nxt_idx, fid_resource->nxt_resource_idx);
    if (!nxt_idx) {
    ...
    ULP_FLOW_DB_RES_NXT_SET(fid_resource->nxt_resource_idx, nxt_idx);
  }
  ...
  if (params->resource_type == TF_TBL_TYPE_ACT_STATS_64 &&
      params->resource_sub_type ==
          BNXT_ULP_RESOURCE_SUB_TYPE_INDEX_TABLE_INT_COUNT) {
    ulp_fc_mgr_cntr_reset(ulp_ctxt, params->direction, params->resource_hndl);
  }
...
int32_t ulp_flow_db_resource_get(struct bnxt_ulp_context *ulp_ctxt,
  ...
  if (!*nxt_idx) {
    ...
    ulp_flow_db_res_info_to_params(nxt_resource, params);
    *nxt_idx = 0;
    ULP_FLOW_DB_RES_NXT_SET(*nxt_idx, nxt_resource->nxt_resource_idx);
  }
 ...
 * returns 0 on success and negative on failure.
 */
static int32_t ulp_flow_db_next_entry_get(struct bnxt_ulp_flow_db *flow_db,
                                          enum bnxt_ulp_fdb_type flow_type,
                                          uint32_t *fid) {
  uint32_t lfid = *fid;
  ...
  } while (!ulp_flow_db_active_flows_bit_is_set(flow_db, flow_type, lfid));
 ...
 * returns 0 on success or negative number on failure
 */
int32_t ulp_flow_db_function_flow_flush(struct bnxt_ulp_context *ulp_ctx,
                                        uint16_t func_id) {
  uint32_t flow_id = 0;
  struct bnxt_ulp_flow_db *flow_db;
  ...
  while (!ulp_flow_db_next_entry_get(flow_db, BNXT_ULP_FDB_TYPE_REGULAR,
    ...
    if (flow_db->func_id_tbl[flow_id] == func_id)
      ulp_mapper_resources_free(ulp_ctx, BNXT_ULP_FDB_TYPE_REGULAR, flow_id);
  }
  bnxt_ulp_cntxt_release_fdb_lock(ulp_ctx);
  return 0;
}
 ...
 * returns 0 on success or negative number on failure
 */
int32_t ulp_flow_db_session_flow_flush(struct bnxt_ulp_context *ulp_ctx) {
  /*
 ...
 */
bool ulp_flow_db_validate_flow_func(struct bnxt_ulp_context *ulp_ctx,
                                    uint32_t flow_id, uint32_t func_id) {
  struct bnxt_ulp_flow_db *flow_db;
 ...
 * the first match.
 */
static int32_t ulp_flow_db_resource_params_get(
    struct bnxt_ulp_context *ulp_ctx, enum bnxt_ulp_fdb_type flow_type,
    uint32_t flow_id, uint32_t resource_func, uint32_t res_subtype,
    struct ulp_flow_db_res_params *params) {
  struct bnxt_ulp_flow_db *flow_db;
  struct bnxt_ulp_flow_tbl *flow_tbl;
  ...
  while (res_id) {
    ...
    if (ulp_flow_db_resource_func_get(fid_res) == resource_func) {
      ...
      } else if (resource_func == BNXT_ULP_RESOURCE_FUNC_EM_TABLE) {
        ulp_flow_db_res_info_to_params(fid_res, params);
        return 0;
      }
    }
 ...
 * returns 0 on success
 */
int32_t ulp_default_flow_db_cfa_action_get(struct bnxt_ulp_context *ulp_ctx,
                                           uint32_t flow_id,
                                           uint16_t *cfa_action) {
  ...
  rc = ulp_flow_db_resource_params_get(
      ulp_ctx, BNXT_ULP_FDB_TYPE_DEFAULT, flow_id,
      BNXT_ULP_RESOURCE_FUNC_INDEX_TABLE, sub_typ, &params);
  if (rc) {
 ...
 */
static int32_t ulp_flow_db_pc_db_idx_alloc(struct bnxt_ulp_context *ulp_ctxt,
                                           uint32_t tun_idx) {
  struct bnxt_ulp_flow_db *flow_db;
 ...
 * returns none.
 */
static void ulp_flow_db_pc_db_entry_free(struct bnxt_ulp_context *ulp_ctxt,
 ...
 */
int32_t ulp_flow_db_pc_db_parent_flow_set(struct bnxt_ulp_context *ulp_ctxt,
                                          uint32_t pc_idx, uint32_t parent_fid,
                                          uint32_t set_flag) {
  struct ulp_fdb_parent_info *pc_entry;
  struct bnxt_ulp_flow_db *flow_db;
 ...
 * returns zero on success and negative on failure.
 */
int32_t ulp_flow_db_pc_db_child_flow_set(struct bnxt_ulp_context *ulp_ctxt,
                                         uint32_t pc_idx, uint32_t child_fid,
                                         uint32_t set_flag) {
  struct ulp_fdb_parent_info *pc_entry;
  struct bnxt_ulp_flow_db *flow_db;
  uint32_t a_idx;
...
ulp_flow_db_parent_child_flow_next_entry_get(struct bnxt_ulp_flow_db *flow_db,
  ...
  do {
    ...
    while (!(bs = child_bitset[idx])) {
      idx++;
      if ((idx * ULP_INDEX_BITMAP_SIZE) >= flow_db->flow_tbl.num_flows)
        return -ENOENT;
    }
    /*
     * remove the previous bits in the bitset bs to find the
 ...
 * Returns 0 on success and negative on failure.
 */
int32_t ulp_flow_db_child_flow_reset(struct bnxt_ulp_context *ulp_ctxt,
                                     enum bnxt_ulp_fdb_type flow_type,
                                     uint32_t fid) {
  struct bnxt_ulp_flow_db *flow_db;
  struct bnxt_ulp_flow_tbl *flow_tbl;
 ...
 * Returns 0 on success and negative on failure.
 */
int32_t ulp_flow_db_parent_flow_create(struct bnxt_ulp_mapper_parms *parms) {
  ...
  if (ulp_flow_db_pc_db_parent_flow_set(parms->ulp_ctx, pc_idx, parms->fid,
                                        1)) {
  ...
  if (!ulp_flow_db_resource_params_get(
          parms->ulp_ctx, BNXT_ULP_FDB_TYPE_REGULAR, parms->fid,
          BNXT_ULP_RESOURCE_FUNC_INDEX_TABLE, sub_typ, &res_params)) {
    /* Enable the counter accumulation in parent entry */
    if (ulp_flow_db_parent_flow_count_accum_set(parms->ulp_ctx, pc_idx)) {
 ...
 * Returns 0 on success and negative on failure.
 */
int32_t ulp_flow_db_child_flow_create(struct bnxt_ulp_mapper_parms *parms) {
  struct ulp_flow_db_res_params fid_parms;
  ...
  rc = ulp_flow_db_pc_db_child_flow_set(parms->ulp_ctx, pc_idx, parms->fid, 1);
  if (rc) {
  ...
  rc =
      ulp_flow_db_resource_params_get(parms->ulp_ctx, BNXT_ULP_FDB_TYPE_REGULAR,
                                      parms->fid, res_fun, sub_type, &res_p);
  if (!rc) {
    /* update the counter manager to include parent fid */
    if (ulp_fc_mgr_cntr_parent_flow_set(parms->ulp_ctx, res_p.direction,
                                        res_p.resource_hndl, pc_idx)) {
 ...
 * returns 0 on success
 */
int32_t ulp_flow_db_parent_flow_count_update(struct bnxt_ulp_context *ulp_ctxt,
                                             uint32_t pc_idx,
 ...
 * returns 0 on success
 */
int32_t ulp_flow_db_parent_flow_count_get(struct bnxt_ulp_context *ulp_ctxt,
                                          uint32_t pc_idx,
                                          uint64_t *packet_count,
                                          uint64_t *byte_count,
                                          uint8_t count_reset) {
 ...
 * returns none
 */
void ulp_flow_db_parent_flow_count_reset(struct bnxt_ulp_context *ulp_ctxt) {
  struct bnxt_ulp_flow_db *flow_db;
  struct ulp_fdb_parent_child_db *p_pdb;
  uint32_t idx;
