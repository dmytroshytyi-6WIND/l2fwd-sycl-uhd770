...
#define ULP_BITMAP_CMP(b1, b2)                                                 \
  memcmp(&(b1)->bits, &(b2)->bits, sizeof((b1)->bits))
/*
 * Macros for bitmap sets and gets
...
#define ULP_INDEX_BITMAP_CSET(i)                                               \
  (1UL << ((ULP_INDEX_BITMAP_SIZE - 1) - ((i) % ULP_INDEX_BITMAP_SIZE)))

#define ULP_INDEX_BITMAP_SET(b, i)                                             \
  ((b) |=                                                                      \
   (1UL << ((ULP_INDEX_BITMAP_SIZE - 1) - ((i) % ULP_INDEX_BITMAP_SIZE))))

#define ULP_INDEX_BITMAP_RESET(b, i)                                           \
  ((b) &=                                                                      \
   (~(1UL << ((ULP_INDEX_BITMAP_SIZE - 1) - ((i) % ULP_INDEX_BITMAP_SIZE)))))

#define ULP_INDEX_BITMAP_GET(b, i)                                             \
  (((b) >> ((ULP_INDEX_BITMAP_SIZE - 1) - ((i) % ULP_INDEX_BITMAP_SIZE))) & 1)
 ...
 * returns None.
 */
void ulp_bs_pull_msb(uint8_t *src, uint8_t *dst, uint32_t offset, uint32_t len);
 ...
 */
uint16_t ulp_operand_read(uint8_t *operand, uint8_t *val, uint16_t bitlen);
