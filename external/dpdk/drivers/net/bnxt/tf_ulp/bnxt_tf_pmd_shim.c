...
int32_t bnxt_rss_config_action_apply(struct bnxt_ulp_mapper_parms *parms) {
  ...
  if (vnic->rx_queue_cnt > 1) {
    ...
    vnic->hash_mode = bnxt_rte_to_hwrm_hash_level(bp, rss_types, rss_level);
    memcpy(vnic->rss_hash_key, rss_key, BNXT_ULP_ACT_PROP_SZ_RSS_KEY);
...
void bnxt_pmd_get_iface_mac(uint16_t port, enum bnxt_ulp_intf_type type,
  ...
  if (type != BNXT_ULP_INTF_TYPE_TRUSTED_VF && type != BNXT_ULP_INTF_TYPE_PF)
    return;
...
uint16_t bnxt_pmd_get_parent_vnic_id(uint16_t port,
                                     enum bnxt_ulp_intf_type type) {
  struct rte_eth_dev *eth_dev;
  struct bnxt *bp;
...
uint16_t bnxt_pmd_get_phy_port_id(uint16_t port_id) {
...
uint16_t bnxt_pmd_get_parif(uint16_t port_id, enum bnxt_ulp_intf_type type) {
  struct rte_eth_dev *eth_dev;
  struct bnxt *bp;
...
uint16_t bnxt_pmd_get_vport(uint16_t port_id) {
