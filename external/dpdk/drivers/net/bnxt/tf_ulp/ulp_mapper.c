...
static uint8_t mapper_fld_ones[16] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                                      0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                                      0xFF, 0xFF, 0xFF, 0xFF};

static uint8_t mapper_fld_one[16] = {0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
...
ulp_mapper_glb_resource_read(struct bnxt_ulp_mapper_data *mapper_data,
                             enum tf_dir dir, uint16_t idx, uint64_t *regval,
                             bool *shared) {
  if (!mapper_data || !regval || !shared || dir >= TF_DIR_MAX ||
      idx >= BNXT_ULP_GLB_RF_IDX_LAST)
    return -EINVAL;
...
ulp_mapper_resource_ident_allocate(struct bnxt_ulp_context *ulp_ctx,
  ...
  if (rc) {
    BNXT_TF_DBG(ERR, "Failed to alloc identifier [%s][%d]\n",
                tf_dir_2_str(iparms.dir), iparms.ident_type);
    return rc;
  }
 ...
 * returns 0 on success
 */
static int32_t ulp_mapper_resource_index_tbl_alloc(
    struct bnxt_ulp_context *ulp_ctx, struct bnxt_ulp_mapper_data *mapper_data,
    struct bnxt_ulp_glb_resource_info *glb_res) {
...
static int32_t ulp_mapper_glb_field_tbl_get(struct bnxt_ulp_mapper_parms *parms,
                                            uint32_t operand, uint8_t *val) {
  uint32_t t_idx;

  t_idx = parms->app_id << (BNXT_ULP_APP_ID_SHIFT + BNXT_ULP_HDR_SIG_ID_SHIFT +
                            BNXT_ULP_GLB_FIELD_TBL_SHIFT);
  t_idx += parms->class_tid
           << (BNXT_ULP_HDR_SIG_ID_SHIFT + BNXT_ULP_GLB_FIELD_TBL_SHIFT);
 ...
 */
static uint32_t ulp_mapper_act_prop_size_get(uint32_t idx) {
...
ulp_mapper_tmpl_reject_list_get(struct bnxt_ulp_mapper_parms *mparms,
                                uint32_t tid, uint32_t *num_tbls,
...
static struct bnxt_ulp_mapper_cond_info *ulp_mapper_tbl_execute_list_get(
    struct bnxt_ulp_mapper_parms *mparms, struct bnxt_ulp_mapper_tbl_info *tbl,
    uint32_t *num_tbls, enum bnxt_ulp_cond_list_opc *opc) {
  uint32_t idx;
  const struct bnxt_ulp_template_device_tbls *dev_tbls;
...
ulp_mapper_tbl_list_get(struct bnxt_ulp_mapper_parms *mparms, uint32_t tid,
                        uint32_t *num_tbls) {
  uint32_t idx;
  const struct bnxt_ulp_template_device_tbls *dev_tbls;
...
ulp_mapper_result_fields_get(struct bnxt_ulp_mapper_parms *mparms,
                             struct bnxt_ulp_mapper_tbl_info *tbl,
                             uint32_t *num_flds, uint32_t *num_encap_flds) {
  uint32_t idx;
  const struct bnxt_ulp_template_device_tbls *dev_tbls;
...
ulp_mapper_dyn_tbl_type_get(struct bnxt_ulp_mapper_parms *mparms,
                            struct bnxt_ulp_mapper_tbl_info *tbl,
                            struct ulp_blob *bdata, uint16_t *out_len) {
...
ulp_mapper_tcam_entry_free(struct bnxt_ulp_context *ulp, struct tf *tfp,
                           struct ulp_flow_db_res_params *res) {
  struct tf_free_tcam_entry_parms fparms = {.dir = res->direction,
  ...
  if (bnxt_ulp_cntxt_ha_enabled(ulp)) {
    ...
    case TF_TCAM_TBL_TYPE_WC_TCAM_LOW:
      ...
      if (rc)
        ...
        BNXT_TF_DBG(ERR, "Unable to get HA region (%d)\n", rc);
      else
        fparms.tcam_tbl_type = (region == ULP_HA_REGION_LOW)
...
static inline int32_t
ulp_mapper_index_entry_free(struct bnxt_ulp_context *ulp, struct tf *tfp,
                            struct ulp_flow_db_res_params *res) {
  struct tf_free_tbl_entry_parms fparms = {.dir = res->direction,
...
static inline int32_t
ulp_mapper_em_entry_free(struct bnxt_ulp_context *ulp, struct tf *tfp,
                         struct ulp_flow_db_res_params *res) {
...
static inline int32_t
ulp_mapper_ident_free(struct bnxt_ulp_context *ulp __rte_unused, struct tf *tfp,
                      struct ulp_flow_db_res_params *res) {
  struct tf_free_identifier_parms fparms = {.dir = res->direction,
...
static inline int32_t ulp_mapper_mark_free(struct bnxt_ulp_context *ulp,
                                           struct ulp_flow_db_res_params *res) {
  return ulp_mark_db_mark_del(ulp, res->resource_type, res->resource_hndl);
}

static inline int32_t
ulp_mapper_parent_flow_free(struct bnxt_ulp_context *ulp, uint32_t parent_fid,
                            struct ulp_flow_db_res_params *res) {
  uint32_t pc_idx;
...
static inline int32_t
ulp_mapper_child_flow_free(struct bnxt_ulp_context *ulp, uint32_t child_fid,
                           struct ulp_flow_db_res_params *res) {
  uint32_t pc_idx;
...
ulp_mapper_fdb_opc_alloc_rid(struct bnxt_ulp_mapper_parms *parms,
  ...
  /* allocate a new fid */
  rc = ulp_flow_db_fid_alloc(parms->ulp_ctx, BNXT_ULP_FDB_TYPE_RID, 0, &rid);
  if (rc) {
  ...
  if (rc) {
    ...
    ulp_flow_db_fid_free(parms->ulp_ctx, BNXT_ULP_FDB_TYPE_RID, rid);
    return -EINVAL;
  }
  return 0;
}
...
ulp_mapper_fdb_opc_process(struct bnxt_ulp_mapper_parms *parms,
  ...
  rc = ulp_flow_db_resource_add(parms->ulp_ctx, flow_type, push_fid, fid_parms);
  if (rc)
...
ulp_mapper_tbl_ident_scan_ext(struct bnxt_ulp_mapper_parms *parms,
                              struct bnxt_ulp_mapper_tbl_info *tbl,
                              uint8_t *byte_data, uint32_t byte_data_size,
  ...
  for (i = 0; i < num_idents; i++) {
    ...
    if ((idents[i].ident_bit_pos + idents[i].ident_bit_size) >
      ...
      BNXT_TF_DBG(ERR, "invalid offset or length %x:%x:%x\n",
                  idents[i].ident_bit_pos, idents[i].ident_bit_size,
                  byte_data_size);
      return -EINVAL;
    }
    val64 = 0;
    if (byte_order == BNXT_ULP_BYTE_ORDER_LE)
      ulp_bs_pull_lsb(byte_data, (uint8_t *)&val64, sizeof(val64),
                      idents[i].ident_bit_pos, idents[i].ident_bit_size);
    else
      ulp_bs_pull_msb(byte_data, (uint8_t *)&val64, idents[i].ident_bit_pos,
                      idents[i].ident_bit_size);

    /* Write it to the regfile, val64 is already in big-endian*/
    if (ulp_regfile_write(parms->regfile, idents[i].regfile_idx, val64)) {
 ...
 */
static int32_t ulp_mapper_ident_process(
    struct bnxt_ulp_mapper_parms *parms, struct bnxt_ulp_mapper_tbl_info *tbl,
    struct bnxt_ulp_mapper_ident_info *ident, uint16_t *val) {
  struct ulp_flow_db_res_params fid_parms;
  uint64_t id = 0;
  int32_t idx;
  struct tf_alloc_identifier_parms iparms = {0};
 ...
 */
static int32_t ulp_mapper_ident_extract(
    struct bnxt_ulp_mapper_parms *parms, struct bnxt_ulp_mapper_tbl_info *tbl,
    struct bnxt_ulp_mapper_ident_info *ident, struct ulp_blob *res_blob) {
  struct ulp_flow_db_res_params fid_parms;
  uint64_t id = 0;
  uint32_t idx = 0;
  ...
  if (rc) {
    BNXT_TF_DBG(ERR, "Search ident %s:%s:%x failed.\n",
                tf_dir_2_str(sparms.dir), tf_ident_2_str(sparms.ident_type),
                sparms.search_id);
    return rc;
  }
  BNXT_TF_INF("Search ident %s:%s:%x.success.\n", tf_dir_2_str(sparms.dir),
              tf_tbl_type_2_str(sparms.ident_type), sparms.search_id);
...
ulp_mapper_field_port_db_process(struct bnxt_ulp_mapper_parms *parms,
                                 uint32_t port_id, uint16_t val16,
                                 uint8_t **val) {
  enum bnxt_ulp_port_table port_data = val16;

  switch (port_data) {
  case BNXT_ULP_PORT_TABLE_DRV_FUNC_PARENT_MAC:
    if (ulp_port_db_parent_mac_addr_get(parms->ulp_ctx, port_id, val)) {
      BNXT_TF_DBG(ERR, "Invalid port id %u\n", port_id);
      return -EINVAL;
    }
    break;
  case BNXT_ULP_PORT_TABLE_DRV_FUNC_MAC:
    if (ulp_port_db_drv_mac_addr_get(parms->ulp_ctx, port_id, val)) {
      BNXT_TF_DBG(ERR, "Invalid port id %u\n", port_id);
      return -EINVAL;
    }
    break;
  case BNXT_ULP_PORT_TABLE_DRV_FUNC_PARENT_VNIC:
    if (ulp_port_db_parent_vnic_get(parms->ulp_ctx, port_id, val)) {
      BNXT_TF_DBG(ERR, "Invalid port id %u\n", port_id);
      return -EINVAL;
    }
    break;
...
static int32_t ulp_mapper_field_src_process(struct bnxt_ulp_mapper_parms *parms,
                                            enum bnxt_ulp_field_src field_src,
                                            uint8_t *field_opr, enum tf_dir dir,
                                            uint8_t is_key, uint32_t bitlen,
  ...
  *value = 0;
  ...
  case BNXT_ULP_FIELD_SRC_CF:
    if (!ulp_operand_read(field_opr, (uint8_t *)&idx, sizeof(uint16_t))) {
  ...
  case BNXT_ULP_FIELD_SRC_ACT_PROP_SZ:
    ...
    *val = &parms->act_prop->act_details[idx];
    ...
    if (!ulp_operand_read(&field_opr[sizeof(uint16_t)], (uint8_t *)&size_idx,
                          sizeof(uint16_t))) {
      BNXT_TF_DBG(ERR, "Action sz operand read failed\n");
      return -EINVAL;
    }
  ...
  case BNXT_ULP_FIELD_SRC_GLB_RF:
    ...
    idx = tfp_be_to_cpu_16(idx);
    if (ulp_mapper_glb_resource_read(parms->mapper_data, dir, idx, &lregval,
                                     &shared) ||
        sizeof(uint64_t) < bytelen) {
  ...
  case BNXT_ULP_FIELD_SRC_SUB_HF:
    ...
    if (field_src == BNXT_ULP_FIELD_SRC_HF) {
      *val = &buffer[field_size - bytelen];
      ...
      /* get the offset next */
      if (!ulp_operand_read(&field_opr[sizeof(uint16_t)], (uint8_t *)&offset,
                            sizeof(uint16_t))) {
  ...
  case BNXT_ULP_FIELD_SRC_HDR_BIT:
    if (!ulp_operand_read(field_opr, (uint8_t *)&lregval, sizeof(uint64_t))) {
  ...
  case BNXT_ULP_FIELD_SRC_PORT_TABLE:
    ...
    port_id = ULP_COMP_FLD_IDX_RD(parms, BNXT_ULP_CF_IDX_DEV_PORT_ID);
    ...
    idx = tfp_be_to_cpu_16(idx);
    if (ulp_mapper_field_port_db_process(parms, port_id, idx, val)) {
...
static int32_t ulp_mapper_field_blob_write(enum bnxt_ulp_field_src fld_src,
                                           struct ulp_blob *blob, uint8_t *val,
                                           uint32_t val_len,
                                           uint8_t **out_val) {
...
static int32_t ulp_mapper_field_opc_process(
    struct bnxt_ulp_mapper_parms *parms, enum tf_dir dir,
    struct bnxt_ulp_mapper_field_info *fld, struct ulp_blob *blob,
    uint8_t is_key, const char *name) {
  ...
  if (process_src1) {
    if (ulp_mapper_field_src_process(parms, fld->field_src1, fld->field_opr1,
                                     dir, is_key, fld->field_bit_size, &val1,
                                     &val1_len, &value1)) {
      BNXT_TF_DBG(ERR, "fld src1 process failed\n");
      goto error;
    }
    if (eval_src1) {
      if (ulp_mapper_field_buffer_eval(val1, val1_len, &val1_int)) {
  ...
  /* process src2 opcode */
  if (process_src2) {
    if (ulp_mapper_field_src_process(parms, fld->field_src2, fld->field_opr2,
                                     dir, is_key, fld->field_bit_size, &val2,
                                     &val2_len, &value2)) {
      BNXT_TF_DBG(ERR, "fld src2 process failed\n");
      goto error;
    }
    if (eval_src2) {
      if (ulp_mapper_field_buffer_eval(val2, val2_len, &val2_int)) {
  ...
  /* process src3 opcode */
  if (process_src3) {
    if (ulp_mapper_field_src_process(parms, fld->field_src3, fld->field_opr3,
                                     dir, is_key, fld->field_bit_size, &val3,
                                     &val3_len, &value3)) {
      BNXT_TF_DBG(ERR, "fld src3 process failed\n");
      goto error;
    }
    if (eval_src3) {
      if (ulp_mapper_field_buffer_eval(val3, val3_len, &val3_int)) {
  ...
  switch (fld->field_opc) {
  case BNXT_ULP_FIELD_OPC_SRC1:
    rc = ulp_mapper_field_blob_write(fld->field_src1, blob, val1, val1_len,
                                     &val);
    val_len = val1_len;
    break;
  case BNXT_ULP_FIELD_OPC_SRC1_THEN_SRC2_ELSE_SRC3:
    if (value1) {
      rc = ulp_mapper_field_blob_write(fld->field_src2, blob, val2, val2_len,
                                       &val);
      val_len = val2_len;
    } else {
      rc = ulp_mapper_field_blob_write(fld->field_src3, blob, val3, val3_len,
                                       &val);
      val_len = val3_len;
    }
    break;
 ...
 */
static int32_t ulp_mapper_tbl_result_build(struct bnxt_ulp_mapper_parms *parms,
                                           struct bnxt_ulp_mapper_tbl_info *tbl,
  ...
  dflds = ulp_mapper_result_fields_get(parms, tbl, &num_flds, &encap_flds);
  ...
  for (i = 0; i < num_flds; i++) {
    rc = ulp_mapper_field_opc_process(parms, tbl->direction, &dflds[i], data, 0,
                                      name);
    if (rc) {
  ...
  if (encap_flds) {
    ...
    if (!tbl->record_size && !parms->device_params->dynamic_sram_en) {
      BNXT_TF_DBG(ERR, "Encap tbl record size incorrect\n");
      return -EINVAL;
    }
    if (!ulp_blob_init(&encap_blob, ULP_BYTE_2_BITS(tbl->record_size),
                       parms->device_params->encap_byte_order)) {
      BNXT_TF_DBG(ERR, "blob inits failed.\n");
      return -EINVAL;
    }
    for (; i < encap_flds; i++) {
      rc = ulp_mapper_field_opc_process(parms, tbl->direction, &dflds[i],
                                        &encap_blob, 0, name);
      if (rc) {
        BNXT_TF_DBG(ERR, "encap field processing failed\n");
        return rc;
      }
    }
    ...
    if (parms->device_params->dynamic_sram_en) {
      ...
      (void)ulp_mapper_dyn_tbl_type_get(parms, tbl, &encap_blob, &rec_s);
...
ulp_mapper_mark_gfid_process(struct bnxt_ulp_mapper_parms *parms,
  ...
  if (mark_op == BNXT_ULP_MARK_DB_OPC_NOP ||
      !(mark_op == BNXT_ULP_MARK_DB_OPC_PUSH_IF_MARK_ACTION &&
        ULP_BITMAP_ISSET(parms->act_bitmap->bits, BNXT_ULP_ACT_BIT_MARK)))
  ...
  rc = ulp_mark_db_mark_add(parms->ulp_ctx, mark_flag, gfid, mark);
  if (rc) {
...
ulp_mapper_mark_act_ptr_process(struct bnxt_ulp_mapper_parms *parms,
  ...
  if (!ulp_regfile_read(parms->regfile, BNXT_ULP_RF_IDX_MAIN_ACTION_PTR,
                        &val64)) {
  ...
  rc = ulp_mark_db_mark_add(parms->ulp_ctx, mark_flag, act_idx, mark);
  if (rc) {
    BNXT_TF_DBG(ERR, "Failed to add mark to flow\n");
    return rc;
  }
...
ulp_mapper_tcam_tbl_scan_ident_alloc(struct bnxt_ulp_mapper_parms *parms,
  ...
  idents = ulp_mapper_ident_fields_get(parms, tbl, &num_idents);
  for (i = 0; i < num_idents; i++) {
    if (ulp_mapper_ident_process(parms, tbl, &idents[i], NULL))
      return -EINVAL;
  }
  return 0;
}
...
ulp_mapper_tcam_tbl_entry_write(struct bnxt_ulp_mapper_parms *parms,
                                ...
                                struct ulp_blob *data, uint16_t idx) {
  ...
  BNXT_TF_INF("tcam[%s][%s][%x] write success.\n",
              tf_tcam_tbl_2_str(sparms.tcam_tbl_type), tf_dir_2_str(sparms.dir),
              sparms.idx);
 ...
 */
static uint32_t ulp_mapper_wc_tcam_tbl_dyn_post_process(
    struct bnxt_ulp_device_params *dparms, struct ulp_blob *key,
    struct ulp_blob *mask, struct ulp_blob *tkey, struct ulp_blob *tmask) {
  ...
  num_slices = 1;
  tlen = slice_width;
  while (tlen < blen && num_slices <= max_slices) {
  ...
  pad = tlen - blen;
  if (ulp_blob_pad_push(key, pad) < 0 || ulp_blob_pad_push(mask, pad) < 0) {
...
ulp_mapper_tcam_tbl_process(struct bnxt_ulp_mapper_parms *parms,
  ...
  if (!ulp_blob_init(key, tbl->blob_key_bit_size, key_byte_order) ||
      ...
      !ulp_blob_init(&data, tbl->result_bit_size, dparms->result_byte_order) ||
  ...
  for (i = 0; i < num_kflds; i++) {
    /* Setup the key */
    rc = ulp_mapper_field_opc_process(
        parms, tbl->direction, &kflds[i].field_info_spec, key, 1, "TCAM Key");
    if (rc) {
    ...
    /* Setup the mask */
    rc = ulp_mapper_field_opc_process(
        parms, tbl->direction, &kflds[i].field_info_mask, mask, 0, "TCAM Mask");
    if (rc) {
  ...
  if (ulp_mapper_tcam_is_wc_tcam(tbl)) {
    if (dparms->dynamic_pad_en) {
      ...
      rc = ulp_mapper_wc_tcam_tbl_dyn_post_process(dparms, key, mask, &tkey,
                                                   &tmask);
      if (rc) {
        BNXT_TF_DBG(ERR, "Failed to post proc WC entry.\n");
        return rc;
      }
  ...
  if (tbl->tbl_opcode == BNXT_ULP_TCAM_TBL_OPC_ALLOC_WR_REGFILE) {
    ...
    rc = ulp_mapper_priority_opc_process(parms, tbl, &aparms.priority);
    if (rc) {
    ...
    /* calculate the entry priority */
    rc = ulp_mapper_priority_opc_process(parms, tbl, &searchparms.priority);
    if (rc) {
      BNXT_TF_DBG(ERR, "entry priority process failed\n");
      return rc;
    }
  ...
  if (!hit || tbl->tbl_opcode == BNXT_ULP_TCAM_TBL_OPC_ALLOC_WR_REGFILE) {
    ...
    if (!rc)
      rc = ulp_mapper_tcam_tbl_entry_write(parms, tbl, key, mask, &data, idx);
  } else {
...
static int32_t ulp_mapper_em_tbl_process(struct bnxt_ulp_mapper_parms *parms,
                                         struct bnxt_ulp_mapper_tbl_info *tbl) {
  struct bnxt_ulp_mapper_key_info *kflds;
  ...
  for (i = 0; i < num_kflds; i++) {
    /* Setup the key */
    rc = ulp_mapper_field_opc_process(
        parms, tbl->direction, &kflds[i].field_info_spec, &key, 1, "EM Key");
    if (rc) {
  ...
  rc = bnxt_ulp_cntxt_tbl_scope_id_get(parms->ulp_ctx, &iparms.tbl_scope_id);
  if (rc) {
    BNXT_TF_DBG(ERR, "Failed to get table scope rc=%d\n", rc);
    return rc;
  }
...
ulp_mapper_index_tbl_process(struct bnxt_ulp_mapper_parms *parms,
  ...
  case BNXT_ULP_INDEX_TBL_OPC_WR_REGFILE:
    ...
    if (!ulp_regfile_read(parms->regfile, tbl->tbl_operand, &regval)) {
      BNXT_TF_DBG(ERR, "Failed to get tbl idx from regfile[%d].\n",
                  tbl->tbl_operand);
      return -EINVAL;
    }
  ...
  case BNXT_ULP_INDEX_TBL_OPC_WR_GLB_REGFILE:
     ...
     * write the table.
     */
    if (ulp_mapper_glb_resource_read(parms->mapper_data, tbl->direction,
                                     tbl->tbl_operand, &regval, &shared)) {
      BNXT_TF_DBG(ERR, "Failed to get tbl idx from Glb RF[%d].\n",
                  tbl->tbl_operand);
      return -EINVAL;
    }
    index = tfp_be_to_cpu_64(regval);
  ...
  case BNXT_ULP_INDEX_TBL_OPC_RD_REGFILE:
    ...
    if (tbl->resource_type == TF_TBL_TYPE_EXT) {
      ...
      BNXT_TF_DBG(ERR, "Ext Table Read Opcode not supported.\n");
      return -EINVAL;
    }
    if (!ulp_regfile_read(parms->regfile, tbl->tbl_operand, &regval)) {
      BNXT_TF_DBG(ERR, "Failed to get tbl idx from regfile[%d]\n",
     ...
     */
    rc = ulp_mapper_tbl_ident_scan_ext(
        parms, tbl, gparms.data, gparms.data_sz_in_bytes, data.byte_order);
    if (rc) {
      BNXT_TF_DBG(ERR, "Failed to get flds on tbl read rc=%d\n", rc);
      return rc;
    }
    return 0;
  default:
  ...
  if (write) {
    ...
    rc = ulp_mapper_tbl_result_build(parms, tbl, &data, "Indexed Result");
    if (rc) {
      BNXT_TF_DBG(ERR, "Failed to build the result blob\n");
      return rc;
    }
  ...
  if (alloc) {
    aparms.dir = tbl->direction;
    tbl_type = ulp_mapper_dyn_tbl_type_get(parms, tbl, &data, &tmplen);
    ...
    if (rc) {
      BNXT_TF_DBG(ERR, "Alloc table[%s][%s] failed rc=%d\n",
                  tf_tbl_type_2_str(aparms.type), tf_dir_2_str(tbl->direction),
                  rc);
      return rc;
    }
    index = aparms.idx;
    ...
    if (global) {
       ...
       */
      rc = ulp_mapper_glb_resource_write(parms->mapper_data, &glb_res, regval,
                                         false);
    } else {
    ...
    if (rc) {
      BNXT_TF_DBG(ERR, "Failed to write %s regfile[%d] rc=%d\n",
                  (global) ? "global" : "reg", tbl->tbl_operand, rc);
      goto error;
    }
  }
  ...
  if (write) {
    ...
    if (rc) {
      BNXT_TF_DBG(ERR, "Index table[%s][%s][%x] write fail rc=%d\n",
    ...
    if (tbl->resource_type == TF_TBL_TYPE_EXT) {
      ...
      act_rec_size--;
      if (ulp_regfile_write(parms->regfile, BNXT_ULP_RF_IDX_ACTION_REC_SIZE,
                            tfp_cpu_to_be_64(act_rec_size)))
        BNXT_TF_DBG(ERR, "Failed write the act rec size\n");
    }
  }
...
static int32_t ulp_mapper_if_tbl_process(struct bnxt_ulp_mapper_parms *parms,
                                         struct bnxt_ulp_mapper_tbl_info *tbl) {
  ...
  case BNXT_ULP_IF_TBL_OPC_RD_COMP_FIELD:
    ...
    if (rc) {
      BNXT_TF_DBG(ERR, "Get table[%d][%s][%x] failed rc=%d\n", get_parms.type,
                  tf_dir_2_str(get_parms.dir), get_parms.idx, rc);
      return rc;
    }
    rc = ulp_mapper_tbl_ident_scan_ext(parms, tbl, res_blob.data, res_size,
                                       res_blob.byte_order);
    if (rc)
  ...
  if (rc) {
    BNXT_TF_DBG(ERR, "Set table[%d][%s][%x] failed rc=%d\n",
                ...
                tf_dir_2_str(iftbl_params.dir), iftbl_params.idx, rc);
    return rc;
  }
  BNXT_TF_INF("Set table[%s][%s][%x] success.\n",
              tf_if_tbl_2_str(iftbl_params.type),
              tf_dir_2_str(iftbl_params.dir), iftbl_params.idx);
...
ulp_mapper_gen_tbl_process(struct bnxt_ulp_mapper_parms *parms,
  ...
  for (i = 0; i < num_kflds; i++) {
    ...
    rc = ulp_mapper_field_opc_process(parms, tbl->direction,
                                      &kflds[i].field_info_spec, &key, 1,
                                      "Gen Tbl Key");
    if (rc) {
      BNXT_TF_DBG(ERR, "Failed to create key for Gen tbl rc=%d\n", rc);
      return -EINVAL;
    }
  }
  ...
  tbl_idx =
      ulp_mapper_gen_tbl_idx_calculate(tbl->resource_sub_type, tbl->direction);
  if (tbl_idx < 0) {
  ...
  if (gen_tbl_list->hash_tbl) {
    if (tbl->gen_tbl_lkup_type != BNXT_ULP_GENERIC_TBL_LKUP_TYPE_HASH) {
    ...
    rc = ulp_gen_hash_tbl_list_key_search(gen_tbl_list->hash_tbl, &hash_entry);
    if (rc) {
    ...
    if (hash_entry.search_flag == ULP_GEN_HASH_SEARCH_FOUND) {
      ...
      if (ulp_mapper_gen_tbl_entry_get(gen_tbl_list, key_index, &gen_tbl_ent))
        return -EINVAL;
  ...
  case BNXT_ULP_GENERIC_TBL_OPC_READ:
    ...
    if (ULP_GEN_TBL_REF_CNT(&gen_tbl_ent)) {
      ...
      rc = ulp_mapper_tbl_ident_scan_ext(parms, tbl, g->byte_data,
                                         g->byte_data_size, g->byte_order);
      if (rc) {
        BNXT_TF_DBG(ERR, "Failed to scan ident list\n");
        return -EINVAL;
      }
      if (tbl->fdb_opcode != BNXT_ULP_FDB_OPC_NOP) {
  ...
  case BNXT_ULP_GENERIC_TBL_OPC_WRITE:
    if (gen_tbl_list->hash_tbl) {
      rc = ulp_mapper_gen_tbl_hash_entry_add(gen_tbl_list, &hash_entry,
                                             &gen_tbl_ent);
      if (rc)
        return rc;
      /* store the hash index in the fdb */
    ...
    /* Initialize the blob data */
    if (!ulp_blob_init(&data, tbl->result_bit_size, gen_tbl_ent.byte_order)) {
      BNXT_TF_DBG(ERR, "Failed initial index table blob\n");
      return -EINVAL;
    }

    /* Get the result fields list */
    rc = ulp_mapper_tbl_result_build(parms, tbl, &data, "Gen tbl Result");
    if (rc) {
      BNXT_TF_DBG(ERR, "Failed to build the result blob\n");
      return rc;
    }
    byte_data = ulp_blob_data_get(&data, &tmplen);
    rc = ulp_mapper_gen_tbl_entry_data_set(&gen_tbl_ent, tmplen, byte_data,
  ...
  rc = ulp_regfile_write(parms->regfile, BNXT_ULP_RF_IDX_GENERIC_TBL_MISS,
...
ulp_mapper_glb_resource_info_init(struct bnxt_ulp_context *ulp_ctx,
  ...
  for (idx = 0; idx < num_glb_res_ids; idx++) {
    if (dev_id != glb_res[idx].device_id || glb_res[idx].app_id != app_id)
      continue;
    switch (glb_res[idx].resource_func) {
    case BNXT_ULP_RESOURCE_FUNC_IDENTIFIER:
      rc = ulp_mapper_resource_ident_allocate(ulp_ctx, mapper_data,
                                              &glb_res[idx]);
      break;
    case BNXT_ULP_RESOURCE_FUNC_INDEX_TABLE:
      rc = ulp_mapper_resource_index_tbl_alloc(ulp_ctx, mapper_data,
                                               &glb_res[idx]);
      break;
    default:
 ...
 */
static int32_t ulp_mapper_app_glb_resource_info_init(
  ...
  for (i = 0; i < num_entries; i++) {
    if (dev_id != glb_res[i].device_id || app_id != glb_res[i].app_id)
      continue;
    ...
    case BNXT_ULP_RESOURCE_FUNC_INDEX_TABLE:
      ...
      if (rc) {
        BNXT_TF_DBG(ERR, "Failed to get addend for %s[%s] rc=(%d)\n",
                    tf_tbl_type_2_str(res), tf_dir_2_str(dir), rc);
        return rc;
      }
     ...
     * this function.
     */
    rc = ulp_mapper_glb_resource_write(mapper_data, &glb_res[i], regval, true);
    if (rc)
      return rc;
  }
 ...
 */
static int32_t ulp_mapper_cond_opc_process(struct bnxt_ulp_mapper_parms *parms,
                                           enum bnxt_ulp_cond_opc opc,
                                           uint32_t operand, int32_t *res) {
  ...
  case BNXT_ULP_COND_OPC_ACT_BIT_IS_SET:
    if (operand < BNXT_ULP_ACT_BIT_LAST) {
      *res = ULP_BITMAP_ISSET(parms->act_bitmap->bits, operand);
    } else {
  ...
  case BNXT_ULP_COND_OPC_HDR_BIT_IS_SET:
    if (operand < BNXT_ULP_HDR_BIT_LAST) {
      *res = ULP_BITMAP_ISSET(parms->hdr_bitmap->bits, operand);
    } else {
  ...
  case BNXT_ULP_COND_OPC_ENC_HDR_BIT_IS_SET:
    if (operand < BNXT_ULP_HDR_BIT_LAST) {
      *res = ULP_BITMAP_ISSET(parms->enc_hdr_bitmap->bits, operand);
    } else {
      BNXT_TF_DBG(ERR, "header bit out of bounds %d\n", operand);
      rc = -EINVAL;
    }
...
static int32_t ulp_mapper_func_opr_compute(struct bnxt_ulp_mapper_parms *parms,
                                           enum tf_dir dir,
  ...
  *result = false;
  ...
  case BNXT_ULP_FUNC_SRC_GLB_REGFILE:
    if (ulp_mapper_glb_resource_read(parms->mapper_data, dir, func_opr, &regval,
                                     &shared)) {
...
ulp_mapper_func_info_process(struct bnxt_ulp_mapper_parms *parms,
  ...
  if (process_src1) {
    rc =
        ulp_mapper_func_opr_compute(parms, tbl->direction, func_info->func_src1,
  ...
  if (process_src2) {
    rc =
        ulp_mapper_func_opr_compute(parms, tbl->direction, func_info->func_src2,
 ...
 */
static int32_t ulp_mapper_cond_opc_list_process(
    struct bnxt_ulp_mapper_parms *parms, enum bnxt_ulp_cond_list_opc list_opc,
    struct bnxt_ulp_mapper_cond_info *list, uint32_t num, int32_t *res) {
  uint32_t i;
  int32_t rc = 0, trc = 0;
  ...
  for (i = 0; i < num; i++) {
    rc = ulp_mapper_cond_opc_process(parms, list[i].cond_opcode,
                                     list[i].cond_operand, &trc);
    if (rc)
      return rc;
...
ulp_mapper_conflict_resolution_process(struct bnxt_ulp_mapper_parms *parms,
  ...
  *res = 0;
  ...
  case BNXT_ULP_ACCEPT_OPC_FLOW_SIG_ID_MATCH:
    ...
    if (tbl->resource_func == BNXT_ULP_RESOURCE_FUNC_GENERIC_TABLE) {
      /* Perform the check that generic table is hit or not */
      if (!ulp_regfile_read(parms->regfile, BNXT_ULP_RF_IDX_GENERIC_TBL_MISS,
                            &regval)) {
        BNXT_TF_DBG(ERR, "regfile[%d] read oob\n",
                    BNXT_ULP_RF_IDX_GENERIC_TBL_MISS);
        return -EINVAL;
    ...
    if (!ulp_regfile_read(parms->regfile, BNXT_ULP_RF_IDX_FLOW_SIG_ID,
                          &regval)) {
      BNXT_TF_DBG(ERR, "regfile[%d] read oob\n", BNXT_ULP_RF_IDX_FLOW_SIG_ID);
      return -EINVAL;
    }
    comp_sig = ULP_COMP_FLD_IDX_RD(parms, BNXT_ULP_CF_IDX_FLOW_SIG_ID);
    regval = tfp_be_to_cpu_64(regval);
    if (comp_sig == regval)
      *res = 1;
    else
      BNXT_TF_DBG(ERR, "failed signature match 0x%016" PRIX64 ":%x\n", comp_sig,
                  (uint32_t)regval);
    break;
  default:
...
static int32_t ulp_mapper_tbls_process(struct bnxt_ulp_mapper_parms *parms,
                                       uint32_t tid) {
  ...
  cond_tbls =
      ulp_mapper_tmpl_reject_list_get(parms, tid, &num_cond_tbls, &cond_opc);
  /*
   * Process the reject list if exists, otherwise assume that the
   * template is allowed.
   */
  if (cond_tbls && num_cond_tbls) {
    rc = ulp_mapper_cond_opc_list_process(parms, cond_opc, cond_tbls,
                                          num_cond_tbls, &cond_rc);
    if (rc)
      return rc;
  ...
  if (!tbls || !num_tbls) {
    BNXT_TF_DBG(ERR, "No %s tables for %d:%d\n",
                ulp_mapper_tmpl_name_str(parms->tmpl_type), parms->dev_id, tid);
    return -EINVAL;
  }
  ...
  for (tbl_idx = 0; tbl_idx < num_tbls && cond_goto;) {
    ...
    cond_tbls =
        ulp_mapper_tbl_execute_list_get(parms, tbl, &num_cond_tbls, &cond_opc);
    rc = ulp_mapper_cond_opc_list_process(parms, cond_opc, cond_tbls,
                                          num_cond_tbls, &cond_rc);
    if (rc) {
  ...
  next_iteration:
    if (cond_goto == BNXT_ULP_COND_GOTO_REJECT) {
      ...
      if (!ulp_regfile_read(parms->regfile, rf_idx, &regval)) {
        BNXT_TF_DBG(ERR, "regfile[%d] read oob\n", rf_idx);
        rc = -EINVAL;
        goto error;
      }
...
static int32_t ulp_mapper_resource_free(struct bnxt_ulp_context *ulp,
                                        uint32_t fid,
                                        struct ulp_flow_db_res_params *res) {
  struct tf *tfp;
  int32_t rc = 0;
...
int32_t ulp_mapper_resources_free(struct bnxt_ulp_context *ulp_ctx,
                                  enum bnxt_ulp_fdb_type flow_type,
                                  uint32_t fid) {
  ...
  while (!rc) {
    ...
    if (trc)
       ...
       */
      BNXT_TF_DBG(ERR,
                  "Flow[%d][0x%x] Res[%d][0x%016" PRIX64 "] failed rc=%d.\n",
...
ulp_mapper_glb_resource_info_deinit(struct bnxt_ulp_context *ulp_ctx,
  ...
  for (dir = TF_DIR_RX; dir < TF_DIR_MAX; dir++) {
    for (idx = 0; idx < BNXT_ULP_GLB_RF_IDX_LAST; idx++) {
      ...
      if (ent->resource_func == BNXT_ULP_RESOURCE_FUNC_INVALID || ent->shared)
        continue;
      ...
      res.resource_hndl = tfp_be_to_cpu_64(ent->resource_hndl);
      ulp_mapper_resource_free(ulp_ctx, 0, &res);
    }
  }
}

int32_t ulp_mapper_flow_destroy(struct bnxt_ulp_context *ulp_ctx,
                                enum bnxt_ulp_fdb_type flow_type,
                                uint32_t fid) {
  int32_t rc;
 ...
 */
int32_t ulp_mapper_flow_create(struct bnxt_ulp_context *ulp_ctx,
   ...
   */
  parms.mapper_data =
      (struct bnxt_ulp_mapper_data *)bnxt_ulp_cntxt_ptr2_mapper_data_get(
          ulp_ctx);
...
flow_error:
  ...
  trc = ulp_mapper_flow_destroy(ulp_ctx, parms.flow_type, parms.fid);
  if (trc)
...
int32_t ulp_mapper_init(struct bnxt_ulp_context *ulp_ctx) {
  ...
  data = rte_zmalloc("ulp_mapper_data", sizeof(struct bnxt_ulp_mapper_data), 0);
  if (!data) {
...
void ulp_mapper_deinit(struct bnxt_ulp_context *ulp_ctx) {
  struct bnxt_ulp_mapper_data *data;
  struct tf *tfp;

  if (!ulp_ctx) {
    BNXT_TF_DBG(
        ERR, "Failed to acquire ulp context, so data may not be released.\n");
    return;
  }

  data = (struct bnxt_ulp_mapper_data *)bnxt_ulp_cntxt_ptr2_mapper_data_get(
      ulp_ctx);
  if (!data) {
