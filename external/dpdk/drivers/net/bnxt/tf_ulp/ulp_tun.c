...
int32_t ulp_app_tun_search_entry(struct bnxt_ulp_context *ulp_ctx,
  ...
  for (i = 0; i < BNXT_ULP_MAX_TUN_CACHE_ENTRIES; i++) {
    if (!tun_ent_list[i].ref_cnt) {
      ...
      if (!memcmp(&tun_ent_list[i].app_tunnel, app_tunnel,
...
void ulp_app_tun_entry_delete(struct bnxt_flow_app_tun_ent *tun_entry) {
  if (tun_entry) {
    if (tun_entry->ref_cnt) {
      tun_entry->ref_cnt--;
      if (!tun_entry->ref_cnt)
        memset(tun_entry, 0, sizeof(struct bnxt_flow_app_tun_ent));
    }
  }
}
...
ulp_app_tun_entry_set_decap_item(struct bnxt_flow_app_tun_ent *tun_entry) {
  ...
  tun_entry->item.type =
      (typeof(tun_entry->item.type))BNXT_RTE_FLOW_ITEM_TYPE_VXLAN_DECAP;
...
static int32_t ulp_get_tun_entry(struct ulp_rte_parser_params *params,
  ...
  for (i = 0; i < BNXT_ULP_MAX_TUN_CACHE_ENTRIES; i++) {
    ...
    if ((use_ipv4 &&
         !memcmp(&tun_tbl[i].t_dst_ip, params->hdr_field[dip_idx].spec,
                 sizeof(rte_be32_t))) ||
        (!use_ipv4 &&
         !memcmp(tun_tbl[i].t_dst_ip6, params->hdr_field[dip_idx].spec,
                 sizeof(((struct bnxt_tun_cache_entry *)NULL)->t_dst_ip6)))) {
      *tun_entry = &tun_tbl[i];
      *tun_idx = i;
      return 0;
    }
  ...
  *tun_entry = &tun_tbl[first_free_entry];
  ...
  if (use_ipv4)
    memcpy(&tun_tbl[first_free_entry].t_dst_ip, params->hdr_field[dip_idx].spec,
           sizeof(rte_be32_t));
  else
    memcpy(tun_tbl[first_free_entry].t_dst_ip6, params->hdr_field[dip_idx].spec,
           sizeof(((struct bnxt_tun_cache_entry *)NULL)->t_dst_ip6));
  memcpy(tun_tbl[first_free_entry].t_dmac, params->hdr_field[dmac_idx].spec,
         RTE_ETHER_ADDR_LEN);
...
void ulp_tunnel_offload_entry_clear(struct bnxt_tun_cache_entry *tun_tbl,
                                    uint8_t tun_idx) {
...
int32_t ulp_tunnel_offload_process(struct ulp_rte_parser_params *params) {
  struct bnxt_tun_cache_entry *tun_entry;
  uint16_t tun_idx;
  int32_t rc = BNXT_TF_RC_SUCCESS;

  /* Perform the tunnel offload only for F1 and F2 flows */
  if (!ULP_BITMAP_ISSET(params->hdr_bitmap.bits, BNXT_ULP_HDR_BIT_F1) &&
      !ULP_BITMAP_ISSET(params->hdr_bitmap.bits, BNXT_ULP_HDR_BIT_F2))
    return rc;
  ...
  } else if (ULP_BITMAP_ISSET(params->hdr_bitmap.bits, BNXT_ULP_HDR_BIT_F2)) {
    ULP_BITMAP_RESET(params->hdr_bitmap.bits, BNXT_ULP_HDR_BIT_F2);
