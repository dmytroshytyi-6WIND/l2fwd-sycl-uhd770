...
static int32_t bnxt_ulp_flow_validate_args(
  ...
  if (attr->egress && attr->ingress) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, attr,
...
void bnxt_ulp_init_mapper_params(
  ...
  ULP_COMP_FLD_IDX_WR(params, BNXT_ULP_CF_IDX_HDR_SIG_ID, params->hdr_sig_id);
  ULP_COMP_FLD_IDX_WR(params, BNXT_ULP_CF_IDX_FLOW_SIG_ID, params->flow_sig_id);
  ...
  if (ULP_HIGH_AVAIL_IS_ENABLED(ulp_flags)) {
    ...
    if (region == ULP_HA_REGION_HI)
      ULP_COMP_FLD_IDX_WR(params, BNXT_ULP_CF_IDX_WC_IS_HA_HIGH_REG, 1);
  }

  /* Update the socket direct flag */
  if (ULP_BITMAP_ISSET(params->hdr_bitmap.bits, BNXT_ULP_HDR_BIT_SVIF_IGNORE)) {
    uint32_t ifindex;
    uint16_t vport;

    /* Get the port db ifindex */
    if (ulp_port_db_dev_port_to_ulp_index(params->ulp_ctx, params->port_id,
                                          &ifindex)) {
...
bnxt_ulp_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
  ...
  if (bnxt_ulp_flow_validate_args(attr, pattern, actions, error) ==
      BNXT_TF_RC_ERROR) {
  ...
  ULP_COMP_FLD_IDX_WR(&params, BNXT_ULP_CF_IDX_INCOMING_IF, dev->data->port_id);
  ULP_COMP_FLD_IDX_WR(&params, BNXT_ULP_CF_IDX_DEV_PORT_ID, dev->data->port_id);
  ULP_COMP_FLD_IDX_WR(&params, BNXT_ULP_CF_IDX_SVIF_FLAG,
                      BNXT_ULP_INVALID_SVIF_VAL);

  /* Get the function id */
  if (ulp_port_db_port_func_id_get(ulp_ctx, dev->data->port_id, &func_id)) {
   ...
   * free each of them.
   */
  rc = ulp_flow_db_fid_alloc(ulp_ctx, BNXT_ULP_FDB_TYPE_REGULAR, func_id, &fid);
  if (rc) {
...
static int bnxt_ulp_flow_validate(struct rte_eth_dev *dev,
                                  const struct rte_flow_attr *attr,
                                  const struct rte_flow_item pattern[],
                                  const struct rte_flow_action actions[],
                                  struct rte_flow_error *error) {
...
int bnxt_ulp_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  if (ulp_flow_db_validate_flow_func(ulp_ctx, flow_id, func_id) == false) {
  ...
  ret = ulp_mapper_flow_destroy(ulp_ctx, BNXT_ULP_FDB_TYPE_REGULAR, flow_id);
  if (ret) {
...
static int32_t bnxt_ulp_flow_flush(struct rte_eth_dev *eth_dev,
                                   struct rte_flow_error *error) {
  struct bnxt_ulp_context *ulp_ctx;
  int32_t ret = 0;
  uint16_t func_id;
  ...
  if (ulp_ctx_deinit_allowed(ulp_ctx)) {
    ...
    ret =
        ulp_port_db_port_func_id_get(ulp_ctx, eth_dev->data->port_id, &func_id);
    if (!ret)
...
static int32_t bnxt_ulp_flow_query(struct rte_eth_dev *eth_dev,
...
static int bnxt_ulp_tunnel_decap_set(struct rte_eth_dev *eth_dev,
...
static int bnxt_ulp_tunnel_match(struct rte_eth_dev *eth_dev,
                                 struct rte_flow_tunnel *tunnel,
...
static int bnxt_ulp_tunnel_decap_release(struct rte_eth_dev *eth_dev,
                                         struct rte_flow_action *pmd_actions,
  ...
  while (action_item && action_item->type != RTE_FLOW_ACTION_TYPE_END) {
    if (action_item->type ==
        (typeof(tun_entry->action.type))BNXT_RTE_FLOW_ACTION_TYPE_VXLAN_DECAP) {
      tun_entry = ulp_app_tun_match_entry(ulp_ctx, action_item->conf);
...
static int bnxt_ulp_tunnel_item_release(struct rte_eth_dev *eth_dev,
                                        struct rte_flow_item *pmd_items,
                                        uint32_t num_items,
                                        struct rte_flow_error *error) {
  struct bnxt_ulp_context *ulp_ctx;
