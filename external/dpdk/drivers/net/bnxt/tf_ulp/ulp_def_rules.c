...
static int32_t
ulp_set_svif_in_comp_fld(struct bnxt_ulp_context *ulp_ctx, uint32_t ifindex,
                         uint8_t svif_type,
                         struct bnxt_ulp_mapper_create_parms *mapper_params) {
  uint16_t svif;
  uint8_t idx;
...
static int32_t
ulp_set_spif_in_comp_fld(struct bnxt_ulp_context *ulp_ctx, uint32_t ifindex,
                         uint8_t spif_type,
                         struct bnxt_ulp_mapper_create_parms *mapper_params) {
  uint16_t spif;
  uint8_t idx;
...
static int32_t
ulp_set_parif_in_comp_fld(struct bnxt_ulp_context *ulp_ctx, uint32_t ifindex,
                          uint8_t parif_type,
                          struct bnxt_ulp_mapper_create_parms *mapper_params) {
  uint16_t parif;
  uint8_t idx;
...
ulp_set_vport_in_comp_fld(struct bnxt_ulp_context *ulp_ctx, uint32_t ifindex,
  ...
  ULP_COMP_FLD_IDX_WR(mapper_params, BNXT_ULP_CF_IDX_PHY_PORT_VPORT, vport);
  return 0;
}

static int32_t
ulp_set_vnic_in_comp_fld(struct bnxt_ulp_context *ulp_ctx, uint32_t ifindex,
                         uint8_t vnic_type,
                         struct bnxt_ulp_mapper_create_parms *mapper_params) {
  uint16_t vnic;
  uint8_t idx;
...
ulp_set_vlan_in_act_prop(uint16_t port_id,
  ...
  ULP_BITMAP_SET(mapper_params->act->bits, BNXT_ULP_ACT_BIT_SET_VLAN_VID);

  memcpy(&act_prop->act_details[BNXT_ULP_ACT_PROP_IDX_ENCAP_VTAG], &port_id,
         sizeof(port_id));
...
ulp_set_mark_in_act_prop(uint16_t port_id,
                         struct bnxt_ulp_mapper_create_parms *mapper_params) {
  if (ULP_BITMAP_ISSET(mapper_params->act->bits, BNXT_ULP_ACT_BIT_MARK)) {
    BNXT_TF_DBG(ERR, "MARK already set, multiple MARKs unsupported\n");
    return BNXT_TF_RC_ERROR;
  }

  ULP_COMP_FLD_IDX_WR(mapper_params, BNXT_ULP_CF_IDX_DEV_PORT_ID, port_id);
...
ulp_df_dev_port_handler(struct bnxt_ulp_context *ulp_ctx,
  ...
  /* Set port PARIF */
  rc = ulp_set_parif_in_comp_fld(ulp_ctx, ifindex, BNXT_ULP_PHY_PORT_PARIF,
                                 mapper_params);
  if (rc)
    return rc;

  /* Set DRV Func PARIF */
  rc = ulp_set_parif_in_comp_fld(ulp_ctx, ifindex, BNXT_ULP_DRV_FUNC_PARIF,
                                 mapper_params);
  if (rc)
    return rc;
...
struct bnxt_ulp_def_param_handler ulp_def_handler_tbl[] = {
    [BNXT_ULP_DF_PARAM_TYPE_DEV_PORT_ID] = {.vfr_func =
                                                ulp_df_dev_port_handler}};
...
int32_t ulp_default_flow_create(struct rte_eth_dev *eth_dev,
  ...
  if (!ulp_ctx) {
    BNXT_TF_DBG(ERR, "ULP is not init'ed. Fail to create dflt flow.\n");
    return -EINVAL;
  }
  ...
  if (ULP_VF_REP_IS_ENABLED(ulp_flags))
    ULP_COMP_FLD_IDX_WR(&mapper_params, BNXT_ULP_CF_IDX_VFR_MODE, 1);

  type = param_list->type;
  while (type != BNXT_ULP_DF_PARAM_TYPE_LAST) {
    if (ulp_def_handler_tbl[type].vfr_func) {
      rc = ulp_def_handler_tbl[type].vfr_func(ulp_ctx, param_list,
                                              &mapper_params);
      if (rc) {
  ...
  /* Get the function id */
  if (ulp_port_db_port_func_id_get(ulp_ctx, eth_dev->data->port_id,
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_default_flow_destroy(struct rte_eth_dev *eth_dev,
                                 uint32_t flow_id) {
  struct bnxt_ulp_context *ulp_ctx;
  int rc = 0;
  ...
  rc = ulp_mapper_flow_destroy(ulp_ctx, BNXT_ULP_FDB_TYPE_DEFAULT, flow_id);
  if (rc)
...
void bnxt_ulp_destroy_df_rules(struct bnxt *bp, bool global) {
  ...
  if (!global) {
    ...
    ulp_default_flow_destroy(bp->eth_dev, info->def_port_flow_id);
...
static int32_t bnxt_create_port_app_df_rule(struct bnxt *bp, uint8_t flow_type,
                                            uint32_t *flow_id) {
  ...
  struct ulp_tlv_param param_list[] = {
      ...
      {.type = BNXT_ULP_DF_PARAM_TYPE_LAST, .length = 0, .value = {0}}};

  if (!flow_type) {
    *flow_id = 0;
    return 0;
  }
  return ulp_default_flow_create(bp->eth_dev, param_list, flow_type, flow_id);
}
...
int32_t bnxt_ulp_create_df_rules(struct bnxt *bp) {
  ...
  port_id = bp->eth_dev->data->port_id;
  info = &bp->ulp_ctx->cfg_data->df_rule_info[port_id];
  rc = bnxt_create_port_app_df_rule(bp, BNXT_ULP_DF_TPL_DEFAULT_UPLINK_PORT,
                                    &info->def_port_flow_id);
  if (rc) {
    BNXT_TF_DBG(ERR, "Failed to create port to app default rule\n");
    return rc;
  }

  rc = ulp_default_flow_db_cfa_action_get(bp->ulp_ctx, info->def_port_flow_id,
                                          &bp->tx_cfa_action);
  if (rc)
    bp->tx_cfa_action = 0;
  info->valid = true;
...
static int32_t bnxt_create_port_vfr_default_rule(struct bnxt *bp,
                                                 uint8_t flow_type,
...
int32_t bnxt_ulp_create_vfr_default_rules(struct rte_eth_dev *vfr_ethdev) {
  ...
  rc = bnxt_create_port_vfr_default_rule(bp, BNXT_ULP_DF_TPL_DEFAULT_VFR,
                                         vfr_port_id, &info->vfr_flow_id);
  if (rc) {
    BNXT_TF_DBG(ERR, "Failed to create VFR default rule\n");
    goto error;
  }
  rc = ulp_default_flow_db_cfa_action_get(bp->ulp_ctx, info->vfr_flow_id,
...
int32_t bnxt_ulp_delete_vfr_default_rules(struct bnxt_representor *vfr) {
  struct bnxt_ulp_vfr_rule_info *info;
  struct rte_eth_dev *parent_dev = vfr->parent_dev;
  struct bnxt *bp = parent_dev->data->dev_private;
  ...
  if (!bp || !BNXT_TRUFLOW_EN(bp))
    return 0;
  info = bnxt_ulp_cntxt_ptr2_ulp_vfr_info_get(bp->ulp_ctx, vfr->dpdk_port_id);
  if (!info) {
    BNXT_TF_DBG(ERR, "Failed to get vfr ulp context\n");
    return -EINVAL;
  }
