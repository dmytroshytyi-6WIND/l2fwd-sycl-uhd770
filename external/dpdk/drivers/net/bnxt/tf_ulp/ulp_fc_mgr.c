...
static int ulp_fc_mgr_shadow_mem_alloc(struct hw_fc_mem_info *parms, int size) {
  /* Allocate memory*/
  if (!parms)
    return -EINVAL;

  parms->mem_va =
      rte_zmalloc("ulp_fc_info", RTE_CACHE_LINE_ROUNDUP(size), 4096);
  if (!parms->mem_va) {
...
static void ulp_fc_mgr_shadow_mem_free(struct hw_fc_mem_info *parms) {
 ...
 *
 */
int32_t ulp_fc_mgr_init(struct bnxt_ulp_context *ctxt) {
  ...
  sw_acc_cntr_tbl_sz =
      sizeof(struct sw_acc_counter) * dparms->flow_count_db_entries;

  for (i = 0; i < TF_DIR_MAX; i++) {
    ulp_fc_info->sw_acc_tbl[i] =
        rte_zmalloc("ulp_sw_acc_cntr_tbl", sw_acc_cntr_tbl_sz, 0);
...
error:
  ...
  BNXT_TF_DBG(DEBUG, "Failed to allocate memory for fc mgr\n");
 ...
 *
 */
int32_t ulp_fc_mgr_deinit(struct bnxt_ulp_context *ctxt) {
  struct bnxt_ulp_fc_info *ulp_fc_info;
 ...
 *
 */
int32_t ulp_fc_mgr_thread_start(struct bnxt_ulp_context *ctxt) {
  struct bnxt_ulp_fc_info *ulp_fc_info;
...
ulp_bulk_get_flow_stats(struct tf *tfp, struct bnxt_ulp_fc_info *fc_info,
                        enum tf_dir dir, struct bnxt_ulp_device_params *dparms)
/* MARK AS UNUSED FOR NOW TO AVOID COMPILATION ERRORS TILL API is RESOLVED */
{
  ...
  parms.physical_mem_addr =
      (uint64_t)((uintptr_t)(fc_info->shadow_hw_tbl[dir].mem_pa));

  if (!stats) {
    PMD_DRV_LOG(ERR, "BULK: Memory not initialized id:0x%x dir:%d\n",
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "BULK: Get failed for id:0x%x rc:%d\n", parms.starting_idx,
                rc);
    return rc;
  }
  ...
  for (i = 0; i < parms.num_entries; i++) {
    ...
    sw_acc_tbl_entry->byte_count += FLOW_CNTR_BYTES(stats[i], dparms);
  }

  return rc;
}

static int32_t ulp_fc_tf_flow_stat_get(struct bnxt_ulp_context *ctxt,
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Get failed for id:0x%x rc:%d\n", parms.idx, rc);
    return rc;
  }
...
static int ulp_get_single_flow_stat(struct bnxt_ulp_context *ctxt,
                                    ...
                                    enum tf_dir dir, uint32_t hw_cntr_id,
                                    struct bnxt_ulp_device_params *dparms) {
  int rc = 0;
  struct tf_get_tbl_entry_parms parms = {0};
  enum tf_tbl_type stype = TF_TBL_TYPE_ACT_STATS_64; /* TBD:Template? */
  ...
  if (sw_acc_tbl_entry->pc_flow_idx & FLOW_CNTR_PC_FLOW_VALID) {
    ...
    if (ulp_flow_db_parent_flow_count_update(ctxt, pc_idx, t_sw->pkt_count,
                                             t_sw->byte_count)) {
...
void ulp_fc_mgr_alarm_cb(void *arg) {
  int rc = 0;
  unsigned int j;
  enum tf_dir i;
  ...
  if (ctxt == NULL) {
    ...
    rte_eal_alarm_set(US_PER_S * ULP_FC_TIMER, ulp_fc_mgr_alarm_cb, arg);
    return;
  }
  ...
  */
  ...
  for (i = 0; i < TF_DIR_MAX; i++) {
    for (j = 0; j < num_entries; j++) {
      ...
      rc = ulp_get_single_flow_stat(ctxt, tfp, ulp_fc_info, i, hw_cntr_id,
                                    dparms);
      if (rc)
        break;
    }
 ...
 *
 */
bool ulp_fc_mgr_start_idx_isset(struct bnxt_ulp_context *ctxt,
                                enum tf_dir dir) {
  struct bnxt_ulp_fc_info *ulp_fc_info;
 ...
 *
 */
int ulp_fc_mgr_query_count_get(struct bnxt_ulp_context *ctxt, uint32_t flow_id,
  ...
  do {
    rc = ulp_flow_db_resource_get(ctxt, BNXT_ULP_FDB_TYPE_REGULAR, flow_id,
                                  &nxt_resource_index, &params);
    if (params.resource_func == BNXT_ULP_RESOURCE_FUNC_INDEX_TABLE &&
  ...
  if (!found_parent_flow &&
    ...
    ulp_flow_db_parent_flow_count_get(ctxt, pc_idx, &count->hits, &count->bytes,
                                      count->reset);
    if (count->hits)
      count->hits_set = 1;
    if (count->bytes)
