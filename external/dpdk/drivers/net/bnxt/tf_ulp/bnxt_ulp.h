...
#define BNXT_ULP_NAT_OUTER_MOST_FLAGS                                          \
  (BNXT_ULP_NAT_OUTER_MOST_L2_HDR_SMAC |                                       \
   BNXT_ULP_NAT_OUTER_MOST_L2_VLAN_TAGS | BNXT_ULP_NAT_OUTER_MOST_L2_HDR_DMAC)
...
#define ULP_SHARED_SESSION_IS_ENABLED(flag)                                    \
  ((flag)&BNXT_ULP_SHARED_SESSION_ENABLED)
#define ULP_APP_DEV_UNSUPPORTED_ENABLED(flag)                                  \
  ((flag)&BNXT_ULP_APP_DEV_UNSUPPORTED)
...
int32_t bnxt_ulp_ctxt_ptr2_mark_db_set(struct bnxt_ulp_context *ulp_ctx,
                                       struct bnxt_ulp_mark_tbl *mark_tbl);
...
int32_t ulp_default_flow_create(struct rte_eth_dev *eth_dev,
                                struct ulp_tlv_param *param_list,
                                uint32_t ulp_class_tid, uint32_t *flow_id);

/* Function to destroy default flows. */
int32_t ulp_default_flow_destroy(struct rte_eth_dev *eth_dev, uint32_t flow_id);

int bnxt_ulp_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
...
int32_t bnxt_ulp_get_df_rule_info(uint16_t port_id,
                                  struct bnxt_ulp_context *ulp_ctx,
...
uint8_t bnxt_ulp_cntxt_num_shared_clients_get(struct bnxt_ulp_context *ulp_ctx);
