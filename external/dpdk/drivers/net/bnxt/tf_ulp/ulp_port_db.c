...
static uint32_t ulp_port_db_allocate_ifindex(struct bnxt_ulp_port_db *port_db) {
  uint32_t idx = 1;
...
int32_t ulp_port_db_init(struct bnxt_ulp_context *ulp_ctxt, uint8_t port_cnt) {
  ...
  port_db = rte_zmalloc("bnxt_ulp_port_db", sizeof(struct bnxt_ulp_port_db), 0);
  if (!port_db) {
    BNXT_TF_DBG(ERR, "Failed to allocate memory for port db\n");
    return -ENOMEM;
  }
  ...
  port_db->ulp_intf_list = rte_zmalloc(
      "bnxt_ulp_port_db_intf_list",
      port_db->ulp_intf_list_size * sizeof(struct ulp_interface_info), 0);
  if (!port_db->ulp_intf_list) {
    BNXT_TF_DBG(ERR, "Failed to allocate mem for port interface list\n");
    goto error_free;
  }

  /* Allocate the phy port list */
  port_db->phy_port_list = rte_zmalloc(
      "bnxt_ulp_phy_port_list", port_cnt * sizeof(struct ulp_phy_port_info), 0);
  if (!port_db->phy_port_list) {
    BNXT_TF_DBG(ERR, "Failed to allocate mem for phy port list\n");
    goto error_free;
  }
...
int32_t ulp_port_db_dev_port_intf_update(struct bnxt_ulp_context *ulp_ctxt,
  ...
  intf->drv_func_id =
      bnxt_pmd_get_fw_func_id(port_id, BNXT_ULP_INTF_TYPE_INVALID);

  func = &port_db->ulp_func_id_tbl[intf->drv_func_id];
  if (!func->func_valid) {
    func->func_svif =
        bnxt_pmd_get_svif(port_id, true, BNXT_ULP_INTF_TYPE_INVALID);
    func->func_spif = bnxt_pmd_get_phy_port_id(port_id);
    func->func_parif = bnxt_pmd_get_parif(port_id, BNXT_ULP_INTF_TYPE_INVALID);
    func->func_vnic = bnxt_pmd_get_vnic_id(port_id, BNXT_ULP_INTF_TYPE_INVALID);
  ...
  if (intf->type == BNXT_ULP_INTF_TYPE_VF_REP) {
    ...
    func->func_spif = bnxt_pmd_get_phy_port_id(port_id);
    func->func_parif = bnxt_pmd_get_parif(port_id, BNXT_ULP_INTF_TYPE_INVALID);
    func->func_vnic = bnxt_pmd_get_vnic_id(port_id, BNXT_ULP_INTF_TYPE_VF_REP);
    func->phy_port_id = bnxt_pmd_get_phy_port_id(port_id);
    func->ifindex = ifindex;
  }
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_dev_port_to_ulp_index(struct bnxt_ulp_context *ulp_ctxt,
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_function_id_get(struct bnxt_ulp_context *ulp_ctxt,
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_svif_get(struct bnxt_ulp_context *ulp_ctxt,
                             uint32_t ifindex, uint32_t svif_type,
                             uint16_t *svif) {
  struct bnxt_ulp_port_db *port_db;
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_spif_get(struct bnxt_ulp_context *ulp_ctxt,
                             uint32_t ifindex, uint32_t spif_type,
                             uint16_t *spif) {
  struct bnxt_ulp_port_db *port_db;
  uint16_t phy_port_id, func_id;
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_parif_get(struct bnxt_ulp_context *ulp_ctxt,
                              uint32_t ifindex, uint32_t parif_type,
                              uint16_t *parif) {
  struct bnxt_ulp_port_db *port_db;
  uint16_t phy_port_id, func_id;
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_default_vnic_get(struct bnxt_ulp_context *ulp_ctxt,
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_vport_get(struct bnxt_ulp_context *ulp_ctxt,
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_phy_port_vport_get(struct bnxt_ulp_context *ulp_ctxt,
                                       uint32_t phy_port, uint16_t *out_port) {
  struct bnxt_ulp_port_db *port_db;
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_phy_port_svif_get(struct bnxt_ulp_context *ulp_ctxt,
                                      uint32_t phy_port, uint16_t *svif) {
  struct bnxt_ulp_port_db *port_db;
...
ulp_port_db_port_type_get(struct bnxt_ulp_context *ulp_ctxt, uint32_t ifindex) {
  struct bnxt_ulp_port_db *port_db;
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_dev_func_id_to_ulp_index(struct bnxt_ulp_context *ulp_ctxt,
                                             uint32_t func_id,
                                             uint32_t *ifindex) {
  struct bnxt_ulp_port_db *port_db;
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_port_func_id_get(struct bnxt_ulp_context *ulp_ctxt,
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_parent_mac_addr_get(struct bnxt_ulp_context *ulp_ctxt,
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_drv_mac_addr_get(struct bnxt_ulp_context *ulp_ctxt,
                                     uint32_t port_id, uint8_t **mac_addr) {
  struct ulp_func_if_info *info;
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_parent_vnic_get(struct bnxt_ulp_context *ulp_ctxt,
 ...
 * Returns 0 on success or negative number on failure.
 */
int32_t ulp_port_db_phy_port_get(struct bnxt_ulp_context *ulp_ctxt,
