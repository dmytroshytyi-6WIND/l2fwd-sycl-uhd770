...
static uint32_t ulp_matcher_class_hash_calculate(uint64_t hi_sig,
                                                 uint64_t lo_sig) {
  uint64_t hash;

  hi_sig |=
      ((hi_sig % BNXT_ULP_CLASS_HID_HIGH_PRIME) << BNXT_ULP_CLASS_HID_SHFTL);
...
static uint32_t ulp_matcher_action_hash_calculate(uint64_t hi_sig,
                                                  uint64_t app_id) {
  uint64_t hash;

  hi_sig |= ((hi_sig % BNXT_ULP_ACT_HID_HIGH_PRIME) << BNXT_ULP_ACT_HID_SHFTL);
 ...
 */
int32_t ulp_matcher_pattern_match(struct ulp_rte_parser_params *params,
                                  uint32_t *class_id) {
  ...
  class_hid = ulp_matcher_class_hash_calculate(
      (params->hdr_bitmap.bits ^ params->app_id), params->fld_s_bitmap.bits);
 ...
 */
int32_t ulp_matcher_action_match(struct ulp_rte_parser_params *params,
                                 uint32_t *act_id) {
  uint32_t act_hid;
  uint16_t tmpl_id;
