...
static uint16_t recv_burst_vec_avx2(void *rx_queue, struct rte_mbuf **rx_pkts,
                                    uint16_t nb_pkts) {
  ...
  int nb_rx_pkts = 0;
  int i;
  const __m256i valid_target = _mm256_set1_epi32(!!(raw_cons & cp_ring_size));
  ...
  const __m256i flags2_mask1 = _mm256_set1_epi32(CMPL_FLAGS2_VLAN_TUN_MSK);
  const __m256i flags2_mask2 = _mm256_set1_epi32(RX_PKT_CMPL_FLAGS2_IP_TYPE);
  const __m256i rss_mask = _mm256_set1_epi32(RX_PKT_CMPL_FLAGS_RSS_VALID);
  ...
  nb_pkts = RTE_MIN(nb_pkts,
                    RTE_MIN(rx_ring_size - mbcons, (cp_ring_size - cons) / 2));
  /*
  ...
  if (nb_pkts < BNXT_RX_DESCS_PER_LOOP_VEC256) {
    desc_valid_mask >>= CHAR_BIT * (BNXT_RX_DESCS_PER_LOOP_VEC256 - nb_pkts);
  } else {
    nb_pkts = RTE_ALIGN_FLOOR(nb_pkts, BNXT_RX_DESCS_PER_LOOP_VEC256);
  }
  ...
  for (i = 0; i < nb_pkts; i += BNXT_RX_DESCS_PER_LOOP_VEC256,
    ...
    ptype_idx = _mm256_srli_epi32(ptype_idx, RX_PKT_CMPL_FLAGS_ITYPE_SFT -
     ...
     */
    ptypes = _mm256_i32gather_epi32((int *)bnxt_ptype_table, ptype_idx,
                                    sizeof(uint32_t));
    /*
    ...
    errors = _mm256_or_si256(errors, _mm256_srli_epi32(is_tunnel, 1));
     ...
     * as possible.
     */
    ol_flags = _mm256_i32gather_epi32((int *)rxr->ol_flags_table, index,
                                      sizeof(uint32_t));
    errors = _mm256_i32gather_epi32((int *)rxr->ol_flags_err_table, errors,
                                    sizeof(uint32_t));
     ...
     */
    const __m256i perm_msk = _mm256_set_epi32(7, 3, 6, 2, 5, 1, 4, 0);
    ...
    mbuf23 = _mm256_blend_epi32(mbuf23, _mm256_srli_si256(ptypes, 4), 0x11);
    mbuf45 = _mm256_blend_epi32(mbuf45, _mm256_srli_si256(ptypes, 8), 0x11);
    mbuf67 = _mm256_blend_epi32(mbuf67, _mm256_srli_si256(ptypes, 12), 0x11);
    ...
    ol_flags_hi = _mm256_permute2f128_si256(ol_flags, ol_flags, 0x11);

    /* Set ol_flags fields for first four packets. */
    rearm0 = _mm256_blend_epi32(rearm0, _mm256_slli_si256(ol_flags, 8), 0x04);
    rearm1 =
        _mm256_blend_epi32(rearm1, _mm256_slli_si256(ol_flags_hi, 8), 0x04);
    rearm2 = _mm256_blend_epi32(rearm2, _mm256_slli_si256(ol_flags, 4), 0x04);
    rearm3 =
        _mm256_blend_epi32(rearm3, _mm256_slli_si256(ol_flags_hi, 4), 0x04);

    /* Store all mbuf fields for first four packets. */
    _mm256_storeu_si256((void *)&rx_pkts[i + 0]->rearm_data, rearm0);
    _mm256_storeu_si256((void *)&rx_pkts[i + 1]->rearm_data, rearm1);
    _mm256_storeu_si256((void *)&rx_pkts[i + 2]->rearm_data, rearm2);
    _mm256_storeu_si256((void *)&rx_pkts[i + 3]->rearm_data, rearm3);
    ...
    rearm6 = _mm256_blend_epi32(rearm6, _mm256_srli_si256(ol_flags, 4), 0x04);
    rearm7 =
        _mm256_blend_epi32(rearm7, _mm256_srli_si256(ol_flags_hi, 4), 0x04);

    /* Store all mbuf fields for final four packets. */
    _mm256_storeu_si256((void *)&rx_pkts[i + 4]->rearm_data, rearm4);
    _mm256_storeu_si256((void *)&rx_pkts[i + 5]->rearm_data, rearm5);
    _mm256_storeu_si256((void *)&rx_pkts[i + 6]->rearm_data, rearm6);
    _mm256_storeu_si256((void *)&rx_pkts[i + 7]->rearm_data, rearm7);
...
uint16_t bnxt_recv_pkts_vec_avx2(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_pkts > RTE_BNXT_MAX_RX_BURST) {
    ...
    burst = recv_burst_vec_avx2(rx_queue, rx_pkts + cnt, RTE_BNXT_MAX_RX_BURST);
...
static void bnxt_handle_tx_cp_vec(struct bnxt_tx_queue *txq) {
...
static inline void bnxt_xmit_one(struct rte_mbuf *mbuf, struct tx_bd_long *txbd,
...
static uint16_t bnxt_xmit_fixed_burst_vec(struct bnxt_tx_queue *txq,
  ...
  while (to_send >= BNXT_TX_DESCS_PER_LOOP) {
    ...
    dsc0_lo = (tx_buf[0]->data_len << 16) |
              bnxt_xmit_flags_len(tx_buf[0]->data_len, TX_BD_FLAGS_NOCMPL);

    dsc1_hi = tx_buf[1]->buf_iova + tx_buf[1]->data_off;
    dsc1_lo = (tx_buf[1]->data_len << 16) |
              bnxt_xmit_flags_len(tx_buf[1]->data_len, TX_BD_FLAGS_NOCMPL);
    ...
    dsc2_lo = (tx_buf[2]->data_len << 16) |
              bnxt_xmit_flags_len(tx_buf[2]->data_len, TX_BD_FLAGS_NOCMPL);

    dsc3_hi = tx_buf[3]->buf_iova + tx_buf[3]->data_off;
    dsc3_lo = (tx_buf[3]->data_len << 16) |
              bnxt_xmit_flags_len(tx_buf[3]->data_len, TX_BD_FLAGS_NOCMPL);
...
uint16_t bnxt_xmit_pkts_vec_avx2(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  while (nb_pkts) {
     ...
     */
    num = RTE_MIN(num, ring_size - (txr->tx_raw_prod & (ring_size - 1)));
