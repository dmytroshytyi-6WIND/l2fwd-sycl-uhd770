...
uint16_t bnxt_vfr_recv(uint16_t port_id, uint16_t queue_id,
...
static uint16_t bnxt_rep_rx_burst(void *rx_queue, struct rte_mbuf **rx_pkts,
                                  uint16_t nb_pkts) {
...
static uint16_t bnxt_rep_tx_burst(void *tx_queue, struct rte_mbuf **tx_pkts,
                                  uint16_t nb_pkts) {
...
static int bnxt_get_dflt_vnic_svif(struct bnxt *bp,
                                   struct bnxt_representor *vf_rep_bp) {
  struct bnxt_rep_info *rep_info;
  int rc;

  rc = bnxt_hwrm_get_dflt_vnic_svif(bp, vf_rep_bp->fw_fid,
                                    &vf_rep_bp->dflt_vnic_id, &vf_rep_bp->svif);
  if (rc) {
...
int bnxt_representor_init(struct rte_eth_dev *eth_dev, void *params) {
  ...
  struct bnxt_representor *rep_params = (struct bnxt_representor *)params;
  ...
  eth_dev->data->mac_addrs = (struct rte_ether_addr *)&vf_rep_bp->mac_addr;
  ...
  PMD_DRV_LOG(INFO, "Switch domain id %d: Representor Device %d init done\n",
  ...
  if (BNXT_REP_BASED_PF(vf_rep_bp)) {
    ...
    if (!(BNXT_REP_PF(vf_rep_bp))) {
      ...
      rc = bnxt_hwrm_first_vf_id_query(parent_bp, vf_rep_bp->fw_fid,
                                       &first_vf_id);
      if (rc)
        return rc;
      if (first_vf_id == 0xffff) {
        PMD_DRV_LOG(ERR, "Invalid first_vf_id fid:%x\n", vf_rep_bp->fw_fid);
        return -EINVAL;
      }
...
int bnxt_representor_uninit(struct rte_eth_dev *eth_dev) {
  ...
  if (parent_bp->rep_info)
    memset(&parent_bp->rep_info[vf_id], 0, sizeof(parent_bp->rep_info[vf_id]));
...
static int bnxt_vfr_alloc(struct rte_eth_dev *vfr_ethdev) {
  ...
  if (!vfr || !vfr->parent_dev) {
    PMD_DRV_LOG(ERR, "No memory allocated for representor\n");
    return -ENOMEM;
  }

  parent_bp = vfr->parent_dev->data->dev_private;
  if (parent_bp && !parent_bp->ulp_ctx) {
    PMD_DRV_LOG(ERR, "ulp context not allocated for parent\n");
    return -EIO;
  }
  ...
  if (!rc)
    ...
    PMD_DRV_LOG(ERR, "Failed to alloc representor %d in FW\n", vfr->vf_id);
...
static int bnxt_vfr_free(struct bnxt_representor *vfr) {
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Failed to free representor %d in FW\n", vfr->vf_id);
  }
...
int bnxt_rep_dev_info_get_op(struct rte_eth_dev *eth_dev,
  ...
  dev_info->switch_info.port_id = rep_bp->vf_id & BNXT_SWITCH_PORT_ID_VF_MASK;
...
static int bnxt_init_rep_rx_ring(struct bnxt_rx_queue *rxq,
  ...
  rxr = rte_zmalloc_socket("bnxt_rep_rx_ring", sizeof(struct bnxt_rx_ring_info),
                           RTE_CACHE_LINE_SIZE, socket_id);
  if (rxr == NULL)
    return -ENOMEM;
  rxq->rx_ring = rxr;

  ring = rte_zmalloc_socket("bnxt_rep_rx_ring_struct", sizeof(struct bnxt_ring),
                            RTE_CACHE_LINE_SIZE, socket_id);
  if (ring == NULL)
    return -ENOMEM;
...
int bnxt_rep_rx_queue_setup_op(
  ...
  if (queue_idx >= rep_bp->rx_nr_rings) {
    PMD_DRV_LOG(ERR, "Cannot create Rx ring %d. %d rings available\n",
  ...
  rxq = rte_zmalloc_socket("bnxt_vfr_rx_queue", sizeof(struct bnxt_rx_queue),
                           RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq) {
...
int bnxt_rep_tx_queue_setup_op(
    struct rte_eth_dev *eth_dev, uint16_t queue_idx, uint16_t nb_desc,
    unsigned int socket_id, __rte_unused const struct rte_eth_txconf *tx_conf) {
  struct bnxt_representor *rep_bp = eth_dev->data->dev_private;
  struct bnxt *parent_bp = rep_bp->parent_dev->data->dev_private;
  ...
  if (queue_idx >= rep_bp->rx_nr_rings) {
    PMD_DRV_LOG(ERR, "Cannot create Tx rings %d. %d rings available\n",
                queue_idx, rep_bp->rx_nr_rings);
    return -EINVAL;
  }
