...
static void bnxt_init_vnics(struct bnxt *bp) {
  ...
  for (i = 0; i < max_vnics; i++) {
    ...
    vnic->hash_mode = HWRM_VNIC_RSS_CFG_INPUT_HASH_MODE_FLAGS_DEFAULT;
    vnic->rx_queue_cnt = 0;
...
int bnxt_alloc_vnic_attributes(struct bnxt *bp, bool reconfig) {
  ...
  if (BNXT_CHIP_P5(bp))
    rss_table_size = BNXT_RSS_TBL_SIZE_P5 * 2 * sizeof(*vnic->rss_table);
  else
  ...
  for (i = 0; i < bp->max_vnics; i++) {
    ...
    snprintf(mz_name, RTE_MEMZONE_NAMESIZE, "bnxt_" PCI_PRI_FMT "_vnicattr_%d",
             pdev->addr.domain, pdev->addr.bus, pdev->addr.devid,
             pdev->addr.function, i);
    mz_name[RTE_MEMZONE_NAMESIZE - 1] = 0;
    mz = rte_memzone_lookup(mz_name);
    if (mz == NULL) {
      mz = rte_memzone_reserve(mz_name, entry_length,
...
int bnxt_alloc_vnic_mem(struct bnxt *bp) {
  ...
  if (vnic_mem == NULL) {
    PMD_DRV_LOG(ERR, "Failed to alloc memory for %d VNICs", max_vnics);
    return -ENOMEM;
  }
...
int bnxt_vnic_grp_alloc(struct bnxt *bp, struct bnxt_vnic_info *vnic) {
  ...
  if (!vnic->fw_grp_ids) {
    PMD_DRV_LOG(ERR, "Failed to alloc %d bytes for group ids\n", size);
    return -ENOMEM;
  }
...
int bnxt_rte_to_hwrm_hash_level(struct bnxt *bp, uint64_t hash_f,
                                uint32_t lvl) {
  uint32_t mode = HWRM_VNIC_RSS_CFG_INPUT_HASH_MODE_FLAGS_DEFAULT;
  bool l3 = (hash_f & (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_IPV6));
  bool l4 =
      (hash_f & (RTE_ETH_RSS_NONFRAG_IPV4_UDP | RTE_ETH_RSS_NONFRAG_IPV6_UDP |
  ...
  case BNXT_RSS_LEVEL_INNERMOST:
    if (l3_and_l4 || l4)
      ...
      mode = HWRM_VNIC_RSS_CFG_INPUT_HASH_MODE_FLAGS_INNERMOST_2;
    break;
  case BNXT_RSS_LEVEL_OUTERMOST:
    if (l3_and_l4 || l4)
      mode = HWRM_VNIC_RSS_CFG_INPUT_HASH_MODE_FLAGS_OUTERMOST_4;
    else if (l3_only)
      mode = HWRM_VNIC_RSS_CFG_INPUT_HASH_MODE_FLAGS_OUTERMOST_2;
    break;
  default:
