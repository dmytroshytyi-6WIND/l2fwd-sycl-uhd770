...
int bnxt_init_tx_ring_struct(struct bnxt_tx_queue *txq,
                             unsigned int socket_id) {
  struct bnxt_cp_ring_info *cpr;
  struct bnxt_tx_ring_info *txr;
  struct bnxt_ring *ring;

  txr = rte_zmalloc_socket("bnxt_tx_ring", sizeof(struct bnxt_tx_ring_info),
                           RTE_CACHE_LINE_SIZE, socket_id);
  if (txr == NULL)
    return -ENOMEM;
  txq->tx_ring = txr;

  ring = rte_zmalloc_socket("bnxt_tx_ring_struct", sizeof(struct bnxt_ring),
                            RTE_CACHE_LINE_SIZE, socket_id);
  if (ring == NULL)
    return -ENOMEM;
...
static bool bnxt_xmit_need_long_bd(struct rte_mbuf *tx_pkt,
...
static uint16_t bnxt_start_xmit(struct rte_mbuf *tx_pkt,
                                struct bnxt_tx_queue *txq, uint16_t *coal_pkts,
  ...
  static const uint32_t lhint_arr[4] = {
      TX_BD_LONG_FLAGS_LHINT_LT512, TX_BD_LONG_FLAGS_LHINT_LT1K,
      TX_BD_LONG_FLAGS_LHINT_LT2K, TX_BD_LONG_FLAGS_LHINT_LT2K};
  ...
  if (unlikely(nr_bds > BNXT_MAX_TSO_SEGS)) {
    PMD_DRV_LOG(ERR, "Num descriptors %d exceeds HW limit\n", nr_bds);
    return -ENOSPC;
  }
  ...
  if (unlikely(rte_pktmbuf_pkt_len(tx_pkt) < BNXT_MIN_PKT_SIZE)) {
    ...
    if (!seg) {
      PMD_DRV_LOG(ERR, "Failed to pad mbuf by %d bytes\n", pad);
      return -ENOMEM;
    }
  ...
  *last_txbd = txbd;
  ...
  if (long_bd) {
    ...
    if (tx_pkt->ol_flags & RTE_MBUF_F_TX_QINQ) {
      vlan_tag_flags =
          TX_BD_LONG_CFA_META_KEY_VLAN_TAG | tx_pkt->vlan_tci_outer;
      outer_tpid_bd = txq->bp->outer_tpid_bd & BNXT_OUTER_TPID_BD_MASK;
       ...
       */
      vlan_tag_flags = TX_BD_LONG_CFA_META_KEY_VLAN_TAG | tx_pkt->vlan_tci;
    ...
    if (tx_pkt->ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
      ...
      /* TSO */
      txbd1->lflags |= TX_BD_LONG_LFLAGS_LSO | TX_BD_LONG_LFLAGS_T_IPID;
      hdr_size = tx_pkt->l2_len + tx_pkt->l3_len + tx_pkt->l4_len;
      hdr_size += (tx_pkt->ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK)
                      ? tx_pkt->outer_l2_len + tx_pkt->outer_l3_len
                      : 0;
...
static void bnxt_tx_cmp_fast(struct bnxt_tx_queue *txq, int nr_pkts) {
  ...
  for (i = 0; i < nr_pkts; i++) {
    ...
    nr_bds = (*tx_buf)->nb_segs + bnxt_xmit_need_long_bd(*tx_buf, txq);
    for (j = 0; j < nr_bds; j++) {
      if (*tx_buf) {
...
uint16_t bnxt_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (nb_tx_pkts = 0; nb_tx_pkts < nb_pkts; nb_tx_pkts++) {
    coal_pkts++;
    rc = bnxt_start_xmit(tx_pkts[nb_tx_pkts], txq, &coal_pkts, &last_txbd);
