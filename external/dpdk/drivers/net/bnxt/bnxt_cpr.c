...
void bnxt_wait_for_device_shutdown(struct bnxt *bp) {
   ...
   */
  if (!(bp->recovery_info && (bp->fw_cap & BNXT_FW_CAP_ERR_RECOVER_RELOAD)))
    return;
...
bnxt_process_default_vnic_change(struct bnxt *bp,
  ...
  for (vf_id = 0; vf_id < BNXT_MAX_VF_REPS(bp); vf_id++) {
    ...
    if (vf_rep_bp && vf_rep_bp->fw_fid == vf_fid) {
      vfr_found = true;
      break;
    }
  }
...
static void bnxt_handle_event_error_report(struct bnxt *bp, uint32_t data1,
                                           uint32_t data2) {
  ...
  default:
    PMD_DRV_LOG(INFO,
                "FW reported unknown error type data1 %d"
                " data2: %d\n",
                data1, data2);
    break;
  }
...
void bnxt_handle_async_event(struct bnxt *bp, struct cmpl_base *cmp) {
  struct hwrm_async_event_cmpl *async_cmp = (struct hwrm_async_event_cmpl *)cmp;
  ...
  case HWRM_ASYNC_EVENT_CMPL_EVENT_ID_LINK_SPEED_CFG_CHANGE:
    ...
    rte_eth_dev_callback_process(bp->eth_dev, RTE_ETH_EVENT_INTR_LSC, NULL);
    break;
  ...
  case HWRM_ASYNC_EVENT_CMPL_EVENT_ID_RESET_NOTIFY:
    ...
    bp->fw_reset_max_msecs =
        async_cmp->timestamp_hi
    ...
    if ((event_data & EVENT_DATA1_REASON_CODE_MASK) ==
      ...
      PMD_DRV_LOG(INFO, "Port %u: Firmware fatal reset event received\n",
                  port_id);
      bp->flags |= BNXT_FLAG_FATAL_ERROR;
    } else {
      PMD_DRV_LOG(INFO, "Port %u: Firmware non-fatal reset event received\n",
                  port_id);
    }
...
void bnxt_handle_fwd_req(struct bnxt *bp, struct cmpl_base *cmpl) {
  ...
  if (bnxt_rcv_msg_from_vf(bp, vf_id, fwd_cmd)) {
    ...
    if (fwd_cmd->req_type == HWRM_CFA_L2_SET_RX_MASK) {
      ...
      srm->mask &=
          ~rte_cpu_to_le_32(HWRM_CFA_L2_SET_RX_MASK_INPUT_MASK_VLANONLY |
    ...
    if (rc) {
      PMD_DRV_LOG(ERR, "Failed to send FWD req VF 0x%x, type 0x%x.\n",
                  fw_vf_id - bp->pf->first_vf_id,
...
reject:
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Failed to send REJECT req VF 0x%x, type 0x%x.\n",
                fw_vf_id - bp->pf->first_vf_id,
                rte_le_to_cpu_16(fwd_cmd->req_type));
  }
