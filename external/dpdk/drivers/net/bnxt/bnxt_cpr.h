...
#define CMP_TYPE(cmp) (((struct cmpl_base *)cmp)->type & CMPL_BASE_TYPE_MASK)
...
#define B_CP_DB_REARM(cpr, raw_cons)                                           \
  rte_write32((DB_CP_REARM_FLAGS | DB_RING_IDX(&((cpr)->cp_db), raw_cons)),    \
              ((cpr)->cp_db.doorbell))

#define B_CP_DB_ARM(cpr) rte_write32((DB_KEY_CP), ((cpr)->cp_db.doorbell))

#define B_CP_DB_DISARM(cpr)                                                    \
  (*(uint32_t *)((cpr)->cp_db.doorbell) = DB_KEY_CP | DB_IRQ_DIS)

#define B_CP_DB_IDX_ARM(cpr, cons)                                             \
  (*(uint32_t *)((cpr)->cp_db.doorbell) = (DB_CP_REARM_FLAGS | (cons)))
...
#define B_CP_DIS_DB(cpr, raw_cons)                                             \
  rte_write32_relaxed((DB_CP_FLAGS | DB_RING_IDX(&((cpr)->cp_db), raw_cons)),  \
                      ((cpr)->cp_db.doorbell))

#define B_CP_DB(cpr, raw_cons, ring_mask)                                      \
  rte_write32((DB_CP_FLAGS | RING_CMPL((ring_mask), raw_cons)),                \
              ((cpr)->cp_db.doorbell))
...
#define DB_EPOCH(db, idx)                                                      \
  (((idx) & (db)->db_epoch_mask) << ((db)->db_epoch_shift))
#define DB_RING_IDX(db, idx) (((idx) & (db)->db_ring_mask) | DB_EPOCH(db, idx))
