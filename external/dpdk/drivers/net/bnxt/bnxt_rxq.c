...
int bnxt_mq_rx_configure(struct bnxt *bp) {
  ...
  if (dev_conf->rxmode.mq_mode & RTE_ETH_MQ_RX_VMDQ_DCB_RSS) {
    ...
    case RTE_ETH_MQ_RX_VMDQ_DCB_RSS:
      ...
      max_pools = RTE_MIN(bp->max_vnics,
                          RTE_MIN(bp->max_l2_ctx, RTE_MIN(bp->max_rsscos_ctx,
                                                          RTE_ETH_64_POOLS)));
      PMD_DRV_LOG(DEBUG, "pools = %u max_pools = %u\n", pools, max_pools);
      if (pools > max_pools)
        pools = max_pools;
      break;
  ...
  for (i = 0; i < pools; i++) {
    ...
    for (j = 0; j < nb_q_per_grp; j++, ring_idx++) {
      ...
      rxq->vnic = vnic;
      PMD_DRV_LOG(DEBUG, "rxq[%d] = %p vnic[%d] = %p\n", ring_idx, rxq, i,
                  vnic);
    }
    if (i == 0) {
...
void bnxt_rx_queue_release_mbufs(struct bnxt_rx_queue *rxq) {
  ...
  if (rxq->bp == NULL || rxq->bp->eth_dev == NULL ||
      !bnxt_need_agg_ring(rxq->bp->eth_dev))
    return;

  sw_ring = rxq->rx_ring->ag_buf_ring;
  if (sw_ring) {
    for (i = 0; i < rxq->rx_ring->ag_ring_struct->ring_size; i++) {
      if (sw_ring[i]) {
        rte_pktmbuf_free_seg(sw_ring[i]);
        sw_ring[i] = NULL;
      }
...
int bnxt_rx_queue_setup_op(struct rte_eth_dev *eth_dev, uint16_t queue_idx,
  ...
  if (queue_idx >= bnxt_max_rings(bp)) {
    PMD_DRV_LOG(ERR, "Cannot create Rx ring %d. Only %d rings available\n",
  ...
  if (rx_conf->rx_drop_en != BNXT_DEFAULT_RX_DROP_EN)
    PMD_DRV_LOG(NOTICE, "Per-queue config of drop-en is not supported.\n");
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "init_rx_ring_struct failed!\n");
    goto err;
  }
  ...
  rc = bnxt_alloc_rings(bp, socket_id, queue_idx, NULL, rxq, rxq->cp_ring, NULL,
                        "rxr");
  if (rc) {
...
int bnxt_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  ...
  if (dev_conf->rxmode.mq_mode & RTE_ETH_MQ_RX_RSS_FLAG) {
    ...
    if (BNXT_HAS_RING_GRPS(bp)) {
      ...
      vnic->fw_grp_ids[rx_queue_id] = bp->grp_info[rx_queue_id].fw_grp_id;
  ...
  PMD_DRV_LOG(INFO, "queue %d, rx_deferred_start %d, state %d!\n", rx_queue_id,
...
int bnxt_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  ...
  if (BNXT_CHIP_P5(bp)) {
    ...
    if (vnic->dflt_ring_grp == bp->grp_info[rx_queue_id].fw_grp_id) {
      for (i = vnic->start_grp_id; i < vnic->end_grp_id; i++) {
        if (bp->rx_queues[i]->rx_started) {
          vnic->dflt_ring_grp = bp->grp_info[i].fw_grp_id;
          bnxt_hwrm_vnic_cfg(bp, vnic);
          break;
        }
      }
