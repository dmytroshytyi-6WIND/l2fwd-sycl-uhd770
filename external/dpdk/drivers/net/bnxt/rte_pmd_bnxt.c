...
int bnxt_rcv_msg_from_vf(struct bnxt *bp, uint16_t vf_id, void *msg) {
  ...
  rte_eth_dev_callback_process(bp->eth_dev, RTE_ETH_EVENT_VF_MBOX, &ret_param);

  /* Default to approve */
  if (ret_param.retval == RTE_PMD_BNXT_MB_EVENT_PROCEED)
    ret_param.retval = RTE_PMD_BNXT_MB_EVENT_NOOP_ACK;

  return ret_param.retval == RTE_PMD_BNXT_MB_EVENT_NOOP_ACK ? true : false;
}
...
int rte_pmd_bnxt_set_tx_loopback(uint16_t port, uint8_t on) {
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to set Tx loopback on non-PF port %d!\n", port);
    return -ENOTSUP;
  }
...
static void rte_pmd_bnxt_set_all_queues_drop_en_cb(struct bnxt_vnic_info *vnic,
                                                   void *onptr) {
  uint8_t *on = onptr;
  vnic->bd_stall = !(*on);
}
...
int rte_pmd_bnxt_set_all_queues_drop_en(uint16_t port, uint8_t on) {
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to set all queues drop on non-PF port!\n");
    return -ENOTSUP;
  }
  ...
  for (i = 0; i < bp->pf->active_vfs; i++) {
    rc = bnxt_hwrm_func_vf_vnic_query_and_config(
        bp, i, rte_pmd_bnxt_set_all_queues_drop_en_cb, &on, bnxt_hwrm_vnic_cfg);
    if (rc) {
...
int rte_pmd_bnxt_set_vf_mac_addr(uint16_t port, uint16_t vf,
  ...
  if (rc != 0) {
    PMD_DRV_LOG(ERR, "Error during getting device (port %u) info: %s\n", port,
                strerror(-rc));
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to set VF %d mac address on non-PF port %d!\n",
                vf, port);
    return -ENOTSUP;
  }
...
int rte_pmd_bnxt_set_vf_rate_limit(uint16_t port, uint16_t vf, uint16_t tx_rate,
                                   uint64_t q_msk) {
  struct rte_eth_dev *eth_dev;
  struct rte_eth_dev_info dev_info;
  struct bnxt *bp;
...
int rte_pmd_bnxt_set_vf_mac_anti_spoof(uint16_t port, uint16_t vf, uint8_t on) {
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to set mac spoof on non-PF port %d!\n", port);
    return -EINVAL;
  }
  ...
  if (on)
    ...
    func_flags |= HWRM_FUNC_CFG_INPUT_FLAGS_SRC_MAC_ADDR_CHECK_DISABLE;
...
int rte_pmd_bnxt_set_vf_vlan_anti_spoof(uint16_t port, uint16_t vf,
                                        uint8_t on) {
  struct rte_eth_dev_info dev_info;
  struct rte_eth_dev *dev;
  struct bnxt *bp;
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to set VLAN spoof on non-PF port %d!\n", port);
    return -EINVAL;
  }
  ...
  if (!rc) {
    ...
    if (on) {
      if (bnxt_hwrm_cfa_vlan_antispoof_cfg(bp, bp->pf->first_vf_id + vf,
...
static void rte_pmd_bnxt_set_vf_vlan_stripq_cb(struct bnxt_vnic_info *vnic,
                                               void *onptr) {
  uint8_t *on = onptr;
  vnic->vlan_strip = *on;
}

int rte_pmd_bnxt_set_vf_vlan_stripq(uint16_t port, uint16_t vf, uint8_t on) {
  struct rte_eth_dev *dev;
  struct rte_eth_dev_info dev_info;
  struct bnxt *bp;
  int rc;
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to set VF %d stripq on non-PF port %d!\n", vf,
                port);
    return -ENOTSUP;
  }

  rc = bnxt_hwrm_func_vf_vnic_query_and_config(
      bp, vf, rte_pmd_bnxt_set_vf_vlan_stripq_cb, &on, bnxt_hwrm_vnic_cfg);
  if (rc)
    PMD_DRV_LOG(ERR, "Failed to update VF VNIC %d.\n", vf);
...
int rte_pmd_bnxt_set_vf_rxmode(uint16_t port, uint16_t vf, uint16_t rx_mask,
                               uint8_t on) {
  struct rte_eth_dev *dev;
  struct rte_eth_dev_info dev_info;
  uint16_t flag = 0;
  ...
  rc = bnxt_hwrm_func_vf_vnic_query_and_config(bp, vf, vf_vnic_set_rxmask_cb,
...
static int bnxt_set_vf_table(struct bnxt *bp, uint16_t vf) {
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to set VLAN table on non-PF port!\n");
    return -EINVAL;
  }
  ...
  for (i = 0; vf_mask; i++, vf_mask >>= 1) {
    ...
    if (vlan_on) {
      ...
      if (j == cnt) {
        ...
        if (cnt ==
            getpagesize() / sizeof(struct bnxt_vlan_antispoof_table_entry)) {
          PMD_DRV_LOG(ERR, "VLAN anti-spoof table is full\n");
      ...
      for (j = 0; j < cnt; j++) {
        ...
        memmove(&bp->pf->vf_info[i].vlan_table[j],
                ...
                getpagesize() -
                    ((j + 1) * sizeof(struct bnxt_vlan_table_entry)));
        memmove(&bp->pf->vf_info[i].vlan_as_table[j],
                &bp->pf->vf_info[i].vlan_as_table[j + 1],
                getpagesize() -
                    ((j + 1) * sizeof(struct bnxt_vlan_antispoof_table_entry)));
        j--;
        cnt = --bp->pf->vf_info[i].vlan_count;
      }
    }
...
int rte_pmd_bnxt_get_vf_stats(uint16_t port, uint16_t vf_id,
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to get VF %d stats on non-PF port %d!\n", vf_id,
                port);
    return -ENOTSUP;
  }

  return bnxt_hwrm_func_qstats(bp, bp->pf->first_vf_id + vf_id, stats, NULL);
}

int rte_pmd_bnxt_reset_vf_stats(uint16_t port, uint16_t vf_id) {
  struct rte_eth_dev *dev;
  struct rte_eth_dev_info dev_info;
  struct bnxt *bp;
  int rc;
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to reset VF %d stats on non-PF port %d!\n", vf_id,
                port);
    return -ENOTSUP;
  }
...
int rte_pmd_bnxt_get_vf_rx_status(uint16_t port, uint16_t vf_id) {
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to query VF %d RX stats on non-PF port %d!\n",
                vf_id, port);
    return -ENOTSUP;
  }
...
int rte_pmd_bnxt_get_vf_tx_drop_count(uint16_t port, uint16_t vf_id,
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to query VF %d TX drops on non-PF port %d!\n",
                vf_id, port);
    return -ENOTSUP;
  }
...
int rte_pmd_bnxt_mac_addr_add(uint16_t port, struct rte_ether_addr *addr,
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to config VF %d MAC on non-PF port %d!\n", vf_id,
                port);
    return -ENOTSUP;
  }
...
int rte_pmd_bnxt_set_vf_vlan_insert(uint16_t port, uint16_t vf,
                                    uint16_t vlan_id) {
  struct rte_eth_dev *dev;
  struct rte_eth_dev_info dev_info;
  struct bnxt *bp;
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to set VF %d vlan insert on non-PF port %d!\n",
                vf, port);
    return -ENOTSUP;
  }
...
int rte_pmd_bnxt_set_vf_persist_stats(uint16_t port, uint16_t vf, uint8_t on) {
  ...
  if (!BNXT_PF(bp)) {
    PMD_DRV_LOG(ERR, "Attempt to set persist stats on non-PF port %d!\n", port);
    return -EINVAL;
  }
  ...
  if (on)
    func_flags |= HWRM_FUNC_CFG_INPUT_FLAGS_NO_AUTOCLEAR_STATISTIC;
  else
    func_flags &= ~HWRM_FUNC_CFG_INPUT_FLAGS_NO_AUTOCLEAR_STATISTIC;
