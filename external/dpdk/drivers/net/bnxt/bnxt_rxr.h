...
#define BNXT_TPA_START_AGG_ID_TH(cmp) rte_le_to_cpu_16((cmp)->agg_id)
...
#define BNXT_TPA_END_AGG_BUFS_TH(cmp) ((cmp)->tpa_agg_bufs)

#define BNXT_TPA_END_AGG_ID(cmp)                                               \
  (((cmp)->agg_id & RX_TPA_END_CMPL_AGG_ID_MASK) >> RX_TPA_END_CMPL_AGG_ID_SFT)

#define BNXT_TPA_END_AGG_ID_TH(cmp) rte_le_to_cpu_16((cmp)->agg_id)

#define BNXT_RX_L2_AGG_BUFS(cmp)                                               \
  (((cmp)->agg_bufs_v1 & RX_PKT_CMPL_AGG_BUFS_MASK) >> RX_PKT_CMPL_AGG_BUFS_SFT)
...
bnxt_cfa_code_dynfield(struct rte_mbuf *mbuf) {
  return RTE_MBUF_DYNFIELD(mbuf, bnxt_cfa_code_dynfield_offset,
                           bnxt_cfa_code_dynfield_t *);
}
...
static inline void bnxt_check_ptype_constants(void) {
...
static inline void bnxt_set_vlan(struct rx_pkt_cmpl_hi *rxcmp1,
  ...
  mbuf->vlan_tci =
      metadata & (RX_PKT_CMPL_METADATA_VID_MASK | RX_PKT_CMPL_METADATA_DE |
...
#define RX_CMP_METADATA0_VID(rxcmp1)                                           \
  ((((struct rx_pkt_v2_cmpl_hi *)rxcmp1)->metadata0) &                         \
   (RX_PKT_V2_CMPL_HI_METADATA0_VID_MASK | RX_PKT_V2_CMPL_HI_METADATA0_DE |    \
...
#define RX_CMP_V2_L4_CS_OK(flags2)                                             \
  (((flags2)&RX_CMP_FLAGS2_L4_CS_OK_MASK) >> RX_CMP_FLAGS2_L4_CS_OK_SFT)

#define RX_CMP_V2_L3_CS_OK(flags2)                                             \
  (((flags2)&RX_CMP_FLAGS2_L3_CS_OK_MASK) >> RX_CMP_FLAGS2_L3_CS_OK_SFT)
...
static inline void bnxt_parse_csum_v2(struct rte_mbuf *mbuf,
                                      struct rx_pkt_cmpl_hi *rxcmp1) {
  struct rx_pkt_v2_cmpl_hi *v2_cmp = (struct rx_pkt_v2_cmpl_hi *)(rxcmp1);
...
static inline void bnxt_parse_pkt_type_v2(struct rte_mbuf *mbuf,
                                          struct rx_pkt_cmpl *rxcmp,
                                          struct rx_pkt_cmpl_hi *rxcmp1) {
  struct rx_pkt_v2_cmpl *v2_cmp = (struct rx_pkt_v2_cmpl *)(rxcmp);
  struct rx_pkt_v2_cmpl_hi *v2_cmp1 = (struct rx_pkt_v2_cmpl_hi *)(rxcmp1);
  uint16_t flags_type =
      v2_cmp->flags_type & rte_cpu_to_le_32(RX_PKT_V2_CMPL_FLAGS_ITYPE_MASK);
