...
static int page_roundup(size_t size) { return 1 << page_getenum(size); }

static void bnxt_hwrm_set_pg_attr(struct bnxt_ring_mem_info *rmem,
                                  uint8_t *pg_attr, uint64_t *pg_dir) {
  if (rmem->nr_pages == 0)
    return;
...
bnxt_get_ring_info_by_id(struct bnxt *bp, uint16_t rid, uint16_t type) {
  ...
  case HWRM_RING_FREE_INPUT_RING_TYPE_RX_AGG:
    ...
    for (i = 0; i < bp->rx_cp_nr_rings; i++) {
      ...
      if (rxq->cp_ring->cp_ring_struct->fw_ring_id == rte_cpu_to_le_16(rid)) {
        return rxq->cp_ring;
      }
    }
    break;
  case HWRM_RING_FREE_INPUT_RING_TYPE_TX:
    for (i = 0; i < bp->tx_cp_nr_rings; i++) {
      ...
      if (txq->cp_ring->cp_ring_struct->fw_ring_id == rte_cpu_to_le_16(rid)) {
        return txq->cp_ring;
      }
    }
    break;
 ...
 */
static int bnxt_check_cq_hwrm_done(struct bnxt_cp_ring_info *cpr, bool tx,
                                   bool rx, bool timeout) {
  int done = 0;
...
static int bnxt_hwrm_send_message(struct bnxt *bp, void *msg, uint32_t msg_len,
  ...
  uint16_t bar_offset = use_kong_mb ? GRCPF_REG_KONG_CHANNEL_OFFSET
                                    : GRCPF_REG_CHIMP_CHANNEL_OFFSET;
  uint16_t mb_trigger_offset =
      use_kong_mb ? GRCPF_REG_KONG_COMM_TRIGGER : GRCPF_REG_CHIMP_COMM_TRIGGER;
  ...
  if (bp->flags & BNXT_FLAG_SHORT_CMD || msg_len > bp->max_req_len) {
    ...
    short_input.signature =
        rte_cpu_to_le_16(HWRM_SHORT_INPUT_SIGNATURE_SHORT_CMD);
    short_input.size = rte_cpu_to_le_16(msg_len);
    short_input.req_addr = rte_cpu_to_le_64(bp->hwrm_short_cmd_req_dma_addr);
  ...
  for (i = 0; i < timeout; i++) {
    ...
    done = bnxt_check_cq_hwrm_done(cpr, is_tx, is_rx, i == timeout - 1);
  ...
  if (i >= timeout) {
    ...
    PMD_DRV_LOG(ERR, "Error(timeout) sending msg 0x%04x, seq_id %d\n",
 ...
 */
#define HWRM_PREP(req, type, kong)                                             \
  do {                                                                         \
...
#define HWRM_CHECK_RESULT_SILENT()                                             \
  do {                                                                         \
    if (rc) {                                                                  \
      rte_spinlock_unlock(&bp->hwrm_lock);                                     \
      return rc;                                                               \
...
#define HWRM_CHECK_RESULT()                                                    \
  do {                                                                         \
    ...
    if (resp->error_code) {                                                    \
      rc = rte_le_to_cpu_16(resp->error_code);                                 \
      if (resp->resp_len >= 16) {                                              \
        struct hwrm_err_output *tmp_hwrm_err_op = (void *)resp;                \
        PMD_DRV_LOG(ERR, "error %d:%d:%08x:%04x\n", rc,                        \
                    tmp_hwrm_err_op->cmd_err,                                  \
                    rte_le_to_cpu_32(tmp_hwrm_err_op->opaque_0),               \
                    rte_le_to_cpu_16(tmp_hwrm_err_op->opaque_1));              \
      } else {                                                                 \
...
int bnxt_hwrm_tf_message_direct(struct bnxt *bp, bool use_kong_mb,
                                uint16_t msg_type, void *msg, uint32_t msg_len,
                                void *resp_msg, uint32_t resp_len) {
  int rc = 0;
...
int bnxt_hwrm_tf_message_tunneled(struct bnxt *bp, bool use_kong_mb,
                                  uint16_t tf_type, uint16_t tf_subtype,
                                  uint32_t *tf_response_code, void *msg,
                                  uint32_t msg_len, void *response,
  ...
  if (response != NULL)
     ...
     */
    if (response_len != 0) {
      memcpy(response, resp->tf_resp, response_len);
    }
...
int bnxt_hwrm_cfa_l2_set_rx_mask(struct bnxt *bp, struct bnxt_vnic_info *vnic,
  ...
  if (vlan_table) {
    ...
    req.vlan_tag_tbl_addr = rte_cpu_to_le_64(rte_malloc_virt2iova(vlan_table));
...
int bnxt_hwrm_cfa_vlan_antispoof_cfg(
    struct bnxt *bp, uint16_t fid, uint16_t vlan_count,
...
int bnxt_hwrm_clear_l2_filter(struct bnxt *bp,
  ...
  PMD_DRV_LOG(DEBUG, "filter: %p l2_filter: %p ref_cnt: %d\n", filter,
              l2_filter, l2_filter->l2_ref_cnt);
  ...
  if (l2_filter->l2_ref_cnt == 0) {
    vnic = l2_filter->vnic;
    if (vnic) {
      STAILQ_REMOVE(&vnic->filter, l2_filter, bnxt_filter_info, next);
...
int bnxt_hwrm_set_l2_filter(struct bnxt *bp, uint16_t dst_id,
                            struct bnxt_filter_info *filter) {
  int rc = 0;
  struct hwrm_cfa_l2_filter_alloc_input req = {.req_type = 0};
  struct hwrm_cfa_l2_filter_alloc_output *resp = bp->hwrm_cmd_resp_addr;
  struct rte_eth_conf *dev_conf = &bp->eth_dev->data->dev_conf;
  const struct rte_eth_vmdq_rx_conf *conf = &dev_conf->rx_adv_conf.vmdq_rx_conf;
  uint32_t enables = 0;
  uint16_t j = dst_id - 1;
  ...
  if ((dev_conf->rxmode.mq_mode & RTE_ETH_MQ_RX_VMDQ_FLAG) &&
    ...
    PMD_DRV_LOG(DEBUG, "Add vlan %u to vmdq pool %u\n",
                conf->pool_map[j].vlan_id, j);

    filter->l2_ivlan = conf->pool_map[j].vlan_id;
    filter->enables |= HWRM_CFA_L2_FILTER_ALLOC_INPUT_ENABLES_L2_IVLAN |
  ...
  if (enables & HWRM_CFA_L2_FILTER_ALLOC_INPUT_ENABLES_L2_ADDR)
    memcpy(req.l2_addr, filter->l2_addr, RTE_ETHER_ADDR_LEN);
  ...
  if (enables & HWRM_CFA_L2_FILTER_ALLOC_INPUT_ENABLES_L2_IVLAN)
    req.l2_ivlan = filter->l2_ivlan;
  if (enables & HWRM_CFA_L2_FILTER_ALLOC_INPUT_ENABLES_L2_OVLAN_MASK)
    req.l2_ovlan_mask = filter->l2_ovlan_mask;
  if (enables & HWRM_CFA_L2_FILTER_ALLOC_INPUT_ENABLES_L2_IVLAN_MASK)
  ...
  if (filter->pri_hint) {
    ...
    req.l2_filter_id_hint = rte_cpu_to_le_64(filter->l2_filter_id_hint);
  }
...
int bnxt_hwrm_ptp_cfg(struct bnxt *bp) {
  ...
  if (ptp->rx_filter)
    ...
    flags |= HWRM_PORT_MAC_CFG_INPUT_FLAGS_PTP_RX_TS_CAPTURE_DISABLE;
  if (ptp->tx_tstamp_en)
    flags |= HWRM_PORT_MAC_CFG_INPUT_FLAGS_PTP_TX_TS_CAPTURE_ENABLE;
  else
    flags |= HWRM_PORT_MAC_CFG_INPUT_FLAGS_PTP_TX_TS_CAPTURE_DISABLE;
...
static int bnxt_hwrm_ptp_qcfg(struct bnxt *bp) {
  ...
  if (!BNXT_CHIP_P5(bp)) {
    ...
    ptp->rx_regs[BNXT_PTP_RX_FIFO] = rte_le_to_cpu_32(resp->rx_ts_reg_off_fifo);
    ...
    ptp->tx_regs[BNXT_PTP_TX_FIFO] = rte_le_to_cpu_32(resp->tx_ts_reg_off_fifo);
  }
...
static int bnxt_alloc_vf_info(struct bnxt *bp, uint16_t max_vfs) {
  ...
  for (i = 0; i < max_vfs; i++) {
    ...
    vf_info[i].vlan_table =
        rte_zmalloc("VF VLAN table", getpagesize(), getpagesize());
    ...
    vf_info[i].vlan_as_table =
        rte_zmalloc("VF VLAN AS table", getpagesize(), getpagesize());
...
int bnxt_hwrm_func_driver_register(struct bnxt *bp) {
  ...
  if (BNXT_PF(bp)) {
    ...
    memcpy(req.vf_req_fwd, bp->pf->vf_req_fwd,
           RTE_MIN(sizeof(req.vf_req_fwd), sizeof(bp->pf->vf_req_fwd)));
  }
  ...
  req.async_event_fwd[1] |= rte_cpu_to_le_32(
      ASYNC_CMPL_EVENT_ID_PF_DRVR_UNLOAD | ASYNC_CMPL_EVENT_ID_VF_CFG_CHANGE);
  if (BNXT_PF(bp))
  ...
  req.async_event_fwd[2] |= rte_cpu_to_le_32(ASYNC_CMPL_EVENT_ID_ECHO_REQUEST |
...
int bnxt_hwrm_func_reserve_vf_resc(struct bnxt *bp, bool test) {
  ...
  req.num_rx_rings = rte_cpu_to_le_16(bp->rx_nr_rings * AGG_RING_MULTIPLIER);
...
int bnxt_hwrm_ver_get(struct bnxt *bp, uint32_t timeout) {
  ...
  bp->fw_ver = ((uint32_t)resp->hwrm_fw_maj_8b << 24) |
               ...
               ((uint32_t)resp->hwrm_fw_bld_8b << 8) | resp->hwrm_fw_rsvd_8b;
  PMD_DRV_LOG(INFO, "Driver HWRM version: %d.%d.%d\n", HWRM_VERSION_MAJOR,
              HWRM_VERSION_MINOR, HWRM_VERSION_UPDATE);
  ...
  if (((dev_caps_cfg & HWRM_VER_GET_OUTPUT_DEV_CAPS_CFG_SHORT_CMD_SUPPORTED) &&
       (dev_caps_cfg & HWRM_VER_GET_OUTPUT_DEV_CAPS_CFG_SHORT_CMD_REQUIRED)) ||
      bp->hwrm_max_ext_req_len > HWRM_MAX_REQ_LEN) {
    sprintf(type, "bnxt_hwrm_short_" PCI_PRI_FMT, bp->pdev->addr.domain,
            bp->pdev->addr.bus, bp->pdev->addr.devid, bp->pdev->addr.function);

    rte_free(bp->hwrm_short_cmd_req_addr);

    bp->hwrm_short_cmd_req_addr = rte_malloc(type, bp->hwrm_max_ext_req_len, 0);
  ...
  if (conf->link_up) {
    ...
    if (!conf->link_speed) {
      ...
      if (bp->link_info->link_signal_mode) {
        enables |= HWRM_PORT_PHY_CFG_IN_EN_FORCE_PAM4_LINK_SPEED;
        req.force_pam4_link_speed = rte_cpu_to_le_16(conf->link_speed);
      } else {
    ...
    if (conf->auto_link_speed_mask &&
      ...
      if (conf->auto_pam4_link_speed_mask && bp->link_info->link_signal_mode) {
        enables |= HWRM_PORT_PHY_CFG_IN_EN_AUTO_PAM4_LINK_SPD_MASK;
        req.auto_link_pam4_speed_mask =
            rte_cpu_to_le_16(conf->auto_pam4_link_speed_mask);
      } else {
        enables |= HWRM_PORT_PHY_CFG_IN_EN_AUTO_LINK_SPEED_MASK;
        req.auto_link_speed_mask = rte_cpu_to_le_16(conf->auto_link_speed_mask);
      }
    }
    if (conf->auto_link_speed &&
        !(conf->phy_flags & HWRM_PORT_PHY_CFG_INPUT_FLAGS_FORCE))
      enables |= HWRM_PORT_PHY_CFG_INPUT_ENABLES_AUTO_LINK_SPEED;
    ...
    req.flags = rte_cpu_to_le_32(HWRM_PORT_PHY_CFG_INPUT_FLAGS_FORCE_LINK_DWN);
...
static int bnxt_hwrm_port_phy_qcfg(struct bnxt *bp,
  ...
  link_info->link_up =
      (link_info->phy_link_status == HWRM_PORT_PHY_QCFG_OUTPUT_LINK_LINK) ? 1
                                                                          : 0;
  ...
  link_info->auto_link_speed_mask =
      rte_le_to_cpu_16(resp->auto_link_speed_mask);
  link_info->preemphasis = rte_le_to_cpu_32(resp->preemphasis);
  link_info->force_link_speed = rte_le_to_cpu_16(resp->force_link_speed);
  link_info->phy_ver[0] = resp->phy_maj;
  link_info->phy_ver[1] = resp->phy_min;
  link_info->phy_ver[2] = resp->phy_bld;
  link_info->link_signal_mode = rte_le_to_cpu_16(resp->active_fec_signal_mode);
  link_info->force_pam4_link_speed =
      rte_le_to_cpu_16(resp->force_pam4_link_speed);
  link_info->support_pam4_speeds = rte_le_to_cpu_16(resp->support_pam4_speeds);
  ...
  PMD_DRV_LOG(DEBUG, "Link Signal:%d,PAM::Auto:%x,Support:%x,Force:%x\n",
              link_info->link_signal_mode, link_info->auto_pam4_link_speed_mask,
              link_info->support_pam4_speeds, link_info->force_pam4_link_speed);
  return rc;
}
...
static bool bnxt_find_lossy_profile(struct bnxt *bp) {
  ...
  for (i = BNXT_COS_QUEUE_COUNT - 1; i >= 0; i--) {
    if (bp->tx_cos_queue[i].profile == HWRM_QUEUE_SERVICE_PROFILE_LOSSY) {
...
static void bnxt_find_first_valid_profile(struct bnxt *bp) {
  ...
  for (i = BNXT_COS_QUEUE_COUNT - 1; i >= 0; i--) {
    if (bp->tx_cos_queue[i].profile != HWRM_QUEUE_SERVICE_PROFILE_UNKNOWN &&
    ...
    if (bp->vnic_cap_flags & BNXT_VNIC_CAP_COS_CLASSIFY) {
      for (j = 0, i = 0; i < BNXT_COS_QUEUE_COUNT; i++) {
        if (bp->tx_cos_queue[i].id != 0xff)
          bp->tx_cosq_id[j++] = bp->tx_cos_queue[i].id;
      }
    } else {
...
int bnxt_hwrm_ring_alloc(struct bnxt *bp, struct bnxt_ring *ring,
  ...
  case HWRM_RING_ALLOC_INPUT_RING_TYPE_TX:
    ...
    if (stats_ctx_id != INVALID_STATS_CTX_ID)
      enables |= HWRM_RING_ALLOC_INPUT_ENABLES_STAT_CTX_ID_VALID;
    break;
  case HWRM_RING_ALLOC_INPUT_RING_TYPE_RX:
    ...
    if (BNXT_CHIP_P5(bp)) {
      ...
      rx_buf_size = rte_pktmbuf_data_room_size(mb_pool) - RTE_PKTMBUF_HEADROOM;
      rx_buf_size = RTE_MIN(BNXT_MAX_PKT_LEN, rx_buf_size);
      req.rx_buf_size = rte_cpu_to_le_16(rx_buf_size);
      enables |= HWRM_RING_ALLOC_INPUT_ENABLES_RX_BUF_SIZE_VALID;
    }
    if (stats_ctx_id != INVALID_STATS_CTX_ID)
      enables |= HWRM_RING_ALLOC_INPUT_ENABLES_STAT_CTX_ID_VALID;
    break;
  case HWRM_RING_ALLOC_INPUT_RING_TYPE_L2_CMPL:
    ...
    if (BNXT_HAS_NQ(bp)) {
      ...
      enables |= HWRM_RING_ALLOC_INPUT_ENABLES_NQ_RING_ID_VALID;
    }
  ...
  if (rc || resp->error_code) {
    ...
    switch (ring_type) {
    case HWRM_RING_ALLOC_INPUT_RING_TYPE_L2_CMPL:
      PMD_DRV_LOG(ERR, "hwrm_ring_alloc cp failed. rc:%d\n", rc);
      HWRM_UNLOCK();
      return rc;
    case HWRM_RING_ALLOC_INPUT_RING_TYPE_RX:
      PMD_DRV_LOG(ERR, "hwrm_ring_alloc rx failed. rc:%d\n", rc);
      HWRM_UNLOCK();
      return rc;
    case HWRM_RING_ALLOC_INPUT_RING_TYPE_RX_AGG:
      PMD_DRV_LOG(ERR, "hwrm_ring_alloc rx agg failed. rc:%d\n", rc);
      HWRM_UNLOCK();
      return rc;
    case HWRM_RING_ALLOC_INPUT_RING_TYPE_TX:
      PMD_DRV_LOG(ERR, "hwrm_ring_alloc tx failed. rc:%d\n", rc);
      HWRM_UNLOCK();
      return rc;
    case HWRM_RING_ALLOC_INPUT_RING_TYPE_NQ:
      PMD_DRV_LOG(ERR, "hwrm_ring_alloc nq failed. rc:%d\n", rc);
      HWRM_UNLOCK();
      return rc;
    default:
...
int bnxt_hwrm_ring_free(struct bnxt *bp, struct bnxt_ring *ring,
                        uint32_t ring_type, uint16_t cp_ring_id) {
  int rc;
  ...
  if (rc || resp->error_code) {
    ...
    case HWRM_RING_FREE_INPUT_RING_TYPE_NQ:
      PMD_DRV_LOG(ERR, "hwrm_ring_free nq failed. rc:%d\n", rc);
      return rc;
    case HWRM_RING_FREE_INPUT_RING_TYPE_RX_AGG:
      PMD_DRV_LOG(ERR, "hwrm_ring_free agg failed. rc:%d\n", rc);
      return rc;
    default:
      PMD_DRV_LOG(ERR, "Invalid ring, rc:%d\n", rc);
      return rc;
...
skip_ring_grps:
  ...
  if (vnic->func_default)
    req.flags = rte_cpu_to_le_32(HWRM_VNIC_ALLOC_INPUT_FLAGS_DEFAULT);
  rc = bnxt_hwrm_send_message(bp, &req, sizeof(req), BNXT_USE_CHIMP_MB);
...
int bnxt_hwrm_vnic_cfg(struct bnxt *bp, struct bnxt_vnic_info *vnic) {
  ...
  if (BNXT_CHIP_P5(bp)) {
    ...
    req.default_rx_ring_id = rte_cpu_to_le_16(rxr->rx_ring_struct->fw_ring_id);
    ...
    if (bp->vnic_cap_flags & BNXT_VNIC_CAP_RX_CMPL_V2) {
      ...
      req.rx_csum_v2_mode = HWRM_VNIC_CFG_INPUT_RX_CSUM_V2_MODE_ALL_OK;
    }
    goto config_mru;
  }
...
config_mru:
  ...
  if (vnic->func_default && !(bp->flags & BNXT_FLAG_DFLT_VNIC_SET)) {
    req.flags |= rte_cpu_to_le_32(HWRM_VNIC_CFG_INPUT_FLAGS_DEFAULT);
    bp->flags |= BNXT_FLAG_DFLT_VNIC_SET;
  }
  if (vnic->vlan_strip)
    req.flags |= rte_cpu_to_le_32(HWRM_VNIC_CFG_INPUT_FLAGS_VLAN_STRIP_MODE);
  if (vnic->bd_stall)
...
int bnxt_hwrm_vnic_qcfg(struct bnxt *bp, struct bnxt_vnic_info *vnic,
  ...
  vnic->func_default =
      rte_le_to_cpu_32(resp->flags) & HWRM_VNIC_QCFG_OUTPUT_FLAGS_DEFAULT;
  vnic->vlan_strip = rte_le_to_cpu_32(resp->flags) &
                     HWRM_VNIC_QCFG_OUTPUT_FLAGS_VLAN_STRIP_MODE;
  vnic->bd_stall =
      rte_le_to_cpu_32(resp->flags) & HWRM_VNIC_QCFG_OUTPUT_FLAGS_BD_STALL_MODE;
...
int bnxt_hwrm_vnic_ctx_alloc(struct bnxt *bp, struct bnxt_vnic_info *vnic,
  ...
  struct hwrm_vnic_rss_cos_lb_ctx_alloc_output *resp = bp->hwrm_cmd_resp_addr;
...
static int _bnxt_hwrm_vnic_ctx_free(struct bnxt *bp,
                                    struct bnxt_vnic_info *vnic,
                                    uint16_t ctx_idx) {
  int rc = 0;
  struct hwrm_vnic_rss_cos_lb_ctx_free_input req = {.req_type = 0};
  struct hwrm_vnic_rss_cos_lb_ctx_free_output *resp = bp->hwrm_cmd_resp_addr;
...
int bnxt_hwrm_vnic_ctx_free(struct bnxt *bp, struct bnxt_vnic_info *vnic) {
  ...
  if (BNXT_CHIP_P5(bp)) {
    ...
    for (j = 0; j < vnic->num_lb_ctxts; j++) {
      rc = _bnxt_hwrm_vnic_ctx_free(bp, vnic, vnic->fw_grp_ids[j]);
...
static int bnxt_hwrm_vnic_rss_cfg_p5(struct bnxt *bp,
                                     struct bnxt_vnic_info *vnic) {
  int i;
  int rc = 0;
  ...
  for (i = 0; i < nr_ctxs; i++) {
    ...
    req.hash_key_tbl_addr = rte_cpu_to_le_64(vnic->rss_hash_key_dma_addr);

    req.ring_grp_tbl_addr =
        rte_cpu_to_le_64(vnic->rss_table_dma_addr + i * HW_HASH_INDEX_SIZE);
...
int bnxt_hwrm_vnic_rss_cfg(struct bnxt *bp, struct bnxt_vnic_info *vnic) {
  ...
  req.ring_grp_tbl_addr = rte_cpu_to_le_64(vnic->rss_table_dma_addr);
  req.hash_key_tbl_addr = rte_cpu_to_le_64(vnic->rss_hash_key_dma_addr);
...
int bnxt_hwrm_vnic_plcmode_cfg(struct bnxt *bp, struct bnxt_vnic_info *vnic) {
  int rc = 0;
  struct hwrm_vnic_plcmodes_cfg_input req = {.req_type = 0};
  struct hwrm_vnic_plcmodes_cfg_output *resp = bp->hwrm_cmd_resp_addr;
  uint16_t size;
  ...
  req.flags =
      rte_cpu_to_le_32(HWRM_VNIC_PLCMODES_CFG_INPUT_FLAGS_JUMBO_PLACEMENT);

  req.enables =
      rte_cpu_to_le_32(HWRM_VNIC_PLCMODES_CFG_INPUT_ENABLES_JUMBO_THRESH_VALID);
...
int bnxt_hwrm_vnic_tpa_cfg(struct bnxt *bp, struct bnxt_vnic_info *vnic,
  ...
  if (enable) {
    req.enables =
        rte_cpu_to_le_32(HWRM_VNIC_TPA_CFG_INPUT_ENABLES_MAX_AGG_SEGS |
                         HWRM_VNIC_TPA_CFG_INPUT_ENABLES_MAX_AGGS |
                         HWRM_VNIC_TPA_CFG_INPUT_ENABLES_MIN_AGG_LEN);
    req.flags =
        rte_cpu_to_le_32(HWRM_VNIC_TPA_CFG_INPUT_FLAGS_TPA |
...
int bnxt_hwrm_func_qstats(struct bnxt *bp, uint16_t fid,
  ...
  HWRM_CHECK_RESULT();
  if (func_qstats)
    memcpy(func_qstats, resp, sizeof(struct hwrm_func_qstats_output));
...
static int bnxt_free_all_hwrm_stat_ctxs(struct bnxt *bp) {
  int rc;
  unsigned int i;
  struct bnxt_cp_ring_info *cpr;
...
static int bnxt_free_all_hwrm_ring_grps(struct bnxt *bp) {
  uint16_t idx;
  uint32_t rc = 0;
...
void bnxt_free_cp_ring(struct bnxt *bp, struct bnxt_cp_ring_info *cpr) {
  ...
  bnxt_hwrm_ring_free(bp, cp_ring, HWRM_RING_FREE_INPUT_RING_TYPE_L2_CMPL,
                      INVALID_HW_RING_ID);
  memset(cpr->cp_desc_ring, 0,
         cpr->cp_ring_struct->ring_size * sizeof(*cpr->cp_desc_ring));
  cpr->cp_raw_cons = 0;
...
void bnxt_free_hwrm_rx_ring(struct bnxt *bp, int queue_index) {
  ...
  if (ring != NULL && cpr != NULL)
    bnxt_hwrm_ring_free(bp, ring, HWRM_RING_FREE_INPUT_RING_TYPE_RX,
   ...
   * flags are updated.
   */
  if (!bnxt_need_agg_ring(bp->eth_dev) || (rxr && rxr->ag_ring_struct == NULL))
    goto no_agg;

  ring = rxr ? rxr->ag_ring_struct : NULL;
  if (ring != NULL && cpr != NULL) {
    bnxt_hwrm_ring_free(bp, ring,
                        BNXT_CHIP_P5(bp) ? HWRM_RING_FREE_INPUT_RING_TYPE_RX_AGG
                                         : HWRM_RING_FREE_INPUT_RING_TYPE_RX,
                        cpr->cp_ring_struct->fw_ring_id);
  }
  if (BNXT_HAS_RING_GRPS(bp))
...
static int bnxt_free_all_hwrm_rings(struct bnxt *bp) {
  unsigned int i;
...
int bnxt_alloc_hwrm_resources(struct bnxt *bp) {
  ...
  sprintf(type, "bnxt_hwrm_" PCI_PRI_FMT, pdev->addr.domain, pdev->addr.bus,
          pdev->addr.devid, pdev->addr.function);
  bp->max_resp_len = BNXT_PAGE_SIZE;
  bp->hwrm_cmd_resp_addr = rte_malloc(type, bp->max_resp_len, 0);
  if (bp->hwrm_cmd_resp_addr == NULL)
    return -ENOMEM;
  bp->hwrm_cmd_resp_dma_addr = rte_malloc_virt2iova(bp->hwrm_cmd_resp_addr);
...
static int bnxt_clear_hwrm_vnic_filters(struct bnxt *bp,
                                        struct bnxt_vnic_info *vnic) {
  struct bnxt_filter_info *filter;
  int rc = 0;
...
static int bnxt_clear_hwrm_vnic_flows(struct bnxt *bp,
                                      struct bnxt_vnic_info *vnic) {
  struct bnxt_filter_info *filter;
...
int bnxt_set_hwrm_vnic_filters(struct bnxt *bp, struct bnxt_vnic_info *vnic) {
  ...
  STAILQ_FOREACH(filter, &vnic->filter, next) {
    if (filter->filter_type == HWRM_CFA_EM_FILTER)
      rc = bnxt_hwrm_set_em_filter(bp, filter->dst_id, filter);
    else if (filter->filter_type == HWRM_CFA_NTUPLE_FILTER)
      rc = bnxt_hwrm_set_ntuple_filter(bp, filter->dst_id, filter);
    else
...
static void bnxt_free_tunnel_ports(struct bnxt *bp) {
...
static uint16_t bnxt_parse_eth_link_duplex(uint32_t conf_link_speed) {
  ...
  if ((conf_link_speed & RTE_ETH_LINK_SPEED_FIXED) ==
...
static uint16_t bnxt_parse_eth_link_speed(uint32_t conf_link_speed,
  ...
  case RTE_ETH_LINK_SPEED_100M_HD:
    /* FALLTHROUGH */
    eth_link_speed = HWRM_PORT_PHY_CFG_INPUT_AUTO_LINK_SPEED_100MB;
    break;
  case RTE_ETH_LINK_SPEED_1G:
    eth_link_speed = HWRM_PORT_PHY_CFG_INPUT_AUTO_LINK_SPEED_1GB;
    break;
  case RTE_ETH_LINK_SPEED_2_5G:
    eth_link_speed = HWRM_PORT_PHY_CFG_INPUT_AUTO_LINK_SPEED_2_5GB;
    break;
  case RTE_ETH_LINK_SPEED_10G:
    eth_link_speed = HWRM_PORT_PHY_CFG_INPUT_FORCE_LINK_SPEED_10GB;
    break;
  case RTE_ETH_LINK_SPEED_20G:
    eth_link_speed = HWRM_PORT_PHY_CFG_INPUT_AUTO_LINK_SPEED_20GB;
    break;
  case RTE_ETH_LINK_SPEED_25G:
    eth_link_speed = HWRM_PORT_PHY_CFG_INPUT_AUTO_LINK_SPEED_25GB;
    break;
  case RTE_ETH_LINK_SPEED_40G:
    eth_link_speed = HWRM_PORT_PHY_CFG_INPUT_FORCE_LINK_SPEED_40GB;
    break;
  ...
  case RTE_ETH_LINK_SPEED_200G:
    ...
    break;
  default:
    PMD_DRV_LOG(ERR, "Unsupported link speed %d; default to AUTO\n",
                conf_link_speed);
    break;
  }
...
#define BNXT_SUPPORTED_SPEEDS                                                  \
  (RTE_ETH_LINK_SPEED_100M | RTE_ETH_LINK_SPEED_100M_HD |                      \
   RTE_ETH_LINK_SPEED_1G | RTE_ETH_LINK_SPEED_2_5G | RTE_ETH_LINK_SPEED_10G |  \
   RTE_ETH_LINK_SPEED_20G | RTE_ETH_LINK_SPEED_25G | RTE_ETH_LINK_SPEED_40G |  \
   RTE_ETH_LINK_SPEED_50G | RTE_ETH_LINK_SPEED_100G | RTE_ETH_LINK_SPEED_200G)
...
static int bnxt_validate_link_speed(struct bnxt *bp) {
  ...
  if (link_speed & RTE_ETH_LINK_SPEED_FIXED) {
    ...
    if ((one_speed & link_speed_capa) != one_speed) {
      PMD_DRV_LOG(ERR, "Unsupported advertised speed (%u) for port %u\n",
                  link_speed, port_id);
      return -EINVAL;
    }
  } else {
    if (!(link_speed & link_speed_capa)) {
      PMD_DRV_LOG(ERR, "Unsupported advertised speeds (%u) for port %u\n",
                  link_speed, port_id);
      return -EINVAL;
    }
  }
...
static uint16_t bnxt_parse_eth_link_speed_mask(struct bnxt *bp,
                                               uint32_t link_speed) {
  uint16_t ret = 0;
...
int bnxt_get_hwrm_link_config(struct bnxt *bp, struct rte_eth_link *link) {
  ...
  link->link_autoneg =
      link_info->auto_mode == HWRM_PORT_PHY_QCFG_OUTPUT_AUTO_MODE_NONE
          ? RTE_ETH_LINK_FIXED
...
int bnxt_set_hwrm_link_config(struct bnxt *bp, bool link_up) {
  ...
  if (BNXT_CHIP_P5(bp) && dev_conf->link_speeds == RTE_ETH_LINK_SPEED_40G) {
  ...
  if (bp->link_info->auto_link_speed == 0 && bp->link_info->link_signal_mode &&
  ...
  if (autoneg == 1 && (bp->link_info->support_auto_speeds ||
    ...
    link_req.phy_flags |= HWRM_PORT_PHY_CFG_INPUT_FLAGS_RESTART_AUTONEG;
    ...
    if (bp->link_info->phy_type == HWRM_PORT_PHY_QCFG_OUTPUT_PHY_TYPE_BASET ||
        bp->link_info->phy_type == HWRM_PORT_PHY_QCFG_OUTPUT_PHY_TYPE_BASETE ||
        bp->link_info->media_type == HWRM_PORT_PHY_QCFG_OUTPUT_MEDIA_TYPE_TP) {
    ...
    if (speed)
      ...
      link_req.link_speed = bp->link_info->force_pam4_link_speed;
    else if (bp->link_info->auto_pam4_link_speed_mask)
      link_req.link_speed = bp->link_info->auto_pam4_link_speed_mask;
    else if (bp->link_info->support_pam4_speeds)
      link_req.link_speed = bp->link_info->support_pam4_speeds;
...
int bnxt_hwrm_func_qcfg(struct bnxt *bp, uint16_t *mtu) {
  ...
  if (svif_info & HWRM_FUNC_QCFG_OUTPUT_SVIF_INFO_SVIF_VALID)
    bp->func_svif = svif_info & HWRM_FUNC_QCFG_OUTPUT_SVIF_INFO_SVIF_MASK;
  ...
  } else if (BNXT_VF(bp) && BNXT_VF_IS_TRUSTED(bp) &&
             !(flags & HWRM_FUNC_QCFG_OUTPUT_FLAGS_TRUSTED_VF)) {
    bp->flags &= ~BNXT_FLAG_TRUSTED_VF_EN;
  ...
  bp->legacy_db_size = rte_le_to_cpu_16(resp->legacy_l2_db_size_kb) * 1024;
...
int bnxt_hwrm_port_mac_qcfg(struct bnxt *bp) {
  ...
  if (port_svif_info & HWRM_PORT_MAC_QCFG_OUTPUT_PORT_SVIF_INFO_PORT_SVIF_VALID)
...
static int bnxt_hwrm_pf_func_cfg(struct bnxt *bp,
  ...
  if (BNXT_HAS_RING_GRPS(bp)) {
    ...
    req.num_hw_ring_grps = rte_cpu_to_le_16(pf_resc->num_hw_ring_grps);
 ...
 * values the same.
 */
static void bnxt_fill_vf_func_cfg_req_new(
    struct bnxt *bp, struct hwrm_func_vf_resource_cfg_input *req, int num_vfs) {
  req->max_rsscos_ctx = rte_cpu_to_le_16(bp->max_rsscos_ctx / (num_vfs + 1));
  ...
  req->max_cmpl_rings = rte_cpu_to_le_16(bp->max_cp_rings / (num_vfs + 1));
  ...
  req->max_hw_ring_grps = rte_cpu_to_le_16(bp->max_ring_grps / (num_vfs + 1));
  req->min_hw_ring_grps = req->max_hw_ring_grps;
  req->max_msix = rte_cpu_to_le_16(bp->max_nq_rings / (num_vfs + 1));
}

static void bnxt_fill_vf_func_cfg_req_old(struct bnxt *bp,
  ...
  req->admin_mtu =
      rte_cpu_to_le_16(bp->eth_dev->data->mtu + RTE_ETHER_HDR_LEN +
                       RTE_ETHER_CRC_LEN + RTE_VLAN_HLEN * BNXT_NUM_VLANS);
  req->mru = rte_cpu_to_le_16(BNXT_VNIC_MRU(bp->eth_dev->data->mtu));
 ...
 */
static int bnxt_update_max_resources(struct bnxt *bp, int vf) {
  struct hwrm_func_qcfg_input req = {0};
  struct hwrm_func_qcfg_output *resp = bp->hwrm_cmd_resp_addr;
  int rc;
...
static void bnxt_calculate_pf_resources(struct bnxt *bp,
                                        struct bnxt_pf_resource_info *pf_resc,
                                        int num_vfs) {
  if (!num_vfs) {
  ...
  pf_resc->num_rsscos_ctxs =
      bp->max_rsscos_ctx / (num_vfs + 1) + bp->max_rsscos_ctx % (num_vfs + 1);
  pf_resc->num_stat_ctxs =
      bp->max_stat_ctx / (num_vfs + 1) + bp->max_stat_ctx % (num_vfs + 1);
  pf_resc->num_cp_rings =
      bp->max_cp_rings / (num_vfs + 1) + bp->max_cp_rings % (num_vfs + 1);
  pf_resc->num_tx_rings =
      bp->max_tx_rings / (num_vfs + 1) + bp->max_tx_rings % (num_vfs + 1);
  pf_resc->num_rx_rings =
      bp->max_rx_rings / (num_vfs + 1) + bp->max_rx_rings % (num_vfs + 1);
  pf_resc->num_l2_ctxs =
      bp->max_l2_ctx / (num_vfs + 1) + bp->max_l2_ctx % (num_vfs + 1);
  pf_resc->num_hw_ring_grps =
      bp->max_ring_grps / (num_vfs + 1) + bp->max_ring_grps % (num_vfs + 1);
  pf_resc->num_nq_rings =
      bp->max_nq_rings / (num_vfs + 1) + bp->max_nq_rings % (num_vfs + 1);
  pf_resc->num_vnics =
      bp->max_vnics / (num_vfs + 1) + bp->max_vnics % (num_vfs + 1);
}
...
static int bnxt_configure_vf_req_buf(struct bnxt *bp, int num_vfs) {
...
static int bnxt_process_vf_resc_config_new(struct bnxt *bp, int num_vfs) {
  ...
  for (i = 0; i < num_vfs; i++) {
    ...
    rc = bnxt_hwrm_send_message(bp, &req, sizeof(req), BNXT_USE_CHIMP_MB);
    if (rc || resp->error_code) {
      PMD_DRV_LOG(ERR, "Failed to initialize VF %d\n", i);
      PMD_DRV_LOG(ERR, "Not all VFs available. (%d, %d)\n", rc,
                  resp->error_code);
      HWRM_UNLOCK();
...
static int bnxt_process_vf_resc_config_old(struct bnxt *bp, int num_vfs) {
  struct hwrm_func_cfg_output *resp = bp->hwrm_cmd_resp_addr;
  struct hwrm_func_cfg_input req = {0};
  int i, rc;
...
static void bnxt_configure_vf_resources(struct bnxt *bp, int num_vfs) {
...
static void bnxt_update_pf_resources(struct bnxt *bp,
                                     struct bnxt_pf_resource_info *pf_resc) {
...
int bnxt_hwrm_tunnel_dst_port_alloc(struct bnxt *bp, uint16_t port,
  ...
  case HWRM_TUNNEL_DST_PORT_ALLOC_INPUT_TUNNEL_TYPE_VXLAN:
    bp->vxlan_fw_dst_port_id = rte_le_to_cpu_16(resp->tunnel_dst_port_id);
    bp->vxlan_port = port;
    break;
  case HWRM_TUNNEL_DST_PORT_ALLOC_INPUT_TUNNEL_TYPE_GENEVE:
    bp->geneve_fw_dst_port_id = rte_le_to_cpu_16(resp->tunnel_dst_port_id);
    bp->geneve_port = port;
    break;
  default:
    break;
...
int bnxt_hwrm_tunnel_dst_port_free(struct bnxt *bp, uint16_t port,
  ...
  if (tunnel_type == HWRM_TUNNEL_DST_PORT_FREE_INPUT_TUNNEL_TYPE_VXLAN) {
    bp->vxlan_port = 0;
    bp->vxlan_port_cnt = 0;
  }

  if (tunnel_type == HWRM_TUNNEL_DST_PORT_FREE_INPUT_TUNNEL_TYPE_GENEVE) {
    bp->geneve_port = 0;
...
int bnxt_hwrm_func_buf_rgtr(struct bnxt *bp, int num_vfs) {
  ...
  if (req.req_buf_page_addr0 == RTE_BAD_IOVA) {
    PMD_DRV_LOG(ERR, "unable to map buffer address to physical memory\n");
    HWRM_UNLOCK();
    return -ENOMEM;
  }
...
int bnxt_hwrm_func_cfg_def_cp(struct bnxt *bp) {
  ...
  req.async_event_cr =
      rte_cpu_to_le_16(bp->async_cp_ring->cp_ring_struct->fw_ring_id);
  rc = bnxt_hwrm_send_message(bp, &req, sizeof(req), BNXT_USE_CHIMP_MB);
...
int bnxt_hwrm_vf_func_cfg_def_cp(struct bnxt *bp) {
  ...
  req.enables = rte_cpu_to_le_32(HWRM_FUNC_VF_CFG_INPUT_ENABLES_ASYNC_EVENT_CR);
  req.async_event_cr =
      rte_cpu_to_le_16(bp->async_cp_ring->cp_ring_struct->fw_ring_id);
  rc = bnxt_hwrm_send_message(bp, &req, sizeof(req), BNXT_USE_CHIMP_MB);
...
int bnxt_hwrm_func_bw_cfg(struct bnxt *bp, uint16_t vf, uint16_t max_bw,
                          uint16_t enables) {
  struct hwrm_func_cfg_output *resp = bp->hwrm_cmd_resp_addr;
  struct hwrm_func_cfg_input req = {0};
  int rc;
...
int bnxt_hwrm_exec_fwd_resp(struct bnxt *bp, uint16_t target_id, void *encaped,
                            size_t ec_size) {
  int rc = 0;
...
int bnxt_hwrm_ring_stats(struct bnxt *bp, uint32_t cid, int idx,
  ...
  if (rx) {
    ...
    bnxt_update_prev_stat(&ring_stats->rx_agg_pkts, &prev_stats->rx_agg_pkts);

    ring_stats->rx_agg_bytes = rte_le_to_cpu_64(resp->rx_agg_bytes);
    bnxt_update_prev_stat(&ring_stats->rx_agg_bytes, &prev_stats->rx_agg_bytes);
...
int bnxt_hwrm_port_clr_stats(struct bnxt *bp) {
  ...
  if (!(bp->flags & BNXT_FLAG_PORT_STATS) || BNXT_VF(bp) || BNXT_NPAR(bp) ||
      BNXT_MH(bp) || BNXT_TOTAL_VFS(bp))
    return 0;
...
int bnxt_hwrm_port_led_qcaps(struct bnxt *bp) {
  ...
  if (resp->num_leds > 0 && resp->num_leds < BNXT_MAX_LED) {
    ...
    memcpy(bp->leds, &resp->led0_id, sizeof(bp->leds[0]) * bp->leds->num_leds);
    for (i = 0; i < bp->leds->num_leds; i++) {
      struct bnxt_led_info *led = &bp->leds[i];

      uint16_t caps = led->led_state_caps;

      if (!led->led_group_id || !BNXT_LED_ALT_BLINK_CAP(caps)) {
        bp->leds->num_leds = 0;
        break;
      }
    }
...
int bnxt_hwrm_get_nvram_item(struct bnxt *bp, uint32_t index, uint32_t offset,
...
static void bnxt_vnic_count(struct bnxt_vnic_info *vnic __rte_unused,
                            void *cbdata) {
...
int bnxt_vf_vnic_count(struct bnxt *bp, uint16_t vf) {
  ...
  bnxt_hwrm_func_vf_vnic_query_and_config(bp, vf, bnxt_vnic_count, &count,
 ...
 */
int bnxt_hwrm_func_vf_vnic_query_and_config(
    struct bnxt *bp, uint16_t vf,
...
int bnxt_hwrm_func_cfg_vf_set_vlan_anti_spoof(struct bnxt *bp, uint16_t vf,
  ...
  req.vlan_antispoof_mode =
      on ? HWRM_FUNC_CFG_INPUT_VLAN_ANTISPOOF_MODE_VALIDATE_VLAN
...
int bnxt_hwrm_set_em_filter(struct bnxt *bp, uint16_t dst_id,
  ...
  enables = filter->enables | HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_DST_ID;
  req.dst_id = rte_cpu_to_le_16(dst_id);
  ...
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_L2_FILTER_ID)
    req.l2_filter_id = rte_cpu_to_le_64(filter->fw_l2_filter_id);
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_SRC_MACADDR)
    memcpy(req.src_macaddr, filter->src_macaddr, RTE_ETHER_ADDR_LEN);
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_DST_MACADDR)
    memcpy(req.dst_macaddr, filter->dst_macaddr, RTE_ETHER_ADDR_LEN);
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_OVLAN_VID)
    req.ovlan_vid = filter->l2_ovlan;
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_IVLAN_VID)
  ...
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_IP_PROTOCOL)
    req.ip_protocol = filter->ip_protocol;
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_SRC_IPADDR)
    req.src_ipaddr[0] = rte_cpu_to_be_32(filter->src_ipaddr[0]);
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_DST_IPADDR)
    req.dst_ipaddr[0] = rte_cpu_to_be_32(filter->dst_ipaddr[0]);
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_SRC_PORT)
    req.src_port = rte_cpu_to_be_16(filter->src_port);
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_DST_PORT)
    req.dst_port = rte_cpu_to_be_16(filter->dst_port);
  if (enables & HWRM_CFA_EM_FLOW_ALLOC_INPUT_ENABLES_MIRROR_VNIC_ID)
...
int bnxt_hwrm_clear_em_filter(struct bnxt *bp,
                              struct bnxt_filter_info *filter) {
  int rc = 0;
...
int bnxt_hwrm_set_ntuple_filter(struct bnxt *bp, uint16_t dst_id,
  ...
  struct hwrm_cfa_ntuple_filter_alloc_output *resp = bp->hwrm_cmd_resp_addr;
  uint32_t enables = 0;
  ...
  enables = filter->enables | HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_DST_ID;
  req.dst_id = rte_cpu_to_le_16(dst_id);

  if (filter->ip_addr_type) {
    req.ip_addr_type = filter->ip_addr_type;
    enables |= HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_IPADDR_TYPE;
  }
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_L2_FILTER_ID)
    req.l2_filter_id = rte_cpu_to_le_64(filter->fw_l2_filter_id);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_SRC_MACADDR)
    memcpy(req.src_macaddr, filter->src_macaddr, RTE_ETHER_ADDR_LEN);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_ETHERTYPE)
    req.ethertype = rte_cpu_to_be_16(filter->ethertype);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_IP_PROTOCOL)
    req.ip_protocol = filter->ip_protocol;
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_SRC_IPADDR)
    req.src_ipaddr[0] = rte_cpu_to_le_32(filter->src_ipaddr[0]);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_SRC_IPADDR_MASK)
    req.src_ipaddr_mask[0] = rte_cpu_to_le_32(filter->src_ipaddr_mask[0]);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_DST_IPADDR)
    req.dst_ipaddr[0] = rte_cpu_to_le_32(filter->dst_ipaddr[0]);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_DST_IPADDR_MASK)
    req.dst_ipaddr_mask[0] = rte_cpu_to_be_32(filter->dst_ipaddr_mask[0]);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_SRC_PORT)
    req.src_port = rte_cpu_to_le_16(filter->src_port);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_SRC_PORT_MASK)
    req.src_port_mask = rte_cpu_to_le_16(filter->src_port_mask);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_DST_PORT)
    req.dst_port = rte_cpu_to_le_16(filter->dst_port);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_DST_PORT_MASK)
    req.dst_port_mask = rte_cpu_to_le_16(filter->dst_port_mask);
  if (enables & HWRM_CFA_NTUPLE_FILTER_ALLOC_INPUT_ENABLES_MIRROR_VNIC_ID)
    req.mirror_vnic_id = filter->mirror_vnic_id;
...
int bnxt_hwrm_clear_ntuple_filter(struct bnxt *bp,
  ...
  struct hwrm_cfa_ntuple_filter_free_output *resp = bp->hwrm_cmd_resp_addr;
...
static int bnxt_vnic_rss_configure_p5(struct bnxt *bp,
                                      struct bnxt_vnic_info *vnic) {
  struct hwrm_vnic_rss_cfg_output *resp = bp->hwrm_cmd_resp_addr;
...
static int bnxt_hwrm_set_coal_params_p5(
    struct bnxt *bp,
...
int bnxt_hwrm_set_ring_coal(struct bnxt *bp, struct bnxt_coal *coal,
                            uint16_t ring_id) {
...
int bnxt_hwrm_func_backing_store_qcaps(struct bnxt *bp) {
  ...
  struct hwrm_func_backing_store_qcaps_output *resp = bp->hwrm_cmd_resp_addr;
  struct bnxt_ctx_pg_info *ctx_pg;
  struct bnxt_ctx_mem_info *ctx;
  int total_alloc_len;
  int rc, i, tqm_rings;

  if (!BNXT_CHIP_P5(bp) || bp->hwrm_spec_code < HWRM_VERSION_1_9_2 ||
      BNXT_VF(bp) || bp->ctx)
    return 0;
  ...
  ctx = rte_zmalloc("bnxt_ctx_mem", total_alloc_len, RTE_CACHE_LINE_SIZE);
  if (!ctx) {
    rc = -ENOMEM;
    goto ctx_err;
  }
  ...
  ctx->mrav_max_entries = rte_le_to_cpu_32(resp->mrav_max_entries);
  ...
  ctx->tqm_fp_rings_count =
      ctx->tqm_fp_rings_count
          ? RTE_MIN(ctx->tqm_fp_rings_count, BNXT_MAX_TQM_FP_LEGACY_RINGS)
          : bp->max_q;
  ...
  if (ctx->tqm_fp_rings_count == BNXT_MAX_TQM_FP_LEGACY_RINGS &&
    ...
    ctx->tqm_fp_rings_count =
        RTE_MIN(BNXT_MAX_TQM_FP_RINGS, ctx->tqm_fp_rings_count);
  }

  tqm_rings = ctx->tqm_fp_rings_count + 1;

  ctx_pg = rte_malloc("bnxt_ctx_pg_mem", sizeof(*ctx_pg) * tqm_rings,
                      RTE_CACHE_LINE_SIZE);
  if (!ctx_pg) {
    rc = -ENOMEM;
    goto ctx_err;
...
int bnxt_hwrm_func_backing_store_cfg(struct bnxt *bp, uint32_t enables) {
  ...
  struct hwrm_func_backing_store_cfg_output *resp = bp->hwrm_cmd_resp_addr;
  ...
  if (enables & HWRM_FUNC_BACKING_STORE_CFG_INPUT_ENABLES_QP) {
    ...
    req.qp_num_qp1_entries = rte_cpu_to_le_16(ctx->qp_min_qp1_entries);
    req.qp_num_l2_entries = rte_cpu_to_le_16(ctx->qp_max_l2_entries);
    req.qp_entry_size = rte_cpu_to_le_16(ctx->qp_entry_size);
    bnxt_hwrm_set_pg_attr(&ctx_pg->ring_mem, &req.qpc_pg_size_qpc_lvl,
                          &req.qpc_page_dir);
  }
  ...
  if (enables & HWRM_FUNC_BACKING_STORE_CFG_INPUT_ENABLES_SRQ) {
    ...
    req.srq_num_l2_entries = rte_cpu_to_le_16(ctx->srq_max_l2_entries);
    req.srq_entry_size = rte_cpu_to_le_16(ctx->srq_entry_size);
    bnxt_hwrm_set_pg_attr(&ctx_pg->ring_mem, &req.srq_pg_size_srq_lvl,
                          &req.srq_page_dir);
  }
  ...
  if (enables & HWRM_FUNC_BACKING_STORE_CFG_INPUT_ENABLES_CQ) {
    ...
    req.cq_num_l2_entries = rte_cpu_to_le_16(ctx->cq_max_l2_entries);
  ...
  if (enables & HWRM_FUNC_BACKING_STORE_CFG_INPUT_ENABLES_VNIC) {
    ctx_pg = &ctx->vnic_mem;
    req.vnic_num_vnic_entries = rte_cpu_to_le_16(ctx->vnic_max_vnic_entries);
    req.vnic_num_ring_table_entries =
        rte_cpu_to_le_16(ctx->vnic_max_ring_table_entries);
    req.vnic_entry_size = rte_cpu_to_le_16(ctx->vnic_entry_size);
    bnxt_hwrm_set_pg_attr(&ctx_pg->ring_mem, &req.vnic_pg_size_vnic_lvl,
                          &req.vnic_page_dir);
  }
  ...
  if (enables & HWRM_FUNC_BACKING_STORE_CFG_INPUT_ENABLES_STAT) {
    ...
    bnxt_hwrm_set_pg_attr(&ctx_pg->ring_mem, &req.stat_pg_size_stat_lvl,
                          &req.stat_page_dir);
  }
...
int bnxt_hwrm_ext_port_qstats(struct bnxt *bp) {
  ...
  if (bp->flags & BNXT_FLAG_EXT_TX_PORT_STATS) {
    req.tx_stat_host_addr = rte_cpu_to_le_64(bp->hw_tx_port_stats_ext_map);
    req.tx_stat_size = rte_cpu_to_le_16(sizeof(struct tx_port_stats_ext));
  }
  if (bp->flags & BNXT_FLAG_EXT_RX_PORT_STATS) {
    req.rx_stat_host_addr = rte_cpu_to_le_64(bp->hw_rx_port_stats_ext_map);
    req.rx_stat_size = rte_cpu_to_le_16(sizeof(struct rx_port_stats_ext));
  }
  rc = bnxt_hwrm_send_message(bp, &req, sizeof(req), BNXT_USE_CHIMP_MB);
  ...
  if (rc) {
    ...
    bp->fw_rx_port_stats_ext_size = rte_le_to_cpu_16(resp->rx_stat_size);
    bp->fw_tx_port_stats_ext_size = rte_le_to_cpu_16(resp->tx_stat_size);
  }
...
int bnxt_hwrm_set_mac(struct bnxt *bp) {
  ...
  req.enables = rte_cpu_to_le_32(HWRM_FUNC_VF_CFG_INPUT_ENABLES_DFLT_MAC_ADDR);
...
int bnxt_hwrm_if_change(struct bnxt *bp, bool up) {
  ...
  if (up)
    req.flags = rte_cpu_to_le_32(HWRM_FUNC_DRV_IF_CHANGE_INPUT_FLAGS_UP);
...
int bnxt_hwrm_error_recovery_qcfg(struct bnxt *bp) {
  ...
  info->driver_polling_freq = rte_le_to_cpu_32(resp->driver_polling_freq) * 100;
  ...
  info->reg_array_cnt = rte_le_to_cpu_32(resp->reg_array_cnt);
  ...
  for (i = 0; i < info->reg_array_cnt; i++) {
    info->reset_reg[i] = rte_le_to_cpu_32(resp->reset_reg[i]);
    info->reset_reg_val[i] = rte_le_to_cpu_32(resp->reset_reg_val[i]);
    info->delay_after_reset[i] = resp->delay_after_reset[i];
  }
err:
  HWRM_UNLOCK();
...
int bnxt_hwrm_port_ts_query(struct bnxt *bp, uint8_t path,
  ...
  if (timestamp) {
    ...
    *timestamp |= (uint64_t)(rte_le_to_cpu_32(resp->ptp_msg_ts[1])) << 32;
  }
  HWRM_UNLOCK();
...
int bnxt_hwrm_cfa_counter_qcaps(struct bnxt *bp, uint16_t *max_fc) {
  ...
  if (!(BNXT_PF(bp) || BNXT_VF_IS_TRUSTED(bp))) {
    PMD_DRV_LOG(DEBUG, "Not a PF or trusted VF. Command not supported\n");
    return 0;
  }
...
int bnxt_hwrm_cfa_counter_qstats(struct bnxt *bp, enum bnxt_flow_dir dir,
                                 uint16_t cntr, uint16_t num_entries) {
...
int bnxt_hwrm_cfa_pair_exists(struct bnxt *bp,
  ...
  req.flags = rte_cpu_to_le_32(HWRM_CFA_PAIR_INFO_INPUT_FLAGS_LOOKUP_TYPE);
...
int bnxt_hwrm_cfa_pair_alloc(struct bnxt *bp, struct bnxt_representor *rep_bp) {
  ...
  req.enables |= rep_bp->flags & BNXT_REP_Q_R2F_VALID
                     ? HWRM_CFA_PAIR_ALLOC_INPUT_ENABLES_Q_AB_VALID
                     : 0;
  ...
  req.enables |= rep_bp->flags & BNXT_REP_FC_R2F_VALID
                     ? HWRM_CFA_PAIR_ALLOC_INPUT_ENABLES_FC_AB_VALID
                     : 0;
  req.enables |= rep_bp->flags & BNXT_REP_FC_F2R_VALID
                     ? HWRM_CFA_PAIR_ALLOC_INPUT_ENABLES_FC_BA_VALID
                     : 0;
...
void bnxt_free_hwrm_tx_ring(struct bnxt *bp, int queue_index) {
  ...
  bnxt_hwrm_ring_free(bp, ring, HWRM_RING_FREE_INPUT_RING_TYPE_TX,
                      cpr->cp_ring_struct->fw_ring_id);
  txr->tx_raw_prod = 0;
  txr->tx_raw_cons = 0;
