...
static int hns3_ring_space(struct hns3_cmq_ring *ring) {
...
static bool is_valid_csq_clean_head(struct hns3_cmq_ring *ring, int head) {
  int ntu = ring->next_to_use;
  int ntc = ring->next_to_clean;
 ...
 */
static int hns3_allocate_dma_mem(struct hns3_hw *hw, struct hns3_cmq_ring *ring,
...
static void hns3_free_dma_mem(struct hns3_cmq_ring *ring) {
...
static int hns3_alloc_cmd_desc(struct hns3_hw *hw, struct hns3_cmq_ring *ring) {
...
static void hns3_free_cmd_desc(__rte_unused struct hns3_hw *hw,
                               struct hns3_cmq_ring *ring) {
  if (ring->desc)
    hns3_free_dma_mem(ring);
}

static int hns3_alloc_cmd_queue(struct hns3_hw *hw, int ring_type) {
  struct hns3_cmq_ring *ring =
...
static void hns3_cmd_clear_regs(struct hns3_hw *hw) {
...
static void hns3_cmd_config_regs(struct hns3_cmq_ring *ring) {
  uint64_t dma = ring->desc_dma_addr;

  if (ring->ring_type == HNS3_TYPE_CSQ) {
    hns3_write_dev(ring->hw, HNS3_CMDQ_TX_ADDR_L_REG, lower_32_bits(dma));
    hns3_write_dev(ring->hw, HNS3_CMDQ_TX_ADDR_H_REG, upper_32_bits(dma));
    ...
    hns3_write_dev(ring->hw, HNS3_CMDQ_RX_ADDR_L_REG, lower_32_bits(dma));
    hns3_write_dev(ring->hw, HNS3_CMDQ_RX_ADDR_H_REG, upper_32_bits(dma));
...
static void hns3_cmd_init_regs(struct hns3_hw *hw) {
  hns3_cmd_config_regs(&hw->cmq.csq);
  hns3_cmd_config_regs(&hw->cmq.crq);
}

static int hns3_cmd_csq_clean(struct hns3_hw *hw) {
...
static int hns3_cmd_csq_done(struct hns3_hw *hw) {
  uint32_t head = hns3_read_dev(hw, HNS3_CMDQ_TX_HEAD_REG);

  return head == hw->cmq.csq.next_to_use;
}

static bool hns3_is_special_opcode(uint16_t opcode) {
  /*
   * These commands have several descriptors,
   * and use the first one to save opcode and return value.
   */
  uint16_t spec_opcode[] = {
      HNS3_OPC_STATS_64_BIT,
...
static int hns3_cmd_convert_err_code(uint16_t desc_ret) {
  static const struct {
    uint16_t imp_errcode;
    int linux_errcode;
  } hns3_cmdq_status[] = {{HNS3_CMD_EXEC_SUCCESS, 0},
...
static int hns3_cmd_get_hardware_reply(struct hns3_hw *hw,
...
static int hns3_cmd_poll_reply(struct hns3_hw *hw) {
  ...
  do {
    ...
    if (__atomic_load_n(&hw->reset.disable_cmd, __ATOMIC_RELAXED)) {
      hns3_err(hw, "Don't wait for reply because of disable_cmd");
      return -EBUSY;
    }
...
int hns3_cmd_send(struct hns3_hw *hw, struct hns3_cmd_desc *desc, int num) {
  ...
  if (HNS3_CMD_SEND_SYNC(rte_le_to_cpu_16(desc->flag))) {
    ...
    if (!retval)
      retval = hns3_cmd_get_hardware_reply(hw, desc, num, ntc);
  }
...
static const char *hns3_get_caps_name(uint32_t caps_id) {
  const struct {
    enum HNS3_CAPS_BITS caps;
    const char *name;
  } dev_caps[] = {{HNS3_CAPS_FD_QUEUE_REGION_B, "fd_queue_region"},
                  {HNS3_CAPS_PTP_B, "ptp"},
...
static void hns3_mask_capability(struct hns3_hw *hw,
...
#define MAX_CAPS_BIT 64
  ...
  for (i = 0; i < MAX_CAPS_BIT; i++) {
    if (!(caps_masked & BIT_ULL(i)))
      continue;
    hns3_info(hw, "mask capability: id-%u, name-%s.", i, hns3_get_caps_name(i));
  }
}
...
static void hns3_parse_capability(struct hns3_hw *hw,
  ...
  if (hns3_get_bit(caps, HNS3_CAPS_FD_QUEUE_REGION_B))
    hns3_set_bit(hw->capability, HNS3_DEV_SUPPORT_FD_QUEUE_REGION_B, 1);
  ...
  if (hns3_get_bit(caps, HNS3_CAPS_RXD_ADV_LAYOUT_B))
    hns3_set_bit(hw->capability, HNS3_DEV_SUPPORT_RXD_ADV_LAYOUT_B, 1);
  if (hns3_get_bit(caps, HNS3_CAPS_UDP_TUNNEL_CSUM_B))
    hns3_set_bit(hw->capability, HNS3_DEV_SUPPORT_OUTER_UDP_CKSUM_B, 1);
...
static uint32_t hns3_build_api_caps(void) {
  uint32_t api_caps = 0;
...
static int hns3_cmd_query_firmware_version_and_capability(struct hns3_hw *hw) {
...
static void hns3_update_dev_lsc_cap(struct hns3_hw *hw,
...
static int hns3_apply_fw_compat_cmd_result(struct hns3_hw *hw, int result) {
  if (result != 0 && hns3_dev_get_support(hw, COPPER)) {
    hns3_err(hw, "firmware fails to initialize the PHY, ret = %d.", result);
    return result;
  }
...
static int hns3_firmware_compat_config(struct hns3_hw *hw, bool is_init) {
  ...
  if (ret != HNS3_PCI_REVISION_ID_LEN) {
    PMD_INIT_LOG(ERR, "failed to read pci revision id, ret = %d", ret);
    return -EIO;
  }
...
int hns3_cmd_init(struct hns3_hw *hw) {
  ...
  PMD_INIT_LOG(
      INFO, "The firmware version is %lu.%lu.%lu.%lu",
  ...
  if (ret)
    PMD_INIT_LOG(WARNING,
                 "firmware compatible features not "
                 "supported, ret = %d.",
                 ret);
...
static void hns3_destroy_queue(struct hns3_hw *hw, struct hns3_cmq_ring *ring) {
