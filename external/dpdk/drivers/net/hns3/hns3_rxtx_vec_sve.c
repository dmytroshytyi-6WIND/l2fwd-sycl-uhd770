...
static inline uint32_t hns3_desc_parse_field_sve(struct hns3_rx_queue *rxq,
  ...
  for (i = 0; i < (int)bd_vld_num; i++) {
    ...
    rx_pkts[i]->packet_type =
        hns3_rx_calc_ptype(rxq, key->l234_info[i], key->ol_info[i]);
...
static inline void hns3_rx_prefetch_mbuf_sve(struct hns3_entry *sw_ring) {
...
#define GEN_VLD_U8_ZIP_INDEX svindex_s8(28, -4)
  ...
  for (pos = 0; pos < nb_pkts; pos += HNS3_SVE_DEFAULT_DESCS_PER_LOOP,
    ...
    vld = svld1_gather_u32offset_u32(
        pg32, (uint32_t *)rxdp, svindex_u32(BD_FIELD_VALID_OFFSET, BD_SIZE));
    vld2 = svlsl_n_u32_z(pg32, vld, HNS3_UINT32_BIT - 1 - HNS3_RXD_VLD_B);
    vld2 = svreinterpret_u32_s32(
        svasr_n_s32_z(pg32, svreinterpret_s32_u32(vld2), HNS3_UINT32_BIT - 1));
    ...
    l234 = svld1_gather_u32offset_u32(
        pg32, (uint32_t *)rxdp2, svindex_u32(BD_FIELD_L234_OFFSET, BD_SIZE));
    ...
    xlen = svld1_gather_u32offset_u32(
        pg32, (uint32_t *)rxdp2, svindex_u32(BD_FIELD_XLEN_OFFSET, BD_SIZE));
    ...
    xlen = svreinterpret_u32_u16(
        svsub_n_u16_z(PG16_256BIT, svreinterpret_u16_u32(xlen), rxq->crc_len));
    ...
    rss1st =
        svreinterpret_u64_u32(svtbl_u32(svreinterpret_u32_u32(rss), rss_tbl1));
    rss2st =
        svreinterpret_u64_u32(svtbl_u32(svreinterpret_u32_u32(rss), rss_tbl2));

    /* save mbuf_initializer */
    svst1_scatter_u64base_offset_u64(
        PG64_256BIT, mbp1st, offsetof(struct rte_mbuf, rearm_data), mbuf_init);
    svst1_scatter_u64base_offset_u64(
        PG64_256BIT, mbp2st, offsetof(struct rte_mbuf, rearm_data), mbuf_init);

    /* save datalen and pktlen and rss */
    svst1_scatter_u64base_offset_u64(
        PG64_256BIT, mbp1st, offsetof(struct rte_mbuf, pkt_len), xlen1st);
    svst1_scatter_u64base_offset_u64(
        PG64_256BIT, mbp1st, offsetof(struct rte_mbuf, hash.rss), rss1st);
    svst1_scatter_u64base_offset_u64(
        PG64_256BIT, mbp2st, offsetof(struct rte_mbuf, pkt_len), xlen2st);
    svst1_scatter_u64base_offset_u64(
        PG64_256BIT, mbp2st, offsetof(struct rte_mbuf, hash.rss), rss2st);

    rte_prefetch_non_temporal(rxdp + HNS3_SVE_DEFAULT_DESCS_PER_LOOP);

    parse_retcode =
        hns3_desc_parse_field_sve(rxq, &rx_pkts[pos], &key_field, bd_valid_num);
    if (unlikely(parse_retcode))
      (*bd_err_mask) |= ((uint64_t)parse_retcode) << pos;

    hns3_rx_prefetch_mbuf_sve(&sw_ring[pos + HNS3_SVE_DEFAULT_DESCS_PER_LOOP]);
...
static inline void hns3_rxq_rearm_mbuf_sve(struct hns3_rx_queue *rxq) {
#define REARM_LOOP_STEP_NUM 4
  ...
  for (i = 0; i < HNS3_DEFAULT_RXQ_REARM_THRESH;
       i += REARM_LOOP_STEP_NUM, rxep_tmp += REARM_LOOP_STEP_NUM) {
  ...
  for (i = 0; i < HNS3_DEFAULT_RXQ_REARM_THRESH; i += REARM_LOOP_STEP_NUM,
    ...
    svst1_scatter_u64offset_u64(PG64_256BIT, (uint64_t *)&rxdp[0].addr,
                                svindex_u64(BD_FIELD_ADDR_OFFSET, BD_SIZE),
                                siova);
    svst1_scatter_u64offset_u64(PG64_256BIT, (uint64_t *)&rxdp[0].addr,
                                svindex_u64(BD_FIELD_OL_OFFSET, BD_SIZE),
                                ol_base);
  }
...
uint16_t hns3_recv_pkts_vec_sve(void *__restrict rx_queue,
  ...
  if (unlikely(
          !(rxdp->rx.bd_base_info & rte_cpu_to_le_32(1u << HNS3_RXD_VLD_B))))
    return 0;
  ...
  if (likely(nb_pkts <= HNS3_DEFAULT_RX_BURST)) {
    pkt_err_mask = 0;
    nb_rx = hns3_recv_burst_vec_sve(rxq, rx_pkts, nb_pkts, &pkt_err_mask);
  ...
  while (nb_pkts > 0) {
    ...
    pkt_err_mask = 0;
    ret = hns3_recv_burst_vec_sve(rxq, &rx_pkts[nb_rx], n, &pkt_err_mask);
    nb_pkts -= ret;
    nb_rx += hns3_rx_reassemble_pkts(&rx_pkts[nb_rx], ret, pkt_err_mask);
    if (ret < n)
      break;
...
static inline void hns3_tx_free_buffers_sve(struct hns3_tx_queue *txq) {
...
#define TX_VLD_U8_ZIP_INDEX svindex_u8(0, 4)
  ...
  for (i = 0; i < txq->tx_rs_thresh; i += HNS3_SVE_CHECK_DESCS_PER_LOOP,
    ...
    vld2 = svld1_gather_u32offset_u32(
        pg32, (uint32_t *)tx_desc, svindex_u32(BD_FIELD_VALID_OFFSET, BD_SIZE));
    vld = svorr_u32_z(pg32, vld, vld2);
  }
  /* shift left and then right to get all valid bit */
  vld = svlsl_n_u32_z(pg32, vld, HNS3_UINT32_BIT - 1 - HNS3_TXD_VLD_B);
  vld = svreinterpret_u32_s32(
      svasr_n_s32_z(pg32, svreinterpret_s32_u32(vld), HNS3_UINT32_BIT - 1));
...
static inline void hns3_tx_fill_hw_ring_sve(struct hns3_tx_queue *txq,
...
#define DATA_OFF_LEN_VAL_MASK 0xFFFF
  ...
  do {
    ...
    buf_iova =
        svadd_n_u64_z(pg, base_addr, offsetof(struct rte_mbuf, buf_iova));
    /* calc mbuf's field data_off address */
    data_off =
        svadd_n_u64_z(pg, base_addr, offsetof(struct rte_mbuf, data_off));
    /* calc mbuf's field data_len address */
    data_len =
        svadd_n_u64_z(pg, base_addr, offsetof(struct rte_mbuf, data_len));
    ...
    svst1_scatter_u64offset_u64(pg, (uint64_t *)&txdp->addr, offsets, addr);
    /* save offset 8~15byte of every BD */
    svst1_scatter_u64offset_u64(pg, (uint64_t *)&txdp->tx.vlan_tag, offsets,
                                data_len);
    /* save offset 16~23byte of every BD */
    svst1_scatter_u64offset_u64(pg, (uint64_t *)&txdp->tx.outer_vlan_tag,
                                offsets, svdup_n_u64(0));
    /* save offset 24~31byte of every BD */
    svst1_scatter_u64offset_u64(pg, (uint64_t *)&txdp->tx.paylen_fd_dop_ol4cs,
...
uint16_t hns3_xmit_pkts_vec_sve(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  while (nb_pkts) {
    ...
    ret = hns3_xmit_fixed_burst_vec_sve(tx_queue, &tx_pkts[nb_tx], new_burst);
    nb_tx += ret;
    nb_pkts -= ret;
    if (ret < new_burst)
      break;
