...
static enum rte_flow_item_type first_items[] = {
    ...
    RTE_FLOW_ITEM_TYPE_UDP,      RTE_FLOW_ITEM_TYPE_SCTP,
    RTE_FLOW_ITEM_TYPE_ICMP,     RTE_FLOW_ITEM_TYPE_NVGRE,
    RTE_FLOW_ITEM_TYPE_VXLAN,    RTE_FLOW_ITEM_TYPE_GENEVE,
    RTE_FLOW_ITEM_TYPE_VXLAN_GPE};

static enum rte_flow_item_type L2_next_items[] = {
    RTE_FLOW_ITEM_TYPE_VLAN, RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_IPV6};

static enum rte_flow_item_type L3_next_items[] = {
    RTE_FLOW_ITEM_TYPE_TCP, RTE_FLOW_ITEM_TYPE_UDP, RTE_FLOW_ITEM_TYPE_SCTP,
    RTE_FLOW_ITEM_TYPE_NVGRE, RTE_FLOW_ITEM_TYPE_ICMP};

static enum rte_flow_item_type L4_next_items[] = {RTE_FLOW_ITEM_TYPE_VXLAN,
                                                  RTE_FLOW_ITEM_TYPE_GENEVE,
                                                  RTE_FLOW_ITEM_TYPE_VXLAN_GPE};

static enum rte_flow_item_type tunnel_next_items[] = {RTE_FLOW_ITEM_TYPE_ETH,
...
static inline void net_addr_to_host(uint32_t *dst, const rte_be32_t *src,
                                    size_t len) {
  size_t i;
...
static int hns3_counter_new(struct rte_eth_dev *dev, uint32_t indirect,
  ...
  if (ret)
    return rte_flow_error_set(error, EIO, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
                              "Clear counter failed!");

  cnt = rte_zmalloc("hns3 counter", sizeof(*cnt), 0);
  if (cnt == NULL)
    return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_HANDLE, cnt,
...
static int hns3_counter_query(struct rte_eth_dev *dev, struct rte_flow *flow,
...
static int hns3_counter_release(struct rte_eth_dev *dev, uint32_t id) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  struct hns3_flow_counter *cnt;
...
static void hns3_counter_flush(struct rte_eth_dev *dev) {
...
static int hns3_handle_action_queue(struct rte_eth_dev *dev,
  ...
  if (queue->index >= hw->data->nb_rx_queues) {
    hns3_err(hw,
             "queue ID(%u) is greater than number of "
...
static int hns3_handle_action_queue_region(struct rte_eth_dev *dev,
                                           const struct rte_flow_action *action,
                                           struct hns3_fdir_rule *rule,
                                           struct rte_flow_error *error) {
  struct hns3_adapter *hns = dev->data->dev_private;
  ...
  if (!hns3_dev_get_support(hw, FD_QUEUE_REGION))
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                              action, "Not support config queue region!");
  ...
  if ((!rte_is_power_of_2(conf->queue_num)) ||
    ...
    return rte_flow_error_set(
        error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF, action,
...
static int hns3_handle_action_indirect(struct rte_eth_dev *dev,
                                       const struct rte_flow_action *action,
                                       struct hns3_fdir_rule *rule,
                                       struct rte_flow_error *error) {
 ...
 * @param error[out]
 */
static int hns3_handle_actions(struct rte_eth_dev *dev,
                               const struct rte_flow_action actions[],
                               struct hns3_fdir_rule *rule,
                               struct rte_flow_error *error) {
  struct hns3_adapter *hns = dev->data->dev_private;
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_QUEUE:
      ret = hns3_handle_action_queue(dev, actions, rule, error);
      if (ret)
        return ret;
      break;
    ...
    case RTE_FLOW_ACTION_TYPE_RSS:
      ret = hns3_handle_action_queue_region(dev, actions, rule, error);
      if (ret)
        return ret;
      break;
    case RTE_FLOW_ACTION_TYPE_MARK:
      ...
      if (mark->id >= HNS3_MAX_FILTER_ID)
        return rte_flow_error_set(error, EINVAL,
                                  RTE_FLOW_ERROR_TYPE_ACTION_CONF, actions,
                                  "Invalid Mark ID");
      rule->fd_id = mark->id;
    ...
    case RTE_FLOW_ACTION_TYPE_COUNT:
      act_count = (const struct rte_flow_action_count *)actions->conf;
    ...
    case RTE_FLOW_ACTION_TYPE_INDIRECT:
      ret = hns3_handle_action_indirect(dev, actions, rule, error);
      if (ret)
        return ret;
      break;
...
static int hns3_check_attr(const struct rte_flow_attr *attr,
                           struct rte_flow_error *error) {
  if (!attr->ingress)
...
static int hns3_parse_eth(const struct rte_flow_item *item,
                          struct hns3_fdir_rule *rule,
  ...
  if (item->mask) {
    ...
    if (eth_mask->type) {
      ...
      rule->key_conf.mask.ether_type = rte_be_to_cpu_16(eth_mask->type);
    }
    if (!rte_is_zero_ether_addr(&eth_mask->src)) {
      hns3_set_bit(rule->input_set, INNER_SRC_MAC, 1);
      memcpy(rule->key_conf.mask.src_mac, eth_mask->src.addr_bytes,
             RTE_ETHER_ADDR_LEN);
    }
    if (!rte_is_zero_ether_addr(&eth_mask->dst)) {
      hns3_set_bit(rule->input_set, INNER_DST_MAC, 1);
      memcpy(rule->key_conf.mask.dst_mac, eth_mask->dst.addr_bytes,
             RTE_ETHER_ADDR_LEN);
    }
  }
...
static int hns3_parse_vlan(const struct rte_flow_item *item,
                           struct hns3_fdir_rule *rule,
                           struct rte_flow_error *error) {
  ...
  if (item->mask) {
    ...
    if (vlan_mask->tci) {
      if (rule->key_conf.vlan_num == 1) {
        hns3_set_bit(rule->input_set, INNER_VLAN_TAG1, 1);
        rule->key_conf.mask.vlan_tag1 = rte_be_to_cpu_16(vlan_mask->tci);
      } else {
        hns3_set_bit(rule->input_set, INNER_VLAN_TAG2, 1);
        rule->key_conf.mask.vlan_tag2 = rte_be_to_cpu_16(vlan_mask->tci);
      }
    }
  }
  ...
  vlan_spec = item->spec;
  if (rule->key_conf.vlan_num == 1)
    rule->key_conf.spec.vlan_tag1 = rte_be_to_cpu_16(vlan_spec->tci);
  else
    rule->key_conf.spec.vlan_tag2 = rte_be_to_cpu_16(vlan_spec->tci);
  return 0;
}
...
static int hns3_parse_ipv4(const struct rte_flow_item *item,
                           struct hns3_fdir_rule *rule,
                           struct rte_flow_error *error) {
  ...
  if (item->mask) {
    ...
    if (ipv4_mask->hdr.type_of_service) {
      ...
      rule->key_conf.mask.ip_tos = ipv4_mask->hdr.type_of_service;
    }

    if (ipv4_mask->hdr.next_proto_id) {
      hns3_set_bit(rule->input_set, INNER_IP_PROTO, 1);
      rule->key_conf.mask.ip_proto = ipv4_mask->hdr.next_proto_id;
    }
  }
...
static int hns3_parse_ipv6(const struct rte_flow_item *item,
                           struct hns3_fdir_rule *rule,
                           struct rte_flow_error *error) {
  ...
  if (item->mask) {
    ...
    net_addr_to_host(rule->key_conf.mask.src_ip,
                     (const rte_be32_t *)ipv6_mask->hdr.src_addr, IP_ADDR_LEN);
    net_addr_to_host(rule->key_conf.mask.dst_ip,
                     (const rte_be32_t *)ipv6_mask->hdr.dst_addr, IP_ADDR_LEN);
  ...
  net_addr_to_host(rule->key_conf.spec.src_ip,
                   (const rte_be32_t *)ipv6_spec->hdr.src_addr, IP_ADDR_LEN);
  net_addr_to_host(rule->key_conf.spec.dst_ip,
                   (const rte_be32_t *)ipv6_spec->hdr.dst_addr, IP_ADDR_LEN);
...
hns3_check_tcp_mask_supported(const struct rte_flow_item_tcp *tcp_mask) {
  if (tcp_mask->hdr.sent_seq || tcp_mask->hdr.recv_ack ||
      ...
      tcp_mask->hdr.rx_win || tcp_mask->hdr.cksum || tcp_mask->hdr.tcp_urp)
    return false;

  return true;
}

static int hns3_parse_tcp(const struct rte_flow_item *item,
                          struct hns3_fdir_rule *rule,
                          struct rte_flow_error *error) {
  ...
  if (item->mask) {
    ...
    if (!hns3_check_tcp_mask_supported(tcp_mask)) {
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                                item, "Only support src & dst port in TCP");
    }

    if (tcp_mask->hdr.src_port) {
      hns3_set_bit(rule->input_set, INNER_SRC_PORT, 1);
      rule->key_conf.mask.src_port = rte_be_to_cpu_16(tcp_mask->hdr.src_port);
    }
    if (tcp_mask->hdr.dst_port) {
      hns3_set_bit(rule->input_set, INNER_DST_PORT, 1);
      rule->key_conf.mask.dst_port = rte_be_to_cpu_16(tcp_mask->hdr.dst_port);
    }
  }
...
static int hns3_parse_udp(const struct rte_flow_item *item,
                          struct hns3_fdir_rule *rule,
                          struct rte_flow_error *error) {
  ...
  if (item->mask) {
    ...
    if (udp_mask->hdr.dgram_len || udp_mask->hdr.dgram_cksum) {
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                                item, "Only support src & dst port in UDP");
    }
    if (udp_mask->hdr.src_port) {
      hns3_set_bit(rule->input_set, INNER_SRC_PORT, 1);
      rule->key_conf.mask.src_port = rte_be_to_cpu_16(udp_mask->hdr.src_port);
    }
    if (udp_mask->hdr.dst_port) {
      hns3_set_bit(rule->input_set, INNER_DST_PORT, 1);
      rule->key_conf.mask.dst_port = rte_be_to_cpu_16(udp_mask->hdr.dst_port);
    }
  }
...
static int hns3_parse_sctp(const struct rte_flow_item *item,
                           struct hns3_fdir_rule *rule,
                           struct rte_flow_error *error) {
  ...
  if (item->mask) {
    ...
    if (sctp_mask->hdr.cksum)
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                                item, "Only support src & dst port in SCTP");
    if (sctp_mask->hdr.src_port) {
      hns3_set_bit(rule->input_set, INNER_SRC_PORT, 1);
      rule->key_conf.mask.src_port = rte_be_to_cpu_16(sctp_mask->hdr.src_port);
    }
    if (sctp_mask->hdr.dst_port) {
      hns3_set_bit(rule->input_set, INNER_DST_PORT, 1);
      rule->key_conf.mask.dst_port = rte_be_to_cpu_16(sctp_mask->hdr.dst_port);
    }
    if (sctp_mask->hdr.tag) {
      hns3_set_bit(rule->input_set, INNER_SCTP_TAG, 1);
      rule->key_conf.mask.sctp_tag = rte_be_to_cpu_32(sctp_mask->hdr.tag);
    }
  }

  sctp_spec = item->spec;
  rule->key_conf.spec.src_port = rte_be_to_cpu_16(sctp_spec->hdr.src_port);
  rule->key_conf.spec.dst_port = rte_be_to_cpu_16(sctp_spec->hdr.dst_port);
 ...
 */
static int hns3_handle_tunnel(const struct rte_flow_item *item,
                              struct hns3_fdir_rule *rule,
                              struct rte_flow_error *error) {
  /* check eth config */
  if (rule->input_set & (BIT(INNER_SRC_MAC) | BIT(INNER_DST_MAC)))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                              "Outer eth mac is unsupported");
  if (rule->input_set & BIT(INNER_ETH_TYPE)) {
    hns3_set_bit(rule->input_set, OUTER_ETH_TYPE, 1);
    rule->key_conf.spec.outer_ether_type = rule->key_conf.spec.ether_type;
    rule->key_conf.mask.outer_ether_type = rule->key_conf.mask.ether_type;
  ...
  if (rule->input_set & BIT(INNER_SRC_PORT)) {
    ...
    rule->key_conf.spec.outer_src_port = rule->key_conf.spec.src_port;
    rule->key_conf.mask.outer_src_port = rule->key_conf.mask.src_port;
...
static int hns3_parse_vxlan(const struct rte_flow_item *item,
                            struct hns3_fdir_rule *rule,
                            struct rte_flow_error *error) {
  ...
  if (vxlan_mask->flags)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                              item, "Flags is not supported in VxLAN");
  ...
  if (vxlan_mask->vni[0]) {
    ...
    memcpy(rule->key_conf.mask.outer_tun_vni, vxlan_mask->vni, VNI_OR_TNI_LEN);
  }
  memcpy(rule->key_conf.spec.outer_tun_vni, vxlan_spec->vni, VNI_OR_TNI_LEN);
  return 0;
}

static int hns3_parse_nvgre(const struct rte_flow_item *item,
                            struct hns3_fdir_rule *rule,
                            struct rte_flow_error *error) {
  ...
  if (nvgre_mask->protocol || nvgre_mask->c_k_s_rsvd0_ver)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                              item, "Ver/protocol is not supported in NVGRE");
  ...
  if (nvgre_mask->tni[0]) {
    hns3_set_bit(rule->input_set, OUTER_TUN_VNI, 1);
    memcpy(rule->key_conf.mask.outer_tun_vni, nvgre_mask->tni, VNI_OR_TNI_LEN);
  }
  memcpy(rule->key_conf.spec.outer_tun_vni, nvgre_spec->tni, VNI_OR_TNI_LEN);
...
static int hns3_parse_geneve(const struct rte_flow_item *item,
                             struct hns3_fdir_rule *rule,
                             struct rte_flow_error *error) {
  ...
  if (geneve_mask->ver_opt_len_o_c_rsvd0 || geneve_mask->protocol)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                              item, "Ver/protocol is not supported in GENEVE");
  /* VNI must be totally masked or not. */
  ...
  if (geneve_mask->vni[0]) {
    hns3_set_bit(rule->input_set, OUTER_TUN_VNI, 1);
    memcpy(rule->key_conf.mask.outer_tun_vni, geneve_mask->vni, VNI_OR_TNI_LEN);
  }
  memcpy(rule->key_conf.spec.outer_tun_vni, geneve_spec->vni, VNI_OR_TNI_LEN);
  return 0;
}

static int hns3_parse_tunnel(const struct rte_flow_item *item,
                             struct hns3_fdir_rule *rule,
                             struct rte_flow_error *error) {
  int ret;
...
static int hns3_parse_normal(const struct rte_flow_item *item,
                             struct hns3_fdir_rule *rule,
...
static int hns3_validate_item(const struct rte_flow_item *item,
                              struct items_step_mngr step_mngr,
                              struct rte_flow_error *error) {
  int i;
  ...
  if (i == step_mngr.count) {
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                              "Inval or missing item");
  }
  return 0;
}

static inline bool is_tunnel_packet(enum rte_flow_item_type type) {
  if (type == RTE_FLOW_ITEM_TYPE_VXLAN_GPE ||
      type == RTE_FLOW_ITEM_TYPE_VXLAN || type == RTE_FLOW_ITEM_TYPE_NVGRE ||
 ...
 */
static int hns3_parse_fdir_filter(struct rte_eth_dev *dev,
...
static void hns3_filterlist_flush(struct rte_eth_dev *dev) {
...
static bool hns3_action_rss_same(const struct rte_flow_action_rss *comp,
  ...
  return (func_is_same && rss_key_is_same &&
          ...
          comp->level == with->level && comp->queue_num == with->queue_num &&
          !memcmp(comp->queue, with->queue,
                  sizeof(*with->queue) * with->queue_num));
}

static int hns3_rss_conf_copy(struct hns3_rss_conf *out,
 ...
 */
static int hns3_parse_rss_filter(struct rte_eth_dev *dev,
                                 const struct rte_flow_action *actions,
                                 struct rte_flow_error *error) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  ...
  for (n = 0; n < rss->queue_num; n++) {
    if (rss->queue[n] < hw->alloc_rss_size)
      continue;
    return rte_flow_error_set(
        error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF, act,
  ...
  if (rss->func >= RTE_ETH_HASH_FUNCTION_MAX)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              act, "RSS hash func are not supported");
  if (rss->level)
    return rte_flow_error_set(
        error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF, act,
  ...
  if (!hns3_rss_input_tuple_supported(hw, rss))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &rss->types, "input RSS types are not supported");
  ...
  if (act->type != RTE_FLOW_ACTION_TYPE_END) {
    ...
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, act,
...
static int hns3_disable_rss(struct hns3_hw *hw) {
  int ret;
...
static void hns3_adjust_rss_key(struct hns3_hw *hw,
...
static int hns3_parse_rss_algorithm(struct hns3_hw *hw,
  ...
  case RTE_ETH_HASH_FUNCTION_SYMMETRIC_TOEPLITZ:
    *hash_algo = HNS3_RSS_HASH_ALGO_SYMMETRIC_TOEP;
    break;
  default:
    hns3_err(hw, "Invalid RSS algorithm configuration(%d)", algo_func);
    return -EINVAL;
  }
  *func = algo_func;
...
static int hns3_hw_rss_hash_set(struct hns3_hw *hw,
                                struct rte_flow_action_rss *rss_config) {
  ...
  ret =
      hns3_parse_rss_algorithm(hw, &rss_config->func, &hw->rss_info.hash_algo);
  if (ret)
    return ret;
...
static int hns3_update_indir_table(struct rte_eth_dev *dev,
                                   const struct rte_flow_action_rss *conf,
                                   uint16_t num) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
...
static int hns3_config_rss_filter(struct rte_eth_dev *dev,
  ...
  struct rte_flow_action_rss rss_flow_conf = {
      ...
      .key = conf->conf.key_len ? (void *)(uintptr_t)conf->conf.key : NULL,
      .queue = conf->conf.queue,
  };

  /* Filter the unsupported flow types */
  flow_types = conf->conf.types ? rss_flow_conf.types & HNS3_ETH_RSS_SUPPORT
                                : hw->rss_info.conf.types;
  if (flow_types != rss_flow_conf.types)
    hns3_warn(hw,
              "modified RSS types based on hardware support, "
...
static int hns3_clear_rss_filter(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_rss_conf_ele *rss_filter_ptr;
  struct hns3_hw *hw = &hns->hw;
  ...
  while (rss_filter_ptr) {
    TAILQ_REMOVE(&hw->flow_rss_list, rss_filter_ptr, entries);
    ret = hns3_config_rss_filter(dev, &rss_filter_ptr->filter_info, false);
    if (ret)
      rss_rule_fail_cnt++;
    else
      rss_rule_succ_cnt++;
  ...
  if (rss_rule_fail_cnt) {
    hns3_err(hw,
             "fail to delete all RSS filters, success num = %d "
             "fail num = %d",
...
static int hns3_flow_parse_rss(struct rte_eth_dev *dev,
                               const struct hns3_rss_conf *conf, bool add) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  bool ret;
...
static int hns3_flow_args_check(const struct rte_flow_attr *attr,
                                const struct rte_flow_item pattern[],
                                const struct rte_flow_action actions[],
                                struct rte_flow_error *error) {
  if (pattern == NULL)
 ...
 */
static int hns3_flow_validate(struct rte_eth_dev *dev,
...
static int hns3_flow_create_rss_rule(struct rte_eth_dev *dev,
                                     const struct rte_flow_action *act,
  ...
  rss_filter_ptr =
      rte_zmalloc("hns3 rss filter", sizeof(struct hns3_rss_conf_ele), 0);
...
static int hns3_flow_create_fdir_rule(struct rte_eth_dev *dev,
                                      const struct rte_flow_item pattern[],
                                      const struct rte_flow_action actions[],
                                      struct rte_flow_error *error,
                                      struct rte_flow *flow) {
  ...
  if (fdir_rule.flags & HNS3_RULE_FLAG_COUNTER) {
    ret = hns3_counter_new(dev, indir, fdir_rule.act_cnt.id, error);
    if (ret != 0)
      return ret;
  ...
  fdir_rule_ptr =
      rte_zmalloc("hns3 fdir rule", sizeof(struct hns3_fdir_rule_ele), 0);
  ...
  memcpy(&fdir_rule_ptr->fdir_conf, &fdir_rule, sizeof(struct hns3_fdir_rule));
 ...
 */
static struct rte_flow *hns3_flow_create(struct rte_eth_dev *dev,
                                         const struct rte_flow_attr *attr,
                                         const struct rte_flow_item pattern[],
                                         const struct rte_flow_action actions[],
                                         struct rte_flow_error *error) {
  ...
  flow_node = rte_zmalloc("hns3 flow node", sizeof(struct hns3_flow_mem), 0);
  if (flow_node == NULL) {
    rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
  ...
  if (act)
    ...
    ret = hns3_flow_create_fdir_rule(dev, pattern, actions, error, flow);
  if (ret == 0)
    return flow;
...
static int hns3_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                             struct rte_flow_error *error) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_fdir_rule_ele *fdir_rule_ptr;
  struct hns3_rss_conf_ele *rss_filter_ptr;
  ...
  case RTE_ETH_FILTER_FDIR:
    ...
    if (ret)
      return rte_flow_error_set(error, EIO, RTE_FLOW_ERROR_TYPE_HANDLE, flow,
...
static int hns3_flow_flush(struct rte_eth_dev *dev,
                           struct rte_flow_error *error) {
  struct hns3_adapter *hns = dev->data->dev_private;
  int ret;
...
static int hns3_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
...
static int hns3_flow_validate_wrap(struct rte_eth_dev *dev,
                                   const struct rte_flow_attr *attr,
                                   const struct rte_flow_item pattern[],
                                   const struct rte_flow_action actions[],
                                   struct rte_flow_error *error) {
...
static int hns3_flow_destroy_wrap(struct rte_eth_dev *dev,
                                  struct rte_flow *flow,
                                  struct rte_flow_error *error) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  int ret;
...
static int hns3_flow_flush_wrap(struct rte_eth_dev *dev,
                                struct rte_flow_error *error) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  int ret;
...
static int hns3_flow_query_wrap(struct rte_eth_dev *dev, struct rte_flow *flow,
                                const struct rte_flow_action *actions,
...
hns3_check_indir_action(const struct rte_flow_indir_action_conf *conf,
  ...
  if (!conf->ingress)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                              "Indir action ingress can't be zero");

  if (conf->egress)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                              "Indir action not support egress");

  if (conf->transfer)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                              "Indir action not support transfer");

  if (action->type != RTE_FLOW_ACTION_TYPE_COUNT)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                              "Indir action only support count");

  return 0;
}

static struct rte_flow_action_handle *hns3_flow_action_create(
  ...
  if (handle == NULL) {
    rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
                       "Failed to allocate action memory");
    return NULL;
  }
  ...
  if (act_count->id >= pf->fdir.fd_cfg.cnt_num[HNS3_FD_STAGE_1]) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF, action,
                       "Invalid counter id");
    goto err_exit;
  }
...
static int hns3_flow_action_destroy(struct rte_eth_dev *dev,
  ...
  if (counter->ref_cnt > 1) {
    pthread_mutex_unlock(&hw->flows_lock);
    return rte_flow_error_set(error, EBUSY, RTE_FLOW_ERROR_TYPE_HANDLE, handle,
                              "Counter id in use");
  }
...
static int hns3_flow_action_query(struct rte_eth_dev *dev,
  ...
  ret = hns3_counter_query(dev, &flow, (struct rte_flow_query_count *)data,
                           error);
  pthread_mutex_unlock(&hw->flows_lock);
  return ret;
}
