...
static const char *get_adapter_state_name(uint32_t state) {
  static const char *const state_name[] = {
      "UNINITIALIZED", "INITIALIZED", "CONFIGURING", "CONFIGURED",
      "STARTING",      "STARTED",     "STOPPING",    "CLOSING",
      "CLOSED",        "REMOVED",     "NSTATES"};
...
static const char *get_io_func_hint_name(uint32_t hint) {
  switch (hint) {
...
static void get_dev_mac_info(FILE *file, struct hns3_adapter *hns) {
  ...
  fprintf(file,
          ...
          hw->mac.query_type, hw->mac.supported_speed, hw->mac.advertising,
          hw->mac.lp_advertising, hw->mac.support_autoneg != 0 ? "Yes" : "No",
          pf->support_fc_autoneg ? "Yes" : "No");
}

static void get_dev_feature_capability(FILE *file, struct hns3_hw *hw) {
...
static const char *get_fdir_tuple_name(uint32_t index) {
  static const char *const tuple_name[] = {
      "outer_dst_mac",      "outer_src_mac",   "outer_vlan_1st_tag",
      "outer_vlan_2nd_tag", "outer_eth_type",  "outer_l2_rsv",
      "outer_ip_tos",       "outer_ip_proto",  "outer_src_ip",
      "outer_dst_ip",       "outer_l3_rsv",    "outer_src_port",
      "outer_dst_port",     "outer_l4_rsv",    "outer_tun_vni",
      "outer_tun_flow_id",  "inner_dst_mac",   "inner_src_mac",
      "inner_vlan_tag1",    "inner_vlan_tag2", "inner_eth_type",
      "inner_l2_rsv",       "inner_ip_tos",    "inner_ip_proto",
      "inner_src_ip",       "inner_dst_ip",    "inner_l3_rsv",
      "inner_src_port",     "inner_dst_port",  "inner_sctp_tag",
  };
...
static void get_fdir_basic_info(FILE *file, struct hns3_pf *pf) {
...
#define PERLINE_TUPLE_NAMES 4
  ...
  fprintf(file,
          ...
          fdcfg->rule_num[HNS3_FD_STAGE_1], fdcfg->cnt_num[HNS3_FD_STAGE_1],
...
static void get_device_basic_info(FILE *file, struct rte_eth_dev *dev) {
  ...
  fprintf(file,
          ...
          dev->data->name, get_adapter_state_name(hw->adapter_state),
          dev->data->nb_rx_queues, dev->data->nb_tx_queues, hw->total_tqps_num,
          hw->tqps_num, hw->intr_tqps_num, hw->rss_size_max, hw->alloc_rss_size,
          hw->tx_qnum_per_tc, hw->min_tx_pkt_len, hw->intr.mapping_mode,
          hw->vlan_mode, hw->tso_mode, hw->max_non_tso_bd_num, hw->max_tm_rate,
          hw->set_link_down ? "Yes" : "No",
          get_io_func_hint_name(hns->rx_func_hint),
          get_io_func_hint_name(hns->tx_func_hint),
          !!(dev->data->dev_flags & RTE_ETH_DEV_INTR_LSC),
          dev->data->dev_conf.intr_conf.lsc, dev->data->dev_conf.intr_conf.rxq);
}
 ...
 */
static struct hns3_rx_queue *get_rx_queue(struct rte_eth_dev *dev,
 ...
 *                           eth_dev->data->nb_tx_queues)
 */
static struct hns3_tx_queue *get_tx_queue(struct rte_eth_dev *dev,
                                          unsigned int queue_id) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  unsigned int offset;
...
static void get_rxtx_fake_queue_info(FILE *file, struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  ...
  if (dev->data->nb_rx_queues != dev->data->nb_tx_queues &&
    ...
    queue_id = RTE_MIN(dev->data->nb_rx_queues, dev->data->nb_tx_queues);
    ...
    fprintf(file,
            ...
            "\t       tx: port=%u nb_desc=%u\n",
            rxq->port_id, rxq->nb_rx_desc, rxq->rx_free_thresh, txq->port_id,
            txq->nb_tx_desc);
  }
}

static void get_queue_enable_state(struct hns3_hw *hw, uint32_t *queue_state,
...
#define STATE_SIZE (sizeof(*queue_state) * CHAR_BIT)
  ...
  for (i = 0; i < nb_queues; i++) {
    ...
    if (hns3_dev_get_support(hw, INDEP_TXRX))
      state = state && hns3_read_dev(hw, reg_offset + queue_en_reg);
    hns3_set_bit(queue_state[i / STATE_SIZE], i % STATE_SIZE, state);
  }
}

static void print_queue_state_perline(FILE *file, const uint32_t *queue_state,
...
#define NUM_QUEUE_PER_LINE (sizeof(*queue_state) * CHAR_BIT)
  ...
  for (j = 0; j < NUM_QUEUE_PER_LINE; j++) {
    ...
    if (qid % CHAR_BIT == CHAR_BIT - 1) {
      fprintf(file, "%s", j == NUM_QUEUE_PER_LINE - 1 ? "\n" : ":");
    }
    qid++;
    if (qid >= nb_queues) {
      fprintf(file, "\n");
...
static void display_queue_enable_state(FILE *file, const uint32_t *queue_state,
                                       uint32_t nb_queues, bool is_rxq) {
#define NUM_QUEUE_PER_LINE (sizeof(*queue_state) * CHAR_BIT)
  uint32_t i;
...
static void get_rxtx_queue_enable_state(FILE *file, struct rte_eth_dev *dev) {
...
#define QUEUE_BITMAP_SIZE (MAX_TQP_NUM / 32)
  ...
  display_queue_enable_state(file, rx_queue_state, nb_rx_queues, true);

  get_queue_enable_state(hw, tx_queue_state, nb_tx_queues, false);
  display_queue_enable_state(file, tx_queue_state, nb_tx_queues, false);
}

static void get_rxtx_queue_info(FILE *file, struct rte_eth_dev *dev) {
  struct hns3_rx_queue *rxq;
  struct hns3_tx_queue *txq;
  ...
  fprintf(file,
          ...
          "\t       tx: port=%u nb_desc=%u\n"
          "\t  -- tx push: %s\n",
          dev->data->nb_rx_queues, dev->data->nb_tx_queues, rxq->port_id,
          rxq->nb_rx_desc, rxq->rx_free_thresh, txq->port_id, txq->nb_tx_desc,
...
static int get_vlan_filter_cfg(FILE *file, struct hns3_hw *hw) {
...
#define HNS3_FILTER_FE_NIC_EGRESS_B BIT(1)
  ...
  static const uint32_t vlan_filter_type[] = {HNS3_FILTER_TYPE_PORT,
                                              HNS3_FILTER_TYPE_VF};

  for (i = 0; i < RTE_DIM(vlan_filter_type); i++) {
    hns3_cmd_setup_basic_desc(&desc, HNS3_OPC_VLAN_FILTER_CTRL, true);
    req = (struct hns3_vlan_filter_ctrl_cmd *)desc.data;
    req->vlan_type = vlan_filter_type[i];
    req->vf_id = HNS3_PF_FUNC_ID;
    ret = hns3_cmd_send(hw, &desc, 1);
    if (ret != 0) {
      hns3_err(hw, "NIC IMP exec ret=%d desc_num=%d optcode=0x%x!", ret, 1,
    ...
    fprintf(file,
            ...
            "\t       nic_ingress           :%s\n"
            "\t       nic_egress            :%s\n",
            req->vlan_type == HNS3_FILTER_TYPE_PORT ? "Port" : "VF",
            req->vlan_fe & HNS3_FILTER_FE_NIC_INGRESS_B ? "Enable" : "Disable",
            req->vlan_fe & HNS3_FILTER_FE_NIC_EGRESS_B ? "Enable" : "Disable");
  }

  return 0;
}

static int get_vlan_rx_offload_cfg(FILE *file, struct hns3_hw *hw) {
  ...
  fprintf(file,
          ...
          hns3_get_bit(req->vport_vlan_cfg, HNS3_REM_TAG1_EN_B) ? "Enable"
                                                                : "Disable",
          hns3_get_bit(req->vport_vlan_cfg, HNS3_REM_TAG2_EN_B) ? "Enable"
                                                                : "Disable",
          hns3_get_bit(req->vport_vlan_cfg, HNS3_SHOW_TAG1_EN_B) ? "Enable"
                                                                 : "Disable",
          hns3_get_bit(req->vport_vlan_cfg, HNS3_SHOW_TAG2_EN_B) ? "Enable"
                                                                 : "Disable",
          hns3_get_bit(req->vport_vlan_cfg, HNS3_DISCARD_TAG1_EN_B) ? "Enable"
                                                                    : "Disable",
          hns3_get_bit(req->vport_vlan_cfg, HNS3_DISCARD_TAG2_EN_B)
              ? "Enable"
              : "Disable");
...
static void parse_tx_vlan_cfg(FILE *file,
...
#define VLAN_PRIO_SHIFT 13
  ...
  fprintf(
      ...
      "\t       vlan_shift_mode       :%s\n",
      hns3_get_bit(req->vport_vlan_cfg, HNS3_ACCEPT_TAG1_B) ? "Enable"
                                                            : "Disable",
      hns3_get_bit(req->vport_vlan_cfg, HNS3_ACCEPT_UNTAG1_B) ? "Enable"
                                                              : "Disable",
      hns3_get_bit(req->vport_vlan_cfg, HNS3_PORT_INS_TAG1_EN_B) ? "Enable"
                                                                 : "Disable",
      req->def_vlan_tag1 & VLAN_VID_MASK, req->def_vlan_tag1 >> VLAN_PRIO_SHIFT,
      hns3_get_bit(req->vport_vlan_cfg, HNS3_ACCEPT_TAG2_B) ? "Enable"
                                                            : "Disable",
      hns3_get_bit(req->vport_vlan_cfg, HNS3_ACCEPT_UNTAG2_B) ? "Enable"
                                                              : "Disable",
      hns3_get_bit(req->vport_vlan_cfg, HNS3_PORT_INS_TAG2_EN_B) ? "Enable"
                                                                 : "Disable",
      req->def_vlan_tag2 & VLAN_VID_MASK, req->def_vlan_tag2 >> VLAN_PRIO_SHIFT,
      hns3_get_bit(req->vport_vlan_cfg, HNS3_TAG_SHIFT_MODE_EN_B) ? "Enable"
                                                                  : "Disable");
}

static int get_vlan_tx_offload_cfg(FILE *file, struct hns3_hw *hw) {
  struct hns3_vport_vtag_tx_cfg_cmd *req;
  struct hns3_cmd_desc desc;
  uint16_t vport_id;
  uint8_t bitmap;
...
static void get_port_pvid_info(FILE *file, struct hns3_hw *hw) {
  fprintf(file, "\t  -- pvid status: %s\n",
          hw->port_base_vlan_cfg.state ? "on" : "off");
}

static void get_vlan_config_info(FILE *file, struct hns3_hw *hw) {
  int ret;
...
static void get_tm_conf_shaper_info(FILE *file, struct hns3_tm_conf *conf) {
  ...
  TAILQ_FOREACH(shaper_profile, shaper_profile_list, node) {
    fprintf(file, "    id=%u reference_count=%u peak_rate=%" PRIu64 "Bps\n",
            shaper_profile->shaper_profile_id, shaper_profile->reference_count,
            shaper_profile->profile.peak.rate);
  }
}

static void get_tm_conf_port_node_info(FILE *file, struct hns3_tm_conf *conf) {
  if (!conf->root)
    return;
  ...
  fprintf(file,
          ...
          conf->root->shaper_profile
              ? (int)conf->root->shaper_profile->shaper_profile_id
              : -1);
}

static void get_tm_conf_tc_node_info(FILE *file, struct hns3_tm_conf *conf) {
  ...
  for (tidx = 0; tidx < HNS3_MAX_TC_NUM; tidx++) {
    ...
    fprintf(file,
            ...
            tm_node->shaper_profile
                ? (int)tm_node->shaper_profile->shaper_profile_id
                : -1);
  }
}

static void get_tm_conf_queue_format_info(FILE *file,
                                          struct hns3_tm_node **queue_node,
  ...
  for (i = 0; i < line_num; i++) {
    ...
    for (j = start_queue; j < nb_tx_queues; j++) {
      ...
      if (j > start_queue && j % PERLINE_STRIDE == 0)
        fprintf(file, ":");
      fprintf(file, "%u", queue_node[j] ? queue_node_tc[j] : HNS3_MAX_TC_NUM);
    }
    fprintf(file, "%s\n", tmpbuf);
  }
}

static void get_tm_conf_queue_node_info(FILE *file, struct hns3_tm_conf *conf,
                                        uint32_t nb_tx_queues) {
  ...
  TAILQ_FOREACH(tm_node, queue_list, node) {
    ...
    queue_node_tc[tm_node->id] =
        tm_node->parent ? hns3_tm_calc_node_tc_no(conf, tm_node->parent->id)
                        : 0;
    nb_tx_queues = RTE_MAX(nb_tx_queues, tm_node->id + 1);
  }

  get_tm_conf_queue_format_info(file, queue_node, queue_node_tc, nb_tx_queues);
}

static void get_tm_conf_info(FILE *file, struct rte_eth_dev *dev) {
  ...
  fprintf(file,
          ...
          "\t  -- committed=%u\n",
          conf->nb_leaf_nodes_max, conf->nb_nodes_max, conf->nb_shaper_profile,
          conf->nb_tc_node, conf->nb_queue_node, conf->committed);
...
static void hns3_fc_mode_to_rxtx_pause(enum hns3_fc_mode fc_mode,
...
static bool is_link_fc_mode(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_pf *pf = &hns->pf;
...
static void get_link_fc_info(FILE *file, struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  ...
  hns3_fc_mode_to_rxtx_pause(hw->requested_fc_mode, &rx_pause1, &tx_pause1);
  hns3_fc_mode_to_rxtx_pause((enum hns3_fc_mode)fc_conf.mode, &rx_pause2,
                             &tx_pause2);
...
static void get_flow_ctrl_info(FILE *file, struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  ...
  fprintf(file,
          ...
          "\t       current_fc_status=%u\n"
          "\t       requested_fc_mode=%u\n",
          hw->current_fc_status, hw->requested_fc_mode);
