...
static const struct errno_respcode_map err_code_map[] = {
    {0, 0},        {1, -EPERM},       {2, -ENOENT}, {5, -EIO},
    {11, -EAGAIN}, {12, -ENOMEM},     {16, -EBUSY}, {22, -EINVAL},
    {28, -ENOSPC}, {95, -EOPNOTSUPP},
};

static int hns3_resp_to_errno(uint16_t resp_code) {
  uint32_t i, num;
...
static void hns3_mbx_proc_timeout(struct hns3_hw *hw, uint16_t code,
                                  uint16_t subcode) {
...
static int hns3_get_mbx_resp(struct hns3_hw *hw, uint16_t code,
                             uint16_t subcode, uint8_t *resp_data,
                             uint16_t resp_len) {
...
#define US_PER_MS 1000
  ...
  if (resp_len > HNS3_MBX_MAX_RESP_DATA_SIZE) {
    hns3_err(hw, "VF mbx response len(=%u) exceeds maximum(=%d)", resp_len,
  ...
  while (wait_time < mbx_time_limit) {
    ...
    if (hw->mbx_resp.matching_scheme ==
        HNS3_MBX_RESP_MATCHING_SCHEME_OF_ORIGINAL)
      received = (hw->mbx_resp.head == hw->mbx_resp.tail + hw->mbx_resp.lost);
    else
...
static void hns3_mbx_prepare_resp(struct hns3_hw *hw, uint16_t code,
                                  uint16_t subcode) {
  /*
...
int hns3_send_mbx_msg(struct hns3_hw *hw, uint16_t code, uint16_t subcode,
  ...
  if (msg_len > (HNS3_MBX_MAX_MSG_SIZE - HNS3_CMD_CODE_OFFSET)) {
    hns3_err(hw, "VF send mbx msg fail, msg len %u exceeds max payload len %d",
  ...
  if (need_resp) {
    ...
    if (ret) {
      ...
      hns3_err(hw, "VF failed(=%d) to send mbx message to PF", ret);
      return ret;
    }
...
static bool hns3_cmd_crq_empty(struct hns3_hw *hw) {
  uint32_t tail = hns3_read_dev(hw, HNS3_CMDQ_RX_TAIL_REG);

  return tail == hw->cmq.crq.next_to_use;
}

static void hns3vf_handle_link_change_event(struct hns3_hw *hw,
...
static void hns3_handle_asserting_reset(struct hns3_hw *hw,
                                        struct hns3_mbx_pf_to_vf_cmd *req) {
 ...
 */
static void hns3_update_resp_position(struct hns3_hw *hw, uint32_t resp_msg) {
  ...
  if (resp->req_msg_data != resp_msg) {
    if (resp->lost)
      resp->lost--;
    hns3_warn(hw,
              "Received a mismatched response req_msg(%x) "
              "resp_msg(%x) head(%u) tail(%u) lost(%u)",
              resp->req_msg_data, resp_msg, resp->head, tail, resp->lost);
  } else if (tail + resp->lost > resp->head) {
    resp->lost--;
    hns3_warn(hw,
              "Received a new response again resp_msg(%x) "
              "head(%u) tail(%u) lost(%u)",
              resp_msg, resp->head, tail, resp->lost);
  }
  rte_io_wmb();
  resp->tail = tail;
}

static void hns3_handle_mbx_response(struct hns3_hw *hw,
                                     struct hns3_mbx_pf_to_vf_cmd *req) {
  struct hns3_mbx_resp_status *resp = &hw->mbx_resp;
  uint32_t msg_data;
  ...
  if (req->match_id != 0) {
    ...
    if (req->match_id == resp->match_id) {
      ...
      memcpy(resp->additional_info, &req->msg[4], HNS3_MBX_MAX_RESP_DATA_SIZE);
      rte_io_wmb();
...
static void hns3_link_fail_parse(struct hns3_hw *hw, uint8_t link_fail_code) {
...
static void hns3pf_handle_link_change_event(struct hns3_hw *hw,
                                            struct hns3_mbx_vf_to_pf_cmd *req) {
...
static void hns3_handle_promisc_info(struct hns3_hw *hw, uint16_t promisc_en) {
  if (!promisc_en) {
    /*
...
static void hns3_handle_mbx_msg_out_intr(struct hns3_hw *hw) {
...
void hns3_dev_handle_mbx_msg(struct hns3_hw *hw) {
  ...
  handle_out =
      (rte_eal_process_type() != RTE_PROC_PRIMARY || !rte_thread_is_intr()) &&
      hns->is_vf;
  if (handle_out) {
    /*
  ...
  while (!hns3_cmd_crq_empty(hw)) {
    ...
    if (unlikely(!hns3_get_bit(flag, HNS3_CMDQ_RX_OUTVLD_B))) {
      hns3_warn(hw, "dropped invalid mailbox message, code = %u", opcode);
    ...
    case HNS3_MBX_PUSH_LINK_STATUS:
       ...
       */
      hns3pf_handle_link_change_event(hw, (struct hns3_mbx_vf_to_pf_cmd *)req);
      break;
    ...
    case HNS3_MBX_PUSH_PROMISC_INFO:
      ...
      break;
    default:
      hns3_err(hw, "received unsupported(%u) mbx msg", opcode);
      break;
    }
