...
#define HNS3_DEFAULT_UMV_SPACE_PER_PF (HNS3_UMV_TBL_SIZE / HNS3_MAX_PF_NUM)

#define HNS3_PF_CFG_BLOCK_SIZE 32
#define HNS3_PF_CFG_DESC_NUM (HNS3_PF_CFG_BLOCK_SIZE / HNS3_CFG_RD_LEN_BYTES)
...
struct hns3_hw_ops {
  int (*add_mc_mac_addr)(struct hns3_hw *hw, struct rte_ether_addr *mac_addr);
  int (*del_mc_mac_addr)(struct hns3_hw *hw, struct rte_ether_addr *mac_addr);
  int (*add_uc_mac_addr)(struct hns3_hw *hw, struct rte_ether_addr *mac_addr);
  int (*del_uc_mac_addr)(struct hns3_hw *hw, struct rte_ether_addr *mac_addr);
  int (*bind_ring_with_vector)(struct hns3_hw *hw, uint16_t vector_id, bool en,
...
#define HNS3_DEV_PRIVATE_TO_HW(adapter) (&((struct hns3_adapter *)adapter)->hw)
#define HNS3_DEV_PRIVATE_TO_PF(adapter) (&((struct hns3_adapter *)adapter)->pf)
#define HNS3_DEV_PRIVATE_TO_VF(adapter) (&((struct hns3_adapter *)adapter)->vf)
#define HNS3_DEV_HW_TO_ADAPTER(hw) container_of(hw, struct hns3_adapter, hw)
...
#define hns3_get_field(origin, mask, shift) (((origin) & (mask)) >> (shift))
...
static inline void hns3_write_reg(void *base, uint32_t reg, uint32_t value) {
  rte_write32(rte_cpu_to_le_32(value), (volatile void *)((char *)base + reg));
}
...
#define hns3_read_dev(a, reg) hns3_read_reg((a)->io_base, (reg))
...
static inline uint64_t hns3_atomic_test_bit(unsigned int nr,
...
static inline void hns3_atomic_set_bit(unsigned int nr,
                                       volatile uint64_t *addr) {
  __atomic_fetch_or(addr, (1UL << nr), __ATOMIC_RELAXED);
}

static inline void hns3_atomic_clear_bit(unsigned int nr,
                                         volatile uint64_t *addr) {
  __atomic_fetch_and(addr, ~(1UL << nr), __ATOMIC_RELAXED);
}

static inline int64_t hns3_test_and_clear_bit(unsigned int nr,
                                              volatile uint64_t *addr) {
...
static inline bool is_reset_pending(struct hns3_adapter *hns) {
  bool ret;
  if (hns->is_vf)
