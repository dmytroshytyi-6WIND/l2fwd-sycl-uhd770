 ...
 */
static int hns3_shaper_para_calc(struct hns3_hw *hw, uint32_t ir,
                                 uint8_t shaper_level,
...
#define DIVISOR_IR_B_126 (126 * DIVISOR_CLK)
  ...
  if (shaper_level >= HNS3_SHAPER_LVL_CNT) {
    hns3_err(hw, "shaper_level(%u) is greater than HNS3_SHAPER_LVL_CNT(%d)",
  ...
  if (ir > hw->max_tm_rate) {
    hns3_err(hw,
             "rate(%u) exceeds the max rate(%u) driver "
             "supported.",
             ir, hw->max_tm_rate);
    return -EINVAL;
  }
  ...
  if (ir_calc == ir) {
    ...
    shaper_para->ir_b =
        (ir * tick * (1 << ir_s_calc) + (DIVISOR_CLK >> 1)) / DIVISOR_CLK;
  } else {
    /*
    ...
    if (ir_calc == ir) {
      ...
      shaper_para->ir_b = (ir * tick + (denominator >> 1)) / denominator;
    }
  }
...
static int hns3_fill_pri_array(struct hns3_hw *hw, uint8_t *pri,
                               uint8_t pri_id) {
...
static int hns3_up_to_tc_map(struct hns3_hw *hw) {
...
static int hns3_pg_to_pri_map_cfg(struct hns3_hw *hw, uint8_t pg_id,
                                  uint8_t pri_bit_map) {
...
static int hns3_pg_to_pri_map(struct hns3_hw *hw) {
...
static int hns3_qs_to_pri_map_cfg(struct hns3_hw *hw, uint16_t qs_id,
                                  uint8_t pri) {
...
static int hns3_dcb_qs_weight_cfg(struct hns3_hw *hw, uint16_t qs_id,
                                  uint8_t dwrr) {
...
static int hns3_dcb_ets_tc_dwrr_cfg(struct hns3_hw *hw) {
...
static int hns3_dcb_pri_weight_cfg(struct hns3_hw *hw, uint8_t pri_id,
                                   uint8_t dwrr) {
...
static int hns3_dcb_pg_weight_cfg(struct hns3_hw *hw, uint8_t pg_id,
                                  uint8_t dwrr) {
...
static int hns3_dcb_pg_schd_mode_cfg(struct hns3_hw *hw, uint8_t pg_id) {
  struct hns3_cmd_desc desc;
...
static uint32_t hns3_dcb_get_shapping_para(uint8_t ir_b, uint8_t ir_u,
                                           uint8_t ir_s, uint8_t bs_b,
                                           uint8_t bs_s) {
...
static int hns3_dcb_port_shaper_cfg(struct hns3_hw *hw, uint32_t speed) {
  ...
  ret =
      hns3_shaper_para_calc(hw, speed, HNS3_SHAPER_LVL_PORT, &shaper_parameter);
  if (ret) {
  ...
  shapping_para = hns3_dcb_get_shapping_para(
      ir_b, ir_u, ir_s, HNS3_SHAPER_BS_U_DEF, HNS3_SHAPER_BS_S_DEF);
...
static int hns3_dcb_pg_shapping_cfg(struct hns3_hw *hw,
                                    enum hns3_shap_bucket bucket, uint8_t pg_id,
                                    uint32_t shapping_para, uint32_t rate) {
  struct hns3_pg_shapping_cmd *shap_cfg_cmd;
  enum hns3_opcode_type opcode;
  struct hns3_cmd_desc desc;

  opcode = bucket ? HNS3_OPC_TM_PG_P_SHAPPING : HNS3_OPC_TM_PG_C_SHAPPING;
...
static int hns3_pg_shaper_rate_cfg(struct hns3_hw *hw, uint8_t pg_id,
                                   uint32_t rate) {
  struct hns3_shaper_parameter shaper_parameter;
  uint32_t ir_u, ir_b, ir_s;
  ...
  /* Calc shaper para */
  ret = hns3_shaper_para_calc(hw, rate, HNS3_SHAPER_LVL_PG, &shaper_parameter);
  if (ret) {
    hns3_err(hw, "calculate shaper parameter fail, ret = %d.", ret);
    return ret;
  }

  shaper_para = hns3_dcb_get_shapping_para(0, 0, 0, HNS3_SHAPER_BS_U_DEF,
                                           HNS3_SHAPER_BS_S_DEF);

  ret = hns3_dcb_pg_shapping_cfg(hw, HNS3_DCB_SHAP_C_BUCKET, pg_id, shaper_para,
                                 rate);
  if (ret) {
    hns3_err(hw, "config PG CIR shaper parameter fail, ret = %d.", ret);
    return ret;
  }
  ...
  ir_u = shaper_parameter.ir_u;
  ir_s = shaper_parameter.ir_s;
  shaper_para = hns3_dcb_get_shapping_para(
      ir_b, ir_u, ir_s, HNS3_SHAPER_BS_U_DEF, HNS3_SHAPER_BS_S_DEF);

  ret = hns3_dcb_pg_shapping_cfg(hw, HNS3_DCB_SHAP_P_BUCKET, pg_id, shaper_para,
                                 rate);
  if (ret) {
    hns3_err(hw, "config PG PIR shaper parameter fail, ret = %d.", ret);
    return ret;
  }
...
static int hns3_dcb_pg_shaper_cfg(struct hns3_hw *hw) {
...
static int hns3_dcb_qs_schd_mode_cfg(struct hns3_hw *hw, uint16_t qs_id,
                                     uint8_t mode) {
  struct hns3_cmd_desc desc;
...
static int hns3_dcb_pri_schd_mode_cfg(struct hns3_hw *hw, uint8_t pri_id) {
  struct hns3_cmd_desc desc;
...
static int hns3_dcb_pri_shapping_cfg(struct hns3_hw *hw,
                                     enum hns3_shap_bucket bucket,
  ...
  opcode = bucket ? HNS3_OPC_TM_PRI_P_SHAPPING : HNS3_OPC_TM_PRI_C_SHAPPING;
...
static int hns3_pri_shaper_rate_cfg(struct hns3_hw *hw, uint8_t tc_no,
                                    uint32_t rate) {
  struct hns3_shaper_parameter shaper_parameter;
  uint32_t ir_u, ir_b, ir_s;
  uint32_t shaper_para;
  ...
  ret = hns3_shaper_para_calc(hw, rate, HNS3_SHAPER_LVL_PRI, &shaper_parameter);
  if (ret) {
    hns3_err(hw, "calculate shaper parameter failed: %d.", ret);
    return ret;
  }
  ...
  ret = hns3_dcb_pri_shapping_cfg(hw, HNS3_DCB_SHAP_C_BUCKET, tc_no,
                                  shaper_para, rate);
  if (ret) {
    hns3_err(hw, "config priority CIR shaper parameter failed: %d.", ret);
    return ret;
  }
  ...
  ret = hns3_dcb_pri_shapping_cfg(hw, HNS3_DCB_SHAP_P_BUCKET, tc_no,
                                  shaper_para, rate);
  if (ret) {
    hns3_err(hw, "config priority PIR shaper parameter failed: %d.", ret);
    return ret;
  }
...
static int hns3_dcb_pri_shaper_cfg(struct hns3_hw *hw) {
  struct hns3_pf *pf = HNS3_DEV_HW_TO_PF(hw);
  uint32_t rate;
  uint8_t i;
  int ret;
...
static int hns3_set_rss_size(struct hns3_hw *hw, uint16_t nb_rx_q) {
  ...
  if (rx_qnum_per_tc > hw->rss_size_max) {
    hns3_err(hw,
             "rx queue number of per tc (%u) is greater than "
  ...
  if (used_rx_queues != nb_rx_q) {
    hns3_err(hw,
             "rx queue number (%u) configured must be an "
  ...
  if (__atomic_load_n(&hw->reset.resetting, __ATOMIC_RELAXED) == 0) {
    for (i = 0; i < hw->rss_ind_tbl_size; i++)
      rss_cfg->rss_indirection_tbl[i] = i % hw->alloc_rss_size;
  }

  return 0;
}

static int hns3_tc_queue_mapping_cfg(struct hns3_hw *hw, uint16_t nb_tx_q) {
...
static int hns3_dcb_update_tc_queue_mapping(struct hns3_hw *hw,
                                            uint16_t nb_rx_q,
                                            uint16_t nb_tx_q) {
...
int hns3_dcb_info_init(struct hns3_hw *hw) {
  ...
  if (pf->tx_sch_mode != HNS3_FLAG_TC_BASE_SCH_MODE && hw->dcb_info.num_pg != 1)
    return -EINVAL;

  /* Initializing PG information */
  memset(hw->dcb_info.pg_info, 0, sizeof(struct hns3_pg_info) * HNS3_PG_NUM);
  for (i = 0; i < hw->dcb_info.num_pg; i++) {
  ...
  for (i = 0; i < hw->dcb_info.num_tc; i++) {
    ...
    hw->dcb_info.tc_info[i].bw_limit = hw->dcb_info.pg_info[0].bw_limit;
  }

  return 0;
}

static int hns3_dcb_lvl2_schd_mode_cfg(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  int ret, i;
...
static int hns3_dcb_lvl34_schd_mode_cfg(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  uint8_t i;
  int ret;
  ...
  if (pf->tx_sch_mode == HNS3_FLAG_TC_BASE_SCH_MODE) {
    for (i = 0; i < hw->dcb_info.num_tc; i++) {
      ...
      ret = hns3_dcb_qs_schd_mode_cfg(hw, i, HNS3_SCH_MODE_DWRR);
      if (ret)
        return ret;
    }
  }
...
static int hns3_dcb_schd_mode_cfg(struct hns3_hw *hw) {
  int ret;
...
static int hns3_dcb_pri_tc_base_dwrr_cfg(struct hns3_hw *hw) {
  struct hns3_pg_info *pg_info;
  uint8_t dwrr;
  int ret, i;
  ...
  for (i = 0; i < hw->dcb_info.num_tc; i++) {
    ...
    if (ret) {
      hns3_err(hw, "fail to send priority weight cmd: %d, ret = %d", i, ret);
      return ret;
    }

    ret = hns3_dcb_qs_weight_cfg(hw, i, BW_MAX_PERCENT);
    if (ret) {
      hns3_err(hw, "fail to send qs_weight cmd: %d, ret = %d", i, ret);
      return ret;
    }
  }
...
static int hns3_dcb_pri_dwrr_cfg(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  uint32_t version;
  int ret;
  ...
  if (ret == -EOPNOTSUPP) {
    ...
    hns3_warn(hw, "fw %lu.%lu.%lu.%lu doesn't support ets tc weight cmd",
...
static int hns3_dcb_pg_dwrr_cfg(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  int ret, i;
...
static int hns3_dcb_dwrr_cfg(struct hns3_hw *hw) {
  int ret;
...
static int hns3_dcb_shaper_cfg(struct hns3_hw *hw) {
  int ret;
...
static int hns3_q_to_qs_map_cfg(struct hns3_hw *hw, uint16_t q_id,
                                uint16_t qs_id) {
   ...
   *          | qs_id_h | vld | qs_id_l |
   */
  qs_id_l = hns3_get_field(qs_id, HNS3_DCB_QS_ID_L_MSK, HNS3_DCB_QS_ID_L_S);
  qs_id_h = hns3_get_field(qs_id, HNS3_DCB_QS_ID_H_MSK, HNS3_DCB_QS_ID_H_S);
  hns3_set_field(tmp_qs_id, HNS3_DCB_QS_ID_L_MSK, HNS3_DCB_QS_ID_L_S, qs_id_l);
  hns3_set_field(tmp_qs_id, HNS3_DCB_QS_ID_H_EXT_MSK, HNS3_DCB_QS_ID_H_EXT_S,
                 qs_id_h);
...
static int hns3_q_to_qs_map(struct hns3_hw *hw) {
  struct hns3_tc_queue_info *tc_queue;
  uint16_t q_id;
  uint32_t i, j;
  int ret;
...
static int hns3_pri_q_qs_cfg(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  uint32_t i;
  int ret;
...
static int hns3_dcb_map_cfg(struct hns3_hw *hw) {
  int ret;
...
static int hns3_pause_param_cfg(struct hns3_hw *hw, const uint8_t *addr,
...
static int hns3_pause_param_setup_hw(struct hns3_hw *hw, uint16_t pause_time) {
...
#define PAUSE_TIME_MIN_VALUE 0x4
  ...
  return hns3_pause_param_cfg(hw, mac->mac_addr, pause_trans_gap, pause_time);
}

static int hns3_mac_pause_en_cfg(struct hns3_hw *hw, bool tx, bool rx) {
  struct hns3_cmd_desc desc;
...
static int hns3_pfc_pause_en_cfg(struct hns3_hw *hw, uint8_t pfc_bitmap,
                                 bool tx, bool rx) {
  struct hns3_cmd_desc desc;
...
static int hns3_qs_bp_cfg(struct hns3_hw *hw, uint8_t tc, uint8_t grp_id,
                          uint32_t bit_map) {
...
static void hns3_get_rx_tx_en_status(struct hns3_hw *hw, bool *tx_en,
                                     bool *rx_en) {
...
static int hns3_mac_pause_setup_hw(struct hns3_hw *hw) {
  bool tx_en, rx_en;
...
static int hns3_pfc_setup_hw(struct hns3_hw *hw) {
  bool tx_en, rx_en;
 ...
 */
static int hns3_bp_setup_hw(struct hns3_hw *hw, uint8_t tc) {
  uint32_t qs_bitmap;
  int ret;
  int i;
  ...
  for (i = 0; i < HNS3_BP_GRP_NUM; i++) {
    ...
    sub_grp = hns3_get_field(tc, HNS3_BP_SUB_GRP_ID_M, HNS3_BP_SUB_GRP_ID_S);
    if (i == grp)
      qs_bitmap |= (1 << sub_grp);
...
static int hns3_dcb_bp_setup(struct hns3_hw *hw) {
  int ret, i;
...
static int hns3_dcb_pause_setup_hw(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  int ret;
...
static uint8_t hns3_dcb_undrop_tc_map(struct hns3_hw *hw, uint8_t pfc_en) {
  uint8_t pfc_map = 0;
  uint8_t *prio_tc;
  uint8_t i, j;
...
static uint8_t hns3_dcb_parse_num_tc(struct hns3_adapter *hns) {
...
static int hns3_dcb_info_cfg(struct hns3_adapter *hns) {
  struct rte_eth_dcb_rx_conf *dcb_rx_conf;
  struct hns3_pf *pf = &hns->pf;
  struct hns3_hw *hw = &hns->hw;
...
static int hns3_dcb_info_update(struct hns3_adapter *hns, uint8_t num_tc) {
  struct hns3_pf *pf = &hns->pf;
  struct hns3_hw *hw = &hns->hw;
  ...
  if (nb_rx_q < num_tc) {
    hns3_err(hw, "number of Rx queues(%u) is less than tcs(%u).", nb_rx_q,
             num_tc);
    return -EINVAL;
  }

  if (nb_tx_q < num_tc) {
    hns3_err(hw, "number of Tx queues(%u) is less than tcs(%u).", nb_tx_q,
             num_tc);
    return -EINVAL;
  }
...
static int hns3_dcb_hw_configure(struct hns3_adapter *hns) {
  struct rte_eth_dcb_rx_conf *dcb_rx_conf;
  struct hns3_pf *pf = &hns->pf;
  struct hns3_hw *hw = &hns->hw;
  ...
  if (hw->data->dev_conf.dcb_capability_en & RTE_ETH_DCB_PFC_SUPPORT) {
    ...
    if (dcb_rx_conf->nb_tcs == 0)
      ...
      hw->dcb_info.pfc_en = RTE_LEN2MASK((uint8_t)dcb_rx_conf->nb_tcs, uint8_t);

    hw->dcb_info.hw_pfc_map = hns3_dcb_undrop_tc_map(hw, hw->dcb_info.pfc_en);
...
int hns3_dcb_init(struct hns3_hw *hw) {
  ...
  if (hw->adapter_state == HNS3_NIC_UNINITIALIZED) {
    ...
    ret =
        hns3_dcb_update_tc_queue_mapping(hw, default_tqp_num, default_tqp_num);
    if (ret) {
      hns3_err(hw, "update tc queue mapping failed, ret = %d.", ret);
      return ret;
    }
...
int hns3_update_queue_map_configure(struct hns3_adapter *hns) {
  ...
  if (ret) {
    hns3_err(hw, "failed to update tc queue mapping, ret = %d.", ret);
    return ret;
  }
  ret = hns3_q_to_qs_map(hw);
  if (ret)
...
static void hns3_get_fc_mode(struct hns3_hw *hw, enum rte_eth_fc_mode mode) {
  switch (mode) {
  ...
  default:
    hw->requested_fc_mode = HNS3_FC_NONE;
    hns3_warn(hw,
              "fc_mode(%u) exceeds member scope and is "
              "configured to RTE_ETH_FC_NONE",
              mode);
    break;
  }
}
