...
#define HNS3_FILTER_FE_EGRESS                                                  \
  (HNS3_FILTER_FE_NIC_EGRESS_B | HNS3_FILTER_FE_ROCE_EGRESS_B)
#define HNS3_FILTER_FE_INGRESS                                                 \
  (HNS3_FILTER_FE_NIC_INGRESS_B | HNS3_FILTER_FE_ROCE_INGRESS_B)
...
static const struct rte_eth_fec_capa speed_fec_capa_tbl[] = {
    ...
    {RTE_ETH_SPEED_NUM_25G,
     RTE_ETH_FEC_MODE_CAPA_MASK(NOFEC) | RTE_ETH_FEC_MODE_CAPA_MASK(AUTO) |
...
static void hns3_pf_enable_irq0(struct hns3_hw *hw) {
  hns3_write_dev(hw, HNS3_MISC_VECTOR_REG_BASE, 1);
}

static enum hns3_evt_cause hns3_proc_imp_reset_event(struct hns3_adapter *hns,
                                                     bool is_delay,
                                                     uint32_t *vec_val) {
...
static enum hns3_evt_cause hns3_check_event_cause(struct hns3_adapter *hns,
                                                  uint32_t *clearval) {
  struct hns3_hw *hw = &hns->hw;
...
static void hns3_clear_event_cause(struct hns3_hw *hw, uint32_t event_type,
                                   uint32_t regclr) {
...
static void hns3_clear_all_event_cause(struct hns3_hw *hw) {
  uint32_t vector0_int_stats;
...
static void hns3_handle_mac_tnl(struct hns3_hw *hw) {
  ...
  if (status) {
    ...
    hns3_cmd_setup_basic_desc(&desc, HNS3_OPC_CLEAR_MAC_TNL_INT, false);
    desc.data[0] = rte_cpu_to_le_32(HNS3_MAC_TNL_INT_CLR);
    ret = hns3_cmd_send(hw, &desc, 1);
    if (ret)
      hns3_err(hw, "failed to clear mac tnl int, ret = %d.", ret);
  }
}

static void hns3_interrupt_handler(void *param) {
  ...
  if (event_cause == HNS3_VECTOR0_EVENT_ERR) {
    hns3_warn(hw,
              "received interrupt: vector0_int_stat:0x%x "
    ...
    hns3_warn(hw,
              "received unknown event: vector0_int_stat:0x%x "
              "ras_int_stat:0x%x cmdq_int_stat:0x%x",
              vector0_int, ras_int, cmdq_int);
  }
...
static int hns3_set_port_vlan_filter(struct hns3_adapter *hns, uint16_t vlan_id,
                                     int on) {
...
#define HNS3_VLAN_BYTE_SIZE 8
  ...
  vlan_offset_byte = (vlan_id % HNS3_VLAN_ID_OFFSET_STEP) / HNS3_VLAN_BYTE_SIZE;
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret)
    hns3_err(hw, "set port vlan id failed, vlan_id =%u, ret =%d", vlan_id, ret);

  return ret;
}

static void hns3_rm_dev_vlan_table(struct hns3_adapter *hns, uint16_t vlan_id) {
...
static void hns3_add_dev_vlan_table(struct hns3_adapter *hns, uint16_t vlan_id,
                                    bool writen_to_tbl) {
  struct hns3_user_vlan_table *vlan_entry;
  struct hns3_hw *hw = &hns->hw;
  struct hns3_pf *pf = &hns->pf;
...
static int hns3_restore_vlan_table(struct hns3_adapter *hns) {
  struct hns3_user_vlan_table *vlan_entry;
  struct hns3_hw *hw = &hns->hw;
  struct hns3_pf *pf = &hns->pf;
  uint16_t vlan_id;
  ...
  if (hw->port_base_vlan_cfg.state == HNS3_PORT_BASE_VLAN_ENABLE)
    return hns3_vlan_pvid_configure(hns, hw->port_base_vlan_cfg.pvid, 1);
...
static int hns3_vlan_filter_configure(struct hns3_adapter *hns,
...
static int hns3_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                int on) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int hns3_vlan_tpid_configure(struct hns3_adapter *hns,
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret) {
    hns3_err(hw, "Send rxvlan protocol type command fail, ret =%d", ret);
    return ret;
  }
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret)
    hns3_err(hw, "Send txvlan protocol type command fail, ret =%d", ret);
  return ret;
}

static int hns3_vlan_tpid_set(struct rte_eth_dev *dev,
...
static int hns3_set_vlan_rx_offload_cfg(struct hns3_adapter *hns,
...
static int hns3_en_hw_strip_rxvtag(struct hns3_adapter *hns, bool enable) {
...
static int hns3_set_vlan_filter_ctrl(struct hns3_hw *hw, uint8_t vlan_type,
                                     uint8_t fe_type, bool filter_en,
                                     uint8_t vf_id) {
...
static int hns3_vlan_filter_init(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;

  ret = hns3_set_vlan_filter_ctrl(
      hw, HNS3_FILTER_TYPE_VF, HNS3_FILTER_FE_EGRESS, false, HNS3_PF_FUNC_ID);
  if (ret) {
  ...
  ret =
      hns3_set_vlan_filter_ctrl(hw, HNS3_FILTER_TYPE_PORT,
                                HNS3_FILTER_FE_INGRESS, false, HNS3_PF_FUNC_ID);
  if (ret)
...
static int hns3_enable_vlan_filter(struct hns3_adapter *hns, bool enable) {
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int hns3_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  ...
  if (tmp_mask & RTE_ETH_VLAN_FILTER_MASK) {
    ...
    if (!dev->data->promiscuous) {
      ...
      enable = rxmode->offloads & RTE_ETH_RX_OFFLOAD_VLAN_FILTER ? true : false;
  ...
  if (tmp_mask & RTE_ETH_VLAN_STRIP_MASK) {
    ...
    enable = rxmode->offloads & RTE_ETH_RX_OFFLOAD_VLAN_STRIP ? true : false;
...
static int hns3_set_vlan_tx_offload_cfg(struct hns3_adapter *hns,
...
static int hns3_vlan_txvlan_cfg(struct hns3_adapter *hns,
                                uint16_t port_base_vlan_state, uint16_t pvid) {
  struct hns3_hw *hw = &hns->hw;
  ...
  if (port_base_vlan_state == HNS3_PORT_BASE_VLAN_DISABLE) {
    ...
    txvlan_cfg.accept_tag1 = hw->vlan_mode == HNS3_HW_SHIFT_AND_DISCARD_MODE;
  ...
  if (ret) {
    hns3_err(hw, "pf vlan set pvid failed, pvid =%u ,ret =%d", pvid, ret);
    return ret;
  }
...
static void hns3_rm_all_vlan_table(struct hns3_adapter *hns, bool is_del_list) {
  struct hns3_user_vlan_table *vlan_entry;
  struct hns3_pf *pf = &hns->pf;
...
static void hns3_add_all_vlan_table(struct hns3_adapter *hns) {
  struct hns3_user_vlan_table *vlan_entry;
  struct hns3_pf *pf = &hns->pf;
...
static void hns3_remove_all_vlan_table(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;

  hns3_rm_all_vlan_table(hns, true);
  if (hw->port_base_vlan_cfg.pvid != HNS3_INVALID_PVID) {
    ret = hns3_set_port_vlan_filter(hns, hw->port_base_vlan_cfg.pvid, 0);
    if (ret) {
      hns3_err(hw, "Failed to remove all vlan table, ret =%d", ret);
      return;
    }
  }
}

static int hns3_update_vlan_filter_entries(struct hns3_adapter *hns,
  ...
  if (port_base_vlan_state == HNS3_PORT_BASE_VLAN_ENABLE) {
    ...
    if (ret) {
      hns3_err(hw, "failed to add new pvid %u, ret = %d", new_pvid, ret);
      return ret;
    }
  } else {
    ret = hns3_set_port_vlan_filter(hns, new_pvid, 0);
    if (ret) {
      hns3_err(hw, "failed to remove pvid %u, ret = %d", new_pvid, ret);
      return ret;
    }
...
static int hns3_en_pvid_strip(struct hns3_adapter *hns, int on) {
...
static int hns3_vlan_pvid_configure(struct hns3_adapter *hns, uint16_t pvid,
  ...
  if (on == 0 && pvid != hw->port_base_vlan_cfg.pvid) {
    if (hw->port_base_vlan_cfg.pvid != HNS3_INVALID_PVID)
      hns3_warn(hw,
                "Invalid operation! As current pvid set "
  ...
  port_base_vlan_state =
      on ? HNS3_PORT_BASE_VLAN_ENABLE : HNS3_PORT_BASE_VLAN_DISABLE;
  ret = hns3_vlan_txvlan_cfg(hns, port_base_vlan_state, pvid);
  if (ret) {
    hns3_err(hw, "failed to config tx vlan for pvid, ret = %d", ret);
    return ret;
  }
  ...
  if (ret) {
    hns3_err(hw,
             "failed to config rx vlan strip for pvid, "
             "ret = %d",
             ret);
  ...
  if (ret) {
    hns3_err(hw, "failed to update vlan filter entries, ret = %d", ret);
...
static int hns3_vlan_pvid_set(struct rte_eth_dev *dev, uint16_t pvid, int on) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  ...
  if (pvid > RTE_ETHER_MAX_VLAN_ID) {
    hns3_err(hw, "Invalid vlan_id = %u > %d", pvid, RTE_ETHER_MAX_VLAN_ID);
    return -EINVAL;
  }
   ...
   */
  if (pvid_en_state_change && hw->vlan_mode == HNS3_SW_SHIFT_AND_DISCARD_MODE)
    hns3_update_all_queues_pvid_proc_en(hw);

  return 0;
}

static int hns3_default_vlan_config(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int hns3_init_vlan_config(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;
  ...
  if (__atomic_load_n(&hw->reset.resetting, __ATOMIC_RELAXED) == 0) {
    ...
    if (ret) {
      hns3_err(hw, "rx strip configure fail in pf, ret =%d", ret);
      return ret;
    }
  }
...
static int hns3_restore_vlan_conf(struct hns3_adapter *hns) {
  struct hns3_pf *pf = &hns->pf;
  struct hns3_hw *hw = &hns->hw;
  uint64_t offloads;
  bool enable;
  ...
  if (!hw->data->promiscuous) {
    ...
    if (ret) {
      hns3_err(hw,
               "failed to restore vlan rx filter conf, "
               "ret = %d",
               ret);
      return ret;
    }
...
static int hns3_dev_configure_vlan(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  ...
  if (txmode->hw_vlan_reject_tagged || txmode->hw_vlan_reject_untagged)
    hns3_warn(hw,
              ...
              txmode->hw_vlan_reject_tagged, txmode->hw_vlan_reject_untagged);

  /* Apply vlan offload setting */
  mask = RTE_ETH_VLAN_STRIP_MASK | RTE_ETH_VLAN_FILTER_MASK;
  ret = hns3_vlan_offload_set(dev, mask);
  if (ret) {
    hns3_err(hw, "dev config rx vlan offload failed, ret = %d", ret);
    return ret;
  }
  ...
  /* Apply pvid setting */
  ret = hns3_vlan_pvid_set(dev, txmode->pvid, txmode->hw_vlan_insert_pvid);
  if (ret)
    hns3_err(hw, "dev config vlan pvid(%u) failed, ret = %d", txmode->pvid,
             ret);

  return ret;
}

static int hns3_config_tso(struct hns3_hw *hw, unsigned int tso_mss_min,
  ...
  tso_mss = 0;
  hns3_set_field(tso_mss, HNS3_TSO_MSS_MIN_M, HNS3_TSO_MSS_MIN_S, tso_mss_min);
  req->tso_mss_min = rte_cpu_to_le_16(tso_mss);

  tso_mss = 0;
  hns3_set_field(tso_mss, HNS3_TSO_MSS_MIN_M, HNS3_TSO_MSS_MIN_S, tso_mss_max);
  req->tso_mss_max = rte_cpu_to_le_16(tso_mss);

  return hns3_cmd_send(hw, &desc, 1);
}

static int hns3_set_umv_space(struct hns3_hw *hw, uint16_t space_size,
...
static int hns3_init_umv_space(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  uint16_t allocated_size = 0;
  int ret;

  ret = hns3_set_umv_space(hw, pf->wanted_umv_size, &allocated_size, true);
  if (ret)
    return ret;
  ...
  pf->max_umv_size = (!!allocated_size) ? allocated_size : pf->wanted_umv_size;
  pf->used_umv_size = 0;
  return 0;
}

static int hns3_uninit_umv_space(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  int ret;
...
static bool hns3_is_umv_space_full(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  bool is_full;
...
static void hns3_update_umv_space(struct hns3_hw *hw, bool is_free) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
...
static void hns3_prepare_mac_addr(struct hns3_mac_vlan_tbl_entry_cmd *new_req,
...
static int hns3_get_mac_vlan_cmd_status(struct hns3_hw *hw, uint16_t cmdq_resp,
                                        uint8_t resp_code,
  ...
  if (op == HNS3_MAC_VLAN_ADD) {
    ...
    hns3_err(hw, "add mac addr failed for undefined, code=%u", resp_code);
    return -EIO;
    ...
    hns3_err(hw, "remove mac addr failed for undefined, code=%u", resp_code);
    return -EIO;
    ...
    hns3_err(hw, "lookup mac addr failed for undefined, code=%u", resp_code);
    return -EIO;
  }

  hns3_err(hw, "unknown opcode for get_mac_vlan_cmd_status, opcode=%u", op);

  return -EINVAL;
}

static int hns3_lookup_mac_vlan_tbl(struct hns3_hw *hw,
  ...
  if (desc_num == HNS3_MC_MAC_VLAN_OPS_DESC_NUM) {
    for (i = 0; i < desc_num - 1; i++) {
      hns3_cmd_setup_basic_desc(&desc[i], HNS3_OPC_MAC_VLAN_ADD, true);
      desc[i].flag |= rte_cpu_to_le_16(HNS3_CMD_FLAG_NEXT);
      if (i == 0)
        memcpy(desc[i].data, req, sizeof(struct hns3_mac_vlan_tbl_entry_cmd));
    }
    hns3_cmd_setup_basic_desc(&desc[i], HNS3_OPC_MAC_VLAN_ADD, true);
  } else {
    hns3_cmd_setup_basic_desc(&desc[0], HNS3_OPC_MAC_VLAN_ADD, true);
    memcpy(desc[0].data, req, sizeof(struct hns3_mac_vlan_tbl_entry_cmd));
  }
  ret = hns3_cmd_send(hw, desc, desc_num);
  if (ret) {
    hns3_err(hw, "lookup mac addr failed for cmd_send, ret =%d.", ret);
    return ret;
  }
...
static int hns3_add_mac_vlan_tbl(struct hns3_hw *hw,
  ...
  if (desc_num == HNS3_UC_MAC_VLAN_OPS_DESC_NUM) {
    ...
    cfg_status =
        hns3_get_mac_vlan_cmd_status(hw, retval, resp_code, HNS3_MAC_VLAN_ADD);
  } else {
...
static int hns3_remove_mac_vlan_tbl(struct hns3_hw *hw,
                                    struct hns3_mac_vlan_tbl_entry_cmd *req) {
  struct hns3_cmd_desc desc;
  uint8_t resp_code;
  uint16_t retval;
...
static int hns3_add_uc_mac_addr(struct hns3_hw *hw,
                                struct rte_ether_addr *mac_addr) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_mac_vlan_tbl_entry_cmd req;
  struct hns3_pf *pf = &hns->pf;
  ...
  if (!rte_is_valid_assigned_ether_addr(mac_addr)) {
    ...
    hns3_err(hw, "Add unicast mac addr err! addr(%s) invalid", mac_str);
    return -EINVAL;
  }
  ...
  hns3_set_field(egress_port, HNS3_MAC_EPORT_VFID_M, HNS3_MAC_EPORT_VFID_S,
                 vf_id);
   ...
   */
  ret =
      hns3_lookup_mac_vlan_tbl(hw, &req, &desc, HNS3_UC_MAC_VLAN_OPS_DESC_NUM);
  if (ret == -ENOENT) {
    if (!hns3_is_umv_space_full(hw)) {
      ret =
          hns3_add_mac_vlan_tbl(hw, &req, &desc, HNS3_UC_MAC_VLAN_OPS_DESC_NUM);
      if (!ret)
  ...
  hns3_err(hw, "PF failed to add unicast entry(%s) in the MAC table", mac_str);

  return ret;
}

static int hns3_remove_uc_mac_addr(struct hns3_hw *hw,
                                   struct rte_ether_addr *mac_addr) {
  struct hns3_mac_vlan_tbl_entry_cmd req;
  char mac_str[RTE_ETHER_ADDR_FMT_SIZE];
  int ret;
  ...
  if (!rte_is_valid_assigned_ether_addr(mac_addr)) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
    hns3_err(hw, "remove unicast mac addr err! addr(%s) invalid", mac_str);
    return -EINVAL;
  }
...
static int hns3_set_default_mac_addr(struct rte_eth_dev *dev,
                                     struct rte_ether_addr *mac_addr) {
  ...
  if (ret) {
    ...
    hns3_warn(hw, "Remove old uc mac address(%s) fail: %d", mac_str, ret);
  ...
  rte_ether_addr_copy(mac_addr, (struct rte_ether_addr *)hw->mac.mac_addr);
  rte_spinlock_unlock(&hw->lock);
...
err_pause_addr_cfg:
  ...
  if (ret_val) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
    hns3_warn(hw, "Failed to roll back to del setted mac addr(%s): %d", mac_str,
              ret_val);
  }
...
err_add_uc_addr:
  ...
  if (ret_val) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, oaddr);
    hns3_warn(hw, "Failed to restore old uc mac addr(%s): %d", mac_str,
              ret_val);
  }
  rte_spinlock_unlock(&hw->lock);
...
static void hns3_update_desc_vfid(struct hns3_cmd_desc *desc, uint8_t vfid,
                                  bool clr) {
#define HNS3_VF_NUM_IN_FIRST_DESC 192
  ...
  if (vfid < HNS3_VF_NUM_IN_FIRST_DESC) {
    ...
    bit_num = vfid % 32;
    if (clr)
      desc[1].data[word_num] &= rte_cpu_to_le_32(~(1UL << bit_num));
    else
      desc[1].data[word_num] |= rte_cpu_to_le_32(1UL << bit_num);
  } else {
    ...
    bit_num = vfid % 32;
    if (clr)
      desc[2].data[word_num] &= rte_cpu_to_le_32(~(1UL << bit_num));
    else
      desc[2].data[word_num] |= rte_cpu_to_le_32(1UL << bit_num);
  }
}
...
static int hns3_add_mc_mac_addr(struct hns3_hw *hw,
  ...
  if (!rte_is_multicast_ether_addr(mac_addr)) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
    hns3_err(hw, "failed to add mc mac addr, addr(%s) invalid", mac_str);
    return -EINVAL;
  }
  ...
  ret = hns3_lookup_mac_vlan_tbl(hw, &req, desc, HNS3_MC_MAC_VLAN_OPS_DESC_NUM);
  if (ret) {
  ...
  ret = hns3_add_mac_vlan_tbl(hw, &req, desc, HNS3_MC_MAC_VLAN_OPS_DESC_NUM);
  if (ret) {
    if (ret == -ENOSPC)
...
static int hns3_remove_mc_mac_addr(struct hns3_hw *hw,
  ...
  if (!rte_is_multicast_ether_addr(mac_addr)) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
    hns3_err(hw, "Failed to rm mc mac addr, addr(%s) invalid", mac_str);
    return -EINVAL;
  }
...
static int hns3_check_mq_mode(struct rte_eth_dev *dev) {
  ...
  if ((uint32_t)rx_mq_mode & RTE_ETH_MQ_RX_DCB_FLAG) {
    ...
    if (num_tc > dcb_rx_conf->nb_tcs) {
      hns3_err(hw, "max num_tc(%u) mapped > nb_tcs(%u)", num_tc,
               dcb_rx_conf->nb_tcs);
      return -EINVAL;
    }
  }
...
static int hns3_bind_ring_with_vector(struct hns3_hw *hw, uint16_t vector_id,
  ...
  req->int_vector_id =
      hns3_get_field(vector_id, HNS3_TQP_INT_ID_L_M, HNS3_TQP_INT_ID_L_S);
  req->int_vector_id_h =
      hns3_get_field(vector_id, HNS3_TQP_INT_ID_H_M, HNS3_TQP_INT_ID_H_S);
  ...
  hns3_set_field(tqp_type_and_id, HNS3_INT_TYPE_M, HNS3_INT_TYPE_S, type);
  hns3_set_field(tqp_type_and_id, HNS3_TQP_ID_M, HNS3_TQP_ID_S, queue_id);
  hns3_set_field(tqp_type_and_id, HNS3_INT_GL_IDX_M, HNS3_INT_GL_IDX_S, gl);
...
static int hns3_setup_dcb(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int hns3_check_link_speed(struct hns3_hw *hw, uint32_t link_speeds) {
  int ret;
   ...
   */
  if (link_speeds == RTE_ETH_LINK_SPEED_AUTONEG && hw->mac.support_autoneg == 0)
    return 0;
...
static int hns3_check_dev_conf(struct rte_eth_dev *dev) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int hns3_dev_configure(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct rte_eth_conf *conf = &dev->data->dev_conf;
...
static int hns3_set_mac_mtu(struct hns3_hw *hw, uint16_t new_mps) {
...
static int hns3_config_mtu(struct hns3_hw *hw, uint16_t mps) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  int err;
  int ret;
...
static int hns3_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  ...
  if (dev->data->dev_started) {
    hns3_err(hw,
             "Failed to set mtu, port %u must be stopped "
  ...
  if (ret) {
    rte_spinlock_unlock(&hw->lock);
    hns3_err(hw, "Failed to set mtu, port %u mtu %u: %d", dev->data->port_id,
             mtu, ret);
    return ret;
  }
...
static uint32_t hns3_get_copper_port_speed_capa(uint32_t supported_speed) {
...
static uint32_t hns3_get_firber_port_speed_capa(uint32_t supported_speed) {
  uint32_t speed_capa = 0;
...
uint32_t hns3_get_speed_capa(struct hns3_hw *hw) {
  ...
  if (mac->media_type == HNS3_MEDIA_TYPE_COPPER)
    speed_capa = hns3_get_copper_port_speed_capa(mac->supported_speed);
  else
    speed_capa = hns3_get_firber_port_speed_capa(mac->supported_speed);
...
static int hns3_update_port_link_info(struct rte_eth_dev *eth_dev) {
...
static void hns3_setup_linkstatus(struct rte_eth_dev *eth_dev,
  ...
  new_link->link_status =
      mac->link_status ? RTE_ETH_LINK_UP : RTE_ETH_LINK_DOWN;
  new_link->link_autoneg = mac->link_autoneg;
}

static int hns3_dev_link_update(struct rte_eth_dev *eth_dev,
...
static int hns3_dev_set_link_up(struct rte_eth_dev *dev) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  int ret;
...
static int hns3_dev_set_link_down(struct rte_eth_dev *dev) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  int ret;
...
static int hns3_parse_func_status(struct hns3_hw *hw,
                                  struct hns3_func_status_cmd *status) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
...
static int hns3_query_function_status(struct hns3_hw *hw) {
...
#define HNS3_QUERY_SLEEP_MSCOEND 1
  ...
  do {
    ret = hns3_cmd_send(hw, &desc, 1);
    if (ret) {
      PMD_INIT_LOG(ERR, "query function status failed %d", ret);
      return ret;
    }
...
static int hns3_get_pf_max_tqp_num(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  ...
  if (pf->tqp_config_mode == HNS3_FLEX_MAX_TQP_NUM_MODE) {
    ...
    if (RTE_LIBRTE_HNS3_MAX_TQP_NUM_PER_PF <= 0) {
      hns3_err(hw,
               "RTE_LIBRTE_HNS3_MAX_TQP_NUM_PER_PF(%d) "
    ...
    hw->tqps_num =
        RTE_MIN(RTE_LIBRTE_HNS3_MAX_TQP_NUM_PER_PF, hw->total_tqps_num);
  } else {
    /*
     ...
     */
    hw->tqps_num = RTE_MIN(hw->total_tqps_num, HNS3_MAX_TQP_NUM_HIP08_PF);
  }

  return 0;
}

static int hns3_query_pf_resource(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  ...
  hw->total_tqps_num =
      rte_le_to_cpu_16(req->tqp_num) + rte_le_to_cpu_16(req->ext_tqp_num);
  ...
  if (req->tx_buf_size)
    pf->tx_buf_size = rte_le_to_cpu_16(req->tx_buf_size) << HNS3_BUF_UNIT_S;
  else
  ...
  if (req->dv_buf_size)
    pf->dv_buf_size = rte_le_to_cpu_16(req->dv_buf_size) << HNS3_BUF_UNIT_S;
  else
    pf->dv_buf_size = HNS3_DEFAULT_DV;

  pf->dv_buf_size = roundup(pf->dv_buf_size, HNS3_BUF_SIZE_UNIT);

  hw->num_msi = hns3_get_field(rte_le_to_cpu_16(req->nic_pf_intr_vector_number),
                               HNS3_PF_VEC_NUM_M, HNS3_PF_VEC_NUM_S);

  return 0;
}

static void hns3_parse_cfg(struct hns3_cfg *cfg, struct hns3_cmd_desc *desc) {
  ...
  cfg->phy_addr = hns3_get_field(rte_le_to_cpu_32(req->param[1]),
                                 HNS3_CFG_PHY_ADDR_M, HNS3_CFG_PHY_ADDR_S);
  cfg->media_type = hns3_get_field(rte_le_to_cpu_32(req->param[1]),
                                   HNS3_CFG_MEDIA_TP_M, HNS3_CFG_MEDIA_TP_S);
  /* get mac address */
  mac_addr_tmp = rte_le_to_cpu_32(req->param[2]);
  mac_addr_tmp_high =
      hns3_get_field(rte_le_to_cpu_32(req->param[3]), HNS3_CFG_MAC_ADDR_H_M,
                     HNS3_CFG_MAC_ADDR_H_S);

  mac_addr_tmp |= (mac_addr_tmp_high << 31) << 1;

  cfg->default_speed =
      hns3_get_field(rte_le_to_cpu_32(req->param[3]), HNS3_CFG_DEFAULT_SPEED_M,
                     HNS3_CFG_DEFAULT_SPEED_S);
  cfg->rss_size_max = hns3_get_field(rte_le_to_cpu_32(req->param[3]),
                                     HNS3_CFG_RSS_SIZE_M, HNS3_CFG_RSS_SIZE_S);
  ...
  cfg->speed_ability =
      hns3_get_field(rte_le_to_cpu_32(req->param[1]), HNS3_CFG_SPEED_ABILITY_M,
                     HNS3_CFG_SPEED_ABILITY_S);
  cfg->umv_space =
      hns3_get_field(rte_le_to_cpu_32(req->param[1]), HNS3_CFG_UMV_TBL_SPACE_M,
                     HNS3_CFG_UMV_TBL_SPACE_S);
  if (!cfg->umv_space)
    cfg->umv_space = HNS3_DEFAULT_UMV_SPACE_PER_PF;

  ext_rss_size_max =
      hns3_get_field(rte_le_to_cpu_32(req->param[2]), HNS3_CFG_EXT_RSS_SIZE_M,
 ...
 */
static int hns3_get_board_cfg(struct hns3_hw *hw, struct hns3_cfg *hcfg) {
  ...
  for (i = 0; i < HNS3_PF_CFG_DESC_NUM; i++) {
    ...
    hns3_cmd_setup_basic_desc(&desc[i], HNS3_OPC_GET_CFG_PARAM, true);
...
static int hns3_parse_speed(int speed_cmd, uint32_t *speed) {
  switch (speed_cmd) {
...
static void hns3_set_default_dev_specifications(struct hns3_hw *hw) {
...
static int hns3_query_dev_specifications(struct hns3_hw *hw) {
...
static int hns3_get_capability(struct hns3_hw *hw) {
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "failed to query dev specifications, ret = %d", ret);
    return ret;
  }
...
static int hns3_check_media_type(struct hns3_hw *hw, uint8_t media_type) {
  int ret;
...
static int hns3_get_board_configuration(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  struct hns3_cfg cfg;
  int ret;
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "Get wrong speed %u, ret = %d", cfg.default_speed, ret);
    return ret;
  }

  pf->tc_max = cfg.tc_num;
  if (pf->tc_max > HNS3_MAX_TC_NUM || pf->tc_max < 1) {
    PMD_INIT_LOG(WARNING, "Get TC num(%u) from flash, set TC num to 1",
                 pf->tc_max);
    pf->tc_max = 1;
  }
  ...
  hw->alloc_rss_size =
      RTE_MIN(hw->rss_size_max, hw->tqps_num / hw->dcb_info.num_tc);
...
static int hns3_get_configuration(struct hns3_hw *hw) {
  int ret;
  ...
  if (ret)
    PMD_INIT_LOG(ERR, "failed to query FEC information, ret = %d", ret);

  return ret;
}

static int hns3_map_tqps_to_func(struct hns3_hw *hw, uint16_t func_id,
                                 uint16_t tqp_pid, uint16_t tqp_vid,
                                 bool is_pf) {
...
static int hns3_map_tqp(struct hns3_hw *hw) {
  int ret;
  int i;
...
static int hns3_cfg_mac_speed_dup_hw(struct hns3_hw *hw, uint32_t speed,
                                     uint8_t duplex) {
  ...
  switch (speed) {
  case RTE_ETH_SPEED_NUM_10M:
    hns3_set_field(req->speed_dup, HNS3_CFG_SPEED_M, HNS3_CFG_SPEED_S,
                   HNS3_CFG_SPEED_10M);
    break;
  case RTE_ETH_SPEED_NUM_100M:
    hns3_set_field(req->speed_dup, HNS3_CFG_SPEED_M, HNS3_CFG_SPEED_S,
...
static int hns3_tx_buffer_calc(struct hns3_hw *hw,
...
static int hns3_tx_buffer_alloc(struct hns3_hw *hw,
                                struct hns3_pkt_buf_alloc *buf_alloc) {
...
#define HNS3_BUF_SIZE_UPDATE_EN_MSK BIT(15)
  ...
  for (i = 0; i < HNS3_MAX_TC_NUM; i++) {
    ...
    req->tx_pkt_buff[i] =
        rte_cpu_to_le_16(buf_size | HNS3_BUF_SIZE_UPDATE_EN_MSK);
  }
...
static int hns3_get_tc_num(struct hns3_hw *hw) {
  int cnt = 0;
  uint8_t i;
...
static uint32_t hns3_get_tx_buff_alloced(struct hns3_pkt_buf_alloc *buf_alloc) {
...
static int hns3_get_pfc_priv_num(struct hns3_hw *hw,
                                 struct hns3_pkt_buf_alloc *buf_alloc) {
  struct hns3_priv_buf *priv;
  int cnt = 0;
  uint8_t i;
...
static int hns3_get_no_pfc_priv_num(struct hns3_hw *hw,
                                    struct hns3_pkt_buf_alloc *buf_alloc) {
  struct hns3_priv_buf *priv;
  int cnt = 0;
  uint8_t i;
  ...
  for (i = 0; i < HNS3_MAX_TC_NUM; i++) {
    priv = &buf_alloc->priv_buf[i];
    if (hw->hw_tc_map & BIT(i) && !(hw->dcb_info.hw_pfc_map & BIT(i)) &&
        priv->enable)
      cnt++;
  }
...
static bool hns3_is_rx_buf_ok(struct hns3_hw *hw,
                              struct hns3_pkt_buf_alloc *buf_alloc,
                              uint32_t rx_all) {
  ...
  if (hns3_dev_get_support(hw, DCB))
    shared_buf_min = HNS3_BUF_MUL_BY * aligned_mps + pf->dv_buf_size;
  else
    shared_buf_min =
        aligned_mps + HNS3_NON_DCB_ADDITIONAL_BUF + pf->dv_buf_size;

  shared_buf_tc = tc_num * aligned_mps + aligned_mps;
  shared_std =
      roundup(RTE_MAX(shared_buf_min, shared_buf_tc), HNS3_BUF_SIZE_UNIT);
  ...
  if (hns3_dev_get_support(hw, DCB)) {
    ...
    buf_alloc->s_buf.self.low =
        ...
        roundup(aligned_mps / HNS3_BUF_DIV_BY, HNS3_BUF_SIZE_UNIT);
  } else {
    buf_alloc->s_buf.self.high = aligned_mps + HNS3_NON_DCB_ADDITIONAL_BUF;
  ...
  if (hns3_dev_get_support(hw, DCB)) {
    ...
    if (tc_num <= NEED_RESERVE_TC_NUM)
      hi_thrd = hi_thrd * BUF_RESERVE_PERCENT / BUF_MAX_PERCENT;
...
static bool hns3_rx_buf_calc_all(struct hns3_hw *hw, bool max,
                                 struct hns3_pkt_buf_alloc *buf_alloc) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  struct hns3_priv_buf *priv;
  ...
  for (i = 0; i < HNS3_MAX_TC_NUM; i++) {
    ...
    if (hw->dcb_info.hw_pfc_map & BIT(i)) {
      ...
      priv->wl.high = roundup(priv->wl.low + aligned_mps, HNS3_BUF_SIZE_UNIT);
    } else {
      priv->wl.low = 0;
      priv->wl.high = max ? (aligned_mps * HNS3_BUF_MUL_BY) : aligned_mps;
    }
...
static bool hns3_drop_nopfc_buf_till_fit(struct hns3_hw *hw,
                                         struct hns3_pkt_buf_alloc *buf_alloc) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  struct hns3_priv_buf *priv;
  ...
  for (i = HNS3_MAX_TC_NUM - 1; i >= 0; i--) {
    ...
    if (hns3_is_rx_buf_ok(hw, buf_alloc, rx_all) || no_pfc_priv_num == 0)
      break;
  }
...
static bool hns3_drop_pfc_buf_till_fit(struct hns3_hw *hw,
                                       struct hns3_pkt_buf_alloc *buf_alloc) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  struct hns3_priv_buf *priv;
  ...
  for (i = HNS3_MAX_TC_NUM - 1; i >= 0; i--) {
    ...
    if (hns3_is_rx_buf_ok(hw, buf_alloc, rx_all) || pfc_priv_num == 0)
      break;
  }
...
static bool hns3_only_alloc_priv_buff(struct hns3_hw *hw,
                                      struct hns3_pkt_buf_alloc *buf_alloc) {
...
#define PRIV_WL_GAP 0x1800
   ...
   */
  min_rx_priv =
      pf->dv_buf_size + COMPENSATE_BUFFER + COMPENSATE_HALF_MPS_NUM * half_mps;
 ...
 */
static int hns3_rx_buffer_calc(struct hns3_hw *hw,
                               struct hns3_pkt_buf_alloc *buf_alloc) {
...
static int hns3_rx_priv_buf_alloc(struct hns3_hw *hw,
                                  struct hns3_pkt_buf_alloc *buf_alloc) {
  ...
  for (i = 0; i < HNS3_MAX_TC_NUM; i++) {
    ...
    req->buf_num[i] = rte_cpu_to_le_16(priv->buf_size >> HNS3_BUF_UNIT_S);
...
static int hns3_rx_priv_wl_config(struct hns3_hw *hw,
                                  struct hns3_pkt_buf_alloc *buf_alloc) {
#define HNS3_RX_PRIV_WL_ALLOC_DESC_NUM 2
  ...
  for (i = 0; i < HNS3_RX_PRIV_WL_ALLOC_DESC_NUM; i++) {
    hns3_cmd_setup_basic_desc(&desc[i], HNS3_OPC_RX_PRIV_WL_ALLOC, false);
    ...
    for (j = 0; j < HNS3_TC_NUM_ONE_DESC; j++) {
      ...
      req->tc_wl[j].high = rte_cpu_to_le_16(priv->wl.high >> HNS3_BUF_UNIT_S);
      req->tc_wl[j].high |= rte_cpu_to_le_16(BIT(HNS3_RX_PRIV_EN_B));
      req->tc_wl[j].low = rte_cpu_to_le_16(priv->wl.low >> HNS3_BUF_UNIT_S);
      req->tc_wl[j].low |= rte_cpu_to_le_16(BIT(HNS3_RX_PRIV_EN_B));
    }
  }
  ...
  if (ret)
    PMD_INIT_LOG(ERR, "rx private waterline config cmd failed %d", ret);
  return ret;
}

static int hns3_common_thrd_config(struct hns3_hw *hw,
                                   struct hns3_pkt_buf_alloc *buf_alloc) {
#define HNS3_RX_COM_THRD_ALLOC_DESC_NUM 2
  ...
  for (i = 0; i < HNS3_RX_COM_THRD_ALLOC_DESC_NUM; i++) {
    hns3_cmd_setup_basic_desc(&desc[i], HNS3_OPC_RX_COM_THRD_ALLOC, false);
    ...
    for (j = 0; j < HNS3_TC_NUM_ONE_DESC; j++) {
      ...
      req->com_thrd[j].high = rte_cpu_to_le_16(tc->high >> HNS3_BUF_UNIT_S);
      req->com_thrd[j].high |= rte_cpu_to_le_16(BIT(HNS3_RX_PRIV_EN_B));
      req->com_thrd[j].low = rte_cpu_to_le_16(tc->low >> HNS3_BUF_UNIT_S);
      req->com_thrd[j].low |= rte_cpu_to_le_16(BIT(HNS3_RX_PRIV_EN_B));
    }
  }
...
static int hns3_common_wl_config(struct hns3_hw *hw,
                                 struct hns3_pkt_buf_alloc *buf_alloc) {
...
int hns3_buffer_alloc(struct hns3_hw *hw) {
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "could not calc tx buffer size for all TCs %d", ret);
    return ret;
  }
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "could not calc rx priv buffer size for all TCs %d", ret);
    return ret;
  }
  ...
  if (hns3_dev_get_support(hw, DCB)) {
    ...
    if (ret) {
      PMD_INIT_LOG(ERR, "could not configure rx private waterline %d", ret);
      return ret;
    }

    ret = hns3_common_thrd_config(hw, &pkt_buf);
    if (ret) {
      PMD_INIT_LOG(ERR, "could not configure common threshold %d", ret);
      return ret;
    }
  }
  ...
  if (ret)
    PMD_INIT_LOG(ERR, "could not configure common waterline %d", ret);

  return ret;
}

static int hns3_mac_init(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_mac *mac = &hw->mac;
  struct hns3_pf *pf = &hns->pf;
  int ret;
...
static int hns3_get_mac_ethertype_cmd_status(uint16_t cmdq_resp,
                                             uint8_t resp_code) {
...
#define HNS3_ETHERTYPE_KEY_CONFLICT 3
  ...
  case HNS3_ETHERTYPE_MGR_TBL_OVERFLOW:
    PMD_INIT_LOG(ERR, "add mac ethertype failed for manager table overflow.");
    return_status = -EIO;
    break;
  case HNS3_ETHERTYPE_KEY_CONFLICT:
    ...
    break;
  default:
    PMD_INIT_LOG(ERR, "add mac ethertype failed for undefined, code=%u.",
                 resp_code);
    return_status = -EIO;
    break;
  }
...
static int hns3_add_mgr_tbl(struct hns3_hw *hw,
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret) {
    PMD_INIT_LOG(ERR, "add mac ethertype failed for cmd_send, ret =%d.", ret);
    return ret;
  }
...
static void hns3_prepare_mgr_tbl(struct hns3_mac_mgr_tbl_entry_cmd *mgr_table,
                                 int *table_item_num) {
...
static int hns3_init_mgr_tbl(struct hns3_hw *hw) {
#define HNS_MAC_MGR_TBL_MAX_SIZE 16
  ...
  for (i = 0; i < table_item_num; i++) {
    ...
    if (ret) {
      PMD_INIT_LOG(ERR, "add mac ethertype failed, ret =%d", ret);
      return ret;
    }
  }
...
static void hns3_promisc_param_init(struct hns3_promisc_param *param,
...
static int hns3_cmd_set_promisc_mode(struct hns3_hw *hw,
  ...
  req->flag = (param->enable << HNS3_PROMISC_EN_B) | HNS3_PROMISC_TX_EN_B |
              HNS3_PROMISC_RX_EN_B;
...
static int hns3_set_promisc_mode(struct hns3_hw *hw, bool en_uc_pmc,
                                 bool en_mc_pmc) {
  struct hns3_promisc_param param;
  bool en_bc_pmc = true;
  uint8_t vf_id;
...
static int hns3_promisc_init(struct hns3_hw *hw) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  struct hns3_pf *pf = &hns->pf;
  struct hns3_promisc_param param;
  uint16_t func_id;
  ...
  for (func_id = HNS3_1ST_VF_FUNC_ID; func_id < pf->func_num; func_id++) {
    ...
    if (ret) {
      PMD_INIT_LOG(ERR,
                   "failed to clear vf:%u promisc mode,"
                   " ret = %d",
                   func_id, ret);
      return ret;
    }
...
static void hns3_promisc_uninit(struct hns3_hw *hw) {
  struct hns3_promisc_param param;
  uint16_t func_id;
  int ret;
  ...
  if (ret)
    PMD_INIT_LOG(ERR,
                 "failed to clear promisc status during"
                 " uninit, ret = %d",
                 ret);
}

static int hns3_dev_promiscuous_enable(struct rte_eth_dev *dev) {
  ...
  if (ret) {
    rte_spinlock_unlock(&hw->lock);
    hns3_err(hw, "failed to enable promiscuous mode, ret = %d", ret);
    return ret;
  }
...
static int hns3_dev_promiscuous_disable(struct rte_eth_dev *dev) {
  bool allmulti = dev->data->all_multicast ? true : false;
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  uint64_t offloads;
  ...
  if (ret) {
    rte_spinlock_unlock(&hw->lock);
    hns3_err(hw, "failed to disable promiscuous mode, ret = %d", ret);
    return ret;
  }
  ...
  if (offloads & RTE_ETH_RX_OFFLOAD_VLAN_FILTER) {
    ...
    if (ret) {
      hns3_err(hw,
               "failed to disable promiscuous mode due to"
...
static int hns3_dev_allmulticast_enable(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
  ...
  rte_spinlock_unlock(&hw->lock);
  if (ret)
    hns3_err(hw, "failed to enable allmulticast mode, ret = %d", ret);

  return ret;
}

static int hns3_dev_allmulticast_disable(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
  ...
  rte_spinlock_unlock(&hw->lock);
  if (ret)
    hns3_err(hw, "failed to disable allmulticast mode, ret = %d", ret);

  return ret;
}

static int hns3_dev_promisc_restore(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  ...
  if (hw->data->promiscuous) {
    ...
    if (ret)
      hns3_err(hw,
               "failed to restore promiscuous mode, "
               "ret = %d",
               ret);
    return ret;
  }
  ...
  ret = hns3_set_promisc_mode(hw, false, allmulti);
  if (ret)
    hns3_err(hw, "failed to restore allmulticast mode, ret = %d", ret);
  return ret;
}

static int hns3_get_sfp_info(struct hns3_hw *hw, struct hns3_mac *mac_info) {
  ...
  if (ret == -EOPNOTSUPP) {
    hns3_warn(hw,
              "firmware does not support get SFP info,"
              " ret = %d.",
              ret);
    return ret;
  } else if (ret) {
  ...
  if (resp->supported_speed) {
    ...
    mac_info->supported_speed = rte_le_to_cpu_32(resp->supported_speed);
    mac_info->support_autoneg = resp->autoneg_ability;
    mac_info->link_autoneg =
        (resp->autoneg == 0) ? RTE_ETH_LINK_FIXED : RTE_ETH_LINK_AUTONEG;
  } else {
...
static uint8_t hns3_check_speed_dup(uint8_t duplex, uint32_t speed) {
...
static int hns3_cfg_mac_speed_dup(struct hns3_hw *hw, uint32_t speed,
                                  uint8_t duplex) {
  struct hns3_mac *mac = &hw->mac;
  int ret;
...
static int hns3_update_fiber_link_info(struct hns3_hw *hw) {
...
static void hns3_parse_copper_phy_params(struct hns3_cmd_desc *desc,
                                         struct hns3_mac *mac) {
#define HNS3_PHY_SUPPORTED_SPEED_MASK 0x2f
  ...
  mac->link_duplex = hns3_get_bit(req->duplex, HNS3_PHY_DUPLEX_CFG_B);
  mac->link_autoneg = hns3_get_bit(req->autoneg, HNS3_PHY_AUTONEG_CFG_B);
...
static int hns3_get_copper_phy_params(struct hns3_hw *hw,
                                      struct hns3_mac *mac) {
...
static int hns3_update_copper_link_info(struct hns3_hw *hw) {
  struct hns3_mac *mac = &hw->mac;
  struct hns3_mac mac_info;
  int ret;
...
static int hns3_get_mac_link_status(struct hns3_hw *hw) {
...
static void hns3_service_handler(void *param) {
...
static int hns3_init_hardware(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "Failed to init promisc: %d", ret);
    goto err_mac_init;
  }
...
static int hns3_clear_hw(struct hns3_hw *hw) {
  struct hns3_cmd_desc desc;
  int ret;
...
static void hns3_config_all_msix_error(struct hns3_hw *hw, bool enable) {
  uint32_t val;
...
static uint32_t hns3_set_firber_default_support_speed(struct hns3_hw *hw) {
  struct hns3_mac *mac = &hw->mac;
 ...
 *       supported_speed.
 */
static int hns3_get_port_supported_speed(struct rte_eth_dev *eth_dev) {
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  struct hns3_mac *mac = &hw->mac;
  int ret;
  ...
  if (mac->media_type == HNS3_MEDIA_TYPE_FIBER) {
    ...
    if (mac->supported_speed == 0)
      mac->supported_speed = hns3_set_firber_default_support_speed(hw);
  }

  return 0;
}

static void hns3_get_fc_autoneg_capability(struct hns3_adapter *hns) {
...
static int hns3_init_pf(struct rte_eth_dev *eth_dev) {
  ...
  ret = rte_intr_callback_register(pci_dev->intr_handle, hns3_interrupt_handler,
                                   eth_dev);
  if (ret) {
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "fail to enable hw error interrupts: %d", ret);
  ...
  if (ret) {
    PMD_INIT_LOG(ERR,
                 "failed to get speed capabilities supported "
                 "by device, ret = %d.",
                 ret);
...
err_get_config:
  ...
  hns3_intr_unregister(pci_dev->intr_handle, hns3_interrupt_handler, eth_dev);
...
static void hns3_uninit_pf(struct rte_eth_dev *eth_dev) {
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct rte_device *dev = eth_dev->device;
  struct rte_pci_device *pci_dev = RTE_DEV_TO_PCI(dev);
  struct hns3_hw *hw = &hns->hw;
...
static uint32_t hns3_convert_link_speeds2bitmap_copper(uint32_t link_speeds) {
  uint32_t speed_bit;
...
static uint32_t hns3_convert_link_speeds2bitmap_fiber(uint32_t link_speeds) {
  uint32_t speed_bit;
...
static int hns3_check_port_speed(struct hns3_hw *hw, uint32_t link_speeds) {
  ...
  if (!(speed_bit & supported_speed)) {
    hns3_err(hw,
             "link_speeds(0x%x) exceeds the supported speed capability or is "
             "incorrect.",
             link_speeds);
    return -EINVAL;
  }
...
static uint32_t hns3_get_link_speed(uint32_t link_speeds) {
...
static uint8_t hns3_get_link_duplex(uint32_t link_speeds) {
...
hns3_set_copper_port_link_speed(struct hns3_hw *hw,
  ...
  if (cfg->autoneg) {
    req->advertising =
        HNS3_PHY_LINK_SPEED_10M_BIT | HNS3_PHY_LINK_SPEED_10M_HD_BIT |
...
static int hns3_set_autoneg(struct hns3_hw *hw, bool enable) {
...
static int hns3_set_fiber_port_link_speed(struct hns3_hw *hw,
                                          struct hns3_set_link_speed_cfg *cfg) {
  int ret;
  ...
  if (cfg->autoneg) {
    hns3_warn(hw,
              "auto-negotiation is not supported, use default fixed speed!");
    return 0;
  }
...
static int hns3_set_port_link_speed(struct hns3_hw *hw,
  ...
  if (hw->mac.media_type == HNS3_MEDIA_TYPE_COPPER) {
    ...
    if (ret) {
      hns3_err(hw,
               "failed to set copper port link speed,"
               "ret = %d.",
               ret);
      return ret;
    }
    ...
    if (ret) {
      hns3_err(hw,
               "failed to set fiber port link speed,"
               "ret = %d.",
               ret);
      return ret;
    }
...
static int hns3_apply_link_speed(struct hns3_hw *hw) {
...
static int hns3_do_start(struct hns3_adapter *hns, bool reset_queue) {
  struct hns3_hw *hw = &hns->hw;
  bool link_en;
  int ret;
  ...
  if (ret) {
    hns3_err(hw, "failed to update queue mapping configuration, ret = %d", ret);
    return ret;
  }
...
static void hns3_restore_filter(struct rte_eth_dev *dev) {
  hns3_restore_rss_filter(dev);
}

static int hns3_dev_start(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
...
static int hns3_dev_stop(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
...
static int hns3_dev_close(struct rte_eth_dev *eth_dev) {
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret = 0;
...
static void hns3_get_autoneg_rxtx_pause_copper(struct hns3_hw *hw,
...
static enum hns3_fc_mode hns3_get_autoneg_fc_mode(struct hns3_hw *hw) {
  ...
  case HNS3_MEDIA_TYPE_FIBER:
  case HNS3_MEDIA_TYPE_BACKPLANE:
    hns3_err(hw, "autoneg FC mode can't be obtained, but flow control "
                 "auto-negotiation is enabled.");
    current_mode = hw->requested_fc_mode;
    goto out;
  default:
    hns3_err(hw,
             "autoneg FC mode can't be obtained for unknown media type(%u).",
             hw->mac.media_type);
...
static enum hns3_fc_mode hns3_get_current_fc_mode(struct rte_eth_dev *dev) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct hns3_pf *pf = HNS3_DEV_PRIVATE_TO_PF(dev->data->dev_private);
  struct hns3_mac *mac = &hw->mac;
...
int hns3_flow_ctrl_get(struct rte_eth_dev *dev,
...
static int hns3_check_fc_autoneg_valid(struct hns3_hw *hw, uint8_t autoneg) {
  struct hns3_pf *pf = HNS3_DEV_HW_TO_PF(hw);
  ...
  if (!pf->support_fc_autoneg) {
    ...
    if (autoneg != hw->mac.link_autoneg) {
      hns3_err(hw, "please use 'link_speeds' in struct rte_eth_conf to disable "
                   "autoneg!");
      return -EOPNOTSUPP;
    }
  ...
  if (autoneg != hw->mac.link_autoneg) {
    hns3_err(
        hw,
        "please use 'link_speeds' in struct rte_eth_conf to change autoneg!");
    return -EOPNOTSUPP;
  }
...
static int hns3_flow_ctrl_set(struct rte_eth_dev *dev,
                              struct rte_eth_fc_conf *fc_conf) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct hns3_pf *pf = HNS3_DEV_PRIVATE_TO_PF(dev->data->dev_private);
  int ret;

  if (fc_conf->high_water || fc_conf->low_water || fc_conf->send_xon ||
    ...
    hns3_err(hw,
             ...
             fc_conf->high_water, fc_conf->low_water, fc_conf->send_xon,
             fc_conf->mac_ctrl_frame_fwd);
    return -EINVAL;
  }
  ...
  if (!(hw->current_fc_status == HNS3_FC_STATUS_NONE ||
    ...
    hns3_err(hw,
             "PFC is enabled. Cannot set MAC pause. "
             "current_fc_status = %d",
...
static int hns3_priority_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  if (pfc_conf->fc.pause_time == 0) {
    hns3_err(hw, "Invalid pause time %u setting.", pfc_conf->fc.pause_time);
    return -EINVAL;
  }
  ...
  if (!(hw->current_fc_status == HNS3_FC_STATUS_NONE ||
    ...
    hns3_err(hw,
             "MAC pause is enabled. Cannot set PFC."
             "current_fc_status = %d",
             hw->current_fc_status);
    return -EOPNOTSUPP;
  }
...
static int hns3_get_dcb_info(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < hw->num_tc; i++) {
    ...
    dcb_info->tc_queue.tc_txq[0][i].base = hw->tc_queue[i].tqp_offset;
    dcb_info->tc_queue.tc_rxq[0][i].nb_queue = hw->alloc_rss_size;
    dcb_info->tc_queue.tc_txq[0][i].nb_queue = hw->tc_queue[i].tqp_count;
  }
  rte_spinlock_unlock(&hw->lock);
...
static int hns3_reinit_dev(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;
  ...
  ret = hns3_enable_hw_error_intr(hns, true);
  if (ret) {
    hns3_err(hw, "fail to enable hw error interrupts: %d", ret);
    return ret;
  }
...
static bool is_pf_reset_done(struct hns3_hw *hw) {
  ...
  case HNS3_FLR_RESET:
  default:
    hns3_err(hw, "Wait for unsupported reset level: %d", hw->reset.level);
    return true;
  }
...
static int hns3_wait_hardware_ready(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  ...
  wait_data->end_ms = (uint64_t)HNS3_RESET_WAIT_CNT * HNS3_RESET_WAIT_MS +
                      hns3_clock_gettime_ms();
...
static int hns3_func_reset_cmd(struct hns3_hw *hw, int func_id) {
  struct hns3_cmd_desc desc;
...
static int hns3_imp_reset_cmd(struct hns3_hw *hw) {
  struct hns3_cmd_desc desc;
...
static void hns3_msix_process(struct hns3_adapter *hns,
  ...
  case HNS3_IMP_RESET:
    ...
    hns3_warn(hw, "IMP Reset requested time=%ld.%.6ld", tv.tv_sec, tv.tv_usec);
    break;
  case HNS3_GLOBAL_RESET:
    ...
    hns3_warn(hw, "Global Reset requested time=%ld.%.6ld", tv.tv_sec,
              tv.tv_usec);
    break;
  case HNS3_FUNC_RESET:
    hns3_warn(hw, "PF Reset requested time=%ld.%.6ld", tv.tv_sec, tv.tv_usec);
...
static void hns3_record_imp_error(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  uint32_t reg_val;
...
static int hns3_prepare_reset(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  uint32_t reg_val;
  int ret;
  ...
  case HNS3_IMP_RESET:
    ...
    hns3_write_dev(hw, HNS3_VECTOR0_OTER_EN_REG,
                   reg_val | BIT(HNS3_VECTOR0_IMP_RESET_INT_B));
    break;
  default:
    break;
  }
...
static int hns3_set_rst_done(struct hns3_hw *hw) {
...
static int hns3_stop_service(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  struct rte_eth_dev *eth_dev;
...
static int hns3_start_service(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  struct rte_eth_dev *eth_dev;

  if (hw->reset.level == HNS3_IMP_RESET || hw->reset.level == HNS3_GLOBAL_RESET)
  ...
  if (hw->adapter_state == HNS3_NIC_STARTED) {
     ...
     */
    rte_eal_alarm_set(HNS3_SERVICE_QUICK_INTERVAL, hns3_service_handler,
                      eth_dev);
...
static int hns3_restore_conf(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static void hns3_reset_service(void *param) {
  ...
  if (__atomic_load_n(&hw->reset.schedule, __ATOMIC_RELAXED) ==
      SCHEDULE_DEFERRED) {
    __atomic_store_n(&hw->reset.schedule, SCHEDULE_REQUESTED, __ATOMIC_RELAXED);
...
static unsigned int hns3_get_speed_capa_num(uint16_t device_id) {
  unsigned int num;
...
static int hns3_get_speed_fec_capa(struct rte_eth_fec_capa *speed_fec_capa,
                                   uint16_t device_id) {
  switch (device_id) {
  case HNS3_DEV_ID_25GE:
  /* fallthrough */
...
static int hns3_fec_get_capability(struct rte_eth_dev *dev,
  ...
  if (capa_num == 0) {
    hns3_err(hw, "device(0x%x) is not supported by hns3 PMD", device_id);
    return -ENOTSUP;
  }
...
static int get_current_fec_auto_state(struct hns3_hw *hw, uint8_t *state) {
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret) {
    hns3_err(hw, "get current fec auto state failed, ret = %d", ret);
    return ret;
  }
...
static int hns3_fec_get_internal(struct hns3_hw *hw, uint32_t *fec_capa) {
  struct hns3_sfp_info_cmd *resp;
...
static int hns3_fec_get(struct rte_eth_dev *dev, uint32_t *fec_capa) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);

  return hns3_fec_get_internal(hw, fec_capa);
}

static int hns3_set_fec_hw(struct hns3_hw *hw, uint32_t mode) {
  struct hns3_config_fec_cmd *req;
  struct hns3_cmd_desc desc;
  int ret;
...
static uint32_t get_current_speed_fec_cap(struct hns3_hw *hw,
...
static bool is_fec_mode_one_bit_set(uint32_t mode) {
  int cnt = 0;
  uint8_t i;
...
static int hns3_fec_set(struct rte_eth_dev *dev, uint32_t mode) {
#define FEC_CAPA_NUM 2
  ...
  if (!is_fec_mode_one_bit_set(mode)) {
    hns3_err(hw,
             "FEC mode(0x%x) not supported in HNS3 PMD, "
             "FEC mode should be only one bit set",
             mode);
    return -EINVAL;
  }
...
static int hns3_restore_fec(struct hns3_hw *hw) {
  ...
  ret = hns3_set_fec_hw(hw, mode);
  if (ret)
    hns3_err(hw, "restore fec mode(0x%x) failed, ret = %d", mode, ret);
...
static bool hns3_optical_module_existed(struct hns3_hw *hw) {
  struct hns3_cmd_desc desc;
  bool existed;
  int ret;
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret) {
    hns3_err(hw, "fail to get optical module exist state, ret = %d.\n", ret);
    return false;
  }
  existed = !!desc.data[0];
...
static int hns3_get_module_eeprom_data(struct hns3_hw *hw, uint32_t offset,
                                       uint32_t len, uint8_t *data) {
#define HNS3_SFP_INFO_CMD_NUM 6
#define HNS3_SFP_INFO_MAX_LEN                                                  \
  (HNS3_SFP_INFO_BD0_LEN + (HNS3_SFP_INFO_CMD_NUM - 1) * HNS3_SFP_INFO_BDX_LEN)
  ...
  for (i = 0; i < HNS3_SFP_INFO_CMD_NUM; i++) {
    hns3_cmd_setup_basic_desc(&desc[i], HNS3_OPC_GET_SFP_EEPROM, true);
...
static int hns3_get_module_eeprom(struct rte_eth_dev *dev,
  ...
  while (read_len < len) {
    int ret;
    ret = hns3_get_module_eeprom_data(hw, offset + read_len, len - read_len,
                                      data + read_len);
    if (ret < 0)
      return -EIO;
    read_len += ret;
...
static int hns3_get_module_info(struct rte_eth_dev *dev,
...
static void hns3_init_hw_ops(struct hns3_hw *hw) {
...
static int hns3_dev_init(struct rte_eth_dev *eth_dev) {
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int hns3_dev_uninit(struct rte_eth_dev *eth_dev) {
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
...
static int eth_hns3_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                              struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct hns3_adapter),
                                       hns3_dev_init);
}

static int eth_hns3_pci_remove(struct rte_pci_device *pci_dev) {
