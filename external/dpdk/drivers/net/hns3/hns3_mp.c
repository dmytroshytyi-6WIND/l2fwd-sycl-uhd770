 ...
 *   Message type.
 */
static inline void mp_init_msg(struct rte_eth_dev *dev, struct rte_mp_msg *msg,
 ...
 */
static int mp_primary_handle(const struct rte_mp_msg *mp_msg __rte_unused,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mp_secondary_handle(const struct rte_mp_msg *mp_msg,
                               const void *peer) {
  ...
  case HNS3_MP_REQ_START_RXTX:
    PMD_INIT_LOG(INFO, "port %u starting datapath", dev->data->port_id);
    hns3_set_rxtx_function(dev);
    break;
  case HNS3_MP_REQ_STOP_RXTX:
    PMD_INIT_LOG(INFO, "port %u stopping datapath", dev->data->port_id);
    hns3_set_rxtx_function(dev);
    break;
  case HNS3_MP_REQ_START_TX:
    PMD_INIT_LOG(INFO, "port %u starting Tx datapath", dev->data->port_id);
    hns3_start_tx_datapath(dev);
    break;
  case HNS3_MP_REQ_STOP_TX:
    PMD_INIT_LOG(INFO, "port %u stopping Tx datapath", dev->data->port_id);
    hns3_stop_tx_datapath(dev);
    break;
  default:
    rte_errno = EINVAL;
    PMD_INIT_LOG(ERR, "port %u invalid mp request type", dev->data->port_id);
    return -rte_errno;
  }
...
static bool mp_req_type_is_valid(enum hns3_mp_req_type type) {
  return type == HNS3_MP_REQ_START_RXTX || type == HNS3_MP_REQ_STOP_RXTX ||
         type == HNS3_MP_REQ_START_TX || type == HNS3_MP_REQ_STOP_TX;
}
 ...
 *   Request type.
 */
static void mp_req_on_rxtx(struct rte_eth_dev *dev,
                           enum hns3_mp_req_type type) {
  ...
  if (!mp_req_type_is_valid(type)) {
    hns3_err(hw, "port %u unknown request (req_type %d)", dev->data->port_id,
             type);
    return;
  }
  ...
  if (mp_rep.nb_sent != mp_rep.nb_received) {
    PMD_INIT_LOG(ERR, "port %u not all secondaries responded (req_type %d)",
                 dev->data->port_id, type);
    goto exit;
  }
 ...
 */
static int hns3_mp_init_primary(void) {
  int ret;
 ...
 */
static int hns3_mp_init_secondary(void) {
  int ret;
...
int hns3_mp_init(struct rte_eth_dev *dev) {
  ...
  if (rte_eal_process_type() != RTE_PROC_PRIMARY) {
    ...
    if (ret) {
      PMD_INIT_LOG(ERR, "Failed to init for secondary process, ret = %d", ret);
      return ret;
    }
    ...
    if (ret) {
      PMD_INIT_LOG(ERR, "Failed to init for primary process, ret = %d", ret);
      return ret;
    }
  }
