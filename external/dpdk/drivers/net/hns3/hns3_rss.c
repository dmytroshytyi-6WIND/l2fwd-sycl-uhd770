...
#define HNS3_SET_HASH_KEY_BYTE_FOUR 2
  ...
  for (key_offset = 0; key_offset < HNS3_KEY_OFFSET_MAX; key_offset++) {
    ...
    if (key_offset == HNS3_SET_HASH_KEY_BYTE_FOUR)
      key_size = HNS3_RSS_KEY_SIZE -
                 HNS3_RSS_HASH_KEY_NUM * HNS3_SET_HASH_KEY_BYTE_FOUR;
    else
 ...
 */
static int hns3_rss_set_input_tuple(struct hns3_hw *hw) {
...
int hns3_set_rss_indir_table(struct hns3_hw *hw, uint16_t *indir,
  ...
  memcpy(hw->rss_info.rss_indirection_tbl, indir, sizeof(uint16_t) * size);
...
int hns3_set_rss_tuple_by_rss_hf(struct hns3_hw *hw,
  ...
  for (i = 0; i < RTE_DIM(hns3_set_tuple_table); i++) {
    if ((rss_hf & hns3_set_tuple_table[i].rss_types) ==
      ...
      req->tuple_field |= rte_cpu_to_le_64(hns3_set_tuple_table[i].rss_field);
      fields_count++;
    }
  }
  ...
  if (fields_count == 0) {
    for (i = 0; i < RTE_DIM(hns3_set_rss_types); i++) {
      if ((rss_hf & hns3_set_rss_types[i].rss_types) ==
        ...
        req->tuple_field |= rte_cpu_to_le_64(hns3_set_rss_types[i].rss_field);
    }
  }
...
int hns3_dev_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (key) {
    if (key_len != HNS3_RSS_KEY_SIZE) {
      hns3_err(hw, "The hash key len(%u) is invalid", key_len);
      ret = -EINVAL;
      goto conf_err;
    }
...
int hns3_dev_rss_reta_query(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size; i++) {
    ...
    shift = i % RTE_ETH_RETA_GROUP_SIZE;
    if (reta_conf[idx].mask & (1ULL << shift))
      reta_conf[idx].reta[shift] = rss_cfg->rss_indirection_tbl[i];
  }
  rte_spinlock_unlock(&hw->lock);
  return 0;
}
