...
static int hns3vf_set_bus_master(const struct rte_pci_device *device, bool op) {
  uint16_t reg;
  int ret;

  ret = rte_pci_read_config(device, &reg, sizeof(reg), PCI_COMMAND);
  if (ret < 0) {
    PMD_INIT_LOG(ERR, "Failed to read PCI offset 0x%x", PCI_COMMAND);
    return ret;
  }
 ...
 */
static int hns3vf_find_pci_capability(const struct rte_pci_device *device,
                                      int cap) {
#define MAX_PCIE_CAPABILITY 48
  ...
  ret = rte_pci_read_config(device, &pos, sizeof(pos), PCI_CAPABILITY_LIST);
  if (ret < 0) {
    PMD_INIT_LOG(ERR, "Failed to read PCI offset 0x%x", PCI_CAPABILITY_LIST);
    return 0;
  }

  while (ttl-- && pos >= PCI_STD_HEADER_SIZEOF) {
    ret = rte_pci_read_config(device, &id, sizeof(id), (pos + PCI_CAP_LIST_ID));
    if (ret < 0) {
...
static int hns3vf_enable_msix(const struct rte_pci_device *device, bool op) {
  uint16_t control;
  int pos;
  int ret;
...
static int hns3vf_add_uc_mac_addr(struct hns3_hw *hw,
                                  struct rte_ether_addr *mac_addr) {
  /* mac address was checked by upper level interface */
  char mac_str[RTE_ETHER_ADDR_FMT_SIZE];
  int ret;

  ret = hns3_send_mbx_msg(hw, HNS3_MBX_SET_UNICAST, HNS3_MBX_MAC_VLAN_UC_ADD,
                          mac_addr->addr_bytes, RTE_ETHER_ADDR_LEN, false, NULL,
                          0);
  if (ret) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
    hns3_err(hw, "failed to add uc mac addr(%s), ret = %d", mac_str, ret);
  }
  return ret;
}

static int hns3vf_remove_uc_mac_addr(struct hns3_hw *hw,
                                     struct rte_ether_addr *mac_addr) {
  /* mac address was checked by upper level interface */
  char mac_str[RTE_ETHER_ADDR_FMT_SIZE];
  int ret;

  ret = hns3_send_mbx_msg(hw, HNS3_MBX_SET_UNICAST, HNS3_MBX_MAC_VLAN_UC_REMOVE,
                          mac_addr->addr_bytes, RTE_ETHER_ADDR_LEN, false, NULL,
                          0);
  if (ret) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
...
static int hns3vf_set_default_mac_addr(struct rte_eth_dev *dev,
                                       struct rte_ether_addr *mac_addr) {
#define HNS3_TWO_ETHER_ADDR_LEN (RTE_ETHER_ADDR_LEN * 2)
  ...
  ret = hns3_send_mbx_msg(hw, HNS3_MBX_SET_UNICAST, HNS3_MBX_MAC_VLAN_UC_MODIFY,
                          addr_bytes, HNS3_TWO_ETHER_ADDR_LEN, true, NULL, 0);
  if (ret) {
    /*
     ...
     */
    if (ret == -EPERM) {
      hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, old_addr);
      hns3_warn(hw, "Has permanent mac addr(%s) for vf", mac_str);
    } else {
      hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
      hns3_err(hw, "Failed to set mac addr(%s) for vf: %d", mac_str, ret);
    }
  }
...
static int hns3vf_add_mc_mac_addr(struct hns3_hw *hw,
  ...
  ret = hns3_send_mbx_msg(hw, HNS3_MBX_SET_MULTICAST, HNS3_MBX_MAC_VLAN_MC_ADD,
                          mac_addr->addr_bytes, RTE_ETHER_ADDR_LEN, false, NULL,
                          0);
  if (ret) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
    hns3_err(hw, "Failed to add mc mac addr(%s) for vf: %d", mac_str, ret);
  }
...
static int hns3vf_remove_mc_mac_addr(struct hns3_hw *hw,
  ...
  ret = hns3_send_mbx_msg(hw, HNS3_MBX_SET_MULTICAST,
                          HNS3_MBX_MAC_VLAN_MC_REMOVE, mac_addr->addr_bytes,
                          RTE_ETHER_ADDR_LEN, false, NULL, 0);
  if (ret) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
    hns3_err(hw, "Failed to remove mc mac addr(%s) for vf: %d", mac_str, ret);
  }

  return ret;
}

static int hns3vf_set_promisc_mode(struct hns3_hw *hw, bool en_bc_pmc,
...
static int hns3vf_dev_promiscuous_enable(struct rte_eth_dev *dev) {
...
static int hns3vf_dev_promiscuous_disable(struct rte_eth_dev *dev) {
...
static int hns3vf_dev_allmulticast_enable(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int hns3vf_dev_allmulticast_disable(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int hns3vf_restore_promisc(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
...
static int hns3vf_bind_ring_with_vector(struct hns3_hw *hw, uint16_t vector_id,
                                        bool mmap,
  ...
  code = mmap ? HNS3_MBX_MAP_RING_TO_VECTOR : HNS3_MBX_UNMAP_RING_TO_VECTOR;
  ...
  ret = hns3_send_mbx_msg(hw, code, 0, (uint8_t *)&bind_msg, sizeof(bind_msg),
                          false, NULL, 0);
  if (ret)
    hns3_err(hw, "%s TQP %u fail, vector_id is %u, ret is %d.", op_str,
             queue_id, bind_msg.vector_id, ret);

  return ret;
}

static int hns3vf_dev_configure(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
...
static int hns3vf_config_mtu(struct hns3_hw *hw, uint16_t mtu) {
  int ret;
...
static void hns3vf_clear_event_cause(struct hns3_hw *hw, uint32_t regclr) {
  hns3_write_dev(hw, HNS3_VECTOR0_CMDQ_SRC_REG, regclr);
}

static void hns3vf_disable_irq0(struct hns3_hw *hw) {
  hns3_write_dev(hw, HNS3_MISC_VECTOR_REG_BASE, 0);
}

static void hns3vf_enable_irq0(struct hns3_hw *hw) {
  hns3_write_dev(hw, HNS3_MISC_VECTOR_REG_BASE, 1);
}

static enum hns3vf_evt_cause hns3vf_check_event_cause(struct hns3_adapter *hns,
                                                      uint32_t *clearval) {
  struct hns3_hw *hw = &hns->hw;
...
static void hns3vf_interrupt_handler(void *param) {
...
static void hns3vf_set_default_dev_specifications(struct hns3_hw *hw) {
...
static void hns3vf_parse_dev_specifications(struct hns3_hw *hw,
                                            struct hns3_cmd_desc *desc) {
...
static int hns3vf_check_dev_specifications(struct hns3_hw *hw) {
...
static int hns3vf_query_dev_specifications(struct hns3_hw *hw) {
...
static void hns3vf_get_push_lsc_cap(struct hns3_hw *hw) {
#define HNS3_CHECK_PUSH_LSC_CAP_TIMEOUT_MS 500
  ...
  (void)hns3_send_mbx_msg(hw, HNS3_MBX_GET_LINK_STATUS, 0, NULL, 0, false, NULL,
                          0);
...
static int hns3vf_get_port_base_vlan_filter_state(struct hns3_hw *hw) {
  uint8_t resp_msg;
  int ret;

  ret = hns3_send_mbx_msg(hw, HNS3_MBX_SET_VLAN,
                          HNS3_MBX_GET_PORT_BASE_VLAN_STATE, NULL, 0, true,
  ...
  if (ret) {
    if (ret == -ETIME) {
       ...
       * by other functions.
       */
      PMD_INIT_LOG(WARNING, "failed to get PVID state for timeout, maybe "
                            "kernel side PF driver doesn't support this "
                            "mailbox message, or firmware didn't respond.");
      resp_msg = HNS3_PORT_BASE_VLAN_DISABLE;
    } else {
      PMD_INIT_LOG(ERR,
                   "failed to get port based VLAN state,"
                   " ret = %d",
                   ret);
      return ret;
    }
  }
  hw->port_base_vlan_cfg.state =
      resp_msg ? HNS3_PORT_BASE_VLAN_ENABLE : HNS3_PORT_BASE_VLAN_DISABLE;
  return 0;
}

static int hns3vf_get_queue_info(struct hns3_hw *hw) {
#define HNS3VF_TQPS_RSS_INFO_LEN 6
  uint8_t resp_msg[HNS3VF_TQPS_RSS_INFO_LEN];
  int ret;

  ret = hns3_send_mbx_msg(hw, HNS3_MBX_GET_QINFO, 0, NULL, 0, true, resp_msg,
                          HNS3VF_TQPS_RSS_INFO_LEN);
  if (ret) {
...
static void hns3vf_update_caps(struct hns3_hw *hw, uint32_t caps) {
  if (hns3_get_bit(caps, HNS3VF_CAPS_VLAN_FLT_MOD_B))
    hns3_set_bit(hw->capability, HNS3_DEV_SUPPORT_VF_VLAN_FLT_MOD_B, 1);
}

static int hns3vf_get_num_tc(struct hns3_hw *hw) {
  uint8_t num_tc = 0;
  uint32_t i;
...
static int hns3vf_get_basic_info(struct hns3_hw *hw) {
  uint8_t resp_msg[HNS3_MBX_MAX_RESP_DATA_SIZE];
  struct hns3_basic_info *basic_info;
  int ret;

  ret = hns3_send_mbx_msg(hw, HNS3_MBX_GET_BASIC_INFO, 0, NULL, 0, true,
                          resp_msg, sizeof(resp_msg));
  if (ret) {
    hns3_err(hw, "failed to get basic info from PF, ret = %d.", ret);
    return ret;
  }
...
static int hns3vf_get_host_mac_addr(struct hns3_hw *hw) {
  uint8_t host_mac[RTE_ETHER_ADDR_LEN];
  int ret;

  ret = hns3_send_mbx_msg(hw, HNS3_MBX_GET_MAC_ADDR, 0, NULL, 0, true, host_mac,
                          RTE_ETHER_ADDR_LEN);
  if (ret) {
...
static int hns3vf_get_configuration(struct hns3_hw *hw) {
  int ret;
...
static void hns3vf_request_link_info(struct hns3_hw *hw) {
  struct hns3_vf *vf = HNS3_DEV_HW_TO_VF(hw);
  bool send_req;
  int ret;
  ...
  ret = hns3_send_mbx_msg(hw, HNS3_MBX_GET_LINK_STATUS, 0, NULL, 0, false, NULL,
                          0);
  if (ret) {
...
static int hns3vf_vlan_filter_configure(struct hns3_adapter *hns,
...
#define HNS3VF_VLAN_MBX_MSG_LEN 5
  ...
  return hns3_send_mbx_msg(hw, HNS3_MBX_SET_VLAN, HNS3_MBX_VLAN_FILTER,
                           msg_data, HNS3VF_VLAN_MBX_MSG_LEN, true, NULL, 0);
}

static int hns3vf_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                  int on) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
  ...
  if (__atomic_load_n(&hw->reset.resetting, __ATOMIC_RELAXED)) {
    hns3_err(hw, "vf set vlan id failed during resetting, vlan_id =%u",
             vlan_id);
    return -EIO;
  }
  rte_spinlock_lock(&hw->lock);
  ...
  rte_spinlock_unlock(&hw->lock);
  if (ret)
    hns3_err(hw, "vf set vlan id failed, vlan_id =%u, ret =%d", vlan_id, ret);

  return ret;
}

static int hns3vf_en_vlan_filter(struct hns3_hw *hw, bool enable) {
  uint8_t msg_data;
  int ret;
  ...
  msg_data = enable ? 1 : 0;
  ret = hns3_send_mbx_msg(hw, HNS3_MBX_SET_VLAN, HNS3_MBX_ENABLE_VLAN_FILTER,
                          &msg_data, sizeof(msg_data), true, NULL, 0);
  if (ret)
...
static int hns3vf_en_hw_strip_rxvtag(struct hns3_hw *hw, bool enable) {
  uint8_t msg_data;
  int ret;
  ...
  if (ret)
    hns3_err(hw, "vf %s strip failed, ret = %d.", enable ? "enable" : "disable",
             ret);

  return ret;
}

static int hns3vf_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  if (__atomic_load_n(&hw->reset.resetting, __ATOMIC_RELAXED)) {
    hns3_err(hw,
             "vf set vlan offload failed during resetting, "
             "mask = 0x%x",
             mask);
    return -EIO;
  }
...
static int hns3vf_handle_all_vlan_table(struct hns3_adapter *hns, int on) {
  ...
  for (i = 0; i < RTE_DIM(vfc->ids); i++) {
    ...
    while (ids) {
      ...
      ret = hns3vf_vlan_filter_configure(hns, vlan_id, on);
      if (ret) {
        hns3_err(hw, "VF handle vlan table failed, ret =%d, on = %d", ret, on);
        return ret;
      }
    }
  }
...
static int hns3vf_remove_all_vlan_table(struct hns3_adapter *hns) {
  return hns3vf_handle_all_vlan_table(hns, 0);
}

static int hns3vf_restore_vlan_conf(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  ...
  en = dev_conf->rxmode.offloads & RTE_ETH_RX_OFFLOAD_VLAN_STRIP ? true : false;
  ret = hns3vf_en_hw_strip_rxvtag(hw, en);
  if (ret)
    hns3_err(hw, "VF restore vlan conf fail, en =%d, ret =%d", en, ret);
  return ret;
}
...
static int hns3vf_set_alive(struct hns3_hw *hw, bool alive) {
  uint8_t msg_data;
...
static void hns3vf_keep_alive_handler(void *param) {
  struct rte_eth_dev *eth_dev = (struct rte_eth_dev *)param;
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;

  ret = hns3_send_mbx_msg(hw, HNS3_MBX_KEEP_ALIVE, 0, NULL, 0, false, NULL, 0);
  if (ret)
    hns3_err(hw, "VF sends keeping alive cmd failed(=%d)", ret);

  rte_eal_alarm_set(HNS3VF_KEEP_ALIVE_INTERVAL, hns3vf_keep_alive_handler,
                    eth_dev);
}

static void hns3vf_service_handler(void *param) {
  struct rte_eth_dev *eth_dev = (struct rte_eth_dev *)param;
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  ...
  rte_eal_alarm_set(HNS3VF_SERVICE_INTERVAL, hns3vf_service_handler, eth_dev);
}

static void hns3vf_start_poll_job(struct rte_eth_dev *dev) {
...
static void hns3vf_stop_poll_job(struct rte_eth_dev *dev) {
  struct hns3_vf *vf = HNS3_DEV_PRIVATE_TO_VF(dev->data->dev_private);
...
static int hns3_query_vf_resource(struct hns3_hw *hw) {
  ...
  if (num_msi < HNS3_MIN_VECTOR_NUM) {
    hns3_err(hw, "Just %u msi resources, not enough for vf(min:%d)", num_msi,
             HNS3_MIN_VECTOR_NUM);
    return -EINVAL;
  }
...
static int hns3vf_init_hardware(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
...
static int hns3vf_clear_vport_list(struct hns3_hw *hw) {
  return hns3_send_mbx_msg(hw, HNS3_MBX_HANDLE_VF_TBL,
                           HNS3_MBX_VPORT_LIST_CLEAR, NULL, 0, false, NULL, 0);
}

static int hns3vf_init_vf(struct rte_eth_dev *eth_dev) {
...
err_get_config:
  ...
  hns3_intr_unregister(pci_dev->intr_handle, hns3vf_interrupt_handler, eth_dev);
...
static void hns3vf_uninit_vf(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
...
static int hns3vf_do_stop(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int hns3vf_dev_stop(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
...
static int hns3vf_dev_close(struct rte_eth_dev *eth_dev) {
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret = 0;
...
static int hns3vf_do_start(struct hns3_adapter *hns, bool reset_queue) {
  struct hns3_hw *hw = &hns->hw;
...
static void hns3vf_restore_filter(struct rte_eth_dev *dev) {
  hns3_restore_rss_filter(dev);
}

static int hns3vf_dev_start(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static bool is_vf_reset_done(struct hns3_hw *hw) {
#define HNS3_FUN_RST_ING_BITS                                                  \
  (BIT(HNS3_VECTOR0_GLOBALRESET_INT_B) | BIT(HNS3_VECTOR0_CORERESET_INT_B) |   \
   BIT(HNS3_VECTOR0_IMPRESET_INT_B) | BIT(HNS3_VECTOR0_FUNCRESET_INT_B))
...
static int hns3vf_wait_hardware_ready(struct hns3_adapter *hns) {
#define HNS3_WAIT_PF_RESET_READY_TIME 5
  ...
  wait_data->end_ms = (uint64_t)HNS3VF_RESET_WAIT_CNT * HNS3VF_RESET_WAIT_MS +
...
static int hns3vf_prepare_reset(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;

  if (hw->reset.level == HNS3_VF_FUNC_RESET) {
    ret = hns3_send_mbx_msg(hw, HNS3_MBX_RESET, 0, NULL, 0, true, NULL, 0);
    if (ret)
      return ret;
  }
  __atomic_store_n(&hw->reset.disable_cmd, 1, __ATOMIC_RELAXED);
...
static int hns3vf_stop_service(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
...
static int hns3vf_start_service(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  struct rte_eth_dev *eth_dev;
...
static int hns3vf_check_default_mac_change(struct hns3_hw *hw) {
  char mac_str[RTE_ETHER_ADDR_FMT_SIZE];
  ...
  if (ret) {
    ...
    if (!ret) {
      ...
      hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE,
                             &hw->data->mac_addrs[0]);
      hns3_warn(hw,
                "Default MAC address has been changed to:"
...
static int hns3vf_restore_conf(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static void hns3vf_reset_service(void *param) {
...
static int hns3vf_reinit_dev(struct hns3_adapter *hns) {
...
static void hns3vf_init_hw_ops(struct hns3_hw *hw) {
...
static int hns3vf_dev_init(struct rte_eth_dev *eth_dev) {
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int hns3vf_dev_uninit(struct rte_eth_dev *eth_dev) {
  struct hns3_adapter *hns = eth_dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
