...
static const uint32_t cmdq_reg_addrs[] = {
    HNS3_CMDQ_TX_ADDR_L_REG, HNS3_CMDQ_TX_ADDR_H_REG,   HNS3_CMDQ_TX_DEPTH_REG,
    HNS3_CMDQ_TX_TAIL_REG,   HNS3_CMDQ_TX_HEAD_REG,     HNS3_CMDQ_RX_ADDR_L_REG,
    HNS3_CMDQ_RX_ADDR_H_REG, HNS3_CMDQ_RX_DEPTH_REG,    HNS3_CMDQ_RX_TAIL_REG,
    HNS3_CMDQ_RX_HEAD_REG,   HNS3_VECTOR0_CMDQ_SRC_REG, HNS3_CMDQ_INTR_STS_REG,
    HNS3_CMDQ_INTR_EN_REG,   HNS3_CMDQ_INTR_GEN_REG};
...
static const uint32_t common_vf_reg_addrs[] = {
    HNS3_MISC_VECTOR_REG_BASE, HNS3_FUN_RST_ING, HNS3_GRO_EN_REG};

static const uint32_t ring_reg_addrs[] = {
    HNS3_RING_RX_BASEADDR_L_REG, HNS3_RING_RX_BASEADDR_H_REG,
    HNS3_RING_RX_BD_NUM_REG,     HNS3_RING_RX_BD_LEN_REG,
    HNS3_RING_RX_EN_REG,         HNS3_RING_RX_MERGE_EN_REG,
    HNS3_RING_RX_TAIL_REG,       HNS3_RING_RX_HEAD_REG,
    HNS3_RING_RX_FBDNUM_REG,     HNS3_RING_RX_OFFSET_REG,
    HNS3_RING_RX_FBD_OFFSET_REG, HNS3_RING_RX_STASH_REG,
    HNS3_RING_RX_BD_ERR_REG,     HNS3_RING_TX_BASEADDR_L_REG,
    HNS3_RING_TX_BASEADDR_H_REG, HNS3_RING_TX_BD_NUM_REG,
    HNS3_RING_TX_EN_REG,         HNS3_RING_TX_PRIORITY_REG,
    HNS3_RING_TX_TC_REG,         HNS3_RING_TX_MERGE_EN_REG,
    HNS3_RING_TX_TAIL_REG,       HNS3_RING_TX_HEAD_REG,
    HNS3_RING_TX_FBDNUM_REG,     HNS3_RING_TX_OFFSET_REG,
    HNS3_RING_TX_EBD_NUM_REG,    HNS3_RING_TX_EBD_OFFSET_REG,
    HNS3_RING_TX_BD_ERR_REG,     HNS3_RING_EN_REG};

static const uint32_t tqp_intr_reg_addrs[] = {
    HNS3_TQP_INTR_CTRL_REG, HNS3_TQP_INTR_GL0_REG, HNS3_TQP_INTR_GL1_REG,
    HNS3_TQP_INTR_GL2_REG, HNS3_TQP_INTR_RL_REG};

static const uint32_t hns3_dfx_reg_opcode_list[] = {
    HNS3_OPC_DFX_BIOS_COMMON_REG, HNS3_OPC_DFX_SSU_REG_0,
    HNS3_OPC_DFX_SSU_REG_1,       HNS3_OPC_DFX_IGU_EGU_REG,
    HNS3_OPC_DFX_RPU_REG_0,       HNS3_OPC_DFX_RPU_REG_1,
    HNS3_OPC_DFX_NCSI_REG,        HNS3_OPC_DFX_RTC_REG,
    HNS3_OPC_DFX_PPP_REG,         HNS3_OPC_DFX_RCB_REG,
    HNS3_OPC_DFX_TQP_REG,         HNS3_OPC_DFX_SSU_REG_2};

static int hns3_get_regs_num(struct hns3_hw *hw, uint32_t *regs_num_32_bit,
  ...
  if (ret) {
    hns3_err(hw, "Query register number cmd failed, ret = %d", ret);
    return ret;
  }
...
static int hns3_get_regs_length(struct hns3_hw *hw, uint32_t *length) {
  ...
  if (hns->is_vf)
    common_lines = sizeof(common_vf_reg_addrs) / REG_LEN_PER_LINE + 1;
  else
  ...
  if (!hns->is_vf) {
    ...
    if (ret) {
      hns3_err(hw,
               "fail to get the number of registers, "
               "ret = %d.",
               ret);
      return ret;
    }
    dfx_reg_lines = regs_num_32_bit * sizeof(uint32_t) / REG_LEN_PER_LINE + 1;
    dfx_reg_lines += regs_num_64_bit * sizeof(uint64_t) / REG_LEN_PER_LINE + 1;

    ret = hns3_get_dfx_reg_line(hw, &dfx_reg_lines);
    if (ret) {
      hns3_err(hw,
               "fail to get the number of dfx registers, "
               "ret = %d.",
               ret);
      return ret;
    }
...
static int hns3_get_32_bit_regs(struct hns3_hw *hw, uint32_t regs_num,
                                void *data) {
...
#define HNS3_32_BIT_DESC_NODATA_LEN 2
  ...
  desc =
      rte_zmalloc("hns3-32bit-regs", sizeof(struct hns3_cmd_desc) * cmd_num, 0);
  if (desc == NULL) {
    hns3_err(hw,
             "Failed to allocate %zx bytes needed to "
             "store 32bit regs",
  ...
  if (ret) {
    hns3_err(hw, "Query 32 bit register cmd failed, ret = %d", ret);
    rte_free(desc);
    return ret;
  }
  ...
  for (i = 0; i < cmd_num; i++) {
    if (i == 0) {
      ...
      n = HNS3_32_BIT_REG_RTN_DATANUM - HNS3_32_BIT_DESC_NODATA_LEN;
    } else {
...
static int hns3_get_64_bit_regs(struct hns3_hw *hw, uint32_t regs_num,
                                void *data) {
...
#define HNS3_64_BIT_DESC_NODATA_LEN 1
  ...
  desc =
      rte_zmalloc("hns3-64bit-regs", sizeof(struct hns3_cmd_desc) * cmd_num, 0);
  if (desc == NULL) {
    hns3_err(hw,
             "Failed to allocate %zx bytes needed to "
             "store 64bit regs",
  ...
  ret = hns3_cmd_send(hw, desc, cmd_num);
  if (ret) {
    hns3_err(hw, "Query 64 bit register cmd failed, ret = %d", ret);
    rte_free(desc);
    return ret;
  }
  ...
  for (i = 0; i < cmd_num; i++) {
    if (i == 0) {
      ...
      n = HNS3_64_BIT_REG_RTN_DATANUM - HNS3_64_BIT_DESC_NODATA_LEN;
    } else {
...
static int hns3_insert_reg_separator(int reg_num, uint32_t *data) {
  int separator_num;
  int i;
...
static int hns3_direct_access_regs(struct hns3_hw *hw, uint32_t *data) {
  struct hns3_adapter *hns = HNS3_DEV_HW_TO_ADAPTER(hw);
  ...
  for (j = 0; j < hw->tqps_num; j++) {
    ...
    for (i = 0; i < reg_num; i++)
      *data++ = hns3_read_dev(hw, ring_reg_addrs[i] + reg_offset);
    data += hns3_insert_reg_separator(reg_num, data);
  }
  ...
  for (j = 0; j < hw->intr_tqps_num; j++) {
    ...
    for (i = 0; i < reg_num; i++)
      *data++ = hns3_read_dev(hw, tqp_intr_reg_addrs[i] + reg_offset);
    data += hns3_insert_reg_separator(reg_num, data);
  }
  return data - origin_data_ptr;
}

static int hns3_get_dfx_reg_bd_num(struct hns3_hw *hw, uint32_t *bd_num_list,
                                   uint32_t list_size) {
...
static int hns3_dfx_reg_cmd_send(struct hns3_hw *hw, struct hns3_cmd_desc *desc,
  ...
  if (ret) {
    hns3_err(hw,
             "fail to query dfx registers, opcode = 0x%04X, "
             "ret = %d.\n",
             opcode, ret);
  }
...
static int hns3_dfx_reg_fetch_data(struct hns3_cmd_desc *desc, int bd_num,
                                   uint32_t *reg) {
  int desc_index;
  int reg_num;
  int index;
...
static int hns3_get_dfx_regs(struct hns3_hw *hw, void **data) {
  int opcode_num = RTE_DIM(hns3_dfx_reg_opcode_list);
...
#define HNS3_64_BIT_REG_SIZE (sizeof(uint64_t) / sizeof(uint32_t))
  ...
  data +=
      hns3_insert_reg_separator(regs_num_64_bit * HNS3_64_BIT_REG_SIZE, data);
