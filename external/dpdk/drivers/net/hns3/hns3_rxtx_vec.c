...
uint16_t hns3_xmit_pkts_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  while (nb_pkts) {
    ...
    ret = hns3_xmit_fixed_burst_vec(tx_queue, &tx_pkts[nb_tx], new_burst);
    nb_tx += ret;
    nb_pkts -= ret;
    if (ret < new_burst)
      break;
...
static inline void hns3_rxq_rearm_mbuf(struct hns3_rx_queue *rxq) {
#define REARM_LOOP_STEP_NUM 4
  ...
  for (i = 0; i < HNS3_DEFAULT_RXQ_REARM_THRESH; i += REARM_LOOP_STEP_NUM,
    ...
    if (likely(i < HNS3_DEFAULT_RXQ_REARM_THRESH - REARM_LOOP_STEP_NUM)) {
...
uint16_t hns3_recv_pkts_vec(void *__restrict rx_queue,
  ...
  if (likely(nb_pkts <= HNS3_DEFAULT_RX_BURST)) {
    pkt_err_mask = 0;
    nb_rx = hns3_recv_burst_vec(rxq, rx_pkts, nb_pkts, &pkt_err_mask);
  ...
  while (nb_pkts > 0) {
    ...
    pkt_err_mask = 0;
    ret = hns3_recv_burst_vec(rxq, &rx_pkts[nb_rx], n, &pkt_err_mask);
    nb_pkts -= ret;
...
static void hns3_rxq_vec_setup_rearm_data(struct hns3_rx_queue *rxq) {
  uintptr_t p;
...
static int hns3_rxq_vec_check(struct hns3_rx_queue *rxq, void *arg) {
  uint32_t min_vec_bds = HNS3_DEFAULT_RXQ_REARM_THRESH + HNS3_DEFAULT_RX_BURST;
...
int hns3_rx_check_vec_support(struct rte_eth_dev *dev) {
  ...
  uint64_t offloads_mask = RTE_ETH_RX_OFFLOAD_TCP_LRO | RTE_ETH_RX_OFFLOAD_VLAN;
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_pf *pf = &hns->pf;
