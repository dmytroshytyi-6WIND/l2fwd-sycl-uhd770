...
#define HNS3_TX_CKSUM_OFFLOAD_MASK                                             \
  (RTE_MBUF_F_TX_OUTER_UDP_CKSUM | RTE_MBUF_F_TX_OUTER_IP_CKSUM |              \
...
static inline void hns3_rx_set_cksum_flag(struct hns3_rx_queue *rxq,
                                          struct rte_mbuf *rxm,
                                          uint32_t l234_info) {
#define HNS3_RXD_CKSUM_ERR_MASK                                                \
  (BIT(HNS3_RXD_L3E_B) | BIT(HNS3_RXD_L4E_B) | BIT(HNS3_RXD_OL3E_B) |          \
   BIT(HNS3_RXD_OL4E_B))
...
static inline int hns3_handle_bdinfo(struct hns3_rx_queue *rxq,
                                     struct rte_mbuf *rxm,
                                     uint32_t bd_base_info,
                                     uint32_t l234_info) {
#define L2E_TRUNC_ERR_FLAG (BIT(HNS3_RXD_L2E_B) | BIT(HNS3_RXD_TRUNCATE_B))
...
static inline uint32_t hns3_rx_calc_ptype(struct hns3_rx_queue *rxq,
  ...
  if (rxq->ptype_en) {
    ptype = hns3_get_field(ol_info, HNS3_RXD_PTYPE_M, HNS3_RXD_PTYPE_S);
  ...
  if (unlikely(ptype_tbl->ol4table[ol4id]))
    return ptype_tbl->inner_l3table[l3id] | ptype_tbl->inner_l4table[l4id] |
 ...
 */
static inline void hns3_write_txq_tail_reg(struct hns3_tx_queue *txq,
                                           uint32_t value) {
  rte_io_wmb();
