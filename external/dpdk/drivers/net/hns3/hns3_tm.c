...
static inline uint32_t hns3_tm_max_tx_queues_get(struct rte_eth_dev *dev) {
  /*
...
static inline uint32_t hns3_tm_rate_convert_tm2firmware(uint64_t tm_rate) {
#define TM_TO_FIRMWARE_RATE_SCALE 125000
  /* tm rate unit is Bps, firmware rate is Mbps */
  return (uint32_t)(tm_rate / TM_TO_FIRMWARE_RATE_SCALE);
}

static int hns3_tm_capabilities_get(struct rte_eth_dev *dev,
...
hns3_tm_shaper_profile_param_check(struct rte_eth_dev *dev,
  ...
  if (profile->peak.rate > hns3_tm_rate_convert_firmware2tm(hw->max_tm_rate)) {
...
static int hns3_tm_shaper_profile_add(struct rte_eth_dev *dev,
                                      uint32_t shaper_profile_id,
                                      struct rte_tm_shaper_params *profile,
                                      struct rte_tm_error *error) {
  struct hns3_pf *pf = HNS3_DEV_PRIVATE_TO_PF(dev->data->dev_private);
  ...
  if (pf->tm_conf.nb_shaper_profile >= pf->tm_conf.nb_shaper_profile_max) {
  ...
  shaper_profile = rte_zmalloc("hns3_tm_shaper_profile",
                               sizeof(struct hns3_tm_shaper_profile), 0);
  ...
  TAILQ_INSERT_TAIL(&pf->tm_conf.shaper_profile_list, shaper_profile, node);
  pf->tm_conf.nb_shaper_profile++;

  return 0;
}

static int hns3_tm_shaper_profile_del(struct rte_eth_dev *dev,
                                      uint32_t shaper_profile_id,
                                      struct rte_tm_error *error) {
  struct hns3_pf *pf = HNS3_DEV_PRIVATE_TO_PF(dev->data->dev_private);
  struct hns3_tm_shaper_profile *shaper_profile;
...
hns3_tm_node_search(struct rte_eth_dev *dev, uint32_t node_id,
...
static int hns3_tm_nonleaf_node_param_check(struct rte_eth_dev *dev,
  ...
  if (params->shaper_profile_id != RTE_TM_SHAPER_PROFILE_ID_NONE) {
    shaper_profile =
        hns3_tm_shaper_profile_search(dev, params->shaper_profile_id);
    if (shaper_profile == NULL) {
...
static int hns3_tm_leaf_node_param_check(struct rte_eth_dev *dev __rte_unused,
                                         struct rte_tm_node_params *params,
                                         struct rte_tm_error *error)
...
static int hns3_tm_node_param_check(struct rte_eth_dev *dev, uint32_t node_id,
...
static int hns3_tm_port_node_add(struct rte_eth_dev *dev, uint32_t node_id,
                                 uint32_t level_id,
                                 struct rte_tm_node_params *params,
                                 struct rte_tm_error *error) {
  struct hns3_pf *pf = HNS3_DEV_PRIVATE_TO_PF(dev->data->dev_private);
...
static int hns3_tm_tc_node_add(struct rte_eth_dev *dev, uint32_t node_id,
                               uint32_t level_id,
...
static int hns3_tm_queue_node_add(struct rte_eth_dev *dev, uint32_t node_id,
                                  uint32_t level_id,
                                  struct hns3_tm_node *parent_node,
                                  struct rte_tm_node_params *params,
                                  struct rte_tm_error *error) {
...
static int hns3_tm_node_add(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  ret = hns3_tm_node_param_check(dev, node_id, priority, weight, params, error);
  if (ret)
    return ret;

  /* root node who don't have a parent */
  if (parent_node_id == RTE_TM_NODE_ID_NULL)
    return hns3_tm_port_node_add(dev, node_id, level_id, params, error);

  parent_node = hns3_tm_node_search(dev, parent_node_id, &parent_node_type);
  if (parent_node == NULL) {
    error->type = RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID;
  ...
  if (parent_node_type == HNS3_TM_NODE_TYPE_PORT)
    return hns3_tm_tc_node_add(dev, node_id, level_id, parent_node, params,
                               error);
  else
    return hns3_tm_queue_node_add(dev, node_id, level_id, parent_node, params,
                                  error);
}

static void hns3_tm_node_do_delete(struct hns3_pf *pf,
                                   enum hns3_tm_node_type node_type,
                                   struct hns3_tm_node *tm_node) {
...
static int hns3_tm_node_delete(struct rte_eth_dev *dev, uint32_t node_id,
                               struct rte_tm_error *error) {
  struct hns3_pf *pf = HNS3_DEV_PRIVATE_TO_PF(dev->data->dev_private);
  enum hns3_tm_node_type node_type = HNS3_TM_NODE_TYPE_MAX;
  struct hns3_tm_node *tm_node;
...
static int hns3_tm_node_type_get(struct rte_eth_dev *dev, uint32_t node_id,
...
static int hns3_tm_level_capabilities_get(struct rte_eth_dev *dev,
                                          uint32_t level_id,
                                          struct rte_tm_level_capabilities *cap,
                                          struct rte_tm_error *error) {
  if (cap == NULL || error == NULL)
...
static int hns3_tm_node_capabilities_get(struct rte_eth_dev *dev,
                                         uint32_t node_id,
                                         struct rte_tm_node_capabilities *cap,
                                         struct rte_tm_error *error) {
  enum hns3_tm_node_type node_type;
...
hns3_tm_config_tc_rate(struct hns3_hw *hw, uint8_t tc_no,
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret)
    hns3_err(hw, "failed to config tc (%u) rate, ret = %d", tc_no, ret);

  return ret;
}

static bool hns3_tm_configure_check(struct hns3_hw *hw,
                                    struct rte_tm_error *error) {
  ...
  TAILQ_FOREACH(tm_node, tc_list, node) {
    ...
    if (hns3_tm_calc_node_tc_no(tm_conf, tm_node->id) >= hw->num_tc) {
      error->type = RTE_TM_ERROR_TYPE_NODE_ID;
...
static int hns3_tm_hierarchy_do_commit(struct hns3_hw *hw,
                                       struct rte_tm_error *error) {
  struct hns3_pf *pf = HNS3_DEV_HW_TO_PF(hw);
  struct hns3_tm_node_list *tc_list = &pf->tm_conf.tc_list;
  struct hns3_tm_node *tm_node;
  ...
  TAILQ_FOREACH(tm_node, tc_list, node) {
    ...
    ret = hns3_tm_config_tc_rate(hw, tc_no, tm_node->shaper_profile);
    if (ret) {
      error->type = RTE_TM_ERROR_TYPE_NODE_PARAMS;
...
static int hns3_tm_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
                                    struct rte_tm_error *error) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct hns3_pf *pf = HNS3_DEV_PRIVATE_TO_PF(dev->data->dev_private);
  int ret;
...
static int hns3_tm_hierarchy_commit_wrap(struct rte_eth_dev *dev,
                                         int clear_on_fail,
                                         struct rte_tm_error *error) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  int ret;
...
static int hns3_tm_node_shaper_do_update(
    struct hns3_hw *hw, uint32_t node_id, enum hns3_tm_node_type node_type,
    struct hns3_tm_shaper_profile *shaper_profile, struct rte_tm_error *error) {
  struct hns3_pf *pf = HNS3_DEV_HW_TO_PF(hw);
  uint8_t tc_no;
  int ret;
...
static int hns3_tm_node_shaper_update(struct rte_eth_dev *dev, uint32_t node_id,
                                      uint32_t shaper_profile_id,
                                      struct rte_tm_error *error) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  enum hns3_tm_node_type node_type = HNS3_TM_NODE_TYPE_MAX;
  ...
  if (hns3_tm_node_shaper_do_update(hw, node_id, node_type, profile, error))
    return -EINVAL;
...
static int hns3_tm_node_shaper_update_wrap(struct rte_eth_dev *dev,
                                           uint32_t node_id,
                                           uint32_t shaper_profile_id,
                                           struct rte_tm_error *error) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  rte_spinlock_lock(&hw->lock);
  ret = hns3_tm_node_shaper_update(dev, node_id, shaper_profile_id, error);
  rte_spinlock_unlock(&hw->lock);
...
int hns3_tm_ops_get(struct rte_eth_dev *dev, void *arg) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
void hns3_tm_dev_start_proc(struct hns3_hw *hw) {
  ...
  if (pf->tm_conf.root && !pf->tm_conf.committed)
    hns3_warn(hw, "please call hierarchy_commit() before starting the port.");
}
