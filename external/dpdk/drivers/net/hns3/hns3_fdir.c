...
enum HNS3_PORT_TYPE { HOST_PORT, NETWORK_PORT };
...
static const struct key_info meta_data_key_info[] = {
    {PACKET_TYPE_ID, 6}, {IP_FRAGEMENT, 1}, {ROCE_TYPE, 1},  {NEXT_KEY, 5},
    {VLAN_NUMBER, 2},    {SRC_VPORT, 12},   {DST_VPORT, 12}, {TUNNEL_PACKET, 1},
};

static const struct key_info tuple_key_info[] = {
    {OUTER_DST_MAC, 48},      {OUTER_SRC_MAC, 48},   {OUTER_VLAN_TAG_FST, 16},
    {OUTER_VLAN_TAG_SEC, 16}, {OUTER_ETH_TYPE, 16},  {OUTER_L2_RSV, 16},
    {OUTER_IP_TOS, 8},        {OUTER_IP_PROTO, 8},   {OUTER_SRC_IP, 32},
    {OUTER_DST_IP, 32},       {OUTER_L3_RSV, 16},    {OUTER_SRC_PORT, 16},
    {OUTER_DST_PORT, 16},     {OUTER_L4_RSV, 32},    {OUTER_TUN_VNI, 24},
    {OUTER_TUN_FLOW_ID, 8},   {INNER_DST_MAC, 48},   {INNER_SRC_MAC, 48},
    {INNER_VLAN_TAG1, 16},    {INNER_VLAN_TAG2, 16}, {INNER_ETH_TYPE, 16},
    {INNER_L2_RSV, 16},       {INNER_IP_TOS, 8},     {INNER_IP_PROTO, 8},
    {INNER_SRC_IP, 32},       {INNER_DST_IP, 32},    {INNER_L3_RSV, 16},
    {INNER_SRC_PORT, 16},     {INNER_DST_PORT, 16},  {INNER_SCTP_TAG, 32},
};
...
int hns3_init_fd_config(struct hns3_adapter *hns) {
  ...
  case HNS3_FD_MODE_DEPTH_4K_WIDTH_200B_STAGE_1:
    ...
    break;
  default:
    hns3_err(hw, "Unsupported flow director mode %u", pf->fdir.fd_cfg.fd_mode);
    return -EOPNOTSUPP;
  }
   ...
   */
  key_cfg->meta_data_active =
      BIT(DST_VPORT) | BIT(TUNNEL_PACKET) | BIT(VLAN_NUMBER);
  hns3_dbg(hw, "fdir meta data: dst_vport tunnel_packet vlan_number");

  ret = hns3_get_fd_allocation(hw, &pf->fdir.fd_cfg.rule_num[HNS3_FD_STAGE_1],
  ...
  hns3_dbg(hw,
           "fdir: stage1<rules-%u counters-%u> stage2<rules-%u "
           "counters=%u>",
...
#define FD_TCAM_CMD_NUM 3
  ...
  if (ret)
    hns3_err(hw, "Config tcam key fail, ret=%d loc=%d add=%d", ret, loc,
             is_add);
  return ret;
}
...
static int hns3_fd_ad_config(struct hns3_hw *hw, int loc,
  ...
  hns3_set_bit(ad_data, HNS3_FD_AD_WR_RULE_ID_B, action->write_rule_id_to_bd);
  ...
  hns3_set_field(ad_data, HNS3_FD_AD_QID_M, HNS3_FD_AD_QID_S, action->queue_id);
  hns3_set_bit(ad_data, HNS3_FD_AD_USE_COUNTER_B, action->use_counter);
  hns3_set_field(ad_data, HNS3_FD_AD_COUNTER_NUM_M, HNS3_FD_AD_COUNTER_NUM_S,
                 action->counter_id);
...
static bool hns3_fd_convert_tuple(struct hns3_hw *hw, uint32_t tuple,
  ...
  switch (tuple) {
  case INNER_DST_MAC:
    hns3_fd_convert_mac(key_conf->spec.dst_mac, key_conf->mask.dst_mac, key_x,
                        key_y);
    break;
  case INNER_SRC_MAC:
    hns3_fd_convert_mac(key_conf->spec.src_mac, key_conf->mask.src_mac, key_x,
                        key_y);
    break;
  case OUTER_SRC_PORT:
  case OUTER_DST_PORT:
  ...
  case INNER_SRC_IP:
    hns3_fd_convert_int32(key_conf->spec.src_ip[IP_ADDR_KEY_ID],
                          key_conf->mask.src_ip[IP_ADDR_KEY_ID], key_x, key_y);
    break;
  case INNER_DST_IP:
    hns3_fd_convert_int32(key_conf->spec.dst_ip[IP_ADDR_KEY_ID],
                          key_conf->mask.dst_ip[IP_ADDR_KEY_ID], key_x, key_y);
    break;
  case INNER_SCTP_TAG:
    hns3_fd_convert_int32(key_conf->spec.sctp_tag, key_conf->mask.sctp_tag,
                          key_x, key_y);
    break;
  case OUTER_TUN_VNI:
    for (i = 0; i < VNI_OR_TNI_LEN; i++) {
      tmp = VNI_OR_TNI_LEN - 1 - i;
      calc_x(key_x[tmp], key_conf->spec.outer_tun_vni[i],
             key_conf->mask.outer_tun_vni[i]);
      calc_y(key_y[tmp], key_conf->spec.outer_tun_vni[i],
             key_conf->mask.outer_tun_vni[i]);
    }
    break;
  ...
  case INNER_IP_TOS:
    ...
    break;
  case OUTER_IP_PROTO:
    calc_x(*key_x, key_conf->spec.outer_proto, key_conf->mask.outer_proto);
    calc_y(*key_y, key_conf->spec.outer_proto, key_conf->mask.outer_proto);
    break;
  case INNER_IP_PROTO:
    calc_x(*key_x, key_conf->spec.ip_proto, key_conf->mask.ip_proto);
    calc_y(*key_y, key_conf->spec.ip_proto, key_conf->mask.ip_proto);
    break;
  default:
...
static void hns3_fd_convert_meta_data(struct hns3_fd_key_cfg *cfg,
  ...
  for (i = 0; i < MAX_META_DATA; i++) {
    ...
    if (i == TUNNEL_PACKET) {
      hns3_set_bit(meta_data, cur_pos, rule->key_conf.spec.tunnel_type ? 1 : 0);
      cur_pos += tuple_size;
      ...
      hns3_set_field(meta_data, GENMASK(cur_pos + tuple_size, cur_pos), cur_pos,
                     vlan_tag);
      cur_pos += tuple_size;
    } else if (i == DST_VPORT) {
...
static int hns3_config_key(struct hns3_adapter *hns,
  ...
  for (i = 0; i < MAX_TUPLE; i++) {
    ...
    if (key_cfg->tuple_active & BIT(i)) {
      tuple_valid = hns3_fd_convert_tuple(hw, i, cur_key_x, cur_key_y, rule);
      if (tuple_valid) {
  ...
  hns3_fd_convert_meta_data(key_cfg, vf_id, rule, key_x + meta_data_region,
                            key_y + meta_data_region);

  ret = hns3_fd_tcam_config(hw, false, rule->location, key_y, true);
  if (ret) {
    hns3_err(hw, "Config fd key_y fail, loc=%u, ret=%d", rule->queue_id, ret);
    return ret;
  }

  ret = hns3_fd_tcam_config(hw, true, rule->location, key_x, true);
  if (ret)
    hns3_err(hw, "Config fd key_x fail, loc=%u, ret=%d", rule->queue_id, ret);
  return ret;
}
...
int hns3_fdir_filter_init(struct hns3_adapter *hns) {
  ...
  fdir_info->hash_map = rte_zmalloc(
      "hns3 FDIR hash", rule_num * sizeof(struct hns3_fdir_rule_ele *), 0);
...
void hns3_fdir_filter_uninit(struct hns3_adapter *hns) {
  ...
  while (fdir_filter) {
    ...
    hns3_fd_tcam_config(&hns->hw, true, fdir_filter->fdir_conf.location, NULL,
                        false);
...
int hns3_fdir_filter_program(struct hns3_adapter *hns,
  ...
  if (del) {
    ret = hns3_fd_tcam_config(hw, true, rule->location, NULL, false);
    if (ret)
      hns3_err(hw,
               "Failed to delete fdir: %u src_ip:%x "
               "dst_ip:%x src_port:%u dst_port:%u ret = %d",
               rule->location, rule->key_conf.spec.src_ip[IP_ADDR_KEY_ID],
               rule->key_conf.spec.dst_ip[IP_ADDR_KEY_ID],
               rule->key_conf.spec.src_port, rule->key_conf.spec.dst_port, ret);
    else
  ...
  node = rte_zmalloc("hns3 fdir rule", sizeof(struct hns3_fdir_rule_ele), 0);
  if (node == NULL) {
  ...
  if (ret) {
    hns3_err(hw,
             "Failed to config fdir: %u src_ip:%x dst_ip:%x "
...
int hns3_clear_all_fdir_filter(struct hns3_adapter *hns) {
  ...
  while (fdir_filter) {
    TAILQ_REMOVE(&fdir_info->fdir_list, fdir_filter, entries);
    ret = hns3_fd_tcam_config(hw, true, fdir_filter->fdir_conf.location, NULL,
                              false);
    if (ret == 0)
      succ_cnt++;
    else
  ...
  if (fail_cnt > 0) {
    hns3_err(hw,
             "fail to delete all FDIR filter, success num = %d "
             "fail num = %d",
             succ_cnt, fail_cnt);
    ret = -EIO;
  }
