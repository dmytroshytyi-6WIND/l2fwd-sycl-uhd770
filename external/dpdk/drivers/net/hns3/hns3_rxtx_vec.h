...
static inline void hns3_tx_bulk_free_buffers(struct hns3_tx_queue *txq) {
...
static inline void hns3_tx_free_buffers(struct hns3_tx_queue *txq) {
  ...
  for (i = 0; i < txq->tx_rs_thresh; i++, tx_desc++) {
    if (tx_desc->tx.tp_fe_sc_vld_ra_ri & rte_le_to_cpu_16(BIT(HNS3_TXD_VLD_B)))
      return;
  }
...
static inline uint16_t hns3_rx_reassemble_pkts(struct rte_mbuf **rx_pkts,
                                               uint16_t nb_pkts,
