...
int hns3_mbuf_dyn_rx_timestamp_register(struct rte_eth_dev *dev,
  ...
  if (ret) {
    hns3_err(hw, "failed to register Rx timestamp field/flag");
    return ret;
  }
...
static int hns3_ptp_int_en(struct hns3_hw *hw, bool en) {
  ...
  if (ret)
    hns3_err(hw, "failed to %s ptp interrupt, ret = %d\n",
...
static int hns3_timesync_configure(struct hns3_adapter *hns, bool en) {
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret) {
    hns3_err(hw, "configure PTP time failed, en = %d, ret = %d", en, ret);
    return ret;
  }
...
int hns3_timesync_enable(struct rte_eth_dev *dev) {
  ...
  if (pf->ptp_enable)
    return 0;
  hns3_warn(hw, "note: please ensure Rx/Tx burst mode is simple or common when "
                "enabling PTP!");
...
#define TIME_TX_STAMP_CNT_MASK 0x7
  ...
  ts_cnt =
      hns3_read_dev(hw, HNS3_TX_1588_BACK_TSP_CNT) & TIME_TX_STAMP_CNT_MASK;
  if (ts_cnt == 0)
    return -EINVAL;
...
int hns3_timesync_read_time(struct rte_eth_dev *dev, struct timespec *ts) {
  ...
  sec |= (uint64_t)(hns3_read_dev(hw, HNS3_CURR_TIME_OUT_H) & 0xFFFF) << 32;
...
int hns3_restore_ptp(struct hns3_adapter *hns) {
  ...
  if (ret)
    hns3_err(hw, "restore PTP enable state(%d) failed, ret = %d", en, ret);
