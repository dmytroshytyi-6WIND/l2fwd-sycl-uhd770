...
static const struct hns3_hw_error mac_afifo_tnl_int[] = {
    ...
    {.int_msk = BIT(13),
     ...
     .reset_level = HNS3_GLOBAL_RESET},
    {.int_msk = 0, .msg = NULL, .reset_level = HNS3_NONE_RESET}};
...
static const struct hns3_hw_error ppu_mpf_abnormal_int_st2_ras[] = {
    ...
    {.int_msk = BIT(25),
     ...
     .reset_level = HNS3_GLOBAL_RESET},
    {.int_msk = BIT(26), .msg = "rd_bus_err", .reset_level = HNS3_GLOBAL_RESET},
    {.int_msk = BIT(27), .msg = "wr_bus_err", .reset_level = HNS3_GLOBAL_RESET},
    {.int_msk = BIT(30),
...
static const struct hns3_hw_error ssu_common_ecc_int[] = {
    {.int_msk = BIT(0), .msg = "buf_sum_err", .reset_level = HNS3_NONE_RESET},
    {.int_msk = BIT(1),
     .msg = "ppp_mb_num_err",
     .reset_level = HNS3_NONE_RESET},
    {.int_msk = BIT(2),
...
static const struct hns3_hw_error_desc mpf_ras_err_tbl[] = {
    ...
    {.desc_offset = 9,
     ...
     .hw_err = ncsi_ecc_int},
    {.desc_offset = 0, .data_offset = 0, .msg = NULL, .hw_err = NULL}};
...
static const struct hns3_hw_mod_name hns3_hw_module_name[] = {
    {.module_name = MODULE_NONE, .msg = "MODULE_NONE"},
    {.module_name = MODULE_BIOS_COMMON, .msg = "MODULE_BIOS_COMMON"},
    {.module_name = MODULE_GE, .msg = "MODULE_GE"},
    {.module_name = MODULE_IGU_EGU, .msg = "MODULE_IGU_EGU"},
    {.module_name = MODULE_LGE, .msg = "MODULE_LGE"},
    {.module_name = MODULE_NCSI, .msg = "MODULE_NCSI"},
    {.module_name = MODULE_PPP, .msg = "MODULE_PPP"},
    {.module_name = MODULE_QCN, .msg = "MODULE_QCN"},
    {.module_name = MODULE_RCB_RX, .msg = "MODULE_RCB_RX"},
    {.module_name = MODULE_RTC, .msg = "MODULE_RTC"},
    {.module_name = MODULE_SSU, .msg = "MODULE_SSU"},
    {.module_name = MODULE_TM, .msg = "MODULE_TM"},
    {.module_name = MODULE_RCB_TX, .msg = "MODULE_RCB_TX"},
    {.module_name = MODULE_TXDMA, .msg = "MODULE_TXDMA"},
    {.module_name = MODULE_MASTER, .msg = "MODULE_MASTER"},
    {.module_name = MODULE_ROH_MAC, .msg = "MODULE_ROH_MAC"}};

static const struct hns3_hw_err_type hns3_hw_error_type[] = {
    {.error_type = NONE_ERROR, .msg = "none_error"},
    {.error_type = FIFO_ERROR, .msg = "fifo_error"},
    {.error_type = MEMORY_ERROR, .msg = "memory_error"},
    {.error_type = POISION_ERROR, .msg = "poision_error"},
    {.error_type = MSIX_ECC_ERROR, .msg = "msix_ecc_error"},
    {.error_type = TQP_INT_ECC_ERROR, .msg = "tqp_int_ecc_error"},
    {.error_type = PF_ABNORMAL_INT_ERROR, .msg = "pf_abnormal_int_error"},
    {.error_type = MPF_ABNORMAL_INT_ERROR, .msg = "mpf_abnormal_int_error"},
    {.error_type = COMMON_ERROR, .msg = "common_error"},
    {.error_type = PORT_ERROR, .msg = "port_error"},
    {.error_type = ETS_ERROR, .msg = "ets_error"},
    {.error_type = NCSI_ERROR, .msg = "ncsi_error"},
    {.error_type = GLB_ERROR, .msg = "glb_error"}};

static int hns3_config_ncsi_hw_err_int(struct hns3_adapter *hns, bool en) {
...
static int enable_igu_egu_err_intr(struct hns3_adapter *hns, bool en) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_cmd_desc desc;
  int ret;
...
static int config_ppp_err_intr(struct hns3_adapter *hns, uint32_t cmd,
                               bool en) {
  struct hns3_hw *hw = &hns->hw;
  ...
  if (cmd == HNS3_OPC_PPP_CMD0_INT_CMD) {
    if (en) {
      desc[0].data[0] = rte_cpu_to_le_32(HNS3_PPP_MPF_ECC_ERR_INT0_EN);
      desc[0].data[1] = rte_cpu_to_le_32(HNS3_PPP_MPF_ECC_ERR_INT1_EN);
      desc[0].data[4] = rte_cpu_to_le_32(HNS3_PPP_PF_ERR_INT_EN);
    }

    desc[1].data[0] = rte_cpu_to_le_32(HNS3_PPP_MPF_ECC_ERR_INT0_EN_MASK);
    ...
    if (en) {
      desc[0].data[0] = rte_cpu_to_le_32(HNS3_PPP_MPF_ECC_ERR_INT2_EN);
      desc[0].data[1] = rte_cpu_to_le_32(HNS3_PPP_MPF_ECC_ERR_INT3_EN);
    }

    desc[1].data[0] = rte_cpu_to_le_32(HNS3_PPP_MPF_ECC_ERR_INT2_EN_MASK);
    desc[1].data[1] = rte_cpu_to_le_32(HNS3_PPP_MPF_ECC_ERR_INT3_EN_MASK);
  }
...
static int enable_ppp_err_intr(struct hns3_adapter *hns, bool en) {
  int ret;
...
static int enable_ssu_err_intr(struct hns3_adapter *hns, bool en) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_cmd_desc desc[2];
  int ret;
  ...
  if (en) {
    desc[0].data[0] = rte_cpu_to_le_32(HNS3_SSU_1BIT_ECC_ERR_INT_EN);
    desc[0].data[1] = rte_cpu_to_le_32(HNS3_SSU_MULTI_BIT_ECC_ERR_INT_EN);
    desc[0].data[4] = rte_cpu_to_le_32(HNS3_SSU_BIT32_ECC_ERR_INT_EN);
  }

  desc[1].data[0] = rte_cpu_to_le_32(HNS3_SSU_1BIT_ECC_ERR_INT_EN_MASK);
  desc[1].data[1] = rte_cpu_to_le_32(HNS3_SSU_MULTI_BIT_ECC_ERR_INT_EN_MASK);
  ...
  if (en) {
    ...
    desc[0].data[1] = rte_cpu_to_le_32(HNS3_SSU_PORT_BASED_ERR_INT_EN);
    desc[0].data[2] = rte_cpu_to_le_32(HNS3_SSU_FIFO_OVERFLOW_ERR_INT_EN);
  }

  desc[1].data[0] = rte_cpu_to_le_32(HNS3_SSU_COMMON_INT_EN_MASK |
                                     HNS3_SSU_PORT_BASED_ERR_INT_EN_MASK);
  desc[1].data[1] = rte_cpu_to_le_32(HNS3_SSU_FIFO_OVERFLOW_ERR_INT_EN_MASK);
...
void hns3_config_mac_tnl_int(struct hns3_hw *hw, bool en) {
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret)
    hns3_err(hw, "fail to %s mac tnl intr, ret = %d", en ? "enable" : "disable",
             ret);
}

static int config_ppu_err_intrs(struct hns3_adapter *hns, uint32_t cmd,
                                bool en) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_cmd_desc desc[2];
  int num = 1;
  ...
  case HNS3_OPC_PPU_PF_OTHER_INT_CMD:
    ...
    break;
  default:
    hns3_err(hw, "Invalid cmd(%u) to configure PPU error interrupts.", cmd);
    return -EINVAL;
  }
...
static int enable_ppu_err_intr(struct hns3_adapter *hns, bool en) {
  struct hns3_hw *hw = &hns->hw;
  int ret;
...
static int enable_tm_err_intr(struct hns3_adapter *hns, bool en) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_cmd_desc desc;
  int ret;
...
static int enable_common_err_intr(struct hns3_adapter *hns, bool en) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_cmd_desc desc[2];
  int ret;
  ...
  if (en) {
    desc[0].data[0] = rte_cpu_to_le_32(HNS3_IMP_TCM_ECC_ERR_INT_EN);
    desc[0].data[2] = rte_cpu_to_le_32(HNS3_CMDQ_NIC_ECC_ERR_INT_EN);
    desc[0].data[3] = rte_cpu_to_le_32(HNS3_IMP_RD_POISON_ERR_INT_EN);
    desc[0].data[4] = rte_cpu_to_le_32(HNS3_TQP_ECC_ERR_INT_EN |
                                       HNS3_MSIX_SRAM_ECC_ERR_INT_EN);
    desc[0].data[5] = rte_cpu_to_le_32(HNS3_IMP_ITCM4_ECC_ERR_INT_EN);
  }
...
static int enable_mac_err_intr(struct hns3_adapter *hns, bool en) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_cmd_desc desc;
  int ret;
...
hns3_find_highest_level(struct hns3_adapter *hns, const char *reg,
  ...
  while (err->msg) {
    if (err->int_msk & err_sts) {
      hns3_warn(hw, "%s %s found [error status=0x%x]", reg, err->msg, err_sts);
...
static int query_num_bds(struct hns3_hw *hw, bool is_ras, uint32_t *mpf_bd_num,
                         uint32_t *pf_bd_num) {
  ...
  if (mpf_bd_num_val < mpf_min_bd_num || pf_bd_num_val < pf_min_bd_num) {
    hns3_err(hw,
             "error bd num: mpf(%u), min_mpf(%u), "
             "pf(%u), min_pf(%u)\n",
             mpf_bd_num_val, mpf_min_bd_num, pf_bd_num_val, pf_min_bd_num);
    return -EINVAL;
  }
...
static uint32_t hns3_get_hw_error_status(struct hns3_cmd_desc *desc,
                                         uint8_t desc_offset,
                                         uint8_t data_offset) {
  uint32_t status;
  uint32_t *desc_data;
...
static int hns3_handle_hw_error(struct hns3_adapter *hns,
                                struct hns3_cmd_desc *desc, int num,
                                uint64_t *levels,
  ...
  while (err->msg) {
    status = hns3_get_hw_error_status(desc, err->desc_offset, err->data_offset);
    if (status) {
      /*
       ...
       * hns3_msix_process.
       */
      req_level = hns3_find_highest_level(hns, err->msg, err->hw_err, status);
...
void hns3_handle_msix_error(struct hns3_adapter *hns, uint64_t *levels) {
  ...
  if (ret) {
    hns3_err(hw, "fail to query msix int status bd num: ret = %d", ret);
    return;
  }
  ...
  if (desc == NULL) {
    hns3_err(hw, "fail to zmalloc desc for handling msix error, size = %zu",
  ...
  if (ret) {
    hns3_err(hw, "fail to handle all main pf msix errors, ret = %d", ret);
    goto out;
  }
  ...
  if (ret) {
    hns3_err(hw, "fail to handle all pf msix errors, ret = %d", ret);
    goto out;
  }
...
void hns3_handle_ras_error(struct hns3_adapter *hns, uint64_t *levels) {
  ...
  if (ret) {
    hns3_err(hw, "fail to query ras int status bd num: ret = %d", ret);
    return;
  }
  ...
  desc = rte_zmalloc(NULL, bd_num * sizeof(struct hns3_cmd_desc), 0);
  if (desc == NULL) {
    hns3_err(hw, "fail to zmalloc desc for handing ras error, size = %zu",
             bd_num * sizeof(struct hns3_cmd_desc));
    return;
  }
  ...
  if (ret) {
    hns3_err(hw, "fail to handle all main pf ras errors, ret = %d", ret);
    goto out;
  }
...
#define HNS3_ERR_TYPE_IS_RAS_OFFSET 7
  ...
  hns3_err(hw, "total_module:%u, total_type:%u", total_module, total_type);

  if (mod_id < total_module && type_id < total_type)
    hns3_err(hw, "found %s %s, is %s error.", hns3_hw_module_name[mod_id].msg,
             hns3_hw_error_type[type_id].msg, is_ras ? "ras" : "msix");
  else
    hns3_err(hw, "unknown module[%u] or type[%u].", mod_id, type_id);
...
static void hns3_handle_module_error_data(struct hns3_hw *hw, uint32_t *buf,
                                          uint32_t buf_size) {
  ...
  hns3_err(hw, "reset_type = %s, mod_num = %u.", reset_string[reset_type],
           mod_num);

  while (mod_num--) {
    if (offset >= buf_size) {
      hns3_err(hw, "offset(%u) exceeds buf's size(%u).", offset, buf_size);
      return;
    }
    ...
    for (i = 0; i < err_num; i++) {
      if (offset >= buf_size) {
        hns3_err(hw, "offset(%u) exceeds buf size(%u).", offset, buf_size);
        return;
      }

      type_reg_err_info = (struct hns3_type_reg_err_info *)&buf[offset++];
      hns3_handle_type_reg_error_data(hw, mod_err_info, type_reg_err_info);
...
static int hns3_query_all_err_bd_num(struct hns3_hw *hw, uint32_t *bd_num) {
  struct hns3_cmd_desc desc;
...
static int hns3_query_all_err_info(struct hns3_hw *hw,
                                   struct hns3_cmd_desc *desc,
                                   uint32_t bd_num) {
  int ret;
...
static void hns3_handle_hw_error_v2(struct hns3_hw *hw) {
  ...
  desc = rte_zmalloc("hns3_ras", bd_num * sizeof(struct hns3_cmd_desc), 0);
  if (desc == NULL) {
...
int hns3_reset_init(struct hns3_hw *hw) {
  ...
  hw->reset.wait_data =
      rte_zmalloc("wait_data", sizeof(struct hns3_wait_data), 0);
...
void hns3_schedule_reset(struct hns3_adapter *hns) {
  ...
  if (hw->adapter_state == HNS3_NIC_UNINITIALIZED) {
    __atomic_store_n(&hw->reset.schedule, SCHEDULE_PENDING, __ATOMIC_RELAXED);
    return;
  }
...
#define DEFERRED_SCHED_US (3 * MSEC_PER_SEC * USEC_PER_MSEC)
  ...
  if (__atomic_load_n(&hw->reset.schedule, __ATOMIC_RELAXED) != SCHEDULE_NONE)
    return;
  __atomic_store_n(&hw->reset.schedule, SCHEDULE_DEFERRED, __ATOMIC_RELAXED);
...
void hns3_wait_callback(void *param) {
  ...
  if (done)
    ...
    hns3_err(hw, "%s wait timeout at stage %d", reset_string[hw->reset.level],
             hw->reset.stage);
...
int hns3_reset_req_hw_reset(struct hns3_adapter *hns) {
  ...
  if (hw->reset.wait_data->result == HNS3_WAIT_UNKNOWN) {
    ...
    rte_eal_alarm_set(hw->reset.wait_data->interval, hns3_wait_callback,
                      hw->reset.wait_data);
    return -EAGAIN;
...
static void hns3_clear_reset_level(struct hns3_hw *hw, uint64_t *levels) {
  ...
  if (merge_cnt != hw->reset.stats.merge_cnt)
    hns3_warn(hw,
              ...
              hw->reset.stats.merge_cnt - merge_cnt, hw->reset.stats.merge_cnt);
}

static bool hns3_reset_err_handle(struct hns3_adapter *hns) {
#define MAX_RESET_FAIL_CNT 30
  ...
  if (is_reset_pending(hns)) {
    ...
    hns3_warn(hw,
              "%s reset fail because new Reset is pending "
              "attempts:%" PRIu64,
  ...
  if (hw->reset.attempts < MAX_RESET_FAIL_CNT) {
    ...
    hns3_warn(hw, "%s retry to reset attempts: %d",
              reset_string[hw->reset.level], hw->reset.attempts);
    return true;
  }
...
reset_fail:
  ...
  hw->reset.stats.fail_cnt++;
  hns3_warn(hw,
            "%s reset fail fail_cnt:%" PRIu64 " success_cnt:%" PRIu64
            " global_cnt:%" PRIu64 " imp_cnt:%" PRIu64 " request_cnt:%" PRIu64
            " exec_cnt:%" PRIu64 " merge_cnt:%" PRIu64 "adapter_state:%d",
            reset_string[hw->reset.level], hw->reset.stats.fail_cnt,
...
static int hns3_reset_pre(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  struct timeval tv;
  int ret;
  ...
  if (hw->reset.stage == RESET_STAGE_NONE) {
    ...
    if (ret) {
      hns3_warn(hw, "Reset step1 down fail=%d time=%ld.%.6ld", ret, tv.tv_sec,
                tv.tv_usec);
      return ret;
    }
    hns3_warn(hw, "Reset step1 down success time=%ld.%.6ld", tv.tv_sec,
              tv.tv_usec);
  ...
  if (hw->reset.stage == RESET_STAGE_PREWAIT) {
    ...
    hns3_clock_gettime(&tv);
    if (ret) {
      hns3_warn(hw, "Reset step2 prepare wait fail=%d time=%ld.%.6ld", ret,
                tv.tv_sec, tv.tv_usec);
      return ret;
    }
    hns3_warn(hw, "Reset step2 prepare wait success time=%ld.%.6ld", tv.tv_sec,
              tv.tv_usec);
...
static int hns3_reset_post(struct hns3_adapter *hns) {
#define TIMEOUT_RETRIES_CNT 30
  ...
  if (hw->reset.stage == RESET_STAGE_DEV_INIT) {
    ...
    hns3_clock_gettime(&tv);
    if (ret) {
      hns3_warn(hw, "Reset step5 devinit fail=%d retries=%d", ret,
                hw->reset.retries);
      goto err;
    }
    hns3_warn(hw, "Reset step5 devinit success time=%ld.%.6ld", tv.tv_sec,
              tv.tv_usec);
    hw->reset.retries = 0;
  ...
  if (hw->reset.stage == RESET_STAGE_RESTORE) {
    ...
    hns3_clock_gettime(&tv);
    if (ret) {
      hns3_warn(hw, "Reset step6 restore fail=%d retries=%d", ret,
                hw->reset.retries);
      goto err;
    }
    hns3_warn(hw, "Reset step6 restore success time=%ld.%.6ld", tv.tv_sec,
              tv.tv_usec);
    hw->reset.retries = 0;
  ...
  if (hw->reset.stage == RESET_STAGE_DONE) {
    ...
    hns3_warn(hw,
              "%s reset done fail_cnt:%" PRIu64 " success_cnt:%" PRIu64
              " global_cnt:%" PRIu64 " imp_cnt:%" PRIu64 " request_cnt:%" PRIu64
              ...
              hw->reset.stats.imp_cnt, hw->reset.stats.request_cnt,
              hw->reset.stats.exec_cnt, hw->reset.stats.merge_cnt);
    hns3_warn(hw, "%s reset done delta %" PRIu64 " ms time=%ld.%.6ld",
              reset_string[hw->reset.level], hns3_clock_calctime_ms(&tv_delta),
              tv.tv_sec, tv.tv_usec);
    hw->reset.level = HNS3_NONE_RESET;
  }
  return 0;
...
err:
  if (ret == -ETIME) {
    ...
    if (hw->reset.retries < TIMEOUT_RETRIES_CNT) {
      rte_eal_alarm_set(HNS3_RESET_SYNC_US, hw->reset.ops->reset_service, hns);
      return -EAGAIN;
    }
  }
  hw->reset.retries = 0;
...
static void hns3_reset_fail_handle(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  struct timeval tv_delta;
  struct timeval tv;
...
int hns3_reset_process(struct hns3_adapter *hns,
  ...
  if (hw->reset.level == HNS3_NONE_RESET) {
    ...
    hns3_warn(hw, "Start %s reset time=%ld.%.6ld",
              reset_string[hw->reset.level], hw->reset.start_time.tv_sec,
              hw->reset.start_time.tv_usec);
  }

  if (is_reset_pending(hns)) {
    hns3_clock_gettime(&tv);
    hns3_warn(hw, "%s reset is aborted by high level time=%ld.%.6ld",
  ...
  if (hw->reset.stage == RESET_STAGE_REQ_HW_RESET) {
    ...
    if (ret == -EAGAIN)
      return ret;
    hns3_clock_gettime(&tv);
    hns3_warn(hw, "Reset step3 request IMP reset success time=%ld.%.6ld",
              tv.tv_sec, tv.tv_usec);
  ...
  if (hw->reset.stage == RESET_STAGE_WAIT) {
    ...
    if (ret)
      goto retry;
    hns3_clock_gettime(&tv);
    hns3_warn(hw, "Reset step4 reset wait success time=%ld.%.6ld", tv.tv_sec,
              tv.tv_usec);
...
static void hns3_report_lse(void *arg) {
