...
static int hns3_send_reset_tqp_cmd(struct hns3_hw *hw, uint16_t queue_id,
  ...
  if (ret)
    hns3_err(hw,
             "send tqp reset cmd error, queue_id = %u, "
             "ret = %d",
             queue_id, ret);
...
static int hns3_get_tqp_reset_status(struct hns3_hw *hw, uint16_t queue_id,
  ...
  if (ret) {
    hns3_err(hw,
             "get tqp reset status error, queue_id = %u, "
             "ret = %d.",
             queue_id, ret);
    return ret;
  }
...
#define HNS3_RESET_ALL_TQP_SUCCESS 1U
  ...
  if (reset_status == HNS3_RESET_RCB_NOT_SUPPORT) {
    for (i = 0; i < hw->cfg_max_queues; i++) {
      ...
      if (ret) {
        hns3_err(hw, "fail to reset tqp, queue_id = %d, ret = %d.", i, ret);
        return ret;
      }
    }
...
#define HNS3VF_RESET_ALL_TQP_DONE 1U
  ...
  ret =
      hns3_send_mbx_msg(hw, HNS3_MBX_QUEUE_RESET, 0, msg_data, sizeof(msg_data),
...
#define HNS3_QUEUE_RESET_TRY_MS 200
  ...
  if (!reset_status) {
    hns3_err(hw,
             "reset queue timeout, queue_id = %u, "
             "queue_type = %s",
...
uint16_t __rte_weak hns3_recv_pkts_vec(__rte_unused void *tx_queue,
...
uint16_t __rte_weak hns3_recv_pkts_vec_sve(
    __rte_unused void *tx_queue, __rte_unused struct rte_mbuf **rx_pkts,
    __rte_unused uint16_t nb_pkts) {
  return 0;
}
...
static int hns3_tx_free_useless_buffer(struct hns3_tx_queue *txq) {
  ...
  for (i = 0; i < txq->tx_rs_thresh; i++) {
    if (desc[i].tx.tp_fe_sc_vld_ra_ri & rte_le_to_cpu_16(BIT(HNS3_TXD_VLD_B)))
      return -1;
  }
...
static inline int hns3_tx_free_required_buffer(struct hns3_tx_queue *txq,
                                               uint16_t required_bds) {
