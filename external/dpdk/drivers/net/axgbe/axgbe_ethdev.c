...
static int axgbe_dev_uc_all_hash_table_set(struct rte_eth_dev *dev,
                                           uint8_t add);
static int axgbe_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete);
...
static int axgbe_dev_xstats_get_names(struct rte_eth_dev *dev,
                                      struct rte_eth_xstat_name *xstats_names,
                                      unsigned int size);
static int axgbe_dev_xstats_get_by_id(struct rte_eth_dev *dev,
                                      const uint64_t *ids, uint64_t *values,
                                      unsigned int n);
static int
axgbe_dev_xstats_get_names_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
                                 struct rte_eth_xstat_name *xstats_names,
                                 unsigned int size);
...
static int axgbe_timesync_read_rx_timestamp(struct rte_eth_dev *dev,
                                            struct timespec *timestamp,
                                            uint32_t flags);
static int axgbe_timesync_read_tx_timestamp(struct rte_eth_dev *dev,
                                            struct timespec *timestamp);
...
static void axgbe_update_tstamp_addend(struct axgbe_port *pdata,
                                       unsigned int addend);
static int axgbe_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vid, int on);
...
#define AXGMAC_MMC_STAT(_string, _var)                                         \
  { _string, offsetof(struct axgbe_mmc_stats, _var), }
...
static void axgbe_dev_interrupt_handler(void *param) {
  ...
  if (dma_isr) {
    if (dma_isr & 1) {
      dma_ch_isr = AXGMAC_DMA_IOREAD(
          (struct axgbe_rx_queue *)pdata->rx_queues[0], DMA_CH_SR);
      PMD_DRV_LOG(DEBUG, "DMA_CH0_ISR=%#010x\n", dma_ch_isr);
      AXGMAC_DMA_IOWRITE((struct axgbe_rx_queue *)pdata->rx_queues[0],
...
static int axgbe_dev_configure(struct rte_eth_dev *dev) {
  ...
  pdata->rx_csum_enable =
      dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_CHECKSUM;
  return 0;
}

static int axgbe_dev_rx_mq_config(struct rte_eth_dev *dev) {
  struct axgbe_port *pdata = dev->data->dev_private;
...
static int axgbe_dev_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_key != NULL &&
    ...
    rte_memcpy(pdata->rss_key, rss_conf->rss_key, AXGBE_RSS_HASH_KEY_SIZE);
...
static int axgbe_dev_rss_hash_conf_get(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_key != NULL &&
      rss_conf->rss_key_len >= AXGBE_RSS_HASH_KEY_SIZE) {
    rte_memcpy(rss_conf->rss_key, pdata->rss_key, AXGBE_RSS_HASH_KEY_SIZE);
  }
  rss_conf->rss_key_len = AXGBE_RSS_HASH_KEY_SIZE;
...
static int axgbe_dev_set_mc_addr_list(struct rte_eth_dev *dev,
  ...
  for (i = 1; i < hw_feat->addn_mac; i++) {
    if (rte_is_zero_ether_addr(&dev->data->mac_addrs[i]))
      continue;
    memset(&dev->data->mac_addrs[i], 0, sizeof(struct rte_ether_addr));
  }
...
static int axgbe_dev_uc_all_hash_table_set(struct rte_eth_dev *dev,
  ...
  for (index = 0; index < pdata->hash_table_count; index++) {
    ...
    AXGMAC_IOWRITE(pdata, MAC_HTR(index), pdata->uc_hash_table[index]);
  }
...
static int axgbe_dev_get_regs(struct rte_eth_dev *dev,
  ...
  if (regs->length && regs->length != (uint32_t)axgbe_regs_get_count(pdata))
    return -ENOTSUP;

  regs->version =
      pdata->pci_dev->id.vendor_id << 16 | pdata->pci_dev->id.device_id;
...
static void axgbe_read_mmc_stats(struct axgbe_port *pdata) {
  ...
  /* Tx counters */
  stats->txoctetcount_gb += AXGMAC_IOREAD(pdata, MMC_TXOCTETCOUNT_GB_LO);
  stats->txoctetcount_gb +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_TXOCTETCOUNT_GB_HI) << 32);

  stats->txframecount_gb += AXGMAC_IOREAD(pdata, MMC_TXFRAMECOUNT_GB_LO);
  ...
  stats->tx64octets_gb += AXGMAC_IOREAD(pdata, MMC_TX64OCTETS_GB_LO);
  stats->tx64octets_gb +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_TX64OCTETS_GB_HI) << 32);

  stats->tx65to127octets_gb += AXGMAC_IOREAD(pdata, MMC_TX65TO127OCTETS_GB_LO);
  ...
  stats->txunicastframes_gb += AXGMAC_IOREAD(pdata, MMC_TXUNICASTFRAMES_GB_LO);
  ...
  stats->txunderflowerror += AXGMAC_IOREAD(pdata, MMC_TXUNDERFLOWERROR_LO);
  stats->txunderflowerror +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_TXUNDERFLOWERROR_HI) << 32);

  stats->txoctetcount_g += AXGMAC_IOREAD(pdata, MMC_TXOCTETCOUNT_G_LO);
  stats->txoctetcount_g +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_TXOCTETCOUNT_G_HI) << 32);

  stats->txframecount_g += AXGMAC_IOREAD(pdata, MMC_TXFRAMECOUNT_G_LO);
  stats->txframecount_g +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_TXFRAMECOUNT_G_HI) << 32);

  stats->txpauseframes += AXGMAC_IOREAD(pdata, MMC_TXPAUSEFRAMES_LO);
  stats->txpauseframes +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_TXPAUSEFRAMES_HI) << 32);

  stats->txvlanframes_g += AXGMAC_IOREAD(pdata, MMC_TXVLANFRAMES_G_LO);
  stats->txvlanframes_g +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_TXVLANFRAMES_G_HI) << 32);

  /* Rx counters */
  stats->rxframecount_gb += AXGMAC_IOREAD(pdata, MMC_RXFRAMECOUNT_GB_LO);
  stats->rxframecount_gb +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RXFRAMECOUNT_GB_HI) << 32);

  stats->rxoctetcount_gb += AXGMAC_IOREAD(pdata, MMC_RXOCTETCOUNT_GB_LO);
  stats->rxoctetcount_gb +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RXOCTETCOUNT_GB_HI) << 32);

  stats->rxoctetcount_g += AXGMAC_IOREAD(pdata, MMC_RXOCTETCOUNT_G_LO);
  ...
  stats->rxcrcerror += AXGMAC_IOREAD(pdata, MMC_RXCRCERROR_LO);
  stats->rxcrcerror +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RXCRCERROR_HI) << 32);

  stats->rxrunterror += AXGMAC_IOREAD(pdata, MMC_RXRUNTERROR);

  stats->rxjabbererror += AXGMAC_IOREAD(pdata, MMC_RXJABBERERROR);

  stats->rxundersize_g += AXGMAC_IOREAD(pdata, MMC_RXUNDERSIZE_G);

  stats->rxoversize_g += AXGMAC_IOREAD(pdata, MMC_RXOVERSIZE_G);

  stats->rx64octets_gb += AXGMAC_IOREAD(pdata, MMC_RX64OCTETS_GB_LO);
  stats->rx64octets_gb +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RX64OCTETS_GB_HI) << 32);

  stats->rx65to127octets_gb += AXGMAC_IOREAD(pdata, MMC_RX65TO127OCTETS_GB_LO);
  ...
  stats->rxunicastframes_g += AXGMAC_IOREAD(pdata, MMC_RXUNICASTFRAMES_G_LO);
  stats->rxunicastframes_g +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RXUNICASTFRAMES_G_HI) << 32);

  stats->rxlengtherror += AXGMAC_IOREAD(pdata, MMC_RXLENGTHERROR_LO);
  stats->rxlengtherror +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RXLENGTHERROR_HI) << 32);

  stats->rxoutofrangetype += AXGMAC_IOREAD(pdata, MMC_RXOUTOFRANGETYPE_LO);
  stats->rxoutofrangetype +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RXOUTOFRANGETYPE_HI) << 32);

  stats->rxpauseframes += AXGMAC_IOREAD(pdata, MMC_RXPAUSEFRAMES_LO);
  stats->rxpauseframes +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RXPAUSEFRAMES_HI) << 32);

  stats->rxfifooverflow += AXGMAC_IOREAD(pdata, MMC_RXFIFOOVERFLOW_LO);
  stats->rxfifooverflow +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RXFIFOOVERFLOW_HI) << 32);

  stats->rxvlanframes_gb += AXGMAC_IOREAD(pdata, MMC_RXVLANFRAMES_GB_LO);
  stats->rxvlanframes_gb +=
      ((uint64_t)AXGMAC_IOREAD(pdata, MMC_RXVLANFRAMES_GB_HI) << 32);

  stats->rxwatchdogerror += AXGMAC_IOREAD(pdata, MMC_RXWATCHDOGERROR);
...
static int axgbe_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < n && i < AXGBE_XSTATS_COUNT; i++) {
    stats[i].id = i;
    stats[i].value =
        *(u64 *)((uint8_t *)&pdata->mmc_stats + axgbe_xstats_strings[i].offset);
  }

  return i;
}

static int axgbe_dev_xstats_get_names(__rte_unused struct rte_eth_dev *dev,
                                      struct rte_eth_xstat_name *xstats_names,
                                      unsigned int n) {
  unsigned int i;
...
static int axgbe_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa =
      ...
      RTE_ETH_RX_OFFLOAD_VLAN_EXTEND | RTE_ETH_RX_OFFLOAD_IPV4_CKSUM |
      RTE_ETH_RX_OFFLOAD_UDP_CKSUM | RTE_ETH_RX_OFFLOAD_TCP_CKSUM |
      RTE_ETH_RX_OFFLOAD_SCATTER | RTE_ETH_RX_OFFLOAD_KEEP_CRC;
...
static int axgbe_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  AXGMAC_MTL_IOWRITE_BITS(
      pdata, 0, MTL_Q_RQFCR, RFA,
...
static int axgbe_priority_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  AXGMAC_MTL_IOWRITE_BITS(
      pdata, tc_num, MTL_Q_RQFCR, RFA,
      AXGMAC_FLOW_CONTROL_VALUE(1024 * pfc_conf->fc.high_water));
  AXGMAC_MTL_IOWRITE_BITS(
      pdata, tc_num, MTL_Q_RQFCR, RFD,
      AXGMAC_FLOW_CONTROL_VALUE(1024 * pfc_conf->fc.low_water));

  switch (tc_num) {
  case 0:
    AXGMAC_IOWRITE_BITS(pdata, MTL_TCPM0R, PSTC0, pfc_conf->fc.pause_time);
    break;
  case 1:
    AXGMAC_IOWRITE_BITS(pdata, MTL_TCPM0R, PSTC1, pfc_conf->fc.pause_time);
    break;
  case 2:
    AXGMAC_IOWRITE_BITS(pdata, MTL_TCPM0R, PSTC2, pfc_conf->fc.pause_time);
    break;
  case 3:
    AXGMAC_IOWRITE_BITS(pdata, MTL_TCPM0R, PSTC3, pfc_conf->fc.pause_time);
    break;
  case 4:
    AXGMAC_IOWRITE_BITS(pdata, MTL_TCPM1R, PSTC4, pfc_conf->fc.pause_time);
    break;
  case 5:
    AXGMAC_IOWRITE_BITS(pdata, MTL_TCPM1R, PSTC5, pfc_conf->fc.pause_time);
    break;
  case 7:
    AXGMAC_IOWRITE_BITS(pdata, MTL_TCPM1R, PSTC6, pfc_conf->fc.pause_time);
    break;
  case 6:
    AXGMAC_IOWRITE_BITS(pdata, MTL_TCPM1R, PSTC7, pfc_conf->fc.pause_time);
    break;
  }
...
void axgbe_rxq_info_get(struct rte_eth_dev *dev, uint16_t queue_id,
                        struct rte_eth_rxq_info *qinfo) {
  struct axgbe_rx_queue *rxq;
...
void axgbe_txq_info_get(struct rte_eth_dev *dev, uint16_t queue_id,
                        struct rte_eth_txq_info *qinfo) {
  struct axgbe_tx_queue *txq;
...
static void axgbe_update_tstamp_time(struct axgbe_port *pdata, unsigned int sec,
                                     unsigned int nsec, int addsub) {
...
static int axgbe_adjfreq(struct axgbe_port *pdata, int64_t delta) {
  ...
  addend =
      (neg_adjust) ? pdata->tstamp_addend - diff : pdata->tstamp_addend + diff;
...
static int axgbe_timesync_read_rx_timestamp(struct rte_eth_dev *dev,
  ...
  if (AXGMAC_GET_BITS_LE(desc->write.desc3, RX_NORMAL_DESC3, CTXT)) {
    if (AXGMAC_GET_BITS_LE(desc->write.desc3, RX_CONTEXT_DESC3, TSA) &&
        !AXGMAC_GET_BITS_LE(desc->write.desc3, RX_CONTEXT_DESC3, TSD)) {
      pmt = AXGMAC_GET_BITS_LE(desc->write.desc3, RX_CONTEXT_DESC3, PMT);
      ...
      if (nsec != 0xffffffffffffffffULL) {
        if (pmt == 0x01)
          *timestamp = rte_ns_to_timespec(nsec);
        PMD_DRV_LOG(DEBUG, "flags = 0x%x nsec = %" PRIu64 "\n", flags, nsec);
      }
    }
  }
...
static int axgbe_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vid,
                                 int on) {
  struct axgbe_port *pdata = dev->data->dev_private;
  ...
  case RTE_ETH_VLAN_TYPE_INNER:
    ...
    if (qinq) {
      if (tpid != 0x8100 && tpid != 0x88a8)
        PMD_DRV_LOG(ERR, "tag supported 0x8100/0x88A8\n");
      ...
      PMD_DRV_LOG(ERR, "Inner type not supported in single tag\n");
    }
    break;
...
static int axgbe_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  ...
  if (mask & RTE_ETH_VLAN_EXTEND_MASK) {
    if (rxmode->offloads & RTE_ETH_RX_OFFLOAD_VLAN_EXTEND) {
      ...
      axgbe_vlan_tpid_set(dev, RTE_ETH_VLAN_TYPE_OUTER, RTE_ETHER_TYPE_VLAN);
      axgbe_vlan_tpid_set(dev, RTE_ETH_VLAN_TYPE_INNER, RTE_ETHER_TYPE_VLAN);
    } else {
...
static void axgbe_get_all_hw_features(struct axgbe_port *pdata) {
  ...
  hw_feat->addn_mac = AXGMAC_GET_BITS(mac_hfr0, MAC_HWF0R, ADDMACADRSEL);
  ...
  hw_feat->rx_fifo_size = AXGMAC_GET_BITS(mac_hfr1, MAC_HWF1R, RXFIFOSIZE);
  hw_feat->tx_fifo_size = AXGMAC_GET_BITS(mac_hfr1, MAC_HWF1R, TXFIFOSIZE);
  hw_feat->adv_ts_hi = AXGMAC_GET_BITS(mac_hfr1, MAC_HWF1R, ADVTHWORD);
  ...
  hw_feat->hash_table_size = AXGMAC_GET_BITS(mac_hfr1, MAC_HWF1R, HASHTBLSZ);
  hw_feat->l3l4_filter_num = AXGMAC_GET_BITS(mac_hfr1, MAC_HWF1R, L3L4FNUM);
  ...
  hw_feat->aux_snap_num = AXGMAC_GET_BITS(mac_hfr2, MAC_HWF2R, AUXSNAPNUM);

  /* Hardware feature register 3 */
  hw_feat->tx_q_vlan_tag_ins = AXGMAC_GET_BITS(mac_hfr3, MAC_HWF3R, CBTISEL);
  hw_feat->no_of_vlan_extn = AXGMAC_GET_BITS(mac_hfr3, MAC_HWF3R, NRVF);
...
static void axgbe_set_counts(struct axgbe_port *pdata) {
   ...
   */
  pdata->tx_ring_count =
      RTE_MIN(pdata->hw_feat.tx_ch_cnt, pdata->tx_max_channel_count);
  pdata->tx_ring_count = RTE_MIN(pdata->tx_ring_count, pdata->tx_max_q_count);
  ...
  pdata->rx_q_count = RTE_MIN(pdata->hw_feat.rx_q_cnt, pdata->rx_max_q_count);
}
 ...
 */
static uint16_t get_pci_rc_devid(void) {
  ...
  snprintf(pci_sysfs, sizeof(pci_sysfs), "%s/" PCI_PRI_FMT "/device",
           rte_pci_get_sysfs_path(), pci_rc_addr.domain, pci_rc_addr.bus,
           pci_rc_addr.devid, pci_rc_addr.function);
...
static int eth_axgbe_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  pdata->xgmac_regs = (void *)pci_dev->mem_resource[AXGBE_AXGMAC_BAR].addr;
  pdata->xprop_regs =
      (void *)((uint8_t *)pdata->xgmac_regs + AXGBE_MAC_PROP_OFFSET);
  pdata->xi2c_regs =
      (void *)((uint8_t *)pdata->xgmac_regs + AXGBE_I2C_CTRL_OFFSET);
  ...
  PMD_INIT_LOG(DEBUG, "xpcs window :%x, size :%x, mask :%x ",
               pdata->xpcs_window, pdata->xpcs_window_size,
  ...
  eth_dev->data->hash_mac_addrs = rte_zmalloc("axgbe_hash_mac_addr", len, 0);
  ...
  pdata->tx_max_fifo_size =
      RTE_MIN(pdata->tx_max_fifo_size, pdata->vdata->tx_max_fifo_size);
  pdata->rx_max_fifo_size = XP_GET_BITS(reg, XP_PROP_2, RX_FIFO_SIZE);
  pdata->rx_max_fifo_size *= 16384;
  pdata->rx_max_fifo_size =
      RTE_MIN(pdata->rx_max_fifo_size, pdata->vdata->rx_max_fifo_size);
  ...
  rte_intr_callback_register(pci_dev->intr_handle, axgbe_dev_interrupt_handler,
                             (void *)eth_dev);
...
static int axgbe_dev_close(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev;
  ...
  rte_intr_callback_unregister(pci_dev->intr_handle,
                               axgbe_dev_interrupt_handler, (void *)eth_dev);
...
static int eth_axgbe_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                               struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct axgbe_port),
                                       eth_axgbe_dev_init);
}
