...
static int axgbe_phy_i2c_read(struct axgbe_port *pdata, unsigned int target,
                              void *reg, unsigned int reg_len, void *val,
...
static int axgbe_phy_get_comm_ownership(struct axgbe_port *pdata) {
  ...
  while (time_before(rte_get_timer_cycles(), timeout)) {
    ...
    if (XP_IOREAD(pdata, XP_I2C_MUTEX) || XP_IOREAD(pdata, XP_MDIO_MUTEX)) {
      rte_delay_us(100);
      continue;
    }
...
static void axgbe_phy_sfp_parse_eeprom(struct axgbe_port *pdata) {
  ...
  else if (sfp_base[AXGBE_SFP_BASE_10GBE_CC] & AXGBE_SFP_BASE_10GBE_CC_LRM)
...
static int axgbe_phy_sfp_read_eeprom(struct axgbe_port *pdata) {
  ...
  eeprom_addr = 0;
  ret =
      axgbe_phy_i2c_read(pdata, AXGBE_SFP_SERIAL_ID_ADDRESS, &eeprom_addr,
                         sizeof(eeprom_addr), &sfp_eeprom, sizeof(sfp_eeprom));
  if (ret) {
...
static void axgbe_phy_sfp_signals(struct axgbe_port *pdata) {
  ...
  gpio_reg = 0;
  ret = axgbe_phy_i2c_read(pdata, phy_data->sfp_gpio_address, &gpio_reg,
...
static enum axgbe_an_mode
axgbe_phy_an_sfp_mode(struct axgbe_phy_data *phy_data) {
  switch (phy_data->sfp_base) {
  case AXGBE_SFP_BASE_1000_T:
...
static int axgbe_phy_set_redrv_mode_mdio(struct axgbe_port *pdata,
  ...
  return pdata->hw_if.write_ext_mii_regs(pdata, phy_data->redrv_addr, redrv_reg,
                                         redrv_val);
}
...
static void axgbe_phy_pll_ctrl(struct axgbe_port *pdata, bool enable) {
  XMDIO_WRITE_BITS(pdata, MDIO_MMD_PMAPMD, MDIO_VEND2_PMA_MISC_CTRL0,
                   ...
                   enable ? XGBE_PMA_PLL_CTRL_SET : XGBE_PMA_PLL_CTRL_CLEAR);
...
static enum axgbe_mode
axgbe_phy_switch_bp_2500_mode(struct axgbe_port *pdata __rte_unused) {
...
static enum axgbe_mode
axgbe_phy_get_basex_mode(struct axgbe_phy_data *phy_data __rte_unused,
                         int speed) {
  switch (speed) {
  case SPEED_1000:
    return AXGBE_MODE_X;
...
static enum axgbe_mode
axgbe_phy_get_baset_mode(struct axgbe_phy_data *phy_data __rte_unused,
                         int speed) {
  switch (speed) {
  case SPEED_100:
...
static enum axgbe_mode axgbe_phy_get_mode(struct axgbe_port *pdata, int speed) {
  struct axgbe_phy_data *phy_data = pdata->phy_data;
...
static bool axgbe_phy_check_mode(struct axgbe_port *pdata, enum axgbe_mode mode,
                                 u32 advert) {
...
static bool axgbe_phy_use_basex_mode(struct axgbe_port *pdata,
  ...
  switch (mode) {
  case AXGBE_MODE_X:
    return axgbe_phy_check_mode(pdata, mode, ADVERTISED_1000baseT_Full);
  case AXGBE_MODE_KR:
    return axgbe_phy_check_mode(pdata, mode, ADVERTISED_10000baseT_Full);
  default:
    return false;
  }
}
...
static bool axgbe_phy_use_baset_mode(struct axgbe_port *pdata,
  ...
  switch (mode) {
  case AXGBE_MODE_SGMII_100:
    return axgbe_phy_check_mode(pdata, mode, ADVERTISED_100baseT_Full);
  case AXGBE_MODE_SGMII_1000:
    return axgbe_phy_check_mode(pdata, mode, ADVERTISED_1000baseT_Full);
  case AXGBE_MODE_KR:
    return axgbe_phy_check_mode(pdata, mode, ADVERTISED_10000baseT_Full);
...
static bool axgbe_phy_use_bp_2500_mode(struct axgbe_port *pdata,
  ...
  switch (mode) {
  case AXGBE_MODE_KX_2500:
    return axgbe_phy_check_mode(pdata, mode, ADVERTISED_2500baseX_Full);
  default:
    return false;
  }
}
...
static bool axgbe_phy_use_bp_mode(struct axgbe_port *pdata,
  ...
  switch (mode) {
  case AXGBE_MODE_KX_1000:
    return axgbe_phy_check_mode(pdata, mode, ADVERTISED_1000baseKX_Full);
  case AXGBE_MODE_KR:
    return axgbe_phy_check_mode(pdata, mode, ADVERTISED_10000baseKR_Full);
  default:
    return false;
  }
}
...
static void axgbe_phy_sfp_gpio_setup(struct axgbe_port *pdata) {
  ...
  phy_data->sfp_gpio_address =
      AXGBE_GPIO_ADDRESS_PCA9555 + XP_GET_BITS(reg, XP_PROP_3, GPIO_ADDR);

  phy_data->sfp_gpio_mask = XP_GET_BITS(reg, XP_PROP_3, GPIO_MASK);

  phy_data->sfp_gpio_rx_los = XP_GET_BITS(reg, XP_PROP_3, GPIO_RX_LOS);
  phy_data->sfp_gpio_tx_fault = XP_GET_BITS(reg, XP_PROP_3, GPIO_TX_FAULT);
  phy_data->sfp_gpio_mod_absent = XP_GET_BITS(reg, XP_PROP_3, GPIO_MOD_ABS);
  phy_data->sfp_gpio_rate_select =
      XP_GET_BITS(reg, XP_PROP_3, GPIO_RATE_SELECT);
}
...
static int axgbe_phy_mdio_reset_setup(struct axgbe_port *pdata) {
  ...
  if (phy_data->mdio_reset == AXGBE_MDIO_RESET_I2C_GPIO) {
    phy_data->mdio_reset_addr =
        AXGBE_GPIO_ADDRESS_PCA9555 +
        XP_GET_BITS(reg, XP_PROP_3, MDIO_RESET_I2C_ADDR);
    phy_data->mdio_reset_gpio =
        XP_GET_BITS(reg, XP_PROP_3, MDIO_RESET_I2C_GPIO);
  } else if (phy_data->mdio_reset == AXGBE_MDIO_RESET_INT_GPIO) {
    phy_data->mdio_reset_gpio =
        XP_GET_BITS(reg, XP_PROP_3, MDIO_RESET_INT_GPIO);
  }
...
static void axgbe_phy_cdr_track(struct axgbe_port *pdata) {
  ...
  XMDIO_WRITE_BITS(pdata, MDIO_MMD_PMAPMD, MDIO_VEND2_PMA_CDR_CONTROL,
                   AXGBE_PMA_CDR_TRACK_EN_MASK, AXGBE_PMA_CDR_TRACK_EN_ON);
...
static void axgbe_phy_cdr_notrack(struct axgbe_port *pdata) {
  ...
  XMDIO_WRITE_BITS(pdata, MDIO_MMD_PMAPMD, MDIO_VEND2_PMA_CDR_CONTROL,
                   AXGBE_PMA_CDR_TRACK_EN_MASK, AXGBE_PMA_CDR_TRACK_EN_OFF);
