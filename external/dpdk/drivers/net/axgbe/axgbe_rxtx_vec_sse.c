...
static inline void axgbe_vec_tx(volatile struct axgbe_tx_desc *desc,
  ...
  __m128i descriptor =
      _mm_set_epi64x((uint64_t)mbuf->pkt_len << 32 | TX_DESC_CTRL_FLAGS |
                         mbuf->data_len | tmst_en,
                     mbuf->buf_iova + mbuf->data_off);
  _mm_store_si128((__m128i *)desc, descriptor);
}

static void axgbe_xmit_cleanup_vec(struct axgbe_tx_queue *txq) {
...
uint16_t axgbe_xmit_pkts_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  txq->cur += nb_pkts;
  tail_addr =
      (uint32_t)(txq->ring_phys_addr + idx * sizeof(struct axgbe_tx_desc));
