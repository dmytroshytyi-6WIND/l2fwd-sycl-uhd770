...
static void axgbe_an73_enable_interrupts(struct axgbe_port *pdata) {
  XMDIO_WRITE(pdata, MDIO_MMD_AN, MDIO_AN_INTMASK, AXGBE_AN_CL73_INT_MASK);
}
...
static void axgbe_change_mode(struct axgbe_port *pdata, enum axgbe_mode mode) {
  switch (mode) {
...
static void axgbe_set_mode(struct axgbe_port *pdata, enum axgbe_mode mode) {
...
static bool axgbe_use_mode(struct axgbe_port *pdata, enum axgbe_mode mode) {
...
static enum axgbe_an axgbe_an73_rx_bpa(struct axgbe_port *pdata,
  ...
  return ((ad_reg & AXGBE_XNP_NP_EXCHANGE) || (lp_reg & AXGBE_XNP_NP_EXCHANGE))
...
static enum axgbe_an axgbe_an73_page_received(struct axgbe_port *pdata) {
  ...
  if (!pdata->an_start) {
    ...
    an_timeout = pdata->an_start + msecs_to_timer_cycles(AXGBE_AN_MS_TIMEOUT);
    ...
    if (time_after(ticks, an_timeout)) {
      ...
      PMD_DRV_LOG(NOTICE, "CL73 AN timed out, resetting state\n");
    }
  }

  state = axgbe_in_kr_mode(pdata) ? &pdata->kr_state : &pdata->kx_state;
...
again:
  ...
  if (pdata->an_state == AXGBE_AN_NO_LINK) {
    pdata->an_int = 0;
    axgbe_an73_clear_interrupts(pdata);
    pdata->eth_dev->data->dev_link.link_status = RTE_ETH_LINK_DOWN;
...
static void axgbe_check_link_timeout(struct axgbe_port *pdata) {
  ...
  link_timeout =
      pdata->link_check + (AXGBE_LINK_TIMEOUT * 2 * rte_get_timer_hz());
  ticks = rte_get_timer_cycles();
...
static int autoneg_time_out(unsigned long autoneg_start_time) {
  ...
  autoneg_timeout =
      autoneg_start_time + (AXGBE_LINK_TIMEOUT * 2 * rte_get_timer_hz());
  ticks = rte_get_timer_cycles();
...
static void axgbe_phy_status(struct axgbe_port *pdata) {
  ...
  if (pdata->phy.link) {
    if (link_aneg && !axgbe_phy_aneg_done(pdata)) {
      if (axgbe_cur_mode(pdata) == AXGBE_MODE_SGMII_1000) {
        ...
        while (!(reg & AXGBE_AN_CL37_INT_CMPLT)) {
          ret = autoneg_time_out(autoneg_start_time);
          if (ret)
            break;
          reg = XMDIO_READ(pdata, MDIO_MMD_VEND2, MDIO_VEND2_AN_STAT);
    ...
    if (rte_bit_relaxed_get32(AXGBE_LINK_INIT, &pdata->dev_state))
      rte_bit_relaxed_clear32(AXGBE_LINK_INIT, &pdata->dev_state);
  } else {
    if (rte_bit_relaxed_get32(AXGBE_LINK_INIT, &pdata->dev_state)) {
      axgbe_check_link_timeout(pdata);
...
static int axgbe_phy_init(struct axgbe_port *pdata) {
  ...
  pdata->fec_ability =
      XMDIO_READ(pdata, MDIO_MMD_PMAPMD, MDIO_PMA_10GBR_FECABLE);
  pdata->fec_ability &=
