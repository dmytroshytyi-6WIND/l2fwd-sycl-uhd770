...
static void axgbe_rx_queue_release(struct axgbe_rx_queue *rx_queue) {
  uint16_t i;
...
int axgbe_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
   ...
   */
  if ((!rte_is_power_of_2(rx_desc)) || rx_desc > pdata->rx_desc_count)
    return -EINVAL;
  /* First allocate the rx queue data structure */
  rxq = rte_zmalloc_socket("ethdev RX queue", sizeof(struct axgbe_rx_queue),
  ...
  rxq->dma_tail_reg =
      (volatile uint32_t *)((uint8_t *)rxq->dma_regs + DMA_CH_RDTR_LO);
  ...
  rxq->free_thresh =
      rx_conf->rx_free_thresh ? rx_conf->rx_free_thresh : AXGBE_RX_FREE_THRESH;
  ...
  dma =
      rte_eth_dma_zone_reserve(dev, "rx_ring", queue_idx, size, 128, socket_id);
  if (!dma) {
  ...
  rxq->sw_ring =
      rte_zmalloc_socket("sw_ring", size, RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq->sw_ring) {
...
static void axgbe_prepare_rx_stop(struct axgbe_port *pdata,
   ...
   */
  rx_timeout =
      rte_get_timer_cycles() + (AXGBE_DMA_STOP_TIMEOUT * rte_get_timer_hz());
  ...
  if (!time_before(rte_get_timer_cycles(), rx_timeout))
    PMD_DRV_LOG(ERR, "timed out waiting for Rx queue %u to empty\n", queue);
}
...
uint16_t axgbe_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_rx < nb_pkts) {
    ...
    err = AXGMAC_GET_BITS_LE(desc->write.desc3, RX_NORMAL_DESC3, ES);
    error_status = 0;
    if (unlikely(err)) {
    ...
    if (rxq->pdata->rx_csum_enable) {
      ...
      } else if (unlikely(error_status == AXGBE_L4_CSUM_ERR)) {
        mbuf->ol_flags &= ~RTE_MBUF_F_RX_L4_CKSUM_GOOD;
    ...
    etlt = AXGMAC_GET_BITS_LE(desc->write.desc3, RX_NORMAL_DESC3, ETLT);
    offloads = rxq->pdata->eth_dev->data->dev_conf.rxmode.offloads;
    if (!err || !etlt) {
      if (etlt == RX_CVLAN_TAG_PRESENT) {
        mbuf->ol_flags |= RTE_MBUF_F_RX_VLAN;
        mbuf->vlan_tci =
            AXGMAC_GET_BITS_LE(desc->write.desc0, RX_NORMAL_DESC0, OVT);
    ...
    if (AXGMAC_GET_BITS_LE(desc->write.desc3, RX_NORMAL_DESC3, CDA))
      mbuf->ol_flags |=
          RTE_MBUF_F_RX_IEEE1588_PTP | RTE_MBUF_F_RX_IEEE1588_TMST;
    pkt_len = AXGMAC_GET_BITS_LE(desc->write.desc3, RX_NORMAL_DESC3, PL) -
              rxq->crc_len;
    /* Mbuf populate */
    mbuf->next = NULL;
  ...
  err_set:
    ...
    desc->read.baddr = rte_cpu_to_le_64(rte_mbuf_data_iova_default(tmbuf));
  ...
  if (rxq->dirty != old_dirty) {
    ...
    AXGMAC_DMA_IOWRITE(
        rxq, DMA_CH_RDTR_LO,
        low32_value(rxq->ring_phys_addr + (idx * sizeof(union axgbe_rx_desc))));
  }
...
uint16_t eth_axgbe_recv_scattered_pkts(void *rx_queue,
  ...
  next_desc:
    ...
    if (!AXGMAC_GET_BITS_LE(desc->write.desc3, RX_NORMAL_DESC3, LD)) {
      eop = 0;
    ...
    if (first_seg != NULL) {
      if (rte_pktmbuf_chain(first_seg, mbuf) != 0)
        rte_mempool_put(rxq->mb_pool, first_seg);
    } else {
      first_seg = mbuf;
    }
...
int axgbe_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
   ...
   * should be power of 2 and less than h/w supported
   */
  if ((!rte_is_power_of_2(tx_desc)) || tx_desc > pdata->tx_desc_count ||
  ...
  txq->pdata = pdata;
  offloads =
      tx_conf->offloads | txq->pdata->eth_dev->data->dev_conf.txmode.offloads;
  txq->nb_desc = tx_desc;
  txq->free_thresh =
      tx_conf->tx_free_thresh ? tx_conf->tx_free_thresh : AXGBE_TX_FREE_THRESH;
  ...
  txq->dma_tail_reg =
      (volatile uint32_t *)((uint8_t *)txq->dma_regs + DMA_CH_TDTR_LO);
  txq->cur = 0;
  txq->dirty = 0;
  txq->nb_desc_free = txq->nb_desc;
  /* Allocate software ring */
  tsize = txq->nb_desc * sizeof(struct rte_mbuf *);
  txq->sw_ring = rte_zmalloc("tx_sw_ring", tsize, RTE_CACHE_LINE_SIZE);
  if (!txq->sw_ring) {
    axgbe_tx_queue_release(txq);
    return -ENOMEM;
  }
...
int axgbe_dev_fw_version_get(struct rte_eth_dev *eth_dev, char *fw_version,
                             size_t fw_size) {
  struct axgbe_port *pdata;
...
static void axgbe_txq_prepare_tx_stop(struct axgbe_port *pdata,
  ...
  if (!time_before(rte_get_timer_cycles(), tx_timeout))
    PMD_DRV_LOG(ERR, "timed out waiting for Tx queue %u to empty\n", queue);
}
...
static void axgbe_prepare_tx_stop(struct axgbe_port *pdata,
  ...
  if (queue < DMA_DSRX_FIRST_QUEUE) {
    ...
    tx_pos = ((tx_qidx % DMA_DSRX_QPR) * DMA_DSR_Q_WIDTH) + DMA_DSRX_TPS_START;
  }
  ...
  while (time_before(rte_get_timer_cycles(), tx_timeout)) {
    ...
    if ((tx_status == DMA_TPS_STOPPED) || (tx_status == DMA_TPS_SUSPENDED))
      break;
  ...
  if (!time_before(rte_get_timer_cycles(), tx_timeout))
    PMD_DRV_LOG(ERR, "timed out waiting for Tx DMA channel %u to stop\n",
                queue);
}
...
void axgbe_dev_disable_tx(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < pdata->tx_q_count; i++)
    AXGMAC_MTL_IOWRITE_BITS(pdata, i, MTL_Q_TQOMR, TXQEN, 0);
...
void axgbe_dev_enable_tx(struct rte_eth_dev *dev) {
  ...
  /* Enable Tx queue*/
  for (i = 0; i < pdata->tx_q_count; i++)
    AXGMAC_MTL_IOWRITE_BITS(pdata, i, MTL_Q_TQOMR, TXQEN, MTL_Q_ENABLED);
  /* Enable MAC Tx */
 ...
 * mbuf is linear
 */
static int axgbe_xmit_hw(struct axgbe_tx_queue *txq, struct rte_mbuf *mbuf) {
  volatile struct axgbe_tx_desc *desc;
  uint16_t idx;
  uint64_t mask;
  ...
  AXGMAC_SET_BITS_LE(desc->desc2, TX_NORMAL_DESC2, HL_B1L, mbuf->pkt_len);
  /* Total msg length to transmit */
  AXGMAC_SET_BITS_LE(desc->desc3, TX_NORMAL_DESC3, FL, mbuf->pkt_len);
  ...
  if (mbuf->ol_flags & (RTE_MBUF_F_TX_VLAN | RTE_MBUF_F_TX_QINQ)) {
    ...
    AXGMAC_SET_BITS_LE(desc->desc3, TX_CONTEXT_DESC3, CTXT, 1);
    /* Set the VLAN tag */
    AXGMAC_SET_BITS_LE(desc->desc3, TX_CONTEXT_DESC3, VT, mbuf->vlan_tci);
    /* Indicate this descriptor contains the VLAN tag */
    AXGMAC_SET_BITS_LE(desc->desc3, TX_CONTEXT_DESC3, VLTV, 1);
...
out:
  ...
  idx = AXGBE_GET_DESC_IDX(txq, txq->cur);
  tail_addr =
      low32_value(txq->ring_phys_addr + idx * sizeof(struct axgbe_tx_desc));
