...
static inline unsigned int axgbe_get_max_frame(struct axgbe_port *pdata) {
  return pdata->eth_dev->data->mtu + RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN +
         RTE_VLAN_HLEN;
}
...
static int axgbe_write_ext_mii_regs(struct axgbe_port *pdata, int addr, int reg,
                                    u16 val) {
...
static int axgbe_disable_tx_flow_control(struct axgbe_port *pdata) {
  ...
  q_count = RTE_MIN(pdata->tx_q_count, max_q_count);
  reg = MAC_Q0TFCR;
  for (i = 0; i < q_count; i++) {
...
static void axgbe_config_flow_control_threshold(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->rx_q_count; i++) {
    AXGMAC_MTL_IOWRITE_BITS(pdata, i, MTL_Q_RQFCR, RFA, pdata->rx_rfa[i]);
    AXGMAC_MTL_IOWRITE_BITS(pdata, i, MTL_Q_RQFCR, RFD, pdata->rx_rfd[i]);
  }
}
...
static int axgbe_update_vlan_hash_table(struct axgbe_port *pdata) {
  ...
  for (vid = 0; vid < VLAN_N_VID; vid++) {
    ...
    if (vid_valid & 1)
      PMD_DRV_LOG(DEBUG, "vid:%d pdata->active_vlans[%ld]=0x%lx\n", vid,
                  vid_idx, pdata->active_vlans[vid_idx]);
    else
      continue;
...
static int axgbe_flush_tx_queues(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->tx_q_count; i++) {
    count = 2000;
    while (--count && AXGMAC_MTL_IOREAD_BITS(pdata, i, MTL_Q_TQOMR, FTQ))
      rte_delay_us(500);
...
static int axgbe_config_osp_mode(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->eth_dev->data->nb_tx_queues; i++) {
    ...
    AXGMAC_DMA_IOWRITE_BITS(txq, DMA_CH_TCR, OSP, pdata->tx_osp_mode);
  }
...
static int axgbe_config_pblx8(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->eth_dev->data->nb_tx_queues; i++) {
    txq = pdata->eth_dev->data->tx_queues[i];
    AXGMAC_DMA_IOWRITE_BITS(txq, DMA_CH_CR, PBLX8, pdata->pblx8);
  }
  return 0;
}
...
static int axgbe_config_tx_pbl_val(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->eth_dev->data->nb_tx_queues; i++) {
    txq = pdata->eth_dev->data->tx_queues[i];
    AXGMAC_DMA_IOWRITE_BITS(txq, DMA_CH_TCR, PBL, pdata->tx_pbl);
  }
...
static int axgbe_config_rx_pbl_val(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->eth_dev->data->nb_rx_queues; i++) {
    ...
    AXGMAC_DMA_IOWRITE_BITS(rxq, DMA_CH_RCR, PBL, pdata->rx_pbl);
  }
...
static void axgbe_config_rx_buffer_size(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->eth_dev->data->nb_rx_queues; i++) {
    ...
    rxq->buf_size =
        (rxq->buf_size + AXGBE_RX_BUF_ALIGN - 1) & ~(AXGBE_RX_BUF_ALIGN - 1);
...
int axgbe_write_rss_hash_key(struct axgbe_port *pdata) {
  ...
  while (key_regs--) {
    ret = axgbe_write_rss_reg(pdata, AXGBE_RSS_HASH_KEY_TYPE, key_regs, *key++);
    if (ret)
      return ret;
  }
...
int axgbe_write_rss_lookup_table(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < ARRAY_SIZE(pdata->rss_table); i++) {
    ret = axgbe_write_rss_reg(pdata, AXGBE_RSS_LOOKUP_TABLE_TYPE, i,
                              pdata->rss_table[i]);
    if (ret)
      return ret;
  }
...
static void wrapper_tx_desc_init(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->eth_dev->data->nb_tx_queues; i++) {
    ...
    AXGMAC_DMA_IOWRITE(txq, DMA_CH_TDLR_HI, high32_value(txq->ring_phys_addr));
    AXGMAC_DMA_IOWRITE(txq, DMA_CH_TDLR_LO, low32_value(txq->ring_phys_addr));
  }
}
...
static int wrapper_rx_desc_init(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->eth_dev->data->nb_rx_queues; i++) {
    ...
    AXGMAC_DMA_IOWRITE(rxq, DMA_CH_RDRLR, rxq->nb_desc - 1);
    /* Update the starting address of descriptor ring */
    AXGMAC_DMA_IOWRITE(rxq, DMA_CH_RDLR_HI, high32_value(rxq->ring_phys_addr));
    AXGMAC_DMA_IOWRITE(rxq, DMA_CH_RDLR_LO, low32_value(rxq->ring_phys_addr));
    /* Update the Rx Descriptor Tail Pointer */
    AXGMAC_DMA_IOWRITE(
        rxq, DMA_CH_RDTR_LO,
        low32_value(rxq->ring_phys_addr +
                    (rxq->nb_desc - 1) * sizeof(union axgbe_rx_desc)));
  }
  return 0;
}
...
static void axgbe_config_mtl_mode(struct axgbe_port *pdata) {
  ...
  for (i = 0; i < pdata->hw_feat.tc_cnt; i++) {
    AXGMAC_MTL_IOWRITE_BITS(pdata, i, MTL_TC_ETSCR, TSA, MTL_TSA_ETS);
...
static void axgbe_config_rx_fifo_size(struct axgbe_port *pdata) {
  ...
  fifo_size = RTE_MIN(pdata->rx_max_fifo_size, pdata->hw_feat.rx_fifo_size);
...
static void axgbe_config_tx_fifo_size(struct axgbe_port *pdata) {
  ...
  fifo_size = RTE_MIN(pdata->tx_max_fifo_size, pdata->hw_feat.tx_fifo_size);
...
static void axgbe_config_queue_mapping(struct axgbe_port *pdata) {
  ...
  for (i = 0, queue = 0; i < pdata->hw_feat.tc_cnt; i++) {
    for (j = 0; j < qptc; j++) {
      ...
      AXGMAC_MTL_IOWRITE_BITS(pdata, queue, MTL_Q_TQOMR, Q2TCMAP, i);
    }
    if (i < qptc_extra) {
      PMD_DRV_LOG(DEBUG, "TXq%u mapped to TC%u\n", queue, i);
      AXGMAC_MTL_IOWRITE_BITS(pdata, queue, MTL_Q_TQOMR, Q2TCMAP, i);
...
static int axgbe_set_mac_address(struct axgbe_port *pdata, u8 *addr) {
  ...
  mac_addr_lo =
      (addr[3] << 24) | (addr[2] << 16) | (addr[1] << 8) | (addr[0] << 0);
