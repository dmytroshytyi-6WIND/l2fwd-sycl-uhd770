...
static int avp_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
                                  uint16_t nb_rx_desc, unsigned int socket_id,
                                  const struct rte_eth_rxconf *rx_conf,
                                  struct rte_mempool *pool);

static int avp_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id,
...
static uint16_t avp_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                              uint16_t nb_pkts);
...
static uint16_t avp_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                              uint16_t nb_pkts);
...
#define AVP_DEV_PRIVATE_TO_HW(adapter) (&((struct avp_adapter *)adapter)->avp)
 ...
 */
static int avp_dev_process_request(struct avp_dev *avp,
  ...
  if ((count != 1) || (resp_addr != avp->host_sync_addr)) {
    PMD_DRV_LOG(
        ERR, "Invalid response from host, count=%u resp=%p host_sync_addr=%p\n",
...
static int avp_dev_ctrl_set_link_state(struct rte_eth_dev *eth_dev,
                                       unsigned int state) {
...
static int avp_dev_ctrl_set_config(struct rte_eth_dev *eth_dev,
...
static int avp_dev_ctrl_shutdown(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  struct rte_avp_request request;
  int ret;
...
static inline void *avp_dev_translate_buffer(struct avp_dev *avp,
                                             void *host_mbuf_address) {
  return RTE_PTR_ADD(
      RTE_PTR_SUB(host_mbuf_address, (uintptr_t)avp->host_mbuf_addr),
      (uintptr_t)avp->mbuf_addr);
}

/* translate from host physical address to guest virtual address */
static void *avp_dev_translate_address(struct rte_eth_dev *eth_dev,
...
static int avp_dev_version_check(uint32_t version) {
...
static int avp_dev_check_regions(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  ...
  for (i = 0; i < PCI_MAX_RESOURCE; i++) {
    ...
    PMD_DRV_LOG(DEBUG,
                ...
                resource->phys_addr, resource->len, resource->addr);
    ...
    case RTE_AVP_PCI_DEVICE_BAR:
      ...
      if ((info->magic != RTE_AVP_DEVICE_MAGIC) ||
        ...
        PMD_DRV_LOG(
            ...
            info->magic, info->version, AVP_DPDK_DRIVER_VERSION);
        return -EINVAL;
      }
      break;
...
static int avp_dev_detach(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  int ret;
...
static void _avp_set_rx_queue_mappings(struct rte_eth_dev *eth_dev,
  ...
  PMD_DRV_LOG(DEBUG, "rxq %u at %p base %u limit %u\n", rx_queue_id, rxq,
              rxq->queue_base, rxq->queue_limit);

  rxq->queue_id = rxq->queue_base;
}

static void _avp_set_queue_counts(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
   ...
   */
  avp->num_rx_queues =
      RTE_MAX(host_info->min_rx_queues, eth_dev->data->nb_rx_queues);

  PMD_DRV_LOG(DEBUG, "Requesting %u Tx and %u Rx queues from host\n",
              avp->num_tx_queues, avp->num_rx_queues);
}

static int avp_dev_attach(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  struct rte_avp_device_config config;
  unsigned int i;
  int ret;
...
static void avp_dev_interrupt_handler(void *data) {
   ...
   */
  status = AVP_READ32(RTE_PTR_ADD(registers, RTE_AVP_INTERRUPT_STATUS_OFFSET));
  ...
  if (status & RTE_AVP_MIGRATION_INTERRUPT_MASK) {
    ...
    AVP_WRITE32(value, RTE_PTR_ADD(registers, RTE_AVP_MIGRATION_ACK_OFFSET));
...
static int avp_dev_enable_interrupts(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  void *registers = pci_dev->mem_resource[RTE_AVP_PCI_MMIO_BAR].addr;
  int ret;
...
static int avp_dev_disable_interrupts(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  void *registers = pci_dev->mem_resource[RTE_AVP_PCI_MMIO_BAR].addr;
  int ret;
...
static int avp_dev_setup_interrupts(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  int ret;

  /* register a callback handler with UIO for interrupt notifications */
  ret = rte_intr_callback_register(pci_dev->intr_handle,
                                   avp_dev_interrupt_handler, (void *)eth_dev);
  if (ret < 0) {
...
static int avp_dev_migration_pending(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  void *registers = pci_dev->mem_resource[RTE_AVP_PCI_MMIO_BAR].addr;
  uint32_t value;
...
static int avp_dev_create(struct rte_pci_device *pci_dev,
  ...
  if ((host_info->magic != RTE_AVP_DEVICE_MAGIC) ||
    ...
    PMD_DRV_LOG(
        ERR, "Invalid AVP PCI device, magic 0x%08x version 0x%08x > 0x%08x\n",
        host_info->magic, host_info->version, AVP_DPDK_DRIVER_VERSION);
    return -EINVAL;
  }
  ...
  if (avp->magic != AVP_ETHDEV_MAGIC) {
    ...
    memcpy(&avp->ethaddr.addr_bytes[0], host_info->ethaddr, RTE_ETHER_ADDR_LEN);
    /* adjust max values to not exceed our max */
    avp->max_tx_queues = RTE_MIN(host_info->max_tx_queues, RTE_AVP_MAX_QUEUES);
    avp->max_rx_queues = RTE_MIN(host_info->max_rx_queues, RTE_AVP_MAX_QUEUES);
  } else {
  ...
  for (i = 0; i < avp->max_tx_queues; i++) {
    avp->tx_q[i] = avp_dev_translate_address(
        eth_dev, host_info->tx_phys + (i * host_info->tx_size));

    avp->alloc_q[i] = avp_dev_translate_address(
        eth_dev, host_info->alloc_phys + (i * host_info->alloc_size));
  }
  ...
  for (i = 0; i < avp->max_rx_queues; i++) {
    avp->rx_q[i] = avp_dev_translate_address(
        eth_dev, host_info->rx_phys + (i * host_info->rx_size));
    avp->free_q[i] = avp_dev_translate_address(
        eth_dev, host_info->free_phys + (i * host_info->free_size));
  }
...
static int eth_avp_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                             struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct avp_adapter),
                                       eth_avp_dev_init);
}

static int eth_avp_pci_remove(struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_remove(pci_dev, eth_avp_dev_uninit);
}
...
static int avp_dev_enable_scattered(struct rte_eth_dev *eth_dev,
                                    struct avp_dev *avp) {
  unsigned int max_rx_pktlen;

  max_rx_pktlen = eth_dev->data->mtu + RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN;
...
static int avp_dev_rx_queue_setup(struct rte_eth_dev *eth_dev,
  ...
  PMD_DRV_LOG(DEBUG, "AVP max_rx_pkt_len=(%u,%u) mbuf_size=(%u,%u)\n",
              ...
              avp->host_mbuf_size, avp->guest_mbuf_size);
...
static int avp_dev_tx_queue_setup(struct rte_eth_dev *eth_dev,
  ...
  if (tx_queue_id >= eth_dev->data->nb_tx_queues) {
    PMD_DRV_LOG(
        ERR, "TX queue id is out of range: tx_queue_id=%u, nb_tx_queues=%u\n",
...
static inline int _avp_cmp_ether_addr(struct rte_ether_addr *a,
                                      struct rte_ether_addr *b) {
  uint16_t *_a = (uint16_t *)&a->addr_bytes[0];
  uint16_t *_b = (uint16_t *)&b->addr_bytes[0];
  return (_a[0] ^ _b[0]) | (_a[1] ^ _b[1]) | (_a[2] ^ _b[2]);
}

static inline int _avp_mac_filter(struct avp_dev *avp, struct rte_mbuf *m) {
...
static inline void __avp_dev_buffer_sanity_check(struct avp_dev *avp,
...
avp_dev_copy_from_buffers(struct avp_dev *avp, struct rte_avp_desc *buf,
  ...
  for (i = 0; (i < count) && (buf != NULL); i++) {
    ...
    do {
      ...
      rte_memcpy(
          RTE_PTR_ADD(rte_pktmbuf_mtod(m, void *), rte_pktmbuf_data_len(m)),
...
static uint16_t avp_recv_scattered_pkts(void *rx_queue,
  ...
  rxq->queue_id = (rxq->queue_id < rxq->queue_limit) ? (rxq->queue_id + 1)
                                                     : rxq->queue_base;
 ...
 * packet.
 */
static inline uint16_t avp_dev_copy_to_buffers(struct avp_dev *avp,
...
static uint16_t avp_xmit_scattered_pkts(void *tx_queue,
                                        ...
                                        uint16_t nb_pkts) {
  struct rte_avp_desc
      *avp_bufs[(AVP_MAX_TX_BURST * RTE_AVP_MAX_MBUF_SEGMENTS)] = {};
  ...
  if (unlikely(avail > (AVP_MAX_TX_BURST * RTE_AVP_MAX_MBUF_SEGMENTS)))
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (unlikely((required == 0) || (required > RTE_AVP_MAX_MBUF_SEGMENTS)))
      break;
...
static void avp_dev_rx_queue_release(struct rte_eth_dev *eth_dev,
                                     uint16_t rx_queue_id) {
...
static void avp_dev_rx_queue_release_all(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
...
static void avp_dev_tx_queue_release(struct rte_eth_dev *eth_dev,
...
static void avp_dev_tx_queue_release_all(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  struct rte_eth_dev_data *data = avp->dev_data;
  unsigned int i;
...
static int avp_dev_configure(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  struct rte_avp_device_info *host_info;
  struct rte_avp_device_config config;
...
static int avp_dev_start(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  int ret;
...
static int avp_dev_stop(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  int ret;
...
static int avp_dev_close(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  int ret;
...
static int avp_dev_link_update(struct rte_eth_dev *eth_dev,
...
static int avp_dev_promiscuous_enable(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
...
static int avp_dev_promiscuous_disable(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
...
static int avp_dev_info_get(struct rte_eth_dev *eth_dev,
                            struct rte_eth_dev_info *dev_info) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
...
static int avp_vlan_offload_set(struct rte_eth_dev *eth_dev, int mask) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
...
static int avp_dev_stats_get(struct rte_eth_dev *eth_dev,
                             struct rte_eth_stats *stats) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  unsigned int i;
...
static int avp_dev_stats_reset(struct rte_eth_dev *eth_dev) {
  struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  unsigned int i;
