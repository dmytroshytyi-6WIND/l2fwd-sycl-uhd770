...
#define MRVL_RX_OFFLOADS                                                       \
  (RTE_ETH_RX_OFFLOAD_VLAN_FILTER | RTE_ETH_RX_OFFLOAD_CHECKSUM)

/** Port Tx offloads capabilities */
#define MRVL_TX_OFFLOAD_CHECKSUM                                               \
  (RTE_ETH_TX_OFFLOAD_IPV4_CKSUM | RTE_ETH_TX_OFFLOAD_UDP_CKSUM |              \
   RTE_ETH_TX_OFFLOAD_TCP_CKSUM)
#define MRVL_TX_OFFLOADS                                                       \
  (MRVL_TX_OFFLOAD_CHECKSUM | RTE_ETH_TX_OFFLOAD_MULTI_SEGS)

#define MRVL_TX_PKT_OFFLOADS                                                   \
  (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_TCP_CKSUM | RTE_MBUF_F_TX_UDP_CKSUM)

static const char *const valid_args[] = {MRVL_IFACE_NAME_ARG, MRVL_CFG_ARG,
                                         NULL};

static int used_hifs = MRVL_MUSDK_HIFS_RESERVED;
static struct pp2_hif *hifs[RTE_MAX_LCORE];
static int used_bpools[PP2_NUM_PKT_PROC] = {[0 ... PP2_NUM_PKT_PROC - 1] =
                                                MRVL_MUSDK_BPOOLS_RESERVED};

static struct pp2_bpool *mrvl_port_to_bpool_lookup[RTE_MAX_ETHPORTS];
static int mrvl_port_bpool_size[PP2_NUM_PKT_PROC][PP2_BPOOL_NUM_POOLS]
                               [RTE_MAX_LCORE];
...
static inline void mrvl_free_sent_buffers(struct pp2_ppio *ppio,
                                          ...
                                          struct mrvl_shadow_txq *sq, int qid,
                                          int force);
...
static int mrvl_mac_addr_add(struct rte_eth_dev *dev,
                             struct rte_ether_addr *mac_addr, uint32_t index,
                             uint32_t vmdq __rte_unused);
static int mrvl_mac_addr_set(struct rte_eth_dev *dev,
                             struct rte_ether_addr *mac_addr);
static int mrvl_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                int on);
static int mrvl_promiscuous_enable(struct rte_eth_dev *dev);
static int mrvl_allmulticast_enable(struct rte_eth_dev *dev);
static int mrvl_flow_ctrl_set(struct rte_eth_dev *dev,
...
static struct {
  ...
  unsigned int offset;
  unsigned int size;
} mrvl_xstats_tbl[] = {MRVL_XSTATS_TBL_ENTRY(rx_bytes),
...
static inline int mrvl_reserve_bit(int *bitmap, int max) {
...
static int mrvl_pp2_fixup_init(void) {
  ...
  for (i = 0; i < pp2_get_num_inst(); i++) {
    dummy_pool_id[i] = mrvl_reserve_bit(&used_bpools[i], PP2_BPOOL_NUM_POOLS);
...
static void mrvl_pp2_fixup_deinit(void) {
  int i;
...
static inline void mrvl_fill_shadowq(struct mrvl_shadow_txq *sq,
                                     struct rte_mbuf *buf) {
  sq->ent[sq->head].buff.cookie = (uint64_t)buf;
  sq->ent[sq->head].buff.addr = buf ? rte_mbuf_data_iova_default(buf) : 0;

  sq->ent[sq->head].bpool =
      (unlikely(!buf || buf->port >= RTE_MAX_ETHPORTS || buf->refcnt > 1))
          ? NULL
...
static inline void mrvl_fill_desc(struct pp2_ppio_desc *desc,
                                  struct rte_mbuf *buf) {
...
static inline int mrvl_get_bpool_size(int pp2_id, int pool_id) {
  int i;
  int size = 0;
...
static int mrvl_init_hif(int core_id) {
...
static inline struct pp2_hif *mrvl_get_hif(struct mrvl_priv *priv,
                                           int core_id) {
  int ret;
 ...
 */
static void mrvl_set_tx_function(struct rte_eth_dev *dev) {
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_configure_rss(struct mrvl_priv *priv,
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_dev_configure(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  int ret;
  ...
  if (dev->data->dev_conf.rxmode.mtu > priv->max_mtu) {
    MRVL_LOG(ERR, "MTU %u is larger than max_mtu %u\n",
             dev->data->dev_conf.rxmode.mtu, priv->max_mtu);
    return -EINVAL;
  }
  ...
  ret = mrvl_configure_rxqs(priv, dev->data->port_id, dev->data->nb_rx_queues);
  if (ret < 0)
    return ret;

  ret = mrvl_configure_txqs(priv, dev->data->port_id, dev->data->nb_tx_queues);
  if (ret < 0)
    return ret;
  ...
  ret = mrvl_configure_rss(priv, &dev->data->dev_conf.rx_adv_conf.rss_conf);
  if (ret < 0)
    return ret;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct mrvl_priv *priv = dev->data->dev_private;
  uint16_t mru;
  ...
  if (mru - RTE_ETHER_CRC_LEN + MRVL_PKT_OFFS > mbuf_data_size) {
    ...
    MRVL_LOG(WARNING,
             "MTU too big, max MTU possible limited "
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_dev_set_link_up(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  int ret;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_dev_set_link_down(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  int ret;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_tx_queue_start(struct rte_eth_dev *dev, uint16_t queue_id) {
  struct mrvl_priv *priv = dev->data->dev_private;
  int ret;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_tx_queue_stop(struct rte_eth_dev *dev, uint16_t queue_id) {
  struct mrvl_priv *priv = dev->data->dev_private;
  int ret;
 ...
 */
static int mrvl_dev_start(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  char match[MRVL_MATCH_LEN];
   ...
   */
  priv->bpool_max_size =
      (priv->nb_rx_queues * MRVL_PP2_RXD_MAX) + priv->bpool_min_size;
  ...
  if (!priv->uc_mc_flushed) {
    ...
    if (ret) {
      MRVL_LOG(ERR, "Failed to flush uc/mc filter list");
      goto out;
    }
 ...
 *   Pointer to Ethernet device structure.
 */
static void mrvl_flush_rx_queues(struct rte_eth_dev *dev) {
  int i;
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    do {
      ...
      ret = pp2_ppio_recv(q->priv->ppio, q->priv->rxq_map[q->queue_id].tc,
                          q->priv->rxq_map[q->queue_id].inq, descs,
                          (uint16_t *)&num);
    } while (ret == 0 && num);
  }
}
 ...
 *   Pointer to Ethernet device structure.
 */
static void mrvl_flush_tx_shadow_queues(struct rte_eth_dev *dev) {
  int i, j;
  struct mrvl_txq *txq;
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    for (j = 0; j < RTE_MAX_LCORE; j++) {
      ...
      mrvl_free_sent_buffers(txq->priv->ppio, hifs[j], j, sq, txq->queue_id, 1);
 ...
 *   Pointer to Ethernet device structure.
 */
static void mrvl_flush_bpool(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct pp2_hif *hif;
  uint32_t num;
  int ret;
 ...
 *   Pointer to Ethernet device structure.
 */
static int mrvl_dev_stop(struct rte_eth_dev *dev) {
 ...
 *   Pointer to Ethernet device structure.
 */
static int mrvl_dev_close(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  size_t i;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_link_update(struct rte_eth_dev *dev,
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_promiscuous_disable(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  int ret;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_allmulticast_disable(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  int ret;
 ...
 *   MAC address index.
 */
static void mrvl_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
  struct mrvl_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_stats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    if (unlikely(idx >= RTE_ETHDEV_QUEUE_STAT_CNTRS)) {
      MRVL_LOG(ERR, "rx queue %d stats out of range (0 - %d)", idx,
               RTE_ETHDEV_QUEUE_STAT_CNTRS - 1);
      continue;
    }

    ret = pp2_ppio_inq_get_statistics(priv->ppio, priv->rxq_map[idx].tc,
                                      priv->rxq_map[idx].inq, &rx_stats, 0);
    if (unlikely(ret)) {
      MRVL_LOG(ERR, "Failed to update rx queue %d stats", idx);
      break;
    }

    stats->q_ibytes[idx] = rxq->bytes_recv;
    stats->q_ipackets[idx] = rx_stats.enq_desc - rxq->drop_mac;
    stats->q_errors[idx] = rx_stats.drop_early + rx_stats.drop_fullq +
                           rx_stats.drop_bm + rxq->drop_mac;
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    idx = txq->queue_id;
    if (unlikely(idx >= RTE_ETHDEV_QUEUE_STAT_CNTRS)) {
      MRVL_LOG(ERR, "tx queue %d stats out of range (0 - %d)", idx,
               RTE_ETHDEV_QUEUE_STAT_CNTRS - 1);
    }

    ret = pp2_ppio_outq_get_statistics(priv->ppio, idx, &tx_stats, 0);
    if (unlikely(ret)) {
      MRVL_LOG(ERR, "Failed to update tx queue %d stats", idx);
      break;
    }
  ...
  stats->imissed += ppio_stats.rx_fullq_dropped + ppio_stats.rx_bm_dropped +
                    ppio_stats.rx_early_dropped + ppio_stats.rx_fifo_dropped +
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_stats_reset(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  int i;
 ...
 */
static int mrvl_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *stats,
                           unsigned int n) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct pp2_ppio_statistics ppio_stats;
  unsigned int i;
  ...
  for (i = 0; i < n && i < RTE_DIM(mrvl_xstats_tbl); i++) {
    ...
    if (mrvl_xstats_tbl[i].size == sizeof(uint32_t))
      val = *(uint32_t *)((uint8_t *)&ppio_stats + mrvl_xstats_tbl[i].offset);
    else if (mrvl_xstats_tbl[i].size == sizeof(uint64_t))
      val = *(uint64_t *)((uint8_t *)&ppio_stats + mrvl_xstats_tbl[i].offset);
    else
      return -EINVAL;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_xstats_reset(struct rte_eth_dev *dev) {
 ...
 */
static int mrvl_xstats_get_names(struct rte_eth_dev *dev __rte_unused,
 ...
 */
static int mrvl_dev_infos_get(struct rte_eth_dev *dev,
...
mrvl_dev_supported_ptypes_get(struct rte_eth_dev *dev __rte_unused) {
  static const uint32_t ptypes[] = {
      ...
      RTE_PTYPE_L3_IPV4_EXT,   RTE_PTYPE_L3_IPV4_EXT_UNKNOWN,
      RTE_PTYPE_L3_IPV6,       RTE_PTYPE_L3_IPV6_EXT,
      RTE_PTYPE_L2_ETHER_ARP,  RTE_PTYPE_L4_TCP,
      RTE_PTYPE_L4_UDP};
...
static int mrvl_fill_bpool(struct mrvl_rxq *rxq, int num) {
  ...
  for (i = 0; i < num; i++) {
    if (((uint64_t)mbufs[i] & MRVL_COOKIE_HIGH_ADDR_MASK) != cookie_addr_high) {
      MRVL_LOG(ERR,
    ...
    entries[i].buff.addr = rte_mbuf_data_iova_default(mbufs[i]);
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_rx_queue_setup(struct rte_eth_dev *dev, uint16_t idx,
  ...
  if (priv->rxq_map[idx].tc == MRVL_UNKNOWN_TC) {
     ...
     */
    MRVL_LOG(ERR, "Unknown TC mapping for queue %hu eth%hhu", idx,
             priv->ppio_id);
    return -EFAULT;
  }
  ...
  tc = priv->rxq_map[rxq->queue_id].tc, inq = priv->rxq_map[rxq->queue_id].inq;
  priv->ppio_params.inqs_params.tcs_params[tc].inqs_params[inq].size = desc;
 ...
 *   Receive queue index.
 */
static void mrvl_rx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_tx_queue_setup(struct rte_eth_dev *dev, uint16_t idx,
                               uint16_t desc, unsigned int socket,
 ...
 *   Transmit queue index.
 */
static void mrvl_tx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
 ...
 *  0 on success, negative error value otherwise.
 */
static int mrvl_flow_ctrl_get(struct rte_eth_dev *dev,
                              struct rte_eth_fc_conf *fc_conf) {
  struct mrvl_priv *priv = dev->data->dev_private;
  int ret, en;
...
static int mrvl_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  case RTE_ETH_FC_NONE:
    ...
    break;
  default:
    MRVL_LOG(ERR, "Incorrect Flow control flag (%d)", fc_conf->mode);
    return -EINVAL;
  }
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_rss_hash_update(struct rte_eth_dev *dev,
                                struct rte_eth_rss_conf *rss_conf) {
  struct mrvl_priv *priv = dev->data->dev_private;
 ...
 *   Always 0.
 */
static int mrvl_rss_hash_conf_get(struct rte_eth_dev *dev,
                                  struct rte_eth_rss_conf *rss_conf) {
  struct mrvl_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_eth_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
 ...
 *   Always 0.
 */
static int mrvl_mtr_ops_get(struct rte_eth_dev *dev __rte_unused, void *ops) {
 ...
 *   Always 0.
 */
static int mrvl_tm_ops_get(struct rte_eth_dev *dev __rte_unused, void *ops) {
 ...
 *   Mbuf offload flags.
 */
static inline uint64_t mrvl_desc_to_ol_flags(struct pp2_ppio_desc *desc,
                                             uint64_t packet_type) {
  uint64_t flags = 0;
 ...
 */
static uint16_t mrvl_rx_pkt_burst(void *rxq, struct rte_mbuf **rx_pkts,
                                  uint16_t nb_pkts) {
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (likely(nb_pkts - i > MRVL_MUSDK_PREFETCH_SHIFT)) {
      ...
      pref_addr = cookie_addr_high | pp2_ppio_inq_desc_get_cookie(pref_desc);
    ...
    if ((unlikely(status != PP2_DESC_ERR_OK)) &&
      ...
      mrvl_port_bpool_size[bpool->pp2_id][bpool->id][core_id]++;
      q->drop_mac++;
      continue;
    }
    ...
    mbuf->port = q->port_id;
    mbuf->packet_type =
        mrvl_desc_to_packet_type_and_offset(&descs[i], &l3_offset, &l4_offset);
    mbuf->l2_len = l3_offset;
    mbuf->l3_len = l4_offset - l3_offset;

    if (likely(q->cksum_enabled))
      mbuf->ol_flags = mrvl_desc_to_ol_flags(&descs[i], mbuf->packet_type);
  ...
  if (rte_spinlock_trylock(&q->priv->lock) == 1) {
    ...
    if (unlikely(num <= q->priv->bpool_min_size ||
      ...
      for (i = 0; i < pkt_to_remove; i++) {
        ...
        if (ret)
          break;
        mbuf = (struct rte_mbuf *)(cookie_addr_high | buff.cookie);
 ...
 */
static inline void mrvl_prepare_proto_info(uint64_t ol_flags,
...
static inline void mrvl_free_sent_buffers(struct pp2_ppio *ppio,
  ...
  for (i = 0; i < nb_done; i++) {
    ...
    if (unlikely(!entry->buff.addr)) {
      MRVL_LOG(ERR, "Shadow memory @%d: cookie(%lx), pa(%lx)!", sq->tail,
               (u64)entry->buff.cookie, (u64)entry->buff.addr);
      skip_bufs = 1;
      goto skip;
    }
    ...
    mrvl_port_bpool_size[entry->bpool->pp2_id][entry->bpool->id][core_id]++;
    num++;
...
static uint16_t mrvl_tx_pkt_burst(void *txq, struct rte_mbuf **tx_pkts,
  ...
  if (sq->size)
    mrvl_free_sent_buffers(q->priv->ppio, hif, core_id, sq, q->queue_id, 0);
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (!(mbuf->ol_flags & MRVL_TX_PKT_OFFLOADS))
      continue;
    mrvl_prepare_proto_info(mbuf->ol_flags, &l3_type, &l4_type, &gen_l3_cksum,
                            &gen_l4_cksum);

    pp2_ppio_outq_desc_set_proto_info(&descs[i], l3_type, l4_type, mbuf->l2_len,
                                      mbuf->l2_len + mbuf->l3_len, gen_l3_cksum,
                                      gen_l4_cksum);
  }
  ...
  if (unlikely(num > nb_pkts)) {
    for (i = nb_pkts; i < num; i++) {
      sq->head =
          (MRVL_PP2_TX_SHADOWQ_SIZE + sq->head - 1) & MRVL_PP2_TX_SHADOWQ_MASK;
...
static uint16_t mrvl_tx_sg_pkt_burst(void *txq, struct rte_mbuf **tx_pkts,
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (nb_segs > PP2_PPIO_DESC_NUM_FRAGS) {
      total_descs -= nb_segs;
      RTE_LOG(ERR, PMD, "Too many segments. Packet won't be sent.\n");
      break;
    }
    ...
    pp2_ppio_outq_desc_set_proto_info(&descs[tail_first], l3_type, l4_type,
                                      mbuf->l2_len, mbuf->l2_len + mbuf->l3_len,
                                      gen_l3_cksum, gen_l4_cksum);
  }

  num = total_descs;
  pp2_ppio_send_sg(q->priv->ppio, hif, q->queue_id, descs, &total_descs, &pkts);
  /* number of packets that were not sent */
  if (unlikely(num > total_descs)) {
    for (i = total_descs; i < num; i++) {
      ...
      if (addr)
        bytes_sent -=
            rte_pktmbuf_pkt_len((struct rte_mbuf *)(cookie_addr_high | addr));
    }
 ...
 */
static struct mrvl_priv *mrvl_priv_create(const char *dev_name) {
  struct pp2_bpool_params bpool_params;
  char match[MRVL_MATCH_LEN];
  struct mrvl_priv *priv;
  ...
  ret = pp2_netdev_get_ppio_info((char *)(uintptr_t)dev_name, &priv->pp_id,
                                 &priv->ppio_id);
  if (ret)
    goto out_free_priv;
  ...
  priv->max_mtu = max_frame_size + RTE_ETHER_CRC_LEN - MRVL_PP2_ETH_HDRS_LEN;

  bpool_bit = mrvl_reserve_bit(&used_bpools[priv->pp_id], PP2_BPOOL_NUM_POOLS);
  if (bpool_bit < 0)
    goto out_free_priv;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mrvl_eth_dev_create(struct rte_vdev_device *vdev, const char *name) {
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("mac_addrs", RTE_ETHER_ADDR_LEN * MRVL_MAC_ADDRS_MAX, 0);
 ...
 *   Always 0.
 */
static int mrvl_get_ifnames(const char *key __rte_unused, const char *value,
                            void *extra_args) {
 ...
 *   0 on success, negative error value otherwise.
 */
static int rte_pmd_mrvl_probe(struct rte_vdev_device *vdev) {
  ...
  ifnames.idx = 0;
  rte_kvargs_process(kvlist, MRVL_IFACE_NAME_ARG, mrvl_get_ifnames, &ifnames);
  ...
  if (!mrvl_cfg) {
    ...
    if (cfgnum > 1) {
      ...
      rte_kvargs_process(kvlist, MRVL_CFG_ARG, mrvl_get_cfg, &mrvl_cfg);
    }
  }
