 ...
 */
static int mrvl_get_max_rate(struct rte_eth_dev *dev, uint64_t *rate) {
...
void mrvl_tm_deinit(struct rte_eth_dev *dev) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_tm_shaper_profile *profile = LIST_FIRST(&priv->shaper_profiles);
 ...
 */
static struct mrvl_tm_node *mrvl_node_from_id(struct mrvl_priv *priv,
                                              uint32_t node_id) {
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_node_type_get(struct rte_eth_dev *dev, uint32_t node_id,
                              int *is_leaf, struct rte_tm_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_tm_node *node;

  if (!priv->configured)
    return -rte_tm_error_set(error, ENODEV, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
                             "Port didn't configured\n");
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_capabilities_get(struct rte_eth_dev *dev,
  ...
  cap->dynamic_update_mask =
      RTE_TM_UPDATE_NODE_SUSPEND_RESUME | RTE_TM_UPDATE_NODE_STATS;
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_level_capabilities_get(struct rte_eth_dev *dev,
                                       uint32_t level_id,
  ...
  if (level_id == MRVL_NODE_PORT) {
    ...
    cap->nonleaf.sched_wfq_n_children_per_group_max = dev->data->nb_tx_queues;
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_node_capabilities_get(struct rte_eth_dev *dev, uint32_t node_id,
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_shaper_profile_add(struct rte_eth_dev *dev,
                                   uint32_t shaper_profile_id,
  ...
  if (params->peak.rate < MRVL_RATE_MIN || params->peak.rate > priv->rate_max)
    return -rte_tm_error_set(error, EINVAL,
                             RTE_TM_ERROR_TYPE_SHAPER_PROFILE_PEAK_RATE, NULL,
                             "Peak rate is out of range\n");

  if (params->peak.size < MRVL_BURST_MIN || params->peak.size > MRVL_BURST_MAX)
    return -rte_tm_error_set(error, EINVAL,
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_shaper_profile_delete(struct rte_eth_dev *dev,
                                      uint32_t shaper_profile_id,
                                      struct rte_tm_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_tm_shaper_profile *profile;
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_node_check_params(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  if (params->leaf.cman)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_NODE_PARAMS_CMAN,
                             NULL, "Congestion mngmt is not supported\n");
  ...
  if (params->leaf.wred.shared_wred_context_id)
    return -rte_tm_error_set(
        error, EINVAL, RTE_TM_ERROR_TYPE_NODE_PARAMS_SHARED_WRED_CONTEXT_ID,
        NULL, "WRED is not supported\n");
  ...
  if (params->stats_mask & ~RTE_TM_STATS_N_PKTS)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_NODE_PARAMS_STATS,
                             NULL, "Requested txq stats are not supported\n");
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_node_add(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  ret = mrvl_node_check_params(dev, node_id, priority, weight, level_id, params,
                               error);
  if (ret)
    return ret;

  if (params->shaper_profile_id != RTE_TM_SHAPER_PROFILE_ID_NONE) {
    profile = mrvl_shaper_profile_from_id(priv, params->shaper_profile_id);
    if (!profile)
  ...
  node->id = node_id;
  node->type =
      parent_node_id == RTE_TM_NODE_ID_NULL ? MRVL_NODE_PORT : MRVL_NODE_QUEUE;
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_node_delete(struct rte_eth_dev *dev, uint32_t node_id,
                            struct rte_tm_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_tm_node *node;
 ...
 * returns 0 on success, negative value otherwise.
 */
static int mrvl_node_suspend(struct rte_eth_dev *dev, uint32_t node_id,
                             struct rte_tm_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
 ...
 * returns 0 on success, negative value otherwise.
 */
static int mrvl_node_resume(struct rte_eth_dev *dev, uint32_t node_id,
                            struct rte_tm_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_tm_node *node;
  int ret;
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
                                 struct rte_tm_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_tm_node *node;
  int ret;
  ...
  if (priv->ppio) {
    ret = -rte_tm_error_set(error, EPERM, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
                            "Port is already started\n");
    goto out;
  }
  ...
  LIST_FOREACH(node, &priv->nodes, next) {
    ...
    if (node->type == MRVL_NODE_PORT) {
      ...
      MRVL_LOG(INFO, "Port rate limit overrides txqs rate limit");
    ...
    if (node->profile) {
      ...
      p->rate_limit_params.cir = node->profile->params.peak.rate * 8 / 1000;
      /* convert bits to kilo bits */
      p->rate_limit_params.cbs = node->profile->params.peak.size / 1000;
    } else {
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_node_stats_read(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  if (!node->parent) {
    ...
    memset(&s, 0, sizeof(s));
    ret = pp2_ppio_outq_get_statistics(priv->ppio, node_id, &s, clear);
    if (ret)
      return -rte_tm_error_set(error, -ret, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_node_stats_update(struct rte_eth_dev *dev, uint32_t node_id,
                                  uint64_t stats_mask,
                                  struct rte_tm_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_tm_node *node;
