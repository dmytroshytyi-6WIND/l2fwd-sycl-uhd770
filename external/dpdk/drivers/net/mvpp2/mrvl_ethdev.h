...
#define MRVL_PP2_ETH_HDRS_LEN                                                  \
  (RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN + (2 * RTE_VLAN_HLEN))
 ...
 */
static int get_val_securely(const char *string, uint32_t *val) {
  char *endptr;
...
static int get_val_securely8(const char *string, uint32_t base, uint8_t *val) {
  char *endptr;
  size_t len = strlen(string);
