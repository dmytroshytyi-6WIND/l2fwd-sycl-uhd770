 ...
 */
static int mrvl_alloc_key_mask(struct pp2_cls_rule_key_field *field) {
 ...
 * @param field Pointer to the classifier rule.
 */
static void mrvl_free_key_mask(struct pp2_cls_rule_key_field *field) {
  rte_free(field->key);
  rte_free(field->mask);
  field->key = NULL;
  field->mask = NULL;
 ...
 */
static void mrvl_free_all_key_mask(struct pp2_cls_tbl_rule *rule) {
  int i;
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_parse_init(const struct rte_flow_item *item,
                           const void **spec_ptr, const void **mask_ptr,
   ...
   */
  if (last != NULL && !memcmp(last, zeros, size) &&
 ...
 */
static int mrvl_parse_mac(const struct rte_flow_item_eth *spec,
                          const struct rte_flow_item_eth *mask, int parse_dst,
  ...
  snprintf((char *)key_field->key, MRVL_CLS_STR_SIZE_MAX,
           RTE_ETHER_ADDR_PRT_FMT, k[0], k[1], k[2], k[3], k[4], k[5]);

  snprintf((char *)key_field->mask, MRVL_CLS_STR_SIZE_MAX,
           RTE_ETHER_ADDR_PRT_FMT, m[0], m[1], m[2], m[3], m[4], m[5]);

  flow->table_key.proto_field[flow->rule.num_fields].proto = MV_NET_PROTO_ETH;
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_dmac(const struct rte_flow_item_eth *spec,
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_smac(const struct rte_flow_item_eth *spec,
                                  const struct rte_flow_item_eth *mask,
                                  struct rte_flow *flow) {
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static int mrvl_parse_type(const struct rte_flow_item_eth *spec,
...
mrvl_parse_vlan_id(const struct rte_flow_item_vlan *spec,
  ...
  flow->table_key.proto_field[flow->rule.num_fields].proto = MV_NET_PROTO_VLAN;
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static int mrvl_parse_ip4_dscp(const struct rte_flow_item_ipv4 *spec,
  ...
  flow->table_key.proto_field[flow->rule.num_fields].proto = MV_NET_PROTO_IP4;
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static int mrvl_parse_ip4_addr(const struct rte_flow_item_ipv4 *spec,
                               const struct rte_flow_item_ipv4 *mask,
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_ip4_dip(const struct rte_flow_item_ipv4 *spec,
                                     const struct rte_flow_item_ipv4 *mask,
                                     struct rte_flow *flow) {
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_ip4_sip(const struct rte_flow_item_ipv4 *spec,
                                     const struct rte_flow_item_ipv4 *mask,
                                     struct rte_flow *flow) {
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static int mrvl_parse_ip6_addr(const struct rte_flow_item_ipv6 *spec,
  ...
  flow->table_key.proto_field[flow->rule.num_fields].proto = MV_NET_PROTO_IP6;
  flow->table_key.key_size += key_field->size;
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_ip6_dip(const struct rte_flow_item_ipv6 *spec,
                                     const struct rte_flow_item_ipv6 *mask,
                                     struct rte_flow *flow) {
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_ip6_sip(const struct rte_flow_item_ipv6 *spec,
                                     const struct rte_flow_item_ipv6 *mask,
                                     struct rte_flow *flow) {
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static int mrvl_parse_ip6_flow(const struct rte_flow_item_ipv6 *spec,
                               const struct rte_flow_item_ipv6 *mask,
                               struct rte_flow *flow) {
  struct pp2_cls_rule_key_field *key_field;
...
mrvl_parse_tcp_port(const struct rte_flow_item_tcp *spec,
  ...
  flow->table_key.proto_field[flow->rule.num_fields].proto = MV_NET_PROTO_TCP;
  flow->table_key.key_size += key_field->size;
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_tcp_sport(const struct rte_flow_item_tcp *spec,
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_tcp_dport(const struct rte_flow_item_tcp *spec,
                                       const struct rte_flow_item_tcp *mask,
                                       struct rte_flow *flow) {
...
mrvl_parse_udp_port(const struct rte_flow_item_udp *spec,
  ...
  flow->table_key.proto_field[flow->rule.num_fields].proto = MV_NET_PROTO_UDP;
  flow->table_key.key_size += key_field->size;
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_udp_sport(const struct rte_flow_item_udp *spec,
 ...
 * @return 0 in case of success, negative error value otherwise.
 */
static inline int mrvl_parse_udp_dport(const struct rte_flow_item_udp *spec,
                                       const struct rte_flow_item_udp *mask,
                                       struct rte_flow *flow) {
 ...
 */
static int mrvl_parse_eth(const struct rte_flow_item *item,
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_parse_vlan(const struct rte_flow_item *item,
                           struct rte_flow *flow,
                           struct rte_flow_error *error) {
  ...
  if (mask->inner_type) {
    ...
    for (i = 0; i < flow->rule.num_fields; i++)
      if (flow->table_key.proto_field[i].proto == MV_NET_PROTO_ETH &&
          flow->table_key.proto_field[i].field.eth == MV_NET_ETH_F_TYPE) {
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_parse_ip4(const struct rte_flow_item *item,
                          struct rte_flow *flow, struct rte_flow_error *error) {
  ...
  if (mask->hdr.version_ihl || mask->hdr.total_length || mask->hdr.packet_id ||
      mask->hdr.fragment_offset || mask->hdr.time_to_live ||
      mask->hdr.hdr_checksum) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                       "Not supported by classifier\n");
    return -rte_errno;
  }
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_parse_ip6(const struct rte_flow_item *item,
                          struct rte_flow *flow, struct rte_flow_error *error) {
  ...
  if (mask->hdr.payload_len || mask->hdr.hop_limits) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                       "Not supported by classifier\n");
    return -rte_errno;
  }

  if (memcmp(mask->hdr.src_addr, zero.src_addr, sizeof(mask->hdr.src_addr))) {
    ret = mrvl_parse_ip6_sip(spec, mask, flow);
    if (ret)
      goto out;
  }

  if (memcmp(mask->hdr.dst_addr, zero.dst_addr, sizeof(mask->hdr.dst_addr))) {
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_parse_tcp(const struct rte_flow_item *item,
                          struct rte_flow *flow, struct rte_flow_error *error) {
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_parse_udp(const struct rte_flow_item *item,
                          struct rte_flow *flow, struct rte_flow_error *error) {
...
static int mrvl_string_to_hex_values(const uint8_t *input_string,
                                     uint8_t *hex_key, uint8_t *length) {
  ...
  *length = ((*length - 2) + 1) / 2;
  ...
  for (i = 0; i < *length; i++) {
    ...
    tmp_arr[2] = '\0';
    if (get_val_securely8(tmp_arr, 16, &hex_key[*length - 1 - i]) < 0)
      return -1;
    string_iter += 2;
  }
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_parse_raw(const struct rte_flow_item *item,
                          struct rte_flow *flow, struct rte_flow_error *error) {
  ...
  if (!spec->pattern) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, NULL,
                       "pattern pointer MUST be given\n");
    return -rte_errno;
  }
  ...
  if (strncmp((const char *)spec->pattern, "0x", 2) != 0) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, NULL,
                       "'pattern' string must start with '0x'\n");
    return -rte_errno;
  }

  if (mask->pattern && strncmp((const char *)mask->pattern, "0x", 2) != 0) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, NULL,
                       "'mask-pattern' string must start with '0x'\n");
    return -rte_errno;
  }

  if (mask->search && spec->search) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, NULL,
                       "'search' option must be '0'\n");
    return -rte_errno;
  }

  if (mask->offset && spec->offset != 0) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, NULL,
                       "'offset' option must be '0'\n");
    return -rte_errno;
  }

  if (!mask->relative || !spec->relative) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, NULL,
                       "'relative' option must be given and enabled\n");
    return -rte_errno;
  }
  ...
  if (!length) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, NULL,
                       "'length' option must be given bigger than '0'\n");
    return -rte_errno;
  }
  ...
  if (mask->pattern) {
    ret = mrvl_string_to_hex_values(mask->pattern, key_field->mask, &length);
    if (ret) {
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, NULL,
  ...
  flow->table_key.proto_field[flow->rule.num_fields].proto = MV_NET_UDF;
  udf_params = &flow->table_key.proto_field[flow->rule.num_fields].field.udf;
...
static const struct {
  ...
  int (*parse)(const struct rte_flow_item *pattern, struct rte_flow *flow,
               struct rte_flow_error *error);
} mrvl_patterns[] = {{RTE_FLOW_ITEM_TYPE_ETH, mrvl_parse_eth},
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_flow_parse_attr(struct mrvl_priv *priv __rte_unused,
  ...
  if (attr->group) {
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_GROUP, NULL,
                       "Groups are not supported");
    return -rte_errno;
  }
  if (attr->priority) {
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY, NULL,
  ...
  if (attr->egress) {
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS, NULL,
                       "Egress is not supported");
    return -rte_errno;
  }
  if (attr->transfer) {
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_TRANSFER, NULL,
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_flow_parse_pattern(struct mrvl_priv *priv __rte_unused,
  ...
  for (i = 0; pattern[i].type != RTE_FLOW_ITEM_TYPE_END; i++) {
    if (pattern[i].type == RTE_FLOW_ITEM_TYPE_VOID)
      continue;
    for (j = 0; mrvl_patterns[j].pattern_type != RTE_FLOW_ITEM_TYPE_END; j++) {
      if (mrvl_patterns[j].pattern_type != pattern[i].type)
        continue;

      if (flow->rule.num_fields >= PP2_CLS_TBL_MAX_NUM_FIELDS) {
        rte_flow_error_set(error, ENOSPC, RTE_FLOW_ERROR_TYPE_ITEM_NUM, NULL,
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_flow_parse_actions(struct mrvl_priv *priv,
  ...
  for (; action->type != RTE_FLOW_ACTION_TYPE_END; action++) {
    ...
    if (action->type == RTE_FLOW_ACTION_TYPE_DROP) {
      ...
      const struct rte_flow_action_queue *q =
          (const struct rte_flow_action_queue *)action->conf;
      ...
      if (priv->rxq_map[q->index].tc == MRVL_UNKNOWN_TC) {
         ...
         * a correct queue.
         */
        MRVL_LOG(ERR, "Unknown TC mapping for queue %hu eth%hhu", q->index,
                 priv->ppio_id);

        rte_flow_error_set(error, EFAULT, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                           NULL);
        return -rte_errno;
      }

      MRVL_LOG(DEBUG, "Action: Assign packets to queue %d, tc:%d, q:%d",
      ...
      if (!mtr->shared && mtr->refcnt)
        return -rte_flow_error_set(error, EPERM, RTE_FLOW_ERROR_TYPE_ACTION,
                                   NULL, "Meter cannot be shared\n");
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_flow_parse(struct mrvl_priv *priv,
                           const struct rte_flow_attr *attr,
                           const struct rte_flow_item pattern[],
                           const struct rte_flow_action actions[],
                           struct rte_flow *flow,
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_create_cls_table(struct rte_eth_dev *dev,
                                 struct rte_flow *first_flow) {
  ...
  MRVL_LOG(INFO, "Setting cls search engine type to %s",
           priv->cls_tbl_params.type == PP2_CLS_TBL_EXACT_MATCH ? "exact"
                                                                : "maskable");
 ...
 */
static inline int mrvl_flow_can_be_added(struct mrvl_priv *priv,
 ...
 */
static struct rte_flow *mrvl_flow_create(struct rte_eth_dev *dev,
                                         const struct rte_flow_attr *attr,
                                         const struct rte_flow_item pattern[],
                                         const struct rte_flow_action actions[],
                                         struct rte_flow_error *error) {
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_flow_remove(struct mrvl_priv *priv, struct rte_flow *flow,
                            struct rte_flow_error *error) {
  int ret;
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                             struct rte_flow_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct rte_flow *f;
  int ret;
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_flow_validate(struct rte_eth_dev *dev,
                              const struct rte_flow_attr *attr,
                              const struct rte_flow_item pattern[],
                              const struct rte_flow_action actions[],
                              struct rte_flow_error *error) {
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_flow_flush(struct rte_eth_dev *dev,
                           struct rte_flow_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_flow_isolate(struct rte_eth_dev *dev, int enable,
                             struct rte_flow_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
...
const struct rte_flow_ops mrvl_flow_ops = {.validate = mrvl_flow_validate,
 ...
 * @param dev Pointer to the device.
 */
void mrvl_flow_deinit(struct rte_eth_dev *dev) { mrvl_flow_flush(dev, NULL); }
