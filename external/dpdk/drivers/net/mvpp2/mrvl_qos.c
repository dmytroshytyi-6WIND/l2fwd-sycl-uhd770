...
static int get_outq_cfg(struct rte_cfgfile *file, int port, int outq,
  ...
  snprintf(sec_name, sizeof(sec_name), "%s %d %s %d", MRVL_TOK_PORT, port,
           MRVL_TOK_TXQ, outq);
  ...
  if (cfg->port[port].outq[outq].sched_mode == PP2_PPIO_SCHED_M_WRR) {
    entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_WRR_WEIGHT);
    if (entry) {
  ...
  if (cfg->port[port].rate_limit_enable) {
    MRVL_LOG(WARNING, "Port %d rate limiting already enabled", port);
    return 0;
  }

  entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_RATE_LIMIT_ENABLE);
  if (entry) {
    if (get_val_securely(entry, &val) < 0)
      return -1;
 ...
 */
static int get_entry_values(const char *entry, uint8_t *tab, size_t elem_sz,
                            uint8_t max_elems, uint8_t max_val) {
   ...
   */
  nb_tokens =
      rte_strsplit(entry_cpy, strlen(entry_cpy), tokens, max_elems + 1, ' ');
  ...
  for (i = 0; i < nb_tokens; ++i) {
    if (strchr(tokens[i], '-') != NULL) {
       ...
       */
      nb_rng_tokens = rte_strsplit(tokens[i], strlen(tokens[i]), rng_tokens,
                                   RTE_DIM(rng_tokens), '-');
      if (nb_rng_tokens != 2)
        return -3;
      ...
      if (beg < 0 || beg > UCHAR_MAX || end < 0 || end > UCHAR_MAX || end < beg)
        return -4;
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int parse_tc_cfg(struct rte_cfgfile *file, int port, int tc,
                        struct mrvl_cfg *cfg) {
  char sec_name[32];
  const char *entry;
  int n;
  ...
  if (entry) {
    n = get_entry_values(entry, cfg->port[port].tc[tc].inq,
                         sizeof(cfg->port[port].tc[tc].inq[0]),
                         RTE_DIM(cfg->port[port].tc[tc].inq), MRVL_PP2_RXQ_MAX);
    if (n < 0) {
      MRVL_LOG(ERR, "Error %d while parsing: %s", n, entry);
      return n;
    }
  ...
  if (entry) {
    n = get_entry_values(entry, cfg->port[port].tc[tc].pcp,
                         sizeof(cfg->port[port].tc[tc].pcp[0]),
                         RTE_DIM(cfg->port[port].tc[tc].pcp), MAX_PCP);
    if (n < 0) {
      MRVL_LOG(ERR, "Error %d while parsing: %s", n, entry);
      return n;
    }
  ...
  if (entry) {
    n = get_entry_values(entry, cfg->port[port].tc[tc].dscp,
                         sizeof(cfg->port[port].tc[tc].dscp[0]),
                         RTE_DIM(cfg->port[port].tc[tc].dscp), MAX_DSCP);
    if (n < 0) {
      MRVL_LOG(ERR, "Error %d while parsing: %s", n, entry);
      return n;
    }
  ...
  entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_PLCR_DEFAULT_COLOR);
  if (entry) {
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int parse_policer(struct rte_cfgfile *file, int port,
                         const char *sec_name, struct mrvl_cfg *cfg) {
  const char *entry;
  uint32_t val;
  ...
  if (entry) {
    if (!strncmp(entry, MRVL_TOK_PLCR_UNIT_BYTES,
      ...
      cfg->port[port].policer_params.token_unit = PP2_CLS_PLCR_BYTES_TOKEN_UNIT;
  ...
  if (entry) {
    if (!strncmp(entry, MRVL_TOK_PLCR_COLOR_BLIND,
      ...
      cfg->port[port].policer_params.color_mode = PP2_CLS_PLCR_COLOR_BLIND_MODE;
    } else if (!strncmp(entry, MRVL_TOK_PLCR_COLOR_AWARE,
                        sizeof(MRVL_TOK_PLCR_COLOR_AWARE))) {
      cfg->port[port].policer_params.color_mode = PP2_CLS_PLCR_COLOR_AWARE_MODE;
    } else {
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int parse_udf(struct rte_cfgfile *file, const char *sec_name, int udf,
                     struct mrvl_cfg *cfg) {
...
#define FIELD_LEN_IN_CHARS(field_size) (uint32_t)(2 + (field_size)*2)
  ...
  /* Read 'proto' field */
  entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_PARSER_UDF_PROTO);
  if (!entry) {
  ...
  /* Read 'field' field */
  entry_field =
      rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_PARSER_UDF_FIELD);
  if (!entry_field) {
    MRVL_LOG(ERR, "UDF[%d]: '%s' field must be set\n", udf,
  ...
  if (!strncmp(entry, MRVL_TOK_PARSER_UDF_PROTO_ETH,
    ...
    if (!strncmp(entry_field, MRVL_TOK_PARSER_UDF_FIELD_ETH_TYPE,
      ...
      MRVL_LOG(ERR,
               "UDF[%d]: mismatch between '%s' proto "
               "and '%s' field\n",
               udf, MRVL_TOK_PARSER_UDF_PROTO_ETH, entry_field);
      return -1;
    }
    ...
    if (!strncmp(entry_field, MRVL_TOK_PARSER_UDF_FIELD_UDP_DPORT,
      ...
      MRVL_LOG(ERR,
               "UDF[%d]: mismatch between '%s' proto "
               "and '%s' field\n",
               udf, MRVL_TOK_PARSER_UDF_PROTO_UDP, entry_field);
      return -1;
    }
  } else {
  ...
  if (strlen(entry) != FIELD_LEN_IN_CHARS(field_size)) {
    MRVL_LOG(ERR, "UDF[%d]: '%s' field's len must be %d\n", udf,
             MRVL_TOK_PARSER_UDF_KEY, FIELD_LEN_IN_CHARS(field_size));
    return -EINVAL;
  }
  ...
  for (i = 0; i < field_size; i++) {
    ...
    tmp_arr[2] = '\0';
    if (get_val_securely8(tmp_arr, 16, &udf_params->match_key[i]) < 0) {
      MRVL_LOG(ERR,
               "UDF[%d]: '%s' field's value is not in "
               "hex format\n",
  ...
  if (strlen(entry) != FIELD_LEN_IN_CHARS(field_size)) {
    MRVL_LOG(ERR, "UDF[%d]: '%s' field's len must be %d\n", udf,
             MRVL_TOK_PARSER_UDF_MASK, FIELD_LEN_IN_CHARS(field_size));
    return -EINVAL;
  }
  ...
  for (i = 0; i < field_size; i++) {
    strncpy(tmp_arr, entry, 2);
    tmp_arr[2] = '\0';
    if (get_val_securely8(tmp_arr, 16, &udf_params->match_mask[i]) < 0) {
      MRVL_LOG(ERR,
               "UDF[%d]: '%s' field's value is not in "
               "hex format\n",
  ...
  /* Read offset */
  entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_PARSER_UDF_OFFSET);
  if (!entry) {
    MRVL_LOG(ERR, "UDF[%d]: '%s' field must be set\n", udf,
 ...
 */
int mrvl_get_cfg(const char *key __rte_unused, const char *path,
                 void *extra_args) {
  ...
  *cfg = rte_zmalloc("mrvl_cfg", sizeof(struct mrvl_cfg), 0);
  ...
  if (n && n > PP2_MAX_UDFS_SUPPORTED) {
    MRVL_LOG(ERR, "found %d udf sections, but only %d are supported\n", n,
             PP2_MAX_UDFS_SUPPORTED);
    return -1;
  }
  (*cfg)->pp2_cfg.prs_udfs.num_udfs = n;
  for (i = 0; i < n; i++) {
    snprintf(sec_name, sizeof(sec_name), "%s %d", MRVL_TOK_PARSER_UDF, i);
  ...
  n = rte_cfgfile_num_sections(file, MRVL_TOK_PORT, sizeof(MRVL_TOK_PORT) - 1);
  ...
  for (n = 0; n < (PP2_NUM_ETH_PPIO * PP2_NUM_PKT_PROC); ++n) {
    snprintf(sec_name, sizeof(sec_name), "%s %d %s", MRVL_TOK_PORT, n,
             MRVL_TOK_DEFAULT);
     ...
     * compatibility.
     */
    entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_START_HDR);
    /* if start_hsr is missing, check if dsa_mode exist instead */
    if (entry == NULL)
      entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_DSA_MODE);
    if (entry) {
      if (!strncmp(entry, MRVL_TOK_START_HDR_NONE,
        ...
        (*cfg)->port[n].eth_start_hdr = PP2_PPIO_HDR_ETH_DSA;
      else if (!strncmp(entry, MRVL_TOK_START_HDR_CUSTOM,
                        sizeof(MRVL_TOK_START_HDR_CUSTOM)))
        (*cfg)->port[n].eth_start_hdr = PP2_PPIO_HDR_ETH_CUSTOM;
      else if (!strncmp(entry, MRVL_TOK_START_HDR_EXT_DSA,
                        sizeof(MRVL_TOK_START_HDR_EXT_DSA))) {
        (*cfg)->port[n].eth_start_hdr = PP2_PPIO_HDR_ETH_EXT_DSA;
      } else {
        MRVL_LOG(ERR, "Error in parsing %s value (%s)!\n", MRVL_TOK_START_HDR,
                 entry);
        return -1;
      }
    } else {
    ...
    entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_MAPPING_PRIORITY);
    if (entry) {
      (*cfg)->port[n].use_qos_global_defaults = 0;
      if (!strncmp(entry, MRVL_TOK_VLAN_IP, sizeof(MRVL_TOK_VLAN_IP)))
        (*cfg)->port[n].mapping_priority = PP2_CLS_QOS_TBL_VLAN_IP_PRI;
      else if (!strncmp(entry, MRVL_TOK_IP_VLAN, sizeof(MRVL_TOK_IP_VLAN)))
        (*cfg)->port[n].mapping_priority = PP2_CLS_QOS_TBL_IP_VLAN_PRI;
      else if (!strncmp(entry, MRVL_TOK_IP, sizeof(MRVL_TOK_IP)))
        (*cfg)->port[n].mapping_priority = PP2_CLS_QOS_TBL_IP_PRI;
      else if (!strncmp(entry, MRVL_TOK_VLAN, sizeof(MRVL_TOK_VLAN))) {
        (*cfg)->port[n].mapping_priority = PP2_CLS_QOS_TBL_VLAN_PRI;
      } else {
        MRVL_LOG(ERR, "Error in parsing %s value (%s)!\n",
      ...
      (*cfg)->port[n].mapping_priority = PP2_CLS_QOS_TBL_NONE;
    }

    /* Parse policer configuration (if any) */
    entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_PLCR_DEFAULT);
    if (entry) {
      (*cfg)->port[n].use_qos_global_defaults = 0;
      if (get_val_securely(entry, &val) < 0)
        return -1;

      snprintf(sec_name, sizeof(sec_name), "%s %d", MRVL_TOK_PLCR, val);
    ...
    for (i = 0; i < MRVL_PP2_RXQ_MAX; ++i) {
      ...
      if (ret < 0) {
        MRVL_LOG(ERR, "Error %d parsing port %d outq %d!\n", ret, n, i);
        return -1;
      }
    }
    ...
    for (i = 0; i < MRVL_PP2_TC_MAX; ++i) {
      ...
      if (ret < 0) {
        MRVL_LOG(ERR, "Error %d parsing port %d tc %d!\n", ret, n, i);
        return -1;
      }
    }

    entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_DEFAULT_TC);
    if (entry) {
      if (get_val_securely(entry, &val) < 0 || val > USHRT_MAX)
        return -1;
      (*cfg)->port[n].default_tc = (uint8_t)val;
    } else {
      if ((*cfg)->port[n].use_qos_global_defaults == 0) {
        MRVL_LOG(ERR, "Default Traffic Class required in "
    ...
    entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_FWD_BAD_FRAMES);
    if (entry) {
      if (get_val_securely(entry, &val) < 0) {
        MRVL_LOG(ERR, "Error in parsing %s value (%s)!\n",
    ...
    entry = rte_cfgfile_get_entry(file, sec_name, MRVL_TOK_FILL_BPOOL_BUFFS);
    if (entry) {
      if (get_val_securely(entry, &val) < 0) {
        MRVL_LOG(ERR, "Error in parsing %s value (%s)!\n",
 ...
 * @returns 0 in case of success, exits otherwise.
 */
static int setup_tc(struct pp2_ppio_tc_params *param, uint8_t inqs,
  ...
  inq_params = rte_zmalloc_socket("inq_params", inqs * sizeof(*inq_params), 0,
                                  rte_socket_id());
  if (!inq_params)
    return -ENOMEM;
 ...
 */
static int setup_policer(struct mrvl_priv *priv,
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
int mrvl_configure_rxqs(struct mrvl_priv *priv, uint16_t portid,
                        uint16_t max_queues) {
  size_t i, tc;

  if (mrvl_cfg == NULL || mrvl_cfg->port[portid].use_qos_global_defaults) {
    /*
     * No port configuration, use default: 1 TC, no QoS,
     * TC color set to green.
     */
    priv->ppio_params.inqs_params.num_tcs = 1;
    setup_tc(&priv->ppio_params.inqs_params.tcs_params[0], max_queues,
             priv->bpool, PP2_PPIO_COLOR_GREEN);
  ...
  for (tc = 0; tc < RTE_DIM(port_cfg->tc); ++tc) {
    if (port_cfg->tc[tc].pcps > RTE_DIM(port_cfg->tc[0].pcp)) {
      ...
      MRVL_LOG(ERR, "Too many PCPs configured in TC %zu!", tc);
      return -1;
    }
    for (i = 0; i < port_cfg->tc[tc].pcps; ++i) {
      priv->qos_tbl_params.pcp_cos_map[port_cfg->tc[tc].pcp[i]].tc = tc;
    }
  }
  ...
  for (tc = 0; tc < RTE_DIM(port_cfg->tc); ++tc) {
    if (port_cfg->tc[tc].dscps > RTE_DIM(port_cfg->tc[0].dscp)) {
      /* Better safe than sorry. */
      MRVL_LOG(ERR, "Too many DSCPs configured in TC %zu!", tc);
      return -1;
    }
    for (i = 0; i < port_cfg->tc[tc].dscps; ++i) {
      priv->qos_tbl_params.dscp_cos_map[port_cfg->tc[tc].dscp[i]].tc = tc;
    }
  }
  ...
  for (tc = 0; tc < RTE_DIM(port_cfg->tc); ++tc) {
    if (port_cfg->tc[tc].inqs > RTE_DIM(port_cfg->tc[0].inq)) {
      /* Overflow. */
      MRVL_LOG(ERR, "Too many RX queues configured per TC %zu!", tc);
      return -1;
    }
  ...
  for (i = 0; i < MRVL_PP2_TC_MAX; ++i) {
    if (port_cfg->tc[i].inqs == 0)
      break;
    setup_tc(&priv->ppio_params.inqs_params.tcs_params[i], port_cfg->tc[i].inqs,
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
int mrvl_configure_txqs(struct mrvl_priv *priv, uint16_t portid,
                        uint16_t max_queues) {
  /* We need only a subset of configuration. */
  struct port_cfg *port_cfg = &mrvl_cfg->port[portid];
  int i;
  ...
  if (port_cfg->rate_limit_enable)
    priv->ppio_params.rate_limit_params = port_cfg->rate_limit_params;
 ...
 * @returns 0 in case of success, exits otherwise.
 */
int mrvl_start_qos_mapping(struct mrvl_priv *priv) {
  size_t i;
