 ...
 * @returns 0 always.
 */
static int mrvl_capabilities_get(struct rte_eth_dev *dev __rte_unused,
 ...
 */
static int mrvl_meter_profile_add(struct rte_eth_dev *dev,
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_meter_profile_delete(struct rte_eth_dev *dev,
                                     uint32_t meter_profile_id,
                                     struct rte_mtr_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  ...
  if (profile->refcnt)
    return -rte_mtr_error_set(error, EPERM, RTE_MTR_ERROR_TYPE_METER_PROFILE_ID,
 ...
 */
static struct mrvl_mtr *mrvl_mtr_from_id(struct mrvl_priv *priv,
                                         uint32_t mtr_id) {
 ...
 */
static int mrvl_reserve_plcr(uint32_t *plcrs) {
  uint32_t i, num;
 ...
 */
static int mrvl_meter_enable(struct rte_eth_dev *dev, uint32_t mtr_id,
                             struct rte_mtr_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  ...
  if (!priv->ppio)
    return -rte_mtr_error_set(error, EPERM, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Port is uninitialized\n");

  if (!mtr)
    return -rte_mtr_error_set(error, ENODEV, RTE_MTR_ERROR_TYPE_MTR_ID, NULL,
  ...
  if (mtr->plcr_bit < 0)
    return -rte_mtr_error_set(error, ENOSPC, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Failed to reserve plcr entry\n");

  memset(&params, 0, sizeof(params));
  snprintf(match, sizeof(match), "policer-%d:%d", priv->pp_id, mtr->plcr_bit);
  params.match = match;
  ...
  if (ret) {
    ...
    return -rte_mtr_error_set(error, -ret, RTE_MTR_ERROR_TYPE_UNSPECIFIED, NULL,
                              "Failed to setup policer\n");
  }
...
skip:
  ...
  LIST_FOREACH(flow, &priv->flows, next) {
    ...
    ret = pp2_cls_tbl_modify_rule(priv->cls_tbl, &flow->rule, &flow->action);
    if (ret)
      return -rte_mtr_error_set(error, -ret, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_meter_disable(struct rte_eth_dev *dev, uint32_t mtr_id,
                              struct rte_mtr_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_mtr *mtr = mrvl_mtr_from_id(priv, mtr_id);
  struct rte_flow *flow;
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_create(struct rte_eth_dev *dev, uint32_t mtr_id,
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_destroy(struct rte_eth_dev *dev, uint32_t mtr_id,
                        struct rte_mtr_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_mtr *mtr;
 ...
 * @returns 0 on success, negative value otherwise.
 */
static int mrvl_meter_profile_update(struct rte_eth_dev *dev, uint32_t mtr_id,
                                     uint32_t meter_profile_id,
                                     struct rte_mtr_error *error) {
  struct mrvl_priv *priv = dev->data->dev_private;
  struct mrvl_mtr_profile *profile;
...
void mrvl_mtr_deinit(struct rte_eth_dev *dev) {
  ...
  for (mtr = LIST_FIRST(&priv->mtrs);
       mtr && (tmp_mtr = LIST_NEXT(mtr, next), 1); mtr = tmp_mtr)
