...
static inline uint16_t reassemble_packets(struct ixgbe_rx_queue *rxq,
...
static __rte_always_inline void tx_backlog_entry(struct ixgbe_tx_entry_v *txep,
...
_ixgbe_tx_queue_release_mbufs_vec(struct ixgbe_tx_queue *txq) {
  ...
  for (i = txq->tx_next_dd - (txq->tx_rs_thresh - 1); i != txq->tx_tail;
...
static inline void _ixgbe_tx_free_swring_vec(struct ixgbe_tx_queue *txq) {
  if (txq == NULL)
    return;
...
static inline void _ixgbe_reset_tx_queue_vec(struct ixgbe_tx_queue *txq) {
...
static inline int ixgbe_rxq_vec_setup_default(struct ixgbe_rx_queue *rxq) {
  uintptr_t p;
