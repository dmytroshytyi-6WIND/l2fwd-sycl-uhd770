...
#define IXGBE_DEFAULT_FLEXBYTES_OFFSET                                         \
  12                              /* default flexbytes offset in bytes */
...
#define IPV6_ADDR_TO_MASK(ipaddr, ipv6m)                                       \
  do {                                                                         \
...
#define IPV6_MASK_TO_ADDR(ipv6m, ipaddr)                                       \
  do {                                                                         \
    uint8_t ipv6_addr[16];                                                     \
    uint8_t i;                                                                 \
    for (i = 0; i < sizeof(ipv6_addr); i++) {                                  \
...
static int fdir_enable_82599(struct ixgbe_hw *hw, uint32_t fdirctrl) {
  ...
  for (i = 0; i < IXGBE_FDIR_INIT_DONE_POLL; i++) {
    if (IXGBE_READ_REG(hw, IXGBE_FDIRCTRL) & IXGBE_FDIRCTRL_INIT_DONE)
      break;
    msec_delay(1);
  }
 ...
 */
static inline int configure_fdir_flags(const struct rte_eth_fdir_conf *conf,
                                       uint32_t *fdirctrl) {
  *fdirctrl = 0;
  ...
  if (conf->mode >= RTE_FDIR_MODE_PERFECT &&
    ...
    *fdirctrl |= (conf->drop_queue << IXGBE_FDIRCTRL_DROP_Q_SHIFT);
    if (conf->mode == RTE_FDIR_MODE_PERFECT_MAC_VLAN)
      ...
      *fdirctrl |=
          (IXGBE_FDIRCTRL_FILTERMODE_CLOUD << IXGBE_FDIRCTRL_FILTERMODE_SHIFT);
  }
 ...
 *  bits in these masks.
 *  **/
static inline uint32_t reverse_fdir_bitmasks(uint16_t hi_dword,
                                             uint16_t lo_dword) {
...
static int fdir_set_input_mask_82599(struct rte_eth_dev *dev) {
  ...
  fdirtcpm = reverse_fdir_bitmasks(rte_be_to_cpu_16(info->mask.dst_port_mask),
  ...
  *reg = ~(info->mask.dst_ipv4_mask);
  ...
  if (dev->data->dev_conf.fdir_conf.mode == RTE_FDIR_MODE_SIGNATURE) {
     ...
     */
    fdiripv6m = (info->mask.dst_ipv6_mask << 16) | info->mask.src_ipv6_mask;
...
static int fdir_set_input_mask_x550(struct rte_eth_dev *dev) {
  ...
  if (mode == RTE_FDIR_MODE_PERFECT_MAC_VLAN)
    fdiripv6m |= IXGBE_FDIRIP6M_TUNNEL_TYPE | IXGBE_FDIRIP6M_TNI_VNI;

  if (mode == RTE_FDIR_MODE_PERFECT_TUNNEL) {
    fdiripv6m |= IXGBE_FDIRIP6M_INNER_MAC;
    mac_mask = info->mask.mac_addr_byte_mask &
               (IXGBE_FDIRIP6M_INNER_MAC >> IXGBE_FDIRIP6M_INNER_MAC_SHIFT);
...
ixgbe_fdir_store_input_mask(struct rte_eth_dev *dev,
  ...
  if (mode >= RTE_FDIR_MODE_SIGNATURE && mode <= RTE_FDIR_MODE_PERFECT)
...
int ixgbe_fdir_set_flexbytes_offset(struct rte_eth_dev *dev, uint16_t offset) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  IXGBE_WRITE_REG(hw, IXGBE_FDIRCMD,
                  (IXGBE_READ_REG(hw, IXGBE_FDIRCMD) | IXGBE_FDIRCMD_CLEARHT));
  IXGBE_WRITE_FLUSH(hw);
  IXGBE_WRITE_REG(hw, IXGBE_FDIRCMD,
                  (IXGBE_READ_REG(hw, IXGBE_FDIRCMD) & ~IXGBE_FDIRCMD_CLEARHT));
  IXGBE_WRITE_FLUSH(hw);
...
static int ixgbe_set_fdir_flex_conf(struct rte_eth_dev *dev,
  ...
  info->flex_bytes_offset = (uint8_t)((*fdirctrl & IXGBE_FDIRCTRL_FLEX_MASK) >>
                                      IXGBE_FDIRCTRL_FLEX_SHIFT);
  return 0;
}
...
int ixgbe_fdir_configure(struct rte_eth_dev *dev) {
  ...
  if (hw->mac.type != ixgbe_mac_X550 && hw->mac.type != ixgbe_mac_X550EM_x &&
      hw->mac.type != ixgbe_mac_X550EM_a && mode != RTE_FDIR_MODE_SIGNATURE &&
  ...
  err = ixgbe_set_fdir_flex_conf(dev, &dev->data->dev_conf.fdir_conf.flex_conf,
                                 &fdirctrl);
  if (err < 0) {
...
atr_compute_perfect_hash_82599(union ixgbe_atr_input *input,
  ...
  if (pballoc == RTE_ETH_FDIR_PBALLOC_256K)
    return ixgbe_atr_compute_hash_82599(input, IXGBE_ATR_BUCKET_HASH_KEY) &
 ...
 * @hw: pointer to hardware structure
 */
static inline int ixgbe_fdir_check_cmd_complete(struct ixgbe_hw *hw,
                                                uint32_t *fdircmd) {
  int i;
...
atr_compute_sig_hash_82599(union ixgbe_atr_input *input,
  ...
  if (pballoc == RTE_ETH_FDIR_PBALLOC_256K)
    bucket_hash =
        ixgbe_atr_compute_hash_82599(input, IXGBE_ATR_BUCKET_HASH_KEY) &
  ...
  sig_hash = ixgbe_atr_compute_hash_82599(input, IXGBE_ATR_SIGNATURE_HASH_KEY);
...
static int fdir_write_perfect_filter_82599(struct ixgbe_hw *hw,
  ...
  if (mode == RTE_FDIR_MODE_PERFECT) {
    ...
    *reg = input->formatted.dst_ip[0];
    ...
    if (mode == RTE_FDIR_MODE_PERFECT_MAC_VLAN) {
      ...
      IXGBE_WRITE_REG(hw, IXGBE_FDIRSIPv6(2), input->formatted.tni_vni);
    }
  ...
  fdircmd |=
      IXGBE_FDIRCMD_CMD_ADD_FLOW | IXGBE_FDIRCMD_LAST | IXGBE_FDIRCMD_QUEUE_EN;
...
static int fdir_erase_filter_82599(struct ixgbe_hw *hw, uint32_t fdirhash) {
  ...
  if (fdircmd & IXGBE_FDIRCMD_FILTER_VALID) {
    IXGBE_WRITE_REG(hw, IXGBE_FDIRHASH, fdirhash);
    IXGBE_WRITE_FLUSH(hw);
    IXGBE_WRITE_REG(hw, IXGBE_FDIRCMD, IXGBE_FDIRCMD_CMD_REMOVE_FLOW);
  }
  err = ixgbe_fdir_check_cmd_complete(hw, &fdircmd);
  if (err < 0)
...
ixgbe_insert_fdir_filter(struct ixgbe_hw_fdir_info *fdir_info,
  ...
  ret = rte_hash_add_key(fdir_info->hash_handle, &fdir_filter->ixgbe_fdir);
...
static inline int ixgbe_remove_fdir_filter(struct ixgbe_hw_fdir_info *fdir_info,
                                           union ixgbe_atr_input *key) {
  int ret;
  struct ixgbe_fdir_filter *fdir_filter;
...
int ixgbe_fdir_filter_program(struct rte_eth_dev *dev,
                              struct ixgbe_fdir_rule *rule, bool del,
                              bool update) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  if ((!del) &&
      ...
      (info->mask.src_port_mask != 0 || info->mask.dst_port_mask != 0) &&
  ...
  if (is_perfect) {
    if (rule->ixgbe_fdir.formatted.flow_type & IXGBE_ATR_L4TYPE_IPV6_MASK) {
      PMD_DRV_LOG(ERR, "IPv6 is not supported in"
                       " perfect mode!");
      return -ENOTSUP;
    }
    fdirhash = atr_compute_perfect_hash_82599(
        &rule->ixgbe_fdir, dev->data->dev_conf.fdir_conf.pballoc);
    fdirhash |= rule->soft_id << IXGBE_FDIRHASH_SIG_SW_INDEX_SHIFT;
  } else
    fdirhash = atr_compute_sig_hash_82599(
        &rule->ixgbe_fdir, dev->data->dev_conf.fdir_conf.pballoc);
  ...
  if (node) {
    ...
    add_node = TRUE;
    node = rte_zmalloc("ixgbe_fdir", sizeof(struct ixgbe_fdir_filter), 0);
    if (!node)
      return -ENOMEM;
    rte_memcpy(&node->ixgbe_fdir, &rule->ixgbe_fdir,
  ...
  if (is_perfect) {
    err = fdir_write_perfect_filter_82599(hw, &rule->ixgbe_fdir, queue,
                                          fdircmd_flags, fdirhash, fdir_mode);
  } else {
...
void ixgbe_fdir_info_get(struct rte_eth_dev *dev,
  ...
  fdirctrl = IXGBE_READ_REG(hw, IXGBE_FDIRCTRL);
  offset =
      ((fdirctrl & IXGBE_FDIRCTRL_FLEX_MASK) >> IXGBE_FDIRCTRL_FLEX_SHIFT) *
      sizeof(uint16_t);

  fdir_info->mode = dev->data->dev_conf.fdir_conf.mode;
  max_num = (1 << (FDIRENTRIES_NUM_SHIFT + (fdirctrl & FDIRCTRL_PBALLOC_MASK)));
  ...
  IPV6_MASK_TO_ADDR(info->mask.src_ipv6_mask, fdir_info->mask.ipv6_mask.src_ip);
  IPV6_MASK_TO_ADDR(info->mask.dst_ipv6_mask, fdir_info->mask.ipv6_mask.dst_ip);
...
void ixgbe_fdir_stats_get(struct rte_eth_dev *dev,
  ...
  info->collision =
      (uint16_t)((reg & IXGBE_FDIRFREE_COLL_MASK) >> IXGBE_FDIRFREE_COLL_SHIFT);
  info->free =
      (uint16_t)((reg & IXGBE_FDIRFREE_FREE_MASK) >> IXGBE_FDIRFREE_FREE_SHIFT);
  ...
  info->remove +=
      (reg & IXGBE_FDIRUSTAT_REMOVE_MASK) >> IXGBE_FDIRUSTAT_REMOVE_SHIFT;
  info->add += (reg & IXGBE_FDIRUSTAT_ADD_MASK) >> IXGBE_FDIRUSTAT_ADD_SHIFT;

  reg = IXGBE_READ_REG(hw, IXGBE_FDIRFSTAT) & 0xFFFF;
  info->f_remove +=
      (reg & IXGBE_FDIRFSTAT_FREMOVE_MASK) >> IXGBE_FDIRFSTAT_FREMOVE_SHIFT;
  info->f_add +=
      (reg & IXGBE_FDIRFSTAT_FADD_MASK) >> IXGBE_FDIRFSTAT_FADD_SHIFT;
  ...
  max_num = (1 << (FDIRENTRIES_NUM_SHIFT + (reg & FDIRCTRL_PBALLOC_MASK)));
  if (fdir_mode >= RTE_FDIR_MODE_PERFECT &&
      fdir_mode <= RTE_FDIR_MODE_PERFECT_TUNNEL)
...
void ixgbe_fdir_filter_restore(struct rte_eth_dev *dev) {
  ...
  if (is_perfect) {
    TAILQ_FOREACH(node, &fdir_info->fdir_list, entries) {
      (void)fdir_write_perfect_filter_82599(hw, &node->ixgbe_fdir, node->queue,
                                            node->fdirflags, node->fdirhash,
                                            fdir_mode);
    }
  } else {
    TAILQ_FOREACH(node, &fdir_info->fdir_list, entries) {
      (void)fdir_add_signature_filter_82599(hw, &node->ixgbe_fdir, node->queue,
                                            node->fdirflags, node->fdirhash);
    }
  }
}
