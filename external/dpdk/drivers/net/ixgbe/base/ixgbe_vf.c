...
static void ixgbe_virt_clr_reg(struct ixgbe_hw *hw) {
  ...
  vfdca_rxctrl = IXGBE_DCA_RXCTRL_DESC_RRO_EN | IXGBE_DCA_RXCTRL_DATA_WRO_EN |
                 IXGBE_DCA_RXCTRL_HEAD_WRO_EN;

  /* DCA_TXCTRL default value */
  vfdca_txctrl = IXGBE_DCA_TXCTRL_DESC_RRO_EN | IXGBE_DCA_TXCTRL_DESC_WRO_EN |
...
s32 ixgbe_reset_hw_vf(struct ixgbe_hw *hw) {
   ...
   */
  ret_val = mbx->ops.read_posted(hw, msgbuf, IXGBE_VF_PERMADDR_MSG_LEN, 0);
  if (ret_val)
    return ret_val;
...
s32 ixgbevf_rlpml_set_vf(struct ixgbe_hw *hw, u16 max_size) {
  ...
  if (retval)
    return retval;
  if ((msgbuf[0] & IXGBE_VF_SET_LPE) && (msgbuf[0] & IXGBE_VT_MSGTYPE_NACK))
    return IXGBE_ERR_MBX;
