...
s32 ixgbe_setup_sfp_modules_82599(struct ixgbe_hw *hw) {
  ...
  if (hw->phy.sfp_type != ixgbe_sfp_type_unknown) {
    ...
    ret_val =
        ixgbe_get_sfp_init_sequence_offsets(hw, &list_offset, &data_offset);
    ...
    ret_val = hw->mac.ops.prot_autoc_write(
        hw, hw->mac.orig_autoc | IXGBE_AUTOC_LMS_10G_SERIAL, false);
...
s32 ixgbe_init_ops_82599(struct ixgbe_hw *hw) {
  ...
  mac->arc_subsystem_valid =
      !!(IXGBE_READ_REG(hw, IXGBE_FWSM_BY_MAC(hw)) & IXGBE_FWSM_MODE_MASK);
  ...
  mac->ops.get_thermal_sensor_data = ixgbe_get_thermal_sensor_data_generic;
...
s32 ixgbe_start_mac_link_82599(struct ixgbe_hw *hw,
   ...
   */
  if (ixgbe_verify_lesm_fw_enabled_82599(hw)) {
    status = hw->mac.ops.acquire_swfw_sync(hw, IXGBE_GSSR_MAC_CSR_SM);
 ...
 **/
s32 ixgbe_setup_mac_link_smartspeed(struct ixgbe_hw *hw, ixgbe_link_speed speed,
                                    bool autoneg_wait_to_complete) {
  s32 status = IXGBE_SUCCESS;
  ...
  for (j = 0; j < IXGBE_SMARTSPEED_MAX_RETRIES; j++) {
    status = ixgbe_setup_mac_link_82599(hw, speed, autoneg_wait_to_complete);
    if (status != IXGBE_SUCCESS)
      goto out;
...
s32 ixgbe_setup_mac_link_82599(struct ixgbe_hw *hw, ixgbe_link_speed speed,
  ...
  u32 current_autoc =
      IXGBE_READ_REG(hw, IXGBE_AUTOC); /* holds the value of AUTOC register at
                                          this current point in time */
  u32 orig_autoc = 0; /* holds the cached value of AUTOC register */
  u32 autoc =
      current_autoc; /* Temporary variable used for comparison purposes */
...
mac_reset_top:
  ...
  if (hw->mac.orig_link_settings_stored == false) {
     ...
     * LMS state.
     */
    if ((hw->phy.multispeed_fiber && ixgbe_mng_enabled(hw)) || hw->wol_enabled)
      hw->mac.orig_autoc =
          (hw->mac.orig_autoc & ~IXGBE_AUTOC_LMS_MASK) | curr_lms;

    if (autoc != hw->mac.orig_autoc) {
      status = hw->mac.ops.prot_autoc_write(hw, hw->mac.orig_autoc, false);
      if (status != IXGBE_SUCCESS)
        goto reset_hw_out;
    }
  ...
  if (ixgbe_validate_mac_addr(hw->mac.san_addr) == 0) {
    ...
    hw->mac.ops.set_rar(hw, hw->mac.san_mac_rar_index, hw->mac.san_addr, 0,
                        IXGBE_RAH_AV);

    /* clear VMDq pool/queue selection for this RAR */
    hw->mac.ops.clear_vmdq(hw, hw->mac.san_mac_rar_index, IXGBE_CLEAR_VMDQ_ALL);
...
s32 ixgbe_init_fdir_perfect_82599(struct ixgbe_hw *hw, u32 fdirctrl,
   ...
   */
  fdirctrl |= IXGBE_FDIRCTRL_PERFECT_MATCH | IXGBE_FDIRCTRL_REPORT_STATUS |
...
void ixgbe_fdir_add_signature_filter_82599(struct ixgbe_hw *hw,
  ...
  flow_type = input.formatted.flow_type & (IXGBE_ATR_L4TYPE_TUNNEL_MASK - 1);
  switch (flow_type) {
...
s32 ixgbe_fdir_set_input_mask_82599(struct ixgbe_hw *hw,
  ...
  if (!cloud_mode) {
    ...
    IXGBE_WRITE_REG_BE32(hw, IXGBE_FDIRSIP4M, ~input_mask->formatted.src_ip[0]);
    IXGBE_WRITE_REG_BE32(hw, IXGBE_FDIRDIP4M, ~input_mask->formatted.dst_ip[0]);
...
s32 ixgbe_fdir_add_perfect_filter_82599(struct ixgbe_hw *hw,
  ...
  case IXGBE_ATR_FLOW_TYPE_TUNNELED_UDPV4:
    input_mask->formatted.flow_type =
        IXGBE_ATR_L4TYPE_IPV6_MASK | IXGBE_ATR_L4TYPE_MASK;
    break;
  default:
    DEBUGOUT(" Error on flow type input\n");
    return err;
  ...
  return ixgbe_fdir_write_perfect_filter_82599(hw, input, soft_id, queue,
                                               cloud_mode);
}
...
s32 ixgbe_read_analog_reg8_82599(struct ixgbe_hw *hw, u32 reg, u8 *val) {
  ...
  IXGBE_WRITE_REG(hw, IXGBE_CORECTL, IXGBE_CORECTL_WRITE_CMD | (reg << 8));
  IXGBE_WRITE_FLUSH(hw);
  usec_delay(10);
...
u64 ixgbe_get_supported_physical_layer_82599(struct ixgbe_hw *hw) {
  ...
  case IXGBE_AUTOC_LMS_1G_LINK_NO_AN:
    if (pma_pmd_1g == IXGBE_AUTOC_1G_KX_BX) {
      physical_layer =
          IXGBE_PHYSICAL_LAYER_1000BASE_KX | IXGBE_PHYSICAL_LAYER_1000BASE_BX;
      goto out;
    } else
...
STATIC s32 ixgbe_verify_fw_version_82599(struct ixgbe_hw *hw) {
  ...
  if (hw->eeprom.ops.read(hw,
                          (fw_offset + IXGBE_FW_PASSTHROUGH_PATCH_CONFIG_PTR),
                          &fw_ptp_cfg_offset)) {
    ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "eeprom read at offset %d failed",
                  fw_offset + IXGBE_FW_PASSTHROUGH_PATCH_CONFIG_PTR);
    return IXGBE_ERR_EEPROM_VERSION;
  ...
  if (hw->eeprom.ops.read(hw, (fw_ptp_cfg_offset + IXGBE_FW_PATCH_VERSION_4),
                          &fw_version)) {
    ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "eeprom read at offset %d failed",
...
bool ixgbe_verify_lesm_fw_enabled_82599(struct ixgbe_hw *hw) {
  ...
  if ((status != IXGBE_SUCCESS) || (fw_offset == 0) || (fw_offset == 0xFFFF))
    goto out;

  /* get the offset to the LESM Parameters block */
  status = hw->eeprom.ops.read(hw, (fw_offset + IXGBE_FW_LESM_PARAMETERS_PTR),
                               &fw_lesm_param_offset);

  if ((status != IXGBE_SUCCESS) || (fw_lesm_param_offset == 0) ||
      (fw_lesm_param_offset == 0xFFFF))
    goto out;

  /* get the LESM state word */
  status = hw->eeprom.ops.read(
      hw, (fw_lesm_param_offset + IXGBE_FW_LESM_STATE_1), &fw_lesm_state);
...
STATIC s32 ixgbe_read_eeprom_buffer_82599(struct ixgbe_hw *hw, u16 offset,
  ...
  if ((eeprom->type == ixgbe_eeprom_spi) &&
    ...
    ret_val = ixgbe_read_eerd_buffer_generic(hw, offset, words, data);
  else
    ret_val =
        ixgbe_read_eeprom_buffer_bit_bang_generic(hw, offset, words, data);
...
s32 ixgbe_reset_pipeline_82599(struct ixgbe_hw *hw) {
  ...
  IXGBE_WRITE_REG(hw, IXGBE_AUTOC, autoc_reg ^ (0x4 << IXGBE_AUTOC_LMS_SHIFT));
