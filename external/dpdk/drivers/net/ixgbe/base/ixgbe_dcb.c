...
s32 ixgbe_dcb_calculate_tc_credits_cee(struct ixgbe_hw *hw,
  ...
  for (i = 0; i < IXGBE_DCB_MAX_TRAFFIC_CLASS; i++) {
    ...
    credit_refill =
        min(link_percentage * min_multiplier, (u32)IXGBE_DCB_MAX_CREDIT_REFILL);
    ...
    if (direction == IXGBE_DCB_TX_CONFIG) {
       ...
       */
      if (credit_max && (credit_max < IXGBE_DCB_MIN_TSO_CREDIT) &&
 ...
 */
void ixgbe_dcb_unpack_pfc_cee(struct ixgbe_dcb_config *cfg, u8 *map,
                              u8 *pfc_up) {
...
s32 ixgbe_dcb_check_config_cee(struct ixgbe_dcb_config *dcb_config) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    for (j = 0; j < IXGBE_DCB_MAX_BW_GROUP; j++) {
      ...
      } else if (bw_sum[i][j] != IXGBE_DCB_BW_PERCENT && bw_sum[i][j] != 0) {
        ret_val = IXGBE_ERR_CONFIG;
        goto err_config;
      }
    }
...
s32 ixgbe_dcb_config_rx_arbiter_cee(struct ixgbe_hw *hw,
  ...
  case ixgbe_mac_X550EM_x:
  case ixgbe_mac_X550EM_a:
    ret = ixgbe_dcb_config_rx_arbiter_82599(hw, refill, max, bwgid, tsa, map);
    break;
  default:
    break;
  }
...
s32 ixgbe_dcb_config_tx_desc_arbiter_cee(struct ixgbe_hw *hw,
  ...
  switch (hw->mac.type) {
  case ixgbe_mac_82598EB:
    ret = ixgbe_dcb_config_tx_desc_arbiter_82598(hw, refill, max, bwgid, tsa);
    break;
  case ixgbe_mac_82599EB:
  case ixgbe_mac_X540:
  case ixgbe_mac_X550:
  case ixgbe_mac_X550EM_x:
  case ixgbe_mac_X550EM_a:
    ret = ixgbe_dcb_config_tx_desc_arbiter_82599(hw, refill, max, bwgid, tsa);
    break;
  default:
    break;
  }
...
s32 ixgbe_dcb_config_tx_data_arbiter_cee(struct ixgbe_hw *hw,
  ...
  switch (hw->mac.type) {
  case ixgbe_mac_82598EB:
    ret = ixgbe_dcb_config_tx_data_arbiter_82598(hw, refill, max, bwgid, tsa);
    break;
  case ixgbe_mac_82599EB:
  case ixgbe_mac_X540:
  case ixgbe_mac_X550:
  case ixgbe_mac_X550EM_x:
  case ixgbe_mac_X550EM_a:
    ret = ixgbe_dcb_config_tx_data_arbiter_82599(hw, refill, max, bwgid, tsa,
                                                 map);
    break;
  default:
    break;
...
s32 ixgbe_dcb_hw_config_cee(struct ixgbe_hw *hw,
  ...
  switch (hw->mac.type) {
  case ixgbe_mac_82598EB:
    ret = ixgbe_dcb_hw_config_82598(hw, dcb_config->link_speed, refill, max,
                                    bwgid, tsa);
    break;
  case ixgbe_mac_82599EB:
  case ixgbe_mac_X540:
  ...
  case ixgbe_mac_X550EM_a:
    ...
    ret = ixgbe_dcb_hw_config_82599(hw, dcb_config->link_speed, refill, max,
                                    bwgid, tsa, map);
...
s32 ixgbe_dcb_hw_config(struct ixgbe_hw *hw, u16 *refill, u16 *max, u8 *bwg_id,
                        u8 *tsa, u8 *map) {
  switch (hw->mac.type) {
  case ixgbe_mac_82598EB:
