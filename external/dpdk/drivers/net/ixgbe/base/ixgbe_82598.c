...
s32 ixgbe_start_hw_82598(struct ixgbe_hw *hw) {
  ...
  for (i = 0; ((i < hw->mac.max_tx_queues) && (i < IXGBE_DCA_MAX_QUEUES_82598));
       i++) {
  ...
  for (i = 0; ((i < hw->mac.max_rx_queues) && (i < IXGBE_DCA_MAX_QUEUES_82598));
       i++) {
...
STATIC s32 ixgbe_start_mac_link_82598(struct ixgbe_hw *hw,
  ...
  if (autoneg_wait_to_complete) {
    if ((autoc_reg & IXGBE_AUTOC_LMS_MASK) == IXGBE_AUTOC_LMS_KX4_AN ||
        (autoc_reg & IXGBE_AUTOC_LMS_MASK) == IXGBE_AUTOC_LMS_KX4_AN_1G_AN) {
...
STATIC s32 ixgbe_validate_link_ready(struct ixgbe_hw *hw) {
  ...
  for (timeout = 0; timeout < IXGBE_VALIDATE_LINK_READY_TIMEOUT; timeout++) {
...
STATIC s32 ixgbe_check_mac_link_82598(struct ixgbe_hw *hw,
  ...
  if (hw->phy.type == ixgbe_phy_nl) {
    ...
    hw->phy.ops.read_reg(hw, 0xC79F, IXGBE_TWINAX_DEV, &link_reg);
    hw->phy.ops.read_reg(hw, 0xC00C, IXGBE_TWINAX_DEV, &adapt_comp_reg);
...
STATIC s32 ixgbe_reset_hw_82598(struct ixgbe_hw *hw) {
  ...
  if (analog_val & IXGBE_ATLAS_PDN_TX_REG_EN) {
    ...
    hw->mac.ops.write_analog_reg8(hw, IXGBE_ATLAS_PDN_LPBK, analog_val);

    hw->mac.ops.read_analog_reg8(hw, IXGBE_ATLAS_PDN_10G, &analog_val);
    analog_val &= ~IXGBE_ATLAS_PDN_TX_10G_QL_ALL;
    hw->mac.ops.write_analog_reg8(hw, IXGBE_ATLAS_PDN_10G, analog_val);

    hw->mac.ops.read_analog_reg8(hw, IXGBE_ATLAS_PDN_1G, &analog_val);
    analog_val &= ~IXGBE_ATLAS_PDN_TX_1G_QL_ALL;
    hw->mac.ops.write_analog_reg8(hw, IXGBE_ATLAS_PDN_1G, analog_val);

    hw->mac.ops.read_analog_reg8(hw, IXGBE_ATLAS_PDN_AN, &analog_val);
    analog_val &= ~IXGBE_ATLAS_PDN_TX_AN_QL_ALL;
    hw->mac.ops.write_analog_reg8(hw, IXGBE_ATLAS_PDN_AN, analog_val);
  }
...
STATIC s32 ixgbe_clear_vfta_82598(struct ixgbe_hw *hw) {
  ...
  for (vlanbyte = 0; vlanbyte < 4; vlanbyte++)
    for (offset = 0; offset < hw->mac.vft_size; offset++)
      IXGBE_WRITE_REG(hw, IXGBE_VFTAVIND(vlanbyte, offset), 0);
...
STATIC s32 ixgbe_read_i2c_phy_82598(struct ixgbe_hw *hw, u8 dev_addr,
  ...
  if (hw->phy.type == ixgbe_phy_nl) {
    ...
    hw->phy.ops.write_reg_mdi(hw, IXGBE_MDIO_PMA_PMD_SDA_SCL_ADDR,
                              IXGBE_MDIO_PMA_PMD_DEV_TYPE, sfp_addr);

    /* Poll status */
    for (i = 0; i < 100; i++) {
      hw->phy.ops.read_reg_mdi(hw, IXGBE_MDIO_PMA_PMD_SDA_SCL_STAT,
...
s32 ixgbe_read_i2c_eeprom_82598(struct ixgbe_hw *hw, u8 byte_offset,
                                u8 *eeprom_data) {
  return ixgbe_read_i2c_phy_82598(hw, IXGBE_I2C_EEPROM_DEV_ADDR, byte_offset,
                                  eeprom_data);
}
...
STATIC s32 ixgbe_read_i2c_sff8472_82598(struct ixgbe_hw *hw, u8 byte_offset,
                                        u8 *sff8472_data) {
  return ixgbe_read_i2c_phy_82598(hw, IXGBE_I2C_EEPROM_DEV_ADDR2, byte_offset,
                                  sff8472_data);
}
