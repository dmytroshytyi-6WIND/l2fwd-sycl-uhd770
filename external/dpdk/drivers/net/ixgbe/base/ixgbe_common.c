...
s32 ixgbe_init_ops_generic(struct ixgbe_hw *hw) {
  ...
  if (eec & IXGBE_EEC_PRES) {
    ...
    eeprom->ops.read_buffer = ixgbe_read_eeprom_buffer_bit_bang_generic;
  }
...
bool ixgbe_device_supports_autoneg_fc(struct ixgbe_hw *hw) {
  ...
  case ixgbe_media_type_fiber:
    ...
    default:
      ...
      if (link_up)
        supported = speed == IXGBE_LINK_SPEED_1GB_FULL ? true : false;
      else
        supported = true;
    }
...
s32 ixgbe_setup_fc_generic(struct ixgbe_hw *hw) {
  ...
  case ixgbe_fc_none:
    ...
    if (hw->phy.media_type == ixgbe_media_type_backplane)
      reg_bp &= ~(IXGBE_AUTOC_SYM_PAUSE | IXGBE_AUTOC_ASM_PAUSE);
...
s32 ixgbe_clear_hw_cntrs_generic(struct ixgbe_hw *hw) {
  ...
  if (hw->mac.type == ixgbe_mac_X550 || hw->mac.type == ixgbe_mac_X540) {
    ...
    hw->phy.ops.read_reg(hw, IXGBE_PCRC8ECL, IXGBE_MDIO_PCS_DEV_TYPE, &i);
    hw->phy.ops.read_reg(hw, IXGBE_PCRC8ECH, IXGBE_MDIO_PCS_DEV_TYPE, &i);
    hw->phy.ops.read_reg(hw, IXGBE_LDPCECL, IXGBE_MDIO_PCS_DEV_TYPE, &i);
    hw->phy.ops.read_reg(hw, IXGBE_LDPCECH, IXGBE_MDIO_PCS_DEV_TYPE, &i);
  }
...
s32 ixgbe_read_pba_raw(struct ixgbe_hw *hw, u16 *eeprom_buf,
  ...
  if (eeprom_buf == NULL) {
    ret_val =
        hw->eeprom.ops.read_buffer(hw, IXGBE_PBANUM0_PTR, 2, &pba->word[0]);
    if (ret_val)
      return ret_val;
  } else {
  ...
  if (pba->word[0] == IXGBE_PBANUM_PTR_GUARD) {
    ...
    ret_val = ixgbe_get_pba_block_size(hw, eeprom_buf, eeprom_buf_size,
                                       &pba_block_size);
    if (ret_val)
      return ret_val;
    ...
    if (eeprom_buf == NULL) {
      ret_val = hw->eeprom.ops.read_buffer(hw, pba->word[1], pba_block_size,
                                           pba->pba_block);
      if (ret_val)
        return ret_val;
    } else {
      if (eeprom_buf_size > (u32)(pba->word[1] + pba_block_size)) {
        memcpy(pba->pba_block, &eeprom_buf[pba->word[1]],
...
s32 ixgbe_write_pba_raw(struct ixgbe_hw *hw, u16 *eeprom_buf,
  ...
  if (eeprom_buf == NULL) {
    ret_val =
        hw->eeprom.ops.write_buffer(hw, IXGBE_PBANUM0_PTR, 2, &pba->word[0]);
    if (ret_val)
      return ret_val;
  } else {
    if (eeprom_buf_size > IXGBE_PBANUM1_PTR) {
  ...
  if (pba->word[0] == IXGBE_PBANUM_PTR_GUARD) {
    ...
    if (eeprom_buf == NULL) {
      ret_val = hw->eeprom.ops.write_buffer(hw, pba->word[1], pba->pba_block[0],
                                            pba->pba_block);
      if (ret_val)
        return ret_val;
    } else {
      if (eeprom_buf_size > (u32)(pba->word[1] + pba->pba_block[0])) {
        memcpy(&eeprom_buf[pba->word[1]], pba->pba_block,
...
s32 ixgbe_get_pba_block_size(struct ixgbe_hw *hw, u16 *eeprom_buf,
  ...
  if (eeprom_buf == NULL) {
    ret_val =
        hw->eeprom.ops.read_buffer(hw, IXGBE_PBANUM0_PTR, 2, &pba_word[0]);
    if (ret_val)
      return ret_val;
  } else {
    if (eeprom_buf_size > IXGBE_PBANUM1_PTR) {
  ...
  if (pba_word[0] == IXGBE_PBANUM_PTR_GUARD) {
    if (eeprom_buf == NULL) {
      ret_val = hw->eeprom.ops.read(hw, pba_word[1] + 0, &length);
      if (ret_val)
        return ret_val;
    } else {
...
s32 ixgbe_init_led_link_act_generic(struct ixgbe_hw *hw) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if ((led_mode & IXGBE_LED_MODE_MASK_BASE) == IXGBE_LED_LINK_ACTIVE) {
      mac->led_link_act = i;
      return IXGBE_SUCCESS;
    }
  }
...
s32 ixgbe_write_eeprom_buffer_bit_bang_generic(struct ixgbe_hw *hw, u16 offset,
   ...
   */
  if ((hw->eeprom.word_page_size == 0) && (words > IXGBE_EEPROM_PAGE_SIZE_MAX))
  ...
  for (i = 0; i < words; i += IXGBE_EEPROM_RD_BUFFER_MAX_COUNT) {
    count = (words - i) / IXGBE_EEPROM_RD_BUFFER_MAX_COUNT > 0
                ...
                : (words - i);
    status =
        ixgbe_write_eeprom_buffer_bit_bang(hw, offset + i, count, &data[i]);
...
STATIC s32 ixgbe_write_eeprom_buffer_bit_bang(struct ixgbe_hw *hw, u16 offset,
  ...
  if (status == IXGBE_SUCCESS) {
    for (i = 0; i < words; i++) {
       ...
       * in the opcode
       */
      if ((hw->eeprom.address_bits == 8) && ((offset + i) >= 128))
        write_opcode |= IXGBE_EEPROM_A8_OPCODE_SPI;

      /* Send the Write command (8-bit opcode + addr) */
      ixgbe_shift_out_eeprom_bits(hw, write_opcode, IXGBE_EEPROM_OPCODE_BITS);
      ...
      do {
        ...
        if (((offset + i) & (page_size - 1)) == (page_size - 1))
          break;
      } while (++i < words);
...
s32 ixgbe_read_eeprom_buffer_bit_bang_generic(struct ixgbe_hw *hw, u16 offset,
  ...
  for (i = 0; i < words; i += IXGBE_EEPROM_RD_BUFFER_MAX_COUNT) {
    ...
    status = ixgbe_read_eeprom_buffer_bit_bang(hw, offset + i, count, &data[i]);
...
STATIC s32 ixgbe_read_eeprom_buffer_bit_bang(struct ixgbe_hw *hw, u16 offset,
  ...
  if (status == IXGBE_SUCCESS) {
    for (i = 0; i < words; i++) {
      ...
      ixgbe_shift_out_eeprom_bits(hw, read_opcode, IXGBE_EEPROM_OPCODE_BITS);
      ixgbe_shift_out_eeprom_bits(hw, (u16)((offset + i) * 2),
                                  hw->eeprom.address_bits);
...
s32 ixgbe_read_eerd_buffer_generic(struct ixgbe_hw *hw, u16 offset, u16 words,
  ...
  for (i = 0; i < words; i++) {
    ...
    if (status == IXGBE_SUCCESS) {
      data[i] = (IXGBE_READ_REG(hw, IXGBE_EERD) >> IXGBE_EEPROM_RW_REG_DATA);
    } else {
...
STATIC s32 ixgbe_detect_eeprom_page_size_generic(struct ixgbe_hw *hw,
  ...
  DEBUGOUT1("Detected EEPROM page size = %d words.", hw->eeprom.word_page_size);
out:
  return status;
}
...
s32 ixgbe_set_rar_generic(struct ixgbe_hw *hw, u32 index, u8 *addr, u32 vmdq,
  ...
  if (index >= rar_entries) {
    ERROR_REPORT2(IXGBE_ERROR_ARGUMENT, "RAR index %d is out of range.\n",
                  index);
    return IXGBE_ERR_INVALID_ARGUMENT;
  }
...
s32 ixgbe_init_rx_addrs_generic(struct ixgbe_hw *hw) {
   ...
   */
  if (ixgbe_validate_mac_addr(hw->mac.addr) == IXGBE_ERR_INVALID_MAC_ADDR) {
    /* Get the MAC address from the RAR0 for later reference */
    hw->mac.ops.get_mac_addr(hw, hw->mac.addr);

    DEBUGOUT3(" Keeping Current RAR0 Addr =%.2X %.2X %.2X ", hw->mac.addr[0],
              hw->mac.addr[1], hw->mac.addr[2]);
    DEBUGOUT3("%.2X %.2X %.2X\n", hw->mac.addr[3], hw->mac.addr[4],
              hw->mac.addr[5]);
  } else {
    /* Setup the receive address. */
    DEBUGOUT("Overriding MAC Address in RAR[0]\n");
    DEBUGOUT3(" New MAC Addr =%.2X %.2X %.2X ", hw->mac.addr[0],
              hw->mac.addr[1], hw->mac.addr[2]);
    DEBUGOUT3("%.2X %.2X %.2X\n", hw->mac.addr[3], hw->mac.addr[4],
              hw->mac.addr[5]);
...
s32 ixgbe_update_mc_addr_list_generic(struct ixgbe_hw *hw, u8 *mc_addr_list,
  ...
  /* Enable mta */
  for (i = 0; i < hw->mac.mcft_size; i++)
    IXGBE_WRITE_REG_ARRAY(hw, IXGBE_MTA(0), i, hw->mac.mta_shadow[i]);
...
s32 ixgbe_fc_enable_generic(struct ixgbe_hw *hw) {
  ...
  for (i = 0; i < IXGBE_DCB_MAX_TRAFFIC_CLASS; i++) {
    if ((hw->fc.current_mode & ixgbe_fc_tx_pause) && hw->fc.high_water[i]) {
...
STATIC s32 ixgbe_fc_autoneg_fiber(struct ixgbe_hw *hw) {
  ...
  ret_val =
      ixgbe_negotiate_fc(hw, pcs_anadv_reg, pcs_lpab_reg,
                         IXGBE_PCS1GANA_SYM_PAUSE, IXGBE_PCS1GANA_ASM_PAUSE,
                         IXGBE_PCS1GANA_SYM_PAUSE, IXGBE_PCS1GANA_ASM_PAUSE);
...
STATIC s32 ixgbe_fc_autoneg_backplane(struct ixgbe_hw *hw) {
  ...
  ret_val = ixgbe_negotiate_fc(hw, autoc_reg, anlp1_reg, IXGBE_AUTOC_SYM_PAUSE,
                               IXGBE_AUTOC_ASM_PAUSE, IXGBE_ANLP1_SYM_PAUSE,
...
STATIC s32 ixgbe_fc_autoneg_copper(struct ixgbe_hw *hw) {
  ...
  hw->phy.ops.read_reg(hw, IXGBE_MDIO_AUTO_NEG_LP, IXGBE_MDIO_AUTO_NEG_DEV_TYPE,
                       &lp_technology_ability_reg);

  return ixgbe_negotiate_fc(hw, (u32)technology_ability_reg,
                            (u32)lp_technology_ability_reg, IXGBE_TAF_SYM_PAUSE,
                            IXGBE_TAF_ASM_PAUSE, IXGBE_TAF_SYM_PAUSE,
                            IXGBE_TAF_ASM_PAUSE);
}
...
void ixgbe_fc_autoneg(struct ixgbe_hw *hw) {
  ...
  if (hw->fc.disable_fc_autoneg) {
    ...
    ERROR_REPORT1(IXGBE_ERROR_CAUTION, "Flow control autoneg is disabled");
    goto out;
  }
...
STATIC s32 ixgbe_get_san_mac_addr_offset(struct ixgbe_hw *hw,
   ...
   * available.
   */
  ret_val = hw->eeprom.ops.read(hw, IXGBE_SAN_MAC_ADDR_PTR, san_mac_offset);
  if (ret_val) {
    ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "eeprom at offset %d failed",
...
s32 ixgbe_get_san_mac_addr_generic(struct ixgbe_hw *hw, u8 *san_mac_addr) {
  ...
  for (i = 0; i < 3; i++) {
    ret_val = hw->eeprom.ops.read(hw, san_mac_offset, &san_mac_data);
    if (ret_val) {
      ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE,
                    "eeprom read at offset %d failed", san_mac_offset);
      goto san_mac_addr_out;
    }
...
s32 ixgbe_insert_mac_addr_generic(struct ixgbe_hw *hw, u8 *addr, u32 vmdq) {
  ...
  addr_low = addr[0] | (addr[1] << 8) | (addr[2] << 16) | (addr[3] << 24);
...
s32 ixgbe_set_vfta_generic(struct ixgbe_hw *hw, u32 vlan, u32 vind,
   ...
   */
  ret_val = ixgbe_set_vlvf_generic(hw, vlan, vind, vlan_on, &vfta_delta, vfta,
                                   vlvf_bypass);
  if (ret_val != IXGBE_SUCCESS) {
    if (vlvf_bypass)
      goto vfta_update;
...
s32 ixgbe_check_mac_link_generic(struct ixgbe_hw *hw, ixgbe_link_speed *speed,
  ...
  if (ixgbe_need_crosstalk_fix(hw)) {
    ...
    switch (hw->mac.type) {
    case ixgbe_mac_82599EB:
      sfp_cage_full = IXGBE_READ_REG(hw, IXGBE_ESDP) & IXGBE_ESDP_SDP2;
      break;
    case ixgbe_mac_X550EM_x:
    case ixgbe_mac_X550EM_a:
      sfp_cage_full = IXGBE_READ_REG(hw, IXGBE_ESDP) & IXGBE_ESDP_SDP0;
      break;
    default:
...
s32 ixgbe_get_wwn_prefix_generic(struct ixgbe_hw *hw, u16 *wwnn_prefix,
  ...
  *wwpn_prefix = 0xFFFF;
  ...
  if (hw->eeprom.ops.read(hw, offset, wwnn_prefix)) {
    ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "eeprom read at offset %d failed",
                  offset);
  }
...
s32 ixgbe_hic_unlocked(struct ixgbe_hw *hw, u32 *buffer, u32 length,
  ...
  for (i = 0; i < dword_len; i++)
    IXGBE_WRITE_REG_ARRAY(hw, IXGBE_FLEX_MNG, i, IXGBE_CPU_TO_LE32(buffer[i]));
...
s32 ixgbe_host_interface_command(struct ixgbe_hw *hw, u32 *buffer, u32 length,
  ...
  if (resp->cmd == IXGBE_HOST_INTERFACE_FLASH_READ_CMD ||
    ...
    buf_len =
        (((u16)(resp->cmd_or_resp.ret_status) << 3) & 0xF00) | resp->buf_len;
    hdr_size += (2 << 2);
  } else {
...
out:
  ...
  IXGBE_WRITE_REG(hw, IXGBE_GCR_EXT, gcr_ext | IXGBE_GCR_EXT_BUFFERS_CLEAR);
...
STATIC const u8 ixgbe_emc_temp_data[4] = {
    IXGBE_EMC_INTERNAL_DATA, IXGBE_EMC_DIODE1_DATA, IXGBE_EMC_DIODE2_DATA,
    IXGBE_EMC_DIODE3_DATA};
STATIC const u8 ixgbe_emc_therm_limit[4] = {
    IXGBE_EMC_INTERNAL_THERM_LIMIT, IXGBE_EMC_DIODE1_THERM_LIMIT,
    IXGBE_EMC_DIODE2_THERM_LIMIT, IXGBE_EMC_DIODE3_THERM_LIMIT};
...
s32 ixgbe_get_thermal_sensor_data_generic(struct ixgbe_hw *hw) {
  ...
  for (i = 0; i < num_sensors; i++) {
    status = hw->eeprom.ops.read(hw, (ets_offset + 1 + i), &ets_sensor);
    if (status)
      goto out;
    ...
    sensor_location =
        ((ets_sensor & IXGBE_ETS_DATA_LOC_MASK) >> IXGBE_ETS_DATA_LOC_SHIFT);

    if (sensor_location != 0) {
      status = hw->phy.ops.read_i2c_byte(hw, ixgbe_emc_temp_data[sensor_index],
...
s32 ixgbe_init_thermal_sensor_thresh_generic(struct ixgbe_hw *hw) {
  ...
  low_thresh_delta =
      ((ets_cfg & IXGBE_ETS_LTHRES_DELTA_MASK) >> IXGBE_ETS_LTHRES_DELTA_SHIFT);
  num_sensors = (ets_cfg & IXGBE_ETS_NUM_SENSORS_MASK);
  ...
  for (i = 0; i < num_sensors; i++) {
    ...
    hw->phy.ops.write_i2c_byte(hw, ixgbe_emc_therm_limit[sensor_index],
...
void ixgbe_get_orom_version(struct ixgbe_hw *hw,
  ...
  if ((eeprom_cfg_blkl | eeprom_cfg_blkh) == 0x0 ||
      eeprom_cfg_blkl == NVM_VER_INVALID || eeprom_cfg_blkh == NVM_VER_INVALID)
    return;

  nvm_ver->or_valid = true;
  nvm_ver->or_major = eeprom_cfg_blkl >> NVM_OROM_SHIFT;
  nvm_ver->or_build =
      (eeprom_cfg_blkl << NVM_OROM_SHIFT) | (eeprom_cfg_blkh >> NVM_OROM_SHIFT);
...
void ixgbe_get_oem_prod_version(struct ixgbe_hw *hw,
  ...
  if ((rel_num | prod_ver) == 0x0 || rel_num == NVM_VER_INVALID ||
      prod_ver == NVM_VER_INVALID)
    return;
...
void ixgbe_dcb_get_rtrup2tc_generic(struct ixgbe_hw *hw, u8 *map) {
  ...
  for (i = 0; i < IXGBE_DCB_MAX_USER_PRIORITY; i++)
    map[i] = IXGBE_RTRUP2TC_UP_MASK & (reg >> (i * IXGBE_RTRUP2TC_UP_SHIFT));
  return;
}
...
s32 ixgbe_setup_mac_link_multispeed_fiber(struct ixgbe_hw *hw,
  ...
  if (speed & IXGBE_LINK_SPEED_10GB_FULL) {
    ...
    switch (hw->phy.media_type) {
    case ixgbe_media_type_fiber:
      ixgbe_set_rate_select_speed(hw, IXGBE_LINK_SPEED_10GB_FULL);
      break;
    case ixgbe_media_type_fiber_qsfp:
    ...
    status = ixgbe_setup_mac_link(hw, IXGBE_LINK_SPEED_10GB_FULL,
  ...
  if (speed & IXGBE_LINK_SPEED_1GB_FULL) {
    ...
    switch (hw->phy.media_type) {
    case ixgbe_media_type_fiber:
      ixgbe_set_rate_select_speed(hw, IXGBE_LINK_SPEED_1GB_FULL);
      break;
    case ixgbe_media_type_fiber_qsfp:
    ...
    status = ixgbe_setup_mac_link(hw, IXGBE_LINK_SPEED_1GB_FULL,
                                  autoneg_wait_to_complete);
    if (status != IXGBE_SUCCESS)
      return status;
   ...
   */
  if (speedcnt > 1)
    status = ixgbe_setup_mac_link_multispeed_fiber(hw, highest_link_speed,
                                                   autoneg_wait_to_complete);
...
void ixgbe_set_soft_rate_select_speed(struct ixgbe_hw *hw,
  ...
  status = hw->phy.ops.read_i2c_byte(hw, IXGBE_SFF_SFF_8472_OSCB,
                                     IXGBE_I2C_EEPROM_DEV_ADDR2, &eeprom_data);
  if (status) {
