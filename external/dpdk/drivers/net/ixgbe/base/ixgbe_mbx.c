...
s32 ixgbe_write_mbx(struct ixgbe_hw *hw, u32 *msg, u16 size, u16 mbx_id) {
  ...
  if (size > mbx->size) {
    ...
    ERROR_REPORT2(IXGBE_ERROR_ARGUMENT, "Invalid mailbox message size %d",
                  size);
...
STATIC s32 ixgbe_poll_for_ack(struct ixgbe_hw *hw, u16 mbx_id) {
  ...
  if (countdown == 0)
    ERROR_REPORT2(IXGBE_ERROR_POLLING, "Polling for VF%d mailbox ack timedout",
                  mbx_id);
...
STATIC s32 ixgbe_check_for_msg_pf(struct ixgbe_hw *hw, u16 vf_number) {
  ...
  if (!ixgbe_check_for_bit_pf(hw, IXGBE_MBVFICR_VFREQ_VF1 << vf_bit, index)) {
    ret_val = IXGBE_SUCCESS;
    hw->mbx.stats.reqs++;
  }
...
STATIC s32 ixgbe_check_for_ack_pf(struct ixgbe_hw *hw, u16 vf_number) {
  ...
  if (!ixgbe_check_for_bit_pf(hw, IXGBE_MBVFICR_VFACK_VF1 << vf_bit, index)) {
    ret_val = IXGBE_SUCCESS;
    hw->mbx.stats.acks++;
  }
...
STATIC s32 ixgbe_obtain_mbx_lock_pf(struct ixgbe_hw *hw, u16 vf_number) {
  ...
  if (p2v_mailbox & IXGBE_PFMAILBOX_PFU)
    ...
    ERROR_REPORT2(IXGBE_ERROR_POLLING, "Failed to obtain mailbox lock for VF%d",
                  vf_number);
...
void ixgbe_init_mbx_params_pf(struct ixgbe_hw *hw) {
  ...
  if (hw->mac.type != ixgbe_mac_82599EB && hw->mac.type != ixgbe_mac_X550 &&
      hw->mac.type != ixgbe_mac_X550EM_x &&
      hw->mac.type != ixgbe_mac_X550EM_a && hw->mac.type != ixgbe_mac_X540)
    return;
