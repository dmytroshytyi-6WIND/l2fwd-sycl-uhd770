...
s32 ixgbe_validate_eeprom_checksum_X540(struct ixgbe_hw *hw,
   ...
   */
  status = ixgbe_read_eerd_generic(hw, IXGBE_EEPROM_CHECKSUM, &read_checksum);
  if (status)
    goto out;
...
STATIC s32 ixgbe_poll_flash_update_done_X540(struct ixgbe_hw *hw) {
  ...
  if (i == IXGBE_FLUDONE_ATTEMPTS)
    ERROR_REPORT1(IXGBE_ERROR_POLLING, "Flash update status polling timed out");
...
s32 ixgbe_acquire_swfw_sync_X540(struct ixgbe_hw *hw, u32 mask) {
  ...
  for (i = 0; i < timeout; i++) {
    ...
    if (ixgbe_get_swfw_sync_semaphore(hw)) {
      DEBUGOUT("Failed to get NVM access and register semaphore, returning "
               "IXGBE_ERR_SWFW_SYNC\n");
      return IXGBE_ERR_SWFW_SYNC;
    }
   ...
   */
  if (ixgbe_get_swfw_sync_semaphore(hw)) {
    DEBUGOUT("Failed to get NVM semaphore and register semaphore while "
             "forcefully ignoring FW semaphore bit(s) and setting SW semaphore "
             "bit(s), returning IXGBE_ERR_SWFW_SYNC\n");
    return IXGBE_ERR_SWFW_SYNC;
  }
  swfw_sync = IXGBE_READ_REG(hw, IXGBE_SWFW_SYNC_BY_MAC(hw));
   ...
   */
  if (swfw_sync & swmask) {
    u32 rmask = IXGBE_GSSR_EEP_SM | IXGBE_GSSR_PHY0_SM | IXGBE_GSSR_PHY1_SM |
                IXGBE_GSSR_MAC_CSR_SM | IXGBE_GSSR_SW_MNG_SM;
...
void ixgbe_init_swfw_sync_X540(struct ixgbe_hw *hw) {
  ...
  rmask = IXGBE_GSSR_EEP_SM | IXGBE_GSSR_PHY0_SM | IXGBE_GSSR_PHY1_SM |
          IXGBE_GSSR_MAC_CSR_SM | IXGBE_GSSR_SW_MNG_SM;
