...
s32 ixgbe_init_ops_X550(struct ixgbe_hw *hw) {
  ...
  mac->ops.set_source_address_pruning = ixgbe_set_source_address_pruning_X550;
  mac->ops.set_ethertype_anti_spoofing = ixgbe_set_ethertype_anti_spoofing_X550;
...
STATIC s32 ixgbe_read_pe(struct ixgbe_hw *hw, u8 reg, u8 *value) {
  ...
  if (status != IXGBE_SUCCESS)
    ERROR_REPORT2(IXGBE_ERROR_CAUTION, "port expander access failed with %d\n",
                  status);
  return status;
}
...
STATIC s32 ixgbe_reset_cs4227(struct ixgbe_hw *hw) {
  ...
  for (retry = 0; retry < IXGBE_CS4227_RETRIES; retry++) {
    status = ixgbe_read_cs4227(hw, IXGBE_CS4227_EFUSE_STATUS, &value);
    if (status == IXGBE_SUCCESS && value == IXGBE_CS4227_EEPROM_LOAD_OK)
      break;
    msec_delay(IXGBE_CS4227_CHECK_DELAY);
  }
  if (retry == IXGBE_CS4227_RETRIES) {
    ERROR_REPORT1(IXGBE_ERROR_INVALID_STATE, "CS4227 reset did not complete.");
...
STATIC void ixgbe_check_cs4227(struct ixgbe_hw *hw) {
  ...
  for (retry = 0; retry < IXGBE_CS4227_RETRIES; retry++) {
    ...
    if (status != IXGBE_SUCCESS) {
      ERROR_REPORT2(IXGBE_ERROR_CAUTION, "semaphore failed with %d", status);
      msec_delay(IXGBE_CS4227_CHECK_DELAY);
      continue;
    }
    ...
    if (status != IXGBE_SUCCESS || value != IXGBE_CS4227_RESET_PENDING)
      break;
  ...
  if (status != IXGBE_SUCCESS) {
    ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "CS4227 reset failed: %d", status);
    goto out;
  }
   ...
   */
  ixgbe_write_cs4227(hw, IXGBE_CS4227_SCRATCH, IXGBE_CS4227_RESET_PENDING);
  hw->mac.ops.release_swfw_sync(hw, swfw_mask);
  msec_delay(10);
  status = hw->mac.ops.acquire_swfw_sync(hw, swfw_mask);
  if (status != IXGBE_SUCCESS) {
  ...
  status =
      ixgbe_write_cs4227(hw, IXGBE_CS4227_SCRATCH, IXGBE_CS4227_RESET_COMPLETE);
...
s32 ixgbe_fw_phy_activity(struct ixgbe_hw *hw, u16 activity,
  ...
  do {
    ...
    rc = ixgbe_host_interface_command(hw, (u32 *)&hic.cmd, sizeof(hic.cmd),
                                      IXGBE_HI_COMMAND_TIMEOUT, true);
    if (rc != IXGBE_SUCCESS)
      return rc;
    if (hic.rsp.hdr.cmd_or_resp.ret_status == FW_CEM_RESP_STATUS_SUCCESS) {
      for (i = 0; i < FW_PHY_ACT_DATA_COUNT; ++i)
 ...
 * Returns an error code on error.
 **/
STATIC s32 ixgbe_write_i2c_combined_generic(struct ixgbe_hw *hw, u8 addr,
                                            u16 reg, u16 val) {
...
s32 ixgbe_init_ops_X550EM_a(struct ixgbe_hw *hw) {
  ...
  case IXGBE_DEV_ID_X550EM_A_1G_T_L:
    ...
    hw->phy.eee_speeds_supported =
        IXGBE_LINK_SPEED_100_FULL | IXGBE_LINK_SPEED_1GB_FULL;
    hw->phy.eee_speeds_advertised = hw->phy.eee_speeds_supported;
    break;
  default:
    break;
...
s32 ixgbe_init_ops_X550EM_x(struct ixgbe_hw *hw) {
  ...
  link->ops.write_link_unlocked = ixgbe_write_i2c_combined_generic_unlocked;
...
s32 ixgbe_dmac_config_tcs_X550(struct ixgbe_hw *hw) {
  ...
  maxframe_size_kb =
      ((IXGBE_READ_REG(hw, IXGBE_MAXFRS) >> IXGBE_MHADD_MFS_SHIFT) / 1024);
  ...
  for (tc = 0; tc < IXGBE_DCB_MAX_TRAFFIC_CLASS; tc++) {
    ...
    if (tc < hw->mac.dmac_config.num_tcs) {
      ...
      rx_pb_size = (rx_pb_size & IXGBE_RXPBSIZE_MASK) >> IXGBE_RXPBSIZE_SHIFT;
      ...
      reg |= (rx_pb_size > maxframe_size_kb) ? rx_pb_size : maxframe_size_kb;
    }
...
s32 ixgbe_init_eeprom_params_X550(struct ixgbe_hw *hw) {
  ...
  if (eeprom->type == ixgbe_eeprom_uninitialized) {
    ...
    eeprom_size = (u16)((eec & IXGBE_EEC_SIZE) >> IXGBE_EEC_SIZE_SHIFT);
...
s32 ixgbe_write_iosf_sb_reg_x550(struct ixgbe_hw *hw, u32 reg_addr,
  ...
  if ((command & IXGBE_SB_IOSF_CTRL_RESP_STAT_MASK) != 0) {
    ...
    ERROR_REPORT2(IXGBE_ERROR_POLLING, "Failed to write, error %x\n", error);
    ret = IXGBE_ERR_PHY;
  }
...
s32 ixgbe_read_iosf_sb_reg_x550(struct ixgbe_hw *hw, u32 reg_addr,
  ...
  if ((command & IXGBE_SB_IOSF_CTRL_RESP_STAT_MASK) != 0) {
    error = (command & IXGBE_SB_IOSF_CTRL_CMPL_ERR_MASK) >>
            IXGBE_SB_IOSF_CTRL_CMPL_ERR_SHIFT;
    ERROR_REPORT2(IXGBE_ERROR_POLLING, "Failed to read, error %x\n", error);
    ret = IXGBE_ERR_PHY;
  }
...
s32 ixgbe_get_phy_token(struct ixgbe_hw *hw) {
  ...
  token_cmd.pad = 0;
  status = ixgbe_host_interface_command(
      hw, (u32 *)&token_cmd, sizeof(token_cmd), IXGBE_HI_COMMAND_TIMEOUT, true);
  if (status) {
  ...
  if (token_cmd.hdr.cmd_or_resp.ret_status != FW_PHY_TOKEN_RETRY) {
    DEBUGOUT1("Host interface command returned 0x%08x , returning "
...
s32 ixgbe_write_iosf_sb_reg_x550a(struct ixgbe_hw *hw, u32 reg_addr,
  ...
  status =
      ixgbe_host_interface_command(hw, (u32 *)&write_cmd, sizeof(write_cmd),
...
s32 ixgbe_read_iosf_sb_reg_x550a(struct ixgbe_hw *hw, u32 reg_addr,
  ...
  status = ixgbe_host_interface_command(hw, (u32 *)&hic.cmd, sizeof(hic.cmd),
                                        IXGBE_HI_COMMAND_TIMEOUT, true);
 ...
 */
STATIC s32 ixgbe_supported_sfp_modules_X550em(struct ixgbe_hw *hw,
                                              bool *linear) {
...
STATIC s32 ixgbe_restart_an_internal_phy_x550em(struct ixgbe_hw *hw) {
  ...
  status =
      hw->mac.ops.read_iosf_sb_reg(hw, IXGBE_KRM_LINK_CTRL_1(hw->bus.lan_id),
  ...
  status =
      hw->mac.ops.write_iosf_sb_reg(hw, IXGBE_KRM_LINK_CTRL_1(hw->bus.lan_id),
                                    IXGBE_SB_IOSF_TARGET_KR_PHY, link_ctrl);
  ...
  if (hw->mac.type == ixgbe_mac_X550EM_a) {
    ...
    status = hw->mac.ops.read_iosf_sb_reg(
        hw, IXGBE_KRM_PMD_FLX_MASK_ST20(hw->bus.lan_id),
    ...
    status = hw->mac.ops.write_iosf_sb_reg(
        hw, IXGBE_KRM_PMD_FLX_MASK_ST20(hw->bus.lan_id),
        IXGBE_SB_IOSF_TARGET_KR_PHY, flx_mask_st20);
  }
...
STATIC s32 ixgbe_setup_sgmii(struct ixgbe_hw *hw, ixgbe_link_speed speed,
  ...
  rc = mac->ops.read_iosf_sb_reg(hw, IXGBE_KRM_LINK_CTRL_1(hw->bus.lan_id),
  ...
  rc = mac->ops.write_iosf_sb_reg(hw, IXGBE_KRM_LINK_CTRL_1(hw->bus.lan_id),
                                  IXGBE_SB_IOSF_TARGET_KR_PHY, lval);
  if (rc)
    return rc;

  rc = mac->ops.read_iosf_sb_reg(hw, IXGBE_KRM_SGMII_CTRL(hw->bus.lan_id),
  ...
  rc = mac->ops.write_iosf_sb_reg(hw, IXGBE_KRM_SGMII_CTRL(hw->bus.lan_id),
                                  IXGBE_SB_IOSF_TARGET_KR_PHY, sval);
  if (rc)
    return rc;

  rc =
      mac->ops.read_iosf_sb_reg(hw, IXGBE_KRM_PMD_FLX_MASK_ST20(hw->bus.lan_id),
  ...
  if (hw->phy.media_type == ixgbe_media_type_fiber) {
    ...
    *autoneg = true;
    ...
    case ixgbe_phy_x550em_xfi:
      *speed = IXGBE_LINK_SPEED_1GB_FULL | IXGBE_LINK_SPEED_10GB_FULL;
      *autoneg = false;
      break;
    ...
    case ixgbe_phy_x550em_kr:
      if (hw->mac.type == ixgbe_mac_X550EM_a) {
        ...
        if (hw->phy.nw_mng_if_sel & IXGBE_NW_MNG_IF_SEL_PHY_SPEED_2_5G) {
          *speed = IXGBE_LINK_SPEED_2_5GB_FULL;
          break;
        } else if (hw->device_id == IXGBE_DEV_ID_X550EM_A_KR_L) {
          *speed = IXGBE_LINK_SPEED_1GB_FULL;
          break;
        }
      }
...
STATIC s32 ixgbe_get_lasi_ext_t_x550em(struct ixgbe_hw *hw, bool *lsc) {
  ...
  *lsc = false;
  ...
  if (status != IXGBE_SUCCESS || !(reg & IXGBE_MDIO_GLOBAL_VEN_ALM_INT_EN))
    return status;
  ...
  if (status != IXGBE_SUCCESS || !(reg & (IXGBE_MDIO_GLOBAL_AN_VEN_ALM_INT_EN |
...
STATIC s32 ixgbe_enable_lasi_ext_t_x550em(struct ixgbe_hw *hw) {
  ...
  reg |=
      (IXGBE_MDIO_GLOBAL_INT_HI_TEMP_EN | IXGBE_MDIO_GLOBAL_INT_DEV_FAULT_EN);
  ...
  reg |= (IXGBE_MDIO_GLOBAL_AN_VEN_ALM_INT_EN | IXGBE_MDIO_GLOBAL_ALARM_1_INT);
...
STATIC s32 ixgbe_read_mng_if_sel_x550em(struct ixgbe_hw *hw) {
  ...
  if (hw->mac.type == ixgbe_mac_X550EM_a &&
    ...
    hw->phy.addr = (hw->phy.nw_mng_if_sel & IXGBE_NW_MNG_IF_SEL_MDIO_PHY_ADD) >>
...
s32 ixgbe_init_phy_ops_X550em(struct ixgbe_hw *hw) {
  ...
  case ixgbe_phy_x550em_ext_t:
    ...
    if ((hw->mac.type == ixgbe_mac_X550EM_x) &&
        !(IXGBE_FUSES0_REV_MASK & IXGBE_READ_REG(hw, IXGBE_FUSES0_GROUP(0))))
...
s32 ixgbe_init_ext_t_x550em(struct ixgbe_hw *hw) {
  ...
  status = hw->phy.ops.read_reg(hw, IXGBE_MDIO_TX_VENDOR_ALARMS_3,
                                IXGBE_MDIO_PMA_PMD_DEV_TYPE, &reg);
  ...
  if (reg & IXGBE_MDIO_TX_VENDOR_ALARMS_3_RST_MASK) {
    status = hw->phy.ops.read_reg(hw, IXGBE_MDIO_GLOBAL_RES_PR_10,
                                  IXGBE_MDIO_VENDOR_SPECIFIC_1_DEV_TYPE, &reg);
    ...
    status = hw->phy.ops.write_reg(hw, IXGBE_MDIO_GLOBAL_RES_PR_10,
                                   IXGBE_MDIO_VENDOR_SPECIFIC_1_DEV_TYPE, reg);
 ...
 **/
s32 ixgbe_setup_mac_link_sfp_x550em(struct ixgbe_hw *hw, ixgbe_link_speed speed,
  ...
  ret_val = hw->link.ops.write_link(hw, hw->link.addr, reg_slice, reg_val);
  return ret_val;
}
...
STATIC s32 ixgbe_setup_sfi_x550a(struct ixgbe_hw *hw, ixgbe_link_speed *speed) {
  ...
  status = mac->ops.write_iosf_sb_reg(
      hw, IXGBE_KRM_PMD_FLX_MASK_ST20(hw->bus.lan_id),
      IXGBE_SB_IOSF_TARGET_KR_PHY, reg_val);
 ...
 **/
s32 ixgbe_setup_mac_link_sfp_x550a(struct ixgbe_hw *hw, ixgbe_link_speed speed,
                                   bool autoneg_wait_to_complete) {
  s32 ret_val;
  u16 reg_phy_ext;
  bool setup_linear = false;
  ...
  if (hw->device_id == IXGBE_DEV_ID_X550EM_A_SFP_N) {
    ...
    ret_val = hw->mac.ops.read_iosf_sb_reg(
        hw, IXGBE_KRM_PMD_FLX_MASK_ST20(hw->bus.lan_id),
    ...
    ret_val = hw->mac.ops.write_iosf_sb_reg(
        hw, IXGBE_KRM_PMD_FLX_MASK_ST20(hw->bus.lan_id),
        IXGBE_SB_IOSF_TARGET_KR_PHY, reg_phy_int);
    ...
    if (reg_phy_ext == IXGBE_CS4223_SKU_ID)
      slice_offset = (hw->bus.lan_id + (hw->bus.instance_id << 1)) << 12;
    else
    ...
    ret_val = hw->phy.ops.read_reg(hw, reg_slice, IXGBE_MDIO_ZERO_DEV_TYPE,
                                   &reg_phy_ext);
    ...
    reg_phy_ext &=
        ~((IXGBE_CS4227_EDC_MODE_CX1 << 1) | (IXGBE_CS4227_EDC_MODE_SR << 1));

    if (setup_linear)
      reg_phy_ext |= (IXGBE_CS4227_EDC_MODE_CX1 << 1) | 0x1;
    else
      reg_phy_ext |= (IXGBE_CS4227_EDC_MODE_SR << 1) | 0x1;
    ret_val = hw->phy.ops.write_reg(hw, reg_slice, IXGBE_MDIO_ZERO_DEV_TYPE,
                                    reg_phy_ext);
...
STATIC s32 ixgbe_setup_ixfi_x550em_x(struct ixgbe_hw *hw) {
  ...
  status = mac->ops.read_iosf_sb_reg(
      hw, IXGBE_KRM_RX_TRN_LINKUP_CTRL(hw->bus.lan_id),
      IXGBE_SB_IOSF_TARGET_KR_PHY, &reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
  ...
  status =
      mac->ops.read_iosf_sb_reg(hw, IXGBE_KRM_DSP_TXFFE_STATE_4(hw->bus.lan_id),
                                IXGBE_SB_IOSF_TARGET_KR_PHY, &reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
  ...
  status = mac->ops.write_iosf_sb_reg(
      hw, IXGBE_KRM_DSP_TXFFE_STATE_4(hw->bus.lan_id),
      IXGBE_SB_IOSF_TARGET_KR_PHY, reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
  status =
      mac->ops.read_iosf_sb_reg(hw, IXGBE_KRM_DSP_TXFFE_STATE_5(hw->bus.lan_id),
                                IXGBE_SB_IOSF_TARGET_KR_PHY, &reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
  reg_val &= ~IXGBE_KRM_DSP_TXFFE_STATE_C0_EN;
  ...
  reg_val &= ~IXGBE_KRM_DSP_TXFFE_STATE_CO_ADAPT_EN;
  status = mac->ops.write_iosf_sb_reg(
      hw, IXGBE_KRM_DSP_TXFFE_STATE_5(hw->bus.lan_id),
      IXGBE_SB_IOSF_TARGET_KR_PHY, reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
  ...
  status =
      mac->ops.read_iosf_sb_reg(hw, IXGBE_KRM_TX_COEFF_CTRL_1(hw->bus.lan_id),
                                IXGBE_SB_IOSF_TARGET_KR_PHY, &reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
  ...
  status =
      mac->ops.write_iosf_sb_reg(hw, IXGBE_KRM_TX_COEFF_CTRL_1(hw->bus.lan_id),
                                 IXGBE_SB_IOSF_TARGET_KR_PHY, reg_val);
  return status;
}
...
STATIC s32 ixgbe_setup_ixfi_x550em(struct ixgbe_hw *hw,
  ...
  status = mac->ops.read_iosf_sb_reg(hw, IXGBE_KRM_LINK_CTRL_1(hw->bus.lan_id),
                                     IXGBE_SB_IOSF_TARGET_KR_PHY, &reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
  ...
  status = mac->ops.write_iosf_sb_reg(hw, IXGBE_KRM_LINK_CTRL_1(hw->bus.lan_id),
                                      IXGBE_SB_IOSF_TARGET_KR_PHY, reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
...
STATIC s32 ixgbe_ext_phy_t_x550em_get_link(struct ixgbe_hw *hw, bool *link_up) {
  ...
  *link_up = false;
  ...
  ret = hw->phy.ops.read_reg(hw, IXGBE_MDIO_AUTO_NEG_STATUS,
                             IXGBE_MDIO_AUTO_NEG_DEV_TYPE, &autoneg_status);
  if (ret != IXGBE_SUCCESS)
    return ret;
...
s32 ixgbe_setup_internal_phy_t_x550em(struct ixgbe_hw *hw) {
  ...
  if (hw->mac.type == ixgbe_mac_X550EM_x &&
    ...
    status = hw->phy.ops.read_reg(hw, IXGBE_MDIO_AUTO_NEG_VENDOR_STAT,
                                  IXGBE_MDIO_AUTO_NEG_DEV_TYPE, &speed);
    if (status != IXGBE_SUCCESS)
      return status;
...
s32 ixgbe_setup_phy_loopback_x550em(struct ixgbe_hw *hw) {
  ...
  status = hw->mac.ops.read_iosf_sb_reg(
      hw, IXGBE_KRM_PORT_CAR_GEN_CTRL(hw->bus.lan_id),
      IXGBE_SB_IOSF_TARGET_KR_PHY, &reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
  ...
  /* Set loopback enable. */
  status =
      hw->mac.ops.read_iosf_sb_reg(hw, IXGBE_KRM_PMD_DFX_BURNIN(hw->bus.lan_id),
                                   IXGBE_SB_IOSF_TARGET_KR_PHY, &reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
  reg_val |= IXGBE_KRM_PMD_DFX_BURNIN_TX_RX_KR_LB_MASK;
  status = hw->mac.ops.write_iosf_sb_reg(
      hw, IXGBE_KRM_PMD_DFX_BURNIN(hw->bus.lan_id), IXGBE_SB_IOSF_TARGET_KR_PHY,
      reg_val);
  if (status != IXGBE_SUCCESS)
    return status;
...
s32 ixgbe_read_ee_hostif_X550(struct ixgbe_hw *hw, u16 offset, u16 *data) {
  ...
  if (!status) {
    *data = (u16)IXGBE_READ_REG_ARRAY(hw, IXGBE_FLEX_MNG, FW_NVM_DATA_OFFSET);
  }
 ...
 **/
s32 ixgbe_read_ee_hostif_buffer_X550(struct ixgbe_hw *hw, u16 offset, u16 words,
                                     u16 *data) {
  const u32 mask = IXGBE_GSSR_SW_MNG_SM | IXGBE_GSSR_EEP_SM;
  struct ixgbe_hic_read_shadow_ram buffer;
  u32 current_word = 0;
  ...
  while (words) {
    ...
    for (i = 0; i < words_to_read; i++) {
      u32 reg = IXGBE_FLEX_MNG + (FW_NVM_DATA_OFFSET << 2) + 2 * i;
...
s32 ixgbe_write_ee_hostif_data_X550(struct ixgbe_hw *hw, u16 offset, u16 data) {
  ...
  if (buffer.hdr.rsp.buf_lenh_status != FW_CEM_RESP_STATUS_SUCCESS) {
    DEBUGOUT2("for offset %04x host interface return status %02x\n", offset,
              buffer.hdr.rsp.buf_lenh_status);
    return IXGBE_ERR_HOST_INTERFACE_COMMAND;
  }
...
s32 ixgbe_write_ee_hostif_X550(struct ixgbe_hw *hw, u16 offset, u16 data) {
  ...
  if (hw->mac.ops.acquire_swfw_sync(hw, IXGBE_GSSR_EEP_SM) == IXGBE_SUCCESS) {
 ...
 **/
s32 ixgbe_write_ee_hostif_buffer_X550(struct ixgbe_hw *hw, u16 offset,
                                      u16 words, u16 *data) {
  s32 status = IXGBE_SUCCESS;
  u32 i = 0;
  ...
  for (i = 0; i < words; i++) {
    status = ixgbe_write_ee_hostif_data_X550(hw, offset + i, data[i]);
 ...
 */
STATIC s32 ixgbe_checksum_ptr_x550(struct ixgbe_hw *hw, u16 ptr, u16 size,
                                   u16 *csum, u16 *buffer, u32 buffer_size) {
  u16 buf[256];
  s32 status;
 ...
 **/
s32 ixgbe_calc_checksum_X550(struct ixgbe_hw *hw, u16 *buffer,
                             u32 buffer_size) {
  ...
  if (!buffer) {
    /* Read pointer area */
    status = ixgbe_read_ee_hostif_buffer_X550(hw, 0, IXGBE_EEPROM_LAST_WORD + 1,
                                              eeprom_ptrs);
    if (status) {
      DEBUGOUT("Failed to read EEPROM image\n");
      return status;
  ...
  for (i = IXGBE_PCIE_ANALOG_PTR_X550; i < IXGBE_FW_PTR; i++) {
    ...
    if (pointer == 0xFFFF || pointer == 0 || pointer >= hw->eeprom.word_size)
      continue;
    ...
    status = ixgbe_checksum_ptr_x550(hw, pointer, size, &checksum, buffer,
                                     buffer_size);
    if (status)
      return status;
  }
...
s32 ixgbe_validate_eeprom_checksum_X550(struct ixgbe_hw *hw,
  ...
  status = ixgbe_read_ee_hostif_X550(hw, IXGBE_EEPROM_CHECKSUM, &read_checksum);
  if (status)
    return status;
  ...
  if (read_checksum != checksum) {
    ...
    ERROR_REPORT1(IXGBE_ERROR_INVALID_STATE, "Invalid EEPROM checksum");
  }
...
s32 ixgbe_update_eeprom_checksum_X550(struct ixgbe_hw *hw) {
  ...
  status = ixgbe_write_ee_hostif_X550(hw, IXGBE_EEPROM_CHECKSUM, checksum);
  if (status)
    return status;
...
u64 ixgbe_get_supported_physical_layer_X550em(struct ixgbe_hw *hw) {
  ...
  case ixgbe_phy_x550em_xfi:
    physical_layer =
        IXGBE_PHYSICAL_LAYER_10GBASE_KR | IXGBE_PHYSICAL_LAYER_1000BASE_KX;
    break;
  case ixgbe_phy_x550em_kx4:
    physical_layer =
        IXGBE_PHYSICAL_LAYER_10GBASE_KX4 | IXGBE_PHYSICAL_LAYER_1000BASE_KX;
    break;
  case ixgbe_phy_x550em_ext_t:
...
s32 ixgbe_enter_lplu_t_x550em(struct ixgbe_hw *hw) {
  ...
  status = hw->phy.ops.read_reg(hw, IXGBE_MII_10GBASE_T_AUTONEG_CTRL_REG,
                                IXGBE_MDIO_AUTO_NEG_DEV_TYPE, &an_10g_cntl_reg);

  if (status != IXGBE_SUCCESS)
    return status;

  status = hw->phy.ops.read_reg(hw, IXGBE_MII_AUTONEG_VENDOR_PROVISION_1_REG,
                                IXGBE_MDIO_AUTO_NEG_DEV_TYPE, &autoneg_reg);
...
s32 ixgbe_get_lcd_t_x550em(struct ixgbe_hw *hw, ixgbe_link_speed *lcd_speed) {
  ...
  *lcd_speed = IXGBE_LINK_SPEED_UNKNOWN;
  ...
  status = hw->phy.ops.read_reg(hw, IXGBE_AUTO_NEG_LP_STATUS,
                                IXGBE_MDIO_AUTO_NEG_DEV_TYPE, &an_lp_status);
...
s32 ixgbe_setup_fc_X550em(struct ixgbe_hw *hw) {
  ...
  case IXGBE_DEV_ID_X550EM_A_KR_L:
    ret_val =
        hw->mac.ops.read_iosf_sb_reg(hw, IXGBE_KRM_AN_CNTL_1(hw->bus.lan_id),
                                     IXGBE_SB_IOSF_TARGET_KR_PHY, &reg_val);
    if (ret_val != IXGBE_SUCCESS)
      goto out;
    reg_val &= ~(IXGBE_KRM_AN_CNTL_1_SYM_PAUSE | IXGBE_KRM_AN_CNTL_1_ASM_PAUSE);
    if (pause)
      reg_val |= IXGBE_KRM_AN_CNTL_1_SYM_PAUSE;
    if (asm_dir)
      reg_val |= IXGBE_KRM_AN_CNTL_1_ASM_PAUSE;
    ret_val =
        hw->mac.ops.write_iosf_sb_reg(hw, IXGBE_KRM_AN_CNTL_1(hw->bus.lan_id),
                                      IXGBE_SB_IOSF_TARGET_KR_PHY, reg_val);
...
void ixgbe_fc_autoneg_backplane_x550em_a(struct ixgbe_hw *hw) {
  ...
  if (hw->fc.disable_fc_autoneg) {
    ERROR_REPORT1(IXGBE_ERROR_UNSUPPORTED, "Flow control autoneg is disabled");
    goto out;
  }
  ...
  status = hw->mac.ops.read_iosf_sb_reg(hw, IXGBE_KRM_LINK_S1(hw->bus.lan_id),
  ...
  status = hw->mac.ops.read_iosf_sb_reg(
      hw, IXGBE_KRM_LP_BASE_PAGE_HIGH(hw->bus.lan_id),
  ...
  status = ixgbe_negotiate_fc(
      hw, an_cntl_1, lp_an_page_low, IXGBE_KRM_AN_CNTL_1_SYM_PAUSE,
      IXGBE_KRM_AN_CNTL_1_ASM_PAUSE, IXGBE_KRM_LP_BASE_PAGE_HIGH_SYM_PAUSE,
...
void ixgbe_fc_autoneg_sgmii_x550em_a(struct ixgbe_hw *hw) {
  ...
  status = ixgbe_negotiate_fc(
      hw, info[0], info[0], FW_PHY_ACT_GET_LINK_INFO_FC_RX,
      FW_PHY_ACT_GET_LINK_INFO_FC_TX, FW_PHY_ACT_GET_LINK_INFO_LP_FC_RX,
...
s32 ixgbe_setup_fc_backplane_x550em_a(struct ixgbe_hw *hw) {
   ...
   */
  status = hw->mac.ops.read_iosf_sb_reg(hw, IXGBE_KRM_AN_CNTL_1(hw->bus.lan_id),
  ...
  case ixgbe_fc_none:
    ...
    an_cntl &= ~(IXGBE_KRM_AN_CNTL_1_SYM_PAUSE | IXGBE_KRM_AN_CNTL_1_ASM_PAUSE);
    break;
  case ixgbe_fc_tx_pause:
 ...
 * instances.
 **/
s32 ixgbe_read_phy_reg_x550a(struct ixgbe_hw *hw, u32 reg_addr, u32 device_type,
                             u16 *phy_data) {
  s32 status;
 ...
 * Returns error status for any failure
 **/
s32 ixgbe_setup_mac_link_t_X550em(struct ixgbe_hw *hw, ixgbe_link_speed speed,
                                  bool autoneg_wait_to_complete) {
  s32 status;
  ixgbe_link_speed force_speed;
  u32 i;
  ...
  if (hw->mac.type == ixgbe_mac_X550EM_x &&
    ...
    for (i = 0; i < 10; i++) {
      ...
      status = ixgbe_check_link(hw, &force_speed, &link_up, false);
      if (status != IXGBE_SUCCESS)
        return status;
 ...
 **/
s32 ixgbe_set_fw_drv_ver_x550(struct ixgbe_hw *hw, u8 maj, u8 min, u8 build,
