...
static int cons_parse_ntuple_filter(const struct rte_flow_attr *attr,
  ...
  if (act->type == RTE_FLOW_ACTION_TYPE_SECURITY) {
    ...
    return ixgbe_crypto_add_ingress_sa_from_flow(
        conf, item->spec, item->type == RTE_FLOW_ITEM_TYPE_IPV6);
  }
#endif
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_ETH) {
    ...
    if ((item->spec || item->mask) &&
        (memcmp(eth_spec, &eth_null, sizeof(struct rte_flow_item_eth)) ||
         memcmp(eth_mask, &eth_null, sizeof(struct rte_flow_item_eth)))) {
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by ntuple filter");
      return -rte_errno;
    }
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_VLAN) {
    ...
    if ((item->spec || item->mask) &&
        (memcmp(vlan_spec, &vlan_null, sizeof(struct rte_flow_item_vlan)) ||
         memcmp(vlan_mask, &vlan_null, sizeof(struct rte_flow_item_vlan)))) {
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_TCP) {
    ...
    if ((tcp_mask->hdr.src_port != 0 && tcp_mask->hdr.src_port != UINT16_MAX) ||
        (tcp_mask->hdr.dst_port != 0 && tcp_mask->hdr.dst_port != UINT16_MAX)) {
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by ntuple filter");
      return -rte_errno;
    }
    ...
    if ((udp_mask->hdr.src_port != 0 && udp_mask->hdr.src_port != UINT16_MAX) ||
        (udp_mask->hdr.dst_port != 0 && udp_mask->hdr.dst_port != UINT16_MAX)) {
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by ntuple filter");
      return -rte_errno;
    }
...
action:
  ...
  if (attr->transfer) {
    memset(filter, 0, sizeof(struct rte_eth_ntuple_filter));
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_TRANSFER, attr,
...
static int ixgbe_parse_ntuple_filter(struct rte_eth_dev *dev,
...
static int ixgbe_parse_ethertype_filter(struct rte_eth_dev *dev,
                                        const struct rte_flow_attr *attr,
                                        const struct rte_flow_item pattern[],
                                        const struct rte_flow_action actions[],
                                        struct rte_eth_ethertype_filter *filter,
  ...
  ret = cons_parse_ethertype_filter(attr, pattern, actions, filter, error);
 ...
 * item->last should be NULL.
 */
static int cons_parse_syn_filter(const struct rte_flow_attr *attr,
                                 const struct rte_flow_item pattern[],
                                 const struct rte_flow_action actions[],
  ...
  tcp_spec = item->spec;
  tcp_mask = item->mask;
  if (!(tcp_spec->hdr.tcp_flags & RTE_TCP_SYN_FLAG) || tcp_mask->hdr.src_port ||
      tcp_mask->hdr.dst_port || tcp_mask->hdr.sent_seq ||
      tcp_mask->hdr.recv_ack || tcp_mask->hdr.data_off ||
      tcp_mask->hdr.tcp_flags != RTE_TCP_SYN_FLAG || tcp_mask->hdr.rx_win ||
...
static int ixgbe_parse_syn_filter(struct rte_eth_dev *dev,
                                  const struct rte_flow_attr *attr,
                                  const struct rte_flow_item pattern[],
                                  const struct rte_flow_action actions[],
                                  struct rte_eth_syn_filter *filter,
  ...
  ret = cons_parse_syn_filter(attr, pattern, actions, filter, error);
 ...
 * item->last should be NULL.
 */
static int cons_parse_l2_tn_filter(struct rte_eth_dev *dev,
                                   const struct rte_flow_attr *attr,
                                   const struct rte_flow_item pattern[],
                                   const struct rte_flow_action actions[],
  ...
  if (e_tag_mask->epcp_edei_in_ecid_b || e_tag_mask->in_ecid_e ||
      e_tag_mask->ecid_e ||
...
static int ixgbe_parse_l2_tn_filter(struct rte_eth_dev *dev,
                                    const struct rte_flow_attr *attr,
                                    const struct rte_flow_item pattern[],
                                    const struct rte_flow_action actions[],
  ...
  ret =
      cons_parse_l2_tn_filter(dev, attr, pattern, actions, l2_tn_filter, error);
...
static inline const struct rte_flow_item *
next_no_fuzzy_pattern(const struct rte_flow_item pattern[],
                      const struct rte_flow_item *cur) {
  const struct rte_flow_item *next = next_no_void_pattern(pattern, cur);
  while (1) {
 ...
 * Item->last should be NULL.
 */
static int ixgbe_parse_fdir_filter_normal(
    struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
    const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], struct ixgbe_fdir_rule *rule,
    struct rte_flow_error *error) {
  const struct rte_flow_item *item;
  const struct rte_flow_item_eth *eth_spec;
  const struct rte_flow_item_eth *eth_mask;
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_ETH) {
    ...
    if (item->spec) {
      ...
      for (j = 0; j < RTE_ETHER_ADDR_LEN; j++) {
        rule->ixgbe_fdir.formatted.inner_mac[j] = eth_spec->dst.addr_bytes[j];
      }
    }
    ...
    if (item->mask) {
      ...
      if (eth_mask->type || rule->mode == RTE_FDIR_MODE_SIGNATURE) {
        memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                           "Not supported by fdir filter");
        return -rte_errno;
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_IPV6) {
     ...
     */
    if (rule->mode != RTE_FDIR_MODE_SIGNATURE || item->last || !item->mask) {
      memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, item,
                         "Not supported last point for range");
      return -rte_errno;
    ...
    if (item->spec) {
      rule->b_spec = TRUE;
      ipv6_spec = item->spec;
      rte_memcpy(rule->ixgbe_fdir.formatted.src_ip, ipv6_spec->hdr.src_addr,
                 16);
      rte_memcpy(rule->ixgbe_fdir.formatted.dst_ip, ipv6_spec->hdr.dst_addr,
                 16);
    }
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_SCTP) {
    ...
    if (hw->mac.type == ixgbe_mac_X550 || hw->mac.type == ixgbe_mac_X550EM_x ||
      ...
      sctp_mask = item->mask;
      if (sctp_mask && (sctp_mask->hdr.src_port || sctp_mask->hdr.dst_port ||
                        sctp_mask->hdr.tag || sctp_mask->hdr.cksum)) {
        memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                           "Not supported by fdir filter");
        return -rte_errno;
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_RAW) {
    ...
    /* check mask */
    if (raw_mask->relative != 0x1 || raw_mask->search != 0x1 ||
        raw_mask->reserved != 0x0 || (uint32_t)raw_mask->offset != 0xffffffff ||
        raw_mask->limit != 0xffff || raw_mask->length != 0xffff) {
      memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by fdir filter");
      return -rte_errno;
    ...
    /* check spec */
    if (raw_spec->relative != 0 || raw_spec->search != 0 ||
        raw_spec->reserved != 0 ||
        raw_spec->offset > IXGBE_MAX_FLX_SOURCE_OFF || raw_spec->offset % 2 ||
        raw_spec->limit != 0 || raw_spec->length != 2 ||
        /* pattern can't be 0xffff */
        (raw_spec->pattern[0] == 0xff && raw_spec->pattern[1] == 0xff)) {
      memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by fdir filter");
      return -rte_errno;
    ...
    /* check pattern mask */
    if (raw_mask->pattern[0] != 0xff || raw_mask->pattern[1] != 0xff) {
      memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by fdir filter");
      return -rte_errno;
 ...
 * item->last should be NULL.
 */
static int ixgbe_parse_fdir_filter_tunnel(
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_VXLAN) {
    ...
    if ((vxlan_mask->vni[0] || vxlan_mask->vni[1] || vxlan_mask->vni[2]) &&
        ((vxlan_mask->vni[0] != 0xFF) || (vxlan_mask->vni[1] != 0xFF) ||
    ...
    if (item->spec) {
      ...
      rte_memcpy(((uint8_t *)&rule->ixgbe_fdir.formatted.tni_vni),
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_NVGRE) {
    ...
    if (nvgre_mask->tni[0] &&
        ((nvgre_mask->tni[0] != 0xFF) || (nvgre_mask->tni[1] != 0xFF) ||
    ...
    if (item->spec) {
      ...
      /* tni is a 24-bits bit field */
      rte_memcpy(&rule->ixgbe_fdir.formatted.tni_vni, nvgre_spec->tni,
...
static int ixgbe_parse_fdir_filter(struct rte_eth_dev *dev,
                                   const struct rte_flow_attr *attr,
                                   const struct rte_flow_item pattern[],
                                   const struct rte_flow_action actions[],
                                   struct ixgbe_fdir_rule *rule,
  ...
  ret =
      ixgbe_parse_fdir_filter_normal(dev, attr, pattern, actions, rule, error);

  if (!ret)
    goto step_next;

  ret = ixgbe_parse_fdir_filter_tunnel(attr, pattern, actions, rule, error);
...
step_next:
  ...
  if (fdir_mode == RTE_FDIR_MODE_NONE || fdir_mode != rule->mode)
    return -ENOTSUP;
...
static int ixgbe_parse_rss_filter(struct rte_eth_dev *dev,
                                  const struct rte_flow_attr *attr,
                                  const struct rte_flow_action actions[],
...
static void ixgbe_clear_rss_filter(struct rte_eth_dev *dev) {
...
void ixgbe_filterlist_flush(void) {
  ...
  while ((ntuple_filter_ptr = TAILQ_FIRST(&filter_ntuple_list))) {
    TAILQ_REMOVE(&filter_ntuple_list, ntuple_filter_ptr, entries);
    rte_free(ntuple_filter_ptr);
  }

  while ((ethertype_filter_ptr = TAILQ_FIRST(&filter_ethertype_list))) {
    TAILQ_REMOVE(&filter_ethertype_list, ethertype_filter_ptr, entries);
    rte_free(ethertype_filter_ptr);
  }

  while ((syn_filter_ptr = TAILQ_FIRST(&filter_syn_list))) {
    TAILQ_REMOVE(&filter_syn_list, syn_filter_ptr, entries);
    rte_free(syn_filter_ptr);
  }

  while ((l2_tn_filter_ptr = TAILQ_FIRST(&filter_l2_tunnel_list))) {
    TAILQ_REMOVE(&filter_l2_tunnel_list, l2_tn_filter_ptr, entries);
    rte_free(l2_tn_filter_ptr);
  }

  while ((fdir_rule_ptr = TAILQ_FIRST(&filter_fdir_list))) {
    TAILQ_REMOVE(&filter_fdir_list, fdir_rule_ptr, entries);
    rte_free(fdir_rule_ptr);
  }

  while ((rss_filter_ptr = TAILQ_FIRST(&filter_rss_list))) {
    TAILQ_REMOVE(&filter_rss_list, rss_filter_ptr, entries);
    rte_free(rss_filter_ptr);
  }

  while ((ixgbe_flow_mem_ptr = TAILQ_FIRST(&ixgbe_flow_list))) {
    TAILQ_REMOVE(&ixgbe_flow_list, ixgbe_flow_mem_ptr, entries);
...
ixgbe_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
                  const struct rte_flow_item pattern[],
                  const struct rte_flow_action actions[],
                  struct rte_flow_error *error) {
  int ret;
  ...
  ixgbe_flow_mem_ptr =
      rte_zmalloc("ixgbe_flow_mem", sizeof(struct ixgbe_flow_mem), 0);
  ...
  TAILQ_INSERT_TAIL(&ixgbe_flow_list, ixgbe_flow_mem_ptr, entries);

  memset(&ntuple_filter, 0, sizeof(struct rte_eth_ntuple_filter));
  ret = ixgbe_parse_ntuple_filter(dev, attr, pattern, actions, &ntuple_filter,
                                  error);
  ...
  if (!ret) {
    ...
    if (!ret) {
      ntuple_filter_ptr = rte_zmalloc(
          "ixgbe_ntuple_filter", sizeof(struct ixgbe_ntuple_filter_ele), 0);
      ...
      TAILQ_INSERT_TAIL(&filter_ntuple_list, ntuple_filter_ptr, entries);
  ...
  ret = ixgbe_parse_ethertype_filter(dev, attr, pattern, actions,
                                     &ethertype_filter, error);
  if (!ret) {
    ret = ixgbe_add_del_ethertype_filter(dev, &ethertype_filter, TRUE);
    if (!ret) {
      ...
      TAILQ_INSERT_TAIL(&filter_ethertype_list, ethertype_filter_ptr, entries);
  ...
  ret = ixgbe_parse_syn_filter(dev, attr, pattern, actions, &syn_filter, error);
  if (!ret) {
    ...
    if (!ret) {
      ...
      rte_memcpy(&syn_filter_ptr->filter_info, &syn_filter,
                 sizeof(struct rte_eth_syn_filter));
      TAILQ_INSERT_TAIL(&filter_syn_list, syn_filter_ptr, entries);
  ...
  ret = ixgbe_parse_fdir_filter(dev, attr, pattern, actions, &fdir_rule, error);
  if (!ret) {
    /* A mask cannot be deleted. */
    if (fdir_rule.b_mask) {
      if (!fdir_info->mask_added) {
        /* It's the first time the mask is set. */
        rte_memcpy(&fdir_info->mask, &fdir_rule.mask,
                   sizeof(struct ixgbe_hw_fdir_mask));

        if (fdir_rule.mask.flex_bytes_mask) {
          ret =
              ixgbe_fdir_set_flexbytes_offset(dev, fdir_rule.flex_bytes_offset);
          if (ret)
            goto out;
        }
    ...
    if (fdir_rule.b_spec) {
      ret = ixgbe_fdir_filter_program(dev, &fdir_rule, FALSE, FALSE);
      if (!ret) {
  ...
  memset(&l2_tn_filter, 0, sizeof(struct ixgbe_l2_tunnel_conf));
  ret = ixgbe_parse_l2_tn_filter(dev, attr, pattern, actions, &l2_tn_filter,
                                 error);
  if (!ret) {
    ret = ixgbe_dev_l2_tunnel_filter_add(dev, &l2_tn_filter, FALSE);
    if (!ret) {
      l2_tn_filter_ptr = rte_zmalloc(
          "ixgbe_l2_tn_filter", sizeof(struct ixgbe_eth_l2_tunnel_conf_ele), 0);
      if (!l2_tn_filter_ptr) {
        PMD_DRV_LOG(ERR, "failed to allocate memory");
        goto out;
      }
      rte_memcpy(&l2_tn_filter_ptr->filter_info, &l2_tn_filter,
                 sizeof(struct ixgbe_l2_tunnel_conf));
      TAILQ_INSERT_TAIL(&filter_l2_tunnel_list, l2_tn_filter_ptr, entries);
  ...
  memset(&rss_conf, 0, sizeof(struct ixgbe_rte_flow_rss_conf));
  ret = ixgbe_parse_rss_filter(dev, attr, actions, &rss_conf, error);
  if (!ret) {
    ret = ixgbe_config_rss_filter(dev, &rss_conf, TRUE);
    if (!ret) {
      rss_filter_ptr =
          rte_zmalloc("ixgbe_rss_filter", sizeof(struct ixgbe_rss_conf_ele), 0);
      if (!rss_filter_ptr) {
        PMD_DRV_LOG(ERR, "failed to allocate memory");
        goto out;
      }
      ixgbe_rss_conf_init(&rss_filter_ptr->filter_info, &rss_conf.conf);
      TAILQ_INSERT_TAIL(&filter_rss_list, rss_filter_ptr, entries);
 ...
 */
static int ixgbe_flow_validate(struct rte_eth_dev *dev,
                               const struct rte_flow_attr *attr,
                               const struct rte_flow_item pattern[],
                               const struct rte_flow_action actions[],
                               struct rte_flow_error *error) {
...
static int ixgbe_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                              struct rte_flow_error *error) {
  int ret;
  ...
  case RTE_ETH_FILTER_NTUPLE:
    ntuple_filter_ptr = (struct ixgbe_ntuple_filter_ele *)pmd_flow->rule;
  ...
  case RTE_ETH_FILTER_ETHERTYPE:
    ethertype_filter_ptr = (struct ixgbe_ethertype_filter_ele *)pmd_flow->rule;
    rte_memcpy(&ethertype_filter, &ethertype_filter_ptr->filter_info,
               sizeof(struct rte_eth_ethertype_filter));
    ret = ixgbe_add_del_ethertype_filter(dev, &ethertype_filter, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&filter_ethertype_list, ethertype_filter_ptr, entries);
      rte_free(ethertype_filter_ptr);
    }
  ...
  case RTE_ETH_FILTER_SYN:
    syn_filter_ptr = (struct ixgbe_eth_syn_filter_ele *)pmd_flow->rule;
    rte_memcpy(&syn_filter, &syn_filter_ptr->filter_info,
               sizeof(struct rte_eth_syn_filter));
  ...
  case RTE_ETH_FILTER_L2_TUNNEL:
    l2_tn_filter_ptr = (struct ixgbe_eth_l2_tunnel_conf_ele *)pmd_flow->rule;
  ...
  case RTE_ETH_FILTER_HASH:
    rss_filter_ptr = (struct ixgbe_rss_conf_ele *)pmd_flow->rule;
    ret = ixgbe_config_rss_filter(dev, &rss_filter_ptr->filter_info, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&filter_rss_list, rss_filter_ptr, entries);
      rte_free(rss_filter_ptr);
    }
