...
static inline void desc_to_olflags_v(uint8x16x2_t sterr_tmp1,
  ...
  const uint8x16_t udp_hdr_p_msk = {0,
                                    0,
                                    0,
                                    0,
                                    udp_p_flag_hi,
  ...
  const uint8x16_t udp_csum_bad_shuf = {
      0xFF, ~(uint8_t)RTE_MBUF_F_RX_L4_CKSUM_BAD,
      0,    0,
      0,    0,
      0,    0,
      0,    0,
...
static inline uint32_t get_packet_type(uint32_t pkt_info, uint32_t etqf_check,
...
static inline void desc_to_ptype_v(uint64x2_t descs[4], uint16_t pkt_type_mask,
  ...
  ptype0 = vandq_u32(vshrq_n_u32(ptype0, IXGBE_PACKET_TYPE_SHIFT), ptype_mask);

  rx_pkts[0]->packet_type =
      get_packet_type(vgetq_lane_u32(ptype0, 0), vgetq_lane_u32(etqf_check, 0),
                      vgetq_lane_u32(tunnel_check, 0));
  rx_pkts[1]->packet_type =
      get_packet_type(vgetq_lane_u32(ptype0, 1), vgetq_lane_u32(etqf_check, 1),
                      vgetq_lane_u32(tunnel_check, 1));
  rx_pkts[2]->packet_type =
      get_packet_type(vgetq_lane_u32(ptype0, 2), vgetq_lane_u32(etqf_check, 2),
                      vgetq_lane_u32(tunnel_check, 2));
  rx_pkts[3]->packet_type =
      get_packet_type(vgetq_lane_u32(ptype0, 3), vgetq_lane_u32(etqf_check, 3),
...
static inline uint16_t _recv_raw_pkts_vec(struct ixgbe_rx_queue *rxq,
  ...
  for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
    ...
    desc_to_olflags_v(sterr_tmp1, sterr_tmp2, staterr, vlan_flags, udp_p_flag,
                      &rx_pkts[pos]);
    ...
    staterr = vreinterpretq_u8_s8(
        vshrq_n_s8(vreinterpretq_s8_u8(staterr), IXGBE_UINT8_BIT - 1));
    stat = ~vgetq_lane_u32(vreinterpretq_u32_u8(staterr), 0);
