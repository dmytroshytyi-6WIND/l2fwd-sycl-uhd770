 ...
 *    RTE_ETH_VMDQ_ACCEPT_BROADCAST and RTE_ETH_VMDQ_ACCEPT_MULTICAST will be
 * used in rx_mode.
 * @param on
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_ixgbe_bypass_event_show(uint16_t port, uint32_t event,
                                    uint32_t *state);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_ixgbe_bypass_event_store(uint16_t port, uint32_t event,
                                     uint32_t state);
 ...
 */
__rte_experimental int rte_pmd_ixgbe_mdio_unlocked_read(uint16_t port,
                                                        uint32_t reg_addr,
                                                        uint32_t dev_type,
                                                        uint16_t *phy_data);
 ...
 *   - (IXGBE_ERR_PHY) If PHY read command failed
 */
__rte_experimental int rte_pmd_ixgbe_mdio_unlocked_write(uint16_t port,
                                                         uint32_t reg_addr,
                                                         uint32_t dev_type,
                                                         uint16_t phy_data);
...
#define RTE_PMD_IXGBE_BYPASS_MODE_VALID(x)                                     \
  ((x) > RTE_PMD_IXGBE_BYPASS_MODE_NONE && (x) < RTE_PMD_IXGBE_BYPASS_MODE_NUM)
...
#define RTE_PMD_IXGBE_BYPASS_EVENT_VALID(x)                                    \
  ((x) > RTE_PMD_IXGBE_BYPASS_EVENT_NONE && (x) < RTE_PMD_IXGBE_BYPASS_MODE_NUM)
...
#define RTE_PMD_IXGBE_BYPASS_TMT_VALID(x)                                      \
  ((x) == RTE_PMD_IXGBE_BYPASS_TMT_OFF ||                                      \
   ((x) > RTE_PMD_IXGBE_BYPASS_TMT_OFF && (x) < RTE_PMD_IXGBE_BYPASS_TMT_NUM))
