...
static int ixgbe_node_capabilities_get(struct rte_eth_dev *dev,
                                       ...
                                       struct rte_tm_error *error);
static int ixgbe_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
                                  struct rte_tm_error *error);
...
int ixgbe_tm_ops_get(struct rte_eth_dev *dev __rte_unused, void *arg) {
  if (!arg)
    return -EINVAL;
...
static inline uint8_t ixgbe_tc_nb_get(struct rte_eth_dev *dev) {
...
static int ixgbe_tm_capabilities_get(struct rte_eth_dev *dev,
   ...
   */
  /* port + TCs + queues */
  cap->n_nodes_max = 1 + IXGBE_DCB_MAX_TRAFFIC_CLASS + hw->mac.max_tx_queues;
  cap->n_levels_max = 3;
...
static int ixgbe_shaper_profile_add(struct rte_eth_dev *dev,
  ...
  shaper_profile = rte_zmalloc("ixgbe_tm_shaper_profile",
                               sizeof(struct ixgbe_tm_shaper_profile), 0);
  if (!shaper_profile)
    return -ENOMEM;
...
static void ixgbe_queue_base_nb_get(struct rte_eth_dev *dev,
...
static int ixgbe_node_param_check(struct rte_eth_dev *dev, uint32_t node_id,
...
static int ixgbe_node_add(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  ret = ixgbe_node_param_check(dev, node_id, priority, weight, params, error);
  if (ret)
    return ret;
  ...
  if (parent_node_id == RTE_TM_NODE_ID_NULL) {
    ...
    /* add the root node */
    tm_node = rte_zmalloc("ixgbe_tm_node", sizeof(struct ixgbe_tm_node), 0);
    if (!tm_node)
      return -ENOMEM;
    tm_node->id = node_id;
  ...
  parent_node = ixgbe_tm_node_search(dev, parent_node_id, &parent_node_type);
  if (!parent_node) {
    error->type = RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID;
