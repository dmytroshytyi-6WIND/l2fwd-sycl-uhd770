...
#define FUNC_PTR_OR_ERR_RET(func, retval)                                      \
  do {                                                                         \
    if ((func) == NULL) {                                                      \
      PMD_DRV_LOG(ERR, "%s:%d function not supported", __func__, __LINE__);    \
      return retval;                                                           \
    }                                                                          \
  } while (0)
 ...
 *  system clock.
 **/
static void ixgbe_bypass_set_time(struct ixgbe_adapter *adapter) {
  u32 mask, value;
  u32 sec;
  ...
  value = (sec & BYPASS_CTL1_TIME_M) | BYPASS_CTL1_VALID | BYPASS_CTL1_OFFTRST;
...
void ixgbe_bypass_init(struct rte_eth_dev *dev) {
  ...
  if (hw->device_id != IXGBE_DEV_ID_82599_BYPASS || hw->bus.func != 0) {
...
s32 ixgbe_bypass_state_store(struct rte_eth_dev *dev, u32 *new_state) {
  ...
  /* Set the new state */
  ret_val = adapter->bps.ops.bypass_set(hw, BYPASS_PAGE_CTL0, BYPASS_MODE_OFF_M,
                                        *new_state);
  if (ret_val)
    goto exit;
...
s32 ixgbe_bypass_event_store(struct rte_eth_dev *dev, u32 event, u32 state) {
  ...
  ret_val = adapter->bps.ops.bypass_set(hw, BYPASS_PAGE_CTL0, off, status);
...
s32 ixgbe_bypass_wd_timeout_store(struct rte_eth_dev *dev, u32 timeout) {
  ...
  ret_val = adapter->bps.ops.bypass_set(hw, BYPASS_PAGE_CTL0, mask, status);
...
s32 ixgbe_bypass_wd_timeout_show(struct rte_eth_dev *dev, u32 *wd_timeout) {
  ...
  if (!wdg)
    ...
    *wd_timeout = (by_ctl >> BYPASS_WDT_TIME_SHIFT) & BYPASS_WDT_MASK;
