...
#define IXGBE_TX_OFFLOAD_MASK                                                  \
  (RTE_MBUF_F_TX_OUTER_IPV6 | RTE_MBUF_F_TX_OUTER_IPV4 | RTE_MBUF_F_TX_IPV6 |  \
   RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_VLAN | RTE_MBUF_F_TX_IP_CKSUM |          \
   RTE_MBUF_F_TX_L4_MASK | RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_MACSEC |      \
   RTE_MBUF_F_TX_OUTER_IP_CKSUM | RTE_MBUF_F_TX_SEC_OFFLOAD |                  \
 ...
 */
static __rte_always_inline int ixgbe_tx_free_bufs(struct ixgbe_tx_queue *txq) {
...
static inline void tx4(volatile union ixgbe_adv_tx_desc *txdp,
...
static inline void tx1(volatile union ixgbe_adv_tx_desc *txdp,
                       struct rte_mbuf **pkts) {
  uint64_t buf_dma_addr;
  uint32_t pkt_len;
 ...
 */
static inline void ixgbe_tx_fill_hw_ring(struct ixgbe_tx_queue *txq,
                                         struct rte_mbuf **pkts,
                                         uint16_t nb_pkts) {
...
static uint16_t ixgbe_xmit_pkts_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
                                    uint16_t nb_pkts) {
  uint16_t nb_tx = 0;
  struct ixgbe_tx_queue *txq = (struct ixgbe_tx_queue *)tx_queue;
  ...
  while (nb_pkts) {
    ...
    ret = ixgbe_xmit_fixed_burst_vec(tx_queue, &tx_pkts[nb_tx], num);
    nb_tx += ret;
    nb_pkts -= ret;
    if (ret < num)
      break;
...
ixgbe_set_xmit_ctx(struct ixgbe_tx_queue *txq,
  ...
  if (ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    if (ol_flags & RTE_MBUF_F_TX_IP_CKSUM)
      type_tucmd_mlhl = IXGBE_ADVTXD_TUCMD_IPV4 | IXGBE_ADVTXD_TUCMD_L4T_TCP |
                        IXGBE_ADVTXD_DTYP_CTXT | IXGBE_ADVTXD_DCMD_DEXT;
    else
      type_tucmd_mlhl = IXGBE_ADVTXD_TUCMD_IPV6 | IXGBE_ADVTXD_TUCMD_L4T_TCP |
                        IXGBE_ADVTXD_DTYP_CTXT | IXGBE_ADVTXD_DCMD_DEXT;
    ...
    case RTE_MBUF_F_TX_UDP_CKSUM:
      type_tucmd_mlhl |= IXGBE_ADVTXD_TUCMD_L4T_UDP | IXGBE_ADVTXD_DTYP_CTXT |
    ...
    case RTE_MBUF_F_TX_TCP_CKSUM:
      type_tucmd_mlhl |= IXGBE_ADVTXD_TUCMD_L4T_TCP | IXGBE_ADVTXD_DTYP_CTXT |
                         IXGBE_ADVTXD_DCMD_DEXT;
    ...
    case RTE_MBUF_F_TX_SCTP_CKSUM:
      type_tucmd_mlhl |= IXGBE_ADVTXD_TUCMD_L4T_SCTP | IXGBE_ADVTXD_DTYP_CTXT |
                         IXGBE_ADVTXD_DCMD_DEXT;
      ...
      break;
    default:
      type_tucmd_mlhl |= IXGBE_ADVTXD_TUCMD_L4T_RSV | IXGBE_ADVTXD_DTYP_CTXT |
                         IXGBE_ADVTXD_DCMD_DEXT;
      break;
    }
  }
  ...
  if (ol_flags & RTE_MBUF_F_TX_OUTER_IP_CKSUM) {
    ...
    tx_offload_mask.l2_len |= ~0;
    seqnum_seed |= tx_offload.outer_l3_len << IXGBE_ADVTXD_OUTER_IPLEN;
    seqnum_seed |= tx_offload.l2_len << IXGBE_ADVTXD_TUNNEL_LEN;
  }
#ifdef RTE_LIB_SECURITY
  if (ol_flags & RTE_MBUF_F_TX_SEC_OFFLOAD) {
    union ixgbe_crypto_tx_desc_md *md = (union ixgbe_crypto_tx_desc_md *)mdata;
    seqnum_seed |= (IXGBE_ADVTXD_IPSEC_SA_INDEX_MASK & md->sa_idx);
    type_tucmd_mlhl |= md->enc ? (IXGBE_ADVTXD_TUCMD_IPSEC_TYPE_ESP |
                                  IXGBE_ADVTXD_TUCMD_IPSEC_ENCRYPT_EN)
                               : 0;
    type_tucmd_mlhl |= (md->pad_len & IXGBE_ADVTXD_IPSEC_ESP_LEN_MASK);
  ...
  if (ol_flags & RTE_MBUF_F_TX_OUTER_IP_CKSUM)
    vlan_macip_lens |= (tx_offload.outer_l2_len << IXGBE_ADVTXD_MACLEN_SHIFT);
  else
 ...
 */
static inline uint32_t what_advctx_update(struct ixgbe_tx_queue *txq,
                                          uint64_t flags,
...
static inline int ixgbe_xmit_cleanup(struct ixgbe_tx_queue *txq) {
...
uint16_t ixgbe_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (tx_ol_req) {
      ...
      if (use_ipsec) {
        union ixgbe_crypto_tx_desc_md *ipsec_mdata =
            (union ixgbe_crypto_tx_desc_md *)rte_security_dynfield(tx_pkt);
    ...
    PMD_TX_LOG(DEBUG,
               ...
               (unsigned)pkt_len, (unsigned)tx_id, (unsigned)tx_last);
    ...
    if (nb_used > txq->nb_tx_free) {
      ...
      if (unlikely(nb_used > txq->tx_rs_thresh)) {
        PMD_TX_LOG(DEBUG,
                   ...
                   "tx_rs_thresh=%4u. "
                   "(port=%d queue=%d)",
                   nb_used, txq->nb_tx_free, txq->tx_rs_thresh, txq->port_id,
                   txq->queue_id);
        /*
     ...
     *   - IXGBE_TXD_CMD_RS
     */
    cmd_type_len = IXGBE_ADVTXD_DTYP_DATA | IXGBE_ADVTXD_DCMD_IFCS |
                   IXGBE_ADVTXD_DCMD_DEXT;
    ...
    if (tx_ol_req) {
      ...
      if (new_ctx) {
        ...
        ctx_txd = (volatile struct ixgbe_adv_tx_context_desc *)&txr[tx_id];
        ...
        ixgbe_set_xmit_ctx(txq, ctx_txd, tx_ol_req, tx_offload,
 ...
 * to save the space.
 */
const uint32_t ptype_table[IXGBE_PACKET_TYPE_MAX] __rte_cache_aligned = {
    [IXGBE_PACKET_TYPE_ETHER] = RTE_PTYPE_L2_ETHER,
    [IXGBE_PACKET_TYPE_IPV4] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4,
    [IXGBE_PACKET_TYPE_IPV4_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP,
    [IXGBE_PACKET_TYPE_IPV4_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP,
    [IXGBE_PACKET_TYPE_IPV4_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_SCTP,
    [IXGBE_PACKET_TYPE_IPV4_EXT] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT,
    [IXGBE_PACKET_TYPE_IPV4_EXT_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_TCP,
    [IXGBE_PACKET_TYPE_IPV4_EXT_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_UDP,
    [IXGBE_PACKET_TYPE_IPV4_EXT_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_SCTP,
    [IXGBE_PACKET_TYPE_IPV6] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6,
    [IXGBE_PACKET_TYPE_IPV6_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP,
    [IXGBE_PACKET_TYPE_IPV6_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_UDP,
    [IXGBE_PACKET_TYPE_IPV6_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_SCTP,
    [IXGBE_PACKET_TYPE_IPV6_EXT] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT,
    [IXGBE_PACKET_TYPE_IPV6_EXT_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_L4_TCP,
    [IXGBE_PACKET_TYPE_IPV6_EXT_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_L4_UDP,
    [IXGBE_PACKET_TYPE_IPV6_EXT_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_L4_SCTP,
    [IXGBE_PACKET_TYPE_IPV4_IPV6] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 |
                                    RTE_PTYPE_TUNNEL_IP |
                                    RTE_PTYPE_INNER_L3_IPV6,
    [IXGBE_PACKET_TYPE_IPV4_IPV6_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_IPV4_IPV6_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_UDP,
    [IXGBE_PACKET_TYPE_IPV4_IPV6_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_IPV4_EXT_IPV6] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6,
    [IXGBE_PACKET_TYPE_IPV4_EXT_IPV6_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_IPV4_EXT_IPV6_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_UDP,
    [IXGBE_PACKET_TYPE_IPV4_EXT_IPV6_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_IPV4_IPV6_EXT] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 |
                                        RTE_PTYPE_TUNNEL_IP |
                                        RTE_PTYPE_INNER_L3_IPV6_EXT,
    [IXGBE_PACKET_TYPE_IPV4_IPV6_EXT_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_IPV4_IPV6_EXT_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_UDP,
    [IXGBE_PACKET_TYPE_IPV4_IPV6_EXT_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_IPV4_EXT_IPV6_EXT] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6_EXT,
    [IXGBE_PACKET_TYPE_IPV4_EXT_IPV6_EXT_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_IPV4_EXT_IPV6_EXT_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_IP |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_UDP,
...
const uint32_t ptype_table_tn[IXGBE_PACKET_TYPE_TN_MAX] __rte_cache_aligned = {
    [IXGBE_PACKET_TYPE_NVGRE] = RTE_PTYPE_L2_ETHER |
                                RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
                                RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_EXT] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4_EXT,
    [IXGBE_PACKET_TYPE_NVGRE_IPV6] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_IPV6] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_NVGRE_IPV6_EXT] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6_EXT,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_IPV6_EXT] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4 | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_NVGRE_IPV6_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_IPV6_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_NVGRE_IPV6_EXT_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_IPV6_EXT_TCP] =
        ...
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4 | RTE_PTYPE_INNER_L4_UDP,
    [IXGBE_PACKET_TYPE_NVGRE_IPV6_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_UDP,
    [IXGBE_PACKET_TYPE_NVGRE_IPV6_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_IPV6_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_NVGRE_IPV6_EXT_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_UDP,
    ...
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_IPV6_EXT_UDP] =
        ...
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4 | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_EXT_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4_EXT | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_EXT_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4_EXT | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_NVGRE_IPV4_EXT_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
        RTE_PTYPE_TUNNEL_GRE | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4_EXT | RTE_PTYPE_INNER_L4_UDP,

    [IXGBE_PACKET_TYPE_VXLAN] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_EXT] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4_EXT,
    [IXGBE_PACKET_TYPE_VXLAN_IPV6] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_IPV6] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_VXLAN_IPV6_EXT] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6_EXT,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_IPV6_EXT] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4 | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV6_TCP] =
        ...
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_IPV6_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_VXLAN_IPV6_EXT_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_IPV6_EXT_TCP] =
        ...
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4 | RTE_PTYPE_INNER_L4_UDP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV6_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_UDP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV6_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_IPV6_UDP] =
        ...
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_VXLAN_IPV6_EXT_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_UDP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV6_EXT_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_IPV6_EXT_UDP] =
        ...
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4 | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_EXT_SCTP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4_EXT | RTE_PTYPE_INNER_L4_SCTP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_EXT_TCP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4_EXT | RTE_PTYPE_INNER_L4_TCP,
    [IXGBE_PACKET_TYPE_VXLAN_IPV4_EXT_UDP] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_UDP |
        RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_INNER_L2_ETHER |
        RTE_PTYPE_INNER_L3_IPV4_EXT | RTE_PTYPE_INNER_L4_UDP,
};

static int ixgbe_monitor_callback(
...
static inline uint32_t ixgbe_rxd_pkt_info_to_pkt_type(uint32_t pkt_info,
                                                      uint16_t ptype_mask) {
...
static inline uint64_t ixgbe_rxd_pkt_info_to_pkt_flags(uint16_t pkt_info) {
...
rx_desc_error_to_pkt_flags(uint32_t rx_status, uint16_t pkt_info,
  ...
  pkt_flags = error_to_pkt_flags_map[(rx_status >> IXGBE_RXDADV_ERR_CKSUM_BIT) &
  ...
  if ((rx_status & IXGBE_RXDADV_ERR_TCPE) &&
      (pkt_info & IXGBE_RXDADV_PKTTYPE_UDP) && rx_udp_csum_zero_err)
...
#endif
static inline int ixgbe_rx_scan_hw_ring(struct ixgbe_rx_queue *rxq) {
  volatile union ixgbe_adv_rx_desc *rxdp;
  ...
  for (i = 0; i < RTE_PMD_IXGBE_RX_MAX_BURST;
    ...
    for (nb_dd = 0; nb_dd < LOOK_AHEAD && (s[nb_dd] & IXGBE_RXDADV_STAT_DD);
         nb_dd++)
      ;
    ...
    for (j = 0; j < nb_dd; ++j) {
      ...
      pkt_flags = rx_desc_status_to_pkt_flags(s[j], vlan_flags);
      pkt_flags |= rx_desc_error_to_pkt_flags(s[j], (uint16_t)pkt_info[j],
                                              rxq->rx_udp_csum_zero_err);
      pkt_flags |= ixgbe_rxd_pkt_info_to_pkt_flags((uint16_t)pkt_info[j]);
      mb->ol_flags = pkt_flags;
      mb->packet_type =
          ixgbe_rxd_pkt_info_to_pkt_type(pkt_info[j], rxq->pkt_type_mask);

      if (likely(pkt_flags & RTE_MBUF_F_RX_RSS_HASH))
        mb->hash.rss = rte_le_to_cpu_32(rxdp[j].wb.lower.hi_dword.rss);
      else if (pkt_flags & RTE_MBUF_F_RX_FDIR) {
        mb->hash.fdir.hash =
            rte_le_to_cpu_16(rxdp[j].wb.lower.hi_dword.csum_ip.csum) &
            IXGBE_ATR_HASH_MASK;
        mb->hash.fdir.id =
            rte_le_to_cpu_16(rxdp[j].wb.lower.hi_dword.csum_ip.ip_id);
      }
    }
...
static inline int ixgbe_rx_alloc_bufs(struct ixgbe_rx_queue *rxq,
                                      bool reset_mbuf) {
  volatile union ixgbe_adv_rx_desc *rxdp;
  struct ixgbe_rx_entry *rxep;
  struct rte_mbuf *mb;
...
static inline uint16_t ixgbe_rx_fill_from_stage(struct ixgbe_rx_queue *rxq,
...
static inline uint16_t rx_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  if (rxq->rx_tail > rxq->rx_free_trigger) {
    ...
    /* update tail pointer */
    rte_wmb();
    IXGBE_PCI_REG_WC_WRITE_RELAXED(rxq->rdt_reg_addr, cur_free_trigger);
  }
...
uint16_t ixgbe_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_rx < nb_pkts) {
    ...
    PMD_RX_LOG(DEBUG,
               ...
               (unsigned)rxq->port_id, (unsigned)rxq->queue_id, (unsigned)rx_id,
               (unsigned)staterr,
    ...
    if (likely(pkt_flags & RTE_MBUF_F_RX_RSS_HASH))
      ...
      rxm->hash.fdir.hash =
          ...
          IXGBE_ATR_HASH_MASK;
      rxm->hash.fdir.id = rte_le_to_cpu_16(rxd.wb.lower.hi_dword.csum_ip.ip_id);
    }
    /*
 ...
 */
static inline void ixgbe_fill_cluster_head_buf(struct rte_mbuf *head,
 ...
 * receive" interface).
 */
static inline uint16_t ixgbe_recv_pkts_lro(void *rx_queue,
                                           struct rte_mbuf **rx_pkts,
  ...
  next_desc:
    ...
    if (!bulk_alloc) {
      ...
      if (!ixgbe_rx_alloc_bufs(rxq, false)) {
        rte_wmb();
        IXGBE_PCI_REG_WC_WRITE_RELAXED(rxq->rdt_reg_addr, next_rdt);
    ...
    if (!eop) {
      ...
      if (ixgbe_rsc_count(&rxd))
        nextp_id =
            (staterr & IXGBE_RXDADV_NEXTP_MASK) >> IXGBE_RXDADV_NEXTP_SHIFT;
      else
        nextp_id = next_id;
...
static int ixgbe_tx_done_cleanup_full(struct ixgbe_tx_queue *txq,
                                      uint32_t free_cnt) {
...
static int ixgbe_tx_done_cleanup_simple(struct ixgbe_tx_queue *txq,
                                        uint32_t free_cnt) {
  int i, n, cnt;
...
static int ixgbe_tx_done_cleanup_vec(struct ixgbe_tx_queue *txq __rte_unused,
...
static void __rte_cold ixgbe_tx_free_swring(struct ixgbe_tx_queue *txq) {
  if (txq != NULL && txq->sw_ring != NULL)
    rte_free(txq->sw_ring);
}

static void __rte_cold ixgbe_tx_queue_release(struct ixgbe_tx_queue *txq) {
...
void __rte_cold ixgbe_dev_tx_queue_release(struct rte_eth_dev *dev,
                                           uint16_t qid) {
  ixgbe_tx_queue_release(dev->data->tx_queues[qid]);
}

/* (Re)set dynamic ixgbe_tx_queue fields to defaults */
static void __rte_cold ixgbe_reset_tx_queue(struct ixgbe_tx_queue *txq) {
 ...
 */
void __rte_cold ixgbe_set_tx_function(struct rte_eth_dev *dev,
                                      struct ixgbe_tx_queue *txq) {
...
int __rte_cold ixgbe_dev_tx_queue_setup(struct rte_eth_dev *dev,
   ...
   * of IXGBE_ALIGN.
   */
  if (nb_desc % IXGBE_TXD_ALIGN != 0 || (nb_desc > IXGBE_MAX_RING_DESC) ||
   ...
   */
  if (hw->mac.type == ixgbe_mac_82599_vf || hw->mac.type == ixgbe_mac_X540_vf ||
 ...
 */
static void __rte_cold ixgbe_free_sc_cluster(struct rte_mbuf *m) {
...
static void __rte_cold ixgbe_rx_queue_release(struct ixgbe_rx_queue *rxq) {
  if (rxq != NULL) {
...
void __rte_cold ixgbe_dev_rx_queue_release(struct rte_eth_dev *dev,
                                           uint16_t qid) {
...
static void __rte_cold ixgbe_reset_rx_queue(struct ixgbe_adapter *adapter,
                                            struct ixgbe_rx_queue *rxq) {
...
uint64_t ixgbe_get_rx_port_offloads(struct rte_eth_dev *dev) {
  ...
  offloads = RTE_ETH_RX_OFFLOAD_IPV4_CKSUM | RTE_ETH_RX_OFFLOAD_UDP_CKSUM |
             ...
             RTE_ETH_RX_OFFLOAD_VLAN_FILTER | RTE_ETH_RX_OFFLOAD_SCATTER |
...
int __rte_cold ixgbe_dev_rx_queue_setup(struct rte_eth_dev *dev,
                                        uint16_t queue_idx, uint16_t nb_desc,
                                        unsigned int socket_id,
   ...
   * of IXGBE_ALIGN.
   */
  if (nb_desc % IXGBE_RXD_ALIGN != 0 || (nb_desc > IXGBE_MAX_RING_DESC) ||
      (nb_desc < IXGBE_MIN_RING_DESC)) {
    return -EINVAL;
  }
   ...
   */
  if (hw->mac.type == ixgbe_mac_X550 || hw->mac.type == ixgbe_mac_X550EM_x ||
      hw->mac.type == ixgbe_mac_X550EM_a || hw->mac.type == ixgbe_mac_X550_vf ||
      hw->mac.type == ixgbe_mac_X550EM_x_vf ||
      hw->mac.type == ixgbe_mac_X550EM_a_vf)
  ...
  if (hw->mac.type == ixgbe_mac_82599_vf || hw->mac.type == ixgbe_mac_X540_vf ||
    ...
    rxq->rdh_reg_addr = IXGBE_PCI_REG_ADDR(hw, IXGBE_VFRDH(queue_idx));
  } else {
  ...
  rxq->sw_ring =
      rte_zmalloc_socket("rxq->sw_ring", sizeof(struct ixgbe_rx_entry) * len,
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq->sw_ring) {
    ixgbe_rx_queue_release(rxq);
    return -ENOMEM;
  ...
  rxq->sw_sc_ring = rte_zmalloc_socket(
      "rxq->sw_sc_ring", sizeof(struct ixgbe_scattered_rx_entry) * len,
      RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq->sw_sc_ring) {
    ixgbe_rx_queue_release(rxq);
    return -ENOMEM;
...
#define IXGBE_RXQ_SCAN_INTERVAL 4
  ...
  while ((desc < rxq->nb_rx_desc) && (rxdp->wb.upper.status_error &
...
void __rte_cold ixgbe_dev_clear_queues(struct rte_eth_dev *dev) {
  unsigned i;
  struct ixgbe_adapter *adapter = dev->data->dev_private;
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static void ixgbe_rss_disable(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw;
  uint32_t mrqc;
  uint32_t mrqc_reg;
...
static void ixgbe_hw_rss_hash_set(struct ixgbe_hw *hw,
...
static void ixgbe_rss_configure(struct rte_eth_dev *dev) {
  struct rte_eth_rss_conf rss_conf;
  ...
  if (adapter->rss_reta_updated == 0) {
    ...
    for (i = 0, j = 0; i < sp_reta_size; i++, j++) {
      ...
      reta = (reta << 8) | j;
      if ((i & 3) == 3)
        IXGBE_WRITE_REG(hw, reta_reg, rte_bswap32(reta));
    }
  }
...
static void ixgbe_vmdq_dcb_configure(struct rte_eth_dev *dev) {
  ...
  mrqc = (num_pools == RTE_ETH_16_POOLS) ? IXGBE_MRQC_VMDQRT8TCEN
  ...
  for (i = 0; i < cfg->nb_pool_maps; i++) {
    ...
    IXGBE_WRITE_REG(hw, IXGBE_VLVF(i),
                    (IXGBE_VLVF_VIEN | (cfg->pool_map[i].vlan_id & 0xFFF)));
    /*
 ...
 */
static void ixgbe_dcb_tx_hw_config(struct rte_eth_dev *dev,
 ...
 * @dcb_config: pointer to ixgbe_dcb_config structure
 */
static void ixgbe_vmdq_dcb_hw_tx_config(struct rte_eth_dev *dev,
                                        struct ixgbe_dcb_config *dcb_config) {
...
static void ixgbe_vmdq_dcb_rx_config(struct rte_eth_dev *dev,
                                     struct ixgbe_dcb_config *dcb_config) {
  ...
  for (i = 0; i < RTE_ETH_DCB_NUM_USER_PRIORITIES; i++) {
    ...
    tc = &dcb_config->tc_config[j];
    tc->path[IXGBE_DCB_RX_CONFIG].up_to_tc_bitmap |= (uint8_t)(1 << i);
  }
}

static void ixgbe_dcb_vt_tx_config(struct rte_eth_dev *dev,
                                   struct ixgbe_dcb_config *dcb_config) {
  struct rte_eth_vmdq_dcb_tx_conf *vmdq_tx_conf =
      &dev->data->dev_conf.tx_adv_conf.vmdq_dcb_tx_conf;
  struct ixgbe_dcb_tc_config *tc;
  ...
  for (i = 0; i < RTE_ETH_DCB_NUM_USER_PRIORITIES; i++) {
    ...
    tc = &dcb_config->tc_config[j];
    tc->path[IXGBE_DCB_TX_CONFIG].up_to_tc_bitmap |= (uint8_t)(1 << i);
  }
}

static void ixgbe_dcb_rx_config(struct rte_eth_dev *dev,
                                struct ixgbe_dcb_config *dcb_config) {
...
static void ixgbe_dcb_tx_config(struct rte_eth_dev *dev,
                                struct ixgbe_dcb_config *dcb_config) {
 ...
 * @dcb_config: pointer to ixgbe_dcb_config structure
 */
static void ixgbe_dcb_rx_hw_config(struct rte_eth_dev *dev,
                                   struct ixgbe_dcb_config *dcb_config) {
  uint32_t reg;
  uint32_t vlanctrl;
  uint8_t i;
  ...
  if (hw->mac.type != ixgbe_mac_82598EB) {
    ...
    if (dcb_config->num_tcs.pg_tcs == 4) {
      if (dcb_config->vt_mode)
        ...
        reg = (reg & ~IXGBE_MRQC_MRQE_MASK) | IXGBE_MRQC_RTRSS4TCEN;
      }
    }
    if (dcb_config->num_tcs.pg_tcs == 8) {
      if (dcb_config->vt_mode)
        reg = (reg & ~IXGBE_MRQC_MRQE_MASK) | IXGBE_MRQC_VMDQRT8TCEN;
      else {
        IXGBE_WRITE_REG(hw, IXGBE_VT_CTL, 0);
...
static void ixgbe_dcb_hw_arbite_rx_config(struct ixgbe_hw *hw, uint16_t *refill,
...
static void ixgbe_dcb_hw_arbite_tx_config(struct ixgbe_hw *hw, uint16_t *refill,
                                          uint16_t *max, uint8_t *bwg_id,
                                          uint8_t *tsa, uint8_t *map) {
  switch (hw->mac.type) {
  case ixgbe_mac_82598EB:
 ...
 * @dcb_config: pointer to ixgbe_dcb_config structure
 */
static int ixgbe_dcb_hw_configure(struct rte_eth_dev *dev,
                                  struct ixgbe_dcb_config *dcb_config) {
  int ret = 0;
  ...
  if (nb_tcs == RTE_ETH_4_TCS) {
    ...
    for (i = 0; i < nb_tcs; i++) {
      ...
      if (bw_conf->tc_num != nb_tcs)
        tc->path[IXGBE_DCB_TX_CONFIG].bwg_percent = (uint8_t)(100 / nb_tcs);
      tc->path[IXGBE_DCB_RX_CONFIG].bwg_percent = (uint8_t)(100 / nb_tcs);
    }
 ...
 */
static void ixgbe_vmdq_rx_hw_configure(struct rte_eth_dev *dev) {
 ...
 */
static void ixgbe_vmdq_tx_hw_configure(struct ixgbe_hw *hw) {
  uint32_t reg;
  uint32_t q;
...
static int __rte_cold ixgbe_alloc_rx_queue_mbufs(struct ixgbe_rx_queue *rxq) {
...
static int ixgbe_config_vf_rss(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw;
  uint32_t mrqc;
...
static int ixgbe_config_vf_default(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int ixgbe_dev_mq_rx_configure(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int ixgbe_dev_mq_tx_configure(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint32_t mtqc;
  uint32_t rttdcs;
  ...
  if (RTE_ETH_DEV_SRIOV(dev).active == 0) {
    ...
    case RTE_ETH_32_POOLS:
      ...
      break;
    case RTE_ETH_16_POOLS:
      mtqc = IXGBE_MTQC_VT_ENA | IXGBE_MTQC_RT_ENA | IXGBE_MTQC_8TC_8TQ;
      break;
    default:
      mtqc = IXGBE_MTQC_64Q_1PB;
      PMD_INIT_LOG(ERR, "invalid pool number in IOV mode");
 ...
 */
static inline uint32_t ixgbe_get_rscctl_maxdesc(struct rte_mempool *pool) {
 ...
 * @type RX/TX/MISC
 */
static void ixgbe_set_ivar(struct rte_eth_dev *dev, u8 entry, u8 vector,
                           s8 type) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  u32 ivar, index;
...
void __rte_cold ixgbe_set_rx_function(struct rte_eth_dev *dev) {
  ...
  if (dev->data->lro) {
    ...
    PMD_INIT_LOG(DEBUG,
                 ...
                 RTE_IXGBE_DESCS_PER_LOOP, dev->data->port_id);
  ...
  rx_using_sse = (dev->rx_pkt_burst == ixgbe_recv_scattered_pkts_vec ||
                  dev->rx_pkt_burst == ixgbe_recv_pkts_vec);
 ...
 */
static int ixgbe_set_rsc(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    struct ixgbe_rx_queue *rxq = dev->data->rx_queues[i];
    uint32_t srrctl = IXGBE_READ_REG(hw, IXGBE_SRRCTL(rxq->reg_idx));
    uint32_t rscctl = IXGBE_READ_REG(hw, IXGBE_RSCCTL(rxq->reg_idx));
    uint32_t psrtype = IXGBE_READ_REG(hw, IXGBE_PSRTYPE(rxq->reg_idx));
    uint32_t eitr = IXGBE_READ_REG(hw, IXGBE_EITR(rxq->reg_idx));
    ...
    srrctl |=
        (128 << IXGBE_SRRCTL_BSIZEHDRSIZE_SHIFT) & IXGBE_SRRCTL_BSIZEHDR_MASK;
 ...
 */
int __rte_cold ixgbe_dev_rx_init(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw;
  struct ixgbe_rx_queue *rxq;
  uint64_t bus_addr;
  uint32_t rxctrl;
 ...
 */
void __rte_cold ixgbe_dev_tx_init(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw;
  struct ixgbe_tx_queue *txq;
  uint64_t bus_addr;
  uint32_t hlreg0;
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    case ixgbe_mac_X550EM_a:
    default:
      txctrl = IXGBE_READ_REG(hw, IXGBE_DCA_TXCTRL_82599(txq->reg_idx));
      txctrl &= ~IXGBE_DCA_TXCTRL_DESC_WRO_EN;
      IXGBE_WRITE_REG(hw, IXGBE_DCA_TXCTRL_82599(txq->reg_idx), txctrl);
      break;
    }
  }
...
int ixgbe_check_supported_loopback_mode(struct rte_eth_dev *dev) {
  ...
  if (dev->data->dev_conf.lpbk_mode == IXGBE_LPBK_TX_RX)
    if (hw->mac.type == ixgbe_mac_82599EB || hw->mac.type == ixgbe_mac_X540 ||
        hw->mac.type == ixgbe_mac_X550 || hw->mac.type == ixgbe_mac_X550EM_x ||
        hw->mac.type == ixgbe_mac_X550EM_a)
      return 0;
 ...
 */
int __rte_cold ixgbe_dev_rxtx_start(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw;
  struct ixgbe_tx_queue *txq;
  struct ixgbe_rx_queue *rxq;
  uint32_t txdctl;
  ...
  if (dev->data->dev_conf.lpbk_mode != 0) {
    ...
    else if (hw->mac.type == ixgbe_mac_X540 || hw->mac.type == ixgbe_mac_X550 ||
             hw->mac.type == ixgbe_mac_X550EM_x ||
             hw->mac.type == ixgbe_mac_X550EM_a)
  ...
  if ((dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_SECURITY) ||
    ...
    if (ret != 0) {
      PMD_DRV_LOG(ERR, "ixgbe_crypto_enable_ipsec fails with %d.", ret);
      return ret;
    }
  }
#endif
 ...
 */
int __rte_cold ixgbe_dev_rx_queue_start(struct rte_eth_dev *dev,
 ...
 */
int __rte_cold ixgbe_dev_rx_queue_stop(struct rte_eth_dev *dev,
                                       uint16_t rx_queue_id) {
  struct ixgbe_hw *hw;
  struct ixgbe_adapter *adapter = dev->data->dev_private;
  struct ixgbe_rx_queue *rxq;
 ...
 */
int __rte_cold ixgbe_dev_tx_queue_start(struct rte_eth_dev *dev,
 ...
 */
int __rte_cold ixgbe_dev_tx_queue_stop(struct rte_eth_dev *dev,
                                       uint16_t tx_queue_id) {
  struct ixgbe_hw *hw;
  struct ixgbe_tx_queue *txq;
  uint32_t txdctl;
 ...
 */
int __rte_cold ixgbevf_dev_rx_init(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw;
  struct ixgbe_rx_queue *rxq;
  ...
  if (ixgbevf_rlpml_set_vf(hw, frame_size) != 0)
    PMD_INIT_LOG(ERR, "Set max packet length to %d failed.", frame_size);
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    IXGBE_WRITE_REG(hw, IXGBE_VFRDBAL(i),
                    (uint32_t)(bus_addr & 0x00000000ffffffffULL));
    IXGBE_WRITE_REG(hw, IXGBE_VFRDBAH(i), (uint32_t)(bus_addr >> 32));
  ...
  psrtype |= (dev->data->nb_rx_queues >> 1) << IXGBE_PSRTYPE_RQPL_SHIFT;
 ...
 */
void __rte_cold ixgbevf_dev_tx_init(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw;
  struct ixgbe_tx_queue *txq;
  uint64_t bus_addr;
  uint32_t txctrl;
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    IXGBE_WRITE_REG(hw, IXGBE_VFTDBAL(i),
                    (uint32_t)(bus_addr & 0x00000000ffffffffULL));
    IXGBE_WRITE_REG(hw, IXGBE_VFTDBAH(i), (uint32_t)(bus_addr >> 32));
 ...
 */
void __rte_cold ixgbevf_dev_rxtx_start(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw;
  struct ixgbe_tx_queue *txq;
  struct ixgbe_rx_queue *rxq;
  uint32_t txdctl;
...
int ixgbe_config_rss_filter(struct rte_eth_dev *dev,
  ...
  if (!add) {
    if (ixgbe_action_rss_same(&filter_info->rss_info.conf, &conf->conf)) {
      ixgbe_rss_disable(dev);
      memset(&filter_info->rss_info, 0, sizeof(struct ixgbe_rte_flow_rss_conf));
      return 0;
    }
    return -EINVAL;
  }
...
int ixgbe_rx_vec_dev_conf_condition_check(
    struct rte_eth_dev __rte_unused *dev) {
  return -1;
}

uint16_t ixgbe_recv_pkts_vec(void __rte_unused *rx_queue,
...
int ixgbe_rxq_vec_setup(struct ixgbe_rx_queue __rte_unused *rxq) { return -1; }

uint16_t ixgbe_xmit_fixed_burst_vec(void __rte_unused *tx_queue,
                                    struct rte_mbuf __rte_unused **tx_pkts,
                                    uint16_t __rte_unused nb_pkts) {
  return 0;
}

int ixgbe_txq_vec_setup(struct ixgbe_tx_queue __rte_unused *txq) { return -1; }

void ixgbe_rx_queue_release_mbufs_vec(struct ixgbe_rx_queue __rte_unused *rxq) {
  return;
}
#endif
