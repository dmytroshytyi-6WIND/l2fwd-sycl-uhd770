...
int rte_pmd_ixgbe_set_vf_mac_addr(uint16_t port, uint16_t vf,
  ...
  if (rte_is_valid_assigned_ether_addr((struct rte_ether_addr *)new_mac)) {
    rte_memcpy(vfinfo[vf].vf_mac_addresses, new_mac, RTE_ETHER_ADDR_LEN);
...
int rte_pmd_ixgbe_set_vf_mac_anti_spoof(uint16_t port, uint16_t vf,
                                        uint8_t on) {
  struct ixgbe_hw *hw;
  struct ixgbe_mac_info *mac;
  struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_set_all_queues_drop_en(uint16_t port, uint8_t on) {
  ...
  for (i = 0; i <= num_queues; i++) {
    reg_value =
        IXGBE_QDE_WRITE | (i << IXGBE_QDE_IDX_SHIFT) | (on & IXGBE_QDE_ENABLE);
...
int rte_pmd_ixgbe_set_vf_vlan_stripq(uint16_t port, uint16_t vf, uint8_t on) {
  ...
  if (hw->mac.type == ixgbe_mac_82598EB)
    queues_per_pool = (uint16_t)hw->mac.max_rx_queues / RTE_ETH_16_POOLS;
  else
    queues_per_pool = (uint16_t)hw->mac.max_rx_queues / RTE_ETH_64_POOLS;
...
int rte_pmd_ixgbe_set_vf_rxmode(uint16_t port, uint16_t vf, uint16_t rx_mask,
                                uint8_t on) {
  int val = 0;
  struct rte_eth_dev *dev;
  struct rte_pci_device *pci_dev;
...
int rte_pmd_ixgbe_set_vf_vlan_filter(uint16_t port, uint16_t vlan,
  ...
  for (vf_idx = 0; vf_idx < 64; vf_idx++) {
    if (vf_mask & ((uint64_t)(1ULL << vf_idx))) {
      ret = hw->mac.ops.set_vfta(hw, vlan, vf_idx, vlan_on, false);
      if (ret < 0)
        return ret;
    }
  }
...
int rte_pmd_ixgbe_set_tc_bw_alloc(uint16_t port, uint8_t tc_num,
                                  uint8_t *bw_weight) {
  struct rte_eth_dev *dev;
  ...
  if (nb_tcs != tc_num) {
    PMD_DRV_LOG(ERR, "Weight should be set for all %d enabled TCs.", nb_tcs);
    return -EINVAL;
  }
...
int rte_pmd_ixgbe_bypass_init(uint16_t port_id) {
  struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_state_show(uint16_t port_id, uint32_t *state) {
  struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_state_set(uint16_t port_id, uint32_t *new_state) {
  struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_event_show(uint16_t port_id, uint32_t event,
                                    uint32_t *state) {
  struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_event_store(uint16_t port_id, uint32_t event,
                                     uint32_t state) {
  struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_wd_timeout_store(uint16_t port_id, uint32_t timeout) {
  struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_ver_show(uint16_t port_id, uint32_t *ver) {
  struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_wd_timeout_show(uint16_t port_id,
                                         uint32_t *wd_timeout) {
  struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_wd_reset(uint16_t port_id) {
  struct rte_eth_dev *dev;
...
STATIC s32 rte_pmd_ixgbe_acquire_swfw(struct ixgbe_hw *hw, u32 mask) {
  ...
  while (--retries) {
    ...
    if (status != IXGBE_ERR_TOKEN_RETRY) {
      PMD_DRV_LOG(ERR, "Retry get PHY token failed, Status=%d\n", status);
      return status;
    }
  }
...
int rte_pmd_ixgbe_mdio_unlocked_read(uint16_t port, uint32_t reg_addr,
  ...
  command = (reg_addr << IXGBE_MSCA_DEV_TYPE_SHIFT) |
            (dev_type << IXGBE_MSCA_PHY_ADDR_SHIFT) | IXGBE_MSCA_OLD_PROTOCOL |
...
int rte_pmd_ixgbe_get_fdir_info(uint16_t port,
...
int rte_pmd_ixgbe_get_fdir_stats(uint16_t port,
