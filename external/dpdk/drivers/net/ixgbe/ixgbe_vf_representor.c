...
static int ixgbe_vf_representor_link_update(struct rte_eth_dev *ethdev,
                                            int wait_to_complete) {
  struct ixgbe_vf_representor *representor = ethdev->data->dev_private;

  return ixgbe_dev_link_update_share(representor->pf_ethdev, wait_to_complete,
                                     0);
}

static int ixgbe_vf_representor_mac_addr_set(struct rte_eth_dev *ethdev,
                                             struct rte_ether_addr *mac_addr) {
  struct ixgbe_vf_representor *representor = ethdev->data->dev_private;

  return rte_pmd_ixgbe_set_vf_mac_addr(representor->pf_ethdev->data->port_id,
...
ixgbe_vf_representor_dev_infos_get(struct rte_eth_dev *ethdev,
  ...
  struct ixgbe_hw *hw =
      IXGBE_DEV_PRIVATE_TO_HW(representor->pf_ethdev->data->dev_private);
  ...
  dev_info->speed_capa = representor->pf_ethdev->data->dev_link.link_speed;
  /**< Supported speeds bitmap (RTE_ETH_LINK_SPEED_). */

  dev_info->switch_info.name = representor->pf_ethdev->device->name;
...
static int
ixgbe_vf_representor_dev_configure(__rte_unused struct rte_eth_dev *dev) {
  return 0;
}
...
static int
ixgbe_vf_representor_dev_start(__rte_unused struct rte_eth_dev *dev) {
  return 0;
}
...
static int ixgbe_vf_representor_vlan_filter_set(struct rte_eth_dev *ethdev,
                                                uint16_t vlan_id, int on) {
  struct ixgbe_vf_representor *representor = ethdev->data->dev_private;
  uint64_t vf_mask = 1ULL << representor->vf_id;

  return rte_pmd_ixgbe_set_vf_vlan_filter(representor->pf_ethdev->data->port_id,
                                          vlan_id, vf_mask, on);
}

static void ixgbe_vf_representor_vlan_strip_queue_set(
...
int ixgbe_vf_representor_init(struct rte_eth_dev *ethdev, void *init_params) {
  ...
  representor->vf_id = ((struct ixgbe_vf_representor *)init_params)->vf_id;
  ...
  vf_data =
      *IXGBE_DEV_PRIVATE_TO_P_VFDATA(representor->pf_ethdev->data->dev_private);

  ethdev->data->mac_addrs =
      (struct rte_ether_addr *)vf_data[representor->vf_id].vf_mac_addresses;
