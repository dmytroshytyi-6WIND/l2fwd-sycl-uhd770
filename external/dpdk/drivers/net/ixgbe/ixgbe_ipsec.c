...
#define IXGBE_WAIT_RREAD                                                       \
  IXGBE_WRITE_REG_THEN_POLL_MASK(hw, IXGBE_IPSRXIDX, reg_val, IPSRXIDX_READ,   \
                                 RTE_IXGBE_REGISTER_POLL_WAIT_5_MS)
#define IXGBE_WAIT_RWRITE                                                      \
  IXGBE_WRITE_REG_THEN_POLL_MASK(hw, IXGBE_IPSRXIDX, reg_val, IPSRXIDX_WRITE,  \
                                 RTE_IXGBE_REGISTER_POLL_WAIT_5_MS)
#define IXGBE_WAIT_TREAD                                                       \
  IXGBE_WRITE_REG_THEN_POLL_MASK(hw, IXGBE_IPSTXIDX, reg_val, IPSRXIDX_READ,   \
                                 RTE_IXGBE_REGISTER_POLL_WAIT_5_MS)
#define IXGBE_WAIT_TWRITE                                                      \
  IXGBE_WRITE_REG_THEN_POLL_MASK(hw, IXGBE_IPSTXIDX, reg_val, IPSRXIDX_WRITE,  \
                                 RTE_IXGBE_REGISTER_POLL_WAIT_5_MS)
...
static void ixgbe_crypto_clear_ipsec_tables(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct ixgbe_ipsec *priv = IXGBE_DEV_PRIVATE_TO_IPSEC(dev->data->dev_private);
  int i = 0;
...
static int ixgbe_crypto_add_sa(struct ixgbe_crypto_session *ic_session) {
  ...
  if (ic_session->op == IXGBE_OP_AUTHENTICATED_DECRYPTION) {
    ...
    reg_val =
        IPSRXIDX_RX_EN | IPSRXIDX_WRITE | IPSRXIDX_TABLE_IP | (ip_index << 3);
    ...
    reg_val =
        IPSRXIDX_RX_EN | IPSRXIDX_WRITE | IPSRXIDX_TABLE_SPI | (sa_index << 3);
    IXGBE_WRITE_REG(hw, IXGBE_IPSRXSPI, priv->rx_sa_tbl[sa_index].spi);
    IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPIDX, priv->rx_sa_tbl[sa_index].ip_index);
    IXGBE_WAIT_RWRITE;
    ...
    reg_val =
        IPSRXIDX_RX_EN | IPSRXIDX_WRITE | IPSRXIDX_TABLE_KEY | (sa_index << 3);
    ...
    IXGBE_WRITE_REG(hw, IXGBE_IPSRXSALT, rte_cpu_to_be_32(ic_session->salt));
    IXGBE_WRITE_REG(hw, IXGBE_IPSRXMOD, priv->rx_sa_tbl[sa_index].mode);
    IXGBE_WAIT_RWRITE;
    ...
    IXGBE_WRITE_REG(hw, IXGBE_IPSTXKEY(3),
                    rte_cpu_to_be_32(*(uint32_t *)&key[0]));
    IXGBE_WRITE_REG(hw, IXGBE_IPSTXSALT, rte_cpu_to_be_32(ic_session->salt));
    IXGBE_WAIT_TWRITE;
...
static int ixgbe_crypto_remove_sa(struct rte_eth_dev *dev,
  ...
  if (ic_session->op == IXGBE_OP_AUTHENTICATED_DECRYPTION) {
    ...
    if (priv->rx_ip_tbl[ip_index].ref_count == 0) {
      reg_val = IPSRXIDX_WRITE | IPSRXIDX_TABLE_IP | (ip_index << 3);
      IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(0), 0);
      IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(1), 0);
      IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(2), 0);
      IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(3), 0);
...
static int ixgbe_crypto_create_session(void *device,
...
static unsigned int ixgbe_crypto_session_get_size(__rte_unused void *device) {
  return sizeof(struct ixgbe_crypto_session);
}

static int ixgbe_crypto_remove_session(void *device,
                                       struct rte_security_session *session) {
  struct rte_eth_dev *eth_dev = device;
  struct ixgbe_crypto_session *ic_session =
      (struct ixgbe_crypto_session *)get_sec_session_private_data(session);
...
static inline uint8_t ixgbe_crypto_compute_pad_len(struct rte_mbuf *m) {
  if (m->nb_segs == 1) {
...
static int ixgbe_crypto_update_mb(void *device __rte_unused,
                                  struct rte_security_session *session,
                                  struct rte_mbuf *m,
  ...
  if (ic_session->op == IXGBE_OP_AUTHENTICATED_ENCRYPTION) {
    union ixgbe_crypto_tx_desc_md *mdata =
        (union ixgbe_crypto_tx_desc_md *)rte_security_dynfield(m);
    mdata->enc = 1;
...
int ixgbe_crypto_add_ingress_sa_from_flow(const void *sess, const void *ip_spec,
                                          uint8_t is_ipv6) {
...
static int ixgbe_crypto_capable(struct rte_eth_dev *dev) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
