...
static inline int ixgbe_vf_perm_addr_gen(struct rte_eth_dev *dev,
                                         uint16_t vf_num) {
...
static inline int ixgbe_mb_intr_setup(struct rte_eth_dev *dev) {
...
static void ixgbe_add_tx_flow_control_drop_filter(struct rte_eth_dev *eth_dev) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  ...
  i = ixgbe_ethertype_filter_lookup(filter_info, IXGBE_ETHERTYPE_FLOW_CTRL);
  if (i >= 0) {
    PMD_DRV_LOG(
  ...
  ethertype_filter.etqf = IXGBE_ETQF_FILTER_EN | IXGBE_ETQF_TX_ANTISPOOF |
                          IXGBE_ETHERTYPE_FLOW_CTRL;
  ethertype_filter.etqs = 0;
  ethertype_filter.conf = TRUE;
  i = ixgbe_ethertype_filter_insert(filter_info, &ethertype_filter);
  if (i < 0) {
    PMD_DRV_LOG(
        ERR,
...
int ixgbe_pf_host_configure(struct rte_eth_dev *eth_dev) {
  ...
  vtctl |= RTE_ETH_DEV_SRIOV(eth_dev).def_vmdq_idx << IXGBE_VT_CTL_POOL_SHIFT;
...
static void set_rx_mode(struct rte_eth_dev *dev) {
  ...
  if (hw->mac.type != ixgbe_mac_82598EB) {
    vmolr |= IXGBE_READ_REG(hw, IXGBE_VMOLR(vfn)) &
             ~(IXGBE_VMOLR_MPE | IXGBE_VMOLR_ROMPE | IXGBE_VMOLR_ROPE);
...
static inline void ixgbe_vf_reset_event(struct rte_eth_dev *dev, uint16_t vf) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct ixgbe_vf_info *vfinfo =
      *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
  ...
  vmolr |= (IXGBE_VMOLR_ROPE | IXGBE_VMOLR_BAM | IXGBE_VMOLR_AUPE);
...
static inline void ixgbe_vf_reset_msg(struct rte_eth_dev *dev, uint16_t vf) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint32_t reg;
...
static int ixgbe_disable_vf_mc_promisc(struct rte_eth_dev *dev, uint32_t vf) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint32_t vmolr;
...
static int ixgbe_vf_reset(struct rte_eth_dev *dev, uint16_t vf,
                          uint32_t *msgbuf) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct ixgbe_vf_info *vfinfo =
      *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
...
static int ixgbe_vf_set_mac_addr(struct rte_eth_dev *dev, uint32_t vf,
                                 uint32_t *msgbuf) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct ixgbe_vf_info *vfinfo =
      *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
...
static int ixgbe_vf_set_multicast(struct rte_eth_dev *dev, uint32_t vf,
                                  uint32_t *msgbuf) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct ixgbe_vf_info *vfinfo =
      *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
  ...
  for (i = 0; i < vfinfo->num_vf_mc_hashes; i++) {
    mta_idx =
        (vfinfo->vf_mc_hashes[i] >> IXGBE_MTA_BIT_SHIFT) & IXGBE_MTA_INDEX_MASK;
...
static int ixgbe_vf_set_vlan(struct rte_eth_dev *dev, uint32_t vf,
                             uint32_t *msgbuf) {
  int add, vid;
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct ixgbe_vf_info *vfinfo =
...
static int ixgbe_set_vf_lpe(struct rte_eth_dev *dev, uint32_t vf,
                            uint32_t *msgbuf) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  if (hw->mac.type != ixgbe_mac_X540 && hw->mac.type != ixgbe_mac_X550 &&
    ...
    default:
       ...
       * legacy VFs.
       */
      if (max_frame > IXGBE_ETH_MAX_LEN || dev->data->mtu > RTE_ETHER_MTU)
        return -1;
      break;
    }
  }
  ...
  max_frs = (IXGBE_READ_REG(hw, IXGBE_MAXFRS) & IXGBE_MHADD_MFS_MASK) >>
...
static int ixgbe_negotiate_vf_api(struct rte_eth_dev *dev, uint32_t vf,
                                  uint32_t *msgbuf) {
...
static int ixgbe_get_vf_queues(struct rte_eth_dev *dev, uint32_t vf,
                               uint32_t *msgbuf) {
  struct ixgbe_vf_info *vfinfo =
      *IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private);
...
#define VLAN_PRIO_SHIFT 13
  ...
  case RTE_ETH_MQ_TX_VMDQ_ONLY:
    ...
    user_priority = (vmvir & IXGBE_VMVIR_VLAN_UP_MASK) >> VLAN_PRIO_SHIFT;
...
static int ixgbe_set_vf_mc_promisc(struct rte_eth_dev *dev, uint32_t vf,
                                   uint32_t *msgbuf) {
  struct ixgbe_vf_info *vfinfo =
      *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int ixgbe_set_vf_macvlan_msg(struct rte_eth_dev *dev, uint32_t vf,
                                    uint32_t *msgbuf) {
  struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct ixgbe_vf_info *vf_info =
      *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
  uint8_t *new_mac = (uint8_t *)(&msgbuf[1]);
  int index = (msgbuf[0] & IXGBE_VT_MSGINFO_MASK) >> IXGBE_VT_MSGINFO_SHIFT;
  ...
  if (index) {
    ...
    hw->mac.ops.set_rar(hw, vf_info[vf].mac_count, new_mac, vf, IXGBE_RAH_AV);
  } else {
...
static int ixgbe_rcv_msg_from_vf(struct rte_eth_dev *dev, uint16_t vf) {
...
static inline void ixgbe_rcv_ack_from_vf(struct rte_eth_dev *dev, uint16_t vf) {
