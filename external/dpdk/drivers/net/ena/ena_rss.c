...
int ena_rss_reta_update(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size; i++) {
    ...
    if (TEST_BIT(reta_conf[conf_idx].mask, idx)) {
      entry_value = ENA_IO_RXQ_IDX(reta_conf[conf_idx].reta[idx]);

      rc = ena_com_indirect_table_fill_entry(ena_dev, i, entry_value);
      if (unlikely(rc != 0)) {
...
static int ena_fill_indirect_table_default(struct ena_com_dev *ena_dev,
  ...
  for (i = 0; i < tbl_size; ++i) {
    val = i % queue_num;
    rc = ena_com_indirect_table_fill_entry(ena_dev, i, ENA_IO_RXQ_IDX(val));
    if (unlikely(rc != 0)) {
      PMD_DRV_LOG(DEBUG,
...
static int ena_set_hash_fields(struct ena_com_dev *ena_dev, uint64_t rss_hf) {
  ...
  for (i = 0; i < ENA_ADMIN_RSS_PROTO_NUM; i++) {
    rc = ena_com_fill_hash_ctrl(ena_dev, (enum ena_admin_flow_hash_proto)i,
                                selected_fields[i].fields);
    if (unlikely(rc != 0)) {
      PMD_DRV_LOG(DEBUG, "Failed to set ENA HF %d with fields %" PRIu16 "\n", i,
                  selected_fields[i].fields);
      return rc;
    }
  }
...
static int ena_rss_hash_set(struct ena_com_dev *ena_dev,
  ...
  if (rss_conf->rss_key != NULL) {
    ...
    ena_reorder_rss_hash_key(hw_rss_key, rss_conf->rss_key, ENA_HASH_KEY_SIZE);
    rss_key = hw_rss_key;
  } else {
    rss_key = NULL;
  }
  ...
  rc = ena_com_fill_hash_function(ena_dev, ENA_ADMIN_TOEPLITZ, rss_key,
                                  ENA_HASH_KEY_SIZE, 0);
  if (rc != 0 && !(default_allowed && rc == ENA_COM_UNSUPPORTED)) {
    PMD_DRV_LOG(ERR, "Failed to set RSS hash function in the device\n");
    return rc;
  }
  ...
  if (rc == ENA_COM_UNSUPPORTED) {
    if (rss_conf->rss_key == NULL && !default_allowed) {
      PMD_DRV_LOG(ERR, "Setting RSS hash fields is not supported\n");
      return -ENOTSUP;
    }
    PMD_DRV_LOG(WARNING,
                "Setting RSS hash fields is not supported. Using default "
                "values: 0x%" PRIx64 "\n",
...
static int ena_get_rss_hash_key(struct ena_com_dev *ena_dev, uint8_t *rss_key) {
  ...
  if (ena_dev->rss.hash_key == NULL) {
    PMD_DRV_LOG(WARNING, "Retrieving default RSS hash key is not supported\n");
    return -ENOTSUP;
  }
...
int ena_rss_configure(struct ena_adapter *adapter) {
  ...
  if (unlikely(rc != 0)) {
    PMD_DRV_LOG(ERR, "Failed to fill indirection table with default values\n");
    return rc;
  }

  rc = ena_com_indirect_table_set(ena_dev);
  if (unlikely(rc != 0 && rc != ENA_COM_UNSUPPORTED)) {
    PMD_DRV_LOG(ERR, "Failed to set indirection table in the device\n");
    return rc;
  }
...
int ena_rss_hash_conf_get(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_key != NULL) {
    ...
    if (unlikely(rc != 0)) {
      PMD_DRV_LOG(ERR, "Cannot retrieve RSS hash key, err: %d\n", rc);
      return rc;
    }
  }
  ...
  for (i = 0; i < ENA_ADMIN_RSS_PROTO_NUM; ++i) {
    ...
    if (rc == ENA_COM_UNSUPPORTED) {
      ...
      if (!warn_once) {
        PMD_DRV_LOG(WARNING,
                    "Reading hash control from the device is not supported. "
                    ".rss_hf will contain a default value.\n");
        warn_once = true;
      }
