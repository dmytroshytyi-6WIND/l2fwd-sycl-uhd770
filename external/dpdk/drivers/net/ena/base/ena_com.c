...
#define MIN_ENA_CTRL_VER                                                       \
  (((ENA_CTRL_MAJOR) << (ENA_REGS_CONTROLLER_VERSION_MAJOR_VERSION_SHIFT)) |   \
...
static int ena_com_admin_init_aenq(struct ena_com_dev *ena_dev,
  ...
  ENA_MEM_ALLOC_COHERENT(ena_dev->dmadev, size, aenq->entries, aenq->dma_addr,
                         aenq->mem_handle);
  ...
  ENA_REG_WRITE32(ena_dev->bus, addr_low,
                  ena_dev->reg_bar + ENA_REGS_AENQ_BASE_LO_OFF);
  ENA_REG_WRITE32(ena_dev->bus, addr_high,
                  ena_dev->reg_bar + ENA_REGS_AENQ_BASE_HI_OFF);
  ...
  ENA_REG_WRITE32(ena_dev->bus, aenq_caps,
                  ena_dev->reg_bar + ENA_REGS_AENQ_CAPS_OFF);
...
get_comp_ctxt(struct ena_com_admin_queue *admin_queue, u16 command_id,
              bool capture) {
...
static struct ena_comp_ctx *__ena_com_submit_admin_cmd(
    struct ena_com_admin_queue *admin_queue, struct ena_admin_aq_entry *cmd,
    size_t cmd_size_in_bytes, struct ena_admin_acq_entry *comp,
  ...
  cmd->aq_common_descriptor.flags |=
      admin_queue->sq.phase & ENA_ADMIN_AQ_COMMON_DESC_PHASE_MASK;
...
static int ena_com_init_io_sq(struct ena_com_dev *ena_dev,
  ...
  io_sq->desc_entry_size = (io_sq->direction == ENA_COM_IO_QUEUE_DIRECTION_TX)
  ...
  if (io_sq->mem_queue_type == ENA_ADMIN_PLACEMENT_POLICY_HOST) {
    ENA_MEM_ALLOC_COHERENT_NODE(
        ena_dev->dmadev, size, io_sq->desc_addr.virt_addr,
        io_sq->desc_addr.phys_addr, io_sq->desc_addr.mem_handle, ctx->numa_node,
        dev_node);
    if (!io_sq->desc_addr.virt_addr) {
      ENA_MEM_ALLOC_COHERENT(ena_dev->dmadev, size, io_sq->desc_addr.virt_addr,
  ...
  if (io_sq->mem_queue_type == ENA_ADMIN_PLACEMENT_POLICY_DEV) {
    ...
    size =
        io_sq->bounce_buf_ctrl.buffer_size * io_sq->bounce_buf_ctrl.buffers_num;

    ENA_MEM_ALLOC_NODE(ena_dev->dmadev, size,
                       io_sq->bounce_buf_ctrl.base_buffer, ctx->numa_node,
                       dev_node);
    ...
    memset(io_sq->llq_buf_ctrl.curr_bounce_buf, 0x0,
...
static int ena_com_init_io_cq(struct ena_com_dev *ena_dev,
  ...
  ENA_MEM_ALLOC_COHERENT_NODE_ALIGNED(
      ena_dev->dmadev, size, io_cq->cdesc_addr.virt_addr,
      io_cq->cdesc_addr.phys_addr, io_cq->cdesc_addr.mem_handle, ctx->numa_node,
      prev_node, ENA_CDESC_RING_SIZE_ALIGNMENT);
  if (!io_cq->cdesc_addr.virt_addr) {
    ENA_MEM_ALLOC_COHERENT_ALIGNED(
        ena_dev->dmadev, size, io_cq->cdesc_addr.virt_addr,
        io_cq->cdesc_addr.phys_addr, io_cq->cdesc_addr.mem_handle,
...
static int ena_com_comp_status_to_errno(struct ena_com_admin_queue *admin_queue,
  ...
  if (unlikely(comp_status != 0))
    ena_trc_err(admin_queue->ena_dev, "Admin command failed[%u]\n",
                comp_status);
...
static int ena_com_wait_and_process_admin_cq_polling(
    struct ena_comp_ctx *comp_ctx, struct ena_com_admin_queue *admin_queue) {
  unsigned long flags = 0;
  ena_time_t timeout;
  int ret;
  u32 exp = 0;
  ...
  while (1) {
    ...
    ena_delay_exponential_backoff_us(
        exp++, admin_queue->ena_dev->ena_min_poll_delay_us);
  }
  ...
  ENA_WARN(comp_ctx->status != ENA_CMD_COMPLETED, admin_queue->ena_dev,
...
static int ena_com_set_llq(struct ena_com_dev *ena_dev) {
  ...
  ret = ena_com_execute_admin_command(
      admin_queue, (struct ena_admin_aq_entry *)&cmd, sizeof(cmd),
...
static int
ena_com_config_llq_info(struct ena_com_dev *ena_dev,
  ...
  if (likely(supported_feat & llq_default_cfg->llq_ring_entry_size)) {
    ...
    ena_trc_err(ena_dev,
                "Default llq ring entry size is not supported, performing "
                "fallback, default: 0x%x, supported: 0x%x, used: 0x%x\n",
  ...
  if (llq_info->desc_stride_ctrl == ENA_ADMIN_MULTIPLE_DESCS_PER_ENTRY)
    llq_info->descs_per_entry =
        llq_info->desc_list_entry_size / sizeof(struct ena_eth_io_tx_desc);
  else
  ...
  if (likely(supported_feat & llq_default_cfg->llq_num_decs_before_header)) {
    ...
    ena_trc_err(
        ena_dev,
        "Default llq num descs before header is not supported, performing "
        "fallback, default: 0x%x, supported: 0x%x, used: 0x%x\n",
...
static int ena_com_wait_and_process_admin_cq_interrupts(
  ...
  ENA_WAIT_EVENT_WAIT(comp_ctx->wait_event, admin_queue->completion_timeout);
  ...
  if (unlikely(comp_ctx->status == ENA_CMD_SUBMITTED)) {
    ...
    if (comp_ctx->status == ENA_CMD_COMPLETED) {
      ena_trc_err(
          admin_queue->ena_dev,
          "The ena device sent a completion but the driver didn't receive a "
          "MSI-X interrupt (cmd %d), autopolling mode is %s\n",
      ...
      ena_trc_err(admin_queue->ena_dev,
                  "The ena device didn't send a completion for the admin cmd "
                  "%d status %d\n",
...
static u32 ena_com_reg_bar_read32(struct ena_com_dev *ena_dev, u16 offset) {
  ...
  if (unlikely(i == timeout)) {
    ena_trc_err(ena_dev,
                "Reading reg failed for timeout. expected: req id[%hu] "
...
static int ena_com_destroy_io_sq(struct ena_com_dev *ena_dev,
  ...
  destroy_cmd.sq.sq_identity |= (direction << ENA_ADMIN_SQ_SQ_DIRECTION_SHIFT) &
  ...
  ret = ena_com_execute_admin_command(
      admin_queue, (struct ena_admin_aq_entry *)&destroy_cmd,
...
static void ena_com_io_queue_free(struct ena_com_dev *ena_dev,
  ...
  if (io_cq->cdesc_addr.virt_addr) {
    ...
    ENA_MEM_FREE_COHERENT(ena_dev->dmadev, size, io_cq->cdesc_addr.virt_addr,
  ...
  if (io_sq->desc_addr.virt_addr) {
    ...
    ENA_MEM_FREE_COHERENT(ena_dev->dmadev, size, io_sq->desc_addr.virt_addr,
                          io_sq->desc_addr.phys_addr,
                          io_sq->desc_addr.mem_handle);
...
static int ena_com_get_feature_ex(struct ena_com_dev *ena_dev,
  ...
  ret = ena_com_mem_addr_set(ena_dev, &get_cmd.control_buffer.address,
  ...
  ret = ena_com_execute_admin_command(
      admin_queue, (struct ena_admin_aq_entry *)&get_cmd, sizeof(get_cmd),
...
static int ena_com_get_feature(struct ena_com_dev *ena_dev,
                               ...
                               enum ena_admin_aq_feature_id feature_id,
                               u8 feature_ver) {
  return ena_com_get_feature_ex(ena_dev, get_resp, feature_id, 0, 0,
                                feature_ver);
}
...
static int ena_com_hash_key_allocate(struct ena_com_dev *ena_dev) {
  ...
  ENA_MEM_ALLOC_COHERENT(ena_dev->dmadev, sizeof(*rss->hash_key), rss->hash_key,
                         rss->hash_key_dma_addr, rss->hash_key_mem_handle);
...
static int ena_com_indirect_table_allocate(struct ena_com_dev *ena_dev,
  ...
  if ((get_resp.u.ind_table.min_size > log_size) ||
    ...
    ena_trc_err(ena_dev,
                "Indirect table size doesn't fit. requested size: %d while min "
                "is:%d and max %d\n",
...
mem_err2:
  ...
  ENA_MEM_FREE_COHERENT(ena_dev->dmadev, tbl_size, rss->rss_ind_tbl,
                        rss->rss_ind_tbl_dma_addr, rss->rss_ind_tbl_mem_handle);
...
static int ena_com_create_io_sq(struct ena_com_dev *ena_dev,
  ...
  create_cmd.sq_identity |=
      (direction << ENA_ADMIN_AQ_CREATE_SQ_CMD_SQ_DIRECTION_SHIFT) &
      ENA_ADMIN_AQ_CREATE_SQ_CMD_SQ_DIRECTION_MASK;

  create_cmd.sq_caps_2 |=
      io_sq->mem_queue_type & ENA_ADMIN_AQ_CREATE_SQ_CMD_PLACEMENT_POLICY_MASK;
  ...
  if (io_sq->mem_queue_type == ENA_ADMIN_PLACEMENT_POLICY_HOST) {
    ret = ena_com_mem_addr_set(ena_dev, &create_cmd.sq_ba,
                               io_sq->desc_addr.phys_addr);
    if (unlikely(ret)) {
      ena_trc_err(ena_dev, "Memory address set failed\n");
      return ret;
  ...
  ret = ena_com_execute_admin_command(
      admin_queue, (struct ena_admin_aq_entry *)&create_cmd, sizeof(create_cmd),
...
static void ena_com_update_intr_delay_resolution(struct ena_com_dev *ena_dev,
  ...
  if (unlikely(!intr_delay_resolution)) {
    ena_trc_err(ena_dev, "Illegal intr_delay_resolution provided. Going to use "
                         "default 1 usec resolution\n");
...
int ena_com_execute_admin_command(struct ena_com_admin_queue *admin_queue,
  ...
  comp_ctx =
      ena_com_submit_admin_cmd(admin_queue, cmd, cmd_size, comp, comp_size);
  if (IS_ERR(comp_ctx)) {
...
int ena_com_create_io_cq(struct ena_com_dev *ena_dev,
  ...
  ret = ena_com_mem_addr_set(ena_dev, &create_cmd.cq_ba,
                             io_cq->cdesc_addr.phys_addr);
  if (unlikely(ret)) {
    ena_trc_err(ena_dev, "Memory address set failed\n");
    return ret;
...
void ena_com_admin_aenq_enable(struct ena_com_dev *ena_dev) {
   ...
   */
  ENA_REG_WRITE32(ena_dev->bus, depth,
...
int ena_com_set_aenq_config(struct ena_com_dev *ena_dev, u32 groups_flag) {
  ...
  if ((get_resp.u.aenq.supported_groups & groups_flag) != groups_flag) {
    ena_trc_warn(ena_dev,
                 "Trying to set unsupported aenq events. supported flag: 0x%x "
                 "asked flag: 0x%x\n",
...
int ena_com_validate_version(struct ena_com_dev *ena_dev) {
  ...
  ctrl_ver = ena_com_reg_bar_read32(ena_dev, ENA_REGS_CONTROLLER_VERSION_OFF);
  ...
  if (ctrl_ver_masked < MIN_ENA_CTRL_VER) {
    ena_trc_err(ena_dev, "ENA ctrl version is lower than the minimal ctrl "
...
void ena_com_admin_destroy(struct ena_com_dev *ena_dev) {
  ...
  if (sq->entries)
    ENA_MEM_FREE_COHERENT(ena_dev->dmadev, size, sq->entries, sq->dma_addr,
                          sq->mem_handle);
  sq->entries = NULL;

  size = ADMIN_CQ_SIZE(admin_queue->q_depth);
  if (cq->entries)
    ENA_MEM_FREE_COHERENT(ena_dev->dmadev, size, cq->entries, cq->dma_addr,
                          cq->mem_handle);
  cq->entries = NULL;

  size = ADMIN_AENQ_SIZE(aenq->q_depth);
  if (ena_dev->aenq.entries)
    ENA_MEM_FREE_COHERENT(ena_dev->dmadev, size, aenq->entries, aenq->dma_addr,
                          aenq->mem_handle);
  aenq->entries = NULL;
...
int ena_com_mmio_reg_read_request_init(struct ena_com_dev *ena_dev) {
  ...
  ENA_MEM_ALLOC_COHERENT(ena_dev->dmadev, sizeof(*mmio_read->read_resp),
...
void ena_com_mmio_reg_read_request_destroy(struct ena_com_dev *ena_dev) {
  ...
  ENA_REG_WRITE32(ena_dev->bus, 0x0,
                  ena_dev->reg_bar + ENA_REGS_MMIO_RESP_LO_OFF);
  ENA_REG_WRITE32(ena_dev->bus, 0x0,
                  ena_dev->reg_bar + ENA_REGS_MMIO_RESP_HI_OFF);

  ENA_MEM_FREE_COHERENT(ena_dev->dmadev, sizeof(*mmio_read->read_resp),
                        mmio_read->read_resp, mmio_read->read_resp_dma_addr,
                        mmio_read->read_resp_mem_handle);
...
int ena_com_admin_init(struct ena_com_dev *ena_dev,
  ...
  admin_queue->sq.db_addr =
      (u32 __iomem *)((uintptr_t)ena_dev->reg_bar + ENA_REGS_AQ_DB_OFF);

  addr_low = ENA_DMA_ADDR_TO_UINT32_LOW(admin_queue->sq.dma_addr);
  addr_high = ENA_DMA_ADDR_TO_UINT32_HIGH(admin_queue->sq.dma_addr);

  ENA_REG_WRITE32(ena_dev->bus, addr_low,
                  ena_dev->reg_bar + ENA_REGS_AQ_BASE_LO_OFF);
  ENA_REG_WRITE32(ena_dev->bus, addr_high,
                  ena_dev->reg_bar + ENA_REGS_AQ_BASE_HI_OFF);

  addr_low = ENA_DMA_ADDR_TO_UINT32_LOW(admin_queue->cq.dma_addr);
  addr_high = ENA_DMA_ADDR_TO_UINT32_HIGH(admin_queue->cq.dma_addr);

  ENA_REG_WRITE32(ena_dev->bus, addr_low,
                  ena_dev->reg_bar + ENA_REGS_ACQ_BASE_LO_OFF);
  ENA_REG_WRITE32(ena_dev->bus, addr_high,
                  ena_dev->reg_bar + ENA_REGS_ACQ_BASE_HI_OFF);
  ...
  ENA_REG_WRITE32(ena_dev->bus, aq_caps,
                  ena_dev->reg_bar + ENA_REGS_AQ_CAPS_OFF);
  ENA_REG_WRITE32(ena_dev->bus, acq_caps,
                  ena_dev->reg_bar + ENA_REGS_ACQ_CAPS_OFF);
...
int ena_com_get_dev_attr_feat(
  ...
  memcpy(&get_feat_ctx->aenq, &get_resp.u.aenq, sizeof(get_resp.u.aenq));
...
ena_com_get_specific_aenq_cb(struct ena_com_dev *ena_dev, u16 group) {
...
int ena_com_dev_reset(struct ena_com_dev *ena_dev,
  ...
  ENA_REG_WRITE32(ena_dev->bus, reset_val,
                  ena_dev->reg_bar + ENA_REGS_DEV_CTL_OFF);
...
int ena_com_get_eni_stats(struct ena_com_dev *ena_dev,
  ...
  if (likely(ret == 0))
    memcpy(stats, &ctx.get_resp.u.eni_stats, sizeof(ctx.get_resp.u.eni_stats));
...
int ena_com_get_offload_settings(
    struct ena_com_dev *ena_dev,
    struct ena_admin_feature_offload_desc *offload) {
  int ret;
  struct ena_admin_get_feat_resp resp;

  ret = ena_com_get_feature(ena_dev, &resp, ENA_ADMIN_STATELESS_OFFLOAD_CONFIG,
                            0);
  if (unlikely(ret)) {
...
int ena_com_set_hash_function(struct ena_com_dev *ena_dev) {
  ...
  ret = ena_com_mem_addr_set(ena_dev, &cmd.control_buffer.address,
...
int ena_com_fill_hash_function(struct ena_com_dev *ena_dev,
  ...
  rc =
      ena_com_get_feature_ex(ena_dev, &get_resp, ENA_ADMIN_RSS_HASH_FUNCTION,
                             rss->hash_key_dma_addr, sizeof(*rss->hash_key), 0);
  if (unlikely(rc))
    return rc;
...
int ena_com_get_hash_ctrl(struct ena_com_dev *ena_dev,
                          enum ena_admin_flow_hash_proto proto, u16 *fields) {
  struct ena_rss *rss = &ena_dev->rss;
  struct ena_admin_get_feat_resp get_resp;
  int rc;

  rc = ena_com_get_feature_ex(ena_dev, &get_resp, ENA_ADMIN_RSS_HASH_INPUT,
                              rss->hash_ctrl_dma_addr, sizeof(*rss->hash_ctrl),
                              0);
  if (unlikely(rc))
    return rc;
...
int ena_com_set_default_hash_ctrl(struct ena_com_dev *ena_dev) {
  ...
  hash_ctrl->selected_fields[ENA_ADMIN_RSS_TCP4].fields =
      ENA_ADMIN_RSS_L3_SA | ENA_ADMIN_RSS_L3_DA | ENA_ADMIN_RSS_L4_DP |
      ENA_ADMIN_RSS_L4_SP;
  ...
  for (i = 0; i < ENA_ADMIN_RSS_PROTO_NUM; i++) {
    ...
    if (available_fields != hash_ctrl->selected_fields[i].fields) {
      ena_trc_err(ena_dev,
                  "Hash control doesn't support all the desire configuration. "
                  "proto %x supported %x selected %x\n",
...
int ena_com_indirect_table_get(struct ena_com_dev *ena_dev, u32 *ind_tbl) {
  ...
  rc = ena_com_get_feature_ex(ena_dev, &get_resp,
                              ENA_ADMIN_RSS_INDIRECTION_TABLE_CONFIG,
                              rss->rss_ind_tbl_dma_addr, tbl_size, 0);
  if (unlikely(rc))
    return rc;
...
int ena_com_allocate_host_info(struct ena_com_dev *ena_dev) {
  ...
  ENA_MEM_ALLOC_COHERENT(ena_dev->dmadev, SZ_4K, host_attr->host_info,
...
int ena_com_allocate_debug_area(struct ena_com_dev *ena_dev,
  ...
  ENA_MEM_ALLOC_COHERENT(
      ena_dev->dmadev, debug_area_size, host_attr->debug_area_virt_addr,
...
void ena_com_delete_host_info(struct ena_com_dev *ena_dev) {
  ...
  if (host_attr->host_info) {
    ENA_MEM_FREE_COHERENT(ena_dev->dmadev, SZ_4K, host_attr->host_info,
                          host_attr->host_info_dma_addr,
                          host_attr->host_info_dma_handle);
...
int ena_com_set_host_attributes(struct ena_com_dev *ena_dev) {
  ...
  ret = ena_com_mem_addr_set(ena_dev, &cmd.u.host_attr.debug_ba,
                             host_attr->debug_area_dma_addr);
  if (unlikely(ret)) {
    ena_trc_err(ena_dev, "Memory address set failed\n");
    return ret;
  ...
  ret = ena_com_mem_addr_set(ena_dev, &cmd.u.host_attr.os_info_ba,
                             host_attr->host_info_dma_addr);
  if (unlikely(ret)) {
    ena_trc_err(ena_dev, "Memory address set failed\n");
    return ret;
...
static int ena_com_update_nonadaptive_moderation_interval(
    struct ena_com_dev *ena_dev, u32 coalesce_usecs, u32 intr_delay_resolution,
...
int ena_com_update_nonadaptive_moderation_interval_tx(
    struct ena_com_dev *ena_dev, u32 tx_coalesce_usecs) {
  return ena_com_update_nonadaptive_moderation_interval(
      ena_dev, tx_coalesce_usecs, ena_dev->intr_delay_resolution,
      &ena_dev->intr_moder_tx_interval);
}

int ena_com_update_nonadaptive_moderation_interval_rx(
    struct ena_com_dev *ena_dev, u32 rx_coalesce_usecs) {
  return ena_com_update_nonadaptive_moderation_interval(
...
int ena_com_init_interrupt_moderation(struct ena_com_dev *ena_dev) {
  ...
  rc = ena_com_get_feature(ena_dev, &get_resp, ENA_ADMIN_INTERRUPT_MODERATION,
                           0);
