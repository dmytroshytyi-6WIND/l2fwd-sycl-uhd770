...
#define ena_trc_log(dev, level, fmt, arg...)                                   \
  (ENA_TOUCH(dev), rte_log(RTE_LOG_##level, ena_logtype_com,                   \
...
#define ENA_MEM_ALLOC_COHERENT_ALIGNED(dmadev, size, virt, phys, mem_handle,   \
  ...
  do {                                                                         \
    ...
    (mem_handle) = ena_mem_alloc_coherent(                                     \
        (dmadev), (size), SOCKET_ID_ANY, (alignment), &virt_addr, &phys_addr); \
    (virt) = virt_addr;                                                        \
    (phys) = phys_addr;                                                        \
  } while (0)
...
#define ENA_MEM_ALLOC_COHERENT_NODE_ALIGNED(                                   \
  ...
  do {                                                                         \
    ...
    ENA_TOUCH(dev_node);                                                       \
    (mem_handle) = ena_mem_alloc_coherent(                                     \
        (dmadev), (size), (node), (alignment), &virt_addr, &phys_addr);        \
    (virt) = virt_addr;                                                        \
    (phys) = phys_addr;                                                        \
  } while (0)
#define ENA_MEM_ALLOC_COHERENT_NODE(dmadev, size, virt, phys, mem_handle,      \
  ...
  ENA_MEM_ALLOC_COHERENT_NODE_ALIGNED(dmadev, size, virt, phys, mem_handle,    \
                                      node, dev_node, RTE_CACHE_LINE_SIZE)
