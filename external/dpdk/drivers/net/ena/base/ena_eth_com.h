...
static inline bool
ena_com_meta_desc_changed(struct ena_com_io_sq *io_sq,
                          struct ena_com_tx_ctx *ena_tx_ctx) {
  if (!ena_tx_ctx->meta_valid)
    return false;

  return !!memcmp(&io_sq->cached_tx_meta, &ena_tx_ctx->ena_meta,
...
ena_com_is_doorbell_needed(struct ena_com_io_sq *io_sq,
  ...
  if (num_descs > llq_info->descs_num_before_header) {
    ...
    num_entries_needed +=
        DIV_ROUND_UP(descs_after_first_entry, llq_info->descs_per_entry);
  }
...
static inline int ena_com_tx_comp_req_id_get(struct ena_com_io_cq *io_cq,
  ...
  cdesc =
      (struct ena_eth_io_tx_cdesc *)((uintptr_t)io_cq->cdesc_addr.virt_addr +
                                     (masked_head *
