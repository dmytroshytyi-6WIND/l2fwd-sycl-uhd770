...
#define ENA_STAT_ENI_ENTRY(stat) ENA_STAT_ENTRY(stat, eni)
...
static int ena_link_update(struct rte_eth_dev *dev, int wait_to_complete);
...
static int ena_xstats_get_names_by_id(struct rte_eth_dev *dev,
                                      ...
                                      struct rte_eth_xstat_name *xstats_names,
                                      unsigned int size);
static int ena_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *stats,
                          unsigned int n);
static int ena_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
                                uint64_t *values, unsigned int n);
static int ena_process_bool_devarg(const char *key, const char *value,
                                   void *opaque);
...
static int ena_rx_queue_intr_enable(struct rte_eth_dev *dev, uint16_t queue_id);
 ...
 *
 */
static void mp_msg_init(struct rte_mp_msg *msg, enum ena_mp_req type,
                        int port_id) {
...
#define ENA_PROXY_DESC(f, t, prep, proc, ...)                                  \
  ...
  static void mp_prep_##f(struct ena_adapter *adapter,                         \
                          struct ena_mp_body *req, __VA_ARGS__) {              \
    prep;                                                                      \
  }                                                                            \
  static void mp_proc_##f(struct ena_adapter *adapter,                         \
                          struct ena_mp_body *rsp, __VA_ARGS__) {              \
    proc;                                                                      \
  }
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {                          \
      ...
      if (likely(!ret)) {                                                      \
        ...
        if (ret == 0) {                                                        \
          ...
          PMD_DRV_LOG(ERR, "%s returned error: %d\n", mp_name_##f,             \
                      rsp->result);                                            \
        }                                                                      \
        free(mp_rep.msgs);                                                     \
      } else if (rte_errno == ENOTSUP) {                                       \
        PMD_DRV_LOG(ERR, "No IPC, can't proxy to primary\n");                  \
        ret = -rte_errno;                                                      \
      } else {                                                                 \
...
static int ena_close(struct rte_eth_dev *dev) {
  ...
  rte_intr_callback_unregister(intr_handle, ena_interrupt_handler_rte, dev);
...
static int ena_link_update(struct rte_eth_dev *dev,
  ...
  link->link_status =
      adapter->link_status ? RTE_ETH_LINK_UP : RTE_ETH_LINK_DOWN;
...
static int ena_queue_start_all(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < nb_queues; i++) {
    if (queues[i].configured) {
      ...
      if (rc) {
        PMD_INIT_LOG(ERR, "Failed to start queue[%d] of type(%d)\n", i,
                     ring_type);
        goto err;
      }
    }
...
static int ena_calc_io_queue_size(struct ena_calc_queue_size_ctx *ctx,
  ...
  if (ena_dev->supported_features & BIT(ENA_ADMIN_MAX_QUEUES_EXT)) {
    ...
    if (ena_dev->tx_mem_queue_type == ENA_ADMIN_PLACEMENT_POLICY_DEV) {
      max_tx_queue_size = RTE_MIN(max_tx_queue_size, llq->max_llq_depth);
    } else {
      max_tx_queue_size =
  ...
  if (use_large_llq_hdr) {
    if ((llq->entry_size_ctrl_supported & ENA_ADMIN_LIST_ENTRY_SIZE_256B) &&
      ...
      PMD_INIT_LOG(ERR, "Forcing large headers failed: LLQ is disabled or "
...
static int ena_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  rc = ENA_PROXY(adapter, ena_com_get_dev_basic_stats, ena_dev, &ena_stats);
...
static int ena_create_io_queue(struct rte_eth_dev *dev, struct ena_ring *ring) {
  ...
  struct ena_com_create_io_ctx ctx =
      ...
      {ENA_ADMIN_PLACEMENT_POLICY_HOST, 0, 0, 0, 0, 0};
  uint16_t ena_qid;
  unsigned int i;
  int rc;
  ...
  if (ring->type == ENA_RING_TYPE_TX) {
    ...
    if (rte_intr_dp_is_en(intr_handle))
      ctx.msix_vector = rte_intr_vec_list_index_get(intr_handle, ring->id);
...
static int ena_queue_start(struct rte_eth_dev *dev, struct ena_ring *ring) {
  ...
  ena_assert_msg(ring->configured == 1, "Trying to start unconfigured queue\n");
...
static int ena_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (nb_desc > adapter->max_tx_ring_size) {
    PMD_DRV_LOG(ERR, "Unsupported size of Tx queue (max size: %d)\n",
  ...
  txq->tx_buffer_info = rte_zmalloc_socket(
      "txq->tx_buffer_info", sizeof(struct ena_tx_buffer) * txq->ring_size,
  ...
  txq->push_buf_intermediate_buf = rte_zmalloc_socket(
      "txq->push_buf_intermediate_buf", txq->tx_max_header_size,
      RTE_CACHE_LINE_SIZE, socket_id);
...
static int ena_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (buffer_size < ENA_RX_BUF_MIN_SIZE) {
    PMD_DRV_LOG(ERR, "Unsupported size of Rx buffer: %zu (min size: %d)\n",
  ...
  rxq->rx_buffer_info = rte_zmalloc_socket(
      "rxq->buffer_info", sizeof(struct ena_rx_buffer) * nb_desc,
      RTE_CACHE_LINE_SIZE, socket_id);
  ...
  rxq->rx_refill_buffer = rte_zmalloc_socket(
      "rxq->rx_refill_buffer", sizeof(struct rte_mbuf *) * nb_desc,
      RTE_CACHE_LINE_SIZE, socket_id);
  ...
  rxq->empty_rx_reqs =
      rte_zmalloc_socket("rxq->empty_rx_reqs", sizeof(uint16_t) * nb_desc,
                         RTE_CACHE_LINE_SIZE, socket_id);
...
static int ena_populate_rx_queue(struct ena_ring *rxq, unsigned int count) {
  ...
  if (unlikely(i < count)) {
    PMD_RX_LOG(WARNING, "Refilled Rx queue[%d] with only %d/%d buffers\n",
               rxq->id, i, count);
...
static int ena_device_init(struct ena_adapter *adapter,
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Cannot get attribute for ENA device, rc: %d\n", rc);
    goto err_admin_init;
  }
...
static int ena_set_queues_placement_policy(
    struct ena_adapter *adapter, struct ena_com_dev *ena_dev,
    struct ena_admin_feature_llq_desc *llq,
  ...
  if (!(ena_dev->supported_features & llq_feature_mask)) {
    PMD_DRV_LOG(INFO, "LLQ is not supported. Fallback to host mode policy.\n");
...
static int eth_ena_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  rc = ena_calc_io_queue_size(&calc_queue_ctx, adapter->use_large_llq_hdr);
  ...
  adapter->drv_stats = rte_zmalloc("adapter stats", sizeof(*adapter->drv_stats),
                                   RTE_CACHE_LINE_SIZE);
  ...
  rte_intr_callback_register(intr_handle, ena_interrupt_handler_rte, eth_dev);
  rte_intr_enable(intr_handle);
...
static uint16_t eth_ena_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  for (completed = 0; completed < nb_pkts; completed++) {
    ...
    rc = ena_com_rx_pkt(rx_ring->ena_com_io_cq, rx_ring->ena_com_io_sq,
                        &ena_rx_ctx);
    if (unlikely(rc)) {
      PMD_RX_LOG(ERR, "Failed to get the packet from the device, rc: %d\n", rc);
...
static uint16_t eth_ena_prep_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (i = 0; i != nb_pkts; i++) {
    ...
    if ((ol_flags & ENA_TX_OFFLOAD_NOTSUP_MASK) ||
      ...
      PMD_TX_LOG(DEBUG,
                 "mbuf[%" PRIu32
                 "] has unsupported offloads flags set: 0x%" PRIu64 "\n",
                 i, ol_flags);
      rte_errno = ENOTSUP;
      return i;
    }
    ...
    if ((ol_flags & RTE_MBUF_F_TX_IP_CKSUM &&
      ...
      PMD_TX_LOG(DEBUG,
                 "mbuf[%" PRIu32 "]: requested offloads: %" PRIu16
                 " are not enabled for the queue[%u]\n",
    ...
    if (unlikely(ol_flags & (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_L4_MASK) &&
      ...
      PMD_TX_LOG(
          DEBUG,
          "mbuf[%" PRIu32
          "]: l2_len or l3_len values are 0 while the offload was requested\n",
          i);
      rte_errno = EINVAL;
      return i;
    }
    ...
    if (ol_flags & RTE_MBUF_F_TX_IPV4) {
      ...
      if (l4_csum_flag != RTE_MBUF_F_TX_L4_NO_CKSUM &&
        ...
        if (dev_offload_capa & ENA_L4_IPV4_CSUM_PARTIAL) {
      ...
      if (l4_csum_flag != RTE_MBUF_F_TX_L4_NO_CKSUM &&
        ...
        if (dev_offload_capa & ENA_L4_IPV6_CSUM_PARTIAL) {
          need_pseudo_csum = true;
        } else {
          rte_errno = ENOTSUP;
          return i;
...
static void ena_tx_map_mbuf(struct ena_ring *tx_ring,
  ...
  if (seg_len > push_len) {
    ena_buf->paddr = mbuf->buf_iova + mbuf->data_off + push_len;
...
static uint16_t eth_ena_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  if (unlikely(tx_ring->adapter->state != ENA_ADAPTER_STATE_RUNNING)) {
    PMD_TX_LOG(ALERT, "Trying to xmit pkts while device is NOT running\n");
    return 0;
  }
#endif
...
int ena_copy_eni_stats(struct ena_adapter *adapter,
  ...
  if (rc != 0) {
    if (rc == ENA_COM_UNSUPPORTED) {
      PMD_DRV_LOG(DEBUG, "Retrieving ENI metrics is not supported\n");
    } else {
      PMD_DRV_LOG(WARNING, "Failed to get ENI metrics, rc: %d\n", rc);
    }
    return rc;
  }
...
static int ena_xstats_get_names(struct rte_eth_dev *dev,
  ...
  for (stat = 0; stat < ENA_STATS_ARRAY_GLOBAL; stat++, count++)
    strcpy(xstats_names[count].name, ena_stats_global_strings[stat].name);

  for (stat = 0; stat < ENA_STATS_ARRAY_ENI; stat++, count++)
    strcpy(xstats_names[count].name, ena_stats_eni_strings[stat].name);
  ...
  for (stat = 0; stat < ENA_STATS_ARRAY_RX; stat++)
    for (i = 0; i < dev->data->nb_rx_queues; i++, count++)
      snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
               "rx_q%d_%s", i, ena_stats_rx_strings[stat].name);

  for (stat = 0; stat < ENA_STATS_ARRAY_TX; stat++)
    for (i = 0; i < dev->data->nb_tx_queues; i++, count++)
      snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
               "tx_q%d_%s", i, ena_stats_tx_strings[stat].name);
...
static int ena_xstats_get_names_by_id(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < size; ++i) {
    ...
    if (id < ENA_STATS_ARRAY_GLOBAL) {
      strcpy(xstats_names[i].name, ena_stats_global_strings[id].name);
      continue;
    }

    id -= ENA_STATS_ARRAY_GLOBAL;
    if (id < ENA_STATS_ARRAY_ENI) {
      strcpy(xstats_names[i].name, ena_stats_eni_strings[id].name);
      continue;
    }
    ...
    if (id < ENA_STATS_ARRAY_RX) {
      ...
      snprintf(xstats_names[i].name, sizeof(xstats_names[i].name),
               "rx_q%" PRIu64 "d_%s", qid, ena_stats_rx_strings[id].name);
      continue;
    }
    ...
    if (id < ENA_STATS_ARRAY_TX) {
      ...
      snprintf(xstats_names[i].name, sizeof(xstats_names[i].name),
               "tx_q%" PRIu64 "_%s", qid, ena_stats_tx_strings[id].name);
      continue;
    }
  }
 ...
 */
static int ena_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats,
                          unsigned int n) {
  struct ena_adapter *adapter = dev->data->dev_private;
  unsigned int xstats_count = ena_xstats_calc_num(dev->data);
  ...
  for (stat = 0; stat < ENA_STATS_ARRAY_GLOBAL; stat++, count++) {
    ...
    xstats[count].value = *((uint64_t *)((char *)stats_begin + stat_offset));
  }
...
static int ena_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (id < rx_entries) {
      qid = id % dev->data->nb_rx_queues;
      id /= dev->data->nb_rx_queues;
      values[i] = *((uint64_t *)&adapter->rx_ring[qid].rx_stats + id);
      ++valid;
      continue;
    }
    /* Check if id belongs to rx queue statistics */
    ...
    if (id < tx_entries) {
      qid = id % dev->data->nb_tx_queues;
      id /= dev->data->nb_tx_queues;
      values[i] = *((uint64_t *)&adapter->tx_ring[qid].tx_stats + id);
      ++valid;
      continue;
    }
  }
...
static int ena_process_uint_devarg(const char *key, const char *value,
                                   void *opaque) {
  ...
  if (strcmp(key, ENA_DEVARG_MISS_TXC_TO) == 0) {
    if (uint_value > ENA_MAX_TX_TIMEOUT_SECONDS) {
      PMD_INIT_LOG(ERR,
                   "Tx timeout too high: %" PRIu64
                   " sec. Maximum allowed: %d sec.\n",
      ...
      PMD_INIT_LOG(INFO,
                   ...
                   uint_value);
      adapter->missing_tx_completion_to = uint_value * rte_get_timer_hz();
    }
  }
...
static int ena_setup_rx_intr(struct rte_eth_dev *dev) {
  ...
  if (vectors_nb > RTE_MAX_RXTX_INTR_VEC_ID) {
    PMD_DRV_LOG(ERR, "Too many Rx interrupts requested, maximum number: %d\n",
  ...
  if (rte_intr_vec_list_alloc(intr_handle, "intr_vec",
    ...
    PMD_DRV_LOG(ERR, "Failed to allocate interrupt vector for %d queues\n",
                dev->data->nb_rx_queues);
    rc = -ENOMEM;
    goto enable_intr;
  }
  ...
  if (!rte_intr_allow_others(intr_handle)) {
    PMD_DRV_LOG(ERR, "Not enough interrupts available to use both ENA Admin "
                     "and Rx interrupts\n");
...
static void ena_rx_queue_intr_set(struct rte_eth_dev *dev, uint16_t queue_id,
                                  bool unmask) {
  struct ena_adapter *adapter = dev->data->dev_private;
...
static int ena_configure_aenq(struct ena_adapter *adapter) {
  ...
  if (adapter->edev_data->dev_conf.intr_conf.lsc != 0) {
    if (!(aenq_groups & BIT(ENA_ADMIN_LINK_CHANGE))) {
      PMD_DRV_LOG(ERR, "LSC requested, but it's not supported by the AENQ\n");
      return -EINVAL;
    }
  } else {
...
const struct rte_memzone *ena_mem_alloc_coherent(struct rte_eth_dev_data *data,
                                                 size_t size, int socket_id,
...
static int eth_ena_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                             struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct ena_adapter),
                                       eth_ena_dev_init);
}
...
unimplemented_aenq_handler(__rte_unused void *data,
                           __rte_unused struct ena_admin_aenq_entry *aenq_e) {
  PMD_DRV_LOG(
      ERR, "Unknown event was received or event with unimplemented handler\n");
}

static struct ena_aenq_handlers aenq_handlers = {
    .handlers = {[ENA_ADMIN_LINK_CHANGE] = ena_update_on_link_change,
...
static int ena_mp_primary_handle(const struct rte_mp_msg *mp_msg,
                                 const void *peer) {
  const struct ena_mp_body *req = (const struct ena_mp_body *)mp_msg->param;
  struct ena_adapter *adapter;
  struct ena_com_dev *ena_dev;
  ...
  case ENA_MP_DEV_STATS_GET:
    res = ena_com_get_dev_basic_stats(ena_dev, &adapter->basic_stats);
    break;
  case ENA_MP_ENI_STATS_GET:
    res = ena_com_get_eni_stats(
        ena_dev, (struct ena_admin_eni_stats *)&adapter->eni_stats);
    break;
  case ENA_MP_MTU_SET:
    res = ena_com_set_dev_mtu(ena_dev, req->args.mtu);
    break;
  case ENA_MP_IND_TBL_GET:
    res = ena_com_indirect_table_get(ena_dev, adapter->indirect_table);
    break;
