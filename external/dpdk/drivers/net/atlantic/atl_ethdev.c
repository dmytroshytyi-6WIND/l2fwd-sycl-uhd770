...
/* VLAN stuff */
static int atl_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                               int on);

static int atl_vlan_offload_set(struct rte_eth_dev *dev, int mask);

static void atl_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue_id,
                                     int on);
...
#define ATL_RX_OFFLOADS                                                        \
  (RTE_ETH_RX_OFFLOAD_VLAN_STRIP | RTE_ETH_RX_OFFLOAD_IPV4_CKSUM |             \
   RTE_ETH_RX_OFFLOAD_UDP_CKSUM | RTE_ETH_RX_OFFLOAD_TCP_CKSUM |               \
   RTE_ETH_RX_OFFLOAD_MACSEC_STRIP | RTE_ETH_RX_OFFLOAD_VLAN_FILTER)

#define ATL_TX_OFFLOADS                                                        \
  (RTE_ETH_TX_OFFLOAD_VLAN_INSERT | RTE_ETH_TX_OFFLOAD_IPV4_CKSUM |            \
   RTE_ETH_TX_OFFLOAD_UDP_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_CKSUM |               \
   RTE_ETH_TX_OFFLOAD_TCP_TSO | RTE_ETH_TX_OFFLOAD_MACSEC_INSERT |             \
...
#define ATL_XSTATS_FIELD(name)                                                 \
  { #name, offsetof(struct aq_stats_s, name), XSTATS_TYPE_MSM }

#define ATL_MACSEC_XSTATS_FIELD(name)                                          \
  { #name, offsetof(struct macsec_stats, name), XSTATS_TYPE_MACSEC }
...
static inline int32_t atl_reset_hw(struct aq_hw_s *hw) {
  return hw_atl_b0_hw_reset(hw);
}

static inline void atl_enable_intr(struct rte_eth_dev *dev) {
  struct aq_hw_s *hw = ATL_DEV_PRIVATE_TO_HW(dev->data->dev_private);

  hw_atl_itr_irq_msk_setlsw_set(hw, 0xffffffff);
}

static void atl_disable_intr(struct aq_hw_s *hw) {
  PMD_INIT_FUNC_TRACE();
...
static int eth_atl_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  adapter->hw_cfg.link_speed_msk = AQ_NIC_RATE_10G | AQ_NIC_RATE_5G |
  ...
  if (hw->aq_fw_ops->get_mac_permanent(
          hw, eth_dev->data->mac_addrs->addr_bytes) != 0)
    return -EINVAL;

  /* Reset the hw statistics */
  atl_dev_stats_reset(eth_dev);

  rte_intr_callback_register(intr_handle, atl_dev_interrupt_handler, eth_dev);
...
static int eth_atl_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                             struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct atl_adapter),
                                       eth_atl_dev_init);
}
...
static int atl_dev_start(struct rte_eth_dev *dev) {
  ...
  PMD_INIT_LOG(DEBUG, "FW version: %u.%u.%u", hw->fw_ver_actual >> 24,
               (hw->fw_ver_actual >> 16) & 0xFF, hw->fw_ver_actual & 0xFFFF);
  ...
  if (rte_intr_allow_others(intr_handle)) {
    ...
    rte_intr_callback_unregister(intr_handle, atl_dev_interrupt_handler, dev);
    if (dev->data->dev_conf.intr_conf.lsc != 0)
...
static int atl_dev_stop(struct rte_eth_dev *dev) {
  ...
  if (!rte_intr_allow_others(intr_handle))
    ...
    rte_intr_callback_register(intr_handle, atl_dev_interrupt_handler,
                               (void *)dev);
...
static int atl_dev_configure_macsec(struct rte_eth_dev *dev) {
  struct aq_hw_s *hw = ATL_DEV_PRIVATE_TO_HW(dev->data->dev_private);
   ...
   */
  uint32_t sci_hi_part =
      (msg_macsec.txsc.mac_sa[1] << 16) | (msg_macsec.txsc.mac_sa[0] >> 16);
...
int atl_macsec_enable(struct rte_eth_dev *dev, uint8_t encr,
                      uint8_t repl_prot) {
...
int atl_macsec_config_txsc(struct rte_eth_dev *dev, uint8_t *mac) {
  ...
  memcpy((uint8_t *)&cfg->aq_macsec.txsc.mac + 2, mac, RTE_ETHER_ADDR_LEN);

  return 0;
}

int atl_macsec_config_rxsc(struct rte_eth_dev *dev, uint8_t *mac, uint16_t pi) {
  struct aq_hw_cfg_s *cfg = ATL_DEV_PRIVATE_TO_CFG(dev->data->dev_private);

  memset(&cfg->aq_macsec.rxsc.mac, 0, sizeof(cfg->aq_macsec.rxsc.mac));
  memcpy((uint8_t *)&cfg->aq_macsec.rxsc.mac + 2, mac, RTE_ETHER_ADDR_LEN);
  cfg->aq_macsec.rxsc.pi = pi;

  return 0;
}

int atl_macsec_select_txsa(struct rte_eth_dev *dev, uint8_t idx, uint8_t an,
...
int atl_macsec_select_rxsa(struct rte_eth_dev *dev, uint8_t idx, uint8_t an,
                           uint32_t pn, uint8_t *key) {
  struct aq_hw_cfg_s *cfg = ATL_DEV_PRIVATE_TO_CFG(dev->data->dev_private);
...
static int atl_dev_xstats_get_count(struct rte_eth_dev *dev) {
...
static int atl_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < n && i < count; i++) {
    ...
    case XSTATS_TYPE_MSM:
      stats[i].value =
          *(u64 *)((uint8_t *)&hw->curr_stats + atl_xstats_tbl[i].offset);
      break;
    case XSTATS_TYPE_MACSEC:
      if (!err) {
        stats[i].value =
            *(u64 *)((uint8_t *)&resp.stats + atl_xstats_tbl[i].offset);
      }
      break;
    }
  }
...
static const uint32_t *atl_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {
      RTE_PTYPE_L2_ETHER, RTE_PTYPE_L2_ETHER_ARP, RTE_PTYPE_L2_ETHER_VLAN,
...
static int atl_dev_link_update(struct rte_eth_dev *dev, int wait __rte_unused) {
  struct aq_hw_s *hw = ATL_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  link.link_autoneg =
      hw->is_autoneg ? RTE_ETH_LINK_AUTONEG : RTE_ETH_LINK_FIXED;
...
static int atl_dev_lsc_interrupt_setup(struct rte_eth_dev *dev,
                                       uint8_t on __rte_unused) {
  atl_dev_link_status_print(dev);
  return 0;
}

static int atl_dev_rxq_interrupt_setup(struct rte_eth_dev *dev __rte_unused) {
  return 0;
}
...
static void atl_dev_link_status_print(struct rte_eth_dev *dev) {
  ...
  if (link.link_status) {
    ...
    PMD_DRV_LOG(INFO, " Port %d: Link Down", (int)(dev->data->port_id));
  }
...
static int atl_dev_interrupt_action(struct rte_eth_dev *dev,
  ...
  if (!atl_dev_link_update(dev, 0)) {
    ...
    if (resp.stats.egress_threshold_expired ||
        resp.stats.ingress_threshold_expired || resp.stats.egress_expired ||
...
static int atl_dev_get_eeprom_length(struct rte_eth_dev *dev __rte_unused) {
...
int atl_dev_get_eeprom(struct rte_eth_dev *dev,
  ...
  if (eeprom->length + eeprom->offset > SFP_EEPROM_SIZE || eeprom->data == NULL)
    return -EINVAL;
...
static int atl_update_mac_addr(struct rte_eth_dev *dev, uint32_t index,
...
static int atl_enable_vlan_filter(struct rte_eth_dev *dev, int en) {
  struct aq_hw_s *hw = ATL_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct aq_hw_cfg_s *cfg = ATL_DEV_PRIVATE_TO_CFG(dev->data->dev_private);
  int i;
...
static int atl_reta_update(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size && i < cf->aq_rss.indirection_table_size; i++)
    cf->aq_rss.indirection_table[i] =
        min(reta_conf->reta[i], dev->data->nb_rx_queues - 1);
...
static int atl_rss_hash_update(struct rte_eth_dev *dev,
  ...
  struct aq_hw_s *hw = ATL_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct aq_hw_cfg_s *cfg = ATL_DEV_PRIVATE_TO_CFG(dev->data->dev_private);
  static u8 def_rss_key[40] = {0x1e, 0xad, 0x71, 0x87, 0x65, 0xfc, 0x26, 0x7d,
  ...
  if (rss_conf->rss_key) {
    ...
    memcpy(cfg->aq_rss.hash_secret_key, def_rss_key, sizeof(def_rss_key));
