...
static int hw_atl_utils_soft_reset_flb(struct aq_hw_s *self) {
  ...
  for (k = 0; k < 1000; k++) {
    u32 flb_status = aq_hw_read_reg(self, HW_ATL_MPI_DAISY_CHAIN_STATUS);
...
static int hw_atl_utils_soft_reset_rbl(struct aq_hw_s *self) {
  ...
  aq_hw_write_reg_bit(self, HW_ATL_MAC_PHY_CONTROL,
                      ...
                      HW_ATL_MAC_PHY_MPI_RESET_BIT, 0x0);
  gsr = aq_hw_read_reg(self, HW_ATL_GLB_SOFT_RES_ADR);
  aq_hw_write_reg(self, HW_ATL_GLB_SOFT_RES_ADR, (gsr & 0xFFFFBFFF) | 0x8000);
...
int hw_atl_utils_soft_reset(struct aq_hw_s *self) {
  ...
  for (k = 0; k < 1000; ++k) {
    u32 flb_status = aq_hw_read_reg(self, HW_ATL_MPI_DAISY_CHAIN_STATUS);
    boot_exit_code = aq_hw_read_reg(self, HW_ATL_MPI_BOOT_EXIT_CODE);
  ...
  if (!hw_atl_utils_ver_match(HW_ATL_FW_VER_1X,
                              aq_hw_read_reg(self, HW_ATL_MPI_FW_VERSION))) {
...
int hw_atl_utils_fw_downld_dwords(struct aq_hw_s *self, u32 a, u32 *p,
                                  u32 cnt) {
  int err = 0;

  AQ_HW_WAIT_FOR(hw_atl_reg_glb_cpu_sem_get(self, HW_ATL_FW_SM_RAM) == 1U, 1U,
                 10000U);
  ...
  for (++cnt; --cnt && !err;) {
    ...
    if (IS_CHIP_FEATURE(REVISION_B1))
      AQ_HW_WAIT_FOR(a != aq_hw_read_reg(self, HW_ATL_MIF_ADDR), 1, 1000U);
    else
      AQ_HW_WAIT_FOR(!(0x100 & aq_hw_read_reg(self, HW_ATL_MIF_CMD)), 1, 1000U);
    if (err) {
      err = -ETIMEDOUT;
      goto err_exit;
    }
...
int hw_atl_utils_fw_upload_dwords(struct aq_hw_s *self, u32 a, u32 *p,
  ...
  if (IS_CHIP_FEATURE(REVISION_B1)) {
    ...
    for (; data_offset < cnt; ++mbox_offset, ++data_offset) {
      ...
      aq_hw_write_reg(self, 0x32C, (0x80000000 | (0xFFFF & (mbox_offset * 4))));
      hw_atl_mcp_up_force_intr_set(self, 1);
      /* 1000 times by 10us = 10ms */
      AQ_HW_WAIT_FOR((aq_hw_read_reg(self, 0x32C) & 0xF0000000) != 0x80000000,
                     10, 1000);
    }
  } else {
    u32 offset = 0;
    ...
    for (; offset < cnt; ++offset) {
      ...
      AQ_HW_WAIT_FOR((aq_hw_read_reg(self, 0x200U) & 0x100) == 0, 10, 1000);
    }
  }
...
static int hw_atl_utils_init_ucp(struct aq_hw_s *self) {
  ...
  AQ_HW_WAIT_FOR(0U != (self->mbox_addr = aq_hw_read_reg(self, 0x360U)), 1000U,
                 10U);
  AQ_HW_WAIT_FOR(0U != (self->rpc_addr = aq_hw_read_reg(self, 0x334U)), 1000U,
                 100U);
...
int hw_atl_utils_fw_rpc_call(struct aq_hw_s *self, unsigned int rpc_size) {
  ...
  err = hw_atl_utils_fw_upload_dwords(
      self, self->rpc_addr, (u32 *)(void *)&self->rpc,
...
int hw_atl_utils_fw_rpc_wait(struct aq_hw_s *self,
  ...
  do {
    ...
    AQ_HW_WAIT_FOR(
        sw.tid == (fw.val = aq_hw_read_reg(self, HW_ATL_RPC_STATE_ADR), fw.tid),
        1000U, 100U);
    if (err < 0)
      goto err_exit;
  ...
  if (rpc) {
    if (fw.len) {
      err = hw_atl_utils_fw_downld_dwords(
          self, self->rpc_addr, (u32 *)(void *)&self->rpc,
          (fw.len + sizeof(u32) - sizeof(u8)) / sizeof(u32));
      if (err < 0)
        goto err_exit;
    }
...
void hw_atl_utils_mpi_read_stats(struct aq_hw_s *self,
  ...
  err =
      hw_atl_utils_fw_downld_dwords(self, self->mbox_addr, (u32 *)(void *)pmbox,
                                    sizeof(*pmbox) / sizeof(u32));
  if (err < 0)
    goto err_exit;
...
int hw_atl_utils_mpi_set_state(struct aq_hw_s *self,
  ...
  if (state == MPI_RESET) {
    ...
    AQ_HW_WAIT_FOR(transaction_id != (hw_atl_utils_mpi_read_mbox(self, &mbox),
                                      mbox.transaction_id),
                   1000U, 100U);
    if (err < 0)
      goto err_exit;
...
static int hw_atl_utils_get_mac_permanent(struct aq_hw_s *self, u8 *mac) {
  int err = 0;
  u32 h = 0U;
  u32 l = 0U;
  u32 mac_addr[2];
  ...
  err = hw_atl_utils_fw_downld_dwords(
      self, aq_hw_read_reg(self, 0x00000374U) + (40U * 4U), mac_addr,
      ARRAY_SIZE(mac_addr));
  if (err < 0) {
    mac_addr[0] = 0U;
    mac_addr[1] = 0U;
  ...
  if ((mac[0] & 0x01U) || ((mac[0] | mac[1] | mac[2]) == 0x00U)) {
    /* chip revision */
    l = 0xE3000000U | (0xFFFFU & aq_hw_read_reg(self, HW_ATL_UCP_0X370_REG)) |
        (0x00 << 16);
    h = 0x8001300EU;
...
void hw_atl_utils_hw_chip_features_init(struct aq_hw_s *self, u32 *p) {
  ...
  if ((0xFU & mif_rev) == 1U) {
    ...
    chip_features |=
        ...
        HAL_ATLANTIC_UTILS_CHIP_MIPS | HAL_ATLANTIC_UTILS_CHIP_TPO2 |
        HAL_ATLANTIC_UTILS_CHIP_RPF2;
  } else if ((0xFU & mif_rev) == 0xAU) {
    chip_features |=
        HAL_ATLANTIC_UTILS_CHIP_REVISION_B1 | HAL_ATLANTIC_UTILS_CHIP_MPI_AQ |
        HAL_ATLANTIC_UTILS_CHIP_MIPS | HAL_ATLANTIC_UTILS_CHIP_TPO2 |
        HAL_ATLANTIC_UTILS_CHIP_RPF2;
  }
...
#define AQ_SDELTA(_N_)                                                         \
  (self->curr_stats._N_ += mbox.stats._N_ - self->last_stats._N_)
...
static const u32 hw_atl_utils_hw_mac_regs[] = {
    0x00005580U, 0x00005590U, 0x000055B0U, 0x000055B4U, 0x000055C0U,
    0x00005B00U, 0x00005B04U, 0x00005B08U, 0x00005B0CU, 0x00005B10U,
    0x00005B14U, 0x00005B18U, 0x00005B1CU, 0x00005B20U, 0x00005B24U,
    0x00005B28U, 0x00005B2CU, 0x00005B30U, 0x00005B34U, 0x00005B38U,
    0x00005B3CU, 0x00005B40U, 0x00005B44U, 0x00005B48U, 0x00005B4CU,
    0x00005B50U, 0x00005B54U, 0x00005B58U, 0x00005B5CU, 0x00005B60U,
    0x00005B64U, 0x00005B68U, 0x00005B6CU, 0x00005B70U, 0x00005B74U,
    0x00005B78U, 0x00005B7CU, 0x00007C00U, 0x00007C04U, 0x00007C08U,
    0x00007C0CU, 0x00007C10U, 0x00007C14U, 0x00007C18U, 0x00007C1CU,
    0x00007C20U, 0x00007C40U, 0x00007C44U, 0x00007C48U, 0x00007C4CU,
    0x00007C50U, 0x00007C54U, 0x00007C58U, 0x00007C5CU, 0x00007C60U,
    0x00007C80U, 0x00007C84U, 0x00007C88U, 0x00007C8CU, 0x00007C90U,
    0x00007C94U, 0x00007C98U, 0x00007C9CU, 0x00007CA0U, 0x00007CC0U,
    0x00007CC4U, 0x00007CC8U, 0x00007CCCU, 0x00007CD0U, 0x00007CD4U,
    0x00007CD8U, 0x00007CDCU, 0x00007CE0U, 0x00000300U, 0x00000304U,
    0x00000308U, 0x0000030cU, 0x00000310U, 0x00000314U, 0x00000318U,
    0x0000031cU, 0x00000360U, 0x00000364U, 0x00000368U, 0x0000036cU,
    0x00000370U, 0x00000374U, 0x00006900U,
};
...
int hw_atl_utils_hw_get_regs(struct aq_hw_s *self, u32 *regs_buff) {
  unsigned int i = 0U;
  unsigned int mac_regs_count = hw_atl_utils_hw_get_reg_length();

  for (i = 0; i < mac_regs_count; i++)
    regs_buff[i] = aq_hw_read_reg(self, hw_atl_utils_hw_mac_regs[i]);
  return 0;
}
...
static int aq_fw1x_set_power(struct aq_hw_s *self,
                             unsigned int power_state __rte_unused, u8 *mac) {
  struct hw_aq_atl_utils_fw_rpc *prpc = NULL;
  unsigned int rpc_size = 0U;
  int err = 0;
  if (self->aq_nic_cfg->wol & AQ_NIC_WOL_ENABLED) {
    ...
    rpc_size = sizeof(prpc->msg_id) + sizeof(prpc->msg_enable_wakeup);
