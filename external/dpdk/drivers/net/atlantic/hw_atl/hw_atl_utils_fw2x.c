...
static int aq_fw2x_init(struct aq_hw_s *self) {
  ...
  AQ_HW_WAIT_FOR(
      0U != (self->mbox_addr = aq_hw_read_reg(self, HW_ATL_FW2X_MPI_MBOX_ADDR)),
      1000U, 10U);
  AQ_HW_WAIT_FOR(
      0U != (self->rpc_addr = aq_hw_read_reg(self, HW_ATL_FW2X_MPI_RPC_ADDR)),
      1000U, 100U);
...
static int aq_fw2x_set_link_speed(struct aq_hw_s *self, u32 speed) {
  ...
  u32 val = rate_mask | ((BIT(CAPS_LO_SMBUS_READ) | BIT(CAPS_LO_SMBUS_WRITE) |
                          BIT(CAPS_LO_MACSEC)) &
                         reg_val);
...
static int aq_fw2x_update_link_status(struct aq_hw_s *self) {
  ...
  u32 speed = mpi_state & (FW2X_RATE_100M | FW2X_RATE_1G | FW2X_RATE_2G5 |
...
static int aq_fw2x_get_mac_permanent(struct aq_hw_s *self, u8 *mac) {
  ...
  if (efuse_addr != 0) {
    err = hw_atl_utils_fw_downld_dwords(self, efuse_addr + (40U * 4U), mac_addr,
                                        ARRAY_SIZE(mac_addr));
    if (err)
      goto exit;
...
static int aq_fw2x_get_temp(struct aq_hw_s *self, int *temp) {
  ...
  err = hw_atl_utils_fw_downld_dwords(
      self,
      self->mbox_addr + offsetof(struct hw_aq_atl_utils_mbox, info) +
          offsetof(struct hw_aq_info, phy_temperature),
      &temp_res, sizeof(temp_res) / sizeof(u32));
...
static int aq_fw2x_get_cable_len(struct aq_hw_s *self, int *cable_len) {
  ...
  err = hw_atl_utils_fw_downld_dwords(
      ...
      self->mbox_addr + offsetof(struct hw_aq_atl_utils_mbox, info) +
          offsetof(struct hw_aq_info, phy_temperature),
      &cable_len_res, sizeof(cable_len_res) / sizeof(u32));
...
static int aq_fw2x_set_wol_params(struct aq_hw_s *self, u8 *mac) {
  ...
  AQ_HW_WAIT_FOR(
      (aq_hw_read_reg(self, HW_ATL_FW2X_MPI_STATE2_ADDR) & HW_ATL_FW2X_CAP_WOL),
      1U, 10000U);
err_exit:
  return err;
}
...
static int aq_fw2x_set_eee_rate(struct aq_hw_s *self, u32 speed) {
  u32 mpi_opts = aq_hw_read_reg(self, HW_ATL_FW2X_MPI_CONTROL2_ADDR);
  mpi_opts &= ~(HW_ATL_FW2X_CAP_EEE_1G_MASK | HW_ATL_FW2X_CAP_EEE_2G5_MASK |
                HW_ATL_FW2X_CAP_EEE_5G_MASK | HW_ATL_FW2X_CAP_EEE_10G_MASK);
...
static int aq_fw2x_get_eee_rate(struct aq_hw_s *self, u32 *rate,
  ...
  err = hw_atl_utils_fw_downld_dwords(
      ...
      &caps_hi, sizeof(caps_hi) / sizeof(u32));
...
static int aq_fw2x_get_eeprom(struct aq_hw_s *self, int dev_addr, u32 *data,
                              u32 len, u32 offset) {
  ...
  err = hw_atl_utils_fw_downld_dwords(self, self->rpc_addr + sizeof(u32),
                                      &result, sizeof(result) / sizeof(u32));
  ...
  if (bytes_remains) {
    ...
    err = hw_atl_utils_fw_downld_dwords(
        self, self->rpc_addr + (sizeof(u32) * 2) + (num_dwords * sizeof(u32)),
        &val, 1);
...
static int aq_fw2x_set_eeprom(struct aq_hw_s *self, int dev_addr, u32 *data,
                              u32 len, u32 offset) {
  struct smbus_request request;
  ...
  if (bytes_remains) {
    ...
    rte_memcpy(&val, (u8 *)data + (sizeof(u32) * num_dwords), bytes_remains);

    err = hw_atl_utils_fw_upload_dwords(
        self, self->rpc_addr + sizeof(request) + (num_dwords * sizeof(u32)),
        &val, 1);
...
static int
aq_fw2x_send_macsec_request(struct aq_hw_s *self,
  ...
  err = hw_atl_utils_fw_upload_dwords(
      self, self->rpc_addr, (u32 *)(void *)req,
  ...
  /* Read status of write operation */
  err = hw_atl_utils_fw_downld_dwords(
      self, self->rpc_addr + sizeof(u32), (u32 *)(void *)response,
