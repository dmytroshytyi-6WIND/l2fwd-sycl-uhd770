...
static int hw_atl_b0_hw_qos_set(struct aq_hw_s *self) {
  ...
  hw_atl_tpb_tx_buff_hi_threshold_per_tc_set(
      self, (buff_size * (1024 / 32U) * 66U) / 100U, tc);
  hw_atl_tpb_tx_buff_lo_threshold_per_tc_set(
      self, (buff_size * (1024 / 32U) * 50U) / 100U, tc);
  ...
  hw_atl_rpb_rx_buff_hi_threshold_per_tc_set(
      self, (buff_size * (1024U / 32U) * 66U) / 100U, tc);
  hw_atl_rpb_rx_buff_lo_threshold_per_tc_set(
      self, (buff_size * (1024U / 32U) * 50U) / 100U, tc);
...
int hw_atl_b0_hw_rss_hash_set(struct aq_hw_s *self,
  ...
  for (i = 10, addr = 0U; i--; ++addr) {
    u32 key_data = cfg->is_rss ? htonl(rss_params->hash_secret_key[i]) : 0U;
    hw_atl_rpf_rss_key_wr_data_set(self, key_data);
    hw_atl_rpf_rss_key_addr_set(self, addr);
    hw_atl_rpf_rss_key_wr_en_set(self, 1U);
    AQ_HW_WAIT_FOR(hw_atl_rpf_rss_key_wr_en_get(self) == 0, 1000U, 10U);
    if (err < 0)
      goto err_exit;
  }
...
int hw_atl_b0_hw_rss_set(struct aq_hw_s *self,
  ...
  for (i = 0; i < HW_ATL_B0_RSS_REDIRECTION_MAX; i++) {
    ...
    AQ_HW_WAIT_FOR(hw_atl_rpf_rss_redir_wr_en_get(self) == 0, 1000U, 10U);
...
static int hw_atl_b0_hw_init_tx_path(struct aq_hw_s *self) {
  ...
  /* misc */
  aq_hw_write_reg(self, 0x00007040U,
                  IS_CHIP_FEATURE(TPO2) ? 0x00010000U : 0x00000000U);
...
int hw_atl_b0_hw_init(struct aq_hw_s *self, u8 *mac_addr) {
  ...
  aq_hw_write_reg(self, HW_ATL_PCI_REG_CONTROL6_ADR, (val & ~0x707) | 0x404);
  ...
  hw_atl_reg_irq_glb_ctl_set(
      self, aq_hw_atl_igcr_table_[aq_nic_cfg->irq_type]
...
int hw_atl_b0_hw_tx_ring_tail_update(struct aq_hw_s *self, int tail,
                                     int index) {
...
int hw_atl_b0_hw_ring_rx_init(struct aq_hw_s *self, uint64_t base_addr,
                              int index, int size, int buff_size, int cpu,
                              int vec) {
