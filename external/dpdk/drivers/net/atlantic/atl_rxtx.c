...
static inline void atl_reset_rx_queue(struct atl_rx_queue *rxq) {
...
int atl_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
  ...
  if (nb_rx_desc < AQ_HW_MIN_RX_RING_SIZE ||
    ...
    PMD_INIT_LOG(ERR,
                 "Number of Rx descriptors must be "
                 "less than or equal to %d, "
                 "greater than or equal to %d",
                 AQ_HW_MAX_RX_RING_SIZE, AQ_HW_MIN_RX_RING_SIZE);
    return -EINVAL;
  }
  ...
  rxq->sw_ring = rte_zmalloc_socket("atlantic sw rx ring",
                                    ...
                                    RTE_CACHE_LINE_SIZE, socket_id);
  if (rxq->sw_ring == NULL) {
    PMD_INIT_LOG(ERR, "Port %d: Cannot allocate software ring for queue %d",
  ...
  mz = rte_eth_dma_zone_reserve(dev, "rx hw_ring", rx_queue_id,
                                ...
                                128, socket_id);
  if (mz == NULL) {
    PMD_INIT_LOG(ERR, "Port %d: Cannot allocate hardware ring for queue %d",
                 rxq->port_id, rxq->queue_id);
...
static inline void atl_reset_tx_queue(struct atl_tx_queue *txq) {
...
int atl_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id,
  ...
  if (nb_tx_desc < AQ_HW_MIN_TX_RING_SIZE ||
    ...
    PMD_INIT_LOG(ERR,
                 "Number of Tx descriptors must be "
                 "less than or equal to %d, "
                 "greater than or equal to %d",
                 AQ_HW_MAX_TX_RING_SIZE, AQ_HW_MIN_TX_RING_SIZE);
    return -EINVAL;
  }
...
int atl_tx_init(struct rte_eth_dev *eth_dev) {
  ...
  for (i = 0; i < eth_dev->data->nb_tx_queues; i++) {
    ...
    if (err) {
      PMD_INIT_LOG(ERR, "Port %d: Cannot init TX queue %d", txq->port_id,
                   txq->queue_id);
      break;
    }
  }
...
int atl_rx_init(struct rte_eth_dev *eth_dev) {
  struct aq_hw_s *hw = ATL_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  ...
  for (i = 0; i < eth_dev->data->nb_rx_queues; i++) {
    ...
    if (buff_size > HW_ATL_B0_RXD_BUF_SIZE_MAX) {
      PMD_INIT_LOG(WARNING, "Port %d queue %d: mem pool buff size is too big\n",
                   rxq->port_id, rxq->queue_id);
      buff_size = HW_ATL_B0_RXD_BUF_SIZE_MAX;
    }
    if (buff_size < 1024) {
      PMD_INIT_LOG(ERR, "Port %d queue %d: mem pool buff size is too small\n",
                   rxq->port_id, rxq->queue_id);
      return -EINVAL;
    }
    ...
    err =
        hw_atl_b0_hw_ring_rx_init(hw, base_addr, rxq->queue_id, rxq->nb_rx_desc,
                                  buff_size, 0, rxq->port_id);

    if (err) {
      PMD_INIT_LOG(ERR, "Port %d: Cannot init RX queue %d", rxq->port_id,
                   rxq->queue_id);
      break;
    }
  }
...
static int atl_alloc_rx_queue_mbufs(struct atl_rx_queue *rxq) {
  ...
  for (i = 0; i < rxq->nb_rx_desc; i++) {
    ...
    if (mbuf == NULL) {
      PMD_INIT_LOG(ERR, "Port %d: mbuf alloc failed for rx queue %d",
                   rxq->port_id, rxq->queue_id);
      return -ENOMEM;
    }
...
static void atl_rx_queue_release_mbufs(struct atl_rx_queue *rxq) {
  int i;
...
int atl_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  ...
  if (rx_queue_id < dev->data->nb_rx_queues) {
    ...
    if (atl_alloc_rx_queue_mbufs(rxq) != 0) {
      PMD_INIT_LOG(ERR, "Port %d: Allocate mbufs for queue %d failed",
                   rxq->port_id, rxq->queue_id);
      return -1;
    }
...
static void atl_tx_queue_release_mbufs(struct atl_tx_queue *txq) {
  int i;
...
int atl_start_queues(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    if (atl_tx_queue_start(dev, i) != 0) {
      PMD_DRV_LOG(ERR, "Port %d: Start Tx queue %d failed", dev->data->port_id,
                  i);
      return -1;
    }
  }
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    if (atl_rx_queue_start(dev, i) != 0) {
      PMD_DRV_LOG(ERR, "Port %d: Start Rx queue %d failed", dev->data->port_id,
                  i);
      return -1;
    }
  }
...
int atl_stop_queues(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    if (atl_tx_queue_stop(dev, i) != 0) {
      PMD_DRV_LOG(ERR, "Port %d: Stop Tx queue %d failed", dev->data->port_id,
                  i);
      return -1;
    }
  }
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    if (atl_rx_queue_stop(dev, i) != 0) {
      PMD_DRV_LOG(ERR, "Port %d: Stop Rx queue %d failed", dev->data->port_id,
                  i);
      return -1;
    }
  }
...
static int atl_rx_enable_intr(struct rte_eth_dev *dev, uint16_t queue_id,
                              bool enable) {
  struct aq_hw_s *hw = ATL_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct atl_rx_queue *rxq;
...
uint16_t atl_prep_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts,
                       uint16_t nb_pkts) {
  int i, ret;
  uint64_t ol_flags;
  struct rte_mbuf *m;
...
static uint64_t atl_desc_to_offload_flags(struct atl_rx_queue *rxq,
...
static uint32_t atl_desc_to_pkt_type(struct hw_atl_rxd_wb_s *rxd_wb) {
...
uint16_t atl_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_rx < nb_pkts) {
    ...
    PMD_RX_LOG(DEBUG,
               ...
               (unsigned int)rte_le_to_cpu_16(rxd_wb.pkt_len), rxd_wb.rss_hash,
               rxd_wb.rss_type);
    ...
    while (true) {
      ...
      if (rxd_wb.eop) {
        ...
        rx_mbuf->data_len = pkt_len > rxq->buff_size ? rxq->buff_size : pkt_len;
      }
      ...
      if (rx_mbuf->packet_type & RTE_PTYPE_L2_ETHER_VLAN) {
        ...
        if (cfg->vlan_strip)
          rx_mbuf->ol_flags |= RTE_MBUF_F_RX_VLAN_STRIPPED;
      }
    ...
    PMD_RX_LOG(DEBUG, "add mbuf segs=%d pkt_len=%d", rx_mbuf_first->nb_segs,
...
err_stop:
  ...
  if (nb_hold > rxq->rx_free_thresh) {
    PMD_RX_LOG(DEBUG,
               ...
               "nb_hold=%u nb_rx=%u",
               (unsigned int)rxq->port_id, (unsigned int)rxq->queue_id,
               (unsigned int)tail, (unsigned int)nb_hold, (unsigned int)nb_rx);
...
static void atl_xmit_cleanup(struct atl_tx_queue *txq) {
  struct atl_tx_entry *sw_ring;
  struct hw_atl_txd_s *txd;
  int to_clean = 0;
...
static int atl_tso_setup(struct rte_mbuf *tx_pkt, union hw_atl_txc_s *txc) {
  uint32_t tx_cmd = 0;
...
static inline void atl_setup_csum_offload(struct rte_mbuf *mbuf,
                                          struct hw_atl_txd_s *txd,
                                          uint32_t tx_cmd) {
...
static inline void atl_xmit_pkt(struct aq_hw_s *hw, struct atl_tx_queue *txq,
...
uint16_t atl_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  PMD_TX_LOG(DEBUG, "port %d txq %d pkts: %d tx_free=%d tx_tail=%d tx_head=%d",
             txq->port_id, txq->queue_id, nb_pkts, txq->tx_free, txq->tx_tail,
             txq->tx_head);
