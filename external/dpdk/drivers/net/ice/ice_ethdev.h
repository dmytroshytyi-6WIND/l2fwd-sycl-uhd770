...
#define ICE_FLAG_ALL                                                           \
  (ICE_FLAG_RSS | ICE_FLAG_DCB | ICE_FLAG_VMDQ | ICE_FLAG_SRIOV |              \
   ICE_FLAG_HEADER_SPLIT_DISABLED | ICE_FLAG_HEADER_SPLIT_ENABLED |            \
   ICE_FLAG_FDIR | ICE_FLAG_VXLAN | ICE_FLAG_RSS_AQ_CAPABLE |                  \
...
#define ICE_VLAN(tpid, vid) ((struct ice_vlan){tpid, vid})
...
#define ICE_DEV_TO_PCI(eth_dev) RTE_DEV_TO_PCI((eth_dev)->device)

/* ICE_DEV_PRIVATE_TO */
#define ICE_DEV_PRIVATE_TO_PF(adapter) (&((struct ice_adapter *)adapter)->pf)
#define ICE_DEV_PRIVATE_TO_HW(adapter) (&((struct ice_adapter *)adapter)->hw)
#define ICE_DEV_PRIVATE_TO_ADAPTER(adapter) ((struct ice_adapter *)adapter)

/* ICE_VSI_TO */
#define ICE_VSI_TO_HW(vsi) (&(((struct ice_vsi *)vsi)->adapter->hw))
#define ICE_VSI_TO_PF(vsi) (&(((struct ice_vsi *)vsi)->adapter->pf))

/* ICE_PF_TO */
#define ICE_PF_TO_HW(pf) (&(((struct ice_pf *)pf)->adapter->hw))
#define ICE_PF_TO_ADAPTER(pf) ((struct ice_adapter *)(pf)->adapter)
#define ICE_PF_TO_ETH_DEV(pf) (((struct ice_pf *)pf)->adapter->eth_dev)
...
static inline int ice_align_floor(int n) {
  if (n == 0)
    return 0;
