...
static const char *const ice_valid_args[] = {ICE_SAFE_MODE_SUPPORT_ARG,
...
static void ice_init_proto_xtr(struct rte_eth_dev *dev) {
  ...
  if (unlikely(offset == -1)) {
    PMD_DRV_LOG(
        ERR, "Protocol extraction metadata is disabled in mbuf with error %d",
        -rte_errno);
    return;
  }
...
static int ice_parse_devargs(struct rte_eth_dev *dev) {
  ...
  ret = rte_kvargs_process(kvlist, ICE_PIPELINE_MODE_SUPPORT_ARG, &parse_bool,
...
static void ice_get_init_link_status(struct rte_eth_dev *dev) {
  ...
  ret = ice_aq_get_link_info(hw->port_info, enable_lse, &link_status, NULL);
  if (ret != ICE_SUCCESS) {
...
#define MAX_REPEAT_TIME 10 /* 1s (10 * 100ms) in total */
  ...
  do {
    ...
    status =
        ice_aq_get_link_info(hw->port_info, enable_lse, &link_status, NULL);
 ...
 */
static enum ice_status ice_force_phys_link_state(struct ice_hw *hw,
                                                 bool link_up) {
...
static int ice_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  PMD_DRV_LOG(DEBUG, "tx_errors:		%" PRIu64 "",
              ns->eth.tx_errors);
  ...
  PMD_DRV_LOG(DEBUG, "mac_local_faults:	%" PRIu64 "", ns->mac_local_faults);
