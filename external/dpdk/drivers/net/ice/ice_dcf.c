...
ice_dcf_send_cmd_req_no_irq(struct ice_dcf_hw *hw, enum virtchnl_ops op,
  ...
  return iavf_aq_send_msg_to_pf(&hw->avf, op, IAVF_SUCCESS, req_msg, req_msglen,
                                NULL);
}

static int ice_dcf_recv_cmd_rsp_no_irq(struct ice_dcf_hw *hw,
...
ice_dcf_vc_cmd_send(struct ice_dcf_hw *hw, struct dcf_virtchnl_cmd *cmd) {
  return iavf_aq_send_msg_to_pf(&hw->avf, cmd->v_op, IAVF_SUCCESS, cmd->req_msg,
                                cmd->req_msglen, NULL);
}
...
ice_dcf_aq_cmd_handle(struct ice_dcf_hw *hw, struct iavf_arq_event_info *info) {
  ...
  if (unlikely(aq_op != iavf_aqc_opc_send_msg_to_vf)) {
    PMD_DRV_LOG(ERR, "Request %u is not supported yet", aq_op);
    return;
  }
  ...
  if (v_op == VIRTCHNL_OP_EVENT) {
    if (hw->vc_event_msg_cb != NULL)
      hw->vc_event_msg_cb(hw, info->msg_buf, info->msg_len);
    return;
  }
  ...
  TAILQ_FOREACH(cmd, &hw->vc_cmd_queue, next) {
    if (cmd->v_op == v_op && cmd->pending) {
      cmd->v_ret = v_ret;
      cmd->rsp_msglen = RTE_MIN(info->msg_len, cmd->rsp_buflen);
      if (likely(cmd->rsp_msglen != 0))
        rte_memcpy(cmd->rsp_msgbuf, info->msg_buf, cmd->rsp_msglen);
...
static void ice_dcf_handle_virtchnl_msg(struct ice_dcf_hw *hw) {
...
static int ice_dcf_init_check_api_version(struct ice_dcf_hw *hw) {
...
#define ICE_CPF_VIRTCHNL_VERSION_MINOR_START 1
  ...
  err = ice_dcf_recv_cmd_rsp_no_irq(hw, VIRTCHNL_OP_VERSION, (uint8_t *)pver,
                                    sizeof(*pver), NULL);
  if (err) {
  ...
  PMD_INIT_LOG(DEBUG, "Peer PF API version: %u.%u", pver->major, pver->minor);
  ...
  if (pver->major < ICE_CPF_VIRTCHNL_VERSION_MAJOR_START ||
    ...
    PMD_INIT_LOG(ERR, "VIRTCHNL API version should not be lower than (%u.%u)",
    ...
    PMD_INIT_LOG(ERR, "PF/VF API version mismatch:(%u.%u)-(%u.%u)", pver->major,
                 pver->minor, VIRTCHNL_VERSION_MAJOR, VIRTCHNL_VERSION_MINOR);
    return -1;
  }
...
static int ice_dcf_get_vf_resource(struct ice_dcf_hw *hw) {
  uint32_t caps;
  int err, i;

  caps = VIRTCHNL_VF_OFFLOAD_WB_ON_ITR | VIRTCHNL_VF_OFFLOAD_RX_POLLING |
         VIRTCHNL_VF_CAP_ADV_LINK_SPEED | VIRTCHNL_VF_CAP_DCF |
         VIRTCHNL_VF_OFFLOAD_VLAN_V2 | VF_BASE_MODE_OFFLOADS |
         VIRTCHNL_VF_OFFLOAD_RX_FLEX_DESC | VIRTCHNL_VF_OFFLOAD_QOS;
...
static int ice_dcf_get_vf_vsi_map(struct ice_dcf_hw *hw) {
  struct virtchnl_dcf_vsi_map *vsi_map;
  uint32_t valid_msg_len;
  uint16_t len;
  int err;

  err = ice_dcf_send_cmd_req_no_irq(hw, VIRTCHNL_OP_DCF_GET_VSI_MAP, NULL, 0);
  if (err) {
  ...
  err = ice_dcf_recv_cmd_rsp_no_irq(hw, VIRTCHNL_OP_DCF_GET_VSI_MAP,
                                    hw->arq_buf, ICE_DCF_AQ_BUF_SZ, &len);
  if (err) {
  ...
  valid_msg_len =
      (vsi_map->num_vfs - 1) * sizeof(vsi_map->vf_vsi[0]) + sizeof(*vsi_map);
  if (len != valid_msg_len) {
    PMD_DRV_LOG(ERR, "invalid vf vsi map response with length %u", len);
    return -EINVAL;
  }

  if (hw->num_vfs != 0 && hw->num_vfs != vsi_map->num_vfs) {
    PMD_DRV_LOG(ERR,
                "The number VSI map (%u) doesn't match the number of VFs (%u)",
...
static int ice_dcf_mode_disable(struct ice_dcf_hw *hw) {
  int err;

  if (hw->resetting)
    return 0;

  err = ice_dcf_send_cmd_req_no_irq(hw, VIRTCHNL_OP_DCF_DISABLE, NULL, 0);
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to send msg OP_DCF_DISABLE");
    return err;
  }

  err = ice_dcf_recv_cmd_rsp_no_irq(hw, VIRTCHNL_OP_DCF_DISABLE, hw->arq_buf,
                                    ICE_DCF_AQ_BUF_SZ, NULL);
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to get response of OP_DCF_DISABLE %d", err);
    return -1;
  }
...
static int ice_dcf_check_reset_done(struct ice_dcf_hw *hw) {
#define ICE_DCF_RESET_WAIT_CNT 50
  ...
  for (i = 0; i < ICE_DCF_RESET_WAIT_CNT; i++) {
    reset =
        IAVF_READ_REG(avf, IAVF_VFGEN_RSTAT) & IAVF_VFGEN_RSTAT_VFR_STATE_MASK;
...
static inline void ice_dcf_enable_irq0(struct ice_dcf_hw *hw) {
  struct iavf_hw *avf = &hw->avf;

  /* Enable admin queue interrupt trigger */
  IAVF_WRITE_REG(avf, IAVF_VFINT_ICR0_ENA1, IAVF_VFINT_ICR0_ENA1_ADMINQ_MASK);
...
static inline void ice_dcf_disable_irq0(struct ice_dcf_hw *hw) {
  struct iavf_hw *avf = &hw->avf;

  /* Disable all interrupt types */
  IAVF_WRITE_REG(avf, IAVF_VFINT_ICR0_ENA1, 0);
  IAVF_WRITE_REG(avf, IAVF_VFINT_DYN_CTL01, IAVF_VFINT_DYN_CTL01_ITR_INDX_MASK);

  IAVF_WRITE_FLUSH(avf);
}

static void ice_dcf_dev_interrupt_handler(void *param) {
...
int ice_dcf_execute_virtchnl_cmd(struct ice_dcf_hw *hw,
  ...
  if (cmd->v_ret != IAVF_SUCCESS) {
    err = -1;
    PMD_DRV_LOG(ERR, "No response (%d times) or return failure (%d) for cmd %d",
...
int ice_dcf_send_aq_cmd(void *dcf_hw, struct ice_aq_desc *desc, void *buf,
                        uint16_t buf_size) {
  ...
  if ((buf && !buf_size) || (!buf && buf_size) || buf_size > ICE_DCF_AQ_BUF_SZ)
    return -EINVAL;
  ...
  if (desc_cmd.v_ret != IAVF_SUCCESS || buff_cmd.v_ret != IAVF_SUCCESS) {
    err = -1;
    PMD_DRV_LOG(
        ERR, "No response (%d times) or return failure (desc: %d / buff: %d)",
...
int ice_dcf_handle_vsi_update_event(struct ice_dcf_hw *hw) {
  ...
  for (;;) {
    if (ice_dcf_get_vf_resource(hw) == 0 && ice_dcf_get_vf_vsi_map(hw) >= 0) {
      err = 0;
      break;
    }
...
static int ice_dcf_get_supported_rxdid(struct ice_dcf_hw *hw) {
  int err;

  err = ice_dcf_send_cmd_req_no_irq(hw, VIRTCHNL_OP_GET_SUPPORTED_RXDIDS, NULL,
                                    0);
  if (err) {
...
int ice_dcf_init_hw(struct rte_eth_dev *eth_dev, struct ice_dcf_hw *hw) {
  ...
  if (hw->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_RSS_PF) {
    hw->rss_key = rte_zmalloc(NULL, hw->vf_res->rss_key_size, 0);
    if (!hw->rss_key) {
      PMD_INIT_LOG(ERR, "unable to allocate rss_key memory");
      goto err_alloc;
    }
    hw->rss_lut = rte_zmalloc("rss_lut", hw->vf_res->rss_lut_size, 0);
    if (!hw->rss_lut) {
...
void ice_dcf_uninit_hw(struct rte_eth_dev *eth_dev, struct ice_dcf_hw *hw) {
  ...
  rte_intr_callback_unregister(intr_handle, ice_dcf_dev_interrupt_handler, hw);
...
static int ice_dcf_configure_rss_key(struct ice_dcf_hw *hw) {
...
static int ice_dcf_configure_rss_lut(struct ice_dcf_hw *hw) {
...
int ice_dcf_init_rss(struct ice_dcf_hw *hw) {
  ...
  if (!rss_conf->rss_key)
    ...
    rte_memcpy(hw->rss_key, rss_conf->rss_key,
               RTE_MIN(rss_conf->rss_key_len, hw->vf_res->rss_key_size));
...
int ice_dcf_configure_queues(struct ice_dcf_hw *hw) {
  ...
  size = sizeof(*vc_config) + sizeof(vc_config->qpair[0]) * hw->num_queue_pairs;
  ...
  for (i = 0, vc_qp = vc_config->qpair; i < hw->num_queue_pairs; i++, vc_qp++) {
    ...
    if (hw->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_RX_FLEX_DESC &&
        hw->supported_rxdid & BIT(IAVF_RXDID_COMMS_OVS_1)) {
...
#else
    if (hw->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_RX_FLEX_DESC &&
        hw->supported_rxdid & BIT(IAVF_RXDID_LEGACY_0)) {
