...
#define ICE_ACL_INSET_ETH_IPV4                                                 \
  (ICE_INSET_SMAC | ICE_INSET_DMAC | ICE_INSET_IPV4_SRC | ICE_INSET_IPV4_DST)
#define ICE_ACL_INSET_ETH_IPV4_UDP                                             \
  (ICE_ACL_INSET_ETH_IPV4 | ICE_INSET_UDP_SRC_PORT | ICE_INSET_UDP_DST_PORT)
#define ICE_ACL_INSET_ETH_IPV4_TCP                                             \
  (ICE_ACL_INSET_ETH_IPV4 | ICE_INSET_TCP_SRC_PORT | ICE_INSET_TCP_DST_PORT)
#define ICE_ACL_INSET_ETH_IPV4_SCTP                                            \
  (ICE_ACL_INSET_ETH_IPV4 | ICE_INSET_SCTP_SRC_PORT | ICE_INSET_SCTP_DST_PORT)
...
static struct ice_pattern_match_item ice_acl_pattern[] = {
...
static int ice_acl_prof_alloc(struct ice_hw *hw) {
  enum ice_fltr_ptype ptype, fltr_ptype;

  if (!hw->acl_prof) {
    hw->acl_prof = (struct ice_fd_hw_prof **)ice_malloc(
        hw, ICE_FLTR_PTYPE_MAX * sizeof(*hw->acl_prof));
    if (!hw->acl_prof)
      return -ENOMEM;
  }
  ...
  for (ptype = ICE_FLTR_PTYPE_NONF_NONE + 1; ptype < ICE_FLTR_PTYPE_MAX;
    ...
    if (!hw->acl_prof[ptype]) {
      hw->acl_prof[ptype] =
          (struct ice_fd_hw_prof *)ice_malloc(hw, sizeof(**hw->acl_prof));
      if (!hw->acl_prof[ptype])
        goto fail_mem;
    }
  }
 ...
 */
static int ice_acl_setup(struct ice_pf *pf) {
...
static void acl_add_prof_prepare(struct ice_hw *hw,
                                 struct ice_flow_seg_info *seg, bool is_l4,
                                 uint16_t src_port, uint16_t dst_port) {
  uint16_t val_loc, mask_loc;

  if (hw->dev_caps.num_funcs < 4) {
    /* mac source address */
    val_loc = offsetof(struct ice_fdir_fltr, ext_data.src_mac);
    mask_loc = offsetof(struct ice_fdir_fltr, ext_mask.src_mac);
    ice_flow_set_fld(seg, ICE_FLOW_FIELD_IDX_ETH_SA, val_loc, mask_loc,
                     ICE_FLOW_FLD_OFF_INVAL, false);

    /* mac destination address */
    val_loc = offsetof(struct ice_fdir_fltr, ext_data.dst_mac);
    mask_loc = offsetof(struct ice_fdir_fltr, ext_mask.dst_mac);
    ice_flow_set_fld(seg, ICE_FLOW_FIELD_IDX_ETH_DA, val_loc, mask_loc,
                     ICE_FLOW_FLD_OFF_INVAL, false);
  }
  ...
  ice_flow_set_fld(seg, ICE_FLOW_FIELD_IDX_IPV4_SA, val_loc, mask_loc,
                   ICE_FLOW_FLD_OFF_INVAL, false);

  /* IP destination address */
  val_loc = offsetof(struct ice_fdir_fltr, ip.v4.dst_ip);
  mask_loc = offsetof(struct ice_fdir_fltr, mask.v4.dst_ip);
  ice_flow_set_fld(seg, ICE_FLOW_FIELD_IDX_IPV4_DA, val_loc, mask_loc,
                   ICE_FLOW_FLD_OFF_INVAL, false);
  ...
  if (is_l4) {
    ...
    ice_flow_set_fld(seg, src_port, val_loc, mask_loc, ICE_FLOW_FLD_OFF_INVAL,
                     false);

    /* Layer 4 destination port */
    val_loc = offsetof(struct ice_fdir_fltr, ip.v4.dst_port);
    mask_loc = offsetof(struct ice_fdir_fltr, mask.v4.dst_port);
    ice_flow_set_fld(seg, dst_port, val_loc, mask_loc, ICE_FLOW_FLD_OFF_INVAL,
                     false);
  }
}
 ...
 */
static int ice_acl_prof_init(struct ice_pf *pf) {
  struct ice_hw *hw = ICE_PF_TO_HW(pf);
  ...
  seg = (struct ice_flow_seg_info *)ice_malloc(hw, sizeof(*seg));
  if (!seg)
    return -ENOMEM;
  ...
  ret = ice_flow_add_prof(hw, ICE_BLK_ACL, ICE_FLOW_RX,
                          ICE_FLTR_PTYPE_NONF_IPV4_OTHER, seg, 1, NULL, 0,
                          &prof_ipv4);
  if (ret)
    goto err_add_prof;
  ...
  acl_add_prof_prepare(hw, seg, true, ICE_FLOW_FIELD_IDX_UDP_SRC_PORT,
                       ICE_FLOW_FIELD_IDX_UDP_DST_PORT);
  ret = ice_flow_add_prof(hw, ICE_BLK_ACL, ICE_FLOW_RX,
                          ICE_FLTR_PTYPE_NONF_IPV4_UDP, seg, 1, NULL, 0,
                          &prof_ipv4_udp);
  if (ret)
  ...
  acl_add_prof_prepare(hw, seg, true, ICE_FLOW_FIELD_IDX_TCP_SRC_PORT,
                       ICE_FLOW_FIELD_IDX_TCP_DST_PORT);
  ret = ice_flow_add_prof(hw, ICE_BLK_ACL, ICE_FLOW_RX,
                          ICE_FLTR_PTYPE_NONF_IPV4_TCP, seg, 1, NULL, 0,
                          &prof_ipv4_tcp);
  if (ret)
  ...
  acl_add_prof_prepare(hw, seg, true, ICE_FLOW_FIELD_IDX_SCTP_SRC_PORT,
                       ICE_FLOW_FIELD_IDX_SCTP_DST_PORT);
  ret = ice_flow_add_prof(hw, ICE_BLK_ACL, ICE_FLOW_RX,
                          ICE_FLTR_PTYPE_NONF_IPV4_SCTP, seg, 1, NULL, 0,
                          &prof_ipv4_sctp);
  if (ret)
 ...
 */
static int ice_acl_set_input_set(struct ice_acl_conf *filter,
  ...
  case ICE_FLTR_PTYPE_NONF_IPV4_SCTP:
    ...
    ice_memcpy(&input->ext_data.src_mac, &filter->input.ext_data.src_mac,
               RTE_ETHER_ADDR_LEN, ICE_NONDMA_TO_NONDMA);

    ice_memcpy(&input->ext_mask.src_mac, &filter->input.ext_mask.src_mac,
               RTE_ETHER_ADDR_LEN, ICE_NONDMA_TO_NONDMA);

    ice_memcpy(&input->ext_data.dst_mac, &filter->input.ext_data.dst_mac,
               RTE_ETHER_ADDR_LEN, ICE_NONDMA_TO_NONDMA);
    ice_memcpy(&input->ext_mask.dst_mac, &filter->input.ext_mask.dst_mac,
               RTE_ETHER_ADDR_LEN, ICE_NONDMA_TO_NONDMA);

    break;
  case ICE_FLTR_PTYPE_NONF_IPV4_OTHER:
    ice_memcpy(&input->ip.v4, &filter->input.ip.v4, sizeof(struct ice_fdir_v4),
               ICE_NONDMA_TO_NONDMA);
    ice_memcpy(&input->mask.v4, &filter->input.mask.v4,
               sizeof(struct ice_fdir_v4), ICE_NONDMA_TO_NONDMA);
...
static inline int ice_acl_alloc_slot_id(struct rte_bitmap *slots,
                                        uint32_t *slot_id) {
  uint32_t pos = 0;
  uint64_t slab = 0;
  uint32_t i = 0;
...
ice_acl_hw_set_conf(struct ice_pf *pf, struct ice_fdir_fltr *input,
  ...
  if (slot_id < MAX_ACL_NORMAL_ENTRIES) {
    ...
    ret = ice_flow_add_entry(hw, blk, flow_type, entry_id, pf->main_vsi->idx,
                             ICE_FLOW_PRIO_NORMAL, input, acts, act_cnt,
                             &hw_entry);
    if (ret) {
    ...
    PMD_DRV_LOG(ERR,
                "Exceed the maximum entry number(%d)"
                " HW supported!",
                MAX_ACL_NORMAL_ENTRIES);
    return -1;
  }
...
static inline void ice_acl_del_entry(struct ice_hw *hw, uint64_t entry_id) {
  uint64_t hw_entry;
...
static inline void ice_acl_hw_rem_conf(struct ice_pf *pf, struct acl_rule *rule,
                                       int32_t entry_idx) {
  uint32_t slot_id;
  int32_t i;
  uint64_t entry_id;
...
static int ice_acl_create_filter(struct ice_adapter *ad, struct rte_flow *flow,
...
static int ice_acl_destroy_filter(struct ice_adapter *ad, struct rte_flow *flow,
...
static void ice_acl_filter_free(struct rte_flow *flow) {
  rte_free(flow->rule);
  flow->rule = NULL;
}

static int ice_acl_parse_action(__rte_unused struct ice_adapter *ad,
...
static int ice_acl_parse_pattern(__rte_unused struct ice_adapter *ad,
  ...
  for (item = pattern; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    ...
    case RTE_FLOW_ITEM_TYPE_ETH:
      ...
      if (eth_spec && eth_mask) {
        ...
        if (!rte_is_zero_ether_addr(&eth_spec->src) &&
          ...
          ice_memcpy(&filter->input.ext_data.src_mac, &eth_spec->src,
                     RTE_ETHER_ADDR_LEN, ICE_NONDMA_TO_NONDMA);
          ice_memcpy(&filter->input.ext_mask.src_mac, &eth_mask->src,
                     RTE_ETHER_ADDR_LEN, ICE_NONDMA_TO_NONDMA);
        }
        ...
        if (!rte_is_zero_ether_addr(&eth_spec->dst) &&
          ...
          ice_memcpy(&filter->input.ext_data.dst_mac, &eth_spec->dst,
                     RTE_ETHER_ADDR_LEN, ICE_NONDMA_TO_NONDMA);
          ice_memcpy(&filter->input.ext_mask.dst_mac, &eth_mask->dst,
                     RTE_ETHER_ADDR_LEN, ICE_NONDMA_TO_NONDMA);
        }
      }
      break;
    case RTE_FLOW_ITEM_TYPE_IPV4:
      ...
      if (ipv4_spec && ipv4_mask) {
        ...
        if (ipv4_mask->hdr.version_ihl || ipv4_mask->hdr.total_length ||
            ipv4_mask->hdr.packet_id || ipv4_mask->hdr.fragment_offset ||
        ...
        if (ipv4_mask->hdr.src_addr) {
          filter->input.ip.v4.src_ip = ipv4_spec->hdr.src_addr;
          filter->input.mask.v4.src_ip = ipv4_mask->hdr.src_addr;

          input_set |= ICE_INSET_IPV4_SRC;
        }

        if (ipv4_mask->hdr.dst_addr) {
          filter->input.ip.v4.dst_ip = ipv4_spec->hdr.dst_addr;
          filter->input.mask.v4.dst_ip = ipv4_mask->hdr.dst_addr;
    ...
    case RTE_FLOW_ITEM_TYPE_TCP:
      ...
      if (tcp_spec && tcp_mask) {
        ...
        if (l3 == RTE_FLOW_ITEM_TYPE_IPV4 && tcp_mask->hdr.src_port) {
          input_set |= ICE_INSET_TCP_SRC_PORT;
          filter->input.ip.v4.src_port = tcp_spec->hdr.src_port;
          filter->input.mask.v4.src_port = tcp_mask->hdr.src_port;
        }

        if (l3 == RTE_FLOW_ITEM_TYPE_IPV4 && tcp_mask->hdr.dst_port) {
          input_set |= ICE_INSET_TCP_DST_PORT;
          filter->input.ip.v4.dst_port = tcp_spec->hdr.dst_port;
          filter->input.mask.v4.dst_port = tcp_mask->hdr.dst_port;
        }
      }
      break;
    case RTE_FLOW_ITEM_TYPE_UDP:
      ...
      if (udp_spec && udp_mask) {
        ...
        if (l3 == RTE_FLOW_ITEM_TYPE_IPV4 && udp_mask->hdr.src_port) {
          input_set |= ICE_INSET_UDP_SRC_PORT;
          filter->input.ip.v4.src_port = udp_spec->hdr.src_port;
          filter->input.mask.v4.src_port = udp_mask->hdr.src_port;
        }

        if (l3 == RTE_FLOW_ITEM_TYPE_IPV4 && udp_mask->hdr.dst_port) {
          input_set |= ICE_INSET_UDP_DST_PORT;
          filter->input.ip.v4.dst_port = udp_spec->hdr.dst_port;
          filter->input.mask.v4.dst_port = udp_mask->hdr.dst_port;
        }
      }
      break;
    case RTE_FLOW_ITEM_TYPE_SCTP:
      ...
      if (sctp_spec && sctp_mask) {
        ...
        if (l3 == RTE_FLOW_ITEM_TYPE_IPV4 && sctp_mask->hdr.src_port) {
          input_set |= ICE_INSET_SCTP_SRC_PORT;
          filter->input.ip.v4.src_port = sctp_spec->hdr.src_port;
          filter->input.mask.v4.src_port = sctp_mask->hdr.src_port;
        }

        if (l3 == RTE_FLOW_ITEM_TYPE_IPV4 && sctp_mask->hdr.dst_port) {
          input_set |= ICE_INSET_SCTP_DST_PORT;
          filter->input.ip.v4.dst_port = sctp_spec->hdr.dst_port;
          filter->input.mask.v4.dst_port = sctp_mask->hdr.dst_port;
        }
      }
      break;
...
static int
ice_acl_parse(struct ice_adapter *ad, struct ice_pattern_match_item *array,
...
static int ice_acl_bitmap_init(struct ice_pf *pf) {
  uint32_t bmp_size;
  void *mem = NULL;
...
static int ice_acl_init(struct ice_adapter *ad) {
  int ret = 0;
  struct ice_pf *pf = &ad->pf;
  struct ice_hw *hw = ICE_PF_TO_HW(pf);
...
static void ice_acl_uninit(struct ice_adapter *ad) {
  struct ice_pf *pf = &ad->pf;
  struct ice_hw *hw = ICE_PF_TO_HW(pf);
  struct ice_flow_parser *parser = &ice_acl_parser;
...
static struct ice_flow_engine ice_acl_engine = {
    .init = ice_acl_init,
