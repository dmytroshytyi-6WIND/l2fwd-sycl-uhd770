...
static inline uint16_t ice_rx_reassemble_packets(struct ice_rx_queue *rxq,
...
static __rte_always_inline int ice_tx_free_bufs_vec(struct ice_tx_queue *txq) {
...
static __rte_always_inline void ice_tx_backlog_entry(struct ice_tx_entry *txep,
...
static inline void _ice_rx_queue_release_mbufs_vec(struct ice_rx_queue *rxq) {
...
static inline void _ice_tx_queue_release_mbufs_vec(struct ice_tx_queue *txq) {
  uint16_t i;
  ...
  struct rte_eth_dev *dev =
      &rte_eth_devices[txq->vsi->adapter->pf.dev_data->port_id];
...
static inline int ice_rxq_vec_setup_default(struct ice_rx_queue *rxq) {
  uintptr_t p;
...
#define ICE_TX_NO_VECTOR_FLAGS                                                 \
  (RTE_ETH_TX_OFFLOAD_MULTI_SEGS | RTE_ETH_TX_OFFLOAD_OUTER_IPV4_CKSUM |       \
   RTE_ETH_TX_OFFLOAD_TCP_TSO | RTE_ETH_TX_OFFLOAD_OUTER_UDP_CKSUM)

#define ICE_TX_VECTOR_OFFLOAD                                                  \
  (RTE_ETH_TX_OFFLOAD_VLAN_INSERT | RTE_ETH_TX_OFFLOAD_QINQ_INSERT |           \
   RTE_ETH_TX_OFFLOAD_IPV4_CKSUM | RTE_ETH_TX_OFFLOAD_SCTP_CKSUM |             \
   RTE_ETH_TX_OFFLOAD_UDP_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_CKSUM)

#define ICE_RX_VECTOR_OFFLOAD                                                  \
  (RTE_ETH_RX_OFFLOAD_CHECKSUM | RTE_ETH_RX_OFFLOAD_SCTP_CKSUM |               \
   RTE_ETH_RX_OFFLOAD_VLAN | RTE_ETH_RX_OFFLOAD_RSS_HASH)

#define ICE_VECTOR_PATH 0
#define ICE_VECTOR_OFFLOAD_PATH 1

static inline int ice_rx_vec_queue_default(struct ice_rx_queue *rxq) {
  if (!rxq)
    return -1;
...
static inline int ice_tx_vec_queue_default(struct ice_tx_queue *txq) {
  if (!txq)
    return -1;
...
static inline int ice_tx_vec_dev_check_default(struct rte_eth_dev *dev) {
  int i;
...
static inline void ice_txd_enable_offload(struct rte_mbuf *tx_pkt,
                                          uint64_t *txd_hi) {
  ...
  /* Tx Checksum Offload */
  /* SET MACLEN */
  td_offset |= (tx_pkt->l2_len >> 1) << ICE_TX_DESC_LEN_MACLEN_S;

  /* Enable L3 checksum offload */
  if (ol_flags & RTE_MBUF_F_TX_IP_CKSUM) {
    td_cmd |= ICE_TX_DESC_CMD_IIPT_IPV4_CSUM;
    td_offset |= (tx_pkt->l3_len >> 2) << ICE_TX_DESC_LEN_IPLEN_S;
  ...
  case RTE_MBUF_F_TX_SCTP_CKSUM:
    ...
    td_offset |= (sizeof(struct rte_sctp_hdr) >> 2) << ICE_TX_DESC_LEN_L4_LEN_S;
    break;
  ...
  *txd_hi |= ((uint64_t)td_offset) << ICE_TXD_QW1_OFFSET_S;
  ...
  if (ol_flags & (RTE_MBUF_F_TX_VLAN | RTE_MBUF_F_TX_QINQ)) {
    ...
    *txd_hi |= ((uint64_t)tx_pkt->vlan_tci << ICE_TXD_QW1_L2TAG1_S);
  }
