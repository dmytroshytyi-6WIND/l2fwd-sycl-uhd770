...
static __rte_always_inline void ice_dcf_update_vsi_ctx(struct ice_hw *hw,
                                                       uint16_t vsi_handle,
                                                       uint16_t vsi_map) {
  ...
  if (vsi_map & VIRTCHNL_DCF_VF_VSI_VALID) {
    if (!vsi_ctx) {
      ...
      if (!vsi_ctx) {
        PMD_DRV_LOG(ERR, "No memory for vsi context %u", vsi_handle);
        return;
      }
    ...
    new_vsi_num =
        (vsi_map & VIRTCHNL_DCF_VF_VSI_ID_M) >> VIRTCHNL_DCF_VF_VSI_ID_S;
    ...
    PMD_DRV_LOG(DEBUG, "VF%u is assigned with vsi number %u", vsi_handle,
                vsi_ctx->vsi_num);
  } else {
...
static void ice_dcf_update_vf_vsi_map(struct ice_hw *hw, uint16_t num_vfs,
                                      uint16_t *vf_vsi_map) {
  uint16_t vf_id;
...
static void ice_dcf_update_pf_vsi_map(struct ice_hw *hw, uint16_t pf_vsi_idx,
                                      uint16_t pf_vsi_num) {
  struct ice_vsi_ctx *vsi_ctx;
  ...
  if (!vsi_ctx) {
    PMD_DRV_LOG(ERR, "No memory for vsi context %u", pf_vsi_idx);
    return;
  }
  ...
  PMD_DRV_LOG(DEBUG, "VF%u is assigned with vsi number %u", pf_vsi_idx,
              vsi_ctx->vsi_num);
}

static void *ice_dcf_vsi_update_service_handler(void *param) {
  ...
  if (!ice_dcf_handle_vsi_update_event(hw)) {
    ...
    ice_dcf_update_vf_vsi_map(&adapter->parent.hw, hw->num_vfs, hw->vf_vsi_map);
  }
  ...
  if (reset_param->vfr && adapter->repr_infos) {
    ...
    if (vf_rep_eth_dev && vf_rep_eth_dev->data->dev_started) {
      PMD_DRV_LOG(DEBUG, "VF%u representor is resetting", reset_param->vf_id);
...
static void start_vsi_reset_thread(struct ice_dcf_hw *dcf_hw, bool vfr,
                                   uint16_t vf_id) {
...
void ice_dcf_handle_pf_event_msg(struct ice_dcf_hw *dcf_hw, uint8_t *msg,
                                 uint16_t msglen) {
  ...
  case VIRTCHNL_EVENT_LINK_CHANGE:
    ...
    if (dcf_hw->vf_res->vf_cap_flags & VIRTCHNL_VF_CAP_ADV_LINK_SPEED) {
      dcf_hw->link_speed = pf_msg->event_data.link_event_adv.link_speed;
    } else {
    ...
    rte_eth_dev_callback_process(dcf_hw->eth_dev, RTE_ETH_EVENT_INTR_LSC, NULL);
    break;
  ...
  case VIRTCHNL_EVENT_DCF_VSI_MAP_UPDATE:
    PMD_DRV_LOG(
        DEBUG, "VIRTCHNL_EVENT_DCF_VSI_MAP_UPDATE event : VF%u with VSI num %u",
    ...
    start_vsi_reset_thread(dcf_hw, true, pf_msg->event_data.vf_vsi_map.vf_id);
    break;
  default:
...
static int ice_dcf_query_port_ets(struct ice_hw *parent_hw,
                                  struct ice_dcf_hw *real_hw) {
  int ret;

  real_hw->ets_config = (struct ice_aqc_port_ets_elem *)ice_malloc(
      real_hw, sizeof(*real_hw->ets_config));
  if (!real_hw->ets_config)
    return ICE_ERR_NO_MEMORY;

  ret = ice_aq_query_port_ets(parent_hw->port_info, real_hw->ets_config,
                              sizeof(*real_hw->ets_config), NULL);
  if (ret) {
...
static int ice_dcf_init_parent_hw(struct ice_hw *hw) {
  ...
  PMD_INIT_LOG(INFO, "firmware %d.%d.%d api %d.%d.%d build 0x%08x",
               hw->fw_maj_ver, hw->fw_min_ver, hw->fw_patch, hw->api_maj_ver,
               hw->api_min_ver, hw->api_patch, hw->fw_build);
...
static int ice_dcf_load_pkg(struct ice_adapter *adapter) {
...
int ice_dcf_init_parent_adapter(struct rte_eth_dev *eth_dev) {
  ...
  parent_adapter->pf.main_vsi = rte_zmalloc(NULL, sizeof(struct ice_vsi), 0);
  ...
  if (hw->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_QOS) {
    ...
    if (err) {
      PMD_INIT_LOG(ERR, "failed to query port ets with error %d", err);
      goto uninit_hw;
    }
  }
  ...
  if (err) {
    PMD_INIT_LOG(ERR, "failed to load package with error %d", err);
    goto uninit_hw;
  }

  parent_adapter->pf.main_vsi->idx = hw->num_vfs;
  ice_dcf_update_pf_vsi_map(parent_hw, parent_adapter->pf.main_vsi->idx,
                            hw->pf_vsi_id);
