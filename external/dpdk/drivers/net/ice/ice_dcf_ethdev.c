...
static int ice_dcf_dev_init(struct rte_eth_dev *eth_dev);

static int ice_dcf_dev_uninit(struct rte_eth_dev *eth_dev);

static uint16_t ice_dcf_recv_pkts(__rte_unused void *rx_queue,
                                  __rte_unused struct rte_mbuf **bufs,
                                  __rte_unused uint16_t nb_pkts) {
  return 0;
}

static uint16_t ice_dcf_xmit_pkts(__rte_unused void *tx_queue,
                                  __rte_unused struct rte_mbuf **bufs,
                                  __rte_unused uint16_t nb_pkts) {
  return 0;
}

static int ice_dcf_init_rxq(struct rte_eth_dev *dev, struct ice_rx_queue *rxq) {
  ...
  if (max_pkt_len <= RTE_ETHER_MIN_LEN || max_pkt_len > ICE_FRAME_SIZE_MAX) {
    PMD_DRV_LOG(ERR,
...
static int ice_dcf_init_rx_queues(struct rte_eth_dev *dev) {
  struct ice_rx_queue **rxq = (struct ice_rx_queue **)dev->data->rx_queues;
  int i, ret;
...
static inline uint16_t iavf_calc_itr_interval(int16_t interval) {
...
static int ice_dcf_config_rx_queues_irqs(struct rte_eth_dev *dev,
  ...
  if (!dev->data->dev_conf.intr_conf.rxq || !rte_intr_dp_is_en(intr_handle)) {
    ...
    hw->nb_msix = 1;
    if (hw->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_WB_ON_ITR) {
       ...
       */
      IAVF_WRITE_REG(&hw->avf, IAVF_VFINT_DYN_CTLN1(hw->msix_base - 1),
      ...
      /* set ITR to max */
      interval = iavf_calc_itr_interval(IAVF_QUEUE_ITR_INTERVAL_MAX);
      IAVF_WRITE_REG(
          &hw->avf, IAVF_VFINT_DYN_CTL01,
    ...
    if (!rte_intr_allow_others(intr_handle)) {
       ...
       */
      hw->nb_msix =
          RTE_MIN(hw->vf_res->max_vectors, rte_intr_nb_efd_get(intr_handle));
      hw->msix_base = IAVF_MISC_VEC_ID;
      ...
      PMD_DRV_LOG(DEBUG, "%u vectors are mapping to %u Rx queues", hw->nb_msix,
                  dev->data->nb_rx_queues);
    }
  }
...
static int alloc_rxq_mbufs(struct ice_rx_queue *rxq) {
...
static int ice_dcf_rx_queue_start(struct rte_eth_dev *dev,
...
static inline void reset_rx_queue(struct ice_rx_queue *rxq) {
  uint16_t len;
  uint32_t i;
...
static inline void reset_tx_queue(struct ice_tx_queue *txq) {
...
static int ice_dcf_rx_queue_stop(struct rte_eth_dev *dev,
                                 uint16_t rx_queue_id) {
  struct ice_dcf_adapter *ad = dev->data->dev_private;
...
static int ice_dcf_tx_queue_start(struct rte_eth_dev *dev,
...
static int ice_dcf_tx_queue_stop(struct rte_eth_dev *dev,
                                 uint16_t tx_queue_id) {
  struct ice_dcf_adapter *ad = dev->data->dev_private;
  struct ice_dcf_hw *hw = &ad->real_hw;
  struct ice_tx_queue *txq;
...
static int ice_dcf_start_queues(struct rte_eth_dev *dev) {
  struct ice_rx_queue *rxq;
  struct ice_tx_queue *txq;
  int nb_rxq = 0;
  int nb_txq, i;
...
static int ice_dcf_dev_start(struct rte_eth_dev *dev) {
  ...
  if (hw->resetting) {
    PMD_DRV_LOG(ERR, "The DCF has been reset by PF, please reinit first");
    return -EIO;
  }
...
static void ice_dcf_stop_queues(struct rte_eth_dev *dev) {
  struct ice_dcf_adapter *ad = dev->data->dev_private;
  struct ice_dcf_hw *hw = &ad->real_hw;
  struct ice_rx_queue *rxq;
  struct ice_tx_queue *txq;
...
static int ice_dcf_dev_stop(struct rte_eth_dev *dev) {
  struct ice_dcf_adapter *dcf_ad = dev->data->dev_private;
  struct rte_intr_handle *intr_handle = dev->intr_handle;
  struct ice_adapter *ad = &dcf_ad->parent;
  struct ice_dcf_hw *hw = &dcf_ad->real_hw;
...
static int ice_dcf_dev_configure(struct rte_eth_dev *dev) {
  struct ice_dcf_adapter *dcf_ad = dev->data->dev_private;
  struct ice_adapter *ad = &dcf_ad->parent;
...
static int ice_dcf_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->tx_offload_capa =
      ...
      RTE_ETH_TX_OFFLOAD_OUTER_UDP_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_TSO |
...
static int ice_dcf_dev_flow_ops_get(struct rte_eth_dev *dev,
...
static void ice_dcf_stat_update_48(uint64_t *offset, uint64_t *stat) {
  if (*stat >= *offset)
    *stat = *stat - *offset;
  else
    *stat =
        (uint64_t)((*stat + ((uint64_t)1 << ICE_DCF_48_BIT_WIDTH)) - *offset);

  *stat &= ICE_DCF_48_BIT_MASK;
}

static void ice_dcf_stat_update_32(uint64_t *offset, uint64_t *stat) {
  if (*stat >= *offset)
    *stat = (uint64_t)(*stat - *offset);
  else
    *stat =
        (uint64_t)((*stat + ((uint64_t)1 << ICE_DCF_32_BIT_WIDTH)) - *offset);
}

static void ice_dcf_update_stats(struct virtchnl_eth_stats *oes,
...
static int ice_dcf_stats_get(struct rte_eth_dev *dev,
...
static int ice_dcf_stats_reset(struct rte_eth_dev *dev) {
  struct ice_dcf_adapter *ad = dev->data->dev_private;
  struct ice_dcf_hw *hw = &ad->real_hw;
  struct virtchnl_eth_stats pstats;
  int ret;
...
static void ice_dcf_free_repr_info(struct ice_dcf_adapter *dcf_adapter) {
...
static int ice_dcf_init_repr_info(struct ice_dcf_adapter *dcf_adapter) {
...
static int ice_dcf_dev_close(struct rte_eth_dev *dev) {
  struct ice_dcf_adapter *adapter = dev->data->dev_private;
...
int ice_dcf_link_update(struct rte_eth_dev *dev,
  ...
  new_link.link_status = hw->link_up ? RTE_ETH_LINK_UP : RTE_ETH_LINK_DOWN;
...
ice_dcf_dev_udp_tunnel_port_add(struct rte_eth_dev *dev,
  ...
  case RTE_ETH_TUNNEL_TYPE_VXLAN:
    ret = ice_create_tunnel(parent_hw, TNL_VXLAN, udp_tunnel->udp_port);
    break;
  case RTE_ETH_TUNNEL_TYPE_ECPRI:
    ret = ice_create_tunnel(parent_hw, TNL_ECPRI, udp_tunnel->udp_port);
    break;
  default:
...
static int ice_dcf_tm_ops_get(struct rte_eth_dev *dev __rte_unused, void *arg) {
  if (!arg)
    return -EINVAL;
...
static inline void ice_dcf_reset_hw(struct rte_eth_dev *eth_dev,
...
static inline bool ice_dcf_is_reset(struct rte_eth_dev *dev) {
  struct ice_dcf_adapter *ad = dev->data->dev_private;
  struct iavf_hw *hw = &ad->real_hw.avf;
...
static int ice_dcf_dev_reset(struct rte_eth_dev *dev) {
  struct ice_dcf_adapter *ad = dev->data->dev_private;
  struct ice_dcf_hw *hw = &ad->real_hw;
  int ret;
...
static int ice_dcf_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  if (ice_dcf_init_hw(eth_dev, &adapter->real_hw) != 0) {
    ...
    __atomic_store_n(&parent_adapter->dcf_state_on, false, __ATOMIC_RELAXED);
    return -1;
  }
...
static int ice_dcf_cap_check_handler(__rte_unused const char *key,
                                     const char *value,
...
static int ice_dcf_cap_selected(struct rte_devargs *devargs) {
  ...
  if (rte_kvargs_process(kvlist, key, ice_dcf_cap_check_handler, NULL) < 0)
    goto exit;
...
static int eth_ice_dcf_pci_probe(__rte_unused struct rte_pci_driver *pci_drv,
  ...
  ret = rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct ice_dcf_adapter),
                                      ice_dcf_dev_init);
  ...
  for (i = 0; i < eth_da.nb_representor_ports; i++) {
    ...
    if (vf_id >= dcf_adapter->real_hw.num_vfs) {
      PMD_DRV_LOG(ERR, "VF ID %u is out of range (0 ~ %u)", vf_id,
                  dcf_adapter->real_hw.num_vfs - 1);
      ret = -EINVAL;
      break;
    }
    ...
    ret = rte_eth_dev_create(&pci_dev->device, repr_name,
                             sizeof(struct ice_dcf_vf_repr), NULL, NULL,
                             ice_dcf_vf_repr_init, &repr_param);
    if (ret) {
      PMD_DRV_LOG(ERR, "failed to create DCF VF representor %s", repr_name);
      break;
    }

    vf_rep_eth_dev = rte_eth_dev_allocated(repr_name);
    if (!vf_rep_eth_dev) {
      PMD_DRV_LOG(ERR, "Failed to find the ethdev for DCF VF representor: %s",
                  repr_name);
      ret = -ENODEV;
      break;
    }
...
static int eth_ice_dcf_pci_remove(struct rte_pci_device *pci_dev) {
  ...
  if (eth_dev->data->dev_flags & RTE_ETH_DEV_REPRESENTOR)
    return rte_eth_dev_pci_generic_remove(pci_dev, ice_dcf_vf_repr_uninit);
  else
