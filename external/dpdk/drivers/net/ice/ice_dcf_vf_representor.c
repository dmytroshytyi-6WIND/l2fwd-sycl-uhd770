...
static uint16_t ice_dcf_vf_repr_rx_burst(__rte_unused void *rxq,
                                         __rte_unused struct rte_mbuf **rx_pkts,
                                         __rte_unused uint16_t nb_pkts) {
  return 0;
}

static uint16_t ice_dcf_vf_repr_tx_burst(__rte_unused void *txq,
                                         __rte_unused struct rte_mbuf **tx_pkts,
                                         __rte_unused uint16_t nb_pkts) {
  return 0;
}

static int ice_dcf_vf_repr_dev_configure(struct rte_eth_dev *dev) {
  ice_dcf_vf_repr_init_vlan(dev);

  return 0;
}

static int ice_dcf_vf_repr_dev_start(struct rte_eth_dev *dev) {
...
static int ice_dcf_vf_repr_rx_queue_setup(
    __rte_unused struct rte_eth_dev *dev, __rte_unused uint16_t queue_id,
...
static int ice_dcf_vf_repr_tx_queue_setup(
    __rte_unused struct rte_eth_dev *dev, __rte_unused uint16_t queue_id,
    __rte_unused uint16_t nb_desc, __rte_unused unsigned int socket_id,
...
static int ice_dcf_vf_repr_link_update(__rte_unused struct rte_eth_dev *ethdev,
...
ice_dcf_vf_repr_hw(struct ice_dcf_vf_repr *repr) {
  struct ice_dcf_adapter *dcf_adapter = repr->dcf_eth_dev->data->dev_private;
...
static int ice_dcf_vf_repr_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa =
      ...
      RTE_ETH_RX_OFFLOAD_VLAN_FILTER | RTE_ETH_RX_OFFLOAD_VLAN_EXTEND |
...
static int ice_dcf_vf_repr_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  struct ice_dcf_vf_repr *repr = dev->data->dev_private;
  ...
  if (mask & RTE_ETH_VLAN_STRIP_MASK) {
    bool enable = !!(dev_conf->rxmode.offloads & RTE_ETH_RX_OFFLOAD_VLAN_STRIP);

    if (enable && repr->outer_vlan_info.port_vlan_ena) {
      PMD_DRV_LOG(ERR, "Disable the port VLAN firstly\n");
      return -EINVAL;
    }
    ...
    if (enable)
      vlan_offload.vlan_flags = VIRTCHNL_DCF_VLAN_STRIP_INTO_RX_DESC
                                << VIRTCHNL_DCF_VLAN_STRIP_MODE_S;
    else if (repr->outer_vlan_info.stripping_ena && !enable)
      vlan_offload.vlan_flags = VIRTCHNL_DCF_VLAN_STRIP_DISABLE
                                << VIRTCHNL_DCF_VLAN_STRIP_MODE_S;
...
static int ice_dcf_vf_repr_vlan_pvid_set(struct rte_eth_dev *dev, uint16_t pvid,
                                         int on) {
  struct ice_dcf_vf_repr *repr = dev->data->dev_private;
  struct virtchnl_dcf_vlan_offload vlan_offload;
  int err;
  ...
  if (repr->outer_vlan_info.stripping_ena) {
    PMD_DRV_LOG(ERR, "Disable the VLAN stripping firstly\n");
    return -EINVAL;
  }
  ...
  if (on)
    vlan_offload.vlan_flags = (VIRTCHNL_DCF_VLAN_INSERT_PORT_BASED
                               << VIRTCHNL_DCF_VLAN_INSERT_MODE_S);
  else
    vlan_offload.vlan_flags =
        (VIRTCHNL_DCF_VLAN_INSERT_DISABLE << VIRTCHNL_DCF_VLAN_INSERT_MODE_S);

  vlan_offload.vf_id = repr->vf_id;
  vlan_offload.tpid = repr->outer_vlan_info.tpid;
  vlan_offload.vlan_flags |=
      (VIRTCHNL_DCF_VLAN_TYPE_OUTER << VIRTCHNL_DCF_VLAN_TYPE_S);
...
static int ice_dcf_vf_repr_vlan_tpid_set(struct rte_eth_dev *dev,
  ...
  if (tpid != RTE_ETHER_TYPE_QINQ && tpid != RTE_ETHER_TYPE_VLAN &&
      tpid != RTE_ETHER_TYPE_QINQ1) {
    PMD_DRV_LOG(ERR, "Invalid TPID: 0x%04x\n", tpid);
    return -EINVAL;
  }
  ...
  if (repr->outer_vlan_info.port_vlan_ena) {
    err = ice_dcf_vf_repr_vlan_pvid_set(dev, repr->outer_vlan_info.vid, true);
    if (err) {
      PMD_DRV_LOG(ERR, "Failed to reset port VLAN : %d\n", err);
      return err;
    }
  }

  if (repr->outer_vlan_info.stripping_ena) {
    err = ice_dcf_vf_repr_vlan_offload_set(dev, RTE_ETH_VLAN_STRIP_MASK);
    if (err) {
      PMD_DRV_LOG(ERR, "Failed to reset VLAN stripping : %d\n", err);
      return err;
    }
  }
...
int ice_dcf_vf_repr_init_vlan(struct rte_eth_dev *vf_rep_eth_dev) {
  ...
  err =
      ice_dcf_vf_repr_vlan_offload_set(vf_rep_eth_dev, RTE_ETH_VLAN_STRIP_MASK);
  if (err) {
