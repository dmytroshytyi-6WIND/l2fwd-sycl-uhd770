...
static const u32 ice_sect_lkup[ICE_BLK_COUNT][ICE_SECT_COUNT] = {
    /* SWITCH */
    {ICE_SID_XLT0_SW, ICE_SID_XLT_KEY_BUILDER_SW, ICE_SID_XLT1_SW,
     ICE_SID_XLT2_SW, ICE_SID_PROFID_TCAM_SW, ICE_SID_PROFID_REDIR_SW,
     ICE_SID_FLD_VEC_SW, ICE_SID_CDID_KEY_BUILDER_SW, ICE_SID_CDID_REDIR_SW},

    /* ACL */
    {ICE_SID_XLT0_ACL, ICE_SID_XLT_KEY_BUILDER_ACL, ICE_SID_XLT1_ACL,
     ICE_SID_XLT2_ACL, ICE_SID_PROFID_TCAM_ACL, ICE_SID_PROFID_REDIR_ACL,
     ICE_SID_FLD_VEC_ACL, ICE_SID_CDID_KEY_BUILDER_ACL, ICE_SID_CDID_REDIR_ACL},

    /* FD */
    {ICE_SID_XLT0_FD, ICE_SID_XLT_KEY_BUILDER_FD, ICE_SID_XLT1_FD,
     ICE_SID_XLT2_FD, ICE_SID_PROFID_TCAM_FD, ICE_SID_PROFID_REDIR_FD,
     ICE_SID_FLD_VEC_FD, ICE_SID_CDID_KEY_BUILDER_FD, ICE_SID_CDID_REDIR_FD},

    /* RSS */
    {ICE_SID_XLT0_RSS, ICE_SID_XLT_KEY_BUILDER_RSS, ICE_SID_XLT1_RSS,
     ICE_SID_XLT2_RSS, ICE_SID_PROFID_TCAM_RSS, ICE_SID_PROFID_REDIR_RSS,
     ICE_SID_FLD_VEC_RSS, ICE_SID_CDID_KEY_BUILDER_RSS, ICE_SID_CDID_REDIR_RSS},

    /* PE */
    {ICE_SID_XLT0_PE, ICE_SID_XLT_KEY_BUILDER_PE, ICE_SID_XLT1_PE,
     ICE_SID_XLT2_PE, ICE_SID_PROFID_TCAM_PE, ICE_SID_PROFID_REDIR_PE,
     ICE_SID_FLD_VEC_PE, ICE_SID_CDID_KEY_BUILDER_PE, ICE_SID_CDID_REDIR_PE}};
...
static struct ice_buf_table *ice_find_buf_table(struct ice_seg *ice_seg) {
  ...
  return (_FORCE_ struct ice_buf_table *)(nvms->vers +
...
static struct ice_buf_hdr *ice_pkg_enum_buf(struct ice_seg *ice_seg,
  ...
  if (++state->buf_idx < LE32_TO_CPU(state->buf_table->buf_count))
    return ice_pkg_val_buf(state->buf_table->buf_array + state->buf_idx);
  else
    return NULL;
}
 ...
 */
void *ice_pkg_enum_entry(struct ice_seg *ice_seg, struct ice_pkg_enum *state,
  ...
  /* get entry */
  entry =
      state->handler(state->sect_type, state->sect, state->entry_idx, offset);
  if (!entry) {
...
bool ice_hw_ptype_ena(struct ice_hw *hw, u16 ptype) {
  return ptype < ICE_FLOW_PTYPE_MAX && ice_is_bit_set(hw->hw_ptype, ptype);
}
...
static void ice_fill_hw_ptype(struct ice_hw *hw) {
  ...
  do {
    tcam = (struct ice_marker_ptype_tcam_entry *)ice_pkg_enum_entry(
        seg, &state, ICE_SID_RXPARSER_MARKER_PTYPE, NULL,
        ice_marker_ptype_tcam_handler);
    if (tcam && LE16_TO_CPU(tcam->addr) < ICE_MARKER_PTYPE_TCAM_ADDR_MAX &&
...
ice_find_boost_entry(struct ice_seg *ice_seg, u16 addr,
  ...
  do {
    tcam = (struct ice_boost_tcam_entry *)ice_pkg_enum_entry(
        ice_seg, &state, ICE_SID_RXPARSER_BOOST_TCAM, NULL,
        ice_boost_tcam_handler);
...
static char *ice_enum_labels(struct ice_seg *ice_seg, u32 type,
  ...
  label = (struct ice_label *)ice_pkg_enum_entry(ice_seg, state, type, NULL,
                                                 ice_label_enum_handler);
  if (!label)
    return NULL;
...
static void ice_init_pkg_hints(struct ice_hw *hw, struct ice_seg *ice_seg) {
  ...
  label_name =
      ice_enum_labels(ice_seg, ICE_SID_LBL_RXPARSER_TMEM, &state, &val);
 ...
 */
static enum ice_status ice_gen_key_word(u8 val, u8 valid, u8 dont_care,
                                        u8 nvr_mtch, u8 *key, u8 *key_inv) {
 ...
 */
enum ice_status ice_set_key(u8 *key, u16 size, u8 *val, u8 *upd, u8 *dc, u8 *nm,
                            u16 off, u16 len) {
  u16 half_size;
  u16 i;
...
#define ICE_NVR_MTCH_BITS_MAX 1
  ...
  for (i = 0; i < len; i++)
    if (ice_gen_key_word(val[i], upd ? upd[i] : 0xff, dc ? dc[i] : 0,
                         nm ? nm[i] : 0, key + off + i,
                         key + half_size + off + i))
      return ICE_ERR_CFG;
 ...
 */
enum ice_status ice_acquire_change_lock(struct ice_hw *hw,
                                        enum ice_aq_res_access_type access) {
  ice_debug(hw, ICE_DBG_TRACE, "%s\n", __func__);
 ...
 */
static enum ice_status ice_aq_download_pkg(struct ice_hw *hw,
                                           struct ice_buf_hdr *pkg_buf,
                                           u16 buf_size, bool last_buf,
 ...
 */
enum ice_status ice_aq_upload_section(struct ice_hw *hw,
                                      struct ice_buf_hdr *pkg_buf, u16 buf_size,
                                      struct ice_sq_cd *cd) {
  struct ice_aq_desc desc;
 ...
 */
static enum ice_status ice_aq_update_pkg(struct ice_hw *hw,
                                         struct ice_buf_hdr *pkg_buf,
                                         u16 buf_size, bool last_buf,
                                         u32 *error_offset, u32 *error_info,
                                         struct ice_sq_cd *cd) {
...
ice_find_seg_in_pkg(struct ice_hw *hw, u32 seg_type,
  ...
  ice_debug(hw, ICE_DBG_PKG, "Package format version: %d.%d.%d.%d\n",
            ...
            pkg_hdr->pkg_format_ver.update, pkg_hdr->pkg_format_ver.draft);

  /* Search all package segments for the requested segment type */
  for (i = 0; i < LE32_TO_CPU(pkg_hdr->seg_count); i++) {
    struct ice_generic_seg_hdr *seg;

    seg = (struct ice_generic_seg_hdr *)((u8 *)pkg_hdr +
 ...
 */
static enum ice_status ice_update_pkg_no_lock(struct ice_hw *hw,
  ...
  for (i = 0; i < count; i++) {
    ...
    status = ice_aq_update_pkg(hw, bh, LE16_TO_CPU(bh->data_end), last, &offset,
                               &info, NULL);
 ...
 */
enum ice_status ice_update_pkg(struct ice_hw *hw, struct ice_buf *bufs,
                               u32 count) {
  enum ice_status status;
 ...
 */
static enum ice_status ice_dwnld_cfg_bufs(struct ice_hw *hw,
                                          struct ice_buf *bufs, u32 count) {
  enum ice_status status;
  ...
  for (i = 0; i < count; i++) {
    ...
    status = ice_aq_download_pkg(hw, bh, ICE_PKG_BUF_SIZE, last, &offset, &info,
                                 NULL);
 ...
 */
static enum ice_status ice_download_pkg(struct ice_hw *hw,
  ...
  ice_debug(hw, ICE_DBG_TRACE, "%s\n", __func__);
  ice_debug(
      hw, ICE_DBG_PKG, "Segment format version: %d.%d.%d.%d\n",
      ice_seg->hdr.seg_format_ver.major, ice_seg->hdr.seg_format_ver.minor,
      ice_seg->hdr.seg_format_ver.update, ice_seg->hdr.seg_format_ver.draft);
 ...
 */
static enum ice_status ice_init_pkg_info(struct ice_hw *hw,
                                         struct ice_pkg_hdr *pkg_hdr) {
  ...
  seg_hdr = (struct ice_generic_seg_hdr *)ice_find_seg_in_pkg(
      hw, hw->pkg_seg_id, pkg_hdr);
  if (seg_hdr) {
    ...
    meta = (struct ice_meta_sect *)ice_pkg_enum_section(
        (struct ice_seg *)seg_hdr, &state, ICE_SID_METADATA);
    if (!meta) {
    ...
    ice_debug(hw, ICE_DBG_PKG, "Pkg: %d.%d.%d.%d, %s\n", meta->ver.major,
              meta->ver.minor, meta->ver.update, meta->ver.draft, meta->name);

    hw->ice_seg_fmt_ver = seg_hdr->seg_format_ver;
    ice_memcpy(hw->ice_seg_id, seg_hdr->seg_id, sizeof(hw->ice_seg_id),
               ICE_NONDMA_TO_NONDMA);

    ice_debug(hw, ICE_DBG_PKG, "Ice Seg: %d.%d.%d.%d, %s\n",
              seg_hdr->seg_format_ver.major, seg_hdr->seg_format_ver.minor,
              seg_hdr->seg_format_ver.update, seg_hdr->seg_format_ver.draft,
              seg_hdr->seg_id);
  } else {
...
#define ICE_PKG_FLAG_COUNT 4
    ...
    if (pkg_info->pkg_info[i].is_active) {
      ...
      hw->active_track_id = LE32_TO_CPU(pkg_info->pkg_info[i].track_id);
      ice_memcpy(hw->active_pkg_name, pkg_info->pkg_info[i].name,
                 sizeof(pkg_info->pkg_info[i].name), ICE_NONDMA_TO_NONDMA);
    ...
    ice_debug(hw, ICE_DBG_PKG, "Pkg[%d]: %d.%d.%d.%d,%s,%s\n", i,
              pkg_info->pkg_info[i].ver.major, pkg_info->pkg_info[i].ver.minor,
              pkg_info->pkg_info[i].ver.update, pkg_info->pkg_info[i].ver.draft,
 ...
 */
static enum ice_status ice_chk_pkg_compat(struct ice_hw *hw,
  ...
  *seg = (struct ice_seg *)ice_find_seg_in_pkg(hw, hw->pkg_seg_id, ospkg);
  if (!*seg) {
  ...
  for (i = 0; i < LE32_TO_CPU(pkg->count); i++) {
    ...
    if (!pkg->pkg_info[i].is_in_nvm)
      continue;
    if ((*seg)->hdr.seg_format_ver.major != pkg->pkg_info[i].ver.major ||
        (*seg)->hdr.seg_format_ver.minor > pkg->pkg_info[i].ver.minor) {
...
static void *ice_sw_fv_handler(u32 sect_type, void *section, u32 index,
                               u32 *offset) {
  struct ice_sw_fv_section *fv_section = (struct ice_sw_fv_section *)section;
...
static int ice_get_prof_index_max(struct ice_hw *hw) {
  ...
  do {
    fv = (struct ice_fv *)ice_pkg_enum_entry(
        ice_seg, &state, ICE_SID_FLD_VEC_SW, &offset, ice_sw_fv_handler);
    if (!fv)
      break;
    ice_seg = NULL;
...
static struct ice_buf_build *ice_pkg_buf_alloc(struct ice_hw *hw) {
  ...
  buf->data_end = CPU_TO_LE16(offsetof(struct ice_buf_hdr, section_entry));
  return bld;
}
 ...
 */
enum ice_status ice_get_sw_fv_list(struct ice_hw *hw, u8 *prot_ids, u16 ids_cnt,
                                   ice_bitmap_t *bm,
...
void ice_init_prof_result_bm(struct ice_hw *hw) {
  ...
  do {
    ...
    fv = (struct ice_fv *)ice_pkg_enum_entry(
        ice_seg, &state, ICE_SID_FLD_VEC_SW, &off, ice_sw_fv_handler);
    ice_seg = NULL;
    if (!fv)
      break;

    ice_zero_bitmap(hw->switch_info->prof_res_bm[off], ICE_MAX_FV_WORDS);
    ...
    for (i = 1; i < ICE_MAX_FV_WORDS; i++)
      if (fv->ew[i].prot_id == ICE_PROT_INVALID &&
        ...
        ice_set_bit(i, hw->switch_info->prof_res_bm[off]);
  } while (fv);
}
 ...
 * Note: all package contents must be in Little Endian form.
 */
static enum ice_status ice_pkg_buf_reserve_section(struct ice_buf_build *bld,
                                                   u16 count) {
  struct ice_buf_hdr *buf;
  u16 section_count;
  u16 data_end;
  ...
  data_end =
      LE16_TO_CPU(buf->data_end) + FLEX_ARRAY_SIZE(buf, section_entry, count);
 ...
 * Note: all package contents must be in Little Endian form.
 */
static void *ice_pkg_buf_alloc_section(struct ice_buf_build *bld, u32 type,
                                       u16 size) {
  struct ice_buf_hdr *buf;
  u16 sect_count;
  u16 data_end;
 ...
 * index
 */
static bool ice_tunnel_port_in_use_hlpr(struct ice_hw *hw, u16 port,
                                        u16 *index) {
  u16 i;
 ...
 */
bool ice_tunnel_get_type(struct ice_hw *hw, u16 port,
 ...
 */
static bool ice_find_free_tunnel_entry(struct ice_hw *hw,
                                       enum ice_tunnel_type type, u16 *index) {
  u16 i;
 ...
 */
bool ice_get_open_tunnel_port(struct ice_hw *hw, enum ice_tunnel_type type,
                              u16 *port) {
  bool res = false;
  u16 i;
 ...
 */
static enum ice_status ice_upd_dvm_boost_entry(struct ice_hw *hw,
  ...
  sect_rx = (struct ice_boost_tcam_section *)ice_pkg_buf_alloc_section(
      bld, ICE_SID_RXPARSER_BOOST_TCAM, ice_struct_size(sect_rx, tcam, 1));
  if (!sect_rx)
    goto ice_upd_dvm_boost_entry_err;
  sect_rx->count = CPU_TO_LE16(1);

  sect_tx = (struct ice_boost_tcam_section *)ice_pkg_buf_alloc_section(
      bld, ICE_SID_TXPARSER_BOOST_TCAM, ice_struct_size(sect_tx, tcam, 1));
  if (!sect_tx)
    goto ice_upd_dvm_boost_entry_err;
  ...
  ice_set_key((u8 *)&sect_rx->tcam[0].key, sizeof(sect_rx->tcam[0].key), &val,
              NULL, &dc, &nm, 0, sizeof(u8));
...
enum ice_status ice_destroy_tunnel(struct ice_hw *hw, u16 port, bool all) {
  ...
  sect_rx = (struct ice_boost_tcam_section *)ice_pkg_buf_alloc_section(
      bld, ICE_SID_RXPARSER_BOOST_TCAM, size);
  if (!sect_rx)
    goto ice_destroy_tunnel_err;
  sect_rx->count = CPU_TO_LE16(count);

  sect_tx = (struct ice_boost_tcam_section *)ice_pkg_buf_alloc_section(
      bld, ICE_SID_TXPARSER_BOOST_TCAM, size);
  if (!sect_tx)
    goto ice_destroy_tunnel_err;
  ...
  for (i = 0, j = 0; i < hw->tnl.count && i < ICE_TUNNEL_MAX_ENTRIES; i++)
    if (hw->tnl.tbl[i].valid && hw->tnl.tbl[i].in_use &&
        (all || hw->tnl.tbl[i].port == port)) {
      ice_memcpy(sect_rx->tcam + j, hw->tnl.tbl[i].boost_entry,
                 sizeof(*sect_rx->tcam), ICE_NONDMA_TO_NONDMA);
      ice_memcpy(sect_tx->tcam + j, hw->tnl.tbl[i].boost_entry,
 ...
 */
enum ice_status ice_find_prot_off(struct ice_hw *hw, enum ice_block blk,
                                  u8 prof, u16 fv_idx, u8 *prot, u16 *off) {
 ...
 */
static enum ice_status ice_ptg_find_ptype(struct ice_hw *hw, enum ice_block blk,
                                          u16 ptype, u8 *ptg) {
...
ice_ptg_add_mv_ptype(struct ice_hw *hw, enum ice_block blk, u16 ptype, u8 ptg) {
  ...
  hw->blk[blk].xlt1.ptg_tbl[ptg].first_ptype = &hw->blk[blk].xlt1.ptypes[ptype];
...
static const struct ice_blk_size_details blk_sizes[ICE_BLK_COUNT] = {
    ...
    /* SW  */ {ICE_XLT1_CNT, ICE_XLT2_CNT, 512, 256, 0, 256, 256, 48, false,
               false},
    /* ACL */
    {ICE_XLT1_CNT, ICE_XLT2_CNT, 512, 128, 0, 128, 128, 32, false, false},
    /* FD  */
    {ICE_XLT1_CNT, ICE_XLT2_CNT, 512, 128, 0, 128, 128, 24, false, true},
    /* RSS */
    {ICE_XLT1_CNT, ICE_XLT2_CNT, 512, 128, 0, 128, 128, 24, true, true},
    /* PE  */ {ICE_XLT1_CNT, ICE_XLT2_CNT, 64, 32, 0, 32, 32, 24, false, false},
};
 ...
 */
static bool ice_match_prop_lst(struct LIST_HEAD_TYPE *list1,
                               struct LIST_HEAD_TYPE *list2) {
 ...
 */
enum ice_status ice_vsig_find_vsi(struct ice_hw *hw, enum ice_block blk,
                                  u16 vsi, u16 *vsig) {
 ...
 */
static enum ice_status ice_find_dup_props_vsig(struct ice_hw *hw,
                                               enum ice_block blk,
                                               struct LIST_HEAD_TYPE *chs,
                                               u16 *vsig) {
 ...
 */
static enum ice_status ice_vsig_free(struct ice_hw *hw, enum ice_block blk,
                                     u16 vsig) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(del, dtmp, &hw->blk[blk].xlt2.vsig_tbl[idx].prop_lst,
                           ice_vsig_prof, list) {
    LIST_DEL(&del->list);
    ice_free(hw, del);
  }
...
ice_vsig_add_mv_vsi(struct ice_hw *hw, enum ice_block blk, u16 vsi, u16 vsig) {
   ...
   * doesn't exist.
   */
  if (!hw->blk[blk].xlt2.vsig_tbl[idx].in_use && vsig != ICE_DEFAULT_VSIG)
    return ICE_ERR_DOES_NOT_EXIST;
  ...
  hw->blk[blk].xlt2.vsig_tbl[idx].first_vsi = &hw->blk[blk].xlt2.vsis[vsi];
 ...
 */
static enum ice_status ice_find_prof_id_with_mask(struct ice_hw *hw,
                                                  enum ice_block blk,
                                                  struct ice_fv_word *fv,
                                                  u16 *masks, u8 *prof_id) {
 ...
 * block.
 */
static enum ice_status ice_alloc_tcam_ent(struct ice_hw *hw, enum ice_block blk,
                                          bool btm, u16 *tcam_idx) {
  u16 res_type;
 ...
 */
static enum ice_status ice_free_tcam_ent(struct ice_hw *hw, enum ice_block blk,
                                         u16 tcam_idx) {
  u16 res_type;
 ...
 */
static enum ice_status ice_alloc_prof_id(struct ice_hw *hw, enum ice_block blk,
                                         u8 *prof_id) {
  enum ice_status status;
  u16 res_type;
  u16 get_prof;
 ...
 */
static enum ice_status ice_free_prof_id(struct ice_hw *hw, enum ice_block blk,
                                        u8 prof_id) {
 ...
 */
static enum ice_status ice_prof_inc_ref(struct ice_hw *hw, enum ice_block blk,
                                        u8 prof_id) {
...
static void ice_write_prof_mask_enable_res(struct ice_hw *hw,
                                           enum ice_block blk, u16 prof_id,
                                           u32 enable_mask) {
  u32 offset;
...
static void ice_init_prof_masks(struct ice_hw *hw, enum ice_block blk) {
  ...
  ice_memset(hw->blk[blk].masks.masks, 0, sizeof(hw->blk[blk].masks.masks),
             ICE_NONDMA_MEM);
 ...
 */
static enum ice_status ice_alloc_prof_mask(struct ice_hw *hw,
                                           enum ice_block blk, u16 idx,
                                           u16 mask, u16 *mask_idx) {
 ...
 */
static enum ice_status ice_free_prof_mask(struct ice_hw *hw, enum ice_block blk,
                                          u16 mask_idx) {
  if (blk != ICE_BLK_RSS && blk != ICE_BLK_FD)
    return ICE_ERR_PARAM;
 ...
 * @prof_id: profile ID
 */
static enum ice_status ice_free_prof_masks(struct ice_hw *hw,
                                           enum ice_block blk, u16 prof_id) {
  u32 mask_bm;
  u16 i;
 ...
 * @masks: masks
 */
static enum ice_status ice_update_prof_masking(struct ice_hw *hw,
                                               enum ice_block blk, u16 prof_id,
                                               u16 *masks) {
  bool err = false;
  u32 ena_mask = 0;
...
static void ice_write_es(struct ice_hw *hw, enum ice_block blk, u8 prof_id,
  ...
  if (!fv) {
    ice_memset(&hw->blk[blk].es.t[off], 0, hw->blk[blk].es.fvw * sizeof(*fv),
               ICE_NONDMA_MEM);
    hw->blk[blk].es.written[prof_id] = false;
  } else {
    ice_memcpy(&hw->blk[blk].es.t[off], fv, hw->blk[blk].es.fvw * sizeof(*fv),
               ICE_NONDMA_TO_NONDMA);
  }
}
 ...
 */
static enum ice_status ice_prof_dec_ref(struct ice_hw *hw, enum ice_block blk,
                                        u8 prof_id) {
  if (prof_id > hw->blk[blk].es.count)
    return ICE_ERR_PARAM;
...
static const u32 ice_blk_sids[ICE_BLK_COUNT][ICE_SID_OFF_COUNT] = {
    /* SWITCH */
    {ICE_SID_XLT1_SW, ICE_SID_XLT2_SW, ICE_SID_PROFID_TCAM_SW,
     ICE_SID_PROFID_REDIR_SW, ICE_SID_FLD_VEC_SW},

    /* ACL */
    {ICE_SID_XLT1_ACL, ICE_SID_XLT2_ACL, ICE_SID_PROFID_TCAM_ACL,
     ICE_SID_PROFID_REDIR_ACL, ICE_SID_FLD_VEC_ACL},

    /* FD */
    {ICE_SID_XLT1_FD, ICE_SID_XLT2_FD, ICE_SID_PROFID_TCAM_FD,
     ICE_SID_PROFID_REDIR_FD, ICE_SID_FLD_VEC_FD},

    /* RSS */
    {ICE_SID_XLT1_RSS, ICE_SID_XLT2_RSS, ICE_SID_PROFID_TCAM_RSS,
     ICE_SID_PROFID_REDIR_RSS, ICE_SID_FLD_VEC_RSS},

    /* PE */
    {ICE_SID_XLT1_PE, ICE_SID_XLT2_PE, ICE_SID_PROFID_TCAM_PE,
     ICE_SID_PROFID_REDIR_PE, ICE_SID_FLD_VEC_PE}};
...
static void ice_fill_tbl(struct ice_hw *hw, enum ice_block block_id, u32 sid) {
  ...
  while (sect) {
    ...
    case ICE_SID_XLT1_PE:
      ...
      src = xlt1->value;
      sect_len = LE16_TO_CPU(xlt1->count) * sizeof(*hw->blk[block_id].xlt1.t);
      dst = hw->blk[block_id].xlt1.t;
      dst_len =
          hw->blk[block_id].xlt1.count * sizeof(*hw->blk[block_id].xlt1.t);
      break;
    ...
    case ICE_SID_XLT2_PE:
      ...
      sect_len = LE16_TO_CPU(xlt2->count) * sizeof(*hw->blk[block_id].xlt2.t);
      dst = (u8 *)hw->blk[block_id].xlt2.t;
      dst_len =
          hw->blk[block_id].xlt2.count * sizeof(*hw->blk[block_id].xlt2.t);
      break;
    ...
    case ICE_SID_PROFID_TCAM_PE:
      ...
      src = (u8 *)pid->entry;
      sect_len = LE16_TO_CPU(pid->count) * sizeof(*hw->blk[block_id].prof.t);
      dst = (u8 *)hw->blk[block_id].prof.t;
      dst_len =
          hw->blk[block_id].prof.count * sizeof(*hw->blk[block_id].prof.t);
      break;
    ...
    case ICE_SID_PROFID_REDIR_PE:
      ...
      src = pr->redir_value;
      sect_len =
          LE16_TO_CPU(pr->count) * sizeof(*hw->blk[block_id].prof_redir.t);
    ...
    case ICE_SID_FLD_VEC_PE:
      ...
      src = (u8 *)es->fv;
      sect_len = (u32)(LE16_TO_CPU(es->count) * hw->blk[block_id].es.fvw) *
                 sizeof(*hw->blk[block_id].es.t);
      dst = (u8 *)hw->blk[block_id].es.t;
      dst_len = (u32)(hw->blk[block_id].es.count * hw->blk[block_id].es.fvw) *
                sizeof(*hw->blk[block_id].es.t);
      break;
    default:
      return;
...
static void ice_free_prof_map(struct ice_hw *hw, u8 blk_idx) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(del, tmp, &es->prof_map, ice_prof_map, list) {
    LIST_DEL(&del->list);
    ice_free(hw, del);
  }
...
static void ice_free_flow_profs(struct ice_hw *hw, u8 blk_idx) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(p, tmp, &hw->fl_profs[blk_idx], ice_flow_prof,
                           l_entry) {
    struct ice_flow_entry *e, *t;

    LIST_FOR_EACH_ENTRY_SAFE(e, t, &p->entries, ice_flow_entry, l_entry)
    ice_flow_rem_entry(hw, (enum ice_block)blk_idx, ICE_FLOW_ENTRY_HNDL(e));
...
void ice_free_hw_tbls(struct ice_hw *hw) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(r, rt, &hw->rss_list_head, ice_rss_cfg, l_entry) {
    LIST_DEL(&r->l_entry);
    ice_free(hw, r);
  }
...
void ice_clear_hw_tbls(struct ice_hw *hw) {
  ...
  for (i = 0; i < ICE_BLK_COUNT; i++) {
    ...
    ice_memset(xlt1->ptypes, 0, xlt1->count * sizeof(*xlt1->ptypes),
               ICE_NONDMA_MEM);
    ice_memset(xlt1->ptg_tbl, 0, ICE_MAX_PTGS * sizeof(*xlt1->ptg_tbl),
               ICE_NONDMA_MEM);
    ice_memset(xlt1->t, 0, xlt1->count * sizeof(*xlt1->t), ICE_NONDMA_MEM);

    ice_memset(xlt2->vsis, 0, xlt2->count * sizeof(*xlt2->vsis),
               ICE_NONDMA_MEM);
    ice_memset(xlt2->vsig_tbl, 0, xlt2->count * sizeof(*xlt2->vsig_tbl),
               ICE_NONDMA_MEM);
    ice_memset(xlt2->t, 0, xlt2->count * sizeof(*xlt2->t), ICE_NONDMA_MEM);

    ice_memset(prof->t, 0, prof->count * sizeof(*prof->t), ICE_NONDMA_MEM);
    ice_memset(prof_redir->t, 0, prof_redir->count * sizeof(*prof_redir->t),
               ICE_NONDMA_MEM);

    ice_memset(es->t, 0, es->count * sizeof(*es->t) * es->fvw, ICE_NONDMA_MEM);
...
enum ice_status ice_init_hw_tbls(struct ice_hw *hw) {
  ...
  for (i = 0; i < ICE_BLK_COUNT; i++) {
    ...
    xlt1->ptypes = (struct ice_ptg_ptype *)ice_calloc(hw, xlt1->count,
                                                      sizeof(*xlt1->ptypes));

    if (!xlt1->ptypes)
      goto err;

    xlt1->ptg_tbl = (struct ice_ptg_entry *)ice_calloc(hw, ICE_MAX_PTGS,
                                                       sizeof(*xlt1->ptg_tbl));
    ...
    xlt2->vsis =
        (struct ice_vsig_vsi *)ice_calloc(hw, xlt2->count, sizeof(*xlt2->vsis));

    if (!xlt2->vsis)
      goto err;

    xlt2->vsig_tbl = (struct ice_vsig_entry *)ice_calloc(
        hw, xlt2->count, sizeof(*xlt2->vsig_tbl));
    if (!xlt2->vsig_tbl)
      goto err;
    ...
    prof->t = (struct ice_prof_tcam_entry *)ice_calloc(hw, prof->count,
                                                       sizeof(*prof->t));
    ...
    prof_redir->t =
        (u8 *)ice_calloc(hw, prof_redir->count, sizeof(*prof_redir->t));
    ...
    es->t = (struct ice_fv_word *)ice_calloc(hw, (u32)(es->count * es->fvw),
                                             sizeof(*es->t));
    if (!es->t)
      goto err;

    es->ref_count = (u16 *)ice_calloc(hw, es->count, sizeof(*es->ref_count));

    if (!es->ref_count)
      goto err;

    es->written = (u8 *)ice_calloc(hw, es->count, sizeof(*es->written));

    if (!es->written)
      goto err;

    es->mask_ena = (u32 *)ice_calloc(hw, es->count, sizeof(*es->mask_ena));
 ...
 */
static enum ice_status ice_prof_gen_key(struct ice_hw *hw, enum ice_block blk,
                                        u8 ptg, u16 vsig, u8 cdid, u16 flags,
...
#define ICE_CD_8_S 16
  ...
  return ice_set_key(key, ICE_TCAM_KEY_SZ, (u8 *)&inkey, vl_msk, dc_msk, nm_msk,
                     0, ICE_TCAM_KEY_SZ / 2);
}
 ...
 * @nm_msk: never match mask
 */
static enum ice_status ice_tcam_write_entry(
    struct ice_hw *hw, enum ice_block blk, u16 idx, u8 prof_id, u8 ptg,
    u16 vsig, u8 cdid, u16 flags, u8 vl_msk[ICE_TCAM_KEY_VAL_SZ],
    u8 dc_msk[ICE_TCAM_KEY_VAL_SZ], u8 nm_msk[ICE_TCAM_KEY_VAL_SZ]) {
  struct ice_prof_tcam_entry;
  enum ice_status status;

  status = ice_prof_gen_key(hw, blk, ptg, vsig, cdid, flags, vl_msk, dc_msk,
                            nm_msk, hw->blk[blk].prof.t[idx].key);
  if (!status) {
 ...
 */
static enum ice_status ice_vsig_get_ref(struct ice_hw *hw, enum ice_block blk,
                                        u16 vsig, u16 *refs) {
  u16 idx = vsig & ICE_VSIG_IDX_M;
 ...
 */
static enum ice_status ice_prof_bld_es(struct ice_hw *hw, enum ice_block blk,
                                       struct ice_buf_build *bld,
  ...
  if (tmp->type == ICE_PTG_ES_ADD && tmp->add_prof) {
    ...
    p = (struct ice_pkg_es *)ice_pkg_buf_alloc_section(
        bld, id, ice_struct_size(p, es, 1) + vec_size - sizeof(p->es[0]));
    ...
    ice_memcpy(p->es, &hw->blk[blk].es.t[off], vec_size, ICE_NONDMA_TO_NONDMA);
  }
 ...
 * @chgs: the list of changes to make in hardware
 */
static enum ice_status ice_prof_bld_tcam(struct ice_hw *hw, enum ice_block blk,
                                         struct ice_buf_build *bld,
                                         struct LIST_HEAD_TYPE *chgs) {
  struct ice_chs_chg *tmp;
  ...
  if (tmp->type == ICE_TCAM_ADD && tmp->add_tcam_idx) {
    ...
    ice_memcpy(p->entry[0].key, &hw->blk[blk].prof.t[tmp->tcam_idx].key,
               sizeof(hw->blk[blk].prof.t->key), ICE_NONDMA_TO_NONDMA);
  }
 ...
 * @chgs: the list of changes to make in hardware
 */
static enum ice_status ice_prof_bld_xlt1(enum ice_block blk,
                                         struct ice_buf_build *bld,
                                         struct LIST_HEAD_TYPE *chgs) {
  struct ice_chs_chg *tmp;
  ...
  if (tmp->type == ICE_PTG_ES_ADD && tmp->add_ptg) {
    ...
    p = (struct ice_xlt1_section *)ice_pkg_buf_alloc_section(
        bld, id, ice_struct_size(p, value, 1));
 ...
 * @chgs: the list of changes to make in hardware
 */
static enum ice_status ice_prof_bld_xlt2(enum ice_block blk,
                                         struct ice_buf_build *bld,
                                         struct LIST_HEAD_TYPE *chgs) {
  struct ice_chs_chg *tmp;
  ...
  LIST_FOR_EACH_ENTRY(tmp, chgs, ice_chs_chg, list_entry) {
    ...
    case ICE_VSIG_REM:
      ...
      p = (struct ice_xlt2_section *)ice_pkg_buf_alloc_section(
          bld, id, ice_struct_size(p, value, 1));
 ...
 * @chgs: the list of changes to make in hardware
 */
static enum ice_status ice_upd_prof_hw(struct ice_hw *hw, enum ice_block blk,
                                       struct LIST_HEAD_TYPE *chgs) {
...
static const struct ice_fd_src_dst_pair ice_fd_pairs[] = {
    ...
    {ICE_PROT_IPV4_OF_OR_S, 2, 12}, {ICE_PROT_IPV4_OF_OR_S, 2, 16},

    {ICE_PROT_IPV4_IL, 2, 12},      {ICE_PROT_IPV4_IL, 2, 16},

    {ICE_PROT_IPV4_IL_IL, 2, 12},   {ICE_PROT_IPV4_IL_IL, 2, 16},

    {ICE_PROT_IPV6_OF_OR_S, 8, 8},  {ICE_PROT_IPV6_OF_OR_S, 8, 24},

    {ICE_PROT_IPV6_IL, 8, 8},       {ICE_PROT_IPV6_IL, 8, 24},

    {ICE_PROT_IPV6_IL_IL, 8, 8},    {ICE_PROT_IPV6_IL_IL, 8, 24},

    {ICE_PROT_TCP_IL, 1, 0},        {ICE_PROT_TCP_IL, 1, 2},

    {ICE_PROT_UDP_OF, 1, 0},        {ICE_PROT_UDP_OF, 1, 2},

    {ICE_PROT_UDP_IL_OR_S, 1, 0},   {ICE_PROT_UDP_IL_OR_S, 1, 2},

    {ICE_PROT_SCTP_IL, 1, 0},       {ICE_PROT_SCTP_IL, 1, 2}};
 ...
 */
static enum ice_status ice_update_fd_swap(struct ice_hw *hw, u16 prof_id,
...
#define ICE_FD_FV_NOT_FOUND (-2)
  ...
  for (i = 0; i < hw->blk[ICE_BLK_FD].es.fvw; i++) {
     ...
     */
    if (first_free == ICE_FD_FV_NOT_FOUND && es[i].prot_id != ICE_PROT_INVALID)
      first_free = i - 1;
  ...
  for (i = 0; i < ICE_FD_SRC_DST_PAIR_COUNT; i += 2) {
    ...
    if (bit0 ^ bit1) {
      ...
      for (k = 0; k < ice_fd_pairs[index].count; k++) {
        es[first_free - k].prot_id = ice_fd_pairs[index].prot_id;
        es[first_free - k].off = ice_fd_pairs[index].off + (k * 2);
    ...
    for (j = 0; j < ICE_FD_SRC_DST_PAIR_COUNT; j++)
      if (es[si].prot_id == ice_fd_pairs[j].prot_id &&
        ...
        for (k = 0; k < indexes_used; k++) {
          used[si - k] = (pair_start[idx] - k) | ICE_SWAP_VALID;
        }
...
ice_add_prof_attrib(struct ice_prof_map *prof, u8 ptg, u16 ptype,
  ...
  for (i = 0; i < attr_cnt; i++) {
    if (attr[i].ptype == ptype) {
      ...
      ice_get_ptype_attrib_info(attr[i].attrib, &prof->attr[prof->ptg_cnt]);
 ...
 */
enum ice_status ice_add_prof(struct ice_hw *hw, enum ice_block blk, u64 id,
                             u8 ptypes[],
  ...
  while (bytes && prof->ptg_cnt < ICE_MAX_PTG_PER_PROFILE) {
    ...
    ice_for_each_set_bit(bit, (ice_bitmap_t *)&ptypes[byte], BITS_PER_BYTE) {
      u16 ptype;
      u8 ptg;
 ...
 */
static enum ice_status ice_rel_tcam_idx(struct ice_hw *hw, enum ice_block blk,
                                        u16 idx) {
  ...
  status =
      ice_tcam_write_entry(hw, blk, idx, 0, 0, 0, 0, 0, vl_msk, dc_msk, nm_msk);
  if (status)
    return status;
 ...
 */
static enum ice_status ice_rem_prof_id(struct ice_hw *hw, enum ice_block blk,
 ...
 * @chg: the change list
 */
static enum ice_status ice_rem_vsig(struct ice_hw *hw, enum ice_block blk,
                                    u16 vsig, struct LIST_HEAD_TYPE *chg) {
  u16 idx = vsig & ICE_VSIG_IDX_M;
  struct ice_vsig_vsi *vsi_cur;
  ...
  /* remove TCAM entries */
  LIST_FOR_EACH_ENTRY_SAFE(d, t, &hw->blk[blk].xlt2.vsig_tbl[idx].prop_lst,
                           ice_vsig_prof, list) {
 ...
 */
static enum ice_status ice_rem_prof_id_vsig(struct ice_hw *hw,
                                            enum ice_block blk, u16 vsig,
                                            u64 hdl,
  ...
  LIST_FOR_EACH_ENTRY_SAFE(p, t, &hw->blk[blk].xlt2.vsig_tbl[idx].prop_lst,
                           ice_vsig_prof, list)
 ...
 * @id: profile tracking ID
 */
static enum ice_status ice_rem_flow_all(struct ice_hw *hw, enum ice_block blk,
                                        u64 id) {
  ...
  for (i = 1; i < ICE_MAX_VSIGS; i++)
    if (hw->blk[blk].xlt2.vsig_tbl[i].in_use) {
      if (ice_has_prof_vsig(hw, blk, i, id)) {
        status = ice_rem_prof_id_vsig(hw, blk, i, id, &chg);
        if (status)
 ...
 * @chg: change list
 */
static enum ice_status ice_get_prof(struct ice_hw *hw, enum ice_block blk,
                                    u64 hdl, struct LIST_HEAD_TYPE *chg) {
  enum ice_status status = ICE_SUCCESS;
  struct ice_prof_map *map;
  struct ice_chs_chg *p;
  u16 i;
 ...
 */
static enum ice_status ice_get_profs_vsig(struct ice_hw *hw, enum ice_block blk,
                                          u16 vsig,
 ...
 */
static enum ice_status ice_add_prof_to_lst(struct ice_hw *hw,
                                           enum ice_block blk,
                                           struct LIST_HEAD_TYPE *lst,
                                           u64 hdl) {
  enum ice_status status = ICE_SUCCESS;
 ...
 * @chg: the change list
 */
static enum ice_status ice_move_vsi(struct ice_hw *hw, enum ice_block blk,
                                    u16 vsi, u16 vsig,
                                    struct LIST_HEAD_TYPE *chg) {
  enum ice_status status;
  struct ice_chs_chg *p;
 ...
 */
static enum ice_status ice_prof_tcam_ena_dis(struct ice_hw *hw,
                                             enum ice_block blk, bool enable,
                                             u16 vsig,
                                             struct ice_tcam_inf *tcam,
                                             struct LIST_HEAD_TYPE *chg) {
  enum ice_status status;
  struct ice_chs_chg *p;
   ...
   */
  status = ice_alloc_tcam_ent(hw, blk, tcam->attr.mask == 0, &tcam->tcam_idx);
  if (status)
    return status;
  ...
  status =
      ice_tcam_write_entry(hw, blk, tcam->tcam_idx, tcam->prof_id, tcam->ptg,
                           vsig, 0, tcam->attr.flags, vl_msk, dc_msk, nm_msk);
  if (status)
 ...
 */
static bool ice_ptg_attr_in_use(struct ice_tcam_inf *ptg_attr,
                                ice_bitmap_t *ptgs_used,
 ...
 * @chg: the change list
 */
static enum ice_status ice_adj_prof_priorities(struct ice_hw *hw,
                                               enum ice_block blk, u16 vsig,
                                               struct LIST_HEAD_TYPE *chg) {
  ice_declare_bitmap(ptgs_used, ICE_XLT1_CNT);
  struct ice_tcam_inf **attr_used;
...
#define ICE_MAX_PTG_ATTRS 1024
  ...
  LIST_FOR_EACH_ENTRY(t, &hw->blk[blk].xlt2.vsig_tbl[idx].prop_lst,
    ...
    for (i = 0; i < t->tcam_count; i++) {
       ...
       */
      used =
          ice_ptg_attr_in_use(&t->tcam[i], ptgs_used, attr_used, attr_used_cnt);
      ...
      if (used && t->tcam[i].in_use) {
         ...
         * (and lower priority)
         */
        status = ice_prof_tcam_ena_dis(hw, blk, false, vsig, &t->tcam[i], chg);
        if (status)
         ...
         */
        status = ice_prof_tcam_ena_dis(hw, blk, true, vsig, &t->tcam[i], chg);
        if (status)
          goto err_ice_adj_prof_priorities;
      }
 ...
 * @chg: the change list
 */
static enum ice_status ice_add_prof_id_vsig(struct ice_hw *hw,
                                            enum ice_block blk, u16 vsig,
                                            u64 hdl, bool rev,
                                            struct LIST_HEAD_TYPE *chg) {
  ...
  for (i = 0; i < map->ptg_cnt; i++) {
     ...
     * top of the table in order to give it higher priority
     */
    status = ice_alloc_tcam_ent(hw, blk, map->attr[i].mask == 0, &tcam_idx);
    if (status) {
      ice_free(hw, p);
      goto err_ice_add_prof_id_vsig;
    }
    ...
    /* write the TCAM entry */
    status = ice_tcam_write_entry(
        hw, blk, t->tcam[i].tcam_idx, t->tcam[i].prof_id, t->tcam[i].ptg, vsig,
        0, t->tcam[i].attr.flags, vl_msk, dc_msk, nm_msk);
    if (status) {
      ice_free(hw, p);
      goto err_ice_add_prof_id_vsig;
    }
  ...
  if (rev)
    LIST_ADD_TAIL(&t->list, &hw->blk[blk].xlt2.vsig_tbl[vsig_idx].prop_lst);
  else
    LIST_ADD(&t->list, &hw->blk[blk].xlt2.vsig_tbl[vsig_idx].prop_lst);
 ...
 * @chg: the change list
 */
static enum ice_status ice_create_prof_id_vsig(struct ice_hw *hw,
                                               enum ice_block blk, u16 vsi,
                                               u64 hdl,
                                               struct LIST_HEAD_TYPE *chg) {
  enum ice_status status;
  struct ice_chs_chg *p;
 ...
 * @chg: the change list
 */
static enum ice_status ice_create_vsig_from_lst(struct ice_hw *hw,
                                                enum ice_block blk, u16 vsi,
                                                struct LIST_HEAD_TYPE *lst,
                                                u16 *new_vsig,
                                                struct LIST_HEAD_TYPE *chg) {
  ...
  LIST_FOR_EACH_ENTRY(t, lst, ice_vsig_prof, list) {
    ...
    status = ice_add_prof_id_vsig(hw, blk, vsig, t->profile_cookie, true, chg);
    if (status)
      return status;
  }
 ...
 */
enum ice_status ice_add_vsi_flow(struct ice_hw *hw, enum ice_block blk, u16 vsi,
                                 u16 vsig) {
 ...
 */
enum ice_status ice_add_prof_id_flow(struct ice_hw *hw, enum ice_block blk,
                                     u16 vsi, u64 hdl) {
  ...
  if (!status && vsig) {
    ...
    if (!status) {
       ...
       * profile to the VSIG.
       */
      status = ice_add_prof_id_vsig(hw, blk, vsig, hdl, false, &chg);
      if (status)
        goto err_ice_add_prof_id_flow;
      ...
      status = ice_create_vsig_from_lst(hw, blk, vsi, &union_lst, &vsig, &chg);
      if (status)
        goto err_ice_add_prof_id_flow;
    ...
    if (ice_find_prof_vsig(hw, blk, hdl, &vsig)) {
      ...
      status = ice_create_prof_id_vsig(hw, blk, vsi, hdl, &chg);
      if (status)
        goto err_ice_add_prof_id_flow;
    }
  }
 ...
 */
enum ice_status ice_rem_prof_id_flow(struct ice_hw *hw, enum ice_block blk,
                                     u16 vsi, u64 hdl) {
  struct ice_vsig_prof *tmp1, *del1;
  ...
  if (!status && vsig) {
    ...
    if (only_vsi) {
      ...
      if (last_profile) {
        ...
        status = ice_rem_prof_id_vsig(hw, blk, vsig, hdl, &chg);
        if (status)
          goto err_ice_rem_prof_id_flow;
      ...
      if (LIST_EMPTY(&copy)) {
        status = ice_move_vsi(hw, blk, vsi, ICE_DEFAULT_VSIG, &chg);
        if (status)
          goto err_ice_rem_prof_id_flow;

      } else if (!ice_find_dup_props_vsig(hw, blk, &copy, &vsig)) {
        /* found an exact match */
        /* add or move VSI to the VSIG that matches */
         ...
         */
        status = ice_create_vsig_from_lst(hw, blk, vsi, &copy, &vsig, &chg);
        if (status)
          goto err_ice_rem_prof_id_flow;
 ...
 * array. Once successfully called, the flow will be enabled.
 */
enum ice_status ice_flow_assoc_hw_prof(struct ice_hw *hw, enum ice_block blk,
                                       u16 dest_vsi_handle, u16 fdir_vsi_handle,
                                       int id) {
  enum ice_status status = ICE_SUCCESS;
  u16 vsi_num;
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_FLOW,
              "HW profile add failed for main VSI flow entry, %d\n", status);
    goto err_add_prof;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_FLOW,
              "HW profile add failed for ctrl VSI flow entry, %d\n", status);
    goto err_add_entry;
  }
