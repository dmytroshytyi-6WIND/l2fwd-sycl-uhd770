...
bool ice_check_sq_alive(struct ice_hw *hw, struct ice_ctl_q_info *cq) {
  ...
  if (cq->sq.len && cq->sq.len_mask && cq->sq.len_ena_mask)
    return (rd32(hw, cq->sq.len) & (cq->sq.len_mask | cq->sq.len_ena_mask)) ==
 ...
 * @cq: pointer to the specific Control queue
 */
static enum ice_status ice_alloc_ctrlq_sq_ring(struct ice_hw *hw,
  ...
  cq->sq.cmd_buf = ice_calloc(hw, cq->num_sq_entries, sizeof(struct ice_sq_cd));
  if (!cq->sq.cmd_buf) {
 ...
 * @cq: pointer to the specific Control queue
 */
static enum ice_status ice_alloc_ctrlq_rq_ring(struct ice_hw *hw,
                                               struct ice_ctl_q_info *cq) {
 ...
 * @cq: pointer to the specific Control queue
 */
static enum ice_status ice_alloc_rq_bufs(struct ice_hw *hw,
                                         struct ice_ctl_q_info *cq) {
  int i;
   ...
   */
  cq->rq.dma_head = ice_calloc(hw, cq->num_rq_entries, sizeof(cq->rq.desc_buf));
  if (!cq->rq.dma_head)
    return ICE_ERR_NO_MEMORY;
 ...
 * @cq: pointer to the specific Control queue
 */
static enum ice_status ice_alloc_sq_bufs(struct ice_hw *hw,
                                         struct ice_ctl_q_info *cq) {
  int i;

  /* No mapped memory needed yet, just the buffer info structures */
  cq->sq.dma_head = ice_calloc(hw, cq->num_sq_entries, sizeof(cq->sq.desc_buf));
  if (!cq->sq.dma_head)
    return ICE_ERR_NO_MEMORY;
...
static enum ice_status ice_cfg_cq_regs(struct ice_hw *hw,
                                       struct ice_ctl_q_ring *ring,
                                       u16 num_entries) {
  /* Clear Head and Tail */
  wr32(hw, ring->head, 0);
  wr32(hw, ring->tail, 0);
 ...
 */
static enum ice_status ice_cfg_sq_regs(struct ice_hw *hw,
                                       struct ice_ctl_q_info *cq) {
 ...
 */
static enum ice_status ice_cfg_rq_regs(struct ice_hw *hw,
                                       struct ice_ctl_q_info *cq) {
    ...
    if ((qi)->ring.r.ring##_bi) {                                              \
      ...
      for (i = 0; i < (qi)->num_##ring##_entries; i++)                         \
        if ((qi)->ring.r.ring##_bi[i].pa)                                      \
          ice_free_dma_mem((hw), &(qi)->ring.r.ring##_bi[i]);                  \
    }                                                                          \
 ...
 */
static enum ice_status ice_init_sq(struct ice_hw *hw,
                                   struct ice_ctl_q_info *cq) {
 ...
 * called are not going to be atomic context safe
 */
static enum ice_status ice_init_rq(struct ice_hw *hw,
                                   struct ice_ctl_q_info *cq) {
  enum ice_status ret_code;
 ...
 */
static enum ice_status ice_shutdown_sq(struct ice_hw *hw,
                                       struct ice_ctl_q_info *cq) {
...
static bool ice_aq_ver_check(struct ice_hw *hw) {
  if (hw->api_maj_ver > EXP_FW_API_VER_MAJOR) {
    ...
    ice_warn(hw, "The driver for the device stopped because the NVM image is "
                 "newer than expected. You must install the most recent "
                 "version of the network driver.\n");
    return false;
  } else if (hw->api_maj_ver == EXP_FW_API_VER_MAJOR) {
    if (hw->api_min_ver > (EXP_FW_API_VER_MINOR + 2))
      ice_info(hw, "The driver for the device detected a newer version of the "
                   "NVM image than expected. Please install the most recent "
                   "version of the network driver.\n");
    else if ((hw->api_min_ver + 2) < EXP_FW_API_VER_MINOR)
      ice_info(hw, "The driver for the device detected an older version of the "
                   "NVM image than expected. Please update the NVM image.\n");
  } else {
 ...
 */
static enum ice_status ice_shutdown_rq(struct ice_hw *hw,
                                       struct ice_ctl_q_info *cq) {
  enum ice_status ret_code = ICE_SUCCESS;
 ...
 */
static enum ice_status ice_init_ctrlq(struct ice_hw *hw,
  ...
  /* verify input for valid configuration */
  if (!cq->num_rq_entries || !cq->num_sq_entries || !cq->rq_buf_size ||
      !cq->sq_buf_size) {
    return ICE_ERR_CFG;
  }
...
static void ice_debug_cq(struct ice_hw *hw, void *desc, void *buf,
  ...
  if (buf && cq_desc->datalen != 0 &&
      (flags & (ICE_AQ_FLAG_DD | ICE_AQ_FLAG_CMP) || flags & ICE_AQ_FLAG_RD)) {
...
enum ice_status ice_sq_send_cmd_nolock(struct ice_hw *hw,
  ...
  ice_memcpy(desc_on_ring, desc, sizeof(*desc_on_ring), ICE_NONDMA_TO_DMA);
  ...
  if (ice_sq_done(hw, cq)) {
    ice_memcpy(desc, desc_on_ring, sizeof(*desc), ICE_DMA_TO_NONDMA);
    if (buf) {
      ...
      if (copy_size > buf_size) {
        ...
        ice_memcpy(buf, dma_buf->va, copy_size, ICE_DMA_TO_NONDMA);
      }
    }
  ...
  if (details->wb_desc)
    ice_memcpy(details->wb_desc, desc_on_ring, sizeof(*details->wb_desc),
               ICE_DMA_TO_NONDMA);
...
enum ice_status ice_clean_rq_elem(struct ice_hw *hw, struct ice_ctl_q_info *cq,
  ...
  if (e->msg_buf && e->msg_len)
    ice_memcpy(e->msg_buf, cq->rq.r.rq_bi[desc_idx].va, e->msg_len,
               ICE_DMA_TO_NONDMA);
