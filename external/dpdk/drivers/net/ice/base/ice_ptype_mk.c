...
static void _parse_ptype_mk_tcam_item(struct ice_hw *hw, u16 idx, void *item,
  ...
  if (hw->debug_mask & ICE_DBG_PARSER)
    ice_ptype_mk_tcam_dump(hw, (struct ice_ptype_mk_tcam_item *)item);
}
...
struct ice_ptype_mk_tcam_item *ice_ptype_mk_tcam_table_get(struct ice_hw *hw) {
  return (struct ice_ptype_mk_tcam_item *)ice_parser_create_table(
      hw, ICE_SID_RXPARSER_MARKER_PTYPE, sizeof(struct ice_ptype_mk_tcam_item),
      ICE_PTYPE_MK_TCAM_TABLE_SIZE, ice_parser_sect_item_get,
...
ice_ptype_mk_tcam_match(struct ice_ptype_mk_tcam_item *table, u8 *pat,
                        int len) {
  int i;
