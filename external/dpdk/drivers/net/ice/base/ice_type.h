 ...
 */
static inline bool ice_is_pow2(u64 val) { return (val && !(val & (val - 1))); }
...
#define ICE_DBG_AQ                                                             \
  (ICE_DBG_AQ_MSG | ICE_DBG_AQ_DESC | ICE_DBG_AQ_DESC_BUF | ICE_DBG_AQ_CMD)
...
enum ice_phy_cache_mode { ICE_FC_MODE = 0, ICE_SPEED_MODE, ICE_FEC_MODE };

enum ice_fec_mode { ICE_FEC_NONE = 0, ICE_FEC_RS, ICE_FEC_BASER, ICE_FEC_AUTO };
...
#define ICE_TXSCHED_GET_CIR_BWALLOC(x) LE16_TO_CPU((x)->info.cir_bw.bw_alloc)
...
struct ice_hw {
  ...
  int (*aq_send_cmd_fn)(void *param, struct ice_aq_desc *desc, void *buf,
                        u16 buf_size);
...
#define ICE_NETLIST_ID_BLK_OFFSET(n)                                           \
  ICE_NETLIST_LINK_TOPO_OFFSET(0x0004 + 2 * (n))
