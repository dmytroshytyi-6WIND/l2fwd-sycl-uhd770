...
static __rte_always_inline uint32_t readl(volatile void *addr) {
  return rte_le_to_cpu_32(rte_read32(addr));
}

static __rte_always_inline void writel(uint32_t value, volatile void *addr) {
  rte_write32(rte_cpu_to_le_32(value), addr);
}

static __rte_always_inline void writel_relaxed(uint32_t value,
                                               volatile void *addr) {
  rte_write32_relaxed(rte_cpu_to_le_32(value), addr);
}

static __rte_always_inline uint64_t readq(volatile void *addr) {
  return rte_le_to_cpu_64(rte_read64(addr));
}

static __rte_always_inline void writeq(uint64_t value, volatile void *addr) {
...
#define ice_debug(h, m, s, ...)                                                \
  do {                                                                         \
    if (((m) & (h)->debug_mask))                                               \
      PMD_DRV_LOG_RAW(DEBUG, "ice %02x.%x " s, (h)->bus.device, (h)->bus.func, \
                      ##__VA_ARGS__);                                          \
  } while (0)
...
#define ice_debug_array(hw, type, rowsize, groupsize, buf, len)                \
  do {                                                                         \
    ...
    int i;                                                                     \
    for (i = 0; i < len_l; i += 8)                                             \
      ice_debug(hw_l, type, "0x%04X  0x%016" PRIx64 "\n", i,                   \
                *((u64 *)((buf_l) + i)));                                      \
  } while (0)
...
static inline void ice_init_lock(struct ice_lock *sp) {
  rte_spinlock_init(&sp->spinlock);
}

static inline void ice_acquire_lock(struct ice_lock *sp) {
  rte_spinlock_lock(&sp->spinlock);
}

static inline void ice_release_lock(struct ice_lock *sp) {
  rte_spinlock_unlock(&sp->spinlock);
}

static inline void ice_destroy_lock(__rte_unused struct ice_lock *sp) {}
...
static inline void *ice_alloc_dma_mem(__rte_unused struct ice_hw *hw,
  ...
  mz = rte_memzone_reserve_bounded(z_name, size, SOCKET_ID_ANY, 0, 0,
                                   RTE_PGSIZE_2M);
  if (!mz)
    return NULL;
  ...
  PMD_DRV_LOG(DEBUG,
              "memzone %s allocated with physical address: "
              "%" PRIu64,
              mz->name, mem->pa);
...
static inline void ice_free_dma_mem(__rte_unused struct ice_hw *hw,
                                    struct ice_dma_mem *mem) {
  PMD_DRV_LOG(DEBUG,
              "memzone %s to be freed with physical address: "
              "%" PRIu64,
...
static inline u8 ice_hweight8(u32 num) {
  u8 bits = 0;
  u32 i;
...
static inline u8 ice_hweight32(u32 num) {
  u8 bits = 0;
  u32 i;
...
#define LIST_FOR_EACH_ENTRY(pos, head, type, member)                           \
  for ((pos) = (head)->lh_first                                                \
                   ...
                   : 0;                                                        \
       (pos); (pos) = (pos)->member.next.le_next                               \
                          ? container_of((pos)->member.next.le_next,           \
                                         struct type, member)                  \
                          : 0)
...
#define LIST_FOR_EACH_ENTRY_SAFE(pos, tmp, head, type, member)                 \
  for ((pos) = (head)->lh_first                                                \
                   ...
                   : 0,                                                        \
      (tmp) = (pos) == 0 ? 0                                                   \
                         : ((pos)->member.next.le_next                         \
                                ? container_of((pos)->member.next.le_next,     \
                                               struct type, member)            \
                                : 0);                                          \
       (pos); (pos) = (tmp),                                                   \
      (tmp) = (pos) == 0 ? 0                                                   \
                         : ((tmp)->member.next.le_next                         \
                                ? container_of((pos)->member.next.le_next,     \
                                               struct type, member)            \
                                : 0))

#define LIST_REPLACE_INIT(list_head, head)                                     \
  do {                                                                         \
