...
ice_sched_add_root_node(struct ice_port_info *pi,
  ...
  root->children = (struct ice_sched_node **)ice_calloc(hw, hw->max_children[0],
                                                        sizeof(*root));
  if (!root->children) {
    ice_free(hw, root);
    return ICE_ERR_NO_MEMORY;
...
ice_sched_find_node_by_teid(struct ice_sched_node *start_node, u32 teid) {
  ...
  for (i = 0; i < start_node->num_children; i++) {
    ...
    tmp = ice_sched_find_node_by_teid(start_node->children[i], teid);
    if (tmp)
      return tmp;
  }
 ...
 */
enum ice_status ice_aq_query_sched_elems(struct ice_hw *hw, u16 elems_req,
                                         struct ice_aqc_txsched_elem_data *buf,
                                         u16 buf_size, u16 *elems_ret,
                                         struct ice_sq_cd *cd) {
  return ice_aqc_send_sched_elem_cmd(hw, ice_aqc_opc_get_sched_elems, elems_req,
                                     (void *)buf, buf_size, elems_ret, cd);
}
 ...
 */
enum ice_status ice_sched_add_node(struct ice_port_info *pi, u8 layer,
                                   struct ice_aqc_txsched_elem_data *info) {
  ...
  parent =
      ice_sched_find_node_by_teid(pi->root, LE32_TO_CPU(info->parent_teid));
  if (!parent) {
  ...
  if (hw->max_children[layer]) {
    /* coverity[suspicious_sizeof] */
    node->children = (struct ice_sched_node **)ice_calloc(
        hw, hw->max_children[layer], sizeof(*node));
    if (!node->children) {
      ice_free(hw, node);
      return ICE_ERR_NO_MEMORY;
    }
...
ice_aq_delete_sched_elems(struct ice_hw *hw, u16 grps_req,
  ...
  return ice_aqc_send_sched_elem_cmd(hw, ice_aqc_opc_delete_sched_elems,
                                     grps_req, (void *)buf, buf_size, grps_del,
                                     cd);
}
 ...
 */
static enum ice_status ice_sched_remove_elems(struct ice_hw *hw,
                                              struct ice_sched_node *parent,
  ...
  status = ice_aq_delete_sched_elems(hw, 1, buf, buf_size, &num_groups_removed,
                                     NULL);
...
void ice_free_sched_node(struct ice_port_info *pi,
  ...
  if (parent) {
    ...
    if (pi->sib_head[node->tc_num][node->tx_sched_layer] == node)
      pi->sib_head[node->tc_num][node->tx_sched_layer] = node->sibling;
  }
 ...
 */
static enum ice_status ice_aq_get_dflt_topo(struct ice_hw *hw, u8 lport,
                                            struct ice_aqc_get_topo_elem *buf,
                                            u16 buf_size, u8 *num_branches,
                                            struct ice_sq_cd *cd) {
 ...
 */
static enum ice_status ice_aq_add_sched_elems(struct ice_hw *hw, u16 grps_req,
                                              struct ice_aqc_add_elem *buf,
                                              u16 buf_size, u16 *grps_added,
                                              struct ice_sq_cd *cd) {
  return ice_aqc_send_sched_elem_cmd(hw, ice_aqc_opc_add_sched_elems, grps_req,
                                     (void *)buf, buf_size, grps_added, cd);
}
...
ice_aq_cfg_sched_elems(struct ice_hw *hw, u16 elems_req,
  ...
  return ice_aqc_send_sched_elem_cmd(hw, ice_aqc_opc_cfg_sched_elems, elems_req,
                                     (void *)buf, buf_size, elems_cfgd, cd);
}
 ...
 */
static enum ice_status ice_aq_move_sched_elems(struct ice_hw *hw, u16 grps_req,
                                               struct ice_aqc_move_elem *buf,
                                               u16 buf_size, u16 *grps_movd,
                                               struct ice_sq_cd *cd) {
  return ice_aqc_send_sched_elem_cmd(hw, ice_aqc_opc_move_sched_elems, grps_req,
                                     (void *)buf, buf_size, grps_movd, cd);
}
 ...
 */
static enum ice_status ice_aq_suspend_sched_elems(struct ice_hw *hw,
                                                  u16 elems_req, __le32 *buf,
                                                  u16 buf_size, u16 *elems_ret,
                                                  struct ice_sq_cd *cd) {
 ...
 */
static enum ice_status ice_aq_resume_sched_elems(struct ice_hw *hw,
                                                 u16 elems_req, __le32 *buf,
                                                 u16 buf_size, u16 *elems_ret,
                                                 struct ice_sq_cd *cd) {
 ...
 */
static enum ice_status ice_sched_suspend_resume_elems(struct ice_hw *hw,
                                                      u8 num_nodes,
                                                      u32 *node_teids,
                                                      bool suspend) {
  ...
  if (suspend)
    status = ice_aq_suspend_sched_elems(hw, num_nodes, buf, buf_size,
                                        &num_elem_ret, NULL);
  else
    status = ice_aq_resume_sched_elems(hw, num_nodes, buf, buf_size,
                                       &num_elem_ret, NULL);
 ...
 */
static enum ice_status ice_alloc_lan_q_ctx(struct ice_hw *hw, u16 vsi_handle,
                                           u8 tc, u16 new_numqs) {
  ...
  if (!vsi_ctx->lan_q_ctx[tc]) {
    vsi_ctx->lan_q_ctx[tc] =
        (struct ice_q_ctx *)ice_calloc(hw, new_numqs, sizeof(*q_ctx));
    if (!vsi_ctx->lan_q_ctx[tc])
      return ICE_ERR_NO_MEMORY;
  ...
  if (new_numqs > vsi_ctx->num_lan_q_entries[tc]) {
    ...
    q_ctx = (struct ice_q_ctx *)ice_calloc(hw, new_numqs, sizeof(*q_ctx));
    if (!q_ctx)
      return ICE_ERR_NO_MEMORY;
    ice_memcpy(q_ctx, vsi_ctx->lan_q_ctx[tc], prev_num * sizeof(*q_ctx),
               ICE_DMA_TO_NONDMA);
...
ice_aq_add_rl_profile(struct ice_hw *hw, u16 num_profiles,
  ...
  return ice_aq_rl_profile(hw, ice_aqc_opc_add_rl_profiles, num_profiles, buf,
 ...
 */
enum ice_status ice_aq_query_rl_profile(struct ice_hw *hw, u16 num_profiles,
                                        struct ice_aqc_rl_profile_elem *buf,
                                        u16 buf_size, struct ice_sq_cd *cd) {
  return ice_aq_rl_profile(hw, ice_aqc_opc_query_rl_profiles, num_profiles, buf,
                           buf_size, NULL, cd);
}
...
ice_aq_remove_rl_profile(struct ice_hw *hw, u16 num_profiles,
  ...
  return ice_aq_rl_profile(hw, ice_aqc_opc_remove_rl_profiles, num_profiles,
                           buf, buf_size, num_profiles_removed, cd);
}
...
static void ice_sched_clear_rl_prof(struct ice_port_info *pi) {
  ...
  for (ln = 0; ln < hw->num_tx_sched_layers; ln++) {
    ...
    LIST_FOR_EACH_ENTRY_SAFE(rl_prof_elem, rl_prof_tmp, &hw->rl_prof_list[ln],
...
void ice_sched_clear_agg(struct ice_hw *hw) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(agg_info, atmp, &hw->agg_list, ice_sched_agg_info,
                           list_entry) {
    struct ice_sched_agg_vsi_info *agg_vsi_info;
    struct ice_sched_agg_vsi_info *vtmp;

    LIST_FOR_EACH_ENTRY_SAFE(agg_vsi_info, vtmp, &agg_info->agg_vsi_list,
 ...
 */
enum ice_status ice_aq_cfg_l2_node_cgd(struct ice_hw *hw, u16 num_l2_nodes,
...
ice_sched_add_elems(struct ice_port_info *pi, struct ice_sched_node *tc_node,
  ...
  for (i = 0; i < num_nodes; i++) {
    ...
    buf->generic[i].data.valid_sections = ICE_AQC_ELEM_VALID_GENERIC |
                                          ICE_AQC_ELEM_VALID_CIR |
                                          ICE_AQC_ELEM_VALID_EIR;
    buf->generic[i].data.generic = 0;
    buf->generic[i].data.cir_bw.bw_profile_idx =
        CPU_TO_LE16(ICE_SCHED_DFLT_RL_PROF_ID);
    buf->generic[i].data.cir_bw.bw_alloc = CPU_TO_LE16(ICE_SCHED_DFLT_BW_WT);
    buf->generic[i].data.eir_bw.bw_profile_idx =
        CPU_TO_LE16(ICE_SCHED_DFLT_RL_PROF_ID);
    buf->generic[i].data.eir_bw.bw_alloc = CPU_TO_LE16(ICE_SCHED_DFLT_BW_WT);
  }

  status =
      ice_aq_add_sched_elems(hw, 1, buf, buf_size, &num_groups_added, NULL);
 ...
 */
static enum ice_status ice_sched_add_nodes_to_hw_layer(
    struct ice_port_info *pi, struct ice_sched_node *tc_node,
    struct ice_sched_node *parent, u8 layer, u16 num_nodes,
 ...
 */
static enum ice_status ice_sched_add_nodes_to_layer(
    struct ice_port_info *pi, struct ice_sched_node *tc_node,
    struct ice_sched_node *parent, u8 layer, u16 num_nodes,
    u32 *first_node_teid, u16 *num_nodes_added) {
  ...
  *num_nodes_added = 0;
  while (*num_nodes_added < num_nodes) {
    ...
    status = ice_sched_add_nodes_to_hw_layer(
        pi, tc_node, parent, layer, new_num_nodes, first_teid_ptr, &num_added);
    if (status == ICE_SUCCESS)
...
enum ice_status ice_sched_init_port(struct ice_port_info *pi) {
  ...
  buf = (struct ice_aqc_get_topo_elem *)ice_malloc(hw, ICE_AQ_MAX_BUF_LEN);
  if (!buf)
    return ICE_ERR_NO_MEMORY;
   ...
   */
  if (num_elems > 2 &&
      buf[0].generic[num_elems - 1].data.elem_type == ICE_AQC_ELEM_TYPE_LEAF)
    pi->last_node_teid = LE32_TO_CPU(buf[0].generic[num_elems - 2].node_teid);
  else
    pi->last_node_teid = LE32_TO_CPU(buf[0].generic[num_elems - 1].node_teid);
  ...
  for (i = 0; i < num_branches; i++) {
    ...
    for (j = 1; j < num_elems; j++) {
      ...
      if (buf[0].generic[j].data.elem_type == ICE_AQC_ELEM_TYPE_ENTRY_POINT)
...
enum ice_status ice_sched_query_res_alloc(struct ice_hw *hw) {
  ...
  buf = (struct ice_aqc_query_txsched_res_resp *)ice_malloc(hw, sizeof(*buf));
  if (!buf)
    return ICE_ERR_NO_MEMORY;
  ...
  hw->num_tx_sched_phys_layers = LE16_TO_CPU(buf->sched_props.phys_levels);
  ...
  hw->layer_info = (struct ice_aqc_layer_props *)ice_memdup(
      hw, buf->layer_props, (hw->num_tx_sched_layers * sizeof(*hw->layer_info)),
 ...
 * subtree or not
 */
bool ice_sched_find_node_in_subtree(struct ice_hw *hw,
...
ice_sched_get_free_qgrp(struct ice_port_info *pi,
  ...
  while (qgrp_node) {
    ...
    if (ice_sched_find_node_in_subtree(pi->hw, vsi_node, qgrp_node))
      if (qgrp_node->num_children < min_children && qgrp_node->owner == owner) {
...
struct ice_sched_node *ice_sched_get_free_qparent(struct ice_port_info *pi,
  ...
  while (qgrp_node) {
    /* make sure the qgroup node is part of the VSI subtree */
    if (ice_sched_find_node_in_subtree(pi->hw, vsi_node, qgrp_node))
      if (qgrp_node->num_children < max_children && qgrp_node->owner == owner)
        break;
    qgrp_node = qgrp_node->sibling;
  }
 ...
 * TC branch
 */
struct ice_sched_node *ice_sched_get_vsi_node(struct ice_port_info *pi,
 ...
 */
static bool ice_sched_check_node(struct ice_hw *hw,
                                 struct ice_sched_node *node) {
  struct ice_aqc_txsched_elem_data buf;
  enum ice_status status;
  u32 node_teid;
...
ice_sched_add_vsi_child_nodes(struct ice_port_info *pi, u16 vsi_handle,
  ...
  for (i = vsil + 1; i <= qgl; i++) {
    ...
    status = ice_sched_add_nodes_to_layer(pi, tc_node, parent, i, num_nodes[i],
                                          &first_node_teid, &num_added);
 ...
 */
static enum ice_status ice_sched_add_vsi_to_topo(struct ice_port_info *pi,
                                                 u16 vsi_handle, u8 tc) {
  ...
  return ice_sched_add_vsi_support_nodes(pi, vsi_handle, tc_node, num_nodes);
}
...
ice_sched_update_vsi_child_nodes(struct ice_port_info *pi, u16 vsi_handle,
   ...
   */
  status = ice_sched_add_vsi_child_nodes(pi, vsi_handle, tc_node, new_num_nodes,
                                         owner);
  if (status)
    return status;
 ...
 */
enum ice_status ice_sched_cfg_vsi(struct ice_port_info *pi, u16 vsi_handle,
                                  u8 tc, u16 maxqs, u8 owner, bool enable) {
  ...
  if (!enable) {
    if (vsi_node && vsi_node->in_use) {
      ...
      status = ice_sched_suspend_resume_elems(hw, 1, &teid, true);
      if (!status)
  ...
  status = ice_sched_update_vsi_child_nodes(pi, vsi_handle, tc, maxqs, owner);
  if (status)
    return status;
 ...
 * scheduler tree.
 */
static enum ice_status ice_sched_rm_vsi_cfg(struct ice_port_info *pi,
                                            u16 vsi_handle, u8 owner) {
 ...
 * This function needs to be called with the port_info->sched_lock held
 */
bool ice_sched_is_tree_balanced(struct ice_hw *hw,
                                struct ice_sched_node *node) {
  u8 i;
 ...
 */
enum ice_status ice_aq_query_node_to_root(struct ice_hw *hw, u32 node_teid,
                                          struct ice_aqc_txsched_elem_data *buf,
                                          u16 buf_size, struct ice_sq_cd *cd) {
...
static struct ice_sched_agg_info *ice_get_agg_info(struct ice_hw *hw,
  ...
  LIST_FOR_EACH_ENTRY(agg_info, &hw->agg_list, ice_sched_agg_info, list_entry)
...
ice_sched_get_free_vsi_parent(struct ice_hw *hw, struct ice_sched_node *node,
  ...
  for (i = 0; i < node->num_children; i++) {
    ...
    parent = ice_sched_get_free_vsi_parent(hw, node->children[i], num_nodes);
    if (parent)
      return parent;
  }
 ...
 * parent
 */
static void ice_sched_update_parent(struct ice_sched_node *new_parent,
                                    struct ice_sched_node *node) {
  ...
  for (i = 0; i < old_parent->num_children; i++)
    if (old_parent->children[i] == node) {
      for (j = i + 1; j < old_parent->num_children; j++)
        old_parent->children[j - 1] = old_parent->children[j];
 ...
 */
static enum ice_status ice_sched_move_nodes(struct ice_port_info *pi,
                                            struct ice_sched_node *parent,
  ...
  for (i = 0; i < num_items; i++) {
    ...
    status = ice_aq_move_sched_elems(hw, 1, buf, buf_len, &grps_movd, NULL);
 ...
 */
static enum ice_status ice_sched_move_vsi_to_agg(struct ice_port_info *pi,
                                                 u16 vsi_handle, u32 agg_id,
                                                 u8 tc) {
  ...
  for (i = 0; i < agg_node->num_children; i++) {
    parent =
        ice_sched_get_free_vsi_parent(pi->hw, agg_node->children[i], num_nodes);
    if (parent)
      goto move_nodes;
  }
  ...
  for (i = aggl + 1; i < vsil; i++) {
    status = ice_sched_add_nodes_to_layer(pi, tc_node, parent, i, num_nodes[i],
                                          &first_node_teid, &num_nodes_added);
...
ice_move_all_vsi_to_dflt_agg(struct ice_port_info *pi,
  ...
  LIST_FOR_EACH_ENTRY_SAFE(agg_vsi_info, tmp, &agg_info->agg_vsi_list,
    ...
    status = ice_sched_move_vsi_to_agg(pi, vsi_handle, ICE_DFLT_AGG_ID, tc);
    if (status)
      break;
 ...
 * from the given TC
 */
static enum ice_status ice_sched_rm_agg_cfg(struct ice_port_info *pi,
                                            u32 agg_id, u8 tc) {
 ...
 */
static enum ice_status ice_rm_agg_cfg_tc(struct ice_port_info *pi,
                                         struct ice_sched_agg_info *agg_info,
 ...
 * lock held.
 */
static enum ice_status ice_save_agg_tc_bitmap(struct ice_port_info *pi,
                                              u32 agg_id,
  ...
  if (!agg_info)
    return ICE_ERR_PARAM;
  ice_cp_bitmap(agg_info->replay_tc_bitmap, tc_bitmap, ICE_MAX_TRAFFIC_CLASS);
  return ICE_SUCCESS;
}
 ...
 * for the given TC
 */
static enum ice_status ice_sched_add_agg_cfg(struct ice_port_info *pi,
                                             u32 agg_id, u8 tc) {
 ...
 */
static enum ice_status ice_sched_cfg_agg(struct ice_port_info *pi, u32 agg_id,
  ...
  if (!agg_info) {
    ...
    agg_info = (struct ice_sched_agg_info *)ice_malloc(hw, sizeof(*agg_info));
    if (!agg_info)
      return ICE_ERR_NO_MEMORY;
 ...
 */
enum ice_status ice_cfg_agg(struct ice_port_info *pi, u32 agg_id,
                            enum ice_agg_type agg_type, u8 tc_bitmap) {
  ice_bitmap_t bitmap = tc_bitmap;
  enum ice_status status;

  ice_acquire_lock(&pi->sched_lock);
  status = ice_sched_cfg_agg(pi, agg_id, agg_type, (ice_bitmap_t *)&bitmap);
  if (!status)
    status = ice_save_agg_tc_bitmap(pi, agg_id, (ice_bitmap_t *)&bitmap);
  ice_release_lock(&pi->sched_lock);
  return status;
}
 ...
 * lock held.
 */
static enum ice_status ice_save_agg_vsi_tc_bitmap(struct ice_port_info *pi,
                                                  u32 agg_id, u16 vsi_handle,
                                                  ice_bitmap_t *tc_bitmap) {
  struct ice_sched_agg_vsi_info *agg_vsi_info;
  struct ice_sched_agg_info *agg_info;
 ...
 */
static enum ice_status ice_sched_assoc_vsi_to_agg(struct ice_port_info *pi,
                                                  u32 agg_id, u16 vsi_handle,
                                                  ice_bitmap_t *tc_bitmap) {
  ...
  if (!agg_vsi_info) {
    ...
    agg_vsi_info =
        (struct ice_sched_agg_vsi_info *)ice_malloc(hw, sizeof(*agg_vsi_info));
    if (!agg_vsi_info)
      return ICE_ERR_PARAM;
...
ice_sched_update_elem(struct ice_hw *hw, struct ice_sched_node *node,
  ...
  status = ice_aq_cfg_sched_elems(hw, num_elems, &buf, sizeof(buf), &elem_cfgd,
                                  NULL);
 ...
 */
static enum ice_status ice_sched_cfg_node_bw_alloc(struct ice_hw *hw,
                                                   struct ice_sched_node *node,
 ...
 */
enum ice_status ice_move_vsi_to_agg(struct ice_port_info *pi, u32 agg_id,
                                    u16 vsi_handle, u8 tc_bitmap) {
  ice_bitmap_t bitmap = tc_bitmap;
  enum ice_status status;
 ...
 * bw_t_info.
 */
static void ice_set_clear_cir_bw_alloc(struct ice_bw_type_info *bw_t_info,
                                       u16 bw_alloc) {
 ...
 * bw_t_info.
 */
static void ice_set_clear_eir_bw_alloc(struct ice_bw_type_info *bw_t_info,
                                       u16 bw_alloc) {
 ...
 */
static enum ice_status ice_sched_save_vsi_bw_alloc(struct ice_port_info *pi,
                                                   u16 vsi_handle, u8 tc,
                                                   enum ice_rl_type rl_type,
                                                   u16 bw_alloc) {
  struct ice_vsi_ctx *vsi_ctx;
  ...
  switch (rl_type) {
  case ICE_MIN_BW:
    ice_set_clear_cir_bw_alloc(&vsi_ctx->sched.bw_t_info[tc], bw_alloc);
    break;
  case ICE_MAX_BW:
    ice_set_clear_eir_bw_alloc(&vsi_ctx->sched.bw_t_info[tc], bw_alloc);
    break;
  default:
    return ICE_ERR_PARAM;
  }
 ...
 */
static enum ice_status ice_sched_save_vsi_bw(struct ice_port_info *pi,
                                             u16 vsi_handle, u8 tc,
 ...
 */
static enum ice_status ice_sched_save_vsi_prio(struct ice_port_info *pi,
                                               u16 vsi_handle, u8 tc, u8 prio) {
  struct ice_vsi_ctx *vsi_ctx;
 ...
 */
static enum ice_status ice_sched_save_agg_bw_alloc(struct ice_port_info *pi,
                                                   u32 agg_id, u8 tc,
                                                   enum ice_rl_type rl_type,
                                                   u16 bw_alloc) {
  struct ice_sched_agg_info *agg_info;
 ...
 */
static enum ice_status ice_sched_save_agg_bw(struct ice_port_info *pi,
                                             u32 agg_id, u8 tc,
                                             enum ice_rl_type rl_type, u32 bw) {
  struct ice_sched_agg_info *agg_info;
 ...
 * information.
 */
enum ice_status ice_cfg_vsi_bw_lmt_per_tc(struct ice_port_info *pi,
                                          u16 vsi_handle, u8 tc,
                                          enum ice_rl_type rl_type, u32 bw) {
  enum ice_status status;

  status = ice_sched_set_node_bw_lmt_per_tc(pi, vsi_handle, ICE_AGG_TYPE_VSI,
                                            tc, rl_type, bw);
  if (!status) {
    ice_acquire_lock(&pi->sched_lock);
 ...
 * information.
 */
enum ice_status ice_cfg_vsi_bw_dflt_lmt_per_tc(struct ice_port_info *pi,
                                               u16 vsi_handle, u8 tc,
                                               enum ice_rl_type rl_type) {
  enum ice_status status;

  status = ice_sched_set_node_bw_lmt_per_tc(pi, vsi_handle, ICE_AGG_TYPE_VSI,
                                            tc, rl_type, ICE_SCHED_DFLT_BW);
  if (!status) {
    ice_acquire_lock(&pi->sched_lock);
    status =
        ice_sched_save_vsi_bw(pi, vsi_handle, tc, rl_type, ICE_SCHED_DFLT_BW);
    ice_release_lock(&pi->sched_lock);
  }
  return status;
}
 ...
 * information.
 */
enum ice_status ice_cfg_agg_bw_lmt_per_tc(struct ice_port_info *pi, u32 agg_id,
                                          u8 tc, enum ice_rl_type rl_type,
                                          u32 bw) {
  enum ice_status status;

  status = ice_sched_set_node_bw_lmt_per_tc(pi, agg_id, ICE_AGG_TYPE_AGG, tc,
                                            rl_type, bw);
  if (!status) {
    ice_acquire_lock(&pi->sched_lock);
 ...
 * on TC information.
 */
enum ice_status ice_cfg_agg_bw_dflt_lmt_per_tc(struct ice_port_info *pi,
                                               u32 agg_id, u8 tc,
                                               enum ice_rl_type rl_type) {
  enum ice_status status;

  status = ice_sched_set_node_bw_lmt_per_tc(pi, agg_id, ICE_AGG_TYPE_AGG, tc,
                                            rl_type, ICE_SCHED_DFLT_BW);
  if (!status) {
    ice_acquire_lock(&pi->sched_lock);
    status = ice_sched_save_agg_bw(pi, agg_id, tc, rl_type, ICE_SCHED_DFLT_BW);
    ice_release_lock(&pi->sched_lock);
  }
  return status;
}
 ...
 */
enum ice_status ice_cfg_vsi_bw_shared_lmt(struct ice_port_info *pi,
                                          u16 vsi_handle, u32 min_bw,
 ...
 */
enum ice_status ice_cfg_vsi_bw_no_shared_lmt(struct ice_port_info *pi,
                                             u16 vsi_handle) {
  return ice_sched_set_vsi_bw_shared_lmt(pi, vsi_handle, ICE_SCHED_DFLT_BW,
 ...
 */
enum ice_status ice_cfg_agg_bw_shared_lmt(struct ice_port_info *pi, u32 agg_id,
                                          u32 min_bw, u32 max_bw,
                                          u32 shared_bw) {
  return ice_sched_set_agg_bw_shared_lmt(pi, agg_id, min_bw, max_bw, shared_bw);
}
 ...
 * nodes across all traffic classes for aggregator matching agg_id.
 */
enum ice_status ice_cfg_agg_bw_no_shared_lmt(struct ice_port_info *pi,
                                             u32 agg_id) {
 ...
 * nodes across all traffic classes for aggregator matching agg_id.
 */
enum ice_status ice_cfg_agg_bw_shared_lmt_per_tc(struct ice_port_info *pi,
                                                 u32 agg_id, u8 tc, u32 min_bw,
                                                 u32 max_bw, u32 shared_bw) {
  return ice_sched_set_agg_bw_shared_lmt_per_tc(pi, agg_id, tc, min_bw, max_bw,
                                                shared_bw);
}
 ...
 * nodes across all traffic classes for aggregator matching agg_id.
 */
enum ice_status ice_cfg_agg_bw_no_shared_lmt_per_tc(struct ice_port_info *pi,
                                                    u32 agg_id, u8 tc) {
  return ice_sched_set_agg_bw_shared_lmt_per_tc(
      pi, agg_id, tc, ICE_SCHED_DFLT_BW, ICE_SCHED_DFLT_BW, ICE_SCHED_DFLT_BW);
}
 ...
 */
enum ice_status ice_cfg_vsi_q_priority(struct ice_port_info *pi, u16 num_qs,
                                       u32 *q_ids, u8 *q_prio) {
  enum ice_status status = ICE_ERR_PARAM;
  u16 i;
 ...
 */
enum ice_status ice_cfg_agg_vsi_priority_per_tc(struct ice_port_info *pi,
                                                u32 agg_id, u16 num_vsis,
                                                u16 *vsi_handle_arr,
                                                u8 *node_prio, u8 tc) {
  struct ice_sched_agg_vsi_info *agg_vsi_info;
  struct ice_sched_node *tc_node, *agg_node;
  ...
  for (i = 0; i < num_vsis; i++) {
    ...
    if (ice_sched_find_node_in_subtree(hw, agg_node, vsi_node)) {
      /* Configure Priority */
      status = ice_sched_cfg_sibl_node_prio(pi, vsi_node, node_prio[i]);
      if (status)
        break;
      status = ice_sched_save_vsi_prio(pi, vsi_handle, tc, node_prio[i]);
      if (status)
        break;
    }
  }
 ...
 */
enum ice_status ice_cfg_vsi_bw_alloc(struct ice_port_info *pi, u16 vsi_handle,
                                     u8 ena_tcmap, enum ice_rl_type rl_type,
                                     u8 *bw_alloc) {
  enum ice_status status = ICE_SUCCESS;
  u8 tc;
  ...
  ice_for_each_traffic_class(tc) {
    ...
    status =
        ice_sched_cfg_node_bw_alloc(pi->hw, vsi_node, rl_type, bw_alloc[tc]);
    if (status)
      break;
    status =
        ice_sched_save_vsi_bw_alloc(pi, vsi_handle, tc, rl_type, bw_alloc[tc]);
    if (status)
      break;
  }
 ...
 */
enum ice_status ice_cfg_agg_bw_alloc(struct ice_port_info *pi, u32 agg_id,
                                     u8 ena_tcmap, enum ice_rl_type rl_type,
                                     u8 *bw_alloc) {
  struct ice_sched_agg_info *agg_info;
  bool agg_id_present = false;
  ...
  ice_for_each_traffic_class(tc) {
    ...
    status = ice_sched_cfg_node_bw_alloc(hw, agg_node, rl_type, bw_alloc[tc]);
    if (status)
      break;
    status = ice_sched_save_agg_bw_alloc(pi, agg_id, tc, rl_type, bw_alloc[tc]);
    if (status)
      break;
  }
...
static u16 ice_sched_calc_wakeup(struct ice_hw *hw, s32 bw) {
  ...
  if (wakeup_int > 63) {
    ...
    wakeup_a = DIV_64BIT((s64)ICE_RL_PROF_MULTIPLIER * hw->psm_clk_freq,
                         bytes_per_sec);
    ...
    wakeup_f_int =
        (s32)DIV_64BIT(wakeup_f * ICE_RL_PROF_FRACTION, ICE_RL_PROF_MULTIPLIER);
...
ice_sched_bw_to_rl_profile(struct ice_hw *hw, u32 bw,
  ...
  for (i = 0; i < 64; i++) {
    ...
    /* Multiplier value */
    mv_tmp = DIV_64BIT(bytes_per_sec * ICE_RL_PROF_MULTIPLIER, ts_rate);
...
ice_sched_add_rl_profile(struct ice_hw *hw, enum ice_rl_type rl_type, u32 bw,
                         u8 layer_num) {
  struct ice_aqc_rl_profile_info *rl_prof_elem;
  ...
  rl_prof_elem =
      (struct ice_aqc_rl_profile_info *)ice_malloc(hw, sizeof(*rl_prof_elem));
 ...
 */
static enum ice_status ice_sched_cfg_node_bw_lmt(struct ice_hw *hw,
                                                 struct ice_sched_node *node,
                                                 enum ice_rl_type rl_type,
                                                 u16 rl_prof_id) {
  struct ice_aqc_txsched_elem_data buf;
 ...
 * scheduler lock.
 */
static enum ice_status ice_sched_rm_rl_profile(struct ice_hw *hw, u8 layer_num,
                                               u8 profile_type,
                                               u16 profile_id) {
  struct ice_aqc_rl_profile_info *rl_prof_elem;
  enum ice_status status = ICE_SUCCESS;
  ...
  if ((rl_prof_elem->profile.flags & ICE_AQC_RL_PROFILE_TYPE_M) ==
          profile_type &&
      LE16_TO_CPU(rl_prof_elem->profile.profile_id) == profile_id) {
 ...
 */
static enum ice_status ice_sched_set_node_bw_dflt(struct ice_port_info *pi,
                                                  struct ice_sched_node *node,
                                                  enum ice_rl_type rl_type,
                                                  u8 layer_num) {
  enum ice_status status;
  ...
  if (old_id == ICE_SCHED_DFLT_RL_PROF_ID || old_id == ICE_SCHED_INVAL_PROF_ID)
    return ICE_SUCCESS;
 ...
 */
static enum ice_status ice_sched_set_node_bw(struct ice_port_info *pi,
                                             struct ice_sched_node *node,
                                             enum ice_rl_type rl_type, u32 bw,
                                             u8 layer_num) {
  ...
  return ice_sched_rm_rl_profile(
      hw, layer_num, rl_prof_info->profile.flags & ICE_AQC_RL_PROFILE_TYPE_M,
      old_id);
}
 ...
 * settings.
 */
static enum ice_status ice_sched_set_node_bw_lmt(struct ice_port_info *pi,
                                                 struct ice_sched_node *node,
                                                 enum ice_rl_type rl_type,
                                                 u32 bw) {
  struct ice_hw *hw;
  ...
  layer_num = ice_sched_get_rl_prof_layer(pi, rl_type, node->tx_sched_layer);
...
ice_sched_set_node_bw_dflt_lmt(struct ice_port_info *pi,
                               struct ice_sched_node *node,
                               enum ice_rl_type rl_type) {
  return ice_sched_set_node_bw_lmt(pi, node, rl_type, ICE_SCHED_DFLT_BW);
}
 ...
 */
static enum ice_status ice_sched_validate_srl_node(struct ice_sched_node *node,
                                                   u8 sel_layer) {
  ...
  if (sel_layer == node->tx_sched_layer ||
      ((sel_layer == node->tx_sched_layer + 1) && node->num_children == 1) ||
 ...
 */
static enum ice_status ice_sched_save_q_bw(struct ice_q_ctx *q_ctx,
                                           enum ice_rl_type rl_type, u32 bw) {
  switch (rl_type) {
  case ICE_MIN_BW:
...
ice_sched_set_q_bw_lmt(struct ice_port_info *pi, u16 vsi_handle, u8 tc,
  ...
  if (rl_type == ICE_SHARED_BW) {
    ...
    sel_layer = ice_sched_get_rl_prof_layer(pi, rl_type, node->tx_sched_layer);
 ...
 */
enum ice_status ice_cfg_q_bw_lmt(struct ice_port_info *pi, u16 vsi_handle,
                                 u8 tc, u16 q_handle, enum ice_rl_type rl_type,
                                 u32 bw) {
  return ice_sched_set_q_bw_lmt(pi, vsi_handle, tc, q_handle, rl_type, bw);
}
 ...
 */
enum ice_status ice_cfg_q_bw_dflt_lmt(struct ice_port_info *pi, u16 vsi_handle,
                                      u8 tc, u16 q_handle,
                                      enum ice_rl_type rl_type) {
 ...
 */
static enum ice_status ice_sched_save_tc_node_bw(struct ice_port_info *pi,
                                                 u8 tc,
                                                 enum ice_rl_type rl_type,
                                                 u32 bw) {
  if (tc >= ICE_MAX_TRAFFIC_CLASS)
 ...
 */
static enum ice_status ice_sched_set_tc_node_bw_lmt(struct ice_port_info *pi,
                                                    u8 tc,
                                                    enum ice_rl_type rl_type,
                                                    u32 bw) {
  enum ice_status status = ICE_ERR_PARAM;
 ...
 */
enum ice_status ice_cfg_tc_node_bw_lmt(struct ice_port_info *pi, u8 tc,
                                       enum ice_rl_type rl_type, u32 bw) {
 ...
 */
enum ice_status ice_cfg_tc_node_bw_dflt_lmt(struct ice_port_info *pi, u8 tc,
                                            enum ice_rl_type rl_type) {
 ...
 * Save BW alloc information of VSI type node for post replay use.
 */
static enum ice_status ice_sched_save_tc_node_bw_alloc(struct ice_port_info *pi,
                                                       u8 tc,
                                                       enum ice_rl_type rl_type,
                                                       u16 bw_alloc) {
  if (tc >= ICE_MAX_TRAFFIC_CLASS)
  ...
  switch (rl_type) {
  case ICE_MIN_BW:
    ice_set_clear_cir_bw_alloc(&pi->tc_node_bw_t_info[tc], bw_alloc);
    break;
  case ICE_MAX_BW:
    ice_set_clear_eir_bw_alloc(&pi->tc_node_bw_t_info[tc], bw_alloc);
    break;
  default:
    return ICE_ERR_PARAM;
  }
 ...
 */
static enum ice_status ice_sched_set_tc_node_bw_alloc(struct ice_port_info *pi,
                                                      u8 tc,
                                                      enum ice_rl_type rl_type,
                                                      u8 bw_alloc) {
  enum ice_status status = ICE_ERR_PARAM;
  ...
  status = ice_sched_cfg_node_bw_alloc(pi->hw, tc_node, rl_type, bw_alloc);
  if (status)
    goto exit_set_tc_node_bw_alloc;
 ...
 * Note: The minimum guaranteed reservation is done via DCBX.
 */
enum ice_status ice_cfg_tc_node_bw_alloc(struct ice_port_info *pi, u8 tc,
                                         enum ice_rl_type rl_type,
                                         u8 bw_alloc) {
 ...
 */
enum ice_status ice_sched_set_agg_bw_dflt_lmt(struct ice_port_info *pi,
                                              u16 vsi_handle) {
  struct ice_vsi_ctx *vsi_ctx;
  enum ice_status status = ICE_SUCCESS;
  u8 tc;
  ...
  ice_for_each_traffic_class(tc) {
    ...
    status = ice_sched_set_node_bw_dflt_lmt(pi, node, ICE_SHARED_BW);
    if (status)
      break;
  }
 ...
 * called with scheduler lock held.
 */
static enum ice_status ice_sched_validate_vsi_srl_node(struct ice_port_info *pi,
                                                       u16 vsi_handle) {
  ...
  ice_for_each_traffic_class(tc) {
    ...
    if (sel_layer == ICE_SCHED_INVAL_LAYER_NUM) {
      ...
      layer_num = ice_sched_get_rl_prof_layer(pi, rl_type, node_layer);
...
ice_sched_set_vsi_node_srl_per_tc(struct ice_port_info *pi, u16 vsi_handle,
  ...
  layer_num =
      ice_sched_get_rl_prof_layer(pi, ICE_SHARED_BW, vsi_node->tx_sched_layer);
  if (layer_num >= pi->hw->num_tx_sched_layers)
    return ICE_ERR_PARAM;
  ...
  status = ice_sched_set_save_vsi_srl_node_bw(pi, vsi_handle, tc, cfg_node,
                                              ICE_MIN_BW, min_bw);
  if (status)
    return status;
 ...
 */
enum ice_status ice_sched_set_vsi_bw_shared_lmt(struct ice_port_info *pi,
                                                u16 vsi_handle, u32 min_bw,
                                                u32 max_bw, u32 shared_bw) {
  enum ice_status status = ICE_SUCCESS;
  u8 tc;
  ...
  ice_for_each_traffic_class(tc) {
    ...
    status = ice_sched_set_vsi_node_srl_per_tc(pi, vsi_handle, tc, min_bw,
                                               max_bw, shared_bw);
    if (status)
      break;
  }
 ...
 * called with scheduler lock held.
 */
static enum ice_status ice_sched_validate_agg_srl_node(struct ice_port_info *pi,
                                                       u32 agg_id) {
  u8 sel_layer = ICE_SCHED_INVAL_LAYER_NUM;
  struct ice_sched_agg_info *agg_info;
  bool agg_id_present = false;
 ...
 */
static enum ice_status ice_sched_validate_agg_id(struct ice_port_info *pi,
                                                 u32 agg_id) {
  struct ice_sched_agg_info *agg_info;
  ...
  LIST_FOR_EACH_ENTRY_SAFE(agg_info, tmp, &pi->hw->agg_list, ice_sched_agg_info,
                           list_entry)
  if (agg_info->agg_id == agg_id) {
    agg_id_present = true;
    break;
 ...
 */
static enum ice_status
ice_sched_set_agg_node_srl_per_tc(struct ice_port_info *pi, u32 agg_id, u8 tc,
  ...
  layer_num =
      ice_sched_get_rl_prof_layer(pi, rl_type, agg_node->tx_sched_layer);
  if (layer_num >= pi->hw->num_tx_sched_layers)
    return ICE_ERR_PARAM;
 ...
 * node(s).
 */
enum ice_status ice_sched_set_agg_bw_shared_lmt(struct ice_port_info *pi,
                                                u32 agg_id, u32 min_bw,
                                                u32 max_bw, u32 shared_bw) {
  enum ice_status status;
  u8 tc;
  ...
  ice_for_each_traffic_class(tc) {
    ...
    status = ice_sched_set_agg_node_srl_per_tc(pi, agg_id, tc, min_bw, max_bw,
                                               shared_bw);
    if (status)
      break;
  }
 ...
 */
enum ice_status ice_sched_set_agg_bw_shared_lmt_per_tc(struct ice_port_info *pi,
                                                       u32 agg_id, u8 tc,
                                                       u32 min_bw, u32 max_bw,
                                                       u32 shared_bw) {
  enum ice_status status;
 ...
 * function needs to be called with scheduler lock held.
 */
enum ice_status ice_sched_cfg_sibl_node_prio(struct ice_port_info *pi,
                                             struct ice_sched_node *node,
                                             u8 priority) {
  struct ice_aqc_txsched_elem_data buf;
  struct ice_aqc_txsched_elem *data;
  ...
  priority =
      (priority << ICE_AQC_ELEM_GENERIC_PRIO_S) & ICE_AQC_ELEM_GENERIC_PRIO_M;
...
enum ice_status ice_cfg_rl_burst_size(struct ice_hw *hw, u32 bytes) {
  ...
  if (bytes < ICE_MIN_BURST_SIZE_ALLOWED || bytes > ICE_MAX_BURST_SIZE_ALLOWED)
    return ICE_ERR_PARAM;
  if (ice_round_to_num(bytes, 64) <= ICE_MAX_BURST_SIZE_64_BYTE_GRANULARITY) {
 ...
 * needs to be called with scheduler lock held.
 */
static enum ice_status ice_sched_replay_node_prio(struct ice_hw *hw,
                                                  struct ice_sched_node *node,
                                                  u8 priority) {
  struct ice_aqc_txsched_elem_data buf;
  struct ice_aqc_txsched_elem *data;
...
ice_sched_replay_node_bw(struct ice_hw *hw, struct ice_sched_node *node,
  ...
  if (ice_is_bit_set(bw_t_info->bw_t_bitmap, ICE_BW_TYPE_PRIO)) {
    status = ice_sched_replay_node_prio(hw, node, bw_t_info->generic);
    if (status)
      return status;
  }
  if (ice_is_bit_set(bw_t_info->bw_t_bitmap, ICE_BW_TYPE_CIR)) {
    status =
        ice_sched_set_node_bw_lmt(pi, node, ICE_MIN_BW, bw_t_info->cir_bw.bw);
    if (status)
      return status;
  }
  if (ice_is_bit_set(bw_t_info->bw_t_bitmap, ICE_BW_TYPE_CIR_WT)) {
    bw_alloc = bw_t_info->cir_bw.bw_alloc;
    status = ice_sched_cfg_node_bw_alloc(hw, node, ICE_MIN_BW, bw_alloc);
    if (status)
      return status;
  }
  if (ice_is_bit_set(bw_t_info->bw_t_bitmap, ICE_BW_TYPE_EIR)) {
    status =
        ice_sched_set_node_bw_lmt(pi, node, ICE_MAX_BW, bw_t_info->eir_bw.bw);
    if (status)
      return status;
  }
  if (ice_is_bit_set(bw_t_info->bw_t_bitmap, ICE_BW_TYPE_EIR_WT)) {
    bw_alloc = bw_t_info->eir_bw.bw_alloc;
    status = ice_sched_cfg_node_bw_alloc(hw, node, ICE_MAX_BW, bw_alloc);
    if (status)
      return status;
  }
 ...
 */
static enum ice_status
ice_sched_replay_agg_bw(struct ice_hw *hw,
                        struct ice_sched_agg_info *agg_info) {
  struct ice_sched_node *tc_node, *agg_node;
  enum ice_status status = ICE_SUCCESS;
  u8 tc;
  ...
  ice_for_each_traffic_class(tc) {
    ...
    agg_node = ice_sched_get_agg_node(hw->port_info, tc_node, agg_info->agg_id);
    if (!agg_node) {
      status = ICE_ERR_PARAM;
      break;
    }
    status = ice_sched_replay_node_bw(hw, agg_node, &agg_info->bw_t_info[tc]);
    if (status)
      break;
  }
  return status;
...
void ice_sched_replay_agg(struct ice_hw *hw) {
  ...
  if (!ice_cmp_bitmap(agg_info->tc_bitmap, agg_info->replay_tc_bitmap,
                      ICE_MAX_TRAFFIC_CLASS)) {
    ice_declare_bitmap(replay_bitmap, ICE_MAX_TRAFFIC_CLASS);
    enum ice_status status;

    ice_zero_bitmap(replay_bitmap, ICE_MAX_TRAFFIC_CLASS);
    ice_sched_get_ena_tc_bitmap(pi, agg_info->replay_tc_bitmap, replay_bitmap);
...
enum ice_status ice_sched_replay_root_node_bw(struct ice_port_info *pi) {
  ...
  status = ice_sched_replay_node_bw(pi->hw, pi->root, &pi->root_node_bw_t_info);
  ice_release_lock(&pi->sched_lock);
  return status;
}
...
enum ice_status ice_sched_replay_tc_node_bw(struct ice_port_info *pi) {
  ...
  ice_for_each_traffic_class(tc) {
    ...
    status =
        ice_sched_replay_node_bw(pi->hw, tc_node, &pi->tc_node_bw_t_info[tc]);
    if (status)
      break;
  }
  ice_release_lock(&pi->sched_lock);
 ...
 * called with scheduler lock held.
 */
static enum ice_status ice_sched_replay_vsi_bw(struct ice_hw *hw,
                                               u16 vsi_handle,
                                               ice_bitmap_t *tc_bitmap) {
  struct ice_sched_node *vsi_node, *tc_node;
  struct ice_port_info *pi = hw->port_info;
 ...
 * scheduler lock held.
 */
static enum ice_status ice_sched_replay_vsi_agg(struct ice_hw *hw,
                                                u16 vsi_handle) {
  ice_declare_bitmap(replay_bitmap, ICE_MAX_TRAFFIC_CLASS);
  struct ice_sched_agg_vsi_info *agg_vsi_info;
  struct ice_port_info *pi = hw->port_info;
  ...
  status = ice_sched_cfg_agg(hw->port_info, agg_info->agg_id, ICE_AGG_TYPE_AGG,
                             replay_bitmap);
  if (status)
    return status;
  /* Replay aggregator node BW (restore aggregator BW) */
  ...
  return ice_sched_replay_vsi_bw(hw, vsi_handle, agg_vsi_info->tc_bitmap);
}
 ...
 * called with scheduler lock held.
 */
enum ice_status ice_sched_replay_q_bw(struct ice_port_info *pi,
                                      struct ice_q_ctx *q_ctx) {
