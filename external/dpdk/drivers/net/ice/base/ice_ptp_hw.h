 ...
 */
static inline void ice_set_e822_time_ref(struct ice_hw *hw,
...
#define P_REG_LINK_SPEED_FEC_MODE(reg)                                         \
  (((reg)&P_REG_LINK_SPEED_FEC_MODE_M) >> P_REG_LINK_SPEED_FEC_MODE_S)
...
#define TS_H(a, idx)                                                           \
  ((a) + ((idx)*BYTES_PER_IDX_ADDR_L_U + BYTES_PER_IDX_ADDR_L))

/* External PHY timestamp address */
#define TS_EXT(a, port, idx)                                                   \
  ((a) + (0x1000 * (port)) + ((idx)*BYTES_PER_IDX_ADDR_L_U))
