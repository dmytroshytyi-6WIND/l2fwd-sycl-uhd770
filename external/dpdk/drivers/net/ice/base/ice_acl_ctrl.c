...
static void ice_acl_init_entry(struct ice_acl_scen *scen) {
  ...
  scen->first_idx[ICE_ACL_PRIO_NORMAL] =
      scen->num_entry - scen->num_entry / 4 - 1;
  scen->first_idx[ICE_ACL_PRIO_HIGH] = 0;

  scen->last_idx[ICE_ACL_PRIO_LOW] = scen->num_entry - scen->num_entry / 4;
 ...
 */
static enum ice_status ice_acl_scen_free_entry_idx(struct ice_acl_scen *scen,
                                                   u16 idx) {
...
static u16 ice_acl_tbl_calc_end_idx(u16 start, u16 num_entries, u16 width) {
  ...
  if (width > 1) {
    ...
    num_stack_level = (start % ICE_AQC_ACL_TCAM_DEPTH) + num_entries;
    num_stack_level =
        DIVIDE_AND_ROUND_UP(num_stack_level, ICE_AQC_ACL_TCAM_DEPTH);
     ...
     */
    add_entries = (width - 1) * num_stack_level * ICE_AQC_ACL_TCAM_DEPTH;
  }
...
static enum ice_status ice_acl_init_tbl(struct ice_hw *hw) {
  ...
  while (tcam_idx < tbl->last_tcam ||
     ...
     */
    status = ice_aq_program_acl_entry(hw, tcam_idx, idx, &buf, NULL);
    if (status)
      return status;
  ...
  for (i = 0; i < ICE_AQC_MAX_ACTION_MEMORIES; i++) {
    ...
    for (act_entry_idx = start; act_entry_idx <= end; act_entry_idx++) {
      /* Invalidate all allocated action pairs */
      status = ice_aq_program_actpair(hw, i, act_entry_idx, &act_buf, NULL);
      if (status)
        return status;
    }
  }
 ...
 * "curr_tcam" TCAM.
 */
static void ice_acl_assign_act_mems_to_tcam(struct ice_acl_tbl *tbl,
                                            u8 cur_tcam, u8 *cur_mem_idx,
                                            u8 num_mem) {
  u8 mem_cnt;
...
static void ice_acl_divide_act_mems_to_tcams(struct ice_acl_tbl *tbl) {
  ...
  stack_level = DIVIDE_AND_ROUND_UP(tbl->info.depth, ICE_AQC_ACL_TCAM_DEPTH);

  /* Determine number of cascaded TCAMs */
  num_cscd = DIVIDE_AND_ROUND_UP(tbl->info.width, ICE_AQC_ACL_KEY_WIDTH_BYTES);
  ...
  for (stack_idx = 0; stack_idx < stack_level; stack_idx++) {
    ...
    for (i = 0; i < num_cscd; i++) {
      ...
      ice_acl_assign_act_mems_to_tcam(tbl, tcam_idx, &mem_idx, total_act_mem);
 ...
 */
enum ice_status ice_acl_create_tbl(struct ice_hw *hw,
  ...
  if (params->concurr) {
    tbl_alloc.num_dependent_alloc_ids = ICE_AQC_MAX_CONCURRENT_ACL_TBL;

    for (i = 0; i < ICE_AQC_MAX_CONCURRENT_ACL_TBL; i++)
      tbl_alloc.buf.data_buf.alloc_ids[i] = CPU_TO_LE16(params->dep_tbls[i]);
  }
 ...
 */
static enum ice_status ice_acl_alloc_partition(struct ice_hw *hw,
                                               struct ice_acl_scen *req) {
  ...
  do {
     ...
     * current TCAM row
     */
    for (i = 0; i < ICE_AQC_MAX_TCAM_ALLOC_UNITS && cnt < r_entries; i++) {
      bool avail = true;
      u16 w, p;
      ...
      if (!avail) {
        ...
        if (!cnt || dir < 0)
          start =
              (row * ICE_AQC_ACL_TCAM_DEPTH) + (p * ICE_ACL_ENTRY_ALLOC_UNIT);
    ...
    if (cnt >= r_entries) {
      ...
      req->end = ice_acl_tbl_calc_end_idx(start, r_entries, width);
      break;
    }

    row = dir > 0 ? row + width : row - width;
    if (row > hw->acl_tbl->last_tcam || row < hw->acl_tbl->first_tcam) {
      ...
      if (off >= width)
        ...
        row = dir > 0 ? off : hw->acl_tbl->last_tcam + 1 - off - width;
    }
  } while (!done);
...
static void ice_acl_fill_tcam_select(struct ice_aqc_acl_scen *scen_buf,
  ...
  cascade_cnt = DIVIDE_AND_ROUND_UP(scen->width, ICE_AQC_ACL_KEY_WIDTH_BYTES);
...
static void ice_acl_set_scen_chnk_msk(struct ice_aqc_acl_scen *scen_buf,
  ...
  chnk_offst =
      (u8)((scen->start % ICE_AQC_ACL_TCAM_DEPTH) / ICE_ACL_ENTRY_ALLOC_UNIT);
...
static void ice_acl_commit_partition(struct ice_hw *hw,
  ...
  /* Determine the starting TCAM index and offset of the start entry */
  tcam_idx = ICE_ACL_TBL_TCAM_IDX(scen->start);
  off = (scen->start % ICE_AQC_ACL_TCAM_DEPTH) / ICE_ACL_ENTRY_ALLOC_UNIT;
  ...
  for (cnt = 0; cnt < units; cnt++) {
    ...
    for (w = 0; w < num_cscd; w++) {
      ...
      b = ((tcam_idx + w) * ICE_AQC_MAX_TCAM_ALLOC_UNITS) + off;
      if (commit)
 ...
 */
enum ice_status ice_acl_create_scen(struct ice_hw *hw, u16 match_width,
                                    u16 num_entries, u16 *scen_id) {
   ...
   */
  scen->eff_width =
      cascade_cnt * ICE_AQC_ACL_KEY_WIDTH_BYTES - ICE_ACL_SCEN_MIN_WIDTH;
  ...
  while (k <= last_tcam) {
    ...
    scen_buf.tcam_cfg[k].start_cmp_set |= ICE_AQC_ACL_ALLOC_SCE_START_CMP;

    /* cascade TCAMs up to the width of the scenario */
    for (i = k; i < cascade_cnt + k; i++) {
      ice_acl_fill_tcam_select(&scen_buf, scen, i, i - k);
      ice_acl_assign_act_mem_for_scen(hw->acl_tbl, scen, &scen_buf, i,
  ...
  i = 0;
  while (i < first_tcam)
    scen_buf.tcam_cfg[i++].start_cmp_set = ICE_AQC_ACL_ALLOC_SCE_START_CMP;
...
static enum ice_status ice_acl_destroy_scen(struct ice_hw *hw, u16 scen_id) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(p, tmp, &hw->fl_profs[ICE_BLK_ACL], ice_flow_prof,
                           l_entry)
  ...
  LIST_FOR_EACH_ENTRY_SAFE(scen, tmp_scen, &hw->acl_tbl->scens, ice_acl_scen,
                           list_entry)
  if (scen->id == scen_id) {
...
enum ice_status ice_acl_destroy_tbl(struct ice_hw *hw) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(pos_scen, tmp_scen, &hw->acl_tbl->scens,
    ...
    for (i = 0; i < ICE_AQC_ACL_SLICES; i++) {
      ...
      buf.tcam_cfg[i].start_cmp_set = ICE_AQC_ACL_ALLOC_SCE_START_CMP;
    }
 ...
 */
enum ice_status ice_acl_add_entry(struct ice_hw *hw, struct ice_acl_scen *scen,
                                  enum ice_acl_entry_prio prio, u8 *keys,
                                  u8 *inverts, struct ice_acl_act_entry *acts,
                                  u8 acts_cnt, u16 *entry_idx) {
  ...
  *entry_idx = ice_acl_scen_assign_entry_idx(scen, prio);
  ...
  /* Determine number of cascaded TCAMs */
  num_cscd = DIVIDE_AND_ROUND_UP(scen->width, ICE_AQC_ACL_KEY_WIDTH_BYTES);
  ...
  for (i = 0; i < num_cscd; i++) {
     ...
     */
    offset = num_cscd - i - 1;
    ice_memcpy(&buf.entry_key.val, &keys[offset * sizeof(buf.entry_key.val)],
    ...
    ice_memcpy(&buf.entry_key_invert.val,
               ...
               sizeof(buf.entry_key_invert.val), ICE_NONDMA_TO_NONDMA);
    status = ice_aq_program_acl_entry(hw, entry_tcam + offset, idx, &buf, NULL);
    if (status) {
 ...
 */
enum ice_status ice_acl_prog_act(struct ice_hw *hw, struct ice_acl_scen *scen,
  ...
  ice_for_each_set_bit(i, scen->act_mem_bitmap, ICE_AQC_MAX_ACTION_MEMORIES) {
    ...
    if (mem->member_of_tcam >= entry_tcam &&
      ...
      ice_memcpy(&act_buf.act[0], &acts[actx_idx],
                 sizeof(struct ice_acl_act_entry), ICE_NONDMA_TO_NONDMA);
      ...
      status = ice_aq_program_actpair(hw, i, idx, &act_buf, NULL);
      if (status) {
 ...
 */
enum ice_status ice_acl_rem_entry(struct ice_hw *hw, struct ice_acl_scen *scen,
                                  u16 entry_idx) {
  struct ice_aqc_actpair act_buf;
  struct ice_aqc_acl_data buf;
  ...
  ice_memset(&buf, 0, sizeof(buf), ICE_NONDMA_MEM);
  for (i = 0; i < num_cscd; i++) {
    status = ice_aq_program_acl_entry(hw, entry_tcam + i, idx, &buf, NULL);
    if (status)
      ice_debug(hw, ICE_DBG_ACL, "AQ program ACL entry failed status: %d\n",
                status);
  }
