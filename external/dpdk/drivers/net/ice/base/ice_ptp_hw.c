 ...
 */
static void ice_fill_phy_msg_e822(struct ice_sbq_msg_input *msg, u8 port,
                                  u16 offset) {
 ...
 */
static enum ice_status ice_read_phy_reg_e822_lp(struct ice_hw *hw, u8 port,
                                                u16 offset, u32 *val,
                                                bool lock_sbq) {
...
enum ice_status ice_read_phy_reg_e822(struct ice_hw *hw, u8 port, u16 offset,
                                      u32 *val) {
 ...
 */
static enum ice_status ice_read_40b_phy_reg_e822(struct ice_hw *hw, u8 port,
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to read from low register 0x%08x\n, status %d", low_addr,
              status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to read from high register 0x%08x\n, status %d",
              high_addr, status);
    return status;
  }
 ...
 */
static enum ice_status ice_read_64b_phy_reg_e822(struct ice_hw *hw, u8 port,
                                                 u16 low_addr, u64 *val) {
  enum ice_status status;
  u32 low, high;
  u16 high_addr;
 ...
 */
static enum ice_status ice_write_phy_reg_e822_lp(struct ice_hw *hw, u8 port,
                                                 u16 offset, u32 val,
                                                 bool lock_sbq) {
  struct ice_sbq_msg_input msg = {0};
  enum ice_status status;
...
enum ice_status ice_write_phy_reg_e822(struct ice_hw *hw, u8 port, u16 offset,
                                       u32 val) {
 ...
 */
static enum ice_status ice_write_40b_phy_reg_e822(struct ice_hw *hw, u8 port,
                                                  u16 low_addr, u64 val) {
  enum ice_status status;
  u32 low, high;
  u16 high_addr;
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write to low register 0x%08x\n, status %d", low_addr,
              status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write to high register 0x%08x\n, status %d", high_addr,
              status);
    return status;
  }
 ...
 * a 64bit value.
 */
static enum ice_status ice_write_64b_phy_reg_e822(struct ice_hw *hw, u8 port,
                                                  u16 low_addr, u64 val) {
  enum ice_status status;
  u32 low, high;
  u16 high_addr;
 ...
 * multiple PHYs.
 */
static void ice_fill_quad_msg_e822(struct ice_sbq_msg_input *msg, u8 quad,
                                   u16 offset) {
  u32 addr;
 ...
 */
static enum ice_status ice_read_quad_reg_e822_lp(struct ice_hw *hw, u8 quad,
                                                 u16 offset, u32 *val,
                                                 bool lock_sbq) {
  struct ice_sbq_msg_input msg = {0};
  enum ice_status status;
...
enum ice_status ice_read_quad_reg_e822(struct ice_hw *hw, u8 quad, u16 offset,
                                       u32 *val) {
 ...
 * shared between multiple PHYs.
 */
static enum ice_status ice_write_quad_reg_e822_lp(struct ice_hw *hw, u8 quad,
                                                  u16 offset, u32 val,
                                                  bool lock_sbq) {
  struct ice_sbq_msg_input msg = {0};
  enum ice_status status;
...
enum ice_status ice_write_quad_reg_e822(struct ice_hw *hw, u8 quad, u16 offset,
                                        u32 val) {
 ...
 * family of devices.
 */
static enum ice_status ice_read_phy_tstamp_e822(struct ice_hw *hw, u8 quad,
                                                u8 idx, u64 *tstamp) {
  enum ice_status status;
  u16 lo_addr, hi_addr;
  u32 lo, hi;
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to read low PTP timestamp register, status %d\n", status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to read high PTP timestamp register, status %d\n",
              status);
    return status;
  }
 ...
 */
static enum ice_status ice_clear_phy_tstamp_e822(struct ice_hw *hw, u8 quad,
                                                 u8 idx) {
  enum ice_status status;
  u16 lo_addr, hi_addr;
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to clear low PTP timestamp register, status %d\n",
              status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to clear high PTP timestamp register, status %d\n",
              status);
    return status;
  }
 ...
 */
static enum ice_status ice_read_cgu_reg_e822(struct ice_hw *hw, u16 addr,
                                             u32 *val) {
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to read CGU register 0x%04x, status %d\n", addr, status);
    return status;
  }
 ...
 * applicable to E822 devices.
 */
static enum ice_status ice_write_cgu_reg_e822(struct ice_hw *hw, u16 addr,
                                              u32 val) {
  struct ice_sbq_msg_input cgu_msg;
  enum ice_status status;
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write CGU register 0x%04x, status %d\n", addr, status);
    return status;
  }
 ...
 */
enum ice_status ice_cfg_cgu_pll_e822(struct ice_hw *hw,
  ...
  if (clk_src == ICE_CLK_SRC_TCX0 && clk_freq != ICE_TIME_REF_FREQ_25_000) {
  ...
  ice_debug(
      hw, ICE_DBG_PTP,
      "Current CGU configuration -- %s, clk_src %s, clk_freq %s, PLL %s\n",
  ...
  /* Log the current clock configuration */
  ice_debug(hw, ICE_DBG_PTP,
            "New CGU configuration -- %s, clk_src %s, clk_freq %s, PLL %s\n",
            dw24.field.ts_pll_enable ? "enabled" : "disabled",
            ice_clk_src_str(dw24.field.time_ref_sel),
            ice_clk_freq_str(dw9.field.time_ref_freq_sel),
            bwm_lf.field.plllock_true_lock_cri ? "locked" : "unlocked");
...
static enum ice_status ice_init_cgu_e822(struct ice_hw *hw) {
  ...
  status = ice_read_cgu_reg_e822(hw, TSPLL_CNTR_BIST_SETTINGS, &cntr_bist.val);
  if (status)
    return status;
  ...
  status = ice_write_cgu_reg_e822(hw, TSPLL_CNTR_BIST_SETTINGS, cntr_bist.val);
  if (status)
    return status;
...
#define PF_SB_REM_DEV_CTL_PHY0 BIT(2)
  ...
  regval |= (PF_SB_REM_DEV_CTL_SWITCH_READ | PF_SB_REM_DEV_CTL_PHY0);
 ...
 */
static enum ice_status ice_ptp_prep_phy_time_e822(struct ice_hw *hw, u32 time) {
  enum ice_status status;
  u64 phy_time;
  u8 port;
  ...
  for (port = 0; port < ICE_NUM_EXTERNAL_PORTS; port++) {
    ...
    /* Tx case */
    status = ice_write_64b_phy_reg_e822(hw, port, P_REG_TX_TIMER_INC_PRE_L,
                                        phy_time);
    if (status)
      goto exit_err;

    /* Rx case */
    status = ice_write_64b_phy_reg_e822(hw, port, P_REG_RX_TIMER_INC_PRE_L,
                                        phy_time);
    if (status)
      goto exit_err;
  }
...
exit_err:
  ice_debug(hw, ICE_DBG_PTP,
            "Failed to write init time for port %u, status %d\n", port, status);
 ...
 */
enum ice_status ice_ptp_prep_port_adj_e822(struct ice_hw *hw, u8 port, s64 time,
                                           bool lock_sbq) {
  enum ice_status status;
  u32 l_time, u_time;
  ...
  /* Tx case */
  status = ice_write_phy_reg_e822_lp(hw, port, P_REG_TX_TIMER_INC_PRE_L, l_time,
                                     lock_sbq);
  if (status)
    goto exit_err;

  status = ice_write_phy_reg_e822_lp(hw, port, P_REG_TX_TIMER_INC_PRE_U, u_time,
                                     lock_sbq);
  if (status)
    goto exit_err;

  /* Rx case */
  status = ice_write_phy_reg_e822_lp(hw, port, P_REG_RX_TIMER_INC_PRE_L, l_time,
                                     lock_sbq);
  if (status)
    goto exit_err;

  status = ice_write_phy_reg_e822_lp(hw, port, P_REG_RX_TIMER_INC_PRE_U, u_time,
                                     lock_sbq);
  if (status)
    goto exit_err;
...
exit_err:
  ice_debug(hw, ICE_DBG_PTP,
            "Failed to write time adjust for port %u, status %d\n", port,
            status);
  return status;
}
 ...
 */
static enum ice_status ice_ptp_prep_phy_adj_e822(struct ice_hw *hw, s32 adj,
                                                 bool lock_sbq) {
  s64 cycles;
  u8 port;
  ...
  for (port = 0; port < ICE_NUM_EXTERNAL_PORTS; port++) {
    ...
    status = ice_ptp_prep_port_adj_e822(hw, port, cycles, lock_sbq);
    if (status)
      return status;
  }
 ...
 */
static enum ice_status ice_ptp_prep_phy_incval_e822(struct ice_hw *hw,
                                                    u64 incval) {
  enum ice_status status;
  u8 port;
 ...
 */
enum ice_status ice_ptp_read_phy_incval_e822(struct ice_hw *hw, u8 port,
                                             u64 *incval) {
  enum ice_status status;
 ...
 * devices.
 */
static enum ice_status ice_ptp_prep_phy_adj_target_e822(struct ice_hw *hw,
                                                        u32 target_time) {
  enum ice_status status;
  u8 port;
  ...
  for (port = 0; port < ICE_NUM_EXTERNAL_PORTS; port++) {
    ...
    status =
        ice_write_phy_reg_e822_lp(hw, port, P_REG_TX_TIMER_CNT_ADJ_L, 0, true);
    if (status)
      goto exit_err;

    status = ice_write_phy_reg_e822_lp(hw, port, P_REG_TX_TIMER_CNT_ADJ_U,
                                       target_time, true);
    if (status)
      goto exit_err;
    ...
    /* No sub-nanoseconds data */
    status =
        ice_write_phy_reg_e822_lp(hw, port, P_REG_RX_TIMER_CNT_ADJ_L, 0, true);
    if (status)
      goto exit_err;

    status = ice_write_phy_reg_e822_lp(hw, port, P_REG_RX_TIMER_CNT_ADJ_U,
                                       target_time, true);
    if (status)
      goto exit_err;
  }
...
exit_err:
  ice_debug(hw, ICE_DBG_PTP,
            "Failed to write target time for port %u, status %d\n", port,
            status);
 ...
 */
enum ice_status ice_ptp_read_port_capture(struct ice_hw *hw, u8 port,
                                          u64 *tx_ts, u64 *rx_ts) {
  enum ice_status status;
 ...
 */
enum ice_status ice_ptp_one_port_cmd(struct ice_hw *hw, u8 port,
  ...
  /* Tx case */
  /* Read, modify, write */
  status = ice_read_phy_reg_e822_lp(hw, port, P_REG_TX_TMR_CMD, &val, lock_sbq);
  if (status) {
  ...
  status = ice_write_phy_reg_e822_lp(hw, port, P_REG_TX_TMR_CMD, val, lock_sbq);
  if (status) {
  ...
  /* Rx case */
  /* Read, modify, write */
  status = ice_read_phy_reg_e822_lp(hw, port, P_REG_RX_TMR_CMD, &val, lock_sbq);
  if (status) {
  ...
  status = ice_write_phy_reg_e822_lp(hw, port, P_REG_RX_TMR_CMD, val, lock_sbq);
  if (status) {
 ...
 * command.
 */
static enum ice_status ice_ptp_port_cmd_e822(struct ice_hw *hw,
...
enum ice_status ice_ptp_set_vernier_wl(struct ice_hw *hw) {
  ...
  for (port = 0; port < ICE_NUM_EXTERNAL_PORTS; port++) {
    ...
    status =
        ice_write_phy_reg_e822_lp(hw, port, P_REG_WL, PTP_VERNIER_WL, true);
    if (status) {
      ice_debug(hw, ICE_DBG_PTP,
                "Failed to set vernier window length for port %u, status %d\n",
                port, status);
      return status;
    }
  }
 ...
 * algorithm.
 */
enum ice_status ice_phy_get_speed_and_fec_e822(struct ice_hw *hw, u8 port,
...
void ice_phy_cfg_lane_e822(struct ice_hw *hw, u8 port) {
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write back TX_MEM_GBL_CFG, status %d\n", status);
    return;
  }
}
...
#define LINE_UI_25G_100G 256 /* 6600 UIs is 256 nanoseconds at 25Gb/100Gb */
  ...
  status = ice_write_64b_phy_reg_e822(hw, port, P_REG_UIX66_10G_40G_L, uix);
  if (status) {
  ...
  status = ice_write_64b_phy_reg_e822(hw, port, P_REG_UIX66_25G_100G_L, uix);
  if (status) {
...
static enum ice_status ice_phy_cfg_parpcs_e822(struct ice_hw *hw, u8 port) {
  ...
  if (e822_vernier[link_spd].tx_par_clk)
    phy_tus = DIV_64BIT(tu_per_sec, e822_vernier[link_spd].tx_par_clk);
  else
    phy_tus = 0;

  status = ice_write_40b_phy_reg_e822(hw, port, P_REG_PAR_TX_TUS_L, phy_tus);
  if (status)
    return status;

  /* P_REG_PAR_RX_TUS */
  if (e822_vernier[link_spd].rx_par_clk)
    phy_tus = DIV_64BIT(tu_per_sec, e822_vernier[link_spd].rx_par_clk);
  else
    phy_tus = 0;

  status = ice_write_40b_phy_reg_e822(hw, port, P_REG_PAR_RX_TUS_L, phy_tus);
  if (status)
    return status;

  /* P_REG_PCS_TX_TUS */
  if (e822_vernier[link_spd].tx_pcs_clk)
    phy_tus = DIV_64BIT(tu_per_sec, e822_vernier[link_spd].tx_pcs_clk);
  else
    phy_tus = 0;

  status = ice_write_40b_phy_reg_e822(hw, port, P_REG_PCS_TX_TUS_L, phy_tus);
  if (status)
    return status;

  /* P_REG_PCS_RX_TUS */
  if (e822_vernier[link_spd].rx_pcs_clk)
    phy_tus = DIV_64BIT(tu_per_sec, e822_vernier[link_spd].rx_pcs_clk);
  else
    phy_tus = 0;

  status = ice_write_40b_phy_reg_e822(hw, port, P_REG_PCS_RX_TUS_L, phy_tus);
  if (status)
    return status;

  /* P_REG_DESK_PAR_TX_TUS */
  if (e822_vernier[link_spd].tx_desk_rsgb_par)
    phy_tus = DIV_64BIT(tu_per_sec, e822_vernier[link_spd].tx_desk_rsgb_par);
  else
    phy_tus = 0;

  status =
      ice_write_40b_phy_reg_e822(hw, port, P_REG_DESK_PAR_TX_TUS_L, phy_tus);
  if (status)
    return status;

  /* P_REG_DESK_PAR_RX_TUS */
  if (e822_vernier[link_spd].rx_desk_rsgb_par)
    phy_tus = DIV_64BIT(tu_per_sec, e822_vernier[link_spd].rx_desk_rsgb_par);
  else
    phy_tus = 0;

  status =
      ice_write_40b_phy_reg_e822(hw, port, P_REG_DESK_PAR_RX_TUS_L, phy_tus);
  if (status)
    return status;

  /* P_REG_DESK_PCS_TX_TUS */
  if (e822_vernier[link_spd].tx_desk_rsgb_pcs)
    phy_tus = DIV_64BIT(tu_per_sec, e822_vernier[link_spd].tx_desk_rsgb_pcs);
  else
    phy_tus = 0;

  status =
      ice_write_40b_phy_reg_e822(hw, port, P_REG_DESK_PCS_TX_TUS_L, phy_tus);
  if (status)
    return status;

  /* P_REG_DESK_PCS_RX_TUS */
  if (e822_vernier[link_spd].rx_desk_rsgb_pcs)
    phy_tus = DIV_64BIT(tu_per_sec, e822_vernier[link_spd].rx_desk_rsgb_pcs);
  else
    phy_tus = 0;

  return ice_write_40b_phy_reg_e822(hw, port, P_REG_DESK_PCS_RX_TUS_L, phy_tus);
}
 ...
 */
static u64 ice_calc_fixed_tx_offset_e822(struct ice_hw *hw,
                                         enum ice_ptp_link_spd link_spd) {
...
enum ice_status ice_phy_cfg_tx_offset_e822(struct ice_hw *hw, u8 port) {
   ...
   * the total offset.
   */
  if (link_spd == ICE_PTP_LNK_SPD_1G || link_spd == ICE_PTP_LNK_SPD_10G ||
      link_spd == ICE_PTP_LNK_SPD_25G || link_spd == ICE_PTP_LNK_SPD_25G_RS ||
      link_spd == ICE_PTP_LNK_SPD_40G || link_spd == ICE_PTP_LNK_SPD_50G) {
    status =
        ice_read_64b_phy_reg_e822(hw, port, P_REG_PAR_PCS_TX_OFFSET_L, &val);
    if (status)
      return status;
  ...
  if (link_spd == ICE_PTP_LNK_SPD_50G_RS ||
    ...
    status = ice_read_64b_phy_reg_e822(hw, port, P_REG_PAR_TX_TIME_L, &val);
    if (status)
      return status;
 ...
 */
static enum ice_status ice_phy_cfg_fixed_tx_offset_e822(struct ice_hw *hw,
                                                        u8 port) {
  enum ice_ptp_link_spd link_spd;
  enum ice_ptp_fec_mode fec_mode;
  enum ice_status status;
 ...
 */
static enum ice_status ice_phy_calc_pmd_adj_e822(struct ice_hw *hw, u8 port,
                                                 enum ice_ptp_link_spd link_spd,
                                                 enum ice_ptp_fec_mode fec_mode,
                                                 u64 *pmd_adj) {
  ...
  if (link_spd == ICE_PTP_LNK_SPD_1G) {
    ...
    ice_debug(hw, ICE_DBG_PTP,
              "Unknown link speed %d, skipping PMD adjustment\n", link_spd);
    mult = 0;
  }
  ...
  if (link_spd == ICE_PTP_LNK_SPD_25G_RS) {
    ...
    status = ice_read_phy_reg_e822(hw, port, P_REG_RX_40_TO_160_CNT, &val);
    if (status) {
      ice_debug(hw, ICE_DBG_PTP,
                "Failed to read 25G-RS Rx cycle count, status %d\n", status);
      return status;
    }
    ...
    status = ice_read_phy_reg_e822(hw, port, P_REG_RX_80_TO_160_CNT, &val);
    if (status) {
      ice_debug(hw, ICE_DBG_PTP,
                "Failed to read 50G-RS Rx cycle count, status %d\n", status);
      return status;
    }
 ...
 */
static u64 ice_calc_fixed_rx_offset_e822(struct ice_hw *hw,
                                         enum ice_ptp_link_spd link_spd) {
  u64 cur_freq, clk_incval, tu_per_sec, fixed_offset;
...
enum ice_status ice_phy_cfg_rx_offset_e822(struct ice_hw *hw, u8 port) {
   ...
   * the total offset.
   */
  status = ice_read_64b_phy_reg_e822(hw, port, P_REG_PAR_PCS_RX_OFFSET_L, &val);
  if (status)
    return status;
  ...
  if (link_spd == ICE_PTP_LNK_SPD_40G || link_spd == ICE_PTP_LNK_SPD_50G ||
      link_spd == ICE_PTP_LNK_SPD_50G_RS ||
      link_spd == ICE_PTP_LNK_SPD_100G_RS) {
    status = ice_read_64b_phy_reg_e822(hw, port, P_REG_PAR_RX_TIME_L, &val);
    if (status)
      return status;
 ...
 * ready. This can be used when operating in bypass mode.
 */
static enum ice_status ice_phy_cfg_fixed_rx_offset_e822(struct ice_hw *hw,
                                                        u8 port) {
  enum ice_ptp_link_spd link_spd;
  enum ice_ptp_fec_mode fec_mode;
  enum ice_status status;
 ...
 * timer values.
 */
static enum ice_status ice_read_phy_and_phc_time_e822(struct ice_hw *hw,
                                                      u8 port, u64 *phy_time,
                                                      u64 *phc_time) {
  enum ice_status status;
  u64 tx_time, rx_time;
  u32 zo, lo;
  ...
  *phc_time = (u64)lo << 32 | zo;
  ...
  if (tx_time != rx_time)
    ice_warn(hw,
             "PHY port %u Tx and Rx timers do not match, tx_time 0x%016llX, "
             "rx_time 0x%016llX\n",
             port, (unsigned long long)tx_time, (unsigned long long)rx_time);
...
static enum ice_status ice_sync_phy_timer_e822(struct ice_hw *hw, u8 port) {
  ...
  ice_info(hw, "Port %u PHY time synced to PHC: 0x%016llX, 0x%016llX\n", port,
           (unsigned long long)phy_time, (unsigned long long)phc_time);
 ...
 */
enum ice_status ice_stop_phy_timer_e822(struct ice_hw *hw, u8 port,
                                        bool soft_reset) {
  enum ice_status status;
  u32 val;
 ...
 */
enum ice_status ice_start_phy_timer_e822(struct ice_hw *hw, u8 port,
                                         bool bypass) {
  enum ice_status status;
  u32 lo, hi, val;
  u64 incval;
...
enum ice_status ice_phy_exit_bypass_e822(struct ice_hw *hw, u8 port) {
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to read TX_OV_STATUS for port %u, status %d\n", port,
              status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to read RX_OV_STATUS for port %u, status %d\n", port,
              status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to program total Tx offset for port %u, status %d\n",
              port, status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to program total Rx offset for port %u, status %d\n",
              port, status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to read P_REG_PS for port %u, status %d\n", port, status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to disable bypass for port %u, status %d\n", port,
              status);
    return status;
  }
 ...
 */
static enum ice_status ice_read_phy_reg_e810_lp(struct ice_hw *hw, u32 addr,
...
static enum ice_status ice_read_phy_reg_e810(struct ice_hw *hw, u32 addr,
                                             u32 *val) {
 ...
 */
static enum ice_status ice_write_phy_reg_e810_lp(struct ice_hw *hw, u32 addr,
                                                 u32 val, bool lock_sbq) {
  struct ice_sbq_msg_input msg = {0};
  enum ice_status status;
...
static enum ice_status ice_write_phy_reg_e810(struct ice_hw *hw, u32 addr,
                                              u32 val) {
 ...
 * on the E810 device.
 */
static enum ice_status ice_read_phy_tstamp_e810(struct ice_hw *hw, u8 lport,
                                                u8 idx, u64 *tstamp) {
  enum ice_status status;
 ...
 */
static enum ice_status ice_clear_phy_tstamp_e810(struct ice_hw *hw, u8 lport,
                                                 u8 idx) {
  enum ice_status status;
  u32 lo_addr, hi_addr;
 ...
 */
static enum ice_status ice_ptp_prep_phy_adj_e810(struct ice_hw *hw, s32 adj,
                                                 bool lock_sbq) {
  enum ice_status status;
  u8 tmr_idx;
   ...
   */
  status =
      ice_write_phy_reg_e810_lp(hw, ETH_GLTSYN_SHADJ_L(tmr_idx), 0, lock_sbq);
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write adj to PHY SHADJ_L, status %d\n", status);
    return status;
  }

  status =
      ice_write_phy_reg_e810_lp(hw, ETH_GLTSYN_SHADJ_H(tmr_idx), adj, lock_sbq);
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write adj to PHY SHADJ_H, status %d\n", status);
    return status;
  }
 ...
 */
static enum ice_status ice_ptp_prep_phy_incval_e810(struct ice_hw *hw,
                                                    u64 incval) {
  enum ice_status status;
  u32 high, low;
  u8 tmr_idx;
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write incval to PHY SHADJ_L, status %d\n", status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write incval PHY SHADJ_H, status %d\n", status);
    return status;
  }
 ...
 * nominal nanoseconds.
 */
static enum ice_status ice_ptp_prep_phy_adj_target_e810(struct ice_hw *hw,
                                                        u32 target_time) {
  enum ice_status status;
  u8 tmr_idx;
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write target time to SHTIME_0, status %d\n", status);
    return status;
  }

  status =
      ice_write_phy_reg_e810(hw, ETH_GLTSYN_SHTIME_L(tmr_idx), target_time);
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to write target time to SHTIME_L, status %d\n", status);
    return status;
  }
 ...
 * command.
 */
static enum ice_status ice_ptp_port_cmd_e810(struct ice_hw *hw,
                                             enum ice_ptp_tmr_cmd cmd,
                                             bool lock_sbq) {
  enum ice_status status;
  u32 cmd_val, val;
 ...
 */
static enum ice_status ice_get_pca9575_handle(struct ice_hw *hw,
  ...
  cmd->addr.topo_params.node_type_ctx =
      (ICE_AQC_LINK_TOPO_NODE_TYPE_M & ICE_AQC_LINK_TOPO_NODE_TYPE_GPIO_CTRL);
 ...
 */
enum ice_status ice_read_e810t_pca9575_reg(struct ice_hw *hw, u8 offset,
                                           u8 *data) {
  ...
  link_topo.topo_params.node_type_ctx =
      (ICE_AQC_LINK_TOPO_NODE_CTX_PROVIDED << ICE_AQC_LINK_TOPO_NODE_CTX_S);
 ...
 */
enum ice_status ice_write_e810t_pca9575_reg(struct ice_hw *hw, u8 offset,
                                            u8 data) {
  struct ice_aqc_link_topo_addr link_topo;
  enum ice_status status;
  __le16 addr;
...
enum ice_status ice_read_sma_ctrl_e810t(struct ice_hw *hw, u8 *data) {
  ...
  *data = 0;
  ...
  for (i = ICE_E810T_SMA_MIN_BIT; i <= ICE_E810T_SMA_MAX_BIT; i++) {
    ...
    status = ice_aq_get_gpio(hw, handle, i + ICE_E810T_P1_OFFSET, &pin, NULL);
    if (status)
      break;
    *data |= (u8)(!pin) << i;
  }
...
enum ice_status ice_write_sma_ctrl_e810t(struct ice_hw *hw, u8 data) {
  ...
  for (i = ICE_E810T_SMA_MIN_BIT; i <= ICE_E810T_SMA_MAX_BIT; i++) {
    ...
    pin = !(data & (1 << i));
    status = ice_aq_set_gpio(hw, handle, i + ICE_E810T_P1_OFFSET, pin, NULL);
    if (status)
      break;
  }
...
ice_ptp_tmr_cmd(struct ice_hw *hw, enum ice_ptp_tmr_cmd cmd, bool lock_sbq) {
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_PTP,
              "Failed to prepare PHY ports for timer command %u, status %d\n",
              cmd, status);
    return status;
  }
 ...
 */
enum ice_status ice_ptp_adj_clock_at_time(struct ice_hw *hw, u64 at_time,
                                          s32 adj) {
  enum ice_status status;
  u32 time_lo, time_hi;
  u8 tmr_idx;
 ...
 */
enum ice_status ice_read_phy_tstamp(struct ice_hw *hw, u8 block, u8 idx,
                                    u64 *tstamp) {
  if (ice_is_e810(hw))
 ...
 */
enum ice_status ice_clear_phy_tstamp(struct ice_hw *hw, u8 block, u8 idx) {
  if (ice_is_e810(hw))
