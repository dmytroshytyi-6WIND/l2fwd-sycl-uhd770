 ...
 */
static void dump_phy_type(struct ice_hw *hw, u64 phy, u8 i,
                          const char *phy_string, const char *prefix) {
  if (phy & BIT_ULL(i))
 ...
 * @prefix: prefix string to differentiate multiple dumps
 */
static void ice_dump_phy_type_low(struct ice_hw *hw, u64 low,
                                  const char *prefix) {
 ...
 * @prefix: prefix string to differentiate multiple dumps
 */
static void ice_dump_phy_type_high(struct ice_hw *hw, u64 high,
                                   const char *prefix) {
 ...
 */
bool ice_is_e810(struct ice_hw *hw) { return hw->mac_type == ICE_MAC_E810; }
 ...
 * called.
 */
static enum ice_status ice_aq_manage_mac_read(struct ice_hw *hw, void *buf,
                                              u16 buf_size,
                                              struct ice_sq_cd *cd) {
  ...
  for (i = 0; i < cmd->num_addr; i++)
    if (resp[i].addr_type == ICE_AQC_MAN_MAC_ADDR_TYPE_LAN) {
      ice_memcpy(hw->port_info->mac.lan_addr, resp[i].mac_addr, ETH_ALEN,
                 ICE_DMA_TO_NONDMA);
      ice_memcpy(hw->port_info->mac.perm_addr, resp[i].mac_addr, ETH_ALEN,
                 ICE_DMA_TO_NONDMA);
      break;
    }
  return ICE_SUCCESS;
 ...
 */
enum ice_status ice_aq_get_phy_caps(struct ice_port_info *pi, bool qual_mods,
                                    u8 report_mode,
  ...
  if (status == ICE_SUCCESS && report_mode == ICE_AQC_REPORT_TOPO_CAP_MEDIA) {
    ...
    ice_memcpy(pi->phy.link_info.module_type, &pcaps->module_type,
               sizeof(pi->phy.link_info.module_type), ICE_NONDMA_TO_NONDMA);
  }
 ...
 */
static enum ice_status ice_aq_get_link_topo_handle(struct ice_port_info *pi,
                                                   u8 node_type,
                                                   struct ice_sq_cd *cd) {
  ...
  cmd->addr.topo_params.node_type_ctx =
      (ICE_AQC_LINK_TOPO_NODE_CTX_PORT << ICE_AQC_LINK_TOPO_NODE_CTX_S);
...
static bool ice_is_media_cage_present(struct ice_port_info *pi) {
   ...
   * connection type is backplane or BASE-T.
   */
  return !ice_aq_get_link_topo_handle(pi, ICE_AQC_LINK_TOPO_NODE_TYPE_CAGE,
                                      NULL);
}
 ...
 */
enum ice_status ice_aq_get_link_info(struct ice_port_info *pi, bool ena_lse,
  ...
  *hw_media_type = ice_get_media_type(pi);
  ...
  li->pacing = link_data.cfg & (ICE_AQ_CFG_PACING_M | ICE_AQ_CFG_PACING_TYPE_M);
 ...
 * descriptor
 */
static void ice_fill_tx_timer_and_fc_thresh(struct ice_hw *hw,
...
#define IDX_OF_LFC PRTMAC_HSEC_CTL_TX_PAUSE_QUANTA_MAX_INDEX
  ...
  tx_timer_val =
      val & PRTMAC_HSEC_CTL_TX_PAUSE_QUANTA_HSEC_CTL_TX_PAUSE_QUANTA_M;
 ...
 */
enum ice_status ice_aq_set_mac_cfg(struct ice_hw *hw, u16 max_frame_size,
                                   struct ice_sq_cd *cd) {
  struct ice_aqc_set_mac_cfg *cmd;
  struct ice_aq_desc desc;
...
enum ice_status ice_init_fltr_mgmt_struct(struct ice_hw *hw) {
  ...
  hw->switch_info =
      (struct ice_switch_info *)ice_malloc(hw, sizeof(*hw->switch_info));
 ...
 */
static void ice_cleanup_fltr_mgmt_single(struct ice_hw *hw,
                                         struct ice_switch_info *sw) {
  ...
  for (i = 0; i < ICE_MAX_NUM_RECIPES; i++) {
    ...
    recps[i].root_rid = i;
    LIST_FOR_EACH_ENTRY_SAFE(rg_entry, tmprg_entry, &recps[i].rg_list,
    ...
    if (recps[i].adv_rule) {
      ...
      LIST_FOR_EACH_ENTRY_SAFE(lst_itr, tmp_entry, &recps[i].filt_rules,
...
static void ice_get_itr_intrl_gran(struct ice_hw *hw) {
  u8 max_agg_bw = (rd32(hw, GL_PWR_MODE_CTL) & GL_PWR_MODE_CTL_CAR_MAX_BW_M) >>
...
void ice_print_rollback_msg(struct ice_hw *hw) {
  ...
  SNPRINTF(nvm_str, sizeof(nvm_str), "%x.%02x 0x%x %d.%d.%d", nvm->major,
           nvm->minor, nvm->eetrack, orom->major, orom->build, orom->patch);
  ice_warn(hw,
           "Firmware rollback mode detected. Current version is NVM: %s, FW: "
           "%d.%d. Device may exhibit limited functionality. Refer to the "
           "Intel(R) Ethernet Adapters and Devices User Guide for details on "
 ...
 */
void ice_set_umac_shared(struct ice_hw *hw) { hw->umac_shared = true; }
...
enum ice_status ice_init_hw(struct ice_hw *hw) {
  ...
  hw->pf_id = (u8)(rd32(hw, PF_FUNC_RID) & PF_FUNC_RID_FUNCTION_NUMBER_M) >>
  ...
  hw->port_info =
      (struct ice_port_info *)ice_malloc(hw, sizeof(*hw->port_info));
  if (!hw->port_info) {
  ...
  pcaps = (struct ice_aqc_get_phy_caps_data *)ice_malloc(hw, sizeof(*pcaps));
  if (!pcaps) {
    status = ICE_ERR_NO_MEMORY;
    goto err_unroll_sched;
  }
  ...
  /* Get MAC information */
  /* A single port can report up to two (LAN and WoL) addresses */
  mac_buf = ice_calloc(hw, 2, sizeof(struct ice_aqc_manage_mac_read_resp));
...
#define ICE_RESET_DONE_MASK                                                    \
  (GLNVM_ULD_PCIER_DONE_M | GLNVM_ULD_PCIER_DONE_1_M |                         \
   GLNVM_ULD_CORER_DONE_M | GLNVM_ULD_GLOBR_DONE_M | GLNVM_ULD_POR_DONE_M |    \
   GLNVM_ULD_POR_DONE_1_M | GLNVM_ULD_PCIER_DONE_2_M)
...
static enum ice_status ice_pf_reset(struct ice_hw *hw) {
   ...
   */
  for (cnt = 0; cnt < ICE_GLOBAL_CFG_LOCK_TIMEOUT + ICE_PF_RESET_WAIT_COUNT;
       cnt++) {
    reg = rd32(hw, PFGEN_CTRL);
 ...
 */
static enum ice_status ice_copy_rxq_ctx_to_hw(struct ice_hw *hw,
                                              u8 *ice_rxq_ctx, u32 rxq_index) {
  u8 i;
 ...
 */
enum ice_status ice_write_rxq_ctx(struct ice_hw *hw,
 ...
 */
static enum ice_status ice_copy_tx_cmpltnq_ctx_to_hw(struct ice_hw *hw,
                                                     u8 *ice_tx_cmpltnq_ctx,
 ...
 */
static enum ice_status ice_copy_tx_drbell_q_ctx_to_hw(struct ice_hw *hw,
                                                      u8 *ice_tx_drbell_q_ctx,
...
ice_write_tx_drbell_q_ctx(struct ice_hw *hw,
  ...
  ice_set_ctx(hw, (u8 *)tx_drbell_q_ctx, ctx_buf, ice_tx_drbell_q_ctx_info);
 ...
 */
enum ice_status ice_clear_tx_drbell_q_ctx(struct ice_hw *hw,
                                          u32 tx_drbell_q_index) {
  u8 i;
 ...
 */
static enum ice_status ice_sbq_send_cmd(struct ice_hw *hw,
                                        struct ice_sbq_cmd_desc *desc,
                                        void *buf, u16 buf_size,
                                        struct ice_sq_cd *cd) {
  return ice_sq_send_cmd(hw, ice_get_sbq(hw), (struct ice_aq_desc *)desc, buf,
                         buf_size, cd);
}
 ...
 * @cd: pointer to command details structure
 */
static enum ice_status ice_sbq_send_cmd_nolock(struct ice_hw *hw,
                                               struct ice_sbq_cmd_desc *desc,
                                               void *buf, u16 buf_size,
                                               struct ice_sq_cd *cd) {
  return ice_sq_send_cmd_nolock(hw, ice_get_sbq(hw), (struct ice_aq_desc *)desc,
                                buf, buf_size, cd);
}
...
enum ice_status ice_sbq_rw_reg_lp(struct ice_hw *hw,
  ...
  if (!status && !in->opcode)
    in->data = LE32_TO_CPU(((struct ice_sbq_msg_cmpl *)&msg)->data);
  return status;
}
 ...
 * @in: message info to be filled in descriptor
 */
enum ice_status ice_sbq_rw_reg(struct ice_hw *hw,
                               struct ice_sbq_msg_input *in) {
 ...
 */
static enum ice_status ice_sq_send_cmd_retry(struct ice_hw *hw,
  ...
  if (is_cmd_for_retry) {
    ...
    ice_memcpy(&desc_cpy, desc, sizeof(desc_cpy), ICE_NONDMA_TO_NONDMA);
  }
  ...
  do {
    ...
    if (buf_cpy)
      ice_memcpy(buf, buf_cpy, buf_size, ICE_NONDMA_TO_NONDMA);

    ice_memcpy(desc, &desc_cpy, sizeof(desc_cpy), ICE_NONDMA_TO_NONDMA);
 ...
 */
enum ice_status ice_aq_send_cmd(struct ice_hw *hw, struct ice_aq_desc *desc,
  ...
  if (hw->aq_send_cmd_fn) {
    ...
    if (!hw->aq_send_cmd_fn(hw->aq_send_cmd_param, desc, buf, buf_size)) {
 ...
 */
enum ice_status ice_aq_send_driver_ver(struct ice_hw *hw,
  ...
  len = 0;
  while (len < sizeof(dv->driver_string) && IS_ASCII(dv->driver_string[len]) &&
 ...
 */
static enum ice_status ice_aq_req_res(struct ice_hw *hw,
                                      enum ice_aq_res_ids res,
  ...
  if (res == ICE_GLOBAL_CFG_LOCK_RES_ID) {
    if (LE16_TO_CPU(cmd_resp->status) == ICE_AQ_RES_GLBL_SUCCESS) {
      *timeout = LE32_TO_CPU(cmd_resp->timeout);
      return ICE_SUCCESS;
    } else if (LE16_TO_CPU(cmd_resp->status) == ICE_AQ_RES_GLBL_IN_PROG) {
      *timeout = LE32_TO_CPU(cmd_resp->timeout);
      return ICE_ERR_AQ_ERROR;
    } else if (LE16_TO_CPU(cmd_resp->status) == ICE_AQ_RES_GLBL_DONE) {
 ...
 */
static enum ice_status ice_aq_release_res(struct ice_hw *hw,
                                          enum ice_aq_res_ids res,
 ...
 */
enum ice_status ice_acquire_res(struct ice_hw *hw, enum ice_aq_res_ids res,
                                enum ice_aq_res_access_type access,
                                u32 timeout) {
 ...
 */
enum ice_status ice_aq_alloc_free_res(struct ice_hw *hw, u16 num_entries,
                                      struct ice_aqc_alloc_free_res_elem *buf,
                                      u16 buf_size, enum ice_adminq_opc opc,
                                      struct ice_sq_cd *cd) {
 ...
 */
enum ice_status ice_alloc_hw_res(struct ice_hw *hw, u16 type, u16 num, bool btm,
                                 u16 *res) {
  struct ice_aqc_alloc_free_res_elem *buf;
  enum ice_status status;
  u16 buf_len;
  ...
  ice_memcpy(res, buf->elem, sizeof(*buf->elem) * num, ICE_NONDMA_TO_NONDMA);
 ...
 */
enum ice_status ice_free_hw_res(struct ice_hw *hw, u16 type, u16 num,
                                u16 *res) {
  struct ice_aqc_alloc_free_res_elem *buf;
  enum ice_status status;
  u16 buf_len;
  ...
  ice_memcpy(buf->elem, res, sizeof(*buf->elem) * num, ICE_NONDMA_TO_NONDMA);

  status =
      ice_aq_alloc_free_res(hw, num, buf, buf_len, ice_aqc_opc_free_res, NULL);
  if (status)
 ...
 * false otherwise.
 */
static bool ice_parse_common_caps(struct ice_hw *hw,
                                  struct ice_hw_common_caps *caps,
                                  struct ice_aqc_list_caps_elem *elem,
                                  const char *prefix) {
  ...
  case ICE_AQC_CAPS_NVM_MGMT:
    caps->sec_rev_disabled =
        (number & ICE_NVM_MGMT_SEC_REV_DISABLED) ? true : false;
    ice_debug(hw, ICE_DBG_INIT, "%s: sec_rev_disabled = %d\n", prefix,
              caps->sec_rev_disabled);
    caps->update_disabled =
        (number & ICE_NVM_MGMT_UPDATE_DISABLED) ? true : false;
    ice_debug(hw, ICE_DBG_INIT, "%s: update_disabled = %d\n", prefix,
              caps->update_disabled);
    caps->nvm_unified_update =
        (number & ICE_NVM_MGMT_UNIFIED_UPD_SUPPORT) ? true : false;
  ...
  case ICE_AQC_CAPS_PCIE_RESET_AVOIDANCE:
    ...
    ice_debug(hw, ICE_DBG_INIT, "%s: pcie_reset_avoidance = %d\n", prefix,
              caps->pcie_reset_avoidance);
    break;
  case ICE_AQC_CAPS_POST_UPDATE_RESET_RESTRICT:
    caps->reset_restrict_support = (number == 1);
    ice_debug(hw, ICE_DBG_INIT, "%s: reset_restrict_support = %d\n", prefix,
  ...
  case ICE_AQC_CAPS_EXT_TOPO_DEV_IMG3: {
    ...
    ice_debug(hw, ICE_DBG_INIT, "%s: ext_topo_dev_img_ver_high[%d] = %d\n",
              prefix, index, caps->ext_topo_dev_img_ver_high[index]);
    ice_debug(hw, ICE_DBG_INIT, "%s: ext_topo_dev_img_ver_low[%d] = %d\n",
              prefix, index, caps->ext_topo_dev_img_ver_low[index]);
    ice_debug(hw, ICE_DBG_INIT, "%s: ext_topo_dev_img_part_num[%d] = %d\n",
              prefix, index, caps->ext_topo_dev_img_part_num[index]);
    ice_debug(hw, ICE_DBG_INIT, "%s: ext_topo_dev_img_load_en[%d] = %d\n",
              prefix, index, caps->ext_topo_dev_img_load_en[index]);
    ice_debug(hw, ICE_DBG_INIT, "%s: ext_topo_dev_img_prog_en[%d] = %d\n",
              prefix, index, caps->ext_topo_dev_img_prog_en[index]);
    break;
  }
  default:
 ...
 */
static void ice_recalc_port_limited_caps(struct ice_hw *hw,
                                         struct ice_hw_common_caps *caps) {
 ...
 */
static void ice_parse_vsi_func_caps(struct ice_hw *hw,
 ...
 */
static void ice_parse_1588_func_caps(struct ice_hw *hw,
                                     struct ice_hw_func_caps *func_p,
                                     struct ice_aqc_list_caps_elem *cap) {
 ...
 */
static void ice_parse_fdir_func_caps(struct ice_hw *hw,
                                     struct ice_hw_func_caps *func_p) {
  u32 reg_val, val;
  ...
  val = (reg_val & GLQF_FD_SIZE_FD_GSIZE_M) >> GLQF_FD_SIZE_FD_GSIZE_S;
  func_p->fd_fltr_guar = ice_get_num_per_func(hw, val);
  val = (reg_val & GLQF_FD_SIZE_FD_BSIZE_M) >> GLQF_FD_SIZE_FD_BSIZE_S;
 ...
 */
static void ice_parse_func_caps(struct ice_hw *hw,
                                struct ice_hw_func_caps *func_p, void *buf,
                                u32 cap_count) {
  ...
  for (i = 0; i < cap_count; i++) {
    ...
    found = ice_parse_common_caps(hw, &func_p->common_cap, &cap_resp[i],
                                  "func caps");
 ...
 */
static void ice_parse_valid_functions_cap(struct ice_hw *hw,
 ...
 */
static void ice_parse_vsi_dev_caps(struct ice_hw *hw,
                                   struct ice_hw_dev_caps *dev_p,
                                   struct ice_aqc_list_caps_elem *cap) {
  u32 number = LE32_TO_CPU(cap->number);
 ...
 */
static void ice_parse_1588_dev_caps(struct ice_hw *hw,
                                    struct ice_hw_dev_caps *dev_p,
                                    struct ice_aqc_list_caps_elem *cap) {
 ...
 */
static void ice_parse_fdir_dev_caps(struct ice_hw *hw,
                                    struct ice_hw_dev_caps *dev_p,
                                    struct ice_aqc_list_caps_elem *cap) {
  u32 number = LE32_TO_CPU(cap->number);
 ...
 */
static void ice_parse_dev_caps(struct ice_hw *hw, struct ice_hw_dev_caps *dev_p,
  ...
  for (i = 0; i < cap_count; i++) {
    ...
    found =
        ice_parse_common_caps(hw, &dev_p->common_cap, &cap_resp[i], "dev caps");
 ...
 */
static enum ice_status ice_aq_list_caps(struct ice_hw *hw, void *buf,
  ...
  if (opc != ice_aqc_opc_list_func_caps && opc != ice_aqc_opc_list_dev_caps)
    return ICE_ERR_PARAM;
 ...
 * for later use.
 */
static enum ice_status ice_discover_dev_caps(struct ice_hw *hw,
...
#define ICE_RESTORE_FUNC_CAP(name) func_caps->common_cap.name = cached_caps.name
...
#define ICE_RESTORE_DEV_CAP(name) dev_caps->common_cap.name = cached_caps.name
 ...
 */
enum ice_status ice_aq_manage_mac_write(struct ice_hw *hw, const u8 *mac_addr,
                                        u8 flags, struct ice_sq_cd *cd) {
 ...
 */
enum ice_status ice_aq_set_port_params(struct ice_port_info *pi,
 ...
 */
enum ice_status ice_aq_set_phy_cfg(struct ice_hw *hw, struct ice_port_info *pi,
...
enum ice_status ice_update_link_info(struct ice_port_info *pi) {
  ...
  if (li->link_info & ICE_AQ_MEDIA_AVAILABLE) {
    ...
    if (status == ICE_SUCCESS)
      ice_memcpy(li->module_type, &pcaps->module_type, sizeof(li->module_type),
 ...
 */
static void ice_cache_phy_user_req(struct ice_port_info *pi,
  ...
  case ICE_FC_MODE:
    ...
    break;
  case ICE_SPEED_MODE:
    pi->phy.curr_user_speed_req = cache_data.data.curr_user_speed_req;
    break;
  case ICE_FEC_MODE:
...
enum ice_fec_mode ice_caps_to_fec_mode(u8 caps, u8 fec_options) {
  ...
  if (fec_options &
      (ICE_AQC_PHY_FEC_10G_KR_40G_KR4_EN | ICE_AQC_PHY_FEC_10G_KR_40G_KR4_REQ |
       ICE_AQC_PHY_FEC_25G_KR_CLAUSE74_EN | ICE_AQC_PHY_FEC_25G_KR_REQ))
 ...
 */
static enum ice_status ice_cfg_phy_fc(struct ice_port_info *pi,
                                      struct ice_aqc_set_phy_cfg_data *cfg,
  ...
  case ICE_FC_AUTO: {
    ...
    pcaps =
        (struct ice_aqc_get_phy_caps_data *)ice_malloc(pi->hw, sizeof(*pcaps));
    if (!pcaps)
      return ICE_ERR_NO_MEMORY;
 ...
 */
enum ice_status ice_set_fc(struct ice_port_info *pi, u8 *aq_failures,
  ...
  *aq_failures = 0;
  ...
  status =
      ice_aq_get_phy_caps(pi, false, ICE_AQC_REPORT_ACTIVE_CFG, pcaps, NULL);
 ...
 * configuration
 */
bool ice_phy_caps_equals_cfg(struct ice_aqc_get_phy_caps_data *phy_caps,
   ...
   */
  caps_mask =
      ICE_AQC_PHY_CAPS_MASK & ~(ICE_AQC_PHY_AN_MODE | ICE_AQC_PHY_EN_MOD_QUAL);
...
void ice_copy_phy_caps_to_cfg(struct ice_port_info *pi,
  ...
  cfg->module_compliance_enforcement = caps->module_compliance_enforcement;
}
 ...
 */
enum ice_status ice_cfg_phy_fec(struct ice_port_info *pi,
                                struct ice_aqc_set_phy_cfg_data *cfg,
  ...
  case ICE_FEC_BASER:
     ...
     */
    cfg->link_fec_opt &=
        ICE_AQC_PHY_FEC_10G_KR_40G_KR4_EN | ICE_AQC_PHY_FEC_25G_KR_CLAUSE74_EN;
    cfg->link_fec_opt |=
 ...
 */
enum ice_status ice_aq_set_link_restart_an(struct ice_port_info *pi,
                                           bool ena_link,
                                           struct ice_sq_cd *cd) {
 ...
 */
enum ice_status ice_aq_set_event_mask(struct ice_hw *hw, u8 port_num, u16 mask,
                                      struct ice_sq_cd *cd) {
 ...
 */
enum ice_status ice_aq_set_mac_loopback(struct ice_hw *hw, bool ena_lpbk,
                                        struct ice_sq_cd *cd) {
 ...
 */
enum ice_status ice_aq_set_port_id_led(struct ice_port_info *pi,
                                       bool is_orig_mode,
                                       struct ice_sq_cd *cd) {
 ...
 */
enum ice_status ice_aq_sff_eeprom(struct ice_hw *hw, u16 lport, u8 bus_addr,
                                  u16 mem_addr, u8 page, u8 set_page, u8 *data,
                                  u8 length, bool write, struct ice_sq_cd *cd) {
  ...
  cmd->i2c_bus_addr =
      CPU_TO_LE16(((bus_addr >> 1) & ICE_AQC_SFF_I2CBUS_7BIT_M) |
 ...
 *
 */
enum ice_status ice_aq_read_topo_dev_nvm(
    struct ice_hw *hw, struct ice_aqc_link_topo_params *topo_params,
    u32 start_address, u8 *data, u8 data_size, struct ice_sq_cd *cd) {
 ...
 */
static enum ice_status __ice_aq_get_set_rss_lut(
    struct ice_hw *hw, struct ice_aq_get_set_rss_lut_params *params, bool set) {
  ...
  cmd_resp->vsi_id = CPU_TO_LE16(((vsi_id << ICE_AQC_GSET_RSS_LUT_VSI_ID_S) &
 ...
 */
enum ice_status
ice_aq_get_rss_lut(struct ice_hw *hw,
...
__ice_aq_get_set_rss_key(struct ice_hw *hw, u16 vsi_id,
  ...
  cmd_resp->vsi_id = CPU_TO_LE16(((vsi_id << ICE_AQC_GSET_RSS_KEY_VSI_ID_S) &
 ...
 */
enum ice_status ice_aq_get_rss_key(struct ice_hw *hw, u16 vsi_handle,
                                   struct ice_aqc_get_set_rss_keys *key) {
  if (!ice_is_vsi_valid(hw, vsi_handle) || !key)
    return ICE_ERR_PARAM;

  return __ice_aq_get_set_rss_key(hw, ice_get_hw_vsi_num(hw, vsi_handle), key,
                                  false);
}
 ...
 */
enum ice_status ice_aq_set_rss_key(struct ice_hw *hw, u16 vsi_handle,
                                   struct ice_aqc_get_set_rss_keys *keys) {
  if (!ice_is_vsi_valid(hw, vsi_handle) || !keys)
    return ICE_ERR_PARAM;

  return __ice_aq_get_set_rss_key(hw, ice_get_hw_vsi_num(hw, vsi_handle), keys,
                                  true);
}
 ...
 * flow.
 */
enum ice_status ice_aq_add_lan_txq(struct ice_hw *hw, u8 num_qgrps,
                                   struct ice_aqc_add_tx_qgrp *qg_list,
  ...
  for (i = 0, list = qg_list; i < num_qgrps; i++) {
    ...
    list = (struct ice_aqc_add_tx_qgrp *)(list->txqs + list->num_txqs);
  }
 ...
 */
static enum ice_status ice_aq_dis_lan_txq(struct ice_hw *hw, u8 num_qgrps,
                                          struct ice_aqc_dis_txq_item *qg_list,
                                          u16 buf_size,
                                          enum ice_disq_rst_src rst_src,
                                          u16 vmvf_num, struct ice_sq_cd *cd) {
  ...
  cmd->vmvf_and_timeout =
      CPU_TO_LE16((5 << ICE_AQC_Q_DIS_TIMEOUT_S) & ICE_AQC_Q_DIS_TIMEOUT_M);

  switch (rst_src) {
  case ICE_VM_RESET:
    cmd->cmd_type = ICE_AQC_Q_DIS_CMD_VM_RESET;
    cmd->vmvf_and_timeout |= CPU_TO_LE16(vmvf_num & ICE_AQC_Q_DIS_VMVF_NUM_M);
    break;
  case ICE_NO_RESET:
  default:
    break;
  ...
  if (status) {
    if (!qg_list)
      ...
      ice_debug(hw, ICE_DBG_SCHED, "disable queue %d failed %d\n",
                LE16_TO_CPU(qg_list[0].q_id[0]), hw->adminq.sq_last_status);
  }
  return status;
}
 ...
 */
enum ice_status ice_aq_move_recfg_lan_txq(struct ice_hw *hw, u8 num_qs,
                                          bool is_move, bool is_tc_change,
                                          bool subseq_call, bool flush_pipe,
                                          u8 timeout, u32 *blocked_cgds,
                                          struct ice_aqc_move_txqs_data *buf,
                                          u16 buf_size, u8 *txqs_moved,
                                          struct ice_sq_cd *cd) {
...
#define ICE_LAN_TXQ_MOVE_TIMEOUT_MAX 50
  ...
  cmd->num_qs = num_qs;
  cmd->timeout =
      ((timeout << ICE_AQC_Q_CMD_TIMEOUT_S) & ICE_AQC_Q_CMD_TIMEOUT_M);
  ...
  if (hw->adminq.sq_last_status == ICE_AQ_RC_EAGAIN && is_tc_change &&
      !flush_pipe)
  ...
  for (f = 0; ce_info[f].width; f++) {
    ...
    if (ce_info[f].width > (ce_info[f].size_of * BITS_PER_BYTE)) {
      ice_debug(hw, ICE_DBG_QCTX,
                "Field %d width of %d bits larger than size of %d byte(s) ... "
                "skipping write\n",
 ...
 */
enum ice_status ice_aq_get_internal_data(struct ice_hw *hw, u8 cluster_id,
 ...
 * @ce_info:  a description of the struct to be filled
 */
static void ice_read_byte(u8 *src_ctx, u8 *dest_ctx,
 ...
 * @ce_info:  a description of the struct to be filled
 */
static void ice_read_word(u8 *src_ctx, u8 *dest_ctx,
                          struct ice_ctx_ele *ce_info) {
  u16 dest_word, mask;
  u8 *src, *target;
  __le16 src_word;
 ...
 * @ce_info:  a description of the struct to be filled
 */
static void ice_read_dword(u8 *src_ctx, u8 *dest_ctx,
                           struct ice_ctx_ele *ce_info) {
  u32 dest_dword, mask;
  __le32 src_dword;
  u8 *src, *target;
 ...
 * @ce_info:  a description of the struct to be filled
 */
static void ice_read_qword(u8 *src_ctx, u8 *dest_ctx,
                           struct ice_ctx_ele *ce_info) {
  u64 dest_qword, mask;
  __le64 src_qword;
  u8 *src, *target;
 ...
 */
enum ice_status ice_ena_vsi_txq(struct ice_port_info *pi, u16 vsi_handle, u8 tc,
                                u16 q_handle, u8 num_qgrps,
  ...
  /* find a parent node */
  parent =
      ice_sched_get_free_qparent(pi, vsi_handle, tc, ICE_SCHED_NODE_OWNER_LAN);
  if (!parent) {
    status = ICE_ERR_PARAM;
    goto ena_txq_exit;
  }
  ...
  buf->txqs[0].info.cir_bw.bw_alloc = CPU_TO_LE16(ICE_SCHED_DFLT_BW_WT);
  buf->txqs[0].info.eir_bw.bw_profile_idx =
      CPU_TO_LE16(ICE_SCHED_DFLT_RL_PROF_ID);
  buf->txqs[0].info.eir_bw.bw_alloc = CPU_TO_LE16(ICE_SCHED_DFLT_BW_WT);

  /* add the LAN queue */
  status = ice_aq_add_lan_txq(hw, num_qgrps, buf, buf_size, cd);
  if (status != ICE_SUCCESS) {
    ice_debug(hw, ICE_DBG_SCHED, "enable queue %d failed %d\n",
              LE16_TO_CPU(buf->txqs[0].txq_id), hw->adminq.sq_last_status);
    goto ena_txq_exit;
  }
 ...
 */
enum ice_status ice_dis_vsi_txq(struct ice_port_info *pi, u16 vsi_handle, u8 tc,
                                u8 num_queues, u16 *q_handles, u16 *q_ids,
                                u32 *q_teids, enum ice_disq_rst_src rst_src,
                                u16 vmvf_num, struct ice_sq_cd *cd) {
  ...
  if (!num_queues) {
     ...
     */
    if (rst_src)
      return ice_aq_dis_lan_txq(hw, 0, NULL, 0, rst_src, vmvf_num, NULL);
    return ICE_ERR_CFG;
  }
  ...
  for (i = 0; i < num_queues; i++) {
    ...
    status =
        ice_aq_dis_lan_txq(hw, 1, qg_list, buf_size, rst_src, vmvf_num, cd);
 ...
 */
static enum ice_status ice_cfg_vsi_qs(struct ice_port_info *pi, u16 vsi_handle,
                                      u16 tc_bitmap, u16 *maxqs, u8 owner) {
  enum ice_status status = ICE_SUCCESS;
  u8 i;
 ...
 */
enum ice_status ice_cfg_vsi_lan(struct ice_port_info *pi, u16 vsi_handle,
 ...
 */
enum ice_status ice_replay_pre_init(struct ice_hw *hw,
                                    struct ice_switch_info *sw) {
  enum ice_status status;
  u8 i;
 ...
 * descriptors.
 */
void ice_stat_update_repc(struct ice_hw *hw, u16 vsi_handle,
 ...
 */
enum ice_status ice_sched_query_elem(struct ice_hw *hw, u32 node_teid,
  ...
  status = ice_aq_query_sched_elems(hw, 1, buf, buf_size, &num_elem_ret, NULL);
 ...
 * @params: I2C parameters: bit [7] - Repeated start, bits [6:5] data offset
 *size, bit [4] - I2C address type, bits [3:0] - data size to read (0-16 bytes)
 * @data: pointer to data (0 to 16 bytes) to be read from the I2C device
 * @cd: pointer to command details structure or NULL
 *
 * Read I2C (0x06E2)
 */
enum ice_status ice_aq_read_i2c(struct ice_hw *hw,
 ...
 * Write I2C (0x06E3)
 */
enum ice_status ice_aq_write_i2c(struct ice_hw *hw,
                                 struct ice_aqc_link_topo_addr topo_addr,
                                 u16 bus_addr, __le16 addr, u8 params, u8 *data,
                                 struct ice_sq_cd *cd) {
  struct ice_aq_desc desc = {0};
 ...
 * read it.
 */
enum ice_status ice_aq_set_driver_param(struct ice_hw *hw,
 ...
 */
enum ice_status ice_aq_get_driver_param(struct ice_hw *hw,
                                        enum ice_aqc_driver_params idx,
                                        u32 *value, struct ice_sq_cd *cd) {
  struct ice_aqc_driver_shared_params *cmd;
  struct ice_aq_desc desc;
 ...
 */
enum ice_status ice_aq_set_gpio(struct ice_hw *hw, u16 gpio_ctrl_handle,
                                u8 pin_idx, bool value, struct ice_sq_cd *cd) {
...
ice_get_link_default_override(struct ice_link_default_override_tlv *ldo,
  ...
  ldo->phy_config =
      (buf & ICE_LINK_OVERRIDE_PHY_CFG_M) >> ICE_LINK_OVERRIDE_PHY_CFG_S;
...
bool ice_is_phy_caps_an_enabled(struct ice_aqc_get_phy_caps_data *caps) {
  if (caps->caps & ICE_AQC_PHY_AN_MODE ||
      caps->low_power_ctrl_an &
          (ICE_AQC_PHY_AN_EN_CLAUSE28 | ICE_AQC_PHY_AN_EN_CLAUSE73 |
 ...
 */
enum ice_status ice_aq_set_lldp_mib(struct ice_hw *hw, u8 mib_type, void *buf,
                                    u16 buf_size, struct ice_sq_cd *cd) {
 ...
 */
enum ice_status ice_lldp_fltr_add_remove(struct ice_hw *hw, u16 vsi_num,
                                         bool add) {
