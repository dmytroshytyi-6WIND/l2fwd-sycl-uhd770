...
void ice_parse_item_dflt(struct ice_hw *hw, u16 idx, void *item, void *data,
                         int size) {
 ...
 */
void *ice_parser_sect_item_get(u32 sect_type, void *section, u32 index,
                               u32 *offset) {
 ...
 */
void *ice_parser_create_table(
    struct ice_hw *hw, u32 sect_type, u32 item_size, u32 length,
    void *(*item_get)(u32 sect_type, void *section, u32 index, u32 *offset),
    void (*parse_item)(struct ice_hw *hw, u16 idx, void *item, void *data,
                       int size),
    bool no_offset) {
  ...
  if (!table) {
    ice_debug(hw, ICE_DBG_PARSER,
              "failed to allocate memory for table type %d.\n", sect_type);
    return NULL;
  }

  ice_memset(&state, 0, sizeof(state), ICE_NONDMA_MEM);
  do {
    data = ice_pkg_enum_entry(seg, &state, sect_type, NULL, item_get);
    seg = NULL;
    if (data) {
      struct ice_pkg_sect_hdr *hdr = (struct ice_pkg_sect_hdr *)state.sect;

      if (no_offset)
        idx++;
      else
        idx = hdr->offset + state.entry_idx;
      parse_item(hw, idx, (void *)((uintptr_t)table + idx * item_size), data,
                 item_size);
    }
  } while (data);
...
static void _bst_vm_set(struct ice_parser *psr, const char *prefix, bool on) {
  ...
  while (true) {
    item = ice_bst_tcam_search(psr->bst_tcam_table, psr->bst_lbl_table, prefix,
                               &i);
    if (!item)
      break;
...
static enum ice_status _tunnel_port_set(struct ice_parser *psr,
                                        const char *prefix, u16 udp_port,
                                        bool on) {
  u8 *buf = (u8 *)&udp_port;
  struct ice_bst_tcam_item *item;
  u16 i = 0;
  ...
  while (true) {
    ...
    if (on && item->key[16] == 0xfe && item->key_inv[16] == 0xfe) {
      ...
      /* found a matched slot to delete */
               ...
               (item->key_inv[15] == buf[0] || item->key_inv[16] == buf[1])) {
...
void ice_parser_profile_dump(struct ice_hw *hw,
  ...
  for (i = 0; i < prof->fv_num; i++)
    ice_info(hw, "proto = %d, offset = %d spec = 0x%04x, mask = 0x%04x\n",
             prof->fv[i].proto_id, prof->fv[i].offset, prof->fv[i].spec,
             prof->fv[i].msk);
