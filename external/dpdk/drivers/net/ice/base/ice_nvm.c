...
enum ice_status ice_aq_read_nvm(struct ice_hw *hw, u16 module_typeid,
                                ...
                                bool last_command, bool read_shadow_ram,
                                struct ice_sq_cd *cd) {
 ...
 */
enum ice_status ice_read_flat_nvm(struct ice_hw *hw, u32 offset, u32 *length,
  ...
  *length = 0;
  ...
  do {
    ...
    read_size =
        MIN_T(u32, ICE_AQ_MAX_BUF_LEN - sector_offset, inlen - bytes_read);
     ...
     */
    status =
        ice_aq_read_nvm(hw, ICE_AQC_NVM_START_POINT, offset, (u16)read_size,
                        data + bytes_read, last_cmd, read_shadow_ram, NULL);
    if (status)
      break;
 ...
 */
static enum ice_status ice_read_sr_word_aq(struct ice_hw *hw, u16 offset,
                                           u16 *data) {
 ...
 */
static enum ice_status ice_read_sr_buf_aq(struct ice_hw *hw, u16 offset,
                                          u16 *words, u16 *data) {
  u32 bytes = *words * 2, i;
  enum ice_status status;
 ...
 */
static enum ice_status ice_read_flash_module(struct ice_hw *hw,
                                             enum ice_bank_select bank,
                                             u16 module, u32 offset, u8 *data,
                                             u32 length) {
  enum ice_status status;
  u32 start;
 ...
 */
static enum ice_status ice_read_nvm_module(struct ice_hw *hw,
                                           enum ice_bank_select bank,
                                           u32 offset, u16 *data) {
  enum ice_status status;
  __le16 data_local;
 ...
 */
static enum ice_status ice_read_nvm_sr_copy(struct ice_hw *hw,
                                            enum ice_bank_select bank,
                                            u32 offset, u16 *data) {
 ...
 */
static enum ice_status ice_read_orom_module(struct ice_hw *hw,
                                            enum ice_bank_select bank,
                                            u32 offset, u16 *data) {
  enum ice_status status;
  __le16 data_local;
 ...
 */
enum ice_status ice_get_pfa_module_tlv(struct ice_hw *hw, u16 *module_tlv,
...
enum ice_status ice_read_pba_string(struct ice_hw *hw, u8 *pba_num,
  ...
  status =
      ice_get_pfa_module_tlv(hw, &pba_tlv, &pba_tlv_len, ICE_SR_PBA_BLOCK_PTR);
  if (status != ICE_SUCCESS) {
 ...
 * bank.
 */
static enum ice_status ice_get_nvm_srev(struct ice_hw *hw,
                                        enum ice_bank_select bank, u32 *srev) {
  enum ice_status status;
  u16 srev_l, srev_h;
 ...
 */
static enum ice_status ice_get_nvm_ver_info(struct ice_hw *hw,
                                            enum ice_bank_select bank,
 ...
 */
enum ice_status ice_get_inactive_nvm_ver(struct ice_hw *hw,
                                         struct ice_nvm_info *nvm) {
 ...
 * bank.
 */
static enum ice_status ice_get_orom_srev(struct ice_hw *hw,
                                         enum ice_bank_select bank, u32 *srev) {
  enum ice_status status;
  u16 srev_l, srev_h;
  u32 css_start;
  ...
  status =
      ice_read_orom_module(hw, bank, css_start + ICE_NVM_CSS_SREV_L, &srev_l);
  if (status)
    return status;

  status =
      ice_read_orom_module(hw, bank, css_start + ICE_NVM_CSS_SREV_H, &srev_h);
  if (status)
    return status;
 ...
 * the image.
 */
static enum ice_status ice_get_orom_civd_data(struct ice_hw *hw,
                                              enum ice_bank_select bank,
  ...
  for (offset = 0; (offset + 512) <= hw->flash.banks.orom_size; offset += 512) {
    ...
    status = ice_read_flash_module(hw, bank, ICE_SR_1ST_OROM_BANK_PTR, offset,
                                   (u8 *)&tmp, sizeof(tmp));
    if (status) {
...
static enum ice_status ice_get_orom_ver_info(struct ice_hw *hw,
  ...
  orom->build =
      (u16)((combo_ver & ICE_OROM_VER_BUILD_MASK) >> ICE_OROM_VER_BUILD_SHIFT);
 ...
 */
enum ice_status ice_get_inactive_orom_ver(struct ice_hw *hw,
                                          struct ice_orom_info *orom) {
 ...
 */
static enum ice_status ice_read_sr_pointer(struct ice_hw *hw, u16 offset,
                                           u32 *pointer) {
  enum ice_status status;
  u16 value;
 ...
 */
static enum ice_status ice_read_sr_area_size(struct ice_hw *hw, u16 offset,
                                             u32 *size) {
  enum ice_status status;
  u16 value;
 ...
 */
static enum ice_status ice_determine_active_flash_banks(struct ice_hw *hw) {
  struct ice_bank_info *banks = &hw->flash.banks;
  enum ice_status status;
  u16 ctrl_word;
  ...
  status =
      ice_read_sr_area_size(hw, ICE_SR_NETLIST_BANK_SIZE, &banks->netlist_size);
  if (status) {
 ...
 * features structure.
 */
enum ice_status ice_nvm_access_get_features(struct ice_nvm_access_cmd *cmd,
 ...
 */
static enum ice_status ice_validate_nvm_rw_reg(struct ice_nvm_access_cmd *cmd) {
  ...
  if (module != ICE_NVM_REG_RW_MODULE || flags != ICE_NVM_REG_RW_FLAGS ||
 ...
 */
enum ice_status ice_nvm_access_read(struct ice_hw *hw,
 ...
 */
enum ice_status ice_nvm_access_write(struct ice_hw *hw,
                                     struct ice_nvm_access_cmd *cmd,
                                     union ice_nvm_access_data *data) {
  enum ice_status status;
...
enum ice_status ice_handle_nvm_access(struct ice_hw *hw,
  ...
  case ICE_NVM_CMD_READ:
    ...
    if (module == ICE_NVM_GET_FEATURES_MODULE &&
        flags == ICE_NVM_GET_FEATURES_FLAGS && cmd->offset == 0)
