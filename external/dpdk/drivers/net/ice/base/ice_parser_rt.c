...
static void _rt_ho_set(struct ice_parser_rt *rt, u16 ho) {
  ...
  ice_memcpy(&rt->gpr[GPR_HB_IDX], &rt->pkt_buf[ho], 32, ICE_NONDMA_TO_NONDMA);
}
...
void ice_parser_rt_pktbuf_set(struct ice_parser_rt *rt, const u8 *pkt_buf,
  ...
  ice_memcpy(&rt->gpr[GPR_HB_IDX], &rt->pkt_buf[ho], ICE_PARSER_HDR_BUF_LEN,
             ICE_NONDMA_TO_NONDMA);
}

static void _bst_key_init(struct ice_parser_rt *rt,
...
static u32 _pk_build(struct ice_parser_rt *rt, struct ice_np_keybuilder *kb) {
  if (kb->ops == 0)
    ...
    return rt->gpr[kb->start_or_reg0] | ((u32)rt->gpr[kb->len_or_reg1] << 16);
  else if (kb->ops == 2)
    return 0;
...
static void _imem_pgk_init(struct ice_parser_rt *rt,
                           struct ice_imem_item *imem) {
  ...
  ice_debug(
      rt->psr->hw, ICE_DBG_PARSER,
      "Generate Parse Graph Key: node_id(%d),flag0(%d), flag1(%d), flag2(%d), "
      "flag3(%d), boost_idx(%d), alu_reg(0x%04x), next_proto(0x%08x)\n",
      rt->pg_key.node_id, rt->pg_key.flag0, rt->pg_key.flag1, rt->pg_key.flag2,
      rt->pg_key.flag3, rt->pg_key.boost_idx, rt->pg_key.alu_reg,
      rt->pg_key.next_proto);
}

static void _imem_alu0_set(struct ice_parser_rt *rt,
                           struct ice_imem_item *imem) {
  rt->alu0 = &imem->alu0;
...
static void _imem_alu1_set(struct ice_parser_rt *rt,
                           struct ice_imem_item *imem) {
  rt->alu1 = &imem->alu1;
...
static void _imem_alu2_set(struct ice_parser_rt *rt,
                           struct ice_imem_item *imem) {
  rt->alu2 = &imem->alu2;
...
static void _imem_pgp_set(struct ice_parser_rt *rt,
                          struct ice_imem_item *imem) {
  rt->pg = imem->pg;
  ice_debug(rt->psr->hw, ICE_DBG_PARSER,
            "Load PG priority %d from imem pc %d\n", rt->pg, imem->idx);
}

static void _bst_pgk_init(struct ice_parser_rt *rt,
...
static void _bst_pgp_set(struct ice_parser_rt *rt,
  ...
  rt->pg = bst->pg_pri;
  ice_debug(rt->psr->hw, ICE_DBG_PARSER,
            "Load PG priority %d from boost address %d\n", rt->pg,
            bst->address);
}
...
static struct ice_pg_cam_item *_pg_cam_match(struct ice_parser_rt *rt) {
  ...
  item =
      ice_pg_cam_match(psr->pg_cam_table, ICE_PG_CAM_TABLE_SIZE, &rt->pg_key);
  if (item)
    return item;
...
static struct ice_pg_nm_cam_item *_pg_nm_cam_match(struct ice_parser_rt *rt) {
  ...
  item = ice_pg_nm_cam_match(psr->pg_nm_cam_table, ICE_PG_NM_CAM_TABLE_SIZE,
                             &rt->pg_key);
  ...
  item = ice_pg_nm_cam_match(psr->pg_nm_sp_cam_table,
                             ICE_PG_NM_SP_CAM_TABLE_SIZE, &rt->pg_key);
  return item;
}
...
static void _gpr_add(struct ice_parser_rt *rt, int idx, u16 val) {
  ...
  ice_debug(rt->psr->hw, ICE_DBG_PARSER,
            "Pending update for register %d value %d\n", idx, val);
}
...
static void _flg_add(struct ice_parser_rt *rt, int idx, bool val) {
  ...
  ice_debug(rt->psr->hw, ICE_DBG_PARSER,
            "Pending update for flag %d value %d\n", idx, val);
}
    ...
    if (alu->flags_extr_imm) {
      for (i = 0; i < alu->dst_len; i++)
        _flg_add(rt, alu->dst_start + i,
                 (alu->flags_start_imm & (1u << i)) != 0);
    } else {
      for (i = 0; i < alu->dst_len; i++) {
        _flg_add(rt, alu->dst_start + i,
                 _hv_bit_sel(rt, alu->flags_start_imm + i, 1) != 0);
      }
    }
  }
}
...
static u16 _reg_bit_sel(struct ice_parser_rt *rt, int reg_idx, int start,
                        int len) {
  u32 msk;
  union {
    u32 d32;
...
static void _err_add(struct ice_parser_rt *rt, int idx, bool val) {
  ...
  ice_debug(rt->psr->hw, ICE_DBG_PARSER,
            "Pending update for error %d value %d\n", idx, val);
}
...
static void _dst_reg_bit_set(struct ice_parser_rt *rt, struct ice_alu *alu,
  ...
  if (alu->dst_reg_id == GPR_ERR_IDX) {
    ...
    flg_idx = (u16)(((alu->dst_reg_id - GPR_FLG_IDX) << 4) + alu->dst_start);
    ...
    ice_debug(rt->psr->hw, ICE_DBG_PARSER,
              "Unexpected Dest Register Bit set, RegisterID %d Start %d\n",
...
static void _alu_exe(struct ice_parser_rt *rt, struct ice_alu *alu) {
  ...
  if (alu->shift_xlate_select) {
    ice_debug(rt->psr->hw, ICE_DBG_PARSER,
              "shift_xlate_select != 0 is not expected\n");
    return;
  }
...
static void _marker_update(struct ice_parser_rt *rt) {
  ...
  if (rt->action->is_mg) {
    struct ice_mk_grp_item *mk_grp = &psr->mk_grp_table[rt->action->marker_id];
...
static u16 _ptype_resolve(struct ice_parser_rt *rt) {
  ...
  item = ice_ptype_mk_tcam_match(psr->ptype_mk_tcam_table, rt->markers, 9);
  if (item)
    return item->ptype;
  return 0xffff;
}
...
static void _result_resolve(struct ice_parser_rt *rt,
  ...
  ice_memcpy(&rslt->flags_psr, &rt->gpr[GPR_FLG_IDX], 8, ICE_NONDMA_TO_NONDMA);
...
enum ice_status ice_parser_rt_execute(struct ice_parser_rt *rt,
  ...
  while (true) {
    ...
    if (!pg_cam) {
      ...
      if (pg_nm_cam) {
        ice_debug(rt->psr->hw, ICE_DBG_PARSER,
                  "Match ParseGraph Nomatch CAM Address %d\n", pg_nm_cam->idx);
    ...
    if (rt->action->is_last_round) {
      ice_debug(rt->psr->hw, ICE_DBG_PARSER,
                "Last Round in ParseGraph Action, stop parsing.\n");
      break;
    }
