 ...
 */
static enum ice_status ice_pkg_get_supported_vlan_mode(struct ice_hw *hw,
                                                       bool *dvm) {
  ...
  *dvm = false;
  ...
  bld = ice_pkg_buf_alloc_single_section(hw, ICE_SID_RXPARSER_METADATA_INIT,
  ...
  status = ice_aq_upload_section(hw, (struct ice_buf_hdr *)ice_pkg_buf(bld),
...
ice_aq_get_vlan_mode(struct ice_hw *hw,
  ...
  ice_fill_dflt_direct_cmd_desc(&desc, ice_aqc_opc_get_vlan_mode_parameters);

  return ice_aq_send_cmd(hw, &desc, get_params, sizeof(*get_params), NULL);
}
 ...
 */
bool ice_is_dvm_ena(struct ice_hw *hw) { return hw->dvm_ena; }
  ...
  for (i = 0; i < ARRAY_SIZE(ice_dvm_dflt_recipes); i++) {
    ...
    if (status) {
      ice_debug(hw, ICE_DBG_INIT,
                "Failed to update RID %d lkup_idx %d fv_idx %d mask_valid %s "
                "mask 0x%04x\n",
                params->rid, params->lkup_idx, params->fv_idx,
                params->mask_valid ? "true" : "false", params->mask);
      return status;
    }
  }
...
ice_aq_set_vlan_mode(struct ice_hw *hw,
  ...
  ice_fill_dflt_direct_cmd_desc(&desc, ice_aqc_opc_set_vlan_mode_parameters);
  desc.flags |= CPU_TO_LE16(ICE_AQ_FLAG_RD);

  return ice_aq_send_cmd(hw, &desc, set_params, sizeof(*set_params), NULL);
}
...
static enum ice_status ice_set_dvm(struct ice_hw *hw) {
  ...
  status = ice_aq_set_port_params(hw->port_info, 0, false, false, true, NULL);
  if (status) {
    ice_debug(hw, ICE_DBG_INIT,
...
static enum ice_status ice_set_svm(struct ice_hw *hw) {
  ...
  set_params =
      (struct ice_aqc_set_vlan_mode *)ice_malloc(hw, sizeof(*set_params));
  if (!set_params)
    return ICE_ERR_NO_MEMORY;
...
static void ice_print_dvm_not_supported(struct ice_hw *hw) {
  ...
  if (!fw_supports_dvm && !pkg_supports_dvm)
    ice_info(
        hw, "QinQ functionality cannot be enabled on this device. "
