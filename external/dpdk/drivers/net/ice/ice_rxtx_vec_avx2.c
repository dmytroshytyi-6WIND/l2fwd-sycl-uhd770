...
static __rte_always_inline uint16_t _ice_recv_raw_pkts_vec_avx2(
...
#define ICE_DESCS_PER_LOOP_AVX 8
  ...
  for (i = 0, received = 0; i < nb_pkts;
     ...
     * of each 128bits
     */
    const __m256i ptype_mask = _mm256_set1_epi16(ICE_RX_FLEX_DESC_PTYPE_M);
...
uint16_t ice_recv_pkts_vec_avx2(void *rx_queue, struct rte_mbuf **rx_pkts,
                                uint16_t nb_pkts) {
  return _ice_recv_raw_pkts_vec_avx2(rx_queue, rx_pkts, nb_pkts, NULL, false);
}

uint16_t ice_recv_pkts_vec_avx2_offload(void *rx_queue,
                                        struct rte_mbuf **rx_pkts,
                                        uint16_t nb_pkts) {
  return _ice_recv_raw_pkts_vec_avx2(rx_queue, rx_pkts, nb_pkts, NULL, true);
}
 ...
 * - nb_pkts < ICE_DESCS_PER_LOOP, just return no packet
 */
static __rte_always_inline uint16_t ice_recv_scattered_burst_vec_avx2(
  ...
  uint16_t nb_bufs =
      _ice_recv_raw_pkts_vec_avx2(rxq, rx_pkts, nb_pkts, split_flags, offload);
  if (nb_bufs == 0)
    return 0;
 ...
 * - nb_pkts < ICE_DESCS_PER_LOOP, just return no packet
 */
static __rte_always_inline uint16_t ice_recv_scattered_pkts_vec_avx2_common(
    void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts, bool offload) {
  uint16_t retval = 0;

  while (nb_pkts > ICE_VPMD_RX_BURST) {
    uint16_t burst = ice_recv_scattered_burst_vec_avx2(
        rx_queue, rx_pkts + retval, ICE_VPMD_RX_BURST, offload);
    retval += burst;
    nb_pkts -= burst;
    if (burst < ICE_VPMD_RX_BURST)
      return retval;
  }
  return retval + ice_recv_scattered_burst_vec_avx2(rx_queue, rx_pkts + retval,
                                                    nb_pkts, offload);
}
...
uint16_t ice_recv_scattered_pkts_vec_avx2(void *rx_queue,
                                          struct rte_mbuf **rx_pkts,
                                          uint16_t nb_pkts) {
  return ice_recv_scattered_pkts_vec_avx2_common(rx_queue, rx_pkts, nb_pkts,
                                                 false);
}
...
static __rte_always_inline uint16_t ice_xmit_fixed_burst_vec_avx2(
...
static __rte_always_inline uint16_t ice_xmit_pkts_vec_avx2_common(
    void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts, bool offload) {
  uint16_t nb_tx = 0;
  struct ice_tx_queue *txq = (struct ice_tx_queue *)tx_queue;
  ...
  while (nb_pkts) {
    ...
    ret =
        ice_xmit_fixed_burst_vec_avx2(tx_queue, &tx_pkts[nb_tx], num, offload);
    nb_tx += ret;
    nb_pkts -= ret;
    if (ret < num)
      break;
...
uint16_t ice_xmit_pkts_vec_avx2_offload(void *tx_queue,
