...
static inline __m128i ice_flex_rxd_to_fdir_flags_vec(const __m128i fdir_id0_3) {
...
static inline void ice_rxq_rearm(struct ice_rx_queue *rxq) {
  int i;
  uint16_t rx_id;
  ...
  if (rte_mempool_get_bulk(rxq->mp, (void *)rxep, ICE_RXQ_REARM_THRESH) < 0) {
...
static inline void ice_rx_desc_to_olflags_v(struct ice_rx_queue *rxq,
                                            __m128i descs[4],
   ...
   */
  const __m128i desc_mask = _mm_set_epi32(0x30f0, 0x30f0, 0x30f0, 0x30f0);
  ...
  if (rxq->fdir_enabled) {
    ...
    const __m128i fdir_flags = ice_flex_rxd_to_fdir_flags_vec(fdir_id0_3);
...
static inline void ice_rx_desc_to_ptype_v(__m128i descs[4],
                                          struct rte_mbuf **rx_pkts,
                                          uint32_t *ptype_tbl) {
  const __m128i ptype_mask =
      _mm_set_epi16(ICE_RX_FLEX_DESC_PTYPE_M, 0, ICE_RX_FLEX_DESC_PTYPE_M, 0,
                    ICE_RX_FLEX_DESC_PTYPE_M, 0, ICE_RX_FLEX_DESC_PTYPE_M, 0);
 ...
 */
static inline uint16_t _ice_recv_raw_pkts_vec(struct ice_rx_queue *rxq,
                                              struct rte_mbuf **rx_pkts,
                                              uint16_t nb_pkts,
  ...
  for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
       pos += ICE_DESCS_PER_LOOP, rxdp += ICE_DESCS_PER_LOOP) {
 ...
 * - nb_pkts < ICE_DESCS_PER_LOOP, just return no packet
 */
static uint16_t ice_recv_scattered_burst_vec(void *rx_queue,
                                             struct rte_mbuf **rx_pkts,
                                             uint16_t nb_pkts) {
  ...
  uint16_t nb_bufs = _ice_recv_raw_pkts_vec(rxq, rx_pkts, nb_pkts, split_flags);
  if (nb_bufs == 0)
    return 0;
...
uint16_t ice_recv_scattered_pkts_vec(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_pkts > ICE_VPMD_RX_BURST) {
    ...
    burst = ice_recv_scattered_burst_vec(rx_queue, rx_pkts + retval,
                                         ICE_VPMD_RX_BURST);
    retval += burst;
    nb_pkts -= burst;
  ...
  return retval +
         ice_recv_scattered_burst_vec(rx_queue, rx_pkts + retval, nb_pkts);
}
...
static inline void ice_vtx(volatile struct ice_tx_desc *txdp,
...
static uint16_t ice_xmit_fixed_burst_vec(void *tx_queue,
...
int __rte_cold ice_rxq_vec_setup(struct ice_rx_queue *rxq) {
  if (!rxq)
    return -1;
...
int __rte_cold ice_txq_vec_setup(struct ice_tx_queue __rte_unused *txq) {
  if (!txq)
    return -1;
...
int __rte_cold ice_rx_vec_dev_check(struct rte_eth_dev *dev) {
  return ice_rx_vec_dev_check_default(dev);
}

int __rte_cold ice_tx_vec_dev_check(struct rte_eth_dev *dev) {
