...
void ice_dcf_tm_conf_uninit(struct rte_eth_dev *dev) {
  ...
  while ((shaper_profile = TAILQ_FIRST(&hw->tm_conf.shaper_profile_list))) {
...
ice_dcf_tm_node_search(struct rte_eth_dev *dev, uint32_t node_id,
...
static int ice_dcf_node_param_check(struct ice_dcf_hw *hw, uint32_t node_id,
...
static int ice_dcf_node_add(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  ret = ice_dcf_node_param_check(hw, node_id, priority, weight, params, error);
  if (ret)
    return ret;
  ...
  if (params->shaper_profile_id != RTE_TM_SHAPER_PROFILE_ID_NONE) {
    shaper_profile =
        ice_dcf_shaper_profile_search(dev, params->shaper_profile_id);
    if (!shaper_profile) {
  ...
  if (parent_node_id == RTE_TM_NODE_ID_NULL) {
    ...
    /* add the root node */
    tm_node = rte_zmalloc("ice_dcf_tm_node", sizeof(struct ice_dcf_tm_node), 0);
    if (!tm_node)
      return -ENOMEM;
    tm_node->id = node_id;
    tm_node->parent = NULL;
  ...
  parent_node = ice_dcf_tm_node_search(dev, parent_node_id, &parent_node_type);
  if (!parent_node) {
    error->type = RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID;
  ...
  rte_memcpy(&tm_node->params, params, sizeof(struct rte_tm_node_params));
  if (parent_node_type == ICE_DCF_TM_NODE_TYPE_PORT) {
    TAILQ_INSERT_TAIL(&hw->tm_conf.tc_list, tm_node, node);
    tm_node->tc = hw->tm_conf.nb_tc_node;
    hw->tm_conf.nb_tc_node++;
  } else {
    TAILQ_INSERT_TAIL(&hw->tm_conf.vsi_list, tm_node, node);
...
static int ice_dcf_shaper_profile_add(struct rte_eth_dev *dev,
  ...
  shaper_profile = rte_zmalloc("ice_dcf_tm_shaper_profile",
                               sizeof(struct ice_dcf_tm_shaper_profile), 0);
  if (!shaper_profile)
    return -ENOMEM;
  shaper_profile->shaper_profile_id = shaper_profile_id;
  rte_memcpy(&shaper_profile->profile, profile,
             sizeof(struct rte_tm_shaper_params));
  TAILQ_INSERT_TAIL(&hw->tm_conf.shaper_profile_list, shaper_profile, node);
...
static int ice_dcf_set_vf_bw(struct ice_dcf_hw *hw,
  ...
  if (err)
    PMD_DRV_LOG(ERR, "fail to execute command %s", "VIRTCHNL_OP_DCF_CONFIG_BW");
  return err;
}

static int ice_dcf_validate_tc_bw(struct virtchnl_dcf_bw_cfg_list *tc_bw,
                                  uint32_t port_bw) {
  ...
  for (i = 0; i < tc_bw->num_elem; i++) {
    ...
    if (committed && committed + rest_peak > port_bw) {
      PMD_DRV_LOG(ERR,
                  "Total value of TC%u min bandwidth and other TCs' max "
                  "bandwidth %ukbps should be less than port link speed %ukbps",
    ...
    if (committed && committed < ICE_SCHED_MIN_BW) {
      PMD_DRV_LOG(
          ERR,
          "If TC%u min Tx bandwidth is set, it cannot be less than 500Kbps",
          cfg->tc_num);
      return -EINVAL;
    }
    ...
    if (peak && committed > peak) {
      PMD_DRV_LOG(
          ERR, "TC%u Min Tx bandwidth cannot be greater than max Tx bandwidth",
          cfg->tc_num);
      return -EINVAL;
    }
    ...
    if (peak > port_bw) {
      PMD_DRV_LOG(ERR,
                  "TC%u max Tx bandwidth %uKbps is greater than current link "
                  "speed %uKbps",
...
static int ice_dcf_commit_check(struct ice_dcf_hw *hw) {
  ...
  TAILQ_FOREACH(tm_node, tc_list, node) {
    if (tm_node->reference_count != hw->num_vfs) {
      PMD_DRV_LOG(ERR, "Not all VFs are binded to TC%u", tm_node->tc);
      return ICE_ERR_PARAM;
    }
  }
...
int ice_dcf_replay_vf_bw(struct ice_dcf_hw *hw, uint16_t vf_id) {
  ...
  ret = ice_aq_query_port_ets(parent_hw->port_info, hw->ets_config,
                              sizeof(*hw->ets_config), NULL);
  if (ret) {
  ...
  size = sizeof(struct virtchnl_dcf_bw_cfg_list) +
         sizeof(struct virtchnl_dcf_bw_cfg) * (hw->tm_conf.nb_tc_node - 1);
...
static int ice_dcf_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
                                    __rte_unused struct rte_tm_error *error) {
  struct ice_dcf_adapter *adapter = dev->data->dev_private;
  struct ice_dcf_hw *hw = &adapter->real_hw;
  struct virtchnl_dcf_bw_cfg_list *vf_bw;
  ...
  for (i = 0; i < tc_bw->num_elem; i++) {
    ...
    tc_bw->cfg[i].bw_type |= VIRTCHNL_DCF_BW_PIR | VIRTCHNL_DCF_BW_CIR;
  }
  ...
  for (vf_id = 1; vf_id < hw->num_vfs; vf_id++) {
    ...
    TAILQ_FOREACH(tm_node, vsi_list, node) {
      ...
      tc_bw->cfg[tm_node->tc].shaper.peak += vf_bw->cfg[num_elem].shaper.peak;
    ...
    ice_memcpy(hw->qos_bw_cfg[vf_id], vf_bw, size, ICE_NONDMA_TO_NONDMA);
  ...
  ice_memcpy(hw->qos_bw_cfg[ICE_DCF_VFID], tc_bw, size, ICE_NONDMA_TO_NONDMA);
