  ...
  for (i = 0; i < ICE_RX_MAX_BURST;
    ...
    for (j = 0; j < nb_dd; j++) {
      ...
      if (ice_timestamp_dynflag > 0) {
        ts_ns = ice_tstamp_convert_32b_64b(
            hw, ad, rxq->hw_register_set,
...
static uint16_t ice_recv_scattered_pkts(void *rx_queue,
  ...
  while (nb_rx < nb_pkts) {
    ...
    if (ice_timestamp_dynflag > 0) {
      ts_ns =
          ice_tstamp_convert_32b_64b(hw, ad, rxq->hw_register_set,
...
static inline uint16_t ice_calc_context_desc(uint64_t flags) {
  static uint64_t mask = RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_QINQ |
                         RTE_MBUF_F_TX_OUTER_IP_CKSUM |
...
uint16_t ice_prep_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (ol_flags & RTE_MBUF_F_TX_TCP_SEG &&
        (m->tso_segsz < ICE_MIN_TSO_MSS || m->tso_segsz > ICE_MAX_TSO_MSS ||
