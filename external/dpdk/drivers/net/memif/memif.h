...
typedef struct __rte_packed {
  uint8_t
      name[MEMIF_NAME_SZ]; /**< Client app name. In this case DPDK version */
...
#define MEMIF_MSG_ADD_RING_FLAG_C2S 1 /**< ring is in C2S direction */
  ...
  memif_region_index_t
      region; /**< region index on which this ring is located */
...
#define MEMIF_DESC_FLAG_NEXT 1 /**< is chained buffer */
  memif_region_index_t
      region;      /**< region index on which the buffer is located */
  uint32_t length; /**< buffer length */
  memif_region_offset_t offset; /**< buffer offset */
  uint32_t metadata;
} memif_desc_t;

#define MEMIF_CACHELINE_ALIGN_MARK(mark) RTE_MARKER mark __rte_cache_aligned;
