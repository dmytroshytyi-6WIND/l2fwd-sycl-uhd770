...
static ssize_t memif_msg_send(int fd, memif_msg_t *msg, int afd) {
  struct msghdr mh = {0};
  struct iovec iov[1];
  struct cmsghdr *cmsg;
...
static int memif_msg_send_from_queue(struct memif_control_channel *cc) {
  ssize_t size;
  int ret = 0;
  ...
  size = memif_msg_send(rte_intr_fd_get(cc->intr_handle), &e->msg, e->fd);
...
void memif_msg_enq_disconnect(struct memif_control_channel *cc,
  ...
  if (reason != NULL) {
    ...
    if (cc->dev != NULL) {
      ...
      strlcpy(pmd->local_disc_string, reason, sizeof(pmd->local_disc_string));
    }
  }
}
...
static int memif_msg_receive_hello(struct rte_eth_dev *dev, memif_msg_t *msg) {
  ...
  pmd->run.num_c2s_rings = RTE_MIN(h->max_c2s_ring + 1, pmd->cfg.num_c2s_rings);
  pmd->run.num_s2c_rings = RTE_MIN(h->max_s2c_ring + 1, pmd->cfg.num_s2c_rings);
  pmd->run.log2_ring_size =
      RTE_MIN(h->max_log2_ring_size, pmd->cfg.log2_ring_size);
...
static int memif_msg_receive_init(struct memif_control_channel *cc,
                                  memif_msg_t *msg) {
  ...
  TAILQ_FOREACH(elt, &cc->socket->dev_queue, next) {
    ...
    if (((pmd->flags & ETH_MEMIF_FLAG_DISABLED) == 0) && (pmd->id == i->id) &&
        (pmd->role == MEMIF_ROLE_SERVER)) {
      if (pmd->flags & (ETH_MEMIF_FLAG_CONNECTING | ETH_MEMIF_FLAG_CONNECTED)) {
      ...
      strlcpy(pmd->remote_name, (char *)i->name, sizeof(pmd->remote_name));
      ...
      if (*pmd->secret != '\0') {
        ...
        if (strncmp(pmd->secret, (char *)i->secret, ETH_MEMIF_SECRET_SIZE) !=
            0) {
...
static int memif_msg_receive_add_region(struct rte_eth_dev *dev,
...
static int memif_msg_receive_add_ring(struct rte_eth_dev *dev, memif_msg_t *msg,
                                      int fd) {
  struct pmd_internals *pmd = dev->data->dev_private;
  ...
  mq = (ar->flags & MEMIF_MSG_ADD_RING_FLAG_C2S)
           ? dev->data->rx_queues[ar->index]
           : dev->data->tx_queues[ar->index];
...
static int memif_msg_receive_connect(struct rte_eth_dev *dev,
                                     memif_msg_t *msg) {
  struct pmd_internals *pmd = dev->data->dev_private;
  ...
  strlcpy(pmd->remote_if_name, (char *)c->if_name, sizeof(pmd->remote_if_name));
  MIF_LOG(INFO, "Remote interface %s connected.", pmd->remote_if_name);

  return 0;
}

static int memif_msg_receive_connected(struct rte_eth_dev *dev,
                                       memif_msg_t *msg) {
  struct pmd_internals *pmd = dev->data->dev_private;
...
static int memif_msg_receive_disconnect(struct rte_eth_dev *dev,
                                        memif_msg_t *msg) {
  struct pmd_internals *pmd = dev->data->dev_private;
...
static int memif_msg_enq_ack(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
...
static int memif_msg_enq_init(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct memif_msg_queue_elt *e = memif_msg_enq(pmd->cc);
  memif_msg_init_t *i;
...
static int memif_msg_enq_add_region(struct rte_eth_dev *dev, uint8_t idx) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct pmd_process_private *proc_private = dev->process_private;
  struct memif_msg_queue_elt *e = memif_msg_enq(pmd->cc);
...
static int memif_msg_enq_add_ring(struct rte_eth_dev *dev, uint8_t idx,
...
static int memif_msg_enq_connect(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct memif_msg_queue_elt *e = memif_msg_enq(pmd->cc);
...
static int memif_msg_enq_connected(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct memif_msg_queue_elt *e = memif_msg_enq(pmd->cc);
...
static void memif_intr_unregister_handler(struct rte_intr_handle *intr_handle,
                                          void *arg) {
...
void memif_disconnect(struct rte_eth_dev *dev) {
  ...
  if (pmd->cc != NULL) {
    ...
    if (TAILQ_FIRST(&pmd->cc->msg_queue) != NULL) {
      MIF_LOG(WARNING, "Unexpected message(s) in message queue.");
    }

    ih = pmd->cc->intr_handle;
    if (rte_intr_fd_get(ih) > 0) {
      ret = rte_intr_callback_unregister(ih, memif_intr_handler, pmd->cc);
      /*
      ...
      if (ret == -EAGAIN) {
        ret = rte_intr_callback_unregister_pending(
            ih, memif_intr_handler, pmd->cc, memif_intr_unregister_handler);
      } else if (ret > 0) {
...
static int memif_msg_receive(struct memif_control_channel *cc) {
  ...
  case MEMIF_MSG_TYPE_HELLO:
    ...
    for (i = 0; i < pmd->run.num_c2s_rings; i++) {
      ret = memif_msg_enq_add_ring(cc->dev, i, MEMIF_RING_C2S);
      if (ret < 0)
        goto exit;
    }
    for (i = 0; i < pmd->run.num_s2c_rings; i++) {
      ret = memif_msg_enq_add_ring(cc->dev, i, MEMIF_RING_S2C);
      if (ret < 0)
        goto exit;
    }
...
static void memif_intr_handler(void *arg) {
  ...
  if (cc->dev == NULL) {
    ...
    ret = rte_intr_callback_unregister_pending(
        cc->intr_handle, memif_intr_handler, cc, memif_intr_unregister_handler);
    if (ret < 0)
      MIF_LOG(WARNING, "Failed to unregister control channel callback.");
    return;
  }
...
static struct memif_socket *memif_socket_create(char *key, uint8_t listener,
                                                bool is_abstract) {
  ...
  if (listener != 0) {
    ...
    if (is_abstract) {
      ...
      sunlen = RTE_MIN(1 + strlen(sock->filename), MEMIF_SOCKET_UN_SIZE) +
    ...
    /* Allocate interrupt instance */
    sock->intr_handle = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_SHARED);
    ...
    ret = rte_intr_callback_register(sock->intr_handle, memif_listener_handler,
                                     sock);
    if (ret < 0) {
...
static struct rte_hash *memif_create_socket_hash(void) {
...
int memif_socket_init(struct rte_eth_dev *dev, const char *socket_filename) {
  ...
  if (ret < 0) {
    socket = memif_socket_create(key, (pmd->role == MEMIF_ROLE_CLIENT) ? 0 : 1,
  ...
  TAILQ_FOREACH(elt, &socket->dev_queue, next) {
    ...
    if (tmp_pmd->id == pmd->id && tmp_pmd->role == pmd->role) {
      MIF_LOG(ERR,
              "Two interfaces with the same id (%d) can "
...
void memif_socket_remove_device(struct rte_eth_dev *dev) {
  ...
  if (TAILQ_EMPTY(&socket->dev_queue)) {
    ...
    if (socket->listener && !(pmd->flags & ETH_MEMIF_FLAG_SOCKET_ABSTRACT)) {
      ...
      if (ret < 0)
        MIF_LOG(ERR, "Failed to remove socket file: %s", socket->filename);
    }
    if (pmd->role != MEMIF_ROLE_CLIENT)
...
int memif_connect_client(struct rte_eth_dev *dev) {
  ...
  if (pmd->flags & ETH_MEMIF_FLAG_SOCKET_ABSTRACT) {
    ...
    sunlen = RTE_MIN(strlen(pmd->socket_filename) + 1, MEMIF_SOCKET_UN_SIZE) +
  ...
  pmd->cc = rte_zmalloc("memif-cc", sizeof(struct memif_control_channel), 0);
  if (pmd->cc == NULL) {
    MIF_LOG(ERR, "Failed to allocate control channel.");
    goto error;
  }
  ...
  /* Allocate interrupt instance */
  pmd->cc->intr_handle = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_SHARED);
  ...
  ret = rte_intr_callback_register(pmd->cc->intr_handle, memif_intr_handler,
                                   pmd->cc);
  if (ret < 0) {
