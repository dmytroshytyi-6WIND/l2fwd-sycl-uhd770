...
static const char *const valid_arguments[] = {ETH_MEMIF_ID_ARG,
                                              ETH_MEMIF_ROLE_ARG,
...
const char *memif_version(void) {
  return (
      "memif-" RTE_STR(MEMIF_VERSION_MAJOR) "." RTE_STR(MEMIF_VERSION_MINOR));
}
...
static int memif_mp_send_region(const struct rte_mp_msg *msg,
                                const void *peer) {
  struct rte_eth_dev *dev;
  struct pmd_process_private *proc_private;
  const struct mp_region_msg *msg_param =
      (const struct mp_region_msg *)msg->param;
  ...
  if (!dev) {
    MIF_LOG(ERR, "Failed to get port id for %s", msg_param->port_name);
    return -1;
  }
 ...
 */
static int memif_mp_request_regions(struct rte_eth_dev *dev) {
  int ret, i;
  ...
  uint16_t max_region_num =
      (pmd->flags & ETH_MEMIF_FLAG_ZERO_COPY) ? 1 : ETH_MEMIF_MAX_REGION_NUM;
  ...
  for (i = 0; i < max_region_num; i++) {
    ...
    if (ret < 0 || replies.nb_received != 1) {
      MIF_LOG(ERR, "Failed to send mp msg: %d", rte_errno);
      return -1;
    }
...
static int memif_dev_info(struct rte_eth_dev *dev __rte_unused,
...
static memif_ring_t *memif_get_ring(struct pmd_internals *pmd,
                                    struct pmd_process_private *proc_private,
  ...
  int ring_size = sizeof(memif_ring_t) +
                  sizeof(memif_desc_t) * (1 << pmd->run.log2_ring_size);
...
static memif_region_offset_t memif_get_ring_offset(struct rte_eth_dev *dev,
...
static void *memif_get_buffer(struct pmd_process_private *proc_private,
                              memif_desc_t *d) {
  return ((uint8_t *)proc_private->regions[d->region]->addr + d->offset);
}

/* Free mbufs received by server */
static void memif_free_stored_mbufs(struct pmd_process_private *proc_private,
                                    struct memif_queue *mq) {
  uint16_t cur_tail;
...
static int memif_pktmbuf_chain(struct rte_mbuf *head, struct rte_mbuf *cur_tail,
...
static uint16_t eth_memif_rx(void *queue, struct rte_mbuf **bufs,
                             uint16_t nb_pkts) {
  ...
  uint16_t mbuf_size =
      rte_pktmbuf_data_room_size(mq->mempool) - RTE_PKTMBUF_HEADROOM;
  ...
  if (unlikely(ring == NULL)) {
    ...
    if (ret < 0)
      MIF_LOG(ERR, "Failed to get port %u link info: %s", mq->in_port,
              rte_strerror(-ret));
    return 0;
  }
  ...
  if (((ring->flags & MEMIF_RING_FLAG_MASK_INT) == 0) &&
    ...
    size = read(rte_intr_fd_get(mq->intr_handle), &b, sizeof(b));
...
static uint16_t eth_memif_rx_zc(void *queue, struct rte_mbuf **bufs,
                                uint16_t nb_pkts) {
  struct memif_queue *mq = queue;
  struct pmd_internals *pmd = rte_eth_devices[mq->in_port].data->dev_private;
  struct pmd_process_private *proc_private =
...
refill:
  ...
  while (n_slots--) {
    ...
    mbuf = mq->buffers[s0];
    /* populate descriptor */
    d0->length = rte_pktmbuf_data_room_size(mq->mempool) - RTE_PKTMBUF_HEADROOM;
    d0->region = 1;
...
static uint16_t eth_memif_tx(void *queue, struct rte_mbuf **bufs,
                             uint16_t nb_pkts) {
  struct memif_queue *mq = queue;
  struct pmd_internals *pmd = rte_eth_devices[mq->in_port].data->dev_private;
  struct pmd_process_private *proc_private =
  ...
  while (n_tx_pkts < nb_pkts && n_free) {
    ...
    dst_off = 0;
    dst_len = (type == MEMIF_RING_C2S) ? pmd->run.pkt_buffer_size : d0->length;
    ...
    while (src_len) {
      if (dst_len == 0) {
        if (n_free) {
          ...
          dst_off = 0;
          dst_len =
              (type == MEMIF_RING_C2S) ? pmd->run.pkt_buffer_size : d0->length;
          d0->flags = 0;
        } else {
          slot = saved_slot;
          goto no_free_slots;
...
no_free_slots:
  ...
  if (((ring->flags & MEMIF_RING_FLAG_MASK_INT) == 0) &&
      (rte_intr_fd_get(mq->intr_handle) >= 0)) {
    a = 1;
    size = write(rte_intr_fd_get(mq->intr_handle), &a, sizeof(a));
    if (unlikely(size < 0)) {
...
static int memif_tx_one_zc(struct pmd_process_private *proc_private,
                           struct memif_queue *mq, memif_ring_t *ring,
                           struct rte_mbuf *mbuf, const uint16_t mask,
...
static uint16_t eth_memif_tx_zc(void *queue, struct rte_mbuf **bufs,
                                uint16_t nb_pkts) {
  struct memif_queue *mq = queue;
  struct pmd_internals *pmd = rte_eth_devices[mq->in_port].data->dev_private;
  struct pmd_process_private *proc_private =
  ...
  while (n_free && (n_tx_pkts < nb_pkts)) {
    while ((n_free > 4) && ((nb_pkts - n_tx_pkts) > 4)) {
      ...
      used_slots =
          memif_tx_one_zc(proc_private, mq, ring, *bufs++, mask, slot, n_free);
...
no_free_slots:
  ...
  if ((ring->flags & MEMIF_RING_FLAG_MASK_INT) == 0) {
    ...
    ssize_t size = write(rte_intr_fd_get(mq->intr_handle), &a, sizeof(a));
    if (unlikely(size < 0)) {
      MIF_LOG(WARNING, "Failed to send interrupt. %s", strerror(errno));
    }
  }
...
static int memif_region_init_shm(struct rte_eth_dev *dev, uint8_t has_buffers) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct pmd_process_private *proc_private = dev->process_private;
  ...
  if (has_buffers == 1)
    r->region_size +=
        (uint32_t)(pmd->run.pkt_buffer_size * (1 << pmd->run.log2_ring_size) *
                   (pmd->run.num_c2s_rings + pmd->run.num_s2c_rings));
  ...
  r->addr =
      mmap(NULL, r->region_size, PROT_READ | PROT_WRITE, MAP_SHARED, r->fd, 0);
...
static int memif_regions_init(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  int ret;
...
static void memif_init_rings(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct pmd_process_private *proc_private = dev->process_private;
  memif_ring_t *ring;
  int i, j;
...
static int memif_init_queues(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  struct memif_queue *mq;
  int i;
  ...
  for (i = 0; i < pmd->run.num_c2s_rings; i++) {
    ...
    if (rte_intr_fd_get(mq->intr_handle) < 0) {
      MIF_LOG(WARNING, "Failed to create eventfd for tx queue %d: %s.", i,
              strerror(errno));
    }
    mq->buffers = NULL;
    if (pmd->flags & ETH_MEMIF_FLAG_ZERO_COPY) {
      mq->buffers = rte_zmalloc(
          "bufs", sizeof(struct rte_mbuf *) * (1 << mq->log2_ring_size), 0);
      if (mq->buffers == NULL)
        return -ENOMEM;
    }
  }
  ...
  for (i = 0; i < pmd->run.num_s2c_rings; i++) {
    ...
    if (rte_intr_fd_set(mq->intr_handle, eventfd(0, EFD_NONBLOCK)))
      return -rte_errno;
    if (rte_intr_fd_get(mq->intr_handle) < 0) {
      MIF_LOG(WARNING, "Failed to create eventfd for rx queue %d: %s.", i,
              strerror(errno));
    }
    mq->buffers = NULL;
    if (pmd->flags & ETH_MEMIF_FLAG_ZERO_COPY) {
  ...
  for (i = 0; i < proc_private->regions_num; i++) {
    ...
    if (mr != NULL) {
      if (mr->addr == NULL) {
        if (mr->fd < 0)
          return -1;
        mr->addr = mmap(NULL, mr->region_size, PROT_READ | PROT_WRITE,
                        MAP_SHARED, mr->fd, 0);
...
static int memif_dev_stop(struct rte_eth_dev *dev) {
  memif_disconnect(dev);
  return 0;
}

static int memif_dev_close(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  int i;
...
static int memif_dev_configure(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
...
static int
memif_tx_queue_setup(struct rte_eth_dev *dev, uint16_t qid,
  ...
  mq->type = (pmd->role == MEMIF_ROLE_CLIENT) ? MEMIF_RING_C2S : MEMIF_RING_S2C;
  mq->n_pkts = 0;
  mq->n_bytes = 0;
...
static int
memif_rx_queue_setup(struct rte_eth_dev *dev, uint16_t qid,
...
static int memif_link_update(struct rte_eth_dev *dev,
...
static int memif_stats_get(struct rte_eth_dev *dev,
  ...
  nq = (tmp < RTE_ETHDEV_QUEUE_STAT_CNTRS) ? tmp : RTE_ETHDEV_QUEUE_STAT_CNTRS;
...
static int memif_stats_reset(struct rte_eth_dev *dev) {
  struct pmd_internals *pmd = dev->data->dev_private;
  int i;
  struct memif_queue *mq;
...
static int memif_create(struct rte_vdev_device *vdev, enum memif_role_t role,
  ...
  process_private = (struct pmd_process_private *)rte_zmalloc(
...
static int memif_set_role(const char *key __rte_unused, const char *value,
                          void *extra_args) {
...
static int memif_set_zc(const char *key __rte_unused, const char *value,
                        void *extra_args) {
...
static int memif_set_id(const char *key __rte_unused, const char *value,
                        void *extra_args) {
...
static int memif_set_bs(const char *key __rte_unused, const char *value,
                        void *extra_args) {
  unsigned long tmp;
...
static int memif_set_rs(const char *key __rte_unused, const char *value,
                        void *extra_args) {
  unsigned long tmp;
  ...
  if (tmp == 0 || tmp > ETH_MEMIF_MAX_LOG2_RING_SIZE) {
    MIF_LOG(ERR, "Invalid ring size: %s (max %u).", value,
...
static int memif_check_socket_filename(const char *filename) {
  char *dir = NULL, *tmp;
  uint32_t idx;
  int ret = 0;
...
static int memif_set_socket_filename(const char *key __rte_unused,
...
static int memif_set_is_socket_abstract(const char *key __rte_unused,
                                        const char *value, void *extra_args) {
  uint32_t *flags = (uint32_t *)extra_args;
...
static int memif_set_mac(const char *key __rte_unused, const char *value,
                         void *extra_args) {
...
static int memif_set_secret(const char *key __rte_unused, const char *value,
                            void *extra_args) {
...
static int rte_pmd_memif_probe(struct rte_vdev_device *vdev) {
  ...
  struct rte_ether_addr *ether_addr =
      rte_zmalloc("", sizeof(struct rte_ether_addr), 0);
  struct rte_eth_dev *eth_dev;
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ...
    eth_dev->process_private = (struct pmd_process_private *)rte_zmalloc(
        name, sizeof(struct pmd_process_private), RTE_CACHE_LINE_SIZE);
  ...
  if (kvlist != NULL) {
    ret =
        rte_kvargs_process(kvlist, ETH_MEMIF_ROLE_ARG, &memif_set_role, &role);
    if (ret < 0)
      goto exit;
    ret = rte_kvargs_process(kvlist, ETH_MEMIF_ID_ARG, &memif_set_id, &id);
    if (ret < 0)
      goto exit;
    ...
    if (ret < 0)
      goto exit;
    ret = rte_kvargs_process(kvlist, ETH_MEMIF_RING_SIZE_ARG, &memif_set_rs,
                             &log2_ring_size);
    if (ret < 0)
      goto exit;
    ...
    if (ret < 0)
      goto exit;
    ret = rte_kvargs_process(kvlist, ETH_MEMIF_MAC_ARG, &memif_set_mac,
                             ether_addr);
    if (ret < 0)
      goto exit;
    ret = rte_kvargs_process(kvlist, ETH_MEMIF_ZC_ARG, &memif_set_zc, &flags);
    if (ret < 0)
      goto exit;
    ret = rte_kvargs_process(kvlist, ETH_MEMIF_SECRET_ARG, &memif_set_secret,
                             (void *)(&secret));
    if (ret < 0)
      goto exit;
  }
  ...
  ret = memif_create(vdev, role, id, flags, socket_filename, log2_ring_size,
                     pkt_buffer_size, secret, ether_addr);
...
RTE_PMD_REGISTER_PARAM_STRING(net_memif, ETH_MEMIF_ID_ARG
                              "=<int>" ETH_MEMIF_ROLE_ARG
                              "=server|client" ETH_MEMIF_PKT_BUFFER_SIZE_ARG
                              "=<int>" ETH_MEMIF_RING_SIZE_ARG
                              "=<int>" ETH_MEMIF_SOCKET_ARG
                              "=<string>" ETH_MEMIF_SOCKET_ABSTRACT_ARG
                              "=yes|no" ETH_MEMIF_MAC_ARG
                              "=xx:xx:xx:xx:xx:xx" ETH_MEMIF_ZC_ARG
                              "=yes|no" ETH_MEMIF_SECRET_ARG "=<string>");
