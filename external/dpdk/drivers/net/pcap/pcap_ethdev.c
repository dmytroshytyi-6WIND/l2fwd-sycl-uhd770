...
static const char *valid_arguments[] = {
    ETH_PCAP_RX_PCAP_ARG,     ETH_PCAP_TX_PCAP_ARG,     ETH_PCAP_RX_IFACE_ARG,
    ETH_PCAP_RX_IFACE_IN_ARG, ETH_PCAP_TX_IFACE_ARG,    ETH_PCAP_IFACE_ARG,
    ETH_PCAP_PHY_MAC_ARG,     ETH_PCAP_INFINITE_RX_ARG, NULL};
...
queue_missed_stat_update(struct rte_eth_dev *dev, unsigned int qid) {
  ...
  struct queue_missed_stat *missed_stat = &internals->rx_queue[qid].missed_stat;
...
static void queue_missed_stat_on_stop_update(struct rte_eth_dev *dev,
                                             unsigned int qid) {
  struct queue_missed_stat *missed_stat = queue_missed_stat_update(dev, qid);

  missed_stat->mnemonic += missed_stat->pcap;
  missed_stat->pcap = 0;
}

static void queue_missed_stat_reset(struct rte_eth_dev *dev, unsigned int qid) {
  struct queue_missed_stat *missed_stat = queue_missed_stat_update(dev, qid);
...
static unsigned long queue_missed_stat_get(struct rte_eth_dev *dev,
                                           unsigned int qid) {
...
static int eth_pcap_rx_jumbo(struct rte_mempool *mb_pool, struct rte_mbuf *mbuf,
...
static uint16_t eth_pcap_rx_infinite(void *queue, struct rte_mbuf **bufs,
                                     uint16_t nb_pkts) {
  int i;
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    rte_memcpy(rte_pktmbuf_mtod(bufs[i], void *),
               rte_pktmbuf_mtod(pcap_buf, void *), pcap_buf->data_len);
...
static uint16_t eth_pcap_rx(void *queue, struct rte_mbuf **bufs,
                            uint16_t nb_pkts) {
  unsigned int i;
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (header.caplen <= rte_pktmbuf_tailroom(mbuf)) {
      ...
      rte_memcpy(rte_pktmbuf_mtod(mbuf, void *), packet, header.caplen);
      mbuf->data_len = (uint16_t)header.caplen;
    } else {
      /* Try read jumbo frame into multi mbufs. */
      if (unlikely(eth_pcap_rx_jumbo(pcap_q->mb_pool, mbuf, packet,
                                     header.caplen) == -1)) {
...
static uint16_t eth_null_rx(void *queue __rte_unused,
 ...
 */
static uint16_t eth_pcap_tx_dumper(void *queue, struct rte_mbuf **bufs,
                                   uint16_t nb_pkts) {
  unsigned int i;
  struct rte_mbuf *mbuf;
  struct pmd_process_private *pp;
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (unlikely(!rte_pktmbuf_is_contiguous(mbuf) && len > sizeof(temp_data))) {
 ...
 */
static uint16_t eth_tx_drop(void *queue, struct rte_mbuf **bufs,
                            uint16_t nb_pkts) {
  unsigned int i;
  uint32_t tx_bytes = 0;
 ...
 */
static uint16_t eth_pcap_tx(void *queue, struct rte_mbuf **bufs,
                            uint16_t nb_pkts) {
  unsigned int i;
  int ret;
  struct rte_mbuf *mbuf;
 ...
 */
static inline int open_iface_live(const char *iface, pcap_t **pcap) {
  *pcap = pcap_open_live(iface, RTE_ETH_PCAP_SNAPLEN, RTE_ETH_PCAP_PROMISC,
                         RTE_ETH_PCAP_TIMEOUT, errbuf);
...
static int open_single_tx_pcap(const char *pcap_filename,
   ...
   * pcap holder.
   */
  tx_pcap = pcap_open_dead_with_tstamp_precision(
      DLT_EN10MB, RTE_ETH_PCAP_SNAPSHOT_LEN, PCAP_TSTAMP_PRECISION_NANO);
  if (tx_pcap == NULL) {
  ...
  *dumper = pcap_dump_open(tx_pcap, pcap_filename);
  if (*dumper == NULL) {
    pcap_close(tx_pcap);
    PMD_LOG(ERR, "Couldn't open %s for writing.", pcap_filename);
    return -1;
  }
...
static int open_single_rx_pcap(const char *pcap_filename, pcap_t **pcap) {
  *pcap = pcap_open_offline(pcap_filename, errbuf);
  if (*pcap == NULL) {
    PMD_LOG(ERR, "Couldn't open %s: %s", pcap_filename, errbuf);
    return -1;
  }
...
static uint64_t count_packets_in_pcap(pcap_t **pcap,
                                      struct pcap_rx_queue *pcap_q) {
  const u_char *packet;
  struct pcap_pkthdr header;
...
static int eth_dev_start(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    if (!pp->tx_dumper[i] && strcmp(tx->type, ETH_PCAP_TX_PCAP_ARG) == 0) {
      if (open_single_tx_pcap(tx->name, &pp->tx_dumper[i]) < 0)
        return -1;
...
static int eth_dev_configure(struct rte_eth_dev *dev __rte_unused) { return 0; }
...
static int eth_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  for (i = 0; i < RTE_ETHDEV_QUEUE_STAT_CNTRS && i < dev->data->nb_rx_queues;
       i++) {
  ...
  for (i = 0; i < RTE_ETHDEV_QUEUE_STAT_CNTRS && i < dev->data->nb_tx_queues;
       i++) {
...
static inline void infinite_rx_ring_free(struct rte_ring *pkts) {
...
static int eth_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
  ...
  if (internals->infinite_rx) {
    ...
    snprintf(ring_name, sizeof(ring_name), "PCAP_RING%" PRIu32, ring_number);

    pcap_q->pkts =
        rte_ring_create(ring_name, rte_align64pow2(pcap_pkt_count + 1), 0,
    ...
    while (eth_pcap_rx(pcap_q, bufs, 1)) {
      ...
      if (bufs[0]->nb_segs != 1) {
        infinite_rx_ring_free(pcap_q->pkts);
        PMD_LOG(ERR, "Multiseg mbufs are not supported in infinite_rx mode.");
        return -EINVAL;
      }

      rte_ring_enqueue_bulk(pcap_q->pkts, (void *const *)bufs, 1, NULL);
    }
...
static int eth_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
...
static int eth_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
...
static int add_queue(struct pmd_devargs *pmd, const char *name,
                     const char *type, pcap_t *pcap, pcap_dumper_t *dumper) {
 ...
 */
static int open_rx_pcap(const char *key, const char *value, void *extra_args) {
 ...
 * for use it later on.
 */
static int open_tx_pcap(const char *key, const char *value, void *extra_args) {
  const char *pcap_filename = value;
 ...
 */
static inline int open_rx_tx_iface(const char *key, const char *value,
                                   void *extra_args) {
...
static inline int set_iface_direction(const char *iface, pcap_t *pcap,
  ...
  if (pcap_setdirection(pcap, direction) < 0) {
    PMD_LOG(ERR, "Setting %s pcap direction %s failed - %s\n", iface,
            direction_str, pcap_geterr(pcap));
    return -1;
  }
...
static inline int open_iface(const char *key, const char *value,
                             void *extra_args) {
  const char *iface = value;
 ...
 */
static inline int open_rx_iface(const char *key, const char *value,
                                void *extra_args) {
  ...
  if (strcmp(key, ETH_PCAP_RX_IFACE_IN_ARG) == 0) {
    ...
    set_iface_direction(pmd->queue[qid].name, pmd->queue[qid].pcap, PCAP_D_IN);
  }

  return 0;
}

static inline int rx_iface_args_process(const char *key, const char *value,
                                        void *extra_args) {
 ...
 */
static int open_tx_iface(const char *key, const char *value, void *extra_args) {
  return open_iface(key, value, extra_args);
}

static int select_phy_mac(const char *key __rte_unused, const char *value,
                          void *extra_args) {
  if (extra_args) {
...
static int get_infinite_rx_arg(const char *key __rte_unused, const char *value,
                               void *extra_args) {
  if (extra_args) {
...
static int pmd_init_internals(struct rte_vdev_device *vdev,
  ...
  PMD_LOG(INFO, "Creating pcap-backed ethdev on numa socket %d", numa_node);

  pp = (struct pmd_process_private *)rte_zmalloc(
      NULL, sizeof(struct pmd_process_private), RTE_CACHE_LINE_SIZE);

  if (pp == NULL) {
    PMD_LOG(ERR, "Failed to allocate memory for process private");
    return -1;
  }
...
static int eth_pcap_update_mac(const char *if_name, struct rte_eth_dev *eth_dev,
...
static int eth_from_pcaps_common(struct rte_vdev_device *vdev,
  ...
  if (pmd_init_internals(vdev, nb_rx_queues, nb_tx_queues, internals, eth_dev) <
      0)
    return -1;
...
static int eth_from_pcaps(struct rte_vdev_device *vdev,
                          struct pmd_devargs_all *devargs_all) {
  ...
  if (single_iface) {
    internals->if_index = osdep_iface_index_get(rx_queues->queue[0].name);

    /* phy_mac arg is applied only only if "iface" devarg is provided */
    if (rx_queues->phy_mac) {
      if (eth_pcap_update_mac(rx_queues->queue[0].name, eth_dev,
  ...
  else if (devargs_all->is_rx_pcap || devargs_all->is_rx_iface || single_iface)
...
static void eth_release_pcaps(struct pmd_devargs *pcaps,
                              struct pmd_devargs *dumpers, int single_iface) {
  unsigned int i;
...
static int pmd_pcap_probe(struct rte_vdev_device *dev) {
  const char *name;
  ...
  if (rte_kvargs_count(kvlist, ETH_PCAP_IFACE_ARG) == 1) {
    ...
    ret = rte_kvargs_process(kvlist, ETH_PCAP_IFACE_ARG, &open_rx_tx_iface,
                             &pcaps);
    if (ret < 0)
      goto free_kvlist;
    ...
    ret = rte_kvargs_process(kvlist, ETH_PCAP_PHY_MAC_ARG, &select_phy_mac,
                             &pcaps.phy_mac);
    if (ret < 0)
      goto free_kvlist;
  ...
  devargs_all.is_rx_iface = (rte_kvargs_count(kvlist, ETH_PCAP_RX_IFACE_ARG) +
                             rte_kvargs_count(kvlist, ETH_PCAP_RX_IFACE_IN_ARG))
                                ? 1
                                : 0;
  ...
  if (devargs_all.is_rx_pcap) {
     ...
     */
    unsigned int infinite_rx_arg_cnt =
        rte_kvargs_count(kvlist, ETH_PCAP_INFINITE_RX_ARG);

    if (infinite_rx_arg_cnt == 1) {
      ret = rte_kvargs_process(kvlist, ETH_PCAP_INFINITE_RX_ARG,
                               &get_infinite_rx_arg, &devargs_all.infinite_rx);
      if (ret < 0)
        goto free_kvlist;
      PMD_LOG(INFO, "infinite_rx has been %s for %s",
              devargs_all.infinite_rx ? "enabled" : "disabled", name);

    } else if (infinite_rx_arg_cnt > 1) {
      PMD_LOG(WARNING,
              "infinite_rx has not been enabled since the "
    ...
    ret =
        rte_kvargs_process(kvlist, ETH_PCAP_RX_PCAP_ARG, &open_rx_pcap, &pcaps);
  } else if (devargs_all.is_rx_iface) {
    ret = rte_kvargs_process(kvlist, NULL, &rx_iface_args_process, &pcaps);
    ...
    for (i = 0; i < num_tx_queues; i++)
      ret = add_queue(&pcaps, "dummy_rx", "rx_null", NULL, NULL);
  } else {
   ...
   */
  if (devargs_all.is_tx_pcap) {
    ret = rte_kvargs_process(kvlist, ETH_PCAP_TX_PCAP_ARG, &open_tx_pcap,
                             &dumpers);
  } else if (devargs_all.is_tx_iface) {
    ret = rte_kvargs_process(kvlist, ETH_PCAP_TX_IFACE_ARG, &open_tx_iface,
                             &dumpers);
  } else {
    unsigned int i;
    ...
    for (i = 0; i < pcaps.num_of_queue; i++)
      ret = add_queue(&dumpers, "dummy_tx", "tx_drop", NULL, NULL);
  }
...
RTE_PMD_REGISTER_PARAM_STRING(net_pcap, ETH_PCAP_RX_PCAP_ARG
                              "=<string> " ETH_PCAP_TX_PCAP_ARG
                              "=<string> " ETH_PCAP_RX_IFACE_ARG
                              "=<ifc> " ETH_PCAP_RX_IFACE_IN_ARG
                              "=<ifc> " ETH_PCAP_TX_IFACE_ARG
                              "=<ifc> " ETH_PCAP_IFACE_ARG
                              "=<ifc> " ETH_PCAP_PHY_MAC_ARG
                              "=<int>" ETH_PCAP_INFINITE_RX_ARG "=<0|1>");
