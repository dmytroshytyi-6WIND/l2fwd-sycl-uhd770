 ...
 */
static const char *iface_guid(const char *name) {
 ...
 */
int osdep_iface_index_get(const char *device_name) {
  ...
  if (guid != NULL)
    StringCbPrintfW(adapter_name, sizeof(adapter_name), L"\\DEVICE\\TCPIP_%S",
                    guid);
  else
    StringCbPrintfW(adapter_name, sizeof(adapter_name), L"%S", device_name);
 ...
 */
int osdep_iface_mac_get(const char *device_name, struct rte_ether_addr *mac) {
    ...
    if (strcmp(cur->AdapterName, adapter_name) == 0) {
      if (cur->PhysicalAddressLength != RTE_ETHER_ADDR_LEN) {
        PMD_LOG(ERR, "Physical address length: want %u, got %lu",
                RTE_ETHER_ADDR_LEN, cur->PhysicalAddressLength);
        break;
      }

      memcpy(mac->addr_bytes, cur->PhysicalAddress, RTE_ETHER_ADDR_LEN);
      ret = 0;
      break;
    }
  }
