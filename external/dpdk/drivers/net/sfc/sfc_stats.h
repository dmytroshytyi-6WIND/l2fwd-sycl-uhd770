 ...
 */
static inline void sfc_pkts_bytes_add(union sfc_pkts_bytes *st, uint64_t pkts,
                                      uint64_t bytes) {
 ...
 */
static inline void sfc_pkts_bytes_get(const union sfc_pkts_bytes *st,
                                      union sfc_pkts_bytes *result) {
#if SFC_SW_STATS_ATOMIC
  result->pkts_bytes.int128 =
      __atomic_load_n(&st->pkts_bytes.int128, __ATOMIC_RELAXED);
#else
  *result = *st;
#endif
}
