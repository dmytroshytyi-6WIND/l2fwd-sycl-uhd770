...
int sfc_efx_tso_alloc_tsoh_objs(struct sfc_efx_tx_sw_desc *sw_ring,
  ...
  for (i = 0; i < txq_entries; ++i) {
    sw_ring[i].tsoh =
        rte_malloc_socket("sfc-efx-txq-tsoh-obj", SFC_TSOH_STD_LEN,
...
unsigned int sfc_tso_prepare_header(uint8_t *tsoh, size_t header_len,
  ...
  do {
    ...
    rte_memcpy(tsoh, rte_pktmbuf_mtod(m, uint8_t *), bytes_to_copy);
...
int sfc_efx_tso_do(struct sfc_efx_txq *txq, unsigned int idx,
  ...
  efx_tx_qdesc_tso2_create(txq->common, packet_id, 0, sent_seq, m->tso_segsz,
                           *pend, EFX_TX_FATSOV2_OPT_NDESCS);

  *pend += EFX_TX_FATSOV2_OPT_NDESCS;
  *pkt_descs += EFX_TX_FATSOV2_OPT_NDESCS;

  efx_tx_qdesc_dma_create(txq->common, header_paddr, header_len, B_FALSE,
                          (*pend)++);
  (*pkt_descs)++;
  *pkt_len -= header_len;
