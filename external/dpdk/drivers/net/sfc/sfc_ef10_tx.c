...
#define SFC_EF10_TX_DMA_DESC_LEN_MAX ((1u << ESF_DZ_TX_KER_BYTE_CNT_WIDTH) - 1)
...
static bool sfc_ef10_tx_get_event(struct sfc_ef10_txq *txq,
                                  efx_qword_t *tx_ev) {
  ...
  *tx_ev = evq_hw_ring[txq->evq_read_ptr & txq->ptr_mask];
  ...
  if (unlikely(EFX_QWORD_FIELD(*tx_ev, FSF_AZ_EV_CODE) !=
    ...
    sfc_ef10_tx_err(&txq->dp.dpq, "TxQ exception at EvQ read ptr %#x",
                    txq->evq_read_ptr);
    return false;
  }
...
static unsigned int sfc_ef10_tx_process_events(struct sfc_ef10_txq *txq) {
...
static void sfc_ef10_tx_reap(struct sfc_ef10_txq *txq) {
...
static void sfc_ef10_tx_qdesc_dma_create(rte_iova_t addr, uint16_t size,
                                         bool eop, efx_qword_t *edp) {
  EFX_POPULATE_QWORD_4(*edp, ESF_DZ_TX_KER_TYPE, 0, ESF_DZ_TX_KER_CONT, !eop,
                       ESF_DZ_TX_KER_BYTE_CNT, size, ESF_DZ_TX_KER_BUF_ADDR,
                       addr);
}

static void sfc_ef10_tx_qdesc_tso2_create(struct sfc_ef10_txq *const txq,
                                          unsigned int added, uint16_t ipv4_id,
                                          uint16_t outer_ipv4_id,
                                          uint32_t tcp_seq, uint16_t tcp_mss) {
  EFX_POPULATE_QWORD_5(txq->txq_hw_ring[added & txq->ptr_mask],
                       ESF_DZ_TX_DESC_IS_OPT, 1, ESF_DZ_TX_OPTION_TYPE,
                       ESE_DZ_TX_OPTION_DESC_TSO, ESF_DZ_TX_TSO_OPTION_TYPE,
                       ESE_DZ_TX_TSO_OPTION_DESC_FATSO2A, ESF_DZ_TX_TSO_IP_ID,
                       ipv4_id, ESF_DZ_TX_TSO_TCP_SEQNO, tcp_seq);
  EFX_POPULATE_QWORD_5(txq->txq_hw_ring[(added + 1) & txq->ptr_mask],
                       ESF_DZ_TX_DESC_IS_OPT, 1, ESF_DZ_TX_OPTION_TYPE,
                       ESE_DZ_TX_OPTION_DESC_TSO, ESF_DZ_TX_TSO_OPTION_TYPE,
                       ESE_DZ_TX_TSO_OPTION_DESC_FATSO2B, ESF_DZ_TX_TSO_TCP_MSS,
                       tcp_mss, ESF_DZ_TX_TSO_OUTER_IPID, outer_ipv4_id);
}

static inline void sfc_ef10_tx_qpush(struct sfc_ef10_txq *txq,
                                     unsigned int added, unsigned int pushed) {
  efx_qword_t desc;
  efx_oword_t oword;
  ...
  EFX_POPULATE_OWORD_3(oword, ERF_DZ_TX_DESC_WPTR, added & txq->ptr_mask,
                       ERF_DZ_TX_DESC_HWORD, EFX_QWORD_FIELD(desc, EFX_DWORD_1),
                       ERF_DZ_TX_DESC_LWORD,
                       EFX_QWORD_FIELD(desc, EFX_DWORD_0));
...
static unsigned int sfc_ef10_tx_pkt_descs_max(const struct rte_mbuf *m) {
...
#define SFC_MBUF_PKT_LEN_MAX UINT32_MAX
   ...
   */
  extra_descs_per_pkt =
      (RTE_MIN((unsigned int)EFX_MAC_PDU_MAX, SFC_MBUF_PKT_LEN_MAX) - 1) /
      SFC_EF10_TX_DMA_DESC_LEN_MAX;

  return m->nb_segs +
         RTE_MIN(m->nb_segs * extra_descs_per_seg, extra_descs_per_pkt);
}

static bool sfc_ef10_try_reap(struct sfc_ef10_txq *const txq,
                              unsigned int added, unsigned int needed_desc,
                              unsigned int *dma_desc_space, bool *reap_done) {
  if (*reap_done)
    return false;
...
static uint16_t sfc_ef10_prepare_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                                      uint16_t nb_pkts) {
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (!(m->ol_flags & RTE_MBUF_F_TX_TCP_SEG)) {
      ...
      for (m_seg = m; m_seg != NULL; m_seg = m_seg->next) {
        if (m_seg->data_len > SFC_EF10_TX_DMA_DESC_LEN_MAX) {
          rte_errno = EINVAL;
          break;
        }
      }
...
#endif
    ret = sfc_dp_tx_prepare_pkt(
        m, 0, SFC_TSOH_STD_LEN, txq->tso_tcp_header_offset_limit,
        txq->max_fill_level, SFC_EF10_TSO_OPT_DESCS_NUM, 0);
    if (unlikely(ret != 0)) {
      rte_errno = ret;
      break;
    }
...
static int sfc_ef10_xmit_tso_pkt(struct sfc_ef10_txq *const txq,
                                 struct rte_mbuf *m_seg, unsigned int *added,
                                 unsigned int *dma_desc_space,
                                 bool *reap_done) {
  size_t iph_off = ((m_seg->ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK)
                        ? m_seg->outer_l2_len + m_seg->outer_l3_len
                        : 0) +
                   m_seg->l2_len;
   ...
   * several descriptors.
   */
  needed_desc = m_seg->nb_segs + (unsigned int)SFC_EF10_TSO_OPT_DESCS_NUM +
                (unsigned int)SFC_EF10_TSO_HDR_DESCS_NUM;

  if (needed_desc > *dma_desc_space &&
      !sfc_ef10_try_reap(txq, pkt_start, needed_desc, dma_desc_space,
                         reap_done)) {
    /*
     ...
     */
    if (*dma_desc_space <
        SFC_EF10_TSO_OPT_DESCS_NUM + SFC_EF10_TSO_HDR_DESCS_NUM + 1)
      return EMSGSIZE;
  }
  ...
  if (rte_pktmbuf_data_len(m_seg) >= header_len) {
    ...
    if (rte_pktmbuf_data_len(m_seg) == header_len) {
       ...
       */
      txq->sw_ring[(pkt_start + SFC_EF10_TSO_OPT_DESCS_NUM) & txq->ptr_mask]
          .mbuf = m_seg;
      m_seg = m_seg->next;
      in_off = 0;
    ...
    unsigned int hdr_addr_off = (*added & txq->ptr_mask) * SFC_TSOH_STD_LEN;
    ...
    copied_segs = sfc_tso_prepare_header(hdr_addr, header_len, &m_seg, &in_off);
  ...
  if (first_m_seg->ol_flags & RTE_MBUF_F_TX_OUTER_IPV4)
    outer_packet_id = sfc_tso_ip4_get_ipid(hdr_addr, first_m_seg->outer_l2_len);
  ...
  do {
    ...
    do {
      ...
      frag_len = RTE_MIN(seg_len, SFC_EF10_TX_DMA_DESC_LEN_MAX);
       ...
       */
      if (*added - pkt_start == *dma_desc_space && !eop &&
          !sfc_ef10_try_reap(txq, pkt_start, needed_desc, dma_desc_space,
                             reap_done)) {
        struct rte_mbuf *m;
      ...
      sfc_ef10_tx_qdesc_dma_create(frag_addr, frag_len, eop,
                                   &txq->txq_hw_ring[id]);
...
static uint16_t sfc_ef10_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                                   uint16_t nb_pkts) {
  struct sfc_ef10_txq *const txq = sfc_ef10_txq_by_dp_txq(tx_queue);
  unsigned int added;
  unsigned int dma_desc_space;
  ...
  for (pktp = &tx_pkts[0], pktp_end = &tx_pkts[nb_pkts]; pktp != pktp_end;
    ...
    if (m_seg->ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
      ...
      rc = sfc_ef10_xmit_tso_pkt(txq, m_seg, &added, &dma_desc_space,
                                 &reap_done);
      if (rc != 0) {
        added = pkt_start;
    ...
    do {
      ...
      sfc_ef10_tx_qdesc_dma_create(seg_addr, seg_len, (pkt_len == 0),
                                   &txq->txq_hw_ring[id]);
...
static void sfc_ef10_simple_tx_reap(struct sfc_ef10_txq *txq) {
  const unsigned int old_read_ptr = txq->evq_read_ptr;
  const unsigned int ptr_mask = txq->ptr_mask;
  unsigned int completed = txq->completed;
  unsigned int pending = completed;
...
#ifdef RTE_LIBRTE_SFC_EFX_DEBUG
static uint16_t sfc_ef10_simple_prepare_pkts(__rte_unused void *tx_queue,
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (unlikely(m->ol_flags & (RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_VLAN))) {
      rte_errno = ENOTSUP;
      break;
    }
...
static uint16_t sfc_ef10_simple_xmit_pkts(void *tx_queue,
                                          struct rte_mbuf **tx_pkts,
                                          uint16_t nb_pkts) {
  struct sfc_ef10_txq *const txq = sfc_ef10_txq_by_dp_txq(tx_queue);
  unsigned int ptr_mask;
  ...
  for (pktp = &tx_pkts[0]; pktp != pktp_end; ++pktp) {
    ...
    SFC_ASSERT(rte_pktmbuf_data_len(pkt) <= SFC_EF10_TX_DMA_DESC_LEN_MAX);

    sfc_ef10_tx_qdesc_dma_create(rte_mbuf_data_iova(pkt),
                                 rte_pktmbuf_data_len(pkt), true,
                                 &txq->txq_hw_ring[id]);
...
static int sfc_ef10_tx_qsize_up_rings(uint16_t nb_tx_desc,
  ...
  *txq_max_fill_level = RTE_MIN(nb_tx_desc, SFC_EF10_TXQ_LIMIT(*evq_entries));
  return 0;
}

static sfc_dp_tx_qcreate_t sfc_ef10_tx_qcreate;
static int sfc_ef10_tx_qcreate(uint16_t port_id, uint16_t queue_id,
  ...
  rc = ENOMEM;
  txq = rte_zmalloc_socket("sfc-ef10-txq", sizeof(*txq), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (txq == NULL)
    goto fail_txq_alloc;
  ...
  rc = ENOMEM;
  txq->sw_ring =
      rte_calloc_socket("sfc-ef10-txq-sw_ring", info->txq_entries,
  ...
  if (info->offloads &
    ...
    txq->tsoh =
        rte_calloc_socket("sfc-ef10-txq-tsoh", info->txq_entries,
                          SFC_TSOH_STD_LEN, RTE_CACHE_LINE_SIZE, socket_id);
    if (txq->tsoh == NULL)
...
static void sfc_ef10_tx_qdestroy(struct sfc_dp_txq *dp_txq) {
...
static int sfc_ef10_tx_qstart(struct sfc_dp_txq *dp_txq,
                              unsigned int evq_read_ptr,
...
static void sfc_ef10_tx_qstop(struct sfc_dp_txq *dp_txq,
                              unsigned int *evq_read_ptr) {
  struct sfc_ef10_txq *txq = sfc_ef10_txq_by_dp_txq(dp_txq);
...
static bool sfc_ef10_tx_qtx_ev(struct sfc_dp_txq *dp_txq,
                               __rte_unused unsigned int id) {
...
static void sfc_ef10_tx_qreap(struct sfc_dp_txq *dp_txq) {
  struct sfc_ef10_txq *txq = sfc_ef10_txq_by_dp_txq(dp_txq);
  unsigned int completed;
...
static unsigned int sfc_ef10_tx_qdesc_npending(struct sfc_ef10_txq *txq) {
  const unsigned int curr_done = txq->completed - 1;
  unsigned int anew_done = curr_done;
  efx_qword_t tx_ev;
...
static int sfc_ef10_tx_qdesc_status(struct sfc_dp_txq *dp_txq,
                                    uint16_t offset) {
  struct sfc_ef10_txq *txq = sfc_ef10_txq_by_dp_txq(dp_txq);
