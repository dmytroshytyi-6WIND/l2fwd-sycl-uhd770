...
static inline void sfc_ef10_rx_ev_to_offloads(const efx_qword_t rx_ev,
                                              struct rte_mbuf *m,
                                              uint64_t ol_mask) {
  ...
  if (tun_ptype == 0) {
    ...
    if (unlikely(EFX_TEST_QWORD_BIT(rx_ev, ESF_DZ_RX_IPCKSUM_ERR_LBN)))
  ...
  case ESE_DZ_ETH_TAG_CLASS_NONE:
    l2_ptype = (tun_ptype == 0) ? RTE_PTYPE_L2_ETHER : RTE_PTYPE_INNER_L2_ETHER;
    break;
  ...
  case ESE_DZ_L3_CLASS_IP4_FRAG:
    l4_ptype = (tun_ptype == 0) ? RTE_PTYPE_L4_FRAG : RTE_PTYPE_INNER_L4_FRAG;
    /* FALLTHROUGH */
  case ESE_DZ_L3_CLASS_IP4:
    l3_ptype = (tun_ptype == 0) ? RTE_PTYPE_L3_IPV4_EXT_UNKNOWN
                                : RTE_PTYPE_INNER_L3_IPV4_EXT_UNKNOWN;
    ol_flags |=
        RTE_MBUF_F_RX_RSS_HASH | ((EFX_TEST_QWORD_BIT(rx_ev, ip_csum_err_bit))
  ...
  case ESE_FZ_L4_CLASS_TCP:
    ...
    l4_ptype = (tun_ptype == 0) ? RTE_PTYPE_L4_TCP : RTE_PTYPE_INNER_L4_TCP;
    ol_flags |= (EFX_TEST_QWORD_BIT(rx_ev, l4_csum_err_bit))
                    ? RTE_MBUF_F_RX_L4_CKSUM_BAD
                    : RTE_MBUF_F_RX_L4_CKSUM_GOOD;
    break;
  case ESE_FZ_L4_CLASS_UDP:
    RTE_BUILD_BUG_ON(ESE_FZ_L4_CLASS_UDP != ESE_DE_L4_CLASS_UDP);
    l4_ptype = (tun_ptype == 0) ? RTE_PTYPE_L4_UDP : RTE_PTYPE_INNER_L4_UDP;
    ol_flags |= (EFX_TEST_QWORD_BIT(rx_ev, l4_csum_err_bit))
                    ? RTE_MBUF_F_RX_L4_CKSUM_BAD
                    : RTE_MBUF_F_RX_L4_CKSUM_GOOD;
    break;
  case ESE_FZ_L4_CLASS_UNKNOWN:
    RTE_BUILD_BUG_ON(ESE_FZ_L4_CLASS_UNKNOWN != ESE_DE_L4_CLASS_UNKNOWN);
    break;
  default:
