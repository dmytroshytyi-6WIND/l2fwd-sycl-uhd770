...
int sfc_flow_rss_attach(struct sfc_adapter *sa) {
  ...
  flow_rss->bounce_tbl =
      rte_calloc("sfc_flow_rss_bounce_tbl", flow_rss->nb_tbl_entries_max,
...
int sfc_flow_rss_parse_conf(struct sfc_adapter *sa,
  ...
  if (in->types != 0) {
    rc = sfc_rx_hf_rte_to_efx(sa, in->types, &out->efx_hash_types);
    if (rc != 0) {
    ...
    sfc_dbg(sa,
            "flow-rss: parse: 'types' is 0; proceeding with ethdev setting");
  ...
  case RTE_ETH_HASH_FUNCTION_TOEPLITZ:
    if (ethdev_rss->hash_alg != EFX_RX_HASHALG_TOEPLITZ) {
      sfc_err(sa,
              "flow-rss: parse: 'func' TOEPLITZ is unavailable; use DEFAULT");
      return EINVAL;
    }
    break;
  default:
  ...
  for (i = 0; i < in->queue_num; ++i) {
    ...
    if (sw_qid >= sas->ethdev_rxq_count) {
      sfc_err(sa, "flow-rss: parse: queue=%u does not exist", sw_qid);
      return EINVAL;
    }
...
int sfc_flow_rss_ctx_add(struct sfc_adapter *sa,
  ...
  if (conf->nb_qid_offsets != 0) {
    ...
    ctx->qid_offsets =
        rte_calloc("sfc_flow_rss_ctx_qid_offsets", conf->nb_qid_offsets,
...
void sfc_flow_rss_ctx_del(struct sfc_adapter *sa,
                          struct sfc_flow_rss_ctx *ctx) {
  struct sfc_flow_rss *flow_rss = &sa->flow_rss;
  ...
  if (ctx->nic_handle_refcnt != 0) {
    sfc_err(sa,
            "flow-rss: deleting ctx=%p abandons its NIC resource: "
            "handle=0x%08x, refcnt=%u",
...
static int sfc_flow_rss_ctx_program_tbl(struct sfc_adapter *sa,
  ...
  return efx_rx_scale_tbl_set(sa->nic, ctx->nic_handle, tbl, nb_tbl_entries);
}
...
int sfc_flow_rss_ctx_program(struct sfc_adapter *sa,
  ...
  if (ctx->nic_handle_refcnt == 0) {
    rc = efx_rx_scale_context_alloc_v2(sa->nic, ctx_type, conf->qid_span,
                                       nb_tbl_entries, &ctx->nic_handle);
    if (rc != 0) {
      sfc_err(sa,
              "flow-rss: failed to allocate NIC resource for ctx=%p: type=%d, "
              "qid_span=%u, nb_tbl_entries=%u; rc=%d",
              ctx, ctx_type, conf->qid_span, nb_tbl_entries, rc);
      goto fail;
    }

    sfc_dbg(sa,
            "flow-rss: allocated NIC resource for ctx=%p: type=%d, "
            "qid_span=%u, nb_tbl_entries=%u; handle=0x%08x",
            ctx, ctx_type, conf->qid_span, nb_tbl_entries, ctx->nic_handle);
  ...
  rc = efx_rx_scale_mode_set(
      sa->nic, ctx->nic_handle, ethdev_rss->hash_alg,
      (ctx->dummy) ? ethdev_rss->hash_types : conf->efx_hash_types, B_TRUE);
  if (rc != 0) {
    sfc_err(sa,
            "flow-rss: failed to configure hash for ctx=%p: efx_hash_alg=%d, "
            "efx_hash_types=0x%08x; rc=%d",
            ctx, ethdev_rss->hash_alg,
            (ctx->dummy) ? ethdev_rss->hash_types : conf->efx_hash_types, rc);
    goto fail;
  }
  ...
  rc = efx_rx_scale_key_set(sa->nic, ctx->nic_handle,
                            ...
                            RTE_DIM(conf->key));
  if (rc != 0) {
    sfc_err(sa, "flow-rss: failed to set key for ctx=%p; rc=%d", ctx, rc);
    goto fail;
  }
  ...
  if (rc != 0) {
    sfc_err(sa,
            "flow-rss: failed to program table for ctx=%p: nb_tbl_entries=%u; "
            "rc=%d",
...
void sfc_flow_rss_ctx_terminate(struct sfc_adapter *sa,
  ...
  if (ctx->nic_handle_refcnt == 0) {
    ...
    if (rc != 0) {
      sfc_err(sa,
              "flow-rss: failed to release NIC resource for ctx=%p: "
              "handle=0x%08x; rc=%d",
    ...
    sfc_dbg(sa, "flow-rss: released NIC resource for ctx=%p; rc=%d", ctx, rc);
  }
}
