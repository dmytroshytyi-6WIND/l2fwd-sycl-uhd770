...
static inline uint16_t sfc_tso_ip4_get_ipid(const uint8_t *pkt_hdrp,
                                            size_t ip_hdr_off) {
...
static inline void sfc_tso_outer_udp_fix_len(const struct rte_mbuf *m,
                                             uint8_t *tsoh) {
  rte_be16_t len =
      rte_cpu_to_be_16(m->l2_len + m->l3_len + m->l4_len + m->tso_segsz);
...
static inline void sfc_tso_innermost_ip_fix_len(const struct rte_mbuf *m,
                                                uint8_t *tsoh,
                                                size_t iph_ofst) {
