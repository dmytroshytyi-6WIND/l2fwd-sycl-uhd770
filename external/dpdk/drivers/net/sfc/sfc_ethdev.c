...
static struct sfc_dp_list sfc_dp_head = TAILQ_HEAD_INITIALIZER(sfc_dp_head);

static void sfc_eth_dev_clear_ops(struct rte_eth_dev *dev);

static int sfc_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
                              size_t fw_size) {
  ...
  if (enfi.enfi_dpcpu_fw_ids_valid) {
    ...
    ret_extra =
        snprintf(fw_version + dpcpu_fw_ids_offset,
                 fw_size - dpcpu_fw_ids_offset, " rx%" PRIx16 " tx%" PRIx16,
...
static int sfc_dev_infos_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa =
      sfc_rx_get_dev_offload_caps(sa) | dev_info->rx_queue_offload_capa;
  ...
  dev_info->tx_offload_capa =
      sfc_tx_get_dev_offload_caps(sa) | dev_info->tx_queue_offload_capa;
...
static const uint32_t *sfc_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  const struct sfc_adapter_priv *sap = sfc_adapter_priv_by_eth_dev(dev);

  return sap->dp_rx->supported_ptypes_get(sap->shared->tunnel_encaps);
}

static int sfc_dev_configure(struct rte_eth_dev *dev) {
  struct rte_eth_dev_data *dev_data = dev->data;
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  int rc;

  sfc_log_init(sa, "entry n_rxq=%u n_txq=%u", dev_data->nb_rx_queues,
               dev_data->nb_tx_queues);
  ...
  case SFC_ETHDEV_INITIALIZED:
    ...
    break;
  default:
    sfc_err(sa, "unexpected adapter state %u to configure", sa->state);
    rc = EINVAL;
    break;
  }
...
static int sfc_dev_start(struct rte_eth_dev *dev) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  int rc;
...
static int sfc_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
...
static int sfc_dev_stop(struct rte_eth_dev *dev) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
...
static int sfc_dev_set_link_up(struct rte_eth_dev *dev) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  int rc;
...
static int sfc_dev_set_link_down(struct rte_eth_dev *dev) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
...
static void sfc_eth_dev_secondary_clear_ops(struct rte_eth_dev *dev) {
  free(dev->process_private);
  rte_eth_dev_release_port(dev);
}

static int sfc_dev_close(struct rte_eth_dev *dev) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
...
static int sfc_dev_filter_set(struct rte_eth_dev *dev,
...
static int sfc_dev_promisc_enable(struct rte_eth_dev *dev) {
...
static int sfc_dev_promisc_disable(struct rte_eth_dev *dev) {
...
static int sfc_dev_allmulti_enable(struct rte_eth_dev *dev) {
...
static int sfc_dev_allmulti_disable(struct rte_eth_dev *dev) {
...
static int sfc_rx_queue_setup(struct rte_eth_dev *dev, uint16_t ethdev_qid,
  ...
  sfc_log_init(sa, "RxQ=%u nb_rx_desc=%u socket_id=%u", ethdev_qid, nb_rx_desc,
               socket_id);

  sfc_adapter_lock(sa);

  sw_index = sfc_rxq_sw_index_by_ethdev_rx_qid(sas, sfc_ethdev_qid);
  rc = sfc_rx_qinit(sa, sw_index, nb_rx_desc, socket_id, rx_conf, mb_pool);
  if (rc != 0)
    goto fail_rx_qinit;
...
static void sfc_rx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
...
static int sfc_tx_queue_setup(struct rte_eth_dev *dev, uint16_t ethdev_qid,
  ...
  sfc_log_init(sa, "TxQ = %u, nb_tx_desc = %u, socket_id = %u", ethdev_qid,
...
static void sfc_tx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
...
static void sfc_stats_get_dp_rx(struct sfc_adapter *sa, uint64_t *pkts,
                                uint64_t *bytes) {
  ...
  for (i = 0; i < sas->ethdev_rxq_count; ++i) {
    ...
    if (rxq_info->state & SFC_RXQ_INITIALIZED) {
      ...
      pkts_sum += qstats.pkts - sa->sw_stats.reset_rx_pkts[i];
      bytes_sum += qstats.bytes - sa->sw_stats.reset_rx_bytes[i];
    }
  }
...
static void sfc_stats_get_dp_tx(struct sfc_adapter *sa, uint64_t *pkts,
                                uint64_t *bytes) {
  struct sfc_adapter_shared *sas = sfc_sa2shared(sa);
  uint64_t pkts_sum = 0;
  uint64_t bytes_sum = 0;
  ...
  for (i = 0; i < sas->ethdev_txq_count; ++i) {
    ...
    if (txq_info->state & SFC_TXQ_INITIALIZED) {
      ...
      pkts_sum += qstats.pkts - sa->sw_stats.reset_tx_pkts[i];
      bytes_sum += qstats.bytes - sa->sw_stats.reset_tx_bytes[i];
    }
  }
 ...
 */
static void sfc_update_diff_stat(uint64_t *stat, uint64_t newval) {
  if ((int64_t)(newval - *stat) > 0 || newval == 0)
    *stat = newval;
}

static int sfc_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  const struct sfc_adapter_priv *sap = sfc_adapter_priv_by_eth_dev(dev);
  ...
  if (EFX_MAC_STAT_SUPPORTED(port->mac_stats_mask,
    ...
    if (!have_dp_rx_stats) {
      stats->ipackets = mac_stats[EFX_MAC_VADAPTER_RX_UNICAST_PACKETS] +
                        mac_stats[EFX_MAC_VADAPTER_RX_MULTICAST_PACKETS] +
                        mac_stats[EFX_MAC_VADAPTER_RX_BROADCAST_PACKETS];
      stats->ibytes = mac_stats[EFX_MAC_VADAPTER_RX_UNICAST_BYTES] +
    ...
    if (!have_dp_tx_stats) {
      stats->opackets = mac_stats[EFX_MAC_VADAPTER_TX_UNICAST_PACKETS] +
                        mac_stats[EFX_MAC_VADAPTER_TX_MULTICAST_PACKETS] +
                        mac_stats[EFX_MAC_VADAPTER_TX_BROADCAST_PACKETS];
      stats->obytes = mac_stats[EFX_MAC_VADAPTER_TX_UNICAST_BYTES] +
     ...
     */
    stats->imissed = mac_stats[EFX_MAC_RX_NODESC_DROP_CNT] +
    ...
    stats->ierrors = mac_stats[EFX_MAC_RX_FCS_ERRORS] +
...
static int sfc_stats_reset(struct rte_eth_dev *dev) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  struct sfc_port *port = &sa->port;
  int rc;
...
static unsigned int sfc_xstats_get_nb_supported(struct sfc_adapter *sa) {
  struct sfc_port *port = &sa->port;
  unsigned int nb_supported;

  sfc_adapter_lock(sa);
  nb_supported =
      port->mac_stats_nb_supported + sfc_sw_xstats_get_nb_supported(sa);
  sfc_adapter_unlock(sa);

  return nb_supported;
}

static int sfc_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats,
  ...
  nb_supported = rc;
  sfc_sw_xstats_get_vals(sa, xstats, xstats_count, &nb_written, &nb_supported);

  return nb_supported;
}

static int sfc_xstats_get_names(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < EFX_MAC_NSTATS; ++i) {
    if (EFX_MAC_STAT_SUPPORTED(port->mac_stats_mask, i)) {
      if (nstats < xstats_count) {
        strlcpy(xstats_names[nstats].name, efx_mac_stat_name(sa->nic, i),
  ...
  ret = sfc_sw_xstats_get_names(sa, xstats_names, xstats_count, &nb_written,
                                &nstats);
  if (ret != 0) {
    SFC_ASSERT(ret < 0);
    return ret;
...
static int sfc_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
...
static int sfc_xstats_get_names_by_id(struct rte_eth_dev *dev,
                                      const uint64_t *ids,
                                      struct rte_eth_xstat_name *xstats_names,
                                      unsigned int size) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  ...
  SFC_ASSERT(port->mac_stats_nb_supported <= RTE_DIM(port->mac_stats_by_id));

  for (i = 0; i < size; i++) {
    if (ids[i] < port->mac_stats_nb_supported) {
      strlcpy(xstats_names[i].name,
              efx_mac_stat_name(sa->nic, port->mac_stats_by_id[ids[i]]),
              sizeof(xstats_names[0].name));
    }
  }
  ...
  ret =
      sfc_sw_xstats_get_names_by_id(sa, ids, xstats_names, size, &nb_supported);
  if (ret != 0) {
    SFC_ASSERT(ret < 0);
    return ret;
  }
...
static int sfc_flow_ctrl_get(struct rte_eth_dev *dev,
  ...
  case (EFX_FCNTL_RESPOND | EFX_FCNTL_GENERATE):
    ...
    break;
  default:
    sfc_err(sa, "%s: unexpected flow control value %#x", __func__, link_fc);
  }
...
static int sfc_flow_ctrl_set(struct rte_eth_dev *dev,
                             struct rte_eth_fc_conf *fc_conf) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  struct sfc_port *port = &sa->port;
  unsigned int fcntl;
...
static int sfc_check_scatter_on_all_rx_queues(struct sfc_adapter *sa,
                                              size_t pdu) {
  ...
  for (i = 0; i < sas->rxq_count; i++) {
    ...
    scatter_enabled = (sas->rxq_info[i].type_flags & EFX_RXQ_FLAG_SCATTER);

    if (!sfc_rx_check_scatter(pdu, sa->rxq_ctrl[i].buf_size,
                              encp->enc_rx_prefix_size, scatter_enabled,
                              encp->enc_rx_scatter_max, &error)) {
      sfc_err(sa, "MTU check for RxQ %u failed: %s", i, error);
      return EINVAL;
    }
  }
...
static int sfc_dev_set_mtu(struct rte_eth_dev *dev, uint16_t mtu) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  ...
  if (pdu < EFX_MAC_PDU_MIN) {
    sfc_err(sa, "too small MTU %u (PDU size %u less than min %u)",
            (unsigned int)mtu, (unsigned int)pdu, EFX_MAC_PDU_MIN);
    goto fail_inval;
  }
...
static int sfc_mac_addr_set(struct rte_eth_dev *dev,
...
static int sfc_set_mc_addr_list(struct rte_eth_dev *dev,
  ...
  if (nb_mc_addr > port->max_mcast_addrs) {
    sfc_err(sa, "too many multicast addresses: %u > %u", nb_mc_addr,
            port->max_mcast_addrs);
    return -EINVAL;
  }

  for (i = 0; i < nb_mc_addr; ++i) {
    rte_memcpy(mc_addrs, mc_addr_set[i].addr_bytes, EFX_MAC_ADDR_LEN);
 ...
 */
static void sfc_rx_queue_info_get(struct rte_eth_dev *dev, uint16_t ethdev_qid,
 ...
 * use any process-local pointers from the adapter data.
 */
static void sfc_tx_queue_info_get(struct rte_eth_dev *dev, uint16_t ethdev_qid,
 ...
 * use any process-local pointers from the adapter data.
 */
static uint32_t sfc_rx_queue_count(void *rx_queue) {
 ...
 * use any process-local pointers from the adapter data.
 */
static int sfc_rx_descriptor_status(void *queue, uint16_t offset) {
 ...
 * use any process-local pointers from the adapter data.
 */
static int sfc_tx_descriptor_status(void *queue, uint16_t offset) {
...
static int sfc_rx_queue_start(struct rte_eth_dev *dev, uint16_t ethdev_qid) {
  struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  sfc_ethdev_qid_t sfc_ethdev_qid = ethdev_qid;
  struct sfc_rxq_info *rxq_info;
...
static int sfc_rx_queue_stop(struct rte_eth_dev *dev, uint16_t ethdev_qid) {
  struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  sfc_ethdev_qid_t sfc_ethdev_qid = ethdev_qid;
  struct sfc_rxq_info *rxq_info;
...
static int sfc_tx_queue_start(struct rte_eth_dev *dev, uint16_t ethdev_qid) {
  struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  struct sfc_txq_info *txq_info;
  sfc_sw_index_t sw_index;
...
static int sfc_tx_queue_stop(struct rte_eth_dev *dev, uint16_t ethdev_qid) {
  struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  struct sfc_txq_info *txq_info;
  sfc_sw_index_t sw_index;
...
static int sfc_dev_udp_tunnel_op(struct rte_eth_dev *dev,
  ...
  sfc_log_init(sa, "%s udp_port=%u prot_type=%u",
               ...
               : (op == SFC_UDP_TUNNEL_DEL_PORT) ? "delete"
                                                 : "unknown",
               tunnel_udp->udp_port, tunnel_udp->prot_type);

  tunnel_proto = sfc_tunnel_rte_type_to_efx_udp_proto(tunnel_udp->prot_type);
  ...
  case SFC_UDP_TUNNEL_ADD_PORT:
    rc = efx_tunnel_config_udp_add(sa->nic, tunnel_udp->udp_port, tunnel_proto);
    break;
  case SFC_UDP_TUNNEL_DEL_PORT:
    rc = efx_tunnel_config_udp_remove(sa->nic, tunnel_udp->udp_port,
                                      tunnel_proto);
    break;
  default:
    rc = EINVAL;
...
fail_reconfigure:
  ...
  switch (op) {
  case SFC_UDP_TUNNEL_ADD_PORT:
    (void)efx_tunnel_config_udp_remove(sa->nic, tunnel_udp->udp_port,
                                       tunnel_proto);
    break;
  case SFC_UDP_TUNNEL_DEL_PORT:
    (void)efx_tunnel_config_udp_add(sa->nic, tunnel_udp->udp_port,
                                    tunnel_proto);
    break;
  }
...
static int sfc_dev_udp_tunnel_port_add(struct rte_eth_dev *dev,
                                       struct rte_eth_udp_tunnel *tunnel_udp) {
  return sfc_dev_udp_tunnel_op(dev, tunnel_udp, SFC_UDP_TUNNEL_ADD_PORT);
}

static int sfc_dev_udp_tunnel_port_del(struct rte_eth_dev *dev,
                                       struct rte_eth_udp_tunnel *tunnel_udp) {
 ...
 * use any process-local pointers from the adapter data.
 */
static int sfc_dev_rss_hash_conf_get(struct rte_eth_dev *dev,
...
static int sfc_dev_rss_hash_update(struct rte_eth_dev *dev,
                                   struct rte_eth_rss_conf *rss_conf) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  ...
  if ((rss_conf->rss_key != NULL) &&
    ...
    sfc_err(sa, "RSS key size is wrong (should be %zu)", sizeof(rss->key));
    return -EINVAL;
  }
  ...
  for (mode_i = 0; mode_i < n_contexts; mode_i++) {
    rc = efx_rx_scale_mode_set(sa->nic, contexts[mode_i], rss->hash_alg,
  ...
  if (rss_conf->rss_key != NULL) {
    if (sa->state == SFC_ETHDEV_STARTED) {
      for (key_i = 0; key_i < n_contexts; key_i++) {
        rc = efx_rx_scale_key_set(sa->nic, contexts[key_i], rss_conf->rss_key,
                                  sizeof(rss->key));
        if (rc != 0)
...
fail_scale_mode_set:
  for (i = 0; i < mode_i; i++) {
    if (efx_rx_scale_mode_set(sa->nic, contexts[i], EFX_RX_HASHALG_TOEPLITZ,
 ...
 * use any process-local pointers from the adapter data.
 */
static int sfc_dev_rss_reta_query(struct rte_eth_dev *dev,
...
static int sfc_dev_rss_reta_update(struct rte_eth_dev *dev,
                                   struct rte_eth_rss_reta_entry64 *reta_conf,
                                   uint16_t reta_size) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  struct sfc_rss *rss = &sfc_sa2shared(sa)->rss;
  ...
  if (reta_size != EFX_RSS_TBL_SIZE) {
    sfc_err(sa, "RETA size is wrong (should be %u)", EFX_RSS_TBL_SIZE);
    return -EINVAL;
  }
  ...
  if (sa->state == SFC_ETHDEV_STARTED) {
    rc = efx_rx_scale_tbl_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss_tbl_new,
                              EFX_RSS_TBL_SIZE);
    if (rc != 0)
...
static int sfc_dev_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
...
static int sfc_rx_queue_intr_enable(struct rte_eth_dev *dev,
                                    uint16_t ethdev_qid) {
  const struct sfc_adapter_priv *sap = sfc_adapter_priv_by_eth_dev(dev);
  struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
  sfc_ethdev_qid_t sfc_ethdev_qid = ethdev_qid;
...
static int sfc_rx_queue_intr_disable(struct rte_eth_dev *dev,
                                     uint16_t ethdev_qid) {
  const struct sfc_adapter_priv *sap = sfc_adapter_priv_by_eth_dev(dev);
  struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
  sfc_ethdev_qid_t sfc_ethdev_qid = ethdev_qid;
...
static int sfc_journal_ctx_add_controller(struct sfc_mport_journal_ctx *ctx,
  ...
  if (ctx->controllers == NULL) {
    ctx->controllers =
        rte_malloc("sfc_controller_mapping", sizeof(ctx->controllers[0]), 0);
    if (ctx->controllers == NULL)
      return ENOMEM;
...
sfc_process_mport_journal_entry(struct sfc_mport_journal_ctx *ctx,
  ...
  sfc_dbg(ctx->sa, "processing mport id %u (controller %u pf %u vf %u)",
          mport->emd_id.id, mport->emd_vnic.ev_intf, mport->emd_vnic.ev_pf,
          mport->emd_vnic.ev_vf);
  efx_mae_mport_invalid(&ethdev_mport);

  if (!ctx->controllers_assigned) {
    rc = sfc_journal_ctx_add_controller(ctx, mport->emd_vnic.ev_intf);
    if (rc != 0)
      return rc;
  }
  ...
  efx_rc = efx_mae_mport_by_pcie_mh_function(
      mport->emd_vnic.ev_intf, mport->emd_vnic.ev_pf, mport->emd_vnic.ev_vf,
      &entity_selector);
  if (efx_rc != 0) {
    sfc_err(ctx->sa, "failed to build entity mport selector for c%upf%uvf%u",
            mport->emd_vnic.ev_intf, mport->emd_vnic.ev_pf,
            mport->emd_vnic.ev_vf);
    return efx_rc;
  }

  rc = sfc_mae_switch_port_id_by_entity(ctx->switch_domain_id, &entity_selector,
  ...
  case ENOENT:
    ...
    rc = sfc_mae_assign_switch_port(ctx->switch_domain_id, &req,
                                    &switch_port_id);
    if (rc != 0) {
      sfc_err(ctx->sa, "failed to assign MAE switch port for c%upf%uvf%u: %s",
              mport->emd_vnic.ev_intf, mport->emd_vnic.ev_pf,
              mport->emd_vnic.ev_vf, rte_strerror(rc));
      return rc;
    }
    break;
  default:
...
static efx_rc_t sfc_process_mport_journal_cb(void *data,
                                             efx_mport_desc_t *mport,
                                             size_t mport_len) {
...
static int sfc_process_mport_journal(struct sfc_adapter *sa) {
  ...
  rc = sfc_mae_switch_domain_controllers(ctx.switch_domain_id, &controllers,
                                         &nb_controllers);
  if (rc != 0) {
  ...
  efx_rc =
      efx_mae_read_mport_journal(sa->nic, sfc_process_mport_journal_cb, &ctx);
  if (efx_rc != 0) {
  ...
  if (controllers == NULL) {
    rc = sfc_mae_switch_domain_map_controllers(
        ctx.switch_domain_id, ctx.controllers, ctx.nb_controllers);
    if (rc != 0)
      return rc;
  }
...
static void sfc_count_representors_cb(
    ...
    union sfc_mae_switch_port_data *port_datap __rte_unused, void *user_datap) {
...
static void sfc_get_representors_cb(
    enum sfc_mae_switch_port_type type,
    const efx_mport_sel_t *ethdev_mportp __rte_unused,
    uint16_t ethdev_port_id __rte_unused,
    const efx_mport_sel_t *entity_mportp __rte_unused, uint16_t switch_port_id,
    union sfc_mae_switch_port_data *port_datap, void *user_datap) {
  ...
  rc = sfc_mae_switch_controller_from_mapping(
      ctx->controllers, ctx->nb_controllers, port_datap->repr.intf,
      &range->controller);
  if (rc != 0) {
  ...
  if (port_datap->repr.vf != EFX_PCI_VF_INVALID) {
    ...
    ret = snprintf(range->name, RTE_DEV_NAME_MAX_LEN, "c%dpf%dvf%d",
                   range->controller, range->pf, range->vf);
  } else {
    range->type = RTE_ETH_REPRESENTOR_PF;
    ret = snprintf(range->name, RTE_DEV_NAME_MAX_LEN, "c%dpf%d",
                   range->controller, range->pf);
  }
  if (ret >= RTE_DEV_NAME_MAX_LEN) {
    sfc_err(ctx->sa, "representor name has been truncated: %s", range->name);
  }

  ctx->info->nb_ranges++;
}

static int sfc_representor_info_get(struct rte_eth_dev *dev,
                                    struct rte_eth_representor_info *info) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  ...
  nb_repr = 0;
  rc = sfc_mae_switch_ports_iterate(switch_domain_id, sfc_count_representors_cb,
                                    &nb_repr);
  if (rc != 0) {
    sfc_adapter_unlock(sa);
    SFC_ASSERT(rc > 0);
  ...
  rc = sfc_mae_switch_domain_get_controller(switch_domain_id, nic_cfg->enc_intf,
                                            &controller);
  if (rc != 0) {
  ...
  rc = sfc_mae_switch_ports_iterate(switch_domain_id, sfc_get_representors_cb,
                                    &get_repr_ctx);
  if (rc != 0) {
    sfc_adapter_unlock(sa);
    SFC_ASSERT(rc > 0);
...
static int sfc_rx_metadata_negotiate(struct rte_eth_dev *dev,
                                     uint64_t *features) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
 ...
 */
static char *sfc_strdup(const char *str) {
  size_t size;
  char *copy;
...
static int sfc_eth_dev_set_ops(struct rte_eth_dev *dev) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
  const struct sfc_dp_rx *dp_rx;
  const struct sfc_dp_tx *dp_tx;
  ...
  rc = sfc_kvargs_process(sa, SFC_KVARG_RX_DATAPATH, sfc_kvarg_string_handler,
                          &rx_name);
  if (rc != 0)
  ...
  if (rx_name != NULL) {
    ...
    if (!sfc_dp_match_hw_fw_caps(&dp_rx->dp, avail_caps)) {
      sfc_err(sa, "Insufficient Hw/FW capabilities to use Rx datapath %s",
              rx_name);
      rc = EINVAL;
    ...
    if (dp_rx == NULL) {
      sfc_err(sa, "Rx datapath by caps %#x not found", avail_caps);
      rc = ENOENT;
      goto fail_dp_rx;
    }
  }
  ...
  rc = sfc_kvargs_process(sa, SFC_KVARG_TX_DATAPATH, sfc_kvarg_string_handler,
                          &tx_name);
  if (rc != 0)
  ...
  if (tx_name != NULL) {
    ...
    if (!sfc_dp_match_hw_fw_caps(&dp_tx->dp, avail_caps)) {
      sfc_err(sa, "Insufficient Hw/FW capabilities to use Tx datapath %s",
              tx_name);
      rc = EINVAL;
    ...
    if (dp_tx == NULL) {
      sfc_err(sa, "Tx datapath by caps %#x not found", avail_caps);
      rc = ENOENT;
      goto fail_dp_tx;
    }
  }
...
static int sfc_eth_dev_secondary_init(struct rte_eth_dev *dev,
  ...
  if (dp_rx == NULL) {
    SFC_LOG(sas, RTE_LOG_ERR, logtype_main, "cannot find %s Rx datapath",
            sas->dp_rx_name);
    rc = ENOENT;
    goto fail_dp_rx;
  }
  if (~dp_rx->features & SFC_DP_RX_FEAT_MULTI_PROCESS) {
    SFC_LOG(sas, RTE_LOG_ERR, logtype_main,
            "%s Rx datapath does not support multi-process", sas->dp_rx_name);
    rc = EINVAL;
  ...
  if (dp_tx == NULL) {
    SFC_LOG(sas, RTE_LOG_ERR, logtype_main, "cannot find %s Tx datapath",
            sas->dp_tx_name);
    rc = ENOENT;
    goto fail_dp_tx;
  }
  if (~dp_tx->features & SFC_DP_TX_FEAT_MULTI_PROCESS) {
    SFC_LOG(sas, RTE_LOG_ERR, logtype_main,
            "%s Tx datapath does not support multi-process", sas->dp_tx_name);
    rc = EINVAL;
...
static void sfc_register_dp(void) {
  /* Register once */
...
static int sfc_parse_switch_mode(struct sfc_adapter *sa,
                                 bool has_representors) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(sa->nic);
  ...
  rc = sfc_kvargs_process(sa, SFC_KVARG_SWITCH_MODE, sfc_kvarg_string_handler,
                          &switch_mode);
  if (rc != 0)
    goto fail_kvargs;
  ...
  if (switch_mode == NULL) {
    sa->switchdev = encp->enc_mae_admin &&
                    (!encp->enc_datapath_cap_evb || has_representors);
  } else if (strcasecmp(switch_mode, SFC_KVARG_SWITCH_MODE_LEGACY) == 0) {
    sa->switchdev = false;
  } else if (strcasecmp(switch_mode, SFC_KVARG_SWITCH_MODE_SWITCHDEV) == 0) {
    sa->switchdev = true;
  } else {
    sfc_err(sa, "invalid switch mode device argument '%s'", switch_mode);
    rc = EINVAL;
    goto fail_mode;
  }
...
static int sfc_eth_dev_init(struct rte_eth_dev *dev, void *init_params) {
  struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
  ...
  if (sfc_efx_dev_class_get(pci_dev->device.devargs) != SFC_EFX_DEV_CLASS_NET) {
    SFC_GENERIC_LOG(DEBUG, "Incompatible device class: skip probing, should be "
  ...
  ret = snprintf(sas->log_prefix, sizeof(sas->log_prefix),
                 ...
                 pci_dev->addr.function, dev->data->port_id);
  if (ret < 0 || ret >= (int)sizeof(sas->log_prefix)) {
    SFC_GENERIC_LOG(ERR, "reserved log prefix is too short for " PCI_PRI_FMT,
  ...
  if (sa->switchdev && sa->mae.status != SFC_MAE_STATUS_ADMIN) {
    sfc_err(sa, "failed to enable switchdev mode without admin MAE privilege");
    rc = ENOTSUP;
...
static int sfc_eth_dev_uninit(struct rte_eth_dev *dev) {
  sfc_dev_close(dev);
...
static int sfc_parse_rte_devargs(const char *args,
                                 struct rte_eth_devargs *devargs) {
  ...
  if (args != NULL) {
    ...
    if (rc != 0) {
      SFC_GENERIC_LOG(ERR, "Failed to parse generic devargs '%s'", args);
      return rc;
    }
  }
...
static int sfc_eth_dev_find_or_create(struct rte_pci_device *pci_dev,
...
static int sfc_eth_dev_create_repr(struct sfc_adapter *sa,
                                   efx_pcie_interface_t controller,
                                   uint16_t port, uint16_t repr_port,
  ...
  rc = efx_mae_mport_by_pcie_mh_function(controller, port, repr_port,
                                         &mport_sel);
  if (rc != 0) {
    sfc_err(sa,
            "failed to get m-port selector for controller %u port %u repr_port "
            "%u: %s",
  ...
  if (rc != 0) {
    sfc_err(sa,
            "failed to create representor for controller %u port %u repr_port "
            "%u: %s",
            controller, port, repr_port, rte_strerror(-rc));
    return rc;
  }
...
static int sfc_eth_dev_create_repr_port(struct sfc_adapter *sa,
  ...
  for (i = 0; i < eth_da->nb_representor_ports; i++) {
    rc = sfc_eth_dev_create_repr(sa, controller, port,
                                 eth_da->representor_ports[i], eth_da->type);
...
sfc_eth_dev_create_repr_controller(struct sfc_adapter *sa,
  ...
  if (eth_da->nb_ports == 0) {
    ...
    return sfc_eth_dev_create_repr_port(sa, eth_da, controller, default_port);
  }

  for (i = 0; i < eth_da->nb_ports; i++) {
    rc = sfc_eth_dev_create_repr_port(sa, eth_da, controller, eth_da->ports[i]);
    if (rc != 0 && first_error == 0)
      first_error = rc;
  }
...
sfc_eth_dev_create_representors(struct rte_eth_dev *dev,
  ...
  case RTE_ETH_REPRESENTOR_SF:
    ...
    return -ENOTSUP;
  default:
    sfc_err(sa, "unknown representor type: %d", eth_da->type);
    return -ENOTSUP;
  }
...
static int sfc_eth_dev_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
   ...
   */
  rc = sfc_eth_dev_find_or_create(pci_dev, &init_data, &dev, &dev_created);
  if (rc != 0)
    return rc;
...
static struct rte_pci_driver sfc_efx_pmd = {
    ...
    .drv_flags = RTE_PCI_DRV_INTR_LSC | RTE_PCI_DRV_NEED_MAPPING |
...
RTE_PMD_REGISTER_KMOD_DEP(net_sfc_efx,
                          "* igb_uio | uio_pci_generic | vfio-pci");
RTE_PMD_REGISTER_PARAM_STRING(
    net_sfc_efx, SFC_KVARG_SWITCH_MODE
    "=" SFC_KVARG_VALUES_SWITCH_MODE " " SFC_KVARG_RX_DATAPATH
    "=" SFC_KVARG_VALUES_RX_DATAPATH " " SFC_KVARG_TX_DATAPATH
    "=" SFC_KVARG_VALUES_TX_DATAPATH " " SFC_KVARG_PERF_PROFILE
    "=" SFC_KVARG_VALUES_PERF_PROFILE " " SFC_KVARG_FW_VARIANT
    "=" SFC_KVARG_VALUES_FW_VARIANT " " SFC_KVARG_RXD_WAIT_TIMEOUT_NS
    "=<long> " SFC_KVARG_STATS_UPDATE_PERIOD_MS "=<long>");
