 ...
 */
int sfc_port_update_mac_stats(struct sfc_adapter *sa, boolean_t force_upload) {
  ...
  if (!port->mac_stats_periodic_dma_supported ||
    ...
    if (port->mac_stats_update_period_ms != 0) {
      ...
      if ((timestamp - port->mac_stats_last_request_timestamp) <
  ...
  do {
    ...
    rc = efx_mac_stats_update(sa->nic, esmp, port->mac_stats_buf, genp);
    if (rc != 0)
      return rc;
...
static void sfc_port_reset_sw_stats(struct sfc_adapter *sa) {
  struct sfc_port *port = &sa->port;
...
static int sfc_port_init_dev_link(struct sfc_adapter *sa) {
  ...
  rte_atomic64_set((rte_atomic64_t *)dev_link, *(uint64_t *)&current_link);
...
static efx_link_mode_t sfc_port_phy_caps_to_max_link_speed(uint32_t phy_caps) {
...
static void sfc_port_fill_mac_stats_info(struct sfc_adapter *sa) {
...
int sfc_port_start(struct sfc_adapter *sa) {
  ...
  if (sa->eth_dev->data->dev_conf.lpbk_mode != 0) {
    ...
    link_mode = sfc_port_phy_caps_to_max_link_speed(port->phy_adv_cap);
  ...
  sfc_log_init(sa, "set flow control to %#x autoneg=%u", port->flow_ctrl,
               port->flow_ctrl_autoneg);
  rc = efx_mac_fcntl_set(sa->nic, port->flow_ctrl, port->flow_ctrl_autoneg);
  if (rc != 0)
  ...
  phy_adv_cap |= port->phy_adv_cap_mask &
                 (1u << EFX_PHY_CAP_BASER_FEC | 1u << EFX_PHY_CAP_RS_FEC |
  ...
  if (!sfc_sa2shared(sa)->isolated) {
    ...
    port->promisc = (sa->eth_dev->data->promiscuous != 0) ? B_TRUE : B_FALSE;
    port->allmulti = (sa->eth_dev->data->all_multicast != 0) ? B_TRUE : B_FALSE;
  ...
  if (port->mac_stats_update_period_ms != 0) {
    ...
    rc = efx_mac_stats_periodic(sa->nic, &port->mac_stats_dma_mem,
                                port->mac_stats_update_period_ms, B_FALSE);
    if (rc == 0) {
...
fail_mac_stats_upload:
  (void)efx_mac_stats_periodic(sa->nic, &port->mac_stats_dma_mem, 0, B_FALSE);
...
void sfc_port_stop(struct sfc_adapter *sa) {
  ...
  (void)efx_mac_stats_periodic(sa->nic, &sa->port.mac_stats_dma_mem, 0,
                               B_FALSE);
...
void sfc_port_close(struct sfc_adapter *sa) { sfc_log_init(sa, "entry"); }
...
int sfc_port_attach(struct sfc_adapter *sa) {
  ...
  port->mcast_addrs =
      rte_calloc_socket("mcast_addr_list_buf", port->max_mcast_addrs,
                        EFX_MAC_ADDR_LEN, 0, sa->socket_id);
  ...
  port->mac_stats_buf = rte_calloc_socket("mac_stats_buf", EFX_MAC_NSTATS,
                                          sizeof(uint64_t), 0, sa->socket_id);
  ...
  rc = sfc_dma_alloc(sa, "mac_stats", 0, EFX_NIC_DMA_ADDR_MAC_STATS_BUF,
                     mac_stats_size, sa->socket_id, &port->mac_stats_dma_mem);
  if (rc != 0)
  ...
  rc =
      sfc_kvargs_process(sa, SFC_KVARG_STATS_UPDATE_PERIOD_MS,
                         sfc_kvarg_long_handler, &kvarg_stats_update_period_ms);
  if ((rc == 0) && ((kvarg_stats_update_period_ms < 0) ||
                    (kvarg_stats_update_period_ms > UINT16_MAX))) {
    sfc_err(sa,
            "wrong '" SFC_KVARG_STATS_UPDATE_PERIOD_MS "' "
            "was set (%ld);",
            kvarg_stats_update_period_ms);
    sfc_err(sa,
            "it must not be less than 0 "
            "or greater than %" PRIu16,
            UINT16_MAX);
    rc = EINVAL;
...
static boolean_t sfc_get_requested_all_ucast(struct sfc_port *port) {
  return port->promisc;
}

static boolean_t sfc_get_requested_all_mcast(struct sfc_port *port) {
...
int sfc_set_rx_mode(struct sfc_adapter *sa) {
  ...
  efx_mac_filter_get_all_ucast_mcast(sa->nic, &old_all_ucast, &old_all_mcast);
  ...
  if (actual_all_ucast != requested_all_ucast ||
     ...
     */
    (void)efx_mac_filter_set(sa->nic, old_all_ucast, B_TRUE, old_all_mcast,
                             B_TRUE);
