...
#define sfcr_info(sr, ...)                                                     \
  do {                                                                         \
    const struct sfc_repr *_sr = (sr);                                         \
                                                                               \
    (void)_sr;                                                                 \
    SFC_GENERIC_LOG(INFO, RTE_FMT("%s() " RTE_FMT_HEAD(__VA_ARGS__, ),         \
...
static inline void sfc_repr_lock_init(struct sfc_repr *sr) {
  rte_spinlock_init(&sr->lock);
}

#if defined(RTE_LIBRTE_SFC_EFX_DEBUG) || defined(RTE_ENABLE_ASSERT)

static inline int sfc_repr_lock_is_locked(struct sfc_repr *sr) {
  return rte_spinlock_is_locked(&sr->lock);
}

#endif

static inline void sfc_repr_lock(struct sfc_repr *sr) {
  rte_spinlock_lock(&sr->lock);
}

static inline void sfc_repr_unlock(struct sfc_repr *sr) {
  rte_spinlock_unlock(&sr->lock);
}

static inline void sfc_repr_lock_fini(__rte_unused struct sfc_repr *sr) {
  /* Just for symmetry of the API */
}

static void sfc_repr_rx_queue_stop(void *queue) {
...
static void sfc_repr_tx_queue_stop(void *queue) {
...
static uint16_t sfc_repr_rx_burst(void *rx_queue, struct rte_mbuf **rx_pkts,
                                  uint16_t nb_pkts) {
...
static uint16_t sfc_repr_tx_burst(void *tx_queue, struct rte_mbuf **tx_pkts,
                                  uint16_t nb_pkts) {
...
static int sfc_repr_start(struct rte_eth_dev *dev) {
...
static int sfc_repr_dev_start(struct rte_eth_dev *dev) {
  struct sfc_repr *sr = sfc_repr_by_eth_dev(dev);
  int ret;
...
static int sfc_repr_stop(struct rte_eth_dev *dev) {
  struct sfc_repr *sr = sfc_repr_by_eth_dev(dev);
  struct sfc_repr_shared *srs;
  unsigned int i;
  int ret;
...
static int sfc_repr_dev_stop(struct rte_eth_dev *dev) {
  struct sfc_repr *sr = sfc_repr_by_eth_dev(dev);
  int ret;
...
static int sfc_repr_check_conf(struct sfc_repr *sr, uint16_t nb_rx_queues,
  ...
  case RTE_ETH_MQ_RX_RSS:
    if (nb_rx_queues != 1) {
      sfcr_err(sr, "Rx RSS is not supported with %u queues", nb_rx_queues);
      ret = -EINVAL;
      break;
    }
...
static int sfc_repr_configure(struct sfc_repr *sr, uint16_t nb_rx_queues,
                              const struct rte_eth_conf *conf) {
  int ret;
...
static int sfc_repr_dev_configure(struct rte_eth_dev *dev) {
  struct sfc_repr *sr = sfc_repr_by_eth_dev(dev);
  struct rte_eth_dev_data *dev_data = dev->data;
  int ret;

  sfcr_info(sr, "entry n_rxq=%u n_txq=%u", dev_data->nb_rx_queues,
  ...
  case SFC_ETHDEV_INITIALIZED:
    ret = sfc_repr_configure(sr, dev_data->nb_rx_queues, &dev_data->dev_conf);
    break;
  default:
    sfcr_err(sr, "unexpected adapter state %u to configure", sr->state);
    ret = -EINVAL;
    break;
  }
  sfc_repr_unlock(sr);
...
static int sfc_repr_dev_infos_get(struct rte_eth_dev *dev,
...
static int sfc_repr_dev_link_update(struct rte_eth_dev *dev,
...
static int sfc_repr_ring_create(uint16_t pf_port_id, uint16_t repr_id,
...
static int sfc_repr_rx_qcheck_conf(struct sfc_repr *sr,
  ...
  if (rx_conf->rx_thresh.pthresh != 0 || rx_conf->rx_thresh.hthresh != 0 ||
    ...
    sfcr_warn(sr, "RxQ prefetch/host/writeback thresholds are not supported");
  }
...
sfc_repr_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
  ...
  ret = -ENOMEM;
  rxq = rte_zmalloc_socket("sfc-repr-rxq", sizeof(*rxq), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (rxq == NULL) {
  ...
  ret = sfc_repr_ring_create(srs->pf_port_id, srs->repr_id, "rx", rx_queue_id,
                             nb_rx_desc, socket_id, &rxq->ring);
  if (ret != 0) {
    sfcr_err(sr, "%s() failed to create ring", __func__);
    goto fail_ring_create;
  }

  ret = sfc_repr_proxy_add_rxq(srs->pf_port_id, srs->repr_id, rx_queue_id,
                               rxq->ring, mb_pool);
  if (ret != 0) {
    SFC_ASSERT(ret > 0);
    ret = -ret;
...
static void sfc_repr_rx_queue_release(struct rte_eth_dev *dev,
...
static int sfc_repr_tx_qcheck_conf(struct sfc_repr *sr,
  ...
  if (tx_conf->tx_thresh.pthresh != 0 || tx_conf->tx_thresh.hthresh != 0 ||
    ...
    sfcr_warn(sr, "prefetch/host/writeback thresholds are not supported");
  }
...
static int sfc_repr_tx_queue_setup(struct rte_eth_dev *dev,
  ...
  ret = -ENOMEM;
  txq = rte_zmalloc_socket("sfc-repr-txq", sizeof(*txq), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (txq == NULL)
    goto fail_txq_alloc;

  ret = sfc_repr_ring_create(srs->pf_port_id, srs->repr_id, "tx", tx_queue_id,
                             nb_tx_desc, socket_id, &txq->ring);
  if (ret != 0)
    goto fail_ring_create;

  ret = sfc_repr_proxy_add_txq(srs->pf_port_id, srs->repr_id, tx_queue_id,
                               txq->ring, &txq->egress_mport);
  if (ret != 0)
...
static void sfc_repr_tx_queue_release(struct rte_eth_dev *dev,
...
static void sfc_repr_close(struct sfc_repr *sr) {
  SFC_ASSERT(sfc_repr_lock_is_locked(sr));
...
static int sfc_repr_dev_close(struct rte_eth_dev *dev) {
  struct sfc_repr *sr = sfc_repr_by_eth_dev(dev);
  struct sfc_repr_shared *srs = sfc_repr_shared_by_eth_dev(dev);
  unsigned int i;
...
static int sfc_repr_stats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    sfc_pkts_bytes_get(&rxq->stats.packets_bytes, &queue_stats);
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    sfc_pkts_bytes_get(&txq->stats.packets_bytes, &queue_stats);
...
static int sfc_repr_eth_dev_init(struct rte_eth_dev *dev, void *init_params) {
  ...
  ret = sfc_repr_assign_mae_switch_port(
      repr_data->switch_domain_id, &switch_port_request, &srs->switch_port_id);
  if (ret != 0) {
    SFC_GENERIC_LOG(ERR, "%s() failed to assign MAE switch port (domain id %u)",
                    __func__, repr_data->switch_domain_id);
    goto fail_mae_assign_switch_port;
  }

  ret = sfc_repr_proxy_add_port(repr_data->pf_port_id, srs->switch_port_id,
                                dev->data->port_id, &repr_data->mport_sel);
  if (ret != 0) {
    SFC_GENERIC_LOG(ERR, "%s() failed to add repr proxy port", __func__);
    SFC_ASSERT(ret > 0);
    ret = -ret;
...
fail_alloc_sr:
  (void)sfc_repr_proxy_del_port(repr_data->pf_port_id, srs->switch_port_id);
...
int sfc_repr_create(struct rte_eth_dev *parent,
  ...
  controller = -1;
  rc = sfc_mae_switch_domain_get_controller(switch_domain_id, entity->intf,
                                            &controller);
  if (rc != 0) {
    SFC_GENERIC_LOG(ERR, "%s() failed to get DPDK controller for %d", __func__,
                    entity->intf);
    return -rc;
  }
  ...
  case RTE_ETH_REPRESENTOR_VF:
    ret = snprintf(name, sizeof(name), "net_%s_representor_c%upf%uvf%u",
                   parent->device->name, controller, entity->pf, entity->vf);
    break;
  ...
  if (dev == NULL) {
    ...
    ret =
        rte_eth_dev_create(parent->device, name, sizeof(struct sfc_repr_shared),
                           NULL, NULL, sfc_repr_eth_dev_init, &repr_data);
    if (ret != 0) {
