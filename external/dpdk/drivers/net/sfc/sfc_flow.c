...
static boolean_t sfc_flow_is_zero(const uint8_t *buf, unsigned int size) {
  uint8_t sum = 0;
  unsigned int i;
 ...
 */
int sfc_flow_parse_init(const struct rte_flow_item *item, const void **spec_ptr,
                        const void **mask_ptr, const void *supp_mask,
                        const void *def_mask, unsigned int size,
                        struct rte_flow_error *error) {
  const uint8_t *spec;
  const uint8_t *mask;
  const uint8_t *last;
   ...
   */
  if (last != NULL && !sfc_flow_is_zero(last, size) &&
 ...
 */
static int sfc_flow_parse_eth(const struct rte_flow_item *item,
  ...
  const uint8_t ig_mask[EFX_MAC_ADDR_LEN] = {0x01, 0x00, 0x00,
                                             0x00, 0x00, 0x00};
  const struct rte_flow_item_eth *supp_mask_p;
  const struct rte_flow_item_eth *def_mask_p;
  uint8_t *loc_mac = NULL;
  boolean_t is_ifrm = (efx_spec->efs_encap_type != EFX_TUNNEL_PROTOCOL_NONE);
  ...
  if (rte_is_same_ether_addr(&mask->dst, &supp_mask.dst)) {
    efx_spec->efs_match_flags |=
        is_ifrm ? EFX_FILTER_MATCH_IFRM_LOC_MAC : EFX_FILTER_MATCH_LOC_MAC;
    rte_memcpy(loc_mac, spec->dst.addr_bytes, EFX_MAC_ADDR_LEN);
  } else if (memcmp(mask->dst.addr_bytes, ig_mask, EFX_MAC_ADDR_LEN) == 0) {
  ...
  if (rte_is_same_ether_addr(&mask->src, &supp_mask.src)) {
    ...
    rte_memcpy(efx_spec->efs_rem_mac, spec->src.addr_bytes, EFX_MAC_ADDR_LEN);
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_vlan(const struct rte_flow_item *item,
                               struct sfc_flow_parse_ctx *parse_ctx,
                               struct rte_flow_error *error) {
  int rc;
  uint16_t vid;
  ...
  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           &supp_mask, NULL, sizeof(struct rte_flow_item_vlan),
                           error);
  if (rc != 0)
    return rc;
  ...
  if (mask->tci == supp_mask.tci) {
    ...
    if (!(efx_spec->efs_match_flags & EFX_FILTER_MATCH_OUTER_VID)) {
      efx_spec->efs_match_flags |= EFX_FILTER_MATCH_OUTER_VID;
      efx_spec->efs_outer_vid = vid;
    } else if (!(efx_spec->efs_match_flags & EFX_FILTER_MATCH_INNER_VID)) {
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_ipv4(const struct rte_flow_item *item,
                               struct sfc_flow_parse_ctx *parse_ctx,
                               struct rte_flow_error *error) {
  int rc;
  efx_filter_spec_t *efx_spec = parse_ctx->filter;
  ...
  const struct rte_flow_item_ipv4 supp_mask = {.hdr = {
                                                   .src_addr = 0xffffffff,
                                                   .dst_addr = 0xffffffff,
                                                   .next_proto_id = 0xff,
                                               }};
  ...
  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           &supp_mask, &rte_flow_item_ipv4_mask,
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_ipv6(const struct rte_flow_item *item,
                               struct sfc_flow_parse_ctx *parse_ctx,
                               struct rte_flow_error *error) {
  int rc;
  efx_filter_spec_t *efx_spec = parse_ctx->filter;
  ...
  const struct rte_flow_item_ipv6 supp_mask = {
      .hdr = {
          .src_addr = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
          .dst_addr = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
          .proto = 0xff,
      }};

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           &supp_mask, &rte_flow_item_ipv6_mask,
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_tcp(const struct rte_flow_item *item,
                              struct sfc_flow_parse_ctx *parse_ctx,
                              struct rte_flow_error *error) {
  int rc;
  efx_filter_spec_t *efx_spec = parse_ctx->filter;
  ...
  const struct rte_flow_item_tcp supp_mask = {.hdr = {
                                                  .src_port = 0xffff,
                                                  .dst_port = 0xffff,
                                              }};

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           &supp_mask, &rte_flow_item_tcp_mask,
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_udp(const struct rte_flow_item *item,
                              struct sfc_flow_parse_ctx *parse_ctx,
                              struct rte_flow_error *error) {
  int rc;
  efx_filter_spec_t *efx_spec = parse_ctx->filter;
  ...
  const struct rte_flow_item_udp supp_mask = {.hdr = {
                                                  .src_port = 0xffff,
                                                  .dst_port = 0xffff,
                                              }};

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           &supp_mask, &rte_flow_item_udp_mask,
 ...
 */
static int sfc_flow_set_match_flags_for_encap_pkts(
    const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
    uint8_t ip_proto, struct rte_flow_error *error) {
  if (!(efx_spec->efs_match_flags & EFX_FILTER_MATCH_IP_PROTO)) {
    efx_spec->efs_match_flags |= EFX_FILTER_MATCH_IP_PROTO;
...
static int sfc_flow_set_efx_spec_vni_or_vsid(efx_filter_spec_t *efx_spec,
                                             const uint8_t *vni_or_vsid_val,
                                             const uint8_t *vni_or_vsid_mask,
                                             const struct rte_flow_item *item,
                                             struct rte_flow_error *error) {
  const uint8_t vni_or_vsid_full_mask[EFX_VNI_OR_VSID_LEN] = {0xff, 0xff, 0xff};

  if (memcmp(vni_or_vsid_mask, vni_or_vsid_full_mask, EFX_VNI_OR_VSID_LEN) ==
      0) {
    efx_spec->efs_match_flags |= EFX_FILTER_MATCH_VNI_OR_VSID;
    rte_memcpy(efx_spec->efs_vni_or_vsid, vni_or_vsid_val, EFX_VNI_OR_VSID_LEN);
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_vxlan(const struct rte_flow_item *item,
                                struct sfc_flow_parse_ctx *parse_ctx,
                                struct rte_flow_error *error) {
  int rc;
  efx_filter_spec_t *efx_spec = parse_ctx->filter;
  ...
  const struct rte_flow_item_vxlan supp_mask = {.vni = {0xff, 0xff, 0xff}};

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           &supp_mask, &rte_flow_item_vxlan_mask,
                           sizeof(struct rte_flow_item_vxlan), error);
  if (rc != 0)
    return rc;

  rc = sfc_flow_set_match_flags_for_encap_pkts(item, efx_spec, EFX_IPPROTO_UDP,
                                               error);
  if (rc != 0)
    return rc;
  ...
  rc = sfc_flow_set_efx_spec_vni_or_vsid(efx_spec, spec->vni, mask->vni, item,
                                         error);
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_geneve(const struct rte_flow_item *item,
                                 struct sfc_flow_parse_ctx *parse_ctx,
                                 struct rte_flow_error *error) {
  int rc;
  efx_filter_spec_t *efx_spec = parse_ctx->filter;
  ...
  const struct rte_flow_item_geneve supp_mask = {.protocol = RTE_BE16(0xffff),
                                                 .vni = {0xff, 0xff, 0xff}};

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           &supp_mask, &rte_flow_item_geneve_mask,
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_nvgre(const struct rte_flow_item *item,
                                struct sfc_flow_parse_ctx *parse_ctx,
                                struct rte_flow_error *error) {
  int rc;
  efx_filter_spec_t *efx_spec = parse_ctx->filter;
  ...
  const struct rte_flow_item_nvgre supp_mask = {.tni = {0xff, 0xff, 0xff}};

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           &supp_mask, &rte_flow_item_nvgre_mask,
                           sizeof(struct rte_flow_item_nvgre), error);
  if (rc != 0)
    return rc;

  rc = sfc_flow_set_match_flags_for_encap_pkts(item, efx_spec, EFX_IPPROTO_GRE,
                                               error);
  if (rc != 0)
    return rc;
  ...
  rc = sfc_flow_set_efx_spec_vni_or_vsid(efx_spec, spec->tni, mask->tni, item,
                                         error);
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_pppoex(const struct rte_flow_item *item,
                                 struct sfc_flow_parse_ctx *parse_ctx,
                                 struct rte_flow_error *error) {
  efx_filter_spec_t *efx_spec = parse_ctx->filter;
  ...
  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           &supp_mask, &def_mask,
                           sizeof(struct rte_flow_item_pppoe), error);
  if (rc != 0)
    return rc;
 ...
 */
static int sfc_flow_parse_attr(struct sfc_adapter *sa,
  ...
  if (attr->group != 0) {
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_GROUP, attr,
                       "Groups are not supported");
    return -rte_errno;
  }
  if (attr->egress != 0 && attr->transfer == 0) {
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS, attr,
                       "Egress is not supported");
    return -rte_errno;
  }
  if (attr->ingress == 0 && attr->transfer == 0) {
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS, attr,
                       "Ingress is compulsory");
    return -rte_errno;
  }
  if (attr->transfer == 0) {
    if (attr->priority != 0) {
      rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY,
...
sfc_flow_get_item(const struct sfc_flow_item *items, unsigned int nb_items,
...
int sfc_flow_parse_pattern(struct sfc_adapter *sa,
  ...
  for (; pattern->type != RTE_FLOW_ITEM_TYPE_END; pattern++) {
    item = sfc_flow_get_item(flow_items, nb_flow_items, pattern->type);
    if (item == NULL) {
      rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, pattern,
    ...
    default:
      if (parse_ctx->type == SFC_FLOW_PARSE_CTX_FILTER && is_ifrm) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, pattern,
    ...
    if (rc != 0) {
      sfc_err(sa, "failed to parse item %s: %s", item->name, strerror(-rc));
      return rc;
    }
...
static int sfc_flow_parse_queue(struct sfc_adapter *sa,
...
static int sfc_flow_parse_rss(struct sfc_adapter *sa,
  ...
  spec_filter->rss_ctx =
      sfc_flow_rss_ctx_reuse(sa, &conf, sw_qid_min, action_rss->queue);
...
static int sfc_flow_spec_flush(struct sfc_adapter *sa,
  ...
  for (i = 0; i < filters_count; i++) {
    ...
    if (ret == 0 && rc != 0) {
      sfc_err(sa,
              "failed to remove filter specification "
              "(rc = %d)",
              rc);
      ret = rc;
    }
...
static int sfc_flow_spec_insert(struct sfc_adapter *sa,
                                struct sfc_flow_spec *spec) {
  struct sfc_flow_spec_filter *spec_filter = &spec->filter;
  unsigned int i;
  int rc = 0;
...
static int sfc_flow_spec_remove(struct sfc_adapter *sa,
                                struct sfc_flow_spec *spec) {
  struct sfc_flow_spec_filter *spec_filter = &spec->filter;
...
static int sfc_flow_filter_insert(struct sfc_adapter *sa,
                                  struct rte_flow *flow) {
  struct sfc_flow_spec_filter *spec_filter = &flow->spec.filter;
...
static int sfc_flow_filter_remove(struct sfc_adapter *sa,
                                  struct rte_flow *flow) {
  struct sfc_flow_spec_filter *spec_filter = &flow->spec.filter;
  int rc = 0;
...
static int sfc_flow_parse_mark(struct sfc_adapter *sa,
...
static int sfc_flow_parse_actions(struct sfc_adapter *sa,
  ...
  const uint32_t mark_actions_mask =
      (1UL << RTE_FLOW_ACTION_TYPE_MARK) | (1UL << RTE_FLOW_ACTION_TYPE_FLAG);
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_VOID:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_VOID, actions_set);
      break;

    case RTE_FLOW_ACTION_TYPE_QUEUE:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_QUEUE, actions_set);
    ...
    case RTE_FLOW_ACTION_TYPE_RSS:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_RSS, actions_set);
      if ((actions_set & fate_actions_mask) != 0)
        goto fail_fate_actions;

      rc = sfc_flow_parse_rss(sa, actions->conf, flow);
      if (rc != 0) {
        rte_flow_error_set(error, -rc, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                           "Bad RSS action");
        return -rte_errno;
      }
      break;
    ...
    case RTE_FLOW_ACTION_TYPE_DROP:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_DROP, actions_set);
      if ((actions_set & fate_actions_mask) != 0)
        goto fail_fate_actions;

      spec_filter->template.efs_dmaq_id = EFX_FILTER_SPEC_RX_DMAQ_ID_DROP;
      break;

    case RTE_FLOW_ACTION_TYPE_FLAG:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_FLAG, actions_set);
      ...
      if ((dp_rx_features & SFC_DP_RX_FEAT_FLOW_FLAG) == 0) {
        ...
        return -rte_errno;
      } else if ((rx_metadata & RTE_ETH_RX_METADATA_USER_FLAG) == 0) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                           "flag delivery has not been negotiated");
        return -rte_errno;
      }

      spec_filter->template.efs_flags |= EFX_FILTER_FLAG_ACTION_FLAG;
      break;

    case RTE_FLOW_ACTION_TYPE_MARK:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_MARK, actions_set);
      if ((actions_set & mark_actions_mask) != 0)
        goto fail_actions_overlap;
      ...
      if ((dp_rx_features & SFC_DP_RX_FEAT_FLOW_MARK) == 0) {
        ...
        return -rte_errno;
      } else if ((rx_metadata & RTE_ETH_RX_METADATA_USER_MARK) == 0) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
...
sfc_flow_set_unknown_dst_flags(struct sfc_flow_spec *spec,
  ...
  static const efx_filter_match_flags_t vals[] = {
      EFX_FILTER_MATCH_UNKNOWN_UCAST_DST, EFX_FILTER_MATCH_UNKNOWN_MCAST_DST};
...
sfc_flow_check_unknown_dst_flags(efx_filter_match_flags_t match,
  ...
  match_mcast_dst = (match & ~EFX_FILTER_MATCH_UNKNOWN_UCAST_DST) |
                    EFX_FILTER_MATCH_UNKNOWN_MCAST_DST;
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_set_ethertypes(struct sfc_flow_spec *spec,
                                   unsigned int filters_count_for_one_val,
                                   struct rte_flow_error *error) {
  unsigned int i;
  struct sfc_flow_spec_filter *spec_filter = &spec->filter;
  static const uint16_t vals[] = {EFX_ETHER_TYPE_IPV4, EFX_ETHER_TYPE_IPV6};
  ...
  for (i = 0; i < spec_filter->count; i++) {
    spec_filter->filters[i].efs_match_flags |= EFX_FILTER_MATCH_ETHER_TYPE;
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_set_outer_vid_flag(struct sfc_flow_spec *spec,
                                       unsigned int filters_count_for_one_val,
                                       struct rte_flow_error *error) {
  struct sfc_flow_spec_filter *spec_filter = &spec->filter;
  unsigned int i;
  ...
  for (i = 0; i < spec_filter->count; i++) {
    spec_filter->filters[i].efs_match_flags |= EFX_FILTER_MATCH_OUTER_VID;
...
sfc_flow_check_ifrm_unknown_dst_flags(efx_filter_match_flags_t match,
  ...
  match_mcast_dst = (match & ~EFX_FILTER_MATCH_IFRM_UNKNOWN_UCAST_DST) |
                    EFX_FILTER_MATCH_IFRM_UNKNOWN_MCAST_DST;
  for (i = 0; i < filter->supported_match_num; i++) {
    if (match_mcast_dst == filter->supported_match[i])
      return B_TRUE;
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_spec_add_match_flag(struct sfc_flow_spec *spec,
                                        efx_filter_match_flags_t flag,
                                        struct rte_flow_error *error) {
  unsigned int i;
  ...
  for (i = spec_filter->count; i < new_filters_count; i++) {
    spec_filter->filters[i] = spec_filter->filters[i - spec_filter->count];
  }
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_spec_filters_complete(struct sfc_adapter *sa,
                                          struct sfc_flow_spec *spec,
                                          struct rte_flow_error *error) {
  struct sfc_flow_spec_filter *spec_filter = &spec->filter;
 ...
 */
static boolean_t sfc_flow_is_match_flags_exception(struct sfc_filter *filter,
                                                   struct sfc_flow_spec *spec) {
  unsigned int i;
  uint16_t ether_type;
  uint8_t ip_proto;
  ...
  for (i = 0; i < spec_filter->count; i++) {
    ...
    if (sfc_flow_is_match_with_vids(match_flags, EFX_FILTER_MATCH_ETHER_TYPE) ||
      ...
      if (filter->supports_rem_or_local_port_filter &&
          (ip_proto == EFX_IPPROTO_TCP || ip_proto == EFX_IPPROTO_UDP))
        return B_TRUE;
    }
  }
...
static int sfc_flow_validate_match_flags(struct sfc_adapter *sa,
                                         struct rte_flow *flow,
                                         struct rte_flow_error *error) {
  struct sfc_flow_spec *spec = &flow->spec;
  struct sfc_flow_spec_filter *spec_filter = &spec->filter;
...
static int sfc_flow_parse_rte_to_filter(struct rte_eth_dev *dev,
                                        const struct rte_flow_item pattern[],
                                        const struct rte_flow_action actions[],
                                        struct rte_flow *flow,
                                        struct rte_flow_error *error) {
...
static int sfc_flow_parse_rte_to_mae(struct rte_eth_dev *dev,
                                     const struct rte_flow_item pattern[],
                                     const struct rte_flow_action actions[],
                                     struct rte_flow *flow,
                                     struct rte_flow_error *error) {
...
static int sfc_flow_parse(struct rte_eth_dev *dev,
                          const struct rte_flow_attr *attr,
                          const struct rte_flow_item pattern[],
                          const struct rte_flow_action actions[],
...
static struct rte_flow *sfc_flow_zmalloc(struct rte_flow_error *error) {
  struct rte_flow *flow;
...
static void sfc_flow_free(struct sfc_adapter *sa, struct rte_flow *flow) {
  const struct sfc_flow_ops_by_spec *ops;
...
static int sfc_flow_insert(struct sfc_adapter *sa, struct rte_flow *flow,
                           struct rte_flow_error *error) {
  const struct sfc_flow_ops_by_spec *ops;
  int rc;
...
static int sfc_flow_remove(struct sfc_adapter *sa, struct rte_flow *flow,
                           struct rte_flow_error *error) {
  const struct sfc_flow_ops_by_spec *ops;
  int rc;
  ...
  if (rc != 0) {
    rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                       "Failed to remove the flow rule");
  }

  return rc;
}

static int sfc_flow_verify(struct sfc_adapter *sa, struct rte_flow *flow,
                           struct rte_flow_error *error) {
  const struct sfc_flow_ops_by_spec *ops;
  int rc = 0;
...
static int sfc_flow_validate(struct rte_eth_dev *dev,
                             const struct rte_flow_attr *attr,
                             const struct rte_flow_item pattern[],
                             const struct rte_flow_action actions[],
                             struct rte_flow_error *error) {
...
static struct rte_flow *sfc_flow_create(struct rte_eth_dev *dev,
                                        const struct rte_flow_attr *attr,
                                        const struct rte_flow_item pattern[],
                                        const struct rte_flow_action actions[],
                                        struct rte_flow_error *error) {
...
static int sfc_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                            struct rte_flow_error *error) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  ...
  if (rc != 0) {
    rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
...
static int sfc_flow_flush(struct rte_eth_dev *dev,
                          struct rte_flow_error *error) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  struct rte_flow *flow;
  int ret = 0;
...
static int sfc_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  if (ops == NULL || ops->query == NULL) {
    ret = rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                             NULL, "No backend to handle this flow");
...
static int sfc_flow_isolate(struct rte_eth_dev *dev, int enable,
                            struct rte_flow_error *error) {
  struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
  int ret = 0;
...
static int sfc_flow_pick_transfer_proxy(struct rte_eth_dev *dev,
...
static void sfc_flow_cleanup(struct sfc_adapter *sa, struct rte_flow *flow) {
  if (flow == NULL)
    return;
