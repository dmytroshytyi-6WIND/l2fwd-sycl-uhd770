...
sfc_ef10_essb_maybe_next_completed(struct sfc_ef10_essb_rxq *rxq) {
  ...
  if (rxq->left_in_completed != 0) {
    ...
    return sfc_ef10_essb_mbuf_by_index(
        rxq, rxd->first_mbuf, rxq->block_size - rxq->left_in_completed);
  } else {
    rxq->completed++;
    rxd = &rxq->sw_ring[rxq->completed & rxq->rxq_ptr_mask];
...
static void sfc_ef10_essb_rx_qrefill(struct sfc_ef10_essb_rxq *rxq) {
  ...
  do {
    ...
    if (unlikely(rte_mempool_get_contig_blocks(rxq->refill_mb_pool, mbuf_blocks,
                                               RTE_DIM(mbuf_blocks)) < 0)) {
      struct rte_eth_dev_data *dev_data = rte_eth_devices[rxq->port_id].data;
    ...
    for (i = 0, id = added & rxq_ptr_mask; i < RTE_DIM(mbuf_blocks);
         ++i, ++id) {
      ...
      EFX_POPULATE_QWORD_2(rxq->rxq_hw_ring[id], ESF_DZ_RX_KER_BYTE_CNT,
  ...
  rxq->added = added;
  sfc_ef10_rx_qpush(rxq->doorbell, added, rxq_ptr_mask, &rxq->dp.dpq.dbells);
}

static bool sfc_ef10_essb_rx_event_get(struct sfc_ef10_essb_rxq *rxq,
                                       efx_qword_t *rx_ev) {
  *rx_ev = rxq->evq_hw_ring[rxq->evq_read_ptr & rxq->evq_ptr_mask];
  ...
  if (unlikely(EFX_QWORD_FIELD(*rx_ev, FSF_AZ_EV_CODE) !=
    ...
    sfc_ef10_essb_rx_err(&rxq->dp.dpq, "RxQ exception at EvQ read ptr %#x",
                         rxq->evq_read_ptr);
    return false;
  }
...
static void sfc_ef10_essb_rx_process_ev(struct sfc_ef10_essb_rxq *rxq,
                                        efx_qword_t rx_ev) {
  unsigned int ready;

  ready = (EFX_QWORD_FIELD(rx_ev, ESF_DZ_RX_DSC_PTR_LBITS) - rxq->bufs_ptr) &
...
static unsigned int sfc_ef10_essb_rx_get_pending(struct sfc_ef10_essb_rxq *rxq,
  ...
  while ((todo_bufs = RTE_MIN(nb_pkts - n_rx_pkts, rxq->bufs_pending)) > 0) {
    ...
    do {
      ...
      /* Parse pseudo-header */
      qwordp =
          (const efx_qword_t *)((uint8_t *)m->buf_addr + RTE_PKTMBUF_HEADROOM);
      pkt_len = EFX_QWORD_FIELD(*qwordp, ES_EZ_ESSB_RX_PREFIX_DATA_LEN);
      ...
      m->ol_flags |=
          (RTE_MBUF_F_RX_RSS_HASH *
           !!EFX_TEST_QWORD_BIT(*qwordp, ES_EZ_ESSB_RX_PREFIX_HASH_VALID_LBN)) |
          (RTE_MBUF_F_RX_FDIR_ID *
           !!EFX_TEST_QWORD_BIT(*qwordp, ES_EZ_ESSB_RX_PREFIX_MARK_VALID_LBN)) |
          (RTE_MBUF_F_RX_FDIR *
           !!EFX_TEST_QWORD_BIT(*qwordp, ES_EZ_ESSB_RX_PREFIX_MATCH_FLAG_LBN));

      /* EFX_QWORD_FIELD converts little-endian to CPU */
      m->hash.rss = EFX_QWORD_FIELD(*qwordp, ES_EZ_ESSB_RX_PREFIX_HASH);
      m->hash.fdir.hi = EFX_QWORD_FIELD(*qwordp, ES_EZ_ESSB_RX_PREFIX_MARK);
...
static uint16_t sfc_ef10_essb_recv_pkts(void *rx_queue,
                                        struct rte_mbuf **rx_pkts,
                                        uint16_t nb_pkts) {
  ...
  while (n_rx_pkts != nb_pkts && sfc_ef10_essb_rx_event_get(rxq, &rx_ev)) {
    /*
    ...
    n_rx_pkts += sfc_ef10_essb_rx_get_pending(rxq, rx_pkts + n_rx_pkts,
                                              nb_pkts - n_rx_pkts);
  }

  sfc_ef10_ev_qclear(rxq->evq_hw_ring, rxq->evq_ptr_mask, evq_old_read_ptr,
                     rxq->evq_read_ptr);
...
static unsigned int sfc_ef10_essb_rx_qdesc_npending(struct sfc_dp_rxq *dp_rxq) {
...
static int sfc_ef10_essb_rx_qdesc_status(struct sfc_dp_rxq *dp_rxq,
                                         uint16_t offset) {
  struct sfc_ef10_essb_rxq *rxq = sfc_ef10_essb_rxq_by_dp_rxq(dp_rxq);
...
static void sfc_ef10_essb_rx_get_dev_info(struct rte_eth_dev_info *dev_info) {
  /*
...
static int sfc_ef10_essb_rx_pool_ops_supported(const char *pool) {
...
static int sfc_ef10_essb_rx_qsize_up_rings(uint16_t nb_rx_desc,
   ...
   */
  nb_hw_rx_desc =
      RTE_MAX(SFC_DIV_ROUND_UP(nb_rx_desc, mp_info.contig_block_size),
  ...
  *evq_entries = RTE_MIN(*evq_entries, limits->evq_max_entries);
   ...
   */
  *rxq_max_fill_level =
      RTE_MIN(nb_rx_desc, SFC_EF10_ESSB_RXQ_LIMIT(*evq_entries));
  return 0;
}

static sfc_dp_rx_qcreate_t sfc_ef10_essb_rx_qcreate;
static int sfc_ef10_essb_rx_qcreate(uint16_t port_id, uint16_t queue_id,
  ...
  rc = ENOMEM;
  rxq = rte_zmalloc_socket("sfc-ef10-rxq", sizeof(*rxq), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (rxq == NULL)
    goto fail_rxq_alloc;
  ...
  rc = ENOMEM;
  rxq->sw_ring =
      rte_calloc_socket("sfc-ef10-rxq-sw_ring", info->rxq_entries,
  ...
  sfc_ef10_essb_rx_info(&rxq->dp.dpq, "RxQ doorbell is %p", rxq->doorbell);
  sfc_ef10_essb_rx_info(&rxq->dp.dpq, "block size is %u, buf stride is %u",
                        rxq->block_size, rxq->buf_stride);
  sfc_ef10_essb_rx_info(
      &rxq->dp.dpq,
      "max fill level is %u descs (%u bufs), "
      "refill threshold %u descs (%u bufs)",
      rxq->max_fill_level, rxq->max_fill_level * rxq->block_size,
      rxq->refill_threshold, rxq->refill_threshold * rxq->block_size);
...
static void sfc_ef10_essb_rx_qdestroy(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef10_essb_rxq *rxq = sfc_ef10_essb_rxq_by_dp_rxq(dp_rxq);
...
static int sfc_ef10_essb_rx_qstart(struct sfc_dp_rxq *dp_rxq,
                                   unsigned int evq_read_ptr,
  ...
  if (efx_rx_prefix_layout_check(pinfo, &sfc_ef10_essb_rx_prefix_layout) != 0)
    return ENOTSUP;
  ...
  rxq->flags &= ~(SFC_EF10_ESSB_RXQ_NOT_RUNNING | SFC_EF10_ESSB_RXQ_EXCEPTION);

  return 0;
}

static sfc_dp_rx_qstop_t sfc_ef10_essb_rx_qstop;
static void sfc_ef10_essb_rx_qstop(struct sfc_dp_rxq *dp_rxq,
                                   unsigned int *evq_read_ptr) {
  struct sfc_ef10_essb_rxq *rxq = sfc_ef10_essb_rxq_by_dp_rxq(dp_rxq);
...
static bool sfc_ef10_essb_rx_qrx_ev(struct sfc_dp_rxq *dp_rxq,
...
static void sfc_ef10_essb_rx_qpurge(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef10_essb_rxq *rxq = sfc_ef10_essb_rxq_by_dp_rxq(dp_rxq);
  unsigned int i;
  const struct sfc_ef10_essb_rx_sw_desc *rxd;
  struct rte_mbuf *m;
    ...
    .dp =
        {
            ...
            .type = SFC_DP_RX,
            .hw_fw_caps =
                SFC_DP_HW_FW_CAP_EF10 | SFC_DP_HW_FW_CAP_RX_ES_SUPER_BUFFER,
        },
    .features = SFC_DP_RX_FEAT_FLOW_FLAG | SFC_DP_RX_FEAT_FLOW_MARK,
    .dev_offload_capa =
