...
static void sfc_sw_stat_get_rx_good_pkts_bytes(struct sfc_adapter *sa,
                                               uint16_t qid, uint64_t *values,
...
static void sfc_sw_stat_get_tx_good_pkts_bytes(struct sfc_adapter *sa,
                                               uint16_t qid, uint64_t *values,
                                               unsigned int values_count) {
  struct sfc_adapter_shared *sas = sfc_sa2shared(sa);
...
static void sfc_get_sw_stat_val_rx_dbells(struct sfc_adapter *sa, uint16_t qid,
                                          uint64_t *values,
                                          unsigned int values_count) {
  struct sfc_adapter_shared *sas = sfc_sa2shared(sa);
  struct sfc_rxq_info *rxq_info;
  ...
  rxq_info = sfc_rxq_info_by_ethdev_qid(sas, qid);
  values[0] =
      rxq_info->state & SFC_RXQ_INITIALIZED ? rxq_info->dp->dpq.dbells : 0;
}

static sfc_get_sw_stat_val_t sfc_get_sw_stat_val_tx_dbells;
static void sfc_get_sw_stat_val_tx_dbells(struct sfc_adapter *sa, uint16_t qid,
                                          uint64_t *values,
                                          unsigned int values_count) {
  struct sfc_adapter_shared *sas = sfc_sa2shared(sa);
  struct sfc_txq_info *txq_info;
  ...
  txq_info = sfc_txq_info_by_ethdev_qid(sas, qid);
  values[0] =
      txq_info->state & SFC_TXQ_INITIALIZED ? txq_info->dp->dpq.dbells : 0;
}
...
static int sfc_sw_stat_get_name(struct sfc_adapter *sa,
  ...
  case SFC_SW_STATS_TX:
    ...
    break;
  default:
    sfc_err(sa, "%s: unknown software statistics type %d", __func__,
            sw_stat->type);
    return -EINVAL;
  }

  if (sw_stat->provide_total && id_off == 0) {
    ret = snprintf(name, name_size, "%s_%s", prefix, sw_stat->name);
    if (ret < 0 || ret >= (int)name_size) {
      sfc_err(sa, "%s: failed to fill xstat name %s_%s, err %d", __func__,
              prefix, sw_stat->name, ret);
      return ret > 0 ? -EINVAL : ret;
    }
  } else {
    uint16_t qid = id_off - sw_stat->provide_total;
    ret = snprintf(name, name_size, "%s_q%u_%s", prefix, qid, sw_stat->name);
    if (ret < 0 || ret >= (int)name_size) {
      sfc_err(sa, "%s: failed to fill xstat name %s_q%u_%s, err %d", __func__,
              prefix, qid, sw_stat->name, ret);
      return ret > 0 ? -EINVAL : ret;
    }
  }
...
static int sfc_sw_stat_get_names(struct sfc_adapter *sa,
                                 const struct sfc_sw_stat_descr *sw_stat,
  ...
  *nb_supported += sfc_sw_xstat_per_queue_get_count(sw_stat, nb_queues);
  ...
  if (*nb_written < xstats_names_sz && sw_stat->provide_total) {
    rc = sfc_sw_stat_get_name(sa, sw_stat, xstats_names[*nb_written].name,
...
static int sfc_sw_xstat_get_names_by_id(struct sfc_adapter *sa,
                                        const struct sfc_sw_stat_descr *sw_stat,
                                        const uint64_t *ids,
                                        struct rte_eth_xstat_name *xstats_names,
                                        unsigned int size,
  ...
  *nb_supported += sfc_sw_xstat_per_queue_get_count(sw_stat, nb_queues);
  ...
  for (i = 0; i < size; i++) {
    if (id_base <= ids[i] && ids[i] < id_end) {
      rc = sfc_sw_stat_get_name(sa, sw_stat, xstats_names[i].name, name_size,
                                ids[i] - id_base);
      if (rc != 0)
        return rc;
    }
...
static uint64_t sfc_sw_stat_get_val(struct sfc_adapter *sa,
  ...
  sw_stats->supp[group_start_idx].descr->get_val(sa, qid, values, group_size);
...
static void sfc_sw_xstat_get_values(struct sfc_adapter *sa,
                                    const struct sfc_sw_stat_descr *sw_stat,
...
static void sfc_sw_xstat_get_values_by_id(
    struct sfc_adapter *sa, const struct sfc_sw_stat_descr *sw_stat,
    unsigned int sw_stat_idx, const uint64_t *ids, uint64_t *values,
    unsigned int ids_size, unsigned int *nb_supported) {
  ...
  if (count_total_value) {
    ...
    for (qid = 0; qid < nb_queues; ++qid) {
      if (rte_bitmap_get(bmp, qid) != 0)
        continue;
      values[total_value_idx] += sfc_sw_stat_get_val(sa, sw_stat_idx, qid);
    }
...
static void sfc_sw_stats_clear_cache(struct sfc_adapter *sa) {
...
void sfc_sw_xstats_get_vals(struct sfc_adapter *sa,
  ...
  for (i = 0; i < sw_stats->supp_count; i++) {
    sfc_sw_xstat_get_values(sa, sw_stats->supp[i].descr, i, xstats,
...
int sfc_sw_xstats_get_names(struct sfc_adapter *sa,
  ...
  for (i = 0; i < sw_stats->supp_count; i++) {
    ret = sfc_sw_stat_get_names(sa, sw_stats->supp[i].descr, xstats_names,
                                xstats_count, nb_written, nb_supported);
    if (ret != 0) {
      sfc_adapter_unlock(sa);
      return ret;
...
void sfc_sw_xstats_get_vals_by_id(struct sfc_adapter *sa, const uint64_t *ids,
  ...
  for (i = 0; i < sw_stats->supp_count; i++) {
    sfc_sw_xstat_get_values_by_id(sa, sw_stats->supp[i].descr, i, ids, values,
                                  n, nb_supported);
  }
...
int sfc_sw_xstats_get_names_by_id(struct sfc_adapter *sa, const uint64_t *ids,
  ...
  for (i = 0; i < sw_stats->supp_count; i++) {
    ret = sfc_sw_xstat_get_names_by_id(sa, sw_stats->supp[i].descr, ids,
                                       xstats_names, size, nb_supported);
    if (ret != 0) {
      sfc_adapter_unlock(sa);
      SFC_ASSERT(ret < 0);
      return ret;
...
static void sfc_sw_xstat_reset(struct sfc_adapter *sa,
                               const struct sfc_sw_stat_descr *sw_stat,
                               unsigned int sw_stat_idx, uint64_t *reset_vals) {
  unsigned int nb_queues;
  unsigned int qid;
...
void sfc_sw_xstats_reset(struct sfc_adapter *sa) {
  ...
  for (i = 0; i < sw_stats->supp_count; i++) {
    ...
    reset_vals += sfc_sw_xstat_get_nb_supported(sa, sw_stats->supp[i].descr);
  }
}

static bool sfc_sw_stats_is_packets_or_bytes(const char *xstat_name) {
  return strcmp(xstat_name, SFC_SW_STAT_GOOD_PACKETS) == 0 ||
         strcmp(xstat_name, SFC_SW_STAT_GOOD_BYTES) == 0;
}

static void sfc_sw_stats_fill_available_descr(struct sfc_adapter *sa) {
  ...
  for (i = 0; i < RTE_DIM(sfc_sw_stats_descr); i++) {
    ...
    if (!have_dp_rx_stats && sw_stat_descr->type == SFC_SW_STATS_RX &&
        sfc_sw_stats_is_packets_or_bytes(sw_stat_descr->name))
      continue;
    if (!have_dp_tx_stats && sw_stat_descr->type == SFC_SW_STATS_TX &&
        sfc_sw_stats_is_packets_or_bytes(sw_stat_descr->name))
      continue;
...
static int sfc_sw_stats_set_reset_basic_stats(struct sfc_adapter *sa) {
  uint64_t *reset_vals = sa->sw_stats.reset_vals;
  struct sfc_sw_stats *sw_stats = &sa->sw_stats;
  const struct sfc_sw_stat_descr *sw_stat;
  unsigned int i;
  ...
  for (i = 0; i < sw_stats->supp_count; i++) {
    ...
    switch (sw_stat->type) {
    case SFC_SW_STATS_RX:
      if (strcmp(sw_stat->name, SFC_SW_STAT_GOOD_PACKETS) == 0)
        sa->sw_stats.reset_rx_pkts = reset_vals;
      else if (strcmp(sw_stat->name, SFC_SW_STAT_GOOD_BYTES) == 0)
...
int sfc_sw_xstats_configure(struct sfc_adapter *sa) {
  ...
  if (sw_stats->supp == NULL) {
    sw_stats->supp =
        rte_malloc(NULL, sw_stats->supp_count * sizeof(*sw_stats->supp), 0);
    if (sw_stats->supp == NULL)
      return -ENOMEM;
  }
  for (i = 0; i < sw_stats->supp_count; i++)
  ...
  for (i = 0; i < sw_stats->supp_count; i++) {
    nb_supported += sfc_sw_xstat_get_nb_supported(sa, sw_stats->supp[i].descr);
    cache_count += sfc_sw_stat_get_queue_count(sa, sw_stats->supp[i].descr);
  }
  ...
  *cache = rte_realloc(*cache, cache_count * sizeof(**cache), 0);
  ...
  for (i = 0; i < sw_stats->supp_count; i++) {
    ...
    stat_cache += sfc_sw_stat_get_queue_count(sa, sw_stats->supp[i].descr);
  }
...
static void sfc_sw_xstats_free_queues_bitmap(struct sfc_adapter *sa) {
  rte_bitmap_free(sa->sw_stats.queues_bitmap);
  rte_free(sa->sw_stats.queues_bitmap_mem);
}

static int sfc_sw_xstats_alloc_queues_bitmap(struct sfc_adapter *sa) {
  ...
  *queues_bitmap_mem =
      rte_calloc_socket("bitmap_mem", bmp_size, 1, 0, sa->socket_id);
  if (*queues_bitmap_mem == NULL)
    return ENOMEM;

  *queues_bitmap =
      rte_bitmap_init(RTE_MAX_QUEUES_PER_PORT, *queues_bitmap_mem, bmp_size);
