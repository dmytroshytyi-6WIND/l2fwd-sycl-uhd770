...
static int sfc_efx_rx_qprime(struct sfc_efx_rxq *rxq) {
  int rc = 0;
...
static void sfc_efx_rx_qrefill(struct sfc_efx_rxq *rxq) {
  ...
  do {
    if (unlikely(rte_mempool_get_bulk(rxq->refill_mb_pool, objs,
       ...
       */
      rxq->evq->sa->eth_dev->data->rx_mbuf_alloc_failed += RTE_DIM(objs);
    ...
    for (i = 0; i < RTE_DIM(objs); ++i, id = (id + 1) & rxq->ptr_mask) {
      m = objs[i];
    ...
    efx_rx_qpost(rxq->common, addr, rxq->buf_size, RTE_DIM(objs),
                 rxq->completed, added);
    added += RTE_DIM(objs);
  } while (--bulks > 0);
...
sfc_efx_rx_desc_flags_to_offload_flags(const unsigned int desc_flags) {
  ...
  switch ((desc_flags & (EFX_PKT_TCP | EFX_PKT_UDP | EFX_CKSUM_TCPUDP))) {
...
sfc_efx_rx_desc_flags_to_packet_type(const unsigned int desc_flags) {
  return RTE_PTYPE_L2_ETHER |
         ((desc_flags & EFX_PKT_IPV4) ? RTE_PTYPE_L3_IPV4_EXT_UNKNOWN : 0) |
         ((desc_flags & EFX_PKT_IPV6) ? RTE_PTYPE_L3_IPV6_EXT_UNKNOWN : 0) |
...
static void sfc_efx_rx_set_rss_hash(struct sfc_efx_rxq *rxq, unsigned int flags,
                                    struct rte_mbuf *m) {
  uint8_t *mbuf_data;
  ...
  if (flags & (EFX_PKT_IPV4 | EFX_PKT_IPV6)) {
    m->hash.rss = efx_pseudo_hdr_hash_get(rxq->common, EFX_RX_HASHALG_TOEPLITZ,
                                          mbuf_data);
...
static uint16_t sfc_efx_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                                  uint16_t nb_pkts) {
  ...
  while (completed != rxq->pending && done_pkts < nb_pkts) {
    ...
    if (desc_flags & EFX_PKT_PREFIX_LEN) {
      ...
      rc = efx_pseudo_hdr_pkt_length_get(
          rxq->common, rte_pktmbuf_mtod(m, uint8_t *), &tmp_size);
      SFC_ASSERT(rc == 0);
      seg_len = tmp_size;
    } else {
    ...
    m->ol_flags = sfc_efx_rx_desc_flags_to_offload_flags(desc_flags);
    m->packet_type = sfc_efx_rx_desc_flags_to_packet_type(desc_flags);
...
static unsigned int sfc_efx_rx_qdesc_npending(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_efx_rxq *rxq = sfc_efx_rxq_by_dp_rxq(dp_rxq);
...
static int sfc_efx_rx_qdesc_status(struct sfc_dp_rxq *dp_rxq, uint16_t offset) {
  struct sfc_efx_rxq *rxq = sfc_efx_rxq_by_dp_rxq(dp_rxq);
...
boolean_t sfc_rx_check_scatter(size_t pdu, size_t rx_buf_size,
                               ...
                               uint32_t rx_scatter_max, const char **error) {
...
static int sfc_efx_rx_qsize_up_rings(
    uint16_t nb_rx_desc, __rte_unused struct sfc_dp_rx_hw_limits *limits,
    __rte_unused struct rte_mempool *mb_pool, unsigned int *rxq_entries,
    unsigned int *evq_entries, unsigned int *rxq_max_fill_level) {
...
static int sfc_efx_rx_qcreate(uint16_t port_id, uint16_t queue_id,
  ...
  rc = ENOMEM;
  rxq = rte_zmalloc_socket("sfc-efx-rxq", sizeof(*rxq), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (rxq == NULL)
    goto fail_rxq_alloc;
  ...
  rc = ENOMEM;
  rxq->sw_desc =
      rte_calloc_socket("sfc-efx-rxq-sw_desc", info->rxq_entries,
                        sizeof(*rxq->sw_desc), RTE_CACHE_LINE_SIZE, socket_id);
  if (rxq->sw_desc == NULL)
...
static void sfc_efx_rx_qdestroy(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_efx_rxq *rxq = sfc_efx_rxq_by_dp_rxq(dp_rxq);
...
static int sfc_efx_rx_qstart(struct sfc_dp_rxq *dp_rxq,
...
static void sfc_efx_rx_qstop(struct sfc_dp_rxq *dp_rxq,
                             __rte_unused unsigned int *evq_read_ptr) {
  struct sfc_efx_rxq *rxq = sfc_efx_rxq_by_dp_rxq(dp_rxq);
...
static void sfc_efx_rx_qpurge(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_efx_rxq *rxq = sfc_efx_rxq_by_dp_rxq(dp_rxq);
  unsigned int i;
  struct sfc_efx_rx_sw_desc *rxd;
...
static int sfc_efx_rx_intr_enable(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_efx_rxq *rxq = sfc_efx_rxq_by_dp_rxq(dp_rxq);
  int rc = 0;
...
static int sfc_efx_rx_intr_disable(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_efx_rxq *rxq = sfc_efx_rxq_by_dp_rxq(dp_rxq);
...
struct sfc_dp_rx sfc_efx_rx = {
    ...
    .dev_offload_capa =
        RTE_ETH_RX_OFFLOAD_CHECKSUM | RTE_ETH_RX_OFFLOAD_RSS_HASH,
...
static void sfc_rx_qflush(struct sfc_adapter *sa, sfc_sw_index_t sw_index) {
   ...
   */
  for (retry_count = 0; ((rxq_info->state & SFC_RXQ_FLUSHED) == 0) &&
                        (retry_count < SFC_RX_QFLUSH_ATTEMPTS);
       ++retry_count) {
    rc = efx_rx_qflush(rxq->common);
    if (rc != 0) {
      rxq_info->state |=
          (rc == EALREADY) ? SFC_RXQ_FLUSHED : SFC_RXQ_FLUSH_FAILED;
      break;
    }
    rxq_info->state &= ~SFC_RXQ_FLUSH_FAILED;
    rxq_info->state |= SFC_RXQ_FLUSHING;
    ...
    if (rxq_info->state & SFC_RXQ_FLUSHING)
      sfc_err(sa, "RxQ %d (internal %u) flush timed out", ethdev_qid, sw_index);

    if (rxq_info->state & SFC_RXQ_FLUSH_FAILED)
      sfc_err(sa, "RxQ %d (internal %u) flush failed", ethdev_qid, sw_index);

    if (rxq_info->state & SFC_RXQ_FLUSHED)
      sfc_notice(sa, "RxQ %d (internal %u) flushed", ethdev_qid, sw_index);
  }

  sa->priv.dp_rx->qpurge(rxq_info->dp);
}

static int sfc_rx_default_rxq_set_filter(struct sfc_adapter *sa,
                                         struct sfc_rxq *rxq) {
...
int sfc_rx_qstart(struct sfc_adapter *sa, sfc_sw_index_t sw_index) {
  ...
  case EFX_RXQ_TYPE_DEFAULT:
    rc =
        efx_rx_qcreate(sa->nic, rxq->hw_index, 0, rxq_info->type, rxq->buf_size,
  ...
  case EFX_RXQ_TYPE_ES_SUPER_BUFFER: {
    ...
    rc = efx_rx_qcreate_es_super_buffer(
        sa->nic, rxq->hw_index, 0, mp_info.contig_block_size, rxq->buf_size,
        mp->header_size + mp->elt_size + mp->trailer_size,
        sa->rxd_wait_timeout_ns, &rxq->mem, rxq_info->entries,
        rxq_info->type_flags, evq->common, &rxq->common);
    break;
  }
  default:
  ...
  if (ethdev_qid != SFC_ETHDEV_QID_INVALID)
    sa->eth_dev->data->rx_queue_state[ethdev_qid] = RTE_ETH_QUEUE_STATE_STARTED;
...
void sfc_rx_qstop(struct sfc_adapter *sa, sfc_sw_index_t sw_index) {
  ...
  /* It seems to be used by DPDK for debug purposes only ('rte_ether') */
  if (ethdev_qid != SFC_ETHDEV_QID_INVALID)
    sa->eth_dev->data->rx_queue_state[ethdev_qid] = RTE_ETH_QUEUE_STATE_STOPPED;
...
static uint64_t sfc_rx_get_offload_mask(struct sfc_adapter *sa) {
...
static int sfc_rx_qcheck_conf(struct sfc_adapter *sa,
                              unsigned int rxq_max_fill_level,
                              const struct rte_eth_rxconf *rx_conf,
                              __rte_unused uint64_t offloads) {
  int rc = 0;

  if (rx_conf->rx_thresh.pthresh != 0 || rx_conf->rx_thresh.hthresh != 0 ||
      rx_conf->rx_thresh.wthresh != 0) {
    sfc_warn(sa, "RxQ prefetch/host/writeback thresholds are not supported");
  }

  if (rx_conf->rx_free_thresh > rxq_max_fill_level) {
    sfc_err(sa, "RxQ free threshold too large: %u vs maximum %u",
...
static unsigned int sfc_rx_mbuf_data_alignment(struct rte_mempool *mb_pool) {
  uint32_t data_off;
  uint32_t order;
...
static uint16_t sfc_rx_mb_pool_buf_size(struct sfc_adapter *sa,
  ...
  if (buf_aligned < nic_align_start) {
    ...
    if (buf_size <= start_alignment) {
      sfc_err(sa,
              "RxQ mbuf pool %s object data room size %u is insufficient for "
              "headroom %u and buffer start alignment %u required by NIC",
              mb_pool->name, rte_pktmbuf_data_room_size(mb_pool),
  ...
  if (buf_aligned < nic_align_end) {
    ...
    end_padding_alignment =
        nic_align_end - MIN(buf_aligned, 1u << (rte_bsf32(buf_size) - 1));
    if (buf_size <= end_padding_alignment) {
      sfc_err(sa,
              "RxQ mbuf pool %s object data room size %u is insufficient for "
              "headroom %u, buffer start alignment %u and end padding "
              "alignment %u required by NIC",
              mb_pool->name, rte_pktmbuf_data_room_size(mb_pool),
              RTE_PKTMBUF_HEADROOM, start_alignment, end_padding_alignment);
      return 0;
    }
...
int sfc_rx_qinit(struct sfc_adapter *sa, sfc_sw_index_t sw_index,
  ...
  if (buf_size == 0) {
    sfc_err(sa, "RxQ %d (internal %u) mbuf pool object size is too small",
            ethdev_qid, sw_index);
    rc = EINVAL;
    goto fail_bad_conf;
  }

  if (!sfc_rx_check_scatter(sa->port.pdu, buf_size, encp->enc_rx_prefix_size,
                            (offloads & RTE_ETH_RX_OFFLOAD_SCATTER),
                            encp->enc_rx_scatter_max, &error)) {
    sfc_err(sa, "RxQ %d (internal %u) MTU check failed: %s", ethdev_qid,
            sw_index, error);
    sfc_err(sa,
            "RxQ %d (internal %u) calculated Rx buffer size is %u vs "
            "PDU size %u plus Rx prefix %u bytes",
            ethdev_qid, sw_index, buf_size, (unsigned int)sa->port.pdu,
  ...
  rxq_info->type_flags |= (offloads & RTE_ETH_RX_OFFLOAD_SCATTER)
  ...
  rc =
      sfc_ev_qinit(sa, SFC_EVQ_TYPE_RX, sw_index, evq_entries, socket_id, &evq);
  if (rc != 0)
    goto fail_ev_qinit;
  ...
  rx_free_thresh = (rx_conf->rx_free_thresh != 0)
                       ...
                       : EFX_DIV_ROUND_UP(nb_rx_desc, 8);
  /* Rx refill threshold cannot be smaller than refill bulk */
  rxq_info->refill_threshold = RTE_MAX(rx_free_thresh, SFC_RX_REFILL_BULK);
  ...
  rc = sfc_dma_alloc(sa, "rxq", sw_index, EFX_NIC_DMA_ADDR_RX_RING,
                     efx_rxq_size(sa->nic, rxq_info->entries), socket_id,
                     &rxq->mem);
  if (rc != 0)
    goto fail_dma_alloc;
...
static const struct sfc_rss_hf_rte_to_efx sfc_rss_hf_map[] = {
    {RTE_ETH_RSS_NONFRAG_IPV4_TCP, EFX_RX_HASH(IPV4_TCP, 4TUPLE)},
    {RTE_ETH_RSS_NONFRAG_IPV4_UDP, EFX_RX_HASH(IPV4_UDP, 4TUPLE)},
...
sfc_rx_hash_types_mask_supp(efx_rx_hash_type_t hash_type,
  ...
  for (i = 0; i < nb_hash_type_flags_supported; ++i) {
    unsigned int class_tuple_lbn[] = {
        EFX_RX_CLASS_IPV4_TCP_LBN, EFX_RX_CLASS_IPV4_UDP_LBN,
        EFX_RX_CLASS_IPV4_LBN,     EFX_RX_CLASS_IPV6_TCP_LBN,
        EFX_RX_CLASS_IPV6_UDP_LBN, EFX_RX_CLASS_IPV6_LBN};
...
int sfc_rx_hash_init(struct sfc_adapter *sa) {
  ...
  hf_map = rte_calloc_socket("sfc-rss-hf-map", RTE_DIM(sfc_rss_hf_map),
  ...
  for (i = 0; i < RTE_DIM(sfc_rss_hf_map); ++i) {
    ...
    ht = sfc_rx_hash_types_mask_supp(sfc_rss_hf_map[i].efx, flags_supp,
                                     nb_flags_supp);
    if (ht != 0) {
...
static int sfc_rx_process_adv_conf_rss(struct sfc_adapter *sa,
  ...
  if (rss->context_type != EFX_RX_SCALE_EXCLUSIVE) {
    if ((conf->rss_hf != 0 && conf->rss_hf != rss_hf) || conf->rss_key != NULL)
      return EINVAL;
  }
...
static int sfc_rx_rss_config(struct sfc_adapter *sa) {
  struct sfc_rss *rss = &sfc_sa2shared(sa)->rss;
  int rc = 0;

  if (rss->channels > 0) {
    rc = efx_rx_scale_mode_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss->hash_alg,
                               rss->hash_types, B_TRUE);
    if (rc != 0)
      goto finish;

    rc = efx_rx_scale_key_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss->key,
                              sizeof(rss->key));
    if (rc != 0)
      goto finish;

    rc = efx_rx_scale_tbl_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss->tbl,
                              RTE_DIM(rss->tbl));
  }
...
static int sfc_rx_check_mode(struct sfc_adapter *sa,
                             struct rte_eth_rxmode *rxmode) {
  struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
  uint64_t offloads_supported =
      sfc_rx_get_dev_offload_caps(sa) | sfc_rx_get_queue_offload_caps(sa);
  ...
  case RTE_ETH_MQ_RX_RSS:
    ...
    break;
  default:
    sfc_err(sa, "Rx multi-queue mode %u not supported", rxmode->mq_mode);
    rc = EINVAL;
  }
 ...
 * or complete close.
 */
static void sfc_rx_fini_queues(struct sfc_adapter *sa,
...
int sfc_rx_configure(struct sfc_adapter *sa) {
  ...
  sfc_log_init(sa, "nb_rx_queues=%u (old %u)", nb_rx_queues,
  ...
  if (sas->rxq_info == NULL) {
    reconfigure = false;
    rc = ENOMEM;
    sas->rxq_info = rte_calloc_socket(
        "sfc-rxqs", nb_rxq_total, sizeof(sas->rxq_info[0]), 0, sa->socket_id);
    if (sas->rxq_info == NULL)
    ...
    rc = ENOMEM;
    new_rxq_info =
        rte_realloc(sas->rxq_info, nb_rxq_total * sizeof(sas->rxq_info[0]), 0);
    if (new_rxq_info == NULL && nb_rxq_total > 0)
      goto fail_rxqs_realloc;

    rc = ENOMEM;
    new_rxq_ctrl =
        realloc(sa->rxq_ctrl, nb_rxq_total * sizeof(sa->rxq_ctrl[0]));
    ...
    if (nb_rxq_total > sas->rxq_count) {
      ...
      memset(&sas->rxq_info[rxq_count], 0,
             (nb_rxq_total - rxq_count) * sizeof(sas->rxq_info[0]));
      memset(&sa->rxq_ctrl[rxq_count], 0,
             (nb_rxq_total - rxq_count) * sizeof(sa->rxq_ctrl[0]));
    }
  }
  ...
  while (sas->ethdev_rxq_count < nb_rx_queues) {
    ...
    sw_index = sfc_rxq_sw_index_by_ethdev_rx_qid(sas, sas->ethdev_rxq_count);
...
configure_rss:
  rss->channels = (dev_conf->rxmode.mq_mode == RTE_ETH_MQ_RX_RSS)
                      ? MIN(sas->ethdev_rxq_count, EFX_MAXRSS)
                      : 0;
