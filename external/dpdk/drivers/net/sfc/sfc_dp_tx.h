...
static inline struct sfc_dp_tx *sfc_dp_find_tx_by_name(struct sfc_dp_list *head,
                                                       const char *name) {
...
static inline uint64_t sfc_dp_tx_offload_capa(const struct sfc_dp_tx *dp_tx) {
...
static inline int sfc_dp_tx_prepare_pkt(struct rte_mbuf *m,
  ...
  if (max_nb_header_segs != 0) {
    ...
    nb_header_segs +=
        sfc_dp_tx_pkt_extra_hdr_segs(&m_seg, &header_len_remaining);
  ...
  if (m->ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    if (tso_bounce_buffer_len != 0) {
      if (nb_header_segs > 1 && unlikely(header_len > tso_bounce_buffer_len)) {
        /*
