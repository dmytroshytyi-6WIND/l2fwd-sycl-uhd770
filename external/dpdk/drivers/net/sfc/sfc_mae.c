...
static int sfc_mae_assign_ethdev_mport(struct sfc_adapter *sa,
                                       efx_mport_sel_t *mportp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(sa->nic);

  return efx_mae_mport_by_pcie_function(encp->enc_pf, encp->enc_vf, mportp);
}

static int sfc_mae_assign_entity_mport(struct sfc_adapter *sa,
                                       efx_mport_sel_t *mportp) {
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(sa->nic);
  int rc = 0;
...
static int sfc_mae_internal_rule_find_empty_slot(struct sfc_adapter *sa,
...
int sfc_mae_rule_add_mport_match_deliver(struct sfc_adapter *sa,
  ...
  rc = efx_mae_match_spec_init(sa->nic, EFX_MAE_RULE_ACTION, (uint32_t)prio,
                               &rule->spec);
  if (rc != 0) {
  ...
  rc = efx_mae_action_set_populate_deliver(rule->actions, mport_deliver);
  if (rc != 0) {
    sfc_err(sa, "failed to populate deliver action");
    goto fail_populate_deliver;
  }

  rc = efx_mae_action_set_alloc(sa->nic, rule->actions, &rule->action_set);
  if (rc != 0) {
    sfc_err(sa, "failed to allocate action set");
    goto fail_action_set_alloc;
  }

  rc = efx_mae_action_rule_insert(sa->nic, rule->spec, NULL, &rule->action_set,
                                  &rule->rule_id);
  if (rc != 0) {
...
int sfc_mae_attach(struct sfc_adapter *sa) {
  ...
  switch_port_request.port_data.indep.mae_admin = encp->enc_mae_admin == B_TRUE;
  rc = sfc_mae_assign_switch_port(mae->switch_domain_id, &switch_port_request,
                                  &mae->switch_port_id);
  if (rc != 0)
  ...
  if (encp->enc_mae_admin) {
    ...
    bounce_eh->buf = rte_malloc("sfc_mae_bounce_eh", bounce_eh->buf_size, 0);
...
static int sfc_mae_outer_rule_add(struct sfc_adapter *sa,
...
static void sfc_mae_outer_rule_del(struct sfc_adapter *sa,
                                   struct sfc_mae_outer_rule *rule) {
  struct sfc_mae *mae = &sa->mae;
  ...
  if (rule->fw_rsrc.rule_id.id != EFX_MAE_RSRC_ID_INVALID ||
    ...
    sfc_err(sa,
            "deleting outer_rule=%p abandons its FW resource: OR_ID=0x%08x, "
            "refcnt=%u",
...
static int sfc_mae_outer_rule_enable(struct sfc_adapter *sa,
                                     struct sfc_mae_outer_rule *rule,
  ...
  if (fw_rsrc->refcnt == 0) {
    ...
    rc = efx_mae_outer_rule_insert(sa->nic, rule->match_spec, rule->encap_type,
                                   &fw_rsrc->rule_id);
    if (rc != 0) {
      sfc_err(sa, "failed to enable outer_rule=%p: %s", rule, strerror(rc));
      return rc;
    }
  }
  ...
  if (rc != 0) {
    if (fw_rsrc->refcnt == 0) {
      (void)efx_mae_outer_rule_remove(sa->nic, &fw_rsrc->rule_id);
...
skip_action_rule:
  if (fw_rsrc->refcnt == 0) {
    sfc_dbg(sa, "enabled outer_rule=%p: OR_ID=0x%08x", rule,
            fw_rsrc->rule_id.id);
  }
...
static void sfc_mae_outer_rule_disable(struct sfc_adapter *sa,
                                       struct sfc_mae_outer_rule *rule) {
  struct sfc_mae_fw_rsrc *fw_rsrc = &rule->fw_rsrc;
  int rc;
  ...
  if (fw_rsrc->rule_id.id == EFX_MAE_RSRC_ID_INVALID || fw_rsrc->refcnt == 0) {
    sfc_err(sa,
            "failed to disable outer_rule=%p: already disabled; OR_ID=0x%08x, "
            "refcnt=%u",
  ...
  if (fw_rsrc->refcnt == 1) {
    ...
    if (rc == 0) {
      sfc_dbg(sa, "disabled outer_rule=%p with OR_ID=0x%08x", rule,
              fw_rsrc->rule_id.id);
    } else {
      sfc_err(sa, "failed to disable outer_rule=%p with OR_ID=0x%08x: %s", rule,
              fw_rsrc->rule_id.id, strerror(rc));
    }
    fw_rsrc->rule_id.id = EFX_MAE_RSRC_ID_INVALID;
  }
...
sfc_mae_mac_addr_attach(struct sfc_adapter *sa,
  ...
  TAILQ_FOREACH(mac_addr, &mae->mac_addrs, entries) {
    if (memcmp(mac_addr->addr_bytes, addr_bytes, EFX_MAC_ADDR_LEN) == 0) {
...
static int sfc_mae_mac_addr_add(struct sfc_adapter *sa,
                                const uint8_t addr_bytes[EFX_MAC_ADDR_LEN],
...
static void sfc_mae_mac_addr_del(struct sfc_adapter *sa,
                                 struct sfc_mae_mac_addr *mac_addr) {
  struct sfc_mae *mae = &sa->mae;
  ...
  if (mac_addr->fw_rsrc.mac_id.id != EFX_MAE_RSRC_ID_INVALID ||
    ...
    sfc_err(sa,
            "deleting mac_addr=%p abandons its FW resource: MAC_ID=0x%08x, "
            "refcnt=%u",
            mac_addr, mac_addr->fw_rsrc.mac_id.id, mac_addr->fw_rsrc.refcnt);
  }
...
enum sfc_mae_mac_addr_type { SFC_MAE_MAC_ADDR_DST, SFC_MAE_MAC_ADDR_SRC };

static int sfc_mae_mac_addr_enable(struct sfc_adapter *sa,
                                   struct sfc_mae_mac_addr *mac_addr,
  ...
  if (fw_rsrc->refcnt == 0) {
    ...
    rc =
        efx_mae_mac_addr_alloc(sa->nic, mac_addr->addr_bytes, &fw_rsrc->mac_id);
    if (rc != 0) {
      sfc_err(sa, "failed to enable mac_addr=%p: %s", mac_addr, strerror(rc));
      return rc;
    }
  }
  ...
  case SFC_MAE_MAC_ADDR_DST:
    rc = efx_mae_action_set_fill_in_dst_mac_id(aset_spec, &fw_rsrc->mac_id);
    break;
  case SFC_MAE_MAC_ADDR_SRC:
    rc = efx_mae_action_set_fill_in_src_mac_id(aset_spec, &fw_rsrc->mac_id);
    break;
  default:
    rc = EINVAL;
    break;
  ...
  if (rc != 0) {
    ...
    sfc_err(sa, "cannot fill in MAC address entry ID: %s", strerror(rc));

    return rc;
  }

  if (fw_rsrc->refcnt == 0) {
    sfc_dbg(sa, "enabled mac_addr=%p: MAC_ID=0x%08x", mac_addr,
            fw_rsrc->mac_id.id);
  }
...
static void sfc_mae_mac_addr_disable(struct sfc_adapter *sa,
                                     struct sfc_mae_mac_addr *mac_addr) {
  struct sfc_mae_fw_rsrc *fw_rsrc;
  int rc;
  ...
  if (fw_rsrc->mac_id.id == EFX_MAE_RSRC_ID_INVALID || fw_rsrc->refcnt == 0) {
    sfc_err(sa,
            "failed to disable mac_addr=%p: already disabled; MAC_ID=0x%08x, "
            "refcnt=%u",
  ...
  if (fw_rsrc->refcnt == 1) {
    ...
    if (rc == 0) {
      sfc_dbg(sa, "disabled mac_addr=%p with MAC_ID=0x%08x", mac_addr,
              fw_rsrc->mac_id.id);
    } else {
...
sfc_mae_encap_header_attach(struct sfc_adapter *sa,
  ...
  TAILQ_FOREACH(encap_header, &mae->encap_headers, entries) {
    if (encap_header->size == bounce_eh->size &&
        memcmp(encap_header->buf, bounce_eh->buf, bounce_eh->size) == 0) {
      sfc_dbg(sa, "attaching to encap_header=%p", encap_header);
      ++(encap_header->refcnt);
      return encap_header;
    }
  }
...
sfc_mae_encap_header_add(struct sfc_adapter *sa,
  ...
  encap_header = rte_zmalloc("sfc_mae_encap_header", sizeof(*encap_header), 0);
  ...
  encap_header->buf =
      rte_malloc("sfc_mae_encap_header_buf", encap_header->size, 0);
...
sfc_mae_encap_header_del(struct sfc_adapter *sa,
  ...
  if (encap_header->fw_rsrc.eh_id.id != EFX_MAE_RSRC_ID_INVALID ||
    ...
    sfc_err(sa,
            "deleting encap_header=%p abandons its FW resource: EH_ID=0x%08x, "
            "refcnt=%u",
...
sfc_mae_encap_header_enable(struct sfc_adapter *sa,
  ...
  if (fw_rsrc->refcnt == 0) {
    ...
    rc = efx_mae_encap_header_alloc(sa->nic, encap_header->type,
                                    encap_header->buf, encap_header->size,
                                    &fw_rsrc->eh_id);
    if (rc != 0) {
      sfc_err(sa, "failed to enable encap_header=%p: %s", encap_header,
              strerror(rc));
      return rc;
    }
  }

  rc = efx_mae_action_set_fill_in_eh_id(action_set_spec, &fw_rsrc->eh_id);
  if (rc != 0) {
    if (fw_rsrc->refcnt == 0) {
      (void)efx_mae_encap_header_free(sa->nic, &fw_rsrc->eh_id);
  ...
  if (fw_rsrc->refcnt == 0) {
    sfc_dbg(sa, "enabled encap_header=%p: EH_ID=0x%08x", encap_header,
            fw_rsrc->eh_id.id);
  }
...
sfc_mae_encap_header_disable(struct sfc_adapter *sa,
  ...
  if (fw_rsrc->eh_id.id == EFX_MAE_RSRC_ID_INVALID || fw_rsrc->refcnt == 0) {
    sfc_err(sa,
            "failed to disable encap_header=%p: already disabled; "
            "EH_ID=0x%08x, refcnt=%u",
  ...
  if (fw_rsrc->refcnt == 1) {
    ...
    if (rc == 0) {
      sfc_dbg(sa, "disabled encap_header=%p with EH_ID=0x%08x", encap_header,
              fw_rsrc->eh_id.id);
    } else {
...
static int sfc_mae_counters_enable(struct sfc_adapter *sa,
  ...
  if (rc != 0) {
    sfc_err(sa, "failed to enable MAE counter %u: %s", counters[0].mae_id.id,
            rte_strerror(rc));
...
static int sfc_mae_counters_disable(struct sfc_adapter *sa,
                                    struct sfc_mae_counter_id *counters,
                                    unsigned int n_counters) {
  if (n_counters == 0)
    return 0;
...
sfc_mae_action_set_attach(struct sfc_adapter *sa,
  ...
  TAILQ_FOREACH(action_set, &mae->action_sets, entries) {
    if (action_set->encap_header == ctx->encap_header &&
        ...
        efx_mae_action_set_specs_equal(action_set->spec, ctx->spec)) {
...
static int sfc_mae_action_set_add(struct sfc_adapter *sa,
  ...
  if (ctx->n_counters > 0) {
    ...
    action_set->counters =
        rte_malloc("sfc_mae_counter_ids",
    ...
    for (i = 0; i < ctx->n_counters; ++i) {
      ...
      action_set->counters[i].ft_group_hit_counter = ctx->ft_group_hit_counter;
      action_set->counters[i].ft = ctx->counter_ft;
    }

    for (action = actions, i = 0;
         action->type != RTE_FLOW_ACTION_TYPE_END && i < ctx->n_counters;
         ++action) {
...
static void sfc_mae_action_set_del(struct sfc_adapter *sa,
                                   struct sfc_mae_action_set *action_set) {
  struct sfc_mae *mae = &sa->mae;
  ...
  if (action_set->fw_rsrc.aset_id.id != EFX_MAE_RSRC_ID_INVALID ||
    ...
    sfc_err(sa,
            "deleting action_set=%p abandons its FW resource: AS_ID=0x%08x, "
            "refcnt=%u",
  ...
  if (action_set->n_counters > 0) {
    ...
    SFC_ASSERT(action_set->counters[0].mae_id.id == EFX_MAE_RSRC_ID_INVALID);
...
static int sfc_mae_action_set_enable(struct sfc_adapter *sa,
                                     struct sfc_mae_action_set *action_set) {
  ...
  if (fw_rsrc->refcnt == 0) {
    ...
    rc = sfc_mae_mac_addr_enable(sa, dst_mac_addr, SFC_MAE_MAC_ADDR_DST,
                                 action_set->spec);
    if (rc != 0)
      return rc;

    rc = sfc_mae_mac_addr_enable(sa, src_mac_addr, SFC_MAE_MAC_ADDR_SRC,
                                 action_set->spec);
    if (rc != 0) {
    ...
    rc = sfc_mae_encap_header_enable(sa, encap_header, action_set->spec);
    if (rc != 0) {
    ...
    rc = sfc_mae_counters_enable(sa, counters, action_set->n_counters,
                                 action_set->spec);
    if (rc != 0) {
    ...
    rc = efx_mae_action_set_alloc(sa->nic, action_set->spec, &fw_rsrc->aset_id);
    if (rc != 0) {
      sfc_err(sa, "failed to enable action_set=%p: %s", action_set,
              strerror(rc));

      (void)sfc_mae_counters_disable(sa, counters, action_set->n_counters);
      sfc_mae_encap_header_disable(sa, encap_header);
      sfc_mae_mac_addr_disable(sa, src_mac_addr);
      sfc_mae_mac_addr_disable(sa, dst_mac_addr);
      return rc;
    ...
    sfc_dbg(sa, "enabled action_set=%p: AS_ID=0x%08x", action_set,
            fw_rsrc->aset_id.id);
  }
...
static void sfc_mae_action_set_disable(struct sfc_adapter *sa,
                                       struct sfc_mae_action_set *action_set) {
  struct sfc_mae_fw_rsrc *fw_rsrc = &action_set->fw_rsrc;
  int rc;
  ...
  if (fw_rsrc->aset_id.id == EFX_MAE_RSRC_ID_INVALID || fw_rsrc->refcnt == 0) {
    sfc_err(sa,
            "failed to disable action_set=%p: already disabled; AS_ID=0x%08x, "
            "refcnt=%u",
  ...
  if (fw_rsrc->refcnt == 1) {
    ...
    if (rc == 0) {
      sfc_dbg(sa, "disabled action_set=%p with AS_ID=0x%08x", action_set,
              fw_rsrc->aset_id.id);
    } else {
...
void sfc_mae_flow_cleanup(struct sfc_adapter *sa, struct rte_flow *flow) {
...
static int sfc_mae_set_ethertypes(struct sfc_mae_parse_ctx *ctx) {
  ...
  rc = efx_mae_match_spec_field_set(
      ctx->match_spec, fremap[EFX_MAE_FIELD_ETHER_TYPE_BE], sizeof(et->value),
      (const uint8_t *)&et->value, sizeof(et->mask),
  ...
  for (i = 0; i < pdata->nb_vlan_tags; ++i) {
    ...
    rc = efx_mae_match_spec_field_set(
        ctx->match_spec, fremap[field_ids[i]], sizeof(et->value),
        (const uint8_t *)&et->value, sizeof(et->mask),
        (const uint8_t *)&et->mask);
    if (rc != 0)
      return rc;
...
static int sfc_mae_rule_process_pattern_data(struct sfc_mae_parse_ctx *ctx,
  ...
  for (ethertype_idx = 0; ethertype_idx < pdata->nb_vlan_tags;
       ++ethertype_idx) {
    ...
    if (tpid_idx == nb_supported_tpids) {
      sfc_err(ctx->sa, "TPID 0x%04x is unsupported", rte_be_to_cpu_16(tpid_v));
      rc = EINVAL;
      goto fail;
    }
  ...
  if (pdata->innermost_ethertype_restriction.mask == RTE_BE16(0xffff)) {
    ...
    } else if (et->mask != RTE_BE16(0xffff) || et->value != enforced_et) {
      sfc_err(
          ctx->sa,
          "L3 EtherType must be 0x0/0x0 or 0x%04x/0xffff; got 0x%04x/0x%04x",
          rte_be_to_cpu_16(enforced_et), rte_be_to_cpu_16(et->value),
  ...
  if (pdata->l3_next_proto_restriction_mask == 0xff) {
    if (pdata->l3_next_proto_mask == 0) {
      ...
      pdata->l3_next_proto_value = pdata->l3_next_proto_restriction_value;
    } else if (pdata->l3_next_proto_mask != 0xff ||
               pdata->l3_next_proto_value !=
                   pdata->l3_next_proto_restriction_value) {
      sfc_err(
          ctx->sa,
          "L3 next protocol must be 0x0/0x0 or 0x%02x/0xff; got 0x%02x/0x%02x",
          pdata->l3_next_proto_restriction_value, pdata->l3_next_proto_value,
  ...
  if (enforce_tag_presence[0] || pdata->has_ovlan_mask) {
    rc = efx_mae_match_spec_bit_set(
        ctx->match_spec, fremap[EFX_MAE_FIELD_HAS_OVLAN],
        enforce_tag_presence[0] || pdata->has_ovlan_value);
    if (rc != 0)
      goto fail;
  }
  ...
  if (enforce_tag_presence[1] || pdata->has_ivlan_mask) {
    rc = efx_mae_match_spec_bit_set(
        ctx->match_spec, fremap[EFX_MAE_FIELD_HAS_IVLAN],
        enforce_tag_presence[1] || pdata->has_ivlan_value);
    if (rc != 0)
      goto fail;
  }
  ...
  rc = efx_mae_match_spec_field_set(ctx->match_spec,
                                    ...
                                    sizeof(pdata->l3_next_proto_value), valuep,
                                    sizeof(pdata->l3_next_proto_mask), maskp);
  if (rc != 0)
    goto fail;
...
static int sfc_mae_rule_parse_item_mark(const struct rte_flow_item *item,
...
static int sfc_mae_rule_parse_item_port_id(const struct rte_flow_item *item,
                                           struct sfc_flow_parse_ctx *ctx,
                                           struct rte_flow_error *error) {
  struct sfc_mae_parse_ctx *ctx_mae = ctx->mae;
  ...
  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
  ...
  if (spec->id > UINT16_MAX) {
    return rte_flow_error_set(error, EOVERFLOW, RTE_FLOW_ERROR_TYPE_ITEM, item,
  ...
  rc = sfc_mae_switch_get_ethdev_mport(ctx_mae->sa->mae.switch_domain_id,
                                       spec->id, &mport_sel);
  if (rc != 0) {
    return rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ITEM, item,
                              "Can't get m-port for the given ethdev");
  }

  rc = efx_mae_match_spec_mport_set(ctx_mae->match_spec, &mport_sel, NULL);
  if (rc != 0) {
    return rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ITEM, item,
...
sfc_mae_rule_parse_item_ethdev_based(const struct rte_flow_item *item,
  ...
  case RTE_FLOW_ITEM_TYPE_REPRESENTED_PORT:
    rc = sfc_mae_switch_get_entity_mport(ctx_mae->sa->mae.switch_domain_id,
                                         spec->port_id, &mport_sel);
    if (rc != 0) {
      return rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ITEM, item,
                                "Can't get m-port for the given ethdev");
...
static int sfc_mae_rule_parse_item_phy_port(const struct rte_flow_item *item,
                                            struct sfc_flow_parse_ctx *ctx,
                                            struct rte_flow_error *error) {
  struct sfc_mae_parse_ctx *ctx_mae = ctx->mae;
...
static int sfc_mae_rule_parse_item_pf(const struct rte_flow_item *item,
                                      struct sfc_flow_parse_ctx *ctx,
                                      struct rte_flow_error *error) {
  struct sfc_mae_parse_ctx *ctx_mae = ctx->mae;
...
static int sfc_mae_rule_parse_item_vf(const struct rte_flow_item *item,
                                      struct sfc_flow_parse_ctx *ctx,
                                      struct rte_flow_error *error) {
  struct sfc_mae_parse_ctx *ctx_mae = ctx->mae;
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(ctx_mae->sa->nic);
...
sfc_mae_parse_item(const struct sfc_mae_field_locator *field_locators,
  ...
  for (i = 0; i < nb_field_locators; ++i) {
    ...
    rc = efx_mae_match_spec_field_set(ctx->match_spec, fremap[fl->field_id],
                                      fl->size, spec + fl->ofst, fl->size,
                                      mask + fl->ofst);
    if (rc != 0)
      break;
  }
  ...
  if (rc != 0) {
    rc = rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                            "Failed to process item fields");
  }
...
static int sfc_mae_rule_parse_item_eth(const struct rte_flow_item *item,
                                       struct sfc_flow_parse_ctx *ctx,
                                       struct rte_flow_error *error) {
  struct sfc_mae_parse_ctx *ctx_mae = ctx->mae;
  ...
  sfc_mae_item_build_supp_mask(flocs_eth, RTE_DIM(flocs_eth), &supp_mask,
                               sizeof(supp_mask));
  supp_mask.has_vlan = 1;

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           (const void *)&supp_mask, &rte_flow_item_eth_mask,
  ...
  if (ctx_mae->ft_rule_type == SFC_FT_RULE_JUMP && mask != NULL) {
    ...
    memset(&override_mask.hdr.dst_addr, 0, sizeof(override_mask.hdr.dst_addr));
    memset(&override_mask.hdr.src_addr, 0, sizeof(override_mask.hdr.src_addr));
  ...
  return sfc_mae_parse_item(flocs_eth, RTE_DIM(flocs_eth), spec, mask, ctx_mae,
                            error);
}
...
static int sfc_mae_rule_parse_item_vlan(const struct rte_flow_item *item,
                                        struct sfc_flow_parse_ctx *ctx,
                                        struct rte_flow_error *error) {
  struct sfc_mae_parse_ctx *ctx_mae = ctx->mae;
  struct sfc_mae_pattern_data *pdata = &ctx_mae->pattern_data;
  ...
  sfc_mae_item_build_supp_mask(flocs, nb_flocs, &supp_mask, sizeof(supp_mask));
  /*
  ...
  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           (const void *)&supp_mask, &rte_flow_item_vlan_mask,
  ...
  if (spec != NULL) {
    ...
    rc = sfc_mae_parse_item(flocs, nb_flocs, spec, mask, ctx_mae, error);
    if (rc != 0)
      return rc;
  }
...
static const struct sfc_mae_field_locator flocs_ipv4[] = {
    ...
    },
    {
        ...
        RTE_SIZEOF_FIELD(struct rte_flow_item_ipv4, hdr.type_of_service),
...
static int sfc_mae_rule_parse_item_ipv4(const struct rte_flow_item *item,
                                        struct sfc_flow_parse_ctx *ctx,
                                        struct rte_flow_error *error) {
  ...
  sfc_mae_item_build_supp_mask(flocs_ipv4, RTE_DIM(flocs_ipv4), &supp_mask,
                               sizeof(supp_mask));

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           (const void *)&supp_mask, &rte_flow_item_ipv4_mask,
...
static int sfc_mae_rule_parse_item_ipv6(const struct rte_flow_item *item,
                                        struct sfc_flow_parse_ctx *ctx,
                                        struct rte_flow_error *error) {
  ...
  sfc_mae_item_build_supp_mask(flocs_ipv6, RTE_DIM(flocs_ipv6), &supp_mask,
                               sizeof(supp_mask));
  ...
  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           (const void *)&supp_mask, &rte_flow_item_ipv6_mask,
  ...
  rc = sfc_mae_parse_item(flocs_ipv6, RTE_DIM(flocs_ipv6), spec, mask, ctx_mae,
                          error);
  if (rc != 0)
    return rc;
  ...
  rc = efx_mae_match_spec_field_set(
      ctx_mae->match_spec, fremap[EFX_MAE_FIELD_IP_TOS], sizeof(tc_value),
      &tc_value, sizeof(tc_mask), &tc_mask);
  if (rc != 0) {
    return rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                              "Failed to process item fields");
  }
...
static int sfc_mae_rule_parse_item_tcp(const struct rte_flow_item *item,
                                       struct sfc_flow_parse_ctx *ctx,
                                       struct rte_flow_error *error) {
  struct sfc_mae_parse_ctx *ctx_mae = ctx->mae;
  struct sfc_mae_pattern_data *pdata = &ctx_mae->pattern_data;
  ...
  sfc_mae_item_build_supp_mask(flocs_tcp, RTE_DIM(flocs_tcp), &supp_mask,
                               sizeof(supp_mask));

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           (const void *)&supp_mask, &rte_flow_item_tcp_mask,
  ...
  return sfc_mae_parse_item(flocs_tcp, RTE_DIM(flocs_tcp), spec, mask, ctx_mae,
                            error);
}
...
static int sfc_mae_rule_parse_item_udp(const struct rte_flow_item *item,
                                       struct sfc_flow_parse_ctx *ctx,
                                       struct rte_flow_error *error) {
  struct sfc_mae_parse_ctx *ctx_mae = ctx->mae;
  struct sfc_mae_pattern_data *pdata = &ctx_mae->pattern_data;
  ...
  sfc_mae_item_build_supp_mask(flocs_udp, RTE_DIM(flocs_udp), &supp_mask,
                               sizeof(supp_mask));

  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           (const void *)&supp_mask, &rte_flow_item_udp_mask,
  ...
  return sfc_mae_parse_item(flocs_udp, RTE_DIM(flocs_udp), spec, mask, ctx_mae,
                            error);
}
...
#define FIELD_ID_NO_REMAP(_field)                                              \
  [EFX_MAE_FIELD_##_field] = EFX_MAE_FIELD_##_field
    ...
    FIELD_ID_NO_REMAP(ETHER_TYPE_BE),  FIELD_ID_NO_REMAP(ETH_SADDR_BE),
    FIELD_ID_NO_REMAP(ETH_DADDR_BE),   FIELD_ID_NO_REMAP(VLAN0_TCI_BE),
    FIELD_ID_NO_REMAP(VLAN0_PROTO_BE), FIELD_ID_NO_REMAP(VLAN1_TCI_BE),
    FIELD_ID_NO_REMAP(VLAN1_PROTO_BE), FIELD_ID_NO_REMAP(SRC_IP4_BE),
    FIELD_ID_NO_REMAP(DST_IP4_BE),     FIELD_ID_NO_REMAP(IP_PROTO),
    FIELD_ID_NO_REMAP(IP_TOS),         FIELD_ID_NO_REMAP(IP_TTL),
    FIELD_ID_NO_REMAP(SRC_IP6_BE),     FIELD_ID_NO_REMAP(DST_IP6_BE),
    FIELD_ID_NO_REMAP(L4_SPORT_BE),    FIELD_ID_NO_REMAP(L4_DPORT_BE),
    FIELD_ID_NO_REMAP(TCP_FLAGS_BE),   FIELD_ID_NO_REMAP(HAS_OVLAN),
...
static int sfc_mae_rule_parse_item_tunnel(const struct rte_flow_item *item,
                                          struct sfc_flow_parse_ctx *ctx,
                                          struct rte_flow_error *error) {
  struct sfc_mae_parse_ctx *ctx_mae = ctx->mae;
  ...
  sfc_mae_item_build_supp_mask(flocs_tunnel, RTE_DIM(flocs_tunnel), &supp_mask,
                               sizeof(supp_mask));
   ...
   */
  rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask,
                           (const void *)&supp_mask, ctx_mae->tunnel_def_mask,
  ...
  rc = efx_mae_match_spec_field_set(
      ctx_mae->match_spec, EFX_MAE_FIELD_ENC_VNET_ID_BE, sizeof(vnet_id_v),
      vnet_id_v, sizeof(vnet_id_m), vnet_id_m);
  if (rc != 0) {
    rc = rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ITEM, item,
                            "Failed to set VXLAN VNI");
  }
...
static int sfc_mae_rule_process_outer(struct sfc_adapter *sa,
  ...
  *rulep =
      sfc_mae_outer_rule_attach(sa, ctx->match_spec_outer, ctx->encap_type);
  if (*rulep != NULL) {
    efx_mae_match_spec_fini(sa->nic, ctx->match_spec_outer);
  } else {
    rc = sfc_mae_outer_rule_add(sa, ctx->match_spec_outer, ctx->encap_type,
                                rulep);
    if (rc != 0) {
      return rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
...
no_or_id:
  ...
  case SFC_FT_RULE_GROUP:
     ...
     */
    rc = efx_mae_match_spec_recirc_id_set(
        ctx->match_spec_action, SFC_FT_ID_TO_TUNNEL_MARK(ctx->ft->id));
    if (rc != 0) {
      return rte_flow_error_set(
          error, rc, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
...
sfc_mae_rule_preparse_item_mark(const struct rte_flow_item_mark *spec,
  ...
  if (ft->refcnt == 0) {
    sfc_err(ctx->sa, "tunnel offload: GROUP: tunnel=%u does not exist", ft->id);
    return ENOENT;
  }
...
static int sfc_mae_rule_encap_parse_init(struct sfc_adapter *sa,
                                         struct sfc_mae_parse_ctx *ctx,
                                         struct rte_flow_error *error) {
  ...
  if (pattern == NULL) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_NUM, NULL,
                       "NULL pattern");
    return -rte_errno;
  }
  ...
  for (;;) {
    ...
    case RTE_FLOW_ITEM_TYPE_MARK:
      rc = sfc_mae_rule_preparse_item_mark(pattern->spec, ctx);
      if (rc != 0) {
        return rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ITEM, pattern,
                                  "tunnel offload: GROUP: invalid item MARK");
      }
      ++pattern;
      continue;
    ...
    case RTE_FLOW_ITEM_TYPE_GENEVE:
      ...
      ctx->tunnel_def_mask_size = sizeof(rte_flow_item_geneve_mask);
      break;
    case RTE_FLOW_ITEM_TYPE_NVGRE:
      ctx->encap_type = EFX_TUNNEL_PROTOCOL_NVGRE;
      ctx->tunnel_def_mask = &rte_flow_item_nvgre_mask;
      ctx->tunnel_def_mask_size = sizeof(rte_flow_item_nvgre_mask);
      break;
  ...
  if ((mae->encap_types_supported & (1U << ctx->encap_type)) == 0) {
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "OR: unsupported tunnel type");
  }
  ...
  case SFC_FT_RULE_NONE:
    ...
    rc = efx_mae_outer_rule_recirc_id_set(ctx->match_spec, recirc_id);
    if (rc != 0) {
      return rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "OR: failed to initialise RECIRC_ID");
    }
    break;
  case SFC_FT_RULE_GROUP:
...
static void sfc_mae_rule_encap_parse_fini(struct sfc_adapter *sa,
                                          struct sfc_mae_parse_ctx *ctx) {
  if (ctx->encap_type == EFX_TUNNEL_PROTOCOL_NONE)
    return;
...
int sfc_mae_rule_parse_pattern(struct sfc_adapter *sa,
  ...
  case SFC_FT_RULE_GROUP:
    if (ctx_mae.priority != 0) {
       ...
       */
      rc = rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY,
                              NULL,
  ...
  if (ctx_mae.match_spec_action != NULL &&
    ...
    rc = rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
...
static int sfc_mae_rule_parse_action_set_mac(
    struct sfc_adapter *sa, enum sfc_mae_mac_addr_type type,
    const struct rte_flow_action_set_mac *conf, struct sfc_mae_aset_ctx *ctx,
    struct rte_flow_error *error) {
  struct sfc_mae_mac_addr **mac_addrp;
  int rc;
  ...
  if (conf == NULL) {
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              NULL, "the MAC address entry definition is NULL");
  }
...
error:
  return rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                            "failed to request set MAC action");
}
...
sfc_mae_actions_bundle_submit(const struct sfc_mae_actions_bundle *bundle,
  ...
  case SFC_MAE_ACTIONS_BUNDLE_VLAN_PUSH:
    rc = efx_mae_action_set_populate_vlan_push(spec, bundle->vlan_push_tpid,
                                               bundle->vlan_push_tci);
    break;
  default:
    SFC_ASSERT(B_FALSE);
    break;
 ...
 */
static int sfc_mae_actions_bundle_sync(const struct rte_flow_action *action,
...
static void sfc_mae_rule_parse_action_of_push_vlan(
    const struct rte_flow_action_of_push_vlan *conf,
    struct sfc_mae_actions_bundle *bundle) {
  bundle->vlan_push_tpid = conf->ethertype;
}

static void sfc_mae_rule_parse_action_of_set_vlan_vid(
    const struct rte_flow_action_of_set_vlan_vid *conf,
    struct sfc_mae_actions_bundle *bundle) {
  bundle->vlan_push_tci |=
      (conf->vlan_vid & rte_cpu_to_be_16(RTE_LEN2MASK(12, uint16_t)));
}

static void sfc_mae_rule_parse_action_of_set_vlan_pcp(
    const struct rte_flow_action_of_set_vlan_pcp *conf,
    struct sfc_mae_actions_bundle *bundle) {
  uint16_t vlan_tci_pcp = (uint16_t)(conf->vlan_pcp & RTE_LEN2MASK(3, uint8_t))
                          << 13;
...
sfc_mae_header_force_item_masks(uint8_t *header_buf,
  ...
  for (item_idx = 0; item_idx < nb_parsed_items; ++item_idx) {
    ...
    for (ofst = 0; ofst < proto_header_size; ofst += sizeof(rte_be16_t)) {
...
static int sfc_mae_rule_parse_action_vxlan_encap(
    struct sfc_mae *mae, const struct rte_flow_action_vxlan_encap *conf,
    efx_mae_actions_t *spec, struct rte_flow_error *error) {
  struct sfc_mae_bounce_eh *bounce_eh = &mae->bounce_eh;
  ...
  struct sfc_mae_parsed_item parsed_items[1 /* Ethernet */ + 2 /* VLAN tags */ +
                                          1 /* IPv4 or IPv6 */ + 1 /* UDP */ +
                                          1 /* VXLAN */];
  ...
  if (pattern == NULL) {
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              NULL, "The encap. header definition is NULL");
  }
  ...
  for (; pattern->type != RTE_FLOW_ITEM_TYPE_END; ++pattern) {
    ...
    const uint64_t exp_items_extra_vlan[] = {RTE_BIT64(RTE_FLOW_ITEM_TYPE_VLAN),
                                             0};
    size_t proto_header_size;
    ...
    if (pattern->spec == NULL) {
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                                NULL, "NULL item spec in the encap. header");
    }

    if (pattern->mask == NULL) {
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                                NULL, "NULL item mask in the encap. header");
    }

    if (pattern->last != NULL) {
      /* This is not a match pattern, so disallow range. */
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                                NULL, "Range item in the encap. header");
    }
    ...
    if ((exp_items & RTE_BIT64(pattern->type)) == 0) {
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                                NULL, "Unexpected item in the encap. header");
    }
    ...
    case RTE_FLOW_ITEM_TYPE_ETH:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ITEM_TYPE_ETH, exp_items);
      RTE_BUILD_BUG_ON(offsetof(struct rte_flow_item_eth, hdr) != 0);
    ...
    case RTE_FLOW_ITEM_TYPE_VLAN:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ITEM_TYPE_VLAN, exp_items);
      RTE_BUILD_BUG_ON(offsetof(struct rte_flow_item_vlan, hdr) != 0);
      ...
      *ethertypep = RTE_BE16(RTE_ETHER_TYPE_VLAN);
      ...
      ethertype_ofst =
          bounce_eh->size + offsetof(struct rte_vlan_hdr, eth_proto);
    ...
    case RTE_FLOW_ITEM_TYPE_IPV4:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ITEM_TYPE_IPV4, exp_items);
      RTE_BUILD_BUG_ON(offsetof(struct rte_flow_item_ipv4, hdr) != 0);
      ...
      *ethertypep = RTE_BE16(RTE_ETHER_TYPE_IPV4);
      ...
      next_proto_ofst =
          bounce_eh->size + offsetof(struct rte_ipv4_hdr, next_proto_id);
    ...
    case RTE_FLOW_ITEM_TYPE_IPV6:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ITEM_TYPE_IPV6, exp_items);
      RTE_BUILD_BUG_ON(offsetof(struct rte_flow_item_ipv6, hdr) != 0);
      ...
      *ethertypep = RTE_BE16(RTE_ETHER_TYPE_IPV6);
      ...
      next_proto_ofst = bounce_eh->size + offsetof(struct rte_ipv6_hdr, proto);
    ...
    case RTE_FLOW_ITEM_TYPE_UDP:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ITEM_TYPE_UDP, exp_items);
      RTE_BUILD_BUG_ON(offsetof(struct rte_flow_item_udp, hdr) != 0);
      ...
      *next_protop = IPPROTO_UDP;
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_VXLAN:
      SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ITEM_TYPE_VXLAN, exp_items);
      RTE_BUILD_BUG_ON(offsetof(struct rte_flow_item_vxlan, hdr) != 0);
      ...
      udp->dgram_len = RTE_BE16(sizeof(*udp) + sizeof(*vxlan));
      udp->dgram_cksum = 0;
    ...
    default:
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                                NULL, "Unknown item in the encap. header");
    }

    if (bounce_eh->size + proto_header_size > bounce_eh->buf_size) {
      return rte_flow_error_set(error, E2BIG, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                                NULL, "The encap. header is too big");
    }

    if ((proto_header_size & 1) != 0) {
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                                NULL, "Odd layer size in the encap. header");
    }
  ...
  if (exp_items != 0) {
    ...
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              NULL, "No item VXLAN in the encap. header");
  }

  /* One of the pointers (ipv4, ipv6) refers to a dummy area. */
  ipv4->version_ihl = RTE_IPV4_VHL_DEF;
  ipv4->time_to_live = SFC_IPV4_TTL_DEF;
  ipv4->total_length = RTE_BE16(sizeof(*ipv4) + sizeof(*udp) + sizeof(*vxlan));
  ...
  if (rc != 0) {
    rc = rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                            "failed to request action ENCAP");
  }

  return rc;
}

static int sfc_mae_rule_parse_action_mark(
    struct sfc_adapter *sa, const struct rte_flow_action_mark *conf,
    const struct sfc_flow_spec_mae *spec_mae, efx_mae_actions_t *spec) {
  int rc;
...
static int sfc_mae_rule_parse_action_count(
    struct sfc_adapter *sa,
    const struct rte_flow_action_count *conf __rte_unused,
    efx_mae_actions_t *spec) {
  int rc;

  if ((sa->counter_rxq.state & SFC_COUNTER_RXQ_INITIALIZED) == 0) {
    sfc_err(sa, "counter queue is not configured for COUNT action");
    rc = EINVAL;
  ...
  if (rc != 0) {
    sfc_err(sa, "failed to populate counters in MAE action set: %s",
            rte_strerror(rc));
...
sfc_mae_rule_parse_action_phy_port(struct sfc_adapter *sa,
  ...
  if (rc != 0) {
    sfc_err(sa,
            "failed to request action DELIVER with m-port selector 0x%08x: %s",
...
sfc_mae_rule_parse_action_pf_vf(struct sfc_adapter *sa,
  ...
  if (rc != 0) {
    sfc_err(sa, "failed to convert PF %u VF %d to m-port: %s", encp->enc_pf,
            (vf != EFX_PCI_VF_INVALID) ? (int)vf : -1, strerror(rc));
    return rc;
  }
...
sfc_mae_rule_parse_action_port_id(struct sfc_adapter *sa,
  ...
  rc = sfc_mae_switch_get_ethdev_mport(mae->switch_domain_id, port_id, &mport);
  if (rc != 0) {
...
static int sfc_mae_rule_parse_action_port_representor(
    struct sfc_adapter *sa, const struct rte_flow_action_ethdev *conf,
    efx_mae_actions_t *spec) {
  struct sfc_mae *mae = &sa->mae;
  efx_mport_sel_t mport;
  int rc;

  rc = sfc_mae_switch_get_ethdev_mport(mae->switch_domain_id, conf->port_id,
                                       &mport);
  if (rc != 0) {
    sfc_err(sa, "failed to get m-port for the given ethdev (port_id=%u): %s",
...
static int sfc_mae_rule_parse_action_represented_port(
    struct sfc_adapter *sa, const struct rte_flow_action_ethdev *conf,
    efx_mae_actions_t *spec) {
  struct sfc_mae *mae = &sa->mae;
  efx_mport_sel_t mport;
  ...
  rc = sfc_mae_switch_get_entity_mport(mae->switch_domain_id, conf->port_id,
                                       &mport);
  if (rc != 0) {
    sfc_err(sa, "failed to get m-port for the given ethdev (port_id=%u): %s",
            conf->port_id, strerror(rc));
...
static int sfc_mae_rule_parse_action(struct sfc_adapter *sa,
                                     const struct rte_flow_action *action,
  ...
  case RTE_FLOW_ACTION_TYPE_DEC_TTL:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_OF_DEC_NW_TTL,
                           bundle->actions_mask);
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_DEC_TTL, bundle->actions_mask);
  ...
  case RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP,
                           bundle->actions_mask);
    rc = sfc_mae_rule_parse_action_vxlan_encap(&sa->mae, action->conf, spec,
                                               error);
    custom_error = B_TRUE;
    break;
  case RTE_FLOW_ACTION_TYPE_COUNT:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_COUNT, bundle->actions_mask);
    rc = sfc_mae_rule_parse_action_count(sa, action->conf, spec);
    break;
  case RTE_FLOW_ACTION_TYPE_FLAG:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_FLAG, bundle->actions_mask);
    if ((rx_metadata & RTE_ETH_RX_METADATA_USER_FLAG) != 0) {
      rc = efx_mae_action_set_populate_flag(spec);
    } else {
      rc = rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                              action, "flag delivery has not been negotiated");
      custom_error = B_TRUE;
    }
    break;
  case RTE_FLOW_ACTION_TYPE_MARK:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_MARK, bundle->actions_mask);
    if ((rx_metadata & RTE_ETH_RX_METADATA_USER_MARK) != 0 ||
        spec_mae->ft_rule_type == SFC_FT_RULE_JUMP) {
      rc = sfc_mae_rule_parse_action_mark(sa, action->conf, spec_mae, spec);
    } else {
      rc = rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                              action, "mark delivery has not been negotiated");
      custom_error = B_TRUE;
    }
    break;
  case RTE_FLOW_ACTION_TYPE_PHY_PORT:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_PHY_PORT, bundle->actions_mask);
    rc = sfc_mae_rule_parse_action_phy_port(sa, action->conf, spec);
    break;
  case RTE_FLOW_ACTION_TYPE_PF:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_PF, bundle->actions_mask);
    rc = sfc_mae_rule_parse_action_pf_vf(sa, NULL, spec);
    break;
  case RTE_FLOW_ACTION_TYPE_VF:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_VF, bundle->actions_mask);
    rc = sfc_mae_rule_parse_action_pf_vf(sa, action->conf, spec);
    break;
  case RTE_FLOW_ACTION_TYPE_PORT_ID:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_PORT_ID, bundle->actions_mask);
  ...
  case RTE_FLOW_ACTION_TYPE_PORT_REPRESENTOR:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_PORT_REPRESENTOR,
                           bundle->actions_mask);
    rc = sfc_mae_rule_parse_action_port_representor(sa, action->conf, spec);
    break;
  case RTE_FLOW_ACTION_TYPE_REPRESENTED_PORT:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_REPRESENTED_PORT,
                           bundle->actions_mask);
    rc = sfc_mae_rule_parse_action_represented_port(sa, action->conf, spec);
    break;
  case RTE_FLOW_ACTION_TYPE_DROP:
    SFC_BUILD_SET_OVERFLOW(RTE_FLOW_ACTION_TYPE_DROP, bundle->actions_mask);
  ...
  if (rc == 0) {
    ...
    if (action->type < RTE_DIM(action_names)) {
      ...
      if (action_name != NULL) {
        sfc_err(sa, "action %s was rejected: %s", action_name, strerror(rc));
      }
    }
    rc = rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
...
int sfc_mae_rule_parse_actions(struct sfc_adapter *sa,
  ...
  if (spec_mae->ft_rule_type == SFC_FT_RULE_GROUP) {
    ...
    if (ctx.n_counters == 0 && sfc_mae_counter_stream_enabled(sa)) {
      /*
  ...
  for (action = actions; action->type != RTE_FLOW_ACTION_TYPE_END; ++action) {
    ...
    rc = sfc_mae_rule_parse_action(sa, action, spec_mae, &bundle, &ctx, error);
    if (rc != 0)
      goto fail_rule_parse_action;
  }
  ...
  rc = sfc_mae_process_encap_header(sa, &mae->bounce_eh, &ctx.encap_header);
  if (rc != 0)
    goto fail_process_encap_header;

  if (ctx.n_counters > 1) {
    rc = ENOTSUP;
    sfc_err(sa, "too many count actions requested: %u", ctx.n_counters);
    goto fail_nb_count;
  }
...
fail_action_set_spec_init:
  if (rc > 0 && rte_errno == 0) {
    rc = rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                            "Failed to process the action");
  }
  return rc;
}

static bool sfc_mae_rules_class_cmp(struct sfc_adapter *sa,
                                    const efx_mae_match_spec_t *left,
                                    const efx_mae_match_spec_t *right) {
  bool have_same_class;
  int rc;

  rc = efx_mae_match_specs_class_cmp(sa->nic, left, right, &have_same_class);

  return (rc == 0) ? have_same_class : false;
}

static int sfc_mae_outer_rule_class_verify(struct sfc_adapter *sa,
                                           struct sfc_mae_outer_rule *rule) {
  struct sfc_mae_fw_rsrc *fw_rsrc = &rule->fw_rsrc;
  ...
  TAILQ_FOREACH_REVERSE(entry, &mae->outer_rules, sfc_mae_outer_rules,
                        entries) {
...
static int sfc_mae_action_rule_class_verify(struct sfc_adapter *sa,
                                            struct sfc_flow_spec_mae *spec) {
 ...
 */
int sfc_mae_flow_verify(struct sfc_adapter *sa, struct rte_flow *flow) {
...
int sfc_mae_flow_insert(struct sfc_adapter *sa, struct rte_flow *flow) {
  struct sfc_flow_spec *spec = &flow->spec;
  struct sfc_flow_spec_mae *spec_mae = &spec->mae;
  struct sfc_mae_outer_rule *outer_rule = spec_mae->outer_rule;
  ...
  if (outer_rule != NULL) {
    rc = sfc_mae_outer_rule_enable(sa, outer_rule, spec_mae->match_spec);
    if (rc != 0)
      goto fail_outer_rule_enable;
  }

  if (spec_mae->ft_rule_type == SFC_FT_RULE_JUMP) {
    spec_mae->ft->reset_jump_hit_counter = spec_mae->ft->group_hit_counter;
  }
  ...
  if (action_set->n_counters > 0) {
    ...
    if (rc != 0) {
      sfc_err(sa, "failed to start MAE counters support: %s", rte_strerror(rc));
  ...
  rc = efx_mae_action_rule_insert(sa->nic, spec_mae->match_spec, NULL,
                                  &fw_rsrc->aset_id, &spec_mae->rule_id);
  if (rc != 0)
    goto fail_action_rule_insert;

  sfc_dbg(sa, "enabled flow=%p: AR_ID=0x%08x", flow, spec_mae->rule_id.id);
...
int sfc_mae_flow_remove(struct sfc_adapter *sa, struct rte_flow *flow) {
  struct sfc_flow_spec *spec = &flow->spec;
  struct sfc_flow_spec_mae *spec_mae = &spec->mae;
  struct sfc_mae_action_set *action_set = spec_mae->action_set;
  struct sfc_mae_outer_rule *outer_rule = spec_mae->outer_rule;
  ...
  if (rc != 0) {
    sfc_err(sa, "failed to disable flow=%p with AR_ID=0x%08x: %s", flow,
            spec_mae->rule_id.id, strerror(rc));
  }
  sfc_dbg(sa, "disabled flow=%p with AR_ID=0x%08x", flow, spec_mae->rule_id.id);
...
static int sfc_mae_query_counter(struct sfc_adapter *sa,
                                 struct sfc_flow_spec_mae *spec,
                                 const struct rte_flow_action *action,
  ...
  return rte_flow_error_set(error, ENOENT, RTE_FLOW_ERROR_TYPE_ACTION, action,
                            "no such flow rule action or such count ID");
}

int sfc_mae_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
...
int sfc_mae_switchdev_init(struct sfc_adapter *sa) {
  ...
  rc = efx_mae_mport_by_pcie_function(encp->enc_pf, EFX_PCI_VF_INVALID, &pf);
  if (rc != 0) {
  ...
  rc = sfc_mae_rule_add_mport_match_deliver(
      sa, &pf, &phy, SFC_MAE_RULE_PRIO_LOWEST, &mae->switchdev_rule_pf_to_ext);
  if (rc != 0) {
  ...
  rc = sfc_mae_rule_add_mport_match_deliver(
      sa, &phy, &pf, SFC_MAE_RULE_PRIO_LOWEST, &mae->switchdev_rule_ext_to_pf);
  if (rc != 0) {
