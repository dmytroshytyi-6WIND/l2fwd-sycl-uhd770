...
void sfc_kvargs_cleanup(struct sfc_adapter *sa) { rte_kvargs_free(sa->kvargs); }

static int sfc_kvarg_match_value(const char *value, const char *const *values,
...
int sfc_kvarg_bool_handler(__rte_unused const char *key, const char *value_str,
                           void *opaque) {
  const char *const true_strs[] = {"1", "y", "yes", "on", "true"};
  const char *const false_strs[] = {"0", "n", "no", "off", "false"};
  bool *value = opaque;

  if (sfc_kvarg_match_value(value_str, true_strs, RTE_DIM(true_strs)))
    *value = true;
  else if (sfc_kvarg_match_value(value_str, false_strs, RTE_DIM(false_strs)))
    *value = false;
  else
    return -EINVAL;
...
int sfc_kvarg_long_handler(__rte_unused const char *key, const char *value_str,
                           void *opaque) {
  long value;
  char *endptr;
...
int sfc_kvarg_string_handler(__rte_unused const char *key,
