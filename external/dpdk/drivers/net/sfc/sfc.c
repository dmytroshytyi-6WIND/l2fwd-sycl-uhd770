...
int sfc_dma_alloc(struct sfc_adapter *sa, const char *name, uint16_t id,
  ...
  sfc_log_init(sa, "name=%s id=%u len=%zu socket_id=%d", name, id, len,
               socket_id);

  mz = rte_eth_dma_zone_reserve(sa->eth_dev, name, id, len,
                                sysconf(_SC_PAGESIZE), socket_id);
  if (mz == NULL) {
    sfc_err(sa, "cannot reserve DMA zone for %s:%u %#x@%d: %s", name,
            (unsigned int)id, (unsigned int)len, socket_id,
  ...
  sfc_info(sa, "DMA name=%s id=%u len=%lu socket_id=%d => virt=%p iova=%lx",
...
static uint32_t sfc_phy_cap_from_link_speeds(uint32_t speeds) {
  uint32_t phy_caps = 0;
  ...
  if (~speeds & RTE_ETH_LINK_SPEED_FIXED) {
    ...
    if (speeds == RTE_ETH_LINK_SPEED_AUTONEG)
      phy_caps |= (1 << EFX_PHY_CAP_1000FDX) | (1 << EFX_PHY_CAP_10000FDX) |
                  (1 << EFX_PHY_CAP_25000FDX) | (1 << EFX_PHY_CAP_40000FDX) |
                  (1 << EFX_PHY_CAP_50000FDX) | (1 << EFX_PHY_CAP_100000FDX);
  }
 ...
 * modules.
 */
static int sfc_check_conf(struct sfc_adapter *sa) {
  ...
  if ((conf->intr_conf.lsc != 0) && (sa->intr.type != EFX_INTR_LINE) &&
 ...
 */
static int sfc_estimate_resource_limits(struct sfc_adapter *sa) {
  ...
  if (sa->tso && encp->enc_fw_assisted_tso_v2_enabled)
    limits.edl_max_txq_count =
        MIN(limits.edl_max_txq_count,
            encp->enc_fw_assisted_tso_v2_n_contexts / encp->enc_hw_pf_count);
  ...
  if (sfc_mae_counter_rxq_required(sa) && rxq_allocated > 0 &&
      evq_allocated > 0) {
    rxq_allocated--;
    evq_allocated--;
  ...
  if (sfc_repr_supported(sa) &&
      evq_allocated >= SFC_REPR_PROXY_NB_RXQ_MIN + SFC_REPR_PROXY_NB_TXQ_MIN &&
    ...
    extra = MIN(extra, SFC_REPR_PROXY_NB_RXQ_MAX - sas->nb_repr_rxq);
    evq_allocated -= extra;
    rxq_allocated -= extra;
    ...
    extra = MIN(extra, SFC_REPR_PROXY_NB_TXQ_MAX - sas->nb_repr_txq);
    evq_allocated -= extra;
    txq_allocated -= extra;
...
static int sfc_set_drv_limits(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *sas = sfc_sa2shared(sa);
  ...
  lim.edl_min_evq_count = lim.edl_max_evq_count =
      1 + data->nb_rx_queues + data->nb_tx_queues + rxq_reserved + txq_reserved;
...
static int sfc_set_fw_subvariant(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *sas = sfc_sa2shared(sa);
  const efx_nic_cfg_t *encp = efx_nic_cfg_get(sa->nic);
  ...
  if (tx_offloads &
      (RTE_ETH_TX_OFFLOAD_IPV4_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_CKSUM |
       RTE_ETH_TX_OFFLOAD_UDP_CKSUM | RTE_ETH_TX_OFFLOAD_OUTER_IPV4_CKSUM))
  ...
  sfc_info(sa, "FW subvariant is %u vs required %u", cur_fw_subvariant,
           req_fw_subvariant);
  ...
  if (rc != 0) {
    sfc_err(sa, "failed to set FW subvariant %u: %d", req_fw_subvariant, rc);
    return rc;
  }
...
static int sfc_try_start(struct sfc_adapter *sa) {
   ...
   */
  sa->priv.shared->tunnel_encaps = encp->enc_tunnel_encapsulations_supported;
...
static int sfc_restart(struct sfc_adapter *sa) {
  int rc;
...
static void sfc_restart_if_required(void *arg) {
  struct sfc_adapter *sa = arg;

  /* If restart is scheduled, clear the flag and do it */
  if (rte_atomic32_cmpset((volatile uint32_t *)&sa->restart_required, 1, 0)) {
    sfc_adapter_lock(sa);
    if (sa->state == SFC_ETHDEV_STARTED)
      (void)sfc_restart(sa);
...
static int sfc_mem_bar_init(struct sfc_adapter *sa,
...
static void sfc_mem_bar_fini(struct sfc_adapter *sa) {
  efsys_bar_t *ebp = &sa->mem_bar;
...
static int sfc_rss_attach(struct sfc_adapter *sa) {
...
static void sfc_rss_detach(struct sfc_adapter *sa) { sfc_rx_hash_fini(sa); }
...
int sfc_attach(struct sfc_adapter *sa) {
  ...
  if (sfc_dp_tx_offload_capa(sa->priv.dp_tx) & RTE_ETH_TX_OFFLOAD_TCP_TSO) {
    sa->tso = encp->enc_fw_assisted_tso_v2_enabled || encp->enc_tso_v3_enabled;
    if (!sa->tso)
  ...
  if (sa->tso && (sfc_dp_tx_offload_capa(sa->priv.dp_tx) &
    ...
    sa->tso_encap =
        encp->enc_fw_assisted_tso_v2_encap_enabled || encp->enc_tso_v3_enabled;
    if (!sa->tso_encap)
...
static int sfc_kvarg_fv_variant_handler(__rte_unused const char *key,
...
static int sfc_get_fw_variant(struct sfc_adapter *sa, efx_fw_variant_t *efv) {
...
static const char *sfc_fw_variant2str(efx_fw_variant_t efv) {
  switch (efv) {
  case EFX_RXDP_FULL_FEATURED_FW_ID:
...
static int sfc_kvarg_rxd_wait_timeout_ns(struct sfc_adapter *sa) {
  int rc;
  long value;
  ...
  if (value < 0 ||
    ...
    sfc_err(sa,
            "wrong '" SFC_KVARG_RXD_WAIT_TIMEOUT_NS "' "
            "was set (%ld);",
            value);
...
static int sfc_nic_probe(struct sfc_adapter *sa) {
  efx_nic_t *enp = sa->nic;
  ...
  rc = sfc_kvargs_process(sa, SFC_KVARG_FW_VARIANT,
                          sfc_kvarg_fv_variant_handler, &preferred_efv);
  if (rc != 0) {
...
int sfc_probe(struct sfc_adapter *sa) {
  ...
  if (rc != 0)
    goto fail_family;
  sfc_log_init(
      sa, "family is %u, membar is %u, function control window offset is %lu",
  ...
  rc = efx_nic_create(sa->family, (efsys_identifier_t *)sa, &sa->mem_bar,
                      mem_ebrp.ebr_offset, &sa->nic_lock, &enp);
  if (rc != 0)
...
void sfc_unprobe(struct sfc_adapter *sa) {
  ...
  sfc_mae_clear_switch_port(sa->mae.switch_domain_id, sa->mae.switch_port_id);
...
void sfc_hw_switch_id_fini(__rte_unused struct sfc_adapter *sa,
                           struct sfc_hw_switch_id *id) {
  rte_free(id);
}
...
bool sfc_hw_switch_ids_equal(const struct sfc_hw_switch_id *left,
  ...
  return strncmp(left->board_sn, right->board_sn, sizeof(left->board_sn)) == 0;
}
