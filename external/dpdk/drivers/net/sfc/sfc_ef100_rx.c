...
#define sfc_ef100_rx_debug(_rxq, ...)                                          \
  SFC_DP_LOG(SFC_KVARG_DATAPATH_EF100, DEBUG, &(_rxq)->dp.dpq, __VA_ARGS__)
...
static inline void sfc_ef100_rx_qpush(struct sfc_ef100_rxq *rxq,
  ...
  sfc_ef100_rx_debug(rxq, "RxQ pushed doorbell at pidx %u (added=%u)",
                     EFX_DWORD_FIELD(dword, ERF_GZ_RX_RING_PIDX), added);
}

static void sfc_ef100_rx_qrefill(struct sfc_ef100_rxq *rxq) {
  ...
  do {
    ...
    for (i = 0; i < RTE_DIM(objs); ++i) {
      ...
      if (rxq->flags & SFC_EF100_RXQ_NIC_DMA_MAP) {
        dma_addr = sfc_nic_dma_map(rxq->nic_dma_info, dma_addr,
...
sfc_ef100_rx_nt_or_inner_l4_csum(const efx_word_t class) {
  return EFX_WORD_FIELD(class, ESF_GZ_RX_PREFIX_HCLASS_NT_OR_INNER_L4_CSUM) ==
                 ESE_GZ_RH_HCLASS_L4_CSUM_GOOD
             ? RTE_MBUF_F_RX_L4_CKSUM_GOOD
             : RTE_MBUF_F_RX_L4_CKSUM_BAD;
}

static inline uint64_t sfc_ef100_rx_tun_outer_l4_csum(const efx_word_t class) {
  return EFX_WORD_FIELD(class, ESF_GZ_RX_PREFIX_HCLASS_TUN_OUTER_L4_CSUM) ==
                 ESE_GZ_RH_HCLASS_L4_CSUM_GOOD
             ? RTE_MBUF_F_RX_OUTER_L4_CKSUM_GOOD
             : RTE_MBUF_F_RX_OUTER_L4_CKSUM_BAD;
}

static uint32_t sfc_ef100_rx_class_decode(const efx_word_t class,
                                          uint64_t *ol_flags) {
  uint32_t ptype;
  ...
  if (no_tunnel) {
    ...
    switch (
        EFX_WORD_FIELD(class, ESF_GZ_RX_PREFIX_HCLASS_NT_OR_INNER_L3_CLASS)) {
    ...
    if (l4_valid) {
      switch (
          EFX_WORD_FIELD(class, ESF_GZ_RX_PREFIX_HCLASS_NT_OR_INNER_L4_CLASS)) {
    ...
    switch (EFX_WORD_FIELD(class, ESF_GZ_RX_PREFIX_HCLASS_TUN_OUTER_L3_CLASS)) {
    case ESE_GZ_RH_HCLASS_L3_CLASS_IP4GOOD:
      ptype |= RTE_PTYPE_L3_IPV4_EXT_UNKNOWN;
      break;
    case ESE_GZ_RH_HCLASS_L3_CLASS_IP4BAD:
...
#define SFC_EF100_RX_PREFIX_FIELD(_name, _big_endian)                          \
  EFX_RX_PREFIX_FIELD(_name, ESF_GZ_RX_PREFIX_##_name, _big_endian)
        ...
        EFX_RX_PREFIX_FIELD(INGRESS_MPORT, ESF_GZ_RX_PREFIX_INGRESS_MPORT,
                            B_FALSE),
...
static bool sfc_ef100_rx_prefix_to_offloads(const struct sfc_ef100_rxq *rxq,
  ...
  if (unlikely(EFX_WORD_FIELD(*class, ESF_GZ_RX_PREFIX_HCLASS_L2_STATUS) !=
  ...
  if ((rxq->flags & SFC_EF100_RXQ_RSS_HASH) &&
    ...
    m->hash.rss = EFX_XWORD_FIELD(rx_prefix[0], ESF_GZ_RX_PREFIX_RSS_HASH);
  }

  if (rxq->flags & SFC_EF100_RXQ_USER_FLAG) {
    uint32_t user_flag;

    user_flag = EFX_XWORD_FIELD(rx_prefix[0], ESF_GZ_RX_PREFIX_USER_FLAG);
    if (user_flag != 0)
  ...
  if (rxq->flags & SFC_EF100_RXQ_USER_MARK) {
    ...
    /* EFX_XWORD_FIELD converts little-endian to CPU */
    mark = EFX_XWORD_FIELD(rx_prefix[0], ESF_GZ_RX_PREFIX_USER_MARK);
    ...
    if (tunnel_mark != SFC_FT_TUNNEL_MARK_INVALID) {
      ...
      *RTE_MBUF_DYNFIELD(m, sfc_dp_ft_id_offset, sfc_ft_id_t *) = ft_id;
    }
  }
  ...
  if (rxq->flags & SFC_EF100_RXQ_INGRESS_MPORT) {
    ...
    *RTE_MBUF_DYNFIELD(m, sfc_dp_mport_offset,
        ...
        EFX_XWORD_FIELD(rx_prefix[0], ESF_GZ_RX_PREFIX_INGRESS_MPORT);
  }
...
static const uint8_t *sfc_ef100_rx_pkt_prefix(const struct rte_mbuf *m) {
  return (const uint8_t *)m->buf_addr + RTE_PKTMBUF_HEADROOM;
}

static struct rte_mbuf *sfc_ef100_rx_next_mbuf(struct sfc_ef100_rxq *rxq) {
  struct rte_mbuf *m;
  unsigned int id;
...
sfc_ef100_rx_process_ready_pkts(struct sfc_ef100_rxq *rxq,
  ...
  while (rxq->ready_pkts > 0 && rx_pkts != rx_pkts_end) {
    ...
    RTE_BUILD_BUG_ON(sizeof(pkt->rearm_data[0]) != sizeof(rxq->rearm_data));
    ...
    pkt_len = EFX_XWORD_FIELD(rx_prefix[0], ESF_GZ_RX_PREFIX_LENGTH);
    ...
    if (likely(deliver)) {
      *rx_pkts++ = pkt;
      sfc_pkts_bytes_add(&rxq->dp.dpq.stats, 1, rte_pktmbuf_pkt_len(pkt));
    } else {
...
static bool sfc_ef100_rx_get_event(struct sfc_ef100_rxq *rxq, efx_qword_t *ev) {
  *ev = rxq->evq_hw_ring[rxq->evq_read_ptr & rxq->ptr_mask];

  if (!sfc_ef100_ev_present(
          ev, (rxq->evq_read_ptr >> rxq->evq_phase_bit_shift) & 1))
    return false;
  ...
  if (unlikely(!sfc_ef100_ev_type_is(ev, ESE_GZ_EF100_EV_RX_PKTS))) {
    ...
    sfc_ef100_rx_err(rxq, "RxQ exception at EvQ ptr %u(%#x), event %08x:%08x",
  ...
  sfc_ef100_rx_debug(rxq, "RxQ got event %08x:%08x at %u (%#x)",
                     EFX_QWORD_FIELD(*ev, EFX_DWORD_1),
                     EFX_QWORD_FIELD(*ev, EFX_DWORD_0), rxq->evq_read_ptr,
                     rxq->evq_read_ptr & rxq->ptr_mask);
...
static uint16_t sfc_ef100_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                                    uint16_t nb_pkts) {
  ...
  while (rx_pkts != rx_pkts_end && sfc_ef100_rx_get_event(rxq, &rx_ev)) {
    rxq->ready_pkts = EFX_QWORD_FIELD(rx_ev, ESF_GZ_EV_RXPKTS_NUM_PKT);
    rx_pkts = sfc_ef100_rx_process_ready_pkts(rxq, rx_pkts, rx_pkts_end);
  }
...
static int sfc_ef100_rx_qdesc_status(__rte_unused struct sfc_dp_rxq *dp_rxq,
                                     __rte_unused uint16_t offset) {
  return -ENOTSUP;
}

static sfc_dp_rx_get_dev_info_t sfc_ef100_rx_get_dev_info;
static void sfc_ef100_rx_get_dev_info(struct rte_eth_dev_info *dev_info) {
  /*
...
static int sfc_ef100_rx_qsize_up_rings(uint16_t nb_rx_desc,
  ...
  *rxq_max_fill_level = RTE_MIN(nb_rx_desc, SFC_EF100_RXQ_LIMIT(*evq_entries));
  return 0;
}

static uint64_t sfc_ef100_mk_mbuf_rearm_data(uint16_t port_id,
                                             uint16_t prefix_size) {
  struct rte_mbuf m;
...
static int sfc_ef100_rx_qcreate(uint16_t port_id, uint16_t queue_id,
  ...
  rc = ENOMEM;
  rxq = rte_zmalloc_socket("sfc-ef100-rxq", sizeof(*rxq), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (rxq == NULL)
    goto fail_rxq_alloc;
  ...
  rc = ENOMEM;
  rxq->sw_ring =
      rte_calloc_socket("sfc-ef100-rxq-sw_ring", info->rxq_entries,
                        sizeof(*rxq->sw_ring), RTE_CACHE_LINE_SIZE, socket_id);
  if (rxq->sw_ring == NULL)
  ...
  rxq->evq_prime = (volatile uint8_t *)info->mem_bar + info->fcw_offset +
...
static void sfc_ef100_rx_qdestroy(struct sfc_dp_rxq *dp_rxq) {
...
static int sfc_ef100_rx_qstart(struct sfc_dp_rxq *dp_rxq,
                               unsigned int evq_read_ptr,
  ...
  if ((unsup_rx_prefix_fields & ((1U << EFX_RX_PREFIX_FIELD_LENGTH) |
                                 (1U << EFX_RX_PREFIX_FIELD_CLASS))) != 0)
    return ENOTSUP;

  if ((unsup_rx_prefix_fields & ((1U << EFX_RX_PREFIX_FIELD_RSS_HASH_VALID) |
                                 (1U << EFX_RX_PREFIX_FIELD_RSS_HASH))) == 0)
    rxq->flags |= SFC_EF100_RXQ_RSS_HASH;
  else
    rxq->flags &= ~SFC_EF100_RXQ_RSS_HASH;

  if ((unsup_rx_prefix_fields & (1U << EFX_RX_PREFIX_FIELD_USER_FLAG)) == 0)
    rxq->flags |= SFC_EF100_RXQ_USER_FLAG;
  else
    rxq->flags &= ~SFC_EF100_RXQ_USER_FLAG;

  if ((unsup_rx_prefix_fields & (1U << EFX_RX_PREFIX_FIELD_USER_MARK)) == 0)
    rxq->flags |= SFC_EF100_RXQ_USER_MARK;
  else
    rxq->flags &= ~SFC_EF100_RXQ_USER_MARK;

  if ((unsup_rx_prefix_fields & (1U << EFX_RX_PREFIX_FIELD_INGRESS_MPORT)) == 0)
  ...
  rxq->rearm_data =
      sfc_ef100_mk_mbuf_rearm_data(rxq->dp.dpq.port_id, rxq->prefix_size);
...
static void sfc_ef100_rx_qstop(struct sfc_dp_rxq *dp_rxq,
                               unsigned int *evq_read_ptr) {
  struct sfc_ef100_rxq *rxq = sfc_ef100_rxq_by_dp_rxq(dp_rxq);
...
static bool sfc_ef100_rx_qrx_ev(struct sfc_dp_rxq *dp_rxq,
...
static void sfc_ef100_rx_qpurge(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef100_rxq *rxq = sfc_ef100_rxq_by_dp_rxq(dp_rxq);
  unsigned int i;
  struct sfc_ef100_rx_sw_desc *rxd;
...
static int sfc_ef100_rx_intr_enable(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef100_rxq *rxq = sfc_ef100_rxq_by_dp_rxq(dp_rxq);
...
static int sfc_ef100_rx_intr_disable(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef100_rxq *rxq = sfc_ef100_rxq_by_dp_rxq(dp_rxq);
...
static unsigned int sfc_ef100_rx_get_pushed(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef100_rxq *rxq = sfc_ef100_rxq_by_dp_rxq(dp_rxq);
...
struct sfc_dp_rx sfc_ef100_rx = {
    ...
    .features = SFC_DP_RX_FEAT_MULTI_PROCESS | SFC_DP_RX_FEAT_FLOW_FLAG |
                SFC_DP_RX_FEAT_FLOW_MARK | SFC_DP_RX_FEAT_INTR |
                SFC_DP_RX_FEAT_STATS,
    .dev_offload_capa = 0,
    .queue_offload_capa =
        RTE_ETH_RX_OFFLOAD_CHECKSUM | RTE_ETH_RX_OFFLOAD_OUTER_IPV4_CKSUM |
