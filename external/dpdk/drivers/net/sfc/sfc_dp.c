...
void sfc_dp_queue_init(struct sfc_dp_queue *dpq, uint16_t port_id,
                       uint16_t queue_id, const struct rte_pci_addr *pci_addr) {
  dpq->port_id = port_id;
...
int sfc_dp_register(struct sfc_dp_list *head, struct sfc_dp *entry) {
  if (sfc_dp_find_by_name(head, entry->type, entry->name) != NULL) {
    SFC_GENERIC_LOG(ERR, "sfc %s datapath '%s' already registered",
...
int sfc_dp_mport_register(void) {
  ...
  if (field_offset < 0) {
    SFC_GENERIC_LOG(ERR, "%s() failed to register mport dynfield", __func__);
    return -1;
  }

  flag = rte_mbuf_dynflag_register(&mport_override);
  if (flag < 0) {
    SFC_GENERIC_LOG(ERR, "%s() failed to register mport dynflag", __func__);
    return -1;
  }
...
int sfc_dp_ft_id_register(void) {
  ...
  if (field_offset < 0) {
    SFC_GENERIC_LOG(ERR, "%s() failed to register ft_id dynfield", __func__);
    return -1;
  }

  flag = rte_mbuf_dynflag_register(&ft_id_valid);
  if (flag < 0) {
    SFC_GENERIC_LOG(ERR, "%s() failed to register ft_id dynflag", __func__);
    return -1;
  }
