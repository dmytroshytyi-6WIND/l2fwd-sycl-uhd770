...
#define sfc_ef100_tx_debug(_txq, ...)                                          \
  SFC_DP_LOG(SFC_KVARG_DATAPATH_EF100, DEBUG, &(_txq)->dp.dpq, __VA_ARGS__)

/** Maximum length of the send descriptor data */
#define SFC_EF100_TX_SEND_DESC_LEN_MAX ((1u << ESF_GZ_TX_SEND_LEN_WIDTH) - 1)

/** Maximum length of the segment descriptor data */
#define SFC_EF100_TX_SEG_DESC_LEN_MAX ((1u << ESF_GZ_TX_SEG_LEN_WIDTH) - 1)
...
#define SFC_EF100_TXQ_LIMIT(_ndesc)                                            \
  ((_ndesc)-1 /* head must not step on tail */ - 1 /* Rx error */ -            \
   1 /* flush */)
...
static int sfc_ef100_tx_prepare_pkt_tso(struct sfc_ef100_txq *const txq,
                                        struct rte_mbuf *m) {
...
static uint16_t sfc_ef100_tx_prepare_pkts(void *tx_queue,
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (calc_phdr_cksum) {
       ...
       */
      ret = rte_net_intel_cksum_flags_prepare(m, m->ol_flags &
...
static bool sfc_ef100_tx_get_event(struct sfc_ef100_txq *txq, efx_qword_t *ev) {
  ...
  *ev = evq_hw_ring[txq->evq_read_ptr & txq->ptr_mask];
  ...
  if (!sfc_ef100_ev_present(
          ev, (txq->evq_read_ptr >> txq->evq_phase_bit_shift) & 1))
    return false;

  if (unlikely(!sfc_ef100_ev_type_is(ev, ESE_GZ_EF100_EV_TX_COMPLETION))) {
    /*
     * Do not move read_ptr to keep the event for exception
     * handling by the control path.
     */
    txq->flags |= SFC_EF100_TXQ_EXCEPTION;
    sfc_ef100_tx_err(txq, "TxQ exception at EvQ ptr %u(%#x), event %08x:%08x",
  ...
  sfc_ef100_tx_debug(txq, "TxQ got event %08x:%08x at %u (%#x)",
                     EFX_QWORD_FIELD(*ev, EFX_DWORD_1),
                     EFX_QWORD_FIELD(*ev, EFX_DWORD_0), txq->evq_read_ptr,
...
static unsigned int sfc_ef100_tx_process_events(struct sfc_ef100_txq *txq) {
...
static void sfc_ef100_tx_reap_num_descs(struct sfc_ef100_txq *txq,
  ...
  if (num_descs > 0) {
    ...
    do {
      ...
      if (nb == RTE_DIM(bulk) || (nb != 0 && m->pool != bulk[0]->pool)) {
...
static void sfc_ef100_tx_reap(struct sfc_ef100_txq *txq) {
  sfc_ef100_tx_reap_num_descs(txq, sfc_ef100_tx_process_events(txq));
}

static void sfc_ef100_tx_qdesc_prefix_create(const struct rte_mbuf *m,
                                             efx_oword_t *tx_desc) {
  efx_mport_id_t *mport_id =
      RTE_MBUF_DYNFIELD(m, sfc_dp_mport_offset, efx_mport_id_t *);

  EFX_POPULATE_OWORD_3(*tx_desc, ESF_GZ_TX_PREFIX_EGRESS_MPORT, mport_id->id,
                       ESF_GZ_TX_PREFIX_EGRESS_MPORT_EN, 1, ESF_GZ_TX_DESC_TYPE,
                       ESE_GZ_TX_DESC_TYPE_PREFIX);
}

static uint8_t sfc_ef100_tx_qdesc_cso_inner_l3(uint64_t tx_tunnel) {
  uint8_t inner_l3;
...
static int sfc_ef100_tx_map(const struct sfc_ef100_txq *txq, rte_iova_t iova,
                            size_t len, rte_iova_t *dma_addr) {
...
static int sfc_ef100_tx_qdesc_send_create(const struct sfc_ef100_txq *txq,
  ...
  if ((m->ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK) == 0) {
    ...
    l4_offset_w =
        (m->outer_l2_len + m->outer_l3_len + m->l2_len + m->l3_len) >> 1;
  }
  ...
  EFX_POPULATE_OWORD_10(
      *tx_desc, ESF_GZ_TX_SEND_ADDR, dma_addr, ESF_GZ_TX_SEND_LEN,
      rte_pktmbuf_data_len(m), ESF_GZ_TX_SEND_NUM_SEGS, m->nb_segs,
      ESF_GZ_TX_SEND_CSO_PARTIAL_START_W, l4_offset_w,
      ESF_GZ_TX_SEND_CSO_PARTIAL_CSUM_W, part_cksum_w,
      ESF_GZ_TX_SEND_CSO_PARTIAL_EN, partial_en, ESF_GZ_TX_SEND_CSO_INNER_L3,
      inner_l3, ESF_GZ_TX_SEND_CSO_OUTER_L3, outer_l3,
      ESF_GZ_TX_SEND_CSO_OUTER_L4, outer_l4, ESF_GZ_TX_DESC_TYPE,
      ESE_GZ_TX_DESC_TYPE_SEND);

  if (m->ol_flags & RTE_MBUF_F_TX_VLAN) {
    efx_oword_t tx_desc_extra_fields;

    EFX_POPULATE_OWORD_2(tx_desc_extra_fields, ESF_GZ_TX_SEND_VLAN_INSERT_EN, 1,
...
static void sfc_ef100_tx_qdesc_seg_create(rte_iova_t addr, uint16_t len,
                                          efx_oword_t *tx_desc) {
  EFX_POPULATE_OWORD_3(*tx_desc, ESF_GZ_TX_SEG_ADDR, addr, ESF_GZ_TX_SEG_LEN,
                       len, ESF_GZ_TX_DESC_TYPE, ESE_GZ_TX_DESC_TYPE_SEG);
}

static void sfc_ef100_tx_qdesc_tso_create(
    const struct rte_mbuf *m, uint16_t nb_header_descs,
    uint16_t nb_payload_descs, size_t header_len, size_t payload_len,
    size_t outer_iph_off, size_t outer_udph_off, size_t iph_off,
    size_t tcph_off, efx_oword_t *tx_desc) {
  efx_oword_t tx_desc_extra_fields;
  int ed_outer_udp_len = (outer_udph_off != 0) ? 1 : 0;
  int ed_outer_ip_len = (outer_iph_off != 0) ? 1 : 0;
  int ed_outer_ip_id =
      (outer_iph_off != 0) ? ESE_GZ_TX_DESC_IP4_ID_INC_MOD16 : 0;
  /*
  ...
  uint8_t inner_l3 =
      sfc_ef100_tx_qdesc_cso_inner_l3(m->ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK);

  EFX_POPULATE_OWORD_10(
      *tx_desc, ESF_GZ_TX_TSO_MSS, m->tso_segsz, ESF_GZ_TX_TSO_HDR_NUM_SEGS,
      nb_header_descs, ESF_GZ_TX_TSO_PAYLOAD_NUM_SEGS, nb_payload_descs,
      ...
      ESF_GZ_TX_TSO_ED_INNER_IP_LEN, 1, ESF_GZ_TX_TSO_ED_OUTER_UDP_LEN,
      ed_outer_udp_len, ESF_GZ_TX_TSO_HDR_LEN_W, header_len >> 1,
      ESF_GZ_TX_TSO_PAYLOAD_LEN, payload_len);

  EFX_POPULATE_OWORD_9(
      tx_desc_extra_fields,
      /*
       ...
       */
      ESF_GZ_TX_TSO_INNER_L3_OFF_W, iph_off >> 1, ESF_GZ_TX_TSO_INNER_L4_OFF_W,
      tcph_off >> 1, ESF_GZ_TX_TSO_CSO_INNER_L4,
      inner_l3 != ESE_GZ_TX_DESC_CS_INNER_L3_OFF, ESF_GZ_TX_TSO_CSO_INNER_L3,
      inner_l3,
      /*
       * Use outer full checksum offloads which do
       * not require any extra information.
       */
      ESF_GZ_TX_TSO_CSO_OUTER_L3, 1, ESF_GZ_TX_TSO_CSO_OUTER_L4, 1,
      ESF_GZ_TX_DESC_TYPE, ESE_GZ_TX_DESC_TYPE_TSO);

  EFX_OR_OWORD(*tx_desc, tx_desc_extra_fields);

  if (m->ol_flags & RTE_MBUF_F_TX_VLAN) {
    EFX_POPULATE_OWORD_2(tx_desc_extra_fields, ESF_GZ_TX_TSO_VLAN_INSERT_EN, 1,
...
static inline void sfc_ef100_tx_qpush(struct sfc_ef100_txq *txq,
                                      unsigned int added) {
  efx_dword_t dword;
  ...
  sfc_ef100_tx_debug(txq, "TxQ pushed doorbell at pidx %u (added=%u)",
                     EFX_DWORD_FIELD(dword, ERF_GZ_TX_RING_PIDX), added);
}

static unsigned int sfc_ef100_tx_pkt_descs_max(const struct rte_mbuf *m) {
...
#define SFC_MBUF_SEG_LEN_MAX UINT16_MAX
  ...
  if (m->ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
     ...
     */
    RTE_BUILD_BUG_ON(
        SFC_EF100_TX_SEND_DESC_LEN_MAX <
...
static int sfc_ef100_xmit_tso_pkt(struct sfc_ef100_txq *const txq,
  ...
  do {
    ...
    if (rte_pktmbuf_data_len(m_seg) <= remaining_hdr_len) {
      ...
      sfc_ef100_tx_qdesc_seg_create(dma_addr, rte_pktmbuf_data_len(m_seg),
                                    &txq->txq_hw_ring[id]);
       ...
       * payload segments
       */
      sfc_ef100_tx_qdesc_seg_create(dma_addr, remaining_hdr_len,
                                    &txq->txq_hw_ring[id]);
  ...
  sfc_ef100_tx_qdesc_tso_create(*m, nb_hdr_descs, nb_pld_descs, header_len,
                                ...
                                outer_iph_off, outer_udph_off, iph_off,
                                tcph_off, &txq->txq_hw_ring[tso_desc_id]);

  *m = m_seg;
  return 0;
}

static uint16_t sfc_ef100_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                                    uint16_t nb_pkts) {
  struct sfc_ef100_txq *const txq = sfc_ef100_txq_by_dp_txq(tx_queue);
  unsigned int added;
  ...
  for (pktp = &tx_pkts[0], pktp_end = &tx_pkts[nb_pkts]; pktp != pktp_end;
       ++pktp) {
    ...
    if (m_seg->ol_flags & sfc_dp_mport_override) {
      id = added++ & txq->ptr_mask;
      sfc_ef100_tx_qdesc_prefix_create(m_seg, &txq->txq_hw_ring[id]);
      txq->sw_ring[id].mbuf = NULL;
    }
    ...
    if (m_seg->ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
      ...
      id = added++ & txq->ptr_mask;
      rc = sfc_ef100_tx_qdesc_send_create(txq, m_seg, &txq->txq_hw_ring[id]);
    ...
    while (likely(rc == 0) && m_seg != NULL) {
      RTE_BUILD_BUG_ON(SFC_MBUF_SEG_LEN_MAX > SFC_EF100_TX_SEG_DESC_LEN_MAX);
    ...
    if (likely(rc == 0)) {
      ...
      sfc_pkts_bytes_add(&txq->dp.dpq.stats, 1, rte_pktmbuf_pkt_len(*pktp));
    } else {
      added = pkt_start;
    }
  }
...
static void sfc_ef100_get_dev_info(struct rte_eth_dev_info *dev_info) {
  /*
...
static int sfc_ef100_tx_qsize_up_rings(uint16_t nb_tx_desc,
  ...
  *txq_max_fill_level = RTE_MIN(nb_tx_desc, SFC_EF100_TXQ_LIMIT(*evq_entries));
  return 0;
}

static sfc_dp_tx_qcreate_t sfc_ef100_tx_qcreate;
static int sfc_ef100_tx_qcreate(uint16_t port_id, uint16_t queue_id,
  ...
  rc = ENOMEM;
  txq = rte_zmalloc_socket("sfc-ef100-txq", sizeof(*txq), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (txq == NULL)
    goto fail_txq_alloc;
  ...
  rc = ENOMEM;
  txq->sw_ring =
      rte_calloc_socket("sfc-ef100-txq-sw_ring", info->txq_entries,
                        sizeof(*txq->sw_ring), RTE_CACHE_LINE_SIZE, socket_id);
  if (txq->sw_ring == NULL)
...
static void sfc_ef100_tx_qdestroy(struct sfc_dp_txq *dp_txq) {
...
static int sfc_ef100_tx_qstart(struct sfc_dp_txq *dp_txq,
                               unsigned int evq_read_ptr,
...
static void sfc_ef100_tx_qstop(struct sfc_dp_txq *dp_txq,
                               unsigned int *evq_read_ptr) {
  struct sfc_ef100_txq *txq = sfc_ef100_txq_by_dp_txq(dp_txq);
...
static bool sfc_ef100_tx_qtx_ev(struct sfc_dp_txq *dp_txq,
                                unsigned int num_descs) {
  struct sfc_ef100_txq *txq = sfc_ef100_txq_by_dp_txq(dp_txq);
...
static void sfc_ef100_tx_qreap(struct sfc_dp_txq *dp_txq) {
  struct sfc_ef100_txq *txq = sfc_ef100_txq_by_dp_txq(dp_txq);
  unsigned int completed;
...
static unsigned int sfc_ef100_tx_qdesc_npending(struct sfc_ef100_txq *txq) {
...
static int sfc_ef100_tx_qdesc_status(struct sfc_dp_txq *dp_txq,
                                     uint16_t offset) {
  struct sfc_ef100_txq *txq = sfc_ef100_txq_by_dp_txq(dp_txq);
  ...
  return (offset >= pushed || offset < sfc_ef100_tx_qdesc_npending(txq))
...
struct sfc_dp_tx sfc_ef100_tx = {
    ...
    .features = SFC_DP_TX_FEAT_MULTI_PROCESS | SFC_DP_TX_FEAT_STATS,
    .dev_offload_capa = 0,
    .queue_offload_capa =
        RTE_ETH_TX_OFFLOAD_VLAN_INSERT | RTE_ETH_TX_OFFLOAD_IPV4_CKSUM |
        RTE_ETH_TX_OFFLOAD_OUTER_IPV4_CKSUM |
        RTE_ETH_TX_OFFLOAD_OUTER_UDP_CKSUM | RTE_ETH_TX_OFFLOAD_UDP_CKSUM |
