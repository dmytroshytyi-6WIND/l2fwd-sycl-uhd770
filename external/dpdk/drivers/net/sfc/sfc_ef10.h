...
#define SFC_EF10_EV_PER_CACHE_LINE (RTE_CACHE_LINE_SIZE / sizeof(efx_qword_t))
...
static inline void sfc_ef10_ev_qclear_cache_line(void *ptr) {
...
static inline void sfc_ef10_ev_qclear(efx_qword_t *hw_ring,
                                      unsigned int ptr_mask,
                                      unsigned int old_read_ptr,
  ...
  while (old_clear_ptr != clear_ptr) {
    sfc_ef10_ev_qclear_cache_line(&hw_ring[old_clear_ptr & ptr_mask]);
...
static inline bool sfc_ef10_ev_present(const efx_qword_t ev) {
  return ~EFX_QWORD_FIELD(ev, EFX_DWORD_0) | ~EFX_QWORD_FIELD(ev, EFX_DWORD_1);
}
...
static inline void sfc_ef10_rx_qpush(volatile void *doorbell,
                                     unsigned int added, unsigned int ptr_mask,
                                     uint32_t *dbell_counter) {
  efx_dword_t dword;
...
static inline void sfc_ef10_ev_qprime(volatile void *qprime,
                                      unsigned int read_ptr,
                                      unsigned int ptr_mask) {
  efx_dword_t dword;
