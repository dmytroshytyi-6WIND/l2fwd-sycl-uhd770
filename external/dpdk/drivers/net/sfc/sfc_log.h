...
#define sfc_err(sa, ...)                                                       \
  do {                                                                         \
    const struct sfc_adapter *_sa = (sa);                                      \
                                                                               \
    SFC_LOG(_sa->priv.shared, RTE_LOG_ERR, _sa->priv.logtype_main,             \
            __VA_ARGS__);                                                      \
  } while (0)
...
#define sfc_warn(sa, ...)                                                      \
  do {                                                                         \
    const struct sfc_adapter *_sa = (sa);                                      \
                                                                               \
    SFC_LOG(_sa->priv.shared, RTE_LOG_WARNING, _sa->priv.logtype_main,         \
            __VA_ARGS__);                                                      \
  } while (0)
...
#define sfc_notice(sa, ...)                                                    \
  do {                                                                         \
    const struct sfc_adapter *_sa = (sa);                                      \
                                                                               \
    SFC_LOG(_sa->priv.shared, RTE_LOG_NOTICE, _sa->priv.logtype_main,          \
            __VA_ARGS__);                                                      \
  } while (0)
...
#define sfc_dbg(sa, ...)                                                       \
  do {                                                                         \
    const struct sfc_adapter *_sa = (sa);                                      \
                                                                               \
    SFC_LOG(_sa->priv.shared, RTE_LOG_DEBUG, _sa->priv.logtype_main,           \
            __VA_ARGS__);                                                      \
  } while (0)
