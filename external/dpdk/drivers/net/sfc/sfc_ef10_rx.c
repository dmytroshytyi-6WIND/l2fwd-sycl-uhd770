...
static void sfc_ef10_rx_qprime(struct sfc_ef10_rxq *rxq) {
  sfc_ef10_ev_qprime(rxq->evq_prime, rxq->evq_read_ptr, rxq->ptr_mask);
  rxq->evq_read_ptr_primed = rxq->evq_read_ptr;
}

static void sfc_ef10_rx_qrefill(struct sfc_ef10_rxq *rxq) {
  ...
  do {
    ...
    for (i = 0, id = added & ptr_mask; i < RTE_DIM(objs); ++i, ++id) {
      ...
      EFX_POPULATE_QWORD_2(rxq->rxq_hw_ring[id], ESF_DZ_RX_KER_BYTE_CNT,
                           buf_size, ESF_DZ_RX_KER_BUF_ADDR, phys_addr);
    }
...
static void sfc_ef10_rx_prefetch_next(struct sfc_ef10_rxq *rxq,
  ...
  if (likely(next_mbuf != NULL)) {
    ...
    rte_prefetch0((uint8_t *)next_mbuf->buf_addr + RTE_PKTMBUF_HEADROOM);
  }
}

static struct rte_mbuf **sfc_ef10_rx_pending(struct sfc_ef10_rxq *rxq,
  ...
  if (n_rx_pkts != 0) {
    ...
    do {
      *rx_pkts++ = rxq->sw_ring[completed++ & rxq->ptr_mask].mbuf;
...
static const efx_rx_prefix_layout_t sfc_ef10_rx_prefix_layout = {
    .erpl_fields = {
        [EFX_RX_PREFIX_FIELD_RSS_HASH] = {0, sizeof(uint32_t) * CHAR_BIT,
                                          B_FALSE},
        [EFX_RX_PREFIX_FIELD_LENGTH] = {8 * CHAR_BIT,
                                        sizeof(uint16_t) * CHAR_BIT, B_FALSE},
    }};
static uint16_t sfc_ef10_rx_pseudo_hdr_get_len(const uint8_t *pseudo_hdr) {
  return rte_le_to_cpu_16(*(const uint16_t *)&pseudo_hdr[8]);
}

static uint32_t sfc_ef10_rx_pseudo_hdr_get_hash(const uint8_t *pseudo_hdr) {
...
sfc_ef10_rx_process_event(struct sfc_ef10_rxq *rxq, efx_qword_t rx_ev,
  ...
  if (rx_ev.eq_u64[0] & rte_cpu_to_le_64((1ull << ESF_DZ_RX_ECC_ERR_LBN) |
  ...
  sfc_ef10_rx_ev_to_offloads(
      rx_ev, m,
      (rxq->flags & SFC_EF10_RXQ_RSS_HASH) ? ~0ull : ~RTE_MBUF_F_RX_RSS_HASH);
  ...
  if (ready == 1)
    seg_len = EFX_QWORD_FIELD(rx_ev, ESF_DZ_RX_BYTES) - rxq->prefix_size;
  else
...
static bool sfc_ef10_rx_get_event(struct sfc_ef10_rxq *rxq,
                                  efx_qword_t *rx_ev) {
  *rx_ev = rxq->evq_hw_ring[rxq->evq_read_ptr & rxq->ptr_mask];
  ...
  if (unlikely(EFX_QWORD_FIELD(*rx_ev, FSF_AZ_EV_CODE) !=
    ...
    sfc_ef10_rx_err(&rxq->dp.dpq, "RxQ exception at EvQ read ptr %#x",
                    rxq->evq_read_ptr);
    return false;
  }
...
static uint16_t sfc_ef10_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                                   uint16_t nb_pkts) {
  ...
  while (rx_pkts != rx_pkts_end && sfc_ef10_rx_get_event(rxq, &rx_ev)) {
    ...
    rx_pkts = sfc_ef10_rx_process_event(rxq, rx_ev, rx_pkts, rx_pkts_end);
  }
...
const uint32_t *sfc_ef10_supported_ptypes_get(uint32_t tunnel_encaps) {
  static const uint32_t ef10_native_ptypes[] = {RTE_PTYPE_L2_ETHER,
  ...
  case (1u << EFX_TUNNEL_PROTOCOL_VXLAN | 1u << EFX_TUNNEL_PROTOCOL_GENEVE |
...
static unsigned int sfc_ef10_rx_qdesc_npending(struct sfc_dp_rxq *dp_rxq) {
...
static int sfc_ef10_rx_qdesc_status(struct sfc_dp_rxq *dp_rxq,
                                    uint16_t offset) {
  struct sfc_ef10_rxq *rxq = sfc_ef10_rxq_by_dp_rxq(dp_rxq);
...
static void sfc_ef10_rx_get_dev_info(struct rte_eth_dev_info *dev_info) {
  /*
...
static int sfc_ef10_rx_qsize_up_rings(uint16_t nb_rx_desc,
  ...
  *rxq_max_fill_level = RTE_MIN(nb_rx_desc, SFC_EF10_RXQ_LIMIT(*evq_entries));
  return 0;
}

static uint64_t sfc_ef10_mk_mbuf_rearm_data(uint16_t port_id,
                                            uint16_t prefix_size) {
  struct rte_mbuf m;
...
static int sfc_ef10_rx_qcreate(uint16_t port_id, uint16_t queue_id,
  ...
  rxq->rearm_data = sfc_ef10_mk_mbuf_rearm_data(port_id, info->prefix_size);
  ...
  rxq->evq_prime = (volatile uint8_t *)info->mem_bar + ER_DZ_EVQ_RPTR_REG_OFST +
...
static void sfc_ef10_rx_qdestroy(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef10_rxq *rxq = sfc_ef10_rxq_by_dp_rxq(dp_rxq);
...
static int sfc_ef10_rx_qstart(struct sfc_dp_rxq *dp_rxq,
                              unsigned int evq_read_ptr,
...
static void sfc_ef10_rx_qstop(struct sfc_dp_rxq *dp_rxq,
                              unsigned int *evq_read_ptr) {
  struct sfc_ef10_rxq *rxq = sfc_ef10_rxq_by_dp_rxq(dp_rxq);
...
static bool sfc_ef10_rx_qrx_ev(struct sfc_dp_rxq *dp_rxq,
...
static void sfc_ef10_rx_qpurge(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef10_rxq *rxq = sfc_ef10_rxq_by_dp_rxq(dp_rxq);
  unsigned int i;
  struct sfc_ef10_rx_sw_desc *rxd;
...
static int sfc_ef10_rx_intr_enable(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef10_rxq *rxq = sfc_ef10_rxq_by_dp_rxq(dp_rxq);
...
static int sfc_ef10_rx_intr_disable(struct sfc_dp_rxq *dp_rxq) {
  struct sfc_ef10_rxq *rxq = sfc_ef10_rxq_by_dp_rxq(dp_rxq);
...
struct sfc_dp_rx sfc_ef10_rx = {
    ...
    .features = SFC_DP_RX_FEAT_MULTI_PROCESS | SFC_DP_RX_FEAT_INTR,
