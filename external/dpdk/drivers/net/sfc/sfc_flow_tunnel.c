...
bool sfc_flow_tunnel_is_active(struct sfc_adapter *sa) {
  ...
  return ((sa->negotiated_rx_metadata & RTE_ETH_RX_METADATA_TUNNEL_ID) != 0);
}
...
int sfc_flow_tunnel_detect_jump_rule(struct sfc_adapter *sa,
  ...
  if (ft != NULL && action_jump != 0) {
    ...
    if (rc != 0) {
      ...
      sfc_err(sa, "tunnel offload: JUMP: invalid actions: %s", strerror(rc));
      goto fail;
    }

    if (ft->refcnt == 0) {
      sfc_err(sa, "tunnel offload: JUMP: tunnel=%u does not exist", ft->id);
      rc = ENOENT;
      goto fail;
    }

    if (ft->jump_rule_is_set) {
      sfc_err(sa, "tunnel offload: JUMP: already exists in tunnel=%u", ft->id);
      rc = EEXIST;
      goto fail;
    }
...
static int sfc_flow_tunnel_attach(struct sfc_adapter *sa,
...
attach:
  sfc_dbg(sa, "tunnel offload: attaching to %s tunnel=%u", ft_status, ft_id);
...
static int sfc_flow_tunnel_detach(struct sfc_adapter *sa, uint32_t ft_mark) {
  struct sfc_flow_tunnel *ft;
...
int sfc_flow_tunnel_get_restore_info(struct rte_eth_dev *dev,
  ...
  if ((m->ol_flags & sfc_dp_ft_id_valid) == 0) {
    sfc_dbg(sa,
            "tunnel offload: get_restore_info: no tunnel mark in the packet");
    rc = EINVAL;
    goto fail;
  }
  ...
  info->flags = RTE_FLOW_RESTORE_INFO_ENCAPSULATED |
                RTE_FLOW_RESTORE_INFO_GROUP_ID | RTE_FLOW_RESTORE_INFO_TUNNEL;
