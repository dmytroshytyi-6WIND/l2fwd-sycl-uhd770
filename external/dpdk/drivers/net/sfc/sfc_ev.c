...
static const char *sfc_evq_type2str(enum sfc_evq_type type) {
  switch (type) {
...
static boolean_t sfc_ev_initialized(void *arg) {
...
static boolean_t sfc_ev_nop_rx(void *arg, uint32_t label, uint32_t id,
...
static boolean_t sfc_ev_efx_rx(void *arg, __rte_unused uint32_t label,
  ...
  if (delta == 0) {
    ...
    if (unlikely(size != 0)) {
      ...
      sfc_err(evq->sa,
              ...
              evq->evq_index, rxq->dp.dpq.queue_id, id, size, flags);
      goto done;
    }
    ...
    sfc_err(evq->sa,
            ...
            evq->evq_index, rxq->dp.dpq.queue_id, id, delta, flags);
...
static boolean_t sfc_ev_dp_rx(void *arg, __rte_unused uint32_t label,
                              uint32_t id, __rte_unused uint32_t size,
                              __rte_unused uint16_t flags) {
  struct sfc_evq *evq = arg;
...
static boolean_t sfc_ev_nop_rx_packets(void *arg, uint32_t label,
...
static boolean_t sfc_ev_dp_rx_packets(void *arg, __rte_unused uint32_t label,
                                      unsigned int num_packets,
                                      __rte_unused uint32_t flags) {
  struct sfc_evq *evq = arg;
  struct sfc_dp_rxq *dp_rxq;
...
static boolean_t sfc_ev_nop_rx_ps(void *arg, uint32_t label, uint32_t id,
...
static boolean_t sfc_ev_dp_rx_ps(void *arg, __rte_unused uint32_t label,
                                 uint32_t id, __rte_unused uint32_t pkt_count,
                                 __rte_unused uint16_t flags) {
  struct sfc_evq *evq = arg;
  struct sfc_dp_rxq *dp_rxq;
...
static boolean_t sfc_ev_nop_tx(void *arg, uint32_t label, uint32_t id) {
  struct sfc_evq *evq = arg;

  sfc_err(evq->sa, "EVQ %u unexpected Tx event label=%u id=%#x", evq->evq_index,
          label, id);
  return B_TRUE;
}

static boolean_t sfc_ev_tx(void *arg, __rte_unused uint32_t label,
                           uint32_t id) {
  struct sfc_evq *evq = arg;
...
static boolean_t sfc_ev_dp_tx(void *arg, __rte_unused uint32_t label,
                              uint32_t id) {
  struct sfc_evq *evq = arg;
  struct sfc_dp_txq *dp_txq;
...
static boolean_t sfc_ev_nop_tx_ndescs(void *arg, uint32_t label,
                                      unsigned int ndescs) {
  struct sfc_evq *evq = arg;
...
static boolean_t sfc_ev_dp_tx_ndescs(void *arg, __rte_unused uint32_t label,
                                     unsigned int ndescs) {
  struct sfc_evq *evq = arg;
  struct sfc_dp_txq *dp_txq;
...
static boolean_t sfc_ev_exception(void *arg, uint32_t code,
...
static boolean_t sfc_ev_nop_rxq_flush_done(void *arg, uint32_t rxq_hw_index) {
  struct sfc_evq *evq = arg;

  sfc_err(evq->sa, "EVQ %u unexpected RxQ %u flush done", evq->evq_index,
          rxq_hw_index);
  return B_TRUE;
}

static boolean_t sfc_ev_rxq_flush_done(void *arg,
                                       __rte_unused uint32_t rxq_hw_index) {
  struct sfc_evq *evq = arg;
  struct sfc_dp_rxq *dp_rxq;
  struct sfc_rxq *rxq;
...
static boolean_t sfc_ev_nop_rxq_flush_failed(void *arg, uint32_t rxq_hw_index) {
  struct sfc_evq *evq = arg;

  sfc_err(evq->sa, "EVQ %u unexpected RxQ %u flush failed", evq->evq_index,
          rxq_hw_index);
  return B_TRUE;
}

static boolean_t sfc_ev_rxq_flush_failed(void *arg,
                                         __rte_unused uint32_t rxq_hw_index) {
  struct sfc_evq *evq = arg;
  struct sfc_dp_rxq *dp_rxq;
  struct sfc_rxq *rxq;
...
static boolean_t sfc_ev_nop_txq_flush_done(void *arg, uint32_t txq_hw_index) {
  struct sfc_evq *evq = arg;

  sfc_err(evq->sa, "EVQ %u unexpected TxQ %u flush done", evq->evq_index,
          txq_hw_index);
  return B_TRUE;
}

static boolean_t sfc_ev_txq_flush_done(void *arg,
                                       __rte_unused uint32_t txq_hw_index) {
  struct sfc_evq *evq = arg;
  struct sfc_dp_txq *dp_txq;
  struct sfc_txq *txq;
...
static boolean_t sfc_ev_software(void *arg, uint16_t magic) {
  struct sfc_evq *evq = arg;
...
static boolean_t sfc_ev_sram(void *arg, uint32_t code) {
  struct sfc_evq *evq = arg;

  sfc_err(evq->sa, "EVQ %u unexpected SRAM event code=%u", evq->evq_index,
          code);
  return B_TRUE;
}

static boolean_t sfc_ev_wake_up(void *arg, uint32_t index) {
  struct sfc_evq *evq = arg;

  sfc_err(evq->sa, "EVQ %u unexpected wake up event index=%u", evq->evq_index,
          index);
  return B_TRUE;
}

static boolean_t sfc_ev_timer(void *arg, uint32_t index) {
  struct sfc_evq *evq = arg;

  sfc_err(evq->sa, "EVQ %u unexpected timer event index=%u", evq->evq_index,
          index);
  return B_TRUE;
}
...
sfc_ev_nop_link_change(void *arg, __rte_unused efx_link_mode_t link_mode) {
  ...
  sfc_err(evq->sa, "EVQ %u unexpected link change event", evq->evq_index);
  return B_TRUE;
}

static boolean_t sfc_ev_link_change(void *arg, efx_link_mode_t link_mode) {
  struct sfc_evq *evq = arg;
...
void sfc_ev_qpoll(struct sfc_evq *evq) {
  ...
  if (unlikely(evq->exception) && sfc_adapter_trylock(evq->sa)) {
    ...
    if (evq->dp_rxq != NULL) {
      ...
      sfc_warn(sa, "restart RxQ %u because of exception on its EvQ %u",
    ...
    if (evq->dp_txq != NULL) {
      ...
      sfc_warn(sa, "restart TxQ %u because of exception on its EvQ %u",
    ...
    if (evq->exception)
      sfc_panic(sa, "unrecoverable exception on EvQ %u", evq->evq_index);
...
int sfc_ev_qstart(struct sfc_evq *evq, unsigned int hw_index) {
  ...
  if (sa->intr.lsc_intr && hw_index == sa->mgmt_evq_index) {
    ...
    ethdev_qid = sfc_ethdev_rx_qid_by_rxq_sw_index(sfc_sa2shared(sa),
  ...
  rc = efx_ev_qcreate_irq(sa->nic, hw_index, esmp, evq->entries,
                          0 /* unused on EF10 */, 0, evq_flags, irq,
                          &evq->common);
  if (rc != 0)
  ...
  if (evq->dp_rxq != 0) {
    if (strcmp(sa->priv.dp_rx->dp.name, SFC_KVARG_DATAPATH_EFX) == 0)
      evq->callbacks = &sfc_ev_callbacks_efx_rx;
    else
      evq->callbacks = &sfc_ev_callbacks_dp_rx;
  } else if (evq->dp_txq != 0) {
    if (strcmp(sa->priv.dp_tx->dp.name, SFC_KVARG_DATAPATH_EFX) == 0)
...
static void sfc_ev_mgmt_periodic_qpoll(void *arg) {
  ...
  if (rc == -ENOTSUP) {
    ...
    sfc_warn(sa, "management EVQ must be polled indirectly using no-wait link "
                 "status update");
  } else if (rc != 0) {
    sfc_err(sa, "cannot rearm management EVQ polling alarm (rc=%d)", rc);
  }
}

static void sfc_ev_mgmt_periodic_qpoll_start(struct sfc_adapter *sa) {
  sfc_ev_mgmt_periodic_qpoll(sa);
}

static void sfc_ev_mgmt_periodic_qpoll_stop(struct sfc_adapter *sa) {
...
int sfc_ev_qinit(struct sfc_adapter *sa, enum sfc_evq_type type,
                 unsigned int type_index, unsigned int entries, int socket_id,
                 struct sfc_evq **evqp) {
  struct sfc_evq *evq;
  int rc;

  sfc_log_init(sa, "type=%s type_index=%u", sfc_evq_type2str(type), type_index);
  ...
  /* Allocate DMA space */
  rc = sfc_dma_alloc(
      sa, sfc_evq_type2str(type), type_index, EFX_NIC_DMA_ADDR_EVENT_RING,
      efx_evq_size(sa->nic, evq->entries, sa->evq_flags), socket_id, &evq->mem);
  if (rc != 0)
    goto fail_dma_alloc;
...
static int sfc_kvarg_perf_profile_handler(__rte_unused const char *key,
...
int sfc_ev_attach(struct sfc_adapter *sa) {
  ...
  rc = sfc_kvargs_process(sa, SFC_KVARG_PERF_PROFILE,
                          sfc_kvarg_perf_profile_handler, &sa->evq_flags);
  if (rc != 0) {
    sfc_err(sa, "invalid %s parameter value", SFC_KVARG_PERF_PROFILE);
...
void sfc_ev_detach(struct sfc_adapter *sa) {
  ...
  if (sa->evq_count != 0)
    sfc_err(sa, "%u EvQs are not destroyed before detach", sa->evq_count);
}
