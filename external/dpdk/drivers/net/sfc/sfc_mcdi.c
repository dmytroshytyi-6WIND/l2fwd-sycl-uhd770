...
static void sfc_mcdi_dma_free(void *cookie, efsys_mem_t *esmp) {
...
static void sfc_mcdi_sched_restart(void *cookie) {
  struct sfc_adapter *sa = cookie;
...
static void sfc_mcdi_mgmt_evq_poll(void *cookie) {
  struct sfc_adapter *sa = cookie;
...
int sfc_mcdi_init(struct sfc_adapter *sa) {
  ...
  logtype = sfc_register_logtype(&sa->priv.shared->pci_addr,
                                 SFC_LOGTYPE_MCDI_STR, RTE_LOG_NOTICE);

  return sfc_efx_mcdi_init(&sa->mcdi, logtype, sa->priv.shared->log_prefix,
                           sa->nic, &sfc_mcdi_ops, sa);
}
