...
static uint32_t sfc_mae_counter_get_service_lcore(struct sfc_adapter *sa) {
  uint32_t cid;
...
int sfc_mae_counter_enable(struct sfc_adapter *sa,
  ...
  rc = efx_mae_counters_alloc(sa->nic, 1, &unused, &mae_counter,
                              &generation_count);
  if (rc != 0) {
    sfc_err(sa, "failed to alloc MAE counter: %s", rte_strerror(rc));
   ...
   */
  __atomic_store(&p->reset.pkts_bytes.int128, &p->value.pkts_bytes.int128,
                 __ATOMIC_RELAXED);
  ...
  sfc_info(sa,
           "enabled MAE counter #%u with reset pkts=%" PRIu64 " bytes=%" PRIu64,
           mae_counter.id, p->reset.pkts, p->reset.bytes);
...
int sfc_mae_counter_disable(struct sfc_adapter *sa,
  ...
  if (rc != 0)
    sfc_err(sa, "failed to free MAE counter %u: %s", counter->mae_id.id,
            rte_strerror(rc));

  sfc_info(
      sa, "disabled MAE counter #%u with reset pkts=%" PRIu64 " bytes=%" PRIu64,
      counter->mae_id.id, p->reset.pkts, p->reset.bytes);
...
static void sfc_mae_counter_increment(struct sfc_adapter *sa,
                                      struct sfc_mae_counters *counters,
                                      uint32_t mae_counter_id,
                                      uint32_t generation_count, uint64_t pkts,
                                      uint64_t bytes) {
   ...
   */
  __atomic_store(&p->value.pkts_bytes, &cnt_val.pkts_bytes, __ATOMIC_RELAXED);
  ...
  sfc_info(sa,
           "update MAE counter #%u: pkts+%" PRIu64 "=%" PRIu64
           ", bytes+%" PRIu64 "=%" PRIu64,
           mae_counter_id, pkts, cnt_val.pkts, bytes, cnt_val.bytes);
}
...
sfc_mae_parse_counter_packet(struct sfc_adapter *sa,
  ...
  if (unlikely(m->data_len < ER_RX_SL_PACKETISER_HEADER_WORD_SIZE)) {
    sfc_err(sa, "too short MAE counters packet (%u bytes)", m->data_len);
    return;
  }
  ...
  if (unlikely(version != ERF_SC_PACKETISER_HEADER_VERSION_2)) {
    sfc_err(sa, "unexpected MAE counters packet version %u", version);
    return;
  }
  ...
  header_offset = EFX_XWORD_FIELD(*hdr, ERF_SC_PACKETISER_HEADER_HEADER_OFFSET);
  ...
  required_len = payload_offset + counter_count * sizeof(counters_data[0]);
  if (unlikely(required_len > m->data_len)) {
    sfc_err(sa,
            "truncated MAE counters packet: %u counters, packet length is %u "
            "vs %u required",
  ...
  if (unlikely(payload_offset % sizeof(uint32_t) != 0)) {
    sfc_err(
        sa,
        "unsupported MAE counters payload offset %u, must be 32-bit aligned",
        payload_offset);
    return;
  }
  ...
  sfc_info(sa, "update %u MAE counters with gc=%u", counter_count,
           generation_count);
  ...
  for (i = 0; i < counter_count; ++i) {
     ...
     */
    packet_count_lo = EFX_OWORD_FIELD32(
        counters_data[i], ERF_SC_PACKETISER_PAYLOAD_PACKET_COUNT_LO);
    packet_count_hi = EFX_OWORD_FIELD32(
        counters_data[i], ERF_SC_PACKETISER_PAYLOAD_PACKET_COUNT_HI);
    byte_count_lo = EFX_OWORD_FIELD32(counters_data[i],
                                      ERF_SC_PACKETISER_PAYLOAD_BYTE_COUNT_LO);
    byte_count_hi = EFX_OWORD_FIELD32(counters_data[i],
                                      ERF_SC_PACKETISER_PAYLOAD_BYTE_COUNT_HI);
    sfc_mae_counter_increment(
        sa, &counter_registry->counters,
...
static int32_t sfc_mae_counter_poll_packets(struct sfc_adapter *sa) {
  struct sfc_mae_counter_registry *counter_registry = &sa->mae.counter_registry;
  ...
  if (pushed_diff >= SFC_COUNTER_RXQ_REFILL_LEVEL) {
    ...
    if (rc == 0) {
       ...
       * from the failure
       */
      SFC_GENERIC_LOG(DEBUG, "Give credits failed: %s", rte_strerror(rc));
    }
  }
...
static int32_t sfc_mae_counter_service_routine(void *arg) {
...
static void *sfc_mae_counter_thread(void *data) {
...
static void sfc_mae_counter_service_unregister(struct sfc_adapter *sa) {
  struct sfc_mae_counter_registry *registry = &sa->mae.counter_registry;
...
static struct sfc_rxq_info *sfc_counter_rxq_info_get(struct sfc_adapter *sa) {
...
sfc_mae_counter_registry_prepare(struct sfc_mae_counter_registry *registry,
  ...
  registry->use_credits =
      counter_stream_flags & EFX_MAE_COUNTERS_STREAM_OUT_USES_CREDITS;
}

static int sfc_mae_counter_service_register(struct sfc_adapter *sa,
                                            uint32_t counter_stream_flags) {
  ...
  sfc_mae_counter_registry_prepare(counter_registry, sa, counter_stream_flags);

  cid = sfc_get_service_lcore(sa->socket_id);
  if (cid == RTE_MAX_LCORE && sa->socket_id != SOCKET_ID_ANY) {
    /* Warn and try to allocate on any NUMA node */
    sfc_warn(sa, "failed to get service lcore for counter service at socket %d",
             sa->socket_id);
...
static void sfc_mae_counter_thread_stop(struct sfc_adapter *sa) {
  struct sfc_mae_counter_registry *counter_registry = &sa->mae.counter_registry;
  int rc;
...
static int sfc_mae_counter_thread_spawn(struct sfc_adapter *sa,
                                        uint32_t counter_stream_flags) {
  struct sfc_mae_counter_registry *counter_registry = &sa->mae.counter_registry;
  int rc;
...
int sfc_mae_counters_init(struct sfc_mae_counters *counters,
  ...
  counters->mae_counters = rte_zmalloc(
      "sfc_mae_counters", sizeof(*counters->mae_counters) * nb_counters_max, 0);
  if (counters->mae_counters == NULL) {
    rc = ENOMEM;
    SFC_GENERIC_LOG(ERR, "%s: failed: %s", __func__, rte_strerror(rc));
    return rc;
  }
...
int sfc_mae_counter_rxq_attach(struct sfc_adapter *sa) {
  ...
  const uint16_t data_room_size =
      RTE_PKTMBUF_HEADROOM + SFC_MAE_COUNTER_STREAM_PACKET_SIZE;
  int rc;
   ...
   */
  mp = rte_pktmbuf_pool_create(name, n_elements, cache_size, priv_size,
                               data_room_size, sa->socket_id);
  if (mp == NULL) {
...
int sfc_mae_counter_rxq_init(struct sfc_adapter *sa) {
  ...
  rc = sfc_rx_qinit_info(sa, sa->counter_rxq.sw_index, EFX_RXQ_FLAG_USER_MARK);
  if (rc != 0)
    goto fail_counter_rxq_init_info;

  rc = sfc_rx_qinit(sa, sa->counter_rxq.sw_index, nb_rx_desc, sa->socket_id,
                    &rxconf, sa->counter_rxq.mp);
  if (rc != 0) {
...
void sfc_mae_counter_stop(struct sfc_adapter *sa) {
  ...
  SFC_ASSERT(mae->counter_registry.polling_mode != SFC_MAE_COUNTER_POLLING_OFF);

  if (mae->counter_registry.polling_mode == SFC_MAE_COUNTER_POLLING_SERVICE)
...
int sfc_mae_counter_start(struct sfc_adapter *sa) {
  ...
  if (rc != 0) {
    sfc_err(sa, "failed to start MAE counters stream: %s", rte_strerror(rc));
...
int sfc_mae_counter_get(struct sfc_mae_counters *counters,
   ...
   * Paired with relaxed ordering in counter increment.
   */
  value.pkts_bytes.int128 =
      __atomic_load_n(&p->value.pkts_bytes.int128, __ATOMIC_RELAXED);
