...
static uint64_t sfc_tx_get_offload_mask(struct sfc_adapter *sa) {
  ...
  if (!sa->tso_encap || (encp->enc_tunnel_encapsulations_supported &
...
static int sfc_tx_qcheck_conf(struct sfc_adapter *sa,
  ...
  if (tx_conf->tx_free_thresh > txq_max_fill_level) {
    sfc_err(sa, "TxQ free threshold too large: %u vs maximum %u",
            tx_conf->tx_free_thresh, txq_max_fill_level);
    rc = EINVAL;
  }

  if (tx_conf->tx_thresh.pthresh != 0 || tx_conf->tx_thresh.hthresh != 0 ||
      tx_conf->tx_thresh.wthresh != 0) {
    sfc_warn(sa, "prefetch/host/writeback thresholds are not supported");
  }
...
int sfc_tx_qinit(struct sfc_adapter *sa, sfc_sw_index_t sw_index,
  ...
  rc = sa->priv.dp_tx->qsize_up_rings(nb_tx_desc, &hw_limits, &txq_entries,
  ...
  rc =
      sfc_ev_qinit(sa, SFC_EVQ_TYPE_TX, sw_index, evq_entries, socket_id, &evq);
  if (rc != 0)
    goto fail_ev_qinit;
  ...
  info.tso_tcp_header_offset_limit = encp->enc_tx_tso_tcp_header_offset_limit;
  info.tso_max_nb_header_descs =
      RTE_MIN(encp->enc_tx_tso_max_header_ndescs, (uint32_t)UINT16_MAX);
  info.tso_max_header_len =
      RTE_MIN(encp->enc_tx_tso_max_header_length, (uint32_t)UINT16_MAX);
  info.tso_max_nb_payload_descs =
      RTE_MIN(encp->enc_tx_tso_max_payload_ndescs, (uint32_t)UINT16_MAX);
...
static int sfc_tx_check_mode(struct sfc_adapter *sa,
  ...
  case RTE_ETH_MQ_TX_NONE:
    break;
  default:
    sfc_err(sa, "Tx multi-queue mode %u not supported", txmode->mq_mode);
    rc = EINVAL;
  }
  ...
  if ((dev_tx_offload_cap & RTE_ETH_TX_OFFLOAD_MBUF_FAST_FREE) != 0 &&
    ...
    sfc_err(
        sa,
        "There is no FAST_FREE flag in the attempted Tx mode configuration");
    sfc_err(
        sa,
        "FAST_FREE is always active as per the current Tx datapath variant");
    rc = EINVAL;
  }
 ...
 * or complete close.
 */
static void sfc_tx_fini_queues(struct sfc_adapter *sa,
...
int sfc_tx_configure(struct sfc_adapter *sa) {
  ...
  sfc_log_init(sa, "nb_tx_queues=%u (old %u)", nb_tx_queues,
  ...
  if (sas->txq_info == NULL) {
    reconfigure = false;
    sas->txq_info = rte_calloc_socket(
        "sfc-txqs", nb_txq_total, sizeof(sas->txq_info[0]), 0, sa->socket_id);
    if (sas->txq_info == NULL)
    ...
    new_txq_info =
        rte_realloc(sas->txq_info, nb_txq_total * sizeof(sas->txq_info[0]), 0);
    if (new_txq_info == NULL && nb_txq_total > 0)
      goto fail_txqs_realloc;

    new_txq_ctrl =
        realloc(sa->txq_ctrl, nb_txq_total * sizeof(sa->txq_ctrl[0]));
    ...
    if (nb_txq_total > sas->txq_count) {
      memset(&sas->txq_info[sas->txq_count], 0,
             (nb_txq_total - sas->txq_count) * sizeof(sas->txq_info[0]));
      memset(&sa->txq_ctrl[sas->txq_count], 0,
             (nb_txq_total - sas->txq_count) * sizeof(sa->txq_ctrl[0]));
    }
  }
  ...
  while (sas->ethdev_txq_count < nb_tx_queues) {
    ...
    sw_index = sfc_txq_sw_index_by_ethdev_tx_qid(sas, sas->ethdev_txq_count);
...
int sfc_tx_qstart(struct sfc_adapter *sa, sfc_sw_index_t sw_index) {
  ...
  uint64_t offloads_supported =
      sfc_tx_get_dev_offload_caps(sa) | sfc_tx_get_queue_offload_caps(sa);
  struct sfc_txq_info *txq_info;
  struct sfc_txq *txq;
  struct sfc_evq *evq;
  uint16_t flags = 0;
  ...
  if (txq_info->offloads &
      (RTE_ETH_TX_OFFLOAD_TCP_TSO | RTE_ETH_TX_OFFLOAD_VXLAN_TNL_TSO |
       RTE_ETH_TX_OFFLOAD_GENEVE_TNL_TSO))
    flags |= EFX_TXQ_FATSOV2;

  rc = efx_tx_qcreate(sa->nic, txq->hw_index, 0, &txq->mem, txq_info->entries,
                      0 /* not used on EF10 */, flags, evq->common,
  ...
  if (ethdev_qid != SFC_ETHDEV_QID_INVALID) {
    ...
    dev_data->tx_queue_state[ethdev_qid] = RTE_ETH_QUEUE_STATE_STARTED;
  }
...
void sfc_tx_qstop(struct sfc_adapter *sa, sfc_sw_index_t sw_index) {
   ...
   */
  for (retry_count = 0; ((txq_info->state & SFC_TXQ_FLUSHED) == 0) &&
                        (retry_count < SFC_TX_QFLUSH_ATTEMPTS);
       ++retry_count) {
    rc = efx_tx_qflush(txq->common);
    if (rc != 0) {
      txq_info->state |=
          (rc == EALREADY) ? SFC_TXQ_FLUSHED : SFC_TXQ_FLUSH_FAILED;
      break;
    }
    ...
    if (txq_info->state & SFC_TXQ_FLUSHING)
      sfc_err(sa, "TxQ %d (internal %u) flush timed out", ethdev_qid, sw_index);

    if (txq_info->state & SFC_TXQ_FLUSHED)
      sfc_notice(sa, "TxQ %d (internal %u) flushed", ethdev_qid, sw_index);
  }
  ...
  if (ethdev_qid != SFC_ETHDEV_QID_INVALID) {
     ...
     */
    dev_data = sa->eth_dev->data;
    dev_data->tx_queue_state[ethdev_qid] = RTE_ETH_QUEUE_STATE_STOPPED;
  }
}
...
int sfc_tx_start(struct sfc_adapter *sa) {
  ...
  sfc_log_init(sa, "txq_count = %u (internal %u)", sas->ethdev_txq_count,
               sas->txq_count);

  if (sa->tso) {
    if (!encp->enc_fw_assisted_tso_v2_enabled && !encp->enc_tso_v3_enabled) {
...
static void sfc_efx_tx_reap(struct sfc_efx_txq *txq) {
  unsigned int completed;

  sfc_ev_qpoll(txq->evq);

  for (completed = txq->completed; completed != txq->pending; completed++) {
 ...
 */
static unsigned int sfc_efx_tx_maybe_insert_tag(struct sfc_efx_txq *txq,
                                                struct rte_mbuf *m,
                                                efx_desc_t **pend) {
  uint16_t this_tag = ((m->ol_flags & RTE_MBUF_F_TX_VLAN) ? m->vlan_tci : 0);
  ...
  efx_tx_qdesc_vlantci_create(txq->common, rte_cpu_to_be_16(this_tag), *pend);
  (*pend)++;
...
static uint16_t sfc_efx_prepare_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                                     uint16_t nb_pkts) {
...
static uint16_t sfc_efx_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                                  uint16_t nb_pkts) {
  ...
  for (pkts_sent = 0, pktp = &tx_pkts[0];
    ...
    if (m_seg->ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
       ...
       */
      if (sfc_efx_tso_do(txq, added, &m_seg, &in_off, &pend, &pkt_descs,
                         &pkt_len) != 0) {
    ...
    for (; m_seg != NULL; m_seg = m_seg->next) {
      ...
      do {
        ...
        efx_tx_qdesc_dma_create(txq->common, frag_addr, frag_len,
                                (pkt_len == 0), pend++);
  ...
  if (likely(pkts_sent > 0)) {
    rc = efx_tx_qdesc_post(txq->common, txq->pend_desc,
                           pend - &txq->pend_desc[0], txq->completed,
                           &txq->added);
    SFC_ASSERT(rc == 0);
...
sfc_efx_tx_qsize_up_rings(uint16_t nb_tx_desc,
                          ...
                          unsigned int *txq_entries, unsigned int *evq_entries,
                          unsigned int *txq_max_fill_level) {
...
static int sfc_efx_tx_qcreate(uint16_t port_id, uint16_t queue_id,
  ...
  rc = ENOMEM;
  txq = rte_zmalloc_socket("sfc-efx-txq", sizeof(*txq), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (txq == NULL)
    goto fail_txq_alloc;
  ...
  txq->pend_desc = rte_calloc_socket("sfc-efx-txq-pend-desc",
                                     ...
                                     sizeof(*txq->pend_desc), 0, socket_id);
  if (txq->pend_desc == NULL)
    goto fail_pend_desc_alloc;

  rc = ENOMEM;
  txq->sw_ring =
      rte_calloc_socket("sfc-efx-txq-sw_ring", info->txq_entries,
  ...
  if (ctrl_txq->evq->sa->tso) {
    rc =
        sfc_efx_tso_alloc_tsoh_objs(txq->sw_ring, info->txq_entries, socket_id);
    if (rc != 0)
...
static void sfc_efx_tx_qdestroy(struct sfc_dp_txq *dp_txq) {
  struct sfc_efx_txq *txq = sfc_efx_txq_by_dp_txq(dp_txq);
...
static int sfc_efx_tx_qstart(struct sfc_dp_txq *dp_txq,
...
static void sfc_efx_tx_qstop(struct sfc_dp_txq *dp_txq,
                             __rte_unused unsigned int *evq_read_ptr) {
  struct sfc_efx_txq *txq = sfc_efx_txq_by_dp_txq(dp_txq);
...
static void sfc_efx_tx_qreap(struct sfc_dp_txq *dp_txq) {
  struct sfc_efx_txq *txq = sfc_efx_txq_by_dp_txq(dp_txq);
  unsigned int txds;
...
static int sfc_efx_tx_qdesc_status(struct sfc_dp_txq *dp_txq, uint16_t offset) {
  struct sfc_efx_txq *txq = sfc_efx_txq_by_dp_txq(dp_txq);
...
struct sfc_dp_tx sfc_efx_tx = {
    ...
    .features = 0,
    .dev_offload_capa =
        RTE_ETH_TX_OFFLOAD_VLAN_INSERT | RTE_ETH_TX_OFFLOAD_MULTI_SEGS,
    .queue_offload_capa =
