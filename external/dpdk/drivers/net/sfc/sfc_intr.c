...
static void sfc_intr_handle_mgmt_evq(struct sfc_adapter *sa) {
  struct sfc_evq *evq;
...
static void sfc_intr_line_handler(void *cb_arg) {
  ...
  if (sa->state != SFC_ETHDEV_STARTED && sa->state != SFC_ETHDEV_STARTING &&
      sa->state != SFC_ETHDEV_STOPPING) {
    sfc_log_init(sa, "interrupt on stopped adapter, don't reenable");
    goto exit;
  }
...
exit:
  if (lsc_seq != sa->port.lsc_seq) {
    sfc_notice(sa, "link status change event: link %s",
               sa->eth_dev->data->dev_link.link_status ? "UP" : "DOWN");
    rte_eth_dev_callback_process(sa->eth_dev, RTE_ETH_EVENT_INTR_LSC, NULL);
  }
}

static void sfc_intr_message_handler(void *cb_arg) {
  struct sfc_adapter *sa = (struct sfc_adapter *)cb_arg;
  efx_nic_t *enp = sa->nic;
  boolean_t fatal;
  unsigned int lsc_seq = sa->port.lsc_seq;
...
int sfc_intr_start(struct sfc_adapter *sa) {
  ...
  if (intr->handler != NULL) {
    ...
    if (rte_intr_dp_is_en(intr_handle)) {
      if (rte_intr_vec_list_alloc(intr_handle, "intr_vec",
        ...
        sfc_err(sa, "Failed to allocate %d rx_queues intr_vec",
                sa->eth_dev->data->nb_rx_queues);
    ...
    rc = rte_intr_callback_register(intr_handle, intr->handler, (void *)sa);
    if (rc != 0) {
      sfc_err(sa, "cannot register interrupt handler (rc=%d)", rc);
      /*
  ...
  sfc_log_init(
      sa, "done type=%u max_intr=%d nb_efd=%u", rte_intr_type_get(intr_handle),
      rte_intr_max_intr_get(intr_handle), rte_intr_nb_efd_get(intr_handle));
  return 0;
...
void sfc_intr_stop(struct sfc_adapter *sa) {
  ...
  if (intr->handler != NULL) {
    ...
    while ((rc = rte_intr_callback_unregister(intr_handle, intr->handler,
                                              (void *)sa)) == -EAGAIN)
      ;
    if (rc != 1)
      sfc_err(sa, "cannot unregister interrupt handler %d", rc);
  }
