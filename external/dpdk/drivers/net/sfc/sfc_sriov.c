 ...
 */
static bool sriov_mac_addr_assigned(const efx_vport_config_t *vport_config,
  ...
  for (i = 1; i <= sriov->num_vfs; ++i) {
    ...
    do {
      rte_eth_random_addr(vport_config[i].evc_mac_addr);
        ...
        sriov_mac_addr_assigned(vport_config, i, vport_config[i].evc_mac_addr));
  }
...
int sfc_sriov_vswitch_create(struct sfc_adapter *sa) {
  ...
  rc = efx_evb_vswitch_create(sa->nic, sriov->num_vfs + 1, vport_config,
                              &sriov->vswitch);
  if (rc != 0) {
