...
static struct sfc_adapter *sfc_get_adapter_by_pf_port_id(uint16_t pf_port_id) {
...
static void sfc_put_adapter(struct sfc_adapter *sa) { sfc_adapter_unlock(sa); }
...
static int sfc_repr_proxy_mbox_send(struct sfc_repr_proxy_mbox *mbox,
                                    struct sfc_repr_proxy_port *port,
  ...
  if (i == wait_ms) {
    SFC_GENERIC_LOG(
        ERR, "%s() failed to wait for representor proxy routine ack", __func__);
    return ETIMEDOUT;
  }
...
static void sfc_repr_proxy_mbox_handle(struct sfc_repr_proxy *rp) {
...
static void sfc_repr_proxy_handle_tx(struct sfc_repr_proxy_dp_txq *rp_txq,
  ...
  if (rp_txq->available < RTE_DIM(rp_txq->tx_pkts)) {
    rp_txq->available += rte_ring_sc_dequeue_burst(
        repr_txq->ring, (void **)(&rp_txq->tx_pkts[rp_txq->available]),
        RTE_DIM(rp_txq->tx_pkts) - rp_txq->available, NULL);
  ...
  rp_txq->transmitted +=
      rp_txq->pkt_burst(rp_txq->dp, &rp_txq->tx_pkts[rp_txq->transmitted],
 ...
 */
static bool sfc_repr_proxy_rx_route(struct sfc_repr_proxy *rp,
                                    struct sfc_repr_proxy_dp_rxq *rp_rxq) {
  unsigned int i;

  for (i = rp_rxq->routed; i < rp_rxq->available && !rp_rxq->stop_route;
       i++, rp_rxq->routed++) {
    struct sfc_repr_proxy_port *port;
...
static void sfc_repr_proxy_rx_forward(struct sfc_repr_proxy *rp,
                                      struct sfc_repr_proxy_dp_rxq *rp_rxq) {
  struct sfc_repr_proxy_port *port;
  ...
  if (rp_rxq->route_port_id != SFC_REPR_INVALID_ROUTE_PORT_ID) {
    ...
    if (port != NULL && port->started) {
      rp_rxq->forwarded += rte_ring_sp_enqueue_burst(
          port->rxq[0].ring, (void **)(&rp_rxq->pkts[rp_rxq->forwarded]),
...
static void sfc_repr_proxy_handle_rx(struct sfc_repr_proxy *rp,
                                     struct sfc_repr_proxy_dp_rxq *rp_rxq) {
  bool route_again;

  if (rp_rxq->available < RTE_DIM(rp_rxq->pkts)) {
    rp_rxq->available +=
        rp_rxq->pkt_burst(rp_rxq->dp, &rp_rxq->pkts[rp_rxq->available],
...
static int32_t sfc_repr_proxy_routine(void *arg) {
  struct sfc_repr_proxy_port *port;
...
static int sfc_repr_proxy_txq_attach(struct sfc_adapter *sa) {
...
static void sfc_repr_proxy_txq_detach(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  unsigned int i;
...
int sfc_repr_proxy_txq_init(struct sfc_adapter *sa) {
  ...
  for (init_i = 0; init_i < sfc_repr_nb_txq(sas); init_i++) {
    ...
    if (txq_info->state == SFC_TXQ_INITIALIZED) {
      sfc_log_init(sa, "representor proxy TxQ %u is already initialized - skip",
                   init_i);
      continue;
    }
    ...
    rc = sfc_tx_qinit(sa, txq->sw_index, SFC_REPR_PROXY_TX_DESC_COUNT,
                      sa->socket_id, &tx_conf);

    if (rc != 0) {
      sfc_err(sa, "failed to init representor proxy TxQ %u", init_i);
      goto fail_init;
    }
  }
...
void sfc_repr_proxy_txq_fini(struct sfc_adapter *sa) {
  ...
  for (i = 0; i < sfc_repr_nb_txq(sas); i++) {
    ...
    txq_info = &sfc_sa2shared(sa)->txq_info[txq->sw_index];
    if (txq_info->state != SFC_TXQ_INITIALIZED) {
      sfc_log_init(sa, "representor proxy TxQ %u is already finalized - skip",
                   i);
      continue;
    }
...
static int sfc_repr_proxy_txq_start(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  unsigned int i;
...
static void sfc_repr_proxy_txq_stop(struct sfc_adapter *sa) {
  sfc_log_init(sa, "entry");
  sfc_log_init(sa, "done");
}

static int sfc_repr_proxy_rxq_attach(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  unsigned int i;
...
static void sfc_repr_proxy_rxq_detach(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  unsigned int i;
...
static int sfc_repr_proxy_rxq_init(struct sfc_adapter *sa,
...
static void sfc_repr_proxy_rxq_fini(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  struct sfc_rxq_info *rxq_info;
  unsigned int i;
  ...
  for (i = 0; i < sfc_repr_nb_rxq(sas); i++) {
    ...
    rxq_info = &sas->rxq_info[rxq->sw_index];
    if (rxq_info->state != SFC_RXQ_INITIALIZED) {
      sfc_log_init(sa, "representor RxQ %u is already finalized - skip", i);
      continue;
    }
...
static void sfc_repr_proxy_rxq_stop(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
  unsigned int i;
...
static int sfc_repr_proxy_rxq_start(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  unsigned int i;
  int rc;
  ...
  for (i = 0; i < sfc_repr_nb_rxq(sas); i++) {
    ...
    if (rc != 0) {
      sfc_err(sa, "failed to init representor proxy RxQ %u", i);
      goto fail_init;
    }

    rc = sfc_rx_qstart(sa, rxq->sw_index);
    if (rc != 0) {
      sfc_err(sa, "failed to start representor proxy RxQ %u", i);
      goto fail_start;
    }
...
static int sfc_repr_proxy_mae_rule_insert(struct sfc_adapter *sa,
                                          struct sfc_repr_proxy_port *port) {
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  ...
  rc = efx_mae_mport_by_id(&port->egress_mport, &mport_vf_selector);
  if (rc != 0) {
    sfc_err(sa, "failed to get VF mport for repr %u", port->repr_id);
    goto fail_get_vf;
  }

  rc = efx_mae_mport_by_id(&rp->mport_alias, &mport_alias_selector);
  if (rc != 0) {
    sfc_err(sa, "failed to get mport selector for repr %u", port->repr_id);
    goto fail_get_alias;
  }

  rc = sfc_mae_rule_add_mport_match_deliver(
      sa, &mport_vf_selector, &mport_alias_selector, -1, &mae_rule);
  if (rc != 0) {
    sfc_err(sa, "failed to insert MAE rule for repr %u", port->repr_id);
    goto fail_rule_add;
  }
...
static void sfc_repr_proxy_mae_rule_remove(struct sfc_adapter *sa,
                                           struct sfc_repr_proxy_port *port) {
...
static int sfc_repr_proxy_mport_filter_insert(struct sfc_adapter *sa) {
  struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  ...
  for (i = 0; i < RTE_DIM(filter->specs); i++) {
    ...
    filter->specs[i].efs_match_flags = flags[i] | EFX_FILTER_MATCH_MPORT;
...
static void sfc_repr_proxy_mport_filter_remove(struct sfc_adapter *sa) {
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  struct sfc_repr_proxy_filter *filter = &rp->mport_filter;
  unsigned int i;
...
static int sfc_repr_proxy_port_rule_insert(struct sfc_adapter *sa,
                                           struct sfc_repr_proxy_port *port) {
  int rc;
...
static void sfc_repr_proxy_port_rule_remove(struct sfc_adapter *sa,
                                            struct sfc_repr_proxy_port *port) {
  sfc_repr_proxy_mae_rule_remove(sa, port);
}

static int sfc_repr_proxy_ports_init(struct sfc_adapter *sa) {
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  int rc;
...
static void sfc_repr_proxy_ports_fini(struct sfc_adapter *sa) {
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
...
int sfc_repr_proxy_attach(struct sfc_adapter *sa) {
  ...
  if (cid == RTE_MAX_LCORE && sa->socket_id != SOCKET_ID_ANY) {
    ...
    sfc_warn(sa, "repr proxy: unable to get service lcore at socket %d",
             sa->socket_id);
  ...
  snprintf(service.name, sizeof(service.name), "net_sfc_%hu_repr_proxy",
           sfc_sa2shared(sa)->port_id);
...
static int sfc_repr_proxy_do_start_port(struct sfc_adapter *sa,
                                        struct sfc_repr_proxy_port *port) {
  struct sfc_repr_proxy *rp = &sa->repr_proxy;
  int rc;
  ...
  if (rp->started) {
    ...
    if (rc != 0) {
      sfc_err(sa, "failed to start proxy port %u", port->repr_id);
...
static int sfc_repr_proxy_do_stop_port(struct sfc_adapter *sa,
                                       struct sfc_repr_proxy_port *port)

{
  ...
  if (rp->started) {
    ...
    if (rc != 0) {
      sfc_err(sa, "failed to stop proxy port %u: %s", port->repr_id,
              rte_strerror(rc));
      return rc;
    }
  } else {
...
static bool sfc_repr_proxy_port_enabled(struct sfc_repr_proxy_port *port) {
  return port->rte_port_id != RTE_MAX_ETHPORTS && port->enabled;
}

static bool sfc_repr_proxy_ports_disabled(struct sfc_repr_proxy *rp) {
  struct sfc_repr_proxy_port *port;
...
int sfc_repr_proxy_start(struct sfc_adapter *sa) {
  ...
  if (rc != 0 && rc != -EALREADY) {
    ...
    sfc_err(sa, "failed to start service core for %s: %s",
            rte_service_get_name(rp->service_id), rte_strerror(rc));
  ...
  if (rc < 0) {
    rc = -rc;
    sfc_err(sa, "failed to run %s: %s", rte_service_get_name(rp->service_id),
            rte_strerror(rc));
...
void sfc_repr_proxy_stop(struct sfc_adapter *sa) {
  ...
  TAILQ_FOREACH(port, &rp->ports, entries) {
    if (sfc_repr_proxy_port_enabled(port)) {
      ...
      if (rc != 0) {
        sfc_err(sa, "failed to stop representor proxy port %u: %s",
  ...
  if (rc < 0) {
    sfc_err(sa, "failed to stop %s: %s", rte_service_get_name(rp->service_id),
            rte_strerror(-rc));
  }
...
int sfc_repr_proxy_add_port(uint16_t pf_port_id, uint16_t repr_id,
  ...
  port = rte_zmalloc("sfc-repr-proxy-port", sizeof(*port), sa->socket_id);
  if (port == NULL) {
    rc = ENOMEM;
  ...
  rc = efx_mae_mport_id_by_selector(sa->nic, mport_sel, &port->egress_mport);
  if (rc != 0) {
    sfc_err(sa, "failed get MAE mport id by selector (repr_id %u): %s", repr_id,
            rte_strerror(rc));
    goto fail_mport_id;
  }
  ...
  if (rp->started) {
    rc =
        sfc_repr_proxy_mbox_send(&rp->mbox, port, SFC_REPR_PROXY_MBOX_ADD_PORT);
    if (rc != 0) {
      sfc_err(sa, "failed to add proxy port %u", port->repr_id);
      goto fail_port_add;
    }
  } else {
...
int sfc_repr_proxy_del_port(uint16_t pf_port_id, uint16_t repr_id) {
  ...
  if (rp->started) {
    rc =
        sfc_repr_proxy_mbox_send(&rp->mbox, port, SFC_REPR_PROXY_MBOX_DEL_PORT);
    if (rc != 0) {
      sfc_err(sa, "failed to remove proxy port %u", port->repr_id);
...
int sfc_repr_proxy_start_repr(uint16_t pf_port_id, uint16_t repr_id) {
  ...
  if (port->enabled) {
    rc = EALREADY;
    sfc_err(sa, "failed: repr %u proxy port already started", repr_id);
  ...
  if (sa->state == SFC_ETHDEV_STARTED) {
    if (sfc_repr_proxy_ports_disabled(rp)) {
      ...
      rc = sfc_repr_proxy_do_start_port(sa, port);
      if (rc != 0) {
        sfc_err(sa, "failed to start repr %u proxy port", repr_id);
        goto fail_start_id;
      }
    }
  }
...
int sfc_repr_proxy_stop_repr(uint16_t pf_port_id, uint16_t repr_id) {
  ...
  if (!port->enabled) {
    sfc_log_init(sa, "repr %u proxy port is not started - skip", repr_id);
    sfc_put_adapter(sa);
    return 0;
  }
  ...
  if (sa->state == SFC_ETHDEV_STARTED) {
    ...
    if (rc != 0) {
      sfc_err(sa, "failed to stop representor proxy TxQ %u: %s", repr_id,
              rte_strerror(rc));
      sfc_put_adapter(sa);
      return rc;
    }
