...
int sfc_mae_switch_controller_from_mapping(
    const efx_pcie_interface_t *controllers, size_t nb_controllers,
    efx_pcie_interface_t intf, int *controller) {
  size_t i;
...
int sfc_mae_switch_domain_get_controller(uint16_t switch_domain_id,
  ...
  return sfc_mae_switch_controller_from_mapping(controllers, nb_controllers,
                                                intf, controller);
}

int sfc_mae_switch_domain_get_intf(uint16_t switch_domain_id, int controller,
                                   efx_pcie_interface_t *intf) {
  const efx_pcie_interface_t *controllers;
  size_t nb_controllers;
  int rc;
...
sfc_mae_find_switch_port_by_entity(const struct sfc_mae_switch_domain *domain,
  ...
  TAILQ_FOREACH(port, &domain->ports, switch_domain_ports) {
    if (port->entity_mport.sel == entity_mportp->sel && port->type == type)
      return port;
  }
...
/* This function expects to be called only when the lock is held */
static int sfc_mae_find_switch_port_id_by_entity(
    uint16_t switch_domain_id, const efx_mport_sel_t *entity_mportp,
    enum sfc_mae_switch_port_type type, uint16_t *switch_port_id) {
  struct sfc_mae_switch_domain *domain;
  struct sfc_mae_switch_port *port;
...
int sfc_mae_assign_switch_port(uint16_t switch_domain_id,
  ...
  port =
      sfc_mae_find_switch_port_by_entity(domain, req->entity_mportp, req->type);
  if (port != NULL)
    goto done;
  ...
  memcpy(&port->data, &req->port_data, sizeof(port->data));
...
int sfc_mae_switch_get_ethdev_mport(uint16_t switch_domain_id,
  ...
  rte_spinlock_lock(&sfc_mae_switch.lock);
  rc = sfc_mae_find_switch_port_by_ethdev(switch_domain_id, ethdev_port_id,
                                          &port);
  if (rc != 0)
    goto unlock;
...
int sfc_mae_switch_port_id_by_entity(uint16_t switch_domain_id,
  ...
  rte_spinlock_lock(&sfc_mae_switch.lock);
  rc = sfc_mae_find_switch_port_id_by_entity(switch_domain_id, entity_mportp,
                                             type, switch_port_id);
  rte_spinlock_unlock(&sfc_mae_switch.lock);
...
static int sfc_mae_get_switch_domain_admin_locked(uint16_t switch_domain_id,
                                                  uint16_t *port_id) {
  struct sfc_mae_switch_domain *domain;
