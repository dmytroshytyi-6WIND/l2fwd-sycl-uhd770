...
static __rte_always_inline int iavf_tx_free_bufs(struct iavf_tx_queue *txq) {
...
static __rte_always_inline void tx_backlog_entry(struct iavf_tx_entry *txep,
...
static inline void _iavf_rx_queue_release_mbufs_vec(struct iavf_rx_queue *rxq) {
...
static inline void _iavf_tx_queue_release_mbufs_vec(struct iavf_tx_queue *txq) {
  unsigned i;
...
static inline int iavf_rxq_vec_setup_default(struct iavf_rx_queue *rxq) {
  uintptr_t p;
...
static inline int iavf_rx_vec_queue_default(struct iavf_rx_queue *rxq) {
  if (!rxq)
    return -1;
...
static inline int iavf_tx_vec_queue_default(struct iavf_tx_queue *txq) {
  if (!txq)
    return -1;
...
static inline int iavf_rx_vec_dev_check_default(struct rte_eth_dev *dev) {
  int i;
...
static inline int iavf_tx_vec_dev_check_default(struct rte_eth_dev *dev) {
  int i;
...
#ifdef IAVF_TX_CSUM_OFFLOAD
  /* Set MACLEN */
  td_offset |= (tx_pkt->l2_len >> 1) << IAVF_TX_DESC_LENGTH_MACLEN_SHIFT;

  /* Enable L3 checksum offloads */
  if (ol_flags & RTE_MBUF_F_TX_IP_CKSUM) {
    td_cmd |= IAVF_TX_DESC_CMD_IIPT_IPV4_CSUM;
    td_offset |= (tx_pkt->l3_len >> 2) << IAVF_TX_DESC_LENGTH_IPLEN_SHIFT;
...
iavf_rxq_rearm_common(struct iavf_rx_queue *rxq, __rte_unused bool avx512) {
  ...
  if (rte_mempool_get_bulk(rxq->mp, (void *)rxp, IAVF_RXQ_REARM_THRESH) < 0) {
  ...
  if (avx512) {
    ...
    for (i = 0; i < IAVF_RXQ_REARM_THRESH; i += 8, rxp += 8, rxdp += 8) {
      ...
      vaddr4_5 =
          _mm256_inserti128_si256(_mm256_castsi128_si256(vaddr4), vaddr5, 1);
      vaddr6_7 =
          _mm256_inserti128_si256(_mm256_castsi128_si256(vaddr6), vaddr7, 1);
      vaddr0_3 =
          _mm512_inserti64x4(_mm512_castsi256_si512(vaddr0_1), vaddr2_3, 1);
      vaddr4_7 =
          _mm512_inserti64x4(_mm512_castsi256_si512(vaddr4_5), vaddr6_7, 1);
...
#endif
  {
    ...
    for (i = 0; i < IAVF_RXQ_REARM_THRESH; i += 4, rxp += 4, rxdp += 4) {
      __m128i vaddr0, vaddr1, vaddr2, vaddr3;
      __m256i vaddr0_1, vaddr2_3;
