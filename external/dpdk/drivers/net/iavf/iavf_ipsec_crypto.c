 ...
 */
static inline uint8_t calc_ipsec_desc_iv_len_field(uint16_t iv_sz) {
...
get_capability(struct iavf_security_ctx *iavf_sctx, uint32_t algo,
               uint32_t type) {
...
static uint16_t get_cipher_blocksize(struct iavf_security_ctx *iavf_sctx,
                                     enum rte_crypto_cipher_algorithm algo) {
...
static uint16_t get_aead_blocksize(struct iavf_security_ctx *iavf_sctx,
                                   enum rte_crypto_aead_algorithm algo) {
  const struct rte_cryptodev_symmetric_capability *capability;
...
static uint16_t get_auth_blocksize(struct iavf_security_ctx *iavf_sctx,
                                   enum rte_crypto_auth_algorithm algo) {
  const struct rte_cryptodev_symmetric_capability *capability;
...
static uint8_t calc_context_desc_cipherblock_sz(size_t len) {
  switch (len) {
  case 8:
    return 0x2;
  case 16:
...
static int valid_length(uint32_t len, uint32_t min, uint32_t max,
                        uint32_t increment) {
  if (len < min || len > max)
    return false;
...
static int valid_auth_xform(struct iavf_security_ctx *iavf_sctx,
  ...
  /* verify key size */
  if (!valid_length(auth->key.length, capability->auth.key_size.min,
...
static int valid_cipher_xform(struct iavf_security_ctx *iavf_sctx,
  ...
  /* verify key size */
  if (!valid_length(cipher->key.length, capability->cipher.key_size.min,
...
static int valid_aead_xform(struct iavf_security_ctx *iavf_sctx,
  ...
  /* verify key size */
  if (!valid_length(aead->key.length, capability->aead.key_size.min,
...
static int iavf_ipsec_crypto_session_validate_conf(
    struct iavf_security_ctx *iavf_sctx,
  ...
  if (conf->ipsec.options.copy_dscp || conf->ipsec.options.copy_flabel ||
      conf->ipsec.options.copy_df || conf->ipsec.options.dec_ttl ||
      conf->ipsec.options.ecn || conf->ipsec.options.stats) {
  ...
  if (conf->crypto_xform->type == RTE_CRYPTO_SYM_XFORM_AEAD) {
    ...
    if (!valid_cipher_xform(iavf_sctx, &conf->crypto_xform->cipher)) {
      PMD_DRV_LOG(ERR, "Invalid IPsec option specified");
      return -EINVAL;
    }

    if (!valid_auth_xform(iavf_sctx, &conf->crypto_xform->next->auth)) {
      PMD_DRV_LOG(ERR, "Invalid IPsec option specified");
      return -EINVAL;
    }
    ...
    if (!valid_cipher_xform(iavf_sctx, &conf->crypto_xform->next->cipher)) {
      PMD_DRV_LOG(ERR, "Invalid IPsec option specified");
      return -EINVAL;
    }
  }
...
static void sa_add_set_aead_params(struct virtchnl_ipsec_crypto_cfg_item *cfg,
                                   struct rte_crypto_aead_xform *aead,
                                   uint32_t salt) {
  ...
  case RTE_CRYPTO_AEAD_AES_CCM:
    cfg->algo_type = VIRTCHNL_AES_CCM;
    break;
  case RTE_CRYPTO_AEAD_AES_GCM:
    cfg->algo_type = VIRTCHNL_AES_GCM;
    break;
  case RTE_CRYPTO_AEAD_CHACHA20_POLY1305:
    cfg->algo_type = VIRTCHNL_CHACHA20_POLY1305;
    break;
  default:
...
static void sa_add_set_cipher_params(struct virtchnl_ipsec_crypto_cfg_item *cfg,
                                     struct rte_crypto_cipher_xform *cipher,
                                     uint32_t salt) {
  ...
  case RTE_CRYPTO_CIPHER_AES_CBC:
    cfg->algo_type = VIRTCHNL_AES_CBC;
    break;
  case RTE_CRYPTO_CIPHER_3DES_CBC:
    cfg->algo_type = VIRTCHNL_3DES_CBC;
    break;
  case RTE_CRYPTO_CIPHER_NULL:
    cfg->algo_type = VIRTCHNL_CIPHER_NO_ALG;
    break;
...
static void sa_add_set_auth_params(struct virtchnl_ipsec_crypto_cfg_item *cfg,
                                   struct rte_crypto_auth_xform *auth,
                                   uint32_t salt) {
  ...
  case RTE_CRYPTO_AUTH_NULL:
    cfg->algo_type = VIRTCHNL_HASH_NO_ALG;
    break;
  case RTE_CRYPTO_AUTH_AES_CBC_MAC:
    cfg->algo_type = VIRTCHNL_AES_CBC_MAC;
    break;
  case RTE_CRYPTO_AUTH_AES_CMAC:
    cfg->algo_type = VIRTCHNL_AES_CMAC;
    break;
  case RTE_CRYPTO_AUTH_AES_XCBC_MAC:
    cfg->algo_type = VIRTCHNL_AES_XCBC_MAC;
    break;
  case RTE_CRYPTO_AUTH_MD5_HMAC:
    cfg->algo_type = VIRTCHNL_MD5_HMAC;
    break;
  case RTE_CRYPTO_AUTH_SHA1_HMAC:
    cfg->algo_type = VIRTCHNL_SHA1_HMAC;
    break;
  case RTE_CRYPTO_AUTH_SHA224_HMAC:
    cfg->algo_type = VIRTCHNL_SHA224_HMAC;
    break;
  case RTE_CRYPTO_AUTH_SHA256_HMAC:
    cfg->algo_type = VIRTCHNL_SHA256_HMAC;
    break;
  case RTE_CRYPTO_AUTH_SHA384_HMAC:
    cfg->algo_type = VIRTCHNL_SHA384_HMAC;
    break;
  case RTE_CRYPTO_AUTH_SHA512_HMAC:
    cfg->algo_type = VIRTCHNL_SHA512_HMAC;
    break;
 ...
 */
static uint32_t iavf_ipsec_crypto_security_association_add(
    struct iavf_adapter *adapter, struct rte_security_session_conf *conf) {
  ...
  request_len =
      sizeof(struct inline_ipsec_msg) + sizeof(struct virtchnl_ipsec_sa_cfg);
  ...
  rc = iavf_ipsec_crypto_request(adapter, (uint8_t *)request, request_len,
...
static void set_session_parameter(struct iavf_security_ctx *iavf_sctx,
                                  struct iavf_security_session *sess,
                                  struct rte_security_session_conf *conf,
                                  uint32_t sa_idx) {
  ...
  if (conf->crypto_xform->type == RTE_CRYPTO_SYM_XFORM_AEAD) {
    sess->block_sz =
        get_aead_blocksize(iavf_sctx, conf->crypto_xform->aead.algo);
    sess->iv_sz = sizeof(uint64_t); /* iv.length includes salt */
    sess->icv_sz = conf->crypto_xform->aead.digest_length;
  } else if (conf->crypto_xform->type == RTE_CRYPTO_SYM_XFORM_CIPHER) {
    sess->block_sz =
        get_cipher_blocksize(iavf_sctx, conf->crypto_xform->cipher.algo);
    sess->iv_sz = conf->crypto_xform->cipher.iv.length;
    sess->icv_sz = conf->crypto_xform->next->auth.digest_length;
  } else if (conf->crypto_xform->type == RTE_CRYPTO_SYM_XFORM_AUTH) {
    if (conf->crypto_xform->auth.algo == RTE_CRYPTO_AUTH_AES_GMAC) {
      sess->block_sz =
          get_auth_blocksize(iavf_sctx, conf->crypto_xform->auth.algo);
      sess->iv_sz = sizeof(uint64_t); /* iv len inc. salt */
      sess->icv_sz = conf->crypto_xform->auth.digest_length;
    } else {
      sess->block_sz = get_cipher_blocksize(
          iavf_sctx, conf->crypto_xform->next->cipher.algo);
      sess->iv_sz = conf->crypto_xform->next->cipher.iv.length;
      sess->icv_sz = conf->crypto_xform->auth.digest_length;
    }
  }
 ...
 */
static int iavf_ipsec_crypto_session_create(
    void *device, struct rte_security_session_conf *conf,
    struct rte_security_session *session, struct rte_mempool *mempool) {
  ...
  if (sa_idx < 0) {
    PMD_DRV_LOG(ERR, "Failed to add SA (spi: %d, mode: %s, direction: %s)",
                conf->ipsec.spi,
                conf->ipsec.mode == RTE_SECURITY_IPSEC_SA_MODE_TRANSPORT
                    ? "transport"
                    : "tunnel",
                conf->ipsec.direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS
                    ? "inbound"
 ...
 * conversion of these parameters.
 */
int iavf_ipsec_crypto_inbound_security_policy_add(
    struct iavf_adapter *adapter, uint32_t esp_spi, uint8_t is_v4,
    rte_be32_t v4_dst_addr, uint8_t *v6_dst_addr, uint8_t drop, bool is_udp,
    uint16_t udp_port) {
  struct inline_ipsec_msg *request = NULL, *response = NULL;
  size_t request_len, response_len;
  int rc = 0;
  ...
  request_len =
      sizeof(struct inline_ipsec_msg) + sizeof(struct virtchnl_ipsec_sp_cfg);
  request =
      rte_malloc("iavf-inbound-security-policy-add-request", request_len, 0);
  if (request == NULL) {
    rc = -ENOMEM;
    goto update_cleanup;
  }
  ...
  /* Destination IP  */
  if (is_v4) {
    request->ipsec_data.sp_cfg->table_id = VIRTCHNL_IPSEC_INBOUND_SPD_TBL_IPV4;
    request->ipsec_data.sp_cfg->dip[0] = htonl(v4_dst_addr);
  } else {
    request->ipsec_data.sp_cfg->table_id = VIRTCHNL_IPSEC_INBOUND_SPD_TBL_IPV6;
    request->ipsec_data.sp_cfg->dip[0] = htonl(((uint32_t *)v6_dst_addr)[0]);
    request->ipsec_data.sp_cfg->dip[1] = htonl(((uint32_t *)v6_dst_addr)[1]);
    request->ipsec_data.sp_cfg->dip[2] = htonl(((uint32_t *)v6_dst_addr)[2]);
    request->ipsec_data.sp_cfg->dip[3] = htonl(((uint32_t *)v6_dst_addr)[3]);
  }
  ...
  response =
      rte_malloc("iavf-inbound-security-policy-add-response", response_len, 0);
  if (response == NULL) {
    rc = -ENOMEM;
    goto update_cleanup;
  }
...
iavf_ipsec_crypto_sa_update_esn(struct iavf_adapter *adapter,
  ...
  request_len =
      sizeof(struct inline_ipsec_msg) + sizeof(struct virtchnl_ipsec_sa_update);
  ...
  response_len =
      sizeof(struct inline_ipsec_msg) + sizeof(struct virtchnl_ipsec_resp);
...
iavf_ipsec_crypto_session_update(void *device,
  ...
  if (iavf_sess->esn.enabled && conf->ipsec.options.esn) {
    ...
    if (iavf_sess->direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS)
      rc = iavf_ipsec_crypto_sa_update_esn(adapter, iavf_sess);
    else
...
static int iavf_ipsec_crypto_session_stats_get(
...
int iavf_ipsec_crypto_security_policy_delete(struct iavf_adapter *adapter,
  ...
  request->ipsec_data.sp_destroy->table_id =
      is_v4 ? VIRTCHNL_IPSEC_INBOUND_SPD_TBL_IPV4
...
static uint32_t iavf_ipsec_crypto_sa_del(struct iavf_adapter *adapter,
                                         struct iavf_security_session *sess) {
  struct inline_ipsec_msg *request = NULL, *response = NULL;
  size_t request_len, response_len;
...
iavf_ipsec_crypto_get_esp_trailer(struct rte_mbuf *m,
                                  struct iavf_security_session *s,
...
static inline uint16_t iavf_ipsec_crypto_compute_l4_payload_length(
    struct rte_mbuf *m, struct iavf_security_session *s, uint16_t esp_tlen) {
  ...
  return rte_pktmbuf_pkt_len(m) -
         (ol2_len + ol3_len + ol4_len + esp_hlen + l3_len + l4_len + esp_tlen);
}
...
iavf_ipsec_crypto_pkt_metadata_set(void *device,
  ...
  esp_tail =
      iavf_ipsec_crypto_get_esp_trailer(m, iavf_sess, &esp_trailer_length);

  /* Set per packet mutable metadata values */
  md->esp_trailer_len = esp_trailer_length;
  md->l4_payload_len = iavf_ipsec_crypto_compute_l4_payload_length(
      m, iavf_sess, esp_trailer_length);
...
static int iavf_ipsec_crypto_device_capabilities_get(
    struct iavf_adapter *adapter, struct virtchnl_ipsec_cap *capability) {
  /* Perform pf-vf comms */
  struct inline_ipsec_msg *request = NULL, *response = NULL;
  size_t request_len, response_len;
  int rc;
  ...
  response_len =
      sizeof(struct inline_ipsec_msg) + sizeof(struct virtchnl_ipsec_cap);
  response = rte_malloc("iavf-device-capability-response", response_len, 0);
  if (response == NULL) {
    rc = -ENOMEM;
    goto update_cleanup;
  }
...
static void update_auth_capabilities(struct rte_cryptodev_capabilities *scap,
...
static void update_cipher_capabilities(struct rte_cryptodev_capabilities *scap,
                                       struct virtchnl_algo_cap *acap) {
  struct rte_cryptodev_symmetric_capability *capability = &scap->sym;
...
static void update_aead_capabilities(struct rte_cryptodev_capabilities *scap,
                                     struct virtchnl_algo_cap *acap) {
  struct rte_cryptodev_symmetric_capability *capability = &scap->sym;
 ...
 */
int iavf_ipsec_crypto_set_security_capabililites(
    struct iavf_security_ctx *iavf_sctx, struct virtchnl_ipsec_cap *vch_cap) {
  ...
  for (i = 0; i < VIRTCHNL_IPSEC_MAX_CRYPTO_CAP_NUM; i++) {
    for (j = 0; j < vch_cap->cap[i].algo_cap_num; j++, ci++) {
      ...
      case VIRTCHNL_AEAD:
        ...
        break;
      default:
        capabilities[ci].op = RTE_CRYPTO_OP_TYPE_UNDEFINED;
        break;
      }
    }
  }
...
iavf_ipsec_crypto_capabilities_get(void *device) {
       ...
       .ipsec =
           {
               ...
               .options = {.udp_encap = 1, .stats = 1, .esn = 1},
           },
  ...
  for (i = 0; i < ((sizeof(iavf_security_capabilities) /
                    sizeof(iavf_security_capabilities[0])) -
                   1);
       i++) {
...
int iavf_security_ctx_create(struct iavf_adapter *adapter) {
  ...
  if (adapter->security_ctx == NULL) {
    adapter->security_ctx =
        rte_malloc("iavf_security_ctx", sizeof(struct iavf_security_ctx), 0);
    if (adapter->security_ctx == NULL)
      return -ENOMEM;
  }
...
int iavf_security_init(struct iavf_adapter *adapter) {
  ...
  return iavf_ipsec_crypto_set_security_capabililites(iavf_sctx, &capabilities);
}
...
int iavf_security_ctx_destroy(struct iavf_adapter *adapter) {
...
int iavf_ipsec_crypto_supported(struct iavf_adapter *adapter) {
  ...
  if (resources &&
      (resources->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_INLINE_IPSEC_CRYPTO))
    return true;

  return false;
}

#define IAVF_IPSEC_INSET_ESP (IAVF_INSET_ESP_SPI)

#define IAVF_IPSEC_INSET_AH (IAVF_INSET_AH_SPI)

#define IAVF_IPSEC_INSET_IPV4_NATT_ESP                                         \
  (IAVF_INSET_IPV4_SRC | IAVF_INSET_IPV4_DST | IAVF_INSET_ESP_SPI)
...
static void parse_eth_item(const struct rte_flow_item_eth *item,
                           struct rte_ether_hdr *eth) {
  memcpy(eth->src_addr.addr_bytes, item->src.addr_bytes, sizeof(eth->src_addr));
  memcpy(eth->dst_addr.addr_bytes, item->dst.addr_bytes, sizeof(eth->dst_addr));
}

static void parse_ipv4_item(const struct rte_flow_item_ipv4 *item,
                            struct rte_ipv4_hdr *ipv4) {
  ipv4->src_addr = item->hdr.src_addr;
  ipv4->dst_addr = item->hdr.dst_addr;
}

static void parse_ipv6_item(const struct rte_flow_item_ipv6 *item,
                            struct rte_ipv6_hdr *ipv6) {
  memcpy(ipv6->src_addr, item->hdr.src_addr, 16);
  memcpy(ipv6->dst_addr, item->hdr.dst_addr, 16);
}

static void parse_udp_item(const struct rte_flow_item_udp *item,
                           struct rte_udp_hdr *udp) {
  udp->dst_port = item->hdr.dst_port;
  udp->src_port = item->hdr.src_port;
}

static int has_security_action(const struct rte_flow_action actions[],
                               const void **session) {
...
static struct iavf_ipsec_flow_item *iavf_ipsec_flow_item_parse(
    struct rte_eth_dev *ethdev, const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], uint32_t type) {
  const void *session;
  struct iavf_ipsec_flow_item *ipsec_flow =
      rte_malloc("security-flow-rule", sizeof(struct iavf_ipsec_flow_item), 0);
  ...
  if (pattern[0].spec)
    parse_eth_item((const struct rte_flow_item_eth *)pattern[0].spec,
                   &ipsec_flow->eth_hdr);

  switch (p_type) {
  case IAVF_PATTERN_ESP:
    if (ipsec_flow->is_ipv4) {
      parse_ipv4_item((const struct rte_flow_item_ipv4 *)pattern[1].spec,
                      &ipsec_flow->ipv4_hdr);
    } else {
      parse_ipv6_item((const struct rte_flow_item_ipv6 *)pattern[1].spec,
                      &ipsec_flow->ipv6_hdr);
    }
    ipsec_flow->spi =
        ((const struct rte_flow_item_esp *)pattern[2].spec)->hdr.spi;
    break;
  case IAVF_PATTERN_AH:
    ...
    ipsec_flow->spi = ((const struct rte_flow_item_ah *)pattern[2].spec)->spi;
    break;
  case IAVF_PATTERN_UDP_ESP:
    ...
    parse_udp_item((const struct rte_flow_item_udp *)pattern[2].spec,
                   &ipsec_flow->udp_hdr);
    ipsec_flow->is_udp = true;
    ipsec_flow->spi =
        ((const struct rte_flow_item_esp *)pattern[3].spec)->hdr.spi;
    break;
  default:
    goto flow_cleanup;
  }
  ...
  if (!iavf_ipsec_crypto_action_valid(ethdev, session, ipsec_flow->spi))
    goto flow_cleanup;
...
static int iavf_ipsec_flow_init(struct iavf_adapter *ad) {
...
static void iavf_ipsec_flow_uninit(struct iavf_adapter *ad) {
  iavf_unregister_parser(&iavf_ipsec_flow_parser, ad);
}

static int iavf_ipsec_flow_create(struct iavf_adapter *ad,
                                  struct rte_flow *flow, void *meta,
  ...
  if (ipsec_flow->is_ipv4) {
    ipsec_flow->id = iavf_ipsec_crypto_inbound_security_policy_add(
        ad, ipsec_flow->spi, 1, ipsec_flow->ipv4_hdr.dst_addr, NULL, 0,
        ipsec_flow->is_udp, ipsec_flow->udp_hdr.dst_port);
  } else {
    ipsec_flow->id = iavf_ipsec_crypto_inbound_security_policy_add(
        ad, ipsec_flow->spi, 0, 0, ipsec_flow->ipv6_hdr.dst_addr, 0,
        ipsec_flow->is_udp, ipsec_flow->udp_hdr.dst_port);
  }
...
static int iavf_ipsec_flow_destroy(struct iavf_adapter *ad,
  ...
  iavf_ipsec_crypto_security_policy_delete(ad, ipsec_flow->is_ipv4,
                                           ipsec_flow->id);
  rte_free(ipsec_flow);
  return 0;
}
...
static int iavf_ipsec_flow_parse(struct iavf_adapter *ad,
  ...
  if (item && item->meta) {
    ...
    struct iavf_ipsec_flow_item *fi =
        iavf_ipsec_flow_item_parse(ad->vf.eth_dev, pattern, actions, type);
    if (fi && meta) {
      *meta = fi;
      ret = 0;
    }
