...
static inline uint16_t _iavf_recv_raw_pkts_vec_avx2(struct iavf_rx_queue *rxq,
...
#define IAVF_DESCS_PER_LOOP_AVX 8
  ...
  const __m256i crc_adjust =
      _mm256_set_epi16(               /* first descriptor */
  ...
  const __m256i shuf_msk =
      _mm256_set_epi8(            /* first descriptor */
  ...
  for (i = 0, received = 0; i < nb_pkts;
    ...
    const __m256i status4_7 = _mm256_unpackhi_epi32(desc6_7, desc4_5);
    ...
    /* merge the status bits into one register */
    const __m256i status0_3 = _mm256_unpackhi_epi32(desc2_3, desc0_1);
     ...
     */
    __m256i status0_7 = _mm256_unpacklo_epi64(status4_7, status0_3);
    ...
    const __m256i vlan_flags = _mm256_shuffle_epi8(vlan_flags_shuf, flag_bits);
     ...
     */
    __m256i l3_l4_flags =
        _mm256_shuffle_epi8(l3_l4_flags_shuf, _mm256_srli_epi32(flag_bits, 22));
    l3_l4_flags = _mm256_slli_epi32(l3_l4_flags, 1);
    l3_l4_flags = _mm256_and_si256(l3_l4_flags, cksum_mask);

    /* merge flags */
    const __m256i mbuf_flags =
        _mm256_or_si256(l3_l4_flags, _mm256_or_si256(rss_flags, vlan_flags));
    /**
...
#define IAVF_DESCS_PER_LOOP_AVX 8
  ...
  const __m256i shuf_msk =
      _mm256_set_epi8(                        /* first descriptor */
                      0xFF, 0xFF, 0xFF, 0xFF, /* rss hash parsed separately */
  ...
  for (i = 0, received = 0; i < nb_pkts;
     ...
     * of each 128bits
     */
    const __m256i ptype_mask = _mm256_set1_epi16(IAVF_RX_FLEX_DESC_PTYPE_M);
    ...
    mb4_5 = _mm256_insert_epi32(mb4_5, type_table[ptype4], 0);
    /* merge the status bits into one register */
    const __m256i status4_7 = _mm256_unpackhi_epi32(raw_desc6_7, raw_desc4_5);
    ...
    mb0_1 = _mm256_insert_epi32(mb0_1, type_table[ptype0], 0);
    /* merge the status bits into one register */
    const __m256i status0_3 = _mm256_unpackhi_epi32(raw_desc2_3, raw_desc0_1);
     ...
     */
    __m256i l3_l4_flags =
        _mm256_shuffle_epi8(l3_l4_flags_shuf, _mm256_srli_epi32(flag_bits, 4));
    l3_l4_flags = _mm256_slli_epi32(l3_l4_flags, 1);
    l3_l4_flags = _mm256_and_si256(l3_l4_flags, cksum_mask);
    ...
    if (rxq->fdir_enabled) {
      ...
      const __m256i fdir_id0_7 = _mm256_unpackhi_epi64(fdir_id4_7, fdir_id0_3);

      const __m256i fdir_flags = flex_rxd_to_fdir_flags_vec_avx2(fdir_id0_7);
    ...
    if (offloads & RTE_ETH_RX_OFFLOAD_RSS_HASH ||
      ...
      if (rxq->rx_flags & IAVF_RX_FLAGS_VLAN_TAG_LOC_L2TAG2_2) {
        ...
        vlan_flags = _mm256_shuffle_epi8(l2tag2_flags_shuf, l2tag2p_flag_bits);
...
uint16_t iavf_recv_pkts_vec_avx2_flex_rxd(void *rx_queue,
                                          struct rte_mbuf **rx_pkts,
                                          uint16_t nb_pkts) {
  return _iavf_recv_raw_pkts_vec_avx2_flex_rxd(rx_queue, rx_pkts, nb_pkts,
                                               NULL);
}
 ...
 * - nb_pkts < IAVF_DESCS_PER_LOOP, just return no packet
 */
static uint16_t iavf_recv_scattered_burst_vec_avx2(void *rx_queue,
                                                   struct rte_mbuf **rx_pkts,
                                                   uint16_t nb_pkts) {
  ...
  uint16_t nb_bufs =
      _iavf_recv_raw_pkts_vec_avx2(rxq, rx_pkts, nb_pkts, split_flags);
  if (nb_bufs == 0)
    return 0;
...
uint16_t iavf_recv_scattered_pkts_vec_avx2(void *rx_queue,
  ...
  while (nb_pkts > IAVF_VPMD_RX_MAX_BURST) {
    uint16_t burst = iavf_recv_scattered_burst_vec_avx2(
  ...
  return retval + iavf_recv_scattered_burst_vec_avx2(rx_queue, rx_pkts + retval,
                                                     nb_pkts);
}
 ...
 * - nb_pkts < IAVF_DESCS_PER_LOOP, just return no packet
 */
static uint16_t iavf_recv_scattered_burst_vec_avx2_flex_rxd(
  ...
  /* get some new buffers */
  uint16_t nb_bufs =
      _iavf_recv_raw_pkts_vec_avx2_flex_rxd(rxq, rx_pkts, nb_pkts, split_flags);
  if (nb_bufs == 0)
    return 0;
...
uint16_t iavf_recv_scattered_pkts_vec_avx2_flex_rxd(void *rx_queue,
  ...
  while (nb_pkts > IAVF_VPMD_RX_MAX_BURST) {
    uint16_t burst = iavf_recv_scattered_burst_vec_avx2_flex_rxd(
        rx_queue, rx_pkts + retval, IAVF_VPMD_RX_MAX_BURST);
    retval += burst;
    nb_pkts -= burst;
    if (burst < IAVF_VPMD_RX_MAX_BURST)
  ...
  return retval + iavf_recv_scattered_burst_vec_avx2_flex_rxd(
                      rx_queue, rx_pkts + retval, nb_pkts);
}

static inline void iavf_vtx1(volatile struct iavf_tx_desc *txdp,
...
static inline void iavf_vtx(volatile struct iavf_tx_desc *txdp,
  ...
  for (; nb_pkts > 3; txdp += 4, pkt += 4, nb_pkts -= 4) {
    ...
    __m256i desc2_3 =
        _mm256_set_epi64x(hi_qw3, pkt[3]->buf_iova + pkt[3]->data_off, hi_qw2,
                          pkt[2]->buf_iova + pkt[2]->data_off);
    __m256i desc0_1 =
        _mm256_set_epi64x(hi_qw1, pkt[1]->buf_iova + pkt[1]->data_off, hi_qw0,
...
static inline uint16_t iavf_xmit_fixed_burst_vec_avx2(void *tx_queue,
...
uint16_t iavf_xmit_pkts_vec_avx2(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  while (nb_pkts) {
    ...
    ret = iavf_xmit_fixed_burst_vec_avx2(tx_queue, &tx_pkts[nb_tx], num);
    nb_tx += ret;
    nb_pkts -= ret;
    if (ret < num)
      break;
