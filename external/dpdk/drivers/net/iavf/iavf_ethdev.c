...
static const char *const iavf_valid_args[] = {IAVF_PROTO_XTR_ARG, NULL};
...
__rte_unused static void iavf_dev_watchdog(void *cb_arg) {
  ...
  /* re-alarm watchdog */
  rc = rte_eal_alarm_set(IAVF_DEV_WATCHDOG_PERIOD, &iavf_dev_watchdog, cb_arg);
...
iavf_dev_watchdog_enable(struct iavf_adapter *adapter __rte_unused) {
  ...
  if (rte_eal_alarm_set(IAVF_DEV_WATCHDOG_PERIOD, &iavf_dev_watchdog,
                        (void *)adapter))
...
static int iavf_dev_rx_queue_intr_disable(struct rte_eth_dev *dev,
  ...
  IAVF_WRITE_REG(hw, IAVF_VFINT_DYN_CTLN1(msix_intr - IAVF_RX_VEC_START), 0);
