...
static enum iavf_aq_result iavf_read_msg_from_pf(struct iavf_adapter *adapter,
                                                 uint16_t buf_len,
                                                 uint8_t *buf) {
  ...
  PMD_DRV_LOG(DEBUG, "AQ from pf carries opcode %u, retval %d", opcode,
              vf->cmd_retval);

  if (opcode == VIRTCHNL_OP_EVENT) {
    struct virtchnl_pf_event *vpe = (struct virtchnl_pf_event *)event.msg_buf;
    ...
    case VIRTCHNL_EVENT_LINK_CHANGE:
      ...
      PMD_DRV_LOG(INFO, "Link status update:%s", vf->link_up ? "up" : "down");
      break;
    ...
    case VIRTCHNL_EVENT_PF_DRIVER_CLOSE:
      ...
      break;
    default:
      PMD_DRV_LOG(ERR, "%s: Unknown event %d from pf", __func__, vpe->event);
    }
  } else {
    /* async reply msg on command issued by vf previously */
    result = IAVF_MSG_CMD;
    if (opcode != vf->pend_cmd) {
      PMD_DRV_LOG(WARNING, "command mismatch, expect %u, get %u", vf->pend_cmd,
                  opcode);
      result = IAVF_MSG_ERR;
    }
  }
...
static int iavf_execute_vf_cmd(struct iavf_adapter *adapter,
  ...
  ret = iavf_aq_send_msg_to_pf(hw, args->ops, IAVF_SUCCESS, args->in_args,
  ...
  case VIRTCHNL_OP_GET_OFFLOAD_VLAN_V2_CAPS:
    ...
    do {
      result = iavf_read_msg_from_pf(adapter, args->out_size, args->out_buffer);
      if (result == IAVF_MSG_CMD)
        break;
      iavf_msec_delay(ASQ_DELAY_MS);
    } while (i++ < MAX_TRY_TIMES);
    if (i >= MAX_TRY_TIMES || vf->cmd_retval != VIRTCHNL_STATUS_SUCCESS) {
      err = -1;
      PMD_DRV_LOG(ERR,
  ...
  case VIRTCHNL_OP_REQUEST_QUEUES:
    ...
    do {
      ...
      if (result == IAVF_MSG_SYS && vf->vf_reset) {
        break;
      } else if (result == IAVF_MSG_CMD || result == IAVF_MSG_ERR) {
        err = -1;
        break;
      }
      iavf_msec_delay(ASQ_DELAY_MS);
  ...
  default:
    ...
    if (i >= MAX_TRY_TIMES) {
      ...
      PMD_DRV_LOG(ERR, "Return failure %d for cmd %d", vf->cmd_retval,
                  args->ops);
      err = -EINVAL;
    }
    break;
...
static void iavf_handle_pf_event_msg(struct rte_eth_dev *dev, uint8_t *msg,
                                     uint16_t msglen) {
...
void iavf_handle_virtchnl_msg(struct rte_eth_dev *dev) {
  ...
  while (pending) {
    ...
    msg_ret = (enum iavf_status)rte_le_to_cpu_32(info.desc.cookie_low);
    switch (aq_opc) {
    case iavf_aqc_opc_send_msg_to_vf:
      if (msg_opc == VIRTCHNL_OP_EVENT) {
        iavf_handle_pf_event_msg(dev, info.msg_buf, info.msg_len);
      } else {
        /* check for inline IPsec events */
        struct inline_ipsec_msg *imsg = (struct inline_ipsec_msg *)info.msg_buf;
        struct rte_eth_event_ipsec_desc desc;
        if (msg_opc == VIRTCHNL_OP_INLINE_IPSEC_CRYPTO &&
            imsg->ipsec_opcode == INLINE_IPSEC_OP_EVENT) {
          struct virtchnl_ipsec_event *ev = imsg->ipsec_data.event;
          desc.subtype = RTE_ETH_EVENT_IPSEC_UNKNOWN;
          desc.metadata = ev->ipsec_event_data;
          rte_eth_dev_callback_process(dev, RTE_ETH_EVENT_IPSEC, &desc);
          return;
        }
        ...
        if (msg_opc == vf->pend_cmd) {
          uint32_t cmd_count =
              __atomic_sub_fetch(&vf->pend_cmd_count, 1, __ATOMIC_RELAXED);
          if (cmd_count == 0)
            _notify_cmd(vf, msg_ret);
        } else {
          PMD_DRV_LOG(ERR, "command mismatch, expect %u, get %u", vf->pend_cmd,
                      msg_opc);
        }
        PMD_DRV_LOG(DEBUG, "adminq response is received, opcode = %d", msg_opc);
      }
      break;
    default:
      PMD_DRV_LOG(DEBUG, "Request %u is not supported yet", aq_opc);
      break;
    }
  }
}
...
int iavf_check_api_version(struct iavf_adapter *adapter) {
  ...
  if (vf->virtchnl_version.major < VIRTCHNL_VERSION_MAJOR_START ||
    ...
    PMD_INIT_LOG(ERR,
                 ...
                 VIRTCHNL_VERSION_MAJOR_START, VIRTCHNL_VERSION_MAJOR_START);
    return -1;
    ...
    PMD_INIT_LOG(ERR, "PF/VF API version mismatch:(%u.%u)-(%u.%u)",
                 vf->virtchnl_version.major, vf->virtchnl_version.minor,
...
int iavf_get_vf_resource(struct iavf_adapter *adapter) {
  ...
  caps = IAVF_BASIC_OFFLOAD_CAPS | VIRTCHNL_VF_CAP_ADV_LINK_SPEED |
         VIRTCHNL_VF_OFFLOAD_RX_FLEX_DESC | VIRTCHNL_VF_OFFLOAD_FDIR_PF |
         VIRTCHNL_VF_OFFLOAD_ADV_RSS_PF | VIRTCHNL_VF_OFFLOAD_REQ_QUEUES |
         VIRTCHNL_VF_OFFLOAD_CRC | VIRTCHNL_VF_OFFLOAD_VLAN_V2 |
         VIRTCHNL_VF_LARGE_NUM_QPAIRS | VIRTCHNL_VF_OFFLOAD_QOS |
  ...
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_GET_VF_RESOURCE");
    return -1;
  }
  ...
  rte_memcpy(vf->vf_res, args.out_buffer, RTE_MIN(args.out_size, len));
...
int iavf_get_supported_rxdid(struct iavf_adapter *adapter) {
  ...
  ret = iavf_execute_vf_cmd(adapter, &args, 0);
  if (ret) {
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_GET_SUPPORTED_RXDIDS");
    return ret;
  }
...
int iavf_enable_queues(struct iavf_adapter *adapter) {
  ...
  err = iavf_execute_vf_cmd(adapter, &args, 0);
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_ENABLE_QUEUES");
    return err;
  }
  return 0;
}
...
int iavf_disable_queues(struct iavf_adapter *adapter) {
  ...
  err = iavf_execute_vf_cmd(adapter, &args, 0);
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_DISABLE_QUEUES");
    return err;
  }
  return 0;
}

int iavf_switch_queue(struct iavf_adapter *adapter, uint16_t qid, bool rx,
                      bool on) {
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(adapter);
  struct virtchnl_queue_select queue_select;
  struct iavf_cmd_info args;
...
int iavf_enable_queues_lv(struct iavf_adapter *adapter) {
  ...
  len = sizeof(struct virtchnl_del_ena_dis_queues) +
        sizeof(struct virtchnl_queue_chunk) * (IAVF_RXTX_QUEUE_CHUNKS_NUM - 1);
  ...
  err = iavf_execute_vf_cmd(adapter, &args, 0);
  if (err)
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_ENABLE_QUEUES_V2");
...
int iavf_disable_queues_lv(struct iavf_adapter *adapter) {
  ...
  err = iavf_execute_vf_cmd(adapter, &args, 0);
  if (err)
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_DISABLE_QUEUES_V2");

  rte_free(queue_select);
  return err;
}

int iavf_switch_queue_lv(struct iavf_adapter *adapter, uint16_t qid, bool rx,
                         bool on) {
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(adapter);
  struct virtchnl_del_ena_dis_queues *queue_select;
  struct virtchnl_queue_chunk *queue_chunk;
...
int iavf_configure_rss_lut(struct iavf_adapter *adapter) {
  ...
  err = iavf_execute_vf_cmd(adapter, &args, 0);
  if (err)
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_CONFIG_RSS_LUT");
...
int iavf_configure_rss_key(struct iavf_adapter *adapter) {
  ...
  err = iavf_execute_vf_cmd(adapter, &args, 0);
  if (err)
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_CONFIG_RSS_KEY");
...
int iavf_configure_queues(struct iavf_adapter *adapter,
  ...
  size = sizeof(*vc_config) + sizeof(vc_config->qpair[0]) * num_queue_pairs;
  ...
  for (i = index, vc_qp = vc_config->qpair; i < index + num_queue_pairs;
       i++, vc_qp++) {
    ...
    if (vf->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_RX_FLEX_DESC &&
      ...
      PMD_DRV_LOG(NOTICE, "request RXDID[%d] in Queue[%d]", vc_qp->rxq.rxdid,
                  i);
    } else {
      PMD_DRV_LOG(NOTICE,
                  "RXDID[%d] is not supported, "
    ...
    if (vf->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_RX_FLEX_DESC &&
      ...
      PMD_DRV_LOG(ERR, "RXDID[%d] is not supported", IAVF_RXDID_LEGACY_0);
      return -1;
    }
#endif
  }
...
int iavf_config_irq_map(struct iavf_adapter *adapter) {
  ...
  for (i = 0; i < adapter->dev_data->nb_rx_queues; i++) {
    vecmap = &map_info->vecmap[vf->qv_map[i].vector_id - vf->msix_base];
...
void iavf_add_del_all_mac_addr(struct iavf_adapter *adapter, bool add) {
  ...
  do {
    ...
    for (i = begin; i < next_begin; i++) {
      ...
      list->list[j].type =
          (j == 0 ? VIRTCHNL_ETHER_ADDR_PRIMARY : VIRTCHNL_ETHER_ADDR_EXTRA);
...
int iavf_add_del_eth_addr(struct iavf_adapter *adapter,
  ...
  rte_memcpy(list->list[0].addr, addr->addr_bytes, sizeof(addr->addr_bytes));
...
int iavf_add_del_vlan(struct iavf_adapter *adapter, uint16_t vlanid, bool add) {
  ...
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(adapter);
  uint8_t
      cmd_buffer[sizeof(struct virtchnl_vlan_filter_list) + sizeof(uint16_t)];
  struct iavf_cmd_info args;
  int err;
...
int iavf_fdir_add(struct iavf_adapter *adapter, struct iavf_fdir_conf *filter) {
  ...
  if (fdir_ret->status == VIRTCHNL_FDIR_SUCCESS) {
    ...
    PMD_DRV_LOG(ERR, "Failed to add rule request due to no hw resource");
    return -1;
  } else if (fdir_ret->status == VIRTCHNL_FDIR_FAILURE_RULE_EXIST) {
    PMD_DRV_LOG(
        ERR, "Failed to add rule request due to the rule is already existed");
    return -1;
  } else if (fdir_ret->status == VIRTCHNL_FDIR_FAILURE_RULE_CONFLICT) {
    PMD_DRV_LOG(ERR, "Failed to add rule request due to the rule is conflict "
                     "with existing rule");
    return -1;
    ...
    PMD_DRV_LOG(ERR, "Failed to add rule request due to other reasons");
    return -1;
  }
...
int iavf_fdir_del(struct iavf_adapter *adapter, struct iavf_fdir_conf *filter) {
  ...
  if (fdir_ret->status == VIRTCHNL_FDIR_SUCCESS) {
    ...
    PMD_DRV_LOG(
        ERR, "Failed to delete rule request due to time out for programming");
    return -1;
  } else {
    PMD_DRV_LOG(ERR, "Failed to delete rule request due to other reasons");
    return -1;
  }
...
int iavf_fdir_check(struct iavf_adapter *adapter,
  ...
  if (fdir_ret->status == VIRTCHNL_FDIR_SUCCESS) {
    ...
    PMD_DRV_LOG(ERR, "Failed to check rule request due to parameters validation"
                     " or HW doesn't support");
    return -1;
  } else {
    PMD_DRV_LOG(ERR, "Failed to check rule request due to other reasons");
    return -1;
  }
...
int iavf_add_del_rss_cfg(struct iavf_adapter *adapter,
  ...
  memset(&args, 0, sizeof(args));
  args.ops = add ? VIRTCHNL_OP_ADD_RSS_CFG : VIRTCHNL_OP_DEL_RSS_CFG;
  ...
  if (err)
    PMD_DRV_LOG(ERR, "Failed to execute command of %s",
                add ? "OP_ADD_RSS_CFG" : "OP_DEL_RSS_INPUT_CFG");
...
int iavf_get_hena_caps(struct iavf_adapter *adapter, uint64_t *caps) {
  ...
  err = iavf_execute_vf_cmd(adapter, &args, 0);
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_GET_RSS_HENA_CAPS");
    return err;
  }
...
int iavf_set_hena(struct iavf_adapter *adapter, uint64_t hena) {
  ...
  err = iavf_execute_vf_cmd(adapter, &args, 0);
  if (err)
    PMD_DRV_LOG(ERR, "Failed to execute command of OP_SET_RSS_HENA");
...
int iavf_get_qos_cap(struct iavf_adapter *adapter) {
  ...
  rte_memcpy(vf->qos_cap, args.out_buffer, RTE_MIN(args.out_size, len));
...
int iavf_add_del_mc_addr_list(struct iavf_adapter *adapter,
  ...
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(adapter);
  uint8_t
      cmd_buffer[sizeof(struct virtchnl_ether_addr_list) +
...
int iavf_request_queues(struct rte_eth_dev *dev, uint16_t num) {
  ...
  if (!(vf->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_REQ_QUEUES)) {
  ...
  PMD_DRV_LOG(ERR,
              "request queues failed, only %u queues "
              "available",
              num_queue_pairs);
...
int iavf_get_max_rss_queue_region(struct iavf_adapter *adapter) {
  ...
  if (err) {
    PMD_DRV_LOG(ERR,
                "Failed to execute command of VIRTCHNL_OP_GET_MAX_RSS_QREGION");
    return err;
  }
...
int iavf_ipsec_crypto_request(struct iavf_adapter *adapter, uint8_t *msg,
                              size_t msg_len, uint8_t *resp_msg,
                              size_t resp_msg_len) {
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(adapter);
  struct iavf_cmd_info args;
  int err;
  ...
  if (err) {
    PMD_DRV_LOG(ERR, "fail to execute command %s", "OP_INLINE_IPSEC_CRYPTO");
    return err;
  }
