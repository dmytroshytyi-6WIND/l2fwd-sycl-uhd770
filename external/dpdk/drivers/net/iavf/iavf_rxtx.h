...
typedef void (*iavf_rxd_to_pkt_fields_t)(
    struct iavf_rx_queue *rxq, struct rte_mbuf *mb,
...
#define IAVF_TXD_DATA_QW1_OFFSET_MASK                                          \
  (0x3FFFFULL << IAVF_TXD_DATA_QW1_OFFSET_SHIFT)
...
#define IAVF_TXD_DATA_QW1_IPLEN_MASK (0x7FUL << IAVF_TX_DESC_LENGTH_IPLEN_SHIFT)
...
static inline void iavf_dump_rx_descriptor(struct iavf_rx_queue *rxq,
  ...
  printf("Queue %d Rx_desc %d: QW0: 0x%016" PRIx64 " QW1: 0x%016" PRIx64 "\n",
         rxq->queue_id, rx_id, rx_desc->read.pkt_addr, rx_desc->read.hdr_addr);
#else
 ...
 * to print the qwords
 */
static inline void iavf_dump_tx_descriptor(const struct iavf_tx_queue *txq,
                                           const volatile void *desc,
                                           uint16_t tx_id) {
  const char *name;
...
#define FDIR_PROC_ENABLE_PER_QUEUE(ad, on)                                     \
  do {                                                                         \
    int i;                                                                     \
...
static inline void iavf_fdir_rx_proc_enable(struct iavf_adapter *ad, bool on) {
  if (on) {
