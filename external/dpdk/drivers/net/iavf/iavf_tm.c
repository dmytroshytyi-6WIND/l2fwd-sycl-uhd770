...
static int iavf_level_capabilities_get(struct rte_eth_dev *dev,
                                       ...
                                       struct rte_tm_error *error);
static int iavf_node_capabilities_get(struct rte_eth_dev *dev, uint32_t node_id,
...
iavf_tm_node_search(struct rte_eth_dev *dev, uint32_t node_id,
...
static int iavf_node_param_check(struct iavf_info *vf, uint32_t node_id,
...
static int iavf_tm_node_add(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  ret = iavf_node_param_check(vf, node_id, priority, weight, params, error);
  if (ret)
    return ret;
  ...
  if (parent_node_id == RTE_TM_NODE_ID_NULL) {
    ...
    /* add the root node */
    tm_node = rte_zmalloc("iavf_tm_node", sizeof(struct iavf_tm_node), 0);
    if (!tm_node)
      return -ENOMEM;
    tm_node->id = node_id;
    tm_node->parent = NULL;
  ...
  parent_node = iavf_tm_node_search(dev, parent_node_id, &parent_node_type);
  if (!parent_node) {
    error->type = RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID;
  ...
  rte_memcpy(&tm_node->params, params, sizeof(struct rte_tm_node_params));
  if (parent_node_type == IAVF_TM_NODE_TYPE_PORT) {
    TAILQ_INSERT_TAIL(&vf->tm_conf.tc_list, tm_node, node);
    tm_node->tc = vf->tm_conf.nb_tc_node;
    vf->tm_conf.nb_tc_node++;
  } else {
    TAILQ_INSERT_TAIL(&vf->tm_conf.queue_list, tm_node, node);
...
static int iavf_level_capabilities_get(struct rte_eth_dev *dev,
  ...
  if (level_id != IAVF_TM_NODE_TYPE_QUEUE) {
    ...
    if (level_id == IAVF_TM_NODE_TYPE_PORT)
      cap->nonleaf.sched_n_children_max = IAVF_MAX_TRAFFIC_CLASS;
    else
...
static int iavf_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
                                 __rte_unused struct rte_tm_error *error) {
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
  ...
  size = sizeof(*q_tc_mapping) +
         sizeof(q_tc_mapping->tc[0]) * (vf->qos_cap->num_elem - 1);
  ...
  for (i = 0; i < q_tc_mapping->num_tc; i++) {
    ...
    qtc_map[i].tc = i;
    qtc_map[i].start_queue_id = q_tc_mapping->tc[i].req.start_queue_id;
