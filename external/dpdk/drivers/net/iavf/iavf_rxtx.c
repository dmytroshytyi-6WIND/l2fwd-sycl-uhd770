...
uint8_t iavf_proto_xtr_type_to_rxdid(uint8_t flex_type) {
  ...
  return flex_type < RTE_DIM(rxdid_map) ? rxdid_map[flex_type]
                                        : IAVF_RXDID_COMMS_OVS_1;
}

static int iavf_monitor_callback(
...
static inline int check_rx_thresh(uint16_t nb_desc, uint16_t thresh) {
  /* The following constraints must be satisfied:
   *   thresh < rxq->nb_rx_desc
   */
  if (thresh >= nb_desc) {
    PMD_INIT_LOG(ERR, "rx_free_thresh (%u) must be less than %u", thresh,
                 nb_desc);
    return -EINVAL;
  }
  return 0;
...
static inline int check_tx_thresh(uint16_t nb_desc, uint16_t tx_rs_thresh,
...
static inline bool check_rx_vec_allow(struct iavf_rx_queue *rxq) {
...
static inline bool check_tx_vec_allow(struct iavf_tx_queue *txq) {
...
static inline bool check_rx_bulk_allow(struct iavf_rx_queue *rxq) {
  int ret = true;
...
static inline void reset_rx_queue(struct iavf_rx_queue *rxq) {
  uint16_t len;
  uint32_t i;
...
static inline void reset_tx_queue(struct iavf_tx_queue *txq) {
...
static int alloc_rxq_mbufs(struct iavf_rx_queue *rxq) {
...
static inline void release_rxq_mbufs(struct iavf_rx_queue *rxq) {
  uint16_t i;
...
static inline void release_txq_mbufs(struct iavf_tx_queue *txq) {
  uint16_t i;
...
static const iavf_rxd_to_pkt_fields_t rxd_to_pkt_fields_ops[IAVF_RXDID_LAST +
                                                            1] = {
...
static void iavf_select_rxd_to_pkt_fields_handler(struct iavf_rx_queue *rxq,
                                                  uint32_t rxdid) {
  rxq->rxdid = rxdid;
...
int iavf_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  rx_free_thresh = (rx_conf->rx_free_thresh == 0) ? IAVF_DEFAULT_RX_FREE_THRESH
  ...
  rxq = rte_zmalloc_socket("iavf rxq", sizeof(struct iavf_rx_queue),
  ...
  rxq->sw_ring =
      rte_zmalloc_socket("iavf rx sw ring", sizeof(struct rte_mbuf *) * len,
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq->sw_ring) {
  ...
  ring_size = RTE_ALIGN(len * sizeof(union iavf_rx_desc), IAVF_DMA_MEM_ALIGN);
  mz = rte_eth_dma_zone_reserve(dev, "rx_ring", queue_idx, ring_size,
...
int iavf_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                            uint16_t nb_desc, unsigned int socket_id,
  ...
  tx_rs_thresh = (uint16_t)((tx_conf->tx_rs_thresh) ? tx_conf->tx_rs_thresh
  ...
  txq = rte_zmalloc_socket("iavf txq", sizeof(struct iavf_tx_queue),
                           RTE_CACHE_LINE_SIZE, socket_id);
  if (!txq) {
    PMD_INIT_LOG(ERR, "Failed to allocate memory for "
                      "tx queue structure");
  ...
  if (iavf_ipsec_crypto_supported(adapter))
    txq->ipsec_crypto_pkt_md_offset = iavf_security_get_pkt_md_offset(adapter);

  /* Allocate software ring */
  txq->sw_ring = rte_zmalloc_socket("iavf tx sw ring",
  ...
  mz = rte_eth_dma_zone_reserve(dev, "tx_ring", queue_idx, ring_size,
                                IAVF_RING_BASE_ALIGN, socket_id);
  if (!mz) {
  ...
  if (vf->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_QOS &&
    ...
    for (tc = 0; tc < vf->qos_cap->num_elem; tc++) {
      if (txq->queue_id >= vf->qtc_map[tc].start_queue_id &&
          txq->queue_id <
              (vf->qtc_map[tc].start_queue_id + vf->qtc_map[tc].queue_count))
        break;
    }
...
int iavf_dev_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  ...
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to switch TX queue %u off", tx_queue_id);
    return err;
  }
...
static inline void iavf_rxd_to_vlan_tci(struct rte_mbuf *mb,
                                        volatile union iavf_rx_desc *rxdp) {
...
iavf_flex_rxd_to_ipsec_crypto_said_get(struct rte_mbuf *mb,
  ...
  mb->dynfield1[0] =
      desc->ipsec_said & IAVF_RX_FLEX_DESC_IPSEC_CRYPTO_SAID_MASK;
}
...
iavf_flex_rxd_to_ipsec_crypto_status(struct rte_mbuf *mb,
  ...
  if (status1 & BIT(IAVF_RX_FLEX_DESC_STATUS1_IPSEC_CRYPTO_PROCESSED)) {
    ...
    ipsec_status = status1 & IAVF_RX_FLEX_DESC_IPSEC_CRYPTO_STATUS_MASK;

    if (unlikely(ipsec_status != IAVF_IPSEC_CRYPTO_STATUS_SUCCESS)) {
...
static inline uint64_t iavf_rxd_to_pkt_flags(uint64_t qword) {
  uint64_t flags;
...
#define IAVF_RX_ERR_BITS 0x3f
  ...
  flags = (((qword >> IAVF_RX_DESC_STATUS_FLTSTAT_SHIFT) &
            IAVF_RX_DESC_FLTSTAT_RSS_HASH) == IAVF_RX_DESC_FLTSTAT_RSS_HASH)
              ? RTE_MBUF_F_RX_RSS_HASH
              : 0;

  /* Check if FDIR Match */
  flags |=
      (qword & (1 << IAVF_RX_DESC_STATUS_FLM_SHIFT) ? RTE_MBUF_F_RX_FDIR : 0);
...
static inline uint64_t iavf_rxd_build_fdir(volatile union iavf_rx_desc *rxdp,
                                           struct rte_mbuf *mb) {
  uint64_t flags = 0;
#ifndef RTE_LIBRTE_IAVF_16BYTE_RX_DESC
  uint16_t flexbh;
...
/* Rx L3/L4 checksum */
static inline uint64_t iavf_flex_rxd_error_to_pkt_flags(uint16_t stat_err0) {
  uint64_t flags = 0;
 ...
 */
static inline void iavf_update_rx_tail(struct iavf_rx_queue *rxq,
                                       uint16_t nb_hold, uint16_t rx_id) {
  nb_hold = (uint16_t)(nb_hold + rxq->nb_rx_hold);

  if (nb_hold > rxq->rx_free_thresh) {
    PMD_RX_LOG(DEBUG, "port_id=%u queue_id=%u rx_tail=%u nb_hold=%u",
...
uint16_t iavf_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_rx < nb_pkts) {
    ...
    rx_status =
        (qword1 & IAVF_RXD_QW1_STATUS_MASK) >> IAVF_RXD_QW1_STATUS_SHIFT;
    ...
    rxm->packet_type = ptype_tbl[(uint8_t)((qword1 & IAVF_RXD_QW1_PTYPE_MASK) >>
...
uint16_t iavf_recv_pkts_flex_rxd(void *rx_queue, struct rte_mbuf **rx_pkts,
                                 uint16_t nb_pkts) {
  volatile union iavf_rx_desc *rx_ring;
  volatile union iavf_rx_flex_desc *rxdp;
  struct iavf_rx_queue *rxq;
  ...
  while (nb_rx < nb_pkts) {
    ...
    rx_packet_len =
        (rte_le_to_cpu_16(rxd.wb.pkt_len) & IAVF_RX_FLX_DESC_PKT_LEN_M) -
        rxq->crc_len;
    ...
    iavf_flex_rxd_to_ipsec_crypto_status(rxm, &rxd, &rxq->stats.ipsec_crypto);
...
uint16_t iavf_recv_scattered_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                                  uint16_t nb_pkts) {
  struct iavf_rx_queue *rxq = rx_queue;
  union iavf_rx_desc rxd;
  struct rte_mbuf *rxe;
  ...
  while (nb_rx < nb_pkts) {
    ...
    pkt_flags = iavf_rxd_to_pkt_flags(qword1);
    first_seg->packet_type = ptype_tbl[(uint8_t)(
        (qword1 & IAVF_RXD_QW1_PTYPE_MASK) >> IAVF_RXD_QW1_PTYPE_SHIFT)];
...
static inline int iavf_rx_scan_hw_ring_flex_rxd(struct iavf_rx_queue *rxq) {
  volatile union iavf_rx_flex_desc *rxdp;
   ...
   */
  for (i = 0; i < IAVF_RX_MAX_BURST;
       i += IAVF_LOOK_AHEAD, rxdp += IAVF_LOOK_AHEAD, rxep += IAVF_LOOK_AHEAD) {
    ...
    for (j = 0; j < nb_dd; j++) {
      IAVF_DUMP_RX_DESC(rxq, &rxdp[j], rxq->rx_tail + i * IAVF_LOOK_AHEAD + j);

      mb = rxep[j];
      pkt_len =
          (rte_le_to_cpu_16(rxdp[j].wb.pkt_len) & IAVF_RX_FLX_DESC_PKT_LEN_M) -
          rxq->crc_len;
      mb->data_len = pkt_len;
      mb->pkt_len = pkt_len;
      mb->ol_flags = 0;
...
static inline int iavf_rx_scan_hw_ring(struct iavf_rx_queue *rxq) {
  volatile union iavf_rx_desc *rxdp;
  struct rte_mbuf **rxep;
  struct rte_mbuf *mb;
  uint16_t pkt_len;
  ...
  qword1 = rte_le_to_cpu_64(rxdp->wb.qword1.status_error_len);
  rx_status = (qword1 & IAVF_RXD_QW1_STATUS_MASK) >> IAVF_RXD_QW1_STATUS_SHIFT;
  ...
  for (i = 0; i < IAVF_RX_MAX_BURST;
    ...
    for (j = IAVF_LOOK_AHEAD - 1; j >= 0; j--) {
      ...
      s[j] = (qword1 & IAVF_RXD_QW1_STATUS_MASK) >> IAVF_RXD_QW1_STATUS_SHIFT;
    }
...
static inline uint16_t iavf_rx_fill_from_stage(struct iavf_rx_queue *rxq,
                                               struct rte_mbuf **rx_pkts,
                                               uint16_t nb_pkts) {
  uint16_t i;
...
static inline int iavf_rx_alloc_bufs(struct iavf_rx_queue *rxq) {
  volatile union iavf_rx_desc *rxdp;
  struct rte_mbuf **rxep;
  struct rte_mbuf *mb;
...
static inline uint16_t rx_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  PMD_RX_LOG(DEBUG, "port_id=%u queue_id=%u rx_tail=%u, nb_rx=%u", rxq->port_id,
             rxq->queue_id, rxq->rx_tail, nb_rx);
...
static uint16_t iavf_recv_pkts_bulk_alloc(void *rx_queue,
                                          struct rte_mbuf **rx_pkts,
                                          uint16_t nb_pkts) {
...
static inline int iavf_xmit_cleanup(struct iavf_tx_queue *txq) {
...
static inline void iavf_fill_ctx_desc_cmd_field(volatile uint64_t *field,
                                                struct rte_mbuf *m,
                                                uint8_t vlan_flag) {
  uint64_t cmd = 0;
  ...
  if (m->ol_flags & RTE_MBUF_F_TX_VLAN &&
      vlan_flag & IAVF_TX_FLAGS_VLAN_TAG_LOC_L2TAG2) {
    cmd |= IAVF_TX_CTX_DESC_IL2TAG2 << IAVF_TXD_CTX_QW1_CMD_SHIFT;
  }

  *field |= cmd;
}

static inline void iavf_fill_ctx_desc_ipsec_field(
    volatile uint64_t *field, struct iavf_ipsec_crypto_pkt_metadata *ipsec_md) {
...
static inline uint16_t iavf_fill_ctx_desc_segmentation_field(
    volatile uint64_t *field, struct rte_mbuf *m,
  ...
  if (!m->l4_len || !m->tso_segsz)
    PMD_TX_LOG(DEBUG, "L4 length %d, LSO Segment size %d", m->l4_len,
               m->tso_segsz);
  if (m->tso_segsz < 88)
...
iavf_fill_context_desc(volatile struct iavf_tx_context_desc *desc,
  ...
  if (m->ol_flags & (RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_UDP_SEG)) {
    ...
    *tlen = iavf_fill_ctx_desc_segmentation_field(&desc_qws->qw1, m, ipsec_md);
  }
...
iavf_fill_ipsec_desc(volatile struct iavf_tx_ipsec_desc *desc,
                     ...
                     uint16_t *ipsec_len) {
  desc->qw0 = rte_cpu_to_le_64(
      ((uint64_t)md->l4_payload_len << IAVF_IPSEC_TX_DESC_QW0_L4PAYLEN_SHIFT) |
      ((uint64_t)md->esn << IAVF_IPSEC_TX_DESC_QW0_IPSECESN_SHIFT) |
      ((uint64_t)md->esp_trailer_len
       << IAVF_IPSEC_TX_DESC_QW0_TRAILERLEN_SHIFT));

  desc->qw1 = rte_cpu_to_le_64(
      ((uint64_t)md->sa_idx << IAVF_IPSEC_TX_DESC_QW1_IPSECSA_SHIFT) |
      ((uint64_t)md->next_proto << IAVF_IPSEC_TX_DESC_QW1_IPSECNH_SHIFT) |
      ((uint64_t)(md->len_iv & 0x3) << IAVF_IPSEC_TX_DESC_QW1_IVLEN_SHIFT) |
      ((uint64_t)(md->ol_flags & IAVF_IPSEC_CRYPTO_OL_FLAGS_NATT ? 1ULL : 0ULL)
...
iavf_build_data_desc_cmd_offset_fields(volatile uint64_t *qw1,
  ...
  *qw1 =
      rte_cpu_to_le_64((((uint64_t)command << IAVF_TXD_DATA_QW1_CMD_SHIFT) &
...
static inline void iavf_fill_data_desc(volatile struct iavf_tx_desc *desc,
                                       struct rte_mbuf *m,
                                       uint64_t desc_template, uint16_t tlen,
                                       uint16_t ipseclen) {
...
uint16_t iavf_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (idx = 0; idx < nb_pkts; idx++) {
    ...
    nb_desc_ctx = iavf_calc_context_desc(mb->ol_flags, txq->vlan_flag);
    ...
    PMD_TX_LOG(DEBUG, "port_id=%u queue_id=%u tx_first=%u tx_last=%u",
    ...
    iavf_build_data_desc_cmd_offset_fields(&ddesc_template, mb, txq->vlan_flag);
    ...
    if (nb_desc_ctx) {
      ...
      *(volatile uint64_t *)ctx_desc = 0;
      ...
      iavf_fill_context_desc(ctx_desc, mb, ipsec_md, &tlen, txq->vlan_flag);
    ...
    ddesc->cmd_type_offset_bsz |=
        rte_cpu_to_le_64(ddesc_cmd << IAVF_TXD_DATA_QW1_CMD_SHIFT);
...
end_of_tx:
  ...
  PMD_TX_LOG(DEBUG, "port_id=%u queue_id=%u tx_tail=%u nb_tx=%u", txq->port_id,
             txq->queue_id, desc_idx, idx);
 ...
 * correct queue.
 */
static int iavf_check_vlan_up2tc(struct iavf_tx_queue *txq,
                                 struct rte_mbuf *m) {
...
/* TX prep functions */
uint16_t iavf_prep_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts,
                        uint16_t nb_pkts) {
  int i, ret;
  uint64_t ol_flags;
  struct rte_mbuf *m;
...
void iavf_set_rx_function(struct rte_eth_dev *dev) {
  ...
  if (check_ret >= 0 && rte_vect_get_max_simd_bitwidth() >= RTE_VECT_SIMD_128) {
    ...
    if (dev->data->scattered_rx) {
      if (!use_avx512) {
        PMD_DRV_LOG(DEBUG, "Using %sVector Scattered Rx (port %d).",
                    use_avx2 ? "avx2 " : "", dev->data->port_id);
      } else {
        if (check_ret == IAVF_VECTOR_PATH)
          PMD_DRV_LOG(DEBUG, "Using AVX512 Vector Scattered Rx (port %d).",
                      dev->data->port_id);
        else
          PMD_DRV_LOG(DEBUG,
      ...
      if (use_flex) {
        ...
        if (use_avx512) {
          if (check_ret == IAVF_VECTOR_PATH)
            dev->rx_pkt_burst = iavf_recv_scattered_pkts_vec_avx512_flex_rxd;
          else
            dev->rx_pkt_burst =
        ...
        dev->rx_pkt_burst = use_avx2 ? iavf_recv_scattered_pkts_vec_avx2
                                     : iavf_recv_scattered_pkts_vec;
#ifdef CC_AVX512_SUPPORT
        if (use_avx512) {
          if (check_ret == IAVF_VECTOR_PATH)
            dev->rx_pkt_burst = iavf_recv_scattered_pkts_vec_avx512;
          else
            dev->rx_pkt_burst = iavf_recv_scattered_pkts_vec_avx512_offload;
        }
#endif
      }
    } else {
      if (!use_avx512) {
        ...
        if (check_ret == IAVF_VECTOR_PATH)
          PMD_DRV_LOG(DEBUG, "Using AVX512 Vector Rx (port %d).",
                      dev->data->port_id);
        else
          PMD_DRV_LOG(DEBUG, "Using AVX512 OFFLOAD Vector Rx (port %d).",
                      dev->data->port_id);
      }
      if (use_flex) {
        dev->rx_pkt_burst = use_avx2 ? iavf_recv_pkts_vec_avx2_flex_rxd
                                     : iavf_recv_pkts_vec_flex_rxd;
#ifdef CC_AVX512_SUPPORT
        if (use_avx512) {
          if (check_ret == IAVF_VECTOR_PATH)
            dev->rx_pkt_burst = iavf_recv_pkts_vec_avx512_flex_rxd;
          else
            dev->rx_pkt_burst = iavf_recv_pkts_vec_avx512_flex_rxd_offload;
        }
#endif
      } else {
        dev->rx_pkt_burst =
            use_avx2 ? iavf_recv_pkts_vec_avx2 : iavf_recv_pkts_vec;
#ifdef CC_AVX512_SUPPORT
        if (use_avx512) {
          if (check_ret == IAVF_VECTOR_PATH)
            dev->rx_pkt_burst = iavf_recv_pkts_vec_avx512;
          else
            dev->rx_pkt_burst = iavf_recv_pkts_vec_avx512_offload;
        }
#endif
      }
    }
  ...
  if (dev->data->scattered_rx) {
    ...
    PMD_DRV_LOG(DEBUG, "Using bulk Rx callback (port=%d).", dev->data->port_id);
  ...
  if (check_ret >= 0 && rte_vect_get_max_simd_bitwidth() >= RTE_VECT_SIMD_128) {
    ...
    if (!use_avx512) {
      PMD_DRV_LOG(DEBUG, "Using %sVector Tx (port %d).",
                  use_avx2 ? "avx2 " : "", dev->data->port_id);
      dev->tx_pkt_burst =
          use_avx2 ? iavf_xmit_pkts_vec_avx2 : iavf_xmit_pkts_vec;
    }
...
normal:
#endif
  PMD_DRV_LOG(DEBUG, "Using Basic Tx callback (port=%d).", dev->data->port_id);
  dev->tx_pkt_burst = iavf_xmit_pkts;
  dev->tx_pkt_prepare = iavf_prep_pkts;
}

static int iavf_tx_done_cleanup_full(struct iavf_tx_queue *txq,
                                     uint32_t free_cnt) {
...
int iavf_dev_tx_desc_status(void *tx_queue, uint16_t offset) {
  ...
  expect = rte_cpu_to_le_64(IAVF_TX_DESC_DTYPE_DESC_DONE
...
static inline uint32_t iavf_get_default_ptype(uint16_t ptype) {
  static const uint32_t ptype_tbl[IAVF_MAX_PKT_TYPE] __rte_cache_aligned = {
      /* L2 types */
      /* [0] reserved */
...
void __rte_cold iavf_set_default_ptype_table(struct rte_eth_dev *dev) {
  struct iavf_adapter *ad = IAVF_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  int i;
