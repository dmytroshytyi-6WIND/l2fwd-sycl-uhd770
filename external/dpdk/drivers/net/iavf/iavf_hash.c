...
#define IAVF_PHINT_GTPU_MSK                                                    \
  (IAVF_PHINT_GTPU | IAVF_PHINT_GTPU_EH | IAVF_PHINT_GTPU_EH_DWN |             \
   IAVF_PHINT_GTPU_EH_UP)

#define IAVF_PHINT_LAYERS_MSK (IAVF_PHINT_OUTER_IPV4 | IAVF_PHINT_OUTER_IPV6)
...
static int iavf_hash_init(struct iavf_adapter *ad);
static int iavf_hash_create(struct iavf_adapter *ad, struct rte_flow *flow,
                            void *meta, struct rte_flow_error *error);
static int iavf_hash_destroy(struct iavf_adapter *ad, struct rte_flow *flow,
                             struct rte_flow_error *error);
static void iavf_hash_uninit(struct iavf_adapter *ad);
static void iavf_hash_free(struct rte_flow *flow);
static int iavf_hash_parse_pattern_action(
    struct iavf_adapter *ad, struct iavf_pattern_match_item *array,
    uint32_t array_len, const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], void **meta,
    struct rte_flow_error *error);
...
struct virtchnl_proto_hdrs outer_ipv4_udp_tmplt = {
    TUNNEL_LEVEL_OUTER,
    5,
    {proto_hdr_eth, proto_hdr_svlan, proto_hdr_cvlan, proto_hdr_ipv4_with_prot,
     proto_hdr_udp}};
...
struct virtchnl_proto_hdrs outer_ipv6_udp_tmplt = {
    TUNNEL_LEVEL_OUTER,
    5,
    {proto_hdr_eth, proto_hdr_svlan, proto_hdr_cvlan, proto_hdr_ipv6_with_prot,
     proto_hdr_udp}};
...
struct virtchnl_proto_hdrs udp_l2tpv2_ppp_ipv4_tmplt = {
    TUNNEL_LEVEL_INNER, 3, {proto_hdr_l2tpv2, proto_hdr_ppp, proto_hdr_ipv4}};

struct virtchnl_proto_hdrs udp_l2tpv2_ppp_ipv6_tmplt = {
    TUNNEL_LEVEL_INNER, 3, {proto_hdr_l2tpv2, proto_hdr_ppp, proto_hdr_ipv6}};
...
/* IPv4 outer */
#define IAVF_RSS_TYPE_OUTER_IPV4                                               \
  (RTE_ETH_RSS_ETH | RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_FRAG_IPV4 |                \
   RTE_ETH_RSS_IPV4_CHKSUM)
#define IAVF_RSS_TYPE_OUTER_IPV4_UDP                                           \
  (IAVF_RSS_TYPE_OUTER_IPV4 | RTE_ETH_RSS_NONFRAG_IPV4_UDP |                   \
   RTE_ETH_RSS_L4_CHKSUM)
#define IAVF_RSS_TYPE_OUTER_IPV4_TCP                                           \
  (IAVF_RSS_TYPE_OUTER_IPV4 | RTE_ETH_RSS_NONFRAG_IPV4_TCP |                   \
   RTE_ETH_RSS_L4_CHKSUM)
#define IAVF_RSS_TYPE_OUTER_IPV4_SCTP                                          \
  (IAVF_RSS_TYPE_OUTER_IPV4 | RTE_ETH_RSS_NONFRAG_IPV4_SCTP |                  \
   RTE_ETH_RSS_L4_CHKSUM)
/* IPv6 outer */
#define IAVF_RSS_TYPE_OUTER_IPV6 (RTE_ETH_RSS_ETH | RTE_ETH_RSS_IPV6)
#define IAVF_RSS_TYPE_OUTER_IPV6_FRAG                                          \
  (IAVF_RSS_TYPE_OUTER_IPV6 | RTE_ETH_RSS_FRAG_IPV6)
#define IAVF_RSS_TYPE_OUTER_IPV6_UDP                                           \
  (IAVF_RSS_TYPE_OUTER_IPV6 | RTE_ETH_RSS_NONFRAG_IPV6_UDP |                   \
   RTE_ETH_RSS_L4_CHKSUM)
#define IAVF_RSS_TYPE_OUTER_IPV6_TCP                                           \
  (IAVF_RSS_TYPE_OUTER_IPV6 | RTE_ETH_RSS_NONFRAG_IPV6_TCP |                   \
   RTE_ETH_RSS_L4_CHKSUM)
#define IAVF_RSS_TYPE_OUTER_IPV6_SCTP                                          \
  (IAVF_RSS_TYPE_OUTER_IPV6 | RTE_ETH_RSS_NONFRAG_IPV6_SCTP |                  \
   RTE_ETH_RSS_L4_CHKSUM)
/* VLAN IPV4 */
#define IAVF_RSS_TYPE_VLAN_IPV4                                                \
  (IAVF_RSS_TYPE_OUTER_IPV4 | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_C_VLAN)
#define IAVF_RSS_TYPE_VLAN_IPV4_UDP                                            \
  (IAVF_RSS_TYPE_OUTER_IPV4_UDP | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_C_VLAN)
#define IAVF_RSS_TYPE_VLAN_IPV4_TCP                                            \
  (IAVF_RSS_TYPE_OUTER_IPV4_TCP | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_C_VLAN)
#define IAVF_RSS_TYPE_VLAN_IPV4_SCTP                                           \
  (IAVF_RSS_TYPE_OUTER_IPV4_SCTP | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_C_VLAN)
/* VLAN IPv6 */
#define IAVF_RSS_TYPE_VLAN_IPV6                                                \
  (IAVF_RSS_TYPE_OUTER_IPV6 | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_C_VLAN)
#define IAVF_RSS_TYPE_VLAN_IPV6_FRAG                                           \
  (IAVF_RSS_TYPE_OUTER_IPV6_FRAG | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_C_VLAN)
#define IAVF_RSS_TYPE_VLAN_IPV6_UDP                                            \
  (IAVF_RSS_TYPE_OUTER_IPV6_UDP | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_C_VLAN)
#define IAVF_RSS_TYPE_VLAN_IPV6_TCP                                            \
  (IAVF_RSS_TYPE_OUTER_IPV6_TCP | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_C_VLAN)
#define IAVF_RSS_TYPE_VLAN_IPV6_SCTP                                           \
  (IAVF_RSS_TYPE_OUTER_IPV6_SCTP | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_C_VLAN)
/* IPv4 inner */
#define IAVF_RSS_TYPE_INNER_IPV4 RTE_ETH_RSS_IPV4
#define IAVF_RSS_TYPE_INNER_IPV4_UDP                                           \
  (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_NONFRAG_IPV4_UDP)
#define IAVF_RSS_TYPE_INNER_IPV4_TCP                                           \
  (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_NONFRAG_IPV4_TCP)
#define IAVF_RSS_TYPE_INNER_IPV4_SCTP                                          \
  (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_NONFRAG_IPV4_SCTP)
/* IPv6 inner */
#define IAVF_RSS_TYPE_INNER_IPV6 RTE_ETH_RSS_IPV6
#define IAVF_RSS_TYPE_INNER_IPV6_UDP                                           \
  (RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_NONFRAG_IPV6_UDP)
#define IAVF_RSS_TYPE_INNER_IPV6_TCP                                           \
  (RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_NONFRAG_IPV6_TCP)
#define IAVF_RSS_TYPE_INNER_IPV6_SCTP                                          \
  (RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_NONFRAG_IPV6_SCTP)
/* GTPU IPv4 */
#define IAVF_RSS_TYPE_GTPU_IPV4 (IAVF_RSS_TYPE_INNER_IPV4 | RTE_ETH_RSS_GTPU)
#define IAVF_RSS_TYPE_GTPU_IPV4_UDP                                            \
  (IAVF_RSS_TYPE_INNER_IPV4_UDP | RTE_ETH_RSS_GTPU)
#define IAVF_RSS_TYPE_GTPU_IPV4_TCP                                            \
  (IAVF_RSS_TYPE_INNER_IPV4_TCP | RTE_ETH_RSS_GTPU)
/* GTPU IPv6 */
#define IAVF_RSS_TYPE_GTPU_IPV6 (IAVF_RSS_TYPE_INNER_IPV6 | RTE_ETH_RSS_GTPU)
#define IAVF_RSS_TYPE_GTPU_IPV6_UDP                                            \
  (IAVF_RSS_TYPE_INNER_IPV6_UDP | RTE_ETH_RSS_GTPU)
#define IAVF_RSS_TYPE_GTPU_IPV6_TCP                                            \
  (IAVF_RSS_TYPE_INNER_IPV6_TCP | RTE_ETH_RSS_GTPU)
...
static struct iavf_pattern_match_item iavf_hash_pattern_list[] = {
    ...
    {iavf_pattern_eth_ipv4_udp, IAVF_RSS_TYPE_OUTER_IPV4_UDP,
     &outer_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv4_tcp, IAVF_RSS_TYPE_OUTER_IPV4_TCP,
     &outer_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv4_sctp, IAVF_RSS_TYPE_OUTER_IPV4_SCTP,
    ...
    {iavf_pattern_eth_ipv4_gtpu_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_ipv4_udp, IAVF_RSS_TYPE_GTPU_IPV4_UDP,
     &inner_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_ipv4_tcp, IAVF_RSS_TYPE_GTPU_IPV4_TCP,
     &inner_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_ipv4_udp, IAVF_RSS_TYPE_GTPU_IPV4_UDP,
     &inner_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_ipv4_tcp, IAVF_RSS_TYPE_GTPU_IPV4_TCP,
     &inner_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_eh_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_eh_ipv4_udp, IAVF_RSS_TYPE_GTPU_IPV4_UDP,
     &inner_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_eh_ipv4_tcp, IAVF_RSS_TYPE_GTPU_IPV4_TCP,
     &inner_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_eh_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_eh_ipv4_udp, IAVF_RSS_TYPE_GTPU_IPV4_UDP,
     &inner_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_eh_ipv4_tcp, IAVF_RSS_TYPE_GTPU_IPV4_TCP,
     &inner_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &second_inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_ipv4_udp, IAVF_RSS_TYPE_GTPU_IPV4_UDP,
     &second_inner_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_ipv4_tcp, IAVF_RSS_TYPE_GTPU_IPV4_TCP,
     &second_inner_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &second_inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_ipv4_udp, IAVF_RSS_TYPE_GTPU_IPV4_UDP,
     &second_inner_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_ipv4_tcp, IAVF_RSS_TYPE_GTPU_IPV4_TCP,
     &second_inner_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &second_inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_ipv4_udp, IAVF_RSS_TYPE_GTPU_IPV4_UDP,
     &second_inner_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_ipv4_tcp, IAVF_RSS_TYPE_GTPU_IPV4_TCP,
     &second_inner_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6_gtpu_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &second_inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6_gtpu_ipv4_udp, IAVF_RSS_TYPE_GTPU_IPV4_UDP,
     &second_inner_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6_gtpu_ipv4_tcp, IAVF_RSS_TYPE_GTPU_IPV4_TCP,
     &second_inner_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_eh_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &second_inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_eh_ipv4_udp,
     IAVF_RSS_TYPE_GTPU_IPV4_UDP, &second_inner_ipv4_udp_tmplt},
    ...
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_eh_ipv4_tcp,
     IAVF_RSS_TYPE_GTPU_IPV4_TCP, &second_inner_ipv4_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_eh_ipv4, IAVF_RSS_TYPE_GTPU_IPV4,
     &second_inner_ipv4_tmplt},
    ...
    {iavf_pattern_eth_ipv4_udp_esp, IAVF_RSS_TYPE_IPV4_ESP,
     &ipv4_udp_esp_tmplt},
    {iavf_pattern_eth_ipv4_ah, IAVF_RSS_TYPE_IPV4_AH, &ipv4_ah_tmplt},
    {iavf_pattern_eth_ipv4_l2tpv3, IAVF_RSS_TYPE_IPV4_L2TPV3,
     &ipv4_l2tpv3_tmplt},
    ...
    {iavf_pattern_eth_ipv4_gre_ipv4, IAVF_RSS_TYPE_INNER_IPV4,
     &inner_ipv4_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4, IAVF_RSS_TYPE_INNER_IPV4,
     &inner_ipv4_tmplt},
    ...
    {iavf_pattern_eth_ipv6_gre_ipv4_udp, IAVF_RSS_TYPE_INNER_IPV4_UDP,
     &inner_ipv4_udp_tmplt},
    {iavf_pattern_eth_ipv4_udp_l2tpv2, IAVF_RSS_TYPE_ETH_L2TPV2,
     &ipv4_l2tpv2_tmplt},
    {iavf_pattern_eth_ipv4_udp_l2tpv2_ppp, IAVF_RSS_TYPE_ETH_L2TPV2,
     &ipv4_l2tpv2_ppp_tmplt},
    {iavf_pattern_eth_ipv4_udp_l2tpv2_ppp_ipv4, IAVF_RSS_TYPE_INNER_IPV4,
    ...
    {iavf_pattern_eth_ipv6_udp_l2tpv2_ppp_ipv4, IAVF_RSS_TYPE_INNER_IPV4,
     &udp_l2tpv2_ppp_ipv4_tmplt},
    ...
    {iavf_pattern_eth_ipv6_frag_ext, IAVF_RSS_TYPE_OUTER_IPV6_FRAG,
     &outer_ipv6_frag_tmplt},
    {iavf_pattern_eth_ipv6_udp, IAVF_RSS_TYPE_OUTER_IPV6_UDP,
     &outer_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv6_tcp, IAVF_RSS_TYPE_OUTER_IPV6_TCP,
     &outer_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv6_sctp, IAVF_RSS_TYPE_OUTER_IPV6_SCTP,
     &outer_ipv6_sctp_tmplt},
    {iavf_pattern_eth_vlan_ipv6, IAVF_RSS_TYPE_VLAN_IPV6, &outer_ipv6_tmplt},
    {iavf_pattern_eth_vlan_ipv6_frag_ext, IAVF_RSS_TYPE_OUTER_IPV6_FRAG,
     &outer_ipv6_frag_tmplt},
    ...
    {iavf_pattern_eth_ipv4_gtpu_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_ipv6_udp, IAVF_RSS_TYPE_GTPU_IPV6_UDP,
     &inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_ipv6_tcp, IAVF_RSS_TYPE_GTPU_IPV6_TCP,
     &inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_ipv6_udp, IAVF_RSS_TYPE_GTPU_IPV6_UDP,
     &inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_ipv6_tcp, IAVF_RSS_TYPE_GTPU_IPV6_TCP,
     &inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_eh_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_eh_ipv6_udp, IAVF_RSS_TYPE_GTPU_IPV6_UDP,
     &inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv4_gtpu_eh_ipv6_tcp, IAVF_RSS_TYPE_GTPU_IPV6_TCP,
     &inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_eh_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_eh_ipv6_udp, IAVF_RSS_TYPE_GTPU_IPV6_UDP,
     &inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv6_gtpu_eh_ipv6_tcp, IAVF_RSS_TYPE_GTPU_IPV6_TCP,
     &inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &second_inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_ipv6_udp, IAVF_RSS_TYPE_GTPU_IPV6_UDP,
     &second_inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_ipv6_tcp, IAVF_RSS_TYPE_GTPU_IPV6_TCP,
     &second_inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &second_inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_ipv6_udp, IAVF_RSS_TYPE_GTPU_IPV6_UDP,
     &second_inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_ipv6_tcp, IAVF_RSS_TYPE_GTPU_IPV6_TCP,
     &second_inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &second_inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_ipv6_udp, IAVF_RSS_TYPE_GTPU_IPV6_UDP,
     &second_inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_ipv6_tcp, IAVF_RSS_TYPE_GTPU_IPV6_TCP,
     &second_inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6_gtpu_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &second_inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6_gtpu_ipv6_udp, IAVF_RSS_TYPE_GTPU_IPV6_UDP,
     &second_inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6_gtpu_ipv6_tcp, IAVF_RSS_TYPE_GTPU_IPV6_TCP,
     &second_inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_eh_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &second_inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_eh_ipv6_udp,
     IAVF_RSS_TYPE_GTPU_IPV6_UDP, &second_inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv4_gtpu_eh_ipv6_tcp,
     IAVF_RSS_TYPE_GTPU_IPV6_TCP, &second_inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_eh_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &second_inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_eh_ipv6_udp,
     IAVF_RSS_TYPE_GTPU_IPV6_UDP, &second_inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv4_gre_ipv6_gtpu_eh_ipv6_tcp,
     IAVF_RSS_TYPE_GTPU_IPV6_TCP, &second_inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_eh_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &second_inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_eh_ipv6_udp,
     IAVF_RSS_TYPE_GTPU_IPV6_UDP, &second_inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv4_gtpu_eh_ipv6_tcp,
     IAVF_RSS_TYPE_GTPU_IPV6_TCP, &second_inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6_gtpu_eh_ipv6, IAVF_RSS_TYPE_GTPU_IPV6,
     &second_inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6_gtpu_eh_ipv6_udp,
     IAVF_RSS_TYPE_GTPU_IPV6_UDP, &second_inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6_gtpu_eh_ipv6_tcp,
     IAVF_RSS_TYPE_GTPU_IPV6_TCP, &second_inner_ipv6_tcp_tmplt},
    {iavf_pattern_eth_ipv6_esp, IAVF_RSS_TYPE_IPV6_ESP, &ipv6_esp_tmplt},
    {iavf_pattern_eth_ipv6_udp_esp, IAVF_RSS_TYPE_IPV6_ESP,
     &ipv6_udp_esp_tmplt},
    {iavf_pattern_eth_ipv6_ah, IAVF_RSS_TYPE_IPV6_AH, &ipv6_ah_tmplt},
    {iavf_pattern_eth_ipv6_l2tpv3, IAVF_RSS_TYPE_IPV6_L2TPV3,
     &ipv6_l2tpv3_tmplt},
    ...
    {iavf_pattern_eth_ipv4_gre_ipv6, IAVF_RSS_TYPE_INNER_IPV6,
     &inner_ipv6_tmplt},
    {iavf_pattern_eth_ipv6_gre_ipv6, IAVF_RSS_TYPE_INNER_IPV6,
     &inner_ipv6_tmplt},
    ...
    {iavf_pattern_eth_ipv6_gre_ipv6_udp, IAVF_RSS_TYPE_INNER_IPV6_UDP,
     &inner_ipv6_udp_tmplt},
    {iavf_pattern_eth_ipv6_udp_l2tpv2, IAVF_RSS_TYPE_ETH_L2TPV2,
     &ipv6_l2tpv2_tmplt},
    {iavf_pattern_eth_ipv6_udp_l2tpv2_ppp, IAVF_RSS_TYPE_ETH_L2TPV2,
     &ipv6_l2tpv2_ppp_tmplt},
    {iavf_pattern_eth_ipv4_udp_l2tpv2_ppp_ipv6, IAVF_RSS_TYPE_INNER_IPV6,
    ...
    {iavf_pattern_eth_ipv6_udp_l2tpv2_ppp_ipv6, IAVF_RSS_TYPE_INNER_IPV6,
     &udp_l2tpv2_ppp_ipv6_tmplt},
...
static int iavf_hash_parse_pattern(const struct rte_flow_item pattern[],
                                   uint64_t *phint,
                                   struct rte_flow_error *error) {
  ...
  for (item = pattern; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    ...
    case RTE_FLOW_ITEM_TYPE_L2TPV2:
      ...
      if (l2tpv2) {
        flags_version = rte_be_to_cpu_16(l2tpv2->hdr.common.flags_version);
...
static void iavf_hash_add_fragment_hdr(struct virtchnl_proto_hdrs *hdrs,
                                       int layer) {
...
static void iavf_refine_proto_hdrs_l234(struct virtchnl_proto_hdrs *proto_hdrs,
                                        uint64_t rss_type) {
...
static void iavf_refine_proto_hdrs_gtpu(struct virtchnl_proto_hdrs *proto_hdrs,
                                        uint64_t rss_type) {
  struct virtchnl_proto_hdr *hdr;
  int i;
...
static uint64_t invalid_rss_comb[] = {
    ...
    RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_NONFRAG_IPV6_UDP,
    RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_NONFRAG_IPV6_TCP,
    RTE_ETH_RSS_L3_PRE32 | RTE_ETH_RSS_L3_PRE40 | RTE_ETH_RSS_L3_PRE48 |
        RTE_ETH_RSS_L3_PRE56 | RTE_ETH_RSS_L3_PRE96};
...
#define VALID_RSS_ATTR                                                         \
  (RTE_ETH_RSS_L3_SRC_ONLY | RTE_ETH_RSS_L3_DST_ONLY |                         \
   RTE_ETH_RSS_L4_SRC_ONLY | RTE_ETH_RSS_L4_DST_ONLY |                         \
   RTE_ETH_RSS_L2_SRC_ONLY | RTE_ETH_RSS_L2_DST_ONLY | RTE_ETH_RSS_L3_PRE64)
...
static bool iavf_any_invalid_rss_type(enum rte_eth_hash_function rss_func,
                                      uint64_t rss_type,
...
static int iavf_hash_parse_action(struct iavf_pattern_match_item *match_item,
                                  const struct rte_flow_action actions[],
                                  uint64_t pattern_hint,
                                  struct iavf_rss_meta *rss_meta,
                                  struct rte_flow_error *error) {
  struct virtchnl_proto_hdrs *proto_hdrs;
  ...
  for (action = actions; action->type != RTE_FLOW_ACTION_TYPE_END; action++) {
    ...
    case RTE_FLOW_ACTION_TYPE_RSS:
      ...
      if (rss->func == RTE_ETH_HASH_FUNCTION_SIMPLE_XOR) {
        rss_meta->rss_algorithm = VIRTCHNL_RSS_ALG_XOR_ASYMMETRIC;
        ...
        rss_meta->rss_algorithm = VIRTCHNL_RSS_ALG_TOEPLITZ_ASYMMETRIC;
      }
...
static int iavf_hash_parse_pattern_action(
    __rte_unused struct iavf_adapter *ad, struct iavf_pattern_match_item *array,
    uint32_t array_len, const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], void **meta,
    struct rte_flow_error *error) {
  ...
  pattern_match_item =
      iavf_search_pattern_match_item(pattern, array, array_len, error);
  ...
  ret = iavf_hash_parse_action(pattern_match_item, actions, phint, rss_meta_ptr,
                               error);
...
static int iavf_hash_create(__rte_unused struct iavf_adapter *ad,
  ...
  rss_cfg = rte_zmalloc("iavf rss rule", sizeof(struct virtchnl_rss_cfg), 0);
  if (!rss_cfg) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
...
static int iavf_hash_destroy(__rte_unused struct iavf_adapter *ad,
...
static void iavf_hash_free(struct rte_flow *flow) { rte_free(flow->rule); }
