...
#define IAVF_BASIC_OFFLOAD_CAPS                                                \
  (VF_BASE_MODE_OFFLOADS | VIRTCHNL_VF_OFFLOAD_WB_ON_ITR |                     \
...
/* IAVF_DEV_PRIVATE_TO */
#define IAVF_DEV_PRIVATE_TO_ADAPTER(adapter) ((struct iavf_adapter *)adapter)
#define IAVF_DEV_PRIVATE_TO_VF(adapter) (&((struct iavf_adapter *)adapter)->vf)
#define IAVF_DEV_PRIVATE_TO_HW(adapter) (&((struct iavf_adapter *)adapter)->hw)
#define IAVF_DEV_PRIVATE_TO_IAVF_SECURITY_CTX(adapter)                         \
  (((struct iavf_adapter *)adapter)->security_ctx)

/* IAVF_VSI_TO */
#define IAVF_VSI_TO_HW(vsi) (&(((struct iavf_vsi *)vsi)->adapter->hw))
#define IAVF_VSI_TO_VF(vsi) (&(((struct iavf_vsi *)vsi)->adapter->vf))
 ...
 */
static inline void _notify_cmd(struct iavf_info *vf, int msg_ret) {
 ...
 * _atomic_set_cmd successfully.
 */
static inline void _clear_cmd(struct iavf_info *vf) {
  rte_wmb();
  vf->pend_cmd = VIRTCHNL_OP_UNKNOWN;
...
static inline int _atomic_set_cmd(struct iavf_info *vf, enum virtchnl_ops ops) {
  enum virtchnl_ops op_unk = VIRTCHNL_OP_UNKNOWN;
  int ret = __atomic_compare_exchange(&vf->pend_cmd, &op_unk, &ops, 0,
...
/* Check there is pending cmd in execution. If none, set new command. */
static inline int _atomic_set_async_response_cmd(struct iavf_info *vf,
                                                 enum virtchnl_ops ops) {
  enum virtchnl_ops op_unk = VIRTCHNL_OP_UNKNOWN;
  int ret = __atomic_compare_exchange(&vf->pend_cmd, &op_unk, &ops, 0,
                                      __ATOMIC_ACQUIRE, __ATOMIC_ACQUIRE);
