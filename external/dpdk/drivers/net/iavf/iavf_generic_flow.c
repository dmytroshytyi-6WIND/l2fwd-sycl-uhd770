...
static struct rte_flow *iavf_flow_create(struct rte_eth_dev *dev,
                                         ...
                                         const struct rte_flow_action actions[],
                                         struct rte_flow_error *error);
static int iavf_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                             struct rte_flow_error *error);
static int iavf_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
...
enum rte_flow_item_type iavf_pattern_eth_ipv4_gre_ipv4_gtpu[] = {
    ...
    RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_UDP,  RTE_FLOW_ITEM_TYPE_GTPU,
    RTE_FLOW_ITEM_TYPE_END,
};
...
enum rte_flow_item_type iavf_pattern_eth_ipv4_gre_ipv6_gtpu[] = {
    RTE_FLOW_ITEM_TYPE_ETH,  RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_GRE,
    RTE_FLOW_ITEM_TYPE_IPV6, RTE_FLOW_ITEM_TYPE_UDP,  RTE_FLOW_ITEM_TYPE_GTPU,
    RTE_FLOW_ITEM_TYPE_END,
};
...
enum rte_flow_item_type iavf_pattern_eth_ipv4_ecpri[] = {
    RTE_FLOW_ITEM_TYPE_ETH,   RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_UDP,
    RTE_FLOW_ITEM_TYPE_ECPRI, RTE_FLOW_ITEM_TYPE_END,
};
...
enum rte_flow_item_type iavf_pattern_eth_ipv4_udp_l2tpv2[] = {
    RTE_FLOW_ITEM_TYPE_ETH,    RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_UDP,
    RTE_FLOW_ITEM_TYPE_L2TPV2, RTE_FLOW_ITEM_TYPE_END,
};

enum rte_flow_item_type iavf_pattern_eth_ipv4_udp_l2tpv2_ppp[] = {
    RTE_FLOW_ITEM_TYPE_ETH,    RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_UDP,
    RTE_FLOW_ITEM_TYPE_L2TPV2, RTE_FLOW_ITEM_TYPE_PPP,  RTE_FLOW_ITEM_TYPE_END,
};
...
enum rte_flow_item_type iavf_pattern_eth_ipv4_udp_l2tpv2_ppp_ipv4[] = {
    RTE_FLOW_ITEM_TYPE_ETH,    RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_UDP,
    RTE_FLOW_ITEM_TYPE_L2TPV2, RTE_FLOW_ITEM_TYPE_PPP,  RTE_FLOW_ITEM_TYPE_IPV4,
    RTE_FLOW_ITEM_TYPE_END,
};

enum rte_flow_item_type iavf_pattern_eth_ipv4_udp_l2tpv2_ppp_ipv6[] = {
    RTE_FLOW_ITEM_TYPE_ETH,    RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_UDP,
    RTE_FLOW_ITEM_TYPE_L2TPV2, RTE_FLOW_ITEM_TYPE_PPP,  RTE_FLOW_ITEM_TYPE_IPV6,
    RTE_FLOW_ITEM_TYPE_END,
};
...
typedef struct iavf_flow_engine *(*parse_engine_t)(
    struct iavf_adapter *ad, struct rte_flow *flow,
    struct iavf_parser_list *parser_list, const struct rte_flow_item pattern[],
...
static int iavf_flow_valid_attr(const struct rte_flow_attr *attr,
                                struct rte_flow_error *error) {
...
static void iavf_pattern_skip_void_item(struct rte_flow_item *items,
                                        const struct rte_flow_item *pattern) {
...
static bool iavf_match_pattern(enum rte_flow_item_type *item_array,
                               const struct rte_flow_item *pattern) {
...
struct iavf_pattern_match_item *iavf_search_pattern_match_item(
    const struct rte_flow_item pattern[], struct iavf_pattern_match_item *array,
    uint32_t array_len, struct rte_flow_error *error) {
  uint16_t i = 0;
  ...
  items =
      rte_zmalloc("iavf_pattern", item_num * sizeof(struct rte_flow_item), 0);
  if (!items) {
  ...
  for (i = 0; i < array_len; i++)
    if (iavf_match_pattern(array[i].pattern_list, items)) {
      pattern_match_item->input_set_mask = array[i].input_set_mask;
...
static struct iavf_flow_engine *iavf_parse_engine_create(
    struct iavf_adapter *ad, struct rte_flow *flow,
    struct iavf_parser_list *parser_list, const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], struct rte_flow_error *error) {
...
static struct iavf_flow_engine *iavf_parse_engine_validate(
    struct iavf_adapter *ad, struct rte_flow *flow,
    struct iavf_parser_list *parser_list, const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], struct rte_flow_error *error) {
  struct iavf_flow_engine *engine = NULL;
...
static int iavf_flow_process_filter(
    struct rte_eth_dev *dev, struct rte_flow *flow,
    const struct rte_flow_attr *attr, const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], struct iavf_flow_engine **engine,
    parse_engine_t iavf_parse_engine, struct rte_flow_error *error) {
  ...
  *engine = iavf_parse_engine(ad, flow, &vf->rss_parser_list, pattern, actions,
                              error);
  if (*engine)
    return 0;

  *engine = iavf_parse_engine(ad, flow, &vf->dist_parser_list, pattern, actions,
                              error);
  if (*engine)
    return 0;

  *engine = iavf_parse_engine(ad, flow, &vf->ipsec_crypto_parser_list, pattern,
                              actions, error);
  if (*engine)
    return 0;
...
static int iavf_flow_validate(struct rte_eth_dev *dev,
  ...
  return iavf_flow_process_filter(dev, NULL, attr, pattern, actions, &engine,
                                  iavf_parse_engine_validate, error);
}
...
static struct rte_flow *iavf_flow_create(struct rte_eth_dev *dev,
  ...
  ret = iavf_flow_process_filter(dev, flow, attr, pattern, actions, &engine,
                                 iavf_parse_engine_create, error);
  if (ret < 0) {
...
static bool iavf_flow_is_valid(struct rte_flow *flow) {
  struct iavf_flow_engine *engine;
  void *temp;
...
int iavf_flow_flush(struct rte_eth_dev *dev, struct rte_flow_error *error) {
  struct iavf_adapter *ad = IAVF_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(ad);
  struct rte_flow *p_flow;
  void *temp;
