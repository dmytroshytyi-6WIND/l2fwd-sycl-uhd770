...
static inline void __rte_hot fill_sq_desc_header(union sq_entry_t *entry,
                                                 struct rte_mbuf *pkt) {
...
fill_sq_desc_header_zero_w1(union sq_entry_t *entry, struct rte_mbuf *pkt) {
  fill_sq_desc_header(entry, pkt);
  entry->buff[1] = 0ULL;
}

void __rte_hot nicvf_single_pool_free_xmited_buffers(struct nicvf_txq *sq) {
  int j = 0;
  uint32_t curr_head;
...
void __rte_hot nicvf_multi_pool_free_xmited_buffers(struct nicvf_txq *sq) {
  uint32_t n = 0;
  uint32_t curr_head;
  uint32_t head = sq->head;
  struct rte_mbuf **txbuffs = sq->txbuffs;
...
static inline uint32_t __rte_hot nicvf_free_tx_desc(struct nicvf_txq *sq) {
...
static inline uint32_t __rte_hot nicvf_free_xmitted_buffers(
    struct nicvf_txq *sq, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
...
uint16_t __rte_hot nicvf_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                                   uint16_t nb_pkts) {
  int i;
  uint32_t free_desc;
  uint32_t tail;
...
uint16_t __rte_hot nicvf_xmit_pkts_multiseg(void *tx_queue,
...
static const uint32_t ptype_table[16][16] __rte_cache_aligned = {
    ...
    [L3_IPV4_OPT][L4_IPSEC_ESP] = RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L3_IPV4,
    ...
    [L3_IPV4_OPT][L4_UDP_GENEVE] =
        RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_GENEVE,
    [L3_IPV4_OPT][L4_UDP_VXLAN] =
        RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_VXLAN,
    [L3_IPV4_OPT][L4_NVGRE] = RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_TUNNEL_NVGRE,
    ...
    [L3_IPV6_OPT][L4_IPSEC_ESP] = RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_L3_IPV4,
    ...
    [L3_IPV6_OPT][L4_UDP_GENEVE] =
        RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_TUNNEL_GENEVE,
    [L3_IPV6_OPT][L4_UDP_VXLAN] =
        RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_TUNNEL_VXLAN,
    [L3_IPV6_OPT][L4_NVGRE] = RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_TUNNEL_NVGRE,
...
static inline int __rte_hot nicvf_fill_rbdr(struct nicvf_rxq *rxq,
                                            int to_fill) {
  int i;
  ...
  if (unlikely(rte_mempool_get_bulk(rxq->pool, obj_p, to_fill) < 0)) {
    rte_eth_devices[rxq->port_id].data->rx_mbuf_alloc_failed += to_fill;
    return 0;
  }

  NICVF_RX_ASSERT((unsigned int)to_fill <=
                  (qlen_mask - (nicvf_addr_read(rbdr->rbdr_status) &
                                NICVF_RBDR_COUNT_MASK)));

  next_tail = __atomic_fetch_add(&rbdr->next_tail, to_fill, __ATOMIC_ACQUIRE);
  ltail = next_tail;
  for (i = 0; i < to_fill; i++) {
    struct rbdr_entry_t *entry = desc + (ltail & qlen_mask);

    entry->full_addr = nicvf_mbuff_virt2phy((uintptr_t)obj_p[i], mbuf_phys_off);
    ltail++;
  }
...
static inline int32_t __rte_hot nicvf_rx_pkts_to_process(
    struct nicvf_rxq *rxq, uint16_t nb_pkts, int32_t available_space) {
  if (unlikely(available_space < nb_pkts))
    rxq->available_space =
        nicvf_addr_read(rxq->cq_status) & NICVF_CQ_CQE_COUNT_MASK;

  return RTE_MIN(nb_pkts, available_space);
}

static inline void __rte_hot nicvf_rx_offload(cqe_rx_word0_t cqe_rx_w0,
...
static __rte_always_inline uint16_t nicvf_recv_pkts(void *rx_queue,
  ...
  for (i = 0; i < to_process; i++) {
    ...
    pkt = (struct rte_mbuf *)nicvf_mbuff_phy2virt(rb0_ptr - cqe_rx_w1.align_pad,
                                                  mbuf_phys_off);
...
uint16_t __rte_hot nicvf_recv_pkts_no_offload(void *rx_queue,
                                              struct rte_mbuf **rx_pkts,
                                              uint16_t nb_pkts) {
  return nicvf_recv_pkts(rx_queue, rx_pkts, nb_pkts, NICVF_RX_OFFLOAD_NONE);
}

uint16_t __rte_hot nicvf_recv_pkts_cksum(void *rx_queue,
                                         struct rte_mbuf **rx_pkts,
                                         uint16_t nb_pkts) {
  return nicvf_recv_pkts(rx_queue, rx_pkts, nb_pkts, NICVF_RX_OFFLOAD_CKSUM);
}

uint16_t __rte_hot nicvf_recv_pkts_vlan_strip(void *rx_queue,
                                              struct rte_mbuf **rx_pkts,
                                              uint16_t nb_pkts) {
...
uint16_t __rte_hot nicvf_recv_pkts_cksum_vlan_strip(void *rx_queue,
                                                    struct rte_mbuf **rx_pkts,
                                                    uint16_t nb_pkts) {
  return nicvf_recv_pkts(rx_queue, rx_pkts, nb_pkts,
...
static __rte_always_inline uint16_t __rte_hot nicvf_process_cq_mseg_entry(
    struct cqe_rx_t *cqe_rx, uint64_t mbuf_phys_off, struct rte_mbuf **rx_pkt,
    uint8_t rbptr_offset, uint64_t mbuf_init, const uint32_t flag) {
  ...
  pkt = (struct rte_mbuf *)nicvf_mbuff_phy2virt(rb_ptr[0] - cqe_rx_w1.align_pad,
                                                mbuf_phys_off);

  pkt->pkt_len = cqe_rx_w1.pkt_len;
  pkt->data_len = rb_sz[nicvf_frag_num(0)];
  nicvf_mbuff_init_mseg_update(pkt, mbuf_init, cqe_rx_w1.align_pad, nb_segs);
  pkt->packet_type = nicvf_rx_classify_pkt(cqe_rx_w0);
  if (flag & NICVF_RX_OFFLOAD_NONE)
    pkt->ol_flags = 0;
  if (flag & NICVF_RX_OFFLOAD_CKSUM)
  ...
  *rx_pkt = pkt;
  ...
  for (seg_idx = 1; seg_idx < nb_segs; seg_idx++) {
    seg =
        (struct rte_mbuf *)nicvf_mbuff_phy2virt(rb_ptr[seg_idx], mbuf_phys_off);
...
nicvf_recv_pkts_multiseg(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  for (i = 0; i < to_process; i++) {
    ...
    nb_segs = nicvf_process_cq_mseg_entry(cqe_rx, mbuf_phys_off, rx_pkts + i,
                                          rbptr_offset, mbuf_init, flag);
...
uint16_t __rte_hot nicvf_recv_pkts_multiseg_no_offload(
...
uint16_t __rte_hot nicvf_recv_pkts_multiseg_cksum(void *rx_queue,
                                                  struct rte_mbuf **rx_pkts,
                                                  uint16_t nb_pkts) {
  return nicvf_recv_pkts_multiseg(rx_queue, rx_pkts, nb_pkts,
...
uint16_t __rte_hot nicvf_recv_pkts_multiseg_vlan_strip(
    void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
  return nicvf_recv_pkts_multiseg(rx_queue, rx_pkts, nb_pkts,
                                  NICVF_RX_OFFLOAD_NONE |
...
uint16_t __rte_hot nicvf_recv_pkts_multiseg_cksum_vlan_strip(
    void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
  return nicvf_recv_pkts_multiseg(rx_queue, rx_pkts, nb_pkts,
                                  NICVF_RX_OFFLOAD_CKSUM |
                                      NICVF_RX_OFFLOAD_VLAN_STRIP);
