...
static inline uint16_t __attribute__((const)) nicvf_frag_num(uint16_t i) {
  return (i & ~3) + 3 - (i & 3);
}

static inline void __rte_hot fill_sq_desc_gather(union sq_entry_t *entry,
                                                 struct rte_mbuf *pkt) {
...
static inline void __rte_hot fill_sq_desc_gather(union sq_entry_t *entry,
  ...
  entry->buff[0] = (uint64_t)SQ_DESC_TYPE_GATHER << 60 |
                   (uint64_t)NIC_SEND_LD_TYPE_E_LDT << 58 | pkt->data_len;
  entry->buff[1] = rte_mbuf_data_iova(pkt);
}
#endif

static inline void nicvf_mbuff_init_update(struct rte_mbuf *pkt,
...
static inline void nicvf_mbuff_init_mseg_update(struct rte_mbuf *pkt,
                                                const uint64_t mbuf_init,
                                                uint16_t apad,
                                                uint16_t nb_segs) {
  union mbuf_initializer init = {.value = mbuf_init};
