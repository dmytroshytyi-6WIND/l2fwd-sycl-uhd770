...
#define NICVF_REG_INFO(reg)                                                    \
  { reg, #reg }

static const struct nicvf_reg_info nicvf_reg_tbl[] = {
    NICVF_REG_INFO(NIC_VF_CFG),       NICVF_REG_INFO(NIC_VF_PF_MAILBOX_0_1),
    NICVF_REG_INFO(NIC_VF_INT),       NICVF_REG_INFO(NIC_VF_INT_W1S),
    NICVF_REG_INFO(NIC_VF_ENA_W1C),   NICVF_REG_INFO(NIC_VF_ENA_W1S),
    NICVF_REG_INFO(NIC_VNIC_RSS_CFG), NICVF_REG_INFO(NIC_VNIC_RQ_GEN_CFG),
};
...
int nicvf_base_init(struct nicvf *nic) {
  ...
  if (nicvf_hw_version(nic) == PCI_SUB_DEVICE_ID_CN83XX_NICVF)
    nic->hwcap |=
        NICVF_CAP_TUNNEL_PARSING | NICVF_CAP_CQE_RX2 | NICVF_CAP_DISABLE_APAD;
  ...
  for (i = 0; i < NICVF_ARRAY_SIZE(nicvf_multi_reg_tbl); i++)
    if (dump_stdout)
      nicvf_log("%24s  = 0x%" PRIx64 "\n", nicvf_multi_reg_tbl[i].name,
                nicvf_reg_read(nic, nicvf_multi_reg_tbl[i].offset));
    else
      *data++ = nicvf_reg_read(nic, nicvf_multi_reg_tbl[i].offset);
    ...
    for (i = 0; i < NICVF_ARRAY_SIZE(nicvf_qset_cq_reg_tbl); i++)
      if (dump_stdout)
        nicvf_log(
            ...
            nicvf_queue_reg_read(nic, nicvf_qset_cq_reg_tbl[i].offset, q));
      else
        *data++ = nicvf_queue_reg_read(nic, nicvf_qset_cq_reg_tbl[i].offset, q);
    ...
    for (i = 0; i < NICVF_ARRAY_SIZE(nicvf_qset_rq_reg_tbl); i++)
      if (dump_stdout)
        nicvf_log(
            ...
            nicvf_queue_reg_read(nic, nicvf_qset_rq_reg_tbl[i].offset, q));
      else
        *data++ = nicvf_queue_reg_read(nic, nicvf_qset_rq_reg_tbl[i].offset, q);
    ...
    for (i = 0; i < NICVF_ARRAY_SIZE(nicvf_qset_sq_reg_tbl); i++)
      if (dump_stdout)
        nicvf_log(
            ...
            nicvf_queue_reg_read(nic, nicvf_qset_sq_reg_tbl[i].offset, q));
      else
        *data++ = nicvf_queue_reg_read(nic, nicvf_qset_sq_reg_tbl[i].offset, q);
    ...
    for (i = 0; i < NICVF_ARRAY_SIZE(nicvf_qset_rbdr_reg_tbl); i++)
      if (dump_stdout)
        nicvf_log(
            ...
            nicvf_queue_reg_read(nic, nicvf_qset_rbdr_reg_tbl[i].offset, q));
      else
        *data++ =
            nicvf_queue_reg_read(nic, nicvf_qset_rbdr_reg_tbl[i].offset, q);
  return 0;
...
int nicvf_reg_get_count(void) {
  ...
  nr_regs += NICVF_ARRAY_SIZE(nicvf_qset_cq_reg_tbl) * MAX_CMP_QUEUES_PER_QS;
  nr_regs += NICVF_ARRAY_SIZE(nicvf_qset_rq_reg_tbl) * MAX_RCV_QUEUES_PER_QS;
  nr_regs += NICVF_ARRAY_SIZE(nicvf_qset_sq_reg_tbl) * MAX_SND_QUEUES_PER_QS;
  nr_regs +=
      NICVF_ARRAY_SIZE(nicvf_qset_rbdr_reg_tbl) * MAX_RCV_BUF_DESC_RINGS_PER_QS;

  return nr_regs;
}

static int nicvf_qset_config_internal(struct nicvf *nic, bool enable) {
  int ret;
...
static uint32_t nicvf_roundup_list(uint32_t val, uint32_t list[],
                                   uint32_t entries) {
  uint32_t i;
...
static void nicvf_handle_qset_err_intr(struct nicvf *nic) {
  uint16_t qidx;
  uint64_t status;
  ...
  for (qidx = 0; qidx < MAX_CMP_QUEUES_PER_QS; qidx++) {
    status = nicvf_queue_reg_read(nic, NIC_QSET_CQ_0_7_STATUS, qidx);
  ...
  for (qidx = 0; qidx < MAX_SND_QUEUES_PER_QS; qidx++) {
    status = nicvf_queue_reg_read(nic, NIC_QSET_SQ_0_7_STATUS, qidx);
  ...
  for (qidx = 0; qidx < MAX_RCV_BUF_DESC_RINGS_PER_QS; qidx++) {
    status = nicvf_queue_reg_read(nic, NIC_QSET_RBDR_0_1_STATUS0, qidx);
...
static int nicvf_qset_poll_reg(struct nicvf *nic, uint16_t qidx,
                               uint32_t offset, uint32_t bit_pos, uint32_t bits,
                               uint64_t val) {
  uint64_t bit_mask;
  uint64_t reg_val;
...
int nicvf_qset_rbdr_reclaim(struct nicvf *nic, uint16_t qidx) {
  ...
  if (rbdr) {
    rbdr->head = nicvf_queue_reg_read(nic, NIC_QSET_RBDR_0_1_HEAD, qidx) >> 3;
    rbdr->tail = nicvf_queue_reg_read(nic, NIC_QSET_RBDR_0_1_TAIL, qidx) >> 3;
    rbdr->next_tail = rbdr->tail;
  }

  /* Reset RBDR */
  nicvf_queue_reg_write(nic, NIC_QSET_RBDR_0_1_CFG, qidx, NICVF_RBDR_RESET);

  /* Disable RBDR */
  nicvf_queue_reg_write(nic, NIC_QSET_RBDR_0_1_CFG, qidx, 0);
  if (nicvf_qset_poll_reg(nic, qidx, NIC_QSET_RBDR_0_1_STATUS0, 62, 2, 0x00))
    return NICVF_ERR_RBDR_DISABLE;

  while (1) {
    status = nicvf_queue_reg_read(nic, NIC_QSET_RBDR_0_1_PRFCH_STATUS, qidx);
  ...
  nicvf_queue_reg_write(nic, NIC_QSET_RBDR_0_1_CFG, qidx, NICVF_RBDR_RESET);
  if (nicvf_qset_poll_reg(nic, qidx, NIC_QSET_RBDR_0_1_STATUS0, 62, 2, 0x02))
...
static int nicvf_qsize_regbit(uint32_t len, uint32_t len_shift) {
  int val;
...
int nicvf_qset_rbdr_config(struct nicvf *nic, uint16_t qidx) {
  ...
  rbdr_cfg.reset = 0;
  rbdr_cfg.ldwb = 0;
  rbdr_cfg.qsize = nicvf_qsize_regbit(rbdr->qlen_mask + 1, RBDR_SIZE_SHIFT);
  rbdr_cfg.avg_con = 0;
...
uint32_t nicvf_qsize_rbdr_roundup(uint32_t val) {
  uint32_t list[] = {RBDR_QUEUE_SZ_8K,  RBDR_QUEUE_SZ_16K,  RBDR_QUEUE_SZ_32K,
                     RBDR_QUEUE_SZ_64K, RBDR_QUEUE_SZ_128K, RBDR_QUEUE_SZ_256K,
                     RBDR_QUEUE_SZ_512K};
  return nicvf_roundup_list(val, list, NICVF_ARRAY_SIZE(list));
}

int nicvf_qset_rbdr_precharge(void *dev, struct nicvf *nic, uint16_t ridx,
  ...
  rbdr->tail = nicvf_queue_reg_read(nic, NIC_QSET_RBDR_0_1_TAIL, ridx) >> 3;
  rbdr->next_tail = rbdr->tail;
  nicvf_smp_rmb();
  return 0;
}
...
uint32_t nicvf_qsize_sq_roundup(uint32_t val) {
  uint32_t list[] = {SND_QUEUE_SZ_1K, SND_QUEUE_SZ_2K,  SND_QUEUE_SZ_4K,
                     SND_QUEUE_SZ_8K, SND_QUEUE_SZ_16K, SND_QUEUE_SZ_32K,
                     SND_QUEUE_SZ_64K};
  return nicvf_roundup_list(val, list, NICVF_ARRAY_SIZE(list));
}
...
uint32_t nicvf_qsize_cq_roundup(uint32_t val) {
  uint32_t list[] = {CMP_QUEUE_SZ_1K, CMP_QUEUE_SZ_2K,  CMP_QUEUE_SZ_4K,
                     CMP_QUEUE_SZ_8K, CMP_QUEUE_SZ_16K, CMP_QUEUE_SZ_32K,
                     CMP_QUEUE_SZ_64K};
  return nicvf_roundup_list(val, list, NICVF_ARRAY_SIZE(list));
}
...
void nicvf_first_skip_config(struct nicvf *nic, uint8_t num_dwords) {
  uint64_t val;
...
int nicvf_rss_term(struct nicvf *nic) {
  ...
  return nicvf_rss_reta_update(nic, disable_rss, NIC_MAX_RSS_IDR_TBL_SIZE);
}
...
void nicvf_hw_get_rx_qstats(struct nicvf *nic,
  ...
  qstats->q_rx_bytes = nicvf_queue_reg_read(nic, NIC_QSET_RQ_0_7_STATUS0, qidx);
  qstats->q_rx_packets =
...
void nicvf_hw_get_tx_qstats(struct nicvf *nic,
  ...
  qstats->q_tx_bytes = nicvf_queue_reg_read(nic, NIC_QSET_SQ_0_7_STATUS0, qidx);
  qstats->q_tx_packets =
