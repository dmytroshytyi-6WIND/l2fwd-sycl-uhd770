...
static inline const char *__rte_unused nicvf_mbox_msg_str(int msg) {
...
static inline void nicvf_mbox_send_msg_to_pf_raw(struct nicvf *nic,
                                                 struct nic_mbx *mbx) {
  uint64_t *mbx_data;
  uint64_t mbx_addr;
  int i;
  ...
  nicvf_mbox_log("msg sent %s (VF%d)", nicvf_mbox_msg_str(mbx->msg.msg),
                 nic->vf_id);
}

static inline void nicvf_mbox_send_async_msg_to_pf(struct nicvf *nic,
                                                   struct nic_mbx *mbx) {
...
static inline int nicvf_mbox_send_msg_to_pf(struct nicvf *nic,
                                            struct nic_mbx *mbx) {
  long timeout;
  long sleep = 10;
  int i, retry = 5;
  ...
  for (i = 0; i < retry; i++) {
    ...
    nicvf_log_error("PF didn't ack to msg 0x%02x %s VF%d (%d/%d)", mbx->msg.msg,
                    nicvf_mbox_msg_str(mbx->msg.msg), nic->vf_id, i, retry);
  }
  return -EBUSY;
}
...
int nicvf_handle_mbx_intr(struct nicvf *nic) {
  ...
  case NIC_MBOX_MSG_ALLOC_SQS:
    ...
    if (mbx.sqs_alloc.qs_count != nic->sqs_count) {
      nicvf_log_error("Received %" PRIu8 "/%" PRIu8 " secondary qsets",
    ...
    for (i = 0; i < mbx.sqs_alloc.qs_count; i++) {
      if (mbx.sqs_alloc.svf[i] != nic->snicvf[i]->vf_id) {
        nicvf_log_error("Received secondary qset[%zu] "
                        "ID %" PRIu8 " expected %" PRIu8,
                        i, mbx.sqs_alloc.svf[i], nic->snicvf[i]->vf_id);
        abort();
      }
    }
    nic->pf_acked = true;
    break;
  default:
    nicvf_log_error("Invalid message from PF, msg_id=0x%hhx %s", mbx.msg.msg,
                    nicvf_mbox_msg_str(mbx.msg.msg));
    break;
  }
  nicvf_smp_wmb();
...
int nicvf_mbox_config_rss(struct nicvf *nic) {
  ...
  while (cur_idx < tot_len) {
    cur_len = nicvf_min(tot_len - cur_idx, (size_t)RSS_IND_TBL_LEN_PER_MBX_MSG);
    mbx.msg.msg =
        (cur_idx > 0) ? NIC_MBOX_MSG_RSS_CFG_CONT : NIC_MBOX_MSG_RSS_CFG;
