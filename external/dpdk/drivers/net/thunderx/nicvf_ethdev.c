...
static void nicvf_link_status_update(struct nicvf *nic,
...
static void nicvf_interrupt(void *arg) {
  ...
  rte_eal_alarm_set(NICVF_INTR_POLL_INTERVAL_MS * 1000, nicvf_interrupt, dev);
}

static void nicvf_vf_interrupt(void *arg) {
  struct nicvf *nic = arg;

  nicvf_reg_poll_interrupts(nic);

  rte_eal_alarm_set(NICVF_INTR_POLL_INTERVAL_MS * 1000, nicvf_vf_interrupt,
                    nic);
}

static int nicvf_periodic_alarm_start(void(fn)(void *), void *arg) {
  return rte_eal_alarm_set(NICVF_INTR_POLL_INTERVAL_MS * 1000, fn, arg);
}

static int nicvf_periodic_alarm_stop(void(fn)(void *), void *arg) {
 ...
 */
static int nicvf_dev_link_update(struct rte_eth_dev *dev,
...
static int nicvf_dev_stats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < nic->sqs_count; i++) {
    ...
    for (qidx = rx_start; qidx <= rx_end; qidx++) {
      ...
      nicvf_hw_get_rx_qstats(snic, &rx_qstats, qidx % MAX_RCV_QUEUES_PER_QS);
      stats->q_ibytes[qidx] = rx_qstats.q_rx_bytes;
      stats->q_ipackets[qidx] = rx_qstats.q_rx_packets;
    }
    ...
    for (qidx = tx_start; qidx <= tx_end; qidx++) {
      ...
      nicvf_hw_get_tx_qstats(snic, &tx_qstats, qidx % MAX_SND_QUEUES_PER_QS);
      stats->q_obytes[qidx] = tx_qstats.q_tx_bytes;
      stats->q_opackets[qidx] = tx_qstats.q_tx_packets;
    }
  }
...
static const uint32_t *nicvf_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  size_t copied;
  ...
  if (nicvf_hw_cap(nic) & NICVF_CAP_TUNNEL_PARSING) {
    memcpy((char *)ptypes + copied, ptypes_tunnel, sizeof(ptypes_tunnel));
...
static int nicvf_dev_stats_reset(struct rte_eth_dev *dev) {
  int i;
...
static int nicvf_dev_promisc_enable(struct rte_eth_dev *dev __rte_unused) {
  return 0;
}

static inline uint64_t nicvf_rss_ethdev_to_nic(struct nicvf *nic,
                                               uint64_t ethdev_rss) {
  uint64_t nic_rss = 0;
...
static inline uint64_t nicvf_rss_nic_to_ethdev(struct nicvf *nic,
                                               uint64_t nic_rss) {
  ...
  if ((nic_rss & RSS_IP_ENA) && (nic_rss & RSS_TCP_ENA))
    ethdev_rss |= (RTE_ETH_RSS_NONFRAG_IPV4_TCP | RTE_ETH_RSS_NONFRAG_IPV6_TCP);

  if ((nic_rss & RSS_IP_ENA) && (nic_rss & RSS_UDP_ENA))
    ethdev_rss |= (RTE_ETH_RSS_NONFRAG_IPV4_UDP | RTE_ETH_RSS_NONFRAG_IPV6_UDP);
...
static int nicvf_dev_reta_query(struct rte_eth_dev *dev,
  ...
  if (reta_size != NIC_MAX_RSS_IDR_TBL_SIZE) {
    PMD_DRV_LOG(ERR,
                ...
                "(%u)",
                reta_size, NIC_MAX_RSS_IDR_TBL_SIZE);
    return -EINVAL;
  }
...
static int nicvf_dev_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_key && rss_conf->rss_key_len != RSS_HASH_KEY_BYTE_SIZE) {
    PMD_DRV_LOG(ERR, "Hash key size mismatch %u", rss_conf->rss_key_len);
    return -EINVAL;
  }
...
static int nicvf_qset_cq_alloc(struct rte_eth_dev *dev, struct nicvf *nic,
                               struct nicvf_rxq *rxq, uint16_t qidx,
                               uint32_t desc_cnt) {
  const struct rte_memzone *rz;
  uint32_t ring_size = CMP_QUEUE_SZ_MAX * sizeof(union cq_entry_t);

  rz =
      rte_eth_dma_zone_reserve(dev, "cq_ring", nicvf_netdev_qidx(nic, qidx),
                               ring_size, NICVF_CQ_BASE_ALIGN_BYTES, nic->node);
  if (rz == NULL) {
...
static int nicvf_qset_sq_alloc(struct rte_eth_dev *dev, struct nicvf *nic,
                               struct nicvf_txq *sq, uint16_t qidx,
                               uint32_t desc_cnt) {
  const struct rte_memzone *rz;
  uint32_t ring_size = SND_QUEUE_SZ_MAX * sizeof(union sq_entry_t);

  rz =
      rte_eth_dma_zone_reserve(dev, "sq", nicvf_netdev_qidx(nic, qidx),
                               ring_size, NICVF_SQ_BASE_ALIGN_BYTES, nic->node);
  if (rz == NULL) {
...
static int nicvf_qset_rbdr_alloc(struct rte_eth_dev *dev, struct nicvf *nic,
  ...
  rz = rte_eth_dma_zone_reserve(dev, "rbdr", nicvf_netdev_qidx(nic, 0),
                                ring_size, NICVF_RBDR_BASE_ALIGN_BYTES,
                                nic->node);
  if (rz == NULL) {
  ...
  rbdr->rbdr_status = nicvf_qset_base(nic, 0) + NIC_QSET_RBDR_0_1_STATUS0;
  rbdr->rbdr_door = nicvf_qset_base(nic, 0) + NIC_QSET_RBDR_0_1_DOOR;

  nic->rbdr = rbdr;
  return 0;
}

static void nicvf_rbdr_release_mbuf(struct rte_eth_dev *dev, struct nicvf *nic,
  ...
  for (qidx = rx_start; qidx <= rx_end; qidx++) {
    ...
    if (rxq->precharge_cnt) {
      obj = (void *)nicvf_mbuff_phy2virt(phy, rxq->mbuf_phys_off);
...
static inline void nicvf_rbdr_release_mbufs(struct rte_eth_dev *dev,
                                            struct nicvf *nic) {
...
static inline void nicvf_tx_queue_release_mbufs(struct nicvf_txq *txq) {
  uint32_t head;
...
static void nicvf_tx_queue_reset(struct nicvf_txq *txq) {
...
static inline int nicvf_vf_start_tx_queue(struct rte_eth_dev *dev,
                                          struct nicvf *nic, uint16_t qidx) {
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "Failed to configure sq VF%d %d %d", nic->vf_id, qidx,
                 ret);
...
static inline int nicvf_vf_stop_tx_queue(struct rte_eth_dev *dev,
                                         struct nicvf *nic, uint16_t qidx) {
  struct nicvf_txq *txq;
  int ret;
  ...
  if (ret)
    PMD_INIT_LOG(ERR, "Failed to reclaim sq VF%d %d %d", nic->vf_id, qidx, ret);
...
static inline int nicvf_configure_cpi(struct rte_eth_dev *dev) {
  struct nicvf *nic = nicvf_pmd_priv(dev);
  uint16_t qidx, qcnt;
  int ret;
...
static inline int nicvf_configure_rss(struct rte_eth_dev *dev) {
  struct nicvf *nic = nicvf_pmd_priv(dev);
  uint64_t rsshf;
  int ret = -EINVAL;

  rsshf = nicvf_rss_ethdev_to_nic(
      nic, dev->data->dev_conf.rx_adv_conf.rss_conf.rss_hf);
  PMD_DRV_LOG(INFO, "mode=%d rx_queues=%d loopback=%d rsshf=0x%" PRIx64,
              dev->data->dev_conf.rxmode.mq_mode, dev->data->nb_rx_queues,
...
static int nicvf_configure_rss_reta(struct rte_eth_dev *dev) {
  struct nicvf *nic = nicvf_pmd_priv(dev);
  ...
  return nicvf_rss_reta_update(nic, default_reta, NIC_MAX_RSS_IDR_TBL_SIZE);
}

static void nicvf_dev_tx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
...
static void nicvf_set_tx_function(struct rte_eth_dev *dev) {
...
static void nicvf_set_rx_function(struct rte_eth_dev *dev) {
  struct nicvf *nic = nicvf_pmd_priv(dev);
  ...
  dev->rx_pkt_burst = rx_burst_func[dev->data->scattered_rx][nic->offload_cksum]
                                   [nic->vlan_strip];
}
...
static int nicvf_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t qidx,
  ...
  if (socket_id != (unsigned int)SOCKET_ID_ANY && socket_id != nic->node)
    PMD_DRV_LOG(WARNING, "socket_id expected %d, configured %d", socket_id,
                nic->node);
  ...
  if (tx_free_thresh > (nb_desc) || tx_free_thresh > NICVF_MAX_TX_FREE_THRESH) {
    PMD_INIT_LOG(ERR,
  ...
  if (!is_single_pool) {
    txq->tx_free_thresh =
        (uint16_t)(tx_conf->tx_free_thresh == NICVF_DEFAULT_TX_FREE_THRESH
  ...
  txq->txbuffs =
      rte_zmalloc_socket("txq->txbuffs", nb_desc * sizeof(struct rte_mbuf *),
                         RTE_CACHE_LINE_SIZE, nic->node);
  ...
  PMD_INIT_LOG(DEBUG,
               ...
               nicvf_netdev_qidx(nic, qidx), txq, nb_desc, txq->desc, txq->phys,
               txq->offloads);
...
static inline void nicvf_rx_queue_release_mbufs(struct rte_eth_dev *dev,
                                                struct nicvf_rxq *rxq) {
  uint32_t rxq_cnt;
  ...
  while ((rxq_cnt = nicvf_dev_rx_queue_count(rxq))) {
    nb_pkts = dev->rx_pkt_burst(rxq, rx_pkts, NICVF_MAX_RX_FREE_THRESH);
  ...
  refill_cnt +=
      nicvf_dev_rbdr_refill(dev, nicvf_netdev_qidx(rxq->nic, rxq->queue_id));

  PMD_DRV_LOG(INFO, "free_cnt=%d  refill_cnt=%d", released_pkts, refill_cnt);
}

static void nicvf_rx_queue_reset(struct nicvf_rxq *rxq) {
  rxq->head = 0;
  rxq->available_space = 0;
  rxq->recv_buffers = 0;
}

static inline int nicvf_vf_start_rx_queue(struct rte_eth_dev *dev,
                                          struct nicvf *nic, uint16_t qidx) {
  struct nicvf_rxq *rxq;
  int ret;
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "Failed to configure rq VF%d %d %d", nic->vf_id, qidx,
                 ret);
    goto config_rq_error;
  }
  ret = nicvf_qset_cq_config(nic, qidx, rxq);
  if (ret) {
    PMD_INIT_LOG(ERR, "Failed to configure cq VF%d %d %d", nic->vf_id, qidx,
                 ret);
...
static inline int nicvf_vf_stop_rx_queue(struct rte_eth_dev *dev,
                                         struct nicvf *nic, uint16_t qidx) {
  struct nicvf_rxq *rxq;
  int ret, other_error;
  ...
  if (ret)
    PMD_INIT_LOG(ERR, "Failed to reclaim rq VF%d %d %d", nic->vf_id, qidx, ret);
  ...
  if (ret)
    PMD_INIT_LOG(ERR, "Failed to reclaim cq VF%d %d %d", nic->vf_id, qidx, ret);
...
static void nicvf_dev_rx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
  PMD_INIT_FUNC_TRACE();
...
static inline void nicvf_rxq_mbuf_setup(struct nicvf_rxq *rxq) {
  uintptr_t p;
  ...
  RTE_BUILD_BUG_ON(offsetof(struct nicvf_rxq, rxq_fastpath_data_end) -
                       offsetof(struct nicvf_rxq, rxq_fastpath_data_start) >
                   128);
  mb_def.nb_segs = 1;
...
static int nicvf_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t qidx,
  ...
  rx_free_thresh =
      (uint16_t)((rx_conf->rx_free_thresh) ? rx_conf->rx_free_thresh
...
static int nicvf_dev_info_get(struct rte_eth_dev *dev,
...
static nicvf_iova_addr_t rbdr_rte_mempool_get(void *dev, void *opaque) {
  uint16_t qidx;
  uintptr_t mbuf;
  struct nicvf_rxq *rxq;
...
static int nicvf_vf_start(struct rte_eth_dev *dev, struct nicvf *nic,
                          uint32_t rbdrsz) {
  int ret;
  ...
  for (qidx = rx_start; qidx <= rx_end; qidx++) {
    ...
    if (data_off % RTE_CACHE_LINE_SIZE) {
      PMD_INIT_LOG(ERR, "%s: unaligned data_off=%d delta=%d", rxq->pool->name,
    ...
    if (mbuf_phys_off != rxq->mbuf_phys_off) {
      PMD_INIT_LOG(ERR, "pool params not same,%s VF%d %" PRIx64,
                   rxq->pool->name, nic->vf_id, mbuf_phys_off);
      return -EINVAL;
    }
  }
  ...
  for (qidx = rx_start; qidx <= rx_end; qidx++) {
    ...
    if (rte_mempool_avail_count(rxq->pool) < exp_buffs) {
      PMD_INIT_LOG(ERR, "Buff shortage in pool=%s (%d/%d)", rxq->pool->name,
                   rte_mempool_avail_count(rxq->pool), exp_buffs);
      return -ENOENT;
    }
  }
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "Failed to enable qset %d VF%d", ret, nic->vf_id);
    return ret;
  }
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "Failed to configure rbdr %d VF%d", ret, nic->vf_id);
    goto qset_rbdr_free;
  }
  ...
  ret = nicvf_qset_rbdr_precharge(dev, nic, 0, rbdr_rte_mempool_get,
                                  total_rxq_desc);
  if (ret) {
    PMD_INIT_LOG(ERR, "Failed to fill rbdr %d VF%d", ret, nic->vf_id);
    goto qset_rbdr_reclaim;
  }

  PMD_DRV_LOG(INFO, "Filled %d out of %d entries in RBDR VF%d", nic->rbdr->tail,
              nb_rbdr_desc, nic->vf_id);
  ...
  /* Configure TX queues */
  for (qidx = tx_start; qidx <= tx_end; qidx++) {
    ret = nicvf_vf_start_tx_queue(dev, nic, qidx % MAX_SND_QUEUES_PER_QS);
    if (ret)
  ...
  /* Configure RX queues */
  for (qidx = rx_start; qidx <= rx_end; qidx++) {
    ret = nicvf_vf_start_rx_queue(dev, nic, qidx % MAX_RCV_QUEUES_PER_QS);
    if (ret)
...
static int nicvf_dev_start(struct rte_eth_dev *dev) {
  ...
  for (qidx = 0; qidx < dev->data->nb_rx_queues; qidx++) {
    ...
    if (rbdrsz == 0)
      rbdrsz = buffsz;
    if (rbdrsz != buffsz) {
      PMD_INIT_LOG(ERR, "buffsz not same, qidx=%d (%d/%d)", qidx, rbdrsz,
                   buffsz);
      return -EINVAL;
    }
  }
  ...
  if (dev->data->mtu + (uint32_t)NIC_HW_L2_OVERHEAD + 2 * VLAN_TAG_SIZE >
      buffsz)
...
static int nicvf_dev_close(struct rte_eth_dev *dev) {
  size_t i;
  struct nicvf *nic = nicvf_pmd_priv(dev);
...
static int nicvf_request_sqs(struct nicvf *nic) {
  size_t i;
...
static int nicvf_dev_set_link_up(struct rte_eth_dev *dev) {
  struct nicvf *nic = nicvf_pmd_priv(dev);
  int rc, i;
...
static int nicvf_dev_set_link_down(struct rte_eth_dev *dev) {
  struct nicvf *nic = nicvf_pmd_priv(dev);
  int i;
...
static inline int nicvf_set_first_skip(struct rte_eth_dev *dev) {
  int bytes_to_skip = 0;
  int ret = 0;
  unsigned int i;
  struct rte_kvargs *kvlist;
  static const char *const skip[] = {SKIP_DATA_BYTES, NULL};
  struct nicvf *nic = nicvf_pmd_priv(dev);
...
static int nicvf_eth_dev_uninit(struct rte_eth_dev *dev) {
  PMD_INIT_FUNC_TRACE();
  nicvf_dev_close(dev);
  return 0;
}
static int nicvf_eth_dev_init(struct rte_eth_dev *eth_dev) {
  int ret;
  ...
  if (ret) {
    ...
    PMD_INIT_LOG(INFO, "node=%d vf=%d mode=%s sqs=%s loopback_supported=%s",
                 nic->node, nic->vf_id,
...
RTE_PMD_REGISTER_KMOD_DEP(net_thunderx,
