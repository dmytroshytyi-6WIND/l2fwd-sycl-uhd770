...
#define PMD_INIT_LOG(level, fmt, args...)                                      \
  rte_log(RTE_LOG_##level, ngbe_logtype_init, "%s(): " fmt "\n", __func__,     \
          ##args)

extern int ngbe_logtype_driver;
#define PMD_DRV_LOG(level, fmt, args...)                                       \
  rte_log(RTE_LOG_##level, ngbe_logtype_driver, "%s(): " fmt "\n", __func__,   \
          ##args)

#ifdef RTE_ETHDEV_DEBUG_RX
extern int ngbe_logtype_rx;
#define PMD_RX_LOG(level, fmt, args...)                                        \
  rte_log(RTE_LOG_##level, ngbe_logtype_rx, "%s(): " fmt "\n", __func__, ##args)
#else
#define PMD_RX_LOG(level, fmt, args...)                                        \
  do {                                                                         \
  } while (0)
...
#define PMD_TX_LOG(level, fmt, args...)                                        \
  rte_log(RTE_LOG_##level, ngbe_logtype_tx, "%s(): " fmt "\n", __func__, ##args)
#else
#define PMD_TX_LOG(level, fmt, args...)                                        \
  do {                                                                         \
  } while (0)
