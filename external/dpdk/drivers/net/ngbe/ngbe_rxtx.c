...
uint16_t ngbe_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (tx_ol_req) {
      tx_offload.ptid =
          tx_desc_ol_flags_to_ptid(tx_ol_req, tx_pkt->packet_type);
...
static inline uint64_t ngbe_rxd_pkt_info_to_pkt_flags(uint32_t pkt_info) {
  ...
  static uint64_t ip_pkt_etqf_map[8] = {
      0, 0, 0, PKT_RX_IEEE1588_PTP, 0, 0, 0, 0,
  };
...
uint64_t ngbe_get_tx_port_offloads(struct rte_eth_dev *dev) {
  ...
  tx_offload_capa =
      ...
      RTE_ETH_TX_OFFLOAD_UDP_TNL_TSO | RTE_ETH_TX_OFFLOAD_IP_TNL_TSO |
      RTE_ETH_TX_OFFLOAD_IPIP_TNL_TSO | RTE_ETH_TX_OFFLOAD_MULTI_SEGS;
