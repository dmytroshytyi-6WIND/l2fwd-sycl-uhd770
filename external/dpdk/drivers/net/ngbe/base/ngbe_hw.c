...
static void ngbe_reset_misc_em(struct ngbe_hw *hw) {
  int i;
  ...
  wr32m(hw, NGBE_MACRXCFG, NGBE_MACRXCFG_JUMBO, NGBE_MACRXCFG_JUMBO);

  wr32m(hw, NGBE_FRMSZ, NGBE_FRMSZ_MAX_MASK,
        NGBE_FRMSZ_MAX(NGBE_FRAME_SIZE_DFT));

  /* clear counters on read */
  wr32m(hw, NGBE_MACCNTCTL, NGBE_MACCNTCTL_RC, NGBE_MACCNTCTL_RC);

  wr32m(hw, NGBE_RXFCCFG, NGBE_RXFCCFG_FC, NGBE_RXFCCFG_FC);
  wr32m(hw, NGBE_TXFCCFG, NGBE_TXFCCFG_FC, NGBE_TXFCCFG_FC);

  wr32m(hw, NGBE_MACRXFLT, NGBE_MACRXFLT_PROMISC, NGBE_MACRXFLT_PROMISC);

  wr32m(hw, NGBE_RSTSTAT, NGBE_RSTSTAT_TMRINIT_MASK, NGBE_RSTSTAT_TMRINIT(30));
  ...
  /* sellect GMII */
  wr32m(hw, NGBE_MACTXCFG, NGBE_MACTXCFG_SPEED_MASK, NGBE_MACTXCFG_SPEED_1G);
...
s32 ngbe_set_rar(struct ngbe_hw *hw, u32 index, u8 *addr, u32 vmdq,
   ...
   */
  rar_low = NGBE_ETHADDRL_AD0(addr[5]) | NGBE_ETHADDRL_AD1(addr[4]) |
            NGBE_ETHADDRL_AD2(addr[3]) | NGBE_ETHADDRL_AD3(addr[2]);
  /*
  ...
  rar_high |= (NGBE_ETHADDRH_AD4(addr[1]) | NGBE_ETHADDRH_AD5(addr[0]));
...
s32 ngbe_init_rx_addrs(struct ngbe_hw *hw) {
   ...
   */
  if (ngbe_validate_mac_addr(hw->mac.addr) == NGBE_ERR_INVALID_MAC_ADDR) {
...
s32 ngbe_update_mc_addr_list(struct ngbe_hw *hw, u8 *mc_addr_list,
  ...
  /* Enable mta */
  for (i = 0; i < hw->mac.mcft_size; i++)
    wr32a(hw, NGBE_MCADDRTBL(0), i, hw->mac.mta_shadow[i]);

  if (hw->addr_ctrl.mta_in_use > 0) {
    u32 psrctl = rd32(hw, NGBE_PSRCTL);
    psrctl &= ~(NGBE_PSRCTL_ADHF12_MASK | NGBE_PSRCTL_MCHFENA);
    psrctl |= NGBE_PSRCTL_MCHFENA | NGBE_PSRCTL_ADHF12(hw->mac.mc_filter_type);
    wr32(hw, NGBE_PSRCTL, psrctl);
  }
...
s32 ngbe_fc_enable(struct ngbe_hw *hw) {
  ...
  if ((hw->fc.current_mode & ngbe_fc_tx_pause) && hw->fc.high_water) {
    fcrtl = NGBE_FCWTRLO_TH(hw->fc.low_water) | NGBE_FCWTRLO_XON;
    fcrth = NGBE_FCWTRHI_TH(hw->fc.high_water) | NGBE_FCWTRHI_XOFF;
  } else {
    /*
 ...
 *  advertised settings
 **/
s32 ngbe_negotiate_fc(struct ngbe_hw *hw, u32 adv_reg, u32 lp_reg, u32 adv_sym,
...
STATIC s32 ngbe_fc_autoneg_em(struct ngbe_hw *hw) {
  ...
  return ngbe_negotiate_fc(hw, (u32)technology_ability_reg,
                           (u32)lp_technology_ability_reg, NGBE_TAF_SYM_PAUSE,
...
s32 ngbe_set_pcie_master(struct ngbe_hw *hw, bool enable) {
  ...
  if (!(rd32(hw, NGBE_BMEPEND)) || NGBE_REMOVED(hw->hw_addr))
    goto out;
...
s32 ngbe_set_vfta(struct ngbe_hw *hw, u32 vlan, u32 vind, bool vlan_on,
   ...
   */
  err = ngbe_set_vlvf(hw, vlan, vind, vlan_on, &vfta_delta, vfta, vlvf_bypass);
  if (err != 0) {
    if (vlvf_bypass)
      goto vfta_update;
    return err;
 ...
 **/
s32 ngbe_set_vlvf(struct ngbe_hw *hw, u32 vlan, u32 vind, bool vlan_on,
  ...
  if (!bits && !rd32(hw, NGBE_PSRVLANPLM(vind / 32))) {
 ...
 **/
s32 ngbe_check_mac_link_em(struct ngbe_hw *hw, u32 *speed, bool *link_up,
...
s32 ngbe_get_link_capabilities_em(struct ngbe_hw *hw, u32 *speed,
                                  bool *autoneg) {
  s32 status = 0;
  u16 value = 0;
  ...
  if (hw->phy.type == ngbe_phy_rtl) {
    *speed = NGBE_LINK_SPEED_1GB_FULL | NGBE_LINK_SPEED_100M_FULL |
...
s32 ngbe_setup_mac_link_em(struct ngbe_hw *hw, u32 speed,
...
s32 ngbe_init_thermal_sensor_thresh(struct ngbe_hw *hw) {
  ...
  wr32(hw, NGBE_TSINTR, NGBE_TSINTR_AEN | NGBE_TSINTR_DEN);
...
s32 ngbe_set_mac_type(struct ngbe_hw *hw) {
  ...
  DEBUGOUT("found mac: %d media: %d, returns: %d", hw->mac.type,
           hw->phy.media_type, err);
  return err;
}
...
void ngbe_map_device_id(struct ngbe_hw *hw) {
  ...
  case NGBE_DEV_ID_EM_WX1860A1L:
    ...
    else if (oem == NGBE_YT8521S_SFP || oem == NGBE_YT8521S_SFP_GPIO ||
  ...
  if (oem == NGBE_LY_M88E1512_SFP || oem == NGBE_YT8521S_SFP_GPIO ||
      oem == NGBE_INTERNAL_YT8521S_SFP_GPIO || oem == NGBE_LY_YT8521S_SFP)
    hw->gpio_ctl = true;
}
...
s32 ngbe_init_ops_pf(struct ngbe_hw *hw) {
  ...
  mac->default_speeds = NGBE_LINK_SPEED_10M_FULL | NGBE_LINK_SPEED_100M_FULL |
