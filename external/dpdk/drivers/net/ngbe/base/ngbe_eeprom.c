...
s32 ngbe_get_eeprom_semaphore(struct ngbe_hw *hw) {
  ...
  if (status == 0) {
    for (i = 0; i < timeout; i++) {
      ...
      wr32m(hw, NGBE_MNGSWSYNC, NGBE_MNGSWSYNC_REQ, NGBE_MNGSWSYNC_REQ);
...
s32 ngbe_ee_readw_buffer(struct ngbe_hw *hw, u32 offset, u32 words,
  ...
  while (len) {
    u32 seg = (len <= NGBE_PMMBX_DATA_SIZE ? len : NGBE_PMMBX_DATA_SIZE);
...
s32 ngbe_save_eeprom_version(struct ngbe_hw *hw) {
  ...
  if (hw->bus.lan_id == 0) {
    ...
    wr32(hw, NGBE_CALSUM_CAP_STATUS, hw->rom.cksum_devcap | 0x10000);
