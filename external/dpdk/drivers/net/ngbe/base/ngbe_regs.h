...
#define NGBE_RST_DEFAULT                                                       \
  (NGBE_RST_SW | NGBE_RST_LAN(0) | NGBE_RST_LAN(1) | NGBE_RST_LAN(2) |         \
   NGBE_RST_LAN(3))
...
#define NGBE_ICRMISC_DEFAULT                                                   \
  (NGBE_ICRMISC_RST | NGBE_ICRMISC_ERRMAC | NGBE_ICRMISC_PHY |                 \
   NGBE_ICRMISC_ERRIG | NGBE_ICRMISC_GPIO | NGBE_ICRMISC_VFMBX |               \
   NGBE_ICRMISC_MNGMBX | NGBE_ICRMISC_STALL | NGBE_ICRMISC_TIMER)
...
#define NGBE_VFMBCTL_R2C_BITS                                                  \
  (NGBE_VFMBCTL_RSTD | NGBE_VFMBCTL_PFSTS | NGBE_VFMBCTL_PFACK)
...
#define NGBE_REG_ADDR(hw, reg) ((volatile u32 *)((char *)(hw)->hw_addr + (reg)))

static inline u32 ngbe_get32(volatile u32 *addr) {
  u32 val = NGBE_REG_READ32(addr);
  return rte_le_to_cpu_32(val);
}

static inline void ngbe_set32(volatile u32 *addr, u32 val) {
  val = rte_cpu_to_le_32(val);
  NGBE_REG_WRITE32(addr, val);
}

static inline u32 ngbe_get32_masked(volatile u32 *addr, u32 mask) {
  u32 val = ngbe_get32(addr);
  val &= mask;
  return val;
}

static inline void ngbe_set32_masked(volatile u32 *addr, u32 mask, u32 field) {
  u32 val = ngbe_get32(addr);
  val = ((val & ~mask) | (field & mask));
  ngbe_set32(addr, val);
}

static inline u32 ngbe_get32_relaxed(volatile u32 *addr) {
  u32 val = NGBE_REG_READ32_RELAXED(addr);
  return rte_le_to_cpu_32(val);
}

static inline void ngbe_set32_relaxed(volatile u32 *addr, u32 val) {
  val = rte_cpu_to_le_32(val);
  NGBE_REG_WRITE32_RELAXED(addr, val);
}

static inline u32 rd32(struct ngbe_hw *hw, u32 reg) {
  if (reg == NGBE_REG_DUMMY)
    return 0;
  return ngbe_get32(NGBE_REG_ADDR(hw, reg));
}

static inline void wr32(struct ngbe_hw *hw, u32 reg, u32 val) {
  if (reg == NGBE_REG_DUMMY)
    return;
  ngbe_set32(NGBE_REG_ADDR(hw, reg), val);
}

static inline u32 rd32m(struct ngbe_hw *hw, u32 reg, u32 mask) {
  u32 val = rd32(hw, reg);
  val &= mask;
  return val;
}

static inline void wr32m(struct ngbe_hw *hw, u32 reg, u32 mask, u32 field) {
  u32 val = rd32(hw, reg);
  val = ((val & ~mask) | (field & mask));
  wr32(hw, reg, val);
}

static inline u64 rd64(struct ngbe_hw *hw, u32 reg) {
  u64 lsb = rd32(hw, reg);
  u64 msb = rd32(hw, reg + 4);
  return (lsb | msb << 32);
}

static inline void wr64(struct ngbe_hw *hw, u32 reg, u64 val) {
  wr32(hw, reg, (u32)val);
  wr32(hw, reg + 4, (u32)(val >> 32));
}

/* poll register */
static inline u32 po32m(struct ngbe_hw *hw, u32 reg, u32 mask, u32 expect,
...
static inline u32 rd32_epcs(struct ngbe_hw *hw, u32 addr) {
  u32 data;
...
static inline void wr32_epcs(struct ngbe_hw *hw, u32 addr, u32 data) {
  wr32(hw, NGBE_XPCS_IDAADDR, addr);
  wr32(hw, NGBE_XPCS_IDADATA, data);
}

static inline u32 rd32_ephy(struct ngbe_hw *hw, u32 addr) {
  u32 data;
...
static inline void wr32_ephy(struct ngbe_hw *hw, u32 addr, u32 data) {
  wr32(hw, NGBE_EPHY_IDAADDR, addr);
