 ...
 *  by the caller.
 **/
static s32 ngbe_hic_unlocked(struct ngbe_hw *hw, u32 *buffer, u32 length,
                             u32 timeout) {
  u32 value, loop;
  u16 i, dword_len;
  ...
  wr32m(hw, NGBE_MNGMBXCTL, NGBE_MNGMBXCTL_SWRDY, NGBE_MNGMBXCTL_SWRDY);

  /* Check command completion */
  loop = po32m(hw, NGBE_MNGMBXCTL, NGBE_MNGMBXCTL_FWRDY, NGBE_MNGMBXCTL_FWRDY,
               &value, timeout, 1000);
 ...
 *  semaphore or NGBE_ERR_HOST_INTERFACE_COMMAND when command fails.
 **/
static s32 ngbe_host_interface_command(struct ngbe_hw *hw, u32 *buffer,
...
s32 ngbe_hic_sr_read(struct ngbe_hw *hw, u32 addr, u8 *buf, int len) {
  ...
  err = ngbe_hic_unlocked(hw, (u32 *)&command, sizeof(command),
...
s32 ngbe_hic_pcie_read(struct ngbe_hw *hw, u16 addr, u32 *buf, int len) {
  ...
  err = ngbe_host_interface_command(hw, (u32 *)&command, sizeof(command),
...
s32 ngbe_hic_check_cap(struct ngbe_hw *hw) {
  ...
  for (i = 0; i <= FW_CEM_MAX_RETRIES; i++) {
    ...
    if (command.hdr.rsp.ret_status != FW_CEM_RESP_STATUS_SUCCESS)
      err = NGBE_ERR_HOST_INTERFACE_COMMAND;
