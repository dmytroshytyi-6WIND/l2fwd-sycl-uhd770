  ...
  if (hw->phy.type == ngbe_phy_mvl) {
    if (!hw->mac.autoneg) {
      ...
      case NGBE_LINK_SPEED_10M_FULL:
        ...
        break;
      default:
        value = MVL_CTRL_SPEED_SELECT0 | MVL_CTRL_SPEED_SELECT1;
    ...
    value &= ~(MVL_PHY_100BASET_FULL | MVL_PHY_100BASET_HALF |
    ...
    value &= ~(MVL_PHY_1000BASET_FULL | MVL_PHY_1000BASET_HALF);
    value_r9 |= value;
...
s32 ngbe_check_phy_link_mvl(struct ngbe_hw *hw, u32 *speed, bool *link_up) {
  s32 status = 0;
  u16 phy_link = 0;
  u16 phy_speed = 0;
  u16 phy_data = 0;
