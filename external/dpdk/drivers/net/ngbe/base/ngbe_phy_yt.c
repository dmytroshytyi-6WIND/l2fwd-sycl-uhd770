...
s32 ngbe_read_phy_reg_yt(struct ngbe_hw *hw, u32 reg_addr, u32 device_type,
  ...
  if (hw->phy.media_type == ngbe_media_type_fiber) {
    ngbe_write_phy_reg_ext_yt(hw, YT_SMI_PHY, reg22.device_type,
                              YT_SMI_PHY_SDS);
    ngbe_read_phy_reg_mdi(hw, reg22.addr, reg22.device_type, phy_data);
    ngbe_write_phy_reg_ext_yt(hw, YT_SMI_PHY, reg22.device_type, 0);
  } else {
    ngbe_read_phy_reg_mdi(hw, reg22.addr, reg22.device_type, phy_data);
  }
...
s32 ngbe_read_phy_reg_ext_yt(struct ngbe_hw *hw, u32 reg_addr, u32 device_type,
                             u16 *phy_data) {
...
s32 ngbe_write_phy_reg_ext_yt(struct ngbe_hw *hw, u32 reg_addr, u32 device_type,
                              u16 phy_data) {
  ngbe_write_phy_reg_mdi(hw, 0x1E, device_type, reg_addr);
...
s32 ngbe_read_phy_reg_sds_ext_yt(struct ngbe_hw *hw, u32 reg_addr,
...
s32 ngbe_write_phy_reg_sds_ext_yt(struct ngbe_hw *hw, u32 reg_addr,
                                  u32 device_type, u16 phy_data) {
  ngbe_write_phy_reg_ext_yt(hw, YT_SMI_PHY, device_type, YT_SMI_PHY_SDS);
...
s32 ngbe_setup_phy_link_yt(struct ngbe_hw *hw, u32 speed,
  ...
  if ((value & YT_CHIP_MODE_MASK) == YT_CHIP_MODE_SEL(0)) {
    ...
    if (!hw->mac.autoneg) {
      ...
      case NGBE_LINK_SPEED_10M_FULL:
        ...
        break;
      default:
        value = YT_BCR_SPEED_SELECT0 | YT_BCR_SPEED_SELECT1;
  ...
  skip_an:
    ...
    value = YT_RGMII_CONF1_RXDELAY | YT_RGMII_CONF1_TXDELAY_FE |
            YT_RGMII_CONF1_TXDELAY;
    ngbe_write_phy_reg_ext_yt(hw, YT_RGMII_CONF1, 0, value);
    value = YT_CHIP_MODE_SEL(1) | YT_CHIP_SW_LDO_EN | YT_CHIP_SW_RST;
    ...
    value |=
        YT_RGMII_CONF2_SPEED(2) | YT_RGMII_CONF2_DUPLEX | YT_RGMII_CONF2_LINKUP;
...
s32 ngbe_check_phy_link_yt(struct ngbe_hw *hw, u32 *speed, bool *link_up) {
  s32 status = 0;
  u16 phy_link = 0;
  u16 phy_speed = 0;
  u16 phy_data = 0;
