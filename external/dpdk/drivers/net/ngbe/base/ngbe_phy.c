...
bool ngbe_validate_phy_addr(struct ngbe_hw *hw, u32 phy_addr) {
  ...
  hw->phy.read_reg(hw, NGBE_MD_PHY_ID_HIGH, NGBE_MD_DEV_PMA_PMD, &phy_id);
...
s32 ngbe_get_phy_id(struct ngbe_hw *hw) {
  ...
  err = hw->phy.read_reg(hw, NGBE_MD_PHY_ID_HIGH, NGBE_MD_DEV_PMA_PMD,
                         &phy_id_high);
  hw->phy.id = (u32)(phy_id_high << 16);

  err = hw->phy.read_reg(hw, NGBE_MD_PHY_ID_LOW, NGBE_MD_DEV_PMA_PMD,
                         &phy_id_low);
...
s32 ngbe_read_phy_reg_mdi(struct ngbe_hw *hw, u32 reg_addr, u32 device_type,
  ...
  command = NGBE_MDIOSCA_REG(reg_addr) | NGBE_MDIOSCA_DEV(device_type) |
            NGBE_MDIOSCA_PORT(hw->phy.addr);
  wr32(hw, NGBE_MDIOSCA, command);

  command = NGBE_MDIOSCD_CMD_READ | NGBE_MDIOSCD_BUSY | NGBE_MDIOSCD_CLOCK(6);
   ...
   * complete
   */
  if (!po32m(hw, NGBE_MDIOSCD, NGBE_MDIOSCD_BUSY, 0, NULL, 100, 100)) {
 ...
 *  @phy_data: Pointer to read data from PHY register
 **/
s32 ngbe_read_phy_reg(struct ngbe_hw *hw, u32 reg_addr, u32 device_type,
                      u16 *phy_data) {
  s32 err;
  ...
  err = hw->phy.read_reg_unlocked(hw, reg_addr, device_type, phy_data);
...
s32 ngbe_write_phy_reg_mdi(struct ngbe_hw *hw, u32 reg_addr, u32 device_type,
  ...
  command = NGBE_MDIOSCD_CMD_WRITE | NGBE_MDIOSCD_DAT(phy_data) |
            NGBE_MDIOSCD_BUSY | NGBE_MDIOSCD_CLOCK(6);
  wr32(hw, NGBE_MDIOSCD, command);
 ...
 *  @phy_data: Data to write to the PHY register
 **/
s32 ngbe_write_phy_reg(struct ngbe_hw *hw, u32 reg_addr, u32 device_type,
                       u16 phy_data) {
  s32 err;
  u32 gssr = hw->phy.phy_semaphore_mask;
  ...
  err = hw->phy.write_reg_unlocked(hw, reg_addr, device_type, phy_data);
