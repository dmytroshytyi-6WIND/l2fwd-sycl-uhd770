...
static inline int ngbe_vf_perm_addr_gen(struct rte_eth_dev *dev,
                                        uint16_t vf_num) {
...
static inline int ngbe_mb_intr_setup(struct rte_eth_dev *dev) {
...
static void ngbe_set_rx_mode(struct rte_eth_dev *eth_dev) {
  ...
  fctrl = rd32m(hw, NGBE_PSRCTL, ~(NGBE_PSRCTL_UCP | NGBE_PSRCTL_MCP));
  fctrl |= NGBE_PSRCTL_BCA | NGBE_PSRCTL_MCHFENA;

  vmolr = rd32m(hw, NGBE_POOLETHCTL(vfn),
                ~(NGBE_POOLETHCTL_UCP | NGBE_POOLETHCTL_MCP |
                  NGBE_POOLETHCTL_UCHA | NGBE_POOLETHCTL_MCHA));
  vmolr |= NGBE_POOLETHCTL_BCA | NGBE_POOLETHCTL_UTA | NGBE_POOLETHCTL_VLA;
...
static inline void ngbe_vf_reset_event(struct rte_eth_dev *eth_dev,
                                       uint16_t vf) {
  struct ngbe_hw *hw = ngbe_dev_hw(eth_dev);
  struct ngbe_vf_info *vfinfo = *(NGBE_DEV_VFDATA(eth_dev));
  ...
  vmolr |= (NGBE_POOLETHCTL_UCHA | NGBE_POOLETHCTL_BCA | NGBE_POOLETHCTL_UTA);
...
static inline void ngbe_vf_reset_msg(struct rte_eth_dev *eth_dev, uint16_t vf) {
  struct ngbe_hw *hw = ngbe_dev_hw(eth_dev);
  uint32_t reg;
  uint32_t vf_shift;
  const uint8_t VFRE_SHIFT = 5; /* VFRE 32 bits per slot */
...
static int ngbe_disable_vf_mc_promisc(struct rte_eth_dev *eth_dev,
                                      uint32_t vf) {
  struct ngbe_hw *hw = ngbe_dev_hw(eth_dev);
  uint32_t vmolr;
...
static int ngbe_vf_reset(struct rte_eth_dev *eth_dev, uint16_t vf,
                         uint32_t *msgbuf) {
  struct ngbe_hw *hw = ngbe_dev_hw(eth_dev);
  struct ngbe_vf_info *vfinfo = *(NGBE_DEV_VFDATA(eth_dev));
...
static int ngbe_vf_set_mac_addr(struct rte_eth_dev *eth_dev, uint32_t vf,
                                uint32_t *msgbuf) {
  struct ngbe_hw *hw = ngbe_dev_hw(eth_dev);
  struct ngbe_vf_info *vfinfo = *(NGBE_DEV_VFDATA(eth_dev));
  int rar_entry = hw->mac.num_rar_entries - (vf + 1);
...
static int ngbe_vf_set_multicast(struct rte_eth_dev *eth_dev, uint32_t vf,
                                 uint32_t *msgbuf) {
  struct ngbe_hw *hw = ngbe_dev_hw(eth_dev);
  struct ngbe_vf_info *vfinfo = *(NGBE_DEV_VFDATA(eth_dev));
  int nb_entries = (msgbuf[0] & NGBE_VT_MSGINFO_MASK) >> NGBE_VT_MSGINFO_SHIFT;
  ...
  for (i = 0; i < vfinfo->num_vf_mc_hashes; i++) {
    mta_idx =
        (vfinfo->vf_mc_hashes[i] >> NGBE_MTA_BIT_SHIFT) & NGBE_MTA_INDEX_MASK;
...
static int ngbe_vf_set_vlan(struct rte_eth_dev *eth_dev, uint32_t vf,
                            uint32_t *msgbuf) {
  int add, vid;
  struct ngbe_hw *hw = ngbe_dev_hw(eth_dev);
  struct ngbe_vf_info *vfinfo = *(NGBE_DEV_VFDATA(eth_dev));

  add = (msgbuf[0] & NGBE_VT_MSGINFO_MASK) >> NGBE_VT_MSGINFO_SHIFT;
...
static int ngbe_set_vf_lpe(struct rte_eth_dev *eth_dev,
  ...
  if (max_frs < max_frame) {
    wr32m(hw, NGBE_FRMSZ, NGBE_FRMSZ_MAX_MASK, NGBE_FRMSZ_MAX(max_frame));
  }

  return 0;
}

static int ngbe_negotiate_vf_api(struct rte_eth_dev *eth_dev, uint32_t vf,
                                 uint32_t *msgbuf) {
...
static int ngbe_get_vf_queues(struct rte_eth_dev *eth_dev, uint32_t vf,
                              uint32_t *msgbuf) {
  struct ngbe_vf_info *vfinfo = *NGBE_DEV_VFDATA(eth_dev);
...
static int ngbe_set_vf_mc_promisc(struct rte_eth_dev *eth_dev, uint32_t vf,
                                  uint32_t *msgbuf) {
  struct ngbe_vf_info *vfinfo = *(NGBE_DEV_VFDATA(eth_dev));
  struct ngbe_hw *hw = ngbe_dev_hw(eth_dev);
  ...
  case NGBEVF_XCAST_MODE_NONE:
    disable = NGBE_POOLETHCTL_BCA | NGBE_POOLETHCTL_MCHA | NGBE_POOLETHCTL_MCP |
              NGBE_POOLETHCTL_UCP | NGBE_POOLETHCTL_VLP;
    enable = 0;
    break;
  case NGBEVF_XCAST_MODE_MULTI:
    disable = NGBE_POOLETHCTL_MCP | NGBE_POOLETHCTL_UCP | NGBE_POOLETHCTL_VLP;
  ...
  case NGBEVF_XCAST_MODE_ALLMULTI:
    ...
    enable = NGBE_POOLETHCTL_BCA | NGBE_POOLETHCTL_MCHA | NGBE_POOLETHCTL_MCP;
    break;
...
static int ngbe_set_vf_macvlan_msg(struct rte_eth_dev *dev, uint32_t vf,
                                   uint32_t *msgbuf) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  struct ngbe_vf_info *vf_info = *(NGBE_DEV_VFDATA(dev));
  uint8_t *new_mac = (uint8_t *)(&msgbuf[1]);
  struct rte_ether_addr *ea = (struct rte_ether_addr *)new_mac;
  int index = (msgbuf[0] & NGBE_VT_MSGINFO_MASK) >> NGBE_VT_MSGINFO_SHIFT;
...
static int ngbe_rcv_msg_from_vf(struct rte_eth_dev *eth_dev, uint16_t vf) {
...
static inline void ngbe_rcv_ack_from_vf(struct rte_eth_dev *eth_dev,
                                        uint16_t vf) {
