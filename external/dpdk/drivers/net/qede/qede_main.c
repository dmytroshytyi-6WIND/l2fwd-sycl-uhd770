...
static void qed_update_pf_params(struct ecore_dev *edev,
...
static void qed_init_pci(struct ecore_dev *edev,
...
static int qed_probe(struct ecore_dev *edev, struct rte_pci_device *pci_dev,
...
static int qed_alloc_stream_mem(struct ecore_dev *edev) {
  ...
  for_each_hwfn(edev, i) {
    ...
    p_hwfn->stream =
        OSAL_ZALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(*p_hwfn->stream));
    if (!p_hwfn->stream)
      return -ENOMEM;
  }
...
static void qed_handle_bulletin_change(struct ecore_hwfn *hwfn) {
  ...
  is_mac_exist = ecore_vf_bulletin_get_forced_mac(hwfn, mac, &is_mac_forced);
...
static int qed_slowpath_start(struct ecore_dev *edev,
  ...
  if (IS_PF(edev)) {
    ...
    drv_version.version = (params->drv_major << 24) |
                          (params->drv_minor << 16) | (params->drv_rev << 8) |
                          (params->drv_eng);
    strlcpy((char *)drv_version.name, (const char *)params->name,
            sizeof(drv_version.name));
    rc = ecore_mcp_send_drv_version(hwfn, hwfn->p_main_ptt, &drv_version);
    if (rc) {
...
static int qed_fill_dev_info(struct ecore_dev *edev,
  ...
  if (IS_PF(edev)) {
    ...
    if (ptt) {
      ecore_mcp_get_mfw_ver(ECORE_LEADING_HWFN(edev), ptt, &dev_info->mfw_rev,
                            NULL);
...
int qed_fill_eth_dev_info(struct ecore_dev *edev,
  ...
  if (IS_PF(edev)) {
    ...
    info->num_queues = 0;
    for_each_hwfn(edev, i) info->num_queues +=
        FEAT_NUM(&edev->hwfns[i], ECORE_PF_L2_QUE);

    if (IS_ECORE_SRIOV(edev))
      max_vf_vlan_filters =
          edev->p_iov_info->total_vfs * ECORE_ETH_VF_NUM_VLAN_FILTERS;
    ...
    ecore_vf_get_num_rxqs(ECORE_LEADING_HWFN(edev), &info->num_queues);
...
static uint32_t qed_sb_init(struct ecore_dev *edev,
                            struct ecore_sb_info *sb_info, void *sb_virt_addr,
                            dma_addr_t sb_phy_addr, uint16_t sb_id) {
  struct ecore_hwfn *p_hwfn;
  int hwfn_index;
  uint16_t rel_sb_id;
  ...
  rc = ecore_int_sb_init(p_hwfn, p_hwfn->p_main_ptt, sb_info, sb_virt_addr,
...
static void qed_fill_link(struct ecore_hwfn *hwfn,
  ...
  if (IS_PF(hwfn->p_dev)) {
    rte_memcpy(&params, ecore_mcp_get_link_params(hwfn), sizeof(params));
  ...
  if (params.pause.autoneg || params.pause.forced_rx || params.pause.forced_tx)
  ...
  if (link_caps.default_eee == ECORE_MCP_EEE_UNSUPPORTED) {
    ...
    if_link->eee.adv_caps =
        link.eee_adv_caps ? link.eee_adv_caps : params.eee.adv_caps;
...
static void qed_get_current_link(struct ecore_dev *edev,
                                 struct qed_link_output *if_link) {
  struct ecore_hwfn *hwfn;
...
static int qed_set_link(struct ecore_dev *edev,
  ...
  if (params->override_flags & QED_LINK_OVERRIDE_EEE_CONFIG)
    memcpy(&link_params->eee, &params->eee, sizeof(link_params->eee));
...
static int qed_send_drv_state(struct ecore_dev *edev, bool active) {
  ...
  status = ecore_mcp_ov_update_driver_state(
      hwfn, ptt,
      active ? ECORE_OV_DRIVER_STATE_ACTIVE : ECORE_OV_DRIVER_STATE_DISABLED);
...
const struct qed_eth_ops *qed_get_eth_ops(void) { return &qed_eth_ops_pass; }
