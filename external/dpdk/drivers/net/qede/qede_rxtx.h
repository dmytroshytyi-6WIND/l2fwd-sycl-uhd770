...
#define QEDE_CEIL_TO_CACHE_LINE_SIZE(n)                                        \
  (((n) + (QEDE_FW_RX_ALIGN_END - 1)) & ~(QEDE_FW_RX_ALIGN_END - 1))
#define QEDE_FLOOR_TO_CACHE_LINE_SIZE(n)                                       \
  RTE_ALIGN_FLOOR(n, QEDE_FW_RX_ALIGN_END)

/* Note: QEDE_LLC_SNAP_HDR_LEN is optional,
 * +2 is for padding in front of L2 header
 */
#define QEDE_ETH_OVERHEAD                                                      \
  (((2 * QEDE_VLAN_TAG_SIZE)) + (QEDE_LLC_SNAP_HDR_LEN) + 2)
...
#define QEDE_RSS_OFFLOAD_ALL                                                   \
  (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_NONFRAG_IPV4_TCP |                           \
   ...
   RTE_ETH_RSS_VXLAN | RTE_ETH_RSS_GENEVE)

#define QEDE_RXTX_MAX(qdev) (RTE_MAX(qdev->num_rx_queues, qdev->num_tx_queues))
...
#define QEDE_TX_CSUM_OFFLOAD_MASK                                              \
  (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_TCP_CKSUM |                          \
   RTE_MBUF_F_TX_UDP_CKSUM | RTE_MBUF_F_TX_OUTER_IP_CKSUM |                    \
   RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IPV6)

#define QEDE_TX_OFFLOAD_MASK                                                   \
  (QEDE_TX_CSUM_OFFLOAD_MASK | RTE_MBUF_F_TX_VLAN | RTE_MBUF_F_TX_TUNNEL_MASK)
...
int qede_rx_descriptor_status(void *rxq, uint16_t offset);
