...
static u8 ecore_vf_calculate_legacy(struct ecore_vf_info *p_vf) {
  ...
  if (p_vf->acquire.vfdev_info.eth_fp_hsi_minor == ETH_HSI_VER_NO_PKT_LEN_TUNN)
    legacy |= ECORE_QCID_LEGACY_VF_RX_PROD;

  if (!(p_vf->acquire.vfdev_info.capabilities & VFPF_ACQUIRE_CAP_QUEUE_QIDS))
...
static enum _ecore_status_t ecore_sp_vf_start(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_sp_init_request(p_hwfn, &p_ent, COMMON_RAMROD_VF_START,
  ...
  if (fp_minor > ETH_HSI_VER_MINOR && fp_minor != ETH_HSI_VER_NO_PKT_LEN_TUNN) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
               "VF [%d] - Requested fp hsi %02x.%02x which is"
               " slightly newer than PF's %02x.%02x; Configuring"
               " PFs version\n",
               p_vf->abs_vf_id, ETH_HSI_VER_MAJOR, fp_minor, ETH_HSI_VER_MAJOR,
               ETH_HSI_VER_MINOR);
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d] - Starting using HSI %02x.%02x\n",
...
static enum _ecore_status_t ecore_sp_vf_stop(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_sp_init_request(p_hwfn, &p_ent, COMMON_RAMROD_VF_STOP,
                             PROTOCOLID_COMMON, &init_data);
  if (rc != ECORE_SUCCESS)
    return rc;
...
bool ecore_iov_is_valid_vfid(struct ecore_hwfn *p_hwfn, int rel_vf_id,
  ...
  if ((rel_vf_id >= p_hwfn->p_dev->p_iov_info->total_vfs) || (rel_vf_id < 0))
    return false;

  if ((!p_hwfn->pf_iov_info->vfs_array[rel_vf_id].b_init) && b_enabled_only)
    return false;
...
struct ecore_vf_info *ecore_iov_get_vf_info(struct ecore_hwfn *p_hwfn,
  ...
  if (ecore_iov_is_valid_vfid(p_hwfn, relative_vf_id, b_enabled_only, false))
...
static bool ecore_iov_validate_queue_mode(struct ecore_vf_info *p_vf, u16 qid,
...
static bool ecore_iov_validate_rxq(struct ecore_hwfn *p_hwfn,
                                   struct ecore_vf_info *p_vf, u16 rx_qid,
                                   enum ecore_iov_validate_q_mode mode) {
...
static bool ecore_iov_validate_txq(struct ecore_hwfn *p_hwfn,
                                   struct ecore_vf_info *p_vf, u16 tx_qid,
                                   enum ecore_iov_validate_q_mode mode) {
...
static bool ecore_iov_validate_sb(struct ecore_hwfn *p_hwfn,
                                  struct ecore_vf_info *p_vf, u16 sb_idx) {
  int i;
...
static bool ecore_iov_validate_active_rxq(struct ecore_vf_info *p_vf) {
  ...
  for (i = 0; i < p_vf->num_rxqs; i++)
    if (ecore_iov_validate_queue_mode(p_vf, i, ECORE_IOV_VALIDATE_Q_ENABLE,
                                      false))
      return true;
...
static enum _ecore_status_t ecore_iov_pci_cfg_info(struct ecore_dev *p_dev) {
  ...
  OSAL_PCI_READ_CONFIG_WORD(p_dev, pos + RTE_PCI_SRIOV_TOTAL_VF,
                            &iov->total_vfs);
  OSAL_PCI_READ_CONFIG_WORD(p_dev, pos + RTE_PCI_SRIOV_INITIAL_VF,
                            &iov->initial_vfs);
  ...
  DP_VERBOSE(p_dev, ECORE_MSG_IOV,
             ...
             iov->nres, iov->cap, iov->ctrl, iov->total_vfs, iov->initial_vfs,
             iov->nr_virtfn, iov->offset, iov->stride, iov->pgsz);

  /* Some sanity checks */
  if (iov->num_vfs > NUM_OF_VFS(p_dev) || iov->total_vfs > NUM_OF_VFS(p_dev)) {
...
static void ecore_iov_setup_vfdb(struct ecore_hwfn *p_hwfn) {
  ...
  if (!p_req_virt_addr || !p_reply_virt_addr || !p_bulletin_virt) {
    DP_ERR(p_hwfn, "ecore_iov_setup_vfdb called without alloc mem first\n");
    return;
  }
  ...
  for (idx = 0; idx < p_iov->total_vfs; idx++) {
    ...
    vf->bulletin.phys =
        idx * sizeof(struct ecore_bulletin_content) + bulletin_p;
    ...
    vf->opaque_fid = (p_hwfn->hw_info.opaque_fid & 0xff) | (vf->abs_vf_id << 8);
...
static enum _ecore_status_t ecore_iov_allocate_vfdb(struct ecore_hwfn *p_hwfn) {
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "ecore_iov_allocate_vfdb for %d VFs\n",
             num_vfs);

  /* Allocate PF Mailbox buffer (per-VF) */
  p_iov_info->mbx_msg_size = sizeof(union vfpf_tlvs) * num_vfs;
  p_v_addr = &p_iov_info->mbx_msg_virt_addr;
  *p_v_addr = OSAL_DMA_ALLOC_COHERENT(
      p_hwfn->p_dev, &p_iov_info->mbx_msg_phys_addr, p_iov_info->mbx_msg_size);
  if (!*p_v_addr)
    return ECORE_NOMEM;
  ...
  *p_v_addr =
      OSAL_DMA_ALLOC_COHERENT(p_hwfn->p_dev, &p_iov_info->mbx_reply_phys_addr,
                              p_iov_info->mbx_reply_size);
  if (!*p_v_addr)
    return ECORE_NOMEM;

  p_iov_info->bulletins_size = sizeof(struct ecore_bulletin_content) * num_vfs;
  p_v_addr = &p_iov_info->p_bulletins;
  *p_v_addr = OSAL_DMA_ALLOC_COHERENT(
      p_hwfn->p_dev, &p_iov_info->bulletins_phys, p_iov_info->bulletins_size);
  if (!*p_v_addr)
    return ECORE_NOMEM;
...
static void ecore_iov_free_vfdb(struct ecore_hwfn *p_hwfn) {
  ...
  if (p_hwfn->pf_iov_info->mbx_msg_virt_addr)
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_iov_info->mbx_msg_virt_addr,
                           p_iov_info->mbx_msg_phys_addr,
                           p_iov_info->mbx_msg_size);

  if (p_hwfn->pf_iov_info->mbx_reply_virt_addr)
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_iov_info->mbx_reply_virt_addr,
                           p_iov_info->mbx_reply_phys_addr,
                           p_iov_info->mbx_reply_size);

  if (p_iov_info->p_bulletins)
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_iov_info->p_bulletins,
...
enum _ecore_status_t ecore_iov_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  if (!IS_PF_SRIOV(p_hwfn)) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "No SR-IOV - no need for IOV db\n");
    return ECORE_SUCCESS;
  }
...
enum _ecore_status_t ecore_iov_hw_info(struct ecore_hwfn *p_hwfn) {
  ...
  pos = OSAL_PCI_FIND_EXT_CAPABILITY(p_hwfn->p_dev, RTE_PCI_EXT_CAP_ID_SRIOV);
  if (!pos) {
  ...
  p_dev->p_iov_info =
      OSAL_ZALLOC(p_dev, GFP_KERNEL, sizeof(*p_dev->p_iov_info));
  if (!p_dev->p_iov_info) {
    DP_NOTICE(p_hwfn, false, "Can't support IOV due to lack of memory\n");
    return ECORE_NOMEM;
  }
  ...
  if (p_hwfn->p_dev->p_iov_info->offset < (256 - p_hwfn->abs_pf_id)) {
    u32 first = p_hwfn->p_dev->p_iov_info->offset + p_hwfn->abs_pf_id - 16;
    ...
    u32 first = p_hwfn->p_dev->p_iov_info->offset + p_hwfn->abs_pf_id - 256;

    p_dev->p_iov_info->first_vf_in_pf = first;
  }

  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "First VF in hwfn 0x%08x\n",
...
static void ecore_iov_vf_igu_reset(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < vf->num_sbs; i++)
    ecore_int_igu_init_pure_rt_single(p_hwfn, p_ptt, vf->igu_sbs[i],
                                      vf->opaque_fid, true);
}
...
ecore_iov_enable_vf_access_msix(struct ecore_hwfn *p_hwfn,
                                struct ecore_ptt *p_ptt, u8 abs_vf_id,
                                u8 num_sbs) {
  u8 current_max = 0;
  int i;
  ...
  if (!ECORE_IS_BB(p_hwfn->p_dev)) {
    ecore_for_each_vf(p_hwfn, i) {
      ...
      current_max = OSAL_MAX_T(u8, current_max, p_vf->num_sbs);
    }
  }

  if (num_sbs > current_max)
    return ecore_mcp_config_vf_msix(p_hwfn, p_ptt, abs_vf_id, num_sbs);

  return ECORE_SUCCESS;
}

static enum _ecore_status_t
ecore_iov_enable_vf_access(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                           struct ecore_vf_info *vf) {
  ...
  ecore_iov_vf_pglue_clear_err(p_hwfn, p_ptt, ECORE_VF_ABS_ID(p_hwfn, vf));

  ecore_iov_vf_igu_reset(p_hwfn, p_ptt, vf);

  rc = ecore_iov_enable_vf_access_msix(p_hwfn, p_ptt, vf->abs_vf_id,
                                       vf->num_sbs);
  if (rc != ECORE_SUCCESS)
    return rc;
...
static void ecore_iov_config_perm_table(struct ecore_hwfn *p_hwfn,
  ...
  for (qid = 0; qid < vf->num_rxqs; qid++) {
    ecore_fw_l2_queue(p_hwfn, vf->vf_queues[qid].fw_rx_qid, &qzone_id);
...
static u8 ecore_iov_alloc_vf_igu_sbs(struct ecore_hwfn *p_hwfn,
  ...
  if (num_rx_queues > p_hwfn->hw_info.p_igu_info->usage.free_cnt_iov)
    num_rx_queues = (u16)p_hwfn->hw_info.p_igu_info->usage.free_cnt_iov;
  ...
  for (qid = 0; qid < num_rx_queues; qid++) {
    ...
    ecore_wr(p_hwfn, p_ptt,
             IGU_REG_MAPPING_MEMORY + sizeof(u32) * p_block->igu_sb_id, val);

    /* Configure igu sb in CAU which were marked valid */
    ecore_init_cau_sb_entry(p_hwfn, &sb_entry, p_hwfn->rel_pf_id, vf->abs_vf_id,
                            1);

    ecore_dmae_host2grc(p_hwfn, p_ptt, (u64)(osal_uintptr_t)&sb_entry,
                        CAU_REG_SB_VAR_MEMORY +
                            p_block->igu_sb_id * sizeof(u64),
                        2, OSAL_NULL /* default parameters */);
  }
...
ecore_iov_init_hw_for_vf(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  for (i = 0; i < p_params->num_queues; i++) {
    ...
    if (qid > (u16)RESC_NUM(p_hwfn, ECORE_L2_QUEUE)) {
      DP_NOTICE(p_hwfn, true,
                "Can't enable Rx qid [%04x] for VF[%d]: qids [0,,...,0x%04x] "
                "available\n",
    ...
    if (qid > (u16)RESC_NUM(p_hwfn, ECORE_L2_QUEUE)) {
      DP_NOTICE(p_hwfn, true,
                "Can't enable Tx qid [%04x] for VF[%d]: qids [0,,...,0x%04x] "
                "available\n",
                qid, p_params->rel_vf_id,
                (u16)RESC_NUM(p_hwfn, ECORE_L2_QUEUE));
      return ECORE_INVAL;
  ...
  num_of_vf_available_chains =
      ecore_iov_alloc_vf_igu_sbs(p_hwfn, p_ptt, vf, num_irqs);
  ...
  for (i = 0; i < vf->num_rxqs; i++) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
               ...
               vf->relative_vf_id, i, vf->igu_sbs[i], p_queue->fw_rx_qid,
               p_queue->fw_tx_qid);
  }
  ...
  OSAL_MEMCPY(&link_caps, ecore_mcp_get_link_capabilities(p_hwfn),
              sizeof(link_caps));
  ecore_iov_set_link(p_hwfn, p_params->rel_vf_id, &link_params, &link_state,
                     &link_caps);
...
static void ecore_emul_iov_release_hw_for_vf(struct ecore_hwfn *p_hwfn,
  ...
  if (!ecore_mcp_is_init(p_hwfn)) {
    u32 sriov_dis =
        ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_SR_IOV_DISABLED_REQUEST);

    ecore_wr(p_hwfn, p_ptt, PGLUE_B_REG_SR_IOV_DISABLED_REQUEST_CLR, sriov_dis);
  }
}
#endif
...
enum _ecore_status_t ecore_iov_release_hw_for_vf(struct ecore_hwfn *p_hwfn,
  ...
  if (vf->bulletin.p_virt)
    OSAL_MEMSET(vf->bulletin.p_virt, 0, sizeof(*vf->bulletin.p_virt));
  ...
  OSAL_MEMCPY(&caps, ecore_mcp_get_link_capabilities(p_hwfn), sizeof(caps));
...
static void ecore_iov_lock_vf_pf_channel(struct ecore_hwfn *p_hwfn,
  ...
  if (ecore_iov_tlv_supported(tlv))
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d]: vf pf channel locked by %s\n",
               vf->abs_vf_id, qede_ecore_channel_tlvs_string[tlv]);
  else
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d]: vf pf channel locked by %04x\n",
               vf->abs_vf_id, tlv);
}
...
static void ecore_iov_unlock_vf_pf_channel(struct ecore_hwfn *p_hwfn,
  ...
  if (ecore_iov_tlv_supported(expected_tlv))
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d]: vf pf channel unlocked by %s\n",
               vf->abs_vf_id, qede_ecore_channel_tlvs_string[expected_tlv]);
  else
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
...
void ecore_dp_tlv_list(struct ecore_hwfn *p_hwfn, void *tlvs_list) {
  ...
  do {
    ...
    if (ecore_iov_tlv_supported(tlv->type))
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "TLV number %d: type %s, length %d\n",
                 i, qede_ecore_channel_tlvs_string[tlv->type], tlv->length);
    else
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "TLV number %d: type %d, length %d\n",
...
static void ecore_iov_send_response(struct ecore_hwfn *p_hwfn,
...
#ifdef CONFIG_ECORE_SW_CHANNEL
  mbx->sw_mbx.response_size = length + sizeof(struct channel_list_end_tlv);
  ...
  ecore_dmae_host2host(p_hwfn, p_ptt, mbx->reply_phys + sizeof(u64),
                       mbx->req_virt->first_tlv.reply_address + sizeof(u64),
                       (sizeof(union pfvf_tlvs) - sizeof(u64)) / 4, &params);
  ...
  ecore_dmae_host2host(p_hwfn, p_ptt, mbx->reply_phys,
                       mbx->req_virt->first_tlv.reply_address, sizeof(u64) / 4,
                       &params);
...
static u16 ecore_iov_prep_vp_update_resp_tlvs(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < ECORE_IOV_VP_UPDATE_MAX; i++) {
    ...
    resp = ecore_add_tlv(&p_mbx->offset, ecore_iov_vport_to_tlv(i), size);
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
               ...
               p_vf->relative_vf_id, ecore_iov_vport_to_tlv(i),
               resp->hdr.status);
...
static void ecore_iov_prepare_resp(struct ecore_hwfn *p_hwfn,
                                   struct ecore_ptt *p_ptt,
                                   struct ecore_vf_info *vf_info, u16 type,
...
ecore_iov_get_public_vf_info(struct ecore_hwfn *p_hwfn, u16 relative_vf_id,
                             bool b_enabled_only) {
  struct ecore_vf_info *vf = OSAL_NULL;
...
static void ecore_iov_vf_cleanup(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < ECORE_MAX_VF_CHAINS_PER_PF; i++) {
    ...
    for (j = 0; j < MAX_QUEUES_PER_QZONE; j++) {
      ...
      ecore_eth_queue_cid_release(p_hwfn, p_queue->cids[j].p_cid);
...
static void ecore_iov_vf_mbx_acquire_resc_cids(struct ecore_hwfn *p_hwfn,
                                               struct ecore_ptt *p_ptt,
                                               struct ecore_vf_info *p_vf,
                                               struct vf_pf_resc_request *p_req,
                                               struct pf_vf_resc *p_resp) {
  u8 num_vf_cons = p_hwfn->pf_params.eth_pf_params.num_vf_cons;
  u8 db_size = DB_ADDR_VF(1, DQ_DEMS_LEGACY) - DB_ADDR_VF(0, DQ_DEMS_LEGACY);
  u32 bar_size;
   ...
   */
  if (p_vf->acquire.vfdev_info.capabilities & VFPF_ACQUIRE_CAP_PHYSICAL_BAR) {
  ...
  if (bar_size / db_size < 256)
    p_resp->num_cids =
        OSAL_MIN_T(u8, p_resp->num_cids, (u8)(bar_size / db_size));
}
...
static u8 ecore_iov_vf_mbx_acquire_resc(struct ecore_hwfn *p_hwfn,
  ...
  p_resp->num_mac_filters =
      OSAL_MIN_T(u8, p_vf->num_mac_filters, p_req->num_mac_filters);
  p_resp->num_vlan_filters =
      OSAL_MIN_T(u8, p_vf->num_vlan_filters, p_req->num_vlan_filters);
  ...
  if (p_resp->num_rxqs < p_req->num_rxqs ||
      p_resp->num_txqs < p_req->num_txqs || p_resp->num_sbs < p_req->num_sbs ||
      p_resp->num_mac_filters < p_req->num_mac_filters ||
      p_resp->num_vlan_filters < p_req->num_vlan_filters ||
      p_resp->num_mc_filters < p_req->num_mc_filters ||
      p_resp->num_cids < p_req->num_cids) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
               "VF[%d] - Insufficient resources: rxq [%02x/%02x] txq "
               "[%02x/%02x] sbs [%02x/%02x] mac [%02x/%02x] vlan [%02x/%02x] "
               "mc [%02x/%02x] cids [%02x/%02x]\n",
               p_vf->abs_vf_id, p_req->num_rxqs, p_resp->num_rxqs,
               p_req->num_rxqs, p_resp->num_txqs, p_req->num_sbs,
               p_resp->num_sbs, p_req->num_mac_filters, p_resp->num_mac_filters,
               p_req->num_vlan_filters, p_resp->num_vlan_filters,
               p_req->num_mc_filters, p_resp->num_mc_filters, p_req->num_cids,
               p_resp->num_cids);
    ...
    if ((p_vf->acquire.vfdev_info.eth_fp_hsi_minor ==
        ...
        (p_vf->acquire.vfdev_info.os_type == VFPF_ACQUIRE_OS_WINDOWS))
...
static void ecore_iov_vf_mbx_acquire(struct ecore_hwfn *p_hwfn,
  ...
  if (req->vfdev_info.eth_fp_hsi_major != ETH_HSI_VER_MAJOR) {
    if (req->vfdev_info.capabilities & VFPF_ACQUIRE_CAP_PRE_FP_HSI) {
      ...
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d] is pre-fastpath HSI\n",
                 vf->abs_vf_id);
      ...
      DP_INFO(p_hwfn,
              ...
              " HSI %02x.%02x\n",
              vf->abs_vf_id, req->vfdev_info.eth_fp_hsi_major,
              req->vfdev_info.eth_fp_hsi_minor, ETH_HSI_VER_MAJOR,
              ETH_HSI_VER_MINOR);
  ...
  pfdev_info->capabilities =
      PFVF_ACQUIRE_CAP_DEFAULT_UNTAGGED | PFVF_ACQUIRE_CAP_POST_FW_OVERRIDE;
  if (ECORE_IS_CMT(p_hwfn->p_dev))
  ...
  resp->pfdev_info.bar_size = (u8)ecore_iov_vf_db_bar_size(p_hwfn, p_ptt);

  ecore_iov_vf_mbx_acquire_stats(&pfdev_info->stats_info);

  OSAL_MEMCPY(pfdev_info->port_mac, p_hwfn->hw_info.hw_mac_addr, ETH_ALEN);
  ...
  pfdev_info->minor_fp_hsi =
      OSAL_MIN_T(u8, ETH_HSI_VER_MINOR, req->vfdev_info.eth_fp_hsi_minor);
  pfdev_info->os_type = OSAL_IOV_GET_OS_TYPE();
  ecore_mcp_get_mfw_ver(p_hwfn, p_ptt, &pfdev_info->mfw_ver, OSAL_NULL);
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
             ...
             " n_vlans-%d\n",
             vf->abs_vf_id, resp->pfdev_info.chip_num, resp->pfdev_info.db_size,
...
static enum _ecore_status_t __ecore_iov_spoofchk_set(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_sp_vport_update(p_hwfn, &params, ECORE_SPQ_MODE_EBLOCK, OSAL_NULL);
  if (rc == ECORE_SUCCESS) {
    p_vf->spoof_chk = val;
    p_vf->req_spoofchk_val = p_vf->spoof_chk;
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "Spoofchk val[%d] configured\n", val);
  } else {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
...
ecore_iov_reconfigure_unicast_vlan(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < ECORE_ETH_VF_NUM_VLAN_FILTERS + 1; i++) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
               ...
               p_vf->relative_vf_id);
    rc = ecore_sp_eth_filter_ucast(p_hwfn, p_vf->opaque_fid, &filter,
...
ecore_iov_configure_vport_forced(struct ecore_hwfn *p_hwfn,
  ...
  if ((events & (1 << MAC_ADDR_FORCED)) ||
    ...
    rc = ecore_sp_eth_filter_ucast(p_hwfn, p_vf->opaque_fid, &filter,
                                   ECORE_SPQ_MODE_CB, OSAL_NULL);
    if (rc) {
      DP_NOTICE(p_hwfn, true, "PF failed to configure MAC for VF\n");
      return rc;
    }

    if (p_hwfn->pf_params.eth_pf_params.allow_vf_mac_change ||
        p_vf->p_vf_info.is_trusted_configured)
      p_vf->configured_features |= 1 << VFPF_BULLETIN_MAC_ADDR;
    else
  ...
  if (events & (1 << VLAN_ADDR_FORCED)) {
    ...
    filter.opcode = filter.vlan ? ECORE_FILTER_REPLACE : ECORE_FILTER_FLUSH;

    /* Send the ramrod */
    rc = ecore_sp_eth_filter_ucast(p_hwfn, p_vf->opaque_fid, &filter,
                                   ECORE_SPQ_MODE_CB, OSAL_NULL);
    if (rc) {
      DP_NOTICE(p_hwfn, true, "PF failed to configure VLAN for VF\n");
      return rc;
    }
    ...
    removal = filter.vlan ? 1 : p_vf->shadow_config.inner_vlan_removal;
    vport_update.inner_vlan_removal_flg = removal;
    vport_update.silent_vlan_removal_flg = filter.vlan ? 1 : 0;
    rc = ecore_sp_vport_update(p_hwfn, &vport_update, ECORE_SPQ_MODE_EBLOCK,
                               OSAL_NULL);
    if (rc) {
      DP_NOTICE(p_hwfn, true, "PF failed to configure VF vport for vlan\n");
      return rc;
    }
    ...
    for (i = 0; i < ECORE_MAX_VF_CHAINS_PER_PF; i++) {
      ...
      rc = ecore_sp_eth_rx_queues_update(p_hwfn, (void **)&p_cid, 1, 0, 1,
...
static void ecore_iov_vf_mbx_start_vport(struct ecore_hwfn *p_hwfn,
  ...
  if (!vf_info) {
    DP_NOTICE(p_hwfn->p_dev, true, "Failed to get VF info, invalid vfid [%d]\n",
              vf->relative_vf_id);
    return;
  }
  ...
  for (sb_id = 0; sb_id < vf->num_sbs; sb_id++) {
    ...
    ecore_int_cau_conf_sb(p_hwfn, p_ptt, start->sb_addr[sb_id],
  ...
  if (rc != ECORE_SUCCESS) {
    ...
    OSAL_IOV_POST_START_VPORT(p_hwfn, vf->relative_vf_id, vf->vport_id,
                              vf->opaque_fid);
...
static void ecore_iov_vf_mbx_start_rxq_resp(struct ecore_hwfn *p_hwfn,
                                            struct ecore_ptt *p_ptt,
                                            struct ecore_vf_info *vf, u8 status,
                                            bool b_legacy) {
  struct ecore_iov_vf_mbx *mbx = &vf->vf_mbx;
...
static u8 ecore_iov_vf_mbx_qid(struct ecore_hwfn *p_hwfn,
  ...
  p_qid_tlv = (struct vfpf_qid_tlv *)ecore_iov_search_list_tlvs(
      p_hwfn, p_mbx->req_virt, CHANNEL_TLV_QID);
  if (p_qid_tlv == OSAL_NULL) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%2x]: Failed to provide qid\n",
               p_vf->relative_vf_id);
...
static void ecore_iov_vf_mbx_start_rxq(struct ecore_hwfn *p_hwfn,
  ...
  p_cid =
      ecore_eth_queue_to_cid(p_hwfn, vf->opaque_fid, &params, true, &vf_params);
  if (p_cid == OSAL_NULL)
    goto out;
  ...
  rc = ecore_eth_rxq_start_ramrod(p_hwfn, p_cid, req->bd_max_bytes,
                                  req->rxq_addr, req->cqe_pbl_addr,
                                  req->cqe_pbl_size);
  if (rc != ECORE_SUCCESS) {
    status = PFVF_STATUS_FAILURE;
...
out:
  ecore_iov_vf_mbx_start_rxq_resp(p_hwfn, p_ptt, vf, status,
                                  !!(vf_legacy & ECORE_QCID_LEGACY_VF_RX_PROD));
}
...
static void ecore_iov_vf_mbx_update_tunn_param(struct ecore_hwfn *p_hwfn,
  ...
  if (!ecore_iov_pf_validate_tunn_param(p_req)) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "No tunnel update requested by VF\n");
    status = PFVF_STATUS_FAILURE;
    goto send_resp;
  }
  ...
  ecore_iov_pf_update_tun_param(p_req, &tunn.vxlan, &tunn.vxlan_port,
                                ...
                                p_req->update_vxlan_port, p_req->vxlan_port);
  ecore_iov_pf_update_tun_param(p_req, &tunn.l2_geneve, &tunn.geneve_port,
                                ECORE_MODE_L2GENEVE_TUNN, p_req->l2geneve_clss,
                                p_req->update_geneve_port, p_req->geneve_port);
  __ecore_iov_pf_update_tun_param(
      p_req, &tunn.ip_geneve, ECORE_MODE_IPGENEVE_TUNN, p_req->ipgeneve_clss);
  __ecore_iov_pf_update_tun_param(p_req, &tunn.l2_gre, ECORE_MODE_L2GRE_TUNN,
                                  p_req->l2gre_clss);
  __ecore_iov_pf_update_tun_param(p_req, &tunn.ip_gre, ECORE_MODE_IPGRE_TUNN,
                                  p_req->ipgre_clss);
  ...
  if (b_update_required) {
    ...
    ecore_for_each_vf(p_hwfn, i) {
      ecore_iov_bulletin_set_udp_ports(p_hwfn, i, p_tun->vxlan_port.port,
                                       geneve_port);
    }
  }

send_resp:
  p_resp = ecore_add_tlv(&mbx->offset, CHANNEL_TLV_UPDATE_TUNN_PARAM,
                         sizeof(*p_resp));
...
static void ecore_iov_vf_mbx_start_txq_resp(struct ecore_hwfn *p_hwfn,
                                            struct ecore_ptt *p_ptt,
                                            struct ecore_vf_info *p_vf, u32 cid,
                                            u8 status) {
  struct ecore_iov_vf_mbx *mbx = &p_vf->vf_mbx;
  struct pfvf_start_queue_resp_tlv *p_tlv;
  bool b_legacy = false;
...
static void ecore_iov_vf_mbx_start_txq(struct ecore_hwfn *p_hwfn,
  ...
  p_cid = ecore_eth_queue_to_cid(p_hwfn, vf->opaque_fid, &params, false,
                                 &vf_params);
  if (p_cid == OSAL_NULL)
    goto out;

  pq = ecore_get_cm_pq_idx_vf(p_hwfn, vf->relative_vf_id);
  rc = ecore_eth_txq_start_ramrod(p_hwfn, p_cid, req->pbl_addr, req->pbl_size,
                                  pq);
  if (rc != ECORE_SUCCESS) {
    status = PFVF_STATUS_FAILURE;
    ecore_eth_queue_cid_release(p_hwfn, p_cid);
...
out:
  ecore_iov_vf_mbx_start_txq_resp(p_hwfn, p_ptt, vf, cid, status);
}

static enum _ecore_status_t ecore_iov_vf_stop_rxqs(struct ecore_hwfn *p_hwfn,
                                                   struct ecore_vf_info *vf,
                                                   u16 rxq_id, u8 qid_usage_idx,
                                                   bool cqe_completion) {
  struct ecore_vf_queue *p_queue;
  enum _ecore_status_t rc = ECORE_SUCCESS;

  if (!ecore_iov_validate_rxq(p_hwfn, vf, rxq_id, ECORE_IOV_VALIDATE_Q_NA)) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
  ...
  rc = ecore_eth_rx_queue_stop(p_hwfn, p_queue->cids[qid_usage_idx].p_cid,
...
static enum _ecore_status_t ecore_iov_vf_stop_txqs(struct ecore_hwfn *p_hwfn,
  ...
  if (!ecore_iov_validate_txq(p_hwfn, vf, txq_id, ECORE_IOV_VALIDATE_Q_NA))
    return ECORE_INVAL;
  ...
  rc = ecore_eth_tx_queue_stop(p_hwfn, p_queue->cids[qid_usage_idx].p_cid);
  if (rc != ECORE_SUCCESS)
    return rc;
...
static void ecore_iov_vf_mbx_stop_rxqs(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_iov_vf_stop_rxqs(p_hwfn, vf, req->rx_qid, qid_usage_idx,
                              req->cqe_completion);
  if (rc == ECORE_SUCCESS)
    status = PFVF_STATUS_SUCCESS;
out:
  ecore_iov_prepare_resp(p_hwfn, p_ptt, vf, CHANNEL_TLV_STOP_RXQS, length,
                         status);
}
...
static void ecore_iov_vf_mbx_stop_txqs(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_iov_vf_stop_txqs(p_hwfn, vf, req->tx_qid, qid_usage_idx);
  if (rc == ECORE_SUCCESS)
    status = PFVF_STATUS_SUCCESS;

out:
  ecore_iov_prepare_resp(p_hwfn, p_ptt, vf, CHANNEL_TLV_STOP_TXQS, length,
                         status);
}
...
static void ecore_iov_vf_mbx_update_rxqs(struct ecore_hwfn *p_hwfn,
   ...
   */
  if ((vf->acquire.vfdev_info.capabilities & VFPF_ACQUIRE_CAP_QUEUE_QIDS) &&
      req->num_rxqs != 1) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
  ...
  for (i = req->rx_qid; i < req->rx_qid + req->num_rxqs; i++) {
    if (!ecore_iov_validate_rxq(p_hwfn, vf, i, ECORE_IOV_VALIDATE_Q_NA) ||
        !vf->vf_queues[i].cids[qid_usage_idx].p_cid ||
        vf->vf_queues[i].cids[qid_usage_idx].b_is_tx) {
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d]: Incorrect Rxqs [%04x, %02x]\n",
  ...
  rc = ecore_sp_eth_rx_queues_update(p_hwfn, (void **)&handlers, req->num_rxqs,
  ...
  status = PFVF_STATUS_SUCCESS;
out:
  ecore_iov_prepare_resp(p_hwfn, p_ptt, vf, CHANNEL_TLV_UPDATE_RXQ, length,
                         status);
}

static enum _ecore_status_t
ecore_iov_vf_pf_update_mtu(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                           struct ecore_vf_info *p_vf) {
  struct ecore_iov_vf_mbx *mbx = &p_vf->vf_mbx;
  struct ecore_sp_vport_update_params params;
  enum _ecore_status_t rc = ECORE_SUCCESS;
  ...
  if (rc)
    status = PFVF_STATUS_FAILURE;
send_status:
  ecore_iov_prepare_resp(p_hwfn, p_ptt, p_vf, CHANNEL_TLV_UPDATE_MTU,
                         sizeof(struct pfvf_def_resp_tlv), status);
  return rc;
}

void *ecore_iov_search_list_tlvs(struct ecore_hwfn *p_hwfn, void *p_tlvs_list,
                                 u16 req_type) {
  ...
  do {
    ...
    if (p_tlv->type == req_type) {
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
                 ...
                 qede_ecore_channel_tlvs_string[p_tlv->type], p_tlv->length);
      return p_tlv;
    }
    ...
    if ((len + p_tlv->length) > TLV_BUFFER_SIZE) {
      DP_NOTICE(p_hwfn, true, "TLVs has overrun the buffer size\n");
      return OSAL_NULL;
    }
...
ecore_iov_vp_update_act_param(struct ecore_hwfn *p_hwfn,
  ...
  p_act_tlv =
      (struct vfpf_vport_update_activate_tlv *)ecore_iov_search_list_tlvs(
          p_hwfn, p_mbx->req_virt, tlv);
  if (!p_act_tlv)
    return;
...
ecore_iov_vp_update_vlan_param(struct ecore_hwfn *p_hwfn,
  ...
  p_vlan_tlv =
      (struct vfpf_vport_update_vlan_strip_tlv *)ecore_iov_search_list_tlvs(
          p_hwfn, p_mbx->req_virt, tlv);
  if (!p_vlan_tlv)
    return;
...
ecore_iov_vp_update_tx_switch(struct ecore_hwfn *p_hwfn,
  ...
  p_tx_switch_tlv =
      (struct vfpf_vport_update_tx_switch_tlv *)ecore_iov_search_list_tlvs(
          p_hwfn, p_mbx->req_virt, tlv);
  if (!p_tx_switch_tlv)
    return;
...
static void ecore_iov_vp_update_mcast_bin_param(
    struct ecore_hwfn *p_hwfn, struct ecore_sp_vport_update_params *p_data,
    struct ecore_iov_vf_mbx *p_mbx, u16 *tlvs_mask) {
  struct vfpf_vport_update_mcast_bin_tlv *p_mcast_tlv;
  u16 tlv = CHANNEL_TLV_VPORT_UPDATE_MCAST;

  p_mcast_tlv =
      (struct vfpf_vport_update_mcast_bin_tlv *)ecore_iov_search_list_tlvs(
          p_hwfn, p_mbx->req_virt, tlv);
  if (!p_mcast_tlv)
    return;
...
static void ecore_iov_vp_update_accept_flag(
    struct ecore_hwfn *p_hwfn, struct ecore_sp_vport_update_params *p_data,
    struct ecore_iov_vf_mbx *p_mbx, u16 *tlvs_mask) {
  ...
  p_accept_tlv =
      (struct vfpf_vport_update_accept_param_tlv *)ecore_iov_search_list_tlvs(
          p_hwfn, p_mbx->req_virt, tlv);
  if (!p_accept_tlv)
    return;
...
static void ecore_iov_vp_update_accept_any_vlan(
    struct ecore_hwfn *p_hwfn, struct ecore_sp_vport_update_params *p_data,
    struct ecore_iov_vf_mbx *p_mbx, u16 *tlvs_mask) {
...
static void ecore_iov_vp_update_rss_param(
    struct ecore_hwfn *p_hwfn, struct ecore_vf_info *vf,
    struct ecore_sp_vport_update_params *p_data, struct ecore_rss_params *p_rss,
    struct ecore_iov_vf_mbx *p_mbx, u16 *tlvs_mask, u16 *tlvs_accepted) {
  ...
  p_rss_tlv = (struct vfpf_vport_update_rss_tlv *)ecore_iov_search_list_tlvs(
      p_hwfn, p_mbx->req_virt, tlv);
  if (!p_rss_tlv) {
  ...
  p_rss->update_rss_config =
      !!(p_rss_tlv->update_rss_flags & VFPF_UPDATE_RSS_CONFIG_FLAG);
  p_rss->update_rss_capabilities =
      !!(p_rss_tlv->update_rss_flags & VFPF_UPDATE_RSS_CAPS_FLAG);
  p_rss->update_rss_ind_table =
      !!(p_rss_tlv->update_rss_flags & VFPF_UPDATE_RSS_IND_TABLE_FLAG);
  ...
  OSAL_MEMCPY(p_rss->rss_key, p_rss_tlv->rss_key, sizeof(p_rss->rss_key));
...
static void ecore_iov_vp_update_sge_tpa_param(
    struct ecore_hwfn *p_hwfn, struct ecore_sp_vport_update_params *p_data,
    struct ecore_sge_tpa_params *p_sge_tpa, struct ecore_iov_vf_mbx *p_mbx,
    u16 *tlvs_mask) {
  struct vfpf_vport_update_sge_tpa_tlv *p_sge_tpa_tlv;
  u16 tlv = CHANNEL_TLV_VPORT_UPDATE_SGE_TPA;

  p_sge_tpa_tlv =
      (struct vfpf_vport_update_sge_tpa_tlv *)ecore_iov_search_list_tlvs(
          p_hwfn, p_mbx->req_virt, tlv);
  ...
  p_sge_tpa->update_tpa_param_flg =
      !!(p_sge_tpa_tlv->update_sge_tpa_flags & VFPF_UPDATE_TPA_PARAM_FLAG);
...
static void ecore_iov_vf_mbx_vport_update(struct ecore_hwfn *p_hwfn,
  ...
  ecore_iov_vp_update_sge_tpa_param(p_hwfn, &params, &sge_tpa_params, mbx,
                                    &tlvs_mask);
   ...
   */
  ecore_iov_vp_update_rss_param(p_hwfn, vf, &params, p_rss_params, mbx,
   ...
   */
  if (OSAL_IOV_VF_VPORT_UPDATE(p_hwfn, vf->relative_vf_id, &params,
                               &tlvs_accepted) != ECORE_SUCCESS) {
    tlvs_accepted = 0;
    status = PFVF_STATUS_NOT_SUPPORTED;
    goto out;
  }
  ...
  if (p_params->opcode == ECORE_FILTER_ADD ||
    ...
    if (i == ECORE_ETH_VF_NUM_VLAN_FILTERS + 1) {
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
                 ...
                 " vlan filters\n",
                 p_vf->relative_vf_id, ECORE_ETH_VF_NUM_VLAN_FILTERS + 1);
      return ECORE_INVAL;
    }
  }
...
ecore_iov_vf_update_mac_shadow(struct ecore_hwfn *p_hwfn,
  ...
  if (p_params->opcode == ECORE_FILTER_REMOVE) {
    for (i = 0; i < ECORE_ETH_VF_NUM_MAC_FILTERS; i++) {
      if (!OSAL_MEMCMP(p_vf->shadow_config.macs[i], p_params->mac, ETH_ALEN)) {
  ...
  for (i = 0; i < ECORE_ETH_VF_NUM_MAC_FILTERS; i++) {
    if (!OSAL_MEMCMP(p_vf->shadow_config.macs[i], empty_mac, ETH_ALEN)) {
      OSAL_MEMCPY(p_vf->shadow_config.macs[i], p_params->mac, ETH_ALEN);
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "Added MAC at %d entry in shadow\n", i);
      break;
    }
  }

  if (i == ECORE_ETH_VF_NUM_MAC_FILTERS) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "No available place for MAC\n");
    return ECORE_INVAL;
  }
...
static void ecore_iov_vf_mbx_ucast_filter(struct ecore_hwfn *p_hwfn,
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
             ...
             params.is_rx_filter ? "RX" : "", params.is_tx_filter ? "TX" : "",
             params.vport_to_add_to, params.mac[0], params.mac[1],
             params.mac[2], params.mac[3], params.mac[4], params.mac[5],
             params.vlan);
   ...
   */
  if (ecore_iov_vf_update_unicast_shadow(p_hwfn, vf, &params) != ECORE_SUCCESS)
    goto out;
...
static void ecore_iov_vf_mbx_int_cleanup(struct ecore_hwfn *p_hwfn,
  ...
  ecore_iov_prepare_resp(p_hwfn, p_ptt, vf, CHANNEL_TLV_INT_CLEANUP,
                         sizeof(struct pfvf_def_resp_tlv), PFVF_STATUS_SUCCESS);
}
...
static void ecore_iov_vf_mbx_close(struct ecore_hwfn *p_hwfn,
  ...
  ecore_iov_prepare_resp(p_hwfn, p_ptt, vf, CHANNEL_TLV_CLOSE, length, status);
}
...
static void ecore_iov_vf_mbx_release(struct ecore_hwfn *p_hwfn,
  ...
  if (p_vf->state != VF_STOPPED && p_vf->state != VF_FREE) {
    /* Stopping the VF */
    rc = ecore_sp_vf_stop(p_hwfn, p_vf->concrete_fid, p_vf->opaque_fid);
  ...
  ecore_iov_prepare_resp(p_hwfn, p_ptt, p_vf, CHANNEL_TLV_RELEASE, length,
                         status);
}
...
static void ecore_iov_vf_pf_get_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  if (b_is_rx) {
    if (!ecore_iov_validate_rxq(p_hwfn, p_vf, qid,
                                ECORE_IOV_VALIDATE_Q_ENABLE)) {
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d]: Invalid Rx queue_id = %d\n",
                 p_vf->abs_vf_id, qid);
      goto send_resp;
    }
    ...
    if (!ecore_iov_validate_txq(p_hwfn, p_vf, qid,
                                ECORE_IOV_VALIDATE_Q_ENABLE)) {
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d]: Invalid Tx queue_id = %d\n",
                 p_vf->abs_vf_id, qid);
      goto send_resp;
    }
    for (i = 0; i < MAX_QUEUES_PER_QZONE; i++) {
      p_queue = &p_vf->vf_queues[qid];
      if ((p_queue->cids[i].p_cid == OSAL_NULL) || (!p_queue->cids[i].b_is_tx))
        continue;

      p_cid = p_queue->cids[i].p_cid;

      rc = ecore_get_txq_coalesce(p_hwfn, p_ptt, p_cid, &coal);
      if (rc != ECORE_SUCCESS)
        goto send_resp;
      break;
    }
...
send_resp:
  p_resp =
      ecore_add_tlv(&mbx->offset, CHANNEL_TLV_COALESCE_READ, sizeof(*p_resp));
  p_resp->coal = coal;
...
static void ecore_iov_vf_pf_set_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  if (!ecore_iov_validate_rxq(p_hwfn, vf, qid, ECORE_IOV_VALIDATE_Q_ENABLE) &&
      rx_coal) {
    DP_ERR(p_hwfn, "VF[%d]: Invalid Rx queue_id = %d\n", vf->abs_vf_id, qid);
    goto out;
  }

  if (!ecore_iov_validate_txq(p_hwfn, vf, qid, ECORE_IOV_VALIDATE_Q_ENABLE) &&
      tx_coal) {
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
             "VF[%d]: Setting coalesce for VF rx_coal = %d, tx_coal = %d at "
             "queue = %d\n",
...
static enum _ecore_status_t
ecore_iov_vf_flr_poll_pbf(struct ecore_hwfn *p_hwfn, struct ecore_vf_info *p_vf,
                          struct ecore_ptt *p_ptt) {
  ...
  for (port_id = 0; port_id < max_ports_per_engine; port_id++) {
    ...
    for (tc = 0; tc < max_phys_tcs_per_port + 1; tc++) {
      ...
      cons[voq] = ecore_rd(p_hwfn, p_ptt, cons_voq0_addr + voq * 0x40);
      prod = ecore_rd(p_hwfn, p_ptt, prod_voq0_addr + voq * 0x40);
  ...
  for (cnt = 0; cnt < 50; cnt++) {
    for (; port_id < max_ports_per_engine; port_id++) {
      ...
      for (; tc < max_phys_tcs_per_port + 1; tc++) {
        tc_id = (tc < max_phys_tcs_per_port) ? tc : PURE_LB_TC;
        voq = VOQ(port_id, tc_id, max_phys_tcs_per_port);
        tmp = ecore_rd(p_hwfn, p_ptt, cons_voq0_addr + voq * 0x40);
...
ecore_iov_execute_vf_flr_cleanup(struct ecore_hwfn *p_hwfn,
  ...
  if (p_hwfn->pf_iov_info->pending_flr[rel_vf_id / 64] &
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d] - Handling FLR\n", vfid);
     ...
     */
    REG_WR(p_hwfn,
           GTT_BAR0_MAP_REG_USDM_RAM + USTORM_VF_PF_CHANNEL_READY_OFFSET(vfid),
           1);
...
bool ecore_iov_mark_vf_flr(struct ecore_hwfn *p_hwfn, u32 *p_disabled_vfs) {
  ...
  for (i = 0; i < VF_BITMAP_SIZE_IN_DWORDS; i++)
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "[%08x,...,%08x]: %08x\n", i * 32,
               (i + 1) * 32 - 1, p_disabled_vfs[i]);
  ...
  for (i = 0; i < p_hwfn->p_dev->p_iov_info->total_vfs; i++) {
    ...
    if ((1 << (vfid % 32)) & p_disabled_vfs[vfid / 32]) {
      ...
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF[%d] [rel %d] got FLR-ed\n", vfid,
                 rel_vf_id);
...
void ecore_iov_process_mbx_req(struct ecore_hwfn *p_hwfn,
  ...
  OSAL_IOV_VF_MSG_TYPE(p_hwfn, p_vf->relative_vf_id, mbx->first_tlv.tl.type);

  /* Lock the per vf op mutex and note the locker's identity.
   * The unlock will take place in mbx response.
   */
  ecore_iov_lock_vf_pf_channel(p_hwfn, p_vf, mbx->first_tlv.tl.type);

  /* check if tlv type is known */
  if (ecore_iov_tlv_supported(mbx->first_tlv.tl.type) && !p_vf->b_malicious) {
    ...
    ecore_iov_prepare_resp(p_hwfn, p_ptt, p_vf, mbx->first_tlv.tl.type,
    ...
    DP_NOTICE(p_hwfn, false,
              ...
              p_vf->abs_vf_id, mbx->first_tlv.tl.type, mbx->first_tlv.tl.length,
    ...
    if (p_vf->acquire.first_tlv.reply_address &&
        (mbx->first_tlv.reply_address == p_vf->acquire.first_tlv.reply_address))
      ecore_iov_prepare_resp(p_hwfn, p_ptt, p_vf, mbx->first_tlv.tl.type,
                             sizeof(struct pfvf_def_resp_tlv),
  ...
  ecore_iov_unlock_vf_pf_channel(p_hwfn, p_vf, mbx->first_tlv.tl.type);
...
static enum _ecore_status_t ecore_sriov_vfpf_msg(struct ecore_hwfn *p_hwfn,
  ...
  struct ecore_vf_info *p_vf = ecore_sriov_get_vf_from_absid(p_hwfn, abs_vfid);
...
static void ecore_sriov_vfpf_malicious(struct ecore_hwfn *p_hwfn,
  ...
  if (!p_vf->b_malicious) {
    DP_NOTICE(p_hwfn, false, "VF [%d] - Malicious behavior [%02x]\n",
              p_vf->abs_vf_id, p_data->err_id);

    p_vf->b_malicious = true;
  } else {
    DP_INFO(p_hwfn, "VF [%d] - Malicious behavior [%02x]\n", p_vf->abs_vf_id,
            p_data->err_id);
  }
...
static enum _ecore_status_t
ecore_sriov_eqe_event(struct ecore_hwfn *p_hwfn, u8 opcode, __le16 echo,
                      union event_ring_data *data,
  ...
  case COMMON_EVENT_VF_FLR:
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "VF-FLR is still not supported\n");
    return ECORE_SUCCESS;
...
enum _ecore_status_t ecore_iov_copy_vf_msg(struct ecore_hwfn *p_hwfn,
  ...
  if (ecore_dmae_host2host(p_hwfn, ptt, vf_info->vf_mbx.pending_req,
                           vf_info->vf_mbx.req_phys,
                           sizeof(union vfpf_tlvs) / 4, &params)) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "Failed to copy message from VF 0x%02x\n",
               vfid);
...
void ecore_iov_bulletin_set_forced_mac(struct ecore_hwfn *p_hwfn, u8 *mac,
  ...
  if (p_hwfn->pf_params.eth_pf_params.allow_vf_mac_change ||
    ...
    vf_info->bulletin.p_virt->valid_bitmap &= ~(1 << MAC_ADDR_FORCED);
  } else {
    feature = 1 << MAC_ADDR_FORCED;
    /* Forced MAC will disable MAC_ADDR */
    vf_info->bulletin.p_virt->valid_bitmap &= ~(1 << VFPF_BULLETIN_MAC_ADDR);
  }
...
ecore_iov_bulletin_set_forced_untagged_default(struct ecore_hwfn *p_hwfn,
  ...
  vf_info->bulletin.p_virt->default_only_untagged = b_untagged_only ? 1 : 0;
...
void ecore_iov_bulletin_set_udp_ports(struct ecore_hwfn *p_hwfn, int vfid,
  ...
  vf_info = ecore_iov_get_vf_info(p_hwfn, (u16)vfid, true);
  if (!vf_info) {
    DP_NOTICE(p_hwfn->p_dev, true, "Can not set udp ports, invalid vfid [%d]\n",
              vfid);
    return;
  }
...
enum _ecore_status_t ecore_iov_spoofchk_set(struct ecore_hwfn *p_hwfn, int vfid,
  ...
  if (!ecore_iov_pf_sanity_check(p_hwfn, vfid)) {
    DP_NOTICE(p_hwfn, true, "SR-IOV sanity check failed, can't set spoofchk\n");
    goto out;
  }
...
u8 ecore_iov_vf_chains_per_pf(struct ecore_hwfn *p_hwfn) {
  ...
  max_chains_per_vf =
      (max_chains_per_vf) ? max_chains_per_vf : ECORE_MAX_VF_CHAINS_PER_PF;
...
u32 ecore_iov_pfvf_msg_length(void) { return sizeof(union pfvf_tlvs); }

u8 *ecore_iov_bulletin_get_mac(struct ecore_hwfn *p_hwfn, u16 rel_vf_id) {
  struct ecore_vf_info *p_vf;
  ...
  if (!(p_vf->bulletin.p_virt->valid_bitmap & (1 << VFPF_BULLETIN_MAC_ADDR)))
    return OSAL_NULL;
...
enum _ecore_status_t ecore_iov_configure_min_tx_rate(struct ecore_dev *p_dev,
  ...
  if (!vf) {
    DP_NOTICE(p_dev, true, "Getting vf info failed, can't set min rate\n");
    return ECORE_INVAL;
  }
...
enum _ecore_status_t ecore_iov_get_vf_stats(struct ecore_hwfn *p_hwfn,
  ...
  __ecore_get_vport_stats(p_hwfn, p_ptt, p_stats, vf->abs_vf_id + 0x10, false);
