...
#define IS_ECORE_PACING(p_hwfn) (!!(p_hwfn->b_en_pacing))
...
/* cau states */
enum ecore_coalescing_mode { ECORE_COAL_MODE_DISABLE, ECORE_COAL_MODE_ENABLE };
...
#define MASK_FIELD(_name, _value) ((_value) &= (_name##_MASK))

#define FIELD_VALUE(_name, _value) ((_value & _name##_MASK) << _name##_SHIFT)
...
#define GET_FIELD(value, name) (((value) >> (name##_SHIFT)) & name##_MASK)
...
static OSAL_INLINE u32 DB_ADDR(u32 cid, u32 DEMS) {
  u32 db_addr =
      FIELD_VALUE(DB_LEGACY_ADDR_DEMS, DEMS) | (cid * ECORE_PF_DEMS_SIZE);
...
#define D_TRINE(val, cond1, cond2, true1, true2, def)                          \
  (val == (cond1) ? true1 : (val == (cond2) ? true2 : def))
...
#define RESC_END(_p_hwfn, resc)                                                \
  (RESC_START(_p_hwfn, resc) + RESC_NUM(_p_hwfn, resc))
#define FEAT_NUM(_p_hwfn, resc) ((_p_hwfn)->hw_info.feat_num[resc])

  /* Amount of traffic classes HW supports */
  u8 num_hw_tc;

  /* Amount of TCs which should be active according to DCBx or upper layer
...
enum ecore_ufp_pri_type { ECORE_UFP_PRI_OS, ECORE_UFP_PRI_VNIC };
...
#ifndef ASIC_ONLY
#define ECORE_IS_BB_B0(dev)                                                    \
  ((ECORE_IS_BB(dev) && CHIP_REV_IS_B0(dev)) || (CHIP_REV_IS_TEDIBEAR(dev)))
#else
...
#else
#define CHIP_REV_IS_A0(_p_dev) (!(_p_dev)->chip_rev && !(_p_dev)->chip_metal)
#define CHIP_REV_IS_B0(_p_dev)                                                 \
  ((_p_dev)->chip_rev == 1 && !(_p_dev)->chip_metal)
#endif
...
#define ECORE_AFFIN_HWFN_IDX(dev) (IS_LEAD_HWFN(ECORE_AFFIN_HWFN(dev)) ? 0 : 1)
...
#define NUM_OF_VFS(dev) ecore_get_hsi_def_val(dev, ECORE_HSI_DEF_MAX_NUM_VFS)
...
#define MFW_PORT(_p_hwfn)                                                      \
  ((_p_hwfn)->abs_pf_id % ecore_device_num_ports((_p_hwfn)->p_dev))
