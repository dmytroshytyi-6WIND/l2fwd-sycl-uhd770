...
#define OSAL_MEMSET(dst, val, length) memset(dst, val, length)

void *osal_dma_alloc_coherent(struct ecore_dev *, dma_addr_t *, size_t);

void *osal_dma_alloc_coherent_aligned(struct ecore_dev *, dma_addr_t *, size_t,
                                      int);
...
#define REG_RD(_p_hwfn, _reg_offset)                                           \
  DIRECT_REG_RD(_p_hwfn, ((u8 *)(uintptr_t)(_p_hwfn->regview) + (_reg_offset)))
...
#define REG_WR(_p_hwfn, _reg_offset, _val)                                     \
  DIRECT_REG_WR(NULL, ((u8 *)((uintptr_t)(_p_hwfn->regview)) + (_reg_offset)), \
                (u32)_val)

#define REG_WR16(_p_hwfn, _reg_offset, _val)                                   \
  DIRECT_REG_WR16(((u8 *)(uintptr_t)(_p_hwfn->regview) + (_reg_offset)),       \
                  (u16)_val)

#define DOORBELL(_p_hwfn, _db_addr, _val)                                      \
  DIRECT_REG_WR_RELAXED((_p_hwfn),                                             \
                        ((u8 *)(uintptr_t)(_p_hwfn->doorbells) + (_db_addr)),  \
                        (u32)_val)
...
#define OSAL_DPC_INIT(dpc, hwfn) OSAL_SPIN_LOCK_INIT(&(hwfn)->spq_lock)
#define OSAL_POLL_MODE_DPC(hwfn) osal_poll_mode_dpc((osal_int_ptr_t)(p_hwfn))
...
#define OSAL_LIST_FIRST_ENTRY(list, type, field) (type *)((list)->head)
...
#define OSAL_LIST_IS_EMPTY(list) ((list)->cnt == 0)

#define OSAL_LIST_NEXT(entry, field, type) (type *)((&((entry)->field))->next)

/* TODO: Check field, type order */

#define OSAL_LIST_FOR_EACH_ENTRY(entry, list, field, type)                     \
  for (entry = OSAL_LIST_FIRST_ENTRY(list, type, field); entry;                \
...
#define OSAL_LIST_FOR_EACH_ENTRY_SAFE(entry, tmp_entry, list, field, type)     \
  for (entry = OSAL_LIST_FIRST_ENTRY(list, type, field),                       \
      tmp_entry = (entry) ? OSAL_LIST_NEXT(entry, field, type) : NULL;         \
       entry != NULL; entry = (type *)tmp_entry,                               \
      tmp_entry = (entry) ? OSAL_LIST_NEXT(entry, field, type) : NULL)
...
/* Bitops */
#define OSAL_SET_BIT(bit, bitmap) rte_bit_relaxed_set32(bit, bitmap)

#define OSAL_CLEAR_BIT(bit, bitmap) rte_bit_relaxed_clear32(bit, bitmap)

#define OSAL_GET_BIT(bit, bitmap) rte_bit_relaxed_get32(bit, bitmap)

u32 qede_find_first_bit(unsigned long *, u32);
#define OSAL_FIND_FIRST_BIT(bitmap, length) qede_find_first_bit(bitmap, length)
...
#define OSAL_VF_FLR_UPDATE(hwfn) osal_vf_flr_update(hwfn)
...
#define OSAL_PF_VF_MSG(hwfn, vfid) osal_pf_vf_msg(hwfn)
...
u32 qede_unzip_data(struct ecore_hwfn *p_hwfn, u32 input_len, u8 *input_buf,
...
#define OSAL_ROUNDUP_POW_OF_TWO(val) qede_log2_align(val)

u32 qede_osal_log2(u32);
#define OSAL_LOG2(val) qede_osal_log2(val)
...
#define OSAL_SPRINTF(name, pattern, ...) sprintf(name, pattern, ##__VA_ARGS__)
...
void qed_set_platform_str(struct ecore_hwfn *p_hwfn, char *buf_str,
                          u32 buf_size);
