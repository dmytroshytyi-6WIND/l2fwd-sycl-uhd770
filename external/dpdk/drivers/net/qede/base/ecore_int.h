...
#define SB_ALIGNED_SIZE(p_hwfn) ALIGNED_TYPE_SIZE(struct status_block, p_hwfn)
 ...
 * @param p_ptt
 */
int ecore_int_igu_reset_cam(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt);
...
#define ECORE_MAPPING_MEMORY_SIZE(dev)                                         \
  ((CHIP_REV_IS_SLOW(dev) && (!(dev)->b_is_emul_full)) ? 136 : NUM_OF_SBS(dev))
#else
