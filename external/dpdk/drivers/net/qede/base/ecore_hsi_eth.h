...
struct eth_in_to_in_pri_map_cfg {
  ...
  u8 reserved[7];
  /* Map for inner to inner vlan priority translation for Non RDMA protocols,
   * used for TenantDcb. Set inner_vlan_pri_remap_en, when init the map.
   */
  u8 non_rdma_in_to_in_pri_map[8];
  /* Map for inner to inner vlan priority translation for RDMA protocols, used
 ...
 * eth IPv4 Fragment Type
 */
enum eth_ip_type { ETH_IPV4 /* IPv4 */, ETH_IPV6 /* IPv6 */, MAX_ETH_IP_TYPE };
...
#define ETH_VPORT_RSS_CONFIG_RESERVED0_SHIFT 7
   ...
   * Total number of RSS engines is ETH_RSS_ENGINE_NUM_ , according to chip
   * type.
   */
  u8 rss_id;
...
struct rx_queue_start_ramrod_data {
  ...
  u8 vf_rx_prod_index;
  /* Backward compatibility mode. If set, unprotected mStorm queue zone will
   * used for VF RX producers instead of VF zone.
   */
...
struct vport_start_ramrod_data {
  ...
  /* If set, ETH header padding will not inserted. placement_offset will be
   * zero.
   */
...
struct vport_update_ramrod_data_cmn {
  ...
  /* Set to remove vlan silently, update_inner_vlan_removal_en_flg must be
   * enabled as well. If Rx is in noSgl mode send rx_queue_update_ramrod_data
   */
  u8 silent_vlan_removal_en;
  ...
  /* If set, control frames will be filtered according to ethtype check. */
  u8 ctl_frame_ethtype_check_en;
  /* Indicates to update RDMA or NON-RDMA vlan remapping priority table
   * according to update_in_to_in_pri_map_mode_enum, used for TenantDcb (use
   * enum update_in_to_in_pri_map_mode_enum)
   */
