...
void *osal_dma_alloc_coherent(struct ecore_dev *p_dev, dma_addr_t *phys,
                              size_t size) {
...
void *osal_dma_alloc_coherent_aligned(struct ecore_dev *p_dev, dma_addr_t *phys,
...
void osal_dma_free_mem(struct ecore_dev *p_dev, dma_addr_t phys) {
  ...
  for (j = 0; j < ecore_mz_count; j++) {
    if (phys == ecore_mz_mapping[j]->iova) {
      DP_VERBOSE(p_dev, ECORE_MSG_SP, "Free memzone %s\n",
...
u32 qede_unzip_data(struct ecore_hwfn *p_hwfn, u32 input_len, u8 *input_buf,
  ...
  if (rc != Z_OK) {
    DP_ERR(p_hwfn, "zlib init failed, rc = %d\n", rc);
    return 0;
  }
  ...
  if (rc != Z_OK && rc != Z_STREAM_END) {
    DP_ERR(p_hwfn, "FW unzip error: %s, rc=%d\n", p_hwfn->stream->msg, rc);
    return 0;
  }
...
void qede_get_mcp_proto_stats(struct ecore_dev *edev,
