...
struct ecore_iov_vf_mbx {
  ...
  u8 flags;
#define VF_MSG_INPROCESS                                                       \
  0x1 /* failsafe - the FW should prevent                                      \
...
struct ecore_vf_info {
  ...
  u8 relative_vf_id;
  u8 abs_vf_id;
#define ECORE_VF_ABS_ID(p_hwfn, p_vf)                                          \
  (ECORE_PATH_ID(p_hwfn) ? (p_vf)->abs_vf_id + MAX_NUM_VFS_BB                  \
                         : (p_vf)->abs_vf_id)
...
#define ECORE_IOV_CONFIGURED_FEATURES_MASK                                     \
  ((1 << MAC_ADDR_FORCED) | (1 << VLAN_ADDR_FORCED))
};
 ...
 */
bool ecore_iov_mark_vf_flr(struct ecore_hwfn *p_hwfn, u32 *disabled_vfs);
