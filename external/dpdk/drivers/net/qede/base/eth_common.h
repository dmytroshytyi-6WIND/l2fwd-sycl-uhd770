...
struct eth_tx_data_1st_bd {
  ...
  __le16 vlan;
  /* Number of BDs in packet. Should be at least 1 in non-LSO packet and at
   * least 3 in LSO (or Tunnel with IPv6+ext) packet.
   */
  u8 nbds;
...
enum eth_l4_pseudo_checksum_mode {
  /* Pseudo Header checksum on packet is calculated with the correct packet
   * length field.
   */
...
enum eth_tpa_end_reason {
  ...
  /* Next segment cant be aggregated due to LLC/SNAP, IP error, IP fragment,
   * IPv4 options, IPv6 extension, IP ECN = CE, TCP errors, TCP options, zero
   * TCP payload length , TCP flags or not supported tunnel header options.
   */
