...
enum _ecore_status_t ecore_l2_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  pp_qids =
      OSAL_VZALLOC(p_hwfn->p_dev, sizeof(unsigned long *) * p_l2_info->queues);
  if (pp_qids == OSAL_NULL)
    return ECORE_NOMEM;
  p_l2_info->pp_qid_usage = pp_qids;

  for (i = 0; i < p_l2_info->queues; i++) {
    pp_qids[i] = OSAL_VZALLOC(p_hwfn->p_dev, MAX_QUEUES_PER_QZONE / 8);
...
_ecore_eth_queue_to_cid(struct ecore_hwfn *p_hwfn, u16 opaque_fid, u32 cid,
  ...
  rc = ecore_fw_l2_queue(p_hwfn, p_cid->rel.queue_id, &p_cid->abs.queue_id);
  if (rc != ECORE_SUCCESS)
    goto fail;
   ...
   */
  if (p_cid->vfid == ECORE_QUEUE_CID_PF) {
    rc = ecore_fw_vport(p_hwfn, p_cid->rel.stats_id, &p_cid->abs.stats_id);
    if (rc != ECORE_SUCCESS)
      goto fail;
  } else {
...
out:
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_SP,
             "opaque_fid: %04x CID %08x vport %02x [%02x] qzone %04x.%02x "
             "[%04x] stats %02x [%02x] SB %04x PI %02x\n",
...
ecore_eth_queue_to_cid(struct ecore_hwfn *p_hwfn, u16 opaque_fid,
   ...
   */
  if (IS_PF(p_hwfn->p_dev) && !b_legacy_vf) {
    if (_ecore_cxt_acquire_cid(p_hwfn, PROTOCOLID_ETH, &cid, vfid) !=
        ECORE_SUCCESS) {
  ...
  p_cid = _ecore_eth_queue_to_cid(p_hwfn, opaque_fid, cid, p_params, b_is_rx,
                                  p_vf_params);
...
ecore_sp_vport_start(struct ecore_hwfn *p_hwfn,
                     struct ecore_sp_vport_start_params *p_params) {
  if (IS_VF(p_hwfn->p_dev))
    return ecore_vf_pf_vport_start(
        p_hwfn, p_params->vport_id, p_params->mtu, p_params->remove_inner_vlan,
...
ecore_sp_vport_update_rss(struct ecore_hwfn *p_hwfn,
  ...
  OSAL_BUILD_BUG_ON(ECORE_RSS_IND_TABLE_SIZE != ETH_RSS_IND_TABLE_ENTRIES_NUM);
  ...
  table_size =
      OSAL_MIN_T(int, ECORE_RSS_IND_TABLE_SIZE, 1 << p_config->tbl_size);
  for (i = 0; i < table_size; i++) {
    ...
    p_config->indirection_table[i] = OSAL_CPU_TO_LE16(p_queue->abs.queue_id);
  }
...
ecore_sp_update_accept_mode(struct ecore_hwfn *p_hwfn,
  ...
  p_ramrod->common.update_rx_mode_flg = accept_flags.update_rx_mode_config;
  p_ramrod->common.update_tx_mode_flg = accept_flags.update_tx_mode_config;
...
enum _ecore_status_t ecore_sp_vport_update(
    struct ecore_hwfn *p_hwfn, struct ecore_sp_vport_update_params *p_params,
...
ecore_filter_accept_cmd(struct ecore_dev *p_dev, u8 vport,
                        struct ecore_filter_accept_flags accept_flags,
                        u8 update_accept_any_vlan, u8 accept_any_vlan,
  ...
  for_each_hwfn(p_dev, i) {
    ...
    rc = ecore_sp_vport_update(p_hwfn, &vport_update_params, comp_mode,
                               p_comp_data);
    if (rc != ECORE_SUCCESS) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_SP,
               ...
               accept_flags.rx_accept_filter, accept_flags.tx_accept_filter);

    if (update_accept_any_vlan)
      DP_VERBOSE(p_hwfn, ECORE_MSG_SP, "accept_any_vlan=%d configured\n",
                 accept_any_vlan);
  }
...
enum _ecore_status_t ecore_eth_rxq_start_ramrod(
    struct ecore_hwfn *p_hwfn, struct ecore_queue_cid *p_cid, u16 bd_max_bytes,
  ...
  DP_VERBOSE(
      ...
      p_cid->opaque_fid, p_cid->cid, p_cid->abs.queue_id, p_cid->abs.vport_id,
      p_cid->sb_igu_id);
...
static enum _ecore_status_t ecore_eth_pf_rx_queue_start(
    struct ecore_hwfn *p_hwfn, struct ecore_queue_cid *p_cid, u16 bd_max_bytes,
    dma_addr_t bd_chain_phys_addr, dma_addr_t cqe_pbl_addr, u16 cqe_pbl_size,
    void OSAL_IOMEM **pp_prod) {
  u32 init_prod_val = 0;

  *pp_prod = (u8 OSAL_IOMEM *)p_hwfn->regview + GTT_BAR0_MAP_REG_MSDM_RAM +
             MSTORM_ETH_PF_PRODS_OFFSET(p_cid->abs.queue_id);

  /* Init the rcq, rx bd and rx sge (if valid) producers to 0 */
  __internal_ram_wr(p_hwfn, *pp_prod, sizeof(u32), (u32 *)(&init_prod_val));
...
ecore_eth_rx_queue_start(struct ecore_hwfn *p_hwfn, u16 opaque_fid,
  ...
  if (IS_PF(p_hwfn->p_dev))
    ...
    rc = ecore_vf_pf_rxq_start(p_hwfn, p_cid, bd_max_bytes, bd_chain_phys_addr,
...
ecore_sp_eth_rx_queues_update(struct ecore_hwfn *p_hwfn, void **pp_rxq_handles,
                              u8 num_rxqs, u8 complete_cqe_flg,
  ...
  if (IS_VF(p_hwfn->p_dev))
    return ecore_vf_pf_rxqs_update(
        p_hwfn, (struct ecore_queue_cid **)pp_rxq_handles, num_rxqs,
...
ecore_eth_pf_rx_queue_stop(struct ecore_hwfn *p_hwfn,
  ...
  p_ramrod->complete_cqe_flg =
      ((p_cid->vfid == ECORE_QUEUE_CID_PF) && !b_eq_completion_only) ||
      b_cqe_completion;
...
enum _ecore_status_t ecore_eth_rx_queue_stop(struct ecore_hwfn *p_hwfn,
  ...
  if (IS_PF(p_hwfn->p_dev))
    rc = ecore_eth_pf_rx_queue_stop(p_hwfn, p_cid, eq_completion_only,
                                    cqe_completion);
  else
...
enum _ecore_status_t ecore_eth_txq_start_ramrod(struct ecore_hwfn *p_hwfn,
                                                struct ecore_queue_cid *p_cid,
                                                dma_addr_t pbl_addr,
                                                u16 pbl_size, u16 pq_id) {
...
static enum _ecore_status_t ecore_eth_pf_tx_queue_start(
    struct ecore_hwfn *p_hwfn, struct ecore_queue_cid *p_cid, u8 tc,
    dma_addr_t pbl_addr, u16 pbl_size, void OSAL_IOMEM **pp_doorbell) {
  enum _ecore_status_t rc;
  u16 pq_id;
  ...
  rc = ecore_eth_txq_start_ramrod(p_hwfn, p_cid, pbl_addr, pbl_size, pq_id);
  if (rc != ECORE_SUCCESS)
    return rc;

  /* Provide the caller with the necessary return values */
  *pp_doorbell =
      (u8 OSAL_IOMEM *)p_hwfn->doorbells + DB_ADDR(p_cid->cid, DQ_DEMS_LEGACY);
...
ecore_eth_tx_queue_start(struct ecore_hwfn *p_hwfn, u16 opaque_fid,
                         struct ecore_queue_start_common_params *p_params,
                         u8 tc, dma_addr_t pbl_addr, u16 pbl_size,
  ...
  if (IS_PF(p_hwfn->p_dev))
    rc = ecore_eth_pf_tx_queue_start(p_hwfn, p_cid, tc, pbl_addr, pbl_size,
...
ecore_filter_ucast_common(struct ecore_hwfn *p_hwfn, u16 opaque_fid,
  ...
  rc = ecore_fw_vport(p_hwfn, p_filter_cmd->vport_to_add_to, &vport_to_add_to);
  if (rc != ECORE_SUCCESS)
    return rc;
  ...
  *pp_ramrod = &(*pp_ent)->ramrod.vport_filter_update;
...
#ifndef ASIC_ONLY
  if (CHIP_REV_IS_SLOW(p_hwfn->p_dev)) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_SP, "Non-Asic - prevent Tx filters\n");
  ...
  if ((p_first_filter->type == ETH_FILTER_TYPE_MAC) ||
      ...
      (p_first_filter->type == ETH_FILTER_TYPE_INNER_MAC_VNI_PAIR) ||
      (p_first_filter->type == ETH_FILTER_TYPE_MAC_VNI_PAIR))
    ecore_set_fw_mac_addr(&p_first_filter->mac_msb, &p_first_filter->mac_mid,
                          &p_first_filter->mac_lsb, (u8 *)p_filter_cmd->mac);
  ...
  if (p_filter_cmd->opcode == ECORE_FILTER_MOVE) {
    ...
    OSAL_MEMCPY(p_second_filter, p_first_filter, sizeof(*p_second_filter));
    ...
    p_first_filter->vport_id = (p_filter_cmd->opcode == ECORE_FILTER_REMOVE)
                                   ? vport_to_remove_from
                                   : vport_to_add_to;
  }
...
ecore_sp_eth_filter_ucast(struct ecore_hwfn *p_hwfn, u16 opaque_fid,
  ...
  rc = ecore_filter_ucast_common(p_hwfn, opaque_fid, p_filter_cmd, &p_ramrod,
                                 &p_ent, comp_mode, p_comp_data);
  if (rc != ECORE_SUCCESS) {
  ...
  DP_VERBOSE(
      p_hwfn, ECORE_MSG_SP,
      "Unicast filter configured, opcode = %s, type = %s, cmd_cnt = %d, "
      "is_rx_filter = %d, is_tx_filter = %d\n",
      (p_filter_cmd->opcode == ECORE_FILTER_ADD)
          ? "ADD"
          : ((p_filter_cmd->opcode == ECORE_FILTER_REMOVE)
                 ? "REMOVE"
                 : ((p_filter_cmd->opcode == ECORE_FILTER_MOVE) ? "MOVE"
                                                                : "REPLACE")),
      (p_filter_cmd->type == ECORE_FILTER_MAC)
          ? "MAC"
          : ((p_filter_cmd->type == ECORE_FILTER_VLAN) ? "VLAN" : "MAC & VLAN"),
 ...
 * Return:
 ******************************************************************************/
static u32 ecore_calc_crc32c(u8 *crc32_packet, u32 crc32_length,
                             u32 crc32_seed) {
...
static enum _ecore_status_t ecore_sp_eth_filter_mcast(
    struct ecore_hwfn *p_hwfn, struct ecore_filter_mcast *p_filter_cmd,
    enum spq_mode comp_mode, struct ecore_spq_comp_cb *p_comp_data) {
  struct vport_update_ramrod_data *p_ramrod = OSAL_NULL;
  ...
  if (p_filter_cmd->opcode == ECORE_FILTER_ADD)
    rc = ecore_fw_vport(p_hwfn, p_filter_cmd->vport_to_add_to, &abs_vport_id);
  else
    rc = ecore_fw_vport(p_hwfn, p_filter_cmd->vport_to_remove_from,
                        &abs_vport_id);
  if (rc != ECORE_SUCCESS)
...
enum _ecore_status_t ecore_filter_mcast_cmd(
    struct ecore_dev *p_dev, struct ecore_filter_mcast *p_filter_cmd,
    enum spq_mode comp_mode, struct ecore_spq_comp_cb *p_comp_data) {
  enum _ecore_status_t rc = ECORE_SUCCESS;
  int i;
  ...
  for_each_hwfn(p_dev, i) {
    ...
    rc =
        ecore_sp_eth_filter_mcast(p_hwfn, p_filter_cmd, comp_mode, p_comp_data);
    if (rc != ECORE_SUCCESS)
      break;
  }
...
enum _ecore_status_t ecore_filter_ucast_cmd(
    struct ecore_dev *p_dev, struct ecore_filter_ucast *p_filter_cmd,
    enum spq_mode comp_mode, struct ecore_spq_comp_cb *p_comp_data) {
  enum _ecore_status_t rc = ECORE_SUCCESS;
  int i;
  ...
  for_each_hwfn(p_dev, i) {
    ...
    rc = ecore_sp_eth_filter_ucast(p_hwfn, opaque_fid, p_filter_cmd, comp_mode,
                                   p_comp_data);
    if (rc != ECORE_SUCCESS)
      break;
  }
...
static void __ecore_get_vport_pstats_addrlen(struct ecore_hwfn *p_hwfn,
                                             ...
                                             u16 statistics_bin) {
  if (IS_PF(p_hwfn->p_dev)) {
    *p_addr = BAR0_MAP_REG_PSDM_RAM + PSTORM_QUEUE_STAT_OFFSET(statistics_bin);
...
static void __ecore_get_vport_pstats(struct ecore_hwfn *p_hwfn,
  ...
  p_stats->common.tx_ucast_bytes += HILO_64_REGPAIR(pstats.sent_ucast_bytes);
  p_stats->common.tx_mcast_bytes += HILO_64_REGPAIR(pstats.sent_mcast_bytes);
  p_stats->common.tx_bcast_bytes += HILO_64_REGPAIR(pstats.sent_bcast_bytes);
  p_stats->common.tx_ucast_pkts += HILO_64_REGPAIR(pstats.sent_ucast_pkts);
  p_stats->common.tx_mcast_pkts += HILO_64_REGPAIR(pstats.sent_mcast_pkts);
  p_stats->common.tx_bcast_pkts += HILO_64_REGPAIR(pstats.sent_bcast_pkts);
  p_stats->common.tx_err_drop_pkts += HILO_64_REGPAIR(pstats.error_drop_pkts);
}
...
static void __ecore_get_vport_tstats(struct ecore_hwfn *p_hwfn,
  ...
  if (IS_PF(p_hwfn->p_dev)) {
    tstats_addr =
        BAR0_MAP_REG_TSDM_RAM + TSTORM_PORT_STAT_OFFSET(MFW_PORT(p_hwfn));
  ...
  p_stats->common.gft_filter_drop += HILO_64_REGPAIR(tstats.eth_gft_drop_pkt);
}
...
static void __ecore_get_vport_ustats_addrlen(struct ecore_hwfn *p_hwfn,
                                             ...
                                             u16 statistics_bin) {
  if (IS_PF(p_hwfn->p_dev)) {
    *p_addr = BAR0_MAP_REG_USDM_RAM + USTORM_QUEUE_STAT_OFFSET(statistics_bin);
...
static void __ecore_get_vport_ustats(struct ecore_hwfn *p_hwfn,
  ...
  p_stats->common.rx_ucast_bytes += HILO_64_REGPAIR(ustats.rcv_ucast_bytes);
  p_stats->common.rx_mcast_bytes += HILO_64_REGPAIR(ustats.rcv_mcast_bytes);
  p_stats->common.rx_bcast_bytes += HILO_64_REGPAIR(ustats.rcv_bcast_bytes);
...
static void __ecore_get_vport_mstats_addrlen(struct ecore_hwfn *p_hwfn,
                                             ...
                                             u16 statistics_bin) {
  if (IS_PF(p_hwfn->p_dev)) {
    *p_addr = BAR0_MAP_REG_MSDM_RAM + MSTORM_QUEUE_STAT_OFFSET(statistics_bin);
...
static void __ecore_get_vport_mstats(struct ecore_hwfn *p_hwfn,
  ...
  p_stats->common.no_buff_discards += HILO_64_REGPAIR(mstats.no_buff_discard);
  ...
  p_stats->common.tpa_aborts_num += HILO_64_REGPAIR(mstats.tpa_aborts_num);
...
static void __ecore_get_vport_port_stats(struct ecore_hwfn *p_hwfn,
  ...
  if (ECORE_IS_BB(p_hwfn->p_dev)) {
    ...
    p_bb->rx_1519_to_1522_byte_packets += port_stats.eth.u0.bb0.r1522;
    p_bb->rx_1519_to_2047_byte_packets += port_stats.eth.u0.bb0.r2047;
    p_bb->rx_2048_to_4095_byte_packets += port_stats.eth.u0.bb0.r4095;
    p_bb->rx_4096_to_9216_byte_packets += port_stats.eth.u0.bb0.r9216;
    p_bb->rx_9217_to_16383_byte_packets += port_stats.eth.u0.bb0.r16383;
    p_bb->tx_1519_to_2047_byte_packets += port_stats.eth.u1.bb1.t2047;
    p_bb->tx_2048_to_4095_byte_packets += port_stats.eth.u1.bb1.t4095;
    p_bb->tx_4096_to_9216_byte_packets += port_stats.eth.u1.bb1.t9216;
    p_bb->tx_9217_to_16383_byte_packets += port_stats.eth.u1.bb1.t16383;
    ...
    p_ah->rx_1519_to_max_byte_packets += port_stats.eth.u0.ah0.r1519_to_max;
    p_ah->tx_1519_to_max_byte_packets = port_stats.eth.u1.ah1.t1519_to_max;
  }

  p_common->link_change_count =
...
void __ecore_get_vport_stats(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                             struct ecore_eth_stats *stats, u16 statistics_bin,
...
static void _ecore_get_vport_stats(struct ecore_dev *p_dev,
  ...
  for_each_hwfn(p_dev, i) {
    struct ecore_hwfn *p_hwfn = &p_dev->hwfns[i];
    struct ecore_ptt *p_ptt =
        IS_PF(p_dev) ? ecore_ptt_acquire(p_hwfn) : OSAL_NULL;
    bool b_get_port_stats;
    ...
    __ecore_get_vport_stats(p_hwfn, p_ptt, stats, fw_vport, b_get_port_stats);
...
void ecore_arfs_mode_configure(struct ecore_hwfn *p_hwfn,
  ...
  if (p_cfg_params->mode != ECORE_FILTER_CONFIG_MODE_DISABLE) {
    ecore_gft_config(p_hwfn, p_ptt, p_hwfn->rel_pf_id, p_cfg_params->tcp,
                     p_cfg_params->udp, p_cfg_params->ipv4, p_cfg_params->ipv6,
...
ecore_configure_rfs_ntuple_filter(struct ecore_hwfn *p_hwfn,
  ...
  p_ramrod->filter_action =
      p_params->b_is_add ? GFT_ADD_FILTER : GFT_DELETE_FILTER;
...
enum _ecore_status_t ecore_get_rxq_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_dmae_grc2host(
      p_hwfn, p_ptt, CAU_REG_SB_VAR_MEMORY + p_cid->sb_igu_id * sizeof(u64),
...
enum _ecore_status_t ecore_get_queue_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  if (IS_VF(p_hwfn->p_dev)) {
    ...
    if (rc != ECORE_SUCCESS)
      DP_NOTICE(p_hwfn, false, "Unable to read queue calescing\n");
...
enum _ecore_status_t ecore_eth_tx_queue_maxrate(struct ecore_hwfn *p_hwfn,
                                                struct ecore_ptt *p_ptt,
                                                struct ecore_queue_cid *p_cid,
                                                u32 rate) {
  u16 rl_id;
...
enum _ecore_status_t
ecore_update_eth_rss_ind_table_entry(struct ecore_hwfn *p_hwfn, u8 vport_id,
  ...
  *(u64 *)(&update_data) = DIRECT_REG_RD64(p_hwfn, addr);
  ...
  for (cnt = 0;
       update_data.valid && cnt < RSS_TSTORM_UPDATE_STATUS_MAX_POLL_COUNT;
       cnt++) {
  ...
  if (update_data.valid) {
    DP_NOTICE(p_hwfn, true,
              "rss update valid status is not clear! valid=0x%x vport id=%d "
              "ind_Table_idx=%d ind_table_value=%d.\n",
              update_data.valid, vport_id, ind_table_index, ind_table_value);
