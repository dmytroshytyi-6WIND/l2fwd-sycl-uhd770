...
#define UNUSABLE_ELEMS_PER_PAGE(elem_size, mode)                               \
  ((mode == ECORE_CHAIN_MODE_NEXT_PTR)                                         \
       ? (u8)(1 + ((sizeof(struct ecore_chain_next) - 1) / (elem_size)))       \
       : 0)

#define USABLE_ELEMS_PER_PAGE(elem_size, mode)                                 \
  ((u32)(ELEMS_PER_PAGE(elem_size) - UNUSABLE_ELEMS_PER_PAGE(elem_size, mode)))
...
ecore_chain_get_prod_idx_u32(struct ecore_chain *p_chain) {
...
static OSAL_INLINE u16 ecore_chain_get_elem_left(struct ecore_chain *p_chain) {
  ...
  used = (u16)(((u32)ECORE_U16_MAX + 1 + (u32)(p_chain->u.chain16.prod_idx)) -
...
ecore_chain_get_elem_left_u32(struct ecore_chain *p_chain) {
  ...
  used = (u32)(((u64)ECORE_U32_MAX + 1 + (u64)(p_chain->u.chain32.prod_idx)) -
...
static OSAL_INLINE u8 ecore_chain_is_full(struct ecore_chain *p_chain) {
  if (is_chain_u16(p_chain))
    return (ecore_chain_get_elem_left(p_chain) == p_chain->capacity);
  else
    return (ecore_chain_get_elem_left_u32(p_chain) == p_chain->capacity);
}
...
ecore_chain_get_elem_per_page(struct ecore_chain *p_chain) {
  return p_chain->elem_per_page;
}

static OSAL_INLINE u16
ecore_chain_get_usable_per_page(struct ecore_chain *p_chain) {
  return p_chain->usable_per_page;
}

static OSAL_INLINE u8
ecore_chain_get_unusable_per_page(struct ecore_chain *p_chain) {
...
static OSAL_INLINE dma_addr_t
ecore_chain_get_pbl_phys(struct ecore_chain *p_chain) {
 ...
 * @param page_to_inc
 */
static OSAL_INLINE void ecore_chain_advance_page(struct ecore_chain *p_chain,
                                                 void **p_next_elem,
                                                 void *idx_to_inc,
                                                 void *page_to_inc) {
...
ecore_chain_return_multi_produced(struct ecore_chain *p_chain, u32 num) {
  if (is_chain_u16(p_chain))
 ...
 */
static OSAL_INLINE void
ecore_chain_return_produced(struct ecore_chain *p_chain) {
  if (is_chain_u16(p_chain))
...
static OSAL_INLINE void *ecore_chain_produce(struct ecore_chain *p_chain) {
  ...
  if (is_chain_u16(p_chain)) {
    if ((p_chain->u.chain16.prod_idx & p_chain->elem_per_page_mask) ==
        p_chain->next_page_mask) {
      p_prod_idx = &p_chain->u.chain16.prod_idx;
      p_prod_page_idx = &p_chain->pbl.c.pbl_u16.prod_page_idx;
      ecore_chain_advance_page(p_chain, &p_chain->p_prod_elem, p_prod_idx,
                               p_prod_page_idx);
    }
    p_chain->u.chain16.prod_idx++;
  } else {
    if ((p_chain->u.chain32.prod_idx & p_chain->elem_per_page_mask) ==
        p_chain->next_page_mask) {
 ...
 */
static OSAL_INLINE void
ecore_chain_recycle_consumed(struct ecore_chain *p_chain) {
...
static OSAL_INLINE void *ecore_chain_consume(struct ecore_chain *p_chain) {
  ...
  if (is_chain_u16(p_chain)) {
    if ((p_chain->u.chain16.cons_idx & p_chain->elem_per_page_mask) ==
        p_chain->next_page_mask) {
      p_cons_idx = &p_chain->u.chain16.cons_idx;
      p_cons_page_idx = &p_chain->pbl.c.pbl_u16.cons_page_idx;
      ecore_chain_advance_page(p_chain, &p_chain->p_cons_elem, p_cons_idx,
                               p_cons_page_idx);
    }
    p_chain->u.chain16.cons_idx++;
  } else {
    if ((p_chain->u.chain32.cons_idx & p_chain->elem_per_page_mask) ==
        p_chain->next_page_mask) {
...
ecore_chain_init_params(struct ecore_chain *p_chain, u32 page_cnt, u8 elem_size,
  ...
  p_chain->next_page_mask =
      (p_chain->usable_per_page & p_chain->elem_per_page_mask);
 ...
 */
static OSAL_INLINE void *
ecore_chain_get_last_elem(struct ecore_chain *p_chain) {
  struct ecore_chain_next *p_next = OSAL_NULL;
...
static OSAL_INLINE void ecore_chain_set_prod(struct ecore_chain *p_chain,
  ...
  if (p_chain->mode == ECORE_CHAIN_MODE_PBL) {
     ...
     */
    page_diff = (((cur_prod - 1) & page_mask) - ((prod_idx - 1) & page_mask)) /
                p_chain->elem_per_page;

    page_cnt = ecore_chain_get_page_cnt(p_chain);
    if (is_chain_u16(p_chain))
      p_chain->pbl.c.pbl_u16.prod_page_idx =
          (p_chain->pbl.c.pbl_u16.prod_page_idx - page_diff + page_cnt) %
          page_cnt;
    else
      p_chain->pbl.c.pbl_u32.prod_page_idx =
          (p_chain->pbl.c.pbl_u32.prod_page_idx - page_diff + page_cnt) %
          page_cnt;
  }
...
static OSAL_INLINE void ecore_chain_set_cons(struct ecore_chain *p_chain,
  ...
  if (p_chain->mode == ECORE_CHAIN_MODE_PBL) {
     ...
     * positive and lower than the chain's capacity.
     */
    page_diff = (((cur_cons - 1) & page_mask) - ((cons_idx - 1) & page_mask)) /
                p_chain->elem_per_page;

    page_cnt = ecore_chain_get_page_cnt(p_chain);
    if (is_chain_u16(p_chain))
      p_chain->pbl.c.pbl_u16.cons_page_idx =
          (p_chain->pbl.c.pbl_u16.cons_page_idx - page_diff + page_cnt) %
          page_cnt;
    else
      p_chain->pbl.c.pbl_u32.cons_page_idx =
          (p_chain->pbl.c.pbl_u32.cons_page_idx - page_diff + page_cnt) %
          page_cnt;
  }
...
static OSAL_INLINE void ecore_chain_pbl_zero_mem(struct ecore_chain *p_chain) {
  ...
  for (i = 0; i < page_cnt; i++)
    OSAL_MEM_ZERO(p_chain->pbl.pp_virt_addr_tbl[i], ECORE_CHAIN_PAGE_SIZE);
}

int ecore_chain_print(
    struct ecore_chain *p_chain, char *buffer, u32 buffer_size,
    u32 *element_indx, u32 stop_indx, bool print_metadata,
    int (*func_ptr_print_element)(struct ecore_chain *p_chain, void *p_element,
                                  char *buffer),
    int (*func_ptr_print_metadata)(struct ecore_chain *p_chain, char *buffer));
