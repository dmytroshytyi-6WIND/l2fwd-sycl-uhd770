 ...
 */
enum core_roce_flavor_type { CORE_ROCE, CORE_RROCE, MAX_CORE_ROCE_FLAVOR_TYPE };
...
struct core_tx_start_ramrod_data {
  ...
  u8 vport_id_valid;
  /* vport id of the current connection, used to access
   * non_rdma_in_to_in_pri_map which is per vport
   */
  u8 vport_id;
};
...
struct eth_tstorm_rss_update_data {
  /* Valid flag. Driver must set this flag, FW clear valid flag when ready for
   * new RSS update command.
   */
  u8 valid;
...
struct outer_tag_config_struct {
  /* Enables updating S-tag priority from inner tag or DCB. Should be 1 for
   * Bette Davis, UFP with Host Control mode, and UFP with DCB over base
   * interface. else - 0.
   */
...
struct pf_update_tunnel_config {
  ...
  /* Update per PORT default tunnel RX classification scheme for traffic with
   * non unicast outer MAC in NPAR mode.
   */
...
struct tstorm_per_port_stat {
  ...
  /* packet passed to Light L2 and dropped because Light L2 is not configured
   * for this PF
   */
...
struct vf_pf_channel_data {
  /* 0: VF-PF Channel NOT ready. Waiting for ack from PF driver. 1: VF-PF
   * Channel is ready for a new transaction.
   */
  __le32 ready;
...
enum dmae_cmd_error_handling_enum {
  ...
  /* Send a completion with an error indication (i.e. set bit 31 of the
   * completion word)
   */
