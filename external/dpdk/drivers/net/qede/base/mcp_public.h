...
#define RTE_ETH_SPEED_SMARTLINQ                                                \
  0x8 /* deprecated - use link_modes field instead */
...
struct eth_stats {
  ...
  /* HSI - Cannot add more stats to this struct. If needed, then need to open
   * new struct
   */
};
...
#define DCBX_OOO_TC_OFFSET 8
  /* Entries in tc table are orginized that the left most is pri 0, right most
   * is prio 7
   */
...
#define DRV_ID_MCP_HSI_VER_CURRENT                                             \
  (LOAD_REQ_HSI_VERSION << DRV_ID_MCP_HSI_VER_OFFSET)
...
#define DRV_MB_PARAM_ESWITCH_MODE_MASK                                         \
  (DRV_MB_PARAM_ESWITCH_MODE_NONE | DRV_MB_PARAM_ESWITCH_MODE_VEB |            \
