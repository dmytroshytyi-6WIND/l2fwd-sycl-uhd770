...
static OSAL_INLINE u16 ecore_sb_update_sb_idx(struct ecore_sb_info *sb_info) {
  ...
  prod =
      OSAL_LE32_TO_CPU(*sb_info->sb_prod_index) & STATUS_BLOCK_PROD_INDEX_MASK;
...
#ifdef ECORE_CONFIG_DIRECT_HWFN
  DIRECT_REG_WR(sb_info->p_hwfn, sb_info->igu_addr, igu_ack.sb_id_and_flags);
#else
...
#endif
{
  ...
  for (i = 0; i < size / sizeof(*data); i++)
    DIRECT_REG_WR_RELAXED(p_hwfn, &((u32 OSAL_IOMEM *)addr)[i], data[i]);
}
...
#else
static OSAL_INLINE void internal_ram_wr(void OSAL_IOMEM *addr, int size,
                                        u32 *data) {
