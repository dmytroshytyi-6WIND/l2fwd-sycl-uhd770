...
#define DBG_ATTN_BLOCK_RESULT_NUM_REGS_SHIFT 2
  /* Offset of this registers block attention names in the attention name
   * offsets array
   */
  u16 names_offset;
 ...
 * attention types
 */
enum dbg_attn_type { ATTN_TYPE_INTERRUPT, ATTN_TYPE_PARITY, MAX_DBG_ATTN_TYPE };
...
#define DBG_BLOCK_CHIP_RESERVED0_SHIFT 5
  ...
  u16 reserved1;
  u8 reserved2;
  /* Number of Debug Bus lines in this block/chip (excluding signature and
   * latency events). Valid only if has_dbg_bus is set.
   */
  u8 num_of_dbg_bus_lines;
  /* Offset of this block/chip Debug Bus lines in the Debug Bus lines array.
   * Valid only if has_dbg_bus is set.
   */
  ...
  /* GRC address of the Debug Bus dbg_force_valid register (in dwords). Valid
   * only if has_dbg_bus is set.
   */
  u32 dbg_force_valid_reg_addr;
  /* GRC address of the Debug Bus dbg_force_frame register (in dwords). Valid
   * only if has_dbg_bus is set.
   */
...
struct dbg_bus_trigger_state_data {
  /* Message length (in cycles) to be used for message-based trigger
   * constraints. If set to 0, message length is based only on frame bit
   * received from HW.
   */
  u8 msg_len;
  /* A bit for each dword in the debug bus cycle, indicating if this dword
   * appears in a trigger constraint (1) or not (0)
   */
