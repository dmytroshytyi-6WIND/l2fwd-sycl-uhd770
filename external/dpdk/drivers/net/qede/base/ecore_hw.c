...
enum _ecore_status_t ecore_ptt_pool_alloc(struct ecore_hwfn *p_hwfn) {
  struct ecore_ptt_pool *p_pool =
      OSAL_ALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(*p_pool));
  int i;
  ...
  for (i = 0; i < PXP_EXTERNAL_BAR_PF_WINDOW_NUM; i++) {
    ...
    if (i >= RESERVED_PTT_MAX)
      OSAL_LIST_PUSH_HEAD(&p_pool->ptts[i].list_entry, &p_pool->free_list);
  }
...
void ecore_gtt_init(struct ecore_hwfn *p_hwfn) {
  ...
  for (i = 0; i < OSAL_ARRAY_SIZE(pxp_global_win); i++)
    if (pxp_global_win[i])
      REG_WR(p_hwfn, gtt_base + i * PXP_GLOBAL_ENTRY_SIZE, pxp_global_win[i]);
}
...
void ecore_ptt_set_win(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                       u32 new_hw_addr) {
  u32 prev_hw_addr;
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_HW, "Updating PTT entry %d to offset 0x%x\n",
  ...
  REG_WR(p_hwfn,
         ecore_ptt_config_addr(p_ptt) + OFFSETOF(struct pxp_ptt_entry, offset),
         OSAL_LE32_TO_CPU(p_ptt->pxp.offset));
}

static u32 ecore_set_ptt(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                         u32 hw_addr) {
  ...
  if (p_ptt->hwfn_id != p_hwfn->my_id)
    DP_NOTICE(p_hwfn, true, "ptt[%d] of hwfn[%02x] is used by hwfn[%02x]!\n",
...
struct ecore_ptt *ecore_get_reserved_ptt(struct ecore_hwfn *p_hwfn,
  ...
  if (ptt_idx >= RESERVED_PTT_MAX) {
    DP_NOTICE(p_hwfn, true, "Requested PTT %d is out of range\n", ptt_idx);
    return OSAL_NULL;
  }
...
void ecore_wr(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt, u32 hw_addr,
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_HW, "bar_addr 0x%x, hw_addr 0x%x, val 0x%x\n",
...
static void ecore_memcpy_hw(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                            void *addr, u32 hw_addr, osal_size_t n,
                            bool to_device) {
  ...
  while (done < n) {
    ...
    if (to_device)
      ...
      while (dw_count--)
        *host_addr++ = DIRECT_REG_RD(p_hwfn, reg_addr++);

    done += quota;
  }
}

void ecore_memcpy_from(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                       void *dest, u32 hw_addr, osal_size_t n) {
  DP_VERBOSE(p_hwfn, ECORE_MSG_HW,
             "hw_addr 0x%x, dest %p hw_addr 0x%x, size %lu\n", hw_addr, dest,
             hw_addr, (unsigned long)n);

  ecore_memcpy_hw(p_hwfn, p_ptt, dest, hw_addr, n, false);
}

void ecore_memcpy_to(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
...
void ecore_fid_pretend(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                       u16 fid) {
  u16 control = 0;
  ...
  REG_WR(p_hwfn,
         ecore_ptt_config_addr(p_ptt) + OFFSETOF(struct pxp_ptt_entry, pretend),
         *(u32 *)&p_ptt->pxp.pretend);
}

void ecore_port_pretend(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                        u8 port_id) {
  u16 control = 0;
...
#define ECORE_DMAE_FLAGS_IS_SET(params, flag)                                  \
  ((params) != OSAL_NULL && GET_FIELD((params)->flags, DMAE_PARAMS_##flag))
...
static void ecore_dmae_opcode(struct ecore_hwfn *p_hwfn,
  ...
  src_pf_id = ECORE_DMAE_FLAGS_IS_SET(p_params, SRC_PF_VALID)
                  ? p_params->src_pf_id
                  : p_hwfn->rel_pf_id;
  opcode |= (src_pf_id & DMAE_CMD_SRC_PF_ID_MASK) << DMAE_CMD_SRC_PF_ID_SHIFT;
  ...
  dst_pf_id = ECORE_DMAE_FLAGS_IS_SET(p_params, DST_PF_VALID)
                  ? p_params->dst_pf_id
                  : p_hwfn->rel_pf_id;
  opcode |= (dst_pf_id & DMAE_CMD_DST_PF_ID_MASK) << DMAE_CMD_DST_PF_ID_SHIFT;
  ...
  port_id = (ECORE_DMAE_FLAGS_IS_SET(p_params, PORT_VALID)) ? p_params->port_id
                                                            : p_hwfn->port_id;
  ...
  if (ECORE_DMAE_FLAGS_IS_SET(p_params, SRC_VF_VALID)) {
    ...
    opcode_b |= (DMAE_CMD_SRC_VF_ID_MASK << DMAE_CMD_SRC_VF_ID_SHIFT);
  }
...
static enum _ecore_status_t ecore_dmae_post_command(struct ecore_hwfn *p_hwfn,
  ...
  if ((((!p_command->dst_addr_lo) && (!p_command->dst_addr_hi)) ||
    ...
    DP_NOTICE(p_hwfn, true,
              ...
              idx_cmd, OSAL_LE32_TO_CPU(p_command->opcode),
  ...
  for (i = 0; i < DMAE_CMD_SIZE; i++) {
    u32 data = (i < DMAE_CMD_SIZE_TO_FILL) ? *(((u32 *)p_command) + i) : 0;

    ecore_wr(p_hwfn, p_ptt,
             DMAE_REG_CMD_MEM + (idx_cmd * DMAE_CMD_SIZE * sizeof(u32)) +
                 (i * sizeof(u32)),
             data);
  }

  ecore_wr(p_hwfn, p_ptt, ecore_dmae_idx_to_go_cmd(idx_cmd), DMAE_GO_VALUE);
...
enum _ecore_status_t ecore_dmae_info_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  *p_comp = OSAL_DMA_ALLOC_COHERENT(p_hwfn->p_dev, p_addr, sizeof(u32));
  if (*p_comp == OSAL_NULL) {
    DP_NOTICE(p_hwfn, false, "Failed to allocate `p_completion_word'\n");
    goto err;
  }

  p_addr = &p_hwfn->dmae_info.dmae_cmd_phys_addr;
  *p_cmd =
      OSAL_DMA_ALLOC_COHERENT(p_hwfn->p_dev, p_addr, sizeof(struct dmae_cmd));
  if (*p_cmd == OSAL_NULL) {
    DP_NOTICE(p_hwfn, false, "Failed to allocate `struct dmae_cmd'\n");
    goto err;
  }
  ...
  *p_buff = OSAL_DMA_ALLOC_COHERENT(p_hwfn->p_dev, p_addr,
  ...
  if (*p_buff == OSAL_NULL) {
    DP_NOTICE(p_hwfn, false, "Failed to allocate `intermediate_buffer'\n");
    goto err;
  }
...
void ecore_dmae_info_free(struct ecore_hwfn *p_hwfn) {
  ...
  if (p_hwfn->dmae_info.p_completion_word != OSAL_NULL) {
    ...
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_hwfn->dmae_info.p_completion_word,
                           p_phys, sizeof(u32));
  ...
  if (p_hwfn->dmae_info.p_dmae_cmd != OSAL_NULL) {
    ...
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_hwfn->dmae_info.p_dmae_cmd, p_phys,
  ...
  if (p_hwfn->dmae_info.p_intermediate_buffer != OSAL_NULL) {
    ...
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev,
                           p_hwfn->dmae_info.p_intermediate_buffer, p_phys,
                           sizeof(u32) * DMAE_MAX_RW_SIZE);
...
ecore_dmae_operation_wait(struct ecore_hwfn *p_hwfn) {
  ...
  u32 factor =
      (CHIP_REV_IS_EMUL(p_hwfn->p_dev)
           ? ECORE_EMUL_FACTOR
           : (CHIP_REV_IS_FPGA(p_hwfn->p_dev) ? ECORE_FPGA_FACTOR : 1));
  ...
  while (*p_hwfn->dmae_info.p_completion_word != DMAE_COMPLETION_VAL) {
    ...
    if (++wait_cnt > wait_cnt_limit) {
      DP_NOTICE(p_hwfn->p_dev, false,
                ...
                " expected 0x%08x.\n",
                *p_hwfn->dmae_info.p_completion_word, DMAE_COMPLETION_VAL);
...
static enum _ecore_status_t ecore_dmae_execute_sub_operation(
    struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt, u64 src_addr,
    u64 dst_addr, u8 src_type, u8 dst_type, u32 length_dw) {
  ...
  case ECORE_DMAE_ADDRESS_HOST_VIRT:
    ...
    OSAL_MEMCPY(&p_hwfn->dmae_info.p_intermediate_buffer[0],
                (void *)(osal_uintptr_t)src_addr, length_dw * sizeof(u32));
    break;
  default:
    return ECORE_INVAL;
  }
  ...
  if (src_type == ECORE_DMAE_ADDRESS_HOST_VIRT ||
    ...
    OSAL_DMA_SYNC(p_hwfn->p_dev,
                  (void *)HILO_U64(cmd->src_addr_hi, cmd->src_addr_lo),
  ...
  if (ecore_status != ECORE_SUCCESS) {
    DP_NOTICE(p_hwfn, false,
              "Wait Failed. source_addr 0x%lx, grc_addr 0x%lx, size_in_dwords "
...
static enum _ecore_status_t
ecore_dmae_execute_command(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                           u64 src_addr, u64 dst_addr, u8 src_type, u8 dst_type,
  ...
  if (!p_hwfn->dmae_info.b_mem_ready) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_HW,
               "No buffers allocated. Avoid DMAE transaction [{src: addr "
               "0x%lx, type %d}, {dst: addr 0x%lx, type %d}, size %d].\n",
               (unsigned long)src_addr, src_type, (unsigned long)dst_addr,
               dst_type, size_in_dwords);
    return ECORE_NOMEM;
  }

  if (p_hwfn->p_dev->recov_in_prog) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_HW,
               "Recovery is in progress. Avoid DMAE transaction [{src: addr "
               "0x%lx, type %d}, {dst: addr 0x%lx, type %d}, size %d].\n",
               (unsigned long)src_addr, src_type, (unsigned long)dst_addr,
               dst_type, size_in_dwords);
  ...
  if (!cmd) {
    DP_NOTICE(
        p_hwfn, true,
        "ecore_dmae_execute_sub_operation failed. Invalid state. source_addr "
        "0x%lx, destination addr 0x%lx, size_in_dwords 0x%x\n",
        (unsigned long)src_addr, (unsigned long)dst_addr, length_cur);
    return ECORE_INVAL;
  }

  ecore_dmae_opcode(p_hwfn, (src_type == ECORE_DMAE_ADDRESS_GRC),
  ...
  for (i = 0; i <= cnt_split; i++) {
    ...
    ecore_status = ecore_dmae_execute_sub_operation(
        p_hwfn, p_ptt, src_addr_split, dst_addr_split, src_type, dst_type,
        length_cur);
    if (ecore_status != ECORE_SUCCESS) {
      DP_NOTICE(p_hwfn, false,
                ...
                ecore_status, (unsigned long)src_addr, (unsigned long)dst_addr,
                length_cur);
...
enum _ecore_status_t ecore_dmae_host2grc(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_dmae_execute_command(
      p_hwfn, p_ptt, source_addr, grc_addr_in_dw, ECORE_DMAE_ADDRESS_HOST_VIRT,
      ECORE_DMAE_ADDRESS_GRC, size_in_dwords, p_params);
...
enum _ecore_status_t ecore_dmae_grc2host(struct ecore_hwfn *p_hwfn,
                                         struct ecore_ptt *p_ptt, u32 grc_addr,
  ...
  rc = ecore_dmae_execute_command(
      p_hwfn, p_ptt, grc_addr_in_dw, dest_addr, ECORE_DMAE_ADDRESS_GRC,
      ECORE_DMAE_ADDRESS_HOST_VIRT, size_in_dwords, p_params);
...
ecore_dmae_host2host(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  rc = ecore_dmae_execute_command(
      p_hwfn, p_ptt, source_addr, dest_addr, ECORE_DMAE_ADDRESS_HOST_PHYS,
      ECORE_DMAE_ADDRESS_HOST_PHYS, size_in_dwords, p_params);
...
enum _ecore_status_t ecore_dmae_sanity(struct ecore_hwfn *p_hwfn,
  ...
  if (!p_virt) {
    DP_NOTICE(p_hwfn, false, "DMAE sanity [%s]: failed to allocate memory\n",
              phase);
    return ECORE_NOMEM;
  }

  /* Fill the bottom half of the allocated memory with a known pattern */
  for (p_tmp = (u32 *)p_virt; p_tmp < (u32 *)((u8 *)p_virt + size); p_tmp++) {
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_SP,
             "DMAE sanity [%s]: src_addr={phys 0x%lx, virt %p}, dst_addr={phys "
             "0x%lx, virt %p}, size 0x%x\n",
             phase, (unsigned long)p_phys, p_virt,
             (unsigned long)(p_phys + size), (u8 *)p_virt + size, size);
  ...
  for (p_tmp = (u32 *)((u8 *)p_virt + size);
    ...
    if (*p_tmp != val) {
      DP_NOTICE(p_hwfn, false,
                "DMAE sanity [%s]: addr={phys 0x%lx, virt %p}, read_val "
                "0x%08x, expected_val 0x%08x\n",
                phase, (unsigned long)p_phys + ((u8 *)p_tmp - (u8 *)p_virt),
                p_tmp, *p_tmp, val);
...
void ecore_ppfid_wr(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  ecore_fid_pretend(p_hwfn, p_ptt, pfid << PXP_PRETEND_CONCRETE_FID_PFID_SHIFT);
  ecore_wr(p_hwfn, p_ptt, hw_addr, val);
  ecore_fid_pretend(p_hwfn, p_ptt,
                    p_hwfn->rel_pf_id << PXP_PRETEND_CONCRETE_FID_PFID_SHIFT);
}
