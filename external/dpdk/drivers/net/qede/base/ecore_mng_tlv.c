...
static enum _ecore_status_t ecore_mfw_get_tlv_group(u8 tlv_type,
                                                    u8 *tlv_group) {
  switch (tlv_type) {
...
static int ecore_mfw_get_gen_tlv_value(struct ecore_drv_tlv_hdr *p_tlv,
...
static int ecore_mfw_get_eth_tlv_value(struct ecore_drv_tlv_hdr *p_tlv,
...
static int ecore_mfw_get_fcoe_tlv_value(struct ecore_drv_tlv_hdr *p_tlv,
...
static int ecore_mfw_get_iscsi_tlv_value(struct ecore_drv_tlv_hdr *p_tlv,
...
static enum _ecore_status_t ecore_mfw_update_tlvs(struct ecore_hwfn *p_hwfn,
  ...
  while (offset < size) {
    ...
    if (tlv_group == ECORE_MFW_TLV_GENERIC)
      len = ecore_mfw_get_gen_tlv_value(&tlv, &p_tlv_data->generic, &p_tlv_ptr);
    else if (tlv_group == ECORE_MFW_TLV_ETH)
      len = ecore_mfw_get_eth_tlv_value(&tlv, &p_tlv_data->eth, &p_tlv_ptr);
    else if (tlv_group == ECORE_MFW_TLV_FCOE)
      len = ecore_mfw_get_fcoe_tlv_value(&tlv, &p_tlv_data->fcoe, &p_tlv_ptr);
    else
      len = ecore_mfw_get_iscsi_tlv_value(&tlv, &p_tlv_data->iscsi, &p_tlv_ptr);

    if (len > 0) {
      OSAL_WARN(len > 4 * tlv.tlv_length, "Incorrect MFW TLV length");
...
drv_done:
  rc =
      ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_GET_TLV_DONE, 0, &resp, &param);
