...
enum _ecore_status_t ecore_sp_init_request(struct ecore_hwfn *p_hwfn,
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_SPQ,
             ...
             "comp_mode [%s]\n",
             opaque_cid, cmd, protocol, (unsigned long)&p_ent->ramrod,
...
static void ecore_set_pf_update_tunn_mode(struct ecore_tunnel_info *p_tun,
  ...
  if (p_src->l2_geneve.b_update_mode || b_pf_start)
    p_tun->l2_geneve.b_mode_enabled = p_src->l2_geneve.b_mode_enabled;

  if (p_src->ip_geneve.b_update_mode || b_pf_start)
    p_tun->ip_geneve.b_mode_enabled = p_src->ip_geneve.b_mode_enabled;
}
...
static void ecore_set_ramrod_tunnel_param(
    u8 *p_tunn_cls, struct ecore_tunn_update_type *tun_type, u8 *p_update_port,
    __le16 *p_port, struct ecore_tunn_update_udp_port *p_udp_port) {
...
ecore_tunn_set_pf_update_params(struct ecore_hwfn *p_hwfn,
  ...
  ecore_set_ramrod_tunnel_param(&p_tunn_cfg->tunnel_clss_vxlan, &p_tun->vxlan,
                                &p_tunn_cfg->set_vxlan_udp_port_flg,
                                &p_tunn_cfg->vxlan_udp_port,
                                &p_tun->vxlan_port);

  ecore_set_ramrod_tunnel_param(
      &p_tunn_cfg->tunnel_clss_l2geneve, &p_tun->l2_geneve,
      &p_tunn_cfg->set_geneve_udp_port_flg, &p_tunn_cfg->geneve_udp_port,
      &p_tun->geneve_port);
...
static void ecore_set_hw_tunn_mode_port(struct ecore_hwfn *p_hwfn,
  ...
  if (ECORE_IS_BB_A0(p_hwfn->p_dev)) {
    DP_NOTICE(p_hwfn, true, "A0 chip: tunnel hw config is not supported\n");
    return;
  }

  if (p_tunn->vxlan_port.b_update_port)
    ecore_set_vxlan_dest_port(p_hwfn, p_ptt, p_tunn->vxlan_port.port);

  if (p_tunn->geneve_port.b_update_port)
    ecore_set_geneve_dest_port(p_hwfn, p_ptt, p_tunn->geneve_port.port);
...
enum _ecore_status_t ecore_sp_pf_start(struct ecore_hwfn *p_hwfn,
  ...
  ecore_eq_prod_update(p_hwfn, ecore_chain_get_prod_idx(&p_hwfn->p_eq->chain));
  ...
  rc = ecore_sp_init_request(p_hwfn, &p_ent, COMMON_RAMROD_PF_START,
  ...
  } else if (OSAL_GET_BIT(ECORE_MF_8021AD_TAGGING, &p_hwfn->p_dev->mf_bits)) {
  ...
  ecore_tunn_set_pf_start_params(p_hwfn, p_tunn, &p_ramrod->tunnel_config);

  if (OSAL_GET_BIT(ECORE_MF_INTER_PF_SWITCH, &p_hwfn->p_dev->mf_bits))
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_SPQ,
             "Setting event_ring_sb [id %04x index %02x], outer_tag.tpid [%d], "
             "outer_tag.tci [%d]\n",
...
enum _ecore_status_t ecore_sp_pf_update_dcbx(struct ecore_hwfn *p_hwfn) {
  ...
  rc = ecore_sp_init_request(p_hwfn, &p_ent, COMMON_RAMROD_PF_UPDATE,
                             PROTOCOLID_COMMON, &init_data);
  if (rc != ECORE_SUCCESS)
    return rc;
...
enum _ecore_status_t ecore_sp_rl_update(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_sp_init_request(p_hwfn, &p_ent, COMMON_RAMROD_RL_UPDATE,
                             PROTOCOLID_COMMON, &init_data);
  if (rc != ECORE_SUCCESS)
    return rc;
  ...
  rl_update->dcqcn_reset_alpha_on_idle = params->dcqcn_reset_alpha_on_idle;
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_SPQ,
             "rl_params: qcn_update_param_flg %x, dcqcn_update_param_flg %x, "
             "rl_init_flg %x, rl_start_flg %x, rl_stop_flg %x, rl_id_first %x, "
             "rl_id_last %x, rl_dc_qcn_flg %x,dcqcn_reset_alpha_on_idle %x, "
             "rl_bc_stage_th %x, rl_timer_stage_th %x, rl_bc_rate %x, "
             "rl_max_rate %x, rl_r_ai %x, rl_r_hai %x, dcqcn_g %x, dcqcn_k_us "
             "%x, dcqcn_timeuot_us %x, qcn_timeuot_us %x\n",
             ...
             rl_update->dcqcn_reset_alpha_on_idle, rl_update->rl_bc_stage_th,
             rl_update->rl_timer_stage_th, rl_update->rl_bc_rate,
             rl_update->rl_max_rate, rl_update->rl_r_ai, rl_update->rl_r_hai,
...
ecore_sp_pf_update_tunn_cfg(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                            struct ecore_tunnel_info *p_tunn,
...
enum _ecore_status_t ecore_sp_pf_stop(struct ecore_hwfn *p_hwfn) {
  ...
  rc = ecore_sp_init_request(p_hwfn, &p_ent, COMMON_RAMROD_PF_STOP,
                             PROTOCOLID_COMMON, &init_data);
  if (rc != ECORE_SUCCESS)
    return rc;
