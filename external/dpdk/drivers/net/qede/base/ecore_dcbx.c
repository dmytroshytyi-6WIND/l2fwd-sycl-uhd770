...
static bool ecore_dcbx_app_ethtype(u32 app_info_bitmap) {
  return !!(GET_MFW_FIELD(app_info_bitmap, DCBX_APP_SF) == DCBX_APP_SF_ETHTYPE);
}
...
static bool ecore_dcbx_app_port(u32 app_info_bitmap) {
  return !!(GET_MFW_FIELD(app_info_bitmap, DCBX_APP_SF) == DCBX_APP_SF_PORT);
}
...
static bool ecore_dcbx_iwarp_tlv(struct ecore_hwfn *p_hwfn, u32 app_info_bitmap,
  ...
  if (ieee)
    port = ecore_dcbx_ieee_app_port(app_info_bitmap, DCBX_APP_SF_IEEE_TCP_PORT);
  else
...
static void ecore_dcbx_dp_protocol(struct ecore_hwfn *p_hwfn,
...
static void ecore_dcbx_set_params(struct ecore_dcbx_results *p_data,
                                  struct ecore_hwfn *p_hwfn,
                                  struct ecore_ptt *p_ptt, bool enable, u8 prio,
                                  u8 tc, enum dcbx_protocol_type type,
...
static void ecore_dcbx_update_app_info(struct ecore_dcbx_results *p_data,
                                       struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < OSAL_ARRAY_SIZE(ecore_dcbx_app_update); i++) {
    ...
    ecore_dcbx_set_params(p_data, p_hwfn, p_ptt, enable, prio, tc, type,
                          personality);
  }
}

static enum _ecore_status_t ecore_dcbx_get_app_priority(u8 pri_bitmap,
                                                        u8 *priority) {
...
static bool ecore_dcbx_get_app_protocol_type(struct ecore_hwfn *p_hwfn,
...
ecore_dcbx_process_tlv(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  /* Parse APP TLV */
  for (i = 0; i < count; i++) {
    protocol_id = GET_MFW_FIELD(p_tbl[i].entry, DCBX_APP_PROTOCOL_ID);
    ...
    if (ecore_dcbx_get_app_protocol_type(p_hwfn, p_tbl[i].entry, protocol_id,
                                         &type, ieee)) {
      ...
      ecore_dcbx_update_app_info(p_data, p_hwfn, p_ptt, enable, priority, tc,
                                 type);
    }
  }

  /* If Eth TLV is not detected, use UFP TC as default TC */
  if (OSAL_GET_BIT(ECORE_MF_UFP_SPECIFIC, &p_hwfn->p_dev->mf_bits) && !eth_tlv)
  ...
  for (type = 0; type < DCBX_MAX_PROTOCOL_TYPE; type++) {
    ...
    ecore_dcbx_update_app_info(p_data, p_hwfn, p_ptt,
                               p_data->arr[DCBX_PROTOCOL_ETH].enable, priority,
                               tc, type);
  }
...
static enum _ecore_status_t
ecore_dcbx_copy_mib(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  do {
    if (type == ECORE_DCBX_REMOTE_LLDP_MIB) {
      ecore_memcpy_from(p_hwfn, p_ptt, p_data->lldp_remote, p_data->addr,
                        p_data->size);
      prefix_seq_num = p_data->lldp_remote->prefix_seq_num;
      suffix_seq_num = p_data->lldp_remote->suffix_seq_num;
    } else if (type == ECORE_DCBX_LLDP_TLVS) {
      ecore_memcpy_from(p_hwfn, p_ptt, p_data->lldp_tlvs, p_data->addr,
                        p_data->size);
      prefix_seq_num = p_data->lldp_tlvs->prefix_seq_num;
      suffix_seq_num = p_data->lldp_tlvs->suffix_seq_num;

    } else {
      ecore_memcpy_from(p_hwfn, p_ptt, p_data->mib, p_data->addr, p_data->size);
...
static void ecore_dcbx_get_priority_info(struct ecore_hwfn *p_hwfn,
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_DCB, "Priorities: eth %d\n", p_prio->eth);
}

static void ecore_dcbx_get_app_data(struct ecore_hwfn *p_hwfn,
                                    struct dcbx_app_priority_feature *p_app,
                                    struct dcbx_app_priority_entry *p_tbl,
  ...
  p_params->num_app_entries = GET_MFW_FIELD(p_app->flags, DCBX_APP_NUM_ENTRIES);
  for (i = 0; i < p_params->num_app_entries; i++) {
    ...
    if (ieee) {
      ...
      sf_ieee = GET_MFW_FIELD(p_tbl[i].entry, DCBX_APP_SF_IEEE);
      switch (sf_ieee) {
      case DCBX_APP_SF_IEEE_RESERVED:
        /* Old MFW */
        val = GET_MFW_FIELD(p_tbl[i].entry, DCBX_APP_SF);
        entry->sf_ieee =
            val ? ECORE_DCBX_SF_IEEE_TCP_UDP_PORT : ECORE_DCBX_SF_IEEE_ETHTYPE;
        break;
      ...
      entry->ethtype = !(GET_MFW_FIELD(p_tbl[i].entry, DCBX_APP_SF));
    }

    pri_map = GET_MFW_FIELD(p_tbl[i].entry, DCBX_APP_PRI_MAP);
    ecore_dcbx_get_app_priority(pri_map, &entry->prio);
    entry->proto_id = GET_MFW_FIELD(p_tbl[i].entry, DCBX_APP_PROTOCOL_ID);
    ecore_dcbx_get_app_protocol_type(p_hwfn, p_tbl[i].entry, entry->proto_id,
                                     &entry->proto_type, ieee);
  }

  DP_VERBOSE(p_hwfn, ECORE_MSG_DCB,
             "APP params: willing %d, valid %d error = %d\n",
             p_params->app_willing, p_params->app_valid, p_params->app_error);
}

static void ecore_dcbx_get_pfc_data(struct ecore_hwfn *p_hwfn, u32 pfc,
                                    struct ecore_dcbx_params *p_params) {
  u8 pfc_map;
...
static void ecore_dcbx_get_ets_data(struct ecore_hwfn *p_hwfn,
                                    struct dcbx_ets_feature *p_ets,
                                    struct ecore_dcbx_params *p_params) {
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_DCB,
             "ETS params: willing %d, enabled = %d ets_cbs %d pri_tc_tbl_0 %x "
             "max_ets_tc %d\n",
             p_params->ets_willing, p_params->ets_enabled, p_params->ets_cbs,
  ...
  for (i = 0; i < ECORE_MAX_PFC_PRIORITIES; i++) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_DCB, "elem %d  bw_tbl %x tsa_tbl %x\n", i,
               p_params->ets_tc_bw_tbl[i], p_params->ets_tc_tsa_tbl[i]);
  }
}

static void ecore_dcbx_get_common_params(
    struct ecore_hwfn *p_hwfn, struct dcbx_app_priority_feature *p_app,
    struct dcbx_app_priority_entry *p_tbl, struct dcbx_ets_feature *p_ets,
    u32 pfc, struct ecore_dcbx_params *p_params, bool ieee) {
...
static void ecore_dcbx_get_local_params(struct ecore_hwfn *p_hwfn,
                                        struct ecore_dcbx_get *params) {
  struct dcbx_features *p_feat;

  p_feat = &p_hwfn->p_dcbx_info->local_admin.features;
  ecore_dcbx_get_common_params(p_hwfn, &p_feat->app, p_feat->app.app_pri_tbl,
                               &p_feat->ets, p_feat->pfc, &params->local.params,
                               false);
  params->local.valid = true;
}

static void ecore_dcbx_get_remote_params(struct ecore_hwfn *p_hwfn,
                                         struct ecore_dcbx_get *params) {
  struct dcbx_features *p_feat;

  p_feat = &p_hwfn->p_dcbx_info->remote.features;
  ecore_dcbx_get_common_params(p_hwfn, &p_feat->app, p_feat->app.app_pri_tbl,
                               &p_feat->ets, p_feat->pfc,
                               &params->remote.params, false);
...
static void ecore_dcbx_get_dscp_params(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0, entry = 0; i < ECORE_DCBX_DSCP_SIZE / 8; i++) {
    ...
    for (j = 0; j < ECORE_DCBX_DSCP_SIZE / 8; j++, entry++)
      p_dscp->dscp_pri_map[entry] = (u32)(pri_map >> (j * 4)) & 0xf;
  }
}

static void ecore_dcbx_get_operational_params(struct ecore_hwfn *p_hwfn,
                                              struct ecore_dcbx_get *params) {
  ...
  val =
      !!(GET_MFW_FIELD(flags, DCBX_CONFIG_VERSION) == DCBX_CONFIG_VERSION_IEEE);
  p_operational->ieee = val;

  val =
      !!(GET_MFW_FIELD(flags, DCBX_CONFIG_VERSION) == DCBX_CONFIG_VERSION_CEE);
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_DCB,
             ...
             p_operational->ieee, p_operational->cee, p_operational->local);

  ecore_dcbx_get_common_params(
      p_hwfn, &p_feat->app, p_feat->app.app_pri_tbl, &p_feat->ets, p_feat->pfc,
      &params->operational.params, p_operational->ieee);
  ecore_dcbx_get_priority_info(p_hwfn, &p_operational->app_prio, p_results);
...
static void ecore_dcbx_get_local_lldp_params(struct ecore_hwfn *p_hwfn,
  ...
  OSAL_MEMCPY(params->lldp_local.local_chassis_id, p_local->local_chassis_id,
...
static void ecore_dcbx_get_remote_lldp_params(struct ecore_hwfn *p_hwfn,
  ...
  OSAL_MEMCPY(params->lldp_remote.peer_chassis_id, p_remote->peer_chassis_id,
...
ecore_dcbx_read_local_lldp_mib(struct ecore_hwfn *p_hwfn,
  ...
  data.addr = p_hwfn->mcp_info->port_addr +
              offsetof(struct public_port, lldp_config_params);
...
ecore_dcbx_read_remote_lldp_mib(struct ecore_hwfn *p_hwfn,
  ...
  OSAL_MEM_ZERO(&data, sizeof(data));
  data.addr = p_hwfn->mcp_info->port_addr +
              offsetof(struct public_port, lldp_status_params);
...
static enum _ecore_status_t
ecore_dcbx_read_remote_mib(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                           enum ecore_mib_read_type type) {
  struct ecore_dcbx_mib_meta_data data;
  enum _ecore_status_t rc = ECORE_SUCCESS;
...
static enum _ecore_status_t ecore_dcbx_read_local_mib(struct ecore_hwfn *p_hwfn,
                                                      struct ecore_ptt *p_ptt) {
  struct ecore_dcbx_mib_meta_data data;
  enum _ecore_status_t rc = ECORE_SUCCESS;
  ...
  ecore_memcpy_from(p_hwfn, p_ptt, data.local_admin, data.addr, data.size);

  return rc;
}

static void ecore_dcbx_read_dscp_mib(struct ecore_hwfn *p_hwfn,
                                     struct ecore_ptt *p_ptt) {
  struct ecore_dcbx_mib_meta_data data;

  data.addr =
      p_hwfn->mcp_info->port_addr + offsetof(struct public_port, dcb_dscp_map);
...
enum _ecore_status_t ecore_dcbx_info_alloc(struct ecore_hwfn *p_hwfn) {
  p_hwfn->p_dcbx_info =
      OSAL_ZALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(*p_hwfn->p_dcbx_info));
  if (!p_hwfn->p_dcbx_info) {
    DP_NOTICE(p_hwfn, false, "Failed to allocate `struct ecore_dcbx_info'");
    return ECORE_NOMEM;
  }
...
static void ecore_dcbx_set_pfc_data(struct ecore_hwfn *p_hwfn, u32 *pfc,
                                    struct ecore_dcbx_params *p_params) {
  u8 pfc_map = 0;
  int i;
...
static void ecore_dcbx_set_ets_data(struct ecore_hwfn *p_hwfn,
                                    struct dcbx_ets_feature *p_ets,
                                    struct ecore_dcbx_params *p_params) {
  u8 *bw_map, *tsa_map;
  u32 val;
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_DCB,
             "flags = 0x%x pri_tc = 0x%x tc_bwl[] = {0x%x, 0x%x} tc_tsa = "
             "{0x%x, 0x%x}\n",
             p_ets->flags, p_ets->pri_tc_tbl[0], p_ets->tc_bw_tbl[0],
             p_ets->tc_bw_tbl[1], p_ets->tc_tsa_tbl[0], p_ets->tc_tsa_tbl[1]);
}

static void ecore_dcbx_set_app_data(struct ecore_hwfn *p_hwfn,
                                    struct dcbx_app_priority_feature *p_app,
                                    struct ecore_dcbx_params *p_params,
                                    bool ieee) {
  u32 *entry;
  ...
  p_app->flags |= (u32)p_params->num_app_entries << DCBX_APP_NUM_ENTRIES_OFFSET;
  ...
  for (i = 0; i < p_params->num_app_entries; i++) {
    ...
    if (ieee) {
      *entry &= ~(DCBX_APP_SF_IEEE_MASK | DCBX_APP_SF_MASK);
      ...
      case ECORE_DCBX_SF_IEEE_ETHTYPE:
        *entry |= ((u32)DCBX_APP_SF_IEEE_ETHTYPE << DCBX_APP_SF_IEEE_OFFSET);
        *entry |= ((u32)DCBX_APP_SF_ETHTYPE << DCBX_APP_SF_OFFSET);
        break;
      case ECORE_DCBX_SF_IEEE_TCP_PORT:
        *entry |= ((u32)DCBX_APP_SF_IEEE_TCP_PORT << DCBX_APP_SF_IEEE_OFFSET);
        *entry |= ((u32)DCBX_APP_SF_PORT << DCBX_APP_SF_OFFSET);
        break;
      case ECORE_DCBX_SF_IEEE_UDP_PORT:
        *entry |= ((u32)DCBX_APP_SF_IEEE_UDP_PORT << DCBX_APP_SF_IEEE_OFFSET);
        *entry |= ((u32)DCBX_APP_SF_PORT << DCBX_APP_SF_OFFSET);
        break;
      case ECORE_DCBX_SF_IEEE_TCP_UDP_PORT:
        *entry |= (u32)DCBX_APP_SF_IEEE_TCP_UDP_PORT << DCBX_APP_SF_IEEE_OFFSET;
        *entry |= ((u32)DCBX_APP_SF_PORT << DCBX_APP_SF_OFFSET);
        break;
      }
    } else {
    ...
    *entry |=
        ((u32)p_params->app_entry[i].proto_id << DCBX_APP_PROTOCOL_ID_OFFSET);
    *entry &= ~DCBX_APP_PRI_MAP_MASK;
    *entry |= ((u32)(p_params->app_entry[i].prio) << DCBX_APP_PRI_MAP_OFFSET);
  }

  DP_VERBOSE(p_hwfn, ECORE_MSG_DCB, "flags = 0x%x\n", p_app->flags);
}

static void ecore_dcbx_set_local_params(struct ecore_hwfn *p_hwfn,
...
ecore_dcbx_set_dscp_params(struct ecore_hwfn *p_hwfn,
  ...
  OSAL_MEMCPY(p_dscp_map, &p_hwfn->p_dcbx_info->dscp_map, sizeof(*p_dscp_map));
  ...
  for (i = 0, entry = 0; i < 8; i++) {
    val = 0;
    for (j = 0; j < 8; j++, entry++)
      val |= (((u32)p_params->dscp.dscp_pri_map[entry]) << (j * 4));
...
enum _ecore_status_t ecore_dcbx_config_params(struct ecore_hwfn *p_hwfn,
  ...
  if (params->override_flags & ECORE_DCBX_OVERRIDE_DSCP_CFG) {
    ...
    data.size = sizeof(struct dcb_dscp_map);
    ecore_memcpy_to(p_hwfn, p_ptt, data.addr, data.dscp_map, data.size);
  }

  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_SET_DCBX,
                     1 << DRV_MB_PARAM_LLDP_SEND_OFFSET, &resp, &param);
  if (rc != ECORE_SUCCESS)
    DP_NOTICE(p_hwfn, false, "Failed to send DCBX update request\n");

  return rc;
}

enum _ecore_status_t
ecore_dcbx_get_config_params(struct ecore_hwfn *p_hwfn,
                             struct ecore_dcbx_set *params) {
  ...
  dcbx_info = OSAL_ALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(*dcbx_info));
  if (!dcbx_info)
    return ECORE_NOMEM;

  OSAL_MEMSET(dcbx_info, 0, sizeof(*dcbx_info));
  rc = ecore_dcbx_query_params(p_hwfn, dcbx_info, ECORE_DCBX_OPERATIONAL_MIB);
  if (rc) {
  ...
  OSAL_MEMCPY(&p_hwfn->p_dcbx_info->set.dscp, &p_hwfn->p_dcbx_info->get.dscp,
  ...
  OSAL_MEMCPY(params, &p_hwfn->p_dcbx_info->set, sizeof(struct ecore_dcbx_set));
...
enum _ecore_status_t ecore_lldp_mib_update_event(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < LLDP_MAX_LLDP_AGENTS; i++) {
    ...
    rc = ecore_dcbx_copy_mib(p_hwfn, p_ptt, &data, ECORE_DCBX_LLDP_TLVS);
    if (rc != ECORE_SUCCESS) {
    ...
    for (i = 0; i < MAX_TLV_BUFFER; i++)
      tlvs.tlvs_buffer[i] = OSAL_CPU_TO_BE32(tlvs.tlvs_buffer[i]);
...
ecore_lldp_get_params(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  ecore_memcpy_from(p_hwfn, p_ptt, &lldp_params, addr, sizeof(lldp_params));

  p_params->tx_interval =
      GET_MFW_FIELD(lldp_params.config, LLDP_CONFIG_TX_INTERVAL);
  p_params->tx_hold = GET_MFW_FIELD(lldp_params.config, LLDP_CONFIG_HOLD);
  p_params->tx_credit =
      GET_MFW_FIELD(lldp_params.config, LLDP_CONFIG_MAX_CREDIT);
  p_params->rx_enable =
      GET_MFW_FIELD(lldp_params.config, LLDP_CONFIG_ENABLE_RX);
  p_params->tx_enable =
      GET_MFW_FIELD(lldp_params.config, LLDP_CONFIG_ENABLE_TX);
  ...
  for (i = 0; i < ECORE_LLDP_CHASSIS_ID_STAT_LEN; i++)
    p_params->chassis_id_tlv[i] = OSAL_BE32_TO_CPU(p_params->chassis_id_tlv[i]);
  ...
  for (i = 0; i < ECORE_LLDP_PORT_ID_STAT_LEN; i++)
    p_params->port_id_tlv[i] = OSAL_BE32_TO_CPU(p_params->port_id_tlv[i]);
...
ecore_lldp_set_params(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  for (i = 0; i < ECORE_LLDP_CHASSIS_ID_STAT_LEN; i++)
    p_params->chassis_id_tlv[i] = OSAL_CPU_TO_BE32(p_params->chassis_id_tlv[i]);
  OSAL_MEMCPY(lldp_params.local_chassis_id, p_params->chassis_id_tlv,
              sizeof(lldp_params.local_chassis_id));

  for (i = 0; i < ECORE_LLDP_PORT_ID_STAT_LEN; i++)
    p_params->port_id_tlv[i] = OSAL_CPU_TO_BE32(p_params->port_id_tlv[i]);
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_SET_LLDP, mb_param, &mcp_resp,
                     &mcp_param);
  if (rc != ECORE_SUCCESS)
...
ecore_lldp_set_system_tlvs(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  SET_MFW_FIELD(lld_tlv_buf.flags, LLDP_SYSTEM_TLV_LENGTH, p_params->buf_size);
...
enum _ecore_status_t ecore_dcbx_get_dscp_priority(struct ecore_hwfn *p_hwfn,
                                                  u8 dscp_index,
                                                  u8 *p_dscp_pri) {
  ...
  p_dcbx_info = OSAL_ALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(*p_dcbx_info));
  if (!p_dcbx_info)
    return ECORE_NOMEM;

  OSAL_MEMSET(p_dcbx_info, 0, sizeof(*p_dcbx_info));
  rc = ecore_dcbx_query_params(p_hwfn, p_dcbx_info, ECORE_DCBX_OPERATIONAL_MIB);
  if (rc) {
...
enum _ecore_status_t ecore_dcbx_set_dscp_priority(struct ecore_hwfn *p_hwfn,
                                                  struct ecore_ptt *p_ptt,
