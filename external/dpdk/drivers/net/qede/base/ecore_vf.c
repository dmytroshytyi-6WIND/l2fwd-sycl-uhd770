 ...
 */
#endif
static enum _ecore_status_t ecore_send_msg2pf(struct ecore_hwfn *p_hwfn,
                                              u8 *done,
                                              __rte_unused u32 resp_size) {
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
             ...
             &zone_data->non_trigger.vf_pf_msg_addr, *((u32 *)&trigger),
             &zone_data->trigger);

  REG_WR(p_hwfn, (osal_uintptr_t)&zone_data->non_trigger.vf_pf_msg_addr.lo,
         U64_LO(p_hwfn->vf_iov_info->vf2pf_request_phys));

  REG_WR(p_hwfn, (osal_uintptr_t)&zone_data->non_trigger.vf_pf_msg_addr.hi,
         U64_HI(p_hwfn->vf_iov_info->vf2pf_request_phys));
  ...
  if (!*done) {
    ...
    if ((*done != PFVF_STATUS_SUCCESS) && (*done != PFVF_STATUS_NO_RESOURCE))
      DP_NOTICE(p_hwfn, false, "PF response: %d [Type %d]\n", *done,
                p_req->first_tlv.tl.type);
    else
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "PF response: %d [Type %d]\n", *done,
                 p_req->first_tlv.tl.type);
  }
...
static void ecore_vf_pf_add_qid(struct ecore_hwfn *p_hwfn,
  ...
  p_qid_tlv =
      ecore_add_tlv(&p_iov->offset, CHANNEL_TLV_QID, sizeof(*p_qid_tlv));
...
enum _ecore_status_t _ecore_vf_pf_release(struct ecore_hwfn *p_hwfn,
  ...
  if (p_iov->vf2pf_request)
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_iov->vf2pf_request,
                           p_iov->vf2pf_request_phys, sizeof(union vfpf_tlvs));
  if (p_iov->pf2vf_reply)
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_iov->pf2vf_reply,
                           p_iov->pf2vf_reply_phys, sizeof(union pfvf_tlvs));
...
static void ecore_vf_pf_acquire_reduce_resc(struct ecore_hwfn *p_hwfn,
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
             "PF unwilling to fullill resource request: rxq [%02x/%02x] txq "
             "[%02x/%02x] sbs [%02x/%02x] mac [%02x/%02x] vlan [%02x/%02x] mc "
             "[%02x/%02x] cids [%02x/%02x]. Try PF recommended amount\n",
             p_req->num_rxqs, p_resp->num_rxqs, p_req->num_rxqs,
             p_resp->num_txqs, p_req->num_sbs, p_resp->num_sbs,
...
enum _ecore_status_t ecore_vf_pf_acquire(struct ecore_hwfn *p_hwfn) {
  ...
  if (p_iov->b_doorbell_bar)
    req->vfdev_info.capabilities |=
        VFPF_ACQUIRE_CAP_PHYSICAL_BAR | VFPF_ACQUIRE_CAP_QUEUE_QIDS;
  ...
  while (!resources_acquired) {
    ...
    OSAL_MEMCPY(&p_iov->acquire_resp, resp, sizeof(p_iov->acquire_resp));
    ...
    if (resp->hdr.status == PFVF_STATUS_SUCCESS) {
      ...
      if (!(resp->pfdev_info.capabilities &
         ...
         * legacy and continue.
         */
        req->vfdev_info.capabilities |= VFPF_ACQUIRE_CAP_PRE_FP_HSI;
      }
      ...
      ecore_vf_pf_acquire_reduce_resc(p_hwfn, p_resc, &resp->resc);
      ...
      if (pfdev_info->major_fp_hsi &&
        ...
        DP_NOTICE(p_hwfn, false,
                  ...
                  " %02x.xx.\n",
                  pfdev_info->major_fp_hsi, pfdev_info->minor_fp_hsi,
      ...
      if (!pfdev_info->major_fp_hsi) {
        if (req->vfdev_info.capabilities & VFPF_ACQUIRE_CAP_PRE_FP_HSI) {
          ...
          DP_INFO(p_hwfn, "PF is old - try re-acquire to"
       ...
       */
      DP_NOTICE(p_hwfn, false, "PF rejected acquisition by VF\n");
      rc = ECORE_INVAL;
      goto exit;
      ...
      DP_ERR(p_hwfn, "PF returned err %d to VF acquisition request\n",
             resp->hdr.status);
      rc = ECORE_AGAIN;
      goto exit;
    }
  ...
  /* @DPDK */
  if (((p_iov->b_pre_fp_hsi == true) & ETH_HSI_VER_MINOR) &&
...
ecore_vf_hw_prepare(struct ecore_hwfn *p_hwfn,
  ...
  if (p_hwfn->doorbells == OSAL_NULL) {
    p_hwfn->doorbells = (u8 OSAL_IOMEM *)p_hwfn->regview + PXP_VF_BAR0_START_DQ;
    ...
    if (p_lead->vf_iov_info->b_doorbell_bar)
      ...
      p_hwfn->doorbells =
          (u8 OSAL_IOMEM *)p_hwfn->regview + PXP_VF_BAR0_START_DQ;
  }

  /* Allocate vf2pf msg */
  p_iov->vf2pf_request = OSAL_DMA_ALLOC_COHERENT(
      p_hwfn->p_dev, &p_iov->vf2pf_request_phys, sizeof(union vfpf_tlvs));
  if (!p_iov->vf2pf_request) {
    DP_NOTICE(p_hwfn, true, "Failed to allocate `vf2pf_request' DMA memory\n");
    goto free_p_iov;
  }

  p_iov->pf2vf_reply = OSAL_DMA_ALLOC_COHERENT(
      p_hwfn->p_dev, &p_iov->pf2vf_reply_phys, sizeof(union pfvf_tlvs));
  if (!p_iov->pf2vf_reply) {
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
             ...
             p_iov->vf2pf_request, (unsigned long)p_iov->vf2pf_request_phys,
             p_iov->pf2vf_reply, (unsigned long)p_iov->pf2vf_reply_phys);

  /* Allocate Bulletin board */
  p_iov->bulletin.size = sizeof(struct ecore_bulletin_content);
  p_iov->bulletin.p_virt = OSAL_DMA_ALLOC_COHERENT(
      p_hwfn->p_dev, &p_iov->bulletin.phys, p_iov->bulletin.size);
  if (!p_iov->bulletin.p_virt) {
   ...
   * doorbell.
   */
  if (rc == ECORE_SUCCESS && p_iov->b_doorbell_bar &&
      !ecore_vf_hw_bar_size(p_hwfn, BAR_ID_1) && ECORE_IS_CMT(p_hwfn->p_dev)) {
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
             ...
             p_hwfn->regview, p_hwfn->doorbells, p_hwfn->p_dev->doorbells);
...
free_bulletin_mem:
  OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_iov->bulletin.p_virt,
                         p_iov->bulletin.phys, p_iov->bulletin.size);
#endif
free_pf2vf_reply:
  OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_iov->pf2vf_reply,
                         p_iov->pf2vf_reply_phys, sizeof(union pfvf_tlvs));
...
static void __ecore_vf_update_tunn_param(struct ecore_tunn_update_type *p_tun,
...
ecore_vf_update_tunn_param(struct ecore_hwfn *p_hwfn,
  ...
  __ecore_vf_update_tunn_param(&p_tun->vxlan, feat_mask, p_resp->vxlan_mode,
                               p_resp->vxlan_clss, ECORE_MODE_VXLAN_TUNN);
  __ecore_vf_update_tunn_param(&p_tun->l2_geneve, feat_mask,
                               p_resp->l2geneve_mode, p_resp->l2geneve_clss,
                               ECORE_MODE_L2GENEVE_TUNN);
  __ecore_vf_update_tunn_param(&p_tun->ip_geneve, feat_mask,
                               p_resp->ipgeneve_mode, p_resp->ipgeneve_clss,
                               ECORE_MODE_IPGENEVE_TUNN);
  __ecore_vf_update_tunn_param(&p_tun->l2_gre, feat_mask, p_resp->l2gre_mode,
                               p_resp->l2gre_clss, ECORE_MODE_L2GRE_TUNN);
  __ecore_vf_update_tunn_param(&p_tun->ip_gre, feat_mask, p_resp->ipgre_mode,
                               p_resp->ipgre_clss, ECORE_MODE_IPGRE_TUNN);
  p_tun->geneve_port.port = p_resp->geneve_udp_port;
  p_tun->vxlan_port.port = p_resp->vxlan_udp_port;

  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV,
             "tunn mode: vxlan=0x%x, l2geneve=0x%x, ipgeneve=0x%x, l2gre=0x%x, "
             "ipgre=0x%x",
             p_tun->vxlan.b_mode_enabled, p_tun->l2_geneve.b_mode_enabled,
             p_tun->ip_geneve.b_mode_enabled, p_tun->l2_gre.b_mode_enabled,
...
ecore_vf_pf_tunnel_param_update(struct ecore_hwfn *p_hwfn,
  ...
  p_req =
      ecore_vf_pf_prep(p_hwfn, CHANNEL_TLV_UPDATE_TUNN_PARAM, sizeof(*p_req));
  ...
  ecore_vf_prep_tunn_req_tlv(p_req, &p_src->l2_geneve, ECORE_MODE_L2GENEVE_TUNN,
  ...
  __ecore_vf_prep_tunn_req_tlv(p_req, &p_src->ip_geneve,
                               ECORE_MODE_IPGENEVE_TUNN, &p_req->ipgeneve_clss);
  __ecore_vf_prep_tunn_req_tlv(p_req, &p_src->l2_gre, ECORE_MODE_L2GRE_TUNN,
                               &p_req->l2gre_clss);
  __ecore_vf_prep_tunn_req_tlv(p_req, &p_src->ip_gre, ECORE_MODE_IPGRE_TUNN,
                               &p_req->ipgre_clss);
  ...
  if (p_resp->hdr.status != PFVF_STATUS_SUCCESS) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "Failed to update tunnel parameters\n");
    rc = ECORE_INVAL;
  }
...
enum _ecore_status_t
ecore_vf_pf_rxq_start(struct ecore_hwfn *p_hwfn, struct ecore_queue_cid *p_cid,
  ...
  if (p_iov->b_pre_fp_hsi) {
    ...
    *pp_prod = (u8 OSAL_IOMEM *)p_hwfn->regview +
               MSTORM_QZONE_START(p_hwfn->p_dev) + (hw_qid)*MSTORM_QZONE_SIZE;
...
enum _ecore_status_t ecore_vf_pf_txq_start(struct ecore_hwfn *p_hwfn,
  ...
  if (!p_iov->b_pre_fp_hsi) {
    ...
    *pp_doorbell =
        (u8 OSAL_IOMEM *)p_hwfn->doorbells + DB_ADDR_VF(cid, DQ_DEMS_LEGACY);
  }
...
enum _ecore_status_t
ecore_vf_pf_vport_start(struct ecore_hwfn *p_hwfn, u8 vport_id, u16 mtu,
...
static void ecore_vf_handle_vp_update_tlvs_resp(
  ...
  for (tlv = CHANNEL_TLV_VPORT_UPDATE_ACTIVATE;
    ...
    p_resp = (struct pfvf_def_resp_tlv *)ecore_iov_search_list_tlvs(
        p_hwfn, p_iov->pf2vf_reply, tlv);
    if (p_resp && p_resp->hdr.status)
      DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "TLV[%d] type %s Configuration %s\n",
                 tlv, qede_ecore_channel_tlvs_string[tlv],
                 (p_resp && p_resp->hdr.status) ? "succeeded" : "failed");
  }
}
...
ecore_vf_pf_vport_update(struct ecore_hwfn *p_hwfn,
  ...
  if (update_rx || update_tx) {
    ...
    p_act_tlv =
        ecore_add_tlv(&p_iov->offset, CHANNEL_TLV_VPORT_UPDATE_ACTIVATE, size);
  ...
  if (p_params->update_inner_vlan_removal_flg) {
    ...
    p_vlan_tlv = ecore_add_tlv(&p_iov->offset,
                               CHANNEL_TLV_VPORT_UPDATE_VLAN_STRIP, size);
    resp_size += sizeof(struct pfvf_def_resp_tlv);
  ...
  if (p_params->update_tx_switching_flg) {
    ...
    p_tx_switch_tlv = ecore_add_tlv(&p_iov->offset, tlv, size);
    resp_size += sizeof(struct pfvf_def_resp_tlv);
  ...
  if (p_params->update_approx_mcast_flg) {
    ...
    p_mcast_tlv =
        ecore_add_tlv(&p_iov->offset, CHANNEL_TLV_VPORT_UPDATE_MCAST, size);
    resp_size += sizeof(struct pfvf_def_resp_tlv);
  ...
  if (update_rx || update_tx) {
    ...
    if (update_rx) {
      ...
      p_accept_tlv->rx_accept_filter = p_params->accept_flags.rx_accept_filter;
    }

    if (update_tx) {
      p_accept_tlv->update_tx_mode = update_tx;
      p_accept_tlv->tx_accept_filter = p_params->accept_flags.tx_accept_filter;
    }
  }
  ...
  if (p_params->rss_params) {
    ...
    p_rss_tlv =
        ecore_add_tlv(&p_iov->offset, CHANNEL_TLV_VPORT_UPDATE_RSS, size);
    resp_size += sizeof(struct pfvf_def_resp_tlv);
  ...
  if (p_params->sge_tpa_params) {
    ...
    p_sge_tpa_tlv =
        ecore_add_tlv(&p_iov->offset, CHANNEL_TLV_VPORT_UPDATE_SGE_TPA, size);
    resp_size += sizeof(struct pfvf_def_resp_tlv);
    ...
    p_sge_tpa_tlv->tpa_max_aggs_num = sge_tpa_params->tpa_max_aggs_num;
    ...
    p_sge_tpa_tlv->tpa_min_size_to_cont = sge_tpa_params->tpa_min_size_to_cont;

    p_sge_tpa_tlv->max_buffers_per_cqe = sge_tpa_params->max_buffers_per_cqe;
  }
...
enum _ecore_status_t
ecore_vf_pf_filter_ucast(struct ecore_hwfn *p_hwfn,
...
enum _ecore_status_t ecore_vf_pf_get_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  /* clear mailbox and prep header tlv */
  req = ecore_vf_pf_prep(p_hwfn, CHANNEL_TLV_COALESCE_READ, sizeof(*req));
...
enum _ecore_status_t ecore_vf_pf_set_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  /* clear mailbox and prep header tlv */
  req = ecore_vf_pf_prep(p_hwfn, CHANNEL_TLV_COALESCE_UPDATE, sizeof(*req));
...
enum _ecore_status_t ecore_vf_pf_update_mtu(struct ecore_hwfn *p_hwfn,
                                            u16 mtu) {
  struct ecore_vf_iov *p_iov = p_hwfn->vf_iov_info;
  ...
  /* clear mailbox and prep header tlv */
  p_req = ecore_vf_pf_prep(p_hwfn, CHANNEL_TLV_UPDATE_MTU, sizeof(*p_req));
  p_req->mtu = mtu;
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "Requesting MTU update to %d\n", mtu);
...
void ecore_vf_set_sb_info(struct ecore_hwfn *p_hwfn, u16 sb_id,
...
enum _ecore_status_t ecore_vf_read_bulletin(struct ecore_hwfn *p_hwfn,
  ...
  *p_change = 0;
  ...
  crc =
      OSAL_CRC32(0, (u8 *)&shadow + crc_size, p_iov->bulletin.size - crc_size);
  if (crc != shadow.crc)
    return ECORE_AGAIN;
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IOV, "Read a bulletin update %08x\n",
             shadow.version);
...
void ecore_vf_get_link_params(struct ecore_hwfn *p_hwfn,
  ...
  __ecore_vf_get_link_params(params, &p_hwfn->vf_iov_info->bulletin_shadow);
}
...
void ecore_vf_get_link_state(struct ecore_hwfn *p_hwfn,
  ...
  __ecore_vf_get_link_state(link, &p_hwfn->vf_iov_info->bulletin_shadow);
}
...
void ecore_vf_get_link_caps(struct ecore_hwfn *p_hwfn,
  ...
  __ecore_vf_get_link_caps(p_link_caps, &p_hwfn->vf_iov_info->bulletin_shadow);
}
...
void ecore_vf_get_port_mac(struct ecore_hwfn *p_hwfn, u8 *port_mac) {
  OSAL_MEMCPY(port_mac, p_hwfn->vf_iov_info->acquire_resp.pfdev_info.port_mac,
              ETH_ALEN);
}
...
void ecore_vf_get_num_sbs(struct ecore_hwfn *p_hwfn, u32 *num_sbs) {
  struct ecore_vf_iov *p_vf;
