...
static OSAL_INLINE void ecore_cxt_set_proto_tid_count(struct ecore_hwfn *p_hwfn,
                                                      enum protocol_type proto,
                                                      u8 seg, u8 seg_type,
                                                      u32 count, bool has_fl) {
  struct ecore_cxt_mngr *p_mngr = p_hwfn->p_cxt_mngr;
...
static void ecore_ilt_cli_adv_line(struct ecore_hwfn *p_hwfn,
                                   ...
                                   struct ecore_ilt_cli_blk *p_blk, u32 *p_line,
  ...
  *p_line += DIV_ROUND_UP(p_blk->total_size, p_blk->real_size_in_page);
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_ILT,
             ...
             client_id, p_cli->first.val, p_cli->last.val, p_blk->total_size,
...
static void ecore_ilt_get_dynamic_line_range(struct ecore_hwfn *p_hwfn,
  ...
  *dynamic_line_cnt = 0;
  ...
  if (ilt_client == ILT_CLI_CDUC) {
    ...
    cxts_per_p =
        ILT_PAGE_IN_BYTES(p_cli->p_size.val) / (u32)CONN_CXT_SIZE(p_hwfn);
...
static u32 ecore_cxt_src_elements(struct ecore_cxt_mngr *p_mngr) {
...
enum _ecore_status_t ecore_cxt_cfg_ilt_compute(struct ecore_hwfn *p_hwfn) {
  ...
  ecore_ilt_cli_blk_fill(p_cli, p_blk, curr_line, total, CONN_CXT_SIZE(p_hwfn));
  ...
  for (i = 0; i < NUM_TASK_PF_SEGMENTS; i++) {
    ...
    ecore_ilt_cli_adv_line(p_hwfn, p_cli, p_blk, &curr_line, ILT_CLI_CDUT);
  }
  ...
  if (p_seg && p_seg->count) {
    ...
    if (!p_seg->has_fl_mem) {
      ...
      ecore_ilt_cli_blk_fill(p_cli, p_blk, curr_line, total, task_size);
      ecore_ilt_cli_adv_line(p_hwfn, p_cli, p_blk, &curr_line, ILT_CLI_CDUT);
    }
    p_cli->vf_total_lines =
        curr_line - (p_cli->first.val + p_cli->pf_total_lines);
  ...
  total = ecore_qm_pf_mem_size(
      p_hwfn, qm_iids.cids, qm_iids.vf_cids, qm_iids.tids,
      p_hwfn->qm_info.num_pqs + OFLD_GRP_SIZE, p_hwfn->qm_info.num_vf_pqs);
  ...
  if (total) {
    p_blk = ecore_cxt_set_blk(&p_cli->pf_blks[0]);
    ecore_ilt_cli_blk_fill(p_cli, p_blk, curr_line, total * TM_ELEM_SIZE,
                           TM_ELEM_SIZE);

    ecore_ilt_cli_adv_line(p_hwfn, p_cli, p_blk, &curr_line, ILT_CLI_TM);
    p_cli->pf_total_lines = curr_line - p_blk->start_line;
  }
  ...
  if (total) {
    ...
    p_blk = ecore_cxt_set_blk(&p_cli->pf_blks[0]);
    ecore_ilt_cli_blk_fill(p_cli, p_blk, curr_line, total_size, elem_size);
    ecore_ilt_cli_adv_line(p_hwfn, p_cli, p_blk, &curr_line, ILT_CLI_SRC);
    p_cli->pf_total_lines = curr_line - p_blk->start_line;
  }
  ...
  if (total) {
    ...
    ecore_ilt_cli_blk_fill(p_cli, p_blk, curr_line, total * SRQ_CXT_SIZE,
                           SRQ_CXT_SIZE);

    ecore_ilt_cli_adv_line(p_hwfn, p_cli, p_blk, &curr_line, ILT_CLI_TSDM);
    p_cli->pf_total_lines = curr_line - p_blk->start_line;
  }
...
static void ecore_cxt_src_t2_free(struct ecore_hwfn *p_hwfn) {
  ...
  for (i = 0; i < p_t2->num_pages; i++)
    if (p_t2->dma_mem[i].virt_addr)
      OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_t2->dma_mem[i].virt_addr,
                             p_t2->dma_mem[i].phys_addr, p_t2->dma_mem[i].size);

  OSAL_FREE(p_hwfn->p_dev, p_t2->dma_mem);
  p_t2->dma_mem = OSAL_NULL;
}

static enum _ecore_status_t ecore_cxt_t2_alloc_pages(struct ecore_hwfn *p_hwfn,
...
static enum _ecore_status_t ecore_cxt_src_t2_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  p_t2->dma_mem = OSAL_ZALLOC(p_hwfn->p_dev, GFP_KERNEL,
                              p_t2->num_pages * sizeof(struct phys_mem_desc));
  if (!p_t2->dma_mem) {
...
static u32 ecore_cxt_ilt_shadow_size(struct ecore_ilt_client_cfg *ilt_clients) {
  ...
  for_each_ilt_valid_client(i, ilt_clients) size +=
      (ilt_clients[i].last.val - ilt_clients[i].first.val + 1);
...
static void ecore_ilt_shadow_free(struct ecore_hwfn *p_hwfn) {
  ...
  for (i = 0; p_mngr->ilt_shadow && i < ilt_size; i++) {
    ...
    if (p_dma->virt_addr)
      OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_dma->p_virt, p_dma->phys_addr,
                             p_dma->size);
...
static enum _ecore_status_t ecore_ilt_blk_alloc(struct ecore_hwfn *p_hwfn,
  ...
  line =
      p_blk->start_line + start_line_offset - p_hwfn->p_cxt_mngr->pf_start_line;
...
#define ILT_BLOCK_ALIGN_SIZE 0x1000
    p_virt = OSAL_DMA_ALLOC_COHERENT_ALIGNED(p_hwfn->p_dev, &p_phys, size,
...
static enum _ecore_status_t ecore_ilt_shadow_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_ILT, "Allocated 0x%x bytes for ilt shadow\n",
  ...
  for_each_ilt_valid_client(i, clients) {
    ...
    for (k = 0; k < p_mngr->vf_count; k++) {
      for (j = 0; j < ILT_CLI_VF_BLOCKS; j++) {
        ...
        rc = ecore_ilt_blk_alloc(p_hwfn, p_blk, i, lines);
        if (rc != ECORE_SUCCESS)
          goto ilt_shadow_fail;
      }
    }
...
static void ecore_cid_map_free(struct ecore_hwfn *p_hwfn) {
  ...
  for (type = 0; type < MAX_CONN_TYPES; type++) {
    ...
    for (vf = 0; vf < max_num_vfs; vf++) {
      OSAL_FREE(p_hwfn->p_dev, p_mngr->acquired_vf[type][vf].cid_map);
...
__ecore_cid_map_alloc_single(struct ecore_hwfn *p_hwfn, u32 type, u32 cid_start,
                             u32 cid_count,
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_CXT, "Type %08x start: %08x count %08x\n", type,
             p_map->start_cid, p_map->max_count);
...
static enum _ecore_status_t ecore_cid_map_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  for (type = 0; type < MAX_CONN_TYPES; type++) {
    rc = ecore_cid_map_alloc_single(p_hwfn, type, start_cid, vf_start_cid);
    if (rc != ECORE_SUCCESS)
      goto cid_map_fail;
...
enum _ecore_status_t ecore_cxt_mngr_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  for (i = 0; i < MAX_CONN_TYPES; i++) {
    acquired_vf = OSAL_CALLOC(p_hwfn->p_dev, GFP_KERNEL, max_num_vfs,
                              sizeof(*acquired_vf));
    if (!acquired_vf) {
      DP_NOTICE(
          p_hwfn, false,
...
void ecore_cxt_mngr_setup(struct ecore_hwfn *p_hwfn) {
  ...
  for (type = 0; type < MAX_CONN_TYPES; type++) {
    ...
    if (p_cfg->cid_count) {
      p_map = &p_mngr->acquired[type];
      len = DIV_ROUND_UP(p_map->max_count, BITS_PER_MAP_WORD) * MAP_WORD_SIZE;
...
/* CDU Common */
#define CDUC_CXT_SIZE_SHIFT CDU_REG_CID_ADDR_PARAMS_CONTEXT_SIZE_SHIFT

#define CDUC_CXT_SIZE_MASK                                                     \
  (CDU_REG_CID_ADDR_PARAMS_CONTEXT_SIZE >> CDUC_CXT_SIZE_SHIFT)

#define CDUC_BLOCK_WASTE_SHIFT CDU_REG_CID_ADDR_PARAMS_BLOCK_WASTE_SHIFT

#define CDUC_BLOCK_WASTE_MASK                                                  \
  (CDU_REG_CID_ADDR_PARAMS_BLOCK_WASTE >> CDUC_BLOCK_WASTE_SHIFT)

#define CDUC_NCIB_SHIFT CDU_REG_CID_ADDR_PARAMS_NCIB_SHIFT

#define CDUC_NCIB_MASK (CDU_REG_CID_ADDR_PARAMS_NCIB >> CDUC_NCIB_SHIFT)

#define CDUT_TYPE0_CXT_SIZE_SHIFT CDU_REG_SEGMENT0_PARAMS_T0_TID_SIZE_SHIFT

#define CDUT_TYPE0_CXT_SIZE_MASK                                               \
  (CDU_REG_SEGMENT0_PARAMS_T0_TID_SIZE >> CDUT_TYPE0_CXT_SIZE_SHIFT)

#define CDUT_TYPE0_BLOCK_WASTE_SHIFT                                           \
  CDU_REG_SEGMENT0_PARAMS_T0_TID_BLOCK_WASTE_SHIFT

#define CDUT_TYPE0_BLOCK_WASTE_MASK                                            \
  (CDU_REG_SEGMENT0_PARAMS_T0_TID_BLOCK_WASTE >> CDUT_TYPE0_BLOCK_WASTE_SHIFT)

#define CDUT_TYPE0_NCIB_SHIFT CDU_REG_SEGMENT0_PARAMS_T0_NUM_TIDS_IN_BLOCK_SHIFT

#define CDUT_TYPE0_NCIB_MASK                                                   \
  (CDU_REG_SEGMENT0_PARAMS_T0_NUM_TIDS_IN_BLOCK >> CDUT_TYPE0_NCIB_SHIFT)

#define CDUT_TYPE1_CXT_SIZE_SHIFT CDU_REG_SEGMENT1_PARAMS_T1_TID_SIZE_SHIFT

#define CDUT_TYPE1_CXT_SIZE_MASK                                               \
  (CDU_REG_SEGMENT1_PARAMS_T1_TID_SIZE >> CDUT_TYPE1_CXT_SIZE_SHIFT)

#define CDUT_TYPE1_BLOCK_WASTE_SHIFT                                           \
  CDU_REG_SEGMENT1_PARAMS_T1_TID_BLOCK_WASTE_SHIFT

#define CDUT_TYPE1_BLOCK_WASTE_MASK                                            \
  (CDU_REG_SEGMENT1_PARAMS_T1_TID_BLOCK_WASTE >> CDUT_TYPE1_BLOCK_WASTE_SHIFT)

#define CDUT_TYPE1_NCIB_SHIFT CDU_REG_SEGMENT1_PARAMS_T1_NUM_TIDS_IN_BLOCK_SHIFT

#define CDUT_TYPE1_NCIB_MASK                                                   \
  (CDU_REG_SEGMENT1_PARAMS_T1_NUM_TIDS_IN_BLOCK >> CDUT_TYPE1_NCIB_SHIFT)
...
static void ecore_cdu_init_pf(struct ecore_hwfn *p_hwfn) {
  ...
  for (i = 0; i < NUM_TASK_PF_SEGMENTS; i++) {
    ...
    offset = (ILT_PAGE_IN_BYTES(p_cli->p_size.val) *
              (p_cli->pf_blks[CDUT_SEG_BLK(i)].start_line - p_cli->first.val)) /
...
void ecore_qm_init_pf(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  ecore_qm_pf_rt_init(
      p_hwfn, p_ptt, p_hwfn->rel_pf_id, qm_info->max_phys_tcs_per_port,
      is_pf_loading, iids.cids, iids.vf_cids, iids.tids, qm_info->start_pq,
      qm_info->num_pqs - qm_info->num_vf_pqs, qm_info->num_vf_pqs,
      qm_info->start_vport, qm_info->num_vports, qm_info->pf_wfq,
...
static void ecore_ilt_bounds_init(struct ecore_hwfn *p_hwfn) {
  ...
  for_each_ilt_valid_client(i, ilt_clients) {
    STORE_RT_REG(p_hwfn, ilt_clients[i].first.reg, ilt_clients[i].first.val);
    STORE_RT_REG(p_hwfn, ilt_clients[i].last.reg, ilt_clients[i].last.val);
    STORE_RT_REG(p_hwfn, ilt_clients[i].p_size.reg, ilt_clients[i].p_size.val);
  }
}
...
static void ecore_ilt_vf_bounds_init(struct ecore_hwfn *p_hwfn) {
  ...
  if (p_hwfn->p_dev->p_iov_info) {
    ...
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_VF_BASE_RT_OFFSET, p_iov->first_vf_in_pf);
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_VF_LAST_ILT_RT_OFFSET,
  ...
  if (p_cli->active) {
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_CDUC_BLOCKS_FACTOR_RT_OFFSET, blk_factor);
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_CDUC_NUMBER_OF_PF_BLOCKS_RT_OFFSET,
                 p_cli->pf_total_lines);
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_CDUC_VF_BLOCKS_RT_OFFSET,
                 p_cli->vf_total_lines);
  }
  ...
  blk_factor = OSAL_LOG2(ILT_PAGE_IN_BYTES(p_cli->p_size.val) >> 10);
  if (p_cli->active) {
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_CDUT_BLOCKS_FACTOR_RT_OFFSET, blk_factor);
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_CDUT_NUMBER_OF_PF_BLOCKS_RT_OFFSET,
                 p_cli->pf_total_lines);
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_CDUT_VF_BLOCKS_RT_OFFSET,
                 p_cli->vf_total_lines);
  }
  ...
  blk_factor = OSAL_LOG2(ILT_PAGE_IN_BYTES(p_cli->p_size.val) >> 10);
  if (p_cli->active) {
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_TM_BLOCKS_FACTOR_RT_OFFSET, blk_factor);
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_TM_NUMBER_OF_PF_BLOCKS_RT_OFFSET,
                 p_cli->pf_total_lines);
    STORE_RT_REG(p_hwfn, PSWRQ2_REG_TM_VF_BLOCKS_RT_OFFSET,
                 p_cli->vf_total_lines);
  }
}
    ...
    for (; line <= clients[i].last.val - p_mngr->pf_start_line;
      ...
      if (p_shdw[line].virt_addr != OSAL_NULL) {
        ...
        DP_VERBOSE(p_hwfn, ECORE_MSG_ILT,
                   ...
                   " Physical addr: 0x%lx\n",
                   rt_offst, line, i,
                   (unsigned long)(p_shdw[line].phys_addr >> 12));
      }
...
static void ecore_prs_init_pf(struct ecore_hwfn *p_hwfn) {
  ...
  STORE_RT_REG_AGG(p_hwfn, PRS_REG_TASK_ID_MAX_INITIATOR_PF_RT_OFFSET,
                   p_tid->count);
}
...
enum _ecore_status_t _ecore_cxt_acquire_cid(struct ecore_hwfn *p_hwfn,
  ...
  rel_cid = OSAL_FIND_FIRST_ZERO_BIT(p_map->cid_map, p_map->max_count);
...
static bool
ecore_cxt_test_cid_acquired(struct ecore_hwfn *p_hwfn, u32 cid, u8 vfid,
  ...
  if (!OSAL_GET_BIT(rel_cid, (*pp_map)->cid_map)) {
    DP_NOTICE(p_hwfn, true, "CID %d [vifd %02x] not acquired", cid, vfid);
    goto fail;
  }
...
void _ecore_cxt_release_cid(struct ecore_hwfn *p_hwfn, u32 cid, u8 vfid) {
  ...
  b_acquired = ecore_cxt_test_cid_acquired(p_hwfn, cid, vfid, &type, &p_map);
...
enum _ecore_status_t ecore_cxt_set_pf_params(struct ecore_hwfn *p_hwfn) {
  ...
  case ECORE_PCI_ETH: {
    ...
    struct ecore_eth_pf_params *p_params = &p_hwfn->pf_params.eth_pf_params;

    if (!p_params->num_vf_cons)
      p_params->num_vf_cons = ETH_PF_PARAMS_VF_CONS_DEFAULT;
    ecore_cxt_set_proto_cid_count(p_hwfn, PROTOCOLID_ETH, p_params->num_cons,
...
ecore_cxt_dynamic_ilt_alloc(struct ecore_hwfn *p_hwfn,
  ...
  case ECORE_ELEM_TASK:
    ...
    break;
  default:
    DP_NOTICE(p_hwfn, false, "ECORE_INVALID elem type = %d", elem_type);
    return ECORE_INVAL;
  }
  ...
  p_virt =
      OSAL_DMA_ALLOC_COHERENT(p_hwfn->p_dev, &p_phys, p_blk->real_size_in_page);
  if (!p_virt) {
    rc = ECORE_NOMEM;
    goto out1;
  }
  ...
  p_hwfn->p_cxt_mngr->ilt_shadow[shadow_line].size = p_blk->real_size_in_page;
...
ecore_cxt_free_ilt_range(struct ecore_hwfn *p_hwfn,
                         enum ecore_cxt_elem_type elem_type, u32 start_iid,
                         u32 count) {
  ...
  for (i = shadow_start_line; i < shadow_end_line; i++) {
    ...
    /* compute absolute offset */
    reg_offset = PSWRQ2_REG_ILT_MEMORY +
                 ((start_line++) * ILT_REG_SIZE_IN_BYTES * ILT_ENTRY_IN_REGS);
