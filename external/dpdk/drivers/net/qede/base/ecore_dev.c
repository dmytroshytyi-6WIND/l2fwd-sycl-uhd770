...
bool ecore_db_rec_sanity(struct ecore_dev *p_dev, void OSAL_IOMEM *db_addr,
  ...
  if (db_addr < p_dev->doorbells ||
      (u8 *)db_addr > (u8 *)p_dev->doorbells + p_dev->db_size) {
    OSAL_WARN(true,
...
struct ecore_hwfn *ecore_db_rec_find_hwfn(struct ecore_dev *p_dev,
  ...
  if (ECORE_IS_CMT(p_dev))
    p_hwfn = db_addr < p_dev->hwfns[1].doorbells ? &p_dev->hwfns[0]
                                                 : &p_dev->hwfns[1];
  else
...
enum _ecore_status_t ecore_db_recovery_add(struct ecore_dev *p_dev,
  ...
  OSAL_LIST_PUSH_TAIL(&db_entry->list_entry, &p_hwfn->db_recovery_info.list);
...
enum _ecore_status_t ecore_db_recovery_del(struct ecore_dev *p_dev,
  ...
  /* protect the list */
  OSAL_SPIN_LOCK(&p_hwfn->db_recovery_info.lock);
  OSAL_LIST_FOR_EACH_ENTRY(db_entry, &p_hwfn->db_recovery_info.list, list_entry,
                           struct ecore_db_recovery_entry) {
  ...
  if (rc == ECORE_INVAL)
    ...
    DP_NOTICE(p_hwfn, false,
              "Failed to find element in list. Key (db_data addr) was %p. "
              "db_addr was %p\n",
              db_data, db_addr);
  else
...
void ecore_db_recovery_teardown(struct ecore_hwfn *p_hwfn) {
  ...
  if (!OSAL_LIST_IS_EMPTY(&p_hwfn->db_recovery_info.list)) {
    DP_VERBOSE(
        p_hwfn, false,
        "Doorbell Recovery teardown found the doorbell recovery list was not "
        "empty (Expected in disorderly driver unload (e.g. recovery) otherwise "
        "this probably means some flow forgot to db_recovery_del). Prepare to "
        "purge doorbell recovery list...\n");
    while (!OSAL_LIST_IS_EMPTY(&p_hwfn->db_recovery_info.list)) {
      db_entry =
          OSAL_LIST_FIRST_ENTRY(&p_hwfn->db_recovery_info.list,
                                struct ecore_db_recovery_entry, list_entry);
...
void ecore_db_recovery_ring(struct ecore_hwfn *p_hwfn,
  ...
  if (db_entry->db_width == DB_REC_WIDTH_32B)
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_SPQ, "%s doorbell address %p data %lx\n",
               db_exec == DB_REC_DRY_RUN ? "would have rung" : "ringing",
               db_entry->db_addr, *(unsigned long *)(db_entry->db_data));

  /* Sanity */
  if (!ecore_db_rec_sanity(p_hwfn->p_dev, db_entry->db_addr, db_entry->db_data))
    return;
...
static enum _ecore_status_t ecore_llh_alloc(struct ecore_dev *p_dev) {
  ...
  size = NIG_REG_LLH_FUNC_FILTER_EN_SIZE * sizeof(**p_llh_info->pp_filters);
  for (i = 0; i < p_llh_info->num_ppfid; i++) {
    p_llh_info->pp_filters[i] = OSAL_ZALLOC(p_dev, GFP_KERNEL, size);
...
static enum _ecore_status_t ecore_llh_shadow_sanity(struct ecore_dev *p_dev,
  ...
  if (filter_idx >= NIG_REG_LLH_FUNC_FILTER_EN_SIZE) {
    DP_NOTICE(p_dev, false,
              "LLH shadow [%s]: using filter_idx %d while only %d filters are "
              "available\n",
...
ecore_llh_shadow_search_filter(struct ecore_dev *p_dev, u8 ppfid,
  ...
  *p_filter_idx = ECORE_LLH_INVALID_FILTER_IDX;
  ...
  for (i = 0; i < NIG_REG_LLH_FUNC_FILTER_EN_SIZE; i++) {
    if (!OSAL_MEMCMP(p_filter, &p_filters[i].filter, sizeof(*p_filter))) {
      *p_filter_idx = i;
      break;
    }
  }
...
static enum _ecore_status_t ecore_llh_shadow_add_filter(
    struct ecore_dev *p_dev, u8 ppfid, enum ecore_llh_filter_type type,
    union ecore_llh_filter *p_filter, u8 *p_filter_idx, u32 *p_ref_cnt) {
  enum _ecore_status_t rc;

  /* Check if the same filter already exist */
  rc = ecore_llh_shadow_search_filter(p_dev, ppfid, p_filter, p_filter_idx);
  if (rc != ECORE_SUCCESS)
    return rc;
...
__ecore_llh_shadow_remove_filter(struct ecore_dev *p_dev, u8 ppfid,
  ...
  *p_ref_cnt = --p_filters[filter_idx].ref_cnt;
  if (!p_filters[filter_idx].ref_cnt)
    OSAL_MEM_ZERO(&p_filters[filter_idx], sizeof(p_filters[filter_idx]));
...
static enum _ecore_status_t
__ecore_llh_set_engine_affin(struct ecore_hwfn *p_hwfn,
  ...
  if (ECORE_IS_ROCE_PERSONALITY(p_hwfn)) {
    ...
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_dev, false, "Failed to set the RoCE engine affinity\n");
      return rc;
    }
  ...
  if (ECORE_IS_FCOE_PERSONALITY(p_hwfn) || ECORE_IS_ISCSI_PERSONALITY(p_hwfn))
    eng = p_dev->fir_affin ? ECORE_ENG1 : ECORE_ENG0;
  else /* L2_PERSONALITY */
    eng = ECORE_BOTH_ENG;
  ...
  for (ppfid = 0; ppfid < p_dev->p_llh_info->num_ppfid; ppfid++) {
    ...
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_dev, false, "Failed to set the engine affinity of ppfid %d\n",
                ppfid);
      return rc;
    }
  }
...
ecore_llh_set_engine_affin(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  if (avoid_eng_affin) {
    ...
    if (ECORE_IS_ROCE_PERSONALITY(p_hwfn)) {
      ...
      DP_VERBOSE(p_dev, ECORE_MSG_SP,
                 "LLH [backwards compatible mode]: Set the engine affinity of "
                 "RoCE packets as %d\n",
                 eng);
    }
...
static enum _ecore_status_t ecore_llh_hw_init_pf(struct ecore_hwfn *p_hwfn,
  ...
  if (OSAL_GET_BIT(ECORE_MF_LLH_MAC_CLSS, &p_dev->mf_bits) &&
    ...
    rc = ecore_llh_add_mac_filter(p_dev, 0, p_hwfn->hw_info.hw_mac_addr);
    if (rc != ECORE_SUCCESS)
      DP_NOTICE(p_dev, false,
...
enum _ecore_status_t ecore_llh_set_ppfid_affinity(struct ecore_dev *p_dev,
  ...
  case ECORE_BOTH_ENG:
    ...
    break;
  default:
    DP_NOTICE(p_dev, false, "Invalid affinity value for ppfid [%d]\n", eng);
    rc = ECORE_INVAL;
    goto out;
  }
...
enum _ecore_status_t ecore_llh_set_roce_affinity(struct ecore_dev *p_dev,
  ...
  case ECORE_BOTH_ENG:
    ...
    break;
  default:
    DP_NOTICE(p_dev, false, "Invalid affinity value for RoCE [%d]\n", eng);
    rc = ECORE_INVAL;
    goto out;
  }
...
static enum _ecore_status_t
ecore_llh_access_filter(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  if (b_write_access && !p_details->enable) {
    ...
    ecore_ppfid_wr(p_hwfn, p_ptt, abs_ppfid, addr, p_details->enable);
  }
  ...
  if (b_write_access) {
    ...
    rc = ecore_dmae_host2grc(p_hwfn, p_ptt,
                             (u64)(osal_uintptr_t)&p_details->value, addr,
  ...
  if (b_write_access)
    ...
    p_details->mode = ecore_ppfid_rd(p_hwfn, p_ptt, abs_ppfid, addr);

  /* Filter protocol type */
  addr = NIG_REG_LLH_FUNC_FILTER_PROTOCOL_TYPE + filter_idx * 0x4;
  if (b_write_access)
    ecore_ppfid_wr(p_hwfn, p_ptt, abs_ppfid, addr, p_details->protocol_type);
  else
    p_details->protocol_type = ecore_ppfid_rd(p_hwfn, p_ptt, abs_ppfid, addr);

  /* Filter header select */
  addr = NIG_REG_LLH_FUNC_FILTER_HDR_SEL + filter_idx * 0x4;
  if (b_write_access)
    ecore_ppfid_wr(p_hwfn, p_ptt, abs_ppfid, addr, p_details->hdr_sel);
  else
    p_details->hdr_sel = ecore_ppfid_rd(p_hwfn, p_ptt, abs_ppfid, addr);
  ...
  if (!b_write_access || p_details->enable) {
    ...
    if (b_write_access)
      ...
      p_details->enable = ecore_ppfid_rd(p_hwfn, p_ptt, abs_ppfid, addr);
  }

  return ECORE_SUCCESS;
}

static enum _ecore_status_t ecore_llh_add_filter(struct ecore_hwfn *p_hwfn,
                                                 struct ecore_ptt *p_ptt,
                                                 u8 abs_ppfid, u8 filter_idx,
                                                 u8 filter_prot_type, u32 high,
                                                 u32 low) {
  ...
  filter_details.mode = filter_prot_type ? 1
...
static enum _ecore_status_t ecore_llh_remove_filter(struct ecore_hwfn *p_hwfn,
                                                    struct ecore_ptt *p_ptt,
                                                    u8 abs_ppfid,
                                                    u8 filter_idx) {
  struct ecore_llh_filter_details filter_details;
...
enum _ecore_status_t ecore_llh_add_mac_filter(struct ecore_dev *p_dev, u8 ppfid,
  ...
  rc = ecore_llh_shadow_add_filter(p_dev, ppfid, ECORE_LLH_FILTER_TYPE_MAC,
  ...
  if (ref_cnt == 1) {
    ...
    low = mac_addr[5] | (mac_addr[4] << 8) | (mac_addr[3] << 16) |
          (mac_addr[2] << 24);
    rc = ecore_llh_add_filter(p_hwfn, p_ptt, abs_ppfid, filter_idx, 0, high,
                              low);
    if (rc != ECORE_SUCCESS)
      goto err;
  }
  ...
  DP_VERBOSE(
      p_dev, ECORE_MSG_SP,
      "LLH: Added MAC filter [%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx] to "
      "ppfid %hhd [abs %hhd] at idx %hhd [ref_cnt %d]\n",
...
err:
  DP_NOTICE(p_dev, false,
            "LLH: Failed to add MAC filter "
            "[%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx] to ppfid %hhd\n",
            mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4],
            mac_addr[5], ppfid);
out:
  ecore_ptt_release(p_hwfn, p_ptt);
...
ecore_llh_protocol_filter_stringify(struct ecore_dev *p_dev,
  ...
  case ECORE_LLH_FILTER_ETHERTYPE:
    OSAL_SNPRINTF(str, str_len, "Ethertype 0x%04x", source_port_or_eth_type);
    break;
  case ECORE_LLH_FILTER_TCP_SRC_PORT:
    OSAL_SNPRINTF(str, str_len, "TCP src port 0x%04x", source_port_or_eth_type);
    break;
  case ECORE_LLH_FILTER_UDP_SRC_PORT:
    OSAL_SNPRINTF(str, str_len, "UDP src port 0x%04x", source_port_or_eth_type);
    break;
  ...
  case ECORE_LLH_FILTER_UDP_SRC_AND_DEST_PORT:
    ...
    break;
  default:
    DP_NOTICE(p_dev, true, "Non valid LLH protocol filter type %d\n", type);
    return ECORE_INVAL;
  }
...
static enum _ecore_status_t ecore_llh_protocol_filter_to_hilo(
    struct ecore_dev *p_dev, enum ecore_llh_prot_filter_type_t type,
    u16 source_port_or_eth_type, u16 dest_port, u32 *p_high, u32 *p_low) {
  *p_high = 0;
  *p_low = 0;
...
ecore_llh_add_protocol_filter(struct ecore_dev *p_dev, u8 ppfid,
  ...
  rc = ecore_llh_protocol_filter_stringify(p_dev, type, source_port_or_eth_type,
  ...
  /* Configure the LLH only in case of a new the filter */
  if (ref_cnt == 1) {
    rc = ecore_llh_protocol_filter_to_hilo(p_dev, type, source_port_or_eth_type,
                                           dest_port, &high, &low);
    if (rc != ECORE_SUCCESS)
      goto err;

    type_bitmap = 0x1 << type;
    rc = ecore_llh_add_filter(p_hwfn, p_ptt, abs_ppfid, filter_idx, type_bitmap,
                              high, low);
    if (rc != ECORE_SUCCESS)
      goto err;
  }
  ...
  DP_VERBOSE(p_dev, ECORE_MSG_SP,
             "LLH: Added protocol filter [%s] to ppfid %hhd [abs %hhd] at idx "
             "%hhd [ref_cnt %d]\n",
...
void ecore_llh_remove_mac_filter(struct ecore_dev *p_dev, u8 ppfid,
  ...
  if (!ref_cnt) {
    rc = ecore_llh_remove_filter(p_hwfn, p_ptt, abs_ppfid, filter_idx);
    if (rc != ECORE_SUCCESS)
      goto err;
  }
  ...
  DP_VERBOSE(
      p_dev, ECORE_MSG_SP,
      "LLH: Removed MAC filter [%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx] "
      "from ppfid %hhd [abs %hhd] at idx %hhd [ref_cnt %d]\n",
      mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4],
      mac_addr[5], ppfid, abs_ppfid, filter_idx, ref_cnt);
...
err:
  DP_NOTICE(p_dev, false,
            "LLH: Failed to remove MAC filter "
            "[%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx] from ppfid %hhd\n",
            mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4],
            mac_addr[5], ppfid);
out:
  ecore_ptt_release(p_hwfn, p_ptt);
...
void ecore_llh_remove_protocol_filter(struct ecore_dev *p_dev, u8 ppfid,
  ...
  DP_VERBOSE(p_dev, ECORE_MSG_SP,
             "LLH: Removed protocol filter [%s] from ppfid %hhd [abs %hhd] at "
             "idx %hhd [ref_cnt %d]\n",
             str, ppfid, abs_ppfid, filter_idx, ref_cnt);
...
enum _ecore_status_t ecore_llh_dump_ppfid(struct ecore_dev *p_dev, u8 ppfid) {
  ...
  DP_NOTICE(p_hwfn, false,
            ...
            p_hwfn->rel_pf_id, ppfid, abs_ppfid, ecore_rd(p_hwfn, p_ptt, addr));

  for (filter_idx = 0; filter_idx < NIG_REG_LLH_FUNC_FILTER_EN_SIZE;
       filter_idx++) {
    OSAL_MEMSET(&filter_details, 0, sizeof(filter_details));
    rc = ecore_llh_access_filter(p_hwfn, p_ptt, abs_ppfid, filter_idx,
                                 &filter_details, false /* read access */);
    if (rc != ECORE_SUCCESS)
      goto out;

    DP_NOTICE(p_hwfn, false,
              "filter %2hhd: enable %d, value 0x%016lx, mode %d, protocol_type "
              "0x%x, hdr_sel 0x%x\n",
              filter_idx, filter_details.enable,
              (unsigned long)filter_details.value, filter_details.mode,
...
/* Configurable */
#define ECORE_MIN_DPIS                                                         \
  (4) /* The minimal num of DPIs required to                                   \
...
static u32 ecore_hw_bar_size(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                             enum BAR_ID bar_id) {
  u32 bar_reg = (bar_id == BAR_ID_0 ? PGLUE_B_REG_PF_BAR0_SIZE
  ...
  if (ECORE_IS_CMT(p_hwfn->p_dev)) {
    DP_INFO(p_hwfn, "BAR size not configured. Assuming BAR size of 256kB for "
                    "GRC and 512kB for DB\n");
    val = BAR_ID_0 ? 256 * 1024 : 512 * 1024;
  } else {
    DP_INFO(p_hwfn, "BAR size not configured. Assuming BAR size of 512kB for "
                    "GRC and 512kB for DB\n");
    val = 512 * 1024;
  }
...
void ecore_resc_free(struct ecore_dev *p_dev) {
  ...
  if (IS_VF(p_dev)) {
    for_each_hwfn(p_dev, i) ecore_l2_free(&p_dev->hwfns[i]);
    return;
  }
...
u16 ecore_init_qm_get_num_vfs(struct ecore_hwfn *p_hwfn) {
  return IS_ECORE_SRIOV(p_hwfn->p_dev) ? p_hwfn->p_dev->p_iov_info->total_vfs
                                       : 0;
}
...
static void ecore_init_qm_port_params(struct ecore_hwfn *p_hwfn) {
  ...
  active_phys_tcs = num_ports == MAX_NUM_PORTS_K2 ? ACTIVE_TCS_BMAP_4PORT_K2
                                                  : ACTIVE_TCS_BMAP;

  for (i = 0; i < num_ports; i++) {
    struct init_qm_port_params *p_qm_port = &p_hwfn->qm_info.qm_port_params[i];
    ...
    p_qm_port->num_btb_blocks = NUM_OF_BTB_BLOCKS(p_dev) / num_ports;
  }
}
...
static void ecore_init_qm_pq(struct ecore_hwfn *p_hwfn,
                             struct ecore_qm_info *qm_info, u8 tc,
                             u32 pq_init_flags) {
  ...
  qm_info->qm_pq_params[pq_idx].rl_id = qm_info->qm_pq_params[pq_idx].vport_id;
...
static void ecore_init_qm_set_idx(struct ecore_hwfn *p_hwfn, u32 pq_flags,
                                  u16 pq_val) {
...
u16 ecore_get_qm_vport_idx_rl(struct ecore_hwfn *p_hwfn, u16 rl) {
  ...
  if (qm_pq_idx > p_hwfn->qm_info.num_pqs) {
    DP_ERR(p_hwfn, "qm_pq_idx %d must be smaller than %d\n", qm_pq_idx,
...
static void ecore_dp_init_qm_params(struct ecore_hwfn *p_hwfn) {
  ...
  for (i = 0; i < qm_info->num_pqs; i++) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_SP,
               "pq idx %d, port %d, vport_id %d, tc %d, wrr_grp %d, rl_valid "
               "%d, rl_id %d\n",
               qm_info->start_pq + i, pq->port_id, pq->vport_id, pq->tc_id,
               pq->wrr_group, pq->rl_valid, pq->rl_id);
  }
}
...
enum _ecore_status_t ecore_qm_reconf(struct ecore_hwfn *p_hwfn,
  ...
  /* stop PF's qm queues */
  b_rc = ecore_send_qm_stop_cmd(p_hwfn, p_ptt, false, true, qm_info->start_pq,
                                qm_info->num_pqs);
  if (!b_rc) {
    rc = ECORE_INVAL;
    goto unlock;
  ...
  b_rc = ecore_send_qm_stop_cmd(p_hwfn, p_ptt, true, true, qm_info->start_pq,
                                qm_info->num_pqs);
  if (!b_rc)
    rc = ECORE_INVAL;
...
enum _ecore_status_t ecore_resc_alloc(struct ecore_dev *p_dev) {
  ...
  p_dev->fw_data = OSAL_ZALLOC(p_dev, GFP_KERNEL, sizeof(*p_dev->fw_data));
  if (!p_dev->fw_data)
    return ECORE_NOMEM;
  ...
  for_each_hwfn(p_dev, i) {
    ...
    p_hwfn->p_dpc_ptt = ecore_get_reserved_ptt(p_hwfn, RESERVED_PTT_DPC);
    ...
    if (ECORE_IS_RDMA_PERSONALITY(p_hwfn)) {
      ...
      if (ECORE_IS_ROCE_PERSONALITY(p_hwfn)) {
        num_cons =
            ecore_cxt_get_proto_cid_count(p_hwfn, PROTOCOLID_ROCE, OSAL_NULL);
        num_cons *= 2;
      } else {
        num_cons =
            ecore_cxt_get_proto_cid_count(p_hwfn, PROTOCOLID_IWARP, OSAL_NULL);
      }
      n_eqes += num_cons + 2 * MAX_NUM_VFS_BB;
    } else if (p_hwfn->hw_info.personality == ECORE_PCI_ISCSI) {
      num_cons =
          ecore_cxt_get_proto_cid_count(p_hwfn, PROTOCOLID_ISCSI, OSAL_NULL);
      n_eqes += 2 * num_cons;
    }
    ...
    debug_status = OSAL_DBG_ALLOC_USER_DATA(p_hwfn, &p_hwfn->dbg_user_info);
    if (debug_status) {
      DP_NOTICE(p_hwfn, false, "Failed to allocate dbg user info structure\n");
  ...
  p_dev->reset_stats =
      OSAL_ZALLOC(p_dev, GFP_KERNEL, sizeof(*p_dev->reset_stats));
...
void ecore_resc_setup(struct ecore_dev *p_dev) {
  ...
  if (IS_VF(p_dev)) {
    for_each_hwfn(p_dev, i) ecore_l2_setup(&p_dev->hwfns[i]);
    return;
  }
  ...
  for_each_hwfn(p_dev, i) {
    ...
    OSAL_MEMCPY(p_hwfn->mcp_info->mfw_mb_shadow, p_hwfn->mcp_info->mfw_mb_cur,
...
enum _ecore_status_t ecore_final_cleanup(struct ecore_hwfn *p_hwfn,
                                         struct ecore_ptt *p_ptt, u16 id,
                                         bool is_vf) {
  u32 command = 0, addr, count = FINAL_CLEANUP_POLL_CNT;
  enum _ecore_status_t rc = ECORE_TIMEOUT;

#ifndef ASIC_ONLY
  if (CHIP_REV_IS_TEDIBEAR(p_hwfn->p_dev) || CHIP_REV_IS_SLOW(p_hwfn->p_dev)) {
  ...
  if (REG_RD(p_hwfn, addr)) {
    DP_NOTICE(p_hwfn, false, "Unexpected; Found final cleanup notification");
    DP_NOTICE(p_hwfn, false, " before initiating final cleanup\n");
...
static enum _ecore_status_t ecore_hw_init_chip(struct ecore_dev *p_dev,
  ...
  if (!CHIP_REV_IS_EMUL(p_dev)) {
    DP_NOTICE(p_dev, false,
              "ecore_hw_init_chip() shouldn't be called in a non-emulation "
              "environment\n");
    return ECORE_INVAL;
  }
  ...
  if (!pos) {
    DP_NOTICE(p_dev, true,
              "Failed to find the PCI Express Capability structure in the PCI "
              "config space\n");
    return ECORE_IO;
  }
...
static void ecore_init_cau_rt_data(struct ecore_dev *p_dev) {
  ...
  for_each_hwfn(p_dev, i) {
    ...
    for (igu_sb_id = 0; igu_sb_id < ECORE_MAPPING_MEMORY_SIZE(p_dev);
         igu_sb_id++) {
      ...
      ecore_init_cau_sb_entry(p_hwfn, &sb_entry, p_block->function_id, 0, 0);
      STORE_RT_REG_AGG(p_hwfn, offset + igu_sb_id * 2, sb_entry);
    }
  }
}
...
static void ecore_init_cache_line_size(struct ecore_hwfn *p_hwfn,
  ...
  default:
    DP_INFO(p_hwfn,
            "Unexpected value of PSWRQ2_REG_WR_MBS0 [0x%x]. Avoid configuring "
            "PGLUE_B_REG_CACHE_LINE_SIZE.\n",
            val);
    return;
  }
  ...
  default:
    DP_INFO(p_hwfn,
            "Unexpected value of cache line size [0x%x]. Avoid configuring "
            "PGLUE_B_REG_CACHE_LINE_SIZE.\n",
            cache_line_size);
  }
  ...
  if (wr_mbs < OSAL_CACHE_LINE_SIZE)
    DP_INFO(p_hwfn,
            "The cache line size for padding is suboptimal for performance [OS "
            "cache line size 0x%x, wr mbs 0x%x]\n",
...
static enum _ecore_status_t ecore_hw_init_common(struct ecore_hwfn *p_hwfn,
  ...
  ecore_qm_common_rt_init(p_hwfn, p_dev->num_ports_in_engine,
                          qm_info->max_phys_tcs_per_port, qm_info->pf_rl_en,
                          qm_info->pf_wfq_en, qm_info->vport_rl_en,
                          qm_info->vport_wfq_en, qm_info->qm_port_params,
...
static void ecore_wr_nw_port(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_LINK,
             ...
             (u32)((data >> 32) & 0xffffffff), (u32)(data & 0xffffffff));

  ecore_wr(p_hwfn, p_ptt, CNIG_REG_PMEG_IF_CMD_BB,
           (ecore_rd(p_hwfn, p_ptt, CNIG_REG_PMEG_IF_CMD_BB) & 0xffff00fe) |
               (8 << PMEG_IF_BYTE_COUNT));
...
static void ecore_emul_link_init_bb(struct ecore_hwfn *p_hwfn,
  ...
  ecore_wr_nw_port(p_hwfn, p_ptt, XLPORT_MODE_REG, (0x4 << 4) | 0x4, 1, port);
...
static void ecore_emul_link_init_ah(struct ecore_hwfn *p_hwfn,
  ...
  ecore_wr(
      p_hwfn, p_ptt, CNIG_REG_NIG_PORT0_CONF_K2 + (port << 2),
      (1 << CNIG_REG_NIG_PORT0_CONF_NIG_PORT_ENABLE_0_K2_SHIFT) |
          (port << CNIG_REG_NIG_PORT0_CONF_NIG_PORT_NWM_PORT_MAP_0_K2_SHIFT) |
  ...
  ecore_wr(p_hwfn, p_ptt, mac_base + ETH_MAC_REG_TX_FIFO_SECTIONS_K2,
           (0xA << ETH_MAC_REG_TX_FIFO_SECTIONS_TX_SECTION_EMPTY_K2_SHIFT) |
...
static void ecore_emul_link_init(struct ecore_hwfn *p_hwfn,
                                 struct ecore_ptt *p_ptt) {
  u8 port = ECORE_IS_BB(p_hwfn->p_dev) ? p_hwfn->port_id * 2 : p_hwfn->port_id;
...
static void ecore_link_init_bb(struct ecore_hwfn *p_hwfn,
  ...
  xmac_rxctrl = ecore_rd(p_hwfn, p_ptt, XMAC_REG_RX_CTRL_BB + port_offset);
...
static u32 ecore_hw_norm_region_conn(struct ecore_hwfn *p_hwfn) {
  ...
  norm_region_conn =
      ...
      ecore_cxt_get_proto_cid_count(p_hwfn, PROTOCOLID_CORE, OSAL_NULL) +
      ecore_cxt_get_proto_cid_count(p_hwfn, PROTOCOLID_ETH, OSAL_NULL);

  return norm_region_conn;
}

static enum _ecore_status_t ecore_hw_init_dpi_size(struct ecore_hwfn *p_hwfn,
                                                   struct ecore_ptt *p_ptt,
                                                   u32 pwm_region_size,
                                                   u32 n_cpus) {
...
ecore_hw_init_pf_doorbell_bar(struct ecore_hwfn *p_hwfn,
  ...
  norm_regsize = ROUNDUP(ECORE_PF_DEMS_SIZE * norm_region_conn, OSAL_PAGE_SIZE);
  ...
  if (pwm_regsize < ECORE_MIN_PWM_REGION) {
    DP_ERR(p_hwfn->p_dev,
           "PWM region size 0x%0x is too small. Should be at least 0x%0x "
           "(Doorbell BAR size is 0x%x and normal region size is 0x%0x)\n",
  ...
  DP_INFO(p_hwfn, "doorbell bar: normal_region_size=%d, pwm_region_size=%d",
          norm_regsize, pwm_regsize);
  DP_INFO(p_hwfn, " dpi_size=%d, dpi_count=%d, roce_edpm=%s\n",
          p_hwfn->dpi_size, p_hwfn->dpi_count,
          (!ecore_edpm_enabled(p_hwfn)) ? "disabled" : "enabled");

  /* Check return codes from above calls */
  if (rc != ECORE_SUCCESS) {
    DP_ERR(p_hwfn, "Failed to allocate enough DPIs\n");
    return ECORE_NORESOURCES;
  }
...
static enum _ecore_status_t ecore_hw_init_port(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_init_run(p_hwfn, p_ptt, PHASE_PORT, p_hwfn->port_id, hw_mode);
  if (rc != ECORE_SUCCESS)
    return rc;
  ...
  if (CHIP_REV_IS_EMUL(p_dev)) {
    if (ECORE_IS_CMT(p_dev)) {
      ...
      ecore_wr(p_hwfn, p_ptt, NIG_REG_LLH_ENG_CLS_TCP_4_TUPLE_SEARCH, 1);
      ecore_wr(p_hwfn, p_ptt, NIG_REG_LLH_ENG_CLS_ENG_ID_TBL, 0x55555555);
      ecore_wr(p_hwfn, p_ptt, NIG_REG_LLH_ENG_CLS_ENG_ID_TBL + 0x4, 0x55555555);
    }
    ...
    if (OSAL_GET_BIT(ECORE_MF_NEED_DEF_PF, &p_dev->mf_bits)) {
      ...
      ecore_wr(p_hwfn, p_ptt, NIG_REG_LLH_TAGMAC_DEF_PF_VECTOR, 1 << pf_id);
    }
...
ecore_hw_init_pf(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  if (hw_mode & (1 << MODE_MF_SD)) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_HW, "Configuring LLH_FUNC_FILTER_HDR_SEL\n");
    STORE_RT_REG(p_hwfn, NIG_REG_LLH_FUNC_FILTER_HDR_SEL_RT_OFFSET, 1);
  }

  /* Enable classification by MAC if needed */
  if (hw_mode & (1 << MODE_MF_SI)) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_HW, "Configuring TAGMAC_CLS_TYPE\n");
    STORE_RT_REG(p_hwfn, NIG_REG_LLH_FUNC_TAGMAC_CLS_TYPE_RT_OFFSET, 1);
  }
  ...
  if (IS_LEAD_HWFN(p_hwfn)) {
    rc = ecore_llh_hw_init_pf(p_hwfn, p_ptt, p_params->avoid_eng_affin);
    if (rc != ECORE_SUCCESS)
      return rc;
  }
  ...
  if (p_params->b_hw_start) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_STORAGE, "PRS_REG_SEARCH_TAG1: %x\n", prs_reg);

    if (p_hwfn->hw_info.personality == ECORE_PCI_FCOE) {
      ecore_wr(p_hwfn, p_ptt, PRS_REG_SEARCH_TAG1, (1 << 2));
      ecore_wr(p_hwfn, p_ptt, PRS_REG_PKT_LEN_STAT_TAGS_NOT_COUNTED_FIRST,
               0x100);
    }
    DP_VERBOSE(p_hwfn, ECORE_MSG_STORAGE,
               "PRS_REG_SEARCH registers after start PFn\n");
    prs_reg = ecore_rd(p_hwfn, p_ptt, PRS_REG_SEARCH_TCP);
    DP_VERBOSE(p_hwfn, ECORE_MSG_STORAGE, "PRS_REG_SEARCH_TCP: %x\n", prs_reg);
    prs_reg = ecore_rd(p_hwfn, p_ptt, PRS_REG_SEARCH_UDP);
    DP_VERBOSE(p_hwfn, ECORE_MSG_STORAGE, "PRS_REG_SEARCH_UDP: %x\n", prs_reg);
    prs_reg = ecore_rd(p_hwfn, p_ptt, PRS_REG_SEARCH_FCOE);
    DP_VERBOSE(p_hwfn, ECORE_MSG_STORAGE, "PRS_REG_SEARCH_FCOE: %x\n", prs_reg);
    prs_reg = ecore_rd(p_hwfn, p_ptt, PRS_REG_SEARCH_ROCE);
    DP_VERBOSE(p_hwfn, ECORE_MSG_STORAGE, "PRS_REG_SEARCH_ROCE: %x\n", prs_reg);
    prs_reg = ecore_rd(p_hwfn, p_ptt, PRS_REG_SEARCH_TCP_FIRST_FRAG);
    DP_VERBOSE(p_hwfn, ECORE_MSG_STORAGE, "PRS_REG_SEARCH_TCP_FIRST_FRAG: %x\n",
               prs_reg);
    prs_reg = ecore_rd(p_hwfn, p_ptt, PRS_REG_SEARCH_TAG1);
    DP_VERBOSE(p_hwfn, ECORE_MSG_STORAGE, "PRS_REG_SEARCH_TAG1: %x\n", prs_reg);
  }
...
enum _ecore_status_t ecore_pglueb_set_pfid_enable(struct ecore_hwfn *p_hwfn,
  ...
  ecore_wr(p_hwfn, p_ptt, PGLUE_B_REG_INTERNAL_PFID_ENABLE_MASTER, set_val);

  /* Wait until value is set - try for 1 second every 50us */
  for (delay_idx = 0; delay_idx < 20000; delay_idx++) {
    val = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_INTERNAL_PFID_ENABLE_MASTER);
    if (val == set_val)
      break;
...
ecore_fill_load_req_params(struct ecore_hwfn *p_hwfn,
  ...
  p_load_req->drv_role =
      p_drv_load->is_crash_kernel ? ECORE_DRV_ROLE_KDUMP : ECORE_DRV_ROLE_OS;
  ...
  case ECORE_TO_FALLBACK_FAIL_LOAD:
    DP_NOTICE(p_hwfn, false,
              "Received %d as a value for MFW timeout while the MFW supports "
              "only default [%d] or none [%d]. Abort.\n",
              p_drv_load->mfw_timeout_val, ECORE_LOAD_REQ_LOCK_TO_DEFAULT,
              ECORE_LOAD_REQ_LOCK_TO_NONE);
    return ECORE_ABORTED;
  }

  DP_INFO(p_hwfn,
          "Modified the MFW timeout value from %d to %s [%d] due to lack of "
          "MFW support\n",
...
enum _ecore_status_t ecore_hw_init(struct ecore_dev *p_dev,
  ...
  if ((p_params->int_mode == ECORE_INT_MODE_MSI) && ECORE_IS_CMT(p_dev)) {
    DP_NOTICE(p_dev, false, "MSI mode is not supported for CMT devices\n");
    return ECORE_INVAL;
  }
  ...
  for_each_hwfn(p_dev, i) {
    ...
    if (IS_PF(p_dev) &&
        (OSAL_GET_BIT(ECORE_MF_8021Q_TAGGING, &p_dev->mf_bits) ||
         OSAL_GET_BIT(ECORE_MF_8021AD_TAGGING, &p_dev->mf_bits))) {
      if (OSAL_GET_BIT(ECORE_MF_8021Q_TAGGING, &p_dev->mf_bits))
        ether_type = ETHER_TYPE_VLAN;
      else
        ether_type = ETHER_TYPE_QINQ;
      STORE_RT_REG(p_hwfn, PRS_REG_TAG_ETHERTYPE_0_RT_OFFSET, ether_type);
      STORE_RT_REG(p_hwfn, NIG_REG_TAG_ETHERTYPE_0_RT_OFFSET, ether_type);
      STORE_RT_REG(p_hwfn, PBF_REG_TAG_ETHERTYPE_0_RT_OFFSET, ether_type);
      STORE_RT_REG(p_hwfn, DORQ_REG_TAG1_ETHERTYPE_RT_OFFSET, ether_type);
    }
    ...
    rc = ecore_mcp_load_req(p_hwfn, p_hwfn->p_main_ptt, &load_req_params);
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_hwfn, false, "Failed sending a LOAD_REQ command\n");
      return rc;
    }

    load_code = load_req_params.load_code;
    DP_VERBOSE(p_hwfn, ECORE_MSG_SP, "Load request was sent. Load code: 0x%x\n",
               load_code);
    ...
    p_hwfn->first_on_engine = (load_code == FW_MSG_CODE_DRV_LOAD_ENGINE);
    ...
    if (load_code != FW_MSG_CODE_DRV_LOAD_ENGINE) {
      rc = ecore_final_cleanup(p_hwfn, p_hwfn->p_main_ptt, p_hwfn->rel_pf_id,
                               false);
      if (rc != ECORE_SUCCESS) {
    ...
    rc = ecore_pglueb_set_pfid_enable(p_hwfn, p_hwfn->p_main_ptt, true);
    if (rc != ECORE_SUCCESS)
      goto load_err;
    ...
    p_hwfn->fw_overlay_mem =
        ecore_fw_overlay_mem_alloc(p_hwfn, fw_overlays, fw_overlays_len);
    if (!p_hwfn->fw_overlay_mem) {
      DP_NOTICE(p_hwfn, false, "Failed to allocate fw overlay memory\n");
      goto load_err;
    }
    ...
    case FW_MSG_CODE_DRV_LOAD_FUNCTION:
      rc = ecore_hw_init_pf(p_hwfn, p_hwfn->p_main_ptt, p_hwfn->hw_info.hw_mode,
                            p_params);
      break;
    default:
      DP_NOTICE(p_hwfn, false, "Unexpected load code [0x%08x]", load_code);
      rc = ECORE_NOTIMPL;
      break;
    }

    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_hwfn, false, "init phase failed for loadcode 0x%x (rc %d)\n",
                load_code, rc);
      goto load_err;
    }
    ...
    rc = ecore_mcp_cmd(p_hwfn, p_hwfn->p_main_ptt, DRV_MSG_CODE_SET_DCBX,
                       1 << DRV_MB_PARAM_DCBX_NOTIFY_OFFSET, &resp, &param);
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_hwfn, false, "Failed to send DCBX attention request\n");
      return rc;
    }
  ...
  if (IS_PF(p_dev)) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_SPQ,
               "Sending GET_OEM_UPDATES command to trigger stag/bandwidth "
               "attention handling\n");
    rc = ecore_mcp_cmd(p_hwfn, p_hwfn->p_main_ptt, DRV_MSG_CODE_GET_OEM_UPDATES,
                       1 << DRV_MB_PARAM_DUMMY_OEM_UPDATES_OFFSET, &resp,
                       &param);
    if (rc != ECORE_SUCCESS)
      DP_NOTICE(p_hwfn, false,
  ...
  if (IS_PF(p_dev)) {
    ...
    rc = ecore_mcp_cmd(p_hwfn, p_hwfn->p_main_ptt,
                       DRV_MSG_CODE_OV_UPDATE_STORM_FW_VER, drv_mb_param, &resp,
                       &param);
    if (rc != ECORE_SUCCESS)
    ...
    rc = ecore_mcp_ov_update_driver_state(p_hwfn, p_hwfn->p_main_ptt,
...
static void ecore_hw_timers_stop(struct ecore_dev *p_dev,
  ...
  for (i = 0; i < ECORE_HW_STOP_RETRY_LIMIT && !p_dev->recov_in_prog; i++) {
    if ((!ecore_rd(p_hwfn, p_ptt, TM_REG_PF_SCAN_ACTIVE_CONN)) &&
...
static enum _ecore_status_t ecore_verify_reg_val(struct ecore_hwfn *p_hwfn,
  ...
  if (val != expected_val) {
    DP_NOTICE(p_hwfn, true,
              "Value at address 0x%08x is 0x%08x while the expected value is "
              "0x%08x\n",
...
enum _ecore_status_t ecore_hw_stop(struct ecore_dev *p_dev) {
  ...
  for_each_hwfn(p_dev, j) {
    ...
    if (IS_VF(p_dev)) {
      ...
      if (rc != ECORE_SUCCESS) {
        DP_NOTICE(p_hwfn, true, "ecore_vf_pf_reset failed. rc = %d.\n", rc);
    ...
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_hwfn, false,
                "Failed to close PF against FW [rc = %d]. Continue to stop HW "
                "to prevent illegal host access by the device.\n",
                rc);
      rc2 = ECORE_UNKNOWN_ERROR;
    }
    ...
    ecore_wr(p_hwfn, p_ptt, NIG_REG_RX_LLH_BRB_GATE_DNTFWD_PERPF, 0x1);
    ...
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_hwfn, true, "Failed to return IGU CAM to default\n");
      rc2 = ECORE_UNKNOWN_ERROR;
    }
    ...
    if (IS_LEAD_HWFN(p_hwfn) &&
        OSAL_GET_BIT(ECORE_MF_LLH_MAC_CLSS, &p_dev->mf_bits) &&
        !ECORE_IS_FCOE_PERSONALITY(p_hwfn))
      ecore_llh_remove_mac_filter(p_dev, 0, p_hwfn->hw_info.hw_mac_addr);

    if (!p_dev->recov_in_prog) {
      ecore_verify_reg_val(p_hwfn, p_ptt, QM_REG_USG_CNT_PF_TX, 0);
      ecore_verify_reg_val(p_hwfn, p_ptt, QM_REG_USG_CNT_PF_OTHER, 0);
    ...
    if (!p_dev->recov_in_prog) {
      ...
      if (rc == ECORE_NOMEM) {
        DP_NOTICE(p_hwfn, false,
                  "Failed sending an UNLOAD_DONE command due to a memory "
                  "allocation failure. Resending.\n");
        rc = ecore_mcp_unload_done(p_hwfn, p_ptt);
      }
      if (rc != ECORE_SUCCESS) {
        DP_NOTICE(p_hwfn, false,
  ...
  if (IS_PF(p_dev) && !p_dev->recov_in_prog) {
     ...
     * hw-functions.
     */
    rc = ecore_pglueb_set_pfid_enable(p_hwfn, p_hwfn->p_main_ptt, false);
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_hwfn, true,
...
enum _ecore_status_t ecore_hw_start_fastpath(struct ecore_hwfn *p_hwfn) {
  ...
  ecore_wr(p_hwfn, p_ptt, NIG_REG_RX_LLH_BRB_GATE_DNTFWD_PERPF, 0x0);
  ecore_ptt_release(p_hwfn, p_ptt);
...
static void ecore_hw_hwfn_prepare(struct ecore_hwfn *p_hwfn) {
  ...
  if (ECORE_IS_AH(p_hwfn->p_dev)) {
    ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PGLUE_B_REG_PGL_ADDR_E8_F0_K2, 0);
    ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PGLUE_B_REG_PGL_ADDR_EC_F0_K2, 0);
    ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PGLUE_B_REG_PGL_ADDR_F0_F0_K2, 0);
    ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PGLUE_B_REG_PGL_ADDR_F4_F0_K2, 0);
  } else {
    ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PGLUE_B_REG_PGL_ADDR_88_F0_BB, 0);
    ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PGLUE_B_REG_PGL_ADDR_8C_F0_BB, 0);
    ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PGLUE_B_REG_PGL_ADDR_90_F0_BB, 0);
    ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PGLUE_B_REG_PGL_ADDR_94_F0_BB, 0);
  }
...
static void get_function_id(struct ecore_hwfn *p_hwfn) {
  ...
  p_hwfn->rel_pf_id =
      GET_FIELD(p_hwfn->hw_info.concrete_fid, PXP_CONCRETE_FID_PFID);
  p_hwfn->port_id =
      GET_FIELD(p_hwfn->hw_info.concrete_fid, PXP_CONCRETE_FID_PORT);
...
static void ecore_hw_set_feat(struct ecore_hwfn *p_hwfn) {
  ...
  if (ECORE_IS_L2_PERSONALITY(p_hwfn)) {
    ...
    feat_num[ECORE_VF_L2_QUE] =
        OSAL_MIN_T(u32, RESC_NUM(p_hwfn, ECORE_L2_QUEUE), sb_cnt.iov_cnt);
    feat_num[ECORE_PF_L2_QUE] = OSAL_MIN_T(
        u32, sb_cnt.cnt - non_l2_sbs,
        RESC_NUM(p_hwfn, ECORE_L2_QUEUE) - FEAT_NUM(p_hwfn, ECORE_VF_L2_QUE));
  }
  ...
  if (ECORE_IS_FCOE_PERSONALITY(p_hwfn) || ECORE_IS_ISCSI_PERSONALITY(p_hwfn)) {
    ...
    if (p_hwfn->p_dev->b_is_target)
      limit *= 2;
    *p_storage_feat = OSAL_MIN_T(u32, limit, RESC_NUM(p_hwfn, ECORE_CMDQS_CQS));
     ...
     */
    *p_storage_feat =
        OSAL_MIN_T(u32, *p_storage_feat, (NUM_OF_GLOBAL_QUEUES / 2));
  }
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_PROBE,
             ...
             (int)FEAT_NUM(p_hwfn, ECORE_ISCSI_CQ), (int)sb_cnt.cnt);
}
...
static enum _ecore_status_t __ecore_hw_set_soft_resc_size(
    struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
    enum ecore_resources res_id, u32 resc_max_val, u32 *p_mcp_resp) {
  enum _ecore_status_t rc;

  rc = ecore_mcp_set_resc_max_val(p_hwfn, p_ptt, res_id, resc_max_val,
                                  p_mcp_resp);
  if (rc != ECORE_SUCCESS) {
    DP_NOTICE(
        p_hwfn, false,
...
ecore_hw_set_soft_resc_size(struct ecore_hwfn *p_hwfn,
  ...
  for (res_id = 0; res_id < ECORE_MAX_RESC; res_id++) {
    ...
    rc = __ecore_hw_set_soft_resc_size(p_hwfn, p_ptt, res_id, resc_max_val,
                                       &mcp_resp);
    if (rc != ECORE_SUCCESS)
      return rc;
...
static enum _ecore_status_t ecore_hw_get_dflt_resc(struct ecore_hwfn *p_hwfn,
...
__ecore_hw_set_resc_info(struct ecore_hwfn *p_hwfn, enum ecore_resources res_id,
  ...
  rc = ecore_hw_get_dflt_resc(p_hwfn, res_id, &dflt_resc_num, &dflt_resc_start);
  if (rc != ECORE_SUCCESS) {
  ...
  rc = ecore_mcp_get_resc_info(p_hwfn, p_hwfn->p_main_ptt, res_id, &mcp_resp,
                               p_resc_num, p_resc_start);
  if (rc != ECORE_SUCCESS) {
    DP_NOTICE(p_hwfn, true,
  ...
  if (mcp_resp != FW_MSG_CODE_RESOURCE_ALLOC_OK) {
    DP_INFO(p_hwfn,
            ...
            " [%d,%d].\n",
            res_id, ecore_hw_get_resc_name(res_id), mcp_resp, dflt_resc_num,
            dflt_resc_start);
  ...
  if ((*p_resc_num != dflt_resc_num || *p_resc_start != dflt_resc_start) &&
      res_id != ECORE_SB) {
    DP_INFO(p_hwfn,
            "MFW allocation for resource %d [%s] differs from default values "
            "[%d,%d vs. %d,%d]%s\n",
            res_id, ecore_hw_get_resc_name(res_id), *p_resc_num, *p_resc_start,
...
static enum _ecore_status_t ecore_hw_get_ppfid_bitmap(struct ecore_hwfn *p_hwfn,
  ...
  if (ecore_device_num_ports(p_dev) == 4) {
    ...
    if (ECORE_IS_BB(p_dev))
      mask = MFW_PORT(p_hwfn) > 1 ? ECORE_NONUSED_PPFID_MASK_BB_4P_HI_PORTS
...
static enum _ecore_status_t ecore_hw_get_resc(struct ecore_hwfn *p_hwfn,
  ...
  if (rc != ECORE_SUCCESS && rc != ECORE_NOTIMPL) {
    ...
    DP_INFO(p_hwfn, "Skip the max values setting of the soft resources since "
                    "the resource lock is not supported by the MFW\n");
  } else if (rc == ECORE_SUCCESS && !resc_lock_params.b_granted) {
    DP_NOTICE(p_hwfn, false,
              "Failed to acquire the resource lock for the resource allocation "
              "commands\n");
    rc = ECORE_BUSY;
    ...
    if (rc != ECORE_SUCCESS && rc != ECORE_NOTIMPL) {
      ...
      DP_INFO(p_hwfn, "Skip the max values setting of the soft resources since "
                      "it is not supported by the MFW\n");
      rc = ecore_mcp_resc_unlock(p_hwfn, p_ptt, &resc_unlock_params);
      if (rc != ECORE_SUCCESS)
        DP_INFO(p_hwfn, "Failed to release the resource lock for the resource "
                        "allocation commands\n");
    }
  }
  ...
  if (CHIP_REV_IS_EMUL(p_dev)) {
    ...
    if (!(p_dev->b_is_emul_full)) {
      ...
      resc_start[ECORE_PQ] = resc_num[ECORE_PQ] * p_hwfn->enabled_func_idx;
    }
    ...
    if (ECORE_IS_AH(p_dev) && p_dev->b_is_emul_full) {
      if (!p_hwfn->rel_pf_id) {
        resc_num[ECORE_ILT] =
            OSAL_MAX_T(u32, resc_num[ECORE_ILT], roce_min_ilt_lines);
      } else if (resc_num[ECORE_ILT] < roce_min_ilt_lines) {
        resc_start[ECORE_ILT] += roce_min_ilt_lines - resc_num[ECORE_ILT];
      }
    }
  }
#endif
  ...
  if (RESC_END(p_hwfn, ECORE_ILT) > max_ilt_lines) {
    DP_NOTICE(p_hwfn, true, "Can't assign ILT pages [%08x,...,%08x]\n",
              RESC_START(p_hwfn, ECORE_ILT), RESC_END(p_hwfn, ECORE_ILT) - 1);
    return ECORE_INVAL;
  }
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_PROBE, "The numbers for each resource are:\n");
  for (res_id = 0; res_id < ECORE_MAX_RESC; res_id++)
    DP_VERBOSE(p_hwfn, ECORE_MSG_PROBE, "%s = %d start = %d\n",
               ecore_hw_get_resc_name(res_id), RESC_NUM(p_hwfn, res_id),
...
unlock_and_exit:
  if (resc_lock_params.b_granted && !resc_unlock_params.b_released)
    ecore_mcp_resc_unlock(p_hwfn, p_ptt, &resc_unlock_params);
  return rc;
}
...
ecore_emul_hw_get_nvm_info(struct ecore_hwfn *p_hwfn) {
  if (IS_LEAD_HWFN(p_hwfn)) {
    ...
    p_dev->mf_bits = 1 << ECORE_MF_LLH_MAC_CLSS | 1 << ECORE_MF_LLH_PROTO_CLSS |
                     1 << ECORE_MF_LL2_NON_UNICAST;
    if (p_hwfn->num_funcs_on_port > 1)
      p_dev->mf_bits |=
          1 << ECORE_MF_INTER_PF_SWITCH | 1 << ECORE_MF_DISABLE_ARFS;
    else
...
static enum _ecore_status_t
ecore_hw_get_nvm_info(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  addr = MCP_REG_SCRATCH + nvm_cfg1_offset + OFFSETOF(struct nvm_cfg1, glob) +
  ...
  dcbx_mode =
      ecore_rd(p_hwfn, p_ptt,
               port_cfg_addr + OFFSETOF(struct nvm_cfg1_port, generic_cont0));
  ...
  link_temp =
      ecore_rd(p_hwfn, p_ptt,
               port_cfg_addr + OFFSETOF(struct nvm_cfg1_port, speed_cap_mask));
  ...
  link_temp =
      ecore_rd(p_hwfn, p_ptt,
               port_cfg_addr + OFFSETOF(struct nvm_cfg1_port, link_settings));
  ...
  link->pause.autoneg = !!(link_temp & NVM_CFG1_PORT_DRV_FLOW_CONTROL_AUTONEG);
  link->pause.forced_rx = !!(link_temp & NVM_CFG1_PORT_DRV_FLOW_CONTROL_RX);
  link->pause.forced_tx = !!(link_temp & NVM_CFG1_PORT_DRV_FLOW_CONTROL_TX);
  link->loopback_mode = 0;

  if (p_hwfn->mcp_info->capabilities & FW_MB_PARAM_FEATURE_SUPPORT_EEE) {
    link_temp = ecore_rd(
        p_hwfn, p_ptt, port_cfg_addr + OFFSETOF(struct nvm_cfg1_port, ext_phy));
    ...
    case NVM_CFG1_PORT_EEE_POWER_SAVING_MODE_AGGRESSIVE:
      p_caps->eee_lpi_timer = EEE_TX_TIMER_USEC_AGGRESSIVE_TIME;
      break;
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_LINK,
             "Read default link: Speed 0x%08x, Adv. Speed 0x%08x, AN: 0x%02x, "
             "PAUSE AN: 0x%02x\n EEE: %02x [%08x usec]",
             link->speed.forced_speed, link->speed.advertised_speeds,
             link->speed.autoneg, link->pause.autoneg, p_caps->default_eee,
  ...
  case NVM_CFG1_GLOB_MF_MODE_BD:
    p_hwfn->p_dev->mf_bits =
        1 << ECORE_MF_OVLAN_CLSS | 1 << ECORE_MF_LLH_PROTO_CLSS |
        1 << ECORE_MF_8021AD_TAGGING | 1 << ECORE_MF_FIP_SPECIAL;
    break;
  case NVM_CFG1_GLOB_MF_MODE_NPAR1_0:
    p_hwfn->p_dev->mf_bits =
        1 << ECORE_MF_LLH_MAC_CLSS | 1 << ECORE_MF_LLH_PROTO_CLSS |
        1 << ECORE_MF_LL2_NON_UNICAST | 1 << ECORE_MF_INTER_PF_SWITCH |
  ...
  if (device_capabilities & NVM_CFG1_GLOB_DEVICE_CAPABILITIES_ETHERNET)
    OSAL_SET_BIT(ECORE_DEV_CAP_ETH, &p_hwfn->hw_info.device_capabilities);
  if (device_capabilities & NVM_CFG1_GLOB_DEVICE_CAPABILITIES_FCOE)
    OSAL_SET_BIT(ECORE_DEV_CAP_FCOE, &p_hwfn->hw_info.device_capabilities);
  if (device_capabilities & NVM_CFG1_GLOB_DEVICE_CAPABILITIES_ISCSI)
    OSAL_SET_BIT(ECORE_DEV_CAP_ISCSI, &p_hwfn->hw_info.device_capabilities);
  if (device_capabilities & NVM_CFG1_GLOB_DEVICE_CAPABILITIES_ROCE)
    OSAL_SET_BIT(ECORE_DEV_CAP_ROCE, &p_hwfn->hw_info.device_capabilities);
  if (device_capabilities & NVM_CFG1_GLOB_DEVICE_CAPABILITIES_IWARP)
    OSAL_SET_BIT(ECORE_DEV_CAP_IWARP, &p_hwfn->hw_info.device_capabilities);
...
static void ecore_get_num_funcs(struct ecore_hwfn *p_hwfn,
  ...
  if (ECORE_IS_BB(p_dev) || ECORE_IS_AH(p_dev)) {
    reg_function_hide = ecore_rd(p_hwfn, p_ptt, MISCS_REG_FUNCTION_HIDE_BB_K2);
  } else { /* E5 */
    reg_function_hide = 0;
  }
  ...
  if (CHIP_REV_IS_FPGA(p_dev)) {
    DP_NOTICE(p_hwfn, false,
              "FPGA: Limit number of PFs to 4 [would affect resource "
              "allocation, needed for IOV]\n");
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_PROBE,
             ...
             p_hwfn->rel_pf_id, p_hwfn->abs_pf_id, p_hwfn->enabled_func_idx,
...
static void ecore_emul_hw_info_port_num(struct ecore_hwfn *p_hwfn,
  ...
  p_dev->num_ports =
      p_dev->num_ports_in_engine * ecore_device_num_engines(p_dev);
}
#endif
...
static void ecore_hw_info_port_num(struct ecore_hwfn *p_hwfn,
  ...
  addr = SECTION_OFFSIZE_ADDR(p_hwfn->mcp_info->public_base, PUBLIC_GLOBAL);
...
static void ecore_mcp_get_eee_caps(struct ecore_hwfn *p_hwfn,
  ...
  eee_status = ecore_rd(p_hwfn, p_ptt,
                        p_hwfn->mcp_info->port_addr +
                            OFFSETOF(struct public_port, eee_status));
  eee_status =
      (eee_status & EEE_SUPPORTED_SPEED_MASK) >> EEE_SUPPORTED_SPEED_OFFSET;
...
ecore_get_hw_info(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  if (IS_LEAD_HWFN(p_hwfn) && !IS_ECORE_PACING(p_hwfn)) {
    ...
    if (rc != ECORE_SUCCESS) {
      if (p_params->b_relaxed_probe)
        p_params->p_relaxed_res = ECORE_HW_PREPARE_BAD_IOV;
      else
        return rc;
    }
  }
...
#endif
    OSAL_MEMCPY(p_hwfn->hw_info.hw_mac_addr, p_hwfn->mcp_info->func_info.mac,
                ETH_ALEN);
#ifndef ASIC_ONLY
  } else {
...
void ecore_get_dev_name(struct ecore_dev *p_dev, u8 *name, u8 max_chars) {
  ...
  OSAL_SNPRINTF((char *)name, n, "%s %c%d", ECORE_IS_BB(p_dev) ? "BB" : "AH",
...
static enum _ecore_status_t ecore_get_dev_info(struct ecore_hwfn *p_hwfn,
  ...
  OSAL_PCI_READ_CONFIG_WORD(p_dev, RTE_PCI_VENDOR_ID, &p_dev->vendor_id);
  OSAL_PCI_READ_CONFIG_WORD(p_dev, RTE_PCI_DEVICE_ID, &p_dev->device_id);
  ...
  DP_INFO(
      ...
      ECORE_IS_BB(p_dev) ? "BB" : "AH", 'A' + p_dev->chip_rev,
      (int)p_dev->chip_metal, p_dev->chip_num, p_dev->chip_rev,
      p_dev->chip_bond_id, p_dev->chip_metal);
  ...
  if (CHIP_REV_IS_EMUL(p_dev)) {
    ...
    DP_NOTICE(p_hwfn, false, "Emulation: Running on a %s build %s MAC\n",
...
ecore_hw_prepare_single(struct ecore_hwfn *p_hwfn, void OSAL_IOMEM *p_regview,
  ...
  if (REG_RD(p_hwfn, PXP_PF_ME_OPAQUE_ADDR) == 0xffffffff) {
    DP_ERR(p_hwfn, "Reading the ME register returns all Fs; Preventing further "
                   "chip access\n");
    if (p_params->b_relaxed_probe)
  ...
  if (IS_LEAD_HWFN(p_hwfn)) {
    ...
    if (rc != ECORE_SUCCESS) {
      if (p_params->b_relaxed_probe)
        p_params->p_relaxed_res = ECORE_HW_PREPARE_FAILED_DEV;
      goto err1;
    }
  }
  ...
  if (CHIP_REV_IS_SLOW(p_hwfn->p_dev) && !b_ptt_gtt_init) {
    ...
    if (val != 1) {
      DP_ERR(p_hwfn, "PTT and GTT init in PGLUE_B didn't complete\n");
      goto err1;
    }
  ...
  rc = ecore_get_hw_info(p_hwfn, p_hwfn->p_main_ptt, p_params->personality,
                         p_params);
  if (rc) {
  ...
  if (IS_LEAD_HWFN(p_hwfn)) {
    rc = ecore_mcp_mdump_get_info(p_hwfn, p_hwfn->p_main_ptt, &mdump_info);
    if (rc == ECORE_SUCCESS && mdump_info.num_of_logs)
      DP_NOTICE(p_hwfn, false,
                "* * * IMPORTANT - HW ERROR register dump captured by device * "
                "* *\n");

    rc = ecore_mcp_mdump_get_retain(p_hwfn, p_hwfn->p_main_ptt, &mdump_retain);
    if (rc == ECORE_SUCCESS && mdump_retain.valid)
      DP_NOTICE(p_hwfn, false,
                "mdump retained data: epoch 0x%08x, pf 0x%x, status 0x%08x\n",
                mdump_retain.epoch, mdump_retain.pf, mdump_retain.status);

    ecore_mcp_mdump_set_values(p_hwfn, p_hwfn->p_main_ptt, p_params->epoch);
  }
  ...
  if (CHIP_REV_IS_FPGA(p_dev)) {
    if (ECORE_IS_AH(p_dev)) {
      DP_NOTICE(p_hwfn, false, "FPGA: workaround; Prevent DMAE parities\n");
      ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PCIE_REG_PRTY_MASK_K2, 7);
    }

    DP_NOTICE(p_hwfn, false, "FPGA: workaround: Set VF bar0 size\n");
    ecore_wr(p_hwfn, p_hwfn->p_main_ptt, PGLUE_B_REG_VF_BAR0_SIZE_K2, 4);
  }
#endif
...
enum _ecore_status_t
ecore_hw_prepare(struct ecore_dev *p_dev,
                 struct ecore_hw_prepare_params *p_params) {
  struct ecore_hwfn *p_hwfn = ECORE_LEADING_HWFN(p_dev);
  enum _ecore_status_t rc;
  ...
  rc = ecore_hw_prepare_single(p_hwfn, p_dev->regview, p_dev->doorbells,
                               p_dev->db_phys_addr, p_params);
  if (rc != ECORE_SUCCESS)
    return rc;
  ...
  if (ECORE_IS_CMT(p_dev)) {
    ...
    offset = ecore_hw_bar_size(p_hwfn, p_hwfn->p_main_ptt, BAR_ID_0) / 2;
    addr = (u8 OSAL_IOMEM *)p_dev->regview + offset;
    p_regview = (void OSAL_IOMEM *)addr;

    offset = ecore_hw_bar_size(p_hwfn, p_hwfn->p_main_ptt, BAR_ID_1) / 2;
    ...
    rc = ecore_hw_prepare_single(&p_dev->hwfns[1], p_regview, p_doorbell,
                                 db_phys_addr, p_params);
    ...
    if (rc != ECORE_SUCCESS) {
      if (p_params->b_relaxed_probe)
        p_params->p_relaxed_res = ECORE_HW_PREPARE_FAILED_ENG2;
...
static void ecore_chain_free_next_ptr(struct ecore_dev *p_dev,
  ...
  for (i = 0; i < p_chain->page_cnt; i++) {
    ...
    OSAL_DMA_FREE_COHERENT(p_dev, p_virt, p_phys, ECORE_CHAIN_PAGE_SIZE);
...
static void ecore_chain_free_single(struct ecore_dev *p_dev,
  ...
  OSAL_DMA_FREE_COHERENT(p_dev, p_chain->p_virt_addr, p_chain->p_phys_addr,
...
static void ecore_chain_free_pbl(struct ecore_dev *p_dev,
  ...
  for (i = 0; i < page_cnt; i++) {
    ...
    OSAL_DMA_FREE_COHERENT(p_dev, pp_virt_addr_tbl[i],
                           *(dma_addr_t *)p_pbl_virt, ECORE_CHAIN_PAGE_SIZE);
...
ecore_chain_alloc_sanity_check(struct ecore_dev *p_dev,
  ...
  if ((cnt_type == ECORE_CHAIN_CNT_TYPE_U16 &&
      ...
      (cnt_type == ECORE_CHAIN_CNT_TYPE_U32 && chain_size > ECORE_U32_MAX)) {
    DP_NOTICE(p_dev, true,
              "The actual chain size (0x%lx) is larger than the maximal "
              "possible value\n",
...
static enum _ecore_status_t
ecore_chain_alloc_next_ptr(struct ecore_dev *p_dev,
                           struct ecore_chain *p_chain) {
  ...
  for (i = 0; i < p_chain->page_cnt; i++) {
    ...
    if (i == 0) {
      ...
      ecore_chain_init_next_ptr_elem(p_chain, p_virt_prev, p_virt, p_phys);
    }
   ...
   * chain.
   */
  ecore_chain_init_next_ptr_elem(p_chain, p_virt_prev, p_chain->p_virt_addr,
                                 p_chain->p_phys_addr);
...
static enum _ecore_status_t
ecore_chain_alloc_pbl(struct ecore_dev *p_dev, struct ecore_chain *p_chain,
  ...
  ecore_chain_init_pbl_mem(p_chain, p_pbl_virt, p_pbl_phys, pp_virt_addr_tbl);
  if (!p_pbl_virt) {
...
enum _ecore_status_t ecore_chain_alloc(struct ecore_dev *p_dev,
  ...
  rc = ecore_chain_alloc_sanity_check(p_dev, cnt_type, elem_size, page_cnt);
  if (rc) {
    DP_NOTICE(
        p_dev, false,
  ...
  ecore_chain_init_params(p_chain, page_cnt, (u8)elem_size, intended_use, mode,
                          cnt_type, p_dev->dp_ctx);
...
ecore_llh_set_function_as_default(struct ecore_hwfn *p_hwfn,
  ...
  if (OSAL_GET_BIT(ECORE_MF_NEED_DEF_PF, &p_hwfn->p_dev->mf_bits)) {
    ecore_wr(p_hwfn, p_ptt, NIG_REG_LLH_TAGMAC_DEF_PF_VECTOR,
             1 << p_hwfn->abs_pf_id / 2);
    ecore_wr(p_hwfn, p_ptt, PRS_REG_MSG_INFO, 0);
    return ECORE_SUCCESS;
  }

  DP_NOTICE(p_hwfn, false, "This function can't be set as default\n");
  return ECORE_INVAL;
}
...
static enum _ecore_status_t ecore_set_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  if (p_hwfn->p_dev->int_coalescing_mode != ECORE_COAL_MODE_ENABLE) {
    DP_NOTICE(p_hwfn, true, "Coalescing configuration not enabled\n");
    return ECORE_INVAL;
  }
...
enum _ecore_status_t ecore_set_queue_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  if (IS_VF(p_hwfn->p_dev))
    return ecore_vf_pf_set_coalesce(p_hwfn, rx_coal, tx_coal, p_cid);
...
enum _ecore_status_t ecore_set_rxq_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_int_set_timer_res(p_hwfn, p_ptt, timer_res, p_cid->sb_igu_id,
                               false);
  if (rc != ECORE_SUCCESS)
    goto out;

  address =
      BAR0_MAP_REG_USDM_RAM + USTORM_ETH_QUEUE_ZONE_OFFSET(p_cid->abs.queue_id);
...
enum _ecore_status_t ecore_set_txq_coalesce(struct ecore_hwfn *p_hwfn,
  ...
  rc =
      ecore_int_set_timer_res(p_hwfn, p_ptt, timer_res, p_cid->sb_igu_id, true);
  if (rc != ECORE_SUCCESS)
    goto out;
...
static void ecore_configure_wfq_for_all_vports(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < p_hwfn->qm_info.num_vports; i++) {
    ...
    vport_params[i].wfq = (wfq_speed * ECORE_WFQ_UNIT) / min_pf_rate;
    ecore_init_vport_wfq(p_hwfn, p_ptt, vport_params[i].first_tx_pq_id,
                         vport_params[i].wfq);
  }
}
...
static enum _ecore_status_t ecore_init_wfq_param(struct ecore_hwfn *p_hwfn,
  ...
  if (req_rate < min_pf_rate / ECORE_WFQ_UNIT) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_LINK,
               "Vport [%d] - Requested rate[%d Mbps] is less than one percent "
               "of configured PF min rate[%d Mbps]\n",
  ...
  if (num_vports > ECORE_WFQ_UNIT) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_LINK, "Number of vports is greater than %d\n",
               ECORE_WFQ_UNIT);
    return ECORE_INVAL;
  }
  ...
  if (total_req_min_rate > min_pf_rate) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_LINK,
               "Total requested min rate for all vports[%d Mbps] is greater "
  ...
  if (left_rate_per_vp < min_pf_rate / ECORE_WFQ_UNIT) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_LINK,
               "Non WFQ configured vports rate [%d Mbps] is less than one "
...
static int __ecore_configure_vport_wfq(struct ecore_hwfn *p_hwfn,
                                       struct ecore_ptt *p_ptt, u16 vp_id,
                                       u32 rate) {
  ...
  if (rc == ECORE_SUCCESS)
    ecore_configure_wfq_for_all_vports(p_hwfn, p_ptt, p_link->min_pf_rate);
  else
    DP_NOTICE(p_hwfn, false, "Validation failed while configuring min rate\n");
...
void ecore_configure_vp_wfq_on_link_change(struct ecore_dev *p_dev,
  ...
  for_each_hwfn(p_dev, i) {
    ...
    __ecore_configure_vp_wfq_on_link_change(p_hwfn, p_ptt, min_pf_rate);
  }
}
...
int __ecore_configure_pf_max_bandwidth(struct ecore_hwfn *p_hwfn,
  ...
  rc =
      ecore_init_pf_rl(p_hwfn, p_ptt, p_hwfn->rel_pf_id, p_hwfn->qm_info.pf_rl);
...
int ecore_configure_pf_max_bandwidth(struct ecore_dev *p_dev, u8 max_bw) {
  ...
  for_each_hwfn(p_dev, i) {
    ...
    rc = __ecore_configure_pf_max_bandwidth(p_hwfn, p_ptt, p_link, max_bw);
...
int ecore_configure_pf_min_bandwidth(struct ecore_dev *p_dev, u8 min_bw) {
  ...
  for_each_hwfn(p_dev, i) {
    ...
    rc = __ecore_configure_pf_min_bandwidth(p_hwfn, p_ptt, p_link, min_bw);
    if (rc != ECORE_SUCCESS) {
      ecore_ptt_release(p_hwfn, p_ptt);
      return rc;
    }
    ...
    if (p_link->min_pf_rate) {
      ...
      rc = __ecore_configure_vp_wfq_on_link_change(p_hwfn, p_ptt, min_rate);
    }
...
int ecore_device_num_ports(struct ecore_dev *p_dev) { return p_dev->num_ports; }
...
void ecore_set_platform_str(struct ecore_hwfn *p_hwfn, char *buf_str,
                            u32 buf_size) {
  u32 len;

  OSAL_SNPRINTF(buf_str, buf_size, "Ecore %d.%d.%d.%d. ", ECORE_MAJOR_VERSION,
                ECORE_MINOR_VERSION, ECORE_REVISION_VERSION,
                ECORE_ENGINEERING_VERSION);
