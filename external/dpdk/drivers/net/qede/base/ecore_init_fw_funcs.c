...
#define QM_RL_INC_VAL(rate)                                                    \
  OSAL_MAX_T(u32,                                                              \
             (u32)(((rate ? rate : 100000) * QM_RL_PERIOD * 101) / (8 * 100)), \
             1)
...
#define QM_INIT_TX_PQ_MAP(p_hwfn, map, pq_id, vp_pq_id, rl_valid, rl_id, voq,  \
                          wrr)                                                 \
  do {                                                                         \
...
#define PQ_INFO_ELEMENT(vp_pq_id, pf, tc, port, rl_valid, rl_id)               \
  (((vp_pq_id) << 0) | ((pf) << 12) | ((tc) << 16) | ((port) << 20) |          \
   ((rl_valid ? 1 : 0) << 22) | (((rl_id)&255) << 24) | (((rl_id) >> 8) << 9))
...
static void ecore_enable_pf_rl(struct ecore_hwfn *p_hwfn, bool pf_rl_en) {
  ...
  if (pf_rl_en) {
    ...
    STORE_RT_REG(p_hwfn, QM_REG_RLPFVOQENABLE_RT_OFFSET, VOQS_BIT_MASK);
 ...
 * the specified VOQ
 */
static void ecore_cmdq_lines_voq_rt_init(struct ecore_hwfn *p_hwfn, u8 voq,
                                         u16 cmdq_lines) {
...
static void ecore_cmdq_lines_rt_init(
    struct ecore_hwfn *p_hwfn, u8 max_ports_per_engine,
  ...
  for (port_id = 0; port_id < max_ports_per_engine; port_id++) {
    ...
    ecore_cmdq_lines_voq_rt_init(p_hwfn, voq, PBF_CMDQ_PURE_LB_LINES);
  }
}
 ...
 */
static void ecore_btb_blocks_rt_init(
    struct ecore_hwfn *p_hwfn, u8 max_ports_per_engine,
    u8 max_phys_tcs_per_port,
    struct init_qm_port_params port_params[MAX_NUM_PORTS]) {
  ...
  for (port_id = 0; port_id < max_ports_per_engine; port_id++) {
    ...
    /* Init pure LB TC */
    voq = VOQ(port_id, PURE_LB_TC, max_phys_tcs_per_port);
    STORE_RT_REG(p_hwfn, PBF_BTB_GUARANTEED_RT_OFFSET(voq), pure_lb_blocks);
  }
}
 ...
 */
static int
ecore_global_rl_rt_init(struct ecore_hwfn *p_hwfn,
                        struct init_qm_global_rl_params
                            global_rl_params[COMMON_MAX_QM_GLOBAL_RLS]) {
  u32 upper_bound =
      QM_VP_RL_UPPER_BOUND(QM_MAX_LINK_SPEED) | (u32)QM_RL_CRD_REG_SIGN_BIT;
  u32 inc_val;
  u16 rl_id;
  ...
  for (rl_id = 0; rl_id < MAX_QM_GLOBAL_RLS; rl_id++) {
    ...
    STORE_RT_REG(p_hwfn, QM_REG_RLGLBLUPPERBOUND_RT_OFFSET + rl_id,
                 upper_bound);
    STORE_RT_REG(p_hwfn, QM_REG_RLGLBLINCVAL_RT_OFFSET + rl_id, inc_val);
  }
...
static int ecore_tx_pq_map_rt_init(struct ecore_hwfn *p_hwfn,
                                   struct ecore_ptt *p_ptt, u8 pf_id,
                                   u8 max_phys_tcs_per_port, bool is_pf_loading,
                                   u32 num_pf_cids, u32 num_vf_cids,
                                   u16 start_pq, u16 num_pf_pqs, u16 num_vf_pqs,
                                   u16 start_vport, u32 base_mem_addr_4kb,
  ...
  for (i = 0, pq_id = start_pq; i < num_pqs; i++, pq_id++) {
    ...
    voq = VOQ(pq_params[i].port_id, pq_params[i].tc_id, max_phys_tcs_per_port);
    ...
    if (first_tx_pq_id == QM_INVALID_PQ_ID) {
      u32 map_val =
          (voq << QM_WFQ_VP_PQ_VOQ_SHIFT) | (pf_id << QM_WFQ_VP_PQ_PF_SHIFT);
    ...
    QM_INIT_TX_PQ_MAP(p_hwfn, tx_pq_map, pq_id, first_tx_pq_id,
                      pq_params[i].rl_valid, pq_params[i].rl_id, voq,
...
static void ecore_other_pq_map_rt_init(struct ecore_hwfn *p_hwfn, u8 pf_id,
                                       bool is_pf_loading, u32 num_pf_cids,
  ...
  for (i = 0, pq_id = pf_id * QM_PF_QUEUE_GROUP_SIZE; i < QM_OTHER_PQS_PER_PF;
       i++, pq_id++) {
    /* Set PQ base address */
 ...
 * Return -1 on error.
 */
static int ecore_pf_wfq_rt_init(struct ecore_hwfn *p_hwfn, u8 pf_id, u16 pf_wfq,
                                u8 max_phys_tcs_per_port, u16 num_tx_pqs,
                                struct init_qm_pq_params *pq_params) {
 ...
 * Return -1 on error.
 */
static int ecore_vp_wfq_rt_init(struct ecore_hwfn *p_hwfn, u16 num_vports,
                                struct init_qm_vport_params *vport_params) {
...
static bool ecore_send_qm_cmd(struct ecore_hwfn *p_hwfn,
                              struct ecore_ptt *p_ptt, u32 cmd_addr,
                              u32 cmd_data_lsb, u32 cmd_data_msb) {
...
u32 ecore_qm_pf_mem_size(struct ecore_hwfn *p_hwfn, u32 num_pf_cids,
                         u32 num_vf_cids, u32 num_tids, u16 num_pf_pqs,
                         u16 num_vf_pqs) {
...
int ecore_qm_common_rt_init(
    struct ecore_hwfn *p_hwfn, u8 max_ports_per_engine,
    u8 max_phys_tcs_per_port, bool pf_rl_en, bool pf_wfq_en, bool global_rl_en,
    bool vport_wfq_en, struct init_qm_port_params port_params[MAX_NUM_PORTS],
    struct init_qm_global_rl_params
        global_rl_params[COMMON_MAX_QM_GLOBAL_RLS]) {
  u32 mask = 0;
  ...
  ecore_cmdq_lines_rt_init(p_hwfn, max_ports_per_engine, max_phys_tcs_per_port,
                           port_params);

  /* Init BTB blocks in PBF */
  ecore_btb_blocks_rt_init(p_hwfn, max_ports_per_engine, max_phys_tcs_per_port,
                           port_params);
...
int ecore_qm_pf_rt_init(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                        u8 pf_id, u8 max_phys_tcs_per_port, bool is_pf_loading,
                        u32 num_pf_cids, u32 num_vf_cids, u32 num_tids,
                        u16 start_pq, u16 num_pf_pqs, u16 num_vf_pqs,
                        u16 start_vport, u16 num_vports, u16 pf_wfq, u32 pf_rl,
                        struct init_qm_pq_params *pq_params,
                        struct init_qm_vport_params *vport_params) {
  ...
  if (ecore_tx_pq_map_rt_init(p_hwfn, p_ptt, pf_id, max_phys_tcs_per_port,
                              is_pf_loading, num_pf_cids, num_vf_cids, start_pq,
                              num_pf_pqs, num_vf_pqs, start_vport,
...
int ecore_init_pf_wfq(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                      u8 pf_id, u16 pf_wfq) {
  u32 inc_val;
...
int ecore_init_pf_rl(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                     u8 pf_id, u32 pf_rl) {
  u32 inc_val;
...
int ecore_init_vport_wfq(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
...
int ecore_init_global_rl(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
...
int ecore_init_vport_rl(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
...
bool ecore_send_qm_stop_cmd(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                            bool is_release_cmd, bool is_tx_pq, u16 start_pq,
                            u16 num_pqs) {
  ...
  for (pq_id = start_pq; pq_id <= last_pq; pq_id++) {
    ...
    if ((pq_id == last_pq) ||
        (pq_id % QM_STOP_PQ_MASK_WIDTH == (QM_STOP_PQ_MASK_WIDTH - 1))) {
      QM_CMD_SET_FIELD(cmd_arr, QM_STOP_CMD, PAUSE_MASK, pq_mask);
      QM_CMD_SET_FIELD(cmd_arr, QM_STOP_CMD, GROUP_ID,
                       pq_id / QM_STOP_PQ_MASK_WIDTH);
      if (!ecore_send_qm_cmd(p_hwfn, p_ptt, QM_STOP_CMD_ADDR, cmd_arr[0],
                             cmd_arr[1]))
        return false;
      pq_mask = 0;
    }
...
void ecore_init_nig_ets(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  tc_weight_base_addr =
      is_lb ? NIG_REG_LB_ARB_CREDIT_WEIGHT_0 : NIG_REG_TX_ARB_CREDIT_WEIGHT_0;
  tc_weight_addr_diff =
      is_lb ? NIG_REG_LB_ARB_CREDIT_WEIGHT_1 - NIG_REG_LB_ARB_CREDIT_WEIGHT_0
  ...
  for (tc = 0; tc < num_tc; tc++, tc_client_offset++) {
    ...
    /* Write WFQ weight */
    ecore_wr(p_hwfn, p_ptt,
             tc_weight_base_addr + tc_weight_addr_diff * tc_client_offset,
             byte_weight);

    /* Write WFQ upper bound */
    ecore_wr(p_hwfn, p_ptt,
             tc_bound_base_addr + tc_bound_addr_diff * tc_client_offset,
             NIG_ETS_UP_BOUND(byte_weight, req->mtu));
  }
}

void ecore_init_nig_lb_rl(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  for (tc = 0, reg_offset = 0; tc < NUM_OF_PHYS_TCS; tc++, reg_offset += 4) {
    ...
    /* Configure */
    ecore_wr(p_hwfn, p_ptt, NIG_REG_LB_TCRATELIMIT_INC_PERIOD_0 + reg_offset,
             NIG_RL_PERIOD_CLK_25M);
    inc_val = NIG_RL_INC_VAL(req->tc_rate[tc]);
    ecore_wr(p_hwfn, p_ptt, NIG_REG_LB_TCRATELIMIT_INC_VALUE_0 + reg_offset,
             inc_val);
    ecore_wr(p_hwfn, p_ptt, NIG_REG_LB_TCRATELIMIT_MAX_VALUE_0 + reg_offset,
             NIG_RL_MAX_VAL(inc_val, req->mtu));
...
void ecore_init_prs_ets(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  for (tc = 0; tc < NUM_OF_TCS; tc++) {
    ...
    /* Write WFQ weight */
    ecore_wr(p_hwfn, p_ptt,
             PRS_REG_ETS_ARB_CREDIT_WEIGHT_0 + tc * tc_weight_addr_diff,
             byte_weight);

    /* Write WFQ upper bound */
    ecore_wr(p_hwfn, p_ptt,
             PRS_REG_ETS_ARB_CREDIT_UPPER_BOUND_0 + tc * tc_bound_addr_diff,
...
void ecore_init_brb_ram(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  for (port = 0; port < req->max_ports_per_engine; port++) {
    ...
    for (tc = 0; tc < NUM_OF_TCS; tc++, reg_offset += 4) {
      ...
      ecore_wr(p_hwfn, p_ptt, BRB_REG_TC_GUARANTIED_0 + reg_offset,
               tc_guaranteed_blocks);
      ecore_wr(p_hwfn, p_ptt, BRB_REG_MAIN_TC_GUARANTIED_HYST_0 + reg_offset,
               BRB_HYST_BLOCKS);

      /* Init pause/full thresholds per physical TC - for
       * loopback traffic.
       */
      ecore_wr(p_hwfn, p_ptt, BRB_REG_LB_TC_FULL_XOFF_THRESHOLD_0 + reg_offset,
               full_xoff_th);
      ecore_wr(p_hwfn, p_ptt, BRB_REG_LB_TC_FULL_XON_THRESHOLD_0 + reg_offset,
               full_xon_th);
      ecore_wr(p_hwfn, p_ptt, BRB_REG_LB_TC_PAUSE_XOFF_THRESHOLD_0 + reg_offset,
               pause_xoff_th);
      ecore_wr(p_hwfn, p_ptt, BRB_REG_LB_TC_PAUSE_XON_THRESHOLD_0 + reg_offset,
               pause_xon_th);
       ...
       */
      ecore_wr(p_hwfn, p_ptt,
               BRB_REG_MAIN_TC_FULL_XOFF_THRESHOLD_0 + reg_offset,
               full_xoff_th);
      ecore_wr(p_hwfn, p_ptt, BRB_REG_MAIN_TC_FULL_XON_THRESHOLD_0 + reg_offset,
               full_xon_th);
      ecore_wr(p_hwfn, p_ptt,
               BRB_REG_MAIN_TC_PAUSE_XOFF_THRESHOLD_0 + reg_offset,
               pause_xoff_th);
      ecore_wr(p_hwfn, p_ptt,
               BRB_REG_MAIN_TC_PAUSE_XON_THRESHOLD_0 + reg_offset,
               pause_xon_th);
    }
  }
}
...
#define ARR_REG_WR(dev, ptt, addr, arr, arr_size)                              \
  do {                                                                         \
    ...
    for (i = 0; i < (arr_size); i++)                                           \
      ecore_wr(dev, ptt, ((addr) + (4 * i)), ((u32 *)&(arr))[i]);              \
  } while (0)
 ...
 */
static int ecore_dmae_to_grc(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  read_using_dmae =
      !ecore_dmae_host2grc(p_hwfn, p_ptt, (u64)(osal_uintptr_t)(pData), addr,
                           len_in_dwords, &params);
  if (!read_using_dmae)
    DP_VERBOSE(p_hwfn, ECORE_MSG_DEBUG,
...
void ecore_set_vxlan_enable(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                            bool vxlan_enable) {
  u32 reg_val;
  ...
  SET_TUNNEL_TYPE_ENABLE_BIT(
      reg_val, NIG_REG_ENC_TYPE_ENABLE_VXLAN_ENABLE_SHIFT, vxlan_enable);
...
void ecore_set_gre_enable(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  SET_TUNNEL_TYPE_ENABLE_BIT(
      ...
      eth_gre_enable);
  SET_TUNNEL_TYPE_ENABLE_BIT(
      reg_val, PRS_REG_ENCAPSULATION_TYPE_EN_IP_OVER_GRE_ENABLE_SHIFT,
      ip_gre_enable);
  ecore_wr(p_hwfn, p_ptt, PRS_REG_ENCAPSULATION_TYPE_EN, reg_val);
  if (reg_val) { /* TODO: handle E5 init */
    reg_val = ecore_rd(p_hwfn, p_ptt, PRS_REG_OUTPUT_FORMAT_4_0_BB_K2);
  ...
  SET_TUNNEL_TYPE_ENABLE_BIT(reg_val,
                             ...
                             eth_gre_enable);
  SET_TUNNEL_TYPE_ENABLE_BIT(
      reg_val, NIG_REG_ENC_TYPE_ENABLE_IP_OVER_GRE_ENABLE_SHIFT, ip_gre_enable);
  ecore_wr(p_hwfn, p_ptt, NIG_REG_ENC_TYPE_ENABLE, reg_val);
...
void ecore_set_geneve_enable(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
...
void ecore_gft_disable(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                       u16 pf_id) {
...
void ecore_gft_config(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                      u16 pf_id, bool tcp, bool udp, bool ipv4, bool ipv6,
  ...
  if (!(tcp && udp)) {
    ...
    if (tcp)
      SET_FIELD(cam_line, GFT_CAM_LINE_MAPPED_UPPER_PROTOCOL_TYPE,
...
void ecore_config_vf_zone_size_mode(struct ecore_hwfn *p_hwfn,
  ...
  if (runtime_init) {
    STORE_RT_REG(p_hwfn, PGLUE_REG_B_MSDM_VF_SHIFT_B_RT_OFFSET,
                 msdm_vf_size_log);
    STORE_RT_REG(p_hwfn, PGLUE_REG_B_MSDM_OFFSET_MASK_B_RT_OFFSET,
                 msdm_vf_offset_mask);
  } else {
    ecore_wr(p_hwfn, p_ptt, PGLUE_B_REG_MSDM_VF_SHIFT_B, msdm_vf_size_log);
    ecore_wr(p_hwfn, p_ptt, PGLUE_B_REG_MSDM_OFFSET_MASK_B,
             msdm_vf_offset_mask);
  }
}
...
u32 ecore_get_mstorm_queue_stat_offset(struct ecore_hwfn *p_hwfn,
  ...
  if ((vf_zone_size_mode != VF_ZONE_SIZE_MODE_DEFAULT) &&
    ...
    if (vf_zone_size_mode == VF_ZONE_SIZE_MODE_DOUBLE)
      offset +=
          (1 << MSTORM_VF_ZONE_DEFAULT_SIZE_LOG) * (stat_cnt_id - MAX_NUM_PFS);
...
u32 ecore_get_mstorm_eth_vf_prods_offset(struct ecore_hwfn *p_hwfn, u8 vf_id,
                                         u8 vf_queue_id,
...
void ecore_calc_task_ctx_validation(struct ecore_hwfn *p_hwfn, void *p_ctx_mem,
  ...
  *region1_val_ptr = ecore_calc_cdu_validation_byte(p_hwfn, ctx_type, 1, tid);
}
...
ecore_fw_overlay_mem_alloc(struct ecore_hwfn *p_hwfn,
  ...
  allocated_mem = (struct phys_mem_desc *)OSAL_ZALLOC(
      p_hwfn->p_dev, GFP_KERNEL, NUM_STORMS * sizeof(struct phys_mem_desc));
  if (!allocated_mem)
    return OSAL_NULL;
  ...
  while (buf_offset < buf_size) {
    ...
    storm_mem_desc->virt_addr = OSAL_DMA_ALLOC_COHERENT(
        p_hwfn->p_dev, &storm_mem_desc->phys_addr, storm_mem_desc->size);
