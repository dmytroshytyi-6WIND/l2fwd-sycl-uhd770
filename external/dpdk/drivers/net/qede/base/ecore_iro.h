...
#define PSTORM_CTL_FRAME_ETHTYPE_OFFSET(ethType_id)                            \
  (IRO[34].base + ((ethType_id)*IRO[34].m1))
...
/* Ystorm cqe producer */
#define YSTORM_TOE_CQ_PROD_OFFSET(rss_id) (IRO[39].base + ((rss_id)*IRO[39].m1))
#define YSTORM_TOE_CQ_PROD_SIZE (IRO[39].size)
/* Ustorm cqe producer */
#define USTORM_TOE_CQ_PROD_OFFSET(rss_id) (IRO[40].base + ((rss_id)*IRO[40].m1))
#define USTORM_TOE_CQ_PROD_SIZE (IRO[40].size)
/* Ustorm grq producer */
#define USTORM_TOE_GRQ_PROD_OFFSET(pf_id) (IRO[41].base + ((pf_id)*IRO[41].m1))
...
#define TSTORM_SCSI_BDQ_EXT_PROD_OFFSET(storage_func_id, bdq_id)               \
  (IRO[43].base + ((storage_func_id)*IRO[43].m1) + ((bdq_id)*IRO[43].m2))
...
#define MSTORM_SCSI_BDQ_EXT_PROD_OFFSET(storage_func_id, bdq_id)               \
  (IRO[44].base + ((storage_func_id)*IRO[44].m1) + ((bdq_id)*IRO[44].m2))
...
#define TSTORM_FCOE_RX_STATS_OFFSET(pf_id) (IRO[51].base + ((pf_id)*IRO[51].m1))
#define TSTORM_FCOE_RX_STATS_SIZE (IRO[51].size)
/* Pstorm FCoE TX stats */
#define PSTORM_FCOE_TX_STATS_OFFSET(pf_id) (IRO[52].base + ((pf_id)*IRO[52].m1))
...
#define TSTORM_NVMF_PORT_TASKPOOL_PRODUCER_CONSUMER_OFFSET(port_num_id,        \
                                                           taskpool_index)     \
  (IRO[67].base + ((port_num_id)*IRO[67].m1) + ((taskpool_index)*IRO[67].m2))
...
#define USTORM_NVMF_PORT_COUNTERS_OFFSET(port_num_id)                          \
  (IRO[68].base + ((port_num_id)*IRO[68].m1))
