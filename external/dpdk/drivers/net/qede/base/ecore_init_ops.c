...
void ecore_init_store_rt_reg(struct ecore_hwfn *p_hwfn, u32 rt_offset,
                             u32 val) {
  if (rt_offset >= RUNTIME_ARRAY_SIZE) {
    DP_ERR(p_hwfn,
           "Avoid storing %u in rt_data at index %u since RUNTIME_ARRAY_SIZE "
           "is %u!\n",
...
void ecore_init_store_rt_agg(struct ecore_hwfn *p_hwfn, u32 rt_offset,
                             u32 *p_val, osal_size_t size) {
  osal_size_t i;

  if ((rt_offset + size - 1) >= RUNTIME_ARRAY_SIZE) {
    DP_ERR(p_hwfn,
           "Avoid storing values in rt_data at indices %u-%u since "
           "RUNTIME_ARRAY_SIZE is %u!\n",
...
static enum _ecore_status_t ecore_init_rt(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < size; i++) {
    ...
    rc = ecore_dmae_host2grc(p_hwfn, p_ptt, (osal_uintptr_t)(p_init_val + i),
                             addr + (i << 2), segment,
...
enum _ecore_status_t ecore_init_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  rt_data->b_valid =
      OSAL_ZALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(bool) * RUNTIME_ARRAY_SIZE);
  if (!rt_data->b_valid)
    return ECORE_NOMEM;

  rt_data->init_val =
      OSAL_ZALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(u32) * RUNTIME_ARRAY_SIZE);
  if (!rt_data->init_val) {
...
ecore_init_array_dmae(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                      u32 addr, u32 dmae_data_offset, u32 size,
                      const u32 *p_buf, bool b_must_dmae, bool b_can_dmae) {
  enum _ecore_status_t rc = ECORE_SUCCESS;

  /* Perform DMAE only for lengthy enough sections or for wide-bus */
#ifndef ASIC_ONLY
  if ((CHIP_REV_IS_SLOW(p_hwfn->p_dev) && (size < 16)) || !b_can_dmae ||
      (!b_must_dmae && (size < 16))) {
#else
  if (!b_can_dmae || (!b_must_dmae && (size < 16))) {
    ...
    rc = ecore_dmae_host2grc(p_hwfn, p_ptt,
                             (osal_uintptr_t)(p_buf + dmae_data_offset), addr,
                             size, OSAL_NULL /* default parameters */);
  }
...
static enum _ecore_status_t ecore_init_fill_dmae(struct ecore_hwfn *p_hwfn,
  ...
  return ecore_dmae_host2grc(p_hwfn, p_ptt, (osal_uintptr_t)&zero_buffer[0],
                             addr, fill_count, &params);
}

static void ecore_init_fill(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
...
static enum _ecore_status_t ecore_init_cmd_array(struct ecore_hwfn *p_hwfn,
  ...
  hdr = (union init_array_hdr *)(uintptr_t)(array_data + dmae_array_offset);
  ...
  case INIT_ARR_ZIPPED:
    ...
    output_len =
        OSAL_UNZIP_DATA(p_hwfn, input_len, (u8 *)(uintptr_t)&array_data[offset],
                        max_size, (u8 *)p_hwfn->unzip_buf);
    if (output_len) {
      rc = ecore_init_array_dmae(p_hwfn, p_ptt, addr, 0, output_len,
                                 p_hwfn->unzip_buf, b_must_dmae, b_can_dmae);
    } else {
      DP_NOTICE(p_hwfn, true, "Failed to unzip dmae data\n");
      rc = ECORE_INVAL;
    }
#else
    DP_NOTICE(p_hwfn, true, "Using zipped firmware without config enabled\n");
    rc = ECORE_INVAL;
#endif
    break;
  case INIT_ARR_PATTERN: {
    u32 repeats = GET_FIELD(data, INIT_ARRAY_PATTERN_HDR_REPETITIONS);
    u32 i;

    size = GET_FIELD(data, INIT_ARRAY_PATTERN_HDR_PATTERN_SIZE);

    for (i = 0; i < repeats; i++, addr += size << 2) {
      rc = ecore_init_array_dmae(p_hwfn, p_ptt, addr, dmae_array_offset + 1,
                                 size, array_data, b_must_dmae, b_can_dmae);
      if (rc)
        break;
    }
    break;
  ...
  case INIT_ARR_STANDARD:
    ...
    rc = ecore_init_array_dmae(p_hwfn, p_ptt, addr, dmae_array_offset + 1, size,
                               array_data, b_must_dmae, b_can_dmae);
    break;
  }
...
static enum _ecore_status_t ecore_init_cmd_wr(struct ecore_hwfn *p_hwfn,
  ...
  case INIT_SRC_ZEROS:
    ...
    break;
  case INIT_SRC_ARRAY:
    rc = ecore_init_cmd_array(p_hwfn, p_ptt, p_cmd, b_must_dmae, b_can_dmae);
    break;
  case INIT_SRC_RUNTIME:
    rc = ecore_init_rt(p_hwfn, p_ptt, addr,
                       OSAL_LE16_TO_CPU(p_cmd->args.runtime.offset),
                       OSAL_LE16_TO_CPU(p_cmd->args.runtime.size), b_must_dmae);
    break;
  }
...
static void ecore_init_cmd_rd(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < ECORE_INIT_MAX_POLL_COUNT && !comp_check(val, data); i++) {
    OSAL_UDELAY(delay);
    val = ecore_rd(p_hwfn, p_ptt, addr);
  }
  ...
  if (i == ECORE_INIT_MAX_POLL_COUNT)
    DP_ERR(p_hwfn,
           "Timeout when polling reg: 0x%08x [ Waiting-for: %08x Got: %08x "
           "(comparison %08x)]\n",
...
static u8 ecore_init_cmd_mode_match(struct ecore_hwfn *p_hwfn, u16 *p_offset,
                                    int modes) {
  struct ecore_dev *p_dev = p_hwfn->p_dev;
...
static u32 ecore_init_cmd_phase(struct init_if_phase_op *p_cmd, u32 phase,
                                u32 phase_id) {
...
enum _ecore_status_t ecore_init_run(struct ecore_hwfn *p_hwfn,
                                    struct ecore_ptt *p_ptt, int phase,
                                    int phase_id, int modes) {
  struct ecore_dev *p_dev = p_hwfn->p_dev;
...
#ifdef CONFIG_ECORE_ZIPPED_FW
  p_hwfn->unzip_buf =
      OSAL_ZALLOC(p_hwfn->p_dev, GFP_ATOMIC, MAX_ZIPPED_SIZE * 4);
  if (!p_hwfn->unzip_buf) {
  ...
  for (cmd_num = 0; cmd_num < num_init_ops; cmd_num++) {
    ...
    case INIT_OP_WRITE:
      rc = ecore_init_cmd_wr(p_hwfn, p_ptt, &cmd->write, b_dmae);
      break;
    ...
    case INIT_OP_IF_MODE:
      cmd_num += ecore_init_cmd_mode(p_hwfn, &cmd->if_mode, modes);
      break;
    case INIT_OP_IF_PHASE:
      cmd_num += ecore_init_cmd_phase(&cmd->if_phase, phase, phase_id);
      break;
    case INIT_OP_DELAY:
    ...
    case INIT_OP_CALLBACK:
      ...
      if (phase == PHASE_ENGINE && cmd->callback.callback_id == DMAE_READY_CB)
        b_dmae = true;
      break;
    }
