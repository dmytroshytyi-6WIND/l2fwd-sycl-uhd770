...
err:
  DP_NOTICE(p_hwfn, true,
            ...
            OSAL_LE32_TO_CPU(p_ent->elem.hdr.cid), p_ent->elem.hdr.cmd_id,
...
void ecore_set_spq_block_timeout(struct ecore_hwfn *p_hwfn,
                                 u32 spq_timeout_ms) {
  p_hwfn->p_spq->block_sleep_max_iter =
      spq_timeout_ms ? spq_timeout_ms / SPQ_BLOCK_SLEEP_MS
...
ecore_spq_fill_entry(struct ecore_hwfn *p_hwfn, struct ecore_spq_entry *p_ent) {
  ...
  DP_VERBOSE(
      ...
      p_ent->elem.hdr.cid, p_ent->elem.hdr.cmd_id, p_ent->elem.hdr.protocol_id,
      p_ent->elem.data_ptr.hi, p_ent->elem.data_ptr.lo,
      D_TRINE(p_ent->comp_mode, ECORE_SPQ_MODE_EBLOCK, ECORE_SPQ_MODE_BLOCK,
              "MODE_EBLOCK", "MODE_BLOCK", "MODE_CB"));
...
static void ecore_spq_hw_initialize(struct ecore_hwfn *p_hwfn,
  ...
  *p_spq_base_hi = DMA_HI_LE(p_spq->chain.p_phys_addr);
  ...
  DMA_REGPAIR_LE(*p_consolid_base_addr, p_hwfn->p_consq->chain.p_phys_addr);
}
...
static enum _ecore_status_t ecore_spq_hw_post(struct ecore_hwfn *p_hwfn,
  ...
  *elem = p_ent->elem; /* Struct assignment */
  ...
  p_db_data->spq_prod = OSAL_CPU_TO_LE16(ecore_chain_get_prod_idx(p_chain));
...
ecore_async_event_completion(struct ecore_hwfn *p_hwfn,
  ...
  if (!cb) {
    DP_NOTICE(p_hwfn, true, "Unknown Async completion for protocol: %d\n",
              p_eqe->protocol_id);
    return ECORE_INVAL;
  }

  rc = cb(p_hwfn, p_eqe->opcode, p_eqe->echo, &p_eqe->data,
          p_eqe->fw_return_code);
  if (rc != ECORE_SUCCESS)
    DP_NOTICE(p_hwfn, true,
              "Async completion callback failed, rc = %d [opcode %x, echo %x, "
              "fw_return_code %x]",
              rc, p_eqe->opcode, p_eqe->echo, p_eqe->fw_return_code);

  return rc;
}

enum _ecore_status_t ecore_spq_register_async_cb(struct ecore_hwfn *p_hwfn,
...
void ecore_spq_unregister_async_cb(struct ecore_hwfn *p_hwfn,
                                   enum protocol_type protocol_id) {
  if (!p_hwfn->p_spq || (protocol_id >= MAX_PROTOCOL_TYPE))
    return;
...
void ecore_eq_prod_update(struct ecore_hwfn *p_hwfn, u16 prod) {
  u32 addr =
      GTT_BAR0_MAP_REG_USDM_RAM + USTORM_EQE_CONS_OFFSET(p_hwfn->rel_pf_id);
...
enum _ecore_status_t ecore_eq_completion(struct ecore_hwfn *p_hwfn,
  ...
  while (fw_cons_idx != ecore_chain_get_cons_idx(p_chain)) {
    ...
    if (!p_eqe) {
      DP_ERR(p_hwfn, "Unexpected NULL chain consumer entry\n");
      break;
    }
    ...
    if (GET_FIELD(p_eqe->flags, EVENT_RING_ENTRY_ASYNC))
      ...
      ecore_spq_completion(p_hwfn, p_eqe->echo, p_eqe->fw_return_code,
                           &p_eqe->data);
...
enum _ecore_status_t ecore_eq_alloc(struct ecore_hwfn *p_hwfn, u16 num_elem) {
  ...
  if (!p_eq) {
    DP_NOTICE(p_hwfn, false, "Failed to allocate `struct ecore_eq'\n");
    return ECORE_NOMEM;
  }

  /* Allocate and initialize EQ chain*/
  if (ecore_chain_alloc(p_hwfn->p_dev, ECORE_CHAIN_USE_TO_PRODUCE,
                        ECORE_CHAIN_MODE_PBL, ECORE_CHAIN_CNT_TYPE_U16,
  ...
  ecore_int_register_cb(p_hwfn, ecore_eq_completion, p_eq, &p_eq->eq_sb_index,
                        &p_eq->p_fw_cons);
 ...
 ***************************************************************************/
static enum _ecore_status_t
ecore_cqe_completion(struct ecore_hwfn *p_hwfn,
...
ecore_eth_cqe_completion(struct ecore_hwfn *p_hwfn,
                         struct eth_slow_path_rx_cqe *cqe) {
  enum _ecore_status_t rc;

  rc = ecore_cqe_completion(p_hwfn, cqe, PROTOCOLID_ETH);
  if (rc) {
    DP_NOTICE(p_hwfn, true, "Failed to handle RXQ CQE [cmd 0x%02x]\n",
              cqe->ramrod_cmd_id);
  }
...
void ecore_spq_setup(struct ecore_hwfn *p_hwfn) {
  ...
  if (rc != ECORE_SUCCESS)
    DP_INFO(p_hwfn, "Failed to register the SPQ doorbell with the doorbell "
                    "recovery mechanism\n");
}
...
enum _ecore_status_t ecore_spq_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  /* SPQ struct */
  p_spq = OSAL_ZALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(struct ecore_spq));
  if (!p_spq) {
  ...
  /* SPQ ring  */
  if (ecore_chain_alloc(
          p_hwfn->p_dev, ECORE_CHAIN_USE_TO_PRODUCE, ECORE_CHAIN_MODE_SINGLE,
          ECORE_CHAIN_CNT_TYPE_U16, 0, /* N/A when the mode is SINGLE */
          sizeof(struct slow_path_element), &p_spq->chain, OSAL_NULL)) {
  ...
  p_virt = OSAL_DMA_ALLOC_COHERENT(p_hwfn->p_dev, &p_phys,
                                   capacity * sizeof(struct ecore_spq_entry));
  if (!p_virt)
    goto spq_allocate_fail;
...
void ecore_spq_free(struct ecore_hwfn *p_hwfn) {
  ...
  if (p_spq->p_virt) {
    capacity = ecore_chain_get_capacity(&p_spq->chain);
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_spq->p_virt, p_spq->p_phys,
                           capacity * sizeof(struct ecore_spq_entry));
  }
...
enum _ecore_status_t ecore_spq_get_entry(struct ecore_hwfn *p_hwfn,
  ...
  if (OSAL_LIST_IS_EMPTY(&p_spq->free_pool)) {
    ...
    p_ent =
        OSAL_LIST_FIRST_ENTRY(&p_spq->free_pool, struct ecore_spq_entry, list);
...
static enum _ecore_status_t ecore_spq_add_entry(struct ecore_hwfn *p_hwfn,
  ...
  if (p_ent->queue == &p_spq->unlimited_pending) {
    if (OSAL_LIST_IS_EMPTY(&p_spq->free_pool)) {
      OSAL_LIST_PUSH_TAIL(&p_ent->list, &p_spq->unlimited_pending);
      ...
      p_en2 = OSAL_LIST_FIRST_ENTRY(&p_spq->free_pool, struct ecore_spq_entry,
                                    list);
...
enum _ecore_status_t ecore_spq_completion(struct ecore_hwfn *p_hwfn,
  ...
  OSAL_SPIN_LOCK(&p_spq->lock);
  OSAL_LIST_FOR_EACH_ENTRY_SAFE(p_ent, tmp, &p_spq->completion_pending, list,
    ...
    if (p_ent->elem.hdr.echo == echo) {
      ...
      while (SPQ_COMP_BMAP_GET_BIT(p_spq, p_spq->comp_bitmap_idx)) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_SPQ,
               ...
               OSAL_LE16_TO_CPU(echo), OSAL_LE16_TO_CPU(p_ent->elem.hdr.echo));
  }
  ...
  DP_VERBOSE(
      p_hwfn, ECORE_MSG_SPQ, "Complete EQE [echo %04x]: func %p cookie %p)\n",
      OSAL_LE16_TO_CPU(echo), p_ent->comp_cb.function, p_ent->comp_cb.cookie);
...
enum _ecore_status_t ecore_consq_alloc(struct ecore_hwfn *p_hwfn) {
  ...
  if (!p_consq) {
    DP_NOTICE(p_hwfn, false, "Failed to allocate `struct ecore_consq'\n");
    return ECORE_NOMEM;
  }
  ...
  if (ecore_chain_alloc(p_hwfn->p_dev, ECORE_CHAIN_USE_TO_PRODUCE,
                        ECORE_CHAIN_MODE_PBL, ECORE_CHAIN_CNT_TYPE_U16,
                        ECORE_CHAIN_PAGE_SIZE / 0x80, 0x80, &p_consq->chain,
                        OSAL_NULL) != ECORE_SUCCESS) {
