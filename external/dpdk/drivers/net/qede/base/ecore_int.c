...
#define ATTENTION_LENGTH(flags)                                                \
  (((flags)&ATTENTION_LENGTH_MASK) >> ATTENTION_LENGTH_SHIFT)
...
static enum _ecore_status_t ecore_pswhst_attn_cb(struct ecore_hwfn *p_hwfn) {
  u32 tmp =
      ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, PSWHST_REG_VF_DISABLED_ERROR_VALID);
  ...
  if (tmp & ECORE_PSWHST_ATTENTION_VF_DISABLED) {
    ...
    data =
        ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, PSWHST_REG_VF_DISABLED_ERROR_DATA);
  ...
  tmp = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, PSWHST_REG_INCORRECT_ACCESS_VALID);
  if (tmp & ECORE_PSWHST_ATTENTION_INCORRECT_ACCESS) {
    ...
    data =
        ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, PSWHST_REG_INCORRECT_ACCESS_DATA);
    length =
        ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, PSWHST_REG_INCORRECT_ACCESS_LENGTH);
...
static enum _ecore_status_t ecore_grc_attn_cb(struct ecore_hwfn *p_hwfn) {
   ...
   */
  tmp = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, GRC_REG_TIMEOUT_ATTN_ACCESS_VALID);
  if (!(GET_FIELD(tmp, ECORE_GRC_ATTENTION_VALID_BIT)))
    goto out;

  /* Read the GRC timeout information */
  tmp = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, GRC_REG_TIMEOUT_ATTN_ACCESS_DATA_0);
  tmp2 =
      ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, GRC_REG_TIMEOUT_ATTN_ACCESS_DATA_1);

  DP_NOTICE(
      p_hwfn->p_dev, false,
      "GRC timeout [%08x:%08x] - %s Address [%08x] [Master %s] [PF: %02x %s "
      "%02x]\n",
      tmp2, tmp,
      ...
      (((tmp2 & ECORE_GRC_ATTENTION_PRIV_MASK) >>
        ECORE_GRC_ATTENTION_PRIV_SHIFT) == ECORE_GRC_ATTENTION_PRIV_VF)
          ? "VF"
          : "(Irrelevant:)",
      (tmp2 & ECORE_GRC_ATTENTION_VF_MASK) >> ECORE_GRC_ATTENTION_VF_SHIFT);

  /* Clean the validity bit */
  ecore_wr(p_hwfn, p_hwfn->p_dpc_ptt, GRC_REG_TIMEOUT_ATTN_ACCESS_VALID, 0);
out:
  return rc;
}
...
enum _ecore_status_t ecore_pglueb_rbc_attn_handler(struct ecore_hwfn *p_hwfn,
  ...
  if (tmp & ECORE_PGLUE_ATTENTION_VALID) {
    ...
    addr_lo = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_TX_ERR_WR_ADD_31_0);
    addr_hi = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_TX_ERR_WR_ADD_63_32);
    details = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_TX_ERR_WR_DETAILS);
    OSAL_SNPRINTF(
        str, 512,
        "Illegal write by chip to [%08x:%08x] blocked. Details: %08x [PFID "
        "%02x, VFID %02x, VF_VALID %02x] Details2 %08x [Was_error %02x BME "
        "deassert %02x FID_enable deassert %02x]\n",
        addr_hi, addr_lo, details,
        (u8)((details & ECORE_PGLUE_ATTENTION_DETAILS_PFID_MASK) >>
             ECORE_PGLUE_ATTENTION_DETAILS_PFID_SHIFT),
        (u8)((details & ECORE_PGLUE_ATTENTION_DETAILS_VFID_MASK) >>
             ECORE_PGLUE_ATTENTION_DETAILS_VFID_SHIFT),
        (u8)((details & ECORE_PGLUE_ATTENTION_DETAILS_VF_VALID) ? 1 : 0), tmp,
  ...
  if (tmp & ECORE_PGLUE_ATTENTION_RD_VALID) {
    ...
    addr_lo = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_TX_ERR_RD_ADD_31_0);
    addr_hi = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_TX_ERR_RD_ADD_63_32);
    details = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_TX_ERR_RD_DETAILS);

    DP_NOTICE(p_hwfn, false,
              "Illegal read by chip from [%08x:%08x] blocked. Details: %08x "
              "[PFID %02x, VFID %02x, VF_VALID %02x] Details2 %08x [Was_error "
              "%02x BME deassert %02x FID_enable deassert %02x]\n",
              addr_hi, addr_lo, details,
              (u8)((details & ECORE_PGLUE_ATTENTION_DETAILS_PFID_MASK) >>
                   ECORE_PGLUE_ATTENTION_DETAILS_PFID_SHIFT),
              (u8)((details & ECORE_PGLUE_ATTENTION_DETAILS_VFID_MASK) >>
              ...
              tmp, (u8)((tmp & ECORE_PGLUE_ATTENTION_DETAILS2_WAS_ERR) ? 1 : 0),
              (u8)((tmp & ECORE_PGLUE_ATTENTION_DETAILS2_BME) ? 1 : 0),
              (u8)((tmp & ECORE_PGLUE_ATTENTION_DETAILS2_FID_EN) ? 1 : 0));
  }
  ...
  if (tmp & ECORE_PGLUE_ATTENTION_ZLR_VALID) {
    ...
    addr_lo = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_MASTER_ZLR_ERR_ADD_31_0);
    addr_hi = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_MASTER_ZLR_ERR_ADD_63_32);

    DP_NOTICE(p_hwfn, false, "ICPL erorr - %08x [Address %08x:%08x]\n", tmp,
              addr_hi, addr_lo);
  }
  ...
  if (tmp & ECORE_PGLUE_ATTENTION_ILT_VALID) {
    ...
    addr_lo = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_VF_ILT_ERR_ADD_31_0);
    addr_hi = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_VF_ILT_ERR_ADD_63_32);
    details = ecore_rd(p_hwfn, p_ptt, PGLUE_B_REG_VF_ILT_ERR_DETAILS);
...
ecore_pglueb_rbc_attn_cb(struct ecore_hwfn *p_hwfn) {
...
static enum _ecore_status_t ecore_db_rec_flush_queue(struct ecore_hwfn *p_hwfn,
  ...
  if (usage) {
    DP_NOTICE(p_hwfn->p_dev, false,
              "DB recovery: doorbell usage failed to zero after %d usec. usage "
              "was %x\n",
...
static enum _ecore_status_t ecore_dorq_attn_cb(struct ecore_hwfn *p_hwfn) {
  ...
  if (int_sts &
    ...
    first_drop_reason = ecore_rd(p_hwfn, p_ptt, DORQ_REG_DB_DROP_REASON) &
                        ECORE_DORQ_ATTENTION_REASON_MASK;
    details = ecore_rd(p_hwfn, p_ptt, DORQ_REG_DB_DROP_DETAILS);
    address = ecore_rd(p_hwfn, p_ptt, DORQ_REG_DB_DROP_DETAILS_ADDRESS);
    all_drops_reason = ecore_rd(p_hwfn, p_ptt, DORQ_REG_DB_DROP_DETAILS_REASON);
    ...
    DP_NOTICE(
        ...
        GET_FIELD(details, ECORE_DORQ_ATTENTION_SIZE) * 4, first_drop_reason,
        all_drops_reason);
...
static enum _ecore_status_t ecore_tm_attn_cb(struct ecore_hwfn *p_hwfn) {
  ...
  if (CHIP_REV_IS_EMUL_B0(p_hwfn->p_dev)) {
    u32 val = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, TM_REG_INT_STS_1);

    if (val &
        ~(TM_REG_INT_STS_1_PEND_TASK_SCAN | TM_REG_INT_STS_1_PEND_CONN_SCAN))
      return ECORE_INVAL;

    if (val &
        (TM_REG_INT_STS_1_PEND_TASK_SCAN | TM_REG_INT_STS_1_PEND_CONN_SCAN))
      DP_INFO(p_hwfn, "TM attention on emulation - most likely"
                      " results of clock-ratios\n");
    val = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, TM_REG_INT_MASK_1);
    val |= TM_REG_INT_MASK_1_PEND_CONN_SCAN | TM_REG_INT_MASK_1_PEND_TASK_SCAN;
...
static struct aeu_invert_reg aeu_descs[NUM_ATTN_REGS] = {
    ...
    {{
        ...
        {"AVS",
         ATTENTION_PAR | ATTENTION_BB_DIFFERENT |
             ATTENTION_BB(AEU_INVERT_REG_SPECIAL_MCP_UMP_TX),
         OSAL_NULL, BLOCK_AVS_WRAP},
        {"AVS",
         ATTENTION_SINGLE | ATTENTION_BB_DIFFERENT |
             ATTENTION_BB(AEU_INVERT_REG_SPECIAL_MCP_SCPAD),
         OSAL_NULL, BLOCK_AVS_WRAP},
        ...
        {"Reserved %d", (9 << ATTENTION_LENGTH_SHIFT), OSAL_NULL, MAX_BLOCK_ID},
    }},
...
static bool ecore_int_is_parity_flag(struct ecore_hwfn *p_hwfn,
                                     struct aeu_invert_reg_bit *p_bit) {
  return !!(ecore_int_aeu_translate(p_hwfn, p_bit)->flags & ATTENTION_PARITY);
}
...
static enum _ecore_status_t ecore_int_assertion(struct ecore_hwfn *p_hwfn,
  ...
  igu_mask = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, IGU_REG_ATTENTION_ENABLE);
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_INTR,
             ...
             sb_attn_sw->known_attn, sb_attn_sw->known_attn | asserted_bits);
  ...
  if (asserted_bits & 0x100) {
    ...
    ecore_wr(p_hwfn, p_hwfn->p_dpc_ptt, sb_attn_sw->mfw_attn_addr, 0);
  }

  /* FIXME - this will change once we'll have GOOD gtt definitions */
  DIRECT_REG_WR(p_hwfn,
                (u8 OSAL_IOMEM *)p_hwfn->regview + GTT_BAR0_MAP_REG_IGU_CMD +
...
static void ecore_int_attn_print(struct ecore_hwfn *p_hwfn, enum block_id id,
...
ecore_int_deassertion_aeu_bit(struct ecore_hwfn *p_hwfn,
                              struct aeu_invert_reg_bit *p_aeu, u32 aeu_en_reg,
  ...
  if (p_aeu->block_index != MAX_BLOCK_ID) {
    ecore_int_attn_print(p_hwfn, p_aeu->block_index, ATTN_TYPE_INTERRUPT,
                         !b_fatal);
  }
...
static void ecore_int_deassertion_parity(struct ecore_hwfn *p_hwfn,
  ...
  if (block_id != MAX_BLOCK_ID) {
    ...
    if (block_id == BLOCK_BTB) {
      ecore_int_attn_print(p_hwfn, BLOCK_OPTE, ATTN_TYPE_PARITY, false);
      ecore_int_attn_print(p_hwfn, BLOCK_MCP, ATTN_TYPE_PARITY, false);
    }
  }
...
#define MISC_REG_AEU_ENABLE_IGU_OUT(n, group)                                  \
  (MISC_REG_AEU_ENABLE1_IGU_OUT_0 + (n)*0x4 + (group)*0x4 * NUM_ATTN_REGS)
...
static enum _ecore_status_t ecore_int_deassertion(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < NUM_ATTN_REGS; i++) {
    aeu_inv_arr[i] =
        ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, MISC_REG_AEU_AFTER_INVERT_IGU(i));
    DP_VERBOSE(p_hwfn, ECORE_MSG_INTR, "Deasserted bits [%d]: %08x\n", i,
               aeu_inv_arr[i]);
  }
  ...
  for (i = 0; i < NUM_ATTN_REGS; i++) {
    ...
    for (j = 0, bit_idx = 0; bit_idx < 32; j++) {
      ...
      if (ecore_int_is_parity_flag(p_hwfn, p_bit) &&
          !!(parities & (1 << bit_idx)))
        ecore_int_deassertion_parity(p_hwfn, p_bit, aeu_en, bit_idx);
    ...
    for (i = 0; i < NUM_ATTN_REGS; i++) {
      ...
      for (j = 0, bit_idx = 0; bit_idx < 32; j++) {
        ...
        if (bitmask) {
          ...
          num = (u8)OSAL_FIND_FIRST_BIT(&bitmask, bit_len);
          ...
          if (ATTENTION_LENGTH(flags) > 2 ||
              ((flags & ATTENTION_PAR_INT) && ATTENTION_LENGTH(flags) > 1))
            OSAL_SNPRINTF(bit_name, 30, p_aeu->bit_name, num);
          else
            strlcpy(bit_name, p_aeu->bit_name, sizeof(bit_name));
          ...
          ecore_int_deassertion_aeu_bit(p_hwfn, p_aeu, aeu_en, bit_name,
                                        bitmask);
        }
  ...
  aeu_mask = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, IGU_REG_ATTENTION_ENABLE);
...
static enum _ecore_status_t ecore_int_attentions(struct ecore_hwfn *p_hwfn) {
   ...
   */
  asserted_bits =
      (attn_bits & ~attn_acks & ATTN_STATE_BITS) & ~p_sb_attn_sw->known_attn;
  deasserted_bits =
      (~attn_bits & attn_acks & ATTN_STATE_BITS) & p_sb_attn_sw->known_attn;

  if ((asserted_bits & ~0x100) || (deasserted_bits & ~0x100))
    DP_INFO(p_hwfn,
            "Attention: Index: 0x%04x, Bits: 0x%08x, Acks: 0x%08x, asserted: "
            "0x%04x, De-asserted 0x%04x [Prev. known: 0x%04x]\n",
            index, attn_bits, attn_acks, asserted_bits, deasserted_bits,
            p_sb_attn_sw->known_attn);
  else if (asserted_bits == 0x100)
    DP_INFO(p_hwfn, "MFW indication via attention\n");
  else
    DP_VERBOSE(p_hwfn, ECORE_MSG_INTR, "MFW indication [deassertion]\n");
...
void ecore_int_sp_dpc(osal_int_ptr_t hwfn_cookie) {
  ...
  if (!sb_info->sb_virt) {
    DP_ERR(p_hwfn->p_dev, "Interrupt Status block is NULL -"
  ...
  if (!sb_attn || !sb_attn->sb_attn) {
    DP_ERR(p_hwfn->p_dev, "Attentions Status block is NULL -"
...
static void ecore_int_sb_attn_free(struct ecore_hwfn *p_hwfn) {
  ...
  if (p_sb->sb_attn) {
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_sb->sb_attn, p_sb->sb_phys,
...
static void ecore_int_sb_attn_init(struct ecore_hwfn *p_hwfn,
                                   struct ecore_ptt *p_ptt, void *sb_virt_addr,
  ...
  for (i = 0; i < NUM_ATTN_REGS; i++) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_INTR, "Attn Mask [Reg %d]: 0x%08x\n", i,
               sb_info->parity_mask[i]);
  }

  /* Set the address of cleanup for the mcp attention */
  sb_info->mfw_attn_addr =
      (p_hwfn->rel_pf_id << 3) + MISC_REG_AEU_GENERAL_ATTN_0;
...
static enum _ecore_status_t ecore_int_sb_attn_alloc(struct ecore_hwfn *p_hwfn,
  ...
  /* SB ring  */
  p_virt =
      OSAL_DMA_ALLOC_COHERENT(p_dev, &p_phys, SB_ATTN_ALIGNED_SIZE(p_hwfn));
  if (!p_virt) {
...
static void _ecore_int_cau_conf_pi(struct ecore_hwfn *p_hwfn,
                                   struct ecore_ptt *p_ptt, u16 igu_sb_id,
                                   u32 pi_index,
  ...
  if (p_hwfn->hw_init_done) {
    ecore_wr(p_hwfn, p_ptt, CAU_REG_PI_MEMORY + pi_offset * sizeof(u32),
             *((u32 *)&(pi_entry)));
  } else {
...
void ecore_int_cau_conf_pi(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                           ...
                           enum ecore_coalescing_fsm coalescing_fsm,
                           u8 timeset) {
  _ecore_int_cau_conf_pi(p_hwfn, p_ptt, p_sb->igu_sb_id, pi_index,
                         coalescing_fsm, timeset);
}

void ecore_int_cau_conf_sb(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                           dma_addr_t sb_phys, u16 igu_sb_id, u16 vf_number,
                           u8 vf_valid) {
  struct cau_sb_entry sb_entry;

  ecore_init_cau_sb_entry(p_hwfn, &sb_entry, p_hwfn->rel_pf_id, vf_number,
                          vf_valid);
  ...
  if (p_hwfn->hw_init_done) {
    ...
    ecore_dmae_host2grc(p_hwfn, p_ptt, (u64)(osal_uintptr_t)&phys_addr,
                        CAU_REG_SB_ADDR_MEMORY + igu_sb_id * sizeof(u64), 2,
                        OSAL_NULL /* default parameters */);
    ecore_dmae_host2grc(p_hwfn, p_ptt, (u64)(osal_uintptr_t)&sb_entry,
                        CAU_REG_SB_VAR_MEMORY + igu_sb_id * sizeof(u64), 2,
                        OSAL_NULL /* default parameters */);
  } else {
    /* Initialize Status Block Address */
    STORE_RT_REG_AGG(p_hwfn, CAU_REG_SB_ADDR_MEMORY_RT_OFFSET + igu_sb_id * 2,
                     sb_phys);

    STORE_RT_REG_AGG(p_hwfn, CAU_REG_SB_VAR_MEMORY_RT_OFFSET + igu_sb_id * 2,
                     sb_entry);
  }
  ...
  if (p_hwfn->p_dev->int_coalescing_mode == ECORE_COAL_MODE_ENABLE) {
    ...
    for (i = 0; i < num_tc; i++) {
      _ecore_int_cau_conf_pi(p_hwfn, p_ptt, igu_sb_id, TX_PI(i),
                             ECORE_COAL_TX_STATE_MACHINE, timeset);
    }
  }
}
...
void ecore_int_sb_setup(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  if (IS_PF(p_hwfn->p_dev))
    ecore_int_cau_conf_sb(p_hwfn, p_ptt, sb_info->sb_phys, sb_info->igu_sb_id,
                          0, 0);
}

struct ecore_igu_block *ecore_get_igu_free_sb(struct ecore_hwfn *p_hwfn,
                                              bool b_is_pf) {
  ...
  for (igu_id = 0; igu_id < ECORE_MAPPING_MEMORY_SIZE(p_hwfn->p_dev);
    ...
    if (!!(p_block->status & ECORE_IGU_STATUS_PF) == b_is_pf)
      return p_block;
  }
...
static u16 ecore_get_pf_igu_sb_id(struct ecore_hwfn *p_hwfn, u16 vector_id) {
  struct ecore_igu_block *p_block;
  u16 igu_id;
  ...
  for (igu_id = 0; igu_id < ECORE_MAPPING_MEMORY_SIZE(p_hwfn->p_dev);
    ...
    if (!(p_block->status & ECORE_IGU_STATUS_VALID) || !p_block->is_pf ||
...
u16 ecore_get_igu_sb_id(struct ecore_hwfn *p_hwfn, u16 sb_id) {
  ...
  if (igu_sb_id == ECORE_SB_INVALID_IDX)
    DP_NOTICE(p_hwfn, true, "Slowpath SB vector %04x doesn't exist\n", sb_id);
  else if (sb_id == ECORE_SP_SB_ID)
    DP_VERBOSE(p_hwfn, ECORE_MSG_INTR, "Slowpath SB index in IGU is 0x%04x\n",
               igu_sb_id);
  else
    DP_VERBOSE(p_hwfn, ECORE_MSG_INTR, "SB [%04x] <--> IGU SB [%04x]\n", sb_id,
               igu_sb_id);
...
static void ecore_int_sp_sb_free(struct ecore_hwfn *p_hwfn) {
  ...
  if (p_sb->sb_info.sb_virt) {
    OSAL_DMA_FREE_COHERENT(p_hwfn->p_dev, p_sb->sb_info.sb_virt,
                           p_sb->sb_info.sb_phys, SB_ALIGNED_SIZE(p_hwfn));
  }
...
static enum _ecore_status_t ecore_int_sp_sb_alloc(struct ecore_hwfn *p_hwfn,
  ...
  /* SB ring  */
  p_virt =
      OSAL_DMA_ALLOC_COHERENT(p_hwfn->p_dev, &p_phys, SB_ALIGNED_SIZE(p_hwfn));
  if (!p_virt) {
  ...
  /* Status Block setup */
  p_hwfn->p_sp_sb = p_sb;
  ecore_int_sb_init(p_hwfn, p_ptt, &p_sb->sb_info, p_virt, p_phys,
                    ECORE_SP_SB_ID);
...
static void ecore_int_igu_cleanup_sb(struct ecore_hwfn *p_hwfn,
                                     struct ecore_ptt *p_ptt, u32 igu_sb_id,
  ...
  OSAL_BUILD_BUG_ON((IGU_REG_CLEANUP_STATUS_4 - IGU_REG_CLEANUP_STATUS_0) !=
                    0x200);
...
void ecore_int_igu_init_pure_rt_single(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < IGU_CLEANUP_SLEEP_LENGTH; i++) {
    ...
    val = ecore_rd(p_hwfn, p_ptt,
                   IGU_REG_WRITE_DONE_PENDING + ((igu_sb_id / 32) * 4));
  ...
  for (pi = 0; pi < PIS_PER_SB; pi++)
    ecore_wr(p_hwfn, p_ptt,
             CAU_REG_PI_MEMORY + (igu_sb_id * PIS_PER_SB + pi) * 4, 0);
}

void ecore_int_igu_init_pure_rt(struct ecore_hwfn *p_hwfn,
                                struct ecore_ptt *p_ptt, bool b_set,
                                bool b_slowpath) {
  ...
  for (igu_sb_id = 0; igu_sb_id < ECORE_MAPPING_MEMORY_SIZE(p_hwfn->p_dev);
       igu_sb_id++) {
  ...
  if (b_slowpath)
    ecore_int_igu_init_pure_rt_single(p_hwfn, p_ptt, p_info->igu_dsb_id,
                                      p_hwfn->hw_info.opaque_fid, b_set);
}
...
int ecore_int_igu_reset_cam(struct ecore_hwfn *p_hwfn,
  ...
  if (!RESC_NUM(p_hwfn, ECORE_SB)) {
    ...
    if (p_info->usage.cnt != RESC_NUM(p_hwfn, ECORE_SB) - 1) {
      DP_INFO(p_hwfn,
              ...
              RESC_NUM(p_hwfn, ECORE_SB) - 1, p_info->usage.cnt);
    ...
    if (IS_PF_SRIOV(p_hwfn)) {
       ...
       */
      if (vfs > p_info->usage.free_cnt + p_info->usage.free_cnt_iov -
                    p_info->usage.cnt) {
        DP_NOTICE(p_hwfn, true,
                  "Not enough SBs for VFs - 0x%04x SBs, from which %04x PFs "
                  "and %04x are required\n",
                  p_info->usage.free_cnt + p_info->usage.free_cnt_iov,
  ...
  for (igu_sb_id = p_info->igu_dsb_id;
       igu_sb_id < ECORE_MAPPING_MEMORY_SIZE(p_hwfn->p_dev); igu_sb_id++) {
    p_block = &p_info->entry[igu_sb_id];
    val = 0;
    ...
    if (p_block->status & ECORE_IGU_STATUS_DSB) {
      ...
      p_block->status =
          ECORE_IGU_STATUS_VALID | ECORE_IGU_STATUS_PF | ECORE_IGU_STATUS_DSB;
    } else if (pf_sbs) {
      pf_sbs--;
      p_block->function_id = p_hwfn->rel_pf_id;
      p_block->is_pf = 1;
      ...
      p_block->status =
          ECORE_IGU_STATUS_VALID | ECORE_IGU_STATUS_PF | ECORE_IGU_STATUS_FREE;
    } else if (vf_sbs) {
      p_block->function_id = p_hwfn->p_dev->p_iov_info->first_vf_in_pf +
                             p_info->usage.iov_cnt - vf_sbs;
      p_block->is_pf = 0;
      p_block->vector_number = 0;
      p_block->status = ECORE_IGU_STATUS_VALID | ECORE_IGU_STATUS_FREE;
      vf_sbs--;
    } else {
    ...
    if (rval != val) {
      ecore_wr(p_hwfn, p_ptt, IGU_REG_MAPPING_MEMORY + sizeof(u32) * igu_sb_id,
               val);

      DP_VERBOSE(p_hwfn, ECORE_MSG_INTR,
                 "IGU reset: [SB 0x%04x] func_id = %d is_pf = %d vector_num = "
                 "0x%x [%08x -> %08x]\n",
                 igu_sb_id, p_block->function_id, p_block->is_pf,
...
static void ecore_int_igu_read_cam_block(struct ecore_hwfn *p_hwfn,
                                         ...
                                         u16 igu_sb_id) {
  u32 val =
      ecore_rd(p_hwfn, p_ptt, IGU_REG_MAPPING_MEMORY + sizeof(u32) * igu_sb_id);
  struct ecore_igu_block *p_block;
...
enum _ecore_status_t ecore_int_igu_read_cam(struct ecore_hwfn *p_hwfn,
  ...
  p_hwfn->hw_info.p_igu_info =
      OSAL_ZALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(*p_igu_info));
  ...
  for (igu_sb_id = 0; igu_sb_id < ECORE_MAPPING_MEMORY_SIZE(p_hwfn->p_dev);
    ...
    if ((p_block->is_pf) && (p_block->function_id == p_hwfn->rel_pf_id)) {
      p_block->status =
          ECORE_IGU_STATUS_PF | ECORE_IGU_STATUS_VALID | ECORE_IGU_STATUS_FREE;

      if (p_igu_info->igu_dsb_id != ECORE_SB_INVALID_IDX)
        p_igu_info->usage.cnt++;
    } else if (!(p_block->is_pf) && (p_block->function_id >= min_vf) &&
     ...
     */
    if ((p_block->status & ECORE_IGU_STATUS_VALID) || (p_hwfn->abs_pf_id == 0))
      DP_VERBOSE(
          p_hwfn, ECORE_MSG_INTR,
  ...
  if (p_igu_info->igu_dsb_id == ECORE_SB_INVALID_IDX) {
    DP_NOTICE(p_hwfn, true, "IGU CAM returned invalid values igu_dsb_id=0x%x\n",
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_INTR,
             "igu_dsb_id=0x%x, num Free SBs - PF: %04x VF: %04x [might change "
...
enum _ecore_status_t ecore_int_igu_relocate_sb(struct ecore_hwfn *p_hwfn,
   ...
   */
  if (!b_to_vf) {
    if (ecore_get_pf_igu_sb_id(p_hwfn, sb_id + 1) != ECORE_SB_INVALID_IDX)
      return ECORE_INVAL;
  }
   ...
   */
  for (; igu_sb_id < ECORE_MAPPING_MEMORY_SIZE(p_hwfn->p_dev); igu_sb_id++) {
    p_block = &p_info->entry[igu_sb_id];
  ...
  ecore_int_cau_conf_sb(p_hwfn, p_ptt, 0, igu_sb_id, vf_num,
                        p_block->is_pf ? 0 : 1);

  DP_VERBOSE(
      p_hwfn, ECORE_MSG_INTR,
      "Relocation: [SB 0x%04x] func_id = %d is_pf = %d vector_num = 0x%x\n",
      igu_sb_id, p_block->function_id, p_block->is_pf, p_block->vector_number);
...
u64 ecore_int_igu_read_sisr_reg(struct ecore_hwfn *p_hwfn) {
  ...
  intr_status_lo =
      REG_RD(p_hwfn, GTT_BAR0_MAP_REG_IGU_CMD + LSB_IGU_CMD_ADDR * 8);
  intr_status_hi =
      REG_RD(p_hwfn, GTT_BAR0_MAP_REG_IGU_CMD + MSB_IGU_CMD_ADDR * 8);
...
void ecore_int_get_num_sbs(struct ecore_hwfn *p_hwfn,
  ...
  OSAL_MEMCPY(p_sb_cnt_info, &p_igu_info->usage, sizeof(*p_sb_cnt_info));
}

void ecore_int_disable_post_isr_release(struct ecore_dev *p_dev) {
  int i;

  for_each_hwfn(p_dev, i) p_dev->hwfns[i].b_int_requested = false;
}
...
enum _ecore_status_t ecore_int_set_timer_res(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_dmae_grc2host(
      p_hwfn, p_ptt, CAU_REG_SB_VAR_MEMORY + sb_id * sizeof(u64),
      (u64)(osal_uintptr_t)&sb_entry, 2, OSAL_NULL /* default parameters */);
  if (rc != ECORE_SUCCESS) {
  ...
  rc = ecore_dmae_host2grc(p_hwfn, p_ptt, (u64)(osal_uintptr_t)&sb_entry,
...
enum _ecore_status_t ecore_int_get_sb_dbg(struct ecore_hwfn *p_hwfn,
  ...
  p_info->igu_prod =
      ecore_rd(p_hwfn, p_ptt, IGU_REG_PRODUCER_MEMORY + sbid * 4);
  p_info->igu_cons = ecore_rd(p_hwfn, p_ptt, IGU_REG_CONSUMER_MEM + sbid * 4);

  for (i = 0; i < PIS_PER_SB; i++)
    p_info->pi[i] = (u16)ecore_rd(
        p_hwfn, p_ptt, CAU_REG_PI_MEMORY + sbid * 4 * PIS_PER_SB + i * 4);
...
void ecore_pf_flr_igu_cleanup(struct ecore_hwfn *p_hwfn) {
  ...
  struct ecore_ptt *p_dpc_ptt =
      ecore_get_reserved_ptt(p_hwfn, RESERVED_PTT_DPC);
  int i;
