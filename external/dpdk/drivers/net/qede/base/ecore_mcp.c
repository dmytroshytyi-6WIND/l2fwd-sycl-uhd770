...
#define DRV_INNER_WR(_p_hwfn, _p_ptt, _ptr, _offset, _val)                     \
  ecore_wr(_p_hwfn, _p_ptt, (_p_hwfn->mcp_info->_ptr + _offset), _val)
...
#define PDA_COMP                                                               \
  (((FW_MAJOR_VERSION) + (FW_MINOR_VERSION << 8)) << DRV_ID_PDA_COMP_VER_OFFSET)
...
void ecore_mcp_cmd_port_init(struct ecore_hwfn *p_hwfn,
  ...
  u32 addr = SECTION_OFFSIZE_ADDR(p_hwfn->mcp_info->public_base, PUBLIC_PORT);
  u32 mfw_mb_offsize = ecore_rd(p_hwfn, p_ptt, addr);

  p_hwfn->mcp_info->port_addr = SECTION_ADDR(mfw_mb_offsize, MFW_PORT(p_hwfn));
  DP_VERBOSE(p_hwfn, ECORE_MSG_SP, "port_addr = 0x%x, port_id 0x%02x\n",
...
void ecore_mcp_read_mb(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt) {
  ...
  for (i = 0; i < length; i++) {
    tmp = ecore_rd(p_hwfn, p_ptt,
                   p_hwfn->mcp_info->mfw_mb_addr + (i << 2) + sizeof(u32));

    ((u32 *)p_hwfn->mcp_info->mfw_mb_cur)[i] = OSAL_BE32_TO_CPU(tmp);
  }
}
...
ecore_mcp_cmd_add_elem(struct ecore_hwfn *p_hwfn,
  ...
  p_cmd_elem = OSAL_ZALLOC(p_hwfn->p_dev, GFP_ATOMIC, sizeof(*p_cmd_elem));
  if (!p_cmd_elem) {
...
static enum _ecore_status_t ecore_load_mcp_offsets(struct ecore_hwfn *p_hwfn,
  ...
  mfw_mb_offsize = ecore_rd(
      p_hwfn, p_ptt, SECTION_OFFSIZE_ADDR(p_info->public_base, PUBLIC_MFW_MB));
  p_info->mfw_mb_addr = SECTION_ADDR(mfw_mb_offsize, mcp_pf_id);
  p_info->mfw_mb_length = (u16)ecore_rd(p_hwfn, p_ptt, p_info->mfw_mb_addr);
  ...
  drv_mb_offsize = ecore_rd(
      p_hwfn, p_ptt, SECTION_OFFSIZE_ADDR(p_info->public_base, PUBLIC_DRV_MB));
   ...
   */
  p_info->drv_mb_seq =
      DRV_MB_RD(p_hwfn, p_ptt, drv_mb_header) & DRV_MSG_SEQ_NUMBER_MASK;
...
enum _ecore_status_t ecore_mcp_cmd_init(struct ecore_hwfn *p_hwfn,
  ...
  p_hwfn->mcp_info =
      OSAL_ZALLOC(p_hwfn->p_dev, GFP_KERNEL, sizeof(*p_hwfn->mcp_info));
  if (!p_hwfn->mcp_info) {
...
enum _ecore_status_t ecore_mcp_reset(struct ecore_hwfn *p_hwfn,
  ...
  if (p_hwfn->mcp_info->b_block_cmd) {
    DP_NOTICE(p_hwfn, false,
              "The MFW is not responsive. Avoid sending MCP_RESET mailbox "
              "command.\n");
  ...
  do {
    ...
    if (ecore_rd(p_hwfn, p_ptt, MISCS_REG_GENERIC_POR_0) != prev_generic_por_0)
      break;
  } while (cnt++ < retries);

  if (ecore_rd(p_hwfn, p_ptt, MISCS_REG_GENERIC_POR_0) != prev_generic_por_0) {
    DP_VERBOSE(p_hwfn, ECORE_MSG_SP, "MCP was reset after %d usec\n",
               cnt * delay);
  } else {
...
static bool ecore_mcp_has_pending_cmd(struct ecore_hwfn *p_hwfn) {
  ...
  if (!OSAL_LIST_IS_EMPTY(&p_hwfn->mcp_info->cmd_list)) {
    p_cmd_elem = OSAL_LIST_FIRST_ENTRY(&p_hwfn->mcp_info->cmd_list,
                                       struct ecore_mcp_cmd_elem, list);
...
/* Must be called while cmd_lock is acquired */
static enum _ecore_status_t
ecore_mcp_update_pending_cmd(struct ecore_hwfn *p_hwfn,
                             struct ecore_ptt *p_ptt) {
  struct ecore_mcp_mb_params *p_mb_params;
  struct ecore_mcp_cmd_elem *p_cmd_elem;
  u32 mcp_resp;
  ...
  if (!p_cmd_elem) {
    DP_ERR(p_hwfn,
           "Failed to find a pending mailbox cmd that expects sequence number "
           "%d\n",
           seq_num);
  ...
  /* Get the union data */
  if (p_mb_params->p_data_dst != OSAL_NULL && p_mb_params->data_dst_size) {
    u32 union_data_addr = p_hwfn->mcp_info->drv_mb_addr +
                          OFFSETOF(struct public_drv_mb, union_data);
    ecore_memcpy_from(p_hwfn, p_ptt, p_mb_params->p_data_dst, union_data_addr,
...
static void __ecore_mcp_cmd_and_union(struct ecore_hwfn *p_hwfn,
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_HW, "MFW mailbox: command 0x%08x param 0x%08x\n",
...
_ecore_mcp_cmd_and_union(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  if (cnt >= max_retries) {
    DP_NOTICE(p_hwfn, false,
              "The MFW mailbox is occupied by an uncompleted command. Failed "
              "to send command 0x%08x [param 0x%08x].\n",
...
ecore_mcp_cmd_and_union(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  if (p_mb_params->data_src_size > union_data_size ||
    ...
    DP_ERR(p_hwfn,
           "The provided size is larger than the union data size [src_size %u, "
           "dst_size %u, union_data_size %zu]\n",
  ...
  if (p_hwfn->mcp_info->b_block_cmd) {
    DP_NOTICE(p_hwfn, false,
              "The MFW is not responsive. Avoid sending mailbox command 0x%08x "
              "[param 0x%08x].\n",
              p_mb_params->cmd, p_mb_params->param);
    return ECORE_ABORTED;
  }
...
enum _ecore_status_t ecore_mcp_nvm_wr_cmd(struct ecore_hwfn *p_hwfn,
                                          struct ecore_ptt *p_ptt, u32 cmd,
                                          u32 param, u32 *o_mcp_resp,
                                          u32 *o_mcp_param, u32 i_txn_size,
                                          u32 *i_buf) {
  struct ecore_mcp_mb_params mb_params;
  enum _ecore_status_t rc;
...
enum _ecore_status_t ecore_mcp_nvm_rd_cmd(struct ecore_hwfn *p_hwfn,
                                          struct ecore_ptt *p_ptt, u32 cmd,
                                          u32 param, u32 *o_mcp_resp,
                                          u32 *o_mcp_param, u32 *o_txn_size,
                                          u32 *o_buf) {
  struct ecore_mcp_mb_params mb_params;
...
ecore_mcp_can_force_load(u8 drv_role, u8 exist_drv_role,
  ...
  default:
    can_force_load =
        (drv_role == DRV_ROLE_OS && exist_drv_role == DRV_ROLE_PREBOOT) ||
...
static enum _ecore_status_t ecore_mcp_cancel_load_req(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_CANCEL_LOAD_REQ, 0, &resp,
                     &param);
  if (rc != ECORE_SUCCESS)
    DP_NOTICE(p_hwfn, false, "Failed to send cancel load request, rc = %d\n",
              rc);
...
__ecore_mcp_load_req(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  SET_MFW_FIELD(load_req.misc0, LOAD_REQ_LOCK_TO, p_in_params->timeout_val);
  SET_MFW_FIELD(load_req.misc0, LOAD_REQ_FORCE, p_in_params->force_cmd);
  SET_MFW_FIELD(load_req.misc0, LOAD_REQ_FLAGS0, p_in_params->avoid_eng_reset);
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_SP,
             "Load Request: param 0x%08x [init_hw %d, drv_type %d, hsi_ver %d, "
             "pda 0x%04x]\n",
             mb_params.param,
  ...
  if (p_in_params->hsi_ver != ECORE_LOAD_REQ_HSI_VER_1)
    DP_VERBOSE(p_hwfn, ECORE_MSG_SP,
               "Load Request: drv_ver 0x%08x_0x%08x, fw_ver 0x%08x, misc0 "
               "0x%08x [role %d, timeout %d, force %d, flags0 0x%x]\n",
               load_req.drv_ver_0, load_req.drv_ver_1, load_req.fw_ver,
               load_req.misc0, GET_MFW_FIELD(load_req.misc0, LOAD_REQ_ROLE),
  ...
  rc = ecore_mcp_cmd_and_union(p_hwfn, p_ptt, &mb_params);
  if (rc != ECORE_SUCCESS) {
    DP_NOTICE(p_hwfn, false, "Failed to send load request, rc = %d\n", rc);
    return rc;
  }

  DP_VERBOSE(p_hwfn, ECORE_MSG_SP, "Load Response: resp 0x%08x\n",
             mb_params.mcp_resp);
  ...
  if (p_in_params->hsi_ver != ECORE_LOAD_REQ_HSI_VER_1 &&
    ...
    DP_VERBOSE(
        p_hwfn, ECORE_MSG_SP,
        "Load Response: exist_drv_ver 0x%08x_0x%08x, exist_fw_ver 0x%08x, "
        "misc0 0x%08x [exist_role %d, mfw_hsi %d, flags0 0x%x]\n",
        load_rsp.drv_ver_0, load_rsp.drv_ver_1, load_rsp.fw_ver, load_rsp.misc0,
    ...
    p_out_params->exist_drv_role = GET_MFW_FIELD(load_rsp.misc0, LOAD_RSP_ROLE);
    p_out_params->mfw_hsi_ver = GET_MFW_FIELD(load_rsp.misc0, LOAD_RSP_HSI);
    p_out_params->drv_exists = GET_MFW_FIELD(load_rsp.misc0, LOAD_RSP_FLAGS0) &
...
ecore_mcp_load_req(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  if (out_params.load_code == FW_MSG_CODE_DRV_LOAD_REFUSED_HSI_1) {
    ...
    if (ecore_mcp_can_force_load(in_params.drv_role, out_params.exist_drv_role,
                                 p_params->override_force_load)) {
      DP_INFO(p_hwfn,
              "A force load is required [{role, fw_ver, drv_ver}: loading={%d, "
              "0x%08x, 0x%08x_%08x}, existing={%d, 0x%08x, 0x%08x_%08x}]\n",
              in_params.drv_role, in_params.fw_ver, in_params.drv_ver_0,
              in_params.drv_ver_1, out_params.exist_drv_role,
              out_params.exist_fw_ver, out_params.exist_drv_ver_0,
              out_params.exist_drv_ver_1);

      ecore_get_mfw_force_cmd(ECORE_LOAD_REQ_FORCE_ALL, &mfw_force_cmd);
      ...
      DP_NOTICE(p_hwfn, false,
                "A force load is required [{role, fw_ver, drv_ver}: "
                "loading={%d, 0x%08x, x%08x_0x%08x}, existing={%d, 0x%08x, "
                "0x%08x_0x%08x}] - Avoid\n",
                in_params.drv_role, in_params.fw_ver, in_params.drv_ver_0,
                in_params.drv_ver_1, out_params.exist_drv_role,
                out_params.exist_fw_ver, out_params.exist_drv_ver_0,
  ...
  case FW_MSG_CODE_DRV_LOAD_FUNCTION:
    if (out_params.mfw_hsi_ver != ECORE_LOAD_REQ_HSI_VER_1 &&
       ...
       */
      DP_NOTICE(p_hwfn, false,
                "PF is already loaded - shouldn't have got here since a "
                "quasi-FLR request was previously sent!\n");
      return ECORE_INVAL;
    }
    break;
  default:
...
enum _ecore_status_t ecore_mcp_load_done(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_LOAD_DONE, 0, &resp, &param);
  if (rc != ECORE_SUCCESS) {
    DP_NOTICE(p_hwfn, false, "Failed to send a LOAD_DONE command, rc = %d\n",
              rc);
    return rc;
  }
  ...
  if (param & FW_MB_PARAM_LOAD_DONE_DID_EFUSE_ERROR)
    DP_NOTICE(p_hwfn, false,
              "warning: device configuration is not supported on this board "
...
static void ecore_mcp_handle_vf_flr(struct ecore_hwfn *p_hwfn,
                                    struct ecore_ptt *p_ptt) {
  u32 addr = SECTION_OFFSIZE_ADDR(p_hwfn->mcp_info->public_base, PUBLIC_PATH);
  u32 mfw_path_offsize = ecore_rd(p_hwfn, p_ptt, addr);
  u32 path_addr = SECTION_ADDR(mfw_path_offsize, ECORE_PATH_ID(p_hwfn));
  ...
  for (i = 0; i < VF_BITMAP_SIZE_IN_DWORDS; i++) {
    disabled_vfs[i] =
        ecore_rd(p_hwfn, p_ptt,
                 path_addr + OFFSETOF(struct public_path, mcp_vf_disabled) +
                     sizeof(u32) * i);
...
enum _ecore_status_t ecore_mcp_ack_vf_flr(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_mcp_cmd_and_union(p_hwfn, p_ptt, &mb_params);
  if (rc != ECORE_SUCCESS) {
    DP_NOTICE(p_hwfn, false, "Failed to pass ACK for VF flr to MFW\n");
    return ECORE_TIMEOUT;
  }
...
static void ecore_mcp_handle_transceiver_change(struct ecore_hwfn *p_hwfn,
  ...
  transceiver_state = GET_MFW_FIELD(transceiver_state, ETH_TRANSCEIVER_STATE);
...
static u32 ecore_mcp_get_shmem_func(struct ecore_hwfn *p_hwfn,
  ...
  u32 addr = SECTION_OFFSIZE_ADDR(p_hwfn->mcp_info->public_base, PUBLIC_FUNC);
  u32 mfw_path_offsize = ecore_rd(p_hwfn, p_ptt, addr);
  ...
  for (i = 0; i < size / sizeof(u32); i++)
    ((u32 *)p_data)[i] = ecore_rd(p_hwfn, p_ptt, func_addr + (i << 2));
...
static void ecore_mcp_handle_link_change(struct ecore_hwfn *p_hwfn,
  ...
  if (!b_reset) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_LINK, "Resetting link indications\n");
    goto out;
  }
  ...
  if (p_hwfn->b_drv_link_init) {
     ...
     * indication.
     */
    if (p_hwfn->mcp_info->capabilities & FW_MB_PARAM_FEATURE_SUPPORT_VLINK) {
      struct public_func shmem_info;

      ecore_mcp_get_shmem_func(p_hwfn, p_ptt, &shmem_info, MCP_PF_ID(p_hwfn));
      p_link->link_up = !!(shmem_info.status & FUNC_STATUS_VIRTUAL_LINK_UP);
    } else {
  ...
  p_link->partner_adv_speed |=
      (status & LINK_STATUS_LINK_PARTNER_1000THD_CAPABLE)
          ...
          : 0;
  p_link->partner_adv_speed |= (status & LINK_STATUS_LINK_PARTNER_10G_CAPABLE)
                                   ? ECORE_LINK_PARTNER_SPEED_10G
                                   : 0;
  p_link->partner_adv_speed |= (status & LINK_STATUS_LINK_PARTNER_20G_CAPABLE)
                                   ? ECORE_LINK_PARTNER_SPEED_20G
                                   : 0;
  p_link->partner_adv_speed |= (status & LINK_STATUS_LINK_PARTNER_25G_CAPABLE)
                                   ? ECORE_LINK_PARTNER_SPEED_25G
                                   : 0;
  p_link->partner_adv_speed |= (status & LINK_STATUS_LINK_PARTNER_40G_CAPABLE)
                                   ? ECORE_LINK_PARTNER_SPEED_40G
                                   : 0;
  p_link->partner_adv_speed |= (status & LINK_STATUS_LINK_PARTNER_50G_CAPABLE)
                                   ? ECORE_LINK_PARTNER_SPEED_50G
                                   : 0;
  p_link->partner_adv_speed |= (status & LINK_STATUS_LINK_PARTNER_100G_CAPABLE)
...
enum _ecore_status_t ecore_mcp_set_link(struct ecore_hwfn *p_hwfn,
  ...
  if ((p_hwfn->mcp_info->capabilities & FW_MB_PARAM_FEATURE_SUPPORT_EEE) &&
    ...
    phy_cfg.eee_cfg |= (params->eee.tx_lpi_timer << EEE_TX_TIMER_USEC_OFFSET) &
  ...
  if (b_up)
    DP_VERBOSE(p_hwfn, ECORE_MSG_LINK,
               "Configuring Link: Speed 0x%08x, Pause 0x%08x, adv_speed "
               "0x%08x, loopback 0x%08x\n",
...
u32 ecore_get_process_kill_counter(struct ecore_hwfn *p_hwfn,
  ...
  path_offsize_addr =
      SECTION_OFFSIZE_ADDR(p_hwfn->mcp_info->public_base, PUBLIC_PATH);
  path_offsize = ecore_rd(p_hwfn, p_ptt, path_offsize_addr);
  path_addr = SECTION_ADDR(path_offsize, ECORE_PATH_ID(p_hwfn));

  proc_kill_cnt =
      ecore_rd(p_hwfn, p_ptt,
               path_addr + OFFSETOF(struct public_path, process_kill)) &
...
static void ecore_mcp_send_protocol_stats(struct ecore_hwfn *p_hwfn,
  ...
  case MFW_DRV_MSG_GET_LAN_STATS:
    ...
    break;
  default:
    DP_VERBOSE(p_hwfn, ECORE_MSG_SP, "Invalid protocol type %d\n", type);
    return;
  }
...
static void ecore_read_pf_bandwidth(struct ecore_hwfn *p_hwfn,
   ...
   * range.
   */
  p_info->bandwidth_min = (p_shmem_info->config & FUNC_MF_CFG_MIN_BW_MASK) >>
  ...
  p_info->bandwidth_max = (p_shmem_info->config & FUNC_MF_CFG_MAX_BW_MASK) >>
...
static void ecore_mcp_update_bw(struct ecore_hwfn *p_hwfn,
                                struct ecore_ptt *p_ptt) {
  struct ecore_mcp_function_info *p_info;
  struct public_func shmem_info;
  u32 resp = 0, param = 0;
  ...
  /* Acknowledge the MFW */
  ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_BW_UPDATE_ACK, 0, &resp, &param);
}
...
static void ecore_mcp_update_stag(struct ecore_hwfn *p_hwfn,
  ...
  p_hwfn->mcp_info->func_info.ovlan =
      (u16)shmem_info.ovlan_stag & FUNC_MF_CFG_OV_STAG_MASK;
  ...
  if (OSAL_GET_BIT(ECORE_MF_OVLAN_CLSS, &p_hwfn->p_dev->mf_bits)) {
    if (p_hwfn->hw_info.ovlan != ECORE_MCP_VLAN_UNSET) {
      ...
      ecore_wr(p_hwfn, p_ptt, DORQ_REG_PF_EXT_VID, p_hwfn->hw_info.ovlan);
    } else {
  ...
  /* Acknowledge the MFW */
  ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_S_TAG_UPDATE_ACK, 0, &resp, &param);
}
...
ecore_mcp_mdump_get_info(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  p_mdump_info->reason =
      ecore_rd(p_hwfn, p_ptt,
               global_addr + OFFSETOF(struct public_global, mdump_reason));
  ...
  if (p_mdump_info->reason) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_SP,
               "MFW mdump info: reason %d, version 0x%x, config 0x%x, epoch "
               "0x%x, num_of_logs 0x%x, valid_logs 0x%x\n",
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_SP, "MFW mdump info: reason %d\n",
               p_mdump_info->reason);
  }
...
static void ecore_mcp_handle_critical_error(struct ecore_hwfn *p_hwfn,
  ...
  if (rc == ECORE_SUCCESS && mdump_retain.valid) {
    DP_NOTICE(p_hwfn, false,
              ...
              "[epoch 0x%08x, pf 0x%x, status 0x%08x]\n",
  ...
  DP_NOTICE(p_hwfn, false,
            "Acknowledging the notification to not allow the MFW crash dump "
            "[driver debug data collection is preferable]\n");
  ecore_mcp_mdump_ack(p_hwfn, p_ptt);
  ecore_hw_err_notify(p_hwfn, ECORE_HW_ERR_HW_ATTN);
}

void ecore_mcp_read_ufp_config(struct ecore_hwfn *p_hwfn,
                               struct ecore_ptt *p_ptt) {
  struct public_func shmem_info;
  u32 port_cfg, val;
  ...
  port_cfg = ecore_rd(p_hwfn, p_ptt,
                      p_hwfn->mcp_info->port_addr +
  ...
  val = GET_MFW_FIELD(shmem_info.oem_cfg_func, OEM_CFG_FUNC_HOST_PRI_CTRL);
...
ecore_mcp_handle_ufp_event(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt) {
  ...
  if (p_hwfn->ufp_info.mode == ECORE_UFP_MODE_VNIC_BW) {
    ...
    ecore_dcbx_mib_update_event(p_hwfn, p_ptt, ECORE_DCBX_OPERATIONAL_MIB);
  }
...
enum _ecore_status_t ecore_mcp_handle_events(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < info->mfw_mb_length; i++) {
    ...
    DP_VERBOSE(p_hwfn, ECORE_MSG_LINK,
               ...
               info->mfw_mb_shadow[i], info->mfw_mb_cur[i]);
    ...
    case MFW_DRV_MSG_LLDP_DATA_UPDATED:
      ecore_dcbx_mib_update_event(p_hwfn, p_ptt, ECORE_DCBX_REMOTE_LLDP_MIB);
      break;
    case MFW_DRV_MSG_DCBX_REMOTE_MIB_UPDATED:
      ecore_dcbx_mib_update_event(p_hwfn, p_ptt, ECORE_DCBX_REMOTE_MIB);
      break;
    case MFW_DRV_MSG_DCBX_OPERATIONAL_MIB_UPDATED:
      ecore_dcbx_mib_update_event(p_hwfn, p_ptt, ECORE_DCBX_OPERATIONAL_MIB);
      /* clear the user-config cache */
      OSAL_MEMSET(&p_hwfn->p_dcbx_info->set, 0, sizeof(struct ecore_dcbx_set));
      break;
  ...
  for (i = 0; i < MFW_DRV_MSG_MAX_DWORDS(info->mfw_mb_length); i++) {
    ...
    ecore_wr(p_hwfn, p_ptt,
             info->mfw_mb_addr + sizeof(u32) +
                 MFW_DRV_MSG_MAX_DWORDS(info->mfw_mb_length) * sizeof(u32) +
                 i * sizeof(u32),
             val);
  }
...
enum _ecore_status_t ecore_mcp_get_mfw_ver(struct ecore_hwfn *p_hwfn,
  ...
  global_offsize = ecore_rd(
      p_hwfn, p_ptt,
      SECTION_OFFSIZE_ADDR(p_hwfn->mcp_info->public_base, PUBLIC_GLOBAL));
  *p_mfw_ver = ecore_rd(p_hwfn, p_ptt,
  ...
  if (p_running_bundle_id != OSAL_NULL) {
    *p_running_bundle_id =
        ecore_rd(p_hwfn, p_ptt,
                 SECTION_ADDR(global_offsize, 0) +
                     OFFSETOF(struct public_global, running_bundle_id));
  }

  return ECORE_SUCCESS;
}

int ecore_mcp_get_mbi_ver(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                          u32 *p_mbi_ver) {
  ...
  *p_mbi_ver =
      ecore_rd(p_hwfn, p_ptt, mbi_ver_addr) &
      (NVM_CFG1_GLOB_MBI_VERSION_0_MASK | NVM_CFG1_GLOB_MBI_VERSION_1_MASK |
...
enum _ecore_status_t ecore_mcp_get_transceiver_data(struct ecore_hwfn *p_hwfn,
  ...
  *p_transceiver_state = GET_MFW_FIELD(transceiver_info, ETH_TRANSCEIVER_STATE);

  if (*p_transceiver_state == ETH_TRANSCEIVER_STATE_PRESENT) {
    *p_transceiver_type = GET_MFW_FIELD(transceiver_info, ETH_TRANSCEIVER_TYPE);
  } else {
...
enum _ecore_status_t ecore_mcp_get_board_config(struct ecore_hwfn *p_hwfn,
  ...
  if (!p_ptt) {
    ...
    *p_board_config =
        ecore_rd(p_hwfn, p_ptt,
                 port_cfg_addr + offsetof(struct nvm_cfg1_port, board_cfg));
  }
...
ecore_mcp_get_shmem_proto_mfw(struct ecore_hwfn *p_hwfn,
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_IFUP,
             "According to capabilities, L2 personality is %08x [resp %08x "
             "param %08x]\n",
...
static enum _ecore_status_t
ecore_mcp_get_shmem_proto(struct ecore_hwfn *p_hwfn, struct public_func *p_info,
                          struct ecore_ptt *p_ptt,
                          enum ecore_pci_personality *p_proto) {
  enum _ecore_status_t rc = ECORE_SUCCESS;
  ...
  case FUNC_MF_CFG_PROTOCOL_ETHERNET:
    if (ecore_mcp_get_shmem_proto_mfw(p_hwfn, p_ptt, p_proto) != ECORE_SUCCESS)
...
enum _ecore_status_t ecore_mcp_fill_shmem_func_info(struct ecore_hwfn *p_hwfn,
  ...
  info->pause_on_host =
      (shmem_info.config & FUNC_MF_CFG_PAUSE_ON_HOST_RING) ? 1 : 0;

  if (ecore_mcp_get_shmem_proto(p_hwfn, &shmem_info, p_ptt, &info->protocol)) {
  ...
  DP_VERBOSE(p_hwfn, (ECORE_MSG_SP | ECORE_MSG_IFUP),
             ...
             " node %lx ovlan %04x\n",
             info->pause_on_host, info->protocol, info->bandwidth_min,
             info->bandwidth_max, info->mac[0], info->mac[1], info->mac[2],
             info->mac[3], info->mac[4], info->mac[5],
             (unsigned long)info->wwn_port, (unsigned long)info->wwn_node,
             info->ovlan);
...
enum _ecore_status_t ecore_mcp_drain(struct ecore_hwfn *p_hwfn,
  ...
  rc =
      ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_NIG_DRAIN, 1000, &resp, &param);
...
int ecore_mcp_get_personality_cnt(struct ecore_hwfn *p_hwfn,
  ...
  for (i = 0; i < num_pfs; i++) {
    ...
    if (ecore_mcp_get_shmem_proto(p_hwfn, &shmem_info, p_ptt, &protocol) !=
        ECORE_SUCCESS)
      continue;
...
static enum _ecore_status_t
ecore_mcp_config_vf_msix_bb(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                            u8 vf_id, u8 num) {
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_CFG_VF_MSIX, param, &resp,
                     &rc_param);
...
static enum _ecore_status_t
ecore_mcp_config_vf_msix_ah(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                            u8 num) {
  u32 resp = 0, param = num, rc_param = 0;
  enum _ecore_status_t rc;

  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_CFG_PF_VFS_MSIX, param, &resp,
                     &rc_param);
...
enum _ecore_status_t ecore_mcp_config_vf_msix(struct ecore_hwfn *p_hwfn,
                                              struct ecore_ptt *p_ptt, u8 vf_id,
                                              u8 num) {
#ifndef ASIC_ONLY
  if (CHIP_REV_IS_EMUL(p_hwfn->p_dev) && !ecore_mcp_is_init(p_hwfn)) {
    DP_INFO(p_hwfn, "Emulation: Avoid sending the %s mailbox command\n",
            ECORE_IS_BB(p_hwfn->p_dev) ? "CFG_VF_MSIX" : "CFG_PF_VFS_MSIX");
    return ECORE_SUCCESS;
  }
#endif
...
enum _ecore_status_t ecore_mcp_halt(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_MCP_HALT, 0, &resp, &param);
  if (rc != ECORE_SUCCESS) {
    DP_ERR(p_hwfn, "MCP response failure, aborting\n");
    return rc;
  }
...
enum _ecore_status_t ecore_mcp_ov_update_mtu(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_OV_UPDATE_MTU, drv_mb_param,
                     &resp, &param);
  if (rc != ECORE_SUCCESS)
...
enum _ecore_status_t ecore_mcp_ov_update_mac(struct ecore_hwfn *p_hwfn,
                                             struct ecore_ptt *p_ptt, u8 *mac) {
  struct ecore_mcp_mb_params mb_params;
  union drv_union_data union_data;
  enum _ecore_status_t rc;
...
enum _ecore_status_t ecore_mcp_set_led(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_SET_LED_MODE, drv_mb_param,
                     &resp, &param);
  if (rc != ECORE_SUCCESS)
    DP_ERR(p_hwfn, "MCP response failure, aborting\n");
...
enum _ecore_status_t ecore_mcp_mask_parities(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_MASK_PARITIES, mask_parities,
                     &resp, &param);

  if (rc != ECORE_SUCCESS) {
    DP_ERR(p_hwfn, "MCP response failure for mask parities, aborting\n");
  } else if (resp != FW_MSG_CODE_OK) {
    DP_ERR(p_hwfn, "MCP did not ack mask parity request. Old MFW?\n");
    rc = ECORE_INVAL;
  }
...
enum _ecore_status_t ecore_mcp_nvm_read(struct ecore_dev *p_dev, u32 addr,
  ...
  offset = 0;
  while (bytes_left > 0) {
    bytes_to_copy = OSAL_MIN_T(u32, bytes_left, MCP_DRV_NVM_BUF_LEN);
    nvm_offset =
        (addr + offset) | (bytes_to_copy << DRV_MB_PARAM_NVM_LEN_OFFSET);
    rc = ecore_mcp_nvm_rd_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_NVM_READ_NVRAM,
                              nvm_offset, &resp, &param, &buf_size,
                              (u32 *)(p_buf + offset));
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_dev, false, "ecore_mcp_nvm_rd_cmd() failed, rc = %d\n", rc);
      resp = FW_MSG_CODE_ERROR;
      break;
    }

    if (resp != FW_MSG_CODE_NVM_OK) {
      DP_NOTICE(p_dev, false, "nvm read failed, resp = 0x%08x\n", resp);
      rc = ECORE_UNKNOWN_ERROR;
      break;
    }
     ...
     */
    if (bytes_left % 0x1000 < (bytes_left - buf_size) % 0x1000)
      OSAL_MSLEEP(1);
...
enum _ecore_status_t ecore_mcp_nvm_del_file(struct ecore_dev *p_dev, u32 addr) {
  ...
  if (!p_ptt)
    return ECORE_BUSY;
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_NVM_DEL_FILE, addr, &resp,
                     &param);
  p_dev->mcp_nvm_resp = resp;
  ecore_ptt_release(p_hwfn, p_ptt);
...
enum _ecore_status_t ecore_mcp_nvm_write(struct ecore_dev *p_dev, u32 cmd,
  ...
  while (buf_idx < len) {
    ...
    nvm_offset = ((buf_size << DRV_MB_PARAM_NVM_LEN_OFFSET) | addr) + buf_idx;
    rc = ecore_mcp_nvm_wr_cmd(p_hwfn, p_ptt, nvm_cmd, nvm_offset, &resp, &param,
                              buf_size, (u32 *)&p_buf[buf_idx]);
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(p_dev, false, "ecore_mcp_nvm_write() failed, rc = %d\n", rc);
      resp = FW_MSG_CODE_ERROR;
      break;
    }

    if (resp != FW_MSG_CODE_OK && resp != FW_MSG_CODE_NVM_OK &&
        resp != FW_MSG_CODE_NVM_PUT_FILE_FINISH_OK) {
      DP_NOTICE(p_dev, false, "nvm write failed, resp = 0x%08x\n", resp);
      rc = ECORE_UNKNOWN_ERROR;
      break;
    }
...
enum _ecore_status_t ecore_mcp_phy_write(struct ecore_dev *p_dev, u32 cmd,
  ...
  rc = ecore_mcp_nvm_wr_cmd(p_hwfn, p_ptt, nvm_cmd, addr, &resp, &param, len,
                            (u32 *)p_buf);
  if (rc != ECORE_SUCCESS)
    DP_NOTICE(p_dev, false, "MCP command rc = %d\n", rc);
  p_dev->mcp_nvm_resp = resp;
...
enum _ecore_status_t ecore_mcp_nvm_set_secure_mode(struct ecore_dev *p_dev,
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_SET_SECURE_MODE, addr, &resp,
                     &param);
  p_dev->mcp_nvm_resp = resp;
  ecore_ptt_release(p_hwfn, p_ptt);
...
enum _ecore_status_t ecore_mcp_phy_sfp_read(struct ecore_hwfn *p_hwfn,
  ...
  bytes_left = len;
  while (bytes_left > 0) {
    bytes_to_copy = OSAL_MIN_T(u32, bytes_left, MAX_I2C_TRANSACTION_SIZE);
    nvm_offset &= (DRV_MB_PARAM_TRANSCEIVER_I2C_ADDRESS_MASK |
                   DRV_MB_PARAM_TRANSCEIVER_PORT_MASK);
    nvm_offset |= ((addr + offset) << DRV_MB_PARAM_TRANSCEIVER_OFFSET_OFFSET);
    nvm_offset |= (bytes_to_copy << DRV_MB_PARAM_TRANSCEIVER_SIZE_OFFSET);
    rc = ecore_mcp_nvm_rd_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_TRANSCEIVER_READ,
                              nvm_offset, &resp, &param, &buf_size,
                              (u32 *)(p_buf + offset));
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(
...
enum _ecore_status_t ecore_mcp_phy_sfp_write(struct ecore_hwfn *p_hwfn,
  ...
  buf_idx = 0;
  while (buf_idx < len) {
    buf_size = OSAL_MIN_T(u32, (len - buf_idx), MAX_I2C_TRANSACTION_SIZE);
    nvm_offset &= (DRV_MB_PARAM_TRANSCEIVER_I2C_ADDRESS_MASK |
                   DRV_MB_PARAM_TRANSCEIVER_PORT_MASK);
    nvm_offset |=
        ((offset + buf_idx) << DRV_MB_PARAM_TRANSCEIVER_OFFSET_OFFSET);
    nvm_offset |= (buf_size << DRV_MB_PARAM_TRANSCEIVER_SIZE_OFFSET);
    rc = ecore_mcp_nvm_wr_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_TRANSCEIVER_WRITE,
                              nvm_offset, &resp, &param, buf_size,
                              (u32 *)&p_buf[buf_idx]);
    if (rc != ECORE_SUCCESS) {
      DP_NOTICE(
...
enum _ecore_status_t ecore_mcp_gpio_read(struct ecore_hwfn *p_hwfn,
                                         struct ecore_ptt *p_ptt, u16 gpio,
                                         u32 *gpio_val) {
  enum _ecore_status_t rc = ECORE_SUCCESS;
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_GPIO_READ, drv_mb_param, &rsp,
                     gpio_val);
...
enum _ecore_status_t ecore_mcp_gpio_write(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_GPIO_WRITE, drv_mb_param, &rsp,
                     &param);
...
enum _ecore_status_t ecore_mcp_gpio_info(struct ecore_hwfn *p_hwfn,
                                         struct ecore_ptt *p_ptt, u16 gpio,
                                         u32 *gpio_direction, u32 *gpio_ctrl) {
  u32 drv_mb_param = 0, rsp, val = 0;
  enum _ecore_status_t rc = ECORE_SUCCESS;

  drv_mb_param = gpio << DRV_MB_PARAM_GPIO_NUMBER_OFFSET;

  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_GPIO_INFO, drv_mb_param, &rsp,
                     &val);
  if (rc != ECORE_SUCCESS)
    return rc;
  ...
  *gpio_ctrl =
      (val & DRV_MB_PARAM_GPIO_CTRL_MASK) >> DRV_MB_PARAM_GPIO_CTRL_OFFSET;
...
enum _ecore_status_t ecore_mcp_bist_register_test(struct ecore_hwfn *p_hwfn,
  ...
  drv_mb_param =
      (DRV_MB_PARAM_BIST_REGISTER_TEST << DRV_MB_PARAM_BIST_TEST_INDEX_OFFSET);

  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_BIST_TEST, drv_mb_param, &rsp,
                     &param);
...
enum _ecore_status_t ecore_mcp_bist_clock_test(struct ecore_hwfn *p_hwfn,
  ...
  drv_mb_param =
      (DRV_MB_PARAM_BIST_CLOCK_TEST << DRV_MB_PARAM_BIST_TEST_INDEX_OFFSET);
...
enum _ecore_status_t ecore_mcp_bist_nvm_test_get_image_att(
  ...
  nvm_offset = (DRV_MB_PARAM_BIST_NVM_TEST_IMAGE_BY_INDEX
                << DRV_MB_PARAM_BIST_TEST_INDEX_OFFSET);
  nvm_offset |= (image_index << DRV_MB_PARAM_BIST_TEST_IMAGE_INDEX_OFFSET);
  rc = ecore_mcp_nvm_rd_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_BIST_TEST, nvm_offset,
                            &resp, &param, &buf_size, (u32 *)p_image_att);
  if (rc != ECORE_SUCCESS)
    return rc;
...
enum _ecore_status_t ecore_mcp_bist_nvm_get_image_att(
    struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
    struct bist_nvm_image_att *p_image_att, u32 image_index) {
  ...
  SET_MFW_FIELD(nvm_offset, DRV_MB_PARAM_BIST_TEST_IMAGE_INDEX, image_index);
  rc = ecore_mcp_nvm_rd_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_BIST_TEST, nvm_offset,
                            &resp, &param, &buf_size, (u32 *)p_image_att);
  if (rc != ECORE_SUCCESS)
    return rc;
...
enum _ecore_status_t ecore_mcp_nvm_info_populate(struct ecore_hwfn *p_hwfn) {
...
#ifndef ASIC_ONLY
  if (CHIP_REV_IS_EMUL(p_hwfn->p_dev) || CHIP_REV_IS_TEDIBEAR(p_hwfn->p_dev))
    return ECORE_SUCCESS;
#endif
  ...
  rc = ecore_mcp_bist_nvm_get_num_images(p_hwfn, p_ptt, &nvm_info.num_images);
  if (rc == ECORE_NOTIMPL) {
  ...
  nvm_info.image_att =
      OSAL_ALLOC(p_hwfn->p_dev, GFP_KERNEL,
                 nvm_info.num_images * sizeof(struct bist_nvm_image_att));
  ...
  for (i = 0; i < nvm_info.num_images; i++) {
    rc = ecore_mcp_bist_nvm_get_image_att(p_hwfn, p_ptt, &nvm_info.image_att[i],
                                          i);
    if (rc != ECORE_SUCCESS) {
      DP_ERR(p_hwfn, "Failed getting image index %d attributes\n", i);
      goto err1;
    }
...
enum _ecore_status_t ecore_mcp_get_nvm_image(struct ecore_hwfn *p_hwfn,
  ...
  if (image_att.length > buffer_len) {
    DP_VERBOSE(
        p_hwfn, ECORE_MSG_STORAGE,
        "Image [%d] is too big - %08x bytes where only %08x are available\n",
        image_id, image_att.length, buffer_len);
    return ECORE_NOMEM;
  }

  return ecore_mcp_nvm_read(p_hwfn->p_dev, image_att.start_addr, (u8 *)p_buffer,
                            image_att.length);
}
...
ecore_mcp_get_temperature_info(struct ecore_hwfn *p_hwfn,
  ...
  p_temp_info->num_sensors =
      OSAL_MIN_T(u32, mfw_temp_info.num_of_sensors, ECORE_MAX_NUM_OF_SENSORS);
  for (i = 0; i < p_temp_info->num_sensors; i++) {
    ...
    p_temp_sensor->sensor_location =
        (val & SENSOR_LOCATION_MASK) >> SENSOR_LOCATION_OFFSET;
    p_temp_sensor->threshold_high =
        (val & THRESHOLD_HIGH_MASK) >> THRESHOLD_HIGH_OFFSET;
    p_temp_sensor->critical =
        (val & CRITICAL_TEMPERATURE_MASK) >> CRITICAL_TEMPERATURE_OFFSET;
    p_temp_sensor->current_temp =
        (val & CURRENT_TEMP_MASK) >> CURRENT_TEMP_OFFSET;
  }
...
enum _ecore_status_t
ecore_mcp_get_mba_versions(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
                           struct ecore_mba_vers *p_mba_vers) {
  u32 buf_size, resp, param;
  enum _ecore_status_t rc;

  rc = ecore_mcp_nvm_rd_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_GET_MBA_VERSION, 0,
                            &resp, &param, &buf_size, &p_mba_vers->mba_vers[0]);
...
enum _ecore_status_t ecore_mcp_mem_ecc_events(struct ecore_hwfn *p_hwfn,
  ...
  return ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_MEM_ECC_EVENTS, 0, &rsp,
                       (u32 *)num_events);
}
...
static enum _ecore_status_t ecore_mcp_resc_allocation_msg(
    struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  if (mfw_resc_info.res_id == RESOURCE_NUM_INVALID) {
    DP_ERR(p_hwfn, "Failed to match resource %d [%s] with the MFW resources\n",
           p_in_params->res_id, ecore_hw_get_resc_name(p_in_params->res_id));
    return ECORE_INVAL;
  }
  ...
  DP_VERBOSE(
      p_hwfn, ECORE_MSG_SP,
      "Resource message request: cmd 0x%08x, res_id %d [%s], hsi_version "
      "%d.%d, val 0x%x\n",
      p_in_params->cmd, p_in_params->res_id,
      ecore_hw_get_resc_name(p_in_params->res_id),
      GET_MFW_FIELD(mb_params.param, DRV_MB_PARAM_RESOURCE_ALLOC_VERSION_MAJOR),
      GET_MFW_FIELD(mb_params.param, DRV_MB_PARAM_RESOURCE_ALLOC_VERSION_MINOR),
  ...
  DP_VERBOSE(p_hwfn, ECORE_MSG_SP,
             "Resource message response: mfw_hsi_version %d.%d, num 0x%x, "
             "start 0x%x, vf_num 0x%x, vf_start 0x%x, flags 0x%08x\n",
...
enum _ecore_status_t ecore_mcp_set_resc_max_val(struct ecore_hwfn *p_hwfn,
                                                struct ecore_ptt *p_ptt,
                                                enum ecore_resources res_id,
                                                u32 resc_max_val,
                                                u32 *p_mcp_resp) {
  ...
  OSAL_MEM_ZERO(&out_params, sizeof(out_params));
  rc = ecore_mcp_resc_allocation_msg(p_hwfn, p_ptt, &in_params, &out_params);
  if (rc != ECORE_SUCCESS)
    return rc;
...
static enum _ecore_status_t ecore_mcp_resource_cmd(struct ecore_hwfn *p_hwfn,
  ...
  if (*p_mcp_resp == FW_MSG_CODE_UNSUPPORTED) {
    DP_INFO(p_hwfn, "The resource command is unsupported by the MFW\n");
    return ECORE_NOTIMPL;
  }
  ...
  if (*p_mcp_param == RESOURCE_OPCODE_UNKNOWN_CMD) {
    ...
    DP_NOTICE(p_hwfn, false,
              "The resource command is unknown to the MFW [param 0x%08x, "
              "opcode %d]\n",
              param, opcode);
    return ECORE_INVAL;
  }
...
__ecore_mcp_resc_lock(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  rc = ecore_mcp_resource_cmd(p_hwfn, p_ptt, param, &mcp_resp, &mcp_param);
  if (rc != ECORE_SUCCESS)
    return rc;
  ...
  default:
    DP_NOTICE(p_hwfn, false,
              "Unexpected opcode in resource lock response [mcp_param 0x%08x, "
              "opcode %d]\n",
              mcp_param, opcode);
    return ECORE_INVAL;
  }
  ...
  do {
    ...
    if (retry_cnt) {
      if (p_params->sleep_b4_retry) {
        u16 retry_interval_in_ms = DIV_ROUND_UP(p_params->retry_interval, 1000);
...
void ecore_mcp_resc_lock_default_init(struct ecore_resc_lock_params *p_lock,
  ...
  if (p_lock != OSAL_NULL) {
    ...
    if (b_is_permanent) {
      ...
      p_lock->retry_interval = ECORE_MCP_RESC_LOCK_RETRY_VAL_DFLT;
...
ecore_mcp_resc_unlock(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  default:
    DP_NOTICE(p_hwfn, false,
              "Unexpected opcode in resource unlock response [mcp_param "
              "0x%08x, opcode %d]\n",
              mcp_param, opcode);
    return ECORE_INVAL;
  }
...
enum _ecore_status_t ecore_mcp_get_capabilities(struct ecore_hwfn *p_hwfn,
  ...
  rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_GET_MFW_FEATURE_SUPPORT, 0,
                     &mcp_resp, &p_hwfn->mcp_info->capabilities);
  if (rc == ECORE_SUCCESS)
...
enum _ecore_status_t ecore_mcp_set_capabilities(struct ecore_hwfn *p_hwfn,
  ...
  return ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_FEATURE_SUPPORT, features,
                       &mcp_resp, &mcp_param);
}
...
ecore_mcp_drv_attribute(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  SET_MFW_FIELD(mb_params.param, DRV_MB_PARAM_ATTRIBUTE_KEY,
                p_drv_attr->attr_num);
  SET_MFW_FIELD(mb_params.param, DRV_MB_PARAM_ATTRIBUTE_CMD, mfw_attr_cmd);
  ...
  if (mb_params.mcp_resp == FW_MSG_CODE_UNSUPPORTED) {
    DP_INFO(p_hwfn, "The attribute command is not supported by the MFW\n");
    return ECORE_NOTIMPL;
  } else if (mb_params.mcp_resp != FW_MSG_CODE_OK) {
    DP_INFO(p_hwfn,
            "Failed to send an attribute command [mcp_resp 0x%x, attr_cmd %d, "
            "attr_num %d]\n",
            mb_params.mcp_resp, p_drv_attr->attr_cmd, p_drv_attr->attr_num);
    return ECORE_INVAL;
  }

  DP_VERBOSE(p_hwfn, ECORE_MSG_SP,
             "Attribute Command: cmd %d [mfw_cmd %d], num %d, in={val 0x%08x, "
             "mask 0x%08x, offset 0x%08x}, out={val 0x%08x}\n",
...
enum _ecore_status_t ecore_mcp_get_engine_config(struct ecore_hwfn *p_hwfn,
  ...
  fir_valid =
      GET_MFW_FIELD(mb_params.mcp_param, FW_MB_PARAM_ENG_CFG_FIR_AFFIN_VALID);
  if (fir_valid)
    p_dev->fir_affin =
        GET_MFW_FIELD(mb_params.mcp_param, FW_MB_PARAM_ENG_CFG_FIR_AFFIN_VALUE);

  l2_valid =
      GET_MFW_FIELD(mb_params.mcp_param, FW_MB_PARAM_ENG_CFG_L2_AFFIN_VALID);
  if (l2_valid)
    p_dev->l2_affin_hint =
        GET_MFW_FIELD(mb_params.mcp_param, FW_MB_PARAM_ENG_CFG_L2_AFFIN_VALUE);

  DP_INFO(p_hwfn,
          "Engine affinity config: FIR={valid %hhd, value %hhd}, "
          "L2_hint={valid %hhd, value %hhd}\n",
...
enum _ecore_status_t ecore_mcp_get_ppfid_bitmap(struct ecore_hwfn *p_hwfn,
  ...
  if (mb_params.mcp_resp == FW_MSG_CODE_UNSUPPORTED) {
    DP_INFO(p_hwfn, "The get_ppfid_bitmap command is unsupported by the MFW\n");
    return ECORE_NOTIMPL;
  }

  p_dev->ppfid_bitmap =
      GET_MFW_FIELD(mb_params.mcp_param, FW_MB_PARAM_PPFID_BITMAP);
...
void ecore_mcp_wol_wr(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
  ...
  rc = ecore_mcp_cmd_and_union(p_hwfn, p_ptt, &mb_params);
  if (rc != ECORE_SUCCESS) {
    DP_NOTICE(p_hwfn, false, "Failed to wol write request, rc = %d\n", rc);
  }
