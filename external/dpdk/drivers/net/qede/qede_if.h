...
struct qed_eth_ops {
  ...
  int (*fill_dev_info)(struct ecore_dev *edev, struct qed_dev_eth_info *info);
...
struct qed_common_ops {
  int (*probe)(struct ecore_dev *edev, struct rte_pci_device *pci_dev,
  ...
  enum _ecore_status_t (*chain_alloc)(struct ecore_dev *edev,
                                      ...
                                      uint32_t num_elems, osal_size_t elem_size,
                                      struct ecore_chain *p_chain,
                                      struct ecore_chain_ext_pbl *ext_pbl);

  void (*chain_free)(struct ecore_dev *edev, struct ecore_chain *p_chain);

  void (*get_link)(struct ecore_dev *edev, struct qed_link_output *if_link);
  int (*set_link)(struct ecore_dev *edev, struct qed_link_params *params);
  ...
  uint32_t (*sb_init)(struct ecore_dev *edev, struct ecore_sb_info *sb_info,
                      void *sb_virt_addr, dma_addr_t sb_phy_addr,
                      uint16_t sb_id);

  int (*get_sb_info)(struct ecore_dev *edev, struct ecore_sb_info *sb, u16 qid,
                     struct ecore_sb_info_dbg *sb_dbg);

  bool (*can_link_change)(struct ecore_dev *edev);

  void (*update_msglvl)(struct ecore_dev *edev, uint32_t dp_module,
                        uint8_t dp_level);
  ...
  int (*dbg_grc)(struct ecore_dev *edev, void *buffer, u32 *num_dumped_bytes);
  int (*dbg_grc_size)(struct ecore_dev *edev);

  int (*dbg_idle_chk)(struct ecore_dev *edev, void *buffer,
                      u32 *num_dumped_bytes);
  int (*dbg_idle_chk_size)(struct ecore_dev *edev);

  int (*dbg_reg_fifo)(struct ecore_dev *edev, void *buffer,
                      u32 *num_dumped_bytes);
  int (*dbg_reg_fifo_size)(struct ecore_dev *edev);

  int (*dbg_mcp_trace)(struct ecore_dev *edev, void *buffer,
                       u32 *num_dumped_bytes);
  ...
  int (*dbg_ilt)(struct ecore_dev *edev, void *buffer, u32 *num_dumped_bytes);

  int (*dbg_ilt_size)(struct ecore_dev *edev);

  u8 (*dbg_get_debug_engine)(struct ecore_dev *edev);
  void (*dbg_set_debug_engine)(struct ecore_dev *edev, int engine_number);
};
