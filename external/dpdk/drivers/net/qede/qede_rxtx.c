...
static inline int qede_alloc_rx_bulk_mbufs(struct qede_rx_queue *rxq,
  ...
  ret =
      rte_mempool_get_bulk(rxq->mb_pool, (void **)&rxq->sw_rx_ring[idx], count);

  if (unlikely(ret)) {
    PMD_RX_LOG(ERR, rxq,
               "Failed to allocate %d rx buffers "
               "sw_rx_prod %u sw_rx_cons %u mp entries %u free %u",
               count, rxq->sw_rx_prod & NUM_RX_BDS(rxq),
...
int qede_calc_rx_buf_size(struct rte_eth_dev *dev, uint16_t mbufsz,
  ...
  if (dev->data->scattered_rx) {
    ...
    if ((mbufsz * ETH_RX_MAX_BUFF_PER_PKT) <
      ...
      DP_ERR(edev,
             "mbuf %d size is not enough to hold max fragments (%d) for max rx "
             "packet length (%d)\n",
             mbufsz, ETH_RX_MAX_BUFF_PER_PKT, max_frame_size);
      return -EINVAL;
    }

    rx_buf_size = RTE_MAX(mbufsz, (max_frame_size + QEDE_ETH_OVERHEAD) /
...
qede_alloc_rx_queue_mem(struct rte_eth_dev *dev, uint16_t queue_idx,
                        uint16_t nb_desc, unsigned int socket_id,
                        struct rte_mempool *mp, uint16_t bufsz) {
  struct qede_dev *qdev = QEDE_INIT_QDEV(dev);
  struct ecore_dev *edev = QEDE_INIT_EDEV(qdev);
  ...
  if (!rxq) {
    DP_ERR(edev, "Unable to allocate memory for rxq on socket %u", socket_id);
    return NULL;
  }
  ...
  DP_INFO(edev, "mtu %u mbufsz %u bd_max_bytes %u scatter_mode %d\n", qdev->mtu,
          bufsz, rxq->rx_buf_size, dev->data->scattered_rx);

  /* Allocate the parallel driver ring for Rx buffers */
  size = sizeof(*rxq->sw_rx_ring) * rxq->nb_rx_desc;
  rxq->sw_rx_ring =
      rte_zmalloc_socket("sw_rx_ring", size, RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq->sw_rx_ring) {
    DP_ERR(edev,
           "Memory allocation fails for sw_rx_ring on"
           " socket %u\n",
           socket_id);
    rte_free(rxq);
    return NULL;
  ...
  /* Allocate FW Rx ring  */
  rc = qdev->ops->common->chain_alloc(
      edev, ECORE_CHAIN_USE_TO_CONSUME_PRODUCE, ECORE_CHAIN_MODE_NEXT_PTR,
      ECORE_CHAIN_CNT_TYPE_U16, rxq->nb_rx_desc, sizeof(struct eth_rx_bd),
      &rxq->rx_bd_ring, NULL);

  if (rc != ECORE_SUCCESS) {
    DP_ERR(edev,
           "Memory allocation fails for RX BD ring"
           " on socket %u\n",
           socket_id);
  ...
  rc = qdev->ops->common->chain_alloc(
      edev, ECORE_CHAIN_USE_TO_CONSUME, ECORE_CHAIN_MODE_PBL,
      ECORE_CHAIN_CNT_TYPE_U16, rxq->nb_rx_desc, sizeof(union eth_rx_cqe),
      &rxq->rx_comp_ring, NULL);

  if (rc != ECORE_SUCCESS) {
    DP_ERR(edev,
           "Memory allocation fails for RX CQE ring"
           " on socket %u\n",
           socket_id);
...
int qede_rx_queue_setup(struct rte_eth_dev *dev, uint16_t qid, uint16_t nb_desc,
  ...
  if (ECORE_IS_CMT(edev)) {
    rxq = qede_alloc_rx_queue_mem(dev, qid * 2, nb_desc, socket_id, mp, bufsz);
    if (!rxq)
      return -ENOMEM;

    qdev->fp_array[qid * 2].rxq = rxq;
    rxq = qede_alloc_rx_queue_mem(dev, qid * 2 + 1, nb_desc, socket_id, mp,
                                  bufsz);
    if (!rxq)
      return -ENOMEM;
    ...
    rxq = qede_alloc_rx_queue_mem(dev, qid, nb_desc, socket_id, mp, bufsz);
    if (!rxq)
      return -ENOMEM;
  ...
  DP_INFO(edev, "rxq %d num_desc %u rx_buf_size=%u socket %u\n", qid, nb_desc,
          rxq->rx_buf_size, socket_id);

  return 0;
}

static void qede_rx_queue_reset(__rte_unused struct qede_dev *qdev,
                                struct qede_rx_queue *rxq) {
...
static int qede_rx_queue_stop(struct rte_eth_dev *eth_dev,
  ...
  if (rx_queue_id < qdev->num_rx_queues) {
    ...
    rc = ecore_eth_rx_queue_stop(p_hwfn, rxq->handle, true, false);
    if (rc != ECORE_SUCCESS) {
...
qede_alloc_tx_queue_mem(struct rte_eth_dev *dev, uint16_t queue_idx,
                        uint16_t nb_desc, unsigned int socket_id,
  ...
  if (txq == NULL) {
    DP_ERR(edev, "Unable to allocate memory for txq on socket %u", socket_id);
    return NULL;
  }
  ...
  rc = qdev->ops->common->chain_alloc(
      edev, ECORE_CHAIN_USE_TO_CONSUME_PRODUCE, ECORE_CHAIN_MODE_PBL,
      ECORE_CHAIN_CNT_TYPE_U16, txq->nb_tx_desc, sizeof(union eth_tx_bd_types),
      &txq->tx_pbl, NULL);
  if (rc != ECORE_SUCCESS) {
    DP_ERR(edev, "Unable to allocate memory for txbd ring on socket %u",
           socket_id);
  ...
  txq->sw_tx_ring = rte_zmalloc_socket("txq->sw_tx_ring", sw_tx_ring_size,
                                       RTE_CACHE_LINE_SIZE, socket_id);
  ...
  DP_INFO(edev, "txq %u num_desc %u tx_free_thresh %u socket %u\n", queue_idx,
          nb_desc, txq->tx_free_thresh, socket_id);
  return txq;
}

int qede_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                        uint16_t nb_desc, unsigned int socket_id,
                        const struct rte_eth_txconf *tx_conf) {
  struct qede_dev *qdev = dev->data->dev_private;
  struct ecore_dev *edev = &qdev->edev;
  ...
  if (ECORE_IS_CMT(edev)) {
    txq = qede_alloc_tx_queue_mem(dev, queue_idx * 2, nb_desc, socket_id,
                                  tx_conf);
    if (!txq)
      return -ENOMEM;

    qdev->fp_array[queue_idx * 2].txq = txq;
    txq = qede_alloc_tx_queue_mem(dev, (queue_idx * 2) + 1, nb_desc, socket_id,
                                  tx_conf);
    if (!txq)
      return -ENOMEM;

    qdev->fp_array[(queue_idx * 2) + 1].txq = txq;
    dev->data->tx_queues[queue_idx] = &qdev->fp_array_cmt[queue_idx];
  } else {
    txq = qede_alloc_tx_queue_mem(dev, queue_idx, nb_desc, socket_id, tx_conf);
    if (!txq)
      return -ENOMEM;
...
static void qede_tx_queue_reset(__rte_unused struct qede_dev *qdev,
                                struct qede_tx_queue *txq) {
...
static int qede_alloc_mem_sb(struct qede_dev *qdev,
                             struct ecore_sb_info *sb_info, uint16_t sb_id) {
  struct ecore_dev *edev = QEDE_INIT_EDEV(qdev);
  ...
  sb_virt =
      OSAL_DMA_ALLOC_COHERENT(edev, &sb_phys, sizeof(struct status_block));
  if (!sb_virt) {
    DP_ERR(edev, "Status block allocation failed\n");
    return -ENOMEM;
  }
  rc = qdev->ops->common->sb_init(edev, sb_info, sb_virt, sb_phys, sb_id);
  if (rc) {
    DP_ERR(edev, "Status block initialization failed\n");
    OSAL_DMA_FREE_COHERENT(edev, sb_virt, sb_phys, sizeof(struct status_block));
    return rc;
  }
...
int qede_alloc_fp_resc(struct qede_dev *qdev) {
  ...
  if (IS_VF(edev))
    ...
    num_sbs = ecore_cxt_get_proto_cid_count(ECORE_LEADING_HWFN(edev),
                                            PROTOCOLID_ETH, NULL);
  ...
  memset((void *)qdev->fp_array, 0,
         QEDE_RXTX_MAX(qdev) * sizeof(*qdev->fp_array));

  if (ECORE_IS_CMT(edev)) {
    qdev->fp_array_cmt =
        rte_calloc("fp_cmt", QEDE_RXTX_MAX(qdev) / 2,
                   sizeof(*qdev->fp_array_cmt), RTE_CACHE_LINE_SIZE);
  ...
  for (sb_idx = 0; sb_idx < QEDE_RXTX_MAX(qdev); sb_idx++) {
    ...
    fp->sb_info =
        rte_calloc("sb", 1, sizeof(struct ecore_sb_info), RTE_CACHE_LINE_SIZE);
    if (!fp->sb_info) {
...
void qede_dealloc_fp_resc(struct rte_eth_dev *eth_dev) {
  ...
  for (sb_idx = 0; sb_idx < QEDE_RXTX_MAX(qdev); sb_idx++) {
    ...
    if (fp->sb_info) {
      ...
      OSAL_DMA_FREE_COHERENT(edev, fp->sb_info->sb_virt, fp->sb_info->sb_phys,
...
static inline void qede_update_rx_prod(__rte_unused struct qede_dev *edev,
                                       struct qede_rx_queue *rxq) {
...
static int qede_rx_queue_start(struct rte_eth_dev *eth_dev,
                               uint16_t rx_queue_id) {
  struct qede_dev *qdev = QEDE_INIT_QDEV(eth_dev);
  struct ecore_dev *edev = QEDE_INIT_EDEV(qdev);
  ...
  if (rx_queue_id < qdev->num_rx_queues) {
    ...
    rc = ecore_eth_rx_queue_start(
        p_hwfn, p_hwfn->hw_info.opaque_fid, &params, fp->rxq->rx_buf_size,
        fp->rxq->rx_bd_ring.p_phys_addr, p_phys_table, page_cnt, &ret_params);
    if (rc) {
...
static int qede_tx_queue_start(struct rte_eth_dev *eth_dev,
  ...
  if (tx_queue_id < qdev->num_tx_queues) {
    ...
    rc = ecore_eth_tx_queue_start(p_hwfn, p_hwfn->hw_info.opaque_fid, &params,
                                  0 /* tc */, p_phys_table, page_cnt,
                                  &ret_params);
    if (rc != ECORE_SUCCESS) {
...
static inline void qede_process_tx_compl(__rte_unused struct ecore_dev *edev,
                                         struct qede_tx_queue *txq) {
  uint16_t hw_bd_cons;
  uint16_t sw_tx_cons;
  uint16_t remaining;
...
static int qede_drain_txq(struct qede_dev *qdev, struct qede_tx_queue *txq,
                          bool allow_drain) {
  struct ecore_dev *edev = &qdev->edev;
  int rc, cnt = 1000;
  ...
  while (txq->sw_tx_cons != txq->sw_tx_prod) {
    ...
    if (!cnt) {
      ...
      DP_ERR(edev,
             ...
             "PROD=%d, CONS=%d\n",
             txq->queue_id, txq->sw_tx_prod, txq->sw_tx_cons);
      return -1;
    }
    cnt--;
    DELAY(1000);
...
static int qede_tx_queue_stop(struct rte_eth_dev *eth_dev,
                              uint16_t tx_queue_id) {
  struct qede_dev *qdev = QEDE_INIT_QDEV(eth_dev);
  struct ecore_dev *edev = QEDE_INIT_EDEV(qdev);
  struct ecore_hwfn *p_hwfn;
...
static inline uint32_t qede_rx_cqe_to_pkt_type_inner(uint16_t flags) {
  ...
  /* Lookup table */
  static const uint32_t ptype_lkup_tbl[QEDE_PKT_TYPE_MAX] __rte_cache_aligned =
      {
          [QEDE_PKT_TYPE_IPV4] =
              RTE_PTYPE_INNER_L3_IPV4 | RTE_PTYPE_INNER_L2_ETHER,
          [QEDE_PKT_TYPE_IPV6] =
              RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L2_ETHER,
          ...
          [QEDE_PKT_TYPE_IPV6_FRAG] = RTE_PTYPE_INNER_L3_IPV6 |
                                      ...
                                      RTE_PTYPE_INNER_L2_ETHER,
          /* VLANs */
          [QEDE_PKT_TYPE_IPV4_VLAN] =
              RTE_PTYPE_INNER_L3_IPV4 | RTE_PTYPE_INNER_L2_ETHER_VLAN,
          [QEDE_PKT_TYPE_IPV6_VLAN] =
              RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L2_ETHER_VLAN,
  ...
  static const uint32_t ptype_lkup_tbl[QEDE_PKT_TYPE_MAX] __rte_cache_aligned =
      {
          ...
          [QEDE_PKT_TYPE_IPV4_TCP] =
              RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP | RTE_PTYPE_L2_ETHER,
          [QEDE_PKT_TYPE_IPV6_TCP] =
              RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP | RTE_PTYPE_L2_ETHER,
          [QEDE_PKT_TYPE_IPV4_UDP] =
              RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP | RTE_PTYPE_L2_ETHER,
          [QEDE_PKT_TYPE_IPV6_UDP] =
              RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_UDP | RTE_PTYPE_L2_ETHER,
          /* Frags with no VLAN */
          [QEDE_PKT_TYPE_IPV4_FRAG] =
              RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_FRAG | RTE_PTYPE_L2_ETHER,
          [QEDE_PKT_TYPE_IPV6_FRAG] =
              RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_FRAG | RTE_PTYPE_L2_ETHER,
          /* VLANs */
          [QEDE_PKT_TYPE_IPV4_VLAN] =
              RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L2_ETHER_VLAN,
          [QEDE_PKT_TYPE_IPV6_VLAN] =
              RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L2_ETHER_VLAN,
          [QEDE_PKT_TYPE_IPV4_TCP_VLAN] =
              RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP | RTE_PTYPE_L2_ETHER_VLAN,
          [QEDE_PKT_TYPE_IPV6_TCP_VLAN] =
              RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP | RTE_PTYPE_L2_ETHER_VLAN,
          [QEDE_PKT_TYPE_IPV4_UDP_VLAN] =
              RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP | RTE_PTYPE_L2_ETHER_VLAN,
          [QEDE_PKT_TYPE_IPV6_UDP_VLAN] =
              RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_UDP | RTE_PTYPE_L2_ETHER_VLAN,
          /* Frags with VLAN */
          [QEDE_PKT_TYPE_IPV4_VLAN_FRAG] =
              RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_FRAG | RTE_PTYPE_L2_ETHER_VLAN,
          [QEDE_PKT_TYPE_IPV6_VLAN_FRAG] =
              RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_FRAG | RTE_PTYPE_L2_ETHER_VLAN,
      };
...
static inline uint8_t qede_check_notunn_csum_l3(struct rte_mbuf *m,
                                                uint16_t flag) {
...
static inline void qede_reuse_page(__rte_unused struct qede_dev *qdev,
                                   struct qede_rx_queue *rxq,
...
static inline void qede_recycle_rx_bd_ring(struct qede_rx_queue *rxq,
                                           struct qede_dev *qdev,
                                           uint8_t count) {
  struct rte_mbuf *curr_cons;
...
static inline void
qede_rx_process_tpa_cont_cqe(struct qede_dev *qdev, struct qede_rx_queue *rxq,
                             struct eth_fast_path_rx_tpa_cont_cqe *cqe) {
  PMD_RX_LOG(INFO, rxq, "TPA cont[%d] - len [%d]\n", cqe->tpa_agg_index,
             rte_le_to_cpu_16(cqe->len_list[0]));
...
static inline void
qede_rx_process_tpa_end_cqe(struct qede_dev *qdev, struct qede_rx_queue *rxq,
  ...
  PMD_RX_LOG(INFO, rxq,
             "TPA End[%d] reason %d cqe_len %d nb_segs %d"
             " pkt_len %d\n",
...
static inline uint32_t qede_rx_cqe_to_tunn_pkt_type(uint16_t flags) {
  ...
  static const uint32_t
      ptype_tunn_lkup_tbl[QEDE_PKT_TYPE_TUNN_MAX_TYPE] __rte_cache_aligned = {
          ...
          [QEDE_PKT_TYPE_TUNN_L2_TENID_NOEXIST_GRE] = RTE_PTYPE_TUNNEL_GRE,
          [QEDE_PKT_TYPE_TUNN_L2_TENID_NOEXIST_VXLAN] = RTE_PTYPE_TUNNEL_VXLAN,
          [QEDE_PKT_TYPE_TUNN_L2_TENID_EXIST_GENEVE] = RTE_PTYPE_TUNNEL_GENEVE,
          [QEDE_PKT_TYPE_TUNN_L2_TENID_EXIST_GRE] = RTE_PTYPE_TUNNEL_GRE,
          [QEDE_PKT_TYPE_TUNN_L2_TENID_EXIST_VXLAN] = RTE_PTYPE_TUNNEL_VXLAN,
...
static inline int qede_process_sg_pkts(void *p_rxq, struct rte_mbuf *rx_mb,
  ...
  seg1 = rx_mb;
  while (num_segs) {
    cur_size = pkt_len > rxq->rx_buf_size ? rxq->rx_buf_size : pkt_len;
    if (unlikely(!cur_size)) {
      PMD_RX_LOG(ERR, rxq,
                 "Length is 0 while %u BDs"
...
static inline void print_rx_bd_info(struct rte_mbuf *m,
                                    struct qede_rx_queue *rxq,
                                    uint8_t bitfield) {
  PMD_RX_LOG(INFO, rxq,
...
uint16_t qede_recv_pkts_regular(void *p_rxq, struct rte_mbuf **rx_pkts,
                                uint16_t nb_pkts) {
  ...
  if (rxq->rx_alloc_count) {
    if (unlikely(qede_alloc_rx_bulk_mbufs(rxq, rxq->rx_alloc_count))) {
      ...
      dev->data->rx_mbuf_alloc_failed += rxq->rx_alloc_count;
  ...
  while (sw_comp_cons != hw_comp_cons) {
    ...
    if (likely(cqe_type == ETH_RX_CQE_TYPE_REGULAR)) {
      ...
      if (cqe_type == ETH_RX_CQE_TYPE_SLOW_PATH) {
        ...
        ecore_eth_cqe_completion(&edev->hwfns[rxq->queue_id % edev->num_hwfns],
    ...
    if (unlikely(qede_tunn_exist(parse_flag))) {
      ...
      if (unlikely(qede_check_tunn_csum_l4(parse_flag))) {
        PMD_RX_LOG(ERR, rxq, "L4 csum failed, flags = 0x%x\n", parse_flag);
        rxq->rx_hw_errors++;
      ...
      if (unlikely(qede_check_tunn_csum_l3(parse_flag))) {
        PMD_RX_LOG(ERR, rxq, "Outer L3 csum failed, flags = 0x%x\n",
                   parse_flag);
        rxq->rx_hw_errors++;
      ...
      /* Tunnel_type */
      packet_type = qede_rx_cqe_to_tunn_pkt_type(flags);

      /* Inner header */
      packet_type |= qede_rx_cqe_to_pkt_type_inner(parse_flag);
    ...
    if (unlikely(CQE_HAS_VLAN(parse_flag) || CQE_HAS_OUTER_VLAN(parse_flag))) {
  ...
  next_cqe:
    ...
    sw_comp_cons = ecore_chain_get_cons_idx(&rxq->rx_comp_ring);
    if (rx_pkt == nb_pkts) {
      PMD_RX_LOG(DEBUG, rxq, "Budget reached nb_pkts=%u received=%u", rx_pkt,
                 nb_pkts);
      break;
    }
  }
...
uint16_t qede_recv_pkts(void *p_rxq, struct rte_mbuf **rx_pkts,
  ...
  while (sw_comp_cons != hw_comp_cons) {
    ...
    case ETH_RX_CQE_TYPE_TPA_CONT:
      qede_rx_process_tpa_cont_cqe(qdev, rxq, &cqe->fast_path_tpa_cont);
      goto next_cqe;
    case ETH_RX_CQE_TYPE_TPA_END:
      qede_rx_process_tpa_end_cqe(qdev, rxq, &cqe->fast_path_tpa_end);
    ...
    if (!tpa_start_flg) {
      ...
      parse_flag = rte_le_to_cpu_16(cqe_start_tpa->pars_flags.flags);
    ...
    if (qede_tunn_exist(parse_flag)) {
      ...
      /* Tunnel_type */
      packet_type = qede_rx_cqe_to_tunn_pkt_type(tunn_parse_flag);
    ...
    if (CQE_HAS_VLAN(parse_flag) || CQE_HAS_OUTER_VLAN(parse_flag)) {
      /* Note: FW doesn't indicate Q-in-Q packet */
      ol_flags |= RTE_MBUF_F_RX_VLAN;
      if (qdev->vlan_strip_flg) {
        ol_flags |= RTE_MBUF_F_RX_VLAN_STRIPPED;
    ...
    if (!tpa_start_flg && fp_cqe->bd_num > 1) {
      PMD_RX_LOG(DEBUG, rxq,
                 "Jumbo-over-BD packet: %02x BDs"
                 " len on first: %04x Total Len: %04x",
                 fp_cqe->bd_num, len, pkt_len);
      num_segs = fp_cqe->bd_num - 1;
      seg1 = rx_mb;
      if (qede_process_sg_pkts(p_rxq, seg1, num_segs, pkt_len - len))
        goto next_cqe;
...
uint16_t qede_recv_pkts_cmt(void *p_fp_cmt, struct rte_mbuf **rx_pkts,
                            uint16_t nb_pkts) {
  ...
  eng1_pkts = qede_recv_pkts(fp_cmt->fp1->rxq, rx_pkts + eng0_pkts, eng1_pkts);
...
static inline uint16_t qede_encode_sg_bd(struct qede_tx_queue *p_txq,
  ...
  while (m_seg) {
    if (start_seg == 0) {
      if (!*bd2) {
        *bd2 = (struct eth_tx_2nd_bd *)ecore_chain_produce(&txq->tx_pbl);
      ...
      if (!*bd3) {
        *bd3 = (struct eth_tx_3rd_bd *)ecore_chain_produce(&txq->tx_pbl);
      ...
      tx_bd = (struct eth_tx_bd *)ecore_chain_produce(&txq->tx_pbl);
...
#ifdef RTE_LIBRTE_QEDE_DEBUG_TX
static inline void print_tx_bd_info(struct qede_tx_queue *txq,
  ...
  if (bd1)
    PMD_TX_LOG(
        INFO, txq, "BD1: nbytes=0x%04x nbds=0x%04x bd_flags=0x%04x bf=0x%04x",
        rte_cpu_to_le_16(bd1->nbytes), bd1->data.nbds,
        bd1->data.bd_flags.bitfields, rte_cpu_to_le_16(bd1->data.bitfields));
  if (bd2)
    PMD_TX_LOG(INFO, txq,
  ...
  if (bd3)
    PMD_TX_LOG(INFO, txq,
               ...
               bd3->data.tunn_l4_hdr_start_offset_w, bd3->data.tunn_hdr_size_w);
...
uint16_t
#ifdef RTE_LIBRTE_QEDE_DEBUG_TX
qede_xmit_prep_pkts(void *p_txq, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
  struct qede_tx_queue *txq = p_txq;
#else
...
#ifdef RTE_LIBRTE_QEDE_DEBUG_TX
static inline void qede_mpls_tunn_tx_sanity_check(struct rte_mbuf *mbuf,
                                                  struct qede_tx_queue *txq) {
  if (((mbuf->outer_l2_len + mbuf->outer_l3_len) / 2) > 0xff)
    PMD_TX_LOG(ERR, txq, "tunn_l4_hdr_start_offset overflow\n");
  if (((mbuf->outer_l2_len + mbuf->outer_l3_len + MPLSINUDP_HDR_SIZE) / 2) >
      0xff)
...
uint16_t qede_xmit_pkts_regular(void *p_txq, struct rte_mbuf **tx_pkts,
  ...
  if (unlikely(txq->nb_tx_avail < txq->tx_free_thresh)) {
    PMD_TX_LOG(DEBUG, txq, "send=%u avail=%u free_thresh=%u", nb_pkts,
               txq->nb_tx_avail, txq->tx_free_thresh);
    qede_process_tx_compl(edev, txq);
  }
  ...
  while (nb_tx_pkts--) {
    ...
    if (unlikely(txq->nb_tx_avail < ETH_TX_MIN_BDS_PER_NON_LSO_PKT))
      break;
    bd1_bf |= (mbuf->pkt_len & ETH_TX_DATA_1ST_BD_PKT_LEN_MASK)
              << ETH_TX_DATA_1ST_BD_PKT_LEN_SHIFT;

    /* Offload the IP checksum in the hardware */
    if (tx_ol_flags & RTE_MBUF_F_TX_IP_CKSUM)
      bd1_bd_flags_bf |= 1 << ETH_TX_1ST_BD_FLAGS_IP_CSUM_SHIFT;

    /* L4 checksum offload (tcp or udp) */
    if ((tx_ol_flags & (RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IPV6)) &&
        (tx_ol_flags & (RTE_MBUF_F_TX_UDP_CKSUM | RTE_MBUF_F_TX_TCP_CKSUM)))
      bd1_bd_flags_bf |= 1 << ETH_TX_1ST_BD_FLAGS_L4_CSUM_SHIFT;
    ...
    QEDE_BD_SET_ADDR_LEN(bd1, rte_mbuf_data_iova(mbuf), mbuf->data_len);
  ...
  PMD_TX_LOG(DEBUG, txq, "to_send=%u sent=%u bd_prod=%u core=%d", nb_pkts,
             nb_pkt_sent, TX_PROD(txq), rte_lcore_id());
  ...
  while (nb_tx_pkts--) {
    ...
    if (tunn_flg) {
      ...
      if (unlikely(txq->is_legacy)) {
        bd1_bf ^= 1 << ETH_TX_DATA_1ST_BD_TUNN_FLAG_SHIFT;
      }

      /* Outer IP checksum offload */
      if (tx_ol_flags &
          (RTE_MBUF_F_TX_OUTER_IP_CKSUM | RTE_MBUF_F_TX_OUTER_IPV4)) {
        bd1_bd_flags_bf |= ETH_TX_1ST_BD_FLAGS_TUNN_IP_CSUM_MASK
      ...
      if ((tx_ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK) ==
        ...
        tunn_hdr_size =
            (mbuf->outer_l2_len + mbuf->outer_l3_len + MPLSINUDP_HDR_SIZE) / 2;
        /* Inner L2 header size in two byte words */
        inner_l2_hdr_size = (mbuf->l2_len - MPLSINUDP_HDR_SIZE) / 2;
        ...
        bd2_bf1 |=
            (UNICAST_ADDRESS & ETH_TX_DATA_2ND_BD_TUNN_INNER_ETH_TYPE_MASK)
            << ETH_TX_DATA_2ND_BD_TUNN_INNER_ETH_TYPE_SHIFT;
        /* Treated as IPv6+Ext */
        bd2_bf1 |= 1 << ETH_TX_DATA_2ND_BD_TUNN_IPV6_EXT_SHIFT;

        /* Mark inner IPv6 if present */
        if (tx_ol_flags & RTE_MBUF_F_TX_IPV6)
          bd2_bf1 |= 1 << ETH_TX_DATA_2ND_BD_TUNN_INNER_IPV6_SHIFT;

        /* Inner L4 offsets */
        if ((tx_ol_flags & (RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IPV6)) &&
            (tx_ol_flags &
             (RTE_MBUF_F_TX_UDP_CKSUM | RTE_MBUF_F_TX_TCP_CKSUM))) {
          /* Determines if BD3 is needed */
          tunn_ipv6_ext_flg = true;
          if ((tx_ol_flags & RTE_MBUF_F_TX_L4_MASK) ==
              RTE_MBUF_F_TX_UDP_CKSUM) {
            bd2_bf1 |= 1 << ETH_TX_DATA_2ND_BD_L4_UDP_SHIFT;
          }

          /* TODO other pseudo checksum modes are
           * not supported
           */
          bd2_bf1 |= ETH_L4_PSEUDO_CSUM_CORRECT_LENGTH
    ...
    if (tx_ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
      lso_flg = true;
      if (unlikely(txq->nb_tx_avail < ETH_TX_MIN_BDS_PER_LSO_PKT))
        break;
      ...
      if (tunn_flg)
        hdr_size += mbuf->outer_l2_len + mbuf->outer_l3_len;
      ...
      /* Using one header BD */
      bd3_bf |= rte_cpu_to_le_16(1 << ETH_TX_DATA_3RD_BD_HDR_NBD_SHIFT);
    } else {
      if (unlikely(txq->nb_tx_avail < ETH_TX_MIN_BDS_PER_NON_LSO_PKT))
        break;
      bd1_bf |= (mbuf->pkt_len & ETH_TX_DATA_1ST_BD_PKT_LEN_MASK)
    ...
    if (tx_ol_flags & RTE_MBUF_F_TX_VLAN) {
      ...
      bd1_bd_flags_bf |= 1 << ETH_TX_1ST_BD_FLAGS_VLAN_INSERTION_SHIFT;
    }
    ...
    if (tx_ol_flags & RTE_MBUF_F_TX_IP_CKSUM) {
      ...
      if (tunn_flg && ((tx_ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK) !=
        ...
        bd1_bd_flags_bf |= ETH_TX_1ST_BD_FLAGS_TUNN_L4_CSUM_MASK
    ...
    if (lso_flg || mplsoudp_flg) {
      ...
      /* BD1 */
      QEDE_BD_SET_ADDR_LEN(bd1, rte_mbuf_data_iova(mbuf), hdr_size);
      /* BD2 */
      QEDE_BD_SET_ADDR_LEN(bd2, (hdr_size + rte_mbuf_data_iova(mbuf)),
                           mbuf->data_len - hdr_size);
      bd2->data.bitfields1 = rte_cpu_to_le_16(bd2_bf1);
      if (mplsoudp_flg) {
        bd2->data.bitfields2 = rte_cpu_to_le_16(bd2_bf2);
        /* Outer L3 size */
        bd2->data.tunn_ip_size = rte_cpu_to_le_16(mbuf->outer_l3_len);
      }
      /* BD3 */
      if (lso_flg || (mplsoudp_flg && tunn_ipv6_ext_flg)) {
        ...
        if (mplsoudp_flg) {
          bd3->data.tunn_l4_hdr_start_offset_w = tunn_l4_hdr_start_offset;
          bd3->data.tunn_hdr_size_w = tunn_hdr_size;
        }
      }
    }
...
uint16_t qede_xmit_pkts_cmt(void *p_fp_cmt, struct rte_mbuf **tx_pkts,
                            uint16_t nb_pkts) {
  struct qede_fastpath_cmt *fp_cmt = p_fp_cmt;
  uint16_t eng0_pkts, eng1_pkts;
  ...
  eng1_pkts = qede_xmit_pkts(fp_cmt->fp1->txq, tx_pkts + eng0_pkts, eng1_pkts);
 ...
 */
static uint16_t qede_parse_fp_cqe(struct qede_rx_queue *rxq) {
...
int qede_rx_descriptor_status(void *p_rxq, uint16_t offset) {
