...
int qede_get_regs_len(struct qede_dev *qdev) {
  ...
  if (qdev->ops && qdev->ops->common) {
    ...
    for (cur_engine = 0; cur_engine < num_of_hwfns; cur_engine++) {
      ...
      regs_len +=
          REGDUMP_HEADER_SIZE + qdev->ops->common->dbg_idle_chk_size(edev) +
          REGDUMP_HEADER_SIZE + qdev->ops->common->dbg_idle_chk_size(edev) +
          REGDUMP_HEADER_SIZE + qdev->ops->common->dbg_grc_size(edev) +
          REGDUMP_HEADER_SIZE + qdev->ops->common->dbg_reg_fifo_size(edev) +
          REGDUMP_HEADER_SIZE +
          qdev->ops->common->dbg_protection_override_size(edev) +
          REGDUMP_HEADER_SIZE + qdev->ops->common->dbg_igu_fifo_size(edev) +
          REGDUMP_HEADER_SIZE + qdev->ops->common->dbg_fw_asserts_size(edev);
    }
    /* compute required buffer size for mcp trace and add it to the
     * total required buffer size
     */
    regs_len +=
        REGDUMP_HEADER_SIZE + qdev->ops->common->dbg_mcp_trace_size(edev);
...
static uint32_t qede_calc_regdump_header(enum debug_print_features feature,
                                         int engine, uint32_t feature_size,
                                         uint8_t omit_engine) {
...
int qede_get_regs(struct rte_eth_dev *eth_dev, struct rte_dev_reg_info *regs) {
  ...
  for (cur_engine = 0; cur_engine < num_of_hwfns; cur_engine++) {
    ...
    /* first idle_chk */
    qdev->ops->common->dbg_idle_chk(
        edev, (uint8_t *)buffer + offset + REGDUMP_HEADER_SIZE, &feature_size);
    *(uint32_t *)((uint8_t *)buffer + offset) = qede_calc_regdump_header(
        IDLE_CHK, cur_engine, feature_size, omit_engine);
    ...
    *(uint32_t *)((uint8_t *)buffer + offset) = qede_calc_regdump_header(
    ...
    /* reg_fifo dump */
    qdev->ops->common->dbg_reg_fifo(
        edev, (uint8_t *)buffer + offset + REGDUMP_HEADER_SIZE, &feature_size);
    *(uint32_t *)((uint8_t *)buffer + offset) = qede_calc_regdump_header(
        REG_FIFO, cur_engine, feature_size, omit_engine);
    offset += (feature_size + REGDUMP_HEADER_SIZE);
    DP_NOTICE(edev, false, "Reg fifo feature_size %u\n", feature_size);

    /* igu_fifo dump */
    qdev->ops->common->dbg_igu_fifo(
        edev, (uint8_t *)buffer + offset + REGDUMP_HEADER_SIZE, &feature_size);
    *(uint32_t *)((uint8_t *)buffer + offset) = qede_calc_regdump_header(
        IGU_FIFO, cur_engine, feature_size, omit_engine);
    offset += (feature_size + REGDUMP_HEADER_SIZE);
    DP_NOTICE(edev, false, "IGU fifo feature_size %u\n", feature_size);

    /* protection_override dump */
    qdev->ops->common->dbg_protection_override(
        edev, (uint8_t *)buffer + offset + REGDUMP_HEADER_SIZE, &feature_size);
    *(uint32_t *)((uint8_t *)buffer + offset) = qede_calc_regdump_header(
        PROTECTION_OVERRIDE, cur_engine, feature_size, omit_engine);
    offset += (feature_size + REGDUMP_HEADER_SIZE);
    DP_NOTICE(edev, false, "Protection override feature_size %u\n",
              feature_size);

    /* fw_asserts dump */
    qdev->ops->common->dbg_fw_asserts(
        edev, (uint8_t *)buffer + offset + REGDUMP_HEADER_SIZE, &feature_size);
    *(uint32_t *)((uint8_t *)buffer + offset) = qede_calc_regdump_header(
        FW_ASSERTS, cur_engine, feature_size, omit_engine);
    offset += (feature_size + REGDUMP_HEADER_SIZE);
    DP_NOTICE(edev, false, "FW assert feature_size %u\n", feature_size);

    /* grc dump */
    qdev->ops->common->dbg_grc(
        edev, (uint8_t *)buffer + offset + REGDUMP_HEADER_SIZE, &feature_size);
    *(uint32_t *)((uint8_t *)buffer + offset) = qede_calc_regdump_header(
        GRC_DUMP, cur_engine, feature_size, omit_engine);
    offset += (feature_size + REGDUMP_HEADER_SIZE);
    DP_NOTICE(edev, false, "GRC dump feature_size %u\n", feature_size);
  }

  /* mcp_trace */
  qdev->ops->common->dbg_mcp_trace(
      edev, (uint8_t *)buffer + offset + REGDUMP_HEADER_SIZE, &feature_size);
  *(uint32_t *)((uint8_t *)buffer + offset) = qede_calc_regdump_header(
      MCP_TRACE, cur_engine, feature_size, omit_engine);
  offset += (feature_size + REGDUMP_HEADER_SIZE);
...
static void qede_set_fw_dump_file_name(struct qede_dev *qdev) {
  time_t ltime;
  struct tm *tm;
  ...
  snprintf(qdev->dump_file, QEDE_FW_DUMP_FILE_SIZE,
           "qede_pmd_dump_%02d-%02d-%02d_%02d-%02d-%02d.bin", tm->tm_mon + 1,
           (int)tm->tm_mday, 1900 + tm->tm_year, tm->tm_hour, tm->tm_min,
           tm->tm_sec);
}

static int qede_write_fwdump(const char *dump_file, void *dump, size_t len) {
  int err = 0;
  FILE *f;
  size_t bytes;
