...
static void
qed_sriov_enable_qid_config(struct ecore_hwfn *hwfn, u16 vfid,
...
static void qed_handle_vf_msg(struct ecore_hwfn *hwfn) {
  ...
  ecore_for_each_vf(hwfn, i) {
    ...
    DP_VERBOSE(hwfn, ECORE_MSG_IOV,
               ...
               hwfn->p_dev->p_iov_info->first_vf_in_pf + i);
...
static void qed_handle_bulletin_post(struct ecore_hwfn *hwfn) {
   ...
   * bulletins updated.
   */
  ecore_for_each_vf(hwfn, i) ecore_iov_post_vf_bulletin(hwfn, i, ptt);
...
void qed_iov_pf_task(void *arg) {
  ...
  if (OSAL_GET_BIT(QED_IOV_WQ_BULLETIN_UPDATE_FLAG, &p_hwfn->iov_task_flags)) {
    OSAL_CLEAR_BIT(QED_IOV_WQ_BULLETIN_UPDATE_FLAG, &p_hwfn->iov_task_flags);
...
void qed_inform_vf_link_state(struct ecore_hwfn *hwfn) {
  ...
  rte_memcpy(&params, ecore_mcp_get_link_params(lead_hwfn), sizeof(params));
  rte_memcpy(&link, ecore_mcp_get_link_state(lead_hwfn), sizeof(link));
  rte_memcpy(&caps, ecore_mcp_get_link_capabilities(lead_hwfn), sizeof(caps));

  /* Update bulletin of all future possible VFs with link configuration */
  for (i = 0; i < hwfn->p_dev->p_iov_info->total_vfs; i++) {
    ecore_iov_set_link(hwfn, i, &params, &link, &caps);
  }
