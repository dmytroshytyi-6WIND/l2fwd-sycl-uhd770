...
const struct _qede_udp_tunn_types {
  ...
  const char *string;
    ...
    {RTE_ETH_TUNNEL_FILTER_OMAC, ECORE_FILTER_MAC, ECORE_TUNN_CLSS_MAC_VLAN,
     "outer-mac"},
    {RTE_ETH_TUNNEL_FILTER_TENID, ECORE_FILTER_VNI, ECORE_TUNN_CLSS_MAC_VNI,
     "vni"},
    {RTE_ETH_TUNNEL_FILTER_IMAC, ECORE_FILTER_INNER_MAC,
     ECORE_TUNN_CLSS_INNER_MAC_VLAN, "inner-mac"},
    {RTE_ETH_TUNNEL_FILTER_IVLAN, ECORE_FILTER_INNER_VLAN,
     ECORE_TUNN_CLSS_INNER_MAC_VLAN, "inner-vlan"},
    {RTE_ETH_TUNNEL_FILTER_OMAC | RTE_ETH_TUNNEL_FILTER_TENID,
     ECORE_FILTER_MAC_VNI_PAIR, ECORE_TUNN_CLSS_MAC_VNI, "outer-mac and vni"},
    {RTE_ETH_TUNNEL_FILTER_OMAC | RTE_ETH_TUNNEL_FILTER_IMAC,
     ECORE_FILTER_UNUSED, MAX_ECORE_TUNN_CLSS, "outer-mac and inner-mac"},
    {RTE_ETH_TUNNEL_FILTER_OMAC | RTE_ETH_TUNNEL_FILTER_IVLAN,
     ECORE_FILTER_UNUSED, MAX_ECORE_TUNN_CLSS, "outer-mac and inner-vlan"},
    {
    ...
    {RTE_ETH_TUNNEL_FILTER_OIP, ECORE_FILTER_UNUSED, MAX_ECORE_TUNN_CLSS,
     "outer-IP"},
    {RTE_ETH_TUNNEL_FILTER_IIP, ECORE_FILTER_UNUSED, MAX_ECORE_TUNN_CLSS,
     "inner-IP"},
    {RTE_ETH_TUNNEL_FILTER_IMAC_IVLAN, ECORE_FILTER_UNUSED, MAX_ECORE_TUNN_CLSS,
     "IMAC_IVLAN"},
    {RTE_ETH_TUNNEL_FILTER_IMAC_IVLAN_TENID, ECORE_FILTER_UNUSED,
     MAX_ECORE_TUNN_CLSS, "IMAC_IVLAN_TENID"},
    {RTE_ETH_TUNNEL_FILTER_IMAC_TENID, ECORE_FILTER_UNUSED, MAX_ECORE_TUNN_CLSS,
     "IMAC_TENID"},
    {RTE_ETH_TUNNEL_FILTER_OMAC_TENID_IMAC, ECORE_FILTER_UNUSED,
     MAX_ECORE_TUNN_CLSS, "OMAC_TENID_IMAC"},
};
...
qede_arfs_construct_pkt(struct rte_eth_dev *eth_dev,
                        struct qede_arfs_entry *arfs, void *buff,
...
int qede_check_fdir_support(struct rte_eth_dev *eth_dev) {
  ...
  case RTE_FDIR_MODE_PERFECT:
    ...
    qdev->arfs_info.arfs.mode = ECORE_FILTER_CONFIG_MODE_5_TUPLE;
...
static int qede_config_arfs_filter(struct rte_eth_dev *eth_dev,
                                   struct qede_arfs_entry *arfs, bool add) {
  struct qede_dev *qdev = QEDE_INIT_QDEV(eth_dev);
  struct ecore_dev *edev = QEDE_INIT_EDEV(qdev);
  ...
  pkt_len = qede_arfs_construct_pkt(eth_dev, arfs, pkt, &qdev->arfs_info.arfs);
  if (pkt_len == 0) {
    rc = -EINVAL;
    goto err1;
  }
  ...
  if (add) {
    if (qdev->arfs_info.arfs.mode == ECORE_FILTER_CONFIG_MODE_DISABLE) {
      /* Force update */
      eth_dev->data->dev_conf.fdir_conf.mode = RTE_FDIR_MODE_PERFECT;
      qdev->arfs_info.arfs.mode = ECORE_FILTER_CONFIG_MODE_5_TUPLE;
  ...
  rc = ecore_configure_rfs_ntuple_filter(p_hwfn, NULL, &params);
  if (rc == ECORE_SUCCESS) {
    if (add) {
      arfs->pkt_len = pkt_len;
      arfs->mz = mz;
      SLIST_INSERT_HEAD(&qdev->arfs_info.arfs_list_head, arfs, list);
  ...
  if (qdev->arfs_info.filter_count == 0) {
    memset(&qdev->arfs_info.arfs, 0, sizeof(struct ecore_arfs_config_params));
...
static uint16_t qede_arfs_construct_pkt(struct rte_eth_dev *eth_dev,
...

{
  ...
  *ether_type = rte_cpu_to_be_16(arfs->tuple.eth_proto);
  ...
  case RTE_ETHER_TYPE_IPV6:
    ...
    ip6->vtc_flow = rte_cpu_to_be_32(QEDE_FDIR_IPV6_DEFAULT_VTC_FLOW);

    rte_memcpy(&ip6->src_addr, arfs->tuple.src_ipv6, IPV6_ADDR_LEN);
    rte_memcpy(&ip6->dst_addr, arfs->tuple.dst_ipv6, IPV6_ADDR_LEN);
...
static int qede_tunnel_update(struct qede_dev *qdev,
  ...
  for_each_hwfn(edev, i) {
    ...
    rc = ecore_sp_pf_update_tunn_cfg(p_hwfn, p_ptt, tunn_info,
...
static int qede_vxlan_enable(struct rte_eth_dev *eth_dev, uint8_t clss,
                             bool enable) {
  struct qede_dev *qdev = QEDE_INIT_QDEV(eth_dev);
  struct ecore_dev *edev = QEDE_INIT_EDEV(qdev);
  enum _ecore_status_t rc = ECORE_INVAL;
...
static int qede_geneve_enable(struct rte_eth_dev *eth_dev, uint8_t clss,
                              bool enable) {
  struct qede_dev *qdev = QEDE_INIT_QDEV(eth_dev);
  struct ecore_dev *edev = QEDE_INIT_EDEV(qdev);
  enum _ecore_status_t rc = ECORE_INVAL;
...
int qede_udp_dst_port_del(struct rte_eth_dev *eth_dev,
  ...
  case RTE_ETH_TUNNEL_TYPE_VXLAN:
    ...
    if (qdev->vxlan.enable && qdev->vxlan.num_filters == 0)
      return qede_vxlan_enable(eth_dev, ECORE_TUNN_CLSS_MAC_VLAN, false);
  ...
  case RTE_ETH_TUNNEL_TYPE_GENEVE:
    ...
    if (qdev->geneve.enable && qdev->geneve.num_filters == 0)
      return qede_geneve_enable(eth_dev, ECORE_TUNN_CLSS_MAC_VLAN, false);
...
int qede_udp_dst_port_add(struct rte_eth_dev *eth_dev,
  ...
  case RTE_ETH_TUNNEL_TYPE_VXLAN:
    if (qdev->vxlan.udp_port == tunnel_udp->udp_port) {
      DP_INFO(edev, "UDP port %u for VXLAN was already configured\n",
              tunnel_udp->udp_port);
      return ECORE_SUCCESS;
    }
    ...
    if (!qdev->vxlan.enable) {
      rc = qede_vxlan_enable(eth_dev, ECORE_TUNN_CLSS_MAC_VLAN, true);
      if (rc != ECORE_SUCCESS) {
  ...
  case RTE_ETH_TUNNEL_TYPE_GENEVE:
    if (qdev->geneve.udp_port == tunnel_udp->udp_port) {
      DP_INFO(edev, "UDP port %u for GENEVE was already configured\n",
              tunnel_udp->udp_port);
      return ECORE_SUCCESS;
    }
    ...
    if (!qdev->geneve.enable) {
      rc = qede_geneve_enable(eth_dev, ECORE_TUNN_CLSS_MAC_VLAN, true);
      if (rc != ECORE_SUCCESS) {
...
static int qede_flow_validate_attr(__rte_unused struct rte_eth_dev *dev,
  ...
  if (attr->priority != 0) {
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY, attr,
  ...
  if (attr->transfer != 0) {
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_TRANSFER, attr,
...
static int qede_flow_parse_pattern(__rte_unused struct rte_eth_dev *dev,
  ...
  for (; pattern->type != RTE_FLOW_ITEM_TYPE_END; pattern++) {
    ...
    case RTE_FLOW_ITEM_TYPE_IPV6:
      ...
      if (flow) {
        ...
        spec = pattern->spec;
        rte_memcpy(flow->entry.tuple.src_ipv6, spec->hdr.src_addr,
                   IPV6_ADDR_LEN);
        rte_memcpy(flow->entry.tuple.dst_ipv6, spec->hdr.dst_addr,
                   IPV6_ADDR_LEN);
    ...
    case RTE_FLOW_ITEM_TYPE_UDP:
      ...
      if (flow) {
        ...
        spec = pattern->spec;
        flow->entry.tuple.src_port = spec->hdr.src_port;
        flow->entry.tuple.dst_port = spec->hdr.dst_port;
    ...
    default:
      rte_flow_error_set(
          error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, pattern,
...
static int qede_flow_parse_actions(struct rte_eth_dev *dev,
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    default:
      rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                         "Action is not supported - only ACTION_TYPE_QUEUE and "
                         "ACTION_TYPE_DROP supported");
      return -rte_errno;
    }
  }
...
static int qede_flow_parse(struct rte_eth_dev *dev,
                           const struct rte_flow_attr *attr,
                           const struct rte_flow_item patterns[],
                           const struct rte_flow_action actions[],
                           struct rte_flow_error *error, struct rte_flow *flow)
...
static int qede_flow_validate(struct rte_eth_dev *dev,
                              const struct rte_flow_attr *attr,
                              const struct rte_flow_item patterns[],
                              const struct rte_flow_action actions[],
                              struct rte_flow_error *error) {
...
static struct rte_flow *qede_flow_create(struct rte_eth_dev *dev,
                                         const struct rte_flow_attr *attr,
                                         const struct rte_flow_item pattern[],
                                         const struct rte_flow_action actions[],
                                         struct rte_flow_error *error) {
...
static int qede_flow_destroy(struct rte_eth_dev *eth_dev, struct rte_flow *flow,
                             struct rte_flow_error *error) {
  int rc = 0;
...
static int qede_flow_flush(struct rte_eth_dev *eth_dev,
                           struct rte_flow_error *error) {
  struct qede_dev *qdev = QEDE_INIT_QDEV(eth_dev);
  struct qede_arfs_entry *tmp = NULL;
  int rc = 0;
...
int qede_dev_flow_ops_get(struct rte_eth_dev *eth_dev,
