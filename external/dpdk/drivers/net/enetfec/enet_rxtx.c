...
uint16_t enetfec_recv_pkts(void *rxq1, struct rte_mbuf **rx_pkts,
  ...
  while ((status & RX_BD_EMPTY) == 0) {
    ...
    /* Check for errors. */
    status ^= RX_BD_LAST;
    if (status & (RX_BD_LG | RX_BD_SH | RX_BD_NO | RX_BD_CR | RX_BD_OV |
                  RX_BD_LAST | RX_BD_TR)) {
      stats->ierrors++;
      if (status & RX_BD_OV) {
        /* FIFO overrun */
      ...
      if (status & (RX_BD_LG | RX_BD_SH | RX_BD_LAST)) {
    ...
    if ((rx_offloads & RTE_ETH_RX_OFFLOAD_VLAN) && rxq->fep->bufdesc_ex &&
        (rte_read32(&ebdp->bd_esc) & rte_cpu_to_le_32(BD_ENETFEC_RX_VLAN))) {
      /* Push and remove the vlan tag */
      struct rte_vlan_hdr *vlan_header =
          (struct rte_vlan_hdr *)((uint8_t *)data + ETH_HLEN);
      vlan_tag = rte_be_to_cpu_16(vlan_header->vlan_tci);

      vlan_packet_rcvd = true;
      memmove((uint8_t *)mbuf_data + RTE_VLAN_HLEN, data,
              RTE_ETHER_ADDR_LEN * 2);
      rte_pktmbuf_adj(mbuf, RTE_VLAN_HLEN);
    }

    if (rxq->fep->bufdesc_ex && (rxq->fep->flag_csum & RX_FLAG_CSUM_EN)) {
      if ((rte_read32(&ebdp->bd_esc) & rte_cpu_to_le_32(RX_FLAG_CSUM_ERR)) ==
          0) {
        /* don't check it */
    ...
    if (vlan_packet_rcvd) {
      ...
      mbuf->ol_flags |= RTE_MBUF_F_RX_VLAN_STRIPPED | RTE_MBUF_F_RX_VLAN;
    }

    rxq->rx_mbuf[index] = new_mbuf;
    rte_write32(rte_cpu_to_le_32(rte_pktmbuf_iova(new_mbuf)), &bdp->bd_bufaddr);
  rx_processing_done:
    ...
    if (rxq->fep->bufdesc_ex) {
      ...
      rte_write32(rte_cpu_to_le_32(RX_BD_INT), &ebdp->bd_esc);
...
uint16_t enetfec_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                           uint16_t nb_pkts) {
  struct enetfec_priv_tx_q *txq = (struct enetfec_priv_tx_q *)tx_queue;
  ...
  while (tx_st) {
    ...
    if (txq->fep->bufdesc_ex) {
      ...
      rte_write32(0, &ebdp->bd_bdu);
      rte_write32(rte_cpu_to_le_32(estatus), &ebdp->bd_esc);
    }
