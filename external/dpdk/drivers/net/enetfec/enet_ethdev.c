...
#define ENETFEC_RACC_OPTIONS (ENETFEC_RACC_IPDIS | ENETFEC_RACC_PRODIS)
...
static uint64_t dev_rx_offloads_sup =
    RTE_ETH_RX_OFFLOAD_CHECKSUM | RTE_ETH_RX_OFFLOAD_VLAN;
 ...
 */
static void enetfec_restart(struct rte_eth_dev *dev) {
  ...
  if (fep->bufdesc_ex)
    ecntl |= (1 << 4);
  if (fep->quirks & QUIRK_SUPPORT_DELAYED_CLKS && fep->rgmii_txc_delay)
    ecntl |= ENETFEC_TXC_DLY;
  if (fep->quirks & QUIRK_SUPPORT_DELAYED_CLKS && fep->rgmii_rxc_delay)
...
static int enetfec_eth_configure(struct rte_eth_dev *dev) {
  struct enetfec_private *fep = dev->data->dev_private;
...
static int enetfec_eth_start(struct rte_eth_dev *dev) {
  enetfec_restart(dev);
 ...
 */
static void enetfec_disable(struct enetfec_private *fep) {
  rte_write32(rte_read32((uint8_t *)fep->hw_baseaddr_v + ENETFEC_ECR) &
                  ~(fep->enetfec_e_cntl),
              (uint8_t *)fep->hw_baseaddr_v + ENETFEC_ECR);
}

static int enetfec_eth_stop(struct rte_eth_dev *dev) {
  struct enetfec_private *fep = dev->data->dev_private;
...
static int enetfec_eth_close(struct rte_eth_dev *dev) {
  enet_free_buffers(dev);
  return 0;
}

static int enetfec_eth_link_update(struct rte_eth_dev *dev,
...
static int enetfec_promiscuous_enable(struct rte_eth_dev *dev) {
  struct enetfec_private *fep = dev->data->dev_private;
  uint32_t tmp;
...
static int enetfec_multicast_enable(struct rte_eth_dev *dev) {
  struct enetfec_private *fep = dev->data->dev_private;
...
static int enetfec_set_mac_address(struct rte_eth_dev *dev,
...
static int enetfec_stats_get(struct rte_eth_dev *dev,
...
static int enetfec_eth_info(__rte_unused struct rte_eth_dev *dev,
...
static int enetfec_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                                  uint16_t nb_desc,
  ...
  unsigned int size;
  unsigned int dsize =
      fep->bufdesc_ex ? sizeof(struct bufdesc_ex) : sizeof(struct bufdesc);
  ...
  txq->bd.active_reg_desc =
      (uint8_t *)fep->hw_baseaddr_v + offset_des_active_txq[queue_idx];
...
static int enetfec_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                                  uint16_t nb_rx_desc,
                                  unsigned int socket_id __rte_unused,
  ...
  rxq->bd.active_reg_desc =
      (uint8_t *)fep->hw_baseaddr_v + offset_des_active_rxq[queue_idx];
  bd_base = (struct bufdesc *)(((uintptr_t)bd_base) + size);
  ...
  for (i = 0; i < nb_rx_desc; i++) {
    ...
    rte_write32(rte_cpu_to_le_32(rte_pktmbuf_iova(mbuf)), &bdp->bd_bufaddr);
...
static int enetfec_eth_init(struct rte_eth_dev *dev) {
  struct enetfec_private *fep = dev->data->dev_private;
...
static int pmd_enetfec_probe(struct rte_vdev_device *vdev) {
  ...
  fep->quirks = QUIRK_HAS_ENETFEC_MAC | QUIRK_GBIT | QUIRK_RACC;
...
static int pmd_enetfec_remove(struct rte_vdev_device *vdev) {
