...
static int file_read_first_line(const char root[], const char subdir[],
  ...
  snprintf(absolute_file_name, FEC_UIO_MAX_ATTR_FILE_NAME, "%s/%s/%s", root,
           subdir, filename);
 ...
 */
static void *uio_map_mem(int uio_device_fd, int uio_device_id, int uio_map_id,
                         int *map_size, uint64_t *map_addr) {
  ...
  ret = file_read_first_line(uio_sys_root, uio_sys_map_subdir, "addr",
                             uio_map_p_addr_str);
  if (ret < 0) {
    ENETFEC_PMD_ERR("file_read_first_line() failed");
    return NULL;
  ...
  if (uio_map_id == 0) {
    ...
    mapped_address =
        mmap(0 /*dynamically choose virtual address */, uio_map_size,
             PROT_READ | PROT_WRITE, MAP_SHARED, uio_device_fd, 0);
  } else {
    /* Map the BD memory in user space */
    mapped_address = mmap(NULL, uio_map_size, PROT_READ | PROT_WRITE,
...
int config_enetfec_uio(struct enetfec_private *fep) {
  ...
  fep->hw_baseaddr_v =
      uio_map_mem(uio_job->uio_fd, uio_job->uio_minor_number,
                  FEC_UIO_REG_MAP_ID, &uio_job->map_size, &uio_job->map_addr);
  ...
  fep->bd_addr_v =
      uio_map_mem(uio_job->uio_fd, uio_job->uio_minor_number, FEC_UIO_BD_MAP_ID,
...
int enetfec_configure(void) {
  ...
  while ((dir = readdir(d)) != NULL) {
    ...
    if (file_name_match_extract(dir->d_name, "uio")) {
      ...
      ret = file_read_first_line(FEC_UIO_DEVICE_SYS_ATTR_PATH, dir->d_name,
                                 "name", uio_name);
      if (ret != 0) {
      ...
      if (file_name_match_extract(uio_name, FEC_UIO_DEVICE_NAME)) {
        enetfec_uio_job.uio_minor_number = uio_minor_number;
        ENETFEC_PMD_INFO("enetfec device uio name: %s", uio_name);
      }
    }
  }
  closedir(d);
