...
static void txgbe_virt_clr_reg(struct txgbe_hw *hw) {
  ...
  for (i = 0; i < 8; i++) {
    wr32m(hw, TXGBE_RXCFG(i),
          (TXGBE_RXCFG_HDRLEN_MASK | TXGBE_RXCFG_PKTLEN_MASK), vfsrrctl);
  }
...
s32 txgbe_reset_hw_vf(struct txgbe_hw *hw) {
   ...
   */
  ret_val = mbx->read_posted(hw, msgbuf, TXGBE_VF_PERMADDR_MSG_LEN, 0);
  if (ret_val)
    return ret_val;
 ...
 **/
s32 txgbe_set_vfta_vf(struct txgbe_hw *hw, u32 vlan, u32 vind, bool vlan_on,
                      bool vlvf_bypass) {
  u32 msgbuf[2];
  s32 ret_val;
 ...
 **/
s32 txgbe_check_mac_link_vf(struct txgbe_hw *hw, u32 *speed, bool *link_up,
  ...
  if (mac->type == txgbe_mac_raptor_vf && wait_to_complete) {
    if (po32m(hw, TXGBE_VFSTATUS, TXGBE_VFSTATUS_UP, 0, NULL, 5, 100))
      goto out;
  }
...
s32 txgbevf_rlpml_set_vf(struct txgbe_hw *hw, u16 max_size) {
  ...
  if (retval)
    return retval;
  if ((msgbuf[0] & TXGBE_VF_SET_LPE) && (msgbuf[0] & TXGBE_VT_MSGTYPE_NACK))
    return TXGBE_ERR_MBX;
