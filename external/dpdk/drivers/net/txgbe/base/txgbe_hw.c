...
static s32 txgbe_setup_copper_link_raptor(struct txgbe_hw *hw, u32 speed,
...
bool txgbe_device_supports_autoneg_fc(struct txgbe_hw *hw) {
  ...
  case txgbe_media_type_fiber:
    ...
    if (link_up)
      supported = speed == TXGBE_LINK_SPEED_1GB_FULL ? true : false;
    else
      supported = true;
  ...
  if (!supported)
    DEBUGOUT("Device %x does not support flow control autoneg", hw->device_id);
  return supported;
}
...
s32 txgbe_setup_fc(struct txgbe_hw *hw) {
  ...
  case txgbe_fc_full:
    ...
    reg |= SR_MII_MMD_AN_ADV_PAUSE_SYM | SR_MII_MMD_AN_ADV_PAUSE_ASM;
    reg_bp |= SR_AN_MMD_ADV_REG1_PAUSE_SYM | SR_AN_MMD_ADV_REG1_PAUSE_ASM;
    break;
  default:
  ...
  value =
      (value & ~(SR_MII_MMD_AN_ADV_PAUSE_ASM | SR_MII_MMD_AN_ADV_PAUSE_SYM)) |
      reg;
  ...
  if (hw->phy.media_type == txgbe_media_type_backplane) {
    ...
    value = (value &
             ~(SR_AN_MMD_ADV_REG1_PAUSE_ASM | SR_AN_MMD_ADV_REG1_PAUSE_SYM)) |
            reg_bp;
    ...
    hw->phy.write_reg(hw, TXGBE_MD_AUTO_NEG_ADVT, TXGBE_MD_DEV_AUTO_NEG,
                      reg_cu);
  }
...
s32 txgbe_set_rar(struct txgbe_hw *hw, u32 index, u8 *addr, u32 vmdq,
   ...
   */
  rar_low = TXGBE_ETHADDRL_AD0(addr[5]) | TXGBE_ETHADDRL_AD1(addr[4]) |
            TXGBE_ETHADDRL_AD2(addr[3]) | TXGBE_ETHADDRL_AD3(addr[2]);
  /*
  ...
  rar_high |= (TXGBE_ETHADDRH_AD4(addr[1]) | TXGBE_ETHADDRH_AD5(addr[0]));
...
s32 txgbe_init_rx_addrs(struct txgbe_hw *hw) {
   ...
   */
  if (txgbe_validate_mac_addr(hw->mac.addr) == TXGBE_ERR_INVALID_MAC_ADDR) {
    /* Get the MAC address from the RAR0 for later reference */
    hw->mac.get_mac_addr(hw, hw->mac.addr);

    DEBUGOUT(" Keeping Current RAR0 Addr = " RTE_ETHER_ADDR_PRT_FMT,
             hw->mac.addr[0], hw->mac.addr[1], hw->mac.addr[2], hw->mac.addr[3],
             hw->mac.addr[4], hw->mac.addr[5]);
  } else {
    /* Setup the receive address. */
    DEBUGOUT("Overriding MAC Address in RAR[0]");
    DEBUGOUT(" New MAC Addr = " RTE_ETHER_ADDR_PRT_FMT, hw->mac.addr[0],
...
s32 txgbe_update_mc_addr_list(struct txgbe_hw *hw, u8 *mc_addr_list,
  ...
  /* Enable mta */
  for (i = 0; i < hw->mac.mcft_size; i++)
    wr32a(hw, TXGBE_MCADDRTBL(0), i, hw->mac.mta_shadow[i]);

  if (hw->addr_ctrl.mta_in_use > 0) {
    u32 psrctl = rd32(hw, TXGBE_PSRCTL);
    psrctl &= ~(TXGBE_PSRCTL_ADHF12_MASK | TXGBE_PSRCTL_MCHFENA);
    psrctl |=
        TXGBE_PSRCTL_MCHFENA | TXGBE_PSRCTL_ADHF12(hw->mac.mc_filter_type);
    wr32(hw, TXGBE_PSRCTL, psrctl);
  }
...
s32 txgbe_fc_enable(struct txgbe_hw *hw) {
  ...
  for (i = 0; i < TXGBE_DCB_TC_MAX; i++) {
    if ((hw->fc.current_mode & txgbe_fc_tx_pause) && hw->fc.high_water[i]) {
      if (!hw->fc.low_water[i] || hw->fc.low_water[i] >= hw->fc.high_water[i]) {
  ...
  for (i = 0; i < TXGBE_DCB_TC_MAX; i++) {
    if ((hw->fc.current_mode & txgbe_fc_tx_pause) && hw->fc.high_water[i]) {
      fcrtl = TXGBE_FCWTRLO_TH(hw->fc.low_water[i]) | TXGBE_FCWTRLO_XON;
      fcrth = TXGBE_FCWTRHI_TH(hw->fc.high_water[i]) | TXGBE_FCWTRHI_XOFF;
    } else {
      /*
...
s32 txgbe_negotiate_fc(struct txgbe_hw *hw, u32 adv_reg, u32 lp_reg,
  ...
  if ((!(adv_reg)) || (!(lp_reg))) {
    DEBUGOUT("Local or link partner's advertised flow control settings are "
             "NULL. Local: %x, link partner: %x",
             adv_reg, lp_reg);
...
STATIC s32 txgbe_fc_autoneg_fiber(struct txgbe_hw *hw) {
  ...
  err = txgbe_negotiate_fc(
      hw, pcs_anadv_reg, pcs_lpab_reg, SR_MII_MMD_AN_ADV_PAUSE_SYM,
      SR_MII_MMD_AN_ADV_PAUSE_ASM, SR_MII_MMD_AN_ADV_PAUSE_SYM,
...
STATIC s32 txgbe_fc_autoneg_backplane(struct txgbe_hw *hw) {
  ...
  err = txgbe_negotiate_fc(
      hw, autoc_reg, anlp1_reg, SR_AN_MMD_ADV_REG1_PAUSE_SYM,
      SR_AN_MMD_ADV_REG1_PAUSE_ASM, SR_AN_MMD_ADV_REG1_PAUSE_SYM,
...
STATIC s32 txgbe_fc_autoneg_copper(struct txgbe_hw *hw) {
  ...
  hw->phy.read_reg(hw, TXGBE_MD_AUTO_NEG_ADVT, TXGBE_MD_DEV_AUTO_NEG,
                   &technology_ability_reg);
  hw->phy.read_reg(hw, TXGBE_MD_AUTO_NEG_LP, TXGBE_MD_DEV_AUTO_NEG,
                   &lp_technology_ability_reg);

  return txgbe_negotiate_fc(hw, (u32)technology_ability_reg,
                            (u32)lp_technology_ability_reg, TXGBE_TAF_SYM_PAUSE,
                            TXGBE_TAF_ASM_PAUSE, TXGBE_TAF_SYM_PAUSE,
                            TXGBE_TAF_ASM_PAUSE);
}
...
#define TXGBE_MAX_SECRX_POLL 4000
  ...
  if (i >= TXGBE_MAX_SECRX_POLL)
    DEBUGOUT("Rx unit being enabled before security path fully disabled.  "
...
#define TXGBE_MAX_SECTX_POLL 40
  ...
  if (i >= TXGBE_MAX_SECTX_POLL)
    DEBUGOUT("Tx unit being enabled before security path fully disabled.  "
             "Continuing with init.");
...
static s32 txgbe_get_san_mac_addr_offset(struct txgbe_hw *hw,
   ...
   * available.
   */
  err = hw->rom.readw_sw(hw, TXGBE_SAN_MAC_ADDR_PTR, san_mac_offset);
  if (err) {
    DEBUGOUT("eeprom at offset %d failed", TXGBE_SAN_MAC_ADDR_PTR);
  }
...
s32 txgbe_get_san_mac_addr(struct txgbe_hw *hw, u8 *san_mac_addr) {
  ...
  for (i = 0; i < 3; i++) {
    err = hw->rom.read16(hw, san_mac_offset, &san_mac_data);
    if (err) {
      DEBUGOUT("eeprom read at offset %d failed", san_mac_offset);
      goto san_mac_addr_out;
    }
 ...
 **/
s32 txgbe_set_vfta(struct txgbe_hw *hw, u32 vlan, u32 vind, bool vlan_on,
                   bool vlvf_bypass) {
   ...
   */
  err = txgbe_set_vlvf(hw, vlan, vind, vlan_on, &vfta_delta, vfta, vlvf_bypass);
  if (err != 0) {
    if (vlvf_bypass)
      goto vfta_update;
    return err;
 ...
 **/
s32 txgbe_set_vlvf(struct txgbe_hw *hw, u32 vlan, u32 vind, bool vlan_on,
  ...
  if (!bits && !rd32(hw, TXGBE_PSRVLANPLM(vind / 32))) {
 ...
 **/
s32 txgbe_check_mac_link(struct txgbe_hw *hw, u32 *speed, bool *link_up,
  ...
  if (txgbe_need_crosstalk_fix(hw)) {
    ...
    switch (hw->mac.type) {
    case txgbe_mac_raptor:
      sfp_cage_full = !rd32m(hw, TXGBE_GPIODATA, TXGBE_GPIOBIT_2);
      break;
    default:
  ...
  if (links_orig != links_reg) {
    DEBUGOUT("LINKS changed from %08X to %08X", links_orig, links_reg);
  }
 ...
 *
 **/
void txgbe_set_ethertype_anti_spoofing(struct txgbe_hw *hw, bool enable,
                                       int vf) {
  int vf_target_reg = vf >> 3;
  int vf_target_shift = vf % 8;
  u32 pfvfspoof;
...
s32 txgbe_get_thermal_sensor_data(struct txgbe_hw *hw) {
  ...
  tsv = -(48380 << 8) / 1000 + tsv * (31020 << 8) / 100000 -
...
s32 txgbe_init_thermal_sensor_thresh(struct txgbe_hw *hw) {
  ...
  wr32(hw, TXGBE_TSINTR, TXGBE_TSINTR_AEN | TXGBE_TSINTR_DEN);
 ...
 **/
s32 txgbe_setup_mac_link_multispeed_fiber(struct txgbe_hw *hw, u32 speed,
                                          bool autoneg_wait_to_complete) {
  ...
  if (speed & TXGBE_LINK_SPEED_10GB_FULL) {
    ...
    switch (hw->phy.media_type) {
    case txgbe_media_type_fiber:
      hw->mac.set_rate_select_speed(hw, TXGBE_LINK_SPEED_10GB_FULL);
      break;
    case txgbe_media_type_fiber_qsfp:
  ...
  if (speed & TXGBE_LINK_SPEED_1GB_FULL) {
    ...
    switch (hw->phy.media_type) {
    case txgbe_media_type_fiber:
      hw->mac.set_rate_select_speed(hw, TXGBE_LINK_SPEED_1GB_FULL);
      break;
    case txgbe_media_type_fiber_qsfp:
    ...
    status = hw->mac.setup_mac_link(hw, TXGBE_LINK_SPEED_1GB_FULL,
                                    autoneg_wait_to_complete);
    if (status != 0)
      return status;
   ...
   */
  if (speedcnt > 1)
    status = txgbe_setup_mac_link_multispeed_fiber(hw, highest_link_speed,
                                                   autoneg_wait_to_complete);
...
s32 txgbe_set_mac_type(struct txgbe_hw *hw) {
  ...
  DEBUGOUT("found mac: %d, returns: %d", hw->mac.type, err);
  return err;
}
...
void txgbe_init_mac_link_ops(struct txgbe_hw *hw) {
   ...
   * and MNG not enabled
   */
  if (hw->phy.media_type == txgbe_media_type_fiber && !txgbe_mng_enabled(hw)) {
    mac->disable_tx_laser = txgbe_disable_tx_laser_multispeed_fiber;
...
s32 txgbe_init_phy_raptor(struct txgbe_hw *hw) {
  ...
  if (phy->media_type == txgbe_media_type_copper) {
    ...
    mac->get_link_capabilities = txgbe_get_copper_link_capabilities;
  }
 ...
 */
s32 txgbe_prot_autoc_read_raptor(struct txgbe_hw *hw, bool *locked,
                                 u64 *value) {
  s32 err;
 ...
 **/
s32 txgbe_get_link_capabilities_raptor(struct txgbe_hw *hw, u32 *speed,
                                       bool *autoneg) {
  s32 status = 0;
  u32 autoc = 0;
  ...
  case TXGBE_AUTOC_LMS_SGMII_1G_100M:
    *speed = TXGBE_LINK_SPEED_1GB_FULL | TXGBE_LINK_SPEED_100M_FULL |
  ...
  if (hw->phy.multispeed_fiber) {
    *speed |= TXGBE_LINK_SPEED_10GB_FULL | TXGBE_LINK_SPEED_1GB_FULL;
...
void txgbe_disable_tx_laser_multispeed_fiber(struct txgbe_hw *hw) {
  ...
  if (txgbe_close_notify(hw))
    txgbe_led_off(hw, TXGBE_LEDCTL_UP | TXGBE_LEDCTL_10G | TXGBE_LEDCTL_1G |
                          TXGBE_LEDCTL_ACTIVE);
 ...
 **/
s32 txgbe_setup_mac_link_smartspeed(struct txgbe_hw *hw, u32 speed,
                                    bool autoneg_wait_to_complete) {
  s32 status = 0;
  u32 link_speed = TXGBE_LINK_SPEED_UNKNOWN;
  s32 i, j;
  ...
  for (j = 0; j < TXGBE_SMARTSPEED_MAX_RETRIES; j++) {
    status = txgbe_setup_mac_link(hw, speed, autoneg_wait_to_complete);
    if (status != 0)
      goto out;
...
s32 txgbe_setup_mac_link(struct txgbe_hw *hw, u32 speed,
  ...
  status = hw->mac.get_link_capabilities(hw, &link_capabilities, &autoneg);
  if (status)
    return status;
  ...
  if (link_mode == TXGBE_AUTOC_LMS_KX4_KX_KR ||
    ...
    autoc &=
        ~(TXGBE_AUTOC_KX_SUPP | TXGBE_AUTOC_KX4_SUPP | TXGBE_AUTOC_KR_SUPP);
    if (speed & TXGBE_LINK_SPEED_10GB_FULL) {
...
static s32 txgbe_setup_copper_link_raptor(struct txgbe_hw *hw, u32 speed,
  ...
  status = hw->phy.setup_link_speed(hw, speed, autoneg_wait_to_complete);
  /* Set up MAC */
...
static int txgbe_check_flash_load(struct txgbe_hw *hw, u32 check_bit) {
  u32 reg = 0;
  u32 i;
  int err = 0;
...
static void txgbe_reset_misc(struct txgbe_hw *hw) {
  ...
  wr32m(hw, TXGBE_MACRXCFG, TXGBE_MACRXCFG_JUMBO, TXGBE_MACRXCFG_JUMBO);

  wr32m(hw, TXGBE_FRMSZ, TXGBE_FRMSZ_MAX_MASK,
        TXGBE_FRMSZ_MAX(TXGBE_FRAME_SIZE_DFT));

  /* clear counters on read */
  wr32m(hw, TXGBE_MACCNTCTL, TXGBE_MACCNTCTL_RC, TXGBE_MACCNTCTL_RC);

  wr32m(hw, TXGBE_RXFCCFG, TXGBE_RXFCCFG_FC, TXGBE_RXFCCFG_FC);
  wr32m(hw, TXGBE_TXFCCFG, TXGBE_TXFCCFG_FC, TXGBE_TXFCCFG_FC);

  wr32m(hw, TXGBE_MACRXFLT, TXGBE_MACRXFLT_PROMISC, TXGBE_MACRXFLT_PROMISC);

  wr32m(hw, TXGBE_RSTSTAT, TXGBE_RSTSTAT_TMRINIT_MASK,
        TXGBE_RSTSTAT_TMRINIT(30));
...
mac_reset_top:
  ...
  if (hw->bus.lan_id == 0) {
    status = txgbe_check_flash_load(hw, TXGBE_ILDRSTAT_SWRST_LAN0);
  } else {
    status = txgbe_check_flash_load(hw, TXGBE_ILDRSTAT_SWRST_LAN1);
  }
  if (status != 0)
    return status;
  ...
  if (txgbe_validate_mac_addr(hw->mac.san_addr) == 0) {
    ...
    hw->mac.set_rar(hw, hw->mac.san_mac_rar_index, hw->mac.san_addr, 0, true);

    /* clear VMDq pool/queue selection for this RAR */
    hw->mac.clear_vmdq(hw, hw->mac.san_mac_rar_index, BIT_MASK32);
  ...
  hw->mac.get_wwn_prefix(hw, &hw->mac.wwnn_prefix, &hw->mac.wwpn_prefix);
...
s32 txgbe_reinit_fdir_tables(struct txgbe_hw *hw) {
  ...
  if (err) {
    DEBUGOUT("Flow Director previous command did not complete, aborting table "
             "re-initialization.");
    return err;
  }
...
bool txgbe_verify_lesm_fw_enabled_raptor(struct txgbe_hw *hw) {
  ...
  status = hw->rom.read16(hw, (fw_offset + TXGBE_FW_LESM_PARAMETERS_PTR),
                          &fw_lesm_param_offset);

  if (status != 0 || fw_lesm_param_offset == 0 ||
      fw_lesm_param_offset == 0xFFFF)
    goto out;

  /* get the LESM state word */
  status = hw->rom.read16(hw, (fw_lesm_param_offset + TXGBE_FW_LESM_STATE_1),
                          &fw_lesm_state);
