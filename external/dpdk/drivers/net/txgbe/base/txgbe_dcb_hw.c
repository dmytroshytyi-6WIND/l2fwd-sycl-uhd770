...
s32 txgbe_dcb_config_rx_arbiter_raptor(struct txgbe_hw *hw, u16 *refill,
  ...
  for (i = 0; i < TXGBE_DCB_TC_MAX; i++) {
    ...
    credit_max = max[i];
    reg = TXGBE_QARBRXCFG_CRQ(credit_refill) | TXGBE_QARBRXCFG_MCL(credit_max) |
...
s32 txgbe_dcb_config_tx_desc_arbiter_raptor(struct txgbe_hw *hw, u16 *refill,
  ...
  for (i = 0; i < TXGBE_DCB_TC_MAX; i++) {
    max_credits = max[i];
    reg = TXGBE_QARBTXCFG_MCL(max_credits) | TXGBE_QARBTXCFG_CRQ(refill[i]) |
...
s32 txgbe_dcb_config_tx_data_arbiter_raptor(struct txgbe_hw *hw, u16 *refill,
   ...
   */
  reg = TXGBE_PARBTXCTL_SP | TXGBE_PARBTXCTL_RECYC | TXGBE_PARBTXCTL_DA;
  ...
  /* Configure traffic class credits and priority */
  for (i = 0; i < TXGBE_DCB_TC_MAX; i++) {
    reg = TXGBE_PARBTXCFG_CRQ(refill[i]) | TXGBE_PARBTXCFG_MCL(max[i]) |
...
s32 txgbe_dcb_config_pfc_raptor(struct txgbe_hw *hw, u8 pfc_en, u8 *map) {
  ...
  wr32m(hw, TXGBE_RXFCCFG, TXGBE_RXFCCFG_PFC, pfc_en ? TXGBE_RXFCCFG_PFC : 0);
  ...
  for (i = 0; i <= max_tc; i++) {
    ...
    if (enabled) {
      reg = TXGBE_FCWTRHI_TH(hw->fc.high_water[i]) | TXGBE_FCWTRHI_XOFF;
