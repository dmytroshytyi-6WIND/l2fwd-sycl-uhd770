 ...
 **/
static u8 txgbe_calculate_checksum(u8 *buffer, u32 length) {
  u32 i;
  u8 sum = 0;
 ...
 *  by the caller.
 **/
static s32 txgbe_hic_unlocked(struct txgbe_hw *hw, u32 *buffer, u32 length,
                              u32 timeout) {
  u32 value, loop;
  u16 i, dword_len;
  ...
  wr32m(hw, TXGBE_MNGMBXCTL, TXGBE_MNGMBXCTL_SWRDY, TXGBE_MNGMBXCTL_SWRDY);

  /* Check command completion */
  loop = po32m(hw, TXGBE_MNGMBXCTL, TXGBE_MNGMBXCTL_FWRDY,
               TXGBE_MNGMBXCTL_FWRDY, &value, timeout, 1000);
 ...
 *  semaphore or TXGBE_ERR_HOST_INTERFACE_COMMAND when command fails.
 **/
static s32 txgbe_host_interface_command(struct txgbe_hw *hw, u32 *buffer,
...
s32 txgbe_hic_sr_read(struct txgbe_hw *hw, u32 addr, u8 *buf, int len) {
  ...
  err = txgbe_hic_unlocked(hw, (u32 *)&command, sizeof(command),
...
s32 txgbe_close_notify(struct txgbe_hw *hw) {
  ...
  status = txgbe_host_interface_command(hw, (u32 *)&buffer, sizeof(buffer),
 ...
 *  semaphore or TXGBE_ERR_HOST_INTERFACE_COMMAND when command fails.
 **/
s32 txgbe_hic_set_drv_ver(struct txgbe_hw *hw, u8 maj, u8 min, u8 build, u8 sub,
  ...
  fw_cmd.pad = 0;
  fw_cmd.pad2 = 0;
  fw_cmd.hdr.checksum = txgbe_calculate_checksum(
      (u8 *)&fw_cmd, (FW_CEM_HDR_LEN + fw_cmd.hdr.buf_len));

  for (i = 0; i <= FW_CEM_MAX_RETRIES; i++) {
    ret_val = txgbe_host_interface_command(hw, (u32 *)&fw_cmd, sizeof(fw_cmd),
...
s32 txgbe_hic_reset(struct txgbe_hw *hw) {
  ...
  reset_cmd.hdr.checksum = txgbe_calculate_checksum(
      (u8 *)&reset_cmd, (FW_CEM_HDR_LEN + reset_cmd.hdr.buf_len));

  for (i = 0; i <= FW_CEM_MAX_RETRIES; i++) {
    err = txgbe_host_interface_command(hw, (u32 *)&reset_cmd, sizeof(reset_cmd),
                                       TXGBE_HI_COMMAND_TIMEOUT, true);
    if (err != 0)
      continue;
