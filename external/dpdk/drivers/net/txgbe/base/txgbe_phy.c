...
static void txgbe_read_phy_lane_tx_eq(u16 lane, struct txgbe_hw *hw, s32 post,
                                      s32 mode);
...
static bool txgbe_identify_extphy(struct txgbe_hw *hw) {
  ...
  if (!txgbe_validate_phy_addr(hw, phy_addr)) {
    DEBUGOUT("Unable to validate PHY address 0x%04X", phy_addr);
    return false;
  }
  ...
  if (hw->phy.type == txgbe_phy_unknown) {
    u16 ext_ability = 0;
    hw->phy.read_reg(hw, TXGBE_MD_PHY_EXT_ABILITY, TXGBE_MD_DEV_PMA_PMD,
                     &ext_ability);

    if (ext_ability &
        (TXGBE_MD_PHY_10GBASET_ABILITY | TXGBE_MD_PHY_1000BASET_ABILITY))
...
bool txgbe_validate_phy_addr(struct txgbe_hw *hw, u32 phy_addr) {
  ...
  hw->phy.read_reg(hw, TXGBE_MD_PHY_ID_HIGH, TXGBE_MD_DEV_PMA_PMD, &phy_id);
...
s32 txgbe_get_phy_id(struct txgbe_hw *hw) {
  ...
  err = hw->phy.read_reg(hw, TXGBE_MD_PHY_ID_HIGH, TXGBE_MD_DEV_PMA_PMD,
                         &phy_id_high);

  if (err == 0) {
    hw->phy.id = (u32)(phy_id_high << 16);
    err = hw->phy.read_reg(hw, TXGBE_MD_PHY_ID_LOW, TXGBE_MD_DEV_PMA_PMD,
                           &phy_id_low);
    hw->phy.id |= (u32)(phy_id_low & TXGBE_PHY_REVISION_MASK);
    hw->phy.revision = (u32)(phy_id_low & ~TXGBE_PHY_REVISION_MASK);
  }
  DEBUGOUT("PHY_ID_HIGH 0x%04X, PHY_ID_LOW 0x%04X", phy_id_high, phy_id_low);
...
static s32 txgbe_reset_extphy(struct txgbe_hw *hw) {
  u16 ctrl = 0;
  int err, i;

  err = hw->phy.read_reg(hw, TXGBE_MD_PORT_CTRL, TXGBE_MD_DEV_GENERAL, &ctrl);
  if (err != 0)
    return err;
  ctrl |= TXGBE_MD_PORT_CTRL_RESET;
  err = hw->phy.write_reg(hw, TXGBE_MD_PORT_CTRL, TXGBE_MD_DEV_GENERAL, ctrl);
  if (err != 0)
    return err;
...
s32 txgbe_read_phy_reg_mdi(struct txgbe_hw *hw, u32 reg_addr, u32 device_type,
  ...
  command = TXGBE_MDIOSCA_REG(reg_addr) | TXGBE_MDIOSCA_DEV(device_type) |
            TXGBE_MDIOSCA_PORT(hw->phy.addr);
  wr32(hw, TXGBE_MDIOSCA, command);

  command = TXGBE_MDIOSCD_CMD_READ | TXGBE_MDIOSCD_BUSY;
   ...
   * complete
   */
  if (!po32m(hw, TXGBE_MDIOSCD, TXGBE_MDIOSCD_BUSY, 0, NULL, 100, 100)) {
 ...
 *  @phy_data: Pointer to read data from PHY register
 **/
s32 txgbe_read_phy_reg(struct txgbe_hw *hw, u32 reg_addr, u32 device_type,
                       u16 *phy_data) {
  s32 err;
...
s32 txgbe_write_phy_reg_mdi(struct txgbe_hw *hw, u32 reg_addr, u32 device_type,
  ...
  command = TXGBE_MDIOSCD_CMD_WRITE | TXGBE_MDIOSCD_DAT(phy_data) |
            TXGBE_MDIOSCD_BUSY;
  wr32(hw, TXGBE_MDIOSCD, command);
 ...
 *  @phy_data: Data to write to the PHY register
 **/
s32 txgbe_write_phy_reg(struct txgbe_hw *hw, u32 reg_addr, u32 device_type,
                        u16 phy_data) {
  s32 err;
  u32 gssr = hw->phy.phy_semaphore_mask;
  ...
  err = hw->phy.write_reg_mdi(hw, reg_addr, device_type, phy_data);
  hw->mac.release_swfw_sync(hw, gssr);
...
s32 txgbe_setup_phy_link(struct txgbe_hw *hw) {
  ...
  if ((hw->phy.autoneg_advertised & TXGBE_LINK_SPEED_2_5GB_FULL) &&
  ...
  hw->phy.read_reg(hw, TXGBE_MII_AUTONEG_ADVERTISE_REG, TXGBE_MD_DEV_AUTO_NEG,
                   &autoneg_reg);

  autoneg_reg &=
      ~(TXGBE_MII_100BASE_T_ADVERTISE | TXGBE_MII_100BASE_T_ADVERTISE_HALF);
  if ((hw->phy.autoneg_advertised & TXGBE_LINK_SPEED_100M_FULL) &&
      (speed & TXGBE_LINK_SPEED_100M_FULL))
    autoneg_reg |= TXGBE_MII_100BASE_T_ADVERTISE;

  hw->phy.write_reg(hw, TXGBE_MII_AUTONEG_ADVERTISE_REG, TXGBE_MD_DEV_AUTO_NEG,
                    autoneg_reg);
  ...
  hw->phy.read_reg(hw, TXGBE_MD_AUTO_NEG_CONTROL, TXGBE_MD_DEV_AUTO_NEG,
                   &autoneg_reg);

  autoneg_reg |= TXGBE_MII_RESTART;

  hw->phy.write_reg(hw, TXGBE_MD_AUTO_NEG_CONTROL, TXGBE_MD_DEV_AUTO_NEG,
                    autoneg_reg);
 ...
 **/
s32 txgbe_setup_phy_link_speed(struct txgbe_hw *hw, u32 speed,
...
static s32 txgbe_get_copper_speeds_supported(struct txgbe_hw *hw) {
  ...
  err = hw->phy.read_reg(hw, TXGBE_MD_PHY_SPEED_ABILITY, TXGBE_MD_DEV_PMA_PMD,
                         &speed_ability);
  if (err)
    return err;
 ...
 **/
s32 txgbe_get_copper_link_capabilities(struct txgbe_hw *hw, u32 *speed,
                                       bool *autoneg) {
  s32 err = 0;
 ...
 *  the PHY.
 **/
s32 txgbe_check_phy_link_tnx(struct txgbe_hw *hw, u32 *speed, bool *link_up) {
  s32 err = 0;
  u32 time_out;
  u32 max_time_out = 10;
  u16 phy_link = 0;
  ...
  *speed = TXGBE_LINK_SPEED_10GB_FULL;
  ...
  for (time_out = 0; time_out < max_time_out; time_out++) {
    usec_delay(10);
    err = hw->phy.read_reg(hw, TXGBE_MD_VENDOR_SPECIFIC_1_STATUS,
                           TXGBE_MD_DEV_VENDOR_1, &phy_data);
    phy_link = phy_data & TXGBE_MD_VENDOR_SPECIFIC_1_LINK_STATUS;
    phy_speed = phy_data & TXGBE_MD_VENDOR_SPECIFIC_1_SPEED_STATUS;
    if (phy_link == TXGBE_MD_VENDOR_SPECIFIC_1_LINK_STATUS) {
      *link_up = true;
      if (phy_speed == TXGBE_MD_VENDOR_SPECIFIC_1_SPEED_STATUS)
        *speed = TXGBE_LINK_SPEED_1GB_FULL;
      break;
    }
  }
...
s32 txgbe_setup_phy_link_tnx(struct txgbe_hw *hw) {
  ...
  if (speed & TXGBE_LINK_SPEED_1GB_FULL) {
    /* Set or unset auto-negotiation 1G advertisement */
    hw->phy.read_reg(hw, TXGBE_MII_AUTONEG_XNP_TX_REG, TXGBE_MD_DEV_AUTO_NEG,
                     &autoneg_reg);
    ...
    hw->phy.write_reg(hw, TXGBE_MII_AUTONEG_XNP_TX_REG, TXGBE_MD_DEV_AUTO_NEG,
                      autoneg_reg);
  }
...
s32 txgbe_identify_sfp_module(struct txgbe_hw *hw) {
  ...
  err = hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_1GBE_COMP_CODES, &comp_codes_1g);
  if (err != 0)
    goto ERR_I2C;

  err =
      hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_10GBE_COMP_CODES, &comp_codes_10g);
  if (err != 0)
    goto ERR_I2C;

  err = hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_CABLE_TECHNOLOGY, &cable_tech);
  if (err != 0)
    goto ERR_I2C;
  ...
  if (cable_tech & TXGBE_SFF_CABLE_DA_PASSIVE) {
    ...
    if (cable_spec & TXGBE_SFF_DA_SPEC_ACTIVE_LIMITING) {
      hw->phy.sfp_type =
          (hw->bus.lan_id == 0 ? txgbe_sfp_type_da_act_lmt_core0
                               : txgbe_sfp_type_da_act_lmt_core1);
    } else {
      hw->phy.sfp_type = txgbe_sfp_type_unknown;
    }
  } else if (comp_codes_10g &
             (TXGBE_SFF_10GBASESR_CAPABLE | TXGBE_SFF_10GBASELR_CAPABLE)) {
    hw->phy.sfp_type = (hw->bus.lan_id == 0 ? txgbe_sfp_type_srlr_core0
                                            : txgbe_sfp_type_srlr_core1);
  } else if (comp_codes_1g & TXGBE_SFF_1GBASET_CAPABLE) {
    hw->phy.sfp_type = (hw->bus.lan_id == 0 ? txgbe_sfp_type_1g_cu_core0
                                            : txgbe_sfp_type_1g_cu_core1);
  } else if (comp_codes_1g & TXGBE_SFF_1GBASESX_CAPABLE) {
    hw->phy.sfp_type = (hw->bus.lan_id == 0 ? txgbe_sfp_type_1g_sx_core0
                                            : txgbe_sfp_type_1g_sx_core1);
  } else if (comp_codes_1g & TXGBE_SFF_1GBASELX_CAPABLE) {
    hw->phy.sfp_type = (hw->bus.lan_id == 0 ? txgbe_sfp_type_1g_lx_core0
  ...
  if (hw->phy.type != txgbe_phy_nl) {
    ...
    err =
        hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_VENDOR_OUI_BYTE0, &oui_bytes[0]);
    if (err != 0)
      goto ERR_I2C;

    err =
        hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_VENDOR_OUI_BYTE1, &oui_bytes[1]);
    if (err != 0)
      goto ERR_I2C;

    err =
        hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_VENDOR_OUI_BYTE2, &oui_bytes[2]);
    if (err != 0)
      goto ERR_I2C;

    vendor_oui = ((u32)oui_bytes[0] << 24) | ((u32)oui_bytes[1] << 16) |
                 ((u32)oui_bytes[2] << 8);
    switch (vendor_oui) {
  ...
  if (cable_tech & (TXGBE_SFF_CABLE_DA_PASSIVE | TXGBE_SFF_CABLE_DA_ACTIVE)) {
    return 0;
  }
...
ERR_I2C:
  ...
  err = hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_QSFP_10GBE_COMP, &comp_codes_10g);

  if (err != 0)
    goto ERR_I2C;

  err = hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_QSFP_1GBE_COMP, &comp_codes_1g);
  ...
  if (comp_codes_10g & TXGBE_SFF_QSFP_DA_PASSIVE_CABLE) {
    ...
    if (!active_cable) {
      hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_QSFP_CONNECTOR, &connector);

      hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_QSFP_CABLE_LENGTH, &cable_length);

      hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_QSFP_DEVICE_TECH, &device_tech);

      if (connector == TXGBE_SFF_QSFP_CONNECTOR_NOT_SEPARABLE &&
          cable_length > 0 &&
          ((device_tech >> 4) == TXGBE_SFF_QSFP_TRANSMITTER_850NM_VCSEL))
        active_cable = true;
    }
  ...
  if (comp_codes_10g &
      (TXGBE_SFF_10GBASESR_CAPABLE | TXGBE_SFF_10GBASELR_CAPABLE)) {
    err = hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_QSFP_VENDOR_OUI_BYTE0,
                                  &oui_bytes[0]);

    if (err != 0)
      goto ERR_I2C;

    err = hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_QSFP_VENDOR_OUI_BYTE1,
                                  &oui_bytes[1]);

    if (err != 0)
      goto ERR_I2C;

    err = hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_QSFP_VENDOR_OUI_BYTE2,
                                  &oui_bytes[2]);
    ...
    vendor_oui =
        ((oui_bytes[0] << 24) | (oui_bytes[1] << 16) | (oui_bytes[2] << 8));
    ...
    if (!(enforce_sfp & TXGBE_DEVICE_CAPS_ALLOW_ANY_SFP)) {
      ...
      if (hw->phy.type == txgbe_phy_qsfp_intel) {
        ...
        if (hw->allow_unsupported_sfp) {
          DEBUGOUT("WARNING: Wangxun (R) Network Connections are quality "
                   "tested using Wangxun (R) Ethernet Optics. "
                   "Using untested modules is not supported and may cause "
                   "unstable operation or damage to the module or the adapter. "
                   "Wangxun Corporation is not responsible for any harm caused "
...
s32 txgbe_read_i2c_eeprom(struct txgbe_hw *hw, u8 byte_offset,
                          u8 *eeprom_data) {
  return hw->phy.read_i2c_byte(hw, byte_offset, TXGBE_I2C_EEPROM_DEV_ADDR,
                               eeprom_data);
}
...
s32 txgbe_read_i2c_sff8472(struct txgbe_hw *hw, u8 byte_offset,
                           u8 *sff8472_data) {
  return hw->phy.read_i2c_byte(hw, byte_offset, TXGBE_I2C_EEPROM_DEV_ADDR2,
                               sff8472_data);
}
...
s32 txgbe_write_i2c_eeprom(struct txgbe_hw *hw, u8 byte_offset,
                           u8 eeprom_data) {
  return hw->phy.write_i2c_byte(hw, byte_offset, TXGBE_I2C_EEPROM_DEV_ADDR,
                                eeprom_data);
}
...
s32 txgbe_read_i2c_byte_unlocked(struct txgbe_hw *hw, u8 byte_offset,
  ...
  /* wait tx empty */
  if (!po32m(hw, TXGBE_I2CICR, TXGBE_I2CICR_TXEMPTY, TXGBE_I2CICR_TXEMPTY, NULL,
             100, 100)) {
    return -TERR_TIMEOUT;
  }
  ...
  if (!po32m(hw, TXGBE_I2CICR, TXGBE_I2CICR_RXFULL, TXGBE_I2CICR_RXFULL, NULL,
             100, 100)) {
    return -TERR_TIMEOUT;
  }
 ...
 *  a specified device address.
 **/
s32 txgbe_read_i2c_byte(struct txgbe_hw *hw, u8 byte_offset, u8 dev_addr,
                        u8 *data) {
 ...
 *  a specified device address.
 **/
s32 txgbe_write_i2c_byte(struct txgbe_hw *hw, u8 byte_offset, u8 dev_addr,
                         u8 data) {
  u32 swfw_mask = hw->phy.phy_semaphore_mask;
  int err = 0;
...
static void txgbe_i2c_start(struct txgbe_hw *hw, u8 dev_addr) {
  ...
  wr32(hw, TXGBE_I2CCON,
       (TXGBE_I2CCON_MENA | TXGBE_I2CCON_SPEED(1) | TXGBE_I2CCON_RESTART |
        TXGBE_I2CCON_SDIA));
...
static void txgbe_i2c_stop(struct txgbe_hw *hw) {
  /* wait for completion */
  if (!po32m(hw, TXGBE_I2CSTAT, TXGBE_I2CSTAT_MST, 0, NULL, 100, 100)) {
...
static s32 txgbe_set_link_to_kr(struct txgbe_hw *hw, bool autoneg) {
  u32 i;
  u16 value;
  s32 err = 0;
  ...
  if (!autoneg) {
    ...
    wr32_epcs(hw, SR_AN_CTRL, SR_AN_CTRL_AN_EN | SR_AN_CTRL_EXT_NP);
    ...
    if (hw->devarg.poll == 1) {
      wr32_epcs(hw, VR_PMA_KRTR_TIMER_CTRL0, VR_PMA_KRTR_TIMER_MAX_WAIT);
...
static s32 txgbe_set_link_to_kx4(struct txgbe_hw *hw, bool autoneg) {
  u32 i;
  s32 err = 0;
  u32 value;
   ...
   * PCS_TYPE_SEL: non KR
   */
  wr32_epcs(hw, SR_XS_PCS_CTRL2, SR_PCS_CTRL2_TYPE_SEL_X);

  /* Set SR PMA MMD Control1 Register Bit[13] = 1'b1
   * SS13: 10G speed
   */
  wr32_epcs(hw, SR_PMA_CTRL1, SR_PMA_CTRL1_SS13_KX4);
   ...
   */
  wr32_epcs(hw, TXGBE_PHY_MPLLA_CTL0, TXGBE_PHY_MPLLA_CTL0_MULTIPLIER_OTHER);

  /* Set VR XS, PMA or MII Gen5 12G PHY MPLLA
   * Control 3 Register Bit[10:0] = 11'd86  //MPLLA_BANDWIDTH
   */
  wr32_epcs(hw, TXGBE_PHY_MPLLA_CTL3, TXGBE_PHY_MPLLA_CTL3_MULTIPLIER_BW_OTHER);
...
static s32 txgbe_set_link_to_kx(struct txgbe_hw *hw, u32 speed, bool autoneg) {
  u32 i;
  s32 err = 0;
  u32 wdata = 0;
  u32 value;
   ...
   * SS13: 1G speed
   */
  wr32_epcs(hw, SR_PMA_CTRL1, SR_PMA_CTRL1_SS13_KX);
  ...
  if (speed == TXGBE_LINK_SPEED_100M_FULL)
    ...
    wdata = 0x0100;
  wr32_epcs(hw, SR_MII_MMD_CTL, wdata);
   ...
   */
  wr32_epcs(hw, TXGBE_PHY_VCO_CAL_LD0, TXGBE_PHY_VCO_CAL_LD0_1GBASEX_KX);
   ...
   */
  wr32_epcs(hw, TXGBE_PHY_VCO_CAL_REF0, TXGBE_PHY_VCO_CAL_REF0_LD0_1GBASEX_KX);
   ...
   */
  wr32_epcs(hw, TXGBE_PHY_TX_GEN_CTL2, TXGBE_PHY_TX_GEN_CTL2_TX0_WIDTH_OTHER);
  /* Set VR XS, PMA, or MII Gen5 12G PHY Rx General
...
u64 txgbe_autoc_read(struct txgbe_hw *hw) {
  ...
  if (hw->phy.multispeed_fiber) {
    ...
    hw->phy.link_mode =
        TXGBE_PHYSICAL_LAYER_1000BASE_T | TXGBE_PHYSICAL_LAYER_100BASE_TX;
...
void txgbe_autoc_write(struct txgbe_hw *hw, u64 autoc) {
  ...
  wr32m(hw, TXGBE_MACTXCFG, TXGBE_MACTXCFG_SPEED_MASK | TXGBE_MACTXCFG_TXE,
...
void txgbe_set_phy_temp(struct txgbe_hw *hw) {
  ...
  if (hw->phy.ffe_set == TXGBE_BP_M_SFI) {
    BP_LOG("Set SFI TX_EQ MAIN:%d PRE:%d POST:%d\n", hw->phy.ffe_main,
           hw->phy.ffe_pre, hw->phy.ffe_post);

    value = rd32_epcs(hw, TXGBE_PHY_TX_EQ_CTL0);
    value = (value & ~0x3F3F) | (hw->phy.ffe_main << 8) | hw->phy.ffe_pre;
    wr32_epcs(hw, TXGBE_PHY_TX_EQ_CTL0, value);
...
static void txgbe_get_bp_ability(struct txgbe_backplane_ability *ability,
  ...
  if (link_partner == 1) {
    ...
    ability->link_ability = value & SR_AN_MMD_LP_ABL2_BP_TYPE_KR_KX4_KX;
    ...
    ability->link_ability = value & SR_AN_MMD_ADV_REG2_BP_TYPE_KR_KX4_KX;
    BP_LOG("  Link Ability (bit[15:0]): 0x%x\n", ability->link_ability);
    BP_LOG("  (0x20- KX_ONLY, 0x40- KX4_ONLY, 0x60- KX4_KX\n");
    BP_LOG("   0x80- KR_ONLY, 0xA0- KR_KX, 0xC0- KR_KX4, 0xE0- KR_KX4_KX)\n");
...
static s32 txgbe_enable_kr_training(struct txgbe_hw *hw) {
  ...
  if (CL72_KRTR_PRBS_MODE_EN != 0xFFFF) {
     ...
     */
    wr32_epcs(hw, VR_PMA_KRTR_PRBS_CTRL1, VR_PMA_KRTR_PRBS_TIME_LMT);
...
static void txgbe_read_phy_lane_tx_eq(u16 lane, struct txgbe_hw *hw, s32 post,
  ...
  if (mode == 1) {
    ...
    wr32_epcs(hw, TXGBE_PHY_EQ_INIT_CTL0, tx_pre_cursor | (lmain << 8));
    value = rd32_epcs(hw, TXGBE_PHY_TX_EQ_CTL1);
    value &= ~TXGBE_PHY_TX_EQ_CTL1_DEF;
    wr32_epcs(hw, TXGBE_PHY_TX_EQ_CTL1, value);
  }
