...
#define TXGBE_RST_DEFAULT (TXGBE_RST_SW | TXGBE_RST_LAN(0) | TXGBE_RST_LAN(1))
...
#define TXGBE_TSTIMEINC_VP(v, p) (((v)&MS(0, 0xFFFFFF)) | TXGBE_TSTIMEINC_IP(p))
...
#define TXGBE_ICRMISC_DEFAULT                                                  \
  (TXGBE_ICRMISC_LNKDN | TXGBE_ICRMISC_RST | TXGBE_ICRMISC_ERRMAC |            \
   TXGBE_ICRMISC_LNKUP | TXGBE_ICRMISC_ANDONE | TXGBE_ICRMISC_ERRIG |          \
   TXGBE_ICRMISC_VFMBX | TXGBE_ICRMISC_MNGMBX | TXGBE_ICRMISC_STALL |          \
   TXGBE_ICRMISC_TIMER)
#define TXGBE_ICRMISC_LSC (TXGBE_ICRMISC_LNKDN | TXGBE_ICRMISC_LNKUP)
...
#define TXGBE_VFMBCTL_R2C_BITS                                                 \
  (TXGBE_VFMBCTL_RSTD | TXGBE_VFMBCTL_PFSTS | TXGBE_VFMBCTL_PFACK)
...
static inline u32 txgbe_map_reg(struct txgbe_hw *hw, u32 reg) {
  switch (reg) {
...
static inline u32 txgbe_get32(volatile u32 *addr) {
  u32 val = TXGBE_REG_READ32(addr);
  return rte_le_to_cpu_32(val);
}

static inline void txgbe_set32(volatile u32 *addr, u32 val) {
  val = rte_cpu_to_le_32(val);
  TXGBE_REG_WRITE32(addr, val);
}

static inline u32 txgbe_get32_masked(volatile u32 *addr, u32 mask) {
  u32 val = txgbe_get32(addr);
  val &= mask;
  return val;
}

static inline void txgbe_set32_masked(volatile u32 *addr, u32 mask, u32 field) {
  u32 val = txgbe_get32(addr);
  val = ((val & ~mask) | (field & mask));
  txgbe_set32(addr, val);
}

static inline u32 txgbe_get32_relaxed(volatile u32 *addr) {
  u32 val = TXGBE_REG_READ32_RELAXED(addr);
  return rte_le_to_cpu_32(val);
}

static inline void txgbe_set32_relaxed(volatile u32 *addr, u32 val) {
  val = rte_cpu_to_le_32(val);
  TXGBE_REG_WRITE32_RELAXED(addr, val);
}

static inline u32 rd32(struct txgbe_hw *hw, u32 reg) {
  if (reg == TXGBE_REG_DUMMY)
    return 0;
  return txgbe_get32(TXGBE_REG_ADDR(hw, reg));
}

static inline void wr32(struct txgbe_hw *hw, u32 reg, u32 val) {
  if (reg == TXGBE_REG_DUMMY)
    return;
  txgbe_set32(TXGBE_REG_ADDR(hw, reg), val);
}

static inline u32 rd32m(struct txgbe_hw *hw, u32 reg, u32 mask) {
  u32 val = rd32(hw, reg);
  val &= mask;
  return val;
}

static inline void wr32m(struct txgbe_hw *hw, u32 reg, u32 mask, u32 field) {
  u32 val = rd32(hw, reg);
  val = ((val & ~mask) | (field & mask));
  wr32(hw, reg, val);
}

static inline u64 rd64(struct txgbe_hw *hw, u32 reg) {
  u64 lsb = rd32(hw, reg);
  u64 msb = rd32(hw, reg + 4);
  return (lsb | msb << 32);
}

static inline void wr64(struct txgbe_hw *hw, u32 reg, u64 val) {
  wr32(hw, reg, (u32)val);
  wr32(hw, reg + 4, (u32)(val >> 32));
}

/* poll register */
static inline u32 po32m(struct txgbe_hw *hw, u32 reg, u32 mask, u32 expect,
                        u32 *actual, u32 loop, u32 slice) {
  bool usec = true;
  u32 value = 0, all = 0;
...
#define rd32a(hw, reg, idx) (rd32((hw), (reg) + ((idx) << 2)))
#define wr32a(hw, reg, idx, val) wr32((hw), (reg) + ((idx) << 2), (val))

#define rd32at(hw, reg, idx) rd32a(hw, txgbe_map_reg(hw, reg), idx)
#define wr32at(hw, reg, idx, val) wr32a(hw, txgbe_map_reg(hw, reg), idx, val)

#define rd32w(hw, reg, mask, slice)                                            \
  do {                                                                         \
    rd32((hw), reg);                                                           \
    po32m((hw), reg, mask, mask, NULL, 5, slice);                              \
  } while (0)

#define wr32w(hw, reg, val, mask, slice)                                       \
  do {                                                                         \
    wr32((hw), reg, val);                                                      \
    po32m((hw), reg, mask, mask, NULL, 5, slice);                              \
  } while (0)
...
static inline u32 rd32_epcs(struct txgbe_hw *hw, u32 addr) {
  u32 data;
...
static inline void wr32_epcs(struct txgbe_hw *hw, u32 addr, u32 data) {
  wr32(hw, TXGBE_XPCS_IDAADDR, addr);
  wr32(hw, TXGBE_XPCS_IDADATA, data);
}

static inline u32 rd32_ephy(struct txgbe_hw *hw, u32 addr) {
  u32 data;
...
static inline void wr32_ephy(struct txgbe_hw *hw, u32 addr, u32 data) {
  wr32(hw, TXGBE_EPHY_IDAADDR, addr);
