...
s32 txgbe_get_eeprom_semaphore(struct txgbe_hw *hw) {
  ...
  if (status == 0) {
    for (i = 0; i < timeout; i++) {
      ...
      wr32m(hw, TXGBE_MNGSWSYNC, TXGBE_MNGSWSYNC_REQ, TXGBE_MNGSWSYNC_REQ);
...
s32 txgbe_ee_readw_buffer(struct txgbe_hw *hw, u32 offset, u32 words,
  ...
  while (len) {
    u32 seg = (len <= TXGBE_PMMBX_DATA_SIZE ? len : TXGBE_PMMBX_DATA_SIZE);
...
s32 txgbe_calc_eeprom_checksum(struct txgbe_hw *hw) {
  ...
  for (i = 0; i < TXGBE_EE_CSUM_MAX; i += seg) {
    seg =
        (i + BUFF_SIZE < TXGBE_EE_CSUM_MAX ? BUFF_SIZE : TXGBE_EE_CSUM_MAX - i);
