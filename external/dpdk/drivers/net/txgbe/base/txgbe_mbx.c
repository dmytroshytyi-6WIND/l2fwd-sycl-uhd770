...
s32 txgbe_check_for_rst_vf(struct txgbe_hw *hw, u16 mbx_id) {
  ...
  if (!txgbe_check_for_bit_vf(hw, (TXGBE_VFMBCTL_RSTD | TXGBE_VFMBCTL_RSTI))) {
    ret_val = 0;
    hw->mbx.stats.rsts++;
  }
...
s32 txgbe_check_for_msg_pf(struct txgbe_hw *hw, u16 vf_number) {
  ...
  if (!txgbe_check_for_bit_pf(hw, TXGBE_MBVFICR_VFREQ_VF1 << vf_bit, index)) {
    ret_val = 0;
    hw->mbx.stats.reqs++;
  }
...
s32 txgbe_check_for_ack_pf(struct txgbe_hw *hw, u16 vf_number) {
  ...
  if (!txgbe_check_for_bit_pf(hw, TXGBE_MBVFICR_VFACK_VF1 << vf_bit, index)) {
    ret_val = 0;
    hw->mbx.stats.acks++;
  }
