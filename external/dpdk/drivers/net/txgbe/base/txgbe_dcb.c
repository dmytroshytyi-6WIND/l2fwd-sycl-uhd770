 ...
 */
int txgbe_dcb_pfc_enable(struct txgbe_hw *hw, uint8_t tc_num) {
  int ret_val = 0;
  ...
  if ((hw->fc.current_mode & txgbe_fc_tx_pause) && hw->fc.high_water[tc_num]) {
    fcrtl = TXGBE_FCWTRLO_TH(hw->fc.low_water[tc_num]) | TXGBE_FCWTRLO_XON;
    fcrth = TXGBE_FCWTRHI_TH(hw->fc.high_water[tc_num]) | TXGBE_FCWTRHI_XOFF;
  } else {
    /*
...
s32 txgbe_dcb_calculate_tc_credits_cee(struct txgbe_hw *hw,
  ...
  for (i = 0; i < TXGBE_DCB_TC_MAX; i++) {
    ...
    credit_refill =
        min(link_percentage * min_multiplier, (u32)TXGBE_DCB_MAX_CREDIT_REFILL);
 ...
 */
void txgbe_dcb_unpack_pfc_cee(struct txgbe_dcb_config *cfg, u8 *map,
                              u8 *pfc_up) {
...
u8 txgbe_dcb_get_tc_from_up(struct txgbe_dcb_config *cfg, int direction,
                            u8 up) {
  struct txgbe_dcb_tc_config *tc_config = &cfg->tc_config[0];
  u8 prio_mask = 1 << up;
