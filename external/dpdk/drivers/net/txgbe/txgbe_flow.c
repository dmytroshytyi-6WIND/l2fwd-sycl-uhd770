...
static int cons_parse_ntuple_filter(const struct rte_flow_attr *attr,
  ...
  if (act->type == RTE_FLOW_ACTION_TYPE_SECURITY) {
    ...
    return txgbe_crypto_add_ingress_sa_from_flow(
...
static int txgbe_parse_ntuple_filter(struct rte_eth_dev *dev,
...
static int txgbe_parse_ethertype_filter(struct rte_eth_dev *dev,
                                        const struct rte_flow_attr *attr,
                                        const struct rte_flow_item pattern[],
                                        const struct rte_flow_action actions[],
                                        struct rte_eth_ethertype_filter *filter,
...
static int txgbe_parse_syn_filter(struct rte_eth_dev *dev,
                                  const struct rte_flow_attr *attr,
                                  const struct rte_flow_item pattern[],
                                  const struct rte_flow_action actions[],
                                  struct rte_eth_syn_filter *filter,
...
static int txgbe_parse_l2_tn_filter(struct rte_eth_dev *dev,
                                    const struct rte_flow_attr *attr,
                                    const struct rte_flow_item pattern[],
                                    const struct rte_flow_action actions[],
...
static int txgbe_parse_fdir_act_attr(const struct rte_flow_attr *attr,
                                     const struct rte_flow_action actions[],
 ...
 * Item->last should be NULL.
 */
static int txgbe_parse_fdir_filter_normal(
    struct rte_eth_dev *dev __rte_unused, const struct rte_flow_attr *attr,
    const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], struct txgbe_fdir_rule *rule,
    struct rte_flow_error *error) {
  const struct rte_flow_item *item;
  const struct rte_flow_item_eth *eth_mask;
  const struct rte_flow_item_ipv4 *ipv4_spec;
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_IPV4) {
    ...
    if (item->spec) {
      rule->b_spec = TRUE;
      ipv4_spec = item->spec;
      rule->input.dst_ip[0] = ipv4_spec->hdr.dst_addr;
      rule->input.src_ip[0] = ipv4_spec->hdr.src_addr;
    }
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_IPV6) {
    ...
    if (item->spec) {
      rule->b_spec = TRUE;
      ipv6_spec = item->spec;
      rte_memcpy(rule->input.src_ip, ipv6_spec->hdr.src_addr, 16);
      rte_memcpy(rule->input.dst_ip, ipv6_spec->hdr.dst_addr, 16);
    }
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_TCP) {
    ...
    if (item->spec) {
      rule->b_spec = TRUE;
      tcp_spec = item->spec;
      rule->input.src_port = tcp_spec->hdr.src_port;
      rule->input.dst_port = tcp_spec->hdr.dst_port;
    }
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_UDP) {
    ...
    if (item->spec) {
      rule->b_spec = TRUE;
      udp_spec = item->spec;
      rule->input.src_port = udp_spec->hdr.src_port;
      rule->input.dst_port = udp_spec->hdr.dst_port;
    }
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_SCTP) {
    ...
    if (item->spec) {
      rule->b_spec = TRUE;
      sctp_spec = item->spec;
      rule->input.src_port = sctp_spec->hdr.src_port;
      rule->input.dst_port = sctp_spec->hdr.dst_port;
    }
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_RAW) {
    ...
    if (raw_spec->relative != 0 || raw_spec->search != 0 ||
        raw_spec->reserved != 0 ||
        raw_spec->offset > TXGBE_MAX_FLX_SOURCE_OFF || raw_spec->offset % 2 ||
 ...
 * item->last should be NULL.
 */
static int txgbe_parse_fdir_filter_tunnel(
...
static int txgbe_parse_fdir_filter(struct rte_eth_dev *dev,
                                   const struct rte_flow_attr *attr,
                                   const struct rte_flow_item pattern[],
                                   const struct rte_flow_action actions[],
                                   struct txgbe_fdir_rule *rule,
  ...
  ret =
      txgbe_parse_fdir_filter_normal(dev, attr, pattern, actions, rule, error);
  if (!ret)
    goto step_next;

  ret = txgbe_parse_fdir_filter_tunnel(attr, pattern, actions, rule, error);
  if (ret)
    return ret;
...
static int txgbe_parse_rss_filter(struct rte_eth_dev *dev,
                                  const struct rte_flow_attr *attr,
                                  const struct rte_flow_action actions[],
...
static void txgbe_clear_rss_filter(struct rte_eth_dev *dev) {
...
txgbe_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
                  const struct rte_flow_item pattern[],
                  const struct rte_flow_action actions[],
                  struct rte_flow_error *error) {
  int ret;
  ...
  txgbe_flow_mem_ptr =
      rte_zmalloc("txgbe_flow_mem", sizeof(struct txgbe_flow_mem), 0);
  ...
  TAILQ_INSERT_TAIL(&txgbe_flow_list, txgbe_flow_mem_ptr, entries);

  memset(&ntuple_filter, 0, sizeof(struct rte_eth_ntuple_filter));
  ret = txgbe_parse_ntuple_filter(dev, attr, pattern, actions, &ntuple_filter,
                                  error);
  ...
  if (!ret) {
    ...
    if (!ret) {
      ntuple_filter_ptr = rte_zmalloc(
          "txgbe_ntuple_filter", sizeof(struct txgbe_ntuple_filter_ele), 0);
  ...
  ret = txgbe_parse_ethertype_filter(dev, attr, pattern, actions,
                                     &ethertype_filter, error);
  if (!ret) {
    ret = txgbe_add_del_ethertype_filter(dev, &ethertype_filter, TRUE);
    if (!ret) {
  ...
  ret = txgbe_parse_syn_filter(dev, attr, pattern, actions, &syn_filter, error);
  if (!ret) {
  ...
  ret = txgbe_parse_fdir_filter(dev, attr, pattern, actions, &fdir_rule, error);
  if (!ret) {
    ...
    if (fdir_rule.b_mask) {
      if (!fdir_info->mask_added) {
        ...
        if (fdir_rule.mask.flex_bytes_mask)
          txgbe_fdir_set_flexbytes_offset(dev, fdir_rule.flex_bytes_offset);
  ...
  ret = txgbe_parse_l2_tn_filter(dev, attr, pattern, actions, &l2_tn_filter,
                                 error);
  if (!ret) {
    ret = txgbe_dev_l2_tunnel_filter_add(dev, &l2_tn_filter, FALSE);
    if (!ret) {
      l2_tn_filter_ptr = rte_zmalloc(
          "txgbe_l2_tn_filter", sizeof(struct txgbe_eth_l2_tunnel_conf_ele), 0);
      if (!l2_tn_filter_ptr) {
        PMD_DRV_LOG(ERR, "failed to allocate memory");
        goto out;
      }
  ...
  memset(&rss_conf, 0, sizeof(struct txgbe_rte_flow_rss_conf));
  ret = txgbe_parse_rss_filter(dev, attr, actions, &rss_conf, error);
  if (!ret) {
    ret = txgbe_config_rss_filter(dev, &rss_conf, TRUE);
    if (!ret) {
      rss_filter_ptr =
          rte_zmalloc("txgbe_rss_filter", sizeof(struct txgbe_rss_conf_ele), 0);
      if (!rss_filter_ptr) {
        PMD_DRV_LOG(ERR, "failed to allocate memory");
        goto out;
      }
      txgbe_rss_conf_init(&rss_filter_ptr->filter_info, &rss_conf.conf);
 ...
 */
static int txgbe_flow_validate(struct rte_eth_dev *dev,
                               const struct rte_flow_attr *attr,
                               const struct rte_flow_item pattern[],
                               const struct rte_flow_action actions[],
                               struct rte_flow_error *error) {
...
static int txgbe_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                              struct rte_flow_error *error) {
  int ret = 0;
  ...
  case RTE_ETH_FILTER_NTUPLE:
    ntuple_filter_ptr = (struct txgbe_ntuple_filter_ele *)pmd_flow->rule;
  ...
  case RTE_ETH_FILTER_ETHERTYPE:
    ethertype_filter_ptr = (struct txgbe_ethertype_filter_ele *)pmd_flow->rule;
    rte_memcpy(&ethertype_filter, &ethertype_filter_ptr->filter_info,
               sizeof(struct rte_eth_ethertype_filter));
    ret = txgbe_add_del_ethertype_filter(dev, &ethertype_filter, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&filter_ethertype_list, ethertype_filter_ptr, entries);
      rte_free(ethertype_filter_ptr);
    }
  ...
  case RTE_ETH_FILTER_SYN:
    syn_filter_ptr = (struct txgbe_eth_syn_filter_ele *)pmd_flow->rule;
  ...
  case RTE_ETH_FILTER_L2_TUNNEL:
    l2_tn_filter_ptr = (struct txgbe_eth_l2_tunnel_conf_ele *)pmd_flow->rule;
  ...
  case RTE_ETH_FILTER_HASH:
    rss_filter_ptr = (struct txgbe_rss_conf_ele *)pmd_flow->rule;
    ret = txgbe_config_rss_filter(dev, &rss_filter_ptr->filter_info, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&filter_rss_list, rss_filter_ptr, entries);
      rte_free(rss_filter_ptr);
    }
...
static int txgbe_flow_flush(struct rte_eth_dev *dev,
                            struct rte_flow_error *error) {
  int ret = 0;
