...
static int txgbe_node_capabilities_get(struct rte_eth_dev *dev,
                                       ...
                                       struct rte_tm_error *error);
static int txgbe_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
                                  struct rte_tm_error *error);
...
int txgbe_tm_ops_get(struct rte_eth_dev *dev __rte_unused, void *arg) {
  if (!arg)
    return -EINVAL;
...
void txgbe_tm_conf_uninit(struct rte_eth_dev *dev) {
  ...
  while ((shaper_profile = TAILQ_FIRST(&tm_conf->shaper_profile_list))) {
    TAILQ_REMOVE(&tm_conf->shaper_profile_list, shaper_profile, node);
    rte_free(shaper_profile);
  }
}

static inline uint8_t txgbe_tc_nb_get(struct rte_eth_dev *dev) {
...
static int txgbe_tm_capabilities_get(struct rte_eth_dev *dev,
   ...
   */
  /* port + TCs + queues */
  cap->n_nodes_max = 1 + TXGBE_DCB_TC_MAX + hw->mac.max_tx_queues;
  cap->n_levels_max = 3;
...
static int txgbe_shaper_profile_add(struct rte_eth_dev *dev,
  ...
  shaper_profile = rte_zmalloc("txgbe_tm_shaper_profile",
                               sizeof(struct txgbe_tm_shaper_profile), 0);
  if (!shaper_profile)
    return -ENOMEM;
...
static void txgbe_queue_base_nb_get(struct rte_eth_dev *dev,
                                    uint16_t tc_node_no, uint16_t *base,
                                    uint16_t *nb) {
...
static int txgbe_node_param_check(struct rte_eth_dev *dev, uint32_t node_id,
...
static int txgbe_node_add(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  ret = txgbe_node_param_check(dev, node_id, priority, weight, params, error);
  if (ret)
    return ret;
  ...
  if (params->shaper_profile_id != RTE_TM_SHAPER_PROFILE_ID_NONE) {
    shaper_profile =
        txgbe_shaper_profile_search(dev, params->shaper_profile_id);
    if (!shaper_profile) {
  ...
  if (parent_node_id == RTE_TM_NODE_ID_NULL) {
    ...
    /* add the root node */
    tm_node = rte_zmalloc("txgbe_tm_node", sizeof(struct txgbe_tm_node), 0);
    if (!tm_node)
      return -ENOMEM;
    tm_node->id = node_id;
  ...
  parent_node = txgbe_tm_node_search(dev, parent_node_id, &parent_node_type);
  if (!parent_node) {
    error->type = RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID;
...
static int txgbe_level_capabilities_get(struct rte_eth_dev *dev,
  ...
  if (level_id != TXGBE_TM_NODE_TYPE_QUEUE) {
    ...
    if (level_id == TXGBE_TM_NODE_TYPE_PORT)
      cap->nonleaf.sched_n_children_max = TXGBE_DCB_TC_MAX;
    else
      cap->nonleaf.sched_n_children_max = hw->mac.max_tx_queues;
...
static int txgbe_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
  ...
  TAILQ_FOREACH(tm_node, &tm_conf->tc_list, node) {
    if (tm_node->shaper_profile && tm_node->shaper_profile->profile.peak.rate) {
      error->type = RTE_TM_ERROR_TYPE_SHAPER_PROFILE;
  ...
  TAILQ_FOREACH(tm_node, &tm_conf->queue_list, node) {
    ...
    if (bw) {
      ...
      if (ret) {
        error->type = RTE_TM_ERROR_TYPE_SHAPER_PROFILE;
        error->message = "failed to set queue max bandwidth";
        goto fail_clear;
      }
    }
  }
