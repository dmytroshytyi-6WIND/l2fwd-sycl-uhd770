...
#define PMD_INIT_LOG(level, fmt, args...)                                      \
  rte_log(RTE_LOG_##level, txgbe_logtype_init, "%s(): " fmt "\n", __func__,    \
          ##args)

extern int txgbe_logtype_driver;
#define PMD_DRV_LOG(level, fmt, args...)                                       \
  rte_log(RTE_LOG_##level, txgbe_logtype_driver, "%s(): " fmt "\n", __func__,  \
          ##args)

#ifdef RTE_LIBRTE_TXGBE_DEBUG_RX
extern int txgbe_logtype_rx;
#define PMD_RX_LOG(level, fmt, args...)                                        \
  rte_log(RTE_LOG_##level, txgbe_logtype_rx, "%s(): " fmt "\n", __func__,      \
          ##args)
#else
#define PMD_RX_LOG(level, fmt, args...)                                        \
  do {                                                                         \
...
#define PMD_TX_LOG(level, fmt, args...)                                        \
  rte_log(RTE_LOG_##level, txgbe_logtype_tx, "%s(): " fmt "\n", __func__,      \
          ##args)
#else
#define PMD_TX_LOG(level, fmt, args...)                                        \
  do {                                                                         \
...
#define PMD_TX_FREE_LOG(level, fmt, args...)                                   \
  rte_log(RTE_LOG_##level, txgbe_logtype_tx_free, "%s(): " fmt "\n", __func__, \
          ##args)
#else
#define PMD_TX_FREE_LOG(level, fmt, args...)                                   \
  do {                                                                         \
...
#define BP_LOG(fmt, args...)                                                   \
  rte_log(RTE_LOG_DEBUG, txgbe_logtype_bp,                                     \
          "[%" PRIu64 ".%" PRIu64 "]%s(%d): " fmt, usec_stamp() / 1000000,     \
          usec_stamp() % 1000000, __func__, __LINE__, ##args)
