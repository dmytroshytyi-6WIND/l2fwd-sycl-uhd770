...
static void txgbe_crypto_clear_ipsec_tables(struct rte_eth_dev *dev) {
...
static int txgbe_crypto_add_sa(struct txgbe_crypto_session *ic_session) {
  ...
  if (ic_session->op == TXGBE_OP_AUTHENTICATED_DECRYPTION) {
    ...
    priv->rx_ip_tbl[ip_index].ip.ipv6[0] = ic_session->dst_ip.ipv6[0];
    priv->rx_ip_tbl[ip_index].ip.ipv6[1] = ic_session->dst_ip.ipv6[1];
    priv->rx_ip_tbl[ip_index].ip.ipv6[2] = ic_session->dst_ip.ipv6[2];
    priv->rx_ip_tbl[ip_index].ip.ipv6[3] = ic_session->dst_ip.ipv6[3];
    ...
    if (ic_session->op == TXGBE_OP_AUTHENTICATED_DECRYPTION)
      priv->rx_sa_tbl[sa_index].mode |= (IPSRXMOD_PROTO | IPSRXMOD_DECRYPT);
    ...
    reg_val = TXGBE_IPSRXIDX_ENA | TXGBE_IPSRXIDX_WRITE | TXGBE_IPSRXIDX_TB_IP |
              (ip_index << 3);
    if (priv->rx_ip_tbl[ip_index].ip.type == IPv4) {
      ...
      wr32(hw, TXGBE_IPSRXADDR(0), priv->rx_ip_tbl[ip_index].ip.ipv6[0]);
      wr32(hw, TXGBE_IPSRXADDR(1), priv->rx_ip_tbl[ip_index].ip.ipv6[1]);
      wr32(hw, TXGBE_IPSRXADDR(2), priv->rx_ip_tbl[ip_index].ip.ipv6[2]);
      wr32(hw, TXGBE_IPSRXADDR(3), priv->rx_ip_tbl[ip_index].ip.ipv6[3]);
    }
    wr32w(hw, TXGBE_IPSRXIDX, reg_val, TXGBE_IPSRXIDX_WRITE, 1000);
    ...
    wr32(hw, TXGBE_IPSRXSPI, priv->rx_sa_tbl[sa_index].spi);
    wr32(hw, TXGBE_IPSRXADDRIDX, priv->rx_sa_tbl[sa_index].ip_index);
    wr32w(hw, TXGBE_IPSRXIDX, reg_val, TXGBE_IPSRXIDX_WRITE, 1000);
    ...
    wr32(hw, TXGBE_IPSRXKEY(0), rte_cpu_to_be_32(*(uint32_t *)&key[12]));
    wr32(hw, TXGBE_IPSRXKEY(1), rte_cpu_to_be_32(*(uint32_t *)&key[8]));
    wr32(hw, TXGBE_IPSRXKEY(2), rte_cpu_to_be_32(*(uint32_t *)&key[4]));
    wr32(hw, TXGBE_IPSRXKEY(3), rte_cpu_to_be_32(*(uint32_t *)&key[0]));
    wr32(hw, TXGBE_IPSRXSALT, rte_cpu_to_be_32(ic_session->salt));
    wr32(hw, TXGBE_IPSRXMODE, priv->rx_sa_tbl[sa_index].mode);
    wr32w(hw, TXGBE_IPSRXIDX, reg_val, TXGBE_IPSRXIDX_WRITE, 1000);
    ...
    wr32(hw, TXGBE_IPSTXKEY(0), rte_cpu_to_be_32(*(uint32_t *)&key[12]));
    wr32(hw, TXGBE_IPSTXKEY(1), rte_cpu_to_be_32(*(uint32_t *)&key[8]));
    wr32(hw, TXGBE_IPSTXKEY(2), rte_cpu_to_be_32(*(uint32_t *)&key[4]));
    wr32(hw, TXGBE_IPSTXKEY(3), rte_cpu_to_be_32(*(uint32_t *)&key[0]));
    wr32(hw, TXGBE_IPSTXSALT, rte_cpu_to_be_32(ic_session->salt));
    wr32w(hw, TXGBE_IPSTXIDX, reg_val, TXGBE_IPSTXIDX_WRITE, 1000);
...
static int txgbe_crypto_remove_sa(struct rte_eth_dev *dev,
  ...
  if (ic_session->op == TXGBE_OP_AUTHENTICATED_DECRYPTION) {
    ...
    if (priv->rx_ip_tbl[ip_index].ref_count == 0) {
      reg_val = TXGBE_IPSRXIDX_WRITE | TXGBE_IPSRXIDX_TB_IP | (ip_index << 3);
      wr32(hw, TXGBE_IPSRXADDR(0), 0);
      wr32(hw, TXGBE_IPSRXADDR(1), 0);
      wr32(hw, TXGBE_IPSRXADDR(2), 0);
      wr32(hw, TXGBE_IPSRXADDR(3), 0);
...
static int txgbe_crypto_create_session(void *device,
  ...
  if (conf->crypto_xform->type != RTE_CRYPTO_SYM_XFORM_AEAD ||
      conf->crypto_xform->aead.algo != RTE_CRYPTO_AEAD_AES_GCM) {
  ...
  memcpy(&ic_session->salt, &aead_xform->key.data[aead_xform->key.length], 4);
...
static unsigned int txgbe_crypto_session_get_size(__rte_unused void *device) {
  return sizeof(struct txgbe_crypto_session);
}

static int txgbe_crypto_remove_session(void *device,
                                       struct rte_security_session *session) {
  struct rte_eth_dev *eth_dev = device;
  struct txgbe_crypto_session *ic_session =
      (struct txgbe_crypto_session *)get_sec_session_private_data(session);
...
static inline uint8_t txgbe_crypto_compute_pad_len(struct rte_mbuf *m) {
  if (m->nb_segs == 1) {
    /* 16 bytes ICV + 2 bytes ESP trailer + payload padding size
     * payload padding size is stored at <pkt_len - 18>
     */
    uint8_t *esp_pad_len = rte_pktmbuf_mtod_offset(
        m, uint8_t *,
        rte_pktmbuf_pkt_len(m) - (ESP_TRAILER_SIZE + ESP_ICV_SIZE));
    return *esp_pad_len + ESP_TRAILER_SIZE + ESP_ICV_SIZE;
  }
  return 0;
}

static int txgbe_crypto_update_mb(void *device __rte_unused,
                                  struct rte_security_session *session,
                                  struct rte_mbuf *m,
  ...
  if (ic_session->op == TXGBE_OP_AUTHENTICATED_ENCRYPTION) {
    union txgbe_crypto_tx_desc_md *mdata =
        (union txgbe_crypto_tx_desc_md *)rte_security_dynfield(m);
    mdata->enc = 1;
...
txgbe_crypto_capabilities_get(void *device __rte_unused) {
  ...
  static const struct rte_security_capability txgbe_security_capabilities[] = {
...
int txgbe_crypto_add_ingress_sa_from_flow(const void *sess, const void *ip_spec,
                                          uint8_t is_ipv6) {
  ...
  if (ic_session->op == TXGBE_OP_AUTHENTICATED_DECRYPTION) {
    if (is_ipv6) {
      ...
      rte_memcpy(ic_session->src_ip.ipv6, ipv6->hdr.src_addr, 16);
      rte_memcpy(ic_session->dst_ip.ipv6, ipv6->hdr.dst_addr, 16);
    } else {
...
static int txgbe_crypto_capable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
