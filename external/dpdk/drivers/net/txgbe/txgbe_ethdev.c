...
static int txgbe_dev_stats_get(struct rte_eth_dev *dev,
  ...
  stats->ierrors = hw_stats->rx_crc_errors +
                   ...
                   hw_stats->rx_oversize_errors + hw_stats->rx_drop_packets +
                   hw_stats->rx_illegal_byte_errors + hw_stats->rx_error_bytes +
                   hw_stats->rx_fragment_errors + hw_stats->rx_fcoe_crc_errors +
...
int txgbe_dev_link_update_share(struct rte_eth_dev *dev, int wait_to_complete) {
  ...
  if (link_up == 0) {
    if ((hw->subsystem_device_id & 0xFF) == TXGBE_DEV_ID_KR_KX_KX4) {
      ...
      intr->flags |= TXGBE_FLAG_NEED_LINK_CONFIG;
      rte_eal_alarm_set(10, txgbe_dev_setup_link_alarm_handler, dev);
    }
    return rte_eth_linkstatus_set(dev, &link);
