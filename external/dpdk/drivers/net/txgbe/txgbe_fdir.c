...
#define TXGBE_DEFAULT_FLEXBYTES_OFFSET                                         \
  12 /*default flexbytes offset in                                             \
        bytes*/
...
static int txgbe_fdir_enable(struct txgbe_hw *hw, uint32_t fdirctrl) {
   ...
   */
  fdirctrl |= TXGBE_FDIRCTL_MAXLEN(0xA) | TXGBE_FDIRCTL_FULLTHR(4);
...
static int txgbe_fdir_store_input_mask(struct rte_eth_dev *dev) {
  struct rte_eth_fdir_masks *input_mask = &dev->data->dev_conf.fdir_conf.mask;
  enum rte_fdir_mode mode = dev->data->dev_conf.fdir_conf.mode;
  struct txgbe_hw_fdir_info *info = TXGBE_DEV_FDIR(dev);
...
static int txgbe_set_fdir_flex_conf(struct rte_eth_dev *dev, uint32_t flex) {
  ...
  for (i = 0; i < conf->nb_payloads; i++) {
    ...
    if (((flex_cfg->src_offset[0] & 0x1) == 0) &&
      ...
      flex |= TXGBE_FDIRFLEXCFG_OFST(flex_cfg->src_offset[0] / 2);
    } else {
...
int txgbe_fdir_configure(struct rte_eth_dev *dev) {
  ...
  err = configure_fdir_flags(&dev->data->dev_conf.fdir_conf, &fdirctrl, &flex);
  if (err)
    return err;
...
atr_compute_perfect_hash(struct txgbe_atr_input *input,
  ...
  bucket_hash = txgbe_atr_compute_hash(input, TXGBE_ATR_BUCKET_HASH_KEY);
 ...
 * @hw: pointer to hardware structure
 */
static inline int txgbe_fdir_check_cmd_complete(struct txgbe_hw *hw,
                                                uint32_t *fdircmd) {
  int i;
...
atr_compute_signature_hash(struct txgbe_atr_input *input,
  ...
  sig_hash = txgbe_atr_compute_hash(input, TXGBE_ATR_SIGNATURE_HASH_KEY);

  return TXGBE_FDIRPIHASH_SIG(sig_hash) | TXGBE_FDIRPIHASH_BKT(bucket_hash);
}
...
static inline int txgbe_remove_fdir_filter(struct txgbe_hw_fdir_info *fdir_info,
                                           struct txgbe_atr_input *input) {
  int ret;
  struct txgbe_fdir_filter *fdir_filter;
...
int txgbe_fdir_filter_program(struct rte_eth_dev *dev,
  ...
  if (is_perfect) {
    ...
    fdirhash = atr_compute_signature_hash(
        &rule->input, dev->data->dev_conf.fdir_conf.pballoc);
  }
  ...
  if (node) {
    ...
    node = rte_zmalloc("txgbe_fdir", sizeof(struct txgbe_fdir_filter), 0);
    if (!node)
      return -ENOMEM;
    rte_memcpy(&node->input, &rule->input, sizeof(struct txgbe_atr_input));
    node->fdirflags = rule->fdirflags;
    node->fdirhash = fdirhash;
    node->queue = queue;
  ...
  if (is_perfect)
    err = fdir_write_perfect_filter(hw, &node->input, node->queue,
                                    node->fdirflags, node->fdirhash, fdir_mode);
  else
    err = fdir_add_signature_filter(hw, &node->input, node->queue,
...
void txgbe_fdir_filter_restore(struct rte_eth_dev *dev) {
  ...
  if (is_perfect) {
    TAILQ_FOREACH(node, &fdir_info->fdir_list, entries) {
      (void)fdir_write_perfect_filter(hw, &node->input, node->queue,
                                      node->fdirflags, node->fdirhash,
                                      fdir_mode);
    }
  } else {
    TAILQ_FOREACH(node, &fdir_info->fdir_list, entries) {
      (void)fdir_add_signature_filter(hw, &node->input, node->queue,
                                      node->fdirflags, node->fdirhash);
    }
  }
}
