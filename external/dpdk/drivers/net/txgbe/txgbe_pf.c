...
static inline uint16_t dev_num_vf(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);

  return pci_dev->max_vfs;
}

static inline int txgbe_vf_perm_addr_gen(struct rte_eth_dev *dev,
                                         uint16_t vf_num) {
  ...
  for (vfn = 0; vfn < vf_num; vfn++) {
    ...
    memcpy(vfinfo[vfn].vf_mac_addresses, vf_mac_addr, RTE_ETHER_ADDR_LEN);
  }

  return 0;
}

static inline int txgbe_mb_intr_setup(struct rte_eth_dev *dev) {
...
int txgbe_pf_host_init(struct rte_eth_dev *eth_dev) {
  ...
  *vfinfo = rte_zmalloc("vf_info", sizeof(struct txgbe_vf_info) * vf_num, 0);
  ...
  if (ret) {
    PMD_INIT_LOG(ERR, "failed to allocate switch domain for device %d", ret);
    rte_free(*vfinfo);
    *vfinfo = NULL;
    return ret;
  }
...
static void txgbe_add_tx_flow_control_drop_filter(struct rte_eth_dev *eth_dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  ...
  i = txgbe_ethertype_filter_lookup(filter_info, TXGBE_ETHERTYPE_FLOW_CTRL);
  if (i >= 0) {
    PMD_DRV_LOG(
        ERR, "A ether type filter entity for flow control already exists!\n");
    return;
  }

  ethertype_filter.ethertype = TXGBE_ETHERTYPE_FLOW_CTRL;
  ethertype_filter.etqf =
      TXGBE_ETFLT_ENA | TXGBE_ETFLT_TXAS | TXGBE_ETHERTYPE_FLOW_CTRL;
  ethertype_filter.etqs = 0;
  ethertype_filter.conf = TRUE;
  i = txgbe_ethertype_filter_insert(filter_info, &ethertype_filter);
  if (i < 0) {
    PMD_DRV_LOG(
        ERR,
...
static void txgbe_set_rx_mode(struct rte_eth_dev *eth_dev) {
  ...
  fctrl = rd32m(hw, TXGBE_PSRCTL, ~(TXGBE_PSRCTL_UCP | TXGBE_PSRCTL_MCP));
  fctrl |= TXGBE_PSRCTL_BCA | TXGBE_PSRCTL_MCHFENA;

  vmolr = rd32m(hw, TXGBE_POOLETHCTL(vfn),
                ~(TXGBE_POOLETHCTL_UCP | TXGBE_POOLETHCTL_MCP |
                  TXGBE_POOLETHCTL_UCHA | TXGBE_POOLETHCTL_MCHA));
  vmolr |= TXGBE_POOLETHCTL_BCA | TXGBE_POOLETHCTL_UTA | TXGBE_POOLETHCTL_VLA;
...
static inline void txgbe_vf_reset_event(struct rte_eth_dev *eth_dev,
                                        uint16_t vf) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  struct txgbe_vf_info *vfinfo = *(TXGBE_DEV_VFDATA(eth_dev));
  ...
  vmolr |=
      (TXGBE_POOLETHCTL_UCHA | TXGBE_POOLETHCTL_BCA | TXGBE_POOLETHCTL_UTA);
...
static inline void txgbe_vf_reset_msg(struct rte_eth_dev *eth_dev,
                                      uint16_t vf) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  uint32_t reg;
...
static int txgbe_disable_vf_mc_promisc(struct rte_eth_dev *eth_dev,
                                       uint32_t vf) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  uint32_t vmolr;
...
static int txgbe_vf_reset(struct rte_eth_dev *eth_dev, uint16_t vf,
                          uint32_t *msgbuf) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  struct txgbe_vf_info *vfinfo = *(TXGBE_DEV_VFDATA(eth_dev));
...
static int txgbe_vf_set_mac_addr(struct rte_eth_dev *eth_dev, uint32_t vf,
                                 uint32_t *msgbuf) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  struct txgbe_vf_info *vfinfo = *(TXGBE_DEV_VFDATA(eth_dev));
  int rar_entry = hw->mac.num_rar_entries - (vf + 1);
...
static int txgbe_vf_set_multicast(struct rte_eth_dev *eth_dev, uint32_t vf,
                                  uint32_t *msgbuf) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  struct txgbe_vf_info *vfinfo = *(TXGBE_DEV_VFDATA(eth_dev));
  int nb_entries =
      (msgbuf[0] & TXGBE_VT_MSGINFO_MASK) >> TXGBE_VT_MSGINFO_SHIFT;
  ...
  for (i = 0; i < vfinfo->num_vf_mc_hashes; i++) {
    mta_idx =
        (vfinfo->vf_mc_hashes[i] >> TXGBE_MTA_BIT_SHIFT) & TXGBE_MTA_INDEX_MASK;
...
static int txgbe_vf_set_vlan(struct rte_eth_dev *eth_dev, uint32_t vf,
                             uint32_t *msgbuf) {
  int add, vid;
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  struct txgbe_vf_info *vfinfo = *(TXGBE_DEV_VFDATA(eth_dev));

  add = (msgbuf[0] & TXGBE_VT_MSGINFO_MASK) >> TXGBE_VT_MSGINFO_SHIFT;
...
static int txgbe_set_vf_lpe(struct rte_eth_dev *eth_dev,
  ...
  if (max_frs < max_frame) {
    wr32m(hw, TXGBE_FRMSZ, TXGBE_FRMSZ_MAX_MASK, TXGBE_FRMSZ_MAX(max_frame));
  }

  return 0;
}

static int txgbe_negotiate_vf_api(struct rte_eth_dev *eth_dev, uint32_t vf,
                                  uint32_t *msgbuf) {
...
static int txgbe_get_vf_queues(struct rte_eth_dev *eth_dev, uint32_t vf,
                               uint32_t *msgbuf) {
  struct txgbe_vf_info *vfinfo = *TXGBE_DEV_VFDATA(eth_dev);
  ...
  case RTE_ETH_MQ_TX_VMDQ_ONLY:
    ...
    user_priority = TXGBD_POOLTAG_VTAG_UP(vmvir);
    if (vlana == TXGBE_POOLTAG_ACT_ALWAYS && (vid != 0 || user_priority != 0))
      num_tcs = 1;
    else
      num_tcs = 0;
    break;
...
static int txgbe_set_vf_mc_promisc(struct rte_eth_dev *eth_dev, uint32_t vf,
                                   uint32_t *msgbuf) {
  struct txgbe_vf_info *vfinfo = *(TXGBE_DEV_VFDATA(eth_dev));
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  ...
  case TXGBEVF_XCAST_MODE_MULTI:
    disable =
        TXGBE_POOLETHCTL_MCP | TXGBE_POOLETHCTL_UCP | TXGBE_POOLETHCTL_VLP;
  ...
  case TXGBEVF_XCAST_MODE_ALLMULTI:
    ...
    enable =
        TXGBE_POOLETHCTL_BCA | TXGBE_POOLETHCTL_MCHA | TXGBE_POOLETHCTL_MCP;
    break;
  case TXGBEVF_XCAST_MODE_PROMISC:
    fctrl = rd32(hw, TXGBE_PSRCTL);
    if (!(fctrl & TXGBE_PSRCTL_UCP)) {
      /* VF promisc requires PF in promisc */
      PMD_DRV_LOG(ERR, "Enabling VF promisc requires PF in promisc\n");
      return -1;
    }
...
static int txgbe_set_vf_macvlan_msg(struct rte_eth_dev *dev, uint32_t vf,
                                    uint32_t *msgbuf) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_vf_info *vf_info = *(TXGBE_DEV_VFDATA(dev));
  uint8_t *new_mac = (uint8_t *)(&msgbuf[1]);
  struct rte_ether_addr *ea = (struct rte_ether_addr *)new_mac;
  int index = (msgbuf[0] & TXGBE_VT_MSGINFO_MASK) >> TXGBE_VT_MSGINFO_SHIFT;
  ...
  if (index) {
    ...
    hw->mac.set_rar(hw, vf_info[vf].mac_count, new_mac, vf, true);
  } else {
...
static int txgbe_rcv_msg_from_vf(struct rte_eth_dev *eth_dev, uint16_t vf) {
  ...
  if (msgbuf[0] == TXGBE_VF_RESET) {
    ...
    rte_eth_dev_callback_process(eth_dev, RTE_ETH_EVENT_VF_MBOX, &ret_param);
    return ret;
  }
...
static inline void txgbe_rcv_ack_from_vf(struct rte_eth_dev *eth_dev,
                                         uint16_t vf) {
