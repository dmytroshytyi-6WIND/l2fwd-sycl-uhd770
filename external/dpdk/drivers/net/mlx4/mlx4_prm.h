...
#define MLX4_CQE_STATUS_PTYPE_MASK                                             \
  (MLX4_CQE_STATUS_IPV4 | MLX4_CQE_STATUS_IPV4F | MLX4_CQE_STATUS_IPV6 |       \
   MLX4_CQE_STATUS_IPV4OPT | MLX4_CQE_STATUS_TCP | MLX4_CQE_STATUS_UDP)
 ...
 */
static inline volatile struct mlx4_cqe *mlx4_get_cqe(struct mlx4_cq *cq,
                                                     uint32_t index) {
 ...
 */
static inline uint64_t mlx4_transpose(uint64_t val, uint64_t from,
                                      uint64_t to) {
  return (from >= to ? (val & from) / (from / to) : (val & from) * (to / from));
}
