 ...
 */
static void mlx4_rx_intr_vec_disable(struct mlx4_priv *priv) {
 ...
 */
static int mlx4_rx_intr_vec_enable(struct mlx4_priv *priv) {
  unsigned int i;
  ...
  for (i = 0; i != n; ++i) {
    ...
    if (rte_intr_efds_index_set(intr_handle, i, rxq->channel->fd))
      return -rte_errno;
 ...
 *   Pointer to private structure.
 */
static void mlx4_link_status_alarm(struct mlx4_priv *priv) {
  ...
  if (intr_conf->lsc && !mlx4_link_status_check(priv))
    rte_eth_dev_callback_process(ETH_DEV(priv), RTE_ETH_EVENT_INTR_LSC, NULL);
}
  ...
  if ((!link->link_speed && link->link_status) ||
    ...
    if (!priv->intr_alarm) {
      ...
      ret = rte_eal_alarm_set(MLX4_INTR_ALARM_TIMEOUT,
                              (void (*)(void *))mlx4_link_status_alarm, priv);
      if (ret)
        return ret;
      priv->intr_alarm = 1;
    }
 ...
 *   Pointer to private structure.
 */
static void mlx4_interrupt_handler(struct mlx4_priv *priv) {
  enum {
    LSC,
    RMV,
  };
  ...
  while (!mlx4_glue->get_async_event(priv->ctx, &event)) {
    ...
    case IBV_EVENT_DEVICE_FATAL:
      ...
      break;
    default:
      DEBUG("event type %d on physical port %d not handled", event.event_type,
  ...
  for (i = 0; i != RTE_DIM(caught); ++i)
    if (caught[i])
      rte_eth_dev_callback_process(ETH_DEV(priv), type[i], NULL);
}
 ...
 */
static void mlx4_arm_cq(struct rxq *rxq, int solicited) {
...
int mlx4_intr_uninstall(struct mlx4_priv *priv) {
  ...
  if (rte_intr_fd_get(priv->intr_handle) != -1) {
    rte_intr_callback_unregister(
        priv->intr_handle, (void (*)(void *))mlx4_interrupt_handler, priv);
...
int mlx4_rx_intr_disable(struct rte_eth_dev *dev, uint16_t idx) {
  ...
  if (!rxq || !rxq->channel) {
    ...
    ret = mlx4_glue->get_cq_event(rxq->cq->channel, &ev_cq, &ev_ctx);
  ...
  if (ret) {
    rte_errno = ret;
    if (ret != EAGAIN)
      WARN("unable to disable interrupt on rx queue %d", idx);
  } else {
    rxq->mcq.arm_sn++;
