 ...
 */
static int mp_primary_handle(const struct rte_mp_msg *mp_msg,
                             const void *peer) {
...
static void mp_req_on_rxtx(struct rte_eth_dev *dev,
  ...
  if (type != MLX4_MP_REQ_START_RXTX && type != MLX4_MP_REQ_STOP_RXTX) {
    ERROR("port %u unknown request (req_type %d)", dev->data->port_id, type);
    return;
  }
  ...
  for (i = 0; i < mp_rep.nb_received; i++) {
    ...
    if (res->result) {
      ERROR("port %u request failed on secondary #%d", dev->data->port_id, i);
      goto exit;
    }
  }
exit:
...
int mlx4_mp_req_mr_create(struct rte_eth_dev *dev, uintptr_t addr) {
  ...
  ret = rte_mp_request_sync(&mp_req, &mp_rep, &ts);
  if (ret) {
    ERROR("port %u request to primary process failed", dev->data->port_id);
    return -rte_errno;
  }
...
int mlx4_mp_req_verbs_cmd_fd(struct rte_eth_dev *dev) {
  ...
  ret = mp_res->fds[0];
  DEBUG("port %u command FD from primary is %d", dev->data->port_id, ret);
exit:
  free(mp_rep.msgs);
  return ret;
}
