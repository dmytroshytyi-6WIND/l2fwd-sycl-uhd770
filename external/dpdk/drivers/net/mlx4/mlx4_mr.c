 ...
 */
static int mr_btree_expand(struct mlx4_mr_btree *bt, int n) {
  void *mem;
  int ret = 0;
 ...
 */
static uint32_t mr_btree_lookup(struct mlx4_mr_btree *bt, uint16_t *idx,
                                uintptr_t addr) {
 ...
 *   0 on success, -1 on failure.
 */
static int mr_btree_insert(struct mlx4_mr_btree *bt,
...
int mlx4_mr_btree_init(struct mlx4_mr_btree *bt, int n, int socket) {
  ...
  bt->table = rte_calloc_socket("B-tree table", n, sizeof(struct mlx4_mr_cache),
                                0, socket);
  if (bt->table == NULL) {
    rte_errno = ENOMEM;
    ERROR("failed to allocate memory for btree cache on socket %d", socket);
    return -rte_errno;
  }
  bt->size = n;
  ...
  DEBUG("initialized B-tree %p with table %p", (void *)bt, (void *)bt->table);
  return 0;
}
 ...
 */
static int mr_find_next_chunk(struct mlx4_mr *mr, struct mlx4_mr_cache *entry,
                              int base_idx) {
  uintptr_t start = 0;
  uintptr_t end = 0;
  uint32_t idx = 0;
 ...
 *   0 on success, -1 on failure.
 */
static int mr_insert_dev_cache(struct rte_eth_dev *dev, struct mlx4_mr *mr) {
  struct mlx4_priv *priv = dev->data->dev_private;
  unsigned int n;

  DEBUG("port %u inserting MR(%p) to global cache", dev->data->port_id,
        (void *)mr);
 ...
 */
static struct mlx4_mr *mr_lookup_dev_list(struct rte_eth_dev *dev,
                                          struct mlx4_mr_cache *entry,
                                          uintptr_t addr) {
  struct mlx4_priv *priv = dev->data->dev_private;
  struct mlx4_mr *mr;
 ...
 */
static uint32_t mr_lookup_dev(struct rte_eth_dev *dev,
 ...
 */
static void mr_free(struct mlx4_mr *mr) {
  if (mr == NULL)
    return;
 ...
 *   Pointer to Ethernet device.
 */
static void mlx4_mr_garbage_collect(struct rte_eth_dev *dev) {
  struct mlx4_priv *priv = dev->data->dev_private;
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
static uint32_t mlx4_mr_create_secondary(struct rte_eth_dev *dev,
                                         struct mlx4_mr_cache *entry,
                                         uintptr_t addr) {
  struct mlx4_priv *priv = dev->data->dev_private;
  int ret;

  DEBUG("port %u requesting MR creation for address (%p)", dev->data->port_id,
        (void *)addr);
  ...
  DEBUG("port %u MR CREATED by primary process for %p:\n"
        "  [0x%" PRIxPTR ", 0x%" PRIxPTR "), lkey=0x%x",
        dev->data->port_id, (void *)addr, entry->start, entry->end,
        entry->lkey);
  return entry->lkey;
}
...
uint32_t mlx4_mr_create_primary(struct rte_eth_dev *dev,
  ...
  DEBUG("port %u creating a MR using address (%p)", dev->data->port_id,
        (void *)addr);
  /*
...
alloc_resources:
  ...
  DEBUG("port %u extending %p to [0x%" PRIxPTR ", 0x%" PRIxPTR "),"
        ...
        dev->data->port_id, (void *)addr, data.start, data.end, msl->page_sz,
        ms_n);
  /* Size of memory for bitmap. */
  bmp_size = rte_bitmap_get_memory_footprint(ms_n);
  mr = rte_zmalloc_socket(
      NULL, RTE_ALIGN_CEIL(sizeof(*mr), RTE_CACHE_LINE_SIZE) + bmp_size,
  ...
  if (mr_lookup_dev(dev, entry, addr) != UINT32_MAX) {
    ...
    DEBUG("port %u found MR for %p on final lookup, abort", dev->data->port_id,
          (void *)addr);
    rte_rwlock_write_unlock(&priv->mr.rwlock);
    rte_mcfg_mem_read_unlock();
    /*
  ...
  DEBUG("port %u MR CREATED (%p) for %p:\n"
        ...
        dev->data->port_id, (void *)mr, (void *)addr, data.start, data.end,
        rte_cpu_to_be_32(mr->ibv_mr->lkey), mr->ms_base_idx, mr->ms_n,
        mr->ms_bmp_n);
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
static uint32_t mlx4_mr_create(struct rte_eth_dev *dev,
                               struct mlx4_mr_cache *entry, uintptr_t addr) {
  uint32_t ret = 0;
 ...
 *   Pointer to Ethernet device.
 */
static void mr_rebuild_dev_cache(struct rte_eth_dev *dev) {
  struct mlx4_priv *priv = dev->data->dev_private;
  struct mlx4_mr *mr;
 ...
 */
static void mlx4_mr_mem_event_free_cb(struct rte_eth_dev *dev, const void *addr,
                                      size_t len) {
  struct mlx4_priv *priv = dev->data->dev_private;
  const struct rte_memseg_list *msl;
  struct mlx4_mr *mr;
  ...
  DEBUG("port %u free callback: addr=%p, len=%zu", dev->data->port_id, addr,
        len);
  msl = rte_mem_virt2memseg_list(addr);
  /* addr and len must be page-aligned. */
  MLX4_ASSERT((uintptr_t)addr == RTE_ALIGN((uintptr_t)addr, msl->page_sz));
  ...
  for (i = 0; i < ms_n; ++i) {
    ...
    DEBUG("port %u MR(%p): clear bitmap[%u] for addr %p", dev->data->port_id,
          (void *)mr, pos, (void *)start);
    rte_bitmap_clear(mr->ms_bmp, pos);
    if (--mr->ms_n == 0) {
      LIST_REMOVE(mr, mr);
      LIST_INSERT_HEAD(&priv->mr.mr_free_list, mr, mr);
      DEBUG("port %u remove MR(%p) from list", dev->data->port_id, (void *)mr);
    }
    /*
  ...
  if (rebuild) {
     ...
     */
    ++priv->mr.dev_gen;
    DEBUG("broadcasting local cache flush, gen=%d", priv->mr.dev_gen);
  }
  rte_rwlock_write_unlock(&priv->mr.rwlock);
#ifdef RTE_LIBRTE_MLX4_DEBUG
  if (rebuild)
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mlx4_mr_lookup_dev(struct rte_eth_dev *dev,
                                   struct mlx4_mr_ctrl *mr_ctrl,
                                   struct mlx4_mr_cache *entry,
                                   uintptr_t addr) {
  struct mlx4_priv *priv = dev->data->dev_private;
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mlx4_mr_addr2mr_bh(struct rte_eth_dev *dev,
                                   struct mlx4_mr_ctrl *mr_ctrl,
                                   uintptr_t addr) {
  uint32_t lkey;
  uint16_t bh_idx = 0;
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mlx4_tx_addr2mr_bh(struct txq *txq, uintptr_t addr) {
...
void mlx4_mr_flush_local_cache(struct mlx4_mr_ctrl *mr_ctrl) {
  ...
  DEBUG("mr_ctrl(%p): flushed, cur_gen=%d", (void *)mr_ctrl, mr_ctrl->cur_gen);
}
 ...
 */
static void mlx4_mr_update_ext_mp_cb(struct rte_mempool *mp, void *opaque,
  ...
  if (lkey != UINT32_MAX)
    return;
  mr =
      rte_zmalloc_socket(NULL, RTE_ALIGN_CEIL(sizeof(*mr), RTE_CACHE_LINE_SIZE),
  ...
  DEBUG("port %u register MR for chunk #%d of mempool (%s)", dev->data->port_id,
        mem_idx, mp->name);
  mr->ibv_mr =
      mlx4_glue->reg_mr(priv->pd, (void *)addr, len, IBV_ACCESS_LOCAL_WRITE);
  if (mr->ibv_mr == NULL) {
    WARN("port %u fail to create a verbs MR for address (%p)",
         dev->data->port_id, (void *)addr);
    rte_free(mr);
  ...
  DEBUG("port %u MR CREATED (%p) for external memory %p:\n"
        "  [0x%" PRIxPTR ", 0x%" PRIxPTR "),"
        " lkey=0x%x base_idx=%u ms_n=%u, ms_bmp_n=%u",
        dev->data->port_id, (void *)mr, (void *)addr, addr, addr + len,
        rte_cpu_to_be_32(mr->ibv_mr->lkey), mr->ms_base_idx, mr->ms_n,
        mr->ms_bmp_n);
  /* Insert to the global cache table. */
  mr_insert_dev_cache(dev, mr);
 ...
 *   0 on success, -1 on failure.
 */
static uint32_t mlx4_mr_update_ext_mp(struct rte_eth_dev *dev,
                                      struct mlx4_mr_ctrl *mr_ctrl,
...
static void mlx4_mr_update_mp_cb(struct rte_mempool *mp __rte_unused,
                                 void *opaque,
                                 struct rte_mempool_memhdr *memhdr,
                                 unsigned mem_idx __rte_unused) {
  struct mr_update_mp_data *data = opaque;
  ...
  lkey = mlx4_mr_addr2mr_bh(data->dev, data->mr_ctrl, (uintptr_t)memhdr->addr);
  if (lkey == UINT32_MAX)
    data->ret = -1;
}
...
void mlx4_mr_dump_dev(struct rte_eth_dev *dev) {
  ...
  LIST_FOREACH(mr, &priv->mr.mr_list, mr) {
    ...
    DEBUG("port %u MR[%u], LKey = 0x%x, ms_n = %u, ms_bmp_n = %u",
          dev->data->port_id, mr_n++, rte_cpu_to_be_32(mr->ibv_mr->lkey),
          mr->ms_n, mr->ms_bmp_n);
    if (mr->ms_n == 0)
      continue;
    for (n = 0; n < mr->ms_bmp_n;) {
      ...
      if (!ret.end)
        break;
      DEBUG("  chunk[%u], [0x%" PRIxPTR ", 0x%" PRIxPTR ")", chunk_n++,
            ret.start, ret.end);
    }
  }
