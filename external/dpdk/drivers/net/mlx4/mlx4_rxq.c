...
struct mlx4_rss *mlx4_rss_get(struct mlx4_priv *priv, uint64_t fields,
  ...
  if (fields == rss->fields && queues == rss->queues &&
      !memcmp(key, rss->key, MLX4_RSS_HASH_KEY_SIZE) &&
      !memcmp(queue_id, rss->queue_id, queue_id_size)) {
    ++rss->refcnt;
    return rss;
  }
  rss = rte_malloc(__func__,
                   offsetof(struct mlx4_rss, queue_id) + queue_id_size, 0);
  if (!rss)
    goto error;
  *rss = (struct mlx4_rss){
      .priv = priv,
...
int mlx4_rss_attach(struct mlx4_rss *rss) {
  ...
  rss->ind = mlx4_glue->create_rwq_ind_table(
      priv->ctx, &(struct ibv_rwq_ind_table_init_attr){
  ...
  rss->qp = mlx4_glue->create_qp_ex(
      ...
      &(struct ibv_qp_init_attr_ex){
          .comp_mask = (IBV_QP_INIT_ATTR_PD | IBV_QP_INIT_ATTR_RX_HASH |
  ...
  ret = mlx4_glue->modify_qp(rss->qp,
                             &(struct ibv_qp_attr){
...
int mlx4_rss_init(struct mlx4_priv *priv) {
  ...
  if (ETH_DEV(priv)->data->nb_rx_queues > priv->hw_rss_max_qps) {
    ERROR("RSS does not support more than %d queues", priv->hw_rss_max_qps);
    rte_errno = EINVAL;
    return -rte_errno;
  }
  /* Prepare range for RSS contexts before creating the first WQ. */
  ret = mlx4_glue->dv_set_context_attr(
      priv->ctx, MLX4DV_SET_CTX_ATTR_LOG_WQS_RANGE_SZ, &log2_range);
  if (ret) {
  ...
  for (i = 0; i != ETH_DEV(priv)->data->nb_rx_queues; ++i) {
    ...
    wq = mlx4_glue->create_wq(priv->ctx, &(struct ibv_wq_init_attr){
                                             .wq_type = IBV_WQT_RQ,
                                             .max_wr = 1,
                                             .max_sge = 1,
                                             .pd = priv->pd,
  ...
  return 0;
error:
  ERROR("cannot initialize common RSS resources (queue %u): %s: %s", i, msg,
        strerror(ret));
  while (i--) {
    struct rxq *rxq = ETH_DEV(priv)->data->rx_queues[i];
...
int mlx4_rxq_attach(struct rxq *rxq) {
  ...
  cq = mlx4_glue->create_cq(priv->ctx, elts_n / sges_n, NULL, rxq->channel, 0);
  if (!cq) {
    ret = ENOMEM;
  ...
  ret = mlx4_glue->modify_wq(wq, &(struct ibv_wq_attr){
  ...
  DEBUG("port %u Rx queue %u registering mp %s having %u chunks",
        ETH_DEV(priv)->data->port_id, rxq->stats.idx, rxq->mp->name,
        rxq->mp->nb_mem_chunks);
  mlx4_mr_update_mp(dev, &rxq->mr_ctrl, rxq->mp);
  wqes = (volatile struct mlx4_wqe_data_seg(*)[])((uintptr_t)dv_rwq.buf.buf +
                                                  dv_rwq.rq.offset);
...
error:
  ...
  --rxq->usecnt;
  rte_errno = ret;
  ERROR("error while attaching Rx queue %p: %s: %s", (void *)rxq, msg,
        strerror(ret));
  priv->verbs_alloc_ctx.type = MLX4_VERBS_ALLOC_TYPE_NONE;
  return -ret;
}
...
uint64_t mlx4_get_rx_queue_offloads(struct mlx4_priv *priv) {
  uint64_t offloads = RTE_ETH_RX_OFFLOAD_SCATTER | RTE_ETH_RX_OFFLOAD_KEEP_CRC |
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
int mlx4_rx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc,
  ...
  DEBUG("%p: configuring queue %u for %u descriptors", (void *)dev, idx, desc);

  if (idx >= dev->data->nb_rx_queues) {
    rte_errno = EOVERFLOW;
    ERROR("%p: queue index out of range (%u >= %u)", (void *)dev, idx,
  ...
  if (rxq) {
    rte_errno = EEXIST;
    ERROR("%p: Rx queue %u already configured, release it first", (void *)dev,
          idx);
    return -rte_errno;
  }
  if (!desc) {
  ...
  DEBUG("%p: CRC stripping is %s, %u bytes will be subtracted from"
        ...
        (void *)dev, crc_present ? "disabled" : "enabled", crc_present << 2);
  /* Allocate and initialize Rx queue. */
  mlx4_zmallocv_socket("RXQ", vec, RTE_DIM(vec), socket);
  if (!rxq) {
    ERROR("%p: unable to allocate queue index %u", (void *)dev, idx);
    return -rte_errno;
  }
  *rxq = (struct rxq){
      .priv = priv,
      ...
      .csum = priv->hw_csum && (offloads & RTE_ETH_RX_OFFLOAD_CHECKSUM),
      .csum_l2tun =
          priv->hw_csum_l2tun && (offloads & RTE_ETH_RX_OFFLOAD_CHECKSUM),
  ...
  if (max_rx_pktlen <= (mb_len - RTE_PKTMBUF_HEADROOM)) {
    ...
    if (size < max_rx_pktlen) {
      ...
      ERROR("%p: too many SGEs (%u) needed to handle"
            ...
            (void *)dev, 1 << sges_n, max_rx_pktlen);
      goto error;
    }
  } else {
    WARN("%p: the requested maximum Rx packet size (%u) is"
         ...
         (void *)dev, max_rx_pktlen, mb_len - RTE_PKTMBUF_HEADROOM);
  }
  DEBUG("%p: maximum number of segments per packet: %u", (void *)dev,
        1 << rxq->sges_n);
  if (desc % (1 << rxq->sges_n)) {
    rte_errno = EINVAL;
    ERROR("%p: number of Rx queue descriptors (%u) is not a"
          " multiple of maximum segments per packet (%u)",
          (void *)dev, desc, 1 << rxq->sges_n);
    goto error;
  }
  if (mlx4_mr_btree_init(&rxq->mr_ctrl.cache_bh, MLX4_MR_BTREE_CACHE_N,
                         socket)) {
