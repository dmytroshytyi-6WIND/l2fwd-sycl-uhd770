...
struct mlx4_flow_proc_item {
  ...
  int (*merge)(struct rte_flow *flow, const struct rte_flow_item *item,
...
uint64_t mlx4_conv_rss_types(struct mlx4_priv *priv, uint64_t types,
  ...
  if ((verbs_to_dpdk || (conv & priv->hw_rss_sup) == conv) && !(types & ~seen))
    return conv;
  rte_errno = ENOTSUP;
  return (uint64_t)-1;
}
 ...
 */
static int mlx4_flow_merge_eth(struct rte_flow *flow,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_merge_vlan(struct rte_flow *flow,
                                const struct rte_flow_item *item,
                                const struct mlx4_flow_proc_item *proc,
                                struct rte_flow_error *error) {
  ...
  if (!flow->ibv_attr)
    return 0;
  eth =
      (void *)((uintptr_t)flow->ibv_attr + flow->ibv_attr_size - sizeof(*eth));
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_merge_ipv4(struct rte_flow *flow,
                                const struct rte_flow_item *item,
                                const struct mlx4_flow_proc_item *proc,
                                struct rte_flow_error *error) {
  ...
  if (mask && ((uint32_t)(mask->hdr.src_addr + 1) > UINT32_C(1) ||
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_merge_udp(struct rte_flow *flow,
                               const struct rte_flow_item *item,
                               const struct mlx4_flow_proc_item *proc,
                               struct rte_flow_error *error) {
  ...
  if (mask && ((uint16_t)(mask->hdr.src_port + 1) > UINT16_C(1) ||
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_merge_tcp(struct rte_flow *flow,
                               const struct rte_flow_item *item,
                               const struct mlx4_flow_proc_item *proc,
                               struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_item_check(const struct rte_flow_item *item,
                                const struct mlx4_flow_proc_item *proc,
                                struct rte_flow_error *error) {
  const uint8_t *mask;
  unsigned int i;
...
static const struct mlx4_flow_proc_item mlx4_flow_proc_item_list[] = {
    ...
    [RTE_FLOW_ITEM_TYPE_ETH] =
        {
            .next_item =
                NEXT_ITEM(RTE_FLOW_ITEM_TYPE_VLAN, RTE_FLOW_ITEM_TYPE_IPV4),
            .mask_support =
    ...
    [RTE_FLOW_ITEM_TYPE_IPV4] =
        {
            .next_item =
                NEXT_ITEM(RTE_FLOW_ITEM_TYPE_UDP, RTE_FLOW_ITEM_TYPE_TCP),
            .mask_support =
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_prepare(struct mlx4_priv *priv,
  ...
  if (attr->priority > MLX4_FLOW_PRIORITY_LAST)
    return rte_flow_error_set(
        ...
        "maximum priority level is " MLX4_STR_EXPAND(MLX4_FLOW_PRIORITY_LAST));
  if (attr->egress)
...
fill:
  ...
  for (action = actions; action->type; ++action) {
    ...
    case RTE_FLOW_ACTION_TYPE_QUEUE:
      ...
      flow->rss =
          mlx4_rss_get(priv, 0, mlx4_rss_hash_key_default, 1, &queue->index);
      if (!flow->rss) {
    ...
    case RTE_FLOW_ACTION_TYPE_RSS:
      ...
      if (rss->key_len) {
        rss_key = rss->key ? rss->key : mlx4_rss_hash_key_default;
      ...
      for (i = 0; i < rss->queue_num; ++i)
        if (rss->queue[i] >= ETH_DEV(priv)->data->nb_rx_queues)
          break;
      if (i != rss->queue_num) {
      ...
      if (rss_key_len != sizeof(flow->rss->key)) {
        msg = "mlx4 supports exactly one RSS hash key"
              " length: " MLX4_STR_EXPAND(MLX4_RSS_HASH_KEY_SIZE);
        goto exit_action_not_supported;
      }
      ...
      flow->rss =
          mlx4_rss_get(priv, fields, rss_key, rss->queue_num, rss->queue);
      if (!flow->rss) {
 ...
 * @see rte_flow_ops
 */
static int mlx4_flow_validate(struct rte_eth_dev *dev,
                              const struct rte_flow_attr *attr,
                              const struct rte_flow_item pattern[],
                              const struct rte_flow_action actions[],
                              struct rte_flow_error *error) {
 ...
 *   is set.
 */
static struct mlx4_drop *mlx4_drop_get(struct mlx4_priv *priv) {
  ...
  *drop = (struct mlx4_drop){
  ...
  if (!drop->cq)
    goto error;
  drop->qp = mlx4_glue->create_qp(priv->pd, &(struct ibv_qp_init_attr){
                                                .send_cq = drop->cq,
                                                .recv_cq = drop->cq,
 ...
 */
static void mlx4_drop_put(struct mlx4_drop *drop) {
  MLX4_ASSERT(drop->refcnt);
  if (--drop->refcnt)
    return;
  drop->priv->drop = NULL;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_toggle(struct mlx4_priv *priv, struct rte_flow *flow,
                            int enable, struct rte_flow_error *error) {
  ...
  if (!flow->internal && !priv->isolated &&
    ...
    err = EACCES;
    msg = ("priority level " MLX4_STR_EXPAND(
        MLX4_FLOW_PRIORITY_LAST) " is reserved when not in isolated mode");
    goto error;
  }
  if (flow->rss) {
    ...
    for (i = 0; i != rss->queues; ++i)
      if (rss->queue_id[i] >= ETH_DEV(priv)->data->nb_rx_queues ||
...
error:
  return rte_flow_error_set(error, err, RTE_FLOW_ERROR_TYPE_HANDLE, flow, msg);
}
 ...
 * @see rte_flow_ops
 */
static struct rte_flow *mlx4_flow_create(struct rte_eth_dev *dev,
                                         const struct rte_flow_attr *attr,
                                         const struct rte_flow_item pattern[],
                                         const struct rte_flow_action actions[],
                                         struct rte_flow_error *error) {
  ...
  if (!err) {
    ...
    if (!curr || !curr->internal) {
      ...
      while (LIST_NEXT(curr, next) && LIST_NEXT(curr, next)->internal)
 ...
 * @see rte_flow_ops
 */
static int mlx4_flow_isolate(struct rte_eth_dev *dev, int enable,
                             struct rte_flow_error *error) {
  struct mlx4_priv *priv = dev->data->dev_private;
 ...
 * @see rte_flow_ops
 */
static int mlx4_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                             struct rte_flow_error *error) {
  struct mlx4_priv *priv = dev->data->dev_private;
 ...
 * @see rte_flow_ops
 */
static int mlx4_flow_flush(struct rte_eth_dev *dev,
                           struct rte_flow_error *error) {
  struct mlx4_priv *priv = dev->data->dev_private;
 ...
 */
static uint16_t mlx4_flow_internal_next_vlan(struct mlx4_priv *priv,
                                             uint16_t vlan) {
  while (vlan < 4096) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_internal(struct mlx4_priv *priv,
                              struct rte_flow_error *error) {
   ...
   * get RSS by default.
   */
  uint32_t queues = rte_align32pow2(ETH_DEV(priv)->data->nb_rx_queues + 1) >> 1;
  ...
  rte_be16_t *rule_vlan = (ETH_DEV(priv)->data->dev_conf.rxmode.offloads &
  ...
  for (i = 0; i != RTE_DIM(priv->mac) + 1; ++i) {
    ...
    for (flow = LIST_FIRST(&priv->flows); flow && flow->internal;
         flow = LIST_NEXT(flow, next)) {
      const struct ibv_flow_spec_eth *eth =
          (const void *)((uintptr_t)flow->ibv_attr + sizeof(*flow->ibv_attr));
      unsigned int j;

      if (!flow->mac)
        continue;
      MLX4_ASSERT(flow->ibv_attr->type == IBV_FLOW_ATTR_NORMAL);
      MLX4_ASSERT(flow->ibv_attr->num_of_specs == 1);
      MLX4_ASSERT(eth->type == IBV_FLOW_SPEC_ETH);
      MLX4_ASSERT(flow->rss);
      if (rule_vlan && (eth->val.vlan_tag != *rule_vlan ||
    ...
    if (!flow || !flow->internal) {
      ...
      flow = mlx4_flow_create(ETH_DEV(priv), &attr, pattern, actions, error);
      if (!flow) {
        err = -rte_errno;
        goto error;
      }
  ...
  if (ETH_DEV(priv)->data->promiscuous || ETH_DEV(priv)->data->all_multicast) {
    for (flow = LIST_FIRST(&priv->flows); flow && flow->internal;
         flow = LIST_NEXT(flow, next)) {
...
error:
  ...
  while (flow && flow->internal) {
    ...
    if (!flow->select)
      claim_zero(mlx4_flow_destroy(ETH_DEV(priv), flow, error));
    else
      flow->select = 0;
    flow = next;
  }
 ...
 * @return 0
 */
int mlx4_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
