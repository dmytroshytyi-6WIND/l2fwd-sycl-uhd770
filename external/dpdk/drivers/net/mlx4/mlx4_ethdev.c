...
int mlx4_get_ifname(const struct mlx4_priv *priv, char (*ifname)[IF_NAMESIZE]) {
  ...
  while ((dent = readdir(dir)) != NULL) {
    ...
    MKSTR(path, "%s/device/net/%s/%s", priv->ctx->device->ibdev_path, name,
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_ifreq(const struct mlx4_priv *priv, int req,
                      struct ifreq *ifr) {
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_set_flags(struct mlx4_priv *priv, unsigned int keep,
                          unsigned int flags) {
  struct ifreq request;
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_dev_set_link(struct mlx4_priv *priv, int up) {
  int err;
 ...
 */
static int mlx4_rxmode_toggle(struct rte_eth_dev *dev,
...
int mlx4_set_mc_addr_list(struct rte_eth_dev *dev, struct rte_ether_addr *list,
  ...
  if (num > priv->mac_mc) {
    ...
    for (i = RTE_DIM(priv->mac) - num; i != RTE_DIM(priv->mac) - priv->mac_mc;
         ++i)
    ...
    memset(priv->mac + RTE_DIM(priv->mac) - priv->mac_mc, 0,
...
int mlx4_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on) {
  ...
  *v |= (uint64_t) !!on << vbit;
  ...
  ERROR("failed to synchronize flow rules after %s VLAN filter on ID %u"
        " (code %d, \"%s\"), "
        " flow error type %d, cause %p, message: %s",
        on ? "enabling" : "disabling", vlan_id, rte_errno, strerror(rte_errno),
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
int mlx4_mac_addr_set(struct rte_eth_dev *dev,
 ...
 */
int mlx4_dev_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *info) {
  struct mlx4_priv *priv = dev->data->dev_private;
  unsigned int max;
  ...
  max = ((priv->device_attr.max_cq > priv->device_attr.max_qp)
             ? priv->device_attr.max_qp
             : priv->device_attr.max_cq);
  ...
  info->rx_offload_capa =
      (mlx4_get_rx_port_offloads(priv) | info->rx_queue_offload_capa);
  info->if_index = priv->if_index;
  info->hash_key_size = MLX4_RSS_HASH_KEY_SIZE;
  info->speed_capa = RTE_ETH_LINK_SPEED_1G | RTE_ETH_LINK_SPEED_10G |
                     RTE_ETH_LINK_SPEED_20G | RTE_ETH_LINK_SPEED_40G |
...
int mlx4_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  for (i = 0; i != dev->data->nb_rx_queues; ++i) {
    ...
    if (idx < RTE_ETHDEV_QUEUE_STAT_CNTRS) {
      ...
      tmp.q_errors[idx] += (rxq->stats.idropped + rxq->stats.rx_nombuf);
    }
...
int mlx4_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
  ...
  if (mlx4_ifreq(priv, SIOCETHTOOL, &ifr)) {
    WARN("ioctl(SIOCETHTOOL, ETHTOOL_GSET) failed: %s", strerror(rte_errno));
    return -rte_errno;
  }
...
const uint32_t *mlx4_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {/* refers to rxq_cq_to_pkt_type() */
                                    ...
                                    RTE_PTYPE_L4_UDP,
                                    RTE_PTYPE_UNKNOWN};
  static const uint32_t ptypes_l2tun[] = {/* refers to rxq_cq_to_pkt_type() */
                                          RTE_PTYPE_L2_ETHER,
                                          RTE_PTYPE_L3_IPV4_EXT_UNKNOWN,
                                          RTE_PTYPE_L3_IPV6_EXT_UNKNOWN,
                                          RTE_PTYPE_L4_FRAG,
