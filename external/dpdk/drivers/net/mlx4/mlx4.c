 ...
 */
static int mlx4_init_shared_data(void) {
  ...
  if (mlx4_shared_data == NULL) {
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
      ...
      mz = rte_memzone_reserve(MZ_MLX4_PMD_SHARED_DATA,
                               sizeof(*mlx4_shared_data), SOCKET_ID_ANY, 0);
      if (mz == NULL) {
 ...
 */
static void *mlx4_alloc_verbs_buf(size_t size, void *data) {
  ...
  if (priv->verbs_alloc_ctx.type == MLX4_VERBS_ALLOC_TYPE_TX_QUEUE) {
    ...
    socket = txq->socket;
  } else if (priv->verbs_alloc_ctx.type == MLX4_VERBS_ALLOC_TYPE_RX_QUEUE) {
 ...
 *   A pointer to the callback data.
 */
static void mlx4_free_verbs_buf(void *ptr, void *data __rte_unused) {
  MLX4_ASSERT(data != NULL);
  rte_free(ptr);
}
#endif
...
int mlx4_proc_priv_init(struct rte_eth_dev *dev) {
   ...
   */
  ppriv_size =
      sizeof(struct mlx4_proc_priv) + dev->data->nb_tx_queues * sizeof(void *);
  ppriv = rte_zmalloc_socket("mlx4_proc_priv", ppriv_size, RTE_CACHE_LINE_SIZE,
 ...
 */
static int mlx4_dev_configure(struct rte_eth_dev *dev) {
  ...
  if (ret) {
    ERROR("%p: interrupt handler installation failed", (void *)dev);
    goto exit;
  }
  ret = mlx4_proc_priv_init(dev);
  if (ret) {
    ERROR("%p: process private data allocation failed", (void *)dev);
    goto exit;
  }
exit:
  return ret;
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_dev_start(struct rte_eth_dev *dev) {
  struct mlx4_priv *priv = dev->data->dev_private;
  struct rte_flow_error error;
  int ret;
  ...
  if (ret) {
    ERROR("%p: cannot initialize RSS resources: %s", (void *)dev,
          strerror(-ret));
    goto err;
  }
  ...
  if (ret) {
    ERROR("%p: cannot attach flow rules (code %d, \"%s\"),"
          " flow error type %d, cause %p, message: %s",
          (void *)dev, -ret, strerror(-ret), error.type, error.cause,
          error.message ? error.message : "(unspecified)");
    goto err;
  }
  rte_wmb();
 ...
 *   Pointer to Ethernet device structure.
 */
static int mlx4_dev_close(struct rte_eth_dev *dev) {
  struct mlx4_priv *priv = dev->data->dev_private;
  unsigned int i;
  ...
  DEBUG("%p: closing device \"%s\"", (void *)dev,
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_ibv_device_to_pci_addr(const struct ibv_device *device,
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_arg_parse(const char *key, const char *val,
  ...
  if (strcmp(MLX4_PMD_PORT_KVARG, key) == 0) {
    ...
    if (tmp >= ports) {
      ERROR("port index %lu outside range [0,%" PRIu32 ")", tmp, ports);
      return -EINVAL;
    }
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_args(struct rte_devargs *devargs, struct mlx4_conf *conf) {
  ...
  for (i = 0; pmd_mlx4_init_params[i]; ++i) {
    ...
    while (arg_count-- > 0) {
      ret = rte_kvargs_process(
          kvlist, pmd_mlx4_init_params[i],
          (int (*)(const char *, const char *, void *))mlx4_arg_parse, conf);
      if (ret != 0)
        goto free_kvlist;
    }
  }
 ...
 */
static uint64_t mlx4_hw_rss_sup(struct ibv_context *ctx, struct ibv_pd *pd,
  ...
  if (!hw_rss_sup) {
    ...
    return IBV_RX_HASH_SRC_IPV4 | IBV_RX_HASH_DST_IPV4 | IBV_RX_HASH_SRC_IPV6 |
           IBV_RX_HASH_DST_IPV6 | IBV_RX_HASH_SRC_PORT_TCP |
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_init_once(void) {
  ...
  case RTE_PROC_PRIMARY:
    ...
    rte_mem_event_callback_register("MLX4_MEM_EVENT_CB", mlx4_mr_mem_event_cb,
                                    NULL);
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_pci_probe(struct rte_pci_driver *pci_drv,
  ...
  err = mlx4_init_once();
  if (err) {
    ERROR("unable to init PMD global data: %s", strerror(rte_errno));
    return -rte_errno;
  }
  ...
  while (i != 0) {
    ...
    if ((pci_dev->addr.domain != pci_addr.domain) ||
      ...
      continue;
    vf = (pci_dev->id.device_id == PCI_DEVICE_ID_MELLANOX_CONNECTX3VF);
    INFO("PCI information matches, using device \"%s\" (VF: %s)", list[i]->name,
         (vf ? "true" : "false"));
  ...
  for (i = 0; i < device_attr.phys_port_cnt; i++) {
    ...
    if (port_attr.link_layer != IBV_LINK_LAYER_ETHERNET) {
      err = ENOTSUP;
      ERROR("port %d is not configured in Ethernet mode", port);
      goto port_error;
    }
    if (port_attr.state != IBV_PORT_ACTIVE)
      DEBUG("port %d is not active: \"%s\" (%d)", port,
            mlx4_glue->port_state_str(port_attr.state), port_attr.state);
    /* Make asynchronous FD non-blocking to handle interrupts. */
    err = mlx4_fd_set_non_blocking(ctx->async_fd);
    if (err) {
      ERROR("cannot make asynchronous FD non-blocking: %s", strerror(err));
      goto port_error;
    }
    ...
    /* from rte_ethdev.c */
    priv = rte_zmalloc("ethdev private structure", sizeof(*priv),
                       RTE_CACHE_LINE_SIZE);
    if (priv == NULL) {
      err = ENOMEM;
    ...
    priv->vf = vf;
    priv->hw_csum = !!(device_attr.device_cap_flags & IBV_DEVICE_RAW_IP_CSUM);
    DEBUG("checksum offloading is %ssupported", (priv->hw_csum ? "" : "not "));
    /* Only ConnectX-3 Pro supports tunneling. */
    priv->hw_csum_l2tun =
        priv->hw_csum &&
        (device_attr.vendor_part_id == PCI_DEVICE_ID_MELLANOX_CONNECTX3PRO);
    DEBUG("L2 tunnel checksum offloads are %ssupported",
          priv->hw_csum_l2tun ? "" : "not ");
    priv->hw_rss_sup = mlx4_hw_rss_sup(priv->ctx, priv->pd, &device_attr_ex);
    DEBUG("supported RSS hash fields mask: %016" PRIx64, priv->hw_rss_sup);
    priv->hw_rss_max_qps =
        device_attr_ex.rss_caps.max_rwq_indirection_table_size;
    DEBUG("MAX RSS queues %d", priv->hw_rss_max_qps);
    priv->hw_fcs_strip =
        !!(device_attr_ex.raw_packet_caps & IBV_RAW_PACKET_CAP_SCATTER_FCS);
    DEBUG("FCS stripping toggling is %ssupported",
          priv->hw_fcs_strip ? "" : "not ");
    priv->tso =
        ((device_attr_ex.tso_caps.max_tso > 0) &&
         (device_attr_ex.tso_caps.supported_qpts & (1 << IBV_QPT_RAW_PACKET)));
    if (priv->tso)
      priv->tso_max_payload_sz = device_attr_ex.tso_caps.max_tso;
    DEBUG("TSO is %ssupported", priv->tso ? "" : "not ");
    ...
    INFO("port %u MAC address is " RTE_ETHER_ADDR_PRT_FMT, priv->port,
     ...
     */
    err = mlx4_mr_btree_init(&priv->mr.cache, MLX4_MR_BTREE_CACHE_N * 2,
    ...
    LIST_INSERT_HEAD(&mlx4_shared_data->mem_event_cb_list, priv, mem_event_cb);
 ...
 */
static int mlx4_pci_remove(struct rte_pci_device *pci_dev) {
  uint16_t port_id;
  int ret = 0;
...
static const struct rte_pci_id mlx4_pci_id_map[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX, PCI_DEVICE_ID_MELLANOX_CONNECTX3)},
 ...
 */
static char *mlx4_glue_path(char *buf, size_t size) {
 ...
 */
static int mlx4_glue_init(void) {
  ...
  const char *path[] = {
       ...
       */
      (geteuid() == getuid() && getegid() == getgid() ? getenv("MLX4_GLUE_PATH")
                                                      : NULL),
      /*
       ...
       * own.
       */
      (*RTE_EAL_PMD_PATH ? mlx4_glue_path(glue_path, sizeof(glue_path)) : ""),
  };
  unsigned int i = 0;
  void *handle = NULL;
  void **sym;
  ...
  while (!handle && i != RTE_DIM(path)) {
    ...
    do {
      ...
      ret = snprintf(name, sizeof(name), "%.*s%s" MLX4_GLUE, (int)len, path[i],
                     (!len || *(end - 1) == '/') ? "" : "/");
      if (ret == -1)
        break;
