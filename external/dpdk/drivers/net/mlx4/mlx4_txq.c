 ...
 */
static void txq_uar_init(struct txq *txq) {
 ...
 */
static int txq_uar_init_secondary(struct txq *txq, int fd) {
  struct mlx4_priv *priv = txq->priv;
  struct mlx4_proc_priv *ppriv = MLX4_PROC_PRIV(PORT_ID(priv));
  void *addr;
  uintptr_t uar_va;
  ...
  if (addr == MAP_FAILED) {
    ERROR("port %u mmap failed for BF reg of txq %u", txq->port_id,
          txq->stats.idx);
    rte_errno = ENXIO;
    return -rte_errno;
  }
 ...
 *   Pointer to Tx queue structure.
 */
static void txq_uar_uninit_secondary(struct txq *txq) {
...
void mlx4_tx_uar_uninit_secondary(struct rte_eth_dev *dev) {
  struct mlx4_proc_priv *ppriv = (struct mlx4_proc_priv *)dev->process_private;
  const size_t page_size = sysconf(_SC_PAGESIZE);
  void *addr;
  size_t i;
...
#else
int mlx4_tx_uar_init_secondary(struct rte_eth_dev *dev __rte_unused,
                               int fd __rte_unused) {
  MLX4_ASSERT(rte_eal_process_type() == RTE_PROC_SECONDARY);
...
void mlx4_tx_uar_uninit_secondary(struct rte_eth_dev *dev __rte_unused) {
 ...
 *   Pointer to Tx queue structure.
 */
static void mlx4_txq_free_elts(struct txq *txq) {
 ...
 */
static void mlx4_txq_fill_dv_obj_info(struct txq *txq,
  ...
  sq->stamp = rte_cpu_to_be_32(MLX4_SQ_STAMP_VAL | (0u << MLX4_SQ_OWNER_BIT));
#ifdef HAVE_IBV_MLX4_UAR_MMAP_OFFSET
 ...
 */
int mlx4_tx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc,
                        unsigned int socket,
  ...
  if (txq) {
    rte_errno = EEXIST;
    DEBUG("%p: Tx queue %u already configured, release it first", (void *)dev,
          idx);
    return -rte_errno;
  }
  if (!desc) {
  ...
  *txq = (struct txq){
       ...
       */
      .elts_comp_cd = RTE_MIN(MLX4_PMD_TX_PER_COMP_REQ, desc / 4),
      .elts_comp_cd_init = RTE_MIN(MLX4_PMD_TX_PER_COMP_REQ, desc / 4),
      .csum = priv->hw_csum && (offloads & (RTE_ETH_TX_OFFLOAD_IPV4_CKSUM |
  ...
  if (!txq->cq) {
    rte_errno = ENOMEM;
    ERROR("%p: CQ creation failure: %s", (void *)dev, strerror(rte_errno));
    goto error;
  }
  qp_init_attr = (struct ibv_qp_init_attr){
      ...
      .cap =
          {
              .max_send_wr = RTE_MIN(priv->device_attr.max_qp_wr, desc),
              .max_send_sge = 1,
  ...
  if (!txq->qp) {
    ...
    ERROR("%p: QP creation failure: %s", (void *)dev, strerror(rte_errno));
    goto error;
  }
  txq->max_inline = qp_init_attr.cap.max_inline_data;
  ret = mlx4_glue->modify_qp(txq->qp,
                             &(struct ibv_qp_attr){
  ...
  if (ret) {
    rte_errno = ret;
    ERROR("%p: QP state to IBV_QPS_INIT failed: %s", (void *)dev,
          strerror(rte_errno));
    goto error;
  }
  ret = mlx4_glue->modify_qp(txq->qp,
  ...
  if (ret) {
    rte_errno = ret;
    ERROR("%p: QP state to IBV_QPS_RTR failed: %s", (void *)dev,
          strerror(rte_errno));
    goto error;
  }
  ret = mlx4_glue->modify_qp(txq->qp,
  ...
  if (ret) {
    rte_errno = ret;
    ERROR("%p: QP state to IBV_QPS_RTS failed: %s", (void *)dev,
          strerror(rte_errno));
    goto error;
  }
  ...
  (&(*txq->elts)[0])->wqe = (volatile struct mlx4_wqe_ctrl_seg *)txq->msq.buf;
  if (mlx4_mr_btree_init(&txq->mr_ctrl.cache_bh, MLX4_MR_BTREE_CACHE_N,
                         socket)) {
