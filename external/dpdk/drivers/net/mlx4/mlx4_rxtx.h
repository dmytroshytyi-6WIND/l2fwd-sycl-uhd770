...
uint16_t mlx4_tx_burst(void *dpdk_txq, struct rte_mbuf **pkts, uint16_t pkts_n);
uint16_t mlx4_rx_burst(void *dpdk_rxq, struct rte_mbuf **pkts, uint16_t pkts_n);
 ...
 */
static inline struct rte_mempool *mlx4_mb2mp(struct rte_mbuf *buf) {
 ...
 */
static __rte_always_inline uint32_t mlx4_rx_addr2mr(struct rxq *rxq,
                                                    uintptr_t addr) {
  struct mlx4_mr_ctrl *mr_ctrl = &rxq->mr_ctrl;
  uint32_t lkey;

  /* Linear search on MR cache array. */
  lkey = mlx4_mr_lookup_cache(mr_ctrl->cache, &mr_ctrl->mru, MLX4_MR_CACHE_N,
                              addr);
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static __rte_always_inline uint32_t mlx4_tx_mb2mr(struct txq *txq,
                                                  struct rte_mbuf *mb) {
