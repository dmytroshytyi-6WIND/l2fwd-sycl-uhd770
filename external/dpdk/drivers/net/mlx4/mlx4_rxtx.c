...
uint32_t mlx4_ptype_table[0x100] __rte_cache_aligned = {
    ...
    [0x08] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_NONFRAG,
    [0x09] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_NONFRAG,
    [0x0a] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_FRAG,
    [0x0b] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_FRAG,
    /* TCP */
    [0x11] =
    ...
    [0x16] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN | RTE_PTYPE_L4_FRAG,
    [0x18] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_TCP,
    [0x19] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_TCP,
    /* UDP */
    [0x21] =
    ...
    [0x26] =
        RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN | RTE_PTYPE_L4_FRAG,
    [0x28] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_UDP,
    [0x29] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_UDP,
    /* Tunneled - L3 IPV6 */
 ...
 */
static uint32_t mlx4_txq_stamp_freed_wqe(struct mlx4_sq *sq,
 ...
 *   Pointer to the SQ structure.
 */
static void mlx4_txq_complete(struct txq *txq, const unsigned int elts_m,
                              struct mlx4_sq *sq) {
  ...
  sq->remain_size += mlx4_txq_stamp_freed_wqe(
      sq, first_txbb, (&(*txq->elts)[elts_tail & elts_m])->eocb);
 ...
 */
static inline void
mlx4_fill_tx_data_seg(volatile struct mlx4_wqe_data_seg *dseg, uint32_t lkey,
                      uintptr_t addr, rte_be32_t byte_count) {
 ...
 */
static inline int mlx4_tx_burst_tso_get_params(struct rte_mbuf *buf,
                                               struct txq *txq,
                                               struct tso_info *tinfo) {
  struct mlx4_sq *sq = &txq->msq;
  const uint8_t tunneled =
      txq->priv->hw_csum_l2tun && (buf->ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK);

  tinfo->tso_header_size = buf->l2_len + buf->l3_len + buf->l4_len;
  if (tunneled)
    tinfo->tso_header_size += buf->outer_l2_len + buf->outer_l3_len;
  if (unlikely(buf->tso_segsz == 0 || tinfo->tso_header_size == 0 ||
   ...
   */
  tinfo->wqe_tso_seg_size =
      RTE_ALIGN(sizeof(struct mlx4_wqe_lso_seg) + tinfo->tso_header_size,
                sizeof(struct mlx4_wqe_data_seg));
  tinfo->fence_size =
      ((sizeof(struct mlx4_wqe_ctrl_seg) + tinfo->wqe_tso_seg_size) >>
       MLX4_SEG_SHIFT) +
      buf->nb_segs;
  tinfo->wqe_size = RTE_ALIGN((uint32_t)(tinfo->fence_size << MLX4_SEG_SHIFT),
                              MLX4_TXBB_SIZE);
  /* Validate WQE size and WQE space in the send queue. */
  if (sq->remain_size < tinfo->wqe_size || tinfo->wqe_size > MLX4_MAX_WQE_SIZE)
    return -ENOMEM;
  /* Init pv. */
...
mlx4_tx_burst_fill_tso_dsegs(struct rte_mbuf *buf, struct txq *txq,
                             struct tso_info *tinfo,
                             volatile struct mlx4_wqe_data_seg *dseg,
  ...
  volatile struct mlx4_wqe_ctrl_seg *ctrl_next =
      (volatile struct mlx4_wqe_ctrl_seg *)((volatile uint8_t *)ctrl +
                                            tinfo->wqe_size);
  uint16_t data_len = sbuf->data_len - tinfo->tso_header_size;
  uintptr_t data_addr =
      rte_pktmbuf_mtod_offset(sbuf, uintptr_t, tinfo->tso_header_size);

  do {
    /* how many dseg entries do we have in the current TXBB ? */
    nb_segs_txbb =
        (MLX4_TXBB_SIZE - ((uintptr_t)dseg & (MLX4_TXBB_SIZE - 1))) >>
        MLX4_SEG_SHIFT;
    switch (nb_segs_txbb) {
#ifdef RTE_LIBRTE_MLX4_DEBUG
    default:
      /* Should never happen. */
      rte_panic("%p: Invalid number of SGEs(%d) for a TXBB", (void *)txq,
                nb_segs_txbb);
    ...
    case 4:
       ...
       */
      pv[(*pv_counter)++].val =
          rte_cpu_to_be_32(data_len ? data_len : 0x80000000);
      if (--nb_segs == 0)
        return ctrl_next;
    ...
    if ((volatile uint8_t *)dseg >= sq->eob)
      dseg = (volatile struct mlx4_wqe_data_seg *)((volatile uint8_t *)dseg -
                                                   sq->size);
  } while (true);
err:
  return NULL;
...
mlx4_tx_burst_fill_tso_hdr(struct rte_mbuf *buf, struct txq *txq,
                           struct tso_info *tinfo,
                           volatile struct mlx4_wqe_ctrl_seg *ctrl) {
  ...
  union {
    ...
    uint8_t *to;
      ...
      .vto = (volatile uint8_t *)tseg->header,
  };
   ...
   */
  txbb_avail_space = MLX4_TXBB_SIZE - (sizeof(struct mlx4_wqe_ctrl_seg) +
  ...
  while (remain_size >= (int)(txbb_avail_space + sizeof(uint32_t))) {
    ...
    pv[(*pv_counter)++].val = *(uint32_t *)from, from += sizeof(uint32_t);
  ...
  tseg->mss_hdr_size =
      rte_cpu_to_be_32((buf->tso_segsz << 16) | tinfo->tso_header_size);
  /* Calculate data segment location */
  return (volatile struct mlx4_wqe_data_seg *)((uintptr_t)tseg +
                                               tinfo->wqe_tso_seg_size);
}
...
mlx4_tx_burst_tso(struct rte_mbuf *buf, struct txq *txq,
  ...
  if ((uintptr_t)dseg >= (uintptr_t)sq->eob)
    dseg = (volatile struct mlx4_wqe_data_seg *)((uintptr_t)dseg - sq->size);
...
mlx4_tx_burst_segs(struct rte_mbuf *buf, struct txq *txq,
  ...
  wqe_size =
      RTE_ALIGN((uint32_t)(ctrl->fence_size << MLX4_SEG_SHIFT), MLX4_TXBB_SIZE);
  /* Validate WQE size and WQE space in the send queue. */
  if (sq->remain_size < wqe_size || wqe_size > MLX4_MAX_WQE_SIZE)
    return NULL;
  /*
...
txbb_head_seg:
   ...
   */
  pv[pv_counter++].val =
      rte_cpu_to_be_32(sbuf->data_len ? sbuf->data_len : 0x80000000);
  sbuf = sbuf->next;
  dseg++;
  nb_segs--;
txbb_tail_segs:
  ...
  default:
    ...
    mlx4_fill_tx_data_seg(
        dseg, lkey, rte_pktmbuf_mtod(sbuf, uintptr_t),
        rte_cpu_to_be_32(sbuf->data_len ? sbuf->data_len : 0x80000000));
    sbuf = sbuf->next;
    dseg++;
    nb_segs--;
  ...
  return (volatile struct mlx4_wqe_ctrl_seg *)((volatile uint8_t *)ctrl +
                                               wqe_size);
}
...
uint16_t mlx4_tx_burst(void *dpdk_txq, struct rte_mbuf **pkts,
  ...
  for (i = 0; (i != max); ++i) {
    ...
    if (tso) {
      ...
      mlx4_fill_tx_data_seg(dseg++, lkey, rte_pktmbuf_mtod(buf, uintptr_t),
    ...
    if ((volatile uint8_t *)ctrl_next >= sq->eob) {
      ctrl_next =
          (volatile struct mlx4_wqe_ctrl_seg *)((volatile uint8_t *)ctrl_next -
                                                sq->size);
    ...
    if (--txq->elts_comp_cd == 0) {
      ...
      srcrb.flags = RTE_BE32(MLX4_WQE_CTRL_SOLICIT | MLX4_WQE_CTRL_CQ_UPDATE);
    } else {
      srcrb.flags = RTE_BE32(MLX4_WQE_CTRL_SOLICIT);
    }
    /* Enable HW checksum offload if requested */
    if (txq->csum &&
        (buf->ol_flags & (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_TCP_CKSUM |
      ...
      if (is_tunneled && txq->csum_l2tun) {
        owner_opcode |= MLX4_WQE_CTRL_IIP_HDR_CSUM | MLX4_WQE_CTRL_IL4_HDR_CSUM;
        if (buf->ol_flags & RTE_MBUF_F_TX_OUTER_IP_CKSUM)
          srcrb.flags |= RTE_BE32(MLX4_WQE_CTRL_IP_HDR_CSUM);
      } else {
        srcrb.flags |=
            RTE_BE32(MLX4_WQE_CTRL_IP_HDR_CSUM | MLX4_WQE_CTRL_TCP_UDP_CSUM);
      }
    }
    if (txq->lb) {
      /*
      ...
      ctrl->imm = *(rte_pktmbuf_mtod_offset(buf, uint32_t *, sizeof(uint16_t)));
    } else {
      ctrl->imm = 0;
    }
 ...
 */
static inline uint32_t rxq_cq_to_pkt_type(volatile struct mlx4_cqe *cqe,
 ...
 */
static inline uint32_t rxq_cq_to_ol_flags(uint32_t flags, int csum,
                                          int csum_l2tun) {
  uint32_t ol_flags = 0;

  if (csum)
    ol_flags |= mlx4_transpose(flags, MLX4_CQE_STATUS_IP_HDR_CSUM_OK,
                               RTE_MBUF_F_RX_IP_CKSUM_GOOD) |
                mlx4_transpose(flags, MLX4_CQE_STATUS_TCP_UDP_CSUM_OK,
                               RTE_MBUF_F_RX_L4_CKSUM_GOOD);
  if ((flags & MLX4_CQE_L2_TUNNEL) && csum_l2tun)
    ol_flags |= mlx4_transpose(flags, MLX4_CQE_L2_TUNNEL_IPOK,
                               RTE_MBUF_F_RX_IP_CKSUM_GOOD) |
                mlx4_transpose(flags, MLX4_CQE_L2_TUNNEL_L4_CSUM,
                               RTE_MBUF_F_RX_L4_CKSUM_GOOD);
  return ol_flags;
}
 ...
 */
static inline uint32_t mlx4_cqe_flags(volatile struct mlx4_cqe *cqe, int csum,
                                      int csum_l2tun) {
  uint32_t flags = 0;
   ...
   */
  if (csum)
    flags = (rte_be_to_cpu_32(cqe->status) & MLX4_CQE_STATUS_IPV4_CSUM_OK);
  if (csum_l2tun)
    flags |= (rte_be_to_cpu_32(cqe->vlan_my_qpn) &
              (MLX4_CQE_L2_TUNNEL | MLX4_CQE_L2_TUNNEL_IPOK |
               MLX4_CQE_L2_TUNNEL_L4_CSUM | MLX4_CQE_L2_TUNNEL_IPV4));
  return flags;
}
 ...
 */
static unsigned int mlx4_cq_poll_one(struct rxq *rxq,
                                     volatile struct mlx4_cqe **out) {
  int ret = 0;
...
uint16_t mlx4_rx_burst(void *dpdk_rxq, struct rte_mbuf **pkts,
  ...
  while (pkts_n) {
    ...
    if (!pkt) {
      ...
      pkt->packet_type = rxq_cq_to_pkt_type(cqe, rxq->l2tun_offload);
      ...
      if (rxq->csum | rxq->csum_l2tun) {
        uint32_t flags = mlx4_cqe_flags(cqe, rxq->csum, rxq->csum_l2tun);

        pkt->ol_flags = rxq_cq_to_ol_flags(flags, rxq->csum, rxq->csum_l2tun);
      }
    }
    rep->nb_segs = 1;
