...
static int mlx4_glue_fork_init(void) { return ibv_fork_init(); }

static int mlx4_glue_get_async_event(struct ibv_context *context,
                                     struct ibv_async_event *event) {
  return ibv_get_async_event(context, event);
}

static void mlx4_glue_ack_async_event(struct ibv_async_event *event) {
  ibv_ack_async_event(event);
}

static struct ibv_pd *mlx4_glue_alloc_pd(struct ibv_context *context) {
  return ibv_alloc_pd(context);
}

static int mlx4_glue_dealloc_pd(struct ibv_pd *pd) {
  return ibv_dealloc_pd(pd);
}

static struct ibv_device **mlx4_glue_get_device_list(int *num_devices) {
  return ibv_get_device_list(num_devices);
}

static void mlx4_glue_free_device_list(struct ibv_device **list) {
  ibv_free_device_list(list);
}

static struct ibv_context *mlx4_glue_open_device(struct ibv_device *device) {
  return ibv_open_device(device);
}

static int mlx4_glue_close_device(struct ibv_context *context) {
  return ibv_close_device(context);
}

static const char *mlx4_glue_get_device_name(struct ibv_device *device) {
  return ibv_get_device_name(device);
}

static int mlx4_glue_query_device(struct ibv_context *context,
...
static int mlx4_glue_query_port(struct ibv_context *context, uint8_t port_num,
                                struct ibv_port_attr *port_attr) {
  return ibv_query_port(context, port_num, port_attr);
}

static const char *mlx4_glue_port_state_str(enum ibv_port_state port_state) {
...
static int mlx4_glue_destroy_comp_channel(struct ibv_comp_channel *channel) {
  return ibv_destroy_comp_channel(channel);
}

static struct ibv_cq *mlx4_glue_create_cq(struct ibv_context *context, int cqe,
                                          void *cq_context,
...
static int mlx4_glue_destroy_cq(struct ibv_cq *cq) {
  return ibv_destroy_cq(cq);
}

static int mlx4_glue_get_cq_event(struct ibv_comp_channel *channel,
                                  struct ibv_cq **cq, void **cq_context) {
  return ibv_get_cq_event(channel, cq, cq_context);
}

static void mlx4_glue_ack_cq_events(struct ibv_cq *cq, unsigned int nevents) {
  ibv_ack_cq_events(cq, nevents);
}

static struct ibv_flow *mlx4_glue_create_flow(struct ibv_qp *qp,
                                              struct ibv_flow_attr *flow) {
  return ibv_create_flow(qp, flow);
}

static int mlx4_glue_destroy_flow(struct ibv_flow *flow_id) {
...
static int mlx4_glue_destroy_qp(struct ibv_qp *qp) {
  return ibv_destroy_qp(qp);
}

static int mlx4_glue_modify_qp(struct ibv_qp *qp, struct ibv_qp_attr *attr,
                               int attr_mask) {
  return ibv_modify_qp(qp, attr, attr_mask);
}

static struct ibv_mr *mlx4_glue_reg_mr(struct ibv_pd *pd, void *addr,
                                       size_t length, int access) {
  return ibv_reg_mr(pd, addr, length, access);
}

static int mlx4_glue_dereg_mr(struct ibv_mr *mr) { return ibv_dereg_mr(mr); }
...
static int mlx4_glue_destroy_wq(struct ibv_wq *wq) {
  return ibv_destroy_wq(wq);
}
static int mlx4_glue_modify_wq(struct ibv_wq *wq, struct ibv_wq_attr *wq_attr) {
  return ibv_modify_wq(wq, wq_attr);
}

static int mlx4_glue_dv_init_obj(struct mlx4dv_obj *obj, uint64_t obj_type) {
