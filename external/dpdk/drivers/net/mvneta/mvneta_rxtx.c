...
#define MVNETA_SET_COOKIE_HIGH_ADDR(addr)                                      \
  {                                                                            \
    if (unlikely(cookie_addr_high == MVNETA_COOKIE_ADDR_INVALID))              \
      cookie_addr_high = (uint64_t)(addr)&MVNETA_COOKIE_HIGH_ADDR_MASK;        \
  }
...
static inline int mvneta_buffs_refill(struct mvneta_priv *priv,
                                      struct mvneta_rxq *rxq, u16 *num) {
  ...
  for (i = 0; i < nb_desc; i++) {
    if (unlikely(!MVNETA_CHECK_COOKIE_HIGH_ADDR(mbufs[i]))) {
      MVNETA_LOG(ERR, "mbuf virt high addr 0x%lx out of range 0x%lx",
                 (uint64_t)mbufs[i] >> 32, cookie_addr_high >> 32);
      *num = 0;
      goto out;
    }
 ...
 */
static inline int mvneta_buffs_alloc(struct mvneta_priv *priv,
                                     struct mvneta_rxq *rxq, int *num) {
  ...
  do {
    nb_desc_burst = (nb_desc < MRVL_NETA_BUF_RELEASE_BURST_SIZE_MAX)
                        ? nb_desc
...
static inline void mvneta_fill_shadowq(struct mvneta_shadow_txq *sq,
                                       struct rte_mbuf *buf) {
  sq->ent[sq->head].cookie = (uint64_t)buf;
  sq->ent[sq->head].addr = buf ? rte_mbuf_data_iova_default(buf) : 0;

  sq->head = (sq->head + 1) & MRVL_NETA_TX_SHADOWQ_MASK;
  sq->size++;
}

static inline void mvneta_fill_desc(struct neta_ppio_desc *desc,
                                    struct rte_mbuf *buf) {
 ...
 */
static inline void mvneta_sent_buffers_free(struct neta_ppio *ppio,
  ...
  if (nb_done > sq->size) {
    MVNETA_LOG(ERR, "nb_done: %d, sq->size %d", nb_done, sq->size);
    return;
  }
  ...
  for (i = 0; i < nb_done; i++) {
    ...
    if (unlikely(!entry->addr)) {
      MVNETA_LOG(DEBUG, "Shadow memory @%d: cookie(%lx), pa(%lx)!", tail,
                 (u64)entry->cookie, (u64)entry->addr);
    ...
    mbuf = (struct rte_mbuf *)(cookie_addr_high | entry->cookie);
 ...
 */
static inline void mvneta_prepare_proto_info(uint64_t ol_flags,
 ...
 *   Mbuf offload flags.
 */
static inline uint64_t mvneta_desc_to_ol_flags(struct neta_ppio_desc *desc) {
  uint64_t flags;
 ...
 */
static uint16_t mvneta_tx_pkt_burst(void *txq, struct rte_mbuf **tx_pkts,
                                    uint16_t nb_pkts) {
  ...
  if (sq->size)
    mvneta_sent_buffers_free(q->priv->ppio, sq, q->queue_id);
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (!(mbuf->ol_flags & MVNETA_TX_PKT_OFFLOADS))
      continue;
    mvneta_prepare_proto_info(mbuf->ol_flags, &l3_type, &l4_type, &gen_l3_cksum,
                              &gen_l4_cksum);

    neta_ppio_outq_desc_set_proto_info(
        &descs[i], l3_type, l4_type, mbuf->l2_len, mbuf->l2_len + mbuf->l3_len,
 ...
 *   Number of packets successfully transmitted.
 */
static uint16_t mvneta_tx_sg_pkt_burst(void *txq, struct rte_mbuf **tx_pkts,
                                       uint16_t nb_pkts) {
  struct mvneta_txq *q = txq;
  struct mvneta_shadow_txq *sq;
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (unlikely(nb_segs > NETA_PPIO_DESC_NUM_FRAGS)) {
      total_descs -= nb_segs;
      MVNETA_LOG(ERR, "Too many segments. Packet won't be sent.");
      break;
    }
    ...
    neta_ppio_outq_desc_set_proto_info(
        &descs[tail_first], l3_type, l4_type, mbuf->l2_len,
        mbuf->l2_len + mbuf->l3_len, gen_l3_cksum, gen_l4_cksum);
  }
  num = total_descs;
  neta_ppio_send_sg(q->priv->ppio, q->queue_id, descs, &total_descs, &pkts);
 ...
 */
uint16_t mvneta_rx_pkt_burst(void *rxq, struct rte_mbuf **rx_pkts,
                             uint16_t nb_pkts) {
  ...
  ret = neta_ppio_recv(q->priv->ppio, q->queue_id, descs, &nb_pkts);
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    addr = cookie_addr_high | neta_ppio_inq_desc_get_cookie(&descs[i]);
    ...
    mbuf->port = q->port_id;
    mbuf->packet_type = mvneta_desc_to_packet_type_and_offset(
        &descs[i], &l3_offset, &l4_offset);
 ...
 *   0 on success, negative error value otherwise.
 */
int mvneta_rx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc,
                          unsigned int socket,
  ...
  if (frame_size < max_rx_pktlen) {
    MVNETA_LOG(ERR,
               ...
               "to %u bytes of data.",
               max_rx_pktlen + buf_size - frame_size, max_rx_pktlen);
  ...
  priv->ppio_params.inqs_params.tcs_params[MRVL_NETA_DEFAULT_TC].size = desc;
 ...
 *   0 on success, negative error value otherwise.
 */
int mvneta_tx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc,
                          unsigned int socket,
 ...
 */
static void mvneta_recv_buffs_free(struct neta_ppio_desc *desc, uint16_t num) {
  uint64_t addr;
  uint8_t i;

  for (i = 0; i < num; i++) {
    if (desc) {
      addr = cookie_addr_high | neta_ppio_inq_desc_get_cookie(desc);
      if (addr)
 ...
 *   Array of rx descriptors
 */
static void mvneta_rx_queue_flush(struct mvneta_rxq *rxq) {
  ...
  do {
    ...
    ret = neta_ppio_recv(rxq->priv->ppio, rxq->queue_id, descs, &num);
 ...
 */
static void mvneta_tx_queue_flush(struct mvneta_txq *txq) {
  struct mvneta_shadow_txq *sq = &txq->shadow_txq;

  if (sq->size)
    mvneta_sent_buffers_free(txq->priv->ppio, sq, txq->queue_id);

  /* free the rest of them */
  while (sq->tail != sq->head) {
    uint64_t addr = cookie_addr_high | sq->ent[sq->tail].cookie;
    rte_pktmbuf_free((struct rte_mbuf *)addr);
