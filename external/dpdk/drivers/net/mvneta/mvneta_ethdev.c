...
static const char *const valid_args[] = {MVNETA_IFACE_NAME_ARG, NULL};
 ...
 */
static void mvneta_neta_deinit(void) { neta_deinit(); }
 ...
 */
static int mvneta_neta_init(void) { return neta_init(); }
 ...
 *   Always 0.
 */
static int mvneta_ifnames_get(const char *key __rte_unused, const char *value,
                              void *extra_args) {
 ...
 *   0 on success, negative error value otherwise.
 */
static int mvneta_dev_configure(struct rte_eth_dev *dev) {
...
static int mvneta_dev_infos_get(struct rte_eth_dev *dev __rte_unused,
  ...
  info->speed_capa = RTE_ETH_LINK_SPEED_10M | RTE_ETH_LINK_SPEED_100M |
...
mvneta_dev_supported_ptypes_get(struct rte_eth_dev *dev __rte_unused) {
  static const uint32_t ptypes[] = {RTE_PTYPE_L2_ETHER, RTE_PTYPE_L2_ETHER_VLAN,
 ...
 *   0 on success, negative error value otherwise.
 */
static int mvneta_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct mvneta_priv *priv = dev->data->dev_private;
   ...
   */
  if (!dev->data->scattered_rx && (mru + MRVL_NETA_PKT_OFFS > mbuf_data_size)) {
    mru = mbuf_data_size - MRVL_NETA_PKT_OFFS;
    mtu = MRVL_NETA_MRU_TO_MTU(mru);
    MVNETA_LOG(WARNING,
               "MTU too big, max MTU possible limited by"
 ...
 *   0 on success, negative error value otherwise.
 */
static int mvneta_dev_set_link_up(struct rte_eth_dev *dev) {
  struct mvneta_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mvneta_dev_set_link_down(struct rte_eth_dev *dev) {
  struct mvneta_priv *priv = dev->data->dev_private;
 ...
 */
static int mvneta_dev_start(struct rte_eth_dev *dev) {
  struct mvneta_priv *priv = dev->data->dev_private;
  ...
  if (!priv->uc_mc_flushed) {
    ...
    if (ret) {
      MVNETA_LOG(ERR, "Failed to flush uc/mc filter list");
      goto out;
    }
 ...
 *   Pointer to Ethernet device structure.
 */
static int mvneta_dev_stop(struct rte_eth_dev *dev) {
  struct mvneta_priv *priv = dev->data->dev_private;
 ...
 *   Pointer to Ethernet device structure.
 */
static int mvneta_dev_close(struct rte_eth_dev *dev) {
  struct mvneta_priv *priv = dev->data->dev_private;
  int i, ret = 0;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mvneta_link_update(struct rte_eth_dev *dev,
  ...
  dev->data->dev_link.link_duplex =
      edata.duplex ? RTE_ETH_LINK_FULL_DUPLEX : RTE_ETH_LINK_HALF_DUPLEX;
  dev->data->dev_link.link_autoneg =
      edata.autoneg ? RTE_ETH_LINK_AUTONEG : RTE_ETH_LINK_FIXED;

  neta_ppio_get_link_state(priv->ppio, &link_up);
  dev->data->dev_link.link_status =
      link_up ? RTE_ETH_LINK_UP : RTE_ETH_LINK_DOWN;
 ...
 *   always 0
 */
static int mvneta_promiscuous_enable(struct rte_eth_dev *dev) {
  struct mvneta_priv *priv = dev->data->dev_private;
  int ret, en;
 ...
 *   always 0
 */
static int mvneta_promiscuous_disable(struct rte_eth_dev *dev) {
  struct mvneta_priv *priv = dev->data->dev_private;
  int ret, en;
 ...
 *   MAC address index.
 */
static void mvneta_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
  struct mvneta_priv *priv = dev->data->dev_private;
  ...
  if (ret) {
    rte_ether_format_addr(buf, sizeof(buf), &dev->data->mac_addrs[index]);
 ...
 *   0 on success, negative error value otherwise.
 */
static int mvneta_mac_addr_add(struct rte_eth_dev *dev,
 ...
 *   MAC address to register.
 */
static int mvneta_mac_addr_set(struct rte_eth_dev *dev,
 ...
 *   0 on success, negative error value otherwise.
 */
static int mvneta_stats_get(struct rte_eth_dev *dev,
  ...
  stats->ipackets += ppio_stats.rx_packets + ppio_stats.rx_broadcast_packets +
                     ppio_stats.rx_multicast_packets -
                     priv->prev_stats.ipackets;
  stats->opackets += ppio_stats.tx_packets + ppio_stats.tx_broadcast_packets +
  ...
  stats->imissed +=
      ppio_stats.rx_discard + ppio_stats.rx_overrun - priv->prev_stats.imissed;
  stats->ierrors = ppio_stats.rx_packets_err - priv->prev_stats.ierrors;
 ...
 *   0 on success, negative error value otherwise.
 */
static int mvneta_eth_dev_create(struct rte_vdev_device *vdev,
                                 const char *name) {
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("mac_addrs", RTE_ETHER_ADDR_LEN * MVNETA_MAC_ADDRS_MAX, 0);
  ...
  memcpy(eth_dev->data->mac_addrs[0].addr_bytes, req.ifr_addr.sa_data,
         RTE_ETHER_ADDR_LEN);
 ...
 */
static void mvneta_eth_dev_destroy(struct rte_eth_dev *eth_dev) {
  rte_eth_dev_release_port(eth_dev);
}
 ...
 */
static void mvneta_eth_dev_destroy_name(const char *name) {
  struct rte_eth_dev *eth_dev;
 ...
 *   0 on success, negative error value otherwise.
 */
static int rte_pmd_mvneta_probe(struct rte_vdev_device *vdev) {
  ...
  ifnames.idx = 0;
  rte_kvargs_process(kvlist, MVNETA_IFACE_NAME_ARG, mvneta_ifnames_get,
                     &ifnames);
