...
static inline uint32_t __rte_hot dpaa2_dev_rx_parse_slow(
    struct rte_mbuf *mbuf, struct dpaa2_annot_hdr *annotation);

static void enable_tx_tstamp(struct qbman_fd *fd) __rte_unused;

static inline rte_mbuf_timestamp_t *
dpaa2_timestamp_dynfield(struct rte_mbuf *mbuf) {
  return RTE_MBUF_DYNFIELD(mbuf, dpaa2_timestamp_dynfield_offset,
                           rte_mbuf_timestamp_t *);
}

#define DPAA2_MBUF_TO_CONTIG_FD(_mbuf, _fd, _bpid)                             \
  do {                                                                         \
...
static inline void __rte_hot dpaa2_dev_rx_parse_new(struct rte_mbuf *m,
                                                    const struct qbman_fd *fd,
                                                    void *hw_annot_addr) {
  uint16_t frc = DPAA2_GET_FD_FRC_PARSE_SUM(fd);
  struct dpaa2_annot_hdr *annotation = (struct dpaa2_annot_hdr *)hw_annot_addr;
  ...
  case DPAA2_PKT_TYPE_IPV4:
    m->packet_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4;
    break;
  case DPAA2_PKT_TYPE_IPV6:
    m->packet_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6;
    break;
  case DPAA2_PKT_TYPE_IPV4_EXT:
    m->packet_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT;
    break;
  case DPAA2_PKT_TYPE_IPV6_EXT:
    m->packet_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT;
    break;
  ...
  case DPAA2_PKT_TYPE_IPV6_UDP:
    m->packet_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_UDP;
    break;
  ...
  case DPAA2_PKT_TYPE_IPV6_SCTP:
    m->packet_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_SCTP;
    break;
  case DPAA2_PKT_TYPE_IPV4_ICMP:
    m->packet_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_ICMP;
    break;
  case DPAA2_PKT_TYPE_IPV6_ICMP:
    m->packet_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_ICMP;
    break;
  default:
...
static inline uint32_t __rte_hot dpaa2_dev_rx_parse_slow(
  ...
  if (BIT_ISSET_AT_POS(annotation->word3, L2_VLAN_1_PRESENT)) {
    vlan_tci = rte_pktmbuf_mtod_offset(
        mbuf, uint16_t *, (VLAN_TCI_OFFSET_1(annotation->word5) >> 16));
  ...
  if (BIT_ISSET_AT_POS(annotation->word3,
                       L2_MPLS_1_PRESENT | L2_MPLS_N_PRESENT))
    pkt_type |= RTE_PTYPE_L2_ETHER_MPLS;

  if (BIT_ISSET_AT_POS(annotation->word4,
                       L3_IPV4_1_PRESENT | L3_IPV4_N_PRESENT)) {
    pkt_type |= RTE_PTYPE_L3_IPV4;
    if (BIT_ISSET_AT_POS(annotation->word4,
                         L3_IP_1_OPT_PRESENT | L3_IP_N_OPT_PRESENT))
      pkt_type |= RTE_PTYPE_L3_IPV4_EXT;

  } else if (BIT_ISSET_AT_POS(annotation->word4,
                              L3_IPV6_1_PRESENT | L3_IPV6_N_PRESENT)) {
  ...
  if (BIT_ISSET_AT_POS(annotation->word4,
                       L3_IP_1_FIRST_FRAGMENT | L3_IP_1_MORE_FRAGMENT |
                           L3_IP_N_FIRST_FRAGMENT | L3_IP_N_MORE_FRAGMENT)) {
...
static inline uint32_t __rte_hot dpaa2_dev_rx_parse(struct rte_mbuf *mbuf,
                                                    void *hw_annot_addr) {
  struct dpaa2_annot_hdr *annotation = (struct dpaa2_annot_hdr *)hw_annot_addr;
  ...
  case DPAA2_L3_IPv4_TCP:
    return RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP;
  case DPAA2_L3_IPv4_UDP:
    return RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP;
  case DPAA2_L3_IPv6_TCP:
    return RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP;
  case DPAA2_L3_IPv6_UDP:
    return RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_UDP;
  default:
    break;
  }
...
eth_sg_fd_to_mbuf(const struct qbman_fd *fd, int port_id) {
  ...
  first_seg = DPAA2_INLINE_MBUF_FROM_BUF(
      sg_addr, rte_dpaa2_bpid_info[DPAA2_GET_FD_BPID(fd)].meta_data_size);
  ...
  if (dpaa2_svr_family == SVR_LX2160A)
    ...
    first_seg->packet_type = dpaa2_dev_rx_parse(first_seg, hw_annot_addr);
  ...
  while (!DPAA2_SG_IS_FINAL(sge)) {
    sge = &sgt[i++];
    sg_addr = (size_t)DPAA2_IOVA_TO_VADDR(DPAA2_GET_FLE_ADDR(sge));
    next_seg = DPAA2_INLINE_MBUF_FROM_BUF(
        sg_addr, rte_dpaa2_bpid_info[DPAA2_GET_FLE_BPID(sge)].meta_data_size);
  ...
  temp = DPAA2_INLINE_MBUF_FROM_BUF(
      fd_addr, rte_dpaa2_bpid_info[DPAA2_GET_FD_BPID(fd)].meta_data_size);
...
eth_fd_to_mbuf(const struct qbman_fd *fd, int port_id) {
  ...
  struct rte_mbuf *mbuf = DPAA2_INLINE_MBUF_FROM_BUF(
      v_addr, rte_dpaa2_bpid_info[DPAA2_GET_FD_BPID(fd)].meta_data_size);
...
static int __rte_noinline __rte_hot eth_mbuf_to_sg_fd(struct rte_mbuf *mbuf,
                                                      struct qbman_fd *fd,
  ...
  if (RTE_MBUF_DIRECT(mbuf) &&
      (mbuf->data_off > (mbuf->nb_segs * sizeof(struct qbman_sge) + offset))) {
    temp = mbuf;
  ...
  sgt =
      (struct qbman_sge *)((size_t)DPAA2_IOVA_TO_VADDR(DPAA2_GET_FD_ADDR(fd)) +
...
static void eth_mbuf_to_fd(struct rte_mbuf *mbuf, struct qbman_fd *fd,
                           uint16_t bpid) __rte_unused;

static void __rte_noinline __rte_hot eth_mbuf_to_fd(struct rte_mbuf *mbuf,
                                                    struct qbman_fd *fd,
                                                    uint16_t bpid) {
...
static inline int __rte_hot eth_copy_mbuf_to_fd(struct rte_mbuf *mbuf,
                                                struct qbman_fd *fd,
                                                uint16_t bpid) {
  struct rte_mbuf *m;
  void *mb = NULL;

  if (rte_dpaa2_mbuf_alloc_bulk(rte_dpaa2_bpid_info[bpid].bp_list->mp, &mb,
                                1)) {
  ...
  memcpy((char *)m->buf_addr + mbuf->data_off,
         (void *)((char *)mbuf->buf_addr + mbuf->data_off), mbuf->pkt_len);
  ...
  DPAA2_PMD_DP_DEBUG("mbuf: %p, BMAN buf addr: %p, fdaddr: %" PRIx64
                     ", bpid: %d,"
                     " meta: %d, off: %d, len: %d\n",
                     (void *)mbuf, mbuf->buf_addr, DPAA2_GET_FD_ADDR(fd),
...
static void dump_err_pkts(struct dpaa2_queue *dpaa2_q) {
  ...
  do {
    ...
    DPAA2_PMD_ERR("\n\n[%d] error packet on port[%d]:"
                  ...
                  rte_lcore_id(), eth_data->port_id, DPAA2_GET_FD_OFFSET(fd),
                  DPAA2_GET_FD_ERR(fd), fas->status);
...
uint16_t dpaa2_dev_prefetch_rx(void *queue, struct rte_mbuf **bufs,
  ...
  do {
    ...
    if (dpaa2_svr_family != SVR_LX2160A) {
      const struct qbman_fd *next_fd = qbman_result_DQ_fd(dq_storage + 1);
      /* Prefetch Annotation address for the parse results */
      rte_prefetch0(DPAA2_IOVA_TO_VADDR(
          (DPAA2_GET_FD_ADDR(next_fd) + DPAA2_FD_PTA_SIZE + 16)));
    }
#endif
...
void __rte_hot dpaa2_dev_process_parallel_event(struct qbman_swp *swp,
                                                const struct qbman_fd *fd,
                                                const struct qbman_result *dq,
                                                struct dpaa2_queue *rxq,
                                                struct rte_event *ev) {
  rte_prefetch0(
      (void *)(size_t)(DPAA2_GET_FD_ADDR(fd) + DPAA2_FD_PTA_SIZE + 16));
...
void __rte_hot dpaa2_dev_process_atomic_event(
    struct qbman_swp *swp __rte_unused, const struct qbman_fd *fd,
...
void __rte_hot dpaa2_dev_process_ordered_event(struct qbman_swp *swp,
                                               const struct qbman_fd *fd,
                                               const struct qbman_result *dq,
                                               struct dpaa2_queue *rxq,
                                               struct rte_event *ev) {
  ...
  *dpaa2_seqn(ev->mbuf) |= qbman_result_DQ_seqnum(dq)
  ...
  do {
    ...
    do {
      ...
      if (dpaa2_svr_family != SVR_LX2160A) {
         ...
         * results.
         */
        rte_prefetch0((DPAA2_IOVA_TO_VADDR(DPAA2_GET_FD_ADDR(next_fd) +
  ...
  do {
    ...
    do {
      ...
      if (mbuf->ol_flags & PKT_TX_IEEE1588_TMST) {
        annotation = (struct dpaa2_annot_hdr *)((size_t)DPAA2_IOVA_TO_VADDR(
...
static void enable_tx_tstamp(struct qbman_fd *fd) {
  ...
  fd_faead = (struct dpaa2_faead *)((size_t)DPAA2_IOVA_TO_VADDR(
                                        DPAA2_GET_FD_ADDR(fd)) +
                                    DPAA2_FD_PTA_SIZE +
                                    DPAA2_FD_HW_ANNOT_FAEAD_OFFSET);
  fd_faead->ctrl =
      DPAA2_ANNOT_FAEAD_A2V | DPAA2_ANNOT_FAEAD_UPDV | DPAA2_ANNOT_FAEAD_UPD;
}
...
uint16_t dpaa2_dev_tx(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts) {
  ...
  while (nb_pkts) {
    ...
    frames_to_send = (nb_pkts > dpaa2_eqcr_size) ? dpaa2_eqcr_size : nb_pkts;
    ...
    for (loop = 0; loop < frames_to_send; loop++) {
      ...
      if (likely(RTE_MBUF_DIRECT(*bufs))) {
         ...
         */
        if (likely(mp && mp->ops_index == priv->bp_list->dpaa2_ops_index &&
                   (*bufs)->nb_segs == 1 &&
          ...
          DPAA2_MBUF_TO_CONTIG_FD((*bufs), &fd_arr[loop], mempool_to_bpid(mp));
          bufs++;
#ifdef RTE_LIBRTE_IEEE1588
      ...
      if (unlikely(RTE_MBUF_HAS_EXTBUF(*bufs))) {
        if (unlikely((*bufs)->nb_segs > 1)) {
          if (eth_mbuf_to_sg_fd(*bufs, &fd_arr[loop], mp, 0))
            goto send_n_return;
        } else {
          eth_mbuf_to_fd(*bufs, &fd_arr[loop], 0);
        }
        bufs++;
#ifdef RTE_LIBRTE_IEEE1588
        enable_tx_tstamp(&fd_arr[loop]);
      ...
      if (mp->ops_index != priv->bp_list->dpaa2_ops_index) {
        ...
        if (unlikely((*bufs)->nb_segs > 1)) {
          if (eth_mbuf_to_sg_fd(*bufs, &fd_arr[loop], mp, bpid))
            goto send_n_return;
        } else {
...
send_n_return:
  ...
  if (loop) {
    ...
    retry_count = 0;
    while (i < loop) {
      ret = qbman_swp_enqueue_multiple(swp, &eqdesc, &fd_arr[i], &flags[i],
                                       loop - i);
      if (unlikely(ret < 0)) {
        retry_count++;
        if (retry_count > DPAA2_MAX_TX_RETRY_COUNT)
...
static void dpaa2_set_enqueue_descriptor(struct dpaa2_queue *dpaa2_q,
                                         struct rte_mbuf *m,
                                         struct qbman_eq_desc *eqdesc) {
  struct rte_eth_dev_data *eth_data = dpaa2_q->eth_data;
  struct dpaa2_dev_priv *priv = eth_data->dev_private;
...
uint16_t dpaa2_dev_tx_multi_txq_ordered(void **queue, struct rte_mbuf **bufs,
                                        uint16_t nb_pkts) {
  ...
  for (loop = 0; loop < nb_pkts; loop++) {
    ...
    if (*dpaa2_seqn(*bufs) && priv->en_ordered) {
      ...
      dpaa2_set_enqueue_descriptor(order_sendq, (*bufs), &eqdesc[loop]);
    } else {
    ...
    if (likely(RTE_MBUF_DIRECT(*bufs))) {
       ...
       */
      if (likely(mp && mp->ops_index == priv->bp_list->dpaa2_ops_index &&
                 (*bufs)->nb_segs == 1 && rte_mbuf_refcnt_read((*bufs)) == 1)) {
        if (unlikely((*bufs)->ol_flags & RTE_MBUF_F_TX_VLAN)) {
          ret = rte_vlan_insert(bufs);
          if (ret)
            goto send_frames;
  ...
  while (nb_pkts) {
    ...
    for (loop = 0; loop < frames_to_send; loop++) {
      ...
      if (*dpaa2_seqn(*bufs)) {
        ...
        qbman_eq_desc_set_no_orp(&eqdesc[loop], DPAA2_EQ_RESP_ERR_FQ);
        qbman_eq_desc_set_fq(&eqdesc[loop], dpaa2_q->fqid);
      }
...
send_n_return:
  ...
  if (loop) {
    ...
    retry_count = 0;
    while (i < loop) {
      ret = qbman_swp_enqueue_multiple_desc(swp, &eqdesc[loop], &fd_arr[i],
                                            loop - i);
      if (unlikely(ret < 0)) {
        retry_count++;
        if (retry_count > DPAA2_MAX_TX_RETRY_COUNT)
...
uint16_t dpaa2_dev_loopback_rx(void *queue, struct rte_mbuf **bufs __rte_unused,
                               uint16_t nb_pkts) {
  /* Function receive frames for a given device and VQ*/
  struct dpaa2_queue *dpaa2_q = (struct dpaa2_queue *)queue;
  struct qbman_result *dq_storage, *dq_storage1 = NULL;
  ...
  if (unlikely(!q_storage->active_dqs)) {
    ...
    while (1) {
      if (qbman_swp_pull(swp, &pulldesc)) {
        DPAA2_PMD_DP_DEBUG("VDQ command not issued.QBMAN busy\n");
        /* Portal was busy, try again */
        continue;
      }
      break;
  ...
  while (num_tx < num_rx) {
    num_tx += qbman_swp_enqueue_multiple_fd(swp, &eqdesc, &fd[num_tx], 0,
                                            num_rx - num_tx);
  }
