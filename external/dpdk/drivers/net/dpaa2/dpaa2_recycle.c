...
static const uint8_t ls_sd1_prot_idx_map[] = {
    0x03, 0x05, 0x07, 0x09, 0x0a, 0x0c, 0x0e, 0x10, 0x12, 0x14, 0x16, 0x18,
    0x1a, 0x1c, 0x1e, 0x20, 0x22, 0x24, 0x26, 0x28, 0x2a, 0x2b, 0x2d, 0x2e,
    0x30, 0x32, 0x33, 0x35, 0x37, 0x39, 0x3b, 0x4b, 0x4c, 0x4d, 0x58};

static const uint8_t ls_sd2_prot_idx_map[] = {
    0x07, 0x09, 0x0a, 0x0c, 0x0e, 0x10, 0x12, 0x14, 0x16, 0x18, 0x1a,
    0x1c, 0x1e, 0x20, 0x22, 0x24, 0x3d, 0x3f, 0x41, 0x43, 0x45, 0x47,
    0x49, 0x4f, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57};
...
enum lsx_serdes_id { LSX_SERDES_1 = 1, LSX_SERDES_2 = 2 };
...
static inline int ls_mac_to_serdes_id(uint8_t mac_id) {
...
static inline int lx_mac_to_serdes_id(uint8_t mac_id) {
...
static inline int ls_serdes_cfg_to_idx(uint8_t sd_cfg, int sd_id) {
  int i;
...
static inline int lx_serdes_cfg_to_idx(uint8_t sd_cfg, int sd_id __rte_unused) {
  return sd_cfg;
}

static inline int ls_mac_serdes_lpbk_support(uint16_t mac_id,
                                             uint16_t *serdes_id,
                                             uint16_t *lan_id) {
  struct ccsr_gur *gur_base = lsx_ccsr_map_region(
      CONFIG_SYS_FSL_GUTS_ADDR, sizeof(struct ccsr_gur) / 64 * 64 + 64);
  uint32_t sd_cfg;
  int sd_id, sd_idx;
  ...
  if (sd_id == LSX_SERDES_1) {
    ls_sd_loopback_support = &ls_sd1_eth_loopback_support[sd_idx][0];
  } else {
    ls_sd_loopback_support = &ls_sd2_eth_loopback_support[sd_idx][0];
  }
...
static inline int lx_mac_serdes_lpbk_support(uint16_t mac_id,
                                             uint16_t *serdes_id,
                                             uint16_t *lan_id) {
  struct ccsr_gur *gur_base = lsx_ccsr_map_region(
      CONFIG_SYS_FSL_GUTS_ADDR, sizeof(struct ccsr_gur) / 64 * 64 + 64);
...
static inline int ls_serdes_eth_lpbk(uint16_t mac_id, int en) {
  ...
  serdes_base = lsx_ccsr_map_region(
      CONFIG_SYS_FSL_SERDES_ADDR + (serdes_id - LSX_SERDES_1) * 0x10000,
...
static inline int lx_serdes_eth_lpbk(uint16_t mac_id, int en) {
...
int dpaa2_dev_recycle_config(struct rte_eth_dev *eth_dev) {
  ...
  if (dpaa2_dev->ep_dev_type == DPAA2_MAC) {
    ...
    ret = dpni_set_port_cfg(dpni_dev, CMD_PRI_LOW, priv->token,
...
int dpaa2_dev_recycle_deconfig(struct rte_eth_dev *eth_dev) {
  ...
  if (priv->flags & DPAA2_TX_SERDES_LOOPBACK_MODE) {
    if (dpaa2_svr_family == SVR_LS2088A) {
      ...
      if (ret) {
        DPAA2_PMD_WARN("Error(%d) to disable Serdes loopback", ret);
      } else {
        priv->flags &= ~DPAA2_TX_SERDES_LOOPBACK_MODE;
      }
    } else if (dpaa2_svr_family == SVR_LX2160A) {
  ...
  if (priv->flags & DPAA2_TX_MAC_LOOPBACK_MODE) {
    ...
    ret = dpni_set_port_cfg(dpni_dev, CMD_PRI_LOW, priv->token,
                            DPNI_PORT_CFG_LOOPBACK, &port_cfg);
    if (ret) {
      DPAA2_PMD_ERR("Error(%d) to disable TX mac loopback", ret);
    } else {
      priv->flags &= ~DPAA2_TX_MAC_LOOPBACK_MODE;
    }
  }
...
int dpaa2_dev_recycle_qp_setup(struct rte_dpaa2_device *dpaa2_dev,
  ...
  if (!(priv->flags & DPAA2_TX_LOOPBACK_MODE) && (tx_lpbk || rx_lpbk)) {
