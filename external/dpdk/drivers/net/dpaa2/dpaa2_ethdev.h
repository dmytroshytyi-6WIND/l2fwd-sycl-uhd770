...
#define DPAA2_ETH_MAX_LEN                                                      \
  (RTE_ETHER_MTU + RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN + VLAN_TAG_SIZE)
...
#define DPAA2_TX_LOOPBACK_MODE                                                 \
  (DPAA2_TX_MAC_LOOPBACK_MODE | DPAA2_TX_SERDES_LOOPBACK_MODE |                \
   DPAA2_TX_DPNI_LOOPBACK_MODE)

#define DPAA2_RSS_OFFLOAD_ALL                                                  \
  (RTE_ETH_RSS_L2_PAYLOAD | RTE_ETH_RSS_IP | RTE_ETH_RSS_UDP |                 \
   RTE_ETH_RSS_TCP | RTE_ETH_RSS_SCTP | RTE_ETH_RSS_MPLS |                     \
   RTE_ETH_RSS_C_VLAN | RTE_ETH_RSS_S_VLAN | RTE_ETH_RSS_ESP |                 \
   RTE_ETH_RSS_AH | RTE_ETH_RSS_PPPOE)
...
#define DPAA2_PKT_TYPE_IPV4_EXT (0x0001 | DPAA2_PKT_TYPE_IPV4)
#define DPAA2_PKT_TYPE_IPV6_EXT (0x0001 | DPAA2_PKT_TYPE_IPV6)
#define DPAA2_PKT_TYPE_IPV4_TCP (0x000e | DPAA2_PKT_TYPE_IPV4)
#define DPAA2_PKT_TYPE_IPV6_TCP (0x000e | DPAA2_PKT_TYPE_IPV6)
#define DPAA2_PKT_TYPE_IPV4_UDP (0x0010 | DPAA2_PKT_TYPE_IPV4)
#define DPAA2_PKT_TYPE_IPV6_UDP (0x0010 | DPAA2_PKT_TYPE_IPV6)
#define DPAA2_PKT_TYPE_IPV4_SCTP (0x000f | DPAA2_PKT_TYPE_IPV4)
#define DPAA2_PKT_TYPE_IPV6_SCTP (0x000f | DPAA2_PKT_TYPE_IPV6)
#define DPAA2_PKT_TYPE_IPV4_ICMP (0x0003 | DPAA2_PKT_TYPE_IPV4_EXT)
#define DPAA2_PKT_TYPE_IPV6_ICMP (0x0003 | DPAA2_PKT_TYPE_IPV6_EXT)
...
__rte_internal int dpaa2_eth_eventq_attach(
    const struct rte_eth_dev *dev, int eth_rx_queue_id,
    struct dpaa2_dpcon_dev *dpcon,
    const struct rte_event_eth_rx_adapter_queue_conf *queue_conf);

__rte_internal int dpaa2_eth_eventq_detach(const struct rte_eth_dev *dev,
                                           int eth_rx_queue_id);
...
uint16_t dpaa2_dev_tx_ordered(void *queue, struct rte_mbuf **bufs,
                              uint16_t nb_pkts);
__rte_internal uint16_t dpaa2_dev_tx_multi_txq_ordered(void **queue,
