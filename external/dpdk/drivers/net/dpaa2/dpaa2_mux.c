...
rte_pmd_dpaa2_mux_flow_create(uint32_t dpdmux_id,
  ...
  key_cfg_iova = rte_zmalloc(NULL, DIST_PARAM_IOVA_SIZE, RTE_CACHE_LINE_SIZE);
  if (!key_cfg_iova) {
    DPAA2_PMD_ERR("Unable to allocate flow-dist parameters");
    return NULL;
  }
  flow = rte_zmalloc(NULL, sizeof(struct rte_flow) + (2 * DIST_PARAM_IOVA_SIZE),
                     RTE_CACHE_LINE_SIZE);
  if (!flow) {
  ...
  case RTE_FLOW_ITEM_TYPE_IPV4: {
    ...
    memcpy(key_iova, (const void *)(&spec->hdr.next_proto_id), sizeof(uint8_t));
  ...
  case RTE_FLOW_ITEM_TYPE_UDP: {
    ...
    memcpy((void *)key_iova, (const void *)&udp_dst_port, sizeof(rte_be16_t));
  ...
  case RTE_FLOW_ITEM_TYPE_RAW: {
    ...
    kg_cfg.num_extracts = 1;
    memcpy((void *)key_iova, (const void *)spec->pattern, spec->length);
  ...
  default:
    DPAA2_PMD_ERR("Not supported pattern type: %d", pattern[0]->type);
    goto creation_error;
  }
  ...
  ret = dpdmux_add_custom_cls_entry(&dpdmux_dev->dpdmux, CMD_PRI_LOW,
                                    ...
                                    &dpdmux_action);
  if (ret) {
    DPAA2_PMD_ERR("dpdmux_add_custom_cls_entry failed: err(%d)", ret);
    goto creation_error;
  }
...
int rte_pmd_dpaa2_mux_rx_frame_len(uint32_t dpdmux_id,
  ...
  ret = dpdmux_set_max_frame_length(&dpdmux_dev->dpdmux, CMD_PRI_LOW,
                                    dpdmux_dev->token, max_rx_frame_len);
  if (ret) {
...
dpaa2_create_dpdmux_device(int vdev_fd __rte_unused,
  ...
  ret = dpdmux_if_set_default(&dpdmux_dev->dpdmux, CMD_PRI_LOW,
                              dpdmux_dev->token, 1);
  if (ret) {
    DPAA2_PMD_ERR("setting default interface failed in %s", __func__);
    goto init_err;
  }

  ret = dpdmux_get_api_version(&dpdmux_dev->dpdmux, CMD_PRI_LOW, &maj_ver,
                               &min_ver);
  if (ret) {
  ...
  if (maj_ver >= 6 && min_ver >= 6) {
    ret = dpdmux_set_resetable(
        &dpdmux_dev->dpdmux, CMD_PRI_LOW, dpdmux_dev->token,
        DPDMUX_SKIP_DEFAULT_INTERFACE | DPDMUX_SKIP_UNICAST_RULES |
  ...
  if (maj_ver >= 6 && min_ver >= 9) {
    ...
    ret = dpdmux_if_set_errors_behavior(&dpdmux_dev->dpdmux, CMD_PRI_LOW,
                                        dpdmux_dev->token, dpdmux_id,
                                        &mux_err_cfg);
    if (ret) {
      DPAA2_PMD_ERR("dpdmux_if_set_errors_behavior %s err %d", __func__, ret);
      goto init_err;
    }
  }
