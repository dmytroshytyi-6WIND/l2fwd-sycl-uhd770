...
static uint64_t dev_rx_offloads_nodis =
    RTE_ETH_RX_OFFLOAD_RSS_HASH | RTE_ETH_RX_OFFLOAD_SCATTER;
...
static uint64_t dev_tx_offloads_nodis = RTE_ETH_TX_OFFLOAD_MULTI_SEGS;
...
static const struct rte_dpaa2_xstats_name_off dpaa2_xstats_strings[] = {
    {"ingress_multicast_frames", 0, 2},  {"ingress_multicast_bytes", 0, 3},
    {"ingress_broadcast_frames", 0, 4},  {"ingress_broadcast_bytes", 0, 5},
    {"egress_multicast_frames", 1, 2},   {"egress_multicast_bytes", 1, 3},
    {"egress_broadcast_frames", 1, 4},   {"egress_broadcast_bytes", 1, 5},
    {"ingress_filtered_frames", 2, 0},   {"ingress_discarded_frames", 2, 1},
    {"ingress_nobuffer_discards", 2, 2}, {"egress_discarded_frames", 2, 3},
    {"egress_confirmed_frames", 2, 4},   {"cgr_reject_frames", 4, 0},
    {"cgr_reject_bytes", 4, 1},
};

static struct rte_dpaa2_driver rte_dpaa2_pmd;
static int dpaa2_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete);
static int dpaa2_dev_set_link_up(struct rte_eth_dev *dev);
static int dpaa2_dev_set_link_down(struct rte_eth_dev *dev);
static int dpaa2_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu);

static int dpaa2_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                 int on) {
  int ret;
  ...
  if (on)
    ret = dpni_add_vlan_id(dpni, CMD_PRI_LOW, priv->token, vlan_id, 0, 0, 0);
  else
    ret = dpni_remove_vlan_id(dpni, CMD_PRI_LOW, priv->token, vlan_id);

  if (ret < 0)
    DPAA2_PMD_ERR("ret = %d Unable to add/rem vlan %d hwid =%d", ret, vlan_id,
                  priv->hw_id);

  return ret;
}

static int dpaa2_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = dev->process_private;
  int ret = 0;
  ...
  if (mask & RTE_ETH_VLAN_FILTER_MASK) {
    ...
    if (dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_VLAN_FILTER)
      ret = dpni_enable_vlan_filter(dpni, CMD_PRI_LOW, priv->token, true);
    else
      ret = dpni_enable_vlan_filter(dpni, CMD_PRI_LOW, priv->token, false);
    if (ret < 0)
...
static int dpaa2_vlan_tpid_set(struct rte_eth_dev *dev,
  ...
  ret = dpni_add_custom_tpid(dpni, CMD_PRI_LOW, priv->token, tpid);
  if (ret < 0)
  ...
  if (ret == -EBUSY) {
    ...
    ret = dpni_get_custom_tpid(dpni, CMD_PRI_LOW, priv->token, &tpid_list);
    if (ret < 0)
      goto fail;
    ret = dpni_remove_custom_tpid(dpni, CMD_PRI_LOW, priv->token,
                                  tpid_list.tpid1);
    if (ret < 0)
      goto fail;
    ret = dpni_add_custom_tpid(dpni, CMD_PRI_LOW, priv->token, tpid);
...
static int dpaa2_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
                                size_t fw_size) {
  int ret;
  struct fsl_mc_io *dpni = dev->process_private;
  ...
  ret = snprintf(fw_version, fw_size, "%x-%d.%d.%d", mc_plat_info.svr,
...
static int dpaa2_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa = dev_rx_offloads_sup | dev_rx_offloads_nodis;
  dev_info->tx_offload_capa = dev_tx_offloads_sup | dev_tx_offloads_nodis;
  dev_info->speed_capa =
  ...
  if (dpaa2_svr_family == SVR_LX2160A) {
    dev_info->speed_capa |= RTE_ETH_LINK_SPEED_25G | RTE_ETH_LINK_SPEED_40G |
...
static int dpaa2_dev_rx_burst_mode_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < RTE_DIM(rx_offload_map); i++) {
    if (eth_conf->rxmode.offloads & rx_offload_map[i].flags) {
      snprintf(mode->info, sizeof(mode->info), "%s", rx_offload_map[i].output);
      ret = 0;
      break;
    }
  }
...
static int dpaa2_dev_tx_burst_mode_get(struct rte_eth_dev *dev,
                                       __rte_unused uint16_t queue_id,
                                       struct rte_eth_burst_mode *mode) {
  struct rte_eth_conf *eth_conf = &dev->data->dev_conf;
  int ret = -EINVAL;
  ...
  for (i = 0; i < RTE_DIM(tx_offload_map); i++) {
    if (eth_conf->txmode.offloads & tx_offload_map[i].flags) {
      snprintf(mode->info, sizeof(mode->info), "%s", tx_offload_map[i].output);
      ret = 0;
      break;
    }
  }
...
static int dpaa2_alloc_rx_tx_queues(struct rte_eth_dev *dev) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  uint16_t dist_idx;
  uint32_t vq_id;
  ...
  for (i = 0; i < priv->nb_rx_queues; i++) {
    ...
    dpaa2_q->q_storage = rte_malloc(
        "dq_storage", sizeof(struct queue_storage_info_t), RTE_CACHE_LINE_SIZE);
  ...
  if (dpaa2_enable_err_queue) {
    priv->rx_err_vq = rte_zmalloc("dpni_rx_err", sizeof(struct dpaa2_queue), 0);
    if (!priv->rx_err_vq)
      goto fail;

    dpaa2_q = (struct dpaa2_queue *)priv->rx_err_vq;
    dpaa2_q->q_storage = rte_malloc(
        "err_dq_storage", sizeof(struct queue_storage_info_t) * RTE_MAX_LCORE,
        RTE_CACHE_LINE_SIZE);
    if (!dpaa2_q->q_storage)
      goto fail;
  ...
  if (priv->flags & DPAA2_TX_CONF_ENABLE) {
    ...
    for (i = 0; i < priv->nb_tx_queues; i++) {
      ...
      dpaa2_q->q_storage =
          rte_malloc("dq_storage", sizeof(struct queue_storage_info_t),
                     RTE_CACHE_LINE_SIZE);
      if (!dpaa2_q->q_storage)
        goto fail_tx_conf;
...
static void dpaa2_free_rx_tx_queues(struct rte_eth_dev *dev) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct dpaa2_queue *dpaa2_q;
  int i;
...
static int dpaa2_eth_dev_configure(struct rte_eth_dev *dev) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = dev->process_private;
  struct rte_eth_conf *eth_conf = &dev->data->dev_conf;
  ...
  max_rx_pktlen = eth_conf->rxmode.mtu + RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN +
                  VLAN_TAG_SIZE;
  if (max_rx_pktlen <= DPAA2_MAX_RX_PKT_LEN) {
    ret = dpni_set_max_frame_length(dpni, CMD_PRI_LOW, priv->token,
                                    max_rx_pktlen - RTE_ETHER_CRC_LEN);
    if (ret != 0) {
      DPAA2_PMD_ERR("Unable to set mtu. check config");
      return ret;
    }
    DPAA2_PMD_INFO("MTU configured for the device: %d", dev->data->mtu);
  } else {
    return -1;
  }
  ...
  if (eth_conf->rxmode.mq_mode == RTE_ETH_MQ_RX_RSS) {
    for (tc_index = 0; tc_index < priv->num_rx_tc; tc_index++) {
      ret = dpaa2_setup_flow_dist(dev, eth_conf->rx_adv_conf.rss_conf.rss_hf,
                                  tc_index);
      if (ret) {
        DPAA2_PMD_ERR("Unable to set flow distribution on tc%d."
                      "Check queue config",
                      tc_index);
        return ret;
      }
  ...
  ret = dpni_set_offload(dpni, CMD_PRI_LOW, priv->token, DPNI_OFF_RX_L3_CSUM,
                         rx_l3_csum_offload);
  if (ret) {
  ...
  ret = dpni_set_offload(dpni, CMD_PRI_LOW, priv->token, DPNI_OFF_RX_L4_CSUM,
                         rx_l4_csum_offload);
  if (ret) {
...
#endif
  {
    ret = rte_mbuf_dyn_rx_timestamp_register(&dpaa2_timestamp_dynfield_offset,
  ...
  ret = dpni_set_offload(dpni, CMD_PRI_LOW, priv->token, DPNI_OFF_TX_L3_CSUM,
                         tx_l3_csum_offload);
  if (ret) {
  ...
  ret = dpni_set_offload(dpni, CMD_PRI_LOW, priv->token, DPNI_OFF_TX_L4_CSUM,
                         tx_l4_csum_offload);
  if (ret) {
   ...
   */
  if (dpaa2_svr_family == SVR_LX2160A) {
    ret = dpni_set_offload(dpni, CMD_PRI_LOW, priv->token, DPNI_FLCTYPE_HASH,
                           true);
    if (ret) {
  ...
  if (eth_conf->lpbk_mode) {
    ...
    if (ret) {
      DPAA2_PMD_ERR("Error to configure %s to recycle port.", dev->data->name);
    ...
    if (ret) {
      DPAA2_PMD_ERR("Error to de-configure recycle port %s.", dev->data->name);
 ...
 */
static int dpaa2_dev_rx_queue_setup(struct rte_eth_dev *dev,
  ...
  DPAA2_PMD_DEBUG("dev =%p, queue =%d, pool = %p, conf =%p", dev, rx_queue_id,
                  mb_pool, rx_conf);
  ...
  if (total_nb_rx_desc > MAX_NB_RX_DESC) {
    DPAA2_PMD_WARN(
        ...
        MAX_NB_RX_DESC);
    DPAA2_PMD_WARN("To use Normal buffers, run 'export DPNI_NORMAL_BUF=1' "
                   "before running dynamic_dpl.sh script");
  }

  /* Rx deferred start is not supported */
  if (rx_conf->rx_deferred_start) {
    DPAA2_PMD_ERR("%p:Rx deferred start not supported", (void *)dev);
    return -EINVAL;
  }
  ...
  if (!priv->bp_list || priv->bp_list->mp != mb_pool) {
    ...
    ret = dpaa2_attach_bp_list(priv, dpni, rte_dpaa2_bpid_info[bpid].bp_list);
    if (ret)
      return ret;
  }
  ...
  if (!(priv->flags & DPAA2_RX_TAILDROP_OFF)) {
    ...
    if (dpaa2_q->cgid != 0xff) {
      ...
      taildrop.oal = 0;
      DPAA2_PMD_DEBUG("Enabling CG Tail Drop on queue = %d", rx_queue_id);
      ret = dpni_set_taildrop(dpni, CMD_PRI_LOW, priv->token,
                              DPNI_CP_CONGESTION_GROUP, DPNI_QUEUE_RX,
                              dpaa2_q->tc_index, dpaa2_q->cgid, &taildrop);
    } else {
      /*enabling per rx queue congestion control */
      taildrop.threshold = CONG_THRESHOLD_RX_BYTES_Q;
      taildrop.units = DPNI_CONGESTION_UNIT_BYTES;
      taildrop.oal = CONG_RX_OAL;
      DPAA2_PMD_DEBUG("Enabling Byte based Drop on queue= %d", rx_queue_id);
      ret = dpni_set_taildrop(dpni, CMD_PRI_LOW, priv->token, DPNI_CP_QUEUE,
                              DPNI_QUEUE_RX, dpaa2_q->tc_index, flow_id,
                              &taildrop);
    }
    if (ret) {
...
static int dpaa2_dev_tx_queue_setup(struct rte_eth_dev *dev,
                                    uint16_t tx_queue_id, uint16_t nb_tx_desc,
                                    unsigned int socket_id __rte_unused,
                                    const struct rte_eth_txconf *tx_conf) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct dpaa2_queue *dpaa2_q = (struct dpaa2_queue *)priv->tx_vq[tx_queue_id];
  struct dpaa2_queue *dpaa2_tx_conf_q =
      (struct dpaa2_queue *)priv->tx_conf_vq[tx_queue_id];
  struct fsl_mc_io *dpni = dev->process_private;
  ...
  if (tx_conf->tx_deferred_start) {
    DPAA2_PMD_ERR("%p:Tx deferred start not supported", (void *)dev);
    return -EINVAL;
  }
  ...
  if (tx_queue_id == 0) {
    ...
    if (priv->flags & DPAA2_TX_CONF_ENABLE)
      ret = dpni_set_tx_confirmation_mode(dpni, CMD_PRI_LOW, priv->token,
                                          DPNI_CONF_AFFINE);
    else
      ret = dpni_set_tx_confirmation_mode(dpni, CMD_PRI_LOW, priv->token,
                                          DPNI_CONF_DISABLE);
  ...
  ret = dpni_get_queue(dpni, CMD_PRI_LOW, priv->token, DPNI_QUEUE_TX,
  ...
  if (!(priv->flags & DPAA2_TX_CGR_OFF)) {
    ...
    ret = dpni_set_congestion_notification(
  ...
  if (priv->flags & DPAA2_TX_CONF_ENABLE) {
    ...
    ret = dpni_set_queue(dpni, CMD_PRI_LOW, priv->token, DPNI_QUEUE_TX_CONFIRM,
    ...
    ret = dpni_get_queue(dpni, CMD_PRI_LOW, priv->token, DPNI_QUEUE_TX_CONFIRM,
                         ((channel_id << 8) | dpaa2_tx_conf_q->tc_index),
...
static void dpaa2_dev_rx_queue_release(struct rte_eth_dev *dev,
  ...
  if (dpaa2_q->cgid != 0xff) {
    ...
    ret = dpni_set_queue(dpni, CMD_PRI_LOW, priv->token, DPNI_QUEUE_RX,
                         dpaa2_q->tc_index, dpaa2_q->flow_id, options, &cfg);
    if (ret)
      DPAA2_PMD_ERR("Unable to clear CGR from q=%u err=%d", dpaa2_q->fqid, ret);
    priv->cgid_in_use[dpaa2_q->cgid] = 0;
    dpaa2_q->cgid = 0xff;
  }
}

static uint32_t dpaa2_dev_rx_queue_count(void *rx_queue) {
  int32_t ret;
  struct dpaa2_queue *dpaa2_q;
  ...
  if (qbman_fq_query_state(swp, dpaa2_q->fqid, &state) == 0) {
    ...
    DPAA2_PMD_DP_DEBUG("RX frame count for q(%p) is %u", rx_queue, frame_cnt);
  }
  return frame_cnt;
}

static const uint32_t *dpaa2_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {/*todo -= add more types */
                                    RTE_PTYPE_L2_ETHER,    RTE_PTYPE_L3_IPV4,
                                    RTE_PTYPE_L3_IPV4_EXT, RTE_PTYPE_L3_IPV6,
 ...
 *  void
 */
static void dpaa2_interrupt_handler(void *param) {
  ...
  ret = dpni_get_irq_status(dpni, CMD_PRI_LOW, priv->token, irq_index, &status);
  if (unlikely(ret)) {
...
static int dpaa2_eth_setup_irqs(struct rte_eth_dev *dev, int enable) {
  int err = 0;
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
  int irq_index = DPNI_IRQ_INDEX;
  ...
  err = dpni_set_irq_mask(dpni, CMD_PRI_LOW, priv->token, irq_index, mask);
  if (err < 0) {
    DPAA2_PMD_ERR("Error: dpni_set_irq_mask():%d (%s)", err, strerror(-err));
    return err;
  }

  err = dpni_set_irq_enable(dpni, CMD_PRI_LOW, priv->token, irq_index, enable);
  if (err < 0)
    DPAA2_PMD_ERR("Error: dpni_set_irq_enable():%d (%s)", err, strerror(-err));

  return err;
}

static int dpaa2_dev_start(struct rte_eth_dev *dev) {
  ...
  if (ret) {
    DPAA2_PMD_ERR("Failure in enabling dpni %d device: err=%d", priv->hw_id,
                  ret);
    return ret;
  }
  ...
  for (i = 0; i < data->nb_rx_queues; i++) {
    ...
    ret = dpni_get_queue(dpni, CMD_PRI_LOW, priv->token, DPNI_QUEUE_RX,
                         dpaa2_q->tc_index, dpaa2_q->flow_id, &cfg, &qid);
    if (ret) {
  ...
  if (dpaa2_enable_err_queue) {
    ret = dpni_get_queue(dpni, CMD_PRI_LOW, priv->token, DPNI_QUEUE_RX_ERR, 0,
                         0, &cfg, &qid);
    if (ret) {
      DPAA2_PMD_ERR("Error getting rx err flow information: err=%d", ret);
      return ret;
    }
    dpaa2_q = (struct dpaa2_queue *)priv->rx_err_vq;
    dpaa2_q->fqid = qid.fqid;
  ...
  if (intr_handle && rte_intr_fd_get(intr_handle) &&
    ...
    rte_intr_callback_register(intr_handle, dpaa2_interrupt_handler,
                               (void *)dev);
 ...
 */
static int dpaa2_dev_stop(struct rte_eth_dev *dev) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
  int ret;
  ...
  if (intr_handle && rte_intr_fd_get(intr_handle) &&
    ...
    rte_intr_callback_unregister(intr_handle, dpaa2_interrupt_handler,
                                 (void *)dev);
  }
  ...
  if (ret) {
    DPAA2_PMD_ERR("Failure (ret %d) in disabling dpni %d dev", ret,
                  priv->hw_id);
    return ret;
  }
...
static int dpaa2_dev_close(struct rte_eth_dev *dev) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
  int i, ret;
  struct rte_eth_link link;
  ...
  if (ret) {
    DPAA2_PMD_ERR("Failure closing dpni device with err code %d", ret);
  }
...
static int dpaa2_dev_promiscuous_enable(struct rte_eth_dev *dev) {
  int ret;
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
...
static int dpaa2_dev_promiscuous_disable(struct rte_eth_dev *dev) {
  int ret;
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
  ...
  if (dev->data->all_multicast == 0) {
    ...
    if (ret < 0)
      DPAA2_PMD_ERR("Unable to disable M promisc mode %d", ret);
  }

  return ret;
}

static int dpaa2_dev_allmulticast_enable(struct rte_eth_dev *dev) {
  int ret;
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
...
static int dpaa2_dev_allmulticast_disable(struct rte_eth_dev *dev) {
  int ret;
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
...
static int dpaa2_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  ...
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
  uint32_t frame_size =
      mtu + RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN + VLAN_TAG_SIZE;
...
static int dpaa2_dev_add_mac_addr(struct rte_eth_dev *dev,
  ...
  ret = dpni_add_mac_addr(dpni, CMD_PRI_LOW, priv->token, addr->addr_bytes, 0,
                          0, 0);
  if (ret)
    DPAA2_PMD_ERR("error: Adding the MAC ADDR failed: err = %d", ret);
  return 0;
}

static void dpaa2_dev_remove_mac_addr(struct rte_eth_dev *dev, uint32_t index) {
  int ret;
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
  struct rte_eth_dev_data *data = dev->data;
  ...
  ret =
      dpni_remove_mac_addr(dpni, CMD_PRI_LOW, priv->token, macaddr->addr_bytes);
  if (ret)
    DPAA2_PMD_ERR("error: Removing the MAC ADDR failed: err = %d", ret);
}

static int dpaa2_dev_set_mac_addr(struct rte_eth_dev *dev,
                                  struct rte_ether_addr *addr) {
  int ret;
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
  ...
  ret = dpni_set_primary_mac_addr(dpni, CMD_PRI_LOW, priv->token,
                                  addr->addr_bytes);

  if (ret)
    DPAA2_PMD_ERR("error: Setting the MAC ADDR failed %d", ret);

  return ret;
}

static int dpaa2_dev_stats_get(struct rte_eth_dev *dev,
  ...
  retcode =
      dpni_get_statistics(dpni, CMD_PRI_LOW, priv->token, page0, 0, &value);
  if (retcode)
    goto err;
  ...
  retcode =
      dpni_get_statistics(dpni, CMD_PRI_LOW, priv->token, page1, 0, &value);
  if (retcode)
    goto err;
  ...
  retcode =
      dpni_get_statistics(dpni, CMD_PRI_LOW, priv->token, page2, 0, &value);
  if (retcode)
    goto err;
...
static int dpaa2_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  /* Get Counters from page_0*/
  retcode =
      dpni_get_statistics(dpni, CMD_PRI_LOW, priv->token, 0, 0, &value[0]);
  if (retcode)
    goto err;

  /* Get Counters from page_1*/
  retcode =
      dpni_get_statistics(dpni, CMD_PRI_LOW, priv->token, 1, 0, &value[1]);
  if (retcode)
    goto err;

  /* Get Counters from page_2*/
  retcode =
      dpni_get_statistics(dpni, CMD_PRI_LOW, priv->token, 2, 0, &value[2]);
  if (retcode)
    goto err;
  ...
  for (i = 0; i < priv->max_cgs; i++) {
    if (!priv->cgid_in_use[i]) {
      ...
      retcode =
          dpni_get_statistics(dpni, CMD_PRI_LOW, priv->token, 4, 0, &value[4]);
      if (retcode)
        goto err;
      break;
    }
...
static int dpaa2_xstats_get_names(__rte_unused struct rte_eth_dev *dev,
  ...
  if (xstats_names != NULL)
    for (i = 0; i < stat_cnt; i++)
      strlcpy(xstats_names[i].name, dpaa2_xstats_strings[i].name,
              sizeof(xstats_names[i].name));

  return stat_cnt;
}

static int dpaa2_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
...
static int dpaa2_xstats_get_names_by_id(struct rte_eth_dev *dev,
                                        const uint64_t *ids,
                                        struct rte_eth_xstat_name *xstats_names,
                                        unsigned int limit) {
  unsigned int i, stat_cnt = RTE_DIM(dpaa2_xstats_strings);
...
static int dpaa2_dev_stats_reset(struct rte_eth_dev *dev) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)dev->process_private;
  int retcode;
  int i;
...
static int dpaa2_dev_link_update(struct rte_eth_dev *dev,
  ...
  for (count = 0; count <= MAX_REPEAT_TIME; count++) {
    ...
    if (state.up == RTE_ETH_LINK_DOWN && wait_to_complete)
...
static int dpaa2_dev_set_link_down(struct rte_eth_dev *dev) {
  ...
  if (!retries) {
    ...
    DPAA2_PMD_INFO("Port %d Link DOWN successful", dev->data->port_id);
  }
...
static int dpaa2_flow_ctrl_get(struct rte_eth_dev *dev,
...
static int dpaa2_flow_ctrl_set(struct rte_eth_dev *dev,
                               struct rte_eth_fc_conf *fc_conf) {
  int ret = -EINVAL;
  struct dpaa2_dev_priv *priv;
  struct fsl_mc_io *dpni;
  ...
  case RTE_ETH_FC_NONE:
    ...
    break;
  default:
    DPAA2_PMD_ERR("Incorrect Flow control flag (%d)", fc_conf->mode);
    return -1;
  }

  ret = dpni_set_link_cfg(dpni, CMD_PRI_LOW, priv->token, &cfg);
  if (ret)
    DPAA2_PMD_ERR("Unable to set Link configuration (err=%d)", ret);
...
static int dpaa2_dev_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_hf) {
    for (tc_index = 0; tc_index < priv->num_rx_tc; tc_index++) {
      ret = dpaa2_setup_flow_dist(dev, rss_conf->rss_hf, tc_index);
      if (ret) {
        DPAA2_PMD_ERR("Unable to set flow dist on tc%d", tc_index);
        return ret;
      }
    }
  } else {
    for (tc_index = 0; tc_index < priv->num_rx_tc; tc_index++) {
      ...
      if (ret) {
        DPAA2_PMD_ERR("Unable to remove flow dist on tc%d", tc_index);
        return ret;
      }
    }
  }
...
static int dpaa2_dev_rss_hash_conf_get(struct rte_eth_dev *dev,
                                       struct rte_eth_rss_conf *rss_conf) {
  struct rte_eth_dev_data *data = dev->data;
  struct rte_eth_conf *eth_conf = &data->dev_conf;
...
int dpaa2_eth_eventq_attach(
    const struct rte_eth_dev *dev, int eth_rx_queue_id,
  ...
  if (queue_conf->ev.sched_type == RTE_SCHED_TYPE_ORDERED &&
    ...
    ret = dpni_set_opr(dpni, CMD_PRI_LOW, eth_priv->token, dpaa2_ethq->tc_index,
                       flow_id, OPR_OPT_CREATE, &ocfg, 0);
    if (ret) {
...
static int dpaa2_dev_flow_ops_get(struct rte_eth_dev *dev,
...
static void dpaa2_rxq_info_get(struct rte_eth_dev *dev, uint16_t queue_id,
...
static void dpaa2_txq_info_get(struct rte_eth_dev *dev, uint16_t queue_id,
...
static int dpaa2_tm_ops_get(struct rte_eth_dev *dev __rte_unused, void *ops) {
  *(const void **)ops = &dpaa2_tm_ops;

  return 0;
}

void rte_pmd_dpaa2_thread_init(void) {
  int ret;
 ...
 */
static int populate_mac_addr(struct fsl_mc_io *dpni_dev,
                             struct dpaa2_dev_priv *priv,
  ...
  if (!rte_is_zero_ether_addr(&phy_mac)) {
    ...
    if (!rte_is_same_ether_addr(&phy_mac, &prime_mac)) {
      ret = dpni_set_primary_mac_addr(dpni_dev, CMD_PRI_LOW, priv->token,
                                      phy_mac.addr_bytes);
      if (ret) {
...
static int dpaa2_get_devargs(struct rte_devargs *devargs, const char *key) {
  ...
  if (rte_kvargs_process(kvlist, key, check_devargs_handler, NULL) < 0) {
    rte_kvargs_free(kvlist);
    return 0;
  }
  rte_kvargs_free(kvlist);
...
static int dpaa2_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  if (ret) {
    DPAA2_PMD_ERR("Failure in opening dpni@%d with err code %d", hw_id, ret);
    rte_free(dpni_dev);
    return -1;
  }
  ...
  if (ret) {
    DPAA2_PMD_ERR("Failure cleaning dpni@%d with err code %d", hw_id, ret);
    goto init_err;
  }

  ret = dpni_get_attributes(dpni_dev, CMD_PRI_LOW, priv->token, &attr);
  if (ret) {
    DPAA2_PMD_ERR("Failure in get dpni@%d attribute, err code %d", hw_id, ret);
    goto init_err;
  }
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("dpni", RTE_ETHER_ADDR_LEN * attr.mac_filter_entries, 0);
  if (eth_dev->data->mac_addrs == NULL) {
    DPAA2_PMD_ERR("Failed to allocate %d bytes needed to store MAC addresses",
  ...
  if (priv->flags & DPAA2_TX_CONF_ENABLE) {
    layout.options =
        DPNI_BUF_LAYOUT_OPT_FRAME_STATUS | DPNI_BUF_LAYOUT_OPT_TIMESTAMP;
  ...
  memset(&priv->extract.qos_key_extract, 0, sizeof(struct dpaa2_key_extract));
  ...
  for (i = 0; i < MAX_TCS; i++) {
    ...
    priv->extract.tc_extract_param[i] = (size_t)rte_malloc(NULL, 256, 64);
    if (!priv->extract.tc_extract_param[i]) {
      DPAA2_PMD_ERR(
          " Error(%d) in allocation resources for flow classification", ret);
      goto init_err;
    }
  ...
  if (getenv("DPAA2_ENABLE_SOFT_PARSER")) {
    ...
    ret = dpaa2_eth_enable_wriop_soft_parser(priv, DPNI_SS_INGRESS);
    if (ret < 0) {
...
static int rte_dpaa2_probe(struct rte_dpaa2_driver *dpaa2_drv,
                           struct rte_dpaa2_device *dpaa2_dev) {
  ...
  if ((DPAA2_MBUF_HW_ANNOTATION + DPAA2_FD_PTA_SIZE) > RTE_PKTMBUF_HEADROOM) {
    DPAA2_PMD_ERR(
        ...
        RTE_PKTMBUF_HEADROOM, DPAA2_MBUF_HW_ANNOTATION + DPAA2_FD_PTA_SIZE);
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    ...
    dev_priv = rte_zmalloc("ethdev private structure",
                           sizeof(struct dpaa2_dev_priv), RTE_CACHE_LINE_SIZE);
    if (dev_priv == NULL) {
      DPAA2_PMD_CRIT("Unable to allocate memory for private data");
...
static int rte_dpaa2_remove(struct rte_dpaa2_device *dpaa2_dev) {
  struct rte_eth_dev *eth_dev;
  int ret;
...
RTE_PMD_REGISTER_PARAM_STRING(NET_DPAA2_PMD_DRIVER_NAME, DRIVER_LOOPBACK_MODE
                              "=<int> " DRIVER_NO_PREFETCH_MODE
                              "=<int>" DRIVER_TX_CONF
                              "=<int>" DRIVER_ERROR_QUEUE "=<int>");
