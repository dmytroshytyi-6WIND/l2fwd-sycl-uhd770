 ...
 */
int dprtc_open(struct fsl_mc_io *mc_io, uint32_t cmd_flags, int dprtc_id,
               uint16_t *token) {
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_OPEN, cmd_flags, 0);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_close(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_CLOSE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_create(struct fsl_mc_io *mc_io, uint16_t dprc_token,
                 uint32_t cmd_flags, const struct dprtc_cfg *cfg,
                 uint32_t *obj_id) {
  struct mc_command cmd = {0};
  int err;
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_CREATE, cmd_flags, dprc_token);
 ...
 * Return:	'0' on Success; error code otherwise.
 */
int dprtc_destroy(struct fsl_mc_io *mc_io, uint16_t dprc_token,
                  uint32_t cmd_flags, uint32_t object_id) {
  struct dprtc_cmd_destroy *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_DESTROY, cmd_flags, dprc_token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_enable(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_ENABLE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_disable(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_DISABLE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_is_enabled(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_IS_ENABLED, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_reset(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_RESET, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_get_attributes(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                         uint16_t token, struct dprtc_attr *attr) {
  struct dprtc_rsp_get_attributes *rsp_params;
  struct mc_command cmd = {0};
  int err;

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_GET_ATTR, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_set_clock_offset(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                           uint16_t token, int64_t offset) {
  struct dprtc_cmd_set_clock_offset *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPRTC_CMDID_SET_CLOCK_OFFSET, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_set_freq_compensation(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                uint16_t token, uint32_t freq_compensation) {
  struct dprtc_get_freq_compensation *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPRTC_CMDID_SET_FREQ_COMPENSATION, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_get_freq_compensation(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                uint16_t token, uint32_t *freq_compensation) {
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPRTC_CMDID_GET_FREQ_COMPENSATION, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_get_time(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token,
                   uint64_t *time) {
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_GET_TIME, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_set_time(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token,
                   uint64_t time) {
  struct dprtc_time *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_SET_TIME, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_set_alarm(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token,
                    uint64_t time) {
  struct dprtc_time *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_SET_ALARM, cmd_flags, token);
  cmd_params = (struct dprtc_time *)cmd.params;
  cmd_params->time = cpu_to_le64(time);
 ...
 * Return:  '0' on Success; Error code otherwise.
 */
int dprtc_get_api_version(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                          uint16_t *major_ver, uint16_t *minor_ver) {
  struct dprtc_rsp_get_api_version *rsp_params;
  struct mc_command cmd = {0};
  int err;

  cmd.header = mc_encode_cmd_header(DPRTC_CMDID_GET_API_VERSION, cmd_flags, 0);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dprtc_get_ext_trigger_timestamp(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                    uint16_t token, uint8_t id,
 ...
 * Return:  '0' on Success; Error code otherwise.
 */
int dprtc_set_fiper_loopback(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                             uint16_t token, uint8_t id,
  ...
  cmd.header =
      mc_encode_cmd_header(DPRTC_CMDID_SET_FIPER_LOOPBACK, cmd_flags, token);
