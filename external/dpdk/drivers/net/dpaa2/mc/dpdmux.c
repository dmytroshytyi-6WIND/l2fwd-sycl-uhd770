 ...
 */
int dpdmux_open(struct fsl_mc_io *mc_io, uint32_t cmd_flags, int dpdmux_id,
                uint16_t *token) {
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_OPEN, cmd_flags, 0);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_close(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_CLOSE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_create(struct fsl_mc_io *mc_io, uint16_t dprc_token,
                  uint32_t cmd_flags, const struct dpdmux_cfg *cfg,
                  uint32_t *obj_id) {
  struct mc_command cmd = {0};
 ...
 * Return:	'0' on Success; error code otherwise.
 */
int dpdmux_destroy(struct fsl_mc_io *mc_io, uint16_t dprc_token,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_DESTROY, cmd_flags, dprc_token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_enable(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_ENABLE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_disable(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                   uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_DISABLE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_is_enabled(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_IS_ENABLED, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_reset(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_RESET, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_set_resetable(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                         uint16_t token, uint8_t skip_reset_flags) {
  struct mc_command cmd = {0};
  struct dpdmux_cmd_set_skip_reset_flags *cmd_params;

  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_SET_RESETABLE, cmd_flags, token);
  cmd_params = (struct dpdmux_cmd_set_skip_reset_flags *)cmd.params;
  dpdmux_set_field(cmd_params->skip_reset_flags, SKIP_RESET_FLAGS,
                   skip_reset_flags);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_get_resetable(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                         uint16_t token, uint8_t *skip_reset_flags) {
  struct mc_command cmd = {0};
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_GET_RESETABLE, cmd_flags, token);
  ...
  *skip_reset_flags =
      dpdmux_get_field(rsp_params->skip_reset_flags, SKIP_RESET_FLAGS);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_get_attributes(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                          uint16_t token, struct dpdmux_attr *attr) {
  struct mc_command cmd = {0};
  struct dpdmux_rsp_get_attr *rsp_params;
  int err;

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_GET_ATTR, cmd_flags, token);
 ...
 */
int dpdmux_if_enable(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                     uint16_t token, uint16_t if_id) {
  struct dpdmux_cmd_if *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_IF_ENABLE, cmd_flags, token);
 ...
 * Return:	Completion status. '0' on Success; Error code otherwise.
 */
int dpdmux_if_disable(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                      uint16_t token, uint16_t if_id) {
  struct dpdmux_cmd_if *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_IF_DISABLE, cmd_flags, token);
  cmd_params = (struct dpdmux_cmd_if *)cmd.params;
  cmd_params->if_id = cpu_to_le16(if_id);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_set_max_frame_length(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_SET_MAX_FRAME_LENGTH, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_get_max_frame_length(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                uint16_t token, uint16_t if_id,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_GET_MAX_FRAME_LENGTH, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_ul_reset_counters(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                             uint16_t token) {
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_UL_RESET_COUNTERS, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_if_set_accepted_frames(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                  uint16_t token, uint16_t if_id,
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_if_get_attributes(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                             uint16_t token, uint16_t if_id,
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_IF_GET_ATTR, cmd_flags, token);
  cmd_params = (struct dpdmux_cmd_if *)cmd.params;
  cmd_params->if_id = cpu_to_le16(if_id);
  ...
  attr->accept_frame_type =
      dpdmux_get_field(rsp_params->accepted_frames_type, ACCEPTED_FRAMES_TYPE);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_if_remove_l2_rule(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                             uint16_t token, uint16_t if_id,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_IF_REMOVE_L2_RULE, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_if_add_l2_rule(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                          uint16_t token, uint16_t if_id,
                          const struct dpdmux_l2_rule *rule) {
  struct mc_command cmd = {0};
  struct dpdmux_cmd_if_l2_rule *cmd_params;
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_IF_ADD_L2_RULE, cmd_flags, token);
  cmd_params = (struct dpdmux_cmd_if_l2_rule *)cmd.params;
  cmd_params->if_id = cpu_to_le16(if_id);
  cmd_params->vlan_id = cpu_to_le16(rule->vlan_id);
  cmd_params->mac_addr5 = rule->mac_addr[5];
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_if_get_counter(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                          uint16_t token, uint16_t if_id,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_IF_GET_COUNTER, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpdmux_if_set_link_cfg(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                           uint16_t token, uint16_t if_id,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_IF_SET_LINK_CFG, cmd_flags, token);
 ...
 */
int dpdmux_if_get_link_state(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                             uint16_t token, uint16_t if_id,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_IF_GET_LINK_STATE, cmd_flags, token);
 ...
 * @returns	'0' on Success; Error code otherwise.
 */
int dpdmux_if_set_default(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                          uint16_t token, uint16_t if_id) {
  struct dpdmux_cmd_if *cmd_params;
  struct mc_command cmd = {0};
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_IF_SET_DEFAULT, cmd_flags, token);
  cmd_params = (struct dpdmux_cmd_if *)cmd.params;
  cmd_params->if_id = cpu_to_le16(if_id);
 ...
 * @returns	'0' on Success; Error code otherwise.
 */
int dpdmux_if_get_default(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                          uint16_t token, uint16_t *if_id) {
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_IF_GET_DEFAULT, cmd_flags, token);
 ...
 * @returns	'0' on Success; Error code otherwise.
 */
int dpdmux_set_custom_key(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                          uint16_t token, uint64_t key_cfg_iova) {
  struct dpdmux_set_custom_key *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_SET_CUSTOM_KEY, cmd_flags, token);
 ...
 * @returns	'0' on Success; Error code otherwise.
 */
int dpdmux_add_custom_cls_entry(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                uint16_t token, struct dpdmux_rule_cfg *rule,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_ADD_CUSTOM_CLS_ENTRY, cmd_flags, token);
 ...
 * @returns	'0' on Success; Error code otherwise.
 */
int dpdmux_remove_custom_cls_entry(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                   uint16_t token,
 ...
 * Return:  '0' on Success; Error code otherwise.
 */
int dpdmux_get_api_version(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                           uint16_t *major_ver, uint16_t *minor_ver) {
  struct mc_command cmd = {0};
  struct dpdmux_rsp_get_api_version *rsp_params;
  int err;

  cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_GET_API_VERSION, cmd_flags, 0);
...
int dpdmux_if_set_errors_behavior(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                                  uint16_t token, uint16_t if_id,
                                  struct dpdmux_error_cfg *cfg) {
  struct mc_command cmd = {0};
  struct dpdmux_cmd_set_errors_behavior *cmd_params;

  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPDMUX_CMDID_SET_ERRORS_BEHAVIOR, cmd_flags, token);
