 ...
 * @oal:		Overhead accounting length. 11bit value added to the
 *size of each frame. Used only for LNI shaping. If set to zero, will use
 *default value of 24. Ignored if shaping_lni is set to zero.
 * @shaping_lni:	1 for LNI shaping (configure whole throughput of the
 *dpni object) 0 for channel shaping (configure shaping for individual channels)
 *Set to one only if dpni is connected to a dpmac object.
 * @channel_id:		Channel to be configured. Ignored shaping_lni is set to
 *1
 * @coupled:		Committed and excess rates are coupled
 */
#define DPNI_TX_SHAPING_PARAM(oal, shaping_lni, channel_id, coupled)           \
  (((uint32_t)(((oal)&0x7ff) << 16)) | ((uint32_t)((channel_id)&0xff) << 8) |  \
   ((uint32_t)(!!shaping_lni) << 1) | ((uint32_t) !!coupled))
...
int dpni_set_mtu(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token,
                 uint16_t mtu);

int dpni_get_mtu(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token,
                 uint16_t *mtu);
 ...
 */
enum dpni_dest { DPNI_DEST_NONE = 0, DPNI_DEST_DPIO = 1, DPNI_DEST_DPCON = 2 };
...
int dpni_set_taildrop(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                      ...
                      enum dpni_queue_type q_type, uint16_t param,
                      uint8_t q_index, struct dpni_taildrop *taildrop);

int dpni_get_taildrop(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                      uint16_t token, enum dpni_congestion_point cg_point,
                      enum dpni_queue_type q_type, uint8_t tc, uint8_t q_index,
