...
int rte_pmd_dpaa2_set_custom_hash(uint16_t port_id, uint16_t offset,
  ...
  p_params = rte_zmalloc(NULL, DIST_PARAM_IOVA_SIZE, RTE_CACHE_LINE_SIZE);
  if (!p_params) {
  ...
  ret = dpni_set_rx_tc_dist(dpni, CMD_PRI_LOW, priv->token, tc_index, &tc_cfg);
  rte_free(p_params);
  if (ret) {
    DPAA2_PMD_ERR("Setting distribution for Rx failed with err: %d", ret);
    return ret;
  }
...
int dpaa2_setup_flow_dist(struct rte_eth_dev *eth_dev, uint64_t req_dist_set,
                          int tc_index) {
  struct dpaa2_dev_priv *priv = eth_dev->data->dev_private;
   ...
   */
  tc_dist_queues = eth_dev->data->nb_rx_queues - tc_index * priv->dist_queues;
  if (tc_dist_queues <= 0) {
  ...
  p_params = rte_malloc(NULL, DIST_PARAM_IOVA_SIZE, RTE_CACHE_LINE_SIZE);
  if (!p_params) {
    DPAA2_PMD_ERR("Unable to allocate flow-dist parameters");
    return -ENOMEM;
  }
  ...
  if (ret) {
    DPAA2_PMD_ERR("Given RSS Hash (%" PRIx64 ") not supported", req_dist_set);
    rte_free(p_params);
    return ret;
  }
...
int dpaa2_remove_flow_dist(struct rte_eth_dev *eth_dev, uint8_t tc_index) {
  struct dpaa2_dev_priv *priv = eth_dev->data->dev_private;
  struct fsl_mc_io *dpni = priv->hw;
  struct dpni_rx_dist_cfg tc_cfg;
  struct dpkg_profile_cfg kg_cfg;
  ...
  while (req_dist_set) {
    if (req_dist_set % 2 != 0) {
      ...
      case RTE_ETH_RSS_ETH:
        ...
        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_ETH;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_ETH_TYPE;
      ...
      case RTE_ETH_RSS_PPPOE:
        if (pppoe_configured)
          break;
        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_PPPOE;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_PPPOE_SID;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;
        break;
      ...
      case RTE_ETH_RSS_ESP:
        ...
        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_IPSEC_ESP;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_IPSEC_ESP_SPI;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;
        break;
      ...
      case RTE_ETH_RSS_AH:
        ...
        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_IPSEC_AH;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_IPSEC_AH_SPI;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;
        break;
      ...
      case RTE_ETH_RSS_MPLS:
        ...
        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_MPLS;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_MPLS_MPLSL_1;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;

        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_MPLS;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_MPLS_MPLSL_2;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;

        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_MPLS;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_MPLS_MPLSL_N;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;
        break;
      ...
      case RTE_ETH_RSS_IPV6_EX:
        ...
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_IP_SRC;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;

        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_IP;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_IP_DST;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;

        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_IP;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_IP_PROTO;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        kg_cfg->num_extracts++;
        i++;
      ...
      case RTE_ETH_RSS_IPV6_UDP_EX:
        ...
        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_TCP;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_TCP_PORT_SRC;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;
      ...
      case RTE_ETH_RSS_NONFRAG_IPV6_SCTP:
        ...
        kg_cfg->extracts[i].extract.from_hdr.prot = NET_PROT_SCTP;
        kg_cfg->extracts[i].extract.from_hdr.field = NH_FLD_SCTP_PORT_SRC;
        kg_cfg->extracts[i].type = DPKG_EXTRACT_FROM_HDR;
        kg_cfg->extracts[i].extract.from_hdr.type = DPKG_FULL_FIELD;
        i++;
      ...
      default:
        DPAA2_PMD_WARN("unsupported flow dist option 0x%" PRIx64, dist_field);
        return -EINVAL;
      }
    }
...
int dpaa2_attach_bp_list(struct dpaa2_dev_priv *priv, struct fsl_mc_io *dpni,
                         void *blist) {
  ...
  layout.options =
      DPNI_BUF_LAYOUT_OPT_DATA_HEAD_ROOM | DPNI_BUF_LAYOUT_OPT_FRAME_STATUS |
      DPNI_BUF_LAYOUT_OPT_PARSER_RESULT | DPNI_BUF_LAYOUT_OPT_DATA_ALIGN |
      DPNI_BUF_LAYOUT_OPT_TIMESTAMP | DPNI_BUF_LAYOUT_OPT_PRIVATE_DATA_SIZE;
  ...
  if (retcode) {
    DPAA2_PMD_ERR("Error configuring buffer pool Rx layout (%d)", retcode);
    return retcode;
  }
  ...
  bpool_cfg.pools[0].buffer_size =
      RTE_ALIGN_CEIL(bp_list->buf_pool.size, DPAA2_PACKET_LAYOUT_ALIGN);
