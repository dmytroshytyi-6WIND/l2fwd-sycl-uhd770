...
void dpaa2_tm_deinit(struct rte_eth_dev *dev) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct dpaa2_tm_shaper_profile *profile = LIST_FIRST(&priv->shaper_profiles);
...
static struct dpaa2_tm_node *dpaa2_node_from_id(struct dpaa2_dev_priv *priv,
                                                uint32_t node_id) {
...
static int dpaa2_capabilities_get(struct rte_eth_dev *dev,
...
static int dpaa2_level_capabilities_get(struct rte_eth_dev *dev,
                                        uint32_t level_id,
  ...
  if (level_id == LNI_LEVEL) { /* Root node (LNI) */
    ...
    cap->nonleaf.sched_n_children_max =
        priv->num_channels; /* no. of channels */
    cap->nonleaf.sched_sp_n_priorities_max = 1;
    cap->nonleaf.sched_wfq_n_children_per_group_max = 1;
    cap->nonleaf.sched_wfq_n_groups_max = 1;
    cap->nonleaf.sched_wfq_weight_max = 1;
    cap->nonleaf.stats_mask = RTE_TM_STATS_N_PKTS | RTE_TM_STATS_N_BYTES;
    ...
    cap->leaf.stats_mask = RTE_TM_STATS_N_PKTS | RTE_TM_STATS_N_BYTES;
  }

  return 0;
}

static int dpaa2_node_capabilities_get(struct rte_eth_dev *dev,
                                       uint32_t node_id,
  ...
  if (!node)
    return -rte_tm_error_set(error, ENODEV, RTE_TM_ERROR_TYPE_NODE_ID, NULL,
...
static int dpaa2_node_type_get(struct rte_eth_dev *dev, uint32_t node_id,
...
static int dpaa2_shaper_profile_add(struct rte_eth_dev *dev,
                                    uint32_t shaper_profile_id,
  ...
  if (params->pkt_length_adjust > DPAA2_PKT_ADJUST_LEN_MAX ||
    ...
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_CAPABILITIES,
                             NULL, "Not supported pkt adjust length\n");
  ...
  profile = rte_zmalloc_socket(NULL, sizeof(*profile), 0, rte_socket_id());
  if (!profile)
...
static int dpaa2_shaper_profile_delete(struct rte_eth_dev *dev,
                                       uint32_t shaper_profile_id,
                                       struct rte_tm_error *error) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct dpaa2_tm_shaper_profile *profile;
  ...
  profile = dpaa2_shaper_profile_from_id(priv, shaper_profile_id);
  if (!profile)
    return -rte_tm_error_set(error, ENODEV, RTE_TM_ERROR_TYPE_SHAPER_PROFILE_ID,
                             NULL, "Profile id does not exist\n");

  if (profile->refcnt)
    return -rte_tm_error_set(error, EPERM, RTE_TM_ERROR_TYPE_SHAPER_PROFILE_ID,
...
static int dpaa2_node_check_params(struct rte_eth_dev *dev, uint32_t node_id,
                                   __rte_unused uint32_t priority,
                                   uint32_t weight, uint32_t level_id,
                                   struct rte_tm_node_params *params,
                                   struct rte_tm_error *error) {
  if (node_id == RTE_TM_NODE_ID_NULL)
    return -rte_tm_error_set(error, EINVAL, RTE_TM_NODE_ID_NULL, NULL,
                             "Node id is invalid\n");
  ...
  if (level_id == QUEUE_LEVEL) {
    if (params->leaf.cman != RTE_TM_CMAN_TAIL_DROP)
      return -rte_tm_error_set(error, ENODEV,
                               RTE_TM_ERROR_TYPE_NODE_PARAMS_CMAN, NULL,
                               "Only taildrop is supported\n");
    if (params->stats_mask & ~(RTE_TM_STATS_N_PKTS | RTE_TM_STATS_N_BYTES))
      return -rte_tm_error_set(error, EINVAL,
                               RTE_TM_ERROR_TYPE_NODE_PARAMS_STATS, NULL,
...
static int dpaa2_node_add(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  if (0 /* If device is started*/)
    return -rte_tm_error_set(error, EPERM, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
  ...
  if (params->shaper_profile_id != RTE_TM_SHAPER_PROFILE_ID_NONE) {
    profile = dpaa2_shaper_profile_from_id(priv, params->shaper_profile_id);
    if (!profile)
      return -rte_tm_error_set(error, ENODEV,
                               RTE_TM_ERROR_TYPE_SHAPER_PROFILE_ID, NULL,
...
static int dpaa2_node_delete(struct rte_eth_dev *dev, uint32_t node_id,
                             struct rte_tm_error *error) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct dpaa2_tm_node *node;
  ...
  if (node->refcnt)
    return -rte_tm_error_set(error, EPERM, RTE_TM_ERROR_TYPE_NODE_ID, NULL,
                             "Node id is used\n");
...
static int dpaa2_tm_configure_queue(struct rte_eth_dev *dev,
                                    struct dpaa2_tm_node *node) {
  int ret = 0;
  uint32_t tc_id;
  ...
  if (ret) {
    printf("Error in setting the tx flow: "
           ...
           ((node->parent->channel_id << 8) | tc_id), flow_id, ret);
    return -1;
  }
  ...
  if (!(priv->flags & DPAA2_TX_CGR_OFF)) {
    ...
    ret = dpni_set_congestion_notification(
        ...
        ((node->parent->channel_id << 8) | tc_id), &cong_notif_cfg);
    if (ret) {
...
static void dpaa2_tm_sort_and_configure(struct rte_eth_dev *dev,
  ...
  if (n == 1) {
    DPAA2_PMD_DEBUG("node id = %d\n, priority = %d, index = %d\n",
                    nodes[n - 1]->id, nodes[n - 1]->priority, n - 1);
...
static int dpaa2_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
                                  struct rte_tm_error *error) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct dpaa2_tm_node *node;
  ...
  LIST_FOREACH(channel_node, &priv->nodes, next) {
    ...
    LIST_FOREACH(leaf_node, &priv->nodes, next) {
      ...
      if (leaf_node->parent == channel_node) {
        if (i >= DPNI_MAX_TC) {
          ret = -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_UNSPECIFIED,
  ...
  LIST_FOREACH(node, &priv->nodes, next) {
    if (node->type == NON_LEAF_NODE) {
      ...
      tx_cr_shaper.max_burst_size = node->profile->params.committed.size;
      tx_cr_shaper.rate_limit =
          node->profile->params.committed.rate / (1024 * 1024);
      tx_er_shaper.max_burst_size = node->profile->params.peak.size;
      tx_er_shaper.rate_limit = node->profile->params.peak.rate / (1024 * 1024);
      /* root node */
      if (node->parent == NULL) {
      ...
      ret = dpni_set_tx_shaping(dpni, 0, priv->token, &tx_cr_shaper,
                                &tx_er_shaper, param);
      if (ret) {
        ret = -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_SHAPER_PROFILE,
                                NULL, "Error in setting Shaping\n");
        goto out;
      }
      continue;
    }
  ...
  LIST_FOREACH(channel_node, &priv->nodes, next) {
    ...
    LIST_FOREACH(leaf_node, &priv->nodes, next) {
      ...
      if (leaf_node->id >= dev->data->nb_tx_queues) {
        ret = -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_NODE_ID, NULL,
      ...
      LIST_FOREACH(temp_leaf_node, &priv->nodes, next) {
        ...
        if (leaf_node->priority == temp_leaf_node->priority) {
          ...
          temp_leaf_dpaa2_q =
              (struct dpaa2_queue *)dev->data->tx_queues[temp_leaf_node->id];
          ...
          if (wfq_grp == 0) {
            ...
            ret =
                -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_UNSPECIFIED,
                                  NULL, "Only 2 WFQ Groups are supported\n");
            goto out;
          }
          is_wfq_grp = 1;
...
static int dpaa2_node_stats_read(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  if (node->level_id == LNI_LEVEL) {
    ...
    ret = dpni_get_statistics(dpni, CMD_PRI_LOW, priv->token, page1, 0, &value);
    if (ret)
      return -rte_tm_error_set(error, -ret, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
    ...
    ret = dpni_get_statistics(
        dpni, CMD_PRI_LOW, priv->token, page3,
        (node->parent->channel_id << 8 | dpaa2_q->tc_index), &value);
    if (ret)
      return -rte_tm_error_set(error, -ret, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
    ...
    return -rte_tm_error_set(error, -1, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
