...
static uint16_t dpaa2_flow_miss_flow_id = DPNI_FS_MISS_DROP;

#define FIXED_ENTRY_SIZE DPNI_MAX_KEY_SIZE

enum flow_rule_ipaddr_type { FLOW_NONE_IPADDR, FLOW_IPV4_ADDR, FLOW_IPV6_ADDR };
...
static const enum rte_flow_item_type dpaa2_supported_pattern_type[] = {
    RTE_FLOW_ITEM_TYPE_END,  RTE_FLOW_ITEM_TYPE_ETH,  RTE_FLOW_ITEM_TYPE_VLAN,
    RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_IPV6, RTE_FLOW_ITEM_TYPE_ICMP,
    RTE_FLOW_ITEM_TYPE_UDP,  RTE_FLOW_ITEM_TYPE_TCP,  RTE_FLOW_ITEM_TYPE_SCTP,
    RTE_FLOW_ITEM_TYPE_GRE,
};

static const enum rte_flow_action_type dpaa2_supported_action_type[] = {
...
static const enum rte_flow_action_type dpaa2_supported_fs_action_type[] = {
    RTE_FLOW_ACTION_TYPE_QUEUE, RTE_FLOW_ACTION_TYPE_PHY_PORT,
...
static const struct rte_flow_item_ipv6 dpaa2_flow_item_ipv6_mask = {
    .hdr = {.src_addr = "\xff\xff\xff\xff\xff\xff\xff\xff"
...
static inline void dpaa2_prot_field_string(enum net_prot prot, uint32_t field,
                                           char *string) {
...
static inline void
dpaa2_flow_qos_table_extracts_log(const struct dpaa2_dev_priv *priv) {
  int idx;
  char string[32];
  ...
  for (idx = 0; idx < priv->extract.qos_key_extract.dpkg.num_extracts; idx++) {
    dpaa2_prot_field_string(
        priv->extract.qos_key_extract.dpkg.extracts[idx].extract.from_hdr.prot,
        priv->extract.qos_key_extract.dpkg.extracts[idx].extract.from_hdr.field,
        string);
    printf("%s", string);
...
static inline void
dpaa2_flow_fs_table_extracts_log(const struct dpaa2_dev_priv *priv, int tc_id) {
  int idx;
  char string[32];
  ...
  for (idx = 0; idx < priv->extract.tc_key_extract[tc_id].dpkg.num_extracts;
       idx++) {
    dpaa2_prot_field_string(priv->extract.tc_key_extract[tc_id]
                            ...
                            string);
    printf("%s", string);
    if ((idx + 1) < priv->extract.tc_key_extract[tc_id].dpkg.num_extracts)
      printf(" / ");
  }
  printf("\r\n");
}

static inline void dpaa2_flow_qos_entry_log(const char *log_info,
  ...
  printf("\r\n%s QoS entry[%d] for TC[%d], extracts size is %d\r\n", log_info,
         qos_index, flow->tc_id, flow->qos_real_key_size);
...
static inline void dpaa2_flow_fs_entry_log(const char *log_info,
                                           const struct rte_flow *flow) {
  int idx;
  uint8_t *key, *mask;
  ...
  printf("\r\n%s FS/TC entry[%d] of TC[%d], extracts size is %d\r\n", log_info,
         flow->tc_index, flow->tc_id, flow->fs_real_key_size);
  ...
  printf("\r\n%s FS ipsrc: %d, ipdst: %d\r\n", log_info,
         flow->ipaddr_rule.fs_ipsrc_offset, flow->ipaddr_rule.fs_ipdst_offset);
}

static inline void dpaa2_flow_extract_key_set(struct dpaa2_key_info *key_info,
                                              int index, uint8_t size) {
  key_info->key_size[index] = size;
  if (index > 0) {
    key_info->key_offset[index] =
        key_info->key_offset[index - 1] + key_info->key_size[index - 1];
  } else {
...
static int dpaa2_flow_extract_add(struct dpaa2_key_extract *key_extract,
  ...
  for (index = 0; index < dpkg->num_extracts; index++) {
    if (dpkg->extracts[index].extract.from_hdr.prot == NET_PROT_IP) {
      if (dpkg->extracts[index].extract.from_hdr.field == NH_FLD_IP_SRC) {
        ip_src = index;
      }
      if (dpkg->extracts[index].extract.from_hdr.field == NH_FLD_IP_DST) {
        ip_dst = index;
      }
    }
  }
    ...
    if (field == NH_FLD_IP_SRC) {
      if (key_info->ipv4_dst_offset >= 0) {
        key_info->ipv4_src_offset =
            key_info->ipv4_dst_offset + NH_FLD_IPV4_ADDR_SIZE;
      } else {
        key_info->ipv4_src_offset =
            key_info->key_offset[index - 1] + key_info->key_size[index - 1];
      }
      if (key_info->ipv6_dst_offset >= 0) {
        key_info->ipv6_src_offset =
            key_info->ipv6_dst_offset + NH_FLD_IPV6_ADDR_SIZE;
      } else {
        key_info->ipv6_src_offset =
            key_info->key_offset[index - 1] + key_info->key_size[index - 1];
      }
      ...
      if (key_info->ipv4_src_offset >= 0) {
        key_info->ipv4_dst_offset =
            key_info->ipv4_src_offset + NH_FLD_IPV4_ADDR_SIZE;
      } else {
        key_info->ipv4_dst_offset =
            key_info->key_offset[index - 1] + key_info->key_size[index - 1];
      }
      if (key_info->ipv6_src_offset >= 0) {
        key_info->ipv6_dst_offset =
            key_info->ipv6_src_offset + NH_FLD_IPV6_ADDR_SIZE;
      } else {
        key_info->ipv6_dst_offset =
            key_info->key_offset[index - 1] + key_info->key_size[index - 1];
      }
  ...
  if (ip_src >= 0) {
    ip_src++;
    dpkg->extracts[ip_src].type = DPKG_EXTRACT_FROM_HDR;
    dpkg->extracts[ip_src].extract.from_hdr.type = DPKG_FULL_FIELD;
    dpkg->extracts[ip_src].extract.from_hdr.prot = NET_PROT_IP;
    dpkg->extracts[ip_src].extract.from_hdr.field = NH_FLD_IP_SRC;
  ...
  if (ip_dst >= 0) {
    ip_dst++;
    dpkg->extracts[ip_dst].type = DPKG_EXTRACT_FROM_HDR;
    dpkg->extracts[ip_dst].extract.from_hdr.type = DPKG_FULL_FIELD;
    dpkg->extracts[ip_dst].extract.from_hdr.prot = NET_PROT_IP;
    dpkg->extracts[ip_dst].extract.from_hdr.field = NH_FLD_IP_DST;
...
static int dpaa2_flow_extract_add_raw(struct dpaa2_key_extract *key_extract,
  ...
  if (dpkg->num_extracts != 0 &&
      dpkg->extracts[0].type != DPKG_EXTRACT_FROM_DATA) {
  ...
  for (index = 0; index < dpkg->num_extracts; index++) {
    ...
    if (index == dpkg->num_extracts - 1)
      dpkg->extracts[index].extract.from_data.size = last_extract_size;
    else
      dpkg->extracts[index].extract.from_data.size = DPAA2_FLOW_MAX_KEY_SIZE;
 ...
 */
static inline int
dpaa2_flow_proto_discrimination_extract(struct dpaa2_key_extract *key_extract,
                                        enum rte_flow_item_type type) {
  if (type == RTE_FLOW_ITEM_TYPE_ETH) {
    return dpaa2_flow_extract_add(key_extract, NET_PROT_ETH, NH_FLD_ETH_TYPE,
                                  sizeof(rte_be16_t));
  } else if (type == (enum rte_flow_item_type)DPAA2_FLOW_ITEM_TYPE_GENERIC_IP) {
    return dpaa2_flow_extract_add(key_extract, NET_PROT_IP, NH_FLD_IP_PROTO,
...
static inline int dpaa2_flow_extract_search(struct dpkg_profile_cfg *dpkg,
                                            enum net_prot prot,
                                            uint32_t field) {
  int i;
...
static inline int
dpaa2_flow_extract_key_offset(struct dpaa2_key_extract *key_extract,
                              enum net_prot prot, uint32_t field) {
  int i;
  struct dpkg_profile_cfg *dpkg = &key_extract->dpkg;
  struct dpaa2_key_info *key_info = &key_extract->key_info;
...
static int dpaa2_flow_proto_discrimination_rule(
  ...
  if (proto.type == RTE_FLOW_ITEM_TYPE_ETH) {
    ...
    DPAA2_PMD_ERR("Only Eth and IP support to discriminate next proto.");
    return -1;
  }

  offset = dpaa2_flow_extract_key_offset(&priv->extract.qos_key_extract, prot,
                                         field);
  if (offset < 0) {
    DPAA2_PMD_ERR("QoS prot %d field %d extract failed", prot, field);
    return -1;
  }
  ...
  if (proto.type == RTE_FLOW_ITEM_TYPE_ETH) {
    ...
    memcpy((void *)key_iova, (const void *)(&eth_type), sizeof(rte_be16_t));
    eth_type = 0xffff;
    memcpy((void *)mask_iova, (const void *)(&eth_type), sizeof(rte_be16_t));
  } else {
    ip_proto = proto.ip_proto;
    memcpy((void *)key_iova, (const void *)(&ip_proto), sizeof(uint8_t));
    ip_proto = 0xff;
    memcpy((void *)mask_iova, (const void *)(&ip_proto), sizeof(uint8_t));
  }

  offset = dpaa2_flow_extract_key_offset(&priv->extract.tc_key_extract[group],
                                         prot, field);
  if (offset < 0) {
    DPAA2_PMD_ERR("FS prot %d field %d extract failed", prot, field);
    return -1;
  }
...
static inline int
dpaa2_flow_rule_data_set(struct dpaa2_key_extract *key_extract,
                         struct dpni_rule_cfg *rule, enum net_prot prot,
                         uint32_t field, const void *key, const void *mask,
                         int size) {
  int offset = dpaa2_flow_extract_key_offset(key_extract, prot, field);

  if (offset < 0) {
    DPAA2_PMD_ERR("prot %d, field %d extract failed", prot, field);
    return -1;
  }
...
static inline int dpaa2_flow_rule_data_set_raw(struct dpni_rule_cfg *rule,
                                               const void *key,
                                               const void *mask, int size) {
  int offset = 0;
...
static inline int
_dpaa2_flow_rule_move_ipaddr_tail(struct dpaa2_key_extract *key_extract,
  ...
  if (ipv4)
    ...
    prot = NET_PROT_IPV6;
  dst_offset = dpaa2_flow_extract_key_offset(key_extract, prot, field);
  if (dst_offset < 0) {
...
static inline int dpaa2_flow_rule_move_ipaddr_tail(struct rte_flow *flow,
  ...
  if (flow->ipaddr_rule.qos_ipsrc_offset >= 0) {
    ret = _dpaa2_flow_rule_move_ipaddr_tail(
        &priv->extract.qos_key_extract, &flow->qos_rule,
        flow->ipaddr_rule.qos_ipsrc_offset, NH_FLD_IP_SRC,
        prot == NET_PROT_IPV4);
    if (ret) {
      DPAA2_PMD_ERR("QoS src address reorder failed");
      return -1;
    }
    flow->ipaddr_rule.qos_ipsrc_offset = dpaa2_flow_extract_key_offset(
        &priv->extract.qos_key_extract, prot, NH_FLD_IP_SRC);
  }

  if (flow->ipaddr_rule.qos_ipdst_offset >= 0) {
    ret = _dpaa2_flow_rule_move_ipaddr_tail(
        &priv->extract.qos_key_extract, &flow->qos_rule,
        flow->ipaddr_rule.qos_ipdst_offset, NH_FLD_IP_DST,
        prot == NET_PROT_IPV4);
    if (ret) {
      DPAA2_PMD_ERR("QoS dst address reorder failed");
      return -1;
    }
    flow->ipaddr_rule.qos_ipdst_offset = dpaa2_flow_extract_key_offset(
        &priv->extract.qos_key_extract, prot, NH_FLD_IP_DST);
  }

  if (flow->ipaddr_rule.fs_ipsrc_offset >= 0) {
    ret = _dpaa2_flow_rule_move_ipaddr_tail(
        &priv->extract.tc_key_extract[fs_group], &flow->fs_rule,
        flow->ipaddr_rule.fs_ipsrc_offset, NH_FLD_IP_SRC,
        prot == NET_PROT_IPV4);
    if (ret) {
      DPAA2_PMD_ERR("FS src address reorder failed");
      return -1;
    }
    flow->ipaddr_rule.fs_ipsrc_offset = dpaa2_flow_extract_key_offset(
        &priv->extract.tc_key_extract[fs_group], prot, NH_FLD_IP_SRC);
  }
  if (flow->ipaddr_rule.fs_ipdst_offset >= 0) {
    ret = _dpaa2_flow_rule_move_ipaddr_tail(
        &priv->extract.tc_key_extract[fs_group], &flow->fs_rule,
        flow->ipaddr_rule.fs_ipdst_offset, NH_FLD_IP_DST,
        prot == NET_PROT_IPV4);
    if (ret) {
      DPAA2_PMD_ERR("FS dst address reorder failed");
      return -1;
    }
    flow->ipaddr_rule.fs_ipdst_offset = dpaa2_flow_extract_key_offset(
        &priv->extract.tc_key_extract[fs_group], prot, NH_FLD_IP_DST);
  }

  return 0;
}

static int dpaa2_flow_extract_support(const uint8_t *mask_src,
                                      enum rte_flow_item_type type) {
  char mask[64];
  int i, size = 0;
...
static int dpaa2_configure_flow_eth(
    struct rte_flow *flow, struct rte_eth_dev *dev,
    const struct rte_flow_attr *attr, const struct rte_flow_item *pattern,
    const struct rte_flow_action actions[] __rte_unused,
    struct rte_flow_error *error __rte_unused, int *device_configured) {
  int index, ret;
  int local_cfg = 0;
  uint32_t group;
  ...
  mask = (const struct rte_flow_item_eth *)(pattern->mask
                                                ? pattern->mask
                                                : &dpaa2_flow_item_eth_mask);
  if (!spec) {
  ...
  if (memcmp((const char *)&mask->src, zero_cmp, RTE_ETHER_ADDR_LEN)) {
    index = dpaa2_flow_extract_search(&priv->extract.qos_key_extract.dpkg,
                                      NET_PROT_ETH, NH_FLD_ETH_SA);
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_ETH,
                                   NH_FLD_ETH_SA, RTE_ETHER_ADDR_LEN);
      if (ret) {
    ...
    index = dpaa2_flow_extract_search(&priv->extract.tc_key_extract[group].dpkg,
                                      NET_PROT_ETH, NH_FLD_ETH_SA);
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.tc_key_extract[group],
                                   NET_PROT_ETH, NH_FLD_ETH_SA,
                                   RTE_ETHER_ADDR_LEN);
      if (ret) {
    ...
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before ETH_SA rule set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   &flow->qos_rule, NET_PROT_ETH, NH_FLD_ETH_SA,
                                   &spec->src.addr_bytes, &mask->src.addr_bytes,
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   &flow->fs_rule, NET_PROT_ETH, NH_FLD_ETH_SA,
                                   &spec->src.addr_bytes, &mask->src.addr_bytes,
                                   sizeof(struct rte_ether_addr));
    if (ret) {
  ...
  if (memcmp((const char *)&mask->dst, zero_cmp, RTE_ETHER_ADDR_LEN)) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_ETH,
                                   NH_FLD_ETH_DA, RTE_ETHER_ADDR_LEN);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before ETH DA rule set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   &flow->qos_rule, NET_PROT_ETH, NH_FLD_ETH_DA,
                                   &spec->dst.addr_bytes, &mask->dst.addr_bytes,
                                   sizeof(struct rte_ether_addr));
    if (ret) {
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   &flow->fs_rule, NET_PROT_ETH, NH_FLD_ETH_DA,
                                   &spec->dst.addr_bytes, &mask->dst.addr_bytes,
                                   sizeof(struct rte_ether_addr));
    if (ret) {
  ...
  if (memcmp((const char *)&mask->type, zero_cmp, sizeof(rte_be16_t))) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_ETH,
                                   NH_FLD_ETH_TYPE, RTE_ETHER_TYPE_LEN);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before ETH TYPE rule set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(
        &priv->extract.qos_key_extract, &flow->qos_rule, NET_PROT_ETH,
        NH_FLD_ETH_TYPE, &spec->type, &mask->type, sizeof(rte_be16_t));
    if (ret) {
    ...
    ret = dpaa2_flow_rule_data_set(
        &priv->extract.tc_key_extract[group], &flow->fs_rule, NET_PROT_ETH,
        NH_FLD_ETH_TYPE, &spec->type, &mask->type, sizeof(rte_be16_t));
    if (ret) {
...
static int dpaa2_configure_flow_vlan(
    struct rte_flow *flow, struct rte_eth_dev *dev,
    const struct rte_flow_attr *attr, const struct rte_flow_item *pattern,
    const struct rte_flow_action actions[] __rte_unused,
    struct rte_flow_error *error __rte_unused, int *device_configured) {
  ...
  mask = (const struct rte_flow_item_vlan *)(pattern->mask
                                                 ? pattern->mask
                                                 : &dpaa2_flow_item_vlan_mask);
  ...
  if (!spec) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          &priv->extract.qos_key_extract, RTE_FLOW_ITEM_TYPE_ETH);
      if (ret) {
        DPAA2_PMD_ERR("QoS Ext ETH_TYPE to discriminate vLan failed");
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          &priv->extract.tc_key_extract[group], RTE_FLOW_ITEM_TYPE_ETH);
      if (ret) {
        DPAA2_PMD_ERR("FS Ext ETH_TYPE to discriminate vLan failed.");
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before vLan discrimination set failed");
      return -1;
    }
  ...
  if (index < 0) {
    ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_VLAN,
                                 NH_FLD_VLAN_TCI, sizeof(rte_be16_t));
    if (ret) {
  ...
  ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
  if (ret) {
    DPAA2_PMD_ERR("Move ipaddr before VLAN TCI rule set failed");
    return -1;
  }

  ret = dpaa2_flow_rule_data_set(
      &priv->extract.qos_key_extract, &flow->qos_rule, NET_PROT_VLAN,
      NH_FLD_VLAN_TCI, &spec->tci, &mask->tci, sizeof(rte_be16_t));
  if (ret) {
  ...
  ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                 &flow->fs_rule, NET_PROT_VLAN, NH_FLD_VLAN_TCI,
                                 &spec->tci, &mask->tci, sizeof(rte_be16_t));
  if (ret) {
...
static int dpaa2_configure_flow_ip_discrimation(
    struct dpaa2_dev_priv *priv, struct rte_flow *flow,
    const struct rte_flow_item *pattern, int *local_cfg, int *device_configured,
    uint32_t group) {
  int index, ret;
  struct proto_discrimination proto;
  ...
  if (index < 0) {
    ret = dpaa2_flow_proto_discrimination_extract(
        &priv->extract.qos_key_extract, RTE_FLOW_ITEM_TYPE_ETH);
    if (ret) {
      DPAA2_PMD_ERR("QoS Extract ETH_TYPE to discriminate IP failed.");
      return -1;
    }
    (*local_cfg) |= DPAA2_QOS_TABLE_RECONFIGURE;
  }
  ...
  if (index < 0) {
    ret = dpaa2_flow_proto_discrimination_extract(
        &priv->extract.tc_key_extract[group], RTE_FLOW_ITEM_TYPE_ETH);
    if (ret) {
      DPAA2_PMD_ERR("FS Extract ETH_TYPE to discriminate IP failed.");
      return -1;
    }
    (*local_cfg) |= DPAA2_FS_TABLE_RECONFIGURE;
  }
  ...
  ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
  if (ret) {
    DPAA2_PMD_ERR("Move ipaddr before IP discrimination set failed");
    return -1;
  }
...
static int dpaa2_configure_flow_generic_ip(
    struct rte_flow *flow, struct rte_eth_dev *dev,
    const struct rte_flow_attr *attr, const struct rte_flow_item *pattern,
    const struct rte_flow_action actions[] __rte_unused,
    struct rte_flow_error *error __rte_unused, int *device_configured) {
  ...
  if (pattern->type == RTE_FLOW_ITEM_TYPE_IPV4) {
    ...
    mask_ipv4 =
        (const struct rte_flow_item_ipv4 *)(pattern->mask
                                                ? pattern->mask
                                                : &dpaa2_flow_item_ipv4_mask);
  } else {
    spec_ipv6 = (const struct rte_flow_item_ipv6 *)pattern->spec;
    mask_ipv6 =
        (const struct rte_flow_item_ipv6 *)(pattern->mask
                                                ? pattern->mask
  ...
  ret = dpaa2_configure_flow_ip_discrimation(priv, flow, pattern, &local_cfg,
  ...
  if (mask_ipv4 && (mask_ipv4->hdr.src_addr || mask_ipv4->hdr.dst_addr)) {
    flow->ipaddr_rule.ipaddr_type = FLOW_IPV4_ADDR;
  } else if (mask_ipv6 && (memcmp((const char *)mask_ipv6->hdr.src_addr,
  ...
  if ((mask_ipv4 && mask_ipv4->hdr.src_addr) ||
      (mask_ipv6 && memcmp((const char *)mask_ipv6->hdr.src_addr, zero_cmp,
                           NH_FLD_IPV6_ADDR_SIZE))) {
    index = dpaa2_flow_extract_search(&priv->extract.qos_key_extract.dpkg,
                                      NET_PROT_IP, NH_FLD_IP_SRC);
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_IP,
                                   NH_FLD_IP_SRC, 0);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   &flow->qos_rule, prot, NH_FLD_IP_SRC, key,
                                   mask, size);
    if (ret) {
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   &flow->fs_rule, prot, NH_FLD_IP_SRC, key,
                                   mask, size);
    if (ret) {
    ...
    flow->ipaddr_rule.qos_ipsrc_offset = dpaa2_flow_extract_key_offset(
        &priv->extract.qos_key_extract, prot, NH_FLD_IP_SRC);
    flow->ipaddr_rule.fs_ipsrc_offset = dpaa2_flow_extract_key_offset(
        &priv->extract.tc_key_extract[group], prot, NH_FLD_IP_SRC);
  }

  if ((mask_ipv4 && mask_ipv4->hdr.dst_addr) ||
      (mask_ipv6 && memcmp((const char *)mask_ipv6->hdr.dst_addr, zero_cmp,
                           NH_FLD_IPV6_ADDR_SIZE))) {
    index = dpaa2_flow_extract_search(&priv->extract.qos_key_extract.dpkg,
    ...
    if (index < 0) {
      if (mask_ipv4)
        ...
        size = NH_FLD_IPV6_ADDR_SIZE;
      ret = dpaa2_flow_extract_add(&priv->extract.tc_key_extract[group],
                                   NET_PROT_IP, NH_FLD_IP_DST, size);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   &flow->qos_rule, prot, NH_FLD_IP_DST, key,
                                   mask, size);
    if (ret) {
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   &flow->fs_rule, prot, NH_FLD_IP_DST, key,
                                   mask, size);
    if (ret) {
    ...
    flow->ipaddr_rule.qos_ipdst_offset = dpaa2_flow_extract_key_offset(
        &priv->extract.qos_key_extract, prot, NH_FLD_IP_DST);
    flow->ipaddr_rule.fs_ipdst_offset = dpaa2_flow_extract_key_offset(
        &priv->extract.tc_key_extract[group], prot, NH_FLD_IP_DST);
  }
  ...
  if ((mask_ipv4 && mask_ipv4->hdr.next_proto_id) ||
    ...
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_IP,
                                   NH_FLD_IP_PROTO, NH_FLD_IP_PROTO_SIZE);
      if (ret) {
        DPAA2_PMD_ERR("QoS Extract add IP_DST failed.");
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr after NH_FLD_IP_PROTO rule set failed");
      return -1;
    }
    ...
    ret = dpaa2_flow_rule_data_set(
        &priv->extract.qos_key_extract, &flow->qos_rule, NET_PROT_IP,
        NH_FLD_IP_PROTO, key, mask, NH_FLD_IP_PROTO_SIZE);
    if (ret) {
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   &flow->fs_rule, NET_PROT_IP, NH_FLD_IP_PROTO,
...
static int dpaa2_configure_flow_icmp(
    struct rte_flow *flow, struct rte_eth_dev *dev,
    const struct rte_flow_attr *attr, const struct rte_flow_item *pattern,
    const struct rte_flow_action actions[] __rte_unused,
    struct rte_flow_error *error __rte_unused, int *device_configured) {
  ...
  mask = (const struct rte_flow_item_icmp *)(pattern->mask
                                                 ? pattern->mask
                                                 : &dpaa2_flow_item_icmp_mask);
  ...
  if (!spec) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          &priv->extract.qos_key_extract, DPAA2_FLOW_ITEM_TYPE_GENERIC_IP);
      if (ret) {
        DPAA2_PMD_ERR("QoS Extract IP protocol to discriminate ICMP failed.");
    ...
    if (index < 0) {
      ...
      if (ret) {
        DPAA2_PMD_ERR("FS Extract IP protocol to discriminate ICMP failed.");
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move IP addr before ICMP discrimination set failed");
      return -1;
    }
  ...
  if (mask->hdr.icmp_type) {
    ...
    if (index < 0) {
      ret =
          dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_ICMP,
                                 NH_FLD_ICMP_TYPE, NH_FLD_ICMP_TYPE_SIZE);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before ICMP TYPE set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   &flow->qos_rule, NET_PROT_ICMP,
                                   NH_FLD_ICMP_TYPE, &spec->hdr.icmp_type,
                                   &mask->hdr.icmp_type, NH_FLD_ICMP_TYPE_SIZE);
    if (ret) {
  ...
  if (mask->hdr.icmp_code) {
    ...
    if (index < 0) {
      ret =
          dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_ICMP,
                                 NH_FLD_ICMP_CODE, NH_FLD_ICMP_CODE_SIZE);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr after ICMP CODE set failed");
      return -1;
    }
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   ...
                                   &mask->hdr.icmp_code, NH_FLD_ICMP_CODE_SIZE);
    if (ret) {
...
static int dpaa2_configure_flow_udp(
    struct rte_flow *flow, struct rte_eth_dev *dev,
    const struct rte_flow_attr *attr, const struct rte_flow_item *pattern,
    const struct rte_flow_action actions[] __rte_unused,
    struct rte_flow_error *error __rte_unused, int *device_configured) {
  ...
  mask = (const struct rte_flow_item_udp *)(pattern->mask
                                                ? pattern->mask
                                                : &dpaa2_flow_item_udp_mask);
  ...
  if (!spec || !mc_l4_port_identification) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          &priv->extract.qos_key_extract, DPAA2_FLOW_ITEM_TYPE_GENERIC_IP);
      if (ret) {
        DPAA2_PMD_ERR("QoS Extract IP protocol to discriminate UDP failed.");
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          ...
          DPAA2_FLOW_ITEM_TYPE_GENERIC_IP);
      if (ret) {
        DPAA2_PMD_ERR("FS Extract IP protocol to discriminate UDP failed.");
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move IP addr before UDP discrimination set failed");
      return -1;
    }
  ...
  if (mask->hdr.src_port) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_UDP,
                                   NH_FLD_UDP_PORT_SRC, NH_FLD_UDP_PORT_SIZE);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before UDP_PORT_SRC set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   &flow->qos_rule, NET_PROT_UDP,
                                   NH_FLD_UDP_PORT_SRC, &spec->hdr.src_port,
                                   &mask->hdr.src_port, NH_FLD_UDP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("QoS NH_FLD_UDP_PORT_SRC rule data set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   &flow->fs_rule, NET_PROT_UDP,
                                   NH_FLD_UDP_PORT_SRC, &spec->hdr.src_port,
                                   &mask->hdr.src_port, NH_FLD_UDP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("FS NH_FLD_UDP_PORT_SRC rule data set failed");
      return -1;
    }
  }
  ...
  if (mask->hdr.dst_port) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_UDP,
                                   NH_FLD_UDP_PORT_DST, NH_FLD_UDP_PORT_SIZE);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before UDP_PORT_DST set failed");
      return -1;
    }
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   ...
                                   &mask->hdr.dst_port, NH_FLD_UDP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("QoS NH_FLD_UDP_PORT_DST rule data set failed");
      return -1;
    }
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   ...
                                   &mask->hdr.dst_port, NH_FLD_UDP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("FS NH_FLD_UDP_PORT_DST rule data set failed");
      return -1;
    }
  }
...
static int dpaa2_configure_flow_tcp(
    struct rte_flow *flow, struct rte_eth_dev *dev,
    const struct rte_flow_attr *attr, const struct rte_flow_item *pattern,
    const struct rte_flow_action actions[] __rte_unused,
    struct rte_flow_error *error __rte_unused, int *device_configured) {
  ...
  mask = (const struct rte_flow_item_tcp *)(pattern->mask
                                                ? pattern->mask
                                                : &dpaa2_flow_item_tcp_mask);
  ...
  if (!spec || !mc_l4_port_identification) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          &priv->extract.qos_key_extract, DPAA2_FLOW_ITEM_TYPE_GENERIC_IP);
      if (ret) {
        DPAA2_PMD_ERR("QoS Extract IP protocol to discriminate TCP failed.");
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          ...
          DPAA2_FLOW_ITEM_TYPE_GENERIC_IP);
      if (ret) {
        DPAA2_PMD_ERR("FS Extract IP protocol to discriminate TCP failed.");
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move IP addr before TCP discrimination set failed");
      return -1;
    }
  ...
  if (mask->hdr.src_port) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_TCP,
                                   NH_FLD_TCP_PORT_SRC, NH_FLD_TCP_PORT_SIZE);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before TCP_PORT_SRC set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   &flow->qos_rule, NET_PROT_TCP,
                                   NH_FLD_TCP_PORT_SRC, &spec->hdr.src_port,
                                   &mask->hdr.src_port, NH_FLD_TCP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("QoS NH_FLD_TCP_PORT_SRC rule data set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   &flow->fs_rule, NET_PROT_TCP,
                                   NH_FLD_TCP_PORT_SRC, &spec->hdr.src_port,
                                   &mask->hdr.src_port, NH_FLD_TCP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("FS NH_FLD_TCP_PORT_SRC rule data set failed");
      return -1;
    }
  }
  ...
  if (mask->hdr.dst_port) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_TCP,
                                   NH_FLD_TCP_PORT_DST, NH_FLD_TCP_PORT_SIZE);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before TCP_PORT_DST set failed");
      return -1;
    }
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   ...
                                   &mask->hdr.dst_port, NH_FLD_TCP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("QoS NH_FLD_TCP_PORT_DST rule data set failed");
      return -1;
    }
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   ...
                                   &mask->hdr.dst_port, NH_FLD_TCP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("FS NH_FLD_TCP_PORT_DST rule data set failed");
      return -1;
    }
  }
...
static int dpaa2_configure_flow_sctp(
    struct rte_flow *flow, struct rte_eth_dev *dev,
    const struct rte_flow_attr *attr, const struct rte_flow_item *pattern,
    const struct rte_flow_action actions[] __rte_unused,
    struct rte_flow_error *error __rte_unused, int *device_configured) {
  ...
  mask = (const struct rte_flow_item_sctp *)(pattern->mask
                                                 ? pattern->mask
  ...
  if (!spec || !mc_l4_port_identification) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          &priv->extract.qos_key_extract, DPAA2_FLOW_ITEM_TYPE_GENERIC_IP);
      if (ret) {
        DPAA2_PMD_ERR("QoS Extract IP protocol to discriminate SCTP failed.");
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          ...
          DPAA2_FLOW_ITEM_TYPE_GENERIC_IP);
      if (ret) {
        DPAA2_PMD_ERR("FS Extract IP protocol to discriminate SCTP failed.");
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before SCTP discrimination set failed");
      return -1;
    }
  ...
  if (mask->hdr.src_port) {
    ...
    if (index < 0) {
      ret =
          dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_SCTP,
                                 NH_FLD_SCTP_PORT_SRC, NH_FLD_SCTP_PORT_SIZE);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before SCTP_PORT_SRC set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   &flow->qos_rule, NET_PROT_SCTP,
                                   NH_FLD_SCTP_PORT_SRC, &spec->hdr.src_port,
                                   &mask->hdr.src_port, NH_FLD_SCTP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("QoS NH_FLD_SCTP_PORT_SRC rule data set failed");
      return -1;
    }

    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   &flow->fs_rule, NET_PROT_SCTP,
                                   NH_FLD_SCTP_PORT_SRC, &spec->hdr.src_port,
                                   &mask->hdr.src_port, NH_FLD_SCTP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("FS NH_FLD_SCTP_PORT_SRC rule data set failed");
      return -1;
    }
  }
  ...
  if (mask->hdr.dst_port) {
    ...
    if (index < 0) {
      ret =
          dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_SCTP,
                                 NH_FLD_SCTP_PORT_DST, NH_FLD_SCTP_PORT_SIZE);
      if (ret) {
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move ipaddr before SCTP_PORT_DST set failed");
      return -1;
    }
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
                                   ...
                                   &mask->hdr.dst_port, NH_FLD_SCTP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("QoS NH_FLD_SCTP_PORT_DST rule data set failed");
      return -1;
    }
    ...
    ret = dpaa2_flow_rule_data_set(&priv->extract.tc_key_extract[group],
                                   ...
                                   &mask->hdr.dst_port, NH_FLD_SCTP_PORT_SIZE);
    if (ret) {
      DPAA2_PMD_ERR("FS NH_FLD_SCTP_PORT_DST rule data set failed");
      return -1;
    }
  }
...
static int dpaa2_configure_flow_gre(
    struct rte_flow *flow, struct rte_eth_dev *dev,
    const struct rte_flow_attr *attr, const struct rte_flow_item *pattern,
    const struct rte_flow_action actions[] __rte_unused,
    struct rte_flow_error *error __rte_unused, int *device_configured) {
  ...
  mask = (const struct rte_flow_item_gre *)(pattern->mask
                                                ? pattern->mask
  ...
  if (!spec) {
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          &priv->extract.qos_key_extract, DPAA2_FLOW_ITEM_TYPE_GENERIC_IP);
      if (ret) {
        DPAA2_PMD_ERR("QoS Extract IP protocol to discriminate GRE failed.");
    ...
    if (index < 0) {
      ret = dpaa2_flow_proto_discrimination_extract(
          ...
          DPAA2_FLOW_ITEM_TYPE_GENERIC_IP);
      if (ret) {
        DPAA2_PMD_ERR("FS Extract IP protocol to discriminate GRE failed.");
    ...
    ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
    if (ret) {
      DPAA2_PMD_ERR("Move IP addr before GRE discrimination set failed");
      return -1;
    }
  ...
  if (index < 0) {
    ret = dpaa2_flow_extract_add(&priv->extract.qos_key_extract, NET_PROT_GRE,
                                 NH_FLD_GRE_TYPE, sizeof(rte_be16_t));
    if (ret) {
  ...
  ret = dpaa2_flow_rule_move_ipaddr_tail(flow, priv, group);
  if (ret) {
    DPAA2_PMD_ERR("Move ipaddr before GRE_TYPE set failed");
    return -1;
  }

  ret = dpaa2_flow_rule_data_set(
      &priv->extract.qos_key_extract, &flow->qos_rule, NET_PROT_GRE,
      NH_FLD_GRE_TYPE, &spec->protocol, &mask->protocol, sizeof(rte_be16_t));
  if (ret) {
    DPAA2_PMD_ERR("QoS NH_FLD_GRE_TYPE rule data set failed");
    return -1;
  }

  ret = dpaa2_flow_rule_data_set(
      &priv->extract.tc_key_extract[group], &flow->fs_rule, NET_PROT_GRE,
      NH_FLD_GRE_TYPE, &spec->protocol, &mask->protocol, sizeof(rte_be16_t));
  if (ret) {
    DPAA2_PMD_ERR("FS NH_FLD_GRE_TYPE rule data set failed");
    return -1;
  }
...
static int dpaa2_configure_flow_raw(
    struct rte_flow *flow, struct rte_eth_dev *dev,
    const struct rte_flow_attr *attr, const struct rte_flow_item *pattern,
    const struct rte_flow_action actions[] __rte_unused,
    struct rte_flow_error *error __rte_unused, int *device_configured) {
  ...
  int prev_key_size = priv->extract.qos_key_extract.key_info.key_total_size;
  int local_cfg = 0, ret;
  uint32_t group;
...
static inline int dpaa2_fs_action_supported(enum rte_flow_action_type action) {
  int i;
 ...
 */
static int dpaa2_flow_entry_update(struct dpaa2_dev_priv *priv, uint8_t tc_id) {
  ...
  struct dpaa2_key_extract *qos_key_extract = &priv->extract.qos_key_extract;
  ...
  while (curr) {
    if (curr->ipaddr_rule.ipaddr_type == FLOW_NONE_IPADDR) {
    ...
    if (curr->ipaddr_rule.qos_ipsrc_offset >= 0) {
      RTE_ASSERT(qos_ipsrc_offset >= curr->ipaddr_rule.qos_ipsrc_offset);
      extend1 = qos_ipsrc_offset - curr->ipaddr_rule.qos_ipsrc_offset;
      if (extend >= 0)
    ...
    if (curr->ipaddr_rule.qos_ipdst_offset >= 0) {
      RTE_ASSERT(qos_ipdst_offset >= curr->ipaddr_rule.qos_ipdst_offset);
      extend1 = qos_ipdst_offset - curr->ipaddr_rule.qos_ipdst_offset;
      if (extend >= 0)
        RTE_ASSERT(extend == extend1);
      else
        extend = extend1;
    ...
    if (priv->num_rx_tc > 1) {
      ret = dpni_add_qos_entry(dpni, CMD_PRI_LOW, priv->token, &curr->qos_rule,
                               curr->tc_id, qos_index, 0, 0);
      if (ret) {
    ...
    ret = dpni_remove_fs_entry(dpni, CMD_PRI_LOW, priv->token, curr->tc_id,
                               &curr->fs_rule);
    if (ret) {
    ...
    if (curr->ipaddr_rule.fs_ipsrc_offset >= 0 && tc_id == curr->tc_id) {
      RTE_ASSERT(fs_ipsrc_offset >= curr->ipaddr_rule.fs_ipsrc_offset);
      extend1 = fs_ipsrc_offset - curr->ipaddr_rule.fs_ipsrc_offset;
      if (extend >= 0)
        RTE_ASSERT(extend == extend1);
      else
        extend = extend1;
    ...
    if (curr->ipaddr_rule.fs_ipdst_offset >= 0 && tc_id == curr->tc_id) {
      RTE_ASSERT(fs_ipdst_offset >= curr->ipaddr_rule.fs_ipdst_offset);
      extend1 = fs_ipdst_offset - curr->ipaddr_rule.fs_ipdst_offset;
      if (extend >= 0)
        RTE_ASSERT(extend == extend1);
      else
        extend = extend1;
    ...
    ret = dpni_add_fs_entry(dpni, CMD_PRI_LOW, priv->token, curr->tc_id,
                            curr->tc_index, &curr->fs_rule, &curr->action_cfg);
    if (ret) {
...
static inline int dpaa2_flow_verify_attr(struct dpaa2_dev_priv *priv,
                                         const struct rte_flow_attr *attr) {
  struct rte_flow *curr = LIST_FIRST(&priv->flows);

  while (curr) {
    if (curr->tc_id == attr->group && curr->tc_index == attr->priority) {
      DPAA2_PMD_ERR("Flow with group %d and priority %d already exists.",
...
dpaa2_flow_redirect_dev(struct dpaa2_dev_priv *priv,
  ...
  if (action->type == RTE_FLOW_ACTION_TYPE_PHY_PORT) {
    phy_port = (const struct rte_flow_action_phy_port *)action->conf;
    if (!phy_port->original)
      idx = phy_port->index;
  } else if (action->type == RTE_FLOW_ACTION_TYPE_PORT_ID) {
    port_id = (const struct rte_flow_action_port_id *)action->conf;
    if (!port_id->original)
      idx = port_id->id;
  } else {
    return NULL;
  ...
  while (!end_of_list) {
    ...
    case RTE_FLOW_ACTION_TYPE_QUEUE:
      ...
      if (attr->group != rxq->tc_index) {
        DPAA2_PMD_ERR("RXQ[%d] does not belong to the group %d",
    ...
    case RTE_FLOW_ACTION_TYPE_RSS:
      ...
      if (rss_conf->queue_num > priv->dist_queues) {
        DPAA2_PMD_ERR("RSS number exceeds the distribution size");
        return -ENOTSUP;
      }
      for (i = 0; i < (int)rss_conf->queue_num; i++) {
        if (rss_conf->queue[i] >= priv->nb_rx_queues) {
          DPAA2_PMD_ERR("RSS queue index exceeds the number of RXQs");
          return -ENOTSUP;
        }
        rxq = priv->rx_vq[rss_conf->queue[i]];
        if (rxq->tc_index != attr->group) {
          DPAA2_PMD_ERR("Queue/Group combination are not supported\n");
          return -ENOTSUP;
        }
      }
...
static int dpaa2_generic_flow_set(struct rte_flow *flow,
  ...
  while (!end_of_list) {
    ...
    case RTE_FLOW_ITEM_TYPE_ETH:
      ret = dpaa2_configure_flow_eth(flow, dev, attr, &pattern[i], actions,
                                     error, &is_keycfg_configured);
      if (ret) {
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_VLAN:
      ret = dpaa2_configure_flow_vlan(flow, dev, attr, &pattern[i], actions,
                                      error, &is_keycfg_configured);
      if (ret) {
    ...
    case RTE_FLOW_ITEM_TYPE_IPV4:
    case RTE_FLOW_ITEM_TYPE_IPV6:
      ret = dpaa2_configure_flow_generic_ip(
          flow, dev, attr, &pattern[i], actions, error, &is_keycfg_configured);
      if (ret) {
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_ICMP:
      ret = dpaa2_configure_flow_icmp(flow, dev, attr, &pattern[i], actions,
                                      error, &is_keycfg_configured);
      if (ret) {
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_UDP:
      ret = dpaa2_configure_flow_udp(flow, dev, attr, &pattern[i], actions,
                                     error, &is_keycfg_configured);
      if (ret) {
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_TCP:
      ret = dpaa2_configure_flow_tcp(flow, dev, attr, &pattern[i], actions,
                                     error, &is_keycfg_configured);
      if (ret) {
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_SCTP:
      ret = dpaa2_configure_flow_sctp(flow, dev, attr, &pattern[i], actions,
                                      error, &is_keycfg_configured);
      if (ret) {
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_GRE:
      ret = dpaa2_configure_flow_gre(flow, dev, attr, &pattern[i], actions,
                                     error, &is_keycfg_configured);
      if (ret) {
    ...
    case RTE_FLOW_ITEM_TYPE_RAW:
      ret = dpaa2_configure_flow_raw(flow, dev, attr, &pattern[i], actions,
                                     error, &is_keycfg_configured);
      if (ret) {
  ...
  while (!end_of_list) {
    ...
    case RTE_FLOW_ACTION_TYPE_PORT_ID:
      ...
      if (actions[j].type == RTE_FLOW_ACTION_TYPE_QUEUE) {
        ...
        dest_dev = dpaa2_flow_redirect_dev(priv, &actions[j]);
        if (!dest_dev) {
        ...
        action.options = DPNI_FS_OPT_REDIRECT_TO_DPNI_TX;
      ...
      if (is_keycfg_configured & DPAA2_FS_TABLE_RECONFIGURE) {
        ...
        if (dpkg_prepare_key_cfg(
                ...
                (uint8_t *)(size_t)
                    priv->extract.tc_extract_param[flow->tc_id]) < 0) {
        ...
        if (ret < 0) {
          DPAA2_PMD_ERR("TC hash cannot be disabled.(%d)", ret);
          return -1;
        }
        tc_cfg.enable = true;
        ...
        if (ret < 0) {
          DPAA2_PMD_ERR("TC distribution cannot be configured.(%d)", ret);
          return -1;
        }
      }
      ...
      if (is_keycfg_configured & DPAA2_QOS_TABLE_RECONFIGURE) {
        ...
        if (priv->num_rx_tc > 1) {
          ...
          if (ret < 0) {
            DPAA2_PMD_ERR("RSS QoS table can not be configured(%d)\n", ret);
            return -1;
          }
        }
      }
      ...
      if (flow->ipaddr_rule.ipaddr_type == FLOW_IPV4_ADDR) {
        if (flow->ipaddr_rule.qos_ipdst_offset >=
            flow->ipaddr_rule.qos_ipsrc_offset) {
          flow->qos_real_key_size =
              flow->ipaddr_rule.qos_ipdst_offset + NH_FLD_IPV4_ADDR_SIZE;
        } else {
          flow->qos_real_key_size =
              flow->ipaddr_rule.qos_ipsrc_offset + NH_FLD_IPV4_ADDR_SIZE;
        }
      } else if (flow->ipaddr_rule.ipaddr_type == FLOW_IPV6_ADDR) {
        if (flow->ipaddr_rule.qos_ipdst_offset >=
            flow->ipaddr_rule.qos_ipsrc_offset) {
          flow->qos_real_key_size =
              flow->ipaddr_rule.qos_ipdst_offset + NH_FLD_IPV6_ADDR_SIZE;
        } else {
          flow->qos_real_key_size =
              flow->ipaddr_rule.qos_ipsrc_offset + NH_FLD_IPV6_ADDR_SIZE;
        }
      }
      ...
      if (priv->num_rx_tc > 1) {
        ...
        ret = dpni_add_qos_entry(dpni, CMD_PRI_LOW, priv->token,
                                 &flow->qos_rule, flow->tc_id, qos_index, 0, 0);
        if (ret < 0) {
          DPAA2_PMD_ERR("Error in adding entry to QoS table(%d)", ret);
          return ret;
        }
      }

      if (flow->tc_index >= priv->fs_entries) {
        DPAA2_PMD_ERR("FS table with %d entries full", priv->fs_entries);
        return -1;
      }

      flow->fs_real_key_size =
          priv->extract.tc_key_extract[flow->tc_id].key_info.key_total_size;

      if (flow->ipaddr_rule.ipaddr_type == FLOW_IPV4_ADDR) {
        if (flow->ipaddr_rule.fs_ipdst_offset >=
            flow->ipaddr_rule.fs_ipsrc_offset) {
          flow->fs_real_key_size =
              flow->ipaddr_rule.fs_ipdst_offset + NH_FLD_IPV4_ADDR_SIZE;
        } else {
          flow->fs_real_key_size =
              flow->ipaddr_rule.fs_ipsrc_offset + NH_FLD_IPV4_ADDR_SIZE;
        }
      } else if (flow->ipaddr_rule.ipaddr_type == FLOW_IPV6_ADDR) {
        if (flow->ipaddr_rule.fs_ipdst_offset >=
            flow->ipaddr_rule.fs_ipsrc_offset) {
          flow->fs_real_key_size =
              flow->ipaddr_rule.fs_ipdst_offset + NH_FLD_IPV6_ADDR_SIZE;
        } else {
          flow->fs_real_key_size =
              flow->ipaddr_rule.fs_ipsrc_offset + NH_FLD_IPV6_ADDR_SIZE;
        }
      }
      ...
      ret = dpni_add_fs_entry(dpni, CMD_PRI_LOW, priv->token, flow->tc_id,
                              flow->tc_index, &flow->fs_rule, &action);
      if (ret < 0) {
        DPAA2_PMD_ERR("Error in adding entry to FS table(%d)", ret);
        return ret;
      }
      memcpy(&flow->action_cfg, &action, sizeof(struct dpni_fs_action_cfg));
      break;
    case RTE_FLOW_ACTION_TYPE_RSS:
      rss_conf = (const struct rte_flow_action_rss *)(actions[j].conf);

      flow->action = RTE_FLOW_ACTION_TYPE_RSS;
      ret = dpaa2_distset_to_dpkg_profile_cfg(
          rss_conf->types, &priv->extract.tc_key_extract[flow->tc_id].dpkg);
      if (ret < 0) {
        DPAA2_PMD_ERR(
      ...
      if (dpkg_prepare_key_cfg(&priv->extract.tc_key_extract[flow->tc_id].dpkg,
                               (uint8_t *)param) < 0) {
        DPAA2_PMD_ERR("Unable to prepare extract parameters");
        rte_free((void *)param);
        return -1;
      ...
      ret = dpni_set_rx_hash_dist(dpni, CMD_PRI_LOW, priv->token, &tc_cfg);
      if (ret < 0) {
        DPAA2_PMD_ERR("RSS TC table cannot be configured: %d\n", ret);
        rte_free((void *)param);
        return -1;
      }
      ...
      if (is_keycfg_configured & DPAA2_QOS_TABLE_RECONFIGURE) {
        ...
        ret = dpni_set_qos_table(dpni, CMD_PRI_LOW, priv->token, &qos_cfg);
        if (ret < 0) {
          DPAA2_PMD_ERR("RSS QoS dist can't be configured-%d\n", ret);
          return -1;
        }
      }
      ...
      flow->qos_real_key_size =
          priv->extract.qos_key_extract.key_info.key_total_size;
      flow->qos_rule.key_size = FIXED_ENTRY_SIZE;
      ret = dpni_add_qos_entry(dpni, CMD_PRI_LOW, priv->token, &flow->qos_rule,
                               flow->tc_id, qos_index, 0, 0);
      if (ret < 0) {
        DPAA2_PMD_ERR("Error in entry addition in QoS table(%d)", ret);
        return ret;
      }
      break;
    case RTE_FLOW_ACTION_TYPE_END:
  ...
  if (!ret) {
    if (is_keycfg_configured &
        (DPAA2_QOS_TABLE_RECONFIGURE | DPAA2_FS_TABLE_RECONFIGURE)) {
...
static inline int dpaa2_dev_verify_attr(struct dpni_attr *dpni_attr,
                                        const struct rte_flow_attr *attr) {
  int ret = 0;
  ...
  if (unlikely(attr->egress)) {
    DPAA2_PMD_ERR("Flow configuration is not supported on egress side\n");
    ret = -ENOTSUP;
  }
...
static int dpaa2_flow_validate(struct rte_eth_dev *dev,
  ...
  if (ret < 0) {
    DPAA2_PMD_ERR("Failure to get dpni@%p attribute, err code  %d\n", dpni,
                  ret);
    rte_flow_error_set(error, EPERM, RTE_FLOW_ERROR_TYPE_ATTR, flow_attr,
                       "invalid");
    return ret;
  }
  ...
  if (ret < 0) {
    ...
    rte_flow_error_set(error, EPERM, RTE_FLOW_ERROR_TYPE_ITEM, pattern,
                       "invalid");
    goto not_valid_params;
  }
  ...
  if (ret < 0) {
    ...
    rte_flow_error_set(error, EPERM, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                       "invalid");
    goto not_valid_params;
  }
not_valid_params:
...
dpaa2_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
  ...
  if (getenv("DPAA2_FLOW_CONTROL_MISS_FLOW")) {
    ...
    dpaa2_flow_miss_flow_id = atoi(getenv("DPAA2_FLOW_CONTROL_MISS_FLOW"));
    if (dpaa2_flow_miss_flow_id >= priv->dist_queues) {
      DPAA2_PMD_ERR("The missed flow ID %d exceeds the max flow ID %d",
                    dpaa2_flow_miss_flow_id, priv->dist_queues - 1);
      return NULL;
    }
  }
  ...
  if (!key_iova) {
    DPAA2_PMD_ERR("Memory allocation failure for rule configuration\n");
    goto mem_failure;
  }
  ...
  flow->ipaddr_rule.qos_ipsrc_offset = IP_ADDRESS_OFFSET_INVALID;
  flow->ipaddr_rule.qos_ipdst_offset = IP_ADDRESS_OFFSET_INVALID;
  flow->ipaddr_rule.fs_ipsrc_offset = IP_ADDRESS_OFFSET_INVALID;
  flow->ipaddr_rule.fs_ipdst_offset = IP_ADDRESS_OFFSET_INVALID;

  ret = dpaa2_generic_flow_set(flow, dev, attr, pattern, actions, error);
  if (ret < 0) {
    if (error && error->type > RTE_FLOW_ERROR_TYPE_ACTION)
      rte_flow_error_set(error, EPERM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, attr,
                         "unknown");
...
static int dpaa2_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                              struct rte_flow_error *error) {
  int ret = 0;
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
  struct fsl_mc_io *dpni = (struct fsl_mc_io *)priv->hw;
  ...
  case RTE_FLOW_ACTION_TYPE_PORT_ID:
    if (priv->num_rx_tc > 1) {
      ...
      ret = dpni_remove_qos_entry(dpni, CMD_PRI_LOW, priv->token,
                                  &flow->qos_rule);
      if (ret < 0) {
        DPAA2_PMD_ERR("Error in removing entry from QoS table(%d)", ret);
        goto error;
      }
    }

    /* Then remove entry from FS table */
    ret = dpni_remove_fs_entry(dpni, CMD_PRI_LOW, priv->token, flow->tc_id,
                               &flow->fs_rule);
    if (ret < 0) {
      DPAA2_PMD_ERR("Error in removing entry from FS table(%d)", ret);
      goto error;
    }
    break;
  case RTE_FLOW_ACTION_TYPE_RSS:
    ...
    break;
  default:
    DPAA2_PMD_ERR("Action type (%d) is not supported", flow->action);
    ret = -ENOTSUP;
    break;
  }
 ...
 * @see rte_flow_ops
 */
static int dpaa2_flow_flush(struct rte_eth_dev *dev,
                            struct rte_flow_error *error) {
  struct dpaa2_dev_priv *priv = dev->data->dev_private;
...
static int dpaa2_flow_query(struct rte_eth_dev *dev __rte_unused,
