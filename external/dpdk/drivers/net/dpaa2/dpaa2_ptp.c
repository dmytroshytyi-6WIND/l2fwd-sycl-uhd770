...
int dpaa2_timesync_enable(struct rte_eth_dev *dev __rte_unused) { return 0; }

int dpaa2_timesync_disable(struct rte_eth_dev *dev __rte_unused) { return 0; }
...
int dpaa2_timesync_read_time(struct rte_eth_dev *dev,
  ...
  ret = dprtc_get_time(&dprtc_dev->dprtc, CMD_PRI_LOW, dprtc_dev->token, &ns);
  if (ret) {
...
int dpaa2_timesync_write_time(struct rte_eth_dev *dev,
  ...
  ret = dprtc_set_time(&dprtc_dev->dprtc, CMD_PRI_LOW, dprtc_dev->token, ns);
  if (ret) {
...
dpaa2_create_dprtc_device(int vdev_fd __rte_unused,
  ...
  ret = dprtc_open(&dprtc_dev->dprtc, CMD_PRI_LOW, dprtc_id, &dprtc_dev->token);
  if (ret) {
    DPAA2_PMD_ERR("Unable to open dprtc object: err(%d)", ret);
    goto init_err;
  }

  ret = dprtc_get_attributes(&dprtc_dev->dprtc, CMD_PRI_LOW, dprtc_dev->token,
                             &attr);
  if (ret) {
