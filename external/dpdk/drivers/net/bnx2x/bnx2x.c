...
static inline const char *bnx2x_pmd_version(void) {
  static char version[32];

  snprintf(version, sizeof(version), "%s %s_%d.%d.%d.%d", BNX2X_PMD_VER_PREFIX,
           BNX2X_DRIVER_VERSION, BNX2X_PMD_VERSION_MAJOR,
           BNX2X_PMD_VERSION_MINOR, BNX2X_PMD_VERSION_REVISION,
...
static LIST_HEAD(, bnx2x_prev_list_node)
    bnx2x_prev_list = LIST_HEAD_INITIALIZER(bnx2x_prev_list);
...
static void bnx2x_update_rx_prod(struct bnx2x_softc *sc,
                                 struct bnx2x_fastpath *fp, uint16_t rx_bd_prod,
                                 uint16_t rx_cq_prod);
...
static __rte_noinline int bnx2x_nic_load(struct bnx2x_softc *sc);
...
int bnx2x_dma_alloc(struct bnx2x_softc *sc, size_t size, struct bnx2x_dma *dma,
  ...
  z = rte_memzone_reserve_aligned(mz_name, (uint64_t)size, SOCKET_ID_ANY,
  ...
  PMD_DRV_LOG(DEBUG, sc, "%s: virt=%p phys=%" PRIx64, msg, dma->vaddr,
              dma->paddr);
...
static int bnx2x_acquire_hw_lock(struct bnx2x_softc *sc, uint32_t resource) {
  ...
  if (resource > HW_LOCK_MAX_RESOURCE_VALUE) {
    PMD_DRV_LOG(NOTICE, sc, "resource 0x%x > HW_LOCK_MAX_RESOURCE_VALUE",
                resource);
    return -1;
  }
  ...
  if (lock_status & resource_bit) {
    PMD_DRV_LOG(NOTICE, sc, "resource in use (status 0x%x bit 0x%x)",
...
static int bnx2x_release_hw_lock(struct bnx2x_softc *sc, uint32_t resource) {
  ...
  lock_status = REG_RD(sc, hw_lock_control_reg);
  if (!(lock_status & resource_bit)) {
    PMD_DRV_LOG(NOTICE, sc, "resource not in use (status 0x%x bit 0x%x)",
                lock_status, resource_bit);
    return -1;
  }
...
void bnx2x_post_dmae(struct bnx2x_softc *sc, struct dmae_command *dmae,
                     int idx) {
  uint32_t cmd_offset;
  uint32_t i;
...
uint32_t bnx2x_dmae_opcode_add_comp(uint32_t opcode, uint8_t comp_type) {
  return opcode |
         ((comp_type << DMAE_COMMAND_C_DST_SHIFT) | DMAE_COMMAND_C_TYPE_ENABLE);
}
...
static void bnx2x_prep_dmae_with_comp(struct bnx2x_softc *sc,
                                      struct dmae_command *dmae,
                                      uint8_t src_type, uint8_t dst_type) {
  memset(dmae, 0, sizeof(struct dmae_command));

  /* set the opcode */
  dmae->opcode = bnx2x_dmae_opcode(sc, src_type, dst_type, TRUE, DMAE_COMP_PCI);
...
static int bnx2x_issue_dmae_with_comp(struct bnx2x_softc *sc,
                                      struct dmae_command *dmae) {
  ...
  *wb_comp = 0;
  ...
  while ((*wb_comp & ~DMAE_PCI_ERR_FLAG) != DMAE_COMP_VAL) {
    if (!timeout || (sc->recovery_state != BNX2X_RECOVERY_DONE &&
...
static void bnx2x_write_dmae_phys_len(struct bnx2x_softc *sc,
                                      rte_iova_t phys_addr, uint32_t addr,
                                      uint32_t len) {
...
void bnx2x_set_ctx_validation(struct bnx2x_softc *sc, struct eth_context *cxt,
  ...
  cxt->ustorm_ag_context.cdu_usage = CDU_RSRVD_VALUE_TYPE_A(
      HW_CID(sc, cid), CDU_REGION_NUMBER_UCM_AG, ETH_CONNECTION_TYPE);
  /* xcontext validation */
  cxt->xstorm_ag_context.cdu_reserved = CDU_RSRVD_VALUE_TYPE_A(
      HW_CID(sc, cid), CDU_REGION_NUMBER_XCM_AG, ETH_CONNECTION_TYPE);
}

static void bnx2x_storm_memset_hc_timeout(struct bnx2x_softc *sc,
...
static void bnx2x_storm_memset_hc_disable(struct bnx2x_softc *sc,
...
void elink_cb_reg_write(struct bnx2x_softc *sc, uint32_t reg_addr,
                        uint32_t val) {
  REG_WR(sc, reg_addr, val);
}

void elink_cb_event_log(__rte_unused struct bnx2x_softc *sc,
...
static int bnx2x_gpio_read(struct bnx2x_softc *sc, int gpio_num, uint8_t port) {
  ...
  int gpio_port =
      ((REG_RD(sc, NIG_REG_PORT_SWAP) && REG_RD(sc, NIG_REG_STRAP_OVERRIDE)) ^
       port);
...
static int bnx2x_gpio_write(struct bnx2x_softc *sc, int gpio_num, uint32_t mode,
                            uint8_t port) {
  /* The GPIO should be swapped if swap register is set and active */
  int gpio_port =
      ((REG_RD(sc, NIG_REG_PORT_SWAP) && REG_RD(sc, NIG_REG_STRAP_OVERRIDE)) ^
...
static int bnx2x_gpio_mult_write(struct bnx2x_softc *sc, uint8_t pins,
                                 uint32_t mode) {
  uint32_t gpio_reg;
  ...
  default:
    PMD_DRV_LOG(NOTICE, sc, "Invalid GPIO mode assignment %d", mode);
    bnx2x_release_hw_lock(sc, HW_LOCK_RESOURCE_GPIO);
    return -1;
  }
...
static int bnx2x_gpio_int_write(struct bnx2x_softc *sc, int gpio_num,
                                uint32_t mode, uint8_t port) {
  /* The GPIO should be swapped if swap register is set and active */
  int gpio_port =
      ((REG_RD(sc, NIG_REG_PORT_SWAP) && REG_RD(sc, NIG_REG_STRAP_OVERRIDE)) ^
       port);
...
uint32_t elink_cb_gpio_read(struct bnx2x_softc *sc, uint16_t gpio_num,
                            uint8_t port) {
  return bnx2x_gpio_read(sc, gpio_num, port);
}

uint8_t elink_cb_gpio_write(struct bnx2x_softc *sc, uint16_t gpio_num,
                            uint8_t mode, /* 0=low 1=high */
                            uint8_t port) {
  return bnx2x_gpio_write(sc, gpio_num, mode, port);
}

uint8_t elink_cb_gpio_mult_write(struct bnx2x_softc *sc, uint8_t pins,
                                 uint8_t mode /* 0=low 1=high */) {
  return bnx2x_gpio_mult_write(sc, pins, mode);
}

uint8_t elink_cb_gpio_int_write(struct bnx2x_softc *sc, uint16_t gpio_num,
                                uint8_t mode, /* 0=low 1=high */
                                uint8_t port) {
...
void elink_cb_notify_link_changed(struct bnx2x_softc *sc) {
  REG_WR(sc, (MISC_REG_AEU_GENERAL_ATTN_12 + (SC_FUNC(sc) * sizeof(uint32_t))),
         1);
}

/* send the MCP a request, block until there is a reply */
uint32_t elink_cb_fw_command(struct bnx2x_softc *sc, uint32_t command,
                             uint32_t param) {
  ...
  PMD_DRV_LOG(DEBUG, sc, "wrote command 0x%08x to FW MB param 0x%08x",
              (command | seq), param);
...
static uint32_t bnx2x_fw_command(struct bnx2x_softc *sc, uint32_t command,
                                 uint32_t param) {
  return elink_cb_fw_command(sc, command, param);
}

static void __storm_memset_dma_mapping(struct bnx2x_softc *sc, uint32_t addr,
                                       rte_iova_t mapping) {
  REG_WR(sc, addr, U64_LO(mapping));
  REG_WR(sc, (addr + 4), U64_HI(mapping));
}

static void storm_memset_spq_addr(struct bnx2x_softc *sc, rte_iova_t mapping,
                                  uint16_t abs_fid) {
  uint32_t addr = (XSEM_REG_FAST_MEMORY + XSTORM_SPQ_PAGE_BASE_OFFSET(abs_fid));
  __storm_memset_dma_mapping(sc, addr, mapping);
}

static void storm_memset_vf_to_pf(struct bnx2x_softc *sc, uint16_t abs_fid,
                                  uint16_t pf_id) {
...
static void storm_memset_func_en(struct bnx2x_softc *sc, uint16_t abs_fid,
                                 uint8_t enable) {
...
static void storm_memset_eq_data(struct bnx2x_softc *sc,
...
static void storm_memset_eq_prod(struct bnx2x_softc *sc, uint16_t eq_prod,
                                 uint16_t pfid) {
  uint32_t addr = (BAR_CSTRORM_INTMEM + CSTORM_EVENT_RING_PROD_OFFSET(pfid));
...
int bnx2x_sp_post(struct bnx2x_softc *sc, int command, int cid,
  ...
  type |= ((SC_FUNC(sc) << SPE_HDR_FUNCTION_ID_SHIFT) & SPE_HDR_FUNCTION_ID);
  ...
  PMD_DRV_LOG(DEBUG, sc,
              ...
              sc->spq_prod_idx, (uint32_t)U64_HI(sc->spq_dma.paddr),
              (uint32_t)(U64_LO(sc->spq_dma.paddr) +
                         (uint8_t *)sc->spq_prod_bd - (uint8_t *)sc->spq),
              command, common, HW_CID(sc, cid), data_hi, data_lo, type,
...
static void bnx2x_drv_pulse(struct bnx2x_softc *sc) {
  SHMEM_WR(sc, func_mb[SC_FW_MB_IDX(sc)].drv_pulse_mb, sc->fw_drv_pulse_wr_seq);
}
...
static int bnx2x_has_rx_work(struct bnx2x_fastpath *fp) {
  ...
  if (unlikely((rx_cq_cons_sb & MAX_RCQ_ENTRIES(rxq)) == MAX_RCQ_ENTRIES(rxq)))
    rx_cq_cons_sb++;

  PMD_RX_LOG(DEBUG, "hw CQ cons = %d, sw CQ cons = %d", rx_cq_cons_sb,
             rxq->rx_cq_head);

  return rxq->rx_cq_head != rx_cq_cons_sb;
}

static void bnx2x_sp_event(struct bnx2x_softc *sc, struct bnx2x_fastpath *fp,
  ...
  PMD_DRV_LOG(DEBUG, sc, "fp=%d cid=%d got ramrod #%d state is %x type is %d",
  ...
  case (RAMROD_CMD_ID_ETH_TX_QUEUE_SETUP):
    PMD_DRV_LOG(DEBUG, sc, "got MULTI[%d] tx-only setup ramrod", cid);
  ...
  if ((drv_cmd != ECORE_Q_CMD_MAX) && q_obj->complete_cmd(sc, q_obj, drv_cmd)) {
    /*
...
static uint8_t bnx2x_rxeof(struct bnx2x_softc *sc, struct bnx2x_fastpath *fp) {
  ...
  while (sw_cq_cons != hw_cq_cons) {
    ...
    if (unlikely(cqe_fp_flags & ETH_FAST_PATH_RX_CQE_PHY_DECODE_ERR_FLG)) {
      PMD_RX_LOG(DEBUG, "flags 0x%x rx packet %u", cqe_fp_flags, sw_cq_cons);
      goto next_rx;
    }
  ...
  PMD_RX_LOG(DEBUG, "BD prod = %d, sw CQ prod = %d", bd_prod_fw, sw_cq_prod);
...
static uint16_t bnx2x_free_tx_pkt(__rte_unused struct bnx2x_fastpath *fp,
                                  struct bnx2x_tx_queue *txq, uint16_t pkt_idx,
                                  uint16_t bd_idx) {
  ...
  if (likely(tx_mbuf != NULL)) {
    ...
    PMD_RX_LOG(ERR, "fp[%02d] lost mbuf %lu", fp->index,
               (unsigned long)TX_BD(pkt_idx, txq));
  }
...
uint8_t bnx2x_txeof(__rte_unused struct bnx2x_softc *sc,
  ...
  PMD_TX_LOG(DEBUG,
             ...
             fp->index, tx_bd_avail, hw_cons, txq->tx_pkt_head,
             txq->tx_pkt_tail, txq->tx_bd_head, txq->tx_bd_tail);
  return TRUE;
}
...
static int bnx2x_del_all_macs(struct bnx2x_softc *sc,
                              struct ecore_vlan_mac_obj *mac_obj, int mac_type,
...
static int bnx2x_fill_accept_flags(struct bnx2x_softc *sc, uint32_t rx_mode,
  ...
  *tx_accept_flags = 0;
  ...
  case BNX2X_RX_MODE_ALLMULTI:
    rte_bit_relaxed_set32(ECORE_ACCEPT_UNICAST, rx_accept_flags);
    rte_bit_relaxed_set32(ECORE_ACCEPT_ALL_MULTICAST, rx_accept_flags);
    rte_bit_relaxed_set32(ECORE_ACCEPT_BROADCAST, rx_accept_flags);

    /* internal switching mode */
    rte_bit_relaxed_set32(ECORE_ACCEPT_UNICAST, tx_accept_flags);
    rte_bit_relaxed_set32(ECORE_ACCEPT_ALL_MULTICAST, tx_accept_flags);
    rte_bit_relaxed_set32(ECORE_ACCEPT_BROADCAST, tx_accept_flags);
  ...
  case BNX2X_RX_MODE_PROMISC:
    ...
    if (IS_MF_SI(sc)) {
      rte_bit_relaxed_set32(ECORE_ACCEPT_ALL_UNICAST, tx_accept_flags);
    } else {
      rte_bit_relaxed_set32(ECORE_ACCEPT_UNICAST, tx_accept_flags);
    }
...
static int bnx2x_set_q_rx_mode(struct bnx2x_softc *sc, uint8_t cl_id,
  ...
  ramrod_param.rdata_mapping = (rte_iova_t)BNX2X_SP_MAPPING(sc, rx_mode_rdata),
...
int bnx2x_set_storm_rx_mode(struct bnx2x_softc *sc) {
  ...
  return bnx2x_set_q_rx_mode(sc, sc->fp[0].cl_id, rx_mode_flags,
                             rx_accept_flags, tx_accept_flags, ramrod_flags);
}
...
static int bnx2x_nic_load_no_mcp(struct bnx2x_softc *sc) {
  ...
  PMD_DRV_LOG(INFO, sc, "NO MCP - load counts[%d]      %d, %d, %d", path,
              load_count[path][0], load_count[path][1], load_count[path][2]);

  load_count[path][0]++;
  load_count[path][1 + port]++;
  PMD_DRV_LOG(INFO, sc, "NO MCP - new load counts[%d]  %d, %d, %d", path,
              load_count[path][0], load_count[path][1], load_count[path][2]);
...
static void bnx2x_send_unload_done(struct bnx2x_softc *sc, uint8_t keep_link) {
  uint32_t reset_param = keep_link ? DRV_MSG_CODE_UNLOAD_SKIP_LINK_RESET : 0;
...
static int bnx2x_func_wait_started(struct bnx2x_softc *sc) {
  ...
  while (ecore_func_get_state(sc, &sc->func_obj) != ECORE_F_STATE_STARTED &&
         tout--) {
    DELAY(20000);
  }
  ...
  if (ecore_func_get_state(sc, &sc->func_obj) != ECORE_F_STATE_STARTED) {
    ...
    rte_bit_relaxed_set32(RAMROD_DRV_CLR_ONLY, &func_params.ramrod_flags);
...
static void bnx2x_chip_cleanup(struct bnx2x_softc *sc, uint32_t unload_mode,
                               uint8_t keep_link) {
  int port = SC_PORT(sc);
  ...
  /* Clean all ETH MACs */
  rc = bnx2x_del_all_macs(sc, &sc->sp_objs[0].mac_obj, ECORE_ETH_MAC, FALSE);
  if (rc < 0) {
    PMD_DRV_LOG(NOTICE, sc, "Failed to delete all ETH MACs (%d)", rc);
  }

  /* Clean up UC list  */
  rc = bnx2x_del_all_macs(sc, &sc->sp_objs[0].mac_obj, ECORE_UC_LIST_MAC, TRUE);
  if (rc < 0) {
    PMD_DRV_LOG(NOTICE, sc, "Failed to delete UC MACs list (%d)", rc);
  }
  ...
  if (rte_bit_relaxed_get32(ECORE_FILTER_RX_MODE_PENDING, &sc->sp_state))
    rte_bit_relaxed_set32(ECORE_FILTER_RX_MODE_SCHED, &sc->sp_state);
  else
  ...
  if (rc < 0) {
    PMD_DRV_LOG(NOTICE, sc, "Failed to send DEL MCAST command (%d)", rc);
  }
...
static void bnx2x_squeeze_objects(struct bnx2x_softc *sc) {
  ...
  rc = mac_obj->delete_all(sc, mac_obj, &vlan_mac_flags, &ramrod_flags);
  if (rc != 0) {
    PMD_DRV_LOG(NOTICE, sc, "Failed to clean UC list MACs (%d)", rc);
  }
  ...
  while (rc != 0) {
    if (rc < 0) {
      PMD_DRV_LOG(NOTICE, sc, "Failed to clean MCAST object (%d)", rc);
      return;
    }
...
/* stop the controller */
__rte_noinline int bnx2x_nic_unload(struct bnx2x_softc *sc,
                                    uint32_t unload_mode, uint8_t keep_link) {
...
int bnx2x_tx_encap(struct bnx2x_tx_queue *txq, struct rte_mbuf *m0) {
  ...
  tx_start_bd->addr_lo = rte_cpu_to_le_32(U64_LO(rte_mbuf_data_iova(m0)));
  tx_start_bd->addr_hi = rte_cpu_to_le_32(U64_HI(rte_mbuf_data_iova(m0)));
  ...
  if (m0->ol_flags & RTE_MBUF_F_TX_VLAN) {
    ...
    if (IS_PF(sc))
      tx_start_bd->vlan_or_ethertype = rte_cpu_to_le_16(pkt_prod);
    else {
      /* when transmitting in a vf, start bd
       * must hold the ethertype for fw to enforce it
       */
      struct rte_ether_hdr *eh = rte_pktmbuf_mtod(m0, struct rte_ether_hdr *);

      /* Still need to consider inband vlan for enforced */
      if (eh->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_VLAN)) {
        struct rte_vlan_hdr *vh = (struct rte_vlan_hdr *)(eh + 1);
        tx_start_bd->bd_flags.as_bitfield |=
            (X_ETH_INBAND_VLAN << ETH_TX_BD_FLAGS_VLAN_MODE_SHIFT);
        tx_start_bd->vlan_or_ethertype = rte_cpu_to_le_16(ntohs(vh->vlan_tci));
      } else {
  ...
  if (IS_VF(sc)) {
    ...
    tx_parse_bd = &txq->tx_ring[TX_BD(bd_prod, txq)].parse_bd_e2;
    ...
    rte_memcpy(&tx_parse_bd->data.mac_addr.dst_hi, &eh->dst_addr.addr_bytes[0],
               2);
    rte_memcpy(&tx_parse_bd->data.mac_addr.dst_mid, &eh->dst_addr.addr_bytes[2],
               2);
    rte_memcpy(&tx_parse_bd->data.mac_addr.dst_lo, &eh->dst_addr.addr_bytes[4],
               2);
    rte_memcpy(&tx_parse_bd->data.mac_addr.src_hi, &eh->src_addr.addr_bytes[0],
               2);
    rte_memcpy(&tx_parse_bd->data.mac_addr.src_mid, &eh->src_addr.addr_bytes[2],
               2);
    rte_memcpy(&tx_parse_bd->data.mac_addr.src_lo, &eh->src_addr.addr_bytes[4],
               2);

    tx_parse_bd->data.mac_addr.dst_hi =
        rte_cpu_to_be_16(tx_parse_bd->data.mac_addr.dst_hi);
    tx_parse_bd->data.mac_addr.dst_mid =
        rte_cpu_to_be_16(tx_parse_bd->data.mac_addr.dst_mid);
    ...
    tx_parse_bd->data.mac_addr.src_mid =
        rte_cpu_to_be_16(tx_parse_bd->data.mac_addr.src_mid);
    tx_parse_bd->data.mac_addr.src_lo =
        rte_cpu_to_be_16(tx_parse_bd->data.mac_addr.src_lo);

    PMD_TX_LOG(
        DEBUG, "PBD dst %x %x %x src %x %x %x p_data %x",
        tx_parse_bd->data.mac_addr.dst_hi, tx_parse_bd->data.mac_addr.dst_mid,
        tx_parse_bd->data.mac_addr.dst_lo, tx_parse_bd->data.mac_addr.src_hi,
        tx_parse_bd->data.mac_addr.src_mid, tx_parse_bd->data.mac_addr.src_lo,
        tx_parse_bd->parsing_data);
  }

  PMD_TX_LOG(DEBUG, "start bd: nbytes %d flags %x vlan %x", tx_start_bd->nbytes,
             tx_start_bd->bd_flags.as_bitfield, tx_start_bd->vlan_or_ethertype);
...
static void bnx2x_ilt_set_info(struct bnx2x_softc *sc) {
  ...
  if (QM_INIT(sc->qm_cid_count)) {
    ...
    line +=
        DIV_ROUND_UP(sc->qm_cid_count * QM_QUEUES_PER_FUNC * 4, QM_ILT_PAGE_SZ);
...
static int bnx2x_alloc_ilt_lines_mem(struct bnx2x_softc *sc) {
  sc->ilt->lines = rte_calloc("", sizeof(struct ilt_line), ILT_MAX_LINES,
                              RTE_CACHE_LINE_SIZE);
...
static int bnx2x_alloc_mem(struct bnx2x_softc *sc) {
  ...
  for (i = 0, allocated = 0; allocated < context_size; i++) {
    sc->context[i].size = min(CDU_ILT_PAGE_SZ, (context_size - allocated));

    snprintf(cdu_name, sizeof(cdu_name), "cdu_%d", i);
    if (bnx2x_dma_alloc(sc, sc->context[i].size, &sc->context[i].vcxt_dma,
                        cdu_name, BNX2X_PAGE_SIZE) != 0) {
      bnx2x_free_mem(sc);
      return -1;
    }

    sc->context[i].vcxt = (union cdu_context *)sc->context[i].vcxt_dma.vaddr;
...
static int bnx2x_alloc_fw_stats_mem(struct bnx2x_softc *sc) {
  ...
  sc->fw_stats_req_size = (sizeof(struct stats_query_header) +
  ...
  sc->fw_stats_data_size =
      (sizeof(struct stats_counter) + sizeof(struct per_port_stats) +
  ...
  sc->fw_stats_data_mapping = (sc->fw_stats_dma.paddr + sc->fw_stats_req_size);
...
static void bnx2x_set_reset_done(struct bnx2x_softc *sc) {
  uint32_t val;
  uint32_t bit =
      SC_PATH(sc) ? BNX2X_PATH1_RST_IN_PROG_BIT : BNX2X_PATH0_RST_IN_PROG_BIT;
...
static uint8_t bnx2x_reset_is_done(struct bnx2x_softc *sc, int engine) {
  ...
  uint32_t bit =
      engine ? BNX2X_PATH1_RST_IN_PROG_BIT : BNX2X_PATH0_RST_IN_PROG_BIT;
...
static uint8_t bnx2x_get_load_status(struct bnx2x_softc *sc, int engine) {
  uint32_t mask =
      engine ? BNX2X_PATH1_LOAD_CNT_MASK : BNX2X_PATH0_LOAD_CNT_MASK;
  uint32_t shift =
      engine ? BNX2X_PATH1_LOAD_CNT_SHIFT : BNX2X_PATH0_LOAD_CNT_SHIFT;
  uint32_t val = REG_RD(sc, BNX2X_RECOVERY_GLOB_REG);
...
static void bnx2x_set_pf_load(struct bnx2x_softc *sc) {
  ...
  uint32_t val1;
  uint32_t mask =
      SC_PATH(sc) ? BNX2X_PATH1_LOAD_CNT_MASK : BNX2X_PATH0_LOAD_CNT_MASK;
  uint32_t shift =
      SC_PATH(sc) ? BNX2X_PATH1_LOAD_CNT_SHIFT : BNX2X_PATH0_LOAD_CNT_SHIFT;
...
static int bnx2x_nic_load_request(struct bnx2x_softc *sc, uint32_t *load_code) {
  ...
  /* init fw_seq */
  sc->fw_seq = (SHMEM_RD(sc, func_mb[SC_FW_MB_IDX(sc)].drv_mb_header) &
 ...
 */
static int bnx2x_nic_load_analyze_req(struct bnx2x_softc *sc,
                                      uint32_t load_code) {
  ...
  if ((load_code != FW_MSG_CODE_DRV_LOAD_COMMON_CHIP) &&
    ...
    my_fw = (BNX2X_5710_FW_MAJOR_VERSION + (BNX2X_5710_FW_MINOR_VERSION << 8) +
             (BNX2X_5710_FW_REVISION_VERSION << 16) +
             (BNX2X_5710_FW_ENGINEERING_VERSION << 24));

    /* read loaded FW from chip */
    loaded_fw = REG_RD(sc, XSEM_REG_PRAM);
    PMD_DRV_LOG(DEBUG, sc, "loaded FW 0x%08x / my FW 0x%08x", loaded_fw, my_fw);

    /* abort nic load if version mismatch */
    if (my_fw != loaded_fw) {
      PMD_DRV_LOG(NOTICE, sc, "FW 0x%08x already loaded (mine is 0x%08x)",
                  loaded_fw, my_fw);
      return -1;
    }
  }
...
static void bnx2x_link_attn(struct bnx2x_softc *sc) {
  ...
  if (sc->link_vars.link_up) {
    ...
    if (sc->dropless_fc) {
      ...
      REG_WR(
          sc,
          (BAR_USTRORM_INTMEM + USTORM_ETH_PAUSE_ENABLED_OFFSET(SC_PORT(sc))),
          pause_enabled);
    }
...
static void bnx2x_attn_int_asserted(struct bnx2x_softc *sc, uint32_t asserted) {
  int port = SC_PORT(sc);
  uint32_t aeu_addr =
      port ? MISC_REG_AEU_MASK_ATTN_FUNC_1 : MISC_REG_AEU_MASK_ATTN_FUNC_0;
  uint32_t nig_int_mask_addr =
      port ? NIG_REG_MASK_INTERRUPT_PORT1 : NIG_REG_MASK_INTERRUPT_PORT0;
  uint32_t aeu_mask;
  uint32_t nig_mask = 0;
  uint32_t reg_addr;
  uint32_t igu_acked;
  ...
  PMD_DRV_LOG(DEBUG, sc, "about to mask 0x%08x at %s addr 0x%08x", asserted,
  ...
  if (asserted & ATTN_NIG_FOR_FUNC) {
    ...
    if (sc->devinfo.int_block != INT_BLOCK_HC) {
      ...
      if (!igu_acked) {
        PMD_DRV_LOG(ERR, sc, "Failed to verify IGU ack on time");
      }
...
static void bnx2x_print_next_block(__rte_unused struct bnx2x_softc *sc,
                                   __rte_unused int idx,
                                   __rte_unused const char *blk) {
  PMD_DRV_LOG(INFO, sc, "%s%s", idx ? ", " : "", blk);
}

static int bnx2x_check_blocks_with_parity0(struct bnx2x_softc *sc, uint32_t sig,
                                           int par_num, uint8_t print) {
  uint32_t cur_bit = 0;
  int i = 0;
  ...
  for (i = 0; sig; i++) {
    ...
    if (sig & cur_bit) {
      ...
      case AEU_INPUTS_ATTN_BITS_BRB_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "BRB");
        break;
      case AEU_INPUTS_ATTN_BITS_PARSER_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "PARSER");
        break;
      case AEU_INPUTS_ATTN_BITS_TSDM_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "TSDM");
        break;
      case AEU_INPUTS_ATTN_BITS_SEARCHER_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "SEARCHER");
        break;
      case AEU_INPUTS_ATTN_BITS_TCM_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "TCM");
        break;
      case AEU_INPUTS_ATTN_BITS_TSEMI_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "TSEMI");
        break;
      case AEU_INPUTS_ATTN_BITS_PBCLIENT_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "XPB");
        break;
      }
...
static int bnx2x_check_blocks_with_parity1(struct bnx2x_softc *sc, uint32_t sig,
                                           int par_num, uint8_t *global,
                                           uint8_t print) {
  int i = 0;
  uint32_t cur_bit = 0;
  for (i = 0; sig; i++) {
    ...
    if (sig & cur_bit) {
      ...
      case AEU_INPUTS_ATTN_BITS_PBF_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "PBF");
        break;
      case AEU_INPUTS_ATTN_BITS_QM_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "QM");
        break;
      case AEU_INPUTS_ATTN_BITS_TIMERS_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "TM");
        break;
      case AEU_INPUTS_ATTN_BITS_XSDM_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "XSDM");
        break;
      case AEU_INPUTS_ATTN_BITS_XCM_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "XCM");
        break;
      case AEU_INPUTS_ATTN_BITS_XSEMI_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "XSEMI");
        break;
      case AEU_INPUTS_ATTN_BITS_DOORBELLQ_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "DOORBELLQ");
        break;
      case AEU_INPUTS_ATTN_BITS_NIG_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "NIG");
        break;
      case AEU_INPUTS_ATTN_BITS_VAUX_PCI_CORE_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "VAUX PCI CORE");
        *global = TRUE;
        break;
      case AEU_INPUTS_ATTN_BITS_DEBUG_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "DEBUG");
        break;
      case AEU_INPUTS_ATTN_BITS_USDM_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "USDM");
        break;
      case AEU_INPUTS_ATTN_BITS_UCM_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "UCM");
        break;
      case AEU_INPUTS_ATTN_BITS_USEMI_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "USEMI");
        break;
      case AEU_INPUTS_ATTN_BITS_UPB_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "UPB");
        break;
      case AEU_INPUTS_ATTN_BITS_CSDM_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "CSDM");
        break;
      case AEU_INPUTS_ATTN_BITS_CCM_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "CCM");
        break;
      }
...
static int bnx2x_check_blocks_with_parity2(struct bnx2x_softc *sc, uint32_t sig,
                                           int par_num, uint8_t print) {
  uint32_t cur_bit = 0;
  int i = 0;
  ...
  for (i = 0; sig; i++) {
    ...
    if (sig & cur_bit) {
      ...
      case AEU_INPUTS_ATTN_BITS_CSEMI_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "CSEMI");
        break;
      case AEU_INPUTS_ATTN_BITS_PXP_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "PXP");
        break;
      case AEU_IN_ATTN_BITS_PXPPCICLOCKCLIENT_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "PXPPCICLOCKCLIENT");
        break;
      case AEU_INPUTS_ATTN_BITS_CFC_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "CFC");
        break;
      case AEU_INPUTS_ATTN_BITS_CDU_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "CDU");
        break;
      case AEU_INPUTS_ATTN_BITS_DMAE_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "DMAE");
        break;
      case AEU_INPUTS_ATTN_BITS_IGU_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "IGU");
        break;
      case AEU_INPUTS_ATTN_BITS_MISC_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "MISC");
        break;
      }
...
static int bnx2x_check_blocks_with_parity3(struct bnx2x_softc *sc, uint32_t sig,
                                           int par_num, uint8_t *global,
                                           uint8_t print) {
  uint32_t cur_bit = 0;
  int i = 0;
  ...
  for (i = 0; sig; i++) {
    ...
    if (sig & cur_bit) {
      ...
      case AEU_INPUTS_ATTN_BITS_MCP_LATCHED_ROM_PARITY:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "MCP ROM");
        *global = TRUE;
        break;
      case AEU_INPUTS_ATTN_BITS_MCP_LATCHED_UMP_RX_PARITY:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "MCP UMP RX");
        *global = TRUE;
        break;
      case AEU_INPUTS_ATTN_BITS_MCP_LATCHED_UMP_TX_PARITY:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "MCP UMP TX");
        *global = TRUE;
        break;
      case AEU_INPUTS_ATTN_BITS_MCP_LATCHED_SCPAD_PARITY:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "MCP SCPAD");
        *global = TRUE;
        break;
      }
...
static int bnx2x_check_blocks_with_parity4(struct bnx2x_softc *sc, uint32_t sig,
                                           int par_num, uint8_t print) {
  uint32_t cur_bit = 0;
  int i = 0;
  ...
  for (i = 0; sig; i++) {
    ...
    if (sig & cur_bit) {
      ...
      case AEU_INPUTS_ATTN_BITS_PGLUE_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "PGLUE_B");
        break;
      case AEU_INPUTS_ATTN_BITS_ATC_PARITY_ERROR:
        if (print)
          bnx2x_print_next_block(sc, par_num++, "ATC");
        break;
      }
...
static uint8_t bnx2x_parity_attn(struct bnx2x_softc *sc, uint8_t *global,
                                 uint8_t print, uint32_t *sig) {
  int par_num = 0;

  if ((sig[0] & HW_PRTY_ASSERT_SET_0) || (sig[1] & HW_PRTY_ASSERT_SET_1) ||
      (sig[2] & HW_PRTY_ASSERT_SET_2) || (sig[3] & HW_PRTY_ASSERT_SET_3) ||
    ...
    par_num = bnx2x_check_blocks_with_parity0(sc, sig[0] & HW_PRTY_ASSERT_SET_0,
                                              par_num, print);
    par_num = bnx2x_check_blocks_with_parity1(sc, sig[1] & HW_PRTY_ASSERT_SET_1,
                                              par_num, global, print);
    par_num = bnx2x_check_blocks_with_parity2(sc, sig[2] & HW_PRTY_ASSERT_SET_2,
                                              par_num, print);
    par_num = bnx2x_check_blocks_with_parity3(sc, sig[3] & HW_PRTY_ASSERT_SET_3,
                                              par_num, global, print);
    par_num = bnx2x_check_blocks_with_parity4(sc, sig[4] & HW_PRTY_ASSERT_SET_4,
                                              par_num, print);
...
static void bnx2x_attn_int_deasserted4(struct bnx2x_softc *sc, uint32_t attn) {
  ...
  if (attn & AEU_INPUTS_ATTN_BITS_PGLUE_HW_INTERRUPT) {
    ...
    if (val & PGLUE_B_PGLUE_B_INT_STS_REG_ADDRESS_ERROR)
      PMD_DRV_LOG(INFO, sc, "ERROR: PGLUE_B_PGLUE_B_INT_STS_REG_ADDRESS_ERROR");
  ...
  if (attn & AEU_INPUTS_ATTN_BITS_ATC_HW_INTERRUPT) {
    ...
    if (val & ATC_ATC_INT_STS_REG_ADDRESS_ERROR)
      PMD_DRV_LOG(INFO, sc, "ERROR: ATC_ATC_INT_STS_REG_ADDRESS_ERROR");
    if (val & ATC_ATC_INT_STS_REG_ATC_TCPL_TO_NOT_PEND)
      PMD_DRV_LOG(INFO, sc, "ERROR: ATC_ATC_INT_STS_REG_ATC_TCPL_TO_NOT_PEND");
    if (val & ATC_ATC_INT_STS_REG_ATC_GPA_MULTIPLE_HITS)
      PMD_DRV_LOG(INFO, sc, "ERROR: ATC_ATC_INT_STS_REG_ATC_GPA_MULTIPLE_HITS");
    if (val & ATC_ATC_INT_STS_REG_ATC_RCPL_TO_EMPTY_CNT)
      PMD_DRV_LOG(INFO, sc, "ERROR: ATC_ATC_INT_STS_REG_ATC_RCPL_TO_EMPTY_CNT");
    if (val & ATC_ATC_INT_STS_REG_ATC_TCPL_ERROR)
      PMD_DRV_LOG(INFO, sc, "ERROR: ATC_ATC_INT_STS_REG_ATC_TCPL_ERROR");
  ...
  if (attn & (AEU_INPUTS_ATTN_BITS_PGLUE_PARITY_ERROR |
    ...
    PMD_DRV_LOG(INFO, sc, "ERROR: FATAL parity attention set4 0x%08x",
...
static void bnx2x_drv_info_ether_stat(struct bnx2x_softc *sc) {
  ...
  strncpy(ether_stat->version, BNX2X_DRIVER_VERSION, ETH_STAT_INFO_VERSION_LEN);

  sc->sp_objs[0].mac_obj.get_n_elements(
      sc, &sc->sp_objs[0].mac_obj, DRV_INFO_ETH_STAT_NUM_MACS_REQUIRED,
      ether_stat->mac_local + MAC_PAD, MAC_PAD, ETH_ALEN);
...
static int bnx2x_mc_assert(struct bnx2x_softc *sc) {
  ...
  for (i = 0; i < STORM_ASSERT_ARRAY_SIZE; i++) {
    ...
    if (row0 != COMMON_ASM_INVALID_ASSERT_OPCODE) {
      PMD_DRV_LOG(ERR, sc,
                  "XSTORM_ASSERT_INDEX 0x%x = 0x%08x 0x%08x 0x%08x 0x%08x", i,
                  row3, row2, row1, row0);
      rc++;
    } else {
      break;
  ...
  for (i = 0; i < STORM_ASSERT_ARRAY_SIZE; i++) {
    ...
    if (row0 != COMMON_ASM_INVALID_ASSERT_OPCODE) {
      PMD_DRV_LOG(ERR, sc,
                  "TSTORM_ASSERT_INDEX 0x%x = 0x%08x 0x%08x 0x%08x 0x%08x", i,
                  row3, row2, row1, row0);
      rc++;
    } else {
      break;
  ...
  for (i = 0; i < STORM_ASSERT_ARRAY_SIZE; i++) {
    ...
    if (row0 != COMMON_ASM_INVALID_ASSERT_OPCODE) {
      PMD_DRV_LOG(ERR, sc,
                  "CSTORM_ASSERT_INDEX 0x%x = 0x%08x 0x%08x 0x%08x 0x%08x", i,
                  row3, row2, row1, row0);
      rc++;
    } else {
      break;
  ...
  for (i = 0; i < STORM_ASSERT_ARRAY_SIZE; i++) {
    ...
    if (row0 != COMMON_ASM_INVALID_ASSERT_OPCODE) {
      PMD_DRV_LOG(ERR, sc,
                  "USTORM_ASSERT_INDEX 0x%x = 0x%08x 0x%08x 0x%08x 0x%08x", i,
                  row3, row2, row1, row0);
      rc++;
    } else {
      break;
...
static void bnx2x_attn_int_deasserted3(struct bnx2x_softc *sc, uint32_t attn) {
  ...
  if (attn & EVEREST_GEN_ATTN_IN_USE_MASK) {
    ...
    if (attn & BNX2X_PMF_LINK_ASSERT(sc)) {
      ...
      if (sc->link_vars.periodic_flags & ELINK_PERIODIC_FLAGS_LINK_EVENT) {
        /* sync with link */
        bnx2x_acquire_phy_lock(sc);
        sc->link_vars.periodic_flags &= ~ELINK_PERIODIC_FLAGS_LINK_EVENT;
        bnx2x_release_phy_lock(sc);
        if (IS_MF(sc)) {
          bnx2x_link_sync_notify(sc);
        }
      ...
      PMD_DRV_LOG(ERR, sc, "Unknown HW assert! (attn 0x%08x)", attn);
    }
  }
...
#define AEU_PXP2_HW_INT_BIT AEU_INPUTS_ATTN_BITS_PXPPCICLOCKCLIENT_HW_INTERRUPT
  ...
  if (attn & AEU_PXP2_HW_INT_BIT) {
    ...
    if (!CHIP_IS_E1x(sc)) {
      ...
      PMD_DRV_LOG(ERR, sc, "PXP2_REG_PXP2_INT_STS_CLR_0 0x%08x", val0);
  ...
  if (attn & HW_INTERRUT_ASSERT_SET_2) {
    ...
    PMD_DRV_LOG(ERR, sc, "FATAL HW block attention set2 0x%x",
...
static void bnx2x_attn_int_deasserted1(struct bnx2x_softc *sc, uint32_t attn) {
  ...
  if (attn & HW_INTERRUT_ASSERT_SET_1) {
    ...
    PMD_DRV_LOG(ERR, sc, "FATAL HW block attention set1 0x%08x",
...
static void bnx2x_attn_int_deasserted(struct bnx2x_softc *sc,
                                      uint32_t deasserted) {
  ...
  for (index = 0; index < MAX_DYNAMIC_ATTN_GRPS; index++) {
    if (deasserted & (1 << index)) {
      ...
      bnx2x_attn_int_deasserted4(sc, attn.sig[4] & group_mask->sig[4]);
      bnx2x_attn_int_deasserted3(sc, attn.sig[3] & group_mask->sig[3]);
      bnx2x_attn_int_deasserted1(sc, attn.sig[1] & group_mask->sig[1]);
      bnx2x_attn_int_deasserted2(sc, attn.sig[2] & group_mask->sig[2]);
      bnx2x_attn_int_deasserted0(sc, attn.sig[0] & group_mask->sig[0]);
    }
  }
  ...
  val = ~deasserted;
  PMD_DRV_LOG(DEBUG, sc, "about to mask 0x%08x at %s addr 0x%08x", val,
              (sc->devinfo.int_block == INT_BLOCK_HC) ? "HC" : "IGU", reg_addr);
  REG_WR(sc, reg_addr, val);
...
static void bnx2x_handle_mcast_eqe(struct bnx2x_softc *sc) {
  ...
  if (sc->mcast_obj.check_pending(&sc->mcast_obj)) {
    rc = ecore_config_mcast(sc, &rparam, ECORE_MCAST_CMD_CONT);
    if (rc < 0) {
      PMD_DRV_LOG(INFO, sc, "Failed to send pending mcast commands (%d)", rc);
    }
  }
}

static void bnx2x_handle_classification_eqe(struct bnx2x_softc *sc,
  ...
  if (rc < 0) {
    PMD_DRV_LOG(NOTICE, sc, "Failed to schedule new commands (%d)", rc);
  } else if (rc > 0) {
...
static void bnx2x_eq_int(struct bnx2x_softc *sc) {
  ...
  for (; sw_cons != hw_cons;
    ...
    case EVENT_RING_OPCODE_CFC_DEL:
      ...
      PMD_DRV_LOG(DEBUG, sc, "got delete ramrod for MULTI[%d]", cid);
    ...
    case EVENT_RING_OPCODE_FUNCTION_UPDATE:
      ...
      if (echo == SWITCH_UPDATE) {
        PMD_DRV_LOG(DEBUG, sc, "got FUNC_SWITCH_UPDATE ramrod");
        if (f_obj->complete_cmd(sc, f_obj, ECORE_F_CMD_SWITCH_UPDATE)) {
          break;
        }
      } else {
        PMD_DRV_LOG(DEBUG, sc, "AFEX: ramrod completed FUNCTION_UPDATE");
    ...
    case EVENT_RING_OPCODE_FORWARD_SETUP:
      ...
      if (q_obj->complete_cmd(sc, q_obj, ECORE_Q_CMD_SETUP_TX_ONLY)) {
        break;
      }
      goto next_spqe;
    ...
    case (EVENT_RING_OPCODE_RSS_UPDATE_RULES |
      ...
      cid = elem->message.data.eth_event.echo & BNX2X_SWCID_MASK;
      PMD_DRV_LOG(DEBUG, sc, "got RSS_UPDATE ramrod. CID %d", cid);
    ...
    case (EVENT_RING_OPCODE_CLASSIFICATION_RULES |
      ...
      PMD_DRV_LOG(DEBUG, sc, "got (un)set mac ramrod");
    ...
    case (EVENT_RING_OPCODE_MULTICAST_RULES | BNX2X_STATE_CLOSING_WAITING_HALT):
      PMD_DRV_LOG(DEBUG, sc, "got mcast ramrod");
      bnx2x_handle_mcast_eqe(sc);
      break;
    ...
    case (EVENT_RING_OPCODE_FILTERS_RULES | BNX2X_STATE_CLOSING_WAITING_HALT):
      PMD_DRV_LOG(DEBUG, sc, "got rx_mode ramrod");
...
static int bnx2x_handle_sp_tq(struct bnx2x_softc *sc) {
  ...
  if (status & BNX2X_DEF_SB_IDX) {
    ...
    bnx2x_eq_int(sc);
    bnx2x_ack_sb(sc, sc->igu_dsb_id, USTORM_ID, le16toh(sc->def_idx),
                 IGU_INT_NOP, 1);
  ...
  if (unlikely(status)) {
    PMD_DRV_LOG(INFO, sc, "Got an unknown SP interrupt! (0x%04x)", status);
  }

  /* ack status block only if something was actually handled */
  bnx2x_ack_sb(sc, sc->igu_dsb_id, ATTENTION_ID, le16toh(sc->def_att_idx),
               IGU_INT_ENABLE, 1);
...
static void bnx2x_handle_fp_tq(struct bnx2x_fastpath *fp) {
  ...
  PMD_DEBUG_PERIODIC_LOG(DEBUG, sc, "---> FP TASK QUEUE (%d) <--", fp->index);
  ...
  bnx2x_ack_sb(sc, fp->igu_sb_id, USTORM_ID, le16toh(fp->fp_hc_idx),
               IGU_INT_ENABLE, 1);
}
...
int bnx2x_intr_legacy(struct bnx2x_softc *sc) {
  ...
  FOR_EACH_ETH_QUEUE(sc, i) {
    ...
    if (status & mask) {
      ...
      bnx2x_ack_sb(sc, fp->igu_sb_id, USTORM_ID, 0, IGU_INT_DISABLE, 0);
      bnx2x_handle_fp_tq(fp);
      status &= ~mask;
    }
  }
  ...
  if (unlikely(status & 0x1)) {
    ...
    bnx2x_ack_sb(sc, sc->igu_dsb_id, USTORM_ID, 0, IGU_INT_DISABLE, 0);
    rc = bnx2x_handle_sp_tq(sc);
    status &= ~0x1;
  }

  if (unlikely(status)) {
    PMD_DRV_LOG(WARNING, sc, "Unexpected fastpath status (0x%08x)!", status);
  }
...
static struct ecore_func_sp_drv_ops bnx2x_func_sp_drv = {
...
static void bnx2x_init_func_obj(struct bnx2x_softc *sc) {
  ...
  ecore_init_func_obj(sc, &sc->func_obj, BNX2X_SP(sc, func_rdata),
...
static void bnx2x_fill(struct bnx2x_softc *sc, uint32_t addr, int fill,
                       uint32_t len) {
  uint32_t i;
...
static void bnx2x_wr_fp_sb_data(struct bnx2x_softc *sc, int fw_sb_id,
                                uint32_t *sb_data_p, uint32_t data_size) {
  uint32_t index;

  for (index = 0; index < data_size; index++) {
    REG_WR(sc,
           (BAR_CSTRORM_INTMEM + CSTORM_STATUS_BLOCK_DATA_OFFSET(fw_sb_id) +
...
static void bnx2x_zero_fp_sb(struct bnx2x_softc *sc, int fw_sb_id) {
  ...
  if (!CHIP_IS_E1x(sc)) {
    ...
    data_size = (sizeof(struct hc_status_block_data_e2) / sizeof(uint32_t));
  } else {
    memset(&sb_data_e1x, 0, sizeof(struct hc_status_block_data_e1x));
    sb_data_e1x.common.state = SB_DISABLED;
    sb_data_e1x.common.p_func.vf_valid = FALSE;
    sb_data_p = (uint32_t *)&sb_data_e1x;
    data_size = (sizeof(struct hc_status_block_data_e1x) / sizeof(uint32_t));
  }

  bnx2x_wr_fp_sb_data(sc, fw_sb_id, sb_data_p, data_size);

  bnx2x_fill(sc, (BAR_CSTRORM_INTMEM + CSTORM_STATUS_BLOCK_OFFSET(fw_sb_id)), 0,
             CSTORM_STATUS_BLOCK_SIZE);
  bnx2x_fill(sc, (BAR_CSTRORM_INTMEM + CSTORM_SYNC_BLOCK_OFFSET(fw_sb_id)), 0,
             CSTORM_SYNC_BLOCK_SIZE);
}

static void bnx2x_wr_sp_sb_data(struct bnx2x_softc *sc,
                                struct hc_sp_status_block_data *sp_sb_data) {
  uint32_t i;

  for (i = 0; i < (sizeof(struct hc_sp_status_block_data) / sizeof(uint32_t));
       i++) {
    REG_WR(sc,
           (BAR_CSTRORM_INTMEM +
...
static void bnx2x_zero_sp_sb(struct bnx2x_softc *sc) {
  ...
  bnx2x_fill(sc,
             (BAR_CSTRORM_INTMEM + CSTORM_SP_STATUS_BLOCK_OFFSET(SC_FUNC(sc))),
             0, CSTORM_SP_STATUS_BLOCK_SIZE);
  bnx2x_fill(sc,
             (BAR_CSTRORM_INTMEM + CSTORM_SP_SYNC_BLOCK_OFFSET(SC_FUNC(sc))), 0,
...
static void bnx2x_init_sb(struct bnx2x_softc *sc, rte_iova_t busaddr, int vfid,
...
static uint32_t bnx2x_rx_ustorm_prods_offset(struct bnx2x_softc *sc,
                                             struct bnx2x_fastpath *fp) {
...
static void bnx2x_init_eth_fp(struct bnx2x_softc *sc, int idx) {
  ...
  bnx2x_init_sb(sc, fp->sb_dma.paddr, BNX2X_VF_ID_INVALID, FALSE, fp->fw_sb_id,
                fp->igu_sb_id);
  ...
  ecore_init_queue_obj(sc, &sc->sp_objs[idx].q_obj, fp->cl_id, cids,
                       sc->max_cos, SC_FUNC(sc), BNX2X_SP(sc, q_rdata),
                       (rte_iova_t)BNX2X_SP_MAPPING(sc, q_rdata), q_type);

  /* configure classification DBs */
  ecore_init_mac_obj(sc, &sc->sp_objs[idx].mac_obj, fp->cl_id, idx, SC_FUNC(sc),
...
static void bnx2x_init_def_sb(struct bnx2x_softc *sc) {
  ...
  section = ((uint64_t)mapping + offsetof(struct host_sp_status_block, sp_sb));
...
static void bnx2x_init_eq_ring(struct bnx2x_softc *sc) {
  ...
  for (i = 1; i <= NUM_EQ_PAGES; i++) {
    ...
    elem->next_page.addr.hi = htole32(
        U64_HI(sc->eq_dma.paddr + BNX2X_PAGE_SIZE * (i % NUM_EQ_PAGES)));
    elem->next_page.addr.lo = htole32(
        U64_LO(sc->eq_dma.paddr + BNX2X_PAGE_SIZE * (i % NUM_EQ_PAGES)));
  }
  ...
  atomic_store_rel_long(
      &sc->eq_spq_left,
      (min((MAX_SP_DESC_CNT - MAX_SPQ_PENDING), NUM_EQ_DESC) - 1));
}
...
static void bnx2x_init_internal_common(struct bnx2x_softc *sc) {
  ...
  for (i = 0; i < (USTORM_AGG_DATA_SIZE >> 2); i++) {
    REG_WR(sc, (BAR_USTRORM_INTMEM + USTORM_AGG_DATA_OFFSET + (i * 4)), 0);
  }
...
static void bnx2x_init_internal(struct bnx2x_softc *sc, uint32_t load_code) {
  ...
  default:
    PMD_DRV_LOG(NOTICE, sc, "Unknown load_code (0x%x) from MCP", load_code);
    break;
  }
}
...
storm_memset_func_cfg(struct bnx2x_softc *sc,
  ...
  addr = (BAR_TSTRORM_INTMEM + TSTORM_FUNCTION_COMMON_CONFIG_OFFSET(abs_fid));
  size = sizeof(struct tstorm_eth_function_common_config);
  ecore_storm_memset_struct(sc, addr, size, (uint32_t *)tcfg);
}

static void bnx2x_func_init(struct bnx2x_softc *sc,
  ...
  if (p->func_flgs & FUNC_FLG_SPQ) {
    ...
    REG_WR(sc, (XSEM_REG_FAST_MEMORY + XSTORM_SPQ_PROD_OFFSET(p->func_id)),
           p->spq_prod);
  }
}
 ...
 */
static void bnx2x_calc_vn_min(struct bnx2x_softc *sc,
  ...
  for (vn = VN_0; vn < SC_MAX_VN_NUM(sc); vn++) {
    ...
    vn_min_rate =
        (((vn_cfg & FUNC_MF_CFG_MIN_BW_MASK) >> FUNC_MF_CFG_MIN_BW_SHIFT) *
         100);
...
static uint16_t bnx2x_extract_max_cfg(__rte_unused struct bnx2x_softc *sc,
                                      uint32_t mf_cfg) {
  uint16_t max_cfg =
      ((mf_cfg & FUNC_MF_CFG_MAX_BW_MASK) >> FUNC_MF_CFG_MAX_BW_SHIFT);

  if (!max_cfg) {
    PMD_DRV_LOG(DEBUG, sc, "Max BW configured to 0 - using 100 instead");
    max_cfg = 100;
  }
...
static void bnx2x_calc_vn_max(struct bnx2x_softc *sc, int vn,
                              struct cmng_init_input *input) {
...
static void storm_memset_cmng(struct bnx2x_softc *sc, struct cmng_init *cmng,
  ...
  for (vn = VN_0; vn < SC_MAX_VN_NUM(sc); vn++) {
    ...
    addr = (BAR_XSTRORM_INTMEM + XSTORM_RATE_SHAPING_PER_VN_VARS_OFFSET(func));
    size = sizeof(struct rate_shaping_vars_per_vn);
    ecore_storm_memset_struct(sc, addr, size,
                              (uint32_t *)&cmng->vnic.vnic_max_rate[vn]);

    addr = (BAR_XSTRORM_INTMEM + XSTORM_FAIRNESS_PER_VN_VARS_OFFSET(func));
...
static void bnx2x_pf_init(struct bnx2x_softc *sc) {
  ...
  if (!CHIP_IS_E1x(sc)) {
    ...
    REG_WR(sc,
           (IGU_REG_STATISTIC_NUM_MESSAGE_SENT +
            (BNX2X_IGU_STAS_MSG_VF_CNT * 4) + (BNX2X_IGU_STAS_MSG_PF_CNT * 4) +
            ((CHIP_IS_MODE_4_PORT(sc) ? SC_FUNC(sc) : SC_VN(sc)) * 4)),
           0);
  }
...
static void bnx2x_hc_int_enable(struct bnx2x_softc *sc) {
  ...
  if (msix) {
    val &= ~(HC_CONFIG_0_REG_SINGLE_ISR_EN_0 | HC_CONFIG_0_REG_INT_LINE_EN_0);
    ...
    val |= (HC_CONFIG_0_REG_SINGLE_ISR_EN_0 |
            HC_CONFIG_0_REG_MSI_MSIX_INT_EN_0 | HC_CONFIG_0_REG_ATTN_BIT_EN_0);
  } else {
    val |=
        (HC_CONFIG_0_REG_SINGLE_ISR_EN_0 | HC_CONFIG_0_REG_MSI_MSIX_INT_EN_0 |
...
static void bnx2x_igu_int_enable(struct bnx2x_softc *sc) {
  ...
  if (msix) {
    ...
    val |= (IGU_PF_CONF_INT_LINE_EN | IGU_PF_CONF_ATTN_BIT_EN |
            IGU_PF_CONF_SINGLE_ISR_EN);
  }
  ...
  PMD_DRV_LOG(DEBUG, sc, "write 0x%x to IGU mode %s", val,
...
static void bnx2x_hc_int_disable(struct bnx2x_softc *sc) {
  ...
  val &= ~(HC_CONFIG_0_REG_SINGLE_ISR_EN_0 | HC_CONFIG_0_REG_MSI_MSIX_INT_EN_0 |
           HC_CONFIG_0_REG_INT_LINE_EN_0 | HC_CONFIG_0_REG_ATTN_BIT_EN_0);
...
static void bnx2x_igu_int_disable(struct bnx2x_softc *sc) {
  ...
  val &= ~(IGU_PF_CONF_MSI_MSIX_EN | IGU_PF_CONF_INT_LINE_EN |
...
static void bnx2x_nic_init(struct bnx2x_softc *sc, int load_code) {
  ...
  elink_init_mod_abs_int(sc, &sc->link_vars, sc->devinfo.chip_id,
  ...
  bnx2x_attn_int_deasserted0(
      sc, REG_RD(sc, (MISC_REG_AEU_AFTER_INVERT_1_FUNC_0 + SC_PORT(sc) * 4)) &
...
static void bnx2x_init_objs(struct bnx2x_softc *sc) {
  ...
  ecore_init_mcast_obj(sc, &sc->mcast_obj, sc->fp[0].cl_id, sc->fp[0].index,
                       SC_FUNC(sc), SC_FUNC(sc), BNX2X_SP(sc, mcast_rdata),
                       (rte_iova_t)BNX2X_SP_MAPPING(sc, mcast_rdata),
                       ECORE_FILTER_MCAST_PENDING, &sc->sp_state, o_type);

  /* Setup CAM credit pools */
  ecore_init_mac_credit_pool(sc, &sc->macs_pool, SC_FUNC(sc),
                             CHIP_IS_E1x(sc) ? VNICS_PER_PORT(sc)
                                             : VNICS_PER_PATH(sc));

  ecore_init_vlan_credit_pool(sc, &sc->vlans_pool, SC_ABS_FUNC(sc) >> 1,
                              CHIP_IS_E1x(sc) ? VNICS_PER_PORT(sc)
                                              : VNICS_PER_PATH(sc));

  /* RSS configuration object */
  ecore_init_rss_config_obj(sc, &sc->rss_conf_obj, sc->fp->cl_id, sc->fp->index,
                            SC_FUNC(sc), SC_FUNC(sc), BNX2X_SP(sc, rss_rdata),
...
static int bnx2x_set_power_state(struct bnx2x_softc *sc, uint8_t state) {
  ...
  pci_read(sc, (sc->devinfo.pcie_pm_cap_reg + PCIR_POWER_STATUS), &pmcsr, 2);

  switch (state) {
  case PCI_PM_D0:
    pci_write_word(sc, (sc->devinfo.pcie_pm_cap_reg + PCIR_POWER_STATUS),
  ...
  case PCI_PM_D3hot:
    ...
    pci_write_long(sc, (sc->devinfo.pcie_pm_cap_reg + PCIR_POWER_STATUS),
                   pmcsr);
  ...
  default:
    PMD_DRV_LOG(NOTICE, sc, "Can't support PCI power state = %d", state);
    return -1;
  }
...
static uint8_t bnx2x_trylock_hw_lock(struct bnx2x_softc *sc,
                                     uint32_t resource) {
  uint32_t lock_status;
  uint32_t resource_bit = (1 << resource);
  int func = SC_FUNC(sc);
  ...
  if (resource > HW_LOCK_MAX_RESOURCE_VALUE) {
    PMD_DRV_LOG(INFO, sc, "resource(0x%x) > HW_LOCK_MAX_RESOURCE_VALUE(0x%x)",
...
static void bnx2x_set_234_gates(struct bnx2x_softc *sc, uint8_t close) {
  ...
  if (CHIP_IS_E1x(sc)) {
    ...
    if (close)
      ...
      REG_WR(sc, HC_REG_CONFIG_1, (val | HC_CONFIG_1_REG_BLOCK_DISABLE_1));
    ...
    if (close)
      ...
      REG_WR(sc, HC_REG_CONFIG_0, (val | HC_CONFIG_0_REG_BLOCK_DISABLE_0));
...
static void bnx2x_reset_mcp_prep(struct bnx2x_softc *sc, uint32_t *magic_val) {
  ...
  validity_offset = offsetof(struct shmem_region, validity_map[SC_PORT(sc)]);
...
static int bnx2x_init_shmem(struct bnx2x_softc *sc) {
  ...
  do {
    sc->devinfo.shmem_base = sc->link_params.shmem_base =
 ...
 *      - RBCN, RBCP
 */
static void bnx2x_process_kill_chip_reset(struct bnx2x_softc *sc,
                                          uint8_t global) {
   ...
   * (per chip) blocks.
   */
  global_bits2 = MISC_REGISTERS_RESET_REG_2_RST_MCP_N_RESET_CMN_CPU |
   ...
   */
  not_reset_mask1 = MISC_REGISTERS_RESET_REG_1_RST_HC |
  ...
  not_reset_mask2 =
      ...
      MISC_REGISTERS_RESET_REG_2_RST_RBCN | MISC_REGISTERS_RESET_REG_2_RST_GRC |
      MISC_REGISTERS_RESET_REG_2_RST_MCP_N_RESET_REG_HARD_CORE |
      MISC_REGISTERS_RESET_REG_2_RST_MCP_N_HARD_CORE_RST_B |
      MISC_REGISTERS_RESET_REG_2_RST_ATC | MISC_REGISTERS_RESET_REG_2_PGLC |
      MISC_REGISTERS_RESET_REG_2_RST_BMAC0 |
      MISC_REGISTERS_RESET_REG_2_RST_BMAC1 |
      MISC_REGISTERS_RESET_REG_2_RST_EMAC0 |
      MISC_REGISTERS_RESET_REG_2_RST_EMAC1 | MISC_REGISTERS_RESET_REG_2_UMAC0 |
   ...
   */
  stay_reset2 =
      MISC_REGISTERS_RESET_REG_2_XMAC | MISC_REGISTERS_RESET_REG_2_XMAC_SOFT;
...
static int bnx2x_process_kill(struct bnx2x_softc *sc, uint8_t global) {
  ...
  do {
    ...
    if ((sr_cnt == 0x7e) && (blk_cnt == 0xa0) &&
        ((port_is_idle_0 & 0x1) == 0x1) && ((port_is_idle_1 & 0x1) == 0x1) &&
...
static int bnx2x_leader_reset(struct bnx2x_softc *sc) {
  ...
  if (!global && !BNX2X_NOMCP(sc)) {
    ...
    if ((load_code != FW_MSG_CODE_DRV_LOAD_COMMON_CHIP) &&
        (load_code != FW_MSG_CODE_DRV_LOAD_COMMON)) {
      PMD_DRV_LOG(NOTICE, sc, "MCP unexpected response, aborting");
      rc = -1;
 ...
 *   - Queue's CDU context
 */
static void bnx2x_pf_q_prep_init(struct bnx2x_softc *sc,
                                 struct bnx2x_fastpath *fp,
  ...
  /* HC rate */
  init_params->rx.hc_rate = sc->hc_rx_ticks ? (1000000 / sc->hc_rx_ticks) : 0;
  init_params->tx.hc_rate = sc->hc_tx_ticks ? (1000000 / sc->hc_tx_ticks) : 0;
...
static unsigned long bnx2x_get_common_flags(struct bnx2x_softc *sc,
                                            uint8_t zero_stats) {
  uint32_t flags = 0;
...
static unsigned long bnx2x_get_q_flags(struct bnx2x_softc *sc,
                                       uint8_t leading) {
  uint32_t flags = 0;
...
static void bnx2x_pf_q_prep_general(struct bnx2x_softc *sc,
                                    struct bnx2x_fastpath *fp,
...
static void bnx2x_pf_rx_q_prep(struct bnx2x_softc *sc,
                               struct bnx2x_fastpath *fp,
  ...
  if (sc->dropless_fc && pause->bd_th_hi + FW_PREFETCH_CNT > sc->rx_ring_size) {
  ...
  rxq_init->rcq_np_map = (rte_iova_t)(rxq->cq_ring_phys_addr + BNX2X_PAGE_SIZE);
   ...
   */
  if (IS_MF_AFEX(sc)) {
    rxq_init->silent_removal_value = sc->devinfo.mf_info.afex_def_vlan_tag;
    rxq_init->silent_removal_mask = EVL_VLID_MASK;
  }
}

static void bnx2x_pf_tx_q_prep(struct bnx2x_softc *sc,
                               struct bnx2x_fastpath *fp,
 ...
 *   2) INIT->SETUP
 */
static int bnx2x_setup_queue(struct bnx2x_softc *sc, struct bnx2x_fastpath *fp,
                             uint8_t leading) {
  struct ecore_queue_state_params q_params = {NULL};
  ...
  bnx2x_pf_rx_q_prep(sc, fp, &setup_params->pause_params,
...
static int bnx2x_config_rss_pf(struct bnx2x_softc *sc,
  ...
  rte_memcpy(params.ind_table, rss_obj->ind_table, sizeof(params.ind_table));
...
static int bnx2x_init_rss_pf(struct bnx2x_softc *sc) {
  ...
  for (i = 0; i < sizeof(sc->rss_conf_obj.ind_table); i++) {
    sc->rss_conf_obj.ind_table[i] = (sc->fp->cl_id + (i % num_eth_queues));
  }
...
static int bnx2x_set_mac_one(struct bnx2x_softc *sc, uint8_t *mac,
                             struct ecore_vlan_mac_obj *obj, uint8_t set,
                             int mac_type, uint32_t *ramrod_flags) {
  ...
  if (!rte_bit_relaxed_get32(RAMROD_CONT, ramrod_flags)) {
    rte_memcpy(ramrod_param.user_req.u.mac.mac, mac, ETH_ALEN);

    rte_bit_relaxed_set32(mac_type, &ramrod_param.user_req.vlan_mac_flags);

    /* Set the command: ADD or DEL */
    ramrod_param.user_req.cmd = (set) ? ECORE_VLAN_MAC_ADD : ECORE_VLAN_MAC_DEL;
  }
  ...
  if (rc == ECORE_EXISTS) {
    ...
    PMD_DRV_LOG(ERR, sc, "%s MAC failed (%d)", (set ? "Set" : "Delete"), rc);
  }
...
static int bnx2x_set_eth_mac(struct bnx2x_softc *sc, uint8_t set) {
  ...
  return bnx2x_set_mac_one(sc, sc->link_params.mac_addr, &sc->sp_objs->mac_obj,
...
static int bnx2x_get_cur_phy_idx(struct bnx2x_softc *sc) {
  ...
  if (sc->link_vars.link_up) {
    ...
    if ((sc->link_vars.link_status & LINK_STATUS_SERDES_LINK) &&
        (sc->link_params.phy[ELINK_EXT_PHY2].supported & ELINK_SUPPORTED_FIBRE))
...
static void bnx2x_calc_fc_adv(struct bnx2x_softc *sc) {
  ...
  switch (sc->link_vars.ieee_fc & MDIO_COMBO_IEEE0_AUTO_NEG_ADV_PAUSE_MASK) {
  case MDIO_COMBO_IEEE0_AUTO_NEG_ADV_PAUSE_NONE:
  default:
    sc->port.advertising[cfg_idx] &=
        ~(ADVERTISED_Asym_Pause | ADVERTISED_Pause);
    break;

  case MDIO_COMBO_IEEE0_AUTO_NEG_ADV_PAUSE_BOTH:
    sc->port.advertising[cfg_idx] |= (ADVERTISED_Asym_Pause | ADVERTISED_Pause);
    break;
...
static uint16_t bnx2x_get_mf_speed(struct bnx2x_softc *sc) {
  ...
  if (IS_MF(sc)) {
    uint16_t maxCfg =
        bnx2x_extract_max_cfg(sc, sc->devinfo.mf_info.mf_config[SC_VN(sc)]);
...
static void bnx2x_fill_report_data(struct bnx2x_softc *sc,
  ...
  if (sc->link_vars.flow_ctrl & ELINK_FLOW_CTRL_RX) {
    rte_bit_relaxed_set32(BNX2X_LINK_REPORT_RX_FC_ON, &data->link_report_flags);
  }

  /* Tx Flow Control is ON */
  if (sc->link_vars.flow_ctrl & ELINK_FLOW_CTRL_TX) {
    rte_bit_relaxed_set32(BNX2X_LINK_REPORT_TX_FC_ON, &data->link_report_flags);
  }
}
...
static void bnx2x_link_report_locked(struct bnx2x_softc *sc) {
  ...
  if (rte_bit_relaxed_get32(BNX2X_LINK_REPORT_LINK_DOWN,
    ...
    if (rte_bit_relaxed_test_and_clear32(BNX2X_LINK_REPORT_FULL_DUPLEX,
                                         &cur_data.link_report_flags)) {
      duplex = "full";
    ...
    if (cur_data.link_report_flags) {
      if (rte_bit_relaxed_get32(BNX2X_LINK_REPORT_RX_FC_ON,
                                &cur_data.link_report_flags) &&
          rte_bit_relaxed_get32(BNX2X_LINK_REPORT_TX_FC_ON,
                                &cur_data.link_report_flags)) {
        flow = "ON - receive & transmit";
      } else if (rte_bit_relaxed_get32(BNX2X_LINK_REPORT_RX_FC_ON,
                                       &cur_data.link_report_flags) &&
                 !rte_bit_relaxed_get32(BNX2X_LINK_REPORT_TX_FC_ON,
                                        &cur_data.link_report_flags)) {
        flow = "ON - receive";
    ...
    PMD_DRV_LOG(INFO, sc, "NIC Link is Up, %d Mbps %s duplex, Flow control: %s",
  ...
  if (IS_PF(sc) && !CHIP_REV_IS_SLOW(sc)) {
    ...
    sc->port.supported[0] |=
        (ELINK_SUPPORTED_10baseT_Half | ELINK_SUPPORTED_10baseT_Full |
         ...
         ELINK_SUPPORTED_10000baseT_Full | ELINK_SUPPORTED_TP |
         ELINK_SUPPORTED_FIBRE | ELINK_SUPPORTED_Autoneg |
    ...
    if (CHIP_REV_IS_FPGA(sc)) {
      ...
      sc->link_vars.link_status =
          (LINK_STATUS_LINK_UP | LINK_STATUS_SPEED_AND_DUPLEX_1000TFD);
    } else {
...
static int bnx2x_initial_phy_init(struct bnx2x_softc *sc, int load_mode) {
  ...
  if (load_mode == LOAD_DIAG) {
    ...
    if (lp->req_line_speed[cfg_idx] < ELINK_SPEED_10000) {
      if (lp->speed_cap_mask[cfg_idx] & PORT_HW_CFG_SPEED_CAPABILITY_D0_10G) {
...
static void bnx2x_update_drv_flags(struct bnx2x_softc *sc, uint32_t flags,
                                   uint32_t set) {
  uint32_t drv_flags;
...
void bnx2x_periodic_callout(struct bnx2x_softc *sc) {
  if ((sc->state != BNX2X_STATE_OPEN) ||
    ...
    PMD_DRV_LOG(DEBUG, sc, "periodic callout exit (state=0x%x)", sc->state);
    return;
  }
  if (!CHIP_REV_IS_SLOW(sc)) {
    /*
  ...
  if (IS_PF(sc) && !BNX2X_NOMCP(sc)) {
    ...
    mcp_pulse =
        (SHMEM_RD(sc, func_mb[mb_idx].mcp_pulse_mb) & MCP_PULSE_SEQ_MASK);
    ...
    if ((drv_pulse != mcp_pulse) &&
      ...
      PMD_DRV_LOG(ERR, sc, "drv_pulse (0x%x) != mcp_pulse (0x%x)", drv_pulse,
                  mcp_pulse);
    }
  }
#endif
...
static __rte_noinline int bnx2x_nic_load(struct bnx2x_softc *sc) {
  ...
  sc->link_params.feature_config_flags &= ~ELINK_FEATURE_CONFIG_BOOT_FROM_SAN;
    ...
    if (!bnx2x_reset_is_done(sc, SC_PATH(sc)) ||
      ...
      do {
        ...
        if ((!load_status && (!global || !other_load_status)) &&
            bnx2x_trylock_leader_lock(sc) && !bnx2x_leader_reset(sc)) {
          PMD_DRV_LOG(INFO, sc, "Recovered during init");
          break;
        }
...
static void bnx2x_get_function_num(struct bnx2x_softc *sc) {
  ...
  sc->pfunc_rel = (uint8_t)((val & ME_REG_PF_NUM) >> ME_REG_PF_NUM_SHIFT);
  sc->path_id =
  ...
  PMD_DRV_LOG(DEBUG, sc, "Relative function %d, Absolute function %d, Path %d",
...
static uint32_t bnx2x_get_shmem_mf_cfg_base(struct bnx2x_softc *sc) {
  ...
  /* Non 57712 */
  offset =
      (SHMEM_ADDR(sc, func_mb) + (MAX_FUNC_NUM * sizeof(struct drv_func_mb)));
...
static void bnx2x_probe_pci_caps(struct bnx2x_softc *sc) {
  ...
  sc->devinfo.pcie_link_width = ((link_status & PCIM_LINK_STA_WIDTH) >> 4);
...
static int bnx2x_get_shmem_mf_cfg_info_sd(struct bnx2x_softc *sc) {
  ...
  mf_info->vnics_per_port = (CHIP_PORT_MODE(sc) == CHIP_4_PORT_MODE) ? 2 : 4;

  return 0;
}

static uint32_t
bnx2x_get_shmem_ext_proto_support_flags(struct bnx2x_softc *sc) {
  uint32_t retval = 0;
  uint32_t val;
...
static int bnx2x_get_shmem_mf_cfg_info_si(struct bnx2x_softc *sc) {
  ...
  mf_info->mf_protos_supported = bnx2x_get_shmem_ext_proto_support_flags(sc);
...
static int bnx2x_get_shmem_mf_cfg_info_niv(struct bnx2x_softc *sc) {
  ...
  mf_info->ext_id = (uint16_t)((e1hov_tag & FUNC_MF_CFG_E1HOV_TAG_MASK) >>
                               FUNC_MF_CFG_E1HOV_TAG_SHIFT);

  mf_info->default_vlan = (uint16_t)((e1hov_tag & FUNC_MF_CFG_AFEX_VLAN_MASK) >>
  ...
  mf_info->afex_vlan_mode = ((niv_config & FUNC_MF_CFG_AFEX_VLAN_MODE_MASK) >>
...
static int bnx2x_check_valid_mf_cfg(struct bnx2x_softc *sc) {
  ...
  if (mf_info->mf_config[SC_VN(sc)] & FUNC_MF_CFG_FUNC_HIDE) {
    PMD_DRV_LOG(NOTICE, sc, "Enumerated function %d is marked as hidden",
                SC_PORT(sc));
    return 1;
  }
  ...
  if (mf_info->mf_mode == MULTI_FUNCTION_SD) {
    ...
    if ((SC_VN(sc) > 0) && !VALID_OVLAN(OVLAN(sc))) {
      PMD_DRV_LOG(NOTICE, sc, "mf_mode=SD vnic_id=%d ovlan=%d", SC_VN(sc),
                  OVLAN(sc));
      return 1;
    }

    if (!VALID_OVLAN(OVLAN(sc)) && mf_info->multi_vnics_mode) {
      PMD_DRV_LOG(NOTICE, sc, "mf_mode=SD multi_vnics_mode=%d ovlan=%d",
    ...
    FOREACH_ABS_FUNC_IN_PORT(sc, i) {
      ...
      for (j = i + 2; j < MAX_FUNC_NUM; j += 2) {
        ...
        if (!(mf_cfg1 & FUNC_MF_CFG_FUNC_HIDE) && VALID_OVLAN(ovlan1) &&
            !(mf_cfg2 & FUNC_MF_CFG_FUNC_HIDE) && VALID_OVLAN(ovlan2) &&
            (ovlan1 == ovlan2)) {
          PMD_DRV_LOG(NOTICE, sc,
...
static int bnx2x_get_mf_cfg_info(struct bnx2x_softc *sc) {
  ...
  case SHARED_FEAT_CFG_FORCE_SF_MODE_SWITCH_INDEPT:
    ...
    if (mac_upper != FUNC_MF_CFG_UPPERMAC_DEFAULT) {
      ...
      PMD_DRV_LOG(NOTICE, sc, "Invalid config for Switch Independent mode");
    }
  ...
  case SHARED_FEAT_CFG_FORCE_SF_MODE_SPIO4:
    ...
    if ((val & FUNC_MF_CFG_E1HOV_TAG_MASK) != FUNC_MF_CFG_E1HOV_TAG_DEFAULT) {
      mf_info->mf_mode = MULTI_FUNCTION_SD;
    } else {
      PMD_DRV_LOG(NOTICE, sc, "Invalid config for Switch Dependent mode");
    }
...
static int bnx2x_get_shmem_info(struct bnx2x_softc *sc) {
  ...
  sc->devinfo.hw_config2 = SHMEM_RD(sc, dev_info.shared_hw_config.config2);
  ...
  sc->port.config = SHMEM_RD(sc, dev_info.port_feature_config[port].config);
  ...
  if ((mac_lo == 0) && (mac_hi == 0)) {
    *sc->mac_addr_str = 0;
    ...
    snprintf(sc->mac_addr_str, sizeof(sc->mac_addr_str), RTE_ETHER_ADDR_PRT_FMT,
    ...
    PMD_DRV_LOG(DEBUG, sc, "Ethernet address: %s", sc->mac_addr_str);
  }
...
static int bnx2x_get_igu_cam_info(struct bnx2x_softc *sc) {
  ...
  if (CHIP_INT_MODE_IS_BC(sc)) {
    ...
    sc->igu_base_sb = ((CHIP_IS_MODE_4_PORT(sc) ? pfid : vn) * FP_SB_MAX_E1x);
    sc->igu_dsb_id =
        (E1HVN_MAX * FP_SB_MAX_E1x + (CHIP_IS_MODE_4_PORT(sc) ? pfid : vn));
    return 0;
  }
...
static int bnx2x_get_device_info(struct bnx2x_softc *sc) {
  ...
  if (REG_RD(sc, MISC_REG_CHIP_TYPE) & MISC_REG_CHIP_TYPE_57811_MASK) {
    if (CHIP_IS_57810(sc)) {
      sc->devinfo.chip_id =
          ((CHIP_NUM_57811 << 16) | (sc->devinfo.chip_id & 0x0000ffff));
    } else if (CHIP_IS_57810_MF(sc)) {
      sc->devinfo.chip_id =
          ((CHIP_NUM_57811_MF << 16) | (sc->devinfo.chip_id & 0x0000ffff));
    }
  ...
  PMD_DRV_LOG(DEBUG, sc,
              ...
              sc->devinfo.chip_id, ((sc->devinfo.chip_id >> 16) & 0xffff),
  ...
  if (CHIP_IS_E2E3(sc)) {
    ...
    sc->devinfo.chip_port_mode = (val) ? CHIP_4_PORT_MODE : CHIP_2_PORT_MODE;
  ...
  sc->devinfo.shmem2_base =
      REG_RD(sc, (SC_PATH(sc) ? MISC_REG_GENERIC_CR_1 : MISC_REG_GENERIC_CR_0));
  ...
  if ((val & (SHR_MEM_VALIDITY_DEV_INFO | SHR_MEM_VALIDITY_MB)) !=
    ...
    PMD_DRV_LOG(NOTICE, sc, "Invalid SHMEM validity signature: 0x%08x", val);
    return 0;
  }

  /* get the bootcode version */
  sc->devinfo.bc_ver = SHMEM_RD(sc, dev_info.bc_rev);
  snprintf(sc->devinfo.bc_ver_str, sizeof(sc->devinfo.bc_ver_str), "%d.%d.%d",
  ...
  if (CHIP_IS_E1x(sc)) {
    ...
    if (val & IGU_BLOCK_CONFIGURATION_REG_BACKWARD_COMP_EN) {
      ...
      if (REG_RD(sc, IGU_REG_RESET_MEMORIES)) {
        PMD_DRV_LOG(NOTICE, sc, "FORCING IGU Normal Mode failed!!!");
...
static void bnx2x_link_settings_supported(struct bnx2x_softc *sc,
                                          uint32_t switch_cfg) {
  uint32_t cfg_size = 0;
  uint32_t idx;
  ...
  case 3:
    if (sc->link_params.multi_phy_config & PORT_HW_CFG_PHY_SWAPPED_ENABLED) {
      sc->port.supported[1] = sc->link_params.phy[ELINK_EXT_PHY1].supported;
      sc->port.supported[0] = sc->link_params.phy[ELINK_EXT_PHY2].supported;
    } else {
      sc->port.supported[0] = sc->link_params.phy[ELINK_EXT_PHY1].supported;
      sc->port.supported[1] = sc->link_params.phy[ELINK_EXT_PHY2].supported;
    }
    cfg_size = 2;
    break;
  }
  ...
  if (!(sc->port.supported[0] || sc->port.supported[1])) {
    PMD_DRV_LOG(
        ERR, sc, "Invalid phy config in NVRAM (PHY1=0x%08x PHY2=0x%08x)",
        SHMEM_RD(sc, dev_info.port_hw_config[port].external_phy_config),
        SHMEM_RD(sc, dev_info.port_hw_config[port].external_phy_config2));
    return;
  }
  ...
  if (CHIP_IS_E3(sc))
    ...
    case ELINK_SWITCH_CFG_10G:
      sc->port.phy_addr = REG_RD(sc, NIG_REG_XGXS0_CTRL_PHY_ADDR + port * 0x18);
      break;
    default:
      PMD_DRV_LOG(ERR, sc,
  ...
  for (idx = 0; idx < cfg_size; idx++) {
    ...
    case PORT_FEATURE_LINK_SPEED_10M_FULL:
      if (sc->port.supported[idx] & ELINK_SUPPORTED_10baseT_Full) {
        ...
        sc->port.advertising[idx] |= (ADVERTISED_10baseT_Full | ADVERTISED_TP);
      } else {
        PMD_DRV_LOG(ERR, sc,
    ...
    case PORT_FEATURE_LINK_SPEED_10M_HALF:
      if (sc->port.supported[idx] & ELINK_SUPPORTED_10baseT_Half) {
        ...
        sc->port.advertising[idx] |= (ADVERTISED_10baseT_Half | ADVERTISED_TP);
      } else {
        PMD_DRV_LOG(ERR, sc,
                    "Invalid NVRAM config link_config=0x%08x "
                    "speed_cap_mask=0x%08x",
    ...
    case PORT_FEATURE_LINK_SPEED_100M_FULL:
      if (sc->port.supported[idx] & ELINK_SUPPORTED_100baseT_Full) {
        ...
        sc->port.advertising[idx] |= (ADVERTISED_100baseT_Full | ADVERTISED_TP);
      } else {
        PMD_DRV_LOG(ERR, sc,
                    "Invalid NVRAM config link_config=0x%08x "
                    "speed_cap_mask=0x%08x",
    ...
    case PORT_FEATURE_LINK_SPEED_100M_HALF:
      if (sc->port.supported[idx] & ELINK_SUPPORTED_100baseT_Half) {
        sc->link_params.req_line_speed[idx] = ELINK_SPEED_100;
        sc->link_params.req_duplex[idx] = DUPLEX_HALF;
        sc->port.advertising[idx] |= (ADVERTISED_100baseT_Half | ADVERTISED_TP);
      } else {
        PMD_DRV_LOG(ERR, sc,
                    "Invalid NVRAM config link_config=0x%08x "
                    "speed_cap_mask=0x%08x",
...
static void bnx2x_get_phy_info(struct bnx2x_softc *sc) {
  ...
  if (eee_mode != PORT_FEAT_CFG_EEE_POWER_MODE_DISABLED) {
    sc->link_params.eee_mode =
        (ELINK_EEE_MODE_ADV_LPI | ELINK_EEE_MODE_ENABLE_LPI |
...
int bnx2x_alloc_hsi_mem(struct bnx2x_softc *sc) {
  ...
  if (IS_PF(sc)) {
    ...
    if (bnx2x_dma_alloc(sc, sizeof(struct host_sp_status_block),
                        &sc->def_sb_dma, "def_sb", RTE_CACHE_LINE_SIZE) != 0) {
      return -1;
    }
    ...
    if (bnx2x_dma_alloc(sc, BNX2X_PAGE_SIZE, &sc->eq_dma, "ev_queue",
                        RTE_CACHE_LINE_SIZE) != 0) {
      sc->def_sb = NULL;
      return -1;
    }
    ...
    if (bnx2x_dma_alloc(sc, sizeof(struct bnx2x_slowpath), &sc->sp_dma, "sp",
                        RTE_CACHE_LINE_SIZE) != 0) {
      sc->eq = NULL;
      sc->def_sb = NULL;
      return -1;
    ...
    if (bnx2x_dma_alloc(sc, BNX2X_PAGE_SIZE, &sc->spq_dma, "sp_queue",
                        RTE_CACHE_LINE_SIZE) != 0) {
      sc->sp = NULL;
      sc->eq = NULL;
      sc->def_sb = NULL;
    ...
    if (bnx2x_dma_alloc(sc, FW_BUF_SIZE, &sc->gz_buf_dma, "fw_buf",
                        RTE_CACHE_LINE_SIZE) != 0) {
      sc->spq = NULL;
      sc->sp = NULL;
      sc->eq = NULL;
...
static struct bnx2x_prev_list_node *
bnx2x_prev_path_get_entry(struct bnx2x_softc *sc) {
  struct bnx2x_prev_list_node *tmp;

  LIST_FOREACH(tmp, &bnx2x_prev_list, node) {
    if ((sc->pcie_bus == tmp->bus) && (sc->pcie_device == tmp->slot) &&
        (SC_PATH(sc) == tmp->path)) {
      return tmp;
    }
  }
...
static uint8_t bnx2x_prev_is_path_marked(struct bnx2x_softc *sc) {
  ...
  if (tmp) {
    if (tmp->aer) {
      PMD_DRV_LOG(DEBUG, sc, "Path %d/%d/%d was marked by AER", sc->pcie_bus,
                  sc->pcie_device, SC_PATH(sc));
    } else {
      rc = TRUE;
      PMD_DRV_LOG(DEBUG, sc,
...
static int bnx2x_prev_mark_path(struct bnx2x_softc *sc, uint8_t after_undi) {
  ...
  if (tmp) {
    if (!tmp->aer) {
      PMD_DRV_LOG(DEBUG, sc, "Re-marking AER in path %d/%d/%d", sc->pcie_bus,
                  sc->pcie_device, SC_PATH(sc));
    } else {
      PMD_DRV_LOG(DEBUG, sc, "Removing AER indication from path %d/%d/%d",
                  sc->pcie_bus, sc->pcie_device, SC_PATH(sc));
      tmp->aer = 0;
    }
  ...
  tmp =
      rte_malloc("", sizeof(struct bnx2x_prev_list_node), RTE_CACHE_LINE_SIZE);
  if (!tmp) {
...
static int bnx2x_do_flr(struct bnx2x_softc *sc) {
  ...
  if (sc->devinfo.bc_ver < REQ_BC_VER_4_INITIATE_FLR) {
    PMD_DRV_LOG(WARNING, sc, "FLR not supported by BC_VER: 0x%08x",
                sc->devinfo.bc_ver);
    return -1;
  }
...
static void bnx2x_prev_unload_close_mac(struct bnx2x_softc *sc,
  ...
  if (!CHIP_IS_E3(sc)) {
    ...
    if ((mask & reset_reg) && val) {
      base_addr =
          SC_PORT(sc) ? NIG_REG_INGRESS_BMAC1_MEM : NIG_REG_INGRESS_BMAC0_MEM;
...
static void bnx2x_prev_unload_undi_inc(struct bnx2x_softc *sc, uint8_t port,
                                       uint8_t inc) {
  uint16_t rcq, bd;
...
static void bnx2x_dcbx_set_state(struct bnx2x_softc *sc, uint8_t dcb_on,
  ...
  PMD_DRV_LOG(DEBUG, sc, "DCB state [%s:%s]", dcb_on ? "ON" : "OFF",
              (dcbx_enabled == BNX2X_DCBX_ENABLED_OFF) ? "user-mode"
              : (dcbx_enabled == BNX2X_DCBX_ENABLED_ON_NEG_OFF)
                  ? "on-chip static"
              : (dcbx_enabled == BNX2X_DCBX_ENABLED_ON_NEG_ON)
                  ? "on-chip with negotiation"
                  : "invalid");
}
...
static int bnx2x_pci_get_caps(struct bnx2x_softc *sc) {
  ...
  cap = sc->pci_caps =
      rte_zmalloc("caps", sizeof(struct bnx2x_pci_cap), RTE_CACHE_LINE_SIZE);
  if (!cap) {
  ...
  while (pci_cap.next) {
    ...
    cap->next = rte_zmalloc("pci_cap", sizeof(struct bnx2x_pci_cap),
                            RTE_CACHE_LINE_SIZE);
    if (!cap->next) {
      PMD_DRV_LOG(NOTICE, sc, "Failed to allocate memory");
      return -ENOMEM;
...
static void bnx2x_init_rte(struct bnx2x_softc *sc) {
  if (IS_VF(sc)) {
    sc->max_tx_queues = min(BNX2X_VF_MAX_QUEUES_PER_VF, sc->igu_sb_cnt);
    sc->max_rx_queues = min(BNX2X_VF_MAX_QUEUES_PER_VF, sc->igu_sb_cnt);
  } else {
...
void bnx2x_load_firmware(struct bnx2x_softc *sc) {
  ...
  fwname =
      sc->devinfo.device_id == CHIP_NUM_57711 ? FW_NAME_57711 : FW_NAME_57810;
  ...
  if (sc->fw_len < FW_HEADER_LEN) {
    PMD_DRV_LOG(NOTICE, sc, "Invalid fw size: %" PRIu64, sc->fw_len);
    goto out;
  }
...
static void bnx2x_data_to_init_ops(uint8_t *data, struct raw_op *dst,
                                   uint32_t len) {
...
static void bnx2x_data_to_init_offsets(uint8_t *data, uint16_t *dst,
                                       uint32_t len) {
...
static void bnx2x_data_to_init_data(uint8_t *data, uint32_t *dst,
                                    uint32_t len) {
  uint32_t *src = (uint32_t *)data;
  uint32_t i;
...
static void bnx2x_data_to_iro_array(uint8_t *data, struct iro *dst,
                                    uint32_t len) {
  uint32_t *src = (uint32_t *)data;
  uint32_t i, j, tmp;
...
int bnx2x_attach(struct bnx2x_softc *sc) {
  ...
  if (sc->devinfo.pcie_msix_cap_reg != 0) {
    uint32_t val;
    pci_read(sc, (sc->devinfo.pcie_msix_cap_reg + PCIR_MSIX_CTRL), &val, 2);
  ...
  if (IS_PF(sc) && !BNX2X_NOMCP(sc)) {
    ...
    sc->fw_seq = (SHMEM_RD(sc, func_mb[SC_FW_MB_IDX(sc)].drv_mb_header) &
                  DRV_MSG_SEQ_NUMBER_MASK);
    PMD_DRV_LOG(DEBUG, sc, "prev unload fw_seq 0x%04x", sc->fw_seq);
...
static void bnx2x_igu_ack_sb(struct bnx2x_softc *sc, uint8_t igu_sb_id,
...
static void bnx2x_igu_clear_sb_gen(struct bnx2x_softc *sc, uint8_t func,
                                   uint8_t idu_sb_id, uint8_t is_pf) {
  ...
  uint32_t igu_addr_ack =
      IGU_REG_CSTORM_TYPE_0_SB_CLEANUP + (idu_sb_id / 32) * 4;
  ...
  PMD_DRV_LOG(DEBUG, sc, "write 0x%08x to IGU(via GRC) addr 0x%x", ctl,
              igu_addr_ctl);
...
static void bnx2x_common_init_phy(struct bnx2x_softc *sc) {
  ...
  if (SHMEM2_RD(sc, size) >
      (uint32_t)offsetof(struct shmem2_region, lfa_host_addr[SC_PORT(sc)])) {
    return;
  }
  ...
  bnx2x_acquire_phy_lock(sc);
  elink_common_init_phy(sc, shmem_base, shmem2_base, sc->devinfo.chip_id, 0);
  bnx2x_release_phy_lock(sc);
}
 ...
 * combination.
 */
static int bnx2x_pretend_func(struct bnx2x_softc *sc,
...
static void bnx2x_setup_fan_failure_detection(struct bnx2x_softc *sc) {
   ...
   */
    ...
    for (port = PORT_0; port < PORT_MAX; port++) {
      is_required |= elink_fan_failure_det_req(sc, sc->devinfo.shmem_base,
                                               sc->devinfo.shmem2_base, port);
    }
  }
...
static int bnx2x_init_hw_common(struct bnx2x_softc *sc) {
  ...
  PMD_DRV_LOG(DEBUG, sc, "starting common init for func %d", SC_ABS_FUNC(sc));
  ...
  if (!CHIP_IS_E1x(sc)) {
     ...
     */
    for (abs_func_id = SC_PATH(sc); abs_func_id < (E2_FUNC_MAX * 2);
         abs_func_id += 2) {
  ...
  if (!CHIP_IS_E1x(sc)) {
     ...
     *      Having an MCP lock on the load/unload process does not guarantee
     * that there is no Timer disable during Func6/7 enable. This is because the
  ...
  if (sizeof(union cdu_context) != 1024) {
    ...
    PMD_DRV_LOG(NOTICE, sc, "please adjust the size of cdu_context(%ld)",
  ...
  if (!CHIP_IS_E1x(sc)) {
    REG_WR(
        sc, PCICFG_OFFSET + PXPCS_TL_CONTROL_5,
...
static int bnx2x_init_hw_port(struct bnx2x_softc *sc) {
  ...
  if (CHIP_IS_MODE_4_PORT(sc)) {
    REG_WR(sc,
           SC_PORT(sc) ? BRB1_REG_MAC_GUARANTIED_1 : BRB1_REG_MAC_GUARANTIED_0,
           40);
  }
  ...
  if (CHIP_IS_E3B0(sc)) {
    if (IS_MF_AFEX(sc)) {
      ...
      if (SC_PORT(sc)) {
        REG_WR(sc, PRS_REG_HDRS_AFTER_BASIC_PORT_1, 0xE);
        REG_WR(sc, PRS_REG_HDRS_AFTER_TAG_0_PORT_1, 0x6);
        REG_WR(sc, PRS_REG_MUST_HAVE_HDRS_PORT_1, 0xA);
      } else {
        REG_WR(sc, PRS_REG_HDRS_AFTER_BASIC_PORT_0, 0xE);
        REG_WR(sc, PRS_REG_HDRS_AFTER_TAG_0_PORT_0, 0x6);
       ...
       */
      REG_WR(sc,
             SC_PORT(sc) ? PRS_REG_HDRS_AFTER_BASIC_PORT_1
  ...
  if (!CHIP_IS_E1x(sc)) {
    ...
    if (IS_MF_AFEX(sc)) {
      REG_WR(sc,
             SC_PORT(sc) ? NIG_REG_P1_HDRS_AFTER_BASIC
    ...
    if (CHIP_IS_E3(sc)) {
      REG_WR(sc, SC_PORT(sc) ? NIG_REG_LLH1_MF_MODE : NIG_REG_LLH_MF_MODE,
             IS_MF(sc));
    }
  }
  if (!CHIP_IS_E3(sc)) {
  ...
  if (!CHIP_IS_E1x(sc)) {
    ...
    REG_WR(sc, (SC_PORT(sc) ? NIG_REG_LLH1_CLS_TYPE : NIG_REG_LLH0_CLS_TYPE),
           val);
  }
...
static uint32_t bnx2x_flr_clnup_reg_poll(struct bnx2x_softc *sc, uint32_t reg,
                                         uint32_t expected,
                                         uint32_t poll_count) {
...
static int bnx2x_flr_clnup_poll_hw_counter(struct bnx2x_softc *sc, uint32_t reg,
                                           __rte_unused const char *msg,
                                           uint32_t poll_cnt) {
...
static int bnx2x_poll_hw_usage_counters(struct bnx2x_softc *sc,
                                        uint32_t poll_cnt) {
  /* wait for CFC PF usage-counter to zero (includes all the VFs) */
  if (bnx2x_flr_clnup_poll_hw_counter(sc, CFC_REG_NUM_LCIDS_INSIDE_PF,
  ...
  if (bnx2x_flr_clnup_poll_hw_counter(sc, DORQ_REG_PF_USAGE_CNT,
  ...
  if (bnx2x_flr_clnup_poll_hw_counter(sc, QM_REG_PF_USG_CNT_0 + 4 * SC_FUNC(sc),
  ...
  if (bnx2x_flr_clnup_poll_hw_counter(sc, TM_REG_LIN0_VNIC_UC + 4 * SC_PORT(sc),
  ...
  if (bnx2x_flr_clnup_poll_hw_counter(
          sc, TM_REG_LIN0_NUM_SCANS + 4 * SC_PORT(sc),
          "Timers NUM_SCANS usage counter timed out", poll_cnt)) {
    return -1;
  }

  /* Wait DMAE PF usage counter to zero */
  if (bnx2x_flr_clnup_poll_hw_counter(sc, dmae_reg_go_c[INIT_DMAE_C(sc)],
...
static int bnx2x_send_final_clnup(struct bnx2x_softc *sc, uint8_t clnup_func,
                                  uint32_t poll_cnt) {
  uint32_t op_gen_command = 0;
  uint32_t comp_addr =
      (BAR_CSTRORM_INTMEM + CSTORM_FINAL_CLEANUP_COMPLETE_OFFSET(clnup_func));
  int ret = 0;

  if (REG_RD(sc, comp_addr)) {
    PMD_DRV_LOG(NOTICE, sc, "Cleanup complete was not 0 before sending");
    return -1;
  }
...
static void bnx2x_pbf_pN_buf_flushed(struct bnx2x_softc *sc,
...
static void bnx2x_pbf_pN_cmd_flushed(struct bnx2x_softc *sc,
...
static void bnx2x_tx_hw_flushed(struct bnx2x_softc *sc, uint32_t poll_count) {
  struct pbf_pN_cmd_regs cmd_regs[] = {
      {0,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_TQ_OCCUPANCY_Q0 : PBF_REG_P0_TQ_OCCUPANCY,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_TQ_LINES_FREED_CNT_Q0
                          : PBF_REG_P0_TQ_LINES_FREED_CNT},
      {1,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_TQ_OCCUPANCY_Q1 : PBF_REG_P1_TQ_OCCUPANCY,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_TQ_LINES_FREED_CNT_Q1
                          : PBF_REG_P1_TQ_LINES_FREED_CNT},
      {4,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_TQ_OCCUPANCY_LB_Q : PBF_REG_P4_TQ_OCCUPANCY,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_TQ_LINES_FREED_CNT_LB_Q
                          : PBF_REG_P4_TQ_LINES_FREED_CNT}};

  struct pbf_pN_buf_regs buf_regs[] = {
      {0, (CHIP_IS_E3B0(sc)) ? PBF_REG_INIT_CRD_Q0 : PBF_REG_P0_INIT_CRD,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_CREDIT_Q0 : PBF_REG_P0_CREDIT,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_INTERNAL_CRD_FREED_CNT_Q0
                          : PBF_REG_P0_INTERNAL_CRD_FREED_CNT},
      {1, (CHIP_IS_E3B0(sc)) ? PBF_REG_INIT_CRD_Q1 : PBF_REG_P1_INIT_CRD,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_CREDIT_Q1 : PBF_REG_P1_CREDIT,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_INTERNAL_CRD_FREED_CNT_Q1
                          : PBF_REG_P1_INTERNAL_CRD_FREED_CNT},
      {4, (CHIP_IS_E3B0(sc)) ? PBF_REG_INIT_CRD_LB_Q : PBF_REG_P4_INIT_CRD,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_CREDIT_LB_Q : PBF_REG_P4_CREDIT,
       (CHIP_IS_E3B0(sc)) ? PBF_REG_INTERNAL_CRD_FREED_CNT_LB_Q
...
static void bnx2x_hw_enable_status(struct bnx2x_softc *sc) {
  ...
  PMD_DRV_LOG(DEBUG, sc, "PGLUE_B_REG_SHADOW_BME_PF_7_0_CLR is 0x%x", val);

  val = REG_RD(sc, PGLUE_B_REG_FLR_REQUEST_PF_7_0_CLR);
  PMD_DRV_LOG(DEBUG, sc, "PGLUE_B_REG_FLR_REQUEST_PF_7_0_CLR is 0x%x", val);
...
static int bnx2x_init_hw_func(struct bnx2x_softc *sc) {
  ...
  if (sc->devinfo.int_block == INT_BLOCK_HC) {
    ...
    if (!CHIP_IS_E1x(sc)) {
      ...
      num_segs = CHIP_INT_MODE_IS_BC(sc) ? IGU_BC_NDSB_NUM_SEGS
                                         : IGU_NORM_NDSB_NUM_SEGS;
      for (sb_idx = 0; sb_idx < sc->igu_sb_cnt; sb_idx++) {
        prod_offset = (sc->igu_base_sb + sb_idx) * num_segs;
        ...
        bnx2x_ack_sb(sc, sc->igu_base_sb + sb_idx, USTORM_ID, 0, IGU_INT_NOP,
                     1);
      ...
      num_segs =
          CHIP_INT_MODE_IS_BC(sc) ? IGU_BC_DSB_NUM_SEGS : IGU_NORM_DSB_NUM_SEGS;
      ...
      prod_offset =
          (CHIP_INT_MODE_IS_BC(sc) ? IGU_BC_BASE_DSB_PROD + dsb_idx
      ...
      if (CHIP_INT_MODE_IS_BC(sc)) {
        bnx2x_ack_sb(sc, sc->igu_dsb_id, USTORM_ID, 0, IGU_INT_NOP, 1);
        bnx2x_ack_sb(sc, sc->igu_dsb_id, CSTORM_ID, 0, IGU_INT_NOP, 1);
        bnx2x_ack_sb(sc, sc->igu_dsb_id, XSTORM_ID, 0, IGU_INT_NOP, 1);
        bnx2x_ack_sb(sc, sc->igu_dsb_id, TSTORM_ID, 0, IGU_INT_NOP, 1);
        bnx2x_ack_sb(sc, sc->igu_dsb_id, ATTENTION_ID, 0, IGU_INT_NOP, 1);
      } else {
        bnx2x_ack_sb(sc, sc->igu_dsb_id, USTORM_ID, 0, IGU_INT_NOP, 1);
        bnx2x_ack_sb(sc, sc->igu_dsb_id, ATTENTION_ID, 0, IGU_INT_NOP, 1);
      }
  ...
  if (CHIP_IS_E1x(sc)) {
    ...
    main_mem_base = HC_REG_MAIN_MEMORY + SC_PORT(sc) * (main_mem_size * 4);
    ...
    for (i = main_mem_base; i < main_mem_base + main_mem_size * 4;
         i += main_mem_width) {
      bnx2x_read_dmae(sc, i, main_mem_width / 4);
      bnx2x_write_dmae(sc, BNX2X_SP_MAPPING(sc, wb_data), i,
                       main_mem_width / 4);
    }
  ...
  REG_WR8(sc, BAR_USTRORM_INTMEM + USTORM_RECORD_SLOW_PATH_OFFSET(SC_FUNC(sc)),
          1);
  REG_WR8(sc, BAR_TSTRORM_INTMEM + TSTORM_RECORD_SLOW_PATH_OFFSET(SC_FUNC(sc)),
          1);
  REG_WR8(sc, BAR_CSTRORM_INTMEM + CSTORM_RECORD_SLOW_PATH_OFFSET(SC_FUNC(sc)),
          1);
  REG_WR8(sc, BAR_XSTRORM_INTMEM + XSTORM_RECORD_SLOW_PATH_OFFSET(SC_FUNC(sc)),
          1);
...
static void bnx2x_link_reset(struct bnx2x_softc *sc) {
  if (!BNX2X_NOMCP(sc)) {
    ...
    if (!CHIP_REV_IS_SLOW(sc)) {
      PMD_DRV_LOG(WARNING, sc, "Bootcode is missing - cannot reset link");
    }
  }
}
...
static void bnx2x_reset_port(struct bnx2x_softc *sc) {
  ...
  REG_WR(sc, (port ? NIG_REG_LLH1_BRB1_NOT_MCP : NIG_REG_LLH0_BRB1_NOT_MCP),
         0x0);
  ...
  if (val) {
    PMD_DRV_LOG(DEBUG, sc, "BRB1 is not empty, %d blocks are occupied", val);
  }
}
...
static void bnx2x_reset_func(struct bnx2x_softc *sc) {
  ...
  /* SP SB */
  REG_WR8(sc,
          BAR_CSTRORM_INTMEM + CSTORM_SP_STATUS_BLOCK_DATA_STATE_OFFSET(func),
          SB_DISABLED);
...
static int ecore_gunzip(struct bnx2x_softc *sc, const uint8_t *zbuf, int len) {
  ...
  if ((ret != Z_STREAM_END) && (ret != Z_OK)) {
    PMD_DRV_LOG(NOTICE, sc, "zlib inflate error: %d %s", ret, zlib_stream.msg);
  }
...
static const char *get_ext_phy_type(uint32_t ext_phy_type) {
  ...
  static const char *types[] = {
      "DIRECT",         "BNX2X-8071",  "BNX2X-8072", "BNX2X-8073", "BNX2X-8705",
      "BNX2X-8706",     "BNX2X-8726",  "BNX2X-8481", "SFX-7101",   "BNX2X-8727",
...
static const char *get_state(uint32_t state) {
  ...
  static const char *states[] = {"CLOSED",
                                 "OPENING_WAIT4_LOAD",
                                 "OPENING_WAIT4_PORT",
                                 "OPEN",
                                 "CLOSING_WAIT4_HALT",
                                 "CLOSING_WAIT4_DELETE",
...
static const char *get_bnx2x_flags(uint32_t flags) {
  ...
  static const char *flag[] = {
      "ONE_PORT ",     "NO_ISCSI ",    "NO_FCOE ",     "NO_WOL ",  "USING_DAC ",
      "USING_MSIX ",   "USING_MSI ",   "DISABLE_MSI ", "UNKNOWN ", "NO_MCP ",
...
void bnx2x_print_adapter_info(struct bnx2x_softc *sc) {
  ...
  PMD_DRV_LOG(INFO, sc, "%12s : %s", "DPDK", rte_version());
  PMD_DRV_LOG(INFO, sc, "%12s : %s", "Driver", bnx2x_pmd_version());
  /* Firmware versions. */
  PMD_DRV_LOG(INFO, sc, "%12s : %d.%d.%d", "Firmware",
              BNX2X_5710_FW_MAJOR_VERSION, BNX2X_5710_FW_MINOR_VERSION,
              BNX2X_5710_FW_REVISION_VERSION);
  PMD_DRV_LOG(INFO, sc, "%12s : %s", "Bootcode", sc->devinfo.bc_ver_str);
  /* Hardware chip info. */
  PMD_DRV_LOG(INFO, sc, "%12s : %#08x", "ASIC", sc->devinfo.chip_id);
  PMD_DRV_LOG(INFO, sc, "%12s : %c%d", "Rev", (CHIP_REV(sc) >> 12) + 'A',
              (CHIP_METAL(sc) >> 4));
  /* Bus PCIe info. */
  PMD_DRV_LOG(INFO, sc, "%12s : 0x%x", "Vendor Id", sc->devinfo.vendor_id);
  PMD_DRV_LOG(INFO, sc, "%12s : 0x%x", "Device Id", sc->devinfo.device_id);
...
void bnx2x_print_device_info(struct bnx2x_softc *sc) {
  ...
  PMD_DRV_LOG(INFO, sc, "%12s : %s", "Bnx2x Flags", get_bnx2x_flags(sc->flags));
  PMD_DRV_LOG(INFO, sc, "%12s : %s", "DMAE Is",
              (sc->dmae_ready ? "Ready" : "Not Ready"));
  PMD_DRV_LOG(INFO, sc, "%12s : %u", "MTU", sc->mtu);
  PMD_DRV_LOG(INFO, sc, "%12s : %s", "PHY Type",
              get_ext_phy_type(ext_phy_type));
  ...
  PMD_DRV_LOG(INFO, sc, "%12s : %x", "Switch", sc->link_params.switch_cfg);
  PMD_DRV_LOG(INFO, sc, "pcie_bus=%d, pcie_device=%d", sc->pcie_bus,
              sc->pcie_device);
  PMD_DRV_LOG(INFO, sc, "bar0.addr=%p, bar1.addr=%p", sc->bar[BAR0].base_addr,
              sc->bar[BAR1].base_addr);
  PMD_DRV_LOG(INFO, sc, "port=%d, path=%d, vnic=%d, func=%d", PORT_ID(sc),
              PATH_ID(sc), VNIC_ID(sc), FUNC_ID(sc));
}
