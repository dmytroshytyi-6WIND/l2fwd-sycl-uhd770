...
ring_dma_zone_reserve(struct rte_eth_dev *dev, const char *ring_name,
  ...
  return rte_eth_dma_zone_reserve(dev, ring_name, queue_id, ring_size,
                                  BNX2X_PAGE_SIZE, socket_id);
}

static void bnx2x_rx_queue_release(struct bnx2x_rx_queue *rx_queue) {
  uint16_t i;
...
int bnx2x_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  rxq->sw_ring =
      rte_zmalloc_socket("sw_ring", dma_size, RTE_CACHE_LINE_SIZE, socket_id);
  if (NULL == rxq->sw_ring) {
  ...
  for (idx = 0; idx < rxq->nb_rx_desc; idx = NEXT_RX_BD(idx)) {
    ...
    rxq->rx_ring[idx] = rte_cpu_to_le_64(rte_mbuf_data_iova_default(mbuf));
  }
  ...
  if (!sc->rx_queues)
    sc->rx_queues = dev->data->rx_queues;

  return 0;
}

static void bnx2x_tx_queue_release(struct bnx2x_tx_queue *tx_queue) {
  uint16_t i;
  struct rte_mbuf **sw_ring;
...
static uint16_t bnx2x_xmit_pkts(void *p_txq, struct rte_mbuf **tx_pkts,
                                uint16_t nb_pkts) {
  ...
  if ((unlikely((txq->nb_tx_desc - txq->nb_tx_avail) > txq->tx_free_thresh)))
    bnx2x_txeof(sc, fp);
...
int bnx2x_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                             uint16_t nb_desc, unsigned int socket_id,
  ...
  txq->tx_free_thresh =
      min(txq->tx_free_thresh, txq->nb_tx_desc - BDS_PER_TX_PKT);

  PMD_DRV_LOG(DEBUG, sc,
              "fp[%02d] req_bd=%u, thresh=%u, usable_bd=%lu, "
              "total_bd=%lu, tx_pages=%u",
              queue_idx, nb_desc, txq->tx_free_thresh,
              (unsigned long)USABLE_TX_BD(txq), (unsigned long)TOTAL_TX_BD(txq),
              txq->nb_tx_pages);
  ...
  if (!sc->tx_queues)
    sc->tx_queues = dev->data->tx_queues;

  return 0;
}

static inline void bnx2x_upd_rx_prod_fast(struct bnx2x_softc *sc,
                                          struct bnx2x_fastpath *fp,
                                          uint16_t rx_bd_prod,
                                          uint16_t rx_cq_prod) {
  union {
...
static uint16_t bnx2x_recv_pkts(void *p_rxq, struct rte_mbuf **rx_pkts,
                                uint16_t nb_pkts) {
  ...
  while (nb_rx < nb_pkts && sw_cq_cons != hw_cq_cons) {
    ...
    if (unlikely(CQE_TYPE_SLOW(cqe_fp->type_error_flags &
                               ETH_FAST_PATH_RX_CQE_TYPE))) {
      PMD_RX_LOG(ERR, "slowpath event during traffic processing");
      break;
    }

    if (unlikely(cqe_fp->type_error_flags &
                 ETH_FAST_PATH_RX_CQE_PHY_DECODE_ERR_FLG)) {
      PMD_RX_LOG(ERR, "flags 0x%x rx packet %u", cqe_fp->type_error_flags,
                 sw_cq_cons);
      goto next_rx;
    }
    ...
    if (unlikely(buf_len < len + (pad + RTE_PKTMBUF_HEADROOM))) {
      if (unlikely(log_once)) {
        PMD_DRV_LOG(
            ERR, sc,
            "mbuf size %d is not enough to hold Rx packet length more than %d",
            buf_len - RTE_PKTMBUF_HEADROOM,
            buf_len - (pad + RTE_PKTMBUF_HEADROOM));
        log_once = false;
      }
      goto next_rx;
    }
