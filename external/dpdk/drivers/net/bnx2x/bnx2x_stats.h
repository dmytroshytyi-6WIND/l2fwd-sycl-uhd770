...
#define ADD_64_LE(s_hi, a_hi_le, s_lo, a_lo_le)                                \
  ADD_64(s_hi, le32toh(a_hi_le), s_lo, le32toh(a_lo_le))

#define ADD_64_LE16(s_hi, a_hi_le, s_lo, a_lo_le)                              \
  ADD_64(s_hi, le16toh(a_hi_le), s_lo, le16toh(a_lo_le))
...
#define UPDATE_STAT64(s, t)                                                    \
  do {                                                                         \
    DIFF_64(diff.hi, new->s##_hi, pstats->mac_stx[0].t##_hi, diff.lo,          \
            new->s##_lo, pstats->mac_stx[0].t##_lo);                           \
    pstats->mac_stx[0].t##_hi = new->s##_hi;                                   \
    pstats->mac_stx[0].t##_lo = new->s##_lo;                                   \
    ADD_64(pstats->mac_stx[1].t##_hi, diff.hi, pstats->mac_stx[1].t##_lo,      \
           diff.lo);                                                           \
  } while (0)

#define UPDATE_STAT64_NIG(s, t)                                                \
  do {                                                                         \
    DIFF_64(diff.hi, new->s##_hi, old->s##_hi, diff.lo, new->s##_lo,           \
            old->s##_lo);                                                      \
    ADD_64(estats->t##_hi, diff.hi, estats->t##_lo, diff.lo);                  \
  } while (0)
...
#define UPDATE_EXTEND_STAT(s)                                                  \
  do {                                                                         \
    ADD_EXTEND_64(pstats->mac_stx[1].s##_hi, pstats->mac_stx[1].s##_lo,        \
                  new->s);                                                     \
  } while (0)

#define UPDATE_EXTEND_TSTAT_X(s, t, size)                                      \
  do {                                                                         \
    diff = le##size##toh(tclient->s) - le##size##toh(old_tclient->s);          \
...
#define UPDATE_ESTAT_QSTAT_64(s)                                               \
  do {                                                                         \
    ADD_64(estats->s##_hi, qstats->s##_hi, estats->s##_lo, qstats->s##_lo);    \
    SUB_64(estats->s##_hi, qstats_old->s##_hi_old, estats->s##_lo,             \
           qstats_old->s##_lo_old);                                            \
...
#define UPDATE_FSTAT_QSTAT(s)                                                  \
  do {                                                                         \
    ADD_64(fstats->s##_hi, qstats->s##_hi, fstats->s##_lo, qstats->s##_lo);    \
    SUB_64(fstats->s##_hi, qstats_old->s##_hi, fstats->s##_lo,                 \
           qstats_old->s##_lo);                                                \
...
#define UPDATE_ESTAT(s, t)                                                     \
  do {                                                                         \
    SUB_64(estats->s##_hi, estats_old->t##_hi, estats->s##_lo,                 \
           estats_old->t##_lo);                                                \
    ADD_64(estats->s##_hi, estats->t##_hi, estats->s##_lo, estats->t##_lo);    \
