...
static inline uint32_t bnx2x_vf_crc(struct bnx2x_vf_bulletin *bull) {
...
int bnx2x_check_bull(struct bnx2x_softc *sc) {
  ...
  if (old_version == bull->version) {
    ...
    if (tries == BNX2X_VF_BULLETIN_TRIES) {
      PMD_DRV_LOG(ERR, sc,
                  "pf to vf bulletin board crc was wrong %d consecutive times. "
                  "Aborting",
                  tries);
      return FALSE;
    }
  ...
  if (valid_bitmap & (1 << MAC_ADDR_VALID) &&
      memcmp(bull->mac, sc->old_bulletin.mac, ETH_ALEN))
...
static void bnx2x_add_tlv(__rte_unused struct bnx2x_softc *sc, void *tlvs_list,
                          uint16_t offset, uint16_t type, uint16_t length) {
  struct channel_tlv *tl =
      (struct channel_tlv *)((unsigned long)tlvs_list + offset);
...
static void bnx2x_vf_prep(struct bnx2x_softc *sc,
                          struct vf_first_tlv *first_tlv, uint16_t type,
                          uint16_t length) {
...
static void bnx2x_vf_finalize(struct bnx2x_softc *sc,
...
static int bnx2x_do_req4pf(struct bnx2x_softc *sc, rte_iova_t phys_addr) {
...
static inline int bnx2x_read_vf_id(struct bnx2x_softc *sc, uint32_t *vf_id) {
  ...
  while (i <= BNX2X_ME_ANSWER_TRIES) {
    ...
    if (bnx2x_check_me_flags(val)) {
      PMD_DRV_LOG(DEBUG, sc, "valid register value: 0x%08x", val);
      *vf_id = VF_ID(val);
      return 0;
    }
...
static int bnx2x_loop_obtain_resources(struct bnx2x_softc *sc) {
  ...
  do {
    ...
    if (sc_resp->status == BNX2X_VF_STATUS_SUCCESS) {
      ...
      res_query->num_mac_filters =
          min(res_query->num_mac_filters, resc->num_mac_filters);
      res_query->num_vlan_filters =
          min(res_query->num_vlan_filters, resc->num_vlan_filters);
      res_query->num_mc_filters =
          min(res_query->num_mc_filters, resc->num_mc_filters);
...
int bnx2x_vf_get_resources(struct bnx2x_softc *sc, uint8_t tx_count,
  ...
  bnx2x_add_tlv(sc, acq, acq->first_tlv.tl.length, BNX2X_VF_TLV_PHYS_PORT_ID,
                sizeof(struct channel_tlv));

  bnx2x_add_tlv(sc, acq,
                (acq->first_tlv.tl.length + sizeof(struct channel_tlv)),
                BNX2X_VF_TLV_LIST_END, sizeof(struct channel_list_end_tlv));
  ...
  sc->flags |= BNX2X_NO_WOL_FLAG | BNX2X_NO_ISCSI_OOO_FLAG |
               BNX2X_NO_ISCSI_FLAG | BNX2X_NO_FCOE_FLAG;
...
void bnx2x_vf_close(struct bnx2x_softc *sc) {
  ...
  bnx2x_vf_prep(sc, &query->first_tlv, BNX2X_VF_TLV_RELEASE, sizeof(*query));
  ...
  bnx2x_add_tlv(sc, query, query->first_tlv.tl.length, BNX2X_VF_TLV_LIST_END,
...
int bnx2x_vf_init(struct bnx2x_softc *sc) {
  ...
  bnx2x_vf_prep(sc, &query->first_tlv, BNX2X_VF_TLV_INIT, sizeof(*query));
...
void bnx2x_vf_unload(struct bnx2x_softc *sc) {
  ...
  bnx2x_vf_prep(sc, &query->first_tlv, BNX2X_VF_TLV_CLOSE, sizeof(*query));
  ...
  rc = bnx2x_do_req4pf(sc, sc->vf2pf_mbox_mapping.paddr);
  if (rc || reply->status != BNX2X_VF_STATUS_SUCCESS)
    PMD_DRV_LOG(ERR, sc, "Bad reply from PF for close message");

out:
  bnx2x_vf_finalize(sc, &query->first_tlv);
}

static inline uint16_t bnx2x_vf_q_flags(uint8_t leading) {
...
static void bnx2x_vf_rx_q_prep(struct bnx2x_softc *sc,
...
static void bnx2x_vf_tx_q_prep(struct bnx2x_softc *sc,
                               struct bnx2x_fastpath *fp,
...
int bnx2x_vf_setup_queue(struct bnx2x_softc *sc, struct bnx2x_fastpath *fp,
  ...
  bnx2x_vf_prep(sc, &query->first_tlv, BNX2X_VF_TLV_SETUP_Q, sizeof(*query));
  ...
  if (rc)
    goto out;
  if (reply->status != BNX2X_VF_STATUS_SUCCESS) {
    PMD_DRV_LOG(ERR, sc, "Failed to setup VF queue[%d]", fp->index);
    rc = -EINVAL;
  }
out:
  bnx2x_vf_finalize(sc, &query->first_tlv);
...
int bnx2x_vf_teardown_queue(struct bnx2x_softc *sc, int qid) {
  ...
  bnx2x_vf_prep(sc, &query_op->first_tlv, BNX2X_VF_TLV_TEARDOWN_Q,
                sizeof(*query_op));

  query_op->vf_qid = qid;

  bnx2x_add_tlv(sc, query_op, query_op->first_tlv.tl.length,
                BNX2X_VF_TLV_LIST_END, sizeof(struct channel_list_end_tlv));

  rc = bnx2x_do_req4pf(sc, sc->vf2pf_mbox_mapping.paddr);
  if (rc || reply->status != BNX2X_VF_STATUS_SUCCESS)
    PMD_DRV_LOG(ERR, sc, "Bad reply for vf_q %d teardown", qid);
...
int bnx2x_vf_set_mac(struct bnx2x_softc *sc, int set) {
  ...
  query->filters[0].flags =
      (set ? BNX2X_VF_Q_FILTER_SET_MAC : 0) | BNX2X_VF_Q_FILTER_DEST_MAC_VALID;
  ...
  while (BNX2X_VF_STATUS_FAILURE == reply->status && bnx2x_check_bull(sc)) {
    /* A new mac was configured by PF for us */
    rte_memcpy(sc->link_params.mac_addr, sc->pf2vf_bulletin->mac, ETH_ALEN);
    rte_memcpy(query->filters[0].mac, sc->pf2vf_bulletin->mac, ETH_ALEN);
...
int bnx2x_vf_config_rss(struct bnx2x_softc *sc,
  ...
  bnx2x_vf_prep(sc, &query->first_tlv, BNX2X_VF_TLV_UPDATE_RSS, sizeof(*query));
...
int bnx2x_vfpf_set_mcast(struct bnx2x_softc *sc,
  ...
  if (mc_addrs_num > VF_MAX_MULTICAST_PER_VF) {
    PMD_DRV_LOG(ERR, sc, "VF supports not more than %d multicast MAC addresses",
  ...
  for (i = 0; i < mc_addrs_num; i++) {
    PMD_DRV_LOG(DEBUG, sc, "Adding mcast MAC:" RTE_ETHER_ADDR_PRT_FMT,
  ...
  if (reply->status != BNX2X_VF_STATUS_SUCCESS) {
    PMD_DRV_LOG(ERR, sc, "Set Rx mode/multicast failed: %d", reply->status);
    rc = -EINVAL;
  }
