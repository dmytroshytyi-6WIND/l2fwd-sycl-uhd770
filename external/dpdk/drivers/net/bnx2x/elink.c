...
typedef elink_status_t (*read_sfp_module_eeprom_func_p)(
    struct elink_phy *phy, struct elink_params *params, uint8_t dev_addr,
    uint16_t addr, uint8_t byte_cnt, uint8_t *o_buf, uint8_t);
/********************************************************/
...
#define ELINK_LINK_UPDATE_MASK                                                 \
  (LINK_STATUS_SPEED_AND_DUPLEX_MASK | LINK_STATUS_LINK_UP |                   \
   LINK_STATUS_PHYSICAL_LINK_FLAG | LINK_STATUS_AUTO_NEGOTIATE_COMPLETE |      \
...
#define CL22_WR_OVER_CL45(_sc, _phy, _bank, _addr, _val)                       \
  elink_cl45_write(_sc, _phy, (_phy)->def_md_devad, (_bank + (_addr & 0xf)),   \
                   _val)

#define CL22_RD_OVER_CL45(_sc, _phy, _bank, _addr, _val)                       \
  elink_cl45_read(_sc, _phy, (_phy)->def_md_devad, (_bank + (_addr & 0xf)),    \
                  _val)
...
static uint32_t elink_bits_en(struct bnx2x_softc *sc, uint32_t reg,
                              uint32_t bits) {
...
static int elink_check_lfa(struct elink_params *params) {
  ...
  additional_config = REG_RD(
      sc, params->lfa_base + offsetof(struct shmem_lfa, additional_config));
  ...
  if (additional_config & NO_LFA_DUE_TO_DCC_MASK) {
    ...
    REG_WR(sc, params->lfa_base + offsetof(struct shmem_lfa, additional_config),
  ...
  link_status = REG_RD(
      sc, params->shmem_base +
              offsetof(struct shmem_region, port_mb[params->port].link_status));
  ...
  /* Compare Duplex */
  saved_val =
      REG_RD(sc, params->lfa_base + offsetof(struct shmem_lfa, req_duplex));
  req_val = params->req_duplex[0] | (params->req_duplex[1] << 16);
  if ((saved_val & lfa_mask) != (req_val & lfa_mask)) {
    ELINK_DEBUG_P2(sc, "Duplex mismatch %x vs. %x", (saved_val & lfa_mask),
                   (req_val & lfa_mask));
    return LFA_DUPLEX_MISMATCH;
  }
  /* Compare Flow Control */
  saved_val =
      REG_RD(sc, params->lfa_base + offsetof(struct shmem_lfa, req_flow_ctrl));
  ...
  /* Compare Link Speed */
  saved_val =
      REG_RD(sc, params->lfa_base + offsetof(struct shmem_lfa, req_line_speed));
  req_val = params->req_line_speed[0] | (params->req_line_speed[1] << 16);
  if ((saved_val & lfa_mask) != (req_val & lfa_mask)) {
    ELINK_DEBUG_P2(sc, "Link speed mismatch %x vs. %x", (saved_val & lfa_mask),
                   (req_val & lfa_mask));
  ...
  for (cfg_idx = 0; cfg_idx < cfg_size; cfg_idx++) {
    cur_speed_cap_mask =
        REG_RD(sc, params->lfa_base +
                       offsetof(struct shmem_lfa, speed_cap_mask[cfg_idx]));

    if (cur_speed_cap_mask != params->speed_cap_mask[cfg_idx]) {
      ELINK_DEBUG_P2(sc, "Speed Cap mismatch %x vs. %x", cur_speed_cap_mask,
  ...
  cur_req_fc_auto_adv =
      REG_RD(sc,
             params->lfa_base + offsetof(struct shmem_lfa, additional_config)) &
      REQ_FC_AUTO_ADV_MASK;

  if ((uint16_t)cur_req_fc_auto_adv != params->req_fc_auto_adv) {
    ELINK_DEBUG_P2(sc, "Flow Ctrl AN mismatch %x vs. %x", cur_req_fc_auto_adv,
  ...
  if (((eee_status & SHMEM_EEE_LPI_REQUESTED_BIT) ^
    ...
    ELINK_DEBUG_P2(sc, "EEE mismatch %x vs. %x", params->eee_mode, eee_status);
...
static void elink_set_epio(struct bnx2x_softc *sc, uint32_t epio_pin,
                           uint32_t en) {
...
static uint32_t elink_ets_get_credit_upper_bound(const uint32_t min_w_val) {
  const uint32_t credit_upper_bound =
      (uint32_t)ELINK_MAXVAL((150 * min_w_val), ELINK_MAX_PACKET_SIZE);
 ...
 ******************************************************************************/
static void
elink_ets_e3b0_set_credit_upper_bound_nig(const struct elink_params *params,
  ...
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_UPPER_BOUND_0
                : NIG_REG_P0_TX_ARB_CREDIT_UPPER_BOUND_0,
         credit_upper_bound);
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_UPPER_BOUND_1
                : NIG_REG_P0_TX_ARB_CREDIT_UPPER_BOUND_1,
         credit_upper_bound);
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_UPPER_BOUND_2
                : NIG_REG_P0_TX_ARB_CREDIT_UPPER_BOUND_2,
         credit_upper_bound);
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_UPPER_BOUND_3
                : NIG_REG_P0_TX_ARB_CREDIT_UPPER_BOUND_3,
         credit_upper_bound);
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_UPPER_BOUND_4
                : NIG_REG_P0_TX_ARB_CREDIT_UPPER_BOUND_4,
         credit_upper_bound);
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_UPPER_BOUND_5
                : NIG_REG_P0_TX_ARB_CREDIT_UPPER_BOUND_5,
         credit_upper_bound);

  if (!port) {
    REG_WR(sc, NIG_REG_P0_TX_ARB_CREDIT_UPPER_BOUND_6, credit_upper_bound);
    REG_WR(sc, NIG_REG_P0_TX_ARB_CREDIT_UPPER_BOUND_7, credit_upper_bound);
    REG_WR(sc, NIG_REG_P0_TX_ARB_CREDIT_UPPER_BOUND_8, credit_upper_bound);
  }
}
/******************************************************************************
 * Description:
...
static void elink_ets_e3b0_nig_disabled(const struct elink_params *params,
   ...
   */
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_NUM_STRICT_ARB_SLOTS
  ...
  if (port) {
    ...
    /*Port 0 has 9 COS*/
    REG_WR(sc, NIG_REG_P0_TX_ARB_CLIENT_CREDIT_MAP2_LSB, 0x43210876);
  ...
  /* defines which entries (clients) are subjected to WFQ arbitration */
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CLIENT_IS_SUBJECT2WFQ
  ...
  REG_WR(sc,
         ...
         0x0);
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_WEIGHT_1
  ...
  REG_WR(sc,
         ...
         0x0);
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_WEIGHT_3
                : NIG_REG_P0_TX_ARB_CREDIT_WEIGHT_3,
         0x0);
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_WEIGHT_4
                : NIG_REG_P0_TX_ARB_CREDIT_WEIGHT_4,
         0x0);
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CREDIT_WEIGHT_5
 ...
 ******************************************************************************/
static void
elink_ets_e3b0_set_credit_upper_bound_pbf(const struct elink_params *params,
                                          const uint32_t min_w_val) {
  struct bnx2x_softc *sc = params->sc;
  const uint32_t credit_upper_bound =
      elink_ets_get_credit_upper_bound(min_w_val);
...
static void elink_ets_e3b0_pbf_disabled(const struct elink_params *params) {
  ...
  REG_WR(sc,
         (port) ? PBF_REG_ETS_ARB_NUM_STRICT_ARB_SLOTS_P1
                : PBF_REG_ETS_ARB_NUM_STRICT_ARB_SLOTS_P0,
         0x100);

  REG_WR(sc,
         (port) ? PBF_REG_ETS_ARB_CLIENT_IS_STRICT_P1
                : PBF_REG_ETS_ARB_CLIENT_IS_STRICT_P0,
         0);

  REG_WR(sc,
         (port) ? PBF_REG_ETS_ARB_CLIENT_IS_SUBJECT2WFQ_P1
...
static elink_status_t elink_ets_e3b0_disabled(const struct elink_params *params,
  ...
  if (!CHIP_IS_E3B0(sc)) {
    ELINK_DEBUG_P0(sc, "elink_ets_e3b0_disabled the chip isn't E3B0");
    return ELINK_STATUS_ERROR;
  }
...
elink_ets_e3b0_cli_map(const struct elink_params *params,
  ...
  REG_WR(sc,
         (port) ? NIG_REG_P1_TX_ARB_CLIENT_IS_STRICT
 ...
 ******************************************************************************/
static elink_status_t
elink_ets_e3b0_set_cos_bw(struct bnx2x_softc *sc, const uint8_t cos_entry,
                          const uint32_t min_w_val_nig,
                          const uint32_t min_w_val_pbf, const uint16_t total_bw,
                          const uint8_t bw, const uint8_t port) {
  ...
  switch (cos_entry) {
  case 0:
    nig_reg_address_crd_weight = (port) ? NIG_REG_P1_TX_ARB_CREDIT_WEIGHT_0
                                        : NIG_REG_P0_TX_ARB_CREDIT_WEIGHT_0;
    pbf_reg_address_crd_weight =
        (port) ? PBF_REG_COS0_WEIGHT_P1 : PBF_REG_COS0_WEIGHT_P0;
    break;
  case 1:
    nig_reg_address_crd_weight = (port) ? NIG_REG_P1_TX_ARB_CREDIT_WEIGHT_1
                                        : NIG_REG_P0_TX_ARB_CREDIT_WEIGHT_1;
    pbf_reg_address_crd_weight =
        (port) ? PBF_REG_COS1_WEIGHT_P1 : PBF_REG_COS1_WEIGHT_P0;
    break;
  case 2:
    nig_reg_address_crd_weight = (port) ? NIG_REG_P1_TX_ARB_CREDIT_WEIGHT_2
                                        : NIG_REG_P0_TX_ARB_CREDIT_WEIGHT_2;

    pbf_reg_address_crd_weight =
        (port) ? PBF_REG_COS2_WEIGHT_P1 : PBF_REG_COS2_WEIGHT_P0;
    break;
  case 3:
    if (port)
      return ELINK_STATUS_ERROR;
    nig_reg_address_crd_weight = NIG_REG_P0_TX_ARB_CREDIT_WEIGHT_3;
    pbf_reg_address_crd_weight = PBF_REG_COS3_WEIGHT_P0;
    break;
  case 4:
    if (port)
      return ELINK_STATUS_ERROR;
    nig_reg_address_crd_weight = NIG_REG_P0_TX_ARB_CREDIT_WEIGHT_4;
    pbf_reg_address_crd_weight = PBF_REG_COS4_WEIGHT_P0;
    break;
  case 5:
    if (port)
      return ELINK_STATUS_ERROR;
    nig_reg_address_crd_weight = NIG_REG_P0_TX_ARB_CREDIT_WEIGHT_5;
 ...
 ******************************************************************************/
static elink_status_t
elink_ets_e3b0_get_total_bw(const struct elink_params *params,
  ...
  *total_bw = 0;
  ...
  for (cos_idx = 0; cos_idx < ets_params->num_of_cos; cos_idx++) {
    if (ets_params->cos[cos_idx].state == elink_cos_state_bw) {
      ...
      if (!ets_params->cos[cos_idx].params.bw_params.bw) {
         ...
         * can't be sent
         */
        ets_params->cos[cos_idx].params.bw_params.bw = 1;
      }
      *total_bw += ets_params->cos[cos_idx].params.bw_params.bw;
    }
  }
  ...
  if ((is_bw_cos_exist == 1) && (*total_bw != 100)) {
    if (*total_bw == 0) {
      ELINK_DEBUG_P0(sc, "elink_ets_E3B0_config total BW shouldn't be 0");
      return ELINK_STATUS_ERROR;
    }
    ELINK_DEBUG_P0(sc, "elink_ets_E3B0_config total BW should be 100");
 ...
 ******************************************************************************/
static elink_status_t
elink_ets_e3b0_sp_pri_to_cos_set(const struct elink_params *params,
...
static uint64_t elink_e3b0_sp_get_pri_cli_reg(const uint8_t cos,
  ...
  pri_cli_nig = ((uint64_t)(cos + cos_offset))
                << (entry_size * (pri_set + pri_offset));
 ...
 ******************************************************************************/
static elink_status_t
elink_ets_e3b0_sp_set_pri_cli_reg(const struct elink_params *params,
  ...
  for (i = 0; i < max_num_of_cos; i++) {
    if (sp_pri_to_cos[i] != DCBX_INVALID_COS) {
      if (sp_pri_to_cos[i] >= ELINK_DCBX_MAX_NUM_COS) {
        ELINK_DEBUG_P0(sc, "elink_ets_e3b0_sp_set_pri_cli_reg "
                           "invalid cos entry");
        return ELINK_STATUS_ERROR;
      }

      pri_cli_nig |=
          elink_e3b0_sp_get_pri_cli_reg_nig(sp_pri_to_cos[i], pri_set);

      pri_cli_pbf |=
          elink_e3b0_sp_get_pri_cli_reg_pbf(sp_pri_to_cos[i], pri_set);
  ...
  for (i = 0; i < max_num_of_cos; i++) {
    ...
    if (pri_bitmask & cos_bit_to_set) {
      ...
      pri_cli_nig |= elink_e3b0_sp_get_pri_cli_reg_nig(i, pri_set);

      pri_cli_pbf |= elink_e3b0_sp_get_pri_cli_reg_pbf(i, pri_set);
      /* COS is used remove it from bitmap.*/
      cos_bit_to_set &= ~pri_bitmask;
      pri_set++;
    }
  ...
  if (port) {
    ...
    REG_WR(sc, NIG_REG_P1_TX_ARB_PRIORITY_CLIENT2_LSB, (uint32_t)pri_cli_nig);
    ...
    REG_WR(sc, NIG_REG_P0_TX_ARB_PRIORITY_CLIENT2_LSB, pri_cli_nig_lsb);
    REG_WR(sc, NIG_REG_P0_TX_ARB_PRIORITY_CLIENT2_MSB, pri_cli_nig_msb);
...
elink_status_t elink_ets_e3b0_config(const struct elink_params *params,
  ...
  elink_status = elink_ets_e3b0_get_total_bw(params, ets_params, &total_bw);
  if (elink_status != ELINK_STATUS_OK) {
    ELINK_DEBUG_P0(sc, "elink_ets_E3B0_config get_total_bw failed");
    return ELINK_STATUS_ERROR;
  }
  ...
  for (cos_entry = 0; cos_entry < ets_params->num_of_cos; cos_entry++) {
    if (elink_cos_state_bw == ets_params->cos[cos_entry].state) {
      ...
      elink_status = elink_ets_e3b0_set_cos_bw(
          sc, cos_entry, min_w_val_nig, min_w_val_pbf, total_bw,
          ets_params->cos[cos_entry].params.bw_params.bw, port);
    } else if (elink_cos_state_strict == ets_params->cos[cos_entry].state) {
      cos_sp_bitmap |= (1 << cos_entry);

      elink_status = elink_ets_e3b0_sp_pri_to_cos_set(
          params, sp_pri_to_cos,
          ets_params->cos[cos_entry].params.sp_params.pri, cos_entry);

    } else {
      ELINK_DEBUG_P0(sc, "elink_ets_e3b0_config cos state not valid");
      return ELINK_STATUS_ERROR;
    }
    if (elink_status != ELINK_STATUS_OK) {
      ELINK_DEBUG_P0(sc, "elink_ets_e3b0_config set cos bw failed");
      return elink_status;
    }
  }

  /* Set SP register (which COS has higher priority) */
  elink_status = elink_ets_e3b0_sp_set_pri_cli_reg(params, sp_pri_to_cos);

  if (elink_status != ELINK_STATUS_OK) {
    ELINK_DEBUG_P0(sc, "elink_ets_E3B0_config set_pri_cli_reg failed");
    return elink_status;
  }

  /* Set client mapping of BW and strict */
  elink_status =
      elink_ets_e3b0_cli_map(params, ets_params, cos_sp_bitmap, cos_bw_bitmap);
...
static void elink_ets_bw_limit_common(const struct elink_params *params) {
  ...
  /* Upper bound that COS0_WEIGHT can reach in the WFQ arbiter.*/
  REG_WR(sc, PBF_REG_COS0_UPPER_BOUND, ELINK_ETS_BW_LIMIT_CREDIT_UPPER_BOUND);
  REG_WR(sc, PBF_REG_COS1_UPPER_BOUND, ELINK_ETS_BW_LIMIT_CREDIT_UPPER_BOUND);
}
...
void elink_ets_bw_limit(const struct elink_params *params,
  ...
  if ((!total_bw) || (!cos0_bw) || (!cos1_bw)) {
    ELINK_DEBUG_P0(sc, "Total BW can't be zero");
    return;
  }

  cos0_credit_weight = (cos0_bw * ELINK_ETS_BW_LIMIT_CREDIT_WEIGHT) / total_bw;
  cos1_credit_weight = (cos1_bw * ELINK_ETS_BW_LIMIT_CREDIT_WEIGHT) / total_bw;
...
static void elink_update_pfc_xmac(struct elink_params *params,
  ...
  if (!(params->feature_config_flags & ELINK_FEATURE_CONFIG_PFC_ENABLED)) {
    ...
    pfc1_val |=
        ...
        XMAC_PFC_CTRL_HI_REG_PFC_STATS_EN | XMAC_PFC_CTRL_HI_REG_RX_PFC_EN |
        XMAC_PFC_CTRL_HI_REG_TX_PFC_EN | XMAC_PFC_CTRL_HI_REG_FORCE_PFC_XON;
  ...
  REG_WR(sc, xmac_base + XMAC_REG_CTRL_SA_LO,
         ((params->mac_addr[2] << 24) | (params->mac_addr[3] << 16) |
...
static void elink_emac_get_pfc_stat(struct elink_params *params,
  ...
  /* PFC received frames */
  val_xoff = REG_RD(sc, emac_base + EMAC_REG_RX_PFC_STATS_XOFF_RCVD);
  ...
  /* PFC received sent */
  val_xoff = REG_RD(sc, emac_base + EMAC_REG_RX_PFC_STATS_XOFF_SENT);
...
void elink_pfc_statistic(struct elink_params *params, struct elink_vars *vars,
  ...
  if (vars->mac_type == ELINK_MAC_TYPE_EMAC) {
    ...
    elink_emac_get_pfc_stat(params, pfc_frames_sent, pfc_frames_received);
  }
}
/******************************************************************/
/*			MAC/PBF section				  */
...
static void elink_set_mdio_clk(struct bnx2x_softc *sc,
  ...
  new_mode = cur_mode & ~(EMAC_MDIO_MODE_AUTO_POLL | EMAC_MDIO_MODE_CLOCK_CNT);
  new_mode |= clc_cnt;
  new_mode |= (EMAC_MDIO_MODE_CLAUSE_45);

  ELINK_DEBUG_P2(sc, "Changing emac_mode from 0x%x to 0x%x", cur_mode,
                 new_mode);
...
static void elink_set_mdio_emac_per_phy(struct bnx2x_softc *sc,
  ...
  for (phy_index = ELINK_INT_PHY; phy_index < params->num_phys; phy_index++)
    elink_set_mdio_clk(sc, params->chip_id, params->phy[phy_index].mdio_ctrl);
}
...
static void elink_set_xumac_nig(struct elink_params *params,
  ...
  REG_WR(sc, params->port ? NIG_REG_P1_MAC_OUT_EN : NIG_REG_P0_MAC_OUT_EN,
         enable);
  REG_WR(sc,
         params->port ? NIG_REG_P1_MAC_PAUSE_OUT_EN
...
static void elink_umac_enable(struct elink_params *params,
  ...
  val = UMAC_COMMAND_CONFIG_REG_PROMIS_EN | UMAC_COMMAND_CONFIG_REG_PAD_EN |
  ...
  elink_set_xumac_nig(params, ((vars->flow_ctrl & ELINK_FLOW_CTRL_TX) != 0), 1);
...
static void elink_xmac_init(struct elink_params *params, uint32_t max_speed) {
  ...
  if (((CHIP_NUM(sc) == CHIP_NUM_57840_4_10) ||
      ...
      is_port4mode &&
      (REG_RD(sc, MISC_REG_RESET_REG_2) & MISC_REGISTERS_RESET_REG_2_XMAC)) {
    ELINK_DEBUG_P0(sc, "XMAC already out of reset in 4-port mode");
    return;
  }
  ...
  if (is_port4mode) {
    ...
    if (max_speed == ELINK_SPEED_10000) {
      ELINK_DEBUG_P0(sc, "Init XMAC to 10G x 1 port per path");
      /* Set the number of ports on the Warp Core to 10G */
      REG_WR(sc, MISC_REG_XMAC_PHY_PORT_MODE, 3);
    } else {
      ELINK_DEBUG_P0(sc, "Init XMAC to 20G x 2 ports per path");
...
static void elink_set_xmac_rxtx(struct elink_params *params, uint8_t en) {
  ...
  if (REG_RD(sc, MISC_REG_RESET_REG_2) & MISC_REGISTERS_RESET_REG_2_XMAC) {
    ...
    REG_WR(sc, xmac_base + XMAC_REG_PFC_CTRL_HI, (pfc_ctrl & ~(1 << 1)));
    REG_WR(sc, xmac_base + XMAC_REG_PFC_CTRL_HI, (pfc_ctrl | (1 << 1)));
...
static elink_status_t elink_emac_enable(struct elink_params *params,
  ...
  /* for fpga */
    ...
    uint32_t ser_lane =
        ((params->lane_config & PORT_HW_CFG_LANE_SWAP_CFG_MASTER_MASK) >>
  ...
  elink_bits_en(sc, emac_base + EMAC_REG_EMAC_RX_MODE, EMAC_RX_MODE_RESET);
  elink_bits_en(sc, emac_base + EMAC_REG_EMAC_TX_MODE, EMAC_TX_MODE_RESET);
  ...
  if (CHIP_REV_IS_SLOW(sc)) {
    ...
    elink_bits_dis(sc, emac_base + EMAC_REG_EMAC_RX_MODE, EMAC_RX_MODE_FLOW_EN);
  ...
  if (params->feature_config_flags & ELINK_FEATURE_CONFIG_PFC_ENABLED) {
    ...
    elink_cb_reg_write(sc, emac_base + EMAC_REG_RX_PFC_MODE,
                       EMAC_REG_RX_PFC_MODE_RX_EN | EMAC_REG_RX_PFC_MODE_TX_EN |
                           EMAC_REG_RX_PFC_MODE_PRIORITIES);

    elink_cb_reg_write(
        sc, emac_base + EMAC_REG_RX_PFC_PARAM,
        ((0x0101 << EMAC_REG_RX_PFC_PARAM_OPCODE_BITSHIFT) |
         (0x00ff << EMAC_REG_RX_PFC_PARAM_PRIORITY_EN_BITSHIFT)));
...
static void elink_update_pfc_bmac1(struct elink_params *params,
  ...
  struct bnx2x_softc *sc = params->sc;
  uint32_t bmac_addr =
      params->port ? NIG_REG_INGRESS_BMAC1_MEM : NIG_REG_INGRESS_BMAC0_MEM;
...
static void elink_update_pfc_bmac2(struct elink_params *params,
  ...
  wb_data[0] = val;
  wb_data[1] = 0;
  REG_WR_DMAE(sc, bmac_addr + BIGMAC2_REGISTER_TX_PAUSE_CONTROL, wb_data, 2);
...
static elink_status_t elink_pfc_nig_rx_priority_mask(struct bnx2x_softc *sc,
  ...
  switch (cos_entry) {
  case 0:
    nig_reg_rx_priority_mask_add = (port) ? NIG_REG_P1_RX_COS0_PRIORITY_MASK
                                          : NIG_REG_P0_RX_COS0_PRIORITY_MASK;
    break;
  case 1:
    nig_reg_rx_priority_mask_add = (port) ? NIG_REG_P1_RX_COS1_PRIORITY_MASK
                                          : NIG_REG_P0_RX_COS1_PRIORITY_MASK;
    break;
  case 2:
    nig_reg_rx_priority_mask_add = (port) ? NIG_REG_P1_RX_COS2_PRIORITY_MASK
...
static void
elink_update_pfc_nig(struct elink_params *params,
                     __rte_unused struct elink_vars *vars,
  ...
  int set_pfc = params->feature_config_flags & ELINK_FEATURE_CONFIG_PFC_ENABLED;
   ...
   */
  xcm_mask = REG_RD(sc, port ? NIG_REG_LLH1_XCM_MASK : NIG_REG_LLH0_XCM_MASK);
  ...
  if (CHIP_IS_E3(sc))
    REG_WR(sc, port ? NIG_REG_BRB1_PAUSE_IN_EN : NIG_REG_BRB0_PAUSE_IN_EN,
           pause_enable);
  REG_WR(sc, port ? NIG_REG_LLFC_OUT_EN_1 : NIG_REG_LLFC_OUT_EN_0, llfc_out_en);
  REG_WR(sc, port ? NIG_REG_LLFC_ENABLE_1 : NIG_REG_LLFC_ENABLE_0, llfc_enable);
  REG_WR(sc, port ? NIG_REG_PAUSE_ENABLE_1 : NIG_REG_PAUSE_ENABLE_0,
         pause_enable);

  REG_WR(sc, port ? NIG_REG_PPP_ENABLE_1 : NIG_REG_PPP_ENABLE_0, ppp_enable);

  REG_WR(sc, port ? NIG_REG_LLH1_XCM_MASK : NIG_REG_LLH0_XCM_MASK, xcm_mask);

  REG_WR(sc,
         port ? NIG_REG_LLFC_EGRESS_SRC_ENABLE_1
              : NIG_REG_LLFC_EGRESS_SRC_ENABLE_0,
         0x7);

  /* Output enable for RX_XCM # IF */
  REG_WR(sc, port ? NIG_REG_XCM1_OUT_EN : NIG_REG_XCM0_OUT_EN, xcm_out_en);

  /* HW PFC TX enable */
  REG_WR(sc, port ? NIG_REG_P1_HWPFC_ENABLE : NIG_REG_P0_HWPFC_ENABLE,
         hwpfc_enable);
  ...
  if (nig_params) {
    ...
    for (i = 0; i < nig_params->num_of_rx_cos_priority_mask; i++)
      elink_pfc_nig_rx_priority_mask(sc, i, nig_params->rx_cos_priority_mask[i],
                                     port);

    REG_WR(sc,
           port ? NIG_REG_LLFC_HIGH_PRIORITY_CLASSES_1
                : NIG_REG_LLFC_HIGH_PRIORITY_CLASSES_0,
           nig_params->llfc_high_priority_classes);

    REG_WR(sc,
           port ? NIG_REG_LLFC_LOW_PRIORITY_CLASSES_1
                : NIG_REG_LLFC_LOW_PRIORITY_CLASSES_0,
           nig_params->llfc_low_priority_classes);
  }
  REG_WR(sc,
         port ? NIG_REG_P1_PKT_PRIORITY_TO_COS : NIG_REG_P0_PKT_PRIORITY_TO_COS,
         pkt_priority_to_cos);
}
...
elink_update_pfc(struct elink_params *params, struct elink_vars *vars,
  ...
  if (CHIP_IS_E3(sc)) {
    ...
    if ((val & (MISC_REGISTERS_RESET_REG_2_RST_BMAC0 << params->port)) == 0) {
...
static elink_status_t elink_bmac1_enable(struct elink_params *params,
  ...
  uint8_t port = params->port;
  uint32_t bmac_addr =
      port ? NIG_REG_INGRESS_BMAC1_MEM : NIG_REG_INGRESS_BMAC0_MEM;
  uint32_t wb_data[2];
  uint32_t val;
  ...
  wb_data[0] = 0x3c;
  wb_data[1] = 0;
  REG_WR_DMAE(sc, bmac_addr + BIGMAC_REGISTER_BMAC_XGXS_CONTROL, wb_data, 2);

  /* TX MAC SA */
  wb_data[0] = ((params->mac_addr[2] << 24) | (params->mac_addr[3] << 16) |
                (params->mac_addr[4] << 8) | params->mac_addr[5]);
  ...
  wb_data[0] = 0x1000200;
  wb_data[1] = 0;
  REG_WR_DMAE(sc, bmac_addr + BIGMAC_REGISTER_RX_LLFC_MSG_FLDS, wb_data, 2);
#ifdef ELINK_INCLUDE_EMUL
  /* Fix for emulation */
  if (CHIP_REV_IS_EMUL(sc)) {
    wb_data[0] = 0xf000;
    wb_data[1] = 0;
    REG_WR_DMAE(sc, bmac_addr + BIGMAC_REGISTER_TX_PAUSE_THRESHOLD, wb_data, 2);
  }
#endif
...
static elink_status_t elink_bmac2_enable(struct elink_params *params,
  ...
  wb_data[0] = 0x3c;
  wb_data[1] = 0;
  REG_WR_DMAE(sc, bmac_addr + BIGMAC2_REGISTER_BMAC_XGXS_CONTROL, wb_data, 2);
  ...
  wb_data[0] = ((params->mac_addr[2] << 24) | (params->mac_addr[3] << 16) |
                (params->mac_addr[4] << 8) | params->mac_addr[5]);
  wb_data[1] = ((params->mac_addr[0] << 8) | params->mac_addr[1]);
  REG_WR_DMAE(sc, bmac_addr + BIGMAC2_REGISTER_TX_SOURCE_ADDR, wb_data, 2);
  ...
  wb_data[0] = 0x1000200;
  wb_data[1] = 0;
  REG_WR_DMAE(sc, bmac_addr + BIGMAC2_REGISTER_RX_LLFC_MSG_FLDS, wb_data, 2);
  DELAY(30);
...
static void elink_set_bmac_rx(struct bnx2x_softc *sc,
                              __rte_unused uint32_t chip_id, uint8_t port,
                              uint8_t en) {
  uint32_t bmac_addr =
      port ? NIG_REG_INGRESS_BMAC1_MEM : NIG_REG_INGRESS_BMAC0_MEM;
  uint32_t wb_data[2];
...
static elink_status_t elink_pbf_update(struct elink_params *params,
  ...
  crd = REG_RD(sc, PBF_REG_P0_CREDIT + port * 8);
  if (init_crd != crd) {
    ELINK_DEBUG_P2(sc, "BUG! init_crd 0x%x != crd 0x%x", init_crd, crd);
    return ELINK_STATUS_ERROR;
  }

  if (flow_ctrl & ELINK_FLOW_CTRL_RX || line_speed == ELINK_SPEED_10 ||
      line_speed == ELINK_SPEED_100 || line_speed == ELINK_SPEED_1000 ||
    ...
    uint32_t thresh =
        (ELINK_ETH_MAX_JUMBO_PACKET_SIZE + ELINK_ETH_OVREHEAD) / 16;
  ...
  ELINK_DEBUG_P2(sc, "PBF updated to speed %d credit %d", line_speed, init_crd);
...
static elink_status_t elink_cl22_write(struct bnx2x_softc *sc,
                                       struct elink_phy *phy, uint16_t reg,
                                       uint16_t val) {
  uint32_t tmp, mode;
  uint8_t i;
  elink_status_t rc = ELINK_STATUS_OK;
...
static elink_status_t elink_cl22_read(struct bnx2x_softc *sc,
  ...
  /* Address */
  val = ((phy->addr << 21) | (reg << 16) | EMAC_MDIO_COMM_COMMAND_READ_22 |
...
static elink_status_t elink_cl45_read(struct bnx2x_softc *sc,
  ...
  if (val & EMAC_MDIO_COMM_START_BUSY) {
    ...
    *ret_val = 0;
    ...
    /* Data */
    val = ((phy->addr << 21) | (devad << 16) | EMAC_MDIO_COMM_COMMAND_READ_45 |
           EMAC_MDIO_COMM_START_BUSY);
    REG_WR(sc, phy->mdio_ctrl + EMAC_REG_EMAC_MDIO_COMM, val);
...
static elink_status_t elink_cl45_write(struct bnx2x_softc *sc,
  ...
  if (tmp & EMAC_MDIO_COMM_START_BUSY) {
    ...
    tmp = ((phy->addr << 21) | (devad << 16) | val |
           EMAC_MDIO_COMM_COMMAND_WRITE_45 | EMAC_MDIO_COMM_START_BUSY);
    REG_WR(sc, phy->mdio_ctrl + EMAC_REG_EMAC_MDIO_COMM, tmp);
...
static uint32_t elink_eee_calc_timer(struct elink_params *params) {
  ...
  if (params->eee_mode & ELINK_EEE_MODE_OVERRIDE_NVRAM) {
    if (params->eee_mode & ELINK_EEE_MODE_OUTPUT_TIME) {
      ...
      if (elink_eee_nvram_to_time(params->eee_mode & ELINK_EEE_MODE_NVRAM_MASK,
                                  &eee_idle))
        return 0;
    }
  } else {
...
static elink_status_t elink_eee_set_timers(struct elink_params *params,
  ...
  if (eee_idle) {
    REG_WR(sc, MISC_REG_CPMU_LP_IDLE_THR_P0 + (params->port << 2), eee_idle);
  ...
  if (params->eee_mode & ELINK_EEE_MODE_OUTPUT_TIME) {
    ...
    eee_idle >>= 4;
    vars->eee_status |=
        (eee_idle & SHMEM_EEE_TIMER_MASK) | SHMEM_EEE_TIME_OUTPUT_BIT;
  } else {
...
static void elink_bsc_module_sel(struct elink_params *params) {
  ...
  board_cfg = REG_RD(
      sc, params->shmem_base +
              offsetof(struct shmem_region, dev_info.shared_hw_config.board));
...
static elink_status_t elink_bsc_read(struct bnx2x_softc *sc, uint8_t sl_devid,
                                     uint16_t sl_addr, uint8_t lc_addr,
                                     uint8_t xfer_cnt, uint32_t *data_array) {
  uint32_t val, i;
  elink_status_t rc = ELINK_STATUS_OK;

  if (xfer_cnt > 16) {
    ELINK_DEBUG_P1(sc, "invalid xfer_cnt %d. Max is 16 bytes", xfer_cnt);
    return ELINK_STATUS_ERROR;
  }
  ...
  val = (MCPR_IMC_COMMAND_ENABLE) |
        (MCPR_IMC_COMMAND_WRITE_OP << MCPR_IMC_COMMAND_OPERATION_BITSHIFT) |
  ...
  while (((val >> MCPR_IMC_COMMAND_IMC_STATUS_BITSHIFT) & 0x3) != 1) {
    ...
    val = REG_RD(sc, MCP_REG_MCPR_IMC_COMMAND);
    if (i++ > 1000) {
      ELINK_DEBUG_P1(sc, "wr 0 byte timed out after %d try", i);
      rc = ELINK_STATUS_TIMEOUT;
      break;
    }
  }
  ...
  val = (MCPR_IMC_COMMAND_ENABLE) |
        (MCPR_IMC_COMMAND_READ_OP << MCPR_IMC_COMMAND_OPERATION_BITSHIFT) |
        (lc_addr << MCPR_IMC_COMMAND_TRANSFER_ADDRESS_BITSHIFT) | (xfer_cnt);
  REG_WR(sc, MCP_REG_MCPR_IMC_COMMAND, val);
...
elink_status_t elink_phy_read(struct elink_params *params, uint8_t phy_addr,
  ...
  for (phy_index = 0; phy_index < params->num_phys; phy_index++) {
    if (params->phy[phy_index].addr == phy_addr) {
      return elink_cl45_read(params->sc, &params->phy[phy_index], devad, reg,
                             ret_val);
    }
  }
  return ELINK_STATUS_ERROR;
...
elink_status_t elink_phy_write(struct elink_params *params, uint8_t phy_addr,
  ...
  for (phy_index = 0; phy_index < params->num_phys; phy_index++) {
    if (params->phy[phy_index].addr == phy_addr) {
      return elink_cl45_write(params->sc, &params->phy[phy_index], devad, reg,
                              val);
    }
  }
  return ELINK_STATUS_ERROR;
...
static void elink_set_aer_mmd(struct elink_params *params,
  ...
  struct bnx2x_softc *sc = params->sc;
  ser_lane = ((params->lane_config & PORT_HW_CFG_LANE_SWAP_CFG_MASTER_MASK) >>
              PORT_HW_CFG_LANE_SWAP_CFG_MASTER_SHIFT);
  ...
  CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_AER_BLOCK, MDIO_AER_BLOCK_AER_REG,
                    aer_val);
}
...
static void elink_xgxs_deassert(struct elink_params *params) {
  ...
  DELAY(500);
  REG_WR(sc, GRCBASE_MISC + MISC_REGISTERS_RESET_REG_3_SET, val);
  elink_xgxs_specific_func(&params->phy[ELINK_INT_PHY], params, ELINK_PHY_INIT);
}
...
static void set_phy_vars(struct elink_params *params, struct elink_vars *vars) {
  ...
  uint8_t phy_config_swapped =
      params->multi_phy_config & PORT_HW_CFG_PHY_SWAPPED_ENABLED;
  for (phy_index = ELINK_INT_PHY; phy_index < params->num_phys; phy_index++) {
    ...
    params->phy[actual_phy_idx].req_duplex = params->req_duplex[link_cfg_idx];

    if (params->req_line_speed[link_cfg_idx] == ELINK_SPEED_AUTO_NEG)
...
static void elink_ext_phy_update_adv_fc(struct elink_phy *phy,
  ...
  if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY2_TYPE_BNX2X54618SE) {
    ...
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_WC_REG_GP2_STATUS_GP_2_4,
                    &gp_status);
    ...
    if ((gp_status & gp_mask) == gp_mask) {
      elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_ADV_PAUSE, &ld_pause);
      elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_LP_AUTO_NEG,
                      &lp_pause);
    } else {
      elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LD,
                      &ld_pause);
      elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LP,
                      &lp_pause);
      ld_pause = ((ld_pause & MDIO_COMBO_IEEE0_AUTO_NEG_ADV_PAUSE_BOTH) << 3);
      lp_pause = ((lp_pause & MDIO_COMBO_IEEE0_AUTO_NEG_ADV_PAUSE_BOTH) << 3);
    }
  } else {
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_ADV_PAUSE, &ld_pause);
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_LP_AUTO_NEG, &lp_pause);
  }
...
static void elink_update_link_attr(struct elink_params *params,
  ...
  if (SHMEM2_HAS(sc, link_attr_sync))
    REG_WR(sc,
           params->shmem2_base +
               offsetof(struct shmem2_region, link_attr_sync[params->port]),
           link_attr);
}
...
static void elink_warpcore_enable_AN_KR2(struct elink_phy *phy,
  ...
  elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_CL49_USERB0_CTRL,
                           (3 << 6));

  for (i = 0; i < ARRAY_SIZE(reg_set); i++)
    elink_cl45_write(sc, phy, reg_set[i].devad, reg_set[i].reg, reg_set[i].val);
...
static void elink_warpcore_set_lpi_passthrough(struct elink_phy *phy,
  ...
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_EEE_COMBO_CONTROL0,
                   0x7c);
  elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL4_MISC5,
                           0xc000);
}
...
static void elink_warpcore_restart_AN_KR(struct elink_phy *phy,
  ...
  CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_AER_BLOCK, MDIO_AER_BLOCK_AER_REG,
                    lane);
  elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_WC_REG_IEEE0BLK_MIICNTL,
                   0x1200);
...
static void elink_warpcore_enable_AN_KR(struct elink_phy *phy,
  ...
  if (((vars->line_speed == ELINK_SPEED_AUTO_NEG) &&
    ...
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_WC_REG_PAR_DET_10G_CTRL, 1);
    elink_set_aer_mmd(params, phy);
  ...
  if (REG_RD(sc,
    ...
    elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD,
                             MDIO_WC_REG_DIGITAL6_MP5_NEXTPAGECTRL, 1);
  ...
  elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL5_MISC7,
                           0x100);

  /* Over 1G - AN local device user page 1 */
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL3_UP1, 0x1f);
  ...
  if (((phy->req_line_speed == ELINK_SPEED_AUTO_NEG) &&
    ...
    elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_XGXS_X2_CONTROL3, 0x7);
    elink_set_aer_mmd(params, phy);
    ...
    wc_lane_config =
        REG_RD(sc, params->shmem_base +
                       offsetof(struct shmem_region,
                                dev_info.shared_hw_config.wc_lane_config));
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD,
     ...
     * previous link owner
     */
    if (wc_lane_config & (SHARED_HW_CFG_RX_LANE0_POL_FLIP_ENABLED << lane))
      val |= 3 << 2;
    else
      val &= ~(3 << 2);
    elink_cl45_write(sc, phy, MDIO_WC_DEVAD,
...
static void elink_warpcore_set_10G_KR(struct elink_phy *phy,
  ...
  static struct elink_reg_set reg_set[] = {
      /* Disable Autoneg */
      {MDIO_WC_DEVAD, MDIO_WC_REG_SERDESDIGITAL_CONTROL1000X2, 0x7},
      {MDIO_WC_DEVAD, MDIO_WC_REG_CL72_USERB0_CL72_MISC1_CONTROL, 0x3f00},
  ...
  CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_AER_BLOCK, MDIO_AER_BLOCK_AER_REG,
                    0);
  /* Disable CL36 PCS Tx */
  elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_XGXSBLK1_LANECTRL0,
                  &val16);
  val16 &= ~(0x0011 << lane);
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_XGXSBLK1_LANECTRL0,
                   val16);

  elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_XGXSBLK1_LANECTRL1,
                  &val16);
  val16 |= (0x0303 << (lane << 1));
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_XGXSBLK1_LANECTRL1,
                   val16);
  /* Restore AER */
  elink_set_aer_mmd(params, phy);
  /* Set speed via PMA/PMD register */
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_WC_REG_IEEE0BLK_MIICNTL,
                   0x2040);

  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_WC_REG_IEEE0BLK_AUTONEGNP,
                   0xB);

  /* Enable encoded forced speed */
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_SERDESDIGITAL_MISC2,
                   0x30);
  ...
  elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_RX66_CONTROL,
                           0xF9);

  /* Set and clear loopback to cause a reset to 64/66 decoder */
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_IEEE0BLK_MIICNTL,
                   0x4000);
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_IEEE0BLK_MIICNTL, 0x0);
}
...
static void elink_warpcore_set_10G_XFI(struct elink_phy *phy,
  ...
  elink_cl45_read_and_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_FX100_CTRL1,
                            0xFFFA);

  /* Disable 100FX Idle detect */
  elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_FX100_CTRL3,
                           0x0080);

  /* Set Block address to Remote PHY & Clear forced_speed[5] */
  elink_cl45_read_and_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL4_MISC3,
                            0xFF7F);

  /* Turn off auto-detect & fiber mode */
  elink_cl45_read_and_write(sc, phy, MDIO_WC_DEVAD,
                            MDIO_WC_REG_SERDESDIGITAL_CONTROL1000X1, 0xFFEE);
  ...
  if (is_xfi) {
    ...
    cfg_tap_val = REG_RD(
        sc, params->shmem_base +
                offsetof(struct shmem_region,
                         dev_info.port_hw_config[params->port].sfi_tap_values));
     ...
     */
    if (cfg_tap_val &
        (PORT_HW_CFG_TX_DRV_IFIR_MASK | PORT_HW_CFG_TX_DRV_IPREDRIVER_MASK |
         PORT_HW_CFG_TX_DRV_POST2_MASK)) {
      ifir_val = (cfg_tap_val & PORT_HW_CFG_TX_DRV_IFIR_MASK) >>
                 PORT_HW_CFG_TX_DRV_IFIR_SHIFT;
      ipre_driver_val = (cfg_tap_val & PORT_HW_CFG_TX_DRV_IPREDRIVER_MASK) >>
                        PORT_HW_CFG_TX_DRV_IPREDRIVER_SHIFT;
      ipost2_val = (cfg_tap_val & PORT_HW_CFG_TX_DRV_POST2_MASK) >>
                   PORT_HW_CFG_TX_DRV_POST2_SHIFT;
    }

    if (cfg_tap_val & PORT_HW_CFG_TX_DRV_BROADCAST_MASK) {
      tx_drv_brdct = (cfg_tap_val & PORT_HW_CFG_TX_DRV_BROADCAST_MASK) >>
                     PORT_HW_CFG_TX_DRV_BROADCAST_SHIFT;
    }

    tx_driver_val =
        WC_TX_DRIVER(ipost2_val, tx_drv_brdct, ipre_driver_val, ifir_val);
  }
  ...
  /* Set Transmit PMD settings */
  lane = elink_get_warpcore_lane(phy, params);
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_TX_FIR_TAP,
                   tap_val | MDIO_WC_REG_TX_FIR_TAP_ENABLE);
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD,
                   MDIO_WC_REG_TX0_TX_DRIVER + 0x10 * lane, tx_driver_val);
  ...
  elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL4_MISC3,
                           0x8080);

  elink_warpcore_set_lpi_passthrough(phy, params);

  /* 10G XFI Full Duplex */
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_IEEE0BLK_MIICNTL, 0x100);

  /* Release tx_fifo_reset */
  elink_cl45_read_and_write(sc, phy, MDIO_WC_DEVAD,
                            MDIO_WC_REG_SERDESDIGITAL_CONTROL1000X3, 0xFFFE);
  /* Release rxSeqStart */
  elink_cl45_read_and_write(sc, phy, MDIO_WC_DEVAD,
...
static void elink_warpcore_set_20G_force_KR2(struct elink_phy *phy,
  ...
  elink_cl45_read_and_write(sc, phy, MDIO_PMA_DEVAD, MDIO_WC_REG_PMD_KR_CONTROL,
                            ~(1 << 1));
  elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CTRL, 0);
  /* Turn off CL73 */
  elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_CL73_USERB0_CTRL, &val);
  val &= ~(1 << 5);
  val |= (1 << 6);
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_CL73_USERB0_CTRL, val);
...
static void elink_warpcore_set_20G_DXGXS(struct bnx2x_softc *sc,
  ...
  /* Rx0 anaRxControl1G */
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_RX0_ANARXCONTROL1G,
                   0x90);

  /* Rx2 anaRxControl1G */
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_RX2_ANARXCONTROL1G,
                   0x90);
  ...
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_RX66_SCW0_MASK, 0xF0F0);

  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_RX66_SCW1_MASK, 0xF0F0);

  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_RX66_SCW2_MASK, 0xF0F0);

  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_RX66_SCW3_MASK, 0xF0F0);
  ...
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL4_MISC3, 0x8088);

  /* Set Transmit PMD settings */
  elink_cl45_write(
      sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_TX_FIR_TAP,
      (WC_TX_FIR(0x12, 0x2d, 0x00) | MDIO_WC_REG_TX_FIR_TAP_ENABLE));
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD,
                   MDIO_WC_REG_TX0_TX_DRIVER + 0x10 * lane,
...
static void elink_warpcore_set_sgmii_speed(struct elink_phy *phy,
  ...
  elink_cl45_read_and_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_RX66_CONTROL,
                            ~(3 << 13));
  ...
  if (always_autoneg || phy->req_line_speed == ELINK_SPEED_AUTO_NEG) {
    /* SGMII Autoneg */
    elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD,
                             MDIO_WC_REG_COMBO_IEEE0_MIICTRL, 0x1000);
    ELINK_DEBUG_P0(sc, "set SGMII AUTONEG");
  } else {
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_COMBO_IEEE0_MIICTRL,
                    &val16);
    val16 &= 0xcebf;
    ...
    case ELINK_SPEED_1000:
      ...
      break;
    default:
      ELINK_DEBUG_P1(sc, "Speed not supported: 0x%x", phy->req_line_speed);
      return;
    }
    ...
    elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_COMBO_IEEE0_MIICTRL,
                     val16);

    ELINK_DEBUG_P1(sc, "set SGMII force speed %d", phy->req_line_speed);
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_COMBO_IEEE0_MIICTRL,
                    &val16);
...
static void elink_warpcore_reset_lane(struct bnx2x_softc *sc,
  ...
  elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL5_MISC6, &val);
  if (reset)
    val |= 0xC000;
  else
    val &= 0x3FFF;
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL5_MISC6, val);
  elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL5_MISC6, &val);
}
...
static void elink_warpcore_clear_regs(struct elink_phy *phy,
  ...
  static struct elink_reg_set wc_regs[] = {
      ...
      {MDIO_WC_DEVAD, MDIO_WC_REG_SERDESDIGITAL_CONTROL1000X1, 0x0195},
      {MDIO_WC_DEVAD, MDIO_WC_REG_SERDESDIGITAL_CONTROL1000X2, 0x0007},
      {MDIO_WC_DEVAD, MDIO_WC_REG_SERDESDIGITAL_CONTROL1000X3, 0x0002},
  ...
  for (i = 0; i < ARRAY_SIZE(wc_regs); i++)
    elink_cl45_write(sc, phy, wc_regs[i].devad, wc_regs[i].reg, wc_regs[i].val);
...
static elink_status_t elink_get_mod_abs_int_cfg(struct bnx2x_softc *sc,
  ...
  *gpio_port = 0;
  if (CHIP_IS_E3(sc)) {
     ...
     */
    if ((cfg_pin < PIN_CFG_GPIO0_P0) || (cfg_pin > PIN_CFG_GPIO3_P1)) {
...
static void elink_warpcore_config_runtime(struct elink_phy *phy,
  ...
  if (vars->rx_tx_asic_rst) {
    ...
    case PORT_HW_CFG_NET_SERDES_IF_KR:
      /* Do we get link yet? */
      elink_cl45_read(sc, phy, MDIO_WC_DEVAD, 0x81d1, &gp_status1);
      ...
      if (lnkup_kr || lnkup) {
        ...
        vars->rx_tx_asic_rst--;
        ELINK_DEBUG_P1(sc, "0x%x retry left", vars->rx_tx_asic_rst);
      }
      break;
  ...
  if ((serdes_net_if == PORT_HW_CFG_NET_SERDES_IF_SGMII) ||
    ...
    case PORT_HW_CFG_NET_SERDES_IF_XFI:
      ...
      if (vars->line_speed == ELINK_SPEED_10000) {
        ...
        elink_warpcore_set_sgmii_speed(phy, params, fiber_mode, 0);
      }
    ...
    case PORT_HW_CFG_NET_SERDES_IF_SFI:
      ...
      if ((params->loopback_mode == ELINK_LOOPBACK_NONE) ||
        ...
        if (elink_is_sfp_module_plugged(phy, params))
          ...
          elink_sfp_e3_set_transmitter(params, phy, 1);
      }
    ...
    case PORT_HW_CFG_NET_SERDES_IF_KR2:
      ...
      break;
    default:
      ELINK_DEBUG_P1(sc, "Unsupported Serdes Net Interface 0x%x",
                     serdes_net_if);
      return 0;
    }
  }
...
static void elink_set_warpcore_loopback(struct elink_phy *phy,
  ...
  if (phy->req_line_speed < ELINK_SPEED_10000 ||
    ...
    lane = elink_get_warpcore_lane(phy, params);
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_XGXSBLK1_LANECTRL2,
                    &val16);
    val16 |= (1 << lane);
    if (phy->flags & ELINK_FLAGS_WC_DUAL_MODE)
      val16 |= (2 << lane);
    elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_XGXSBLK1_LANECTRL2,
                     val16);
    ...
    /* 10G / 20G-DXGXS */
    elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD,
                             MDIO_WC_REG_COMBO_IEEE0_MIICTRL, 0x4000);
    elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD,
...
static void elink_sync_link(struct elink_params *params,
  ...
  if (vars->link_up) {
    ...
    if (vars->line_speed && ((vars->line_speed == ELINK_SPEED_10) ||
                             (vars->line_speed == ELINK_SPEED_100))) {
      vars->phy_flags |= PHY_SGMII_FLAG;
    } else {
      vars->phy_flags &= ~PHY_SGMII_FLAG;
    }
    if (vars->line_speed && USES_WARPCORE(sc) &&
        (vars->line_speed == ELINK_SPEED_1000))
      vars->phy_flags |= PHY_SGMII_FLAG;
...
void elink_link_status_update(struct elink_params *params,
  ...
  if (elink_eee_has_cap(params))
    vars->eee_status =
        REG_RD(sc, params->shmem2_base + offsetof(struct shmem2_region,
  ...
  sync_offset =
      params->shmem_base +
      offsetof(struct shmem_region, dev_info.port_hw_config[port].media_type);
  ...
  sync_offset =
      params->shmem_base +
      offsetof(struct shmem_region, dev_info.port_hw_config[port].aeu_int_mask);

  vars->aeu_int_mask = REG_RD(sc, sync_offset);

  /* Sync PFC status */
  if (vars->link_status & LINK_STATUS_PFC_ENABLED)
    params->feature_config_flags |= ELINK_FEATURE_CONFIG_PFC_ENABLED;
  else
    params->feature_config_flags &= ~ELINK_FEATURE_CONFIG_PFC_ENABLED;

  if (SHMEM2_HAS(sc, link_attr_sync))
    params->link_attr_sync = SHMEM2_RD(sc, link_attr_sync[params->port]);
...
static void elink_set_master_ln(struct elink_params *params,
  ...
  CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_XGXS_BLOCK2,
                    MDIO_XGXS_BLOCK2_TEST_MODE_LANE, &new_master_ln);

  CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_XGXS_BLOCK2,
...
static elink_status_t elink_reset_unicore(struct elink_params *params,
  ...
  uint16_t i;
  CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_COMBO_IEEE0,
...
static void elink_set_swap_lanes(struct elink_params *params,
  ...
  rx_lane_swap = ((params->lane_config & PORT_HW_CFG_LANE_SWAP_CFG_RX_MASK) >>
                  PORT_HW_CFG_LANE_SWAP_CFG_RX_SHIFT);
  tx_lane_swap = ((params->lane_config & PORT_HW_CFG_LANE_SWAP_CFG_TX_MASK) >>
...
static void elink_set_parallel_detection(struct elink_phy *phy,
  ...
  struct bnx2x_softc *sc = params->sc;
  uint16_t control2;
  CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_SERDES_DIGITAL,
  ...
  CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_SERDES_DIGITAL,
                    MDIO_SERDES_DIGITAL_A_1000X_CONTROL2, control2);
  ...
  if ((phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_DIRECT) &&
    ...
    CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_10G_PARALLEL_DETECT,
                      MDIO_10G_PARALLEL_DETECT_PAR_DET_10G_LINK,
                      MDIO_10G_PARALLEL_DETECT_PAR_DET_10G_LINK_CNT);

    CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_10G_PARALLEL_DETECT,
                      MDIO_10G_PARALLEL_DETECT_PAR_DET_10G_CONTROL, &control2);

    control2 |= MDIO_10G_PARALLEL_DETECT_PAR_DET_10G_CONTROL_PARDET10G_EN;
...
static void elink_set_autoneg(struct elink_phy *phy,
  ...
  CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_BAM_NEXT_PAGE,
  ...
  CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_BAM_NEXT_PAGE,
                    MDIO_BAM_NEXT_PAGE_MP5_NEXT_PAGE_CTRL, reg_val);

  if (enable_cl73) {
    /* Enable Cl73 FSM status bits */
    CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_CL73_USERB0,
    ...
    CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_CL73_IEEEB1,
                      MDIO_CL73_IEEEB1_AN_ADV2, reg_val);
...
static void elink_program_serdes(struct elink_phy *phy,
  ...
  if (!((vars->line_speed == ELINK_SPEED_1000) ||
    ...
    reg_val |= (MDIO_SERDES_DIGITAL_MISC1_REFCLK_SEL_156_25M |
                MDIO_SERDES_DIGITAL_MISC1_FORCE_SPEED_SEL);
    if (vars->line_speed == ELINK_SPEED_10000)
      reg_val |= MDIO_SERDES_DIGITAL_MISC1_FORCE_SPEED_10G_CX4;
  }
...
static void elink_set_brcm_cl37_advertisement(struct elink_phy *phy,
  ...
  CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_OVER_1G, MDIO_OVER_1G_UP1, val);

  CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_OVER_1G, MDIO_OVER_1G_UP3, 0x400);
}
...
static void elink_restart_autoneg(struct elink_phy *phy,
  ...
  if (enable_cl73) {
    CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_CL73_IEEEB0,
                      MDIO_CL73_IEEEB0_CL73_AN_CONTROL, &mii_control);
    ...
    CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_COMBO_IEEE0,
                      MDIO_COMBO_IEEE0_MII_CONTROL, &mii_control);
    ELINK_DEBUG_P1(sc, "elink_restart_autoneg mii_control before = 0x%x",
                   mii_control);
    CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_COMBO_IEEE0,
                      MDIO_COMBO_IEEE0_MII_CONTROL,
...
static void elink_initialize_sgmii_process(struct elink_phy *phy,
  ...
  if (!(vars->line_speed == ELINK_SPEED_AUTO_NEG)) {
    ...
    case ELINK_SPEED_100:
      ...
      break;
    case ELINK_SPEED_1000:
      mii_control |= MDIO_COMBO_IEEO_MII_CONTROL_MAN_SGMII_SP_1000;
      break;
    case ELINK_SPEED_10:
 ...
 */
static elink_status_t
elink_direct_parallel_detect_used(struct elink_phy *phy,
                                  struct elink_params *params) {
  struct bnx2x_softc *sc = params->sc;
  ...
  if (status2_1000x & MDIO_SERDES_DIGITAL_A_1000X_STATUS2_AN_DISABLED) {
    ELINK_DEBUG_P1(sc, "1G parallel detect link on port %d", params->port);
    return 1;
  }
  ...
  if (pd_10g & MDIO_10G_PARALLEL_DETECT_PAR_DET_10G_STATUS_PD_LINK) {
    ELINK_DEBUG_P1(sc, "10G parallel detect link on port %d", params->port);
    return 1;
  }
  return ELINK_STATUS_OK;
}
...
static void elink_update_adv_fc(struct elink_phy *phy,
  ...
  uint16_t pause_result;
  struct bnx2x_softc *sc = params->sc;
  if ((gp_status & (MDIO_GP_STATUS_TOP_AN_STATUS1_CL73_AUTONEG_COMPLETE |
    ...
    pause_result = (ld_pause & MDIO_CL73_IEEEB1_AN_ADV1_PAUSE_MASK) >> 8;
    pause_result |= (lp_pause & MDIO_CL73_IEEEB1_AN_LP_ADV1_PAUSE_MASK) >> 10;
    ...
    CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_COMBO_IEEE0,
                      ...
                      &lp_pause);
    pause_result = (ld_pause & MDIO_COMBO_IEEE0_AUTO_NEG_ADV_PAUSE_MASK) >> 5;
    pause_result |= (lp_pause & MDIO_COMBO_IEEE0_AUTO_NEG_ADV_PAUSE_MASK) >> 7;
...
static void elink_check_fallback_to_cl37(struct elink_phy *phy,
  ...
  CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_RX0, MDIO_RX0_RX_STATUS, &rx_status);
  if ((rx_status & MDIO_RX0_RX_STATUS_SIGDET) != (MDIO_RX0_RX_STATUS_SIGDET)) {
    ELINK_DEBUG_P1(sc,
  ...
  CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_CL73_USERB0,
   ...
   * supports only CL37
   */
  CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_REMOTE_PHY,
...
static elink_status_t
elink_get_link_speed_duplex(struct elink_phy *phy, struct elink_params *params,
                            struct elink_vars *vars, uint16_t is_link_up,
                            uint16_t speed_mask, uint16_t is_duplex) {
  struct bnx2x_softc *sc = params->sc;
  if (phy->req_line_speed == ELINK_SPEED_AUTO_NEG)
    vars->link_status |= LINK_STATUS_AUTO_NEGOTIATE_ENABLED;
  if (is_link_up) {
    ...
    case ELINK_GP_STATUS_6G:
      ELINK_DEBUG_P1(sc, "link speed unsupported  gp_status 0x%x", speed_mask);
      return ELINK_STATUS_ERROR;
  ...
  ELINK_DEBUG_P2(sc,
                 " in elink_get_link_speed_duplex vars->link_status = %x, "
                 "vars->duplex = %x",
                 vars->link_status, vars->duplex);
  ELINK_DEBUG_P2(sc, " phy_link_up %x line_speed %d", vars->phy_link_up,
                 vars->line_speed);
  return ELINK_STATUS_OK;
}
...
static uint8_t elink_link_settings_status(struct elink_phy *phy,
  ...
  /* Read gp_status */
  CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_GP_STATUS,
                    MDIO_GP_STATUS_TOP_AN_STATUS1, &gp_status);
  if (gp_status & MDIO_GP_STATUS_TOP_AN_STATUS1_DUPLEX_STATUS) {
    duplex = DUPLEX_FULL;
    ELINK_DEBUG_P1(sc, "duplex status read from phy is = %x", duplex);
  } else {
    ELINK_DEBUG_P1(sc,
  ...
  if (gp_status & MDIO_GP_STATUS_TOP_AN_STATUS1_LINK_STATUS) {
    if (ELINK_SINGLE_MEDIA_DIRECT(params)) {
      ...
      if (phy->req_line_speed == ELINK_SPEED_AUTO_NEG)
        elink_xgxs_an_resolve(phy, params, vars, gp_status);
    }
  } else { /* Link_down */
    if ((phy->req_line_speed == ELINK_SPEED_AUTO_NEG) &&
  ...
  if (ELINK_SINGLE_MEDIA_DIRECT(params) &&
    ...
    if (val & MDIO_CL73_IEEEB1_AN_ADV2_ADVR_1000M_KX)
      vars->link_status |= LINK_STATUS_LINK_PARTNER_1000TFD_CAPABLE;
    if (val & (MDIO_CL73_IEEEB1_AN_ADV2_ADVR_10G_KX4 |
               MDIO_CL73_IEEEB1_AN_ADV2_ADVR_10G_KR))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_10GXFD_CAPABLE;

    CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_OVER_1G, MDIO_OVER_1G_LP_UP1,
                      &val);

    if (val & MDIO_OVER_1G_UP1_2_5G)
      vars->link_status |= LINK_STATUS_LINK_PARTNER_2500XFD_CAPABLE;
    if (val & (MDIO_OVER_1G_UP1_10G | MDIO_OVER_1G_UP1_10GH))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_10GXFD_CAPABLE;
  }

  ELINK_DEBUG_P3(sc, "duplex %x  flow_ctrl 0x%x link_status 0x%x", vars->duplex,
                 vars->flow_ctrl, vars->link_status);
  return rc;
}
...
static uint8_t elink_warpcore_read_status(struct elink_phy *phy,
  ...
  if ((params->loopback_mode) && (phy->flags & ELINK_FLAGS_WC_DUAL_MODE)) {
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL5_LINK_STATUS,
                    &link_up);
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL5_LINK_STATUS,
                    &link_up);
    link_up &= 0x1;
    ELINK_DEBUG_P1(sc, "params->loopback_mode link_up read = %x", link_up);
    ...
    ELINK_DEBUG_P2(sc, "PCS RX link status = 0x%x-->0x%x", temp_link_up,
                   link_up);
    link_up &= (1 << 2);
    if (link_up)
      elink_ext_phy_resolve_fc(phy, params, vars);
  } else {
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_GP2_STATUS_GP_2_1,
                    &gp_status1);
    ELINK_DEBUG_P1(sc, "0x81d1 = 0x%x", gp_status1);
    /* Check for either KR, 1G, or AN up. */
    link_up =
        ((gp_status1 >> 8) | (gp_status1 >> 12) | (gp_status1)) & (1 << lane);
    if (phy->supported & ELINK_SUPPORTED_20000baseKR2_Full) {
      uint16_t an_link;
      elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_STATUS, &an_link);
      elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_STATUS, &an_link);
      link_up |= (an_link & (1 << 2));
      ELINK_DEBUG_P2(sc, "an_link = %x, link_up = %x", an_link, link_up);
    }
    if (link_up && ELINK_SINGLE_MEDIA_DIRECT(params)) {
      uint16_t pd, gp_status4;
      if (phy->req_line_speed == ELINK_SPEED_AUTO_NEG) {
        /* Check Autoneg complete */
        elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_GP2_STATUS_GP_2_4,
                        &gp_status4);
        if (gp_status4 & ((1 << 12) << lane))
          vars->link_status |= LINK_STATUS_AUTO_NEGOTIATE_COMPLETE;

        /* Check parallel detect used */
        elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_PAR_DET_10G_STATUS,
                        &pd);
        if (pd & (1 << 15))
          vars->link_status |= LINK_STATUS_PARALLEL_DETECTION_USED;
        ELINK_DEBUG_P2(sc, "pd = %x, link_status = %x", pd, vars->link_status);
      }
      elink_ext_phy_resolve_fc(phy, params, vars);
      vars->duplex = duplex;
      ELINK_DEBUG_P3(sc,
                     " ELINK_SINGLE_MEDIA_DIRECT duplex %x  flow_ctrl 0x%x "
                     "link_status 0x%x",
  ...
  if ((vars->link_status & LINK_STATUS_AUTO_NEGOTIATE_COMPLETE) &&
    ...
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_LP_AUTO_NEG2, &val);
    ...
    if (val & (MDIO_CL73_IEEEB1_AN_ADV2_ADVR_10G_KX4 |
               MDIO_CL73_IEEEB1_AN_ADV2_ADVR_10G_KR))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_10GXFD_CAPABLE;
    ELINK_DEBUG_P2(sc, "val = %x, link_status = %x", val, vars->link_status);
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL3_LP_UP1, &val);
  ...
  if (lane < 2) {
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_GP2_STATUS_GP_2_2,
                    &gp_speed);
  } else {
    elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_GP2_STATUS_GP_2_3,
                    &gp_speed);
  }
  ...
  rc =
      elink_get_link_speed_duplex(phy, params, vars, link_up, gp_speed, duplex);
...
static void elink_set_gmii_tx_driver(struct elink_params *params) {
  ...
  /* Read precomp */
  CL22_RD_OVER_CL45(sc, phy, MDIO_REG_BANK_OVER_1G, MDIO_OVER_1G_LP_UP2,
                    &lp_up2);
  ...
  for (bank = MDIO_REG_BANK_TX0; bank <= MDIO_REG_BANK_TX3;
    ...
    CL22_RD_OVER_CL45(sc, phy, bank, MDIO_TX0_TX_DRIVER, &tx_driver);

    /* Replace tx_driver bits [15:12] */
    if (lp_up2 != (tx_driver & MDIO_TX0_TX_DRIVER_PREEMPHASIS_MASK)) {
      tx_driver &= ~MDIO_TX0_TX_DRIVER_PREEMPHASIS_MASK;
      tx_driver |= lp_up2;
      CL22_WR_OVER_CL45(sc, phy, bank, MDIO_TX0_TX_DRIVER, tx_driver);
    }
  }
}
...
static elink_status_t elink_emac_program(struct elink_params *params,
  ...
  elink_bits_dis(
      sc, GRCBASE_EMAC0 + port * 0x400 + EMAC_REG_EMAC_MODE,
      (EMAC_MODE_25G_MODE | EMAC_MODE_PORT_MII_10M | EMAC_MODE_HALF_DUPLEX));
  switch (vars->line_speed) {
  case ELINK_SPEED_10:
  ...
  elink_bits_en(sc, GRCBASE_EMAC0 + port * 0x400 + EMAC_REG_EMAC_MODE, mode);
...
static void elink_set_preemphasis(struct elink_phy *phy,
  ...
  for (bank = MDIO_REG_BANK_RX0, i = 0; bank <= MDIO_REG_BANK_RX3;
    ...
    CL22_WR_OVER_CL45(sc, phy, bank, MDIO_RX0_RX_EQ_BOOST,
                      phy->rx_preemphasis[i]);
  }

  for (bank = MDIO_REG_BANK_TX0, i = 0; bank <= MDIO_REG_BANK_TX3;
       bank += (MDIO_REG_BANK_TX1 - MDIO_REG_BANK_TX0), i++) {
    CL22_WR_OVER_CL45(sc, phy, bank, MDIO_TX0_TX_DRIVER,
...
static uint8_t elink_xgxs_config_init(struct elink_phy *phy,
  ...
  if (!(vars->phy_flags & PHY_SGMII_FLAG)) {
    ...
    if (vars->line_speed != ELINK_SPEED_AUTO_NEG ||
      ...
      elink_set_ieee_aneg_advertisement(phy, params, vars->ieee_fc);
...
static elink_status_t elink_prepare_xgxs(struct elink_phy *phy,
  ...
  elink_status_t rc;
  vars->phy_flags |= PHY_XGXS_FLAG;
  if ((phy->req_line_speed && ((phy->req_line_speed == ELINK_SPEED_100) ||
                               (phy->req_line_speed == ELINK_SPEED_10))) ||
      (!phy->req_line_speed &&
       (phy->speed_cap_mask >= PORT_HW_CFG_SPEED_CAPABILITY_D0_10M_FULL) &&
       (phy->speed_cap_mask < PORT_HW_CFG_SPEED_CAPABILITY_D0_1G)) ||
...
static uint16_t elink_wait_reset_complete(struct bnx2x_softc *sc,
  ...
  for (cnt = 0; cnt < 1000; cnt++) {
    if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY2_TYPE_BNX2X54618SE)
      ...
      elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_CTRL, &ctrl);
    if (!(ctrl & (1 << 15)))
      break;
    DELAY(1000 * 1);
  }
...
static void elink_link_int_enable(struct elink_params *params) {
  ...
  if (CHIP_IS_E3(sc)) {
    ...
    mask = (ELINK_NIG_MASK_XGXS0_LINK10G | ELINK_NIG_MASK_XGXS0_LINK_STATUS);
...
static void elink_rearm_latch_signal(struct bnx2x_softc *sc, uint8_t port,
  ...
  if (exp_mi_int)
    elink_bits_en(sc, NIG_REG_STATUS_INTERRUPT_PORT0 + port * 4,
    ...
    if (USES_WARPCORE(sc))
      ...
      if (is_10g_plus)
        ...
        uint32_t ser_lane =
            ((params->lane_config & PORT_HW_CFG_LANE_SWAP_CFG_MASTER_MASK) >>
             PORT_HW_CFG_LANE_SWAP_CFG_MASTER_SHIFT);
        mask = ((1 << ser_lane) << ELINK_NIG_STATUS_XGXS0_LINK_STATUS_SIZE);
      } else
        mask = ELINK_NIG_STATUS_SERDES0_LINK_STATUS;
    }
    ELINK_DEBUG_P1(sc, "Ack link up interrupt with mask 0x%x", mask);
    elink_bits_en(sc, NIG_REG_STATUS_INTERRUPT_PORT0 + port * 4, mask);
  }
}
...
elink_status_t elink_get_ext_phy_fw_version(struct elink_params *params,
  ...
  if (params->phy[ELINK_EXT_PHY1].format_fw_ver) {
    status |= params->phy[ELINK_EXT_PHY1].format_fw_ver(spirom_ver, ver_p,
                                                        &remain_len);
  ...
  if ((params->num_phys == ELINK_MAX_PHYS) &&
    ...
    if (params->phy[ELINK_EXT_PHY2].format_fw_ver) {
      *ver_p = '/';
      ver_p++;
      remain_len--;
      status |= params->phy[ELINK_EXT_PHY2].format_fw_ver(spirom_ver, ver_p,
                                                          &remain_len);
...
static void elink_set_xgxs_loopback(struct elink_phy *phy,
  ...
  if (phy->req_line_speed != ELINK_SPEED_1000) {
    ...
    if (!CHIP_IS_E3(sc)) {
      ...
      md_devad = REG_RD(sc, (NIG_REG_XGXS0_CTRL_MD_DEVAD + port * 0x18));
    ...
    elink_cl45_write(
        sc, phy, 5,
        (MDIO_REG_BANK_CL73_IEEEB0 + (MDIO_CL73_IEEEB0_CL73_AN_CONTROL & 0xf)),
        0x6041);
    DELAY(1000 * 200);
    /* Set aer mmd back */
    elink_set_aer_mmd(params, phy);
    ...
    elink_cl45_read(
        sc, phy, 5,
        (MDIO_REG_BANK_COMBO_IEEE0 + (MDIO_COMBO_IEEE0_MII_CONTROL & 0xf)),
        &mii_ctrl);
    elink_cl45_write(
        sc, phy, 5,
        (MDIO_REG_BANK_COMBO_IEEE0 + (MDIO_COMBO_IEEE0_MII_CONTROL & 0xf)),
        mii_ctrl | MDIO_COMBO_IEEO_MII_CONTROL_LOOPBACK);
  }
}
...
elink_status_t elink_set_led(struct elink_params *params,
  ...
  ELINK_DEBUG_P2(sc, "speed 0x%x, hw_led_mode 0x%x", speed, hw_led_mode);
  /* In case */
  for (phy_idx = ELINK_EXT_PHY1; phy_idx < ELINK_MAX_PHYS; phy_idx++) {
    if (params->phy[phy_idx].set_link_led) {
      params->phy[phy_idx].set_link_led(&params->phy[phy_idx], params, mode);
    }
  }
#ifdef ELINK_INCLUDE_EMUL
  if (params->feature_config_flags & ELINK_FEATURE_CONFIG_EMUL_DISABLE_EMAC)
    return rc;
#endif
  ...
  case ELINK_LED_MODE_OFF:
    ...
    REG_WR(sc, NIG_REG_LED_MODE_P0 + port * 4, SHARED_HW_CFG_LED_MAC1);
  ...
  case ELINK_LED_MODE_ON:
    if (((params->phy[ELINK_EXT_PHY1].type ==
      ...
      if (mode == ELINK_LED_MODE_ON || speed == ELINK_SPEED_10000) {
       ...
       * is up in CL73
       */
      if ((!CHIP_IS_E3(sc)) || (CHIP_IS_E3(sc) && mode == ELINK_LED_MODE_ON))
        REG_WR(sc, NIG_REG_LED_10G_P0 + port * 4, 1);

      if (CHIP_IS_E1x(sc) || CHIP_IS_E2(sc) || (mode == ELINK_LED_MODE_ON))
        REG_WR(sc, NIG_REG_LED_MODE_P0 + port * 4, 0);
      else
      ...
      tmp = elink_cb_reg_read(sc, emac_base + EMAC_REG_EMAC_LED);
      elink_cb_reg_write(sc, emac_base + EMAC_REG_EMAC_LED,
                         tmp | EMAC_LED_OVERRIDE | EMAC_LED_1000MB_OVERRIDE);
      /* Break here; otherwise, it'll disable the
       * intended override.
       */
      break;
    } else {
      uint32_t nig_led_mode =
          ((params->hw_led_mode << SHARED_HW_CFG_LED_MODE_SHIFT) ==
           SHARED_HW_CFG_LED_EXTPHY2)
              ? (SHARED_HW_CFG_LED_PHY1 >> SHARED_HW_CFG_LED_MODE_SHIFT)
              : hw_led_mode;
      REG_WR(sc, NIG_REG_LED_MODE_P0 + port * 4, nig_led_mode);
    }
    ...
    if (CHIP_IS_E1(sc) &&
        ((speed == ELINK_SPEED_2500) || (speed == ELINK_SPEED_1000) ||
         (speed == ELINK_SPEED_100) || (speed == ELINK_SPEED_10))) {
      /* For speeds less than 10G LED scheme is different */
      REG_WR(sc, NIG_REG_LED_CONTROL_OVERRIDE_TRAFFIC_P0 + port * 4, 1);
      REG_WR(sc, NIG_REG_LED_CONTROL_TRAFFIC_P0 + port * 4, 0);
      REG_WR(sc, NIG_REG_LED_CONTROL_BLINK_TRAFFIC_P0 + port * 4, 1);
    }
    break;

  default:
    rc = ELINK_STATUS_ERROR;
    ELINK_DEBUG_P1(sc, "elink_set_led: Invalid led mode %d", mode);
    break;
  }
  return rc;
}
...
elink_status_t elink_test_link(struct elink_params *params,
  ...
  if (CHIP_IS_E3(sc)) {
    ...
    if (params->req_line_speed[ELINK_LINK_CONFIG_IDX(ELINK_INT_PHY)] >
        ELINK_SPEED_10000) {
      /* Check 20G link */
      elink_cl45_read(sc, int_phy, MDIO_WC_DEVAD, 1, &link_up);
      elink_cl45_read(sc, int_phy, MDIO_WC_DEVAD, 1, &link_up);
      link_up &= (1 << 2);
    } else {
      /* Check 10G link and below*/
      uint8_t lane = elink_get_warpcore_lane(int_phy, params);
      elink_cl45_read(sc, int_phy, MDIO_WC_DEVAD, MDIO_WC_REG_GP2_STATUS_GP_2_1,
                      &gp_status);
      gp_status = ((gp_status >> 8) & 0xf) | ((gp_status >> 12) & 0xf);
      link_up = gp_status & (1 << lane);
    }
    if (!link_up)
      return ELINK_STATUS_NO_LINK;
  } else {
    CL22_RD_OVER_CL45(sc, int_phy, MDIO_REG_BANK_GP_STATUS,
                      MDIO_GP_STATUS_TOP_AN_STATUS1, &gp_status);
  ...
  case 2:
    ext_phy_link_up = params->phy[ELINK_EXT_PHY1].read_status(
        &params->phy[ELINK_EXT_PHY1], params, &temp_vars);
    break;
  case 3: /* Dual Media */
    for (phy_index = ELINK_EXT_PHY1; phy_index < params->num_phys;
      ...
      serdes_phy_type =
          ((params->phy[phy_index].media_type ==
           ...
           (params->phy[phy_index].media_type == ELINK_ETH_PHY_SFP_1G_FIBER) ||
           (params->phy[phy_index].media_type == ELINK_ETH_PHY_XFP_FIBER) ||
           (params->phy[phy_index].media_type == ELINK_ETH_PHY_DA_TWINAX));

      if (is_serdes != serdes_phy_type)
        continue;
      if (params->phy[phy_index].read_status) {
        ext_phy_link_up |= params->phy[phy_index].read_status(
            &params->phy[phy_index], params, &temp_vars);
      }
    }
    break;
  }
...
static elink_status_t elink_link_initialize(struct elink_params *params,
  ...
  /* Init external phy*/
  if (non_ext_phy) {
    if (params->phy[ELINK_INT_PHY].supported & ELINK_SUPPORTED_FIBRE)
    ...
    for (phy_index = ELINK_EXT_PHY1; phy_index < params->num_phys;
       ...
       * connected.
       */
      if (params->phy[phy_index].supported & ELINK_SUPPORTED_FIBRE)
        vars->link_status |= LINK_STATUS_SERDES_LINK;

      if (phy_index == ELINK_EXT_PHY2 &&
          (elink_phy_selection(params) ==
           PORT_HW_CFG_PHY_SELECTION_FIRST_PHY)) {
        ELINK_DEBUG_P0(sc, "Not initializing second phy");
        continue;
      }
      params->phy[phy_index].config_init(&params->phy[phy_index], params, vars);
    }
  }
  /* Reset the interrupt indication after phy was initialized */
  elink_bits_dis(
      sc, NIG_REG_STATUS_INTERRUPT_PORT0 + params->port * 4,
      (ELINK_NIG_STATUS_XGXS0_LINK10G | ELINK_NIG_STATUS_XGXS0_LINK_STATUS |
       ELINK_NIG_STATUS_SERDES0_LINK_STATUS | ELINK_NIG_MASK_MI_INT));
  return ELINK_STATUS_OK;
}
...
static void elink_common_ext_link_reset(__rte_unused struct elink_phy *phy,
  ...
  elink_cb_gpio_write(sc, MISC_REGISTERS_GPIO_1, MISC_REGISTERS_GPIO_OUTPUT_LOW,
                      gpio_port);
  elink_cb_gpio_write(sc, MISC_REGISTERS_GPIO_2, MISC_REGISTERS_GPIO_OUTPUT_LOW,
                      gpio_port);
...
static elink_status_t elink_update_link_down(struct elink_params *params,
  ...
  if (CHIP_IS_E3(sc)) {
    ...
    REG_WR(sc, MISC_REG_CPMU_LP_FW_ENABLE_P0 + (params->port << 2), 0);
    REG_WR(sc, MISC_REG_CPMU_LP_MASK_ENT_P0 + (params->port << 2), 0);
    vars->eee_status &= ~(SHMEM_EEE_LP_ADV_STATUS_MASK | SHMEM_EEE_ACTIVE_BIT);
...
static elink_status_t elink_update_link_up(struct elink_params *params,
  ...
  vars->link_status |= (LINK_STATUS_LINK_UP | LINK_STATUS_PHYSICAL_LINK_FLAG);
  vars->phy_flags |= PHY_PHYSICAL_LINK_FLAG;
  ...
  if (USES_WARPCORE(sc)) {
    if (link_10g) {
      if (elink_xmac_enable(params, vars, 0) == ELINK_STATUS_NO_LINK) {
    ...
    if ((vars->eee_status & SHMEM_EEE_ACTIVE_BIT) &&
      ...
      REG_WR(sc, MISC_REG_CPMU_LP_FW_ENABLE_P0 + (params->port << 2), 1);
      REG_WR(sc, MISC_REG_CPMU_LP_DR_ENABLE, 1);
      REG_WR(sc, MISC_REG_CPMU_LP_MASK_ENT_P0 + (params->port << 2), 0xfc20);
    }
  }
  if ((CHIP_IS_E1x(sc) || CHIP_IS_E2(sc))) {
    if (link_10g) {
      if (elink_bmac_enable(params, vars, 0, 1) == ELINK_STATUS_NO_LINK) {
      ...
      elink_set_led(params, vars, ELINK_LED_MODE_OPER, ELINK_SPEED_10000);
    } else {
  ...
  /* PBF - link up */
  if (CHIP_IS_E1x(sc))
    rc |= elink_pbf_update(params, vars->flow_ctrl, vars->line_speed);
...
elink_status_t elink_link_update(struct elink_params *params,
  ...
  ELINK_DEBUG_P3(sc, "port %x, XGXS?%x, int_status 0x%x", port,
                 (vars->phy_flags & PHY_XGXS_FLAG),
                 REG_RD(sc, NIG_REG_STATUS_INTERRUPT_PORT0 + port * 4));
   ...
   * speed/duplex result
   */
  for (phy_index = ELINK_EXT_PHY1; phy_index < params->num_phys; phy_index++) {
    struct elink_phy *phy = &params->phy[phy_index];
    if (!phy->read_status)
      continue;
    /* Read link status and params of this ext phy */
    cur_link_up = phy->read_status(phy, params, &phy_vars[phy_index]);
    if (cur_link_up) {
      ELINK_DEBUG_P1(sc, "phy in index %d link is up", phy_index);
    } else {
      ELINK_DEBUG_P1(sc, "phy in index %d link is down", phy_index);
      continue;
    }
  ...
  if (params->phy[ELINK_INT_PHY].read_status)
    params->phy[ELINK_INT_PHY].read_status(&params->phy[ELINK_INT_PHY], params,
                                           vars);
  ...
  if (active_external_phy > ELINK_INT_PHY) {
    ...
    if (active_external_phy == ELINK_EXT_PHY1) {
      if (params->phy[ELINK_EXT_PHY2].phy_specific_func) {
        ELINK_DEBUG_P0(sc, "Disabling TX on EXT_PHY2");
        params->phy[ELINK_EXT_PHY2].phy_specific_func(
            &params->phy[ELINK_EXT_PHY2], params, ELINK_DISABLE_TX);
      }
    }

    ext_phy_line_speed = phy_vars[active_external_phy].line_speed;
    vars->duplex = phy_vars[active_external_phy].duplex;
    if (params->phy[active_external_phy].supported & ELINK_SUPPORTED_FIBRE)
      vars->link_status |= LINK_STATUS_SERDES_LINK;
    else
      vars->link_status &= ~LINK_STATUS_SERDES_LINK;
    ...
    ELINK_DEBUG_P1(sc, "Active external phy selected: %x", active_external_phy);
  }
  ...
  ELINK_DEBUG_P3(sc,
                 "vars : eee_status = %x, fault_detected = %x, "
                 "check_kr2_recovery_cnt = %x",
                 vars->eee_status, vars->fault_detected,
                 vars->check_kr2_recovery_cnt);
  ELINK_DEBUG_P3(
      sc, "vars : periodic_flags = %x, aeu_int_mask = %x, rx_tx_asic_rst = %x",
  ...
  for (phy_index = ELINK_EXT_PHY1; phy_index < params->num_phys; phy_index++) {
    if (params->phy[phy_index].flags & ELINK_FLAGS_REARM_LATCH_SIGNAL) {
      elink_rearm_latch_signal(sc, port, phy_index == active_external_phy);
      break;
    }
  }
  ELINK_DEBUG_P3(sc,
                 ...
                 vars->flow_ctrl, vars->link_status, ext_phy_line_speed);
  ...
  if (!(ELINK_SINGLE_MEDIA_DIRECT(params))) {
    ...
    if (!(params->phy[ELINK_EXT_PHY1].flags & ELINK_FLAGS_INIT_XGXS_FIRST) &&
        ext_phy_link_up && !vars->phy_link_up) {
      vars->line_speed = ext_phy_line_speed;
      if (vars->line_speed < ELINK_SPEED_1000)
        vars->phy_flags |= PHY_SGMII_FLAG;
      ...
      if (params->phy[ELINK_INT_PHY].config_init)
        params->phy[ELINK_INT_PHY].config_init(&params->phy[ELINK_INT_PHY],
                                               params, vars);
    }
  }
  ...
  vars->link_up = (vars->phy_link_up &&
                   (ext_phy_link_up || ELINK_SINGLE_MEDIA_DIRECT(params)) &&
...
static void elink_save_bnx2x_spirom_ver(struct bnx2x_softc *sc,
  ...
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_ROM_VER1, &fw_ver1);
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_ROM_VER2, &fw_ver2);
...
static void elink_8073_resolve_fc(struct elink_phy *phy,
  ...
  if (elink_ext_phy_resolve_fc(phy, params, vars) &&
    ...
    uint16_t ld_pause; /* local */
    uint16_t lp_pause; /* link partner */
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LD, &ld_pause);

    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LP, &lp_pause);
    pause_result = (ld_pause & MDIO_COMBO_IEEE0_AUTO_NEG_ADV_PAUSE_BOTH) >> 5;
    pause_result |= (lp_pause & MDIO_COMBO_IEEE0_AUTO_NEG_ADV_PAUSE_BOTH) >> 7;

    elink_pause_resolve(phy, params, vars, pause_result);
    ELINK_DEBUG_P1(sc, "Ext PHY CL37 pause result 0x%x", pause_result);
  }
}
static elink_status_t elink_8073_8727_external_rom_boot(struct bnx2x_softc *sc,
  ...
  /* EDC grst */
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_GEN_CTRL, 0x0001);

  /* Ucode reboot and rst */
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_GEN_CTRL, 0x008c);

  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_MISC_CTRL1, 0x0001);

  /* Reset internal microprocessor */
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_GEN_CTRL,
  ...
  do {
    ...
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_ROM_VER1, &fw_ver1);
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_M8051_MSGOUT_REG,
                    &fw_msgout);

    DELAY(1000 * 1);
  } while (fw_ver1 == 0 || fw_ver1 == 0x4321 ||
           ((fw_msgout & 0xff) != 0x03 &&
            (phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X8073)));

  /* Clear ser_boot_ctl bit */
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_MISC_CTRL1, 0x0000);
...
static elink_status_t elink_8073_is_snr_needed(struct bnx2x_softc *sc,
  ...
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8073_CHIP_REV, &val);
  ...
  for (cnt = 0; cnt < 1000; cnt++) {
    ...
    if (!(val & (1 << 14)) || !(val & (1 << 13))) {
      ...
      for (cnt1 = 0; cnt1 < 1000; cnt1++) {
        elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8073_XAUI_WA,
                        &val);
        if (val & (1 << 15)) {
          ELINK_DEBUG_P0(sc, "XAUI workaround has completed");
          return ELINK_STATUS_OK;
        }
        DELAY(1000 * 3);
      }
...
static void elink_807x_force_10G(struct bnx2x_softc *sc,
                                 struct elink_phy *phy) {
  /* Force KR or KX */
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_CTRL, 0x2040);
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_10G_CTRL2, 0x000b);
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_BCM_CTRL, 0x0000);
...
static void elink_8073_set_pause_cl37(struct elink_params *params,
  ...
  uint16_t cl37_val;
  struct bnx2x_softc *sc = params->sc;
  elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LD, &cl37_val);
  ...
  elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LD, cl37_val);
  DELAY(1000 * 500);
}
...
static void elink_8073_specific_func(struct elink_phy *phy,
  ...
  case ELINK_PHY_INIT:
    /* Enable LASI */
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_RXCTRL, (1 << 2));
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_CTRL, 0x0004);
    break;
  }
}
...
static uint8_t elink_8073_config_init(struct elink_phy *phy,
  ...
  if (params->lane_config & PORT_HW_CFG_SWAP_PHY_POLARITY_ENABLED) {
    ...
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8073_OPT_DIGITAL_CTRL,
                    &val);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD,
                     MDIO_PMA_REG_8073_OPT_DIGITAL_CTRL, (val | (3 << 9)));
  }
  ...
  if (REG_RD(sc,
    ...
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8073_BAM, &val);
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8073_BAM, val | 1);
    ELINK_DEBUG_P0(sc, "Enable CL37 BAM on KR");
  }
  if (params->loopback_mode == ELINK_LOOPBACK_EXT) {
    ...
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_BCM_CTRL, 0x0002);
  }
  if (phy->req_line_speed != ELINK_SPEED_AUTO_NEG) {
    if (phy->req_line_speed == ELINK_SPEED_10000) {
      val = (1 << 7);
  ...
  if (((phy->speed_cap_mask & PORT_HW_CFG_SPEED_CAPABILITY_D0_2_5G) &&
    ...
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8073_CHIP_REV,
                    &phy_ver);
  ...
  elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LD,
                   (tmp1 | ((phy->req_duplex == DUPLEX_FULL) ? 0x20 : 0x40)));
  ...
  if (elink_8073_is_snr_needed(sc, phy))
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_EDC_FFE_MAIN,
                     0xFB0C);
...
static uint8_t elink_8073_read_status(struct elink_phy *phy,
  ...
  elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_LINK_STATUS,
                  &an1000_status);
  elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_LINK_STATUS,
                  &an1000_status);
  ...
  if (link_up && elink_8073_is_snr_needed(sc, phy)) {
     ...
     */
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_PLL_BANDWIDTH,
                     0x26BC);

    /* Change CDR Bandwidth in EDC register */
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_CDR_BANDWIDTH,
                     0x0333);
  }
  ...
  if ((link_status & (1 << 2)) && (!(link_status & (1 << 15)))) {
    link_up = 1;
    vars->line_speed = ELINK_SPEED_10000;
    ELINK_DEBUG_P1(sc, "port %x: External link up in 10G", params->port);
  } else if ((link_status & (1 << 1)) && (!(link_status & (1 << 14)))) {
    link_up = 1;
    vars->line_speed = ELINK_SPEED_2500;
    ELINK_DEBUG_P1(sc, "port %x: External link up in 2.5G", params->port);
  } else if ((link_status & (1 << 0)) && (!(link_status & (1 << 13)))) {
    link_up = 1;
    vars->line_speed = ELINK_SPEED_1000;
    ELINK_DEBUG_P1(sc, "port %x: External link up in 1G", params->port);
  } else {
    link_up = 0;
    ELINK_DEBUG_P1(sc, "port %x: External link is down", params->port);
  }
  ...
  if (link_up) {
    ...
    if (params->lane_config & PORT_HW_CFG_SWAP_PHY_POLARITY_ENABLED) {
      ...
      elink_cl45_read(sc, phy, MDIO_XS_DEVAD, MDIO_XS_REG_8073_RX_CTRL_PCIE,
                      &val1);
      ...
      elink_cl45_write(sc, phy, MDIO_XS_DEVAD, MDIO_XS_REG_8073_RX_CTRL_PCIE,
                       val1);
    }
  ...
  if (vars->link_status & LINK_STATUS_AUTO_NEGOTIATE_COMPLETE) {
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_LP_AUTO_NEG2, &val1);
...
static void elink_8073_link_reset(__rte_unused struct elink_phy *phy,
  ...
  if (CHIP_IS_E2(sc))
    ...
    gpio_port = params->port;
  ELINK_DEBUG_P1(sc, "Setting 8073 port %d into low power mode", gpio_port);
  elink_cb_gpio_write(sc, MISC_REGISTERS_GPIO_2, MISC_REGISTERS_GPIO_OUTPUT_LOW,
                      gpio_port);
}
...
static uint8_t elink_8705_config_init(struct elink_phy *phy,
  ...
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_MISC_CTRL, 0x8288);
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_PHY_IDENTIFIER,
                   0x7fbf);
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_CMU_PLL_BYPASS,
                   0x0100);
...
static uint8_t elink_8705_read_status(struct elink_phy *phy,
  ...
  elink_cl45_read(sc, phy, MDIO_WIS_DEVAD, MDIO_WIS_REG_LASI_STATUS, &val1);
...
static void elink_set_disable_pmd_transmit(struct elink_params *params,
  ...
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_TX_DISABLE, pmd_dis);
}
...
static void elink_sfp_e1e2_set_transmitter(struct elink_params *params,
  ...
  case PORT_HW_CFG_TX_LASER_MDIO:
    ...
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_PHY_IDENTIFIER, &val);
    ...
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_PHY_IDENTIFIER, val);
    break;
...
static elink_status_t elink_8726_read_sfp_module_eeprom(
    struct elink_phy *phy, struct elink_params *params, uint8_t dev_addr,
    uint16_t addr, uint8_t byte_cnt, uint8_t *o_buf,
    __rte_unused uint8_t is_init) {
  struct bnx2x_softc *sc = params->sc;
  uint16_t val = 0;
  uint16_t i;
  if (byte_cnt > ELINK_SFP_EEPROM_PAGE_SIZE) {
    ELINK_DEBUG_P0(sc, "Reading from eeprom is limited to 0xf");
    return ELINK_STATUS_ERROR;
  }
  /* Set the read command byte count */
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_SFP_TWO_WIRE_BYTE_CNT,
                   (byte_cnt | (dev_addr << 8)));

  /* Set the read command address */
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_SFP_TWO_WIRE_MEM_ADDR,
                   addr);

  /* Activate read command */
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_SFP_TWO_WIRE_CTRL,
                   0x2c0f);

  /* Wait up to 500us for command complete status */
  for (i = 0; i < 100; i++) {
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_SFP_TWO_WIRE_CTRL,
                    &val);
  ...
  if ((val & MDIO_PMA_REG_SFP_TWO_WIRE_CTRL_STATUS_MASK) !=
      MDIO_PMA_REG_SFP_TWO_WIRE_STATUS_COMPLETE) {
    ELINK_DEBUG_P1(sc, "Got bad status 0x%x when reading from SFP+ EEPROM",
...
static void elink_warpcore_power_module(struct elink_params *params,
  ...
  if (pin_cfg == PIN_CFG_NA)
    return;
  ELINK_DEBUG_P2(sc, "Setting SFP+ module power to %d using pin cfg %d", power,
                 pin_cfg);
...
static elink_status_t elink_warpcore_read_sfp_module_eeprom(
    __rte_unused struct elink_phy *phy, struct elink_params *params,
    uint8_t dev_addr, uint16_t addr, uint8_t byte_cnt, uint8_t *o_buf,
    uint8_t is_init) {
  elink_status_t rc = ELINK_STATUS_OK;
  uint8_t i, j = 0, cnt = 0;
  ...
  do {
    ...
    rc = elink_bsc_read(sc, dev_addr, addr32, 0, byte_cnt, data_array);
...
static elink_status_t elink_8727_read_sfp_module_eeprom(
    struct elink_phy *phy, struct elink_params *params, uint8_t dev_addr,
    uint16_t addr, uint8_t byte_cnt, uint8_t *o_buf,
    __rte_unused uint8_t is_init) {
  struct bnx2x_softc *sc = params->sc;
  ...
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, 0x8004,
...
elink_status_t elink_read_sfp_module_eeprom(struct elink_phy *phy,
  ...
  while (!rc && (byte_cnt > 0)) {
    xfer_size = (byte_cnt > ELINK_SFP_EEPROM_PAGE_SIZE)
                    ...
                    : byte_cnt;
    rc = read_func(phy, params, dev_addr, addr, xfer_size, user_data, 0);
    byte_cnt -= xfer_size;
    user_data += xfer_size;
    addr += xfer_size;
  }
...
static elink_status_t elink_get_edc_mode(struct elink_phy *phy,
  ...
  *edc_mode = ELINK_EDC_MODE_LIMITING;
  ...
  if (elink_read_sfp_module_eeprom(phy, params, ELINK_I2C_DEV_ADDR_A0, 0,
    ...
    if (copper_module_type &
      ...
      *edc_mode = ELINK_EDC_MODE_PASSIVE_DAC;
      ...
      if (copper_module_type &
        ...
        ELINK_DEBUG_P0(sc, "Passive Copper cable detected");
      } else {
        ELINK_DEBUG_P0(sc, "Unknown copper-cable-type");
      }
    }
    break;
  }
  ...
  case ELINK_SFP_EEPROM_CON_TYPE_VAL_RJ45:
    ...
    if (((val[ELINK_SFP_EEPROM_10G_COMP_CODE_ADDR] &
      ...
      if (phy->req_line_speed != ELINK_SPEED_1000) {
        ...
        if (!CHIP_IS_E1x(sc)) {
          gport = SC_PATH(sc) + (params->port << 1);
        }
        elink_cb_event_log(sc, ELINK_LOG_ID_NON_10G_MODULE, gport);
  ...
  for (phy_idx = ELINK_INT_PHY; phy_idx < ELINK_MAX_PHYS; phy_idx++) {
    if (&(params->phy[phy_idx]) == phy) {
      ...
      media_types |= ((phy->media_type & PORT_HW_CFG_MEDIA_TYPE_PHY0_MASK)
                      << (PORT_HW_CFG_MEDIA_TYPE_PHY1_SHIFT * phy_idx));
      break;
    }
  }
  ...
  if (check_limiting_mode) {
    ...
    if (elink_read_sfp_module_eeprom(
            phy, params, ELINK_I2C_DEV_ADDR_A0, ELINK_SFP_EEPROM_OPTIONS_ADDR,
            ELINK_SFP_EEPROM_OPTIONS_SIZE, options) != 0) {
      ELINK_DEBUG_P0(sc, "Failed to read Option field from module EEPROM");
      return ELINK_STATUS_ERROR;
    }
...
static elink_status_t elink_verify_sfp_module(struct elink_phy *phy,
  ...
  if (params->feature_config_flags &
    ...
    if (ELINK_DUAL_MEDIA(params)) {
      ELINK_DEBUG_P0(sc, "FW does not support OPT MDL verification");
      return ELINK_STATUS_ERROR;
    }
  ...
  if (elink_read_sfp_module_eeprom(
          phy, params, ELINK_I2C_DEV_ADDR_A0, ELINK_SFP_EEPROM_VENDOR_NAME_ADDR,
          ELINK_SFP_EEPROM_VENDOR_NAME_SIZE, (uint8_t *)vendor_name))
    vendor_name[0] = '\0';
  else
    vendor_name[ELINK_SFP_EEPROM_VENDOR_NAME_SIZE] = '\0';
  if (elink_read_sfp_module_eeprom(
          phy, params, ELINK_I2C_DEV_ADDR_A0, ELINK_SFP_EEPROM_PART_NO_ADDR,
          ELINK_SFP_EEPROM_PART_NO_SIZE, (uint8_t *)vendor_pn))
    vendor_pn[0] = '\0';
  else
...
static elink_status_t
elink_wait_for_sfp_module_initialized(struct elink_phy *phy,
                                      struct elink_params *params)
  ...
  for (timeout = 0; timeout < 60; timeout++) {
    if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_DIRECT)
      rc = elink_warpcore_read_sfp_module_eeprom(
          phy, params, ELINK_I2C_DEV_ADDR_A0, 1, 1, &val, 1);
    else
      rc = elink_read_sfp_module_eeprom(phy, params, ELINK_I2C_DEV_ADDR_A0, 1,
                                        1, &val);
    if (rc == 0) {
      ELINK_DEBUG_P1(sc, "SFP+ module initialization took %d ms", timeout * 5);
      return ELINK_STATUS_OK;
    }
    DELAY(1000 * 5);
  }
  rc = elink_read_sfp_module_eeprom(phy, params, ELINK_I2C_DEV_ADDR_A0, 1, 1,
                                    &val);
  return rc;
}
...
static void elink_8727_power_module(struct bnx2x_softc *sc,
  ...
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_GPIO_CTRL, val);
}
...
static elink_status_t elink_8726_set_limiting_mode(struct bnx2x_softc *sc,
  ...
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_ROM_VER2,
                  &cur_limiting_mode);
  ELINK_DEBUG_P1(sc, "Current Limiting mode is 0x%x", cur_limiting_mode);

  if (edc_mode == ELINK_EDC_MODE_LIMITING) {
    ELINK_DEBUG_P0(sc, "Setting LIMITING MODE");
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_ROM_VER2,
    ...
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_LRM_MODE, 0);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_ROM_VER2, 0x128);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_MISC_CTRL0, 0x4008);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_LRM_MODE, 0xaaaa);
  }
  return ELINK_STATUS_OK;
}
...
static void elink_8727_specific_func(struct elink_phy *phy,
  ...
  case ELINK_ENABLE_TX:
    ...
    break;
  case ELINK_PHY_INIT:
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_RXCTRL,
                     (1 << 2) | (1 << 5));
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_TXCTRL, 0);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_CTRL, 0x0006);
    /* Make MOD_ABS give interrupt on change */
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_PCS_OPT_CTRL,
                    &val);
    val |= (1 << 12);
    if (phy->flags & ELINK_FLAGS_NOC)
      val |= (3 << 5);
    ...
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_PCS_OPT_CTRL,
                     val);
    break;
  default:
    ELINK_DEBUG_P1(sc, "Function 0x%x not supported by 8727", action);
    return;
  }
}
...
static void elink_set_e1e2_module_fault_led(struct elink_params *params,
  ...
  case PORT_HW_CFG_FAULT_MODULE_LED_GPIO3: {
    ...
    uint16_t gpio_pin = fault_led_gpio - PORT_HW_CFG_FAULT_MODULE_LED_GPIO0;
    ELINK_DEBUG_P3(sc,
  ...
  default:
    ELINK_DEBUG_P1(sc, "Error: Invalid fault led mode 0x%x", fault_led_gpio);
  }
}
...
static void elink_set_e3_module_fault_led(struct elink_params *params,
  ...
  ELINK_DEBUG_P2(sc, "Setting Fault LED to %d using pin cfg %d", gpio_mode,
                 pin_cfg);
...
static void elink_warpcore_set_limiting_mode(struct elink_params *params,
  ...
  elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_UC_INFO_B1_FIRMWARE_MODE,
                  &val);
  val &= ~(0xf << (lane << 2));
  ...
  val |= (mode << (lane << 2));
  elink_cl45_write(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_UC_INFO_B1_FIRMWARE_MODE,
                   val);
  /* A must read */
  elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_UC_INFO_B1_FIRMWARE_MODE,
                  &val);
...
void elink_handle_module_detect_int(struct elink_params *params) {
  ...
  if (gpio_val == 0) {
    ...
    elink_power_sfp_module(params, phy, 1);
    elink_cb_gpio_int_write(sc, gpio_num, MISC_REGISTERS_GPIO_INT_OUTPUT_CLR,
                            gpio_port);
    if (elink_wait_for_sfp_module_initialized(phy, params) == 0) {
      ...
      if (CHIP_IS_E3(sc)) {
         ...
         * module limitation.
         */
        elink_cl45_read(sc, phy, MDIO_WC_DEVAD, MDIO_WC_REG_DIGITAL5_MISC6,
                        &rx_tx_in_reset);
        if ((!rx_tx_in_reset) && (params->link_flags & ELINK_PHY_INITIALIZED)) {
          elink_warpcore_reset_lane(sc, phy, 1);
          elink_warpcore_config_sfi(phy, params);
          elink_warpcore_reset_lane(sc, phy, 0);
        }
    ...
    elink_cb_gpio_int_write(sc, gpio_num, MISC_REGISTERS_GPIO_INT_OUTPUT_SET,
                            gpio_port);
...
/******************************************************************/
static void elink_sfp_mask_fault(struct bnx2x_softc *sc, struct elink_phy *phy,
                                 uint16_t alarm_status_offset,
                                 uint16_t alarm_ctrl_offset) {
  uint16_t alarm_status, val;
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, alarm_status_offset, &alarm_status);
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, alarm_status_offset, &alarm_status);
...
static uint8_t elink_8706_8726_read_status(struct elink_phy *phy,
  ...
  elink_sfp_mask_fault(sc, phy, MDIO_PMA_LASI_TXSTAT, MDIO_PMA_LASI_TXCTRL);
  ...
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_RX_SD, &rx_sd);
  elink_cl45_read(sc, phy, MDIO_PCS_DEVAD, MDIO_PCS_REG_STATUS, &pcs_status);
  ...
  if (vars->line_speed == ELINK_SPEED_10000) {
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_TXSTAT, &val1);
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_TXSTAT, &val1);
    if (val1 & (1 << 0))
...
static uint8_t elink_8706_config_init(struct elink_phy *phy,
  ...
  for (cnt = 0; cnt < 100; cnt++) {
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_ROM_VER1, &val);
    if (val)
      break;
    DELAY(1000 * 10);
  }
  ...
  if ((params->feature_config_flags &
    ...
    for (i = 0; i < 4; i++) {
      reg = MDIO_XS_8706_REG_BANK_RX0 +
            i * (MDIO_XS_8706_REG_BANK_RX1 - MDIO_XS_8706_REG_BANK_RX0);
  ...
  if (phy->req_line_speed == ELINK_SPEED_10000) {
    ...
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_DIGITAL_CTRL, 0x400);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_TXCTRL, 0);
    /* Arm LASI for link and Tx fault. */
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_CTRL, 3);
  } else {
    /* Force 1Gbps using autoneg with 1G advertisement */

    /* Allow CL37 through CL73 */
    ELINK_DEBUG_P0(sc, "XGXS 8706 AutoNeg");
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_CL73, 0x040c);

    /* Enable Full-Duplex advertisement on CL37 */
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LP, 0x0020);
    /* Enable CL37 AN */
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_AN, 0x1000);
    /* 1G support */
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_ADV, (1 << 5));

    /* Enable clause 73 AN */
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CTRL, 0x1200);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_RXCTRL, 0x0400);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_CTRL, 0x0004);
  }
  ...
  if (tx_en_mode == PORT_HW_CFG_TX_LASER_GPIO0) {
    ...
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_DIGITAL_CTRL, &tmp1);
    tmp1 |= 0x1;
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_DIGITAL_CTRL, tmp1);
  }
...
static void elink_8726_external_rom_boot(struct elink_phy *phy,
  ...
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_GEN_CTRL, 0x018B);
...
static uint8_t elink_8726_config_init(struct elink_phy *phy,
  ...
  if (phy->req_line_speed == ELINK_SPEED_1000) {
    ...
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_CTRL, 0x40);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_10G_CTRL2, 0xD);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_CTRL, 0x5);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_RXCTRL, 0x400);
  } else if ((phy->req_line_speed == ELINK_SPEED_AUTO_NEG) &&
             (phy->speed_cap_mask & PORT_HW_CFG_SPEED_CAPABILITY_D0_1G) &&
             ((phy->speed_cap_mask & PORT_HW_CFG_SPEED_CAPABILITY_D0_10G) !=
    ...
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_CL73, 0x040c);
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LD, 0x0020);
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_AN, 0x1000);
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CTRL, 0x1200);
    /* Enable RX-ALARM control to receive interrupt for 1G speed
     * change
     */
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_CTRL, 0x4);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_RXCTRL, 0x400);

  } else { /* Default 10G. Set only LASI control */
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_CTRL, 1);
  }
  ...
  if ((params->feature_config_flags &
    ...
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8726_TX_CTRL1,
                     phy->tx_preemphasis[0]);

    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8726_TX_CTRL2,
...
static void elink_8727_set_link_led(struct elink_phy *phy,
  ...
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_PCS_OPT_CTRL,
                   val);
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_GPIO_CTRL, &val);
  val &= 0xffe0;
...
static void elink_8727_config_speed(struct elink_phy *phy,
  ...
  if ((phy->req_line_speed == ELINK_SPEED_1000) ||
    ...
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_CTRL, 0x40);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_10G_CTRL2, 0xD);
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_10G_CTRL2, &tmp1);
    ELINK_DEBUG_P1(sc, "1.7 = 0x%x", tmp1);
    /* Power down the XAUI until link is up in case of dual-media
     * and 1G
     */
    if (ELINK_DUAL_MEDIA(params)) {
      elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_PCS_GP, &val);
      val |= (3 << 10);
      elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_PCS_GP, val);
    }
  } else if ((phy->req_line_speed == ELINK_SPEED_AUTO_NEG) &&
             ((phy->speed_cap_mask & PORT_HW_CFG_SPEED_CAPABILITY_D0_1G)) &&
             ((phy->speed_cap_mask & PORT_HW_CFG_SPEED_CAPABILITY_D0_10G) !=
    ...
    ELINK_DEBUG_P0(sc, "Setting 1G clause37");
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8727_MISC_CTRL, 0);
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_AN, 0x1300);
  } else {
    /* Since the 8727 has only single reset pin, need to set the 10G
     * registers although it is default
     */
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8727_MISC_CTRL,
                     0x0020);
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_AN, 0x0100);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_CTRL, 0x2040);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_10G_CTRL2, 0x0008);
  }
}
...
static uint8_t elink_8727_config_init(struct elink_phy *phy,
  ...
  if ((params->feature_config_flags &
    ...
    ELINK_DEBUG_P2(sc, "Setting TX_CTRL1 0x%x, TX_CTRL2 0x%x",
                   phy->tx_preemphasis[0], phy->tx_preemphasis[1]);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_TX_CTRL1,
                     phy->tx_preemphasis[0]);

    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_TX_CTRL2,
                     phy->tx_preemphasis[1]);
  }
  ...
  if (tx_en_mode == PORT_HW_CFG_TX_LASER_GPIO0) {
    ...
    tmp2 |= 0x1000;
    tmp2 &= 0xFFEF;
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_OPT_CFG_REG,
                     tmp2);
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_PHY_IDENTIFIER,
                    &tmp2);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_PHY_IDENTIFIER,
...
static void elink_8727_handle_mod_abs(struct elink_phy *phy,
  ...
  if (mod_abs & (1 << 8)) {
    ...
    /* Module is absent */
    ELINK_DEBUG_P0(sc, "MOD_ABS indication show module is absent");
    phy->media_type = ELINK_ETH_PHY_NOT_PRESENT;
     ...
     */
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_RXSTAT,
                    &rx_alarm_status);

  } else {
    /* Module is present */
    ELINK_DEBUG_P0(sc, "MOD_ABS indication show module is present");
...

{
  ...
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_CTRL, &lasi_ctrl);
  if (!lasi_ctrl)
    return 0;
  ...
  if (!(phy->flags & ELINK_FLAGS_NOC) && !(rx_alarm_status & (1 << 5))) {
    ...
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_GPIO_CTRL,
                    &val1);
    ...
    if ((val1 & (1 << 8)) == 0) {
      ...
      ELINK_DEBUG_P1(sc, "8727 Power fault has been detected on port %d",
                     oc_port);
      ...
      elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_RXCTRL, (1 << 5));
  ...
  if ((ELINK_DUAL_MEDIA(params)) && (phy->req_line_speed == ELINK_SPEED_1000)) {
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_PCS_GP, &val1);
    ...
    if (link_up)
      ...
      val1 |= (3 << 10);
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8727_PCS_GP, val1);
  }
  return link_up;
}
...
static void elink_save_848xx_spirom_version(struct elink_phy *phy,
  ...
  static struct elink_reg_set reg_set[] = {{MDIO_PMA_DEVAD, 0xA819, 0x0014},
  ...
  if (elink_is_8483x_8485x(phy)) {
    ...
    elink_save_spirom_version(sc, port, fw_ver1 & 0xfff, phy->ver_addr);
  } else {
...
static void elink_848xx_set_led(struct bnx2x_softc *sc, struct elink_phy *phy) {
  ...
  /* PHYC_CTL_LED_CTL */
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_LINK_SIGNAL, &val);
  val &= 0xFE00;
  val |= 0x0092;

  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_LINK_SIGNAL, val);
  ...
  /* stretch_en for LED3*/
  elink_cl45_read_or_write(sc, phy, MDIO_PMA_DEVAD, offset,
...
static elink_status_t elink_848xx_cmn_config_init(struct elink_phy *phy,
  ...
  elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8481_1000T_CTRL,
                  &an_1000_val);

  elink_ext_phy_set_pause(params, phy, vars);
  elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8481_LEGACY_AN_ADV,
                  &an_10_100_val);
  elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8481_LEGACY_MII_CTRL,
                  &autoneg_val);
  ...
  elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8481_1000T_CTRL,
                   an_1000_val);

  /* Set 10/100 speed advertisement */
  if (phy->req_line_speed == ELINK_SPEED_AUTO_NEG) {
    if (phy->speed_cap_mask & PORT_HW_CFG_SPEED_CAPABILITY_D0_100M_FULL) {
    ...
    if (phy->speed_cap_mask & PORT_HW_CFG_SPEED_CAPABILITY_D0_100M_HALF) {
      /* Enable autoneg and restart autoneg for legacy speeds
       */
      autoneg_val |= (1 << 9 | 1 << 12);
      an_10_100_val |= (1 << 7);
    ...
    if ((phy->speed_cap_mask & PORT_HW_CFG_SPEED_CAPABILITY_D0_10M_FULL) &&
    ...
    if ((phy->speed_cap_mask & PORT_HW_CFG_SPEED_CAPABILITY_D0_10M_HALF) &&
  ...
  if ((phy->req_line_speed == ELINK_SPEED_100) &&
    ...
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8481_AUX_CTRL,
                     (1 << 15 | 1 << 9 | 7 << 0));
  ...
  elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8481_LEGACY_AN_ADV,
                   an_10_100_val);
   ...
   */
  if (!elink_is_8483x_8485x(phy) || ((autoneg_val & (1 << 12)) == 0))
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8481_LEGACY_MII_CTRL,
                     autoneg_val);
  ...
  if (((phy->req_line_speed == ELINK_SPEED_AUTO_NEG) &&
    ...
    elink_cl45_read_or_write(sc, phy, MDIO_AN_DEVAD,
                             MDIO_AN_REG_8481_10GBASE_T_AN_CTRL, 0x1000);
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CTRL, 0x3200);
  } else
    elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8481_10GBASE_T_AN_CTRL,
                     1);
...
static elink_status_t elink_84858_cmd_hdlr(struct elink_phy *phy,
  ...
  for (idx = 0; idx < PHY848xx_CMDHDLR_WAIT; idx++) {
    elink_cl45_read(sc, phy, MDIO_CTL_DEVAD, MDIO_848xx_CMD_HDLR_STATUS, &val);
  ...
  for (idx = 0; idx < argc; idx++) {
    elink_cl45_write(sc, phy, MDIO_CTL_DEVAD, MDIO_848xx_CMD_HDLR_DATA1 + idx,
                     cmd_args[idx]);
  }
   ...
   */
  elink_cl45_write(sc, phy, MDIO_CTL_DEVAD, MDIO_848xx_CMD_HDLR_COMMAND,
                   fw_cmd);
  ...
  for (idx = 0; idx < argc; idx++) {
    elink_cl45_read(sc, phy, MDIO_CTL_DEVAD, MDIO_848xx_CMD_HDLR_DATA1 + idx,
                    &cmd_args[idx]);
  }
...
static elink_status_t elink_84833_cmd_hdlr(struct elink_phy *phy,
  ...
  if (process == PHY84833_MB_PROCESS2) {
    ...
    elink_cl45_write(sc, phy, MDIO_CTL_DEVAD, MDIO_848xx_CMD_HDLR_STATUS,
  ...
  if (process == PHY84833_MB_PROCESS1 || process == PHY84833_MB_PROCESS2) {
    /* Prepare argument(s) */
    for (idx = 0; idx < argc; idx++) {
      elink_cl45_write(sc, phy, MDIO_CTL_DEVAD, MDIO_848xx_CMD_HDLR_DATA1 + idx,
                       cmd_args[idx]);
...
static elink_status_t elink_848xx_cmd_hdlr(struct elink_phy *phy,
  ...
  if ((phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84858) ||
      (REG_RD(sc,
              params->shmem2_base + offsetof(struct shmem2_region,
                                             link_attr_sync[params->port])) &
       LINK_ATTR_84858)) {
    return elink_84858_cmd_hdlr(phy, params, fw_cmd, cmd_args, argc);
  } else {
    return elink_84833_cmd_hdlr(phy, params, fw_cmd, cmd_args, argc, process);
  }
}

static elink_status_t
elink_848xx_pair_swap_cfg(struct elink_phy *phy, struct elink_params *params,
                          __rte_unused struct elink_vars *vars) {
  uint32_t pair_swap;
  ...
  status = elink_848xx_cmd_hdlr(phy, params, PHY848xx_CMD_SET_PAIR_SWAP, data,
...
static uint8_t elink_84833_get_reset_gpios(struct bnx2x_softc *sc,
  ...
  if (CHIP_IS_E3(sc)) {
    ...
    for (idx = 0; idx < 2; idx++) {
      ...
      reset_pin[idx] = (reset_pin[idx] & PORT_HW_CFG_E3_PHY_RESET_MASK) >>
...
static void elink_84833_hw_reset_phy(struct elink_phy *phy,
  ...
  uint32_t other_shmem_base_addr =
      REG_RD(sc, params->shmem2_base +
                     offsetof(struct shmem2_region, other_shmem_base_addr));
  ...
  elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_8481_1G_100T_EXT_CTRL,
  ...
  reset_gpios =
      elink_84833_get_reset_gpios(sc, shmem_base_path, params->chip_id);

  elink_cb_gpio_mult_write(sc, reset_gpios, MISC_REGISTERS_GPIO_OUTPUT_LOW);
  DELAY(10);
  ELINK_DEBUG_P1(sc, "84833 hw reset on pin values 0x%x", reset_gpios);
}
...
static elink_status_t elink_8483x_disable_eee(struct elink_phy *phy,
  ...
  rc = elink_848xx_cmd_hdlr(phy, params, PHY848xx_CMD_SET_EEE_MODE, &cmd_args,
...
static uint8_t elink_848x3_config_init(struct elink_phy *phy,
  ...
  if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84823) {
    ...
    /* MDIO reset */
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_CTRL, 0x8000);
  }
  ...
  if (phy->type != PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84858) {
    ...
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_848xx_ID_MSB, &hw_rev);
  ...
  elink_cl45_read(sc, phy, MDIO_CTL_DEVAD, MDIO_CTL_REG_84823_MEDIA, &val);
  val &=
      ~(MDIO_CTL_REG_84823_MEDIA_MAC_MASK | MDIO_CTL_REG_84823_MEDIA_LINE_MASK |
  ...
  elink_cl45_write(sc, phy, MDIO_CTL_DEVAD, MDIO_CTL_REG_84823_MEDIA, val);
  ...
  if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84823) {
    ...
    elink_cl45_read(sc, phy, MDIO_CTL_DEVAD, MDIO_CTL_REG_84823_USER_CTRL_REG,
                    &val);
    if (cms_enable)
      val |= MDIO_CTL_REG_84823_USER_CTRL_CMS;
    else
      val &= ~MDIO_CTL_REG_84823_USER_CTRL_CMS;
    elink_cl45_write(sc, phy, MDIO_CTL_DEVAD, MDIO_CTL_REG_84823_USER_CTRL_REG,
                     val);
  }

  elink_cl45_read(sc, phy, MDIO_CTL_DEVAD, MDIO_84833_TOP_CFG_FW_REV, &val);

  /* Configure EEE support */
  if ((val >= MDIO_84833_TOP_CFG_FW_EEE) &&
      (val != MDIO_84833_TOP_CFG_FW_NO_EEE) && elink_eee_has_cap(params)) {
  ...
  if (elink_is_8483x_8485x(phy)) {
    ...
    elink_cl45_read_and_write(sc, phy, MDIO_CTL_DEVAD,
...
static uint8_t elink_848xx_read_status(struct elink_phy *phy,
  ...
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_PMD_SIGNAL, &val2);
  ...
  if (val2 & (1 << 11)) {
    ...
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD,
                    MDIO_AN_REG_8481_EXPANSION_REG_RD_RW, &legacy_status);

    ELINK_DEBUG_P1(sc, "Legacy speed status = 0x%x", legacy_status);
    ...
    if (params->feature_config_flags & ELINK_FEATURE_CONFIG_IEEE_PHY_TEST) {
      uint16_t mii_ctrl;
  ...
  if (link_up) {
    ...
    /* Read LP advertised speeds */
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CL37_FC_LP, &val);
    if (val & (1 << 5))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_10THD_CAPABLE;
    if (val & (1 << 6))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_10TFD_CAPABLE;
    if (val & (1 << 7))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_100TXHD_CAPABLE;
    if (val & (1 << 8))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_100TXFD_CAPABLE;
    if (val & (1 << 9))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_100T4_CAPABLE;

    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_1000T_STATUS, &val);

    if (val & (1 << 10))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_1000THD_CAPABLE;
    if (val & (1 << 11))
      vars->link_status |= LINK_STATUS_LINK_PARTNER_1000TFD_CAPABLE;

    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_MASTER_STATUS, &val);
...
static void elink_8481_link_reset(struct elink_phy *phy,
                                  struct elink_params *params) {
  elink_cl45_write(params->sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_CTRL, 0x0000);
...
static void elink_848x3_link_reset(struct elink_phy *phy,
  ...
  if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84823) {
    ...
    elink_cl45_read(sc, phy, MDIO_CTL_DEVAD, MDIO_84833_TOP_CFG_XGPHY_STRAP1,
                    &val16);
    val16 |= MDIO_84833_SUPER_ISOLATE;
    elink_cl45_write(sc, phy, MDIO_CTL_DEVAD, MDIO_84833_TOP_CFG_XGPHY_STRAP1,
                     val16);
  }
}
...
static void elink_848xx_set_link_led(struct elink_phy *phy,
  ...
  case ELINK_LED_MODE_OFF:
    ...
    if ((params->hw_led_mode << SHARED_HW_CFG_LED_MODE_SHIFT) ==
      ...
      /* Set LED masks */
      elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_LED1_MASK,
                       0x0);

      elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_LED2_MASK,
                       0x0);

      elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_LED3_MASK,
                       0x0);

      elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_LED5_MASK,
                       0x0);
  ...
  case ELINK_LED_MODE_FRONT_PANEL_OFF:
    ...
    ELINK_DEBUG_P1(sc, "Port 0x%x: LED MODE FRONT PANEL OFF", port);
    ...
    if ((params->hw_led_mode << SHARED_HW_CFG_LED_MODE_SHIFT) ==
      ...
      if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84834) {
         ...
         */
        if (REG_RD(sc, NIG_REG_MASK_INTERRUPT_PORT0 + params->port * 4) &
            ELINK_NIG_MASK_MI_INT) {
          params->link_flags |= ELINK_LINK_FLAGS_INT_DISABLED;

          elink_bits_dis(sc, NIG_REG_MASK_INTERRUPT_PORT0 + params->port * 4,
                         ELINK_NIG_MASK_MI_INT);
        }
        elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_SIGNAL_MASK,
                         0x0);
      }
    }
    break;
  case ELINK_LED_MODE_ON:
    ...
    if ((params->hw_led_mode << SHARED_HW_CFG_LED_MODE_SHIFT) ==
        SHARED_HW_CFG_LED_EXTPHY1) {
      /* Set control reg */
      elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_LINK_SIGNAL,
                      &val);
      val &= 0x8000;
      val |= 0x2492;

      elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_LINK_SIGNAL,
                       val);
  ...
  case ELINK_LED_MODE_OPER:
    ...
    if ((params->hw_led_mode << SHARED_HW_CFG_LED_MODE_SHIFT) ==
      ...
      if (!((val & MDIO_PMA_REG_8481_LINK_SIGNAL_LED4_ENABLE_MASK) >>
       ...
       * 10G in other modes.
       */
      val = ((params->hw_led_mode << SHARED_HW_CFG_LED_MODE_SHIFT) ==
             SHARED_HW_CFG_LED_EXTPHY2)
                ? 0x98
                : 0x80;
      ...
      if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84834) {
        ...
        elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_8481_SIGNAL_MASK,
                         0x40);
        if (params->link_flags & ELINK_LINK_FLAGS_INT_DISABLED) {
...
static void elink_54618se_specific_func(struct elink_phy *phy,
  ...
  case ELINK_PHY_INIT:
    ...
    elink_cl22_write(sc, phy, MDIO_REG_GPHY_SHADOW,
    ...
    elink_cl22_write(sc, phy, MDIO_REG_INTR_MASK,
...
static uint8_t elink_54618se_config_init(struct elink_phy *phy,
  ...
  if ((phy->flags & ELINK_FLAGS_EEE) && elink_eee_has_cap(params)) {
    ...
    if (rc != ELINK_STATUS_OK) {
       ...
       */
      elink_eee_advertise(phy, params, vars, SHMEM_EEE_1G_ADV);
    } else {
    ...
    vars->eee_status &=
        ((uint32_t)(~SHMEM_EEE_1G_ADV) << SHMEM_EEE_SUPPORTED_SHIFT);
    ...
    if (phy->flags & ELINK_FLAGS_EEE) {
      ...
      elink_cl45_write(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_EEE_ADV, temp);
    }
  }
...
static uint8_t elink_54618se_read_status(struct elink_phy *phy,
  ...
  if (link_up) {
    ...
    ELINK_DEBUG_P1(sc, "BNX2X4618SE: link speed is %d", vars->line_speed);
...
static void elink_7101_config_loopback(struct elink_phy *phy,
  ...
  struct bnx2x_softc *sc = params->sc;
  /* SFX7101_XGXS_TEST1 */
  elink_cl45_write(sc, phy, MDIO_XS_DEVAD, MDIO_XS_SFX7101_XGXS_TEST1, 0x100);
}
...
static uint8_t elink_7101_config_init(struct elink_phy *phy,
  ...
  /* Save spirom version */
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_7101_VER1, &fw_ver1);

  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_7101_VER2, &fw_ver2);
...
static uint8_t elink_7101_read_status(struct elink_phy *phy,
  ...
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_STAT, &val2);
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_LASI_STAT, &val1);
  ELINK_DEBUG_P2(sc, "10G-base-T LASI status 0x%x->0x%x", val2, val1);
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_STATUS, &val2);
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_STATUS, &val1);
  ELINK_DEBUG_P2(sc, "10G-base-T PMA status 0x%x->0x%x", val2, val1);
  link_up = ((val1 & 4) == 4);
  /* If link is up print the AN outcome of the SFX7101 PHY */
  if (link_up) {
    elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_MASTER_STATUS, &val2);
    vars->line_speed = ELINK_SPEED_10000;
    vars->duplex = DUPLEX_FULL;
    ELINK_DEBUG_P2(sc, "SFX7101 AN status 0x%x->Master=%x", val2,
                   (val2 & (1 << 14)));
    elink_ext_phy_10G_an_resolve(sc, phy, vars);
    elink_ext_phy_resolve_fc(phy, params, vars);
...
void elink_sfx7101_sp_sw_reset(struct bnx2x_softc *sc, struct elink_phy *phy) {
  ...
  elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_7101_RESET, &val);

  for (cnt = 0; cnt < 10; cnt++) {
    DELAY(1000 * 50);
    /* Writes a self-clearing reset */
    elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_7101_RESET,
                     (val | (1 << 15)));
    /* Wait for clear */
    elink_cl45_read(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_7101_RESET, &val);
...
static void elink_7101_set_link_led(struct elink_phy *phy,
  ...
  elink_cl45_write(sc, phy, MDIO_PMA_DEVAD, MDIO_PMA_REG_7107_LINK_LED_CNTL,
                   val);
}
...
static const struct elink_phy phy_serdes = {
    ...
    .supported =
        (ELINK_SUPPORTED_10baseT_Half | ELINK_SUPPORTED_10baseT_Full |
         ...
         ELINK_SUPPORTED_TP | ELINK_SUPPORTED_Autoneg | ELINK_SUPPORTED_Pause |
...
static const struct elink_phy phy_xgxs = {
    ...
    .supported =
        (ELINK_SUPPORTED_10baseT_Half | ELINK_SUPPORTED_10baseT_Full |
         ELINK_SUPPORTED_100baseT_Half | ELINK_SUPPORTED_100baseT_Full |
         ELINK_SUPPORTED_1000baseT_Full | ELINK_SUPPORTED_2500baseX_Full |
         ELINK_SUPPORTED_10000baseT_Full | ELINK_SUPPORTED_FIBRE |
         ELINK_SUPPORTED_Autoneg | ELINK_SUPPORTED_Pause |
         ELINK_SUPPORTED_Asym_Pause),
...
static const struct elink_phy phy_warpcore = {
    ...
    .supported =
        (ELINK_SUPPORTED_10baseT_Half | ELINK_SUPPORTED_10baseT_Full |
         ELINK_SUPPORTED_100baseT_Half | ELINK_SUPPORTED_100baseT_Full |
         ELINK_SUPPORTED_1000baseT_Full | ELINK_SUPPORTED_1000baseKX_Full |
         ELINK_SUPPORTED_10000baseT_Full | ELINK_SUPPORTED_10000baseKR_Full |
         ELINK_SUPPORTED_20000baseKR2_Full |
         ELINK_SUPPORTED_20000baseMLD2_Full | ELINK_SUPPORTED_FIBRE |
         ELINK_SUPPORTED_Autoneg | ELINK_SUPPORTED_Pause |
         ELINK_SUPPORTED_Asym_Pause),
...
static const struct elink_phy phy_7101 = {
    ...
    .tx_preemphasis = {0xffff, 0xffff, 0xffff, 0xffff},
    .mdio_ctrl = 0,
    .supported = (ELINK_SUPPORTED_10000baseT_Full | ELINK_SUPPORTED_TP |
                  ELINK_SUPPORTED_Autoneg | ELINK_SUPPORTED_Pause |
                  ELINK_SUPPORTED_Asym_Pause),
    .media_type = ELINK_ETH_PHY_BASE_T,
    .ver_addr = 0,
...
static const struct elink_phy phy_8073 = {
    ...
    .mdio_ctrl = 0,
    .supported =
        (ELINK_SUPPORTED_10000baseT_Full | ELINK_SUPPORTED_2500baseX_Full |
         ELINK_SUPPORTED_1000baseT_Full | ELINK_SUPPORTED_FIBRE |
         ELINK_SUPPORTED_Autoneg | ELINK_SUPPORTED_Pause |
         ELINK_SUPPORTED_Asym_Pause),
...
static const struct elink_phy phy_8705 = {
    ...
    .tx_preemphasis = {0xffff, 0xffff, 0xffff, 0xffff},
    .mdio_ctrl = 0,
    .supported = (ELINK_SUPPORTED_10000baseT_Full | ELINK_SUPPORTED_FIBRE |
...
static const struct elink_phy phy_8726 = {
    ...
    .mdio_ctrl = 0,
    .supported =
        (ELINK_SUPPORTED_10000baseT_Full | ELINK_SUPPORTED_1000baseT_Full |
         ELINK_SUPPORTED_Autoneg | ELINK_SUPPORTED_FIBRE |
         ELINK_SUPPORTED_Pause | ELINK_SUPPORTED_Asym_Pause),
    .media_type = ELINK_ETH_PHY_NOT_PRESENT,
    .ver_addr = 0,
    .req_flow_ctrl = 0,
...
static const struct elink_phy phy_8481 = {
    ...
    .addr = 0xff,
    .def_md_devad = 0,
    .flags = ELINK_FLAGS_FAN_FAILURE_DET_REQ | ELINK_FLAGS_REARM_LATCH_SIGNAL,
    .rx_preemphasis = {0xffff, 0xffff, 0xffff, 0xffff},
    .tx_preemphasis = {0xffff, 0xffff, 0xffff, 0xffff},
    .mdio_ctrl = 0,
    .supported =
        (ELINK_SUPPORTED_10baseT_Half | ELINK_SUPPORTED_10baseT_Full |
         ELINK_SUPPORTED_100baseT_Half | ELINK_SUPPORTED_100baseT_Full |
         ELINK_SUPPORTED_1000baseT_Full | ELINK_SUPPORTED_10000baseT_Full |
         ELINK_SUPPORTED_TP | ELINK_SUPPORTED_Autoneg | ELINK_SUPPORTED_Pause |
         ELINK_SUPPORTED_Asym_Pause),
    .media_type = ELINK_ETH_PHY_BASE_T,
    .ver_addr = 0,
...
static const struct elink_phy phy_54618se = {
    ...
    .supported =
        (ELINK_SUPPORTED_10baseT_Half | ELINK_SUPPORTED_10baseT_Full |
         ELINK_SUPPORTED_100baseT_Half | ELINK_SUPPORTED_100baseT_Full |
         ELINK_SUPPORTED_1000baseT_Full | ELINK_SUPPORTED_TP |
         ELINK_SUPPORTED_Autoneg | ELINK_SUPPORTED_Pause |
         ELINK_SUPPORTED_Asym_Pause),
    .media_type = ELINK_ETH_PHY_BASE_T,
    .ver_addr = 0,
...
static elink_status_t elink_populate_int_phy(struct bnx2x_softc *sc,
  ...
  if (USES_WARPCORE(sc)) {
    ...
    *phy = phy_warpcore;
    ...
    serdes_net_if =
        (REG_RD(sc, shmem_base +
                        offsetof(struct shmem_region,
                                 dev_info.port_hw_config[port].default_cfg)) &
         PORT_HW_CFG_NET_SERDES_IF_MASK);
    ...
    case PORT_HW_CFG_NET_SERDES_IF_KR:
      ...
      phy->supported &=
          (ELINK_SUPPORTED_1000baseKX_Full | ELINK_SUPPORTED_10000baseKR_Full |
    ...
    case PORT_HW_CFG_NET_SERDES_IF_KR2:
      ...
      phy->supported &=
          (ELINK_SUPPORTED_20000baseKR2_Full |
           ELINK_SUPPORTED_10000baseKR_Full | ELINK_SUPPORTED_1000baseKX_Full |
           ELINK_SUPPORTED_Autoneg | ELINK_SUPPORTED_FIBRE |
           ELINK_SUPPORTED_Pause | ELINK_SUPPORTED_Asym_Pause);
      phy->flags &= ~ELINK_FLAGS_TX_ERROR_CHECK;
      break;
    default:
      ELINK_DEBUG_P1(sc, "Unknown WC interface type 0x%x", serdes_net_if);
      break;
    }
    ...
    case ELINK_SWITCH_CFG_1G:
      phy_addr = REG_RD(sc, NIG_REG_SERDES0_CTRL_PHY_ADDR + port * 0x10);
      *phy = phy_serdes;
      break;
    case ELINK_SWITCH_CFG_10G:
      phy_addr = REG_RD(sc, NIG_REG_XGXS0_CTRL_PHY_ADDR + port * 0x18);
      *phy = phy_xgxs;
      break;
    default:
  ...
  phy->mdio_ctrl =
      elink_get_emac_base(sc, SHARED_HW_CFG_MDC_MDIO_ACCESS1_BOTH, port);
  if (CHIP_IS_E2(sc))
    phy->def_md_devad = ELINK_E2_DEFAULT_PHY_DEV_ADDR;
  else
    phy->def_md_devad = ELINK_DEFAULT_PHY_DEV_ADDR;

  ELINK_DEBUG_P3(sc, "Internal phy port=%d, addr=0x%x, mdio_ctl=0x%x", port,
...
static elink_status_t elink_populate_ext_phy(struct bnx2x_softc *sc,
  ...
  ext_phy_config = elink_get_ext_phy_config(sc, shmem_base, phy_index, port);
   ...
   */
  config2 =
      REG_RD(sc, shmem_base + offsetof(struct shmem_region,
  ...
  if (phy_index == ELINK_EXT_PHY1) {
    ...
    if (config2 & SHARED_HW_CFG_MDC_MDIO_ACCESS1_MASK)
      mdc_mdio_access = config2 & SHARED_HW_CFG_MDC_MDIO_ACCESS1_MASK;
  } else {
    uint32_t size = REG_RD(sc, shmem2_base);

    if (size > offsetof(struct shmem2_region, ext_phy_fw_version2)) {
      phy->ver_addr = shmem2_base +
                      offsetof(struct shmem2_region, ext_phy_fw_version2[port]);
    }
    /* Check specific mdc mdio settings */
  ...
  ELINK_DEBUG_P3(sc, "phy_type 0x%x port %d found in index %d", phy_type, port,
                 phy_index);
  ELINK_DEBUG_P2(sc, "             addr=0x%x, mdio_ctl=0x%x", phy->addr,
                 phy->mdio_ctrl);
  return ELINK_STATUS_OK;
}
...
static elink_status_t elink_populate_phy(struct bnx2x_softc *sc,
  ...
  status =
      elink_populate_ext_phy(sc, phy_index, shmem_base, shmem2_base, port, phy);
  return status;
}
...
static void elink_phy_def_cfg(struct elink_params *params,
  ...
  if (phy_index == ELINK_EXT_PHY2) {
    link_config = REG_RD(
        ...
        params->shmem_base +
            offsetof(struct shmem_region,
                     dev_info.port_feature_config[params->port].link_config2));
    phy->speed_cap_mask = REG_RD(
        sc,
        params->shmem_base +
            offsetof(
                struct shmem_region,
                dev_info.port_hw_config[params->port].speed_capability_mask2));
  } else {
    link_config = REG_RD(
        sc,
        params->shmem_base +
            offsetof(struct shmem_region,
                     dev_info.port_feature_config[params->port].link_config));
    phy->speed_cap_mask = REG_RD(
        sc,
        params->shmem_base +
            offsetof(
                struct shmem_region,
                dev_info.port_hw_config[params->port].speed_capability_mask));
  }
  ELINK_DEBUG_P3(sc, "Default config phy idx %x cfg 0x%x speed_cap_mask 0x%x",
...
elink_status_t elink_phy_probe(struct elink_params *params) {
  ...
  for (phy_index = ELINK_INT_PHY; phy_index < ELINK_MAX_PHYS; phy_index++) {
    ...
    ELINK_DEBUG_P3(sc,
                   ...
                   " actual_phy_idx %x",
                   phy_config_swapped, phy_index, actual_phy_idx);
    phy = &params->phy[actual_phy_idx];
    if (elink_populate_phy(sc, phy_index, params->shmem_base,
                           params->shmem2_base, params->port,
                           phy) != ELINK_STATUS_OK) {
      params->num_phys = 0;
      ELINK_DEBUG_P1(sc, "phy probe failed in phy index %d", phy_index);
      for (phy_index = ELINK_INT_PHY; phy_index < ELINK_MAX_PHYS; phy_index++)
        *phy = phy_null;
      return ELINK_STATUS_ERROR;
    }
    ...
    if (!(params->feature_config_flags & ELINK_FEATURE_CONFIG_MT_SUPPORT))
    ...
    if ((media_types &
         (PORT_HW_CFG_MEDIA_TYPE_PHY0_MASK
          << (PORT_HW_CFG_MEDIA_TYPE_PHY1_SHIFT * actual_phy_idx))) == 0) {
      media_types |= ((phy->media_type & PORT_HW_CFG_MEDIA_TYPE_PHY0_MASK)
                      << (PORT_HW_CFG_MEDIA_TYPE_PHY1_SHIFT * actual_phy_idx));
    }
    REG_WR(sc, sync_offset, media_types);
...
static elink_status_t elink_init_e3_emul_mac(struct elink_params *params,
  ...
  if (params->req_line_speed[0] == ELINK_SPEED_AUTO_NEG) {
    if (params->feature_config_flags & ELINK_FEATURE_CONFIG_EMUL_DISABLE_XMAC)
      vars->line_speed = ELINK_SPEED_2500;
  ...
  if (vars->line_speed < ELINK_SPEED_10000) {
    ...
    case ELINK_SPEED_2500:
      ...
      break;
    default:
      ELINK_DEBUG_P1(sc, "Invalid line speed %d for UMAC", vars->line_speed);
      return ELINK_STATUS_ERROR;
    }
    vars->link_status |= LINK_STATUS_LINK_UP;
    ...
    case ELINK_SPEED_20000:
      ...
      break;
    default:
      ELINK_DEBUG_P1(sc, "Invalid line speed %d for XMAC", vars->line_speed);
      return ELINK_STATUS_ERROR;
    }
    vars->link_status |= LINK_STATUS_LINK_UP;
...
static elink_status_t elink_init_emul(struct elink_params *params,
  ...
  if (CHIP_IS_E3(sc)) {
    ...
    if (params->feature_config_flags & ELINK_FEATURE_CONFIG_EMUL_DISABLE_BMAC) {
      vars->line_speed = ELINK_SPEED_1000;
      vars->link_status = (LINK_STATUS_LINK_UP | ELINK_LINK_1000XFD);
...
static elink_status_t elink_init_fpga(struct elink_params *params,
  ...
  if (CHIP_IS_E3(sc)) {
    ...
    case ELINK_SPEED_10:
      ...
      break;
    default:
      ELINK_DEBUG_P1(sc, "Invalid link speed %d", params->req_line_speed[0]);
      return ELINK_STATUS_ERROR;
    }
    vars->link_status |= LINK_STATUS_LINK_UP;
    if (params->loopback_mode == ELINK_LOOPBACK_UMAC)
  ...
  if (params->loopback_mode == ELINK_LOOPBACK_XGXS) {
    ...
    for (phy_index = ELINK_EXT_PHY1; phy_index < params->num_phys; phy_index++)
      if (params->phy[phy_index].config_loopback)
        params->phy[phy_index].config_loopback(&params->phy[phy_index], params);
  }
  REG_WR(sc, NIG_REG_EGRESS_DRAIN0_MODE + params->port * 4, 0);
...
void elink_set_rx_filter(struct elink_params *params, uint8_t en) {
  ...
  REG_WR(sc,
         (params->port ? NIG_REG_LLH1_BRB1_NOT_MCP : NIG_REG_LLH0_BRB1_NOT_MCP),
         en);
}
static elink_status_t elink_avoid_link_flap(struct elink_params *params,
  ...
  lfa_sts = REG_RD(sc, params->lfa_base + offsetof(struct shmem_lfa, lfa_sts));
  ...
  if (CHIP_IS_E3(sc)) {
    if (!dont_clear_stat) {
      REG_WR(sc, GRCBASE_MISC + MISC_REGISTERS_RESET_REG_2_CLEAR,
             (MISC_REGISTERS_RESET_REG_2_MSTAT0 << params->port));
      REG_WR(sc, GRCBASE_MISC + MISC_REGISTERS_RESET_REG_2_SET,
             (MISC_REGISTERS_RESET_REG_2_MSTAT0 << params->port));
    }
    if (vars->line_speed < ELINK_SPEED_10000)
  ...
  REG_WR(sc, params->lfa_base + offsetof(struct shmem_lfa, lfa_sts), lfa_sts);
...
static void elink_cannot_avoid_link_flap(struct elink_params *params,
  ...
  REG_WR(sc, params->lfa_base + offsetof(struct shmem_lfa, req_duplex),
         params->req_duplex[0] | (params->req_duplex[1] << 16));

  REG_WR(sc, params->lfa_base + offsetof(struct shmem_lfa, req_flow_ctrl),
         params->req_flow_ctrl[0] | (params->req_flow_ctrl[1] << 16));

  REG_WR(sc, params->lfa_base + offsetof(struct shmem_lfa, req_line_speed),
  ...
  lfa_sts &= ~LFA_LINK_FLAP_REASON_MASK;
  lfa_sts |=
      ((lfa_status & LFA_LINK_FLAP_REASON_MASK) << LFA_LINK_FLAP_REASON_OFFSET);

  /* Increment link flap counter */
  lfa_sts =
      ((lfa_sts & ~LINK_FLAP_COUNT_MASK) |
       (((((lfa_sts & LINK_FLAP_COUNT_MASK) >> LINK_FLAP_COUNT_OFFSET) + 1) &
         0xff)
...
elink_status_t elink_phy_init(struct elink_params *params,
  ...
  ELINK_DEBUG_P3(
      sc, " params : switch_cfg = %x, lane_config = %x req_duplex[1] = %x",
      params->switch_cfg, params->lane_config, params->req_duplex[1]);
  ELINK_DEBUG_P3(
      sc, " params : chip_id = %x, feature_config_flags = %x, num_phys = %x",
  ...
  ELINK_DEBUG_P3(
      sc, " params : multi_phy = %x, req_fc_auto_adv = %x, link_flags = %x",
  ...
  elink_bits_dis(sc, NIG_REG_MASK_INTERRUPT_PORT0 + params->port * 4,
                 (ELINK_NIG_MASK_XGXS0_LINK_STATUS |
                  ...
                  ELINK_NIG_MASK_SERDES0_LINK_STATUS | ELINK_NIG_MASK_MI_INT));
#ifdef ELINK_INCLUDE_EMUL
  if (!(params->feature_config_flags & ELINK_FEATURE_CONFIG_EMUL_DISABLE_EMAC))
#endif
...
elink_status_t elink_link_reset(struct elink_params *params,
  ...
  if (reset_ext_phy && (!CHIP_REV_IS_SLOW(sc))) {
    for (phy_index = ELINK_EXT_PHY1; phy_index < params->num_phys;
      ...
      if (params->phy[phy_index].link_reset) {
        elink_set_aer_mmd(params, &params->phy[phy_index]);
        params->phy[phy_index].link_reset(&params->phy[phy_index], params);
      }
      if (params->phy[phy_index].flags & ELINK_FLAGS_REARM_LATCH_SIGNAL)
        clear_latch_ind = 1;
    }
  ...
  if (!CHIP_REV_IS_SLOW(sc))
    ...
    if (params->phy[ELINK_INT_PHY].link_reset)
      params->phy[ELINK_INT_PHY].link_reset(&params->phy[ELINK_INT_PHY],
                                            params);
  ...
  if (!CHIP_IS_E3(sc)) {
    ...
    if (REG_RD(sc, MISC_REG_RESET_REG_2) & MISC_REGISTERS_RESET_REG_2_XMAC)
      REG_WR(sc, xmac_base + XMAC_REG_CTRL, XMAC_CTRL_REG_SOFT_RESET);
  }
  vars->link_up = 0;
  vars->phy_flags = 0;
  return ELINK_STATUS_OK;
...
/****************************************************************************/
static elink_status_t
elink_8073_common_init_phy(struct bnx2x_softc *sc, uint32_t shmem_base_path[],
  ...
  for (port = PORT_MAX - 1; port >= PORT_0; port--) {
    ...
    if (elink_populate_phy(sc, phy_index, shmem_base, shmem2_base, port_of_path,
                           &phy[port]) != ELINK_STATUS_OK) {
      ELINK_DEBUG_P0(sc, "populate_phy failed");
      return ELINK_STATUS_ERROR;
    }
    /* Disable attentions */
    elink_bits_dis(
        sc, NIG_REG_MASK_INTERRUPT_PORT0 + port_of_path * 4,
        (ELINK_NIG_MASK_XGXS0_LINK_STATUS | ELINK_NIG_MASK_XGXS0_LINK10G |
         ELINK_NIG_MASK_SERDES0_LINK_STATUS | ELINK_NIG_MASK_MI_INT));
    ...
    /* Reset the phy */
    elink_cl45_write(sc, &phy[port], MDIO_PMA_DEVAD, MDIO_PMA_REG_CTRL,
                     1 << 15);
  }
  ...
  for (port = PORT_MAX - 1; port >= PORT_0; port--) {
    ...
    ELINK_DEBUG_P1(sc, "Loading spirom for phy address 0x%x",
                   phy_blk[port]->addr);
    if (elink_8073_8727_external_rom_boot(sc, phy_blk[port], port_of_path))
      return ELINK_STATUS_ERROR;

    /* Only set bit 10 = 1 (Tx power down) */
    elink_cl45_read(sc, phy_blk[port], MDIO_PMA_DEVAD,
                    MDIO_PMA_REG_TX_POWER_DOWN, &val);

    /* Phase1 of TX_POWER_DOWN reset */
    elink_cl45_write(sc, phy_blk[port], MDIO_PMA_DEVAD,
...
static elink_status_t
elink_8726_common_init_phy(struct bnx2x_softc *sc, uint32_t shmem_base_path[],
                           uint32_t shmem2_base_path[], uint8_t phy_index,
                           __rte_unused uint32_t chip_id) {
  uint32_t val;
  int8_t port;
  ...
  for (port = 0; port < PORT_MAX; port++) {
    ...
    /* Extract the ext phy address for the port */
    if (elink_populate_phy(sc, phy_index, shmem_base, shmem2_base, port,
                           &phy) != ELINK_STATUS_OK) {
    ...
    elink_cb_gpio_write(sc, MISC_REGISTERS_GPIO_0, MISC_REGISTERS_GPIO_HIGH,
                        port);
  }
...
static void elink_get_ext_phy_reset_gpio(struct bnx2x_softc *sc,
  ...
  uint32_t phy_gpio_reset = REG_RD(
      sc, shmem_base + offsetof(struct shmem_region,
...
static elink_status_t
elink_8727_common_init_phy(struct bnx2x_softc *sc, uint32_t shmem_base_path[],
                           uint32_t shmem2_base_path[], uint8_t phy_index,
                           __rte_unused uint32_t chip_id) {
   ...
   */
  elink_get_ext_phy_reset_gpio(sc, shmem_base_path[0], (uint8_t *)&reset_gpio,
                               (uint8_t *)&port);
  ...
  /* Initiate PHY reset*/
  elink_cb_gpio_write(sc, reset_gpio, MISC_REGISTERS_GPIO_OUTPUT_LOW, port);
  DELAY(1000 * 1);
  elink_cb_gpio_write(sc, reset_gpio, MISC_REGISTERS_GPIO_OUTPUT_HIGH, port);
  ...
  for (port = PORT_MAX - 1; port >= PORT_0; port--) {
    ...
    elink_cl45_write(sc, phy_blk[port], MDIO_PMA_DEVAD, MDIO_PMA_REG_TX_DISABLE,
                     1);
  }
  return ELINK_STATUS_OK;
}
...
static elink_status_t
elink_84833_common_init_phy(struct bnx2x_softc *sc, uint32_t shmem_base_path[],
                            __rte_unused uint32_t shmem2_base_path[],
                            __rte_unused uint8_t phy_index, uint32_t chip_id) {
  uint8_t reset_gpios;
  ...
  ELINK_DEBUG_P1(sc, "84833 reset pulse on pin values 0x%x", reset_gpios);
  return ELINK_STATUS_OK;
}
static elink_status_t
elink_ext_phy_common_init(struct bnx2x_softc *sc, uint32_t shmem_base_path[],
                          uint32_t shmem2_base_path[], uint8_t phy_index,
  ...
  switch (ext_phy_type) {
  case PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X8073:
    rc = elink_8073_common_init_phy(sc, shmem_base_path, shmem2_base_path,
                                    phy_index, chip_id);
    break;
  case PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X8722:
  case PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X8727:
  case PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X8727_NOC:
    rc = elink_8727_common_init_phy(sc, shmem_base_path, shmem2_base_path,
                                    phy_index, chip_id);
    break;
  ...
  case PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X8726:
     ...
     */
    rc = elink_8726_common_init_phy(sc, shmem_base_path, shmem2_base_path,
                                    phy_index, chip_id);
    break;
  case PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84833:
  case PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84834:
  case PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BNX2X84858:
     ...
     */
    rc = elink_84833_common_init_phy(sc, shmem_base_path, shmem2_base_path,
                                     phy_index, chip_id);
    break;
  case PORT_HW_CFG_XGXS_EXT_PHY_TYPE_FAILURE:
    rc = ELINK_STATUS_ERROR;
    break;
  default:
    ELINK_DEBUG_P1(sc, "ext_phy 0x%x common init not required", ext_phy_type);
    break;
  }
...
elink_status_t elink_common_init_phy(struct bnx2x_softc *sc,
  ...
  if (phy_ver) {
    ELINK_DEBUG_P1(sc, "Not doing common init; phy ver is 0x%x", phy_ver);
    return ELINK_STATUS_OK;
  }

  /* Read the ext_phy_type for arbitrary port(0) */
  for (phy_index = ELINK_EXT_PHY1; phy_index < ELINK_MAX_PHYS; phy_index++) {
    ext_phy_config =
        elink_get_ext_phy_config(sc, shmem_base_path[0], phy_index, 0);
    ext_phy_type = ELINK_XGXS_EXT_PHY_TYPE(ext_phy_config);
    rc |= elink_ext_phy_common_init(sc, shmem_base_path, shmem2_base_path,
                                    phy_index, ext_phy_type, chip_id);
  }
  return rc;
}
...
static void elink_check_over_curr(struct elink_params *params,
  ...
  if (!pin_val) {
    if ((vars->phy_flags & PHY_OVER_CURRENT_FLAG) == 0) {
      elink_cb_event_log(sc, ELINK_LOG_ID_OVER_CURRENT, params->port);
      /* "Error:  Power fault on Port %d has"
       *  " been detected and the power to "
       *  "that SFP+ module has been removed"
       *  " to prevent failure of the card."
...
static uint8_t elink_analyze_link_error(struct elink_params *params,
  ...
  ELINK_DEBUG_P3(sc, "Link changed:[%x %x]->%x", vars->link_up, old_status,
                 status);
...
static elink_status_t elink_check_half_open_conn(struct elink_params *params,
  ...
  if (CHIP_IS_E3(sc) &&
    ...
    elink_analyze_link_error(params, vars, lss_status, PHY_HALF_OPEN_CONN_FLAG,
...
static void elink_sfp_tx_fault_detection(struct elink_phy *phy,
  ...
  led_change = elink_analyze_link_error(
      params, vars, value, PHY_SFP_TX_FAULT_FLAG, LINK_STATUS_SFP_TX_FAULT, 1);
  ...
  if (led_change) {
    ...
    if (!(phy->flags & ELINK_FLAGS_SFP_NOT_APPROVED)) {
      ELINK_DEBUG_P1(sc, "Change TX_Fault LED: ->%x", led_mode);
...
static void elink_check_kr2_wa(struct elink_params *params,
  ...
  CL22_WR_OVER_CL45(sc, phy, MDIO_REG_BANK_AER_BLOCK, MDIO_AER_BLOCK_AER_REG,
                    lane);
  elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_LP_AUTO_NEG, &base_page);
  elink_cl45_read(sc, phy, MDIO_AN_DEVAD, MDIO_AN_REG_LP_AUTO_NEG2, &next_page);
  elink_set_aer_mmd(params, phy);
  ...
  not_kr2_device = (((base_page & 0x8000) == 0) ||
                    (((base_page & 0x8000) && ((next_page & 0xe0) == 0x20))));
...
void elink_period_func(struct elink_params *params, struct elink_vars *vars) {
  ...
  for (phy_idx = ELINK_INT_PHY; phy_idx < ELINK_MAX_PHYS; phy_idx++) {
    if (params->phy[phy_idx].flags & ELINK_FLAGS_TX_ERROR_CHECK) {
      ...
      if (elink_check_half_open_conn(params, vars, 1) != ELINK_STATUS_OK)
  ...
  if (CHIP_IS_E3(sc)) {
    ...
    if ((REG_RD(
             ...
             params->shmem_base +
                 offsetof(struct shmem_region,
                          dev_info.port_hw_config[params->port].default_cfg)) &
         PORT_HW_CFG_NET_SERDES_IF_MASK) == PORT_HW_CFG_NET_SERDES_IF_SFI) {
      if (elink_is_sfp_module_plugged(phy, params)) {
...
uint8_t elink_fan_failure_det_req(struct bnx2x_softc *sc, uint32_t shmem_base,
  ...
  for (phy_index = ELINK_EXT_PHY1; phy_index < ELINK_MAX_PHYS; phy_index++) {
    ...
    fan_failure_det_req |= (phy.flags & ELINK_FLAGS_FAN_FAILURE_DET_REQ);
  }
...
void elink_hw_reset_phy(struct elink_params *params) {
  ...
  for (phy_index = ELINK_INT_PHY; phy_index < ELINK_MAX_PHYS; phy_index++) {
    if (params->phy[phy_index].hw_reset) {
      params->phy[phy_index].hw_reset(&params->phy[phy_index], params);
...
void elink_init_mod_abs_int(struct bnx2x_softc *sc, struct elink_vars *vars,
  ...
  if (CHIP_IS_E3(sc)) {
    if (elink_get_mod_abs_int_cfg(sc, chip_id, shmem_base, port, &gpio_num,
                                  &gpio_port) != ELINK_STATUS_OK)
      return;
  } else {
    struct elink_phy phy;
  ...
  ELINK_DEBUG_P3(sc, "Setting MOD_ABS (GPIO%d_P%d) AEU to 0x%x", gpio_num,
                 gpio_port, vars->aeu_int_mask);
