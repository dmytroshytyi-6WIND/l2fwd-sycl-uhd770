...
static void ecore_write_big_buf_wb(struct bnx2x_softc *sc, uint32_t addr,
                                   uint32_t len) {
  if (DMAE_READY(sc))
    ecore_write_dmae_phys_len(sc, GUNZIP_PHYS(sc), addr, len);
...
static const uint8_t *ecore_sel_blob(struct bnx2x_softc *sc, uint32_t addr,
  ...
  else IF_IS_INT_TABLE_ADDR(CSEM_REG_INT_TABLE, addr) data =
      INIT_CSEM_INT_TABLE_DATA(sc);
  else IF_IS_INT_TABLE_ADDR(USEM_REG_INT_TABLE, addr) data =
      INIT_USEM_INT_TABLE_DATA(sc);
  else IF_IS_INT_TABLE_ADDR(XSEM_REG_INT_TABLE, addr) data =
      INIT_XSEM_INT_TABLE_DATA(sc);
  else IF_IS_PRAM_ADDR(TSEM_REG_PRAM, addr) data = INIT_TSEM_PRAM_DATA(sc);
  else IF_IS_PRAM_ADDR(CSEM_REG_PRAM, addr) data = INIT_CSEM_PRAM_DATA(sc);
  else IF_IS_PRAM_ADDR(USEM_REG_PRAM, addr) data = INIT_USEM_PRAM_DATA(sc);
  else IF_IS_PRAM_ADDR(XSEM_REG_PRAM, addr) data = INIT_XSEM_PRAM_DATA(sc);
...
static void ecore_init_wr_zp(struct bnx2x_softc *sc, uint32_t addr,
  ...
  for (i = 0; i < len; i++)
    ((uint32_t *)GUNZIP_BUF(sc))[i] =
        (uint32_t)ECORE_CPU_TO_LE32(((uint32_t *)GUNZIP_BUF(sc))[i]);

  ecore_write_big_buf_wb(sc, addr, len);
}

static void ecore_init_block(struct bnx2x_softc *sc, uint32_t block,
                             uint32_t stage) {
  uint16_t op_start =
      INIT_OPS_OFFSETS(sc)[BLOCK_OPS_IDX(block, stage, STAGE_START)];
  uint16_t op_end =
      INIT_OPS_OFFSETS(sc)[BLOCK_OPS_IDX(block, stage, STAGE_END)];
  ...
  for (op_idx = op_start; op_idx < op_end; op_idx++) {
    ...
    case OP_IF_MODE_AND:
       ...
       * conditional block.
       */
      if ((INIT_MODE_FLAGS(sc) & op->if_mode.mode_bit_map) !=
    ...
    case OP_IF_MODE_OR:
       ...
       * block.
       */
      if ((INIT_MODE_FLAGS(sc) & op->if_mode.mode_bit_map) == 0)
        op_idx += op->if_mode.cmd_offset;
      break;
    default:
...
static const struct arb_line read_arb_addr[NUM_RD_Q - 1] = {
    ...
    {PXP2_REG_PSWRQ_BW_L1, PXP2_REG_PSWRQ_BW_ADD1, PXP2_REG_PSWRQ_BW_UB1},
    {PXP2_REG_PSWRQ_BW_L2, PXP2_REG_PSWRQ_BW_ADD2, PXP2_REG_PSWRQ_BW_UB2},
    {PXP2_REG_PSWRQ_BW_L3, PXP2_REG_PSWRQ_BW_ADD3, PXP2_REG_PSWRQ_BW_UB3},
    {PXP2_REG_RQ_BW_RD_L4, PXP2_REG_RQ_BW_RD_ADD4, PXP2_REG_RQ_BW_RD_UBOUND4},
    {PXP2_REG_RQ_BW_RD_L5, PXP2_REG_RQ_BW_RD_ADD5, PXP2_REG_RQ_BW_RD_UBOUND5},
    ...
    {PXP2_REG_PSWRQ_BW_L8, PXP2_REG_PSWRQ_BW_ADD8, PXP2_REG_PSWRQ_BW_UB8},
    /* 10 */
    {PXP2_REG_PSWRQ_BW_L9, PXP2_REG_PSWRQ_BW_ADD9, PXP2_REG_PSWRQ_BW_UB9},
    {PXP2_REG_PSWRQ_BW_L10, PXP2_REG_PSWRQ_BW_ADD10, PXP2_REG_PSWRQ_BW_UB10},
    {PXP2_REG_PSWRQ_BW_L11, PXP2_REG_PSWRQ_BW_ADD11, PXP2_REG_PSWRQ_BW_UB11},
    ...
    {PXP2_REG_PSWRQ_BW_L28, PXP2_REG_PSWRQ_BW_ADD28, PXP2_REG_PSWRQ_BW_UB28}};
...
static void ecore_init_pxp_arb(struct bnx2x_softc *sc, int r_order,
  ...
  if (r_order > MAX_RD_ORD) {
    ECORE_MSG(sc, "read order of %d  order adjusted to %d", r_order,
              MAX_RD_ORD);
    r_order = MAX_RD_ORD;
  }
  if (w_order > MAX_WR_ORD) {
    ECORE_MSG(sc, "write order of %d  order adjusted to %d", w_order,
              MAX_WR_ORD);
    w_order = MAX_WR_ORD;
  }
  ...
  for (i = 0; i < NUM_RD_Q - 1; i++) {
    ...
    REG_WR(sc, read_arb_addr[i].ubound, read_arb_data[i][r_order].ubound);
  }
  ...
  for (i = 0; i < NUM_WR_Q - 1; i++) {
    if ((write_arb_addr[i].l == PXP2_REG_RQ_BW_WR_L29) ||
      ...
      REG_WR(sc, write_arb_addr[i].l, write_arb_data[i][w_order].l);

      REG_WR(sc, write_arb_addr[i].add, write_arb_data[i][w_order].add);

      REG_WR(sc, write_arb_addr[i].ubound, write_arb_data[i][w_order].ubound);
    } else {
...
static int ecore_ilt_client_mem_op(struct bnx2x_softc *sc, int cli_num,
  ...
  for (rc = 0, i = ilt_cli->start; i <= ilt_cli->end && !rc; i++) {
    rc = ecore_ilt_line_mem_op(sc, &ilt->lines[i], ilt_cli->page_size, memop);
  }
  return rc;
}
...
static void ecore_ilt_boundary_init_op(struct bnx2x_softc *sc,
  ...
  if (CHIP_IS_E1(sc)) {
    ...
    REG_WR(sc, start_reg + SC_FUNC(sc) * 4,
           ILT_RANGE((ilt_start + ilt_cli->start), (ilt_start + ilt_cli->end)));
  } else {
    switch (ilt_cli->client_num) {
    case ILT_CLIENT_CDU:
...
static void ecore_ilt_client_id_init_op(struct bnx2x_softc *sc, int cli_num,
                                        uint8_t initop) {
  struct ecore_ilt *ilt = SC_ILT(sc);
  struct ilt_client_info *ilt_cli = &ilt->clients[cli_num];
...
static void ecore_ilt_init_page_size(struct bnx2x_softc *sc, uint8_t initop) {
  ecore_ilt_init_client_psz(sc, ILT_CLIENT_CDU, PXP2_REG_RQ_CDU_P_SIZE, initop);
  ecore_ilt_init_client_psz(sc, ILT_CLIENT_QM, PXP2_REG_RQ_QM_P_SIZE, initop);
  ecore_ilt_init_client_psz(sc, ILT_CLIENT_SRC, PXP2_REG_RQ_SRC_P_SIZE, initop);
  ecore_ilt_init_client_psz(sc, ILT_CLIENT_TM, PXP2_REG_RQ_TM_P_SIZE, initop);
}
...
static void ecore_qm_init_cid_count(struct bnx2x_softc *sc, int qm_cid_count,
  ...
  if (QM_INIT(qm_cid_count)) {
    ...
    case INITOP_INIT:
      /* set in the init-value array */
    case INITOP_SET:
      REG_WR(sc, QM_REG_CONNNUM_0 + port * 4, qm_cid_count / 16 - 1);
      break;
    case INITOP_CLEAR:
      break;
    }
...
static void ecore_qm_set_ptr_table(struct bnx2x_softc *sc, int qm_cid_count,
  ...
  for (i = 0; i < 4 * QM_QUEUES_PER_FUNC; i++) {
    REG_WR(sc, base_reg + i * 4, qm_cid_count * 4 * (i % QM_QUEUES_PER_FUNC));
    ecore_init_wr_wb(sc, reg + i * 8, wb_data, 2);
  }
}
...
static void ecore_qm_init_ptr_table(struct bnx2x_softc *sc, int qm_cid_count,
  ...
  case INITOP_INIT:
    /* set in the init-value array */
  case INITOP_SET:
    ecore_qm_set_ptr_table(sc, qm_cid_count, QM_REG_BASEADDR, QM_REG_PTRTBL);
    if (CHIP_IS_E1H(sc))
      ecore_qm_set_ptr_table(sc, qm_cid_count, QM_REG_BASEADDR_EXT_A,
                             QM_REG_PTRTBL_EXT_A);
    break;
  case INITOP_CLEAR:
    break;
