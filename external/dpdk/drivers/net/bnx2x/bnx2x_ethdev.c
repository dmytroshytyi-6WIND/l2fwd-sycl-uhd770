...
static const struct rte_bnx2x_xstats_name_off bnx2x_xstats_strings[] = {
    {"rx_buffer_drops", offsetof(struct bnx2x_eth_stats, brb_drop_hi),
     offsetof(struct bnx2x_eth_stats, brb_drop_lo)},
    {"rx_buffer_truncates", offsetof(struct bnx2x_eth_stats, brb_truncate_hi),
    ...
    {"rx_buffer_truncate_discard",
     ...
     offsetof(struct bnx2x_eth_stats, brb_truncate_discard)},
    {"mac_filter_discard", offsetof(struct bnx2x_eth_stats, mac_filter_discard),
    ...
    {"no_match_vlan_tag_discard",
     ...
     offsetof(struct bnx2x_eth_stats, mf_tag_discard)},
    {"tx_pause", offsetof(struct bnx2x_eth_stats, pause_frames_sent_hi),
     offsetof(struct bnx2x_eth_stats, pause_frames_sent_lo)},
    {"rx_pause", offsetof(struct bnx2x_eth_stats, pause_frames_received_hi),
...
static int bnx2x_link_update(struct rte_eth_dev *dev) {
...
static void bnx2x_interrupt_action(struct rte_eth_dev *dev, int intr_cxt) {
  struct bnx2x_softc *sc = dev->data->dev_private;
  uint32_t link_status;

  bnx2x_intr_legacy(sc);

  if ((atomic_load_acq_long(&sc->periodic_flags) == PERIODIC_GO) && !intr_cxt)
    bnx2x_periodic_callout(sc);
  link_status =
      REG_RD(sc, sc->link_params.shmem_base +
...
static void bnx2x_interrupt_handler(void *param) {
...
static void bnx2x_periodic_start(void *param) {
  ...
  if (IS_PF(sc)) {
    ret = rte_eal_alarm_set(BNX2X_SP_TIMER_PERIOD, bnx2x_periodic_start,
                            (void *)dev);
    if (ret) {
      PMD_DRV_LOG(ERR, sc,
...
static int bnx2x_dev_configure(struct rte_eth_dev *dev) {
  struct bnx2x_softc *sc = dev->data->dev_private;
  ...
  PMD_DRV_LOG(DEBUG, sc, "num_queues=%d, mtu=%d", sc->num_queues, sc->mtu);
...
static int bnx2x_dev_start(struct rte_eth_dev *dev) {
  struct bnx2x_softc *sc = dev->data->dev_private;
  int ret = 0;
...
static int bnx2x_dev_stop(struct rte_eth_dev *dev) {
  struct bnx2x_softc *sc = dev->data->dev_private;
  int ret = 0;
...
static int bnx2x_dev_close(struct rte_eth_dev *dev) {
  struct bnx2x_softc *sc = dev->data->dev_private;
...
static int bnx2x_promisc_enable(struct rte_eth_dev *dev) {
  struct bnx2x_softc *sc = dev->data->dev_private;
...
static int bnx2x_promisc_disable(struct rte_eth_dev *dev) {
  struct bnx2x_softc *sc = dev->data->dev_private;
...
static int bnx2x_dev_allmulticast_enable(struct rte_eth_dev *dev) {
  struct bnx2x_softc *sc = dev->data->dev_private;
...
static int bnx2x_dev_allmulticast_disable(struct rte_eth_dev *dev) {
  struct bnx2x_softc *sc = dev->data->dev_private;
...
static int bnx2x_dev_set_mc_addr_list(struct rte_eth_dev *dev,
...
static int bnx2x_dev_link_update(struct rte_eth_dev *dev,
...
static int bnx2xvf_dev_link_update(struct rte_eth_dev *dev,
                                   __rte_unused int wait_to_complete) {
  struct bnx2x_softc *sc = dev->data->dev_private;
  int ret = 0;
...
static int bnx2x_dev_stats_get(struct rte_eth_dev *dev,
  ...
  stats->ibytes = HILO_U64(sc->eth_stats.total_bytes_received_hi,
                           sc->eth_stats.total_bytes_received_lo);

  stats->obytes = HILO_U64(sc->eth_stats.total_bytes_transmitted_hi,
                           sc->eth_stats.total_bytes_transmitted_lo);

  stats->ierrors = HILO_U64(sc->eth_stats.error_bytes_received_hi,
                            sc->eth_stats.error_bytes_received_lo);

  stats->oerrors = 0;

  stats->rx_nombuf = HILO_U64(sc->eth_stats.no_buff_discard_hi,
                              sc->eth_stats.no_buff_discard_lo);

  brb_drops = HILO_U64(sc->eth_stats.brb_drop_hi, sc->eth_stats.brb_drop_lo);

  brb_truncates =
      HILO_U64(sc->eth_stats.brb_truncate_hi, sc->eth_stats.brb_truncate_lo);

  brb_truncate_discard = sc->eth_stats.brb_truncate_discard;

  stats->imissed =
      brb_drops + brb_truncates + brb_truncate_discard + stats->rx_nombuf;

  return 0;
}

static int bnx2x_get_xstats_names(__rte_unused struct rte_eth_dev *dev,
  ...
  if (xstats_names != NULL)
    for (i = 0; i < stat_cnt; i++)
      strlcpy(xstats_names[i].name, bnx2x_xstats_strings[i].name,
              sizeof(xstats_names[i].name));

  return stat_cnt;
}

static int bnx2x_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  for (num = 0; num < n; num++) {
    if (bnx2x_xstats_strings[num].offset_hi !=
      ...
      xstats[num].value =
          HILO_U64(*(uint32_t *)((char *)&sc->eth_stats +
                                 bnx2x_xstats_strings[num].offset_hi),
                   *(uint32_t *)((char *)&sc->eth_stats +
                                 bnx2x_xstats_strings[num].offset_lo));
    else
      xstats[num].value = *(uint64_t *)((char *)&sc->eth_stats +
                                        bnx2x_xstats_strings[num].offset_lo);
    xstats[num].id = num;
  }
...
static int bnx2x_dev_infos_get(struct rte_eth_dev *dev,
...
static int bnx2x_mac_addr_add(struct rte_eth_dev *dev,
...
static void bnx2x_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
  struct bnx2x_softc *sc = dev->data->dev_private;
...
static int bnx2x_common_dev_init(struct rte_eth_dev *eth_dev, int is_vf) {
  int ret = 0;
  ...
  if (is_vf)
    sc->bar[BAR1].base_addr =
        (void *)((uintptr_t)pci_dev->mem_resource[0].addr +
  ...
  if (eth_dev->data->dev_conf.rx_adv_conf.rss_conf.rss_hf &
  ...
  if (IS_VF(sc)) {
    ...
    sc->pf2vf_bulletin =
        (struct bnx2x_vf_bulletin *)sc->pf2vf_bulletin_mapping.vaddr;

    ret = bnx2x_vf_get_resources(sc, sc->max_tx_queues, sc->max_rx_queues);
    if (ret)
      goto out;
  }
...
static int eth_bnx2x_dev_init(struct rte_eth_dev *eth_dev) {
  struct bnx2x_softc *sc = eth_dev->data->dev_private;
  PMD_INIT_FUNC_TRACE(sc);
  return bnx2x_common_dev_init(eth_dev, 0);
}

static int eth_bnx2xvf_dev_init(struct rte_eth_dev *eth_dev) {
  struct bnx2x_softc *sc = eth_dev->data->dev_private;
  PMD_INIT_FUNC_TRACE(sc);
...
static int eth_bnx2x_pci_probe(struct rte_pci_driver *pci_drv,
  ...
  if (pci_drv == &rte_bnx2x_pmd)
    return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct bnx2x_softc),
                                         eth_bnx2x_dev_init);
