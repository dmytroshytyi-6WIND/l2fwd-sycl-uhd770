...
#define PMD_DRV_LOG_RAW(level, sc, fmt, args...)                               \
  rte_log(RTE_LOG_##level, bnx2x_logtype_driver, "[%s:%d(%s)] " fmt, __func__, \
          __LINE__, (sc)->devinfo.name ? (sc)->devinfo.name : "", ##args)
...
#define PMD_RX_LOG(level, fmt, args...)                                        \
  rte_log(RTE_LOG_##level, bnx2x_logtype_driver, "%s(): " fmt "\n", __func__,  \
          ##args)
#else
#define PMD_RX_LOG(level, fmt, args...)                                        \
  do {                                                                         \
...
#define PMD_DEBUG_PERIODIC_LOG(level, sc, fmt, args...)                        \
  rte_log(RTE_LOG_##level, bnx2x_logtype_driver, "%s(%s): " fmt "\n",          \
          __func__, (sc)->devinfo.name ? (sc)->devinfo.name : "", ##args)
#else
#define PMD_DEBUG_PERIODIC_LOG(level, sc, fmt, args...)                        \
  do {                                                                         \
  } while (0)
