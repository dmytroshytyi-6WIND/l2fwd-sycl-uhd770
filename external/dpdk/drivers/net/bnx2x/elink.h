...
extern void elink_cb_reg_write(struct bnx2x_softc *sc, uint32_t reg_addr,
                               uint32_t val);
...
/* mode - 0( LOW ) /1(HIGH)*/
extern uint8_t elink_cb_gpio_write(struct bnx2x_softc *sc, uint16_t gpio_num,
                                   uint8_t mode, uint8_t port);
extern uint8_t elink_cb_gpio_mult_write(struct bnx2x_softc *sc, uint8_t pins,
                                        uint8_t mode);
...
extern uint8_t elink_cb_gpio_int_write(struct bnx2x_softc *sc,
                                       uint16_t gpio_num, uint8_t mode,
                                       uint8_t port);

extern uint32_t elink_cb_fw_command(struct bnx2x_softc *sc, uint32_t command,
                                    uint32_t param);
...
extern void elink_cb_download_progress(struct bnx2x_softc *sc, uint32_t cur,
                                       uint32_t total);

/* Each log type has its own parameters */
typedef enum elink_log_id {
  ELINK_LOG_ID_UNQUAL_IO_MODULE =
      0, /* uint8_t port, const char* vendor_name, const char* vendor_pn */
...
extern void elink_cb_event_log(struct bnx2x_softc *sc,
                               const elink_log_id_t log_id, ...);
...
#define ELINK_DEBUG_P1(sc, fmt, arg1) PMD_DRV_LOG(DEBUG, sc, fmt, arg1)
...
#define ELINK_FW_PARAM_PHY_ADDR(fw_param)                                      \
  (fw_param & ELINK_FW_PARAM_PHY_ADDR_MASK)
#define ELINK_FW_PARAM_PHY_TYPE(fw_param)                                      \
  (fw_param & ELINK_FW_PARAM_PHY_TYPE_MASK)
#define ELINK_FW_PARAM_MDIO_CTRL(fw_param)                                     \
  ((fw_param & ELINK_FW_PARAM_MDIO_CTRL_MASK) >>                               \
...
#define ELINK_LINK_CONFIG_IDX(_phy_idx)                                        \
  ((_phy_idx == ELINK_INT_PHY) ? 0 : (_phy_idx - 1))
/***********************************************************/
/*                      elink_phy struct                   */
...
typedef uint8_t (*config_init_t)(struct elink_phy *phy,
                                 struct elink_params *params,
                                 struct elink_vars *vars);
typedef uint8_t (*read_status_t)(struct elink_phy *phy,
                                 struct elink_params *params,
                                 struct elink_vars *vars);
...
typedef void (*config_loopback_t)(struct elink_phy *phy,
                                  struct elink_params *params);
typedef elink_status_t (*format_fw_ver_t)(uint32_t raw, uint8_t *str,
                                          uint16_t *len);
...
elink_status_t elink_link_reset(struct elink_params *params,
                                struct elink_vars *vars, uint8_t reset_ext_phy);
...
elink_status_t elink_test_link(struct elink_params *params,
                               struct elink_vars *vars, uint8_t is_serdes);
...
elink_status_t elink_read_sfp_module_eeprom(struct elink_phy *phy,
                                            ...
                                            uint16_t byte_cnt, uint8_t *o_buf);
...
#define ELINK_DCBX_MAX_NUM_COS                                                 \
  (ELINK_MAXVAL(ELINK_DCBX_E3B0_MAX_NUM_COS, ELINK_DCBX_E2E3_MAX_NUM_COS))
 ...
 */
elink_status_t
elink_update_pfc(struct elink_params *params, struct elink_vars *vars,
...
void elink_ets_bw_limit(const struct elink_params *params,
                        const uint32_t cos0_bw, const uint32_t cos1_bw);
