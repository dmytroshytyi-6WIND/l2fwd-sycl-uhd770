...
#define CSTORM_EVENT_RING_DATA_OFFSET(pfId)                                    \
  (IRO[157].base + (((pfId) >> 1) * IRO[157].m1) + (((pfId)&1) * IRO[157].m2))
#define CSTORM_EVENT_RING_PROD_OFFSET(pfId)                                    \
  (IRO[158].base + (((pfId) >> 1) * IRO[158].m1) + (((pfId)&1) * IRO[158].m2))
...
#define XSTORM_TCP_TX_SWS_TIMER_VAL_OFFSET(pfId)                               \
  (IRO[220].base + (((pfId) >> 1) * IRO[220].m1) + (((pfId)&1) * IRO[220].m2))
...
#define U_ETH_BDS_PER_PAGE (rte_mem_page_size() / (STRUCT_SIZE(eth_rx_bd) / 8))
