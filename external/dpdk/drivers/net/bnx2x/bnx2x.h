...
#endif
#ifndef ilog2
static inline int bnx2x_ilog2(int x) {
  int log = 0;
  x >>= 1;
...
#define TOTAL_TX_BD_PER_PAGE                                                   \
  (BNX2X_PAGE_SIZE / sizeof(union eth_tx_bd_types))      /*  256 */
...
#define NEXT_TX_BD(x)                                                          \
  ((((x)&USABLE_TX_BD_PER_PAGE) == (USABLE_TX_BD_PER_PAGE - 1)) ? (x) + 2      \
                                                                : (x) + 1)
...
#define TOTAL_RX_BD_PER_PAGE                                                   \
  (BNX2X_PAGE_SIZE / sizeof(struct eth_rx_bd))                   /*  512 */
...
#define NEXT_RX_BD(x)                                                          \
  ((((x)&RX_BD_PER_PAGE_MASK) == (USABLE_RX_BD_PER_PAGE - 1)) ? (x) + 3        \
                                                              : (x) + 1)
...
#define TOTAL_RCQ_ENTRIES_PER_PAGE                                             \
  (BNX2X_PAGE_SIZE / sizeof(union eth_rx_cqe)) /*  128 */
#define USABLE_RCQ_ENTRIES_PER_PAGE                                            \
  (TOTAL_RCQ_ENTRIES_PER_PAGE - 1) /*  127                                     \
                                    */
#define TOTAL_RCQ_ENTRIES(q)                                                   \
  (TOTAL_RCQ_ENTRIES_PER_PAGE * q->nb_cq_pages) /*  512 */
#define USABLE_RCQ_ENTRIES(q)                                                  \
  (USABLE_RCQ_ENTRIES_PER_PAGE * q->nb_cq_pages)      /*  508 */
#define MAX_RCQ_ENTRIES(q) (TOTAL_RCQ_ENTRIES(q) - 1) /*  511 */
#define RCQ_NEXT_PAGE_DESC_CNT 1

#define NEXT_RCQ_IDX(x)                                                        \
  ((((x)&USABLE_RCQ_ENTRIES_PER_PAGE) == (USABLE_RCQ_ENTRIES_PER_PAGE - 1))    \
       ? (x) + 2                                                               \
       : (x) + 1)

#define CQE_BD_REL (sizeof(union eth_rx_cqe) / sizeof(struct eth_rx_bd))

#define RCQ_BD_PAGES(q) (q->nb_rx_pages * CQE_BD_REL)
...
#define BD_TH_LO(sc)                                                           \
  (NUM_BD_REQ(sc) + NUM_BD_PG_REQ(sc) * RX_BD_NEXT_PAGE_DESC_CNT +             \
   FW_DROP_LEVEL(sc))
...
#define MIN_RX_SIZE_NONTPA                                                     \
  (RTE_MAX((uint32_t)MIN_RX_SIZE_NONTPA_HW, (uint32_t)MIN_RX_AVAIL(sc)))
...
#define NUM_RCQ_PG_REQ(sc)                                                     \
  ((NUM_RCQ_REQ(sc) + USABLE_RCQ_ENTRIES_PER_PAGE - 1) /                       \
   USABLE_RCQ_ENTRIES_PER_PAGE)
#define RCQ_TH_LO(sc)                                                          \
  (NUM_RCQ_REQ(sc) + NUM_RCQ_PG_REQ(sc) * RCQ_NEXT_PAGE_DESC_CNT +             \
   FW_DROP_LEVEL(sc))
...
#define FOR_EACH_CNIC_QUEUE(sc, var)                                           \
  for ((var) = BNX2X_NUM_ETH_QUEUES(sc); (var) < BNX2X_NUM_QUEUES(sc); (var)++)
...
#define bnx2x_fcoe_tx(sc, var)                                                 \
  (bnx2x_fcoe_fp(sc)->txdata_ptr[FIRST_TX_COS_INDEX]->var)
...
struct bnx2x_mf_info {
  ...
  uint32_t path_has_ovlan; /* MF mode in the path (can be different than the MF
...
#define NIV_ALLOWED_PRIORITIES(sc)                                             \
  ((sc)->devinfo.mf_info.niv_allowed_priorities)
...
#define IS_MF(sc) (IS_MULTI_VNIC(sc) && ((sc)->devinfo.mf_info.mf_mode != 0))
#define IS_MF_SD(sc)                                                           \
  (IS_MULTI_VNIC(sc) && ((sc)->devinfo.mf_info.mf_mode == MULTI_FUNCTION_SD))
#define IS_MF_SI(sc)                                                           \
  (IS_MULTI_VNIC(sc) && ((sc)->devinfo.mf_info.mf_mode == MULTI_FUNCTION_SI))
#define IS_MF_AFEX(sc)                                                         \
  (IS_MULTI_VNIC(sc) && ((sc)->devinfo.mf_info.mf_mode == MULTI_FUNCTION_AFEX))
...
#define CHIP_IS_E2(sc) (CHIP_IS_57712(sc) || CHIP_IS_57712_MF(sc))
...
#define CHIP_IS_E3(sc)                                                         \
  (CHIP_IS_57800(sc) || CHIP_IS_57800_MF(sc) || CHIP_IS_57800_VF(sc) ||        \
   CHIP_IS_57810(sc) || CHIP_IS_57810_MF(sc) || CHIP_IS_57810_VF(sc) ||        \
   CHIP_IS_57811(sc) || CHIP_IS_57811_MF(sc) || CHIP_IS_57811_VF(sc) ||        \
   CHIP_IS_57840(sc) || CHIP_IS_57840_MF(sc) || CHIP_IS_57840_VF(sc))
#define CHIP_IS_E3A0(sc) (CHIP_IS_E3(sc) && (CHIP_REV(sc) == CHIP_REV_Ax))
#define CHIP_IS_E3B0(sc) (CHIP_IS_E3(sc) && (CHIP_REV(sc) == CHIP_REV_Bx))

#define USES_WARPCORE(sc) (CHIP_IS_E3(sc))
#define CHIP_IS_E2E3(sc) (CHIP_IS_E2(sc) || CHIP_IS_E3(sc))

#define CHIP_IS_MF_CAP(sc)                                                     \
  (CHIP_IS_57711E(sc) || CHIP_IS_57712_MF(sc) || CHIP_IS_E3(sc))
...
#define CHIP_INT_MODE_IS_NBC(sc)                                               \
  (!CHIP_IS_E1x(sc) && !((sc)->devinfo.int_block & INT_BLOCK_MODE_BW_COMP))
...
#define SC_FW_MB_IDX_VN(sc, vn)                                                \
  (SC_PORT(sc) +                                                               \
   (vn) * ((CHIP_IS_E1x(sc) || (CHIP_IS_MODE_4_PORT(sc))) ? 2 : 1))
...
#define NEXT_EQ_IDX(x)                                                         \
  ((((x)&EQ_DESC_MAX_PAGE) == (EQ_DESC_MAX_PAGE - 1)) ? ((x) + 2) : ((x) + 1))
...
#define L2_ILT_LINES(sc) (DIV_ROUND_UP(BNX2X_L2_CID_COUNT(sc), ILT_PAGE_CIDS))
...
static inline void bnx2x_reg_write8(struct bnx2x_softc *sc, size_t offset,
                                    uint8_t val) {
...
static inline void bnx2x_reg_write16(struct bnx2x_softc *sc, size_t offset,
                                     uint16_t val) {
...
static inline void bnx2x_reg_write32(struct bnx2x_softc *sc, size_t offset,
                                     uint32_t val) {
#ifdef RTE_LIBRTE_BNX2X_DEBUG_PERIODIC
  if ((offset % 4) != 0)
...
static inline uint8_t bnx2x_reg_read8(struct bnx2x_softc *sc, size_t offset) {
  uint8_t val;
...
static inline uint16_t bnx2x_reg_read16(struct bnx2x_softc *sc, size_t offset) {
  uint16_t val;
...
static inline uint32_t bnx2x_reg_read32(struct bnx2x_softc *sc, size_t offset) {
  uint32_t val;
...
#define DOORBELL(sc, cid, val)                                                 \
  if (IS_PF(sc))                                                               \
    BNX2X_DB_WRITE(                                                            \
        (DOORBELL_ADDR(sc, sc->doorbell_size * (cid) + DPM_TRIGGER_TYPE)),     \
        (val));                                                                \
  else                                                                         \
...
#define DMAE_COMP_VAL                                                          \
  0x60d0d0ae /* E2 and beyond, upper bit indicates error                       \
              */
...
#define FIRST_ABS_FUNC_IN_PORT(sc)                                             \
  ((CHIP_PORT_MODE(sc) == CHIP_PORT_MODE_NONE)                                 \
       ? PORT_ID(sc)                                                           \
       : (PATH_ID(sc) + (2 * PORT_ID(sc))))

#define FOREACH_ABS_FUNC_IN_PORT(sc, i)                                        \
  for ((i) = FIRST_ABS_FUNC_IN_PORT(sc); (i) < MAX_FUNC_NUM;                   \
...
static inline uint32_t reg_poll(struct bnx2x_softc *sc, uint32_t reg,
                                uint32_t expected, int ms, int wait) {
  uint32_t val;
  do {
    val = REG_RD(sc, reg);
    if (val == expected) {
...
static inline void bnx2x_update_fp_sb_idx(struct bnx2x_fastpath *fp) {
  mb(); /* status block is written to by the chip */
  fp->fp_hc_idx = fp->sb_running_index[SM_RX_ID];
}

static inline void bnx2x_igu_ack_sb_gen(struct bnx2x_softc *sc, uint8_t segment,
                                        uint16_t index, uint8_t op,
                                        uint8_t update, uint32_t igu_addr) {
  struct igu_regular cmd_data = {0};

  cmd_data.sb_id_and_flags = ((index << IGU_REGULAR_SB_INDEX_SHIFT) |
...
static inline void bnx2x_hc_ack_sb(struct bnx2x_softc *sc, uint8_t sb_id,
                                   uint8_t storm, uint16_t index, uint8_t op,
                                   uint8_t update) {
  uint32_t hc_addr =
      (HC_REG_COMMAND_REG + SC_PORT(sc) * 32 + COMMAND_REG_INT_ACK);
  union {
...
static inline uint32_t bnx2x_hc_ack_int(struct bnx2x_softc *sc) {
  uint32_t hc_addr =
      (HC_REG_COMMAND_REG + SC_PORT(sc) * 32 + COMMAND_REG_SIMD_MASK);
...
static inline uint32_t bnx2x_igu_ack_int(struct bnx2x_softc *sc) {
...
static inline uint32_t bnx2x_ack_int(struct bnx2x_softc *sc) {
  mb();
...
static inline int func_by_vn(struct bnx2x_softc *sc, int vn) {
 ...
 */
static inline void bnx2x_link_sync_notify(struct bnx2x_softc *sc) {
  int func, vn;
  ...
  for (vn = VN_0; vn < SC_MAX_VN_NUM(sc); vn++) {
    ...
    REG_WR(sc,
           MISC_REG_AEU_GENERAL_ATTN_0 +
               (LINK_SYNC_ATTENTION_BIT_FUNC_0 + func) * 4,
           1);
  }
}
...
int bnx2x_nic_unload(struct bnx2x_softc *sc, uint32_t unload_mode,
                     uint8_t keep_link);
...
int bnx2x_vf_get_resources(struct bnx2x_softc *sc, uint8_t tx_count,
                           uint8_t rx_count);
...
static inline struct bnx2x_pci_cap *pci_find_cap(struct bnx2x_softc *sc,
...
static inline void bnx2x_set_rx_mode(struct bnx2x_softc *sc) {
...
static inline int pci_read(struct bnx2x_softc *sc, size_t addr, void *val,
                           uint8_t size) {
...
static inline int pci_write_word(struct bnx2x_softc *sc, size_t addr,
                                 off_t val) {
  uint16_t val16 = val;

  if (rte_pci_write_config(sc->pci_dev, &val16, sizeof(val16), addr) <= 0) {
...
static inline int pci_write_long(struct bnx2x_softc *sc, size_t addr,
                                 off_t val) {
  uint32_t val32 = val;
  if (rte_pci_write_config(sc->pci_dev, &val32, sizeof(val32), addr) <= 0) {
    PMD_DRV_LOG(ERR, sc, "Can't write to PCI config space");
    return ENXIO;
  }
