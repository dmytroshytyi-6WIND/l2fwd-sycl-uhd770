...
static inline void ecore_map_q_cos(struct bnx2x_softc *sc, uint32_t q_num,
                                   uint32_t new_cos) {
  ...
  if (curr_cos != new_cos) {
    ...
    for (vnic = 0; vnic < num_vnics; vnic++) {
      uint32_t pf_q_num = ECORE_PF_Q_NUM(q_num, SC_PORT(sc), vnic);
      ...
      if (!(INIT_MODE_FLAGS(sc) & MODE_E3_B0)) {
        ...
        reg_bit_map =
            new_cos ? (reg_bit_map | q_bit_map) : (reg_bit_map & (~q_bit_map));
...
static inline void ecore_dcb_config_qm(struct bnx2x_softc *sc,
                                       enum cos_mode mode,
                                       struct priority_cos *traffic_cos) {
  ecore_map_q_cos(sc, ECORE_FCOE_Q, traffic_cos[LLFC_TRAFFIC_TYPE_FCOE].cos);
  ecore_map_q_cos(sc, ECORE_ISCSI_Q, traffic_cos[LLFC_TRAFFIC_TYPE_ISCSI].cos);
  ecore_map_q_cos(sc, ECORE_ISCSI_ACK_Q,
                  traffic_cos[LLFC_TRAFFIC_TYPE_ISCSI].cos);
  if (mode != STATIC_COS) {
    /* required only in OVERRIDE_COS mode */
    ecore_map_q_cos(sc, ECORE_ETH_Q, traffic_cos[LLFC_TRAFFIC_TYPE_NW].cos);
    ecore_map_q_cos(sc, ECORE_TOE_Q, traffic_cos[LLFC_TRAFFIC_TYPE_NW].cos);
    ecore_map_q_cos(sc, ECORE_TOE_ACK_Q, traffic_cos[LLFC_TRAFFIC_TYPE_NW].cos);
  }
}
...
static inline void ecore_init_max(const struct cmng_init_input *input_data,
  ...
  pdata->rs_vars.rs_periodic_timeout = RS_PERIODIC_TIMEOUT_USEC / SDM_TICKS;

  /* this is the threshold below which no timer arming will occur.
   *  1.25 coefficient is for the threshold to be a little bigger
   *  then the real time to compensate for timer in-accuracy
   */
  pdata->rs_vars.rs_threshold = (5 * RS_PERIODIC_TIMEOUT_USEC * r_param) / 4;
...
static inline void
ecore_init_max_per_vn(uint16_t vnic_max_rate,
...
static inline void ecore_init_min(const struct cmng_init_input *input_data,
  ...
  /* since each tick is 4 microSeconds */
  pdata->fair_vars.fairness_timeout = fair_periodic_timeout_usec / SDM_TICKS;
  ...
  if (vnicWeightSum > 0) {
    ...
    for (vnic = 0; vnic < ECORE_PORT2_MODE_NUM_VNICS; vnic++) {
      ...
      if (vdata->vnic_min_rate[vnic].vn_credit_delta <
          pdata->fair_vars.fair_threshold + MIN_ABOVE_THRESH) {
        vdata->vnic_min_rate[vnic].vn_credit_delta =
            pdata->fair_vars.fair_threshold + MIN_ABOVE_THRESH;
      }
    }
  ...
  if (cosWeightSum > 0) {
    ...
    for (vnic = 0; vnic < ECORE_PORT2_MODE_NUM_VNICS; vnic++) {
      ...
      for (cos = 0; cos < MAX_COS_NUMBER; cos++) {
         ...
         */
        ccd[cos] = ((uint32_t)input_data->cos_min_rate[cos] * 100 *
                    (T_FAIR_COEF / (8 * 100 * cosWeightSum)));
        if (ccd[cos] < pdata->fair_vars.fair_threshold + MIN_ABOVE_THRESH) {
          ccd[cos] = pdata->fair_vars.fair_threshold + MIN_ABOVE_THRESH;
        }
      }
    }
  }
...
#define BLOCK_PRTY_INFO(block, en_mask, m1, m1h, m2, m3)                       \
  {                                                                            \
    block##_REG_##block##_PRTY_MASK, block##_REG_##block##_PRTY_STS_CLR,       \
        en_mask, {m1, m1h, m2, m3}, #block                                     \
  }

#define BLOCK_PRTY_INFO_0(block, en_mask, m1, m1h, m2, m3)                     \
  {                                                                            \
    block##_REG_##block##_PRTY_MASK_0, block##_REG_##block##_PRTY_STS_CLR_0,   \
        en_mask, {m1, m1h, m2, m3}, #block "_0"                                \
  }

#define BLOCK_PRTY_INFO_1(block, en_mask, m1, m1h, m2, m3)                     \
  {                                                                            \
    block##_REG_##block##_PRTY_MASK_1, block##_REG_##block##_PRTY_STS_CLR_1,   \
...
static const struct {
  ...
  char name[8];   /* Block's longest name is 7 characters long
     ...
     */
    BLOCK_PRTY_INFO(PXP, 0x7ffffff, 0x3ffffff, 0x3ffffff, 0x7ffffff, 0x7ffffff),
    ...
    {GRCBASE_UPB + PB_REG_PB_PRTY_MASK,
     GRCBASE_UPB + PB_REG_PB_PRTY_STS_CLR,
     0xf,
     {0xf, 0xf, 0xf, 0xf},
     "UPB"},
    {GRCBASE_XPB + PB_REG_PB_PRTY_MASK,
     GRCBASE_XPB + PB_REG_PB_PRTY_STS_CLR,
     0,
     {0xf, 0xf, 0xf, 0xf},
     "XPB"},
    ...
    BLOCK_PRTY_INFO_0(TSEM, 0, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff),
    BLOCK_PRTY_INFO_1(TSEM, 0, 0x3, 0x1f, 0x3f, 0x3f),
    BLOCK_PRTY_INFO_0(USEM, 0, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff),
    BLOCK_PRTY_INFO_1(USEM, 0, 0x3, 0x1f, 0x1f, 0x1f),
    BLOCK_PRTY_INFO_0(CSEM, 0, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff),
    BLOCK_PRTY_INFO_1(CSEM, 0, 0x3, 0x1f, 0x1f, 0x1f),
    BLOCK_PRTY_INFO_0(XSEM, 0, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff),
...
static const struct {
  ...
  uint32_t bits;
    ...
    {MISC_REG_AEU_ENABLE4_FUNC_0_OUT_0, MISC_AEU_ENABLE_MCP_PRTY_BITS},
    {MISC_REG_AEU_ENABLE4_NIG_0, MISC_AEU_ENABLE_MCP_PRTY_SUB_BITS},
    {MISC_REG_AEU_ENABLE4_PXP_0, MISC_AEU_ENABLE_MCP_PRTY_SUB_BITS},
...
static inline void ecore_set_mcp_parity(struct bnx2x_softc *sc,
                                        uint8_t enable) {
  unsigned int i;
  uint32_t reg_val;
...
static inline void ecore_disable_blocks_parity(struct bnx2x_softc *sc) {
  ...
  for (i = 0; i < ARRAY_SIZE(ecore_blocks_parity_data); i++) {
    ...
    if (dis_mask) {
      REG_WR(sc, ecore_blocks_parity_data[i].mask_addr, dis_mask);
      ECORE_MSG(sc,
                "Setting parity mask "
                "for %s to\t\t0x%x",
...
static inline void ecore_clear_blocks_parity(struct bnx2x_softc *sc) {
  unsigned int i;
  uint32_t reg_val,
      mcp_aeu_bits = AEU_INPUTS_ATTN_BITS_MCP_LATCHED_ROM_PARITY |
                     AEU_INPUTS_ATTN_BITS_MCP_LATCHED_SCPAD_PARITY |
                     AEU_INPUTS_ATTN_BITS_MCP_LATCHED_UMP_RX_PARITY |
                     AEU_INPUTS_ATTN_BITS_MCP_LATCHED_UMP_TX_PARITY;
  ...
  for (i = 0; i < ARRAY_SIZE(ecore_blocks_parity_data); i++) {
    ...
    if (reg_mask) {
      reg_val = REG_RD(sc, ecore_blocks_parity_data[i].sts_clr_addr);
      if (reg_val & reg_mask)
        ECORE_MSG(sc, "Parity errors in %s: 0x%x",
                  ecore_blocks_parity_data[i].name, reg_val & reg_mask);
    }
  }
  ...
  if (reg_val & mcp_aeu_bits)
    ECORE_MSG(sc, "Parity error in MCP: 0x%x", reg_val & mcp_aeu_bits);
