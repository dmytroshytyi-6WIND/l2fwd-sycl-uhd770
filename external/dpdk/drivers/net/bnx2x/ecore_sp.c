 ...
 */
static void ecore_exe_queue_init(struct bnx2x_softc *sc __rte_unused,
                                 struct ecore_exe_queue_obj *o, int exe_len,
                                 union ecore_qable_obj *owner,
                                 exe_q_validate validate, exe_q_remove remove,
                                 exe_q_optimize optimize, exe_q_execute exec,
                                 exe_q_get get) {
...
static inline int ecore_exe_queue_length(struct ecore_exe_queue_obj *o) {
  ...
  ECORE_LIST_FOR_EACH_ENTRY(elem, &o->exe_queue, link, struct ecore_exeq_elem)
  cnt++;
...
static void __ecore_exe_queue_reset_pending(struct bnx2x_softc *sc,
                                            struct ecore_exe_queue_obj *o) {
  struct ecore_exeq_elem *elem;

  while (!ECORE_LIST_IS_EMPTY(&o->pending_comp)) {
    elem =
        ECORE_LIST_FIRST_ENTRY(&o->pending_comp, struct ecore_exeq_elem, link);
...
static inline void
ecore_exe_queue_reset_pending(struct bnx2x_softc *sc,
                              struct ecore_exe_queue_obj *o) {
  ECORE_SPIN_LOCK_BH(&o->lock);
...
static int ecore_exe_queue_step(struct bnx2x_softc *sc,
  ...
  while (!ECORE_LIST_IS_EMPTY(&o->exe_queue)) {
    elem = ECORE_LIST_FIRST_ENTRY(&o->exe_queue, struct ecore_exeq_elem, link);
...
static struct ecore_exeq_elem *
ecore_exe_queue_alloc_elem(struct bnx2x_softc *sc __rte_unused) {
...
static void __ecore_vlan_mac_h_exec_pending(struct bnx2x_softc *sc,
  ...
  if (rc != ECORE_SUCCESS) {
    PMD_DRV_LOG(ERR, sc, "execution of pending commands failed with rc %d", rc);
#ifdef ECORE_STOP_ON_ERROR
    ecore_panic();
#endif
  }
...
static void __ecore_vlan_mac_h_pend(struct bnx2x_softc *sc __rte_unused,
  ...
  ECORE_MSG(sc, "Placing pending execution with ramrod flags %u", ramrod_flags);
}
...
static int __ecore_vlan_mac_h_read_lock(struct bnx2x_softc *sc __rte_unused,
  ...
  o->head_reader++;
  ECORE_MSG(sc, "vlan_mac_lock - locked reader - number %d", o->head_reader);
...
static void __ecore_vlan_mac_h_read_unlock(struct bnx2x_softc *sc,
  ...
  if (!o->head_reader) {
    ...
    o->head_reader--;
    ECORE_MSG(sc, "vlan_mac_lock - decreased readers to %d", o->head_reader);
  }
...
static int ecore_get_n_elements(struct bnx2x_softc *sc,
  ...
  if (read_lock != ECORE_SUCCESS)
    PMD_DRV_LOG(ERR, sc,
                "get_n_elements failed to get vlan mac reader lock; Access "
                "without lock");
  ...
  ECORE_LIST_FOR_EACH_ENTRY(pos, &o->head, link,
    ...
    if (counter < n) {
      ...
      counter++;
      ECORE_MSG(sc,
                "copied element number %d to address %p element was:", counter,
                next);
      next += stride + size;
    }
  }
...
ecore_check_mac_del(struct bnx2x_softc *sc __rte_unused,
                    struct ecore_vlan_mac_obj *o,
                    union ecore_classification_ramrod_data *data) {
  struct ecore_vlan_mac_registry_elem *pos;
...
static bool ecore_check_move_always_err(
    __rte_unused struct bnx2x_softc *sc,
    __rte_unused struct ecore_vlan_mac_obj *src_o,
    __rte_unused struct ecore_vlan_mac_obj *dst_o,
    __rte_unused union ecore_classification_ramrod_data *data) {
  return FALSE;
}
...
void ecore_set_mac_in_nig(struct bnx2x_softc *sc, bool add,
                          unsigned char *dev_addr, int index) {
  uint32_t wb_data[2];
  uint32_t reg_offset =
      ECORE_PORT_ID(sc) ? NIG_REG_LLH1_FUNC_MEM : NIG_REG_LLH0_FUNC_MEM;
  ...
  REG_WR(sc,
         (ECORE_PORT_ID(sc) ? NIG_REG_LLH1_FUNC_MEM_ENABLE
...
static void ecore_vlan_mac_set_cmd_hdr_e2(struct ecore_vlan_mac_obj *o,
  ...
  hdr->cmd_general_data |= (opcode << ETH_CLASSIFY_CMD_HEADER_OPCODE_SHIFT);
}
 ...
 * opcode type.
 */
static void ecore_vlan_mac_set_rdata_hdr_e2(uint32_t cid, int type,
                                            struct eth_classify_header *hdr,
                                            int rule_cnt) {
  hdr->echo =
      ECORE_CPU_TO_LE32((cid & ECORE_SWCID_MASK) | (type << ECORE_SWCID_SHIFT));
...
static void ecore_set_one_mac_e2(struct bnx2x_softc *sc,
  ...
  if (cmd != ECORE_VLAN_MAC_MOVE) {
    if (ECORE_TEST_BIT(ECORE_ISCSI_ETH_MAC, vlan_mac_flags))
      ecore_set_mac_in_nig(sc, add, mac, ECORE_LLH_CAM_ISCSI_ETH_LINE);
    else if (ECORE_TEST_BIT(ECORE_ETH_MAC, vlan_mac_flags))
      ecore_set_mac_in_nig(sc, add, mac, ECORE_LLH_CAM_ETH_LINE);
  }
  ...
  ECORE_MSG(sc, "About to %s MAC " RTE_ETHER_ADDR_PRT_FMT " for Queue %d",
            (add ? "add" : "delete"), mac[0], mac[1], mac[2], mac[3], mac[4],
            mac[5], raw->cl_id);

  /* Set a MAC itself */
  ecore_set_fw_mac_addr(&rule_entry->mac.mac_msb, &rule_entry->mac.mac_mid,
  ...
  if (cmd == ECORE_VLAN_MAC_MOVE) {
    ...
    /* Setup ramrod data */
    ecore_vlan_mac_set_cmd_hdr_e2(elem->cmd_data.vlan_mac.target_obj, TRUE,
 ...
 */
static void
ecore_vlan_mac_set_rdata_hdr_e1x(struct ecore_vlan_mac_obj *o, int type,
                                 int cam_offset,
...
static void
ecore_vlan_mac_set_cfg_entry_e1x(struct ecore_vlan_mac_obj *o, int add,
                                 int opcode, uint8_t *mac, uint16_t vlan_id,
  ...
  if (add) {
    ...
    ecore_set_fw_mac_addr(&cfg_entry->msb_mac_addr, &cfg_entry->middle_mac_addr,
...
static void ecore_vlan_mac_set_rdata_e1x(struct bnx2x_softc *sc __rte_unused,
                                         struct ecore_vlan_mac_obj *o, int type,
                                         int cam_offset, int add, uint8_t *mac,
  ...
  ecore_vlan_mac_set_cfg_entry_e1x(o, add, opcode, mac, vlan_id, cfg_entry);

  ECORE_MSG(sc, "%s  MAC " RTE_ETHER_ADDR_PRT_FMT " CLID %d CAM offset %d",
            (add ? "setting" : "clearing"), mac[0], mac[1], mac[2], mac[3],
            mac[4], mac[5], o->raw.cl_id, cam_offset);
}
...
static void ecore_set_one_mac_e1x(struct bnx2x_softc *sc,
   ...
   */
  int add = (elem->cmd_data.vlan_mac.cmd == ECORE_VLAN_MAC_ADD) ? TRUE : FALSE;

  /* Reset the ramrod data buffer */
  ECORE_MEMSET(config, 0, sizeof(*config));

  ecore_vlan_mac_set_rdata_e1x(sc, o, raw->state, cam_offset, add,
...
static int ecore_vlan_mac_restore(struct bnx2x_softc *sc,
  ...
  if (*ppos == NULL)
    *ppos = ECORE_LIST_FIRST_ENTRY(&o->head,
                                   struct ecore_vlan_mac_registry_elem, link);
  else
    *ppos = ECORE_LIST_NEXT(*ppos, link, struct ecore_vlan_mac_registry_elem);
 ...
 */
static struct ecore_exeq_elem *
ecore_exeq_get_mac(struct ecore_exe_queue_obj *o,
                   struct ecore_exeq_elem *elem) {
  struct ecore_exeq_elem *pos;
  struct ecore_mac_ramrod_data *data = &elem->cmd_data.vlan_mac.u.mac;

  /* Check pending for execution commands */
  ECORE_LIST_FOR_EACH_ENTRY(pos, &o->exe_queue, link, struct ecore_exeq_elem)
  if (!ECORE_MEMCMP(&pos->cmd_data.vlan_mac.u.mac, data, sizeof(*data)) &&
...
static int ecore_optimize_vlan_mac(struct bnx2x_softc *sc,
  ...
  if (pos) {
    ...
    if (!ECORE_TEST_BIT(ECORE_DONT_CONSUME_CAM_CREDIT,
      ...
      if ((query.cmd_data.vlan_mac.cmd == ECORE_VLAN_MAC_ADD) &&
          !o->put_credit(o)) {
        PMD_DRV_LOG(
            ERR, sc,
 ...
 */
static int
ecore_vlan_mac_get_registry_elem(struct bnx2x_softc *sc,
                                 struct ecore_vlan_mac_obj *o,
                                 struct ecore_exeq_elem *elem, int restore,
  ...
  if (!restore &&
    ...
    /* Set a VLAN-MAC data */
    ECORE_MEMCPY(&reg_elem->u, &elem->cmd_data.vlan_mac.u, sizeof(reg_elem->u));

    /* Copy the flags (needed for DEL and RESTORE flows) */
    reg_elem->vlan_mac_flags = elem->cmd_data.vlan_mac.vlan_mac_flags;
  } else /* DEL, RESTORE */
...
static int ecore_execute_vlan_mac(struct bnx2x_softc *sc,
  ...
  if (!drv_only) {
    ...
    ECORE_LIST_FOR_EACH_ENTRY(elem, exe_chunk, link, struct ecore_exeq_elem) {
      ...
      rc = ecore_vlan_mac_get_registry_elem(sc, cam_obj, elem, restore,
                                            &reg_elem);
      if (rc)
        goto error_exit;
      ...
      if (!restore &&
          ((cmd == ECORE_VLAN_MAC_ADD) || (cmd == ECORE_VLAN_MAC_MOVE)))
        ECORE_LIST_PUSH_HEAD(&reg_elem->link, &cam_obj->head);
    ...
    rc = ecore_sp_post(sc, o->ramrod_cmd, r->cid, r->rdata_mapping,
                       ETH_CONNECTION_TYPE);
    if (rc)
      goto error_exit;
  }
...
error_exit:
  ...
  ECORE_LIST_FOR_EACH_ENTRY(elem, exe_chunk, link, struct ecore_exeq_elem) {
    ...
    if (!restore &&
        ((cmd == ECORE_VLAN_MAC_ADD) || (cmd == ECORE_VLAN_MAC_MOVE))) {
      reg_elem = o->check_del(sc, cam_obj, &elem->cmd_data.vlan_mac.u);
      if (reg_elem) {
        ECORE_LIST_REMOVE_ENTRY(&reg_elem->link, &cam_obj->head);
        ECORE_FREE(sc, reg_elem, sizeof(*reg_elem));
      }
    }
  }
...
static int ecore_vlan_mac_push_new_cmd(struct bnx2x_softc *sc,
                                       struct ecore_vlan_mac_ramrod_params *p) {
  struct ecore_exeq_elem *elem;
  struct ecore_vlan_mac_obj *o = p->vlan_mac_obj;
...
int ecore_config_vlan_mac(struct bnx2x_softc *sc,
  ...
  if (cont || ECORE_TEST_BIT(RAMROD_EXEC, ramrod_flags) ||
    ...
    rc = __ecore_vlan_mac_execute_step(sc, p->vlan_mac_obj, &p->ramrod_flags);
    if (rc < 0)
      return rc;
  }
  ...
  if (ECORE_TEST_BIT(RAMROD_COMP_WAIT, &p->ramrod_flags)) {
    ...
    while (!ecore_exe_queue_empty(&o->exe_queue) && max_iterations--) {
...
static int ecore_vlan_mac_del_all(struct bnx2x_softc *sc,
  ...
  ECORE_LIST_FOR_EACH_ENTRY_SAFE(exeq_pos, exeq_pos_n, &exeq->exe_queue, link,
                                 struct ecore_exeq_elem) {
    if (exeq_pos->cmd_data.vlan_mac.vlan_mac_flags == *vlan_mac_flags) {
      ...
      ECORE_LIST_REMOVE_ENTRY(&exeq_pos->link, &exeq->exe_queue);
  ...
  ECORE_LIST_FOR_EACH_ENTRY(pos, &o->head, link,
    ...
    if (pos->vlan_mac_flags == *vlan_mac_flags) {
      ...
      if (rc < 0) {
        PMD_DRV_LOG(ERR, sc, "Failed to add a new DEL command");
        ecore_vlan_mac_h_read_unlock(sc, o);
        return rc;
      }
    }
...
static void ecore_init_vlan_mac_common(
    struct ecore_vlan_mac_obj *o, uint8_t cl_id, uint32_t cid, uint8_t func_id,
    void *rdata, ecore_dma_addr_t rdata_mapping, int state, uint32_t *pstate,
    ecore_obj_type type, struct ecore_credit_pool_obj *macs_pool,
  ...
  ecore_init_raw_obj(&o->raw, cl_id, cid, func_id, rdata, rdata_mapping, state,
                     pstate, type);
}

void ecore_init_mac_obj(struct bnx2x_softc *sc,
                        struct ecore_vlan_mac_obj *mac_obj, uint8_t cl_id,
                        uint32_t cid, uint8_t func_id, void *rdata,
                        ecore_dma_addr_t rdata_mapping, int state,
                        uint32_t *pstate, ecore_obj_type type,
  ...
  ecore_init_vlan_mac_common(mac_obj, cl_id, cid, func_id, rdata, rdata_mapping,
                             state, pstate, type, macs_pool, NULL);
  ...
  if (CHIP_IS_E1x(sc)) {
    ...
    /* Exe Queue */
    ecore_exe_queue_init(sc, &mac_obj->exe_queue, 1, qable_obj,
                         ecore_validate_vlan_mac, ecore_remove_vlan_mac,
                         ecore_optimize_vlan_mac, ecore_execute_vlan_mac,
                         ecore_exeq_get_mac);
  } else {
    ...
    /* Exe Queue */
    ecore_exe_queue_init(sc, &mac_obj->exe_queue, CLASSIFY_RULES_COUNT,
                         qable_obj, ecore_validate_vlan_mac,
                         ecore_remove_vlan_mac, ecore_optimize_vlan_mac,
                         ecore_execute_vlan_mac, ecore_exeq_get_mac);
  }
}

/* RX_MODE verbs: DROP_ALL/ACCEPT_ALL/ACCEPT_ALL_MULTI/ACCEPT_ALL_VLAN/NORMAL */
static void
__storm_memset_mac_filters(struct bnx2x_softc *sc,
                           struct tstorm_eth_mac_filter_config *mac_filters,
                           uint16_t pf_id) {
  size_t size = sizeof(struct tstorm_eth_mac_filter_config);

  uint32_t addr = BAR_TSTRORM_INTMEM + TSTORM_MAC_FILTER_CONFIG_OFFSET(pf_id);
...
static int ecore_set_rx_mode_e1x(struct bnx2x_softc *sc,
  ...
  ECORE_MSG(sc,
            "drop_ucast 0x%xdrop_mcast 0x%x accp_ucast 0x%x"
...
static void ecore_rx_mode_set_cmd_state_e2(uint32_t *accept_flags,
  ...
  /* start with 'drop-all' */
  state =
      ETH_FILTER_RULES_CMD_UCAST_DROP_ALL | ETH_FILTER_RULES_CMD_MCAST_DROP_ALL;
...
static int ecore_set_rx_mode_e2(struct bnx2x_softc *sc,
  ...
  if (ECORE_TEST_BIT(RAMROD_TX, &p->ramrod_flags)) {
    ...
    data->rules[rule_idx].cmd_general_data = ETH_FILTER_RULES_CMD_TX_CMD;
  ...
  if (ECORE_TEST_BIT(RAMROD_RX, &p->ramrod_flags)) {
    ...
    data->rules[rule_idx].cmd_general_data = ETH_FILTER_RULES_CMD_RX_CMD;
  ...
  ECORE_MSG(
      sc,
      "About to configure %d rules, rx_accept_flags 0x%x, tx_accept_flags 0x%x",
  ...
  /* Send a ramrod */
  rc = ecore_sp_post(sc, RAMROD_CMD_ID_ETH_FILTER_RULES, p->cid,
                     p->rdata_mapping, ETH_CONNECTION_TYPE);
  if (rc)
    return rc;
...
static int
ecore_empty_rx_mode_wait(__rte_unused struct bnx2x_softc *sc,
                         __rte_unused struct ecore_rx_mode_ramrod_params *p) {
  /* Do nothing */
  return ECORE_SUCCESS;
}
...
static int ecore_mcast_enqueue_cmd(struct bnx2x_softc *sc __rte_unused,
  ...
  int macs_list_len = ((cmd == ECORE_MCAST_CMD_ADD) ? p->mcast_list_len : 0);
  ...
  total_sz =
      sizeof(*new_cmd) + macs_list_len * sizeof(struct ecore_mcast_mac_elem);
  ...
  ECORE_MSG(sc, "About to enqueue a new %d command. macs_list_len=%d", cmd,
            macs_list_len);
  ...
  case ECORE_MCAST_CMD_ADD:
    cur_mac =
        (struct ecore_mcast_mac_elem *)((uint8_t *)new_cmd + sizeof(*new_cmd));
    ...
    ECORE_LIST_FOR_EACH_ENTRY(pos, &p->mcast_list, link,
      ...
      ECORE_LIST_PUSH_TAIL(&cur_mac->link, &new_cmd->data.macs_head);
      cur_mac++;
    }
  ...
  for (i = last / BIT_VEC64_ELEM_SZ; i < ECORE_MCAST_VEC_SZ; i++) {
    if (o->registry.aprox_match.vec[i])
      for (j = inner_start; j < BIT_VEC64_ELEM_SZ; j++) {
        ...
        if (BIT_VEC64_TEST_BIT(o->registry.aprox_match.vec, cur_bit)) {
          return cur_bit;
        }
      }
    inner_start = 0;
...
static void ecore_mcast_set_one_rule_e2(struct bnx2x_softc *sc __rte_unused,
  ...
  ECORE_MSG(sc, "%s bin %d",
            ((rx_tx_add_flag & ETH_MULTICAST_RULES_CMD_IS_ADD) ? "Setting"
                                                               : "Clearing"),
            bin);
...
static void ecore_mcast_hdl_pending_add_e2(
    struct bnx2x_softc *sc, struct ecore_mcast_obj *o,
    struct ecore_pending_mcast_cmd *cmd_pos, int *line_idx) {
  struct ecore_mcast_mac_elem *pmac_pos, *pmac_pos_n;
  int cnt = *line_idx;
  union ecore_mcast_config_data cfg_data = {NULL};

  ECORE_LIST_FOR_EACH_ENTRY_SAFE(pmac_pos, pmac_pos_n, &cmd_pos->data.macs_head,
    ...
    ECORE_MSG(sc, "About to configure " RTE_ETHER_ADDR_PRT_FMT " mcast MAC",
              pmac_pos->mac[0], pmac_pos->mac[1], pmac_pos->mac[2],
              pmac_pos->mac[3], pmac_pos->mac[4], pmac_pos->mac[5]);

    ECORE_LIST_REMOVE_ENTRY(&pmac_pos->link, &cmd_pos->data.macs_head);
...
static void ecore_mcast_hdl_pending_del_e2(
    struct bnx2x_softc *sc, struct ecore_mcast_obj *o,
    struct ecore_pending_mcast_cmd *cmd_pos, int *line_idx) {
  int cnt = *line_idx;
  ...
  while (cmd_pos->data.macs_num) {
    ...
    ECORE_MSG(sc, "Deleting MAC. %d left,cnt is %d", cmd_pos->data.macs_num,
              cnt);
...
static void ecore_mcast_hdl_pending_restore_e2(
    struct bnx2x_softc *sc, struct ecore_mcast_obj *o,
    struct ecore_pending_mcast_cmd *cmd_pos, int *line_idx) {
  cmd_pos->data.next_bin =
      o->hdl_restore(sc, o, cmd_pos->data.next_bin, line_idx);
...
static int
ecore_mcast_handle_pending_cmds_e2(struct bnx2x_softc *sc,
                                   struct ecore_mcast_ramrod_params *p) {
  ...
  ECORE_LIST_FOR_EACH_ENTRY_SAFE(cmd_pos, cmd_pos_n, &o->pending_cmds_head,
    ...
    case ECORE_MCAST_CMD_RESTORE:
      ecore_mcast_hdl_pending_restore_e2(sc, o, cmd_pos, &cnt);
      break;
...
static void ecore_mcast_hdl_del(struct bnx2x_softc *sc,
  ...
  for (i = 0; i < p->mcast_list_len; i++) {
    ...
    ECORE_MSG(sc, "Deleting MAC. %d left", p->mcast_list_len - i - 1);
  }
 ...
 */
static int ecore_mcast_handle_current_cmd(struct bnx2x_softc *sc,
                                          struct ecore_mcast_ramrod_params *p,
                                          enum ecore_mcast_cmd cmd,
                                          int start_cnt) {
  struct ecore_mcast_obj *o = p->mcast_obj;
...
static void ecore_mcast_revert_e2(__rte_unused struct bnx2x_softc *sc,
                                  struct ecore_mcast_ramrod_params *p,
                                  int old_num_bins, enum ecore_mcast_cmd cmd) {
  struct ecore_mcast_obj *o = p->mcast_obj;
 ...
 */
static void ecore_mcast_set_rdata_hdr_e2(__rte_unused struct bnx2x_softc *sc,
                                         struct ecore_mcast_ramrod_params *p,
                                         uint8_t len) {
  ...
  data->header.echo =
      ECORE_CPU_TO_LE32((r->cid & ECORE_SWCID_MASK) |
...
static int ecore_mcast_setup_e2(struct bnx2x_softc *sc,
  ...
  if (ECORE_TEST_BIT(RAMROD_DRV_CLR_ONLY, &p->ramrod_flags)) {
    ...
    /* Send a ramrod */
    rc = ecore_sp_post(sc, RAMROD_CMD_ID_ETH_MULTICAST_RULES, raw->cid,
...
static void
ecore_mcast_revert_e1h(__rte_unused struct bnx2x_softc *sc,
                       __rte_unused struct ecore_mcast_ramrod_params *p,
                       __rte_unused int old_num_bins,
...
static void ecore_mcast_hdl_add_e1h(struct bnx2x_softc *sc __rte_unused,
  ...
  ECORE_LIST_FOR_EACH_ENTRY(mlist_pos, &p->mcast_list, link,
    ...
    ECORE_MSG(sc,
              "About to configure " RTE_ETHER_ADDR_PRT_FMT " mcast MAC, bin %d",
              mlist_pos->mac[0], mlist_pos->mac[1], mlist_pos->mac[2],
              mlist_pos->mac[3], mlist_pos->mac[4], mlist_pos->mac[5], bit);
...
static void ecore_mcast_hdl_restore_e1h(struct bnx2x_softc *sc __rte_unused,
                                        struct ecore_mcast_obj *o,
                                        uint32_t *mc_filter) {
  int bit;

  for (bit = ecore_mcast_get_next_bin(o, 0); bit >= 0;
...
void ecore_init_mcast_obj(struct bnx2x_softc *sc,
  ...
  ecore_init_raw_obj(&mcast_obj->raw, mcast_cl_id, mcast_cid, func_id, rdata,
  ...
  if (CHIP_IS_E1H(sc)) {
    ...
    mcast_obj->get_registry_size = ecore_mcast_get_registry_size_aprox;
    mcast_obj->set_registry_size = ecore_mcast_set_registry_size_aprox;
  } else {
...
static bool
ecore_credit_pool_always_TRUE(__rte_unused struct ecore_credit_pool_obj *o,
...
static bool ecore_credit_pool_get_entry(struct ecore_credit_pool_obj *o,
  ...
  *offset = -1;
  ...
  for (vec = 0; vec < ECORE_POOL_VEC_SIZE; vec++) {
    ...
    for (idx = vec * BIT_VEC64_ELEM_SZ, i = 0; i < BIT_VEC64_ELEM_SZ;
         idx++, i++)
...
static bool ecore_credit_pool_put_entry_always_TRUE(
    __rte_unused struct ecore_credit_pool_obj *o, __rte_unused int offset) {
  return TRUE;
}
...
static int ecore_setup_rss(struct bnx2x_softc *sc,
  ...
  if (ECORE_TEST_BIT(ECORE_RSS_IPV4, &p->rss_flags))
    data->capabilities |= ETH_RSS_UPDATE_RAMROD_DATA_IPV4_CAPABILITY;

  if (ECORE_TEST_BIT(ECORE_RSS_IPV4_TCP, &p->rss_flags))
    data->capabilities |= ETH_RSS_UPDATE_RAMROD_DATA_IPV4_TCP_CAPABILITY;

  if (ECORE_TEST_BIT(ECORE_RSS_IPV4_UDP, &p->rss_flags))
    data->capabilities |= ETH_RSS_UPDATE_RAMROD_DATA_IPV4_UDP_CAPABILITY;

  if (ECORE_TEST_BIT(ECORE_RSS_IPV6, &p->rss_flags))
    data->capabilities |= ETH_RSS_UPDATE_RAMROD_DATA_IPV6_CAPABILITY;

  if (ECORE_TEST_BIT(ECORE_RSS_IPV6_TCP, &p->rss_flags))
    data->capabilities |= ETH_RSS_UPDATE_RAMROD_DATA_IPV6_TCP_CAPABILITY;

  if (ECORE_TEST_BIT(ECORE_RSS_IPV6_UDP, &p->rss_flags))
    data->capabilities |= ETH_RSS_UPDATE_RAMROD_DATA_IPV6_UDP_CAPABILITY;
  ...
  if (ECORE_TEST_BIT(ECORE_RSS_SET_SRCH, &p->rss_flags)) {
    ECORE_MEMCPY(&data->rss_key[0], &p->rss_key[0], sizeof(data->rss_key));
  ...
  /* Send a ramrod */
  rc = ecore_sp_post(sc, RAMROD_CMD_ID_ETH_RSS_UPDATE, r->cid, r->rdata_mapping,
                     ETH_CONNECTION_TYPE);
...
void ecore_init_rss_config_obj(struct bnx2x_softc *sc __rte_unused,
                               ...
                               ecore_dma_addr_t rdata_mapping, int state,
                               uint32_t *pstate, ecore_obj_type type) {
  ecore_init_raw_obj(&rss_obj->raw, cl_id, cid, func_id, rdata, rdata_mapping,
                     state, pstate, type);
...
int ecore_queue_state_change(struct bnx2x_softc *sc,
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, sc, "check transition returned an error. rc %d", rc);
    return ECORE_INVAL;
  }
...
static int ecore_queue_comp_cmd(struct bnx2x_softc *sc __rte_unused,
  ...
  if (!ECORE_TEST_AND_CLEAR_BIT(cmd, &cur_pending)) {
    PMD_DRV_LOG(
        ...
        cmd, o->cids[ECORE_PRIMARY_CID_INDEX], o->state, cur_pending,
        o->next_state);
    return ECORE_INVAL;
  }
  ...
  if (o->next_tx_only >= o->max_cos)
     ...
     */
    PMD_DRV_LOG(ERR, sc, "illegal value for next tx_only: %d. max cos was %d",
                o->next_tx_only, o->max_cos);

  ECORE_MSG(sc, "Completing command %d for queue %d, setting state to %d", cmd,
            o->cids[ECORE_PRIMARY_CID_INDEX], o->next_state);
...
static void
ecore_q_fill_setup_data_e2(struct ecore_queue_state_params *cmd_params,
  ...
  data->rx.tpa_en |= ECORE_TEST_BIT(ECORE_Q_FLG_TPA_IPV6, &params->flags) *
                     CLIENT_INIT_RX_DATA_TPA_EN_IPV6;
}

static void ecore_q_fill_init_general_data(
    struct bnx2x_softc *sc __rte_unused, struct ecore_queue_sp_obj *o,
    struct ecore_general_setup_params *params,
    struct client_init_general_data *gen_data, uint32_t *flags) {
  ...
  gen_data->traffic_type = ECORE_TEST_BIT(ECORE_Q_FLG_FCOE, flags)
                               ? LLFC_TRAFFIC_TYPE_FCOE
                               : LLFC_TRAFFIC_TYPE_NW;

  ECORE_MSG(sc, "flags: active %d, cos %d, stats en %d", gen_data->activate_flg,
            gen_data->cos, gen_data->statistics_en_flg);
}

static void ecore_q_fill_init_tx_data(struct ecore_txq_setup_params *params,
                                      struct client_init_tx_data *tx_data,
                                      uint32_t *flags) {
  tx_data->enforce_security_flg = ECORE_TEST_BIT(ECORE_Q_FLG_TX_SEC, flags);
  ...
  tx_data->tx_switching_flg = ECORE_TEST_BIT(ECORE_Q_FLG_TX_SWITCH, flags);
  tx_data->anti_spoofing_flg = ECORE_TEST_BIT(ECORE_Q_FLG_ANTI_SPOOF, flags);
  ...
  tx_data->tx_bd_page_base.lo = ECORE_CPU_TO_LE32(U64_LO(params->dscr_map));
  tx_data->tx_bd_page_base.hi = ECORE_CPU_TO_LE32(U64_HI(params->dscr_map));
...
static void ecore_q_fill_init_rx_data(struct ecore_rxq_setup_params *params,
                                      ...
                                      uint32_t *flags) {
  rx_data->tpa_en =
      ECORE_TEST_BIT(ECORE_Q_FLG_TPA, flags) * CLIENT_INIT_RX_DATA_TPA_EN_IPV4;
  rx_data->tpa_en |=
      ECORE_TEST_BIT(ECORE_Q_FLG_TPA_GRO, flags) * CLIENT_INIT_RX_DATA_TPA_MODE;
  rx_data->vmqueue_mode_en_flg = 0;

  rx_data->extra_data_over_sgl_en_flg = ECORE_TEST_BIT(ECORE_Q_FLG_OOO, flags);
  ...
  rx_data->is_leading_rss = ECORE_TEST_BIT(ECORE_Q_FLG_LEADING_RSS, flags);
  ...
  rx_data->silent_vlan_value = ECORE_CPU_TO_LE16(params->silent_removal_value);
  rx_data->silent_vlan_mask = ECORE_CPU_TO_LE16(params->silent_removal_mask);
}

/* initialize the general, tx and rx parts of a queue object */
static void
ecore_q_fill_setup_data_cmn(struct bnx2x_softc *sc,
                            struct ecore_queue_state_params *cmd_params,
                            struct client_init_ramrod_data *data) {
  ecore_q_fill_init_general_data(
      sc, cmd_params->q_obj, &cmd_params->params.setup.gen_params,
      &data->general, &cmd_params->params.setup.flags);

  ecore_q_fill_init_tx_data(&cmd_params->params.setup.txq_params, &data->tx,
                            &cmd_params->params.setup.flags);

  ecore_q_fill_init_rx_data(&cmd_params->params.setup.rxq_params, &data->rx,
                            &cmd_params->params.setup.flags);
...
static void
ecore_q_fill_setup_tx_only(struct bnx2x_softc *sc,
                           struct ecore_queue_state_params *cmd_params,
                           struct tx_queue_init_ramrod_data *data) {
  ecore_q_fill_init_general_data(
      sc, cmd_params->q_obj, &cmd_params->params.tx_only.gen_params,
      &data->general, &cmd_params->params.tx_only.flags);

  ecore_q_fill_init_tx_data(&cmd_params->params.tx_only.txq_params, &data->tx,
                            &cmd_params->params.tx_only.flags);

  ECORE_MSG(sc, "cid %d, tx bd page lo %x hi %x", cmd_params->q_obj->cids[0],
...
static int ecore_q_init(struct bnx2x_softc *sc,
  ...
  if (ECORE_TEST_BIT(ECORE_Q_TYPE_HAS_TX, &o->type) &&
    ...
    ECORE_UPDATE_COALESCE_SB_INDEX(
        ...
        !ECORE_TEST_BIT(ECORE_Q_FLG_HC_EN, &init->tx.flags), hc_usec);
  }
  ...
  if (ECORE_TEST_BIT(ECORE_Q_TYPE_HAS_RX, &o->type) &&
    ...
    ECORE_UPDATE_COALESCE_SB_INDEX(
        ...
        !ECORE_TEST_BIT(ECORE_Q_FLG_HC_EN, &init->rx.flags), hc_usec);
  }

  /* Set CDU context validation values */
  for (cos = 0; cos < o->max_cos; cos++) {
    ECORE_MSG(sc, "setting context validation. cid %d, cos %d", o->cids[cos],
              cos);
...
static int ecore_q_send_setup_e1x(struct bnx2x_softc *sc,
                                  struct ecore_queue_state_params *params) {
  struct ecore_queue_sp_obj *o = params->q_obj;
  ...
  return ecore_sp_post(sc, ramrod, o->cids[ECORE_PRIMARY_CID_INDEX],
...
static int ecore_q_send_setup_tx_only(struct bnx2x_softc *sc,
                                      struct ecore_queue_state_params *params) {
  struct ecore_queue_sp_obj *o = params->q_obj;
  ...
  if (cid_index >= o->max_cos) {
    PMD_DRV_LOG(ERR, sc, "queue[%d]: cid_index (%d) is out of range", o->cl_id,
                cid_index);
    return ECORE_INVAL;
  }

  ECORE_MSG(sc, "parameters received: cos: %d sp-id: %d",
            tx_only_params->gen_params.cos, tx_only_params->gen_params.spcl_id);
  ...
  ECORE_MSG(
      ...
      o->cids[cid_index], rdata->general.client_id, rdata->general.sp_client_id,
      rdata->general.cos);
  ...
  return ecore_sp_post(sc, ramrod, o->cids[cid_index], data_mapping,
                       ETH_CONNECTION_TYPE);
}
...
static void ecore_q_fill_update_data(struct ecore_queue_sp_obj *obj,
  ...
  data->inner_vlan_removal_change_flg =
      ECORE_TEST_BIT(ECORE_Q_UPDATE_IN_VLAN_REM_CHNG, &params->update_flags);

  /* Outer VLAN stripping */
  data->outer_vlan_removal_enable_flg =
      ECORE_TEST_BIT(ECORE_Q_UPDATE_OUT_VLAN_REM, &params->update_flags);
  data->outer_vlan_removal_change_flg =
      ECORE_TEST_BIT(ECORE_Q_UPDATE_OUT_VLAN_REM_CHNG, &params->update_flags);
  ...
  data->anti_spoofing_change_flg =
      ECORE_TEST_BIT(ECORE_Q_UPDATE_ANTI_SPOOF_CHNG, &params->update_flags);
  ...
  data->default_vlan_change_flg =
      ECORE_TEST_BIT(ECORE_Q_UPDATE_DEF_VLAN_EN_CHNG, &params->update_flags);

  /* silent vlan removal */
  data->silent_vlan_change_flg = ECORE_TEST_BIT(
      ECORE_Q_UPDATE_SILENT_VLAN_REM_CHNG, &params->update_flags);
  data->silent_vlan_removal_flg =
      ECORE_TEST_BIT(ECORE_Q_UPDATE_SILENT_VLAN_REM, &params->update_flags);
  ...
  data->tx_switching_change_flg =
      ECORE_TEST_BIT(ECORE_Q_UPDATE_TX_SWITCHING_CHNG, &params->update_flags);
}
...
static int ecore_q_send_update(struct bnx2x_softc *sc,
  ...
  ecore_dma_addr_t data_mapping = o->rdata_mapping;
  struct ecore_queue_update_params *update_params = &params->params.update;
  ...
  return ecore_sp_post(sc, RAMROD_CMD_ID_ETH_CLIENT_UPDATE, o->cids[cid_index],
                       data_mapping, ETH_CONNECTION_TYPE);
}
 ...
 */
static int ecore_q_send_deactivate(struct bnx2x_softc *sc,
                                   struct ecore_queue_state_params *params) {
...
static int
ecore_q_send_update_tpa(__rte_unused struct bnx2x_softc *sc,
                        __rte_unused struct ecore_queue_state_params *params) {
  /* Not implemented yet. */
  return -1;
}
...
static int ecore_q_send_halt(struct bnx2x_softc *sc,
  ...
  return ecore_sp_post(sc, RAMROD_CMD_ID_ETH_HALT,
                       o->cids[ECORE_PRIMARY_CID_INDEX], data_mapping,
                       ETH_CONNECTION_TYPE);
}
...
static int ecore_q_send_cfc_del(struct bnx2x_softc *sc,
  ...
  return ecore_sp_post(sc, RAMROD_CMD_ID_COMMON_CFC_DEL, o->cids[cid_idx], 0,
                       NONE_CONNECTION_TYPE);
}

static int ecore_q_send_terminate(struct bnx2x_softc *sc,
                                  struct ecore_queue_state_params *params) {
  struct ecore_queue_sp_obj *o = params->q_obj;
  ...
  return ecore_sp_post(sc, RAMROD_CMD_ID_ETH_TERMINATE, o->cids[cid_index], 0,
                       ETH_CONNECTION_TYPE);
}
...
static int ecore_queue_send_cmd_cmn(struct bnx2x_softc *sc,
                                    struct ecore_queue_state_params *params) {
  switch (params->cmd) {
...
static int ecore_queue_chk_transition(struct bnx2x_softc *sc __rte_unused,
  ...
  case ECORE_Q_STATE_INITIALIZED:
    if (cmd == ECORE_Q_CMD_SETUP) {
      if (ECORE_TEST_BIT(ECORE_Q_FLG_ACTIVE, &params->params.setup.flags))
  ...
  case ECORE_Q_STATE_ACTIVE:
    ...
    else if ((cmd == ECORE_Q_CMD_EMPTY) || (cmd == ECORE_Q_CMD_UPDATE_TPA))
      next_state = ECORE_Q_STATE_ACTIVE;
  ...
  if (next_state != ECORE_Q_STATE_MAX) {
    ECORE_MSG(sc, "Good state transition: %d(%d)->%d", state, cmd, next_state);
 ...
 */
static int
ecore_queue_chk_fwd_transition(struct bnx2x_softc *sc __rte_unused,
                               struct ecore_queue_sp_obj *o,
                               struct ecore_queue_state_params *params) {
  enum ecore_q_state state = o->state, next_state = ECORE_Q_STATE_MAX;
  enum ecore_queue_cmd cmd = params->cmd;
  ...
  case ECORE_Q_STATE_INITIALIZED:
    if (cmd == ECORE_Q_CMD_SETUP_TX_ONLY) {
      if (ECORE_TEST_BIT(ECORE_Q_FLG_ACTIVE, &params->params.tx_only.flags))
        next_state = ECORE_Q_STATE_ACTIVE;
      else
        next_state = ECORE_Q_STATE_INACTIVE;
    }
...
void ecore_init_queue_obj(struct bnx2x_softc *sc,
                          struct ecore_queue_sp_obj *obj, uint8_t cl_id,
                          uint32_t *cids, uint8_t cid_cnt, uint8_t func_id,
 ...
 */
static int ecore_func_state_change_comp(struct bnx2x_softc *sc __rte_unused,
                                        struct ecore_func_sp_obj *o,
                                        enum ecore_func_cmd cmd) {
  uint32_t cur_pending = o->pending;
  ...
  if (!ECORE_TEST_AND_CLEAR_BIT(cmd, &cur_pending)) {
    PMD_DRV_LOG(
        ...
        cmd, ECORE_FUNC_ID(sc), o->state, cur_pending, o->next_state);
    return ECORE_INVAL;
  }

  ECORE_MSG(sc, "Completing command %d for func %d, setting state to %d", cmd,
            ECORE_FUNC_ID(sc), o->next_state);
...
static int ecore_func_chk_transition(struct bnx2x_softc *sc __rte_unused,
  ...
  if (next_state != ECORE_F_STATE_MAX) {
    ECORE_MSG(sc, "Good function state transition: %d(%d)->%d", state, cmd,
              next_state);
    o->next_state = next_state;
    return ECORE_SUCCESS;
  }

  ECORE_MSG(sc, "Bad function state transition request: %d %d", state, cmd);
 ...
 */
static int ecore_func_init_cmn_chip(struct bnx2x_softc *sc,
                                    const struct ecore_func_sp_drv_ops *drv) {
...
static int ecore_func_hw_init(struct bnx2x_softc *sc,
  ...
  ECORE_MSG(sc, "function %d  load_code %x", ECORE_ABS_FUNC_ID(sc), load_code);
  ...
  case FW_MSG_CODE_DRV_LOAD_FUNCTION:
    ...
    break;
  default:
    PMD_DRV_LOG(ERR, sc, "Unknown load_code (0x%x) from MCP", load_code);
    rc = ECORE_INVAL;
  }
 ...
 * FUNCTION-only HW blocks.
 */
static void ecore_func_reset_func(struct bnx2x_softc *sc,
                                  const struct ecore_func_sp_drv_ops *drv) {
 ...
 */
static void ecore_func_reset_port(struct bnx2x_softc *sc,
                                  const struct ecore_func_sp_drv_ops *drv) {
...
static int ecore_func_hw_reset(struct bnx2x_softc *sc,
  ...
  case FW_MSG_CODE_DRV_UNLOAD_FUNCTION:
    ...
    break;
  default:
    PMD_DRV_LOG(ERR, sc, "Unknown reset_phase (0x%x) from MCP", reset_phase);
    break;
  }
...
static int ecore_func_send_start(struct bnx2x_softc *sc,
                                 struct ecore_func_state_params *params) {
  struct ecore_func_sp_obj *o = params->f_obj;
  struct function_start_data *rdata = (struct function_start_data *)o->rdata;
  ecore_dma_addr_t data_mapping = o->rdata_mapping;
  ...
  return ecore_sp_post(sc, RAMROD_CMD_ID_COMMON_FUNCTION_START, 0, data_mapping,
                       NONE_CONNECTION_TYPE);
}

static int
ecore_func_send_switch_update(struct bnx2x_softc *sc,
                              struct ecore_func_state_params *params) {
  struct ecore_func_sp_obj *o = params->f_obj;
  struct function_update_data *rdata = (struct function_update_data *)o->rdata;
  ecore_dma_addr_t data_mapping = o->rdata_mapping;
  ...
  if (ECORE_TEST_BIT(ECORE_F_UPDATE_TX_SWITCH_SUSPEND_CHNG,
    ...
    rdata->tx_switch_suspend = ECORE_TEST_BIT(ECORE_F_UPDATE_TX_SWITCH_SUSPEND,
                                              &switch_update_params->changes);
  }
...
static int ecore_func_send_afex_update(struct bnx2x_softc *sc,
                                       struct ecore_func_state_params *params) {
  struct ecore_func_sp_obj *o = params->f_obj;
  ...
  ECORE_MSG(sc, "afex: sending func_update vif_id 0x%x dvlan 0x%x prio 0x%x",
            rdata->vif_id, rdata->afex_default_vlan, rdata->allowed_priorities);
...
static inline int
ecore_func_send_afex_viflists(struct bnx2x_softc *sc,
                              struct ecore_func_state_params *params) {
  struct ecore_func_sp_obj *o = params->f_obj;
  ...
  ECORE_MSG(sc,
            "afex: ramrod lists, cmd 0x%x index 0x%x func_bit_map 0x%x "
            "func_to_clr 0x%x",
            rdata->afex_vif_list_command, rdata->vif_list_index,
            rdata->func_bit_map, rdata->func_to_clear);

  /* this ramrod sends data directly and not through DMA mapping */
  return ecore_sp_post(sc, RAMROD_CMD_ID_COMMON_AFEX_VIF_LISTS, 0, *p_rdata,
                       NONE_CONNECTION_TYPE);
}

static int
ecore_func_send_stop(struct bnx2x_softc *sc,
                     __rte_unused struct ecore_func_state_params *params) {
  return ecore_sp_post(sc, RAMROD_CMD_ID_COMMON_FUNCTION_STOP, 0, 0,
                       NONE_CONNECTION_TYPE);
}

static int
ecore_func_send_tx_stop(struct bnx2x_softc *sc,
                        __rte_unused struct ecore_func_state_params *params) {
  return ecore_sp_post(sc, RAMROD_CMD_ID_COMMON_STOP_TRAFFIC, 0, 0,
                       NONE_CONNECTION_TYPE);
}

static int ecore_func_send_tx_start(struct bnx2x_softc *sc,
                                    struct ecore_func_state_params *params) {
  struct ecore_func_sp_obj *o = params->f_obj;
  struct flow_control_configuration *rdata =
      (struct flow_control_configuration *)o->rdata;
  ecore_dma_addr_t data_mapping = o->rdata_mapping;
  struct ecore_func_tx_start_params *tx_start_params = &params->params.tx_start;
  uint32_t i;
  ...
  return ecore_sp_post(sc, RAMROD_CMD_ID_COMMON_START_TRAFFIC, 0, data_mapping,
                       NONE_CONNECTION_TYPE);
}
...
void ecore_init_func_obj(__rte_unused struct bnx2x_softc *sc,
                         struct ecore_func_sp_obj *obj, void *rdata,
                         ecore_dma_addr_t rdata_mapping, void *afex_rdata,
...
int ecore_func_state_change(struct bnx2x_softc *sc,
  ...
  if ((rc == ECORE_BUSY) &&
    ...
    if (rc == ECORE_BUSY) {
      ECORE_MUTEX_UNLOCK(&o->one_pending_mutex);
      PMD_DRV_LOG(ERR, sc, "timeout waiting for previous ramrod completion");
      return rc;
    }
  } else if (rc) {
    ECORE_MUTEX_UNLOCK(&o->one_pending_mutex);
...
uint32_t ecore_calc_crc32(uint32_t crc, uint8_t const *p, uint32_t len,
                          uint32_t magic) {
  int i;
  while (len--) {
    crc ^= *p++;
