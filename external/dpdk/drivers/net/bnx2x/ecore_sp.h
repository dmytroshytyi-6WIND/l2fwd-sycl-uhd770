...
#define ECORE_SPIN_LOCK_BH(_spin)                                              \
  rte_spinlock_lock(_spin) /* bh = bottom-half */
#define ECORE_SPIN_UNLOCK_BH(_spin)                                            \
  rte_spinlock_unlock(_spin) /* bh = bottom-half */
...
#define ECORE_FREE(_s, _buf, _size) rte_free(_buf)
...
#define ECORE_ILT_ZALLOC(x, y, size)                                           \
  do {                                                                         \
    ...
    if (x) {                                                                   \
      if (bnx2x_dma_alloc((struct bnx2x_softc *)sc, size,                      \
                          (struct bnx2x_dma *)x, "ILT",                        \
...
static inline uint32_t ECORE_CRC32_LE(uint32_t seed, uint8_t *mac,
                                      uint32_t len) {
...
#define ECORE_MSG(sc, m, ...) PMD_DRV_LOG(DEBUG, sc, m, ##__VA_ARGS__)
...
#define ECORE_LIST_FOR_EACH_ENTRY_SAFE(pos, n, _list, _link, cast)             \
  for (pos = ECORE_LIST_FIRST_ENTRY(_list, cast, _lint),                       \
      n = (pos) ? ECORE_LIST_NEXT(pos, _link, cast) : NULL;                    \
       pos != NULL;                                                            \
       pos = (cast *)n, n = (pos) ? ECORE_LIST_NEXT(pos, _link, cast) : NULL)
...
#define BIT_VEC64_TEST_BIT(vec64, idx)                                         \
  (((vec64)[(idx) >> BIT_VEC64_ELEM_SHIFT] >> ((idx)&BIT_VEC64_ELEM_MASK)) &   \
   0x1)
...
static inline void ecore_set_fw_mac_addr(uint16_t *fw_hi, uint16_t *fw_mid,
...
struct ecore_raw_obj {
  ...
  int (*wait_comp)(struct bnx2x_softc *sc, struct ecore_raw_obj *o);
...
typedef int (*exe_q_validate)(struct bnx2x_softc *sc, union ecore_qable_obj *o,
                              struct ecore_exeq_elem *elem);

typedef int (*exe_q_remove)(struct bnx2x_softc *sc, union ecore_qable_obj *o,
                            struct ecore_exeq_elem *elem);

/* Return positive if entry was optimized, 0 - if not, negative
 * in case of an error.
 */
typedef int (*exe_q_optimize)(struct bnx2x_softc *sc, union ecore_qable_obj *o,
                              struct ecore_exeq_elem *elem);
typedef int (*exe_q_execute)(struct bnx2x_softc *sc, union ecore_qable_obj *o,
                             ecore_list_t *exe_chunk, uint32_t *ramrod_flags);
typedef struct ecore_exeq_elem *(*exe_q_get)(struct ecore_exe_queue_obj *o,
                                             struct ecore_exeq_elem *elem);
...
#define ECORE_VLAN_MAC_CMP_MASK                                                \
  (1 << ECORE_UC_LIST_MAC | 1 << ECORE_ETH_MAC | 1 << ECORE_ISCSI_ETH_MAC |    \
   1 << ECORE_NETQ_ETH_MAC | 1 << ECORE_VLAN)
...
struct ecore_vlan_mac_obj {
  ...
  int (*get_n_elements)(struct bnx2x_softc *sc, struct ecore_vlan_mac_obj *o,
  ...
  int (*check_add)(struct bnx2x_softc *sc, struct ecore_vlan_mac_obj *o,
   ...
   */
  struct ecore_vlan_mac_registry_elem *(*check_del)(
      struct bnx2x_softc *sc, struct ecore_vlan_mac_obj *o,
      union ecore_classification_ramrod_data *data);
   ...
   * @return true if the element may be deleted
   */
  bool (*check_move)(struct bnx2x_softc *sc, struct ecore_vlan_mac_obj *src_o,
   ...
   */
  void (*set_one_rule)(struct bnx2x_softc *sc, struct ecore_vlan_mac_obj *o,
   ...
   */
  int (*delete_all)(struct bnx2x_softc *sc, struct ecore_vlan_mac_obj *o,
                    uint32_t *vlan_mac_flags, uint32_t *ramrod_flags);
   ...
   * @return int
   */
  int (*restore)(struct bnx2x_softc *sc, struct ecore_vlan_mac_ramrod_params *p,
  ...
  int (*complete)(struct bnx2x_softc *sc, struct ecore_vlan_mac_obj *o,
                  union event_ring_elem *cqe, uint32_t *ramrod_flags);
...
#define ECORE_MCAST_VEC_SZ (ECORE_MCAST_BINS_NUM / 64)
  ...
  void (*set_one_rule)(struct bnx2x_softc *sc, struct ecore_mcast_obj *o,
   ...
   * feasible.
   */
  int (*validate)(struct bnx2x_softc *sc, struct ecore_mcast_ramrod_params *p,
                  enum ecore_mcast_cmd cmd);

  /**
   * Restore the values of internal counters in case of a failure.
   */
  void (*revert)(struct bnx2x_softc *sc, struct ecore_mcast_ramrod_params *p,
...
struct ecore_rss_config_obj {
  ...
  int (*config_rss)(struct bnx2x_softc *sc, struct ecore_config_rss_params *p);
};
...
struct ecore_queue_sp_obj {
   ...
   */
  int (*check_transition)(struct bnx2x_softc *sc, struct ecore_queue_sp_obj *o,
                          struct ecore_queue_state_params *params);

  /**
   * Completes the pending command.
   */
  int (*complete_cmd)(struct bnx2x_softc *sc, struct ecore_queue_sp_obj *o,
                      enum ecore_queue_cmd);

  int (*wait_comp)(struct bnx2x_softc *sc, struct ecore_queue_sp_obj *o,
                   enum ecore_queue_cmd cmd);
};
...
struct ecore_func_sp_obj {
   ...
   * Checks that the requested state transition is legal.
   */
  int (*check_transition)(struct bnx2x_softc *sc, struct ecore_func_sp_obj *o,
                          struct ecore_func_state_params *params);

  /**
   * Completes the pending command.
   */
  int (*complete_cmd)(struct bnx2x_softc *sc, struct ecore_func_sp_obj *o,
                      enum ecore_func_cmd cmd);
...
/************** Function state update *********/
void ecore_init_func_obj(struct bnx2x_softc *sc, struct ecore_func_sp_obj *obj,
...
void ecore_init_vlan_obj(struct bnx2x_softc *sc,
                         struct ecore_vlan_mac_obj *vlan_obj, uint8_t cl_id,
                         uint32_t cid, uint8_t func_id, void *rdata,
                         ecore_dma_addr_t rdata_mapping, int state,
                         uint32_t *pstate, ecore_obj_type type,
                         struct ecore_credit_pool_obj *vlans_pool);
...
void ecore_init_vlan_mac_obj(struct bnx2x_softc *sc,
                             ...
                             void *rdata, ecore_dma_addr_t rdata_mapping,
                             int state, uint32_t *pstate, ecore_obj_type type,
                             struct ecore_credit_pool_obj *macs_pool,
                             struct ecore_credit_pool_obj *vlans_pool);
