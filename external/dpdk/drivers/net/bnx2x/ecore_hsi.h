...
#define SHMEM_ARRAY_SET(a, i, eb, fb, val)                                     \
  do {                                                                         \
    a[SHMEM_ARRAY_ENTRY(i, eb)] &=                                             \
        ~(SHMEM_ARRAY_MASK(eb) << SHMEM_ARRAY_BITPOS(i, eb, fb));              \
    a[SHMEM_ARRAY_ENTRY(i, eb)] |=                                             \
        (((val)&SHMEM_ARRAY_MASK(eb)) << SHMEM_ARRAY_BITPOS(i, eb, fb));       \
  } while (0)
...
#define DRV_FLAGS_GET_PORT_OFFSET(_port)                                       \
  ((0 == _port) ? DRV_FLAGS_P0_OFFSET : DRV_FLAGS_P1_OFFSET)

#define DRV_FLAGS_GET_PORT_MASK(_port)                                         \
  (DRV_FLAGS_PORT_MASK << DRV_FLAGS_GET_PORT_OFFSET(_port))

#define DRV_FLAGS_FILED_BY_PORT(_field_bit, _port)                             \
  (1 << ((_field_bit) + DRV_FLAGS_GET_PORT_OFFSET(_port)))
...
#define PF_ALLOACTION_MSIX_VECTORS_MASK                                        \
  0x000000ff /* real value, as PCI config space can show only maximum of 64    \
                vectors */
...
/*
 * union for mac addresses and for tunneling data. considered as tunneling data
 * only if (tunnel_exist == 1).
 */
...
/*
 * In case of LSO over IPv4 tunnel, whether to increment IP ID on external IP
 * header or internal IP header
 */
...
/*
 * In case tunnel exist and L4 checksum offload (or outer ip header checksum),
