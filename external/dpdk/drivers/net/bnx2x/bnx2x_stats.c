...
static inline uint16_t bnx2x_get_port_stats_dma_len(struct bnx2x_softc *sc) {
  uint16_t res = 0;
  uint32_t size;
 ...
 * sent in order).
 */
static void bnx2x_storm_stats_post(struct bnx2x_softc *sc) {
  int rc;
  ...
  if (!sc->stats_pending) {
    ...
    sc->fw_stats_req->hdr.drv_stats_counter = htole16(sc->stats_counter++);

    PMD_DEBUG_PERIODIC_LOG(DEBUG, sc, "sending statistics ramrod %d",
    ...
    rc = bnx2x_sp_post(sc, RAMROD_CMD_ID_COMMON_STAT_QUERY, 0,
                       ...
                       U64_LO(sc->fw_stats_req_mapping), NONE_CONNECTION_TYPE);
    if (rc == 0) {
      sc->stats_pending = 1;
    }
  }
...
static void bnx2x_hw_stats_post(struct bnx2x_softc *sc) {
  ...
  *stats_comp = DMAE_COMP_VAL;
  ...
  if (sc->executer_idx) {
    ...
    opcode =
        bnx2x_dmae_opcode(sc, DMAE_SRC_PCI, DMAE_DST_GRC, TRUE, DMAE_COMP_GRC);
    ...
    dmae->dst_addr_lo =
        ((DMAE_REG_CMD_MEM + sizeof(struct dmae_command) * (loader_idx + 1)) >>
         2);
    dmae->dst_addr_hi = 0;
...
static int bnx2x_stats_comp(struct bnx2x_softc *sc) {
  uint32_t *stats_comp = BNX2X_SP(sc, stats_comp);
  int cnt = 10;
...
static void bnx2x_stats_pmf_update(struct bnx2x_softc *sc) {
  ...
  dmae->src_addr_hi = 0;
  dmae->dst_addr_lo =
      U64_LO(BNX2X_SP_MAPPING(sc, port_stats) + DMAE_LEN32_RD_MAX * 4);
  dmae->dst_addr_hi =
      U64_HI(BNX2X_SP_MAPPING(sc, port_stats) + DMAE_LEN32_RD_MAX * 4);
...
static void bnx2x_port_stats_init(struct bnx2x_softc *sc) {
  struct dmae_command *dmae;
  int port = SC_PORT(sc);
  uint32_t opcode;
  int loader_idx = PMF_DMAE_C(sc);
  ...
  /* MAC */
  opcode =
      bnx2x_dmae_opcode(sc, DMAE_SRC_GRC, DMAE_DST_PCI, TRUE, DMAE_COMP_GRC);
  ...
  if (sc->link_vars.mac_type == ELINK_MAC_TYPE_EMAC) {
    ...
    dmae->src_addr_hi = 0;
    dmae->dst_addr_lo =
        U64_LO(BNX2X_SP_MAPPING(sc, mac_stats) +
               offsetof(struct emac_stats, rx_stat_falsecarriererrors));
    dmae->dst_addr_hi =
        U64_HI(BNX2X_SP_MAPPING(sc, mac_stats) +
               offsetof(struct emac_stats, rx_stat_falsecarriererrors));
    dmae->len = 1;
    dmae->comp_addr_lo = (dmae_reg_go_c[loader_idx] >> 2);
    dmae->comp_addr_hi = 0;
    ...
    case ELINK_MAC_TYPE_BMAC:
      mac_addr = (port) ? NIG_REG_INGRESS_BMAC1_MEM : NIG_REG_INGRESS_BMAC0_MEM;

      /* BIGMAC_REGISTER_TX_STAT_GTPKT ..
         BIGMAC_REGISTER_TX_STAT_GTBYT */
      if (CHIP_IS_E1x(sc)) {
        tx_src_addr_lo = ((mac_addr + BIGMAC_REGISTER_TX_STAT_GTPKT) >> 2);
        tx_len = ((8 + BIGMAC_REGISTER_TX_STAT_GTBYT -
                   BIGMAC_REGISTER_TX_STAT_GTPKT) >>
                  2);
        rx_src_addr_lo = ((mac_addr + BIGMAC_REGISTER_RX_STAT_GR64) >> 2);
        rx_len = ((8 + BIGMAC_REGISTER_RX_STAT_GRIPJ -
                   BIGMAC_REGISTER_RX_STAT_GR64) >>
                  2);
      } else {
        tx_src_addr_lo = ((mac_addr + BIGMAC2_REGISTER_TX_STAT_GTPOK) >> 2);
        tx_len = ((8 + BIGMAC2_REGISTER_TX_STAT_GTBYT -
                   BIGMAC2_REGISTER_TX_STAT_GTPOK) >>
                  2);
        rx_src_addr_lo = ((mac_addr + BIGMAC2_REGISTER_RX_STAT_GR64) >> 2);
    ...
    default:
      ...
      tx_len = (sizeof(sc->sp->mac_stats.mstat_stats.stats_tx) >> 2);
      rx_len = (sizeof(sc->sp->mac_stats.mstat_stats.stats_rx) >> 2);
      break;
    }
    ...
    dmae->dst_addr_lo = U64_LO(BNX2X_SP_MAPPING(sc, mac_stats) + (tx_len << 2));
    dmae->dst_addr_hi = U64_HI(BNX2X_SP_MAPPING(sc, mac_stats) + (tx_len << 2));
    dmae->len = rx_len;
    dmae->comp_addr_lo = dmae_reg_go_c[loader_idx] >> 2;
    dmae->comp_addr_hi = 0;
    dmae->comp_val = 1;
  ...
  dmae = BNX2X_SP(sc, dmae[sc->executer_idx++]);
  dmae->opcode =
      bnx2x_dmae_opcode(sc, DMAE_SRC_GRC, DMAE_DST_PCI, TRUE, DMAE_COMP_PCI);
  dmae->src_addr_lo =
      (port ? NIG_REG_STAT1_BRB_DISCARD : NIG_REG_STAT0_BRB_DISCARD) >> 2;
  dmae->src_addr_hi = 0;
  dmae->dst_addr_lo = U64_LO(BNX2X_SP_MAPPING(sc, nig_stats));
  dmae->dst_addr_hi = U64_HI(BNX2X_SP_MAPPING(sc, nig_stats));
...
static void bnx2x_func_stats_init(struct bnx2x_softc *sc) {
  struct dmae_command *dmae = &sc->stats_dmae;
  uint32_t *stats_comp = BNX2X_SP(sc, stats_comp);
  ...
  dmae->opcode =
      bnx2x_dmae_opcode(sc, DMAE_SRC_PCI, DMAE_DST_GRC, TRUE, DMAE_COMP_PCI);
  dmae->src_addr_lo = U64_LO(BNX2X_SP_MAPPING(sc, func_stats));
  dmae->src_addr_hi = U64_HI(BNX2X_SP_MAPPING(sc, func_stats));
  dmae->dst_addr_lo = (sc->func_stx >> 2);
  dmae->dst_addr_hi = 0;
...
static void bnx2x_stats_start(struct bnx2x_softc *sc) {
  /*
...
static void bnx2x_stats_pmf_start(struct bnx2x_softc *sc) {
  bnx2x_stats_comp(sc);
  bnx2x_stats_pmf_update(sc);
  bnx2x_stats_start(sc);
}

static void bnx2x_stats_restart(struct bnx2x_softc *sc) {
  /*
   * VFs travel through here as part of the statistics FSM, but no action
   * is required
   */
...
static void bnx2x_bmac_stats_update(struct bnx2x_softc *sc) {
  ...
  if (CHIP_IS_E1x(sc)) {
    ...
    UPDATE_STAT64(tx_stat_gt255, tx_stat_etherstatspkts128octetsto255octets);
    UPDATE_STAT64(tx_stat_gt511, tx_stat_etherstatspkts256octetsto511octets);
    UPDATE_STAT64(tx_stat_gt1023, tx_stat_etherstatspkts512octetsto1023octets);
    ...
    UPDATE_STAT64(tx_stat_gterr, tx_stat_dot3statsinternalmactransmiterrors);
...
static void bnx2x_mstat_stats_update(struct bnx2x_softc *sc) {
  struct host_port_stats *pstats = BNX2X_SP(sc, port_stats);
  struct bnx2x_eth_stats *estats = &sc->eth_stats;
  ...
  ADD_STAT64(stats_tx.tx_gt1023, tx_stat_etherstatspkts512octetsto1023octets);
  ADD_STAT64(stats_tx.tx_gt1518, tx_stat_etherstatspkts1024octetsto1522octets);
...
static void bnx2x_emac_stats_update(struct bnx2x_softc *sc) {
  ...
  estats->pause_frames_sent_hi = pstats->mac_stx[1].tx_stat_outxonsent_hi;
  estats->pause_frames_sent_lo = pstats->mac_stx[1].tx_stat_outxonsent_lo;
  ADD_64(
      estats->pause_frames_sent_hi, pstats->mac_stx[1].tx_stat_outxoffsent_hi,
      estats->pause_frames_sent_lo, pstats->mac_stx[1].tx_stat_outxoffsent_lo);
}

static int bnx2x_hw_stats_update(struct bnx2x_softc *sc) {
  ...
  case ELINK_MAC_TYPE_NONE: /* unreached */
    PMD_DRV_LOG(DEBUG, sc, "stats updated by DMAE but no MAC active");
    return -1;
  ...
  if (!CHIP_IS_E3(sc)) {
    UPDATE_STAT64_NIG(egress_mac_pkt0, etherstatspkts1024octetsto1522octets);
  ...
  if (CHIP_IS_E3(sc)) {
    lpi_reg = (SC_PORT(sc)) ? MISC_REG_CPMU_LP_SM_ENT_CNT_P1
  ...
  if (!BNX2X_NOMCP(sc)) {
    ...
    if (nig_timer_max != estats->nig_timer_max) {
      ...
      PMD_DRV_LOG(ERR, sc, "invalid NIG timer max (%u)", estats->nig_timer_max);
    }
  }
...
static int bnx2x_storm_stats_validate_counters(struct bnx2x_softc *sc) {
...
static int bnx2x_storm_stats_update(struct bnx2x_softc *sc) {
  ...
  for (i = 0; i < sc->num_queues; i++) {
    ...
    /* PMD_DRV_LOG(DEBUG, sc,
                    "queue[%d]: ucast_sent 0x%x bcast_sent 0x%x mcast_sent
    0x%x", i, xclient->ucast_pkts_sent, xclient->bcast_pkts_sent,
                    xclient->mcast_pkts_sent);

    PMD_DRV_LOG(DEBUG, sc, "---------------");
     */

    UPDATE_QSTAT(tclient->rcv_bcast_bytes, total_broadcast_bytes_received);
    UPDATE_QSTAT(tclient->rcv_mcast_bytes, total_multicast_bytes_received);
    UPDATE_QSTAT(tclient->rcv_ucast_bytes, total_unicast_bytes_received);

    /*
     * sum to total_bytes_received all
     * unicast/multicast/broadcast
     */
    qstats->total_bytes_received_hi = qstats->total_broadcast_bytes_received_hi;
    qstats->total_bytes_received_lo = qstats->total_broadcast_bytes_received_lo;
    ...
    UPDATE_EXTEND_E_TSTAT(pkts_too_big_discard, etherstatsoverrsizepkts, 32);
    ...
    SUB_EXTEND_USTAT(bcast_no_buff_pkts, total_broadcast_packets_received);
    ...
    UPDATE_QSTAT(xclient->bcast_bytes_sent, total_broadcast_bytes_transmitted);
    UPDATE_QSTAT(xclient->mcast_bytes_sent, total_multicast_bytes_transmitted);
    UPDATE_QSTAT(xclient->ucast_bytes_sent, total_unicast_bytes_transmitted);
    ...
    UPDATE_EXTEND_XSTAT(ucast_pkts_sent, total_unicast_packets_transmitted);
    UPDATE_EXTEND_XSTAT(mcast_pkts_sent, total_multicast_packets_transmitted);
    UPDATE_EXTEND_XSTAT(bcast_pkts_sent, total_broadcast_packets_transmitted);

    UPDATE_EXTEND_TSTAT(checksum_discard,
                        total_packets_received_checksum_discarded);
    UPDATE_EXTEND_TSTAT(ttl0_discard, total_packets_received_ttl0_discarded);
  ...
  ADD_64(estats->total_bytes_received_hi, estats->rx_stat_ifhcinbadoctets_hi,
         estats->total_bytes_received_lo, estats->rx_stat_ifhcinbadoctets_lo);

  ADD_64_LE(estats->total_bytes_received_hi, tfunc->rcv_error_bytes.hi,
            estats->total_bytes_received_lo, tfunc->rcv_error_bytes.lo);

  ADD_64_LE(estats->error_bytes_received_hi, tfunc->rcv_error_bytes.hi,
            estats->error_bytes_received_lo, tfunc->rcv_error_bytes.lo);

  UPDATE_ESTAT(etherstatsoverrsizepkts, rx_stat_dot3statsframestoolong);

  ADD_64(estats->error_bytes_received_hi, estats->rx_stat_ifhcinbadoctets_hi,
         estats->error_bytes_received_lo, estats->rx_stat_ifhcinbadoctets_lo);
...
static void bnx2x_drv_stats_update(struct bnx2x_softc *sc) {
  struct bnx2x_eth_stats *estats = &sc->eth_stats;
  int i;
...
static uint8_t bnx2x_edebug_stats_stopped(struct bnx2x_softc *sc) {
  uint32_t val;
...
static void bnx2x_stats_update(struct bnx2x_softc *sc) {
  uint32_t *stats_comp = BNX2X_SP(sc, stats_comp);
...
static void bnx2x_port_stats_stop(struct bnx2x_softc *sc) {
  struct dmae_command *dmae;
  uint32_t opcode;
  int loader_idx = PMF_DMAE_C(sc);
  uint32_t *stats_comp = BNX2X_SP(sc, stats_comp);
...
static void bnx2x_stats_stop(struct bnx2x_softc *sc) {
...
static void bnx2x_stats_do_nothing(__rte_unused struct bnx2x_softc *sc) {
  return;
}
...
void bnx2x_stats_handle(struct bnx2x_softc *sc, enum bnx2x_stats_event event) {
  ...
  if (event != STATS_EVENT_UPDATE) {
    PMD_DRV_LOG(DEBUG, sc, "state %d -> event %d -> state %d", state, event,
                sc->stats_state);
  }
}

static void bnx2x_port_stats_base_init(struct bnx2x_softc *sc) {
  struct dmae_command *dmae;
  uint32_t *stats_comp = BNX2X_SP(sc, stats_comp);
 ...
 */
static void bnx2x_prep_fw_stats_req(struct bnx2x_softc *sc) {
  int i;
  ...
  cur_data_offset =
      (sc->fw_stats_data_mapping + offsetof(struct bnx2x_fw_stats_data, port));
  ...
  cur_data_offset =
      (sc->fw_stats_data_mapping + offsetof(struct bnx2x_fw_stats_data, pf));
  ...
  for (i = 0; i < sc->num_queues; i++) {
    cur_query_entry = &sc->fw_stats_req->query[first_queue_query_index + i];
...
void bnx2x_memset_stats(struct bnx2x_softc *sc) {
  ...
  for (i = 0; i < sc->num_queues; i++) {
    ...
    if (sc->stats_init) {
      ...
      memset(&fp->eth_q_stats_old, 0, sizeof(fp->eth_q_stats_old));
    }
  }
...
void bnx2x_stats_init(struct bnx2x_softc *sc) {
  ...
  PMD_DRV_LOG(DEBUG, sc, "port_stx 0x%x func_stx 0x%x", sc->port.port_stx,
              sc->func_stx);
  ...
  for (i = 0; i < sc->num_queues; i++) {
    ...
    if (sc->stats_init) {
      memset(&sc->fp[i].eth_q_stats, 0, sizeof(sc->fp[i].eth_q_stats));
      memset(&sc->fp[i].eth_q_stats_old, 0, sizeof(sc->fp[i].eth_q_stats_old));
    }
  }
