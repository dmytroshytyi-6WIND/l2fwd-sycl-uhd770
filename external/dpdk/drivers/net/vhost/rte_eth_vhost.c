...
static const char *valid_arguments[] = {ETH_VHOST_IFACE_ARG,
...
static struct rte_ether_addr base_eth_addr = {
    .addr_bytes = {0x56 /* V */, 0x48 /* H */, 0x4F /* O */, 0x53 /* S */,
                   0x54 /* T */, 0x00}};
...
static struct rte_eth_link pmd_link = {.link_speed = 10000,
...
static const struct vhost_xstats_name_off vhost_rxport_stat_strings[] = {
    {"good_packets", offsetof(struct vhost_queue, stats.xstats[VHOST_PKT])},
    {"total_bytes", offsetof(struct vhost_queue, stats.xstats[VHOST_BYTE])},
    {"missed_pkts",
...
#define VHOST_NB_XSTATS_RXPORT                                                 \
  (sizeof(vhost_rxport_stat_strings) / sizeof(vhost_rxport_stat_strings[0]))

#define VHOST_NB_XSTATS_TXPORT                                                 \
  (sizeof(vhost_txport_stat_strings) / sizeof(vhost_txport_stat_strings[0]))

static int vhost_dev_xstats_reset(struct rte_eth_dev *dev) {
...
static int vhost_dev_xstats_get_names(struct rte_eth_dev *dev __rte_unused,
...
static int vhost_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  for (t = 0; t < VHOST_NB_XSTATS_RXPORT; t++) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
      ...
      if (!vq)
        continue;
      xstats[count].value +=
          *(uint64_t *)(((char *)vq) + vhost_rxport_stat_strings[t].offset);
    }
  ...
  for (t = 0; t < VHOST_NB_XSTATS_TXPORT; t++) {
    ...
    for (i = 0; i < dev->data->nb_tx_queues; i++) {
      ...
      if (!vq)
        continue;
      xstats[count].value +=
          *(uint64_t *)(((char *)vq) + vhost_txport_stat_strings[t].offset);
    }
    xstats[count].id = count;
    count++;
  }
...
static inline void vhost_count_xcast_packets(struct vhost_queue *vq,
...
vhost_update_single_packet_xstats(struct vhost_queue *vq,
                                  struct rte_mbuf *buf) {
  uint32_t pkt_len = 0;
  uint64_t index;
  struct vhost_stats *pstats = &vq->stats;
  ...
  if (pkt_len == 64) {
    ...
    index = (sizeof(pkt_len) * 8) - __builtin_clz(pkt_len) - 5;
    pstats->xstats[index]++;
  } else {
    if (pkt_len < 64)
...
static uint16_t eth_vhost_rx(void *q, struct rte_mbuf **bufs,
                             uint16_t nb_bufs) {
  ...
  while (nb_receive) {
    ...
    nb_pkts = rte_vhost_dequeue_burst(r->vid, r->virtqueue_id, r->mb_pool,
                                      &bufs[nb_rx], num);
...
static uint16_t eth_vhost_tx(void *q, struct rte_mbuf **bufs,
                             uint16_t nb_bufs) {
  struct vhost_queue *r = q;
  uint16_t i, nb_tx = 0;
  uint16_t nb_send = 0;
  ...
  while (nb_send) {
    ...
    nb_pkts =
        rte_vhost_enqueue_burst(r->vid, r->virtqueue_id, &bufs[nb_tx], num);
...
static int eth_vhost_update_intr(struct rte_eth_dev *eth_dev,
                                 uint16_t rxq_idx) {
  ...
  epfd = elist->epfd;
  rev = *elist;
  ret = rte_epoll_ctl(epfd, EPOLL_CTL_DEL, rev.fd, elist);
  if (ret) {
...
static int eth_rxq_intr_enable(struct rte_eth_dev *dev, uint16_t qid) {
...
static int eth_rxq_intr_disable(struct rte_eth_dev *dev, uint16_t qid) {
  struct rte_vhost_vring vring;
  struct vhost_queue *vq;
  int ret = 0;
...
static void eth_vhost_uninstall_intr(struct rte_eth_dev *dev) {
...
static int eth_vhost_install_intr(struct rte_eth_dev *dev) {
  struct rte_vhost_vring vring;
  struct vhost_queue *vq;
  ...
  for (i = 0; i < nb_rxq; i++) {
    ...
    if (ret < 0) {
      VHOST_LOG(INFO, "Failed to get rxq-%d's vring, skip!\n", i);
      continue;
    }

    if (vring.kickfd < 0) {
      VHOST_LOG(INFO, "rxq-%d's kickfd is invalid, skip!\n", i);
      continue;
    }
...
static void update_queuing_status(struct rte_eth_dev *dev) {
...
static void queue_setup(struct rte_eth_dev *eth_dev,
                        struct pmd_internal *internal) {
  struct vhost_queue *vq;
  int i;
  ...
  if (rte_atomic32_read(&internal->started) == 1) {
    ...
    if (dev_conf->intr_conf.rxq) {
      if (eth_vhost_install_intr(eth_dev) < 0) {
        VHOST_LOG(INFO, "Failed to install interrupt handler.");
        return -1;
      }
    }
  } else {
...
static int vring_conf_update(int vid, struct rte_eth_dev *eth_dev,
                             uint16_t vring_id) {
  ...
  if (rx_idx >= 0 && rx_idx < eth_dev->data->nb_rx_queues &&
      ...
      rte_atomic32_read(&internal->started) && dev_conf->intr_conf.rxq) {
    ...
    if (rte_intr_efds_index_set(eth_dev->intr_handle, rx_idx, vring.kickfd))
      return -rte_errno;
...
static int vring_state_changed(int vid, uint16_t vring, int enable) {
  struct rte_vhost_vring_state *state;
  struct rte_eth_dev *eth_dev;
  struct internal_list *list;
  char ifname[PATH_MAX];
...
static int vhost_driver_setup(struct rte_eth_dev *eth_dev) {
  struct pmd_internal *internal = eth_dev->data->dev_private;
  ...
  vring_state = rte_zmalloc_socket(name, sizeof(*vring_state), 0, numa_node);
  if (vring_state == NULL)
    goto free_list;
  ...
  if (rte_vhost_driver_callback_register(internal->iface_name, &vhost_ops) <
      0) {
...
static int eth_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
  ...
  vq = rte_zmalloc_socket(NULL, sizeof(struct vhost_queue), RTE_CACHE_LINE_SIZE,
                          socket_id);
  if (vq == NULL) {
...
eth_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id,
                   uint16_t nb_tx_desc __rte_unused, unsigned int socket_id,
...
static int eth_tx_done_cleanup(void *txq __rte_unused,
...
static uint32_t eth_rx_queue_count(void *rx_queue) {
  struct vhost_queue *vq;
...
static int vhost_get_monitor_addr(void *rx_queue,
  ...
  if (vq == NULL)
    return -EINVAL;
  ret = rte_vhost_get_monitor_addr(vq->vid, vq->virtqueue_id, &vhost_pmc);
  if (ret < 0)
    return -EINVAL;
...
static int eth_dev_vhost_create(struct rte_vdev_device *dev, char *iface_name,
  ...
  *eth_addr = base_eth_addr;
  ...
  internal = eth_dev->data->dev_private;
  internal->iface_name =
      rte_malloc_socket(name, strlen(iface_name) + 1, 0, numa_node);
...
static inline int open_int(const char *key __rte_unused, const char *value,
                           void *extra_args) {
...
static int rte_pmd_vhost_probe(struct rte_vdev_device *dev) {
  ...
  if (rte_kvargs_count(kvlist, ETH_VHOST_IFACE_ARG) == 1) {
    ret = rte_kvargs_process(kvlist, ETH_VHOST_IFACE_ARG, &open_iface,
                             &iface_name);
    if (ret < 0)
      goto out_free;
  } else {
  ...
  if (rte_kvargs_count(kvlist, ETH_VHOST_QUEUES_ARG) == 1) {
    ret = rte_kvargs_process(kvlist, ETH_VHOST_QUEUES_ARG, &open_int, &queues);
  ...
  if (rte_kvargs_count(kvlist, ETH_VHOST_CLIENT_ARG) == 1) {
    ret = rte_kvargs_process(kvlist, ETH_VHOST_CLIENT_ARG, &open_int,
                             &client_mode);
    if (ret < 0)
      goto out_free;
  ...
  if (rte_kvargs_count(kvlist, ETH_VHOST_IOMMU_SUPPORT) == 1) {
    ret = rte_kvargs_process(kvlist, ETH_VHOST_IOMMU_SUPPORT, &open_int,
                             &iommu_support);
    if (ret < 0)
      goto out_free;
  ...
  if (rte_kvargs_count(kvlist, ETH_VHOST_POSTCOPY_SUPPORT) == 1) {
    ret = rte_kvargs_process(kvlist, ETH_VHOST_POSTCOPY_SUPPORT, &open_int,
                             &postcopy_support);
    if (ret < 0)
      goto out_free;
  ...
  if (rte_kvargs_count(kvlist, ETH_VHOST_VIRTIO_NET_F_HOST_TSO) == 1) {
    ret = rte_kvargs_process(kvlist, ETH_VHOST_VIRTIO_NET_F_HOST_TSO, &open_int,
                             &tso);
    if (ret < 0)
      goto out_free;
  ...
  if (rte_kvargs_count(kvlist, ETH_VHOST_LINEAR_BUF) == 1) {
    ret = rte_kvargs_process(kvlist, ETH_VHOST_LINEAR_BUF, &open_int,
                             &linear_buf);
    if (ret < 0)
      goto out_free;
  ...
  if (rte_kvargs_count(kvlist, ETH_VHOST_EXT_BUF) == 1) {
    ret = rte_kvargs_process(kvlist, ETH_VHOST_EXT_BUF, &open_int, &ext_buf);
    if (ret < 0)
      goto out_free;
  ...
  ret = eth_dev_vhost_create(dev, iface_name, queues, dev->device.numa_node,
                             flags, disable_flags);
  if (ret == -1)
...
static int rte_pmd_vhost_remove(struct rte_vdev_device *dev) {
  const char *name;
  struct rte_eth_dev *eth_dev = NULL;
...
RTE_PMD_REGISTER_PARAM_STRING(net_vhost, "iface=<ifc> "
                                         "queues=<int> "
                                         "client=<0|1> "
                                         "iommu-support=<0|1> "
