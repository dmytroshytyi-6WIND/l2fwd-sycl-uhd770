...
static int __nfp_net_reconfig(struct nfp_net_hw *hw, uint32_t update) {
  int cnt;
  uint32_t new;
  struct timespec wait;

  PMD_DRV_LOG(DEBUG, "Writing to the configuration queue (%p)...", hw->qcp_cfg);
...
int nfp_net_reconfig(struct nfp_net_hw *hw, uint32_t ctrl, uint32_t update) {
  ...
  PMD_DRV_LOG(DEBUG, "nfp_net_reconfig: ctrl=%08x update=%08x", ctrl, update);
   ...
   */
  PMD_INIT_LOG(ERR, "Error nfp_net reconfig for ctrl: %x update: %x", ctrl,
               update);
  return -EIO;
}
...
void nfp_net_write_mac(struct nfp_net_hw *hw, uint8_t *mac) {
  ...
  mac += 4;
  mac1 = *(uint16_t *)mac;
  nn_writew(rte_cpu_to_be_16(mac1), hw->ctrl_bar + NFP_NET_CFG_MACADDR + 6);
}
...
int nfp_net_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    nfp_dev_stats.q_ipackets[i] = nn_cfg_readq(hw, NFP_NET_CFG_RXR_STATS(i));
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    nfp_dev_stats.q_opackets[i] = nn_cfg_readq(hw, NFP_NET_CFG_TXR_STATS(i));
  ...
  nfp_dev_stats.opackets = nn_cfg_readq(hw, NFP_NET_CFG_STATS_TX_FRAMES);

  nfp_dev_stats.opackets -= hw->eth_stats_base.opackets;

  nfp_dev_stats.obytes = nn_cfg_readq(hw, NFP_NET_CFG_STATS_TX_OCTETS);

  nfp_dev_stats.obytes -= hw->eth_stats_base.obytes;

  /* reading general device stats */
  nfp_dev_stats.ierrors = nn_cfg_readq(hw, NFP_NET_CFG_STATS_RX_ERRORS);

  nfp_dev_stats.ierrors -= hw->eth_stats_base.ierrors;

  nfp_dev_stats.oerrors = nn_cfg_readq(hw, NFP_NET_CFG_STATS_TX_ERRORS);
  ...
  nfp_dev_stats.imissed = nn_cfg_readq(hw, NFP_NET_CFG_STATS_RX_DISCARDS);
...
int nfp_net_stats_reset(struct rte_eth_dev *dev) {
  ...
  hw->eth_stats_base.ipackets = nn_cfg_readq(hw, NFP_NET_CFG_STATS_RX_FRAMES);

  hw->eth_stats_base.ibytes = nn_cfg_readq(hw, NFP_NET_CFG_STATS_RX_OCTETS);

  hw->eth_stats_base.opackets = nn_cfg_readq(hw, NFP_NET_CFG_STATS_TX_FRAMES);

  hw->eth_stats_base.obytes = nn_cfg_readq(hw, NFP_NET_CFG_STATS_TX_OCTETS);

  /* reading general device stats */
  hw->eth_stats_base.ierrors = nn_cfg_readq(hw, NFP_NET_CFG_STATS_RX_ERRORS);

  hw->eth_stats_base.oerrors = nn_cfg_readq(hw, NFP_NET_CFG_STATS_TX_ERRORS);

  /* RX ring mbuf allocation failures */
  dev->data->rx_mbuf_alloc_failed = 0;

  hw->eth_stats_base.imissed = nn_cfg_readq(hw, NFP_NET_CFG_STATS_RX_DISCARDS);
...
const uint32_t *nfp_net_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {
      ...
      RTE_PTYPE_INNER_L3_IPV6_EXT, RTE_PTYPE_INNER_L4_MASK, RTE_PTYPE_UNKNOWN};
...
int nfp_rx_queue_intr_enable(struct rte_eth_dev *dev, uint16_t queue_id) {
  ...
  rte_wmb();
  nn_cfg_writeb(hw, NFP_NET_CFG_ICR(base + queue_id), NFP_NET_CFG_ICR_UNMASKED);
  return 0;
}
 ...
 */
static void nfp_net_irq_unmask(struct rte_eth_dev *dev) {
  struct nfp_net_hw *hw;
  struct rte_pci_device *pci_dev;
...
int nfp_net_rss_config_default(struct rte_eth_dev *dev) {
  ...
  PMD_DRV_LOG(INFO, "setting default RSS conf for %u queues", rx_queues);
