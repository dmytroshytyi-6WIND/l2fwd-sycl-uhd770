...
static int nfp_net_rx_fill_freelist(struct nfp_net_rxq *rxq) {
  ...
  PMD_RX_LOG(DEBUG, "Fill Rx Freelist for %u descriptors", rxq->rx_count);
...
uint16_t nfp_net_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (avail < nb_pkts) {
    ...
    if (unlikely(new_mb == NULL)) {
      RTE_LOG_DP(DEBUG, PMD, "RX mbuf alloc failed port_id=%u queue_id=%u\n",
    ...
    if (unlikely((mb->data_len + hw->rx_offset) > rxq->mbuf_size)) {
      ...
      RTE_LOG_DP(ERR, PMD,
                 ...
                 hw->rx_offset, rxq->mbuf_size - hw->rx_offset, mb->data_len);
      return -EINVAL;
    }
  ...
  if (nb_hold > rxq->rx_free_thresh) {
    PMD_RX_LOG(DEBUG, "port=%u queue=%u nb_hold=%u avail=%u", rxq->port_id,
               (unsigned int)rxq->qidx, (unsigned int)nb_hold,
               (unsigned int)avail);
...
int nfp_net_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (rx_desc_sz % NFP_ALIGN_RING_DESC != 0 || nb_desc > NFP_NET_MAX_RX_DESC ||
  ...
  PMD_RX_LOG(DEBUG, "rxbufs=%p hw_ring=%p dma_addr=0x%" PRIx64, rxq->rxbufs,
             rxq->rxds, (unsigned long)rxq->dma);
...
int nfp_net_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (tx_desc_sz % NFP_ALIGN_RING_DESC != 0 || nb_desc > NFP_NET_MAX_TX_DESC ||
  ...
  if (tx_free_thresh > (nb_desc)) {
    PMD_DRV_LOG(ERR,
                ...
                "queue=%d)",
                (unsigned int)tx_free_thresh, dev->data->port_id,
                (int)queue_idx);
    return -(EINVAL);
  }
  ...
  txq->txbufs =
      rte_zmalloc_socket("txq->txbufs", sizeof(*txq->txbufs) * nb_desc,
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (txq->txbufs == NULL) {
    nfp_net_tx_queue_release(dev, queue_idx);
    dev->data->tx_queues[queue_idx] = NULL;
  ...
  PMD_TX_LOG(DEBUG, "txbufs=%p hw_ring=%p dma_addr=0x%" PRIx64, txq->txbufs,
             txq->txds, (unsigned long)txq->dma);
...
uint16_t nfp_net_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  PMD_TX_LOG(DEBUG, "working for queue %u at pos %d and %u packets", txq->qidx,
             txq->wr_p, nb_pkts);
  ...
  while ((i < nb_pkts) && free_descs) {
    ...
    if (unlikely(pkt->nb_segs > 1 && !(hw->cap & NFP_NET_CFG_CTRL_GATHER))) {
