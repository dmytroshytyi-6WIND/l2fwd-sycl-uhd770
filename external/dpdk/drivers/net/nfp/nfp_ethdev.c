...
static int nfp_fw_upload(struct rte_pci_device *dev, struct nfp_nsp *nsp,
                         char *card);
...
static int nfp_net_init(struct rte_eth_dev *eth_dev) {
  ...
  if (rte_mem_check_dma_mask(40)) {
    RTE_LOG(ERR, PMD, "device %s can not be used:", pci_dev->device.name);
  ...
  PMD_INIT_LOG(DEBUG,
               "Working with physical port number: %d, "
  ...
  PMD_INIT_LOG(INFO,
               ...
               pci_dev->id.device_id, hw->mac_addr[0], hw->mac_addr[1],
               hw->mac_addr[2], hw->mac_addr[3], hw->mac_addr[4],
               hw->mac_addr[5]);
...
static int nfp_fw_upload(struct rte_pci_device *dev, struct nfp_nsp *nsp,
  ...
  snprintf(serial, sizeof(serial),
           "serial-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x", cpp->serial[0],
           cpp->serial[1], cpp->serial[2], cpp->serial[3], cpp->serial[4],
           cpp->serial[5], cpp->interface >> 8, cpp->interface & 0xff);
...
static int nfp_pf_init(struct rte_pci_device *pci_dev) {
  ...
  pf_dev->ctrl_bar =
      nfp_rtsym_map(pf_dev->sym_tbl, "_pf0_net_bar0",
  ...
  pf_dev->hw_queues =
      nfp_cpp_map_area(pf_dev->cpp, 0, 0, NFP_PCIE_QUEUE(0),
                       NFP_QCP_QUEUE_AREA_SZ, &pf_dev->hwqueues_area);
  if (!pf_dev->hw_queues) {
...
static int nfp_pf_secondary_init(struct rte_pci_device *pci_dev) {
  ...
  for (i = 0; i < total_ports; i++) {
    ...
    snprintf(port_name, sizeof(port_name), "%s_port%d", pci_dev->device.name,
             i);
