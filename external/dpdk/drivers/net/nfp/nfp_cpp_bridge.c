...
void nfp_register_cpp_service(struct nfp_cpp *cpp) {
  ...
  if (rte_service_component_register(&service, cpp_service_id))
...
static int nfp_cpp_bridge_serve_write(int sockfd, struct nfp_cpp *cpp) {
  ...
  if (((nfp_offset + (off_t)count - 1) & ~(NFP_CPP_MEMIO_BOUNDARY - 1)) !=
    ...
    curlen =
        NFP_CPP_MEMIO_BOUNDARY - (nfp_offset & (NFP_CPP_MEMIO_BOUNDARY - 1));
  }

  while (count > 0) {
    /* configure a CPP PCIe2CPP BAR for mapping the CPP target */
    area = nfp_cpp_area_alloc_with_name(cpp, cpp_id, "nfp.cdev", nfp_offset,
                                        curlen);
    if (!area) {
    ...
    for (pos = 0; pos < curlen; pos += len) {
      ...
      if (err != (int)len) {
        RTE_LOG(ERR, PMD, "%s: error when receiving, %d of %zu\n", __func__,
                err, count);
    ...
    count -= pos;
    curlen = (count > NFP_CPP_MEMIO_BOUNDARY) ? NFP_CPP_MEMIO_BOUNDARY : count;
  }
...
static int nfp_cpp_bridge_serve_read(int sockfd, struct nfp_cpp *cpp) {
  ...
  while (count > 0) {
    ...
    for (pos = 0; pos < curlen; pos += len) {
      ...
      if (err != (int)len) {
        RTE_LOG(ERR, PMD, "%s: error when sending: %d of %zu\n", __func__, err,
                count);
        nfp_cpp_area_release(area);
        nfp_cpp_area_free(area);
        return -EIO;
...
int32_t nfp_cpp_bridge_service_func(void *args) {
  ...
  ret =
      bind(sockfd, (const struct sockaddr *)&address, sizeof(struct sockaddr));
  if (ret < 0) {
