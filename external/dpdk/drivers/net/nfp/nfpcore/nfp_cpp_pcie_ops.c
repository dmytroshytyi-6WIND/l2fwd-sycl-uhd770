...
#define NFP_PCIE_CPP_BAR_PCIETOCPPEXPBAR(bar, slot) (((bar)*8 + (slot)) * 4)
...
static uint32_t nfp_bar_maptype(struct nfp_bar *bar) {
...
static int nfp_compute_bar(const struct nfp_bar *bar, uint32_t *bar_config,
  ...
  if (act != NFP_CPP_ACTION_RW && act != 0) {
    ...
    newcfg |=
        NFP_PCIE_BAR_PCIE2CPP_MAPTYPE(NFP_PCIE_BAR_PCIE2CPP_MAPTYPE_FIXED);
    ...
    printf("\t%d:%d:%d:0x%#llx-0x%#llx>\n", tgt, act, tok,
           (unsigned long long)offset, (unsigned long long)(offset + mask));
#endif
    ...
    /* Bulk mapping */
    newcfg |= NFP_PCIE_BAR_PCIE2CPP_MAPTYPE(NFP_PCIE_BAR_PCIE2CPP_MAPTYPE_BULK);
    ...
    if ((offset & mask) != ((offset + size - 1) & mask)) {
      ...
      printf("\tBAR too small (%#llx) - (%#llx != %#llx).\n",
             (unsigned long long)mask, (unsigned long long)(offset & mask),
...
static int nfp_bar_write(struct nfp_pcie_user *nfp, struct nfp_bar *bar,
                         uint32_t newcfg) {
  int base, slot;
  ...
  bar->csr = nfp->cfg + NFP_PCIE_CFG_BAR_PCIETOCPPEXPBAR(base, slot);
...
static int nfp_reconfigure_bar(struct nfp_pcie_user *nfp, struct nfp_bar *bar,
  ...
  err = nfp_compute_bar(bar, &newcfg, &newbase, tgt, act, tok, offset, size,
                        width);
  if (err)
    return err;
 ...
 *         now.
 */
static int nfp_enable_bars(struct nfp_pcie_user *nfp) {
  struct nfp_bar *bar;
  int x, start, end;
  ...
  for (x = start; x > end; x--) {
    ...
    bar->iomem = NULL;
    bar->lock = 0;
    bar->csr = nfp->cfg + NFP_PCIE_CFG_BAR_PCIETOCPPEXPBAR(bar->index >> 3,
                                                           bar->index & 7);
...
static struct nfp_bar *nfp_alloc_bar(struct nfp_pcie_user *nfp) {
  struct nfp_bar *bar;
  int x, start, end;
...
static void nfp_disable_bars(struct nfp_pcie_user *nfp) {
  struct nfp_bar *bar;
  int x, start, end;
...
static int nfp6000_area_init(struct nfp_cpp_area *area, uint32_t dest,
  ...
  pp = nfp6000_target_pushpull(NFP_CPP_ID(target, action, token), address);
  if (pp < 0)
    return pp;
  ...
  if (priv->width.read > 0 && priv->width.write > 0 &&
...
static int nfp6000_area_acquire(struct nfp_cpp_area *area) {
  ...
  if (nfp_bar_maptype(priv->bar) == NFP_PCIE_BAR_PCIE2CPP_MAPTYPE_GENERAL) {
    priv->bar_offset =
        priv->offset & (NFP_PCIE_P2C_GENERAL_SIZE(priv->bar) - 1);
    priv->bar_offset +=
...
static void *nfp6000_area_mapped(struct nfp_cpp_area *area) {
...
static void *nfp6000_area_iomem(struct nfp_cpp_area *area) {
  struct nfp6000_area_priv *priv = nfp_cpp_area_priv(area);
  return priv->iomem;
}
...
static int nfp6000_area_read(struct nfp_cpp_area *area, void *kernel_vaddr,
  ...
  if (is_64) {
    if (offset % sizeof(uint64_t) != 0 || length % sizeof(uint64_t) != 0)
      return -EINVAL;
  } else {
...
static int nfp_acquire_process_lock(struct nfp_pcie_user *desc) {
  int rc;
  struct flock lock;
  char lockname[30];
...
static int nfp_acquire_secondary_process_lock(struct nfp_pcie_user *desc) {
  int rc;
  struct flock lock;
  ...
  lockfile = calloc(strlen(home_path) + strlen(lockname) + 1, sizeof(char));
  ...
  desc->secondary_lock = open(lockfile, O_RDWR | O_CREAT | O_NONBLOCK, 0666);
...
static int nfp6000_set_model(struct rte_pci_device *dev, struct nfp_cpp *cpp) {
  uint32_t model;
...
static int nfp6000_set_interface(struct rte_pci_device *dev,
                                 struct nfp_cpp *cpp) {
  uint16_t interface;
...
static int nfp6000_set_serial(struct rte_pci_device *dev, struct nfp_cpp *cpp) {
  uint16_t tmp;
  uint8_t serial[6];
  int serial_len = 6;
  off_t pos;
...
static int nfp6000_set_barsz(struct rte_pci_device *dev,
...
static int nfp6000_init(struct nfp_cpp *cpp, struct rte_pci_device *dev) {
  int ret = 0;
  struct nfp_pcie_user *desc;
