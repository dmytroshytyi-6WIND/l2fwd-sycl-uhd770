...
static void nfp_rtsym_sw_entry_init(struct nfp_rtsym_table *cache,
                                    uint32_t strtab_size, struct nfp_rtsym *sw,
                                    struct nfp_rtsym_entry *fw) {
  sw->type = fw->type;
  sw->name = cache->strtab + rte_le_to_cpu_16(fw->name) % strtab_size;
  sw->addr = ((uint64_t)fw->addr_hi << 32) | rte_le_to_cpu_32(fw->addr_lo);
  sw->size = ((uint64_t)fw->size_hi << 32) | rte_le_to_cpu_32(fw->size_lo);

#ifdef DEBUG
  printf("rtsym_entry_init\n");
  printf("\tname=%s, addr=%" PRIx64 ", size=%" PRIu64 ",target=%d\n", sw->name,
         sw->addr, sw->size, sw->target);
#endif
  switch (fw->target) {
  case SYM_TGT_LMEM:
...
struct nfp_rtsym_table *__nfp_rtsym_table_read(struct nfp_cpp *cpp,
  ...
  const uint32_t dram =
      NFP_CPP_ID(NFP_CPP_TARGET_MU, NFP_CPP_ACTION_RW, 0) | NFP_ISL_EMEM0;
  int err, n, size;
  ...
  for (n = 0; n < cache->num; n++)
    nfp_rtsym_sw_entry_init(cache, strtab_size, &cache->symtab[n],
                            &rtsymtab[n]);
