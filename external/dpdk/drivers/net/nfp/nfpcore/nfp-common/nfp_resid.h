 ...
 * write instruction/call is performed on the NFP_CPP_ID.  It is recommended
 * that the RW action is used even if all actions to be performed on a
 * NFP_CPP_ID are known to be only reads or writes. Doing so will in many cases
 * save NFP CPP internal software resources.
 */
...
#define NFP_CPP_ID(target, action, token)                                      \
  ((((target)&0x7f) << 24) | (((token)&0xff) << 16) | (((action)&0xff) << 8))
 ...
 * @return      NFP CPP action
 */
static inline uint8_t NFP_CPP_ID_ISLAND_of(uint32_t id) { return (id)&0xff; }
...
#define NFP_FAMILY_ARCH(x)                                                     \
  (__extension__({                                                             \
    ...
    (NFP_FAMILY_IS_ARCH_TH(_x)   ? NFP_CHIP_ARCH_TH                            \
     : NFP_FAMILY_IS_ARCH_YD(_x) ? NFP_CHIP_ARCH_YD                            \
                                 : -1)                                         \
  }))
 ...
 */
int nfp_str2cpp(int chip_family, const char *tid, uint32_t *cpp_idp,
                uint64_t *cpp_addrp);
