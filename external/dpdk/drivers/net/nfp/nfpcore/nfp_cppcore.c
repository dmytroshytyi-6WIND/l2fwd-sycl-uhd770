...
void nfp_cpp_priv_set(struct nfp_cpp *cpp, void *priv) { cpp->priv = priv; }

void *nfp_cpp_priv(struct nfp_cpp *cpp) { return cpp->priv; }
...
void *nfp_cpp_area_priv(struct nfp_cpp_area *cpp_area) { return &cpp_area[1]; }
...
struct nfp_cpp_area *nfp_cpp_area_alloc_with_name(struct nfp_cpp *cpp,
  ...
  area = calloc(1, sizeof(*area) + cpp->op->area_priv_size + strlen(name) + 1);
  if (!area)
    return NULL;
...
static struct nfp_cpp *nfp_cpp_alloc(struct rte_pci_device *dev,
  ...
  if (NFP_CPP_MODEL_IS_6000(nfp_cpp_model(cpp))) {
    ...
    for (tgt = 0; tgt < ARRAY_SIZE(cpp->imb_cat_table); tgt++) {
      ...
      err = nfp_xpb_readl(cpp, xpbaddr, (uint32_t *)&cpp->imb_cat_table[tgt]);
      if (err < 0) {
        free(cpp);
        return NULL;
      }
...
int nfp_cpp_area_fill(struct nfp_cpp_area *area, unsigned long offset,
  ...
  for (i = 0; (i + sizeof(value)) < length; i += sizeof(value64)) {
    err = nfp_cpp_area_write(area, offset + i, &value64, sizeof(value64));
    if (err < 0)
      return err;
...
uint32_t __nfp_cpp_model_autodetect(struct nfp_cpp *cpp, uint32_t *model) {
  ...
  err = nfp_xpb_readl(cpp, NFP_XPB_DEVICE(1, 1, 16) + NFP_PL_DEVICE_ID, &reg);
  if (err < 0)
    return err;
