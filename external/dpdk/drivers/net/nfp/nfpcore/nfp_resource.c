...
#define NFP_RESOURCE_TBL_ENTRIES                                               \
  (int)(NFP_RESOURCE_TBL_SIZE / sizeof(struct nfp_resource_entry))
...
static int nfp_cpp_resource_find(struct nfp_cpp *cpp,
  ...
  for (i = 0; i < NFP_RESOURCE_TBL_ENTRIES; i++) {
    uint64_t addr =
        NFP_RESOURCE_TBL_BASE + sizeof(struct nfp_resource_entry) * i;
    ...
    /* Found key! */
    res->mutex = nfp_cpp_mutex_alloc(cpp, NFP_RESOURCE_TBL_TARGET, addr, key);
    res->cpp_id = NFP_CPP_ID(entry.region.cpp_target, entry.region.cpp_action,
 ...
 */
uint64_t nfp_resource_size(const struct nfp_resource *res) { return res->size; }
