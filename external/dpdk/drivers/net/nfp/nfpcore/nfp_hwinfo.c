...
static int nfp_hwinfo_db_walk(struct nfp_hwinfo *hwinfo, uint32_t size) {
  const char *key, *val, *end = hwinfo->data + size;

  for (key = hwinfo->data; *key && key < end; key = val + strlen(val) + 1) {
...
static int nfp_hwinfo_db_validate(struct nfp_hwinfo *db, uint32_t len) {
...
static struct nfp_hwinfo *nfp_hwinfo_try_fetch(struct nfp_cpp *cpp,
                                               size_t *cpp_size) {
...
static struct nfp_hwinfo *nfp_hwinfo_fetch(struct nfp_cpp *cpp,
                                           size_t *hwdb_size) {
