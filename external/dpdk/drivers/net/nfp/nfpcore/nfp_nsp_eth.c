...
static void nfp_eth_copy_mac_reverse(uint8_t *dst, const uint8_t *src) {
  int i;
...
static void nfp_eth_port_translate(struct nfp_nsp *nsp,
...
static void nfp_eth_calc_port_geometry(struct nfp_eth_table *table) {
  unsigned int i, j;

  for (i = 0; i < table->count; i++) {
    table->max_index = RTE_MAX(table->max_index, table->ports[i].index);

    for (j = 0; j < table->count; j++) {
      if (table->ports[i].label_port != table->ports[j].label_port)
        continue;
      table->ports[i].port_lanes += table->ports[j].lanes;

      if (i == j)
        continue;
      if (table->ports[i].label_subport == table->ports[j].label_subport)
        printf("Port %d subport %d is a duplicate\n",
               table->ports[i].label_port, table->ports[i].label_subport);
...
static void nfp_eth_calc_port_type(struct nfp_eth_table_port *entry) {
...
static struct nfp_eth_table *__nfp_eth_read_ports(struct nfp_nsp *nsp) {
...
static int nfp_eth_set_bit_config(struct nfp_nsp *nsp, unsigned int raw_idx,
...
#define NFP_ETH_SET_BIT_CONFIG(nsp, raw_idx, mask, val, ctrl_bit)              \
  (__extension__({                                                             \
    typeof(mask) _x = (mask);                                                  \
    nfp_eth_set_bit_config(nsp, raw_idx, _x, __bf_shf(_x), val, ctrl_bit);     \
  }))
...
int __nfp_eth_set_aneg(struct nfp_nsp *nsp, enum nfp_eth_aneg mode) {
  return NFP_ETH_SET_BIT_CONFIG(nsp, NSP_ETH_RAW_STATE, NSP_ETH_STATE_ANEG,
                                mode, NSP_ETH_CTRL_SET_ANEG);
}
...
static int __nfp_eth_set_fec(struct nfp_nsp *nsp, enum nfp_eth_fec mode) {
  return NFP_ETH_SET_BIT_CONFIG(nsp, NSP_ETH_RAW_STATE, NSP_ETH_STATE_FEC, mode,
...
int __nfp_eth_set_speed(struct nfp_nsp *nsp, unsigned int speed) {
  ...
  return NFP_ETH_SET_BIT_CONFIG(nsp, NSP_ETH_RAW_STATE, NSP_ETH_STATE_RATE,
                                rate, NSP_ETH_CTRL_SET_RATE);
}
