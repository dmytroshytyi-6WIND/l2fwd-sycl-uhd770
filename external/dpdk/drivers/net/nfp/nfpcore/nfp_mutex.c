...
static int _nfp_cpp_mutex_validate(uint32_t model, int *target,
                                   unsigned long long address) {
...
uint32_t nfp_cpp_mutex_key(struct nfp_cpp_mutex *mutex) { return mutex->key; }
...
int nfp_cpp_mutex_target(struct nfp_cpp_mutex *mutex) { return mutex->target; }
