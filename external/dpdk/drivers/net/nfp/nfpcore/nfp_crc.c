...
static inline uint32_t nfp_crc32_be_generic(uint32_t crc,
...
static inline uint32_t nfp_crc32_be(uint32_t crc, unsigned char const *p,
                                    size_t len) {
  return nfp_crc32_be_generic(crc, p, len, CRCPOLY_BE);
}

static uint32_t nfp_crc32_posix_end(uint32_t crc, size_t total_len) {
