...
static int nfp_mip_try_read(struct nfp_cpp *cpp, uint32_t cpp_id, uint64_t addr,
                            struct nfp_mip *mip) {
  int ret;
...
void nfp_mip_close(struct nfp_mip *mip) { free(mip); }

const char *nfp_mip_name(const struct nfp_mip *mip) { return mip->name; }
