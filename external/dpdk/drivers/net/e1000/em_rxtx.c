...
#define E1000_TX_OFFLOAD_MASK                                                  \
  (RTE_MBUF_F_TX_IPV6 | RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IP_CKSUM |          \
   RTE_MBUF_F_TX_L4_MASK | RTE_MBUF_F_TX_VLAN)
 ...
 */
static inline void em_set_xmit_ctx(struct em_tx_queue *txq,
                                   volatile struct e1000_context_desc *ctx_txd,
                                   uint64_t flags, union em_vlan_macip hdrlen) {
  ...
  ctx.lower_setup.ip_fields.ipcso =
      (uint8_t)(l2len + offsetof(struct rte_ipv4_hdr, hdr_checksum));
  ...
  if (flags & RTE_MBUF_F_TX_IP_CKSUM) {
    ctx.lower_setup.ip_fields.ipcse = (uint16_t)rte_cpu_to_le_16(ipcse - 1);
  ...
  case RTE_MBUF_F_TX_UDP_CKSUM:
    ctx.upper_setup.tcp_fields.tucso =
        (uint8_t)(ipcse + offsetof(struct rte_udp_hdr, dgram_cksum));
    cmp_mask |= TX_MACIP_LEN_CMP_MASK;
    break;
  case RTE_MBUF_F_TX_TCP_CKSUM:
    ctx.upper_setup.tcp_fields.tucso =
        (uint8_t)(ipcse + offsetof(struct rte_tcp_hdr, cksum));
 ...
 */
static inline uint32_t what_ctx_update(struct em_tx_queue *txq, uint64_t flags,
...
static inline int em_xmit_cleanup(struct em_tx_queue *txq) {
...
static inline uint32_t tx_desc_cksum_flags_to_upper(uint64_t ol_flags) {
...
uint16_t eth_em_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    PMD_TX_LOG(DEBUG,
               ...
               (unsigned)tx_pkt->pkt_len, (unsigned)tx_id, (unsigned)tx_last);
     ...
     *   - E1000_TXD_CMD_RS
     */
    cmd_type_len = E1000_TXD_CMD_DEXT | E1000_TXD_DTYP_D | E1000_TXD_CMD_IFCS;
    popts_spec = 0;
    ...
    if (tx_ol_req) {
      ...
      if (new_ctx) {
        ...
        ctx_txd = (volatile struct e1000_context_desc *)&txr[tx_id];
        ...
        em_set_xmit_ctx(txq, ctx_txd, tx_ol_req, hdrlen);
 ...
 *
 **********************************************************************/
uint16_t eth_em_prep_pkts(__rte_unused void *tx_queue,
...
static inline uint64_t rx_desc_error_to_pkt_flags(uint32_t rx_error) {
...
uint16_t eth_em_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_rx < nb_pkts) {
    ...
    PMD_RX_LOG(DEBUG,
               ...
               (unsigned)status, (unsigned)rte_le_to_cpu_16(rxd.length));
     ...
     *    - error flags.
     */
    pkt_len = (uint16_t)(rte_le_to_cpu_16(rxd.length) - rxq->crc_len);
    ...
    rxm->ol_flags = rxm->ol_flags | rx_desc_error_to_pkt_flags(rxd.errors);
...
static void em_tx_queue_release_mbufs(struct em_tx_queue *txq) {
  unsigned i;
...
static void em_tx_queue_release(struct em_tx_queue *txq) {
  if (txq != NULL) {
...
static void em_reset_tx_queue(struct em_tx_queue *txq) {
...
int eth_em_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
   ...
   * of E1000_ALIGN.
   */
  if (nb_desc % EM_TXD_ALIGN != 0 || (nb_desc > E1000_MAX_RING_DESC) ||
  ...
  if (tx_free_thresh == 0)
    tx_free_thresh = (uint16_t)RTE_MIN(nb_desc / 4, DEFAULT_TX_FREE_THRESH);

  tx_rs_thresh = tx_conf->tx_rs_thresh;
  if (tx_rs_thresh == 0)
    tx_rs_thresh = (uint16_t)RTE_MIN(tx_free_thresh, DEFAULT_TX_RS_THRESH);
  ...
  if ((txq->sw_ring =
           rte_zmalloc("txq->sw_ring", sizeof(txq->sw_ring[0]) * nb_desc,
                       RTE_CACHE_LINE_SIZE)) == NULL) {
...
static void em_rx_queue_release_mbufs(struct em_rx_queue *rxq) {
  unsigned i;
...
static void em_rx_queue_release(struct em_rx_queue *rxq) {
  if (rxq != NULL) {
...
static void em_reset_rx_queue(struct em_rx_queue *rxq) {
  rxq->rx_tail = 0;
  rxq->nb_rx_hold = 0;
...
int eth_em_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                          uint16_t nb_desc, unsigned int socket_id,
   ...
   * of E1000_ALIGN.
   */
  if (nb_desc % EM_RXD_ALIGN != 0 || (nb_desc > E1000_MAX_RING_DESC) ||
      (nb_desc < E1000_MIN_RING_DESC)) {
    return -EINVAL;
  }
  ...
  /* Allocate software ring. */
  if ((rxq->sw_ring =
           rte_zmalloc("rxq->sw_ring", sizeof(rxq->sw_ring[0]) * nb_desc,
                       RTE_CACHE_LINE_SIZE)) == NULL) {
...
#define EM_RXQ_SCAN_INTERVAL 4
  ...
  while ((desc < rxq->nb_rx_desc) && (rxdp->status & E1000_RXD_STAT_DD)) {
 ...
 */
static uint32_t em_rctl_bsize(__rte_unused enum e1000_mac_type hwtyp,
                              uint32_t *bufsz) {
  /*
  ...
  for (i = 0; i != sizeof(bufsz_to_rctl) / sizeof(bufsz_to_rctl[0]); i++) {
...
static int em_alloc_rx_queue_mbufs(struct em_rx_queue *rxq) {
...
int eth_em_rx_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
     ...
     * one buffer.
     */
    if (dev->data->mtu > RTE_ETHER_MTU || rctl_bsize < RTE_ETHER_MAX_LEN) {
...
static void e1000_flush_tx_ring(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues && i < E1000_I219_MAX_TX_QUEUE_NUM;
       i++) {
    txq = dev->data->tx_queues[i];
...
static void e1000_flush_rx_ring(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues && i < E1000_I219_MAX_RX_QUEUE_NUM;
       i++) {
...
void em_flush_desc_rings(struct rte_eth_dev *dev) {
  ...
  ret = rte_pci_read_config(pci_dev, &pci_cfg_status, sizeof(pci_cfg_status),
                            PCI_CFG_STATUS_REG);
  if (ret < 0) {
    PMD_DRV_LOG(ERR, "Failed to read PCI offset 0x%x", PCI_CFG_STATUS_REG);
    return;
  }
