...
bool e1000_enable_tx_pkt_filtering_generic(struct e1000_hw *hw) {
  ...
  for (i = 0; i < len; i++)
    *(buffer + i) = E1000_READ_REG_ARRAY_DWORD(hw, E1000_HOST_IF, offset + i);
 ...
 **/
s32 e1000_mng_write_cmd_header_generic(
    struct e1000_hw *hw, struct e1000_host_mng_command_header *hdr) {
 ...
 **/
s32 e1000_mng_host_if_write_generic(struct e1000_hw *hw, u8 *buffer, u16 length,
                                    u16 offset, u8 *sum) {
  u8 *tmp;
  u8 *bufptr = buffer;
  u32 data = 0;
...
s32 e1000_mng_write_dhcp_info_generic(struct e1000_hw *hw, u8 *buffer,
  ...
  ret_val = e1000_mng_host_if_write_generic(hw, buffer, length, sizeof(hdr),
                                            &(hdr.checksum));
  if (ret_val)
    return ret_val;
...
bool e1000_enable_mng_pass_thru(struct e1000_hw *hw) {
  ...
  if (hw->mac.has_fwsm) {
    ...
    if (!(factps & E1000_FACTPS_MNGCG) &&
      ...
      return true;
  } else if ((hw->mac.type == e1000_82574) || (hw->mac.type == e1000_82583)) {
    u16 data;
    s32 ret_val;
    ...
    if (!(factps & E1000_FACTPS_MNGCG) &&
        ((data & E1000_NVM_INIT_CTRL2_MNGM) == (e1000_mng_mode_pt << 13)))
      return true;
...
s32 e1000_host_interface_command(struct e1000_hw *hw, u8 *buffer, u32 length) {
  ...
  if (length == 0 || length & 0x3 || length > E1000_HI_MAX_BLOCK_BYTE_LENGTH) {
   ...
   */
  for (i = 0; i < length; i++)
    E1000_WRITE_REG_ARRAY_DWORD(hw, E1000_HOST_IF, i, *((u32 *)buffer + i));
  ...
  for (i = 0; i < length; i++)
    *((u32 *)buffer + i) = E1000_READ_REG_ARRAY_DWORD(hw, E1000_HOST_IF, i);
...
s32 e1000_load_firmware(struct e1000_hw *hw, u8 *buffer, u32 length) {
  ...
  for (i = 0; i < length; i++) {
    if (!(i % E1000_HI_FW_BLOCK_DWORD_LENGTH)) {
      ...
      hibba =
          E1000_HI_FW_BASE_ADDRESS + ((E1000_HI_FW_BLOCK_DWORD_LENGTH << 2) *
