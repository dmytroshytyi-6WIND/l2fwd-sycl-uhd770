...
STATIC void e1000_toggle_lanphypc_pch_lpt(struct e1000_hw *hw) {
  ...
  if (hw->mac.type < e1000_pch_lpt) {
    ...
    do {
      msec_delay(5);
    } while (!(E1000_READ_REG(hw, E1000_CTRL_EXT) & E1000_CTRL_EXT_LPCD) &&
             count--);
...
STATIC s32 e1000_init_phy_workarounds_pchlan(struct e1000_hw *hw) {
  ...
  case e1000_pch2lan:
    ...
    /* fall-through */
  case e1000_pchlan:
    if ((hw->mac.type == e1000_pchlan) && (fwsm & E1000_ICH_FWSM_FW_VALID))
      break;
...
out:
  ...
  if ((hw->mac.type == e1000_pch2lan) && !(fwsm & E1000_ICH_FWSM_FW_VALID)) {
    msec_delay(10);
...
STATIC s32 e1000_init_nvm_params_ich8lan(struct e1000_hw *hw) {
  ...
  if (hw->mac.type >= e1000_pch_spt) {
    ...
    nvm_size = (((E1000_READ_REG(hw, E1000_STRAP) >> 1) & 0x1F) + 1) *
               NVM_SIZE_MULTIPLIER;
     ...
     */
    nvm->flash_bank_size =
        ((sector_end_addr - sector_base_addr) << FLASH_SECTOR_ADDR_SHIFT);
...
STATIC s32 e1000_init_mac_params_ich8lan(struct e1000_hw *hw) {
  ...
  case e1000_pch_adp:
    ...
    mac->ops.update_mc_addr_list = e1000_update_mc_addr_list_pch2lan;
    /* fall-through */
  case e1000_pchlan:
...
STATIC s32 __e1000_access_emi_reg_locked(struct e1000_hw *hw, u16 address,
  ...
  if (read)
    ret_val = hw->phy.ops.read_reg_locked(hw, I82579_EMI_DATA, data);
  else
    ret_val = hw->phy.ops.write_reg_locked(hw, I82579_EMI_DATA, *data);
...
s32 e1000_set_eee_pchlan(struct e1000_hw *hw) {
  ...
  if (!dev_spec->eee_disable) {
    ...
    ret_val = e1000_read_emi_reg_locked(hw, lpa, &dev_spec->eee_lp_ability);
    if (ret_val)
      goto release;
    ...
    if (adv & dev_spec->eee_lp_ability & I82579_EEE_100_SUPPORTED) {
      ...
      if (data & NWAY_LPAR_100TX_FD_CAPS)
         ...
         * is not advertised.
         */
        dev_spec->eee_lp_ability &= ~I82579_EEE_100_SUPPORTED;
    }
  }

  if (hw->phy.type == e1000_phy_82579) {
    ret_val = e1000_read_emi_reg_locked(hw, I82579_LPI_PLL_SHUT, &data);
    if (ret_val)
      goto release;

    data &= ~I82579_LPI_100_PLL_SHUT;
    ret_val = e1000_write_emi_reg_locked(hw, I82579_LPI_PLL_SHUT, data);
  }
...
STATIC s32 e1000_k1_workaround_lpt_lp(struct e1000_hw *hw, bool link) {
  ...
  if (link && (status & E1000_STATUS_SPEED_1000)) {
    ...
    ret_val = e1000_read_kmrn_reg_locked(hw, E1000_KMRNCTRLSTA_K1_CONFIG, &reg);
    if (ret_val)
      goto release;
    ...
    ret_val = e1000_write_kmrn_reg_locked(hw, E1000_KMRNCTRLSTA_K1_CONFIG, reg);
  release:
    hw->phy.ops.release(hw);
  } else {
    ...
    if ((hw->phy.revision > 5) || !link ||
        ((status & E1000_STATUS_SPEED_100) && (status & E1000_STATUS_FD)))
...
s32 e1000_enable_ulp_lpt_lp(struct e1000_hw *hw, bool to_sx) {
  ...
  if (!to_sx) {
    ...
    while (
        !(E1000_READ_REG(hw, E1000_FEXT) & E1000_FEXT_PHY_CABLE_DISCONNECTED)) {
    ...
    DEBUGOUT2(
        ...
        (E1000_READ_REG(hw, E1000_FEXT) & E1000_FEXT_PHY_CABLE_DISCONNECTED)
            ...
            : "not",
        i * 50);
    if (!(E1000_READ_REG(hw, E1000_FEXT) & E1000_FEXT_PHY_CABLE_DISCONNECTED))
      return 0;
  }
  ...
  if ((hw->phy.type == e1000_phy_i217) && (hw->phy.revision == 6)) {
    ret_val = e1000_read_phy_reg_hv_locked(hw, HV_OEM_BITS, &oem_reg);
    if (ret_val)
      goto release;
    ...
    ret_val = e1000_write_phy_reg_hv_locked(hw, HV_OEM_BITS, phy_reg);
...
skip_smbus:
  if (!to_sx) {
     ...
     */
    ret_val =
        e1000_read_kmrn_reg_locked(hw, E1000_KMRNCTRLSTA_OP_MODES, &phy_reg);
    if (ret_val)
      goto release;
  ...
  if (ret_val)
    goto release;
  phy_reg |=
      (I218_ULP_CONFIG1_RESET_TO_SMBUS | I218_ULP_CONFIG1_DISABLE_SMB_PERST);
  if (to_sx) {
  ...
  if ((hw->phy.type == e1000_phy_i217) && (hw->phy.revision == 6) && to_sx &&
      (E1000_READ_REG(hw, E1000_STATUS) & E1000_STATUS_LU)) {
    ret_val = e1000_write_phy_reg_hv_locked(hw, HV_OEM_BITS, oem_reg);
    if (ret_val)
      goto release;
  }
...
STATIC s32 e1000_check_for_copper_link_ich8lan(struct e1000_hw *hw) {
  ...
  if ((hw->mac.type >= e1000_pch2lan) && link) {
    ...
    if (duplex == HALF_DUPLEX && speed == SPEED_10) {
      ...
      emi_val = 0;
    } else if (hw->mac.type >= e1000_pch_spt && duplex == FULL_DUPLEX &&
               speed != SPEED_1000) {
      tipg_reg |= 0xC;
      emi_val = 1;
    } else {
    ...
    if (hw->mac.type >= e1000_pch_lpt) {
      hw->phy.ops.read_reg_locked(hw, I217_PLL_CLOCK_GATE_REG, &phy_reg);
      phy_reg &= ~I217_PLL_CLOCK_GATE_MASK;
      if (speed == SPEED_100 || speed == SPEED_10)
        phy_reg |= 0x3E8;
      else
        phy_reg |= 0xFA;
      hw->phy.ops.write_reg_locked(hw, I217_PLL_CLOCK_GATE_REG, phy_reg);

      if (speed == SPEED_1000) {
        hw->phy.ops.read_reg_locked(hw, HV_PM_CTRL, &phy_reg);

        phy_reg |= HV_PM_CTRL_K1_CLK_REQ;

        hw->phy.ops.write_reg_locked(hw, HV_PM_CTRL, phy_reg);
      }
    }
    hw->phy.ops.release(hw);
    ...
    if (hw->mac.type >= e1000_pch_spt) {
      ...
      if (speed == SPEED_1000) {
        ...
        ret_val = hw->phy.ops.read_reg_locked(hw, PHY_REG(776, 20), &data);
        if (ret_val) {
          hw->phy.ops.release(hw);
          return ret_val;
        }
        ...
        if (ptr_gap < 0x18) {
          data &= ~(0x3FF << 2);
          data |= (0x18 << 2);
          ret_val = hw->phy.ops.write_reg_locked(hw, PHY_REG(776, 20), data);
        }
        hw->phy.ops.release(hw);
        if (ret_val)
          return ret_val;
        ...
        ret_val = hw->phy.ops.write_reg_locked(hw, PHY_REG(776, 20), 0xC023);
        hw->phy.ops.release(hw);
        if (ret_val)
          return ret_val;
      }
  ...
  case e1000_pchlan:
    ...
    if ((E1000_READ_REG(hw, E1000_STATUS) & E1000_STATUS_FD) != E1000_STATUS_FD)
...
STATIC s32 e1000_acquire_swflag_ich8lan(struct e1000_hw *hw) {
  ...
  if (!timeout) {
    DEBUGOUT2("Failed to acquire the semaphore, FW or HW has it: FWSM=0x%8.8x "
              "EXTCNF_CTRL=0x%8.8x)\n",
...
STATIC int e1000_rar_set_pch_lpt(struct e1000_hw *hw, u8 *addr, u32 index) {
  ...
  if (index < hw->mac.rar_entry_count) {
    wlock_mac = E1000_READ_REG(hw, E1000_FWSM) & E1000_FWSM_WLOCK_MAC_MASK;
...
STATIC void e1000_update_mc_addr_list_pch2lan(struct e1000_hw *hw,
  ...
  for (i = 0; i < hw->mac.mta_reg_count; i++) {
    ...
    hw->phy.ops.write_reg_page(hw, (BM_MTA(i) + 1),
                               (u16)((hw->mac.mta_shadow[i] >> 16) & 0xFFFF));
  }
...
STATIC s32 e1000_write_smbus_addr(struct e1000_hw *hw) {
  ...
  u32 freq = (strap & E1000_STRAP_SMT_FREQ_MASK) >> E1000_STRAP_SMT_FREQ_SHIFT;
  s32 ret_val;
  ...
  if (hw->phy.type == e1000_phy_i217) {
    ...
    if (freq--) {
      ...
      phy_data |= (freq & (1 << 0)) << HV_SMB_ADDR_FREQ_LOW_SHIFT;
      phy_data |= (freq & (1 << 1)) << (HV_SMB_ADDR_FREQ_HIGH_SHIFT - 1);
    } else {
...
STATIC s32 e1000_sw_lcd_config_ich8lan(struct e1000_hw *hw) {
  ...
  if (((hw->mac.type == e1000_pchlan) &&
    ...
    ret_val = e1000_write_phy_reg_hv_locked(hw, HV_LED_CONFIG, (u16)data);
    if (ret_val)
      goto release;
  }
  ...
  for (i = 0; i < cnf_size; i++) {
    ret_val = hw->nvm.ops.read(hw, (word_addr + i * 2), 1, &reg_data);
    if (ret_val)
      goto release;

    ret_val = hw->nvm.ops.read(hw, (word_addr + i * 2 + 1), 1, &reg_addr);
    if (ret_val)
      goto release;
    ...
    ret_val = phy->ops.write_reg_locked(hw, (u32)reg_addr, reg_data);
    if (ret_val)
      goto release;
  }
...
STATIC s32 e1000_k1_gig_workaround_hv(struct e1000_hw *hw, bool link) {
  ...
  if (link) {
    if (hw->phy.type == e1000_phy_82578) {
      ret_val = hw->phy.ops.read_reg_locked(hw, BM_CS_STATUS, &status_reg);
      if (ret_val)
        goto release;

      status_reg &= (BM_CS_STATUS_LINK_UP | BM_CS_STATUS_RESOLVED |
                     BM_CS_STATUS_SPEED_MASK);

      if (status_reg == (BM_CS_STATUS_LINK_UP | BM_CS_STATUS_RESOLVED |
                         BM_CS_STATUS_SPEED_1000))
        k1_enable = false;
    }

    if (hw->phy.type == e1000_phy_82577) {
      ret_val = hw->phy.ops.read_reg_locked(hw, HV_M_STATUS, &status_reg);
      if (ret_val)
        goto release;

      status_reg &= (HV_M_STATUS_LINK_UP | HV_M_STATUS_AUTONEG_COMPLETE |
    ...
    ret_val = hw->phy.ops.write_reg_locked(hw, PHY_REG(770, 19), 0x0100);
    if (ret_val)
      goto release;

  } else {
    /* Link stall fix for link down */
    ret_val = hw->phy.ops.write_reg_locked(hw, PHY_REG(770, 19), 0x4100);
    if (ret_val)
      goto release;
  }
...
s32 e1000_configure_k1_ich8lan(struct e1000_hw *hw, bool k1_enable) {
  ...
  ret_val =
      e1000_read_kmrn_reg_locked(hw, E1000_KMRNCTRLSTA_K1_CONFIG, &kmrn_reg);
  if (ret_val)
    return ret_val;
  ...
  ret_val =
      e1000_write_kmrn_reg_locked(hw, E1000_KMRNCTRLSTA_K1_CONFIG, kmrn_reg);
  if (ret_val)
    return ret_val;
...
STATIC s32 e1000_oem_bits_config_ich8lan(struct e1000_hw *hw, bool d0_state) {
  ...
  if (d0_state) {
    ...
    if (mac_reg & (E1000_PHY_CTRL_D0A_LPLU | E1000_PHY_CTRL_NOND0A_LPLU))
      oem_reg |= HV_OEM_BITS_LPLU;
  }
...
STATIC s32 e1000_hv_phy_workarounds_ich8lan(struct e1000_hw *hw) {
  ...
  if (hw->phy.type == e1000_phy_82578) {
    ...
    if (hw->phy.revision < 2) {
      ...
      ret_val = hw->phy.ops.write_reg(hw, PHY_CONTROL, 0x3140);
    }
  }
  ...
  if (ret_val)
    goto release;
  ret_val =
      hw->phy.ops.write_reg_locked(hw, BM_PORT_GEN_CFG, phy_data & 0x00FF);
  if (ret_val)
    goto release;
...
void e1000_copy_rx_addrs_to_phy_ich8lan(struct e1000_hw *hw) {
  ...
  for (i = 0; i < (hw->mac.rar_entry_count); i++) {
    ...
    hw->phy.ops.write_reg_page(hw, BM_RAR_L(i), (u16)(mac_reg & 0xFFFF));
    hw->phy.ops.write_reg_page(hw, BM_RAR_M(i),
                               (u16)((mac_reg >> 16) & 0xFFFF));

    mac_reg = E1000_READ_REG(hw, E1000_RAH(i));
    hw->phy.ops.write_reg_page(hw, BM_RAR_H(i), (u16)(mac_reg & 0xFFFF));
...
s32 e1000_lv_jumbo_workaround_ich8lan(struct e1000_hw *hw, bool enable) {
  ...
  if (enable) {
    ...
    for (i = 0; i < hw->mac.rar_entry_count; i++) {
      ...
      E1000_WRITE_REG(hw, E1000_PCH_RAICC(i), e1000_calc_rx_da_crc(mac_addr));
    }
    ...
    ret_val =
        e1000_read_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_CTRL_OFFSET, &data);
    if (ret_val)
      return ret_val;
    ret_val = e1000_write_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_CTRL_OFFSET,
                                           data | (1 << 0));
    if (ret_val)
      return ret_val;
    ret_val = e1000_read_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_HD_CTRL, &data);
    if (ret_val)
      return ret_val;
    data &= ~(0xF << 8);
    data |= (0xB << 8);
    ret_val = e1000_write_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_HD_CTRL, data);
    if (ret_val)
      return ret_val;
  ...
  return hw->phy.ops.write_reg(hw, PHY_REG(769, 20), phy_reg & ~(1 << 14));
}
...
STATIC s32 e1000_k1_workaround_lv(struct e1000_hw *hw) {
  ...
  if ((status_reg & (HV_M_STATUS_LINK_UP | HV_M_STATUS_AUTONEG_COMPLETE)) ==
    ...
    if (status_reg & (HV_M_STATUS_SPEED_1000 | HV_M_STATUS_SPEED_100)) {
      u16 pm_phy_reg;

      /* LV 1G/100 Packet drop issue wa  */
      ret_val = hw->phy.ops.read_reg(hw, HV_PM_CTRL, &pm_phy_reg);
      if (ret_val)
        return ret_val;
      pm_phy_reg &= ~HV_PM_CTRL_K1_ENABLE;
      ret_val = hw->phy.ops.write_reg(hw, HV_PM_CTRL, pm_phy_reg);
      if (ret_val)
        return ret_val;
    } else {
      u32 mac_reg;
...
STATIC s32 e1000_post_phy_reset_ich8lan(struct e1000_hw *hw) {
  ...
  if (hw->mac.type == e1000_pch2lan) {
    ...
    if (ret_val)
      return ret_val;
    ret_val = e1000_write_emi_reg_locked(hw, I82579_LPI_UPDATE_TIMER, 0x1387);
    hw->phy.ops.release(hw);
  }
...
STATIC s32 e1000_valid_nvm_bank_detect_ich8lan(struct e1000_hw *hw, u32 *bank) {
  ...
  case e1000_pch_adp:
    ...
    *bank = 0;
    ...
    if ((sig_byte & E1000_ICH_NVM_VALID_SIG_MASK) == E1000_ICH_NVM_SIG_VALUE) {
      *bank = 0;
      return E1000_SUCCESS;
    }

    /* Check bank 1 */
    ret_val = e1000_read_flash_dword_ich8lan(hw, act_offset + bank1_offset,
                                             &nvm_dword);
    if (ret_val)
      return ret_val;
    sig_byte = (u8)((nvm_dword & 0xFF00) >> 8);
  ...
  case e1000_ich9lan:
    ...
    DEBUGOUT("Unable to determine valid NVM bank via EEC - reading flash "
             "signature\n");
    /* fall-thru */
  default:
    /* set bank to 0 in case flash read fails */
    *bank = 0;
    ...
    /* Check bank 1 */
    ret_val =
        e1000_read_flash_byte_ich8lan(hw, act_offset + bank1_offset, &sig_byte);
    if (ret_val)
      return ret_val;
    if ((sig_byte & E1000_ICH_NVM_VALID_SIG_MASK) == E1000_ICH_NVM_SIG_VALUE) {
      *bank = 1;
  ...
  for (i = 0; i < words; i += 2) {
    if (words - i == 1) {
      if (dev_spec->shadow_ram[offset + i].modified) {
        ...
        offset_to_read = act_offset + i - ((act_offset + i) % 2);
        ret_val = e1000_read_flash_dword_ich8lan(hw, offset_to_read, &dword);
        if (ret_val)
          break;
        if ((act_offset + i) % 2 == 0)
      ...
      if (dev_spec->shadow_ram[offset + i + 1].modified)
        data[i + 1] = dev_spec->shadow_ram[offset + i + 1].value;
      else
...
STATIC s32 e1000_read_nvm_ich8lan(struct e1000_hw *hw, u16 offset, u16 words,
  ...
  for (i = 0; i < words; i++) {
    if (dev_spec->shadow_ram[offset + i].modified) {
      ...
      ret_val = e1000_read_flash_word_ich8lan(hw, act_offset + i, &word);
      if (ret_val)
        break;
      data[i] = word;
    }
...
STATIC s32 e1000_flash_cycle_init_ich8lan(struct e1000_hw *hw) {
  ...
  if (hw->mac.type >= e1000_pch_spt)
    E1000_WRITE_FLASH_REG(hw, ICH_FLASH_HSFSTS, hsfsts.regval & 0xFFFF);
  else
...
STATIC s32 e1000_flash_cycle_ich8lan(struct e1000_hw *hw, u32 timeout) {
  ...
  if (hw->mac.type >= e1000_pch_spt)
    E1000_WRITE_FLASH_REG(hw, ICH_FLASH_HSFSTS, hsflctl.regval << 16);
  else
...
STATIC s32 e1000_read_flash_data_ich8lan(struct e1000_hw *hw, u32 offset,
  ...
  if (size < 1 || size > 2 || offset > ICH_FLASH_LINEAR_ADDR_MASK)
    return -E1000_ERR_NVM;
  flash_linear_addr =
      ((ICH_FLASH_LINEAR_ADDR_MASK & offset) + hw->nvm.flash_base_addr);
...
STATIC s32 e1000_update_nvm_checksum_spt(struct e1000_hw *hw) {
  ...
  for (i = 0; i < E1000_SHADOW_RAM_WORDS; i += 2) {
    ...
    if (dev_spec->shadow_ram[i + 1].modified) {
      dword &= 0x0000ffff;
      dword |= ((dev_spec->shadow_ram[i + 1].value & 0xffff) << 16);
    }
    if (ret_val)
      break;
...
STATIC s32 e1000_update_nvm_checksum_ich8lan(struct e1000_hw *hw) {
  ...
  for (i = 0; i < E1000_SHADOW_RAM_WORDS; i++) {
    ...
    ret_val = e1000_retry_write_flash_byte_ich8lan(hw, act_offset, (u8)data);
    if (ret_val)
      break;

    usec_delay(100);
    ret_val = e1000_retry_write_flash_byte_ich8lan(hw, act_offset + 1,
                                                   (u8)(data >> 8));
    if (ret_val)
      break;
  }
...
STATIC s32 e1000_init_hw_ich8lan(struct e1000_hw *hw) {
  ...
  txdctl =
      ((txdctl & ~E1000_TXDCTL_PTHRESH) | E1000_TXDCTL_MAX_TX_DESC_PREFETCH);
...
STATIC s32 e1000_setup_link_ich8lan(struct e1000_hw *hw) {
  ...
  if ((hw->phy.type == e1000_phy_82578) || (hw->phy.type == e1000_phy_82579) ||
      (hw->phy.type == e1000_phy_i217) || (hw->phy.type == e1000_phy_82577)) {
    E1000_WRITE_REG(hw, E1000_FCRTV_PCH, hw->fc.refresh_time);

    ret_val = hw->phy.ops.write_reg(hw, PHY_REG(BM_PORT_CTRL_PAGE, 27),
                                    hw->fc.pause_time);
    if (ret_val)
      return ret_val;
  }
...
STATIC s32 e1000_setup_copper_link_ich8lan(struct e1000_hw *hw) {
   ...
   */
  ret_val =
      e1000_write_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_TIMEOUTS, 0xFFFF);
  if (ret_val)
    return ret_val;
  ret_val = e1000_read_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_INBAND_PARAM,
                                        &reg_data);
  if (ret_val)
    return ret_val;
  reg_data |= 0x3F;
  ret_val = e1000_write_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_INBAND_PARAM,
                                         reg_data);
  if (ret_val)
    return ret_val;
  ...
  case e1000_phy_82579:
    ...
    break;
  case e1000_phy_ife:
    ret_val = hw->phy.ops.read_reg(hw, IFE_PHY_MDIX_CONTROL, &reg_data);
    if (ret_val)
      return ret_val;
    ...
    ret_val = hw->phy.ops.write_reg(hw, IFE_PHY_MDIX_CONTROL, reg_data);
    if (ret_val)
      return ret_val;
    break;
  default:
...
STATIC s32 e1000_get_link_up_info_ich8lan(struct e1000_hw *hw, u16 *speed,
  ...
  if ((hw->mac.type == e1000_ich8lan) && (hw->phy.type == e1000_phy_igp_3) &&
      (*speed == SPEED_1000)) {
...
STATIC s32 e1000_kmrn_lock_loss_workaround_ich8lan(struct e1000_hw *hw) {
  ...
  phy_ctrl = E1000_READ_REG(hw, E1000_PHY_CTRL);
  phy_ctrl |= (E1000_PHY_CTRL_GBE_DISABLE | E1000_PHY_CTRL_NOND0A_GBE_DISABLE);
  E1000_WRITE_REG(hw, E1000_PHY_CTRL, phy_ctrl);
...
void e1000_igp3_phy_powerdown_workaround_ich8lan(struct e1000_hw *hw) {
  ...
  do {
    ...
    hw->phy.ops.write_reg(hw, IGP3_VR_CTRL, data | IGP3_VR_CTRL_MODE_SHUTDOWN);
...
void e1000_gig_downshift_workaround_ich8lan(struct e1000_hw *hw) {
  ...
  ret_val =
      e1000_read_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_DIAG_OFFSET, &reg_data);
  if (ret_val)
    return;
  reg_data |= E1000_KMRNCTRLSTA_DIAG_NELPBK;
  ret_val =
      e1000_write_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_DIAG_OFFSET, reg_data);
  if (ret_val)
    return;
  reg_data &= ~E1000_KMRNCTRLSTA_DIAG_NELPBK;
  e1000_write_kmrn_reg_generic(hw, E1000_KMRNCTRLSTA_DIAG_OFFSET, reg_data);
...
void e1000_suspend_workarounds_ich8lan(struct e1000_hw *hw) {
  ...
  if (hw->phy.type == e1000_phy_i217) {
    ...
    if (!dev_spec->eee_disable) {
      ...
      ret_val =
          e1000_read_emi_reg_locked(hw, I217_EEE_ADVERTISEMENT, &eee_advert);
      if (ret_val)
        goto release;
      ...
      if ((eee_advert & I82579_EEE_100_SUPPORTED) &&
        ...
        phy_ctrl &= ~(E1000_PHY_CTRL_D0A_LPLU | E1000_PHY_CTRL_NOND0A_LPLU);
    ...
    if (!(E1000_READ_REG(hw, E1000_FWSM) & E1000_ICH_FWSM_FW_VALID)) {
      ...
      hw->phy.ops.read_reg_locked(hw, I217_PROXY_CTRL, &phy_reg);
      phy_reg |= I217_PROXY_CTRL_AUTO_DISABLE;
      hw->phy.ops.write_reg_locked(hw, I217_PROXY_CTRL, phy_reg);
...
u32 e1000_resume_workarounds_pchlan(struct e1000_hw *hw) {
  ...
  if (hw->phy.type == e1000_phy_i217) {
    ...
    if (!(E1000_READ_REG(hw, E1000_FWSM) & E1000_ICH_FWSM_FW_VALID)) {
       ...
       * is present
       */
      ret_val = hw->phy.ops.read_reg_locked(hw, I217_MEMPWR, &phy_reg);
      if (ret_val)
        goto release;
      phy_reg |= I217_MEMPWR_DISABLE_SMB_RELEASE;
      hw->phy.ops.write_reg_locked(hw, I217_MEMPWR, phy_reg);
    ...
    /* Enable reset on MTA */
    ret_val = hw->phy.ops.read_reg_locked(hw, I217_CGFREG, &phy_reg);
    if (ret_val)
      goto release;
    phy_reg &= ~I217_CGFREG_ENABLE_MTA_RESET;
    hw->phy.ops.write_reg_locked(hw, I217_CGFREG, phy_reg);
...
STATIC s32 e1000_cleanup_led_ich8lan(struct e1000_hw *hw) {
  ...
  if (hw->phy.type == e1000_phy_ife)
    return hw->phy.ops.write_reg(hw, IFE_PHY_SPECIAL_CONTROL_LED, 0);
...
STATIC s32 e1000_led_on_ich8lan(struct e1000_hw *hw) {
  ...
  if (hw->phy.type == e1000_phy_ife)
    return hw->phy.ops.write_reg(
        hw, IFE_PHY_SPECIAL_CONTROL_LED,
...
STATIC s32 e1000_cleanup_led_pchlan(struct e1000_hw *hw) {
  ...
  return hw->phy.ops.write_reg(hw, HV_LED_CONFIG, (u16)hw->mac.ledctl_default);
}
...
STATIC void e1000_power_down_phy_copper_ich8lan(struct e1000_hw *hw) {
  ...
  if (!(hw->mac.ops.check_mng_mode(hw) || hw->phy.ops.check_reset_block(hw)))
 ...
 **/
s32 e1000_configure_k0s_lpt(struct e1000_hw *hw, u8 entry_latency,
                            u8 min_time) {
  s32 ret_val;
  u16 kmrn_reg = 0;
  ...
  ret_val =
      e1000_read_kmrn_reg_locked(hw, E1000_KMRNCTRLSTA_K0S_CTRL, &kmrn_reg);
  if (ret_val)
    return ret_val;
  ...
  ret_val =
      e1000_write_kmrn_reg_locked(hw, E1000_KMRNCTRLSTA_K0S_CTRL, kmrn_reg);
  if (ret_val)
    return ret_val;
