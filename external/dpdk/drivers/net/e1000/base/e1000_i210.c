...
s32 e1000_read_nvm_srrd_i210(struct e1000_hw *hw, u16 offset, u16 words,
  ...
  for (i = 0; i < words; i += E1000_EERD_EEWR_MAX_COUNT) {
    ...
    if (hw->nvm.ops.acquire(hw) == E1000_SUCCESS) {
      status = e1000_read_nvm_eerd(hw, offset, count, data + i);
...
s32 e1000_write_nvm_srwr_i210(struct e1000_hw *hw, u16 offset, u16 words,
  ...
  for (i = 0; i < words; i += E1000_EERD_EEWR_MAX_COUNT) {
    count = (words - i) / E1000_EERD_EEWR_MAX_COUNT > 0
                ...
                : (words - i);
    if (hw->nvm.ops.acquire(hw) == E1000_SUCCESS) {
      status = e1000_write_nvm_srwr(hw, offset, count, data + i);
      hw->nvm.ops.release(hw);
    } else {
      status = E1000_ERR_SWFW_SYNC;
    }
...
STATIC s32 e1000_write_nvm_srwr(struct e1000_hw *hw, u16 offset, u16 words,
  ...
  for (i = 0; i < words; i++) {
    ...
    for (k = 0; k < attempts; k++) {
      if (E1000_NVM_RW_REG_DONE & E1000_READ_REG(hw, E1000_SRWR)) {
 ...
 *  stored in OTP.
 **/
STATIC s32 e1000_read_invm_word_i210(struct e1000_hw *hw, u8 address,
                                     u16 *data) {
    ...
    if (record_type == E1000_INVM_WORD_AUTOLOAD_STRUCTURE) {
      ...
      if (word_address == address) {
        *data = INVM_DWORD_TO_WORD_DATA(invm_dword);
        DEBUGOUT2("Read INVM Word 0x%02x = %x", address, *data);
...
STATIC s32 e1000_read_invm_i210(struct e1000_hw *hw, u16 offset,
  ...
  case NVM_MAC_ADDR:
    ...
    ret_val |= e1000_read_invm_word_i210(hw, (u8)offset + 1, &data[1]);
    ret_val |= e1000_read_invm_word_i210(hw, (u8)offset + 2, &data[2]);
    if (ret_val != E1000_SUCCESS)
...
s32 e1000_read_invm_version(struct e1000_hw *hw,
  ...
  for (i = 1; i < invm_blocks; i++) {
    ...
    else if ((i == 1) && ((*record & E1000_INVM_VER_FIELD_TWO) == 0)) {
     ...
     */
              ...
              ((*record & 0x3) == 0)) ||
             (((*record & 0x3) != 0) && (i != 1))) {
      version = (*next_record & E1000_INVM_VER_FIELD_TWO) >> 13;
      status = E1000_SUCCESS;
      break;
    }
    /*
  ...
  if (status == E1000_SUCCESS) {
    invm_ver->invm_major =
        (version & E1000_INVM_MAJOR_MASK) >> E1000_INVM_MAJOR_SHIFT;
  ...
  for (i = 1; i < invm_blocks; i++) {
    ...
    /* Check if we have image type in first location used */
             ...
             ((((*record & 0x3) != 0) && (i != 1)))) {
      invm_ver->invm_img_type = (*next_record & E1000_INVM_IMGTYPE_FIELD) >> 23;
      status = E1000_SUCCESS;
      break;
    }
  }
...
s32 e1000_update_nvm_checksum_i210(struct e1000_hw *hw) {
  ...
  if (hw->nvm.ops.acquire(hw) == E1000_SUCCESS) {
    ...
    ret_val = e1000_write_nvm_srwr(hw, NVM_CHECKSUM_REG, 1, &checksum);
    if (ret_val != E1000_SUCCESS) {
      hw->nvm.ops.release(hw);
...
s32 e1000_init_hw_i210(struct e1000_hw *hw) {
  ...
  if ((hw->mac.type >= e1000_i210) && !(e1000_get_flash_presence_i210(hw))) {
