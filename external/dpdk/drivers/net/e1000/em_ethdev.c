...
static int eth_em_link_update(struct rte_eth_dev *dev, int wait_to_complete);
...
static int eth_em_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                  int on);
...
static int eth_em_rx_queue_intr_enable(struct rte_eth_dev *dev,
                                       uint16_t queue_id);
static int eth_em_rx_queue_intr_disable(struct rte_eth_dev *dev,
                                        uint16_t queue_id);
 ...
 **/
static bool eth_em_dev_is_ich8(struct e1000_hw *hw) {
...
static int eth_em_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  if (rte_eal_process_type() != RTE_PROC_PRIMARY) {
    if (eth_dev->data->scattered_rx)
      eth_dev->rx_pkt_burst = (eth_rx_burst_t)&eth_em_recv_scattered_pkts;
    return 0;
  }
  ...
  rte_intr_callback_register(intr_handle, eth_em_interrupt_handler, eth_dev);

  return 0;
}

static int eth_em_dev_uninit(struct rte_eth_dev *eth_dev) {
  PMD_INIT_FUNC_TRACE();
...
static void em_set_pba(struct e1000_hw *hw) {
  uint32_t pba;
...
static void eth_em_rxtx_control(struct rte_eth_dev *dev, bool enable) {
...
static int eth_em_start(struct rte_eth_dev *dev) {
  ...
  if (rte_intr_allow_others(intr_handle)) {
    ...
    rte_intr_callback_unregister(intr_handle, eth_em_interrupt_handler,
                                 (void *)dev);
    if (dev->data->dev_conf.intr_conf.lsc != 0)
...
static int eth_em_stop(struct rte_eth_dev *dev) {
  ...
  if (!rte_intr_allow_others(intr_handle))
    ...
    rte_intr_callback_register(intr_handle, eth_em_interrupt_handler,
                               (void *)dev);
...
static int eth_em_close(struct rte_eth_dev *dev) {
  ...
  rte_intr_callback_unregister(intr_handle, eth_em_interrupt_handler, dev);
...
static int em_hardware_init(struct e1000_hw *hw) {
  ...
  } else if (hw->mac.type == e1000_pch_lpt || hw->mac.type == e1000_pch_spt ||
...
static int eth_em_infos_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa =
      em_get_rx_port_offloads_capa() | dev_info->rx_queue_offload_capa;
  dev_info->tx_queue_offload_capa = em_get_tx_queue_offloads_capa(dev);
  dev_info->tx_offload_capa =
      em_get_tx_port_offloads_capa(dev) | dev_info->tx_queue_offload_capa;
...
static int eth_em_interrupt_setup(struct rte_eth_dev *dev) {
  ...
  E1000_WRITE_REG(hw, E1000_IMS, regval | E1000_ICR_LSC | E1000_ICR_OTHER);
  return 0;
}
...
RTE_PMD_REGISTER_KMOD_DEP(net_e1000_em,
