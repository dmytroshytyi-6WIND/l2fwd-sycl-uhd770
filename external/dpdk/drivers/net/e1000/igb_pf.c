...
static inline int igb_vf_perm_addr_gen(struct rte_eth_dev *dev,
                                       uint16_t vf_num) {
...
static inline int igb_mb_intr_setup(struct rte_eth_dev *dev) {
...
static void set_rx_mode(struct rte_eth_dev *dev) {
  ...
  if ((hw->mac.type == e1000_82576) || (hw->mac.type == e1000_i350)) {
    vmolr |= E1000_READ_REG(hw, E1000_VMOLR(vfn)) &
             ~(E1000_VMOLR_MPME | E1000_VMOLR_ROMPE | E1000_VMOLR_ROPE);
...
static inline void igb_vf_reset_event(struct rte_eth_dev *dev, uint16_t vf) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_vf_info *vfinfo =
      *(E1000_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
  uint32_t vmolr = E1000_READ_REG(hw, E1000_VMOLR(vf));

  vmolr |= (E1000_VMOLR_ROPE | E1000_VMOLR_ROMPE | E1000_VMOLR_BAM |
            E1000_VMOLR_AUPE);
...
static inline void igb_vf_reset_msg(struct rte_eth_dev *dev, uint16_t vf) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint32_t reg;
...
static int igb_vf_reset(struct rte_eth_dev *dev, uint16_t vf,
                        uint32_t *msgbuf) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_vf_info *vfinfo =
      *(E1000_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
...
static int igb_vf_set_mac_addr(struct rte_eth_dev *dev, uint32_t vf,
                               uint32_t *msgbuf) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_vf_info *vfinfo =
      *(E1000_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
...
static int igb_vf_set_multicast(struct rte_eth_dev *dev,
                                __rte_unused uint32_t vf, uint32_t *msgbuf) {
  int i;
  uint32_t vector_bit;
  uint32_t vector_reg;
  uint32_t mta_reg;
  int entries = (msgbuf[0] & E1000_VT_MSGINFO_MASK) >> E1000_VT_MSGINFO_SHIFT;
...
static int igb_vf_set_vlan(struct rte_eth_dev *dev, uint32_t vf,
                           uint32_t *msgbuf) {
  int add, vid;
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_vf_info *vfinfo =
  ...
  add = (msgbuf[0] & E1000_VT_MSGINFO_MASK) >> E1000_VT_MSGINFO_SHIFT;
  ...
  vid_idx = (uint32_t)((vid >> E1000_VFTA_ENTRY_SHIFT) & E1000_VFTA_ENTRY_MASK);
...
static int igb_vf_set_rlpml(struct rte_eth_dev *dev, uint32_t vf,
                            uint32_t *msgbuf) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int igb_rcv_msg_from_vf(struct rte_eth_dev *dev, uint16_t vf) {
  ...
  case E1000_VF_SET_VLAN:
    ...
    break;
  default:
    PMD_INIT_LOG(DEBUG, "Unhandled Msg %8.8x", (unsigned)msgbuf[0]);
...
static inline void igb_rcv_ack_from_vf(struct rte_eth_dev *dev, uint16_t vf) {
