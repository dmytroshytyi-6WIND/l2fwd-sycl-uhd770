...
#define IGB_TX_OFFLOAD_MASK                                                    \
  (RTE_MBUF_F_TX_OUTER_IPV6 | RTE_MBUF_F_TX_OUTER_IPV4 | RTE_MBUF_F_TX_IPV6 |  \
   ...
   RTE_MBUF_F_TX_L4_MASK | RTE_MBUF_F_TX_TCP_SEG | IGB_TX_IEEE1588_TMST)
...
static inline uint64_t check_tso_para(uint64_t ol_req,
                                      union igb_tx_offload ol_para) {
  if (!(ol_req & RTE_MBUF_F_TX_TCP_SEG))
    return ol_req;
  if ((ol_para.tso_segsz > IGB_TSO_MAX_MSS) ||
      (ol_para.l2_len + ol_para.l3_len + ol_para.l4_len > IGB_TSO_MAX_HDRLEN)) {
...
igbe_set_xmit_ctx(struct igb_tx_queue *txq,
  ...
  if (ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    if (ol_flags & RTE_MBUF_F_TX_IP_CKSUM)
      type_tucmd_mlhl = E1000_ADVTXD_TUCMD_IPV4 | E1000_ADVTXD_TUCMD_L4T_TCP |
    ...
    case RTE_MBUF_F_TX_UDP_CKSUM:
      type_tucmd_mlhl |= E1000_ADVTXD_TUCMD_L4T_UDP | E1000_ADVTXD_DTYP_CTXT |
                         E1000_ADVTXD_DCMD_DEXT;
      mss_l4len_idx |= sizeof(struct rte_udp_hdr) << E1000_ADVTXD_L4LEN_SHIFT;
      break;
    case RTE_MBUF_F_TX_TCP_CKSUM:
      type_tucmd_mlhl |= E1000_ADVTXD_TUCMD_L4T_TCP | E1000_ADVTXD_DTYP_CTXT |
                         E1000_ADVTXD_DCMD_DEXT;
      mss_l4len_idx |= sizeof(struct rte_tcp_hdr) << E1000_ADVTXD_L4LEN_SHIFT;
      break;
    case RTE_MBUF_F_TX_SCTP_CKSUM:
      type_tucmd_mlhl |= E1000_ADVTXD_TUCMD_L4T_SCTP | E1000_ADVTXD_DTYP_CTXT |
                         E1000_ADVTXD_DCMD_DEXT;
      mss_l4len_idx |= sizeof(struct rte_sctp_hdr) << E1000_ADVTXD_L4LEN_SHIFT;
      break;
    default:
      type_tucmd_mlhl |= E1000_ADVTXD_TUCMD_L4T_RSV | E1000_ADVTXD_DTYP_CTXT |
                         E1000_ADVTXD_DCMD_DEXT;
      break;
    }
  }
 ...
 */
static inline uint32_t what_advctx_update(struct igb_tx_queue *txq,
                                          uint64_t flags,
                                          union igb_tx_offload tx_offload) {
...
uint16_t eth_igb_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                           uint16_t nb_pkts) {
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
     ...
     */
    cmd_type_len =
        txq->txd_type | E1000_ADVTXD_DCMD_IFCS | E1000_ADVTXD_DCMD_DEXT;
 ...
 *
 **********************************************************************/
uint16_t eth_igb_prep_pkts(__rte_unused void *tx_queue,
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (m->ol_flags & RTE_MBUF_F_TX_TCP_SEG)
      if ((m->tso_segsz > IGB_TSO_MAX_MSS) ||
          (m->l2_len + m->l3_len + m->l4_len > IGB_TSO_MAX_HDRLEN)) {
        rte_errno = EINVAL;
        return i;
      }
...
static inline uint32_t igb_rxd_pkt_info_to_pkt_type(uint16_t pkt_info) {
  static const uint32_t ptype_table[IGB_PACKET_TYPE_MAX] __rte_cache_aligned = {
      [IGB_PACKET_TYPE_IPV4] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4,
      [IGB_PACKET_TYPE_IPV4_EXT] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT,
      [IGB_PACKET_TYPE_IPV6] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6,
      [IGB_PACKET_TYPE_IPV4_IPV6] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 |
                                    RTE_PTYPE_TUNNEL_IP |
                                    RTE_PTYPE_INNER_L3_IPV6,
      [IGB_PACKET_TYPE_IPV6_EXT] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT,
      [IGB_PACKET_TYPE_IPV4_IPV6_EXT] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 |
                                        RTE_PTYPE_TUNNEL_IP |
                                        RTE_PTYPE_INNER_L3_IPV6_EXT,
      [IGB_PACKET_TYPE_IPV4_TCP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP,
      [IGB_PACKET_TYPE_IPV6_TCP] =
...
rx_desc_hlen_type_rss_to_pkt_flags(struct igb_rx_queue *rxq,
                                   uint32_t hl_tp_rs) {
...
static inline uint64_t rx_desc_error_to_pkt_flags(uint32_t rx_status) {
  ...
  return error_to_pkt_flags_map[(rx_status >> E1000_RXD_ERR_CKSUM_BIT) &
                                E1000_RXD_ERR_CKSUM_MSK];
}

uint16_t eth_igb_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                           uint16_t nb_pkts) {
  ...
  while (nb_rx < nb_pkts) {
    ...
    rxm->packet_type =
        igb_rxd_pkt_info_to_pkt_type(rxd.wb.lower.lo_dword.hs_rss.pkt_info);
...
uint16_t eth_igb_recv_scattered_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                                     uint16_t nb_pkts) {
  struct igb_rx_queue *rxq;
  volatile union e1000_adv_rx_desc *rx_ring;
  volatile union e1000_adv_rx_desc *rxdp;
  ...
  next_desc:
    ...
    if (unlikely(rxq->crc_len > 0)) {
      ...
      if (data_len <= RTE_ETHER_CRC_LEN) {
        ...
        first_seg->nb_segs--;
        last_seg->data_len =
            (uint16_t)(last_seg->data_len - (RTE_ETHER_CRC_LEN - data_len));
        last_seg->next = NULL;
      } else
    ...
    if ((staterr & rte_cpu_to_le_32(E1000_RXDEXT_STATERR_LB)) &&
        (rxq->flags & IGB_RXQ_FLAG_LB_BSWAP_VLAN)) {
      first_seg->vlan_tci = rte_be_to_cpu_16(rxd.wb.upper.vlan);
    } else {
...
static void igb_tx_queue_release_mbufs(struct igb_tx_queue *txq) {
  unsigned i;
...
static void igb_tx_queue_release(struct igb_tx_queue *txq) {
  if (txq != NULL) {
...
static int igb_tx_done_cleanup(struct igb_tx_queue *txq, uint32_t free_cnt) {
  struct igb_tx_entry *sw_ring;
  volatile union e1000_adv_tx_desc *txr;
  ...
  while (1) {
    ...
    if (sw_ring[tx_last].mbuf) {
      if (txr[tx_last].wb.status & E1000_TXD_STAT_DD) {
...
static void igb_reset_tx_queue_stat(struct igb_tx_queue *txq) {
  txq->tx_head = 0;
  txq->tx_tail = 0;
  txq->ctx_curr = 0;
...
static void igb_reset_tx_queue(struct igb_tx_queue *txq,
...
int eth_igb_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
   ...
   * of E1000_ALIGN.
   */
  if (nb_desc % IGB_TXD_ALIGN != 0 || (nb_desc > E1000_MAX_RING_DESC) ||
  ...
  tz = rte_eth_dma_zone_reserve(dev, "tx_ring", queue_idx, size, E1000_ALIGN,
                                socket_id);
  if (tz == NULL) {
  ...
  txq->sw_ring =
      rte_zmalloc("txq->sw_ring", sizeof(struct igb_tx_entry) * nb_desc,
                  RTE_CACHE_LINE_SIZE);
  if (txq->sw_ring == NULL) {
    igb_tx_queue_release(txq);
    return -ENOMEM;
...
static void igb_rx_queue_release_mbufs(struct igb_rx_queue *rxq) {
  unsigned i;
...
static void igb_rx_queue_release(struct igb_rx_queue *rxq) {
  if (rxq != NULL) {
...
int eth_igb_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                           uint16_t nb_desc, unsigned int socket_id,
   ...
   * of E1000_ALIGN.
   */
  if (nb_desc % IGB_RXD_ALIGN != 0 || (nb_desc > E1000_MAX_RING_DESC) ||
      (nb_desc < E1000_MIN_RING_DESC)) {
    return -EINVAL;
  }
  ...
  rz = rte_eth_dma_zone_reserve(dev, "rx_ring", queue_idx, size, E1000_ALIGN,
                                socket_id);
  if (rz == NULL) {
  ...
  /* Allocate software ring. */
  rxq->sw_ring =
      rte_zmalloc("rxq->sw_ring", sizeof(struct igb_rx_entry) * nb_desc,
                  RTE_CACHE_LINE_SIZE);
  if (rxq->sw_ring == NULL) {
    igb_rx_queue_release(rxq);
    return -ENOMEM;
  }
  PMD_INIT_LOG(DEBUG, "sw_ring=%p hw_ring=%p dma_addr=0x%" PRIx64, rxq->sw_ring,
...
static void igb_rss_disable(struct rte_eth_dev *dev) {
  struct e1000_hw *hw;
  uint32_t mrqc;
...
static void igb_hw_rss_hash_set(struct e1000_hw *hw,
...
static void igb_rss_configure(struct rte_eth_dev *dev) {
  struct rte_eth_rss_conf rss_conf;
  struct e1000_hw *hw;
  uint32_t shift;
  uint16_t i;
 ...
 */
static int igb_is_vmdq_supported(const struct rte_eth_dev *dev) {
...
static int igb_vmdq_rx_hw_configure(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < E1000_VMOLR_SIZE; i++) {
    ...
    vmolr &= ~(E1000_VMOLR_AUPE | E1000_VMOLR_ROMPE | E1000_VMOLR_ROPE |
               E1000_VMOLR_BAM | E1000_VMOLR_MPME);
  ...
  for (i = 0; i < cfg->nb_pool_maps; i++) {
    ...
    E1000_WRITE_REG(hw, E1000_VLVF(i),
...
static int igb_dev_mq_rx_configure(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint32_t mrqc;
...
int eth_igb_rx_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    E1000_WRITE_REG(hw, E1000_RDLEN(rxq->reg_idx),
                    rxq->nb_rx_desc * sizeof(union e1000_adv_rx_desc));
    ...
    if (buf_size >= 1024) {
      ...
      buf_size = (uint16_t)((srrctl & E1000_SRRCTL_BSIZEPKT_MASK)
  ...
  if (dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_KEEP_CRC) {
    ...
    if (hw->mac.type == e1000_i350 || hw->mac.type == e1000_i210 ||
        hw->mac.type == e1000_i211 || hw->mac.type == e1000_i354) {
      for (i = 0; i < dev->data->nb_rx_queues; i++) {
        rxq = dev->data->rx_queues[i];
...
void eth_igb_tx_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    E1000_WRITE_REG(hw, E1000_TDLEN(txq->reg_idx),
                    txq->nb_tx_desc * sizeof(union e1000_adv_tx_desc));
...
void eth_igbvf_tx_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    E1000_WRITE_REG(hw, E1000_TDLEN(i),
                    txq->nb_tx_desc * sizeof(union e1000_adv_tx_desc));
    E1000_WRITE_REG(hw, E1000_TDBAH(i), (uint32_t)(bus_addr >> 32));
...
int igb_rss_conf_init(struct rte_eth_dev *dev,
  ...
  if (in->key_len > RTE_DIM(out->key) ||
      ...
      ((hw->mac.type != e1000_82576) && (in->queue_num > IGB_MAX_RX_QUEUE_NUM)))
    return -EINVAL;
...
int igb_config_rss_filter(struct rte_eth_dev *dev,
  ...
  if (!add) {
    if (igb_action_rss_same(&filter_info->rss_info.conf, &conf->conf)) {
      igb_rss_disable(dev);
