...
static int igb_parse_ntuple_filter(struct rte_eth_dev *dev,
  ...
  if (hw->mac.type == e1000_82576) {
    ...
    if (filter->src_ip_mask || filter->dst_ip_mask || filter->src_port_mask) {
      memset(filter, 0, sizeof(struct rte_eth_ntuple_filter));
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                         "only two tuple are "
...
static int igb_parse_ethertype_filter(struct rte_eth_dev *dev,
                                      const struct rte_flow_attr *attr,
                                      const struct rte_flow_item pattern[],
                                      const struct rte_flow_action actions[],
                                      struct rte_eth_ethertype_filter *filter,
...
static int igb_parse_syn_filter(struct rte_eth_dev *dev,
                                const struct rte_flow_attr *attr,
                                const struct rte_flow_item pattern[],
                                const struct rte_flow_action actions[],
                                struct rte_eth_syn_filter *filter,
 ...
 * item->last should be NULL.
 */
static int cons_parse_flex_filter(const struct rte_flow_attr *attr,
                                  const struct rte_flow_item pattern[],
                                  const struct rte_flow_action actions[],
...
item_loop:
  ...
  if (!raw_mask->length || !raw_mask->relative) {
  ...
  if ((raw_spec->length + offset + total_offset) > IGB_FLEX_FILTER_MAXLEN) {
    memset(filter, 0, sizeof(struct igb_flex_filter));
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                       "Not supported by flex filter");
    return -rte_errno;
  ...
  if (raw_spec->relative == 0) {
    for (j = 0; j < raw_spec->length; j++)
      filter->bytes[offset + j] = raw_spec->pattern[j];
    j = offset / CHAR_BIT;
    shift = offset % CHAR_BIT;
  } else {
    for (j = 0; j < raw_spec->length; j++)
      filter->bytes[total_offset + offset + j] = raw_spec->pattern[j];
...
static int igb_parse_flex_filter(struct rte_eth_dev *dev,
                                 const struct rte_flow_attr *attr,
                                 const struct rte_flow_item pattern[],
                                 const struct rte_flow_action actions[],
                                 struct igb_flex_filter *filter,
  ...
  ret = cons_parse_flex_filter(attr, pattern, actions, filter, error);
...
static int igb_parse_rss_filter(struct rte_eth_dev *dev,
                                const struct rte_flow_attr *attr,
                                const struct rte_flow_action actions[],
 ...
 */
static struct rte_flow *igb_flow_create(struct rte_eth_dev *dev,
                                        const struct rte_flow_attr *attr,
                                        const struct rte_flow_item pattern[],
                                        const struct rte_flow_action actions[],
                                        struct rte_flow_error *error) {
  ...
  igb_flow_mem_ptr =
      rte_zmalloc("igb_flow_mem", sizeof(struct igb_flow_mem), 0);
  if (!igb_flow_mem_ptr) {
    PMD_DRV_LOG(ERR, "failed to allocate memory");
    rte_free(flow);
    return NULL;
  ...
  TAILQ_INSERT_TAIL(&igb_flow_list, igb_flow_mem_ptr, entries);

  memset(&ntuple_filter, 0, sizeof(struct rte_eth_ntuple_filter));
  ret = igb_parse_ntuple_filter(dev, attr, pattern, actions, &ntuple_filter,
                                error);
  if (!ret) {
    ...
    if (!ret) {
      ...
      TAILQ_INSERT_TAIL(&igb_filter_ntuple_list, ntuple_filter_ptr, entries);
  ...
  ret = igb_parse_ethertype_filter(dev, attr, pattern, actions,
                                   &ethertype_filter, error);
  if (!ret) {
    ret = igb_add_del_ethertype_filter(dev, &ethertype_filter, TRUE);
    if (!ret) {
      ethertype_filter_ptr = rte_zmalloc(
          "igb_ethertype_filter", sizeof(struct igb_ethertype_filter_ele), 0);
      ...
      TAILQ_INSERT_TAIL(&igb_filter_ethertype_list, ethertype_filter_ptr,
                        entries);
  ...
  ret = igb_parse_syn_filter(dev, attr, pattern, actions, &syn_filter, error);
  if (!ret) {
    ...
    if (!ret) {
      ...
      TAILQ_INSERT_TAIL(&igb_filter_syn_list, syn_filter_ptr, entries);
  ...
  ret = igb_parse_flex_filter(dev, attr, pattern, actions, &flex_filter, error);
  if (!ret) {
    ret = eth_igb_add_del_flex_filter(dev, &flex_filter, TRUE);
    if (!ret) {
      flex_filter_ptr =
          rte_zmalloc("igb_flex_filter", sizeof(struct igb_flex_filter_ele), 0);
      if (!flex_filter_ptr) {
        PMD_DRV_LOG(ERR, "failed to allocate memory");
        goto out;
      }

      rte_memcpy(&flex_filter_ptr->filter_info, &flex_filter,
                 sizeof(struct igb_flex_filter));
      TAILQ_INSERT_TAIL(&igb_filter_flex_list, flex_filter_ptr, entries);
  ...
  memset(&rss_conf, 0, sizeof(struct igb_rte_flow_rss_conf));
  ret = igb_parse_rss_filter(dev, attr, actions, &rss_conf, error);
  if (!ret) {
    ret = igb_config_rss_filter(dev, &rss_conf, TRUE);
    if (!ret) {
      rss_filter_ptr =
          rte_zmalloc("igb_rss_filter", sizeof(struct igb_rss_conf_ele), 0);
      if (!rss_filter_ptr) {
        PMD_DRV_LOG(ERR, "failed to allocate memory");
        goto out;
      }
      igb_rss_conf_init(dev, &rss_filter_ptr->filter_info, &rss_conf.conf);
      TAILQ_INSERT_TAIL(&igb_filter_rss_list, rss_filter_ptr, entries);
...
out:
  TAILQ_REMOVE(&igb_flow_list, igb_flow_mem_ptr, entries);
 ...
 */
static int igb_flow_validate(__rte_unused struct rte_eth_dev *dev,
                             const struct rte_flow_attr *attr,
                             const struct rte_flow_item pattern[],
                             const struct rte_flow_action actions[],
                             struct rte_flow_error *error) {
...
static int igb_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                            struct rte_flow_error *error) {
  int ret;
  ...
  case RTE_ETH_FILTER_NTUPLE:
    ntuple_filter_ptr = (struct igb_ntuple_filter_ele *)pmd_flow->rule;
    ret =
        igb_add_del_ntuple_filter(dev, &ntuple_filter_ptr->filter_info, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&igb_filter_ntuple_list, ntuple_filter_ptr, entries);
      rte_free(ntuple_filter_ptr);
    }
    break;
  case RTE_ETH_FILTER_ETHERTYPE:
    ethertype_filter_ptr = (struct igb_ethertype_filter_ele *)pmd_flow->rule;
    ret = igb_add_del_ethertype_filter(dev, &ethertype_filter_ptr->filter_info,
                                       FALSE);
    if (!ret) {
      TAILQ_REMOVE(&igb_filter_ethertype_list, ethertype_filter_ptr, entries);
      rte_free(ethertype_filter_ptr);
    }
    break;
  case RTE_ETH_FILTER_SYN:
    syn_filter_ptr = (struct igb_eth_syn_filter_ele *)pmd_flow->rule;
    ret = eth_igb_syn_filter_set(dev, &syn_filter_ptr->filter_info, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&igb_filter_syn_list, syn_filter_ptr, entries);
      rte_free(syn_filter_ptr);
    }
    break;
  case RTE_ETH_FILTER_FLEXIBLE:
    flex_filter_ptr = (struct igb_flex_filter_ele *)pmd_flow->rule;
    ret =
        eth_igb_add_del_flex_filter(dev, &flex_filter_ptr->filter_info, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&igb_filter_flex_list, flex_filter_ptr, entries);
      rte_free(flex_filter_ptr);
    }
    break;
  case RTE_ETH_FILTER_HASH:
    rss_filter_ptr = (struct igb_rss_conf_ele *)pmd_flow->rule;
    ret = igb_config_rss_filter(dev, &rss_filter_ptr->filter_info, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&igb_filter_rss_list, rss_filter_ptr, entries);
...
static void igb_clear_all_ntuple_filter(struct rte_eth_dev *dev) {
...
static void igb_clear_all_ethertype_filter(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_filter_info *filter_info =
      E1000_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
  int i;
  ...
  for (i = 0; i < E1000_MAX_ETQF_FILTERS; i++) {
    if (filter_info->ethertype_mask & (1 << i)) {
      (void)igb_ethertype_filter_remove(filter_info, (uint8_t)i);
...
static void igb_clear_syn_filter(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_filter_info *filter_info =
      E1000_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
...
static void igb_clear_all_flex_filter(struct rte_eth_dev *dev) {
  struct e1000_filter_info *filter_info =
      E1000_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
...
static void igb_clear_rss_filter(struct rte_eth_dev *dev) {
    ...
    if (igb_flow_mem_ptr->dev == dev) {
      ...
      case RTE_ETH_FILTER_ETHERTYPE:
        ethertype_filter_ptr =
            (struct igb_ethertype_filter_ele *)pmd_flow->rule;
        TAILQ_REMOVE(&igb_filter_ethertype_list, ethertype_filter_ptr, entries);
        rte_free(ethertype_filter_ptr);
        break;
      case RTE_ETH_FILTER_SYN:
...
static int igb_flow_flush(struct rte_eth_dev *dev,
