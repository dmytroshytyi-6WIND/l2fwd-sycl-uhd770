...
#define E1000_FLEX_FILTERS_MASK_SIZE (E1000_MAX_FLEX_FILTER_DWDS / 2)
...
#define MAC_TYPE_FILTER_SUP(type)                                              \
  do {                                                                         \
    if ((type) != e1000_82580 && (type) != e1000_i350 &&                       \
        (type) != e1000_82576 && (type) != e1000_i210 && (type) != e1000_i211) \
      return -ENOTSUP;                                                         \
  } while (0)
...
struct e1000_5tuple_filter_info {
  ...
  /* the packet matched above 5tuple and contain any set bit will hit this
   * filter. */
  uint8_t tcp_flags;
...
struct e1000_2tuple_filter_info {
  uint16_t dst_port;
  uint8_t proto; /* l4 protocol. */
  /* the packet matched above 2tuple and contain any set bit will hit this
   * filter. */
  uint8_t tcp_flags;
  uint8_t priority;        /* seven levels (001b-111b), 111b is highest,
                                used when more than one filter matches. */
...
#define E1000_DEV_PRIVATE(adapter) ((struct e1000_adapter *)adapter)
...
uint16_t eth_igb_recv_scattered_pkts(void *rxq, struct rte_mbuf **rx_pkts,
                                     uint16_t nb_pkts);
