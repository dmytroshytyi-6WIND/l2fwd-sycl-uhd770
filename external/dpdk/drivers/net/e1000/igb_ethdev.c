...
static int eth_igb_link_update(struct rte_eth_dev *dev, int wait_to_complete);
...
static int eth_igb_xstats_get_names(struct rte_eth_dev *dev,
                                    ...
                                    unsigned int size);
static int
eth_igb_xstats_get_names_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
                               struct rte_eth_xstat_name *xstats_names,
                               unsigned int limit);
static int eth_igb_stats_reset(struct rte_eth_dev *dev);
static int eth_igb_xstats_reset(struct rte_eth_dev *dev);
static int eth_igb_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
                                  size_t fw_size);
...
static int eth_igb_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                   int on);
...
static int igbvf_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                 int on);
...
static const struct rte_igb_xstats_name_off rte_igb_stats_strings[] = {
    ...
    {"tx_excessive_collision_packets", offsetof(struct e1000_hw_stats, ecol)},
    ...
    {"rx_flow_control_unsupported_packets",
     offsetof(struct e1000_hw_stats, fcruc)},
    ...
    {"rx_size_512_to_1023_packets", offsetof(struct e1000_hw_stats, prc1023)},
    {"rx_size_1024_to_max_packets", offsetof(struct e1000_hw_stats, prc1522)},
    ...
    {"tx_size_512_to_1023_packets", offsetof(struct e1000_hw_stats, ptc1023)},
    {"tx_size_1023_to_max_packets", offsetof(struct e1000_hw_stats, ptc1522)},
...
#define IGB_NB_XSTATS                                                          \
  (sizeof(rte_igb_stats_strings) / sizeof(rte_igb_stats_strings[0]))
...
#define IGBVF_NB_XSTATS                                                        \
  (sizeof(rte_igbvf_stats_strings) / sizeof(rte_igbvf_stats_strings[0]))
...
static inline void igbvf_intr_enable(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static void igbvf_set_ivar_map(struct e1000_hw *hw, uint8_t msix_vector) {
  uint32_t tmp = 0;
...
static void eth_igbvf_configure_msix_intr(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static inline int32_t igb_pf_reset_hw(struct e1000_hw *hw) {
  uint32_t ctrl_ext;
  int32_t status;
...
static void igb_identify_hardware(struct rte_eth_dev *dev,
...
static int igb_reset_swfw_lock(struct e1000_hw *hw) {
  int ret_val;
...
static int igb_ntuple_filter_uninit(struct rte_eth_dev *eth_dev) {
  ...
  while ((p_2tuple = TAILQ_FIRST(&filter_info->twotuple_list))) {
    TAILQ_REMOVE(&filter_info->twotuple_list, p_2tuple, entries);
    rte_free(p_2tuple);
  }
...
static int eth_igb_dev_init(struct rte_eth_dev *eth_dev) {
  int error = 0;
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("e1000", RTE_ETHER_ADDR_LEN * hw->mac.rar_entry_count, 0);
  ...
  rte_intr_callback_register(pci_dev->intr_handle, eth_igb_interrupt_handler,
                             (void *)eth_dev);
  ...
  memset(filter_info, 0, sizeof(struct e1000_filter_info));
...
static int eth_igb_dev_uninit(struct rte_eth_dev *eth_dev) {
  PMD_INIT_FUNC_TRACE();
 ...
 */
static int eth_igbvf_dev_init(struct rte_eth_dev *eth_dev) {
  struct rte_pci_device *pci_dev;
  ...
  if (diag != 0) {
    PMD_INIT_LOG(ERR, "Shared code init failed for igbvf: %d", diag);
    return -EIO;
  }
  ...
  /* Allocate memory for storing MAC addresses */
  eth_dev->data->mac_addrs =
      rte_zmalloc("igbvf", RTE_ETHER_ADDR_LEN * hw->mac.rar_entry_count, 0);
  if (eth_dev->data->mac_addrs == NULL) {
    PMD_INIT_LOG(ERR,
  ...
  rte_intr_callback_register(intr_handle, eth_igbvf_interrupt_handler, eth_dev);

  return 0;
}

static int eth_igbvf_dev_uninit(struct rte_eth_dev *eth_dev) {
  PMD_INIT_FUNC_TRACE();
...
static int eth_igb_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                             struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct e1000_adapter),
                                       eth_igb_dev_init);
}
...
static void igb_vmdq_vlan_hw_filter_enable(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int igb_check_mq_mode(struct rte_eth_dev *dev) {
  ...
  if ((rx_mq_mode & RTE_ETH_MQ_RX_DCB_FLAG) ||
      tx_mq_mode == RTE_ETH_MQ_TX_DCB || tx_mq_mode == RTE_ETH_MQ_TX_VMDQ_DCB) {
  ...
  if (RTE_ETH_DEV_SRIOV(dev).active != 0) {
    ...
    if (rx_mq_mode != RTE_ETH_MQ_RX_NONE &&
      ...
      PMD_INIT_LOG(ERR, "RX mode %d is not supported.", rx_mq_mode);
      return -EINVAL;
    }
...
static int eth_igb_configure(struct rte_eth_dev *dev) {
  ...
  if (ret != 0) {
    PMD_DRV_LOG(ERR, "igb_check_mq_mode fails with %d.", ret);
    return ret;
  }
...
static void eth_igb_rxtx_control(struct rte_eth_dev *dev, bool enable) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint32_t tctl, rctl;
...
static int eth_igb_start(struct rte_eth_dev *dev) {
  ...
  if (rte_intr_allow_others(intr_handle)) {
    ...
    rte_intr_callback_unregister(intr_handle, eth_igb_interrupt_handler,
                                 (void *)dev);
    if (dev->data->dev_conf.intr_conf.lsc != 0)
...
static int eth_igb_stop(struct rte_eth_dev *dev) {
  ...
  if (!rte_intr_allow_others(intr_handle))
    ...
    rte_intr_callback_register(intr_handle, eth_igb_interrupt_handler,
                               (void *)dev);
...
static int eth_igb_close(struct rte_eth_dev *dev) {
  ...
  rte_intr_callback_unregister(intr_handle, eth_igb_interrupt_handler, dev);
  ...
  memset(&filter_info->rss_info, 0, sizeof(struct igb_rte_flow_rss_conf));
...
static void igb_read_stats_registers(struct e1000_hw *hw,
...
static int eth_igb_stats_get(struct rte_eth_dev *dev,
  ...
  rte_stats->ierrors = stats->crcerrs + stats->rlec + stats->rxerrc +
                       stats->algnerrc + stats->cexterr;
...
eth_igb_xstats_get_names_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
  ...
  if (!ids) {
    ...
    eth_igb_xstats_get_names_by_id(dev, NULL, xstats_names_copy, IGB_NB_XSTATS);
...
static int eth_igb_xstats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < IGB_NB_XSTATS; i++) {
    xstats[i].id = i;
    xstats[i].value =
        *(uint64_t *)(((char *)hw_stats) + rte_igb_stats_strings[i].offset);
  }
...
static int eth_igb_xstats_get_by_id(struct rte_eth_dev *dev,
  ...
  if (!ids) {
    ...
    eth_igb_xstats_get_by_id(dev, NULL, values_copy, IGB_NB_XSTATS);
...
static void igbvf_read_stats_registers(struct e1000_hw *hw,
                                       struct e1000_vf_stats *hw_stats) {
  /* Good Rx packets, include VF loopback */
  UPDATE_VF_STAT(E1000_VFGPRC, hw_stats->last_gprc, hw_stats->gprc);

  /* Good Rx octets, include VF loopback */
  UPDATE_VF_STAT(E1000_VFGORC, hw_stats->last_gorc, hw_stats->gorc);

  /* Good Tx packets, include VF loopback */
  UPDATE_VF_STAT(E1000_VFGPTC, hw_stats->last_gptc, hw_stats->gptc);

  /* Good Tx octets, include VF loopback */
  UPDATE_VF_STAT(E1000_VFGOTC, hw_stats->last_gotc, hw_stats->gotc);

  /* Rx Multicst packets */
  UPDATE_VF_STAT(E1000_VFMPRC, hw_stats->last_mprc, hw_stats->mprc);

  /* Good Rx loopback packets */
  UPDATE_VF_STAT(E1000_VFGPRLBC, hw_stats->last_gprlbc, hw_stats->gprlbc);

  /* Good Rx loopback octets */
  UPDATE_VF_STAT(E1000_VFGORLBC, hw_stats->last_gorlbc, hw_stats->gorlbc);

  /* Good Tx loopback packets */
  UPDATE_VF_STAT(E1000_VFGPTLBC, hw_stats->last_gptlbc, hw_stats->gptlbc);

  /* Good Tx loopback octets */
  UPDATE_VF_STAT(E1000_VFGOTLBC, hw_stats->last_gotlbc, hw_stats->gotlbc);
}
...
static int eth_igbvf_xstats_get_names(__rte_unused struct rte_eth_dev *dev,
  ...
  if (xstats_names != NULL)
    for (i = 0; i < IGBVF_NB_XSTATS; i++) {
      strlcpy(xstats_names[i].name, rte_igbvf_stats_strings[i].name,
              sizeof(xstats_names[i].name));
    }
...
static int eth_igbvf_xstats_get(struct rte_eth_dev *dev,
  ...
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_vf_stats *hw_stats =
      (struct e1000_vf_stats *)E1000_DEV_PRIVATE_TO_STATS(
  ...
  for (i = 0; i < IGBVF_NB_XSTATS; i++) {
    xstats[i].id = i;
    xstats[i].value =
        *(uint64_t *)(((char *)hw_stats) + rte_igbvf_stats_strings[i].offset);
  }
...
static int eth_igbvf_stats_reset(struct rte_eth_dev *dev) {
  ...
  memset(&hw_stats->gprc, 0,
         sizeof(*hw_stats) - offsetof(struct e1000_vf_stats, gprc));
...
static int eth_igb_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
  ...
  case e1000_i211:
    if (!(e1000_get_flash_presence_i210(hw))) {
      ret = snprintf(fw_version, fw_size, "%2d.%2d-%d", fw.invm_major,
                     fw.invm_minor, fw.invm_img_type);
      break;
    }
    /* fall through */
  default:
    ...
    if (fw.or_valid) {
      ret = snprintf(fw_version, fw_size, "%d.%d, 0x%08x, %d.%d.%d",
                     fw.eep_major, fw.eep_minor, fw.etrack_id, fw.or_major,
                     fw.or_build, fw.or_patch);
      /* no option rom */
    } else {
      if (fw.etrack_id != 0X0000) {
        ret = snprintf(fw_version, fw_size, "%d.%d, 0x%08x", fw.eep_major,
                       fw.eep_minor, fw.etrack_id);
      } else {
        ret = snprintf(fw_version, fw_size, "%d.%d.%d", fw.eep_major,
...
static int eth_igb_infos_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa =
      igb_get_rx_port_offloads_capa(dev) | dev_info->rx_queue_offload_capa;
  dev_info->tx_queue_offload_capa = igb_get_tx_queue_offloads_capa(dev);
  dev_info->tx_offload_capa =
      igb_get_tx_port_offloads_capa(dev) | dev_info->tx_queue_offload_capa;
...
static int eth_igb_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
  ...
  if (link_check) {
    ...
    link.link_duplex = (duplex == FULL_DUPLEX) ? RTE_ETH_LINK_FULL_DUPLEX
...
static int eth_igb_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
  ...
  vid_idx =
      (uint32_t)((vlan_id >> E1000_VFTA_ENTRY_SHIFT) & E1000_VFTA_ENTRY_MASK);
...
static int eth_igbvf_interrupt_get_status(struct rte_eth_dev *dev) {
  uint32_t eicr;
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_interrupt *intr =
      E1000_DEV_PRIVATE_TO_INTR(dev->data->dev_private);
...
static int eth_igbvf_interrupt_action(struct rte_eth_dev *dev,
                                      struct rte_intr_handle *intr_handle) {
  struct e1000_interrupt *intr =
      E1000_DEV_PRIVATE_TO_INTR(dev->data->dev_private);
...
static int eth_igb_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  enum e1000_fc_mode rte_fcmode_2_e1000_fcmode[] = {
      e1000_fc_none, e1000_fc_rx_pause, e1000_fc_tx_pause, e1000_fc_full};
  uint32_t rx_buf_size;
...
static void igbvf_stop_adapter(struct rte_eth_dev *dev) {
  u32 reg_val;
  u16 i;
  struct rte_eth_dev_info dev_info;
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int igbvf_dev_close(struct rte_eth_dev *dev) {
  ...
  rte_intr_callback_unregister(pci_dev->intr_handle,
                               eth_igbvf_interrupt_handler, (void *)dev);
...
static void igbvf_set_vfta_all(struct rte_eth_dev *dev, bool on) {
  ...
  for (i = 0; i < IGB_VFTA_SIZE; i++) {
    ...
    if (vfta) {
      ...
      for (j = 0; j < 32; j++) {
        if (vfta & mask)
          igbvf_set_vfta(hw, (uint16_t)((i << 5) + j), on);
        mask <<= 1;
      }
    }
  }
...
static int eth_igb_rss_reta_update(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size; i += IGB_4_BIT_WIDTH) {
    ...
    mask = (uint8_t)((reta_conf[idx].mask >> shift) & IGB_4_BIT_MASK);
    if (!mask)
      continue;
    ...
    for (j = 0; j < IGB_4_BIT_WIDTH; j++) {
      if (mask & (0x1 << j))
        reta_conf[idx].reta[shift + j] =
            ((reta >> (CHAR_BIT * j)) & IGB_8_BIT_MASK);
    }
  }
...
static inline void igb_inject_2uple_filter(struct rte_eth_dev *dev,
...
static int igb_add_2tuple_filter(struct rte_eth_dev *dev,
  ...
  filter =
      rte_zmalloc("e1000_2tuple_filter", sizeof(struct e1000_2tuple_filter), 0);
  if (filter == NULL)
    return -ENOMEM;

  ret = ntuple_filter_to_2tuple(ntuple_filter, &filter->filter_info);
  if (ret < 0) {
    rte_free(filter);
    return ret;
  }
  ...
  for (i = 0; i < E1000_MAX_TTQF_FILTERS; i++) {
    if (!(filter_info->twotuple_mask & (1 << i))) {
      ...
      filter->index = i;
      TAILQ_INSERT_TAIL(&filter_info->twotuple_list, filter, entries);
      break;
    }
  }
...
igb_remove_2tuple_filter(struct rte_eth_dev *dev,
  ...
  filter =
      igb_2tuple_filter_lookup(&filter_info->twotuple_list, &filter_2tuple);
  if (filter == NULL) {
...
static inline void igb_inject_flex_filter(struct rte_eth_dev *dev,
...
eth_igb_flex_filter_lookup(struct e1000_flex_filter_list *filter_list,
  ...
  TAILQ_FOREACH(it, filter_list, entries) {
    if (memcmp(key, &it->filter_info, sizeof(struct e1000_flex_filter_info)) ==
        0)
      return it;
  }
...
void igb_remove_flex_filter(struct rte_eth_dev *dev,
  ...
  E1000_WRITE_REG(hw, E1000_WUFC, wufc & (~(E1000_WUFC_FLX0 << filter->index)));
...
int eth_igb_add_del_flex_filter(struct rte_eth_dev *dev,
                                struct igb_flex_filter *filter, bool add) {
  struct e1000_filter_info *filter_info =
      E1000_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
  ...
  flex_filter =
      rte_zmalloc("e1000_flex_filter", sizeof(struct e1000_flex_filter), 0);
  if (flex_filter == NULL)
    return -ENOMEM;
  ...
  if (add) {
    ...
    for (i = 0; i < E1000_MAX_FLEX_FILTERS; i++) {
      if (!(filter_info->flex_mask & (1 << i))) {
        ...
        flex_filter->index = i;
        TAILQ_INSERT_TAIL(&filter_info->flex_list, flex_filter, entries);
        break;
      }
    }
...
igb_inject_5tuple_filter_82576(struct rte_eth_dev *dev,
  ...
  ftqf |= (filter->queue << E1000_FTQF_QUEUE_SHIFT) & E1000_FTQF_QUEUE_MASK;
...
igb_add_5tuple_filter_82576(struct rte_eth_dev *dev,
  ...
  filter =
      rte_zmalloc("e1000_5tuple_filter", sizeof(struct e1000_5tuple_filter), 0);
  if (filter == NULL)
    return -ENOMEM;

  ret = ntuple_filter_to_5tuple_82576(ntuple_filter, &filter->filter_info);
  if (ret < 0) {
    rte_free(filter);
    return ret;
  }
...
igb_remove_5tuple_filter_82576(struct rte_eth_dev *dev,
  ...
  ret = ntuple_filter_to_5tuple_82576(ntuple_filter, &filter_5tuple);
  if (ret < 0)
    return ret;
...
int igb_add_del_ntuple_filter(struct rte_eth_dev *dev,
  ...
  case (RTE_5TUPLE_FLAGS | RTE_NTUPLE_FLAGS_TCP_FLAG):
    if (hw->mac.type != e1000_82576)
      return -ENOTSUP;
    if (add)
      ret = igb_add_5tuple_filter_82576(dev, ntuple_filter);
    else
      ret = igb_remove_5tuple_filter_82576(dev, ntuple_filter);
    break;
...
int igb_add_del_ethertype_filter(struct rte_eth_dev *dev,
  ...
  if (add) {
    ...
    ret = igb_ethertype_filter_insert(filter_info, filter->ether_type, etqf);
    if (ret < 0) {
...
static int eth_igb_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
                                const struct rte_flow_ops **ops) {
  *ops = &igb_flow_ops;
  return 0;
}
...
static uint64_t igb_read_systime_cyclecounter(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  case e1000_i211:
    ...
    systime_cycles +=
        (uint64_t)E1000_READ_REG(hw, E1000_SYSTIMH) * NSEC_PER_SEC;
    break;
  case e1000_82580:
  case e1000_i350:
  case e1000_i354:
    ...
    systime_cycles |= (uint64_t)(E1000_READ_REG(hw, E1000_SYSTIMH) & 0xff)
                      << 32;
    break;
  default:
    systime_cycles = (uint64_t)E1000_READ_REG(hw, E1000_SYSTIML);
    systime_cycles |= (uint64_t)E1000_READ_REG(hw, E1000_SYSTIMH) << 32;
    break;
  }
...
static uint64_t igb_read_rx_tstamp_cyclecounter(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  case e1000_i211:
    ...
    rx_tstamp_cycles +=
        (uint64_t)E1000_READ_REG(hw, E1000_RXSTMPH) * NSEC_PER_SEC;
    break;
  case e1000_82580:
  case e1000_i350:
  case e1000_i354:
    rx_tstamp_cycles = (uint64_t)E1000_READ_REG(hw, E1000_RXSTMPL);
    /* Only the 8 LSB are valid. */
    rx_tstamp_cycles |= (uint64_t)(E1000_READ_REG(hw, E1000_RXSTMPH) & 0xff)
                        << 32;
    break;
  default:
    rx_tstamp_cycles = (uint64_t)E1000_READ_REG(hw, E1000_RXSTMPL);
    rx_tstamp_cycles |= (uint64_t)E1000_READ_REG(hw, E1000_RXSTMPH) << 32;
    break;
  }
...
static uint64_t igb_read_tx_tstamp_cyclecounter(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  case e1000_i211:
    ...
    tx_tstamp_cycles +=
        (uint64_t)E1000_READ_REG(hw, E1000_TXSTMPH) * NSEC_PER_SEC;
    break;
  case e1000_82580:
  case e1000_i350:
  case e1000_i354:
    tx_tstamp_cycles = (uint64_t)E1000_READ_REG(hw, E1000_TXSTMPL);
    /* Only the 8 LSB are valid. */
    tx_tstamp_cycles |= (uint64_t)(E1000_READ_REG(hw, E1000_TXSTMPH) & 0xff)
                        << 32;
    break;
  default:
    tx_tstamp_cycles = (uint64_t)E1000_READ_REG(hw, E1000_TXSTMPL);
    tx_tstamp_cycles |= (uint64_t)E1000_READ_REG(hw, E1000_TXSTMPH) << 32;
    break;
  }
...
static void igb_start_timecounters(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int igb_timesync_enable(struct rte_eth_dev *dev) {
  ...
  E1000_WRITE_REG(
      hw, E1000_ETQF(E1000_ETQF_FILTER_1588),
      (RTE_ETHER_TYPE_1588 | E1000_ETQF_FILTER_ENABLE | E1000_ETQF_1588));
...
static int eth_igb_get_reg_length(struct rte_eth_dev *dev __rte_unused) {
  int count = 0;
  int g_ind = 0;
...
static int igbvf_get_reg_length(struct rte_eth_dev *dev __rte_unused) {
  int count = 0;
  int g_ind = 0;
  const struct reg_info *reg_group;
...
static int eth_igb_get_regs(struct rte_eth_dev *dev,
  ...
  if ((regs->length == 0) ||
    ...
    while ((reg_group = igb_regs[g_ind++]))
      count += igb_read_regs_group(dev, &data[count], reg_group);
    return 0;
  }
...
static int eth_igb_get_eeprom(struct rte_eth_dev *dev,
  ...
  if ((first >= hw->nvm.word_size) || ((first + length) >= hw->nvm.word_size))
    return -EINVAL;

  in_eeprom->magic = hw->vendor_id | ((uint32_t)hw->device_id << 16);
...
static int eth_igb_set_eeprom(struct rte_eth_dev *dev,
  ...
  in_eeprom->magic = (uint32_t)hw->vendor_id | ((uint32_t)hw->device_id << 16);
...
static int eth_igb_get_module_eeprom(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < last_word - first_word + 1; i++) {
    status = e1000_read_phy_reg_i2c(hw, (first_word + i) * 2, &dataword[i]);
    if (status) {
  ...
  if (hw->mac.type == e1000_82575) {
    ...
    if ((direction == 0) || (direction == 1))
      eth_igb_write_ivar(hw, msix_vector, queue & 0x7,
             ...
             (hw->mac.type == e1000_i354) || (hw->mac.type == e1000_i210) ||
             (hw->mac.type == e1000_i211)) {
    if ((direction == 0) || (direction == 1))
      eth_igb_write_ivar(hw, msix_vector, queue >> 1,
...
static void eth_igb_configure_msix_intr(struct rte_eth_dev *dev) {
  ...
  } else if ((hw->mac.type == e1000_82576) || (hw->mac.type == e1000_82580) ||
             (hw->mac.type == e1000_i350) || (hw->mac.type == e1000_i354) ||
             (hw->mac.type == e1000_i210) || (hw->mac.type == e1000_i211)) {
    /* turn on MSI-X capability first */
    E1000_WRITE_REG(hw, E1000_GPIE,
                    E1000_GPIE_MSIX_MODE | E1000_GPIE_PBA | E1000_GPIE_EIAME |
                        E1000_GPIE_NSICR);
...
static inline void igb_ntuple_filter_restore(struct rte_eth_dev *dev) {
  struct e1000_filter_info *filter_info =
      E1000_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
  struct e1000_5tuple_filter *p_5tuple;
  struct e1000_2tuple_filter *p_2tuple;
...
/* restore SYN filter */
static inline void igb_syn_filter_restore(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_filter_info *filter_info =
      E1000_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
  uint32_t synqf;
...
static inline void igb_ethertype_filter_restore(struct rte_eth_dev *dev) {
  struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct e1000_filter_info *filter_info =
      E1000_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
  int i;
...
static inline void igb_flex_filter_restore(struct rte_eth_dev *dev) {
  struct e1000_filter_info *filter_info =
      E1000_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
...
/* restore rss filter */
static inline void igb_rss_filter_restore(struct rte_eth_dev *dev) {
  struct e1000_filter_info *filter_info =
      E1000_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
...
RTE_PMD_REGISTER_KMOD_DEP(net_e1000_igb,
