...
static const char *valid_arguments[] = {ETH_RING_NUMA_NODE_ACTION_ARG,
                                        ETH_RING_INTERNAL_ARG, NULL};
...
enum dev_action { DEV_CREATE, DEV_ATTACH };
...
#define PMD_LOG(level, fmt, args...)                                           \
  rte_log(RTE_LOG_##level, eth_ring_logtype, "%s(): " fmt "\n", __func__,      \
          ##args)
...
static uint16_t eth_ring_rx(void *q, struct rte_mbuf **bufs, uint16_t nb_bufs) {
  ...
  const uint16_t nb_rx =
      (uint16_t)rte_ring_dequeue_burst(r->rng, ptrs, nb_bufs, NULL);
...
static uint16_t eth_ring_tx(void *q, struct rte_mbuf **bufs, uint16_t nb_bufs) {
  ...
  const uint16_t nb_tx =
      (uint16_t)rte_ring_enqueue_burst(r->rng, ptrs, nb_bufs, NULL);
...
static void eth_mac_addr_remove(struct rte_eth_dev *dev __rte_unused,
                                uint32_t index __rte_unused) {}

static int eth_mac_addr_add(struct rte_eth_dev *dev __rte_unused,
...
static int eth_allmulticast_enable(struct rte_eth_dev *dev __rte_unused) {
  return 0;
}

static int eth_allmulticast_disable(struct rte_eth_dev *dev __rte_unused) {
  return 0;
}
...
static int do_eth_dev_ring_create(
    const char *name, struct rte_vdev_device *vdev,
    struct rte_ring *const rx_queues[], const unsigned int nb_rx_queues,
    struct rte_ring *const tx_queues[], const unsigned int nb_tx_queues,
  ...
  PMD_LOG(INFO, "Creating rings-backed ethdev on numa socket %u", numa_node);

  rx_queues_local =
      rte_calloc_socket(name, nb_rx_queues, sizeof(void *), 0, numa_node);
  ...
  tx_queues_local =
      rte_calloc_socket(name, nb_tx_queues, sizeof(void *), 0, numa_node);
...
int rte_eth_from_rings(const char *name, struct rte_ring *const rx_queues[],
                       const unsigned int nb_rx_queues,
                       struct rte_ring *const tx_queues[],
                       const unsigned int nb_tx_queues,
                       const unsigned int numa_node) {
  ...
  snprintf(args_str, sizeof(args_str), "%s=%p", ETH_RING_INTERNAL_ARG, &args);
...
static int eth_dev_ring_create(const char *name, struct rte_vdev_device *vdev,
                               const unsigned int numa_node,
                               enum dev_action action,
  ...
  unsigned int num_rings =
      RTE_MIN(RTE_PMD_RING_MAX_RX_RINGS, RTE_PMD_RING_MAX_TX_RINGS);

  for (i = 0; i < num_rings; i++) {
    int cc;

    cc = snprintf(rng_name, sizeof(rng_name), "ETH_RXTX%u_%s", i, name);
...
static int parse_kvlist(const char *key __rte_unused, const char *value,
  ...
  node = strchr(name, ':');
  if (!node) {
    PMD_LOG(WARNING, "could not parse node value from %s", name);
    goto out;
  }
  ...
  *node = '\0';
  ...
  action = strchr(node, ':');
  if (!action) {
    PMD_LOG(WARNING, "could not parse action value from %s", node);
    goto out;
  }
  ...
  *action = '\0';
  ...
  if ((errno != 0) || (*end != '\0')) {
    PMD_LOG(WARNING, "node value %s is unparseable as a number", node);
    goto out;
  }
...
static int parse_internal_args(const char *key __rte_unused, const char *value,
                               void *data) {
...
static int rte_pmd_ring_probe(struct rte_vdev_device *dev) {
  ...
  if (params == NULL || params[0] == '\0') {
    ret = eth_dev_ring_create(name, dev, rte_socket_id(), DEV_CREATE, &eth_dev);
    if (ret == -1) {
      PMD_LOG(INFO, "Attach to pmd_ring for %s", name);
      ret =
          eth_dev_ring_create(name, dev, rte_socket_id(), DEV_ATTACH, &eth_dev);
    }
  } else {
    ...
    if (!kvlist) {
      ...
      ret =
          eth_dev_ring_create(name, dev, rte_socket_id(), DEV_CREATE, &eth_dev);
      if (ret == -1) {
        PMD_LOG(INFO, "Attach to pmd_ring for %s", name);
        ret = eth_dev_ring_create(name, dev, rte_socket_id(), DEV_ATTACH,
                                  &eth_dev);
      }
    ...
    if (rte_kvargs_count(kvlist, ETH_RING_INTERNAL_ARG) == 1) {
      ret = rte_kvargs_process(kvlist, ETH_RING_INTERNAL_ARG,
                               parse_internal_args, &internal_args);
      if (ret < 0)
        goto out_free;

      ret = do_eth_dev_ring_create(
          name, dev, internal_args->rx_queues, internal_args->nb_rx_queues,
          internal_args->tx_queues, internal_args->nb_tx_queues,
          internal_args->numa_node, DEV_ATTACH, &eth_dev);
      if (ret >= 0)
        ret = 0;
    } else {
      ...
      for (info->count = 0; info->count < info->total; info->count++) {
        ret = eth_dev_ring_create(info->list[info->count].name, dev,
                                  ...
                                  info->list[info->count].action, &eth_dev);
        if ((ret == -1) && (info->list[info->count].action == DEV_CREATE)) {
          PMD_LOG(INFO, "Attach to pmd_ring for %s", name);
          ret = eth_dev_ring_create(name, dev, info->list[info->count].node,
                                    DEV_ATTACH, &eth_dev);
        }
      }
    }
...
static int rte_pmd_ring_remove(struct rte_vdev_device *dev) {
...
RTE_PMD_REGISTER_PARAM_STRING(net_ring, ETH_RING_NUMA_NODE_ACTION_ARG
