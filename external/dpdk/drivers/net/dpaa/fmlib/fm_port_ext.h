...
#define FM_PORT_IOC_SET_TX_PAUSE_FRAMES                                        \
  _IOW(FM_IOC_TYPE_BASE, FM_PORT_IOC_NUM(40), ioc_fm_port_tx_pause_frames_t)
 ...
 */
typedef e_rx_store_response(t_fm_port_im_rx_store_callback)(
    t_handle h_app, uint8_t *p_data, uint16_t length, uint16_t status,
    uint8_t position, t_handle h_buf_context);
 ...
 * @Param[in]	  h_buf_context	A handle of the user associated with this buffer
 */
typedef void(t_fm_port_im_tx_conf_callback)(t_handle h_app, uint8_t *p_data,
                                            uint16_t status,
 ...
 *		  fm_port_init().
 */
uint32_t fm_port_config_deq_prefetch_option(
    t_handle h_fm_port, e_fm_port_deq_prefetch_option deq_prefetch_option);
 ...
 *		  fm_port_init().
 */
uint32_t fm_port_config_deq_byte_cnt(t_handle h_fm_port, uint16_t deq_byte_cnt);
 ...
 *		  fm_port_init().
 */
uint32_t fm_port_config_buffer_prefix_content(
    t_handle h_fm_port, t_fm_buffer_prefix_content *p_fm_buffer_prefix_content);
 ...
 */
uint32_t
fm_port_config_checksum_last_bytes_ignore(t_handle h_fm_port,
 ...
 */
uint32_t
fm_port_config_backup_pools(t_handle h_fm_port,
 ...
 *		  fm_port_init().
 */
uint32_t fm_port_config_frm_discard_override(t_handle h_fm_port, bool override);
 ...
 */
uint32_t
fm_port_config_dma_ic_cache_attr(t_handle h_fm_port,
 ...
 *		  fm_port_init().
 */
uint32_t fm_port_config_dma_scatter_gather_attr(
    t_handle h_fm_port, e_fm_dma_cache_option scatter_gather_cache_attr);
 ...
 *		  fm_port_init().
 */
uint32_t fm_port_config_dma_write_optimize(t_handle h_fm_port, bool optimize);
 ...
 * @Return	  E_OK on success; Error code otherwise.
 */
uint32_t fm_port_config_imfman_ctrl_external_structs_memory(
    t_handle h_fm_port, uint8_t mem_id, uint32_t mem_attributes);
 ...
 *		  Independent-Mode is enabled.
 */
uint32_t fm_port_config_max_frame_length(t_handle h_fm_port, uint16_t length);
 ...
 */
uint32_t
fm_port_config_rx_fifo_pri_elevation_level(t_handle h_fm_port,
 ...
 * @Cautions	  Allowed only following fm_port_init().
 */
uint32_t fm_port_set_frame_queue_counters(t_handle h_fm_port, bool enable);
 ...
 * @Cautions	  Allowed only following fm_port_init().
 */
uint32_t fm_port_set_alloc_buf_counter(t_handle h_fm_port, uint8_t pool_id,
                                       bool enable);
 ...
 *		  a disabled counter is accessed.
 */
uint32_t fm_port_get_alloc_buf_counter(t_handle h_fm_port, uint8_t pool_id);
 ...
 * @Cautions	  Allowed only following fm_port_init().
 */
uint32_t fm_port_modify_alloc_buf_counter(t_handle h_fm_port, uint8_t pool_id,
                                          uint32_t value);
 ...
 */
uint32_t
fm_port_add_congestion_grps(t_handle h_fm_port,
 ...
 */
uint32_t
fm_port_remove_congestion_grps(t_handle h_fm_port,
                               t_fm_port_congestion_grps *p_congestion_grps);
 ...
 * @Cautions	  Allowed only following fm_port_init().
 */
uint32_t fm_port_set_rx_l4checksum_verify(t_handle h_fm_port, bool l_4checksum);
 ...
 * @Cautions	Allowed only following fm_port_init().
 */
uint32_t fm_port_set_performance_counters(t_handle h_fm_port, bool enable);
 ...
 * @Cautions	  Allowed only following fm_port_init().
 */
uint32_t fm_port_set_performance_counters_params(
    t_handle h_fm_port, t_fm_port_performance_cnt *p_fm_port_performance_cnt);
 ...
 */
uint32_t fm_port_pcd_kg_modify_initial_scheme(
    t_handle h_fm_port, ioc_fm_pcd_kg_scheme_select_t *p_fm_pcd_kg_scheme);
 ...
 */
uint32_t
fm_port_pcd_kg_bind_schemes(t_handle h_fm_port,
 ...
 */
uint32_t
fm_port_pcd_kg_unbind_schemes(t_handle h_fm_port,
                              ioc_fm_pcd_port_schemes_params_t *p_port_scheme);
