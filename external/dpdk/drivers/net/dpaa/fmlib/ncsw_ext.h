 ...
 */
typedef uint32_t(t_put_buf_function)(t_handle h_buffer_pool, uint8_t *p_buffer,
 ...
 */
typedef e_rx_store_response(t_rx_store_function)(
    t_handle h_app, uint32_t queue_id, uint8_t *p_data, t_handle h_buf_context,
    uint32_t length, uint16_t status, uint8_t position, uint32_t flags);
