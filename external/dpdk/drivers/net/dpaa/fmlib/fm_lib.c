  ...
  if (!called) {
    ...
    if (ver.version.major != FMD_API_VERSION_MAJOR ||
      ...
      DPAA_PMD_WARN("Compiled against FMD API ver %u.%u.%u",
                    FMD_API_VERSION_MAJOR, FMD_API_VERSION_MINOR,
                    FMD_API_VERSION_RESPIN);
      DPAA_PMD_WARN("Running with FMD API ver %u.%u.%u", ver.version.major,
                    ver.version.minor, ver.version.respin);
    }
  }
  _fml_dbg("Finishing.\n");
...
fm_pcd_net_env_characteristics_set(t_handle h_fm_pcd,
  ...
  if (ioctl(p_pcd_dev->fd, FM_PCD_IOC_NET_ENV_CHARACTERISTICS_SET, params))
    return NULL;
...
uint32_t fm_pcd_net_env_characteristics_delete(t_handle h_net_env) {
  ...
  if (ioctl(p_pcd_dev->fd, FM_PCD_IOC_NET_ENV_CHARACTERISTICS_DELETE, &id))
    RETURN_ERROR(MINOR, E_INVALID_OPERATION, NO_MSG);
...
uint32_t fm_port_set_pcd(t_handle h_fm_port, ioc_fm_port_pcd_params_t *p) {
  t_device *p_dev = (t_device *)h_fm_port;
