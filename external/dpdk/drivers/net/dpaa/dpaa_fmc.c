...
struct fmc_model_t {
  ...
  enum ioc_fm_pcd_engine ccentry_action_type[FMC_CC_NODES_NUM]
  ...
  struct ioc_fm_pcd_cc_key_params_t htentry[FMC_CC_NODES_NUM]
                                           [FM_PCD_MAX_NUM_OF_KEYS];
  ...
  enum ioc_fm_pcd_engine htentry_action_type[FMC_CC_NODES_NUM]
                                            [FM_PCD_MAX_NUM_OF_KEYS];
  ...
  unsigned int repentry_action_index[FMC_REPLICATORS_NUM][FM_PCD_MAX_REPS];
  ...
  fmc_apply_order apply_order[FMC_FMAN_NUM * FMC_PORTS_PER_FMAN *
...
static int dpaa_port_fmc_scheme_parse(struct fman_if *fif,
                                      ...
                                      int apply_idx, uint16_t *rxq_idx,
                                      int max_nb_rxq, uint32_t *fqids,
                                      int8_t *vspids) {
  int idx = fmc->apply_order[apply_idx].index;
  uint32_t i;

  if (!fmc->scheme[idx].override_storage_profile && fif->is_shared_mac) {
    DPAA_PMD_WARN("No VSP assigned to scheme %d for sharemac %d!", idx,
                  fif->mac_idx);
    DPAA_PMD_WARN("Risk to receive pkts from skb pool to CRASH!");
  }

  if (e_IOC_FM_PCD_DONE == fmc->scheme[idx].next_engine) {
    for (i = 0; i < fmc->scheme[idx].key_ext_and_hash.hash_dist_num_of_fqids;
         i++) {
      ...
      if (fqid == fif->fqid_rx_def ||
        ...
        if (fif->is_shared_mac && fmc->scheme[idx].override_storage_profile &&
            fmc->scheme[idx].storage_profile.direct &&
            fmc->scheme[idx]
                    .storage_profile.profile_select
                    .direct_relative_profile_id != fif->base_profile_id) {
      ...
      if (fif->is_shared_mac && fmc->scheme[idx].override_storage_profile &&
          fmc->scheme[idx].storage_profile.direct &&
          fmc->scheme[idx]
                  .storage_profile.profile_select.direct_relative_profile_id ==
              fif->base_profile_id) {
...
static int dpaa_port_fmc_ccnode_parse(struct fman_if *fif,
  ...
  if ((*rxq_idx) >= max_nb_rxq) {
    DPAA_PMD_WARN("Too many queues in FMC policy %d overflow %d", (*rxq_idx),
                  max_nb_rxq);
  ...
  for (j = 0; j < keys_params->num_of_keys; ++j) {
    found = 0;
    fqid = keys_params->key_params[j]
               .cc_next_engine_params.params.enqueue_params.new_fqid;
     ...
     * application.
     */
    if (keys_params->key_params[j].cc_next_engine_params.next_engine !=
        e_IOC_FM_PCD_DONE) {
      DPAA_PMD_WARN("FMC CC next engine not support");
      continue;
    }
    if (keys_params->key_params[j]
            .cc_next_engine_params.params.enqueue_params.action !=
    ...
    fqids[(*rxq_idx)] = fqid;
    vspids[(*rxq_idx)] = keys_params->key_params[j]
                             .cc_next_engine_params.params.enqueue_params
                             .new_relative_storage_profile_id;

    if (vspids[(*rxq_idx)] == fif->base_profile_id && fif->is_shared_mac) {
      DPAA_PMD_ERR("VSP %d can NOT be used on DPDK.", vspids[(*rxq_idx)]);
...
int dpaa_port_fmc_init(struct fman_if *fif, uint32_t *fqids, int8_t *vspids,
                       int max_nb_rxq) {
  ...
  for (i = 0; i < fmc_model->apply_order_count; i++) {
    ...
    case fmcengine_end:
      break;
    case fmcport_start:
      current_port = dpaa_port_fmc_port_parse(fif, fmc_model, i);
      break;
    case fmcport_end:
      break;
    case fmcscheme:
      ...
      ret = dpaa_port_fmc_scheme_parse(fif, fmc_model, i, &rxq_idx, max_nb_rxq,
                                       fqids, vspids);
      if (ret)
        return ret;
    ...
    case fmcccnode:
      ...
      ret = dpaa_port_fmc_ccnode_parse(fif, fmc_model, i, &rxq_idx, max_nb_rxq,
                                       fqids, vspids);
      if (ret)
        return ret;
