...
__rte_internal int dpaa_eth_eventq_attach(
    const struct rte_eth_dev *dev, int eth_rx_queue_id, u16 ch_id,
    const struct rte_event_eth_rx_adapter_queue_conf *queue_conf);

__rte_internal int dpaa_eth_eventq_detach(const struct rte_eth_dev *dev,
                                          int eth_rx_queue_id);
...
#define DPAA_PMD_DEBUG(fmt, args...) DPAA_PMD_LOG(DEBUG, fmt, ##args)
#define DPAA_PMD_ERR(fmt, args...) DPAA_PMD_LOG(ERR, fmt, ##args)
#define DPAA_PMD_INFO(fmt, args...) DPAA_PMD_LOG(INFO, fmt, ##args)
#define DPAA_PMD_WARN(fmt, args...) DPAA_PMD_LOG(WARNING, fmt, ##args)

/* DP Logs, toggled out at compile time if level lower than current level */
#define DPAA_DP_LOG(level, fmt, args...) RTE_LOG_DP(level, PMD, fmt, ##args)
