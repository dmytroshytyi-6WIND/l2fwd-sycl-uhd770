...
static uint64_t dev_rx_offloads_sup = RTE_ETH_RX_OFFLOAD_SCATTER;
...
static const struct rte_dpaa_xstats_name_off dpaa_xstats_strings[] = {
    {"rx_align_err", offsetof(struct dpaa_if_stats, raln)},
    {"rx_valid_pause", offsetof(struct dpaa_if_stats, rxpf)},
    {"rx_fcs_err", offsetof(struct dpaa_if_stats, rfcs)},
    {"rx_vlan_frame", offsetof(struct dpaa_if_stats, rvlan)},
    {"rx_frame_err", offsetof(struct dpaa_if_stats, rerr)},
    {"rx_drop_err", offsetof(struct dpaa_if_stats, rdrp)},
    {"rx_undersized", offsetof(struct dpaa_if_stats, rund)},
    {"rx_oversize_err", offsetof(struct dpaa_if_stats, rovr)},
    {"rx_fragment_pkt", offsetof(struct dpaa_if_stats, rfrg)},
    {"tx_valid_pause", offsetof(struct dpaa_if_stats, txpf)},
    {"tx_fcs_err", offsetof(struct dpaa_if_stats, terr)},
    {"tx_vlan_frame", offsetof(struct dpaa_if_stats, tvlan)},
    {"rx_undersized", offsetof(struct dpaa_if_stats, tund)},
};
...
static inline void dpaa_poll_queue_default_config(struct qm_mcc_initfq *opts) {
  memset(opts, 0, sizeof(struct qm_mcc_initfq));
  opts->we_mask = QM_INITFQ_WE_FQCTRL | QM_INITFQ_WE_CONTEXTA;
  opts->fqd.fq_ctrl =
      QM_FQCTRL_AVOIDBLOCK | QM_FQCTRL_CTXASTASHING | QM_FQCTRL_PREFERINCACHE;
...
static int dpaa_eth_dev_configure(struct rte_eth_dev *dev) {
  ...
  if (fif->is_shared_mac) {
    ...
    if (!rx_status) {
      DPAA_PMD_ERR("%s Interface not enabled in kernel!", dpaa_intf->name);
      return -EHOSTDOWN;
    }
  }
  ...
  if (!(default_q || fmc_q)) {
    if (dpaa_fm_config(dev, eth_conf->rx_adv_conf.rss_conf.rss_hf)) {
  ...
  if (intr_handle && rte_intr_fd_get(intr_handle)) {
    ...
    ret = dpaa_intr_enable(__fif->node_name, rte_intr_fd_get(intr_handle));
    if (ret) {
      if (dev->data->dev_conf.intr_conf.lsc != 0) {
        rte_intr_callback_unregister(intr_handle, dpaa_interrupt_handler,
                                     (void *)dev);
        if (ret == EINVAL)
...
static const uint32_t *dpaa_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {RTE_PTYPE_L2_ETHER,
...
static void dpaa_interrupt_handler(void *param) {
  ...
  bytes_read = read(rte_intr_fd_get(intr_handle), &buf, sizeof(uint64_t));
  if (bytes_read < 0)
...
static int dpaa_fw_version_get(struct rte_eth_dev *dev __rte_unused,
  ...
  ret = snprintf(fw_version, fw_size, "SVR:%x-fman-v%x", svr_ver, fman_ip_rev);
  if (ret < 0)
    return -EINVAL;
...
static int dpaa_eth_dev_info(struct rte_eth_dev *dev,
  ...
  if (fif->mac_type == fman_mac_1g) {
    ...
    dev_info->speed_capa = RTE_ETH_LINK_SPEED_10M_HD | RTE_ETH_LINK_SPEED_10M |
                           RTE_ETH_LINK_SPEED_100M_HD |
                           RTE_ETH_LINK_SPEED_100M | RTE_ETH_LINK_SPEED_1G |
                           RTE_ETH_LINK_SPEED_2_5G | RTE_ETH_LINK_SPEED_10G;
  } else {
    DPAA_PMD_ERR("invalid link_speed: %s, %d", dpaa_intf->name, fif->mac_type);
    return -EINVAL;
  }
...
static int dpaa_dev_rx_burst_mode_get(struct rte_eth_dev *dev,
...
static int dpaa_dev_tx_burst_mode_get(struct rte_eth_dev *dev,
                                      __rte_unused uint16_t queue_id,
                                      struct rte_eth_burst_mode *mode) {
  struct rte_eth_conf *eth_conf = &dev->data->dev_conf;
  int ret = -EINVAL;
...
static int dpaa_eth_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
  struct dpaa_if *dpaa_intf = dev->data->dev_private;
  struct rte_eth_link *link = &dev->data->dev_link;
  struct fman_if *fif = dev->process_private;
  ...
  if (dev->data->dev_flags & RTE_ETH_DEV_INTR_LSC) {
    for (count = 0; count <= MAX_REPEAT_TIME; count++) {
      ...
      if (ret)
        return ret;
      if (link->link_status == RTE_ETH_LINK_DOWN && wait_to_complete)
  ...
  if (ioctl_version < 2) {
    ...
    if (fif->mac_type == fman_mac_1g)
      ...
      DPAA_PMD_ERR("invalid link_speed: %s, %d", dpaa_intf->name,
                   fif->mac_type);
  }
...
static int dpaa_dev_xstats_get(struct rte_eth_dev *dev,
...
static int dpaa_xstats_get_names(__rte_unused struct rte_eth_dev *dev,
  ...
  if (xstats_names != NULL)
    for (i = 0; i < stat_cnt; i++)
      strlcpy(xstats_names[i].name, dpaa_xstats_strings[i].name,
              sizeof(xstats_names[i].name));

  return stat_cnt;
}

static int dpaa_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
  ...
  if (!ids) {
    ...
    for (i = 0; i < stat_cnt; i++)
      values[i] = values_copy[dpaa_xstats_strings[i].offset / 8];
...
static int dpaa_xstats_get_names_by_id(struct rte_eth_dev *dev,
                                       const uint64_t *ids,
                                       struct rte_eth_xstat_name *xstats_names,
                                       unsigned int limit) {
  unsigned int i, stat_cnt = RTE_DIM(dpaa_xstats_strings);
...
static void dpaa_fman_if_pool_setup(struct rte_eth_dev *dev) {
  ...
  fman_if_set_bp(dev->process_private, dpaa_intf->bp_info->mp->size,
...
static inline int dpaa_eth_rx_queue_bp_check(struct rte_eth_dev *dev,
  ...
  if (dpaa_intf->vsp_bpid[vsp_id] && bpid != dpaa_intf->vsp_bpid[vsp_id]) {
...
static int dpaa_eth_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                                   uint16_t nb_desc,
  ...
  if (queue_idx >= dev->data->nb_rx_queues) {
    rte_errno = EOVERFLOW;
    DPAA_PMD_ERR("%p: queue index out of range (%u >= %u)", (void *)dev,
  ...
  DPAA_PMD_INFO("Rx queue setup for queue index: %d fq_id (0x%x)", queue_idx,
                rxq->fqid);
  ...
  max_rx_pktlen =
      dev->data->mtu + RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN + VLAN_TAG_SIZE;
  /* Max packet can fit in single buffer */
  if (max_rx_pktlen <= buffsz) {
    ;
  } else if (dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_SCATTER) {
  ...
  if (fif->num_profiles) {
    ...
    if (vsp_id >= 0) {
      ret = dpaa_port_vsp_update(dpaa_intf, fmc_q, vsp_id,
                                 DPAA_MEMPOOL_TO_POOL_INFO(mp)->bpid, fif);
      if (ret) {
    ...
    dpaa_intf->vsp_bpid[0] = DPAA_MEMPOOL_TO_POOL_INFO(mp)->bpid;
  }
  ...
  if (!rxq->is_static && dpaa_push_mode_max_queue > dpaa_push_queue_idx) {
    ...
    if (dev->intr_handle == NULL) {
      ...
      if (rte_intr_vec_list_alloc(dev->intr_handle, NULL,
                                  dpaa_push_mode_max_queue)) {
        DPAA_PMD_ERR("intr_vec alloc failed");
        return -ENOMEM;
      }
      if (rte_intr_nb_efd_set(dev->intr_handle, dpaa_push_mode_max_queue))
        return -rte_errno;

      if (rte_intr_max_intr_set(dev->intr_handle, dpaa_push_mode_max_queue))
        return -rte_errno;
    }
    ...
    if (rte_intr_vec_list_index_set(dev->intr_handle, queue_idx, queue_idx + 1))
      return -rte_errno;

    if (rte_intr_efds_index_set(dev->intr_handle, queue_idx, q_fd))
      return -rte_errno;
  ...
  if (dpaa_intf->cgr_rx) {
    ...
    if (ret) {
      DPAA_PMD_WARN("rx taildrop modify fail on fqid %d (ret=%d)", rxq->fqid,
                    ret);
    }
  }
...
int dpaa_eth_eventq_attach(
    const struct rte_eth_dev *dev, int eth_rx_queue_id, u16 ch_id,
...
int dpaa_eth_eventq_detach(const struct rte_eth_dev *dev, int eth_rx_queue_id) {
  ...
  ret = qman_init_fq(rxq, flags, &opts);
  if (ret) {
    DPAA_PMD_ERR("init rx fqid %d failed with ret: %d", rxq->fqid, ret);
  }
...
static int dpaa_eth_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  DPAA_PMD_INFO("Tx queue setup for queue index: %d fq_id (0x%x)", queue_idx,
                txq->fqid);
...
static uint32_t dpaa_dev_rx_queue_count(void *rx_queue) {
  ...
  if (qman_query_fq_frm_cnt(rxq, &frm_cnt) == 0) {
    DPAA_PMD_DEBUG("RX frame count for q(%p) is %u", rx_queue, frm_cnt);
  }
  return frm_cnt;
}
...
static int dpaa_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  if (!(dpaa_intf->fc_conf)) {
    dpaa_intf->fc_conf =
        rte_zmalloc(NULL, sizeof(struct rte_eth_fc_conf), MAX_CACHELINE);
    if (!dpaa_intf->fc_conf) {
  ...
  if (fc_conf->mode == RTE_ETH_FC_NONE) {
    ...
    fman_if_set_fc_threshold(dev->process_private, fc_conf->high_water,
                             fc_conf->low_water, dpaa_intf->bp_info->bpid);
...
static int dpaa_flow_ctrl_get(struct rte_eth_dev *dev,
                              struct rte_eth_fc_conf *fc_conf) {
  struct dpaa_if *dpaa_intf = dev->data->dev_private;
  ...
  if (ret) {
    ...
    fc_conf->pause_time = fman_if_get_fc_quanta(dev->process_private);
  } else {
...
static int dpaa_dev_add_mac_addr(struct rte_eth_dev *dev,
  ...
  ret = fman_if_add_mac_addr(dev->process_private, addr->addr_bytes, index);
...
static void dpaa_dev_remove_mac_addr(struct rte_eth_dev *dev, uint32_t index) {
  PMD_INIT_FUNC_TRACE();

  fman_if_clear_mac_addr(dev->process_private, index);
}

static int dpaa_dev_set_mac_addr(struct rte_eth_dev *dev,
...
static int dpaa_dev_rss_hash_update(struct rte_eth_dev *dev,
...
static int dpaa_dev_rss_hash_conf_get(struct rte_eth_dev *dev,
                                      struct rte_eth_rss_conf *rss_conf) {
  struct rte_eth_dev_data *data = dev->data;
  struct rte_eth_conf *eth_conf = &data->dev_conf;
...
static void dpaa_rxq_info_get(struct rte_eth_dev *dev, uint16_t queue_id,
...
static void dpaa_txq_info_get(struct rte_eth_dev *dev, uint16_t queue_id,
...
static bool is_dpaa_supported(struct rte_eth_dev *dev) {
...
static int dpaa_rx_queue_init(struct qman_fq *fq, struct qman_cgr *cgr_rx,
  ...
  struct qm_mcc_initcgr cgr_opts = {
      .we_mask = QM_CGR_WE_CS_THRES | QM_CGR_WE_CSTD_EN | QM_CGR_WE_MODE,
      .cgr = {.cstd_en = QM_CGR_EN, .mode = QMAN_CGR_MODE_FRAME}};

  if (fmc_q || default_q) {
    ret = qman_reserve_fqid(fqid);
    if (ret) {
      DPAA_PMD_ERR("reserve rx fqid 0x%x failed, ret: %d", fqid, ret);
      return -EINVAL;
    }
  }
  ...
  if (ret) {
    DPAA_PMD_ERR("create rx fqid 0x%x failed with ret: %d", fqid, ret);
    return ret;
  }
  fq->is_static = false;
  ...
  if (cgr_rx) {
    ...
    cgr_rx->cb = NULL;
    ret = qman_create_cgr(cgr_rx, QMAN_CGR_FLAG_USE_INIT, &cgr_opts);
    if (ret) {
      DPAA_PMD_WARN("rx taildrop init fail on rx fqid 0x%x(ret=%d)", fq->fqid,
                    ret);
      goto without_cgr;
    }
    opts.we_mask |= QM_INITFQ_WE_CGID;
...
/* Initialise a Tx FQ */
static int dpaa_tx_queue_init(struct qman_fq *fq, struct fman_if *fman_intf,
  ...
  ret = qman_create_fq(0, QMAN_FQ_FLAG_DYNAMIC_FQID | QMAN_FQ_FLAG_TO_DCPORTAL,
                       fq);
  if (ret) {
  ...
  if (cgr_tx) {
    /* Enable tail drop with cgr on this queue */
    qm_cgr_cs_thres_set64(&cgr_opts.cgr.cs_thres, td_tx_threshold, 0);
    cgr_tx->cb = NULL;
    ret = qman_create_cgr(cgr_tx, QMAN_CGR_FLAG_USE_INIT, &cgr_opts);
    if (ret) {
      DPAA_PMD_WARN("rx taildrop init fail on rx fqid 0x%x(ret=%d)", fq->fqid,
                    ret);
      goto without_cgr;
...
static int dpaa_debug_queue_init(struct qman_fq *fq, uint32_t fqid) {
  ...
  ret = qman_reserve_fqid(fqid);
  if (ret) {
    DPAA_PMD_ERR("Reserve debug fqid %d failed with ret: %d", fqid, ret);
    return -EINVAL;
  }
  ...
  if (ret) {
    DPAA_PMD_ERR("create debug fqid %d failed with ret: %d", fqid, ret);
    return ret;
  }
  opts.we_mask = QM_INITFQ_WE_DESTWQ | QM_INITFQ_WE_FQCTRL;
  ...
  ret = qman_init_fq(fq, 0, &opts);
  if (ret)
    DPAA_PMD_ERR("init debug fqid %d failed with ret: %d", fqid, ret);
  return ret;
}
#endif

/* Initialise a network interface */
static int dpaa_dev_init_secondary(struct rte_eth_dev *eth_dev) {
  ...
  qman_set_fq_lookup_table(dpaa_intf->rx_queues->qman_fq_lookup_table);
#endif
...
/* Initialise a network interface */
static int dpaa_dev_init(struct rte_eth_dev *eth_dev) {
  int num_rx_fqs, fqid;
  int loop, ret = 0;
  int dev_id;
  struct rte_dpaa_device *dpaa_device;
  ...
  memset((char *)dev_rx_fqids, 0, sizeof(uint32_t) * DPAA_MAX_NUM_PCD_QUEUES);
  ...
  if (default_q) {
    ...
    num_rx_fqs = dpaa_port_fmc_init(fman_intf, dev_rx_fqids, dev_vspids,
                                    DPAA_MAX_NUM_PCD_QUEUES);
    if (num_rx_fqs < 0) {
      DPAA_PMD_ERR("%s FMC initializes failed!", dpaa_intf->name);
      goto free_rx;
    }
    if (!num_rx_fqs) {
      DPAA_PMD_WARN("%s is not configured by FMC.", dpaa_intf->name);
    }
  } else {
  ...
  if (num_rx_fqs > 0) {
    dpaa_intf->rx_queues =
        rte_zmalloc(NULL, sizeof(struct qman_fq) * num_rx_fqs, MAX_CACHELINE);
  ...
  if (getenv("DPAA_TX_TAILDROP_THRESHOLD")) {
    ...
    DPAA_PMD_DEBUG("Tail drop threshold env configured: %u", td_tx_threshold);
  ...
  if (num_rx_fqs > 0 && td_threshold) {
    dpaa_intf->cgr_rx =
        rte_zmalloc(NULL, sizeof(struct qman_cgr) * num_rx_fqs, MAX_CACHELINE);
    if (!dpaa_intf->cgr_rx) {
  ...
  for (loop = 0; loop < num_rx_fqs; loop++) {
    ...
    ret = dpaa_rx_queue_init(
        &dpaa_intf->rx_queues[loop],
        dpaa_intf->cgr_rx ? &dpaa_intf->cgr_rx[loop] : NULL, fqid);
    if (ret)
      goto free_rx;
  ...
  dpaa_intf->tx_queues =
      rte_zmalloc(NULL, sizeof(struct qman_fq) * MAX_DPAA_CORES, MAX_CACHELINE);
  ...
  /* If congestion control is enabled globally*/
  if (td_tx_threshold) {
    dpaa_intf->cgr_tx = rte_zmalloc(
        NULL, sizeof(struct qman_cgr) * MAX_DPAA_CORES, MAX_CACHELINE);
    if (!dpaa_intf->cgr_tx) {
    ...
    ret = qman_alloc_cgrid_range(&cgrid_tx[0], MAX_DPAA_CORES, 1, 0);
  ...
  for (loop = 0; loop < MAX_DPAA_CORES; loop++) {
    ...
    ret =
        dpaa_tx_queue_init(&dpaa_intf->tx_queues[loop], fman_intf,
  ...
  ret = dpaa_debug_queue_init(&dpaa_intf->debug_queues[DPAA_DEBUG_FQ_TX_ERROR],
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("mac_addr", RTE_ETHER_ADDR_LEN * DPAA_MAX_MAC_FILTER, 0);
...
static int rte_dpaa_probe(struct rte_dpaa_driver *dpaa_drv,
                          struct rte_dpaa_device *dpaa_dev) {
  int diag;
  int ret;
  ...
  if ((DPAA_MBUF_HW_ANNOTATION + DPAA_FD_PTA_SIZE) > RTE_PKTMBUF_HEADROOM) {
    DPAA_PMD_ERR("RTE_PKTMBUF_HEADROOM(%d) shall be > DPAA Annotation req(%d)",
  ...
  if (!is_global_init && (rte_eal_process_type() == RTE_PROC_PRIMARY)) {
    ...
    if (getenv("DPAA_PUSH_QUEUES_NUMBER")) {
      dpaa_push_mode_max_queue = atoi(getenv("DPAA_PUSH_QUEUES_NUMBER"));
  ...
  eth_dev->data->dev_private = rte_zmalloc(
      "ethdev private structure", sizeof(struct dpaa_if), RTE_CACHE_LINE_SIZE);
...
static int rte_dpaa_remove(struct rte_dpaa_device *dpaa_dev) {
  struct rte_eth_dev *eth_dev;
  int ret;
