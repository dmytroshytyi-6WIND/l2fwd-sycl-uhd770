...
static inline ioc_fm_pcd_extract_entry_t *
...
static void fm_prev_cleanup(void) {
  ...
  while (i < fm_model.dev_count) {
    ...
    dpaa_intf.scheme_handle[0] =
        create_device(fm_info.pcd_handle, fm_model.scheme_devid[devid][0]);
    dpaa_intf.scheme_count = 1;
    if (fm_model.scheme_devid[devid][1]) {
      dpaa_intf.scheme_handle[1] =
          create_device(fm_info.pcd_handle, fm_model.scheme_devid[devid][1]);
      if (dpaa_intf.scheme_handle[1])
        dpaa_intf.scheme_count++;
    }

    dpaa_intf.netenv_handle =
        create_device(fm_info.pcd_handle, fm_model.netenv_devid[devid]);
    i++;
    if (!dpaa_intf.netenv_handle || !dpaa_intf.scheme_handle[0] ||
...
set_hash_params_eth(ioc_fm_pcd_kg_scheme_params_t *scheme_params, int hdr_idx) {
  ...
  for (k = 0; k < 2; k++) {
    SCH_EXT_ARR(scheme_params, hdr_idx)->type = e_IOC_FM_PCD_EXTRACT_BY_HDR;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr = HEADER_TYPE_ETH;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr_index = e_IOC_FM_PCD_HDR_INDEX_NONE;
    SCH_EXT_HDR(scheme_params, hdr_idx).type = e_IOC_FM_PCD_EXTRACT_FULL_FIELD;
    if (k == 0)
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).eth = IOC_NET_HF_ETH_SA;
    else
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).eth = IOC_NET_HF_ETH_DA;
    hdr_idx++;
  }
  return hdr_idx;
}
...
static inline int
set_hash_params_ipv4(ioc_fm_pcd_kg_scheme_params_t *scheme_params,
                     int hdr_idx) {
  int k;

  for (k = 0; k < 2; k++) {
    SCH_EXT_ARR(scheme_params, hdr_idx)->type = e_IOC_FM_PCD_EXTRACT_BY_HDR;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr = HEADER_TYPE_IPV4;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr_index = e_IOC_FM_PCD_HDR_INDEX_NONE;
    SCH_EXT_HDR(scheme_params, hdr_idx).type = e_IOC_FM_PCD_EXTRACT_FULL_FIELD;
    if (k == 0)
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).ipv4 = ioc_net_hf_ipv_4_src_ip;
    else
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).ipv4 = ioc_net_hf_ipv_4_dst_ip;
    hdr_idx++;
  }
  return hdr_idx;
}
...
static inline int
set_hash_params_ipv6(ioc_fm_pcd_kg_scheme_params_t *scheme_params,
                     int hdr_idx) {
  int k;

  for (k = 0; k < 2; k++) {
    SCH_EXT_ARR(scheme_params, hdr_idx)->type = e_IOC_FM_PCD_EXTRACT_BY_HDR;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr = HEADER_TYPE_IPV6;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr_index = e_IOC_FM_PCD_HDR_INDEX_NONE;
    SCH_EXT_HDR(scheme_params, hdr_idx).type = e_IOC_FM_PCD_EXTRACT_FULL_FIELD;
    if (k == 0)
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).ipv6 = ioc_net_hf_ipv_6_src_ip;
    else
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).ipv6 = ioc_net_hf_ipv_6_dst_ip;
    hdr_idx++;
  }
  return hdr_idx;
}
...
set_hash_params_udp(ioc_fm_pcd_kg_scheme_params_t *scheme_params, int hdr_idx) {
  ...
  for (k = 0; k < 2; k++) {
    SCH_EXT_ARR(scheme_params, hdr_idx)->type = e_IOC_FM_PCD_EXTRACT_BY_HDR;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr = HEADER_TYPE_UDP;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr_index = e_IOC_FM_PCD_HDR_INDEX_NONE;
    SCH_EXT_HDR(scheme_params, hdr_idx).type = e_IOC_FM_PCD_EXTRACT_FULL_FIELD;
    if (k == 0)
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).udp = IOC_NET_HF_UDP_PORT_SRC;
    else
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).udp = IOC_NET_HF_UDP_PORT_DST;
    hdr_idx++;
  }
  return hdr_idx;
}
...
set_hash_params_tcp(ioc_fm_pcd_kg_scheme_params_t *scheme_params, int hdr_idx) {
  ...
  for (k = 0; k < 2; k++) {
    SCH_EXT_ARR(scheme_params, hdr_idx)->type = e_IOC_FM_PCD_EXTRACT_BY_HDR;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr = HEADER_TYPE_TCP;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr_index = e_IOC_FM_PCD_HDR_INDEX_NONE;
    SCH_EXT_HDR(scheme_params, hdr_idx).type = e_IOC_FM_PCD_EXTRACT_FULL_FIELD;
    if (k == 0)
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).tcp = IOC_NET_HF_TCP_PORT_SRC;
    else
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).tcp = IOC_NET_HF_TCP_PORT_DST;
    hdr_idx++;
  }
  return hdr_idx;
}
...
static inline int
set_hash_params_sctp(ioc_fm_pcd_kg_scheme_params_t *scheme_params,
                     int hdr_idx) {
  int k;

  for (k = 0; k < 2; k++) {
    SCH_EXT_ARR(scheme_params, hdr_idx)->type = e_IOC_FM_PCD_EXTRACT_BY_HDR;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr = HEADER_TYPE_SCTP;
    SCH_EXT_HDR(scheme_params, hdr_idx).hdr_index = e_IOC_FM_PCD_HDR_INDEX_NONE;
    SCH_EXT_HDR(scheme_params, hdr_idx).type = e_IOC_FM_PCD_EXTRACT_FULL_FIELD;
    if (k == 0)
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).sctp = IOC_NET_HF_SCTP_PORT_SRC;
    else
      SCH_EXT_FULL_FLD(scheme_params, hdr_idx).sctp = IOC_NET_HF_SCTP_PORT_DST;
    hdr_idx++;
  }
  return hdr_idx;
}
...
static int set_scheme_params(ioc_fm_pcd_kg_scheme_params_t *scheme_params,
                             ...
                             struct dpaa_if *dpaa_intf, struct fman_if *fif) {
  ...
  scheme_params->param.net_env_params.net_env_id = dpaa_intf->netenv_handle;
  ...
  for (dist_idx = 0; dist_idx < dist_units->param.num_of_distinction_units;
       dist_idx++) {
...
static void set_dist_units(ioc_fm_pcd_net_env_params_t *dist_units,
  ...
  if (!req_dist_set)
    dist_units->param.units[dist_idx++].hdrs[0].hdr = HEADER_TYPE_ETH;
    ...
    if (req_dist_set % 2 != 0) {
      ...
      case RTE_ETH_RSS_NONFRAG_IPV4_OTHER:
        ...
        if (ipv4_configured)
          break;
        ipv4_configured = 1;
        dist_units->param.units[dist_idx++].hdrs[0].hdr = HEADER_TYPE_IPV4;
        break;
      ...
      case RTE_ETH_RSS_IPV6_EX:
        ...
        if (ipv6_configured)
          break;
        ipv6_configured = 1;
        dist_units->param.units[dist_idx++].hdrs[0].hdr = HEADER_TYPE_IPV6;
        break;
      ...
      case RTE_ETH_RSS_IPV6_TCP_EX:
        ...
        if (tcp_configured)
          break;
        tcp_configured = 1;
        dist_units->param.units[dist_idx++].hdrs[0].hdr = HEADER_TYPE_TCP;
        break;
      ...
      case RTE_ETH_RSS_IPV6_UDP_EX:
        ...
        if (udp_configured)
          break;
        udp_configured = 1;
        dist_units->param.units[dist_idx++].hdrs[0].hdr = HEADER_TYPE_UDP;
        break;
      ...
      case RTE_ETH_RSS_NONFRAG_IPV6_SCTP:
        ...
        dist_units->param.units[dist_idx++].hdrs[0].hdr = HEADER_TYPE_SCTP;
        break;
...
static inline void unset_pcd_netenv_scheme(struct dpaa_if *dpaa_intf) {
  ...
  DPAA_PMD_DEBUG("KG SCHEME DEL %d handle =%p", dpaa_intf->scheme_count,
...
static inline int set_default_scheme(struct dpaa_if *dpaa_intf) {
   ...
   * every scheme.
   */
  scheme_params.param.scm_id.relative_scheme_id = 10 + dpaa_intf->ifid;
  ...
  DPAA_PMD_DEBUG("KG SCHEME SET %d handle =%p", idx,
                 dpaa_intf->scheme_handle[idx]);
...
static inline int set_fm_port_handle(struct dpaa_if *dpaa_intf,
  ...
  dpaa_intf->netenv_handle =
      fm_pcd_net_env_characteristics_set(fm_info.pcd_handle, &dist_units);
...
int dpaa_fm_deconfig(struct dpaa_if *dpaa_intf,
  ...
  for (idx = 0; idx < dpaa_intf->scheme_count; idx++) {
    DPAA_PMD_DEBUG("KG SCHEME DEL %d, handle =%p", idx,
                   dpaa_intf->scheme_handle[idx]);
...
static int dpaa_port_vsp_configure(struct dpaa_if *dpaa_intf, uint8_t vsp_id,
                                   t_handle fman_handle, struct fman_if *fif) {
  ...
  if (vsp_id >= DPAA_VSP_PROFILE_MAX_NUM) {
    DPAA_PMD_ERR("VSP ID %d exceeds MAX number %d", vsp_id,
                 DPAA_VSP_PROFILE_MAX_NUM);
    return -1;
  }
  ...
  vsp_params.ext_buf_pools.ext_buf_pool[0].size = RTE_MBUF_DEFAULT_BUF_SIZE;
  ...
  if (ret != E_OK) {
    DPAA_PMD_ERR(
        "fm_vsp_config_buffer_prefix_content error for profile %d err: %d",
        vsp_id, ret);
    return ret;
  }
  ...
  if (ret != E_OK) {
    DPAA_PMD_ERR("fm_vsp_init error for profile %d err:%d", vsp_id, ret);
    return ret;
  }
...
int dpaa_port_vsp_update(struct dpaa_if *dpaa_intf, bool fmc_mode,
                         uint8_t vsp_id, uint32_t bpid, struct fman_if *fif) {
  int ret = 0;
  t_handle fman_handle;
  ...
  if (dpaa_intf->vsp_handle[vsp_id]) {
    ...
    if (ret != E_OK) {
      DPAA_PMD_ERR("Error fm_vsp_free: err %d vsp_handle[%d]", ret, vsp_id);
      return ret;
    }
