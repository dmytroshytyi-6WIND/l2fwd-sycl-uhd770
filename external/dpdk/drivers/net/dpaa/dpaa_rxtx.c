...
static void dpaa_display_frame_info(const struct qm_fd *fd, uint32_t fqid,
                                    bool rx) {
  int ii;
  char *ptr;
  ...
  DISPLAY_PRINT("fqid %d bpid %d addr 0x%lx, format %d\r\n", fqid, fd->bpid,
                (unsigned long)fd->addr, fd->format);
  DISPLAY_PRINT("off %d, len %d stat 0x%x\r\n", fd->offset, fd->length20,
                fd->status);
  if (rx) {
    ptr = (char *)&annot->parse;
    DISPLAY_PRINT("RX parser result:\r\n");
    for (ii = 0; ii < (int)sizeof(struct dpaa_eth_parse_results_t); ii++) {
...
static inline void dpaa_eth_packet_info(struct rte_mbuf *m,
  ...
  case DPAA_PKT_TYPE_IPV4_FRAG_SCTP:
    m->packet_type = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_FRAG;
    break;
...
static inline void dpaa_checksum(struct rte_mbuf *mbuf) {
  ...
  if ((mbuf->packet_type & RTE_PTYPE_L4_MASK) == RTE_PTYPE_L4_TCP) {
    ...
    struct rte_udp_hdr *udp_hdr = (struct rte_udp_hdr *)(l3_hdr + mbuf->l3_len);
...
static inline void dpaa_unsegmented_checksum(struct rte_mbuf *mbuf,
                                             struct qm_fd *fd_arr) {
  if (!mbuf->packet_type) {
    struct rte_net_hdr_lens hdr_lens;

    mbuf->packet_type = rte_net_get_ptype(
        mbuf, &hdr_lens,
  ...
  if (mbuf->data_off <
      (DEFAULT_TX_ICEOF + sizeof(struct dpaa_eth_parse_results_t))) {
...
static struct rte_mbuf *dpaa_eth_sg_to_mbuf(const struct qm_fd *fd,
                                            uint32_t ifid) {
...
/* Specific for LS1043 */
void dpaa_rx_cb_no_prefetch(struct qman_fq **fq, struct qm_dqrr_entry **dqrr,
  ...
  for (i = 0; i < num_bufs; i++) {
    if (i < num_bufs - 1) {
      ...
      rte_prefetch0((void *)((uint8_t *)ptr + DEFAULT_RX_ICEOF));
      bufs[i + 1] = (struct rte_mbuf *)((char *)ptr - bp_info->meta_data_size);
    }
...
void dpaa_rx_cb(struct qman_fq **fq, struct qm_dqrr_entry **dqrr, void **bufs,
                int num_bufs) {
  struct rte_mbuf *mbuf;
  const struct qm_fd *fd;
  struct dpaa_if *dpaa_intf;
...
static uint16_t dpaa_eth_queue_portal_rx(struct qman_fq *fq,
...
dpaa_rx_cb_parallel(void *event, struct qman_portal *qm __always_unused,
...
enum qman_cb_dqrr_result
dpaa_rx_cb_atomic(void *event, struct qman_portal *qm __always_unused,
                  struct qman_fq *fq, const struct qm_dqrr_entry *dqrr,
                  void **bufs) {
  u8 index;
  u32 ifid = ((struct dpaa_if *)fq->dpaa_intf)->ifid;
  ...
  for (i = 0; i <= DPAA_DEBUG_FQ_TX_ERROR; i++) {
    ...
    do {
      ...
      if (i == DPAA_DEBUG_FQ_RX_ERROR)
        DPAA_PMD_ERR("RX ERROR status: 0x%08x", fd->status);
      else
        DPAA_PMD_ERR("TX ERROR status: 0x%08x", fd->status);
      dpaa_display_frame_info(fd, debug_fq->fqid, i == DPAA_DEBUG_FQ_RX_ERROR);
...
static int dpaa_eth_mbuf_to_sg_fd(struct rte_mbuf *mbuf, struct qm_fd *fd,
                                  struct dpaa_bp_info *bp_info) {
  ...
  if (temp->buf_len <
      ((mbuf->nb_segs * sizeof(struct qm_sg_entry)) + temp->data_off)) {
...
static inline void tx_on_dpaa_pool_unsegmented(struct rte_mbuf *mbuf,
                                               struct dpaa_bp_info *bp_info,
                                               struct qm_fd *fd_arr) {
...
static inline uint16_t tx_on_dpaa_pool(struct rte_mbuf *mbuf,
                                       struct dpaa_bp_info *bp_info,
                                       struct qm_fd *fd_arr) {
...
static inline struct rte_mbuf *reallocate_mbuf(struct qman_fq *txq,
                                               struct rte_mbuf *mbuf) {
  ...
  mbufs_size = bp_info->size - bp_info->meta_data_size - RTE_PKTMBUF_HEADROOM;
  ...
  while (temp_mbuf) {
    ...
    } else if ((temp_mbuf->data_len - offset1) > (mbufs_size - offset2)) {
    ...
    rte_memcpy((uint8_t *)data, rte_pktmbuf_mtod_offset(mbuf, void *, offset1),
               bytes_to_copy);
...
uint16_t dpaa_eth_queue_tx(void *q, struct rte_mbuf **bufs, uint16_t nb_bufs) {
  ...
  while (nb_bufs) {
    frames_to_send =
        (nb_bufs > DPAA_TX_BURST_SIZE) ? DPAA_TX_BURST_SIZE : nb_bufs;
    for (loop = 0; loop < frames_to_send; loop++) {
      ...
      if (seqn != DPAA_INVALID_MBUF_SEQN) {
        ...
        if (DPAA_PER_LCORE_DQRR_HELD & (1 << index)) {
          flags[loop] = ((index & QM_EQCR_DCA_IDXMASK) << 8);
      ...
      if (likely(RTE_MBUF_DIRECT(mbuf))) {
        ...
        if (likely(mp->ops_index == bp_info->dpaa_ops_index &&
                   mbuf->nb_segs == 1 && realloc_mbuf == 0 &&
                   rte_mbuf_refcnt_read(mbuf) == 1)) {
          DPAA_MBUF_TO_CONTIG_FD(mbuf, &fd_arr[loop], bp_info->bpid);
          if (mbuf->ol_flags & DPAA_TX_CKSUM_OFFLOAD_MASK)
    ...
    indirect_buf:
      state = tx_on_dpaa_pool(mbuf, bp_info, &fd_arr[loop]);
      if (unlikely(state)) {
        /* Set frames_to_send & nb_bufs so
         * that packets are transmitted till
         * previous frame.
  ...
  send_pkts:
    ...
    while (loop < frames_to_send) {
      loop += qman_enqueue_multi(q, &fd_arr[loop], &flags[loop],
                                 frames_to_send - loop);
    }
