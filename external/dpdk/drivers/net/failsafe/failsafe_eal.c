...
static int fs_ethdev_portid_get(const char *name, uint16_t *port_id) {
  uint16_t pid;
  size_t len;
...
static int fs_bus_init(struct rte_eth_dev *dev) {
  ...
  FOREACH_SUBDEV(sdev, i, dev) {
    ...
    if (fs_ethdev_portid_get(da->name, &pid) != 0) {
      ...
      if (ret < 0) {
        ERROR("sub_device %d probe failed %s%s%s", i, rte_errno ? "(" : "",
              rte_errno ? strerror(rte_errno) : "", rte_errno ? ")" : "");
        continue;
      }
      if (fs_ethdev_portid_get(da->name, &pid) != 0) {
      ...
      if (pid_owner.id != PRIV(dev)->my_owner.id) {
        INFO("sub_device %d owner(%s_%016" PRIX64 ") is not my,"
             ...
             i, pid_owner.name, pid_owner.id, PRIV(dev)->my_owner.name,
      ...
      struct rte_devargs *probed_da = rte_eth_devices[pid].device->devargs;

      /* Take control of probed device. */
      rte_devargs_reset(da);
      memset(da, 0, sizeof(*da));
      if (probed_da != NULL)
        snprintf(devstr, sizeof(devstr), "%s,%s", probed_da->name,
                 probed_da->args);
      else
        strlcpy(devstr, rte_eth_devices[pid].device->name, sizeof(devstr));
      ...
      if (ret < 0) {
        ...
        INFO("sub_device %d was removed before taking"
             " ownership, will try again later",
             i);
        continue;
      }
    }
...
static int fs_bus_uninit(struct rte_eth_dev *dev) {
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_PROBED) {
    ...
    if (sdev_ret < 0) {
      ERROR("Failed to remove requested device %s (err: %d)", sdev->dev->name,
            sdev_ret);
      continue;
    }
