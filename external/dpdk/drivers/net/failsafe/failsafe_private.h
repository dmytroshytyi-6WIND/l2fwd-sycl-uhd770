...
#define FS_RX_PROXY_INIT                                                       \
  (struct rx_proxy) {                                                          \
    .efd = -1, .evec = NULL, .sid = 0, .scid = 0, .sstate = SS_NO_SERVICE,     \
  }
...
uint16_t failsafe_rx_burst(void *rxq, struct rte_mbuf **rx_pkts,
                           uint16_t nb_pkts);
uint16_t failsafe_tx_burst(void *txq, struct rte_mbuf **tx_pkts,
                           uint16_t nb_pkts);

uint16_t failsafe_rx_burst_fast(void *rxq, struct rte_mbuf **rx_pkts,
                                uint16_t nb_pkts);
uint16_t failsafe_tx_burst_fast(void *txq, struct rte_mbuf **tx_pkts,
                                uint16_t nb_pkts);
...
int failsafe_eth_rmv_event_callback(uint16_t port_id,
                                    enum rte_eth_event_type type, void *arg,
                                    void *out);
...
#define PRIV(dev) ((struct fs_priv *)(dev)->data->dev_private)

/* sdev: (struct sub_device *) */
#define ETH(sdev)                                                              \
  ((sdev)->sdev_port_id == RTE_MAX_ETHPORTS                                    \
       ? NULL                                                                  \
       : &rte_eth_devices[(sdev)->sdev_port_id])

/* sdev: (struct sub_device *) */
#define PORT_ID(sdev) ((sdev)->sdev_port_id)

/* sdev: (struct sub_device *) */
#define SUB_ID(sdev) ((sdev)->sid)
...
#define FOREACH_SUBDEV_STATE(s, i, dev, state)                                 \
  for (s = fs_find_next((dev), 0, state, &i); s != NULL;                       \
 ...
 * dev: (struct rte_eth_dev *), fail-safe ethdev
 */
#define FOREACH_SUBDEV(s, i, dev) FOREACH_SUBDEV_STATE(s, i, dev, DEV_UNDEFINED)

/* dev: (struct rte_eth_dev *) fail-safe device */
#define PREFERRED_SUBDEV(dev) (&PRIV(dev)->subs[0])

/* dev: (struct rte_eth_dev *) fail-safe device */
#define TX_SUBDEV(dev)                                                         \
  (PRIV(dev)->subs_tx >= PRIV(dev)->subs_tail                                  \
       ? NULL                                                                  \
       : (PRIV(dev)->subs[PRIV(dev)->subs_tx].state < DEV_PROBED               \
              ? NULL                                                           \
 ...
 */
#define SUBOPS(s, ops) (ETH(s)->dev_ops->ops)
 ...
 * a: (rte_atomic64_t)
 */
#define FS_ATOMIC_P(a) rte_atomic64_set(&(a), 1)

/**
 * a: (rte_atomic64_t)
 */
#define FS_ATOMIC_V(a) rte_atomic64_set(&(a), 0)
...
#define FS_ATOMIC_RX(s, i)                                                     \
  rte_atomic64_read(                                                           \
      &((struct rxq *)(fs_dev(s)->data->rx_queues[i]))->refcnt[(s)->sid])
/**
 * s: (struct sub_device *)
 * i: uint16_t qid
 */
#define FS_ATOMIC_TX(s, i)                                                     \
  rte_atomic64_read(                                                           \
      &((struct txq *)(fs_dev(s)->data->tx_queues[i]))->refcnt[(s)->sid])
...
#define LOG__(l, m, ...)                                                       \
  rte_log(RTE_LOG_##l, failsafe_logtype, "net_failsafe: " m "%c", __VA_ARGS__)
...
static inline struct sub_device *fs_find_next(struct rte_eth_dev *dev,
                                              uint8_t sid,
...
static inline struct rte_eth_dev *fs_dev(struct sub_device *sdev) {
 ...
 */
static inline int fs_lock(struct rte_eth_dev *dev, unsigned int is_alarm) {
  int ret;
 ...
 * is_alarm means that the caller is, for sure, the hot-plug alarm mechanism.
 */
static inline void fs_unlock(struct rte_eth_dev *dev, unsigned int is_alarm) {
  int ret;
 ...
 */
static inline void fs_switch_dev(struct rte_eth_dev *dev,
  ...
  if (PREFERRED_SUBDEV(dev)->state >= req_state &&
    ...
    if (txd != PREFERRED_SUBDEV(dev) &&
        (txd == NULL || (req_state == DEV_STARTED) ||
         (txd && txd->state < DEV_STARTED))) {
      DEBUG("Switching tx_dev to preferred sub_device");
      PRIV(dev)->subs_tx = 0;
    }
  } else if ((txd && txd->state < req_state) || txd == NULL || txd == banned) {
 ...
 */
static inline int fs_err(struct sub_device *sdev, int err) {
