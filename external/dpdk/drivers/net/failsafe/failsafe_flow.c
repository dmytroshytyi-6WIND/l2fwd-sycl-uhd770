...
fs_flow_allocate(const struct rte_flow_attr *attr,
  ...
  if (ret < 0) {
    ERROR("Unable to process flow rule (%s): %s",
          error.message ? error.message : "unspecified", strerror(rte_errno));
    return NULL;
  }
...
static void fs_flow_release(struct rte_flow **flow) {
  rte_free(*flow);
  *flow = NULL;
}

static int fs_flow_validate(struct rte_eth_dev *dev,
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    ret = rte_flow_validate(PORT_ID(sdev), attr, patterns, actions, error);
...
static struct rte_flow *fs_flow_create(struct rte_eth_dev *dev,
                                       const struct rte_flow_attr *attr,
                                       const struct rte_flow_item patterns[],
                                       const struct rte_flow_action actions[],
                                       struct rte_flow_error *error) {
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    flow->flows[i] =
        rte_flow_create(PORT_ID(sdev), attr, patterns, actions, error);
    if (flow->flows[i] == NULL && fs_err(sdev, -rte_errno)) {
      ERROR("Failed to create flow on sub_device %d", i);
      goto err;
    }
  }
...
err:
  FOREACH_SUBDEV(sdev, i, dev) {
    if (flow->flows[i] != NULL)
      rte_flow_destroy(PORT_ID(sdev), flow->flows[i], error);
  }
...
static int fs_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                           struct rte_flow_error *error) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    if (flow->flows[i] == NULL)
      continue;
    local_ret = rte_flow_destroy(PORT_ID(sdev), flow->flows[i], error);
    if ((local_ret = fs_err(sdev, local_ret))) {
      ERROR("Failed to destroy flow on sub_device %d: %d", i, local_ret);
      if (ret == 0)
        ret = local_ret;
    }
  }
...
static int fs_flow_flush(struct rte_eth_dev *dev,
                         struct rte_flow_error *error) {
  struct sub_device *sdev;
  struct rte_flow *flow;
  void *tmp;
...
static int fs_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  sdev = TX_SUBDEV(dev);
  if (sdev != NULL) {
    int ret = rte_flow_query(PORT_ID(sdev), flow->flows[SUB_ID(sdev)], action,
                             arg, error);
...
static int fs_flow_isolate(struct rte_eth_dev *dev, int set,
                           struct rte_flow_error *error) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
  ...
  FOREACH_SUBDEV(sdev, i, dev) {
    ...
    if (PRIV(dev)->flow_isolated != sdev->flow_isolated)
      WARN("flow isolation mode of sub_device %d in incoherent state.", i);
