...
static int fs_flow_complain(struct rte_flow_error *error) {
...
static int eth_dev_flow_isolate_set(struct rte_eth_dev *dev,
  ...
  if (!PRIV(dev)->flow_isolated) {
    ...
    ret = rte_flow_isolate(PORT_ID(sdev), PRIV(dev)->flow_isolated, &ferror);
    if (ret) {
...
static int fs_eth_dev_conf_apply(struct rte_eth_dev *dev,
                                 struct sub_device *sdev) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    ret = rte_eth_rx_queue_setup(PORT_ID(sdev), i, rxq->info.nb_desc,
                                 rxq->socket_id, &rxq->info.conf, rxq->info.mp);
    if (ret) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    ret = rte_eth_tx_queue_setup(PORT_ID(sdev), i, txq->info.nb_desc,
                                 txq->socket_id, &txq->info.conf);
    if (ret) {
  ...
  if (dev->data->dev_link.link_status != edev->data->dev_link.link_status) {
  ...
  ret =
      rte_eth_dev_default_mac_addr_set(PORT_ID(sdev), &dev->data->mac_addrs[0]);
  if (ret) {
  ...
  if (PRIV(dev)->nb_mcast_addr > 0) {
    ...
    ret = rte_eth_dev_set_mc_addr_list(PORT_ID(sdev), PRIV(dev)->mcast_addrs,
  ...
  if (memcmp(vfc1, vfc2, sizeof(struct rte_vlan_filter_conf))) {
    ...
    for (i = 0; i < RTE_DIM(vfc1->ids); i++) {
      ...
      while (ids) {
        ...
        for (; vbit; vlan_id++)
          vbit >>= 1;
        ret = rte_eth_dev_vlan_filter(PORT_ID(sdev), vlan_id, 1);
        if (ret) {
          ERROR("Failed to apply VLAN filter %hu", vlan_id);
          return ret;
        }
      }
    }
  ...
  if (TAILQ_EMPTY(&PRIV(dev)->flow_list)) {
    ...
    TAILQ_FOREACH(flow, &PRIV(dev)->flow_list, next) {
      ...
      flow->flows[SUB_ID(sdev)] =
          rte_flow_create(PORT_ID(sdev), flow->rule.attr, flow->rule.pattern,
...
static void fs_dev_remove(struct sub_device *sdev) {
  int ret;
  ...
  case DEV_ACTIVE:
    ...
    if (ret < 0) {
      ERROR("Port close failed for sub-device %u", PORT_ID(sdev));
    }
  ...
  case DEV_PROBED:
    ...
    if (ret < 0) {
      ERROR("Bus detach failed for sub_device %u", SUB_ID(sdev));
    } else {
...
static void fs_dev_stats_save(struct sub_device *sdev) {
  ...
  failsafe_stats_increment(&PRIV(fs_dev(sdev))->stats_accumulator,
                           err ? &sdev->stats_snapshot.stats : &stats);
  memset(&sdev->stats_snapshot, 0, sizeof(sdev->stats_snapshot));
}

static inline int fs_rxtx_clean(struct sub_device *sdev) {
  uint16_t i;
...
void failsafe_eth_dev_unregister_callbacks(struct sub_device *sdev) {
  ...
  if (sdev->rmv_callback) {
    ret =
        rte_eth_dev_callback_unregister(PORT_ID(sdev), RTE_ETH_EVENT_INTR_RMV,
  ...
  if (sdev->lsc_callback) {
    ret =
        rte_eth_dev_callback_unregister(PORT_ID(sdev), RTE_ETH_EVENT_INTR_LSC,
                                        failsafe_eth_lsc_event_callback, sdev);
    if (ret)
...
void failsafe_dev_remove(struct rte_eth_dev *dev) {
  ...
  FOREACH_SUBDEV(sdev, i, dev) {
     ...
     */
    if (sdev->state >= DEV_ACTIVE && fs_rxtx_clean(sdev) == 0)
      continue;
    if (fs_lock(dev, 1) != 0)
      return;
...
static int failsafe_eth_dev_rx_queues_sync(struct rte_eth_dev *dev) {
  struct rxq *rxq;
  int ret;
  uint16_t i;
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    } else if (dev->data->rx_queue_state[i] == RTE_ETH_QUEUE_STATE_STOPPED) {
      /*
...
static int failsafe_eth_dev_tx_queues_sync(struct rte_eth_dev *dev) {
  struct txq *txq;
  int ret;
  uint16_t i;
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    } else if (dev->data->tx_queue_state[i] == RTE_ETH_QUEUE_STATE_STOPPED) {
      /*
...
int failsafe_eth_dev_state_sync(struct rte_eth_dev *dev) {
  ...
  FOREACH_SUBDEV(sdev, i, dev) {
    if (sdev->state == DEV_PROBED) {
      ...
      if (ret) {
        ERROR("Could not apply configuration to sub_device %d", i);
        goto err_remove;
      }
    }
  }
...
int failsafe_eth_rmv_event_callback(uint16_t port_id __rte_unused,
...
int failsafe_eth_lsc_event_callback(uint16_t port_id __rte_unused,
                                    enum rte_eth_event_type event __rte_unused,
                                    void *cb_arg, void *out __rte_unused) {
  ...
  if (ret)
    return rte_eth_dev_callback_process(dev, RTE_ETH_EVENT_INTR_LSC, NULL);
  else
    return 0;
}
