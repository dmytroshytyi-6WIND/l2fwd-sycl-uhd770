 ...
 */
static int fs_epoll_create1(int flags) {
#if defined(LINUX)
 ...
 *   0 on success, negative errno value otherwise.
 */
static int fs_rx_event_proxy_routine(void *data) {
  struct fs_priv *priv;
  struct rxq *rxq;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (rxq->enable_events && rxq->event_fd != -1) {
      if (write(rxq->event_fd, &u64, sizeof(u64)) != sizeof(u64)) {
        ERROR("Failed to proxy Rx event to socket %d", rxq->event_fd);
        rc = -EIO;
      }
    }
  }
 ...
 *   Pointer to failsafe private structure.
 */
static void fs_rx_event_proxy_service_uninstall(struct fs_priv *priv) {
 ...
 *   0 on success, negative errno value otherwise.
 */
static int fs_rx_event_proxy_service_install(struct fs_priv *priv) {
  ...
  if (priv->rxp.sstate == SS_NO_SERVICE) {
    ...
    int32_t ret = rte_service_component_register(&service, &priv->rxp.sid);
    if (ret) {
    ...
    ret = rte_service_map_lcore_set(priv->rxp.sid, priv->rxp.scid, 1);
    if (ret) {
      ERROR("Failed to install Rx interrupts, "
 ...
 */
static int fs_rx_event_proxy_install(struct fs_priv *priv) {
  int rc = 0;
 ...
 */
static int failsafe_eth_rx_intr_ctl_subdevice(struct sub_device *sdev, int op) {
  ...
  fsdev = fs_dev(sdev);
  if (sdev == NULL || (ETH(sdev) == NULL) || fsdev == NULL ||
      (PRIV(fsdev) == NULL)) {
  ...
  for (qid = 0; qid < dev->data->nb_rx_queues; qid++) {
    ...
    rc = rte_eth_dev_rx_intr_ctl_q(pid, qid, epfd, op, (void *)fsrxq);
    if (rc) {
...
int failsafe_rx_intr_install_subdevice(struct sub_device *sdev) {
  ...
  for (qid = 0; qid < ETH(sdev)->data->nb_rx_queues; qid++) {
    if (rxq[qid]->enable_events) {
      int ret = rte_eth_dev_rx_intr_enable(PORT_ID(sdev), qid);
      if (ret && (ret != -ENOTSUP)) {
 ...
 *   Pointer to failsafe private structure.
 */
static void fs_rx_event_proxy_uninstall(struct fs_priv *priv) {
 ...
 *   Pointer to failsafe private structure.
 */
static void fs_rx_intr_vec_uninstall(struct fs_priv *priv) {
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int fs_rx_intr_vec_install(struct fs_priv *priv) {
  unsigned int i;
  unsigned int rxqs_n;
  unsigned int n;
  unsigned int count;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (rte_intr_efds_index_set(intr_handle, count, rxq->event_fd))
      return -rte_errno;
    count++;
  }
  if (count == 0) {
    ...
    if (rte_intr_efd_counter_size_set(intr_handle, sizeof(uint64_t)))
      return -rte_errno;
  }
  return 0;
}
