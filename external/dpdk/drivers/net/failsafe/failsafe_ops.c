...
static int fs_dev_configure(struct rte_eth_dev *dev) {
  ...
  FOREACH_SUBDEV(sdev, i, dev) {
    ...
    rmv_interrupt = ETH(sdev)->data->dev_flags & RTE_ETH_DEV_INTR_RMV;
    if (rmv_interrupt) {
    ...
    lsc_interrupt =
        lsc_enabled && (ETH(sdev)->data->dev_flags & RTE_ETH_DEV_INTR_LSC);
    if (lsc_interrupt) {
    ...
    ret = rte_eth_dev_configure(PORT_ID(sdev), dev->data->nb_rx_queues,
    ...
    if (rmv_interrupt && sdev->rmv_callback == 0) {
      ret =
          rte_eth_dev_callback_register(PORT_ID(sdev), RTE_ETH_EVENT_INTR_RMV,
                                        failsafe_eth_rmv_event_callback, sdev);
      if (ret)
        WARN("Failed to register RMV callback for sub_device %d", SUB_ID(sdev));
      else
        sdev->rmv_callback = 1;
    }
    dev->data->dev_conf.intr_conf.rmv = 0;
    if (lsc_interrupt && sdev->lsc_callback == 0) {
      ret = rte_eth_dev_callback_register(PORT_ID(sdev), RTE_ETH_EVENT_INTR_LSC,
                                          failsafe_eth_lsc_event_callback, dev);
      if (ret)
        WARN("Failed to register LSC callback for sub_device %d", SUB_ID(sdev));
      else
        sdev->lsc_callback = 1;
    }
...
static void fs_set_queues_state_start(struct rte_eth_dev *dev) {
  struct rxq *rxq;
  struct txq *txq;
  uint16_t i;
...
static int fs_dev_start(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
...
static void fs_set_queues_state_stop(struct rte_eth_dev *dev) {
  uint16_t i;
...
static int fs_dev_stop(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
...
static int fs_dev_set_link_up(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
...
static int fs_dev_set_link_down(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
...
static int fs_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
  int err = 0;
...
static int fs_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
...
static int fs_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
  int err = 0;
...
static int fs_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
...
static void fs_rx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
  struct sub_device *sdev;
  uint8_t i;
...
static int fs_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
  ...
  rxq = rte_zmalloc(
      NULL, sizeof(*rxq) + sizeof(rte_atomic64_t) * PRIV(dev)->subs_tail,
      RTE_CACHE_LINE_SIZE);
  if (rxq == NULL) {
    fs_unlock(dev, 0);
    return -ENOMEM;
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ret = rte_eth_rx_queue_setup(PORT_ID(sdev), rx_queue_id, nb_rx_desc,
...
static int fs_rx_intr_enable(struct rte_eth_dev *dev, uint16_t idx) {
  struct rxq *rxq;
  struct sub_device *sdev;
  uint8_t i;
  int ret;
...
static int fs_rx_intr_disable(struct rte_eth_dev *dev, uint16_t idx) {
  struct rxq *rxq;
  struct sub_device *sdev;
  uint64_t u64;
  uint8_t i;
...
static void fs_tx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
  struct sub_device *sdev;
  uint8_t i;
...
static int fs_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id,
  ...
  txq =
      rte_zmalloc("ethdev TX queue",
                  sizeof(*txq) + sizeof(rte_atomic64_t) * PRIV(dev)->subs_tail,
                  RTE_CACHE_LINE_SIZE);
  if (txq == NULL) {
    fs_unlock(dev, 0);
    return -ENOMEM;
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ret = rte_eth_tx_queue_setup(PORT_ID(sdev), tx_queue_id, nb_tx_desc,
                                 socket_id, tx_conf);
    if ((ret = fs_err(sdev, ret))) {
...
int failsafe_eth_dev_close(struct rte_eth_dev *dev) {
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    if (err) {
      ret = ret ? ret : err;
      ERROR("Error while closing sub-device %u", PORT_ID(sdev));
    }
...
static int fs_promiscuous_enable(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int ret = 0;
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    ret = fs_err(sdev, ret);
    if (ret != 0) {
      ERROR("Promiscuous mode enable failed for subdevice %d", PORT_ID(sdev));
      break;
    }
  }
  if (ret != 0) {
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
      ...
      if (ret != 0)
        ERROR(
            "Promiscuous mode disable during rollback failed for subdevice %d",
            PORT_ID(sdev));
    }
  }
  fs_unlock(dev, 0);
...
static int fs_promiscuous_disable(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int ret = 0;
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    ret = fs_err(sdev, ret);
    if (ret != 0) {
      ERROR("Promiscuous mode disable failed for subdevice %d", PORT_ID(sdev));
      break;
    }
  }
  if (ret != 0) {
...
static int fs_allmulticast_enable(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int ret = 0;
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    ret = fs_err(sdev, ret);
    if (ret != 0) {
      ERROR("All-multicast mode enable failed for subdevice %d", PORT_ID(sdev));
      break;
    }
  }
  if (ret != 0) {
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
      ...
      ret = fs_err(sdev, ret);
      if (ret != 0)
        ERROR("All-multicast mode disable during rollback failed for subdevice "
              "%d",
              PORT_ID(sdev));
    }
  }
...
static int fs_allmulticast_disable(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int ret = 0;
  ...
  if (ret != 0) {
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
      ...
      if (ret != 0)
        ERROR(
            "All-multicast mode enable during rollback failed for subdevice %d",
            PORT_ID(sdev));
    }
  }
  fs_unlock(dev, 0);
...
static int fs_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    if (ret && ret != -1 && sdev->remove == 0 &&
      ...
      ERROR("Link update failed for sub_device %d with error %d", i, ret);
      fs_unlock(dev, 0);
      return ret;
    }
  }
...
static int fs_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    if (ret) {
      ...
      ERROR(
          "Operation rte_eth_stats_get failed for sub_device %d with error %d",
          i, ret);
      *timestamp = 0;
      fs_unlock(dev, 0);
      return ret;
...
static int fs_stats_reset(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    if (ret) {
      ...
      ERROR("Operation rte_eth_stats_reset failed for sub_device %d with error "
            "%d",
            i, ret);
      fs_unlock(dev, 0);
      return ret;
...
static int __fs_xstats_count(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  int count = 0;
  uint8_t i;
  int ret;
...
static int __fs_xstats_get_names(struct rte_eth_dev *dev,
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    r = rte_eth_xstats_get_names(PORT_ID(sdev), sub_names, limit - count);
    if (r < 0)
      return r;
    ...
    for (j = 0; j < r; j++) {
      ...
      if ((xname[0] == 't' || xname[0] == 'r') && xname[1] == 'x' &&
          xname[2] == '_')
        snprintf(tmp, sizeof(tmp), "%.3ssub%u_%s", xname, i, xname + 3);
      else
        snprintf(tmp, sizeof(tmp), "sub%u_%s", i, xname);
...
static int fs_xstats_get_names(struct rte_eth_dev *dev,
                               struct rte_eth_xstat_name *xstats_names,
                               unsigned int limit) {
  int ret;
...
static int __fs_xstats_get(struct rte_eth_dev *dev,
...
static int fs_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats,
                         unsigned int n) {
  int ret;
...
static int fs_xstats_reset(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
  uint8_t i;
  int r = 0;
...
static void fs_dev_merge_desc_lim(struct rte_eth_desc_lim *to,
 ...
 */
static void fs_dev_merge_info(struct rte_eth_dev_info *info,
  ...
  info->max_hash_mac_addrs =
      RTE_MIN(info->max_hash_mac_addrs, sinfo->max_hash_mac_addrs);
  ...
  info->hash_key_size = RTE_MIN(info->hash_key_size, sinfo->hash_key_size);
}
 ...
 *
 */
static int fs_dev_infos_get(struct rte_eth_dev *dev,
  ...
  infos->rx_offload_capa =
      ...
      RTE_ETH_RX_OFFLOAD_TCP_LRO | RTE_ETH_RX_OFFLOAD_QINQ_STRIP |
      RTE_ETH_RX_OFFLOAD_OUTER_IPV4_CKSUM | RTE_ETH_RX_OFFLOAD_MACSEC_STRIP |
      RTE_ETH_RX_OFFLOAD_HEADER_SPLIT | RTE_ETH_RX_OFFLOAD_VLAN_FILTER |
      RTE_ETH_RX_OFFLOAD_VLAN_EXTEND | RTE_ETH_RX_OFFLOAD_SCATTER |
      RTE_ETH_RX_OFFLOAD_TIMESTAMP | RTE_ETH_RX_OFFLOAD_SECURITY |
  ...
  infos->dev_capa = RTE_ETH_DEV_CAPA_RUNTIME_RX_QUEUE_SETUP |
...
static const uint32_t *fs_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  struct sub_device *sdev;
...
static int fs_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    if ((ret = fs_err(sdev, ret))) {
      ERROR("Operation rte_eth_dev_set_mtu failed for sub_device %d with error "
            "%d",
            i, ret);
      fs_unlock(dev, 0);
      return ret;
...
static int fs_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                              int on) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
...
static int fs_flow_ctrl_get(struct rte_eth_dev *dev,
...
static int fs_flow_ctrl_set(struct rte_eth_dev *dev,
                            struct rte_eth_fc_conf *fc_conf) {
  struct sub_device *sdev;
  uint8_t i;
  int ret;
...
static void fs_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
  struct sub_device *sdev;
  uint8_t i;
   ...
   */
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE)
  rte_eth_dev_mac_addr_remove(PORT_ID(sdev), &dev->data->mac_addrs[index]);
  PRIV(dev)->mac_addr_pool[index] = 0;
  fs_unlock(dev, 0);
}

static int fs_mac_addr_add(struct rte_eth_dev *dev,
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    if ((ret = fs_err(sdev, ret))) {
      ERROR("Operation rte_eth_dev_mac_addr_add failed for sub_device %" PRIu8
            " with error %d",
            i, ret);
      fs_unlock(dev, 0);
      return ret;
...
static int fs_mac_addr_set(struct rte_eth_dev *dev,
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ...
    ret = fs_err(sdev, ret);
    if (ret) {
      ERROR("Operation rte_eth_dev_mac_addr_set failed for sub_device %d with "
            "error %d",
            i, ret);
      fs_unlock(dev, 0);
      return ret;
...
static int fs_set_mc_addr_list(struct rte_eth_dev *dev,
  ...
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    ret = rte_eth_dev_set_mc_addr_list(PORT_ID(sdev), mc_addr_set, nb_mc_addr);
    if (ret != 0) {
      ERROR("Operation rte_eth_dev_set_mc_addr_list failed for sub_device %d "
            "with error %d",
            i, ret);
      goto rollback;
    }
...
rollback:
  FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
    int rc = rte_eth_dev_set_mc_addr_list(PORT_ID(sdev), PRIV(dev)->mcast_addrs,
                                          PRIV(dev)->nb_mcast_addr);
    if (rc != 0) {
      ERROR("Multicast MAC address list rollback for sub_device %d failed with "
            "error %d",
            i, rc);
    }
  }
...
static int fs_rss_hash_update(struct rte_eth_dev *dev,
...
static int fs_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
