...
static int fs_sub_device_alloc(struct rte_eth_dev *dev, const char *params) {
  uint8_t nb_subs;
  int ret;
  int i;
  ...
  if (PRIV(dev)->subs_tail > FAILSAFE_MAX_ETHPORTS) {
    ERROR("Cannot allocate more than %d ports", FAILSAFE_MAX_ETHPORTS);
    return -ENOSPC;
  }
  nb_subs = PRIV(dev)->subs_tail;
  PRIV(dev)->subs = rte_zmalloc(NULL, sizeof(struct sub_device) * nb_subs,
                                RTE_CACHE_LINE_SIZE);
...
int failsafe_hotplug_alarm_install(struct rte_eth_dev *dev) {
  ...
  if (PRIV(dev)->pending_alarm)
    return 0;
  ret = rte_eal_alarm_set(failsafe_hotplug_poll * 1000, fs_hotplug_alarm, dev);
  if (ret) {
...
int failsafe_hotplug_alarm_cancel(struct rte_eth_dev *dev) {
  ...
  if (rte_errno) {
    ERROR("rte_eal_alarm_cancel failed (errno: %s)", strerror(rte_errno));
    ret = -rte_errno;
  } else {
...
static int fs_mutex_init(struct fs_priv *priv) {
  int ret;
...
static int fs_eth_dev_create(struct rte_vdev_device *vdev) {
  ...
  if (failsafe_mac_from_arg) {
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_PROBED) {
      ret = rte_eth_dev_default_mac_addr_set(PORT_ID(sdev), mac);
      if (ret) {
    ...
    if (sdev->state >= DEV_PROBED) {
      rte_ether_addr_copy(&ETH(sdev)->data->mac_addrs[0], mac);
      break;
    }
    /*
  ...
  INFO("MAC address is " RTE_ETHER_ADDR_PRT_FMT, RTE_ETHER_ADDR_BYTES(mac));
  dev->data->dev_flags |=
      RTE_ETH_DEV_INTR_LSC | RTE_ETH_DEV_AUTOFILL_QUEUE_XSTATS;

  /* Allocate interrupt instance */
  PRIV(dev)->intr_handle = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_SHARED);
...
static int fs_rte_eth_free(const char *name) {
  struct rte_eth_dev *dev;
  int ret;
...
static bool devargs_already_listed(struct rte_devargs *devargs) {
...
static int rte_pmd_failsafe_probe(struct rte_vdev_device *vdev) {
  const char *name;
  ...
  INFO("Initializing " FAILSAFE_DRIVER_NAME " for %s", name);
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ...
    FOREACH_SUBDEV(sdev, i, eth_dev) {
      ...
      ret = rte_devargs_parse(&devargs, sdev->devargs.name);
      if (ret != 0) {
        ERROR("Failed to parse devargs %s", devargs.name);
        continue;
      }
      if (!devargs_already_listed(&devargs)) {
        ret = rte_dev_probe(devargs.name);
        if (ret < 0) {
          ERROR("Failed to probe devargs %s", devargs.name);
          continue;
        }
      }
    }
...
static int rte_pmd_failsafe_remove(struct rte_vdev_device *vdev) {
  const char *name;
