...
static inline int fs_rx_unsafe(struct sub_device *sdev) {
  return (ETH(sdev) == NULL) || (ETH(sdev)->rx_pkt_burst == NULL) ||
         (sdev->state != DEV_STARTED) || (sdev->remove != 0);
}

static inline int fs_tx_unsafe(struct sub_device *sdev) {
  return (sdev == NULL) || (ETH(sdev) == NULL) ||
         (ETH(sdev)->tx_pkt_burst == NULL) || (sdev->state != DEV_STARTED);
}
...
void failsafe_set_burst_fn(struct rte_eth_dev *dev, int force_safe) {
  ...
  if (need_safe && !safe_set) {
    DEBUG("Using safe RX bursts%s", (force_safe ? " (forced)" : ""));
  ...
  if (need_safe && !safe_set) {
    DEBUG("Using safe TX bursts%s", (force_safe ? " (forced)" : ""));
 ...
 */
static void failsafe_rx_set_port(struct rte_mbuf **rx_pkts, uint16_t nb_pkts,
                                 uint16_t port) {
  unsigned int i;
...
uint16_t failsafe_rx_burst(void *queue, struct rte_mbuf **rx_pkts,
                           uint16_t nb_pkts) {
  struct sub_device *sdev;
  struct rxq *rxq;
  void *sub_rxq;
  ...
  do {
    ...
    nb_rx = ETH(sdev)->rx_pkt_burst(sub_rxq, rx_pkts, nb_pkts);
    FS_ATOMIC_V(rxq->refcnt[sdev->sid]);
    sdev = sdev->next;
  } while (nb_rx == 0 && sdev != rxq->sdev);
  rxq->sdev = sdev;
  if (nb_rx)
    failsafe_rx_set_port(rx_pkts, nb_rx, rxq->priv->data->port_id);
  return nb_rx;
}

uint16_t failsafe_rx_burst_fast(void *queue, struct rte_mbuf **rx_pkts,
                                uint16_t nb_pkts) {
  struct sub_device *sdev;
  struct rxq *rxq;
  void *sub_rxq;
...
uint16_t failsafe_tx_burst(void *queue, struct rte_mbuf **tx_pkts,
                           uint16_t nb_pkts) {
  struct sub_device *sdev;
  struct txq *txq;
  void *sub_txq;
...
uint16_t failsafe_tx_burst_fast(void *queue, struct rte_mbuf **tx_pkts,
                                uint16_t nb_pkts) {
  struct sub_device *sdev;
  struct txq *txq;
  void *sub_txq;
