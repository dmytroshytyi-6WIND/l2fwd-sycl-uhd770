 ...
 */
static size_t closing_paren(const char *text) {
  int nb_open = 0;
  size_t i = 0;
...
static int fs_parse_device(struct sub_device *sdev, char *args) {
...
static void fs_sanitize_cmdline(char *args) {
  char *nl;
...
static int fs_execute_cmd(struct sub_device *sdev, char *cmdline) {
  FILE *fp;
...
static int fs_read_fd(struct sub_device *sdev, char *fd_str) {
  FILE *fp = NULL;
  int fd = -1;
  /* store possible newline as well */
  char output[DEVARGS_MAXLEN + 1];
...
static int fs_parse_device_param(struct rte_eth_dev *dev, const char *param,
                                 uint8_t head) {
  struct fs_priv *priv;
...
static int fs_parse_sub_devices(parse_cb *cb, struct rte_eth_dev *dev,
                                const char *params) {
  size_t a, b;
  uint8_t head;
  int ret;
  ...
  while (params[a] != '\0') {
    b = a;
    while (params[b] != '(' && params[b] != ',' && params[b] != '\0')
      b++;
    if (b == a) {
...
static int fs_remove_sub_devices_definition(char params[DEVARGS_MAXLEN]) {
  ...
  while (params[a] != '\0') {
    ...
    if (params[b] == ',' || params[b] == '\0') {
      ...
      if (i > 0)
        len += 1;
      snprintf(&buffer[i], len + 1, "%s%s", i ? "," : "", &params[a]);
      i += len;
    } else if (params[b] == '(') {
      size_t start = b;
...
static int fs_get_u64_arg(const char *key __rte_unused, const char *value,
                          void *out) {
  uint64_t *u64 = out;
  char *endptr = NULL;
...
static int fs_get_mac_addr_arg(const char *key __rte_unused, const char *value,
                               void *out) {
...
int failsafe_args_parse(struct rte_eth_dev *dev, const char *params) {
  ...
  if (n >= sizeof(mut_params)) {
    ERROR("Parameter string too long (>=%zu)", sizeof(mut_params));
    return -ENOMEM;
  }
  ret = fs_parse_sub_devices(fs_parse_device_param, dev, params);
  if (ret < 0)
    return ret;
  ...
  if (strnlen(mut_params, sizeof(mut_params)) > 0) {
    kvlist = rte_kvargs_parse(mut_params, pmd_failsafe_init_parameters);
    if (kvlist == NULL) {
      ERROR("Error parsing parameters, usage:\n" PMD_FAILSAFE_PARAM_STRING);
      return -1;
    }
    /* PLUG_IN event poll timer */
    arg_count = rte_kvargs_count(kvlist, PMD_FAILSAFE_HOTPLUG_POLL_KVARG);
    if (arg_count == 1) {
      ret = rte_kvargs_process(kvlist, PMD_FAILSAFE_HOTPLUG_POLL_KVARG,
                               &fs_get_u64_arg, &failsafe_hotplug_poll);
      if (ret < 0)
        goto free_kvlist;
    }
    /* MAC addr */
    arg_count = rte_kvargs_count(kvlist, PMD_FAILSAFE_MAC_KVARG);
    if (arg_count > 0) {
      ret = rte_kvargs_process(kvlist, PMD_FAILSAFE_MAC_KVARG,
                               &fs_get_mac_addr_arg, &dev->data->mac_addrs[0]);
      if (ret < 0)
        goto free_kvlist;
...
static int fs_count_device(struct rte_eth_dev *dev, const char *param,
                           uint8_t head __rte_unused) {
  size_t b = 0;

  while (param[b] != '(' && param[b] != '\0')
    b++;
  if (strncmp(param, "dev", b) != 0 && strncmp(param, "exec", b) != 0 &&
...
int failsafe_args_count_subdevice(struct rte_eth_dev *dev, const char *params) {
  return fs_parse_sub_devices(fs_count_device, dev, params);
}

static int fs_parse_sub_device(struct sub_device *sdev) {
