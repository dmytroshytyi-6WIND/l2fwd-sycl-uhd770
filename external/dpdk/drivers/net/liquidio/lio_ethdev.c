...
static uint16_t lio_wait_for_ctrl_cmd(struct lio_device *lio_dev,
 ...
 */
static int lio_send_rx_ctrl_cmd(struct rte_eth_dev *eth_dev, int start_stop) {
...
static const struct rte_lio_xstats_name_off rte_lio_stats_strings[] = {
    ...
    {"tx_bytes", (offsetof(struct octeon_tx_stats, total_bytes_sent)) +
                     sizeof(struct octeon_rx_stats)},
    {"tx_broadcast_pkts", (offsetof(struct octeon_tx_stats, bcast_pkts_sent)) +
                              sizeof(struct octeon_rx_stats)},
    {"tx_multicast_pkts", (offsetof(struct octeon_tx_stats, mcast_pkts_sent)) +
                              sizeof(struct octeon_rx_stats)},
...
static int lio_dev_xstats_get(struct rte_eth_dev *eth_dev,
  ...
  lio_prepare_soft_command(lio_dev, sc, LIO_OPCODE, LIO_OPCODE_PORT_STATS, 0, 0,
                           0);
  ...
  for (i = 0; i < LIO_NB_XSTATS; i++) {
    xstats[i].id = i;
    xstats[i].value =
        *(uint64_t *)(((char *)hw_stats) + rte_lio_stats_strings[i].offset);
  }
...
static int lio_dev_xstats_get_names(struct rte_eth_dev *eth_dev,
...
static int lio_dev_xstats_reset(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_dev_ctrl_cmd ctrl_cmd;
  struct lio_ctrl_pkt ctrl_pkt;
  int ret;
...
static int lio_dev_stats_get(struct rte_eth_dev *eth_dev,
  ...
  for (i = 0; i < eth_dev->data->nb_rx_queues; i++) {
    ...
    if (droq != NULL) {
      ...
      drop += (oq_stats->rx_dropped + oq_stats->dropped_toomany +
...
static int lio_dev_stats_reset(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_droq_stats *oq_stats;
  struct lio_iq_stats *iq_stats;
  struct lio_instr_queue *txq;
...
static int lio_dev_info_get(struct rte_eth_dev *eth_dev,
  ...
  default:
    devinfo->speed_capa = RTE_ETH_LINK_SPEED_10G;
    lio_dev_err(lio_dev, "Unknown CN23XX subsystem device id. Setting 10G as "
                         "default link speed.\n");
    return -EINVAL;
  }
  ...
  devinfo->flow_type_rss_offloads =
      (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_NONFRAG_IPV4_TCP | RTE_ETH_RSS_IPV6 |
       RTE_ETH_RSS_NONFRAG_IPV6_TCP | RTE_ETH_RSS_IPV6_EX |
       RTE_ETH_RSS_IPV6_TCP_EX);
  return 0;
}

static int lio_dev_mtu_set(struct rte_eth_dev *eth_dev, uint16_t mtu) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_dev_ctrl_cmd ctrl_cmd;
  struct lio_ctrl_pkt ctrl_pkt;
...
static int lio_dev_rss_reta_update(struct rte_eth_dev *eth_dev,
...
static int lio_dev_rss_reta_query(struct rte_eth_dev *eth_dev,
                                  struct rte_eth_rss_reta_entry64 *reta_conf,
                                  uint16_t reta_size) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_rss_ctx *rss_state = &lio_dev->rss_state;
  ...
  for (i = 0; i < num; i++) {
    memcpy(reta_conf->reta, &rss_state->itable[i * RTE_ETH_RETA_GROUP_SIZE],
...
static int lio_dev_rss_hash_conf_get(struct rte_eth_dev *eth_dev,
...
static int lio_dev_rss_hash_update(struct rte_eth_dev *eth_dev,
                                   struct rte_eth_rss_conf *rss_conf) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_rss_ctx *rss_state = &lio_dev->rss_state;
  struct lio_rss_set *rss_param;
  ...
  if (rss_conf->rss_key) {
    ...
    memcpy(rss_state->hash_key, rss_conf->rss_key, rss_state->hash_key_size);
    memcpy(rss_param->key, rss_state->hash_key, rss_state->hash_key_size);
  }
 ...
 *  On failure return -1
 */
static int lio_dev_udp_tunnel_add(struct rte_eth_dev *eth_dev,
 ...
 *  On failure return -1
 */
static int lio_dev_udp_tunnel_del(struct rte_eth_dev *eth_dev,
                                  struct rte_eth_udp_tunnel *udp_tnl) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_dev_ctrl_cmd ctrl_cmd;
  struct lio_ctrl_pkt ctrl_pkt;
...
static int lio_dev_vlan_filter_set(struct rte_eth_dev *eth_dev,
  ...
  ctrl_pkt.ncmd.s.cmd = on ? LIO_CMD_ADD_VLAN_FILTER : LIO_CMD_DEL_VLAN_FILTER;
...
static uint64_t lio_hweight64(uint64_t w) {
...
static int lio_dev_link_update(struct rte_eth_dev *eth_dev,
 ...
 */
static int lio_change_dev_flag(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_dev_ctrl_cmd ctrl_cmd;
  struct lio_ctrl_pkt ctrl_pkt;
...
static int lio_dev_promiscuous_enable(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
...
static int lio_dev_promiscuous_disable(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
...
static int lio_dev_allmulticast_enable(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
...
static int lio_dev_allmulticast_disable(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
...
static void lio_dev_rss_configure(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_rss_ctx *rss_state = &lio_dev->rss_state;
...
static void lio_dev_mq_rx_configure(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_rss_ctx *rss_state = &lio_dev->rss_state;
  struct rte_eth_rss_conf rss_conf;
...
lio_dev_tx_queue_setup(struct rte_eth_dev *eth_dev, uint16_t q_no,
  ...
  retval =
      lio_setup_sglists(lio_dev, q_no, fw_mapped_iq,
                        lio_dev->instr_queue[fw_mapped_iq]->nb_desc, socket_id);
 ...
 */
static void lio_dev_get_link_status(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  uint16_t timeout = LIO_MAX_CMD_TIMEOUT;
  ...
  lio_prepare_soft_command(lio_dev, sc, LIO_OPCODE, LIO_OPCODE_INFO, 0, 0, 0);
 ...
 */
static void lio_sync_link_state_check(void *eth_dev) {
   ...
   */
  if (lio_dev->intf_open)
    rte_eal_alarm_set(LIO_LSC_TIMEOUT, lio_sync_link_state_check, eth_dev);
}

static int lio_dev_start(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  uint16_t timeout = LIO_MAX_CMD_TIMEOUT;
  int ret = 0;
  ...
  ret = rte_eal_alarm_set(LIO_LSC_TIMEOUT, lio_sync_link_state_check, eth_dev);
  if (ret) {
    lio_dev_err(lio_dev, "link state check handler creation failed\n");
...
static int lio_dev_stop(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
...
static int lio_dev_set_link_up(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
...
static int lio_dev_set_link_down(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
 ...
 *    - nothing
 */
static int lio_dev_close(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  int ret = 0;
 ...
 */
static void lio_enable_hw_tunnel_rx_checksum(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_dev_ctrl_cmd ctrl_cmd;
  struct lio_ctrl_pkt ctrl_pkt;
 ...
 */
static void lio_enable_hw_tunnel_tx_checksum(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_dev_ctrl_cmd ctrl_cmd;
  struct lio_ctrl_pkt ctrl_pkt;
...
static int lio_send_queue_count_update(struct rte_eth_dev *eth_dev, int num_txq,
                                       int num_rxq) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  struct lio_dev_ctrl_cmd ctrl_cmd;
  struct lio_ctrl_pkt ctrl_pkt;
...
static int lio_reconf_queues(struct rte_eth_dev *eth_dev, int num_txq,
                             int num_rxq) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
  int ret;

  if (lio_dev->nb_rx_queues != num_rxq || lio_dev->nb_tx_queues != num_txq) {
...
static int lio_dev_configure(struct rte_eth_dev *eth_dev) {
   ...
   */
  if (lio_dev->port_configured)
    return lio_reconf_queues(eth_dev, eth_dev->data->nb_tx_queues,
  ...
  lio_prepare_soft_command(lio_dev, sc, LIO_OPCODE, LIO_OPCODE_IF_CFG, 0,
                           if_cfg.if_cfg64, 0);
  ...
  strlcpy(lio_dev->firmware_version, resp->cfg_info.lio_firmware_version,
          LIO_FW_VERSION_LENGTH);
  ...
  lio_dev_dbg(lio_dev,
              "interface %d, iqmask %016lx, oqmask %016lx, numiqueues %d, "
              "numoqueues %d\n",
              eth_dev->data->port_id, (unsigned long)resp->cfg_info.iqmask,
              (unsigned long)resp->cfg_info.oqmask, num_iqueues, num_oqueues);

  lio_dev->linfo.num_rxpciq = num_oqueues;
  lio_dev->linfo.num_txpciq = num_iqueues;

  for (i = 0; i < num_oqueues; i++) {
    lio_dev->linfo.rxpciq[i].rxpciq64 = resp->cfg_info.linfo.rxpciq[i].rxpciq64;
    lio_dev_dbg(lio_dev, "index %d OQ %d\n", i,
                lio_dev->linfo.rxpciq[i].s.q_no);
  }

  for (i = 0; i < num_iqueues; i++) {
    lio_dev->linfo.txpciq[i].txpciq64 = resp->cfg_info.linfo.txpciq[i].txpciq64;
    lio_dev_dbg(lio_dev, "index %d IQ %d\n", i,
                lio_dev->linfo.txpciq[i].s.q_no);
  }

  lio_dev->linfo.hw_addr = resp->cfg_info.linfo.hw_addr;
  lio_dev->linfo.gmxport = resp->cfg_info.linfo.gmxport;
  lio_dev->linfo.link.link_status64 = resp->cfg_info.linfo.link.link_status64;

  /* 64-bit swap required on LE machines */
  lio_swap_8B_data(&lio_dev->linfo.hw_addr, 1);
  for (i = 0; i < RTE_ETHER_ADDR_LEN; i++)
    mac[i] = *((uint8_t *)(((uint8_t *)&lio_dev->linfo.hw_addr) + 2 + i));
  ...
  lio_dev->glist_head =
      rte_zmalloc(NULL, sizeof(*lio_dev->glist_head) * num_iqueues, 0);
...
static void lio_check_pf_hs_response(void *lio_dev) {
 ...
 */
static int lio_chip_specific_setup(struct lio_device *lio_dev) {
...
static int lio_first_time_init(struct lio_device *lio_dev,
...
static int lio_eth_dev_uninit(struct rte_eth_dev *eth_dev) {
  struct lio_device *lio_dev = LIO_DEV(eth_dev);
...
static int lio_eth_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  snprintf(lio_dev->dev_string, sizeof(lio_dev->dev_string), "%s[%02x:%02x.%x]",
           pdev->driver->driver.name, pdev->addr.bus, pdev->addr.devid,
           pdev->addr.function);
  ...
  if (eth_dev->data->mac_addrs == NULL) {
    lio_dev_err(lio_dev, "MAC addresses memory allocation failed\n");
...
static int lio_eth_dev_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                                 struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct lio_device),
                                       lio_eth_dev_init);
}

static int lio_eth_dev_pci_remove(struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_remove(pci_dev, lio_eth_dev_uninit);
}
