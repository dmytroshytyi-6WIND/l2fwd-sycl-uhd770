...
static void lio_droq_compute_max_packet_bufs(struct lio_droq *droq) {
  uint32_t count = 0;
...
static void lio_droq_reset_indices(struct lio_droq *droq) {
  droq->read_idx = 0;
  droq->write_idx = 0;
  droq->refill_idx = 0;
  droq->refill_count = 0;
...
static void lio_droq_destroy_ring_buffers(struct lio_droq *droq) {
  uint32_t i;

  for (i = 0; i < droq->nb_desc; i++) {
    if (droq->recv_buf_list[i].buffer) {
      rte_pktmbuf_free((struct rte_mbuf *)droq->recv_buf_list[i].buffer);
...
static int lio_droq_setup_ring_buffers(struct lio_device *lio_dev,
  ...
  for (i = 0; i < droq->nb_desc; i++) {
    ...
    desc_ring[i].buffer_ptr = lio_map_ring(droq->recv_buf_list[i].buffer);
  }
...
static void lio_dma_zone_free(struct lio_device *lio_dev,
 ...
 *  @param q_no		- droq no.
 */
static void lio_delete_droq(struct lio_device *lio_dev, uint32_t q_no) {
...
static void *lio_alloc_info_buffer(struct lio_device *lio_dev,
                                   struct lio_droq *droq,
                                   unsigned int socket_id) {
  droq->info_mz = rte_eth_dma_zone_reserve(
      lio_dev->eth_dev, "info_list", droq->q_no,
      (droq->nb_desc * LIO_DROQ_INFO_SIZE), RTE_CACHE_LINE_SIZE, socket_id);
 ...
 */
static int lio_init_droq(struct lio_device *lio_dev, uint32_t q_no,
  ...
  droq->desc_ring_mz =
      rte_eth_dma_zone_reserve(lio_dev->eth_dev, "droq", q_no, desc_ring_size,
                               RTE_CACHE_LINE_SIZE, socket_id);

  if (droq->desc_ring_mz == NULL) {
    lio_dev_err(lio_dev, "Output queue %d ring alloc failed\n", q_no);
    return -1;
  }
  ...
  lio_dev_dbg(lio_dev, "droq[%d]: desc_ring: virt: 0x%p, dma: %lx\n", q_no,
              droq->desc_ring, (unsigned long)droq->desc_ring_dma);
  ...
  droq->recv_buf_list = rte_zmalloc_socket(
      "recv_buf_list", (droq->nb_desc * LIO_DROQ_RECVBUF_SIZE),
      RTE_CACHE_LINE_SIZE, socket_id);
  if (droq->recv_buf_list == NULL) {
    lio_dev_err(lio_dev, "Output queue recv buf list alloc failed\n");
    goto init_droq_fail;
  }
...
int lio_setup_droq(struct lio_device *lio_dev, int oq_no, int num_descs,
  ...
  /* Initialize the Droq */
  if (lio_init_droq(lio_dev, oq_no, num_descs, desc_size, mpool, socket_id)) {
...
static inline uint32_t lio_droq_get_bufcount(uint32_t buf_size,
                                             uint32_t total_len) {
  uint32_t buf_cnt = 0;
...
lio_droq_refill_pullup_descs(struct lio_droq *droq,
  ...
  while (refill_index != droq->read_idx) {
    if (droq->recv_buf_list[refill_index].buffer) {
      ...
      do {
        droq->refill_idx = lio_incr_index(droq->refill_idx, 1, droq->nb_desc);
        desc_refilled++;
        droq->refill_count--;
      } while (droq->recv_buf_list[droq->refill_idx].buffer);
    }
    refill_index = lio_incr_index(refill_index, 1, droq->nb_desc);
  } /* while */
 ...
 */
static uint32_t lio_droq_refill(struct lio_droq *droq) {
  struct lio_droq_desc *desc_ring;
  uint32_t desc_refilled = 0;
  void *buf = NULL;
...
static int lio_droq_fast_process_packet(struct lio_device *lio_dev,
                                        struct lio_droq *droq,
  ...
  if (lio_opcode_slow_path(rh)) {
    ...
    buf_cnt = lio_droq_get_bufcount(droq->buffer_size, (uint32_t)info->length);
    droq->read_idx = lio_incr_index(droq->read_idx, buf_cnt, droq->nb_desc);
    ...
    if (info->length <= droq->buffer_size) {
      ...
      droq->read_idx = lio_incr_index(droq->read_idx, 1, droq->nb_desc);
      droq->refill_count++;
      ...
      if (likely(nicbuf != NULL)) {
        ...
        if (rh->r_dh.has_hash) {
          ...
          hash_ptr = rte_pktmbuf_mtod(nicbuf, uint64_t *);
      ...
      if ((droq->read_idx & 3) == 0) {
        rte_prefetch0(&droq->recv_buf_list[droq->read_idx]);
        rte_prefetch0(&droq->info_list[droq->read_idx]);
      }
    } else {
      ...
      while (pkt_len < info->length) {
        ...
        cpy_len = ((pkt_len + droq->buffer_size) > info->length)
...
static uint32_t lio_droq_fast_process_packets(struct lio_device *lio_dev,
                                              struct lio_droq *droq,
                                              struct rte_mbuf **rx_pkts,
  ...
  for (pkt = 0; pkt < pkts_to_process; pkt++) {
    ret = lio_droq_fast_process_packet(lio_dev, droq, &rx_pkts[data_pkts]);
    if (ret < 0) {
      lio_dev_err(lio_dev, "Port[%d] DROQ[%d] idx: %d len:0, pkt_cnt: %d\n",
                  lio_dev->port_id, droq->q_no, droq->read_idx,
                  pkts_to_process);
      break;
    }
    data_pkts += ret;
...
static inline uint32_t lio_droq_check_hw_for_pkts(struct lio_droq *droq) {
  uint32_t last_count;
  uint32_t pkt_count;
...
uint16_t lio_dev_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  pkts_processed =
      lio_droq_fast_process_packets(lio_dev, droq, rx_pkts, pkt_count);
 ...
 *  @return  Success: 0	Failure: -1
 */
static int lio_init_instr_queue(struct lio_device *lio_dev,
  ...
  iq->iq_mz = rte_eth_dma_zone_reserve(lio_dev->eth_dev, "instr_queue", iq_no,
                                       q_size, RTE_CACHE_LINE_SIZE, socket_id);
  if (iq->iq_mz == NULL) {
   ...
   */
  iq->request_list =
      rte_zmalloc_socket("request_list", sizeof(*iq->request_list) * num_descs,
                         RTE_CACHE_LINE_SIZE, socket_id);
  ...
  lio_dev_dbg(lio_dev, "IQ[%d]: base: %p basedma: %lx count: %d\n", iq_no,
              iq->base_addr, (unsigned long)iq->base_addr_dma, iq->nb_desc);
...
int lio_setup_instr_queue0(struct lio_device *lio_dev) {
  ...
  lio_dev->instr_queue[0] =
      rte_zmalloc(NULL, sizeof(struct lio_instr_queue), 0);
 ...
 */
static void lio_delete_instr_queue(struct lio_device *lio_dev, uint32_t iq_no) {
...
int lio_setup_iq(struct lio_device *lio_dev, int q_index,
  ...
  lio_dev->instr_queue[iq_no] =
      rte_zmalloc_socket("ethdev TX queue", sizeof(struct lio_instr_queue),
                         RTE_CACHE_LINE_SIZE, socket_id);
...
int lio_wait_for_instr_fetch(struct lio_device *lio_dev) {
  ...
  do {
    ...
    for (i = 0; i < LIO_MAX_INSTR_QUEUES(lio_dev); i++) {
      ...
      pending = rte_atomic64_read(&lio_dev->instr_queue[i]->instr_pending);
      if (pending)
...
static inline void lio_ring_doorbell(struct lio_device *lio_dev,
                                     struct lio_instr_queue *iq) {
...
static inline void copy_cmd_into_iq(struct lio_instr_queue *iq, uint8_t *cmd) {
...
post_command2(struct lio_instr_queue *iq, uint8_t *cmd) {
   ...
   */
  if (rte_atomic64_read(&iq->instr_pending) >= (int32_t)(iq->nb_desc - 1)) {
    st.status = LIO_IQ_SEND_FAILED;
    st.index = -1;
    return st;
  }

  if (rte_atomic64_read(&iq->instr_pending) >= (int32_t)(iq->nb_desc - 2))
  ...
  iq->host_write_index = lio_incr_index(iq->host_write_index, 1, iq->nb_desc);
  iq->fill_cnt++;
...
static inline void lio_add_to_request_list(struct lio_instr_queue *iq, int idx,
                                           void *buf, int reqtype) {
  iq->request_list[idx].buf = buf;
  iq->request_list[idx].reqtype = reqtype;
}

static inline void lio_free_netsgbuf(void *buf) {
...
static int lio_process_iq_request_list(struct lio_device *lio_dev,
                                       struct lio_instr_queue *iq) {
  ...
  while (old != iq->lio_read_index) {
    ...
    case LIO_REQTYPE_SOFT_COMMAND:
      ...
      if (irh->rflag) {
        ...
        rte_atomic64_inc(&lio_dev->response_list.pending_req_count);
        STAILQ_INSERT_TAIL(&lio_dev->response_list.head, &sc->node, entries);
        rte_spinlock_unlock(&lio_dev->response_list.lock);
      } else {
        if (sc->callback) {
          /* This callback must not sleep */
          sc->callback(LIO_REQUEST_DONE, sc->callback_arg);
        }
      }
      break;
    default:
      lio_dev_err(lio_dev, "Unknown reqtype: %d buf: %p at idx %d\n", reqtype,
                  buf, old);
    }
...
static void lio_update_read_index(struct lio_instr_queue *iq) {
  ...
  iq->lio_read_index =
      (iq->lio_read_index + (uint32_t)(last_done & LIO_PKT_IN_DONE_CNT_MASK)) %
      iq->nb_desc;
}
...
static int lio_send_command(struct lio_device *lio_dev, uint32_t iq_no,
                            void *cmd, void *buf, uint32_t datasize,
                            uint32_t reqtype) {
  struct lio_instr_queue *iq = lio_dev->instr_queue[iq_no];
  struct lio_iq_post_status st;
  ...
  if (st.status != LIO_IQ_SEND_FAILED) {
    ...
    LIO_INCR_INSTRQUEUE_PKT_COUNT(lio_dev, iq_no, bytes_sent, datasize);
...
void lio_prepare_soft_command(struct lio_device *lio_dev,
                              ...
                              uint8_t subcode, uint32_t irh_ossp,
...
int lio_setup_sc_buffer_pool(struct lio_device *lio_dev) {
  ...
  snprintf(sc_pool_name, sizeof(sc_pool_name), "lio_sc_pool_%u",
           lio_dev->port_id);
  lio_dev->sc_buf_pool =
      rte_pktmbuf_pool_create(sc_pool_name, LIO_MAX_SOFT_COMMAND_BUFFERS, 0, 0,
...
struct lio_soft_command *lio_alloc_soft_command(struct lio_device *lio_dev,
                                                uint32_t datasize,
                                                uint32_t rdatasize,
                                                uint32_t ctxsize) {
...
int lio_process_ordered_list(struct lio_device *lio_dev) {
  ...
  do {
    ...
    if (status64 != LIO_COMPLETION_WORD_INIT) {
      ...
      if ((status64 & 0xff) != 0xff) {
        ...
        if (((status64 & 0xff) != 0xff)) {
          ...
          status = (uint32_t)(status64 & 0xffffULL);
          if (status) {
            status = LIO_FIRMWARE_STATUS_CODE(status);
          } else {
            /* i.e. no error */
    ...
    } else if ((sc->timeout && lio_check_timeout(lio_uptime, sc->timeout))) {
      lio_dev_err(lio_dev, "cmd failed, timeout (%ld, %ld)\n", (long)lio_uptime,
                  (long)sc->timeout);
    ...
    if (status != LIO_REQUEST_PENDING) {
       ...
       */
      STAILQ_REMOVE(&ordered_sc_list->head, &sc->node, lio_stailq_node,
                    entries);
      rte_atomic64_dec(&lio_dev->response_list.pending_req_count);
      rte_spinlock_unlock(&ordered_sc_list->lock);
...
void lio_delete_sglist(struct lio_instr_queue *txq) {
  ...
  do {
    ...
    if (g) {
      if (g->sg)
        rte_free((void *)((unsigned long)g->sg - g->adjust));
      rte_free(g);
    }
  } while (g);
}
 ...
 */
int lio_setup_sglists(struct lio_device *lio_dev, int iq_no, int fw_mapped_iq,
  ...
  for (i = 0; i < num_descs; i++) {
    g = rte_zmalloc_socket(NULL, sizeof(*g), RTE_CACHE_LINE_SIZE, socket_id);
    if (g == NULL) {
      lio_dev_err(lio_dev, "lio_gather memory allocation failed for qno %d\n",
                  iq_no);
      break;
    }

    g->sg_size = ((ROUNDUP4(LIO_MAX_SG) >> 2) * LIO_SG_ENTRY_SIZE);

    g->sg = rte_zmalloc_socket(NULL, g->sg_size + 8, RTE_CACHE_LINE_SIZE,
                               socket_id);
    if (g->sg == NULL) {
      lio_dev_err(lio_dev, "sg list memory allocation failed for qno %d\n",
                  iq_no);
      rte_free(g);
      break;
    }
    ...
    if (((unsigned long)g->sg) & 7) {
      ...
      g->sg = (struct lio_sg_entry *)((unsigned long)g->sg + g->adjust);
    }

    STAILQ_INSERT_TAIL(&lio_dev->glist_head[iq_no], &g->list, entries);
  }
...
static inline uint32_t lio_iq_get_available(struct lio_device *lio_dev,
                                            uint32_t q_no) {
  return (
      (lio_dev->instr_queue[q_no]->nb_desc - 1) -
      (uint32_t)rte_atomic64_read(&lio_dev->instr_queue[q_no]->instr_pending));
}

static inline int lio_iq_is_full(struct lio_device *lio_dev, uint32_t q_no) {
  return (
      (uint32_t)rte_atomic64_read(&lio_dev->instr_queue[q_no]->instr_pending) >=
      (lio_dev->instr_queue[q_no]->nb_desc - 2));
}

static int lio_dev_cleanup_iq(struct lio_device *lio_dev, int iq_no) {
  struct lio_instr_queue *iq = lio_dev->instr_queue[iq_no];
  uint32_t count = 10000;

  while ((lio_iq_get_available(lio_dev, iq_no) < LIO_FLUSH_WM(iq)) && --count)
    lio_flush_iq(lio_dev, iq);

  return count ? 0 : 1;
}

static void lio_ctrl_cmd_callback(uint32_t status __rte_unused, void *sc_ptr) {
...
lio_alloc_ctrl_pkt_sc(struct lio_device *lio_dev,
  ...
  sc = lio_alloc_soft_command(lio_dev, datasize, rdatasize,
  ...
  lio_prepare_soft_command(lio_dev, sc, LIO_OPCODE, LIO_OPCODE_CMD, 0, 0, 0);
 ...
 */
static inline int lio_send_data_pkt(struct lio_device *lio_dev,
                                    struct lio_data_pkt *ndata) {
  return lio_send_command(lio_dev, ndata->q_no, &ndata->cmd, ndata->buf,
                          ndata->datasize, ndata->reqtype);
}
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (lio_iq_is_full(lio_dev, ndata.q_no)) {
      ...
      if (lio_dev_cleanup_iq(lio_dev, iq_no)) {
        PMD_TX_LOG(lio_dev, ERR, "Transmit failed iq:%d full\n", ndata.q_no);
        break;
      }
    }
    ...
    if (m->nb_segs == 1) {
      ...
      lio_prepare_pci_cmd(lio_dev, &ndata.cmd, &cmdsetup, tag);
      ...
      finfo = (struct lio_buf_free_info *)rte_malloc(NULL, sizeof(*finfo), 0);
      if (finfo == NULL) {
        PMD_TX_LOG(lio_dev, ERR, "free buffer alloc failed\n");
        goto xmit_failed;
      }
      ...
      if (g == NULL) {
        PMD_TX_LOG(lio_dev, ERR, "Transmit scatter gather: glist null!\n");
        goto xmit_failed;
      }
      ...
      m = m->next;
      while (frags--) {
        g->sg[(i >> 2)].ptr[(i & 3)] = rte_mbuf_data_iova(m);
        lio_add_sg_size(&g->sg[(i >> 2)], m->data_len, (i & 3));
        pkt_len += m->data_len;
        i++;
        m = m->next;
      }
