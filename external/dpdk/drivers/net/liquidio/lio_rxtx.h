...
#define lio_uptime (size_t)(rte_get_timer_cycles() / rte_get_timer_hz())
 ...
 */
static inline void lio_prepare_pci_cmd(struct lio_device *lio_dev,
                                       union lio_instr_64B *cmd,
...
static inline void lio_swap_8B_data(uint64_t *data, uint32_t blocks) {
  while (blocks) {
...
static inline uint64_t lio_map_ring(void *buf) {
  rte_iova_t dma_addr;
...
static inline uint64_t lio_map_ring_info(struct lio_droq *droq, uint32_t i) {
  rte_iova_t dma_addr;
...
static inline int lio_opcode_slow_path(union octeon_rh *rh) {
...
static inline void lio_add_sg_size(struct lio_sg_entry *sg_entry, uint16_t size,
                                   uint32_t pos) {
#if RTE_BYTE_ORDER == RTE_BIG_ENDIAN
 ...
 */
static inline uint32_t lio_incr_index(uint32_t index, uint32_t count,
                                      uint32_t max) {
  if ((index + count) >= max)
