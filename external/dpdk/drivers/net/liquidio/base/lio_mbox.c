...
int lio_mbox_read(struct lio_mbox *mbox) {
  ...
  if (mbox->state & LIO_MBOX_STATE_REQ_RECEIVING) {
    mbox->mbox_req.data[mbox->mbox_req.recv_len - 1] = msg.mbox_msg64;
    mbox->mbox_req.recv_len++;
  } else {
    if (mbox->state & LIO_MBOX_STATE_RES_RECEIVING) {
      mbox->mbox_resp.data[mbox->mbox_resp.recv_len - 1] = msg.mbox_msg64;
      ...
      if ((mbox->state & LIO_MBOX_STATE_IDLE) &&
        ...
        if ((mbox->state & LIO_MBOX_STATE_RES_PENDING) &&
          ...
          mbox->mbox_resp.msg.mbox_msg64 = msg.mbox_msg64;
          mbox->mbox_resp.q_no = mbox->q_no;
          mbox->mbox_resp.recv_len = 1;
        } else {
          rte_write64(LIO_PFVFERR, mbox->mbox_read_reg);
 ...
 */
int lio_mbox_write(struct lio_device *lio_dev, struct lio_mbox_cmd *mbox_cmd) {
  ...
  if (mbox_cmd->msg.s.type == LIO_MBOX_REQUEST) {
    rte_memcpy(&mbox->mbox_resp, mbox_cmd, sizeof(struct lio_mbox_cmd));
    mbox->state = LIO_MBOX_STATE_RES_PENDING;
  }
  ...
  if (ret == LIO_MBOX_STATUS_SUCCESS) {
    ...
    for (i = 0; i < (uint32_t)(mbox_cmd->msg.s.len - 1); i++) {
      count = 0;
      while (rte_read64(mbox->mbox_write_reg) != LIO_PFVFACK) {
        rte_delay_ms(1);
        if (count++ == 1000) {
          ret = LIO_MBOX_STATUS_FAILED;
          break;
  ...
  if (mbox_cmd->msg.s.type == LIO_MBOX_RESPONSE) {
    ...
    if ((!mbox_cmd->msg.s.resp_needed) || (ret == LIO_MBOX_STATUS_FAILED)) {
      mbox->state &= ~LIO_MBOX_STATE_RES_PENDING;
      if (!(mbox->state &
            (LIO_MBOX_STATE_REQ_RECEIVING | LIO_MBOX_STATE_REQ_RECEIVED)))
        mbox->state = LIO_MBOX_STATE_IDLE;
    }
  }
 ...
 */
static int lio_mbox_process_cmd(struct lio_mbox *mbox,
...
int lio_mbox_process_message(struct lio_mbox *mbox) {
  ...
  if (mbox->state & LIO_MBOX_STATE_ERROR) {
    if (mbox->state &
        (LIO_MBOX_STATE_RES_PENDING | LIO_MBOX_STATE_RES_RECEIVING)) {
      rte_memcpy(&mbox_cmd, &mbox->mbox_resp, sizeof(struct lio_mbox_cmd));
      mbox->state = LIO_MBOX_STATE_IDLE;
      rte_write64(LIO_PFVFSIG, mbox->mbox_read_reg);
  ...
  if (mbox->state & LIO_MBOX_STATE_REQ_RECEIVED) {
    rte_memcpy(&mbox_cmd, &mbox->mbox_req, sizeof(struct lio_mbox_cmd));
