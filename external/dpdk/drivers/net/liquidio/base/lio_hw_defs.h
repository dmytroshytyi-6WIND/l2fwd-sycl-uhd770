...
#define LIO_NUM_DEF_TX_DESCS_CFG(cfg) ((cfg)->default_config->num_def_tx_descs)
...
#define LIO_RSS_OFFLOAD_ALL                                                    \
  (LIO_RSS_HASH_IPV4 | LIO_RSS_HASH_TCP_IPV4 | LIO_RSS_HASH_IPV6 |             \
   LIO_RSS_HASH_TCP_IPV6 | LIO_RSS_HASH_IPV6_EX | LIO_RSS_HASH_TCP_IPV6_EX)
...
#define lio_write_csr(lio_dev, reg_off, value)                                 \
  do {                                                                         \
    ...
    PMD_REGS_LOG(_dev, "Write32: Reg: 0x%08lx Val: 0x%08lx\n",                 \
                 (unsigned long)_reg_off, (unsigned long)_value);              \
...
#define lio_write_csr64(lio_dev, reg_off, val64)                               \
  do {                                                                         \
    ...
    PMD_REGS_LOG(_dev, "Write64: Reg: 0x%08lx Val: 0x%016llx\n",               \
                 (unsigned long)_reg_off, (unsigned long long)_val64);         \
...
#define lio_read_csr(lio_dev, reg_off)                                         \
  ({                                                                           \
    ...
    PMD_REGS_LOG(_dev, "Read32: Reg: 0x%08lx Val: 0x%08lx\n",                  \
                 (unsigned long)_reg_off, (unsigned long)val);                 \
    val;                                                                       \
  })
...
#define lio_read_csr64(lio_dev, reg_off)                                       \
  ({                                                                           \
    ...
    PMD_REGS_LOG(_dev, "Read64: Reg: 0x%08lx Val: 0x%016llx\n",                \
                 (unsigned long)_reg_off, (unsigned long long)val64);          \
    val64;                                                                     \
  })
#else
#define lio_write_csr(lio_dev, reg_off, value)                                 \
