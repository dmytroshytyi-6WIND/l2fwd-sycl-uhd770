...
static int cn23xx_vf_reset_io_queues(struct lio_device *lio_dev,
                                     uint32_t num_queues) {
  ...
  for (q_no = 0; q_no < num_queues; q_no++) {
    ...
    d64 = lio_read_csr64(lio_dev, CN23XX_SLI_IQ_PKT_CONTROL64(q_no));
    d64 = d64 | CN23XX_PKT_INPUT_CTL_RST;
    lio_write_csr64(lio_dev, CN23XX_SLI_IQ_PKT_CONTROL64(q_no), d64);
  }
  ...
  for (q_no = 0; q_no < num_queues; q_no++) {
    ...
    reg_val = lio_read_csr64(lio_dev, CN23XX_SLI_IQ_PKT_CONTROL64(q_no));
    while ((reg_val & CN23XX_PKT_INPUT_CTL_RST) &&
           !(reg_val & CN23XX_PKT_INPUT_CTL_QUIET) && loop) {
      reg_val = lio_read_csr64(lio_dev, CN23XX_SLI_IQ_PKT_CONTROL64(q_no));
      loop = loop - 1;
    }
    ...
    if (loop == 0) {
      lio_dev_err(lio_dev,
                  "clearing the reset reg failed or setting the quiet reg "
                  "failed for qno: %lu\n",
                  (unsigned long)q_no);
      return -1;
    }
    ...
    lio_write_csr64(lio_dev, CN23XX_SLI_IQ_PKT_CONTROL64(q_no), reg_val);

    reg_val = lio_read_csr64(lio_dev, CN23XX_SLI_IQ_PKT_CONTROL64(q_no));
    if (reg_val & CN23XX_PKT_INPUT_CTL_RST) {
      lio_dev_err(lio_dev, "clearing the reset failed for qno: %lu\n",
                  (unsigned long)q_no);
      ret_val = -1;
    }
  }
...
static int cn23xx_vf_setup_global_input_regs(struct lio_device *lio_dev) {
  uint64_t q_no;
  uint64_t d64;

  PMD_INIT_FUNC_TRACE();

  if (cn23xx_vf_reset_io_queues(lio_dev, lio_dev->sriov_info.rings_per_vf))
    return -1;

  for (q_no = 0; q_no < (lio_dev->sriov_info.rings_per_vf); q_no++) {
    lio_write_csr64(lio_dev, CN23XX_SLI_IQ_DOORBELL(q_no), 0xFFFFFFFF);

    d64 = lio_read_csr64(lio_dev, CN23XX_SLI_IQ_INSTR_COUNT64(q_no));

    d64 &= 0xEFFFFFFFFFFFFFFFL;

    lio_write_csr64(lio_dev, CN23XX_SLI_IQ_INSTR_COUNT64(q_no), d64);
...
static void cn23xx_vf_setup_global_output_regs(struct lio_device *lio_dev) {
  uint32_t reg_val;
  uint32_t q_no;
  ...
  for (q_no = 0; q_no < lio_dev->sriov_info.rings_per_vf; q_no++) {
    lio_write_csr(lio_dev, CN23XX_SLI_OQ_PKTS_CREDIT(q_no), 0xFFFFFFFF);

    reg_val = lio_read_csr(lio_dev, CN23XX_SLI_OQ_PKTS_SENT(q_no));

    reg_val &= 0xEFFFFFFFFFFFFFFFL;

    lio_write_csr(lio_dev, CN23XX_SLI_OQ_PKTS_SENT(q_no), reg_val);

    reg_val = lio_read_csr(lio_dev, CN23XX_SLI_OQ_PKT_CONTROL(q_no));

    /* set IPTR & DPTR */
    reg_val |= (CN23XX_PKT_OUTPUT_CTL_IPTR | CN23XX_PKT_OUTPUT_CTL_DPTR);
    ...
    lio_write_csr(lio_dev, CN23XX_SLI_OQ_PKT_CONTROL(q_no), reg_val);
  }
}

static int cn23xx_vf_setup_device_regs(struct lio_device *lio_dev) {
  PMD_INIT_FUNC_TRACE();
...
static void cn23xx_vf_setup_iq_regs(struct lio_device *lio_dev,
                                    uint32_t iq_no) {
  ...
  lio_write_csr64(lio_dev, CN23XX_SLI_IQ_BASE_ADDR64(iq_no), iq->base_addr_dma);
   ...
   */
  iq->doorbell_reg =
      (uint8_t *)lio_dev->hw_addr + CN23XX_SLI_IQ_DOORBELL(iq_no);
  iq->inst_cnt_reg =
      (uint8_t *)lio_dev->hw_addr + CN23XX_SLI_IQ_INSTR_COUNT64(iq_no);
...
static void cn23xx_vf_setup_oq_regs(struct lio_device *lio_dev,
                                    uint32_t oq_no) {
  ...
  droq->pkts_sent_reg =
      (uint8_t *)lio_dev->hw_addr + CN23XX_SLI_OQ_PKTS_SENT(oq_no);
  droq->pkts_credit_reg =
      (uint8_t *)lio_dev->hw_addr + CN23XX_SLI_OQ_PKTS_CREDIT(oq_no);
}

static void cn23xx_vf_free_mbox(struct lio_device *lio_dev) {
  PMD_INIT_FUNC_TRACE();
...
static int cn23xx_vf_setup_mbox(struct lio_device *lio_dev) {
  ...
  mbox->mbox_int_reg =
      (uint8_t *)lio_dev->hw_addr + CN23XX_VF_SLI_PKT_MBOX_INT(0);
  /* VF reads from SIG0 reg */
  mbox->mbox_read_reg =
      (uint8_t *)lio_dev->hw_addr + CN23XX_SLI_PKT_PF_VF_MBOX_SIG(0, 0);
  /* VF writes into SIG1 reg */
  mbox->mbox_write_reg =
      (uint8_t *)lio_dev->hw_addr + CN23XX_SLI_PKT_PF_VF_MBOX_SIG(0, 1);
...
static int cn23xx_vf_enable_io_queues(struct lio_device *lio_dev) {
  uint32_t q_no;
...
static void cn23xx_vf_disable_io_queues(struct lio_device *lio_dev) {
  uint32_t num_queues;
...
static void cn23xx_pfvf_hs_callback(struct lio_device *lio_dev,
...
int cn23xx_pfvf_handshake(struct lio_device *lio_dev) {
  ...
  for (q_no = 0; q_no < lio_dev->num_iqs; q_no++)
    lio_dev->instr_queue[q_no]->txpciq.s.pkind = lio_dev->pfvf_hsword.pkind;

  vfmajor = LIO_BASE_MAJOR_VERSION;
  pfmajor = ret >> 16;
  if (pfmajor != vfmajor) {
    lio_dev_err(lio_dev,
                "VF LiquidIO driver (major version %d) is not compatible with "
                "LiquidIO PF driver (major version %d)\n",
                vfmajor, pfmajor);
    ret = -EPERM;
  } else {
    lio_dev_dbg(lio_dev,
                "VF LiquidIO driver (major version %d), LiquidIO PF driver "
                "(major version %d)\n",
                vfmajor, pfmajor);
    ret = 0;
  }
...
int cn23xx_vf_setup_device(struct lio_device *lio_dev) {
  ...
  lio_dev->sriov_info.rings_per_vf = reg_val & CN23XX_PKT_INPUT_CTL_RPVF_MASK;
