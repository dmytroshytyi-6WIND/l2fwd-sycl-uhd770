...
static const char *valid_arguments[] = {ETH_NULL_PACKET_SIZE_ARG,
                                        ETH_NULL_PACKET_COPY_ARG,
                                        ETH_NULL_PACKET_NO_RX_ARG, NULL};
...
struct pmd_internals {
  ...
  struct rte_eth_rss_reta_entry64
      reta_conf[RTE_ETH_RSS_RETA_SIZE_128 / RTE_ETH_RETA_GROUP_SIZE];
...
#define PMD_LOG(level, fmt, args...)                                           \
  rte_log(RTE_LOG_##level, eth_null_logtype, "%s(): " fmt "\n", __func__,      \
          ##args)

static uint16_t eth_null_rx(void *q, struct rte_mbuf **bufs, uint16_t nb_bufs) {
  int i;
...
static uint16_t eth_null_copy_rx(void *q, struct rte_mbuf **bufs,
                                 uint16_t nb_bufs) {
  int i;
  struct null_queue *h = q;
  unsigned int packet_size;
  ...
  for (i = 0; i < nb_bufs; i++) {
    rte_memcpy(rte_pktmbuf_mtod(bufs[i], void *), h->dummy_packet, packet_size);
    bufs[i]->data_len = (uint16_t)packet_size;
    bufs[i]->pkt_len = packet_size;
    bufs[i]->port = h->internals->port_id;
  }
...
static uint16_t eth_null_no_rx(void *q __rte_unused,
                               struct rte_mbuf **bufs __rte_unused,
                               uint16_t nb_bufs __rte_unused) {
  return 0;
}

static uint16_t eth_null_tx(void *q, struct rte_mbuf **bufs, uint16_t nb_bufs) {
  int i;
  struct null_queue *h = q;
...
static uint16_t eth_null_copy_tx(void *q, struct rte_mbuf **bufs,
                                 uint16_t nb_bufs) {
  int i;
  struct null_queue *h = q;
  unsigned int packet_size;
  ...
  packet_size = h->internals->packet_size;
  for (i = 0; i < nb_bufs; i++) {
    rte_memcpy(h->dummy_packet, rte_pktmbuf_mtod(bufs[i], void *), packet_size);
    rte_pktmbuf_free(bufs[i]);
  }
...
static int eth_mtu_set(struct rte_eth_dev *dev __rte_unused,
...
static int eth_stats_get(struct rte_eth_dev *dev,
  ...
  num_stats = RTE_MIN(
      ...
      RTE_MIN(dev->data->nb_rx_queues, RTE_DIM(internal->rx_null_queues)));
  for (i = 0; i < num_stats; i++) {
    igb_stats->q_ipackets[i] = internal->rx_null_queues[i].rx_pkts.cnt;
    rx_total += igb_stats->q_ipackets[i];
  }

  num_stats = RTE_MIN(
      (unsigned int)RTE_ETHDEV_QUEUE_STAT_CNTRS,
      RTE_MIN(dev->data->nb_tx_queues, RTE_DIM(internal->tx_null_queues)));
  for (i = 0; i < num_stats; i++) {
    igb_stats->q_opackets[i] = internal->tx_null_queues[i].tx_pkts.cnt;
...
static int eth_rss_reta_update(struct rte_eth_dev *dev,
...
static int eth_rss_reta_query(struct rte_eth_dev *dev,
                              struct rte_eth_rss_reta_entry64 *reta_conf,
                              uint16_t reta_size) {
  int i, j;
  struct pmd_internals *internal = dev->data->dev_private;
...
static int eth_rss_hash_conf_get(struct rte_eth_dev *dev,
                                 struct rte_eth_rss_conf *rss_conf) {
  struct pmd_internals *internal = dev->data->dev_private;
...
static int eth_mac_address_set(__rte_unused struct rte_eth_dev *dev,
                               __rte_unused struct rte_ether_addr *addr) {
  return 0;
}

static int eth_dev_close(struct rte_eth_dev *dev) {
  PMD_LOG(INFO, "Closing null ethdev on NUMA socket %u", rte_socket_id());
...
static const struct eth_dev_ops ops = {.dev_close = eth_dev_close,
                                       ...
                                       .rss_hash_conf_get =
                                           eth_rss_hash_conf_get};

static int eth_dev_null_create(struct rte_vdev_device *dev,
  ...
  internals->reta_size =
      RTE_DIM(internals->reta_conf) * RTE_ETH_RETA_GROUP_SIZE;
...
static inline int get_packet_size_arg(const char *key __rte_unused,
...
static inline int get_packet_copy_arg(const char *key __rte_unused,
                                      const char *value, void *extra_args) {
  const char *a = value;
...
static int get_packet_no_rx_arg(const char *key __rte_unused, const char *value,
                                void *extra_args) {
  const char *a = value;
  unsigned int no_rx;
...
static int rte_pmd_null_probe(struct rte_vdev_device *dev) {
  ...
  if (params != NULL) {
    ...
    ret = rte_kvargs_process(kvlist, ETH_NULL_PACKET_NO_RX_ARG,
                             &get_packet_no_rx_arg, &args.no_rx);
    if (ret < 0)
      goto free_kvlist;

    if (args.no_rx && args.packet_copy) {
      PMD_LOG(ERR, "Both %s and %s arguments at the same time not supported",
              ETH_NULL_PACKET_COPY_ARG, ETH_NULL_PACKET_NO_RX_ARG);
      goto free_kvlist;
    }
  }
  ...
  PMD_LOG(INFO,
          ...
          "packet copy is %s",
          args.packet_size, args.packet_copy ? "enabled" : "disabled");
...
static int rte_pmd_null_remove(struct rte_vdev_device *dev) {
  struct rte_eth_dev *eth_dev = NULL;
