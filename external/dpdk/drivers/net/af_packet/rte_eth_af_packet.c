...
static const char *valid_arguments[] = {ETH_AF_PACKET_IFACE_ARG,
...
#define PMD_LOG(level, fmt, args...)                                           \
  rte_log(RTE_LOG_##level, af_packet_logtype, "%s(): " fmt "\n", __func__,     \
          ##args)
...
static uint16_t eth_af_packet_rx(void *queue, struct rte_mbuf **bufs,
                                 uint16_t nb_pkts) {
  unsigned i;
 ...
 */
static inline bool tx_ring_status_available(uint32_t tp_status) {
  /*
 ...
 */
static uint16_t eth_af_packet_tx(void *queue, struct rte_mbuf **bufs,
                                 uint16_t nb_pkts) {
  struct tpacket2_hdr *ppd;
  struct rte_mbuf *mbuf;
  uint8_t *pbuf;
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    pbuf = (uint8_t *)ppd + TPACKET2_HDRLEN - sizeof(struct sockaddr_ll);
...
static int eth_dev_close(struct rte_eth_dev *dev) {
  ...
  PMD_LOG(INFO, "Closing AF_PACKET ethdev on NUMA socket %u", rte_socket_id());
...
static int eth_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
  ...
  if (data_size > buf_size) {
    PMD_LOG(ERR, "%s: %d bytes will not fit in mbuf (%d bytes)",
...
static int eth_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  ...
  int ret;
  int s;
  unsigned int data_size = internals->req.tp_frame_size - TPACKET2_HDRLEN;
...
static int eth_dev_macaddr_set(struct rte_eth_dev *dev,
 ...
 */
static int open_packet_iface(const char *key __rte_unused,
                             const char *value __rte_unused, void *extra_args) {
...
static int rte_pmd_init_internals(struct rte_vdev_device *dev, const int sockfd,
                                  const unsigned nb_queues,
                                  unsigned int blocksize, unsigned int blockcnt,
                                  unsigned int framesize, unsigned int framecnt,
  ...
  if (pair == NULL) {
    PMD_LOG(ERR, "%s: no interface specified for AF_PACKET ethdev", name);
    return -1;
  }

  PMD_LOG(INFO, "%s: creating AF_PACKET-backed ethdev on numa socket %u", name,
          numa_node);
  ...
  *internals = rte_zmalloc_socket(name, sizeof(**internals), 0, numa_node);
  ...
  (*internals)->rx_queue = rte_calloc_socket(
      "af_packet_rx", nb_queues, sizeof(struct pkt_rx_queue), 0, numa_node);
  (*internals)->tx_queue = rte_calloc_socket(
      "af_packet_tx", nb_queues, sizeof(struct pkt_tx_queue), 0, numa_node);
  ...
  if (ifnamelen < sizeof(ifr.ifr_name)) {
    ...
    PMD_LOG(ERR, "%s: I/F name too long (%s)", name, pair->value);
    goto free_internals;
  }
  ...
  for (q = 0; q < nb_queues; q++) {
    ...
    if (qsockfd == -1) {
      PMD_LOG_ERRNO(ERR, "%s: could not open AF_PACKET socket", name);
      goto error;
    }

    tpver = TPACKET_V2;
    rc = setsockopt(qsockfd, SOL_PACKET, PACKET_VERSION, &tpver, sizeof(tpver));
    if (rc == -1) {
      PMD_LOG_ERRNO(
          ERR, "%s: could not set PACKET_VERSION on AF_PACKET socket for %s",
          name, pair->value);
      goto error;
    }
    ...
    discard = 1;
    rc =
        setsockopt(qsockfd, SOL_PACKET, PACKET_LOSS, &discard, sizeof(discard));
    if (rc == -1) {
      PMD_LOG_ERRNO(ERR,
    ...
    if (qdisc_bypass) {
      ...
      rc = setsockopt(qsockfd, SOL_PACKET, PACKET_QDISC_BYPASS, &qdisc_bypass,
                      sizeof(qdisc_bypass));
      if (rc == -1) {
        PMD_LOG_ERRNO(
            ERR,
    ...
    if (rc == -1) {
      PMD_LOG_ERRNO(
          ERR, "%s: could not set PACKET_RX_RING on AF_PACKET socket for %s",
          name, pair->value);
      goto error;
    }
    ...
    rx_queue->map =
        mmap(NULL, 2 * req->tp_block_size * req->tp_block_nr,
             PROT_READ | PROT_WRITE, MAP_SHARED | MAP_LOCKED, qsockfd, 0);
    if (rx_queue->map == MAP_FAILED) {
      PMD_LOG_ERRNO(ERR, "%s: call to mmap failed on AF_PACKET socket for %s",
                    name, pair->value);
      goto error;
    }
    ...
    if (rc == -1) {
      PMD_LOG_ERRNO(ERR, "%s: could not bind AF_PACKET socket to %s", name,
                    pair->value);
      goto error;
    }
    ...
    if (rc == -1) {
      PMD_LOG_ERRNO(
          ERR, "%s: could not set PACKET_FANOUT on AF_PACKET socket for %s",
          name, pair->value);
      goto error;
    }
#endif
...
static int rte_eth_from_packet(struct rte_vdev_device *dev, int const *sockfd,
                               struct rte_kvargs *kvlist) {
  const char *name = rte_vdev_device_name(dev);
  ...
  for (k_idx = 0; k_idx < kvlist->count; k_idx++) {
    ...
    if (strstr(pair->key, ETH_AF_PACKET_BLOCKSIZE_ARG) != NULL) {
      ...
      if (!blocksize) {
        PMD_LOG(ERR, "%s: invalid blocksize value", name);
        return -1;
      }
      continue;
    }
    if (strstr(pair->key, ETH_AF_PACKET_FRAMESIZE_ARG) != NULL) {
      ...
      if (!framesize) {
        PMD_LOG(ERR, "%s: invalid framesize value", name);
        return -1;
      }
      continue;
    }
    if (strstr(pair->key, ETH_AF_PACKET_FRAMECOUNT_ARG) != NULL) {
      ...
      if (!framecount) {
        PMD_LOG(ERR, "%s: invalid framecount value", name);
        return -1;
      }
      continue;
    }
    if (strstr(pair->key, ETH_AF_PACKET_QDISC_BYPASS_ARG) != NULL) {
      ...
      if (qdisc_bypass > 1) {
        PMD_LOG(ERR, "%s: invalid bypass value", name);
        return -1;
      }
      continue;
    }
  ...
  if (framesize > blocksize) {
    PMD_LOG(ERR, "%s: AF_PACKET MMAP frame size exceeds block size!", name);
    return -1;
  }

  blockcount = framecount / (blocksize / framesize);
  if (!blockcount) {
    PMD_LOG(ERR, "%s: invalid AF_PACKET MMAP parameters", name);
    return -1;
  }
  ...
  if (rte_pmd_init_internals(dev, *sockfd, qpairs, blocksize, blockcount,
...
static int rte_pmd_af_packet_probe(struct rte_vdev_device *dev) {
  int ret = 0;
  struct rte_kvargs *kvlist;
  int sockfd = -1;
  struct rte_eth_dev *eth_dev;
...
static int rte_pmd_af_packet_remove(struct rte_vdev_device *dev) {
  struct rte_eth_dev *eth_dev;
...
RTE_PMD_REGISTER_PARAM_STRING(net_af_packet, "iface=<string> "
                                             "qpairs=<int> "
                                             "blocksz=<int> "
                                             "framesz=<int> "
                                             "framecnt=<int> "
