...
#define VMXNET3_TX_OFFLOAD_MASK                                                \
  (RTE_MBUF_F_TX_VLAN | RTE_MBUF_F_TX_IPV6 | RTE_MBUF_F_TX_IPV4 |              \
...
#ifdef RTE_LIBRTE_VMXNET3_DEBUG_DRIVER_NOT_USED
static void vmxnet3_rxq_dump(struct vmxnet3_rx_queue *rxq) {
  uint32_t avail = 0;

  if (rxq == NULL)
    return;

  PMD_RX_LOG(DEBUG, "RXQ: cmd0 base : %p cmd1 base : %p comp ring base : %p.",
...
static void vmxnet3_txq_dump(struct vmxnet3_tx_queue *txq) {
  uint32_t avail = 0;
  ...
  PMD_TX_LOG(DEBUG,
             "TXQ: cmd base : %p comp ring base : %p data ring base : %p.",
             txq->cmd_ring.base, txq->comp_ring.base, txq->data_ring.base);
  PMD_TX_LOG(DEBUG,
             "TXQ: cmd basePA : 0x%lx comp ring basePA : 0x%lx data ring "
             "basePA : 0x%lx.",
  ...
  PMD_TX_LOG(DEBUG, "TXQ: size=%u; free=%u; next2proc=%u; queued=%u",
             (uint32_t)txq->cmd_ring.size, avail, txq->comp_ring.next2proc,
             txq->cmd_ring.size - avail);
}
#endif

static void vmxnet3_tx_cmd_ring_release_mbufs(vmxnet3_cmd_ring_t *ring) {
...
static void vmxnet3_rx_cmd_ring_release_mbufs(vmxnet3_cmd_ring_t *ring) {
  uint32_t i;
...
static void vmxnet3_cmd_ring_release(vmxnet3_cmd_ring_t *ring) {
...
static void vmxnet3_dev_tx_queue_reset(void *txq) {
...
static void vmxnet3_dev_rx_queue_reset(void *rxq) {
  int i;
...
static int vmxnet3_unmap_pkt(uint16_t eop_idx, vmxnet3_tx_queue_t *txq) {
  int completed = 0;
...
static void vmxnet3_tq_tx_complete(vmxnet3_tx_queue_t *txq) {
  int completed = 0;
  vmxnet3_comp_ring_t *comp_ring = &txq->comp_ring;
  struct Vmxnet3_TxCompDesc *tcd =
      (struct Vmxnet3_TxCompDesc *)(comp_ring->base + comp_ring->next2proc);

  while (tcd->gen == comp_ring->gen) {
    completed += vmxnet3_unmap_pkt(tcd->txdIdx, txq);

    vmxnet3_comp_ring_adv_next2proc(comp_ring);
    tcd = (struct Vmxnet3_TxCompDesc *)(comp_ring->base + comp_ring->next2proc);
  }
...
uint16_t vmxnet3_prep_pkts(__rte_unused void *tx_queue,
...
uint16_t vmxnet3_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  while (nb_tx < nb_pkts) {
    ...
    if (count > avail) {
       ...
       */
      PMD_TX_LOG(DEBUG,
                 "Running out of ring descriptors "
                 "(avail %d needed %d)",
                 avail, count);
    ...
    if (unlikely(!tso && count > VMXNET3_MAX_TXD_PER_PKT)) {
      PMD_TX_LOG(ERR,
                 "Non-TSO packet cannot occupy more than %d tx "
    ...
    if (txm->nb_segs == 1 &&
      ...
      tdd = (struct Vmxnet3_TxDataDesc *)((uint8 *)txq->data_ring.base +
    ...
    do {
      ...
      if (copy_size) {
        uint64 offset = (uint64)txq->cmd_ring.next2fill * txq->txdata_desc_size;
        gdesc->txd.addr = rte_cpu_to_le_64(txq->data_ring.basePA + offset);
      } else {
    ...
    if (tso) {
      ...
      case RTE_MBUF_F_TX_TCP_CKSUM:
        gdesc->txd.msscof =
            gdesc->txd.hlen + offsetof(struct rte_tcp_hdr, cksum);
        break;
      case RTE_MBUF_F_TX_UDP_CKSUM:
        gdesc->txd.msscof =
            gdesc->txd.hlen + offsetof(struct rte_udp_hdr, dgram_cksum);
        break;
      default:
  ...
  if (deferred >= rte_le_to_cpu_32(txq_ctrl->txThreshold)) {
    ...
    VMXNET3_WRITE_BAR0_REG(
        hw, (VMXNET3_REG_TXPROD + txq->queue_id * VMXNET3_REG_ALIGN),
        txq->cmd_ring.next2fill);
  }
...
static inline void vmxnet3_renew_desc(vmxnet3_rx_queue_t *rxq, uint8_t ring_id,
                                      struct rte_mbuf *mbuf) {
  uint32_t val;
 ...
 *      only for LRO.
 */
static int vmxnet3_post_rx_bufs(vmxnet3_rx_queue_t *rxq, uint8_t ring_id) {
  int err = 0;
  uint32_t i = 0;
  struct vmxnet3_cmd_ring *ring = &rxq->cmd_ring[ring_id];
...
static uint16_t vmxnet3_guess_mss(struct vmxnet3_hw *hw,
  ...
  if (rcd->v4) {
    if (unlikely(slen < hlen + sizeof(struct rte_ipv4_hdr)))
      return hw->mtu - sizeof(struct rte_ipv4_hdr) - sizeof(struct rte_tcp_hdr);

    ipv4_hdr = (struct rte_ipv4_hdr *)(ptr + hlen);
    hlen += rte_ipv4_hdr_len(ipv4_hdr);
  } else if (rcd->v6) {
    if (unlikely(slen < hlen + sizeof(struct rte_ipv6_hdr)))
      return hw->mtu - sizeof(struct rte_ipv6_hdr) - sizeof(struct rte_tcp_hdr);
    ...
    if (unlikely(ipv6_hdr->proto != IPPROTO_TCP)) {
      ...
      rte_net_skip_ip6_ext(ipv6_hdr->proto, rxm, &hlen, &frag);
    }
  }
...
static inline void vmxnet3_rx_offload(struct vmxnet3_hw *hw,
                                      const Vmxnet3_RxCompDesc *rcd,
  ...
  if (sop) {
    ...
    if (VMXNET3_VERSION_GE_2(hw) && rcd->type == VMXNET3_CDTYPE_RXCOMP_LRO) {
      const Vmxnet3_RxCompDescExt *rcde = (const Vmxnet3_RxCompDescExt *)rcd;
    ...
    if (rcd->cnc) {
      ...
      if ((ol_flags & RTE_MBUF_F_RX_LRO) && rxm->tso_segsz == 0)
        rxm->tso_segsz = vmxnet3_guess_mss(hw, rcd, rxm);
    }
  }
  ...
  while (rcd->gen == rxq->comp_ring.gen) {
    ...
    if (unlikely(rcd->eop && rcd->err)) {
      ...
      PMD_RX_LOG(ERR, "Error in received packet rcd#:%d rxd:%d",
                 (int)(rcd - (struct Vmxnet3_RxCompDesc *)rxq->comp_ring.base),
                 rcd->rxdIdx);
    ...
    if (rcd->sop) {
      ...
      if (unlikely(rcd->len == 0)) {
        ...
        PMD_RX_LOG(DEBUG, "Rx buf was skipped. rxring[%d][%d])", ring_idx, idx);
        rte_pktmbuf_free_seg(rxm);
        goto rcd_done;
      }

      if (vmxnet3_rx_data_ring(hw, rcd->rqID)) {
        uint8_t *rdd = rxq->data_ring.base + idx * rxq->data_desc_size;

        RTE_ASSERT(VMXNET3_VERSION_GE_3(hw));
        rte_memcpy(rte_pktmbuf_mtod(rxm, char *), rdd, rcd->len);
      }
...
int vmxnet3_dev_tx_queue_setup(
  ...
  if (nb_desc < VMXNET3_DEF_TX_RING_SIZE) {
    PMD_INIT_LOG(ERR, "VMXNET3 Tx Ring Size Min: %u", VMXNET3_DEF_TX_RING_SIZE);
    return -EINVAL;
  } else if (nb_desc > VMXNET3_TX_RING_MAX_SIZE) {
    PMD_INIT_LOG(ERR, "VMXNET3 Tx Ring Size Max: %u", VMXNET3_TX_RING_MAX_SIZE);
    return -EINVAL;
  } else {
    ring->size = nb_desc;
  ...
  comp_ring->basePA =
      ring->basePA + (sizeof(struct Vmxnet3_TxDesc) * ring->size);

  /* data_ring initialization */
  data_ring->base = (Vmxnet3_TxDataDesc *)(comp_ring->base + comp_ring->size);
  data_ring->basePA =
      comp_ring->basePA + (sizeof(struct Vmxnet3_TxCompDesc) * comp_ring->size);

  /* cmd_ring0 buf_info allocation */
  ring->buf_info =
      rte_zmalloc("tx_ring_buf_info", ring->size * sizeof(vmxnet3_buf_info_t),
                  RTE_CACHE_LINE_SIZE);
...
int vmxnet3_dev_rx_queue_setup(
    struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t nb_desc,
    unsigned int socket_id, __rte_unused const struct rte_eth_rxconf *rx_conf,
  ...
  comp_ring->basePA =
      ring1->basePA + sizeof(struct Vmxnet3_RxDesc) * ring1->size;

  /* data_ring initialization */
  if (VMXNET3_VERSION_GE_3(hw) && rxq->data_desc_size) {
    data_ring->base = (uint8_t *)(comp_ring->base + comp_ring->size);
    data_ring->basePA =
        comp_ring->basePA + sizeof(struct Vmxnet3_RxCompDesc) * comp_ring->size;
  }
  ...
  for (i = 0; i < VMXNET3_RX_CMDRING_SIZE; i++) {
    ...
    ring->buf_info = rte_zmalloc(
        mem_name, ring->size * sizeof(vmxnet3_buf_info_t), RTE_CACHE_LINE_SIZE);
    if (ring->buf_info == NULL) {
...
int vmxnet3_dev_rxtx_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < hw->num_rx_queues; i++) {
    ...
    for (j = 0; j < VMXNET3_RX_CMDRING_SIZE; j++) {
      ...
      if (unlikely(rxq->shared->ctrl.updateRxProd)) {
        VMXNET3_WRITE_BAR0_REG(
            hw, rxprod_reg[j] + (rxq->queue_id * VMXNET3_REG_ALIGN),
...
int vmxnet3_v4_rss_configure(struct rte_eth_dev *dev) {
  ...
  rss_hf =
      port_rss_conf->rss_hf & (VMXNET3_V4_RSS_MASK | VMXNET3_RSS_OFFLOAD_ALL);
  ...
  VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_CMD, VMXNET3_CMD_SET_RSS_FIELDS);
