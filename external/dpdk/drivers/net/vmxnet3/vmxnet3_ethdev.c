...
#define VMXNET3_TX_OFFLOAD_CAP                                                 \
  (RTE_ETH_TX_OFFLOAD_VLAN_INSERT | RTE_ETH_TX_OFFLOAD_TCP_CKSUM |             \
   RTE_ETH_TX_OFFLOAD_UDP_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_TSO |                 \
   RTE_ETH_TX_OFFLOAD_MULTI_SEGS)

#define VMXNET3_RX_OFFLOAD_CAP                                                 \
  (RTE_ETH_RX_OFFLOAD_VLAN_STRIP | RTE_ETH_RX_OFFLOAD_VLAN_FILTER |            \
   RTE_ETH_RX_OFFLOAD_SCATTER | RTE_ETH_RX_OFFLOAD_UDP_CKSUM |                 \
...
static int vmxnet3_dev_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vid,
                                       int on);
...
static const struct vmxnet3_xstats_name_off vmxnet3_txq_stat_strings[] = {
    ...
    {"drop_too_many_segs",
     offsetof(struct vmxnet3_txq_stats, drop_too_many_segs)},
...
static const struct vmxnet3_xstats_name_off vmxnet3_rxq_stat_strings[] = {
    ...
    {"rx_buf_alloc_failure",
     offsetof(struct vmxnet3_rxq_stats, rx_buf_alloc_failure)},
};

static const struct rte_memzone *gpa_zone_reserve(struct rte_eth_dev *dev,
                                                  uint32_t size,
                                                  const char *post_string,
                                                  int socket_id, uint16_t align,
                                                  bool reuse) {
  char z_name[RTE_MEMZONE_NAMESIZE];
  const struct rte_memzone *mz;

  snprintf(z_name, sizeof(z_name), "eth_p%d_%s", dev->data->port_id,
           post_string);
 ...
 */
static void vmxnet3_enable_intr(struct vmxnet3_hw *hw, unsigned int intr_idx) {
  PMD_INIT_FUNC_TRACE();
 ...
 */
static void vmxnet3_disable_intr(struct vmxnet3_hw *hw, unsigned int intr_idx) {
  PMD_INIT_FUNC_TRACE();
 ...
 */
static void vmxnet3_enable_all_intrs(struct vmxnet3_hw *hw) {
 ...
 */
static void vmxnet3_disable_all_intrs(struct vmxnet3_hw *hw) {
  int i;
 ...
 */
static uint16_t eth_vmxnet3_txdata_get(struct vmxnet3_hw *hw) {
  uint16 txdata_desc_size;

  VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_CMD, VMXNET3_CMD_GET_TXDATA_DESC_SIZE);
...
static int eth_vmxnet3_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  if (ver & (1 << VMXNET3_REV_4)) {
    VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_VRRS, 1 << VMXNET3_REV_4);
    hw->version = VMXNET3_REV_4 + 1;
  } else if (ver & (1 << VMXNET3_REV_3)) {
    VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_VRRS, 1 << VMXNET3_REV_3);
    hw->version = VMXNET3_REV_3 + 1;
  } else if (ver & (1 << VMXNET3_REV_2)) {
    VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_VRRS, 1 << VMXNET3_REV_2);
    hw->version = VMXNET3_REV_2 + 1;
  } else if (ver & (1 << VMXNET3_REV_1)) {
    VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_VRRS, 1 << VMXNET3_REV_1);
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("vmxnet3", RTE_ETHER_ADDR_LEN * VMXNET3_MAX_MAC_ADDRS, 0);
  ...
  PMD_INIT_LOG(DEBUG, "MAC Address : " RTE_ETHER_ADDR_PRT_FMT, hw->perm_addr[0],
               hw->perm_addr[1], hw->perm_addr[2], hw->perm_addr[3],
  ...
  hw->txdata_desc_size = VMXNET3_VERSION_GE_3(hw)
                             ? eth_vmxnet3_txdata_get(hw)
                             : sizeof(struct Vmxnet3_TxDataDesc);

  hw->rxdata_desc_size =
      VMXNET3_VERSION_GE_3(hw) ? VMXNET3_DEF_RXDATA_DESC_SIZE : 0;
...
static int eth_vmxnet3_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                                 struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct vmxnet3_hw),
...
static void vmxnet3_alloc_intr_resources(struct rte_eth_dev *dev) {
  struct vmxnet3_hw *hw = dev->data->dev_private;
  uint32_t cfg;
  int nvec = 1; /* for link event */

  /* intr settings */
  VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_CMD, VMXNET3_CMD_GET_CONF_INTR);
...
static int vmxnet3_dev_configure(struct rte_eth_dev *dev) {
   ...
   * on current socket
   */
  mz = gpa_zone_reserve(dev, sizeof(struct Vmxnet3_DriverShared), "shared",
                        rte_socket_id(), 8, 1);
  ...
  if (dev->data->dev_conf.rxmode.mq_mode == RTE_ETH_MQ_RX_RSS) {
    ...
    mz = gpa_zone_reserve(dev, sizeof(struct VMXNET3_RSSConf), "rss_conf",
                          rte_socket_id(), RTE_CACHE_LINE_SIZE, 1);
    if (mz == NULL) {
...
static void vmxnet3_write_mac(struct vmxnet3_hw *hw, const uint8_t *addr) {
  uint32_t val;

  PMD_INIT_LOG(DEBUG, "Writing MAC Address : " RTE_ETHER_ADDR_PRT_FMT, addr[0],
 ...
 */
static int vmxnet3_configure_msix(struct rte_eth_dev *dev) {
  struct vmxnet3_hw *hw = dev->data->dev_private;
  ...
  if (!rte_intr_allow_others(intr_handle) &&
    ...
    PMD_INIT_LOG(ERR,
                 "not enough intr vector to support both Rx interrupt and LSC");
   ...
   * interrupt mode.
   */
  if (hw->intr.num_intrs != (rte_intr_nb_efd_get(intr_handle) + 1)) {
    PMD_INIT_LOG(ERR, "Device configured with %d Rx intr vectors, expecting %d",
                 hw->intr.num_intrs, rte_intr_nb_efd_get(intr_handle) + 1);
    rte_intr_vec_list_free(intr_handle);
    rte_intr_efd_disable(intr_handle);
    return -1;
  }
...
static int vmxnet3_dev_setup_memreg(struct rte_eth_dev *dev) {
  struct vmxnet3_hw *hw = dev->data->dev_private;
  ...
  if (hw->memRegsPA == 0) {
    ...
    mz = gpa_zone_reserve(dev, size, "memRegs", rte_socket_id(), 8, 1);
    if (mz == NULL) {
  ...
  for (i = 0; i < num; i++) {
    ...
    mr->startPA = (uintptr_t)STAILQ_FIRST(&mp[i]->mem_list)->iova;
    mr->length = STAILQ_FIRST(&mp[i]->mem_list)->len <= INT32_MAX
                     ? STAILQ_FIRST(&mp[i]->mem_list)->len
                     : INT32_MAX;
  ...
  size = sizeof(Vmxnet3_MemRegs) + (j - 1) * sizeof(Vmxnet3_MemoryRegion);
...
static int vmxnet3_setup_driver_shared(struct rte_eth_dev *dev) {
  ...
  devRead->misc.driverInfo.gos.gosBits =
      sizeof(void *) == 4 ? VMXNET3_GOS_BITS_32 : VMXNET3_GOS_BITS_64;
  ...
  for (i = 0; i < hw->num_tx_queues; i++) {
    ...
    if (hw->intr.lsc_only)
      ...
      tqd->conf.intrIdx = rte_intr_vec_list_index_get(intr_handle, i);
  ...
  for (i = 0; i < hw->num_rx_queues; i++) {
    ...
    if (hw->intr.lsc_only)
      ...
      rqd->conf.intrIdx = rte_intr_vec_list_index_get(intr_handle, i);
  ...
  ret = vmxnet3_dev_vlan_offload_set(dev, RTE_ETH_VLAN_STRIP_MASK |
...
static int vmxnet3_dev_start(struct rte_eth_dev *dev) {
  ...
  if (ret == 0) {
    VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_CMD, VMXNET3_CMD_REGISTER_MEMREGS);
    ret = VMXNET3_READ_BAR1_REG(hw, VMXNET3_REG_CMD);
    if (ret != 0)
  ...
  rte_intr_callback_register(dev->intr_handle, vmxnet3_interrupt_handler, dev);
...
static int vmxnet3_dev_stop(struct rte_eth_dev *dev) {
  ...
  do {
     ...
     */
    ret = rte_intr_callback_unregister(intr_handle, vmxnet3_interrupt_handler,
                                       (void *)-1);
  } while (ret == -EAGAIN);

  if (ret < 0)
    PMD_DRV_LOG(ERR, "Error attempting to unregister intr cb: %d", ret);
...
static void vmxnet3_free_queues(struct rte_eth_dev *dev) {
  int i;
...
static void vmxnet3_hw_tx_stats_get(struct vmxnet3_hw *hw, unsigned int q,
                                    struct UPT1_TxStats *res) {
#define VMXNET3_UPDATE_TX_STAT(h, i, f, r)                                     \
  ((r)->f = (h)->tqd_start[(i)].stats.f + (h)->saved_tx_stats[(i)].f)
...
static void vmxnet3_hw_rx_stats_get(struct vmxnet3_hw *hw, unsigned int q,
                                    struct UPT1_RxStats *res) {
#define VMXNET3_UPDATE_RX_STAT(h, i, f, r)                                     \
  ((r)->f = (h)->rqd_start[(i)].stats.f + (h)->saved_rx_stats[(i)].f)
...
static void vmxnet3_tx_stats_get(struct vmxnet3_hw *hw, unsigned int q,
                                 struct UPT1_TxStats *res) {
...
static void vmxnet3_rx_stats_get(struct vmxnet3_hw *hw, unsigned int q,
                                 struct UPT1_RxStats *res) {
...
static int vmxnet3_dev_xstats_get_names(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    for (t = 0; t < RTE_DIM(vmxnet3_rxq_stat_strings); t++) {
      snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
               "rx_q%u_%s", i, vmxnet3_rxq_stat_strings[t].name);
      count++;
    }
  }
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    for (t = 0; t < RTE_DIM(vmxnet3_txq_stat_strings); t++) {
      snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
               "tx_q%u_%s", i, vmxnet3_txq_stat_strings[t].name);
      count++;
    }
  }
...
static int vmxnet3_dev_stats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < hw->num_tx_queues; i++) {
    ...
    stats->q_opackets[i] =
        txStats.ucastPktsTxOK + txStats.mcastPktsTxOK + txStats.bcastPktsTxOK;

    stats->q_obytes[i] = txStats.ucastBytesTxOK + txStats.mcastBytesTxOK +
  ...
  for (i = 0; i < hw->num_rx_queues; i++) {
    ...
    stats->q_ipackets[i] =
        rxStats.ucastPktsRxOK + rxStats.mcastPktsRxOK + rxStats.bcastPktsRxOK;

    stats->q_ibytes[i] = rxStats.ucastBytesRxOK + rxStats.mcastBytesRxOK +
...
static const uint32_t *
vmxnet3_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {RTE_PTYPE_L3_IPV4_EXT, RTE_PTYPE_L3_IPV4,
                                    RTE_PTYPE_UNKNOWN};
...
static int vmxnet3_dev_mtu_set(struct rte_eth_dev *dev,
                               __rte_unused uint16_t mtu) {
...
static int vmxnet3_dev_promiscuous_enable(struct rte_eth_dev *dev) {
  ...
  VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_CMD, VMXNET3_CMD_UPDATE_VLAN_FILTERS);
...
static int vmxnet3_dev_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  ...
  if (mask & RTE_ETH_VLAN_STRIP_MASK) {
    ...
    VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_CMD, VMXNET3_CMD_UPDATE_FEATURE);
  }
...
static void vmxnet3_process_events(struct rte_eth_dev *dev) {
  ...
  if (events & (VMXNET3_ECR_TQERR | VMXNET3_ECR_RQERR)) {
    ...
    if (hw->tqd_start->status.stopped)
      PMD_DRV_LOG(ERR, "tq error 0x%x", hw->tqd_start->status.error);

    if (hw->rqd_start->status.stopped)
      PMD_DRV_LOG(ERR, "rq error 0x%x", hw->rqd_start->status.error);
...
RTE_PMD_REGISTER_KMOD_DEP(net_vmxnet3,
