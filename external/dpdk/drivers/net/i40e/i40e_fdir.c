...
static int i40e_sw_fdir_filter_insert(struct i40e_pf *pf,
                                      struct i40e_fdir_filter *filter);
static int i40e_flow_fdir_filter_programming(
    struct i40e_pf *pf, enum i40e_filter_pctype pctype,
    const struct i40e_fdir_filter_conf *filter, bool add, bool wait_status);
...
static int i40e_fdir_rx_queue_init(struct i40e_rx_queue *rxq) {
  ...
  rxq->qrx_tail = hw->hw_addr + I40E_QRX_TAIL(rxq->vsi->base_queue);
...
int i40e_fdir_setup(struct i40e_pf *pf) {
  ...
  PMD_DRV_LOG(INFO,
              "FDIR HW Capabilities: num_filters_guaranteed = %u,"
  ...
  snprintf(z_name, sizeof(z_name), "%s_%s_%d", eth_dev->device->driver->name,
           I40E_FDIR_MZ_NAME, eth_dev->data->port_id);
  mz = i40e_memzone_reserve(z_name, I40E_FDIR_PKT_LEN * I40E_FDIR_PRG_PKT_CNT,
                            SOCKET_ID_ANY);
  if (!mz) {
  ...
  for (i = 0; i < I40E_FDIR_PRG_PKT_CNT; i++) {
    pf->fdir.prg_pkt[i] = (uint8_t *)mz->addr + I40E_FDIR_PKT_LEN * i;
    pf->fdir.dma_addr[i] = mz->iova + I40E_FDIR_PKT_LEN * i;
  }

  pf->fdir.match_counter_index = I40E_COUNTER_INDEX_FDIR(hw->pf_id);
  pf->fdir.fdir_actual_cnt = 0;
  pf->fdir.fdir_guarantee_free_space = pf->fdir.fdir_guarantee_total_space;
...
static inline int i40e_fdir_empty(struct i40e_hw *hw) {
  ...
  best_cnt = (uint32_t)(
      (I40E_READ_REG(hw, I40E_PFQF_FDSTAT) & I40E_PFQF_FDSTAT_BEST_CNT_MASK) >>
 ...
 * and mask setting
 */
static inline void i40e_init_flx_pld(struct i40e_pf *pf) {
  struct i40e_hw *hw = I40E_PF_TO_HW(pf);
  uint8_t pctype;
  int i, index;
  uint16_t flow_type;
  ...
  for (i = I40E_FLXPLD_L2_IDX; i < I40E_MAX_FLXPLD_LAYER; i++) {
    ...
    I40E_WRITE_REG(hw, I40E_PRTQF_FLX_PIT(index + 1), 0x0000FC29); /*non-used*/
    I40E_WRITE_REG(hw, I40E_PRTQF_FLX_PIT(index + 2), 0x0000FC2A); /*non-used*/
...
#define I40E_VALIDATE_FLEX_PIT(flex_pit1, flex_pit2)                           \
  do {                                                                         \
    if ((flex_pit2).src_offset < (flex_pit1).src_offset + (flex_pit1).size) {  \
...
i40e_check_fdir_flex_conf(const struct i40e_adapter *adapter,
  ...
  for (i = 0; i < conf->nb_flexmasks; i++) {
    ...
    for (j = 0; j < I40E_FDIR_MAX_FLEX_LEN; j += sizeof(uint16_t)) {
      mask_tmp = I40E_WORD(flex_mask->mask[j], flex_mask->mask[j + 1]);
...
static void i40e_set_flx_pld_cfg(struct i40e_pf *pf,
  ...
  for (i = 0; i < num; i++) {
    ...
    pf->fdir.flex_set[field_idx].size = flex_pit[i].size / sizeof(uint16_t);
  ...
  for (; i < I40E_MAX_FLXPLD_FIED; i++) {
    ...
    flx_pit =
        MK_FLX_PIT(min_next_off, NONUSE_FLX_PIT_FSIZE, NONUSE_FLX_PIT_DEST_OFF);
    I40E_WRITE_REG(hw, I40E_PRTQF_FLX_PIT(layer_idx * I40E_MAX_FLXPLD_FIED + i),
                   flx_pit);
    min_next_off++;
  }
}
 ...
 */
static void
i40e_set_flex_mask_on_pctype(struct i40e_pf *pf, enum i40e_filter_pctype pctype,
  ...
  for (i = 0; i < I40E_FDIR_MAX_FLEX_LEN; i += sizeof(uint16_t)) {
    ...
    if (mask_tmp != 0x0) {
      flex_mask->word_mask |= I40E_FLEX_WORD_MASK(i / sizeof(uint16_t));
  ...
  /* write mask to hw */
  flxinset = (flex_mask->word_mask << I40E_PRTQF_FD_FLXINSET_INSET_SHIFT) &
             I40E_PRTQF_FD_FLXINSET_INSET_MASK;
  i40e_write_rx_ctl(hw, I40E_PRTQF_FD_FLXINSET(pctype), flxinset);

  for (i = 0; i < nb_bitmask; i++) {
    fd_mask = (flex_mask->bitmask[i].mask << I40E_PRTQF_FD_MSK_MASK_SHIFT) &
              I40E_PRTQF_FD_MSK_MASK_MASK;
    fd_mask |= ((flex_mask->bitmask[i].offset + I40E_FLX_OFFSET_IN_FIELD_VECTOR)
    ...
    for (i = 0; i < conf->nb_flexmasks; i++) {
      if (hw->mac.type == I40E_MAC_X722) {
        ...
        pctype = (enum i40e_filter_pctype)i40e_read_rx_ctl(
            hw, I40E_GLQF_FD_PCTYPES((int)i40e_flowtype_to_pctype(
                    pf->adapter, conf->flex_mask[i].flow_type)));
      } else {
        pctype =
            i40e_flowtype_to_pctype(pf->adapter, conf->flex_mask[i].flow_type);
      }
...
static inline int fill_ip6_head(const struct i40e_fdir_input *fdir_input,
                                unsigned char *raw_pkt, uint8_t next_proto,
                                uint8_t len, uint16_t *ether_type) {
  struct rte_ipv6_hdr *ip6;

  ip6 = (struct rte_ipv6_hdr *)raw_pkt;

  *ether_type = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6);
  ip6->vtc_flow = rte_cpu_to_be_32(
   ...
   */
  rte_memcpy(&ip6->src_addr, &fdir_input->flow.ipv6_flow.dst_ip, IPV6_ADDR_LEN);
  rte_memcpy(&ip6->dst_addr, &fdir_input->flow.ipv6_flow.src_ip, IPV6_ADDR_LEN);
  len += sizeof(struct rte_ipv6_hdr);

  return len;
}

static inline int fill_ip4_head(const struct i40e_fdir_input *fdir_input,
                                unsigned char *raw_pkt, uint8_t next_proto,
                                uint8_t len, uint16_t *ether_type) {
...
i40e_flow_fdir_fill_eth_ip_head(struct i40e_pf *pf,
                                ...
                                unsigned char *raw_pkt, bool vlan) {
  ...
  if (vlan && fdir_input->flow_ext.vlan_tci) {
    ...
    rte_memcpy(raw_pkt + sizeof(uint16_t), &fdir_input->flow_ext.vlan_tci,
               sizeof(uint16_t));
  ...
  if (pctype == I40E_FILTER_PCTYPE_L2_PAYLOAD)
    *ether_type = fdir_input->flow.l2_flow.ether_type;
           ...
           pctype == I40E_FILTER_PCTYPE_FRAG_IPV6 || is_customized_pctype) {
    if (pctype == I40E_FILTER_PCTYPE_NONF_IPV4_TCP ||
        pctype == I40E_FILTER_PCTYPE_NONF_IPV4_UDP ||
        pctype == I40E_FILTER_PCTYPE_NONF_IPV4_SCTP ||
        pctype == I40E_FILTER_PCTYPE_NONF_IPV4_OTHER ||
        pctype == I40E_FILTER_PCTYPE_FRAG_IPV4) {
      len = fill_ip4_head(fdir_input, raw_pkt, next_proto[pctype], len,
                          ether_type);
      ...
      len = fill_ip6_head(fdir_input, raw_pkt, next_proto[pctype], len,
                          ether_type);
      ...
      len = fill_ip4_head(fdir_input, raw_pkt, IPPROTO_L2TP, len, ether_type);
    } else if (cus_pctype->index == I40E_CUSTOMIZED_ESP_IPV4) {
      len = fill_ip4_head(fdir_input, raw_pkt, IPPROTO_ESP, len, ether_type);
    } else if (cus_pctype->index == I40E_CUSTOMIZED_ESP_IPV4_UDP) {
      len = fill_ip4_head(fdir_input, raw_pkt, IPPROTO_UDP, len, ether_type);
    } else if (cus_pctype->index == I40E_CUSTOMIZED_ESP_IPV4_UDP) {
      len = fill_ip4_head(fdir_input, raw_pkt, IPPROTO_UDP, len, ether_type);
    } else if (cus_pctype->index == I40E_CUSTOMIZED_ESP_IPV6)
      len = fill_ip6_head(fdir_input, raw_pkt, IPPROTO_ESP, len, ether_type);
    else if (cus_pctype->index == I40E_CUSTOMIZED_ESP_IPV6_UDP)
      len = fill_ip6_head(fdir_input, raw_pkt, IPPROTO_UDP, len, ether_type);
    else if (cus_pctype->index == I40E_CUSTOMIZED_IPV6_L2TPV3)
      len = fill_ip6_head(fdir_input, raw_pkt, IPPROTO_L2TP, len, ether_type);
  } else {
    PMD_DRV_LOG(ERR, "unknown pctype %u.", fdir_input->pctype);
    return -1;
  }
...
i40e_flow_fdir_construct_pkt(struct i40e_pf *pf,
  ...
  if (pctype == I40E_FILTER_PCTYPE_NONF_IPV4_UDP) {
    ...
    if (cus_pctype->index == I40E_CUSTOMIZED_GTPC ||
      ...
      gtp = (struct rte_flow_item_gtp *)((unsigned char *)udp +
      ...
      if (cus_pctype->index == I40E_CUSTOMIZED_GTPC) {
        udp->dst_port = rte_cpu_to_be_16(I40E_FDIR_GTPC_DST_PORT);
        gtp->v_pt_rsv_flags = I40E_FDIR_GTP_VER_FLAG_0X32;
      } else {
        udp->dst_port = rte_cpu_to_be_16(I40E_FDIR_GTPU_DST_PORT);
        gtp->v_pt_rsv_flags = I40E_FDIR_GTP_VER_FLAG_0X30;
      }

      if (cus_pctype->index == I40E_CUSTOMIZED_GTPU_IPV4) {
        gtp->msg_type = I40E_FDIR_GTP_MSG_TYPE_0XFF;
        gtp_ipv4 = (struct rte_ipv4_hdr *)((unsigned char *)gtp +
                                           sizeof(struct rte_flow_item_gtp));
        gtp_ipv4->version_ihl = I40E_FDIR_IP_DEFAULT_VERSION_IHL;
        gtp_ipv4->next_proto_id = IPPROTO_IP;
        gtp_ipv4->total_length =
            rte_cpu_to_be_16(I40E_FDIR_INNER_IP_DEFAULT_LEN);
        payload = (unsigned char *)gtp_ipv4 + sizeof(struct rte_ipv4_hdr);
      } else if (cus_pctype->index == I40E_CUSTOMIZED_GTPU_IPV6) {
        gtp->msg_type = I40E_FDIR_GTP_MSG_TYPE_0XFF;
        gtp_ipv6 = (struct rte_ipv6_hdr *)((unsigned char *)gtp +
                                           sizeof(struct rte_flow_item_gtp));
        gtp_ipv6->vtc_flow = rte_cpu_to_be_32(I40E_FDIR_IPv6_DEFAULT_VTC_FLOW |
                                              (0 << I40E_FDIR_IPv6_TC_OFFSET));
        gtp_ipv6->proto = IPPROTO_NONE;
        gtp_ipv6->payload_len =
            rte_cpu_to_be_16(I40E_FDIR_INNER_IPV6_DEFAULT_LEN);
        gtp_ipv6->hop_limits = I40E_FDIR_IPv6_DEFAULT_HOP_LIMITS;
        payload = (unsigned char *)gtp_ipv6 + sizeof(struct rte_ipv6_hdr);
      } else
        payload = (unsigned char *)gtp + sizeof(struct rte_flow_item_gtp);
    } else if (cus_pctype->index == I40E_CUSTOMIZED_IPV4_L2TPV3 ||
               cus_pctype->index == I40E_CUSTOMIZED_IPV6_L2TPV3) {
      l2tpv3oip = (struct rte_flow_item_l2tpv3oip *)(raw_pkt + len);

      if (cus_pctype->index == I40E_CUSTOMIZED_IPV4_L2TPV3)
        l2tpv3oip->session_id = fdir_input->flow.ip4_l2tpv3oip_flow.session_id;
      else
        l2tpv3oip->session_id = fdir_input->flow.ip6_l2tpv3oip_flow.session_id;
      payload =
          (unsigned char *)l2tpv3oip + sizeof(struct rte_flow_item_l2tpv3oip);
    } else if (cus_pctype->index == I40E_CUSTOMIZED_ESP_IPV4 ||
      ...
      if (cus_pctype->index == I40E_CUSTOMIZED_ESP_IPV4) {
        ...
        esp->hdr.spi = fdir_input->flow.esp_ipv4_flow.spi;
        payload = (unsigned char *)esp + sizeof(struct rte_esp_hdr);
        ...
        esp = (struct rte_flow_item_esp *)((unsigned char *)esp_ipv4 +
                                           sizeof(struct rte_udp_hdr));
        esp->hdr.spi = fdir_input->flow.esp_ipv4_udp_flow.spi;
        payload = (unsigned char *)esp + sizeof(struct rte_esp_hdr);
        len += sizeof(struct rte_udp_hdr) + sizeof(struct rte_esp_hdr);
      } else if (cus_pctype->index == I40E_CUSTOMIZED_ESP_IPV6) {
        esp_ipv6 = (struct rte_ipv6_hdr *)(raw_pkt + len);
        esp = (struct rte_flow_item_esp *)esp_ipv6;
        esp->hdr.spi = fdir_input->flow.esp_ipv6_flow.spi;
        payload = (unsigned char *)esp + sizeof(struct rte_esp_hdr);
        len += sizeof(struct rte_esp_hdr);
      } else if (cus_pctype->index == I40E_CUSTOMIZED_ESP_IPV6_UDP) {
        esp_ipv6 = (struct rte_ipv6_hdr *)(raw_pkt + len);
        ...
        esp = (struct rte_flow_item_esp *)((unsigned char *)esp_ipv6 +
                                           sizeof(struct rte_udp_hdr));
        esp->hdr.spi = fdir_input->flow.esp_ipv6_udp_flow.spi;
        payload = (unsigned char *)esp + sizeof(struct rte_esp_hdr);
        len += sizeof(struct rte_udp_hdr) + sizeof(struct rte_esp_hdr);
      }
    }
  ...
  for (i = 0; i < I40E_MAX_FLXPLD_FIED; i++) {
    ...
    ptr = payload + pf->fdir.flex_set[pit_idx].src_offset * sizeof(uint16_t);
    (void)rte_memcpy(ptr, &fdir_input->flow_ext.flexbytes[dst],
  ...
  if (rx_status & (1 << I40E_RX_DESC_STATUS_DD_SHIFT)) {
    ...
    if (len == I40E_RX_PROG_STATUS_DESC_LENGTH &&
      ...
      if (error == (0x1 << I40E_RX_PROG_STATUS_DESC_FD_TBL_FULL_SHIFT)) {
        ...
        PMD_DRV_LOG(ERR,
                    "invalid programming status"
                    " reported, error = %u.",
                    error);
    } else
      PMD_DRV_LOG(INFO,
                  "unknown programming status"
                  " reported, len = %d, id = %u.",
                  len, id);
...
static int i40e_fdir_filter_convert(const struct i40e_fdir_filter_conf *input,
  ...
  if (input->input.flow_ext.pkt_template) {
    ...
    filter->fdir.input.flow.raw_flow.length = rte_hash_crc(
        input->input.flow.raw_flow.packet, input->input.flow.raw_flow.length,
...
i40e_sw_fdir_filter_lookup(struct i40e_fdir_info *fdir_info,
  ...
  if (input->flow_ext.pkt_template)
    ret = rte_hash_lookup_with_hash(fdir_info->hash_table, (const void *)input,
...
static int i40e_sw_fdir_filter_insert(struct i40e_pf *pf,
  ...
  if (filter->fdir.input.flow_ext.pkt_template)
    ret = rte_hash_add_key_with_hash(fdir_info->hash_table, &filter->fdir.input,
                                     filter->fdir.input.flow.raw_flow.length);
  else
    ret = rte_hash_add_key(fdir_info->hash_table, &filter->fdir.input);
  if (ret < 0) {
    PMD_DRV_LOG(ERR, "Failed to insert fdir filter to hash table %d!", ret);
    return ret;
  }
...
int i40e_sw_fdir_filter_del(struct i40e_pf *pf, struct i40e_fdir_input *input) {
  ...
  if (input->flow_ext.pkt_template)
    ret = rte_hash_del_key_with_hash(fdir_info->hash_table, input,
                                     input->flow.raw_flow.length);
  else
...
struct rte_flow *i40e_fdir_entry_pool_get(struct i40e_fdir_info *fdir_info) {
  ...
  ret = rte_bitmap_scan(fdir_info->fdir_flow_pool.bitmap, &pos, &slab);
...
static int i40e_flow_store_flex_pit(struct i40e_pf *pf,
                                    struct i40e_fdir_flex_pit *flex_pit,
...
static void i40e_flow_set_fdir_flex_pit(struct i40e_pf *pf,
                                        enum i40e_flxpld_layer_idx layer_idx,
                                        uint8_t raw_id) {
  struct i40e_hw *hw = I40E_PF_TO_HW(pf);
  uint32_t flx_pit, flx_ort;
...
static int i40e_flow_store_flex_mask(struct i40e_pf *pf,
                                     enum i40e_filter_pctype pctype,
                                     uint8_t *mask) {
  struct i40e_fdir_flex_mask flex_mask;
...
static void i40e_flow_set_fdir_flex_msk(struct i40e_pf *pf,
                                        enum i40e_filter_pctype pctype) {
  struct i40e_hw *hw = I40E_PF_TO_HW(pf);
  struct i40e_fdir_flex_mask *flex_mask;
  uint32_t flxinset, fd_mask;
...
static int i40e_flow_set_fdir_inset(struct i40e_pf *pf,
                                    enum i40e_filter_pctype pctype,
                                    uint64_t input_set) {
  ...
  if (pf->support_multi_driver) {
    for (i = 0; i < num; i++)
      if (i40e_read_rx_ctl(hw, I40E_GLQF_FD_MSK(i, pctype)) != mask_reg[i]) {
    ...
    for (i = num; i < I40E_INSET_MASK_NUM_REG; i++)
      if (i40e_read_rx_ctl(hw, I40E_GLQF_FD_MSK(i, pctype)) != 0) {
        PMD_DRV_LOG(ERR, "Input set setting is not"
                         " supported with"
                         " `support-multi-driver`"
                         " enabled!");
    ...
    for (i = 0; i < num; i++)
      i40e_check_write_reg(hw, I40E_GLQF_FD_MSK(i, pctype), mask_reg[i]);
...
int i40e_flow_add_del_fdir_filter(struct rte_eth_dev *dev,
  ...
  if (add) {
    ...
    if (filter->input.flow_ext.is_flex_flow) {
      for (i = 0; i < filter->input.flow_ext.raw_id; i++) {
        ...
        ret = i40e_flow_store_flex_pit(pf, &flex_pit, layer_idx, i);
        if (ret < 0) {
      ...
      for (i = 0; i < I40E_FDIR_MAX_FLEX_LEN; i++)
        flex_mask[i] = filter->input.flow_ext.flex_mask[i];
    ...
    if (!node) {
      PMD_DRV_LOG(ERR, "There's no corresponding flow director filter!");
      return -EINVAL;
    }

    ret = i40e_sw_fdir_filter_del(pf, &node->fdir.input);
    if (ret < 0) {
      PMD_DRV_LOG(ERR, "Error deleting fdir rule from hash table!");
      return -EINVAL;
    }
  ...
  if (ret < 0) {
    PMD_DRV_LOG(ERR, "fdir programming fails for PCTYPE(%u).", pctype);
    goto error_op;
  }
...
static int i40e_flow_fdir_filter_programming(
  ...
  fdirdp =
      (volatile struct i40e_filter_program_desc *)(&txq->tx_ring[txq->tx_tail]);

  fdirdp->qindex_flex_ptype_vsi = rte_cpu_to_le_32(
      (fdir_action->rx_queue << I40E_TXD_FLTR_QW0_QINDEX_SHIFT) &
      I40E_TXD_FLTR_QW0_QINDEX_MASK);

  fdirdp->qindex_flex_ptype_vsi |= rte_cpu_to_le_32(
      (fdir_action->flex_off << I40E_TXD_FLTR_QW0_FLEXOFF_SHIFT) &
      I40E_TXD_FLTR_QW0_FLEXOFF_MASK);

  fdirdp->qindex_flex_ptype_vsi |=
      rte_cpu_to_le_32((pctype << I40E_TXD_FLTR_QW0_PCTYPE_SHIFT) &
  ...
  fdirdp->dtype_cmd_cntindex = rte_cpu_to_le_32(I40E_TX_DESC_DTYPE_FILTER_PROG);
  ...
  if (fdir_action->behavior == I40E_FDIR_REJECT)
    ...
    PMD_DRV_LOG(ERR,
                "Failed to program FDIR filter: unsupported fdir behavior.");
    return -EINVAL;
  }

  fdirdp->dtype_cmd_cntindex |= rte_cpu_to_le_32(
      (dest << I40E_TXD_FLTR_QW1_DEST_SHIFT) & I40E_TXD_FLTR_QW1_DEST_MASK);

  fdirdp->dtype_cmd_cntindex |= rte_cpu_to_le_32(
      (fdir_action->report_status << I40E_TXD_FLTR_QW1_FD_STATUS_SHIFT) &
      I40E_TXD_FLTR_QW1_FD_STATUS_MASK);

  fdirdp->dtype_cmd_cntindex |=
      rte_cpu_to_le_32(I40E_TXD_FLTR_QW1_CNT_ENA_MASK);
  fdirdp->dtype_cmd_cntindex |=
      rte_cpu_to_le_32(((uint32_t)pf->fdir.match_counter_index
  ...
  td_cmd = I40E_TX_DESC_CMD_EOP | I40E_TX_DESC_CMD_RS | I40E_TX_DESC_CMD_DUMMY;

  txdp->cmd_type_offset_bsz = i40e_build_ctob(td_cmd, 0, I40E_FDIR_PKT_LEN, 0);
  ...
  if (wait_status) {
    ...
    if (i40e_check_fdir_programming_status(rxq) < 0) {
      PMD_DRV_LOG(
          ERR, "Failed to program FDIR filter: programming status reported.");
      return -ETIMEDOUT;
    }
  }
...
i40e_fdir_info_get_flex_set(struct i40e_pf *pf,
  ...
  for (layer_idx = I40E_FLXPLD_L2_IDX; layer_idx <= I40E_FLXPLD_L4_IDX;
       layer_idx++) {
    ...
    for (i = 0; i < I40E_MAX_FLXPLD_FIED; i++) {
      flex_pit = &pf->fdir.flex_set[layer_idx * I40E_MAX_FLXPLD_FIED + i];
      if (flex_pit->size == 0)
        continue;
...
i40e_fdir_info_get_flex_mask(struct i40e_pf *pf,
  ...
  for (i = I40E_FILTER_PCTYPE_NONF_IPV4_UDP; i <= I40E_FILTER_PCTYPE_L2_PAYLOAD;
       i++) {
    mask = &pf->fdir.flex_mask[i];
    flow_type =
        i40e_pctype_to_flowtype(pf->adapter, (enum i40e_filter_pctype)i);
    if (flow_type == RTE_ETH_FLOW_UNKNOWN)
      continue;
...
void i40e_fdir_info_get(struct rte_eth_dev *dev,
  ...
  fdir->guarant_spc = (uint32_t)hw->func_caps.fd_filters_guaranteed;
  fdir->best_spc = (uint32_t)hw->func_caps.fd_filters_best_effort;
  ...
  i40e_fdir_info_get_flex_set(pf, fdir->flex_conf.flex_set, &num_flex_set);
  i40e_fdir_info_get_flex_mask(pf, fdir->flex_conf.flex_mask, &num_flex_mask);
 ...
 * with the flow director statistics.
 */
void i40e_fdir_stats_get(struct rte_eth_dev *dev,
  ...
  stat->guarant_cnt = (uint32_t)((fdstat & I40E_PFQF_FDSTAT_GUARANT_CNT_MASK) >>
                                 I40E_PFQF_FDSTAT_GUARANT_CNT_SHIFT);
  stat->best_cnt = (uint32_t)((fdstat & I40E_PFQF_FDSTAT_BEST_CNT_MASK) >>
                              I40E_PFQF_FDSTAT_BEST_CNT_SHIFT);
}
...
void i40e_fdir_filter_restore(struct i40e_pf *pf) {
  ...
  fdstat = I40E_READ_REG(hw, I40E_PFQF_FDSTAT);
  guarant_cnt = (uint32_t)((fdstat & I40E_PFQF_FDSTAT_GUARANT_CNT_MASK) >>
                           I40E_PFQF_FDSTAT_GUARANT_CNT_SHIFT);
  best_cnt = (uint32_t)((fdstat & I40E_PFQF_FDSTAT_BEST_CNT_MASK) >>
                        I40E_PFQF_FDSTAT_BEST_CNT_SHIFT);

  PMD_DRV_LOG(INFO, "FDIR: Guarant count: %d,  Best count: %d", guarant_cnt,
              best_cnt);
}
