...
static int i40e_pf_host_switch_queues(struct i40e_pf_vf *vf,
 ...
 **/
static int i40e_pf_vf_queues_mapping(struct i40e_pf_vf *vf) {
  int i;
  ...
  I40E_WRITE_REG(hw, I40E_VPLAN_MAPENA(vf_id), I40E_VPLAN_MAPENA_TXRX_ENA_MASK);
  ...
  if (vf->vsi) {
    ...
    for (i = 0; i < vf_msix_num; i++) {
      if (!i)
        ...
        val = I40E_VFINT_DYN_CTLN(((vf_msix_num - 1) * (vf_id)) + (i - 1));
...
#define I40E_VF_PEND_MASK 0x20
  ...
  I40E_WRITE_REG(hw, I40E_PF_PCI_CIAA,
                 I40E_VF_PCI_ADDR |
  ...
  if (pf->floating_veb && pf->floating_veb_list[vf_id]) {
    vf->vsi = i40e_vsi_setup(vf->pf, I40E_VSI_SRIOV, NULL, vf->vf_idx);
  } else {
    vf->vsi =
        i40e_vsi_setup(vf->pf, I40E_VSI_SRIOV, vf->pf->main_vsi, vf->vf_idx);
  }
...
int i40e_pf_host_send_msg_to_vf(struct i40e_pf_vf *vf, uint32_t opcode,
  ...
  ret =
      i40e_aq_send_msg_to_vf(hw, abs_vf_id, opcode, retval, msg, msglen, NULL);
  if (ret) {
...
static void i40e_pf_host_process_cmd_version(struct i40e_pf_vf *vf,
                                             uint8_t *msg, bool b_op) {
  ...
  if (b_op)
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_VERSION, I40E_SUCCESS,
                                (uint8_t *)&info, sizeof(info));
  else
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_VERSION, I40E_NOT_SUPPORTED,
                                (uint8_t *)&info, sizeof(info));
}

static int i40e_pf_host_process_cmd_reset_vf(struct i40e_pf_vf *vf) {
...
static int i40e_pf_host_process_cmd_get_vf_resource(struct i40e_pf_vf *vf,
                                                    uint8_t *msg, bool b_op) {
  ...
  len = sizeof(struct virtchnl_vf_resource) +
        I40E_DEFAULT_VF_VSI_NUM * sizeof(struct virtchnl_vsi_resource);
  ...
  if (VF_IS_V10(&vf->version)) /* doesn't support offload negotiate */
    vf->request_caps = VIRTCHNL_VF_OFFLOAD_L2 | VIRTCHNL_VF_OFFLOAD_VLAN;
  else
  ...
  if (vf->request_caps & VIRTCHNL_VF_OFFLOAD_RSS_PF) {
    I40E_WRITE_REG(hw, I40E_VFQF_HENA1(0, vf->vf_idx), (uint32_t)default_hena);
    I40E_WRITE_REG(hw, I40E_VFQF_HENA1(1, vf->vf_idx),
                   (uint32_t)(default_hena >> 32));
    I40E_WRITE_FLUSH(hw);
  }

  vf_res->vf_cap_flags = vf->request_caps & I40E_VIRTCHNL_OFFLOAD_CAPS;
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_GET_VF_RESOURCES, ret,
                              (uint8_t *)vf_res, len);
  rte_free(vf_res);
...
static int i40e_pf_host_hmc_config_rxq(struct i40e_hw *hw,
...
static inline uint8_t i40e_vsi_get_tc_of_queue(struct i40e_vsi *vsi,
                                               uint16_t queue_id) {
  ...
  for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
    if (vsi->enabled_tc & (1 << i)) {
      qp_idx = rte_le_to_cpu_16(
          (info->tc_mapping[i] & I40E_AQ_VSI_TC_QUE_OFFSET_MASK) >>
          I40E_AQ_VSI_TC_QUE_OFFSET_SHIFT);
      bsf = rte_le_to_cpu_16(
          (info->tc_mapping[i] & I40E_AQ_VSI_TC_QUE_NUMBER_MASK) >>
...
static int i40e_pf_host_hmc_config_txq(struct i40e_hw *hw,
                                       struct i40e_pf_vf *vf,
   ...
   */
  qtx_ctl =
      (I40E_QTX_CTL_VF_QUEUE << I40E_QTX_CTL_PFVF_Q_SHIFT) |
...
static int i40e_pf_host_process_cmd_config_vsi_queues(struct i40e_pf_vf *vf,
                                                      uint8_t *msg,
                                                      uint16_t msglen,
                                                      bool b_op) {
  struct i40e_hw *hw = I40E_PF_TO_HW(vf->pf);
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_VSI_QUEUES, ret, NULL, 0);

  return ret;
}

static void i40e_pf_config_irq_link_list(struct i40e_pf_vf *vf,
...
#define BITS_PER_CHAR 8
  ...
  if (vvm->rxq_map == 0 && vvm->txq_map == 0) {
    I40E_WRITE_REG(hw, reg_idx, I40E_VPINT_LNKLST0_FIRSTQ_INDX_MASK);
    goto cfg_irq_done;
  }
  ...
  do {
    if (tempmap & 0x1) {
      ...
      if (b_first_q) {
        /* This is header */
        b_first_q = false;
        reg = ((qtype << I40E_VPINT_LNKLSTN_FIRSTQ_TYPE_SHIFT) | qid);
      } else {
        /* element in the link list */
        reg = (vector_id) | (qtype << I40E_QINT_RQCTL_NEXTQ_TYPE_SHIFT) |
  ...
  reg = (vector_id) | (0 << I40E_QINT_RQCTL_NEXTQ_TYPE_SHIFT) |
...
static int i40e_pf_host_process_cmd_config_irq_map(struct i40e_pf_vf *vf,
                                                   uint8_t *msg,
                                                   uint16_t msglen, bool b_op) {
  int ret = I40E_SUCCESS;
  struct i40e_pf *pf = vf->pf;
  struct i40e_hw *hw = I40E_PF_TO_HW(vf->pf);
  struct virtchnl_irq_map_info *irqmap = (struct virtchnl_irq_map_info *)msg;
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_IRQ_MAP, ret, NULL, 0);
...
static int i40e_pf_host_process_cmd_enable_queues(struct i40e_pf_vf *vf,
                                                  uint8_t *msg,
                                                  uint16_t msglen) {
  int ret = I40E_SUCCESS;
  struct virtchnl_queue_select *q_sel = (struct virtchnl_queue_select *)msg;
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ENABLE_QUEUES, ret, NULL, 0);

  return ret;
}

static int i40e_pf_host_process_cmd_disable_queues(struct i40e_pf_vf *vf,
                                                   uint8_t *msg,
                                                   uint16_t msglen, bool b_op) {
  int ret = I40E_SUCCESS;
  struct virtchnl_queue_select *q_sel = (struct virtchnl_queue_select *)msg;
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DISABLE_QUEUES, ret, NULL, 0);

  return ret;
}

static int i40e_pf_host_process_cmd_add_ether_address(struct i40e_pf_vf *vf,
                                                      uint8_t *msg,
                                                      uint16_t msglen,
                                                      bool b_op) {
  int ret = I40E_SUCCESS;
  ...
  for (i = 0; i < addr_list->num_elements; i++) {
    ...
    if (rte_is_zero_ether_addr(mac) || i40e_vsi_add_mac(vf->vsi, &filter)) {
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ADD_ETH_ADDR, ret, NULL, 0);

  return ret;
}

static int i40e_pf_host_process_cmd_del_ether_address(struct i40e_pf_vf *vf,
                                                      uint8_t *msg,
                                                      uint16_t msglen,
                                                      bool b_op) {
  int ret = I40E_SUCCESS;
  ...
  for (i = 0; i < addr_list->num_elements; i++) {
    mac = (struct rte_ether_addr *)(addr_list->list[i].addr);
    if (rte_is_zero_ether_addr(mac) || i40e_vsi_delete_mac(vf->vsi, mac)) {
      ret = I40E_ERR_INVALID_MAC_ADDR;
      goto send_msg;
    }
  }
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DEL_ETH_ADDR, ret, NULL, 0);

  return ret;
}

static int i40e_pf_host_process_cmd_add_vlan(struct i40e_pf_vf *vf,
  ...
  if (!b_op) {
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ADD_VLAN, I40E_NOT_SUPPORTED,
                                NULL, 0);
    return ret;
  }
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ADD_VLAN, ret, NULL, 0);

  return ret;
}

static int i40e_pf_host_process_cmd_del_vlan(struct i40e_pf_vf *vf,
                                             uint8_t *msg, uint16_t msglen,
                                             bool b_op) {
  int ret = I40E_SUCCESS;
  struct virtchnl_vlan_filter_list *vlan_filter_list =
  ...
  if (!b_op) {
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DEL_VLAN, I40E_NOT_SUPPORTED,
                                NULL, 0);
    return ret;
  }
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DEL_VLAN, ret, NULL, 0);

  return ret;
}

static int i40e_pf_host_process_cmd_config_promisc_mode(struct i40e_pf_vf *vf,
                                                        uint8_t *msg,
                                                        uint16_t msglen,
                                                        bool b_op) {
  int ret = I40E_SUCCESS;
  struct virtchnl_promisc_info *promisc = (struct virtchnl_promisc_info *)msg;
  struct i40e_hw *hw = I40E_PF_TO_HW(vf->pf);
  bool unicast = FALSE, multicast = FALSE;

  if (!b_op) {
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_PROMISCUOUS_MODE,
                                I40E_NOT_SUPPORTED, NULL, 0);
    return ret;
  }
  ...
  if (promisc->flags & FLAG_VF_UNICAST_PROMISC)
    unicast = TRUE;
  ret = i40e_aq_set_vsi_unicast_promiscuous(hw, vf->vsi->seid, unicast, NULL,
                                            true);
  if (ret != I40E_SUCCESS)
    goto send_msg;
  ...
  if (promisc->flags & FLAG_VF_MULTICAST_PROMISC)
    multicast = TRUE;
  ret =
      i40e_aq_set_vsi_multicast_promiscuous(hw, vf->vsi->seid, multicast, NULL);

send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_PROMISCUOUS_MODE, ret,
                              NULL, 0);

  return ret;
}

static int i40e_pf_host_process_cmd_get_stats(struct i40e_pf_vf *vf,
                                              bool b_op) {
  i40e_update_vsi_stats(vf->vsi);

  if (b_op)
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_GET_STATS, I40E_SUCCESS,
                                (uint8_t *)&vf->vsi->eth_stats,
                                sizeof(vf->vsi->eth_stats));
  else
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_GET_STATS, I40E_NOT_SUPPORTED,
                                (uint8_t *)&vf->vsi->eth_stats,
                                sizeof(vf->vsi->eth_stats));
...
static int i40e_pf_host_process_cmd_enable_vlan_strip(struct i40e_pf_vf *vf,
                                                      bool b_op) {
  int ret = I40E_SUCCESS;
  ...
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ENABLE_VLAN_STRIPPING, ret, NULL,
                              0);

  return ret;
}

static int i40e_pf_host_process_cmd_disable_vlan_strip(struct i40e_pf_vf *vf,
                                                       bool b_op) {
  int ret = I40E_SUCCESS;
  ...
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DISABLE_VLAN_STRIPPING, ret, NULL,
                              0);

  return ret;
}

static int i40e_pf_host_process_cmd_set_rss_lut(struct i40e_pf_vf *vf,
                                                uint8_t *msg, uint16_t msglen,
                                                bool b_op) {
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_RSS_LUT, ret, NULL, 0);

  return ret;
}

static int i40e_pf_host_process_cmd_set_rss_key(struct i40e_pf_vf *vf,
                                                uint8_t *msg, uint16_t msglen,
                                                bool b_op) {
...
send_msg:
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_RSS_KEY, ret, NULL, 0);
...
void i40e_notify_vf_link_status(struct rte_eth_dev *dev,
  ...
  event.event_data.link_event.link_status = dev->data->dev_link.link_status;
  ...
  if (tval & I40E_VF_ATQLEN_ATQLEN_MASK ||
      ...
      rval & I40E_VF_ARQLEN_ARQLEN_MASK || rval & I40E_VF_ARQLEN_ARQENABLE_MASK)
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_EVENT, I40E_SUCCESS,
 ...
 **/
static void i40e_vc_notify_vf_reset(struct i40e_pf_vf *vf) {
  struct i40e_hw *hw = I40E_PF_TO_HW(vf->pf);
...
static int i40e_pf_host_process_cmd_request_queues(struct i40e_pf_vf *vf,
                                                   uint8_t *msg) {
  ...
  if (req_pairs == 0) {
    ...
    PMD_DRV_LOG(ERR, "VF %d tried to request more than %d queues.\n",
                vf->vf_idx, I40E_MAX_QP_NUM_PER_VF);
    vfres->num_queue_pairs = I40E_MAX_QP_NUM_PER_VF;
  } else if (req_pairs > cur_pairs + pf->qp_pool.num_free) {
    PMD_DRV_LOG(ERR,
                "VF %d requested %d queues (rounded to %d) "
                "but only %d available\n",
                vf->vf_idx, vfres->num_queue_pairs, req_pairs,
                cur_pairs + pf->qp_pool.num_free);
    vfres->num_queue_pairs = i40e_align_floor(pf->qp_pool.num_free + cur_pairs);
  } else {
...
static void i40e_pf_host_process_cmd_get_rss_hena(struct i40e_pf_vf *vf) {
  ...
  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_GET_RSS_HENA_CAPS, I40E_SUCCESS,
                              (uint8_t *)&vrh, sizeof(vrh));
}

static void i40e_pf_host_process_cmd_set_rss_hena(struct i40e_pf_vf *vf,
                                                  uint8_t *msg) {
  struct virtchnl_rss_hena *vrh = (struct virtchnl_rss_hena *)msg;
  struct i40e_hw *hw = &vf->pf->adapter->hw;

  i40e_write_rx_ctl(hw, I40E_VFQF_HENA1(0, vf->vf_idx), (uint32_t)vrh->hena);
  i40e_write_rx_ctl(hw, I40E_VFQF_HENA1(1, vf->vf_idx),
                    (uint32_t)(vrh->hena >> 32));

  i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_SET_RSS_HENA, I40E_SUCCESS, NULL,
                              0);
}

void i40e_pf_host_handle_vf_msg(struct rte_eth_dev *dev, uint16_t abs_vf_id,
                                uint32_t opcode, __rte_unused uint32_t retval,
                                uint8_t *msg, uint16_t msglen) {
  ...
  if (!vf->vsi) {
    ...
    i40e_pf_host_send_msg_to_vf(vf, opcode, I40E_ERR_NO_AVAILABLE_VSI, NULL, 0);
    goto check;
  }
  ...
  if (ret) {
    PMD_DRV_LOG(ERR, "Invalid message from VF %u, opcode %u, len %u", vf_id,
                opcode, msglen);
    i40e_pf_host_send_msg_to_vf(vf, opcode, I40E_ERR_PARAM, NULL, 0);
    goto check;
  }
  ...
  if (ret_param.retval != RTE_PMD_I40E_MB_EVENT_PROCEED) {
    PMD_DRV_LOG(WARNING, "VF to PF message(%d) is not permitted!", opcode);
    b_op = FALSE;
  }
  ...
  case VIRTCHNL_OP_CONFIG_VSI_QUEUES:
    ...
    i40e_pf_host_process_cmd_config_vsi_queues(vf, msg, msglen, b_op);
    break;
  ...
  case VIRTCHNL_OP_ADD_ETH_ADDR:
    ...
    i40e_pf_host_process_cmd_add_ether_address(vf, msg, msglen, b_op);
    break;
  case VIRTCHNL_OP_DEL_ETH_ADDR:
    PMD_DRV_LOG(INFO, "OP_DEL_ETHER_ADDRESS received");
    i40e_pf_host_process_cmd_del_ether_address(vf, msg, msglen, b_op);
    break;
  ...
  case VIRTCHNL_OP_CONFIG_PROMISCUOUS_MODE:
    ...
    i40e_pf_host_process_cmd_config_promisc_mode(vf, msg, msglen, b_op);
    break;
...
check:
   ...
   */
  if (first_cycle && cur_cycle < first_cycle + (uint64_t)pf->vf_msg_cfg.period *
                                                   rte_get_timer_hz()) {
    PMD_DRV_LOG(WARNING,
                "VF %u too much messages(%u in %u"
                " seconds),\n\tany new message from which"
                " will be ignored during next %u seconds!",
                vf_id, pf->vf_msg_cfg.max_msg,
                (uint32_t)((cur_cycle - first_cycle + rte_get_timer_hz() - 1) /
                           rte_get_timer_hz()),
                pf->vf_msg_cfg.ignore_second);
    vf->ignore_end_cycle = rte_get_timer_cycles() +
                           pf->vf_msg_cfg.ignore_second * rte_get_timer_hz();
  }
}
...
int i40e_pf_host_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < pf->vf_num; i++) {
    ...
    if (size) {
      ...
      pf->vfs[i].msg_timestamps = rte_zmalloc("i40e_pf_vf", size, 0);
