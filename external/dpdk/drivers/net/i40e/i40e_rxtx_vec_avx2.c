 ...
 * - nb_pkts < RTE_I40E_DESCS_PER_LOOP, just return no packet
 */
static uint16_t i40e_recv_scattered_burst_vec_avx2(void *rx_queue,
  ...
  uint16_t nb_bufs =
      _recv_raw_pkts_vec_avx2(rxq, rx_pkts, nb_pkts, split_flags);
  if (nb_bufs == 0)
    return 0;
...
uint16_t i40e_recv_scattered_pkts_vec_avx2(void *rx_queue,
  ...
  while (nb_pkts > RTE_I40E_VPMD_RX_BURST) {
    uint16_t burst = i40e_recv_scattered_burst_vec_avx2(
  ...
  return retval + i40e_recv_scattered_burst_vec_avx2(rx_queue, rx_pkts + retval,
                                                     nb_pkts);
}
...
static inline uint16_t i40e_xmit_fixed_burst_vec_avx2(void *tx_queue,
...
uint16_t i40e_xmit_pkts_vec_avx2(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  while (nb_pkts) {
    ...
    ret = i40e_xmit_fixed_burst_vec_avx2(tx_queue, &tx_pkts[nb_tx], num);
    nb_tx += ret;
    nb_pkts -= ret;
    if (ret < num)
      break;
