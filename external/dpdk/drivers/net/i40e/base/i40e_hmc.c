...
enum i40e_status_code i40e_add_sd_table_entry(struct i40e_hw *hw,
  ...
  if (!sd_entry->valid) {
    ...
    if (I40E_SD_TYPE_PAGED == type) {
      ret_code =
          i40e_allocate_virt_mem(hw, &sd_entry->u.pd_table.pd_entry_virt_mem,
                                 sizeof(struct i40e_hmc_pd_entry) * 512);
      if (ret_code)
        goto exit;
      sd_entry->u.pd_table.pd_entry =
          (struct i40e_hmc_pd_entry *)sd_entry->u.pd_table.pd_entry_virt_mem.va;
      i40e_memcpy(&sd_entry->u.pd_table.pd_page_addr, &mem,
                  sizeof(struct i40e_dma_mem), I40E_NONDMA_TO_NONDMA);
    } else {
      i40e_memcpy(&sd_entry->u.bp.addr, &mem, sizeof(struct i40e_dma_mem),
...
enum i40e_status_code i40e_add_pd_table_entry(struct i40e_hw *hw,
  ...
  if (I40E_SD_TYPE_PAGED != hmc_info->sd_table.sd_entry[sd_idx].entry_type)
    goto exit;
  ...
  if (!pd_entry->valid) {
    if (rsrc_pg) {
      ...
      ret_code =
          i40e_allocate_dma_mem(hw, page, i40e_mem_bp, I40E_HMC_PAGED_BP_SIZE,
                                I40E_HMC_PD_BP_BUF_ALIGNMENT);
      if (ret_code)
        goto exit;
    ...
    i40e_memcpy(&pd_entry->bp.addr, page, sizeof(struct i40e_dma_mem),
                I40E_NONDMA_TO_NONDMA);
    ...
    i40e_memcpy(pd_addr, &page_desc, sizeof(u64), I40E_NONDMA_TO_DMA);
