...
enum i40e_status_code i40e_diag_reg_test(struct i40e_hw *hw) {
  ...
  for (i = 0; i40e_reg_list[i].offset != 0 && ret_code == I40E_SUCCESS; i++) {
    ...
    if ((i40e_reg_list[i].offset == I40E_PFINT_ITRN(0, 0) ||
      ...
      i40e_reg_list[i].elements = hw->func_caps.num_msix_vectors - 1;

    /* test register access */
    mask = i40e_reg_list[i].mask;
    for (j = 0; j < i40e_reg_list[i].elements && ret_code == I40E_SUCCESS;
         j++) {
      reg = i40e_reg_list[i].offset + (j * i40e_reg_list[i].stride);
...
enum i40e_status_code i40e_diag_eeprom_test(struct i40e_hw *hw) {
  ...
  if ((ret_code == I40E_SUCCESS) && ((reg_val & I40E_SR_CONTROL_WORD_1_MASK) ==
