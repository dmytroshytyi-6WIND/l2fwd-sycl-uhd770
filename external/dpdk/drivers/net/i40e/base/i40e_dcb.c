...
static void i40e_parse_ieee_etscfg_tlv(struct i40e_lldp_org_tlv *tlv,
  ...
  etscfg->cbs =
      (u8)((buf[offset] & I40E_IEEE_ETS_CBS_MASK) >> I40E_IEEE_ETS_CBS_SHIFT);
...
static void i40e_parse_ieee_pfccfg_tlv(struct i40e_lldp_org_tlv *tlv,
  ...
  dcbcfg->pfc.mbc =
      (u8)((buf[0] & I40E_IEEE_PFC_MBC_MASK) >> I40E_IEEE_PFC_MBC_SHIFT);
  dcbcfg->pfc.pfccap =
      (u8)((buf[0] & I40E_IEEE_PFC_CAP_MASK) >> I40E_IEEE_PFC_CAP_SHIFT);
...
static void i40e_parse_ieee_app_tlv(struct i40e_lldp_org_tlv *tlv,
  ...
  length =
      (u16)((typelength & I40E_LLDP_TLV_LEN_MASK) >> I40E_LLDP_TLV_LEN_SHIFT);
  buf = tlv->tlvinfo;
   ...
   */
  while (offset < length) {
    dcbcfg->app[i].priority = (u8)((buf[offset] & I40E_IEEE_APP_PRIO_MASK) >>
                                   I40E_IEEE_APP_PRIO_SHIFT);
    dcbcfg->app[i].selector =
        (u8)((buf[offset] & I40E_IEEE_APP_SEL_MASK) >> I40E_IEEE_APP_SEL_SHIFT);
    dcbcfg->app[i].protocolid = (buf[offset + 1] << 0x8) | buf[offset + 2];
    /* Move to next app */
    offset += 3;
    i++;
...
static void i40e_parse_cee_tlv(struct i40e_lldp_org_tlv *tlv,
  ...
  while (feat_tlv_count < I40E_CEE_MAX_FEAT_TYPE) {
    ...
    sub_tlv =
        (struct i40e_cee_feat_tlv *)((char *)sub_tlv +
...
static void i40e_parse_org_tlv(struct i40e_lldp_org_tlv *tlv,
  ...
  ouisubtype = I40E_NTOHL(tlv->ouisubtype);
  oui = (u32)((ouisubtype & I40E_LLDP_TLV_OUI_MASK) >> I40E_LLDP_TLV_OUI_SHIFT);
  switch (oui) {
...
enum i40e_status_code i40e_lldp_to_dcb_config(u8 *lldpmib,
  ...
  while (1) {
    ...
    /* Move to next TLV */
    tlv = (struct i40e_lldp_org_tlv *)((char *)tlv + sizeof(tlv->typelength) +
                                       length);
  }
...
enum i40e_status_code i40e_aq_get_dcb_config(struct i40e_hw *hw, u8 mib_type,
  ...
  lldpmib = (u8 *)mem.va;
  ret = i40e_aq_get_lldp_mib(hw, bridgetype, mib_type, (void *)lldpmib,
 ...
 **/
static void
i40e_cee_to_dcb_v1_config(struct i40e_aqc_get_cee_dcb_cfg_v1_resp *cee_cfg,
                          struct i40e_dcbx_config *dcbcfg) {
   ...
   */
  for (i = 0; i < 4; i++) {
    tc = (u8)((cee_cfg->oper_prio_tc[i] & I40E_CEE_PGID_PRIO_0_MASK) >>
              I40E_CEE_PGID_PRIO_0_SHIFT);
    dcbcfg->etscfg.prioritytable[i * 2] = tc;
    tc = (u8)((cee_cfg->oper_prio_tc[i] & I40E_CEE_PGID_PRIO_1_MASK) >>
              I40E_CEE_PGID_PRIO_1_SHIFT);
  ...
  for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
    if (dcbcfg->etscfg.prioritytable[i] == I40E_CEE_PGID_STRICT) {
      ...
      dcbcfg->etscfg.prioritytable[i] = cee_cfg->oper_num_tc - 1;
      dcbcfg->etscfg.tsatable[i] = I40E_IEEE_TSA_STRICT;
    } else {
      dcbcfg->etscfg.tsatable[i] = I40E_IEEE_TSA_ETS;
    }
  ...
  if (!err) {
    ...
    dcbcfg->app[0].priority =
        (app_prio & I40E_AQC_CEE_APP_FCOE_MASK) >> I40E_AQC_CEE_APP_FCOE_SHIFT;
    dcbcfg->app[0].selector = I40E_APP_SEL_ETHTYPE;
    dcbcfg->app[0].protocolid = I40E_APP_PROTOID_FCOE;

    /* iSCSI APP */
    dcbcfg->app[1].priority = (app_prio & I40E_AQC_CEE_APP_ISCSI_MASK) >>
    ...
    dcbcfg->app[2].priority =
        (app_prio & I40E_AQC_CEE_APP_FIP_MASK) >> I40E_AQC_CEE_APP_FIP_SHIFT;
 ...
 **/
static void
i40e_cee_to_dcb_config(struct i40e_aqc_get_cee_dcb_cfg_resp *cee_cfg,
                       struct i40e_dcbx_config *dcbcfg) {
  ...
  if (!err && sync && oper) {
    /* iSCSI APP */
    dcbcfg->app[i].priority = (app_prio & I40E_AQC_CEE_APP_ISCSI_MASK) >>
                              I40E_AQC_CEE_APP_ISCSI_SHIFT;
    dcbcfg->app[i].selector = I40E_APP_SEL_TCPIP;
...
enum i40e_status_code i40e_get_dcb_config(struct i40e_hw *hw) {
  ...
  if ((hw->mac.type == I40E_MAC_XL710) &&
      ((hw->aq.fw_maj_ver == 4) && (hw->aq.fw_min_ver == 33))) {
    ret = i40e_aq_get_cee_dcb_config(hw, &cee_v1_cfg, sizeof(cee_v1_cfg), NULL);
    if (ret == I40E_SUCCESS) {
      /* CEE mode */
      hw->local_dcbx_config.dcbx_mode = I40E_DCBX_MODE_CEE;
      hw->local_dcbx_config.tlv_status = LE16_TO_CPU(cee_v1_cfg.tlv_status);
      i40e_cee_to_dcb_v1_config(&cee_v1_cfg, &hw->local_dcbx_config);
    }
  } else {
    ret = i40e_aq_get_cee_dcb_config(hw, &cee_cfg, sizeof(cee_cfg), NULL);
    if (ret == I40E_SUCCESS) {
      /* CEE mode */
      hw->local_dcbx_config.dcbx_mode = I40E_DCBX_MODE_CEE;
      hw->local_dcbx_config.tlv_status = LE32_TO_CPU(cee_cfg.tlv_status);
...
enum i40e_status_code i40e_init_dcb(struct i40e_hw *hw,
  ...
  if (hw->flags & I40E_HW_FLAG_FW_LLDP_PERSISTENT) {
    ...
    ret = i40e_read_nvm_module_data(hw, I40E_SR_EMP_SR_SETTINGS_PTR, offset,
...
i40e_get_fw_lldp_status(struct i40e_hw *hw,
  ...
  lldpmib = (u8 *)mem.va;
  ret = i40e_aq_get_lldp_mib(hw, 0, 0, (void *)lldpmib, I40E_LLDPDU_SIZE, NULL,
                             NULL, NULL);
...
static void i40e_add_ieee_ets_tlv(struct i40e_lldp_org_tlv *tlv,
  ...
  for (i = 0; i < 4; i++) {
    ...
    buf[offset] = (priority0 << I40E_IEEE_ETS_PRIO_1_SHIFT) | priority1;
    offset++;
  }
...
static void i40e_add_ieee_app_pri_tlv(struct i40e_lldp_org_tlv *tlv,
  ...
  typelength =
      (u16)((I40E_TLV_TYPE_ORG << I40E_LLDP_TLV_TYPE_SHIFT) | (length & 0x1FF));
  tlv->typelength = I40E_HTONS(typelength);
}
...
static void i40e_add_dcb_tlv(struct i40e_lldp_org_tlv *tlv,
                             struct i40e_dcbx_config *dcbcfg, u16 tlvid) {
  switch (tlvid) {
...
enum i40e_status_code i40e_dcb_config_to_lldp(u8 *lldpmib, u16 *miblen,
  ...
  while (1) {
    ...
    if (length)
      offset += length + 2;
    /* END TLV or beyond LLDPDU size */
    if ((tlvid >= I40E_TLV_ID_END_OF_LLDPPDU) || (offset > I40E_LLDPDU_SIZE))
      break;
    /* Move to next TLV */
    if (length)
      tlv = (struct i40e_lldp_org_tlv *)((char *)tlv + sizeof(tlv->typelength) +
...
_i40e_read_lldp_cfg(struct i40e_hw *hw, struct i40e_lldp_variables *lldp_cfg,
  ...
  ret = i40e_aq_read_nvm(hw, 0x0, module * 2, sizeof(raw_mem), &raw_mem, true,
                         NULL);
  i40e_release_nvm(hw);
  if (ret != I40E_SUCCESS)
    return ret;
  ...
  ret = i40e_aq_read_nvm(hw, module, offset, sizeof(raw_mem), &raw_mem, true,
                         NULL);
  i40e_release_nvm(hw);
  if (ret != I40E_SUCCESS)
    return ret;
  ...
  ret = i40e_aq_read_nvm(hw, 0, address + offset,
                         sizeof(struct i40e_lldp_variables), lldp_cfg, true,
                         NULL);
  i40e_release_nvm(hw);
...
enum i40e_status_code i40e_read_lldp_cfg(struct i40e_hw *hw,
  ...
  ret = i40e_aq_read_nvm(hw, I40E_SR_NVM_CONTROL_WORD, 0, sizeof(mem), &mem,
                         true, NULL);
  i40e_release_nvm(hw);
  if (ret != I40E_SUCCESS)
    return ret;
