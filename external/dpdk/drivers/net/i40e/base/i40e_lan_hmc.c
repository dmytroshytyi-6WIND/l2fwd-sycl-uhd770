 ...
 **/
u64 i40e_calculate_l2fpm_size(u32 txq_num, u32 rxq_num, u32 fcoe_cntx_num,
                              u32 fcoe_filt_num) {
  u64 fpm_size = 0;
...
enum i40e_status_code i40e_init_lan_hmc(struct i40e_hw *hw, u32 txq_num,
  ...
  if (txq_num > obj->max_cnt) {
    ...
    DEBUGOUT3("i40e_init_lan_hmc: Tx context: asks for 0x%x but max allowed is "
              "0x%x, returns error %d\n",
  ...
  if (rxq_num > obj->max_cnt) {
    ret_code = I40E_ERR_INVALID_HMC_OBJ_COUNT;
    DEBUGOUT3("i40e_init_lan_hmc: Rx context: asks for 0x%x but max allowed is "
              "0x%x, returns error %d\n",
  ...
  if (fcoe_cntx_num > obj->max_cnt) {
    ret_code = I40E_ERR_INVALID_HMC_OBJ_COUNT;
    DEBUGOUT3("i40e_init_lan_hmc: FCoE context: asks for 0x%x but max allowed "
              "is 0x%x, returns error %d\n",
  ...
  if (fcoe_filt_num > obj->max_cnt) {
    ret_code = I40E_ERR_INVALID_HMC_OBJ_COUNT;
    DEBUGOUT3("i40e_init_lan_hmc: FCoE filter: asks for 0x%x but max allowed "
              "is 0x%x, returns error %d\n",
  ...
  l2fpm_size =
      i40e_calculate_l2fpm_size(txq_num, rxq_num, fcoe_cntx_num, fcoe_filt_num);
  if (NULL == hw->hmc.sd_table.sd_entry) {
    hw->hmc.sd_table.sd_cnt = (u32)(l2fpm_size + I40E_HMC_DIRECT_BP_SIZE - 1) /
                              I40E_HMC_DIRECT_BP_SIZE;

    /* allocate the sd_entry members in the sd_table */
    ret_code = i40e_allocate_virt_mem(
        hw, &hw->hmc.sd_table.addr,
        (sizeof(struct i40e_hmc_sd_entry) * hw->hmc.sd_table.sd_cnt));
    if (ret_code)
      goto free_hmc_out;
...
i40e_remove_sd_bp(struct i40e_hw *hw, struct i40e_hmc_info *hmc_info, u32 idx) {
  enum i40e_status_code ret_code = I40E_SUCCESS;
...
i40e_create_lan_hmc_object(struct i40e_hw *hw,
  ...
  I40E_FIND_SD_INDEX_LIMIT(info->hmc_info, info->rsrc_type, info->start_idx,
                           info->count, &sd_idx, &sd_lmt);
  ...
  /* find pd index */
  I40E_FIND_PD_INDEX_LIMIT(info->hmc_info, info->rsrc_type, info->start_idx,
                           info->count, &pd_idx, &pd_lmt);
  ...
  for (j = sd_idx; j < sd_lmt; j++) {
    ...
    ret_code = i40e_add_sd_table_entry(hw, info->hmc_info, j, info->entry_type,
                                       sd_size);
    ...
    if (I40E_SD_TYPE_PAGED == sd_entry->entry_type) {
      ...
      pd_lmt1 = min(pd_lmt, ((j + 1) * I40E_HMC_MAX_BP_COUNT));
      for (i = pd_idx1; i < pd_lmt1; i++) {
        /* update the pd table entry */
        ret_code = i40e_add_pd_table_entry(hw, info->hmc_info, i, NULL);
        if (I40E_SUCCESS != ret_code) {
          pd_error = true;
          break;
        }
      ...
      if (pd_error) {
        ...
        while (i && (i > pd_idx1)) {
          i40e_remove_pd_bp(hw, info->hmc_info, (i - 1));
          i--;
        }
      }
    }
    if (!sd_entry->valid) {
      ...
      case I40E_SD_TYPE_PAGED:
        I40E_SET_PF_SD_ENTRY(hw, sd_entry->u.pd_table.pd_page_addr.pa, j,
                             sd_entry->entry_type);
        break;
      case I40E_SD_TYPE_DIRECT:
        I40E_SET_PF_SD_ENTRY(hw, sd_entry->u.bp.addr.pa, j,
                             sd_entry->entry_type);
        break;
      default:
...
exit_sd_error:
  ...
  while (j && (j > sd_idx)) {
    ...
    switch (sd_entry->entry_type) {
    case I40E_SD_TYPE_PAGED:
      pd_idx1 = max(pd_idx, ((j - 1) * I40E_HMC_MAX_BP_COUNT));
...
#define I40E_HMC_STORE(_struct, _ele)                                          \
  offsetof(struct _struct, _ele), FIELD_SIZEOF(struct _struct, _ele)
 ...
 **/
static void i40e_write_byte(u8 *hmc_bits, struct i40e_context_ele *ce_info,
                            u8 *src) {
 ...
 * @src: the struct to be read from
 **/
static void i40e_write_word(u8 *hmc_bits, struct i40e_context_ele *ce_info,
                            u8 *src) {
  u16 src_word, mask;
  u8 *from, *dest;
  u16 shift_width;
 ...
 * @src: the struct to be read from
 **/
static void i40e_write_dword(u8 *hmc_bits, struct i40e_context_ele *ce_info,
                             u8 *src) {
  u32 src_dword, mask;
  u8 *from, *dest;
  u16 shift_width;
 ...
 * @src: the struct to be read from
 **/
static void i40e_write_qword(u8 *hmc_bits, struct i40e_context_ele *ce_info,
                             u8 *src) {
  u64 src_qword, mask;
  u8 *from, *dest;
  u16 shift_width;
 ...
 **/
static void i40e_read_byte(u8 *hmc_bits, struct i40e_context_ele *ce_info,
                           u8 *dest) {
  u8 dest_byte, mask;
  u8 *src, *target;
  u16 shift_width;
 ...
 * @dest: the struct to be filled
 **/
static void i40e_read_word(u8 *hmc_bits, struct i40e_context_ele *ce_info,
                           u8 *dest) {
  u16 dest_word, mask;
  u8 *src, *target;
  u16 shift_width;
 ...
 * @dest: the struct to be filled
 **/
static void i40e_read_dword(u8 *hmc_bits, struct i40e_context_ele *ce_info,
                            u8 *dest) {
  u32 dest_dword, mask;
  u8 *src, *target;
  u16 shift_width;
  ...
  /* put it back in the struct */
  i40e_memcpy(target, &dest_dword, sizeof(dest_dword), I40E_NONDMA_TO_DMA);
}
 ...
 * @dest: the struct to be filled
 **/
static void i40e_read_qword(u8 *hmc_bits, struct i40e_context_ele *ce_info,
                            u8 *dest) {
  u64 dest_qword, mask;
  u8 *src, *target;
  u16 shift_width;
  ...
  /* put it back in the struct */
  i40e_memcpy(target, &dest_qword, sizeof(dest_qword), I40E_NONDMA_TO_DMA);
}
 ...
 **/
static enum i40e_status_code
i40e_get_hmc_context(u8 *context_bytes, struct i40e_context_ele *ce_info,
                     u8 *dest) {
  int f;
 ...
 **/
static enum i40e_status_code
i40e_clear_hmc_context(struct i40e_hw *hw, u8 *context_bytes,
 ...
 **/
static enum i40e_status_code
i40e_set_hmc_context(u8 *context_bytes, struct i40e_context_ele *ce_info,
                     u8 *dest) {
  int f;
...
STATIC
enum i40e_status_code
i40e_hmc_get_object_va(struct i40e_hw *hw, u8 **object_base,
  ...
  /* find sd index and limit */
  I40E_FIND_SD_INDEX_LIMIT(hmc_info, rsrc_type, obj_idx, 1, &sd_idx, &sd_lmt);
  ...
  if (I40E_SD_TYPE_PAGED == sd_entry->entry_type) {
    I40E_FIND_PD_INDEX_LIMIT(hmc_info, rsrc_type, obj_idx, 1, &pd_idx, &pd_lmt);
    rel_pd_idx = pd_idx % I40E_HMC_PD_CNT_IN_SD;
    pd_entry = &sd_entry->u.pd_table.pd_entry[rel_pd_idx];
    obj_offset_in_pd = (u32)(obj_offset_in_fpm % I40E_HMC_PAGED_BP_SIZE);
    *object_base = (u8 *)pd_entry->bp.addr.va + obj_offset_in_pd;
  } else {
    obj_offset_in_sd = (u32)(obj_offset_in_fpm % I40E_HMC_DIRECT_BP_SIZE);
 ...
 **/
enum i40e_status_code
i40e_get_lan_tx_queue_context(struct i40e_hw *hw, u16 queue,
  ...
  return i40e_get_hmc_context(context_bytes, i40e_hmc_txq_ce_info, (u8 *)s);
}
 ...
 **/
enum i40e_status_code
i40e_set_lan_tx_queue_context(struct i40e_hw *hw, u16 queue,
                              struct i40e_hmc_obj_txq *s) {
  enum i40e_status_code err;
  u8 *context_bytes;
  ...
  return i40e_set_hmc_context(context_bytes, i40e_hmc_txq_ce_info, (u8 *)s);
}
 ...
 **/
enum i40e_status_code
i40e_get_lan_rx_queue_context(struct i40e_hw *hw, u16 queue,
  ...
  return i40e_get_hmc_context(context_bytes, i40e_hmc_rxq_ce_info, (u8 *)s);
}
 ...
 **/
enum i40e_status_code
i40e_set_lan_rx_queue_context(struct i40e_hw *hw, u16 queue,
                              struct i40e_hmc_obj_rxq *s) {
  enum i40e_status_code err;
  u8 *context_bytes;
  ...
  return i40e_set_hmc_context(context_bytes, i40e_hmc_rxq_ce_info, (u8 *)s);
}
