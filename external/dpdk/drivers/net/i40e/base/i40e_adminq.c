...
enum i40e_status_code i40e_alloc_adminq_asq_ring(struct i40e_hw *hw) {
  ...
  ret_code = i40e_allocate_dma_mem(
      hw, &hw->aq.asq.desc_buf, i40e_mem_atq_ring,
      (hw->aq.num_asq_entries * sizeof(struct i40e_aq_desc)),
      I40E_ADMINQ_DESC_ALIGNMENT);
  if (ret_code)
    return ret_code;

  ret_code = i40e_allocate_virt_mem(
      hw, &hw->aq.asq.cmd_buf,
      (hw->aq.num_asq_entries * sizeof(struct i40e_asq_cmd_details)));
  if (ret_code) {
...
enum i40e_status_code i40e_alloc_adminq_arq_ring(struct i40e_hw *hw) {
  ...
  ret_code = i40e_allocate_dma_mem(
      hw, &hw->aq.arq.desc_buf, i40e_mem_arq_ring,
      (hw->aq.num_arq_entries * sizeof(struct i40e_aq_desc)),
      I40E_ADMINQ_DESC_ALIGNMENT);
...
STATIC enum i40e_status_code i40e_alloc_arq_bufs(struct i40e_hw *hw) {
  ...
  for (i = 0; i < hw->aq.num_arq_entries; i++) {
    ...
    ret_code =
        i40e_allocate_dma_mem(hw, bi, i40e_mem_arq_buf, hw->aq.arq_buf_size,
                              I40E_ADMINQ_DESC_ALIGNMENT);
    if (ret_code)
...
STATIC enum i40e_status_code i40e_alloc_asq_bufs(struct i40e_hw *hw) {
  ...
  for (i = 0; i < hw->aq.num_asq_entries; i++) {
    ...
    ret_code =
        i40e_allocate_dma_mem(hw, bi, i40e_mem_asq_buf, hw->aq.asq_buf_size,
                              I40E_ADMINQ_DESC_ALIGNMENT);
    if (ret_code)
...
STATIC enum i40e_status_code i40e_config_asq_regs(struct i40e_hw *hw) {
  ...
  if (!i40e_is_vf(hw))
    wr32(hw, hw->aq.asq.len,
         (hw->aq.num_asq_entries | I40E_PF_ATQLEN_ATQENABLE_MASK));
#else
  wr32(hw, hw->aq.asq.len,
       (hw->aq.num_asq_entries | I40E_PF_ATQLEN_ATQENABLE_MASK));
#endif /* INTEGRATED_VF */
  ...
  if (i40e_is_vf(hw))
    wr32(hw, hw->aq.asq.len,
         (hw->aq.num_asq_entries | I40E_VF_ATQLEN1_ATQENABLE_MASK));
#else
  wr32(hw, hw->aq.asq.len,
       (hw->aq.num_asq_entries | I40E_VF_ATQLEN1_ATQENABLE_MASK));
#endif /* INTEGRATED_VF */
...
STATIC enum i40e_status_code i40e_config_arq_regs(struct i40e_hw *hw) {
  ...
  if (!i40e_is_vf(hw))
    wr32(hw, hw->aq.arq.len,
         (hw->aq.num_arq_entries | I40E_PF_ARQLEN_ARQENABLE_MASK));
#else
  wr32(hw, hw->aq.arq.len,
       (hw->aq.num_arq_entries | I40E_PF_ARQLEN_ARQENABLE_MASK));
#endif /* INTEGRATED_VF */
  ...
  if (i40e_is_vf(hw))
    wr32(hw, hw->aq.arq.len,
         (hw->aq.num_arq_entries | I40E_VF_ARQLEN1_ARQENABLE_MASK));
#else
  wr32(hw, hw->aq.arq.len,
       (hw->aq.num_arq_entries | I40E_VF_ARQLEN1_ARQENABLE_MASK));
#endif /* INTEGRATED_VF */
...
STATIC void i40e_set_hw_flags(struct i40e_hw *hw) {
  ...
  if (aq->api_maj_ver > 1 || (aq->api_maj_ver == 1 && aq->api_min_ver >= 5))
    hw->flags |= I40E_HW_FLAG_NVM_READ_REQUIRES_LOCK;

  if (aq->api_maj_ver > 1 || (aq->api_maj_ver == 1 && aq->api_min_ver >= 8)) {
    hw->flags |= I40E_HW_FLAG_FW_LLDP_PERSISTENT;
    hw->flags |= I40E_HW_FLAG_DROP_MODE;
  }

  if (aq->api_maj_ver > 1 || (aq->api_maj_ver == 1 && aq->api_min_ver >= 9))
...
enum i40e_status_code i40e_init_adminq(struct i40e_hw *hw) {
  ...
  /* verify input for valid configuration */
  if (aq->num_arq_entries == 0 || aq->num_asq_entries == 0 ||
      aq->arq_buf_size == 0 || aq->asq_buf_size == 0) {
    ret_code = I40E_ERR_CONFIG;
    goto init_adminq_exit;
  }
   ...
   */
  do {
    ret_code = i40e_aq_get_firmware_version(
        hw, &aq->fw_maj_ver, &aq->fw_min_ver, &aq->fw_build, &aq->api_maj_ver,
        &aq->api_min_ver, NULL);
  ...
  i40e_read_nvm_word(hw, I40E_SR_NVM_DEV_STARTER_VERSION, &hw->nvm.version);
...
u16 i40e_clean_asq(struct i40e_hw *hw) {
  ...
  while (rd32(hw, hw->aq.asq.head) != ntc) {
    i40e_debug(hw, I40E_DEBUG_AQ_COMMAND, "ntc %d head %d.\n", ntc,
               rd32(hw, hw->aq.asq.head));

    if (details->callback) {
      I40E_ADMINQ_CALLBACK cb_func = (I40E_ADMINQ_CALLBACK)details->callback;
      i40e_memcpy(&desc_cb, desc, sizeof(struct i40e_aq_desc), I40E_DMA_TO_DMA);
      cb_func(hw, &desc_cb);
    }
 ...
 **/
STATIC enum i40e_status_code i40e_asq_send_command_exec(
  ...
  if (val >= hw->aq.num_asq_entries) {
    i40e_debug(hw, I40E_DEBUG_AQ_MESSAGE, "AQTX: head overrun at %d\n", val);
  ...
  if (cmd_details) {
    i40e_memcpy(details, cmd_details, sizeof(struct i40e_asq_cmd_details),
     ...
     */
    if (details->cookie) {
      desc->cookie_high = CPU_TO_LE32(I40E_HI_DWORD(details->cookie));
      desc->cookie_low = CPU_TO_LE32(I40E_LO_DWORD(details->cookie));
    }
  } else {
    i40e_memset(details, 0, sizeof(struct i40e_asq_cmd_details),
                I40E_NONDMA_MEM);
  }
  ...
  if (buff_size > hw->aq.asq_buf_size) {
    i40e_debug(hw, I40E_DEBUG_AQ_MESSAGE, "AQTX: Invalid buffer size: %d.\n",
               buff_size);
  ...
  if (i40e_clean_asq(hw) == 0) {
    i40e_debug(hw, I40E_DEBUG_AQ_MESSAGE, "AQTX: Error queue is full.\n");
    status = I40E_ERR_ADMIN_QUEUE_FULL;
    goto asq_send_command_error;
  }
  ...
  if (buff != NULL) {
    ...
    i40e_memcpy(dma_buff->va, buff, buff_size, I40E_NONDMA_TO_DMA);
  ...
  if (i40e_asq_done(hw)) {
    i40e_memcpy(desc, desc_on_ring, sizeof(struct i40e_aq_desc),
                I40E_DMA_TO_NONDMA);
    if (buff != NULL)
      i40e_memcpy(buff, dma_buff->va, buff_size, I40E_DMA_TO_NONDMA);
  ...
  i40e_debug(hw, I40E_DEBUG_AQ_COMMAND, "AQTX: desc and buffer writeback:\n");
  i40e_debug_aq(hw, I40E_DEBUG_AQ_COMMAND, (void *)desc, buff, buff_size);

  /* save writeback aq if requested */
  if (details->wb_desc)
    i40e_memcpy(details->wb_desc, desc_on_ring, sizeof(struct i40e_aq_desc),
                I40E_DMA_TO_NONDMA);
...
#endif
      i40e_debug(hw, I40E_DEBUG_AQ_MESSAGE, "AQTX: AQ Critical error.\n");
      status = I40E_ERR_ADMIN_QUEUE_CRITICAL_ERROR;
    } else {
      i40e_debug(hw, I40E_DEBUG_AQ_MESSAGE, "AQTX: Writeback timeout.\n");
...
i40e_asq_send_command(struct i40e_hw *hw, struct i40e_aq_desc *desc,
  ...
  i40e_acquire_spinlock(&hw->aq.asq_spinlock);
  status = i40e_asq_send_command_exec(hw, desc, buff, buff_size, cmd_details);
  i40e_release_spinlock(&hw->aq.asq_spinlock);
  return status;
}
...
enum i40e_status_code i40e_clean_arq_element(struct i40e_hw *hw,
  ...
  hw->aq.arq_last_status = (enum i40e_admin_queue_err)LE16_TO_CPU(desc->retval);
  ...
  i40e_memcpy(&e->desc, desc, sizeof(struct i40e_aq_desc), I40E_DMA_TO_NONDMA);
  datalen = LE16_TO_CPU(desc->datalen);
  e->msg_len = min(datalen, e->buf_len);
  if (e->msg_buf != NULL && (e->msg_len != 0))
    i40e_memcpy(e->msg_buf, hw->aq.arq.r.arq_bi[desc_idx].va, e->msg_len,
                I40E_DMA_TO_NONDMA);
