...
enum i40e_status_code i40e_init_nvm(struct i40e_hw *hw) {
  ...
  sr_size =
      ((gens & I40E_GLNVM_GENS_SR_SIZE_MASK) >> I40E_GLNVM_GENS_SR_SIZE_SHIFT);
...
i40e_acquire_nvm(struct i40e_hw *hw, enum i40e_aq_resource_access_type access) {
  ...
  ret_code = i40e_aq_request_resource(hw, I40E_NVM_RESOURCE_ID, access, 0,
                                      &time_left, NULL);
  ...
  if (ret_code)
    i40e_debug(hw, I40E_DEBUG_NVM,
               "NVM acquire type %d failed time_left=%" PRIu64
               " ret=%d aq_err=%d\n",
  ...
  if (ret_code && time_left) {
    ...
    if (ret_code != I40E_SUCCESS) {
      ...
      i40e_debug(hw, I40E_DEBUG_NVM,
                 "NVM acquire timed out, wait %" PRIu64
                 " ms before trying again. status=%d aq_err=%d\n",
...
STATIC enum i40e_status_code i40e_read_nvm_word_srctl(struct i40e_hw *hw,
  ...
  if (ret_code == I40E_SUCCESS) {
    ...
    if (ret_code == I40E_SUCCESS) {
      ...
      *data = (u16)((sr_reg & I40E_GLNVM_SRDATA_RDDATA_MASK) >>
...
STATIC enum i40e_status_code i40e_read_nvm_aq(struct i40e_hw *hw,
  ...
  if ((offset + words) > hw->nvm.sr_size)
    ...
    i40e_debug(hw, I40E_DEBUG_NVM,
               "NVM write error: cannot spread over two sectors in a single "
               "write offset=%d words=%d\n",
               offset, words);
  else
    ret_code = i40e_aq_read_nvm(hw, module_pointer, 2 * offset, /*bytes*/
                                2 * words,                      /*bytes*/
 ...
 **/
STATIC enum i40e_status_code i40e_read_nvm_word_aq(struct i40e_hw *hw,
                                                   u16 offset, u16 *data) {
  enum i40e_status_code ret_code = I40E_ERR_TIMEOUT;
...
#define I40E_NVM_INVALID_VAL 0xFFFF
  ...
  if (ptr_value & I40E_PTR_TYPE) {
    ...
    i40e_debug(hw, I40E_DEBUG_ALL,
               "Reading nvm data failed. Pointer points outside of the Shared "
               "RAM mapped area.\n");
    ...
    status = i40e_read_nvm_word(hw, ptr_value + module_offset, &specific_ptr);
    if (status != I40E_SUCCESS) {
      i40e_debug(hw, I40E_DEBUG_ALL,
                 "Reading nvm word failed.Error code: %d.\n", status);
      return I40E_ERR_NVM;
    ...
    offset = ptr_value + module_offset + specific_ptr + data_offset;

    status = i40e_read_nvm_buffer(hw, offset, &words_data_size, data_ptr);
    if (status != I40E_SUCCESS) {
      i40e_debug(hw, I40E_DEBUG_ALL,
...
i40e_read_nvm_buffer_aq(struct i40e_hw *hw, u16 offset, u16 *words, u16 *data) {
  ...
  do {
    ...
    if (offset % I40E_SR_SECTOR_SIZE_IN_WORDS)
      read_size = min(*words, (u16)(I40E_SR_SECTOR_SIZE_IN_WORDS -
                                    (offset % I40E_SR_SECTOR_SIZE_IN_WORDS)));
    else
      read_size = min((*words - words_read), I40E_SR_SECTOR_SIZE_IN_WORDS);

    /* Check if this is last command, if so set proper flag */
    if ((words_read + read_size) >= *words)
      last_cmd = true;

    ret_code = i40e_read_nvm_aq(hw, 0x0, offset, read_size, data + words_read,
                                last_cmd);
    if (ret_code != I40E_SUCCESS)
...
enum i40e_status_code i40e_write_nvm_aq(struct i40e_hw *hw, u8 module_pointer,
  ...
  if ((offset + words) > hw->nvm.sr_size)
    ...
    /* We can write only up to 4KB (one sector), in one AQ write */
    DEBUGOUT("NVM write fail error: cannot write more than 4KB in a single "
             "write.\n");
  else if (((offset + (words - 1)) / I40E_SR_SECTOR_SIZE_IN_WORDS) !=
           (offset / I40E_SR_SECTOR_SIZE_IN_WORDS))
    /* A single write cannot spread over two sectors */
    DEBUGOUT(
        "NVM write error: cannot spread over two sectors in a single write.\n");
  else
    ret_code = i40e_aq_update_nvm(hw, module_pointer, 2 * offset, /*bytes*/
                                  2 * words,                      /*bytes*/
                                  data, last_command, 0, &cmd_details);
...
enum i40e_status_code __i40e_write_nvm_buffer(struct i40e_hw *hw,
   ...
   */
  return i40e_write_nvm_aq(hw, module_pointer, offset, words, data, false);
}
 ...
 **/
enum i40e_status_code i40e_calc_nvm_checksum(struct i40e_hw *hw,
                                             u16 *checksum) {
  enum i40e_status_code ret_code = I40E_SUCCESS;
  ...
  for (i = 0; i < hw->nvm.sr_size; i++) {
    ...
    if ((i >= (u32)vpd_module) &&
        (i < ((u32)vpd_module + (I40E_SR_VPD_MODULE_MAX_SIZE / 2)))) {
      continue;
    }
    /* Skip PCIe ALT module (convert byte size to word count) */
    if ((i >= (u32)pcie_alt_module) &&
        (i < ((u32)pcie_alt_module + (I40E_SR_PCIE_ALT_MODULE_MAX_SIZE / 2)))) {
      continue;
    }
...
enum i40e_status_code i40e_update_nvm_checksum(struct i40e_hw *hw) {
  ...
  if (ret_code == I40E_SUCCESS) {
    ...
    ret_code =
        i40e_write_nvm_aq(hw, 0x00, I40E_SR_SW_CHECKSUM_WORD, 1, &le_sum, true);
  }
...
STATIC enum i40e_status_code i40e_nvmupd_state_init(struct i40e_hw *hw,
                                                    ...
                                                    u8 *bytes, int *perrno);
STATIC enum i40e_status_code
i40e_nvmupd_state_reading(struct i40e_hw *hw, struct i40e_nvm_access *cmd,
                          u8 *bytes, int *perrno);
STATIC enum i40e_status_code
i40e_nvmupd_state_writing(struct i40e_hw *hw, struct i40e_nvm_access *cmd,
                          u8 *bytes, int *perrno);
...
STATIC enum i40e_status_code i40e_nvmupd_exec_aq(struct i40e_hw *hw,
                                                 ...
                                                 u8 *bytes, int *perrno);
STATIC enum i40e_status_code
i40e_nvmupd_get_aq_result(struct i40e_hw *hw, struct i40e_nvm_access *cmd,
                          u8 *bytes, int *perrno);
STATIC enum i40e_status_code
i40e_nvmupd_get_aq_event(struct i40e_hw *hw, struct i40e_nvm_access *cmd,
                         u8 *bytes, int *perrno);
...
STATIC const char *i40e_nvm_update_state_str[] = {
    "I40E_NVMUPD_INVALID",      "I40E_NVMUPD_READ_CON",
    "I40E_NVMUPD_READ_SNT",     "I40E_NVMUPD_READ_LCB",
    "I40E_NVMUPD_READ_SA",      "I40E_NVMUPD_WRITE_ERA",
    "I40E_NVMUPD_WRITE_CON",    "I40E_NVMUPD_WRITE_SNT",
    "I40E_NVMUPD_WRITE_LCB",    "I40E_NVMUPD_WRITE_SA",
    "I40E_NVMUPD_CSUM_CON",     "I40E_NVMUPD_CSUM_SA",
    "I40E_NVMUPD_CSUM_LCB",     "I40E_NVMUPD_STATUS",
    "I40E_NVMUPD_EXEC_AQ",      "I40E_NVMUPD_GET_AQ_RESULT",
    "I40E_NVMUPD_GET_AQ_EVENT", "I40E_NVMUPD_GET_FEATURES",
};
...
enum i40e_status_code i40e_nvmupd_command(struct i40e_hw *hw,
  ...
  *perrno = 0;
  ...
  i40e_debug(hw, I40E_DEBUG_NVM,
             "%s state %d nvm_release_on_hold %d opc 0x%04x cmd 0x%08x config "
             "0x%08x offset 0x%08x data_size 0x%08x\n",
             i40e_nvm_update_state_str[upd_cmd], hw->nvmupd_state,
             hw->nvm_release_on_done, hw->nvm_wait_opcode, cmd->command,
             cmd->config, cmd->offset, cmd->data_size);
  ...
  if (upd_cmd == I40E_NVMUPD_FEATURES) {
    ...
    if (cmd->data_size > hw->nvmupd_features.size)
      i40e_memset(bytes + hw->nvmupd_features.size, 0x0,
                  cmd->data_size - hw->nvmupd_features.size, I40E_NONDMA_MEM);

    i40e_memcpy(bytes, &hw->nvmupd_features, hw->nvmupd_features.size,
                I40E_NONDMA_MEM);
  ...
  default:
    ...
    i40e_debug(hw, I40E_DEBUG_NVM, "NVMUPD: no such state %d\n",
               hw->nvmupd_state);
...
STATIC enum i40e_status_code i40e_nvmupd_state_init(struct i40e_hw *hw,
  ...
  default:
    i40e_debug(hw, I40E_DEBUG_NVM, "NVMUPD: bad cmd %s in init state\n",
...
i40e_nvmupd_state_reading(struct i40e_hw *hw, struct i40e_nvm_access *cmd,
  ...
  default:
    i40e_debug(hw, I40E_DEBUG_NVM, "NVMUPD: bad cmd %s in reading state.\n",
               i40e_nvm_update_state_str[upd_cmd]);
    status = I40E_NOT_SUPPORTED;
    *perrno = -ESRCH;
    break;
...
retry:
  ...
  default:
    i40e_debug(hw, I40E_DEBUG_NVM, "NVMUPD: bad cmd %s in writing state.\n",
               i40e_nvm_update_state_str[upd_cmd]);
    status = I40E_NOT_SUPPORTED;
    *perrno = -ESRCH;
    break;
  ...
  if (status && (hw->aq.asq_last_status == I40E_AQ_RC_EBUSY) &&
    ...
    if (gtime >= hw->nvm.hw_semaphore_timeout) {
      i40e_debug(hw, I40E_DEBUG_ALL,
                 "NVMUPD: write semaphore expired (%d >= %" PRIu64
                 "), retrying\n",
...
void i40e_nvmupd_clear_wait_state(struct i40e_hw *hw) {
  i40e_debug(hw, I40E_DEBUG_NVM, "NVMUPD: clearing wait on opcode 0x%04x\n",
             hw->nvm_wait_opcode);
...
void i40e_nvmupd_check_wait_event(struct i40e_hw *hw, u16 opcode,
  ...
  if (opcode == hw->nvm_wait_opcode) {
    i40e_memcpy(&hw->nvm_aq_event_desc, desc, aq_desc_len,
...
STATIC enum i40e_status_code i40e_nvmupd_exec_aq(struct i40e_hw *hw,
  ...
  if (buff_size) {
    if (!hw->nvm_buff.va) {
      status = i40e_allocate_virt_mem(hw, &hw->nvm_buff, hw->aq.asq_buf_size);
      if (status)
        i40e_debug(hw, I40E_DEBUG_NVM,
  ...
  /* and away we go! */
  status = i40e_asq_send_command(hw, aq_desc, buff, buff_size, &cmd_details);
  if (status) {
    i40e_debug(hw, I40E_DEBUG_NVM, "i40e_nvmupd_exec_aq err %s aq_err %s\n",
...
i40e_nvmupd_get_aq_result(struct i40e_hw *hw, struct i40e_nvm_access *cmd,
  ...
  if (remainder > 0) {
    ...
    i40e_debug(hw, I40E_DEBUG_NVM, "%s: databuf bytes %d to %d\n", __func__,
               start_byte, start_byte + remainder);
...
STATIC enum i40e_status_code i40e_nvmupd_nvm_read(struct i40e_hw *hw,
  ...
  status = i40e_aq_read_nvm(hw, module, cmd->offset, (u16)cmd->data_size, bytes,
                            last, &cmd_details);
  if (status) {
    i40e_debug(hw, I40E_DEBUG_NVM,
               "i40e_nvmupd_nvm_read mod 0x%x  off 0x%x  len 0x%x\n", module,
               cmd->offset, cmd->data_size);
    i40e_debug(hw, I40E_DEBUG_NVM, "i40e_nvmupd_nvm_read status %d aq %d\n",
...
STATIC enum i40e_status_code i40e_nvmupd_nvm_erase(struct i40e_hw *hw,
  ...
  status = i40e_aq_erase_nvm(hw, module, cmd->offset, (u16)cmd->data_size, last,
                             &cmd_details);
  if (status) {
    i40e_debug(hw, I40E_DEBUG_NVM,
               "i40e_nvmupd_nvm_erase mod 0x%x  off 0x%x len 0x%x\n", module,
               cmd->offset, cmd->data_size);
    i40e_debug(hw, I40E_DEBUG_NVM, "i40e_nvmupd_nvm_erase status %d aq %d\n",
               status, hw->aq.asq_last_status);
    *perrno = i40e_aq_rc_to_posix(status, hw->aq.asq_last_status);
  }
...
STATIC enum i40e_status_code i40e_nvmupd_nvm_write(struct i40e_hw *hw,
  ...
  status = i40e_aq_update_nvm(hw, module, cmd->offset, (u16)cmd->data_size,
                              bytes, last, preservation_flags, &cmd_details);
  if (status) {
    i40e_debug(hw, I40E_DEBUG_NVM,
               "i40e_nvmupd_nvm_write mod 0x%x off 0x%x len 0x%x\n", module,
               cmd->offset, cmd->data_size);
    i40e_debug(hw, I40E_DEBUG_NVM, "i40e_nvmupd_nvm_write status %d aq %d\n",
               status, hw->aq.asq_last_status);
    *perrno = i40e_aq_rc_to_posix(status, hw->aq.asq_last_status);
  }
