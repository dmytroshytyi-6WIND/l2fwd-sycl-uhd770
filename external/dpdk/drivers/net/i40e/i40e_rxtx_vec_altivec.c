...
static inline void i40e_rxq_rearm(struct i40e_rx_queue *rxq) {
  ...
  vector unsigned long hdr_room =
      (vector unsigned long){RTE_PKTMBUF_HEADROOM, RTE_PKTMBUF_HEADROOM};
  vector unsigned long dma_addr0, dma_addr1;

  rxdp = rxq->rx_ring + rxq->rxrearm_start;

  /* Pull 'n' more MBUFs into the software ring */
  if (rte_mempool_get_bulk(rxq->mp, (void *)rxep, RTE_I40E_RXQ_REARM_THRESH) <
      0) {
...
static inline void desc_to_olflags_v(vector unsigned long descs[4],
   ...
   */
  const vector unsigned int rss_vlan_msk =
      (vector unsigned int){(int32_t)0x1c03804, (int32_t)0x1c03804,
  ...
  const vector unsigned char rss_flags =
  ...
  vlan0 = (vector unsigned int)vec_perm(vlan_flags, (vector unsigned char){},
  ...
  l3_l4e = (vector unsigned int)vec_perm(l3_l4e_flags, (vector unsigned char){},
...
static inline void desc_to_ptype_v(vector unsigned long descs[4],
                                   struct rte_mbuf **rx_pkts,
                                   uint32_t *ptype_tbl) {
 ...
 */
static inline uint16_t _recv_raw_pkts_vec(struct i40e_rx_queue *rxq,
                                          struct rte_mbuf **rx_pkts,
                                          uint16_t nb_pkts,
  ...
  /* 4 packets DD mask */
  dd_check =
      (vector unsigned long){0x0000000100000001ULL, 0x0000000100000001ULL};

  /* 4 packets EOP mask */
  eop_check =
      (vector unsigned long){0x0000000200000002ULL, 0x0000000200000002ULL};
  ...
  for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
    ...
    *(vector unsigned long *)&rx_pkts[pos + 2] = mbp2;
    ...
    const vector unsigned int len3 =
        vec_sl(vec_ld(0, (vector unsigned int *)&descs[3]),
               (vector unsigned int){0, 0, 0, PKTLEN_SHIFT});

    const vector unsigned int len2 =
        vec_sl(vec_ld(0, (vector unsigned int *)&descs[2]),
               (vector unsigned int){0, 0, 0, PKTLEN_SHIFT});
    ...
    pkt_mb4 = vec_perm((vector unsigned char)descs[3], (vector unsigned char){},
                       shuf_msk);
    pkt_mb3 = vec_perm((vector unsigned char)descs[2], (vector unsigned char){},
                       shuf_msk);
    ...
    pkt_mb4 = (vector unsigned char)vec_sub((vector unsigned short)pkt_mb4,
                                            crc_adjust);
    pkt_mb3 = (vector unsigned char)vec_sub((vector unsigned short)pkt_mb3,
                                            crc_adjust);

    /* pkt 1,2 shift the pktlen field to be 16-bit aligned*/
    const vector unsigned int len1 =
        vec_sl(vec_ld(0, (vector unsigned int *)&descs[1]),
               (vector unsigned int){0, 0, 0, PKTLEN_SHIFT});
    const vector unsigned int len0 =
        vec_sl(vec_ld(0, (vector unsigned int *)&descs[0]),
               (vector unsigned int){0, 0, 0, PKTLEN_SHIFT});
    ...
    pkt_mb2 = vec_perm((vector unsigned char)descs[1], (vector unsigned char){},
                       shuf_msk);
    pkt_mb1 = vec_perm((vector unsigned char)descs[0], (vector unsigned char){},
                       shuf_msk);
    ...
    pkt_mb2 = (vector unsigned char)vec_sub((vector unsigned short)pkt_mb2,
                                            crc_adjust);
    pkt_mb1 = (vector unsigned char)vec_sub((vector unsigned short)pkt_mb1,
                                            crc_adjust);
    ...
    if (split_packet) {
      vector unsigned char eop_shuf_mask = (vector unsigned char){
          ...
          0xFF, 0xFF, 0xFF, 0xFF, 0x04, 0x0C, 0x00, 0x08};

      /* and with mask to extract bits, flipping 1-0 */
      vector unsigned char eop_bits =
          vec_and((vector unsigned char)vec_nor(staterr, staterr),
      ...
      *split_packet = (vec_ld(0, (vector unsigned int *)&eop_bits))[0];
    ...
    var =
        __builtin_popcountll((vec_ld(0, (vector unsigned long *)&staterr)[0]));
    nb_pkts_recd += var;
...
static inline void vtx1(volatile struct i40e_tx_desc *txdp,
  ...
  vector unsigned long descriptor =
      (vector unsigned long){pkt->buf_iova + pkt->data_off, high_qw};
