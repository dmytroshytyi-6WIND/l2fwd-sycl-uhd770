...
static int i40e_level_capabilities_get(struct rte_eth_dev *dev,
                                       ...
                                       struct rte_tm_error *error);
static int i40e_node_capabilities_get(struct rte_eth_dev *dev, uint32_t node_id,
                                      struct rte_tm_node_capabilities *cap,
                                      struct rte_tm_error *error);
static int i40e_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail,
                                 struct rte_tm_error *error);
...
int i40e_tm_ops_get(struct rte_eth_dev *dev __rte_unused, void *arg) {
  if (!arg)
    return -EINVAL;
...
static inline uint16_t i40e_tc_nb_get(struct rte_eth_dev *dev) {
  struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
...
static int i40e_shaper_profile_param_check(struct rte_tm_shaper_params *profile,
                                           struct rte_tm_error *error) {
...
static int i40e_shaper_profile_add(struct rte_eth_dev *dev,
  ...
  shaper_profile = rte_zmalloc("i40e_tm_shaper_profile",
                               sizeof(struct i40e_tm_shaper_profile), 0);
  if (!shaper_profile)
    return -ENOMEM;
...
i40e_tm_node_search(struct rte_eth_dev *dev, uint32_t node_id,
...
static int i40e_node_param_check(struct rte_eth_dev *dev, uint32_t node_id,
...
static int i40e_node_add(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  ret = i40e_node_param_check(dev, node_id, priority, weight, params, error);
  if (ret)
    return ret;
  ...
  if (params->shaper_profile_id != RTE_TM_SHAPER_PROFILE_ID_NONE) {
    shaper_profile = i40e_shaper_profile_search(dev, params->shaper_profile_id);
    if (!shaper_profile) {
  ...
  if (parent_node_id == RTE_TM_NODE_ID_NULL) {
    ...
    /* add the root node */
    tm_node = rte_zmalloc("i40e_tm_node", sizeof(struct i40e_tm_node), 0);
    if (!tm_node)
      return -ENOMEM;
    tm_node->id = node_id;
  ...
  parent_node = i40e_tm_node_search(dev, parent_node_id, &parent_node_type);
  if (!parent_node) {
    error->type = RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID;
...
static int i40e_level_capabilities_get(struct rte_eth_dev *dev,
  ...
  if (level_id != I40E_TM_NODE_TYPE_QUEUE) {
    ...
    if (level_id == I40E_TM_NODE_TYPE_PORT)
      ...
      cap->nonleaf.sched_n_children_max = hw->func_caps.num_tx_qp;
