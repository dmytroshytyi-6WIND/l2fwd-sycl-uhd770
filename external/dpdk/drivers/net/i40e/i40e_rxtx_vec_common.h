...
static inline uint16_t reassemble_packets(struct i40e_rx_queue *rxq,
...
static __rte_always_inline void tx_backlog_entry(struct i40e_tx_entry *txep,
...
static inline void _i40e_rx_queue_release_mbufs_vec(struct i40e_rx_queue *rxq) {
...
static inline int i40e_rxq_vec_setup_default(struct i40e_rx_queue *rxq) {
  uintptr_t p;
  ...
  if (!dev->data->dev_started) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
      ...
      if (first_queue)
        ...
        ad->rx_vec_allowed =
            ad->rx_vec_allowed ? rte_is_power_of_2(desc) : ad->rx_vec_allowed;
      first_queue = false;
    }
  } else {
