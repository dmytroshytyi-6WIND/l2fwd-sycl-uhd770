...
int rte_pmd_i40e_set_vf_mac_anti_spoof(uint16_t port, uint16_t vf_id,
  ...
  if (vsi->info.valid_sections &
    ...
    if (on) {
      if ((vsi->info.sec_flags & I40E_AQ_VSI_SEC_FLAG_ENABLE_MAC_CHK) ==
          I40E_AQ_VSI_SEC_FLAG_ENABLE_MAC_CHK)
        return 0; /* already on */
    } else {
      if ((vsi->info.sec_flags & I40E_AQ_VSI_SEC_FLAG_ENABLE_MAC_CHK) == 0)
...
static int i40e_add_rm_all_vlan_filter(struct i40e_vsi *vsi, uint8_t add) {
  uint32_t j, k;
  uint16_t vlan_id;
  ...
  for (j = 0; j < I40E_VFTA_SIZE; j++) {
    ...
    for (k = 0; k < I40E_UINT32_BIT_SIZE; k++) {
      ...
      if (ret != I40E_SUCCESS) {
        PMD_DRV_LOG(ERR, "Failed to add/rm vlan filter");
        return ret;
      }
    }
  }
...
static int i40e_vsi_rm_mac_filter(struct i40e_vsi *vsi) {
...
static int i40e_vsi_restore_mac_filter(struct i40e_vsi *vsi) {
  struct i40e_mac_filter *f;
  struct i40e_macvlan_filter *mv_f;
  int i, vlan_num = 0;
  int ret = I40E_SUCCESS;
...
static int i40e_vsi_set_tx_loopback(struct i40e_vsi *vsi, uint8_t on) {
  struct i40e_vsi_context ctxt;
  struct i40e_hw *hw;
  int ret;
  ...
  if (vsi->info.valid_sections &
    ...
    if (on) {
      if ((vsi->info.switch_id & I40E_AQ_VSI_SW_ID_FLAG_ALLOW_LB) ==
          I40E_AQ_VSI_SW_ID_FLAG_ALLOW_LB)
        return 0; /* already on */
    } else {
      if ((vsi->info.switch_id & I40E_AQ_VSI_SW_ID_FLAG_ALLOW_LB) == 0)
        return 0; /* already off */
    }
  }
...
int rte_pmd_i40e_set_vf_unicast_promisc(uint16_t port, uint16_t vf_id,
  ...
  ret = i40e_aq_set_vsi_unicast_promiscuous(hw, vsi->seid, on, NULL, true);
  if (ret != I40E_SUCCESS) {
    ret = -ENOTSUP;
...
int rte_pmd_i40e_set_vf_multicast_promisc(uint16_t port, uint16_t vf_id,
                                          uint8_t on) {
  struct rte_eth_dev *dev;
  struct i40e_pf *pf;
  struct i40e_vsi *vsi;
  ...
  ret = i40e_aq_set_vsi_multicast_promiscuous(hw, vsi->seid, on, NULL);
  if (ret != I40E_SUCCESS) {
    ret = -ENOTSUP;
...
int rte_pmd_i40e_set_vf_mac_addr(uint16_t port, uint16_t vf_id,
  ...
  RTE_TAILQ_FOREACH_SAFE(f, &vsi->mac_list, next, temp)
  if (i40e_vsi_delete_mac(vsi, &f->mac_info.mac_addr) != I40E_SUCCESS)
...
int rte_pmd_i40e_set_vf_vlan_stripq(uint16_t port, uint16_t vf_id, uint8_t on) {
  struct rte_eth_dev *dev;
  struct i40e_pf *pf;
  struct i40e_vsi *vsi;
  int ret;
...
static int i40e_vlan_filter_count(struct i40e_vsi *vsi) {
  uint32_t j, k;
  uint16_t vlan_id;
  int count = 0;
      ...
      if (on) {
        if (!vsi->vlan_filter_on) {
          ...
          if (!vsi->vlan_anti_spoof_on)
            i40e_add_rm_all_vlan_filter(vsi, true);
        }
...
int rte_pmd_i40e_get_vf_stats(uint16_t port, uint16_t vf_id,
  ...
  stats->ipackets = vsi->eth_stats.rx_unicast + vsi->eth_stats.rx_multicast +
                    vsi->eth_stats.rx_broadcast;
  stats->opackets = vsi->eth_stats.tx_unicast + vsi->eth_stats.tx_multicast +
...
int rte_pmd_i40e_set_vf_max_bw(uint16_t port, uint16_t vf_id, uint32_t bw) {
  ...
  if (bw == vsi->bw_info.bw_limit) {
    PMD_DRV_LOG(INFO, "No change for VF max bandwidth. Nothing to do.");
    return 0;
  }
  ...
  if (bw) {
    for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
      if ((vsi->enabled_tc & BIT_ULL(i)) && vsi->bw_info.bw_ets_credits[i])
        break;
    }
    if (i != I40E_MAX_TRAFFIC_CLASS) {
      PMD_DRV_LOG(ERR, "TC max bandwidth has been set on this VF,"
  ...
  if (ret) {
    PMD_DRV_LOG(ERR, "Failed to set VF %d bandwidth, err(%d).", vf_id, ret);
    return -EINVAL;
  }
...
int rte_pmd_i40e_set_vf_tc_bw_alloc(uint16_t port, uint16_t vf_id,
  ...
  if (tc_num > I40E_MAX_TRAFFIC_CLASS) {
    PMD_DRV_LOG(ERR, "TCs should be no more than %d.", I40E_MAX_TRAFFIC_CLASS);
    return -EINVAL;
  }
  ...
  if (sum != tc_num) {
    PMD_DRV_LOG(ERR, "Weight should be set for all %d enabled TCs.", sum);
    return -EINVAL;
  }
  ...
  for (i = 0; i < tc_num; i++) {
    if (!bw_weight[i]) {
      PMD_DRV_LOG(ERR, "The weight should be 1 at least.");
      return -EINVAL;
    }
    sum += bw_weight[i];
  }
  if (sum != 100) {
    PMD_DRV_LOG(ERR, "The summary of the TC weight should be 100.");
    return -EINVAL;
  }
  ...
  for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
    if (vsi->enabled_tc & BIT_ULL(i)) {
      if (bw_weight[j] != vsi->bw_info.bw_ets_share_credits[i])
        b_change = true;
  ...
  /* No change. */
  if (!b_change) {
    PMD_DRV_LOG(INFO, "No change for TC allocated bandwidth."
                      " Nothing to do.");
    return 0;
  }
  ...
  if (ret) {
    PMD_DRV_LOG(ERR, "Failed to set VF %d TC bandwidth weight, err(%d).", vf_id,
                ret);
    return -EINVAL;
  }
...
int rte_pmd_i40e_set_vf_tc_max_bw(uint16_t port, uint16_t vf_id, uint8_t tc_no,
                                  uint32_t bw) {
  struct rte_eth_dev *dev;
  struct i40e_pf *pf;
  struct i40e_vsi *vsi;
  ...
  if (tc_no >= I40E_MAX_TRAFFIC_CLASS) {
    PMD_DRV_LOG(ERR, "TC No. should be less than %d.", I40E_MAX_TRAFFIC_CLASS);
    return -EINVAL;
  }
  ...
  if (!(vsi->enabled_tc & BIT_ULL(tc_no))) {
    PMD_DRV_LOG(ERR, "VF %d TC %d isn't enabled.", vf_id, tc_no);
    return -EINVAL;
  }

  /* No change. */
  if (bw == vsi->bw_info.bw_ets_credits[tc_no]) {
    PMD_DRV_LOG(INFO, "No change for TC max bandwidth. Nothing to do.");
    return 0;
  }
  ...
  if (bw && vsi->bw_info.bw_limit) {
    ...
    PMD_DRV_LOG(INFO, "VF max bandwidth is disabled according"
  ...
  for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
    if (vsi->enabled_tc & BIT_ULL(i)) {
      tc_bw.tc_bw_credits[i] = rte_cpu_to_le_16(vsi->bw_info.bw_ets_credits[i]);
    }
  }
  ...
  if (ret) {
    PMD_DRV_LOG(ERR, "Failed to set VF %d TC %d max bandwidth, err(%d).", vf_id,
                tc_no, ret);
    return -EINVAL;
  }
...
int rte_pmd_i40e_set_tc_strict_prio(uint16_t port, uint8_t tc_map) {
  ...
  if ((tc_map & veb->enabled_tc) != tc_map) {
    PMD_DRV_LOG(ERR, "TC bitmap isn't the subset of enabled TCs 0x%x.",
                veb->enabled_tc);
    return -EINVAL;
  }
  ...
  if (!veb->strict_prio_tc) {
    ...
    if (ret)
      PMD_DRV_LOG(INFO, "Failed to disable DCBx as it's already"
                        " disabled.");
    else
      PMD_DRV_LOG(INFO, "DCBx is disabled according to strict"
                        " priority setting.");
  }
  ...
  for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
    if (veb->enabled_tc & BIT_ULL(i)) {
      ...
      if (veb->bw_info.bw_ets_share_credits[i])
        ets_data.tc_bw_share_credits[i] = veb->bw_info.bw_ets_share_credits[i];
      else
        ets_data.tc_bw_share_credits[i] = I40E_QOS_BW_WEIGHT_MIN;
    }
  }

  if (!veb->strict_prio_tc)
    ret = i40e_aq_config_switch_comp_ets(hw, veb->uplink_seid, &ets_data,
    ...
    ret = i40e_aq_config_switch_comp_ets(
        hw, veb->uplink_seid, &ets_data,
        i40e_aqc_opc_disable_switching_comp_ets, NULL);
  ...
  if (!tc_map) {
    ...
    if (ret) {
      PMD_DRV_LOG(ERR, "Failed to enable DCBx, err(%d).", ret);
      return -EINVAL;
    }

    PMD_DRV_LOG(INFO, "DCBx is enabled again according to strict"
                      " priority setting.");
  }
...
static void i40e_generate_profile_info_sec(char *name,
...
i40e_add_rm_profile_info(struct i40e_hw *hw, uint8_t *profile_info_sec) {
  ...
  sec = (struct i40e_profile_section_header *)profile_info_sec;
  track_id =
      ((struct i40e_profile_info *)(profile_info_sec + sec->section.offset))
          ->track_id;
...
static int i40e_check_profile_info(uint16_t port, uint8_t *profile_info_sec) {
  ...
  pinfo =
      (struct rte_pmd_i40e_profile_info
           *)(profile_info_sec + sizeof(struct i40e_profile_section_header));
  if (pinfo->track_id == 0) {
    PMD_DRV_LOG(INFO, "Read-only profile.");
    return 0;
  }
  buff = rte_zmalloc("pinfo_list",
                     (I40E_PROFILE_INFO_SIZE * I40E_MAX_PROFILE_NUM + 4), 0);
  if (!buff) {
    PMD_DRV_LOG(ERR, "failed to allocate memory");
    return -1;
  }
  ...
  ret = i40e_aq_get_ddp_list(
      hw, (void *)buff, (I40E_PROFILE_INFO_SIZE * I40E_MAX_PROFILE_NUM + 4), 0,
      NULL);
  if (ret) {
  ...
  for (i = 0; i < p_list->p_count; i++) {
    ...
    if ((p->track_id & group_mask) == group_mask)
      continue;
    if ((pinfo->track_id & group_mask) != (p->track_id & group_mask)) {
...
int rte_pmd_i40e_process_ddp_package(uint16_t port, uint8_t *buff,
  ...
  if (op != RTE_PMD_I40E_PKG_OP_WR_ADD && op != RTE_PMD_I40E_PKG_OP_WR_ONLY &&
  ...
  if (size < (sizeof(struct i40e_package_header) +
              sizeof(struct i40e_metadata_segment) + sizeof(uint32_t) * 2)) {
  ...
  profile_seg_hdr = i40e_find_segment_in_package(SEGMENT_TYPE_I40E, pkg_hdr);
  if (!profile_seg_hdr) {
  ...
  i40e_generate_profile_info_sec(
      ...
      &((struct i40e_profile_segment *)profile_seg_hdr)->version, track_id,
      profile_info_sec, op == RTE_PMD_I40E_PKG_OP_WR_ADD);
  ...
  if (op == RTE_PMD_I40E_PKG_OP_WR_DEL) {
    status = i40e_rollback_profile(
        hw, (struct i40e_profile_segment *)profile_seg_hdr, track_id);
    if (status) {
...
int rte_pmd_i40e_get_ddp_info(uint8_t *pkg_buff, uint32_t pkg_size,
  ...
  note_seg_hdr = i40e_find_segment_in_package(SEGMENT_TYPE_NOTES, pkg_hdr);
  ...
  if (type == RTE_PMD_I40E_PKG_INFO_GLOBAL_HEADER) {
    ...
    memcpy(info->name, ((struct i40e_metadata_segment *)metadata_seg_hdr)->name,
           I40E_DDP_NAME_SIZE);
    memcpy(&info->version,
  ...
  if (type == RTE_PMD_I40E_PKG_INFO_HEADER) {
    ...
    memcpy(info->name, ((struct i40e_profile_segment *)i40e_seg_hdr)->name,
           I40E_DDP_NAME_SIZE);
    memcpy(&info->version,
  ...
  if (type == RTE_PMD_I40E_PKG_INFO_DEVID_LIST) {
    uint32_t dev_num;
    dev_num = ((struct i40e_profile_segment *)i40e_seg_hdr)->device_table_count;
  ...
  if (type == RTE_PMD_I40E_PKG_INFO_PROTOCOL_NUM) {
    ...
    proto = i40e_find_section_in_profile(
        SECTION_TYPE_PROTO, (struct i40e_profile_segment *)i40e_seg_hdr);
  ...
  if (type == RTE_PMD_I40E_PKG_INFO_PROTOCOL_LIST) {
    ...
    /* get number of records in the section */
    nb_rec =
        proto->section.size / sizeof(struct i40e_profile_tlv_section_record);
    tlv = (struct i40e_profile_tlv_section_record *)&proto[1];
    for (i = j = 0; i < nb_rec; j++) {
      pinfo[j].proto_id = tlv->data[0];
      strlcpy(pinfo[j].name, (const char *)&tlv->data[1], I40E_DDP_NAME_SIZE);
      i += tlv->len;
      tlv = &tlv[tlv->len];
    }
    return I40E_SUCCESS;
  ...
  if (type == RTE_PMD_I40E_PKG_INFO_PCTYPE_NUM) {
    ...
    pctype = i40e_find_section_in_profile(
        SECTION_TYPE_PCTYPE, (struct i40e_profile_segment *)i40e_seg_hdr);
  ...
  if (type == RTE_PMD_I40E_PKG_INFO_PCTYPE_LIST) {
    ...
    /* get number of records in the section */
    nb_rec =
        pctype->section.size / sizeof(struct i40e_profile_tlv_section_record);
  ...
  if (type == RTE_PMD_I40E_PKG_INFO_PTYPE_NUM) {
    ...
    ptype = i40e_find_section_in_profile(
        SECTION_TYPE_PTYPE, (struct i40e_profile_segment *)i40e_seg_hdr);
  ...
  if (type == RTE_PMD_I40E_PKG_INFO_PTYPE_LIST) {
    ...
    /* get number of records in the section */
    nb_rec =
        ptype->section.size / sizeof(struct i40e_profile_tlv_section_record);
    for (i = j = 0; i < nb_rec; j++) {
...
int rte_pmd_i40e_get_ddp_list(uint16_t port, uint8_t *buff, uint32_t size) {
  ...
  status = i40e_aq_get_ddp_list(hw, (void *)buff, size, 0, NULL);
...
static int check_invalid_pkt_type(uint32_t pkt_type) {
  ...
  if (l2 && l2 != RTE_PTYPE_L2_ETHER && l2 != RTE_PTYPE_L2_ETHER_TIMESYNC &&
      l2 != RTE_PTYPE_L2_ETHER_ARP && l2 != RTE_PTYPE_L2_ETHER_LLDP &&
      l2 != RTE_PTYPE_L2_ETHER_NSH && l2 != RTE_PTYPE_L2_ETHER_VLAN &&
      l2 != RTE_PTYPE_L2_ETHER_QINQ && l2 != RTE_PTYPE_L2_ETHER_PPPOE)
    return -1;

  if (l3 && l3 != RTE_PTYPE_L3_IPV4 && l3 != RTE_PTYPE_L3_IPV4_EXT &&
      l3 != RTE_PTYPE_L3_IPV6 && l3 != RTE_PTYPE_L3_IPV4_EXT_UNKNOWN &&
      l3 != RTE_PTYPE_L3_IPV6_EXT && l3 != RTE_PTYPE_L3_IPV6_EXT_UNKNOWN)
    return -1;

  if (l4 && l4 != RTE_PTYPE_L4_TCP && l4 != RTE_PTYPE_L4_UDP &&
      l4 != RTE_PTYPE_L4_FRAG && l4 != RTE_PTYPE_L4_SCTP &&
      l4 != RTE_PTYPE_L4_ICMP && l4 != RTE_PTYPE_L4_NONFRAG)
    return -1;

  if (tnl && tnl != RTE_PTYPE_TUNNEL_IP && tnl != RTE_PTYPE_TUNNEL_GRENAT &&
      tnl != RTE_PTYPE_TUNNEL_VXLAN && tnl != RTE_PTYPE_TUNNEL_NVGRE &&
      tnl != RTE_PTYPE_TUNNEL_GENEVE && tnl != RTE_PTYPE_TUNNEL_GTPC &&
      tnl != RTE_PTYPE_TUNNEL_GTPU && tnl != RTE_PTYPE_TUNNEL_L2TP &&
      tnl != RTE_PTYPE_TUNNEL_ESP)
    return -1;

  if (il2 && il2 != RTE_PTYPE_INNER_L2_ETHER &&
      il2 != RTE_PTYPE_INNER_L2_ETHER_VLAN &&
      il2 != RTE_PTYPE_INNER_L2_ETHER_QINQ)
    return -1;

  if (il3 && il3 != RTE_PTYPE_INNER_L3_IPV4 &&
      il3 != RTE_PTYPE_INNER_L3_IPV4_EXT && il3 != RTE_PTYPE_INNER_L3_IPV6 &&
      il3 != RTE_PTYPE_INNER_L3_IPV4_EXT_UNKNOWN &&
      il3 != RTE_PTYPE_INNER_L3_IPV6_EXT &&
      il3 != RTE_PTYPE_INNER_L3_IPV6_EXT_UNKNOWN)
    return -1;

  if (il4 && il4 != RTE_PTYPE_INNER_L4_TCP && il4 != RTE_PTYPE_INNER_L4_UDP &&
      il4 != RTE_PTYPE_INNER_L4_FRAG && il4 != RTE_PTYPE_INNER_L4_SCTP &&
      il4 != RTE_PTYPE_INNER_L4_ICMP && il4 != RTE_PTYPE_INNER_L4_NONFRAG)
    return -1;
...
static int
check_invalid_ptype_mapping(struct rte_pmd_i40e_ptype_mapping *mapping_table,
                            uint16_t count) {
  int i;
...
int rte_pmd_i40e_ptype_mapping_update(
    uint16_t port, struct rte_pmd_i40e_ptype_mapping *mapping_items,
  ...
  for (i = 0; i < count; i++)
    ad->ptype_tbl[mapping_items[i].hw_ptype] = mapping_items[i].sw_ptype;
...
int rte_pmd_i40e_ptype_mapping_replace(uint16_t port, uint32_t target,
  ...
  for (i = 0; i < I40E_MAX_PKT_TYPE; i++) {
    if (mask) {
      if ((target | ad->ptype_tbl[i]) == target && (target & ad->ptype_tbl[i]))
...
int rte_pmd_i40e_flow_type_mapping_get(
    uint16_t port, struct rte_pmd_i40e_flow_type_mapping *mapping_items) {
  struct rte_eth_dev *dev;
  struct i40e_adapter *ad;
  uint16_t i;
...
static int i40e_vsi_update_queue_region_mapping(struct i40e_hw *hw,
                                                struct i40e_pf *pf) {
  uint16_t i;
  ...
  struct i40e_queue_regions *region_info = &pf->queue_region;
  int32_t ret = -EINVAL;
  ...
  for (i = 0; i < region_info->queue_region_number; i++) {
    ...
    vsi_info->tc_mapping[tc_index] =
        rte_cpu_to_le_16((queue_offset << I40E_AQ_VSI_TC_QUE_OFFSET_SHIFT) |
                         (bsf << I40E_AQ_VSI_TC_QUE_NUMBER_SHIFT));
  }

  /* Associate queue number with VSI, Keep vsi->nb_qps unchanged */
  vsi_info->mapping_flags |= rte_cpu_to_le_16(I40E_AQ_VSI_QUE_MAP_CONTIG);
...
i40e_queue_region_set_region(struct i40e_pf *pf,
  ...
  if (!((rte_is_power_of_2(conf_ptr->queue_num)) &&
    ...
    PMD_DRV_LOG(ERR, "The region sizes should be any of the following values: "
                     "1, 2, 4, 8, 16, 32, 64 as long as the "
  ...
  if (i == info->queue_region_number && i <= I40E_REGION_MAX_INDEX) {
    info->region[i].region_id = conf_ptr->region_id;
    info->region[i].queue_num = conf_ptr->queue_num;
    info->region[i].queue_start_index = conf_ptr->queue_start_index;
    info->queue_region_number++;
  } else {
    PMD_DRV_LOG(ERR, "queue region number exceeds maxnum 8 or the queue region "
...
static int i40e_queue_region_set_flowtype(
    struct i40e_pf *pf,
  ...
  for (i = 0; i < info->queue_region_number; i++) {
    for (j = 0; j < info->region[i].flowtype_num; j++) {
      if (rss_region_conf->hw_flowtype == info->region[i].hw_flowtype[j]) {
...
static void i40e_queue_region_pf_flowtype_conf(struct i40e_hw *hw,
                                               struct i40e_pf *pf) {
  uint8_t hw_flowtype;
...
static int i40e_queue_region_set_user_priority(
    struct i40e_pf *pf,
    struct rte_pmd_i40e_queue_region_conf *rss_region_conf) {
  struct i40e_queue_regions *info = &pf->queue_region;
  int32_t ret = -EINVAL;
  ...
  for (i = 0; i < info->queue_region_number; i++) {
    for (j = 0; j < info->region[i].user_priority_num; j++) {
      if (info->region[i].user_priority[j] == rss_region_conf->user_priority) {
  ...
  info->region[region_index].user_priority[j] = rss_region_conf->user_priority;
  info->region[region_index].user_priority_num++;

  return 0;
}

static int i40e_queue_region_dcb_configure(struct i40e_hw *hw,
                                           struct i40e_pf *pf) {
  ...
  for (i = 0; i < info->queue_region_number; i++) {
    for (j = 0; j < info->region[i].user_priority_num; j++) {
      ...
      dcb_cfg->etscfg.prioritytable[prio_index] = region_index;
    }
  }
...
int i40e_flush_queue_region_all_conf(struct rte_eth_dev *dev,
                                     struct i40e_hw *hw, struct i40e_pf *pf,
                                     uint16_t on) {
  int32_t ret = -EINVAL;
  struct i40e_queue_regions *info = &pf->queue_region;
  struct i40e_vsi *main_vsi = pf->main_vsi;
...
static int i40e_queue_region_pf_check_rss(struct i40e_pf *pf) {
...
i40e_queue_region_get_all_info(struct i40e_pf *pf,
  ...
  rte_memcpy(regions_ptr, info, sizeof(struct i40e_queue_regions));

  return 0;
}

int rte_pmd_i40e_rss_queue_region_conf(
    uint16_t port_id, enum rte_pmd_i40e_queue_region_op op_type, void *arg) {
  ...
  case RTE_PMD_I40E_RSS_QUEUE_REGION_SET:
    ret = i40e_queue_region_set_region(
        pf, (struct rte_pmd_i40e_queue_region_conf *)arg);
    break;
  case RTE_PMD_I40E_RSS_QUEUE_REGION_FLOWTYPE_SET:
    ret = i40e_queue_region_set_flowtype(
        pf, (struct rte_pmd_i40e_queue_region_conf *)arg);
    break;
  case RTE_PMD_I40E_RSS_QUEUE_REGION_USER_PRIORITY_SET:
    ret = i40e_queue_region_set_user_priority(
        pf, (struct rte_pmd_i40e_queue_region_conf *)arg);
    break;
  ...
  case RTE_PMD_I40E_RSS_QUEUE_REGION_INFO_GET:
    ret = i40e_queue_region_get_all_info(pf, (struct i40e_queue_regions *)arg);
    break;
  default:
    PMD_DRV_LOG(WARNING, "op type (%d) not supported", op_type);
    ret = -EINVAL;
  }
...
int rte_pmd_i40e_flow_add_del_packet_template(
    uint16_t port, const struct rte_pmd_i40e_pkt_template_conf *conf,
    uint8_t add) {
  struct rte_eth_dev *dev = &rte_eth_devices[port];
  ...
  filter_conf.action.behavior = (enum i40e_fdir_behavior)conf->action.behavior;
...
int rte_pmd_i40e_inset_get(uint16_t port, uint8_t pctype,
  ...
  case INSET_HASH:
    ...
    /* Get field mask */
    mask_reg[0] = i40e_read_rx_ctl(hw, I40E_GLQF_HASH_MSK(0, pctype));
    mask_reg[1] = i40e_read_rx_ctl(hw, I40E_GLQF_HASH_MSK(1, pctype));
    break;
  case INSET_FDIR:
    inset_reg = i40e_read_rx_ctl(hw, I40E_PRTQF_FD_INSET(pctype, 1));
    inset_reg <<= I40E_32_BIT_WIDTH;
    inset_reg |= i40e_read_rx_ctl(hw, I40E_PRTQF_FD_INSET(pctype, 0));
    mask_reg[0] = i40e_read_rx_ctl(hw, I40E_GLQF_FD_MSK(0, pctype));
    mask_reg[1] = i40e_read_rx_ctl(hw, I40E_GLQF_FD_MSK(1, pctype));
    break;
  case INSET_FDIR_FLX:
    inset_reg = i40e_read_rx_ctl(hw, I40E_PRTQF_FD_FLXINSET(pctype));
    mask_reg[0] = i40e_read_rx_ctl(hw, I40E_PRTQF_FD_MSK(pctype, 0));
    mask_reg[1] = i40e_read_rx_ctl(hw, I40E_PRTQF_FD_MSK(pctype, 1));
    break;
  default:
...
int rte_pmd_i40e_inset_set(uint16_t port, uint8_t pctype,
  ...
  for (i = 0; i < 2; i++)
    mask_reg[i] = (inset->mask[i].field_idx << 16) | inset->mask[i].mask;
  ...
  case INSET_FDIR:
    ...
    i40e_check_write_reg(
        ...
        (uint32_t)((inset_reg >> I40E_32_BIT_WIDTH) & UINT32_MAX));
    for (i = 0; i < 2; i++)
      i40e_check_write_global_reg(hw, I40E_GLQF_FD_MSK(i, pctype), mask_reg[i]);
    break;
  case INSET_FDIR_FLX:
    i40e_check_write_reg(hw, I40E_PRTQF_FD_FLXINSET(pctype),
                         (uint32_t)(inset_reg & UINT32_MAX));
    for (i = 0; i < 2; i++)
      i40e_check_write_reg(hw, I40E_PRTQF_FD_MSK(pctype, i), mask_reg[i]);
    break;
  default:
    PMD_DRV_LOG(ERR, "Unsupported input set type.");
    return -EINVAL;
...
int rte_pmd_i40e_get_fdir_info(uint16_t port,
...
int rte_pmd_i40e_get_fdir_stats(uint16_t port,
...
int rte_pmd_i40e_set_gre_key_len(uint16_t port, uint8_t len) {
  struct rte_eth_dev *dev;
  struct i40e_pf *pf;
  struct i40e_hw *hw;
...
int rte_pmd_i40e_set_switch_dev(uint16_t port_id,
