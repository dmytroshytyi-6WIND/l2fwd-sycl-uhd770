  ...
  for (i = 0; i < RTE_PMD_I40E_RX_MAX_BURST;
    ...
    for (j = 0; j < nb_dd; j++) {
      ...
      mb->packet_type = ptype_tbl[(uint8_t)(
          (qword1 & I40E_RXD_QW1_PTYPE_MASK) >> I40E_RXD_QW1_PTYPE_SHIFT)];
...
const uint32_t *i40e_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {/* refers to i40e_rxd_pkt_type_mapping() */
                                    RTE_PTYPE_L2_ETHER,
...
static inline bool get_avx_supported(bool request_avx512) {
  if (request_avx512) {
    ...
    if (rte_vect_get_max_simd_bitwidth() >= RTE_VECT_SIMD_256 &&
      ...
      return true;
#else
      PMD_DRV_LOG(NOTICE, "AVX2 is not supported in build env");
    return false;
#endif
  }
