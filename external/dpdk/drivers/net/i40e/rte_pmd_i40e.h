...
struct rte_pmd_i40e_queue_regions {
  ...
  struct rte_pmd_i40e_queue_region_info region[RTE_PMD_I40E_REGION_MAX_NUM];
};
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_i40e_get_ddp_info(uint8_t *pkg, uint32_t pkg_size, uint8_t *info,
                              uint32_t size,
...
int rte_pmd_i40e_cfg_hash_inset(uint16_t port, uint64_t pctype, uint64_t inset);
 ...
 *   - (0) if cleared.
 */
static inline int rte_pmd_i40e_inset_field_get(uint64_t inset,
                                               uint8_t field_idx) {
  uint8_t bit_idx;
 ...
 *   - (0) if success.
 */
static inline int rte_pmd_i40e_inset_field_set(uint64_t *inset,
                                               uint8_t field_idx) {
  uint8_t bit_idx;
 ...
 *   - (0) if success.
 */
static inline int rte_pmd_i40e_inset_field_clear(uint64_t *inset,
                                                 uint8_t field_idx) {
  uint8_t bit_idx;
