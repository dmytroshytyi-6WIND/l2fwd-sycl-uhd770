...
static __rte_always_inline void i40e_rxq_rearm(struct i40e_rx_queue *rxq) {
  int i;
  uint16_t rx_id;
  ...
  if (cache->len < RTE_I40E_RXQ_REARM_THRESH) {
    ...
    uint32_t req = RTE_I40E_RXQ_REARM_THRESH + (cache->size - cache->len);
    ...
    if (ret == 0) {
      ...
      if (rxq->rxrearm_nb + RTE_I40E_RXQ_REARM_THRESH >= rxq->nb_rx_desc) {
        __m128i dma_addr0;
  ...
  for (i = 0; i < RTE_I40E_RXQ_REARM_THRESH / 8; i++) {
     ...
     */
    const __m512i desc_rd_0_1 = _mm512_permutexvar_epi64(permute_idx, iovas0);
    const __m512i desc_rd_2_3 = _mm512_bsrli_epi128(desc_rd_0_1, 8);

    const __m512i desc_rd_4_5 = _mm512_permutexvar_epi64(permute_idx, iovas1);
     ...
     * enabled.
     */
    const __m512i desc_rd_0_3 =
        _mm512_permutexvar_epi64(permute_idx, iova_addrs);
...
desc_fdir_processing_32b(volatile union i40e_rx_desc *rxdp,
                         struct rte_mbuf **rx_pkts, const uint32_t pkt_idx,
   ...
   */
  const __m256i flexbh_mask =
      _mm256_set_epi32(-1, -1, -1, 3 << 4, -1, -1, -1, 3 << 4);
  const __m256i flexbh_id =
      _mm256_set_epi32(-1, -1, -1, 1 << 4, -1, -1, -1, 1 << 4);

  /* Load descriptor, check for FLEXBH bits, generate a mask for both
   * packets in the register.
   */
  __m256i desc_qw2_0_1 = _mm256_inserti128_si256(
      _mm256_castsi128_si256(desc_qw2_0), desc_qw2_1, 1);
  ...
  __m256i mbuf_flag_mask = _mm256_set_epi32(0, 0, 0, 1 << 13, 0, 0, 0, 1 << 13);
...
_recv_raw_pkts_vec_avx512(struct i40e_rx_queue *rxq, struct rte_mbuf **rx_pkts,
  ...
  const __m256i eop_check =
      _mm256_slli_epi32(dd_check, I40E_RX_DESC_STATUS_EOF_SHIFT);
  ...
  for (i = 0, received = 0; i < nb_pkts;
       i += RTE_I40E_DESCS_PER_LOOP_AVX, rxdp += RTE_I40E_DESCS_PER_LOOP_AVX) {
    ...
    const __m512i desc4_7 =
        _mm512_mask_blend_epi16(0x80808080, raw_desc4_7, len4_7);
    const __m512i desc0_3 =
        _mm512_mask_blend_epi16(0x80808080, raw_desc0_3, len0_3);
    ...
    const __m512i raw_status0_7 =
        _mm512_permutex2var_epi32(desc4_7, status_permute_msk, desc0_3);
    ...
    const __m256i rss_flags =
        _mm256_shuffle_epi8(rss_flags_shuf, rss_fdir_bits);
...
#define FDIR_BLEND_MASK ((1 << 3) | (1 << 7))
      ...
      __m256i tmp0_1 =
          _mm256_blend_epi32(fdir_zero_mask, fdir_mask, FDIR_BLEND_MASK);
      ...
      tmp2_3 = _mm256_blend_epi32(fdir_zero_mask, tmp2_3, FDIR_BLEND_MASK);
      ...
      tmp4_5 = _mm256_blend_epi32(fdir_zero_mask, tmp4_5, FDIR_BLEND_MASK);
      ...
      tmp6_7 = _mm256_blend_epi32(fdir_zero_mask, tmp6_7, FDIR_BLEND_MASK);
    ...
    if (split_packet) {
      const __m128i eop_mask =
          _mm_set1_epi16(1 << I40E_RX_DESC_STATUS_EOF_SHIFT);
      ...
      __m128i eop_shuffle = _mm_set_epi8(
          ...
          0xFF, 0xFF, 0xFF, 0xFF, 8, 0, 10, 2, /* move values to lo 64b */
          12, 4, 14, 6);
 ...
 */
uint16_t i40e_recv_pkts_vec_avx512(void *rx_queue, struct rte_mbuf **rx_pkts,
                                   uint16_t nb_pkts) {
 ...
 * - nb_pkts < RTE_I40E_DESCS_PER_LOOP, just return no packet
 */
static uint16_t i40e_recv_scattered_burst_vec_avx512(void *rx_queue,
  ...
  uint16_t nb_bufs =
      _recv_raw_pkts_vec_avx512(rxq, rx_pkts, nb_pkts, split_flags);
  if (nb_bufs == 0)
    return 0;
...
uint16_t i40e_recv_scattered_pkts_vec_avx512(void *rx_queue,
  ...
  while (nb_pkts > RTE_I40E_VPMD_RX_BURST) {
    uint16_t burst = i40e_recv_scattered_burst_vec_avx512(
        rx_queue, rx_pkts + retval, RTE_I40E_VPMD_RX_BURST);
    retval += burst;
    nb_pkts -= burst;
    if (burst < RTE_I40E_VPMD_RX_BURST)
      return retval;
  }
  return retval + i40e_recv_scattered_burst_vec_avx512(
...
static inline void vtx1(volatile struct i40e_tx_desc *txdp,
                        struct rte_mbuf *pkt, uint64_t flags) {
  uint64_t high_qw =
      (I40E_TX_DESC_DTYPE_DATA | ((uint64_t)flags << I40E_TXD_QW1_CMD_SHIFT) |
...
static inline void vtx(volatile struct i40e_tx_desc *txdp,
...
i40e_xmit_fixed_burst_vec_avx512(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  if (tx_id > txq->tx_next_rs) {
    txq->tx_ring[txq->tx_next_rs].cmd_type_offset_bsz |= rte_cpu_to_le_64(
...
uint16_t i40e_xmit_pkts_vec_avx512(void *tx_queue, struct rte_mbuf **tx_pkts,
                                   uint16_t nb_pkts) {
  uint16_t nb_tx = 0;
  struct i40e_tx_queue *txq = (struct i40e_tx_queue *)tx_queue;
  ...
  while (nb_pkts) {
    ...
    ret = i40e_xmit_fixed_burst_vec_avx512(tx_queue, &tx_pkts[nb_tx], num);
    nb_tx += ret;
    nb_pkts -= ret;
    if (ret < num)
      break;
