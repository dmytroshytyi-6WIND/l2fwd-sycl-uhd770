...
#define CNXK_NIX_L2_OVERHEAD                                                   \
  (RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN + CNXK_NIX_MAX_VTAG_ACT_SIZE)
...
#define CNXK_NIX_TX_MSEG_SG_DWORDS                                             \
  ((RTE_ALIGN_MUL_CEIL(CNXK_NIX_TX_NB_SEG_MAX, 3) / 3) + CNXK_NIX_TX_NB_SEG_MAX)
...
static inline struct cnxk_eth_rxq_sp *cnxk_eth_rxq_to_sp(void *__rxq) {
  return ((struct cnxk_eth_rxq_sp *)__rxq) - 1;
}

static inline struct cnxk_eth_txq_sp *cnxk_eth_txq_to_sp(void *__txq) {
...
__rte_internal int cnxk_nix_inb_mode_set(struct cnxk_eth_dev *dev,
                                         bool use_inl_dev);
...
/* Inlines */
static __rte_always_inline uint64_t cnxk_pktmbuf_detach(struct rte_mbuf *m) {
...
static __rte_always_inline uint64_t cnxk_nix_prefree_seg(struct rte_mbuf *m) {
