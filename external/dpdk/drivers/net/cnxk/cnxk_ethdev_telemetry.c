...
static int ethdev_tel_handle_info(const char *cmd __rte_unused,
  ...
  for (i = 0; i < RTE_MAX_ETHPORTS; i++) {
    ...
    if (eth_dev) {
      ...
      if (dev) {
        ...
        info->dmac_filter_count = dev->dmac_filter_count;
      ...
      for (j = 0; j < ETH_INFO_SZ; j++)
        rte_tel_data_add_array_u64(i_data, eth_info.val[j]);
