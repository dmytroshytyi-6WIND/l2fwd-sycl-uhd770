...
static int cnxk_nix_tm_node_type_get(struct rte_eth_dev *eth_dev,
...
static int cnxk_nix_tm_capa_get(struct rte_eth_dev *eth_dev,
  ...
  cap->dynamic_update_mask =
      RTE_TM_UPDATE_NODE_PARENT_KEEP_LEVEL | RTE_TM_UPDATE_NODE_SUSPEND_RESUME;
...
static int cnxk_nix_tm_level_capa_get(struct rte_eth_dev *eth_dev, uint32_t lvl,
  ...
  if (roc_nix_tm_lvl_is_leaf(nix, lvl)) {
    ...
    if (roc_nix_tm_lvl_have_link_access(nix, lvl))
      cap->nonleaf.stats_mask =
          RTE_TM_STATS_N_PKTS_RED_DROPPED | RTE_TM_STATS_N_BYTES_RED_DROPPED;
...
static int cnxk_nix_tm_node_capa_get(struct rte_eth_dev *eth_dev,
                                     uint32_t node_id,
...
static int cnxk_nix_tm_shaper_profile_add(struct rte_eth_dev *eth_dev,
                                          uint32_t id,
  ...
  rte_memcpy(&profile->params, params, sizeof(struct rte_tm_shaper_params));
...
static int cnxk_nix_tm_shaper_profile_delete(struct rte_eth_dev *eth_dev,
                                             uint32_t profile_id,
                                             struct rte_tm_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_nix *nix = &dev->nix;
...
static int cnxk_nix_tm_node_add(struct rte_eth_dev *eth_dev, uint32_t node_id,
  ...
  node = rte_zmalloc("cnxk_nix_tm_node", sizeof(struct cnxk_nix_tm_node), 0);
  if (!node)
    return -ENOMEM;
  ...
  if (!roc_nix_tm_lvl_is_leaf(nix, lvl) &&
      ((profile && profile->pkt_mode) ||
       (params->nonleaf.wfq_weight_mode && params->nonleaf.n_sp_priorities &&
...
static int cnxk_nix_tm_node_delete(struct rte_eth_dev *eth_dev,
                                   uint32_t node_id,
                                   struct rte_tm_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_nix *nix = &dev->nix;
...
static int cnxk_nix_tm_node_suspend(struct rte_eth_dev *eth_dev,
                                    uint32_t node_id,
                                    struct rte_tm_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  int rc;
...
static int cnxk_nix_tm_node_resume(struct rte_eth_dev *eth_dev,
                                   uint32_t node_id,
                                   struct rte_tm_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  int rc;
...
static int cnxk_nix_tm_hierarchy_commit(struct rte_eth_dev *eth_dev,
...
static int cnxk_nix_tm_node_shaper_update(struct rte_eth_dev *eth_dev,
                                          uint32_t node_id, uint32_t profile_id,
                                          struct rte_tm_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_nix_tm_shaper_profile *profile;
  struct roc_nix *nix = &dev->nix;
...
static int cnxk_nix_tm_node_parent_update(struct rte_eth_dev *eth_dev,
                                          uint32_t node_id,
  ...
  rc = roc_nix_tm_node_parent_update(nix, node_id, new_parent_id, priority,
                                     weight);
  if (rc) {
    error->type = roc_nix_tm_err_to_rte_err(rc);
    error->message = roc_error_msg_get(rc);
...
static int cnxk_nix_tm_node_stats_read(struct rte_eth_dev *eth_dev,
                                       uint32_t node_id,
...
int cnxk_nix_tm_mark_vlan_dei(struct rte_eth_dev *eth_dev, int mark_green,
  ...
  rc = roc_nix_tm_mark_config(roc_nix, ROC_NIX_TM_MARK_VLAN_DEI, mark_yellow,
                              mark_red);
  if (rc) {
    error->type = roc_nix_tm_err_to_rte_err(rc);
    error->message = roc_error_msg_get(rc);
...
int cnxk_nix_tm_mark_ip_ecn(struct rte_eth_dev *eth_dev, int mark_green,
  ...
  rc = roc_nix_tm_mark_config(roc_nix, ROC_NIX_TM_MARK_IPV4_ECN, mark_yellow,
                              mark_red);
  if (rc < 0)
    goto exit;

  rc = roc_nix_tm_mark_config(roc_nix, ROC_NIX_TM_MARK_IPV6_ECN, mark_yellow,
                              mark_red);
exit:
  if (rc < 0) {
    error->type = roc_nix_tm_err_to_rte_err(rc);
...
int cnxk_nix_tm_mark_ip_dscp(struct rte_eth_dev *eth_dev, int mark_green,
  ...
  rc = roc_nix_tm_mark_config(roc_nix, ROC_NIX_TM_MARK_IPV4_DSCP, mark_yellow,
                              mark_red);
  if (rc < 0)
    goto exit;

  rc = roc_nix_tm_mark_config(roc_nix, ROC_NIX_TM_MARK_IPV6_DSCP, mark_yellow,
                              mark_red);
exit:
  if (rc < 0) {
    error->type = roc_nix_tm_err_to_rte_err(rc);
...
int cnxk_nix_tm_ops_get(struct rte_eth_dev *eth_dev __rte_unused, void *arg) {
  if (!arg)
    return -EINVAL;
