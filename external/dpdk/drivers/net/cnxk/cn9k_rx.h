...
nix_cqe_xtract_mseg(const union nix_rx_parse_u *rx, struct rte_mbuf *mbuf,
  ...
  eol = ((const rte_iova_t *)(rx + 1) + ((rx->cn9k.desc_sizem1 + 1) << 1));
...
static inline int ipsec_antireplay_check(struct roc_onf_ipsec_inb_sa *sa,
                                         struct cn9k_inb_priv_data *priv,
                                         uintptr_t data, uint32_t win_sz) {
  ...
  if (!esn) {
    ...
    seqh = rte_be_to_cpu_32(*((uint32_t *)(data + IPSEC_SQ_HI_IDX)));
...
nix_rx_sec_mbuf_err_update(const union nix_rx_parse_u *rx, uint16_t res,
  ...
  case ROC_IE_ON_UCC_PADDING_INVALID:
    ...
    data_off += ROC_ONF_IPSEC_INB_SPI_SEQ_SZ + ROC_ONF_IPSEC_INB_MAX_L2_SZ;
    ol_flags = RTE_MBUF_F_RX_SEC_OFFLOAD | RTE_MBUF_F_RX_SEC_OFFLOAD_FAILED;
    break;
...
cn9k_nix_cqe_to_mbuf(const struct nix_cqe_hdr_s *cq, const uint32_t tag,
  ...
  if ((flag & NIX_RX_OFFLOAD_SECURITY_F) &&
    ...
    ol_flags |= nix_rx_sec_mbuf_update(cq, mbuf, sa_base, &val, &len);
...
skip_parse:
  ...
  if (flag & NIX_RX_OFFLOAD_MARK_UPDATE_F)
    ol_flags = nix_update_match_id(rx->cn9k.match_id, ol_flags, mbuf);
...
static inline uint16_t nix_rx_nb_pkts(struct cn9k_eth_rxq *rxq,
...
cn9k_nix_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t pkts,
  ...
  while (packets < nb_pkts) {
    ...
    cn9k_nix_cqe_to_mbuf(cq, cq->tag, mbuf, lookup_mem, mbuf_init, flags);
...
cn9k_nix_recv_pkts_vector(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (packets < pkts) {
    ...
    if (flags & NIX_RX_OFFLOAD_MARK_UPDATE_F) {
      ol_flags0 = nix_update_match_id(*(uint16_t *)(cq0 + CQE_SZ(0) + 38),
                                      ol_flags0, mbuf0);
      ol_flags1 = nix_update_match_id(*(uint16_t *)(cq0 + CQE_SZ(1) + 38),
                                      ol_flags1, mbuf1);
      ol_flags2 = nix_update_match_id(*(uint16_t *)(cq0 + CQE_SZ(2) + 38),
                                      ol_flags2, mbuf2);
      ol_flags3 = nix_update_match_id(*(uint16_t *)(cq0 + CQE_SZ(3) + 38),
                                      ol_flags3, mbuf3);
    }
    ...
    if (flags & NIX_RX_OFFLOAD_TSTAMP_F) {
      ...
      *cnxk_nix_timestamp_dynfield(mbuf0, rxq->tstamp) = ts[0];
      *cnxk_nix_timestamp_dynfield(mbuf1, rxq->tstamp) = ts[1];
      *cnxk_nix_timestamp_dynfield(mbuf2, rxq->tstamp) = ts[2];
      *cnxk_nix_timestamp_dynfield(mbuf3, rxq->tstamp) = ts[3];
      ...
      if (res) {
        ...
        rxq->tstamp->rx_tstamp = ts[31 - __builtin_clz(res)];
      }
    }
    ...
    if (flags & NIX_RX_MULTI_SEG_F) {
       ...
       */
      nix_cqe_xtract_mseg((union nix_rx_parse_u *)(cq0 + CQE_SZ(0) + 8), mbuf0,
                          mbuf_initializer, flags);
      nix_cqe_xtract_mseg((union nix_rx_parse_u *)(cq0 + CQE_SZ(1) + 8), mbuf1,
                          mbuf_initializer, flags);
      nix_cqe_xtract_mseg((union nix_rx_parse_u *)(cq0 + CQE_SZ(2) + 8), mbuf2,
                          mbuf_initializer, flags);
      nix_cqe_xtract_mseg((union nix_rx_parse_u *)(cq0 + CQE_SZ(3) + 8), mbuf3,
                          mbuf_initializer, flags);
    } else {
  ...
  if (unlikely(pkts_left))
    packets +=
        cn9k_nix_recv_pkts(rx_queue, &rx_pkts[packets], pkts_left, flags);
...
static inline uint16_t cn9k_nix_recv_pkts_vector(void *rx_queue,
...
#define NIX_RX_RECV_VEC(fn, flags)                                             \
  uint16_t __rte_noinline __rte_hot fn(                                        \
      void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t pkts) {              \
    return cn9k_nix_recv_pkts_vector(rx_queue, rx_pkts, pkts, (flags));        \
  }
