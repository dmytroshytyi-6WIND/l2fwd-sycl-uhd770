 ...
 *
 */
static void nix_create_non_tunnel_ptype_array(uint16_t *ptype) {
  uint8_t lb, lc, ld, le;
  uint16_t val;
  uint32_t idx;
...
static void nix_create_tunnel_ptype_array(uint16_t *ptype) {
  uint8_t lf, lg, lh;
  uint16_t val;
  uint32_t idx;
...
static void nix_create_rx_ol_flags_array(void *mem) {
  ...
  for (idx = 0; idx < BIT(ERRCODE_ERRLEN_WIDTH); idx++) {
    ...
    case NPC_ERRLEV_RE:
      ...
      break;
    case NPC_ERRLEV_LC:
      if (errcode == NPC_EC_OIP4_CSUM || errcode == NPC_EC_IP_FRAG_OFFSET_1) {
        val |= RTE_MBUF_F_RX_IP_CKSUM_BAD;
...
int cnxk_nix_lookup_mem_sa_base_set(struct cnxk_eth_dev *dev) {
  ...
  sa_w =
      plt_log2_u32(dev->nix.ipsec_in_max_spi + 1 - dev->nix.ipsec_in_min_spi);
