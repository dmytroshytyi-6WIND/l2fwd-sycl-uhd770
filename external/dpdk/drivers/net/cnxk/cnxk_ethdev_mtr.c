...
#define NIX_BPF_STATS_MASK_ALL                                                 \
  {                                                                            \
    ROC_NIX_BPF_GREEN_PKT_F_PASS | ROC_NIX_BPF_GREEN_OCTS_F_PASS |             \
        ROC_NIX_BPF_GREEN_PKT_F_DROP | ROC_NIX_BPF_GREEN_OCTS_F_DROP |         \
        ROC_NIX_BPF_YELLOW_PKT_F_PASS | ROC_NIX_BPF_YELLOW_OCTS_F_PASS |       \
        ROC_NIX_BPF_YELLOW_PKT_F_DROP | ROC_NIX_BPF_YELLOW_OCTS_F_DROP |       \
        ROC_NIX_BPF_RED_PKT_F_PASS | ROC_NIX_BPF_RED_OCTS_F_PASS |             \
        ROC_NIX_BPF_RED_PKT_F_DROP | ROC_NIX_BPF_RED_OCTS_F_DROP               \
  }

static const enum roc_nix_bpf_level_flag lvl_map[] = {
    ROC_NIX_BPF_LEVEL_F_LEAF, ROC_NIX_BPF_LEVEL_F_MID, ROC_NIX_BPF_LEVEL_F_TOP};
...
static struct rte_mtr_capabilities mtr_capa = {
    ...
    .stats_mask = RTE_MTR_STATS_N_PKTS_GREEN | RTE_MTR_STATS_N_PKTS_YELLOW |
                  ...
                  RTE_MTR_STATS_N_BYTES_GREEN | RTE_MTR_STATS_N_BYTES_YELLOW |
                  RTE_MTR_STATS_N_BYTES_RED | RTE_MTR_STATS_N_BYTES_DROPPED};

static struct cnxk_meter_node *nix_mtr_find(struct cnxk_eth_dev *dev,
                                            uint32_t meter_id) {
...
static int nix_mtr_profile_validate(struct cnxk_eth_dev *dev,
                                    uint32_t profile_id,
  ...
  case RTE_MTR_SRTCM_RFC2697:
    if (profile->srtcm_rfc2697.cir > mtr_capa.meter_rate_max)
      rc = -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_PROFILE,
                              NULL, "CIR exceeds max meter rate");

    if (profile->srtcm_rfc2697.cbs > NIX_BPF_BURST_MAX)
      rc = -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_PROFILE,
                              NULL, "CBS exceeds max meter burst size");

    if (profile->srtcm_rfc2697.ebs > NIX_BPF_BURST_MAX)
      rc = -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_PROFILE,
                              NULL, "EBS exceeds max meter burst size");
    break;
  ...
  case RTE_MTR_TRTCM_RFC2698:
    ...
    if (profile->trtcm_rfc2698.pir > mtr_capa.meter_rate_max)
      rc = -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_PROFILE,
                              NULL, "PIR exceeds max meter rate");
    ...
    if (profile->trtcm_rfc2698.pbs > NIX_BPF_BURST_MAX)
      rc = -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_PROFILE,
                              NULL, "PBS exceeds max meter burst size");
    break;
...
static int cnxk_nix_mtr_capabilities_get(struct rte_eth_dev *dev,
  ...
  mtr_capa.meter_rate_max = NIX_BPF_RATE(time_unit, NIX_BPF_MAX_RATE_EXPONENT,
...
static int cnxk_nix_mtr_profile_add(struct rte_eth_dev *eth_dev,
                                    uint32_t profile_id,
                                    struct rte_mtr_meter_profile *profile,
                                    struct rte_mtr_error *error) {
...
static int cnxk_nix_mtr_profile_delete(struct rte_eth_dev *eth_dev,
                                       uint32_t profile_id,
                                       struct rte_mtr_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct cnxk_mtr_profile_node *fmp;
  ...
  if (fmp == NULL)
    return -rte_mtr_error_set(error, ENOENT,
                              RTE_MTR_ERROR_TYPE_METER_PROFILE_ID, &profile_id,
...
static int update_mtr_err(uint32_t act_color, struct rte_mtr_error *error,
                          bool action) {
  const char *str;
  switch (act_color) {
...
static void cnxk_fill_policy_actions(struct cnxk_mtr_policy_node *fmp,
                                     struct rte_mtr_meter_policy_params *policy)
  ...
  for (i = 0; i < RTE_COLORS; i++) {
    if (policy->actions[i]) {
      for (action = policy->actions[i];
           action->type != RTE_FLOW_ACTION_TYPE_END; action++) {
        if (action->type == RTE_FLOW_ACTION_TYPE_METER) {
          fmp->actions[i].action_fate = action->type;
...
static int cnxk_nix_mtr_policy_add(struct rte_eth_dev *eth_dev,
                                   uint32_t policy_id,
                                   struct rte_mtr_meter_policy_params *policy,
                                   struct rte_mtr_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
...
static int cnxk_nix_mtr_policy_delete(struct rte_eth_dev *eth_dev,
                                      uint32_t policy_id,
                                      struct rte_mtr_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct cnxk_mtr_policy_node *fmp;
...
static int cnxk_nix_mtr_create(struct rte_eth_dev *eth_dev, uint32_t mtr_id,
  ...
  if (params == NULL)
    return -rte_mtr_error_set(error, ENOENT, RTE_MTR_ERROR_TYPE_MTR_PARAMS,
                              NULL, "Meter params are invalid.");

  profile = nix_mtr_profile_find(dev, params->meter_profile_id);
  if (profile == NULL)
    return -rte_mtr_error_set(
        error, ENOENT, RTE_MTR_ERROR_TYPE_METER_PROFILE_ID,
        &params->meter_profile_id, "Meter profile is invalid.");
  ...
  if (mtr == NULL) {
    return -rte_mtr_error_set(error, ENOMEM, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Meter memory alloc failed.");
  }
  ...
  if (params->dscp_table) {
    mtr->params.dscp_table = plt_zmalloc(ROC_NIX_BPF_PRE_COLOR_MAX, ROC_ALIGN);
...
static int cnxk_nix_mtr_destroy(struct rte_eth_dev *eth_dev, uint32_t mtr_id,
                                struct rte_mtr_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  ...
  mtr = nix_mtr_find(dev, mtr_id);
  if (mtr == NULL) {
    return -rte_mtr_error_set(error, ENOENT, RTE_MTR_ERROR_TYPE_MTR_ID, &mtr_id,
                              "Meter id is invalid.");
  }

  if (mtr->ref_cnt) {
    return -rte_mtr_error_set(error, EADDRINUSE, RTE_MTR_ERROR_TYPE_MTR_ID,
  ...
  case ROC_NIX_BPF_LEVEL_F_LEAF:
    if (mtr->is_next) {
      rc = roc_nix_bpf_connect(nix, ROC_NIX_BPF_LEVEL_F_LEAF, mtr->bpf_id,
                               ROC_NIX_BPF_ID_INVALID);
    }
    break;
  case ROC_NIX_BPF_LEVEL_F_MID:
    while ((mtr->prev_cnt) + 1) {
      mid_mtr = nix_mtr_find(dev, mtr->prev_id[mtr->prev_cnt]);
      rc = roc_nix_bpf_connect(nix, ROC_NIX_BPF_LEVEL_F_LEAF, mid_mtr->bpf_id,
                               ROC_NIX_BPF_ID_INVALID);
      mtr->prev_cnt--;
    }
    if (mtr->is_next) {
      rc = roc_nix_bpf_connect(nix, ROC_NIX_BPF_LEVEL_F_MID, mtr->bpf_id,
                               ROC_NIX_BPF_ID_INVALID);
    }
    break;
  case ROC_NIX_BPF_LEVEL_F_TOP:
    while (mtr->prev_cnt) {
      top_mtr = nix_mtr_find(dev, mtr->prev_id[mtr->prev_cnt]);
      rc = roc_nix_bpf_connect(nix, ROC_NIX_BPF_LEVEL_F_MID, top_mtr->bpf_id,
                               ROC_NIX_BPF_ID_INVALID);
      mtr->prev_cnt--;
    }
    break;
...
static int cnxk_nix_mtr_enable(struct rte_eth_dev *eth_dev, uint32_t mtr_id,
                               struct rte_mtr_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_nix *nix = &dev->nix;
  struct cnxk_meter_node *mtr;
...
static int cnxk_nix_mtr_disable(struct rte_eth_dev *eth_dev, uint32_t mtr_id,
                                struct rte_mtr_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_nix *nix = &dev->nix;
  struct cnxk_meter_node *mtr;
...
static int cnxk_nix_mtr_dscp_table_update(struct rte_eth_dev *eth_dev,
                                          uint32_t mtr_id,
                                          enum rte_color *dscp_table,
                                          struct rte_mtr_error *error) {
  enum roc_nix_bpf_color nix_dscp_tbl[ROC_NIX_BPF_PRE_COLOR_MAX];
  enum roc_nix_bpf_color color_map[] = {
      ROC_NIX_BPF_COLOR_GREEN, ROC_NIX_BPF_COLOR_YELLOW, ROC_NIX_BPF_COLOR_RED};
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  ...
  rc = roc_nix_bpf_pre_color_tbl_setup(nix, mtr->bpf_id, lvl_map[mtr->level],
                                       &table);
  if (rc) {
    rte_mtr_error_set(error, rc, RTE_MTR_ERROR_TYPE_UNSPECIFIED, NULL, NULL);
    goto exit;
  }
...
static int cnxk_nix_mtr_stats_update(struct rte_eth_dev *eth_dev,
                                     uint32_t mtr_id, uint64_t stats_mask,
                                     struct rte_mtr_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct cnxk_meter_node *mtr;
  ...
  if (!stats_mask)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_MTR_PARAMS,
                              NULL, "no bit is set to stats mask");
...
static int cnxk_nix_mtr_stats_read(struct rte_eth_dev *eth_dev, uint32_t mtr_id,
  ...
  green_octs_pass = roc_nix_bpf_stats_to_idx(ROC_NIX_BPF_GREEN_OCTS_F_PASS);
  green_pkt_drop = roc_nix_bpf_stats_to_idx(ROC_NIX_BPF_GREEN_PKT_F_DROP);
  green_octs_drop = roc_nix_bpf_stats_to_idx(ROC_NIX_BPF_GREEN_OCTS_F_DROP);
  yellow_pkt_pass = roc_nix_bpf_stats_to_idx(ROC_NIX_BPF_YELLOW_PKT_F_PASS);
  yellow_octs_pass = roc_nix_bpf_stats_to_idx(ROC_NIX_BPF_YELLOW_OCTS_F_PASS);
  yellow_pkt_drop = roc_nix_bpf_stats_to_idx(ROC_NIX_BPF_YELLOW_PKT_F_DROP);
  yellow_octs_drop = roc_nix_bpf_stats_to_idx(ROC_NIX_BPF_YELLOW_OCTS_F_DROP);
  ...
  if (clear) {
    rc = roc_nix_bpf_stats_reset(nix, mtr->bpf_id, mask, lvl_map[mtr->level]);
    if (rc) {
      rte_mtr_error_set(error, rc, RTE_MTR_ERROR_TYPE_UNSPECIFIED, NULL, NULL);
      goto exit;
    }
...
int nix_mtr_validate(struct rte_eth_dev *eth_dev, uint32_t id) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct cnxk_mtr_profile_node *profile;
  struct cnxk_mtr_policy_node *policy;
  struct cnxk_meter_node *mtr;
...
struct cnxk_meter_node *nix_get_mtr(struct rte_eth_dev *eth_dev, uint32_t id) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct cnxk_meter_node *mtr;
...
static void nix_mtr_config_map(struct cnxk_meter_node *mtr,
                               struct roc_nix_bpf_cfg *cfg) {
  enum roc_nix_bpf_algo alg_map[] = {
      ROC_NIX_BPF_ALGO_NONE, ROC_NIX_BPF_ALGO_2697, ROC_NIX_BPF_ALGO_2698,
  ...
  if (policy->actions[RTE_COLOR_GREEN].action_fate == RTE_FLOW_ACTION_TYPE_DROP)
  ...
  if (policy->actions[RTE_COLOR_RED].action_fate == RTE_FLOW_ACTION_TYPE_DROP)
    cfg->action[ROC_NIX_BPF_COLOR_RED] = ROC_NIX_BPF_ACTION_DROP;
}

static void nix_dscp_table_map(struct cnxk_meter_node *mtr,
  ...
  for (i = 0; i < ROC_NIX_BPF_LEVEL_MAX; i++) {
    ...
    if (base_mtr) {
      ...
      if (base_mtr->is_next) {
        ...
        if (next_mtr) {
          if (!base_mtr->is_used) {
            rc = roc_nix_bpf_connect(nix, lvl_flag, base_mtr->bpf_id,
                                     next_mtr->bpf_id);
            if (rc)
              return rc;
          }
  ...
  for (i = 0; i < ROC_NIX_BPF_LEVEL_MAX; i++) {
    if (num_mtr[i]) {
      if (!mtr[i]->is_used) {
        ...
        rc = roc_nix_bpf_config(nix, mtr[i]->bpf_id, lvl_map[mtr[i]->level],
                                &cfg);
...
int nix_mtr_color_action_validate(struct rte_eth_dev *eth_dev, uint32_t id,
  ...
  uint32_t action_fate_green = policy->actions[RTE_COLOR_GREEN].action_fate;
  uint32_t action_fate_yellow = policy->actions[RTE_COLOR_YELLOW].action_fate;
  ...
  if (next_mtr_id != 0xffff) {
    ...
    case 0:
      ...
      if (mtr->level == ROC_NIX_BPF_LEVEL_IDX_INVALID) {
        ...
        nix_mtr_chain_update(eth_dev, cur_mtr_id, -1, next_mtr_id);
      } else {
        if (mtr->level == 0)
          mtr->is_used = true;
        else
    ...
    case 1:
      ...
      if (mtr->level == ROC_NIX_BPF_LEVEL_IDX_INVALID) {
        ...
        prev_mtr_id = id;
        nix_mtr_chain_update(eth_dev, cur_mtr_id, prev_mtr_id, next_mtr_id);
      } else {
        if (mtr->level == 1) {
          mtr->prev_cnt++;
          prev_mtr_id = id;
    ...
    case 1:
      ...
      if (mtr->level == ROC_NIX_BPF_LEVEL_IDX_INVALID) {
        nix_mtr_level_update(eth_dev, cur_mtr_id, 1);
        prev_mtr_id = id;
        nix_mtr_chain_update(eth_dev, cur_mtr_id, prev_mtr_id, -1);
      } else {
        if (mtr->level == 1) {
          mtr->prev_cnt++;
          prev_mtr_id = id;
