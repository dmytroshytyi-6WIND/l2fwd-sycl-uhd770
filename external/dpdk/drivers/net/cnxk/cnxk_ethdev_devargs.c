...
static int parse_outb_nb_desc(const char *key, const char *value,
                              void *extra_args) {
  RTE_SET_USED(key);
  uint32_t val;
...
static int parse_outb_nb_crypto_qs(const char *key, const char *value,
                                   void *extra_args) {
  RTE_SET_USED(key);
  uint32_t val;
...
static int parse_ipsec_in_spi_range(const char *key, const char *value,
                                    void *extra_args) {
  RTE_SET_USED(key);
  uint32_t val;
...
static int parse_ipsec_out_max_sa(const char *key, const char *value,
                                  void *extra_args) {
  RTE_SET_USED(key);
  uint32_t val;
...
static int parse_flow_max_priority(const char *key, const char *value,
                                   void *extra_args) {
  RTE_SET_USED(key);
  uint16_t val;
...
static int parse_flow_prealloc_size(const char *key, const char *value,
                                    void *extra_args) {
  RTE_SET_USED(key);
  uint16_t val;
...
static int parse_reta_size(const char *key, const char *value,
                           void *extra_args) {
  RTE_SET_USED(key);
  uint32_t val;
...
static int parse_pre_l2_hdr_info(const char *key, const char *value,
                                 void *extra_args) {
...
static int parse_flag(const char *key, const char *value, void *extra_args) {
  RTE_SET_USED(key);
...
static int parse_sqb_count(const char *key, const char *value,
                           void *extra_args) {
  RTE_SET_USED(key);
  uint32_t val;
...
static int parse_switch_header_type(const char *key, const char *value,
                                    void *extra_args) {
  RTE_SET_USED(key);
...
static int parse_sdp_channel_mask(const char *key, const char *value,
                                  void *extra_args) {
  RTE_SET_USED(key);
...
int cnxk_ethdev_parse_devargs(struct rte_devargs *devargs,
  ...
  rte_kvargs_process(kvlist, CNXK_RSS_RETA_SIZE, &parse_reta_size, &reta_sz);
  rte_kvargs_process(kvlist, CNXK_SCL_ENABLE, &parse_flag, &scalar_enable);
  rte_kvargs_process(kvlist, CNXK_MAX_SQB_COUNT, &parse_sqb_count, &sqb_count);
  rte_kvargs_process(kvlist, CNXK_FLOW_PREALLOC_SIZE, &parse_flow_prealloc_size,
                     &flow_prealloc_size);
  rte_kvargs_process(kvlist, CNXK_FLOW_MAX_PRIORITY, &parse_flow_max_priority,
                     &flow_max_priority);
  rte_kvargs_process(kvlist, CNXK_SWITCH_HEADER_TYPE, &parse_switch_header_type,
                     &switch_header_type);
  rte_kvargs_process(kvlist, CNXK_RSS_TAG_AS_XOR, &parse_flag, &rss_tag_as_xor);
  rte_kvargs_process(kvlist, CNXK_LOCK_RX_CTX, &parse_flag, &lock_rx_ctx);
  rte_kvargs_process(kvlist, CNXK_IPSEC_IN_MIN_SPI, &parse_ipsec_in_spi_range,
                     &ipsec_in_min_spi);
  rte_kvargs_process(kvlist, CNXK_IPSEC_IN_MAX_SPI, &parse_ipsec_in_spi_range,
                     &ipsec_in_max_spi);
  rte_kvargs_process(kvlist, CNXK_IPSEC_OUT_MAX_SA, &parse_ipsec_out_max_sa,
                     &ipsec_out_max_sa);
  rte_kvargs_process(kvlist, CNXK_OUTB_NB_DESC, &parse_outb_nb_desc,
                     &outb_nb_desc);
  rte_kvargs_process(kvlist, CNXK_OUTB_NB_CRYPTO_QS, &parse_outb_nb_crypto_qs,
                     &outb_nb_crypto_qs);
  rte_kvargs_process(kvlist, CNXK_NO_INL_DEV, &parse_flag, &no_inl_dev);
  rte_kvargs_process(kvlist, CNXK_SDP_CHANNEL_MASK, &parse_sdp_channel_mask,
                     &sdp_chan);
  rte_kvargs_process(kvlist, CNXK_FLOW_PRE_L2_INFO, &parse_pre_l2_hdr_info,
                     &pre_l2_info);
...
RTE_PMD_REGISTER_PARAM_STRING(
    net_cnxk, CNXK_RSS_RETA_SIZE
    "=<64|128|256>" CNXK_SCL_ENABLE "=1" CNXK_MAX_SQB_COUNT
    "=<8-512>" CNXK_FLOW_PREALLOC_SIZE "=<1-32>" CNXK_FLOW_MAX_PRIORITY
    "=<1-32>" CNXK_SWITCH_HEADER_TYPE
    "=<higig2|dsa|chlen90b>" CNXK_RSS_TAG_AS_XOR "=1" CNXK_IPSEC_IN_MAX_SPI
    "=<1-65535>" CNXK_OUTB_NB_DESC "=<1-65535>" CNXK_FLOW_PRE_L2_INFO
    "=<0-255>/<1-255>/<0-1>" CNXK_OUTB_NB_CRYPTO_QS "=<1-64>" CNXK_NO_INL_DEV
    "=0" CNXK_SDP_CHANNEL_MASK "=<1-4095>/<1-4095>");
