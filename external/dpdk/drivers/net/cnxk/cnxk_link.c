...
static inline int nix_wait_for_link_cfg(struct cnxk_eth_dev *dev) {
  uint16_t wait = 1000;
...
static void nix_link_status_print(struct rte_eth_dev *eth_dev,
                                  struct rte_eth_link *link) {
  if (link && link->link_status)
    plt_info("Port %d: Link Up - speed %u Mbps - %s",
             (int)(eth_dev->data->port_id), (uint32_t)link->link_speed,
