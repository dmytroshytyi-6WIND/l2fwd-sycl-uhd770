...
int cnxk_nix_info_get(struct rte_eth_dev *eth_dev,
  ...
  devinfo->max_mtu =
      devinfo->max_rx_pktlen - (RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN);
  ...
  devinfo->rx_desc_lim.nb_max = RTE_ALIGN_MUL_FLOOR(devinfo->rx_desc_lim.nb_max,
...
int cnxk_nix_rx_burst_mode_get(struct rte_eth_dev *eth_dev, uint16_t queue_id,
  ...
  for (i = 0; i < RTE_DIM(rx_offload_map); i++) {
    if (dev->rx_offloads & rx_offload_map[i].flags) {
      rc = rte_strscpy(mode->info + bytes, rx_offload_map[i].output,
                       str_size - bytes);
      if (rc < 0)
        goto done;
...
int cnxk_nix_tx_burst_mode_get(struct rte_eth_dev *eth_dev, uint16_t queue_id,
  ...
  for (i = 0; i < RTE_DIM(tx_offload_map); i++) {
    if (dev->tx_offloads & tx_offload_map[i].flags) {
      rc = rte_strscpy(mode->info + bytes, tx_offload_map[i].output,
                       str_size - bytes);
      if (rc < 0)
        goto done;
...
int cnxk_nix_flow_ctrl_get(struct rte_eth_dev *eth_dev,
  ...
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  enum rte_eth_fc_mode mode_map[] = {RTE_ETH_FC_NONE, RTE_ETH_FC_RX_PAUSE,
...
static int nix_fc_cq_config_set(struct cnxk_eth_dev *dev, uint16_t qid,
                                bool enable) {
  struct roc_nix *nix = &dev->nix;
...
int cnxk_nix_flow_ctrl_set(struct rte_eth_dev *eth_dev,
  ...
  if (fc->tx_pause ^ tx_pause) {
    ...
    for (i = 0; i < data->nb_rx_queues; i++) {
      ...
      memset(&fc_cfg, 0, sizeof(struct roc_nix_fc_cfg));
      rxq = ((struct cnxk_eth_rxq_sp *)data->rx_queues[i]) - 1;
  ...
  if (fc->rx_pause ^ rx_pause) {
    for (i = 0; i < data->nb_tx_queues; i++) {
      ...
      memset(&fc_cfg, 0, sizeof(struct roc_nix_fc_cfg));
      txq = ((struct cnxk_eth_txq_sp *)data->tx_queues[i]) - 1;
...
int cnxk_nix_priority_flow_ctrl_queue_info_get(
    struct rte_eth_dev *eth_dev, struct rte_eth_pfc_queue_info *pfc_info) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
...
int cnxk_nix_priority_flow_ctrl_queue_config(
...
int cnxk_nix_mtu_set(struct rte_eth_dev *eth_dev, uint16_t mtu) {
  ...
  if ((frame_size - RTE_ETHER_CRC_LEN) > ((uint32_t)roc_nix_max_pkt_len(nix))) {
  ...
  if (rc) {
    ...
    roc_nix_mac_mtu_set(nix, old_frame_size - RTE_ETHER_CRC_LEN);
...
int cnxk_nix_allmulticast_enable(struct rte_eth_dev *eth_dev) {
  ...
  return roc_nix_npc_mcast_config(&dev->nix, true, eth_dev->data->promiscuous);
}

int cnxk_nix_allmulticast_disable(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);

  return roc_nix_npc_mcast_config(&dev->nix, false, eth_dev->data->promiscuous);
}
...
static inline int nix_offset_has_packet(uint32_t head, uint32_t tail,
...
int cnxk_nix_dev_get_reg(struct rte_eth_dev *eth_dev,
  ...
  if (!regs->length || regs->length == (uint32_t)roc_nix_lf_get_reg_count(nix))
...
int cnxk_nix_rss_hash_update(struct rte_eth_dev *eth_dev,
  ...
  if (rss_conf->rss_key && rss_conf->rss_key_len != ROC_NIX_RSS_KEY_LEN) {
    plt_err("Hash key size mismatch %d vs %d", rss_conf->rss_key_len,
            ROC_NIX_RSS_KEY_LEN);
    goto fail;
  }
  ...
  if (rss_hash_level)
    rss_hash_level -= 1;
  flowkey_cfg = cnxk_rss_ethdev_to_nix(dev, rss_conf->rss_hf, rss_hash_level);
...
int cnxk_nix_mc_addr_list_configure(struct rte_eth_dev *eth_dev,
  ...
  for (i = 0; i < dev->max_mac_entries; i++) {
    if (rte_is_multicast_ether_addr(&data->mac_addrs[i])) {
      ...
      if (rc) {
        plt_err("Failed to flush mcast address, rc=%d", rc);
        return rc;
      }

      dev->dmac_filter_count--;
      /* Update address in NIC data structure */
      rte_ether_addr_copy(&null_mac_addr, &data->mac_addrs[i]);
    }
  }
  ...
  for (i = 0; i < nb_mc_addr; i++) {
    ...
    if (index < 0) {
      plt_err("Failed to add mcast mac address, rc=%d", index);
      return index;
    }
