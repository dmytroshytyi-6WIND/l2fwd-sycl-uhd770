...
static int npc_rss_action_validate(struct rte_eth_dev *eth_dev,
...
static void npc_rss_flowkey_get(struct cnxk_eth_dev *eth_dev,
...
static int cnxk_map_actions(struct rte_eth_dev *eth_dev,
                            const struct rte_flow_attr *attr,
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_PORT_ID:
      ...
      if (rte_eth_dev_get_name_by_port(port_act->id, if_name)) {
      ...
      rc = roc_npc_validate_portid_action(roc_npc_src, roc_npc_dst);
...
static int cnxk_map_flow_data(struct rte_eth_dev *eth_dev,
                              const struct rte_flow_attr *attr,
  ...
  return cnxk_map_actions(eth_dev, attr, actions, in_actions, flowkey_cfg);
}

static int cnxk_flow_validate(struct rte_eth_dev *eth_dev,
                              const struct rte_flow_attr *attr,
                              const struct rte_flow_item pattern[],
                              const struct rte_flow_action actions[],
  ...
  rc = cnxk_map_flow_data(eth_dev, attr, pattern, actions, &in_attr, in_pattern,
                          in_actions, &flowkey_cfg);
  if (rc) {
...
struct roc_npc_flow *cnxk_flow_create(struct rte_eth_dev *eth_dev,
                                      const struct rte_flow_attr *attr,
                                      const struct rte_flow_item pattern[],
                                      const struct rte_flow_action actions[],
                                      struct rte_flow_error *error) {
  ...
  flow = roc_npc_flow_create(npc, &in_attr, in_pattern, in_actions, &errcode);
  if (errcode != 0) {
...
int cnxk_flow_destroy(struct rte_eth_dev *eth_dev, struct roc_npc_flow *flow,
                      struct rte_flow_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_npc *npc = &dev->npc;
  int rc;
...
static int cnxk_flow_flush(struct rte_eth_dev *eth_dev,
                           struct rte_flow_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_npc *npc = &dev->npc;
  int rc;
...
static int cnxk_flow_query(struct rte_eth_dev *eth_dev, struct rte_flow *flow,
...
err_exit:
  rte_flow_error_set(error, errcode, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                     errmsg);
  return -rte_errno;
}

static int cnxk_flow_isolate(struct rte_eth_dev *eth_dev __rte_unused,
...
static int cnxk_flow_dev_dump(struct rte_eth_dev *eth_dev,
