...
static uint16_t nix_rx_offload_flags(struct rte_eth_dev *eth_dev) {
...
static uint16_t nix_tx_offload_flags(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
...
static int cn10k_nix_ptypes_set(struct rte_eth_dev *eth_dev,
                                uint32_t ptype_mask) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
...
static void nix_form_default_desc(struct cnxk_eth_dev *dev,
                                  struct cn10k_eth_txq *txq, uint16_t qid) {
...
static int cn10k_nix_tx_queue_setup(struct rte_eth_dev *eth_dev, uint16_t qid,
...
static int cn10k_nix_rx_queue_setup(struct rte_eth_dev *eth_dev, uint16_t qid,
                                    uint16_t nb_desc, unsigned int socket,
...
static int cn10k_nix_tx_queue_stop(struct rte_eth_dev *eth_dev, uint16_t qidx) {
...
static int cn10k_nix_configure(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  int rc;
...
static void nix_ptp_enable_vf(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
...
static uint16_t nix_ptp_vf_burst(void *queue, struct rte_mbuf **mbufs,
                                 uint16_t pkts) {
...
static int cn10k_nix_ptp_info_update_cb(struct roc_nix *nix, bool ptp_en) {
...
static int cn10k_nix_timesync_enable(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  int i, rc;
...
static int cn10k_nix_timesync_disable(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  int i, rc;
...
static int cn10k_nix_timesync_read_tx_timestamp(struct rte_eth_dev *eth_dev,
...
static int cn10k_nix_dev_start(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_nix *nix = &dev->nix;
  int rc;
...
static int cn10k_nix_rx_metadata_negotiate(struct rte_eth_dev *eth_dev,
                                           uint64_t *features) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);

  *features &= (RTE_ETH_RX_METADATA_USER_FLAG | RTE_ETH_RX_METADATA_USER_MARK);
...
static int cn10k_nix_reassembly_capability_get(
    struct rte_eth_dev *eth_dev,
...
static int cn10k_nix_tm_mark_vlan_dei(struct rte_eth_dev *eth_dev,
                                      int mark_green, int mark_yellow,
                                      int mark_red,
  ...
  rc = cnxk_nix_tm_mark_vlan_dei(eth_dev, mark_green, mark_yellow, mark_red,
                                 error);
...
static int cn10k_nix_tm_mark_ip_ecn(struct rte_eth_dev *eth_dev, int mark_green,
...
static int cn10k_nix_tm_mark_ip_dscp(struct rte_eth_dev *eth_dev,
                                     int mark_green, int mark_yellow,
  ...
  rc = cnxk_nix_tm_mark_ip_dscp(eth_dev, mark_green, mark_yellow, mark_red,
                                error);
  if (rc)
    goto exit;
...
static void nix_eth_dev_ops_override(void) {
  ...
  cnxk_eth_dev_ops.rx_metadata_negotiate = cn10k_nix_rx_metadata_negotiate;
...
static void nix_tm_ops_override(void) {
  static int init_once;
...
static void npc_flow_ops_override(void) {
  static int init_once;
...
static int cn10k_nix_remove(struct rte_pci_device *pci_dev) {
  return cnxk_nix_remove(pci_dev);
}

static int cn10k_nix_probe(struct rte_pci_driver *pci_drv,
