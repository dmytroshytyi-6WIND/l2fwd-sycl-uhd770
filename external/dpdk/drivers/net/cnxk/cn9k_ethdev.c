...
static int cn9k_nix_ptypes_set(struct rte_eth_dev *eth_dev,
                               uint32_t ptype_mask) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
...
static void nix_form_default_desc(struct cnxk_eth_dev *dev,
                                  struct cn9k_eth_txq *txq, uint16_t qid) {
...
static int cn9k_nix_tx_queue_setup(struct rte_eth_dev *eth_dev, uint16_t qid,
...
static int cn9k_nix_rx_queue_setup(struct rte_eth_dev *eth_dev, uint16_t qid,
                                   uint16_t nb_desc, unsigned int socket,
...
static int cn9k_nix_tx_queue_stop(struct rte_eth_dev *eth_dev, uint16_t qidx) {
...
static int cn9k_nix_configure(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
...
static int cn9k_nix_ptp_info_update_cb(struct roc_nix *nix, bool ptp_en) {
...
static int cn9k_nix_timesync_enable(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  int i, rc;
...
static int cn9k_nix_timesync_disable(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  int i, rc;
...
static int cn9k_nix_dev_start(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
...
static int cn9k_nix_timesync_read_tx_timestamp(struct rte_eth_dev *eth_dev,
...
static int cn9k_nix_rx_metadata_negotiate(struct rte_eth_dev *eth_dev,
                                          uint64_t *features) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
...
static int cn9k_nix_tm_mark_vlan_dei(struct rte_eth_dev *eth_dev,
...
static int cn9k_nix_tm_mark_ip_ecn(struct rte_eth_dev *eth_dev, int mark_green,
...
static int cn9k_nix_tm_mark_ip_dscp(struct rte_eth_dev *eth_dev, int mark_green,
                                    int mark_yellow, int mark_red,
                                    struct rte_tm_error *error) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_nix *roc_nix = &dev->nix;
...
static int cn9k_nix_remove(struct rte_pci_device *pci_dev) {
  return cnxk_nix_remove(pci_dev);
}

static int cn9k_nix_probe(struct rte_pci_driver *pci_drv,
