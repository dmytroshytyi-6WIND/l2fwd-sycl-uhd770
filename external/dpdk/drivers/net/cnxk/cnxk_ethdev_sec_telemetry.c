...
static int copy_outb_sa_9k(struct rte_tel_data *d, uint32_t i, void *sa) {
...
static int copy_inb_sa_9k(struct rte_tel_data *d, uint32_t i, void *sa) {
...
static int copy_outb_sa_10k(struct rte_tel_data *d, uint32_t i, void *sa) {
  ...
  for (j = 0; j < RTE_DIM(out_sa->outer_hdr.ipv6.src_addr); j++)
    rte_tel_data_add_array_u64(outer_hdr, out_sa->outer_hdr.ipv6.src_addr[j]);

  for (j = 0; j < RTE_DIM(out_sa->outer_hdr.ipv6.dst_addr); j++)
    rte_tel_data_add_array_u64(outer_hdr, out_sa->outer_hdr.ipv6.dst_addr[j]);
...
static int copy_inb_sa_10k(struct rte_tel_data *d, uint32_t i, void *sa) {
  ...
  for (j = 0; j < RTE_DIM(in_sa->outer_hdr.ipv6.src_addr); j++)
    rte_tel_data_add_array_u64(outer_hdr, in_sa->outer_hdr.ipv6.src_addr[j]);

  for (j = 0; j < RTE_DIM(in_sa->outer_hdr.ipv6.dst_addr); j++)
    rte_tel_data_add_array_u64(outer_hdr, in_sa->outer_hdr.ipv6.dst_addr[j]);
...
static int ethdev_sec_tel_handle_info(const char *cmd __rte_unused,
                                      const char *params,
...
RTE_INIT(cnxk_ipsec_init_telemetry) {
  rte_telemetry_register_cmd("/cnxk/ipsec/info", ethdev_sec_tel_handle_info,
