...
static inline void cnxk_pktmbuf_free_no_cache(struct rte_mbuf *mbuf) {
...
static void outb_dbg_iv_update(struct roc_ot_ipsec_outb_sa *outb_sa,
                               const char *__iv_str) {
...
static int cn10k_eth_sec_outb_sa_misc_fill(
    struct roc_nix *roc_nix, struct roc_ot_ipsec_outb_sa *sa, void *sa_cptr,
    struct rte_security_ipsec_xform *ipsec_xfrm, uint32_t sa_idx) {
...
static int cn10k_eth_sec_session_create(void *device,
  ...
  if (inbound) {
    ...
    rc = cnxk_ot_ipsec_inb_sa_fill(inb_sa_dptr, ipsec, crypto, true);
    if (rc) {
    ...
    inb_sa_dptr->w1.s.cookie = rte_cpu_to_be_32(ipsec->spi & spi_mask);
    ...
    rc =
        roc_nix_inl_ctx_write(&dev->nix, inb_sa_dptr, eth_sec->sa, eth_sec->inb,
    ...
    rc = cn10k_eth_sec_outb_sa_misc_fill(&dev->nix, outb_sa_dptr, outb_sa,
                                         ipsec, sa_idx);
    if (rc) {
...
static int cn10k_eth_sec_session_destroy(void *device,
                                         struct rte_security_session *sess) {
  struct rte_eth_dev *eth_dev = (struct rte_eth_dev *)device;
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct cnxk_eth_sec_sess *eth_sec;
