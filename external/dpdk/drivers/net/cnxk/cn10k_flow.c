...
static int cn10k_mtr_connect(struct rte_eth_dev *eth_dev, uint32_t mtr_id) {
  return nix_mtr_connect(eth_dev, mtr_id);
}

static int cn10k_mtr_destroy(struct rte_eth_dev *eth_dev, uint32_t mtr_id) {
  struct rte_mtr_error mtr_error;

  return nix_mtr_destroy(eth_dev, mtr_id, &mtr_error);
}

static int cn10k_mtr_configure(struct rte_eth_dev *eth_dev,
  ...
  while (next_mtr_id != 0xffff) {
    ...
    rc = nix_mtr_color_action_validate(eth_dev, mtr_id, &prev_mtr_id,
                                       &next_mtr_id, policy, &tree_level);
    if (rc)
      return rc;
  }
...
static int cn10k_rss_action_validate(struct rte_eth_dev *eth_dev,
...
struct rte_flow *cn10k_flow_create(struct rte_eth_dev *eth_dev,
                                   const struct rte_flow_attr *attr,
  ...
  if (req_act & ROC_NPC_ACTION_TYPE_METER) {
    ...
    if (req_act & ROC_NPC_ACTION_TYPE_RSS) {
      rc = cn10k_rss_action_validate(eth_dev, attr, action_rss);
      if (rc)
        return NULL;
...
int cn10k_flow_destroy(struct rte_eth_dev *eth_dev, struct rte_flow *rte_flow,
                       struct rte_flow_error *error) {
