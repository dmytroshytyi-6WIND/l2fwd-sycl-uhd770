...
static unsigned int cnxk_eth_sec_session_get_size(void *device __rte_unused) {
  return sizeof(struct cnxk_eth_sec_sess);
}

struct rte_security_ops cnxk_eth_sec_ops = {.session_get_size =
                                                cnxk_eth_sec_session_get_size};
...
static int parse_selftest(const char *key, const char *value,
                          void *extra_args) {
  RTE_SET_USED(key);
  uint32_t val;
...
static int parse_inl_cpt_channel(const char *key, const char *value,
                                 void *extra_args) {
  RTE_SET_USED(key);
...
static int nix_inl_parse_devargs(struct rte_devargs *devargs,
  ...
  rte_kvargs_process(kvlist, CNXK_NIX_INL_SELFTEST, &parse_selftest, &selftest);
...
static inline char *nix_inl_dev_to_name(struct rte_pci_device *pci_dev,
                                        char *name) {
  snprintf(name, CNXK_NIX_INL_DEV_NAME_LEN, CNXK_NIX_INL_DEV_NAME PCI_PRI_FMT,
...
static int cnxk_nix_inl_dev_remove(struct rte_pci_device *pci_dev) {
  ...
  if (rc) {
    plt_err("Failed to cleanup inl dev, rc=%d(%s)", rc, roc_error_msg_get(rc));
    return rc;
  }
...
static int cnxk_nix_inl_dev_probe(struct rte_pci_driver *pci_drv,
...
RTE_PMD_REGISTER_PARAM_STRING(cnxk_nix_inl, CNXK_NIX_INL_SELFTEST
                              "=1" CNXK_NIX_INL_IPSEC_IN_MAX_SPI
                              "=<1-65535>" CNXK_INL_CPT_CHANNEL
                              "=<1-4095>/<1-4095>");
