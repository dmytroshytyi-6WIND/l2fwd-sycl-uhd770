...
static inline int ar_window_init(struct cn9k_inb_priv_data *inb_priv) {
  if (inb_priv->replay_win_sz > CNXK_ON_AR_WIN_SIZE_MAX) {
    plt_err("Replay window size:%u is not supported", inb_priv->replay_win_sz);
    return -ENOTSUP;
  }
...
static int cn9k_eth_sec_session_create(void *device,
  ...
  if (inbound) {
     ...
     */
    inb_sa = (struct roc_onf_ipsec_inb_sa *)roc_nix_inl_inb_sa_get(nix, false,
                                                                   ipsec->spi);
    if (!inb_sa) {
...
static int cn9k_eth_sec_session_destroy(void *device,
                                        struct rte_security_session *sess) {
  struct rte_eth_dev *eth_dev = (struct rte_eth_dev *)device;
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct roc_onf_ipsec_outb_sa *outb_sa;
