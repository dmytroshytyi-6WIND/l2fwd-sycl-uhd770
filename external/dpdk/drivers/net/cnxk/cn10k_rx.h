...
static inline void nix_mbuf_validate_next(struct rte_mbuf *m) {
  if (m->nb_segs == 1 && m->next) {
    rte_panic("mbuf->next[%p] valid when mbuf->nb_segs is %d", m->next,
              m->nb_segs);
  }
}
#else
...
static __rte_always_inline uint64_t nix_clear_data_off(uint64_t oldval) {
...
static __rte_always_inline void nix_sec_flush_meta(uintptr_t laddr,
                                                   uint16_t lmt_id,
                                                   uint8_t loff,
                                                   uintptr_t aura_handle) {
  uint64_t pa;
  ...
  *rte_security_dynfield(mbuf) = (uint64_t)inb_priv->userdata;
  ...
  if (hdr->w0.num_frags > 1) {
    ...
    *rte_security_dynfield(mbuf) = (uint64_t)inb_priv->userdata;
    ...
    cnxk_ip_reassembly_dynfield(mbuf, off)->nb_frags = hdr->w0.num_frags - 2;
  ...
  if (hdr->w0.num_frags > 2) {
    ...
    *rte_security_dynfield(mbuf) = (uint64_t)inb_priv->userdata;
    ...
    cnxk_ip_reassembly_dynfield(mbuf, off)->nb_frags = hdr->w0.num_frags - 3;
    cnxk_ip_reassembly_dynfield(mbuf, off)->next_frag = NULL;
    cnxk_ip_reassembly_dynfield(mbuf_prev, off)->next_frag = mbuf;
    mbuf_prev = mbuf;
  }
  ...
  if (hdr->w0.num_frags > 3) {
    ...
    *rte_security_dynfield(mbuf) = (uint64_t)inb_priv->userdata;
    ...
    cnxk_ip_reassembly_dynfield(mbuf, off)->nb_frags = hdr->w0.num_frags - 4;
    cnxk_ip_reassembly_dynfield(mbuf, off)->next_frag = NULL;
    cnxk_ip_reassembly_dynfield(mbuf_prev, off)->next_frag = mbuf;
  }
  return head;
...
nix_sec_reassemble_frags(const struct cpt_parse_hdr_s *hdr, uint64_t cq_w1,
  ...
  *(uint64_t *)(&mbuf->rearm_data) = mbuf_init | b_off;
  ...
  if (inner_rx->lctype == NPC_LT_LC_IP) {
    struct rte_ipv4_hdr *hdr =
        (struct rte_ipv4_hdr *)RTE_PTR_ADD(inner_iova, inner_rx->lcptr);

    l3_hdr_size = (hdr->version_ihl & 0xf) << 2;
  } else {
    struct rte_ipv6_hdr *hdr =
        (struct rte_ipv6_hdr *)RTE_PTR_ADD(inner_iova, inner_rx->lcptr);
  ...
  *(uint64_t *)(&mbuf->rearm_data) = mbuf_init | data_off;
  ...
  if (inner_rx->lctype == NPC_LT_LC_IP) {
    ...
    hdr->payload_len = rte_cpu_to_be_16(fragx_sum + l3_hdr_size - 8 -
  ...
  if ((flags & NIX_RX_REAS_F) && (cq_w1 & BIT(11))) {
    ...
    if (!hdr->w0.num_frags) {
      ...
      *(uint64_t *)(&inner->rearm_data) = mbuf_init;
      ...
      inner->ol_flags =
          ((uc_cc == CPT_COMP_WARN) ? RTE_MBUF_F_RX_SEC_OFFLOAD
                                    : (RTE_MBUF_F_RX_SEC_OFFLOAD |
                                       RTE_MBUF_F_RX_SEC_OFFLOAD_FAILED));
    } else if (!(hdr->w0.err_sum) && !(hdr->w0.reas_sts)) {
      /* Reassembly success */
      inner = nix_sec_reassemble_frags(hdr, cq_w1, cq_w5, mbuf_init);
    ...
    *ol_flags &= ~(RTE_MBUF_F_RX_L4_CKSUM_MASK | RTE_MBUF_F_RX_IP_CKSUM_MASK);
    ...
    if (!hdr->w0.num_frags) {
      ...
      *ol_flags |= RTE_MBUF_F_RX_SEC_OFFLOAD;
      ...
      /* Update pkt_len and data_len */
      *rx_desc_field1 = vsetq_lane_u16(inner->pkt_len, *rx_desc_field1, 2);
      *rx_desc_field1 = vsetq_lane_u16(inner->data_len, *rx_desc_field1, 4);
...
static __rte_always_inline uint32_t nix_ptype_get(const void *const lookup_mem,
                                                  const uint64_t in) {
...
static inline uint64_t nix_update_match_id(const uint16_t match_id,
                                           uint64_t ol_flags,
...
nix_cqe_xtract_mseg(const union nix_rx_parse_u *rx, struct rte_mbuf *mbuf,
  ...
  mbuf->pkt_len =
      (rx->pkt_lenm1 + 1) -
      (flags & NIX_RX_OFFLOAD_TSTAMP_F ? CNXK_NIX_TIMESYNC_RX_OFFSET : 0);
  mbuf->data_len =
      (sg & 0xFFFF) -
      (flags & NIX_RX_OFFLOAD_TSTAMP_F ? CNXK_NIX_TIMESYNC_RX_OFFSET : 0);
...
static inline uint16_t nix_rx_nb_pkts(struct cn10k_eth_rxq *rxq,
                                      const uint64_t wdata, const uint16_t pkts,
...
cn10k_nix_mbuf_to_tstamp(struct rte_mbuf *mbuf,
  ...
  if (ts_enable) {
     ...
     */
    *tstamp_ptr =
        ((*tstamp_ptr >> 32) * NSEC_PER_SEC) + (*tstamp_ptr & 0xFFFFFFFFUL);
    *cnxk_nix_timestamp_dynfield(mbuf, tstamp) = rte_be_to_cpu_64(*tstamp_ptr);
    ...
    if (mbuf->packet_type == RTE_PTYPE_L2_ETHER_TIMESYNC) {
      ...
      mbuf->ol_flags |= RTE_MBUF_F_RX_IEEE1588_PTP |
                        RTE_MBUF_F_RX_IEEE1588_TMST | tstamp->rx_tstamp_dynflag;
    }
  }
}
...
cn10k_nix_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t pkts,
  ...
  while (packets < nb_pkts) {
    ...
    if (flags & NIX_RX_OFFLOAD_SECURITY_F) {
      ...
      mbuf = nix_sec_meta_to_mbuf_sc(cq_w1, cq_w5, sa_base, laddr, &loff, mbuf,
                                     data_off, flags, mbuf_init);
    }

    cn10k_nix_cqe_to_mbuf(cq, cq->tag, mbuf, lookup_mem, mbuf_init, flags);
...
static __rte_always_inline uint64_t nix_vlan_update(const uint64_t w2,
                                                    uint64_t ol_flags,
                                                    uint8x16_t *f) {
...
static __rte_always_inline uint64_t nix_qinq_update(const uint64_t w2,
                                                    uint64_t ol_flags,
                                                    struct rte_mbuf *mbuf) {
...
static __rte_always_inline uint16_t cn10k_nix_recv_pkts_vector(
    void *args, struct rte_mbuf **mbufs, uint16_t pkts, const uint16_t flags,
  ...
  if (flags & NIX_RX_OFFLOAD_SECURITY_F) {
    if (flags & NIX_RX_VWQE_F) {
      ...
      mbuf0 =
          (struct rte_mbuf *)((uintptr_t)mbufs[0] - sizeof(struct rte_mbuf));
  ...
  while (packets < pkts) {
    ...
    if (flags & NIX_RX_VWQE_F) {
      if (pkts - packets > 4) {
        ...
        if (likely(pkts - packets > 8)) {
          rte_prefetch0(CQE_PTR_DIFF(cq0, 8, RTE_PKTMBUF_HEADROOM, flags));
          rte_prefetch0(CQE_PTR_DIFF(cq0, 9, RTE_PKTMBUF_HEADROOM, flags));
          rte_prefetch0(CQE_PTR_DIFF(cq0, 10, RTE_PKTMBUF_HEADROOM, flags));
          rte_prefetch0(CQE_PTR_DIFF(cq0, 11, RTE_PKTMBUF_HEADROOM, flags));
        }
      }
    } else {
      if (pkts - packets > 4) {
    ...
    if (!(flags & NIX_RX_VWQE_F)) {
      ...
      mbuf01 = vsubq_u64(vld1q_u64((uint64_t *)cq0), data_off);
      mbuf23 = vsubq_u64(vld1q_u64((uint64_t *)(cq0 + 16)), data_off);
    }
    ...
    if (flags & NIX_RX_OFFLOAD_SECURITY_F) {
      ...
      mbuf0 = nix_sec_meta_to_mbuf(cq0_w1, cq0_w5, sa_base, laddr, &loff, mbuf0,
                                   d_off, &f0, &ol_flags0, flags, &rearm0);
      mbuf01 = vsetq_lane_u64((uint64_t)mbuf0, mbuf01, 0);

      mbuf1 = nix_sec_meta_to_mbuf(cq1_w1, cq1_w5, sa_base, laddr, &loff, mbuf1,
                                   d_off, &f1, &ol_flags1, flags, &rearm1);
      mbuf01 = vsetq_lane_u64((uint64_t)mbuf1, mbuf01, 1);

      mbuf2 = nix_sec_meta_to_mbuf(cq2_w1, cq2_w5, sa_base, laddr, &loff, mbuf2,
                                   d_off, &f2, &ol_flags2, flags, &rearm2);
      mbuf23 = vsetq_lane_u64((uint64_t)mbuf2, mbuf23, 0);

      mbuf3 = nix_sec_meta_to_mbuf(cq3_w1, cq3_w5, sa_base, laddr, &loff, mbuf3,
                                   d_off, &f3, &ol_flags3, flags, &rearm3);
    ...
    if (flags & NIX_RX_OFFLOAD_MARK_UPDATE_F) {
      ol_flags0 = nix_update_match_id(
          *(uint16_t *)CQE_PTR_OFF(cq0, 0, 38, flags), ol_flags0, mbuf0);
      ol_flags1 = nix_update_match_id(
          *(uint16_t *)CQE_PTR_OFF(cq0, 1, 38, flags), ol_flags1, mbuf1);
      ol_flags2 = nix_update_match_id(
          *(uint16_t *)CQE_PTR_OFF(cq0, 2, 38, flags), ol_flags2, mbuf2);
      ol_flags3 = nix_update_match_id(
          *(uint16_t *)CQE_PTR_OFF(cq0, 3, 38, flags), ol_flags3, mbuf3);
    }
    ...
    if (flags & NIX_RX_MULTI_SEG_F) {
       ...
       */
      nix_cqe_xtract_mseg(
          (union nix_rx_parse_u *)(CQE_PTR_OFF(cq0, 0, 8, flags)), mbuf0,
          mbuf_initializer, flags);
      nix_cqe_xtract_mseg(
          (union nix_rx_parse_u *)(CQE_PTR_OFF(cq0, 1, 8, flags)), mbuf1,
          mbuf_initializer, flags);
      nix_cqe_xtract_mseg(
          (union nix_rx_parse_u *)(CQE_PTR_OFF(cq0, 2, 8, flags)), mbuf2,
          mbuf_initializer, flags);
      nix_cqe_xtract_mseg(
          (union nix_rx_parse_u *)(CQE_PTR_OFF(cq0, 3, 8, flags)), mbuf3,
          mbuf_initializer, flags);
    }
  ...
  if (unlikely(pkts_left))
    packets += cn10k_nix_recv_pkts(args, &mbufs[packets], pkts_left, flags);
...
static inline uint16_t cn10k_nix_recv_pkts_vector(
    void *args, struct rte_mbuf **mbufs, uint16_t pkts, const uint16_t flags,
    void *lookup_mem, struct cnxk_timesync_info *tstamp, uintptr_t lmt_base) {
  RTE_SET_USED(args);
  RTE_SET_USED(mbufs);
...
#define NIX_RX_RECV_VEC(fn, flags)                                             \
  uint16_t __rte_noinline __rte_hot fn(                                        \
      void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t pkts) {              \
    return cn10k_nix_recv_pkts_vector(rx_queue, rx_pkts, pkts, (flags), NULL,  \
                                      NULL, 0);                                \
  }
