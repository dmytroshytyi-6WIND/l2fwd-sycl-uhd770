...
static inline uint64_t nix_get_rx_offload_capa(struct cnxk_eth_dev *dev) {
...
static inline uint64_t nix_get_tx_offload_capa(struct cnxk_eth_dev *dev) {
  RTE_SET_USED(dev);
  return CNXK_NIX_TX_OFFLOAD_CAPA;
}

static inline uint32_t nix_get_speed_capa(struct cnxk_eth_dev *dev) {
  uint32_t speed_capa;
...
int cnxk_nix_inb_mode_set(struct cnxk_eth_dev *dev, bool use_inl_dev) {
  ...
  plt_nix_dbg("Security sessions(%u) still active, inl=%u!!!", dev->inb.nb_sess,
              !!dev->inb.inl_dev);
...
static int nix_security_setup(struct cnxk_eth_dev *dev) {
  struct roc_nix *nix = &dev->nix;
  int i, rc = 0;
  ...
  if (dev->rx_offloads & RTE_ETH_RX_OFFLOAD_SECURITY) {
    ...
    if (rc) {
      plt_err("Failed to initialize nix inline inb, rc=%d", rc);
      return rc;
    }
     ...
     * WRITE_SA op.
     */
    dev->inb.sa_dptr = plt_zmalloc(ROC_NIX_INL_OT_IPSEC_INB_HW_SZ, 0);
    if (!dev->inb.sa_dptr) {
  ...
  if (dev->tx_offloads & RTE_ETH_TX_OFFLOAD_SECURITY ||
    ...
    if (rc) {
      plt_err("Failed to initialize nix inline outb, rc=%d", rc);
      goto sa_dptr_free;
    }
     ...
     * WRITE_SA op.
     */
    dev->outb.sa_dptr = plt_zmalloc(ROC_NIX_INL_OT_IPSEC_OUTB_HW_SZ, 0);
    if (!dev->outb.sa_dptr) {
      plt_err("Couldn't allocate memory for SA dptr");
      rc = -ENOMEM;
      goto sa_dptr_free;
...
static int nix_meter_fini(struct cnxk_eth_dev *dev) {
...
static int nix_security_release(struct cnxk_eth_dev *dev) {
  ...
  if (dev->rx_offloads & RTE_ETH_RX_OFFLOAD_SECURITY) {
    ...
    cnxk_eth_sec_ops.session_destroy(eth_dev, eth_sec->sess);
...
static void nix_enable_mseg_on_jumbo(struct cnxk_eth_rxq_sp *rxq) {
...
static int nix_init_flow_ctrl_config(struct rte_eth_dev *eth_dev) {
  ...
  fc->mode =
      (fc_mode == ROC_NIX_FC_FULL) ? RTE_ETH_FC_FULL : RTE_ETH_FC_TX_PAUSE;
  return rc;
}

static int nix_update_flow_ctrl_config(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct cnxk_fc_cfg *fc = &dev->fc_cfg;
  ...
  if (roc_model_is_cn96_ax() &&
    ...
    fc_cfg.mode =
        (fc_cfg.mode == RTE_ETH_FC_FULL || fc_cfg.mode == RTE_ETH_FC_TX_PAUSE)
            ? RTE_ETH_FC_TX_PAUSE
            : RTE_ETH_FC_NONE;
  }
...
uint64_t cnxk_nix_rxq_mbuf_setup(struct cnxk_eth_dev *dev) {
  ...
  mb_def.nb_segs = 1;
  mb_def.data_off =
      RTE_PKTMBUF_HEADROOM + (dev->ptp_en * CNXK_NIX_TIMESYNC_RX_OFFSET);
  mb_def.port = port_id;
...
static inline uint8_t nix_sq_max_sqe_sz(struct cnxk_eth_dev *dev) {
  /*
...
int cnxk_nix_tx_queue_setup(struct rte_eth_dev *eth_dev, uint16_t qid,
  ...
  plt_nix_dbg("sq=%d fc=%p offload=0x%" PRIx64 " lmt_addr=%p"
              ...
              qid, sq->fc, dev->tx_offloads, sq->lmt_addr, sq->nb_sqb_bufs,
...
static void cnxk_nix_tx_queue_release(struct rte_eth_dev *eth_dev,
                                      uint16_t qid) {
...
static void cnxk_nix_rx_queue_release(struct rte_eth_dev *eth_dev,
                                      uint16_t qid) {
...
uint32_t cnxk_rss_ethdev_to_nix(struct cnxk_eth_dev *dev, uint64_t ethdev_rss,
  ...
  uint32_t flow_key_type[RSS_MAX_LEVELS][6] = {
      ...
      {FLOW_KEY_TYPE_INNR_IPV4, FLOW_KEY_TYPE_INNR_IPV6, FLOW_KEY_TYPE_INNR_TCP,
       FLOW_KEY_TYPE_INNR_UDP, FLOW_KEY_TYPE_INNR_SCTP,
...
static void nix_free_queue_mem(struct cnxk_eth_dev *dev) {
  plt_free(dev->rqs);
  plt_free(dev->cqs);
  plt_free(dev->sqs);
  dev->rqs = NULL;
...
static int nix_ingress_policer_setup(struct cnxk_eth_dev *dev) {
  struct rte_eth_dev *eth_dev = dev->eth_dev;
  int rc = 0;
...
static int nix_rss_default_setup(struct cnxk_eth_dev *dev) {
  struct rte_eth_dev *eth_dev = dev->eth_dev;
...
static int nix_store_queue_cfg_and_then_release(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  const struct eth_dev_ops *dev_ops = eth_dev->dev_ops;
...
static int nix_restore_queue_cfg(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  const struct eth_dev_ops *dev_ops = eth_dev->dev_ops;
  ...
  for (i = 0; i < nb_rxq; i++) {
    ...
    rc = dev_ops->rx_queue_setup(eth_dev, i, rx_qconf[i].nb_desc, 0,
                                 &rx_qconf[i].conf.rx, rx_qconf[i].mp);
    if (rc) {
...
static void nix_set_nop_rxtx_function(struct rte_eth_dev *eth_dev) {
...
static int nix_lso_tun_fmt_update(struct cnxk_eth_dev *dev) {
...
static int nix_lso_fmt_setup(struct cnxk_eth_dev *dev) {
  struct roc_nix *nix = &dev->nix;
  int rc;
...
int cnxk_nix_configure(struct rte_eth_dev *eth_dev) {
  ...
  if (nix->rx_ptp_ena && dev->npc.switch_header_type == ROC_PRIV_FLAGS_HIGIG) {
    plt_err("Both PTP and switch header enabled");
    goto free_nix_lf;
  }

  rc = roc_nix_switch_hdr_set(
      nix, dev->npc.switch_header_type, dev->npc.pre_l2_size_offset,
      dev->npc.pre_l2_size_offset_mask, dev->npc.pre_l2_size_shift_dir);
  if (rc) {
  ...
  if (eth_dev->data->dev_conf.intr_conf.rxq) {
    if (eth_dev->data->nb_rx_queues > dev->nix.cints) {
      plt_err("Rx interrupt cannot be enabled, rxq > %d", dev->nix.cints);
      goto q_irq_fini;
    }
  ...
  rc = roc_nix_mac_loopback_enable(nix, eth_dev->data->dev_conf.lpbk_mode);
  if (rc) {
  ...
  plt_nix_dbg("Configured port%d mac=%s nb_rxq=%d nb_txq=%d"
              ...
              eth_dev->data->port_id, ea_fmt, nb_rxq, nb_txq, dev->rx_offloads,
              dev->tx_offloads);
...
static int cnxk_nix_rx_queue_start(struct rte_eth_dev *eth_dev, uint16_t qid) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct rte_eth_dev_data *data = eth_dev->data;
...
static int cnxk_nix_rx_queue_stop(struct rte_eth_dev *eth_dev, uint16_t qid) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  struct rte_eth_dev_data *data = eth_dev->data;
  struct roc_nix_rq *rq = &dev->rqs[qid];
  int rc;
...
static int cnxk_nix_dev_stop(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  const struct eth_dev_ops *dev_ops = eth_dev->dev_ops;
...
int cnxk_nix_dev_start(struct rte_eth_dev *eth_dev) {
  ...
  if (dev->rx_offloads & RTE_ETH_RX_OFFLOAD_TIMESTAMP) {
    rc = rte_mbuf_dyn_rx_timestamp_register(&dev->tstamp.tstamp_dynfield_offset,
...
struct eth_dev_ops cnxk_eth_dev_ops = {
    ...
    .priority_flow_ctrl_queue_config = cnxk_nix_priority_flow_ctrl_queue_config,
...
static int cnxk_eth_dev_init(struct rte_eth_dev *eth_dev) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  ...
  /* Register up msg callbacks */
  roc_nix_mac_link_info_get_cb_register(nix, cnxk_eth_dev_link_status_get_cb);
...
static int cnxk_eth_dev_uninit(struct rte_eth_dev *eth_dev, bool reset) {
  struct cnxk_eth_dev *dev = cnxk_eth_pmd_priv(eth_dev);
  const struct eth_dev_ops *dev_ops = eth_dev->dev_ops;
  ...
  for (i = 0; i < CNXK_NIX_PFC_CHAN_COUNT; i++) {
    if (dev->pfc_tc_sq_map[i] != 0xFFFF) {
      ...
      rc = cnxk_nix_priority_flow_ctrl_queue_config(eth_dev, &pfc_conf);
      if (rc)
