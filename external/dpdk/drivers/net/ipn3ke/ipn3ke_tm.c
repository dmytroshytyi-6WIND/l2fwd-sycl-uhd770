...
#define IPN3KE_TM_SHAPER_RANGE_NUM                                             \
  (sizeof(ipn3ke_tm_shaper_params_rang) /                                      \
...
int ipn3ke_hw_tm_init(struct ipn3ke_hw *hw) {
#define SCRATCH_DATA 0xABCDEF
  ...
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_TM_SCRATCH, 0, SCRATCH_DATA, 0xFFFFFFFF);
  scratch_data = IPN3KE_MASK_READ_REG(hw, IPN3KE_TM_SCRATCH, 0, 0xFFFFFFFF);
  if (scratch_data != SCRATCH_DATA)
    return -EINVAL;
#endif
  /* alloc memory for all hierarchy nodes */
  node_num = hw->port_num + IPN3KE_TM_VT_NODE_NUM + IPN3KE_TM_COS_NODE_NUM;

  nodes = rte_zmalloc("ipn3ke_tm_nodes",
                      sizeof(struct ipn3ke_tm_node) * node_num, 0);
  if (!nodes)
    return -ENOMEM;

  /* alloc memory for Tail Drop Profile */
  tdrop_profile = rte_zmalloc(
      "ipn3ke_tm_tdrop_profile",
      sizeof(struct ipn3ke_tm_tdrop_profile) * IPN3KE_TM_TDROP_PROFILE_NUM, 0);
  if (!tdrop_profile) {
    rte_free(nodes);
    return -ENOMEM;
  }
  ...
  for (i = 0, nodes = hw->port_nodes; i < hw->port_num; i++, nodes++) {
    nodes->node_index = i;
  ...
  for (i = 0, nodes = hw->vt_nodes; i < IPN3KE_TM_VT_NODE_NUM; i++, nodes++) {
    nodes->node_index = i;
  ...
  for (i = 0, nodes = hw->cos_nodes; i < IPN3KE_TM_COS_NODE_NUM; i++, nodes++) {
    nodes->node_index = i;
  ...
  for (i = 0, tdrop_profile = hw->tdrop_profile;
       i < IPN3KE_TM_TDROP_PROFILE_NUM; i++, tdrop_profile++) {
...
void ipn3ke_tm_init(struct ipn3ke_rpst *rpst) {
...
ipn3ke_hw_tm_shaper_profile_search(struct ipn3ke_hw *hw,
  ...
  if (shaper_profile_id == RTE_TM_SHAPER_PROFILE_ID_NONE) {
    rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_SHAPER_PROFILE_ID, NULL,
                     rte_strerror(EINVAL));
  ...
  case IPN3KE_TM_NODE_LEVEL_PORT:
    if (node_index >= hw->port_num)
      rte_tm_error_set(error, EEXIST, RTE_TM_ERROR_TYPE_SHAPER_PROFILE_ID, NULL,
                       rte_strerror(EEXIST));
    else
...
static struct ipn3ke_tm_node *ipn3ke_hw_tm_node_search(struct ipn3ke_hw *hw,
                                                       uint32_t tm_id,
                                                       uint32_t node_id,
                                                       uint32_t state_mask) {
  uint32_t level_of_node_id;
  ...
  if (n->node_state == IPN3KE_TM_NODE_STATE_IDLE) {
    if (n->tm_id != RTE_TM_NODE_ID_NULL ||
        n->parent_node_id != RTE_TM_NODE_ID_NULL || n->parent_node != NULL ||
        n->n_children > 0) {
...
static int ipn3ke_pmd_tm_node_type_get(struct rte_eth_dev *dev,
                                       uint32_t node_id, int *is_leaf,
                                       struct rte_tm_error *error) {
  ...
  if (node_id == RTE_TM_NODE_ID_NULL || node == NULL)
...
static int ipn3ke_tm_capabilities_get(__rte_unused struct rte_eth_dev *dev,
...
static int
ipn3ke_tm_level_capabilities_get(struct rte_eth_dev *dev, uint32_t level_id,
...
static int ipn3ke_tm_node_capabilities_get(struct rte_eth_dev *dev,
                                           uint32_t node_id,
  ...
  case IPN3KE_TM_NODE_LEVEL_PORT:
    ...
    cap->nonleaf.sched_n_children_max = IPN3KE_TM_VT_NODE_NUM;
    cap->nonleaf.sched_sp_n_priorities_max = 1;
    cap->nonleaf.sched_wfq_n_children_per_group_max = IPN3KE_TM_VT_NODE_NUM;
  ...
  case IPN3KE_TM_NODE_LEVEL_VT:
    ...
    cap->nonleaf.sched_n_children_max = IPN3KE_TM_COS_NODE_NUM;
    cap->nonleaf.sched_sp_n_priorities_max = 1;
    cap->nonleaf.sched_wfq_n_children_per_group_max = IPN3KE_TM_COS_NODE_NUM;
    cap->nonleaf.sched_wfq_n_groups_max = 1;
    cap->nonleaf.sched_wfq_weight_max = 1;
    cap->nonleaf.sched_wfq_packet_mode_supported = 0;
    cap->nonleaf.sched_wfq_byte_mode_supported = 0;
...
static int ipn3ke_tm_shaper_parame_trans(
...
static int ipn3ke_tm_shaper_profile_add(struct rte_eth_dev *dev,
                                        uint32_t shaper_profile_id,
                                        struct rte_tm_shaper_params *profile,
                                        struct rte_tm_error *error) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(dev);
  ...
  if (ipn3ke_tm_shaper_parame_trans(profile, sp,
...
static int ipn3ke_tm_shaper_profile_delete(struct rte_eth_dev *dev,
                                           uint32_t shaper_profile_id,
                                           struct rte_tm_error *error) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(dev);
  struct ipn3ke_tm_internals *tm = IPN3KE_DEV_PRIVATE_TO_TM(dev);
...
static int ipn3ke_tm_tdrop_profile_check(__rte_unused struct rte_eth_dev *dev,
                                         uint32_t tdrop_profile_id,
  ...
  for (color = RTE_COLOR_GREEN; color <= RTE_COLOR_GREEN; color++) {
    ...
    if (((min_th >> IPN3KE_TDROP_TH1_SHIFT) >> IPN3KE_TDROP_TH1_SHIFT) ||
        max_th != 0)
      return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_WRED_PROFILE,
                               NULL, rte_strerror(EINVAL));
  }
...
static int ipn3ke_hw_tm_tdrop_wr(struct ipn3ke_hw *hw,
                                 struct ipn3ke_tm_tdrop_profile *tp) {
  if (tp->valid) {
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CCB_PROFILE_MS, 0, tp->th2,
                          IPN3KE_CCB_PROFILE_MS_MASK);

    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CCB_PROFILE_P, tp->tdrop_profile_id,
                          tp->th1, IPN3KE_CCB_PROFILE_MASK);
  } else {
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CCB_PROFILE_MS, 0, 0,
                          IPN3KE_CCB_PROFILE_MS_MASK);

    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CCB_PROFILE_P, tp->tdrop_profile_id, 0,
...
static int ipn3ke_tm_tdrop_profile_add(struct rte_eth_dev *dev,
                                       uint32_t tdrop_profile_id,
                                       struct rte_tm_wred_params *profile,
                                       struct rte_tm_error *error) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(dev);
  ...
  /* Check input params */
  status = ipn3ke_tm_tdrop_profile_check(dev, tdrop_profile_id, profile, error);
  if (status)
    return status;
  ...
  th2 = (uint32_t)((min_th >> IPN3KE_TDROP_TH1_SHIFT) & IPN3KE_TDROP_TH2_MASK);
  tp->th1 = th1;
  tp->th2 = th2;
...
static int ipn3ke_tm_tdrop_profile_delete(struct rte_eth_dev *dev,
                                          uint32_t tdrop_profile_id,
                                          struct rte_tm_error *error) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(dev);
  struct ipn3ke_tm_internals *tm = IPN3KE_DEV_PRIVATE_TO_TM(dev);
...
static int ipn3ke_tm_node_add_check_parameter(
    uint32_t tm_id, uint32_t node_id, uint32_t parent_node_id,
    uint32_t priority, uint32_t weight, uint32_t level_id,
    struct rte_tm_node_params *params, struct rte_tm_error *error) {
  uint32_t level_of_node_id;
  uint32_t node_index;
...
static int ipn3ke_tm_node_add_check_mount(uint32_t tm_id, uint32_t node_id,
...
static int ipn3ke_tm_node_add(struct rte_eth_dev *dev, uint32_t node_id,
  ...
  status = ipn3ke_tm_node_add_check_parameter(tm_id, node_id, parent_node_id,
                                              priority, weight, level_id,
                                              params, error);
  if (status)
    return status;

  status = ipn3ke_tm_node_add_check_mount(tm_id, node_id, parent_node_id,
                                          level_id, error);
  if (status)
    return status;
  ...
  if (parent_node_id != RTE_TM_NODE_ID_NULL) {
    parent_node =
        ipn3ke_hw_tm_node_search(hw, tm_id, parent_node_id, state_mask);
    if (!parent_node)
      return -rte_tm_error_set(error, EINVAL,
                               RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID, NULL,
                               rte_strerror(EINVAL));
  ...
  case IPN3KE_TM_NODE_LEVEL_COS:
    if (node_state == IPN3KE_TM_NODE_STATE_IDLE) {
      TAILQ_INSERT_TAIL(&tm->h.cos_commit_node_list, n, node);
      if (parent_node)
        parent_node->n_children++;
      tm->h.n_cos_nodes++;
    } else if (node_state == IPN3KE_TM_NODE_STATE_CONFIGURED_DEL) {
  ...
  if (n->level == IPN3KE_TM_NODE_LEVEL_COS &&
    ...
    n->tdrop_profile = ipn3ke_hw_tm_tdrop_profile_search(
        hw, params->leaf.wred.wred_profile_id);
...
static int ipn3ke_tm_node_del_check_parameter(uint32_t tm_id, uint32_t node_id,
                                              struct rte_tm_error *error) {
  uint32_t level_of_node_id;
  uint32_t node_index;
...
static int ipn3ke_pmd_tm_node_delete(struct rte_eth_dev *dev, uint32_t node_id,
                                     struct rte_tm_error *error) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(dev);
  struct ipn3ke_tm_internals *tm = IPN3KE_DEV_PRIVATE_TO_TM(dev);
  struct ipn3ke_tm_node *n, *parent_node;
  ...
  status = ipn3ke_tm_node_del_check_parameter(tm_id, node_id, error);
  if (status)
    return status;
  ...
  if (n->parent_node_id != RTE_TM_NODE_ID_NULL) {
    ...
    IPN3KE_BIT_SET(state_mask, IPN3KE_TM_NODE_STATE_COMMITTED);
    parent_node =
        ipn3ke_hw_tm_node_search(hw, tm_id, n->parent_node_id, state_mask);
    if (!parent_node)
      return -rte_tm_error_set(error, EINVAL,
                               RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID, NULL,
                               rte_strerror(EINVAL));
  ...
  case IPN3KE_TM_NODE_LEVEL_VT:
    if (node_state == IPN3KE_TM_NODE_STATE_COMMITTED) {
      if (parent_node)
        TAILQ_REMOVE(&parent_node->children_node_list, n, node);
      TAILQ_INSERT_TAIL(&tm->h.vt_commit_node_list, n, node);
      if (parent_node)
        parent_node->n_children--;
      tm->h.n_vt_nodes--;
...
static int ipn3ke_tm_hierarchy_commit_check(struct rte_eth_dev *dev,
                                            struct rte_tm_error *error) {
  struct ipn3ke_tm_internals *tm = IPN3KE_DEV_PRIVATE_TO_TM(dev);
  uint32_t tm_id;
  ...
  TAILQ_FOREACH(n, nl, node) {
    ...
    if (n->node_state == IPN3KE_TM_NODE_STATE_CONFIGURED_ADD) {
      if (n->parent_node_id == RTE_TM_NODE_ID_NULL ||
          n->level != IPN3KE_TM_NODE_LEVEL_COS || n->tm_id != tm_id ||
          parent_node == NULL ||
          (parent_node &&
           parent_node->node_state == IPN3KE_TM_NODE_STATE_CONFIGURED_DEL) ||
          (parent_node &&
           parent_node->node_state == IPN3KE_TM_NODE_STATE_IDLE) ||
      ...
      if (n->level != IPN3KE_TM_NODE_LEVEL_COS || n->n_children != 0) {
        return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_UNSPECIFIED,
                                 NULL, rte_strerror(EINVAL));
      }
    }
  ...
  TAILQ_FOREACH(n, nl, node) {
    ...
    if (n->node_state == IPN3KE_TM_NODE_STATE_CONFIGURED_ADD) {
      if (n->parent_node_id == RTE_TM_NODE_ID_NULL ||
          n->level != IPN3KE_TM_NODE_LEVEL_VT || n->tm_id != tm_id ||
          parent_node == NULL ||
          (parent_node &&
           parent_node->node_state == IPN3KE_TM_NODE_STATE_CONFIGURED_DEL) ||
          (parent_node &&
  ...
  if (n && (n->parent_node_id != RTE_TM_NODE_ID_NULL ||
            n->level != IPN3KE_TM_NODE_LEVEL_PORT || n->tm_id != tm_id ||
            n->parent_node != NULL || n->shaper_profile.valid == 0)) {
    return -rte_tm_error_set(error, EINVAL, RTE_TM_ERROR_TYPE_UNSPECIFIED, NULL,
                             rte_strerror(EINVAL));
  }
...
static int ipn3ke_hw_tm_node_wr(struct ipn3ke_hw *hw, struct ipn3ke_tm_node *n,
  ...
  case IPN3KE_TM_NODE_LEVEL_PORT:
     ...
     * Configure Type
     */
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_TYPE_L3_X, n->node_index, n->priority,
                          IPN3KE_QOS_TYPE_MASK);

    /**
     * Configure Sch_wt
     */
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_SCH_WT_L3_X, n->node_index, n->weight,
    ...
    if (n->shaper_profile.valid)
      IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_SHAP_WT_L3_X, n->node_index,
                            ((n->shaper_profile.e << 10) | n->shaper_profile.m),
  ...
  case IPN3KE_TM_NODE_LEVEL_VT:
     ...
     * Configure Type
     */
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_TYPE_L2_X, n->node_index, n->priority,
                          IPN3KE_QOS_TYPE_MASK);

    /**
     * Configure Sch_wt
     */
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_SCH_WT_L2_X, n->node_index, n->weight,
                          IPN3KE_QOS_SCH_WT_MASK);
     ...
     */
    if (n->shaper_profile.valid)
      IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_SHAP_WT_L2_X, n->node_index,
                            ((n->shaper_profile.e << 10) | n->shaper_profile.m),
                            IPN3KE_QOS_SHAP_WT_MASK);
     ...
     */
    if (parent_node)
      IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_MAP_L2_X, n->node_index,
                            parent_node->node_index, IPN3KE_QOS_MAP_L2_MASK);
  ...
  case IPN3KE_TM_NODE_LEVEL_COS:
    ...
    if (n->tdrop_profile && n->tdrop_profile->valid) {
      IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CCB_QPROFILE_Q, n->node_index,
     ...
     * Configure Type
     */
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_TYPE_L1_X, n->node_index, n->priority,
                          IPN3KE_QOS_TYPE_MASK);

    /**
     * Configure Sch_wt
     */
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_SCH_WT_L1_X, n->node_index, n->weight,
                          IPN3KE_QOS_SCH_WT_MASK);
     ...
     */
    if (n->shaper_profile.valid)
      IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_SHAP_WT_L1_X, n->node_index,
                            ((n->shaper_profile.e << 10) | n->shaper_profile.m),
                            IPN3KE_QOS_SHAP_WT_MASK);
     ...
     */
    while (IPN3KE_MASK_READ_REG(hw, IPN3KE_QM_UID_CONFIG_CTRL, 0, 0x80000000))
      ;

    if (parent_node && parent_node->parent_node)
      IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QM_UID_CONFIG_DATA, 0,
                            (1 << 8 | parent_node->parent_node->node_index),
                            0x1FF);

    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QM_UID_CONFIG_CTRL, 0, n->node_index,
                          0xFFFFF);
     ...
     */
    if (parent_node)
      IPN3KE_MASK_WRITE_REG(hw, IPN3KE_QOS_MAP_L1_X, n->node_index,
                            parent_node->node_index, IPN3KE_QOS_MAP_L1_MASK);
...
static int ipn3ke_tm_hierarchy_hw_commit(struct rte_eth_dev *dev,
                                         struct rte_tm_error *error) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(dev);
  struct ipn3ke_tm_internals *tm = IPN3KE_DEV_PRIVATE_TO_TM(dev);
  struct ipn3ke_tm_node_list *nl;
  ...
  for (n = TAILQ_FIRST(nl); n != NULL; n = nn) {
    ...
    if (n->node_state == IPN3KE_TM_NODE_STATE_CONFIGURED_ADD) {
      ...
      TAILQ_INSERT_TAIL(&parent_node->children_node_list, n, node);
    } else if (n->node_state == IPN3KE_TM_NODE_STATE_CONFIGURED_DEL) {
      parent_node = n->parent_node;
      TAILQ_REMOVE(nl, n, node);
...
static int ipn3ke_tm_hierarchy_commit_clear(struct rte_eth_dev *dev) {
  struct ipn3ke_tm_internals *tm = IPN3KE_DEV_PRIVATE_TO_TM(dev);
  struct ipn3ke_tm_node_list *nl;
  struct ipn3ke_tm_node *n;
...
static void ipn3ke_tm_show(struct rte_eth_dev *dev) {
  struct ipn3ke_tm_internals *tm = IPN3KE_DEV_PRIVATE_TO_TM(dev);
  uint32_t tm_id;
  struct ipn3ke_tm_node_list *vt_nl, *cos_nl;
  struct ipn3ke_tm_node *port_n, *vt_n, *cos_n;
  const char *str_state[IPN3KE_TM_NODE_STATE_MAX] = {"Idle", "CfgAdd", "CfgDel",
                                                     "Committed"};
  ...
  TAILQ_FOREACH(vt_n, vt_nl, node) {
    ...
    TAILQ_FOREACH(cos_n, cos_nl, node) {
      if (cos_n->parent_node_id !=
        ...
        IPN3KE_AFU_PMD_ERR("(%d|%s), ", cos_n->node_index,
...
static void ipn3ke_tm_show_commmit(struct rte_eth_dev *dev) {
  struct ipn3ke_tm_internals *tm = IPN3KE_DEV_PRIVATE_TO_TM(dev);
  uint32_t tm_id;
  struct ipn3ke_tm_node_list *nl;
  struct ipn3ke_tm_node *n;
  ...
  if (n)
    IPN3KE_AFU_PMD_DEBUG("(%d|%s)", n->node_index, str_state[n->node_state]);
  IPN3KE_AFU_PMD_DEBUG("\n");
...
static int ipn3ke_tm_hierarchy_commit(struct rte_eth_dev *dev,
                                      int clear_on_fail,
                                      struct rte_tm_error *error) {
  struct ipn3ke_tm_internals *tm = IPN3KE_DEV_PRIVATE_TO_TM(dev);
  int status;
...
int ipn3ke_tm_ops_get(struct rte_eth_dev *ethdev, void *arg) {
  ...
  if (hw->acc_tm) {
    *(const void **)arg = &ipn3ke_tm_ops;
    ...
    if (i40e_pf_eth->dev_ops->tm_ops_get == NULL ||
        i40e_pf_eth->dev_ops->tm_ops_get(i40e_pf_eth, &ops) != 0 ||
        ops == NULL) {
      return -EINVAL;
    }
    *(const void **)arg = ops;
