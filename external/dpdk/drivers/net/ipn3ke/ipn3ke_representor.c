...
static int ipn3ke_rpst_link_check(struct ipn3ke_rpst *rpst);

static int ipn3ke_rpst_dev_infos_get(struct rte_eth_dev *ethdev,
                                     struct rte_eth_dev_info *dev_info) {
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(ethdev);
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(ethdev);

  dev_info->speed_capa =
      (hw->retimer.mac_type == IFPGA_RAWDEV_RETIMER_MAC_TYPE_10GE_XFI)
          ? RTE_ETH_LINK_SPEED_10G
          : ((hw->retimer.mac_type == IFPGA_RAWDEV_RETIMER_MAC_TYPE_25GE_25GAUI)
  ...
  dev_info->tx_offload_capa =
      ...
      RTE_ETH_TX_OFFLOAD_MULTI_SEGS | dev_info->tx_queue_offload_capa;
...
static int ipn3ke_rpst_dev_configure(__rte_unused struct rte_eth_dev *dev) {
  return 0;
}

static int ipn3ke_rpst_dev_start(struct rte_eth_dev *dev) {
  ...
  snprintf(attr_name, IPN3KE_RAWDEV_ATTR_LEN_MAX, "%s", "LineSideBaseMAC");
  rawdev->dev_ops->attr_get(rawdev, attr_name, &base_mac);
  rte_ether_addr_copy((struct rte_ether_addr *)&base_mac, &rpst->mac_addr);
  ...
  if (hw->retimer.mac_type == IFPGA_RAWDEV_RETIMER_MAC_TYPE_10GE_XFI) {
    /* Set mac address */
    rte_memcpy(((char *)(&val)), (char *)&dev->data->mac_addrs->addr_bytes[0],
               sizeof(uint32_t));
    (*hw->f_mac_write)(hw, val, IPN3KE_MAC_PRIMARY_MAC_ADDR0, rpst->port_id, 0);
    rte_memcpy(((char *)(&val)), (char *)&dev->data->mac_addrs->addr_bytes[4],
               sizeof(uint16_t));
    (*hw->f_mac_write)(hw, val, IPN3KE_MAC_PRIMARY_MAC_ADDR1, rpst->port_id, 0);
...
static int ipn3ke_rpst_dev_stop(struct rte_eth_dev *dev) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(dev);
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(dev);
...
static int ipn3ke_rpst_dev_close(struct rte_eth_dev *dev) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(dev);
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(dev);
...
static int ipn3ke_rpst_rx_queue_start(__rte_unused struct rte_eth_dev *dev,
                                      __rte_unused uint16_t rx_queue_id) {
  return 0;
}

static int ipn3ke_rpst_rx_queue_stop(__rte_unused struct rte_eth_dev *dev,
                                     __rte_unused uint16_t rx_queue_id) {
  return 0;
}

static int ipn3ke_rpst_tx_queue_start(__rte_unused struct rte_eth_dev *dev,
                                      __rte_unused uint16_t tx_queue_id) {
  return 0;
}

static int ipn3ke_rpst_tx_queue_stop(__rte_unused struct rte_eth_dev *dev,
                                     __rte_unused uint16_t tx_queue_id) {
  return 0;
}

static int ipn3ke_rpst_rx_queue_setup(
    __rte_unused struct rte_eth_dev *dev, __rte_unused uint16_t queue_idx,
    __rte_unused uint16_t nb_desc, __rte_unused unsigned int socket_id,
    __rte_unused const struct rte_eth_rxconf *rx_conf,
    __rte_unused struct rte_mempool *mp) {
  return 0;
}

static int ipn3ke_rpst_tx_queue_setup(
    __rte_unused struct rte_eth_dev *dev, __rte_unused uint16_t queue_idx,
    __rte_unused uint16_t nb_desc, __rte_unused unsigned int socket_id,
...
static const struct ipn3ke_rpst_xstats_name_offset ipn3ke_rpst_stats_strings[] =
    {
        {"tx_multicast_packets",
         offsetof(struct ipn3ke_rpst_eth_stats, tx_multicast)},
        {"tx_broadcast_packets",
         offsetof(struct ipn3ke_rpst_eth_stats, tx_broadcast)},
        {"tx_unicast_packets",
         offsetof(struct ipn3ke_rpst_eth_stats, tx_unicast)},
        {"tx_dropped_packets",
         offsetof(struct ipn3ke_rpst_eth_stats, tx_discards)},
        {"rx_multicast_packets",
         offsetof(struct ipn3ke_rpst_eth_stats, rx_multicast)},
        {"rx_broadcast_packets",
         offsetof(struct ipn3ke_rpst_eth_stats, rx_broadcast)},
        {"rx_unicast_packets",
         offsetof(struct ipn3ke_rpst_eth_stats, rx_unicast)},
        {"rx_dropped_packets",
         offsetof(struct ipn3ke_rpst_eth_stats, rx_discards)},
        {"rx_unknown_protocol_packets",
         offsetof(struct ipn3ke_rpst_eth_stats, rx_unknown_protocol)},
};

#define IPN3KE_RPST_ETH_XSTATS_CNT                                             \
  (sizeof(ipn3ke_rpst_stats_strings) / sizeof(ipn3ke_rpst_stats_strings[0]))
...
static const struct ipn3ke_rpst_xstats_name_offset
    ipn3ke_rpst_hw_port_strings[] = {
        {"tx_link_down_dropped",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_dropped_link_down)},
        {"rx_crc_errors",
         offsetof(struct ipn3ke_rpst_hw_port_stats, crc_errors)},
        {"rx_illegal_byte_errors",
         offsetof(struct ipn3ke_rpst_hw_port_stats, illegal_bytes)},
        {"rx_error_bytes",
         offsetof(struct ipn3ke_rpst_hw_port_stats, error_bytes)},
        {"mac_local_errors",
         offsetof(struct ipn3ke_rpst_hw_port_stats, mac_local_faults)},
        {"mac_remote_errors",
         offsetof(struct ipn3ke_rpst_hw_port_stats, mac_remote_faults)},
        {"rx_length_errors",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_length_errors)},
        {"tx_xon_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, link_xon_tx)},
        {"rx_xon_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, link_xon_rx)},
        {"tx_xoff_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, link_xoff_tx)},
        {"rx_xoff_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, link_xoff_rx)},
        {"rx_size_64_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_size_64)},
        {"rx_size_65_to_127_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_size_65_127)},
        {"rx_size_128_to_255_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_size_128_255)},
        {"rx_size_256_to_511_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_size_256_511)},
        {"rx_size_512_to_1023_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_size_512_1023)},
        {"rx_size_1024_to_1518_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_size_1024_1518)},
        {"rx_size_1519_to_max_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_size_big)},
        {"rx_undersized_errors",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_undersize)},
        {"rx_oversize_errors",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_oversize)},
        {"rx_mac_short_dropped",
         offsetof(struct ipn3ke_rpst_hw_port_stats, mac_short_packet_dropped)},
        {"rx_fragmented_errors",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_fragments)},
        {"rx_jabber_errors",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_jabber)},
        {"tx_size_64_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_size_64)},
        {"tx_size_65_to_127_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_size_65_127)},
        {"tx_size_128_to_255_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_size_128_255)},
        {"tx_size_256_to_511_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_size_256_511)},
        {"tx_size_512_to_1023_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_size_512_1023)},
        {"tx_size_1024_to_1518_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_size_1024_1518)},
        {"tx_size_1519_to_max_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_size_1519_to_max)},
        {"rx_flow_director_atr_match_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, fd_atr_match)},
        {"rx_flow_director_sb_match_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, fd_sb_match)},
        {"tx_low_power_idle_status",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_lpi_status)},
        {"rx_low_power_idle_status",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_lpi_status)},
        {"tx_low_power_idle_count",
         offsetof(struct ipn3ke_rpst_hw_port_stats, tx_lpi_count)},
        {"rx_low_power_idle_count",
         offsetof(struct ipn3ke_rpst_hw_port_stats, rx_lpi_count)},
};

#define IPN3KE_RPST_HW_PORT_XSTATS_CNT                                         \
  (sizeof(ipn3ke_rpst_hw_port_strings) / sizeof(ipn3ke_rpst_hw_port_strings[0]))

static const struct ipn3ke_rpst_xstats_name_offset
    ipn3ke_rpst_rxq_prio_strings[] = {
        {"xon_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, priority_xon_rx)},
        {"xoff_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, priority_xoff_rx)},
};
...
static const struct ipn3ke_rpst_xstats_name_offset
    ipn3ke_rpst_txq_prio_strings[] = {
        {"xon_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, priority_xon_tx)},
        {"xoff_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, priority_xoff_tx)},
        {"xon_to_xoff_packets",
         offsetof(struct ipn3ke_rpst_hw_port_stats, priority_xon_2_xoff)},
};
...
static uint32_t ipn3ke_rpst_xstats_calc_num(void) {
  return IPN3KE_RPST_ETH_XSTATS_CNT + IPN3KE_RPST_HW_PORT_XSTATS_CNT +
         (IPN3KE_RPST_RXQ_PRIO_XSTATS_CNT * IPN3KE_RPST_PRIO_XSTATS_CNT) +
         (IPN3KE_RPST_TXQ_PRIO_XSTATS_CNT * IPN3KE_RPST_PRIO_XSTATS_CNT);
}

static void ipn3ke_rpst_25g_nic_side_tx_stats_reset(struct ipn3ke_hw *hw,
                                                    uint16_t port_id) {
   ...
   */
  (*hw->f_mac_write)(hw, tmp, IPN3KE_25G_TX_STATISTICS_CONFIG, port_id, 1);

  while (tmp & 0x00000001) {
    tmp = 0x00000000;
    (*hw->f_mac_read)(hw, &tmp, IPN3KE_25G_TX_STATISTICS_CONFIG, port_id, 1);
    if (tmp & 0x00000001)
      usleep(5);
    else
      return;
...
static void ipn3ke_rpst_25g_nic_side_rx_stats_reset(struct ipn3ke_hw *hw,
                                                    uint16_t port_id) {
  uint32_t tmp = 0x00000001;
  /* Bit[0]: Software can set this bit to the value of 1
   ...
   * This bit is selfclearing.
   */
  (*hw->f_mac_write)(hw, tmp, IPN3KE_25G_RX_STATISTICS_CONFIG, port_id, 1);

  while (tmp & 0x00000001) {
    tmp = 0x00000000;
    (*hw->f_mac_read)(hw, &tmp, IPN3KE_25G_RX_STATISTICS_CONFIG, port_id, 1);
    if (tmp & 0x00000001)
      usleep(5);
    else
      return;
...
static void ipn3ke_rpst_10g_nic_side_tx_stats_reset(struct ipn3ke_hw *hw,
                                                    uint16_t port_id) {
  uint32_t tmp;
   ...
   */
  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_10G_TX_STATS_CLR, port_id, 1);
  tmp |= 0x00000001;
  (*hw->f_mac_write)(hw, tmp, IPN3KE_10G_TX_STATS_CLR, port_id, 1);
}

static void ipn3ke_rpst_10g_nic_side_rx_stats_reset(struct ipn3ke_hw *hw,
                                                    uint16_t port_id) {
  uint32_t tmp;
   ...
   */
  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_10G_RX_STATS_CLR, port_id, 1);
  tmp |= 0x00000001;
  (*hw->f_mac_write)(hw, tmp, IPN3KE_10G_RX_STATS_CLR, port_id, 1);
}

static uint64_t
ipn3ke_rpst_read_64bits_statistics_register(uint32_t addr_lo, uint32_t addr_hi,
                                            struct ipn3ke_hw *hw,
                                            uint16_t port_id) {
  ...
  (*hw->f_mac_read)(hw, &statistics_lo, addr_lo, port_id, 0);
...
static int ipn3ke_rpst_read_25g_lineside_stats_registers(
    struct ipn3ke_hw *hw, uint16_t port_id,
   ...
   */
  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_25G_TX_STATISTICS_CONFIG, port_id, 0);

  if (tmp & IPN3KE_25G_TX_STATISTICS_CONFIG_SHADOW_REQUEST_MASK) {
    tmp &= 0xfffffffb;
    (*hw->f_mac_write)(hw, tmp, IPN3KE_25G_TX_STATISTICS_CONFIG, port_id, 0);
  }

  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_25G_TX_STATISTICS_STATUS, port_id, 0);
   ...
   */
  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_25G_RX_STATISTICS_CONFIG, port_id, 0);
  if (tmp & IPN3KE_25G_RX_STATISTICS_CONFIG_SHADOW_REQUEST_MASK) {
    tmp &= 0xfffffffb;
    (*hw->f_mac_write)(hw, tmp, IPN3KE_25G_RX_STATISTICS_CONFIG, port_id, 0);
  }

  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_25G_RX_STATISTICS_STATUS, port_id, 0);
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_FRAGMENTS_LO, IPN3KE_25G_CNTR_TX_FRAGMENTS_HI, hw,
      port_id);
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_JABBERS_LO, IPN3KE_25G_CNTR_TX_JABBERS_HI, hw,
      port_id);
  hw_stats->eth.tx_errors += statistics;
  hw_stats->crc_errors += statistics;
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_FCS_LO, IPN3KE_25G_CNTR_TX_FCS_HI, hw, port_id);
  hw_stats->eth.tx_errors += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_CRCERR_LO, IPN3KE_25G_CNTR_TX_CRCERR_HI, hw, port_id);
  hw_stats->eth.tx_errors += statistics;
  hw_stats->crc_errors += statistics;
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      ...
      IPN3KE_25G_CNTR_TX_MCAST_DATA_ERR_HI, hw, port_id);
  hw_stats->eth.tx_errors += statistics;
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      ...
      IPN3KE_25G_CNTR_TX_BCAST_DATA_ERR_HI, hw, port_id);
  hw_stats->eth.tx_errors += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_1519_MAXB_LO, IPN3KE_25G_CNTR_TX_1519_MAXB_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_OVERSIZE_LO, IPN3KE_25G_CNTR_TX_OVERSIZE_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_MCAST_DATA_OK_LO, IPN3KE_25G_CNTR_TX_MCAST_DATA_OK_HI,
      hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_BCAST_DATA_OK_LO, IPN3KE_25G_CNTR_TX_BCAST_DATA_OK_HI,
      hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_UCAST_DATA_OK_LO, IPN3KE_25G_CNTR_TX_UCAST_DATA_OK_HI,
      hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_MCAST_CTRL_LO, IPN3KE_25G_CNTR_TX_MCAST_CTRL_HI, hw,
      port_id);
  hw_stats->eth.tx_multicast += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_BCAST_CTRL_LO, IPN3KE_25G_CNTR_TX_BCAST_CTRL_HI, hw,
      port_id);
  hw_stats->eth.tx_broadcast += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_UCAST_CTRL_LO, IPN3KE_25G_CNTR_TX_UCAST_CTRL_HI, hw,
      port_id);
  hw_stats->eth.tx_unicast += statistics;

  /* Number of valid pause frames transmitted */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_PAUSE_LO, IPN3KE_25G_CNTR_TX_PAUSE_HI, hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_TX_RUNT_LO, IPN3KE_25G_CNTR_TX_RUNT_HI, hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_TX_PAYLOAD_OCTETS_OK_LO, IPN3KE_25G_TX_PAYLOAD_OCTETS_OK_HI,
      hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_TX_FRAME_OCTETS_OK_LO, IPN3KE_25G_TX_FRAME_OCTETS_OK_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_FRAGMENTS_LO, IPN3KE_25G_CNTR_RX_FRAGMENTS_HI, hw,
      port_id);
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_JABBERS_LO, IPN3KE_25G_CNTR_RX_JABBERS_HI, hw,
      port_id);
  hw_stats->eth.rx_discards += statistics;
  hw_stats->crc_errors += statistics;
  hw_stats->rx_length_errors += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_FCS_LO, IPN3KE_25G_CNTR_RX_FCS_HI, hw, port_id);
  hw_stats->eth.rx_discards += statistics;
  hw_stats->checksum_error += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_CRCERR_LO, IPN3KE_25G_CNTR_RX_CRCERR_HI, hw, port_id);
  hw_stats->eth.rx_discards += statistics;
  hw_stats->crc_errors += statistics;
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      ...
      IPN3KE_25G_CNTR_RX_MCAST_DATA_ERR_HI, hw, port_id);
  hw_stats->eth.rx_discards += statistics;
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      ...
      IPN3KE_25G_CNTR_RX_BCAST_DATA_ERR_HI, hw, port_id);
  hw_stats->eth.rx_discards += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_64B_LO, IPN3KE_25G_CNTR_RX_64B_HI, hw, port_id);
  hw_stats->rx_size_64 += statistics;

  /*Number of received frames between 65 and 127 bytes */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_65_127B_LO, IPN3KE_25G_CNTR_RX_65_127B_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_128_255B_LO, IPN3KE_25G_CNTR_RX_128_255B_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_256_511B_LO, IPN3KE_25G_CNTR_RX_256_511B_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_512_1023B_LO, IPN3KE_25G_CNTR_RX_512_1023B_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_1024_1518B_LO, IPN3KE_25G_CNTR_RX_1024_1518B_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_1519_MAXB_LO, IPN3KE_25G_CNTR_RX_1519_MAXB_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_OVERSIZE_LO, IPN3KE_25G_CNTR_RX_OVERSIZE_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_MCAST_DATA_OK_LO, IPN3KE_25G_CNTR_RX_MCAST_DATA_OK_HI,
      hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_BCAST_DATA_OK_LO, IPN3KE_25G_CNTR_RX_BCAST_DATA_OK_HI,
      hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_UCAST_DATA_OK_LO, IPN3KE_25G_CNTR_RX_UCAST_DATA_OK_HI,
      hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_MCAST_CTRL_LO, IPN3KE_25G_CNTR_RX_MCAST_CTRL_HI, hw,
      port_id);
  hw_stats->eth.rx_multicast += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_BCAST_CTRL_LO, IPN3KE_25G_CNTR_RX_BCAST_CTRL_HI, hw,
      port_id);
  hw_stats->eth.rx_broadcast += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_UCAST_CTRL_LO, IPN3KE_25G_CNTR_RX_UCAST_CTRL_HI, hw,
      port_id);
  hw_stats->eth.rx_unicast += statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_PAUSE_LO, IPN3KE_25G_CNTR_RX_PAUSE_HI, hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_CNTR_RX_RUNT_LO, IPN3KE_25G_CNTR_RX_RUNT_HI, hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_RX_PAYLOAD_OCTETS_OK_LO, IPN3KE_25G_RX_PAYLOAD_OCTETS_OK_HI,
      hw, port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      IPN3KE_25G_RX_FRAME_OCTETS_OK_LO, IPN3KE_25G_RX_FRAME_OCTETS_OK_HI, hw,
      port_id);
...
static void ipn3ke_rpst_25g_lineside_tx_stats_reset(struct ipn3ke_hw *hw,
                                                    uint16_t port_id) {
  uint32_t tmp = 0x00000001;
  /* Bit[0]: Software can set this bit to the value of 1
   * to reset all of the TX statistics registers at the same time.
...
static void ipn3ke_rpst_25g_lineside_rx_stats_reset(struct ipn3ke_hw *hw,
                                                    uint16_t port_id) {
  uint32_t tmp = 0x00000001;
  /* Bit[0]: Software can set this bit to the value of 1
   * to reset all of the RX statistics registers at the same time.
...
static uint64_t
ipn3ke_rpst_read_36bits_statistics_register(uint32_t addr_lo, uint32_t addr_hi,
                                            struct ipn3ke_hw *hw,
                                            uint16_t port_id) {
  uint32_t statistics_lo = 0x00000000;
  uint32_t statistics_hi = 0x00000000;
...
static int ipn3ke_rpst_read_10g_lineside_stats_registers(
    struct ipn3ke_hw *hw, uint16_t port_id,
    struct ipn3ke_rpst_hw_port_stats *hw_stats, struct rte_eth_stats *stats) {
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      IPN3KE_10G_TX_STATS_FRAME_OK_LO, IPN3KE_10G_TX_STATS_FRAME_OK_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      IPN3KE_10G_RX_STATS_FRAME_OK_LO, IPN3KE_10G_RX_STATS_FRAME_OK_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      IPN3KE_10G_TX_STATS_FRAME_ERR_LO, IPN3KE_10G_TX_STATS_FRAME_ERR_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      IPN3KE_10G_RX_STATS_FRAME_ERR_LO, IPN3KE_10G_RX_STATS_FRAME_ERR_HI, hw,
      port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_FRAME_CRC_ERR_HI, hw, port_id);
  hw_stats->crc_errors = statistics;
   ...
   */
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      IPN3KE_10G_TX_STATS_OCTETS_OK_LO, IPN3KE_10G_TX_STATS_OCTETS_OK_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      IPN3KE_10G_RX_STATS_OCTETS_OK_LO, IPN3KE_10G_RX_STATS_OCTETS_OK_HI, hw,
      port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_PAUSE_MAC_CTRL_FRAMES_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_PAUSE_MAC_CTRL_FRAMES_HI, hw, port_id);

  /*36-bit statistics counter that collects the number of frames
   *transmitted that are invalid and with error.
   */
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      IPN3KE_10G_TX_STATS_IF_ERRORS_LO, IPN3KE_10G_TX_STATS_IF_ERRORS_HI, hw,
      port_id);
   ...
   */
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      IPN3KE_10G_RX_STATS_IF_ERRORS_LO, IPN3KE_10G_RX_STATS_IF_ERRORS_HI, hw,
      port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_UNICAST_FRAME_OK_HI, hw, port_id);
  hw_stats->eth.tx_unicast = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_UNICAST_FRAME_OK_HI, hw, port_id);
  hw_stats->eth.rx_unicast = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_UNICAST_FRAME_ERR_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_UNICAST_FRAME_ERR_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_MULTICAST_FRAME_OK_HI, hw, port_id);
  hw_stats->eth.tx_multicast = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_MULTICAST_FRAME_OK_HI, hw, port_id);
  hw_stats->eth.rx_multicast = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_MULTICAST_FRAME_ERR_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_MULTICAST_FRAME_ERR_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_BROADCAST_FRAME_OK_HI, hw, port_id);
  hw_stats->eth.tx_broadcast = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_BROADCAST_FRAME_OK_HI, hw, port_id);
  hw_stats->eth.rx_broadcast = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_BROADCAST_FRAME_ERR_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_BROADCAST_FRAME_ERR_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_OCTETS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_64bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_OCTETS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_PKTS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_PKTS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_UNDER_SIZE_PKTS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_UNDER_SIZE_PKTS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_OVER_SIZE_PKTS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_OVER_SIZE_PKTS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_PKTS_64_OCTETS_HI, hw, port_id);
  hw_stats->tx_size_64 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_PKTS_64_OCTETS_HI, hw, port_id);
  hw_stats->rx_size_64 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_PKTS_65_127_OCTETS_HI, hw, port_id);
  hw_stats->tx_size_65_127 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_PKTS_65_127_OCTETS_HI, hw, port_id);
  hw_stats->rx_size_65_127 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_PKTS_128_255_OCTETS_HI, hw, port_id);
  hw_stats->tx_size_128_255 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_PKTS_128_255_OCTETS_HI, hw, port_id);
  hw_stats->rx_size_128_255 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_PKTS_256_511_OCTETS_HI, hw, port_id);
  hw_stats->tx_size_256_511 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_PKTS_256_511_OCTETS_HI, hw, port_id);
  hw_stats->rx_size_256_511 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_PKTS_512_1023_OCTETS_HI, hw, port_id);
  hw_stats->tx_size_512_1023 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_PKTS_512_1023_OCTETS_HI, hw, port_id);
  hw_stats->rx_size_512_1023 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_PKTS_1024_1518_OCTETS_HI, hw, port_id);
  hw_stats->tx_size_1024_1518 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_PKTS_1024_1518_OCTETS_HI, hw, port_id);
  hw_stats->rx_size_1024_1518 = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_ETHER_STATS_PKTS_1519_X_OCTETS_HI, hw, port_id);
  hw_stats->tx_size_1519_to_max = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_PKTS_1519_X_OCTETS_HI, hw, port_id);
  hw_stats->rx_size_big = statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_FRAGMENTS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_JABBERS_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_ETHER_STATS_CRC_ERR_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_UNICAST_MAC_CTRL_FRAMES_HI, hw, port_id);
  hw_stats->eth.tx_unicast += statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_UNICAST_MAC_CTRL_FRAMES_HI, hw, port_id);
  hw_stats->eth.rx_unicast += statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_MULTICAST_MAC_CTRL_FRAMES_HI, hw, port_id);
  hw_stats->eth.tx_multicast += statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_MULTICAST_MAC_CTRL_FRAMES_HI, hw, port_id);
  hw_stats->eth.rx_multicast += statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_BROADCAST_MAC_CTRL_FRAMES_HI, hw, port_id);
  hw_stats->eth.tx_broadcast += statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_BROADCAST_MAC_CTRL_FRAMES_HI, hw, port_id);
  hw_stats->eth.rx_broadcast += statistics;
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_TX_STATS_PFC_MAC_CTRL_FRAMES_HI, hw, port_id);
  ...
  statistics = ipn3ke_rpst_read_36bits_statistics_register(
      ...
      IPN3KE_10G_RX_STATS_PFC_MAC_CTRL_FRAMES_HI, hw, port_id);

  return 0;
}

static void ipn3ke_rpst_10g_lineside_tx_stats_reset(struct ipn3ke_hw *hw,
                                                    uint16_t port_id) {
  uint32_t tmp;
   ...
   */
  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_10G_TX_STATS_CLR, port_id, 0);
  tmp |= 0x00000001;
  (*hw->f_mac_write)(hw, tmp, IPN3KE_10G_TX_STATS_CLR, port_id, 0);
}

static void ipn3ke_rpst_10g_lineside_rx_stats_reset(struct ipn3ke_hw *hw,
                                                    uint16_t port_id) {
  uint32_t tmp;
   ...
   */
  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_10G_RX_STATS_CLR, port_id, 0);
  tmp |= 0x00000001;
  (*hw->f_mac_write)(hw, tmp, IPN3KE_10G_RX_STATS_CLR, port_id, 0);
}

static int ipn3ke_rpst_stats_reset(struct rte_eth_dev *ethdev) {
  uint16_t port_id = 0;
  char *ch;
  int cnt = 0;
  ...
  } else if (hw->retimer.mac_type == IFPGA_RAWDEV_RETIMER_MAC_TYPE_10GE_XFI) {
...
static int ipn3ke_rpst_stats_get(struct rte_eth_dev *ethdev,
  ...
  if (hw->retimer.mac_type == IFPGA_RAWDEV_RETIMER_MAC_TYPE_25GE_25GAUI) {
    ipn3ke_rpst_read_25g_lineside_stats_registers(hw, port_id, &hw_stats);

    stats->ipackets = hw_stats.rx_size_64 + hw_stats.rx_size_65_127 +
                      hw_stats.rx_size_128_255 + hw_stats.rx_size_256_511 +
                      hw_stats.rx_size_512_1023 + hw_stats.rx_size_1024_1518 +
                      hw_stats.rx_size_big + hw_stats.rx_undersize +
                      hw_stats.rx_fragments + hw_stats.rx_oversize +
                      hw_stats.rx_jabber;
    stats->opackets = hw_stats.tx_size_64 + hw_stats.tx_size_65_127 +
                      hw_stats.tx_size_128_255 + hw_stats.tx_size_256_511 +
                      hw_stats.tx_size_512_1023 + hw_stats.tx_size_1024_1518 +
                      hw_stats.tx_size_1519_to_max;
    stats->ibytes = hw_stats.eth.rx_bytes;
    stats->obytes = hw_stats.eth.tx_bytes;
    stats->imissed = 0;
    stats->ierrors =
        hw_stats.eth.rx_discards + hw_stats.eth.rx_unknown_protocol;
    stats->oerrors = hw_stats.eth.tx_discards + hw_stats.eth.tx_errors;
    stats->rx_nombuf = 0;
    ...
    ipn3ke_rpst_read_10g_lineside_stats_registers(hw, port_id, &hw_stats,
                                                  stats);
  }
...
static int ipn3ke_rpst_xstats_get(struct rte_eth_dev *ethdev,
...
static int ipn3ke_rpst_xstats_get_names(__rte_unused struct rte_eth_dev *dev,
  ...
  for (i = 0; i < IPN3KE_RPST_RXQ_PRIO_XSTATS_CNT; i++) {
    for (prio = 0; prio < 8; prio++) {
      snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
               "rx_priority%u_%s", prio, ipn3ke_rpst_rxq_prio_strings[i].name);
      count++;
    }
  }
  ...
  for (i = 0; i < IPN3KE_RPST_TXQ_PRIO_XSTATS_CNT; i++) {
    for (prio = 0; prio < 8; prio++) {
      snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
               "tx_priority%u_%s", prio, ipn3ke_rpst_txq_prio_strings[i].name);
      count++;
    }
  }
  return count;
...
static void ipn3ke_update_link(struct rte_rawdev *rawdev, uint16_t port,
                               struct rte_eth_link *link) {
  uint64_t line_link_bitmap = 0;
  enum ifpga_rawdev_link_speed link_speed;

  rawdev->dev_ops->attr_get(rawdev, "LineSideLinkStatus",
  ...
  rawdev->dev_ops->attr_get(rawdev, "LineSideLinkSpeed",
                            (uint64_t *)&link_speed);
  switch (link_speed) {
 ...
 * Set device link up.
 */
int ipn3ke_rpst_dev_set_link_up(struct rte_eth_dev *dev) {
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(dev);
 ...
 */
int ipn3ke_rpst_dev_set_link_down(struct rte_eth_dev *dev) {
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(dev);
  struct rte_eth_dev *pf;
  int ret = 0;
...
int ipn3ke_rpst_link_update(struct rte_eth_dev *ethdev,
  ...
  link.link_autoneg =
      !(ethdev->data->dev_conf.link_speeds & RTE_ETH_LINK_SPEED_FIXED);

  rawdev = hw->rawdev;
  ipn3ke_update_link(rawdev, rpst->port_id, &link);

  if (!rpst->ori_linfo.link_status && link.link_status) {
  ...
  } else if (rpst->ori_linfo.link_status && !link.link_status) {
...
static int ipn3ke_rpst_link_check(struct ipn3ke_rpst *rpst) {
  ...
  link.link_duplex = RTE_ETH_LINK_FULL_DUPLEX;
  link.link_autoneg =
      !(rpst->ethdev->data->dev_conf.link_speeds & RTE_ETH_LINK_SPEED_FIXED);
...
static void *ipn3ke_rpst_scan_handle_request(__rte_unused void *param) {
...
#define SCAN_NUM 32
  ...
  for (;;) {
    ...
    TAILQ_FOREACH(rpst, &ipn3ke_rpst_list, next) {
      if (rpst->i40e_pf_eth && rpst->ethdev->data->dev_started &&
...
static int ipn3ke_rpst_scan_check(void) {
  int ret;

  if (ipn3ke_rpst_scan_num == 1) {
    ret = rte_ctrl_thread_create(&ipn3ke_rpst_scan_thread, "ipn3ke scanner",
                                 NULL, ipn3ke_rpst_scan_handle_request, NULL);
    if (ret) {
...
int ipn3ke_rpst_promiscuous_enable(struct rte_eth_dev *ethdev) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(ethdev);
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(ethdev);
  uint32_t rddata, val;
  ...
  if (hw->retimer.mac_type == IFPGA_RAWDEV_RETIMER_MAC_TYPE_10GE_XFI) {
    /* Enable all unicast */
    (*hw->f_mac_read)(hw, &rddata, IPN3KE_MAC_RX_FRAME_CONTROL, rpst->port_id,
                      0);
    val = 1;
    val &= IPN3KE_MAC_RX_FRAME_CONTROL_EN_ALLUCAST_MASK;
    val |= rddata;
    (*hw->f_mac_write)(hw, val, IPN3KE_MAC_RX_FRAME_CONTROL, rpst->port_id, 0);
  }

  return 0;
}

int ipn3ke_rpst_promiscuous_disable(struct rte_eth_dev *ethdev) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(ethdev);
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(ethdev);
  uint32_t rddata, val;
...
int ipn3ke_rpst_allmulticast_enable(struct rte_eth_dev *ethdev) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(ethdev);
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(ethdev);
  uint32_t rddata, val;
...
int ipn3ke_rpst_allmulticast_disable(struct rte_eth_dev *ethdev) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(ethdev);
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(ethdev);
  uint32_t rddata, val;
...
int ipn3ke_rpst_mac_addr_set(struct rte_eth_dev *ethdev,
...
int ipn3ke_rpst_mtu_set(struct rte_eth_dev *ethdev, uint16_t mtu) {
  int ret = 0;
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(ethdev);
  ...
  if (rpst->i40e_pf_eth) {
    ret = rpst->i40e_pf_eth->dev_ops->mtu_set(rpst->i40e_pf_eth, mtu);
    if (!ret)
...
static int ipn3ke_afu_flow_ops_get(struct rte_eth_dev *ethdev,
...
static uint16_t ipn3ke_rpst_xmit_pkts(__rte_unused void *tx_queue,
                                      __rte_unused struct rte_mbuf **tx_pkts,
                                      __rte_unused uint16_t nb_pkts) {
  return 0;
}

int ipn3ke_rpst_init(struct rte_eth_dev *ethdev, void *init_params) {
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(ethdev);
  struct ipn3ke_rpst *representor_param = (struct ipn3ke_rpst *)init_params;
  ...
  if (rpst->i40e_pf_eth)
    ipn3ke_bridge_func.set_i40e_sw_dev(rpst->i40e_pf_eth_port_id, rpst->ethdev);
  ...
  ethdev->data->mac_addrs =
      rte_zmalloc("ipn3ke_afu_representor", RTE_ETHER_ADDR_LEN, 0);
  if (!ethdev->data->mac_addrs) {
    IPN3KE_AFU_PMD_ERR("Failed to "
                       "allocated memory for storing mac address");
    return -ENODEV;
  ...
  ethdev->data->dev_flags |=
      RTE_ETH_DEV_REPRESENTOR | RTE_ETH_DEV_AUTOFILL_QUEUE_XSTATS;
...
int ipn3ke_rpst_uninit(struct rte_eth_dev *ethdev) {
  struct ipn3ke_rpst *rpst = IPN3KE_DEV_PRIVATE_TO_RPST(ethdev);
