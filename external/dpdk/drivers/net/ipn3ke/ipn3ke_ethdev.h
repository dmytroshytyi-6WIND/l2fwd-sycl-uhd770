...
#define IPN3KE_BIT_SET(var, pos) ((var) |= ((uint32_t)1 << ((pos))))

/** Reset the bit in the variable */
#define IPN3KE_BIT_RESET(var, pos) ((var) &= ~((uint32_t)1 << ((pos))))

/** Check the bit is set in the variable */
#define IPN3KE_BIT_ISSET(var, pos) (((var) & ((uint32_t)1 << ((pos)))) ? 1 : 0)
...
#define IPN3KE_CLASSIFY_OFFSET (IPN3KE_HW_BASE + hw->hw_cap.classify_offset)

#define IPN3KE_POLICER_OFFSET (IPN3KE_HW_BASE + hw->hw_cap.policer_offset)
...
#define IPN3KE_DMAC_MAP_OFFSET (IPN3KE_HW_BASE + hw->hw_cap.dmac_map_offset)

#define IPN3KE_QM_OFFSET (IPN3KE_HW_BASE + hw->hw_cap.qm_offset)

#define IPN3KE_CCB_OFFSET (IPN3KE_HW_BASE + hw->hw_cap.ccb_offset)

#define IPN3KE_QOS_OFFSET (IPN3KE_HW_BASE + hw->hw_cap.qos_offset)
 ...
 */
#define RTE_DEV_TO_AFU(ptr) container_of(ptr, struct rte_afu_device, device)

#define RTE_DEV_TO_AFU_CONST(ptr)                                              \
  container_of(ptr, const struct rte_afu_device, device)

#define RTE_ETH_DEV_TO_AFU(eth_dev) RTE_DEV_TO_AFU((eth_dev)->device)
...
static inline void _ipn3ke_indrct_write(struct ipn3ke_hw *hw, uint32_t addr,
                                        uint32_t value) {
  uint64_t word_offset;
  uint64_t indirect_value;
...
#define IPN3KE_READ_REG(hw, reg) _ipn3ke_indrct_read((hw), (reg))
...
#define IPN3KE_CLF_MHL_TOTAL_ENTRIES (IPN3KE_CLASSIFY_OFFSET + 0x50000 + 0x0080)
#define IPN3KE_CLF_MHL_ONEHIT_BUCKETS                                          \
  (IPN3KE_CLASSIFY_OFFSET + 0x50000 + 0x0084)
...
static inline void ipn3ke_xmac_tx_enable(struct ipn3ke_hw *hw, uint32_t mac_num,
                                         uint32_t eth_group_sel) {
#define IPN3KE_XMAC_TX_ENABLE (0 & (IPN3KE_MAC_TX_PACKET_CONTROL_MASK))

  (*hw->f_mac_write)(hw, IPN3KE_XMAC_TX_ENABLE, IPN3KE_MAC_TX_PACKET_CONTROL,
                     mac_num, eth_group_sel);
}
...
#define IPN3KE_XMAC_TX_DISABLE (1 & (IPN3KE_MAC_TX_PACKET_CONTROL_MASK))
  ...
  (*hw->f_mac_write)(hw, IPN3KE_XMAC_TX_DISABLE, IPN3KE_MAC_TX_PACKET_CONTROL,
                     mac_num, eth_group_sel);
}

static inline void ipn3ke_xmac_rx_enable(struct ipn3ke_hw *hw, uint32_t mac_num,
                                         uint32_t eth_group_sel) {
#define IPN3KE_XMAC_RX_ENABLE (0 & (IPN3KE_MAC_RX_TRANSFER_CONTROL_MASK))

  (*hw->f_mac_write)(hw, IPN3KE_XMAC_RX_ENABLE, IPN3KE_MAC_RX_TRANSFER_CONTROL,
                     mac_num, eth_group_sel);
}
...
#define IPN3KE_XMAC_RX_DISABLE (1 & (IPN3KE_MAC_RX_TRANSFER_CONTROL_MASK))
  ...
  (*hw->f_mac_write)(hw, IPN3KE_XMAC_RX_DISABLE, IPN3KE_MAC_RX_TRANSFER_CONTROL,
                     mac_num, eth_group_sel);
}
...
static inline void ipn3ke_xmac_smac_ovd_dis(struct ipn3ke_hw *hw,
                                            uint32_t mac_num,
                                            uint32_t eth_group_sel) {
#define IPN3KE_XMAC_SMAC_OVERRIDE_DISABLE                                      \
  (0 & (IPN3KE_MAC_TX_SRC_ADDR_OVERRIDE_MASK))

  (*hw->f_mac_write)(hw, IPN3KE_XMAC_SMAC_OVERRIDE_DISABLE,
                     IPN3KE_MAC_TX_SRC_ADDR_OVERRIDE, mac_num, eth_group_sel);
}
...
static inline void ipn3ke_xmac_tx_clr_10G_stcs(struct ipn3ke_hw *hw,
  ...
  uint32_t tmp;
  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_10G_TX_STATS_CLR, mac_num, eth_group_sel);
  tmp |= 0x00000001;
  (*hw->f_mac_write)(hw, tmp, IPN3KE_10G_TX_STATS_CLR, mac_num, eth_group_sel);
}
...
static inline void ipn3ke_xmac_rx_clr_10G_stcs(struct ipn3ke_hw *hw,
  ...
  uint32_t tmp;
  tmp = 0x00000000;
  (*hw->f_mac_read)(hw, &tmp, IPN3KE_10G_RX_STATS_CLR, mac_num, eth_group_sel);
  tmp |= 0x00000001;
  (*hw->f_mac_write)(hw, tmp, IPN3KE_10G_RX_STATS_CLR, mac_num, eth_group_sel);
}
...
static inline void ipn3ke_xmac_tx_clr_25G_stcs(struct ipn3ke_hw *hw,
   ...
   */
  (*hw->f_mac_write)(hw, tmp, IPN3KE_25G_TX_STATISTICS_CONFIG, mac_num,
                     eth_group_sel);
}
...
static inline void ipn3ke_xmac_rx_clr_25G_stcs(struct ipn3ke_hw *hw,
   ...
   * This bit is selfclearing.
   */
  (*hw->f_mac_write)(hw, tmp, IPN3KE_25G_RX_STATISTICS_CONFIG, mac_num,
                     eth_group_sel);
}
