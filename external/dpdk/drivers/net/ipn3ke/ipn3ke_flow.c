 ...
 */
static int ipn3ke_pattern_vxlan(const struct rte_flow_item patterns[],
                                struct rte_flow_error *error,
                                struct ipn3ke_flow_parse *parser) {
  ...
  for (item = patterns; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    ...
    case RTE_FLOW_ITEM_TYPE_ETH:
      ...
      rte_memcpy(&parser->key[0], eth->src.addr_bytes, RTE_ETHER_ADDR_LEN);
      break;
  ...
  rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, patterns,
 ...
 * RTE_FLOW_ITEM_TYPE_ETH
 */
static int ipn3ke_pattern_mac(const struct rte_flow_item patterns[],
                              struct rte_flow_error *error,
                              struct ipn3ke_flow_parse *parser) {
  const struct rte_flow_item_eth *eth = NULL;
  const struct rte_flow_item *item;
  ...
  for (item = patterns; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    ...
    case RTE_FLOW_ITEM_TYPE_ETH:
      ...
      rte_memcpy(parser->key, eth->src.addr_bytes, RTE_ETHER_ADDR_LEN);
      break;
 ...
 * RTE_FLOW_ITEM_TYPE_VLAN
 */
static int ipn3ke_pattern_qinq(const struct rte_flow_item patterns[],
                               struct rte_flow_error *error,
                               struct ipn3ke_flow_parse *parser) {
 ...
 * RTE_FLOW_ITEM_TYPE_MPLS
 */
static int ipn3ke_pattern_mpls(const struct rte_flow_item patterns[],
                               struct rte_flow_error *error,
                               struct ipn3ke_flow_parse *parser) {
  ...
  for (item = patterns; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    ...
    case RTE_FLOW_ITEM_TYPE_MPLS:
      if (!mpls1) {
        ...
        parser->key[2] |= ((mpls2->label_tc_s[0] & 0xf0) >> 4);
        parser->key[3] = ((mpls2->label_tc_s[0] & 0xf) << 4) |
                         ((mpls2->label_tc_s[1] & 0xf0) >> 4);
        parser->key[4] = ((mpls2->label_tc_s[1] & 0xf) << 4) |
 ...
 */
static int ipn3ke_pattern_ip_tcp(const struct rte_flow_item patterns[],
                                 struct rte_flow_error *error,
                                 struct ipn3ke_flow_parse *parser) {
 ...
 */
static int ipn3ke_pattern_ip_udp(const struct rte_flow_item patterns[],
                                 struct rte_flow_error *error,
                                 struct ipn3ke_flow_parse *parser) {
  const struct rte_flow_item_ipv4 *ipv4 = NULL;
 ...
 */
static int ipn3ke_pattern_ip_nvgre(const struct rte_flow_item patterns[],
                                   struct rte_flow_error *error,
                                   struct ipn3ke_flow_parse *parser) {
 ...
 * RTE_FLOW_ITEM_TYPE_UDP
 */
static int ipn3ke_pattern_vxlan_ip_udp(const struct rte_flow_item patterns[],
                                       struct rte_flow_error *error,
                                       struct ipn3ke_flow_parse *parser) {
  const struct rte_flow_item_vxlan *vxlan = NULL;
  const struct rte_flow_item_ipv4 *ipv4 = NULL;
...
static const struct ipn3ke_flow_pattern ipn3ke_supported_patterns[] = {
    [IPN3KE_HASH_KEY_VXLAN] =
        {
            .items =
                FLOW_PATTERNS(RTE_FLOW_ITEM_TYPE_VXLAN, RTE_FLOW_ITEM_TYPE_ETH),
    ...
    [IPN3KE_HASH_KEY_QINQ] =
        {
            .items =
                FLOW_PATTERNS(RTE_FLOW_ITEM_TYPE_VLAN, RTE_FLOW_ITEM_TYPE_VLAN),
    ...
    [IPN3KE_HASH_KEY_MPLS] =
        {
            .items =
                FLOW_PATTERNS(RTE_FLOW_ITEM_TYPE_MPLS, RTE_FLOW_ITEM_TYPE_MPLS),
    ...
    [IPN3KE_HASH_KEY_IP_TCP] =
        {
            .items =
                FLOW_PATTERNS(RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_TCP),
    ...
    [IPN3KE_HASH_KEY_IP_UDP] =
        {
            .items =
                FLOW_PATTERNS(RTE_FLOW_ITEM_TYPE_IPV4, RTE_FLOW_ITEM_TYPE_UDP),
    ...
    [IPN3KE_HASH_KEY_VXLAN_IP_UDP] =
        {
            .items =
                FLOW_PATTERNS(RTE_FLOW_ITEM_TYPE_VXLAN, RTE_FLOW_ITEM_TYPE_IPV4,
                              RTE_FLOW_ITEM_TYPE_UDP),
            .filter = ipn3ke_pattern_vxlan_ip_udp,
        },
};

static int ipn3ke_flow_convert_attributes(const struct rte_flow_attr *attr,
                                          struct rte_flow_error *error) {
  if (!attr) {
...
static int ipn3ke_flow_convert_actions(const struct rte_flow_action actions[],
                                       struct rte_flow_error *error,
                                       struct ipn3ke_flow_parse *parser) {
...
static bool ipn3ke_match_pattern(const enum rte_flow_item_type *patterns,
                                 const struct rte_flow_item *input) {
  const struct rte_flow_item *item = input;

  while ((*patterns == item->type) && (*patterns != RTE_FLOW_ITEM_TYPE_END)) {
    patterns++;
    item++;
  }
...
ipn3ke_find_filter_func(const struct rte_flow_item *input, uint32_t *idx) {
  pattern_filter_t filter = NULL;
  uint32_t i;

  for (i = 0; i < RTE_DIM(ipn3ke_supported_patterns); i++) {
    if (ipn3ke_match_pattern(ipn3ke_supported_patterns[i].items, input)) {
...
static int ipn3ke_flow_convert_items(const struct rte_flow_item items[],
                                     struct rte_flow_error *error,
                                     struct ipn3ke_flow_parse *parser) {
  pattern_filter_t filter = NULL;
  uint32_t idx;
  ...
  if (!filter) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items,
                       "Unsupported pattern");
    return -rte_errno;
  }
 ...
 *     offset
 */
static void copy_data_bits(uint8_t *dst, uint64_t data, uint32_t offset,
                           uint8_t nbits) {
...
static void ipn3ke_flow_key_generation(struct ipn3ke_flow_parse *parser,
  ...
  copy_data_bits(dst, parser->key_type, IPN3KE_FLOW_KEY_ID_OFFSET,
  ...
  offset = (IPN3KE_FLOW_KEY_DATA_OFFSET + IPN3KE_FLOW_KEY_DATA_BITS -
            parser->key_len);
  ...
  for (i = 0; i < len_in_bytes; i++) {
    ...
    if (++shift_bytes == sizeof(key)) {
      ...
      copy_data_bits(dst, key, offset, sizeof(key) * BITS_PER_BYTE);
...
static void ipn3ke_flow_result_generation(struct ipn3ke_flow_parse *parser,
                                          struct rte_flow *flow) {
  uint8_t *dst;
  ...
  copy_data_bits(dst, 1, IPN3KE_FLOW_RESULT_ACL_OFFSET,
                 IPN3KE_FLOW_RESULT_ACL_BITS);

  copy_data_bits(dst, parser->mark_id, IPN3KE_FLOW_RESULT_UID_OFFSET,
...
static int ipn3ke_flow_hw_update(struct ipn3ke_hw *hw, struct rte_flow *flow,
                                 uint32_t is_add) {
  uint32_t *pdata = NULL;
  uint32_t data;
  ...
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_KEY_0, 0, ipn3ke_swap32(pdata[3]),
                        IPN3KE_CLF_MHL_KEY_MASK);

  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_KEY_1, 0, ipn3ke_swap32(pdata[2]),
                        IPN3KE_CLF_MHL_KEY_MASK);

  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_KEY_2, 0, ipn3ke_swap32(pdata[1]),
                        IPN3KE_CLF_MHL_KEY_MASK);

  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_KEY_3, 0, ipn3ke_swap32(pdata[0]),
                        IPN3KE_CLF_MHL_KEY_MASK);

  pdata = (uint32_t *)flow->rule.result;
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_RES, 0, ipn3ke_swap32(pdata[0]),
                        IPN3KE_CLF_MHL_RES_MASK);

  /* insert/delete the key and result */
  data = 0;
  data = IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_MHL_MGMT_CTRL, 0, 0x80000000);
  ...
  if (!time_out)
    return -1;
  if (is_add)
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_MGMT_CTRL, 0,
                          IPN3KE_CLF_MHL_MGMT_CTRL_INSERT, 0x3);
  else
    IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_MGMT_CTRL, 0,
                          IPN3KE_CLF_MHL_MGMT_CTRL_DELETE, 0x3);

  return 0;
}

static int ipn3ke_flow_hw_flush(struct ipn3ke_hw *hw) {
  uint32_t data;
  uint32_t time_out = MHL_COMMAND_TIME_COUNT;
  ...
  if (!time_out)
    return -1;
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_MGMT_CTRL, 0,
                        IPN3KE_CLF_MHL_MGMT_CTRL_FLUSH, 0x3);

  return 0;
}

static void ipn3ke_flow_convert_finalise(struct ipn3ke_hw *hw,
                                         struct ipn3ke_flow_parse *parser,
                                         struct rte_flow *flow) {
...
static int ipn3ke_flow_convert(const struct rte_flow_attr *attr,
...
static int ipn3ke_flow_validate(__rte_unused struct rte_eth_dev *dev,
...
ipn3ke_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
                   const struct rte_flow_item pattern[],
                   const struct rte_flow_action actions[],
                   struct rte_flow_error *error) {
...
static int ipn3ke_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                               struct rte_flow_error *error) {
  struct ipn3ke_hw *hw = IPN3KE_DEV_PRIVATE_TO_HW(dev);
  int ret = 0;
...
static int ipn3ke_flow_flush(struct rte_eth_dev *dev,
...
int ipn3ke_flow_init(void *dev) {
  ...
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_RX_TEST, 0, 0, 0x1);

  data = 0;
  data = IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_RX_TEST, 0, 0x1);
  IPN3KE_AFU_PMD_DEBUG("IPN3KE_CLF_RX_TEST: %x\n", data);

  /* configure base mac address */
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_BASE_DST_MAC_ADDR_HI, 0, 0x2457, 0xFFFF);

  data = 0;
  data = IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_BASE_DST_MAC_ADDR_HI, 0, 0xFFFF);
  IPN3KE_AFU_PMD_DEBUG("IPN3KE_CLF_BASE_DST_MAC_ADDR_HI: %x\n", data);

  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_BASE_DST_MAC_ADDR_LOW, 0, 0x9bdf1000,
                        0xFFFFFFFF);

  data = 0;
  data =
      IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_BASE_DST_MAC_ADDR_LOW, 0, 0xFFFFFFFF);
  IPN3KE_AFU_PMD_DEBUG("IPN3KE_CLF_BASE_DST_MAC_ADDR_LOW: %x\n", data);

  /* configure hash lookup rules enable */
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_LKUP_ENABLE, 0, 0xFD, 0xFF);

  data = 0;
  data = IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_LKUP_ENABLE, 0, 0xFF);
  IPN3KE_AFU_PMD_DEBUG("IPN3KE_CLF_LKUP_ENABLE: %x\n", data);

  /* configure rx parse config, settings associated with VxLAN */
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_RX_PARSE_CFG, 0, 0x212b5, 0x3FFFF);

  data = 0;
  data = IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_RX_PARSE_CFG, 0, 0x3FFFF);
  IPN3KE_AFU_PMD_DEBUG("IPN3KE_CLF_RX_PARSE_CFG: %x\n", data);

  /* configure QinQ S-Tag */
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_QINQ_STAG, 0, 0x88a8, 0xFFFF);

  data = 0;
  data = IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_QINQ_STAG, 0, 0xFFFF);
  IPN3KE_AFU_PMD_DEBUG("IPN3KE_CLF_QINQ_STAG: %x\n", data);

  /* configure gen ctrl */
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_GEN_CTRL, 0, 0x3, 0x3);

  data = 0;
  data = IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_MHL_GEN_CTRL, 0, 0x1F);
  IPN3KE_AFU_PMD_DEBUG("IPN3KE_CLF_MHL_GEN_CTRL: %x\n", data);

  /* clear monitoring register */
  IPN3KE_MASK_WRITE_REG(hw, IPN3KE_CLF_MHL_MON_0, 0, 0xFFFFFFFF, 0xFFFFFFFF);

  data = 0;
  data = IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_MHL_MON_0, 0, 0xFFFFFFFF);
  ...
  hw->flow_max_entries =
      IPN3KE_MASK_READ_REG(hw, IPN3KE_CLF_EM_NUM, 0, 0xFFFFFFFF);
