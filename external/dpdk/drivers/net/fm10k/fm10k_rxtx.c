...
static inline void dump_rxd(union fm10k_rx_desc *rxd) {
  ...
  PMD_RX_LOG(DEBUG, "|   0x%08x   |   0x%08x   |", rxd->d.glort, rxd->d.data);
  PMD_RX_LOG(DEBUG, "+----------------|----------------+");
 ...
 */
static inline void rx_desc_to_ol_flags(struct rte_mbuf *m,
                                       const union fm10k_rx_desc *d) {
  static const uint32_t
      ptype_table[FM10K_RXD_PKTTYPE_MASK >>
                  FM10K_RXD_PKTTYPE_SHIFT] __rte_cache_aligned = {
          [FM10K_PKTTYPE_OTHER] = RTE_PTYPE_L2_ETHER,
          [FM10K_PKTTYPE_IPV4] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4,
          [FM10K_PKTTYPE_IPV4_EX] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT,
          [FM10K_PKTTYPE_IPV6] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6,
          [FM10K_PKTTYPE_IPV6_EX] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT,
          [FM10K_PKTTYPE_IPV4 | FM10K_PKTTYPE_TCP] =
              RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP,
          [FM10K_PKTTYPE_IPV6 | FM10K_PKTTYPE_TCP] =
              RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP,
          [FM10K_PKTTYPE_IPV4 | FM10K_PKTTYPE_UDP] =
              RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP,
          [FM10K_PKTTYPE_IPV6 | FM10K_PKTTYPE_UDP] =
  ...
  if (unlikely(
          (d->d.staterr & (FM10K_RXD_STATUS_IPCS | FM10K_RXD_STATUS_IPE)) ==
  ...
  if (unlikely(
          (d->d.staterr & (FM10K_RXD_STATUS_L4CS | FM10K_RXD_STATUS_L4E)) ==
...
uint16_t fm10k_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  if ((q->next_dd > q->next_trigger) || (alloc == 1)) {
    ret = rte_mempool_get_bulk(q->mp, (void **)&q->sw_ring[q->next_alloc],
                               q->alloc_thresh);
    ...
    if (unlikely(ret != 0)) {
       ...
       */
      q->next_dd = (q->next_dd + q->nb_desc - count) % q->nb_desc;
...
uint16_t fm10k_recv_scattered_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                                   uint16_t nb_pkts) {
  struct rte_mbuf *mbuf;
  union fm10k_rx_desc desc;
  struct fm10k_rx_queue *q = rx_queue;
  ...
  for (count = 0; count < nb_seg; count++) {
    ...
    if (!first_seg) {
      ...
      first_seg->pkt_len =
          (uint16_t)(first_seg->pkt_len + rte_pktmbuf_data_len(mbuf));
      first_seg->nb_segs++;
      last_seg->next = mbuf;
    }
...
uint32_t fm10k_dev_rx_queue_count(void *rx_queue) {
#define FM10K_RXQ_SCAN_INTERVAL 4
  ...
  while ((desc < rxq->nb_desc) &&
    ...
    if (rxq->next_dd + desc >= rxq->nb_desc)
      rxdp = &rxq->hw_ring[rxq->next_dd + desc - rxq->nb_desc];
  }

  return desc;
}

int fm10k_dev_rx_descriptor_status(void *rx_queue, uint16_t offset) {
  volatile union fm10k_rx_desc *rxdp;
  ...
  if (rxq->next_trigger < rxq->alloc_thresh)
    trigger_last = rxq->next_trigger + rxq->nb_desc - rxq->alloc_thresh;
  else
  ...
  ret = !!(rxdp->w.status & rte_cpu_to_le_16(FM10K_RXD_STATUS_DD));

  return ret;
}

int fm10k_dev_tx_descriptor_status(void *tx_queue, uint16_t offset) {
  ...
  desc = (desc / txq->rs_thresh + 1) * txq->rs_thresh - 1;
...
static inline void tx_xmit_pkt(struct fm10k_tx_queue *q, struct rte_mbuf *mb) {
  ...
  q->hw_ring[q->next_free].buffer_addr = rte_cpu_to_le_64(MBUF_DMA_ADDR(mb));
  q->hw_ring[q->next_free].buflen = rte_cpu_to_le_16(rte_pktmbuf_data_len(mb));

  if (mb->ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    hdrlen = mb->l2_len + mb->l3_len + mb->l4_len;
    hdrlen += (mb->ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK)
                  ? mb->outer_l2_len + mb->outer_l3_len
                  : 0;
...
uint16_t fm10k_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (count = 0; count < nb_pkts; ++count) {
    ...
    if ((mb->nb_segs == 0) || ((mb->nb_segs > 1) && (mb->next == NULL)))
      break;
...
uint16_t fm10k_prep_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts,
                         uint16_t nb_pkts) {
  int i, ret;
  struct rte_mbuf *m;
