...
s32 fm10k_init_hw(struct fm10k_hw *hw) {
  return fm10k_call_func(hw, hw->mac.ops.init_hw, (hw), FM10K_NOT_IMPLEMENTED);
}
...
s32 fm10k_stop_hw(struct fm10k_hw *hw) {
  return fm10k_call_func(hw, hw->mac.ops.stop_hw, (hw), FM10K_NOT_IMPLEMENTED);
}
...
s32 fm10k_configure_dglort_map(struct fm10k_hw *hw,
  ...
  return fm10k_call_func(hw, hw->mac.ops.configure_dglort_map, (hw, dglort),
                         FM10K_NOT_IMPLEMENTED);
}
 ...
 **/
s32 fm10k_update_uc_addr(struct fm10k_hw *hw, u16 lport, const u8 *mac, u16 vid,
                         bool add, u8 flags) {
 ...
 **/
s32 fm10k_update_mc_addr(struct fm10k_hw *hw, u16 lport, const u8 *mac, u16 vid,
                         bool add) {
  return fm10k_call_func(hw, hw->mac.ops.update_mc_addr,
                         (hw, lport, mac, vid, add), FM10K_NOT_IMPLEMENTED);
}
...
s32 fm10k_adjust_systime(struct fm10k_hw *hw, s32 ppb) {
  return fm10k_call_func(hw, hw->mac.ops.adjust_systime, (hw, ppb),
                         FM10K_NOT_IMPLEMENTED);
}
...
s32 fm10k_notify_offset(struct fm10k_hw *hw, u64 offset) {
  return fm10k_call_func(hw, hw->mac.ops.notify_offset, (hw, offset),
                         FM10K_NOT_IMPLEMENTED);
}
