...
STATIC s32 fm10k_init_hw_pf(struct fm10k_hw *hw) {
  ...
  for (i = 0; i < FM10K_MAX_QUEUES; i++) {
    ...
    FM10K_WRITE_REG(hw, FM10K_TPH_TXCTRL(i),
                    FM10K_TPH_TXCTRL_DESC_TPHEN | FM10K_TPH_TXCTRL_DESC_RROEN |
                        FM10K_TPH_TXCTRL_DESC_WROEN |
                        FM10K_TPH_TXCTRL_DATA_RROEN);
    FM10K_WRITE_REG(hw, FM10K_TPH_RXCTRL(i),
                    FM10K_TPH_RXCTRL_DESC_TPHEN | FM10K_TPH_RXCTRL_DESC_RROEN |
 ...
 **/
STATIC s32 fm10k_update_vlan_pf(struct fm10k_hw *hw, u32 vid, u8 vsi,
                                bool set) {
  ...
  for (reg = FM10K_VLAN_TABLE(vsi, vid / 32), bit = vid % 32;
       len < FM10K_VLAN_TABLE_VID_MAX; len -= 32 - bit, reg++, bit = 0) {
...
STATIC s32 fm10k_update_xc_addr_pf(struct fm10k_hw *hw, u16 glort,
  ...
  /* record fields */
  mac_update.mac_lower =
      FM10K_CPU_TO_LE32(((u32)mac[2] << 24) | ((u32)mac[3] << 16) |
                        ((u32)mac[4] << 8) | ((u32)mac[5]));
  mac_update.mac_upper = FM10K_CPU_TO_LE16(((u16)mac[0] << 8) | ((u16)mac[1]));
  ...
  fm10k_tlv_attr_put_le_struct(msg, FM10K_PF_ATTR_ID_MAC_UPDATE, &mac_update,
                               sizeof(mac_update));
...
STATIC s32 fm10k_configure_dglort_map_pf(struct fm10k_hw *hw,
  ...
  if ((dglort->idx > 7) || (dglort->rss_l > 7) || (dglort->pc_l > 3) ||
      (dglort->vsi_l > 6) || (dglort->vsi_b > 64) || (dglort->queue_l > 8) ||
      (dglort->queue_b >= 256))
    return FM10K_ERR_PARAM;
  ...
  /* configure DGLORTMAP */
  dglortmap = (dglort->idx == fm10k_dglort_default) ? FM10K_DGLORTMAP_ANY
...
STATIC u16 fm10k_vectors_per_pool(struct fm10k_hw *hw) {
  ...
  return (num_pools > 32) ? 8 : (num_pools > 16) ? 16 : FM10K_MAX_VECTORS_POOL;
}
...
STATIC s32 fm10k_iov_assign_resources_pf(struct fm10k_hw *hw, u16 num_vfs,
  ...
  for (i = 0; i < num_vfs; i++) {
    ...
    FM10K_WRITE_REG(hw, FM10K_TC_CREDIT(i), FM10K_TC_CREDIT_CREDIT_MASK);
  }
  ...
  for (i = FM10K_MAX_QUEUES_PF; i < vf_q_idx; i++) {
    ...
    FM10K_WRITE_REG(hw, FM10K_TXQCTL(i),
                    FM10K_TXQCTL_PF | FM10K_TXQCTL_UNLIMITED_BW | vid);
  ...
  FM10K_WRITE_REG(hw, FM10K_ITR2(0), fm10k_vf_vector_index(hw, num_vfs - 1));
  ...
  for (i = 0; i < num_vfs; i++) {
    ...
    for (j = 0; j < qpp; j++, qmap_idx++, vf_q_idx++) {
      ...
      FM10K_WRITE_REG(hw, FM10K_TXQCTL(vf_q_idx),
                      (i << FM10K_TXQCTL_TC_SHIFT) | i | FM10K_TXQCTL_VF | vid);
 ...
 **/
STATIC s32 fm10k_iov_configure_tc_pf(struct fm10k_hw *hw, u16 vf_idx,
                                     int rate) {
  /* configure defaults */
...
STATIC s32 fm10k_iov_assign_default_mac_vlan_pf(struct fm10k_hw *hw,
  ...
  fm10k_tlv_attr_put_mac_vlan(msg, FM10K_MAC_VLAN_MSG_DEFAULT_MAC, vf_info->mac,
                              vf_vid);
   ...
   */
  txqctl = ((u32)vf_vid << FM10K_TXQCTL_VID_SHIFT) & FM10K_TXQCTL_VID_MASK;
  txqctl |= (vf_idx << FM10K_TXQCTL_TC_SHIFT) | FM10K_TXQCTL_VF | vf_idx;
  ...
  if (IS_VALID_ETHER_ADDR(vf_info->mac)) {
    tdbal = (((u32)vf_info->mac[3]) << 24) | (((u32)vf_info->mac[4]) << 16) |
            (((u32)vf_info->mac[5]) << 8);

    tdbah = (((u32)0xFF) << 24) | (((u32)vf_info->mac[0]) << 16) |
            (((u32)vf_info->mac[1]) << 8) | ((u32)vf_info->mac[2]);
  }
   ...
   */
  FM10K_WRITE_REG(hw, FM10K_TDLEN(vf_q_idx),
                  hw->mac.itr_scale << FM10K_TDLEN_ITR_SCALE_SHIFT);
...
STATIC s32 fm10k_iov_reset_resources_pf(struct fm10k_hw *hw,
  ...
  txqctl = ((u32)vf_vid << FM10K_TXQCTL_VID_SHIFT) |
           (vf_idx << FM10K_TXQCTL_TC_SHIFT) | FM10K_TXQCTL_VF | vf_idx;
  ...
  FM10K_WRITE_REG(hw, FM10K_TC_CREDIT(vf_idx), FM10K_TC_CREDIT_CREDIT_MASK);
...
STATIC s32 fm10k_iov_set_lport_pf(struct fm10k_hw *hw,
                                  struct fm10k_vf_info *vf_info, u16 lport_idx,
                                  u8 flags) {
...
STATIC void fm10k_iov_update_stats_pf(struct fm10k_hw *hw,
                                      struct fm10k_hw_stats_q *q, u16 vf_idx) {
  u32 idx, qpp;
...
s32 fm10k_iov_msg_mac_vlan_pf(struct fm10k_hw *hw, u32 **results,
  ...
  if (!err && !!results[FM10K_MAC_VLAN_MSG_MAC]) {
    ...
    err = hw->mac.ops.update_uc_addr(hw, vf_info->glort, mac, vlan, set, 0);
  }
  ...
  if (!err && !!results[FM10K_MAC_VLAN_MSG_MULTICAST]) {
    ...
    err = hw->mac.ops.update_mc_addr(hw, vf_info->glort, mac, vlan, set);
  }
...
s32 fm10k_iov_msg_lport_state_pf(struct fm10k_hw *hw, u32 **results,
  ...
  if (!err && (!FM10K_VF_FLAG_ENABLED(vf_info) != !mode))
    err = fm10k_update_lport_state_pf(hw, vf_info->glort, 1, !!mode);
...
void fm10k_update_hw_stats_pf(struct fm10k_hw *hw,
  ...
  do {
    timeout = fm10k_read_hw_stats_32b(hw, FM10K_STATS_TIMEOUT, &stats->timeout);
    ...
    vlan_drop =
        fm10k_read_hw_stats_32b(hw, FM10K_STATS_VLAN_DROP, &stats->vlan_drop);
    loopback_drop = fm10k_read_hw_stats_32b(hw, FM10K_STATS_LOOPBACK_DROP,
                                            &stats->loopback_drop);
    nodesc_drop = fm10k_read_hw_stats_32b(hw, FM10K_STATS_NODESC_DROP,
                                          &stats->nodesc_drop);
...
STATIC s32 fm10k_get_fault_pf(struct fm10k_hw *hw, int type,
  ...
  if (func & FM10K_FAULT_FUNC_PF)
    ...
    fault->func =
        1 + ((func & FM10K_FAULT_FUNC_VF_MASK) >> FM10K_FAULT_FUNC_VF_SHIFT);
...
const struct fm10k_tlv_attr fm10k_lport_map_msg_attr[] = {
    ...
    FM10K_TLV_ATTR_U32(FM10K_PF_ATTR_ID_LPORT_MAP), FM10K_TLV_ATTR_LAST};
...
s32 fm10k_msg_lport_map_pf(struct fm10k_hw *hw, u32 **results,
  ...
  err =
      fm10k_tlv_attr_get_u32(results[FM10K_PF_ATTR_ID_LPORT_MAP], &dglort_map);
  if (err)
    return err;
...
const struct fm10k_tlv_attr fm10k_update_pvid_msg_attr[] = {
    FM10K_TLV_ATTR_U32(FM10K_PF_ATTR_ID_UPDATE_PVID), FM10K_TLV_ATTR_LAST};
...
s32 fm10k_msg_err_pf(struct fm10k_hw *hw, u32 **results,
  ...
  err = fm10k_tlv_attr_get_le_struct(results[FM10K_PF_ATTR_ID_ERR], &err_msg,
                                     sizeof(err_msg));
  if (err)
    return err;
...
s32 fm10k_msg_1588_clock_owner_pf(struct fm10k_hw *hw, u32 **results,
  ...
  err = fm10k_tlv_attr_get_le_struct(results[FM10K_PF_ATTR_ID_1588_CLOCK_OWNER],
                                     &msg, sizeof(msg));
  if (err)
    return err;
