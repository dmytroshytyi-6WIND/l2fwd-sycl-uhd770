 ...
 **/
STATIC void fm10k_fifo_init(struct fm10k_mbx_fifo *fifo, u32 *buffer,
                            u16 size) {
  fifo->buffer = buffer;
  fifo->size = size;
  fifo->head = 0;
 ...
 **/
STATIC void fm10k_fifo_write_copy(struct fm10k_mbx_fifo *fifo, const u32 *msg,
...
STATIC void fm10k_mbx_pull_head(struct fm10k_hw *hw, struct fm10k_mbx_info *mbx,
  ...
  for (len = fm10k_fifo_head_len(fifo); len && (mbx->pulled >= len);
...
STATIC s32 fm10k_mbx_push_tail(struct fm10k_hw *hw, struct fm10k_mbx_info *mbx,
  ...
  for (len = fm10k_mbx_pushed_tail_len(mbx); len && (mbx->pushed >= len);
...
static const u16 fm10k_crc_16b_table[256] = {
    0x0000, 0x7956, 0xF2AC, 0x8BFA, 0xBC6D, 0xC53B, 0x4EC1, 0x3797, 0x21EF,
    0x58B9, 0xD343, 0xAA15, 0x9D82, 0xE4D4, 0x6F2E, 0x1678, 0x43DE, 0x3A88,
    0xB172, 0xC824, 0xFFB3, 0x86E5, 0x0D1F, 0x7449, 0x6231, 0x1B67, 0x909D,
    0xE9CB, 0xDE5C, 0xA70A, 0x2CF0, 0x55A6, 0x87BC, 0xFEEA, 0x7510, 0x0C46,
    0x3BD1, 0x4287, 0xC97D, 0xB02B, 0xA653, 0xDF05, 0x54FF, 0x2DA9, 0x1A3E,
    0x6368, 0xE892, 0x91C4, 0xC462, 0xBD34, 0x36CE, 0x4F98, 0x780F, 0x0159,
    0x8AA3, 0xF3F5, 0xE58D, 0x9CDB, 0x1721, 0x6E77, 0x59E0, 0x20B6, 0xAB4C,
    0xD21A, 0x564D, 0x2F1B, 0xA4E1, 0xDDB7, 0xEA20, 0x9376, 0x188C, 0x61DA,
    0x77A2, 0x0EF4, 0x850E, 0xFC58, 0xCBCF, 0xB299, 0x3963, 0x4035, 0x1593,
    0x6CC5, 0xE73F, 0x9E69, 0xA9FE, 0xD0A8, 0x5B52, 0x2204, 0x347C, 0x4D2A,
    0xC6D0, 0xBF86, 0x8811, 0xF147, 0x7ABD, 0x03EB, 0xD1F1, 0xA8A7, 0x235D,
    0x5A0B, 0x6D9C, 0x14CA, 0x9F30, 0xE666, 0xF01E, 0x8948, 0x02B2, 0x7BE4,
    0x4C73, 0x3525, 0xBEDF, 0xC789, 0x922F, 0xEB79, 0x6083, 0x19D5, 0x2E42,
    0x5714, 0xDCEE, 0xA5B8, 0xB3C0, 0xCA96, 0x416C, 0x383A, 0x0FAD, 0x76FB,
    0xFD01, 0x8457, 0xAC9A, 0xD5CC, 0x5E36, 0x2760, 0x10F7, 0x69A1, 0xE25B,
    0x9B0D, 0x8D75, 0xF423, 0x7FD9, 0x068F, 0x3118, 0x484E, 0xC3B4, 0xBAE2,
    0xEF44, 0x9612, 0x1DE8, 0x64BE, 0x5329, 0x2A7F, 0xA185, 0xD8D3, 0xCEAB,
    0xB7FD, 0x3C07, 0x4551, 0x72C6, 0x0B90, 0x806A, 0xF93C, 0x2B26, 0x5270,
    0xD98A, 0xA0DC, 0x974B, 0xEE1D, 0x65E7, 0x1CB1, 0x0AC9, 0x739F, 0xF865,
    0x8133, 0xB6A4, 0xCFF2, 0x4408, 0x3D5E, 0x68F8, 0x11AE, 0x9A54, 0xE302,
    0xD495, 0xADC3, 0x2639, 0x5F6F, 0x4917, 0x3041, 0xBBBB, 0xC2ED, 0xF57A,
    0x8C2C, 0x07D6, 0x7E80, 0xFAD7, 0x8381, 0x087B, 0x712D, 0x46BA, 0x3FEC,
    0xB416, 0xCD40, 0xDB38, 0xA26E, 0x2994, 0x50C2, 0x6755, 0x1E03, 0x95F9,
    0xECAF, 0xB909, 0xC05F, 0x4BA5, 0x32F3, 0x0564, 0x7C32, 0xF7C8, 0x8E9E,
    0x98E6, 0xE1B0, 0x6A4A, 0x131C, 0x248B, 0x5DDD, 0xD627, 0xAF71, 0x7D6B,
    0x043D, 0x8FC7, 0xF691, 0xC106, 0xB850, 0x33AA, 0x4AFC, 0x5C84, 0x25D2,
    0xAE28, 0xD77E, 0xE0E9, 0x99BF, 0x1245, 0x6B13, 0x3EB5, 0x47E3, 0xCC19,
    0xB54F, 0x82D8, 0xFB8E, 0x7074, 0x0922, 0x1F5A, 0x660C, 0xEDF6, 0x94A0,
    0xA337, 0xDA61, 0x519B, 0x28CD};
 ...
 **/
STATIC u16 fm10k_fifo_crc(struct fm10k_mbx_fifo *fifo, u16 offset, u16 len,
                          u16 seed) {
...
STATIC u16 fm10k_mbx_dequeue_rx(struct fm10k_hw *hw,
  ...
  for (cnt = 0; !fm10k_fifo_empty(fifo); cnt++) {
    err =
        fm10k_tlv_msg_parse(hw, fifo->buffer + fifo->head, mbx, mbx->msg_data);
    if (err < 0)
      mbx->rx_parse_err++;
...
STATIC void fm10k_mbx_update_max_size(struct fm10k_mbx_info *mbx, u16 size) {
  ...
  for (len = fm10k_fifo_head_len(&mbx->tx); len > size;
...
STATIC void fm10k_mbx_disconnect(struct fm10k_hw *hw,
  ...
  FM10K_WRITE_MBX(hw, mbx->mbx_reg,
                  FM10K_MBX_REQ | FM10K_MBX_INTERRUPT_DISABLE);
  do {
 ...
 *  This function returns a connection mailbox header
 **/
STATIC void fm10k_sm_mbx_create_connect_hdr(struct fm10k_mbx_info *mbx,
                                            u8 err) {
  if (mbx->local)
    mbx->mbx_lock |= FM10K_MBX_REQ;
 ...
 *  or is currently in use.
 **/
STATIC s32 fm10k_sm_mbx_connect(struct fm10k_hw *hw,
                                struct fm10k_mbx_info *mbx) {
