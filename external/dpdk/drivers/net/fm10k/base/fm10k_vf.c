...
STATIC s32 fm10k_stop_hw_vf(struct fm10k_hw *hw) {
  ...
  if (IS_VALID_ETHER_ADDR(perm_addr)) {
    bal = (((u32)perm_addr[3]) << 24) | (((u32)perm_addr[4]) << 16) |
          (((u32)perm_addr[5]) << 8);
    bah = (((u32)0xFF) << 24) | (((u32)perm_addr[0]) << 16) |
          (((u32)perm_addr[1]) << 8) | ((u32)perm_addr[2]);
  }
...
STATIC s32 fm10k_init_hw_vf(struct fm10k_hw *hw) {
  ...
  hw->mac.default_vid =
      (FM10K_READ_REG(hw, FM10K_TXQCTL(0)) & FM10K_TXQCTL_VID_MASK) >>
   ...
   */
  hw->mac.itr_scale =
      (FM10K_READ_REG(hw, FM10K_TDLEN(0)) & FM10K_TDLEN_ITR_SCALE_MASK) >>
...
s32 fm10k_msg_mac_vlan_vf(struct fm10k_hw *hw, u32 **results,
  ...
  err = fm10k_tlv_attr_get_mac_vlan(results[FM10K_MAC_VLAN_MSG_DEFAULT_MAC],
                                    perm_addr, &vid);
  if (err)
    return err;
...
STATIC s32 fm10k_update_mc_addr_vf(struct fm10k_hw *hw, u16 glort,
  ...
  /* generate VLAN request */
  fm10k_tlv_msg_init(msg, FM10K_VF_MSG_ID_MAC_VLAN);
  fm10k_tlv_attr_put_mac_vlan(msg, FM10K_MAC_VLAN_MSG_MULTICAST, mac, vid);
...
const struct fm10k_tlv_attr fm10k_lport_state_msg_attr[] = {
    ...
    FM10K_TLV_ATTR_BOOL(FM10K_LPORT_STATE_MSG_READY), FM10K_TLV_ATTR_LAST};
...
const struct fm10k_tlv_attr fm10k_1588_msg_attr[] = {
    FM10K_TLV_ATTR_U64(FM10K_1588_MSG_CLK_OFFSET), FM10K_TLV_ATTR_LAST};
