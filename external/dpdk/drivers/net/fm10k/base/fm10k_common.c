...
s32 fm10k_disable_queues_generic(struct fm10k_hw *hw, u16 q_cnt) {
  ...
  for (i = 0; i < q_cnt; i++) {
    ...
    FM10K_WRITE_REG(hw, FM10K_TXDCTL(i), reg & ~FM10K_TXDCTL_ENABLE);
    reg = FM10K_READ_REG(hw, FM10K_RXQCTL(i));
    FM10K_WRITE_REG(hw, FM10K_RXQCTL(i), reg & ~FM10K_RXQCTL_ENABLE);
  }
...
STATIC void fm10k_update_hw_stats_tx_q(struct fm10k_hw *hw,
                                       struct fm10k_hw_stats_q *q, u32 idx) {
  ...
  /* Process TX Ring */
  do {
    tx_packets = fm10k_read_hw_stats_32b(hw, FM10K_QPTC(idx), &q->tx_packets);

    if (tx_packets)
      tx_bytes = fm10k_read_hw_stats_48b(hw, FM10K_QBTC_L(idx), &q->tx_bytes);
...
STATIC void fm10k_update_hw_stats_rx_q(struct fm10k_hw *hw,
  ...
  /* Process RX Ring */
  do {
    rx_drops = fm10k_read_hw_stats_32b(hw, FM10K_QPRDC(idx), &q->rx_drops);

    rx_packets = fm10k_read_hw_stats_32b(hw, FM10K_QPRC(idx), &q->rx_packets);

    if (rx_packets)
      rx_bytes = fm10k_read_hw_stats_48b(hw, FM10K_QBRC_L(idx), &q->rx_bytes);
