...
#define MBUF_DMA_ADDR(mb) ((uint64_t)((mb)->buf_iova + (mb)->data_off))
...
static inline uint16_t fifo_peek(struct fifo *fifo) { return *fifo->tail; }
...
static inline void fm10k_pktmbuf_reset(struct rte_mbuf *mb, uint16_t in_port) {
  ...
  mb->data_off =
      (uint16_t)(RTE_PTR_ALIGN((char *)mb->buf_addr + RTE_PKTMBUF_HEADROOM,
                               FM10K_RX_DATABUF_ALIGN) -
                 (char *)mb->buf_addr);
  mb->port = in_port;
}
 ...
 *       frame.
 */
static inline int fm10k_addr_alignment_valid(struct rte_mbuf *mb) {
  ...
  if (RTE_ALIGN(addr, 8) == addr) {
    ...
    boundary2 = RTE_ALIGN_FLOOR(addr + RTE_ETHER_MAX_VLAN_FRAME_LEN, 4096);
...
int fm10k_dev_tx_descriptor_status(void *rx_queue, uint16_t offset);
...
uint16_t fm10k_recv_scattered_pkts_vec(void *, struct rte_mbuf **, uint16_t);
