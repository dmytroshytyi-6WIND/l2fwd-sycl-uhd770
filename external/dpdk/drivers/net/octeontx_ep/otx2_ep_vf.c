...
static void otx2_vf_setup_global_iq_reg(struct otx_ep_device *otx_ep,
                                        int q_no) {
...
static void otx2_vf_setup_global_oq_reg(struct otx_ep_device *otx_ep,
                                        int q_no) {
  volatile uint64_t reg_val = 0ull;
...
static void otx2_vf_setup_global_input_regs(struct otx_ep_device *otx_ep) {
  uint64_t q_no = 0ull;
...
static void otx2_vf_setup_global_output_regs(struct otx_ep_device *otx_ep) {
  uint32_t q_no;
...
static void otx2_vf_setup_device_regs(struct otx_ep_device *otx_ep) {
  otx2_vf_setup_global_input_regs(otx_ep);
  otx2_vf_setup_global_output_regs(otx_ep);
}

static void otx2_vf_setup_iq_regs(struct otx_ep_device *otx_ep,
                                  uint32_t iq_no) {
  ...
  otx2_write64(iq->base_addr_dma,
               otx_ep->hw_addr + SDP_VF_R_IN_INSTR_BADDR(iq_no));
  otx2_write64(iq->nb_desc, otx_ep->hw_addr + SDP_VF_R_IN_INSTR_RSIZE(iq_no));

  /* Remember the doorbell & instruction count register addr
   * for this queue
   */
  iq->doorbell_reg =
      (uint8_t *)otx_ep->hw_addr + SDP_VF_R_IN_INSTR_DBELL(iq_no);
  iq->inst_cnt_reg = (uint8_t *)otx_ep->hw_addr + SDP_VF_R_IN_CNTS(iq_no);

  otx_ep_dbg("InstQ[%d]:dbell reg @ 0x%p inst_cnt_reg @ 0x%p", iq_no,
...
static void otx2_vf_setup_oq_regs(struct otx_ep_device *otx_ep,
                                  uint32_t oq_no) {
  volatile uint64_t reg_val = 0ull;
  ...
  otx2_write64(droq->desc_ring_dma,
               otx_ep->hw_addr + SDP_VF_R_OUT_SLIST_BADDR(oq_no));
  otx2_write64(droq->nb_desc,
               otx_ep->hw_addr + SDP_VF_R_OUT_SLIST_RSIZE(oq_no));
  ...
  droq->pkts_credit_reg =
      (uint8_t *)otx_ep->hw_addr + SDP_VF_R_OUT_SLIST_DBELL(oq_no);

  rte_write64(OTX_EP_CLEAR_OUT_INT_LVLS,
              otx_ep->hw_addr + SDP_VF_R_OUT_INT_LEVELS(oq_no));

  /* Clear PKT_CNT register */
  rte_write64(OTX_EP_CLEAR_SDP_OUT_PKT_CNT,
              (uint8_t *)otx_ep->hw_addr + SDP_VF_R_OUT_PKT_CNT(oq_no));
...
static int otx2_vf_enable_iq(struct otx_ep_device *otx_ep, uint32_t q_no) {
   ...
   */
  otx2_write64(0xFFFFFFFF, otx_ep->hw_addr + SDP_VF_R_IN_INSTR_DBELL(q_no));

  while (((otx2_read64(otx_ep->hw_addr + SDP_VF_R_IN_INSTR_DBELL(q_no))) !=
          0ull) &&
         loop--) {
    rte_delay_ms(1);
  }
...
static int otx2_vf_enable_oq(struct otx_ep_device *otx_ep, uint32_t q_no) {
  uint64_t reg_val = 0ull;
...
static int otx2_vf_enable_io_queues(struct otx_ep_device *otx_ep) {
  uint32_t q_no = 0;
  int ret;
...
static void otx2_vf_disable_iq(struct otx_ep_device *otx_ep, uint32_t q_no) {
  uint64_t reg_val = 0ull;
...
static void otx2_vf_disable_oq(struct otx_ep_device *otx_ep, uint32_t q_no) {
  volatile uint64_t reg_val = 0ull;
...
static void otx2_vf_disable_io_queues(struct otx_ep_device *otx_ep) {
  uint32_t q_no = 0;
    ...
    .iq =
        {
            ...
            .pending_list_size =
                (OTX_EP_MAX_IQ_DESCRIPTORS * OTX_EP_CFG_IO_QUEUES),
        },
...
int otx2_ep_vf_setup_device(struct otx_ep_device *otx_ep) {
  uint64_t reg_val = 0ull;
  ...
  otx_ep->sriov_info.rings_per_vf =
      ((reg_val >> SDP_VF_R_IN_CTL_RPVF_POS) & SDP_VF_R_IN_CTL_RPVF_MASK);
