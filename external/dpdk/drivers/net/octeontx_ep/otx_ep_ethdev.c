...
static int otx_ep_dev_info_get(struct rte_eth_dev *eth_dev,
...
static int otx_ep_dev_start(struct rte_eth_dev *eth_dev) {
  struct otx_ep_device *otx_epvf;
  unsigned int q;
  int ret;
  ...
  for (q = 0; q < otx_epvf->nb_rx_queues; q++) {
    rte_write32(otx_epvf->droq[q]->nb_desc, otx_epvf->droq[q]->pkts_credit_reg);
...
static int otx_ep_dev_stop(struct rte_eth_dev *eth_dev) {
...
static int otx_ep_chip_specific_setup(struct otx_ep_device *otx_epvf) {
...
static int otx_epdev_init(struct otx_ep_device *otx_epvf) {
...
static int otx_ep_dev_configure(struct rte_eth_dev *eth_dev) {
  struct otx_ep_device *otx_epvf = OTX_EP_DEV(eth_dev);
...
otx_ep_rx_queue_setup(struct rte_eth_dev *eth_dev, uint16_t q_no,
  ...
  if (num_rx_descs < (SDP_GBL_WMARK * 8)) {
    otx_ep_err(
        "Invalid rx desc number should at least be greater than 8xwmark  %u\n",
        num_rx_descs);
    return -EINVAL;
  }
 ...
 *    - nothing
 */
static void otx_ep_rx_queue_release(struct rte_eth_dev *dev, uint16_t q_no) {
 ...
 *    - nothing
 */
static void otx_ep_tx_queue_release(struct rte_eth_dev *dev, uint16_t q_no) {
...
static int otx_epdev_exit(struct rte_eth_dev *eth_dev) {
  struct otx_ep_device *otx_epvf;
...
static int otx_ep_eth_dev_uninit(struct rte_eth_dev *eth_dev) {
...
static int otx_ep_eth_dev_init(struct rte_eth_dev *eth_dev) {
...
static int otx_ep_eth_dev_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                                    struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct otx_ep_device),
                                       otx_ep_eth_dev_init);
}

static int otx_ep_eth_dev_pci_remove(struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_remove(pci_dev, otx_ep_eth_dev_uninit);
}
