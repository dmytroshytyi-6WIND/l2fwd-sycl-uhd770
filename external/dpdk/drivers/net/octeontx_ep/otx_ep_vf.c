...
static void otx_ep_setup_global_iq_reg(struct otx_ep_device *otx_ep, int q_no) {
...
static void otx_ep_setup_global_oq_reg(struct otx_ep_device *otx_ep, int q_no) {
  volatile uint64_t reg_val = 0ull;
...
static void otx_ep_setup_global_input_regs(struct otx_ep_device *otx_ep) {
  uint64_t q_no = 0ull;
...
static void otx_ep_setup_global_output_regs(struct otx_ep_device *otx_ep) {
  uint32_t q_no;
...
static void otx_ep_setup_device_regs(struct otx_ep_device *otx_ep) {
  otx_ep_setup_global_input_regs(otx_ep);
  otx_ep_setup_global_output_regs(otx_ep);
}

static void otx_ep_setup_iq_regs(struct otx_ep_device *otx_ep, uint32_t iq_no) {
   ...
   */
  iq->doorbell_reg =
      (uint8_t *)otx_ep->hw_addr + OTX_EP_R_IN_INSTR_DBELL(iq_no);
  iq->inst_cnt_reg = (uint8_t *)otx_ep->hw_addr + OTX_EP_R_IN_CNTS(iq_no);
...
static void otx_ep_setup_oq_regs(struct otx_ep_device *otx_ep, uint32_t oq_no) {
  volatile uint64_t reg_val = 0ull;
  ...
  droq->pkts_sent_reg = (uint8_t *)otx_ep->hw_addr + OTX_EP_R_OUT_CNTS(oq_no);
  droq->pkts_credit_reg =
      (uint8_t *)otx_ep->hw_addr + OTX_EP_R_OUT_SLIST_DBELL(oq_no);
  ...
  otx_ep_dbg("OTX_EP_R[%d]_sent: %x\n", oq_no, rte_read32(droq->pkts_sent_reg));
...
static int otx_ep_enable_iq(struct otx_ep_device *otx_ep, uint32_t q_no) {
  ...
  while (
      ((rte_read64(otx_ep->hw_addr + OTX_EP_R_IN_INSTR_DBELL(q_no))) != 0ull) &&
      loop--) {
    rte_delay_ms(1);
  }
...
static int otx_ep_enable_oq(struct otx_ep_device *otx_ep, uint32_t q_no) {
  ...
  while (((rte_read64(otx_ep->hw_addr + OTX_EP_R_OUT_SLIST_DBELL(q_no))) !=
          0ull) &&
         loop--) {
    rte_delay_ms(1);
  }
...
static int otx_ep_enable_io_queues(struct otx_ep_device *otx_ep) {
  uint32_t q_no = 0;
  int ret;
...
static void otx_ep_disable_iq(struct otx_ep_device *otx_ep, uint32_t q_no) {
  uint64_t reg_val = 0ull;
...
static void otx_ep_disable_oq(struct otx_ep_device *otx_ep, uint32_t q_no) {
  uint64_t reg_val = 0ull;
...
static void otx_ep_disable_io_queues(struct otx_ep_device *otx_ep) {
  uint32_t q_no = 0;
...
int otx_ep_vf_setup_device(struct otx_ep_device *otx_ep) {
  uint64_t reg_val = 0ull;
  ...
  otx_ep->sriov_info.rings_per_vf =
      ((reg_val >> OTX_EP_R_IN_CTL_RPVF_POS) & OTX_EP_R_IN_CTL_RPVF_MASK);
