...
static void otx_ep_dmazone_free(const struct rte_memzone *mz) {
...
/* IQ initialization */
static int otx_ep_init_instr_queue(struct otx_ep_device *otx_ep, int iq_no,
  ...
  iq->iq_mz =
      rte_eth_dma_zone_reserve(otx_ep->eth_dev, "instr_queue", iq_no, q_size,
   ...
   */
  iq->req_list =
      rte_zmalloc_socket("request_list", (iq->nb_desc * OTX_EP_IQREQ_LIST_SIZE),
  ...
  otx_ep_info("IQ[%d]: base: %p basedma: %lx count: %d\n", iq_no, iq->base_addr,
              (unsigned long)iq->base_addr_dma, iq->nb_desc);
...
static void otx_ep_droq_reset_indices(struct otx_ep_droq *droq) {
  droq->read_idx = 0;
  droq->write_idx = 0;
  droq->refill_idx = 0;
  droq->refill_count = 0;
...
static void otx_ep_droq_destroy_ring_buffers(struct otx_ep_droq *droq) {
  uint32_t idx;
...
static int otx_ep_droq_setup_ring_buffers(struct otx_ep_droq *droq) {
...
/* OQ initialization */
static int otx_ep_init_droq(struct otx_ep_device *otx_ep, uint32_t q_no,
  ...
  droq->desc_ring_mz =
      rte_eth_dma_zone_reserve(otx_ep->eth_dev, "droq", q_no, desc_ring_size,
                               OTX_EP_PCI_RING_ALIGN, socket_id);
  ...
  otx_ep_dbg("OQ[%d]: desc_ring: virt: 0x%p, dma: %lx\n", q_no, droq->desc_ring,
             (unsigned long)droq->desc_ring_dma);
  otx_ep_dbg("OQ[%d]: num_desc: %d\n", q_no, droq->nb_desc);

  /* OQ buf_list set up */
  droq->recv_buf_list = rte_zmalloc_socket(
      "recv_buf_list", (droq->nb_desc * sizeof(struct rte_mbuf *)),
...
int otx_ep_setup_oqs(struct otx_ep_device *otx_ep, int oq_no, int num_descs,
  ...
  /* Allocate new droq. */
  droq = (struct otx_ep_droq *)rte_zmalloc("otx_ep_OQ", sizeof(*droq),
                                           RTE_CACHE_LINE_SIZE);
  if (droq == NULL) {
...
static inline void otx_ep_iqreq_delete(struct otx_ep_instr_queue *iq,
                                       uint32_t idx) {
  uint32_t reqtype;
  void *buf;
...
static inline void otx_ep_iqreq_add(struct otx_ep_instr_queue *iq, void *buf,
                                    uint32_t reqtype, int index) {
  iq->req_list[index].buf = buf;
  iq->req_list[index].reqtype = reqtype;
}

static uint32_t otx_vf_update_read_index(struct otx_ep_instr_queue *iq) {
...
static void otx_ep_flush_iq(struct otx_ep_instr_queue *iq) {
  ...
  while (iq->flush_index != iq->otx_read_index) {
    ...
    iq->flush_index = otx_ep_incr_index(iq->flush_index, 1, iq->nb_desc);
...
static inline int post_iqcmd(struct otx_ep_instr_queue *iq, uint8_t *iqcmd) {
...
static int otx_ep_send_data(struct otx_ep_device *otx_ep,
                            struct otx_ep_instr_queue *iq, void *cmd,
                            int dbell) {
  uint32_t ret;
...
static inline void set_sg_size(struct otx_ep_sg_entry *sg_entry, uint16_t size,
                               uint32_t pos) {
 ...
 */
uint16_t otx_ep_xmit_pkts(void *tx_queue, struct rte_mbuf **pkts,
                          uint16_t nb_pkts) {
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (m->nb_segs == 1) {
      ...
      finfo =
          (struct otx_ep_buf_free_info *)rte_malloc(NULL, sizeof(*finfo), 0);
      if (finfo == NULL) {
      ...
      finfo->g.sg = rte_zmalloc(NULL, sgbuf_sz, OTX_EP_SG_ALIGN);
      if (finfo->g.sg == NULL) {
        rte_free(finfo);
 ...
 * returns number of requests enqueued successfully
 */
uint16_t otx2_ep_xmit_pkts(void *tx_queue, struct rte_mbuf **pkts,
                           uint16_t nb_pkts) {
...
static uint32_t otx_ep_droq_refill(struct otx_ep_droq *droq) {
  ...
  while (droq->refill_count && (desc_refilled < droq->nb_desc)) {
    ...
    droq->refill_idx = otx_ep_incr_index(droq->refill_idx, 1, droq->nb_desc);
...
static struct rte_mbuf *otx_ep_droq_read_packet(struct otx_ep_device *otx_ep,
                                                struct otx_ep_droq *droq,
                                                int next_fetch) {
  ...
  droq_pkt->packet_type =
      rte_net_get_ptype(droq_pkt, &hdr_lens, RTE_PTYPE_ALL_MASK);
...
static inline uint32_t otx_ep_check_droq_pkts(struct otx_ep_droq *droq) {
 ...
 */
uint16_t otx_ep_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                          uint16_t budget) {
  ...
  for (pkts = 0; pkts < new_pkts; pkts++) {
    ...
    if (!oq_pkt) {
      RTE_LOG_DP(ERR, PMD,
                 ...
                 droq->pkts_pending, droq->last_pkt_count, new_pkts);
