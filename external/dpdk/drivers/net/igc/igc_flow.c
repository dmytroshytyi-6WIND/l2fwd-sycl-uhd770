...
#define IGC_FILTER_MASK_ALL                                                    \
  (IGC_FILTER_MASK_ETHER | IGC_FILTER_MASK_NTUPLE | IGC_FILTER_MASK_TCP_SYN |  \
   IGC_FILTER_MASK_RSS)
...
static int igc_parse_attribute(const struct rte_flow_attr *attr,
  ...
  if (attr->egress)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS,
                              attr, "Not support");

  if (attr->transfer)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_TRANSFER,
                              attr, "Not support");

  if (!attr->ingress)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS,
                              attr, "A rule must apply to ingress traffic");

  if (attr->priority == 0)
    return 0;

  /* only n-tuple and SYN filter have priority level */
  IGC_SET_FILTER_MASK(filter, IGC_FILTER_MASK_NTUPLE | IGC_FILTER_MASK_TCP_SYN);
  ...
  if (attr->priority > IGC_NTUPLE_MAX_PRI)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY,
                              attr, "Priority value is invalid.");
...
/* Parse rule patterns */
static int igc_parse_patterns(const struct rte_flow_item patterns[],
                              struct igc_all_filter *filter,
                              struct rte_flow_error *error) {
  ...
  for (; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    ...
    if (item->last)
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_LAST,
                                item, "Range not been supported");
    ...
    igc_pattern_parse parse_func = pattern_parse_list[item->type];
    if (!parse_func)
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                                "Not been supported");
...
/* Parse flow actions */
static int igc_parse_actions(struct rte_eth_dev *dev,
...
/* Parse a flow rule */
static int igc_parse_flow(struct rte_eth_dev *dev,
...
static int igc_parse_pattern_ether(const struct rte_flow_item *item,
  ...
  if (!rte_is_zero_ether_addr(&mask->src) ||
    ...
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                              item, "Only support ether-type");

  /* ether-type mask bits must be all 1 */
  if (IGC_NOT_ALL_BITS_SET(mask->type))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                              item, "Ethernet type mask bits must be all 1");
  ...
  if (ether->ether_type == RTE_ETHER_TYPE_IPV4 ||
      ether->ether_type == RTE_ETHER_TYPE_IPV6 || ether->ether_type == 0)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
...
static int igc_parse_pattern_ip(const struct rte_flow_item *item,
  ...
  if (mask->hdr.version_ihl || mask->hdr.type_of_service ||
      ...
      mask->hdr.hdr_checksum || mask->hdr.dst_addr || mask->hdr.src_addr)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                              item, "IPv4 only support protocol");
 ...
 */
static inline bool igc_is_zero_ipv6_addr(const void *ipv6_addr) {
...
static int igc_parse_pattern_ipv6(const struct rte_flow_item *item,
  ...
  /* only protocol is used */
  if (mask->hdr.vtc_flow || mask->hdr.payload_len || mask->hdr.hop_limits ||
...
static int igc_parse_pattern_udp(const struct rte_flow_item *item,
  ...
  if (mask->hdr.dgram_len || mask->hdr.dgram_cksum || mask->hdr.src_port)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                              item, "UDP only support destination port");
  ...
  if (IGC_NOT_ALL_BITS_SET(mask->hdr.dst_port))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                              item, "UDP port mask bits must be all 0 or 1");
...
static int igc_parse_pattern_tcp(const struct rte_flow_item *item,
  ...
  if (mask->hdr.sent_seq || mask->hdr.recv_ack || mask->hdr.data_off ||
      mask->hdr.rx_win || mask->hdr.cksum || mask->hdr.tcp_urp ||
      mask->hdr.src_port)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                              item,
  ...
  if (mask->hdr.dst_port) {
    ...
    if (IGC_NOT_ALL_BITS_SET(mask->hdr.dst_port))
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_MASK,
                                item, "TCP port mask bits must be all 1");
...
static int igc_parse_action_queue(struct rte_eth_dev *dev,
  ...
  IGC_SET_FILTER_MASK(filter, IGC_FILTER_MASK_ETHER | IGC_FILTER_MASK_NTUPLE |
...
igc_alloc_flow(const void *filter, enum igc_filter_type type, size_t inbytes) {
  ...
  struct rte_flow *flow =
      rte_malloc("igc flow filter", sizeof(struct rte_flow) + inbytes, 8);
  if (flow == NULL) {
...
static inline void igc_append_flow(struct igc_flow_list *list,
                                   struct rte_flow *flow) {
 ...
 **/
static inline void igc_remove_flow(struct igc_flow_list *list,
                                   struct rte_flow *flow) {
...
static inline bool igc_is_flow_in_list(struct igc_flow_list *list,
                                       struct rte_flow *flow) {
  struct rte_flow *it;
 ...
 **/
static struct rte_flow *igc_flow_create(struct rte_eth_dev *dev,
                                        const struct rte_flow_attr *attr,
                                        const struct rte_flow_item patterns[],
                                        const struct rte_flow_action actions[],
                                        struct rte_flow_error *error) {
  ...
  case IGC_FILTER_MASK_ETHER:
    flow = igc_alloc_flow(&filter.ethertype, IGC_FILTER_TYPE_ETHERTYPE,
  ...
  case IGC_FILTER_MASK_NTUPLE:
    ...
    if (filter.ntuple.tuple_info.dst_port_mask == 0 &&
      ...
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_NONE, NULL,
                         "Flow can't be recognized");
      return NULL;
    }
  ...
  case IGC_FILTER_MASK_TCP_SYN:
    flow = igc_alloc_flow(&filter.syn, IGC_FILTER_TYPE_SYN, sizeof(filter.syn));
    if (flow)
      ret = igc_set_syn_filter(dev, &filter.syn);
    break;
  case IGC_FILTER_MASK_RSS:
    flow =
        igc_alloc_flow(&filter.rss, IGC_FILTER_TYPE_HASH, sizeof(filter.rss));
    if (flow) {
      struct igc_rss_filter *rss = (struct igc_rss_filter *)flow->filter;
 ...
 **/
static int igc_flow_validate(struct rte_eth_dev *dev,
                             const struct rte_flow_attr *attr,
                             const struct rte_flow_item patterns[],
                             const struct rte_flow_action actions[],
                             struct rte_flow_error *error) {
  ...
  case IGC_FILTER_MASK_NTUPLE:
    ...
    if (filter.ntuple.tuple_info.dst_port_mask == 0 &&
        filter.ntuple.tuple_info.proto_mask == 0)
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_NONE, NULL,
                                "Flow can't be recognized");
    break;
  }
 ...
 **/
static int igc_disable_flow(struct rte_eth_dev *dev, struct rte_flow *flow) {
  int ret = 0;

  switch (flow->filter_type) {
  case IGC_FILTER_TYPE_ETHERTYPE:
    ret = igc_del_ethertype_filter(
        dev, (struct igc_ethertype_filter *)&flow->filter);
    break;
  case IGC_FILTER_TYPE_NTUPLE:
    ret = igc_del_ntuple_filter(dev, (struct igc_ntuple_filter *)&flow->filter);
    break;
  ...
  case IGC_FILTER_TYPE_HASH:
    ...
    break;
  default:
    PMD_DRV_LOG(ERR, "Filter type (%d) not supported", flow->filter_type);
    ret = -EINVAL;
  }
...
/* Destroy a flow rule */
static int igc_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                            struct rte_flow_error *error) {
