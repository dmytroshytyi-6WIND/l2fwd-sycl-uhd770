 ...
 */
static inline int igc_ethertype_filter_lookup(const struct igc_adapter *igc,
...
int igc_add_ethertype_filter(struct rte_eth_dev *dev,
  ...
  if (filter->ether_type == RTE_ETHER_TYPE_IPV4 ||
      filter->ether_type == RTE_ETHER_TYPE_IPV6 || filter->ether_type == 0) {
    PMD_DRV_LOG(ERR, "Unsupported ether_type(0x%04x) in ethertype filter",
                filter->ether_type);
    return -EINVAL;
  }
...
static void igc_clear_all_ethertype_filter(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  struct igc_adapter *igc = IGC_DEV_PRIVATE(dev);
  int i;
 ...
 * -1, the item not been found
 */
static int igc_tuple_filter_lookup(const struct igc_adapter *igc,
...
static void igc_enable_tuple_filter(struct rte_eth_dev *dev,
                                    const struct igc_adapter *igc,
                                    uint8_t index) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
...
static void igc_disable_tuple_filter(struct rte_eth_dev *dev, uint8_t index) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
...
static void igc_clear_all_ntuple_filter(struct rte_eth_dev *dev) {
  struct igc_adapter *igc = IGC_DEV_PRIVATE(dev);
  int i;
...
int igc_set_syn_filter(struct rte_eth_dev *dev,
  ...
  if (filter->queue >= IGC_QUEUE_PAIRS_NUM) {
    PMD_DRV_LOG(ERR, "out of range queue %u(max is %u)", filter->queue,
                IGC_QUEUE_PAIRS_NUM);
    return -EINVAL;
  }
...
int eth_igc_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
