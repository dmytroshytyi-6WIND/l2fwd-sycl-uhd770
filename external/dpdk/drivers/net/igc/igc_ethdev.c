...
static const struct rte_igc_xstats_name_off rte_igc_stats_strings[] = {
    ...
    {"tx_excessive_collision_packets", offsetof(struct igc_hw_stats, ecol)},
    ...
    {"rx_flow_control_unsupported_packets",
     offsetof(struct igc_hw_stats, fcruc)},
    ...
    {"rx_size_512_to_1023_packets", offsetof(struct igc_hw_stats, prc1023)},
    {"rx_size_1024_to_max_packets", offsetof(struct igc_hw_stats, prc1522)},
    ...
    {"tx_size_512_to_1023_packets", offsetof(struct igc_hw_stats, ptc1023)},
    {"tx_size_1023_to_max_packets", offsetof(struct igc_hw_stats, ptc1522)},
    ...
    {"rx_descriptor_lower_threshold", offsetof(struct igc_hw_stats, icrxdmtc)},
};

#define IGC_NB_XSTATS                                                          \
  (sizeof(rte_igc_stats_strings) / sizeof(rte_igc_stats_strings[0]))
...
static int eth_igc_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
                                  size_t fw_size);
...
static int eth_igc_xstats_get_names(struct rte_eth_dev *dev,
                                    ...
                                    unsigned int size);
static int
eth_igc_xstats_get_names_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
                               struct rte_eth_xstat_name *xstats_names,
                               unsigned int limit);
static int eth_igc_xstats_reset(struct rte_eth_dev *dev);
static int eth_igc_queue_stats_mapping_set(struct rte_eth_dev *dev,
                                           uint16_t queue_id, uint8_t stat_idx,
                                           uint8_t is_rx);
static int eth_igc_rx_queue_intr_disable(struct rte_eth_dev *dev,
                                         uint16_t queue_id);
static int eth_igc_rx_queue_intr_enable(struct rte_eth_dev *dev,
                                        uint16_t queue_id);
static int eth_igc_flow_ctrl_get(struct rte_eth_dev *dev,
                                 struct rte_eth_fc_conf *fc_conf);
static int eth_igc_flow_ctrl_set(struct rte_eth_dev *dev,
                                 struct rte_eth_fc_conf *fc_conf);
...
static int eth_igc_rss_hash_conf_get(struct rte_eth_dev *dev,
                                     struct rte_eth_rss_conf *rss_conf);
static int eth_igc_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                   int on);
 ...
 */
static int igc_check_mq_mode(struct rte_eth_dev *dev) {
  ...
  if (tx_mq_mode != RTE_ETH_MQ_TX_NONE)
    PMD_INIT_LOG(WARNING,
                 "TX mode %d is not supported. Due to meaningless in this "
                 "driver, just ignore",
                 tx_mq_mode);
 ...
 */
static void igc_intr_other_disable(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
  struct rte_intr_handle *intr_handle = pci_dev->intr_handle;

  if (rte_intr_allow_others(intr_handle) && dev->data->dev_conf.intr_conf.lsc) {
 ...
 */
static inline void igc_intr_other_enable(struct rte_eth_dev *dev) {
  struct igc_interrupt *intr = IGC_DEV_PRIVATE_INTR(dev);
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
  struct rte_intr_handle *intr_handle = pci_dev->intr_handle;
 ...
 */
static void eth_igc_interrupt_get_status(struct rte_eth_dev *dev) {
  uint32_t icr;
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  struct igc_interrupt *intr = IGC_DEV_PRIVATE_INTR(dev);
...
static int eth_igc_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
  ...
  for (count = 0; count < IGC_LINK_UPDATE_CHECK_TIMEOUT; count++) {
    ...
    case igc_media_type_fiber:
      igc_check_for_link(hw);
      link_check = (IGC_READ_REG(hw, IGC_STATUS) & IGC_STATUS_LU);
      break;
 ...
 */
static void eth_igc_interrupt_action(struct rte_eth_dev *dev) {
  struct igc_interrupt *intr = IGC_DEV_PRIVATE_INTR(dev);
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
  struct rte_eth_link link;
  int ret;
  ...
  if (intr->flags & IGC_FLAG_NEED_LINK_UPDATE) {
    ...
    if (link.link_status)
      PMD_DRV_LOG(INFO, " Port %d: Link Up - speed %u Mbps - %s",
                  dev->data->port_id, (unsigned int)link.link_speed,
                  link.link_duplex == RTE_ETH_LINK_FULL_DUPLEX ? "full-duplex"
                                                               : "half-duplex");
    else
      PMD_DRV_LOG(INFO, " Port %d: Link Down", dev->data->port_id);

    PMD_DRV_LOG(DEBUG, "PCI Address: " PCI_PRI_FMT, pci_dev->addr.domain,
 ...
 */
static void eth_igc_interrupt_handler(void *param) {
 ...
 *  The address of parameter (struct rte_eth_dev *) registered before.
 */
static void igc_update_queue_stats_handler(void *param) {
  struct rte_eth_dev *dev = param;
  igc_read_queue_stats_register(dev);
  rte_eal_alarm_set(IGC_ALARM_INTERVAL, igc_update_queue_stats_handler, dev);
}

/*
 * rx,tx enable/disable
 */
static void eth_igc_rxtx_control(struct rte_eth_dev *dev, bool enable) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  uint32_t tctl, rctl;
...
static int eth_igc_stop(struct rte_eth_dev *dev) {
  ...
  if (!rte_intr_allow_others(intr_handle))
    ...
    rte_intr_callback_register(intr_handle, eth_igc_interrupt_handler,
                               (void *)dev);
 ...
 */
static void igc_write_ivar(struct igc_hw *hw, uint8_t queue_index, bool tx,
                           uint8_t msix_vector) {
  uint8_t offset = 0;
 ...
 *  board private structure
 */
static void igc_configure_msix_intr(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
  struct rte_intr_handle *intr_handle = pci_dev->intr_handle;
  ...
  IGC_WRITE_REG(hw, IGC_GPIE,
                IGC_GPIE_MSIX_MODE | IGC_GPIE_PBA | IGC_GPIE_EIAME |
                    IGC_GPIE_NSICR);
 ...
 *  Enable or Disable
 */
static void igc_lsc_interrupt_setup(struct rte_eth_dev *dev, uint8_t on) {
  struct igc_interrupt *intr = IGC_DEV_PRIVATE_INTR(dev);
 ...
 */
static void igc_rxq_interrupt_setup(struct rte_eth_dev *dev) {
  uint32_t mask;
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
  struct rte_intr_handle *intr_handle = pci_dev->intr_handle;
 ...
 */
static inline int igc_get_rx_buffer_size(struct igc_hw *hw) {
 ...
 */
static void igc_hw_control_acquire(struct igc_hw *hw) {
  uint32_t ctrl_ext;
 ...
 */
static void igc_hw_control_release(struct igc_hw *hw) {
  uint32_t ctrl_ext;
...
static int igc_hardware_init(struct igc_hw *hw) {
...
static int eth_igc_start(struct rte_eth_dev *dev) {
  ...
  eth_igc_vlan_offload_set(dev, RTE_ETH_VLAN_STRIP_MASK |
  ...
  if (*speeds == RTE_ETH_LINK_SPEED_AUTONEG) {
    ...
    if (*speeds & RTE_ETH_LINK_SPEED_FIXED) {
      PMD_DRV_LOG(ERR, "Force speed mode currently not supported");
      igc_dev_clear_queues(dev);
      return -EINVAL;
    }
  ...
  if (rte_intr_allow_others(intr_handle)) {
    ...
    rte_intr_callback_unregister(intr_handle, eth_igc_interrupt_handler,
                                 (void *)dev);
    if (dev->data->dev_conf.intr_conf.lsc)
      PMD_DRV_LOG(INFO, "LSC won't enable because of no intr multiplex");
  }
  ...
  if (dev->data->dev_conf.lpbk_mode == 1) {
    ...
    reg_val |= IGC_CTRL_SLU | IGC_CTRL_FRCSPD | IGC_CTRL_FRCDPX | IGC_CTRL_FD |
               IGC_CTRL_SPEED_2500;
...
static int igc_reset_swfw_lock(struct igc_hw *hw) {
  int ret_val;
 ...
 */
static void igc_dev_free_queues(struct rte_eth_dev *dev) {
  uint16_t i;
...
static void igc_identify_hardware(struct rte_eth_dev *dev,
...
static int eth_igc_dev_init(struct rte_eth_dev *dev) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
  ...
  dev->data->mac_addrs =
      rte_zmalloc("igc", RTE_ETHER_ADDR_LEN * hw->mac.rar_entry_count, 0);
  ...
  if (igc_check_reset_block(hw) < 0)
    PMD_INIT_LOG(ERR, "PHY reset is blocked due to SOL/IDER session.");

  PMD_INIT_LOG(DEBUG, "port_id %d vendorID=0x%x deviceID=0x%x",
               dev->data->port_id, pci_dev->id.vendor_id,
               pci_dev->id.device_id);

  rte_intr_callback_register(pci_dev->intr_handle, eth_igc_interrupt_handler,
                             (void *)dev);
...
static int eth_igc_dev_uninit(__rte_unused struct rte_eth_dev *eth_dev) {
  PMD_INIT_FUNC_TRACE();
...
static int eth_igc_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
  ...
  if (fw.or_valid) {
    ret = snprintf(fw_version, fw_size, "%d.%d, 0x%08x, %d.%d.%d", fw.eep_major,
                   fw.eep_minor, fw.etrack_id, fw.or_major, fw.or_build,
                   fw.or_patch);
    /* no option rom */
  } else {
 ...
 */
static void igc_read_stats_registers(struct igc_hw *hw,
 ...
 */
static void igc_reset_queue_stats_register(struct igc_hw *hw) {
  int i;
...
static void igc_read_queue_stats_register(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  struct igc_hw_queue_stats *queue_stats = IGC_DEV_PRIVATE_QUEUE_STATS(dev);
  int i;
...
static int eth_igc_stats_get(struct rte_eth_dev *dev,
  ...
  rte_stats->ierrors =
      stats->crcerrs + stats->rlec + stats->rxerrc + stats->algnerrc;
...
static int eth_igc_xstats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < IGC_NB_XSTATS; i++) {
    xstats[i].id = i;
    xstats[i].value =
        *(uint64_t *)(((char *)hw_stats) + rte_igc_stats_strings[i].offset);
  }
...
static int eth_igc_xstats_get_names(__rte_unused struct rte_eth_dev *dev,
                                    struct rte_eth_xstat_name *xstats_names,
                                    unsigned int size) {
  unsigned int i;
...
eth_igc_xstats_get_names_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
  ...
  for (i = 0; i < limit; i++) {
    ...
    strlcpy(xstats_names[i].name, rte_igc_stats_strings[ids[i]].name,
            sizeof(xstats_names[i].name));
  }
  return limit;
}
...
static int eth_igc_queue_stats_mapping_set(struct rte_eth_dev *dev,
  ...
  if (queue_id >= IGC_QUEUE_PAIRS_NUM) {
    PMD_DRV_LOG(ERR, "queue id(%u) error, max is %u", queue_id,
...
static int eth_igc_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  if (fc_conf->high_water > max_high_water ||
    ...
    PMD_DRV_LOG(ERR, "Incorrect high(%u)/low(%u) water value, max is %u",
                fc_conf->high_water, fc_conf->low_water, max_high_water);
    return -EINVAL;
  }
...
static int eth_igc_rss_reta_update(struct rte_eth_dev *dev,
  ...
  if (reta_size != RTE_ETH_RSS_RETA_SIZE_128) {
    PMD_DRV_LOG(ERR,
                "The size of RSS redirection table configured(%d) doesn't "
  ...
  for (i = 0; i < RTE_ETH_RSS_RETA_SIZE_128; i += IGC_RSS_RDT_REG_SIZE) {
    ...
    mask =
        (uint8_t)((reta_conf[idx].mask >> shift) & IGC_RSS_RDT_REG_SIZE_MASK);

    /* if no need to update the register */
    if (!mask || shift > (RTE_ETH_RETA_GROUP_SIZE - IGC_RSS_RDT_REG_SIZE))
      continue;
    ...
    if (mask == IGC_RSS_RDT_REG_SIZE_MASK)
      ...
      reg.dword = IGC_READ_REG_LE_VALUE(hw, IGC_RETA(i / IGC_RSS_RDT_REG_SIZE));

    /* update the register */
    RTE_BUILD_BUG_ON(sizeof(reta.bytes) != IGC_RSS_RDT_REG_SIZE);
    for (j = 0; j < IGC_RSS_RDT_REG_SIZE; j++) {
      if (mask & (1u << j))
        reta.bytes[j] = (uint8_t)reta_conf[idx].reta[shift + j];
      else
        reta.bytes[j] = reg.bytes[j];
    }
    IGC_WRITE_REG_LE_VALUE(hw, IGC_RETA(i / IGC_RSS_RDT_REG_SIZE), reta.dword);
  }
...
static int eth_igc_rss_reta_query(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < RTE_ETH_RSS_RETA_SIZE_128; i += IGC_RSS_RDT_REG_SIZE) {
    ...
    RTE_BUILD_BUG_ON(sizeof(reta.bytes) != IGC_RSS_RDT_REG_SIZE);
    reta.dword = IGC_READ_REG_LE_VALUE(hw, IGC_RETA(i / IGC_RSS_RDT_REG_SIZE));
    for (j = 0; j < IGC_RSS_RDT_REG_SIZE; j++) {
      if (mask & (1u << j))
...
static int eth_igc_rss_hash_conf_get(struct rte_eth_dev *dev,
  ...
  if (hash_key != NULL) {
    ...
    if (rss_conf->rss_key_len != IGC_HKEY_SIZE) {
      PMD_DRV_LOG(ERR,
                  "RSS hash key size %u in parameter doesn't match the "
                  "hardware hash key size %u",
...
static void igc_vlan_hw_filter_disable(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  igc_read_reg_check_clear_bits(hw, IGC_RCTL, IGC_RCTL_CFIEN | IGC_RCTL_VFE);
}

static void igc_vlan_hw_filter_enable(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  struct igc_vfta *shadow_vfta = IGC_DEV_PRIVATE_VFTA(dev);
  uint32_t reg_val;
  int i;
...
static void igc_vlan_hw_strip_disable(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);

  igc_read_reg_check_clear_bits(hw, IGC_CTRL, IGC_CTRL_VME);
}

static void igc_vlan_hw_strip_enable(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);

  igc_read_reg_check_set_bits(hw, IGC_CTRL, IGC_CTRL_VME);
}

static int igc_vlan_hw_extend_disable(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  ...
  if (frame_size < RTE_ETHER_MIN_MTU + VLAN_TAG_SIZE) {
    PMD_DRV_LOG(ERR, "Maximum packet length %u error, min is %u", frame_size,
...
static int igc_vlan_hw_extend_enable(struct rte_eth_dev *dev) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  uint32_t frame_size = dev->data->mtu + IGC_ETH_OVERHEAD;
  uint32_t ctrl_ext;
  ...
  if (frame_size > MAX_RX_JUMBO_FRAME_SIZE) {
    PMD_DRV_LOG(ERR, "Maximum packet length %u error, max is %u", frame_size,
                MAX_RX_JUMBO_FRAME_SIZE);
    return -EINVAL;
  }
  IGC_WRITE_REG(hw, IGC_RLPML, frame_size);
...
static int eth_igc_vlan_tpid_set(struct rte_eth_dev *dev,
  ...
  if (vlan_type == RTE_ETH_VLAN_TYPE_OUTER) {
    ...
    reg_val =
        (reg_val & (~IGC_VET_EXT)) | ((uint32_t)tpid << IGC_VET_EXT_SHIFT);
...
static int eth_igc_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                             struct rte_pci_device *pci_dev) {
  PMD_INIT_FUNC_TRACE();
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct igc_adapter),
                                       eth_igc_dev_init);
}

static int eth_igc_pci_remove(struct rte_pci_device *pci_dev) {
  PMD_INIT_FUNC_TRACE();
