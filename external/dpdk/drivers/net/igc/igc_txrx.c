...
#define IGC_TX_OFFLOAD_MASK                                                    \
  (RTE_MBUF_F_TX_OUTER_IPV4 | RTE_MBUF_F_TX_IPV6 | RTE_MBUF_F_TX_IPV4 |        \
   RTE_MBUF_F_TX_VLAN | RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_L4_MASK |       \
...
static inline uint64_t rx_desc_statuserr_to_pkt_flags(uint32_t statuserr) {
  static uint64_t l4_chksum_flags[] = {0, 0, RTE_MBUF_F_RX_L4_CKSUM_GOOD,
                                       RTE_MBUF_F_RX_L4_CKSUM_BAD};

  static uint64_t l3_chksum_flags[] = {0, 0, RTE_MBUF_F_RX_IP_CKSUM_GOOD,
...
static inline uint32_t rx_desc_pkt_info_to_pkt_type(uint32_t pkt_info) {
  static const uint32_t ptype_table[IGC_PACKET_TYPE_MAX] __rte_cache_aligned = {
      [IGC_PACKET_TYPE_IPV4] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4,
      [IGC_PACKET_TYPE_IPV4_EXT] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT,
      [IGC_PACKET_TYPE_IPV6] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6,
      [IGC_PACKET_TYPE_IPV4_IPV6] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 |
                                    RTE_PTYPE_TUNNEL_IP |
                                    RTE_PTYPE_INNER_L3_IPV6,
      [IGC_PACKET_TYPE_IPV6_EXT] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT,
      [IGC_PACKET_TYPE_IPV4_IPV6_EXT] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 |
                                        RTE_PTYPE_TUNNEL_IP |
                                        RTE_PTYPE_INNER_L3_IPV6_EXT,
      [IGC_PACKET_TYPE_IPV4_TCP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP,
      [IGC_PACKET_TYPE_IPV6_TCP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP,
      [IGC_PACKET_TYPE_IPV4_IPV6_TCP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
          RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_TCP,
      [IGC_PACKET_TYPE_IPV6_EXT_TCP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_L4_TCP,
      [IGC_PACKET_TYPE_IPV4_IPV6_EXT_TCP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
          RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_TCP,
      [IGC_PACKET_TYPE_IPV4_UDP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP,
      [IGC_PACKET_TYPE_IPV6_UDP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_UDP,
      [IGC_PACKET_TYPE_IPV4_IPV6_UDP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
          RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_UDP,
      [IGC_PACKET_TYPE_IPV6_EXT_UDP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_L4_UDP,
      [IGC_PACKET_TYPE_IPV4_IPV6_EXT_UDP] =
          RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
          RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_UDP,
      [IGC_PACKET_TYPE_IPV4_SCTP] =
...
static inline void rx_desc_get_pkt_info(struct igc_rx_queue *rxq,
                                        struct rte_mbuf *rxm,
  ...
  pkt_flags =
      (hlen_type_rss & IGC_RXD_RSS_TYPE_MASK) ? RTE_MBUF_F_RX_RSS_HASH : 0;
...
static uint16_t igc_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                              uint16_t nb_pkts) {
  ...
  while (nb_rx < nb_pkts) {
    ...
    rxdp->read.pkt_addr = rte_cpu_to_le_64(rte_mbuf_data_iova_default(nmb));
    rxm->next = NULL;
...
static uint16_t igc_recv_scattered_pkts(void *rx_queue,
  ...
  next_desc:
    ...
    if (unlikely(rxq->crc_len > 0)) {
      ...
      if (data_len <= RTE_ETHER_CRC_LEN) {
        ...
        first_seg->nb_segs--;
        last_seg->data_len =
            last_seg->data_len - (RTE_ETHER_CRC_LEN - data_len);
        last_seg->next = NULL;
      } else {
        rxm->data_len = (uint16_t)(data_len - RTE_ETHER_CRC_LEN);
      }
    }
...
static void igc_rx_queue_release_mbufs(struct igc_rx_queue *rxq) {
  unsigned int i;
...
static void igc_rx_queue_release(struct igc_rx_queue *rxq) {
...
#define IGC_RXQ_SCAN_INTERVAL 4
  ...
  while (desc < rxq->nb_rx_desc - rxq->rx_tail) {
    if (unlikely(!(rxdp->wb.upper.status_error & IGC_RXD_STAT_DD)))
      return desc;
...
static int igc_alloc_rx_queue_mbufs(struct igc_rx_queue *rxq) {
  ...
  for (i = 0; i < rxq->nb_rx_desc; i++) {
    ...
    if (mbuf == NULL) {
      PMD_DRV_LOG(ERR, "RX mbuf alloc failed, queue_id=%hu", rxq->queue_id);
      return -ENOMEM;
    }
...
void igc_hw_rss_hash_set(struct igc_hw *hw, struct rte_eth_rss_conf *rss_conf) {
...
static void igc_rss_configure(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < IGC_RSS_RDT_SIZD; i++) {
    ...
    q_idx = (uint8_t)(
        (dev->data->nb_rx_queues > 1) ? i % dev->data->nb_rx_queues : 0);
    reta_idx = i % sizeof(reta);
    reta.bytes[reta_idx] = q_idx;
    if (reta_idx == sizeof(reta) - 1)
      IGC_WRITE_REG_LE_VALUE(hw, IGC_RETA(i / sizeof(reta)), reta.dword);
  }
...
void igc_rss_conf_set(struct igc_rss_filter *out,
  ...
  if (rss->queue_num <= IGC_RSS_RDT_SIZD) {
    memcpy(out->queue, rss->queue, sizeof(*out->queue) * rss->queue_num);
...
int igc_add_rss_filter(struct rte_eth_dev *dev, struct igc_rss_filter *rss) {
  struct rte_eth_rss_conf rss_conf = {
      .rss_key = rss->conf.key_len ? (void *)(uintptr_t)rss->conf.key : NULL,
  ...
  if ((rss_conf.rss_hf & IGC_RSS_OFFLOAD_ALL) == 0) {
    PMD_DRV_LOG(ERR,
                ...
                " been supported",
                rss_conf.rss_hf, (uint64_t)IGC_RSS_OFFLOAD_ALL);
    return -EINVAL;
  }
  ...
  for (i = 0; i < rss->conf.queue_num; i++)
    if (rss->conf.queue[i] >= dev->data->nb_rx_queues) {
      PMD_DRV_LOG(ERR, "Queue id %u is invalid!", rss->conf.queue[i]);
      return -EINVAL;
    }
...
static int igc_dev_mq_rx_configure(struct rte_eth_dev *dev) {
...
int igc_rx_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
     ...
     */
    rxq->crc_len =
        (offloads & RTE_ETH_RX_OFFLOAD_KEEP_CRC) ? RTE_ETHER_CRC_LEN : 0;

    bus_addr = rxq->rx_ring_phys_addr;
    IGC_WRITE_REG(hw, IGC_RDLEN(rxq->reg_idx),
                  rxq->nb_rx_desc * sizeof(union igc_adv_rx_desc));
    IGC_WRITE_REG(hw, IGC_RDBAH(rxq->reg_idx), (uint32_t)(bus_addr >> 32));
    IGC_WRITE_REG(hw, IGC_RDBAL(rxq->reg_idx), (uint32_t)bus_addr);
    ...
    if (buf_size >= 1024) {
      ...
      srrctl |=
          ((buf_size >> IGC_SRRCTL_BSIZEPKT_SHIFT) & IGC_SRRCTL_BSIZEPKT_MASK);
      buf_size = (uint16_t)((srrctl & IGC_SRRCTL_BSIZEPKT_MASK)
  ...
  rctl |= IGC_RCTL_EN | IGC_RCTL_BAM | IGC_RCTL_LBM_NO | IGC_RCTL_DPF |
          (hw->mac.mc_filter_type << IGC_RCTL_MO_SHIFT);

  if (dev->data->dev_conf.lpbk_mode == 1)
    rctl |= IGC_RCTL_LBM_MAC;

  rctl &= ~(IGC_RCTL_HSEL_MSK | IGC_RCTL_CFIEN | IGC_RCTL_CFI | IGC_RCTL_PSP |
            IGC_RCTL_PMCF);
...
static void igc_reset_rx_queue(struct igc_rx_queue *rxq) {
...
int eth_igc_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
   ...
   */
  if (nb_desc % IGC_RX_DESCRIPTOR_MULTIPLE != 0 || nb_desc > IGC_MAX_RXD ||
      nb_desc < IGC_MIN_RXD) {
    PMD_DRV_LOG(
        ERR,
        "RX descriptor must be multiple of %u(cur: %u) and between %u and %u",
        IGC_RX_DESCRIPTOR_MULTIPLE, nb_desc, IGC_MIN_RXD, IGC_MAX_RXD);
    return -EINVAL;
  }
  ...
  rz = rte_eth_dma_zone_reserve(dev, "rx_ring", queue_idx, size, IGC_ALIGN,
                                socket_id);
  if (rz == NULL) {
  ...
  rxq->sw_ring =
      rte_zmalloc("rxq->sw_ring", sizeof(struct igc_rx_entry) * nb_desc,
                  RTE_CACHE_LINE_SIZE);
  if (rxq->sw_ring == NULL) {
    igc_rx_queue_release(rxq);
    return -ENOMEM;
  ...
  PMD_DRV_LOG(DEBUG, "sw_ring=%p hw_ring=%p dma_addr=0x%" PRIx64, rxq->sw_ring,
              rxq->rx_ring, rxq->rx_ring_phys_addr);
...
static uint16_t eth_igc_prep_pkts(__rte_unused void *tx_queue,
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (m->ol_flags & IGC_TX_OFFLOAD_SEG)
      if (m->tso_segsz > IGC_TSO_MAX_MSS ||
          m->l2_len + m->l3_len + m->l4_len > IGC_TSO_MAX_HDRLEN) {
        rte_errno = EINVAL;
        return i;
      }
 ...
 */
static inline uint64_t check_tso_para(uint64_t ol_req,
                                      union igc_tx_offload ol_para) {
  if (!(ol_req & IGC_TX_OFFLOAD_SEG))
    return ol_req;
  if (ol_para.tso_segsz > IGC_TSO_MAX_MSS ||
      ol_para.l2_len + ol_para.l3_len + ol_para.l4_len > IGC_TSO_MAX_HDRLEN) {
 ...
 */
static inline uint32_t what_advctx_update(struct igc_tx_queue *txq,
                                          uint64_t flags,
                                          union igc_tx_offload tx_offload) {
  ...
  if (likely(
          txq->ctx_cache[curr].flags == flags &&
          txq->ctx_cache[curr].tx_offload.data ==
              (txq->ctx_cache[curr].tx_offload_mask.data & tx_offload.data))) {
    return curr;
  }
...
igc_set_xmit_ctx(struct igc_tx_queue *txq,
  ...
  if (ol_flags & IGC_TX_OFFLOAD_SEG) {
    ...
    if (ol_flags & RTE_MBUF_F_TX_IP_CKSUM)
      type_tucmd_mlhl =
          IGC_ADVTXD_TUCMD_IPV4 | IGC_ADVTXD_DTYP_CTXT | IGC_ADVTXD_DCMD_DEXT;
    else
      type_tucmd_mlhl =
          IGC_ADVTXD_TUCMD_IPV6 | IGC_ADVTXD_DTYP_CTXT | IGC_ADVTXD_DCMD_DEXT;
    ...
    mss_l4len_idx |= (uint32_t)tx_offload.tso_segsz << IGC_ADVTXD_MSS_SHIFT;
    mss_l4len_idx |= (uint32_t)tx_offload.l4_len << IGC_ADVTXD_L4LEN_SHIFT;
    ...
    case RTE_MBUF_F_TX_TCP_CKSUM:
      type_tucmd_mlhl |= IGC_ADVTXD_TUCMD_L4T_TCP | IGC_ADVTXD_DTYP_CTXT |
                         IGC_ADVTXD_DCMD_DEXT;
      mss_l4len_idx |= (uint32_t)sizeof(struct rte_tcp_hdr)
                       << IGC_ADVTXD_L4LEN_SHIFT;
      break;
    case RTE_MBUF_F_TX_UDP_CKSUM:
      type_tucmd_mlhl |= IGC_ADVTXD_TUCMD_L4T_UDP | IGC_ADVTXD_DTYP_CTXT |
                         IGC_ADVTXD_DCMD_DEXT;
    ...
    case RTE_MBUF_F_TX_SCTP_CKSUM:
      ...
      break;
    default:
      type_tucmd_mlhl |= IGC_ADVTXD_TUCMD_L4T_RSV | IGC_ADVTXD_DTYP_CTXT |
                         IGC_ADVTXD_DCMD_DEXT;
      break;
    }
  }
...
static inline uint32_t tx_desc_vlan_flags_to_cmdtype(uint64_t ol_flags) {
  uint32_t cmdtype;
...
static inline uint32_t tx_desc_cksum_flags_to_olinfo(uint64_t ol_flags) {
  ...
  tmp = l4_olinfo[(ol_flags & RTE_MBUF_F_TX_L4_MASK) !=
...
static uint16_t igc_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                              uint16_t nb_pkts) {
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (tx_ol_req) {
      ...
      new_ctx = what_advctx_update(txq, tx_ol_req, tx_offload);
    ...
    PMD_TX_LOG(DEBUG, "port_id=%u queue_id=%u pktlen=%u tx_first=%u tx_last=%u",
     ...
     *   - IGC_TXD_CMD_RS
     */
    cmd_type_len = txq->txd_type | IGC_ADVTXD_DCMD_IFCS | IGC_ADVTXD_DCMD_DEXT;
    ...
    if (tx_ol_req) {
      ...
      if (new_ctx) {
        volatile struct igc_adv_tx_context_desc *ctx_txd =
            (volatile struct igc_adv_tx_context_desc *)&txr[tx_id];
        ...
        igc_set_xmit_ctx(txq, ctx_txd, tx_ol_req, tx_offload);
      ...
      olinfo_status |= (uint32_t)txq->ctx_curr << IGC_ADVTXD_IDX_SHIFT;
    }
...
static void igc_tx_queue_release_mbufs(struct igc_tx_queue *txq) {
  unsigned int i;
...
static void igc_tx_queue_release(struct igc_tx_queue *txq) {
...
static void igc_reset_tx_queue_stat(struct igc_tx_queue *txq) {
  txq->tx_head = 0;
  txq->tx_tail = 0;
  txq->ctx_curr = 0;
...
static void igc_reset_tx_queue(struct igc_tx_queue *txq) {
...
int eth_igc_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (nb_desc % IGC_TX_DESCRIPTOR_MULTIPLE != 0 || nb_desc > IGC_MAX_TXD ||
      nb_desc < IGC_MIN_TXD) {
    PMD_DRV_LOG(
        ERR,
        "TX-descriptor must be a multiple of %u and between %u and %u, cur: %u",
        IGC_TX_DESCRIPTOR_MULTIPLE, IGC_MAX_TXD, IGC_MIN_TXD, nb_desc);
    return -EINVAL;
  }
  ...
  if (tx_conf->tx_thresh.wthresh == 0)
    PMD_DRV_LOG(INFO, "To improve 2.5G driver performance, consider setting "
  ...
  tz = rte_eth_dma_zone_reserve(dev, "tx_ring", queue_idx, size, IGC_ALIGN,
                                socket_id);
  if (tz == NULL) {
  ...
  /* Allocate software ring */
  txq->sw_ring =
      rte_zmalloc("txq->sw_ring", sizeof(struct igc_tx_entry) * nb_desc,
                  RTE_CACHE_LINE_SIZE);
  if (txq->sw_ring == NULL) {
    igc_tx_queue_release(txq);
    return -ENOMEM;
  }
  PMD_DRV_LOG(DEBUG, "sw_ring=%p hw_ring=%p dma_addr=0x%" PRIx64, txq->sw_ring,
...
int eth_igc_tx_done_cleanup(void *txqueue, uint32_t free_cnt) {
  ...
  while (1) {
    ...
    if (sw_ring[tx_last].mbuf) {
      if (!(txr[tx_last].wb.status & rte_cpu_to_le_32(IGC_TXD_STAT_DD)))
        break;
...
void igc_tx_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    IGC_WRITE_REG(hw, IGC_TDLEN(txq->reg_idx),
                  txq->nb_tx_desc * sizeof(union igc_adv_tx_desc));
    IGC_WRITE_REG(hw, IGC_TDBAH(txq->reg_idx), (uint32_t)(bus_addr >> 32));
    IGC_WRITE_REG(hw, IGC_TDBAL(txq->reg_idx), (uint32_t)bus_addr);
...
void eth_igc_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t rx_queue_id,
                                  int on) {
  struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
  ...
  if (rx_queue_id >= IGC_QUEUE_PAIRS_NUM) {
    PMD_DRV_LOG(ERR, "Queue index(%u) illegal, max is %u", rx_queue_id,
                IGC_QUEUE_PAIRS_NUM - 1);
    return;
  }
