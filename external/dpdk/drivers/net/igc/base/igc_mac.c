 ...
 *  @d: dummy variable
 **/
s32 igc_null_link_info(struct igc_hw IGC_UNUSEDARG *hw, u16 IGC_UNUSEDARG *s,
                       u16 IGC_UNUSEDARG *d) {
 ...
 *  @a: dummy variable
 **/
void igc_null_update_mc(struct igc_hw IGC_UNUSEDARG *hw, u8 IGC_UNUSEDARG *h,
                        u32 IGC_UNUSEDARG a) {
 ...
 *  @b: dummy variable
 **/
void igc_null_write_vfta(struct igc_hw IGC_UNUSEDARG *hw, u32 IGC_UNUSEDARG a,
                         u32 IGC_UNUSEDARG b) {
 ...
 *  @a: dummy variable
 **/
int igc_null_rar_set(struct igc_hw IGC_UNUSEDARG *hw, u8 IGC_UNUSEDARG *h,
                     u32 IGC_UNUSEDARG a) {
...
s32 igc_get_bus_info_pci_generic(struct igc_hw *hw) {
  ...
  /* PCI or PCI-X? */
  bus->type =
      (status & IGC_STATUS_PCIX_MODE) ? igc_bus_type_pcix : igc_bus_type_pci;

  /* Bus speed */
  if (bus->type == igc_bus_type_pci) {
    bus->speed =
        (status & IGC_STATUS_PCI66) ? igc_bus_speed_66 : igc_bus_speed_33;
  } else {
  ...
  /* Bus width */
  bus->width =
      (status & IGC_STATUS_BUS64) ? igc_bus_width_64 : igc_bus_width_32;
...
s32 igc_get_bus_info_pcie_generic(struct igc_hw *hw) {
  ...
  ret_val = igc_read_pcie_cap_reg(hw, PCIE_LINK_STATUS, &pcie_link_status);
  if (ret_val) {
    ...
    bus->width = (enum igc_bus_width)(
        (pcie_link_status & PCIE_LINK_WIDTH_MASK) >> PCIE_LINK_WIDTH_SHIFT);
  }
...
void igc_set_lan_id_multi_port_pci(struct igc_hw *hw) {
  ...
  if (pci_header_type & PCI_HEADER_TYPE_MULTIFUNC) {
    ...
    bus->func = (status & IGC_STATUS_FUNC_MASK) >> IGC_STATUS_FUNC_SHIFT;
  } else {
    bus->func = 0;
  }
}
...
s32 igc_check_alt_mac_addr_generic(struct igc_hw *hw) {
  ...
  ret_val =
      hw->nvm.ops.read(hw, NVM_ALT_MAC_ADDR_PTR, 1, &nvm_alt_mac_addr_offset);
  if (ret_val) {
    DEBUGOUT("NVM Read Error\n");
    return ret_val;
  }

  if (nvm_alt_mac_addr_offset == 0xFFFF || nvm_alt_mac_addr_offset == 0x0000)
...
u32 igc_hash_mc_addr_generic(struct igc_hw *hw, u8 *mc_addr) {
  ...
  hash_value =
      hash_mask &
      (((mc_addr[4] >> (8 - bit_shift)) | (((u16)mc_addr[5]) << bit_shift)));
 ...
 **/
void igc_update_mc_addr_list_generic(struct igc_hw *hw, u8 *mc_addr_list,
                                     u32 mc_addr_count) {
...
void igc_pcix_mmrbc_workaround_generic(struct igc_hw *hw) {
  ...
  cmd_mmrbc = (pcix_cmd & PCIX_COMMAND_MMRBC_MASK) >> PCIX_COMMAND_MMRBC_SHIFT;
...
s32 igc_set_default_fc_generic(struct igc_hw *hw) {
  ...
  if (hw->mac.type == igc_i350) {
    ...
    ret_val =
        hw->nvm.ops.read(hw, NVM_INIT_CONTROL2_REG + nvm_offset, 1, &nvm_data);
  } else {
...
s32 igc_config_fc_after_link_up_generic(struct igc_hw *hw) {
  ...
  if (hw->phy.media_type == igc_media_type_copper && mac->autoneg) {
     ...
     */
    ret_val = hw->phy.ops.read_reg(hw, PHY_AUTONEG_ADV, &mii_nway_adv_reg);
    if (ret_val)
      return ret_val;
    ret_val =
        hw->phy.ops.read_reg(hw, PHY_LP_ABILITY, &mii_nway_lp_ability_reg);
    if (ret_val)
      return ret_val;
...
s32 igc_get_speed_and_duplex_copper_generic(struct igc_hw *hw, u16 *speed,
  ...
  if (status & IGC_STATUS_SPEED_1000) {
     ...
     */
    if (hw->mac.type == igc_i225 && (status & IGC_STATUS_SPEED_2500)) {
      *speed = SPEED_2500;
      DEBUGOUT("2500 Mbs, ");
    } else {
      *speed = SPEED_1000;
 ...
 **/
s32 igc_get_speed_and_duplex_fiber_serdes_generic(struct igc_hw *hw, u16 *speed,
                                                  u16 *duplex) {
...
s32 igc_setup_led_generic(struct igc_hw *hw) {
  ...
  if (hw->phy.media_type == igc_media_type_fiber) {
    ...
    ledctl |= (IGC_LEDCTL_MODE_LED_OFF << IGC_LEDCTL_LED0_MODE_SHIFT);
...
s32 igc_blink_led_generic(struct igc_hw *hw) {
  ...
  if (hw->phy.media_type == igc_media_type_fiber) {
    ...
    for (i = 0; i < 32; i += 8) {
      u32 mode = (hw->mac.ledctl_mode2 >> i) & IGC_LEDCTL_LED0_MODE_MASK;
      ...
      if ((!(led_default & IGC_LEDCTL_LED0_IVRT) &&
        ...
        ledctl_blink &= ~(IGC_LEDCTL_LED0_MODE_MASK << i);
        ledctl_blink |= (IGC_LEDCTL_LED0_BLINK | IGC_LEDCTL_MODE_LED_ON) << i;
      }
    }
  }
...
s32 igc_disable_pcie_master_generic(struct igc_hw *hw) {
  ...
  while (timeout) {
    if (!(IGC_READ_REG(hw, IGC_STATUS) & IGC_STATUS_GIO_MASTER_ENABLE) ||
  ...
  if ((mac->collision_delta * mac->ifs_ratio) > mac->tx_packet_delta) {
    if (mac->tx_packet_delta > MIN_NUM_XMITS) {
      ...
      if (mac->current_ifs_val < mac->ifs_max_val) {
        ...
        IGC_WRITE_REG(hw, IGC_AIT, mac->current_ifs_val);
      }
    }
  } else {
 ...
 *  operation.
 **/
s32 igc_validate_mdi_setting_crossover_generic(
    struct igc_hw IGC_UNUSEDARG *hw) {
 ...
 *  completion.
 **/
s32 igc_write_8bit_ctrl_reg_generic(struct igc_hw *hw, u32 reg, u32 offset,
                                    u8 data) {
  u32 i, regvalue = 0;
