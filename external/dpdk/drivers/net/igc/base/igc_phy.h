...
s32 igc_read_i2c_byte_null(struct igc_hw *hw, u8 byte_offset, u8 dev_addr,
                           u8 *data);
s32 igc_write_i2c_byte_null(struct igc_hw *hw, u8 byte_offset, u8 dev_addr,
                            u8 data);
...
#define BM_PHY_REG(page, reg)                                                  \
  (__extension__({                                                             \
    ...
    typeof(reg) _reg = (reg);                                                  \
    (_reg & MAX_PHY_REG_ADDRESS) | ((_page & 0xFFFF) << PHY_PAGE_SHIFT) |      \
        ((_reg & ~MAX_PHY_REG_ADDRESS) << (PHY_UPPER_SHIFT - PHY_PAGE_SHIFT)); \
  }))
