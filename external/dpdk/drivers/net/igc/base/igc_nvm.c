 ...
 *  @c: dummy variable
 **/
s32 igc_null_read_nvm(struct igc_hw IGC_UNUSEDARG *hw, u16 IGC_UNUSEDARG a,
 ...
 *  @c: dummy variable
 **/
s32 igc_null_write_nvm(struct igc_hw IGC_UNUSEDARG *hw, u16 IGC_UNUSEDARG a,
                       u16 IGC_UNUSEDARG b, u16 IGC_UNUSEDARG *c) {
...
static s32 igc_ready_nvm_eeprom(struct igc_hw *hw) {
  ...
  if (nvm->type == igc_nvm_eeprom_microwire) {
     ...
     */
    while (timeout) {
      igc_shift_out_eec_bits(hw, NVM_RDSR_OPCODE_SPI, hw->nvm.opcode_bits);
...
s32 igc_read_nvm_microwire(struct igc_hw *hw, u16 offset, u16 words,
  ...
  for (i = 0; i < words; i++) {
    /* Send the READ command (opcode + addr) */
    igc_shift_out_eec_bits(hw, read_opcode, nvm->opcode_bits);
    igc_shift_out_eec_bits(hw, (u16)(offset + i), nvm->address_bits);
...
s32 igc_read_nvm_eerd(struct igc_hw *hw, u16 offset, u16 words, u16 *data) {
  ...
  for (i = 0; i < words; i++) {
    eerd = ((offset + i) << IGC_NVM_RW_ADDR_SHIFT) + IGC_NVM_RW_REG_START;
...
s32 igc_write_nvm_spi(struct igc_hw *hw, u16 offset, u16 words, u16 *data) {
  ...
  while (widx < words) {
    ...
    igc_shift_out_eec_bits(hw, NVM_WREN_OPCODE_SPI, nvm->opcode_bits);
    ...
    igc_shift_out_eec_bits(hw, (u16)((offset + widx) * 2), nvm->address_bits);
...
s32 igc_write_nvm_microwire(struct igc_hw *hw, u16 offset, u16 words,
  ...
  while (words_written < words) {
    igc_shift_out_eec_bits(hw, NVM_WRITE_OPCODE_MICROWIRE, nvm->opcode_bits);
 ...
 *
 **/
s32 igc_read_pba_raw(struct igc_hw *hw, u16 *eeprom_buf, u32 eeprom_buf_size,
                     u16 max_pba_block_size, struct igc_pba *pba) {
  s32 ret_val;
  u16 pba_block_size;
  ...
  if (eeprom_buf == NULL) {
    ret_val = igc_read_nvm(hw, NVM_PBA_OFFSET_0, 2, &pba->word[0]);
    if (ret_val)
      return ret_val;
  } else {
  ...
  if (pba->word[0] == NVM_PBA_PTR_GUARD) {
    ...
    ret_val = igc_get_pba_block_size(hw, eeprom_buf, eeprom_buf_size,
                                     &pba_block_size);
    if (ret_val)
      return ret_val;
    ...
    if (eeprom_buf == NULL) {
      ret_val = igc_read_nvm(hw, pba->word[1], pba_block_size, pba->pba_block);
      if (ret_val)
        return ret_val;
    } else {
 ...
 *
 **/
s32 igc_write_pba_raw(struct igc_hw *hw, u16 *eeprom_buf, u32 eeprom_buf_size,
                      struct igc_pba *pba) {
  s32 ret_val;
  ...
  if (eeprom_buf == NULL) {
    ret_val = igc_write_nvm(hw, NVM_PBA_OFFSET_0, 2, &pba->word[0]);
    if (ret_val)
      return ret_val;
  } else {
    if (eeprom_buf_size > NVM_PBA_OFFSET_1) {
  ...
  if (pba->word[0] == NVM_PBA_PTR_GUARD) {
    ...
    if (eeprom_buf == NULL) {
      ret_val =
          igc_write_nvm(hw, pba->word[1], pba->pba_block[0], pba->pba_block);
      if (ret_val)
        return ret_val;
    } else {
...
s32 igc_get_pba_block_size(struct igc_hw *hw, u16 *eeprom_buf,
  ...
  if (pba_word[0] == NVM_PBA_PTR_GUARD) {
    if (eeprom_buf == NULL) {
      ret_val = igc_read_nvm(hw, pba_word[1] + 0, 1, &length);
      if (ret_val)
        return ret_val;
    } else {
...
void igc_get_fw_version(struct igc_hw *hw, struct igc_fw_version *fw_vers) {
  ...
  case igc_i225:
    ...
    if (comb_offset && comb_offset != NVM_VER_INVALID) {
      hw->nvm.ops.read(hw, NVM_COMB_VER_OFF + comb_offset + 1, 1, &comb_verh);
      hw->nvm.ops.read(hw, NVM_COMB_VER_OFF + comb_offset, 1, &comb_verl);

      /* get Option Rom version if it exists and is valid */
      if (comb_verh && comb_verl && comb_verh != NVM_VER_INVALID &&
          comb_verl != NVM_VER_INVALID) {
        fw_vers->or_valid = true;
        fw_vers->or_major = comb_verl >> NVM_COMB_VER_SHFT;
        fw_vers->or_build =
            (comb_verl << NVM_COMB_VER_SHFT) | (comb_verh >> NVM_COMB_VER_SHFT);
