...
bool igc_enable_tx_pkt_filtering_generic(struct igc_hw *hw) {
  ...
  for (i = 0; i < len; i++)
    *(buffer + i) = IGC_READ_REG_ARRAY_DWORD(hw, IGC_HOST_IF, offset + i);
  hdr_csum = hdr->checksum;
  hdr->checksum = 0;
  csum = igc_calculate_checksum((u8 *)hdr, IGC_MNG_DHCP_COOKIE_LENGTH);
...
s32 igc_mng_write_cmd_header_generic(struct igc_hw *hw,
  ...
  for (i = 0; i < length; i++) {
    IGC_WRITE_REG_ARRAY_DWORD(hw, IGC_HOST_IF, i, *((u32 *)hdr + i));
    IGC_WRITE_FLUSH(hw);
  }
 ...
 **/
s32 igc_mng_host_if_write_generic(struct igc_hw *hw, u8 *buffer, u16 length,
                                  u16 offset, u8 *sum) {
  u8 *tmp;
  u8 *bufptr = buffer;
  u32 data = 0;
  ...
  for (i = 0; i < length; i++) {
    ...
    IGC_WRITE_REG_ARRAY_DWORD(hw, IGC_HOST_IF, offset + i, data);
  }
  if (remaining) {
    for (j = 0; j < sizeof(u32); j++) {
      if (j < remaining)
 ...
 **/
s32 igc_mng_write_dhcp_info_generic(struct igc_hw *hw, u8 *buffer, u16 length) {
  struct igc_host_mng_command_header hdr;
  s32 ret_val;
  u32 hicr;
  ...
  ret_val = igc_mng_host_if_write_generic(hw, buffer, length, sizeof(hdr),
                                          &hdr.checksum);
  if (ret_val)
    return ret_val;
...
bool igc_enable_mng_pass_thru(struct igc_hw *hw) {
  ...
  if (hw->mac.has_fwsm) {
    ...
    if (!(factps & IGC_FACTPS_MNGCG) &&
      ...
      return true;
  } else if ((hw->mac.type == igc_82574) || (hw->mac.type == igc_82583)) {
    u16 data;
    s32 ret_val;
    ...
    if (!(factps & IGC_FACTPS_MNGCG) &&
        ((data & IGC_NVM_INIT_CTRL2_MNGM) == (igc_mng_mode_pt << 13)))
      return true;
  } else if ((manc & IGC_MANC_SMBUS_EN) && !(manc & IGC_MANC_ASF_EN)) {
    return true;
  }
...
s32 igc_host_interface_command(struct igc_hw *hw, u8 *buffer, u32 length) {
  ...
  if (length == 0 || length & 0x3 || length > IGC_HI_MAX_BLOCK_BYTE_LENGTH) {
   ...
   */
  for (i = 0; i < length; i++)
    IGC_WRITE_REG_ARRAY_DWORD(hw, IGC_HOST_IF, i, *((u32 *)buffer + i));
  ...
  for (i = 0; i < length; i++)
    *((u32 *)buffer + i) = IGC_READ_REG_ARRAY_DWORD(hw, IGC_HOST_IF, i);
...
s32 igc_load_firmware(struct igc_hw *hw, u8 *buffer, u32 length) {
  ...
  for (i = 0; i < length; i++) {
    if (!(i % IGC_HI_FW_BLOCK_DWORD_LENGTH)) {
      ...
      hibba = IGC_HI_FW_BASE_ADDRESS + ((IGC_HI_FW_BLOCK_DWORD_LENGTH << 2) *
                                        (i / IGC_HI_FW_BLOCK_DWORD_LENGTH));

      IGC_WRITE_REG(hw, IGC_HIBBA, hibba);
    }

    IGC_WRITE_REG_ARRAY_DWORD(hw, IGC_HOST_IF, i % IGC_HI_FW_BLOCK_DWORD_LENGTH,
                              *((u32 *)buffer + i));
  }
