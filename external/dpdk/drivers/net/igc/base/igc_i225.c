...
s32 igc_read_nvm_srrd_i225(struct igc_hw *hw, u16 offset, u16 words,
  ...
  for (i = 0; i < words; i += IGC_EERD_EEWR_MAX_COUNT) {
    count = (words - i) / IGC_EERD_EEWR_MAX_COUNT > 0 ? IGC_EERD_EEWR_MAX_COUNT
                                                      : (words - i);
...
s32 igc_write_nvm_srwr_i225(struct igc_hw *hw, u16 offset, u16 words,
  ...
  for (i = 0; i < words; i += IGC_EERD_EEWR_MAX_COUNT) {
    count = (words - i) / IGC_EERD_EEWR_MAX_COUNT > 0 ? IGC_EERD_EEWR_MAX_COUNT
                                                      : (words - i);
    if (hw->nvm.ops.acquire(hw) == IGC_SUCCESS) {
      status = __igc_write_nvm_srwr(hw, offset, count, data + i);
      hw->nvm.ops.release(hw);
    } else {
      status = IGC_ERR_SWFW_SYNC;
    }
...
s32 igc_read_invm_version_i225(struct igc_hw *hw,
  ...
  u32 invm_dword = 0;
  u32 invm_blocks =
      IGC_INVM_SIZE - (IGC_INVM_ULT_BYTES_SIZE / IGC_INVM_RECORD_SIZE_IN_BYTES);
  ...
  for (i = 1; i < invm_blocks; i++) {
    ...
    else if ((i == 1) && ((*record & IGC_INVM_VER_FIELD_TWO) == 0)) {
     ...
     */
              ...
              ((*record & 0x3) == 0)) ||
             (((*record & 0x3) != 0) && (i != 1))) {
      version = (*next_record & IGC_INVM_VER_FIELD_TWO) >> 13;
      status = IGC_SUCCESS;
      break;
    }
  ...
  if (status == IGC_SUCCESS) {
    invm_ver->invm_major =
        (version & IGC_INVM_MAJOR_MASK) >> IGC_INVM_MAJOR_SHIFT;
  ...
  for (i = 1; i < invm_blocks; i++) {
    ...
    /* Check if we have image type in first location used */
             ...
             ((((*record & 0x3) != 0) && (i != 1)))) {
      invm_ver->invm_img_type = (*next_record & IGC_INVM_IMGTYPE_FIELD) >> 23;
      status = IGC_SUCCESS;
      break;
    }
  }
...
s32 igc_validate_nvm_checksum_i225(struct igc_hw *hw) {
  s32 status = IGC_SUCCESS;
  s32 (*read_op_ptr)(struct igc_hw * hw, u16 offset, u16 count, u16 * data);
...
s32 igc_update_nvm_checksum_i225(struct igc_hw *hw) {
  ...
  if (hw->nvm.ops.acquire(hw) == IGC_SUCCESS) {
    ...
    ret_val = __igc_write_nvm_srwr(hw, NVM_CHECKSUM_REG, 1, &checksum);
    if (ret_val != IGC_SUCCESS) {
      hw->nvm.ops.release(hw);
...
s32 igc_update_flash_i225(struct igc_hw *hw) {
  ...
  block_sw_protect =
      IGC_READ_REG(hw, IGC_I225_FLSECU) & IGC_FLSECU_BLK_SW_ACCESS_I225;
  ...
  if (fw_valid_bit) {
    ...
    /* Valid sector erase */
    ret_val = igc_write_erase_flash_command_i225(hw, IGC_I225_ERASE_CMD_OPCODE,
                                                 base_address);
    if (!ret_val) {
    ...
    for (i = 0; i < IGC_I225_SHADOW_RAM_SIZE / 2; i++) {
      ...
      ret_val = igc_set_flsw_flash_burst_counter_i225(hw, 0x2);
      if (ret_val != IGC_SUCCESS)
        break;

      /* Set address and opcode */
      ret_val = igc_write_erase_flash_command_i225(
          hw, IGC_I225_WRITE_CMD_OPCODE, 2 * current_offset);
      if (ret_val != IGC_SUCCESS)
        break;

      ret_val = igc_read_nvm_eerd(hw, current_offset, 1, &current_offset_data);
      if (ret_val) {
      ...
      IGC_WRITE_REG(hw, IGC_I225_FLSWDATA, current_offset_data);
      current_offset++;
...
static s32 igc_set_ltr_i225(struct igc_hw *hw, bool link) {
  ...
  if (link) {
    ...
    if (hw->phy.media_type == igc_media_type_copper &&
      ...
      if (speed == SPEED_100)
        tw_system = ((IGC_READ_REG(hw, IGC_EEE_SU) & IGC_TW_SYSTEM_100_MASK) >>
                     IGC_TW_SYSTEM_100_SHIFT) *
                    500;
      else
        tw_system =
            (IGC_READ_REG(hw, IGC_EEE_SU) & IGC_TW_SYSTEM_1000_MASK) * 500;
    } else {
      tw_system = 0;
    }
    ...
    if (IGC_READ_REG(hw, IGC_DMACR) & IGC_DMACR_DMAC_EN) {
      size -= (IGC_READ_REG(hw, IGC_DMACR) & IGC_DMACR_DMACTHR_MASK) >>
...
s32 igc_set_eee_i225(struct igc_hw *hw, bool adv2p5G, bool adv1G,
  ...
  if (hw->mac.type != igc_i225 || hw->phy.media_type != igc_media_type_copper)
    goto out;
