...
#define M88IGC_CABLE_LENGTH_TABLE_SIZE                                         \
  (sizeof(igc_m88_cable_length_table) / sizeof(igc_m88_cable_length_table[0]))

static const u16 igc_igp_2_cable_length_table[] = {
    0,   0,   0,   0,   0,   0,   0,   0,  3,  5,   8,   11,  13,  16,  18,
    21,  0,   0,   0,   3,   6,   10,  13, 16, 19,  23,  26,  29,  32,  35,
    38,  41,  6,   10,  14,  18,  22,  26, 30, 33,  37,  41,  44,  48,  51,
    54,  58,  61,  21,  26,  31,  35,  40, 44, 49,  53,  57,  61,  65,  68,
    72,  75,  79,  82,  40,  45,  51,  56, 61, 66,  70,  75,  79,  83,  87,
    91,  94,  98,  101, 104, 60,  66,  72, 77, 82,  87,  92,  96,  100, 104,
    108, 111, 114, 117, 119, 121, 83,  89, 95, 100, 105, 109, 113, 116, 119,
    122, 124, 104, 109, 114, 118, 121, 124};
 ...
 *  @data: dummy variable
 **/
s32 igc_null_set_page(struct igc_hw IGC_UNUSEDARG *hw, u16 IGC_UNUSEDARG data) {
 ...
 *  @data: dummy variable
 **/
s32 igc_null_read_reg(struct igc_hw IGC_UNUSEDARG *hw, u32 IGC_UNUSEDARG offset,
...
s32 igc_read_i2c_byte_null(struct igc_hw IGC_UNUSEDARG *hw,
                           ...
                           u8 IGC_UNUSEDARG dev_addr, u8 IGC_UNUSEDARG *data) {
...
s32 igc_check_reset_block_generic(struct igc_hw *hw) {
  ...
  return (manc & IGC_MANC_BLK_PHY_RST_ON_IDE) ? IGC_BLK_PHY_RESET : IGC_SUCCESS;
}
...
s32 igc_read_phy_reg_mdic(struct igc_hw *hw, u32 offset, u16 *data) {
   ...
   */
  mdic = ((offset << IGC_MDIC_REG_SHIFT) | (phy->addr << IGC_MDIC_PHY_SHIFT) |
          (IGC_MDIC_OP_READ));
...
s32 igc_read_phy_reg_i2c(struct igc_hw *hw, u32 offset, u16 *data) {
   ...
   */
  i2ccmd =
      ((offset << IGC_I2CCMD_REG_ADDR_SHIFT) |
       (phy->addr << IGC_I2CCMD_PHY_ADDR_SHIFT) | (IGC_I2CCMD_OPCODE_READ));
...
s32 igc_write_phy_reg_i2c(struct igc_hw *hw, u32 offset, u16 data) {
  ...
  i2ccmd = ((offset << IGC_I2CCMD_REG_ADDR_SHIFT) |
            (phy->addr << IGC_I2CCMD_PHY_ADDR_SHIFT) | IGC_I2CCMD_OPCODE_WRITE |
            phy_data_swapped);
...
s32 igc_read_sfp_data_byte(struct igc_hw *hw, u16 offset, u8 *data) {
   ...
   */
  i2ccmd = ((offset << IGC_I2CCMD_REG_ADDR_SHIFT) | IGC_I2CCMD_OPCODE_READ);
...
s32 igc_write_sfp_data_byte(struct igc_hw *hw, u16 offset, u8 data) {
  ...
  for (i = 0; i < IGC_I2CCMD_PHY_TIMEOUT; i++) {
    ...
    if (i2ccmd & IGC_I2CCMD_READY) {
      ...
      if ((i2ccmd & IGC_I2CCMD_OPCODE_READ) == IGC_I2CCMD_OPCODE_READ) {
...
s32 igc_read_phy_reg_m88(struct igc_hw *hw, u32 offset, u16 *data) {
  ...
  ret_val = igc_read_phy_reg_mdic(hw, MAX_PHY_REG_ADDRESS & offset, data);
...
s32 igc_write_phy_reg_m88(struct igc_hw *hw, u32 offset, u16 data) {
  ...
  ret_val = igc_write_phy_reg_mdic(hw, MAX_PHY_REG_ADDRESS & offset, data);
...
static s32 __igc_read_phy_reg_igp(struct igc_hw *hw, u32 offset, u16 *data,
  ...
  if (offset > MAX_PHY_MULTI_PAGE_REG)
    ret_val = igc_write_phy_reg_mdic(hw, IGP01IGC_PHY_PAGE_SELECT, (u16)offset);
  if (!ret_val)
    ret_val = igc_read_phy_reg_mdic(hw, MAX_PHY_REG_ADDRESS & offset, data);
  if (!locked)
    hw->phy.ops.release(hw);
...
static s32 __igc_read_kmrn_reg(struct igc_hw *hw, u32 offset, u16 *data,
  ...
  kmrnctrlsta =
      ((offset << IGC_KMRNCTRLSTA_OFFSET_SHIFT) & IGC_KMRNCTRLSTA_OFFSET) |
      IGC_KMRNCTRLSTA_REN;
...
static s32 igc_set_master_slave_mode(struct igc_hw *hw) {
  ...
  hw->phy.original_ms_type =
      (phy_data & CR_1000T_MS_ENABLE)
          ? ((phy_data & CR_1000T_MS_VALUE) ? igc_ms_force_master
                                            : igc_ms_force_slave)
          : igc_ms_auto;
...
s32 igc_copper_link_setup_m88(struct igc_hw *hw) {
  ...
  if (phy->type == igc_phy_m88 && phy->revision < IGC_REVISION_4 &&
    ...
    if (phy->revision == IGC_REVISION_2 && phy->id == M88E1111_I_PHY_ID) {
      ...
      phy_data |=
          (M88IGC_EPSCR_MASTER_DOWNSHIFT_1X | M88IGC_EPSCR_SLAVE_DOWNSHIFT_1X);
    }
...
s32 igc_copper_link_setup_igp(struct igc_hw *hw) {
  ...
  if (hw->mac.autoneg) {
    ...
    if (phy->autoneg_advertised == ADVERTISE_1000_FULL) {
      /* Disable SmartSpeed */
      ret_val = phy->ops.read_reg(hw, IGP01IGC_PHY_PORT_CONFIG, &data);
      if (ret_val)
        return ret_val;

      data &= ~IGP01IGC_PSCFR_SMART_SPEED;
      ret_val = phy->ops.write_reg(hw, IGP01IGC_PHY_PORT_CONFIG, data);
      if (ret_val)
        return ret_val;
...
s32 igc_phy_setup_autoneg(struct igc_hw *hw) {
  ...
  if (phy->autoneg_mask & ADVERTISE_1000_FULL) {
    ...
    ret_val = phy->ops.read_reg(hw, PHY_1000T_CTRL, &mii_1000t_ctrl_reg);
    if (ret_val)
      return ret_val;
  }
  ...
  if ((phy->autoneg_mask & ADVERTISE_2500_FULL) &&
    ...
    ret_val = phy->ops.read_reg(
        hw, (STANDARD_AN_REG_MASK << MMD_DEVADDR_SHIFT) | ANEG_MULTIGBT_AN_CTRL,
  ...
  if (phy->autoneg_mask & ADVERTISE_1000_FULL)
    ret_val = phy->ops.write_reg(hw, PHY_1000T_CTRL, mii_1000t_ctrl_reg);

  if ((phy->autoneg_mask & ADVERTISE_2500_FULL) && hw->phy.id == I225_I_PHY_ID)
    ret_val = phy->ops.write_reg(
        hw, (STANDARD_AN_REG_MASK << MMD_DEVADDR_SHIFT) | ANEG_MULTIGBT_AN_CTRL,
        aneg_multigbt_an_ctrl);
...
s32 igc_phy_force_speed_duplex_igp(struct igc_hw *hw) {
  ...
  if (phy->autoneg_wait_to_complete) {
    ...
    ret_val = igc_phy_has_link_generic(hw, PHY_FORCE_LIMIT, 100000, &link);
    if (ret_val)
      return ret_val;
...
s32 igc_phy_force_speed_duplex_m88(struct igc_hw *hw) {
  ...
  if (phy->autoneg_wait_to_complete) {
    ...
    if (!link) {
      ...
      if (!reset_dsp) {
         ...
         */
        ret_val = phy->ops.write_reg(hw, M88IGC_PHY_PAGE_SELECT, 0x001d);
        if (ret_val)
          return ret_val;
  ...
  if (hw->phy.id == I347AT4_E_PHY_ID || hw->phy.id == M88E1340M_E_PHY_ID ||
...
s32 igc_check_polarity_m88(struct igc_hw *hw) {
  ...
  if (!ret_val)
    phy->cable_polarity =
        ((data & M88IGC_PSSR_REV_POLARITY) ? igc_rev_polarity_reversed
...
s32 igc_check_polarity_igp(struct igc_hw *hw) {
  ...
  if ((data & IGP01IGC_PSSR_SPEED_MASK) == IGP01IGC_PSSR_SPEED_1000MBPS) {
  ...
  if (!ret_val)
    phy->cable_polarity =
        ((data & mask) ? igc_rev_polarity_reversed : igc_rev_polarity_normal);
...
s32 igc_check_polarity_ife(struct igc_hw *hw) {
  ...
  if (!ret_val)
    phy->cable_polarity = ((phy_data & mask) ? igc_rev_polarity_reversed
                                             : igc_rev_polarity_normal);
...
s32 igc_get_cable_length_m88(struct igc_hw *hw) {
  ...
  index =
      ((phy_data & M88IGC_PSSR_CABLE_LENGTH) >> M88IGC_PSSR_CABLE_LENGTH_SHIFT);
...
s32 igc_get_cable_length_m88_gen2(struct igc_hw *hw) {
  ...
  case I210_I_PHY_ID:
    ...
    ret_val = phy->ops.read_reg(
        hw, (0x7 << GS40G_PAGE_SHIFT) + (I347AT4_PCDL + phy->addr), &phy_data);
    if (ret_val)
      return ret_val;

    /* Check if the unit of cable length is meters or cm */
    ret_val = phy->ops.read_reg(hw, (0x7 << GS40G_PAGE_SHIFT) + I347AT4_PCDC,
                                &phy_data2);
    if (ret_val)
      return ret_val;
  ...
  case I347AT4_E_PHY_ID:
    ...
    ret_val = phy->ops.read_reg(hw, I347AT4_PAGE_SELECT, &default_page);
    if (ret_val)
      return ret_val;
    ...
    /* Get cable length from PHY Cable Diagnostics Control Reg */
    ret_val = phy->ops.read_reg(hw, (I347AT4_PCDL + phy->addr), &phy_data);
    if (ret_val)
      return ret_val;
    ...
    ret_val = phy->ops.write_reg(hw, I347AT4_PAGE_SELECT, default_page);
    if (ret_val)
      return ret_val;
    break;
  ...
  case M88E1112_E_PHY_ID:
    ...
    ret_val = phy->ops.read_reg(hw, M88E1112_VCT_DSP_DISTANCE, &phy_data);
    if (ret_val)
      return ret_val;
...
s32 igc_get_cable_length_igp_2(struct igc_hw *hw) {
  ...
  static const u16 agc_reg_array[IGP02IGC_PHY_CHANNEL_NUM] = {
      IGP02IGC_PHY_AGC_A, IGP02IGC_PHY_AGC_B, IGP02IGC_PHY_AGC_C,
      IGP02IGC_PHY_AGC_D};
  ...
  for (i = 0; i < IGP02IGC_PHY_CHANNEL_NUM; i++) {
     ...
     */
    cur_agc_index =
        ((phy_data >> IGP02IGC_AGC_LENGTH_SHIFT) & IGP02IGC_AGC_LENGTH_MASK);

    /* Array index bound check. */
    if (cur_agc_index >= IGP02IGC_CABLE_LENGTH_TABLE_SIZE || cur_agc_index == 0)
      return -IGC_ERR_PHY;
  ...
  phy->min_cable_length =
      (((agc_value - IGP02IGC_AGC_RANGE) > 0) ? (agc_value - IGP02IGC_AGC_RANGE)
                                              : 0);
...
s32 igc_get_phy_info_m88(struct igc_hw *hw) {
  ...
  phy->polarity_correction = !!(phy_data & M88IGC_PSCR_POLARITY_REVERSAL);
...
s32 igc_get_phy_info_ife(struct igc_hw *hw) {
  ...
  if (phy->polarity_correction) {
    ...
    /* Polarity is forced */
    phy->cable_polarity =
        ((data & IFE_PSC_FORCE_POLARITY) ? igc_rev_polarity_reversed
                                         : igc_rev_polarity_normal);
  }
...
s32 igc_write_phy_reg_bm(struct igc_hw *hw, u32 offset, u16 data) {
  ...
  if (page == BM_WUC_PAGE) {
    ret_val = igc_access_phy_wakeup_reg_bm(hw, offset, &data, false, false);
    goto release;
  }
  ...
  if (offset > MAX_PHY_MULTI_PAGE_REG) {
    ...
    ret_val = igc_write_phy_reg_mdic(hw, page_select, (page << page_shift));
    if (ret_val)
      goto release;
  }
...
s32 igc_read_phy_reg_bm(struct igc_hw *hw, u32 offset, u16 *data) {
  ...
  /* Page 800 works differently than the rest so it has its own func */
  if (page == BM_WUC_PAGE) {
    ret_val = igc_access_phy_wakeup_reg_bm(hw, offset, data, true, false);
    goto release;
  }
...
s32 igc_read_phy_reg_bm2(struct igc_hw *hw, u32 offset, u16 *data) {
  ...
  if (offset > MAX_PHY_MULTI_PAGE_REG) {
    /* Page is shifted left, PHY expects (page x 32) */
    ret_val = igc_write_phy_reg_mdic(hw, BM_PHY_PAGE_SELECT, page);
...
static s32 igc_access_phy_wakeup_reg_bm(struct igc_hw *hw, u32 offset,
  ...
  if (read) {
    ...
    ret_val = igc_read_phy_reg_mdic(hw, BM_WUC_DATA_OPCODE, data);
  } else {
    /* Write the Wakeup register page value using opcode 0x12 */
    ret_val = igc_write_phy_reg_mdic(hw, BM_WUC_DATA_OPCODE, *data);
  }
...
static s32 __igc_read_phy_reg_hv(struct igc_hw *hw, u32 offset, u16 *data,
  ...
  /* Page 800 works differently than the rest so it has its own func */
  if (page == BM_WUC_PAGE) {
    ret_val = igc_access_phy_wakeup_reg_bm(hw, offset, data, true, page_set);
    goto out;
  }

  if (page > 0 && page < HV_INTC_FC_PAGE_START) {
    ret_val = igc_access_phy_debug_regs_hv(hw, offset, data, true);
    goto out;
  }
  ...
  if (!page_set) {
    ...
    if (reg > MAX_PHY_MULTI_PAGE_REG) {
      /* Page is shifted left, PHY expects (page x 32) */
      ret_val = igc_set_page_igp(hw, (page << IGP_PAGE_SHIFT));
  ...
  ret_val = igc_read_phy_reg_mdic(hw, MAX_PHY_REG_ADDRESS & reg, data);
out:
  if (!locked)
    hw->phy.ops.release(hw);
...
static s32 __igc_write_phy_reg_hv(struct igc_hw *hw, u32 offset, u16 data,
  ...
  /* Page 800 works differently than the rest so it has its own func */
  if (page == BM_WUC_PAGE) {
    ret_val = igc_access_phy_wakeup_reg_bm(hw, offset, &data, false, page_set);
    goto out;
  }

  if (page > 0 && page < HV_INTC_FC_PAGE_START) {
    ret_val = igc_access_phy_debug_regs_hv(hw, offset, &data, false);
    goto out;
  }
  ...
  if (!page_set) {
     ...
     */
    if (hw->phy.type == igc_phy_82578 && hw->phy.revision >= 1 &&
        hw->phy.addr == 2 && !(MAX_PHY_REG_ADDRESS & reg) &&
        (data & (1 << 11))) {
      u16 data2 = 0x7EFF;
      ret_val = igc_access_phy_debug_regs_hv(hw, (1 << 6) | 0x3, &data2, false);
      if (ret_val)
        goto out;
    }
  ...
  ret_val = igc_write_phy_reg_mdic(hw, MAX_PHY_REG_ADDRESS & reg, data);
...
static s32 igc_access_phy_debug_regs_hv(struct igc_hw *hw, u32 offset,
  ...
  addr_reg =
      ((hw->phy.type == igc_phy_82578) ? I82578_ADDR_REG : I82577_ADDR_REG);
  data_reg = addr_reg + 1;
...
s32 igc_link_stall_workaround_hv(struct igc_hw *hw) {
  ...
  data &=
      (BM_CS_STATUS_LINK_UP | BM_CS_STATUS_RESOLVED | BM_CS_STATUS_SPEED_MASK);
...
s32 igc_check_polarity_82577(struct igc_hw *hw) {
  ...
  if (!ret_val)
    phy->cable_polarity =
        ((data & I82577_PHY_STATUS2_REV_POLARITY) ? igc_rev_polarity_reversed
                                                  : igc_rev_polarity_normal);
...
s32 igc_read_phy_reg_mphy(struct igc_hw *hw, u32 address, u32 *data) {
  ...
  mphy_ctrl =
      (mphy_ctrl & ~IGC_MPHY_ADDRESS_MASK & ~IGC_MPHY_ADDRESS_FNC_OVERRIDE) |
  ...
  *data = IGC_READ_REG(hw, IGC_MPHY_DATA);
  ...
  if (!ready)
    return -IGC_ERR_PHY;
  IGC_WRITE_REG(hw, IGC_MPHY_ADDR_CTRL, IGC_MPHY_DIS_ACCESS);
...
s32 igc_write_phy_reg_mphy(struct igc_hw *hw, u32 address, u32 data,
  ...
  if (line_override)
    ...
    mphy_ctrl &= ~IGC_MPHY_ADDRESS_FNC_OVERRIDE;
  mphy_ctrl =
      (mphy_ctrl & ~IGC_MPHY_ADDRESS_MASK) | (address & IGC_MPHY_ADDRESS_MASK);
  IGC_WRITE_REG(hw, IGC_MPHY_ADDR_CTRL, mphy_ctrl);
 ...
 **/
static s32 __igc_access_xmdio_reg(struct igc_hw *hw, u16 address, u8 dev_addr,
                                  u16 *data, bool read) {
  s32 ret_val;
  ...
  ret_val =
      hw->phy.ops.write_reg(hw, IGC_MMDAC, IGC_MMDAC_FUNC_DATA | dev_addr);
  if (ret_val)
    return ret_val;
