...
#define GET_EQ_NUM_PAGES(eq, size)                                             \
  ((u16)(ALIGN((eq)->eq_len * (u32)(eq)->elem_size, (size)) / (size)))
...
static void set_eq_cons_idx(struct hinic_eq *eq, u32 arm_state) {
  ...
  val = EQ_CONS_IDX_CLEAR(val, CONS_IDX) & EQ_CONS_IDX_CLEAR(val, INT_ARMED) &
 ...
 * @eq: the event queue to update the cons idx for
 */
void eq_update_ci(struct hinic_eq *eq) { set_eq_cons_idx(eq, HINIC_EQ_ARMED); }
...
static void set_aeq_ctrls(struct hinic_eq *eq) {
  ...
  val = AEQ_CTRL_0_CLEAR(val, INTR_IDX) & AEQ_CTRL_0_CLEAR(val, DMA_ATTR) &
        AEQ_CTRL_0_CLEAR(val, PCI_INTF_IDX) & AEQ_CTRL_0_CLEAR(val, INTR_MODE);
...
static int alloc_eq_pages(struct hinic_eq *eq) {
  ...
  for (pg_num = 0; pg_num < eq->num_pages; pg_num++) {
    eq->virt_addr[pg_num] = (u8 *)dma_zalloc_coherent_aligned(
        eq->hwdev, eq->page_size, &eq->dma_addr[pg_num], SOCKET_ID_ANY);
    ...
    hinic_hwif_write_reg(hwif,
                         HINIC_EQ_HI_PHYS_ADDR_REG(eq->type, eq->q_id, pg_num),
                         upper_32_bits(eq->dma_addr[pg_num]));

    hinic_hwif_write_reg(hwif,
                         HINIC_EQ_LO_PHYS_ADDR_REG(eq->type, eq->q_id, pg_num),
...
dma_alloc_err:
  for (i = 0; i < pg_num; i++)
    dma_free_coherent(eq->hwdev, eq->page_size, eq->virt_addr[i],
                      eq->dma_addr[i]);
...
static void free_eq_pages(struct hinic_eq *eq) {
  ...
  for (pg_num = 0; pg_num < eq->num_pages; pg_num++)
    dma_free_coherent(hwdev, eq->page_size, eq->virt_addr[pg_num],
                      eq->dma_addr[pg_num]);
...
static int init_aeq(struct hinic_eq *eq, struct hinic_hwdev *hwdev, u16 q_id,
  ...
  hinic_hwif_write_reg(eq->hwdev->hwif, HINIC_CSR_AEQ_CTRL_1_ADDR(eq->q_id), 0);
  ...
  if (eq->num_pages > HINIC_EQ_MAX_PAGES) {
    PMD_DRV_LOG(ERR, "Too many pages for eq, num_pages: %d", eq->num_pages);
    return -EINVAL;
  }
...
static void remove_aeq(struct hinic_eq *eq) {
  ...
  if (eq->q_id == 0)
    hinic_set_msix_state(eq->hwdev, entry->msix_entry_idx, HINIC_MSIX_DISABLE);
  ...
  eq->cons_idx =
      (u16)hinic_hwif_read_reg(eq->hwdev->hwif, EQ_PROD_IDX_REG_ADDR(eq));
 ...
 * Return: 0 - Success, Negative - failure
 */
static int hinic_aeqs_init(struct hinic_hwdev *hwdev, u16 num_aeqs,
  ...
  for (q_id = HINIC_AEQN_START; q_id < num_aeqs; q_id++) {
    err = init_aeq(&aeqs->aeq[q_id], hwdev, q_id, HINIC_DEFAULT_AEQ_LEN,
...
void hinic_dump_aeq_info(struct hinic_hwdev *hwdev) {
  ...
  for (q_id = 0; q_id < hwdev->aeqs->num_aeqs; q_id++) {
    ...
    PMD_DRV_LOG(ERR, "aeq id: %d, ci: 0x%x, pi: 0x%x", q_id, ci, pi);
  }
}
...
void hinic_comm_aeqs_free(struct hinic_hwdev *hwdev) { hinic_aeqs_free(hwdev); }
