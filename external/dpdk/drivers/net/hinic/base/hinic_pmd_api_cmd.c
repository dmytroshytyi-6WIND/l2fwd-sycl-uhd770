...
static u16 get_cell_data_size(enum hinic_api_cmd_chain_type type,
  ...
  case HINIC_API_CMD_WRITE_ASYNC_TO_MGMT_CPU:
  case HINIC_API_CMD_PMD_WRITE_TO_MGMT:
    cell_data_size = ALIGN(API_CMD_CELL_DESC_SIZE + API_CMD_CELL_DATA_ADDR_SIZE,
...
static void prepare_api_cmd(struct hinic_api_cmd_chain *chain,
  ...
  cell->desc |= HINIC_API_CMD_DESC_SET(xor_chksum_set(&cell->desc), XOR_CHKSUM);
 ...
 */
static int
wait_for_api_cmd_completion(struct hinic_api_cmd_chain *chain,
 ...
 * Return: 0 - success, negative - failure
 */
static int api_cmd(struct hinic_api_cmd_chain *chain, enum hinic_node_id dest,
...
static void api_cmd_ctrl_init(struct hinic_api_cmd_chain *chain) {
  ...
  cell_size = (u32)ilog2(chain->cell_size >> API_CMD_CHAIN_CELL_SIZE_SHIFT);
...
static int alloc_cmd_buf(struct hinic_api_cmd_chain *chain,
  ...
  cmd_vaddr_alloc =
      dma_zalloc_coherent(dev, (API_CMD_BUF_SIZE + API_PAYLOAD_ALIGN_SIZE),
...
static int api_cmd_create_cell(struct hinic_api_cmd_chain *chain, u32 cell_idx,
  ...
  node_vaddr_alloc =
      dma_zalloc_coherent(dev, (chain->cell_size + API_CMD_NODE_ALIGN_SIZE),
...
static void api_cmd_destroy_cell(struct hinic_api_cmd_chain *chain,
  ...
  if (cell_ctxt->api_cmd_vaddr) {
    ...
    dma_free_coherent(dev, (chain->cell_size + API_CMD_NODE_ALIGN_SIZE), node,
                      node_paddr);
  }
}
...
static int api_chain_init(struct hinic_api_cmd_chain *chain,
  ...
  chain->wb_status = (struct hinic_api_cmd_status *)dma_zalloc_coherent(
      dev, sizeof(*chain->wb_status), &chain->wb_status_paddr, SOCKET_ID_ANY);
  if (!chain->wb_status) {
...
int hinic_api_cmd_init(struct hinic_hwdev *hwdev,
  ...
  for (; chain_type < HINIC_API_CMD_MAX; chain_type++) {
    ...
    if (err) {
      PMD_DRV_LOG(ERR, "Create chain %d failed", chain_type);
