...
#define CMDQ_DB_INFO_SET(val, member)                                          \
  (((val)&CMDQ_DB_INFO_##member##_MASK) << CMDQ_DB_INFO_##member##_SHIFT)
...
#define CMDQ_WQE_HEADER_SET(val, member)                                       \
  (((val)&CMDQ_WQE_HEADER_##member##_MASK) << CMDQ_WQE_HEADER_##member##_SHIFT)
...
#define CMDQ_CTXT_PAGE_INFO_CLEAR(val, member)                                 \
  ((val) & (~((u64)CMDQ_CTXT_##member##_MASK << CMDQ_CTXT_##member##_SHIFT)))
...
#define cmdq_to_cmdqs(cmdq)                                                    \
  container_of((cmdq) - (cmdq)->cmdq_type, struct hinic_cmdqs, cmdq[0])
...
static void cmdq_set_completion(struct hinic_cmdq_completion *complete,
  ...
  hinic_set_sge(&sge_resp->sge, buf_out->dma_addr, HINIC_CMDQ_BUF_SIZE);
}
...
static void cmdq_set_db(struct hinic_cmdq *cmdq, enum hinic_cmdq_type cmdq_type,
                        u16 prod_idx) {
...
static void cmdq_prepare_wqe_ctrl(struct hinic_cmdq_wqe *wqe, int wrapped,
  ...
  ctrl->ctrl_info = CMDQ_CTRL_SET(prod_idx, PI) | CMDQ_CTRL_SET(cmd, CMD) |
                    CMDQ_CTRL_SET(mod, MOD) | CMDQ_CTRL_SET(ack_type, ACK_TYPE);
  ...
  if (cmd == CMDQ_SET_ARM_CMD && mod == HINIC_MOD_COMM) {
    ...
    WQE_HEADER(wqe)->saved_data = saved_data | SAVED_DATA_SET(1, ARM);
  } else {
    saved_data &= SAVED_DATA_CLEAR(saved_data, ARM);
...
static void cmdq_set_lcmd_wqe(struct hinic_cmdq_wqe *wqe,
  ...
  cmdq_prepare_wqe_ctrl(wqe, wrapped, ack_type, mod, cmd, prod_idx,
                        complete_format, DATA_SGE, BUFDESC_LCMD_LEN);
...
static int hinic_set_cmdq_ctxts(struct hinic_hwdev *hwdev) {
  ...
  for (; cmdq_type < HINIC_MAX_CMDQ_TYPES; cmdq_type++) {
    ...
    err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM,
                                 HINIC_MGMT_CMD_CMDQ_CTXT_SET, cmdq_ctxt,
                                 in_size, &cmdq_ctxt_out, &out_size, 0);
...
static int hinic_cmdqs_init(struct hinic_hwdev *hwdev) {
  ...
  cmdqs->cmd_buf_pool = dma_pool_create(
      "hinic_cmdq", hwdev, HINIC_CMDQ_BUF_SIZE, HINIC_CMDQ_BUF_SIZE, 0ULL);
  ...
  err = hinic_cmdq_alloc(cmdqs->saved_wqs, hwdev, HINIC_MAX_CMDQ_TYPES,
                         HINIC_CMDQ_WQ_BUF_SIZE, CMDQ_WQEBB_SHIFT,
                         HINIC_CMDQ_DEPTH);
  if (err) {
...
static void cmdq_init_queue_ctxt(struct hinic_cmdq *cmdq,
  ...
  ctxt_info->curr_wqe_page_pfn =
      ...
      CMDQ_CTXT_PAGE_INFO_SET(1, CEQ_EN) | CMDQ_CTXT_PAGE_INFO_SET(0, CEQ_ARM) |
...
static int hinic_cmdq_poll_msg(struct hinic_cmdq *cmdq, u32 timeout) {
  ...
  if (cmd_info->cmd_type == HINIC_CMD_TYPE_NONE) {
    PMD_DRV_LOG(ERR,
                "Cmdq msg has not been filled and send to hw, or get TMO msg "
                "ack. cmdq ci: %u",
                ci);
    return -EINVAL;
  }
...
static int cmdq_sync_cmd_direct_resp(struct hinic_cmdq *cmdq,
  ...
  cmdq_set_lcmd_wqe(&wqe, SYNC_CMD_DIRECT_RESP, buf_in, NULL, wrapped, ack_type,
...
int hinic_cmdq_direct_resp(void *hwdev, enum hinic_ack_type ack_type,
  ...
  return cmdq_sync_cmd_direct_resp(&cmdqs->cmdq[HINIC_CMDQ_SYNC], ack_type, mod,
