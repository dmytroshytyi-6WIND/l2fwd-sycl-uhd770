...
#define WQE_PAGE_NUM(wq, idx)                                                  \
  (((idx) >> ((wq)->wqebbs_per_page_shift)) & ((wq)->num_q_pages - 1))

#define WQE_PAGE_OFF(wq, idx)                                                  \
  ((u64)((wq)->wqebb_size) * ((idx) & ((wq)->num_wqebbs_per_page - 1)))
...
#define CMDQ_PAGE_SIZE                                                         \
  ALIGN((CMDQ_BLOCKS_PER_PAGE * CMDQ_BLOCK_SIZE), PAGE_SIZE)
...
#define WQ_BASE_PADDR(wqs, wq)                                                 \
  (((wqs)->page_paddr[(wq)->page_idx]) + (u64)(wq)->block_idx * WQ_BLOCK_SIZE)
...
#define WQ_WQE_ADDR(wq, idx)                                                   \
  ((void *)((u64)((wq)->queue_buf_vaddr) + ((idx) << (wq)->wqebb_shift)))
