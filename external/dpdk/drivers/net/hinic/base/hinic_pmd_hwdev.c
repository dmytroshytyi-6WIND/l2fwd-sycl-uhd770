...
static const char *__hw_to_char_port_type[LINK_PORT_MAX_TYPE] = {
    "Unknown", "Fibre",      "Electric", "Direct Attach Copper",
    "AOC",     "Back plane", "BaseT"};
...
static void *hinic_dma_mem_zalloc(struct hinic_hwdev *hwdev, size_t size,
  ...
  snprintf(z_name, sizeof(z_name), "%s_%d", hwdev->pcidev_hdl->name, alloc_cnt);
  ...
  if (!mz) {
    PMD_DRV_LOG(
        ERR,
        "Alloc dma able memory failed, errno: %d, ma_name: %s, size: 0x%zx",
  ...
  sig = HINIC_HASH_FUNC(&iova, HINIC_HASH_KEY_LEN, HINIC_HASH_FUNC_INIT_VAL);
  rc = rte_hash_lookup_with_hash(hwdev->os_dep.dma_addr_hash, &iova, sig);
  if (rc >= 0) {
    PMD_DRV_LOG(ERR,
                "Dma addr: %p already in hash table, error: %d, mz_name: %s",
                (void *)iova, rc, z_name);
  ...
  rc = rte_hash_add_key_with_hash_data(hwdev->os_dep.dma_addr_hash, &iova, sig,
                                       (void *)(u64)mz);
...
static void hinic_dma_mem_free(struct hinic_hwdev *hwdev, size_t size,
  ...
  sig = HINIC_HASH_FUNC(&phys, HINIC_HASH_KEY_LEN, HINIC_HASH_FUNC_INIT_VAL);
  rc = rte_hash_lookup_with_hash_data(hash, &phys, sig, (void **)&mz);
  if (rc < 0) {
    PMD_DRV_LOG(ERR, "Can not find phys_addr: %p, error: %d", (void *)phys, rc);
    return;
  }

  if (virt != mz->addr || size > mz->len) {
    PMD_DRV_LOG(ERR,
                "Match mz_info failed: "
                "mz.name: %s, mz.phys: %p, mz.virt: %p, mz.len: %zu, "
                "phys: %p, virt: %p, size: %zu",
                mz->name, (void *)mz->iova, mz->addr, mz->len, (void *)phys,
                virt, size);
  }
...
void *dma_zalloc_coherent(void *hwdev, size_t size, dma_addr_t *dma_handle,
                          unsigned int socket_id) {
  return hinic_dma_mem_zalloc(hwdev, size, dma_handle, RTE_CACHE_LINE_SIZE,
                              socket_id);
}
...
void *dma_zalloc_coherent_aligned256k(void *hwdev, size_t size,
                                      dma_addr_t *dma_handle,
                                      unsigned int socket_id) {
  return hinic_dma_mem_zalloc(hwdev, size, dma_handle, HINIC_PAGE_SIZE * 64,
                              socket_id);
}
...
void dma_free_coherent_volatile(void *hwdev, size_t size, volatile void *virt,
                                dma_addr_t phys) {
  int rc;
  struct rte_memzone *mz = NULL;
...
void dma_pool_destroy(struct dma_pool *pool) {
  ...
  if (rte_atomic32_read(&pool->inuse) != 0) {
    PMD_DRV_LOG(ERR, "Leak memory, dma_pool: %s, inuse_count: %d", pool->name,
                rte_atomic32_read(&pool->inuse));
  }
...
int hinic_osdep_init(struct hinic_hwdev *hwdev) {
  ...
  if (paddr_hash == NULL) {
    ...
    PMD_DRV_LOG(INFO, "Using existing dma hash table %s", dh_params.name);
  }
...
void hinic_osdep_deinit(struct hinic_hwdev *hwdev) {
  ...
  if (paddr_hash) {
    ...
    while (rte_hash_iterate(paddr_hash, (const void **)&key_pa,
                            (void **)&data_mz, &iter) >= 0) {
      if (data_mz) {
        PMD_DRV_LOG(WARNING, "Free leaked dma_addr: %p, mz: %s", (void *)key_pa,
                    data_mz->name);
...
int hinic_set_ci_table(void *hwdev, u16 q_id, struct hinic_sq_attr *attr) {
  ...
  err = hinic_msg_to_mgmt_sync(
      hwdev, HINIC_MOD_COMM, HINIC_MGMT_CMD_L2NIC_SQ_CI_ATTR_SET,
      &cons_idx_attr, sizeof(cons_idx_attr), &cons_idx_attr, &out_size, 0);
...
int hinic_set_pagesize(void *hwdev, u8 page_size) {
  ...
  if (page_size > HINIC_PAGE_SIZE_MAX) {
    PMD_DRV_LOG(ERR, "Invalid page_size %u, bigger than %u", page_size,
                HINIC_PAGE_SIZE_MAX);
    return -EINVAL;
  }
  ...
  err = hinic_msg_to_mgmt_sync(
      hwdev, HINIC_MOD_COMM, HINIC_MGMT_CMD_PAGESIZE_SET, &page_size_info,
      sizeof(page_size_info), &page_size_info, &out_size, 0);
  if (err || !out_size || page_size_info.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Set wq page size failed, err: %d, status: 0x%x, out_size: 0x%0x",
...
static int hinic_vf_rx_tx_flush(struct hinic_hwdev *hwdev) {
  ...
  err = hinic_mbox_to_pf_no_ack(hwdev, HINIC_MOD_COMM, HINIC_MGMT_CMD_START_FLR,
...
static int hinic_pf_rx_tx_flush(struct hinic_hwdev *hwdev) {
  ...
  if (err || !out_size || clear_db.mgmt_msg_head.status) {
    PMD_DRV_LOG(WARNING,
                "Flush doorbell failed, err: %d, status: 0x%x, out_size: 0x%x",
  ...
  err =
      hinic_msg_to_mgmt_no_ack(hwdev, HINIC_MOD_COMM, HINIC_MGMT_CMD_START_FLR,
                               &clr_res, sizeof(clr_res));
  if (err) {
  ...
  err = hinic_reinit_cmdq_ctxts(hwdev);
  if (err) {
    PMD_DRV_LOG(WARNING, "Reinit cmdq failed when pf flush, err: %d", err);
    ret = err;
  }
...
static int hinic_get_interrupt_cfg(struct hinic_hwdev *hwdev,
  ...
  err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM,
                               HINIC_MGMT_CMD_MSI_CTRL_REG_RD_BY_UP, &msix_cfg,
                               sizeof(msix_cfg), &msix_cfg, &out_size, 0);
...
int hinic_set_interrupt_cfg(struct hinic_hwdev *hwdev,
  ...
  err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM,
                               HINIC_MGMT_CMD_MSI_CTRL_REG_WR_BY_UP, &msix_cfg,
                               sizeof(msix_cfg), &msix_cfg, &out_size, 0);
  if (err || !out_size || msix_cfg.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR,
...
static int set_vf_dma_attr_entry(struct hinic_hwdev *hwdev, u8 entry_idx, u8 st,
                                 u8 at, u8 ph,
                                 enum hinic_pcie_nosnoop no_snooping,
                                 enum hinic_pcie_tph tph_en) {
  ...
  err =
      hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM, HINIC_MGMT_CMD_DMA_ATTR_SET,
                             &attr, sizeof(attr), &attr, &out_size, 0);
  if (err || !out_size || attr.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Set dma attribute failed, err: %d, status: 0x%x, out_size: 0x%x",
...
static int dma_attr_table_init(struct hinic_hwdev *hwdev) {
  ...
  if (HINIC_IS_VF(hwdev))
    ...
    set_pf_dma_attr_entry(hwdev, PCIE_MSIX_ATTR_ENTRY, HINIC_PCIE_ST_DISABLE,
...
int hinic_init_attr_table(struct hinic_hwdev *hwdev) {
  ...
  if (err) {
    PMD_DRV_LOG(ERR, "Initialize dma attribute table failed, err: %d", err);
    return err;
  }

  err = init_aeqs_msix_attr(hwdev);
  if (err) {
    PMD_DRV_LOG(ERR, "Initialize aeqs msix attribute failed, err: %d", err);
    return err;
  }
...
static void fault_report_show(struct hinic_hwdev *hwdev,
  ...
  PMD_DRV_LOG(WARNING, "fault type:    %d [%s]", event->type, type_str);
  PMD_DRV_LOG(WARNING, "fault val[0]:  0x%08x", event->event.val[0]);
  PMD_DRV_LOG(WARNING, "fault val[1]:  0x%08x", event->event.val[1]);
  PMD_DRV_LOG(WARNING, "fault val[2]:  0x%08x", event->event.val[2]);
  PMD_DRV_LOG(WARNING, "fault val[3]:  0x%08x", event->event.val[3]);

  switch (event->type) {
  case FAULT_TYPE_CHIP:
    err_level = event->event.chip.err_level;
    if (err_level < FAULT_LEVEL_MAX)
      strncpy(level_str, fault_level[err_level], FAULT_SHOW_STR_LEN);
    else
      strncpy(level_str, "unknown", FAULT_SHOW_STR_LEN);

    PMD_DRV_LOG(WARNING, "err_level:     %d [%s]", err_level, level_str);

    if (err_level == FAULT_LEVEL_SERIOUS_FLR) {
      PMD_DRV_LOG(WARNING, "flr func_id:   %d", event->event.chip.func_id);
    } else {
      PMD_DRV_LOG(WARNING, "node_id:       %d", event->event.chip.node_id);
      PMD_DRV_LOG(WARNING, "err_type:      %d", event->event.chip.err_type);
      PMD_DRV_LOG(WARNING, "err_csr_addr:  %d", event->event.chip.err_csr_addr);
      PMD_DRV_LOG(WARNING, "err_csr_value: %d",
                  event->event.chip.err_csr_value);
    }
    break;
  case FAULT_TYPE_UCODE:
    PMD_DRV_LOG(WARNING, "cause_id:      %d", event->event.ucode.cause_id);
    PMD_DRV_LOG(WARNING, "core_id:       %d", event->event.ucode.core_id);
    PMD_DRV_LOG(WARNING, "c_id:          %d", event->event.ucode.c_id);
    PMD_DRV_LOG(WARNING, "epc:           %d", event->event.ucode.epc);
    break;
  ...
  case FAULT_TYPE_REG_WR_TIMEOUT:
    PMD_DRV_LOG(WARNING, "err_csr:       %d", event->event.reg_timeout.err_csr);
    break;
  default:
    break;
  }
...
static int resources_state_set(struct hinic_hwdev *hwdev,
  ...
  err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM,
                               HINIC_MGMT_CMD_RES_STATE_SET, &res_state,
                               sizeof(res_state), &res_state, &out_size, 0);
  if (err || !out_size || res_state.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR,
        "Set resources state failed, err: %d, status: 0x%x, out_size: 0x%x",
...
int hinic_get_board_info(void *hwdev, struct hinic_board_info *info) {
  ...
  err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM,
                               HINIC_MGMT_CMD_GET_BOARD_INFO, &board_info,
                               sizeof(board_info), &board_info, &out_size, 0);
  if (err || board_info.mgmt_msg_head.status || !out_size) {
    PMD_DRV_LOG(
        ERR, "Failed to get board info, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_l2nic_reset(struct hinic_hwdev *hwdev) {
  ...
  err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM,
                               ...
                               sizeof(l2nic_reset), &l2nic_reset, &out_size, 0);
  if (err || !out_size || l2nic_reset.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR,
        "Reset L2NIC resources failed, err: %d, status: 0x%x, out_size: 0x%x",
...
static void hinic_show_sw_watchdog_timeout_info(void *buf_in, u16 in_size,
  ...
  PMD_DRV_LOG(ERR,
              "Stack current used: 0x%x, peak used: 0x%x, overflow flag: 0x%x, "
              "top: 0x%x, bottom: 0x%x",
  ...
  for (i = 0; i < 3; i++) {
    ...
    PMD_DRV_LOG(ERR, "0x%08x 0x%08x 0x%08x 0x%08x", *(reg), *(reg + 1),
                *(reg + 2), *(reg + 3));
  }
  ...
  for (i = 0; i < (stack_len / 16); i++) {
    ...
    PMD_DRV_LOG(ERR, "0x%08x 0x%08x 0x%08x 0x%08x", *dump_addr,
                *(dump_addr + 1), *(dump_addr + 2), *(dump_addr + 3));
  }

  for (j = 0; j < ((stack_len % 16) / 4); j++) {
    dump_addr = (u32 *)(watchdog_info->data + ((u64)(u32)(i * 16 + j * 4)));
...
static void hinic_show_pcie_dfx_info(struct hinic_hwdev *hwdev, void *buf_in,
                                     u16 in_size, void *buf_out,
                                     u16 *out_size) {
  struct hinic_pcie_dfx_ntc *notice_info = (struct hinic_pcie_dfx_ntc *)buf_in;
  ...
  *out_size = sizeof(*notice_info);
  ...
  for (i = 0; i < num; i++) {
    ...
    err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM,
                                 HINIC_MGMT_CMD_PCIE_DFX_GET, &dfx_info,
                                 sizeof(dfx_info), &dfx_info, &size, 0);
    ...
    for (j = 0; j < 256; j = j + 8) {
      PMD_DRV_LOG(
          ERR,
          "0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x",
          cnt, reg[j], reg[(u32)(j + 1)], reg[(u32)(j + 2)], reg[(u32)(j + 3)],
          reg[(u32)(j + 4)], reg[(u32)(j + 5)], reg[(u32)(j + 6)],
          reg[(u32)(j + 7)]);
      cnt = cnt + 32;
    }
...
static void hinic_show_ffm_info(struct hinic_hwdev *hwdev, void *buf_in,
                                u16 in_size) {
  ...
  if (hwdev->ffm_num < FFM_RECORD_NUM_MAX) {
    ...
    PMD_DRV_LOG(WARNING,
                "node_id(%d),err_csr_addr(0x%x),err_csr_val(0x%x),err_level(0x%"
                "x),err_type(0x%x)",
                intr->node_id, intr->err_csr_addr, intr->err_csr_value,
                intr->err_level, intr->err_type);
  }
}
...
void hinic_comm_async_event_handle(struct hinic_hwdev *hwdev, u8 cmd,
  ...
  *out_size = 0;
  ...
  case HINIC_MGMT_CMD_FAULT_REPORT:
    ...
    if (hinic_func_type(hwdev) != TYPE_VF) {
      ret_fault_event = (struct hinic_cmd_fault_event *)buf_out;
  ...
  case HINIC_MGMT_CMD_WATCHDOG_INFO:
    hinic_show_sw_watchdog_timeout_info(buf_in, in_size, buf_out, out_size);
    break;

  case HINIC_MGMT_CMD_PCIE_DFX_NTC:
    hinic_show_pcie_dfx_info(hwdev, buf_in, in_size, buf_out, out_size);
    break;
...
static void hinic_cable_status_event(u8 cmd, void *buf_in,
                                     __rte_unused u16 in_size, void *buf_out,
                                     u16 *out_size) {
  ...
  if (cmd == HINIC_PORT_CMD_CABLE_PLUG_EVENT) {
    ...
    *out_size = sizeof(*plug_event);
    ...
    if (link_err->err_type >= LINK_ERR_NUM) {
      PMD_DRV_LOG(ERR, "Link failed, Unknown type: 0x%x", link_err->err_type);
    } else {
      PMD_DRV_LOG(INFO, "Link failed, type: 0x%x: %s", link_err->err_type,
...
static int hinic_link_event_process(struct hinic_hwdev *hwdev,
  ...
  uint32_t port_speed[LINK_SPEED_MAX] = {
  ...
  if (!status) {
    ...
    if (rc) {
      ...
      link.link_speed = port_speed[port_info.speed % LINK_SPEED_MAX];
...
static void hinic_lsc_process(struct hinic_hwdev *hwdev,
  ...
  if (ret == 0)
    rte_eth_dev_callback_process(rte_dev, RTE_ETH_EVENT_INTR_LSC, NULL);
}
...
void hinic_l2nic_async_event_handle(struct hinic_hwdev *hwdev, void *param,
  ...
  *out_size = 0;
  ...
  case HINIC_PORT_CMD_LINK_STATUS_REPORT:
    ...
    PMD_DRV_LOG(
        INFO,
        "Link status event report, dev_name: %s, port_id: %d, link_status: %s",
  ...
  case HINIC_PORT_CMD_LINK_ERR_EVENT:
    hinic_cable_status_event(cmd, buf_in, in_size, buf_out, out_size);
    break;
  ...
  default:
    PMD_DRV_LOG(ERR, "Unsupported event %d to process", cmd);
    break;
  }
}
...
static void print_cable_info(struct hinic_link_info *info) {
  ...
  if (info->port_type < LINK_PORT_MAX_TYPE)
    ...
    PMD_DRV_LOG(INFO, "Unknown port type: %u", info->port_type);
  ...
  snprintf(tmp_str, sizeof(tmp_str),
           "Vendor: %s, %s, %s, length: %um, max_speed: %uGbps", tmp_vendor,
           info->sfp_type ? "SFP" : "QSFP", port_type, info->cable_length,
           info->cable_max_speed);
  if (info->port_type != LINK_PORT_COPPER)
    snprintf(tmp_str + strlen(tmp_str), sizeof(tmp_str) - strlen(tmp_str),
...
static void print_hi30_status(struct hinic_link_info *info) {
  ...
  PMD_DRV_LOG(INFO,
              "TX_FFE: PRE2=%s%d; PRE1=%s%d; MAIN=%d; POST1=%s%d; POST1X=%s%d",
              (ffe_data->PRE1 & 0x10) ? "-" : "", (int)(ffe_data->PRE1 & 0xf),
              (ffe_data->PRE2 & 0x10) ? "-" : "", (int)(ffe_data->PRE2 & 0xf),
              (int)ffe_data->MAIN, (ffe_data->POST1 & 0x10) ? "-" : "",
              (int)(ffe_data->POST1 & 0xf), (ffe_data->POST2 & 0x10) ? "-" : "",
              (int)(ffe_data->POST2 & 0xf));
  PMD_DRV_LOG(INFO,
              "RX_CTLE: Gain1~3=%u %u %u; Boost1~3=%u %u %u; Zero1~3=%u %u %u; "
              "Squelch1~3=%u %u %u",
...
static void print_link_info(struct hinic_link_info *info,
  ...
  if (info->fec < HILINK_FEC_MAX_TYPE)
    ...
    PMD_DRV_LOG(INFO, "Unknown fec type: %u", info->fec);
...
static void hinic_print_hilink_info(void *buf_in, u16 in_size, void *buf_out,
                                    u16 *out_size) {
  ...
  *out_size = sizeof(*hilink_info);
  ...
  if (type < HILINK_EVENT_LINK_UP || type >= HILINK_EVENT_MAX_TYPE) {
    PMD_DRV_LOG(INFO, "Invalid hilink info report, type: %d", type);
    return;
  }
  ...
  if (type == HILINK_EVENT_CABLE_PLUGGED) {
    PMD_DRV_LOG(INFO, "alos: %u, rx_los: %u", info->alos, info->rx_los);
    return;
  }

  PMD_DRV_LOG(INFO,
              "PMA ctrl: %s, MAC tx %s, MAC rx %s, PMA debug inforeg: 0x%x, "
  ...
  PMD_DRV_LOG(INFO,
              "alos: %u, rx_los: %u, PCS block counter reg: 0x%x,PCS link: "
              "0x%x, MAC link: 0x%x PCS_err_cnt: 0x%x",
...
void hinic_hilink_async_event_handle(struct hinic_hwdev *hwdev, u8 cmd,
  ...
  *out_size = 0;
  ...
  case HINIC_HILINK_CMD_GET_LINK_INFO:
    hinic_print_hilink_info(buf_in, in_size, buf_out, out_size);
    break;
