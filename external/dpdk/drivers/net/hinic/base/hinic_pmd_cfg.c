...
static void hinic_parse_shared_res_cap(struct service_cap *cap,
  ...
  PMD_DRV_LOG(INFO,
              "host_pctxs: 0x%x, host_cctxs: 0x%x, host_scqs: 0x%x, host_srqs: "
              "0x%x, host_mpts: 0x%x",
...
static void hinic_parse_l2nic_res_cap(struct service_cap *cap,
  ...
  PMD_DRV_LOG(
      INFO, "max_sqs: 0x%x, max_rqs: 0x%x, vf_max_sqs: 0x%x, vf_max_rqs: 0x%x",
      nic_cap->max_sqs, nic_cap->max_rqs, nic_cap->vf_max_sqs,
      nic_cap->vf_max_rqs);
}
...
static void hinic_parse_pub_res_cap(struct service_cap *cap,
  ...
  PMD_DRV_LOG(INFO,
              "host_id: 0x%x, ep_id: 0x%x, intr_type: 0x%x, "
              "max_cos_id: 0x%x, cos_bitmap: 0x%x, er_id: 0x%x, port_id: 0x%x",
              cap->host_id, cap->ep_id, cap->intr_chip_en, cap->max_cos_id,
              cap->valid_cos_bitmap, cap->er_id, cap->port_id);
  PMD_DRV_LOG(
      INFO,
      "host_total_function: 0x%x, host_oq_id_mask_val: 0x%x, max_vf: 0x%x",
      cap->host_total_function, cap->host_oq_id_mask_val, cap->max_vf);
  PMD_DRV_LOG(INFO, "chip_svc_type: 0x%x", cap->chip_svc_type);
  PMD_DRV_LOG(
      INFO, "pf_num: 0x%x, pf_id_start: 0x%x, vf_num: 0x%x, vf_id_start: 0x%x",
...
static int get_cap_from_fw(struct hinic_hwdev *dev, enum func_type type) {
  ...
  err = hinic_msg_to_mgmt_sync(dev, HINIC_MOD_CFGM, HINIC_CFG_NIC_CAP, &dev_cap,
                               in_len, &dev_cap, &out_len, 0);
  if (err || dev_cap.mgmt_msg_head.status || !out_len) {
    PMD_DRV_LOG(
        ERR, "Get capability from FW failed, err: %d, status: %d, out_len: %d",
...
static int get_cap_from_pf(struct hinic_hwdev *dev, enum func_type type) {
  ...
  in_len = sizeof(dev_cap);
  out_len = in_len;
  err = hinic_mbox_to_pf(dev, HINIC_MOD_CFGM, HINIC_CFG_MBOX_CAP, &dev_cap,
                         in_len, &dev_cap, &out_len, CFG_MAX_CMD_TIMEOUT);
  if (err || dev_cap.mgmt_msg_head.status || !out_len) {
    PMD_DRV_LOG(
        ERR, "Get capability from PF failed, err: %d, status: %d, out_len: %d",
        err, dev_cap.mgmt_msg_head.status, out_len);
    return -EIO;
  }
...
static int get_dev_cap(struct hinic_hwdev *dev) {
  ...
  case TYPE_VF:
    ...
    if (err) {
      PMD_DRV_LOG(ERR, "Get VF capability failed, err: %d", err);
      return err;
    }
    break;
  default:
