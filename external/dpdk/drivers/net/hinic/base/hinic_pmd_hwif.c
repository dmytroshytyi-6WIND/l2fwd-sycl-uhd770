...
#define HINIC_MSIX_CNT_SET(val, member)                                        \
  (((val)&HINIC_MSIX_CNT_##member##_MASK) << HINIC_MSIX_CNT_##member##_SHIFT)
...
static void set_hwif_attr(struct hinic_hwif *hwif, u32 attr0, u32 attr1,
  ...
  hwif->attr.global_vf_id_of_pf = HINIC_AF2_GET(attr2, GLOBAL_VF_ID_OF_PF);
}
...
void hinic_set_msix_state(void *hwdev, u16 msix_idx,
  ...
  u32 offset =
      msix_idx * HINIC_PCI_MSIX_ENTRY_SIZE + HINIC_PCI_MSIX_ENTRY_VECTOR_CTRL;
  u32 mask_bits;
...
static int hinic_init_hwif(struct hinic_hwdev *hwdev, void *cfg_reg_base,
                           void *intr_reg_base, u64 db_base_phy, void *db_base,
                           __rte_unused void *dwqe_mapping) {
...
static void hinic_get_mmio(struct hinic_hwdev *hwdev, void **cfg_regs_base,
  ...
  *db_base = pci_dev->mem_resource[HINIC_DB_MEM_BAR].addr;
  ...
  if (pagesize == PAGE_SIZE_64K && (bar0_size % pagesize != 0)) {
    bar0_phy_addr = pci_dev->mem_resource[HINIC_CFG_REGS_BAR].phys_addr;
    if (bar0_phy_addr % pagesize != 0 && (bar0_size + bar2_size <= pagesize) &&
        bar2_size >= bar0_size) {
      *cfg_regs_base = (void *)((uint8_t *)(*intr_base) + bar2_size);
    }
  }
}
...
int hinic_hwif_res_init(struct hinic_hwdev *hwdev) {
  ...
  /* hinic related init */
  hwdev->hwif =
      rte_zmalloc("hinic_hwif", sizeof(*hwdev->hwif), RTE_CACHE_LINE_SIZE);
  if (!hwdev->hwif) {
  ...
  err = hinic_init_hwif(hwdev, cfg_regs_base, intr_base, 0, db_base, NULL);
  if (err) {
