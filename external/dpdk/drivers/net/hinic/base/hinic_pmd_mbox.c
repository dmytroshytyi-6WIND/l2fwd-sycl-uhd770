...
#define HINIC_MBOX_INT_SET(val, field)                                         \
  (((val)&HINIC_MBOX_INT_##field##_MASK) << HINIC_MBOX_INT_##field##_SHIFT)
...
#define HINIC_MBOX_CTRL_SET(val, field)                                        \
  (((val)&HINIC_MBOX_CTRL_##field##_MASK) << HINIC_MBOX_CTRL_##field##_SHIFT)
...
enum mbox_send_mod { MBOX_SEND_MSG_POLL = 1 };
...
static int recv_vf_mbox_handler(struct hinic_mbox_func_to_func *func_to_func,
  ...
  *out_size = 0;
  ...
  case HINIC_MOD_COMM:
    hinic_comm_async_event_handle(func_to_func->hwdev, recv_mbox->cmd,
                                  recv_mbox->mbox, recv_mbox->mbox_len, buf_out,
                                  out_size);
    break;
  case HINIC_MOD_L2NIC:
    hinic_l2nic_async_event_handle(func_to_func->hwdev, param, recv_mbox->cmd,
...
static void recv_func_mbox_handler(struct hinic_mbox_func_to_func *func_to_func,
  ...
  if (recv_mbox->ack_type == MBOX_ACK) {
    ...
    send_mbox_to_func(func_to_func, recv_mbox->mod, recv_mbox->cmd, buf_out,
                      out_size, src_func_idx, HINIC_HWIF_RESPONSE, MBOX_ACK,
                      &msg_info);
  }
}
...
static void mbox_copy_send_data(struct hinic_send_mbox *mbox, void *seg,
                                u16 seg_len) {
  u32 *data = (u32 *)seg;
  ...
  for (i = 0; i < idx_max; i++)
    __raw_writel(*(data + i), mbox->data + MBOX_HEADER_SZ + i * sizeof(u32));
}
...
static void write_mbox_msg_attr(struct hinic_mbox_func_to_func *func_to_func,
  ...
  mbox_int =
      ...
      HINIC_MBOX_INT_SET(ALIGN(MBOX_SIZE, MBOX_SEG_LEN_ALIGN) >> 2, TX_SIZE) |
...
static int alloc_mbox_info(struct hinic_recv_mbox *mbox_info) {
  ...
  for (func_idx = 0; func_idx < HINIC_MAX_FUNCTIONS; func_idx++) {
    ...
    if (err) {
      PMD_DRV_LOG(ERR,
                  "Initialize function[%d] mailbox information failed, err: %d",
                  func_idx, err);
...
static int alloc_mbox_wb_status(struct hinic_mbox_func_to_func *func_to_func) {
  ...
  send_mbox->wb_vaddr = dma_zalloc_coherent(
      hwdev, MBOX_WB_STATUS_LEN, &send_mbox->wb_paddr, SOCKET_ID_ANY);
...
static void free_mbox_wb_status(struct hinic_mbox_func_to_func *func_to_func) {
  ...
  dma_free_coherent(hwdev, MBOX_WB_STATUS_LEN, send_mbox->wb_vaddr,
                    send_mbox->wb_paddr);
}
...
static int recv_mbox_handler(struct hinic_mbox_func_to_func *func_to_func,
  ...
  if (!check_mbox_seq_id_and_seg_len(recv_mbox, seq_id, seg_len, msg_id)) {
    PMD_DRV_LOG(ERR,
  ...
  memcpy((u8 *)recv_mbox->mbox + seq_id * HINIC_MSG_SEG_LEN, mbox_body,
         seg_len);
  ...
  if (direction == HINIC_HWIF_RESPONSE) {
    ...
    PMD_DRV_LOG(ERR,
                "Mbox response timeout, current send msg id(0x%x), recv msg "
                "id(0x%x), status(0x%x)",
 ...
 */
int hinic_mbox_func_aeqe_handler(void *handle, u8 *header, __rte_unused u8 size,
                                 void *param) {
  ...
  if (src >= HINIC_MAX_FUNCTIONS) {
    PMD_DRV_LOG(ERR, "Mailbox source function id: %d is invalid", src);
    return HINIC_ERROR;
  }

  recv_mbox =
      (HINIC_MBOX_HEADER_GET(mbox_header, DIRECTION) == HINIC_HWIF_DIRECT_SEND)
...
static void dump_mox_reg(struct hinic_hwdev *hwdev) {
  ...
  val = hinic_hwif_read_reg(hwdev->hwif, HINIC_FUNC_CSR_MAILBOX_CONTROL_OFF);
  PMD_DRV_LOG(WARNING, "Mailbox control reg: 0x%x", val);
  val = hinic_hwif_read_reg(hwdev->hwif, HINIC_FUNC_CSR_MAILBOX_INT_OFFSET_OFF);
...
static int send_mbox_seg(struct hinic_mbox_func_to_func *func_to_func,
  ...
  write_mbox_msg_attr(func_to_func, dst_func, dst_aeqn, seg_ack_aeqn, seg_len,
                      MBOX_SEND_MSG_POLL);
...
static int send_mbox_to_func(struct hinic_mbox_func_to_func *func_to_func,
  ...
  header = HINIC_MBOX_HEADER_SET(msg_len, MSG_LEN) |
           ...
           HINIC_MBOX_HEADER_SET(hinic_global_func_id(hwdev), SRC_GLB_FUNC_IDX);
  ...
  while (!(HINIC_MBOX_HEADER_GET(header, LAST))) {
    ...
    err = send_mbox_seg(func_to_func, header, dst_func, msg_seg, seg_len);
    if (err) {
    ...
    seq_id++;
    header &= ~(HINIC_MBOX_HEADER_SET(HINIC_MBOX_HEADER_SEQID_MASK, SEQID));
...
static int hinic_mbox_to_func(struct hinic_mbox_func_to_func *func_to_func,
  ...
  struct hinic_recv_mbox *mbox_for_resp = &func_to_func->mbox_resp[dst_func];
  struct mbox_msg_info msg_info = {0};
  u32 time;
  int err;
  ...
  err = send_mbox_to_func(func_to_func, mod, cmd, buf_in, in_size, dst_func,
                          HINIC_HWIF_DIRECT_SEND, MBOX_ACK, &msg_info);
  if (err)
    goto send_err;
  ...
  if (mbox_for_resp->mbox_len && buf_out && out_size) {
    if (mbox_for_resp->mbox_len <= *out_size) {
      memcpy(buf_out, mbox_for_resp->mbox, mbox_for_resp->mbox_len);
...
mbox_func_params_valid(__rte_unused struct hinic_mbox_func_to_func *mbox_obj,
  ...
  if (in_size > HINIC_MBOX_DATA_SIZE) {
    PMD_DRV_LOG(ERR, "Mailbox message len(%d) exceed limit(%d)", in_size,
                HINIC_MBOX_DATA_SIZE);
    return -EINVAL;
  }
...
int hinic_mbox_to_pf(struct hinic_hwdev *hwdev, enum hinic_mod_type mod, u8 cmd,
  ...
  return hinic_mbox_to_func(func_to_func, mod, cmd, hinic_pf_id_of_vf(hwdev),
