...
const u32 hinic_hw_rx_buf_size[] = {
    HINIC_RX_BUF_SIZE_32B,  HINIC_RX_BUF_SIZE_64B,  HINIC_RX_BUF_SIZE_96B,
    HINIC_RX_BUF_SIZE_128B, HINIC_RX_BUF_SIZE_192B, HINIC_RX_BUF_SIZE_256B,
    HINIC_RX_BUF_SIZE_384B, HINIC_RX_BUF_SIZE_512B, HINIC_RX_BUF_SIZE_768B,
    HINIC_RX_BUF_SIZE_1K,   HINIC_RX_BUF_SIZE_1_5K, HINIC_RX_BUF_SIZE_2K,
    HINIC_RX_BUF_SIZE_3K,   HINIC_RX_BUF_SIZE_4K,   HINIC_RX_BUF_SIZE_8K,
...
hinic_qp_prepare_cmdq_header(struct hinic_qp_ctxt_header *qp_ctxt_hdr,
                             enum hinic_qp_ctxt_type ctxt_type, u16 num_queues,
                             u16 max_queues, u16 q_id) {
  qp_ctxt_hdr->queue_type = ctxt_type;
  qp_ctxt_hdr->num_queues = num_queues;

  if (ctxt_type == HINIC_QP_CTXT_TYPE_SQ)
    qp_ctxt_hdr->addr_offset = SQ_CTXT_OFFSET(max_queues, max_queues, q_id);
  else
    qp_ctxt_hdr->addr_offset = RQ_CTXT_OFFSET(max_queues, max_queues, q_id);
...
static void hinic_sq_prepare_ctxt(struct hinic_sq *sq, u16 global_qpn,
  ...
  sq_ctxt->ci_owner = SQ_CTXT_CI_SET(ci_start, IDX) | SQ_CTXT_CI_SET(1, OWNER);

  sq_ctxt->wq_pfn_hi = SQ_CTXT_WQ_PAGE_SET(wq_page_pfn_hi, HI_PFN) |
  ...
  sq_ctxt->pref_wq_pfn_hi_ci = SQ_CTXT_PREF_SET(ci_start, CI) |
...
static void hinic_rq_prepare_ctxt(struct hinic_rq *rq,
  ...
  rq_ctxt->ceq_attr =
      RQ_CTXT_CEQ_ATTR_SET(0, EN) | RQ_CTXT_CEQ_ATTR_SET(1, OWNER);
...
static int init_sq_ctxts(struct hinic_nic_io *nic_io) {
  ...
  while (q_id < nic_io->num_sqs) {
    ...
    err = hinic_cmdq_direct_resp(hwdev, HINIC_ACK_TYPE_CMDQ, HINIC_MOD_L2NIC,
                                 HINIC_UCODE_CMD_MDY_QUEUE_CONTEXT, cmd_buf,
                                 &out_param, 0);
    if (err || out_param != 0) {
...
static int clean_queue_offload_ctxt(struct hinic_nic_io *nic_io,
  ...
  err = hinic_cmdq_direct_resp(hwdev, HINIC_ACK_TYPE_CMDQ, HINIC_MOD_L2NIC,
                               HINIC_UCODE_CMD_CLEAN_QUEUE_CONTEXT, cmd_buf,
                               &out_param, 0);
...
static u16 get_hw_rx_buf_size(u32 rx_buf_sz) {
  u16 num_hw_types =
      sizeof(hinic_hw_rx_buf_size) / sizeof(hinic_hw_rx_buf_size[0]);
  u16 i;
 ...
 */
static int hinic_set_root_ctxt(void *hwdev, u16 rq_depth, u16 sq_depth,
                               int rx_buf_sz) {
  ...
  err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM, HINIC_MGMT_CMD_VAT_SET,
                               &root_ctxt, sizeof(root_ctxt), &root_ctxt,
                               &out_size, 0);
  if (err || !out_size || root_ctxt.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Set root context failed, err: %d, status: 0x%x, out_size: 0x%x",
...
static int hinic_clean_root_ctxt(void *hwdev) {
  ...
  if (err || !out_size || root_ctxt.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Clean root context failed, err: %d, status: 0x%x, out_size: 0x%x",
        err, root_ctxt.mgmt_msg_head.status, out_size);
    return -EIO;
  }
...
int hinic_init_qp_ctxts(struct hinic_hwdev *hwdev) {
  ...
  err =
      hinic_set_root_ctxt(hwdev, nic_io->rq_depth, nic_io->sq_depth, rx_buf_sz);
  if (err) {
  ...
  for (q_id = 0; q_id < nic_io->num_sqs; q_id++) {
    sq_attr.ci_dma_base = HINIC_CI_PADDR(nic_io->ci_dma_base, q_id) >> 2;
...
void hinic_return_sq_wqe(struct hinic_hwdev *hwdev, u16 q_id, int num_wqebbs,
                         u16 owner) {
  struct hinic_nic_io *nic_io = hwdev->nic_io;
...
void hinic_update_rq_local_ci(struct hinic_hwdev *hwdev, u16 q_id,
                              int wqe_cnt) {
  struct hinic_nic_io *nic_io = hwdev->nic_io;
  struct hinic_rq *rq = &nic_io->qps[q_id].rq;
...
static int hinic_alloc_nicio(struct hinic_hwdev *hwdev) {
  ...
  nic_io->qps = kzalloc_aligned(num_qp * sizeof(*nic_io->qps), GFP_KERNEL);
  if (!nic_io->qps) {
  ...
  nic_io->ci_vaddr_base =
      dma_zalloc_coherent(hwdev, CI_TABLE_SIZE(num_qp, HINIC_PAGE_SIZE),
                          &nic_io->ci_dma_base, pdev->device.numa_node);
  ...
  nic_io->sq_wq = kzalloc_aligned(num_qp * sizeof(*nic_io->sq_wq), GFP_KERNEL);
  if (!nic_io->sq_wq) {
  ...
  nic_io->rq_wq = kzalloc_aligned(num_qp * sizeof(*nic_io->rq_wq), GFP_KERNEL);
  if (!nic_io->rq_wq) {
...
static void hinic_free_nicio(struct hinic_hwdev *hwdev) {
  ...
  /* nic_io->ci_vaddr_base */
  dma_free_coherent(hwdev, CI_TABLE_SIZE(nic_io->max_qps, HINIC_PAGE_SIZE),
                    nic_io->ci_vaddr_base, nic_io->ci_dma_base);
...
int hinic_init_nicio(struct hinic_hwdev *hwdev) {
  ...
  hwdev->nic_io =
      rte_zmalloc("hinic_nicio", sizeof(*hwdev->nic_io), RTE_CACHE_LINE_SIZE);
  if (!hwdev->nic_io) {
...
int hinic_convert_rx_buf_size(u32 rx_buf_sz, u32 *match_sz) {
  ...
  num_hw_types = sizeof(hinic_hw_rx_buf_size) / sizeof(hinic_hw_rx_buf_size[0]);
