...
#define SYNC_MSG_ID_INC(pf_to_mgmt)                                            \
  (SYNC_MSG_ID(pf_to_mgmt) = (SYNC_MSG_ID(pf_to_mgmt) + 1) & SYNC_MSG_ID_MASK)
...
#define EQ_ELEM_DESC_GET(val, member)                                          \
  (((val) >> EQ_ELEM_DESC_##member##_SHIFT) & EQ_ELEM_DESC_##member##_MASK)
...
static u16 mgmt_msg_len(u16 msg_data_len) {
  ...
  u16 msg_size = (u16)(MGMT_MSG_RSVD_FOR_DEV + sizeof(u64) + msg_data_len);

  if (msg_size > MGMT_MSG_SIZE_MIN)
    msg_size = MGMT_MSG_SIZE_MIN +
               ALIGN((msg_size - MGMT_MSG_SIZE_MIN), MGMT_MSG_SIZE_STEP);
  else
    msg_size = MGMT_MSG_SIZE_MIN;
 ...
 * @msg_id: message id
 */
static void prepare_header(struct hinic_msg_pf_to_mgmt *pf_to_mgmt, u64 *header,
                           int msg_len, enum hinic_mod_type mod,
                           enum hinic_msg_ack_type ack_type,
                           enum hinic_msg_direction_type direction, u8 cmd,
                           u32 msg_id) {
...
static int send_msg_to_mgmt_async(struct hinic_msg_pf_to_mgmt *pf_to_mgmt,
                                  enum hinic_mod_type mod, u8 cmd, void *msg,
                                  u16 msg_len,
  ...
  if (direction == HINIC_MSG_RESPONSE)
    prepare_header(pf_to_mgmt, &header, msg_len, mod, HINIC_MSG_ACK, direction,
                   cmd, resp_msg_id);
  else
    prepare_header(pf_to_mgmt, &header, msg_len, mod, HINIC_MSG_ACK, direction,
...
static int send_msg_to_mgmt_sync(struct hinic_msg_pf_to_mgmt *pf_to_mgmt,
                                 enum hinic_mod_type mod, u8 cmd, void *msg,
                                 u16 msg_len, enum hinic_msg_ack_type ack_type,
                                 enum hinic_msg_direction_type direction,
  ...
  if (direction == HINIC_MSG_RESPONSE)
    prepare_header(pf_to_mgmt, &header, msg_len, mod, ack_type, direction, cmd,
                   resp_msg_id);
  else
    prepare_header(pf_to_mgmt, &header, msg_len, mod, ack_type, direction, cmd,
...
static int hinic_pf_to_mgmt_sync(struct hinic_hwdev *hwdev,
  ...
  err =
      send_msg_to_mgmt_sync(pf_to_mgmt, mod, cmd, buf_in, in_size,
                            HINIC_MSG_ACK, HINIC_MSG_DIRECT_SEND, MSG_NO_RESP);
  if (err) {
  ...
  for (i = 0; i < pf_to_mgmt->rx_aeq->poll_retry_nr; i++) {
    ...
    if (err) {
      ...
      PMD_DRV_LOG(ERR,
                  "AEQ[%d] poll(mod=%d, cmd=%d, msg_id=%u) an "
                  "unexpected(mod=%d, cmd=%d, msg_id=%u) response",
                  pf_to_mgmt->rx_aeq->q_id, mod, cmd, pf_to_mgmt->sync_msg_id,
                  recv_msg->mod, recv_msg->cmd, recv_msg->msg_id);
    }
  }

  if (i == pf_to_mgmt->rx_aeq->poll_retry_nr) {
    PMD_DRV_LOG(ERR,
                "Get %d unexpected mgmt rsp from AEQ[%d], poll mgmt rsp failed",
  ...
  if (recv_msg->msg_len && buf_out && out_size) {
    if (recv_msg->msg_len <= *out_size) {
      ...
      *out_size = recv_msg->msg_len;
      ...
      PMD_DRV_LOG(ERR, "Mgmt rsp's msg len: %u overflow.", recv_msg->msg_len);
      err = -ERANGE;
    }
  }
...
int hinic_msg_to_mgmt_sync(void *hwdev, enum hinic_mod_type mod, u8 cmd,
  ...
  if (hinic_func_type(hwdev) == TYPE_VF) {
    rc = hinic_mbox_to_pf(hwdev, mod, cmd, buf_in, in_size, buf_out, out_size,
                          timeout);
  } else {
    rc = hinic_pf_to_mgmt_sync(hwdev, mod, cmd, buf_in, in_size, buf_out,
                               out_size, timeout);
  }
...
static void hinic_mgmt_recv_msg_handler(struct hinic_msg_pf_to_mgmt *pf_to_mgmt,
  ...
  switch (recv_msg->mod) {
  case HINIC_MOD_COMM:
    hinic_comm_async_event_handle(pf_to_mgmt->hwdev, recv_msg->cmd,
                                  recv_msg->msg, recv_msg->msg_len, buf_out,
                                  &out_size);
    break;
  case HINIC_MOD_L2NIC:
    hinic_l2nic_async_event_handle(pf_to_mgmt->hwdev, param, recv_msg->cmd,
                                   recv_msg->msg, recv_msg->msg_len, buf_out,
                                   &out_size);
    break;
  case HINIC_MOD_HILINK:
    hinic_hilink_async_event_handle(pf_to_mgmt->hwdev, recv_msg->cmd,
                                    recv_msg->msg, recv_msg->msg_len, buf_out,
                                    &out_size);
    break;
  default:
  ...
  if (!recv_msg->async_mgmt_to_pf) {
    ...
    (void)send_msg_to_mgmt_async(pf_to_mgmt, recv_msg->mod, recv_msg->cmd,
                                 buf_out, out_size, HINIC_MSG_RESPONSE,
                                 recv_msg->msg_id);
  }
}
...
static int recv_mgmt_msg_handler(struct hinic_msg_pf_to_mgmt *pf_to_mgmt,
  ...
  if (!check_mgmt_seq_id_and_seg_len(recv_msg, seq_id, seq_len, msg_id)) {
    PMD_DRV_LOG(
        ...
        hinic_global_func_id(pf_to_mgmt->hwdev), front_id, seq_id, seq_len,
        recv_msg->msg_id, msg_id);
    return HINIC_ERROR;
  }
...
static int hinic_mgmt_msg_aeqe_handler(void *hwdev, u8 *header,
  ...
  recv_msg =
      (HINIC_MSG_HEADER_GET(*(u64 *)header, DIRECTION) == HINIC_MSG_DIRECT_SEND)
...
static int hinic_handle_aeqe(void *handle, enum hinic_aeq_type event, u8 *data,
                             u8 size, void *param) {
  int rc = 0;
  ...
  case HINIC_MBX_FROM_FUNC:
    ...
    break;
  default:
    PMD_DRV_LOG(ERR, "Unknown event type: 0x%x, size: %d", event, size);
    rc = HINIC_ERROR;
    break;
  }
...
int hinic_aeq_poll_msg(struct hinic_eq *eq, u32 timeout, void *param) {
  ...
  for (i = 0; ((timeout == 0) && (i < eq->eq_len)) ||
              ((timeout > 0) && (done != HINIC_OK) && (i < eq->eq_len));
       i++) {
    err = -EIO;
    ...
    if (EQ_ELEM_DESC_GET(aeqe_desc, SRC)) {
      ...
      size = EQ_ELEM_DESC_GET(aeqe_desc, SIZE);
          ...
          hinic_handle_aeqe(eq->hwdev, event, aeqe_pos->aeqe_data, size, param);
    }
