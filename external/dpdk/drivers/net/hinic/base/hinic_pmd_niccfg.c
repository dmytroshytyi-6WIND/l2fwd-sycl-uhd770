...
#define l2nic_msg_to_mgmt_sync(hwdev, cmd, buf_in, in_size, buf_out, out_size) \
  hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_L2NIC, cmd, buf_in, in_size,         \
                         buf_out, out_size, 0)
...
int hinic_init_function_table(void *hwdev, u16 rx_buf_sz) {
  ...
  err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_L2NIC, HINIC_PORT_CMD_INIT_FUNC,
                               &function_table, sizeof(function_table),
                               &function_table, &out_size, 0);
  if (err || function_table.mgmt_msg_head.status || !out_size) {
    PMD_DRV_LOG(
        ERR, "Failed to init func table, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_get_base_qpn(void *hwdev, u16 *global_qpn) {
  ...
  err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_L2NIC,
                               HINIC_PORT_CMD_GET_GLOBAL_QPN, &cmd_qpn,
                               sizeof(cmd_qpn), &cmd_qpn, &out_size, 0);
...
int hinic_get_default_mac(void *hwdev, u8 *mac_addr) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_MAC, &mac_info,
                               sizeof(mac_info), &mac_info, &out_size);
...
int hinic_update_mac(void *hwdev, u8 *old_mac, u8 *new_mac, u16 vlan_id,
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_UPDATE_MAC, &mac_info,
                               sizeof(mac_info), &mac_info, &out_size);
  if (err || !out_size ||
      (mac_info.mgmt_msg_head.status &&
       mac_info.mgmt_msg_head.status != HINIC_PF_SET_VF_ALREADY)) {
...
int hinic_set_port_mtu(void *hwdev, u32 new_mtu) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_CHANGE_MTU, &mtu_info,
                               sizeof(mtu_info), &mtu_info, &out_size);
...
int hinic_add_remove_vlan(void *hwdev, u16 vlan_id, u16 func_id, bool add) {
  ...
  if (err || !out_size || vlan_info.mgmt_msg_head.status) {
    PMD_DRV_LOG(ERR, "Failed to %s vlan, err: %d, status: 0x%x, out size: 0x%x",
                add ? "add" : "remove", err, vlan_info.mgmt_msg_head.status,
                out_size);
    return -EIO;
  }
...
int hinic_config_vlan_filter(void *hwdev, u32 vlan_filter_ctrl) {
  ...
  err = l2nic_msg_to_mgmt_sync(nic_hwdev, HINIC_PORT_CMD_SET_VLAN_FILTER,
                               &vlan_filter, sizeof(vlan_filter), &vlan_filter,
                               &out_size);
  if (vlan_filter.mgmt_msg_head.status == HINIC_MGMT_CMD_UNSUPPORTED) {
    err = HINIC_MGMT_CMD_UNSUPPORTED;
  } else if ((err == HINIC_MBOX_VF_CMD_ERROR) && (HINIC_IS_VF(nic_hwdev))) {
    err = HINIC_MGMT_CMD_UNSUPPORTED;
  } else if (err || !out_size || vlan_filter.mgmt_msg_head.status) {
    PMD_DRV_LOG(ERR,
                "Failed to config vlan filter, vlan_filter_ctrl: 0x%x, err: "
                "%d, status: 0x%x, out size: 0x%x",
                vlan_filter_ctrl, err, vlan_filter.mgmt_msg_head.status,
                out_size);
    err = -EIO;
  }
...
int hinic_set_rx_vlan_offload(void *hwdev, u8 en) {
  ...
  err =
      l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_RX_VLAN_OFFLOAD,
...
int hinic_get_link_status(void *hwdev, u8 *link_state) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_LINK_STATE, &get_link,
                               sizeof(get_link), &get_link, &out_size);
  if (err || !out_size || get_link.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Failed to get link state, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_set_vport_enable(void *hwdev, bool enable) {
  ...
  err =
      l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_VPORT_ENABLE, &en_state,
                             sizeof(en_state), &en_state, &out_size);
  if (err || !out_size || en_state.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Failed to set vport state, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_set_port_enable(void *hwdev, bool enable) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_PORT_ENABLE, &en_state,
                               sizeof(en_state), &en_state, &out_size);
  if (err || !out_size || en_state.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR,
        "Failed to set phy port state, err: %d, status: 0x%x, out size: 0x%x",
        err, en_state.mgmt_msg_head.status, out_size);
    return -EIO;
  }
...
int hinic_get_port_info(void *hwdev, struct nic_port_info *port_info) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_PORT_INFO, &port_msg,
                               sizeof(port_msg), &port_msg, &out_size);
  if (err || !out_size || port_msg.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Failed to get port info, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_set_pause_config(void *hwdev, struct nic_pause_config nic_pause) {
  ...
  err =
      l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_PAUSE_INFO, &pause_info,
                             sizeof(pause_info), &pause_info, &out_size);
  if (err || !out_size || pause_info.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Failed to set pause info, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_get_pause_info(void *hwdev, struct nic_pause_config *nic_pause) {
  ...
  err =
      l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_PAUSE_INFO, &pause_info,
                             sizeof(pause_info), &pause_info, &out_size);
  if (err || !out_size || pause_info.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR,
...
int hinic_dcb_set_ets(void *hwdev, u8 *up_tc, u8 *pg_bw, u8 *pgid, u8 *up_bw,
  ...
  for (i = 0; i < HINIC_DCB_TC_MAX; i++) {
    ...
    if (*(up_tc + i) > HINIC_DCB_TC_MAX) {
      PMD_DRV_LOG(ERR, "Invalid up %d mapping tc: %d", i, *(up_tc + i));
      return -EINVAL;
    }
  }

  if (pg_bw_t != 100 || (up_bw_t % 100) != 0) {
    PMD_DRV_LOG(ERR, "Invalid pg_bw: %d or up_bw: %d", pg_bw_t, up_bw_t);
    return -EINVAL;
  }
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_ETS, &ets, sizeof(ets),
                               &ets, &out_size);
  if (err || ets.mgmt_msg_head.status || !out_size) {
    PMD_DRV_LOG(ERR, "Failed to set ets, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_set_rss_type(void *hwdev, u32 tmpl_idx,
  ...
  err = hinic_cmdq_direct_resp(hwdev, HINIC_ACK_TYPE_CMDQ, HINIC_MOD_L2NIC,
                               HINIC_UCODE_CMD_SET_RSS_CONTEXT_TABLE, cmd_buf,
                               &out_param, 0);
...
int hinic_get_rss_type(void *hwdev, u32 tmpl_idx,
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_RSS_CTX_TBL, &ctx_tbl,
                               sizeof(ctx_tbl), &ctx_tbl, &out_size);
  if (err || !out_size || ctx_tbl.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Failed to get hash type, err: %d, status: 0x%x, out size: 0x%x",
  ...
  rss_type->tcp_ipv6_ext = HINIC_RSS_TYPE_GET(ctx_tbl.context, TCP_IPV6_EXT);
...
int hinic_rss_set_template_tbl(void *hwdev, u32 tmpl_idx, u8 *temp) {
  ...
  err =
      l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_RSS_TEMPLATE_TBL,
                             &temp_key, sizeof(temp_key), &temp_key, &out_size);
...
int hinic_rss_get_template_tbl(void *hwdev, u32 tmpl_idx, u8 *temp) {
  ...
  err =
      l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_RSS_TEMPLATE_TBL,
                             &temp_key, sizeof(temp_key), &temp_key, &out_size);
  if (err || !out_size || temp_key.mgmt_msg_head.status) {
    PMD_DRV_LOG(ERR,
                "Failed to get hash key, err: %d, status: 0x%x, out size: 0x%x",
                err, temp_key.mgmt_msg_head.status, out_size);
    return -EIO;
  }
...
int hinic_rss_set_hash_engine(void *hwdev, u8 tmpl_idx, u8 type) {
  ...
  if (err || !out_size || hash_type.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Failed to get hash engine, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_rss_set_indir_tbl(void *hwdev, u32 tmpl_idx, u32 *indir_table) {
  ...
  err = hinic_cmdq_direct_resp(hwdev, HINIC_ACK_TYPE_CMDQ, HINIC_MOD_L2NIC,
                               HINIC_UCODE_CMD_SET_RSS_INDIR_TABLE, cmd_buf,
                               &out_param, 0);
  if (err || out_param != 0) {
...
int hinic_rss_get_indir_tbl(void *hwdev, u32 tmpl_idx, u32 *indir_table) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_RSS_TEMPLATE_INDIR_TBL,
                               &rss_cfg, sizeof(rss_cfg), &rss_cfg, &out_size);
  if (err || !out_size || rss_cfg.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Failed to get indir table, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_rss_cfg(void *hwdev, u8 rss_en, u8 tmpl_idx, u8 tc_num, u8 *prio_tc) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_RSS_CFG, &rss_cfg,
...
int hinic_rss_template_alloc(void *hwdev, u8 *tmpl_idx) {
  ...
  err =
      l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_RSS_TEMP_MGR, &template_mgmt,
                             sizeof(template_mgmt), &template_mgmt, &out_size);
...
int hinic_set_rx_vhd_mode(void *hwdev, u16 vhd_mode, u16 rx_buf_sz) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_VHD_CFG, &vhd_mode_cfg,
                               sizeof(vhd_mode_cfg), &vhd_mode_cfg, &out_size);
...
int hinic_set_rx_mode(void *hwdev, u32 enable) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_RX_MODE, &rx_mode_cfg,
                               sizeof(rx_mode_cfg), &rx_mode_cfg, &out_size);
...
int hinic_get_mgmt_version(void *hwdev, char *fw) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_MGMT_VERSION, &fw_ver,
...
int hinic_set_rx_csum_offload(void *hwdev, u32 en) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_RX_CSUM, &rx_csum_cfg,
                               sizeof(rx_csum_cfg), &rx_csum_cfg, &out_size);
...
int hinic_set_rx_lro(void *hwdev, u8 ipv4_en, u8 ipv6_en, u8 max_wqe_num) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_LRO, &lro_cfg,
                               sizeof(lro_cfg), &lro_cfg, &out_size);
  if (err || !out_size || lro_cfg.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Failed to set lro offload, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_set_anti_attack(void *hwdev, bool enable) {
  ...
  if (err || !out_size || rate.mgmt_msg_head.status) {
    PMD_DRV_LOG(ERR,
                "Can't %s port Anti-Attack rate limit, err: %d, status: 0x%x, "
                "out size: 0x%x",
                (enable ? "enable" : "disable"), err, rate.mgmt_msg_head.status,
                out_size);
    return -EIO;
  }
...
int hinic_reset_port_link_cfg(void *hwdev) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_RESET_LINK_CFG, &reset_cfg,
                               sizeof(reset_cfg), &reset_cfg, &out_size);
  if (err || !out_size || reset_cfg.mgmt_msg_head.status) {
    PMD_DRV_LOG(ERR,
                "Reset port link configure failed, err: %d, status: 0x%x, out "
                "size: 0x%x",
...
int hinic_vf_func_init(struct hinic_hwdev *hwdev) {
  ...
  err = hinic_mbox_to_pf(hwdev, HINIC_MOD_L2NIC, HINIC_PORT_CMD_VF_REGISTER,
                         &state, sizeof(state), NULL, NULL, 0);
  if (err) {
...
void hinic_vf_func_free(struct hinic_hwdev *hwdev) {
  ...
  err = hinic_mbox_to_pf(hwdev, HINIC_MOD_L2NIC, HINIC_PORT_CMD_VF_UNREGISTER,
                         &err, sizeof(err), NULL, NULL, 0);
  if (err)
...
int hinic_set_fast_recycle_mode(void *hwdev, u8 mode) {
  ...
  err = hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_COMM,
                               ...
                               &fast_recycled_mode, sizeof(fast_recycled_mode),
...
int hinic_clear_vport_stats(struct hinic_hwdev *hwdev) {
  ...
  if (err || !out_size || clear_vport_stats.mgmt_msg_head.status) {
    PMD_DRV_LOG(ERR,
                "Failed to clear vport statistics, err: %d, status: 0x%x, out "
                "size: 0x%x",
...
int hinic_clear_phy_port_stats(struct hinic_hwdev *hwdev) {
  ...
  err = l2nic_msg_to_mgmt_sync(
      hwdev, HINIC_PORT_CMD_CLEAR_PORT_STATISTICS, &clear_phy_port_stats,
      sizeof(clear_phy_port_stats), &clear_phy_port_stats, &out_size);
  if (err || !out_size || clear_phy_port_stats.mgmt_msg_head.status) {
    PMD_DRV_LOG(ERR,
                "Failed to clear phy port statistics, err: %d, status: 0x%x, "
                "out size: 0x%x",
                err, clear_phy_port_stats.mgmt_msg_head.status, out_size);
    return -EIO;
  }
...
int hinic_set_link_status_follow(void *hwdev,
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_LINK_FOLLOW, &follow,
                               sizeof(follow), &follow, &out_size);
  if ((follow.mgmt_msg_head.status != HINIC_MGMT_CMD_UNSUPPORTED &&
       follow.mgmt_msg_head.status) ||
      err || !out_size) {
    PMD_DRV_LOG(ERR,
...
int hinic_get_link_mode(void *hwdev, u32 *supported, u32 *advertised) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_LINK_MODE, &link_mode,
                               sizeof(link_mode), &link_mode, &out_size);
  if (err || !out_size || link_mode.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Failed to get link mode, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_flush_qp_res(void *hwdev) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_CLEAR_QP_RES, &qp_res,
                               sizeof(qp_res), &qp_res, &out_size);
...
int hinic_vf_get_default_cos(struct hinic_hwdev *hwdev, u8 *cos_id) {
  ...
  err =
      hinic_msg_to_mgmt_sync(hwdev, HINIC_MOD_L2NIC, HINIC_PORT_CMD_GET_VF_COS,
                             &vf_cos, sizeof(vf_cos), &vf_cos, &out_size, 0);
  if (err || !out_size || vf_cos.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Get VF default cos failed, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_set_fdir_filter(void *hwdev, u8 filter_type, u8 qid, u8 type_enable,
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_Q_FILTER, &port_filer_cmd,
                               sizeof(port_filer_cmd), &port_filer_cmd,
                               &out_size);
  if (err || !out_size || port_filer_cmd.mgmt_msg_head.status) {
    PMD_DRV_LOG(ERR,
                "Set port Q filter failed, err: %d, status: 0x%x, out size: "
                "0x%x, type: 0x%x,"
                " enable: 0x%x, qid: 0x%x, filter_type_enable: 0x%x\n",
                err, port_filer_cmd.mgmt_msg_head.status, out_size, filter_type,
                enable, qid, type_enable);
    return -EIO;
  }
 ...
 *   negative error value otherwise.
 */
int hinic_set_normal_filter(void *hwdev, u8 qid, u8 normal_type_enable, u32 key,
                            bool enable, u8 flag) {
  struct hinic_port_qfilter_info port_filer_cmd;
  u16 out_size = sizeof(port_filer_cmd);
  int err;
  ...
  if (err || !out_size || port_filer_cmd.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR,
        "Set normal filter failed, err: %d, status: 0x%x, out size: 0x%x, "
        "fdir_flag: 0x%x,"
        " enable: 0x%x, qid: 0x%x, normal_type_enable: 0x%x, key:0x%x\n",
        err, port_filer_cmd.mgmt_msg_head.status, out_size, flag, enable, qid,
        normal_type_enable, key);
    return -EIO;
  }
...
int hinic_set_fdir_tcam(void *hwdev, u16 type_mask,
  ...
  memcpy((void *)&port_tcam_cmd.filter_rule, (void *)filter_rule,
         sizeof(struct tag_pa_rule));
  memcpy((void *)&port_tcam_cmd.filter_action, (void *)filter_action,
         sizeof(struct tag_pa_action));

  err =
      l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_TCAM_FILTER, &port_tcam_cmd,
                             sizeof(port_tcam_cmd), &port_tcam_cmd, &out_size);
  if (err || !out_size || port_tcam_cmd.mgmt_msg_head.status) {
    PMD_DRV_LOG(ERR,
                "Set tcam table failed, err: %d, status: 0x%x, out size: 0x%x",
...
int hinic_clear_fdir_tcam(void *hwdev, u16 type_mask) {
  ...
  if (err || !out_size || port_tcam_cmd.mgmt_msg_head.status) {
    PMD_DRV_LOG(
        ERR, "Clear tcam table failed, err: %d, status: 0x%x, out size: 0x%x",
        err, port_tcam_cmd.mgmt_msg_head.status, out_size);
    return -EIO;
  }
...
int hinic_add_tcam_rule(void *hwdev, struct tag_tcam_cfg_rule *tcam_rule) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_UP_TC_ADD_FLOW, &tcam_cmd,
                               sizeof(tcam_cmd), &tcam_cmd, &out_size);
...
int hinic_del_tcam_rule(void *hwdev, u32 index) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_UP_TC_DEL_FLOW, &tcam_cmd,
                               sizeof(tcam_cmd), &tcam_cmd, &out_size);
  if (err || tcam_cmd.mgmt_msg_head.status || !out_size) {
    PMD_DRV_LOG(ERR,
...
static int hinic_mgmt_tcam_block(void *hwdev, u8 alloc_en, u8 block_type,
                                 u16 *index) {
  ...
  err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_UP_TC_CTRL_TCAM_BLOCK,
                               &tcam_block_info, sizeof(tcam_block_info),
                               &tcam_block_info, &out_size);
  if (tcam_block_info.mgmt_msg_head.status == HINIC_MGMT_CMD_UNSUPPORTED) {
    err = HINIC_MGMT_CMD_UNSUPPORTED;
...
int hinic_alloc_tcam_block(void *hwdev, u8 block_type, u16 *index) {
  return hinic_mgmt_tcam_block(hwdev, HINIC_TCAM_BLOCK_ENABLE, block_type,
                               index);
}

int hinic_free_tcam_block(void *hwdev, u8 block_type, u16 *index) {
  return hinic_mgmt_tcam_block(hwdev, HINIC_TCAM_BLOCK_DISABLE, block_type,
                               index);
}
...
int hinic_flush_tcam_rule(void *hwdev) {
  ...
  err = l2nic_msg_to_mgmt_sync(
      hwdev, HINIC_PORT_CMD_UP_TC_FLUSH_TCAM, &tcam_flush,
      sizeof(struct hinic_cmd_flush_tcam_rules), &tcam_flush, &out_size);
...
int hinic_set_fdir_tcam_rule_filter(void *hwdev, bool enable) {
  ...
  err =
      l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_UP_TC_ENABLE, &port_tcam_cmd,
                             sizeof(port_tcam_cmd), &port_tcam_cmd, &out_size);
  if ((port_tcam_cmd.mgmt_msg_head.status != HINIC_MGMT_CMD_UNSUPPORTED &&
       port_tcam_cmd.mgmt_msg_head.status) ||
      err || !out_size) {
    ...
    PMD_DRV_LOG(ERR,
                ...
                err, port_tcam_cmd.mgmt_msg_head.status, out_size, enable);
    return -EIO;
  }
