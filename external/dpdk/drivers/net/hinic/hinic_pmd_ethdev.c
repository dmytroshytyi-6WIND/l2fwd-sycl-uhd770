...
#define HINIC_VPORT_XSTATS_NUM                                                 \
  (sizeof(hinic_vport_stats_strings) / sizeof(hinic_vport_stats_strings[0]))
...
#define HINIC_PHYPORT_XSTATS_NUM                                               \
  (sizeof(hinic_phyport_stats_strings) / sizeof(hinic_phyport_stats_strings[0]))
...
#define HINIC_RXQ_XSTATS_NUM                                                   \
  (sizeof(hinic_rxq_stats_strings) / sizeof(hinic_rxq_stats_strings[0]))
...
#define HINIC_TXQ_XSTATS_NUM                                                   \
  (sizeof(hinic_txq_stats_strings) / sizeof(hinic_txq_stats_strings[0]))

static int hinic_xstats_calc_num(struct hinic_nic_dev *nic_dev) {
  if (HINIC_IS_VF(nic_dev->hwdev)) {
    return (HINIC_VPORT_XSTATS_NUM + HINIC_RXQ_XSTATS_NUM * nic_dev->num_rq +
            HINIC_TXQ_XSTATS_NUM * nic_dev->num_sq);
  } else {
    return (HINIC_VPORT_XSTATS_NUM + HINIC_PHYPORT_XSTATS_NUM +
...
static void hinic_dev_interrupt_handler(void *param) {
  ...
  if (!rte_bit_relaxed_get32(HINIC_DEV_INTR_EN, &nic_dev->dev_status)) {
    PMD_DRV_LOG(WARNING,
                "Device's interrupt is disabled, ignore interrupt event, "
...
static int hinic_dev_configure(struct rte_eth_dev *dev) {
  ...
  nic_dev->num_qps =
      (nic_dev->num_sq > nic_dev->num_rq) ? nic_dev->num_sq : nic_dev->num_rq;
  ...
  /* init VLAN offload */
  err = hinic_vlan_offload_set(dev, RTE_ETH_VLAN_STRIP_MASK |
 ...
 */
static int
hinic_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  rq_depth =
      (nb_desc & (nb_desc - 1)) ? ((u16)(1U << (ilog2(nb_desc) + 1))) : nb_desc;

  /*
   * Validate number of receive descriptors.
   * It must not exceed hardware maximum and minimum.
   */
  if (rq_depth > HINIC_MAX_QUEUE_DEPTH || rq_depth < HINIC_MIN_QUEUE_DEPTH) {
    PMD_DRV_LOG(ERR,
                "RX queue depth is out of range from %d to %d, (nb_desc=%d, "
                "q_depth=%d, port=%d queue=%d)",
                HINIC_MIN_QUEUE_DEPTH, HINIC_MAX_QUEUE_DEPTH, (int)nb_desc,
                (int)rq_depth, (int)dev->data->port_id, (int)queue_idx);
    return -EINVAL;
  }
   ...
   */
  rx_free_thresh =
      (u16)((rx_conf->rx_free_thresh) ? rx_conf->rx_free_thresh
                                      : HINIC_DEFAULT_RX_FREE_THRESH);
  if (rx_free_thresh >= (rq_depth - 1)) {
    PMD_DRV_LOG(ERR,
                "rx_free_thresh must be less than the number of RX descriptors "
                "minus 1. (rx_free_thresh=%u port=%d queue=%d)",
  ...
  if (!rxq) {
    PMD_DRV_LOG(ERR, "Allocate rxq[%d] failed, dev_name: %s", queue_idx,
                dev->data->name);
    return -ENOMEM;
  }
  ...
  rc = hinic_convert_rx_buf_size(rte_pktmbuf_data_room_size(rxq->mb_pool) -
                                 ...
                                 &buf_size);
  if (rc) {
    PMD_DRV_LOG(ERR, "Adjust buf size failed, dev_name: %s", dev->data->name);
 ...
 */
static int
hinic_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                     uint16_t nb_desc, unsigned int socket_id,
   ...
   * It must not exceed hardware maximum and minimum.
   */
  if (sq_depth > HINIC_MAX_QUEUE_DEPTH || sq_depth < HINIC_MIN_QUEUE_DEPTH) {
    PMD_DRV_LOG(ERR,
                "TX queue depth is out of range from %d to %d, (nb_desc=%d, "
                "q_depth=%d, port=%d queue=%d)",
                HINIC_MIN_QUEUE_DEPTH, HINIC_MAX_QUEUE_DEPTH, (int)nb_desc,
                (int)sq_depth, (int)dev->data->port_id, (int)queue_idx);
    return -EINVAL;
  }
   ...
   */
  tx_free_thresh =
      (u16)((tx_conf->tx_free_thresh) ? tx_conf->tx_free_thresh
                                      : HINIC_DEFAULT_TX_FREE_THRESH);
  if (tx_free_thresh >= (sq_depth - 1)) {
    PMD_DRV_LOG(ERR,
                "tx_free_thresh must be less than the number of TX descriptors "
                "minus 1. (tx_free_thresh=%u port=%d queue=%d)",
  ...
  txq = rte_zmalloc_socket("hinic_tx_queue", sizeof(struct hinic_txq),
                           RTE_CACHE_LINE_SIZE, socket_id);
  if (!txq) {
    PMD_DRV_LOG(ERR, "Allocate txq[%d] failed, dev_name: %s", queue_idx,
                dev->data->name);
    return -ENOMEM;
  }
  ...
  txq->sq_bot_sge_addr =
      HINIC_GET_WQ_TAIL(txq) - sizeof(struct hinic_sq_bufdesc);
 ...
 */
static void hinic_get_speed_capa(struct rte_eth_dev *dev,
                                 uint32_t *speed_capa) {
  struct hinic_nic_dev *nic_dev = HINIC_ETH_DEV_TO_PRIVATE_NIC_DEV(dev);
...
#define HINIC_LINK_MODE_SUPPORT_25G                                            \
  ((1U << HINIC_25GE_BASE_KR_S) | (1U << HINIC_25GE_BASE_CR_S) |               \
   (1U << HINIC_25GE_BASE_KR) | (1U << HINIC_25GE_BASE_CR))

#define HINIC_LINK_MODE_SUPPORT_40G                                            \
  ((1U << HINIC_40GE_BASE_KR4) | (1U << HINIC_40GE_BASE_CR4))

#define HINIC_LINK_MODE_SUPPORT_100G                                           \
  ((1U << HINIC_100GE_BASE_KR4) | (1U << HINIC_100GE_BASE_CR4))

  err = hinic_get_link_mode(nic_dev->hwdev, &supported_link, &advertised_link);
  if (err || supported_link == HINIC_SUPPORTED_UNKNOWN ||
      advertised_link == HINIC_SUPPORTED_UNKNOWN) {
    PMD_DRV_LOG(WARNING,
                "Get speed capability info failed, device: %s, port_id: %u",
                nic_dev->proc_dev_name, dev->data->port_id);
  } else {
    *speed_capa = 0;
 ...
 */
static int hinic_dev_infos_get(struct rte_eth_dev *dev,
...
static int hinic_config_rx_mode(struct hinic_nic_dev *nic_dev,
                                u32 rx_mode_ctrl) {
  int err;
...
static int hinic_rxtx_configure(struct rte_eth_dev *dev) {
  ...
  if (err) {
    PMD_DRV_LOG(ERR, "Configure rx_mode:0x%x failed", HINIC_DEFAULT_RX_MODE);
...
static int hinic_dev_start(struct rte_eth_dev *dev) {
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Initialize qp context failed, dev_name: %s", name);
    goto init_qp_fail;
  }
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Configure mq mode failed, dev_name: %s", name);
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Set mtu_size[%d] failed, dev_name: %s", nic_dev->mtu_size,
                name);
    goto set_mtu_fail;
  }
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Configure tx and rx failed, dev_name: %s", name);
    goto cfg_rxtx_fail;
  }
  ...
  if (rc) {
    PMD_DRV_LOG(ERR, "Enable physical port failed, dev_name: %s", name);
    goto en_port_fail;
  }
...
static int hinic_set_dev_promiscuous(struct hinic_nic_dev *nic_dev,
                                     bool enable) {
  u32 rx_mode_ctrl;
  int err;
 ...
 */
static int hinic_dev_stats_get(struct rte_eth_dev *dev,
  ...
  stats->ipackets =
      (vport_stats.rx_unicast_pkts_vport + vport_stats.rx_multicast_pkts_vport +
       vport_stats.rx_broadcast_pkts_vport - rx_discards_pmd);

  stats->opackets =
      (vport_stats.tx_unicast_pkts_vport + vport_stats.tx_multicast_pkts_vport +
...
static int hinic_init_mac_addr(struct rte_eth_dev *eth_dev) {
  ...
  rc = hinic_set_mac(nic_dev->hwdev, eth_dev->data->mac_addrs[0].addr_bytes, 0,
                     func_id);
  if (rc && rc != HINIC_PF_SET_VF_ALREADY)
    return rc;

  rte_ether_addr_copy(&eth_dev->data->mac_addrs[0], &nic_dev->default_addr);
...
static void hinic_delete_mc_addr_list(struct hinic_nic_dev *nic_dev) {
  ...
  for (i = 0; i < HINIC_MAX_MC_MAC_ADDRS; i++) {
    ...
    hinic_del_mac(nic_dev->hwdev, nic_dev->mc_list[i].addr_bytes, 0, func_id);
...
static void hinic_deinit_mac_addr(struct rte_eth_dev *eth_dev) {
  ...
  for (i = 0; i < HINIC_MAX_UC_MAC_ADDRS; i++) {
    ...
    rc = hinic_del_mac(nic_dev->hwdev, eth_dev->data->mac_addrs[i].addr_bytes,
                       0, func_id);
    if (rc && rc != HINIC_PF_SET_VF_ALREADY)
      PMD_DRV_LOG(ERR, "Delete mac table failed, dev_name: %s",
                  eth_dev->data->name);

    memset(&eth_dev->data->mac_addrs[i], 0, sizeof(struct rte_ether_addr));
  }
...
static void hinic_store_vlan_filter(struct hinic_nic_dev *nic_dev, u16 vlan_id,
                                    bool on) {
  u32 vid_idx, vid_bit;
 ...
 */
static int hinic_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                 int enable) {
  struct hinic_nic_dev *nic_dev = HINIC_ETH_DEV_TO_PRIVATE_NIC_DEV(dev);
  int err = 0;
  u16 func_id;
  ...
  if (enable) {
    ...
    if (hinic_find_vlan_filter(nic_dev, vlan_id)) {
      PMD_DRV_LOG(INFO, "Vlan %u has been added, device: %s", vlan_id,
                  nic_dev->proc_dev_name);
      return 0;
    }

    err = hinic_add_remove_vlan(nic_dev->hwdev, vlan_id, func_id, TRUE);
  } else {
    ...
    err = hinic_add_remove_vlan(nic_dev->hwdev, vlan_id, func_id, FALSE);
  }
  ...
  PMD_DRV_LOG(INFO, "%s vlan %u succeed, device: %s", enable ? "Add" : "Remove",
              vlan_id, nic_dev->proc_dev_name);
  return 0;
}
...
static int hinic_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  ...
  if (mask & RTE_ETH_VLAN_FILTER_MASK) {
    ...
    if (err == HINIC_MGMT_CMD_UNSUPPORTED) {
      PMD_DRV_LOG(WARNING,
                  "Current matching version does not support vlan filter "
                  "configuration, device: %s, port_id: %d",
                  nic_dev->proc_dev_name, dev->data->port_id);
    } else if (err) {
      PMD_DRV_LOG(ERR,
                  "Failed to %s vlan filter, device: %s, port_id: %d, err: %d",
                  on ? "enable" : "disable", nic_dev->proc_dev_name,
  ...
  if (mask & RTE_ETH_VLAN_STRIP_MASK) {
    ...
    if (err) {
      PMD_DRV_LOG(ERR,
                  "Failed to %s vlan strip, device: %s, port_id: %d, err: %d",
                  on ? "enable" : "disable", nic_dev->proc_dev_name,
                  dev->data->port_id, err);
      return err;
    }
...
static int hinic_dev_promiscuous_disable(struct rte_eth_dev *dev) {
  ...
  PMD_DRV_LOG(
      INFO, "Disable promiscuous, nic_dev: %s, port_id: %d, promisc: %d",
      nic_dev->proc_dev_name, dev->data->port_id, dev->data->promiscuous);
...
static int hinic_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_key_len > HINIC_RSS_KEY_SIZE) {
    PMD_DRV_LOG(ERR, "Invalid rss key, rss_key_len: %d", rss_conf->rss_key_len);
    return HINIC_ERROR;
  }

  if (rss_conf->rss_key) {
    memcpy(hashkey, rss_conf->rss_key, rss_conf->rss_key_len);
    err = hinic_rss_set_template_tbl(nic_dev->hwdev, tmpl_idx, hashkey);
    if (err) {
...
static int hinic_rss_conf_get(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_key && rss_conf->rss_key_len >= HINIC_RSS_KEY_SIZE) {
  ...
  rss_conf->rss_hf = 0;
  rss_conf->rss_hf |=
      rss_type.ipv4 ? (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_FRAG_IPV4) : 0;
  rss_conf->rss_hf |= rss_type.tcp_ipv4 ? RTE_ETH_RSS_NONFRAG_IPV4_TCP : 0;
  rss_conf->rss_hf |=
      rss_type.ipv6 ? (RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_FRAG_IPV6) : 0;
...
static int hinic_rss_indirtbl_update(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size; i++) {
    ...
    if (reta_conf[idx].reta[shift] >= nic_dev->num_rq) {
      PMD_DRV_LOG(ERR,
                  "Invalid reta entry, indirtbl[%d]: %d "
                  "exceeds the maximum rxq num: %d",
                  i, reta_conf[idx].reta[shift], nic_dev->num_rq);
      return -EINVAL;
    }
...
static int hinic_rss_indirtbl_query(struct rte_eth_dev *dev,
  ...
  err = hinic_rss_get_indir_tbl(nic_dev->hwdev, tmpl_idx, indirtbl);
  if (err) {
    PMD_DRV_LOG(ERR, "Get rss indirect table failed, error: %d", err);
    return err;
  }
...
static int hinic_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < HINIC_VPORT_XSTATS_NUM; i++) {
    xstats[count].value = *(uint64_t *)(((char *)&vport_stats) +
...
static int hinic_dev_xstats_get_names(struct rte_eth_dev *dev,
  ...
  for (q_num = 0; q_num < nic_dev->num_rq; q_num++) {
    for (i = 0; i < HINIC_RXQ_XSTATS_NUM; i++) {
      snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
               "rxq%d_%s_pmd", q_num, hinic_rxq_stats_strings[i].name);
      count++;
    }
  }
  ...
  for (q_num = 0; q_num < nic_dev->num_sq; q_num++) {
    for (i = 0; i < HINIC_TXQ_XSTATS_NUM; i++) {
      snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
               "txq%d_%s_pmd", q_num, hinic_txq_stats_strings[i].name);
      count++;
    }
  }
...
static void hinic_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
  ...
  if (index >= HINIC_MAX_UC_MAC_ADDRS) {
    PMD_DRV_LOG(INFO, "Remove mac index(%u) is out of range", index);
    return;
  }

  func_id = hinic_global_func_id(nic_dev->hwdev);
  ret = hinic_del_mac(nic_dev->hwdev, dev->data->mac_addrs[index].addr_bytes, 0,
                      func_id);
  if (ret)
    return;
...
static int hinic_set_mc_addr_list(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < nb_mc_addr; i++) {
    ret = hinic_set_mac(nic_dev->hwdev, mc_addr_set[i].addr_bytes, 0, func_id);
...
static int hinic_set_default_dcb_feature(struct hinic_nic_dev *nic_dev) {
  ...
  return hinic_dcb_set_ets(nic_dev->hwdev, up_tc, pg_bw, up_pgid, up_bw,
                           up_strict);
}
...
static int hinic_set_default_hw_feature(struct hinic_nic_dev *nic_dev) {
  ...
  err = hinic_set_link_status_follow(nic_dev->hwdev, HINIC_LINK_FOLLOW_PORT);
  if (err == HINIC_MGMT_CMD_UNSUPPORTED)
    PMD_DRV_LOG(WARNING,
                "Don't support to set link status follow phy port status");
  else if (err)
    return err;
...
static int32_t hinic_card_workmode_check(struct hinic_nic_dev *nic_dev) {
  ...
  return (info.service_mode == HINIC_SERVICE_MODE_NIC ? HINIC_OK : HINIC_ERROR);
}
...
static int hinic_copy_mempool_init(struct hinic_nic_dev *nic_dev) {
  ...
  if (nic_dev->cpy_mpool == NULL) {
    nic_dev->cpy_mpool = rte_pktmbuf_pool_create(
        nic_dev->proc_dev_name, HINIC_COPY_MEMPOOL_DEPTH, 0, 0,
        HINIC_COPY_MBUF_SIZE, rte_socket_id());
...
static int hinic_nic_dev_create(struct rte_eth_dev *eth_dev) {
  ...
  nic_dev->hwdev =
      rte_zmalloc("hinic_hwdev", sizeof(*nic_dev->hwdev), RTE_CACHE_LINE_SIZE);
  if (!nic_dev->hwdev) {
  ...
  if (rc) {
    PMD_DRV_LOG(ERR,
                "Initialize hardware default features failed, dev_name: %s",
                eth_dev->data->name);
...
static void hinic_nic_dev_destroy(struct rte_eth_dev *eth_dev) {
  ...
  (void)hinic_set_link_status_follow(nic_dev->hwdev, HINIC_LINK_FOLLOW_DEFAULT);
  hinic_copy_mempool_uninit(nic_dev);
  hinic_deinit_sw_rxtxqs(nic_dev);
  hinic_deinit_nicio(nic_dev->hwdev);
  hinic_deactivate_hwdev_state(nic_dev->hwdev);
...
static int hinic_dev_close(struct rte_eth_dev *dev) {
  ...
  if (rte_bit_relaxed_test_and_set32(HINIC_DEV_CLOSE, &nic_dev->dev_status)) {
    PMD_DRV_LOG(WARNING, "Device %s already closed", dev->data->name);
    return 0;
  }
...
static int hinic_func_init(struct rte_eth_dev *eth_dev) {
  ...
  snprintf(nic_dev->proc_dev_name, sizeof(nic_dev->proc_dev_name),
           "hinic-%.4x:%.2x:%.2x.%x", pci_dev->addr.domain, pci_dev->addr.bus,
...
enable_intr_fail:
  (void)rte_intr_callback_unregister(
...
eth_addr_fail:
  PMD_DRV_LOG(ERR, "Initialize %s in primary failed", eth_dev->data->name);
  return rc;
}
...
static int hinic_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                           struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct hinic_nic_dev),
                                       hinic_dev_init);
}
