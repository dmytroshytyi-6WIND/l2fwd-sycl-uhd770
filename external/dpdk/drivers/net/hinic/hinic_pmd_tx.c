...
#define SQ_TASK_INFO0_SET(val, member)                                         \
  (((u32)(val)&SQ_TASK_INFO0_##member##_MASK) << SQ_TASK_INFO0_##member##_SHIFT)
...
#define SQ_TASK_INFO1_SET(val, member)                                         \
  (((val)&SQ_TASK_INFO1_##member##_MASK) << SQ_TASK_INFO1_##member##_SHIFT)
...
#define SQ_TASK_INFO2_SET(val, member)                                         \
  (((val)&SQ_TASK_INFO2_##member##_MASK) << SQ_TASK_INFO2_##member##_SHIFT)
...
#define SQ_TASK_INFO4_SET(val, member)                                         \
  (((u32)(val)&SQ_TASK_INFO4_##member##_MASK) << SQ_TASK_INFO4_##member##_SHIFT)
...
#define SQ_DB_INFO_SET(val, member)                                            \
  (((u32)(val)&SQ_DB_INFO_##member##_MASK) << SQ_DB_INFO_##member##_SHIFT)
...
#define HINIC_SQ_WQEBB_CNT(num_sge)                                            \
  (int)(ALIGN(HINIC_SQ_WQE_SIZE((u32)num_sge), HINIC_SQ_WQEBB_SIZE) >>         \
        HINIC_SQ_WQEBB_SHIFT)
...
static inline void hinic_sq_wqe_cpu_to_be32(void *data, int nr_wqebb) {
  ...
  const uint8x16_t shuf_mask = {3,  2,  1, 0, 7,  6,  5,  4,
...
static inline struct rte_mbuf *hinic_copy_tx_mbuf(struct hinic_nic_dev *nic_dev,
  ...
  for (i = 0; i < sge_cnt; i++) {
    rte_memcpy((char *)dst_mbuf->buf_addr + offset,
               (char *)mbuf->buf_addr + mbuf->data_off, mbuf->data_len);
...
static inline bool hinic_mbuf_dma_map_sge(struct hinic_txq *txq,
  ...
  if (unlikely(sqe_info->around)) {
     ...
     */
    i = 0;
    for (sge_idx = sges; (u64)sge_idx <= txq->sq_bot_sge_addr; sge_idx++) {
      ...
      hinic_set_sge((struct hinic_sge *)sge_idx, dma_addr, mbuf->data_len);
      mbuf = mbuf->next;
      i++;
    }
  ...
  if (unlikely(sqe_info->cpy_mbuf_cnt != 0)) {
    ...
    mbuf = hinic_copy_tx_mbuf(txq->nic_dev, mbuf, real_nb_segs - nb_segs);
    if (unlikely(!mbuf))
      return false;
...
static inline void hinic_fill_sq_wqe_header(struct hinic_sq_ctrl *ctrl,
  ...
  ctrl->ctrl_fmt = SQ_CTRL_SET(bufdesc_size, BUFDESC_SECT_LEN) |
                   ...
                   SQ_CTRL_SET(ctrl_size, LEN) | SQ_CTRL_SET(owner, OWNER);
  ...
  if (!SQ_CTRL_QUEUE_INFO_GET(ctrl->queue_info, MSS)) {
    ...
    ctrl->queue_info = SQ_CTRL_QUEUE_INFO_CLEAR(ctrl->queue_info, MSS);
    ctrl->queue_info |= SQ_CTRL_QUEUE_INFO_SET(TX_MSS_MIN, MSS);
  }
}

static inline bool
hinic_is_tso_sge_valid(struct rte_mbuf *mbuf,
  ...
  if (unlikely(left_sges >= HINIC_NONTSO_PKT_MAX_SGE)) {
    ...
    while (left_sges >= HINIC_NONTSO_PKT_MAX_SGE) {
      ...
      if (unlikely(first_mss_sges == i)) {
        ...
        sqe_info->sge_cnt = mbuf_head->nb_segs - left_sges;
...
static inline void
hinic_set_l4_csum_info(struct hinic_sq_task *task, u32 *queue_info,
                       struct hinic_tx_offload_info *poff_info) {
  ...
  task->pkt_info1 |= SQ_TASK_INFO1_SET(poff_info->inner_l3_len, INNER_L3LEN);
  task->pkt_info0 |= SQ_TASK_INFO0_SET(poff_info->inner_l3_type, INNER_L3TYPE);
  task->pkt_info1 |= SQ_TASK_INFO1_SET(poff_info->inner_l4_len, INNER_L4LEN);
  task->pkt_info0 |= SQ_TASK_INFO0_SET(poff_info->inner_l4_type, L4OFFLOAD);
  *queue_info |= SQ_CTRL_QUEUE_INFO_SET(poff_info->payload_offset, PLDOFF) |
                 SQ_CTRL_QUEUE_INFO_SET(tcp_udp_cs, TCPUDP_CS) |
                 SQ_CTRL_QUEUE_INFO_SET(sctp, SCTP);
}

static inline void hinic_set_tso_info(struct hinic_sq_task *task,
...
static inline void hinic_set_vlan_tx_offload(struct hinic_sq_task *task,
                                             u32 *queue_info, u16 vlan_tag,
                                             u16 vlan_pri) {
...
static inline void
hinic_fill_tx_offload_info(struct rte_mbuf *mbuf, struct hinic_sq_task *task,
                           u32 *queue_info,
...
static inline void hinic_xmit_mbuf_cleanup(struct hinic_txq *txq) {
  ...
  for (i = 0; i < txq->tx_free_thresh; ++i) {
    ...
    if (hw_ci == sw_ci || (((hw_ci - sw_ci) & sq_mask) < tx_info->wqebb_cnt))
      break;
...
hinic_ipv4_phdr_cksum(const struct rte_ipv4_hdr *ipv4_hdr, uint64_t ol_flags) {
  ...
  if (ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    psd_hdr.len = rte_cpu_to_be_16(rte_be_to_cpu_16(ipv4_hdr->total_length) -
...
static inline void
hinic_get_outer_cs_pld_offset(struct rte_mbuf *m,
                              struct hinic_tx_offload_info *off_info) {
  uint64_t ol_flags = m->ol_flags;

  if ((ol_flags & RTE_MBUF_F_TX_L4_MASK) == RTE_MBUF_F_TX_UDP_CKSUM)
    off_info->payload_offset =
        m->outer_l2_len + m->outer_l3_len + m->l2_len + m->l3_len;
  else if ((ol_flags & RTE_MBUF_F_TX_TCP_CKSUM) ||
           (ol_flags & RTE_MBUF_F_TX_TCP_SEG))
    off_info->payload_offset =
        m->outer_l2_len + m->outer_l3_len + m->l2_len + m->l3_len + m->l4_len;
}

static inline void
hinic_get_pld_offset(struct rte_mbuf *m,
                     struct hinic_tx_offload_info *off_info) {
  uint64_t ol_flags = m->ol_flags;
...
static inline void
hinic_analyze_outer_ip_vxlan(struct rte_mbuf *mbuf,
                             struct hinic_tx_offload_info *off_info) {
  struct rte_ether_hdr *eth_hdr;
  struct rte_vlan_hdr *vlan_hdr;
  struct rte_ipv4_hdr *ipv4_hdr;
  ...
  if (eth_type == RTE_ETHER_TYPE_IPV4) {
    ...
    udp_hdr = rte_pktmbuf_mtod_offset(
        mbuf, struct rte_udp_hdr *, (mbuf->outer_l2_len + mbuf->outer_l3_len));
    udp_hdr->dgram_cksum = 0;
  }
}
...
static inline uint8_t hinic_analyze_l3_type(struct rte_mbuf *mbuf) {
  ...
  if (ol_flags & RTE_MBUF_F_TX_IPV4)
    l3_type = (ol_flags & RTE_MBUF_F_TX_IP_CKSUM) ? IPV4_PKT_WITH_CHKSUM_OFFLOAD
...
static inline void
hinic_calculate_tcp_checksum(struct rte_mbuf *mbuf,
                             struct hinic_tx_offload_info *off_info,
  ...
  if (ol_flags & RTE_MBUF_F_TX_IPV4) {
    ipv4_hdr =
        rte_pktmbuf_mtod_offset(mbuf, struct rte_ipv4_hdr *, inner_l3_offset);
    ...
    ipv6_hdr =
        rte_pktmbuf_mtod_offset(mbuf, struct rte_ipv6_hdr *, inner_l3_offset);
...
static inline void
hinic_calculate_udp_checksum(struct rte_mbuf *mbuf,
                             struct hinic_tx_offload_info *off_info,
                             uint64_t inner_l3_offset) {
  struct rte_ipv4_hdr *ipv4_hdr;
  struct rte_ipv6_hdr *ipv6_hdr;
  ...
  if (ol_flags & RTE_MBUF_F_TX_IPV4) {
    ...
    udp_hdr->dgram_cksum = hinic_ipv4_phdr_cksum(ipv4_hdr, ol_flags);
  } else {
    ipv6_hdr =
        rte_pktmbuf_mtod_offset(mbuf, struct rte_ipv6_hdr *, inner_l3_offset);
    ...
    udp_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_udp_hdr *,
                                      (inner_l3_offset + mbuf->l3_len));
    udp_hdr->dgram_cksum = hinic_ipv6_phdr_cksum(ipv6_hdr, ol_flags);
  }
...
static inline void
hinic_calculate_checksum(struct rte_mbuf *mbuf,
                         struct hinic_tx_offload_info *off_info,
                         uint64_t inner_l3_offset) {
  uint64_t ol_flags = mbuf->ol_flags;
...
hinic_tx_offload_pkt_prepare(struct rte_mbuf *m,
  ...
  if (ol_flags & RTE_MBUF_F_TX_TUNNEL_VXLAN) {
    ...
    if ((ol_flags & RTE_MBUF_F_TX_OUTER_IP_CKSUM) ||
        ...
        (ol_flags & RTE_MBUF_F_TX_TCP_SEG)) {
      inner_l3_offset = m->l2_len + m->outer_l2_len + m->outer_l3_len;
      off_info->outer_l2_len = m->outer_l2_len;
      off_info->outer_l3_len = m->outer_l3_len;
      /* just support vxlan tunneling pkt */
      off_info->inner_l2_len =
          m->l2_len - VXLANLEN - sizeof(struct rte_udp_hdr);
      ...
      off_info->tunnel_length =
          m->l2_len - off_info->outer_l2_len - off_info->outer_l3_len;
...
static inline bool
hinic_get_sge_txoff_info(struct rte_mbuf *mbuf_pkt,
                         struct hinic_wqe_info *sqe_info,
                         struct hinic_tx_offload_info *off_info) {
  ...
  if (likely(!(mbuf_pkt->ol_flags & RTE_MBUF_F_TX_TCP_SEG))) {
    if (unlikely(mbuf_pkt->pkt_len > MAX_SINGLE_SGE_SIZE)) {
      ...
      if ((u32)(total_len + (u16)HINIC_COPY_MBUF_SIZE) < mbuf_pkt->pkt_len)
        return false;
    ...
    if (unlikely(!hinic_is_tso_sge_valid(mbuf_pkt, off_info, sqe_info)))
      return false;
  }
...
static inline void hinic_sq_write_db(struct hinic_sq *sq, int cos) {
  ...
  sq_db.db_info = SQ_DB_INFO_SET(hi_prod_idx, HI_PI) |
                  ...
                  SQ_DB_INFO_SET(cos, COS) | SQ_DB_INFO_SET(sq->q_id, QID);
...
u16 hinic_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, u16 nb_pkts) {
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (unlikely(!hinic_get_sge_txoff_info(mbuf_pkt, &sqe_info, &off_info))) {
    ...
    if (unlikely(!hinic_mbuf_dma_map_sge(txq, mbuf_pkt, sq_wqe->buf_descs,
                                         &sqe_info))) {
      hinic_return_sq_wqe(txq->nic_dev->hwdev, txq->q_id, wqe_wqebb_cnt,
                          sqe_info.owner);
      txq->txq_stats.off_errs++;
      break;
    }
    ...
    hinic_fill_tx_offload_info(mbuf_pkt, task, &queue_info, &off_info);
    ...
    hinic_fill_sq_wqe_header(&sq_wqe->ctrl, queue_info, sqe_info.sge_cnt,
                             sqe_info.owner);
...
void hinic_free_all_tx_mbufs(struct hinic_txq *txq) {
  ...
  struct hinic_tx_info *tx_info;
  int free_wqebbs = hinic_get_sq_free_wqebbs(nic_dev->hwdev, txq->q_id) + 1;
  ...
  while (free_wqebbs < txq->q_depth) {
    ...
    hinic_update_sq_local_ci(nic_dev->hwdev, txq->q_id, tx_info->wqebb_cnt);
...
int hinic_setup_tx_resources(struct hinic_txq *txq) {
  ...
  txq->tx_info = rte_zmalloc_socket("tx_info", tx_info_sz, RTE_CACHE_LINE_SIZE,
                                    txq->socket_id);
  if (!txq->tx_info)
    return -ENOMEM;
...
int hinic_create_sq(struct hinic_hwdev *hwdev, u16 q_id, u16 sq_depth,
  ...
  err = hinic_wq_allocate(nic_io->hwdev, &nic_io->sq_wq[q_id],
                          HINIC_SQ_WQEBB_SHIFT, nic_io->sq_depth, socket_id);
  if (err) {
