...
static int hinic_check_ethertype_first_item(const struct rte_flow_item *item,
  ...
  if (item->last) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, item,
                       "Not supported last point for range");
    return -rte_errno;
  }
...
static int hinic_parse_ethertype_aciton(
    const struct rte_flow_action *actions, const struct rte_flow_action *act,
...
hinic_check_ntuple_act_ele(__rte_unused const struct rte_flow_item *item,
  ...
  if (act->type != RTE_FLOW_ACTION_TYPE_QUEUE) {
    memset(filter, 0, sizeof(struct rte_eth_ntuple_filter));
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, act,
                       "Flow action type is not QUEUE.");
    return -rte_errno;
  }
  filter->queue = ((const struct rte_flow_action_queue *)act->conf)->index;
...
hinic_ntuple_item_check_ipv4(const struct rte_flow_item **in_out_item,
   ...
   */
  if (ipv4_mask->hdr.version_ihl || ipv4_mask->hdr.type_of_service ||
      ipv4_mask->hdr.total_length || ipv4_mask->hdr.packet_id ||
      ipv4_mask->hdr.fragment_offset || ipv4_mask->hdr.time_to_live ||
      ipv4_mask->hdr.hdr_checksum || !ipv4_mask->hdr.next_proto_id) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                       "Not supported by ntuple filter");
    return -rte_errno;
  }
...
static int hinic_ntuple_item_check_l4(const struct rte_flow_item **in_out_item,
  ...
  /* Get TCP or UDP info */
  if (item->type != RTE_FLOW_ITEM_TYPE_END && (!item->spec || !item->mask)) {
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_TCP) {
    ...
    if (tcp_mask->hdr.sent_seq || tcp_mask->hdr.recv_ack ||
        tcp_mask->hdr.data_off || tcp_mask->hdr.rx_win || tcp_mask->hdr.cksum ||
    ...
    if (icmp_mask->hdr.icmp_cksum || icmp_mask->hdr.icmp_ident ||
        icmp_mask->hdr.icmp_seq_nb || icmp_mask->hdr.icmp_type ||
...
static int hinic_normal_item_check_ether(const struct rte_flow_item **ip_item,
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_ETH) {
    ...
    if (item->spec || item->mask) {
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by fdir filter,support mac");
      return -rte_errno;
    }
    /* Check if the next not void item is IPv4 */
    item = next_no_void_pattern(pattern, item);
...
static int hinic_normal_item_check_ip(const struct rte_flow_item **in_out_item,
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_IPV4) {
    ...
    if (ipv6_mask->hdr.vtc_flow || ipv6_mask->hdr.payload_len ||
        ipv6_mask->hdr.proto || ipv6_mask->hdr.hop_limits) {
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
    ...
    rte_memcpy(rule->hinic_fdir.dst_ipv6, ipv6_spec->hdr.dst_addr, 16);
...
static int
hinic_normal_item_check_l4(const struct rte_flow_item **in_out_item,
  ...
  if (item->type != RTE_FLOW_ITEM_TYPE_END) {
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                       "Not supported by normal fdir filter, not support l4");
    return -rte_errno;
  }
...
static int hinic_normal_item_check_end(const struct rte_flow_item *item,
  ...
  if (item->type != RTE_FLOW_ITEM_TYPE_END) {
    memset(rule, 0, sizeof(struct hinic_fdir_rule));
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                       "Not supported by fdir filter, support end");
    return -rte_errno;
  }
...
hinic_tcam_normal_item_check_l4(const struct rte_flow_item **in_out_item,
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_ICMP) {
    ...
    if (tcp_mask->hdr.sent_seq || tcp_mask->hdr.recv_ack ||
      ...
      (void)memset(rule, 0, sizeof(struct hinic_fdir_rule));
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by fdir normal tcam filter");
      return -rte_errno;
    }
    ...
    if (udp_mask->hdr.dgram_len || udp_mask->hdr.dgram_cksum) {
      (void)memset(rule, 0, sizeof(struct hinic_fdir_rule));
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by fdir filter, support udp");
      return -rte_errno;
    }
    ...
    rte_flow_error_set(
        error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
        "Not supported by fdir filter tcam normal, l4 only support icmp, tcp");
    return -rte_errno;
  }
  ...
  if (item->type != RTE_FLOW_ITEM_TYPE_END) {
    (void)memset(rule, 0, sizeof(struct hinic_fdir_rule));
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                       "Not supported by fdir filter tcam normal, support end");
    return -rte_errno;
  }
...
static int hinic_check_tcam_normal_item_ele(
    const struct rte_flow_item *item, const struct rte_flow_item pattern[],
    struct hinic_fdir_rule *rule, struct rte_flow_error *error) {
  if (hinic_normal_item_check_ether(&item, pattern, error) ||
      hinic_normal_item_check_ip(&item, pattern, rule, error) ||
...
static int hinic_tunnel_item_check_l4(const struct rte_flow_item **in_out_item,
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_UDP) {
    ...
    if (item->type != RTE_FLOW_ITEM_TYPE_VXLAN) {
      (void)memset(rule, 0, sizeof(struct hinic_fdir_rule));
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by fdir filter, support vxlan");
      return -rte_errno;
    }
    ...
    *in_out_item = item;
    ...
    rte_flow_error_set(
        error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
        "Not supported by fdir filter tcam tunnel, outer l4 only support udp");
    return -rte_errno;
  }
...
hinic_tunnel_item_check_vxlan(const struct rte_flow_item **in_out_item,
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_VXLAN) {
    ...
    if (item->type != RTE_FLOW_ITEM_TYPE_TCP &&
      ...
      (void)memset(rule, 0, sizeof(struct hinic_fdir_rule));
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                         "Not supported by fdir filter, support tcp/udp");
      return -rte_errno;
    }
  ...
  if (item->type != RTE_FLOW_ITEM_TYPE_END) {
    ...
    if (item->type == RTE_FLOW_ITEM_TYPE_TCP) {
      ...
      if (tcp_mask->hdr.sent_seq || tcp_mask->hdr.recv_ack ||
        ...
        (void)memset(rule, 0, sizeof(struct hinic_fdir_rule));
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                           "Not supported by fdir filter, support tcp");
        return -rte_errno;
      }

      rule->mode = HINIC_FDIR_MODE_TCAM;
      rule->mask.tunnel_flag = UINT16_MAX;
      rule->mask.tunnel_inner_src_port_mask = tcp_mask->hdr.src_port;
      rule->mask.tunnel_inner_dst_port_mask = tcp_mask->hdr.dst_port;
      rule->mask.proto_mask = UINT16_MAX;

      rule->hinic_fdir.proto = IP_HEADER_PROTOCOL_TYPE_TCP;
      if (item->spec) {
        tcp_spec = (const struct rte_flow_item_tcp *)item->spec;
        rule->hinic_fdir.tunnel_inner_src_port = tcp_spec->hdr.src_port;
        rule->hinic_fdir.tunnel_inner_dst_port = tcp_spec->hdr.dst_port;
      }
    } else if (item->type == RTE_FLOW_ITEM_TYPE_UDP) {
      /*
       * Only care about src & dst ports,
      ...
      rule->mode = HINIC_FDIR_MODE_TCAM;
      rule->mask.tunnel_flag = UINT16_MAX;
      rule->mask.tunnel_inner_src_port_mask = udp_mask->hdr.src_port;
      rule->mask.tunnel_inner_dst_port_mask = udp_mask->hdr.dst_port;
      rule->mask.proto_mask = UINT16_MAX;

      rule->hinic_fdir.proto = IP_HEADER_PROTOCOL_TYPE_UDP;
      if (item->spec) {
        udp_spec = (const struct rte_flow_item_udp *)item->spec;
        rule->hinic_fdir.tunnel_inner_src_port = udp_spec->hdr.src_port;
        rule->hinic_fdir.tunnel_inner_dst_port = udp_spec->hdr.dst_port;
      }
    } else if (item->type == RTE_FLOW_ITEM_TYPE_ANY) {
      rule->mode = HINIC_FDIR_MODE_TCAM;
      rule->mask.tunnel_flag = UINT16_MAX;
...
static int hinic_check_tcam_tunnel_item_ele(
    const struct rte_flow_item *item, const struct rte_flow_item pattern[],
    struct hinic_fdir_rule *rule, struct rte_flow_error *error) {
  if (hinic_normal_item_check_ether(&item, pattern, error) ||
      hinic_normal_item_check_ip(&item, pattern, rule, error) ||
...
static int hinic_check_normal_act_ele(const struct rte_flow_item *item,
  ...
  if (act->type != RTE_FLOW_ACTION_TYPE_QUEUE) {
    memset(rule, 0, sizeof(struct hinic_fdir_rule));
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, item,
                       "Not supported action.");
    return -rte_errno;
  }
 ...
 * Item->last should be NULL.
 */
static int hinic_parse_fdir_filter_normal(
    const struct rte_flow_attr *attr, const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], struct hinic_fdir_rule *rule,
    struct rte_flow_error *error) {
  const struct rte_flow_item *item = NULL;
 ...
 * Item->last should be NULL.
 */
static int hinic_parse_fdir_filter_tcam_normal(
    const struct rte_flow_attr *attr, const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], struct hinic_fdir_rule *rule,
    struct rte_flow_error *error) {
  const struct rte_flow_item *item = NULL;
 ...
 * Item->last should be NULL.
 */
static int hinic_parse_fdir_filter_tacm_tunnel(
    const struct rte_flow_attr *attr, const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], struct hinic_fdir_rule *rule,
    struct rte_flow_error *error) {
  const struct rte_flow_item *item = NULL;
...
static int hinic_parse_fdir_filter(struct rte_eth_dev *dev,
  ...
  ret = hinic_parse_fdir_filter_normal(attr, pattern, actions, rule, error);
  if (!ret)
    goto step_next;

  ret =
      hinic_parse_fdir_filter_tcam_normal(attr, pattern, actions, rule, error);
  if (!ret)
    goto step_next;

  ret =
      hinic_parse_fdir_filter_tacm_tunnel(attr, pattern, actions, rule, error);
  if (ret)
    return ret;
...
static int hinic_flow_validate(struct rte_eth_dev *dev,
  ...
  ret = hinic_parse_ntuple_filter(dev, attr, pattern, actions, &ntuple_filter,
                                  error);
  if (!ret)
    return 0;
  ...
  ret = hinic_parse_fdir_filter(dev, attr, pattern, actions, &fdir_rule, error);
...
static int hinic_set_lacp_tcam(struct hinic_nic_dev *nic_dev) {
  ...
  return hinic_set_fdir_tcam(nic_dev->hwdev, TCAM_PKT_LACP, &lacp_rule,
                             &lacp_action);
}
...
static int hinic_set_bgp_dport_tcam(struct hinic_nic_dev *nic_dev) {
  ...
  return hinic_set_fdir_tcam(nic_dev->hwdev, TCAM_PKT_BGP_DPORT, &bgp_rule,
                             &bgp_action);
}
...
static int hinic_set_bgp_sport_tcam(struct hinic_nic_dev *nic_dev) {
  ...
  return hinic_set_fdir_tcam(nic_dev->hwdev, TCAM_PKT_BGP_SPORT, &bgp_rule,
                             &bgp_action);
}
...
static int hinic_set_vrrp_tcam(struct hinic_nic_dev *nic_dev) {
  ...
  return hinic_set_fdir_tcam(nic_dev->hwdev, TCAM_PKT_VRRP, &vrrp_rule,
                             &vrrp_action);
}
...
static int hinic_filter_info_init(struct hinic_5tuple_filter *filter,
  ...
  default:
    PMD_DRV_LOG(ERR,
                "5tuple filters just support BGP/VRRP/ICMP now, "
                "proto: 0x%x, dst_port: 0x%x, dst_port_mask: 0x%x."
                "src_port: 0x%x, src_port_mask: 0x%x.",
                filter->filter_info.proto, filter->filter_info.dst_port,
                filter->filter_info.dst_port_mask, filter->filter_info.src_port,
                filter->filter_info.src_port_mask);
    return -EINVAL;
  }
...
static int hinic_lookup_new_filter(struct hinic_5tuple_filter *filter,
  ...
  if (!(filter_info->type_mask & (1 << type_id))) {
    ...
    filter_info->pkt_filters[type_id].pkt_proto = filter->filter_info.proto;
...
static int hinic_add_5tuple_filter(struct rte_eth_dev *dev,
  ...
  if (ret_fw) {
    PMD_DRV_LOG(ERR,
                "Set fdir filter failed, type: 0x%x, qid: 0x%x, enable: 0x%x",
  ...
  case IPPROTO_TCP:
    if (filter->filter_info.dst_port == RTE_BE16(BGP_DPORT_ID)) {
      ...
      PMD_DRV_LOG(INFO, "Set dport bgp succeed, qid: 0x%x, enable: 0x%x",
                  filter->queue, filter_info->pkt_filters[i].enable);
    } else if (filter->filter_info.src_port == RTE_BE16(BGP_DPORT_ID)) {
      ret_fw = hinic_set_bgp_sport_tcam(nic_dev);
      if (ret_fw) {
        PMD_DRV_LOG(ERR,
                    "Set sport bgp failed, "
                    "type: 0x%x, qid: 0x%x, enable: 0x%x",
                    filter_info->pkt_type, filter->queue,
                    filter_info->pkt_filters[i].enable);
        return -EFAULT;
 ...
 */
static int
hinic_add_del_ntuple_filter(struct rte_eth_dev *dev,
                            struct rte_eth_ntuple_filter *ntuple_filter,
                            bool add) {
  struct hinic_filter_info *filter_info =
      HINIC_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
  ...
  filter =
      hinic_5tuple_filter_lookup(&filter_info->fivetuple_list, &filter_5tuple);
  if (filter != NULL && add) {
...
hinic_ethertype_filter_insert(struct hinic_filter_info *filter_info,
  ...
  if (!(filter_info->type_mask & (1 << id))) {
    ...
    filter_info->pkt_filters[id].pkt_proto = ethertype_filter->pkt_proto;
...
hinic_add_del_ethertype_filter(struct rte_eth_dev *dev,
  ...
  if (add) {
    ...
    i = hinic_ethertype_filter_insert(filter_info, &ethertype_filter);
    if (i < 0)
      return -ENOSPC;
    ...
    if (ret_fw) {
      PMD_DRV_LOG(ERR,
                  "add ethertype failed, type: 0x%x, qid: 0x%x, enable: 0x%x",
                  filter_info->pkt_type, filter->queue,
                  filter_info->pkt_filters[i].enable);
    ...
    PMD_DRV_LOG(INFO,
                "Add ethertype succeed, type: 0x%x, qid: 0x%x, enable: 0x%x",
                filter_info->pkt_type, filter->queue,
                filter_info->pkt_filters[i].enable);
    ...
    ethertype_filter.pkt_proto = filter->ether_type;
    i = hinic_ethertype_filter_lookup(filter_info, &ethertype_filter);
    if (i < 0)
      return -EINVAL;
    ...
    if ((filter_info->type_mask & (1 << i))) {
      ...
      PMD_DRV_LOG(INFO,
                  "Del ethertype succeed, type: 0x%x, qid: 0x%x, enable: 0x%x",
                  filter_info->pkt_type, filter_info->pkt_filters[i].qid,
                  filter_info->pkt_filters[i].enable);
      ...
      PMD_DRV_LOG(
          ERR, "Ethertype doesn't exist, type: 0x%x, qid: 0x%x, enable: 0x%x",
          filter_info->pkt_type, filter->queue,
          filter_info->pkt_filters[i].enable);
      return -ENOENT;
    }
...
static inline int hinic_add_del_fdir_filter(struct rte_eth_dev *dev,
  ...
  if (add) {
    ret =
        hinic_set_normal_filter(nic_dev->hwdev, fdir_info.qid, true,
                                fdir_info.fdir_key, true, fdir_info.fdir_flag);
    if (ret) {
      PMD_DRV_LOG(ERR,
                  "Add fdir filter failed, flag: 0x%x, qid: 0x%x, key: 0x%x",
                  fdir_info.fdir_flag, fdir_info.qid, fdir_info.fdir_key);
      return -ENOENT;
    }
    PMD_DRV_LOG(INFO,
                "Add fdir filter succeed, flag: 0x%x, qid: 0x%x, key: 0x%x",
                fdir_info.fdir_flag, fdir_info.qid, fdir_info.fdir_key);
  } else {
    ret =
        hinic_set_normal_filter(nic_dev->hwdev, fdir_info.qid, false,
                                fdir_info.fdir_key, true, fdir_info.fdir_flag);
    if (ret) {
      PMD_DRV_LOG(ERR,
                  "Del fdir filter failed, flag: 0x%x, qid: 0x%x, key: 0x%x",
                  fdir_info.fdir_flag, fdir_info.qid, fdir_info.fdir_key);
      return -ENOENT;
    }
    PMD_DRV_LOG(INFO,
                "Del fdir filter succeed, flag: 0x%x, qid: 0x%x, key: 0x%x",
                fdir_info.fdir_flag, fdir_info.qid, fdir_info.fdir_key);
  }
...
static int hinic_fdir_tcam_ipv4_init(struct rte_eth_dev *dev,
  ...
  switch (rule->mask.dst_ipv4_mask) {
  case UINT32_MAX:
    tcam_key->key_info.ext_dip_h = (rule->hinic_fdir.dst_ip >> 16) & 0xffffU;
    tcam_key->key_info.ext_dip_l = rule->hinic_fdir.dst_ip & 0xffffU;
    tcam_key->key_mask.ext_dip_h = (rule->mask.dst_ipv4_mask >> 16) & 0xffffU;
    tcam_key->key_mask.ext_dip_l = rule->mask.dst_ipv4_mask & 0xffffU;
    break;
  ...
  if (rule->mask.tunnel_inner_dst_port_mask > 0) {
    tcam_key->key_info.dst_port = rule->hinic_fdir.tunnel_inner_dst_port;
    tcam_key->key_mask.dst_port = rule->mask.tunnel_inner_dst_port_mask;
  }

  if (rule->mask.tunnel_inner_src_port_mask > 0) {
    tcam_key->key_info.src_port = rule->hinic_fdir.tunnel_inner_src_port;
    tcam_key->key_mask.src_port = rule->mask.tunnel_inner_src_port_mask;
  }
...
static int hinic_fdir_tcam_ipv6_init(struct rte_eth_dev *dev,
  ...
  switch (rule->mask.proto_mask) {
  case UINT16_MAX:
    tcam_key->key_info_ipv6.protocol = (rule->hinic_fdir.proto) & 0x7F;
...
hinic_tcam_filter_lookup(struct hinic_tcam_filter_list *filter_list,
  ...
  TAILQ_FOREACH(it, filter_list, entries) {
    if (memcmp(key, &it->tcam_key, sizeof(struct tag_tcam_key)) == 0) {
      return it;
    }
  }
...
static int hinic_add_tcam_filter(struct rte_eth_dev *dev,
  ...
  if (tcam_info->tcam_rule_nums == 0) {
    if (hinic_func_type(nic_dev->hwdev) == TYPE_VF) {
      rc = hinic_alloc_tcam_block(nic_dev->hwdev, HINIC_TCAM_BLOCK_TYPE_VF,
                                  &tcam_block_index);
      if (rc != 0) {
      ...
      rc = hinic_alloc_tcam_block(nic_dev->hwdev, HINIC_TCAM_BLOCK_TYPE_PF,
                                  &tcam_block_index);
      if (rc != 0) {
  ...
  PMD_DRV_LOG(
      INFO,
      "Add fdir_tcam_rule function_id: 0x%x,"
  ...
  if (tcam_info->tcam_rule_nums == 0) {
    ...
    if (rc < 0) {
      (void)hinic_del_tcam_rule(nic_dev->hwdev, fdir_tcam_rule->index);
      return rc;
    }
...
static int hinic_del_tcam_filter(struct rte_eth_dev *dev,
  ...
  if (hinic_func_type(nic_dev->hwdev) == TYPE_VF) {
    index =
        HINIC_PKT_VF_TCAM_INDEX_START(tcam_block_index) + tcam_filter->index;
    block_type = HINIC_TCAM_BLOCK_TYPE_VF;
  } else {
    index = tcam_block_index * HINIC_PF_MAX_TCAM_FILTERS + tcam_filter->index;
  ...
  PMD_DRV_LOG(INFO,
              "Del fdir_tcam_rule function_id: 0x%x, "
  ...
  if (tcam_info->tcam_rule_nums == 0) {
    (void)hinic_free_tcam_block(nic_dev->hwdev, block_type, &tcam_block_index);
  }
...
static int hinic_add_del_tcam_fdir_filter(struct rte_eth_dev *dev,
  ...
  tcam_filter = hinic_tcam_filter_lookup(&tcam_info->tcam_list, &tcam_key);
  ...
  if (add) {
    tcam_filter =
        rte_zmalloc("hinic_5tuple_filter", sizeof(struct hinic_tcam_filter), 0);
    if (tcam_filter == NULL)
      return -ENOMEM;
    (void)rte_memcpy(&tcam_filter->tcam_key, &tcam_key,
                     sizeof(struct tag_tcam_key));
...
hinic_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
                  const struct rte_flow_item pattern[],
                  const struct rte_flow_action actions[],
                  struct rte_flow_error *error) {
  int ret;
  ...
  hinic_flow_mem_ptr =
      rte_zmalloc("hinic_flow_mem", sizeof(struct hinic_flow_mem), 0);
  ...
  TAILQ_INSERT_TAIL(&nic_dev->hinic_flow_list, hinic_flow_mem_ptr, entries);
  ...
  if (!ret) {
    ...
    if (!ret) {
      ntuple_filter_ptr = rte_zmalloc(
          "hinic_ntuple_filter", sizeof(struct hinic_ntuple_filter_ele), 0);
      if (ntuple_filter_ptr == NULL) {
        PMD_DRV_LOG(ERR, "Failed to allocate ntuple_filter_ptr");
        (void)hinic_add_del_ntuple_filter(dev, &ntuple_filter, FALSE);
        goto out;
      }
      rte_memcpy(&ntuple_filter_ptr->filter_info, &ntuple_filter,
                 sizeof(struct rte_eth_ntuple_filter));
      TAILQ_INSERT_TAIL(&nic_dev->filter_ntuple_list, ntuple_filter_ptr,
                        entries);
  ...
  ret = hinic_parse_ethertype_filter(dev, attr, pattern, actions,
                                     &ethertype_filter, error);
  if (!ret) {
    ret = hinic_add_del_ethertype_filter(dev, &ethertype_filter, TRUE);
    if (!ret) {
      ...
      if (ethertype_filter_ptr == NULL) {
        ...
        (void)hinic_add_del_ethertype_filter(dev, &ethertype_filter, FALSE);
        goto out;
      }
      rte_memcpy(&ethertype_filter_ptr->filter_info, &ethertype_filter,
                 sizeof(struct rte_eth_ethertype_filter));
      TAILQ_INSERT_TAIL(&nic_dev->filter_ethertype_list, ethertype_filter_ptr,
                        entries);
  ...
  if (!ret) {
    if (fdir_rule.mode == HINIC_FDIR_MODE_NORMAL) {
      ...
      ret = hinic_add_del_tcam_fdir_filter(dev, &fdir_rule, TRUE);
    } else {
    ...
    if (!ret) {
      fdir_rule_ptr =
          rte_zmalloc("hinic_fdir_rule", sizeof(struct hinic_fdir_rule_ele), 0);
      if (fdir_rule_ptr == NULL) {
        PMD_DRV_LOG(ERR, "Failed to allocate fdir_rule_ptr");
        if (fdir_rule.mode == HINIC_FDIR_MODE_NORMAL)
          hinic_add_del_fdir_filter(dev, &fdir_rule, FALSE);
        else if (fdir_rule.mode == HINIC_FDIR_MODE_TCAM)
          hinic_add_del_tcam_fdir_filter(dev, &fdir_rule, FALSE);

        goto out;
      }
      rte_memcpy(&fdir_rule_ptr->filter_info, &fdir_rule,
                 sizeof(struct hinic_fdir_rule));
      TAILQ_INSERT_TAIL(&nic_dev->filter_fdir_rule_list, fdir_rule_ptr,
                        entries);
...
static int hinic_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  case RTE_ETH_FILTER_NTUPLE:
    ntuple_filter_ptr = (struct hinic_ntuple_filter_ele *)pmd_flow->rule;
    rte_memcpy(&ntuple_filter, &ntuple_filter_ptr->filter_info,
               sizeof(struct rte_eth_ntuple_filter));
    ret = hinic_add_del_ntuple_filter(dev, &ntuple_filter, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&nic_dev->filter_ntuple_list, ntuple_filter_ptr, entries);
      rte_free(ntuple_filter_ptr);
    }
    break;
  case RTE_ETH_FILTER_ETHERTYPE:
    ethertype_filter_ptr = (struct hinic_ethertype_filter_ele *)pmd_flow->rule;
    rte_memcpy(&ethertype_filter, &ethertype_filter_ptr->filter_info,
               sizeof(struct rte_eth_ethertype_filter));
    ret = hinic_add_del_ethertype_filter(dev, &ethertype_filter, FALSE);
    if (!ret) {
      TAILQ_REMOVE(&nic_dev->filter_ethertype_list, ethertype_filter_ptr,
                   entries);
  ...
  case RTE_ETH_FILTER_FDIR:
    ...
    rte_memcpy(&fdir_rule, &fdir_rule_ptr->filter_info,
               sizeof(struct hinic_fdir_rule));
    if (fdir_rule.mode == HINIC_FDIR_MODE_NORMAL) {
      ret = hinic_add_del_fdir_filter(dev, &fdir_rule, FALSE);
    } else if (fdir_rule.mode == HINIC_FDIR_MODE_TCAM) {
      ret = hinic_add_del_tcam_fdir_filter(dev, &fdir_rule, FALSE);
    } else {
      PMD_DRV_LOG(ERR, "FDIR Filter type is wrong!");
      ret = -EINVAL;
    }
    if (!ret) {
      TAILQ_REMOVE(&nic_dev->filter_fdir_rule_list, fdir_rule_ptr, entries);
      rte_free(fdir_rule_ptr);
    }
    break;
  default:
    PMD_DRV_LOG(WARNING, "Filter type (%d) is not supported", filter_type);
    ret = -EINVAL;
    break;
  }
...
static void hinic_clear_all_ethertype_filter(struct rte_eth_dev *dev) {
  ...
  if (filter_info->type_mask & (1 << HINIC_PKT_TYPE_FIND_ID(PKT_LACP_TYPE))) {
    hinic_ethertype_filter_remove(filter_info,
                                  HINIC_PKT_TYPE_FIND_ID(PKT_LACP_TYPE));
    ret = hinic_set_fdir_filter(nic_dev->hwdev, PKT_LACP_TYPE, filter_info->qid,
                                false, true);

    (void)hinic_clear_fdir_tcam(nic_dev->hwdev, TCAM_PKT_LACP);
  }

  if (filter_info->type_mask & (1 << HINIC_PKT_TYPE_FIND_ID(PKT_ARP_TYPE))) {
    hinic_ethertype_filter_remove(filter_info,
                                  HINIC_PKT_TYPE_FIND_ID(PKT_ARP_TYPE));
    ret = hinic_set_fdir_filter(nic_dev->hwdev, PKT_ARP_TYPE, filter_info->qid,
                                false, true);
  }
...
static void hinic_filterlist_flush(struct rte_eth_dev *dev) {
  ...
  while ((ntuple_filter_ptr = TAILQ_FIRST(&nic_dev->filter_ntuple_list))) {
    TAILQ_REMOVE(&nic_dev->filter_ntuple_list, ntuple_filter_ptr, entries);
    rte_free(ntuple_filter_ptr);
  }

  while (
      (ethertype_filter_ptr = TAILQ_FIRST(&nic_dev->filter_ethertype_list))) {
    TAILQ_REMOVE(&nic_dev->filter_ethertype_list, ethertype_filter_ptr,
                 entries);
    rte_free(ethertype_filter_ptr);
  }

  while ((fdir_rule_ptr = TAILQ_FIRST(&nic_dev->filter_fdir_rule_list))) {
    TAILQ_REMOVE(&nic_dev->filter_fdir_rule_list, fdir_rule_ptr, entries);
    rte_free(fdir_rule_ptr);
  }

  while ((hinic_flow_mem_ptr = TAILQ_FIRST(&nic_dev->hinic_flow_list))) {
    TAILQ_REMOVE(&nic_dev->hinic_flow_list, hinic_flow_mem_ptr, entries);
