...
/* rxq wq operations */
#define HINIC_GET_RQ_WQE_MASK(rxq) ((rxq)->wq->mask)
...
#define HINIC_GET_RX_DONE_BE(status) ((status)&0x80U)
...
#define HINIC_GET_RX_VLAN_TAG(vlan_len) RQ_CQE_SGE_GET(vlan_len, VLAN)

#define HINIC_GET_RX_PKT_LEN(vlan_len) RQ_CQE_SGE_GET(vlan_len, LEN)
...
#define RQ_CQE_STATUS_GET(val, member)                                         \
  (((val) >> RQ_CQE_STATUS_##member##_SHIFT) & RQ_CQE_STATUS_##member##_MASK)

#define RQ_CQE_STATUS_CLEAR(val, member)                                       \
  ((val) & (~(RQ_CQE_STATUS_##member##_MASK << RQ_CQE_STATUS_##member##_SHIFT)))

#define HINIC_GET_RX_CSUM_ERR(status) RQ_CQE_STATUS_GET(status, CSUM_ERR)

#define HINIC_GET_RX_DONE(status) RQ_CQE_STATUS_GET(status, RXDONE)

#define HINIC_GET_RX_FLUSH(status) RQ_CQE_STATUS_GET(status, FLUSH)

#define HINIC_GET_RX_BP_EN(status) RQ_CQE_STATUS_GET(status, BP_EN)

#define HINIC_GET_RX_NUM_LRO(status) RQ_CQE_STATUS_GET(status, NUM_LRO)
...
#define HINIC_CSUM_ERR_IP(csum_err) ((csum_err)&RQ_CQE_STATUS_CSUM_ERR_IP_MASK)

#define HINIC_CSUM_ERR_L4(csum_err) ((csum_err)&RQ_CQE_STATUS_CSUM_ERR_L4_MASK)
...
int hinic_create_rq(struct hinic_hwdev *hwdev, u16 q_id, u16 rq_depth,
  ...
  err = hinic_wq_allocate(hwdev, &nic_io->rq_wq[q_id], HINIC_RQ_WQEBB_SHIFT,
                          nic_io->rq_depth, socket_id);
  if (err) {
  ...
  rq->pi_virt_addr = (volatile u16 *)dma_zalloc_coherent(
      hwdev, HINIC_PAGE_SIZE, &rq->pi_dma_addr, socket_id);
  if (!rq->pi_virt_addr) {
...
static void hinic_prepare_rq_wqe(void *wqe, __rte_unused u16 pi,
                                 dma_addr_t buf_addr, dma_addr_t cqe_dma) {
...
static int hinic_rx_alloc_cqe(struct hinic_rxq *rxq, unsigned int socket_id) {
  ...
  rxq->cqe_start_vaddr = dma_zalloc_coherent(rxq->nic_dev->hwdev, cqe_mem_size,
                                             &rxq->cqe_start_paddr, socket_id);
...
static void hinic_rx_free_cqe(struct hinic_rxq *rxq) {
  ...
  cqe_mem_size = sizeof(struct hinic_rq_cqe) * rxq->q_depth;
  dma_free_coherent(rxq->nic_dev->hwdev, cqe_mem_size, rxq->cqe_start_vaddr,
                    rxq->cqe_start_paddr);
...
int hinic_setup_rx_resources(struct hinic_rxq *rxq) {
  ...
  rxq->rx_info = rte_zmalloc_socket("rx_info", rx_info_sz, RTE_CACHE_LINE_SIZE,
                                    rxq->socket_id);
  if (!rxq->rx_info)
    return -ENOMEM;
...
static void hinic_recv_jumbo_pkt(struct hinic_rxq *rxq,
  ...
  while (remain_pkt_len > 0) {
    ...
    pkt_len = remain_pkt_len > rx_buf_len ? rx_buf_len : remain_pkt_len;
...
static void hinic_rss_deinit(struct hinic_nic_dev *nic_dev) {
  ...
  (void)hinic_rss_cfg(nic_dev->hwdev, 0, nic_dev->rss_tmpl_idx, 0, prio_tc);
}
...
static void hinic_fill_rss_type(struct nic_rss_type *rss_type,
  ...
  rss_type->ipv4 =
      (rss_hf & (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_FRAG_IPV4)) ? 1 : 0;
  rss_type->tcp_ipv4 = (rss_hf & RTE_ETH_RSS_NONFRAG_IPV4_TCP) ? 1 : 0;
  rss_type->ipv6 =
      (rss_hf & (RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_FRAG_IPV6)) ? 1 : 0;
...
static void hinic_fillout_indir_tbl(struct hinic_nic_dev *nic_dev, u32 *indir) {
  ...
  if (rss_queue_count == 0) {
    ...
    while (i < HINIC_RSS_INDIR_SIZE)
      for (j = 0; (j < rss_queue_count) && (i < HINIC_RSS_INDIR_SIZE); j++)
...
static void hinic_add_rq_to_rx_queue_list(struct hinic_nic_dev *nic_dev,
                                          u16 queue_id) {
  u8 rss_queue_count = nic_dev->num_rss;
...
static void hinic_destroy_num_qps(struct hinic_nic_dev *nic_dev) {
  if (nic_dev->flags & RTE_ETH_MQ_RX_RSS_FLAG) {
    if (hinic_rss_template_free(nic_dev->hwdev, nic_dev->rss_tmpl_idx))
...
int hinic_rx_configure(struct rte_eth_dev *dev) {
  ...
  err = hinic_set_rx_csum_offload(nic_dev->hwdev, HINIC_RX_CSUM_OFFLOAD_EN);
  if (err)
  ...
  if (err) {
    PMD_DRV_LOG(ERR, "%s %s lro failed, err: %d, max_lro_size: %d",
                dev->data->name, lro_en ? "Enable" : "Disable", err,
                max_lro_size);
    goto set_rx_lro_err;
  }
...
static void hinic_rx_remove_lro(struct hinic_nic_dev *nic_dev) {
  ...
  if (err)
    PMD_DRV_LOG(ERR, "%s disable LRO failed", nic_dev->proc_dev_name);
}
...
void hinic_free_all_rx_mbufs(struct hinic_rxq *rxq) {
  struct hinic_nic_dev *nic_dev = rxq->nic_dev;
  struct hinic_rx_info *rx_info;
  int free_wqebbs = hinic_get_rq_free_wqebbs(nic_dev->hwdev, rxq->q_id) + 1;
...
static inline void hinic_rearm_rxq_mbuf(struct hinic_rxq *rxq) {
  ...
  for (i = 0; i < rearm_wqebbs; i++) {
    ...
    rq_wqe->buf_desc.addr_high = cpu_to_be32(upper_32_bits(dma_addr));
    rq_wqe->buf_desc.addr_low = cpu_to_be32(lower_32_bits(dma_addr));
    rq_wqe++;
  }
...
u16 hinic_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, u16 nb_pkts) {
  ...
  while (pkts < nb_pkts) {
    ...
    /* vlan offload */
    rxm->ol_flags |= hinic_rx_vlan(offload_type, vlan_len, &rxm->vlan_tci);
    ...
    rxm->ol_flags |= hinic_rx_rss_hash(offload_type, rss_hash, &rxm->hash.rss);
