...
static const char *const valid_arguments[] = {ETH_KNI_NO_REQUEST_THREAD_ARG,
                                              NULL};
...
#define PMD_LOG(level, fmt, args...)                                           \
  rte_log(RTE_LOG_##level, eth_kni_logtype, "%s(): " fmt "\n", __func__, ##args)
static uint16_t eth_kni_rx(void *q, struct rte_mbuf **bufs, uint16_t nb_bufs) {
...
static uint16_t eth_kni_tx(void *q, struct rte_mbuf **bufs, uint16_t nb_bufs) {
  struct pmd_queue *kni_q = q;
  struct rte_kni *kni = kni_q->internals->kni;
  uint16_t nb_pkts;
...
static void *kni_handle_request(void *param) {
...
static int eth_kni_start(struct rte_eth_dev *dev) {
  ...
  conf.mbuf_size = rte_pktmbuf_data_room_size(mb_pool) - RTE_PKTMBUF_HEADROOM;
  conf.mtu = KNI_ETHER_MTU(conf.mbuf_size);

  internals->kni = rte_kni_alloc(mb_pool, &conf, NULL);
  if (internals->kni == NULL) {
    PMD_LOG(ERR, "Fail to create kni interface for port: %d", port_id);
    return -1;
  }
...
static int eth_kni_dev_start(struct rte_eth_dev *dev) {
  struct pmd_internals *internals = dev->data->dev_private;
  int ret;
  ...
  if (internals->no_request_thread == 0) {
    ...
    ret = rte_ctrl_thread_create(&internals->thread, "kni_handle_req", NULL,
                                 kni_handle_request, internals);
    if (ret) {
      PMD_LOG(ERR, "Fail to create kni request thread");
      return -1;
    }
  }
...
static int eth_kni_dev_stop(struct rte_eth_dev *dev) {
  struct pmd_internals *internals = dev->data->dev_private;
  int ret;
...
static int eth_kni_close(struct rte_eth_dev *eth_dev) {
  struct pmd_internals *internals;
  int ret;
  ...
  if (ret)
    PMD_LOG(WARNING, "Not able to stop kni for %s", eth_dev->data->name);
  ...
  if (ret)
    PMD_LOG(WARNING, "Not able to release kni for %s", eth_dev->data->name);

  return ret;
}

static int eth_kni_dev_configure(struct rte_eth_dev *dev __rte_unused) {
  return 0;
}

static int eth_kni_dev_info(struct rte_eth_dev *dev __rte_unused,
...
static int
eth_kni_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
...
static int
eth_kni_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id,
...
static int eth_kni_link_update(struct rte_eth_dev *dev __rte_unused,
                               int wait_to_complete __rte_unused) {
  return 0;
}

static int eth_kni_stats_get(struct rte_eth_dev *dev,
  ...
  num_stats =
      RTE_MIN((unsigned int)RTE_ETHDEV_QUEUE_STAT_CNTRS, data->nb_rx_queues);
  ...
  num_stats =
      RTE_MIN((unsigned int)RTE_ETHDEV_QUEUE_STAT_CNTRS, data->nb_tx_queues);
  for (i = 0; i < num_stats; i++) {
    q = data->tx_queues[i];
...
static int eth_kni_stats_reset(struct rte_eth_dev *dev) {
  struct rte_eth_dev_data *data = dev->data;
  struct pmd_queue *q;
  unsigned int i;
...
static struct rte_eth_dev *eth_kni_create(struct rte_vdev_device *vdev,
  ...
  PMD_LOG(INFO, "Creating kni ethdev on numa socket %u", numa_node);
...
static int eth_kni_kvargs_process(struct eth_kni_args *args,
                                  const char *params) {
...
static int eth_kni_probe(struct rte_vdev_device *vdev) {
  struct rte_eth_dev *eth_dev;
  struct eth_kni_args args;
  const char *name;
  const char *params;
...
static int eth_kni_remove(struct rte_vdev_device *vdev) {
  struct rte_eth_dev *eth_dev;
  const char *name;
  int ret;
