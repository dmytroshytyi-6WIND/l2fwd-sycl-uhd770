...
#else
#define VIRTIO_DUMP_PACKET(m, len)                                             \
  do {                                                                         \
  } while (0)
#endif
...
static inline void virtio_rx_stats_updated(struct virtnet_rx *rxvq,
                                           struct rte_mbuf *m) {
...
static uint16_t virtqueue_dequeue_rx_inorder(struct virtqueue *vq,
                                             struct rte_mbuf **rx_pkts,
                                             uint32_t *len, uint16_t num) {
  struct vring_used_elem *uep;
  struct rte_mbuf *cookie;
...
static inline int virtqueue_enqueue_refill_inorder(struct virtqueue *vq,
  ...
  while (i < num) {
    ...
    start_dp[idx].len =
        cookies[i]->buf_len - RTE_PKTMBUF_HEADROOM + hw->vtnet_hdr_size;
...
static inline int virtqueue_enqueue_recv_refill(struct virtqueue *vq,
                                                struct rte_mbuf **cookie,
                                                uint16_t num) {
  struct vq_desc_extra *dxp;
  struct virtio_hw *hw = vq->hw;
  ...
  for (i = 0; i < num; i++) {
    ...
    start_dp[idx].addr = VIRTIO_MBUF_ADDR(cookie[i], vq) +
                         RTE_PKTMBUF_HEADROOM - hw->vtnet_hdr_size;
    start_dp[idx].len =
        cookie[i]->buf_len - RTE_PKTMBUF_HEADROOM + hw->vtnet_hdr_size;
    start_dp[idx].flags = VRING_DESC_F_WRITE;
...
static inline void virtqueue_refill_single_packed(struct virtqueue *vq,
...
static inline int virtqueue_enqueue_recv_refill_packed_init(
...
static inline int virtqueue_enqueue_recv_refill_packed(struct virtqueue *vq,
                                                       struct rte_mbuf **cookie,
                                                       uint16_t num) {
  struct vring_packed_desc *start_dp = vq->vq_packed.ring.desc;
  struct vq_desc_extra *dxp;
 ...
 * expected.
 */
static void virtio_tso_fix_cksum(struct rte_mbuf *m) {
  /* common case: header is not fragmented */
  if (likely(rte_pktmbuf_data_len(m) >= m->l2_len + m->l3_len + m->l4_len)) {
    ...
    if ((iph->version_ihl >> 4) == 4) {
      ...
      ip_paylen = rte_cpu_to_be_16(rte_be_to_cpu_16(ip_len) - m->l3_len);
    } else {
...
static inline void virtqueue_enqueue_xmit_inorder(struct virtnet_tx *txvq,
                                                  struct rte_mbuf **cookies,
                                                  uint16_t num) {
  struct vq_desc_extra *dxp;
  ...
  while (i < num) {
    ...
    hdr = rte_pktmbuf_mtod_offset(cookies[i], struct virtio_net_hdr *,
                                  -head_size);
...
static inline void virtqueue_enqueue_xmit_packed_fast(struct virtnet_tx *txvq,
  ...
  hdr = rte_pktmbuf_mtod_offset(cookie, struct virtio_net_hdr *, -head_size);
...
static inline void virtqueue_enqueue_xmit(struct virtnet_tx *txvq,
                                          struct rte_mbuf *cookie,
                                          uint16_t needed, int use_indirect,
                                          int can_push, int in_order) {
  ...
  if (can_push) {
     ...
     */
    start_dp[idx].addr =
        txvq->virtio_net_hdr_mem + RTE_PTR_DIFF(&txr[idx].tx_indir, txr);
     ...
     */
    start_dp[idx].addr =
        txvq->virtio_net_hdr_mem + RTE_PTR_DIFF(&txr[idx].tx_hdr, txr);
...
int virtio_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                              uint16_t nb_desc,
  ...
  if (!virtio_rx_check_scatter(hw->max_rx_pkt_len, buf_size, hw->rx_ol_scatter,
                               &error)) {
    PMD_INIT_LOG(ERR, "RxQ %u Rx scatter check failed: %s", queue_idx, error);
    return -EINVAL;
  }
...
int virtio_dev_rx_queue_setup_finish(struct rte_eth_dev *dev,
  ...
  if (hw->use_vec_rx && !virtio_with_packed_queue(hw)) {
    for (desc_idx = 0; desc_idx < vq->vq_nentries; desc_idx++) {
      ...
      vq->vq_split.ring.desc[desc_idx].flags = VRING_DESC_F_WRITE;
    }
  ...
  if (hw->use_vec_rx && !virtio_with_packed_queue(hw)) {
    ...
    if ((!virtqueue_full(vq))) {
      ...
      if (!rte_pktmbuf_alloc_bulk(rxvq->mpool, pkts, free_cnt)) {
        error = virtqueue_enqueue_refill_inorder(vq, pkts, free_cnt);
        if (unlikely(error)) {
    ...
    while (!virtqueue_full(vq)) {
      ...
      if (virtio_with_packed_queue(vq->hw))
        error = virtqueue_enqueue_recv_refill_packed_init(vq, &m, 1);
      else
 ...
 */
int virtio_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                              uint16_t nb_desc,
                              unsigned int socket_id __rte_unused,
  ...
  if (tx_free_thresh >= (vq->vq_nentries - 3)) {
    PMD_DRV_LOG(ERR,
                ...
                vq->vq_nentries - 3, tx_free_thresh, dev->data->port_id,
                queue_idx);
    return -EINVAL;
  }
...
static inline void virtio_discard_rxbuf(struct virtqueue *vq,
                                        struct rte_mbuf *m) {
  int error;
  /*
...
static inline void virtio_discard_rxbuf_inorder(struct virtqueue *vq,
                                                struct rte_mbuf *m) {
  int error;
...
static inline int virtio_rx_offload(struct rte_mbuf *m,
                                    struct virtio_net_hdr *hdr) {
...
uint16_t virtio_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  if (likely(!virtqueue_full(vq))) {
    ...
    if (likely(rte_pktmbuf_alloc_bulk(rxvq->mpool, new_pkts, free_cnt) == 0)) {
      error = virtqueue_enqueue_recv_refill(vq, new_pkts, free_cnt);
      if (unlikely(error)) {
        for (i = 0; i < free_cnt; i++)
...
uint16_t virtio_recv_pkts_inorder(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  for (i = 0; i < num; i++) {
    ...
    if (vq->hw->has_rx_offload && virtio_rx_offload(rxm, &header->hdr) < 0) {
      virtio_discard_rxbuf_inorder(vq, rxm);
      rxvq->stats.errors++;
      continue;
    }
  ...
  while (seg_res != 0) {
    uint16_t rcv_cnt = RTE_MIN((uint16_t)seg_res, VIRTIO_MBUF_BURST_SZ);

    if (likely(virtqueue_nused(vq) >= rcv_cnt)) {
      num = virtqueue_dequeue_rx_inorder(vq, rcv_pkts, len, rcv_cnt);
  ...
  if (likely(!virtqueue_full(vq))) {
    ...
    if (!rte_pktmbuf_alloc_bulk(rxvq->mpool, new_pkts, free_cnt)) {
      error = virtqueue_enqueue_refill_inorder(vq, new_pkts, free_cnt);
      if (unlikely(error)) {
        for (i = 0; i < free_cnt; i++)
          rte_pktmbuf_free(new_pkts[i]);
      }
...
uint16_t virtio_recv_mergeable_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  for (i = 0; i < num; i++) {
    ...
    if (hw->has_rx_offload && virtio_rx_offload(rxm, &header->hdr) < 0) {
      virtio_discard_rxbuf(vq, rxm);
      rxvq->stats.errors++;
      continue;
    }
  ...
  while (seg_res != 0) {
    ...
    if (likely(virtqueue_nused(vq) >= rcv_cnt)) {
      num = virtqueue_dequeue_burst_rx(vq, rcv_pkts, len, rcv_cnt);
      uint16_t extra_idx = 0;
...
uint16_t virtio_xmit_pkts_prepare(void *tx_queue __rte_unused,
...
uint16_t virtio_xmit_pkts_packed(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (can_push)
      ...
      virtqueue_enqueue_xmit_packed(txvq, txm, slots, use_indirect, 0,
                                    in_order);
...
uint16_t virtio_xmit_pkts_inorder(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (nb_inorder_pkts) {
      ...
      virtqueue_enqueue_xmit_inorder(txvq, inorder_pkts, nb_inorder_pkts);
      nb_inorder_pkts = 0;
    }
...
__rte_weak uint16_t virtio_recv_pkts_packed_vec(
    void *rx_queue __rte_unused, struct rte_mbuf **rx_pkts __rte_unused,
    uint16_t nb_pkts __rte_unused) {
  return 0;
}

__rte_weak uint16_t virtio_xmit_pkts_packed_vec(
    void *tx_queue __rte_unused, struct rte_mbuf **tx_pkts __rte_unused,
    uint16_t nb_pkts __rte_unused) {
  return 0;
}
