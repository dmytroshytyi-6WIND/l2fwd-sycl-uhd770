 ...
 */
static int virtio_remap_pci(struct rte_pci_device *pci_dev,
...
static int eth_virtio_pci_init(struct rte_eth_dev *eth_dev) {
...
static int eth_virtio_pci_uninit(struct rte_eth_dev *eth_dev) {
  int ret;
  PMD_INIT_FUNC_TRACE();
...
static int vdpa_check_handler(__rte_unused const char *key, const char *value,
                              void *ret_val) {
  if (strcmp(value, "1") == 0)
    *(int *)ret_val = 1;
  else
...
static int virtio_pci_devargs_parse(struct rte_devargs *devargs, int *vdpa) {
  ...
  if (rte_kvargs_count(kvlist, VIRTIO_ARG_VDPA) == 1) {
     ...
     * vdpa=1
     */
    ret = rte_kvargs_process(kvlist, VIRTIO_ARG_VDPA, vdpa_check_handler, vdpa);
    if (ret < 0)
