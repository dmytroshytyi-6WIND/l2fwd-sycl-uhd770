...
virtqueue_enqueue_batch_packed_vec(struct virtnet_tx *txvq,
  ...
  uint8x16_t ref_seg_msk = {2,  3,  4,  5,  10, 11, 12, 13,
                            18, 19, 20, 21, 26, 27, 28, 29};

  /* Map four data_off from mbufs to one NEON register. */
  uint8x8_t data_msk = {0, 1, 8, 9, 16, 17, 24, 25};

  uint16x8_t net_hdr_msk = {0xFFFF, 0xFFFF, 0, 0, 0, 0};
...
virtqueue_dequeue_batch_packed_vec(struct virtnet_rx *rxvq,
  ...
  uint64x2_t desc_stats =
      vreinterpretq_u64_u32(~vceqq_u32(v_flag, v_used_flag));
  ...
  if (hw->has_rx_offload) {
    virtio_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) {
      char *addr =
          (char *)rx_pkts[i]->buf_addr + RTE_PKTMBUF_HEADROOM - head_size;
