 ...
 *     barriers.
 */
static inline void virtio_mb(uint8_t weak_barriers) {
  if (weak_barriers)
...
static inline void virtio_rmb(uint8_t weak_barriers) {
  if (weak_barriers)
...
static inline void virtio_wmb(uint8_t weak_barriers) {
  if (weak_barriers)
...
struct virtio_tx_region {
  ...
  union {
    ...
    struct vring_packed_desc tx_packed_indir[VIRTIO_MAX_TX_INDIRECT];
  } __rte_aligned(16);
};

static inline int desc_is_used(struct vring_packed_desc *desc,
                               struct virtqueue *vq) {
...
static inline void vring_desc_init_packed(struct virtqueue *vq, int n) {
  int i;
  for (i = 0; i < n - 1; i++) {
...
static inline void vring_desc_init_split(struct vring_desc *dp, uint16_t n) {
  uint16_t i;
...
static inline void vring_desc_init_indirect_packed(struct vring_packed_desc *dp,
                                                   int n) {
  int i;
  for (i = 0; i < n; i++) {
    dp[i].id = (uint16_t)i;
 ...
 */
static inline void virtqueue_disable_intr_packed(struct virtqueue *vq) {
 ...
 */
static inline void virtqueue_disable_intr_split(struct virtqueue *vq) {
 ...
 */
static inline void virtqueue_enable_intr_packed(struct virtqueue *vq) {
  if (vq->vq_packed.event_flags_shadow == RING_EVENT_FLAGS_DISABLE) {
 ...
 */
static inline void virtqueue_enable_intr_split(struct virtqueue *vq) {
  vq->vq_split.ring.avail->flags &= (~VRING_AVAIL_F_NO_INTERRUPT);
}
...
static inline int virtqueue_full(const struct virtqueue *vq) {
  return vq->vq_free_cnt == 0;
}

static inline int virtio_get_queue_type(struct virtio_hw *hw, uint16_t vq_idx) {
...
static inline uint16_t virtqueue_nused(const struct virtqueue *vq) {
  uint16_t idx;
  ...
  if (vq->hw->weak_barriers) {
    ...
    rte_smp_rmb();
#else
    idx = __atomic_load_n(&(vq)->vq_split.ring.used->idx, __ATOMIC_ACQUIRE);
#endif
  } else {
    idx = vq->vq_split.ring.used->idx;
    rte_io_rmb();
...
#else
    __atomic_store_n(&vq->vq_split.ring.avail->idx, vq->vq_avail_idx,
                     __ATOMIC_RELEASE);
#endif
  } else {
    rte_io_wmb();
...
static inline void vq_update_avail_ring(struct virtqueue *vq,
                                        uint16_t desc_idx) {
  uint16_t avail_idx;
  /*
...
static inline int virtqueue_kick_prepare_packed(struct virtqueue *vq) {
  uint16_t flags;
...
#define virtqueue_clear_net_hdr(hdr)                                           \
  do {                                                                         \
    typeof(hdr) hdr_ = (hdr);                                                  \
...
static inline void virtqueue_xmit_offload(struct virtio_net_hdr *hdr,
  ...
  if (cookie->ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    hdr->hdr_len = o_l23_len + cookie->l2_len + cookie->l3_len + cookie->l4_len;
  } else {
...
static void virtio_xmit_cleanup_inorder_packed(struct virtqueue *vq,
                                               uint16_t num) {
...
static void virtio_xmit_cleanup_normal_packed(struct virtqueue *vq,
                                              uint16_t num) {
...
static inline void virtio_xmit_cleanup_packed(struct virtqueue *vq,
                                              uint16_t num, int in_order) {
  if (in_order)
...
static inline void virtio_xmit_cleanup(struct virtqueue *vq, uint16_t num) {
