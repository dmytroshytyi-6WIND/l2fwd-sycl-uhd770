...
uint16_t virtio_xmit_pkts_packed_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  while (remained) {
    if (remained >= PACKED_BATCH_SIZE) {
      if (!virtqueue_enqueue_batch_packed_vec(txvq, &tx_pkts[nb_tx])) {
        nb_tx += PACKED_BATCH_SIZE;
        remained -= PACKED_BATCH_SIZE;
        continue;
      }
    }
    if (!virtqueue_enqueue_single_packed_vec(txvq, tx_pkts[nb_tx])) {
      nb_tx++;
      remained--;
      continue;
    }
...
uint16_t virtio_recv_pkts_packed_vec(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (num) {
    if (num >= PACKED_BATCH_SIZE) {
      if (!virtqueue_dequeue_batch_packed_vec(rxvq, &rx_pkts[nb_rx])) {
        nb_rx += PACKED_BATCH_SIZE;
        num -= PACKED_BATCH_SIZE;
        continue;
      }
    }
    if (!virtqueue_dequeue_single_packed_vec(rxvq, &rx_pkts[nb_rx])) {
      nb_rx++;
      num--;
      continue;
    }
  ...
  if (likely(vq->vq_free_cnt >= free_cnt)) {
    ...
    if (likely(rte_pktmbuf_alloc_bulk(rxvq->mpool, new_pkts, free_cnt) == 0)) {
      virtio_recv_refill_packed_vec(rxvq, new_pkts, free_cnt);
