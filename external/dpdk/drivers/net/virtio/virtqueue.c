...
struct rte_mbuf *virtqueue_detach_unused(struct virtqueue *vq) {
  ...
  for (idx = 0; idx < vq->vq_nentries; idx++) {
    if (hw->use_vec_rx && !virtio_with_packed_queue(hw) && type == VTNET_RQ) {
...
/* Flush used descs */
static void virtqueue_rxvq_flush_packed(struct virtqueue *vq) {
...
static void virtqueue_rxvq_flush_split(struct virtqueue *vq) {
...
int virtqueue_txvq_reset_packed(struct virtqueue *vq) {
  ...
  for (desc_idx = 0; desc_idx < vq->vq_nentries; desc_idx++) {
    ...
    if (virtio_with_feature(vq->hw, VIRTIO_RING_F_INDIRECT_DESC)) {
      ...
      start_dp->addr = txvq->virtio_net_hdr_mem + desc_idx * sizeof(*txr) +
