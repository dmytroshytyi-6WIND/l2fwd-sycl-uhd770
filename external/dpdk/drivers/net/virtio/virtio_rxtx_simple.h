...
static inline void virtio_rxq_rearm_vec(struct virtnet_rx *rxvq) {
  int i;
  uint16_t desc_idx;
  ...
  for (i = 0; i < RTE_VIRTIO_VPMD_RX_REARM_THRESH; i++) {
    ...
    *(uint64_t *)p = rxvq->mbuf_initializer;
    ...
    start_dp[i].addr = VIRTIO_MBUF_ADDR(sw_ring[i], vq) + RTE_PKTMBUF_HEADROOM -
                       vq->hw->vtnet_hdr_size;
    start_dp[i].len =
        sw_ring[i]->buf_len - RTE_PKTMBUF_HEADROOM + vq->hw->vtnet_hdr_size;
  }
