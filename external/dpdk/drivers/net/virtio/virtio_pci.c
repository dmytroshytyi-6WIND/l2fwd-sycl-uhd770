...
static inline int check_vq_phys_addr_ok(struct virtqueue *vq) {
...
static enum virtio_msix_status vtpci_msix_detect(struct rte_pci_device *dev) {
  uint8_t pos;
  int ret;
 ...
 */
static void legacy_read_dev_config(struct virtio_hw *hw, size_t offset,
                                   void *dst, int length) {
...
static void legacy_write_dev_config(struct virtio_hw *hw, size_t offset,
...
static uint64_t legacy_get_features(struct virtio_hw *hw) {
  uint32_t dst;
...
static void legacy_set_features(struct virtio_hw *hw, uint64_t features) {
  if ((features >> 32) != 0) {
    PMD_DRV_LOG(ERR, "only 32 bit features are allowed for legacy virtio!");
    return;
  }
  rte_pci_ioport_write(VTPCI_IO(hw), &features, 4, VIRTIO_PCI_GUEST_FEATURES);
}

static int legacy_features_ok(struct virtio_hw *hw __rte_unused) { return 0; }

static uint8_t legacy_get_status(struct virtio_hw *hw) {
  uint8_t dst;
...
static void legacy_set_status(struct virtio_hw *hw, uint8_t status) {
  rte_pci_ioport_write(VTPCI_IO(hw), &status, 1, VIRTIO_PCI_STATUS);
}

static uint8_t legacy_get_isr(struct virtio_hw *hw) {
  uint8_t dst;
...
static uint16_t legacy_set_config_irq(struct virtio_hw *hw, uint16_t vec) {
  uint16_t dst;
...
static uint16_t legacy_set_queue_irq(struct virtio_hw *hw, struct virtqueue *vq,
                                     uint16_t vec) {
  uint16_t dst;
...
static uint16_t legacy_get_queue_num(struct virtio_hw *hw, uint16_t queue_id) {
  uint16_t dst;
...
static int legacy_setup_queue(struct virtio_hw *hw, struct virtqueue *vq) {
  uint32_t src;
...
static void legacy_del_queue(struct virtio_hw *hw, struct virtqueue *vq) {
  uint32_t src = 0;
...
static void legacy_notify_queue(struct virtio_hw *hw, struct virtqueue *vq) {
  rte_pci_ioport_write(VTPCI_IO(hw), &vq->vq_queue_index, 2,
                       VIRTIO_PCI_QUEUE_NOTIFY);
}

static void legacy_intr_detect(struct virtio_hw *hw) {
  struct virtio_pci_dev *dev = virtio_pci_get_dev(hw);
...
static int legacy_dev_close(struct virtio_hw *hw) {
...
static inline void io_write64_twopart(uint64_t val, uint32_t *lo,
                                      uint32_t *hi) {
...
static void modern_set_features(struct virtio_hw *hw, uint64_t features) {
  ...
  rte_write32(features & ((1ULL << 32) - 1), &dev->common_cfg->guest_feature);

  rte_write32(1, &dev->common_cfg->guest_feature_select);
  rte_write32(features >> 32, &dev->common_cfg->guest_feature);
}

static int modern_features_ok(struct virtio_hw *hw) {
...
static uint16_t modern_set_queue_irq(struct virtio_hw *hw, struct virtqueue *vq,
                                     uint16_t vec) {
  struct virtio_pci_dev *dev = virtio_pci_get_dev(hw);
...
static int modern_setup_queue(struct virtio_hw *hw, struct virtqueue *vq) {
  ...
  PMD_INIT_LOG(DEBUG, "\t notify addr: %p (notify offset: %u)", vq->notify_addr,
               notify_off);
...
static void modern_notify_queue(struct virtio_hw *hw, struct virtqueue *vq) {
  ...
  if (virtio_with_packed_queue(hw)) {
     ...
     */
    notify_data =
        ((uint32_t)(!!(vq->vq_packed.cached_flags & VRING_PACKED_DESC_F_AVAIL))
         << 31) |
        ((uint32_t)vq->vq_avail_idx << 16) | vq->vq_queue_index;
  } else {
    /*
     * Bit[0:15]: vq queue index
...
static void modern_intr_detect(struct virtio_hw *hw) {
  struct virtio_pci_dev *dev = virtio_pci_get_dev(hw);
...
static int modern_dev_close(struct virtio_hw *hw) {
  rte_pci_unmap_device(VTPCI_DEV(hw));
...
static void *get_cfg_addr(struct rte_pci_device *dev,
...
static int virtio_read_caps(struct rte_pci_device *pci_dev,
                            struct virtio_hw *hw) {
  struct virtio_pci_dev *dev = virtio_pci_get_dev(hw);
  uint8_t pos;
  struct virtio_pci_cap cap;
  ...
  while (pos) {
    ...
    if (cap.cap_vndr == PCI_CAP_ID_MSIX) {
      ...
      ret = rte_pci_read_config(pci_dev, &flags, sizeof(flags), pos + 2);
      if (ret != sizeof(flags)) {
        PMD_INIT_LOG(DEBUG,
                     "failed to read pci cap at pos:"
                     " %x ret %d",
    ...
    if (cap.cap_vndr != PCI_CAP_ID_VNDR) {
      PMD_INIT_LOG(DEBUG, "[%2x] skipping non VNDR cap id: %02x", pos,
                   cap.cap_vndr);
      goto next;
    }
    ...
    case VIRTIO_PCI_CAP_NOTIFY_CFG:
      ret = rte_pci_read_config(pci_dev, &dev->notify_off_multiplier, 4,
                                pos + sizeof(cap));
      if (ret != 4)
        PMD_INIT_LOG(DEBUG, "failed to read notify_off_multiplier, ret %d",
                     ret);
      else
  ...
  if (rte_pci_ioport_map(pci_dev, 0, VTPCI_IO(hw)) < 0) {
    ...
    if (pci_dev->kdrv == RTE_PCI_KDRV_UNKNOWN &&
        (!pci_dev->device.devargs ||
         pci_dev->device.devargs->bus != rte_bus_find_by_name("pci"))) {
