...
uint16_t virtio_recv_pkts_vec(void *rx_queue, struct rte_mbuf **rx_pkts,
   ...
   */
  const vector unsigned short len_adjust = {0,
                                            0,
  ...
  for (nb_pkts_received = 0; nb_pkts_received < nb_total;) {
    ...
    *(vector unsigned char *)&rx_pkts[6] = mbp[3];
    ...
    pkt_mb[0] =
        (vector unsigned char)((vector unsigned short)pkt_mb[0] + len_adjust);
    pkt_mb[1] =
        (vector unsigned char)((vector unsigned short)pkt_mb[1] + len_adjust);
    *(vector unsigned char *)&rx_pkts[0]->rx_descriptor_fields1 = pkt_mb[0];
    *(vector unsigned char *)&rx_pkts[1]->rx_descriptor_fields1 = pkt_mb[1];

    pkt_mb[2] = vec_perm(desc[1], zero, shuf_msk1);
    pkt_mb[3] = vec_perm(desc[1], zero, shuf_msk2);
    pkt_mb[2] =
        (vector unsigned char)((vector unsigned short)pkt_mb[2] + len_adjust);
    pkt_mb[3] =
        (vector unsigned char)((vector unsigned short)pkt_mb[3] + len_adjust);
    *(vector unsigned char *)&rx_pkts[2]->rx_descriptor_fields1 = pkt_mb[2];
    *(vector unsigned char *)&rx_pkts[3]->rx_descriptor_fields1 = pkt_mb[3];

    pkt_mb[4] = vec_perm(desc[2], zero, shuf_msk1);
    pkt_mb[5] = vec_perm(desc[2], zero, shuf_msk2);
    pkt_mb[4] =
        (vector unsigned char)((vector unsigned short)pkt_mb[4] + len_adjust);
    pkt_mb[5] =
        (vector unsigned char)((vector unsigned short)pkt_mb[5] + len_adjust);
    *(vector unsigned char *)&rx_pkts[4]->rx_descriptor_fields1 = pkt_mb[4];
    *(vector unsigned char *)&rx_pkts[5]->rx_descriptor_fields1 = pkt_mb[5];

    pkt_mb[6] = vec_perm(desc[3], zero, shuf_msk1);
    pkt_mb[7] = vec_perm(desc[3], zero, shuf_msk2);
    pkt_mb[6] =
        (vector unsigned char)((vector unsigned short)pkt_mb[6] + len_adjust);
    pkt_mb[7] =
        (vector unsigned char)((vector unsigned short)pkt_mb[7] + len_adjust);
    *(vector unsigned char *)&rx_pkts[6]->rx_descriptor_fields1 = pkt_mb[6];
    *(vector unsigned char *)&rx_pkts[7]->rx_descriptor_fields1 = pkt_mb[7];
