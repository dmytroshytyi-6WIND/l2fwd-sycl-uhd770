...
struct virtio_ops {
  ...
  void (*write_dev_cfg)(struct virtio_hw *hw, size_t offset, const void *src,
                        int len);
...
static inline int virtio_with_feature(struct virtio_hw *hw, uint64_t bit) {
  return (hw->guest_features & (1ULL << bit)) != 0;
}

static inline int virtio_with_packed_queue(struct virtio_hw *hw) {
  return virtio_with_feature(hw, VIRTIO_F_RING_PACKED);
}

uint64_t virtio_negotiate_features(struct virtio_hw *hw,
                                   uint64_t host_features);
uint8_t virtio_get_status(struct virtio_hw *hw);
void virtio_set_status(struct virtio_hw *hw, uint8_t status);
void virtio_write_dev_config(struct virtio_hw *hw, size_t offset,
