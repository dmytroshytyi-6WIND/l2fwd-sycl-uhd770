...
static inline size_t vring_size(struct virtio_hw *hw, unsigned int num,
                                unsigned long align) {
  size_t size;
...
static inline void vring_init_split(struct vring *vr, uint8_t *p,
...
static inline void vring_init_packed(struct vring_packed *vr, uint8_t *p,
  ...
  vr->driver =
      (struct vring_packed_desc_event *)(p +
                                         vr->num *
                                             sizeof(struct vring_packed_desc));
  vr->device = (struct vring_packed_desc_event *)RTE_ALIGN_CEIL(
      ((uintptr_t)vr->driver + sizeof(struct vring_packed_desc_event)), align);
}
