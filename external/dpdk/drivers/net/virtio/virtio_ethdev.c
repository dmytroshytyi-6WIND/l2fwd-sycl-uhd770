...
static int virtio_dev_rss_hash_conf_get(struct rte_eth_dev *dev,
                                        struct rte_eth_rss_conf *rss_conf);
static int
virtio_dev_rss_reta_update(struct rte_eth_dev *dev,
...
static int virtio_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                  int on);
...
static int virtio_dev_queue_stats_mapping_set(struct rte_eth_dev *eth_dev,
                                              uint16_t queue_id,
...
#define VIRTIO_NB_RXQ_XSTATS                                                   \
  (sizeof(rte_virtio_rxq_stat_strings) / sizeof(rte_virtio_rxq_stat_strings[0]))
#define VIRTIO_NB_TXQ_XSTATS                                                   \
  (sizeof(rte_virtio_txq_stat_strings) / sizeof(rte_virtio_txq_stat_strings[0]))

struct virtio_hw_internal virtio_hw_internal[RTE_MAX_ETHPORTS];

static struct virtio_pmd_ctrl *
virtio_send_command_packed(struct virtnet_ctl *cvq,
                           struct virtio_pmd_ctrl *ctrl, int *dlen,
                           int pkt_num) {
  ...
  for (k = 0; k < pkt_num; k++) {
    ...
    desc[vq->vq_avail_idx].flags =
        VRING_DESC_F_NEXT | vq->vq_packed.cached_flags;
    sum += dlen[k];
    vq->vq_free_cnt--;
    nb_descs++;
    if (++vq->vq_avail_idx >= vq->vq_nentries) {
  ...
  desc[vq->vq_avail_idx].addr =
      cvq->virtio_net_hdr_mem + sizeof(struct virtio_net_ctrl_hdr);
  desc[vq->vq_avail_idx].len = sizeof(ctrl->status);
  desc[vq->vq_avail_idx].flags =
      VRING_DESC_F_WRITE | vq->vq_packed.cached_flags;
  vq->vq_free_cnt--;
  nb_descs++;
  if (++vq->vq_avail_idx >= vq->vq_nentries) {
    vq->vq_avail_idx -= vq->vq_nentries;
  ...
  PMD_INIT_LOG(DEBUG,
               ...
               vq->vq_free_cnt, vq->vq_avail_idx, vq->vq_used_cons_idx,
               vq->vq_packed.cached_flags, vq->vq_packed.used_wrap_counter);
...
static struct virtio_pmd_ctrl *
virtio_send_command_split(struct virtnet_ctl *cvq, struct virtio_pmd_ctrl *ctrl,
  ...
  while (virtqueue_nused(vq)) {
    ...
    while (vq->vq_split.ring.desc[desc_idx].flags & VRING_DESC_F_NEXT) {
...
static int virtio_send_command(struct virtnet_ctl *cvq,
                               struct virtio_pmd_ctrl *ctrl, int *dlen,
                               int pkt_num) {
  ...
  PMD_INIT_LOG(DEBUG,
               "vq->vq_desc_head_idx = %d, status = %d, "
               "vq->hw->cvq = %p vq = %p",
  ...
  memcpy(cvq->virtio_net_hdr_mz->addr, ctrl, sizeof(struct virtio_pmd_ctrl));
...
static int virtio_set_multiple_queues_rss(struct rte_eth_dev *dev,
                                          uint16_t nb_queues) {
...
static int virtio_set_multiple_queues_auto(struct rte_eth_dev *dev,
                                           uint16_t nb_queues) {
  struct virtio_hw *hw = dev->data->dev_private;
  struct virtio_pmd_ctrl ctrl;
  int dlen;
...
static int virtio_set_multiple_queues(struct rte_eth_dev *dev,
                                      uint16_t nb_queues) {
  struct virtio_hw *hw = dev->data->dev_private;
...
static void virtio_init_vring(struct virtqueue *vq) {
  ...
  if (virtio_with_packed_queue(vq->hw)) {
    vring_init_packed(&vq->vq_packed.ring, ring_mem, VIRTIO_VRING_ALIGN, size);
...
static int virtio_init_queue(struct rte_eth_dev *dev, uint16_t queue_idx) {
  ...
  PMD_INIT_LOG(INFO, "setting up queue: %u on NUMA node %d", queue_idx,
               numa_node);
  ...
  mz = rte_memzone_reserve_aligned(vq_name, vq->vq_ring_size, numa_node,
  ...
  if (sz_hdr_mz) {
    ...
    hdr_mz = rte_memzone_reserve_aligned(vq_hdr_name, sz_hdr_mz, numa_node,
  ...
  if (queue_type == VTNET_RQ) {
    ...
    sw_ring =
        rte_zmalloc_socket("sw_ring", sz_sw, RTE_CACHE_LINE_SIZE, numa_node);
    if (!sw_ring) {
...
static void virtio_free_queues(struct virtio_hw *hw) {
...
static int virtio_alloc_queues(struct rte_eth_dev *dev) {
  struct virtio_hw *hw = dev->data->dev_private;
  uint16_t nr_vq = virtio_get_nr_vq(hw);
  uint16_t i;
  int ret;
...
static void virtio_free_rss(struct virtio_hw *hw) {
  rte_free(hw->rss_key);
  hw->rss_key = NULL;
...
static bool virtio_check_scatter_on_all_rx_queues(struct rte_eth_dev *dev,
                                                  uint16_t frame_size) {
  struct virtio_hw *hw = dev->data->dev_private;
  ...
  for (qidx = 0; qidx < hw->max_queue_pairs; qidx++) {
    ...
    if (!virtio_rx_check_scatter(frame_size, buf_size, hw->rx_ol_scatter,
                                 &error)) {
      PMD_INIT_LOG(ERR, "MTU check for RxQ %u failed: %s", qidx, error);
      return false;
    }
  }
...
static int virtio_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct virtio_hw *hw = dev->data->dev_private;
  uint32_t ether_hdr_len =
      RTE_ETHER_HDR_LEN + VLAN_TAG_LEN + hw->vtnet_hdr_size;
  ...
  if (mtu < RTE_ETHER_MIN_MTU || frame_size > max_frame_size) {
    PMD_INIT_LOG(ERR, "MTU should be between %d and %d", RTE_ETHER_MIN_MTU,
...
static int virtio_dev_rx_queue_intr_enable(struct rte_eth_dev *dev,
                                           uint16_t queue_id) {
  struct virtio_hw *hw = dev->data->dev_private;
...
static int virtio_dev_rx_queue_intr_disable(struct rte_eth_dev *dev,
                                            uint16_t queue_id) {
  struct virtnet_rx *rxvq = dev->data->rx_queues[queue_id];
  struct virtqueue *vq = virtnet_rxq_to_vq(rxvq);
...
static void virtio_update_stats(struct rte_eth_dev *dev,
                                struct rte_eth_stats *stats) {
  unsigned i;
  ...
  if (xstats_names != NULL) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
      ...
      for (t = 0; t < VIRTIO_NB_RXQ_XSTATS; t++) {
        snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
                 "rx_q%u_%s", i, rte_virtio_rxq_stat_strings[t].name);
        count++;
      }
    }
    ...
    for (i = 0; i < dev->data->nb_tx_queues; i++) {
      ...
      for (t = 0; t < VIRTIO_NB_TXQ_XSTATS; t++) {
        snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
                 "tx_q%u_%s", i, rte_virtio_txq_stat_strings[t].name);
        count++;
      }
    }
    return count;
...
static int virtio_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    for (t = 0; t < VIRTIO_NB_RXQ_XSTATS; t++) {
      xstats[count].value =
          *(uint64_t *)(((char *)rxvq) + rte_virtio_rxq_stat_strings[t].offset);
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    for (t = 0; t < VIRTIO_NB_TXQ_XSTATS; t++) {
      xstats[count].value =
          *(uint64_t *)(((char *)txvq) + rte_virtio_txq_stat_strings[t].offset);
      xstats[count].id = count;
      count++;
    }
  }
...
static int virtio_dev_stats_reset(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    memset(txvq->stats.size_bins, 0, sizeof(txvq->stats.size_bins[0]) * 8);
  }
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    memset(rxvq->stats.size_bins, 0, sizeof(rxvq->stats.size_bins[0]) * 8);
  }
...
static void virtio_set_hwaddr(struct virtio_hw *hw) {
  virtio_write_dev_config(hw, offsetof(struct virtio_net_config, mac),
                          &hw->mac_addr, RTE_ETHER_ADDR_LEN);
}

static void virtio_get_hwaddr(struct virtio_hw *hw) {
  if (virtio_with_feature(hw, VIRTIO_NET_F_MAC)) {
    virtio_read_dev_config(hw, offsetof(struct virtio_net_config, mac),
                           &hw->mac_addr, RTE_ETHER_ADDR_LEN);
  } else {
...
static int virtio_mac_table_set(struct virtio_hw *hw,
...
static int virtio_mac_addr_add(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < VIRTIO_MAX_MAC_ADDRS; i++) {
    const struct rte_ether_addr *addr = (i == index) ? mac_addr : addrs + i;
...
static int virtio_get_monitor_addr(void *rx_queue,
  ...
  if (virtio_with_packed_queue(hw)) {
    ...
    if (vq->vq_packed.used_wrap_counter)
      pmc->opaque[CLB_VAL_IDX] = VRING_PACKED_DESC_F_AVAIL_USED;
    else
    ...
    pmc->opaque[CLB_VAL_IDX] = vq->vq_used_cons_idx & (vq->vq_nentries - 1);
...
static int virtio_intr_unmask(struct rte_eth_dev *dev) {
  struct virtio_hw *hw = dev->data->dev_private;
...
static int virtio_ethdev_negotiate_features(struct virtio_hw *hw,
  ...
  if (host_features & req_features & (1ULL << VIRTIO_NET_F_MTU)) {
    ...
    virtio_read_dev_config(hw, offsetof(struct virtio_net_config, mtu),
...
void virtio_interrupt_handler(void *param) {
  ...
  if (isr & VIRTIO_ISR_CONFIG) {
    ...
    if (virtio_with_feature(hw, VIRTIO_NET_F_STATUS)) {
      virtio_read_dev_config(hw, offsetof(struct virtio_net_config, status),
...
static void set_rxtx_funcs(struct rte_eth_dev *eth_dev) {
  struct virtio_hw *hw = eth_dev->data->dev_private;

  eth_dev->tx_pkt_prepare = virtio_xmit_pkts_prepare;
  if (virtio_with_packed_queue(hw)) {
    PMD_INIT_LOG(INFO, "virtio: using packed ring %s Tx path on port %u",
  ...
  if (virtio_with_packed_queue(hw)) {
    if (hw->use_vec_rx) {
      PMD_INIT_LOG(INFO,
                   ...
                   eth_dev->data->port_id);
      eth_dev->rx_pkt_burst = &virtio_recv_pkts_packed_vec;
    } else if (virtio_with_feature(hw, VIRTIO_NET_F_MRG_RXBUF)) {
      PMD_INIT_LOG(
          INFO, "virtio: using packed ring mergeable buffer Rx path on port %u",
          eth_dev->data->port_id);
    ...
    if (hw->use_vec_rx) {
      ...
      PMD_INIT_LOG(INFO, "virtio: using inorder Rx path on port %u",
                   eth_dev->data->port_id);
      eth_dev->rx_pkt_burst = &virtio_recv_pkts_inorder;
    } else if (virtio_with_feature(hw, VIRTIO_NET_F_MRG_RXBUF)) {
      PMD_INIT_LOG(INFO, "virtio: using mergeable buffer Rx path on port %u",
                   eth_dev->data->port_id);
 ...
 */
static int virtio_queues_bind_intr(struct rte_eth_dev *dev) {
  uint32_t i;
  struct virtio_hw *hw = dev->data->dev_private;
...
static void virtio_queues_unbind_intr(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; ++i)
    VIRTIO_OPS(hw)->set_queue_irq(hw, hw->vqs[i * VTNET_CQ],
                                  VIRTIO_MSI_NO_VECTOR);
}

static int virtio_configure_intr(struct rte_eth_dev *dev) {
  struct virtio_hw *hw = dev->data->dev_private;
  ...
  if (rte_intr_vec_list_alloc(dev->intr_handle, "intr_vec",
                              hw->max_queue_pairs)) {
    PMD_INIT_LOG(ERR, "Failed to allocate %u rxq vectors", hw->max_queue_pairs);
    return -ENOMEM;
  }

  if (dev->data->dev_flags & RTE_ETH_DEV_INTR_LSC) {
    /* Re-register callback to update max_intr */
    rte_intr_callback_unregister(dev->intr_handle, virtio_interrupt_handler,
                                 dev);
    rte_intr_callback_register(dev->intr_handle, virtio_interrupt_handler, dev);
  }
...
static void virtio_get_speed_duplex(struct rte_eth_dev *eth_dev,
  ...
  config = &local_config;
  virtio_read_dev_config(hw, offsetof(struct virtio_net_config, speed),
                         &config->speed, sizeof(config->speed));
  virtio_read_dev_config(hw, offsetof(struct virtio_net_config, duplex),
  ...
  PMD_INIT_LOG(DEBUG, "link speed = %d, duplex = %d", hw->speed, hw->duplex);
}

static uint64_t ethdev_to_virtio_rss_offloads(uint64_t ethdev_hash_types) {
...
static uint64_t virtio_to_ethdev_rss_offloads(uint64_t virtio_hash_types) {
...
static int virtio_dev_get_rss_config(struct virtio_hw *hw,
                                     uint32_t *rss_hash_types) {
  struct virtio_net_config local_config;
  struct virtio_net_config *config = &local_config;

  virtio_read_dev_config(
      hw, offsetof(struct virtio_net_config, rss_max_key_size),
      &config->rss_max_key_size, sizeof(config->rss_max_key_size));
  ...
  virtio_read_dev_config(
      hw, offsetof(struct virtio_net_config, rss_max_indirection_table_length),
  ...
  virtio_read_dev_config(
      hw, offsetof(struct virtio_net_config, supported_hash_types),
      &config->supported_hash_types, sizeof(config->supported_hash_types));
  ...
  *rss_hash_types = config->supported_hash_types & VIRTIO_NET_HASH_TYPE_MASK;
  ...
  PMD_INIT_LOG(DEBUG, "\t-Max reta size: %u",
               config->rss_max_indirection_table_length);
...
static int virtio_dev_rss_init(struct rte_eth_dev *eth_dev) {
  struct virtio_hw *hw = eth_dev->data->dev_private;
  ...
  if (!hw->rss_key) {
    ...
    hw->rss_key = rte_malloc_socket("rss_key", VIRTIO_NET_RSS_KEY_SIZE, 0,
  ...
  if (!hw->rss_reta) {
    ...
    hw->rss_reta = rte_zmalloc_socket(
        "rss_reta", VIRTIO_NET_RSS_RETA_SIZE * sizeof(uint16_t), 0,
        eth_dev->device->numa_node);
    if (!hw->rss_reta) {
...
static int virtio_init_device(struct rte_eth_dev *eth_dev,
                              uint64_t req_features) {
  struct virtio_hw *hw = eth_dev->data->dev_private;
  struct virtio_net_config *config;
  struct virtio_net_config local_config;
  ...
  PMD_INIT_LOG(DEBUG, "PORT MAC: " RTE_ETHER_ADDR_PRT_FMT, hw->mac_addr[0],
               hw->mac_addr[1], hw->mac_addr[2], hw->mac_addr[3],
  ...
  if (virtio_with_feature(hw, VIRTIO_NET_F_CTRL_VQ)) {
    ...
    if (virtio_with_feature(hw, VIRTIO_NET_F_MQ) ||
      ...
      virtio_read_dev_config(
          ...
          &config->max_virtqueue_pairs, sizeof(config->max_virtqueue_pairs));
    } else {
      PMD_INIT_LOG(
          DEBUG, "Neither VIRTIO_NET_F_MQ nor VIRTIO_NET_F_RSS are supported");
    ...
    if (virtio_with_feature(hw, VIRTIO_NET_F_MTU)) {
       ...
       */
      if (config->mtu < RTE_ETHER_MIN_MTU) {
        PMD_INIT_LOG(ERR, "invalid max MTU value (%u)", config->mtu);
        return -1;
      }
      ...
      hw->max_mtu = VIRTIO_MAX_RX_PKTLEN - RTE_ETHER_HDR_LEN - VLAN_TAG_LEN -
                    hw->vtnet_hdr_size;
    }
    ...
    PMD_INIT_LOG(DEBUG, "PORT MAC: " RTE_ETHER_ADDR_PRT_FMT, config->mac[0],
...
int eth_virtio_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  if (sizeof(struct virtio_net_hdr_mrg_rxbuf) > RTE_PKTMBUF_HEADROOM) {
    PMD_INIT_LOG(ERR, "Not sufficient headroom required = %d, avail = %d",
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("virtio", VIRTIO_MAX_MAC_ADDRS * RTE_ETHER_ADDR_LEN, 0);
...
static int link_speed_handler(const char *key __rte_unused, const char *value,
                              void *ret_val) {
  uint32_t val;
  if (!value || !ret_val)
    return -EINVAL;
...
static int virtio_dev_devargs_parse(struct rte_devargs *devargs,
  ...
  if (speed && rte_kvargs_count(kvlist, VIRTIO_ARG_SPEED) == 1) {
    ret =
        rte_kvargs_process(kvlist, VIRTIO_ARG_SPEED, link_speed_handler, speed);
    if (ret < 0) {
      PMD_INIT_LOG(ERR, "Failed to parse %s", VIRTIO_ARG_SPEED);
      goto exit;
    }
  }

  if (vectorized && rte_kvargs_count(kvlist, VIRTIO_ARG_VECTORIZED) == 1) {
    ret = rte_kvargs_process(kvlist, VIRTIO_ARG_VECTORIZED,
                             vectorized_check_handler, vectorized);
    if (ret < 0) {
      PMD_INIT_LOG(ERR, "Failed to parse %s", VIRTIO_ARG_VECTORIZED);
      goto exit;
    }
  }
...
static uint8_t rx_offload_enabled(struct virtio_hw *hw) {
...
static uint8_t tx_offload_enabled(struct virtio_hw *hw) {
...
static int virtio_dev_configure(struct rte_eth_dev *dev) {
  ...
  if ((rx_offloads & RTE_ETH_RX_OFFLOAD_VLAN_FILTER) &&
    ...
    PMD_DRV_LOG(ERR, "vlan filtering not available on this host");
    return -ENOTSUP;
  }
  ...
  if (virtio_with_packed_queue(hw)) {
    ...
    if (hw->use_vec_rx) {
      ...
      if (rx_offloads &
          (RTE_ETH_RX_OFFLOAD_UDP_CKSUM | RTE_ETH_RX_OFFLOAD_TCP_CKSUM |
           RTE_ETH_RX_OFFLOAD_TCP_LRO | RTE_ETH_RX_OFFLOAD_VLAN_STRIP)) {
        PMD_DRV_LOG(INFO,
...
static int virtio_dev_start(struct rte_eth_dev *dev) {
   ...
   * stopped.
   */
  if (dev->data->dev_conf.intr_conf.lsc || dev->data->dev_conf.intr_conf.rxq) {
    virtio_intr_disable(dev);

    /* Setup interrupt callback  */
    if (dev->data->dev_flags & RTE_ETH_DEV_INTR_LSC)
      rte_intr_callback_register(dev->intr_handle, virtio_interrupt_handler,
                                 dev);
...
static void virtio_tx_completed_cleanup(struct rte_eth_dev *dev) {
  struct virtio_hw *hw = dev->data->dev_private;
  struct virtqueue *vq;
  int qidx;
...
static int virtio_dev_link_update(struct rte_eth_dev *dev,
  ...
  if (!hw->started) {
    ...
    if ((status & VIRTIO_NET_S_LINK_UP) == 0) {
      link.link_status = RTE_ETH_LINK_DOWN;
      link.link_speed = RTE_ETH_SPEED_NUM_NONE;
      PMD_INIT_LOG(DEBUG, "Port %d is down", dev->data->port_id);
    } else {
      link.link_status = RTE_ETH_LINK_UP;
      if (hw->get_speed_via_feat)
        virtio_get_speed_duplex(dev, &link);
      PMD_INIT_LOG(DEBUG, "Port %d is up", dev->data->port_id);
    }
  } else {
    link.link_status = RTE_ETH_LINK_UP;
    if (hw->get_speed_via_feat)
...
static int virtio_dev_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  ...
  if (mask & RTE_ETH_VLAN_FILTER_MASK) {
    if ((offloads & RTE_ETH_RX_OFFLOAD_VLAN_FILTER) &&
      ...
      PMD_DRV_LOG(NOTICE, "vlan filtering not available on this host");
...
static int virtio_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->max_rx_queues = RTE_MIN(hw->max_queue_pairs, VIRTIO_MAX_RX_QUEUES);
  dev_info->max_tx_queues = RTE_MIN(hw->max_queue_pairs, VIRTIO_MAX_TX_QUEUES);
  ...
  dev_info->rx_desc_lim.nb_min =
      RTE_MAX(DEFAULT_RX_FREE_THRESH, RTE_VIRTIO_VPMD_RX_REARM_THRESH);
...
static int virtio_dev_queue_stats_mapping_set(
    __rte_unused struct rte_eth_dev *eth_dev, __rte_unused uint16_t queue_id,
    __rte_unused uint8_t stat_idx, __rte_unused uint8_t is_rx) {
  return 0;
}
