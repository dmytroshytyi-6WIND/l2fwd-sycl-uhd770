...
#define FLAGS_BITS_OFFSET                                                      \
  ((offsetof(struct vring_packed_desc, flags) -                                \
    offsetof(struct vring_packed_desc, len)) *                                 \
   BYTE_SIZE)
#elif defined(RTE_ARCH_ARM)
/* flag bits offset in packed ring desc from ID */
#define FLAGS_BITS_OFFSET                                                      \
  ((offsetof(struct vring_packed_desc, flags) -                                \
    offsetof(struct vring_packed_desc, id)) *                                  \
   BYTE_SIZE)
#define FLAGS_LEN_BITS_OFFSET                                                  \
  ((offsetof(struct vring_packed_desc, flags) -                                \
    offsetof(struct vring_packed_desc, len)) *                                 \
   BYTE_SIZE)
#endif

#define PACKED_FLAGS_MASK                                                      \
  ((0ULL | VRING_PACKED_DESC_F_AVAIL_USED) << FLAGS_BITS_OFFSET)

/* reference count offset in mbuf rearm data */
#define REFCNT_BITS_OFFSET                                                     \
...
#define SEG_NUM_BITS_OFFSET                                                    \
  ((offsetof(struct rte_mbuf, nb_segs) -                                       \
    offsetof(struct rte_mbuf, rearm_data)) *                                   \
   BYTE_SIZE)
/* default rearm data */
#define DEFAULT_REARM_DATA                                                     \
  (1ULL << SEG_NUM_BITS_OFFSET | 1ULL << REFCNT_BITS_OFFSET)
#endif

/* id bits offset in packed ring desc higher 64bits */
#define ID_BITS_OFFSET                                                         \
  ((offsetof(struct vring_packed_desc, id) -                                   \
    offsetof(struct vring_packed_desc, len)) *                                 \
   BYTE_SIZE)
...
static inline void virtio_update_batch_stats(struct virtnet_stats *stats,
                                             uint16_t pkt_len1,
                                             uint16_t pkt_len2,
                                             uint16_t pkt_len3,
                                             uint16_t pkt_len4) {
...
static inline int virtqueue_enqueue_single_packed_vec(struct virtnet_tx *txvq,
                                                      struct rte_mbuf *txm) {
   ...
   */
  can_push = rte_mbuf_refcnt_read(txm) == 1 && RTE_MBUF_DIRECT(txm) &&
             txm->nb_segs == 1 && rte_pktmbuf_headroom(txm) >= hdr_size;
  ...
  virtqueue_enqueue_xmit_packed(txvq, txm, slots, use_indirect, can_push, 1);
...
static inline int virtio_vec_rx_offload(struct rte_mbuf *m,
...
static inline void virtio_recv_refill_packed_vec(struct virtnet_rx *rxvq,
                                                 struct rte_mbuf **cookie,
                                                 uint16_t num) {
  struct virtqueue *vq = virtnet_rxq_to_vq(rxvq);
  ...
  do {
    ...
    virtio_for_each_try_unroll(i, 0, batch_num) {
      ...
      start_dp[idx + i].len = cookie[total_num + i]->buf_len -
                              RTE_PKTMBUF_HEADROOM + hw->vtnet_hdr_size;
      if (total_num || i) {
        virtqueue_store_flags_packed(&start_dp[idx + i], flags,
                                     hw->weak_barriers);
      }
    }
