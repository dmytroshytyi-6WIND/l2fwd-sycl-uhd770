...
#define VHOST_VDPA_SUPPORTED_BACKEND_FEATURES                                  \
  (1ULL << VHOST_BACKEND_F_IOTLB_MSG_V2 | 1ULL << VHOST_BACKEND_F_IOTLB_BATCH)
...
static int vhost_vdpa_ioctl(int fd, uint64_t request, void *arg) {
  int ret;

  ret = ioctl(fd, request, arg);
  if (ret) {
    PMD_DRV_LOG(ERR, "Vhost-vDPA ioctl %" PRIu64 " failed (%s)", request,
                strerror(errno));
    return -1;
  }
...
static int vhost_vdpa_set_owner(struct virtio_user_dev *dev) {
  struct vhost_vdpa_data *data = dev->backend_data;

  return vhost_vdpa_ioctl(data->vhostfd, VHOST_SET_OWNER, NULL);
}

static int vhost_vdpa_get_protocol_features(struct virtio_user_dev *dev,
                                            uint64_t *features) {
  struct vhost_vdpa_data *data = dev->backend_data;
...
static int vhost_vdpa_set_protocol_features(struct virtio_user_dev *dev,
                                            uint64_t features) {
  struct vhost_vdpa_data *data = dev->backend_data;
...
static int vhost_vdpa_get_features(struct virtio_user_dev *dev,
                                   uint64_t *features) {
  struct vhost_vdpa_data *data = dev->backend_data;
  int ret;
...
static int vhost_vdpa_set_features(struct virtio_user_dev *dev,
                                   uint64_t features) {
  struct vhost_vdpa_data *data = dev->backend_data;
...
static int vhost_vdpa_iotlb_batch_begin(struct virtio_user_dev *dev) {
  struct vhost_vdpa_data *data = dev->backend_data;
  ...
  if (write(data->vhostfd, &msg, sizeof(msg)) != sizeof(msg)) {
    PMD_DRV_LOG(ERR, "Failed to send IOTLB batch begin (%s)", strerror(errno));
    return -1;
  }
...
static int vhost_vdpa_iotlb_batch_end(struct virtio_user_dev *dev) {
  struct vhost_vdpa_data *data = dev->backend_data;
  struct vhost_msg msg = {};
  ...
  if (write(data->vhostfd, &msg, sizeof(msg)) != sizeof(msg)) {
    PMD_DRV_LOG(ERR, "Failed to send IOTLB batch end (%s)", strerror(errno));
    return -1;
  }
...
static int vhost_vdpa_dma_map(struct virtio_user_dev *dev, void *addr,
  ...
  PMD_DRV_LOG(DEBUG, "%s: iova: 0x%" PRIx64 ", addr: %p, len: 0x%zx", __func__,
              iova, addr, len);

  if (write(data->vhostfd, &msg, sizeof(msg)) != sizeof(msg)) {
    PMD_DRV_LOG(ERR, "Failed to send IOTLB update (%s)", strerror(errno));
    return -1;
  }
...
static int vhost_vdpa_dma_unmap(struct virtio_user_dev *dev,
                                __rte_unused void *addr, uint64_t iova,
                                size_t len) {
  struct vhost_vdpa_data *data = dev->backend_data;
  struct vhost_msg msg = {};
  ...
  PMD_DRV_LOG(DEBUG, "%s: iova: 0x%" PRIx64 ", len: 0x%zx", __func__, iova,
              len);

  if (write(data->vhostfd, &msg, sizeof(msg)) != sizeof(msg)) {
    PMD_DRV_LOG(ERR, "Failed to send IOTLB invalidate (%s)", strerror(errno));
    return -1;
  }
...
static int vhost_vdpa_dma_map_batch(struct virtio_user_dev *dev, void *addr,
                                    uint64_t iova, size_t len) {
  int ret;
...
static int vhost_vdpa_dma_unmap_batch(struct virtio_user_dev *dev, void *addr,
                                      uint64_t iova, size_t len) {
  int ret;
...
static int vhost_vdpa_map_contig(const struct rte_memseg_list *msl,
...
static int vhost_vdpa_map(const struct rte_memseg_list *msl,
...
static int vhost_vdpa_set_memory_table(struct virtio_user_dev *dev) {
  int ret;
  ...
  if (rte_eal_iova_mode() == RTE_IOVA_VA) {
     ...
     */
    ret = rte_memseg_contig_walk_thread_unsafe(vhost_vdpa_map_contig, dev);
    if (ret)
      goto batch_end;
...
static int vhost_vdpa_set_vring_enable(struct virtio_user_dev *dev,
...
static int vhost_vdpa_set_vring_num(struct virtio_user_dev *dev,
                                    struct vhost_vring_state *state) {
  struct vhost_vdpa_data *data = dev->backend_data;
...
static int vhost_vdpa_set_vring_base(struct virtio_user_dev *dev,
                                     struct vhost_vring_state *state) {
  struct vhost_vdpa_data *data = dev->backend_data;
...
static int vhost_vdpa_get_vring_base(struct virtio_user_dev *dev,
                                     struct vhost_vring_state *state) {
  struct vhost_vdpa_data *data = dev->backend_data;
...
static int vhost_vdpa_set_vring_call(struct virtio_user_dev *dev,
...
static int vhost_vdpa_set_vring_kick(struct virtio_user_dev *dev,
                                     struct vhost_vring_file *file) {
  struct vhost_vdpa_data *data = dev->backend_data;
...
static int vhost_vdpa_set_vring_addr(struct virtio_user_dev *dev,
...
static int vhost_vdpa_get_status(struct virtio_user_dev *dev, uint8_t *status) {
  struct vhost_vdpa_data *data = dev->backend_data;

  return vhost_vdpa_ioctl(data->vhostfd, VHOST_VDPA_GET_STATUS, status);
}

static int vhost_vdpa_set_status(struct virtio_user_dev *dev, uint8_t status) {
  struct vhost_vdpa_data *data = dev->backend_data;

  return vhost_vdpa_ioctl(data->vhostfd, VHOST_VDPA_SET_STATUS, &status);
}

static int vhost_vdpa_get_config(struct virtio_user_dev *dev, uint8_t *data,
  ...
  if (ret) {
    PMD_DRV_LOG(ERR, "Failed to get vDPA config (offset 0x%x, len 0x%x)", off,
                len);
    ret = -1;
    goto out;
  }
...
static int vhost_vdpa_set_config(struct virtio_user_dev *dev,
  ...
  if (ret) {
    PMD_DRV_LOG(ERR, "Failed to set vDPA config (offset 0x%x, len 0x%x)", off,
                len);
    ret = -1;
  }
 ...
 *   - (>=0) if successful.
 */
static int vhost_vdpa_setup(struct virtio_user_dev *dev) {
  ...
  if (data->vhostfd < 0) {
    PMD_DRV_LOG(ERR, "Failed to open %s: %s", dev->path, strerror(errno));
    free(data);
    return -1;
  }
...
static int vhost_vdpa_destroy(struct virtio_user_dev *dev) {
  struct vhost_vdpa_data *data = dev->backend_data;
...
static int vhost_vdpa_enable_queue_pair(struct virtio_user_dev *dev,
...
static int vhost_vdpa_get_backend_features(uint64_t *features) {
  *features = 0;
...
static int vhost_vdpa_get_intr_fd(struct virtio_user_dev *dev __rte_unused) {
