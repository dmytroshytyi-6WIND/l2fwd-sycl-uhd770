...
static int virtio_user_create_queue(struct virtio_user_dev *dev,
                                    uint32_t queue_sel) {
...
static int virtio_user_kick_queue(struct virtio_user_dev *dev,
                                  uint32_t queue_sel) {
  int ret;
  struct vhost_vring_file file;
  ...
  struct vhost_vring_addr addr = {
      .index = queue_sel, .log_guest_addr = 0, .flags = 0, /* disable log */
  };

  if (dev->features & (1ULL << VIRTIO_F_RING_PACKED)) {
    addr.desc_user_addr = (uint64_t)(uintptr_t)pq_vring->desc;
    addr.avail_user_addr = (uint64_t)(uintptr_t)pq_vring->driver;
    addr.used_user_addr = (uint64_t)(uintptr_t)pq_vring->device;
  } else {
...
static int virtio_user_queue_setup(struct virtio_user_dev *dev,
                                   int (*fn)(struct virtio_user_dev *,
                                             uint32_t)) {
...
static void virtio_user_dev_init_mac(struct virtio_user_dev *dev,
                                     const char *mac) {
...
static int virtio_user_dev_init_notify(struct virtio_user_dev *dev) {
  uint32_t i, j;
  int callfd;
  int kickfd;
...
static void virtio_user_dev_uninit_notify(struct virtio_user_dev *dev) {
  uint32_t i;
...
static int virtio_user_fill_intr_handle(struct virtio_user_dev *dev) {
  uint32_t i;
  ...
  for (i = 0; i < dev->max_queue_pairs; ++i) {
    if (rte_intr_efds_index_set(eth_dev->intr_handle, i, dev->callfds[i]))
      return -rte_errno;
  }
  ...
  if (rte_intr_max_intr_set(eth_dev->intr_handle, dev->max_queue_pairs + 1))
    return -rte_errno;
...
static void virtio_user_mem_event_cb(enum rte_mem_event type __rte_unused,
                                     const void *addr, size_t len __rte_unused,
                                     void *arg) {
...
static int virtio_user_dev_setup(struct virtio_user_dev *dev) {
  if (dev->is_server) {
...
#define VIRTIO_USER_SUPPORTED_FEATURES                                         \
  (1ULL << VIRTIO_NET_F_MAC | 1ULL << VIRTIO_NET_F_STATUS |                    \
   1ULL << VIRTIO_NET_F_MQ | 1ULL << VIRTIO_NET_F_CTRL_MAC_ADDR |              \
   1ULL << VIRTIO_NET_F_CTRL_VQ | 1ULL << VIRTIO_NET_F_CTRL_RX |               \
   1ULL << VIRTIO_NET_F_CTRL_VLAN | 1ULL << VIRTIO_NET_F_CSUM |                \
   1ULL << VIRTIO_NET_F_HOST_TSO4 | 1ULL << VIRTIO_NET_F_HOST_TSO6 |           \
   1ULL << VIRTIO_NET_F_MRG_RXBUF | 1ULL << VIRTIO_RING_F_INDIRECT_DESC |      \
   1ULL << VIRTIO_NET_F_GUEST_CSUM | 1ULL << VIRTIO_NET_F_GUEST_TSO4 |         \
   1ULL << VIRTIO_NET_F_GUEST_TSO6 | 1ULL << VIRTIO_F_IN_ORDER |               \
   1ULL << VIRTIO_F_VERSION_1 | 1ULL << VIRTIO_F_RING_PACKED)
...
int virtio_user_dev_init(struct virtio_user_dev *dev, char *path, int queues,
  ...
  dev->unsupported_features =
      ~(VIRTIO_USER_SUPPORTED_FEATURES | backend_features);
...
static uint32_t virtio_user_handle_ctrl_msg(struct virtio_user_dev *dev,
                                            struct vring *vring,
                                            uint16_t idx_hdr) {
...
static inline int desc_is_avail(struct vring_packed_desc *desc,
                                bool wrap_counter) {
...
static uint32_t virtio_user_handle_ctrl_msg_packed(struct virtio_user_dev *dev,
...
void virtio_user_handle_cq_packed(struct virtio_user_dev *dev,
   ...
   * content.
   */
  while (desc_is_avail(&vring->desc[vq->used_idx], vq->used_wrap_counter)) {

    n_descs = virtio_user_handle_ctrl_msg_packed(dev, vring, vq->used_idx);

    flags = VRING_DESC_F_WRITE;
    if (vq->used_wrap_counter)
      flags |= VRING_PACKED_DESC_F_AVAIL_USED;

    __atomic_store_n(&vring->desc[vq->used_idx].flags, flags, __ATOMIC_RELEASE);
...
void virtio_user_handle_cq(struct virtio_user_dev *dev, uint16_t queue_idx) {
  ...
  while (__atomic_load_n(&vring->used->idx, __ATOMIC_RELAXED) !=
         vring->avail->idx) {
    avail_idx =
        __atomic_load_n(&vring->used->idx, __ATOMIC_RELAXED) & (vring->num - 1);
...
int virtio_user_dev_update_status(struct virtio_user_dev *dev) {
  ...
  if (!ret) {
    ...
    PMD_INIT_LOG(DEBUG,
                 ...
                 "\t-DEVICE_NEED_RESET: %u\n"
                 "\t-FAILED: %u",
                 dev->status, (dev->status == VIRTIO_CONFIG_STATUS_RESET),
...
static void virtio_user_dev_reset_queues_packed(struct rte_eth_dev *eth_dev) {
...
void virtio_user_dev_delayed_disconnect_handler(void *param) {
  ...
  if (rte_intr_callback_unregister(eth_dev->intr_handle,
                                   virtio_interrupt_handler, eth_dev) != 1)
...
static void virtio_user_dev_delayed_intr_reconfig_handler(void *param) {
  struct virtio_user_dev *dev = param;
  struct rte_eth_dev *eth_dev = &rte_eth_devices[dev->hw.port_id];
  ...
  if (rte_intr_callback_register(eth_dev->intr_handle, virtio_interrupt_handler,
                                 eth_dev))
    PMD_DRV_LOG(ERR, "interrupt register failed");
...
int virtio_user_dev_server_reconnect(struct virtio_user_dev *dev) {
  ...
  if (eth_dev->data->dev_flags & RTE_ETH_DEV_INTR_LSC) {
     ...
     */
    rte_eal_alarm_set(1, virtio_user_dev_delayed_intr_reconfig_handler,
                      (void *)dev);
  }
