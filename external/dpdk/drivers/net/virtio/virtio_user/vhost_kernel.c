...
static void get_vhost_kernel_max_regions(void) {
  int fd;
  char buf[20] = {'\0'};
...
static int vhost_kernel_ioctl(int fd, uint64_t request, void *arg) {
  int ret;

  ret = ioctl(fd, request, arg);
  if (ret) {
    PMD_DRV_LOG(ERR, "Vhost-kernel ioctl %" PRIu64 " failed (%s)", request,
                strerror(errno));
    return -1;
  }
...
static int vhost_kernel_set_owner(struct virtio_user_dev *dev) {
  int ret;
  uint32_t i;
...
static int vhost_kernel_get_features(struct virtio_user_dev *dev,
                                     uint64_t *features) {
  struct vhost_kernel_data *data = dev->backend_data;
...
static int vhost_kernel_set_features(struct virtio_user_dev *dev,
                                     uint64_t features) {
  struct vhost_kernel_data *data = dev->backend_data;
  uint32_t i;
  int ret;
...
static int add_memseg_list(const struct rte_memseg_list *msl, void *arg) {
  ...
  PMD_DRV_LOG(DEBUG, "index=%u addr=%p len=%" PRIu64, vm->nregions - 1,
              start_addr, len);
 ...
 */
static int vhost_kernel_set_memory_table(struct virtio_user_dev *dev) {
  uint32_t i;
  struct vhost_kernel_data *data = dev->backend_data;
  ...
  vm = malloc(sizeof(struct vhost_memory_kernel) +
              max_regions * sizeof(struct vhost_memory_region));
  if (!vm)
    goto err;
...
static int vhost_kernel_set_vring(struct virtio_user_dev *dev, uint64_t req,
...
static int vhost_kernel_set_vring_num(struct virtio_user_dev *dev,
                                      struct vhost_vring_state *state) {
  return vhost_kernel_set_vring(dev, VHOST_SET_VRING_NUM, state);
}

static int vhost_kernel_set_vring_base(struct virtio_user_dev *dev,
                                       struct vhost_vring_state *state) {
  return vhost_kernel_set_vring(dev, VHOST_SET_VRING_BASE, state);
}

static int vhost_kernel_get_vring_base(struct virtio_user_dev *dev,
                                       struct vhost_vring_state *state) {
  return vhost_kernel_set_vring(dev, VHOST_GET_VRING_BASE, state);
}

static int vhost_kernel_set_vring_file(struct virtio_user_dev *dev,
                                       uint64_t req,
...
static int vhost_kernel_set_vring_kick(struct virtio_user_dev *dev,
                                       struct vhost_vring_file *file) {
  return vhost_kernel_set_vring_file(dev, VHOST_SET_VRING_KICK, file);
}

static int vhost_kernel_set_vring_call(struct virtio_user_dev *dev,
                                       struct vhost_vring_file *file) {
  return vhost_kernel_set_vring_file(dev, VHOST_SET_VRING_CALL, file);
}

static int vhost_kernel_set_vring_addr(struct virtio_user_dev *dev,
...
static int vhost_kernel_get_status(struct virtio_user_dev *dev __rte_unused,
                                   uint8_t *status __rte_unused) {
  return -ENOTSUP;
}

static int vhost_kernel_set_status(struct virtio_user_dev *dev __rte_unused,
                                   uint8_t status __rte_unused) {
  return -ENOTSUP;
}
 ...
 *   - (>=0) if successful.
 */
static int vhost_kernel_setup(struct virtio_user_dev *dev) {
...
static int vhost_kernel_destroy(struct virtio_user_dev *dev) {
  struct vhost_kernel_data *data = dev->backend_data;
  uint32_t i;
...
static int vhost_kernel_set_backend(int vhostfd, int tapfd) {
  struct vhost_vring_file f;

  f.fd = tapfd;
  f.index = 0;
  if (ioctl(vhostfd, VHOST_NET_SET_BACKEND, &f) < 0) {
    PMD_DRV_LOG(ERR, "VHOST_NET_SET_BACKEND fails, %s", strerror(errno));
    return -1;
  }
...
static int vhost_kernel_enable_queue_pair(struct virtio_user_dev *dev,
...
static int vhost_kernel_get_backend_features(uint64_t *features) {
  *features = 0;
...
static int vhost_kernel_get_intr_fd(struct virtio_user_dev *dev __rte_unused) {
