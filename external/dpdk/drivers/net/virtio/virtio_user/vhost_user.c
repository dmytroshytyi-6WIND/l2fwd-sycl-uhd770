...
static int vhost_user_write(int fd, struct vhost_user_msg *msg, int *fds,
                            int fd_num) {
  int r;
  struct msghdr msgh;
  struct iovec iov;
...
static int vhost_user_read(int fd, struct vhost_user_msg *msg) {
  ...
  if (ret < 0) {
    ...
    PMD_DRV_LOG(ERR, "Failed to recv msg hdr: %d instead of %d.", ret, sz_hdr);
    return -1;
  }
  ...
  if (sz_payload) {
    ...
    if (ret < 0) {
      ...
      PMD_DRV_LOG(ERR, "Failed to recv msg payload: %d instead of %u.", ret,
                  msg->size);
      return -1;
    }
  }
...
static int vhost_user_check_reply_ack(struct virtio_user_dev *dev,
...
static int vhost_user_set_owner(struct virtio_user_dev *dev) {
  int ret;
  struct vhost_user_data *data = dev->backend_data;
  struct vhost_user_msg msg = {
...
static int vhost_user_get_protocol_features(struct virtio_user_dev *dev,
                                            uint64_t *features) {
  int ret;
  struct vhost_user_data *data = dev->backend_data;
  struct vhost_user_msg msg = {
...
static int vhost_user_set_protocol_features(struct virtio_user_dev *dev,
                                            uint64_t features) {
  int ret;
  struct vhost_user_data *data = dev->backend_data;
  struct vhost_user_msg msg = {
...
static int vhost_user_get_features(struct virtio_user_dev *dev,
                                   uint64_t *features) {
  int ret;
  struct vhost_user_data *data = dev->backend_data;
  struct vhost_user_msg msg = {
...
static int vhost_user_set_features(struct virtio_user_dev *dev,
                                   uint64_t features) {
  int ret;
  struct vhost_user_data *data = dev->backend_data;
  struct vhost_user_msg msg = {
  ...
  msg.payload.u64 |=
      dev->device_features & (1ULL << VHOST_USER_F_PROTOCOL_FEATURES);
...
static int update_memory_region(const struct rte_memseg_list *msl __rte_unused,
  ...
  if (fd < 0) {
    PMD_DRV_LOG(ERR, "Failed to get fd, ms=%p rte_errno=%d", ms, rte_errno);
    return -1;
  }

  if (rte_memseg_get_fd_offset_thread_unsafe(ms, &offset) < 0) {
    PMD_DRV_LOG(ERR, "Failed to get offset, ms=%p rte_errno=%d", ms, rte_errno);
    return -1;
  }
  ...
  for (i = 0; i < wa->region_nr; i++) {
    ...
    PMD_DRV_LOG(DEBUG,
                "index=%d fd=%d offset=0x%" PRIx64 " addr=0x%" PRIx64
                " len=%" PRIu64,
                i, fd, mr->mmap_offset, mr->userspace_addr, mr->memory_size);
...
static int vhost_user_set_memory_table(struct virtio_user_dev *dev) {
  struct walk_arg wa;
...
static int vhost_user_set_vring(struct virtio_user_dev *dev,
...
static int vhost_user_set_vring_enable(struct virtio_user_dev *dev,
                                       struct vhost_vring_state *state) {
  return vhost_user_set_vring(dev, VHOST_USER_SET_VRING_ENABLE, state);
}

static int vhost_user_set_vring_num(struct virtio_user_dev *dev,
                                    struct vhost_vring_state *state) {
  return vhost_user_set_vring(dev, VHOST_USER_SET_VRING_NUM, state);
}

static int vhost_user_set_vring_base(struct virtio_user_dev *dev,
                                     struct vhost_vring_state *state) {
  return vhost_user_set_vring(dev, VHOST_USER_SET_VRING_BASE, state);
}

static int vhost_user_get_vring_base(struct virtio_user_dev *dev,
                                     struct vhost_vring_state *state) {
  int ret;
  struct vhost_user_msg msg;
  struct vhost_user_data *data = dev->backend_data;
...
static int vhost_user_set_vring_file(struct virtio_user_dev *dev,
                                     enum vhost_user_request req,
...
static int vhost_user_set_vring_call(struct virtio_user_dev *dev,
                                     struct vhost_vring_file *file) {
  return vhost_user_set_vring_file(dev, VHOST_USER_SET_VRING_CALL, file);
}

static int vhost_user_set_vring_kick(struct virtio_user_dev *dev,
                                     struct vhost_vring_file *file) {
  return vhost_user_set_vring_file(dev, VHOST_USER_SET_VRING_KICK, file);
}

static int vhost_user_set_vring_addr(struct virtio_user_dev *dev,
                                     struct vhost_vring_addr *addr) {
  int ret;
  struct vhost_user_data *data = dev->backend_data;
  struct vhost_user_msg msg = {
...
static int vhost_user_get_status(struct virtio_user_dev *dev, uint8_t *status) {
  int ret;
  struct vhost_user_data *data = dev->backend_data;
  struct vhost_user_msg msg = {
...
static int vhost_user_set_status(struct virtio_user_dev *dev, uint8_t status) {
  int ret;
  struct vhost_user_data *data = dev->backend_data;
  struct vhost_user_msg msg = {
...
static int vhost_user_start_server(struct virtio_user_dev *dev,
...
static int vhost_user_server_disconnect(struct virtio_user_dev *dev) {
  struct vhost_user_data *data = dev->backend_data;
...
static int vhost_user_server_reconnect(struct virtio_user_dev *dev) {
  struct vhost_user_data *data = dev->backend_data;
  int fd;
 ...
 *   - (0) if succeed.
 */
static int vhost_user_setup(struct virtio_user_dev *dev) {
  int fd;
  int flag;
  struct sockaddr_un un;
...
static int vhost_user_destroy(struct virtio_user_dev *dev) {
  struct vhost_user_data *data = dev->backend_data;
...
static int vhost_user_enable_queue_pair(struct virtio_user_dev *dev,
...
static int vhost_user_get_backend_features(uint64_t *features) {
  *features = 1ULL << VHOST_USER_F_PROTOCOL_FEATURES;

  return 0;
}

static int vhost_user_update_link_state(struct virtio_user_dev *dev) {
  struct vhost_user_data *data = dev->backend_data;
  char buf[128];
  ...
  if (data->vhostfd >= 0) {
    ...
    if (r == 0 || (r < 0 && errno != EAGAIN)) {
       ...
       */
      rte_eal_alarm_set(1, virtio_user_dev_delayed_disconnect_handler,
                        (void *)dev);
    } else {
      dev->net_status |= VIRTIO_NET_S_LINK_UP;
    }
...
static int vhost_user_get_intr_fd(struct virtio_user_dev *dev) {
  struct vhost_user_data *data = dev->backend_data;
