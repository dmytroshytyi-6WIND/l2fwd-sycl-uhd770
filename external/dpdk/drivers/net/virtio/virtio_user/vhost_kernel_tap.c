...
int tap_get_name(int tapfd, char **name) {
  struct ifreq ifr;
  int ret;
...
static int vhost_kernel_tap_set_offload(int fd, uint64_t features) {
  ...
  if (ioctl(fd, TUNSETOFFLOAD, offload) != 0) {
    offload &= ~TUN_F_UFO;
    if (ioctl(fd, TUNSETOFFLOAD, offload) != 0) {
      PMD_DRV_LOG(ERR, "TUNSETOFFLOAD ioctl() failed: %s", strerror(errno));
      return -1;
    }
  }
