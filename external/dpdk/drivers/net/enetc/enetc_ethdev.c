...
static int enetc_dev_start(struct rte_eth_dev *dev) {
  ...
  if (enetc_port_rd(enetc_hw, ENETC_PM0_IF_MODE) & ENETC_PMO_IFM_RG) {
    enetc_port_wr(enetc_hw, ENETC_PM0_IF_MODE, ENETC_PM0_IFM_RGAUTO);
    enetc_port_wr(enetc_hw, ENETC_PM1_IF_MODE, ENETC_PM0_IFM_RGAUTO);
  }
  if (enetc_global_rd(enetc_hw, ENETC_G_EPFBLPR(1)) == ENETC_G_EPFBLPR1_XGMII) {
    enetc_port_wr(enetc_hw, ENETC_PM0_IF_MODE, ENETC_PM0_IFM_XGMII);
    enetc_port_wr(enetc_hw, ENETC_PM1_IF_MODE, ENETC_PM0_IFM_XGMII);
  }

  return 0;
}

static int enetc_dev_stop(struct rte_eth_dev *dev) {
  struct enetc_eth_hw *hw = ENETC_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct enetc_hw *enetc_hw = &hw->hw;
  uint32_t val;
...
enetc_supported_ptypes_get(struct rte_eth_dev *dev __rte_unused) {
  static const uint32_t ptypes[] = {RTE_PTYPE_L2_ETHER, RTE_PTYPE_L3_IPV4,
...
static int enetc_link_update(struct rte_eth_dev *dev,
...
static int enetc_hardware_init(struct enetc_eth_hw *hw) {
  ...
  if ((high_mac | low_mac) == 0) {
    ...
    *mac = (uint16_t)rte_rand();
    ...
    print_ethaddr("New address: ", (const struct rte_ether_addr *)hw->mac.addr);
  }

  return 0;
}

static int enetc_dev_infos_get(struct rte_eth_dev *dev __rte_unused,
...
static int enetc_alloc_txbdr(struct enetc_bdr *txr, uint16_t nb_desc) {
  int size;
...
static void enetc_setup_txbdr(struct enetc_hw *hw, struct enetc_bdr *tx_ring) {
  ...
  bd_address = (phys_addr_t)rte_mem_virt2iova((const void *)tx_ring->bd_base);
  enetc_txbdr_wr(hw, idx, ENETC_TBBAR0, lower_32_bits((uint64_t)bd_address));
  enetc_txbdr_wr(hw, idx, ENETC_TBBAR1, upper_32_bits((uint64_t)bd_address));
  enetc_txbdr_wr(hw, idx, ENETC_TBLENR, ENETC_RTBLENR_LEN(tx_ring->bd_count));

  enetc_txbdr_wr(hw, idx, ENETC_TBCIR, 0);
  enetc_txbdr_wr(hw, idx, ENETC_TBCISR, 0);
  tx_ring->tcir = (void *)((size_t)hw->reg + ENETC_BDR(TX, idx, ENETC_TBCIR));
  tx_ring->tcisr = (void *)((size_t)hw->reg + ENETC_BDR(TX, idx, ENETC_TBCISR));
}

static int enetc_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                                uint16_t nb_desc,
  ...
  struct enetc_eth_adapter *priv = ENETC_DEV_PRIVATE(data->dev_private);
  ...
  if (!tx_conf->tx_deferred_start) {
    /* enable ring */
    enetc_txbdr_wr(&priv->hw.hw, tx_ring->index, ENETC_TBMR, ENETC_TBMR_EN);
    dev->data->tx_queue_state[tx_ring->index] = RTE_ETH_QUEUE_STATE_STARTED;
  } else {
    dev->data->tx_queue_state[tx_ring->index] = RTE_ETH_QUEUE_STATE_STOPPED;
  }
...
static void enetc_tx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
...
static int enetc_alloc_rxbdr(struct enetc_bdr *rxr, uint16_t nb_rx_desc) {
  int size;
...
static void enetc_setup_rxbdr(struct enetc_hw *hw, struct enetc_bdr *rx_ring,
  ...
  bd_address = (phys_addr_t)rte_mem_virt2iova((const void *)rx_ring->bd_base);
  enetc_rxbdr_wr(hw, idx, ENETC_RBBAR0, lower_32_bits((uint64_t)bd_address));
  enetc_rxbdr_wr(hw, idx, ENETC_RBBAR1, upper_32_bits((uint64_t)bd_address));
  enetc_rxbdr_wr(hw, idx, ENETC_RBLENR, ENETC_RTBLENR_LEN(rx_ring->bd_count));

  rx_ring->mb_pool = mb_pool;
  rx_ring->rcir = (void *)((size_t)hw->reg + ENETC_BDR(RX, idx, ENETC_RBCIR));
...
static int enetc_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
  ...
  struct rte_eth_dev_data *data = dev->data;
  struct enetc_eth_adapter *adapter = ENETC_DEV_PRIVATE(data->dev_private);
  ...
  if (!rx_conf->rx_deferred_start) {
    /* enable ring */
    enetc_rxbdr_wr(&adapter->hw.hw, rx_ring->index, ENETC_RBMR, ENETC_RBMR_EN);
  ...
  rx_ring->crc_len = (uint8_t)(
      (rx_offloads & RTE_ETH_RX_OFFLOAD_KEEP_CRC) ? RTE_ETHER_CRC_LEN : 0);
...
static void enetc_rx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
...
static int enetc_stats_reset(struct rte_eth_dev *dev) {
  struct enetc_eth_hw *hw = ENETC_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct enetc_hw *enetc_hw = &hw->hw;
...
static int enetc_dev_close(struct rte_eth_dev *dev) {
  uint16_t i;
  int ret;
...
static int enetc_promiscuous_enable(struct rte_eth_dev *dev) {
  struct enetc_eth_hw *hw = ENETC_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct enetc_hw *enetc_hw = &hw->hw;
  uint32_t psipmr = 0;
...
static int enetc_promiscuous_disable(struct rte_eth_dev *dev) {
  struct enetc_eth_hw *hw = ENETC_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct enetc_hw *enetc_hw = &hw->hw;
  uint32_t psipmr = 0;
...
static int enetc_allmulticast_enable(struct rte_eth_dev *dev) {
  struct enetc_eth_hw *hw = ENETC_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct enetc_hw *enetc_hw = &hw->hw;
  uint32_t psipmr = 0;
...
static int enetc_allmulticast_disable(struct rte_eth_dev *dev) {
  struct enetc_eth_hw *hw = ENETC_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct enetc_hw *enetc_hw = &hw->hw;
  uint32_t psipmr = 0;
  ...
  psipmr = enetc_port_rd(enetc_hw, ENETC_PSIPMR) & ~(ENETC_PSIPMR_SET_MP(0));
...
static int enetc_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct enetc_eth_hw *hw = ENETC_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct enetc_hw *enetc_hw = &hw->hw;
...
static int enetc_dev_configure(struct rte_eth_dev *dev) {
  ...
  max_len =
      dev->data->dev_conf.rxmode.mtu + RTE_ETHER_HDR_LEN + RTE_ETHER_CRC_LEN;
...
static int enetc_rx_queue_start(struct rte_eth_dev *dev, uint16_t qidx) {
  struct enetc_eth_adapter *priv = ENETC_DEV_PRIVATE(dev->data->dev_private);
  struct enetc_bdr *rx_ring;
  uint32_t rx_data;

  rx_ring = dev->data->rx_queues[qidx];
  if (dev->data->rx_queue_state[qidx] == RTE_ETH_QUEUE_STATE_STOPPED) {
    rx_data = enetc_rxbdr_rd(&priv->hw.hw, rx_ring->index, ENETC_RBMR);
    rx_data = rx_data | ENETC_RBMR_EN;
    enetc_rxbdr_wr(&priv->hw.hw, rx_ring->index, ENETC_RBMR, rx_data);
...
static int enetc_rx_queue_stop(struct rte_eth_dev *dev, uint16_t qidx) {
  struct enetc_eth_adapter *priv = ENETC_DEV_PRIVATE(dev->data->dev_private);
  struct enetc_bdr *rx_ring;
  uint32_t rx_data;
...
static int enetc_tx_queue_start(struct rte_eth_dev *dev, uint16_t qidx) {
  struct enetc_eth_adapter *priv = ENETC_DEV_PRIVATE(dev->data->dev_private);
  struct enetc_bdr *tx_ring;
  uint32_t tx_data;
  ...
  if (dev->data->tx_queue_state[qidx] == RTE_ETH_QUEUE_STATE_STOPPED) {
    tx_data = enetc_txbdr_rd(&priv->hw.hw, tx_ring->index, ENETC_TBMR);
    tx_data = tx_data | ENETC_TBMR_EN;
    enetc_txbdr_wr(&priv->hw.hw, tx_ring->index, ENETC_TBMR, tx_data);
...
static int enetc_tx_queue_stop(struct rte_eth_dev *dev, uint16_t qidx) {
  struct enetc_eth_adapter *priv = ENETC_DEV_PRIVATE(dev->data->dev_private);
  struct enetc_bdr *tx_ring;
  uint32_t tx_data;
 ...
 */
static int enetc_dev_init(struct rte_eth_dev *eth_dev) {
  int error = 0;
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(eth_dev);
  struct enetc_eth_hw *hw = ENETC_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
  ...
  /* Set MTU */
  enetc_port_wr(&hw->hw, ENETC_PM0_MAXFRM, ENETC_SET_MAXFRM(RTE_ETHER_MAX_LEN));
  eth_dev->data->mtu =
      RTE_ETHER_MAX_LEN - RTE_ETHER_HDR_LEN - RTE_ETHER_CRC_LEN;
...
static int enetc_dev_uninit(struct rte_eth_dev *eth_dev) {
  PMD_INIT_FUNC_TRACE();
...
static int enetc_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  return rte_eth_dev_pci_generic_probe(
      pci_dev, sizeof(struct enetc_eth_adapter), enetc_dev_init);
}
