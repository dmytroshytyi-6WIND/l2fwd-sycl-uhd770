...
#define ENETC_CACHE_LINE_RXBDS (RTE_CACHE_LINE_SIZE / sizeof(union enetc_rx_bd))
...
static int enetc_clean_tx_ring(struct enetc_bdr *tx_ring) {
   ...
   * from HW.
   */
  hwci = (int)(rte_read32_relaxed(tx_ring->tcisr) & ENETC_TBCISR_IDX_MASK);
...
uint16_t enetc_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                         uint16_t nb_pkts) {
  ...
  while (nb_pkts--) {
    ...
    txbd->addr = (uint64_t)(uintptr_t)rte_cpu_to_le_64(
        (size_t)tx_swbd->buffer_addr->buf_iova +
...
int enetc_refill_rx_ring(struct enetc_bdr *rx_ring, const int buff_cnt) {
  ...
  for (j = 0; j < buff_cnt; j++) {
    ...
    rxbd->w.addr = (uint64_t)(uintptr_t)rx_swbd->buffer_addr->buf_iova +
...
static inline void enetc_slow_parsing(struct rte_mbuf *m,
  ...
  case ENETC_PARSE_ERROR | ENETC_PKT_TYPE_IPV4_TCP:
    ...
    m->ol_flags |= RTE_MBUF_F_RX_IP_CKSUM_GOOD | RTE_MBUF_F_RX_L4_CKSUM_BAD;
    return;
...
static inline void __rte_hot enetc_dev_rx_parse(struct rte_mbuf *m,
                                                uint16_t parse_results) {
...
static int enetc_clean_rx_ring(struct enetc_bdr *rx_ring,
                               struct rte_mbuf **rx_pkts, int work_limit) {
  int rx_frm_cnt = 0;
   ...
   */
  rte_prefetch0(ENETC_RXBD(*rx_ring, (i + ENETC_CACHE_LINE_RXBDS) % bd_count));
  rte_prefetch0(
      ENETC_RXBD(*rx_ring, (i + ENETC_CACHE_LINE_RXBDS * 2) % bd_count));
  ...
  while (likely(rx_frm_cnt < work_limit)) {
    ...
    rx_swbd->buffer_addr->pkt_len = rxbd->r.buf_len - rx_ring->crc_len;
    rx_swbd->buffer_addr->data_len = rxbd->r.buf_len - rx_ring->crc_len;
    ...
    rxbd = ENETC_RXBD(*rx_ring, i);
    rte_prefetch0(
        ENETC_RXBD(*rx_ring, (i + ENETC_CACHE_LINE_RXBDS) % bd_count));
    rte_prefetch0(
        ENETC_RXBD(*rx_ring, (i + ENETC_CACHE_LINE_RXBDS * 2) % bd_count));
