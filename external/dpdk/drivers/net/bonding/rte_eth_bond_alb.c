...
static inline uint8_t simple_hash(uint8_t *hash_start, int hash_size) {
  int i;
  uint8_t hash;
...
static uint16_t calculate_slave(struct bond_dev_private *internals) {
  uint16_t idx;
...
int bond_mode_alb_enable(struct rte_eth_dev *bond_dev) {
  ...
  if (internals->mode6.mempool == NULL) {
    ...
    snprintf(mem_name, sizeof(mem_name), "%s_ALB", bond_dev->device->name);
    internals->mode6.mempool = rte_pktmbuf_pool_create(
        mem_name, 512 * RTE_MAX_ETHPORTS,
...
uint16_t bond_mode_alb_arp_xmit(struct rte_ether_hdr *eth_h, uint16_t offset,
                                struct bond_dev_private *internals) {
  struct rte_arp_hdr *arp;
  ...
  hash_index = simple_hash((uint8_t *)&arp->arp_data.arp_tip, sizeof(uint32_t));
  client_info = &hash_table[hash_index];
  ...
  rte_spinlock_unlock(&internals->mode6.lock);
  rte_eth_macaddr_get(internals->current_primary_port, &arp->arp_data.arp_sha);
  return internals->current_primary_port;
}

uint16_t bond_mode_alb_arp_upd(struct client_data *client_info,
                               struct rte_mbuf *pkt,
                               struct bond_dev_private *internals) {
...
void bond_mode_alb_client_list_upd(struct rte_eth_dev *bond_dev) {
  struct bond_dev_private *internals = bond_dev->data->dev_private;
  struct client_data *client_info;
