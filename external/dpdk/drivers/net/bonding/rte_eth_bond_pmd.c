...
static inline size_t get_vlan_offset(struct rte_ether_hdr *eth_hdr,
                                     uint16_t *proto) {
...
static uint16_t bond_ethdev_rx_burst(void *queue, struct rte_mbuf **bufs,
                                     uint16_t nb_pkts) {
...
static uint16_t bond_ethdev_rx_burst_active_backup(void *queue,
  ...
  return rte_eth_rx_burst(internals->current_primary_port, bd_rx_q->queue_id,
                          bufs, nb_pkts);
}

static inline uint8_t is_lacp_packets(uint16_t ethertype, uint8_t subtype,
...
int bond_ethdev_8023ad_flow_verify(struct rte_eth_dev *bond_dev,
                                   uint16_t slave_port) {
  ...
  const struct rte_flow_action actions[] = {
      {.type = RTE_FLOW_ACTION_TYPE_QUEUE, .conf = &lacp_queue_conf},
      {
          .type = RTE_FLOW_ACTION_TYPE_END,
      }};

  int ret = rte_flow_validate(slave_port, &flow_attr_8023ad, flow_item_8023ad,
                              actions, &error);
  if (ret < 0) {
    RTE_BOND_LOG(ERR, "%s: %s (slave_port=%d queue_id=%d)", __func__,
  ...
  if (ret != 0) {
    RTE_BOND_LOG(ERR, "%s: Error during getting device (port %u) info: %s\n",
...
int bond_8023ad_slow_pkt_hw_filter_supported(uint16_t port_id) {
  ...
  if (internals->slave_count > 0) {
    ...
    if (ret != 0) {
      RTE_BOND_LOG(ERR, "%s: Error during getting device (port %u) info: %s\n",
                   __func__, bond_dev->data->port_id, strerror(-ret));
...
int bond_ethdev_8023ad_flow_set(struct rte_eth_dev *bond_dev,
                                uint16_t slave_port) {
  ...
  internals->mode4.dedicated_queues.flow[slave_port] = rte_flow_create(
      slave_port, &flow_attr_8023ad, flow_item_8023ad, actions, &error);
  if (internals->mode4.dedicated_queues.flow[slave_port] == NULL) {
    RTE_BOND_LOG(ERR,
                 "bond_ethdev_8023ad_flow_set: %s "
...
static inline uint16_t rx_burst_8023ad(void *queue, struct rte_mbuf **bufs,
                                       uint16_t nb_pkts, bool dedicated_rxq) {
  /* Cast to structure, containing bonded device's port id and queue id */
  struct bond_rx_queue *bd_rx_q = (struct bond_rx_queue *)queue;
  struct bond_dev_private *internals = bd_rx_q->dev_private;
  struct rte_eth_dev *bonded_eth_dev = &rte_eth_devices[internals->port_id];
  ...
  for (i = 0; i < slave_count && num_rx_total < nb_pkts; i++) {
    j = num_rx_total;
    collecting = ACTOR_STATE(&bond_mode_8023ad_ports[slaves[idx]], COLLECTING);

    /* Read packets from this slave */
    num_rx_total +=
        rte_eth_rx_burst(slaves[idx], bd_rx_q->queue_id, &bufs[num_rx_total],
                         nb_pkts - num_rx_total);
      ...
      if (unlikely(
              ...
              (!promisc &&
               ((rte_is_unicast_ether_addr(&hdr->dst_addr) &&
                 !rte_is_same_ether_addr(bond_mac, &hdr->dst_addr)) ||
                (!allmulti && rte_is_multicast_ether_addr(&hdr->dst_addr)))))) {

        if (hdr->ether_type == ether_type_slow_be) {
          bond_mode_8023ad_handle_slow_pkt(internals, slaves[idx], bufs[j]);
        } else
        ...
        num_rx_total--;
        if (j < num_rx_total) {
          memmove(&bufs[j], &bufs[j + 1], sizeof(bufs[0]) * (num_rx_total - j));
        }
      } else
        j++;
    }
...
static uint16_t bond_ethdev_rx_burst_8023ad(void *queue, struct rte_mbuf **bufs,
                                            uint16_t nb_pkts) {
  return rx_burst_8023ad(queue, bufs, nb_pkts, false);
}

static uint16_t bond_ethdev_rx_burst_8023ad_fast_queue(void *queue,
                                                       struct rte_mbuf **bufs,
                                                       uint16_t nb_pkts) {
...
static void arp_op_name(uint16_t arp_op, char *buf, size_t buf_len) {
  switch (arp_op) {
...
static void ipv4_addr_to_dot(uint32_t be_ipv4_addr, char *buf,
                             uint8_t buf_size) {
  uint32_t ipv4_addr;
...
static void update_client_stats(uint32_t addr, uint16_t port,
                                uint32_t *TXorRXindicator) {
  int i = 0;
...
#define MODE6_DEBUG(info, src_ip, dst_ip, eth_h, arp_op, port, burstnumber)    \
  rte_log(RTE_LOG_DEBUG, bond_logtype,                                         \
          ...
          info, port, RTE_ETHER_ADDR_BYTES(&eth_h->src_addr), src_ip,          \
          RTE_ETHER_ADDR_BYTES(&eth_h->dst_addr), dst_ip, arp_op,              \
          ++burstnumber)
#endif

static void mode6_debug(const char __rte_unused *info,
  ...
  if (ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)) {
    ...
    arp_op_name(rte_be_to_cpu_16(arp_h->arp_opcode), ArpOp, sizeof(ArpOp));
...
static uint16_t bond_ethdev_rx_burst_alb(void *queue, struct rte_mbuf **bufs,
                                         uint16_t nb_pkts) {
  struct bond_rx_queue *bd_rx_q = (struct bond_rx_queue *)queue;
  struct bond_dev_private *internals = bd_rx_q->dev_private;
...
static uint16_t bond_ethdev_tx_burst_round_robin(void *queue,
                                                 struct rte_mbuf **bufs,
                                                 uint16_t nb_pkts) {
  struct bond_dev_private *internals;
  ...
  for (i = 0; i < num_of_slaves; i++) {
    if (slave_nb_pkts[i] > 0) {
      ...
      if (unlikely(num_tx_slave < slave_nb_pkts[i])) {
        ...
        memcpy(&bufs[nb_pkts - tx_fail_total], &slave_bufs[i][num_tx_slave],
...
static uint16_t bond_ethdev_tx_burst_active_backup(void *queue,
                                                   struct rte_mbuf **bufs,
                                                   uint16_t nb_pkts) {
  struct bond_dev_private *internals;
  struct bond_tx_queue *bd_tx_q;
...
static inline uint16_t ether_hash(struct rte_ether_hdr *eth_hdr) {
...
static inline uint32_t ipv4_hash(struct rte_ipv4_hdr *ipv4_hdr) {
  return ipv4_hdr->src_addr ^ ipv4_hdr->dst_addr;
}

static inline uint32_t ipv6_hash(struct rte_ipv6_hdr *ipv6_hdr) {
...
void burst_xmit_l2_hash(struct rte_mbuf **buf, uint16_t nb_pkts,
...
void burst_xmit_l23_hash(struct rte_mbuf **buf, uint16_t nb_pkts,
                         uint16_t slave_count, uint16_t *slaves) {
  uint16_t i;
  struct rte_ether_hdr *eth_hdr;
  uint16_t proto;
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4) == proto) {
      struct rte_ipv4_hdr *ipv4_hdr =
          (struct rte_ipv4_hdr *)((char *)(eth_hdr + 1) + vlan_offset);
      l3hash = ipv4_hash(ipv4_hdr);

    } else if (rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6) == proto) {
      struct rte_ipv6_hdr *ipv6_hdr =
          (struct rte_ipv6_hdr *)((char *)(eth_hdr + 1) + vlan_offset);
...
void burst_xmit_l34_hash(struct rte_mbuf **buf, uint16_t nb_pkts,
                         uint16_t slave_count, uint16_t *slaves) {
  struct rte_ether_hdr *eth_hdr;
  uint16_t proto;
  size_t vlan_offset;
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4) == proto) {
      ...
      if (likely(rte_ipv4_frag_pkt_is_fragmented(ipv4_hdr) == 0)) {
        ip_hdr_offset = (ipv4_hdr->version_ihl & RTE_IPV4_HDR_IHL_MASK) *
                        RTE_IPV4_IHL_MULTIPLIER;

        if (ipv4_hdr->next_proto_id == IPPROTO_TCP) {
          tcp_hdr = (struct rte_tcp_hdr *)((char *)ipv4_hdr + ip_hdr_offset);
          if ((size_t)tcp_hdr + sizeof(*tcp_hdr) < pkt_end)
            l4hash = HASH_L4_PORTS(tcp_hdr);
        } else if (ipv4_hdr->next_proto_id == IPPROTO_UDP) {
          udp_hdr = (struct rte_udp_hdr *)((char *)ipv4_hdr + ip_hdr_offset);
          if ((size_t)udp_hdr + sizeof(*udp_hdr) < pkt_end)
...
void bond_tlb_activate_slave(struct bond_dev_private *internals) {
  int i;
...
static int bandwidth_cmp(const void *a, const void *b) {
  ...
  int64_t diff2 =
      (int64_t)bwg_b->bwg_left_remainder - (int64_t)bwg_a->bwg_left_remainder;
  if (diff > 0)
    return 1;
  else if (diff < 0)
    return -1;
...
static void bandwidth_left(uint16_t port_id, uint64_t load, uint8_t update_idx,
  ...
  if (ret < 0) {
    RTE_BOND_LOG(ERR, "Slave (port %u) link get failed: %s", port_id,
                 rte_strerror(-ret));
    return;
  }
...
static void bond_ethdev_update_tlb_slave_cb(void *arg) {
  ...
  for (i = 0; i < internals->active_slave_count; i++) {
    ...
    bandwidth_left(slave_id, tx_bytes, internals->slave_update_idx,
                   &bwg_array[i]);
...
static uint16_t bond_ethdev_tx_burst_tlb(void *queue, struct rte_mbuf **bufs,
                                         uint16_t nb_pkts) {
  ...
  for (i = 0; i < num_of_slaves; i++) {
    ...
    for (j = num_tx_total; j < nb_pkts; j++) {
      ...
      if (rte_is_same_ether_addr(&ether_hdr->src_addr, &primary_slave_addr))
        rte_ether_addr_copy(&active_slave_addr, &ether_hdr->src_addr);
#if defined(RTE_LIBRTE_BOND_DEBUG_ALB) || defined(RTE_LIBRTE_BOND_DEBUG_ALB_L1)
    ...
    num_tx_total +=
        rte_eth_tx_burst(slaves[i], bd_tx_q->queue_id, bufs + num_tx_total,
...
void bond_tlb_disable(struct bond_dev_private *internals) {
  rte_eal_alarm_cancel(bond_ethdev_update_tlb_slave_cb, internals);
}

void bond_tlb_enable(struct bond_dev_private *internals) {
  bond_ethdev_update_tlb_slave_cb(internals);
}

static uint16_t bond_ethdev_tx_burst_alb(void *queue, struct rte_mbuf **bufs,
                                         uint16_t nb_pkts) {
  struct bond_tx_queue *bd_tx_q = (struct bond_tx_queue *)queue;
  struct bond_dev_private *internals = bd_tx_q->dev_private;
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_ARP)) {
      ...
      slave_bufs[RTE_MAX_ETHPORTS][slave_bufs_pkts[RTE_MAX_ETHPORTS]] = bufs[i];
  ...
  if (internals->mode6.ntt) {
    for (i = 0; i < ALB_HASH_TABLE_SIZE; i++) {
      ...
      if (client_info->in_use) {
        ...
        if (upd_pkt == NULL) {
          RTE_BOND_LOG(ERR, "Failed to allocate ARP packet from pool");
          continue;
        }
  ...
  for (i = 0; i < RTE_MAX_ETHPORTS; i++) {
    if (slave_bufs_pkts[i] > 0) {
      num_send = rte_eth_tx_burst(i, bd_tx_q->queue_id, slave_bufs[i],
                                  slave_bufs_pkts[i]);
      for (j = 0; j < slave_bufs_pkts[i] - num_send; j++) {
        bufs[nb_pkts - 1 - num_not_send - j] = slave_bufs[i][nb_pkts - 1 - j];
      }
  ...
  if (slave_bufs_pkts[RTE_MAX_ETHPORTS] > 0) {
    num_send = bond_ethdev_tx_burst_tlb(queue, slave_bufs[RTE_MAX_ETHPORTS],
                                        slave_bufs_pkts[RTE_MAX_ETHPORTS]);
...
static inline uint16_t tx_burst_balance(void *queue, struct rte_mbuf **bufs,
                                        uint16_t nb_bufs,
                                        uint16_t *slave_port_ids,
                                        uint16_t slave_count) {
  struct bond_tx_queue *bd_tx_q = (struct bond_tx_queue *)queue;
  struct bond_dev_private *internals = bd_tx_q->dev_private;
   ...
   */
  internals->burst_xmit_hash(bufs, nb_bufs, slave_count, bufs_slave_port_idxs);
  ...
  for (i = 0; i < slave_count; i++) {
    ...
    slave_tx_count = rte_eth_tx_burst(slave_port_ids[i], bd_tx_q->queue_id,
                                      slave_bufs[i], slave_nb_bufs[i]);

    total_tx_count += slave_tx_count;

    /* If tx burst fails move packets to end of bufs */
    if (unlikely(slave_tx_count < slave_nb_bufs[i])) {
      int slave_tx_fail_count = slave_nb_bufs[i] - slave_tx_count;
...
static uint16_t bond_ethdev_tx_burst_balance(void *queue,
                                             struct rte_mbuf **bufs,
                                             uint16_t nb_bufs) {
  struct bond_tx_queue *bd_tx_q = (struct bond_tx_queue *)queue;
  struct bond_dev_private *internals = bd_tx_q->dev_private;
  ...
  return tx_burst_balance(queue, bufs, nb_bufs, slave_port_ids, slave_count);
}

static inline uint16_t tx_burst_8023ad(void *queue, struct rte_mbuf **bufs,
                                       uint16_t nb_bufs, bool dedicated_txq) {
  struct bond_tx_queue *bd_tx_q = (struct bond_tx_queue *)queue;
  struct bond_dev_private *internals = bd_tx_q->dev_private;
  ...
  for (i = 0; i < slave_count; i++) {
    ...
    if (rte_ring_dequeue(port->tx_ring, (void **)&ctrl_pkt) != -ENOENT) {
      slave_tx_count =
          rte_eth_tx_burst(slave_port_ids[i], bd_tx_q->queue_id, &ctrl_pkt, 1);
      /*
...
skip_tx_ring:
  ...
  for (i = 0; i < slave_count; i++) {
    ...
    if (ACTOR_STATE(port, DISTRIBUTING))
      dist_slave_port_ids[dist_slave_count++] = slave_port_ids[i];
  }
...
static uint16_t bond_ethdev_tx_burst_8023ad(void *queue, struct rte_mbuf **bufs,
                                            uint16_t nb_bufs) {
  return tx_burst_8023ad(queue, bufs, nb_bufs, false);
}

static uint16_t bond_ethdev_tx_burst_8023ad_fast_queue(void *queue,
                                                       struct rte_mbuf **bufs,
                                                       uint16_t nb_bufs) {
  return tx_burst_8023ad(queue, bufs, nb_bufs, true);
}

static uint16_t bond_ethdev_tx_burst_broadcast(void *queue,
                                               struct rte_mbuf **bufs,
                                               uint16_t nb_pkts) {
  struct bond_dev_private *internals;
  struct bond_tx_queue *bd_tx_q;
  ...
  for (i = 0; i < num_of_slaves; i++) {
    slave_tx_total[i] =
        rte_eth_tx_burst(slaves[i], bd_tx_q->queue_id, bufs, nb_pkts);
...
static void link_properties_set(struct rte_eth_dev *ethdev,
...
static int link_properties_valid(struct rte_eth_dev *ethdev,
                                 struct rte_eth_link *slave_link) {
  struct bond_dev_private *bond_ctx = ethdev->data->dev_private;
...
int mac_address_get(struct rte_eth_dev *eth_dev,
...
int mac_address_set(struct rte_eth_dev *eth_dev,
 ...
 */
int slave_add_mac_addresses(struct rte_eth_dev *bonded_eth_dev,
 ...
 */
int slave_remove_mac_addresses(struct rte_eth_dev *bonded_eth_dev,
                               uint16_t slave_port_id) {
  int i, rc, ret;
  struct rte_ether_addr *mac_addr;
...
int mac_address_slaves_update(struct rte_eth_dev *bonded_eth_dev) {
  ...
  case BONDING_MODE_BROADCAST:
    for (i = 0; i < internals->slave_count; i++) {
      if (rte_eth_dev_default_mac_addr_set(internals->slaves[i].port_id,
  ...
  default:
    set = true;
    for (i = 0; i < internals->slave_count; i++) {
      if (internals->slaves[i].port_id == internals->current_primary_port) {
        if (rte_eth_dev_default_mac_addr_set(internals->current_primary_port,
                                             bonded_eth_dev->data->mac_addrs)) {
          RTE_BOND_LOG(ERR, "Failed to update port Id %d MAC address",
...
int bond_ethdev_mode_set(struct rte_eth_dev *eth_dev, uint8_t mode) {
  struct bond_dev_private *internals;
  ...
  case BONDING_MODE_8023AD:
    ...
    if (internals->mode4.dedicated_queues.enabled == 0) {
      ...
      RTE_BOND_LOG(WARNING, "Using mode 4, it is necessary to do TX burst "
                            "and RX burst at least every 100ms.");
    } else {
      /* Use flow director's optimization */
      eth_dev->rx_pkt_burst = bond_ethdev_rx_burst_8023ad_fast_queue;
      eth_dev->tx_pkt_burst = bond_ethdev_tx_burst_8023ad_fast_queue;
    }
    break;
  case BONDING_MODE_TLB:
...
static int slave_configure_slow_queue(struct rte_eth_dev *bonded_eth_dev,
  ...
  if (port->slow_pool == NULL) {
    ...
    snprintf(mem_name, RTE_DIM(mem_name), "slave_port%u_slow_pool", slave_id);
    port->slow_pool = rte_pktmbuf_pool_create(mem_name, 8191, 250, 0,
  ...
  if (internals->mode4.dedicated_queues.enabled == 1) {
    ...
    errval = rte_eth_rx_queue_setup(
        slave_eth_dev->data->port_id, internals->mode4.dedicated_queues.rx_qid,
        128, rte_eth_dev_socket_id(slave_eth_dev->data->port_id), NULL,
        port->slow_pool);
    if (errval != 0) {
      RTE_BOND_LOG(ERR, "rte_eth_rx_queue_setup: port=%d queue_id %d, err (%d)",
                   slave_eth_dev->data->port_id,
                   internals->mode4.dedicated_queues.rx_qid, errval);
      return errval;
    }

    errval = rte_eth_tx_queue_setup(
        slave_eth_dev->data->port_id, internals->mode4.dedicated_queues.tx_qid,
        512, rte_eth_dev_socket_id(slave_eth_dev->data->port_id), NULL);
    if (errval != 0) {
      RTE_BOND_LOG(ERR, "rte_eth_tx_queue_setup: port=%d queue_id %d, err (%d)",
                   slave_eth_dev->data->port_id,
                   internals->mode4.dedicated_queues.tx_qid, errval);
      return errval;
    }
  }
  return 0;
...
int slave_configure(struct rte_eth_dev *bonded_eth_dev,
                    struct rte_eth_dev *slave_eth_dev) {
  ...
  /* Configure device */
  errval =
      rte_eth_dev_configure(slave_eth_dev->data->port_id, nb_rx_queues,
                            nb_tx_queues, &(slave_eth_dev->data->dev_conf));
  if (errval != 0) {
...
int slave_start(struct rte_eth_dev *bonded_eth_dev,
                struct rte_eth_dev *slave_eth_dev) {
  int errval = 0;
  ...
  for (q_id = 0; q_id < bonded_eth_dev->data->nb_rx_queues; q_id++) {
    ...
    errval = rte_eth_rx_queue_setup(
        slave_eth_dev->data->port_id, q_id, bd_rx_q->nb_rx_desc,
  ...
  for (q_id = 0; q_id < bonded_eth_dev->data->nb_tx_queues; q_id++) {
    ...
    errval = rte_eth_tx_queue_setup(
        ...
        rte_eth_dev_socket_id(slave_eth_dev->data->port_id), &bd_tx_q->tx_conf);
    if (errval != 0) {
      RTE_BOND_LOG(ERR, "rte_eth_tx_queue_setup: port=%d queue_id %d, err (%d)",
                   slave_eth_dev->data->port_id, q_id, errval);
      return errval;
  ...
  if (internals->mode == BONDING_MODE_8023AD &&
    ...
    if (slave_configure_slow_queue(bonded_eth_dev, slave_eth_dev) != 0)
      return errval;

    errval = bond_ethdev_8023ad_flow_verify(bonded_eth_dev,
                                            slave_eth_dev->data->port_id);
    if (errval != 0) {
      RTE_BOND_LOG(ERR, "bond_ethdev_8023ad_flow_verify: port=%d, err (%d)",
                   slave_eth_dev->data->port_id, errval);
      return errval;
    }

    if (internals->mode4.dedicated_queues.flow[slave_eth_dev->data->port_id] !=
        NULL) {
      errval = rte_flow_destroy(
          slave_eth_dev->data->port_id,
    ...
    errval = bond_ethdev_8023ad_flow_set(bonded_eth_dev,
                                         slave_eth_dev->data->port_id);
    if (errval != 0) {
      RTE_BOND_LOG(ERR, "bond_ethdev_8023ad_flow_set: port=%d, err (%d)",
                   slave_eth_dev->data->port_id, errval);
      return errval;
    }
  }
  ...
  if (bonded_eth_dev->data->dev_conf.rxmode.mq_mode & RTE_ETH_MQ_RX_RSS) {
    ...
    for (i = 0; i < internals->slave_count; i++) {
      if (internals->slaves[i].port_id == slave_eth_dev->data->port_id) {
        errval = rte_eth_dev_rss_reta_update(slave_eth_dev->data->port_id,
  ...
  if (slave_eth_dev->data->dev_flags & RTE_ETH_DEV_INTR_LSC) {
    ...
    bond_ethdev_lsc_event_callback(slave_eth_dev->data->port_id,
                                   ...
                                   &bonded_eth_dev->data->port_id, NULL);
  }

  return 0;
}

void slave_remove(struct bond_dev_private *internals,
                  struct rte_eth_dev *slave_eth_dev) {
  uint16_t i;
  ...
  if (i < (internals->slave_count - 1)) {
    ...
    TAILQ_FOREACH(flow, &internals->flow_list, next) {
      memmove(&flow->flows[i], &flow->flows[i + 1],
              sizeof(flow->flows[0]) * (internals->slave_count - i - 1));
...
static void bond_ethdev_slave_link_status_change_monitor(void *cb_arg);

void slave_add(struct bond_dev_private *internals,
               struct rte_eth_dev *slave_eth_dev) {
...
void bond_ethdev_primary_set(struct bond_dev_private *internals,
                             uint16_t slave_port_id) {
  int i;
...
static int bond_ethdev_promiscuous_enable(struct rte_eth_dev *eth_dev);

static int bond_ethdev_start(struct rte_eth_dev *eth_dev) {
  struct bond_dev_private *internals;
  int i;
  ...
  if (internals->mode == BONDING_MODE_8023AD) {
    if (internals->mode4.dedicated_queues.enabled == 1) {
      internals->mode4.dedicated_queues.rx_qid = eth_dev->data->nb_rx_queues;
      internals->mode4.dedicated_queues.tx_qid = eth_dev->data->nb_tx_queues;
    }
  }
  ...
  for (i = 0; i < internals->slave_count; i++) {
    ...
    if (slave_start(eth_dev, slave_ethdev) != 0) {
      RTE_BOND_LOG(ERR, "bonded port (%d) failed to start slave device (%d)",
                   eth_dev->data->port_id, internals->slaves[i].port_id);
      goto out_err;
    }
...
static void bond_ethdev_free_queues(struct rte_eth_dev *dev) {
  uint16_t i;
...
int bond_ethdev_stop(struct rte_eth_dev *eth_dev) {
  ...
  for (i = 0; i < internals->slave_count; i++) {
    ...
    if (find_slave_by_id(internals->active_slaves,
                         ...
                         slave_id) != internals->active_slave_count) {
      internals->slaves[i].last_link_status = 0;
      ret = rte_eth_dev_stop(slave_id);
      if (ret != 0) {
        RTE_BOND_LOG(ERR, "Failed to stop device on port %u", slave_id);
        return ret;
      }
...
int bond_ethdev_close(struct rte_eth_dev *dev) {
  ...
  while (internals->slave_count != skipped) {
    ...
    if (rte_eth_dev_stop(port_id) != 0) {
      RTE_BOND_LOG(ERR, "Failed to stop device on port %u", port_id);
      skipped++;
    }

    if (rte_eth_bond_slave_remove(bond_port_id, port_id) != 0) {
      RTE_BOND_LOG(ERR, "Failed to remove port %d from bonded device %s",
...
static int bond_ethdev_info(struct rte_eth_dev *dev,
    ...
    for (idx = 0; idx < internals->slave_count; idx++) {
      ...
      if (ret != 0) {
        RTE_BOND_LOG(ERR,
                     ...
                     __func__, slave.port_id, strerror(-ret));
...
static int bond_ethdev_vlan_filter_set(struct rte_eth_dev *dev,
...
static int bond_ethdev_rx_queue_setup(struct rte_eth_dev *dev,
                                      uint16_t rx_queue_id, uint16_t nb_rx_desc,
                                      unsigned int socket_id __rte_unused,
                                      const struct rte_eth_rxconf *rx_conf,
                                      struct rte_mempool *mb_pool) {
  struct bond_rx_queue *bd_rx_q = (struct bond_rx_queue *)rte_zmalloc_socket(
      NULL, sizeof(struct bond_rx_queue), 0, dev->data->numa_node);
  if (bd_rx_q == NULL)
    return -1;
...
static int bond_ethdev_tx_queue_setup(struct rte_eth_dev *dev,
                                      uint16_t tx_queue_id, uint16_t nb_tx_desc,
                                      unsigned int socket_id __rte_unused,
                                      const struct rte_eth_txconf *tx_conf) {
  struct bond_tx_queue *bd_tx_q = (struct bond_tx_queue *)rte_zmalloc_socket(
      NULL, sizeof(struct bond_tx_queue), 0, dev->data->numa_node);
...
static void bond_ethdev_rx_queue_release(struct rte_eth_dev *dev,
                                         uint16_t queue_id) {
...
static void bond_ethdev_tx_queue_release(struct rte_eth_dev *dev,
                                         uint16_t queue_id) {
...
static void bond_ethdev_slave_link_status_change_monitor(void *cb_arg) {
  ...
  if (rte_spinlock_trylock(&internals->lock)) {
    ...
    for (i = 0; i < internals->slave_count; i++) {
      ...
      (*slave_ethdev->dev_ops->link_update)(
          slave_ethdev, internals->slaves[i].link_status_wait_to_complete);
...
static int bond_ethdev_link_update(struct rte_eth_dev *ethdev,
  ...
  if (ethdev->data->dev_started == 0 || bond_ctx->active_slave_count == 0) {
  ...
  case BONDING_MODE_BROADCAST:
    ...
    for (idx = 0; idx < bond_ctx->active_slave_count; idx++) {
      ret = link_update(bond_ctx->active_slaves[idx], &slave_link);
      if (ret < 0) {
        ethdev->data->dev_link.link_speed = RTE_ETH_SPEED_NUM_NONE;
        RTE_BOND_LOG(ERR, "Slave (port %u) link get failed: %s",
                     bond_ctx->active_slaves[idx], rte_strerror(-ret));
        return 0;
      }

      if (slave_link.link_speed < ethdev->data->dev_link.link_speed)
  ...
  case BONDING_MODE_ACTIVE_BACKUP:
    ...
    if (ret < 0) {
      RTE_BOND_LOG(ERR, "Slave (port %u) link get failed: %s",
                   bond_ctx->current_primary_port, rte_strerror(-ret));
      return 0;
    }
  ...
  case BONDING_MODE_8023AD:
    ...
    ethdev->data->dev_link.link_duplex = bond_ctx->mode4.slave_link.link_duplex;
    /* fall through */
...
static int bond_ethdev_stats_get(struct rte_eth_dev *dev,
...
static int bond_ethdev_stats_reset(struct rte_eth_dev *dev) {
  struct bond_dev_private *internals = dev->data->dev_private;
  int i;
  int err;
  int ret;
...
static int bond_ethdev_promiscuous_disable(struct rte_eth_dev *dev) {
  struct bond_dev_private *internals = dev->data->dev_private;
  int i;
  int ret = 0;
  uint16_t port_id;
...
static int bond_ethdev_promiscuous_update(struct rte_eth_dev *dev) {
  struct bond_dev_private *internals = dev->data->dev_private;
...
static int bond_ethdev_allmulticast_enable(struct rte_eth_dev *eth_dev) {
  struct bond_dev_private *internals = eth_dev->data->dev_private;
  int i;
  int ret = 0;
  uint16_t port_id;
...
static int bond_ethdev_allmulticast_disable(struct rte_eth_dev *eth_dev) {
  struct bond_dev_private *internals = eth_dev->data->dev_private;
  int i;
  int ret = 0;
  uint16_t port_id;
...
static int bond_ethdev_allmulticast_update(struct rte_eth_dev *dev) {
  struct bond_dev_private *internals = dev->data->dev_private;
  uint16_t port_id = internals->current_primary_port;
...
static void bond_ethdev_delayed_lsc_propagation(void *arg) {
  if (arg == NULL)
    return;
...
int bond_ethdev_lsc_event_callback(uint16_t port_id,
  ...
  if (ret == 0 && link.link_status) {
    ...
    if (internals->active_slave_count < 1) {
      ...
      bonded_eth_dev->data->dev_link.link_status = RTE_ETH_LINK_UP;
    ...
    if (port_id == internals->current_primary_port) {
      if (internals->active_slave_count > 0)
        bond_ethdev_primary_set(internals, internals->active_slaves[0]);
      else
...
link_update:
  ...
  if (lsc_flag) {
    ...
    if (internals->link_up_delay_ms > 0 || internals->link_down_delay_ms > 0)
      rte_eal_alarm_cancel(bond_ethdev_delayed_lsc_propagation, bonded_eth_dev);
    ...
    if (bonded_eth_dev->data->dev_link.link_status) {
      if (internals->link_up_delay_ms > 0)
        ...
        rte_eth_dev_callback_process(bonded_eth_dev, RTE_ETH_EVENT_INTR_LSC,
                                     NULL);
...
bond_ethdev_rss_reta_update(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < internals->slave_count; i++) {
    ...
    result =
        rte_eth_dev_rss_reta_update(internals->slaves[i].port_id,
...
static int bond_ethdev_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (bond_rss_conf.rss_key) {
    ...
    memcpy(internals->rss_key, bond_rss_conf.rss_key, internals->rss_key_len);
...
static int bond_ethdev_rss_hash_conf_get(struct rte_eth_dev *dev,
                                         struct rte_eth_rss_conf *rss_conf) {
  struct bond_dev_private *internals = dev->data->dev_private;
...
static int bond_ethdev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct rte_eth_dev *slave_eth_dev;
  struct bond_dev_private *internals = dev->data->dev_private;
  int ret, i;
...
static int bond_ethdev_mac_address_set(struct rte_eth_dev *dev,
...
static int bond_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
                             const struct rte_flow_ops **ops) {
  *ops = &bond_flow_ops;
  return 0;
}

static int bond_ethdev_mac_addr_add(struct rte_eth_dev *dev,
                                    struct rte_ether_addr *mac_addr,
  ...
  for (i = 0; i < internals->slave_count; i++) {
    ret =
        rte_eth_dev_mac_addr_add(internals->slaves[i].port_id, mac_addr, vmdq);
    if (ret < 0) {
      /* rollback */
      for (i--; i >= 0; i--)
        rte_eth_dev_mac_addr_remove(internals->slaves[i].port_id, mac_addr);
      goto end;
    }
  }
...
static void bond_ethdev_mac_addr_remove(struct rte_eth_dev *dev,
                                        uint32_t index) {
  struct rte_eth_dev *slave_eth_dev;
  struct bond_dev_private *internals = dev->data->dev_private;
  int i;
...
static int bond_alloc(struct rte_vdev_device *dev, uint8_t mode) {
  ...
  eth_dev->data->mac_addrs = rte_zmalloc_socket(
      name, RTE_ETHER_ADDR_LEN * BOND_MAX_MAC_ADDRS, 0, socket_id);
  ...
  eth_dev->data->dev_flags =
      RTE_ETH_DEV_INTR_LSC | RTE_ETH_DEV_AUTOFILL_QUEUE_XSTATS;
  ...
  memset(&internals->default_rxconf, 0, sizeof(internals->default_rxconf));
  memset(&internals->default_txconf, 0, sizeof(internals->default_txconf));
  ...
  internals->vlan_filter_bmpmem =
      rte_malloc(name, vlan_filter_bmp_size, RTE_CACHE_LINE_SIZE);
  if (internals->vlan_filter_bmpmem == NULL) {
    RTE_BOND_LOG(ERR, "Failed to allocate vlan bitmap for bonded device %u",
                 eth_dev->data->port_id);
    goto err;
  }

  internals->vlan_filter_bmp =
      rte_bitmap_init(RTE_ETHER_MAX_VLAN_ID + 1, internals->vlan_filter_bmpmem,
                      vlan_filter_bmp_size);
  if (internals->vlan_filter_bmp == NULL) {
    RTE_BOND_LOG(ERR, "Failed to init vlan bitmap for bonded device %u",
                 eth_dev->data->port_id);
    rte_free(internals->vlan_filter_bmpmem);
    goto err;
  }
...
static int bond_probe(struct rte_vdev_device *dev) {
  const char *name;
  struct bond_dev_private *internals;
  ...
  if (rte_kvargs_count(kvlist, PMD_BOND_MODE_KVARG) == 1) {
    if (rte_kvargs_process(kvlist, PMD_BOND_MODE_KVARG,
                           ...
                           &bonding_mode) != 0) {
      RTE_BOND_LOG(ERR, "Invalid mode for bonded device %s", name);
      goto parse_error;
    }
  } else {
    RTE_BOND_LOG(ERR,
                 "Mode must be specified only once for bonded "
                 "device %s",
                 name);
    goto parse_error;
  }
  ...
  if (arg_count == 1) {
    if (rte_kvargs_process(kvlist, PMD_BOND_SOCKET_ID_KVARG,
                           &bond_ethdev_parse_socket_id_kvarg,
                           &socket_id) != 0) {
      RTE_BOND_LOG(ERR,
                   "Invalid socket Id specified for "
                   "bonded device %s",
                   name);
      goto parse_error;
    }
    ...
    RTE_BOND_LOG(ERR,
                 "Socket Id can be specified only once for "
                 "bonded device %s",
                 name);
    goto parse_error;
  } else {
  ...
  if (port_id < 0) {
    RTE_BOND_LOG(ERR,
                 "Failed to create socket %s in mode %u on "
                 "socket %u.",
  ...
  if (rte_kvargs_count(kvlist, PMD_BOND_AGG_MODE_KVARG) == 1) {
    if (rte_kvargs_process(kvlist, PMD_BOND_AGG_MODE_KVARG,
                           ...
                           &agg_mode) != 0) {
      RTE_BOND_LOG(
          ERR, "Failed to parse agg selection mode for bonded device %s", name);
      goto parse_error;
    }
  ...
  RTE_BOND_LOG(INFO,
               "Create bonded device %s on port %d in mode %u on "
               "socket %u.",
...
static int bond_remove(struct rte_vdev_device *dev) {
  struct rte_eth_dev *eth_dev;
  struct bond_dev_private *internals;
  const char *name;
  int ret = 0;
...
static int bond_ethdev_configure(struct rte_eth_dev *dev) {
  ...
  if (dev->data->dev_conf.rxmode.mq_mode & RTE_ETH_MQ_RX_RSS) {
    ...
    if (rss_conf->rss_key != NULL) {
      if (internals->rss_key_len > rss_conf->rss_key_len) {
        RTE_BOND_LOG(ERR, "Invalid rss key length(%u)", rss_conf->rss_key_len);
        return -EINVAL;
      }

      memcpy(internals->rss_key, rss_conf->rss_key, internals->rss_key_len);
    } else {
      if (internals->rss_key_len > sizeof(default_rss_key)) {
        RTE_BOND_LOG(ERR, "There is no suitable default hash key");
        return -EINVAL;
      }
    ...
    for (i = 0; i < RTE_DIM(internals->reta_conf); i++) {
      ...
      for (j = 0; j < RTE_ETH_RETA_GROUP_SIZE; j++)
        internals->reta_conf[i].reta[j] =
            (i * RTE_ETH_RETA_GROUP_SIZE + j) % dev->data->nb_rx_queues;
    }
  }
  ...
  if (arg_count == 1) {
    ...
    if (rte_kvargs_process(kvlist, PMD_BOND_MAC_ADDR_KVARG,
                           &bond_ethdev_parse_bond_mac_addr_kvarg,
                           &bond_mac) < 0) {
      RTE_BOND_LOG(INFO, "Invalid mac address for bonded device %s", name);
      return -1;
    }

    /* Set MAC address */
    if (rte_eth_bond_mac_address_set(port_id, &bond_mac) != 0) {
      RTE_BOND_LOG(ERR, "Failed to set mac address on bonded device %s", name);
      return -1;
    }
  } else if (arg_count > 1) {
    RTE_BOND_LOG(ERR,
  ...
  if (arg_count == 1) {
    ...
    if (rte_kvargs_process(kvlist, PMD_BOND_XMIT_POLICY_KVARG,
                           ...
                           &xmit_policy) != 0) {
      RTE_BOND_LOG(INFO, "Invalid xmit policy specified for bonded device %s",
                   name);
      return -1;
    }
    ...
    if (rte_eth_bond_xmit_policy_set(port_id, xmit_policy) != 0) {
      RTE_BOND_LOG(ERR, "Failed to set balance xmit policy on bonded device %s",
                   name);
      return -1;
    }
  } else if (arg_count > 1) {
    RTE_BOND_LOG(
        ERR, "Transmit policy can be specified only once for bonded device %s",
        name);
    return -1;
  }
  ...
  if (rte_kvargs_count(kvlist, PMD_BOND_AGG_MODE_KVARG) == 1) {
    ...
    if (internals->mode == BONDING_MODE_8023AD) {
      int ret = rte_eth_bond_8023ad_agg_selection_set(port_id, agg_mode);
      if (ret < 0) {
        RTE_BOND_LOG(ERR,
  ...
  if (rte_kvargs_count(kvlist, PMD_BOND_SLAVE_PORT_KVARG) > 0) {
    ...
    if (rte_kvargs_process(kvlist, PMD_BOND_SLAVE_PORT_KVARG,
                           &bond_ethdev_parse_slave_port_kvarg,
                           &slave_ports) != 0) {
      RTE_BOND_LOG(ERR, "Failed to parse slave ports for bonded device %s",
                   name);
      return -1;
    }
    ...
    for (i = 0; i < slave_ports.slave_count; i++) {
      if (rte_eth_bond_slave_add(port_id, slave_ports.slaves[i]) != 0) {
        RTE_BOND_LOG(ERR, "Failed to add port %d as slave to bonded device %s",
  ...
  if (arg_count == 1) {
    ...
    if (rte_kvargs_process(kvlist, PMD_BOND_PRIMARY_SLAVE_KVARG,
                           &bond_ethdev_parse_primary_slave_port_id_kvarg,
                           &primary_slave_port_id) < 0) {
      RTE_BOND_LOG(
          INFO, "Invalid primary slave port id specified for bonded device %s",
          name);
      return -1;
    }

    /* Set balance mode transmit policy*/
    if (rte_eth_bond_primary_set(port_id, primary_slave_port_id) != 0) {
      RTE_BOND_LOG(ERR,
    ...
    RTE_BOND_LOG(
        INFO, "Primary slave can be specified only once for bonded device %s",
        name);
    return -1;
  }
  ...
  if (arg_count == 1) {
    ...
    if (rte_kvargs_process(kvlist, PMD_BOND_LSC_POLL_PERIOD_KVARG,
    ...
    if (rte_eth_bond_link_monitoring_set(port_id, lsc_poll_interval_ms) != 0) {
      RTE_BOND_LOG(ERR,
  ...
  if (arg_count == 1) {
    ...
    if (rte_kvargs_process(kvlist, PMD_BOND_LINK_UP_PROP_DELAY_KVARG,
                           &bond_ethdev_parse_time_ms_kvarg,
                           &link_up_delay_ms) < 0) {
      RTE_BOND_LOG(INFO,
    ...
    /* Set balance mode transmit policy*/
    if (rte_eth_bond_link_up_prop_delay_set(port_id, link_up_delay_ms) != 0) {
      RTE_BOND_LOG(ERR,
  ...
  if (arg_count == 1) {
    ...
    if (rte_kvargs_process(kvlist, PMD_BOND_LINK_DOWN_PROP_DELAY_KVARG,
                           &bond_ethdev_parse_time_ms_kvarg,
  ...
  for (i = 0; i < internals->slave_count; i++) {
    ...
    if (slave_configure(dev, slave_ethdev) != 0) {
      RTE_BOND_LOG(ERR,
                   ...
                   dev->data->port_id, internals->slaves[i].port_id);
      return -1;
    }
  }
  return 0;
