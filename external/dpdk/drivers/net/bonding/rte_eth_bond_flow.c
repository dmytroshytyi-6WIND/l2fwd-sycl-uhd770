...
static struct rte_flow *bond_flow_alloc(int numa_node,
  ...
  ret = rte_flow_conv(RTE_FLOW_CONV_OP_RULE, &flow->rule, ret, &rule, &error);
  if (ret < 0) {
...
static void bond_flow_release(struct rte_flow **flow) {
  rte_free(*flow);
  *flow = NULL;
}

static int bond_flow_validate(struct rte_eth_dev *dev,
                              const struct rte_flow_attr *attr,
  ...
  for (i = 0; i < internals->slave_count; i++) {
    ret = rte_flow_validate(internals->slaves[i].port_id, attr, patterns,
                            actions, err);
    if (ret) {
      RTE_BOND_LOG(ERR,
                   "Operation rte_flow_validate failed"
                   " for slave %d with error %d",
                   i, ret);
      return ret;
    }
  }
...
static struct rte_flow *bond_flow_create(struct rte_eth_dev *dev,
                                         const struct rte_flow_attr *attr,
                                         const struct rte_flow_item patterns[],
                                         const struct rte_flow_action actions[],
                                         struct rte_flow_error *err) {
  ...
  if (unlikely(flow == NULL)) {
    rte_flow_error_set(err, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                       rte_strerror(ENOMEM));
    return NULL;
  }
  for (i = 0; i < internals->slave_count; i++) {
    flow->flows[i] = rte_flow_create(internals->slaves[i].port_id, attr,
                                     patterns, actions, err);
    if (unlikely(flow->flows[i] == NULL)) {
      RTE_BOND_LOG(ERR, "Failed to create flow on slave %d", i);
      goto err;
    }
  }
...
err:
  ...
  for (i = 0; i < internals->slave_count; i++) {
    if (flow->flows[i] != NULL)
      rte_flow_destroy(internals->slaves[i].port_id, flow->flows[i], err);
  }
  bond_flow_release(&flow);
  return NULL;
}

static int bond_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                             struct rte_flow_error *err) {
  struct bond_dev_private *internals = dev->data->dev_private;
  int i;
  int ret = 0;
  ...
  for (i = 0; i < internals->slave_count; i++) {
    ...
    if (unlikely(flow->flows[i] == NULL))
      continue;
    lret = rte_flow_destroy(internals->slaves[i].port_id, flow->flows[i], err);
    if (unlikely(lret != 0)) {
      RTE_BOND_LOG(ERR,
...
static int bond_flow_flush(struct rte_eth_dev *dev,
                           struct rte_flow_error *err) {
  struct bond_dev_private *internals = dev->data->dev_private;
  struct rte_flow *flow;
  void *tmp;
...
static int bond_flow_query_count(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  for (i = 0; i < internals->slave_count; i++) {
    ret = rte_flow_query(internals->slaves[i].port_id, flow->flows[i], action,
                         &slave_count, err);
    if (unlikely(ret != 0)) {
      RTE_BOND_LOG(ERR,
...
static int bond_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  default:
    return rte_flow_error_set(err, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, arg,
                              rte_strerror(ENOTSUP));
  }
}

static int bond_flow_isolate(struct rte_eth_dev *dev, int set,
                             struct rte_flow_error *err) {
  struct bond_dev_private *internals = dev->data->dev_private;
  int i;
  int ret;
