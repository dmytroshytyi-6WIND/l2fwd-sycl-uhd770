...
int check_for_bonded_ethdev(const struct rte_eth_dev *eth_dev) {
  /* Check valid pointer */
  if (eth_dev == NULL || eth_dev->device == NULL ||
      eth_dev->device->driver == NULL || eth_dev->device->driver->name == NULL)
    return -1;
...
int valid_bonded_port_id(uint16_t port_id) {
  RTE_ETH_VALID_PORTID_OR_ERR_RET(port_id, -1);
  return check_for_bonded_ethdev(&rte_eth_devices[port_id]);
}

int check_for_master_bonded_ethdev(const struct rte_eth_dev *eth_dev) {
  int i;
...
int valid_slave_port_id(struct bond_dev_private *internals,
  ...
  if (internals->port_id == slave_port_id) {
    RTE_BOND_LOG(ERR, "Cannot add the bonded device itself as its slave.");
    return -1;
  }
...
void activate_slave(struct rte_eth_dev *eth_dev, uint16_t port_id) {
...
void deactivate_slave(struct rte_eth_dev *eth_dev, uint16_t port_id) {
  uint16_t slave_pos;
  struct bond_dev_private *internals = eth_dev->data->dev_private;
  uint16_t active_count = internals->active_slave_count;
  ...
  slave_pos = find_slave_by_id(internals->active_slaves, active_count, port_id);
  ...
  if (slave_pos < active_count) {
    ...
    memmove(internals->active_slaves + slave_pos,
            ...
            (active_count - slave_pos) * sizeof(internals->active_slaves[0]));
  }
...
int rte_eth_bond_free(const char *name) { return rte_vdev_uninit(name); }

static int slave_vlan_filter_set(uint16_t bonded_port_id,
                                 uint16_t slave_port_id) {
  ...
  do {
    ...
    for (i = 0, mask = 1; i < RTE_BITMAP_SLAB_BIT_SIZE; i++, mask <<= 1) {
      if (unlikely(slab & mask)) {
        ...
        res = rte_eth_dev_vlan_filter(slave_port_id, vlan_id, 1);
      }
    }
    found = rte_bitmap_scan(internals->vlan_filter_bmp, &pos, &slab);
...
static int slave_rte_flow_prepare(uint16_t slave_id,
                                  struct bond_dev_private *internals) {
  ...
  if (internals->flow_isolated_valid != 0) {
    if (rte_eth_dev_stop(slave_port_id) != 0) {
      RTE_BOND_LOG(ERR, "Failed to stop device on port %u", slave_port_id);
      return -1;
    }

    if (rte_flow_isolate(slave_port_id, internals->flow_isolated, &ferror)) {
      RTE_BOND_LOG(ERR,
                   "rte_flow_isolate failed for slave"
                   " %d: %s",
                   slave_id,
                   ferror.message ? ferror.message : "(no stated reason)");
      return -1;
    }
  }
  TAILQ_FOREACH(flow, &internals->flow_list, next) {
    flow->flows[slave_id] =
        rte_flow_create(slave_port_id, flow->rule.attr, flow->rule.pattern,
                        flow->rule.actions, &ferror);
    if (flow->flows[slave_id] == NULL) {
      RTE_BOND_LOG(ERR,
                   "Cannot create flow for slave"
                   " %d: %s",
                   slave_id,
                   ferror.message ? ferror.message : "(no stated reason)");
      ...
      TAILQ_FOREACH(flow, &internals->flow_list, next) {
        if (flow->flows[slave_id] != NULL) {
          rte_flow_destroy(slave_port_id, flow->flows[slave_id], &ferror);
...
static void eth_bond_slave_inherit_desc_lim_first(
    struct rte_eth_desc_lim *bond_desc_lim,
    const struct rte_eth_desc_lim *slave_desc_lim) {
  memcpy(bond_desc_lim, slave_desc_lim, sizeof(*bond_desc_lim));
}

static int eth_bond_slave_inherit_desc_lim_next(
    struct rte_eth_desc_lim *bond_desc_lim,
    const struct rte_eth_desc_lim *slave_desc_lim) {
  bond_desc_lim->nb_max =
      RTE_MIN(bond_desc_lim->nb_max, slave_desc_lim->nb_max);
  bond_desc_lim->nb_min =
      RTE_MAX(bond_desc_lim->nb_min, slave_desc_lim->nb_min);
  bond_desc_lim->nb_align =
      RTE_MAX(bond_desc_lim->nb_align, slave_desc_lim->nb_align);
...
static int __eth_bond_slave_add_lock_free(uint16_t bonded_port_id,
                                          uint16_t slave_port_id) {
  ...
  if (dev_info.max_rx_pktlen < internals->max_rx_pktlen) {
    RTE_BOND_LOG(ERR, "Slave (port %u) max_rx_pktlen too small", slave_port_id);
    return -1;
  }
  ...
  if (internals->slave_count < 1) {
    ...
    if (!internals->user_defined_mac) {
      if (mac_address_set(bonded_eth_dev, slave_eth_dev->data->mac_addrs)) {
    ...
    ret = eth_bond_slave_inherit_desc_lim_next(&internals->rx_desc_lim,
                                               &dev_info.rx_desc_lim);
    if (ret != 0)
      return ret;

    ret = eth_bond_slave_inherit_desc_lim_next(&internals->tx_desc_lim,
                                               &dev_info.tx_desc_lim);
    if (ret != 0)
      return ret;
  }
  ...
  if (bonded_eth_dev->data->dev_started) {
    if (slave_configure(bonded_eth_dev, slave_eth_dev) != 0) {
      internals->slave_count--;
      RTE_BOND_LOG(ERR, "rte_bond_slaves_configure: port=%d", slave_port_id);
      return -1;
    }
    if (slave_start(bonded_eth_dev, slave_eth_dev) != 0) {
      internals->slave_count--;
      RTE_BOND_LOG(ERR, "rte_bond_slaves_start: port=%d", slave_port_id);
      return -1;
    }
  }
...
static int __eth_bond_slave_remove_lock_free(uint16_t bonded_port_id,
                                             uint16_t slave_port_id) {
  struct rte_eth_dev *bonded_eth_dev;
  struct bond_dev_private *internals;
  ...
  rte_eth_dev_callback_unregister(
      slave_port_id, RTE_ETH_EVENT_INTR_LSC, bond_ethdev_lsc_event_callback,
      &rte_eth_devices[bonded_port_id].data->port_id);

  /* Restore original MAC address of slave device */
  rte_eth_dev_default_mac_addr_set(
      slave_port_id, &(internals->slaves[slave_idx].persisted_mac_addr));
  ...
  TAILQ_FOREACH(flow, &internals->flow_list, next) {
    if (flow->flows[slave_idx] != NULL) {
      rte_flow_destroy(slave_port_id, flow->flows[slave_idx], &flow_error);
...
int rte_eth_bond_slaves_get(uint16_t bonded_port_id, uint16_t slaves[],
                            uint16_t len) {
  struct bond_dev_private *internals;
  uint16_t i;
...
int rte_eth_bond_active_slaves_get(uint16_t bonded_port_id, uint16_t slaves[],
                                   uint16_t len) {
  struct bond_dev_private *internals;
...
int rte_eth_bond_mac_address_set(uint16_t bonded_port_id,
...
int rte_eth_bond_mac_address_reset(uint16_t bonded_port_id) {
  ...
  if (internals->slave_count > 0) {
     ...
     */
    for (slave_port = 0; slave_port < internals->slave_count; slave_port++) {
      if (internals->slaves[slave_port].port_id == internals->primary_port)
        break;
    }
...
int rte_eth_bond_link_monitoring_set(uint16_t bonded_port_id,
...
int rte_eth_bond_link_down_prop_delay_set(uint16_t bonded_port_id,
                                          uint32_t delay_ms)
...
int rte_eth_bond_link_up_prop_delay_set(uint16_t bonded_port_id,
                                        uint32_t delay_ms)
