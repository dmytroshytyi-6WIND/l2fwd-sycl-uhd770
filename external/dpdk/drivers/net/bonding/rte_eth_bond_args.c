...
const char *pmd_bond_init_valid_arguments[] = {PMD_BOND_SLAVE_PORT_KVARG,
...
static inline int bond_pci_addr_cmp(const struct rte_device *dev,
...
static inline int find_port_id_by_dev_name(const char *name) {
  unsigned i;
 ...
 * and finally port id.
 */
static inline int parse_port_id(const char *port_str) {
...
int bond_ethdev_parse_slave_port_kvarg(const char *key, const char *value,
  ...
  if (strcmp(key, PMD_BOND_SLAVE_PORT_KVARG) == 0) {
    ...
    if (port_id < 0) {
      ...
      slave_ports->slaves[slave_ports->slave_count++] = port_id;
  }
  return 0;
}

int bond_ethdev_parse_slave_mode_kvarg(const char *key __rte_unused,
...
int bond_ethdev_parse_socket_id_kvarg(const char *key __rte_unused,
                                      const char *value, void *extra_args) {
  long socket_id;
  char *endptr;
...
int bond_ethdev_parse_primary_slave_port_id_kvarg(const char *key __rte_unused,
                                                  const char *value,
                                                  void *extra_args) {
...
int bond_ethdev_parse_balance_xmit_policy_kvarg(const char *key __rte_unused,
                                                const char *value,
                                                void *extra_args) {
  uint8_t *xmit_policy;
...
int bond_ethdev_parse_bond_mac_addr_kvarg(const char *key __rte_unused,
                                          const char *value, void *extra_args) {
  if (value == NULL || extra_args == NULL)
    return -1;
...
int bond_ethdev_parse_time_ms_kvarg(const char *key __rte_unused,
                                    const char *value, void *extra_args) {
  uint32_t time_ms;
  char *endptr;
