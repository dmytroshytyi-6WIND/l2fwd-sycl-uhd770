...
#define MODE4_DEBUG(fmt, ...)                                                  \
  rte_log(RTE_LOG_DEBUG, bond_logtype, "%6u [Port %u: %s] " fmt,               \
          bond_dbg_get_time_diff_ms(), slave_id, __func__, ##__VA_ARGS__)
...
static void show_warnings(uint16_t slave_id) {
  ...
  if (warnings & WRN_RX_MARKER_TO_FAST)
    RTE_BOND_LOG(INFO, "Slave %u: marker to early - ignoring.", slave_id);

  if (warnings & WRN_UNKNOWN_SLOW_TYPE) {
    RTE_BOND_LOG(INFO, "Slave %u: ignoring unknown slow protocol frame type",
                 slave_id);
  }
 ...
 */
static void rx_machine(struct bond_dev_private *internals, uint16_t slave_id,
                       struct lacpdu *lacp) {
 ...
 * @param port			Port to handle state machine.
 */
static void mux_machine(struct bond_dev_private *internals, uint16_t slave_id) {
  struct port *port = &bond_mode_8023ad_ports[slave_id];
  ...
  if (!ACTOR_STATE(port, SYNCHRONIZATION)) {
    ...
    if (!ACTOR_STATE(port, DISTRIBUTING)) {
      if (PARTNER_STATE(port, COLLECTING)) {
        ...
        RTE_BOND_LOG(INFO, "Bond %u: slave id %u distributing started.",
      ...
      if (!PARTNER_STATE(port, COLLECTING)) {
        ...
        RTE_BOND_LOG(INFO, "Bond %u: slave id %u distributing stopped.",
                     internals->port_id, slave_id);
      }
    }
  }
 ...
 * @param port
 */
static void tx_machine(struct bond_dev_private *internals, uint16_t slave_id) {
  struct port *agg, *port = &bond_mode_8023ad_ports[slave_id];
  ...
  if (internals->mode4.dedicated_queues.enabled == 0) {
    ...
    uint16_t pkts_sent = rte_eth_tx_burst(
        slave_id, internals->mode4.dedicated_queues.tx_qid, &lacp_pkt, 1);
    if (pkts_sent != 1) {
      rte_pktmbuf_free(lacp_pkt);
      set_warning_flags(port, WRN_TX_QUEUE_FULL);
      return;
...
static uint16_t max_index(uint64_t *a, int n) {
  if (n <= 0)
    return -1;
...
static void rx_machine_update(struct bond_dev_private *internals,
                              uint16_t slave_id, struct rte_mbuf *lacp_pkt) {
...
#define DEDICATED_QUEUE_BURST_SIZE 32
  ...
  uint16_t rx_count =
      rte_eth_rx_burst(slave_id, internals->mode4.dedicated_queues.rx_qid,
  ...
  if (rx_count) {
    ...
    for (i = 0; i < rx_count; i++)
      bond_mode_8023ad_handle_slow_pkt(internals, slave_id, lacp_pkt[i]);
  } else {
...
static void bond_mode_8023ad_periodic_cb(void *arg) {
  ...
  for (i = 0; i < internals->active_slave_count; i++) {
    ...
    if (internals->mode4.dedicated_queues.enabled == 0) {
      ...
      bond_mode_8023ad_dedicated_rxq_process(internals, slave_id);
    }
...
static int bond_mode_8023ad_register_lacp_mac(uint16_t slave_id) {
  int ret;
  ...
  if (rte_eth_allmulticast_get(slave_id)) {
    RTE_BOND_LOG(DEBUG, "forced allmulti for port %u", slave_id);
  ...
  if (rte_eth_promiscuous_get(slave_id)) {
    RTE_BOND_LOG(DEBUG, "forced promiscuous for port %u", slave_id);
    bond_mode_8023ad_ports[slave_id].forced_rx_flags =
...
static void bond_mode_8023ad_unregister_lacp_mac(uint16_t slave_id) {
  int ret;
...
void bond_mode_8023ad_activate_slave(struct rte_eth_dev *bond_dev,
                                     uint16_t slave_id) {
  struct bond_dev_private *internals = bond_dev->data->dev_private;
  ...
  if (bond_mode_8023ad_register_lacp_mac(slave_id) < 0) {
    RTE_BOND_LOG(
        WARNING,
        "slave %u is most likely broken and won't receive LACP packets",
        slave_id);
  }
  ...
  port->rx_ring = rte_ring_create(
      mem_name, rte_align32pow2(BOND_MODE_8023AX_SLAVE_RX_PKTS), socket_id, 0);
  ...
  port->tx_ring = rte_ring_create(
      mem_name, rte_align32pow2(BOND_MODE_8023AX_SLAVE_TX_PKTS + 1), socket_id,
      0);
...
int bond_mode_8023ad_deactivate_slave(struct rte_eth_dev *bond_dev __rte_unused,
                                      uint16_t slave_id) {
  void *pkt = NULL;
...
void bond_mode_8023ad_handle_slow_pkt(struct bond_dev_private *internals,
  ...
  if (subtype == SLOW_SUBTYPE_MARKER) {
    ...
    if (internals->mode4.dedicated_queues.enabled == 0) {
      ...
      uint16_t tx_count = rte_eth_tx_burst(
          slave_id, internals->mode4.dedicated_queues.tx_qid, &pkt, 1);
      if (tx_count != 1) {
        /* reset timer */
        port->rx_marker_timer = 0;
        wrn = WRN_TX_QUEUE_FULL;
...
int rte_eth_bond_8023ad_slave_info(
    uint16_t port_id, uint16_t slave_id,
  ...
  internals = bond_dev->data->dev_private;
  if (find_slave_by_id(internals->active_slaves, internals->active_slave_count,
                       slave_id) == internals->active_slave_count)
    return -EINVAL;
...
static int bond_8023ad_ext_validate(uint16_t port_id, uint16_t slave_id) {
  struct rte_eth_dev *bond_dev;
  struct bond_dev_private *internals;
  struct mode8023ad_private *mode4;
...
int rte_eth_bond_8023ad_ext_collect(uint16_t port_id, uint16_t slave_id,
                                    int enabled) {
  struct port *port;
  int res;
...
int rte_eth_bond_8023ad_ext_distrib(uint16_t port_id, uint16_t slave_id,
                                    int enabled) {
  struct port *port;
  int res;
...
int rte_eth_bond_8023ad_ext_distrib_get(uint16_t port_id, uint16_t slave_id) {
  struct port *port;
  int err;
...
int rte_eth_bond_8023ad_ext_collect_get(uint16_t port_id, uint16_t slave_id) {
  struct port *port;
  int err;
...
int rte_eth_bond_8023ad_ext_slowtx(uint16_t port_id, uint16_t slave_id,
