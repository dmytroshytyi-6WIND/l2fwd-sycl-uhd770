...
static inline uint64_t readq(void __iomem *reg) {
  return ((uint64_t)readl((char *)reg + 0x4UL) << 32) | (uint64_t)readl(reg);
}
...
dma_addr_t vnic_dev_get_res_bus_addr(struct vnic_dev *vdev,
                                     enum vnic_res_type type,
                                     unsigned int index);
uint8_t vnic_dev_get_res_bar(struct vnic_dev *vdev, enum vnic_res_type type);
uint32_t vnic_dev_get_res_offset(struct vnic_dev *vdev, enum vnic_res_type type,
                                 unsigned int index);
...
int vnic_dev_alloc_desc_ring(struct vnic_dev *vdev, struct vnic_dev_ring *ring,
                             ...
                             unsigned int socket_id, char *z_name);
...
int vnic_dev_packet_filter_all(struct vnic_dev *vdev, int directed,
                               int multicast, int broadcast, int promisc,
                               int allmulti);
