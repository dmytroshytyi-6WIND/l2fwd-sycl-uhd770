...
static int vnic_wq_alloc_bufs(struct vnic_wq *wq) {
  ...
  wq->bufs = (struct rte_mbuf **)rte_zmalloc_socket(
      "wq->bufs", sizeof(struct rte_mbuf *) * count, RTE_CACHE_LINE_SIZE,
      wq->socket_id);
  wq->head_idx = 0;
  wq->tail_idx = 0;
  if (wq->bufs == NULL)
...
void vnic_wq_enable(struct vnic_wq *wq) { iowrite32(1, &wq->ctrl->enable); }
