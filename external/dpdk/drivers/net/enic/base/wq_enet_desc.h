...
static inline void
wq_enet_desc_enc(struct wq_enet_desc *desc, uint64_t address, uint16_t length,
                 uint16_t mss, uint16_t header_length, uint8_t offload_mode,
                 uint8_t eop, uint8_t cq_entry, uint8_t fcoe_encap,
  ...
  desc->mss_loopback =
      rte_cpu_to_le_16((mss & WQ_ENET_MSS_MASK) << WQ_ENET_MSS_SHIFT |
                       (loopback & 1) << WQ_ENET_LOOPBACK_SHIFT);
...
static inline void wq_enet_desc_dec(struct wq_enet_desc *desc,
  ...
  *mss = (rte_le_to_cpu_16(desc->mss_loopback) >> WQ_ENET_MSS_SHIFT) &
         WQ_ENET_MSS_MASK;
  *loopback = (uint8_t)(
      (rte_le_to_cpu_16(desc->mss_loopback) >> WQ_ENET_LOOPBACK_SHIFT) & 1);
  *header_length =
      rte_le_to_cpu_16(desc->header_length_flags) & WQ_ENET_HDRLEN_MASK;
  *offload_mode = (uint8_t)(
      (rte_le_to_cpu_16(desc->header_length_flags) >> WQ_ENET_HDRLEN_BITS) &
      WQ_ENET_FLAGS_OM_MASK);
  *eop = (uint8_t)(
      (rte_le_to_cpu_16(desc->header_length_flags) >> WQ_ENET_FLAGS_EOP_SHIFT) &
      1);
  ...
  *fcoe_encap = (uint8_t)((rte_le_to_cpu_16(desc->header_length_flags) >>
                          ...
                          1);
  *vlan_tag_insert = (uint8_t)((rte_le_to_cpu_16(desc->header_length_flags) >>
                                WQ_ENET_FLAGS_VLAN_TAG_INSERT_SHIFT) &
                               1);
