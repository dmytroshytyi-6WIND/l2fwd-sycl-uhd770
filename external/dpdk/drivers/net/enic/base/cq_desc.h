...
static inline void cq_desc_enc(struct cq_desc *desc, const uint8_t type,
                               const uint8_t color, const uint16_t q_number,
  ...
  desc->completed_index =
      rte_cpu_to_le_16(completed_index & CQ_DESC_COMP_NDX_MASK);
}

static inline void cq_desc_dec(const struct cq_desc *desc_arg, uint8_t *type,
  ...
  *completed_index =
      rte_le_to_cpu_16(desc->completed_index) & CQ_DESC_COMP_NDX_MASK;
}

static inline void cq_color_dec(const struct cq_desc *desc_arg,
                                uint8_t *color) {
