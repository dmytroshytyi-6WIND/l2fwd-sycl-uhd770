...
struct vnic_dev {
  ...
  void *(*alloc_consistent)(void *priv, size_t size, dma_addr_t *dma_handle,
                            uint8_t *name);
...
void *vnic_dev_priv(struct vnic_dev *vdev) { return vdev->priv; }

void vnic_register_cbacks(struct vnic_dev *vdev,
                          void *(*alloc_consistent)(void *priv, size_t size,
                                                    dma_addr_t *dma_handle,
                                                    uint8_t *name),
                          void (*free_consistent)(void *priv, size_t size,
                                                  void *vaddr,
                                                  dma_addr_t dma_handle)) {
    ...
    case RES_TYPE_INTR_CTRL:
      ...
      if (len + bar_offset > bar[bar_num].len) {
        pr_err("vNIC BAR0 resource %d "
               ...
               type, bar_offset, len, bar[bar_num].len);
        return -EINVAL;
      }
      break;
...
void __iomem *vnic_dev_get_res(struct vnic_dev *vdev, enum vnic_res_type type,
  ...
  case RES_TYPE_CQ:
  case RES_TYPE_INTR_CTRL:
    return (char __iomem *)vdev->res[type].vaddr + index * VNIC_RES_STRIDE;
  default:
...
int vnic_dev_alloc_desc_ring(struct vnic_dev *vdev, struct vnic_dev_ring *ring,
  ...
  alloc_addr = vdev->alloc_consistent(vdev->priv, ring->size_unaligned,
  ...
  if (!alloc_pa) {
    pr_err("Failed to map allocated ring (size=%d), aborting\n",
           (int)ring->size);
    vdev->free_consistent(vdev->priv, ring->size_unaligned, alloc_addr,
                          alloc_pa);
    return -ENOMEM;
  }
  ring->base_addr_unaligned = alloc_pa;

  ring->base_addr = VNIC_ALIGN(ring->base_addr_unaligned, ring->base_align);
...
void vnic_dev_free_desc_ring(__rte_unused struct vnic_dev *vdev,
  ...
  if (ring->descs) {
    vdev->free_consistent(vdev->priv, ring->size_unaligned,
                          ring->descs_unaligned, ring->base_addr_unaligned);
    ring->descs = NULL;
  }
}
  ...
  for (delay = 0; delay < wait; delay++) {
    ...
    if (!(status & STAT_BUSY)) {
      if (status & STAT_ERROR) {
        ...
        if (cmd != CMD_CAPABILITY && cmd != CMD_OVERLAY_OFFLOAD_CTRL &&
...
static int vnic_dev_cmd_proxy(struct vnic_dev *vdev,
                              enum vnic_devcmd_cmd proxy_cmd,
                              enum vnic_devcmd_cmd cmd, uint64_t *args,
                              int nargs, int wait) {
  uint32_t status;
  int err;
...
int vnic_dev_cmd(struct vnic_dev *vdev, enum vnic_devcmd_cmd cmd, uint64_t *a0,
                 uint64_t *a1, int wait) {
  uint64_t args[2];
  bool vf_rep;
  int vf_idx;
  int err;
...
int vnic_dev_cmd_args(struct vnic_dev *vdev, enum vnic_devcmd_cmd cmd,
  ...
  switch (vdev->proxy) {
  case PROXY_BY_INDEX:
    err = vnic_dev_cmd_proxy(vdev, CMD_PROXY_BY_INDEX, cmd, args, nargs, wait);
    break;
  case PROXY_BY_BDF:
    err = vnic_dev_cmd_proxy(vdev, CMD_PROXY_BY_BDF, cmd, args, nargs, wait);
    break;
  case PROXY_NONE:
  default:
...
int vnic_dev_fw_info(struct vnic_dev *vdev,
  ...
  if (!vdev->fw_info) {
    snprintf((char *)name, sizeof(name), "vnic_fw_info-%u", instance++);
    vdev->fw_info =
        vdev->alloc_consistent(vdev->priv, sizeof(struct vnic_devcmd_fw_info),
...
static int vnic_dev_flowman_enable(struct vnic_dev *vdev, uint32_t *mode,
  ...
  if (!vdev->flowman_info) {
    snprintf((char *)name, sizeof(name), "vnic_fm_info-%u", instance++);
    vdev->flowman_info =
        vdev->alloc_consistent(vdev->priv, sizeof(struct fm_info),
  ...
  ops = (1ULL << FMOP_END) | (1ULL << FMOP_DROP) | (1ULL << FMOP_RQ_STEER) |
        (1ULL << FMOP_EXACT_MATCH) | (1ULL << FMOP_MARK) | (1ULL << FMOP_TAG) |
        (1ULL << FMOP_EG_HAIRPIN) | (1ULL << FMOP_ENCAP) |
...
int vnic_dev_set_ig_vlan_rewrite_mode(struct vnic_dev *vdev,
  ...
  if (vnic_dev_capable(vdev, CMD_IG_VLAN_REWRITE_MODE))
    return vnic_dev_cmd(vdev, CMD_IG_VLAN_REWRITE_MODE, &a0, &a1, wait);
  else
    return 0;
}
...
int vnic_dev_notify_setcmd(struct vnic_dev *vdev, void *notify_addr,
                           dma_addr_t notify_pa, uint16_t intr) {
  uint64_t a0, a1;
  int wait = 1000;
  int r;
...
int vnic_dev_notify_set(struct vnic_dev *vdev, uint16_t intr) {
  ...
  if (vdev->notify || vdev->notify_pa) {
    return vnic_dev_notify_setcmd(vdev, vdev->notify, vdev->notify_pa, intr);
  }
  if (!vnic_dev_in_reset(vdev)) {
    snprintf((char *)name, sizeof(name), "vnic_notify-%u", instance++);
    notify_addr =
        vdev->alloc_consistent(vdev->priv, sizeof(struct vnic_devcmd_notify),
...
int vnic_dev_notify_unset(struct vnic_dev *vdev) {
  if (vdev->notify && !vnic_dev_in_reset(vdev)) {
    vdev->free_consistent(vdev->priv, sizeof(struct vnic_devcmd_notify),
...
int vnic_dev_alloc_stats_mem(struct vnic_dev *vdev) {
  ...
  vdev->stats = vdev->alloc_consistent(vdev->priv, sizeof(struct vnic_stats),
  ...
  if (vdev) {
    if (vdev->notify)
      vdev->free_consistent(vdev->priv, sizeof(struct vnic_devcmd_notify),
                            vdev->notify, vdev->notify_pa);
    if (vdev->stats)
      vdev->free_consistent(vdev->priv, sizeof(struct vnic_stats), vdev->stats,
                            vdev->stats_pa);
    if (vdev->flowman_info)
      vdev->free_consistent(vdev->priv, sizeof(struct fm_info),
                            vdev->flowman_info, vdev->flowman_info_pa);
    if (vdev->fw_info)
      vdev->free_consistent(vdev->priv, sizeof(struct vnic_devcmd_fw_info),
                            vdev->fw_info, vdev->fw_info_pa);
    rte_free(vdev);
  }
}

struct vnic_dev *vnic_dev_register(struct vnic_dev *vdev, void *priv,
                                   struct rte_pci_device *pdev,
                                   struct vnic_dev_bar *bar,
                                   unsigned int num_bars) {
  if (!vdev) {
    char name[RTE_MEMZONE_NAMESIZE];
    snprintf((char *)name, sizeof(name), "%s-vnic", pdev->device.name);
    vdev = (struct vnic_dev *)rte_zmalloc_socket(name, sizeof(struct vnic_dev),
                                                 RTE_CACHE_LINE_SIZE,
...
struct vnic_dev *vnic_vf_rep_register(void *priv, struct vnic_dev *pf_vdev,
  ...
  vdev = (struct vnic_dev *)rte_zmalloc(
      "enic-vf-rep-vdev", sizeof(struct vnic_dev), RTE_CACHE_LINE_SIZE);
  if (!vdev)
    return NULL;
  vdev->priv = priv;
...
int vnic_dev_classifier(struct vnic_dev *vdev, uint8_t cmd, uint16_t *entry,
                        ...
                        struct filter_action_v2 *action_v2) {
  uint64_t a0 = 0, a1 = 0;
  int wait = 1000;
  dma_addr_t tlv_pa;
  int ret = -EINVAL;
  ...
  if (cmd == CLSF_ADD) {
    dev_cmd =
        (data->type >= FILTER_DPDK_1) ? CMD_ADD_ADV_FILTER : CMD_ADD_FILTER;
    ...
    snprintf((char *)z_name, sizeof(z_name), "vnic_clsf_%u", unique_id++);
    tlv_va = vdev->alloc_consistent(vdev->priv, tlv_size, &tlv_pa,
                                    (uint8_t *)z_name);
    if (!tlv_va)
      return -ENOMEM;
    tlv = tlv_va;
...
int vnic_dev_capable_vxlan(struct vnic_dev *vdev) {
  ...
  return ret == 0 && (a1 & (FEATURE_VXLAN_IPV6 | FEATURE_VXLAN_MULTI_WQ)) ==
