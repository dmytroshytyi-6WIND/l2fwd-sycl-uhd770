...
static int enicpmd_dev_flow_ops_get(struct rte_eth_dev *dev,
...
static void enicpmd_dev_tx_queue_release(struct rte_eth_dev *dev,
                                         uint16_t qid) {
...
static int enicpmd_dev_tx_queue_setup(struct rte_eth_dev *eth_dev,
  ...
  wq->offloads = tx_conf->offloads | eth_dev->data->dev_conf.txmode.offloads;
...
static void enicpmd_dev_rx_queue_release(struct rte_eth_dev *dev,
                                         uint16_t qid) {
...
static int enicpmd_dev_configure(struct rte_eth_dev *eth_dev) {
  ...
  enic->mc_count = 0;
  enic->hw_ip_checksum =
      !!(eth_dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_CHECKSUM);
...
static int enicpmd_dev_info_get(struct rte_eth_dev *eth_dev,
  ...
  device_info->default_rxconf =
      (struct rte_eth_rxconf){.rx_free_thresh = ENIC_DEFAULT_RX_FREE_THRESH};
  ...
  device_info->default_rxportconf = (struct rte_eth_dev_portconf){
      ...
      .ring_size =
          RTE_MIN(device_info->rx_desc_lim.nb_max, ENIC_DEFAULT_RX_RING_SIZE),
  ...
  device_info->default_txportconf = (struct rte_eth_dev_portconf){
      ...
      .ring_size =
          RTE_MIN(device_info->tx_desc_lim.nb_max, ENIC_DEFAULT_TX_RING_SIZE),
...
enicpmd_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {RTE_PTYPE_L2_ETHER,
                                    ...
                                    RTE_PTYPE_UNKNOWN};
  static const uint32_t ptypes_overlay[] = {RTE_PTYPE_L2_ETHER,
                                            RTE_PTYPE_L2_ETHER_VLAN,
                                            RTE_PTYPE_L3_IPV4_EXT_UNKNOWN,
                                            RTE_PTYPE_L3_IPV6_EXT_UNKNOWN,
                                            RTE_PTYPE_L4_TCP,
...
static void enicpmd_remove_mac_addr(struct rte_eth_dev *eth_dev,
                                    uint32_t index) {
  struct enic *enic = pmd_priv(eth_dev);
...
static int
enicpmd_dev_rss_reta_query(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size; i++) {
    ...
    if (reta_conf[idx].mask & (1ULL << shift))
      reta_conf[idx].reta[shift] =
          enic_sop_rq_idx_to_rte_idx(enic->rss_cpu.cpu[i / 4].b[i % 4]);
  }
...
static int
enicpmd_dev_rss_reta_update(struct rte_eth_dev *dev,
                            struct rte_eth_rss_reta_entry64 *reta_conf,
                            uint16_t reta_size) {
  struct enic *enic = pmd_priv(dev);
  ...
  if (reta_size != ENIC_RSS_RETA_SIZE) {
    dev_err(enic,
            "reta_update: wrong reta_size. given=%u"
            " expected=%u\n",
            reta_size, ENIC_RSS_RETA_SIZE);
    return -EINVAL;
  }
  ...
  for (i = 0; i < reta_size; i++) {
    ...
    if (reta_conf[idx].mask & (1ULL << shift))
      rss_cpu.cpu[i / 4].b[i % 4] =
          enic_rte_rq_idx_to_sop_idx(reta_conf[idx].reta[shift]);
  }
...
static int enicpmd_dev_rss_hash_conf_get(struct rte_eth_dev *dev,
  ...
  if (rss_conf->rss_key != NULL &&
    ...
    dev_err(enic,
            "rss_hash_conf_get: wrong rss_key_len. given=%u"
            " expected=%u+\n",
            rss_conf->rss_key_len, ENIC_RSS_HASH_KEY_SIZE);
    return -EINVAL;
  }
  ...
  if (rss_conf->rss_key != NULL) {
    ...
    for (i = 0; i < ENIC_RSS_HASH_KEY_SIZE; i++) {
      rss_conf->rss_key[i] = enic->rss_key.key[i / 10].b[i % 10];
    }
    rss_conf->rss_key_len = ENIC_RSS_HASH_KEY_SIZE;
  }
  return 0;
...
static int update_tunnel_port(struct enic *enic, uint16_t port, bool vxlan) {
  ...
  cfg = vxlan ? OVERLAY_CFG_VXLAN_PORT_UPDATE : OVERLAY_CFG_GENEVE_PORT_UPDATE;
...
static int enicpmd_dev_fw_version_get(struct rte_eth_dev *eth_dev,
  ...
  if (ret)
    return ret;
  ret =
      snprintf(fw_version, fw_size, "%s %s", info->fw_version, info->fw_build);
  if (ret < 0)
    return -EINVAL;
...
static int enic_parse_zero_one(const char *key, const char *value,
                               void *opaque) {
  struct enic *enic;
  bool b;
...
static int enic_parse_ig_vlan_rewrite(__rte_unused const char *key,
  ...
  if (strcmp(value, "trunk") == 0) {
    ...
    dev_err(enic,
            "Invalid value for " ENIC_DEVARG_IG_VLAN_REWRITE
...
static int enic_check_devargs(struct rte_eth_dev *dev) {
  static const char *const valid_keys[] = {
      ENIC_DEVARG_CQ64,           ENIC_DEVARG_DISABLE_OVERLAY,
      ENIC_DEVARG_ENABLE_AVX2_RX, ENIC_DEVARG_IG_VLAN_REWRITE,
      ENIC_DEVARG_REPRESENTOR,    NULL};
  struct enic *enic = pmd_priv(dev);
  ...
  if (!kvlist)
    return -EINVAL;
  if (rte_kvargs_process(kvlist, ENIC_DEVARG_CQ64, enic_parse_zero_one, enic) <
          0 ||
...
static int eth_enic_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  retval = rte_eth_dev_create(&pci_dev->device, pci_dev->device.name,
                              sizeof(struct enic), eth_dev_pci_specific_init,
                              pci_dev, eth_enic_dev_init, NULL);
  ...
  for (i = 0; i < eth_da.nb_representor_ports; i++) {
    pf_enic->max_vf_id =
        RTE_MAX(pf_enic->max_vf_id, eth_da.representor_ports[i]);
  }
  for (i = 0; i < eth_da.nb_representor_ports; i++) {
    struct enic_vf_representor representor;

    representor.vf_id = eth_da.representor_ports[i];
    representor.switch_domain_id = pmd_priv(pf_ethdev)->switch_domain_id;
    ...
    if (retval) {
      ENICPMD_LOG(ERR, "failed to create enic vf representor %s", name);
      return retval;
    }
  }
  return 0;
...
RTE_PMD_REGISTER_PARAM_STRING(net_enic, ENIC_DEVARG_CQ64
                              "=0|1" ENIC_DEVARG_DISABLE_OVERLAY
                              "=0|1 " ENIC_DEVARG_ENABLE_AVX2_RX
                              "=0|1 " ENIC_DEVARG_IG_VLAN_REWRITE
                              "=trunk|untag|priority|pass");
