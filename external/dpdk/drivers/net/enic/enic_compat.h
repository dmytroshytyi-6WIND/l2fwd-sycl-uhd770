...
#define dev_printk(level, fmt, args...)                                        \
  rte_log(RTE_LOG_##level, enic_pmd_logtype, "PMD: rte_enic_pmd: " fmt, ##args)
...
#define ENICPMD_LOG(level, fmt, args...)                                       \
  rte_log(RTE_LOG_##level, enic_pmd_logtype, "%s " fmt "\n", __func__, ##args)
...
static inline uint8_t ioread8(volatile void *addr) { return rte_read8(addr); }
