...
static const enum rte_flow_action_type enic_supported_actions_v2_id[] = {
    RTE_FLOW_ACTION_TYPE_QUEUE,    RTE_FLOW_ACTION_TYPE_MARK,
    RTE_FLOW_ACTION_TYPE_FLAG,     RTE_FLOW_ACTION_TYPE_RSS,
    RTE_FLOW_ACTION_TYPE_PASSTHRU, RTE_FLOW_ACTION_TYPE_END,
};

static const enum rte_flow_action_type enic_supported_actions_v2_drop[] = {
    RTE_FLOW_ACTION_TYPE_QUEUE, RTE_FLOW_ACTION_TYPE_MARK,
    RTE_FLOW_ACTION_TYPE_FLAG,  RTE_FLOW_ACTION_TYPE_DROP,
    RTE_FLOW_ACTION_TYPE_RSS,   RTE_FLOW_ACTION_TYPE_PASSTHRU,
    RTE_FLOW_ACTION_TYPE_END,
};
...
static int mask_exact_match(const uint8_t *supported, const uint8_t *supplied,
                            unsigned int size) {
  unsigned int i;
  for (i = 0; i < size; i++) {
...
static int enic_copy_item_ipv4_v1(struct copy_item_args *arg) {
...
static int enic_copy_item_udp_v1(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
...
static int enic_copy_item_tcp_v1(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
 ...
 */
static int copy_inner_common(struct filter_generic_1 *gp, uint8_t *inner_ofst,
                             const void *val, const void *mask,
                             uint8_t val_size, uint8_t proto_off,
                             uint16_t proto_val, uint8_t proto_size) {
...
static int enic_copy_item_inner_eth_v2(struct copy_item_args *arg) {
  ...
  return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec,
                           mask, sizeof(struct rte_ether_hdr),
                           0 /* no previous protocol */, 0, 0);
}

static int enic_copy_item_inner_vlan_v2(struct copy_item_args *arg) {
  const void *mask = arg->item->mask;
  uint8_t *off = arg->inner_ofst;
  ...
  return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec,
                           mask, sizeof(struct rte_vlan_hdr), eth_type_off,
                           rte_cpu_to_be_16(RTE_ETHER_TYPE_VLAN), 2);
}

static int enic_copy_item_inner_ipv4_v2(struct copy_item_args *arg) {
  const void *mask = arg->item->mask;
  uint8_t *off = arg->inner_ofst;
  ...
  return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec,
                           mask, sizeof(struct rte_ipv4_hdr), arg->l2_proto_off,
                           rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4), 2);
}

static int enic_copy_item_inner_ipv6_v2(struct copy_item_args *arg) {
  const void *mask = arg->item->mask;
  uint8_t *off = arg->inner_ofst;
  ...
  return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec,
                           mask, sizeof(struct rte_ipv6_hdr), arg->l2_proto_off,
                           rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6), 2);
}

static int enic_copy_item_inner_udp_v2(struct copy_item_args *arg) {
  const void *mask = arg->item->mask;
  uint8_t *off = arg->inner_ofst;
  ...
  return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec,
                           mask, sizeof(struct rte_udp_hdr), arg->l3_proto_off,
                           IPPROTO_UDP, 1);
}

static int enic_copy_item_inner_tcp_v2(struct copy_item_args *arg) {
  const void *mask = arg->item->mask;
  uint8_t *off = arg->inner_ofst;
  ...
  return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec,
                           mask, sizeof(struct rte_tcp_hdr), arg->l3_proto_off,
                           IPPROTO_TCP, 1);
}

static int enic_copy_item_eth_v2(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
...
static int enic_copy_item_vlan_v2(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
...
static int enic_copy_item_ipv4_v2(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
  const struct rte_flow_item_ipv4 *spec = item->spec;
  const struct rte_flow_item_ipv4 *mask = item->mask;
...
static int enic_copy_item_ipv6_v2(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
...
static int enic_copy_item_udp_v2(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
  const struct rte_flow_item_udp *spec = item->spec;
  const struct rte_flow_item_udp *mask = item->mask;
...
static int enic_copy_item_tcp_v2(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
  const struct rte_flow_item_tcp *spec = item->spec;
  const struct rte_flow_item_tcp *mask = item->mask;
...
static int enic_copy_item_sctp_v2(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
...
static int enic_copy_item_vxlan_v2(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
 ...
 * or geneve).
 */
static int enic_copy_item_raw_v2(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  struct filter_v2 *enic_filter = arg->filter;
  uint8_t *inner_ofst = arg->inner_ofst;
 ...
 */
static int item_stacking_valid(enum rte_flow_item_type prev_item,
                               const struct enic_items *item_info,
 ...
 */
static void fixup_l5_layer(struct enic *enic, struct filter_generic_1 *gp,
                           uint8_t inner_ofst) {
 ...
 * @param error[out]
 */
static int enic_copy_filter(const struct rte_flow_item pattern[],
                            const struct enic_filter_cap *cap,
                            struct enic *enic, struct filter_v2 *enic_filter,
  ...
  for (; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    ...
    if (item->type > cap->max_item_type || item_info->copy_item == NULL ||
    ...
    args.item = item;
    copy_fn =
        inner_ofst > 0 ? item_info->inner_copy_item : item_info->copy_item;
    ret = copy_fn(&args);
    if (ret)
...
item_not_supported:
  rte_flow_error_set(error, ret, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                     "enic type error");
  return -rte_errno;
 ...
 * @param error[out]
 */
static int enic_copy_action_v1(__rte_unused struct enic *enic,
                               const struct rte_flow_action actions[],
                               struct filter_action_v2 *enic_action) {
  enum {
    FATE = 1,
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_QUEUE: {
      ...
      if (overlap & FATE)
        return ENOTSUP;
      overlap |= FATE;
      enic_action->rq_idx = enic_rte_rq_idx_to_sop_idx(queue->index);
      break;
    }
    default:
      RTE_ASSERT(0);
 ...
 * @param error[out]
 */
static int enic_copy_action_v2(struct enic *enic,
                               const struct rte_flow_action actions[],
                               struct filter_action_v2 *enic_action) {
  enum {
    FATE = 1,
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_RSS: {
      const struct rte_flow_action_rss *rss =
          (const struct rte_flow_action_rss *)actions->conf;
      bool allow;
      uint16_t i;
      ...
      allow = rss->func == RTE_ETH_HASH_FUNCTION_DEFAULT && rss->level == 0 &&
              ...
              rss->queue_num == enic->rq_count && rss->key_len == 0;
...
static const struct enic_filter_cap *enic_get_filter_cap(struct enic *enic) {
...
static const struct enic_action_cap *enic_get_action_cap(struct enic *enic) {
...
static void enic_dump_actions(const struct filter_action_v2 *ea) {
...
static void enic_dump_filter(const struct filter_v2 *filt) {
  ...
  case FILTER_DPDK_1:
    ...
    if (gp->mask_flags & FILTER_GENERIC_1_IPV4)
      sprintf(ip4, "%s ",
              (gp->val_flags & FILTER_GENERIC_1_IPV4) ? "ip4(y)" : "ip4(n)");
    else
      sprintf(ip4, "%s ", "ip4(x)");

    if (gp->mask_flags & FILTER_GENERIC_1_IPV6)
      sprintf(ip6, "%s ",
              (gp->val_flags & FILTER_GENERIC_1_IPV6) ? "ip6(y)" : "ip6(n)");
    else
      sprintf(ip6, "%s ", "ip6(x)");

    if (gp->mask_flags & FILTER_GENERIC_1_UDP)
      sprintf(udp, "%s ",
              (gp->val_flags & FILTER_GENERIC_1_UDP) ? "udp(y)" : "udp(n)");
    else
      sprintf(udp, "%s ", "udp(x)");

    if (gp->mask_flags & FILTER_GENERIC_1_TCP)
      sprintf(tcp, "%s ",
              (gp->val_flags & FILTER_GENERIC_1_TCP) ? "tcp(y)" : "tcp(n)");
    else
      sprintf(tcp, "%s ", "tcp(x)");

    if (gp->mask_flags & FILTER_GENERIC_1_TCP_OR_UDP)
      sprintf(tcpudp, "%s ",
              (gp->val_flags & FILTER_GENERIC_1_TCP_OR_UDP) ? "tcpudp(y)"
                                                            : "tcpudp(n)");
    else
    ...
    if (gp->mask_flags & FILTER_GENERIC_1_IP4SUM_OK)
      sprintf(ip4csum, "%s ",
              (gp->val_flags & FILTER_GENERIC_1_IP4SUM_OK) ? "ip4csum(y)"
                                                           : "ip4csum(n)");
    else
    ...
    if (gp->mask_flags & FILTER_GENERIC_1_L4SUM_OK)
      sprintf(l4csum, "%s ",
              (gp->val_flags & FILTER_GENERIC_1_L4SUM_OK) ? "l4csum(y)"
                                                          : "l4csum(n)");
    else
    ...
    if (gp->mask_flags & FILTER_GENERIC_1_IPFRAG)
      sprintf(ipfrag, "%s ",
              (gp->val_flags & FILTER_GENERIC_1_IPFRAG) ? "ipfrag(y)"
                                                        : "ipfrag(n)");
    else
      sprintf(ipfrag, "%s ", "ipfrag(x)");
    ENICPMD_LOG(INFO, "\tFlags: %s%s%s%s%s%s%s%s\n", ip4, ip6, udp, tcp, tcpudp,
                ip4csum, l4csum, ipfrag);
    ...
    for (i = 0; i < FILTER_GENERIC_1_NUM_LAYERS; i++) {
      ...
      bp = buf;
      for (j = 0; j <= mbyte; j++) {
        sprintf(bp, "%02x", gp->layer[i].mask[j]);
        bp += 2;
      }
      *bp = '\0';
      ...
      bp = buf;
      for (j = 0; j <= mbyte; j++) {
        sprintf(bp, "%02x", gp->layer[i].val[j]);
        bp += 2;
      }
      *bp = '\0';
...
static void enic_dump_flow(const struct filter_action_v2 *ea,
                           const struct filter_v2 *filt) {
 ...
 */
static int enic_flow_parse(struct rte_eth_dev *dev,
  ...
  if (attrs) {
    if (attrs->group) {
      ...
      rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY,
                         NULL, "priorities are not supported");
      return -rte_errno;
  ...
  ret = enic_copy_filter(pattern, enic_filter_cap, enic, enic_filter, error);
  return ret;
}
 ...
 * @param error[out]
 */
static int enic_flow_del_filter(struct enic *enic, struct rte_flow *flow,
                                struct rte_flow_error *error) {
  uint16_t filter_id;
  int err;
 ...
 * @see rte_flow_ops
 */
static int enic_flow_validate(struct rte_eth_dev *dev,
                              const struct rte_flow_attr *attrs,
                              const struct rte_flow_item pattern[],
                              const struct rte_flow_action actions[],
                              struct rte_flow_error *error) {
 ...
 * @see rte_flow_ops
 */
static struct rte_flow *enic_flow_create(struct rte_eth_dev *dev,
                                         const struct rte_flow_attr *attrs,
                                         const struct rte_flow_item pattern[],
                                         const struct rte_flow_action actions[],
                                         struct rte_flow_error *error) {
  ...
  flow = enic_flow_add_filter(enic, &enic_filter, &enic_action, error);
  if (flow)
 ...
 * @see rte_flow_ops
 */
static int enic_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
 ...
 * @see rte_flow_ops
 */
static int enic_flow_flush(struct rte_eth_dev *dev,
                           struct rte_flow_error *error) {
  struct rte_flow *flow;
  struct enic *enic = pmd_priv(dev);
