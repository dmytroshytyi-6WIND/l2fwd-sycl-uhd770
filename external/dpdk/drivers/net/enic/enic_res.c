...
int enic_get_vnic_config(struct enic *enic) {
  ...
  if (err) {
    dev_err(enic_get_dev(enic), "Error getting MAC addr, %d\n", err);
    return err;
  }

#define GET_CONFIG(m)                                                          \
  do {                                                                         \
    err = vnic_dev_spec(enic->vdev, offsetof(struct vnic_enet_config, m),      \
                        sizeof(c->m), &c->m);                                  \
    if (err) {                                                                 \
      dev_err(enic_get_dev(enic), "Error getting %s, %d\n", #m, err);          \
      return err;                                                              \
    }                                                                          \
  } while (0)
  ...
  dev_info(
      enic, "Flow api filter mode: %s Actions: %s%s%s%s\n",
      ((enic->flow_filter_mode == FILTER_FLOWMAN)
           ? "FLOWMAN"
           : ((enic->flow_filter_mode == FILTER_DPDK_1)
                  ? "DPDK"
                  : ((enic->flow_filter_mode == FILTER_USNIC_IP)
                         ? "USNIC"
                         : ((enic->flow_filter_mode == FILTER_IPV4_5TUPLE)
                                ? "5TUPLE"
                                : "NONE")))),
      ((enic->filter_actions & FILTER_ACTION_RQ_STEERING_FLAG) ? "steer " : ""),
      ((enic->filter_actions & FILTER_ACTION_FILTER_ID_FLAG) ? "tag " : ""),
      ((enic->filter_actions & FILTER_ACTION_DROP_FLAG) ? "drop " : ""),
      ((enic->filter_actions & FILTER_ACTION_COUNTER_FLAG) ? "count " : ""));
   ...
   */
  max_wq_descs = (enic->cq64_request && c->max_wq_ring)
                     ? c->max_wq_ring
                     : ENIC_LEGACY_MAX_WQ_DESCS;
  c->wq_desc_count = RTE_MIN(
      max_wq_descs, RTE_MAX((uint32_t)ENIC_MIN_WQ_DESCS, c->wq_desc_count));
  c->wq_desc_count &= 0xffffffe0; /* must be aligned to groups of 32 */
  max_rq_descs = (enic->cq64_request && c->max_rq_ring)
                     ? c->max_rq_ring
                     : ENIC_LEGACY_MAX_WQ_DESCS;
  c->rq_desc_count = RTE_MIN(
      max_rq_descs, RTE_MAX((uint32_t)ENIC_MIN_RQ_DESCS, c->rq_desc_count));
  c->rq_desc_count &= 0xffffffe0; /* must be aligned to groups of 32 */
  dev_debug(NULL, "Max supported VIC descriptors: WQ:%u, RQ:%u\n", max_wq_descs,
            max_rq_descs);

  c->intr_timer_usec =
      RTE_MIN(c->intr_timer_usec, vnic_dev_get_intr_coal_timer_max(enic->vdev));
  ...
  dev_info(enic_get_dev(enic),
           ...
           c->wq_desc_count, c->rq_desc_count, enic->rte_dev->data->mtu,
           enic->max_mtu);
  dev_info(enic_get_dev(enic),
           "vNIC csum tx/rx %s/%s "
           ...
           ENIC_SETTING(enic, RSS)
               ? (ENIC_SETTING(enic, RSSHASH_UDPIPV4)
                      ? "+UDP"
                      : ((enic->udp_rss_weak ? "+udp" : "yes")))
               : "no",
  ...
  if (ENIC_SETTING(enic, RSSHASH_IPV4))
     ...
     */
    enic->flow_type_rss_offloads |= RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_FRAG_IPV4 |
  ...
  if (ENIC_SETTING(enic, RSSHASH_IPV6))
     ...
     */
    enic->flow_type_rss_offloads |= RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_IPV6_EX |
  ...
  enic->vxlan = ENIC_SETTING(enic, VXLAN) && vnic_dev_capable_vxlan(enic->vdev);
  enic->geneve =
      ENIC_SETTING(enic, GENEVE) && vnic_dev_capable_geneve(enic->vdev);
  ...
  enic->cq64 = enic->cq64_request && (sizes & VNIC_RQ_CQ_ENTRY_SIZE_64_CAPABLE);
  ...
  enic->tx_offload_capa =
      enic->tx_queue_offload_capa | RTE_ETH_TX_OFFLOAD_MULTI_SEGS |
      RTE_ETH_TX_OFFLOAD_VLAN_INSERT | RTE_ETH_TX_OFFLOAD_IPV4_CKSUM |
      RTE_ETH_TX_OFFLOAD_UDP_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_CKSUM |
      RTE_ETH_TX_OFFLOAD_TCP_TSO;
  enic->rx_offload_capa =
      RTE_ETH_RX_OFFLOAD_SCATTER | RTE_ETH_RX_OFFLOAD_VLAN_STRIP |
      RTE_ETH_RX_OFFLOAD_IPV4_CKSUM | RTE_ETH_RX_OFFLOAD_UDP_CKSUM |
      RTE_ETH_RX_OFFLOAD_TCP_CKSUM | RTE_ETH_RX_OFFLOAD_RSS_HASH;
  enic->tx_offload_mask = RTE_MBUF_F_TX_IPV6 | RTE_MBUF_F_TX_IPV4 |
                          RTE_MBUF_F_TX_VLAN | RTE_MBUF_F_TX_IP_CKSUM |
...
int enic_set_nic_cfg(struct enic *enic, uint8_t rss_default_cpu,
  ...
  vnic_set_nic_cfg(&nic_cfg, rss_default_cpu, rss_hash_type, rss_hash_bits,
...
void enic_get_res_counts(struct enic *enic) {
  ...
  dev_info(enic_get_dev(enic),
           ...
           enic->conf_wq_count, enic->conf_rq_count, enic->conf_cq_count,
           enic->conf_intr_count);
}
