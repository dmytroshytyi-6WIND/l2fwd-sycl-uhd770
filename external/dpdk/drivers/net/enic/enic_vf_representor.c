...
static uint16_t enic_vf_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                                  uint16_t nb_pkts) {
  return enic_recv_pkts(rx_queue, rx_pkts, nb_pkts);
}

static uint16_t enic_vf_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                                  uint16_t nb_pkts) {
...
static void enic_vf_dev_tx_queue_release(struct rte_eth_dev *dev,
                                         uint16_t qid) {
...
static int enic_vf_dev_rx_queue_setup(struct rte_eth_dev *eth_dev,
  ...
  vf = eth_dev->data->dev_private;
  pf = vf->pf;
  eth_dev->data->rx_queues[queue_idx] = (void *)&pf->rq[vf->pf_rq_sop_idx];
...
static void enic_vf_dev_rx_queue_release(struct rte_eth_dev *dev,
                                         uint16_t qid) {
...
static int setup_rep_vf_fwd(struct enic_vf_representor *vf) {
  int ret;
...
static int enic_vf_dev_start(struct rte_eth_dev *eth_dev) {
  ...
  vnic_cq_init(&pf->cq[cq_idx], 0 /* flow_control_enable */,
  ...
  vnic_cq_init(&pf->cq[cq_idx], 0 /* flow_control_enable */,
               1 /* color_enable */, 0 /* cq_head */, 0 /* cq_tail */,
               1 /* cq_tail_color */, 0, 1 /* cq_entry_enable */,
               0 /* cq_message_enable */, 0, 0 /* cq_message_addr */);
  /* enic_enable */
...
static int adjust_flow_attr(const struct rte_flow_attr *attrs,
  ...
  *vf_attrs = *attrs;
  ...
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS,
                            NULL, "representor only supports ingress");
}

static int enic_vf_flow_validate(struct rte_eth_dev *dev,
...
enic_vf_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attrs,
                    const struct rte_flow_item pattern[],
                    const struct rte_flow_action actions[],
                    struct rte_flow_error *error) {
  struct rte_flow_attr vf_attrs;
...
static int enic_vf_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                                struct rte_flow_error *error) {
  return enic_fm_flow_ops.destroy(dev, flow, error);
}

static int enic_vf_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
                              const struct rte_flow_action *actions, void *data,
                              struct rte_flow_error *error) {
  return enic_fm_flow_ops.query(dev, flow, actions, data, error);
}

static int enic_vf_flow_flush(struct rte_eth_dev *dev,
                              struct rte_flow_error *error) {
...
static int enic_vf_flow_ops_get(struct rte_eth_dev *eth_dev,
...
static int get_vf_config(struct enic_vf_representor *vf) {
  ...
  err = vnic_dev_spec(vf->enic.vdev, offsetof(struct vnic_enet_config, mtu),
                      sizeof(c->mtu), &c->mtu);
  if (err) {
...
int enic_vf_representor_init(struct rte_eth_dev *eth_dev, void *init_params) {
  ...
  if (enic_cq_rq(pf, vf->pf_rq_sop_idx) >= pf->conf_cq_count) {
    ENICPMD_LOG(ERR,
                "Insufficient CQs. Please ensure number of CQs (%u)"
  ...
  if (vf->vf_id >= pdev->max_vfs) {
    ENICPMD_LOG(ERR, "VF ID is invalid. vf_id %u max_vfs %u", vf->vf_id,
                pdev->max_vfs);
    return -ENODEV;
  }
