...
static inline uint16_t enic_recv_pkts_common(void *rx_queue,
                                             struct rte_mbuf **rx_pkts,
                                             uint16_t nb_pkts,
                                             const bool use_64b_desc) {
  ...
  const int desc_size = use_64b_desc ? sizeof(struct cq_enet_rq_desc_64)
  ...
  while (max_rx) {
    ...
    if (use_64b_desc) {
      rq_idx_msbs =
          (((volatile struct cq_enet_rq_desc_64 *)cqd_ptr)->fetch_idx_flags &
           CQ_ENET_RQ_DESC_FETCH_IDX_MASK)
          << CQ_DESC_COMP_NDX_BITS;
      cqd.type_color = tc;
    }
    rq_num = cqd.q_number & CQ_DESC_Q_NUM_MASK;
    rq_idx = rq_idx_msbs + (cqd.completed_index & CQ_DESC_COMP_NDX_MASK);
     ...
     */
    rqd_ptr->address = rte_cpu_to_le_64(nmb->buf_iova + RTE_PKTMBUF_HEADROOM);
    ...
    if (rq->is_sop) {
      ...
      first_seg->pkt_len = (uint16_t)(first_seg->pkt_len + seg_length);
      first_seg->nb_segs++;
      last_seg->next = rxmb;
    }
     ...
     * packet is tunnelled.
     */
    tnl = enic->overlay_offload && (ciflags & CQ_ENET_RQ_DESC_FLAGS_FCOE) != 0;
    /* cq rx flags are only valid if eop bit is set */
    first_seg->packet_type = enic_cq_rx_flags_to_pkt_type(&cqd, tnl);
    enic_cq_rx_to_pkt_flags(&cqd, first_seg);

    /* Wipe the outer types set by enic_cq_rx_flags_to_pkt_type() */
    if (tnl) {
      first_seg->packet_type &= ~(RTE_PTYPE_L3_MASK | RTE_PTYPE_L4_MASK);
    }
    ...
    rte_packet_prefetch(RTE_PTR_ADD(first_seg->buf_addr, RTE_PKTMBUF_HEADROOM));
  ...
  if ((sop_rq->rx_nb_hold + data_rq->rx_nb_hold) > sop_rq->rx_free_thresh) {
    if (data_rq->in_use) {
      data_rq->posted_index = enic_ring_add(
          data_rq->ring.desc_count, data_rq->posted_index, data_rq->rx_nb_hold);
      data_rq->rx_nb_hold = 0;
    }
    sop_rq->posted_index = enic_ring_add(
        sop_rq->ring.desc_count, sop_rq->posted_index, sop_rq->rx_nb_hold);
    sop_rq->rx_nb_hold = 0;

    rte_mb();
    if (data_rq->in_use)
      iowrite32_relaxed(data_rq->posted_index, &data_rq->ctrl->posted_index);
    rte_compiler_barrier();
    iowrite32_relaxed(sop_rq->posted_index, &sop_rq->ctrl->posted_index);
  }
...
uint16_t enic_noscatter_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (max_rx) {
    ...
    mb = *rxmb++;
    /* prefetch mbuf data for caller */
    rte_packet_prefetch(RTE_PTR_ADD(mb->buf_addr, RTE_PKTMBUF_HEADROOM));
    mb->data_len =
        cqd->bytes_written_flags & CQ_ENET_RQ_DESC_BYTES_WRITTEN_MASK;
...
static inline void enic_free_wq_bufs(struct vnic_wq *wq,
   ...
   */
  nb_to_free =
      RTE_MIN(enic_ring_sub(desc_count, wq->tail_idx, completed_index) + 1,
...
unsigned int enic_cleanup_wq(__rte_unused struct enic *enic,
                             struct vnic_wq *wq) {
  uint16_t completed_index;
...
uint16_t enic_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (index = 0; index < nb_pkts; index++) {
    ...
    if (tso) {
      header_len = tx_pkt->l2_len + tx_pkt->l3_len + tx_pkt->l4_len;

      /* Drop if non-TCP packet or TSO seg size is too big */
      if (unlikely(header_len == 0 ||
                   ((tx_pkt->tso_segsz + header_len) > ENIC_TX_MAX_PKT_SIZE))) {
        rte_pktmbuf_free(tx_pkt);
        rte_atomic64_inc(tx_oversized);
        continue;
      }
      ...
      if (ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK) {
        header_len += tx_pkt->outer_l2_len + tx_pkt->outer_l3_len;
      }
    }
    ...
    wq_enet_desc_enc(&desc_tmp, bus_addr, data_len, mss, header_len,
                     offload_mode, eop, cq, 0, vlan_tag_insert, vlan_id, 0);
    ...
    *desc_p = desc_tmp;
    ...
    if (!eop) {
      for (tx_pkt = tx_pkt->next; tx_pkt; tx_pkt = tx_pkt->next) {
        data_len = tx_pkt->data_len;
        ...
        desc_p = descs + head_idx;
        bus_addr = (dma_addr_t)(tx_pkt->buf_iova + tx_pkt->data_off);
        wq_enet_desc_enc((struct wq_enet_desc *)&desc_tmp, bus_addr, data_len,
                         mss, 0, offload_mode, eop, cq, 0, vlan_tag_insert,
                         vlan_id, 0);
...
static void enqueue_simple_pkts(struct rte_mbuf **pkts,
                                struct wq_enet_desc *desc, uint16_t n,
                                struct enic *enic) {
  struct rte_mbuf *p;
  uint16_t mss;
  ...
  while (n) {
    ...
    desc->header_length_flags &=
        ((1 << WQ_ENET_FLAGS_EOP_SHIFT) | (1 << WQ_ENET_FLAGS_CQ_ENTRY_SHIFT));
    if (p->ol_flags & RTE_MBUF_F_TX_VLAN) {
      desc->header_length_flags |= 1 << WQ_ENET_FLAGS_VLAN_TAG_INSERT_SHIFT;
    }
    /*
