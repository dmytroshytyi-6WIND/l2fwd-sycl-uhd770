...
static inline uint16_t enic_cq_rx_desc_ciflags(struct cq_enet_rq_desc *crd) {
  return rte_le_to_cpu_16(crd->completed_index_flags) & ~CQ_DESC_COMP_NDX_MASK;
}

static inline uint16_t enic_cq_rx_desc_bwflags(struct cq_enet_rq_desc *crd) {
  return rte_le_to_cpu_16(crd->bytes_written_flags) &
         ~CQ_ENET_RQ_DESC_BYTES_WRITTEN_MASK;
}

static inline uint8_t enic_cq_rx_desc_packet_error(uint16_t bwflags) {
  return (bwflags & CQ_ENET_RQ_DESC_FLAGS_TRUNCATED) ==
         CQ_ENET_RQ_DESC_FLAGS_TRUNCATED;
}

static inline uint8_t enic_cq_rx_desc_eop(uint16_t ciflags) {
  return (ciflags & CQ_ENET_RQ_DESC_FLAGS_EOP) == CQ_ENET_RQ_DESC_FLAGS_EOP;
}
...
static inline uint8_t enic_cq_rx_desc_rss_type(struct cq_enet_rq_desc *cqrd) {
  return (uint8_t)(
      (rte_le_to_cpu_16(cqrd->q_number_rss_type_flags) >> CQ_DESC_Q_NUM_BITS) &
      CQ_ENET_RQ_DESC_RSS_TYPE_MASK);
}

static inline uint32_t enic_cq_rx_desc_rss_hash(struct cq_enet_rq_desc *cqrd) {
  return rte_le_to_cpu_32(cqrd->rss_hash);
}

static inline uint16_t enic_cq_rx_desc_vlan(struct cq_enet_rq_desc *cqrd) {
  return rte_le_to_cpu_16(cqrd->vlan);
}

static inline uint16_t enic_cq_rx_desc_n_bytes(struct cq_desc *cqd) {
  struct cq_enet_rq_desc *cqrd = (struct cq_enet_rq_desc *)cqd;
  return rte_le_to_cpu_16(cqrd->bytes_written_flags) &
         CQ_ENET_RQ_DESC_BYTES_WRITTEN_MASK;
}

static inline uint8_t enic_cq_rx_check_err(struct cq_desc *cqd) {
  struct cq_enet_rq_desc *cqrd = (struct cq_enet_rq_desc *)cqd;
  uint16_t bwflags;
...
static uint32_t enic_cq_rx_flags_to_pkt_type(struct cq_desc *cqd, uint8_t tnl) {
  struct cq_enet_rq_desc *cqrd = (struct cq_enet_rq_desc *)cqd;
  ...
  static const uint32_t cq_type_table[128] __rte_cache_aligned = {
      ...
      [0x01] = RTE_PTYPE_UNKNOWN | RTE_PTYPE_TUNNEL_GRENAT |
...
static void enic_cq_rx_to_pkt_flags(struct cq_desc *cqd,
  ...
  if (mbuf->packet_type & (RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L3_IPV6)) {
    if (!enic_cq_rx_desc_csum_not_calc(cqrd)) {
      ...
      if (l4_flags == RTE_PTYPE_L4_UDP || l4_flags == RTE_PTYPE_L4_TCP) {
