...
static int is_mcast_addr(uint8_t *addr) { return addr[0] & 1; }

static int is_eth_addr_valid(uint8_t *addr) {
  return !is_mcast_addr(addr) && !is_zero_addr(addr);
}

void enic_rxmbuf_queue_release(__rte_unused struct enic *enic,
                               struct vnic_rq *rq) {
  uint16_t i;
...
int enic_dev_stats_get(struct enic *enic, struct rte_eth_stats *r_stats) {
  ...
  r_stats->oerrors =
      stats->tx.tx_errors + rte_atomic64_read(&soft_stats->tx_oversized);
...
void enic_init_vnic_resources(struct enic *enic) {
  ...
  for (index = 0; index < enic->rq_count; index++) {
    ...
    vnic_rq_init(&enic->rq[enic_rte_rq_idx_to_sop_idx(index)], cq_idx,
                 error_interrupt_enable, error_interrupt_offset);

    data_rq = &enic->rq[enic_rte_rq_idx_to_data_idx(index, enic)];
    if (data_rq->in_use)
      vnic_rq_init(data_rq, cq_idx, error_interrupt_enable,
  ...
  for (index = 0; index < enic->wq_count; index++) {
    vnic_wq_init(&enic->wq[index], enic_cq_wq(enic, index),
                 error_interrupt_enable, error_interrupt_offset);
    ...
    vnic_cq_init(&enic->cq[cq_idx], 0 /* flow_control_enable */,
                 1 /* color_enable */, 0 /* cq_head */, 0 /* cq_tail */,
                 1 /* cq_tail_color */, 0 /* interrupt_enable */,
                 0 /* cq_entry_enable */, 1 /* cq_message_enable */,
                 0 /* interrupt offset */,
                 (uint64_t)enic->wq[index].cqmsg_rz->iova);
  }

  for (index = 0; index < enic->intr_count; index++) {
    vnic_intr_init(&enic->intr[index], enic->config.intr_timer_usec,
...
int enic_alloc_rx_queue_mbufs(struct enic *enic, struct vnic_rq *rq) {
  ...
  for (i = 0; i < rq->ring.desc_count; i++, rqd++) {
    ...
    dma_addr = (dma_addr_t)(mb->buf_iova + RTE_PKTMBUF_HEADROOM);
    rq_enet_desc_enc(
        rqd, dma_addr,
        (rq->is_sop ? RQ_ENET_TYPE_ONLY_SOP : RQ_ENET_TYPE_NOT_SOP),
        rq_buf_len);
    rq->mbuf_ring[i] = mb;
  }
  /*
 ...
 */
static void enic_initial_post_rx(struct enic *enic, struct vnic_rq *rq) {
...
void *enic_alloc_consistent(void *priv, size_t size, dma_addr_t *dma_handle,
  ...
  *dma_handle = 0;
  ...
  rz = rte_memzone_reserve_aligned((const char *)name, size, SOCKET_ID_ANY,
  ...
  *dma_handle = (dma_addr_t)rz->iova;
  ...
  mze = rte_malloc("enic memzone entry", sizeof(struct enic_memzone_entry), 0);
...
void enic_free_consistent(void *priv, __rte_unused size_t size, void *vaddr,
  ...
  LIST_FOREACH(mze, &enic->memzone_list, entries) {
    if (mze->rz->addr == vaddr && mze->rz->iova == dma_handle)
      break;
  }
  if (mze == NULL) {
    rte_spinlock_unlock(&enic->memzone_list_lock);
...
static void enic_intr_handler(void *arg) {
...
static int enic_rxq_intr_init(struct enic *enic) {
  ...
  if (rte_intr_vec_list_alloc(intr_handle, "enic_intr_vec", rxq_intr_count)) {
...
static void enic_prep_wq_for_simple_tx(struct enic *enic, uint16_t queue_idx) {
  ...
  for (i = 0; i < wq->ring.desc_count; i++, desc++) {
    ...
    if (i % ENIC_WQ_CQ_THRESH == ENIC_WQ_CQ_THRESH - 1)
      desc->header_length_flags |= (1 << WQ_ENET_FLAGS_CQ_ENTRY_SHIFT);
  }
}
...
int enic_enable(struct enic *enic) {
  ...
  for (index = 0; index < enic->rq_count; index++) {
    err = enic_alloc_rx_queue_mbufs(
        enic, &enic->rq[enic_rte_rq_idx_to_sop_idx(index)]);
    if (err) {
      dev_err(enic, "Failed to alloc sop RX queue mbufs\n");
      return err;
    }
    err = enic_alloc_rx_queue_mbufs(
        enic, &enic->rq[enic_rte_rq_idx_to_data_idx(index, enic)]);
    if (err) {
  ...
  if ((eth_dev->data->dev_conf.txmode.offloads & ~simple_tx_offloads) == 0) {
    ENICPMD_LOG(DEBUG, " use the simple tx handler");
    eth_dev->tx_pkt_burst = &enic_simple_xmit_pkts;
    for (index = 0; index < enic->wq_count; index++)
  ...
  rte_intr_callback_register(enic->pdev->intr_handle, enic_intr_handler,
                             (void *)enic->rte_dev);
...
int enic_alloc_intr_resources(struct enic *enic) {
  ...
  dev_info(enic,
           ...
           enic->wq_count, enic_vnic_rq_count(enic), enic->cq_count,
           enic->intr_count);
...
void enic_free_rq(void *rxq) {
  ...
  if (rq_sop->free_mbufs) {
    ...
    for (i = ENIC_RX_BURST_MAX - rq_sop->num_free_mbufs; i < ENIC_RX_BURST_MAX;
         i++)
...
int enic_alloc_rq(struct enic *enic, uint16_t queue_idx, unsigned int socket_id,
  ...
  if (enic->rte_dev->data->dev_conf.rxmode.offloads &
    ...
    if (max_rx_pktlen > mbuf_size) {
      dev_warning(enic,
                  "The maximum Rx packet size (%u) is"
  ...
  if (mbufs_per_pkt > 1) {
    ...
    if (enic->rte_dev->data->mtu < enic->max_mtu) {
      dev_warning(enic, "mtu is ignored when scatter rx mode is in use.\n");
    }
  } else {
  ...
  if (mbufs_per_pkt > 1) {
    ...
    max_sop = ((enic->config.rq_desc_count / (mbufs_per_pkt - 1)) &
               ENIC_ALIGN_DESCS_MASK);
  ...
  if (nb_desc < (min_sop + min_data)) {
    dev_warning(enic, "Number of rx descs too low, adjusting to minimum\n");
    nb_sop_desc = min_sop;
    nb_data_desc = min_data;
  } else if (nb_desc > (max_sop + max_data)) {
    dev_warning(enic, "Number of rx_descs too high, adjusting to maximum\n");
    nb_sop_desc = max_sop;
    nb_data_desc = max_data;
  }
  if (mbufs_per_pkt > 1) {
    dev_info(enic,
             "For max packet size %u and mbuf size %u valid"
             " rx descriptor range is %u to %u\n",
             max_rx_pktlen, mbuf_size, min_sop + min_data, max_sop + max_data);
  }
  dev_info(enic, "Using %d rx descriptors (sop %d, data %d)\n",
           nb_sop_desc + nb_data_desc, nb_sop_desc, nb_data_desc);

  /* Allocate sop queue resources */
  rc = vnic_rq_alloc(enic->vdev, rq_sop, sop_queue_idx, nb_sop_desc,
  ...
  if (rq_data->in_use) {
    ...
    rc = vnic_rq_alloc(enic->vdev, rq_data, data_queue_idx, nb_data_desc,
                       sizeof(struct rq_enet_desc));
    if (rc) {
  ...
  if (enic->cq64 && vnic_dev_set_cq_entry_size(enic->vdev, sop_queue_idx,
  ...
  rc = vnic_cq_alloc(enic->vdev, &enic->cq[cq_idx], cq_idx, socket_id,
                     nb_sop_desc + nb_data_desc,
  ...
  rq_sop->mbuf_ring = (struct rte_mbuf **)rte_zmalloc_socket(
      "rq->mbuf_ring", sizeof(struct rte_mbuf *) * nb_sop_desc,
      RTE_CACHE_LINE_SIZE, rq_sop->socket_id);
  if (rq_sop->mbuf_ring == NULL)
    goto err_free_cq;

  if (rq_data->in_use) {
    rq_data->mbuf_ring = (struct rte_mbuf **)rte_zmalloc_socket(
        "rq->mbuf_ring", sizeof(struct rte_mbuf *) * nb_data_desc,
        RTE_CACHE_LINE_SIZE, rq_sop->socket_id);
    if (rq_data->mbuf_ring == NULL)
      goto err_free_sop_mbuf;
  }

  rq_sop->free_mbufs = (struct rte_mbuf **)rte_zmalloc_socket(
      "rq->free_mbufs", sizeof(struct rte_mbuf *) * ENIC_RX_BURST_MAX,
      RTE_CACHE_LINE_SIZE, rq_sop->socket_id);
...
int enic_alloc_wq(struct enic *enic, uint16_t queue_idx, unsigned int socket_id,
  ...
  err = vnic_wq_alloc(enic->vdev, &enic->wq[queue_idx], queue_idx, nb_desc,
  ...
  err = vnic_cq_alloc(enic->vdev, &enic->cq[cq_index], cq_index, socket_id,
                      nb_desc, sizeof(struct cq_enet_wq_desc));
  if (err) {
    vnic_wq_free(wq);
  ...
  /* setup up CQ message */
  snprintf((char *)name, sizeof(name), "vnic_cqmsg-%s-%d-%d", enic->bdf_name,
           queue_idx, instance++);

  wq->cqmsg_rz = rte_memzone_reserve_aligned(
      (const char *)name, sizeof(uint32_t), SOCKET_ID_ANY,
      RTE_MEMZONE_IOVA_CONTIG, ENIC_PAGE_SIZE);
  if (!wq->cqmsg_rz)
    return -ENOMEM;
...
int enic_disable(struct enic *enic) {
  ...
  rte_intr_callback_unregister(enic->pdev->intr_handle, enic_intr_handler,
                               (void *)enic->rte_dev);
...
static int enic_dev_open(struct enic *enic) {
  ...
  err = enic_dev_wait(enic->vdev, vnic_dev_open, vnic_dev_open_done, flags);
  if (err)
...
static int enic_set_rsskey(struct enic *enic, uint8_t *user_key) {
  ...
  err = enic_set_rss_key(enic, rss_key_buf_pa, sizeof(union vnic_rss_key));

  /* Save for later queries */
  if (!err) {
    rte_memcpy(&enic->rss_key, rss_key_buf_va, sizeof(union vnic_rss_key));
  }
  enic_free_consistent(enic, sizeof(union vnic_rss_key), rss_key_buf_va,
                       rss_key_buf_pa);
...
int enic_set_rss_reta(struct enic *enic, union vnic_rss_cpu *rss_cpu) {
  ...
  err = enic_set_rss_cpu(enic, rss_cpu_buf_pa, sizeof(union vnic_rss_cpu));

  enic_free_consistent(enic, sizeof(union vnic_rss_cpu), rss_cpu_buf_va,
                       rss_cpu_buf_pa);
...
static int enic_set_niccfg(struct enic *enic, uint8_t rss_default_cpu,
  ...
  err = enic_set_nic_cfg(enic, rss_default_cpu, rss_hash_type, rss_hash_bits,
                         rss_base_cpu, rss_enable, tso_ipid_split_en,
...
int enic_init_rss_nic_cfg(struct enic *enic) {
  static uint8_t default_rss_key[] = {
      85,  67,  83,  97,  119, 101, 115, 111, 109, 101, 80,  65,  76,  79,
      117, 110, 105, 113, 117, 101, 76,  73,  78,  85,  88,  114, 111, 99,
      107, 115, 69,  78,  73,  67,  105, 115, 99,  111, 111, 108,
  };
...
int enic_setup_finish(struct enic *enic) {
  ...
  if (enic->switchdev_mode) {
    vnic_dev_packet_filter(enic->vdev, 0 /* directed  */, 0 /* multicast */,
                           0 /* broadcast */, 1 /* promisc   */,
                           0 /* allmulti  */);
    enic->promisc = 1;
    enic->allmulti = 0;
    return 0;
  ...
  /* Default conf */
  vnic_dev_packet_filter(enic->vdev, 1 /* directed  */, 1 /* multicast */,
                         1 /* broadcast */, 0 /* promisc   */,
                         1 /* allmulti  */);
...
int enic_set_rss_conf(struct enic *enic, struct rte_eth_rss_conf *rss_conf) {
  ...
  ret = enic_set_niccfg(enic, ENIC_RSS_DEFAULT_CPU, rss_hash_type,
                        ENIC_RSS_HASH_BITS, ENIC_RSS_BASE_CPU, rss_enable);
  if (!ret) {
    enic->rss_hf = rss_hf;
    ...
    dev_err(enic,
            "Failed to update RSS configurations."
            " hash=0x%x\n",
            rss_hash_type);
  }
  return ret;
...
int enic_add_packet_filter(struct enic *enic) {
  ...
  return vnic_dev_packet_filter(enic->vdev, 1, 1, 1, enic->promisc,
                                enic->allmulti);
}
...
int enic_set_vnic_res(struct enic *enic) {
  ...
  ENICPMD_LOG(DEBUG, "Required queues for PF: rq %u wq %u cq %u", required_rq,
              required_wq, required_cq);
  ...
  if (enic->conf_rq_count < required_rq) {
    dev_err(dev,
            "Not enough Receive queues. Requested:%u which uses %d RQs on VIC, "
            "Configured:%u\n",
            eth_dev->data->nb_rx_queues, required_rq, enic->conf_rq_count);
    rc = -EINVAL;
  }
  ...
  if (enic->conf_intr_count < required_intr) {
    dev_err(dev,
            "Not enough Interrupts to support Rx queue"
...
static int enic_reinit_rq(struct enic *enic, unsigned int rq_idx) {
  ...
  vnic_cq_init(&enic->cq[cq_idx], 0 /* flow_control_enable */,
               ...
               1 /* cq_tail_color */, 0 /* interrupt_enable */,
               1 /* cq_entry_enable */, 0 /* cq_message_enable */,
               0 /* interrupt offset */, 0 /* cq_message_addr */);

  vnic_rq_init_start(sop_rq,
                     enic_cq_rq(enic, enic_rte_rq_idx_to_sop_idx(rq_idx)), 0,
                     sop_rq->ring.desc_count - 1, 1, 0);
  if (data_rq->in_use) {
    vnic_rq_init_start(
        data_rq, enic_cq_rq(enic, enic_rte_rq_idx_to_data_idx(rq_idx, enic)), 0,
...
int enic_set_mtu(struct enic *enic, uint16_t new_mtu) {
  ...
  if (new_mtu > enic->max_mtu) {
    dev_err(enic, "MTU not updated: requested (%u) greater than max (%u)\n",
            new_mtu, enic->max_mtu);
    return -EINVAL;
  }
  if (new_mtu < ENIC_MIN_MTU) {
    dev_info(enic, "MTU not updated: requested (%u) less than min (%u)\n",
             new_mtu, ENIC_MIN_MTU);
    return -EINVAL;
  }
  if (new_mtu > config_mtu)
    dev_warning(enic, "MTU (%u) is greater than value configured in NIC (%u)\n",
                new_mtu, config_mtu);
  ...
  for (rq_idx = 0; rq_idx < enic->rq_count * 2; rq_idx++) {
    ...
    if (rq->is_sop && rq->in_use) {
      rc = enic_stop_rq(enic, enic_sop_rq_idx_to_rte_idx(rq_idx));
      if (rc) {
  ...
  for (rq_idx = 0; rq_idx < enic->rq_count; rq_idx++) {
    ...
    enic_free_rq(rq);
    rc = enic_alloc_rq(enic, rq_idx, rq->socket_id, rq->mp, rq->tot_nb_desc,
                       rq->rx_free_thresh);
    if (rc) {
      dev_err(enic, "Fatal MTU alloc error- No traffic will pass\n");
      goto set_mtu_done;
    }

    rc = enic_reinit_rq(enic, rq_idx);
    if (rc) {
      dev_err(enic, "Fatal MTU RQ reinit- No traffic will pass\n");
      goto set_mtu_done;
    }
  }
...
static void enic_disable_overlay_offload(struct enic *enic) {
  /*
  ...
  if (enic->geneve) {
    vnic_dev_overlay_offload_ctrl(enic->vdev, OVERLAY_FEATURE_GENEVE,
                                  OVERLAY_OFFLOAD_DISABLE);
  }
}

static int enic_enable_overlay_offload(struct enic *enic) {
  if (enic->vxlan &&
      vnic_dev_overlay_offload_ctrl(enic->vdev, OVERLAY_FEATURE_VXLAN,
  ...
  enic->tx_offload_mask |= RTE_MBUF_F_TX_OUTER_IPV6 | RTE_MBUF_F_TX_OUTER_IPV4 |
...
static int enic_reset_overlay_port(struct enic *enic) {
  if (enic->vxlan) {
     ...
     */
    if (vnic_dev_overlay_offload_cfg(enic->vdev, OVERLAY_CFG_VXLAN_PORT_UPDATE,
  ...
  if (enic->geneve) {
    ...
    if (vnic_dev_overlay_offload_cfg(enic->vdev, OVERLAY_CFG_GENEVE_PORT_UPDATE,
...
static int enic_dev_init(struct enic *enic) {
  ...
  if (enic->conf_rq_count == 1) {
    dev_err(
        enic,
        "Running with only 1 RQ configured in the vNIC is not supported.\n");
    dev_err(
        enic,
        "Please configure 2 RQs in the vNIC for each Rx queue used by DPDK.\n");
    dev_err(enic, "See the ENIC PMD guide for more information.\n");
    return -EINVAL;
  }
  /* Queue counts may be zeros. rte_zmalloc returns NULL in that case. */
  enic->cq = rte_zmalloc("enic_vnic_cq",
                         sizeof(struct vnic_cq) * enic->conf_cq_count, 8);
  enic->intr = rte_zmalloc("enic_vnic_intr",
                           sizeof(struct vnic_intr) * enic->conf_intr_count, 8);
  enic->rq = rte_zmalloc("enic_vnic_rq",
                         sizeof(struct vnic_rq) * enic->conf_rq_count, 8);
  enic->wq = rte_zmalloc("enic_vnic_wq",
                         sizeof(struct vnic_wq) * enic->conf_wq_count, 8);
  ...
  eth_dev->data->mac_addrs = rte_zmalloc(
      "enic_mac_addr",
      sizeof(struct rte_ether_addr) * ENIC_UNICAST_PERFECT_FILTERS, 0);
...
int enic_probe(struct enic *enic) {
  ...
  vnic_register_cbacks(enic->vdev, enic_alloc_consistent, enic_free_consistent);
  ...
  err =
      vnic_dev_set_ig_vlan_rewrite_mode(enic->vdev, enic->ig_vlan_rewrite_mode);
  if (err) {
    dev_err(enic, "Failed to set ingress vlan rewrite mode, aborting.\n");
