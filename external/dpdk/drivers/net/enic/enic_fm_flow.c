...
static int enic_fm_copy_item_eth(struct copy_item_args *arg) {
...
static int enic_fm_copy_item_vlan(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
...
static int enic_fm_copy_item_ipv4(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
...
static int enic_fm_copy_item_ipv6(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
...
static int enic_fm_copy_item_udp(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
...
static int enic_fm_copy_item_tcp(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
...
static int enic_fm_copy_item_sctp(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
...
static int enic_fm_copy_item_vxlan(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
...
static int enic_fm_copy_item_gtp(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  ...
  case RTE_FLOW_ITEM_TYPE_GTP: {
    ...
    if (!(fm_data->fk_metadata & FKM_UDP) ||
        !(fm_data->fk_header_select & FKH_UDP) || fm_data->l4.udp.fk_dest == 0)
      return -EINVAL;
  ...
  if (udp_gtp_uc_port_be) {
    if (fm_data->fk_metadata & FKM_UDP && fm_data->fk_header_select & FKH_UDP &&
        fm_data->l4.udp.fk_dest != udp_gtp_uc_port_be)
      return -EINVAL;
    if (fm_mask->fk_metadata & FKM_UDP && fm_mask->fk_header_select & FKH_UDP &&
        fm_mask->l4.udp.fk_dest != 0xFFFF)
      return -EINVAL;
...
static int enic_fm_copy_item_geneve(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
...
static int enic_fm_copy_item_geneve_opt(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  ...
  if (spec->option_len > 0 && (spec->data == NULL || mask->data == NULL)) {
    return rte_flow_error_set(arg->error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM,
  ...
  if (geneve->opt_len == 0) {
    return rte_flow_error_set(
        arg->error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
        "enic: geneve_opt requires non-zero geneve option length");
  }
  geneve = (struct rte_geneve_hdr *)&fm_mask->l4.rawdata[off];
  if (geneve->opt_len == 0) {
    return rte_flow_error_set(
        arg->error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
        "enic: geneve_opt requires non-zero geneve option length mask");
  }
...
static int enic_fm_copy_item_ecpri(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  ...
  if (!(fm_data->fk_metadata & FKM_UDP) &&
      (fm_mask->l2.eth.fk_ethtype != UINT16_MAX ||
       rte_cpu_to_be_16(fm_data->l2.eth.fk_ethtype) != RTE_ETHER_TYPE_ECPRI))
    return -EINVAL;
 ...
 */
static int enic_fm_copy_item_raw(struct copy_item_args *arg) {
  const struct rte_flow_item *item = arg->item;
  ...
  fm_data->fk_header_select &= ~FKH_UDP;
  fm_mask->fk_header_select &= ~FKH_UDP;
  memcpy(fm_data->l4.rawdata + sizeof(struct rte_udp_hdr), spec->pattern,
         spec->length);
  memcpy(fm_mask->l4.rawdata + sizeof(struct rte_udp_hdr), mask->pattern,
         spec->length);
  return 0;
}

static int flowman_cmd(struct enic_flowman *fm, uint64_t *args, int nargs) {
  return vnic_dev_flowman_cmd(fm->owner_enic->vdev, args, nargs);
}

static int enic_fet_alloc(struct enic_flowman *fm, uint8_t ingress,
...
static void enic_fet_free(struct enic_flowman *fm, struct enic_fm_fet *fet) {
  ENICPMD_FUNC_TRACE();
 ...
 */
static int enic_fet_get(struct enic_flowman *fm, uint32_t group,
                        uint8_t ingress, struct fm_key_template *key,
                        struct enic_fm_fet **fet_out,
                        struct rte_flow_error *error) {
  struct enic_fm_fet *fet;
  ...
  *fet_out = fet;
  ENICPMD_LOG(DEBUG, "fet_get: %s %s group=%u ref=%u",
              ...
              fet->ingress ? "ingress" : "egress", fet->group, fet->ref);
  return 0;
}

static void enic_fet_put(struct enic_flowman *fm, struct enic_fm_fet *fet) {
  ENICPMD_FUNC_TRACE();
...
static int fm_item_stacking_valid(enum rte_flow_item_type prev_item,
 ...
 */
static int enic_fm_copy_entry(struct enic_flowman *fm,
  ...
  for (; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    ...
    if (item->type >= RTE_DIM(enic_fm_items) || item_info->copy_item == NULL) {
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                                "enic: unsupported item");
    }
    /*
     * Check vNIC feature dependencies. Geneve item needs
     * Geneve offload feature
     */
    if (item->type == RTE_FLOW_ITEM_TYPE_GENEVE && !fm->user_enic->geneve) {
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                                "enic: geneve not supported");
    }
    /* check to see if item stacking is valid */
    if (!fm_item_stacking_valid(prev_item, item_info, is_first_item))
      goto stacking_error;
...
item_not_supported:
  return rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                            "enic: unsupported item type");

stacking_error:
  return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                            "enic: unsupported item stack");
}

static void flow_item_skip_void(const struct rte_flow_item **item) {
  for (;; (*item)++)
    if ((*item)->type != RTE_FLOW_ITEM_TYPE_VOID)
      return;
}

static void append_template(void **template, uint8_t *off, const void *data,
                            int len) {
...
static int enic_fm_append_action_op(struct enic_flowman *fm,
                                    struct fm_action_op *fm_op,
                                    struct rte_flow_error *error) {
  int count;

  count = fm->action_op_count;
  ENICPMD_LOG(DEBUG, "append action op: idx=%d op=%u", count, fm_op->fa_op);
...
static struct fm_action_op *find_prev_action_op(struct enic_flowman *fm,
                                                uint32_t opcode) {
 ...
 */
static void enic_fm_reorder_action_op(struct enic_flowman *fm) {
  ...
  while (op->fa_op != FMOP_END) {
    if (!decap &&
        (op->fa_op == FMOP_DECAP_NOSTRIP || op->fa_op == FMOP_DECAP_STRIP))
      decap = op;
  ...
  if (steer && decap && decap < steer) {
    op = fm->action.fma_action_ops;
    ENICPMD_LOG(DEBUG, "swap decap %ld <-> steer %ld", (long)(decap - op),
                (long)(steer - op));
    tmp_op = *decap;
    *decap = *steer;
    *steer = tmp_op;
...
static int enic_fm_copy_vxlan_decap(struct enic_flowman *fm,
...
static uint16_t gen_src_port(void) {
...
static int enic_fm_copy_vxlan_encap(struct enic_flowman *fm,
                                    const struct rte_flow_item *item,
                                    struct rte_flow_error *error) {
  struct fm_action_op fm_op;
  ...
  append_template(&template, &off, item->spec, sizeof(struct rte_ether_hdr));
  item++;
  flow_item_skip_void(&item);
  /* Optional VLAN */
  ...
  case RTE_FLOW_ITEM_TYPE_IPV4: {
    ...
    *ethertype = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4);
     ...
     */
    fm_op.encap.len1_offset = off + offsetof(struct rte_ipv4_hdr, total_length);
    fm_op.encap.len1_delta = sizeof(struct rte_ipv4_hdr) +
                             sizeof(struct rte_udp_hdr) +
                             sizeof(struct rte_vxlan_hdr);
    append_template(&template, &off, item->spec, sizeof(struct rte_ipv4_hdr));
  ...
  case RTE_FLOW_ITEM_TYPE_IPV6: {
    ...
    *ethertype = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6);
    ...
    fm_op.encap.len1_offset = off + offsetof(struct rte_ipv6_hdr, payload_len);
    fm_op.encap.len1_delta =
        sizeof(struct rte_udp_hdr) + sizeof(struct rte_vxlan_hdr);
    append_template(&template, &off, item->spec, sizeof(struct rte_ipv6_hdr));
  ...
  fm_op.encap.len2_offset = off + offsetof(struct rte_udp_hdr, dgram_len);
  fm_op.encap.len2_delta =
      sizeof(struct rte_udp_hdr) + sizeof(struct rte_vxlan_hdr);
  udp = (struct rte_udp_hdr *)template;
  append_template(&template, &off, item->spec, sizeof(struct rte_udp_hdr));
  /*
...
static int enic_fm_find_vnic(struct enic *enic, const struct rte_pci_addr *addr,
                             uint64_t *handle) {
  uint32_t bdf;
  uint64_t args[2];
  int rc;
  ...
  ENICPMD_FUNC_TRACE();
  ENICPMD_LOG(DEBUG, "bdf=%x:%x:%x", addr->bus, addr->devid, addr->function);
 ...
 */
static int vf_egress_port_id_action(struct enic_flowman *fm,
  ...
  if (!(src_enic->rte_dev->data->dev_flags & RTE_ETH_DEV_REPRESENTOR)) {
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                              "source port is not VF representor");
  }
...
static int enic_fm_check_transfer_dst(struct enic *enic, uint16_t dst_port_id,
                                      struct rte_eth_dev **dst_dev,
                                      struct rte_flow_error *error) {
  ...
  if (enic->switch_domain_id != pmd_priv(dev)->switch_domain_id) {
    return rte_flow_error_set(
        ...
        "destination and source ports are not in the same switch domain");
  }
...
static int enic_fm_copy_action(struct enic_flowman *fm,
                               const struct rte_flow_action actions[],
                               uint8_t ingress, struct rte_flow_error *error) {
  enum {
    FATE = 1 << 0,
    DECAP = 1 << 1,
    PASSTHRU = 1 << 2,
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_JUMP: {
      ...
      if (overlap & FATE)
        goto unsupported;
      ret = enic_fet_get(fm, jump->group, ingress, NULL, &fet, error);
      if (ret)
        return ret;
      overlap |= FATE;
      memset(&fm_op, 0, sizeof(fm_op));
    ...
    case RTE_FLOW_ACTION_TYPE_QUEUE: {
      ...
      fm_op.rq_steer.rq_index = enic_rte_rq_idx_to_sop_idx(queue->index);
      fm_op.rq_steer.rq_count = 1;
      fm_op.rq_steer.vnic_handle = vnic_h;
      ret = enic_fm_append_action_op(fm, &fm_op, error);
      if (ret)
        return ret;
      ENICPMD_LOG(DEBUG, "create QUEUE action rq: %u", fm_op.rq_steer.rq_index);
      steer = true;
      break;
    }
    ...
    case RTE_FLOW_ACTION_TYPE_RSS: {
       ...
       * sequential.
       */
      allow = rss->func == RTE_ETH_HASH_FUNCTION_DEFAULT && rss->level == 0 &&
              (rss->types == 0 || rss->types == enic->rss_hf) &&
      ...
      for (i = 1; i < rss->queue_num; i++)
        allow = allow && (rss->queue[i] == rss->queue[i - 1] + 1);
      if (!allow)
        goto unsupported;

      memset(&fm_op, 0, sizeof(fm_op));
      fm_op.fa_op = FMOP_RQ_STEER;
      fm_op.rq_steer.rq_index = enic_rte_rq_idx_to_sop_idx(rss->queue[0]);
    ...
    case RTE_FLOW_ACTION_TYPE_PORT_ID: {
      ...
      ret = enic_fm_check_transfer_dst(enic, port->id, &dev, error);
      if (ret)
        return ret;
      ...
      if (ingress)
        break;
      /* Egress */
      ret = vf_egress_port_id_action(fm, dev, vnic_h, &fm_op, error);
      if (ret)
        return ret;
      break;
    }
    case RTE_FLOW_ACTION_TYPE_VXLAN_DECAP: {
      ...
      ret = enic_fm_copy_vxlan_decap(fm, fmt, actions, error);
      if (ret != 0)
        return ret;
      break;
    }
    case RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP: {
      ...
      if (overlap & ENCAP)
        goto unsupported;
      overlap |= ENCAP;
      ret = enic_fm_copy_vxlan_encap(fm, encap->definition, error);
      if (ret != 0)
        return ret;
      break;
    }
    ...
    case RTE_FLOW_ACTION_TYPE_PORT_REPRESENTOR: {
      ...
      ethdev = actions->conf;
      ret = enic_fm_check_transfer_dst(enic, ethdev->port_id, &dev, error);
      if (ret)
        return ret;
      vnic_h = pmd_priv(dev)->fm_vnic_handle;
      overlap |= PORT_ID;
...
enic_fm_match_action(const struct rte_flow_action *action,
  ...
  for (; *supported_actions != RTE_FLOW_ACTION_TYPE_END; supported_actions++) {
...
static void enic_fm_dump_tcam_actions(const struct fm_action *fm_action) {
...
static int bits_to_str(uint32_t bits, const char *strings[], int max, char *buf,
                       int buf_len) {
  int i, n = 0, len = 0;
...
static void __enic_fm_dump_tcam_match(const struct fm_header_set *fk_hdrset,
  ...
  const char *fm_fkh_str[FKH_BIT_COUNT] = {
      [FKH_ETHER_BIT] = "eth",   [FKH_QTAG_BIT] = "qtag",
      [FKH_L2RAW_BIT] = "l2raw", [FKH_IPV4_BIT] = "ip4",
      [FKH_IPV6_BIT] = "ip6",    [FKH_L3RAW_BIT] = "l3raw",
      [FKH_UDP_BIT] = "udp",     [FKH_TCP_BIT] = "tcp",
      [FKH_ICMP_BIT] = "icmp",   [FKH_VXLAN_BIT] = "vxlan",
      [FKH_L4RAW_BIT] = "l4raw",
  };
...
static void enic_fm_dump_tcam_match(const struct fm_tcam_match_entry *match,
                                    uint8_t ingress) {
  char buf[256];

  memset(buf, 0, sizeof(buf));
  __enic_fm_dump_tcam_match(&match->ftm_mask.fk_hdrset[0], buf, sizeof(buf));
  ENICPMD_LOG(DEBUG, " TCAM %s Outer: %s %scounter position %u",
              (ingress) ? "IG" : "EG", buf,
              (match->ftm_flags & FMEF_COUNTER) ? "" : "no ",
              match->ftm_position);
  memset(buf, 0, sizeof(buf));
  __enic_fm_dump_tcam_match(&match->ftm_mask.fk_hdrset[1], buf, sizeof(buf));
  if (buf[0])
    ENICPMD_LOG(DEBUG, "         Inner: %s", buf);
}

/* Debug function to dump internal NIC flow structures. */
static void enic_fm_dump_tcam_entry(const struct fm_tcam_match_entry *fm_match,
...
static int enic_fm_flow_parse(struct enic_flowman *fm,
  ...
  if (attrs) {
    if (attrs->group != FM_TCAM_RTE_GROUP && attrs->priority) {
      rte_flow_error_set(
          error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY, NULL,
          "priorities are not supported for non-default (0) groups");
      return -rte_errno;
    } else if (!fm->owner_enic->switchdev_mode && attrs->transfer) {
      rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_TRANSFER,
                         NULL, "transfer is not supported");
      return -rte_errno;
    } else if (attrs->ingress && attrs->egress) {
      rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS, NULL,
  ...
  if (action->type != RTE_FLOW_ACTION_TYPE_END) {
    rte_flow_error_set(error, EPERM, RTE_FLOW_ERROR_TYPE_ACTION, action,
                       "invalid action");
    return -rte_errno;
  }
...
static void enic_fm_counter_free(struct enic_flowman *fm,
...
static int enic_fm_more_counters(struct enic_flowman *fm) {
  ...
  ENICPMD_FUNC_TRACE();
  new_stack = rte_realloc(fm->counter_stack,
                          (fm->counters_alloced + FM_COUNTERS_EXPAND) *
                              sizeof(struct enic_fm_counter),
                          0);
  if (new_stack == NULL) {
  ...
  ctrs = (struct enic_fm_counter *)fm->counter_stack + fm->counters_alloced;
  for (i = 0; i < FM_COUNTERS_EXPAND; i++, ctrs++) {
    ctrs->handle = fm->counters_alloced + i;
    SLIST_INSERT_HEAD(&fm->counters, ctrs, next);
  }
  fm->counters_alloced += FM_COUNTERS_EXPAND;
  ENICPMD_LOG(DEBUG, "%u counters allocated, total: %u", FM_COUNTERS_EXPAND,
              fm->counters_alloced);
  return 0;
}

static int enic_fm_counter_zero(struct enic_flowman *fm,
  ...
  if (ret) {
    ENICPMD_LOG(ERR, "counter init: rc=%d handle=0x%x", ret, c->handle);
    return ret;
  }
  return 0;
}

static int enic_fm_counter_alloc(struct enic_flowman *fm,
                                 struct rte_flow_error *error,
...
static int enic_fm_action_free(struct enic_flowman *fm,
  ...
  if (ah->ref == 0) {
    ...
    if (ret)
      ...
      ENICPMD_LOG(ERR, "freeing action rc=%d handle=0x%" PRIx64, ret,
                  ah->handle);
...
static int enic_fm_entry_free(struct enic_flowman *fm, uint64_t handle) {
  uint64_t args[2];
  int rc;
  ...
  if (rc)
    ENICPMD_LOG(ERR,
                "cannot free match entry: rc=%d"
                " handle=0x%" PRIx64,
                rc, handle);
  return rc;
}

static struct enic_fm_jump_flow *find_jump_flow(struct enic_flowman *fm,
                                                uint32_t group) {
...
static void remove_jump_flow(struct enic_flowman *fm, struct rte_flow *flow) {
  struct enic_fm_jump_flow *j;
...
static int save_jump_flow(struct enic_flowman *fm, struct rte_flow *flow,
                          uint32_t group, struct fm_tcam_match_entry *match,
                          struct fm_action *action) {
  struct enic_fm_jump_flow *j;
...
static void __enic_fm_flow_free(struct enic_flowman *fm,
                                struct enic_fm_flow *fm_flow) {
...
static void enic_fm_flow_free(struct enic_flowman *fm, struct rte_flow *flow) {
...
static int enic_fm_add_tcam_entry(struct enic_flowman *fm,
                                  struct fm_tcam_match_entry *match_in,
                                  uint64_t *entry_handle, uint8_t ingress,
                                  struct rte_flow_error *error) {
  ...
  if (ret != 0) {
    ...
    rte_flow_error_set(error, ret, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                       "enic: devcmd(tcam-entry-install)");
    return ret;
  }
...
static int enic_fm_add_exact_entry(struct enic_flowman *fm,
                                   struct fm_tcam_match_entry *match_in,
                                   uint64_t *entry_handle,
                                   struct enic_fm_fet *fet,
                                   struct rte_flow_error *error) {
  ...
  if (ret != 0) {
    ...
    rte_flow_error_set(error, ret, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                       "enic: devcmd(exact-entry-install)");
    return ret;
  }
  ENICPMD_LOG(DEBUG,
              "installed %s exact entry: group=%u"
              " handle=0x%" PRIx64,
              fet->ingress ? "ingress" : "egress", fet->group,
              (uint64_t)args[0]);
  *entry_handle = args[0];
...
static int enic_action_handle_get(struct enic_flowman *fm,
  ...
  ret = rte_hash_lookup_data(fm->action_hash, action_in, (void **)&ah);
  if (ret < 0 && ret != -ENOENT)
    return rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
  ...
  if (ret == -ENOENT) {
    ...
    if (ah == NULL)
      return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
                                "enic: calloc(fm-action)");
    ...
    if (ret != 0) {
      rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                         "enic: devcmd(action-alloc)");
      goto error_with_ah;
    }
    ah->handle = args[0];
    ret = rte_hash_add_key_data(fm->action_hash, (const void *)action_in,
                                (void *)ah);
    if (ret != 0) {
      rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
    ...
    ENICPMD_LOG(DEBUG, "action allocated: handle=0x%" PRIx64, ah->handle);
  }
...
static int __enic_fm_flow_add_entry(struct enic_flowman *fm,
                                    struct enic_fm_flow *fm_flow,
                                    struct fm_tcam_match_entry *match_in,
                                    struct fm_action *action_in, uint32_t group,
                                    uint8_t ingress,
                                    struct rte_flow_error *error) {
  struct enic_fm_counter *ctr;
  ...
  if (group == FM_TCAM_RTE_GROUP) {
    ret = enic_fm_add_tcam_entry(fm, match_in, &entry_h, ingress, error);
    if (ret)
      return ret;
    ...
    ret = enic_fet_get(fm, group, ingress, &match_in->ftm_mask, &fet, error);
    if (ret)
      return ret;
    fm_flow->fet = fet;
    ret = enic_fm_add_exact_entry(fm, match_in, &entry_h, fet, error);
    if (ret)
      return ret;
  }
...
/* Push match-action to the NIC. */
static struct rte_flow *enic_fm_flow_add_entry(
    struct enic_flowman *fm, struct fm_tcam_match_entry *match_in,
    struct fm_action *action_in, const struct rte_flow_attr *attrs,
    struct rte_flow_error *error) {
  struct enic_fm_flow *fm_flow;
  struct rte_flow *flow;
...
static void convert_jump_flows(struct enic_flowman *fm, struct enic_fm_fet *fet,
                               struct rte_flow_error *error) {
  struct enic_fm_flow *fm_flow;
  struct enic_fm_jump_flow *j;
  struct fm_action *fma;
  ...
  while (j) {
    ENICPMD_LOG(DEBUG, "convert jump flow: flow=%p group=%u", j->flow, group);
    /* Delete old entry */
    fm_flow = j->flow->fm;
    ...
    if (__enic_fm_flow_add_entry(fm, fm_flow, &j->match, fma, FM_TCAM_RTE_GROUP,
                                 fet->ingress, error)) {
      /* Cannot roll back changes at the moment */
      ENICPMD_LOG(ERR, "cannot convert jump flow: flow=%p", j->flow);
    } else {
      fm_flow->fet = fet;
      fet->ref++;
      ENICPMD_LOG(DEBUG, "convert ok: group=%u ref=%u", fet->group, fet->ref);
    }
...
static int add_hairpin_steer(struct enic_flowman *fm, struct rte_flow *flow,
                             struct rte_flow_error *error) {
  struct fm_tcam_match_entry *fm_tcam_entry;
  struct enic_fm_flow *fm_flow;
  struct fm_action *fm_action;
...
static void enic_fm_open_scratch(struct enic_flowman *fm) {
  fm->action_op_count = 0;
  fm->fet = NULL;
...
static void enic_fm_close_scratch(struct enic_flowman *fm) {
  if (fm->fet) {
...
static int enic_fm_flow_validate(struct rte_eth_dev *dev,
                                 const struct rte_flow_attr *attrs,
                                 const struct rte_flow_item pattern[],
                                 const struct rte_flow_action actions[],
                                 struct rte_flow_error *error) {
  ...
  if (!ret) {
    fm_tcam_entry = &fm->tcam_entry;
    fm_action = &fm->action;
    enic_fm_dump_tcam_entry(fm_tcam_entry, fm_action, attrs->ingress);
  }
...
static int enic_fm_flow_query_count(struct rte_eth_dev *dev,
  ...
  fm_flow = flow->fm;
  if (!fm_flow->counter_valid) {
    rc = rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, "enic: flow does not have counter");
    goto exit;
  }
  ...
  if (rc) {
    ENICPMD_LOG(ERR, "cannot query counter: rc=%d handle=0x%x", rc,
                fm_flow->counter->handle);
    goto exit;
  }
  query->hits_set = 1;
  query->hits = args[0];
...
static int enic_fm_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
...
enic_fm_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attrs,
                    const struct rte_flow_item pattern[],
                    const struct rte_flow_action actions[],
                    struct rte_flow_error *error) {
  struct fm_tcam_match_entry *fm_tcam_entry;
  ...
  fm_tcam_entry = &fm->tcam_entry;
  fm_action = &fm->action;
  flow = enic_fm_flow_add_entry(fm, fm_tcam_entry, fm_action, attrs, error);
  if (flow) {
    ...
    if (fet && fet->default_key) {
       ...
       * materializes.
       */
      save_jump_flow(fm, flow, fet->group, fm_tcam_entry, fm_action);
...
static int enic_fm_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
...
static int enic_fm_flow_flush(struct rte_eth_dev *dev,
                              __rte_unused struct rte_flow_error *error) {
...
static int enic_fm_tbl_free(struct enic_flowman *fm, uint64_t handle) {
  ...
  rc = flowman_cmd(fm, args, 2);
  if (rc)
    ENICPMD_LOG(ERR, "cannot free table: rc=%d handle=0x%" PRIx64, rc, handle);
  return rc;
}

static int enic_fm_tcam_tbl_alloc(struct enic_flowman *fm, uint32_t direction,
...
static int enic_fm_init_actions(struct enic_flowman *fm) {
  ...
  ENICPMD_FUNC_TRACE();
  snprintf((char *)name, sizeof(name), "fm-ah-%s", fm->owner_enic->bdf_name);
  params.name = name;
...
static int enic_fm_init_counters(struct enic_flowman *fm) {
  ENICPMD_FUNC_TRACE();
  SLIST_INIT(&fm->counters);
  return enic_fm_more_counters(fm);
}

static void enic_fm_free_all_counters(struct enic_flowman *fm) {
  uint64_t args[2];
  int rc;
...
static int enic_fm_alloc_tcam_tables(struct enic_flowman *fm) {
  int rc;
...
static void enic_fm_free_tcam_tables(struct enic_flowman *fm) {
  ENICPMD_FUNC_TRACE();
  if (fm->ig_tcam_hndl) {
...
int enic_fm_init(struct enic *enic) {
  ...
  if (rc) {
    ENICPMD_LOG(ERR, "cannot find vnic handle for %x:%x:%x", addr->bus,
  ...
  fm->cmd.va = enic_alloc_consistent(enic, sizeof(union enic_flowman_cmd_mem),
                                     &fm->cmd.pa, name);
  if (!fm->cmd.va) {
...
error_cmd:
  enic_free_consistent(enic, sizeof(union enic_flowman_cmd_mem), fm->cmd.va,
                       fm->cmd.pa);
error_fm:
  free(fm);
  return rc;
...
int enic_fm_allocate_switch_domain(struct enic *pf) {
  ...
  RTE_ETH_FOREACH_DEV(pid) {
    ...
    if (!enic_fm_find_vnic(cur, prev_a, &vnic_h)) {
      ENICPMD_LOG(DEBUG,
                  "Port %u (PF BDF %x:%x:%x) and port %u (PF BDF %x:%x:%x "
                  "domain %u) are on the same VIC",
                  cur->rte_dev->data->port_id, cur_a->bus, cur_a->devid,
                  cur_a->function, dev->data->port_id, prev_a->bus,
                  prev_a->devid, prev_a->function, prev->switch_domain_id);
  ...
  if (ret) {
    ENICPMD_LOG(WARNING, "failed to allocate switch domain for device %d", ret);
  }
  cur->switch_domain_id = domain_id;
  ENICPMD_LOG(DEBUG,
              "Port %u (PF BDF %x:%x:%x) is the 1st PF on the VIC. Allocated "
              "switch domain id %u",
              cur->rte_dev->data->port_id, cur_a->bus, cur_a->devid,
              cur_a->function, domain_id);
  return ret;
}
...
int enic_fm_add_rep2vf_flow(struct enic_vf_representor *vf) {
  ...
  flow0 = enic_fm_flow_add_entry(fm, fm_tcam_entry, fm_action, &attrs, &error);
  enic_fm_close_scratch(fm);
  if (flow0 == NULL) {
  ...
  attrs.egress = 0;
  attrs.priority = FM_HIGHEST_PRIORITY;
  flow1 = enic_fm_flow_add_entry(fm, fm_tcam_entry, fm_action, &attrs, &error);
  enic_fm_close_scratch(fm);
  if (flow1 == NULL) {
  ...
  flow1->internal = 1;
  ENICPMD_LOG(DEBUG,
              "representor->VF %d flow created: tag %d hairpinned -> VF RQ %d",
...
int enic_fm_add_vf2rep_flow(struct enic_vf_representor *vf) {
  ...
  ENICPMD_LOG(
      DEBUG,
      "VF %d->representor flow created: wq %d (low prio) -> tag %d hairpin",
  ...
  flow1->internal = 1;
  ENICPMD_LOG(DEBUG,
              "VF %d->representor flow created: tag %d hairpinned -> PF RQ %d",
