 ...
 *   0 on success, else -1.
 */
static int tx_recover_qp(struct mlx5_txq_ctrl *txq_ctrl) {
...
static int check_err_cqe_seen(volatile struct mlx5_err_cqe *err_cqe) {
 ...
 */
static int mlx5_tx_error_cqe_handle(struct mlx5_txq_data *__rte_restrict txq,
  ...
  if (err_cqe->syndrome != MLX5_CQE_SYNDROME_WR_FLUSH_ERR) {
    ...
    if (!seen &&
        txq_ctrl->dump_file_n < txq_ctrl->priv->config.max_dump_files_num) {
      MKSTR(err_str,
            "Unexpected CQE error syndrome "
            "0x%02x CQN = %u SQN = %u wqe_counter = %u "
            "wq_ci = %u cq_ci = %u",
            err_cqe->syndrome, txq->cqe_s, txq->qp_num_8s >> 8,
            rte_be_to_cpu_16(err_cqe->wqe_counter), txq->wqe_ci, txq->cq_ci);
      MKSTR(name, "dpdk_mlx5_port_%u_txq_%u_index_%u_%u",
            PORT_ID(txq_ctrl->priv), txq->idx, txq_ctrl->dump_file_n,
            (uint32_t)rte_rdtsc());
      mlx5_dump_debug_information(name, NULL, err_str, 0);
      mlx5_dump_debug_information(
          name, "MLX5 Error CQ:", (const void *)((uintptr_t)txq->cqes),
          sizeof(*err_cqe) * (1 << txq->cqe_n));
      mlx5_dump_debug_information(
          name, "MLX5 Error SQ:", (const void *)((uintptr_t)txq->wqes),
          MLX5_WQE_SIZE * (1 << txq->wqe_n));
    ...
    if (!seen)
       ...
       */
      txq->stats.oerrors += ((txq->wqe_ci & wqe_m) - new_wqe_pi) & wqe_m;
...
void mlx5_tx_handle_completion(struct mlx5_txq_data *__rte_restrict txq,
  ...
  do {
    ...
    if (unlikely(ret != MLX5_CQE_STATUS_SW_OWN)) {
       ...
       */
      rte_wmb();
      ret = mlx5_tx_error_cqe_handle(txq, (volatile struct mlx5_err_cqe *)cqe);
      if (unlikely(ret < 0)) {
        /*
    ...
    MLX5_ASSERT((txq->fcqs[txq->cq_ci & txq->cqe_m] >> 16) == cqe->wqe_counter);
#endif
    ring_doorbell = true;
    ++txq->cq_ci;
    last_cqe = cqe;
  ...
  unsigned int olx;
    ...
    full_empw, MLX5_TXOFF_CONFIG_MULTI | MLX5_TXOFF_CONFIG_TSO |
                   ...
                   MLX5_TXOFF_CONFIG_METADATA | MLX5_TXOFF_CONFIG_EMPW)
                      ...
                      MLX5_TXOFF_INFO(none_empw, MLX5_TXOFF_CONFIG_NONE |
                                                     MLX5_TXOFF_CONFIG_EMPW)

                          MLX5_TXOFF_INFO(md_empw, MLX5_TXOFF_CONFIG_METADATA |
                                                       MLX5_TXOFF_CONFIG_EMPW)
                                                                                          ...
                                                                                          MLX5_TXOFF_INFO(
                                                                                              ...
                                                                                              MLX5_TXOFF_INFO(
                                                                                                  ...
                                                                                                  MLX5_TXOFF_INFO(
                                                                                                      ...
                                                                                                      MLX5_TXOFF_INFO(mti_ts, MLX5_TXOFF_CONFIG_MULTI | MLX5_TXOFF_CONFIG_TSO | MLX5_TXOFF_CONFIG_INLINE | MLX5_TXOFF_CONFIG_METADATA |
                                                                                                                                  MLX5_TXOFF_CONFIG_TXPP |
                                                                                                                                  MLX5_TXOFF_CONFIG_EMPW)
                                                                                                                                  ...
                                                                                                                                  MLX5_TXOFF_INFO(
                                                                                                                                      ...
                                                                                                                                      MLX5_TXOFF_INFO(
                                                                                                                                          ...
                                                                                                                                          MLX5_TXOFF_INFO(
                                                                                                                                              ...
                                                                                                                                              MLX5_TXOFF_INFO(
                                                                                                                                                  sc,
                                                                                                                                                  MLX5_TXOFF_CONFIG_SWP | MLX5_TXOFF_CONFIG_CSUM | MLX5_TXOFF_CONFIG_METADATA)
                                                                                                                                                                  ...
                                                                                                                                                                  MLX5_TXOFF_INFO(
                                                                                                                                                                      ...
                                                                                                                                                                      MLX5_TXOFF_INFO(
                                                                                                                                                                          ...
                                                                                                                                                                          MLX5_TXOFF_INFO(
                                                                                                                                                                              ...
                                                                                                                                                                              MLX5_TXOFF_INFO(
                                                                                                                                                                                  mci_mpw, MLX5_TXOFF_CONFIG_MULTI | MLX5_TXOFF_CONFIG_CSUM |
                                                                                                                                                                                               MLX5_TXOFF_CONFIG_INLINE |
                                                                                                                                                                                               MLX5_TXOFF_CONFIG_EMPW |
                                                                                                                                                                                               MLX5_TXOFF_CONFIG_MPW)
...
eth_tx_burst_t mlx5_select_tx_function(struct rte_eth_dev *dev) {
  ...
  if (tx_offloads & RTE_ETH_TX_OFFLOAD_SEND_ON_TIMESTAMP &&
      rte_mbuf_dynflag_lookup(RTE_MBUF_DYNFLAG_TX_TIMESTAMP_NAME, NULL) >= 0 &&
      rte_mbuf_dynfield_lookup(RTE_MBUF_DYNFIELD_TIMESTAMP_NAME, NULL) >= 0) {
  ...
  if (config->mps == MLX5_MPW_ENHANCED && config->txq_inline_min <= 0) {
    /*
  ...
  if (config->mps == MLX5_MPW) {
     ...
     */
    if (!(olx & (MLX5_TXOFF_CONFIG_TSO | MLX5_TXOFF_CONFIG_SWP |
                 MLX5_TXOFF_CONFIG_VLAN | MLX5_TXOFF_CONFIG_METADATA)))
      olx |= MLX5_TXOFF_CONFIG_EMPW | MLX5_TXOFF_CONFIG_MPW;
  }
  /*
  ...
  if (m >= RTE_DIM(txoff_func)) {
    DRV_LOG(DEBUG,
            "port %u has no selected Tx function"
            " for requested offloads %04X",
            dev->data->port_id, olx);
    return NULL;
  }
  DRV_LOG(DEBUG,
          "port %u has selected Tx function"
 ...
 *   None.
 */
void mlx5_txq_info_get(struct rte_eth_dev *dev, uint16_t tx_queue_id,
...
int mlx5_tx_burst_mode_get(struct rte_eth_dev *dev, uint16_t tx_queue_id,
  ...
  for (i = 0; i < RTE_DIM(txoff_func); i++) {
    if (pkt_burst == txoff_func[i].func) {
      ...
      snprintf(
          mode->info, sizeof(mode->info), "%s%s%s%s%s%s%s%s%s%s",
          (olx & MLX5_TXOFF_CONFIG_EMPW)
              ? ((olx & MLX5_TXOFF_CONFIG_MPW) ? "Legacy MPW" : "Enhanced MPW")
              : "No MPW",
          (olx & MLX5_TXOFF_CONFIG_MULTI) ? " + MULTI" : "",
          (olx & MLX5_TXOFF_CONFIG_TSO) ? " + TSO" : "",
          (olx & MLX5_TXOFF_CONFIG_SWP) ? " + SWP" : "",
          (olx & MLX5_TXOFF_CONFIG_CSUM) ? "  + CSUM" : "",
          (olx & MLX5_TXOFF_CONFIG_INLINE) ? " + INLINE" : "",
          (olx & MLX5_TXOFF_CONFIG_VLAN) ? " + VLAN" : "",
          (olx & MLX5_TXOFF_CONFIG_METADATA) ? " + METADATA" : "",
          (olx & MLX5_TXOFF_CONFIG_TXPP) ? " + TXPP" : "",
          (txq && txq->fast_free) ? " + Fast Free" : "");
      return 0;
    }
  }
  return -EINVAL;
