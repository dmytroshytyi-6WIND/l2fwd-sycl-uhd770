 ...
 */
static uint16_t rxq_handle_pending_error(struct mlx5_rxq_data *rxq,
 ...
 *   Pointer to RX queue structure.
 */
static inline void mlx5_rx_replenish_bulk_mbuf(struct mlx5_rxq_data *rxq) {
  ...
  if (n >= rxq->rq_repl_thresh) {
    ...
    MLX5_ASSERT(MLX5_VPMD_RXQ_RPLNSH_THRESH(q_n) > MLX5_VPMD_DESCS_PER_LOOP);
    ...
    if (unlikely(mlx5_mr_btree_len(&rxq->mr_ctrl.cache_bh) > 1)) {
      for (i = 0; i < n; ++i) {
        ...
        wq[i].addr =
            rte_cpu_to_be_64((uintptr_t)buf_addr + RTE_PKTMBUF_HEADROOM);
 ...
 *   Pointer to RX queue structure.
 */
static inline void mlx5_rx_mprq_replenish_bulk_mbuf(struct mlx5_rxq_data *rxq) {
  ...
  if (n >= rxq->rq_repl_thresh &&
    ...
    MLX5_ASSERT(MLX5_VPMD_RXQ_RPLNSH_THRESH(elts_n) > MLX5_VPMD_DESCS_PER_LOOP);
    /* Not to cross queue end. */
 ...
 */
static inline uint16_t rxq_copy_mprq_mbuf_v(struct mlx5_rxq_data *rxq,
                                            struct rte_mbuf **pkts,
                                            uint16_t pkts_n) {
  const uint16_t wqe_n = 1 << rxq->elts_n;
  ...
  for (i = 0; i < pkts_n; ++i) {
    ...
    strd_cnt =
        (elts[i]->pkt_len / strd_sz) + ((elts[i]->pkt_len % strd_sz) ? 1 : 0);
 ...
 */
static inline uint16_t rxq_burst_v(struct mlx5_rxq_data *rxq,
  ...
  if (rcvd_pkt > 0) {
    ...
    rxq_copy_mbuf_v(&(*rxq->elts)[rxq->rq_pi & e_mask], pkts, rcvd_pkt);
    rxq->rq_pi += rcvd_pkt;
  ...
  if (comp_idx < MLX5_VPMD_DESCS_PER_LOOP) {
    ...
    rxq->decompressed = rxq_cq_decompress_v(rxq, &cq[nocmp_n], &elts[nocmp_n]);
    ...
    if (nocmp_n < pkts_n) {
      ...
      rxq_copy_mbuf_v(&(*rxq->elts)[rxq->rq_pi & e_mask], &pkts[nocmp_n], n);
      rxq->rq_pi += n;
      rcvd_pkt += n;
      rxq->decompressed -= n;
    }
 ...
 *   Number of packets received including errors (<= pkts_n).
 */
static inline uint16_t rxq_burst_mprq_v(struct mlx5_rxq_data *rxq,
                                        struct rte_mbuf **pkts, uint16_t pkts_n,
                                        uint64_t *err, bool *no_cq) {
  const uint16_t q_n = 1 << rxq->cqe_n;
  const uint16_t q_mask = q_n - 1;
...
uint16_t mlx5_rx_burst_mprq_vec(void *dpdk_rxq, struct rte_mbuf **pkts,
  ...
  do {
    nb_rx = rxq_burst_mprq_v(rxq, pkts + tn, pkts_n - tn, &err, &no_cq);
    if (unlikely(err | rxq->err_state))
      nb_rx = rxq_handle_pending_error(rxq, pkts + tn, nb_rx);
    tn += nb_rx;
    if (unlikely(no_cq))
 ...
 */
int __rte_cold mlx5_rxq_check_vec_support(struct mlx5_rxq_data *rxq) {
 ...
 *   1 if supported, negative errno value if not.
 */
int __rte_cold mlx5_check_vec_rx_support(struct rte_eth_dev *dev) {
