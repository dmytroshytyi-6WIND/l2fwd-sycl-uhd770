 ...
 */
static void flow_hw_rxq_flag_set(struct rte_eth_dev *dev, bool enable) {
  struct mlx5_priv *priv = dev->data->dev_private;
  unsigned int i;

  if ((!priv->mark_enabled && !enable) || (priv->mark_enabled && enable))
    return;
 ...
 *   Item flags.
 */
static uint64_t flow_hw_rss_item_flags_get(const struct rte_flow_item items[]) {
 ...
 */
static struct mlx5_hw_jump_action *flow_hw_jump_action_register(
    struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
    uint32_t dest_group, struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
...
static void flow_hw_jump_release(struct rte_eth_dev *dev,
...
flow_hw_tir_action_register(struct rte_eth_dev *dev, uint32_t hws_flags,
  ...
  if (action->type == RTE_FLOW_ACTION_TYPE_QUEUE) {
    ...
    memcpy(rss_desc.key, !rss->key ? rss_hash_default_key : rss->key,
           MLX5_RSS_HASH_KEY_LEN);
    rss_desc.key_len = MLX5_RSS_HASH_KEY_LEN;
    rss_desc.types = !rss->types ? RTE_ETH_RSS_IP : rss->types;
    flow_dv_hashfields_set(0, &rss_desc, &rss_desc.hash_fields);
    flow_dv_action_rss_l34_hash_adjust(rss->types, &rss_desc.hash_fields);
    if (rss->level > 1) {
 ...
 */
static void __flow_hw_action_template_destroy(struct rte_eth_dev *dev,
  ...
  if (acts->jump) {
    ...
    grp = container_of(acts->jump, struct mlx5_flow_group, jump);
    mlx5_hlist_unregister(priv->sh->flow_tbls, &grp->entry);
    acts->jump = NULL;
  }
}
...
__flow_hw_act_data_alloc(struct mlx5_priv *priv, enum rte_flow_action_type type,
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static __rte_always_inline int __flow_hw_act_data_general_append(
    struct mlx5_priv *priv, struct mlx5_hw_actions *acts,
    enum rte_flow_action_type type, uint16_t action_src, uint16_t action_dst) {
  struct mlx5_action_construct_data *act_data;
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static __rte_always_inline int __flow_hw_act_data_encap_append(
    struct mlx5_priv *priv, struct mlx5_hw_actions *acts,
    enum rte_flow_action_type type, uint16_t action_src, uint16_t action_dst,
    uint16_t encap_src, uint16_t encap_dst, uint16_t len) {
  struct mlx5_action_construct_data *act_data;
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static __rte_always_inline int __flow_hw_act_data_shared_rss_append(
    struct mlx5_priv *priv, struct mlx5_hw_actions *acts,
    enum rte_flow_action_type type, uint16_t action_src, uint16_t action_dst,
    uint32_t idx, struct mlx5_shared_action_rss *rss) {
  struct mlx5_action_construct_data *act_data;
  ...
  act_data->shared_rss.types =
      !rss->origin.types ? RTE_ETH_RSS_IP : rss->origin.types;
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static __rte_always_inline int flow_hw_shared_action_translate(
    struct rte_eth_dev *dev, const struct rte_flow_action *action,
    struct mlx5_hw_actions *acts, uint16_t action_src, uint16_t action_dst) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  case MLX5_INDIRECT_ACTION_TYPE_RSS:
    ...
    if (!shared_rss ||
        __flow_hw_act_data_shared_rss_append(
            priv, acts,
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static __rte_always_inline int flow_hw_encap_item_translate(
    struct rte_eth_dev *dev, struct mlx5_hw_actions *acts,
    enum rte_flow_action_type type, uint16_t action_src, uint16_t action_dst,
    const struct rte_flow_item *items, const struct rte_flow_item *items_m) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++, items_m++, i++) {
    ...
    if ((!items_m->spec || memcmp(items_m->spec, items->spec, len)) &&
        __flow_hw_act_data_encap_append(priv, acts, type, action_src,
                                        action_dst, i, total_len, len))
      return -1;
    total_len += len;
  }
  return 0;
...
flow_hw_actions_translate(struct rte_eth_dev *dev,
  ...
  for (i = 0; !actions_end; actions++, masks++) {
    ...
    case RTE_FLOW_ACTION_TYPE_INDIRECT:
      ...
      if (actions->conf && masks->conf) {
        if (flow_hw_shared_action_translate(dev, actions, acts,
          ...
          goto err;
      } else if (__flow_hw_act_data_general_append(priv, acts, actions->type,
                                                   actions - action_start, i)) {
        goto err;
      }
      i++;
    ...
    case RTE_FLOW_ACTION_TYPE_DROP:
      acts->rule_acts[i++].action = priv->hw_drop[!!attr->group][type];
      break;
    case RTE_FLOW_ACTION_TYPE_MARK:
      acts->mark = true;
      if (masks->conf)
        acts->rule_acts[i].tag.value = mlx5_flow_mark_set(
            ((const struct rte_flow_action_mark *)(masks->conf))->id);
      else if (__flow_hw_act_data_general_append(priv, acts, actions->type,
                                                 actions - action_start, i))
        goto err;
      acts->rule_acts[i++].action = priv->hw_tag[!!attr->group];
      flow_hw_rxq_flag_set(dev, true);
      break;
    case RTE_FLOW_ACTION_TYPE_JUMP:
      if (masks->conf) {
        uint32_t jump_group =
            ((const struct rte_flow_action_jump *)actions->conf)->group;
        acts->jump = flow_hw_jump_action_register(dev, attr, jump_group, error);
        if (!acts->jump)
          goto err;
        acts->rule_acts[i].action =
            (!!attr->group) ? acts->jump->hws_action : acts->jump->root_action;
      } else if (__flow_hw_act_data_general_append(priv, acts, actions->type,
                                                   actions - action_start, i)) {
        goto err;
      }
    ...
    case RTE_FLOW_ACTION_TYPE_QUEUE:
      if (masks->conf) {
        acts->tir = flow_hw_tir_action_register(
            dev, mlx5_hw_act_flag[!!attr->group][type], actions);
        if (!acts->tir)
          goto err;
    ...
    case RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP:
      ...
      enc_item = ((const struct rte_flow_action_vxlan_encap *)actions->conf)
                     ->definition;
      enc_item_m =
          ((const struct rte_flow_action_vxlan_encap *)masks->conf)->definition;
      reformat_pos = i++;
    ...
    case RTE_FLOW_ACTION_TYPE_NVGRE_ENCAP:
      MLX5_ASSERT(reformat_pos == MLX5_HW_MAX_ACTS);
      enc_item = ((const struct rte_flow_action_nvgre_encap *)actions->conf)
                     ->definition;
      enc_item_m = ((const struct rte_flow_action_nvgre_encap *)actions->conf)
                       ->definition;
      reformat_pos = i++;
      reformat_src = actions - action_start;
      refmt_type = MLX5DR_ACTION_REFORMAT_TYPE_L2_TO_TNL_L2;
    ...
    case RTE_FLOW_ACTION_TYPE_RAW_ENCAP:
      raw_encap_data = (const struct rte_flow_action_raw_encap *)actions->conf;
      encap_data = raw_encap_data->data;
      data_size = raw_encap_data->size;
      if (reformat_pos != MLX5_HW_MAX_ACTS) {
        refmt_type = data_size < MLX5_ENCAPSULATION_DECISION_SIZE
  ...
  if (reformat_pos != MLX5_HW_MAX_ACTS) {
    ...
    if (enc_item) {
      ...
      if (flow_dv_convert_encap_data(enc_item, buf, &data_size, error) ||
          flow_hw_encap_item_translate(
              dev, acts, (action_start + reformat_src)->type, reformat_src,
              reformat_pos, enc_item, enc_item_m))
        goto err;
      encap_data = buf;
    } else if (encap_data &&
               __flow_hw_act_data_encap_append(
                   priv, acts, (action_start + reformat_src)->type,
                   reformat_src, reformat_pos, 0, 0, data_size)) {
      goto err;
    }
    acts->encap_decap =
        mlx5_malloc(MLX5_MEM_ZERO, sizeof(*acts->encap_decap) + data_size, 0,
                    SOCKET_ID_ANY);
    if (!acts->encap_decap)
      goto err;
    if (data_size) {
    ...
    acts->encap_decap->action = mlx5dr_action_create_reformat(
        priv->dr_ctx, refmt_type, data_size, encap_data,
        rte_log2_u32(table_attr->nb_flows),
        mlx5_hw_act_flag[!!attr->group][type]);
    if (!acts->encap_decap->action)
      goto err;
    acts->rule_acts[reformat_pos].action = acts->encap_decap->action;
...
err:
  ...
  return rte_flow_error_set(error, err, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static __rte_always_inline int flow_hw_shared_action_get(
    struct rte_eth_dev *dev, struct mlx5_action_construct_data *act_data,
    const uint64_t item_flags, struct mlx5dr_rule_action *rule_act) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  case MLX5_RTE_FLOW_ACTION_TYPE_RSS:
    ...
    hrxq_idx = flow_dv_action_rss_hrxq_lookup(dev, act_data->shared_rss.idx,
                                              hash_fields);
    if (hrxq_idx)
    ...
    break;
  default:
    DRV_LOG(WARNING, "Unsupported shared action type:%d", act_data->type);
    break;
  }
  return -1;
}
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static __rte_always_inline int flow_hw_shared_action_construct(
    struct rte_eth_dev *dev, const struct rte_flow_action *action,
    struct rte_flow_template_table *table, const uint8_t it_idx,
  ...
  case MLX5_INDIRECT_ACTION_TYPE_RSS:
    ...
    act_data.shared_rss.types =
        !shared_rss->origin.types ? RTE_ETH_RSS_IP : shared_rss->origin.types;
    item_flags = table->its[it_idx]->item_flags;
    if (flow_hw_shared_action_get(dev, &act_data, item_flags, rule_act))
      return -1;
    break;
  default:
    DRV_LOG(WARNING, "Unsupported shared action type:%d", type);
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static __rte_always_inline int flow_hw_actions_construct(
    struct rte_eth_dev *dev, struct mlx5_hw_q_job *job,
    const struct mlx5_hw_actions *hw_acts, const uint8_t it_idx,
    const struct rte_flow_action actions[],
    struct mlx5dr_rule_action *rule_acts, uint32_t *acts_num) {
  ...
  memcpy(rule_acts, hw_acts->rule_acts, sizeof(*rule_acts) * hw_acts->acts_num);
  *acts_num = hw_acts->acts_num;
  ...
  if (hw_acts->encap_decap && hw_acts->encap_decap->data_size)
    memcpy(buf, hw_acts->encap_decap->data, hw_acts->encap_decap->data_size);
  LIST_FOREACH(act_data, &hw_acts->act_list, next) {
    ...
    case RTE_FLOW_ACTION_TYPE_INDIRECT:
      if (flow_hw_shared_action_construct(dev, action, table, it_idx,
    ...
    case RTE_FLOW_ACTION_TYPE_MARK:
      tag = mlx5_flow_mark_set(
          ((const struct rte_flow_action_mark *)(action->conf))->id);
      rule_acts[act_data->action_dst].tag.value = tag;
      break;
    case RTE_FLOW_ACTION_TYPE_JUMP:
      jump_group = ((const struct rte_flow_action_jump *)action->conf)->group;
      jump = flow_hw_jump_action_register(dev, &attr, jump_group, NULL);
      if (!jump)
        return -1;
    ...
    case MLX5_RTE_FLOW_ACTION_TYPE_RSS:
      item_flags = table->its[it_idx]->item_flags;
      if (flow_hw_shared_action_get(dev, act_data, item_flags,
                                    &rule_acts[act_data->action_dst]))
        return -1;
      break;
    case RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP:
      enc_item = ((const struct rte_flow_action_vxlan_encap *)action->conf)
                     ->definition;
      rte_memcpy((void *)&buf[act_data->encap.dst],
                 enc_item[act_data->encap.src].spec, act_data->encap.len);
      break;
    case RTE_FLOW_ACTION_TYPE_NVGRE_ENCAP:
      enc_item = ((const struct rte_flow_action_nvgre_encap *)action->conf)
                     ->definition;
      rte_memcpy((void *)&buf[act_data->encap.dst],
                 enc_item[act_data->encap.src].spec, act_data->encap.len);
      break;
    case RTE_FLOW_ACTION_TYPE_RAW_ENCAP:
      ...
      rte_memcpy((void *)&buf[act_data->encap.dst], raw_encap_data->data,
                 act_data->encap.len);
  ...
  if (hw_acts->encap_decap) {
    rule_acts[hw_acts->encap_decap_pos].reformat.offset = job->flow->idx - 1;
 ...
 */
static struct rte_flow *flow_hw_async_flow_create(
  ...
  flow_hw_actions_construct(dev, job, hw_acts, pattern_template_index, actions,
                            rule_acts, &acts_num);
  ret = mlx5dr_rule_create(table->matcher, pattern_template_index, items,
                           rule_acts, acts_num, &rule_attr, &flow->rule);
  if (likely(!ret))
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static int flow_hw_async_flow_destroy(struct rte_eth_dev *dev, uint32_t queue,
 ...
 */
static int flow_hw_pull(struct rte_eth_dev *dev, uint32_t queue,
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static int flow_hw_push(struct rte_eth_dev *dev, uint32_t queue,
                        struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  int ret;
 ...
 *    0 on success, negative value otherwise and rte_errno is set.
 */
static int __flow_hw_pull_comp(struct rte_eth_dev *dev, uint32_t queue,
  ...
  while (pending_rules) {
    ...
    if ((uint32_t)ret > pending_rules) {
      ...
      return rte_flow_error_set(error, ERANGE, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "get extra CQE");
    }
    pending_rules -= ret;
    empty_loop = 0;
...
int flow_hw_q_flow_flush(struct rte_eth_dev *dev,
  ...
  for (queue = 0; queue < priv->nb_queue; queue++) {
    ...
    if (__flow_hw_pull_comp(dev, queue, hw_q->size - hw_q->job_idx, error))
      return -1;
  }
  ...
  LIST_FOREACH(tbl, &priv->flow_hw_tbl, next) {
    MLX5_IPOOL_FOREACH(tbl->flow, fidx, flow) {
      if (flow_hw_async_flow_destroy(dev, MLX5_DEFAULT_FLUSH_QUEUE, &attr,
                                     (struct rte_flow *)flow, NULL, error))
        return -1;
      pending_rules++;
      /* Drain completion with queue size. */
      if (pending_rules >= hw_q->size) {
        if (__flow_hw_pull_comp(dev, MLX5_DEFAULT_FLUSH_QUEUE, pending_rules,
                                error))
          return -1;
        pending_rules = 0;
      }
  ...
  if (pending_rules &&
      __flow_hw_pull_comp(dev, MLX5_DEFAULT_FLUSH_QUEUE, pending_rules, error))
    return -1;
  return 0;
}
 ...
 *    Table on success, NULL otherwise and rte_errno is set.
 */
static struct rte_flow_template_table *flow_hw_table_create(
  ...
  for (i = 0; i < nb_item_templates; i++) {
    ...
    ret = __atomic_add_fetch(&item_templates[i]->refcnt, 1, __ATOMIC_RELAXED);
    if (ret <= 1) {
      rte_errno = EINVAL;
      goto it_error;
    }
  ...
  tbl->matcher = mlx5dr_matcher_create(tbl->grp->tbl, mt, nb_item_templates,
                                       &matcher_attr);
  if (!tbl->matcher)
    goto it_error;
  ...
  for (i = 0; i < nb_action_templates; i++) {
    ...
    ret = __atomic_add_fetch(&action_templates[i]->refcnt, 1, __ATOMIC_RELAXED);
    if (ret <= 1) {
      rte_errno = EINVAL;
      goto at_error;
    }
    LIST_INIT(&tbl->ats[i].acts.act_list);
    err = flow_hw_actions_translate(dev, attr, &tbl->ats[i].acts,
  ...
  tbl->type = attr->flow_attr.transfer
...
at_error:
  while (i--) {
    ...
    __atomic_sub_fetch(&action_templates[i]->refcnt, 1, __ATOMIC_RELAXED);
  }
  i = nb_item_templates;
it_error:
  while (i--)
    __atomic_sub_fetch(&item_templates[i]->refcnt, 1, __ATOMIC_RELAXED);
  mlx5dr_matcher_destroy(tbl->matcher);
error:
  err = rte_errno;
  if (tbl) {
    if (tbl->grp)
      mlx5_hlist_unregister(priv->sh->groups, &tbl->grp->entry);
    if (tbl->flow)
 ...
 */
static int flow_hw_table_destroy(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < table->nb_item_templates; i++)
    __atomic_sub_fetch(&table->its[i]->refcnt, 1, __ATOMIC_RELAXED);
  for (i = 0; i < table->nb_action_templates; i++) {
    ...
    __atomic_sub_fetch(&table->ats[i].action_template->refcnt, 1,
                       __ATOMIC_RELAXED);
  }
 ...
 */
static struct rte_flow_actions_template *flow_hw_actions_template_create(
  ...
  act_len = rte_flow_conv(RTE_FLOW_CONV_OP_ACTIONS, NULL, 0, actions, error);
  if (act_len <= 0)
    return NULL;
  len = RTE_ALIGN(act_len, 16);
  mask_len = rte_flow_conv(RTE_FLOW_CONV_OP_ACTIONS, NULL, 0, masks, error);
  if (mask_len <= 0)
    return NULL;
  ...
  act_len =
      rte_flow_conv(RTE_FLOW_CONV_OP_ACTIONS, at->actions, len, actions, error);
  if (act_len <= 0)
    goto error;
  at->masks = (struct rte_flow_action *)(((uint8_t *)at->actions) + act_len);
  mask_len = rte_flow_conv(RTE_FLOW_CONV_OP_ACTIONS, at->masks, len - act_len,
                           masks, error);
  if (mask_len <= 0)
    goto error;
  /*
...
flow_hw_actions_template_destroy(struct rte_eth_dev *dev __rte_unused,
  ...
  if (__atomic_load_n(&template->refcnt, __ATOMIC_RELAXED) > 1) {
    DRV_LOG(WARNING, "Action template %p is still in use.", (void *)template);
    return rte_flow_error_set(error, EBUSY, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "action template in using");
  }
 ...
 */
static struct rte_flow_pattern_template *flow_hw_pattern_template_create(
...
flow_hw_pattern_template_destroy(struct rte_eth_dev *dev __rte_unused,
                                 ...
                                 struct rte_flow_error *error __rte_unused) {
  if (__atomic_load_n(&template->refcnt, __ATOMIC_RELAXED) > 1) {
    DRV_LOG(WARNING, "Item template %p is still in use.", (void *)template);
    return rte_flow_error_set(error, EBUSY, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "item template in using");
  }
  LIST_REMOVE(template, next);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_hw_info_get(struct rte_eth_dev *dev __rte_unused,
...
struct mlx5_list_entry *flow_hw_grp_create_cb(void *tool_ctx, void *cb_ctx) {
  ...
  if (attr->group) {
    ...
    jump = mlx5dr_action_create_dest_table(
        priv->dr_ctx, tbl, mlx5_hw_act_flag[!!attr->group][dr_tbl_attr.type]);
    if (!jump)
      goto error;
    ...
    jump = mlx5dr_action_create_dest_table(
        priv->dr_ctx, tbl,
        mlx5_hw_act_flag[MLX5_HW_ACTION_FLAG_ROOT][dr_tbl_attr.type]);
    if (!jump)
      goto error;
 ...
 */
int flow_hw_grp_match_cb(void *tool_ctx __rte_unused,
  ...
  return (grp_data->group_id != attr->group) ||
         ((grp_data->type != MLX5DR_TABLE_TYPE_FDB) && attr->transfer) ||
         ((grp_data->type != MLX5DR_TABLE_TYPE_NIC_TX) && attr->egress) ||
         ((grp_data->type != MLX5DR_TABLE_TYPE_NIC_RX) && attr->ingress);
}
...
static int flow_hw_configure(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < nb_queue; i++) {
    ...
    mem_size +=
        (sizeof(struct mlx5_hw_q_job *) + sizeof(uint8_t) * MLX5_ENCAP_MAX_LEN +
         sizeof(struct mlx5_hw_q_job)) *
        queue_attr[0]->size;
  }
  priv->hw_q = mlx5_malloc(MLX5_MEM_ZERO, mem_size, 64, SOCKET_ID_ANY);
  if (!priv->hw_q) {
    rte_errno = ENOMEM;
    goto err;
  }
  for (i = 0; i < nb_queue; i++) {
    ...
    if (i == 0)
      priv->hw_q[i].job = (struct mlx5_hw_q_job **)&priv->hw_q[nb_queue];
    else
      priv->hw_q[i].job =
          (struct mlx5_hw_q_job **)&job[queue_attr[i - 1]->size];
    job = (struct mlx5_hw_q_job *)&priv->hw_q[i].job[queue_attr[i]->size];
  ...
  for (i = 0; i < MLX5_HW_ACTION_FLAG_MAX; i++) {
    for (j = 0; j < MLX5DR_TABLE_TYPE_MAX; j++) {
      priv->hw_drop[i][j] =
          mlx5dr_action_create_dest_drop(priv->dr_ctx, mlx5_hw_act_flag[i][j]);
      if (!priv->hw_drop[i][j])
        goto err;
    }
    priv->hw_tag[i] =
        mlx5dr_action_create_tag(priv->dr_ctx, mlx5_hw_act_flag[i][0]);
    if (!priv->hw_tag[i])
      goto err;
  }
  return 0;
 ...
 *   Pointer to the rte_eth_dev structure.
 */
void flow_hw_resource_release(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct rte_flow_template_table *tbl;
  struct rte_flow_pattern_template *it;
  struct rte_flow_actions_template *at;
 ...
 *   0 on success, negative value otherwise and rte_errno is set.
 */
static int flow_hw_action_handle_update(struct rte_eth_dev *dev, uint32_t queue,
                                        const struct rte_flow_op_attr *attr,
 ...
 *   0 on success, negative value otherwise and rte_errno is set.
 */
static int flow_hw_action_handle_destroy(struct rte_eth_dev *dev,
                                         uint32_t queue,
                                         const struct rte_flow_op_attr *attr,
                                         struct rte_flow_action_handle *handle,
                                         void *user_data,
