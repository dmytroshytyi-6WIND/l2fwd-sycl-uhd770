...
void mlx5_rxq_info_get(struct rte_eth_dev *dev, uint16_t rx_queue_id,
 ...
 */
int mlx5_rx_burst_mode_get(struct rte_eth_dev *dev,
...
void mlx5_rxq_initialize(struct mlx5_rxq_data *rxq) {
  ...
  for (i = 0; (i != wqe_n); ++i) {
    ...
    if (mlx5_rxq_mprq_enabled(rxq)) {
      ...
      scat = &((volatile struct mlx5_wqe_mprq *)rxq->wqes)[i].dseg;
      ...
      scat = &((volatile struct mlx5_wqe_data_seg *)rxq->wqes)[i];
  ...
  rxq->elts_ci = mlx5_rxq_mprq_enabled(rxq)
                     ? (wqe_n >> rxq->sges_n) * RTE_BIT32(rxq->log_strd_num)
                     : 0;
  ...
  case MLX5_RXQ_ERR_STATE_NEED_RESET:
    ...
    if (rxq_ctrl->dump_file_n < RXQ_PORT(rxq_ctrl)->config.max_dump_files_num) {
      MKSTR(err_str,
            ...
            " rq_ci = %u cq_ci = %u",
            u.err_cqe->syndrome, rxq->cqn, rxq_ctrl->wqn,
            rte_be_to_cpu_16(u.err_cqe->wqe_counter), rxq->rq_ci << rxq->sges_n,
            rxq->cq_ci);
      MKSTR(name, "dpdk_mlx5_port_%u_rxq_%u_%u", rxq->port_id, rxq->idx,
            (uint32_t)rte_rdtsc());
      mlx5_dump_debug_information(name, NULL, err_str, 0);
      mlx5_dump_debug_information(
          name, "MLX5 Error CQ:", (const void *)((uintptr_t)rxq->cqes),
          sizeof(*u.cqe) * cqe_n);
      mlx5_dump_debug_information(
          name, "MLX5 Error RQ:", (const void *)((uintptr_t)rxq->wqes),
          16 * wqe_n);
      ...
      *rxq->rq_db = rte_cpu_to_be_32(0);
      ...
      if (vec) {
        const uint32_t elts_n =
            mlx5_rxq_mprq_enabled(rxq) ? wqe_n * strd_n : wqe_n;
        const uint32_t e_mask = elts_n - 1;
        uint32_t elts_ci =
            mlx5_rxq_mprq_enabled(rxq) ? rxq->elts_ci : rxq->rq_ci;
        uint32_t elt_idx;
        struct rte_mbuf **elt;
        int i;
        ...
        for (i = 0; i < (int)elts_n; ++i) {
          elt = &(*rxq->elts)[i];
          DATA_LEN(*elt) =
              (uint16_t)((*elt)->buf_len - rte_pktmbuf_headroom(*elt));
        }
 ...
 */
static inline int mlx5_rx_poll_len(struct mlx5_rxq_data *rxq,
                                   volatile struct mlx5_cqe *cqe,
                                   uint16_t cqe_cnt,
                                   volatile struct mlx5_mini_cqe8 **mcqe) {
  struct rxq_zip *zip = &rxq->zip;
  ...
  do {
    ...
    if (zip->ai) {
      volatile struct mlx5_mini_cqe8(*mc)[8] =
          (volatile struct mlx5_mini_cqe8(*)[8])(uintptr_t)(
              &(*rxq->cqes)[zip->ca & cqe_cnt].pkt_info);
      ...
      *mcqe = &(*mc)[zip->ai & 7];
      ...
      if (MLX5_CQE_FORMAT(op_own) == MLX5_COMPRESSED) {
        volatile struct mlx5_mini_cqe8(*mc)[8] =
            (volatile struct mlx5_mini_cqe8(*)[8])(uintptr_t)(
                &(*rxq->cqes)[cq_ci & cqe_cnt].pkt_info);
...
static inline uint32_t rxq_cq_to_ol_flags(volatile struct mlx5_cqe *cqe) {
  ...
  ol_flags =
      TRANSPOSE(flags, MLX5_CQE_RX_L3_HDR_VALID, RTE_MBUF_F_RX_IP_CKSUM_GOOD) |
      TRANSPOSE(flags, MLX5_CQE_RX_L4_HDR_VALID, RTE_MBUF_F_RX_L4_CKSUM_GOOD);
  return ol_flags;
}
 ...
 */
static inline void rxq_cq_to_mbuf(struct mlx5_rxq_data *rxq,
                                  struct rte_mbuf *pkt,
                                  volatile struct mlx5_cqe *cqe,
                                  volatile struct mlx5_mini_cqe8 *mcqe) {
  ...
  if (rxq->mark) {
    ...
    if (mcqe == NULL || rxq->mcqe_format != MLX5_CQE_RESP_FORMAT_FTAG_STRIDX)
      ...
      mark = ((mcqe->byte_cnt_flow & 0xff) << 8) | (mcqe->flow_tag_high << 16);
  ...
  if (rxq->dynf_meta) {
    uint32_t meta =
        rte_be_to_cpu_32(cqe->flow_table_metadata) & rxq->flow_meta_port_mask;
  ...
  if (rxq->vlan_strip) {
    ...
    if (mcqe == NULL || rxq->mcqe_format != MLX5_CQE_RESP_FORMAT_L34H_STRIDX)
      ...
      vlan_strip = mcqe->hdr_type & RTE_BE16(MLX5_CQE_VLAN_STRIPPED);
    if (vlan_strip) {
 ...
 */
uint16_t mlx5_rx_burst(void *dpdk_rxq, struct rte_mbuf **pkts,
                       uint16_t pkts_n) {
  ...
  while (pkts_n) {
    ...
    if (!pkt) {
      ...
      if (cqe->lro_num_seg > 1) {
        mlx5_lro_update_hdr(rte_pktmbuf_mtod(pkt, uint8_t *), cqe, mcqe, rxq,
                            len);
...
mlx5_lro_update_hdr(uint8_t *__rte_restrict padd,
  ...
  if (proto == RTE_BE16(RTE_ETHER_TYPE_IPV4)) {
    ...
    h.ipv6->payload_len =
        rte_cpu_to_be_16(len - (h.hdr - padd) - sizeof(*h.ipv6));
    phcsum = rte_ipv6_phdr_cksum(h.ipv6, 0);
    h.ipv6++;
  }
  if (mcqe == NULL || rxq->mcqe_format != MLX5_CQE_RESP_FORMAT_L34H_STRIDX)
    l4_type = (rte_be_to_cpu_16(cqe->hdr_type_etc) & MLX5_CQE_L4_TYPE_MASK) >>
              MLX5_CQE_L4_TYPE_SHIFT;
  else
    l4_type = (rte_be_to_cpu_16(mcqe->hdr_type) & MLX5_CQE_L4_TYPE_MASK) >>
              MLX5_CQE_L4_TYPE_SHIFT;
...
uint16_t mlx5_rx_burst_mprq(void *dpdk_rxq, struct rte_mbuf **pkts,
  ...
  while (i < pkts_n) {
    ...
    if (mcqe && rxq->mcqe_format == MLX5_CQE_RESP_FORMAT_FTAG_STRIDX)
      ...
      strd_cnt =
          (byte_cnt & MLX5_MPRQ_STRIDE_NUM_MASK) >> MLX5_MPRQ_STRIDE_NUM_SHIFT;
    MLX5_ASSERT(strd_cnt);
    consumed_strd += strd_cnt;
    if (byte_cnt & MLX5_MPRQ_FILLER_MASK)
      continue;
    strd_idx =
        rte_be_to_cpu_16(mcqe == NULL ? cqe->wqe_counter : mcqe->stride_idx);
...
__rte_weak uint16_t mlx5_rx_burst_vec(void *dpdk_rxq __rte_unused,
                                      struct rte_mbuf **pkts __rte_unused,
                                      uint16_t pkts_n __rte_unused) {
  return 0;
}

__rte_weak uint16_t mlx5_rx_burst_mprq_vec(void *dpdk_rxq __rte_unused,
                                           struct rte_mbuf **pkts __rte_unused,
                                           uint16_t pkts_n __rte_unused) {
  return 0;
}
