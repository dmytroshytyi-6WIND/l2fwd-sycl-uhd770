...
#define MLX5_TXOFF_CONFIG_FULL                                                 \
  (MLX5_TXOFF_CONFIG_MULTI | MLX5_TXOFF_CONFIG_TSO | MLX5_TXOFF_CONFIG_SWP |   \
   MLX5_TXOFF_CONFIG_CSUM | MLX5_TXOFF_CONFIG_INLINE |                         \
...
#define MLX5_TXOFF_PRE_DECL(func)                                              \
  uint16_t mlx5_tx_burst_##func(void *txq, struct rte_mbuf **pkts,             \
                                uint16_t pkts_n)

#define MLX5_TXOFF_DECL(func, olx)                                             \
  uint16_t mlx5_tx_burst_##func(void *txq, struct rte_mbuf **pkts,             \
                                uint16_t pkts_n) {                             \
    return mlx5_tx_burst_tmpl((struct mlx5_txq_data *)txq, pkts, pkts_n,       \
                              (olx));                                          \
  }
...
__extension__ struct mlx5_txq_local {
...
/* TX queue descriptor. */
__extension__ struct mlx5_txq_data {
 ...
 */
static __rte_always_inline void mlx5_tx_dbrec(struct mlx5_txq_data *txq,
                                              volatile struct mlx5_wqe *wqe) {
  mlx5_doorbell_ring(mlx5_tx_bfreg(txq), *(volatile uint64_t *)wqe, txq->wqe_ci,
                     txq->qp_db, 1);
}
...
mlx5_txpp_convert_tx_ts(struct mlx5_dev_ctx_shared *sh, uint64_t mts) {
  ...
  if (unlikely(mts >= UINT64_MAX / 2)) {
    ...
    __atomic_fetch_add(&sh->txpp.err_ts_past, 1, __ATOMIC_RELAXED);
    return -1;
  }
  tick = sh->txpp.tick;
  MLX5_ASSERT(tick);
  ...
  if (unlikely(mts >= (1 << MLX5_CQ_INDEX_WIDTH) / 2 - 1)) {
    ...
    __atomic_fetch_add(&sh->txpp.err_ts_future, 1, __ATOMIC_RELAXED);
    return -1;
  }
...
txq_mbuf_to_swp(struct mlx5_txq_local *__rte_restrict loc, uint8_t *swp_flags,
                unsigned int olx) {
  uint64_t ol, tunnel;
   ...
   */
  if (likely(tunnel != RTE_MBUF_F_TX_TUNNEL_UDP &&
             tunnel != RTE_MBUF_F_TX_TUNNEL_IP))
    return 0;
  /*
   ...
   */
  idx = (ol & (RTE_MBUF_F_TX_L4_MASK | RTE_MBUF_F_TX_IPV6 |
  ...
  *swp_flags = mlx5_swp_types_table[idx];
  ...
  if (ol & (RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IPV6)) { /* Inner IP. */
    ...
    if (csum == RTE_MBUF_F_TX_TCP_CKSUM || csum == RTE_MBUF_F_TX_UDP_CKSUM ||
 ...
 */
static __rte_always_inline uint8_t txq_ol_cksum_to_cs(struct rte_mbuf *buf) {
  uint32_t idx;
...
mlx5_tx_free_mbuf(struct mlx5_txq_data *__rte_restrict txq,
                  struct rte_mbuf **__rte_restrict pkts, unsigned int pkts_n,
                  unsigned int olx __rte_unused) {
 ...
 */
static __rte_always_inline void
mlx5_tx_free_elts(struct mlx5_txq_data *__rte_restrict txq, uint16_t tail,
                  unsigned int olx __rte_unused) {
  ...
  do {
    ...
    mlx5_tx_free_mbuf(txq, &txq->elts[txq->elts_tail & txq->elts_m], part, olx);
    txq->elts_tail += part;
    n_elts -= part;
  } while (n_elts);
}
...
mlx5_tx_copy_elts(struct mlx5_txq_data *__rte_restrict txq,
  ...
  rte_memcpy((void *)(elts + (txq->elts_head & txq->elts_m)), (void *)pkts,
...
mlx5_tx_request_completion(struct mlx5_txq_data *__rte_restrict txq,
  ...
  if ((uint16_t)(head - txq->elts_comp) >= MLX5_TX_COMP_THRESH ||
    ...
    last->cseg.flags = RTE_BE32(MLX5_COMP_ALWAYS << MLX5_COMP_MODE_OFFSET);
    /* Save elts_head in dedicated free on completion queue. */
#ifdef RTE_LIBRTE_MLX5_DEBUG
    txq->fcqs[txq->cq_pi++ & txq->cqe_m] = head | (last->cseg.opcode >> 8)
                                                      << 16;
#else
...
mlx5_tx_cseg_init(struct mlx5_txq_data *__rte_restrict txq,
                  ...
                  struct mlx5_wqe *__rte_restrict wqe, unsigned int ds,
                  unsigned int opcode, unsigned int olx __rte_unused) {
...
mlx5_tx_qseg_init(struct mlx5_txq_data *restrict txq,
                  ...
                  struct mlx5_wqe *restrict wqe, unsigned int wci,
                  unsigned int olx __rte_unused) {
...
mlx5_tx_wseg_init(struct mlx5_txq_data *restrict txq,
                  struct mlx5_txq_local *restrict loc __rte_unused,
                  struct mlx5_wqe *restrict wqe, uint64_t ts,
                  unsigned int olx __rte_unused) {
  ...
  if (txq->rt_timestamp) {
    ts = ts % (uint64_t)NS_PER_S | (ts / (uint64_t)NS_PER_S) << 32;
  }
...
mlx5_tx_eseg_none(struct mlx5_txq_data *__rte_restrict txq __rte_unused,
                  struct mlx5_txq_local *__rte_restrict loc,
                  struct mlx5_wqe *__rte_restrict wqe, unsigned int olx) {
  ...
  if (MLX5_TXOFF_CONFIG(VLAN) && loc->mbuf->ol_flags & RTE_MBUF_F_TX_VLAN) {
    ...
    es->inline_hdr =
        rte_cpu_to_be_32(MLX5_ETH_WQE_VLAN_INSERT | loc->mbuf->vlan_tci);
  } else {
...
mlx5_tx_eseg_dmin(struct mlx5_txq_data *__rte_restrict txq __rte_unused,
                  struct mlx5_txq_local *__rte_restrict loc,
                  struct mlx5_wqe *__rte_restrict wqe, unsigned int vlan,
                  unsigned int olx) {
  struct mlx5_wqe_eseg *__rte_restrict es = &wqe->eseg;
  uint32_t csum;
  uint8_t *psrc, *pdst;
  ...
  if (MLX5_TXOFF_CONFIG(VLAN) && vlan) {
    ...
    *(unaligned_uint32_t *)pdst =
        rte_cpu_to_be_32((RTE_ETHER_TYPE_VLAN << 16) | loc->mbuf->vlan_tci);
...
mlx5_tx_eseg_data(struct mlx5_txq_data *__rte_restrict txq,
                  struct mlx5_txq_local *__rte_restrict loc,
                  struct mlx5_wqe *__rte_restrict wqe, unsigned int vlan,
                  unsigned int inlen, unsigned int tso, unsigned int olx) {
  struct mlx5_wqe_eseg *__rte_restrict es = &wqe->eseg;
  uint32_t csum;
  uint8_t *psrc, *pdst;
  unsigned int part;
...
mlx5_tx_mseg_memcpy(uint8_t *pdst, struct mlx5_txq_local *__rte_restrict loc,
                    unsigned int len, unsigned int must,
                    unsigned int olx __rte_unused) {
  struct rte_mbuf *mbuf;
  ...
  do {
    ...
    dlen -= loc->mbuf_off;
    psrc = rte_pktmbuf_mtod_offset(loc->mbuf, uint8_t *, loc->mbuf_off);
...
mlx5_tx_eseg_mdat(struct mlx5_txq_data *__rte_restrict txq,
  ...
  if (MLX5_TXOFF_CONFIG(VLAN) && vlan) {
    /* Implement VLAN tag insertion as part inline data. */
    mlx5_tx_mseg_memcpy(pdst, loc, 2 * RTE_ETHER_ADDR_LEN,
...
mlx5_tx_dseg_ptr(struct mlx5_txq_data *__rte_restrict txq,
                 struct mlx5_txq_local *__rte_restrict loc,
                 struct mlx5_wqe_dseg *__rte_restrict dseg, uint8_t *buf,
                 unsigned int len, unsigned int olx __rte_unused)
...
mlx5_tx_mseg_build(struct mlx5_txq_data *__rte_restrict txq,
                   struct mlx5_txq_local *__rte_restrict loc,
                   struct mlx5_wqe *__rte_restrict wqe, unsigned int vlan,
                   unsigned int inlen, unsigned int tso,
                   unsigned int olx __rte_unused) {
  ...
  if (loc->mbuf_off) {
    ...
    dptr = rte_pktmbuf_mtod_offset(loc->mbuf, uint8_t *, loc->mbuf_off);
  ...
  do {
    if (unlikely(!rte_pktmbuf_data_len(loc->mbuf))) {
      ...
      if ((uintptr_t)dseg >= (uintptr_t)txq->wqes_end)
        dseg = (struct mlx5_wqe_dseg *)txq->wqes;
      mlx5_tx_dseg_iptr(txq, loc, dseg, rte_pktmbuf_mtod(loc->mbuf, uint8_t *),
...
dseg_done:
  ...
  if ((uintptr_t)wqe < (uintptr_t)dseg)
    ...
    ds = (((uintptr_t)dseg - (uintptr_t)wqe) + txq->wqe_s * MLX5_WQE_SIZE) /
         MLX5_WSEG_SIZE;
  return ds;
}
...
mlx5_tx_schedule_send(struct mlx5_txq_data *restrict txq,
                      struct mlx5_txq_local *restrict loc, unsigned int olx) {
  if (MLX5_TXOFF_CONFIG(TXPP) && loc->mbuf->ol_flags & txq->ts_mask) {
    ...
    if (txq->wait_on_time) {
      ...
      mlx5_tx_cseg_init(txq, loc, wqe,
                        1 + sizeof(struct mlx5_wqe_wseg) / MLX5_WSEG_SIZE,
                        MLX5_OPCODE_WAIT | MLX5_OPC_MOD_WAIT_TIME << 24, olx);
      ...
      mlx5_tx_cseg_init(txq, loc, wqe,
                        1 + sizeof(struct mlx5_wqe_qseg) / MLX5_WSEG_SIZE,
                        MLX5_OPCODE_WAIT | MLX5_OPC_MOD_WAIT_CQ_PI << 24, olx);
...
mlx5_tx_packet_multi_tso(struct mlx5_txq_data *__rte_restrict txq,
  ...
  inlen = loc->mbuf->l2_len + vlan + loc->mbuf->l3_len + loc->mbuf->l4_len;
  ...
  if (unlikely(inlen > MLX5_MAX_TSO_HEADER ||
               inlen <= MLX5_ESEG_MIN_INLINE_SIZE || inlen > (dlen + vlan)))
    return MLX5_TXCMP_CODE_ERROR;
  /*
  ...
  ds = NB_SEGS(loc->mbuf) + 2 +
       (inlen - MLX5_ESEG_MIN_INLINE_SIZE + MLX5_WSEG_SIZE + MLX5_WSEG_SIZE -
        1) /
           MLX5_WSEG_SIZE;
  ...
  ntcp =
      (dlen - (inlen - vlan) + loc->mbuf->tso_segsz - 1) / loc->mbuf->tso_segsz;
  /*
...
mlx5_tx_packet_multi_send(struct mlx5_txq_data *__rte_restrict txq,
  ...
  do {
    if (unlikely(!rte_pktmbuf_data_len(loc->mbuf))) {
      ...
      mlx5_tx_dseg_ptr(txq, loc, dseg, rte_pktmbuf_mtod(loc->mbuf, uint8_t *),
                       rte_pktmbuf_data_len(loc->mbuf), olx);
      txq->elts[txq->elts_head++ & txq->elts_m] = loc->mbuf;
      --loc->elts_free;
      if (--nseg == 0)
...
mlx5_tx_packet_multi_inline(struct mlx5_txq_data *__rte_restrict txq,
  ...
  if (inlen > txq->inlen_send ||
     ...
     */
    if (txq->inlen_mode) {
      MLX5_ASSERT(txq->inlen_mode >= MLX5_ESEG_MIN_INLINE_SIZE);
    ...
    if (unlikely(nxlen <= txq->inlen_send)) {
      ...
      if (nxlen < inlen) {
        ...
        if (unlikely(nxlen > txq->inlen_send)) {
          ...
          smlen = inlen - smlen;
          start = rte_pktmbuf_mtod_offset(mbuf, uintptr_t, smlen);
          goto do_align;
        }
      }
    do_first:
...
mlx5_tx_burst_tso(struct mlx5_txq_data *__rte_restrict txq,
                  struct rte_mbuf **__rte_restrict pkts, unsigned int pkts_n,
                  struct mlx5_txq_local *__rte_restrict loc, unsigned int olx) {
  MLX5_ASSERT(loc->elts_free && loc->wqe_free);
  MLX5_ASSERT(pkts_n > loc->pkts_sent);
  pkts += loc->pkts_sent + 1;
  pkts_n -= loc->pkts_sent;
  for (;;) {
     ...
     */
    hlen = loc->mbuf->l2_len + vlan + loc->mbuf->l3_len + loc->mbuf->l4_len;
    if (unlikely((!hlen || !loc->mbuf->tso_segsz)))
      return MLX5_TXCMP_CODE_ERROR;
    if (loc->mbuf->ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK)
      hlen += loc->mbuf->outer_l2_len + loc->mbuf->outer_l3_len;
    /* Segment must contain all TSO headers. */
    if (unlikely(hlen > MLX5_MAX_TSO_HEADER ||
                 hlen <= MLX5_ESEG_MIN_INLINE_SIZE || hlen > (dlen + vlan)))
      return MLX5_TXCMP_CODE_ERROR;
    /*
     * Check whether there are enough free WQEBBs:
     * - Control Segment
     ...
     */
    ds = 4 + (hlen - MLX5_ESEG_MIN_INLINE_SIZE + MLX5_WSEG_SIZE - 1) /
                 MLX5_WSEG_SIZE;
    ...
    /* Update sent data bytes/packets counters. */
    ntcp =
        (dlen + vlan - hlen + loc->mbuf->tso_segsz - 1) / loc->mbuf->tso_segsz;
    /*
    ...
    if (pkts_n > 1)
      rte_prefetch0(*pkts);
    if (MLX5_TXOFF_CONFIG(MULTI) && unlikely(NB_SEGS(loc->mbuf) > 1))
      return MLX5_TXCMP_CODE_MULTI;
...
mlx5_tx_able_to_empw(struct mlx5_txq_data *__rte_restrict txq,
                     struct mlx5_txq_local *__rte_restrict loc,
                     unsigned int olx, bool newp) {
  /* Check for multi-segment packet. */
  if (newp && MLX5_TXOFF_CONFIG(MULTI) && unlikely(NB_SEGS(loc->mbuf) > 1))
    return MLX5_TXCMP_CODE_MULTI;
  /* Check for TSO packet. */
  if (newp && MLX5_TXOFF_CONFIG(TSO) &&
      unlikely(loc->mbuf->ol_flags & RTE_MBUF_F_TX_TCP_SEG))
    return MLX5_TXCMP_CODE_TSO;
...
mlx5_tx_match_empw(struct mlx5_txq_data *__rte_restrict txq,
                   ...
                   struct mlx5_txq_local *__rte_restrict loc, uint32_t dlen,
                   unsigned int olx) {
  uint8_t swp_flags = 0;

  /* Compare the checksum flags, if any. */
  if (MLX5_TXOFF_CONFIG(CSUM) && txq_ol_cksum_to_cs(loc->mbuf) != es->cs_flags)
    return false;
  ...
  if (MLX5_TXOFF_CONFIG(MPW) && dlen != rte_pktmbuf_data_len(loc->mbuf))
    return false;
...
mlx5_tx_sdone_empw(struct mlx5_txq_data *__rte_restrict txq,
                   struct mlx5_txq_local *__rte_restrict loc, unsigned int ds,
                   unsigned int slen, unsigned int olx __rte_unused) {
  MLX5_ASSERT(!MLX5_TXOFF_CONFIG(INLINE));
#ifdef MLX5_PMD_SOFT_COUNTERS
  /* Update sent data bytes counter. */
...
mlx5_tx_idone_empw(struct mlx5_txq_data *__rte_restrict txq,
                   struct mlx5_txq_local *__rte_restrict loc, unsigned int len,
                   unsigned int slen, struct mlx5_wqe *__rte_restrict wqem,
                   unsigned int olx __rte_unused) {
 ...
 */
static __rte_always_inline enum mlx5_txcmp_code mlx5_tx_burst_empw_simple(
    struct mlx5_txq_data *__rte_restrict txq,
    struct rte_mbuf **__rte_restrict pkts, unsigned int pkts_n,
    struct mlx5_txq_local *__rte_restrict loc, unsigned int olx) {
  /*
  ...
  next_empw:
    ...
    if (unlikely(loc->wqe_free < ((2 + part + 3) / 4))) {
      if (unlikely(loc->wqe_free < ((2 + MLX5_EMPW_MIN_PACKETS + 3) / 4)))
        return MLX5_TXCMP_CODE_EXIT;
    ...
    mlx5_tx_eseg_none(txq, loc, loc->wqe_last, olx & ~MLX5_TXOFF_CONFIG_VLAN);
    eseg = &loc->wqe_last->eseg;
    dseg = &loc->wqe_last->dseg[0];
    loop = part;
    /* Store the packet length for legacy MPW. */
    if (MLX5_TXOFF_CONFIG(MPW))
      eseg->mss = rte_cpu_to_be_16(rte_pktmbuf_data_len(loc->mbuf));
    for (;;) {
 ...
 */
static __rte_always_inline enum mlx5_txcmp_code mlx5_tx_burst_empw_inline(
    struct mlx5_txq_data *__rte_restrict txq,
    struct rte_mbuf **__rte_restrict pkts, unsigned int pkts_n,
    struct mlx5_txq_local *__rte_restrict loc, unsigned int olx) {
  /*
  ...
  for (;;) {
     ...
     */
    nlim = RTE_MIN(pkts_n, MLX5_TXOFF_CONFIG(MPW) ? MLX5_MPW_INLINE_MAX_PACKETS
                                                  : MLX5_EMPW_MAX_PACKETS);
     ...
     * - Ethernet Segment, no inline
     */
    mlx5_tx_cseg_init(txq, loc, wqem, 0, MLX5_OPCODE_ENHANCED_MPSW, olx);
    mlx5_tx_eseg_none(txq, loc, wqem, olx & ~MLX5_TXOFF_CONFIG_VLAN);
    dseg = &wqem->dseg[0];
    /* Store the packet length for legacy MPW. */
    if (MLX5_TXOFF_CONFIG(MPW))
      wqem->eseg.mss = rte_cpu_to_be_16(rte_pktmbuf_data_len(loc->mbuf));
    room = RTE_MIN(MLX5_WQE_SIZE_MAX / MLX5_WQE_SIZE, loc->wqe_free) *
               MLX5_WQE_SIZE -
           MLX5_WQE_CSEG_SIZE - MLX5_WQE_ESEG_SIZE;
    /* Limit the room for legacy MPW sessions for performance. */
    if (MLX5_TXOFF_CONFIG(MPW))
      room = RTE_MIN(
          room,
          RTE_MAX(
              txq->inlen_empw + sizeof(dseg->bcount) +
                  (MLX5_TXOFF_CONFIG(VLAN) ? sizeof(struct rte_vlan_hdr) : 0),
              MLX5_MPW_INLINE_MAX_PACKETS * MLX5_WQE_DSEG_SIZE));
    ...
    for (;;) {
      ...
      if (MLX5_TXOFF_CONFIG(MPW)) {
        ...
        if (part == room) {
          ...
          dseg = RTE_PTR_ADD(dseg, sizeof(dseg->bcount));
        } else {
          /*
      ...
      if (MLX5_TXOFF_CONFIG(VLAN) && loc->mbuf->ol_flags & RTE_MBUF_F_TX_VLAN) {
         ...
         */
        MLX5_ASSERT((dlen + sizeof(struct rte_vlan_hdr)) <= txq->inlen_empw);
        tlen += sizeof(struct rte_vlan_hdr);
        if (room < tlen)
          break;
        dseg = mlx5_tx_dseg_vlan(txq, loc, dseg, dptr, dlen, olx);
#ifdef MLX5_PMD_SOFT_COUNTERS
        /* Update sent data bytes counter. */
        ...
        if (room < tlen)
          break;
        dseg = mlx5_tx_dseg_empw(txq, loc, dseg, dptr, dlen, olx);
      }
      if (!MLX5_TXOFF_CONFIG(MPW))
    ...
    pointer_empw:
       ...
       * intermix for legacy MPW sessions.
       */
      if (MLX5_TXOFF_CONFIG(MPW) && part != room &&
    ...
    next_mbuf:
       ...
       * - scheduling is not required
       */
      if (!mlx5_tx_match_empw(txq, &wqem->eseg, loc, dlen, olx))
        break;
      /* Packet attributes match, continue the same eMPW. */
      if ((uintptr_t)dseg >= (uintptr_t)txq->wqes_end)
        dseg = (struct mlx5_wqe_dseg *)txq->wqes;
 ...
 */
static __rte_always_inline enum mlx5_txcmp_code mlx5_tx_burst_single_send(
    struct mlx5_txq_data *__rte_restrict txq,
    struct rte_mbuf **__rte_restrict pkts, unsigned int pkts_n,
    struct mlx5_txq_local *__rte_restrict loc, unsigned int olx) {
  /*
  ...
  for (;;) {
    ...
    if (MLX5_TXOFF_CONFIG(INLINE)) {
      ...
      if (inlen <= txq->inlen_send) {
        ...
        rte_prefetch0(rte_pktmbuf_mtod(loc->mbuf, uint8_t *));
        /* Check against minimal length. */
        if (inlen <= MLX5_ESEG_MIN_INLINE_SIZE)
          return MLX5_TXCMP_CODE_ERROR;
        if (loc->mbuf->ol_flags & RTE_MBUF_F_TX_DYNF_NOINLINE) {
          ...
          if ((!MLX5_TXOFF_CONFIG(EMPW) && txq->inlen_mode) ||
              (MLX5_TXOFF_CONFIG(MPW) && txq->inlen_mode)) {
            if (inlen <= txq->inlen_send)
              goto single_inline;
            /*
          ...
          if (MLX5_TXOFF_CONFIG(VLAN) && vlan && !txq->vlan_en) {
            /*
      ...
      single_inline:
         ...
         */
        seg_n = (inlen + 3 * MLX5_WSEG_SIZE - MLX5_ESEG_MIN_INLINE_SIZE +
                 MLX5_WSEG_SIZE - 1) /
                MLX5_WSEG_SIZE;
        ...
        wqe = txq->wqes + (txq->wqe_ci & txq->wqe_m);
        loc->wqe_last = wqe;
        mlx5_tx_cseg_init(txq, loc, wqe, seg_n, MLX5_OPCODE_SEND, olx);
      ...
      } else if ((!MLX5_TXOFF_CONFIG(EMPW) || MLX5_TXOFF_CONFIG(MPW)) &&
                 txq->inlen_mode) {
        /*
      ...
      single_min_inline:
         ...
         * - Finishing Data Segment of pointer type
         */
        ds = (MLX5_WQE_CSEG_SIZE + MLX5_WQE_ESEG_SIZE + MLX5_WQE_DSEG_SIZE +
              txq->inlen_mode - MLX5_ESEG_MIN_INLINE_SIZE + MLX5_WQE_DSEG_SIZE +
              MLX5_WSEG_SIZE - 1) /
             MLX5_WSEG_SIZE;
        if (loc->wqe_free < ((ds + 3) / 4))
          return MLX5_TXCMP_CODE_EXIT;
        ...
        loc->wqe_last = wqe;
        mlx5_tx_cseg_init(txq, loc, wqe, ds, MLX5_OPCODE_SEND, olx);
        dseg = mlx5_tx_eseg_data(txq, loc, wqe, vlan, txq->inlen_mode, 0, olx);
        dptr = rte_pktmbuf_mtod(loc->mbuf, uint8_t *) + txq->inlen_mode - vlan;
        inlen -= txq->inlen_mode;
        mlx5_tx_dseg_ptr(txq, loc, dseg, dptr, inlen, olx);
        /*
         * WQE is built, update the loop parameters
      ...
      single_part_inline:
        wqe = txq->wqes + (txq->wqe_ci & txq->wqe_m);
        loc->wqe_last = wqe;
        mlx5_tx_cseg_init(txq, loc, wqe, 4, MLX5_OPCODE_SEND, olx);
        ...
        mlx5_tx_dseg_ptr(txq, loc, &wqe->dseg[1], dptr, dlen, olx);
        ++txq->wqe_ci;
        --loc->wqe_free;
        /* We have to store mbuf in elts.*/
        MLX5_ASSERT(MLX5_TXOFF_CONFIG(INLINE));
...
#ifdef MLX5_PMD_SOFT_COUNTERS
      /* Update sent data bytes counter. */
      txq->stats.obytes += vlan + rte_pktmbuf_data_len(loc->mbuf);
#endif
    } else {
      /*
    ...
    single_no_inline:
      wqe = txq->wqes + (txq->wqe_ci & txq->wqe_m);
      loc->wqe_last = wqe;
      mlx5_tx_cseg_init(txq, loc, wqe, 3, MLX5_OPCODE_SEND, olx);
      mlx5_tx_eseg_none(txq, loc, wqe, olx);
      mlx5_tx_dseg_ptr(txq, loc, &wqe->dseg[0],
...
mlx5_tx_burst_single(struct mlx5_txq_data *__rte_restrict txq,
  ...
  for (;;) {
    ...
    ret = (MLX5_TXOFF_CONFIG(INLINE))
              ? mlx5_tx_burst_empw_inline(txq, pkts, pkts_n, loc, olx)
              : mlx5_tx_burst_empw_simple(txq, pkts, pkts_n, loc, olx);
    if (ret != MLX5_TXCMP_CODE_SINGLE)
      return ret;
 ...
 */
static __rte_always_inline uint16_t mlx5_tx_burst_tmpl(
    struct mlx5_txq_data *__rte_restrict txq,
    struct rte_mbuf **__rte_restrict pkts, uint16_t pkts_n, unsigned int olx) {
...
send_loop:
  ...
  MLX5_ASSERT(txq->wqe_s >= (uint16_t)(txq->wqe_ci - txq->wqe_pi));
  loc.wqe_free = txq->wqe_s - (uint16_t)(txq->wqe_ci - txq->wqe_pi);
  if (unlikely(!loc.elts_free || !loc.wqe_free))
    goto burst_exit;
  for (;;) {
    /*
    ...
    enter_send_multi:
      ...
      if (MLX5_TXOFF_CONFIG(TSO) && ret == MLX5_TXCMP_CODE_TSO) {
        /*
    ...
    enter_send_tso:
      ...
      if (ret == MLX5_TXCMP_CODE_SINGLE)
        goto enter_send_single;
      if (MLX5_TXOFF_CONFIG(MULTI) && ret == MLX5_TXCMP_CODE_MULTI) {
        /*
   ...
   */
  MLX5_ASSERT(MLX5_TXOFF_CONFIG(INLINE) || loc.pkts_sent >= loc.pkts_copy);
   ...
   */
  mlx5_doorbell_ring(mlx5_tx_bfreg(txq), *(volatile uint64_t *)loc.wqe_last,
                     txq->wqe_ci, txq->qp_db,
                     !txq->db_nc &&
