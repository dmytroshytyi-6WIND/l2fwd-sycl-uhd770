 ...
 */
static void mlx5_txq_stop(struct rte_eth_dev *dev) {
 ...
 */
static int mlx5_txq_start(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  unsigned int i;
  int ret;
  ...
  for (i = 0; i != priv->txqs_n; ++i) {
    ...
    txq_ctrl->obj =
        mlx5_malloc(flags, sizeof(struct mlx5_txq_obj), 0, txq_ctrl->socket);
    if (!txq_ctrl->obj) {
      DRV_LOG(ERR,
              "Port %u Tx queue %u cannot allocate "
              "memory resources.",
    ...
    if (!txq_ctrl->is_hairpin) {
      ...
      txq_data->fcqs =
          mlx5_malloc(flags, size, RTE_CACHE_LINE_SIZE, txq_ctrl->socket);
      if (!txq_data->fcqs) {
        DRV_LOG(ERR,
                "Port %u Tx queue %u cannot "
                "allocate memory (FCQ).",
                dev->data->port_id, i);
        rte_errno = ENOMEM;
        goto error;
    ...
    DRV_LOG(DEBUG, "Port %u txq %u updated with %p.", dev->data->port_id, i,
            (void *)&txq_ctrl->obj);
 ...
 */
static int mlx5_rxq_mempool_register(struct mlx5_rxq_ctrl *rxq_ctrl) {
  ...
  for (s = 0; s < rxq_ctrl->rxq.rxseg_n; s++) {
    ...
    is_extmem =
        (rte_pktmbuf_priv_flags(mp) & RTE_PKTMBUF_POOL_F_PINNED_EXT_BUF) != 0;
 ...
 *   Pointer to Ethernet device structure.
 */
static void mlx5_rxq_stop(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  unsigned int i;
...
static int mlx5_rxq_ctrl_prepare(struct rte_eth_dev *dev,
  ...
  rxq_ctrl->obj = mlx5_malloc(MLX5_MEM_RTE | MLX5_MEM_ZERO,
                              sizeof(*rxq_ctrl->obj), 0, rxq_ctrl->socket);
  if (!rxq_ctrl->obj) {
  ...
  DRV_LOG(DEBUG, "Port %u rxq %u updated with %p.", dev->data->port_id, idx,
          (void *)&rxq_ctrl->obj);
  return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rxq_start(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  unsigned int i;
  int ret = 0;
  ...
  DRV_LOG(DEBUG, "Port %u dev_cap.max_qp_wr is %d.", dev->data->port_id,
          priv->sh->dev_cap.max_qp_wr);
  DRV_LOG(DEBUG, "Port %u dev_cap.max_sge is %d.", dev->data->port_id,
          priv->sh->dev_cap.max_sge);
  for (i = 0; i != priv->rxqs_n; ++i) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_hairpin_auto_bind(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  for (i = 0; i != priv->txqs_n; ++i) {
    ...
    if (!txq_ctrl->obj) {
      rte_errno = ENOMEM;
      DRV_LOG(ERR, "port %u no txq object found: %d", dev->data->port_id, i);
      mlx5_txq_release(dev, i);
      return -rte_errno;
    }
    sq = txq_ctrl->obj->sq;
    ...
    if (rxq == NULL) {
      mlx5_txq_release(dev, i);
      rte_errno = EINVAL;
      DRV_LOG(ERR, "port %u no rxq object found: %d", dev->data->port_id,
              txq_ctrl->hairpin_conf.peers[0].queue);
      return -rte_errno;
    }
    rxq_ctrl = rxq->ctrl;
    if (!rxq_ctrl->is_hairpin || rxq->hairpin_conf.peers[0].queue != i) {
      rte_errno = ENOMEM;
      DRV_LOG(ERR,
              "port %u Tx queue %d can't be binded to "
              "Rx queue %d",
              dev->data->port_id, i, txq_ctrl->hairpin_conf.peers[0].queue);
      goto error;
    }
    rq = rxq_ctrl->obj->rq;
    if (!rq) {
      rte_errno = ENOMEM;
      DRV_LOG(ERR, "port %u hairpin no matching rxq: %d", dev->data->port_id,
              txq_ctrl->hairpin_conf.peers[0].queue);
      goto error;
    }
    ...
    sq_attr.hairpin_peer_vhca = priv->sh->cdev->config.hca_attr.vhca_id;
    ...
    rq_attr.hairpin_peer_vhca = priv->sh->cdev->config.hca_attr.vhca_id;
...
int mlx5_hairpin_queue_peer_update(struct rte_eth_dev *dev, uint16_t peer_queue,
  ...
  if (dev->data->dev_started == 0) {
    rte_errno = EBUSY;
    DRV_LOG(ERR, "peer port %u is not started", dev->data->port_id);
    return -rte_errno;
  }
  /*
  ...
  if (direction == 0) {
    ...
    if (txq_ctrl == NULL) {
      rte_errno = EINVAL;
      DRV_LOG(ERR, "Failed to get port %u Tx queue %d", dev->data->port_id,
              peer_queue);
      return -rte_errno;
    }
    if (!txq_ctrl->is_hairpin) {
      rte_errno = EINVAL;
      DRV_LOG(ERR, "port %u queue %d is not a hairpin Txq", dev->data->port_id,
              peer_queue);
    ...
    if (txq_ctrl->obj == NULL || txq_ctrl->obj->sq == NULL) {
      rte_errno = ENOMEM;
      DRV_LOG(ERR, "port %u no Txq object found: %d", dev->data->port_id,
              peer_queue);
      mlx5_txq_release(dev, peer_queue);
      return -rte_errno;
    }
    ...
    if (rxq == NULL) {
      rte_errno = EINVAL;
      DRV_LOG(ERR, "Failed to get port %u Rx queue %d", dev->data->port_id,
              peer_queue);
      return -rte_errno;
    }
    rxq_ctrl = rxq->ctrl;
    if (!rxq_ctrl->is_hairpin) {
      rte_errno = EINVAL;
      DRV_LOG(ERR, "port %u queue %d is not a hairpin Rxq", dev->data->port_id,
              peer_queue);
      return -rte_errno;
    }
...
int mlx5_hairpin_queue_peer_bind(struct rte_eth_dev *dev, uint16_t cur_queue,
  ...
  if (direction != 0) {
    ...
    if (!txq_ctrl->is_hairpin) {
      rte_errno = EINVAL;
      DRV_LOG(ERR, "port %u queue %d not a hairpin Txq", dev->data->port_id,
              cur_queue);
    ...
    if (txq_ctrl->hairpin_status != 0) {
      DRV_LOG(DEBUG, "port %u Tx queue %d is already bound", dev->data->port_id,
              cur_queue);
      mlx5_txq_release(dev, cur_queue);
      return 0;
    }
     ...
     */
    if (peer_info->tx_explicit != txq_ctrl->hairpin_conf.tx_explicit) {
      rte_errno = EINVAL;
      DRV_LOG(ERR,
              "port %u Tx queue %d and peer Tx rule mode"
              " mismatch",
              dev->data->port_id, cur_queue);
      mlx5_txq_release(dev, cur_queue);
      return -rte_errno;
    }
    if (peer_info->manual_bind != txq_ctrl->hairpin_conf.manual_bind) {
      rte_errno = EINVAL;
      DRV_LOG(ERR,
              "port %u Tx queue %d and peer binding mode"
              " mismatch",
              dev->data->port_id, cur_queue);
      mlx5_txq_release(dev, cur_queue);
      return -rte_errno;
    ...
    if (!rxq_ctrl->is_hairpin) {
      rte_errno = EINVAL;
      DRV_LOG(ERR, "port %u queue %d not a hairpin Rxq", dev->data->port_id,
              cur_queue);
      return -rte_errno;
    }
    if (rxq_ctrl->obj == NULL || rxq_ctrl->obj->rq == NULL) {
    ...
    if (rxq->hairpin_status != 0) {
      DRV_LOG(DEBUG, "port %u Rx queue %d is already bound", dev->data->port_id,
              cur_queue);
      return 0;
    }
    if (peer_info->tx_explicit != rxq->hairpin_conf.tx_explicit) {
      rte_errno = EINVAL;
      DRV_LOG(ERR,
              "port %u Rx queue %d and peer Tx rule mode"
              " mismatch",
              dev->data->port_id, cur_queue);
      return -rte_errno;
    }
    if (peer_info->manual_bind != rxq->hairpin_conf.manual_bind) {
      rte_errno = EINVAL;
      DRV_LOG(ERR,
              "port %u Rx queue %d and peer binding mode"
              " mismatch",
              dev->data->port_id, cur_queue);
      return -rte_errno;
    }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_hairpin_bind_single_port(struct rte_eth_dev *dev,
                                         uint16_t rx_port) {
  struct mlx5_priv *priv = dev->data->dev_private;
  int ret = 0;
  struct mlx5_txq_ctrl *txq_ctrl;
  ...
  for (i = 0; i != priv->txqs_n; i++) {
    ...
    if (conf->peers[0].port == rx_port) {
      if (num_q == 0) {
        ...
        if (manual != conf->manual_bind || explicit != conf->tx_explicit) {
          rte_errno = EINVAL;
          DRV_LOG(ERR,
                  "port %u queue %d mode"
                  " mismatch: %u %u, %u %u",
                  local_port, i, manual, conf->manual_bind, explicit,
                  conf->tx_explicit);
          mlx5_txq_release(dev, i);
          return -rte_errno;
        }
  ...
  for (i = 0; i != priv->txqs_n; i++) {
     ...
     */
    ret = rte_eth_hairpin_queue_peer_update(rx_port, rx_queue, NULL, &peer, 1);
    if (ret != 0) {
      mlx5_txq_release(dev, i);
      goto error;
    }
     ...
     */
    ret = rte_eth_hairpin_queue_peer_bind(rx_port, rx_queue, &cur, 0);
    if (ret != 0) {
      mlx5_txq_release(dev, i);
      goto error;
    }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_hairpin_unbind_single_port(struct rte_eth_dev *dev,
                                           uint16_t rx_port) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_txq_ctrl *txq_ctrl;
  uint32_t i;
  ...
  for (i = 0; i != priv->txqs_n; i++) {
    ...
    if (ret) {
      DRV_LOG(ERR, "port %u Rx queue %d unbind - failure", rx_port, rx_queue);
      return ret;
    }
    ret = mlx5_hairpin_queue_peer_unbind(dev, i, 1);
    if (ret) {
      DRV_LOG(ERR, "port %u Tx queue %d unbind - failure", cur_port, i);
      return ret;
    }
  }
  return 0;
...
int mlx5_dev_start(struct rte_eth_dev *dev) {
  ...
  if (dev->data->nb_rx_queues > 0) {
    ...
    if (ret) {
      DRV_LOG(ERR, "port %u reta config failed: %s", dev->data->port_id,
              strerror(rte_errno));
      return -rte_errno;
    }
  }
  ...
  if (ret) {
    DRV_LOG(ERR, "port %u Tx packet pacing init failed: %s", dev->data->port_id,
            strerror(rte_errno));
    goto error;
  }
  if (mlx5_devx_obj_ops_en(priv->sh) && priv->obj_ops.lb_dummy_queue_create) {
  ...
  if (ret) {
    DRV_LOG(ERR, "port %u Tx queue allocation failed: %s", dev->data->port_id,
            strerror(rte_errno));
    goto error;
  }
  if (priv->config.std_delay_drop || priv->config.hp_delay_drop) {
    if (!priv->sh->dev_cap.vf && !priv->sh->dev_cap.sf && !priv->representor) {
      ret = mlx5_get_flag_dropless_rq(dev);
      if (ret < 0)
        DRV_LOG(WARNING, "port %u cannot query dropless flag",
                dev->data->port_id);
      else if (!ret)
        DRV_LOG(WARNING, "port %u dropless_rq OFF, no rearming",
                dev->data->port_id);
    } else {
      DRV_LOG(DEBUG, "port %u doesn't support dropless_rq flag",
              dev->data->port_id);
    }
  }
  ret = mlx5_rxq_start(dev);
  if (ret) {
    DRV_LOG(ERR, "port %u Rx queue allocation failed: %s", dev->data->port_id,
            strerror(rte_errno));
    goto error;
  }
  /*
  ...
  if (ret) {
    DRV_LOG(ERR, "port %u hairpin auto binding failed: %s", dev->data->port_id,
            strerror(rte_errno));
    goto error;
  }
  ...
  if (ret) {
    DRV_LOG(ERR, "port %u failed to attach indirect actions: %s",
            dev->data->port_id, rte_strerror(rte_errno));
    goto error;
  }
  ret = mlx5_traffic_enable(dev);
  if (ret) {
    DRV_LOG(ERR, "port %u failed to set defaults flows", dev->data->port_id);
    goto error;
  }
  ...
  if (rte_intr_fd_get(priv->sh->intr_handle) >= 0) {
    ...
    DRV_LOG(INFO, "port %u starts without RMV interrupts.", dev->data->port_id);
  ...
  if (rte_intr_fd_get(priv->sh->intr_handle_nl) >= 0) {
    ...
    DRV_LOG(INFO, "port %u starts without LSC interrupts.", dev->data->port_id);
...
int mlx5_traffic_enable(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i != priv->txqs_n; ++i) {
    ...
    if (txq_ctrl->is_hairpin && txq_ctrl->hairpin_conf.tx_explicit == 0 &&
        txq_ctrl->hairpin_conf.peers[0].port == priv->dev_data->port_id) {
    ...
    if (priv->sh->config.dv_esw_en) {
      if (mlx5_flow_create_devx_sq_miss_flow(dev, i) == 0) {
        DRV_LOG(ERR,
                "Port %u Tx queue %u SQ create representor devx default miss "
                "rule failed.",
                dev->data->port_id, i);
        goto error;
      }
  ...
  if (priv->sh->config.dv_esw_en) {
    if (mlx5_flow_create_esw_table_zero_flow(dev))
      ...
      DRV_LOG(INFO,
              "port %u FDB default rule cannot be"
  ...
  if (!priv->sh->config.lacp_by_user && priv->pf_bond >= 0) {
    ...
    if (ret)
      DRV_LOG(INFO,
              "port %u LACP rule cannot be created - "
              "forward LACP to kernel.",
              dev->data->port_id);
    else
  ...
  if (dev->data->all_multicast) {
    ...
    for (i = 0; i != vlan_filter_n; ++i) {
      ...
      ret = mlx5_ctrl_flow_vlan(dev, &bcast, &bcast, &vlan_spec, &vlan_mask);
      if (ret)
        goto error;
      ret = mlx5_ctrl_flow_vlan(dev, &ipv6_multi_spec, &ipv6_multi_mask,
                                &vlan_spec, &vlan_mask);
      if (ret)
        goto error;
    }
    if (!vlan_filter_n) {
      ...
      if (ret) {
        ...
        DRV_LOG(WARNING, "IPv6 broadcast is not supported");
        ret = 0;
      }
    }
  }
  ...
  for (i = 0; i != MLX5_MAX_MAC_ADDRESSES; ++i) {
    ...
    if (!memcmp(mac, &cmp, sizeof(*mac)))
      continue;
    memcpy(&unicast.dst.addr_bytes, mac->addr_bytes, RTE_ETHER_ADDR_LEN);
    for (j = 0; j != vlan_filter_n; ++j) {
      ...
      ret = mlx5_ctrl_flow_vlan(dev, &unicast, &unicast_mask, &vlan_spec,
                                &vlan_mask);
      if (ret)
        goto error;
    }
