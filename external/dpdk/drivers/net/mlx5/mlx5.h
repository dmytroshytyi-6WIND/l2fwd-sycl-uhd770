...
#define IS_BATCH_CNT(cnt)                                                      \
  (((cnt) & (MLX5_CNT_SHARED_OFFSET - 1)) >= MLX5_CNT_BATCH_OFFSET)
...
#define MLX5_CNT_LEN(pool)                                                     \
  (MLX5_CNT_SIZE + ((pool)->is_aged ? MLX5_AGE_SIZE : 0))
#define MLX5_POOL_GET_CNT(pool, index)                                         \
  ((struct mlx5_flow_counter *)((uint8_t *)((pool) + 1) +                      \
                                (index) * (MLX5_CNT_LEN(pool))))
#define MLX5_CNT_ARRAY_IDX(pool, cnt)                                          \
  ((int)(((uint8_t *)(cnt) - (uint8_t *)((pool) + 1)) / MLX5_CNT_LEN(pool)))
...
#define MLX5_CNT_TO_AGE(cnt) ((struct mlx5_age_param *)((cnt) + 1))
/*
...
#define MLX5_AGE_SET(age_info, BIT) ((age_info)->flags |= (1 << (BIT)))
#define MLX5_AGE_UNSET(age_info, BIT) ((age_info)->flags &= ~(1 << (BIT)))
#define MLX5_AGE_GET(age_info, BIT) ((age_info)->flags & (1 << (BIT)))
...
#define MLX5_MTR_ALL_DOMAIN_BIT                                                \
  (MLX5_MTR_DOMAIN_INGRESS_BIT | MLX5_MTR_DOMAIN_EGRESS_BIT |                  \
...
struct mlx5_flow_mtr_mng {
  ...
  /* Meter drop table. */
  struct mlx5_flow_dv_matcher *drop_matcher[MLX5_MTR_DOMAIN_MAX][MLX5_REG_BITS];
...
__extension__ struct mlx5_flex_pattern_field {
  uint16_t width : 6;
  uint16_t shift : 5;
  uint16_t reg_id : 5;
};
...
/* Hash Rx queue. */
__extension__ struct mlx5_hrxq {
...
struct mlx5_obj_ops {
  ...
  int (*ind_table_new)(struct rte_eth_dev *dev, const unsigned int log_n,
                       struct mlx5_ind_table_obj *ind_tbl);
  int (*ind_table_modify)(struct rte_eth_dev *dev, const unsigned int log_n,
...
struct mlx5_priv {
  ...
  struct mlx5dr_action *hw_drop[MLX5_HW_ACTION_FLAG_MAX][MLX5DR_TABLE_TYPE_MAX];
  /* HW steering global drop action. */
 ...
 */
static inline bool mlx5_devx_obj_ops_en(struct mlx5_dev_ctx_shared *sh) {
  /*
...
#define MLX5_ETH_FOREACH_DEV(port_id, dev)                                     \
  for (port_id = mlx5_eth_find_next(0, dev); port_id < RTE_MAX_ETHPORTS;       \
...
#define MLX5_REPRESENTOR_REPR(repr_id) ((repr_id)&0xfff)
#define MLX5_REPRESENTOR_TYPE(repr_id) (((repr_id) >> 12) & 3)
...
int mlx5_sysfs_switch_info(unsigned int ifindex, struct mlx5_switch_info *info);
...
int save_dump_file(const unsigned char *data, uint32_t size, uint32_t type,
...
int mlx5_flow_meter_flush(struct rte_eth_dev *dev, struct rte_mtr_error *error);
...
int mlx5_aso_meter_update_by_wqe(struct mlx5_dev_ctx_shared *sh,
                                 struct mlx5_aso_mtr *mtr);
int mlx5_aso_mtr_wait(struct mlx5_dev_ctx_shared *sh, struct mlx5_aso_mtr *mtr);
