...
int mlx5_auxiliary_get_ifindex(const char *sf_name) {
  ...
  if (mlx5_auxiliary_get_child_name(sf_name, "/net", if_name,
                                    sizeof(if_name)) != 0)
    return -rte_errno;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_ifreq_by_ifname(const char *ifname, int req,
                                struct ifreq *ifr) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_ifreq(const struct rte_eth_dev *dev, int req,
                      struct ifreq *ifr) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_set_flags(struct rte_eth_dev *dev, unsigned int keep,
                          unsigned int flags) {
  struct ifreq request;
 ...
 */
static struct rte_eth_dev *mlx5_find_master_dev(struct rte_eth_dev *dev) {
  ...
  MLX5_ETH_FOREACH_DEV(port_id, dev->device) {
    ...
    if (opriv && opriv->master && opriv->domain_id == domain_id &&
        opriv->sh == priv->sh)
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_link_update_unlocked_gset(struct rte_eth_dev *dev,
  ...
  dev_link.link_duplex =
      ((edata.duplex == DUPLEX_HALF) ? RTE_ETH_LINK_HALF_DUPLEX
                                     : RTE_ETH_LINK_FULL_DUPLEX);
  dev_link.link_autoneg =
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_link_update_unlocked_gs(struct rte_eth_dev *dev,
                                        struct rte_eth_link *link)

{
  ...
  *ecmd = gcmd;
  ...
  dev_link.link_speed =
      (ecmd->speed == UINT32_MAX) ? RTE_ETH_SPEED_NUM_UNKNOWN : ecmd->speed;
  sc = ecmd->link_mode_masks[0] | ((uint64_t)ecmd->link_mode_masks[1] << 32);
  priv->link_speed_capa = 0;
  ...
  sc = ecmd->link_mode_masks[2] | ((uint64_t)ecmd->link_mode_masks[3] << 32);
  if (sc & (MLX5_BITSHIFT(ETHTOOL_LINK_MODE_200000baseCR4_Full_BIT) |
            MLX5_BITSHIFT(ETHTOOL_LINK_MODE_200000baseLR4_ER4_FR4_Full_BIT) |
            MLX5_BITSHIFT(ETHTOOL_LINK_MODE_200000baseDR4_Full_BIT)))
    priv->link_speed_capa |= RTE_ETH_LINK_SPEED_200G;
  dev_link.link_duplex =
      ((ecmd->duplex == DUPLEX_HALF) ? RTE_ETH_LINK_HALF_DUPLEX
                                     : RTE_ETH_LINK_FULL_DUPLEX);
  dev_link.link_autoneg =
      !(dev->data->dev_conf.link_speeds & RTE_ETH_LINK_SPEED_FIXED);
  *link = dev_link;
...
int mlx5_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
  ...
  ret = !!memcmp(&dev->data->dev_link, &dev_link, sizeof(struct rte_eth_link));
...
int mlx5_dev_get_flow_ctrl(struct rte_eth_dev *dev,
  ...
  struct ifreq ifr;
  struct ethtool_pauseparam ethpause = {.cmd = ETHTOOL_GPAUSEPARAM};
  int ret;
...
int mlx5_dev_set_flow_ctrl(struct rte_eth_dev *dev,
                           struct rte_eth_fc_conf *fc_conf) {
  struct ifreq ifr;
  struct ethtool_pauseparam ethpause = {.cmd = ETHTOOL_SPAUSEPARAM};
  int ret;
 ...
 */
static void mlx5_dev_interrupt_device_fatal(struct mlx5_dev_ctx_shared *sh) {
  uint32_t i;
...
static void mlx5_dev_interrupt_nl_cb(struct nlmsghdr *hdr, void *cb_arg) {
  ...
  for (i = 0; i < sh->max_port; i++) {
    ...
    if (dev->data->dev_configured && !dev->data->dev_conf.intr_conf.lsc)
      break;
    priv = dev->data->dev_private;
...
void mlx5_dev_interrupt_handler(void *cb_arg) {
  ...
  for (;;) {
    ...
    if (!tmp) {
      ...
      mlx5_glue->ack_async_event(&event);
      DRV_LOG(DEBUG, "unsupported common event (type %d)", event.event_type);
      continue;
    }
    if (tmp > sh->max_port) {
    ...
    MLX5_ASSERT(dev);
    DRV_LOG(DEBUG, "port %u cannot handle an unknown event (type %d)",
 ...
 *   Callback argument.
 */
void mlx5_dev_interrupt_handler_devx(void *cb_arg) {
  ...
  while (!mlx5_glue->devx_get_async_cmd_comp(sh->devx_comp, &out.cmd_resp,
                                             sizeof(out.buf)))
    mlx5_flow_async_pool_query_handle(sh, (uint64_t)out.cmd_resp.wr_id,
 ...
 */
static void mlx5_sysfs_check_switch_info(bool device_dir,
...
int mlx5_sysfs_switch_info(unsigned int ifindex,
  ...
  MKSTR(phys_port_name, "/sys/class/net/%s/phys_port_name", ifname);
  MKSTR(phys_switch_id, "/sys/class/net/%s/phys_switch_id", ifname);
  MKSTR(pci_device, "/sys/class/net/%s/device", ifname);
  ...
  port_switch_id_set =
      fscanf(file, "%" SCNx64 "%c", &data.switch_id, &c) == 2 && c == '\n';
  fclose(file);
...
int mlx5_get_module_eeprom(struct rte_eth_dev *dev,
  ...
  eeprom =
      mlx5_malloc(MLX5_MEM_ZERO, (sizeof(struct ethtool_eeprom) + info->length),
                  0, SOCKET_ID_ANY);
  if (!eeprom) {
    DRV_LOG(WARNING,
            "port %u cannot allocate memory for "
            "eeprom data",
            dev->data->port_id);
    rte_errno = ENOMEM;
    return -rte_errno;
 ...
 *   rte_errno is set.
 */
static int _mlx5_os_read_dev_counters(struct rte_eth_dev *dev, int pf,
                                      uint64_t *stats) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (pf >= 0)
    ret = mlx5_ifreq_by_ifname(priv->sh->bond.ports[pf].ifname, SIOCETHTOOL,
                               &ifr);
  else
    ret = mlx5_ifreq(dev, SIOCETHTOOL, &ifr);
  if (ret) {
    DRV_LOG(WARNING, "port %u unable to read statistic values from device",
            dev->data->port_id);
    return ret;
  }
  for (i = 0; i != xstats_ctrl->mlx5_stats_n; ++i) {
    if (xstats_ctrl->info[i].dev)
      continue;
    stats[i] += (uint64_t)et_stats->data[xstats_ctrl->dev_table_idx[i]];
  }
  return 0;
}
...
int mlx5_os_read_dev_counters(struct rte_eth_dev *dev, uint64_t *stats) {
  ...
  for (i = 0; i != xstats_ctrl->mlx5_stats_n; ++i) {
    if (!xstats_ctrl->info[i].dev)
      continue;
    ret = mlx5_os_read_dev_stat(priv, xstats_ctrl->info[i].ctr_name, &stats[i]);
...
int mlx5_os_get_stats_n(struct rte_eth_dev *dev) {
  ...
  if (priv->master && priv->pf_bond >= 0)
    /* Bonding PF. */
    ret =
        mlx5_ifreq_by_ifname(priv->sh->bond.ports[0].ifname, SIOCETHTOOL, &ifr);
  else
    ret = mlx5_ifreq(dev, SIOCETHTOOL, &ifr);
  if (ret) {
...
void mlx5_os_stats_init(struct rte_eth_dev *dev) {
  ...
  strings = (struct ethtool_gstrings *)mlx5_malloc(
      0, str_sz + sizeof(struct ethtool_gstrings), 0, SOCKET_ID_ANY);
  if (!strings) {
  ...
  if (ret)
    DRV_LOG(ERR, "port %u cannot read device counters: %s", dev->data->port_id,
            strerror(rte_errno));
...
int mlx5_get_flag_dropless_rq(struct rte_eth_dev *dev) {
  ...
  if (!ret) {
    ...
    DRV_LOG(WARNING, "port %u cannot get the sset info", dev->data->port_id);
    goto exit;
  }
  if (!len) {
    DRV_LOG(WARNING, "port %u does not have private flag", dev->data->port_id);
  ...
  if (!strings) {
    DRV_LOG(WARNING,
            "port %u unable to allocate memory for"
            " private flags",
            dev->data->port_id);
    rte_errno = ENOMEM;
    ret = -rte_errno;
