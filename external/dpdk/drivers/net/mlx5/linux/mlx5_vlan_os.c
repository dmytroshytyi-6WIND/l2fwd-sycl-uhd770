...
void mlx5_vlan_vmwa_release(struct rte_eth_dev *dev,
  ...
  if (--vlan_dev[vlan->tag].refcnt == 0 && vlan_dev[vlan->tag].ifindex) {
...
void mlx5_vlan_vmwa_acquire(struct rte_eth_dev *dev,
  ...
  if (vlan_dev[vlan->tag].refcnt == 0) {
    ...
    vlan_dev[vlan->tag].ifindex =
        mlx5_nl_vlan_vmwa_create(vmwa, vmwa->vf_ifindex, vlan->tag);
  }
...
void *mlx5_vlan_vmwa_init(struct rte_eth_dev *dev, uint32_t ifindex) {
  ...
  vmwa = mlx5_malloc(MLX5_MEM_ZERO, sizeof(*vmwa), sizeof(uint32_t),
                     SOCKET_ID_ANY);
  if (!vmwa) {
    DRV_LOG(WARNING, "Can not allocate memory"
  ...
  if (vmwa->nl_socket < 0) {
    DRV_LOG(WARNING, "Can not create Netlink socket"
                     " for VLAN workaround context");
    mlx5_free(vmwa);
    return NULL;
  }
 ...
 */
void mlx5_vlan_vmwa_exit(void *vmctx) {
  unsigned int i;
  ...
  for (i = 0; i < RTE_DIM(vmwa->vlan_dev); i++) {
    if (vmwa->vlan_dev[i].ifindex)
      mlx5_nl_vlan_vmwa_delete(vmwa, vmwa->vlan_dev[i].ifindex);
  }
  if (vmwa->nl_socket >= 0)
