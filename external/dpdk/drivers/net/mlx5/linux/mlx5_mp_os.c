 ...
 */
static int mlx5_mp_os_handle_port_agnostic(const struct rte_mp_msg *mp_msg,
                                           const void *peer) {
  ...
  case MLX5_MP_REQ_CREATE_MR:
    ...
    lkey = mlx5_mr_create(mng->cdev, &mng->cdev->mr_scache, &entry, mng->addr);
    if (lkey == UINT32_MAX)
  ...
  case MLX5_MP_REQ_MEMPOOL_REGISTER:
    mp_init_port_agnostic_msg(&mp_res, param->type);
    res->result =
        mlx5_mr_mempool_register(mng->cdev, mng->mempool, mng->is_extmem);
    return rte_mp_reply(&mp_res, peer);
  case MLX5_MP_REQ_MEMPOOL_UNREGISTER:
    mp_init_port_agnostic_msg(&mp_res, param->type);
    res->result = mlx5_mr_mempool_unregister(mng->cdev, mng->mempool);
    return rte_mp_reply(&mp_res, peer);
  default:
    return 1;
  }
...
int mlx5_mp_os_primary_handle(const struct rte_mp_msg *mp_msg,
  ...
  case MLX5_MP_REQ_QUEUE_STATE_MODIFY:
    mp_init_msg(&priv->mp_id, &mp_res, param->type);
    res->result =
        mlx5_queue_state_modify_primary(dev, &param->args.state_modify);
    ret = rte_mp_reply(&mp_res, peer);
    break;
  case MLX5_MP_REQ_QUEUE_RX_STOP:
    mp_init_msg(&priv->mp_id, &mp_res, param->type);
    res->result =
        mlx5_rx_queue_stop_primary(dev, param->args.queue_id.queue_id);
    ret = rte_mp_reply(&mp_res, peer);
    break;
  case MLX5_MP_REQ_QUEUE_RX_START:
    mp_init_msg(&priv->mp_id, &mp_res, param->type);
    res->result =
        mlx5_rx_queue_start_primary(dev, param->args.queue_id.queue_id);
    ret = rte_mp_reply(&mp_res, peer);
    break;
  case MLX5_MP_REQ_QUEUE_TX_STOP:
    mp_init_msg(&priv->mp_id, &mp_res, param->type);
    res->result =
        mlx5_tx_queue_stop_primary(dev, param->args.queue_id.queue_id);
    ret = rte_mp_reply(&mp_res, peer);
    break;
  case MLX5_MP_REQ_QUEUE_TX_START:
    mp_init_msg(&priv->mp_id, &mp_res, param->type);
    res->result =
        mlx5_tx_queue_start_primary(dev, param->args.queue_id.queue_id);
    ret = rte_mp_reply(&mp_res, peer);
    break;
  default:
    rte_errno = EINVAL;
    DRV_LOG(ERR, "port %u invalid mp request type", dev->data->port_id);
    return -rte_errno;
  }
  return ret;
}
...
static void mp_req_on_rxtx(struct rte_eth_dev *dev,
  ...
  if (type != MLX5_MP_REQ_START_RXTX && type != MLX5_MP_REQ_STOP_RXTX) {
    DRV_LOG(ERR, "port %u unknown request (req_type %d)", dev->data->port_id,
            type);
    return;
  }
  ...
  if (type == MLX5_MP_REQ_START_RXTX) {
    mp_req.num_fds = 1;
    mp_req.fds[0] = ((struct ibv_context *)priv->sh->cdev->ctx)->cmd_fd;
  }
  ...
  if (mp_rep.nb_sent != mp_rep.nb_received) {
    DRV_LOG(ERR, "port %u not all secondaries responded (req_type %d)",
            dev->data->port_id, type);
    goto exit;
  }
