 ...
 */
static int mlx5_rxq_obj_modify_wq_vlan_strip(struct mlx5_rxq_priv *rxq,
                                             int on) {
  uint16_t vlan_offloads = (on ? IBV_WQ_FLAGS_CVLAN_STRIPPING : 0) | 0;
 ...
 */
static int mlx5_ibv_modify_wq(struct mlx5_rxq_priv *rxq, uint8_t type) {
  struct ibv_wq_attr mod = {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_ibv_modify_qp(struct mlx5_txq_obj *obj,
                              enum mlx5_txq_modify_type type,
                              uint8_t dev_port) {
  ...
  if (type != MLX5_TXQ_MOD_RST2RDY) {
    ...
    if (ret) {
      DRV_LOG(ERR, "Cannot change Tx QP state to RESET %s", strerror(errno));
      rte_errno = errno;
      return ret;
    }
  ...
  if (ret) {
    DRV_LOG(ERR, "Cannot change Tx QP state to INIT %s", strerror(errno));
    rte_errno = errno;
    return ret;
  }
  ...
  ret = mlx5_glue->modify_qp(obj->qp, &mod, IBV_QP_STATE);
  if (ret) {
    DRV_LOG(ERR, "Cannot change Tx QP state to RTR %s", strerror(errno));
    rte_errno = errno;
    return ret;
  }
  ...
  ret = mlx5_glue->modify_qp(obj->qp, &mod, IBV_QP_STATE);
  if (ret) {
    DRV_LOG(ERR, "Cannot change Tx QP state to RTS %s", strerror(errno));
    rte_errno = errno;
    return ret;
  }
  return 0;
 ...
 */
static struct ibv_cq *mlx5_rxq_ibv_cq_create(struct mlx5_rxq_priv *rxq) {
  ...
  if (priv->config.cqe_comp && !rxq_data->hw_timestamp) {
    cq_attr.mlx5.comp_mask |= MLX5DV_CQ_INIT_ATTR_MASK_COMPRESSED_CQE;
    rxq_data->byte_mask = UINT32_MAX;
#ifdef HAVE_IBV_DEVICE_STRIDING_RQ_SUPPORT
    if (mlx5_rxq_mprq_enabled(rxq_data)) {
      cq_attr.mlx5.cqe_comp_res_format = MLX5DV_CQE_RES_FORMAT_CSUM_STRIDX;
      rxq_data->mcqe_format = MLX5_CQE_RESP_FORMAT_CSUM_STRIDX;
    } else {
  ...
  return mlx5_glue->cq_ex_to_cq(mlx5_glue->dv_create_cq(
      priv->sh->cdev->ctx, &cq_attr.ibv, &cq_attr.mlx5));
}
 ...
 */
static struct ibv_wq *mlx5_rxq_ibv_wq_create(struct mlx5_rxq_priv *rxq) {
  struct mlx5_priv *priv = rxq->priv;
  struct mlx5_rxq_ctrl *rxq_ctrl = rxq->ctrl;
  struct mlx5_rxq_data *rxq_data = &rxq_ctrl->rxq;
  struct mlx5_rxq_obj *rxq_obj = rxq_ctrl->obj;
  ...
  wq_attr.ibv = (struct ibv_wq_init_attr){
      ...
      .create_flags = (rxq_data->vlan_strip ? IBV_WQ_FLAGS_CVLAN_STRIPPING : 0),
  };
  ...
  rxq_obj->wq =
      mlx5_glue->dv_create_wq(priv->sh->cdev->ctx, &wq_attr.ibv, &wq_attr.mlx5);
#else
  ...
  if (rxq_obj->wq) {
    ...
    if (wq_attr.ibv.max_wr != (wqe_n >> rxq_data->sges_n) ||
      ...
      DRV_LOG(ERR,
              ...
              " %u*%u WRs*SGEs.",
              priv->dev_data->port_id, rxq->idx, wqe_n >> rxq_data->sges_n,
              (1 << rxq_data->sges_n), wq_attr.ibv.max_wr, wq_attr.ibv.max_sge);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rxq_ibv_obj_new(struct mlx5_rxq_priv *rxq) {
  ...
  if (rxq_ctrl->irq) {
    tmpl->ibv_channel = mlx5_glue->create_comp_channel(priv->sh->cdev->ctx);
    if (!tmpl->ibv_channel) {
      DRV_LOG(ERR, "Port %u: comp channel creation failure.", port_id);
      rte_errno = ENOMEM;
      goto error;
    }
  ...
  if (!tmpl->ibv_cq) {
    DRV_LOG(ERR, "Port %u Rx queue %u CQ creation failure.", port_id, idx);
    rte_errno = ENOMEM;
    goto error;
  }
  ...
  if (!tmpl->wq) {
    DRV_LOG(ERR, "Port %u Rx queue %u WQ creation failure.", port_id, idx);
    rte_errno = ENOMEM;
    goto error;
  }
  ...
  if (ret) {
    DRV_LOG(ERR, "Port %u Rx queue %u WQ state to IBV_WQS_RDY failed.", port_id,
            idx);
    rte_errno = ret;
    goto error;
  }
 ...
 *   Pointer to Rx queue.
 */
static void mlx5_rxq_ibv_obj_release(struct mlx5_rxq_priv *rxq) {
  ...
  if (rxq_obj->ibv_channel)
    claim_zero(mlx5_glue->destroy_comp_channel(rxq_obj->ibv_channel));
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rx_ibv_get_event(struct mlx5_rxq_obj *rxq_obj) {
  struct ibv_cq *ev_cq;
  void *ev_ctx;
  int ret = mlx5_glue->get_cq_event(rxq_obj->ibv_channel, &ev_cq, &ev_ctx);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_ibv_ind_table_new(struct rte_eth_dev *dev,
  ...
  for (i = 0; i != ind_tbl->queues_n; ++i) {
    struct mlx5_rxq_priv *rxq = mlx5_rxq_get(dev, ind_tbl->queues[i]);
  ...
  ind_tbl->ind_table = mlx5_glue->create_rwq_ind_table(
      priv->sh->cdev->ctx, &(struct ibv_rwq_ind_table_init_attr){
 ...
 */
static void mlx5_ibv_ind_table_destroy(struct mlx5_ind_table_obj *ind_tbl) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_ibv_hrxq_new(struct rte_eth_dev *dev, struct mlx5_hrxq *hrxq,
  ...
  if (tunnel) {
    qp_init_attr.comp_mask = MLX5DV_QP_INIT_ATTR_MASK_QP_CREATE_FLAGS;
  ...
  if (dev->data->dev_conf.lpbk_mode) {
    ...
    qp_init_attr.comp_mask |= MLX5DV_QP_INIT_ATTR_MASK_QP_CREATE_FLAGS;
    qp_init_attr.create_flags |= MLX5DV_QP_CREATE_TIR_ALLOW_SELF_LOOPBACK_UC;
  }
#endif
  qp = mlx5_glue->dv_create_qp(
      ...
      &(struct ibv_qp_init_attr_ex){
          ...
          .comp_mask = IBV_QP_INIT_ATTR_PD | IBV_QP_INIT_ATTR_IND_TABLE |
 ...
 */
static void mlx5_ibv_qp_destroy(struct mlx5_hrxq *hrxq) {
 ...
 *   Pointer to Ethernet device.
 */
static void mlx5_rxq_ibv_obj_drop_release(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rxq_ibv_obj_drop_create(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  rxq_ctrl = mlx5_malloc(MLX5_MEM_ZERO, sizeof(*rxq_ctrl), 0, SOCKET_ID_ANY);
  if (rxq_ctrl == NULL) {
  ...
  rxq->ctrl = rxq_ctrl;
  rxq_obj = mlx5_malloc(MLX5_MEM_ZERO, sizeof(*rxq_obj), 0, SOCKET_ID_ANY);
  if (rxq_obj == NULL) {
    DRV_LOG(DEBUG, "Port %u cannot allocate drop Rx queue memory.",
            dev->data->port_id);
    rte_errno = ENOMEM;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_ibv_drop_action_create(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  hrxq->qp = mlx5_glue->create_qp_ex(
      priv->sh->cdev->ctx,
      &(struct ibv_qp_init_attr_ex){
          .qp_type = IBV_QPT_RAW_PACKET,
          .comp_mask = IBV_QP_INIT_ATTR_PD | IBV_QP_INIT_ATTR_IND_TABLE |
 ...
 *   Pointer to Ethernet device.
 */
static void mlx5_ibv_drop_action_destroy(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_hrxq *hrxq = priv->drop_queue.hrxq;
 ...
 */
static struct ibv_qp *mlx5_txq_ibv_qp_create(struct rte_eth_dev *dev,
                                             uint16_t idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (qp_obj == NULL) {
    DRV_LOG(ERR, "Port %u Tx queue %u QP creation failure.", dev->data->port_id,
            idx);
    rte_errno = errno;
  }
  return qp_obj;
 ...
 */
static void mlx5_txq_ibv_uar_init(struct mlx5_txq_ctrl *txq_ctrl,
                                  void *bf_reg) {
...
int mlx5_txq_ibv_obj_new(struct rte_eth_dev *dev, uint16_t idx) {
  ...
  if (mlx5_getenv_int("MLX5_ENABLE_CQE_COMPRESSION")) {
    DRV_LOG(ERR,
            "Port %u MLX5_ENABLE_CQE_COMPRESSION "
            "must never be set.",
            dev->data->port_id);
    rte_errno = EINVAL;
    return -rte_errno;
  }
  cqe_n = desc / MLX5_TX_COMP_THRESH + 1 + MLX5_TX_COMP_THRESH_INLINE_DIV;
  txq_obj->cq = mlx5_glue->create_cq(priv->sh->cdev->ctx, cqe_n, NULL, NULL, 0);
  if (txq_obj->cq == NULL) {
  ...
  txq_data->wait_on_time = !!(
      !priv->sh->config.tx_pp && priv->sh->cdev->config.hca_attr.wait_on_time &&
   ...
   */
  if (priv->sh->cdev->config.devx && !priv->sh->tdn) {
    ret = mlx5_devx_cmd_qp_query_tis_td(txq_obj->qp, qp.tisn, &priv->sh->tdn);
    if (ret) {
      DRV_LOG(ERR,
      ...
      DRV_LOG(DEBUG,
              ...
              "transport domain %d.",
              dev->data->port_id, idx, qp.tisn, priv->sh->tdn);
    }
  }
#endif
  if (qp.comp_mask & MLX5DV_QP_MASK_UAR_MMAP_OFFSET) {
    ...
    DRV_LOG(ERR, "Port %u failed to retrieve UAR info, invalid libmlx5.so",
            dev->data->port_id);
    rte_errno = EINVAL;
    goto error;
  }
...
int mlx5_rxq_ibv_obj_dummy_lb_create(struct rte_eth_dev *dev) {
  ...
  sh->self_lb.ibv_cq = mlx5_glue->cq_ex_to_cq(
      mlx5_glue->dv_create_cq(ctx, &cq_attr.ibv, &cq_attr.mlx5));
