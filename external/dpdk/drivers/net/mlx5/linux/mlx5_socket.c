 ...
 */
static void mlx5_pmd_socket_handle(void *cb __rte_unused) {
  int conn_sock;
  int ret;
  ...
  if (ret != sizeof(struct mlx5_flow_dump_req)) {
    DRV_LOG(WARNING, "wrong message received: %s", strerror(errno));
    goto error;
  }
  ...
  if (flow_ptr == 0)
    ...
    ret = mlx5_flow_dev_dump(dev, (struct rte_flow *)((uintptr_t)flow_ptr),
                             file, &err);
  ...
  if (ret < 0)
    DRV_LOG(WARNING, "failed to send response %s", strerror(errno));
error:
  if (conn_sock >= 0)
    close(conn_sock);
  if (file)
 ...
 */
static int mlx5_pmd_interrupt_handler_install(void) {
  MLX5_ASSERT(server_socket != -1);

  server_intr_handle = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_PRIVATE);
  ...
  return rte_intr_callback_register(server_intr_handle, mlx5_pmd_socket_handle,
                                    NULL);
}
 ...
 */
static void mlx5_pmd_interrupt_handler_uninstall(void) {
  if (server_socket != -1) {
    mlx5_intr_callback_unregister(server_intr_handle, mlx5_pmd_socket_handle,
                                  NULL);
  }
...
int mlx5_pmd_socket_init(void) {
  ...
  if (ret < 0) {
    DRV_LOG(WARNING, "Failed to open mlx5 socket: %s", strerror(errno));
    goto error;
  }
  server_socket = ret;
  ...
  if (ret < 0)
    goto close;
  snprintf(sun.sun_path, sizeof(sun.sun_path), MLX5_SOCKET_PATH, getpid());
  remove(sun.sun_path);
  ret = bind(server_socket, (const struct sockaddr *)&sun, sizeof(sun));
  if (ret < 0) {
    DRV_LOG(WARNING, "cannot bind mlx5 socket: %s", strerror(errno));
    goto remove;
  }
  ret = listen(server_socket, 0);
  if (ret < 0) {
    DRV_LOG(WARNING, "cannot listen on mlx5 socket: %s", strerror(errno));
    goto remove;
  }
