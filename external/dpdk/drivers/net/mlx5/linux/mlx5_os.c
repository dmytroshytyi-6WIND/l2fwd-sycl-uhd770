...
int mlx5_os_capabilities_prepare(struct mlx5_dev_ctx_shared *sh) {
...
#else
  DRV_LOG(WARNING, "MPLS over GRE/UDP tunnel offloading disabled due to old "
                   "OFED/rdma-core version or firmware configuration");
#endif
#if defined(HAVE_IBV_WQ_FLAG_RX_END_PADDING)
  sh->dev_cap.hw_padding = !!attr_ex.rx_pad_end_addr_align;
#elif defined(HAVE_IBV_WQ_FLAGS_PCI_WRITE_END_PADDING)
  sh->dev_cap.hw_padding =
      !!(attr_ex.device_cap_flags_ex & IBV_DEVICE_PCI_WRITE_END_PADDING);
#endif
  sh->dev_cap.hw_csum =
      !!(attr_ex.device_cap_flags_ex & IBV_DEVICE_RAW_IP_CSUM);
  DRV_LOG(DEBUG, "Checksum offloading is %ssupported.",
          sh->dev_cap.hw_csum ? "" : "not ");
  sh->dev_cap.hw_vlan_strip =
      !!(attr_ex.raw_packet_caps & IBV_RAW_PACKET_CAP_CVLAN_STRIPPING);
  DRV_LOG(DEBUG, "VLAN stripping is %ssupported.",
          (sh->dev_cap.hw_vlan_strip ? "" : "not "));
  sh->dev_cap.hw_fcs_strip =
      !!(attr_ex.raw_packet_caps & IBV_RAW_PACKET_CAP_SCATTER_FCS);
  ...
  if (dv_attr.comp_mask & MLX5DV_CONTEXT_MASK_STRIDING_RQ) {
    struct mlx5dv_striding_rq_caps *strd_rq_caps = &dv_attr.striding_rq_caps;
    ...
    sh->dev_cap.mprq.log_min_stride_wqe_size =
        cdev->config.devx ? hca_attr->log_min_stride_wqe_sz
    ...
    DRV_LOG(DEBUG, "\tmin_stride_wqe_log_size: %u",
            sh->dev_cap.mprq.log_min_stride_wqe_size);
    DRV_LOG(DEBUG, "\tsupported_qpts: %d", strd_rq_caps->supported_qpts);
  ...
  if (dv_attr.comp_mask & MLX5DV_CONTEXT_MASK_TUNNEL_OFFLOADS) {
    sh->dev_cap.tunnel_en =
        dv_attr.tunnel_offloads_caps &
        (MLX5_TUNNELED_OFFLOADS_VXLAN_CAP | MLX5_TUNNELED_OFFLOADS_GRE_CAP |
         MLX5_TUNNELED_OFFLOADS_GENEVE_CAP);
  }
  if (sh->dev_cap.tunnel_en) {
    DRV_LOG(
        DEBUG, "Tunnel offloading is supported for %s%s%s",
        sh->dev_cap.tunnel_en & MLX5_TUNNELED_OFFLOADS_VXLAN_CAP ? "[VXLAN]"
                                                                 : "",
        sh->dev_cap.tunnel_en & MLX5_TUNNELED_OFFLOADS_GRE_CAP ? "[GRE]" : "",
        sh->dev_cap.tunnel_en & MLX5_TUNNELED_OFFLOADS_GENEVE_CAP ? "[GENEVE]"
                                                                  : "");
  } else {
  ...
  DRV_LOG(DEBUG, "Timestamp counter frequency %u kHz.", hca_attr->dev_freq_khz);
  ...
  if (!hca_attr->cross_channel) {
    DRV_LOG(DEBUG, "Cross channel operations are required for packet pacing.");
    sh->dev_cap.txpp_en = 0;
  }
  if (!hca_attr->wqe_index_ignore) {
    DRV_LOG(DEBUG, "WQE index ignore feature is required for packet pacing.");
    sh->dev_cap.txpp_en = 0;
  }
  if (!hca_attr->non_wire_sq) {
    DRV_LOG(DEBUG, "Non-wire SQ feature is required for packet pacing.");
    sh->dev_cap.txpp_en = 0;
  }
  if (!hca_attr->log_max_static_sq_wq) {
    DRV_LOG(DEBUG, "Static WQE SQ feature is required for packet pacing.");
    sh->dev_cap.txpp_en = 0;
  }
  if (!hca_attr->qos.wqe_rate_pp) {
    DRV_LOG(DEBUG, "WQE rate mode is required for packet pacing.");
    sh->dev_cap.txpp_en = 0;
  }
#ifndef HAVE_MLX5DV_DEVX_UAR_OFFSET
  DRV_LOG(DEBUG, "DevX does not provide UAR offset, can't create queues for "
                 "packet pacing.");
  sh->dev_cap.txpp_en = 0;
#endif
...
static void __mlx5_discovery_misc5_cap(struct mlx5_priv *priv) {
  ...
  misc5_m = MLX5_ADDR_OF(fte_match_param, match_m, misc_parameters_5);
  tunnel_header_m =
  ...
  *tunnel_header_m = 0xffffff;
  ...
  dv_attr.match_criteria_enable = (1 << MLX5_MATCH_CRITERIA_ENABLE_OUTER_BIT) |
  ...
  if (priv->sh->config.dv_esw_en) {
    ...
    misc2_m = MLX5_ADDR_OF(fte_match_param, match_m, misc_parameters_2);
    MLX5_SET(fte_match_set_misc2, misc2_m, metadata_reg_c_0, 0xffff);
  }
#endif
  matcher =
      mlx5_glue->dv_create_flow_matcher(priv->sh->cdev->ctx, &dv_attr, tbl);
  if (matcher) {
    priv->sh->misc5_cap = 1;
 ...
 */
static int mlx5_alloc_shared_dr(struct mlx5_priv *priv) {
  ...
  sh->port_id_action_list =
      mlx5_list_create(s, sh, true, flow_dv_port_id_create_cb,
                       flow_dv_port_id_match_cb, flow_dv_port_id_remove_cb,
  ...
  sh->push_vlan_action_list = mlx5_list_create(
      s, sh, true, flow_dv_push_vlan_create_cb, flow_dv_push_vlan_match_cb,
      flow_dv_push_vlan_remove_cb, flow_dv_push_vlan_clone_cb,
  ...
  sh->sample_action_list =
      mlx5_list_create(s, sh, true, flow_dv_sample_create_cb,
                       flow_dv_sample_match_cb, flow_dv_sample_remove_cb,
                       flow_dv_sample_clone_cb, flow_dv_sample_clone_free_cb);
  if (!sh->sample_action_list)
    goto error;
  /* Init dest array action list. */
  snprintf(s, sizeof(s), "%s_dest_array_list", sh->ibdev_name);
  sh->dest_array_list = mlx5_list_create(
      s, sh, true, flow_dv_dest_array_create_cb, flow_dv_dest_array_match_cb,
      flow_dv_dest_array_remove_cb, flow_dv_dest_array_clone_cb,
  ...
  sh->flex_parsers_dv = mlx5_list_create(
      s, sh, false, mlx5_flex_parser_create_cb, mlx5_flex_parser_match_cb,
      mlx5_flex_parser_remove_cb, mlx5_flex_parser_clone_cb,
  ...
  domain =
      mlx5_glue->dr_create_domain(sh->cdev->ctx, MLX5DV_DR_DOMAIN_TYPE_NIC_RX);
  if (!domain) {
  ...
  sh->rx_domain = domain;
  domain =
      mlx5_glue->dr_create_domain(sh->cdev->ctx, MLX5DV_DR_DOMAIN_TYPE_NIC_TX);
  if (!domain) {
  ...
  if (sh->config.dv_esw_en) {
    domain =
        mlx5_glue->dr_create_domain(sh->cdev->ctx, MLX5DV_DR_DOMAIN_TYPE_FDB);
    if (!domain) {
  ...
  if (!sh->config.allow_duplicate_pattern) {
    ...
    DRV_LOG(WARNING, "Disallow duplicate pattern is not supported - maybe old "
                     "rdma-core version?");
#endif
  ...
  sh->default_miss_action = mlx5_glue->dr_create_flow_action_default_miss();
...
#endif /* HAVE_MLX5DV_DR */
  if (sh->default_miss_action)
    mlx5_glue->destroy_flow_action(sh->default_miss_action);
  if (sh->encaps_decaps) {
    mlx5_hlist_destroy(sh->encaps_decaps);
    sh->encaps_decaps = NULL;
  }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_init_shared_data(void) {
  ...
  if (mlx5_shared_data == NULL) {
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
      ...
      mz = rte_memzone_reserve(MZ_MLX5_PMD_SHARED_DATA,
                               sizeof(*mlx5_shared_data), SOCKET_ID_ANY, 0);
      if (mz == NULL) {
        DRV_LOG(ERR, "Cannot allocate mlx5 shared data");
        ret = -rte_errno;
        goto error;
      }
      ...
      if (mz == NULL) {
        DRV_LOG(ERR, "Cannot attach mlx5 shared data");
        ret = -rte_errno;
        goto error;
      }
      mlx5_shared_data = mz->addr;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_init_once(void) {
  ...
  case RTE_PROC_PRIMARY:
    if (sd->init_done)
      break;
    ret = mlx5_mp_init_primary(MLX5_MP_NAME, mlx5_mp_os_primary_handle);
    if (ret)
      goto out;
    sd->init_done = true;
    break;
  case RTE_PROC_SECONDARY:
    if (ld->init_done)
      break;
    ret = mlx5_mp_init_secondary(MLX5_MP_NAME, mlx5_mp_os_secondary_handle);
    if (ret)
      goto out;
    ++sd->secondary_cnt;
    ld->init_done = true;
 ...
 *
 */
static void mlx5_flow_drop_action_config(struct rte_eth_dev *dev __rte_unused) {
#ifdef HAVE_MLX5DV_DR
...
static void mlx5_queue_counter_id_prepare(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (!priv->q_counters) {
    ...
    if (cq) {
      ...
      if (wq) {
        ...
        int ret = mlx5_glue->modify_wq(wq, &(struct ibv_wq_attr){
                                               .attr_mask = IBV_WQ_ATTR_STATE,
                                               .wq_state = IBV_WQS_RDY,
                                           });

        if (ret == 0)
          mlx5_devx_cmd_wq_query(wq, &priv->counter_set_id);
  ...
  if (priv->counter_set_id == 0)
    DRV_LOG(INFO,
            "Part of the port %d statistics will not be "
            "available.",
            dev->data->port_id);
}
 ...
 *   Match result.
 */
static bool mlx5_representor_match(struct mlx5_dev_spawn_data *spawn,
                                   struct rte_eth_devargs *eth_da) {
  struct mlx5_switch_info *switch_info = &spawn->info;
  unsigned int p, f;
  uint16_t id;
  uint16_t repr_id = mlx5_representor_id_encode(switch_info, eth_da->type);

  switch (eth_da->type) {
  case RTE_ETH_REPRESENTOR_SF:
    if (!(spawn->info.port_name == -1 &&
          switch_info->name_type == MLX5_PHYS_PORT_NAME_TYPE_PFHPF) &&
  ...
  for (p = 0; p < eth_da->nb_ports; ++p) {
    if (spawn->pf_bond < 0) {
      ...
      repr_id = mlx5_representor_id_encode(switch_info, eth_da->type);
    }
    for (f = 0; f < eth_da->nb_representor_ports; ++f) {
      id = MLX5_REPRESENTOR_ID(eth_da->ports[p], eth_da->type,
...
static struct rte_eth_dev *mlx5_dev_spawn(struct rte_device *dpdk_dev,
  ...
  if (spawn->pf_bond < 0) {
    ...
    if (!switch_info->representor)
      ...
      err = snprintf(
          name, sizeof(name), "%s_representor_%s%u", dpdk_dev->name,
          switch_info->name_type == MLX5_PHYS_PORT_NAME_TYPE_PFSF ? "sf" : "vf",
          switch_info->port_name);
  } else {
    /* Bonding device. */
    if (!switch_info->representor) {
      err = snprintf(name, sizeof(name), "%s_%s", dpdk_dev->name,
                     spawn->phys_dev_name);
    } else {
      err = snprintf(
          name, sizeof(name), "%s_%s_representor_c%dpf%d%s%u", dpdk_dev->name,
          spawn->phys_dev_name, switch_info->ctrl_num, switch_info->pf_num,
          switch_info->name_type == MLX5_PHYS_PORT_NAME_TYPE_PFSF ? "sf" : "vf",
          switch_info->port_name);
    }
  }
  ...
  if (spawn->phys_port <= UINT8_MAX) {
    ...
    err = mlx5_glue->query_port(sh->cdev->ctx, spawn->phys_port, &port_attr);
    if (err) {
    ...
    err = mlx5_nl_port_state(nl_rdma, spawn->phys_dev_name, spawn->phys_port);
    if (err < 0) {
  ...
  if (port_attr.state != IBV_PORT_ACTIVE)
    DRV_LOG(INFO, "port is not active: \"%s\" (%d)",
            mlx5_glue->port_state_str(port_attr.state), port_attr.state);
  /* Allocate private eth device data. */
  priv = mlx5_malloc(MLX5_MEM_ZERO | MLX5_MEM_RTE, sizeof(*priv),
  ...
  if (mlx5_imported_pd_and_ctx(sh->cdev) && mlx5_devx_obj_ops_en(sh)) {
    priv->ext_rxqs =
        mlx5_malloc(MLX5_MEM_ZERO | MLX5_MEM_RTE,
                    sizeof(struct mlx5_external_rxq) * MLX5_MAX_EXT_RX_QUEUES,
                    0, SOCKET_ID_ANY);
  ...
  DRV_LOG(DEBUG,
          ...
          priv->pci_dev ? priv->pci_dev->name : "NONE", priv->master,
          priv->representor, priv->pf_bond);
   ...
   */
  if (sh->esw_mode) {
    err = mlx5_glue->devx_port_query(sh->cdev->ctx, spawn->phys_port,
                                     &vport_info);
    if (err) {
      DRV_LOG(WARNING, "Cannot query devx port %d on device %s",
  ...
  if (vport_info.query_flags & MLX5_PORT_QUERY_REG_C0) {
    ...
    if (!priv->vport_meta_mask) {
      DRV_LOG(ERR, "vport zero mask for port %d on bonding device %s",
              spawn->phys_port, spawn->phys_dev_name);
      err = ENOTSUP;
      goto error;
    }
    if (priv->vport_meta_tag & ~priv->vport_meta_mask) {
      DRV_LOG(ERR, "Invalid vport tag for port %d on bonding device %s",
              spawn->phys_port, spawn->phys_dev_name);
      err = ENOTSUP;
      goto error;
    }
  ...
  if (vport_info.query_flags & MLX5_PORT_QUERY_VPORT) {
    ...
    DRV_LOG(ERR, "Cannot deduce vport index for port %d on bonding device %s",
            spawn->phys_port, spawn->phys_dev_name);
    err = ENOTSUP;
    goto error;
  } else {
     ...
     */
    priv->vport_id = switch_info->representor ? switch_info->port_name + 1 : -1;
  }
  ...
  MLX5_ETH_FOREACH_DEV(port_id, dpdk_dev) {
    const struct mlx5_priv *opriv = rte_eth_devices[port_id].data->dev_private;

    if (!opriv || opriv->sh != priv->sh ||
        opriv->domain_id == RTE_ETH_DEV_SWITCH_DOMAIN_ID_INVALID)
      continue;
  ...
  if (priv->domain_id == RTE_ETH_DEV_SWITCH_DOMAIN_ID_INVALID) {
    ...
    if (err) {
      err = rte_errno;
      DRV_LOG(ERR, "unable to allocate switch domain: %s", strerror(rte_errno));
      goto error;
    }
    own_domain_id = 1;
    ...
    if (hca_attr->qos.sup && hca_attr->qos.flow_meter_old &&
      ...
      if (__builtin_popcount(reg_c_mask) < 1) {
        ...
        if (reg_c_mask & (1 << (REG_C_3 - REG_C_0)))
          ...
          priv->mtr_color_reg = ffs(reg_c_mask) - 1 + REG_C_0;
        priv->mtr_en = 1;
        priv->mtr_reg_share = hca_attr->qos.flow_meter;
        DRV_LOG(DEBUG, "The REG_C meter uses is %d", priv->mtr_color_reg);
      }
    }
    if (hca_attr->qos.sup && hca_attr->qos.flow_meter_aso_sup) {
      uint32_t log_obj_size = rte_log2_u32(MLX5_ASO_MTRS_PER_POOL >> 1);
  ...
  if (err) {
    err = rte_errno;
    DRV_LOG(ERR, "Failed to process port configure: %s", strerror(rte_errno));
    goto error;
  }
  ...
  if (priv->representor) {
    ...
    MLX5_ETH_FOREACH_DEV(port_id, dpdk_dev) {
      ...
      if (opriv && opriv->master && opriv->domain_id == priv->domain_id &&
          opriv->sh == priv->sh) {
  ...
  DRV_LOG(INFO, "port %u MAC address is " RTE_ETHER_ADDR_PRT_FMT,
...
#ifdef RTE_LIBRTE_MLX5_DEBUG
  {
    ...
    if (mlx5_get_ifname(eth_dev, &ifname) == 0)
      DRV_LOG(DEBUG, "port %u ifname is \"%s\"", eth_dev->data->port_id,
              ifname);
    else
      DRV_LOG(DEBUG, "port %u ifname is unknown", eth_dev->data->port_id);
  }
#endif
  ...
  DRV_LOG(DEBUG, "port %u MTU is %u", eth_dev->data->port_id, priv->mtu);
  ...
  if (sh->dev_cap.vf && sh->config.vf_nl_en)
    mlx5_nl_mac_addr_sync(priv->nl_socket_route, mlx5_ifindex(eth_dev),
                          eth_dev->data->mac_addrs, MLX5_MAX_MAC_ADDRESSES);
  priv->ctrl_flows = 0;
  ...
  if (mlx5_devx_obj_ops_en(sh)) {
    ...
    priv->obj_ops.lb_dummy_queue_create = mlx5_rxq_ibv_obj_dummy_lb_create;
    priv->obj_ops.lb_dummy_queue_release = mlx5_rxq_ibv_obj_dummy_lb_release;
  ...
  if (sh->config.tx_pp && priv->obj_ops.txq_obj_new != mlx5_txq_devx_obj_new) {
    /*
     ...
     */
    DRV_LOG(ERR,
            "Verbs SQs, UAR can't be shared as required for packet pacing");
    err = ENODEV;
    goto error;
  }
  ...
  if (!priv->drop_queue.hrxq)
    goto error;
  priv->hrxqs = mlx5_list_create("hrxq", eth_dev, true, mlx5_hrxq_create_cb,
                                 mlx5_hrxq_match_cb, mlx5_hrxq_remove_cb,
                                 mlx5_hrxq_clone_cb, mlx5_hrxq_clone_free_cb);
  if (!priv->hrxqs)
    goto error;
  ...
  if (sh->config.dv_xmeta_en != MLX5_XMETA_MODE_LEGACY &&
    ...
    DRV_LOG(ERR,
            "metadata mode %u is not supported "
  ...
  if (!mlx5_flow_ext_mreg_supported(eth_dev)) {
    DRV_LOG(DEBUG, "port %u extensive metadata register is not supported",
            eth_dev->data->port_id);
    if (sh->config.dv_xmeta_en != MLX5_XMETA_MODE_LEGACY) {
      DRV_LOG(ERR,
              "metadata mode %u is not supported "
  ...
  if (sh->config.dv_flow_en &&
      ...
      mlx5_flow_ext_mreg_supported(eth_dev) && priv->sh->dv_regc0_mask) {
    priv->mreg_cp_tbl = mlx5_hlist_create(
        MLX5_FLOW_MREG_HNAME, MLX5_FLOW_MREG_HTABLE_SZ, false, true, eth_dev,
        flow_dv_mreg_create_cb, flow_dv_mreg_match_cb, flow_dv_mreg_remove_cb,
 ...
 */
static int mlx5_dev_spawn_data_cmp(const void *a, const void *b) {
 ...
 */
static int mlx5_device_bond_pci_match(const char *ibdev_name,
  ...
  for (i = 1; i <= np; ++i) {
    ...
    MKSTR(slaves, "/sys/class/net/%s/master/bonding/slaves", ifname);
  ...
  while (fscanf(bond_file, "%" RTE_STR(IF_NAMESIZE) "s", ifname) == 1) {
    ...
    snprintf(tmp_str, sizeof(tmp_str), "/sys/class/net/%s", ifname);
    if (mlx5_get_pci_addr(tmp_str, &pci_addr)) {
      DRV_LOG(WARNING, "Cannot get PCI address for netdev \"%s\".", ifname);
      continue;
    }
    /* Slave interface PCI address match found. */
    snprintf(tmp_str, sizeof(tmp_str), "/sys/class/net/%s/phys_port_name",
             ifname);
    ...
    if (info.port_name >= MLX5_BOND_MAX_PORTS) {
      DRV_LOG(WARNING,
              "bonding index out of range, "
              "please increase MLX5_BOND_MAX_PORTS: %s",
              tmp_str);
      break;
    }
    /* Get ifindex. */
    snprintf(tmp_str, sizeof(tmp_str), "/sys/class/net/%s/ifindex", ifname);
    file = fopen(tmp_str, "rb");
    if (!file)
      break;
    ...
    if (ret < 0)
      ...
      if (!memcmp(guid, cur_guid, sizeof(guid)) && owner == info.port_name &&
          (owner != 0 || (owner == 0 && !rte_pci_addr_cmp(pci_dev, &pci_addr))))
        pf = info.port_name;
               ...
               ((pci_dev->function == 0 &&
                ...
                (pci_dev->function == owner && pci_addr.function == owner)))
      pf = info.port_name;
  }
  if (pf >= 0) {
    /* Get bond interface info */
    ret = mlx5_sysfs_bond_info(ifindex, &bond_info->ifindex, bond_info->ifname);
    if (ret)
      DRV_LOG(ERR, "unable to get bond info: %s", strerror(rte_errno));
    else
      DRV_LOG(INFO, "PF device %u, bond device %u(%s)", ifindex,
              bond_info->ifindex, bond_info->ifname);
  }
  if (owner == 0 && pf != 0) {
    DRV_LOG(INFO, "PCIe instance %04x:%02x:%02x.%x isn't bonding owner",
            pci_dev->domain, pci_dev->bus, pci_dev->devid, pci_dev->function);
  }
  return pf;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_os_pci_probe_pf(struct mlx5_common_device *cdev,
  ...
  while (ret-- > 0) {
    ...
    bd = mlx5_device_bond_pci_match(ibv_list[ret]->name, &owner_pci, nl_rdma,
                                    owner_id, &bond_info);
    if (bd >= 0) {
      /*
      ...
      DRV_LOG(INFO,
              "PCI information matches for slave %d bonding device \"%s\"", bd,
              ibv_list[ret]->name);
      ...
      if (owner_pci.domain != pci_addr.domain ||
          owner_pci.bus != pci_addr.bus || owner_pci.devid != pci_addr.devid ||
  ...
  if (!nd) {
    ...
    DRV_LOG(WARNING,
            ...
            owner_pci.domain, owner_pci.bus, owner_pci.devid,
            owner_pci.function);
    rte_errno = ENOENT;
    ret = -rte_errno;
    goto exit;
  ...
  if (nd == 1) {
    ...
    if (!np)
      DRV_LOG(WARNING, "Cannot get IB device \"%s\" ports number.",
              ibv_match[0]->name);
    if (bd >= 0 && !np) {
  ...
  if (bd >= 0 || np > 1) {
    ...
    for (i = 1; i <= np; ++i) {
      ...
      list[ns].cdev = cdev;
      list[ns].pf_bond = bd;
      list[ns].ifindex = mlx5_nl_ifindex(nl_rdma, ibv_match[0]->name, i);
      if (!list[ns].ifindex) {
        /*
      ...
      ret = -1;
      if (nl_route >= 0)
        ret = mlx5_nl_switch_info(nl_route, list[ns].ifindex, &list[ns].info);
      if (ret || (!list[ns].info.representor && !list[ns].info.master)) {
        /*
         * We failed to recognize representors with
         * Netlink, let's try to perform the task
         * with sysfs.
         */
        ret = mlx5_sysfs_switch_info(list[ns].ifindex, &list[ns].info);
      }
      if (!ret && bd >= 0) {
    ...
    if (!ns) {
      DRV_LOG(ERR, "Unable to recognize master/representors on the IB device "
                   "with multiple ports.");
      rte_errno = ENOENT;
      ret = -rte_errno;
      goto exit;
    ...
    for (i = 0; i != nd; ++i) {
      ...
      list[ns].ifindex = 0;
      if (nl_rdma >= 0)
        list[ns].ifindex = mlx5_nl_ifindex(nl_rdma, ibv_match[i]->name, 1);
      if (!list[ns].ifindex) {
        ...
        ret = mlx5_get_ifname_sysfs(ibv_match[i]->ibdev_path, ifname);
        if (!ret)
          list[ns].ifindex = if_nametoindex(ifname);
        if (!list[ns].ifindex) {
          /*
      ...
      if (!ret && (list[ns].info.representor ^ list[ns].info.master)) {
        ns++;
      } else if ((nd == 1) && !list[ns].info.representor &&
                 !list[ns].info.master) {
        /*
    ...
    if (!ns) {
      DRV_LOG(ERR, "Unable to recognize master/representors on the multiple IB "
                   "devices.");
      rte_errno = ENOENT;
      ret = -rte_errno;
      goto exit;
     ...
     */
    if (nd == 1 && list[0].info.name_type == MLX5_PHYS_PORT_NAME_TYPE_UPLINK) {
  ...
  if (eth_da.type != RTE_ETH_REPRESENTOR_NONE) {
    ...
    if (eth_da.nb_ports == 0 && ns > 0) {
      if (list[0].pf_bond >= 0 && list[0].info.representor)
        DRV_LOG(WARNING,
                "Representor on Bonding device should use pf#vf# syntax: %s",
  ...
  for (i = 0; i != ns; ++i) {
    ...
    list[i].eth_dev = mlx5_dev_spawn(cdev->dev, &list[i], &eth_da, mkvlist);
    if (!list[i].eth_dev) {
  ...
  if (i != ns) {
    DRV_LOG(ERR,
            ...
            owner_pci.domain, owner_pci.bus, owner_pci.devid,
            owner_pci.function, strerror(rte_errno));
    ret = -rte_errno;
    /* Roll back. */
    while (i--) {
      if (!list[i].eth_dev)
...
static int mlx5_os_parse_eth_devargs(struct rte_device *dev,
                                     struct rte_eth_devargs *eth_da) {
  int ret = 0;
  ...
  if (ret != 0) {
    DRV_LOG(ERR, "failed to parse device arguments: %s", dev->devargs->cls_str);
    return -rte_errno;
  }
  if (eth_da->type == RTE_ETH_REPRESENTOR_NONE) {
    ...
    if (ret) {
      DRV_LOG(ERR, "failed to parse device arguments: %s", dev->devargs->args);
      return -rte_errno;
    }
  }
  return 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_os_pci_probe(struct mlx5_common_device *cdev,
                             struct mlx5_kvargs_ctrl *mkvlist) {
  struct rte_pci_device *pci_dev = RTE_DEV_TO_PCI(cdev->dev);
  ...
  if (eth_da.nb_ports > 0) {
    ...
    for (p = 0; p < eth_da.nb_ports; p++) {
      ret = mlx5_os_pci_probe_pf(cdev, &eth_da, eth_da.ports[p], mkvlist);
      if (ret)
        break;
    }
    if (ret) {
      DRV_LOG(ERR,
              "Probe of PCI device " PCI_PRI_FMT " "
              "aborted due to prodding failure of PF %u",
              pci_dev->addr.domain, pci_dev->addr.bus, pci_dev->addr.devid,
              pci_dev->addr.function, eth_da.ports[p]);
...
static int mlx5_os_auxiliary_probe(struct mlx5_common_device *cdev,
                                   struct mlx5_kvargs_ctrl *mkvlist) {
  struct rte_eth_devargs eth_da = {.nb_ports = 0};
...
int mlx5_os_net_probe(struct mlx5_common_device *cdev,
  ...
  ret = mlx5_init_once();
  if (ret) {
    DRV_LOG(ERR, "Unable to init PMD global data: %s", strerror(rte_errno));
    return -rte_errno;
  }
 ...
 */
void mlx5_os_net_cleanup(void) { mlx5_pmd_socket_uninit(); }
...
mlx5_os_dev_shared_handler_install_lsc(struct mlx5_dev_ctx_shared *sh) {
  ...
  if (rte_intr_callback_register(sh->intr_handle_nl,
                                 mlx5_dev_interrupt_handler_nl, sh) != 0) {
...
void mlx5_os_dev_shared_handler_install(struct mlx5_dev_ctx_shared *sh) {
  ...
  if (ret) {
    ...
    if (rte_intr_callback_register(sh->intr_handle, mlx5_dev_interrupt_handler,
                                   sh)) {
      DRV_LOG(INFO, "Fail to install the shared interrupt.");
      rte_intr_fd_set(sh->intr_handle, -1);
    }
  }
  sh->intr_handle_nl = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_SHARED);
  ...
  if (sh->cdev->config.devx) {
    ...
    sh->intr_handle_devx = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_SHARED);
    ...
    rte_intr_type_set(sh->intr_handle_devx, RTE_INTR_HANDLE_EXT);
...
void mlx5_os_dev_shared_handler_uninstall(struct mlx5_dev_ctx_shared *sh) {
  ...
  if (rte_intr_fd_get(sh->intr_handle) >= 0)
    mlx5_intr_callback_unregister(sh->intr_handle, mlx5_dev_interrupt_handler,
                                  sh);
 ...
 *
 */
int mlx5_os_read_dev_stat(struct mlx5_priv *priv, const char *ctr_name,
                          uint64_t *stat) {
  int fd;

  if (priv->sh) {
    if (priv->q_counters != NULL && strcmp(ctr_name, "out_of_buffer") == 0)
      return mlx5_devx_cmd_queue_counter_query(priv->q_counters, 0,
                                               (uint32_t *)stat);
    MKSTR(path, "%s/ports/%d/hw_counters/%s", priv->sh->ibdev_path,
          priv->dev_port, ctr_name);
     ...
     */
    if (fd == -1) {
      MKSTR(path1, "%s/hw_counters/%s", priv->sh->ibdev_path, ctr_name);
...
void mlx5_os_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
  ...
  if (vf)
    mlx5_nl_mac_addr_remove(priv->nl_socket_route, mlx5_ifindex(dev),
                            priv->mac_own, &dev->data->mac_addrs[index], index);
}
...
int mlx5_os_mac_addr_add(struct rte_eth_dev *dev, struct rte_ether_addr *mac,
  ...
  if (vf)
    ret = mlx5_nl_mac_addr_add(priv->nl_socket_route, mlx5_ifindex(dev),
                               priv->mac_own, mac, index);
  return ret;
}
...
int mlx5_os_vf_mac_addr_modify(struct mlx5_priv *priv, unsigned int iface_idx,
                               struct rte_ether_addr *mac_addr, int vf_index) {
  return mlx5_nl_vf_mac_addr_modify(priv->nl_socket_route, iface_idx, mac_addr,
                                    vf_index);
}
...
int mlx5_os_set_promisc(struct rte_eth_dev *dev, int enable) {
  ...
  return mlx5_nl_promisc(priv->nl_socket_route, mlx5_ifindex(dev), !!enable);
}
...
int mlx5_os_set_allmulti(struct rte_eth_dev *dev, int enable) {
  ...
  return mlx5_nl_allmulti(priv->nl_socket_route, mlx5_ifindex(dev), !!enable);
}
...
void mlx5_os_mac_addr_flush(struct rte_eth_dev *dev) {
  ...
  mlx5_nl_mac_addr_flush(priv->nl_socket_route, mlx5_ifindex(dev),
                         dev->data->mac_addrs, MLX5_MAX_MAC_ADDRESSES,
                         priv->mac_own);
}
