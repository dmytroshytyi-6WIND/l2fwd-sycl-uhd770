...
static int flow_dv_tbl_resource_release(struct mlx5_dev_ctx_shared *sh,
                                        struct mlx5_flow_tbl_resource *tbl);

static int flow_dv_encap_decap_resource_release(struct rte_eth_dev *dev,
                                                uint32_t encap_decap_idx);

static int flow_dv_port_id_action_resource_release(struct rte_eth_dev *dev,
                                                   uint32_t port_id);
static void flow_dv_shared_rss_action_release(struct rte_eth_dev *dev,
                                              uint32_t srss);

static int flow_dv_jump_tbl_resource_release(struct rte_eth_dev *dev,
                                             uint32_t rix_jump);

static int16_t flow_dv_get_esw_manager_vport_id(struct rte_eth_dev *dev) {
 ...
 */
static void flow_dv_attr_init(const struct rte_flow_item *item,
  ...
  for (; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    ...
    case RTE_FLOW_ITEM_TYPE_IPV4:
      if (!attr->ipv6)
        attr->ipv4 = 1;
      if (item->mask != NULL &&
          ((const struct rte_flow_item_ipv4 *)item->mask)->hdr.next_proto_id)
        next_protocol = ((const struct rte_flow_item_ipv4 *)(item->spec))
                            ->hdr.next_proto_id &
                        ((const struct rte_flow_item_ipv4 *)(item->mask))
                            ->hdr.next_proto_id;
      if ((next_protocol == IPPROTO_IPIP || next_protocol == IPPROTO_IPV6) &&
          tunnel_decap)
        attr->attr = 0;
      break;
    case RTE_FLOW_ITEM_TYPE_IPV6:
      if (!attr->ipv4)
        attr->ipv6 = 1;
      if (item->mask != NULL &&
          ((const struct rte_flow_item_ipv6 *)item->mask)->hdr.proto)
        next_protocol =
            ((const struct rte_flow_item_ipv6 *)(item->spec))->hdr.proto &
            ((const struct rte_flow_item_ipv6 *)(item->mask))->hdr.proto;
      if ((next_protocol == IPPROTO_IPIP || next_protocol == IPPROTO_IPV6) &&
          tunnel_decap)
        attr->attr = 0;
 ...
 *   mlx5 color.
 */
static inline int rte_col_2_mlx5_col(enum rte_color rcol) {
  switch (rcol) {
...
static inline struct mlx5_hlist *flow_dv_hlist_prepare(
    struct mlx5_dev_ctx_shared *sh, struct mlx5_hlist **phl, const char *name,
    uint32_t size, bool direct_key, bool lcores_share, void *ctx,
    mlx5_list_create_cb cb_create, mlx5_list_match_cb cb_match,
    mlx5_list_remove_cb cb_remove, mlx5_list_clone_cb cb_clone,
    mlx5_list_clone_free_cb cb_clone_free, struct rte_flow_error *error) {
  ...
  hl = mlx5_hlist_create(s, size, direct_key, lcores_share, ctx, cb_create,
                         cb_match, cb_remove, cb_clone, cb_clone_free);
  if (!hl) {
  ...
  if (!__atomic_compare_exchange_n(phl, &expected, hl, false, __ATOMIC_SEQ_CST,
                                   __ATOMIC_SEQ_CST)) {
 ...
 */
static void mlx5_update_vlan_vid_pcp(const struct rte_flow_action *action,
                                     struct rte_vlan_hdr *vlan) {
  uint16_t vlan_tci;
  if (action->type == RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_PCP) {
    vlan_tci = ((const struct rte_flow_action_of_set_vlan_pcp *)action->conf)
                   ->vlan_pcp;
 ...
 */
static inline uint32_t flow_dv_fetch_field(const uint8_t *data, uint32_t size) {
  uint32_t ret;
...
flow_dv_convert_modify_action(struct rte_flow_item *item,
  ...
  do {
    ...
    mask = flow_dv_fetch_field((const uint8_t *)item->mask + field->offset,
                               field->size);
    if (!mask) {
      ++field;
      continue;
    ...
    size_b = sizeof(uint32_t) * CHAR_BIT - off_b - __builtin_clz(mask);
    MLX5_ASSERT(size_b);
    actions[i] = (struct mlx5_modification_cmd){
        .action_type = type,
        .field = field->id,
        .offset = off_b,
        .length = (size_b == sizeof(uint32_t) * CHAR_BIT) ? 0 : size_b,
    };
    if (type == MLX5_MODIFICATION_TYPE_COPY) {
      MLX5_ASSERT(dcopy);
      actions[i].dst_field = dcopy->id;
      actions[i].dst_offset = (int)dcopy->offset < 0 ? off_b : dcopy->offset;
      ...
      if ((size_b > dcopy->size * CHAR_BIT - dcopy->offset) &&
          dcopy->size != 0) {
        actions[i].length = dcopy->size * CHAR_BIT - dcopy->offset;
      ...
      if ((size_b < dcopy->size * CHAR_BIT - dcopy->offset) &&
        ...
        actions[i].length = field->size * CHAR_BIT - off_b;
      ...
      data = flow_dv_fetch_field((const uint8_t *)item->spec + field->offset,
                                 field->size);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_ipv4(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_ipv6(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
  ...
  if (action->type == RTE_FLOW_ACTION_TYPE_SET_IPV6_SRC) {
    memcpy(&ipv6.hdr.src_addr, &conf->ipv6_addr, sizeof(ipv6.hdr.src_addr));
    memcpy(&ipv6_mask.hdr.src_addr, &rte_flow_item_ipv6_mask.hdr.src_addr,
           sizeof(ipv6.hdr.src_addr));
  } else {
    memcpy(&ipv6.hdr.dst_addr, &conf->ipv6_addr, sizeof(ipv6.hdr.dst_addr));
    memcpy(&ipv6_mask.hdr.dst_addr, &rte_flow_item_ipv6_mask.hdr.dst_addr,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_mac(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
  ...
  if (action->type == RTE_FLOW_ACTION_TYPE_SET_MAC_SRC) {
    memcpy(&eth.src.addr_bytes, &conf->mac_addr, sizeof(eth.src.addr_bytes));
    memcpy(&eth_mask.src.addr_bytes, &rte_flow_item_eth_mask.src.addr_bytes,
           sizeof(eth_mask.src.addr_bytes));
  } else {
    memcpy(&eth.dst.addr_bytes, &conf->mac_addr, sizeof(eth.dst.addr_bytes));
    memcpy(&eth_mask.dst.addr_bytes, &rte_flow_item_eth_mask.dst.addr_bytes,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_vlan_vid(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_tp(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, const struct rte_flow_item *items,
    union flow_dv_attr *attr, struct mlx5_flow *dev_flow, bool tunnel_decap,
  ...
  if (attr->udp) {
    ...
    if (action->type == RTE_FLOW_ACTION_TYPE_SET_TP_SRC) {
      ...
      udp_mask.hdr.src_port = rte_flow_item_udp_mask.hdr.src_port;
    } else {
      udp.hdr.dst_port = conf->port;
      udp_mask.hdr.dst_port = rte_flow_item_udp_mask.hdr.dst_port;
    }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_ttl(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, const struct rte_flow_item *items,
    union flow_dv_attr *attr, struct mlx5_flow *dev_flow, bool tunnel_decap,
    struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_dec_ttl(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_item *items, union flow_dv_attr *attr,
    struct mlx5_flow *dev_flow, bool tunnel_decap,
    struct rte_flow_error *error) {
  struct rte_flow_item item;
  struct rte_flow_item_ipv4 ipv4;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_tcp_seq(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_tcp_ack(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
  const rte_be32_t *conf = (const rte_be32_t *)(action->conf);
  uint64_t value = rte_be_to_cpu_32(*conf);
...
static enum mlx5_modification_field reg_to_field[] = {
    [REG_NON] = MLX5_MODI_OUT_NONE,      [REG_A] = MLX5_MODI_META_DATA_REG_A,
    [REG_B] = MLX5_MODI_META_DATA_REG_B, [REG_C_0] = MLX5_MODI_META_REG_C_0,
    [REG_C_1] = MLX5_MODI_META_REG_C_1,  [REG_C_2] = MLX5_MODI_META_REG_C_2,
    [REG_C_3] = MLX5_MODI_META_REG_C_3,  [REG_C_4] = MLX5_MODI_META_REG_C_4,
    [REG_C_5] = MLX5_MODI_META_REG_C_5,  [REG_C_6] = MLX5_MODI_META_REG_C_6,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_set_reg(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_set_tag(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action_set_tag *conf, struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_copy_mreg(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_modify_hdr_resource *res,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
  ...
  if (conf->dst == REG_C_0 || conf->src == REG_C_0) {
    ...
    MLX5_ASSERT(reg_c0);
    MLX5_ASSERT(priv->sh->config.dv_xmeta_en != MLX5_XMETA_MODE_LEGACY);
    if (conf->dst == REG_C_0) {
  ...
  return flow_dv_convert_modify_action(&item, reg_src, &reg_dst, res,
...
flow_dv_convert_action_mark(struct rte_eth_dev *dev,
  ...
  struct mlx5_priv *priv = dev->data->dev_private;
  rte_be32_t mask =
      rte_cpu_to_be_32(MLX5_FLOW_MARK_MASK & priv->sh->dv_mark_mask);
...
flow_dv_get_metadata_reg(struct rte_eth_dev *dev,
  ...
  int reg = mlx5_flow_get_reg_id(dev,
                                 attr->transfer ? MLX5_METADATA_FDB
                                 : attr->egress ? MLX5_METADATA_TX
                                                : MLX5_METADATA_RX,
                                 0, error);
  if (reg < 0)
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_set_meta(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_attr *attr,
    const struct rte_flow_action_set_meta *conf, struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_ipv4_dscp(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_ipv6_dscp(
    struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
  const struct rte_flow_action_set_dscp *conf =
      (const struct rte_flow_action_set_dscp *)(action->conf);
...
static int mlx5_flow_item_field_width(struct rte_eth_dev *dev,
  ...
  case RTE_FLOW_FIELD_META:
    return (flow_dv_get_metadata_reg(dev, attr, error) == REG_C_0)
               ? __builtin_popcount(priv->sh->dv_meta_mask)
               : 32;
...
static void mlx5_flow_field_id_to_modify_info(
    const struct rte_flow_action_modify_data *data,
    struct field_modify_info *info, uint32_t *mask, uint32_t width,
  ...
  case RTE_FLOW_FIELD_MAC_DST:
    ...
    if (mask) {
      if (data->offset < 16) {
        info[idx] = (struct field_modify_info){2, 4, MLX5_MODI_OUT_DMAC_15_0};
        if (width < 16) {
          mask[1] = rte_cpu_to_be_16(0xffff >> (16 - width));
          width = 0;
        } else {
      ...
      info[idx] = (struct field_modify_info){4, 0, MLX5_MODI_OUT_DMAC_47_16};
      mask[0] = rte_cpu_to_be_32((0xffffffff >> (32 - width)) << off);
    } else {
      if (data->offset < 16)
        info[idx++] = (struct field_modify_info){2, 0, MLX5_MODI_OUT_DMAC_15_0};
      info[idx] = (struct field_modify_info){4, off, MLX5_MODI_OUT_DMAC_47_16};
    }
    break;
  case RTE_FLOW_FIELD_MAC_SRC:
    off = data->offset > 16 ? data->offset - 16 : 0;
    if (mask) {
      if (data->offset < 16) {
        info[idx] = (struct field_modify_info){2, 4, MLX5_MODI_OUT_SMAC_15_0};
        if (width < 16) {
          mask[1] = rte_cpu_to_be_16(0xffff >> (16 - width));
          width = 0;
        } else {
      ...
      if (data->offset < 16)
        info[idx++] = (struct field_modify_info){2, 0, MLX5_MODI_OUT_SMAC_15_0};
      info[idx] = (struct field_modify_info){4, off, MLX5_MODI_OUT_SMAC_47_16};
    }
    break;
  case RTE_FLOW_FIELD_VLAN_TYPE:
    /* not supported yet */
    break;
  case RTE_FLOW_FIELD_VLAN_ID:
    info[idx] = (struct field_modify_info){2, 0, MLX5_MODI_OUT_FIRST_VID};
    if (mask)
      mask[idx] = rte_cpu_to_be_16(0x0fff >> (12 - width));
    break;
  case RTE_FLOW_FIELD_MAC_TYPE:
    info[idx] = (struct field_modify_info){2, 0, MLX5_MODI_OUT_ETHERTYPE};
    if (mask)
      mask[idx] = rte_cpu_to_be_16(0xffff >> (16 - width));
    break;
  case RTE_FLOW_FIELD_IPV4_DSCP:
    info[idx] = (struct field_modify_info){1, 0, MLX5_MODI_OUT_IP_DSCP};
    if (mask)
      mask[idx] = 0x3f >> (6 - width);
    break;
  case RTE_FLOW_FIELD_IPV4_TTL:
    info[idx] = (struct field_modify_info){1, 0, MLX5_MODI_OUT_IPV4_TTL};
    if (mask)
      mask[idx] = 0xff >> (8 - width);
    break;
  case RTE_FLOW_FIELD_IPV4_SRC:
    info[idx] = (struct field_modify_info){4, 0, MLX5_MODI_OUT_SIPV4};
    if (mask)
      mask[idx] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
    break;
  case RTE_FLOW_FIELD_IPV4_DST:
    info[idx] = (struct field_modify_info){4, 0, MLX5_MODI_OUT_DIPV4};
    if (mask)
      mask[idx] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
    break;
  case RTE_FLOW_FIELD_IPV6_DSCP:
    ...
    break;
  case RTE_FLOW_FIELD_IPV6_HOPLIMIT:
    info[idx] = (struct field_modify_info){1, 0, MLX5_MODI_OUT_IPV6_HOPLIMIT};
    if (mask)
      mask[idx] = 0xff >> (8 - width);
    break;
  case RTE_FLOW_FIELD_IPV6_SRC:
    if (mask) {
      if (data->offset < 32) {
        info[idx] = (struct field_modify_info){4, 12, MLX5_MODI_OUT_SIPV6_31_0};
        if (width < 32) {
          mask[3] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
          width = 0;
        } else {
      ...
      if (data->offset < 64) {
        info[idx] = (struct field_modify_info){4, 8, MLX5_MODI_OUT_SIPV6_63_32};
        if (width < 32) {
          mask[2] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
          width = 0;
        } else {
      ...
      if (data->offset < 96) {
        info[idx] = (struct field_modify_info){4, 4, MLX5_MODI_OUT_SIPV6_95_64};
        if (width < 32) {
          mask[1] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
          width = 0;
        } else {
      ...
      info[idx] = (struct field_modify_info){4, 0, MLX5_MODI_OUT_SIPV6_127_96};
      mask[0] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
    } else {
      if (data->offset < 32)
        info[idx++] =
            (struct field_modify_info){4, 0, MLX5_MODI_OUT_SIPV6_31_0};
      if (data->offset < 64)
        info[idx++] =
            (struct field_modify_info){4, 0, MLX5_MODI_OUT_SIPV6_63_32};
      if (data->offset < 96)
        info[idx++] =
            (struct field_modify_info){4, 0, MLX5_MODI_OUT_SIPV6_95_64};
      if (data->offset < 128)
        info[idx++] =
            (struct field_modify_info){4, 0, MLX5_MODI_OUT_SIPV6_127_96};
    }
    break;
  case RTE_FLOW_FIELD_IPV6_DST:
    if (mask) {
      if (data->offset < 32) {
        info[idx] = (struct field_modify_info){4, 12, MLX5_MODI_OUT_DIPV6_31_0};
        if (width < 32) {
          mask[3] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
          width = 0;
        } else {
      ...
      if (data->offset < 64) {
        info[idx] = (struct field_modify_info){4, 8, MLX5_MODI_OUT_DIPV6_63_32};
        if (width < 32) {
          mask[2] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
          width = 0;
        } else {
      ...
      if (data->offset < 96) {
        info[idx] = (struct field_modify_info){4, 4, MLX5_MODI_OUT_DIPV6_95_64};
        if (width < 32) {
          mask[1] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
          width = 0;
        } else {
      ...
      info[idx] = (struct field_modify_info){4, 0, MLX5_MODI_OUT_DIPV6_127_96};
      mask[0] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
    } else {
      if (data->offset < 32)
        info[idx++] =
            (struct field_modify_info){4, 0, MLX5_MODI_OUT_DIPV6_31_0};
      if (data->offset < 64)
        info[idx++] =
            (struct field_modify_info){4, 0, MLX5_MODI_OUT_DIPV6_63_32};
      if (data->offset < 96)
        info[idx++] =
            (struct field_modify_info){4, 0, MLX5_MODI_OUT_DIPV6_95_64};
      if (data->offset < 128)
        info[idx++] =
            (struct field_modify_info){4, 0, MLX5_MODI_OUT_DIPV6_127_96};
    }
    break;
  case RTE_FLOW_FIELD_TCP_PORT_SRC:
    ...
    break;
  case RTE_FLOW_FIELD_TCP_PORT_DST:
    info[idx] = (struct field_modify_info){2, 0, MLX5_MODI_OUT_TCP_DPORT};
    if (mask)
      mask[idx] = rte_cpu_to_be_16(0xffff >> (16 - width));
    break;
  case RTE_FLOW_FIELD_TCP_SEQ_NUM:
    info[idx] = (struct field_modify_info){4, 0, MLX5_MODI_OUT_TCP_SEQ_NUM};
    if (mask)
      mask[idx] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
    break;
  case RTE_FLOW_FIELD_TCP_ACK_NUM:
    info[idx] = (struct field_modify_info){4, 0, MLX5_MODI_OUT_TCP_ACK_NUM};
    if (mask)
      mask[idx] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
    break;
  case RTE_FLOW_FIELD_TCP_FLAGS:
    info[idx] = (struct field_modify_info){2, 0, MLX5_MODI_OUT_TCP_FLAGS};
    if (mask)
      mask[idx] = rte_cpu_to_be_16(0x1ff >> (9 - width));
    break;
  case RTE_FLOW_FIELD_UDP_PORT_SRC:
    info[idx] = (struct field_modify_info){2, 0, MLX5_MODI_OUT_UDP_SPORT};
    if (mask)
      mask[idx] = rte_cpu_to_be_16(0xffff >> (16 - width));
    break;
  case RTE_FLOW_FIELD_UDP_PORT_DST:
    info[idx] = (struct field_modify_info){2, 0, MLX5_MODI_OUT_UDP_DPORT};
    if (mask)
      mask[idx] = rte_cpu_to_be_16(0xffff >> (16 - width));
    break;
  case RTE_FLOW_FIELD_VXLAN_VNI:
  ...
  case RTE_FLOW_FIELD_GENEVE_VNI:
    ...
    break;
  case RTE_FLOW_FIELD_GTP_TEID:
    info[idx] = (struct field_modify_info){4, 0, MLX5_MODI_GTP_TEID};
    if (mask)
      mask[idx] = rte_cpu_to_be_32(0xffffffff >> (32 - width));
    break;
  case RTE_FLOW_FIELD_TAG: {
    int reg = mlx5_flow_get_reg_id(dev, MLX5_APP_TAG, data->level, error);
    if (reg < 0)
      return;
    MLX5_ASSERT(reg != REG_NON);
  ...
  case RTE_FLOW_FIELD_MARK: {
    ...
    int reg = mlx5_flow_get_reg_id(dev, MLX5_FLOW_MARK, 0, error);
    if (reg < 0)
      return;
    MLX5_ASSERT(reg != REG_NON);
    MLX5_ASSERT((unsigned int)reg < RTE_DIM(reg_to_field));
  ...
  case RTE_FLOW_FIELD_META: {
    ...
    if (mask)
      mask[idx] =
          rte_cpu_to_be_32((meta_mask >> (meta_count - width)) & meta_mask);
  } break;
  case RTE_FLOW_FIELD_POINTER:
  case RTE_FLOW_FIELD_VALUE:
  default:
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_convert_action_modify_field(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_modify_hdr_resource *resource,
    const struct rte_flow_action *action, const struct rte_flow_attr *attr,
    struct rte_flow_error *error) {
  const struct rte_flow_action_modify_field *conf =
      (const struct rte_flow_action_modify_field *)(action->conf);
  struct rte_flow_item item = {.spec = NULL, .mask = NULL};
  struct field_modify_info field[MLX5_ACT_MAX_MOD_FIELDS] = {{0, 0, 0}};
  struct field_modify_info dcopy[MLX5_ACT_MAX_MOD_FIELDS] = {{0, 0, 0}};
  ...
  if (conf->src.field == RTE_FLOW_FIELD_POINTER ||
    ...
    mlx5_flow_field_id_to_modify_info(&conf->dst, field, mask, conf->width, dev,
                                      attr, error);
    ...
    mlx5_flow_field_id_to_modify_info(&conf->dst, dcopy, NULL, conf->width, dev,
                                      attr, error);
    /** Then construct the source field (field) with mask. */
    mlx5_flow_field_id_to_modify_info(&conf->src, field, mask, conf->width, dev,
                                      attr, error);
  }
  item.mask = &mask;
  return flow_dv_convert_modify_action(&item, field, dcopy, resource, type,
                                       error);
}
...
flow_dv_validate_item_mark(struct rte_eth_dev *dev,
  ...
  if (!spec)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC,
                              item->spec, "data cannot be empty");
  if (spec->id >= (MLX5_FLOW_MARK_MAX & nic_mask.id))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &spec->id, "mark id exceeds the limit");
  if (!mask)
    mask = &nic_mask;
  if (!mask->id)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC,
                              NULL, "mask cannot be zero");

  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_mark), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_item_meta(struct rte_eth_dev *dev __rte_unused,
                                      const struct rte_flow_item *item,
                                      const struct rte_flow_attr *attr,
                                      struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_meta), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  return ret;
}
...
flow_dv_validate_item_tag(struct rte_eth_dev *dev,
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_tag), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  if (mask->index != 0xff)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_item_port_id(struct rte_eth_dev *dev,
                                         const struct rte_flow_item *item,
                                         const struct rte_flow_attr *attr,
                                         uint64_t item_flags,
                                         struct rte_flow_error *error) {
  ...
  if (!attr->transfer)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
  ...
  if (!esw_priv)
    return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ITEM_SPEC,
                              spec,
                              "failed to obtain E-Switch info for"
                              " port");
  dev_priv = mlx5_dev_to_eswitch_info(dev);
  if (!dev_priv)
    return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "failed to obtain E-Switch info");
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_item_vlan(const struct rte_flow_item *item,
                                      uint64_t item_flags,
  ...
  int ret;
  const uint64_t l34m =
      tunnel ? (MLX5_FLOW_LAYER_INNER_L3 | MLX5_FLOW_LAYER_INNER_L4)
             : (MLX5_FLOW_LAYER_OUTER_L3 | MLX5_FLOW_LAYER_OUTER_L4);
  const uint64_t vlanm =
      tunnel ? MLX5_FLOW_LAYER_INNER_VLAN : MLX5_FLOW_LAYER_OUTER_VLAN;
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_vlan), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret)
    return ret;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_item_gtp(struct rte_eth_dev *dev,
                                     const struct rte_flow_item *item,
                                     uint64_t item_flags,
                                     struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  return mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_gtp), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_item_gtp_psc(const struct rte_flow_item *item,
                                         uint64_t last_item,
  ...
  if (gtp_spec && (gtp_mask->v_pt_rsv_flags & ~gtp_spec->v_pt_rsv_flags &
                   MLX5_GTP_EXT_HEADER_FLAG))
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item,
  ...
  if (!attr->transfer && !attr->group)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "GTP PSC is not supported for group 0");
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_item_ipv4(struct rte_eth_dev *dev,
                                      const struct rte_flow_item *item,
  ...
  if (mask && (mask->hdr.version_ihl & RTE_IPV4_HDR_IHL_MASK)) {
    ...
    bool ihl_cap = !tunnel ? attr->outer_ipv4_ihl : attr->inner_ipv4_ihl;
    if (!ihl_cap)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item,
                                "IPV4 ihl offload not supported");
    nic_ipv4_mask.hdr.version_ihl = mask->hdr.version_ihl;
  }
  ret = mlx5_flow_validate_item_ipv4(item, item_flags, last_item, ether_type,
                                     &nic_ipv4_mask, MLX5_ITEM_RANGE_ACCEPTED,
                                     error);
  if (ret < 0)
    return ret;
  if (spec && mask)
    fragment_offset_spec =
        spec->hdr.fragment_offset & mask->hdr.fragment_offset;
  ...
  fragment_offset_last = last->hdr.fragment_offset & mask->hdr.fragment_offset;
  /*
  ...
  if (fragment_offset_spec == RTE_BE16(RTE_IPV4_HDR_MF_FLAG + 1) &&
    ...
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_LAST,
                              last,
                              "match on following "
  ...
  if (!(fragment_offset_spec == RTE_BE16(1) &&
        fragment_offset_last == RTE_BE16(MLX5_IPV4_FRAG_OFFSET_MASK)))
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_LAST,
                              last, "specified range not supported");
  return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_item_ipv6_frag_ext(const struct rte_flow_item *item,
                                               uint64_t item_flags,
                                               struct rte_flow_error *error) {
  ...
  const int tunnel = !!(item_flags & MLX5_FLOW_LAYER_TUNNEL);
  const uint64_t l4m =
      tunnel ? MLX5_FLOW_LAYER_INNER_L4 : MLX5_FLOW_LAYER_OUTER_L4;
  int ret = 0;
   ...
   */
  if (frag_data_spec ==
          RTE_BE16(RTE_IPV6_EHDR_FO_ALIGN | RTE_IPV6_EHDR_MF_MASK) &&
  ...
  if (frag_data_spec == RTE_BE16(RTE_IPV6_EHDR_FO_ALIGN) &&
    ...
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_LAST,
                              ...
                              "fragment not supported");
  /* Other range values are invalid and rejected. */
  return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_LAST, last,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_item_aso_ct(struct rte_eth_dev *dev,
                                        const struct rte_flow_item *item,
                                        uint64_t *item_flags,
                                        struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_pop_vlan(
    struct rte_eth_dev *dev, uint64_t action_flags,
    const struct rte_flow_action *action, uint64_t item_flags,
  ...
  if (attr->transfer) {
    ...
    bool is_cx5 =
        sh->steering_format_version == MLX5_STEERING_LOGIC_FORMAT_CONNECTX_5;
  ...
  if (direction_error)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS,
                              NULL, "pop vlan action not supported for egress");
  ...
  if (action_flags & MLX5_FLOW_ACTION_PORT_ID)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, action,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static void flow_dev_get_vlan_info_from_items(const struct rte_flow_item *items,
                                              struct rte_vlan_hdr *vlan) {
  const struct rte_flow_item_vlan nic_mask = {
      .tci = RTE_BE16(MLX5DV_FLOW_VLAN_PCP_MASK | MLX5DV_FLOW_VLAN_VID_MASK),
  ...
  for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
    ...
    if (type == RTE_FLOW_ITEM_TYPE_VLAN || type == MLX5_RTE_FLOW_ITEM_TYPE_VLAN)
      break;
  }
  if (items->type != RTE_FLOW_ITEM_TYPE_END) {
    ...
    if ((vlan_m->tci & MLX5DV_FLOW_VLAN_PCP_MASK_BE) ==
      ...
      vlan->vlan_tci &= ~MLX5DV_FLOW_VLAN_PCP_MASK;
      vlan->vlan_tci |=
          rte_be_to_cpu_16(vlan_v->tci & MLX5DV_FLOW_VLAN_PCP_MASK_BE);
    }
    if ((vlan_m->tci & MLX5DV_FLOW_VLAN_VID_MASK_BE) ==
        MLX5DV_FLOW_VLAN_VID_MASK_BE) {
      vlan->vlan_tci &= ~MLX5DV_FLOW_VLAN_VID_MASK;
      vlan->vlan_tci |=
          rte_be_to_cpu_16(vlan_v->tci & MLX5DV_FLOW_VLAN_VID_MASK_BE);
    }
    if (vlan_m->inner_type == nic_mask.inner_type)
      vlan->eth_proto =
          rte_be_to_cpu_16(vlan_v->inner_type & vlan_m->inner_type);
  }
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_push_vlan(
    struct rte_eth_dev *dev, uint64_t action_flags,
  ...
  if (direction_error)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS,
                              NULL,
  ...
  if (vlan_m && (vlan_m->tci & MLX5DV_FLOW_VLAN_PCP_MASK_BE) &&
    ...
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, action,
                              ...
                              "PCP value");
  if (vlan_m && (vlan_m->tci & MLX5DV_FLOW_VLAN_VID_MASK_BE) &&
 ...
 */
static int
flow_dv_validate_action_set_vlan_vid(uint64_t item_flags, uint64_t action_flags,
                                     const struct rte_flow_action actions[],
                                     struct rte_flow_error *error) {
  const struct rte_flow_action *action = actions;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_flag(struct rte_eth_dev *dev,
  ...
  if (config->dv_xmeta_en == MLX5_XMETA_MODE_LEGACY)
    return mlx5_flow_validate_action_flag(action_flags, attr, error);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_mark(struct rte_eth_dev *dev,
  ...
  /* Fall back if no extended metadata register support. */
  if (config->dv_xmeta_en == MLX5_XMETA_MODE_LEGACY)
    return mlx5_flow_validate_action_mark(action, action_flags, attr, error);
  /* Extensive metadata mode requires registers. */
  if (!mlx5_flow_ext_mreg_supported(dev))
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                              "no metadata registers "
  ...
  if (mark->id >= (MLX5_FLOW_MARK_MAX & priv->sh->dv_mark_mask))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &mark->id, "mark id exceeds the limit");
  if (action_flags & MLX5_FLOW_ACTION_FLAG)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_set_meta(
    struct rte_eth_dev *dev, const struct rte_flow_action *action,
    uint64_t action_flags __rte_unused, const struct rte_flow_attr *attr,
    struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_sh_config *config = &priv->sh->config;
  ...
  if (reg == REG_NON)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                              action, "unavailable extended metadata register");
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_set_tag(struct rte_eth_dev *dev,
                                           const struct rte_flow_action *action,
                                           uint64_t action_flags,
                                           const struct rte_flow_attr *attr,
                                           struct rte_flow_error *error) {
 ...
 */
static inline bool flow_hit_aso_supported(const struct mlx5_dev_ctx_shared *sh,
                                          const struct rte_flow_attr *attr) {
  MLX5_ASSERT(sh && attr);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_count(struct rte_eth_dev *dev, bool shared,
                                         uint64_t action_flags,
                                         const struct rte_flow_attr *attr,
                                         struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
...
notsup_err:
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, "count action not supported");
}
 ...
 */
static int
flow_dv_validate_action_l2_encap(struct rte_eth_dev *dev, uint64_t action_flags,
                                 const struct rte_flow_action *action,
                                 const struct rte_flow_attr *attr,
                                 struct rte_flow_error *error) {
  const struct mlx5_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_decap(struct rte_eth_dev *dev,
                                         uint64_t action_flags,
                                         const struct rte_flow_action *action,
...
const struct rte_flow_action_raw_decap empty_decap = {
    .data = NULL,
    .size = 0,
};
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_raw_encap_decap(
    struct rte_eth_dev *dev, const struct rte_flow_action_raw_decap *decap,
    const struct rte_flow_action_raw_encap *encap,
    const struct rte_flow_attr *attr, uint64_t *action_flags, int *actions_n,
    const struct rte_flow_action *action, uint64_t item_flags,
    struct rte_flow_error *error) {
  const struct mlx5_priv *priv = dev->data->dev_private;
  int ret;
  ...
  if (decap && encap) {
    if (decap->size <= MLX5_ENCAPSULATION_DECISION_SIZE &&
      ...
      /* L3 encap. */
      decap = NULL;
    else if (encap->size <= MLX5_ENCAPSULATION_DECISION_SIZE &&
      ...
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                NULL,
                                "unsupported too small "
                                "raw decap and too small raw "
                                "encap combination");
  }
  if (decap) {
    ret = flow_dv_validate_action_decap(dev, *action_flags, action, item_flags,
                                        attr, error);
    if (ret < 0)
      return ret;
    *action_flags |= MLX5_FLOW_ACTION_DECAP;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_aso_ct(struct rte_eth_dev *dev,
                                          uint64_t action_flags,
                                          uint64_t item_flags,
                                          const struct rte_flow_attr *attr,
                                          struct rte_flow_error *error) {
  ...
  if (attr->group == 0 && !attr->transfer)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Only support non-root table");
  ...
  if (!(item_flags & MLX5_FLOW_LAYER_OUTER_L4_TCP))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Not a outer TCP packet");
  return 0;
}

int flow_dv_encap_decap_match_cb(void *tool_ctx __rte_unused,
  ...
  resource =
      container_of(entry, struct mlx5_flow_dv_encap_decap_resource, entry);
  if (resource->reformat_type == ctx_resource->reformat_type &&
      ...
      !memcmp((const void *)resource->buf, (const void *)ctx_resource->buf,
              resource->size))
    return 0;
  return -1;
}
...
struct mlx5_list_entry *flow_dv_encap_decap_create_cb(void *tool_ctx,
  ...
  if (!resource) {
    rte_flow_error_set(ctx->error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                       NULL, "cannot allocate resource memory");
    return NULL;
  }
  *resource = *ctx_resource;
  resource->idx = idx;
  ret = mlx5_flow_os_create_flow_action_packet_reformat(
      sh->cdev->ctx, domain, resource, &resource->action);
  if (ret) {
...
flow_dv_encap_decap_clone_cb(void *tool_ctx, struct mlx5_list_entry *oentry,
  ...
  cache_resource = mlx5_ipool_malloc(sh->ipool[MLX5_IPOOL_DECAP_ENCAP], &idx);
  if (!cache_resource) {
    rte_flow_error_set(ctx->error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                       NULL, "cannot allocate resource memory");
    return NULL;
 ...
 */
static int flow_dv_encap_decap_resource_register(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_encap_decap_resource *resource,
    struct mlx5_flow *dev_flow, struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_dev_ctx_shared *sh = priv->sh;
  struct mlx5_list_entry *entry;
  union {
  ...
  encaps_decaps = flow_dv_hlist_prepare(
      sh, &sh->encaps_decaps, "encaps_decaps", MLX5_FLOW_ENCAP_DECAP_HTABLE_SZ,
      true, true, sh, flow_dv_encap_decap_create_cb,
      flow_dv_encap_decap_match_cb, flow_dv_encap_decap_remove_cb,
      flow_dv_encap_decap_clone_cb, flow_dv_encap_decap_clone_free_cb, error);
  if (unlikely(!encaps_decaps))
    return -rte_errno;
  resource->flags = dev_flow->dv.group ? 0 : 1;
  key64 = __rte_raw_cksum(&encap_decap_key.v32, sizeof(encap_decap_key.v32), 0);
 ...
 *   0 on success otherwise -errno and errno is set.
 */
static int flow_dv_jump_tbl_resource_register(
    struct rte_eth_dev *dev __rte_unused, struct mlx5_flow_tbl_resource *tbl,
    struct mlx5_flow *dev_flow, struct rte_flow_error *error __rte_unused) {
...
int flow_dv_port_id_match_cb(void *tool_ctx __rte_unused,
                             struct mlx5_list_entry *entry, void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
...
struct mlx5_list_entry *flow_dv_port_id_create_cb(void *tool_ctx,
                                                  void *cb_ctx) {
  struct mlx5_dev_ctx_shared *sh = tool_ctx;
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
  struct mlx5_flow_dv_port_id_action_resource *ref = ctx->data;
  ...
  if (!resource) {
    rte_flow_error_set(ctx->error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                       NULL, "cannot allocate port_id action memory");
    return NULL;
  }
  *resource = *ref;
  ret = mlx5_flow_os_create_flow_action_dest_port(sh->fdb_domain, ref->port_id,
                                                  &resource->action);
  if (ret) {
...
struct mlx5_list_entry *flow_dv_port_id_clone_cb(
    void *tool_ctx, struct mlx5_list_entry *entry __rte_unused, void *cb_ctx) {
  struct mlx5_dev_ctx_shared *sh = tool_ctx;
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
  struct mlx5_flow_dv_port_id_action_resource *resource;
  uint32_t idx;
 ...
 *   0 on success otherwise -errno and errno is set.
 */
static int flow_dv_port_id_action_resource_register(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_port_id_action_resource *ref,
    struct mlx5_flow *dev_flow, struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_list_entry *entry;
  struct mlx5_flow_dv_port_id_action_resource *resource;
...
int flow_dv_push_vlan_match_cb(void *tool_ctx __rte_unused,
                               struct mlx5_list_entry *entry, void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
...
struct mlx5_list_entry *flow_dv_push_vlan_create_cb(void *tool_ctx,
  ...
  if (!resource) {
    rte_flow_error_set(ctx->error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                       NULL, "cannot allocate push_vlan action memory");
    return NULL;
  }
  *resource = *ref;
  ...
  if (ret) {
    ...
    rte_flow_error_set(ctx->error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                       NULL, "cannot create push vlan action");
    return NULL;
  }
  resource->idx = idx;
  return &resource->entry;
 ...
 *   0 on success otherwise -errno and errno is set.
 */
static int flow_dv_push_vlan_action_resource_register(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_push_vlan_action_resource *ref,
    struct mlx5_flow *dev_flow, struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_dv_push_vlan_action_resource *resource;
  struct mlx5_list_entry *entry;
...
int flow_dv_convert_encap_data(const struct rte_flow_item *items, uint8_t *buf,
  ...
  for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
    ...
    case RTE_FLOW_ITEM_TYPE_VLAN:
      ...
      if (!eth)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  (void *)items->type, "eth header not found");
      if (!eth->ether_type)
    ...
    case RTE_FLOW_ITEM_TYPE_IPV4:
      ...
      if (!ipv4->version_ihl)
        ipv4->version_ihl = MLX5_ENCAP_IPV4_VERSION | MLX5_ENCAP_IPV4_IHL_MIN;
    ...
    case RTE_FLOW_ITEM_TYPE_IPV6:
      ...
      if (!ipv6->vtc_flow)
        ipv6->vtc_flow = RTE_BE32(MLX5_ENCAP_IPV6_VTC_FLOW);
      if (!ipv6->hop_limits)
    ...
    case RTE_FLOW_ITEM_TYPE_UDP:
      ...
      if (!ipv4 && !ipv6)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  (void *)items->type, "ip header not found");
    ...
    case RTE_FLOW_ITEM_TYPE_VXLAN:
      ...
      if (!udp)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  (void *)items->type, "udp header not found");
      if (!udp->dst_port)
        udp->dst_port = RTE_BE16(MLX5_UDP_PORT_VXLAN);
      if (!vxlan->vx_flags)
        vxlan->vx_flags = RTE_BE32(MLX5_ENCAP_VXLAN_FLAGS);
      break;
    case RTE_FLOW_ITEM_TYPE_VXLAN_GPE:
      ...
      if (!udp->dst_port)
        udp->dst_port = RTE_BE16(MLX5_UDP_PORT_VXLAN_GPE);
      if (!vxlan_gpe->vx_flags)
        vxlan_gpe->vx_flags = MLX5_ENCAP_VXLAN_GPE_FLAGS;
      break;
    case RTE_FLOW_ITEM_TYPE_GRE:
    case RTE_FLOW_ITEM_TYPE_NVGRE:
...
static int flow_dv_zero_encap_udp_csum(void *data,
                                       struct rte_flow_error *error) {
  struct rte_ether_hdr *eth = NULL;
  struct rte_vlan_hdr *vlan = NULL;
  struct rte_ipv6_hdr *ipv6 = NULL;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_create_action_l2_encap(struct rte_eth_dev *dev,
                                          const struct rte_flow_action *action,
  ...
  struct mlx5_flow_dv_encap_decap_resource res = {
      .reformat_type = MLX5DV_FLOW_ACTION_PACKET_REFORMAT_TYPE_L2_TO_L2_TUNNEL,
      .ft_type =
          transfer ? MLX5DV_FLOW_TABLE_TYPE_FDB : MLX5DV_FLOW_TABLE_TYPE_NIC_TX,
  };

  if (action->type == RTE_FLOW_ACTION_TYPE_RAW_ENCAP) {
    raw_encap_data = (const struct rte_flow_action_raw_encap *)action->conf;
    ...
    if (action->type == RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP)
      ...
      encap_data = ((const struct rte_flow_action_nvgre_encap *)action->conf)
                       ->definition;
    if (flow_dv_convert_encap_data(encap_data, res.buf, &res.size, error))
      return -rte_errno;
  }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_create_action_l2_decap(struct rte_eth_dev *dev,
                                          struct mlx5_flow *dev_flow,
                                          uint8_t transfer,
                                          struct rte_flow_error *error) {
  struct mlx5_flow_dv_encap_decap_resource res = {
      .size = 0,
      .reformat_type = MLX5DV_FLOW_ACTION_PACKET_REFORMAT_TYPE_L2_TUNNEL_TO_L2,
      .ft_type =
          transfer ? MLX5DV_FLOW_TABLE_TYPE_FDB : MLX5DV_FLOW_TABLE_TYPE_NIC_RX,
  };

  if (flow_dv_encap_decap_resource_register(dev, &res, dev_flow, error))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                              "can't create L2 decap action");
  return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_create_action_raw_encap(struct rte_eth_dev *dev,
                                           const struct rte_flow_action *action,
                                           struct mlx5_flow *dev_flow,
                                           const struct rte_flow_attr *attr,
                                           struct rte_flow_error *error) {
  ...
  res.reformat_type =
      res.size < MLX5_ENCAPSULATION_DECISION_SIZE
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_create_action_push_vlan(struct rte_eth_dev *dev,
                                           const struct rte_flow_attr *attr,
  ...
  memset(&res, 0, sizeof(res));
  res.vlan_tag =
      rte_cpu_to_be_32(((uint32_t)vlan->eth_proto) << 16 | vlan->vlan_tci);
  if (attr->transfer)
    res.ft_type = MLX5DV_FLOW_TABLE_TYPE_FDB;
  else
    res.ft_type = attr->egress ? MLX5DV_FLOW_TABLE_TYPE_NIC_TX
                               : MLX5DV_FLOW_TABLE_TYPE_NIC_RX;
  return flow_dv_push_vlan_action_resource_register(dev, &res, dev_flow, error);
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_modify_mac(
    const uint64_t action_flags, const struct rte_flow_action *action,
    const uint64_t item_flags, struct rte_flow_error *error) {
  int ret = 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_modify_ipv4(
    const uint64_t action_flags, const struct rte_flow_action *action,
    const uint64_t item_flags, struct rte_flow_error *error) {
  int ret = 0;
  uint64_t layer;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_modify_ipv6(
    const uint64_t action_flags, const struct rte_flow_action *action,
    const uint64_t item_flags, struct rte_flow_error *error) {
  int ret = 0;
  uint64_t layer;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_modify_tp(
    const uint64_t action_flags, const struct rte_flow_action *action,
    const uint64_t item_flags, struct rte_flow_error *error) {
  int ret = 0;
  uint64_t layer;
  ...
  ret = flow_dv_validate_action_modify_hdr(action_flags, action, error);
  if (!ret) {
    layer = (action_flags & MLX5_FLOW_ACTION_DECAP) ? MLX5_FLOW_LAYER_INNER_L4
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_modify_tcp_seq(
    const uint64_t action_flags, const struct rte_flow_action *action,
    const uint64_t item_flags, struct rte_flow_error *error) {
  int ret = 0;
  uint64_t layer;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_modify_tcp_ack(
    const uint64_t action_flags, const struct rte_flow_action *action,
    const uint64_t item_flags, struct rte_flow_error *error) {
  int ret = 0;
  uint64_t layer;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_modify_ttl(
    const uint64_t action_flags, const struct rte_flow_action *action,
    const uint64_t item_flags, struct rte_flow_error *error) {
  int ret = 0;
  uint64_t layer;
  ...
  ret = flow_dv_validate_action_modify_hdr(action_flags, action, error);
  if (!ret) {
    layer = (action_flags & MLX5_FLOW_ACTION_DECAP) ? MLX5_FLOW_LAYER_INNER_L3
 ...
 */
static int flow_dv_validate_action_modify_field(
    struct rte_eth_dev *dev, const uint64_t action_flags,
    const struct rte_flow_action *action, const struct rte_flow_attr *attr,
    struct rte_flow_error *error) {
  int ret = 0;
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  uint32_t dst_width = mlx5_flow_item_field_width(
      dev, action_modify_field->dst.field, -1, attr, error);
  uint32_t src_width = mlx5_flow_item_field_width(
      dev, action_modify_field->src.field, dst_width, attr, error);
  ...
  if (action_modify_field->dst.field != RTE_FLOW_FIELD_VALUE &&
    ...
    if ((action_modify_field->dst.offset + action_modify_field->width >
         dst_width) ||
  ...
  if (action_modify_field->src.field != RTE_FLOW_FIELD_VALUE &&
    ...
    if ((action_modify_field->src.offset + action_modify_field->width >
         src_width) ||
  ...
  if ((action_modify_field->dst.field == action_modify_field->src.field) &&
      (action_modify_field->dst.level == action_modify_field->src.level))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, action,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_jump(struct rte_eth_dev *dev,
  ...
  int ret = 0;
  struct flow_grp_info grp_info = {.external = !!external,
                                   .transfer = !!attributes->transfer,
                                   .fdb_def_rule = 1,
                                   .std_tbl_fix = 0};
  if (action_flags & (MLX5_FLOW_FATE_ACTIONS | MLX5_FLOW_FATE_ESWITCH_ACTIONS))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
  ...
  if (!action->conf)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              NULL, "action configuration not set");
  target_group = ((const struct rte_flow_action_jump *)action->conf)->group;
  ret = mlx5_flow_group_to_table(dev, tunnel, target_group, &table, &grp_info,
                                 error);
  if (ret)
    return ret;
  if (attributes->group == target_group &&
      !(action_flags &
        (MLX5_FLOW_ACTION_TUNNEL_SET | MLX5_FLOW_ACTION_TUNNEL_MATCH)))
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_port_id(struct rte_eth_dev *dev,
                                           uint64_t action_flags,
                                           const struct rte_flow_action *action,
                                           const struct rte_flow_attr *attr,
                                           struct rte_flow_error *error) {
  ...
  if (!act_priv)
    return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              action->conf,
...
flow_dv_modify_hdr_action_max(struct rte_eth_dev *dev __rte_unused, bool root) {
  /*
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_validate_action_meter(
    struct rte_eth_dev *dev, uint64_t action_flags, uint64_t item_flags,
    const struct rte_flow_action *action, const struct rte_flow_attr *attr,
    const struct rte_flow_item *port_id_item, bool *def_policy,
    struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (!priv->mtr_en)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "meter action not supported");
  ...
  if (fm->ref_cnt && !priv->sh->meter_aso_en &&
    ...
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                              ...
                              "meter attributes");
  if (fm->def_policy) {
    if (!((attr->transfer && mtrmng->def_policy[MLX5_MTR_DOMAIN_TRANSFER]) ||
          (attr->egress && mtrmng->def_policy[MLX5_MTR_DOMAIN_EGRESS]) ||
          (attr->ingress && mtrmng->def_policy[MLX5_MTR_DOMAIN_INGRESS])))
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
    ...
    *def_policy = true;
    ...
    if (attr->transfer && mtr_policy->dev) {
       ...
       */
      struct mlx5_priv *policy_port_priv = mtr_policy->dev->data->dev_private;
      int32_t flow_src_port = priv->representor_id;

      if (port_id_item) {
        const struct rte_flow_item_port_id *spec = port_id_item->spec;
        struct mlx5_priv *port_priv =
            mlx5_port_to_eswitch_info(spec->id, false);
        if (!port_priv)
      ...
      fp = mlx5_flow_meter_hierarchy_get_final_policy(dev, mtr_policy);
      if (fp == NULL)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL,
      ...
      MLX5_ASSERT(acg->fate_action == MLX5_FLOW_FATE_SHARED_RSS ||
                  acy->fate_action == MLX5_FLOW_FATE_SHARED_RSS);
      if (acg->fate_action == MLX5_FLOW_FATE_SHARED_RSS)
        rss_act = acg->rss;
      else
        rss_act = acy->rss;
      ret = mlx5_flow_validate_action_rss(rss_act, action_flags, dev, attr,
                                          item_flags, error);
      if (ret)
        return ret;
    }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_age(uint64_t action_flags,
                                       const struct rte_flow_action *action,
                                       struct rte_eth_dev *dev,
                                       struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_modify_ipv4_dscp(
    const uint64_t action_flags, const struct rte_flow_action *action,
    const uint64_t item_flags, struct rte_flow_error *error) {
  int ret = 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_modify_ipv6_dscp(
    const uint64_t action_flags, const struct rte_flow_action *action,
    const uint64_t item_flags, struct rte_flow_error *error) {
  int ret = 0;
...
int flow_dv_modify_match_cb(void *tool_ctx __rte_unused,
                            struct mlx5_list_entry *entry, void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
...
flow_dv_modify_ipool_get(struct mlx5_dev_ctx_shared *sh, uint8_t index) {
  struct mlx5_indexed_pool *ipool =
      __atomic_load_n(&sh->mdh_ipools[index], __ATOMIC_SEQ_CST);

  if (!ipool) {
    struct mlx5_indexed_pool *expected = NULL;
    struct mlx5_indexed_pool_config cfg = (struct mlx5_indexed_pool_config){
        .size = sizeof(struct mlx5_flow_dv_modify_hdr_resource) +
                (index + 1) * sizeof(struct mlx5_modification_cmd),
        .trunk_size = 64,
        .grow_trunk = 3,
        .grow_shift = 2,
        .need_lock = 1,
        .release_mem_en = !!sh->config.reclaim_mode,
        .per_core_cache = sh->config.reclaim_mode ? 0 : (1 << 16),
        .malloc = mlx5_malloc,
        .free = mlx5_free,
    ...
    if (!ipool)
      return NULL;
    if (!__atomic_compare_exchange_n(&sh->mdh_ipools[index], &expected, ipool,
                                     false, __ATOMIC_SEQ_CST,
                                     __ATOMIC_SEQ_CST)) {
      mlx5_ipool_destroy(ipool);
      ipool = __atomic_load_n(&sh->mdh_ipools[index], __ATOMIC_SEQ_CST);
    }
  }
  return ipool;
}
...
struct mlx5_list_entry *flow_dv_modify_create_cb(void *tool_ctx, void *cb_ctx) {
  ...
  struct mlx5_flow_dv_modify_hdr_resource *ref = ctx->data;
  struct mlx5_indexed_pool *ipool =
      flow_dv_modify_ipool_get(sh, ref->actions_num - 1);
  int ret;
  ...
  rte_memcpy(&entry->ft_type, RTE_PTR_ADD(ref, offsetof(typeof(*ref), ft_type)),
             key_len + data_len);
  if (entry->ft_type == MLX5DV_FLOW_TABLE_TYPE_FDB)
    ...
    ns = sh->rx_domain;
  ret = mlx5_flow_os_create_flow_action_modify_header(sh->cdev->ctx, ns, entry,
...
struct mlx5_list_entry *flow_dv_modify_clone_cb(void *tool_ctx,
  ...
  entry = mlx5_ipool_malloc(sh->mdh_ipools[ref->actions_num - 1], &idx);
  if (!entry) {
    rte_flow_error_set(ctx->error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                       NULL, "cannot allocate resource memory");
    return NULL;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_action_sample(
    uint64_t *action_flags, const struct rte_flow_action *action,
    struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
    uint64_t item_flags, const struct rte_flow_action_rss *rss,
    const struct rte_flow_action_rss **sample_rss,
    const struct rte_flow_action_count **count, int *fdb_mirror_limit,
    struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (!priv->sh->cdev->config.devx || (sample->ratio > 0 && !priv->sampler_en))
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "sample action not supported");
  ...
  for (; act->type != RTE_FLOW_ACTION_TYPE_END; act++) {
    if (actions_n == MLX5_DV_MAX_NUMBER_OF_ACTIONS)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, act,
                                "too many actions");
    switch (act->type) {
    case RTE_FLOW_ACTION_TYPE_QUEUE:
      ret = mlx5_flow_validate_action_queue(act, sub_action_flags, dev, attr,
                                            error);
      if (ret < 0)
        return ret;
      queue_index = ((const struct rte_flow_action_queue *)(act->conf))->index;
      sub_action_flags |= MLX5_FLOW_ACTION_QUEUE;
      ++actions_n;
      break;
    case RTE_FLOW_ACTION_TYPE_RSS:
      *sample_rss = act->conf;
      ret = mlx5_flow_validate_action_rss(act, sub_action_flags, dev, attr,
                                          item_flags, error);
      if (ret < 0)
        return ret;
      if (rss && *sample_rss &&
    ...
    case RTE_FLOW_ACTION_TYPE_MARK:
      ret =
          flow_dv_validate_action_mark(dev, act, sub_action_flags, attr, error);
      if (ret < 0)
        return ret;
      if (dev_conf->dv_xmeta_en != MLX5_XMETA_MODE_LEGACY)
        sub_action_flags |= MLX5_FLOW_ACTION_MARK | MLX5_FLOW_ACTION_MARK_EXT;
      else
    ...
    case RTE_FLOW_ACTION_TYPE_COUNT:
      ret = flow_dv_validate_action_count(
          dev, false, *action_flags | sub_action_flags, attr, error);
      if (ret < 0)
        return ret;
      *count = act->conf;
    ...
    case RTE_FLOW_ACTION_TYPE_PORT_ID:
    case RTE_FLOW_ACTION_TYPE_REPRESENTED_PORT:
      ret = flow_dv_validate_action_port_id(dev, sub_action_flags, act, attr,
                                            error);
      if (ret)
        return ret;
    ...
    case RTE_FLOW_ACTION_TYPE_RAW_ENCAP:
      ret = flow_dv_validate_action_raw_encap_decap(
          dev, NULL, act->conf, attr, &sub_action_flags, &actions_n, action,
          item_flags, error);
      if (ret < 0)
        return ret;
      ++actions_n;
    ...
    case RTE_FLOW_ACTION_TYPE_NVGRE_ENCAP:
      ret = flow_dv_validate_action_l2_encap(dev, sub_action_flags, act, attr,
                                             error);
      if (ret < 0)
        return ret;
  ...
  if (attr->ingress && !attr->transfer) {
    if (!(sub_action_flags & (MLX5_FLOW_ACTION_QUEUE | MLX5_FLOW_ACTION_RSS)))
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
  ...
  if ((sub_action_flags & MLX5_FLOW_XCAP_ACTIONS) &&
    ...
    if ((sub_action_flags & MLX5_FLOW_XCAP_ACTIONS) == MLX5_FLOW_XCAP_ACTIONS)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                NULL,
                                "encap and decap "
                                "combination aren't "
 ...
 *   0 on success otherwise -errno and errno is set.
 */
static int flow_dv_modify_hdr_resource_register(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_modify_hdr_resource *resource,
    struct mlx5_flow *dev_flow, struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_dev_ctx_shared *sh = priv->sh;
  uint32_t key_len = sizeof(*resource) - offsetof(typeof(*resource), ft_type) +
  ...
  modify_cmds = flow_dv_hlist_prepare(
      sh, &sh->modify_cmds, "hdr_modify", MLX5_FLOW_HDR_MODIFY_HTABLE_SZ, true,
      false, sh, flow_dv_modify_create_cb, flow_dv_modify_match_cb,
      flow_dv_modify_remove_cb, flow_dv_modify_clone_cb,
      flow_dv_modify_clone_free_cb, error);
  if (unlikely(!modify_cmds))
    return -rte_errno;
  resource->root = !dev_flow->dv.group;
  if (resource->actions_num >
      flow_dv_modify_hdr_action_max(dev, resource->root))
    return rte_flow_error_set(error, EOVERFLOW, RTE_FLOW_ERROR_TYPE_ACTION,
                              NULL, "too many modify header items");
...
flow_dv_counter_get_by_idx(struct rte_eth_dev *dev, uint32_t idx,
 ...
 */
static bool flow_dv_is_counter_in_pool(struct mlx5_flow_counter_pool *pool,
                                       int id) {
  int base =
      (pool->min_dcs->id / MLX5_COUNTERS_PER_POOL) * MLX5_COUNTERS_PER_POOL;
 ...
 */
static int flow_dv_container_resize(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_counter_mng *cmng = &priv->sh->cmng;
 ...
 */
static inline int _flow_dv_query_count(struct rte_eth_dev *dev,
                                       uint32_t counter, uint64_t *pkts,
                                       uint64_t *bytes) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_counter_pool *pool = NULL;
  ...
  if (priv->sh->cmng.counter_fallback)
    return mlx5_devx_cmd_flow_counter_query(cnt->dcs_when_active, 0, 0, pkts,
                                            bytes, 0, NULL, NULL, 0);
 ...
 */
static uint32_t flow_dv_counter_alloc(struct rte_eth_dev *dev, uint32_t age) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_counter_pool *pool = NULL;
  ...
  cnt_idx = MLX5_MAKE_CNT_IDX(pool->index, MLX5_CNT_ARRAY_IDX(pool, cnt_free));
  /* Update the counter reset values. */
  if (_flow_dv_query_count(dev, cnt_idx, &cnt_free->hits, &cnt_free->bytes))
    goto err;
...
flow_dv_counter_idx_get_age(struct rte_eth_dev *dev, uint32_t counter) {
  struct mlx5_flow_counter *cnt;
  struct mlx5_flow_counter_pool *pool = NULL;
 ...
 */
static void flow_dv_counter_remove_from_age(struct rte_eth_dev *dev,
                                            uint32_t counter,
                                            struct mlx5_flow_counter *cnt) {
  ...
  if (!__atomic_compare_exchange_n(&age_param->state, &expected, AGE_FREE,
 ...
 *   Index to the counter handler.
 */
static void flow_dv_counter_free(struct rte_eth_dev *dev, uint32_t counter) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_counter_pool *pool = NULL;
  struct mlx5_flow_counter *cnt;
  enum mlx5_counter_type cnt_type;
  ...
  if (pool->is_aged) {
     ...
     */
    if (__atomic_sub_fetch(&cnt->shared_info.refcnt, 1, __ATOMIC_RELAXED))
      return;
  }
  cnt->pool = pool;
  /*
  ...
  if (!priv->sh->cmng.counter_fallback) {
    ...
    cnt_type = pool->is_aged ? MLX5_COUNTER_TYPE_AGE : MLX5_COUNTER_TYPE_ORIGIN;
    rte_spinlock_lock(&priv->sh->cmng.csl[cnt_type]);
    TAILQ_INSERT_TAIL(&priv->sh->cmng.counters[cnt_type], cnt, next);
 ...
 *   0 on success, otherwise negative errno value and rte_errno is set.
 */
static int flow_dv_mtr_container_resize(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (old_pools)
    memcpy(pools, old_pools, pools_mng->n * sizeof(struct mlx5_aso_mtr_pool *));
  pools_mng->n = resize;
...
flow_dv_mtr_pool_create(struct rte_eth_dev *dev,
  ...
  dcs = mlx5_devx_cmd_create_flow_meter_aso_obj(
      priv->sh->cdev->ctx, priv->sh->cdev->pdn, log_obj_size);
  if (!dcs) {
    rte_errno = ENODATA;
    return NULL;
  }
 ...
 */
static void flow_dv_aso_mtr_release_to_pool(struct rte_eth_dev *dev,
                                            uint32_t mtr_idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_aso_mtr_pools_mng *pools_mng = &priv->sh->mtrmng->pools_mng;
 ...
 */
static uint32_t flow_dv_mtr_alloc(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  rte_spinlock_unlock(&pools_mng->mtrsl);
  pool =
      container_of(mtr_free, struct mlx5_aso_mtr_pool, mtrs[mtr_free->offset]);
  ...
  if (!mtr_free->fm.meter_action) {
    ...
    mtr_free->fm.meter_action = mlx5_glue->dv_create_flow_action_aso(
        priv->sh->rx_domain, pool->devx_obj->obj, mtr_free->offset,
        (1 << MLX5_FLOW_COLOR_GREEN), reg_id - REG_C_0);
 ...
 *   - a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate_attributes(struct rte_eth_dev *dev,
                                       const struct mlx5_flow_tunnel *tunnel,
                                       const struct rte_flow_attr *attributes,
  ...
  if (attributes->group)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_GROUP,
  ...
  if (attributes->priority != MLX5_FLOW_LOWEST_PRIO_INDICATOR &&
    ...
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY,
                              NULL, "priority out of range");
  if (attributes->transfer) {
    if (!priv->sh->config.dv_esw_en)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "E-Switch dr is not supported");
    if (attributes->egress)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS,
                                attributes, "egress is not supported");
  }
  if (!(attributes->egress ^ attributes->ingress))
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR, NULL,
                              "must specify exactly one of "
                              "ingress or egress");
  return ret;
}

static int validate_integrity_bits(const struct rte_flow_item_integrity *mask,
...
static int flow_dv_validate_item_integrity_post(
    const struct rte_flow_item *integrity_items[2], int64_t pattern_flags,
    struct rte_flow_error *error) {
  ...
  if (pattern_flags & MLX5_FLOW_ITEM_OUTER_INTEGRITY) {
    ...
    ret = validate_integrity_bits(mask, pattern_flags, MLX5_FLOW_LAYER_OUTER_L3,
                                  MLX5_FLOW_LAYER_OUTER_L4,
                                  MLX5_FLOW_LAYER_OUTER_L3_IPV4, error);
    if (ret)
      return ret;
  }
  if (pattern_flags & MLX5_FLOW_ITEM_INNER_INTEGRITY) {
    mask = (typeof(mask))integrity_items[1]->mask;
    ret = validate_integrity_bits(mask, pattern_flags, MLX5_FLOW_LAYER_INNER_L3,
                                  MLX5_FLOW_LAYER_INNER_L4,
                                  MLX5_FLOW_LAYER_INNER_L3_IPV4, error);
    if (ret)
      return ret;
  }
  return 0;
...
flow_dv_validate_item_integrity(struct rte_eth_dev *dev,
  ...
  struct mlx5_priv *priv = dev->data->dev_private;
  const struct rte_flow_item_integrity *mask =
      (typeof(mask))integrity_item->mask;
  const struct rte_flow_item_integrity *spec =
      (typeof(spec))integrity_item->spec;
  ...
  if (!spec)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM,
                              integrity_item, "no spec for integrity item");
  if (!mask)
    mask = &rte_flow_item_integrity_mask;
  if (!mlx5_validate_integrity_item(mask))
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM,
                              integrity_item, "unsupported integrity filter");
  if (spec->level > 1) {
    if (pattern_flags & MLX5_FLOW_ITEM_INNER_INTEGRITY)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                                "multiple inner integrity items not supported");
    integrity_items[1] = integrity_item;
    *last_item |= MLX5_FLOW_ITEM_INNER_INTEGRITY;
  } else {
    if (pattern_flags & MLX5_FLOW_ITEM_OUTER_INTEGRITY)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                                "multiple outer integrity items not supported");
...
static int flow_dv_validate_item_flex(struct rte_eth_dev *dev,
                                      const struct rte_flow_item *item,
                                      uint64_t item_flags, uint64_t *last_item,
                                      bool is_inner,
                                      struct rte_flow_error *error) {
  ...
  case FLEX_TUNNEL_MODE_SINGLE:
    if (item_flags & (MLX5_FLOW_ITEM_OUTER_FLEX | MLX5_FLOW_ITEM_INNER_FLEX))
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                         "multiple flex items not supported");
    break;
  case FLEX_TUNNEL_MODE_OUTER:
    if (is_inner)
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                         "inner flex item was not configured");
  ...
  case FLEX_TUNNEL_MODE_INNER:
    if (!is_inner)
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                         "outer flex item was not configured");
  ...
  case FLEX_TUNNEL_MODE_TUNNEL:
    if (is_inner || (item_flags & MLX5_FLOW_ITEM_FLEX_TUNNEL))
      rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                         "multiple flex tunnel items not supported");
    break;
  default:
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                       "invalid flex item configuration");
  }
  *last_item = flex->tunnel_mode == FLEX_TUNNEL_MODE_TUNNEL
                   ? MLX5_FLOW_ITEM_FLEX_TUNNEL
               : is_inner ? MLX5_FLOW_ITEM_INNER_FLEX
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_validate(struct rte_eth_dev *dev,
                            const struct rte_flow_attr *attr,
                            const struct rte_flow_item items[],
                            const struct rte_flow_action actions[],
                            bool external, int hairpin,
                            struct rte_flow_error *error) {
  int ret;
      ...
      .hdr =
          {
              .common =
                  {
                      .u32 = RTE_BE32(((const struct rte_ecpri_common_hdr){
                                           .type = 0xFF,
                                       })
                                          .u32),
                  },
  ...
  tunnel = is_tunnel_offload_active(dev)
               ? mlx5_get_tof(items, actions, &tof_rule_type)
               : NULL;
  if (tunnel) {
    if (!dev_conf->dv_flow_en)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL,
                                "tunnel offload requires DV flow interface");
    if (priv->representor)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
    ...
    if (tof_rule_type == MLX5_TUNNEL_OFFLOAD_SET_RULE)
      ...
      action_flags |= MLX5_FLOW_ACTION_TUNNEL_MATCH | MLX5_FLOW_ACTION_DECAP;
  ...
  for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
    ...
    case RTE_FLOW_ITEM_TYPE_PORT_ID:
      ret = flow_dv_validate_item_port_id(dev, items, attr, item_flags, error);
      if (ret < 0)
        return ret;
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_ETH:
      ret = mlx5_flow_validate_item_eth(items, item_flags, true, error);
      if (ret < 0)
        return ret;
      last_item = tunnel ? MLX5_FLOW_LAYER_INNER_L2 : MLX5_FLOW_LAYER_OUTER_L2;
      if (items->mask != NULL && items->spec != NULL) {
        ether_type = ((const struct rte_flow_item_eth *)items->spec)->type;
        ether_type &= ((const struct rte_flow_item_eth *)items->mask)->type;
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_VLAN:
      ret = flow_dv_validate_item_vlan(items, item_flags, dev, error);
      if (ret < 0)
        return ret;
      last_item =
          tunnel ? MLX5_FLOW_LAYER_INNER_VLAN : MLX5_FLOW_LAYER_OUTER_VLAN;
      if (items->mask != NULL && items->spec != NULL) {
        ether_type =
            ((const struct rte_flow_item_vlan *)items->spec)->inner_type;
        ether_type &=
            ((const struct rte_flow_item_vlan *)items->mask)->inner_type;
        ether_type = rte_be_to_cpu_16(ether_type);
      } else {
        ether_type = 0;
      }
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_IPV4:
      mlx5_flow_tunnel_ip_check(items, next_protocol, &item_flags, &tunnel);
      ret = flow_dv_validate_item_ipv4(dev, items, item_flags, last_item,
                                       ether_type, error);
      if (ret < 0)
        return ret;
      last_item = tunnel ? MLX5_FLOW_LAYER_INNER_L3_IPV4
                         : MLX5_FLOW_LAYER_OUTER_L3_IPV4;
      if (items->mask != NULL &&
          ((const struct rte_flow_item_ipv4 *)items->mask)->hdr.next_proto_id) {
        next_protocol = ((const struct rte_flow_item_ipv4 *)(items->spec))
                            ->hdr.next_proto_id;
        next_protocol &= ((const struct rte_flow_item_ipv4 *)(items->mask))
                             ->hdr.next_proto_id;
      } else {
    ...
    case RTE_FLOW_ITEM_TYPE_IPV6:
      mlx5_flow_tunnel_ip_check(items, next_protocol, &item_flags, &tunnel);
      ret = mlx5_flow_validate_item_ipv6(items, item_flags, last_item,
                                         ether_type, &nic_ipv6_mask, error);
      if (ret < 0)
        return ret;
      last_item = tunnel ? MLX5_FLOW_LAYER_INNER_L3_IPV6
                         : MLX5_FLOW_LAYER_OUTER_L3_IPV6;
      if (items->mask != NULL &&
          ((const struct rte_flow_item_ipv6 *)items->mask)->hdr.proto) {
        item_ipv6_proto =
            ((const struct rte_flow_item_ipv6 *)items->spec)->hdr.proto;
        next_protocol =
            ((const struct rte_flow_item_ipv6 *)items->spec)->hdr.proto;
        next_protocol &=
            ((const struct rte_flow_item_ipv6 *)items->mask)->hdr.proto;
    ...
    case RTE_FLOW_ITEM_TYPE_IPV6_FRAG_EXT:
      ret = flow_dv_validate_item_ipv6_frag_ext(items, item_flags, error);
      if (ret < 0)
        return ret;
      last_item = tunnel ? MLX5_FLOW_LAYER_INNER_L3_IPV6_FRAG_EXT
                         : MLX5_FLOW_LAYER_OUTER_L3_IPV6_FRAG_EXT;
      if (items->mask != NULL &&
          ((const struct rte_flow_item_ipv6_frag_ext *)items->mask)
              ->hdr.next_header) {
        next_protocol =
            ((const struct rte_flow_item_ipv6_frag_ext *)items->spec)
                ->hdr.next_header;
        next_protocol &=
            ((const struct rte_flow_item_ipv6_frag_ext *)items->mask)
                ->hdr.next_header;
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_TCP:
      ret = mlx5_flow_validate_item_tcp(items, item_flags, next_protocol,
                                        &nic_tcp_mask, error);
      if (ret < 0)
        return ret;
      last_item =
          tunnel ? MLX5_FLOW_LAYER_INNER_L4_TCP : MLX5_FLOW_LAYER_OUTER_L4_TCP;
      break;
    case RTE_FLOW_ITEM_TYPE_UDP:
      ret =
          mlx5_flow_validate_item_udp(items, item_flags, next_protocol, error);
      const struct rte_flow_item_udp *spec = items->spec;
      const struct rte_flow_item_udp *mask = items->mask;
      if (!mask)
        mask = &rte_flow_item_udp_mask;
      if (spec != NULL)
        udp_dport = rte_be_to_cpu_16(spec->hdr.dst_port & mask->hdr.dst_port);
      if (ret < 0)
        return ret;
      last_item =
          tunnel ? MLX5_FLOW_LAYER_INNER_L4_UDP : MLX5_FLOW_LAYER_OUTER_L4_UDP;
      break;
    case RTE_FLOW_ITEM_TYPE_GRE:
      ret =
          mlx5_flow_validate_item_gre(items, item_flags, next_protocol, error);
      if (ret < 0)
        return ret;
      gre_item = items;
    ...
    case RTE_FLOW_ITEM_TYPE_GRE_OPTION:
      ret = mlx5_flow_validate_item_gre_option(dev, items, item_flags, attr,
                                               gre_item, error);
      if (ret < 0)
        return ret;
      last_item = MLX5_FLOW_LAYER_GRE;
    ...
    case RTE_FLOW_ITEM_TYPE_GRE_KEY:
      ret = mlx5_flow_validate_item_gre_key(items, item_flags, gre_item, error);
      if (ret < 0)
        return ret;
      last_item = MLX5_FLOW_LAYER_GRE_KEY;
      break;
    case RTE_FLOW_ITEM_TYPE_VXLAN:
      ret = mlx5_flow_validate_item_vxlan(dev, udp_dport, items, item_flags,
                                          attr, error);
      if (ret < 0)
        return ret;
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_VXLAN_GPE:
      ret = mlx5_flow_validate_item_vxlan_gpe(items, item_flags, dev, error);
      if (ret < 0)
        return ret;
      last_item = MLX5_FLOW_LAYER_VXLAN_GPE;
      break;
    case RTE_FLOW_ITEM_TYPE_GENEVE:
      ret = mlx5_flow_validate_item_geneve(items, item_flags, dev, error);
      if (ret < 0)
        return ret;
      geneve_item = items;
    ...
    case RTE_FLOW_ITEM_TYPE_GENEVE_OPT:
      ret = mlx5_flow_validate_item_geneve_opt(items, last_item, geneve_item,
                                               dev, error);
      if (ret < 0)
        return ret;
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_MPLS:
      ret = mlx5_flow_validate_item_mpls(dev, items, item_flags, last_item,
                                         error);
      if (ret < 0)
        return ret;
    ...
    case RTE_FLOW_ITEM_TYPE_MARK:
      ret = flow_dv_validate_item_mark(dev, items, attr, error);
      if (ret < 0)
        return ret;
      last_item = MLX5_FLOW_ITEM_MARK;
      break;
    case RTE_FLOW_ITEM_TYPE_META:
      ret = flow_dv_validate_item_meta(dev, items, attr, error);
      if (ret < 0)
        return ret;
    ...
    case RTE_FLOW_ITEM_TYPE_ICMP:
      ret =
          mlx5_flow_validate_item_icmp(items, item_flags, next_protocol, error);
      if (ret < 0)
        return ret;
      last_item = MLX5_FLOW_LAYER_ICMP;
      break;
    case RTE_FLOW_ITEM_TYPE_ICMP6:
      ret = mlx5_flow_validate_item_icmp6(items, item_flags, next_protocol,
                                          error);
      if (ret < 0)
        return ret;
    ...
    case RTE_FLOW_ITEM_TYPE_TAG:
      ret = flow_dv_validate_item_tag(dev, items, attr, error);
      if (ret < 0)
        return ret;
    ...
    case RTE_FLOW_ITEM_TYPE_GTP:
      ret = flow_dv_validate_item_gtp(dev, items, item_flags, error);
      if (ret < 0)
        return ret;
      gtp_item = items;
    ...
    case RTE_FLOW_ITEM_TYPE_GTP_PSC:
      ret = flow_dv_validate_item_gtp_psc(items, last_item, gtp_item, attr,
                                          error);
      if (ret < 0)
        return ret;
    ...
    case RTE_FLOW_ITEM_TYPE_ECPRI:
      ...
      ret = mlx5_flow_validate_item_ecpri(items, item_flags, last_item,
                                          ether_type, &nic_ecpri_mask, error);
      if (ret < 0)
        return ret;
      last_item = MLX5_FLOW_LAYER_ECPRI;
      break;
    case RTE_FLOW_ITEM_TYPE_INTEGRITY:
      ret = flow_dv_validate_item_integrity(dev, items, item_flags, &last_item,
                                            integrity_items, error);
      if (ret < 0)
        return ret;
      break;
    case RTE_FLOW_ITEM_TYPE_CONNTRACK:
      ret = flow_dv_validate_item_aso_ct(dev, items, &item_flags, error);
      if (ret < 0)
        return ret;
      break;
    ...
    case RTE_FLOW_ITEM_TYPE_FLEX:
      ret = flow_dv_validate_item_flex(dev, items, item_flags, &last_item,
                                       tunnel != 0, error);
      if (ret < 0)
        return ret;
      break;
  ...
  if (item_flags & MLX5_FLOW_ITEM_INTEGRITY) {
    ret = flow_dv_validate_item_integrity_post(integrity_items, item_flags,
                                               error);
    if (ret)
      return ret;
  }
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_PORT_ID:
    case RTE_FLOW_ACTION_TYPE_REPRESENTED_PORT:
      ret = flow_dv_validate_action_port_id(dev, action_flags, actions, attr,
                                            error);
      if (ret)
        return ret;
    ...
    case RTE_FLOW_ACTION_TYPE_FLAG:
      ret = flow_dv_validate_action_flag(dev, action_flags, attr, error);
      if (ret < 0)
        return ret;
      if (dev_conf->dv_xmeta_en != MLX5_XMETA_MODE_LEGACY) {
        ...
        if (!(action_flags & MLX5_FLOW_MODIFY_HDR_ACTIONS))
          ++actions_n;
        action_flags |= MLX5_FLOW_ACTION_FLAG | MLX5_FLOW_ACTION_MARK_EXT;
        if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
    ...
    case RTE_FLOW_ACTION_TYPE_MARK:
      ret =
          flow_dv_validate_action_mark(dev, actions, action_flags, attr, error);
      if (ret < 0)
        return ret;
      if (dev_conf->dv_xmeta_en != MLX5_XMETA_MODE_LEGACY) {
        /* Count all modify-header actions as one. */
        if (!(action_flags & MLX5_FLOW_MODIFY_HDR_ACTIONS))
          ++actions_n;
        action_flags |= MLX5_FLOW_ACTION_MARK | MLX5_FLOW_ACTION_MARK_EXT;
        if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
          modify_after_mirror = 1;
      } else {
    ...
    case RTE_FLOW_ACTION_TYPE_SET_META:
      ret = flow_dv_validate_action_set_meta(dev, actions, action_flags, attr,
                                             error);
      if (ret < 0)
        return ret;
    ...
    case RTE_FLOW_ACTION_TYPE_SET_TAG:
      ret = flow_dv_validate_action_set_tag(dev, actions, action_flags, attr,
                                            error);
      if (ret < 0)
        return ret;
      /* Count all modify-header actions as one action. */
    ...
    case RTE_FLOW_ACTION_TYPE_DROP:
      ret = mlx5_flow_validate_action_drop(action_flags, attr, error);
      if (ret < 0)
        return ret;
      ...
      break;
    case RTE_FLOW_ACTION_TYPE_QUEUE:
      ret = mlx5_flow_validate_action_queue(actions, action_flags, dev, attr,
                                            error);
      if (ret < 0)
        return ret;
      queue_index =
          ((const struct rte_flow_action_queue *)(actions->conf))->index;
      action_flags |= MLX5_FLOW_ACTION_QUEUE;
      ++actions_n;
      break;
    case RTE_FLOW_ACTION_TYPE_RSS:
      rss = actions->conf;
      ret = mlx5_flow_validate_action_rss(actions, action_flags, dev, attr,
                                          item_flags, error);
      if (ret < 0)
        return ret;
      if (rss && sample_rss &&
          (sample_rss->level != rss->level || sample_rss->types != rss->types))
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL,
                                  "Can't use the different RSS types "
                                  "or level in the same flow");
    ...
    case MLX5_RTE_FLOW_ACTION_TYPE_DEFAULT_MISS:
      ret = mlx5_flow_validate_action_default_miss(action_flags, attr, error);
      if (ret < 0)
        return ret;
    ...
    case MLX5_RTE_FLOW_ACTION_TYPE_COUNT:
      ...
      /* fall-through. */
    case RTE_FLOW_ACTION_TYPE_COUNT:
      ret = flow_dv_validate_action_count(dev, shared_count, action_flags, attr,
                                          error);
      if (ret < 0)
        return ret;
      action_flags |= MLX5_FLOW_ACTION_COUNT;
    ...
    case RTE_FLOW_ACTION_TYPE_OF_POP_VLAN:
      if (flow_dv_validate_action_pop_vlan(dev, action_flags, actions,
    ...
    case RTE_FLOW_ACTION_TYPE_OF_PUSH_VLAN:
      ret = flow_dv_validate_action_push_vlan(dev, action_flags, vlan_m,
                                              actions, attr, error);
      if (ret < 0)
        return ret;
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
    ...
    case RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_PCP:
      ret = flow_dv_validate_action_set_vlan_pcp(action_flags, actions, error);
      if (ret < 0)
        return ret;
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
        modify_after_mirror = 1;
    ...
    case RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_VID:
      ret = flow_dv_validate_action_set_vlan_vid(item_flags, action_flags,
                                                 actions, error);
      if (ret < 0)
        return ret;
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
    ...
    case RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP:
    case RTE_FLOW_ACTION_TYPE_NVGRE_ENCAP:
      ret = flow_dv_validate_action_l2_encap(dev, action_flags, actions, attr,
                                             error);
      if (ret < 0)
        return ret;
      action_flags |= MLX5_FLOW_ACTION_ENCAP;
    ...
    case RTE_FLOW_ACTION_TYPE_NVGRE_DECAP:
      ret = flow_dv_validate_action_decap(dev, action_flags, actions,
                                          item_flags, attr, error);
      if (ret < 0)
        return ret;
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
    ...
    case RTE_FLOW_ACTION_TYPE_RAW_ENCAP:
      ret = flow_dv_validate_action_raw_encap_decap(
          dev, NULL, actions->conf, attr, &action_flags, &actions_n, actions,
          item_flags, error);
      if (ret < 0)
        return ret;
      break;
    case RTE_FLOW_ACTION_TYPE_RAW_DECAP:
      ...
      ret = flow_dv_validate_action_raw_encap_decap(
          dev, decap ? decap : &empty_decap, encap, attr, &action_flags,
    ...
    case RTE_FLOW_ACTION_TYPE_SET_MAC_DST:
      ret = flow_dv_validate_action_modify_mac(action_flags, actions,
                                               item_flags, error);
      if (ret < 0)
        return ret;
      /* Count all modify-header actions as one action. */
      if (!(action_flags & MLX5_FLOW_MODIFY_HDR_ACTIONS))
        ++actions_n;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_SET_MAC_SRC
    ...
    case RTE_FLOW_ACTION_TYPE_SET_IPV4_DST:
      ret = flow_dv_validate_action_modify_ipv4(action_flags, actions,
                                                item_flags, error);
      if (ret < 0)
        return ret;
      /* Count all modify-header actions as one action. */
      ...
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
        modify_after_mirror = 1;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_SET_IPV4_SRC
    ...
    case RTE_FLOW_ACTION_TYPE_SET_IPV6_DST:
      ret = flow_dv_validate_action_modify_ipv6(action_flags, actions,
                                                item_flags, error);
      if (ret < 0)
        return ret;
      ...
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
        modify_after_mirror = 1;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_SET_IPV6_SRC
    ...
    case RTE_FLOW_ACTION_TYPE_SET_TP_DST:
      ret = flow_dv_validate_action_modify_tp(action_flags, actions, item_flags,
                                              error);
      if (ret < 0)
        return ret;
      /* Count all modify-header actions as one action. */
      ...
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
        modify_after_mirror = 1;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_SET_TP_SRC
    ...
    case RTE_FLOW_ACTION_TYPE_SET_TTL:
      ret = flow_dv_validate_action_modify_ttl(action_flags, actions,
                                               item_flags, error);
      if (ret < 0)
        return ret;
      /* Count all modify-header actions as one action. */
      ...
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
        modify_after_mirror = 1;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_SET_TTL
                          ? MLX5_FLOW_ACTION_SET_TTL
                          : MLX5_FLOW_ACTION_DEC_TTL;
      rw_act_num += MLX5_ACT_NUM_MDF_TTL;
      break;
    case RTE_FLOW_ACTION_TYPE_JUMP:
      ret = flow_dv_validate_action_jump(dev, tunnel, actions, action_flags,
                                         attr, external, error);
      if (ret)
        return ret;
    ...
    case RTE_FLOW_ACTION_TYPE_DEC_TCP_SEQ:
      ret = flow_dv_validate_action_modify_tcp_seq(action_flags, actions,
                                                   item_flags, error);
      if (ret < 0)
        return ret;
      /* Count all modify-header actions as one action. */
      ...
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
        modify_after_mirror = 1;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_INC_TCP_SEQ
    ...
    case RTE_FLOW_ACTION_TYPE_DEC_TCP_ACK:
      ret = flow_dv_validate_action_modify_tcp_ack(action_flags, actions,
                                                   item_flags, error);
      if (ret < 0)
        return ret;
      /* Count all modify-header actions as one action. */
      ...
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
        modify_after_mirror = 1;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_INC_TCP_ACK
    ...
    case RTE_FLOW_ACTION_TYPE_METER:
      ret = mlx5_flow_validate_action_meter(dev, action_flags, item_flags,
                                            actions, attr, port_id_item,
                                            &def_policy, error);
      if (ret < 0)
        return ret;
      action_flags |= MLX5_FLOW_ACTION_METER;
      if (!def_policy)
        action_flags |= MLX5_FLOW_ACTION_METER_WITH_TERMINATED_POLICY;
      ++actions_n;
    ...
    case MLX5_RTE_FLOW_ACTION_TYPE_AGE:
      ...
      if (action_flags & MLX5_FLOW_ACTION_AGE)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL, "duplicate age actions set");
      action_flags |= MLX5_FLOW_ACTION_AGE;
      ++actions_n;
      break;
    case RTE_FLOW_ACTION_TYPE_AGE:
      ret = flow_dv_validate_action_age(action_flags, actions, dev, error);
      if (ret < 0)
        return ret;
      /*
    ...
    case RTE_FLOW_ACTION_TYPE_SET_IPV4_DSCP:
      ret = flow_dv_validate_action_modify_ipv4_dscp(action_flags, actions,
                                                     item_flags, error);
      if (ret < 0)
        return ret;
      /* Count all modify-header actions as one action. */
    ...
    case RTE_FLOW_ACTION_TYPE_SET_IPV6_DSCP:
      ret = flow_dv_validate_action_modify_ipv6_dscp(action_flags, actions,
                                                     item_flags, error);
      if (ret < 0)
        return ret;
      /* Count all modify-header actions as one action. */
    ...
    case RTE_FLOW_ACTION_TYPE_SAMPLE:
      ret = flow_dv_validate_action_sample(
          &action_flags, actions, dev, attr, item_flags, rss, &sample_rss,
          &sample_count, &fdb_mirror_limit, error);
      if (ret < 0)
        return ret;
    ...
    case RTE_FLOW_ACTION_TYPE_MODIFY_FIELD:
      ret = flow_dv_validate_action_modify_field(dev, action_flags, actions,
                                                 attr, error);
      if (ret < 0)
        return ret;
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
    ...
    case RTE_FLOW_ACTION_TYPE_CONNTRACK:
      ret = flow_dv_validate_action_aso_ct(dev, action_flags, item_flags, attr,
                                           error);
      if (ret < 0)
        return ret;
  ...
  if (action_flags & MLX5_FLOW_ACTION_TUNNEL_SET) {
    uint64_t bad_actions_mask = MLX5_FLOW_ACTION_DECAP | MLX5_FLOW_ACTION_MARK |
  ...
  if (action_flags & MLX5_FLOW_ACTION_TUNNEL_MATCH) {
    uint64_t bad_actions_mask = MLX5_FLOW_ACTION_JUMP | MLX5_FLOW_ACTION_MARK |
                                MLX5_FLOW_ACTION_SET_TAG |
                                MLX5_FLOW_ACTION_SET_META;
   ...
   */
  if (action_flags &
      (MLX5_FLOW_ACTION_TUNNEL_MATCH | MLX5_FLOW_ACTION_TUNNEL_MATCH))
    ;
  ...
  if (attr->transfer) {
    ...
    if (!(action_flags & MLX5_FLOW_FATE_ESWITCH_ACTIONS))
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                actions, "no fate action is found");
  } else {
   ...
   */
  if ((action_flags & (MLX5_FLOW_XCAP_ACTIONS | MLX5_FLOW_VLAN_ACTIONS)) &&
      (queue_index == 0xFFFF || !mlx5_rxq_is_hairpin(dev, queue_index) ||
       ((conf = mlx5_rxq_get_hairpin_conf(dev, queue_index)) != NULL &&
        conf->tx_explicit != 0))) {
    if ((action_flags & MLX5_FLOW_XCAP_ACTIONS) == MLX5_FLOW_XCAP_ACTIONS)
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                NULL,
                                "encap and decap "
  ...
  if (action_flags & MLX5_FLOW_ACTION_METER_WITH_TERMINATED_POLICY) {
    if ((action_flags & (MLX5_FLOW_FATE_ACTIONS &
      ...
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                NULL,
                                "fate action not supported for "
                                "meter with policy");
    if (attr->egress) {
      if (action_flags & MLX5_FLOW_MODIFY_HDR_ACTIONS)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL,
                                  "modify header action in egress "
                                  "cannot be done before meter action");
      if (action_flags & MLX5_FLOW_ACTION_ENCAP)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL,
                                  "encap action in egress "
                                  "cannot be done before meter action");
      if (action_flags & MLX5_FLOW_ACTION_OF_PUSH_VLAN)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL,
                                  "push vlan action in egress "
                                  "cannot be done before meter action");
    }
  }
  /*
  ...
  if (dev_conf->dv_flow_en && dev_conf->dv_xmeta_en != MLX5_XMETA_MODE_LEGACY &&
      mlx5_flow_ext_mreg_supported(dev))
    rw_act_num += MLX5_ACT_NUM_SET_TAG;
  if (rw_act_num > flow_dv_modify_hdr_action_max(dev, is_root)) {
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                              "too many header modify"
                              " actions to support");
  }
...
flow_dv_prepare(struct rte_eth_dev *dev,
  ...
  if (wks->flow_idx >= MLX5_NUM_MAX_DEV_FLOWS) {
    rte_flow_error_set(error, ENOSPC, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                       "not free temporary device flow");
    return NULL;
  }
  dev_handle =
      mlx5_ipool_zmalloc(priv->sh->ipool[MLX5_IPOOL_MLX5_FLOW], &handle_idx);
  if (!dev_handle) {
    rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
 ...
 */
static int flow_dv_check_valid_spec(void *match_mask, void *match_value) {
 ...
 */
static inline void flow_dv_set_match_ip_version(uint32_t group, void *headers_v,
                                                void *headers_m,
                                                uint8_t ip_version) {
  if (group == 0)
    MLX5_SET(fte_match_set_lyr_2_4, headers_m, ip_version, 0xf);
  else
    MLX5_SET(fte_match_set_lyr_2_4, headers_m, ip_version, ip_version);
 ...
 */
static void flow_dv_translate_item_eth(void *matcher, void *key,
                                       const struct rte_flow_item *item,
  ...
  if (!eth_m)
    eth_m = &nic_mask;
  if (inner) {
    hdrs_m = MLX5_ADDR_OF(fte_match_param, matcher, inner_headers);
  ...
  memcpy(MLX5_ADDR_OF(fte_match_set_lyr_2_4, hdrs_m, dmac_47_16), &eth_m->dst,
         sizeof(eth_m->dst));
  ...
  memcpy(MLX5_ADDR_OF(fte_match_set_lyr_2_4, hdrs_m, smac_47_16), &eth_m->src,
         sizeof(eth_m->src));
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_vlan(struct mlx5_flow *dev_flow,
  ...
  if (inner) {
     ...
     */
    if (vlan_v)
      dev_flow->handle->vf_vlan.tag = rte_be_to_cpu_16(vlan_v->tci) & 0x0fff;
  }
  /*
 ...
 */
static void flow_dv_translate_item_ipv4(void *matcher, void *key,
                                        const struct rte_flow_item *item,
                                        int inner, uint32_t group) {
  ...
  if (inner) {
    headers_m = MLX5_ADDR_OF(fte_match_param, matcher, inner_headers);
    headers_v = MLX5_ADDR_OF(fte_match_param, key, inner_headers);
  } else {
    headers_m = MLX5_ADDR_OF(fte_match_param, matcher, outer_headers);
 ...
 *   The group to insert the rule.
 */
static void flow_dv_translate_item_ipv6(void *matcher, void *key,
                                        const struct rte_flow_item *item,
                                        int inner, uint32_t group) {
  ...
  /* Label. */
  if (inner) {
    MLX5_SET(fte_match_set_misc, misc_m, inner_ipv6_flow_label, vtc_m);
    MLX5_SET(fte_match_set_misc, misc_v, inner_ipv6_flow_label, vtc_v);
  } else {
    MLX5_SET(fte_match_set_misc, misc_m, outer_ipv6_flow_label, vtc_m);
    MLX5_SET(fte_match_set_misc, misc_v, outer_ipv6_flow_label, vtc_v);
  }
  /* Protocol. */
  MLX5_SET(fte_match_set_lyr_2_4, headers_m, ip_protocol, ipv6_m->hdr.proto);
  MLX5_SET(fte_match_set_lyr_2_4, headers_v, ip_protocol,
  ...
  MLX5_SET(fte_match_set_lyr_2_4, headers_m, frag, !!(ipv6_m->has_frag_ext));
  MLX5_SET(fte_match_set_lyr_2_4, headers_v, frag,
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_ipv6_frag_ext(
    void *matcher, void *key, const struct rte_flow_item *item, int inner) {
  ...
  MLX5_SET(fte_match_set_lyr_2_4, headers_v, ip_protocol,
           ipv6_frag_ext_v->hdr.next_header & ipv6_frag_ext_m->hdr.next_header);
}
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_tcp(void *matcher, void *key,
                                       const struct rte_flow_item *item,
                                       int inner) {
  ...
  MLX5_SET(fte_match_set_lyr_2_4, headers_m, tcp_flags, tcp_m->hdr.tcp_flags);
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_udp(void *matcher, void *key,
                                       const struct rte_flow_item *item,
                                       int inner) {
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_gre_key(void *matcher, void *key,
                                           const struct rte_flow_item *item) {
 ...
 */
static void flow_dv_translate_item_gre(void *matcher, void *key,
                                       const struct rte_flow_item *item,
                                       uint64_t pattern_flags) {
  static const struct rte_flow_item_gre empty_gre = {
      0,
  };
  ...
  MLX5_SET(fte_match_set_misc, misc_v, gre_c_present,
           gre_crks_rsvd0_ver_v.c_present & gre_crks_rsvd0_ver_m.c_present);
  ...
  MLX5_SET(fte_match_set_misc, misc_v, gre_k_present,
           gre_crks_rsvd0_ver_v.k_present & gre_crks_rsvd0_ver_m.k_present);
  ...
  MLX5_SET(fte_match_set_misc, misc_v, gre_s_present,
           gre_crks_rsvd0_ver_v.s_present & gre_crks_rsvd0_ver_m.s_present);
  ...
  MLX5_SET(fte_match_set_misc, misc_v, gre_protocol, protocol_m & protocol_v);
}
 ...
 *   Accumulated pattern flags.
 */
static void flow_dv_translate_item_gre_option(
    void *matcher, void *key, const struct rte_flow_item *item,
    const struct rte_flow_item *gre_item, uint64_t pattern_flags) {
   ...
   * not need using misc.
   */
  if (!(option_m->sequence.sequence || option_m->checksum_rsvd.checksum)) {
 ...
 *   Accumulated pattern flags.
 */
static void flow_dv_translate_item_nvgre(void *matcher, void *key,
                                         const struct rte_flow_item *item,
  ...
  const struct rte_flow_item_gre gre_spec = {.c_rsvd0_ver = RTE_BE16(0x2000),
                                             .protocol =
                                                 RTE_BE16(RTE_ETHER_TYPE_TEB)};
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_vxlan(struct rte_eth_dev *dev,
                                         const struct rte_flow_attr *attr,
                                         void *matcher, void *key,
                                         const struct rte_flow_item *item,
                                         int inner) {
  ...
  dport = item->type == RTE_FLOW_ITEM_TYPE_VXLAN ? MLX5_UDP_PORT_VXLAN
  ...
  if ((priv->sh->steering_format_version ==
    ...
    int size;
    int i;
    misc_m = MLX5_ADDR_OF(fte_match_param, matcher, misc_parameters);
  ...
  misc5_v = MLX5_ADDR_OF(fte_match_param, key, misc_parameters_5);
  tunnel_header_v =
      (uint32_t *)MLX5_ADDR_OF(fte_match_set_misc5, misc5_v, tunnel_header_1);
  tunnel_header_m =
      (uint32_t *)MLX5_ADDR_OF(fte_match_set_misc5, misc5_m, tunnel_header_1);
  *tunnel_header_v = (vxlan_v->vni[0] & vxlan_m->vni[0]) |
                     (vxlan_v->vni[1] & vxlan_m->vni[1]) << 8 |
                     (vxlan_v->vni[2] & vxlan_m->vni[2]) << 16;
  if (*tunnel_header_v)
    *tunnel_header_m =
        vxlan_m->vni[0] | vxlan_m->vni[1] << 8 | vxlan_m->vni[2] << 16;
  else
    *tunnel_header_m = 0x0;
...
static void flow_dv_translate_item_vxlan_gpe(void *matcher, void *key,
                                             const struct rte_flow_item *item,
                                             const uint64_t pattern_flags) {
  static const struct rte_flow_item_vxlan_gpe dummy_vxlan_gpe_hdr = {
      0,
  };
  ...
  void *headers_m = MLX5_ADDR_OF(fte_match_param, matcher, outer_headers);
  void *headers_v = MLX5_ADDR_OF(fte_match_param, key, outer_headers);
  void *misc_m = MLX5_ADDR_OF(fte_match_param, matcher, misc_parameters_3);
  void *misc_v = MLX5_ADDR_OF(fte_match_param, key, misc_parameters_3);
  char *vni_m = MLX5_ADDR_OF(fte_match_set_misc3, misc_m, outer_vxlan_gpe_vni);
  char *vni_v = MLX5_ADDR_OF(fte_match_set_misc3, misc_v, outer_vxlan_gpe_vni);
...
static void flow_dv_translate_item_geneve(void *matcher, void *key,
                                          const struct rte_flow_item *item,
                                          uint64_t pattern_flags) {
  static const struct rte_flow_item_geneve empty_geneve = {
      0,
  };
  ...
  if (!MLX5_GET16(fte_match_set_lyr_2_4, headers_v, udp_dport)) {
    MLX5_SET(fte_match_set_lyr_2_4, headers_m, udp_dport, 0xFFFF);
    MLX5_SET(fte_match_set_lyr_2_4, headers_v, udp_dport, MLX5_UDP_PORT_GENEVE);
  }
  if (!geneve_v) {
  ...
  MLX5_SET(fte_match_set_misc, misc_v, geneve_opt_len,
           MLX5_GENEVE_OPTLEN_VAL(gbhdr_v) & MLX5_GENEVE_OPTLEN_VAL(gbhdr_m));
...
int flow_dev_geneve_tlv_option_resource_register(
    struct rte_eth_dev *dev, const struct rte_flow_item *item,
    struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_dev_ctx_shared *sh = priv->sh;
  ...
  if (geneve_opt_resource != NULL) {
    if (geneve_opt_resource->option_class == geneve_opt_v->option_class &&
      ...
      __atomic_fetch_add(&geneve_opt_resource->refcnt, 1, __ATOMIC_RELAXED);
    } else {
      ret = rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                               NULL, "Only one GENEVE TLV option supported");
      goto exit;
    }
  } else {
    /* Create a GENEVE TLV object and resource. */
    obj = mlx5_devx_cmd_create_geneve_tlv_option(
        sh->cdev->ctx, geneve_opt_v->option_class, geneve_opt_v->option_type,
        geneve_opt_v->option_len);
    if (!obj) {
      ret = rte_flow_error_set(error, ENODATA, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                               NULL, "Failed to create GENEVE TLV Devx object");
      goto exit;
    }
    sh->geneve_tlv_option_resource = mlx5_malloc(
        MLX5_MEM_ZERO, sizeof(*geneve_opt_resource), 0, SOCKET_ID_ANY);
    ...
    geneve_opt_resource->option_type = geneve_opt_v->option_type;
    geneve_opt_resource->length = geneve_opt_v->option_len;
    __atomic_store_n(&geneve_opt_resource->refcnt, 1, __ATOMIC_RELAXED);
  }
exit:
 ...
 *   Pointer to error structure.
 */
static int flow_dv_translate_item_geneve_opt(struct rte_eth_dev *dev,
                                             void *matcher, void *key,
                                             const struct rte_flow_item *item,
                                             struct rte_flow_error *error) {
  ...
  void *misc_m = MLX5_ADDR_OF(fte_match_param, matcher, misc_parameters);
  void *misc_v = MLX5_ADDR_OF(fte_match_param, key, misc_parameters);
  void *misc3_m = MLX5_ADDR_OF(fte_match_param, matcher, misc_parameters_3);
  ...
  ret = flow_dev_geneve_tlv_option_resource_register(dev, item, error);
  if (ret) {
  ...
  if (geneve_opt_v->data) {
    ...
    MLX5_ASSERT((uint32_t)(geneve_opt_v->option_len * 4) <=
                sizeof(opt_data_mask));
    MLX5_SET(fte_match_set_misc3, misc3_m, geneve_tlv_option_0_data,
             rte_be_to_cpu_32(opt_data_mask));
    MLX5_SET(fte_match_set_misc3, misc3_v, geneve_tlv_option_0_data,
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_mpls(void *matcher, void *key,
                                        const struct rte_flow_item *item,
  ...
  void *misc_m = MLX5_ADDR_OF(fte_match_param, matcher, misc_parameters);
  void *misc_v = MLX5_ADDR_OF(fte_match_param, key, misc_parameters);
  void *misc2_m = MLX5_ADDR_OF(fte_match_param, matcher, misc_parameters_2);
  ...
  case MLX5_FLOW_LAYER_OUTER_L4_UDP:
    if (!MLX5_GET16(fte_match_set_lyr_2_4, headers_v, udp_dport)) {
      MLX5_SET(fte_match_set_lyr_2_4, headers_m, udp_dport, 0xffff);
      MLX5_SET(fte_match_set_lyr_2_4, headers_v, udp_dport, MLX5_UDP_PORT_MPLS);
    }
    break;
  ...
  case MLX5_FLOW_LAYER_GRE_KEY:
    if (!MLX5_GET16(fte_match_set_misc, misc_v, gre_protocol)) {
      ...
      MLX5_SET(fte_match_set_misc, misc_v, gre_protocol, RTE_ETHER_TYPE_MPLS);
    }
    break;
  default:
    break;
  ...
  switch (prev_layer) {
  case MLX5_FLOW_LAYER_OUTER_L4_UDP:
    out_mpls_m = (uint32_t *)MLX5_ADDR_OF(fte_match_set_misc2, misc2_m,
                                          outer_first_mpls_over_udp);
    out_mpls_v = (uint32_t *)MLX5_ADDR_OF(fte_match_set_misc2, misc2_v,
                                          outer_first_mpls_over_udp);
    break;
  case MLX5_FLOW_LAYER_GRE:
    out_mpls_m = (uint32_t *)MLX5_ADDR_OF(fte_match_set_misc2, misc2_m,
 ...
 *   Register mask
 */
static void flow_dv_match_meta_reg(void *matcher, void *key,
                                   enum modify_reg reg_type, uint32_t data,
                                   uint32_t mask) {
  void *misc2_m = MLX5_ADDR_OF(fte_match_param, matcher, misc_parameters_2);
  void *misc2_v = MLX5_ADDR_OF(fte_match_param, key, misc_parameters_2);
  uint32_t temp;
 ...
 *   Flow pattern to translate.
 */
static void flow_dv_translate_item_mark(struct rte_eth_dev *dev, void *matcher,
                                        void *key,
                                        const struct rte_flow_item *item) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  mark = item->mask ? (const void *)item->mask : &rte_flow_item_mark_mask;
 ...
 *   Flow pattern to translate.
 */
static void flow_dv_translate_item_meta(struct rte_eth_dev *dev, void *matcher,
                                        void *key,
                                        const struct rte_flow_attr *attr,
                                        const struct rte_flow_item *item) {
 ...
 *   Flow pattern to translate.
 */
static void flow_dv_translate_item_meta_vport(void *matcher, void *key,
 ...
 *   Flow pattern to translate.
 */
static void flow_dv_translate_mlx5_item_tag(struct rte_eth_dev *dev,
                                            void *matcher, void *key,
                                            const struct rte_flow_item *item) {
 ...
 *   Flow pattern to translate.
 */
static void flow_dv_translate_item_tag(struct rte_eth_dev *dev, void *matcher,
                                       void *key,
                                       const struct rte_flow_item *item) {
 ...
 *   Mask
 */
static void flow_dv_translate_item_source_vport(void *matcher, void *key,
 ...
 */
static int flow_dv_translate_item_port_id(struct rte_eth_dev *dev,
                                          void *matcher, void *key,
                                          const struct rte_flow_item *item,
                                          const struct rte_flow_attr *attr) {
  ...
  if (pid_v && pid_v->id == MLX5_PORT_ESW_MGR) {
    flow_dv_translate_item_source_vport(
        matcher, key, flow_dv_get_esw_manager_vport_id(dev), 0xffff);
    return 0;
  }
  ...
  if (priv->vport_meta_mask) {
     ...
     */
    if (mask == 0xffff && priv->vport_id == 0xffff && priv->pf_bond < 0 &&
        attr->transfer)
      flow_dv_translate_item_source_vport(matcher, key, priv->vport_id, mask);
    /*
     ...
     */
    flow_dv_translate_item_meta_vport(matcher, key, priv->vport_meta_tag,
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_icmp6(void *matcher, void *key,
                                         const struct rte_flow_item *item,
                                         int inner) {
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_icmp(void *matcher, void *key,
                                        const struct rte_flow_item *item,
                                        int inner) {
  ...
  MLX5_SET(fte_match_set_misc3, misc3_m, icmp_type, icmp_m->hdr.icmp_type);
  MLX5_SET(fte_match_set_misc3, misc3_v, icmp_type,
           icmp_v->hdr.icmp_type & icmp_m->hdr.icmp_type);
  MLX5_SET(fte_match_set_misc3, misc3_m, icmp_code, icmp_m->hdr.icmp_code);
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_gtp(void *matcher, void *key,
                                       const struct rte_flow_item *item,
                                       int inner) {
  ...
  MLX5_SET(fte_match_set_misc3, misc3_m, gtpu_msg_flags, gtp_m->v_pt_rsv_flags);
 ...
 *   Flow pattern to translate.
 */
static int flow_dv_translate_item_gtp_psc(void *matcher, void *key,
                                          const struct rte_flow_item *item) {
  ...
  MLX5_SET(fte_match_set_misc3, misc3_m, gtpu_dw_2, rte_cpu_to_be_32(dw_2.w32));
  dw_2.seq_num = 0;
  dw_2.npdu_num = 0;
  dw_2.next_ext_header_type = 0x85;
  MLX5_SET(fte_match_set_misc3, misc3_v, gtpu_dw_2, rte_cpu_to_be_32(dw_2.w32));
  if (gtp_psc_v) {
    union {
      uint32_t w32;
      struct {
    ...
    dw_0.w32 = 0;
    dw_0.type_flags =
        MLX5_GTP_PDU_TYPE_SHIFT(gtp_psc_v->hdr.type & gtp_psc_m->hdr.type);
 ...
 *   Last item flags.
 */
static void flow_dv_translate_item_ecpri(struct rte_eth_dev *dev, void *matcher,
                                         void *key,
                                         const struct rte_flow_item *item,
                                         uint64_t last_item) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  void *misc4_m = MLX5_ADDR_OF(fte_match_param, matcher, misc_parameters_4);
  ...
  dw_m = MLX5_ADDR_OF(fte_match_set_misc4, misc4_m, prog_sample_field_value_0);
  dw_v = MLX5_ADDR_OF(fte_match_set_misc4, misc4_v, prog_sample_field_value_0);
  ...
  *(uint32_t *)dw_v = ecpri_v->hdr.common.u32 & ecpri_m->hdr.common.u32;
  ...
  MLX5_SET(fte_match_set_misc4, misc4_m, prog_sample_field_id_0, samples[0]);
  /* It makes no sense to set the sample ID in the mask field. */
  MLX5_SET(fte_match_set_misc4, misc4_v, prog_sample_field_id_0, samples[0]);
  /*
  ...
  if (ecpri_m->hdr.dummy[0]) {
    ...
    case RTE_ECPRI_MSG_TYPE_DLY_MSR:
      dw_m =
          MLX5_ADDR_OF(fte_match_set_misc4, misc4_m, prog_sample_field_value_1);
      dw_v =
          MLX5_ADDR_OF(fte_match_set_misc4, misc4_v, prog_sample_field_value_1);
      ...
      *(uint32_t *)dw_v = ecpri_v->hdr.dummy[0] & ecpri_m->hdr.dummy[0];
      ...
      MLX5_SET(fte_match_set_misc4, misc4_m, prog_sample_field_id_1,
               samples[1]);
      MLX5_SET(fte_match_set_misc4, misc4_v, prog_sample_field_id_1,
               samples[1]);
      break;
    default:
 ...
 *   Flow pattern to translate.
 */
static void flow_dv_translate_item_aso_ct(struct rte_eth_dev *dev,
                                          void *matcher, void *key,
                                          const struct rte_flow_item *item) {
  ...
  if (reg_id == REG_NON)
    return;
  flow_dv_match_meta_reg(matcher, key, (enum modify_reg)reg_id, reg_value,
                         reg_mask);
}

static void flow_dv_translate_item_flex(struct rte_eth_dev *dev, void *matcher,
                                        void *key,
                                        const struct rte_flow_item *item,
                                        struct mlx5_flow *dev_flow,
                                        bool is_inner) {
 ...
 */
static uint8_t flow_dv_matcher_enable(uint32_t *match_criteria) {
  uint8_t match_criteria_enable;

  match_criteria_enable = (!HEADER_IS_ZERO(match_criteria, outer_headers))
                          << MLX5_MATCH_CRITERIA_ENABLE_OUTER_BIT;
  match_criteria_enable |= (!HEADER_IS_ZERO(match_criteria, misc_parameters))
                           << MLX5_MATCH_CRITERIA_ENABLE_MISC_BIT;
  match_criteria_enable |= (!HEADER_IS_ZERO(match_criteria, inner_headers))
                           << MLX5_MATCH_CRITERIA_ENABLE_INNER_BIT;
  match_criteria_enable |= (!HEADER_IS_ZERO(match_criteria, misc_parameters_2))
                           << MLX5_MATCH_CRITERIA_ENABLE_MISC2_BIT;
  match_criteria_enable |= (!HEADER_IS_ZERO(match_criteria, misc_parameters_3))
                           << MLX5_MATCH_CRITERIA_ENABLE_MISC3_BIT;
  match_criteria_enable |= (!HEADER_IS_ZERO(match_criteria, misc_parameters_4))
                           << MLX5_MATCH_CRITERIA_ENABLE_MISC4_BIT;
  match_criteria_enable |= (!HEADER_IS_ZERO(match_criteria, misc_parameters_5))
                           << MLX5_MATCH_CRITERIA_ENABLE_MISC5_BIT;
  return match_criteria_enable;
}

static void __flow_dv_adjust_buf_size(size_t *size, uint8_t match_criteria) {
  /*
  ...
  if (!(match_criteria & (1 << MLX5_MATCH_CRITERIA_ENABLE_MISC5_BIT))) {
    *size = MLX5_ST_SZ_BYTES(fte_match_param) -
    ...
    if (!(match_criteria & (1 << MLX5_MATCH_CRITERIA_ENABLE_MISC4_BIT))) {
...
flow_dv_matcher_clone_cb(void *tool_ctx __rte_unused,
  ...
  struct mlx5_flow_tbl_data_entry *tbl =
      container_of(ref->tbl, typeof(*tbl), tbl);
  struct mlx5_flow_dv_matcher *resource =
      mlx5_malloc(MLX5_MEM_ANY, sizeof(*resource), 0, SOCKET_ID_ANY);
...
static void flow_dv_matcher_clone_free_cb(void *tool_ctx __rte_unused,
                                          struct mlx5_list_entry *entry) {
  mlx5_free(entry);
}
...
struct mlx5_list_entry *flow_dv_tbl_create_cb(void *tool_ctx, void *cb_ctx) {
  ...
  if (ret) {
    rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                       "cannot create flow table object");
    mlx5_ipool_free(sh->ipool[MLX5_IPOOL_JUMP], idx);
    return NULL;
  }
  if (key.level != 0) {
    ret = mlx5_flow_os_create_flow_action_dest_flow_tbl(tbl->obj,
  ...
  MKSTR(matcher_name, "%s_%s_%u_%u_matcher_list", key.is_fdb ? "FDB" : "NIC",
        key.is_egress ? "egress" : "ingress", key.level, key.id);
  tbl_data->matchers =
      mlx5_list_create(matcher_name, sh, true, flow_dv_matcher_create_cb,
                       flow_dv_matcher_match_cb, flow_dv_matcher_remove_cb,
                       flow_dv_matcher_clone_cb, flow_dv_matcher_clone_free_cb);
...
int flow_dv_tbl_match_cb(void *tool_ctx __rte_unused,
                         struct mlx5_list_entry *entry, void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
  struct mlx5_flow_tbl_data_entry *tbl_data =
  ...
  return tbl_data->level != key.level || tbl_data->id != key.id ||
         tbl_data->dummy != key.dummy ||
         tbl_data->is_transfer != !!key.is_fdb ||
         tbl_data->is_egress != !!key.is_egress;
}
...
flow_dv_tbl_resource_get(struct rte_eth_dev *dev, uint32_t table_level,
                         uint8_t egress, uint8_t transfer, bool external,
                         const struct mlx5_flow_tunnel *tunnel,
                         uint32_t group_id, uint8_t dummy, uint32_t table_id,
                         struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  DRV_LOG(DEBUG,
          "table_level %u table_id %u "
          "tunnel %u group %u registered.",
          table_level, table_id, tunnel ? tunnel->tunnel_id : 0, group_id);
...
void flow_dv_tbl_remove_cb(void *tool_ctx, struct mlx5_list_entry *entry) {
  ...
  if (tbl_data->tunnel_offload && tbl_data->external) {
    ...
    union tunnel_tbl_key tunnel_key = {
        .tunnel_id = tbl_data->tunnel ? tbl_data->tunnel->tunnel_id : 0,
    ...
    tunnel_grp_hash =
        tbl_data->tunnel ? tbl_data->tunnel->groups : thub->groups;
    he = mlx5_hlist_lookup(tunnel_grp_hash, tunnel_key.val, &ctx);
    if (he)
      mlx5_hlist_unregister(tunnel_grp_hash, he);
    DRV_LOG(DEBUG, "table_level %u id %u tunnel %u group %u released.",
            table_level, tbl_data->id,
            tbl_data->tunnel ? tbl_data->tunnel->tunnel_id : 0,
            tbl_data->group_id);
  }
  if (tbl_data->matchers)
...
int flow_dv_matcher_match_cb(void *tool_ctx __rte_unused,
                             struct mlx5_list_entry *entry, void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
  struct mlx5_flow_dv_matcher *ref = ctx->data;
  struct mlx5_flow_dv_matcher *cur = container_of(entry, typeof(*cur), entry);

  return cur->crc != ref->crc || cur->priority != ref->priority ||
         memcmp((const void *)cur->mask.buf, (const void *)ref->mask.buf,
                ref->mask.size);
}

struct mlx5_list_entry *flow_dv_matcher_create_cb(void *tool_ctx,
                                                  void *cb_ctx) {
  struct mlx5_dev_ctx_shared *sh = tool_ctx;
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
  struct mlx5_flow_dv_matcher *ref = ctx->data;
  ...
  resource = mlx5_malloc(MLX5_MEM_ZERO, sizeof(*resource), 0, SOCKET_ID_ANY);
  if (!resource) {
    rte_flow_error_set(ctx->error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                       NULL, "cannot create matcher");
    return NULL;
  ...
  *resource = *ref;
  dv_attr.match_criteria_enable = flow_dv_matcher_enable(resource->mask.buf);
  __flow_dv_adjust_buf_size(&ref->mask.size, dv_attr.match_criteria_enable);
  dv_attr.priority = ref->priority;
  if (tbl->is_egress)
    dv_attr.flags |= IBV_FLOW_ATTR_FLAGS_EGRESS;
  ret = mlx5_flow_os_create_flow_matcher(sh->cdev->ctx, &dv_attr, tbl->tbl.obj,
 ...
 *   0 on success otherwise -errno and errno is set.
 */
static int flow_dv_matcher_register(struct rte_eth_dev *dev,
   ...
   */
  tbl = flow_dv_tbl_resource_get(dev, key->level, key->is_egress, key->is_fdb,
                                 dev_flow->external, tunnel, group_id, 0,
                                 key->id, error);
  if (!tbl)
  ...
  if (!entry) {
    ...
    return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "cannot allocate ref memory");
  }
  resource = container_of(entry, typeof(*resource), entry);
...
struct mlx5_list_entry *flow_dv_tag_create_cb(void *tool_ctx, void *cb_ctx) {
  ...
  ret = mlx5_flow_os_create_flow_action_tag(entry->tag_id, &entry->action);
  if (ret) {
...
int flow_dv_tag_match_cb(void *tool_ctx __rte_unused,
                         struct mlx5_list_entry *entry, void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
...
struct mlx5_list_entry *flow_dv_tag_clone_cb(void *tool_ctx,
  ...
  if (!entry) {
    rte_flow_error_set(ctx->error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                       NULL, "cannot allocate tag resource memory");
    return NULL;
  }
 ...
 *   0 on success otherwise -errno and errno is set.
 */
static int flow_dv_tag_resource_register(struct rte_eth_dev *dev,
                                         uint32_t tag_be24,
                                         struct mlx5_flow *dev_flow,
                                         struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  tag_table = flow_dv_hlist_prepare(
      priv->sh, &priv->sh->tag_table, "tags", MLX5_TAGS_HLIST_ARRAY_SIZE, false,
      false, priv->sh, flow_dv_tag_create_cb, flow_dv_tag_match_cb,
      flow_dv_tag_remove_cb, flow_dv_tag_clone_cb, flow_dv_tag_clone_free_cb,
      error);
  if (unlikely(!tag_table))
    return -rte_errno;
  entry = mlx5_hlist_register(tag_table, tag_be24, &ctx);
  if (entry) {
    resource = container_of(entry, struct mlx5_flow_dv_tag_resource, entry);
 ...
 */
static int flow_dv_tag_release(struct rte_eth_dev *dev, uint32_t tag_idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_dv_tag_resource *tag;
  ...
  if (!tag)
    return 0;
  DRV_LOG(DEBUG, "port %u tag %p: refcnt %d--", dev->data->port_id, (void *)tag,
          tag->entry.ref_cnt);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_translate_action_port_id(
    struct rte_eth_dev *dev, const struct rte_flow_action *action,
    uint32_t *dst_port_id, struct rte_flow_error *error) {
  uint32_t port;
  struct mlx5_priv *priv;
  ...
  if (!priv)
    return rte_flow_error_set(error, -rte_errno, RTE_FLOW_ERROR_TYPE_ACTION,
                              NULL, "No eswitch info was found for port");
 ...
 */
static uint32_t flow_dv_translate_create_counter(
    struct rte_eth_dev *dev, struct mlx5_flow *dev_flow,
    const struct rte_flow_action_count *count __rte_unused,
  ...
  age_param = flow_dv_counter_idx_get_age(dev, counter);
  age_param->context =
      age->context ? age->context : (void *)(uintptr_t)(dev_flow->flow_idx);
 ...
 *   Item is inner pattern.
 */
static void flow_dv_translate_item_tx_queue(struct rte_eth_dev *dev,
                                            void *matcher, void *key,
                                            const struct rte_flow_item *item) {
...
void flow_dv_hashfields_set(uint64_t item_flags,
  ...
  *hash_fields = 0;
  ...
  if ((rss_inner && (items & MLX5_FLOW_LAYER_INNER_L3_IPV4)) ||
      (!rss_inner && (items & MLX5_FLOW_LAYER_OUTER_L3_IPV4)) || !items) {
  ...
  if ((rss_inner && (items & MLX5_FLOW_LAYER_INNER_L4_UDP)) ||
      (!rss_inner && (items & MLX5_FLOW_LAYER_OUTER_L4_UDP)) || !items) {
    if (rss_types & RTE_ETH_RSS_UDP) {
      if (rss_types & RTE_ETH_RSS_L4_SRC_ONLY)
        ...
        fields |= MLX5_UDP_IBV_RX_HASH;
             ...
             (!rss_inner && (items & MLX5_FLOW_LAYER_OUTER_L4_TCP)) || !items) {
...
flow_dv_hrxq_prepare(struct rte_eth_dev *dev, struct mlx5_flow *dev_flow,
                     struct mlx5_flow_rss_desc *rss_desc, uint32_t *hrxq_idx) {
...
flow_dv_sample_sub_actions_release(struct rte_eth_dev *dev,
  ...
  if (act_res->rix_port_id_action) {
    flow_dv_port_id_action_resource_release(dev, act_res->rix_port_id_action);
...
int flow_dv_sample_match_cb(void *tool_ctx __rte_unused,
                            struct mlx5_list_entry *entry, void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
  struct rte_eth_dev *dev = ctx->dev;
  ...
  if (ctx_resource->ratio == resource->ratio &&
      ...
      !memcmp((void *)&ctx_resource->sample_act, (void *)&resource->sample_act,
              sizeof(struct mlx5_flow_sub_actions_list))) {
    /*
     * Existing sample action should release the prepared
     * sub-actions reference counter.
     */
    flow_dv_sample_sub_actions_release(dev, &ctx_resource->sample_idx);
    return 0;
  }
  return 1;
}

struct mlx5_list_entry *flow_dv_sample_create_cb(void *tool_ctx __rte_unused,
                                                 void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
  struct rte_eth_dev *dev = ctx->dev;
  struct mlx5_flow_dv_sample_resource *ctx_resource = ctx->data;
  ...
  *resource = *ctx_resource;
  ...
  if (ctx_resource->ft_type == MLX5DV_FLOW_TABLE_TYPE_FDB)
    ...
    is_egress = 1;
  tbl = flow_dv_tbl_resource_get(dev, next_ft_id, is_egress, is_transfer, true,
                                 NULL, 0, 0, 0, error);
  if (!tbl) {
    rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
  ...
  sampler_attr.sample_actions =
      (struct mlx5dv_dr_action **)&sample_dv_actions[0];
  sampler_attr.action = resource->set_action;
  if (mlx5_os_flow_dr_create_flow_action_sampler(&sampler_attr,
...
error:
  ...
  if (resource->normal_path_tbl)
    flow_dv_tbl_resource_release(MLX5_SH(dev), resource->normal_path_tbl);
...
void flow_dv_sample_clone_free_cb(void *tool_ctx __rte_unused,
                                  struct mlx5_list_entry *entry) {
  struct mlx5_flow_dv_sample_resource *resource =
      container_of(entry, typeof(*resource), entry);
 ...
 *   0 on success otherwise -errno and errno is set.
 */
static int flow_dv_sample_resource_register(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_sample_resource *ref,
    struct mlx5_flow *dev_flow, struct rte_flow_error *error) {
  struct mlx5_flow_dv_sample_resource *resource;
  struct mlx5_list_entry *entry;
  struct mlx5_priv *priv = dev->data->dev_private;
...
int flow_dv_dest_array_match_cb(void *tool_ctx __rte_unused,
                                struct mlx5_list_entry *entry, void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
  ...
  if (ctx_resource->num_of_dest == resource->num_of_dest &&
      ctx_resource->ft_type == resource->ft_type &&
      !memcmp((void *)resource->sample_act, (void *)ctx_resource->sample_act,
    ...
    for (idx = 0; idx < ctx_resource->num_of_dest; idx++)
      flow_dv_sample_sub_actions_release(dev, &ctx_resource->sample_idx[idx]);
    return 0;
  }
  return 1;
}
...
flow_dv_dest_array_create_cb(void *tool_ctx __rte_unused, void *cb_ctx) {
  ...
  resource = mlx5_ipool_zmalloc(sh->ipool[MLX5_IPOOL_DEST_ARRAY], &res_idx);
  if (!resource) {
    rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                       "cannot allocate resource memory");
    return NULL;
  ...
  *resource = *ctx_resource;
  if (resource->ft_type == MLX5DV_FLOW_TABLE_TYPE_FDB)
    ...
    domain = sh->tx_domain;
  for (idx = 0; idx < ctx_resource->num_of_dest; idx++) {
    dest_attr[idx] = (struct mlx5dv_dr_action_dest_attr *)mlx5_malloc(
        MLX5_MEM_ZERO, sizeof(struct mlx5dv_dr_action_dest_attr), 0,
        SOCKET_ID_ANY);
    if (!dest_attr[idx]) {
      rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                         "cannot allocate resource memory");
    ...
    case (MLX5_FLOW_ACTION_PORT_ID | MLX5_FLOW_ACTION_ENCAP):
      ...
      dest_attr[idx]->dest_reformat->reformat = sample_act->dr_encap_action;
      dest_attr[idx]->dest_reformat->dest = sample_act->dr_port_id_action;
      break;
  ...
  ret = mlx5_os_flow_dr_create_flow_action_dest_array(
      domain, resource->num_of_dest, dest_attr, &resource->action);
  if (ret) {
    rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
...
error:
  for (idx = 0; idx < ctx_resource->num_of_dest; idx++) {
    flow_dv_sample_sub_actions_release(dev, &resource->sample_idx[idx]);
    if (dest_attr[idx])
      mlx5_free(dest_attr[idx]);
  }
...
void flow_dv_dest_array_clone_free_cb(void *tool_ctx __rte_unused,
                                      struct mlx5_list_entry *entry) {
  struct mlx5_flow_dv_dest_array_resource *resource =
      container_of(entry, typeof(*resource), entry);
  struct rte_eth_dev *dev = resource->dev;
 ...
 *   0 on success otherwise -errno and errno is set.
 */
static int flow_dv_dest_array_resource_register(
    struct rte_eth_dev *dev, struct mlx5_flow_dv_dest_array_resource *ref,
    struct mlx5_flow *dev_flow, struct rte_flow_error *error) {
  struct mlx5_flow_dv_dest_array_resource *resource;
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_list_entry *entry;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_translate_action_sample(
    struct rte_eth_dev *dev, const struct rte_flow_action_sample *action,
    struct mlx5_flow *dev_flow, const struct rte_flow_attr *attr,
    uint32_t *num_of_dest, void **sample_actions,
    struct mlx5_flow_dv_sample_resource *res, struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  for (; sub_actions->type != RTE_FLOW_ACTION_TYPE_END; sub_actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_QUEUE: {
      ...
      hrxq = flow_dv_hrxq_prepare(dev, dev_flow, rss_desc, &hrxq_idx);
      if (!hrxq)
        return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL, "cannot create fate queue");
      sample_act->dr_queue_action = hrxq->action;
      sample_idx->rix_hrxq = hrxq_idx;
      sample_actions[sample_act->actions_num++] = hrxq->action;
      (*num_of_dest)++;
      action_flags |= MLX5_FLOW_ACTION_QUEUE;
      if (action_flags & MLX5_FLOW_ACTION_MARK)
    ...
    case RTE_FLOW_ACTION_TYPE_RSS: {
       ...
       */
      flow_dv_hashfields_set(dev_flow->handle->layers, rss_desc,
    ...
    case RTE_FLOW_ACTION_TYPE_MARK: {
      uint32_t tag_be = mlx5_flow_mark_set(
          ((const struct rte_flow_action_mark *)(sub_actions->conf))->id);
      ...
      sample_act->dr_tag_action = dev_flow->dv.tag_resource->action;
      sample_idx->rix_tag = dev_flow->handle->dvh.rix_tag;
      sample_actions[sample_act->actions_num++] = sample_act->dr_tag_action;
    ...
    case RTE_FLOW_ACTION_TYPE_COUNT: {
      ...
      sample_act->dr_cnt_action =
          (flow_dv_counter_get_by_idx(dev, flow->counter, NULL))->action;
      sample_actions[sample_act->actions_num++] = sample_act->dr_cnt_action;
      action_flags |= MLX5_FLOW_ACTION_COUNT;
      break;
    }
    case RTE_FLOW_ACTION_TYPE_PORT_ID:
    case RTE_FLOW_ACTION_TYPE_REPRESENTED_PORT: {
      ...
      if (flow_dv_translate_action_port_id(dev, sub_actions, &port_id, error))
        return -rte_errno;
      port_id_resource.port_id = port_id;
      if (flow_dv_port_id_action_resource_register(dev, &port_id_resource,
                                                   dev_flow, error))
        return -rte_errno;
      sample_act->dr_port_id_action = dev_flow->dv.port_id_action->action;
      sample_idx->rix_port_id_action = dev_flow->handle->rix_port_id_action;
      sample_actions[sample_act->actions_num++] = sample_act->dr_port_id_action;
    ...
    case RTE_FLOW_ACTION_TYPE_RAW_ENCAP:
      ...
      if (flow_dv_create_action_l2_encap(dev, sub_actions, dev_flow,
                                         attr->transfer, error))
        return -rte_errno;
      sample_act->dr_encap_action = dev_flow->dv.encap_decap->action;
      sample_idx->rix_encap_decap = dev_flow->handle->dvh.rix_encap_decap;
      sample_actions[sample_act->actions_num++] = sample_act->dr_encap_action;
    ...
    default:
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Not support for sampler action");
    }
  }
  ...
  if (attr->transfer) {
    ...
    MLX5_SET(set_action_in, action_ctx.action_in, data, priv->vport_meta_tag);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_create_action_sample(
    struct rte_eth_dev *dev, struct mlx5_flow *dev_flow, uint32_t num_of_dest,
    struct mlx5_flow_dv_sample_resource *res,
    struct mlx5_flow_dv_dest_array_resource *mdest_res, void **sample_actions,
  ...
  if (num_of_dest > 1) {
    ...
    if (sample_act->action_flags & MLX5_FLOW_ACTION_JUMP) {
      normal_idx++;
      mdest_res->sample_idx[dest_index].rix_jump = dev_flow->handle->rix_jump;
      sample_act->dr_jump_action = dev_flow->dv.jump->action;
    ...
    if (flow_dv_dest_array_resource_register(dev, mdest_res, dev_flow, error))
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                                "can't create sample "
                                "action");
  } else {
 ...
 */
static void flow_dv_aso_age_remove_from_age(struct rte_eth_dev *dev,
 ...
 */
static int flow_dv_aso_age_release(struct rte_eth_dev *dev, uint32_t age_idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, otherwise negative errno value and rte_errno is set.
 */
static int flow_dv_aso_age_pools_resize(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_aso_age_mng *mng = priv->sh->aso_age_mng;
  ...
  if (old_pools) {
    memcpy(pools, old_pools, mng->n * sizeof(struct mlx5_flow_counter_pool *));
    mlx5_free(old_pools);
  } else {
 ...
 */
static uint32_t flow_dv_aso_age_alloc(struct rte_eth_dev *dev,
                                      struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (age_free) {
    ...
    rte_spinlock_unlock(&mng->free_sl);
    rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                       "failed to create ASO age pool");
    return 0; /* 0 is an error. */
  }
  rte_spinlock_unlock(&mng->free_sl);
  pool = container_of(
      (const struct mlx5_aso_age_action(*)[MLX5_ASO_AGE_ACTIONS_PER_POOL])(
          age_free - age_free->offset),
      const struct mlx5_aso_age_pool, actions);
  if (!age_free->dr_action) {
    int reg_c = mlx5_flow_get_reg_id(dev, MLX5_ASO_FLOW_HIT, 0, error);
    ...
    age_free->dr_action = mlx5_glue->dv_create_flow_action_aso(
        priv->sh->rx_domain, pool->flow_hit_aso_obj->obj, age_free->offset,
        MLX5DV_DR_ACTION_FLAGS_ASO_FIRST_HIT_SET, (reg_c - REG_C_0));
#endif /* HAVE_MLX5_DR_CREATE_ACTION_ASO */
    if (!age_free->dr_action) {
      rte_errno = errno;
      rte_spinlock_lock(&mng->free_sl);
 ...
 *
 */
static void flow_dv_aso_age_params_init(struct rte_eth_dev *dev,
                                        uint32_t age_idx, void *context,
                                        uint32_t timeout) {
  ...
  __atomic_store_n(&aso_age->age_params.sec_since_last_hit, 0,
                   __ATOMIC_RELAXED);
  __atomic_store_n(&aso_age->age_params.state, AGE_CANDIDATE, __ATOMIC_RELAXED);
}
...
flow_dv_translate_integrity_l4(const struct rte_flow_item_integrity *mask,
  ...
  if (mask->l4_ok) {
    ...
    MLX5_SET(fte_match_set_lyr_2_4, headers_v, l4_checksum_ok, !!value->l4_ok);
  }
  if (mask->l4_csum_ok) {
    MLX5_SET(fte_match_set_lyr_2_4, headers_m, l4_checksum_ok, 1);
...
flow_dv_translate_integrity_l3(const struct rte_flow_item_integrity *mask,
  ...
  if (mask->l3_ok) {
    ...
    if (is_ipv4) {
      ...
      MLX5_SET(fte_match_set_lyr_2_4, headers_m, l3_ok, 1);
      MLX5_SET(fte_match_set_lyr_2_4, headers_v, l3_ok, value->l3_ok);
    }
  }
...
static void set_integrity_bits(void *headers_m, void *headers_v,
                               const struct rte_flow_item *integrity_item,
                               bool is_l3_ip4) {
  ...
  if (!mask)
    mask = &rte_flow_item_integrity_mask;
  flow_dv_translate_integrity_l3(mask, spec, headers_m, headers_v, is_l3_ip4);
  flow_dv_translate_integrity_l4(mask, spec, headers_m, headers_v);
}

static void flow_dv_translate_item_integrity_post(
    void *matcher, void *key, const struct rte_flow_item *integrity_items[2],
    uint64_t pattern_flags) {
  ...
  if (pattern_flags & MLX5_FLOW_ITEM_INNER_INTEGRITY) {
    headers_m = MLX5_ADDR_OF(fte_match_param, matcher, inner_headers);
    headers_v = MLX5_ADDR_OF(fte_match_param, key, inner_headers);
    is_l3_ip4 = (pattern_flags & MLX5_FLOW_LAYER_INNER_L3_IPV4) != 0;
    set_integrity_bits(headers_m, headers_v, integrity_items[1], is_l3_ip4);
  }
  if (pattern_flags & MLX5_FLOW_ITEM_OUTER_INTEGRITY) {
    headers_m = MLX5_ADDR_OF(fte_match_param, matcher, outer_headers);
    headers_v = MLX5_ADDR_OF(fte_match_param, key, outer_headers);
    is_l3_ip4 = (pattern_flags & MLX5_FLOW_LAYER_OUTER_L3_IPV4) != 0;
    set_integrity_bits(headers_m, headers_v, integrity_items[0], is_l3_ip4);
  }
}
 ...
 *   Pointer to the counter, NULL otherwise.
 */
static struct mlx5_flow_counter *flow_dv_prepare_counter(
    struct rte_eth_dev *dev, struct mlx5_flow *dev_flow, struct rte_flow *flow,
    const struct rte_flow_action_count *count,
    const struct rte_flow_action_age *age, struct rte_flow_error *error) {
  if (!flow->counter) {
    flow->counter = flow_dv_translate_create_counter(dev, dev_flow, count, age);
    if (!flow->counter) {
      rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
 ...
 */
static inline int flow_dv_aso_ct_dev_release(struct rte_eth_dev *dev,
                                             uint32_t idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_aso_ct_pools_mng *mng = priv->sh->ct_mng;
  uint32_t ret;
  struct mlx5_aso_ct_action *ct = flow_aso_ct_get_by_dev_idx(dev, idx);
  enum mlx5_aso_ct_state state = __atomic_load_n(&ct->state, __ATOMIC_RELAXED);
  ...
  if (!ret) {
    if (ct->dr_action_orig) {
      ...
      claim_zero(mlx5_glue->destroy_flow_action(ct->dr_action_orig));
#endif
      ct->dr_action_orig = NULL;
    }
    if (ct->dr_action_rply) {
#ifdef HAVE_MLX5_DR_ACTION_ASO_CT
      claim_zero(mlx5_glue->destroy_flow_action(ct->dr_action_rply));
#endif
...
static inline int flow_dv_aso_ct_release(struct rte_eth_dev *dev,
                                         uint32_t own_idx,
                                         struct rte_flow_error *error) {
  ...
  if (dev->data->dev_started != 1)
    return rte_flow_error_set(
        error, EAGAIN, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
 ...
 *   0 on success, otherwise negative errno value and rte_errno is set.
 */
static int flow_dv_aso_ct_pools_resize(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_aso_ct_pools_mng *mng = priv->sh->ct_mng;
  void *old_pools = mng->pools;
  ...
  if (old_pools) {
    ...
    rte_memcpy(pools, old_pools, mng->n * sizeof(struct mlx5_aso_ct_pool *));
    mlx5_free(old_pools);
  }
  mng->n = resize;
  mng->pools = pools;
...
flow_dv_ct_pool_create(struct rte_eth_dev *dev,
  ...
  obj = mlx5_devx_cmd_create_conn_track_offload_obj(
      priv->sh->cdev->ctx, priv->sh->cdev->pdn, log_obj_size);
  if (!obj) {
    rte_errno = ENODATA;
 ...
 */
static uint32_t flow_dv_aso_ct_alloc(struct rte_eth_dev *dev,
                                     struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_aso_ct_pools_mng *mng = priv->sh->ct_mng;
  ...
  if (!ct->dr_action_orig) {
    ...
    ct->dr_action_orig = mlx5_glue->dv_create_flow_action_aso(
        priv->sh->rx_domain, pool->devx_obj->obj, ct->offset,
        MLX5DV_DR_ACTION_FLAGS_ASO_CT_DIRECTION_INITIATOR, reg_c - REG_C_0);
#else
    RTE_SET_USED(reg_c);
#endif
    if (!ct->dr_action_orig) {
  ...
  if (!ct->dr_action_rply) {
    ...
    ct->dr_action_rply = mlx5_glue->dv_create_flow_action_aso(
        priv->sh->rx_domain, pool->devx_obj->obj, ct->offset,
        MLX5DV_DR_ACTION_FLAGS_ASO_CT_DIRECTION_RESPONDER, reg_c - REG_C_0);
#endif
    if (!ct->dr_action_rply) {
      flow_dv_aso_ct_dev_release(dev, ct_idx);
      rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
...
flow_dv_translate_create_conntrack(struct rte_eth_dev *dev,
  ...
  if (!idx)
    return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION,
                              NULL, "Failed to allocate CT object");
...
static int flow_dv_translate(struct rte_eth_dev *dev,
  ...
  union {
    struct mlx5_flow_dv_modify_hdr_resource res;
    uint8_t
        len[sizeof(struct mlx5_flow_dv_modify_hdr_resource) +
            sizeof(struct mlx5_modification_cmd) * (MLX5_MAX_MODIFY_NUM + 1)];
  } mhdr_dummy;
  ...
  struct flow_grp_info grp_info = {
      ...
      .transfer = !!attr->transfer,
      .fdb_def_rule = !!priv->fdb_def_rule,
      .skip_scale = dev_flow->skip_scale & (1 << MLX5_SCALE_FLOW_GROUP_BIT),
      .std_tbl_fix = true,
  };
  const struct rte_flow_item *integrity_items[2] = {NULL, NULL};
  ...
  if (!wks)
    return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "failed to push flow workspace");
  rss_desc = &wks->rss_desc;
  ...
  if (is_tunnel_offload_active(dev)) {
    if (dev_flow->tunnel) {
      RTE_VERIFY(dev_flow->tof_type == MLX5_TUNNEL_OFFLOAD_MISS_RULE);
      tunnel = dev_flow->tunnel;
    } else {
      tunnel = mlx5_get_tof(items, actions, &dev_flow->tof_type);
  ...
  mhdr_res->ft_type = attr->egress ? MLX5DV_FLOW_TABLE_TYPE_NIC_TX
                                   : MLX5DV_FLOW_TABLE_TYPE_NIC_RX;
  ret = mlx5_flow_group_to_table(dev, tunnel, attr->group, &table, &grp_info,
                                 error);
  if (ret)
    return ret;
  ...
  if (is_flow_tunnel_match_rule(dev_flow->tof_type)) {
    ...
    if (add_decap) {
      if (flow_dv_create_action_l2_decap(dev, dev_flow, attr->transfer, error))
        return -rte_errno;
      dev_flow->dv.actions[actions_n++] = dev_flow->dv.encap_decap->action;
      action_flags |= MLX5_FLOW_ACTION_DECAP;
    }
  }
  for (; !actions_end; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_PORT_ID:
    case RTE_FLOW_ACTION_TYPE_REPRESENTED_PORT:
      if (flow_dv_translate_action_port_id(dev, action, &port_id, error))
        return -rte_errno;
      port_id_resource.port_id = port_id;
      ...
      if (flow_dv_port_id_action_resource_register(dev, &port_id_resource,
                                                   dev_flow, error))
        return -rte_errno;
      dev_flow->dv.actions[actions_n++] = dev_flow->dv.port_id_action->action;
      action_flags |= MLX5_FLOW_ACTION_PORT_ID;
    ...
    case RTE_FLOW_ACTION_TYPE_FLAG:
      ...
      if (dev_conf->dv_xmeta_en != MLX5_XMETA_MODE_LEGACY) {
        ...
        if (flow_dv_convert_action_mark(dev, &mark, mhdr_res, error))
          return -rte_errno;
      ...
      if (flow_dv_tag_resource_register(dev, tag_be, dev_flow, error))
        return -rte_errno;
      MLX5_ASSERT(dev_flow->dv.tag_resource);
      dev_flow->dv.actions[actions_n++] = dev_flow->dv.tag_resource->action;
      break;
    case RTE_FLOW_ACTION_TYPE_MARK:
      action_flags |= MLX5_FLOW_ACTION_MARK;
      wks->mark = 1;
    ...
    case RTE_FLOW_ACTION_TYPE_SET_META:
      if (flow_dv_convert_action_set_meta(
              dev, mhdr_res, attr,
              (const struct rte_flow_action_set_meta *)actions->conf, error))
        return -rte_errno;
      action_flags |= MLX5_FLOW_ACTION_SET_META;
      break;
    case RTE_FLOW_ACTION_TYPE_SET_TAG:
      if (flow_dv_convert_action_set_tag(
              dev, mhdr_res,
              (const struct rte_flow_action_set_tag *)actions->conf, error))
        return -rte_errno;
      action_flags |= MLX5_FLOW_ACTION_SET_TAG;
      break;
    case RTE_FLOW_ACTION_TYPE_DROP:
    ...
    case MLX5_RTE_FLOW_ACTION_TYPE_AGE:
      ...
      if (flow->age == 0) {
        flow->age = owner_idx;
        __atomic_fetch_add(&age_act->refcnt, 1, __ATOMIC_RELAXED);
      }
    ...
    case MLX5_RTE_FLOW_ACTION_TYPE_COUNT:
      owner_idx = (uint32_t)(uintptr_t)action->conf;
      cnt_act = flow_dv_counter_get_by_idx(dev, owner_idx, NULL);
      ...
      if (attr->group == MLX5_FLOW_TABLE_LEVEL_METER &&
        ...
        dev_flow->dv.actions[actions_n++] = cnt_act->action;
      } else {
        if (flow->counter == 0) {
          flow->counter = owner_idx;
          __atomic_fetch_add(&cnt_act->shared_info.refcnt, 1, __ATOMIC_RELAXED);
        }
    ...
    case RTE_FLOW_ACTION_TYPE_COUNT:
      ...
      break;
    case RTE_FLOW_ACTION_TYPE_OF_POP_VLAN:
      dev_flow->dv.actions[actions_n++] = priv->sh->pop_vlan_action;
      action_flags |= MLX5_FLOW_ACTION_OF_POP_VLAN;
      break;
    case RTE_FLOW_ACTION_TYPE_OF_PUSH_VLAN:
      if (!(action_flags & MLX5_FLOW_ACTION_OF_SET_VLAN_VID))
      ...
      vlan.eth_proto = rte_be_to_cpu_16(
          (((const struct rte_flow_action_of_push_vlan *)actions->conf)
               ->ethertype));
      found_action = mlx5_flow_find_action(
          actions + 1, RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_VID);
      if (found_action)
        mlx5_update_vlan_vid_pcp(found_action, &vlan);
      found_action = mlx5_flow_find_action(
          actions + 1, RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_PCP);
      if (found_action)
        mlx5_update_vlan_vid_pcp(found_action, &vlan);
      if (flow_dv_create_action_push_vlan(dev, attr, &vlan, dev_flow, error))
        return -rte_errno;
      dev_flow->dv.actions[actions_n++] = dev_flow->dv.push_vlan_res->action;
      action_flags |= MLX5_FLOW_ACTION_OF_PUSH_VLAN;
      break;
    case RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_PCP:
      /* of_vlan_push action handled this action */
      MLX5_ASSERT(action_flags & MLX5_FLOW_ACTION_OF_PUSH_VLAN);
      break;
    case RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_VID:
      if (action_flags & MLX5_FLOW_ACTION_OF_PUSH_VLAN)
        break;
      ...
      if (flow_dv_convert_action_modify_vlan_vid(mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= MLX5_FLOW_ACTION_OF_SET_VLAN_VID;
      break;
    case RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP:
    case RTE_FLOW_ACTION_TYPE_NVGRE_ENCAP:
      if (flow_dv_create_action_l2_encap(dev, actions, dev_flow, attr->transfer,
                                         error))
        return -rte_errno;
      dev_flow->dv.actions[actions_n++] = dev_flow->dv.encap_decap->action;
      action_flags |= MLX5_FLOW_ACTION_ENCAP;
      if (action_flags & MLX5_FLOW_ACTION_SAMPLE)
        sample_act->action_flags |= MLX5_FLOW_ACTION_ENCAP;
      break;
    case RTE_FLOW_ACTION_TYPE_VXLAN_DECAP:
    case RTE_FLOW_ACTION_TYPE_NVGRE_DECAP:
      if (flow_dv_create_action_l2_decap(dev, dev_flow, attr->transfer, error))
    ...
    case RTE_FLOW_ACTION_TYPE_RAW_ENCAP:
      ...
      if (action_flags & MLX5_FLOW_ACTION_DECAP) {
        if (flow_dv_create_action_raw_encap(dev, actions, dev_flow, attr,
                                            error))
          return -rte_errno;
        dev_flow->dv.actions[actions_n++] = dev_flow->dv.encap_decap->action;
      } else {
        /* Handle encap without preceding decap. */
        if (flow_dv_create_action_l2_encap(dev, actions, dev_flow,
                                           attr->transfer, error))
          return -rte_errno;
        dev_flow->dv.actions[actions_n++] = dev_flow->dv.encap_decap->action;
      }
    ...
    case RTE_FLOW_ACTION_TYPE_RAW_DECAP:
      while ((++action)->type == RTE_FLOW_ACTION_TYPE_VOID)
        ;
      if (action->type != RTE_FLOW_ACTION_TYPE_RAW_ENCAP) {
        if (flow_dv_create_action_l2_decap(dev, dev_flow, attr->transfer,
                                           error))
          return -rte_errno;
        dev_flow->dv.actions[actions_n++] = dev_flow->dv.encap_decap->action;
      }
    ...
    case MLX5_RTE_FLOW_ACTION_TYPE_JUMP:
      dev_flow->dv.actions[actions_n++] = (void *)(uintptr_t)action->conf;
      action_flags |= MLX5_FLOW_ACTION_JUMP;
      break;
    case RTE_FLOW_ACTION_TYPE_JUMP:
      jump_group = ((const struct rte_flow_action_jump *)action->conf)->group;
      grp_info.std_tbl_fix = 0;
      if (dev_flow->skip_scale & (1 << MLX5_SCALE_JUMP_FLOW_GROUP_BIT))
        grp_info.skip_scale = 1;
      else
        grp_info.skip_scale = 0;
      ret = mlx5_flow_group_to_table(dev, tunnel, jump_group, &table, &grp_info,
                                     error);
      if (ret)
        return ret;
      tbl = flow_dv_tbl_resource_get(dev, table, attr->egress, attr->transfer,
                                     !!dev_flow->external, tunnel, jump_group,
                                     0, 0, error);
      if (!tbl)
        return rte_flow_error_set(error, errno, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL, "cannot create jump action.");
      if (flow_dv_jump_tbl_resource_register(dev, tbl, dev_flow, error)) {
        flow_dv_tbl_resource_release(MLX5_SH(dev), tbl);
        return rte_flow_error_set(error, errno, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL, "cannot create jump action.");
      }
      dev_flow->dv.actions[actions_n++] = dev_flow->dv.jump->action;
      action_flags |= MLX5_FLOW_ACTION_JUMP;
    ...
    case RTE_FLOW_ACTION_TYPE_SET_MAC_SRC:
    case RTE_FLOW_ACTION_TYPE_SET_MAC_DST:
      if (flow_dv_convert_action_modify_mac(mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_SET_MAC_SRC
                          ? MLX5_FLOW_ACTION_SET_MAC_SRC
                          : MLX5_FLOW_ACTION_SET_MAC_DST;
    ...
    case RTE_FLOW_ACTION_TYPE_SET_IPV4_SRC:
    case RTE_FLOW_ACTION_TYPE_SET_IPV4_DST:
      if (flow_dv_convert_action_modify_ipv4(mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_SET_IPV4_SRC
                          ? MLX5_FLOW_ACTION_SET_IPV4_SRC
                          : MLX5_FLOW_ACTION_SET_IPV4_DST;
    ...
    case RTE_FLOW_ACTION_TYPE_SET_IPV6_SRC:
    case RTE_FLOW_ACTION_TYPE_SET_IPV6_DST:
      if (flow_dv_convert_action_modify_ipv6(mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_SET_IPV6_SRC
                          ? MLX5_FLOW_ACTION_SET_IPV6_SRC
                          : MLX5_FLOW_ACTION_SET_IPV6_DST;
    ...
    case RTE_FLOW_ACTION_TYPE_SET_TP_DST:
      if (flow_dv_convert_action_modify_tp(
              mhdr_res, actions, items, &flow_attr, dev_flow,
              !!(action_flags & MLX5_FLOW_ACTION_DECAP), error))
        return -rte_errno;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_SET_TP_SRC
                          ? MLX5_FLOW_ACTION_SET_TP_SRC
                          : MLX5_FLOW_ACTION_SET_TP_DST;
    ...
    case RTE_FLOW_ACTION_TYPE_INC_TCP_SEQ:
    case RTE_FLOW_ACTION_TYPE_DEC_TCP_SEQ:
      if (flow_dv_convert_action_modify_tcp_seq(mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_INC_TCP_SEQ
                          ? MLX5_FLOW_ACTION_INC_TCP_SEQ
                          : MLX5_FLOW_ACTION_DEC_TCP_SEQ;
    ...
    case RTE_FLOW_ACTION_TYPE_INC_TCP_ACK:
    case RTE_FLOW_ACTION_TYPE_DEC_TCP_ACK:
      if (flow_dv_convert_action_modify_tcp_ack(mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= actions->type == RTE_FLOW_ACTION_TYPE_INC_TCP_ACK
                          ? MLX5_FLOW_ACTION_INC_TCP_ACK
                          : MLX5_FLOW_ACTION_DEC_TCP_ACK;
      break;
    case MLX5_RTE_FLOW_ACTION_TYPE_TAG:
      if (flow_dv_convert_action_set_reg(mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= MLX5_FLOW_ACTION_SET_TAG;
      break;
    case MLX5_RTE_FLOW_ACTION_TYPE_COPY_MREG:
      if (flow_dv_convert_action_copy_mreg(dev, mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= MLX5_FLOW_ACTION_SET_TAG;
      break;
    case MLX5_RTE_FLOW_ACTION_TYPE_DEFAULT_MISS:
      action_flags |= MLX5_FLOW_ACTION_DEFAULT_MISS;
      dev_flow->handle->fate_action = MLX5_FLOW_FATE_DEFAULT_MISS;
      break;
    case RTE_FLOW_ACTION_TYPE_METER:
      if (!wks->fm)
        return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION,
      ...
      dev_flow->dv.actions[actions_n++] = wks->fm->meter_action;
      action_flags |= MLX5_FLOW_ACTION_METER;
      break;
    case RTE_FLOW_ACTION_TYPE_SET_IPV4_DSCP:
      if (flow_dv_convert_action_modify_ipv4_dscp(mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= MLX5_FLOW_ACTION_SET_IPV4_DSCP;
      break;
    case RTE_FLOW_ACTION_TYPE_SET_IPV6_DSCP:
      if (flow_dv_convert_action_modify_ipv6_dscp(mhdr_res, actions, error))
        return -rte_errno;
      action_flags |= MLX5_FLOW_ACTION_SET_IPV6_DSCP;
      break;
    case RTE_FLOW_ACTION_TYPE_SAMPLE:
      sample_act_pos = actions_n;
      sample = (const struct rte_flow_action_sample *)action->conf;
      actions_n++;
      action_flags |= MLX5_FLOW_ACTION_SAMPLE;
      ...
      break;
    case RTE_FLOW_ACTION_TYPE_MODIFY_FIELD:
      if (flow_dv_convert_action_modify_field(dev, mhdr_res, actions, attr,
                                              error))
        return -rte_errno;
      action_flags |= MLX5_FLOW_ACTION_MODIFY_FIELD;
      break;
    case RTE_FLOW_ACTION_TYPE_CONNTRACK:
      ...
      if (!ct)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL, "Failed to get CT object.");
      if (mlx5_aso_ct_available(priv->sh, ct))
        return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION,
                                  NULL, "CT is unavailable.");
      if (ct->is_original)
        dev_flow->dv.actions[actions_n] = ct->dr_action_orig;
      else
        dev_flow->dv.actions[actions_n] = ct->dr_action_rply;
      if (flow->ct == 0) {
    ...
    case RTE_FLOW_ACTION_TYPE_END:
      ...
      if (mhdr_res->actions_num) {
        ...
        if (flow_dv_modify_hdr_resource_register(dev, mhdr_res, dev_flow,
                                                 error))
          return -rte_errno;
      ...
      if (action_flags & MLX5_FLOW_ACTION_AGE) {
        if ((non_shared_age && count) ||
          ...
          cnt_act = flow_dv_prepare_counter(dev, dev_flow, flow, count,
                                            non_shared_age, error);
          if (!cnt_act)
            return -rte_errno;
          dev_flow->dv.actions[age_act_pos] = cnt_act->action;
          break;
        }
        if (!flow->age && non_shared_age) {
          flow->age = flow_dv_aso_age_alloc(dev, error);
          if (!flow->age)
            return -rte_errno;
          flow_dv_aso_age_params_init(
              dev, flow->age,
              non_shared_age->context ? non_shared_age->context
                                      : (void *)(uintptr_t)(dev_flow->flow_idx),
              non_shared_age->timeout);
        }
        age_act = flow_aso_age_get_by_idx(dev, flow->age);
        dev_flow->dv.actions[age_act_pos] = age_act->dr_action;
      }
      if (action_flags & MLX5_FLOW_ACTION_COUNT) {
        /*
         ...
         */
        cnt_act =
            flow_dv_prepare_counter(dev, dev_flow, flow, count, NULL, error);
        if (!cnt_act)
          return -rte_errno;
        dev_flow->dv.actions[actions_n++] = cnt_act->action;
      }
    ...
    if (mhdr_res->actions_num && modify_action_position == UINT32_MAX)
  ...
  for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
    ...
    switch (item_type) {
    case RTE_FLOW_ITEM_TYPE_PORT_ID:
      flow_dv_translate_item_port_id(dev, match_mask, match_value, items, attr);
      last_item = MLX5_FLOW_ITEM_PORT_ID;
      break;
    case RTE_FLOW_ITEM_TYPE_ETH:
      flow_dv_translate_item_eth(match_mask, match_value, items, tunnel,
                                 dev_flow->dv.group);
      matcher.priority =
          action_flags & MLX5_FLOW_ACTION_DEFAULT_MISS && !dev_flow->external
              ? MLX5_PRIORITY_MAP_L3
              : MLX5_PRIORITY_MAP_L2;
      last_item = tunnel ? MLX5_FLOW_LAYER_INNER_L2 : MLX5_FLOW_LAYER_OUTER_L2;
      break;
    case RTE_FLOW_ITEM_TYPE_VLAN:
      flow_dv_translate_item_vlan(dev_flow, match_mask, match_value, items,
                                  tunnel, dev_flow->dv.group);
    ...
    case RTE_FLOW_ITEM_TYPE_IPV4:
      mlx5_flow_tunnel_ip_check(items, next_protocol, &item_flags, &tunnel);
      flow_dv_translate_item_ipv4(match_mask, match_value, items, tunnel,
                                  dev_flow->dv.group);
    ...
    case RTE_FLOW_ITEM_TYPE_IPV6:
      mlx5_flow_tunnel_ip_check(items, next_protocol, &item_flags, &tunnel);
      flow_dv_translate_item_ipv6(match_mask, match_value, items, tunnel,
                                  dev_flow->dv.group);
      matcher.priority = MLX5_PRIORITY_MAP_L3;
      last_item = tunnel ? MLX5_FLOW_LAYER_INNER_L3_IPV6
                         : MLX5_FLOW_LAYER_OUTER_L3_IPV6;
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_IPV6_FRAG_EXT:
      flow_dv_translate_item_ipv6_frag_ext(match_mask, match_value, items,
                                           tunnel);
      last_item = tunnel ? MLX5_FLOW_LAYER_INNER_L3_IPV6_FRAG_EXT
                         : MLX5_FLOW_LAYER_OUTER_L3_IPV6_FRAG_EXT;
      if (items->mask != NULL &&
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_TCP:
      flow_dv_translate_item_tcp(match_mask, match_value, items, tunnel);
      matcher.priority = MLX5_PRIORITY_MAP_L4;
      last_item =
          tunnel ? MLX5_FLOW_LAYER_INNER_L4_TCP : MLX5_FLOW_LAYER_OUTER_L4_TCP;
      break;
    case RTE_FLOW_ITEM_TYPE_UDP:
      flow_dv_translate_item_udp(match_mask, match_value, items, tunnel);
      matcher.priority = MLX5_PRIORITY_MAP_L4;
      last_item =
          tunnel ? MLX5_FLOW_LAYER_INNER_L4_UDP : MLX5_FLOW_LAYER_OUTER_L4_UDP;
      break;
    case RTE_FLOW_ITEM_TYPE_GRE:
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_GRE_KEY:
      flow_dv_translate_item_gre_key(match_mask, match_value, items);
      last_item = MLX5_FLOW_LAYER_GRE_KEY;
      break;
    case RTE_FLOW_ITEM_TYPE_GRE_OPTION:
      matcher.priority = MLX5_TUNNEL_PRIO_GET(rss_desc);
    ...
    case RTE_FLOW_ITEM_TYPE_NVGRE:
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_VXLAN:
      flow_dv_translate_item_vxlan(dev, attr, match_mask, match_value, items,
                                   tunnel);
      matcher.priority = MLX5_TUNNEL_PRIO_GET(rss_desc);
      last_item = MLX5_FLOW_LAYER_VXLAN;
      break;
    ...
    case RTE_FLOW_ITEM_TYPE_GENEVE:
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_GENEVE_OPT:
      ret = flow_dv_translate_item_geneve_opt(dev, match_mask, match_value,
                                              items, error);
      if (ret)
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_MPLS:
      flow_dv_translate_item_mpls(match_mask, match_value, items, last_item,
                                  tunnel);
      matcher.priority = MLX5_TUNNEL_PRIO_GET(rss_desc);
      last_item = MLX5_FLOW_LAYER_MPLS;
      break;
    case RTE_FLOW_ITEM_TYPE_MARK:
      flow_dv_translate_item_mark(dev, match_mask, match_value, items);
      last_item = MLX5_FLOW_ITEM_MARK;
      break;
    case RTE_FLOW_ITEM_TYPE_META:
      flow_dv_translate_item_meta(dev, match_mask, match_value, attr, items);
      last_item = MLX5_FLOW_ITEM_METADATA;
      break;
    case RTE_FLOW_ITEM_TYPE_ICMP:
      flow_dv_translate_item_icmp(match_mask, match_value, items, tunnel);
      matcher.priority = MLX5_PRIORITY_MAP_L4;
      last_item = MLX5_FLOW_LAYER_ICMP;
      break;
    case RTE_FLOW_ITEM_TYPE_ICMP6:
      flow_dv_translate_item_icmp6(match_mask, match_value, items, tunnel);
      matcher.priority = MLX5_PRIORITY_MAP_L4;
      last_item = MLX5_FLOW_LAYER_ICMP6;
      break;
    case RTE_FLOW_ITEM_TYPE_TAG:
      flow_dv_translate_item_tag(dev, match_mask, match_value, items);
      last_item = MLX5_FLOW_ITEM_TAG;
      break;
    case MLX5_RTE_FLOW_ITEM_TYPE_TAG:
      flow_dv_translate_mlx5_item_tag(dev, match_mask, match_value, items);
      last_item = MLX5_FLOW_ITEM_TAG;
      break;
    case MLX5_RTE_FLOW_ITEM_TYPE_TX_QUEUE:
      flow_dv_translate_item_tx_queue(dev, match_mask, match_value, items);
      last_item = MLX5_FLOW_ITEM_TX_QUEUE;
      break;
    case RTE_FLOW_ITEM_TYPE_GTP:
      flow_dv_translate_item_gtp(match_mask, match_value, items, tunnel);
      matcher.priority = MLX5_TUNNEL_PRIO_GET(rss_desc);
      last_item = MLX5_FLOW_LAYER_GTP;
      break;
    case RTE_FLOW_ITEM_TYPE_GTP_PSC:
      ret = flow_dv_translate_item_gtp_psc(match_mask, match_value, items);
      if (ret)
        return rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
    ...
    case RTE_FLOW_ITEM_TYPE_ECPRI:
      ...
      flow_dv_translate_item_ecpri(dev, match_mask, match_value, items,
                                   last_item);
      /* No other protocol should follow eCPRI layer. */
      last_item = MLX5_FLOW_LAYER_ECPRI;
      break;
    case RTE_FLOW_ITEM_TYPE_INTEGRITY:
      flow_dv_translate_item_integrity(items, integrity_items, &last_item);
      break;
    case RTE_FLOW_ITEM_TYPE_CONNTRACK:
      flow_dv_translate_item_aso_ct(dev, match_mask, match_value, items);
      break;
    case RTE_FLOW_ITEM_TYPE_FLEX:
      flow_dv_translate_item_flex(dev, match_mask, match_value, items, dev_flow,
                                  tunnel != 0);
      last_item =
          tunnel ? MLX5_FLOW_ITEM_INNER_FLEX : MLX5_FLOW_ITEM_OUTER_FLEX;
      break;
    default:
      break;
    }
  ...
  if (!(item_flags & MLX5_FLOW_ITEM_PORT_ID) && priv->sh->esw_mode &&
    ...
    if (flow_dv_translate_item_port_id(dev, match_mask, match_value, NULL,
                                       attr))
      return -rte_errno;
  }
  if (item_flags & MLX5_FLOW_ITEM_INTEGRITY) {
    flow_dv_translate_item_integrity_post(match_mask, match_value,
                                          integrity_items, item_flags);
  }
  if (item_flags & MLX5_FLOW_LAYER_VXLAN_GPE)
    flow_dv_translate_item_vxlan_gpe(match_mask, match_value, tunnel_item,
                                     item_flags);
  else if (item_flags & MLX5_FLOW_LAYER_GENEVE)
    flow_dv_translate_item_geneve(match_mask, match_value, tunnel_item,
                                  item_flags);
  else if (item_flags & MLX5_FLOW_LAYER_GRE) {
    if (tunnel_item->type == RTE_FLOW_ITEM_TYPE_GRE)
      flow_dv_translate_item_gre(match_mask, match_value, tunnel_item,
                                 item_flags);
    else if (tunnel_item->type == RTE_FLOW_ITEM_TYPE_NVGRE)
      flow_dv_translate_item_nvgre(match_mask, match_value, tunnel_item,
                                   item_flags);
    else if (tunnel_item->type == RTE_FLOW_ITEM_TYPE_GRE_OPTION)
      flow_dv_translate_item_gre_option(match_mask, match_value, tunnel_item,
                                        gre_item, item_flags);
    else
      MLX5_ASSERT(false);
  }
#ifdef RTE_LIBRTE_MLX5_DEBUG
  MLX5_ASSERT(
      !flow_dv_check_valid_spec(matcher.mask.buf, dev_flow->dv.value.buf));
#endif
  /*
   ...
   */
  if (action_flags & MLX5_FLOW_ACTION_SAMPLE) {
    ret = flow_dv_translate_action_sample(dev, sample, dev_flow, attr,
                                          &num_of_dest, sample_actions,
                                          &sample_res, error);
    if (ret < 0)
      return ret;
    ret = flow_dv_create_action_sample(dev, dev_flow, num_of_dest, &sample_res,
                                       &mdest_res, sample_actions, action_flags,
                                       error);
    if (ret < 0)
      return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION,
  ...
  if (num_of_dest > 1 &&
    ...
    for (i = 0; i < actions_n; i++) {
      if ((sample_act->dr_encap_action &&
           sample_act->dr_encap_action == dev_flow->dv.actions[i]) ||
          (sample_act->dr_port_id_action &&
           sample_act->dr_port_id_action == dev_flow->dv.actions[i]) ||
          (sample_act->dr_jump_action &&
           sample_act->dr_jump_action == dev_flow->dv.actions[i]))
        continue;
      temp_actions[tmp_actions_n++] = dev_flow->dv.actions[i];
    }
    memcpy((void *)dev_flow->dv.actions, (void *)temp_actions,
  ...
  /* Register matcher. */
  matcher.crc =
      rte_raw_cksum((const void *)matcher.mask.buf, matcher.mask.size);
  matcher.priority = mlx5_get_matcher_priority(dev, attr, matcher.priority,
                                               dev_flow->external);
  /**
  ...
  if (flow_dv_matcher_register(dev, &matcher, &tbl_key, dev_flow, tunnel,
                               attr->group, error))
    return -rte_errno;
  return 0;
}
 ...
 */
static int __flow_dv_action_rss_hrxq_set(struct mlx5_shared_action_rss *action,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_apply(struct rte_eth_dev *dev, struct rte_flow *flow,
                         struct rte_flow_error *error) {
  ...
  for (idx = wks->flow_idx - 1; idx >= 0; idx--) {
    ...
    if (dh->fate_action == MLX5_FLOW_FATE_DROP) {
      if (dv->transfer) {
        ...
        MLX5_ASSERT(priv->sh->dr_drop_action);
        dv->actions[n++] =
            dv->group ? priv->sh->dr_drop_action : priv->root_drop_action;
#else
        /* For DV we use the explicit drop queue. */
        MLX5_ASSERT(priv->drop_queue.hrxq);
        dv->actions[n++] = priv->drop_queue.hrxq->action;
#endif
      }
    } else if ((dh->fate_action == MLX5_FLOW_FATE_QUEUE && !dv_h->rix_sample &&
                !dv_h->rix_dest_array)) {
      struct mlx5_hrxq *hrxq;
      uint32_t hrxq_idx;
      ...
      hrxq_idx = flow_dv_action_rss_hrxq_lookup(dev, rss_desc->shared_rss,
                                                dev_flow->hash_fields);
      if (hrxq_idx)
      ...
      if (!priv->sh->default_miss_action) {
        rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                           NULL, "default miss action not be created.");
        goto error;
      }
    ...
    err = mlx5_flow_os_create_flow(dv_h->matcher->matcher_object,
                                   (void *)&dv->value, n, dv->actions,
                                   &dh->drv_flow);
    if (err) {
      rte_flow_error_set(
          error, errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
          (!priv->sh->config.allow_duplicate_pattern && errno == EEXIST)
              ? "duplicating pattern is not allowed"
              : "hardware refuses to create flow");
      goto error;
    }
    if (priv->vmwa_context && dh->vf_vlan.tag && !dh->vf_vlan.created) {
      /*
...
void flow_dv_matcher_remove_cb(void *tool_ctx __rte_unused,
                               struct mlx5_list_entry *entry) {
  struct mlx5_flow_dv_matcher *resource =
      container_of(entry, typeof(*resource), entry);
 ...
 *   1 while a reference on it exists, 0 when freed.
 */
static int flow_dv_matcher_release(struct rte_eth_dev *dev,
                                   struct mlx5_flow_handle *handle) {
  struct mlx5_flow_dv_matcher *matcher = handle->dvh.matcher;
  struct mlx5_flow_tbl_data_entry *tbl =
      container_of(matcher->tbl, typeof(*tbl), tbl);
  int ret;
...
static int flow_dv_encap_decap_resource_release(struct rte_eth_dev *dev,
  ...
  resource =
      mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_DECAP_ENCAP], encap_decap_idx);
  if (!resource)
    return 0;
...
static int flow_dv_jump_tbl_resource_release(struct rte_eth_dev *dev,
  ...
  tbl_data = mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_JUMP], rix_jump);
  if (!tbl_data)
    return 0;
...
static int flow_dv_port_id_action_resource_release(struct rte_eth_dev *dev,
  ...
  if (!resource)
    return 0;
  MLX5_ASSERT(resource->action);
  return mlx5_list_unregister(priv->sh->port_id_action_list, &resource->entry);
}
...
static void flow_dv_shared_rss_action_release(struct rte_eth_dev *dev,
  ...
  shared_rss =
      mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_RSS_SHARED_ACTIONS], srss);
  __atomic_sub_fetch(&shared_rss->refcnt, 1, __ATOMIC_RELAXED);
}

void flow_dv_push_vlan_remove_cb(void *tool_ctx,
                                 struct mlx5_list_entry *entry) {
  struct mlx5_dev_ctx_shared *sh = tool_ctx;
  struct mlx5_flow_dv_push_vlan_action_resource *resource =
      container_of(entry, typeof(*resource), entry);
 ...
 *   Pointer to mlx5_flow_handle.
 */
static void flow_dv_fate_resource_release(struct rte_eth_dev *dev,
                                          struct mlx5_flow_handle *handle) {
  if (!handle->rix_fate)
    return;
...
void flow_dv_sample_remove_cb(void *tool_ctx __rte_unused,
                              struct mlx5_list_entry *entry) {
  struct mlx5_flow_dv_sample_resource *resource =
      container_of(entry, typeof(*resource), entry);
  struct rte_eth_dev *dev = resource->dev;
  ...
  if (resource->verbs_action)
    claim_zero(mlx5_flow_os_destroy_flow_action(resource->verbs_action));
  if (resource->normal_path_tbl)
    flow_dv_tbl_resource_release(MLX5_SH(dev), resource->normal_path_tbl);
  flow_dv_sample_sub_actions_release(dev, &resource->sample_idx);
  mlx5_ipool_free(priv->sh->ipool[MLX5_IPOOL_SAMPLE], resource->idx);
 ...
 *   1 while a reference on it exists, 0 when freed.
 */
static int flow_dv_sample_resource_release(struct rte_eth_dev *dev,
                                           struct mlx5_flow_handle *handle) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_dv_sample_resource *resource;
  ...
  return mlx5_list_unregister(priv->sh->sample_action_list, &resource->entry);
}

void flow_dv_dest_array_remove_cb(void *tool_ctx __rte_unused,
                                  struct mlx5_list_entry *entry) {
  struct mlx5_flow_dv_dest_array_resource *resource =
      container_of(entry, typeof(*resource), entry);
  struct rte_eth_dev *dev = resource->dev;
  ...
  for (; i < resource->num_of_dest; i++)
    flow_dv_sample_sub_actions_release(dev, &resource->sample_idx[i]);
  mlx5_ipool_free(priv->sh->ipool[MLX5_IPOOL_DEST_ARRAY], resource->idx);
  DRV_LOG(DEBUG, "destination array resource %p: removed", (void *)resource);
}
...
flow_dv_dest_array_resource_release(struct rte_eth_dev *dev,
  ...
  if (!resource)
    return 0;
  MLX5_ASSERT(resource->action);
  return mlx5_list_unregister(priv->sh->dest_array_list, &resource->entry);
}
...
flow_dv_geneve_tlv_option_resource_release(struct rte_eth_dev *dev) {
  ...
  if (geneve_opt_resource) {
    if (!(__atomic_sub_fetch(&geneve_opt_resource->refcnt, 1,
                             __ATOMIC_RELAXED))) {
      claim_zero(mlx5_devx_cmd_destroy(geneve_opt_resource->obj));
 ...
 *   Pointer to flow structure.
 */
static void flow_dv_remove(struct rte_eth_dev *dev, struct rte_flow *flow) {
  struct mlx5_flow_handle *dh;
  uint32_t handle_idx;
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  while (handle_idx) {
    dh = mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_MLX5_FLOW], handle_idx);
    if (!dh)
      return;
    if (dh->drv_flow) {
 ...
 *   Pointer to flow structure.
 */
static void flow_dv_destroy(struct rte_eth_dev *dev, struct rte_flow *flow) {
  struct mlx5_flow_handle *dev_handle;
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  while (flow->dev_handles) {
    ...
    dev_handle = mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_MLX5_FLOW], tmp_idx);
    if (!dev_handle)
      return;
    ...
    if (dev_handle->dvh.rix_push_vlan)
      flow_dv_push_vlan_action_resource_release(dev, dev_handle);
    if (dev_handle->dvh.rix_tag)
      flow_dv_tag_release(dev, dev_handle->dvh.rix_tag);
    if (dev_handle->fate_action != MLX5_FLOW_FATE_SHARED_RSS)
      flow_dv_fate_resource_release(dev, dev_handle);
    else if (!srss)
      srss = dev_handle->rix_srss;
    if (fm && dev_handle->is_meter_flow_id && dev_handle->split_flow_id)
      mlx5_ipool_free(fm->flow_ipool, dev_handle->split_flow_id);
    else if (dev_handle->split_flow_id && !dev_handle->is_meter_flow_id)
      mlx5_ipool_free(priv->sh->ipool[MLX5_IPOOL_RSS_EXPANTION_FLOW_ID],
                      dev_handle->split_flow_id);
    mlx5_ipool_free(priv->sh->ipool[MLX5_IPOOL_MLX5_FLOW], tmp_idx);
  }
  if (srss)
 ...
 *   0 on success, otherwise negative errno value.
 */
static int __flow_dv_action_rss_setup(struct rte_eth_dev *dev,
                                      uint32_t action_idx,
                                      struct mlx5_shared_action_rss *shared_rss,
                                      struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  shared_rss->ind_tbl = mlx5_ind_table_obj_new(dev, shared_rss->origin.queue,
                                               shared_rss->origin.queue_num,
                                               true, !!dev->data->dev_started);
  if (!shared_rss->ind_tbl)
    return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "cannot setup indirection table");
  ...
  for (i = 0; i < MLX5_RSS_HASH_FIELDS_LEN; i++) {
    ...
    flow_dv_action_rss_l34_hash_adjust(shared_rss->origin.types, &hash_fields);
    ...
    err = __flow_dv_action_rss_hrxq_set(shared_rss, hash_fields, hrxq->idx);
    MLX5_ASSERT(!err);
  }
  return 0;
error_hrxq_new:
 ...
 *   rte_errno is set.
 */
static uint32_t __flow_dv_action_rss_create(
    struct rte_eth_dev *dev, const struct rte_flow_indir_action_conf *conf,
    const struct rte_flow_action_rss *rss, struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  RTE_SET_USED(conf);
  shared_rss =
      mlx5_ipool_zmalloc(priv->sh->ipool[MLX5_IPOOL_RSS_SHARED_ACTIONS], &idx);
  if (!shared_rss) {
    rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                       "cannot allocate resource memory");
    goto error_rss_init;
  ...
  if (idx > (1u << MLX5_INDIRECT_ACTION_TYPE_OFFSET)) {
    rte_flow_error_set(error, E2BIG, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
...
error_rss_init:
  if (shared_rss) {
    ...
    mlx5_ipool_free(priv->sh->ipool[MLX5_IPOOL_RSS_SHARED_ACTIONS], idx);
  }
  return 0;
}
 ...
 *   0 on success, otherwise negative errno value.
 */
static int __flow_dv_action_rss_release(struct rte_eth_dev *dev, uint32_t idx,
                                        struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_shared_action_rss *shared_rss =
      mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_RSS_SHARED_ACTIONS], idx);
  ...
  if (!__atomic_compare_exchange_n(&shared_rss->refcnt, &old_refcnt, 0, 0,
 ...
 *   rte_errno is set.
 */
struct rte_flow_action_handle *flow_dv_action_create(
    struct rte_eth_dev *dev, const struct rte_flow_indir_action_conf *conf,
    const struct rte_flow_action *action, struct rte_flow_error *err) {
  struct mlx5_priv *priv = dev->data->dev_private;
  uint32_t age_idx = 0;
  uint32_t idx = 0;
  uint32_t ret = 0;
  ...
  case RTE_FLOW_ACTION_TYPE_RSS:
    ...
    idx = (MLX5_INDIRECT_ACTION_TYPE_RSS << MLX5_INDIRECT_ACTION_TYPE_OFFSET) |
          ret;
    break;
  case RTE_FLOW_ACTION_TYPE_AGE:
    ...
    idx = (MLX5_INDIRECT_ACTION_TYPE_AGE << MLX5_INDIRECT_ACTION_TYPE_OFFSET) |
          age_idx;
    flow_dv_aso_age_params_init(
        dev, age_idx,
        ((const struct rte_flow_action_age *)action->conf)->context
            ? ((const struct rte_flow_action_age *)action->conf)->context
            : (void *)(uintptr_t)idx,
        ((const struct rte_flow_action_age *)action->conf)->timeout);
    ret = age_idx;
    break;
  case RTE_FLOW_ACTION_TYPE_COUNT:
    ret = flow_dv_translate_create_counter(dev, NULL, NULL, NULL);
    idx =
        (MLX5_INDIRECT_ACTION_TYPE_COUNT << MLX5_INDIRECT_ACTION_TYPE_OFFSET) |
        ret;
    break;
  case RTE_FLOW_ACTION_TYPE_CONNTRACK:
    ret = flow_dv_translate_create_conntrack(dev, action->conf, err);
    idx = MLX5_INDIRECT_ACT_CT_GEN_IDX(PORT_ID(priv), ret);
    break;
  default:
    rte_flow_error_set(err, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
...
int flow_dv_action_destroy(struct rte_eth_dev *dev,
  ...
  case MLX5_INDIRECT_ACTION_TYPE_COUNT:
    ...
    if (!__atomic_compare_exchange_n(&cnt->shared_info.refcnt, &no_flow_refcnt,
  ...
  case MLX5_INDIRECT_ACTION_TYPE_AGE:
    ...
    if (ret)
       ...
       */
      DRV_LOG(DEBUG,
              "Indirect age action %" PRIu32 " was"
              " released with references %d.",
              idx, ret);
    return 0;
  case MLX5_INDIRECT_ACTION_TYPE_CT:
    ...
    if (ret > 0)
      DRV_LOG(DEBUG,
              "Connection tracking object %u still "
              "has references %d.",
              idx, ret);
    return 0;
  default:
...
__flow_dv_action_rss_update(struct rte_eth_dev *dev, uint32_t idx,
  ...
  queue = mlx5_malloc(MLX5_MEM_ZERO, RTE_ALIGN_CEIL(queue_size, sizeof(void *)),
                      0, SOCKET_ID_ANY);
  if (!queue)
    return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "cannot allocate resource memory");
  ...
  ret = mlx5_ind_table_obj_modify(dev, shared_rss->ind_tbl, queue,
                                  action_conf->queue_num, true /* standalone */,
                                  dev_started /* ref_new_qs */,
                                  dev_started /* deref_old_qs */);
  if (ret) {
    ret = rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
...
__flow_dv_action_ct_update(struct rte_eth_dev *dev, uint32_t idx,
  ...
  if (PORT_ID(priv) != owner)
    return rte_flow_error_set(error, EACCES, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "CT object owned by another port");
  dev_idx = MLX5_INDIRECT_ACT_CT_GET_IDX(idx);
  ct = flow_aso_ct_get_by_dev_idx(dev, dev_idx);
  if (!ct->refcnt)
    return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "CT object is inactive");
  ...
  if (update->state) {
    ...
    if (ret)
      return rte_flow_error_set(error, EIO, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Failed to send CT context update WQE");
    /* Block until ready or a failure. */
    ret = mlx5_aso_ct_available(priv->sh, ct);
    if (ret)
      rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                         NULL, "Timeout to get the CT update");
  }
  return ret;
}
...
int flow_dv_action_update(struct rte_eth_dev *dev,
                          struct rte_flow_action_handle *handle,
                          const void *update, struct rte_flow_error *err) {
  uint32_t act_idx = (uint32_t)(uintptr_t)handle;
  uint32_t type = act_idx >> MLX5_INDIRECT_ACTION_TYPE_OFFSET;
  uint32_t idx = act_idx & ((1u << MLX5_INDIRECT_ACTION_TYPE_OFFSET) - 1);
  ...
  default:
    return rte_flow_error_set(err, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
 ...
 */
static void __flow_dv_destroy_sub_policy_rules(
    struct rte_eth_dev *dev, struct mlx5_flow_meter_sub_policy *sub_policy) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_tbl_data_entry *tbl;
  ...
  for (i = 0; i < RTE_COLORS; i++) {
    ...
    if (i == RTE_COLOR_GREEN && policy &&
      ...
      next_fm =
          mlx5_flow_meter_find(priv, policy->act_cnt[i].next_mtr_id, NULL);
    RTE_TAILQ_FOREACH_SAFE(color_rule, &sub_policy->color_rules[i], next_port,
                           tmp) {
      claim_zero(mlx5_flow_os_destroy_flow(color_rule->rule));
      tbl = container_of(color_rule->matcher->tbl, typeof(*tbl), tbl);
      mlx5_list_unregister(tbl->matchers, &color_rule->matcher->entry);
      TAILQ_REMOVE(&sub_policy->color_rules[i], color_rule, next_port);
  ...
  for (i = 0; i < MLX5_MTR_RTE_COLORS; i++) {
    ...
    if (sub_policy->jump_tbl[i]) {
      flow_dv_tbl_resource_release(MLX5_SH(dev), sub_policy->jump_tbl[i]);
      sub_policy->jump_tbl[i] = NULL;
    }
  }
  if (sub_policy->tbl_rsc) {
    flow_dv_tbl_resource_release(MLX5_SH(dev), sub_policy->tbl_rsc);
...
flow_dv_destroy_policy_rules(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < MLX5_MTR_DOMAIN_MAX; i++) {
    sub_policy_num =
        (mtr_policy->sub_policy_num >> (MLX5_MTR_SUB_POLICY_NUM_SHIFT * i)) &
...
flow_dv_destroy_mtr_policy_acts(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < MLX5_MTR_RTE_COLORS; i++) {
    if (mtr_policy->act_cnt[i].rix_mark) {
      flow_dv_tag_release(dev, mtr_policy->act_cnt[i].rix_mark);
      mtr_policy->act_cnt[i].rix_mark = 0;
    }
    if (mtr_policy->act_cnt[i].modify_hdr) {
      dev_handle.dvh.modify_hdr = mtr_policy->act_cnt[i].modify_hdr;
      flow_dv_modify_hdr_resource_release(dev, &dev_handle);
    }
    ...
    case MLX5_FLOW_FATE_PORT_ID:
      if (mtr_policy->act_cnt[i].rix_port_id_action) {
        flow_dv_port_id_action_resource_release(
            dev, mtr_policy->act_cnt[i].rix_port_id_action);
    ...
    case MLX5_FLOW_FATE_JUMP:
      for (j = 0; j < MLX5_MTR_DOMAIN_MAX; j++)
        mtr_policy->act_cnt[i].dr_jump_action[j] = NULL;
      break;
    default:
 ...
 *   0 on success, otherwise negative errno value.
 */
static int __flow_dv_create_domain_policy_acts(
    struct rte_eth_dev *dev, struct mlx5_flow_meter_policy *mtr_policy,
    const struct rte_flow_action *actions[RTE_COLORS],
    enum mlx5_meter_domain domain, struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  for (i = 0; i < RTE_COLORS; i++) {
    ...
    if ((i == RTE_COLOR_YELLOW && mtr_policy->skip_y) ||
      ...
      continue;
    action_flags = 0;
    for (act = actions[i]; act && act->type != RTE_FLOW_ACTION_TYPE_END;
         act++) {
      switch (act->type) {
      case RTE_FLOW_ACTION_TYPE_MARK: {
        uint32_t tag_be = mlx5_flow_mark_set(
            ((const struct rte_flow_action_mark *)(act->conf))->id);
        ...
        if (flow_dv_tag_resource_register(dev, tag_be, &dev_flow, &flow_err))
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "cannot setup policy mark action");
        MLX5_ASSERT(dev_flow.dv.tag_resource);
        act_cnt->rix_mark = dev_flow.handle->dvh.rix_tag;
        action_flags |= MLX5_FLOW_ACTION_MARK;
        mtr_policy->mark = 1;
        break;
      }
      case RTE_FLOW_ACTION_TYPE_SET_TAG:
        ...
        if (flow_dv_convert_action_set_tag(
                dev, mhdr_res,
                (const struct rte_flow_action_set_tag *)act->conf, &flow_err))
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "cannot convert policy "
                                    "set tag action");
        if (!mhdr_res->actions_num)
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
      ...
      case RTE_FLOW_ACTION_TYPE_DROP: {
        ...
        if (!mtrmng->drop_tbl[domain]) {
          mtrmng->drop_tbl[domain] = flow_dv_tbl_resource_get(
              dev, MLX5_FLOW_TABLE_LEVEL_METER, egress, transfer, false, NULL,
              0, 0, MLX5_MTR_TABLE_ID_DROP, &flow_err);
          if (!mtrmng->drop_tbl[domain])
            return -rte_mtr_error_set(error, ENOTSUP,
                                      RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
        ...
        tbl_data = container_of(mtrmng->drop_tbl[domain],
                                struct mlx5_flow_tbl_data_entry, tbl);
        if (i < MLX5_MTR_RTE_COLORS) {
          act_cnt->dr_jump_action[domain] = tbl_data->jump.action;
          act_cnt->fate_action = MLX5_FLOW_FATE_DROP;
        }
        if (i == RTE_COLOR_RED)
          mtr_policy->dr_drop_action[domain] = tbl_data->jump.action;
        action_flags |= MLX5_FLOW_ACTION_DROP;
        break;
      }
      case RTE_FLOW_ACTION_TYPE_QUEUE: {
        ...
        act_cnt->fate_action = MLX5_FLOW_FATE_QUEUE;
        dev_flow.handle->fate_action = MLX5_FLOW_FATE_QUEUE;
      ...
      case RTE_FLOW_ACTION_TYPE_RSS: {
         ...
         * for translate stage.
         */
        rss_size = (int)rte_flow_conv(RTE_FLOW_CONV_OP_ACTION, NULL, 0, act,
                                      &flow_err);
        if (rss_size <= 0)
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
        ...
        act_cnt->rss = mlx5_malloc(MLX5_MEM_ZERO, rss_size, 0, SOCKET_ID_ANY);
        if (!act_cnt->rss)
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "Fail to malloc rss action memory");
        ret = rte_flow_conv(RTE_FLOW_CONV_OP_ACTION, act_cnt->rss, rss_size,
                            act, &flow_err);
        if (ret < 0)
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
        ...
        act_cnt->fate_action = MLX5_FLOW_FATE_SHARED_RSS;
        action_flags |= MLX5_FLOW_ACTION_RSS;
        break;
      }
      case RTE_FLOW_ACTION_TYPE_PORT_ID:
      case RTE_FLOW_ACTION_TYPE_REPRESENTED_PORT: {
        ...
        memset(&port_id_resource, 0, sizeof(port_id_resource));
        if (flow_dv_translate_action_port_id(dev, act, &port_id, &flow_err))
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "cannot translate "
                                    "policy port action");
        ...
        if (flow_dv_port_id_action_resource_register(dev, &port_id_resource,
          ...
          return -rte_mtr_error_set(error, ENOTSUP,
                                    ...
                                    "cannot setup "
                                    "policy port action");
        act_cnt->rix_port_id_action = dev_flow.handle->rix_port_id_action;
        act_cnt->fate_action = MLX5_FLOW_FATE_PORT_ID;
        action_flags |= MLX5_FLOW_ACTION_PORT_ID;
        break;
      }
      case RTE_FLOW_ACTION_TYPE_JUMP: {
        ...
        struct flow_grp_info grp_info = {
            ...
            .std_tbl_fix = 0,
            .skip_scale =
                dev_flow.skip_scale & (1 << MLX5_SCALE_FLOW_GROUP_BIT),
        };
        struct mlx5_flow_meter_sub_policy *sub_policy =
        ...
        jump_group = ((const struct rte_flow_action_jump *)act->conf)->group;
        if (mlx5_flow_group_to_table(dev, NULL, jump_group, &table, &grp_info,
                                     &flow_err))
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "cannot setup "
                                    "policy jump action");
        sub_policy->jump_tbl[i] = flow_dv_tbl_resource_get(
            dev, table, egress, transfer, !!dev_flow.external, NULL, jump_group,
            0, 0, &flow_err);
        if (!sub_policy->jump_tbl[i])
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "cannot create jump action.");
        tbl_data = container_of(sub_policy->jump_tbl[i],
                                struct mlx5_flow_tbl_data_entry, tbl);
        act_cnt->dr_jump_action[domain] = tbl_data->jump.action;
        act_cnt->fate_action = MLX5_FLOW_FATE_JUMP;
        action_flags |= MLX5_FLOW_ACTION_JUMP;
        break;
      }
      /*
      ...
      case RTE_FLOW_ACTION_TYPE_METER: {
        ...
        mtr = act->conf;
        next_fm = mlx5_flow_meter_find(priv, mtr->mtr_id, &next_mtr_idx);
        if (!next_fm)
          return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_MTR_ID,
                                    NULL, "Fail to find next meter.");
        if (next_fm->def_policy)
          return -rte_mtr_error_set(
              error, EINVAL, RTE_MTR_ERROR_TYPE_MTR_ID, NULL,
              "Hierarchy only supports termination meter.");
        next_policy =
            mlx5_flow_meter_policy_find(dev, next_fm->policy_id, NULL);
        MLX5_ASSERT(next_policy);
        if (next_fm->drop_cnt) {
          set_tag.id = (enum modify_reg)mlx5_flow_get_reg_id(
              dev, MLX5_MTR_ID, 0, (struct rte_flow_error *)error);
          set_tag.offset = (priv->mtr_reg_share ? MLX5_MTR_COLOR_BITS : 0);
          set_tag.length =
              (priv->mtr_reg_share ? MLX5_MTR_IDLE_BITS_IN_COLOR_REG
                                   : MLX5_REG_BITS);
          set_tag.data = next_mtr_idx;
          tag_action.type =
              (enum rte_flow_action_type)MLX5_RTE_FLOW_ACTION_TYPE_TAG;
          tag_action.conf = &set_tag;
          if (flow_dv_convert_action_set_reg(mhdr_res, &tag_action,
                                             (struct rte_flow_error *)error))
            return -rte_errno;
          action_flags |= MLX5_FLOW_ACTION_SET_TAG;
        }
      ...
      if (action_flags & MLX5_FLOW_ACTION_SET_TAG) {
        ...
        if (flow_dv_modify_hdr_resource_register(dev, mhdr_res, &dev_flow,
          ...
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "cannot register policy "
                                    "set tag action");
        act_cnt->modify_hdr = dev_flow.handle->dvh.modify_hdr;
      }
    }
  }
  return 0;
 ...
 *   0 on success, otherwise negative errno value.
 */
static int flow_dv_create_mtr_policy_acts(
    struct rte_eth_dev *dev, struct mlx5_flow_meter_policy *mtr_policy,
    const struct rte_flow_action *actions[RTE_COLORS],
  ...
  for (i = 0; i < MLX5_MTR_DOMAIN_MAX; i++) {
    sub_policy_num =
        ...
        MLX5_MTR_SUB_POLICY_NUM_MASK;
    if (sub_policy_num) {
      ret = __flow_dv_create_domain_policy_acts(
          dev, mtr_policy, actions, (enum mlx5_meter_domain)i, error);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_query_count(struct rte_eth_dev *dev, uint32_t cnt_idx,
  ...
  if (!priv->sh->cdev->config.devx)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "counters are not supported");
  if (cnt_idx) {
    uint64_t pkts, bytes;
    struct mlx5_flow_counter *cnt;
    ...
    if (err)
      return rte_flow_error_set(error, -err, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
...
int flow_dv_action_query(struct rte_eth_dev *dev,
  ...
  case MLX5_INDIRECT_ACTION_TYPE_AGE:
    ...
    resp = data;
    resp->aged =
        __atomic_load_n(&age_param->state, __ATOMIC_RELAXED) == AGE_TMOUT ? 1
                                                                          : 0;
    resp->sec_since_last_hit_valid = !resp->aged;
    if (resp->sec_since_last_hit_valid)
      resp->sec_since_last_hit =
          __atomic_load_n(&age_param->sec_since_last_hit, __ATOMIC_RELAXED);
    return 0;
  case MLX5_INDIRECT_ACTION_TYPE_COUNT:
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_query_age(struct rte_eth_dev *dev, struct rte_flow *flow,
                             void *data, struct rte_flow_error *error) {
  struct rte_flow_query_age *resp = data;
  struct mlx5_age_param *age_param;

  if (flow->age) {
    struct mlx5_aso_age_action *act = flow_aso_age_get_by_idx(dev, flow->age);
  ...
  resp->aged =
      __atomic_load_n(&age_param->state, __ATOMIC_RELAXED) == AGE_TMOUT ? 1 : 0;
  resp->sec_since_last_hit_valid = !resp->aged;
  if (resp->sec_since_last_hit_valid)
    resp->sec_since_last_hit =
        __atomic_load_n(&age_param->sec_since_last_hit, __ATOMIC_RELAXED);
 ...
 * @see rte_flow_ops
 */
static int flow_dv_query(struct rte_eth_dev *dev,
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_VOID:
      break;
    case RTE_FLOW_ACTION_TYPE_COUNT:
      ret = flow_dv_query_count(dev, flow->counter, data, error);
      break;
    case RTE_FLOW_ACTION_TYPE_AGE:
 ...
 */
static void flow_dv_destroy_mtr_tbls(struct rte_eth_dev *dev,
                                     struct mlx5_flow_meter_info *fm) {
  struct mlx5_priv *priv = dev->data->dev_private;
  int i;
...
static void flow_dv_destroy_mtr_drop_tbls(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_mtr_mng *mtrmng = priv->sh->mtrmng;
  struct mlx5_flow_tbl_data_entry *tbl;
  int i, j;
  ...
  for (i = 0; i < MLX5_MTR_DOMAIN_MAX; i++) {
    if (mtrmng->def_rule[i]) {
      claim_zero(mlx5_flow_os_destroy_flow(mtrmng->def_rule[i]));
      mtrmng->def_rule[i] = NULL;
    }
    if (mtrmng->def_matcher[i]) {
      tbl = container_of(mtrmng->def_matcher[i]->tbl,
                         struct mlx5_flow_tbl_data_entry, tbl);
      mlx5_list_unregister(tbl->matchers, &mtrmng->def_matcher[i]->entry);
      mtrmng->def_matcher[i] = NULL;
    }
    for (j = 0; j < MLX5_REG_BITS; j++) {
      if (mtrmng->drop_matcher[i][j]) {
        tbl = container_of(mtrmng->drop_matcher[i][j]->tbl,
                           struct mlx5_flow_tbl_data_entry, tbl);
        mlx5_list_unregister(tbl->matchers, &mtrmng->drop_matcher[i][j]->entry);
        mtrmng->drop_matcher[i][j] = NULL;
      }
    }
    if (mtrmng->drop_tbl[i]) {
      flow_dv_tbl_resource_release(MLX5_SH(dev), mtrmng->drop_tbl[i]);
...
static void __flow_dv_destroy_domain_def_policy(struct rte_eth_dev *dev,
 ...
 *   Pointer to Ethernet device.
 */
static void flow_dv_destroy_def_policy(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  int i;

  for (i = 0; i < MLX5_MTR_DOMAIN_MAX; i++)
    if (priv->sh->mtrmng->def_policy[i])
      __flow_dv_destroy_domain_def_policy(dev, (enum mlx5_meter_domain)i);
  priv->sh->mtrmng->def_policy_id = MLX5_INVALID_POLICY_ID;
}

static int
__flow_dv_create_policy_flow(struct rte_eth_dev *dev, uint32_t color_reg_c_idx,
                             enum rte_color color, void *matcher_object,
                             int actions_n, void *actions, bool match_src_port,
                             const struct rte_flow_item *item, void **rule,
                             const struct rte_flow_attr *attr) {
  int ret;
  ...
  if (match_src_port && priv->sh->esw_mode) {
    if (flow_dv_translate_item_port_id(dev, matcher.buf, value.buf, item,
                                       attr)) {
      DRV_LOG(ERR,
              "Failed to create meter policy%d flow's"
              " value with port.",
              color);
      return -1;
    }
  ...
  ret = mlx5_flow_os_create_flow(matcher_object, (void *)&value, actions_n,
                                 actions, rule);
  if (ret) {
...
static int __flow_dv_create_policy_matcher(
    struct rte_eth_dev *dev, uint32_t color_reg_c_idx, uint16_t priority,
    struct mlx5_flow_meter_sub_policy *sub_policy,
    const struct rte_flow_attr *attr, bool match_src_port,
    const struct rte_flow_item *item,
  ...
  if (match_src_port && priv->sh->esw_mode) {
    if (flow_dv_translate_item_port_id(dev, matcher.mask.buf, value.buf, item,
                                       attr)) {
      DRV_LOG(ERR,
              "Failed to register meter policy%d matcher"
              " with port.",
              priority);
      return -1;
    }
  ...
  *policy_matcher = container_of(entry, struct mlx5_flow_dv_matcher, entry);
  return 0;
}
 ...
 */
static int __flow_dv_create_domain_policy_rules(
    struct rte_eth_dev *dev, struct mlx5_flow_meter_sub_policy *sub_policy,
  ...
  if (!sub_policy->tbl_rsc)
    sub_policy->tbl_rsc = flow_dv_tbl_resource_get(
        dev, MLX5_FLOW_TABLE_LEVEL_POLICY, egress, transfer, false, NULL, 0, 0,
        sub_policy->idx, &flow_err);
  if (!sub_policy->tbl_rsc) {
    DRV_LOG(ERR, "Failed to create meter sub policy table.");
    return -1;
  }
  /* Prepare matchers. */
  color_reg_c_idx = ret;
  for (i = 0; i < RTE_COLORS; i++) {
    ...
    if (!acts[i].actions_n)
      continue;
    color_rule =
        mlx5_malloc(MLX5_MEM_ZERO, sizeof(struct mlx5_sub_policy_color_rule), 0,
                    SOCKET_ID_ANY);
    if (!color_rule) {
    ...
    TAILQ_INSERT_TAIL(&sub_policy->color_rules[i], color_rule, next_port);
    color_rule->src_port = priv->representor_id;
    /* No use. */
    attr.priority = i;
    /* Create matchers for colors. */
    svport_match = (i != RTE_COLOR_RED) ? match_src_port : false;
    if (__flow_dv_create_policy_matcher(
            dev, color_reg_c_idx, MLX5_MTR_POLICY_MATCHER_PRIO, sub_policy,
            &attr, svport_match, NULL, &color_rule->matcher, &flow_err)) {
      DRV_LOG(ERR, "Failed to create color%u matcher.", i);
      goto err_exit;
    }
    /* Create flow, matching color. */
    if (__flow_dv_create_policy_flow(
            dev, color_reg_c_idx, (enum rte_color)i,
            color_rule->matcher->matcher_object, acts[i].actions_n,
            acts[i].dv_actions, svport_match, NULL, &color_rule->rule, &attr)) {
...
static int __flow_dv_create_policy_acts_rules(
    struct rte_eth_dev *dev, struct mlx5_flow_meter_policy *mtr_policy,
    struct mlx5_flow_meter_sub_policy *sub_policy, uint32_t domain) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_meter_policy_acts acts[RTE_COLORS];
  struct mlx5_flow_dv_tag_resource *tag;
  ...
  for (i = 0; i < RTE_COLORS; i++) {
    ...
    if (i == RTE_COLOR_RED) {
      ...
      acts[i].dv_actions[0] = mtr_policy->dr_drop_action[domain];
      acts[i].actions_n = 1;
      continue;
    }
    if (i == RTE_COLOR_GREEN &&
        mtr_policy->act_cnt[i].fate_action == MLX5_FLOW_FATE_MTR) {
      struct rte_flow_attr attr = {.transfer = transfer};

      next_fm =
          mlx5_flow_meter_find(priv, mtr_policy->act_cnt[i].next_mtr_id, NULL);
      if (!next_fm) {
        DRV_LOG(ERR, "Failed to get next hierarchy meter.");
        goto err_exit;
      }
      ...
      if (mtr_first) {
        acts[i].dv_actions[acts[i].actions_n] = next_fm->meter_action;
        acts[i].actions_n++;
      }
    }
    if (mtr_policy->act_cnt[i].rix_mark) {
    ...
    if (mtr_policy->act_cnt[i].fate_action) {
      switch (mtr_policy->act_cnt[i].fate_action) {
      case MLX5_FLOW_FATE_PORT_ID:
        port_action = mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_PORT_ID],
        ...
        acts[i].dv_actions[acts[i].actions_n] = port_action->action;
        acts[i].actions_n++;
        mtr_policy->dev = dev;
        match_src_port = true;
        break;
      ...
      case MLX5_FLOW_FATE_QUEUE:
        ...
        acts[i].dv_actions[acts[i].actions_n] = hrxq->action;
        acts[i].actions_n++;
        break;
      case MLX5_FLOW_FATE_MTR:
        if (!next_fm) {
          DRV_LOG(ERR, "No next hierarchy meter.");
          goto err_exit;
        }
        if (!mtr_first) {
          acts[i].dv_actions[acts[i].actions_n] = next_fm->meter_action;
        ...
        if (mtr_policy->act_cnt[i].next_sub_policy) {
          next_sub_policy = mtr_policy->act_cnt[i].next_sub_policy;
        } else {
          next_policy =
              mlx5_flow_meter_policy_find(dev, next_fm->policy_id, NULL);
          MLX5_ASSERT(next_policy);
        ...
        tbl_data = container_of(next_sub_policy->tbl_rsc,
                                struct mlx5_flow_tbl_data_entry, tbl);
        acts[i].dv_actions[acts[i].actions_n++] = tbl_data->jump.action;
        if (mtr_policy->act_cnt[i].modify_hdr)
  ...
  if (__flow_dv_create_domain_policy_rules(dev, sub_policy, egress, transfer,
                                           match_src_port, acts)) {
...
static int __flow_dv_create_domain_def_policy(struct rte_eth_dev *dev,
                                              uint32_t domain) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_mtr_mng *mtrmng = priv->sh->mtrmng;
  struct mlx5_flow_meter_def_policy *def_policy;
  ...
  if (!def_policy) {
    def_policy =
        mlx5_malloc(MLX5_MEM_ZERO, sizeof(struct mlx5_flow_meter_def_policy),
    ...
    jump_tbl = flow_dv_tbl_resource_get(dev, MLX5_FLOW_TABLE_LEVEL_METER,
                                        egress, transfer, false, NULL, 0, 0,
                                        MLX5_MTR_TABLE_ID_SUFFIX, &error);
    if (!jump_tbl) {
      DRV_LOG(ERR, "Failed to create meter suffix table.");
      goto def_policy_error;
    }
    def_policy->sub_policy.jump_tbl[RTE_COLOR_GREEN] = jump_tbl;
    tbl_data = container_of(jump_tbl, struct mlx5_flow_tbl_data_entry, tbl);
    def_policy->dr_jump_action[RTE_COLOR_GREEN] = tbl_data->jump.action;
    ...
    jump_tbl = flow_dv_tbl_resource_get(dev, MLX5_FLOW_TABLE_LEVEL_METER,
                                        ...
                                        MLX5_MTR_TABLE_ID_SUFFIX, &error);
    if (!jump_tbl) {
      DRV_LOG(ERR, "Failed to get meter suffix table.");
      goto def_policy_error;
    }
    def_policy->sub_policy.jump_tbl[RTE_COLOR_YELLOW] = jump_tbl;
    tbl_data = container_of(jump_tbl, struct mlx5_flow_tbl_data_entry, tbl);
    def_policy->dr_jump_action[RTE_COLOR_YELLOW] = tbl_data->jump.action;
    ...
    if (!mtrmng->drop_tbl[domain]) {
      mtrmng->drop_tbl[domain] = flow_dv_tbl_resource_get(
          dev, MLX5_FLOW_TABLE_LEVEL_METER, egress, transfer, false, NULL, 0, 0,
          MLX5_MTR_TABLE_ID_DROP, &error);
      if (!mtrmng->drop_tbl[domain]) {
    ...
    tbl_data = container_of(mtrmng->drop_tbl[domain],
                            struct mlx5_flow_tbl_data_entry, tbl);
    def_policy->dr_jump_action[RTE_COLOR_RED] = tbl_data->jump.action;
    acts[RTE_COLOR_RED].dv_actions[0] = tbl_data->jump.action;
    acts[RTE_COLOR_RED].actions_n = 1;
    /* Create default policy rules. */
    ret = __flow_dv_create_domain_policy_rules(dev, &def_policy->sub_policy,
  ...
  return 0;
def_policy_error:
  __flow_dv_destroy_domain_def_policy(dev, (enum mlx5_meter_domain)domain);
  return -1;
}
 ...
 *   0 on success, -1 otherwise.
 */
static int flow_dv_create_def_policy(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  int i;

  /* Non-termination policy table. */
  for (i = 0; i < MLX5_MTR_DOMAIN_MAX; i++) {
    if (!priv->sh->config.dv_esw_en && i == MLX5_MTR_DOMAIN_TRANSFER)
      continue;
 ...
 *   0 on success, -1 otherwise.
 */
static int flow_dv_create_mtr_tbls(struct rte_eth_dev *dev,
                                   struct mlx5_flow_meter_info *fm,
                                   uint32_t mtr_idx, uint8_t domain_bitmap) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_mtr_mng *mtrmng = priv->sh->mtrmng;
  struct rte_flow_error error;
  struct mlx5_flow_tbl_data_entry *tbl_data;
  ...
  int mtr_id_reg_c = mlx5_flow_get_reg_id(dev, MLX5_MTR_ID, 0, &error);
  ...
  for (domain = 0; domain < MLX5_MTR_DOMAIN_MAX; domain++) {
    ...
    if (!mtrmng->def_rule[domain]) {
      ...
      ret = mlx5_flow_os_create_flow(
          mtrmng->def_matcher[domain]->matcher_object, (void *)&value, i,
          actions, &mtrmng->def_rule[domain]);
      if (ret) {
        DRV_LOG(ERR, "Failed to create meter "
    ...
    drop_matcher = mtrmng->drop_matcher[domain][mtrmng->max_mtr_bits - 1];
    /* Create drop rule, matching meter_id only. */
    flow_dv_match_meta_reg(matcher_para.buf, value.buf,
                           (enum modify_reg)mtr_id_reg_c,
                           (mtr_idx << mtr_id_offset), UINT32_MAX);
    i = 0;
    cnt = flow_dv_counter_get_by_idx(dev, fm->drop_cnt, NULL);
    actions[i++] = cnt->action;
    actions[i++] = priv->sh->dr_drop_action;
    misc_mask = flow_dv_matcher_enable(value.buf);
    __flow_dv_adjust_buf_size(&value.size, misc_mask);
    ret = mlx5_flow_os_create_flow(drop_matcher->matcher_object, (void *)&value,
                                   i, actions, &fm->drop_rule[domain]);
    if (ret) {
      DRV_LOG(ERR, "Failed to create meter "
...
static struct mlx5_flow_meter_sub_policy *__flow_dv_meter_get_rss_sub_policy(
    struct rte_eth_dev *dev, struct mlx5_flow_meter_policy *mtr_policy,
    struct mlx5_flow_rss_desc *rss_desc[MLX5_MTR_RTE_COLORS],
    struct mlx5_flow_meter_sub_policy *next_sub_policy, bool *is_reuse) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  sub_policy_num =
      (mtr_policy->sub_policy_num >> (MLX5_MTR_SUB_POLICY_NUM_SHIFT * domain)) &
      MLX5_MTR_SUB_POLICY_NUM_MASK;
  for (j = 0; j < sub_policy_num; j++) {
    for (i = 0; i < MLX5_MTR_RTE_COLORS; i++) {
      if (rss_desc[i] &&
          hrxq_idx[i] != mtr_policy->sub_policys[domain][j]->rix_hrxq[i])
        break;
    }
    if (i >= MLX5_MTR_RTE_COLORS) {
      /*
  ...
  if (!mtr_policy->sub_policys[domain][0]->rix_hrxq[0]) {
    ...
    sub_policy = mlx5_ipool_zmalloc(priv->sh->ipool[MLX5_IPOOL_MTR_POLICY],
                                    &sub_policy_idx);
    if (!sub_policy || sub_policy_idx > MLX5_MAX_SUB_POLICY_TBL_NUM) {
      for (i = 0; i < MLX5_MTR_RTE_COLORS; i++)
        mlx5_hrxq_release(dev, hrxq_idx[i]);
  ...
  for (i = 0; i < MLX5_MTR_RTE_COLORS; i++) {
    ...
    if (mtr_policy->is_hierarchy) {
       ...
       */
      hrxq = mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_HRXQ], hrxq_idx[i]);
      if (!hrxq) {
  ...
  if (__flow_dv_create_policy_acts_rules(dev, mtr_policy, sub_policy, domain)) {
  ...
  if (sub_policy != mtr_policy->sub_policys[domain][0]) {
    ...
    mtr_policy->sub_policy_num &=
        ~(MLX5_MTR_SUB_POLICY_NUM_MASK
          << (MLX5_MTR_SUB_POLICY_NUM_SHIFT * domain));
    mtr_policy->sub_policy_num |= (i & MLX5_MTR_SUB_POLICY_NUM_MASK)
                                  << (MLX5_MTR_SUB_POLICY_NUM_SHIFT * domain);
  }
  rte_spinlock_unlock(&mtr_policy->sl);
  *is_reuse = false;
...
rss_sub_policy_error:
  if (sub_policy) {
    ...
    if (sub_policy != mtr_policy->sub_policys[domain][0]) {
      ...
      mlx5_ipool_free(priv->sh->ipool[MLX5_IPOOL_MTR_POLICY], sub_policy->idx);
    }
  }
  rte_spinlock_unlock(&mtr_policy->sl);
  return NULL;
 ...
 */
static struct mlx5_flow_meter_sub_policy *flow_dv_meter_sub_policy_rss_prepare(
    struct rte_eth_dev *dev, struct mlx5_flow_meter_policy *mtr_policy,
    struct mlx5_flow_rss_desc *rss_desc[MLX5_MTR_RTE_COLORS]) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_meter_sub_policy *sub_policy = NULL;
  ...
  while (true) {
    ...
    next_fm = mlx5_flow_meter_find(
        priv, mtr_policy->act_cnt[RTE_COLOR_GREEN].next_mtr_id, NULL);
    if (!next_fm) {
  ...
  while (i) {
     ...
     */
    sub_policy = __flow_dv_meter_get_rss_sub_policy(
        dev, policies[--i], rss_desc, next_sub_policy, &reuse_sub_policy);
    if (!sub_policy) {
  ...
  while (j) {
    ...
    if (sub_policy != mtr_policy->sub_policys[domain][0]) {
      sub_policy_num = (mtr_policy->sub_policy_num >>
                        (MLX5_MTR_SUB_POLICY_NUM_SHIFT * domain)) &
                       MLX5_MTR_SUB_POLICY_NUM_MASK;
      mtr_policy->sub_policys[domain][sub_policy_num - 1] = NULL;
      sub_policy_num--;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_dv_meter_hierarchy_rule_create(struct rte_eth_dev *dev,
                                               struct mlx5_flow_meter_info *fm,
                                               int32_t src_port,
                                               const struct rte_flow_item *item,
                                               struct rte_flow_error *error) {
  ...
  for (i = 0; i < RTE_COLORS; i++) {
    ...
    color_rule =
        mlx5_malloc(MLX5_MEM_ZERO, sizeof(struct mlx5_sub_policy_color_rule), 0,
                    SOCKET_ID_ANY);
    if (!color_rule)
      return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
    ...
    if (__flow_dv_create_policy_flow(dev, color_reg_c_idx, (enum rte_color)i,
                                     color_rule->matcher->matcher_object,
                                     acts.actions_n, acts.dv_actions, true,
                                     item, &color_rule->rule, &attr)) {
      rte_flow_error_set(error, errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
...
exit:
   ...
   * create needed rules.
   */
  return flow_dv_meter_hierarchy_rule_create(dev, next_fm, src_port, item,
                                             error);
err_exit:
  if (color_rule) {
    if (color_rule->rule)
...
flow_dv_destroy_sub_policy_with_rxq(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < MLX5_MTR_RTE_COLORS; i++) {
    ...
    case MLX5_FLOW_FATE_SHARED_RSS:
      ...
      for (j = 0; j < sub_policy_num; j++) {
        sub_policy = mtr_policy->sub_policys[domain][j];
        if (sub_policy) {
          __flow_dv_destroy_sub_policy_rules(dev, sub_policy);
          if (sub_policy != mtr_policy->sub_policys[domain][0]) {
            mtr_policy->sub_policys[domain][j] = NULL;
      ...
      if (new_policy_num != sub_policy_num) {
        mtr_policy->sub_policy_num &=
            ~(MLX5_MTR_SUB_POLICY_NUM_MASK
              << (MLX5_MTR_SUB_POLICY_NUM_SHIFT * domain));
        mtr_policy->sub_policy_num |=
            (new_policy_num & MLX5_MTR_SUB_POLICY_NUM_MASK)
            << (MLX5_MTR_SUB_POLICY_NUM_SHIFT * domain);
      }
      break;
    case MLX5_FLOW_FATE_QUEUE:
...
int mlx5_flow_discover_dr_action_support(struct rte_eth_dev *dev) {
  ...
  tbl = flow_dv_tbl_resource_get(dev, 0, 0, 0, false, NULL, 0, 0, 0, NULL);
  if (!tbl)
    goto err;
  dv_attr.match_criteria_enable = flow_dv_matcher_enable(mask.buf);
  __flow_dv_adjust_buf_size(&mask.size, dv_attr.match_criteria_enable);
  ret = mlx5_flow_os_create_flow_matcher(sh->cdev->ctx, &dv_attr, tbl->obj,
                                         &matcher);
  if (ret)
    goto err;
...
err:
   ...
   */
  if (!flow) {
    if (matcher && (errno == EPROTONOSUPPORT || errno == EOPNOTSUPP))
...
int mlx5_flow_dv_discover_counter_offset_support(struct rte_eth_dev *dev) {
  ...
  tbl = flow_dv_tbl_resource_get(dev, 0, 1, 0, false, NULL, 0, 0, 0, NULL);
  if (!tbl)
    goto err;
  dcs = mlx5_devx_cmd_flow_counter_alloc(priv->sh->cdev->ctx, 0x4);
  if (!dcs)
    goto err;
  ret =
      mlx5_flow_os_create_flow_action_count(dcs->obj, UINT16_MAX, &actions[0]);
  if (ret)
    goto err;
  dv_attr.match_criteria_enable = flow_dv_matcher_enable(mask.buf);
  __flow_dv_adjust_buf_size(&mask.size, dv_attr.match_criteria_enable);
  ...
  if (ret)
    goto err;
  __flow_dv_adjust_buf_size(&value.size, dv_attr.match_criteria_enable);
  ret = mlx5_flow_os_create_flow(matcher, (void *)&value, 1, actions, &flow);
err:
  /*
 ...
 */
static int flow_dv_counter_query(struct rte_eth_dev *dev, uint32_t counter,
 ...
 */
static int flow_dv_get_aged_flows(struct rte_eth_dev *dev, void **context,
 ...
 */
static uint32_t flow_dv_counter_allocate(struct rte_eth_dev *dev) {
...
int flow_dv_action_validate(struct rte_eth_dev *dev,
  ...
  case RTE_FLOW_ACTION_TYPE_AGE:
    if (!priv->sh->aso_age_mng)
      return rte_flow_error_set(err, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Indirect age action not supported");
...
flow_dv_mtr_policy_rss_compare(const struct rte_flow_action_rss *r1,
  ...
  if (r1 == NULL || r2 == NULL)
    return 0;
  if (!(r1->level <= 1 && r2->level <= 1) && !(r1->level > 1 && r2->level > 1))
    return 1;
 ...
 */
static int flow_dv_validate_policy_mtr_hierarchy(
    struct rte_eth_dev *dev, uint32_t meter_id, uint64_t action_flags,
    bool *is_rss, uint8_t *hierarchy_domain, struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_meter_info *fm;
  ...
  *hierarchy_domain = 0;
  while (true) {
    ...
    if (!*hierarchy_domain) {
      if (policy->transfer)
        *hierarchy_domain |= MLX5_MTR_DOMAIN_TRANSFER_BIT;
      if (policy->ingress)
        *hierarchy_domain |= MLX5_MTR_DOMAIN_INGRESS_BIT;
      if (policy->egress)
    ...
    if (++cnt >= MLX5_MTR_CHAIN_MAX_NUM)
      return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_POLICY,
                                NULL, "Exceed max hierarchy meter number.");
  }
  return 0;
}
 ...
 *   0 on success, otherwise negative errno value.
 */
static int flow_dv_validate_mtr_policy_acts(
    struct rte_eth_dev *dev, const struct rte_flow_action *actions[RTE_COLORS],
    struct rte_flow_attr *attr, bool *is_rss, uint8_t *domain_bitmap,
    uint8_t *policy_mode, struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_sh_config *dev_conf = &priv->sh->config;
  const struct rte_flow_action *act;
  ...
  *domain_bitmap = def_domain;
  ...
  if (!actions[RTE_COLOR_RED] ||
    ...
    return -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY,
  ...
  for (i = 0; i < RTE_COLORS; i++) {
    ...
    for (action_flags[i] = 0, actions_n = 0;
      ...
      case RTE_FLOW_ACTION_TYPE_REPRESENTED_PORT:
        if (!dev_conf->dv_esw_en)
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "PORT action validate check"
                                    " fail for ESW disable");
        ret = flow_dv_validate_action_port_id(dev, action_flags[i], act, attr,
                                              &flow_err);
        if (ret)
          return -rte_mtr_error_set(
              error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
        ...
        break;
      case RTE_FLOW_ACTION_TYPE_MARK:
        ret = flow_dv_validate_action_mark(dev, act, action_flags[i], attr,
                                           &flow_err);
        if (ret < 0)
          return -rte_mtr_error_set(
              error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
        ...
        break;
      case RTE_FLOW_ACTION_TYPE_SET_TAG:
        ret = flow_dv_validate_action_set_tag(dev, act, action_flags[i], attr,
                                              &flow_err);
        if (ret)
          return -rte_mtr_error_set(
              error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
        ...
        break;
      case RTE_FLOW_ACTION_TYPE_DROP:
        ret = mlx5_flow_validate_action_drop(action_flags[i], attr, &flow_err);
        if (ret < 0)
          return -rte_mtr_error_set(
              error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
              flow_err.message ? flow_err.message
      ...
      case RTE_FLOW_ACTION_TYPE_QUEUE:
        ...
        if (dev_conf->dv_flow_en &&
          ...
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "Queue action with meta "
                                    "is not supported. Please try use "
                                    "default policy for meter.");
        ret = mlx5_flow_validate_action_queue(act, action_flags[i], dev, attr,
                                              &flow_err);
        if (ret < 0)
          return -rte_mtr_error_set(
              error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
      ...
      case RTE_FLOW_ACTION_TYPE_RSS:
        if (dev_conf->dv_flow_en &&
          ...
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "RSS action with meta "
                                    "is not supported. Please try use "
                                    "default policy for meter.");
        ret = mlx5_validate_action_rss(dev, act, &flow_err);
        if (ret < 0)
          return -rte_mtr_error_set(
              error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
              flow_err.message ? flow_err.message
        ...
        break;
      case RTE_FLOW_ACTION_TYPE_JUMP:
        ret = flow_dv_validate_action_jump(dev, NULL, act, action_flags[i],
                                           attr, true, &flow_err);
        if (ret)
          return -rte_mtr_error_set(
              error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
      ...
      case RTE_FLOW_ACTION_TYPE_METER:
        ...
        mtr = act->conf;
        ret = flow_dv_validate_policy_mtr_hierarchy(dev, mtr->mtr_id,
                                                    action_flags[i], is_rss,
                                                    &hierarchy_domain, error);
        if (ret)
          return ret;
        ++actions_n;
        action_flags[i] |= MLX5_FLOW_ACTION_METER_WITH_TERMINATED_POLICY;
        break;
      default:
        return -rte_mtr_error_set(error, ENOTSUP,
                                  RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
    ...
    if (action_flags[i] & MLX5_FLOW_ACTION_METER_WITH_TERMINATED_POLICY)
    ...
    if (domain_color[i] & MLX5_MTR_DOMAIN_TRANSFER_BIT) {
      ...
      if (!(action_flags[i] & MLX5_FLOW_FATE_ACTIONS) &&
          (domain_color[i] & MLX5_MTR_DOMAIN_INGRESS_BIT)) {
        if ((domain_color[i] & MLX5_MTR_DOMAIN_EGRESS_BIT))
          domain_color[i] = MLX5_MTR_DOMAIN_EGRESS_BIT;
        else
          return -rte_mtr_error_set(error, ENOTSUP,
                                    RTE_MTR_ERROR_TYPE_METER_POLICY, NULL,
                                    "no fate action is found");
...
static int flow_dv_sync_domain(struct rte_eth_dev *dev, uint32_t domains,
                               uint32_t flags) {
  struct mlx5_priv *priv = dev->data->dev_private;
  int ret = 0;

  if ((domains & MLX5_DOMAIN_BIT_NIC_RX) && priv->sh->rx_domain != NULL) {
    ret = mlx5_os_flow_dr_sync_domain(priv->sh->rx_domain, flags);
    if (ret != 0)
      return ret;
  }
 ...
 */
static int flow_dv_discover_priorities(struct rte_eth_dev *dev,
  ...
  flow.dv.actions_n = 1;
  memset(&eth, 0, sizeof(eth));
  flow_dv_translate_item_eth(matcher.mask.buf, flow.dv.value.buf, &item,
                             /* inner */ false, /* group */ 0);
  ...
  for (i = 0; i < vprio_n; i++) {
    ...
    err = mlx5_flow_os_create_flow(flow.handle->dvh.matcher->matcher_object,
                                   (void *)&flow.dv.value, flow.dv.actions_n,
                                   flow.dv.actions, &flow.handle->drv_flow);
    if (err == 0) {
      claim_zero(mlx5_flow_os_destroy_flow(flow.handle->drv_flow));
