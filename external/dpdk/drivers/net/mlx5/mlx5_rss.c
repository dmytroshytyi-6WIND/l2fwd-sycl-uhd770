  ...
  if (rss_conf->rss_key && rss_conf->rss_key_len) {
    ...
    priv->rss_conf.rss_key =
        mlx5_realloc(priv->rss_conf.rss_key, MLX5_MEM_RTE,
                     rss_conf->rss_key_len, 0, SOCKET_ID_ANY);
    if (!priv->rss_conf.rss_key) {
      rte_errno = ENOMEM;
      return -rte_errno;
    }
    memcpy(priv->rss_conf.rss_key, rss_conf->rss_key, rss_conf->rss_key_len);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_rss_reta_index_resize(struct rte_eth_dev *dev,
  ...
  mem =
      mlx5_realloc(priv->reta_idx, MLX5_MEM_RTE,
                   reta_size * sizeof((*priv->reta_idx)[0]), 0, SOCKET_ID_ANY);
  if (!mem) {
    rte_errno = ENOMEM;
    return -rte_errno;
  }
  ...
  if (old_size < reta_size)
    memset(&(*priv->reta_idx)[old_size], 0,
           (reta_size - old_size) * sizeof((*priv->reta_idx)[0]));
  return 0;
}
