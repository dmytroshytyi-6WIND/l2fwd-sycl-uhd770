...
void txq_free_elts(struct mlx5_txq_ctrl *txq_ctrl) {
  ...
  DRV_LOG(DEBUG, "port %u Tx queue %u freeing WRs", PORT_ID(txq_ctrl->priv),
          txq_ctrl->txq.idx);
  txq_ctrl->txq.elts_head = 0;
  txq_ctrl->txq.elts_tail = 0;
  txq_ctrl->txq.elts_comp = 0;
  ...
  while (elts_tail != elts_head) {
    ...
    /* Poisoning. */
    memset(&(*elts)[elts_tail & elts_m], 0x77,
...
uint64_t mlx5_get_tx_port_offloads(struct rte_eth_dev *dev) {
  ...
  if (priv->sh->config.tx_pp || priv->sh->cdev->config.hca_attr.wait_on_time)
  ...
  if (dev_cap->tunnel_en) {
    if (dev_cap->hw_csum)
      offloads |= RTE_ETH_TX_OFFLOAD_OUTER_IPV4_CKSUM;
    if (dev_cap->tso) {
      if (dev_cap->tunnel_en & MLX5_TUNNELED_OFFLOADS_VXLAN_CAP)
        offloads |= RTE_ETH_TX_OFFLOAD_VXLAN_TNL_TSO;
      if (dev_cap->tunnel_en & MLX5_TUNNELED_OFFLOADS_GRE_CAP)
        offloads |= RTE_ETH_TX_OFFLOAD_GRE_TNL_TSO;
      if (dev_cap->tunnel_en & MLX5_TUNNELED_OFFLOADS_GENEVE_CAP)
...
static void txq_sync_cq(struct mlx5_txq_data *txq) {
 ...
 */
int mlx5_tx_queue_stop_primary(struct rte_eth_dev *dev, uint16_t idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_tx_queue_stop(struct rte_eth_dev *dev, uint16_t idx) {
  int ret;
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ret = mlx5_mp_os_req_queue_control(dev, idx, MLX5_MP_REQ_QUEUE_TX_STOP);
  } else {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_tx_queue_start_primary(struct rte_eth_dev *dev, uint16_t idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_txq_data *txq = (*priv->txqs)[idx];
  struct mlx5_txq_ctrl *txq_ctrl = container_of(txq, struct mlx5_txq_ctrl, txq);
  int ret;
  ...
  MLX5_ASSERT(rte_eal_process_type() == RTE_PROC_PRIMARY);
  ret = priv->obj_ops.txq_obj_modify(txq_ctrl->obj, MLX5_TXQ_MOD_RST2RDY,
                                     (uint8_t)priv->dev_port);
  if (ret)
    return ret;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_tx_queue_start(struct rte_eth_dev *dev, uint16_t idx) {
  int ret;
  ...
  if (dev->data->tx_queue_state[idx] == RTE_ETH_QUEUE_STATE_STARTED)
    return 0;
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ret = mlx5_mp_os_req_queue_control(dev, idx, MLX5_MP_REQ_QUEUE_TX_START);
  } else {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_tx_queue_pre_setup(struct rte_eth_dev *dev, uint16_t idx,
                                   uint16_t *desc) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (!mlx5_txq_releasable(dev, idx)) {
    rte_errno = EBUSY;
    DRV_LOG(ERR, "port %u unable to release queue index %u", dev->data->port_id,
            idx);
    return -rte_errno;
  }
...
int mlx5_tx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc,
  ...
  DRV_LOG(DEBUG, "port %u adding Tx queue %u to list", dev->data->port_id, idx);
...
int mlx5_tx_hairpin_queue_setup(
  ...
  if (hairpin_conf->peer_count != 1) {
    rte_errno = EINVAL;
    DRV_LOG(ERR,
            "port %u unable to setup Tx hairpin queue index %u"
            " peer count is %u",
  ...
  if (hairpin_conf->peers[0].port == dev->data->port_id) {
    if (hairpin_conf->peers[0].queue >= priv->rxqs_n) {
      rte_errno = EINVAL;
      DRV_LOG(ERR,
              "port %u unable to setup Tx hairpin queue"
...
void mlx5_tx_queue_release(struct rte_eth_dev *dev, uint16_t qid) {
  ...
  if (txq == NULL)
    return;
  DRV_LOG(DEBUG, "port %u removing Tx queue %u from list", dev->data->port_id,
          qid);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int txq_uar_init_secondary(struct mlx5_txq_ctrl *txq_ctrl, int fd) {
  ...
  addr = rte_mem_map(NULL, page_size, RTE_PROT_WRITE, RTE_MAP_SHARED, fd,
                     txq_ctrl->uar_mmap_offset);
  if (!addr) {
    DRV_LOG(ERR, "Port %u mmap failed for BF reg of txq %u.", txq->port_id,
            txq->idx);
    rte_errno = ENXIO;
    return -rte_errno;
  }
...
#ifndef RTE_ARCH_64
  ppriv->uar_table[txq->idx].sl_p = primary_ppriv->uar_table[txq->idx].sl_p;
#endif
  return 0;
}
 ...
 *   Pointer to Tx queue control structure.
 */
static void txq_uar_uninit_secondary(struct mlx5_txq_ctrl *txq_ctrl) {
...
void mlx5_tx_uar_uninit_secondary(struct rte_eth_dev *dev) {
  struct mlx5_proc_priv *ppriv = (struct mlx5_proc_priv *)dev->process_private;
  const size_t page_size = rte_mem_page_size();
  void *addr;
  unsigned int i;
 ...
 *   The number of WQEBB.
 */
static int txq_calc_wqebb_cnt(struct mlx5_txq_ctrl *txq_ctrl) {
  unsigned int wqe_size;
  const unsigned int desc = 1 << txq_ctrl->txq.elts_n;

  wqe_size = MLX5_WQE_CSEG_SIZE + MLX5_WQE_ESEG_SIZE + MLX5_WSEG_SIZE -
             MLX5_ESEG_MIN_INLINE_SIZE + txq_ctrl->max_inline_data;
 ...
 */
static unsigned int txq_calc_inline_max(struct mlx5_txq_ctrl *txq_ctrl) {
  const unsigned int desc = 1 << txq_ctrl->txq.elts_n;
  struct mlx5_priv *priv = txq_ctrl->priv;
  unsigned int wqe_size;
   ...
   */
  wqe_size = wqe_size * MLX5_WQE_SIZE - MLX5_WQE_CSEG_SIZE -
             MLX5_WQE_ESEG_SIZE - MLX5_WSEG_SIZE - MLX5_WSEG_SIZE +
 ...
 *   Pointer to Tx queue control structure.
 */
static void txq_set_params(struct mlx5_txq_ctrl *txq_ctrl) {
  struct mlx5_priv *priv = txq_ctrl->priv;
  struct mlx5_port_config *config = &priv->config;
  struct mlx5_dev_cap *dev_cap = &priv->sh->dev_cap;
  ...
  bool tso = txq_ctrl->txq.offloads &
             (RTE_ETH_TX_OFFLOAD_TCP_TSO | RTE_ETH_TX_OFFLOAD_VXLAN_TNL_TSO |
              RTE_ETH_TX_OFFLOAD_GRE_TNL_TSO | RTE_ETH_TX_OFFLOAD_IP_TNL_TSO |
  ...
  if (config->txqs_inline == MLX5_ARG_UNSET)
    txqs_inline =
        ...
        (priv->pci_dev &&
         priv->pci_dev->id.device_id == PCI_DEVICE_ID_MELLANOX_CONNECTX5BF)
  ...
  inlen_mode = (config->txq_inline_min == MLX5_ARG_UNSET)
                   ? 0
                   : (unsigned int)config->txq_inline_min;
  ...
  if (inlen_mode) {
    if (inlen_mode <= MLX5_ESEG_MIN_INLINE_SIZE) {
      ...
      temp = RTE_ALIGN(temp, MLX5_WSEG_SIZE) + MLX5_ESEG_MIN_INLINE_SIZE;
  ...
  vlan_inline =
      (dev_txoff & RTE_ETH_TX_OFFLOAD_VLAN_INSERT) && !config->hw_vlan_insert;
  /*
  ...
  if (inlen_send && priv->txqs_n >= txqs_inline) {
     ...
     */
    temp = RTE_MAX(inlen_send, MLX5_ESEG_MIN_INLINE_SIZE + MLX5_WQE_DSEG_SIZE);
    temp -= MLX5_ESEG_MIN_INLINE_SIZE + MLX5_WQE_DSEG_SIZE;
    temp = RTE_ALIGN(temp, MLX5_WQE_SIZE);
    temp += MLX5_ESEG_MIN_INLINE_SIZE + MLX5_WQE_DSEG_SIZE;
    temp = RTE_MIN(temp, MLX5_WQE_SIZE_MAX + MLX5_ESEG_MIN_INLINE_SIZE -
    ...
    MLX5_ASSERT(inlen_send <= MLX5_WQE_SIZE_MAX + MLX5_ESEG_MIN_INLINE_SIZE -
                                  MLX5_WQE_CSEG_SIZE - MLX5_WQE_ESEG_SIZE -
                                  MLX5_WQE_DSEG_SIZE * 2);
  } else if (inlen_mode) {
    /*
  ...
  if (inlen_send && inlen_empw && priv->txqs_n >= txqs_inline) {
     ...
     * length accordingly to fit entire WQEBBs.
     */
    temp = RTE_MAX(inlen_empw, MLX5_WQE_SIZE + MLX5_DSEG_MIN_INLINE_SIZE);
    temp -= MLX5_DSEG_MIN_INLINE_SIZE;
    temp = RTE_ALIGN(temp, MLX5_WQE_SIZE);
    temp += MLX5_DSEG_MIN_INLINE_SIZE;
    temp = RTE_MIN(temp, MLX5_WQE_SIZE_MAX + MLX5_DSEG_MIN_INLINE_SIZE -
                             MLX5_WQE_CSEG_SIZE - MLX5_WQE_ESEG_SIZE -
                             MLX5_WQE_DSEG_SIZE);
    ...
    MLX5_ASSERT(inlen_empw <= MLX5_WQE_SIZE_MAX + MLX5_DSEG_MIN_INLINE_SIZE -
                                  MLX5_WQE_CSEG_SIZE - MLX5_WQE_ESEG_SIZE -
                                  MLX5_WQE_DSEG_SIZE);
  ...
  if (tso) {
    ...
    txq_ctrl->max_inline_data =
        RTE_MAX(txq_ctrl->max_inline_data, MLX5_MAX_TSO_HEADER);
  ...
  txq_ctrl->txq.swp_en =
      (((RTE_ETH_TX_OFFLOAD_IP_TNL_TSO | RTE_ETH_TX_OFFLOAD_UDP_TNL_TSO) &
        txq_ctrl->txq.offloads) &&
       (dev_cap->swp & MLX5_SW_PARSING_TSO_CAP)) |
      ((RTE_ETH_TX_OFFLOAD_OUTER_IPV4_CKSUM & txq_ctrl->txq.offloads) &&
       (dev_cap->swp & MLX5_SW_PARSING_CSUM_CAP));
}
 ...
 */
static int txq_adjust_params(struct mlx5_txq_ctrl *txq_ctrl) {
  struct mlx5_priv *priv = txq_ctrl->priv;
  struct mlx5_port_config *config = &priv->config;
  ...
  txq_ctrl->max_inline_data =
      RTE_MAX(txq_ctrl->txq.inlen_send, txq_ctrl->txq.inlen_empw);
...
struct mlx5_txq_ctrl *mlx5_txq_new(struct rte_eth_dev *dev, uint16_t idx,
  ...
  tmpl =
      mlx5_malloc(MLX5_MEM_RTE | MLX5_MEM_ZERO,
                  sizeof(*tmpl) + desc * sizeof(struct rte_mbuf *), 0, socket);
  if (!tmpl) {
    rte_errno = ENOMEM;
    return NULL;
  }
  if (mlx5_mr_ctrl_init(&tmpl->txq.mr_ctrl, &priv->sh->cdev->mr_scache.dev_gen,
                        socket)) {
    /* rte_errno is already set. */
    goto error;
  }
  MLX5_ASSERT(desc > MLX5_TX_COMP_THRESH);
  tmpl->txq.offloads = conf->offloads | dev->data->dev_conf.txmode.offloads;
  tmpl->priv = priv;
  tmpl->socket = socket;
  ...
  if (txq_adjust_params(tmpl))
    goto error;
  if (txq_calc_wqebb_cnt(tmpl) > priv->sh->dev_cap.max_qp_wr) {
    DRV_LOG(ERR,
...
int mlx5_txq_verify(struct rte_eth_dev *dev) {
  ...
  LIST_FOREACH(txq_ctrl, &priv->txqsctrl, next) {
    DRV_LOG(DEBUG, "port %u Tx queue %u still referenced", dev->data->port_id,
            txq_ctrl->txq.idx);
    ++ret;
  }
  return ret;
...
void mlx5_txq_dynf_timestamp_set(struct rte_eth_dev *dev) {
  ...
  if (sh->dev_cap.rt_timestamp || !sh->cdev->config.hca_attr.dev_freq_khz)
  ...
  off = rte_mbuf_dynfield_lookup(RTE_MBUF_DYNFIELD_TIMESTAMP_NAME, NULL);
  if (nbit >= 0 && off >= 0 &&
  ...
  for (i = 0; i != priv->txqs_n; ++i) {
    ...
    data->rt_timemask =
        (data->offloads & RTE_ETH_TX_OFFLOAD_SEND_ON_TIMESTAMP) ? ts_mask : 0;
  }
}
