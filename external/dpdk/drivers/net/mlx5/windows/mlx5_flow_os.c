...
int mlx5_flow_os_create_flow_matcher(void *ctx, void *attr, void *table,
  ...
  *matcher = NULL;
  ...
  struct mlx5_matcher *mlx5_matcher = mlx5_malloc(
      MLX5_MEM_ZERO,
      sizeof(struct mlx5_matcher) + MLX5_ST_SZ_BYTES(fte_match_param), 0,
      SOCKET_ID_ANY);
  if (!mlx5_matcher) {
    rte_errno = ENOMEM;
    return -rte_errno;
  ...
  memcpy(&mlx5_matcher->match_buf, mattr->match_mask->match_buf,
...
int mlx5_flow_os_create_flow_action_dest_devx_tir(struct mlx5_devx_obj *tir,
  ...
  struct mlx5_action *mlx5_action =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(struct mlx5_action), 0, SOCKET_ID_ANY);
...
int mlx5_flow_os_create_flow(void *matcher, void *match_value,
  ...
  void *matcher_c = MLX5_ADDR_OF(devx_fs_rule_add_in, in, match_criteria);
  void *matcher_v = MLX5_ADDR_OF(devx_fs_rule_add_in, in, match_value);

  MLX5_ASSERT(mlx5_matcher->ctx);
  memcpy(matcher_c, mlx5_matcher->match_buf, mlx5_match_value->size);
  /* Use mlx5_match_value->size for match criteria */
  memcpy(matcher_v, mlx5_match_value->buf, mlx5_match_value->size);
  for (i = 0; i < num_actions; i++) {
    action = actions[i];
    switch (action->type) {
    case MLX5_FLOW_CONTEXT_DEST_TYPE_TIR:
      MLX5_SET(devx_fs_rule_add_in, in, dest.destination_type,
               MLX5_FLOW_CONTEXT_DEST_TYPE_TIR);
      MLX5_SET(devx_fs_rule_add_in, in, dest.destination_id,
               action->dest_tir.id);
      break;
    default:
      break;
...
static bool mlx5_is_thread_alive(HANDLE thread_handle) {
...
static int mlx5_get_current_thread(HANDLE *p_handle) {
  BOOL ret = DuplicateHandle(GetCurrentProcess(), GetCurrentThread(),
                             GetCurrentProcess(), p_handle, 0, 0,
...
static void mlx5_clear_thread_list(void) {
...
static int mlx5_add_workspace_to_list(struct mlx5_flow_workspace *data) {
  HANDLE curr_thread;
