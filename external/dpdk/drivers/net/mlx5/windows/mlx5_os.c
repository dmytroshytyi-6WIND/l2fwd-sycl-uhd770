...
static int mlx5_init_shared_data(void) {
  ...
  if (mlx5_shared_data == NULL) {
    ...
    mz = rte_memzone_reserve(MZ_MLX5_PMD_SHARED_DATA, sizeof(*mlx5_shared_data),
                             SOCKET_ID_ANY, 0);
    if (mz == NULL) {
...
static struct rte_eth_dev *mlx5_dev_spawn(struct rte_device *dpdk_dev,
  ...
  if (err) {
    DRV_LOG(ERR, "failed to init showdown event: %s", strerror(errno));
    goto error;
  }
...
int mlx5_os_read_dev_stat(struct mlx5_priv *priv, const char *ctr_name,
  ...
  if (priv->q_counters != NULL && strcmp(ctr_name, "out_of_buffer") == 0)
    return mlx5_devx_cmd_queue_counter_query(priv->q_counters, 0,
                                             (uint32_t *)stat);
  DRV_LOG(WARNING, "%s: is not supported for the %s counter", __func__,
          ctr_name);
  return -ENOTSUP;
}
...
int mlx5_os_mac_addr_add(struct rte_eth_dev *dev, struct rte_ether_addr *mac,
  ...
  if (!rte_is_same_ether_addr(&lmac, mac)) {
    DRV_LOG(ERR, "adding new mac address to device is unsupported");
    return -ENOTSUP;
  }
  return 0;
}
