 ...
 */
static int rxq_alloc_elts_mprq(struct mlx5_rxq_ctrl *rxq_ctrl) {
  ...
  DRV_LOG(DEBUG, "port %u MPRQ queue %u allocated and configured %u segments",
...
error:
  ...
  for (i = 0; (i != wqe_n); ++i) {
    if ((*rxq->mprq_bufs)[i] != NULL)
      rte_mempool_put(rxq->mprq_mp, (*rxq->mprq_bufs)[i]);
    (*rxq->mprq_bufs)[i] = NULL;
  }
  DRV_LOG(DEBUG, "port %u MPRQ queue %u failed, freed everything", rxq->port_id,
          rxq->idx);
 ...
 */
static int rxq_alloc_elts_sprq(struct mlx5_rxq_ctrl *rxq_ctrl) {
  ...
  for (i = 0; (i != elts_n); ++i) {
    ...
    if (buf == NULL) {
      if (rxq_ctrl->share_group == 0)
        DRV_LOG(ERR, "port %u queue %u empty mbuf pool", RXQ_PORT_ID(rxq_ctrl),
                rxq_ctrl->rxq.idx);
      else
    ...
    MLX5_ASSERT(!has_vec_support || DATA_OFF(buf) >= RTE_PKTMBUF_HEADROOM);
  ...
  if (has_vec_support) {
    ...
    struct rte_pktmbuf_pool_private *priv =
        (struct rte_pktmbuf_pool_private *)rte_mempool_get_priv(
            rxq_ctrl->rxq.mp);
    int j;
    ...
    rxq->mbuf_initializer = *(rte_xmm_t *)&mbuf_init->rearm_data;
  ...
  if (rxq_ctrl->share_group == 0)
    DRV_LOG(DEBUG,
            "port %u SPRQ queue %u allocated and configured %u segments (max "
            "%u packets)",
            RXQ_PORT_ID(rxq_ctrl), rxq_ctrl->rxq.idx, elts_n,
            elts_n / (1 << rxq_ctrl->rxq.sges_n));
  else
    DRV_LOG(DEBUG,
            "share group %u SPRQ queue %u allocated and configured %u segments "
            "(max %u packets)",
 ...
 *   Pointer to RX queue structure.
 */
static void rxq_free_elts_mprq(struct mlx5_rxq_ctrl *rxq_ctrl) {
  struct mlx5_rxq_data *rxq = &rxq_ctrl->rxq;
  uint16_t i;
 ...
 *   Pointer to RX queue structure.
 */
static void rxq_free_elts_sprq(struct mlx5_rxq_ctrl *rxq_ctrl) {
  struct mlx5_rxq_data *rxq = &rxq_ctrl->rxq;
  const uint16_t q_n =
      mlx5_rxq_mprq_enabled(&rxq_ctrl->rxq)
          ? RTE_BIT32(rxq->elts_n) * RTE_BIT32(rxq->log_strd_num)
          : RTE_BIT32(rxq->elts_n);
  const uint16_t q_mask = q_n - 1;
  uint16_t elts_ci =
      mlx5_rxq_mprq_enabled(&rxq_ctrl->rxq) ? rxq->elts_ci : rxq->rq_ci;
  uint16_t used = q_n - (elts_ci - rxq->rq_pi);
  uint16_t i;

  if (rxq_ctrl->share_group == 0)
    DRV_LOG(DEBUG, "port %u Rx queue %u freeing %d WRs", RXQ_PORT_ID(rxq_ctrl),
            rxq->idx, q_n);
  else
 ...
 *   Pointer to RX queue structure.
 */
static void rxq_free_elts(struct mlx5_rxq_ctrl *rxq_ctrl) {
  /*
   * For MPRQ we need to allocate both MPRQ buffers
   * for WQEs and simple mbufs for vector processing.
   */
...
uint64_t mlx5_get_rx_queue_offloads(struct rte_eth_dev *dev) {
  ...
  uint64_t offloads =
      (RTE_ETH_RX_OFFLOAD_SCATTER | RTE_ETH_RX_OFFLOAD_TIMESTAMP |
 ...
 */
static int mlx5_rxq_releasable(struct rte_eth_dev *dev, uint16_t idx) {
...
static void rxq_sync_cq(struct mlx5_rxq_data *rxq) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_rx_queue_stop_primary(struct rte_eth_dev *dev, uint16_t idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_rxq_priv *rxq = mlx5_rxq_get(dev, idx);
  ...
  if (ret) {
    DRV_LOG(ERR, "Cannot change Rx WQ state to RESET:  %s", strerror(errno));
    rte_errno = errno;
    return ret;
  }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_rx_queue_stop(struct rte_eth_dev *dev, uint16_t idx) {
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ret = mlx5_mp_os_req_queue_control(dev, idx, MLX5_MP_REQ_QUEUE_RX_STOP);
  } else {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_rx_queue_start_primary(struct rte_eth_dev *dev, uint16_t idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_rxq_priv *rxq = mlx5_rxq_get(dev, idx);
  ...
  *rxq_data->rq_db = rte_cpu_to_be_32(0);
  ...
  if (ret) {
    DRV_LOG(ERR, "Cannot change Rx WQ state to READY:  %s", strerror(errno));
    rte_errno = errno;
    return ret;
  }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_rx_queue_start(struct rte_eth_dev *dev, uint16_t idx) {
  int ret;
  ...
  if (dev->data->rx_queue_state[idx] == RTE_ETH_QUEUE_STATE_STARTED)
    return 0;
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ret = mlx5_mp_os_req_queue_control(dev, idx, MLX5_MP_REQ_QUEUE_RX_START);
  } else {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rx_queue_pre_setup(struct rte_eth_dev *dev, uint16_t idx,
                                   uint16_t *desc,
 ...
 */
static struct mlx5_rxq_ctrl *mlx5_shared_rxq_get(struct rte_eth_dev *dev,
                                                 uint32_t group,
                                                 uint16_t share_qid) {
  struct mlx5_rxq_ctrl *rxq_ctrl;
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  LIST_FOREACH(rxq_ctrl, &priv->sh->shared_rxqs, share_entry) {
    if (rxq_ctrl->share_group == group && rxq_ctrl->share_qid == share_qid)
      return rxq_ctrl;
  }
  return NULL;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static bool mlx5_shared_rxq_match(struct mlx5_rxq_ctrl *rxq_ctrl,
  ...
  if (rxq_ctrl->socket != socket) {
    DRV_LOG(
        ERR,
        "port %u queue index %u failed to join shared group: socket mismatch",
        dev->data->port_id, idx);
    return false;
  }
  if (rxq_ctrl->rxq.elts_n != log2above(desc)) {
    DRV_LOG(ERR,
            "port %u queue index %u failed to join shared group: descriptor "
            "number mismatch",
            dev->data->port_id, idx);
    return false;
  }
  if (priv->mtu != spriv->mtu) {
    DRV_LOG(ERR,
            "port %u queue index %u failed to join shared group: mtu mismatch",
            dev->data->port_id, idx);
    return false;
  }
  if (priv->dev_data->dev_conf.intr_conf.rxq !=
    ...
    DRV_LOG(ERR,
            "port %u queue index %u failed to join shared group: interrupt "
            "mismatch",
            dev->data->port_id, idx);
    return false;
  }
  if (mp != NULL && rxq_ctrl->rxq.mp != mp) {
    DRV_LOG(
        ERR,
        "port %u queue index %u failed to join shared group: mempool mismatch",
        dev->data->port_id, idx);
    return false;
  } else if (mp == NULL) {
    if (conf->rx_nseg != rxq_ctrl->rxseg_n) {
      DRV_LOG(ERR,
              "port %u queue index %u failed to join shared group: segment "
              "number mismatch",
              dev->data->port_id, idx);
      return false;
    }
  ...
  if (priv->config.hw_padding != spriv->config.hw_padding) {
    DRV_LOG(
        ERR,
        "port %u queue index %u failed to join shared group: padding mismatch",
        dev->data->port_id, idx);
    return false;
  }
  if (priv->config.cqe_comp != spriv->config.cqe_comp ||
    ...
    DRV_LOG(ERR,
            "port %u queue index %u failed to join shared group: CQE "
...
int mlx5_rx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc,
  ...
  if (conf->share_group > 0) {
    ...
    if (conf->share_qid >= priv->rxqs_n) {
      DRV_LOG(ERR, "port %u shared Rx queue index %u > number of Rx queues %u",
              dev->data->port_id, conf->share_qid, priv->rxqs_n);
      rte_errno = EINVAL;
      return -rte_errno;
    }
    if (priv->config.mprq.enabled) {
      DRV_LOG(
          ERR,
          "port %u shared Rx queue index %u: not supported when MPRQ enabled",
          dev->data->port_id, conf->share_qid);
      rte_errno = EINVAL;
      return -rte_errno;
    }
    /* Try to reuse shared RXQ. */
    rxq_ctrl = mlx5_shared_rxq_get(dev, conf->share_group, conf->share_qid);
    if (rxq_ctrl != NULL &&
        !mlx5_shared_rxq_match(rxq_ctrl, dev, idx, desc, socket, conf, mp)) {
      rte_errno = EINVAL;
      return -rte_errno;
    }
  }
  ...
  /* Allocate RXQ. */
  rxq =
      mlx5_malloc(MLX5_MEM_RTE | MLX5_MEM_ZERO, sizeof(*rxq), 0, SOCKET_ID_ANY);
  if (!rxq) {
  ...
  DRV_LOG(DEBUG, "port %u adding Rx queue %u to list", dev->data->port_id, idx);
...
int mlx5_rx_hairpin_queue_setup(
  ...
  if (hairpin_conf->peer_count != 1) {
    rte_errno = EINVAL;
    DRV_LOG(ERR,
            "port %u unable to setup Rx hairpin queue index %u"
            " peer count is %u",
            dev->data->port_id, idx, hairpin_conf->peer_count);
    return -rte_errno;
  }
  if (hairpin_conf->peers[0].port == dev->data->port_id) {
    if (hairpin_conf->peers[0].queue >= priv->txqs_n) {
      rte_errno = EINVAL;
      DRV_LOG(ERR,
              "port %u unable to setup Rx hairpin queue"
              " index %u, Tx %u is larger than %u",
              dev->data->port_id, idx, hairpin_conf->peers[0].queue,
              priv->txqs_n);
      return -rte_errno;
    }
  } else {
    if (hairpin_conf->manual_bind == 0 || hairpin_conf->tx_explicit == 0) {
      rte_errno = EINVAL;
      DRV_LOG(ERR,
              "port %u unable to setup Rx hairpin queue"
              " index %u peer port %u with attributes %u %u",
              dev->data->port_id, idx, hairpin_conf->peers[0].port,
              hairpin_conf->manual_bind, hairpin_conf->tx_explicit);
      return -rte_errno;
    }
  }
  rxq =
  ...
  if (!rxq) {
    DRV_LOG(ERR,
            "port %u unable to allocate hairpin rx queue index %u private data",
            dev->data->port_id, idx);
    rte_errno = ENOMEM;
    return -rte_errno;
  }
...
int mlx5_rx_intr_vec_enable(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i != n; ++i) {
    ...
    if (!rxq_obj || (!rxq_obj->ibv_channel && !rxq_obj->devx_channel)) {
 ...
 */
static inline void mlx5_arm_cq(struct mlx5_rxq_data *rxq, int sq_n_rxq) {
  int sq_n = 0;
  ...
  doorbell |= rxq->cqn;
  mlx5_doorbell_ring(&rxq->uar_data, rte_cpu_to_be_64(doorbell), doorbell_hi,
                     &rxq->cq_db[MLX5_CQ_ARM_DB], 0);
}
...
int mlx5_rxq_obj_verify(struct rte_eth_dev *dev) {
  ...
  LIST_FOREACH(rxq_obj, &priv->rxqsobj, next) {
    ...
    if (rxq_obj->rxq_ctrl->rxq.shared &&
      ...
      continue;
    DRV_LOG(DEBUG, "port %u Rx queue %u still referenced", dev->data->port_id,
            rxq_obj->rxq_ctrl->rxq.idx);
    ++ret;
  }
  return ret;
}
 ...
 */
static inline void mlx5_mprq_buf_init(struct rte_mempool *mp, void *opaque_arg,
...
int mlx5_mprq_free_mp(struct rte_eth_dev *dev) {
  ...
  if (!rte_mempool_full(mp)) {
    DRV_LOG(ERR, "port %u mempool for Multi-Packet RQ is still in use",
            dev->data->port_id);
    rte_errno = EBUSY;
    return -rte_errno;
  }
...
int mlx5_mprq_alloc_mp(struct rte_eth_dev *dev) {
  ...
  obj_size = sizeof(struct mlx5_mprq_buf) + buf_len +
             RTE_BIT32(log_strd_num) * sizeof(struct rte_mbuf_ext_shared_info) +
  ...
  if (mp != NULL && mp->elt_size >= obj_size && mp->size >= obj_num) {
    DRV_LOG(DEBUG, "port %u mempool %s is being reused", dev->data->port_id,
            mp->name);
    /* Reuse. */
    goto exit;
  } else if (mp != NULL) {
  ...
  mp = rte_mempool_create(name, obj_num, obj_size, MLX5_MPRQ_MP_CACHE_SZ, 0,
...
exit:
  ...
  DRV_LOG(INFO, "port %u Multi-Packet RQ is configured", dev->data->port_id);
  return 0;
}

#define MLX5_MAX_TCP_HDR_OFFSET                                                \
  ((unsigned int)(sizeof(struct rte_ether_hdr) +                               \
                  sizeof(struct rte_vlan_hdr) * 2 +                            \
                  sizeof(struct rte_ipv6_hdr)))
#define MAX_TCP_OPTION_SIZE 40u
#define MLX5_MAX_LRO_HEADER_FIX                                                \
  ((unsigned int)(MLX5_MAX_TCP_HDR_OFFSET + sizeof(struct rte_tcp_hdr) +       \
                  MAX_TCP_OPTION_SIZE))
 ...
 */
static void mlx5_max_lro_msg_size_adjust(struct rte_eth_dev *dev, uint16_t idx,
                                         uint32_t max_lro_size) {
  struct mlx5_priv *priv = dev->data->dev_private;

  if (priv->sh->cdev->config.hca_attr.lro_max_msg_sz_mode ==
          MLX5_LRO_MAX_MSG_SIZE_START_FROM_L4 &&
 ...
 */
static int mlx5_mprq_prepare(struct rte_eth_dev *dev, uint16_t idx,
                             uint16_t desc, bool rx_seg_en,
                             uint32_t min_mbuf_size,
  ...
  uint32_t log_def_stride_num =
      RTE_MIN(RTE_MAX(MLX5_MPRQ_DEFAULT_LOG_STRIDE_NUM, log_min_stride_num),
              log_max_stride_num);
  ...
  uint32_t log_def_stride_size =
      RTE_MIN(RTE_MAX(MLX5_MPRQ_DEFAULT_LOG_STRIDE_SIZE, log_min_stride_size),
              log_max_stride_size);
  ...
  if (config->mprq.log_stride_num > log_max_stride_num ||
    ...
    *actual_log_stride_num = log_def_stride_num;
    DRV_LOG(WARNING,
            "Port %u Rx queue %u number of strides for Multi-Packet RQ is out "
            "of range, setting default value (%u)",
  ...
  if (config->mprq.log_stride_size) {
    ...
    if (config->mprq.log_stride_size > log_max_stride_size ||
      ...
      *actual_log_stride_size = log_def_stride_size;
      DRV_LOG(WARNING,
              "Port %u Rx queue %u size of a stride for Multi-Packet RQ is out "
              "of range, setting default value (%u)",
              dev->data->port_id, idx, RTE_BIT32(log_def_stride_size));
    } else {
  ...
  if (log_stride_wqe_size < dev_cap->mprq.log_min_stride_wqe_size) {
    ...
    *actual_log_stride_size = log_def_stride_size;
    DRV_LOG(WARNING,
            "Port %u Rx queue %u size of WQE buffer for Multi-Packet RQ is too "
            "small, setting default values (stride_num_n=%u, stride_size_n=%u)",
            dev->data->port_id, idx, RTE_BIT32(log_def_stride_num),
            RTE_BIT32(log_def_stride_size));
    log_stride_wqe_size = log_def_stride_num + log_def_stride_size;
  }
  MLX5_ASSERT(log_stride_wqe_size >= dev_cap->mprq.log_min_stride_wqe_size);
...
unsupport:
  if (config->mprq.enabled)
    DRV_LOG(WARNING,
            ...
            RTE_BIT32(config->mprq.log_stride_num), config->mprq.min_rxqs_num,
...
struct mlx5_rxq_ctrl *mlx5_rxq_new(struct rte_eth_dev *dev, uint16_t idx,
  ...
  unsigned int max_rx_pktlen =
      lro_on_queue ? dev->data->dev_conf.rxmode.max_lro_pkt_size
                   : dev->data->mtu + (unsigned int)RTE_ETHER_HDR_LEN +
                         RTE_ETHER_CRC_LEN;
  unsigned int non_scatter_min_mbuf_size = max_rx_pktlen + RTE_PKTMBUF_HEADROOM;
  ...
  const int mprq_en = !mlx5_mprq_prepare(
      dev, idx, desc, rx_seg_en, non_scatter_min_mbuf_size,
      &mprq_log_actual_stride_num, &mprq_log_actual_stride_size);
  /*
   ...
   */
  tmpl->rxseg_n = n_seg;
  rte_memcpy(tmpl->rxseg, qs_seg, sizeof(struct rte_eth_rxseg_split) * n_seg);
  /*
  ...
  do {
    struct mlx5_eth_rxseg *hw_seg = &tmpl->rxq.rxseg[tmpl->rxq.rxseg_n];
    ...
    seg_len = tmpl->rxq.rxseg_n >= n_seg ? buf_len
              : qs_seg->length           ? qs_seg->length
                                         : (buf_len - offset);
    /* Check is done in long int, now overflows. */
    if (buf_len < seg_len + offset) {
      DRV_LOG(ERR,
              "port %u Rx queue %u: Split offset/length "
              "%u/%u can't be satisfied",
              dev->data->port_id, idx, qs_seg->length, qs_seg->offset);
      rte_errno = EINVAL;
      goto error;
    }
    if (seg_len > tail_len)
    ...
    if (++tmpl->rxq.rxseg_n > MLX5_MAX_RXQ_NSEG) {
      DRV_LOG(ERR,
              ...
              " supported are %u",
              dev->data->port_id, tmpl->rxq.rxseg_n, max_rx_pktlen,
              MLX5_MAX_RXQ_NSEG);
      rte_errno = ENOTSUP;
      goto error;
    }
  ...
  MLX5_ASSERT(tmpl->rxq.rxseg_n && tmpl->rxq.rxseg_n <= MLX5_MAX_RXQ_NSEG);
  ...
  if (mlx5_mr_ctrl_init(&tmpl->rxq.mr_ctrl, &priv->sh->cdev->mr_scache.dev_gen,
                        socket)) {
  ...
  if (mprq_en) {
    ...
    tmpl->rxq.strd_scatter_en = !!(offloads & RTE_ETH_RX_OFFLOAD_SCATTER);
    tmpl->rxq.mprq_max_memcpy_len =
        RTE_MIN(first_mb_free_size, config->mprq.max_memcpy_len);
    max_lro_size = RTE_MIN(max_rx_pktlen, RTE_BIT32(tmpl->rxq.log_strd_num) *
    ...
    if (lro_on_queue && first_mb_free_size < MLX5_MAX_LRO_HEADER_FIX) {
      DRV_LOG(ERR,
    ...
    if (sges_n > MLX5_MAX_LOG_RQ_SEGS) {
      DRV_LOG(ERR,
              ...
              " requested maximum packet size %u, the maximum"
              " supported are %u",
              dev->data->port_id, 1 << sges_n, max_rx_pktlen,
  ...
  if (desc % (1 << tmpl->rxq.sges_n)) {
    DRV_LOG(ERR,
            ...
            dev->data->port_id, desc, 1 << tmpl->rxq.sges_n);
    rte_errno = EINVAL;
    goto error;
  }
  ...
  if (tmpl->rxq.hw_timestamp &&
      rte_mbuf_dyn_rx_timestamp_register(&tmpl->rxq.timestamp_offset,
                                         &tmpl->rxq.timestamp_rx_flag) != 0) {
  ...
  DRV_LOG(DEBUG,
          ...
          dev->data->port_id, tmpl->rxq.crc_present ? "disabled" : "enabled",
          tmpl->rxq.crc_present << 2);
  tmpl->rxq.rss_hash =
      !!priv->rss_conf.rss_hf &&
 ...
 */
static void mlx5_rxqs_deref(struct rte_eth_dev *dev, uint16_t *queues,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rxqs_ref(struct rte_eth_dev *dev, uint16_t *queues,
                         const uint32_t queues_n) {
  uint32_t i;
...
struct mlx5_ind_table_obj *mlx5_ind_table_obj_get(struct rte_eth_dev *dev,
  ...
  LIST_FOREACH(ind_tbl, &priv->ind_tbls, next) {
    if ((ind_tbl->queues_n == queues_n) &&
        (memcmp(ind_tbl->queues, queues,
                ind_tbl->queues_n * sizeof(ind_tbl->queues[0])) == 0)) {
...
int mlx5_ind_table_obj_verify(struct rte_eth_dev *dev) {
  ...
  rte_rwlock_read_lock(&priv->ind_tbls_lock);
  LIST_FOREACH(ind_tbl, &priv->ind_tbls, next) {
    DRV_LOG(DEBUG, "port %u indirection table obj %p still referenced",
...
int mlx5_ind_table_obj_setup(struct rte_eth_dev *dev,
                             struct mlx5_ind_table_obj *ind_tbl, bool ref_qs) {
  struct mlx5_priv *priv = dev->data->dev_private;
...
mlx5_ind_table_obj_new(struct rte_eth_dev *dev, const uint16_t *queues,
  ...
  ind_tbl =
      mlx5_malloc(MLX5_MEM_ZERO,
                  sizeof(*ind_tbl) +
                      (standalone ? priv->rxqs_n : queues_n) * sizeof(uint16_t),
                  0, SOCKET_ID_ANY);
  if (!ind_tbl) {
    rte_errno = ENOMEM;
    return NULL;
...
mlx5_ind_table_obj_check_standalone(struct rte_eth_dev *dev __rte_unused,
   ...
   */
  DRV_LOG(DEBUG, "Port %u cannot modify indirection table %p (refcnt %u > 1).",
...
int mlx5_ind_table_obj_modify(struct rte_eth_dev *dev,
                              ...
                              bool standalone, bool ref_new_qs,
                              bool deref_old_qs) {
  struct mlx5_priv *priv = dev->data->dev_private;
  int ret;
  const unsigned int n = rte_is_power_of_2(queues_n)
...
int mlx5_ind_table_obj_attach(struct rte_eth_dev *dev,
  ...
  ret = mlx5_ind_table_obj_modify(
      dev, ind_tbl, ind_tbl->queues, ind_tbl->queues_n, true /* standalone */,
      true /* ref_new_qs */, false /* deref_old_qs */);
  if (ret != 0)
...
int mlx5_hrxq_match_cb(void *tool_ctx __rte_unused,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_hrxq_modify(struct rte_eth_dev *dev, uint32_t hrxq_idx,
                     const uint8_t *rss_key, uint32_t rss_key_len,
                     uint64_t hash_fields, const uint16_t *queues,
                     uint32_t queues_n) {
  int err;
  ...
  if (mlx5_ind_table_obj_match_queues(hrxq->ind_table, queues, queues_n)) {
    ...
    if (!ind_tbl)
      ind_tbl = mlx5_ind_table_obj_new(dev, queues, queues_n, hrxq->standalone,
                                       dev_started);
  }
  if (!ind_tbl) {
    rte_errno = ENOMEM;
  ...
  ret = priv->obj_ops.hrxq_modify(dev, hrxq, rss_key, hash_fields, ind_tbl);
  if (ret) {
    rte_errno = errno;
    goto error;
  }
...
static void __mlx5_hrxq_remove(struct rte_eth_dev *dev,
                               struct mlx5_hrxq *hrxq) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (!hrxq->standalone) {
    mlx5_ind_table_obj_release(dev, hrxq->ind_table,
                               hrxq->hws_flags ? (!!dev->data->dev_started)
                                               : true);
  }
...
__mlx5_hrxq_create(struct rte_eth_dev *dev,
  ...
  const uint16_t *queues = standalone ? rss_desc->const_q : rss_desc->queue;
...
void mlx5_hrxq_clone_free_cb(void *tool_ctx, struct mlx5_list_entry *entry) {
  struct rte_eth_dev *dev = tool_ctx;
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_hrxq *hrxq = container_of(entry, typeof(*hrxq), entry);
...
struct mlx5_hrxq *mlx5_drop_action_create(struct rte_eth_dev *dev) {
  ...
  if (!hrxq) {
    DRV_LOG(WARNING, "Port %u cannot allocate memory for drop queue.",
            dev->data->port_id);
    rte_errno = ENOMEM;
    goto error;
  }
  ...
  hrxq->ind_table =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*hrxq->ind_table), 0, SOCKET_ID_ANY);
  if (!hrxq->ind_table) {
    rte_errno = ENOMEM;
    goto error;
  }
...
mlx5_external_rx_queue_get_validate(uint16_t port_id, uint16_t dpdk_idx) {
  ...
  if (dpdk_idx < MLX5_EXTERNAL_RX_QUEUE_ID_MIN) {
    DRV_LOG(ERR, "Queue index %u should be in range: [%u, %u].", dpdk_idx,
            MLX5_EXTERNAL_RX_QUEUE_ID_MIN, UINT16_MAX);
    rte_errno = EINVAL;
    return NULL;
  }
  if (rte_eth_dev_is_valid_port(port_id) < 0) {
    DRV_LOG(ERR, "There is no Ethernet device for port %u.", port_id);
    rte_errno = ENODEV;
    return NULL;
  }
  ...
  if (!mlx5_devx_obj_ops_en(priv->sh)) {
    DRV_LOG(ERR, "Port %u external RxQ isn't supported by Verbs API.", port_id);
    rte_errno = ENOTSUP;
    return NULL;
  }
  /*
...
int rte_pmd_mlx5_external_rx_queue_id_unmap(uint16_t port_id,
  ...
  if (ext_rxq == NULL)
    return -rte_errno;
  if (ext_rxq->refcnt > 1) {
    DRV_LOG(ERR, "Port %u external RxQ index %u still referenced.", port_id,
            dpdk_idx);
    rte_errno = EINVAL;
    return -rte_errno;
  }
  if (!__atomic_compare_exchange_n(&ext_rxq->refcnt, &mapped, 0, false,
                                   __ATOMIC_RELAXED, __ATOMIC_RELAXED)) {
    DRV_LOG(ERR, "Port %u external RxQ index %u doesn't exist.", port_id,
            dpdk_idx);
    rte_errno = EINVAL;
    return -rte_errno;
  }
  DRV_LOG(DEBUG, "Port %u external RxQ index %u is successfully unmapped.",
          port_id, dpdk_idx);
  return 0;
}
