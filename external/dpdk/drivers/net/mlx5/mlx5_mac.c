 ...
 *   MAC address index.
 */
static void mlx5_internal_mac_addr_remove(struct rte_eth_dev *dev,
                                          uint32_t index) {
 ...
 */
static int mlx5_internal_mac_addr_add(struct rte_eth_dev *dev,
...
void mlx5_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
  ...
  if (!dev->data->promiscuous) {
    ...
    if (ret)
      DRV_LOG(ERR, "port %u cannot restart traffic: %s", dev->data->port_id,
              strerror(rte_errno));
  }
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_mac_addr_set(struct rte_eth_dev *dev,
  ...
  if (priv->representor && !mlx5_is_hpf(dev) && !mlx5_is_sf_repr(dev)) {
    ...
    if (priv->pf_bond >= 0) {
      ...
      return mlx5_os_vf_mac_addr_modify(
          priv, priv->sh->bond.ports[priv->pf_bond].ifindex, mac_addr,
  ...
  DRV_LOG(DEBUG, "port %u setting primary MAC address", dev->data->port_id);
