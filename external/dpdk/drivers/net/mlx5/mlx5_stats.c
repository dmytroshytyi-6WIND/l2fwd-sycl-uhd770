  ...
  if (n >= mlx5_stats_n && stats) {
    ...
    for (i = 0; i != mlx5_stats_n; ++i) {
      ...
      if (xstats_ctrl->info[i].dev) {
        ...
        stats[i].value =
            (counters[i] - xstats_ctrl->base[i]) & (uint64_t)UINT32_MAX;
        wrap_n = hw_stat >> 32;
        if (stats[i].value < (hw_stat & (uint64_t)UINT32_MAX))
          wrap_n++;
...
int mlx5_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  if (ret == 0) {
    tmp.imissed =
        (tmp.imissed - stats_ctrl->imissed_base) & (uint64_t)UINT32_MAX;
...
int mlx5_stats_reset(struct rte_eth_dev *dev) {
  ...
  for (i = 0; (i != priv->txqs_n); ++i) {
    if ((*priv->txqs)[i] == NULL)
      continue;
    memset(&(*priv->txqs)[i]->stats, 0, sizeof(struct mlx5_txq_stats));
  }
...
int mlx5_xstats_reset(struct rte_eth_dev *dev) {
  ...
  if (xstats_ctrl->stats_n != stats_n)
    mlx5_os_stats_init(dev);
  counters =
      mlx5_malloc(MLX5_MEM_SYS, sizeof(*counters) * xstats_ctrl->mlx5_stats_n,
                  0, SOCKET_ID_ANY);
  if (!counters) {
    DRV_LOG(WARNING,
...
int mlx5_xstats_get_names(struct rte_eth_dev *dev,
  ...
  if (n >= mlx5_xstats_n && xstats_names) {
    for (i = 0; i != mlx5_xstats_n; ++i) {
      strncpy(xstats_names[i].name, xstats_ctrl->info[i].dpdk_name,
              RTE_ETH_XSTATS_NAME_SIZE);
      xstats_names[i].name[RTE_ETH_XSTATS_NAME_SIZE - 1] = 0;
    }
  }
  mlx5_xstats_n =
      mlx5_txpp_xstats_get_names(dev, xstats_names, n, mlx5_xstats_n);
