...
struct mlx5_indexed_pool_config {
  ...
  void *(*malloc)(uint32_t flags, size_t size, unsigned int align, int socket);
...
static inline void *mlx5_l3t_get_next(struct mlx5_l3t_tbl *tbl, uint32_t *pos) {
      ...
      case MLX5_L3T_TYPE_WORD:
        ...
        for (k = e_start; k < MLX5_L3T_ET_SIZE; k++) {
          if (!w_e_tbl->entry[k].data)
            continue;
          *pos = (i << MLX5_L3T_GT_OFFSET) | (j << MLX5_L3T_MT_OFFSET) | k;
...
#define ILIST_REMOVE(pool, head, idx, elem, field)                             \
  do {                                                                         \
    ...
    MLX5_ASSERT(head);                                                         \
    if ((elem)->field.prev) {                                                  \
      (peer) = mlx5_ipool_get(pool, (elem)->field.prev);                       \
      if (peer)                                                                \
        (peer)->field.next = (elem)->field.next;                               \
    }                                                                          \
    if ((elem)->field.next) {                                                  \
      (peer) = mlx5_ipool_get(pool, (elem)->field.next);                       \
      if (peer)                                                                \
...
#define ILIST_FOREACH(pool, head, idx, elem, field)                            \
  for ((idx) = (head), (elem) = (idx) ? mlx5_ipool_get(pool, (idx)) : NULL;    \
       (elem); idx = (elem)->field.next,                                       \
      (elem) = (idx) ? mlx5_ipool_get(pool, idx) : NULL)
...
#define MLX5_L3T_FOREACH(tbl, idx, entry)                                      \
  for (idx = 0, (entry) = mlx5_l3t_get_next((tbl), &idx); (entry);             \
