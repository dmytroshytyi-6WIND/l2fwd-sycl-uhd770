 ...
 *   MR to free.
 */
static void mlx5_aso_dereg_mr(struct mlx5_common_device *cdev,
 ...
 */
static int mlx5_aso_reg_mr(struct mlx5_common_device *cdev, size_t length,
                           struct mlx5_pmd_mr *mr) {
  int ret;

  mr->addr =
      mlx5_malloc(MLX5_MEM_RTE | MLX5_MEM_ZERO, length, 4096, SOCKET_ID_ANY);
  if (!mr->addr) {
 ...
 *   ASO SQ to destroy.
 */
static void mlx5_aso_destroy_sq(struct mlx5_aso_sq *sq) {
 ...
 */
static void mlx5_aso_age_init_sq(struct mlx5_aso_sq *sq) {
  ...
  for (i = 0, wqe = &sq->sq_obj.aso_wqes[0]; i < size; ++i, ++wqe) {
    wqe->general_cseg.sq_ds =
        rte_cpu_to_be_32((sq->sqn << 8) | (sizeof(*wqe) >> 4));
    wqe->aso_cseg.lkey = rte_cpu_to_be_32(sq->mr.lkey);
    addr = (uint64_t)((uint64_t *)sq->mr.addr +
                      i * MLX5_ASO_AGE_ACTIONS_PER_POOL / 64);
 ...
 *   ASO SQ to initialize.
 */
static void mlx5_aso_mtr_init_sq(struct mlx5_aso_sq *sq) {
  volatile struct mlx5_aso_wqe *restrict wqe;
  int i;
  int size = 1 << sq->log_desc_n;
  ...
  for (i = 0, wqe = &sq->sq_obj.aso_wqes[0]; i < size; ++i, ++wqe) {
    wqe->general_cseg.sq_ds =
        rte_cpu_to_be_32((sq->sqn << 8) | (sizeof(*wqe) >> 4));
    wqe->aso_cseg.operand_masks =
        RTE_BE32(0u | (ASO_OPER_LOGICAL_OR << ASO_CSEG_COND_OPER_OFFSET) |
                 (ASO_OP_ALWAYS_TRUE << ASO_CSEG_COND_1_OPER_OFFSET) |
                 (ASO_OP_ALWAYS_TRUE << ASO_CSEG_COND_0_OPER_OFFSET) |
                 (BYTEWISE_64BYTE << ASO_CSEG_DATA_MASK_MODE_OFFSET));
    wqe->general_cseg.flags =
 ...
 *   ASO SQ to initialize.
 */
static void mlx5_aso_ct_init_sq(struct mlx5_aso_sq *sq) {
  volatile struct mlx5_aso_wqe *restrict wqe;
  int i;
  int size = 1 << sq->log_desc_n;
  uint64_t addr;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_aso_sq_create(struct mlx5_common_device *cdev,
                              struct mlx5_aso_sq *sq, void *uar) {
  ...
  struct mlx5_devx_create_sq_attr sq_attr = {
      ...
      .ts_format = mlx5_ts_format_conv(cdev->config.hca_attr.sq_ts_format),
  };
  ...
  if (mlx5_devx_cq_create(cdev->ctx, &sq->cq.cq_obj, MLX5_ASO_QUEUE_LOG_DESC,
                          &cq_attr, SOCKET_ID_ANY))
    goto error;
  sq->cq.cq_ci = 0;
...
int mlx5_aso_queue_init(struct mlx5_dev_ctx_shared *sh,
  ...
  case ASO_OPC_MOD_FLOW_HIT:
    if (mlx5_aso_reg_mr(cdev, (MLX5_ASO_AGE_ACTIONS_PER_POOL / 8) * sq_desc_n,
                        &sh->aso_age_mng->aso_sq.mr))
      return -1;
    if (mlx5_aso_sq_create(cdev, &sh->aso_age_mng->aso_sq, sh->tx_uar.obj)) {
  ...
  case ASO_OPC_MOD_POLICER:
    if (mlx5_aso_sq_create(cdev, &sh->mtrmng->pools_mng.sq, sh->tx_uar.obj))
      return -1;
    mlx5_aso_mtr_init_sq(&sh->mtrmng->pools_mng.sq);
    break;
  case ASO_OPC_MOD_CONNECTION_TRACKING:
    /* 64B per object for query. */
    if (mlx5_aso_reg_mr(cdev, 64 * sq_desc_n, &sh->ct_mng->aso_sq.mr))
      return -1;
    if (mlx5_aso_sq_create(cdev, &sh->ct_mng->aso_sq, sh->tx_uar.obj)) {
 ...
 */
static uint16_t mlx5_aso_sq_enqueue_burst(struct mlx5_dev_ctx_shared *sh,
                                          uint16_t n) {
  ...
  do {
    ...
    wqe->general_cseg.misc = rte_cpu_to_be_32(
        ((struct mlx5_devx_obj *)(pool->flow_hit_aso_obj))->id);
    wqe->general_cseg.flags =
        RTE_BE32(MLX5_COMP_ONLY_FIRST_ERR << MLX5_COMP_MODE_OFFSET);
    wqe->general_cseg.opcode =
        rte_cpu_to_be_32(MLX5_OPCODE_ACCESS_ASO |
                         (ASO_OPC_MOD_FLOW_HIT << WQE_CSEG_OPC_MOD_OFFSET) |
  ...
  wqe->general_cseg.flags = RTE_BE32(MLX5_COMP_ALWAYS << MLX5_COMP_MODE_OFFSET);
  mlx5_doorbell_ring(&sh->tx_uar.bf_db, *(volatile uint64_t *)wqe, sq->pi,
                     &sq->sq_obj.db_rec[MLX5_SND_DBR], !sh->tx_uar.dbnc);
 ...
 *   Error WQE to dump.
 */
static void mlx5_aso_dump_err_objs(volatile uint32_t *cqe,
                                   volatile uint32_t *wqe) {
  int i;

  DRV_LOG(ERR, "Error cqe:");
  for (i = 0; i < 16; i += 4)
    DRV_LOG(ERR, "%08X %08X %08X %08X", cqe[i], cqe[i + 1], cqe[i + 2],
            cqe[i + 3]);
  DRV_LOG(ERR, "\nError wqe:");
  for (i = 0; i < (int)sizeof(struct mlx5_aso_wqe) / 4; i += 4)
    DRV_LOG(ERR, "%08X %08X %08X %08X", wqe[i], wqe[i + 1], wqe[i + 2],
            wqe[i + 3]);
}
 ...
 *   ASO SQ to use.
 */
static void mlx5_aso_cqe_err_handle(struct mlx5_aso_sq *sq) {
 ...
 */
static void mlx5_aso_age_action_update(struct mlx5_dev_ctx_shared *sh,
                                       uint16_t n) {
  struct mlx5_aso_age_mng *mng = sh->aso_age_mng;
  struct mlx5_aso_sq *sq = &mng->aso_sq;
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (j = 0; j < MLX5_ASO_AGE_ACTIONS_PER_POOL; j++) {
      ...
      if (__atomic_load_n(&ap->state, __ATOMIC_RELAXED) != AGE_CANDIDATE)
        continue;
      byte = 63 - (j / 8);
      offset = j % 8;
      u8addr = (uint8_t *)addr;
      ...
      if (hit) {
        __atomic_store_n(&ap->sec_since_last_hit, 0, __ATOMIC_RELAXED);
      } else {
        struct mlx5_priv *priv;

        __atomic_fetch_add(&ap->sec_since_last_hit, diff, __ATOMIC_RELAXED);
        /* If timeout passed add to aged-out list. */
        if (ap->sec_since_last_hit <= ap->timeout)
          continue;
        priv = rte_eth_devices[ap->port_id].data->dev_private;
        age_info = GET_PORT_AGE_INFO(priv);
        rte_spinlock_lock(&age_info->aged_sl);
        if (__atomic_compare_exchange_n(&ap->state, &expected, AGE_TMOUT, false,
                                        __ATOMIC_RELAXED, __ATOMIC_RELAXED)) {
          LIST_INSERT_HEAD(&age_info->aged_aso, act, next);
 ...
 */
static uint16_t mlx5_aso_completion_handle(struct mlx5_dev_ctx_shared *sh) {
  struct mlx5_aso_age_mng *mng = sh->aso_age_mng;
  struct mlx5_aso_sq *sq = &mng->aso_sq;
  struct mlx5_aso_cq *cq = &sq->cq;
 ...
 */
static void mlx5_flow_aso_alarm(void *arg) {
...
static uint16_t mlx5_aso_mtr_sq_enqueue_single(struct mlx5_dev_ctx_shared *sh,
                                               struct mlx5_aso_sq *sq,
  ...
  wqe->general_cseg.misc =
      rte_cpu_to_be_32(pool->devx_obj->id + (aso_mtr->offset >> 1));
  wqe->general_cseg.opcode =
      rte_cpu_to_be_32(MLX5_OPCODE_ACCESS_ASO |
  ...
  if (fm->is_enable) {
    wqe->aso_dseg.mtrs[dseg_idx].cbs_cir = fm->profile->srtcm_prm.cbs_cir;
    wqe->aso_dseg.mtrs[dseg_idx].ebs_eir = fm->profile->srtcm_prm.ebs_eir;
  } else {
...
static void mlx5_aso_mtrs_status_update(struct mlx5_aso_sq *sq,
  ...
  for (i = 0; i < aso_mtrs_nums; ++i) {
    ...
    MLX5_ASSERT(aso_mtr);
    (void)__atomic_compare_exchange_n(&aso_mtr->state, &exp_state,
                                      ASO_METER_READY, false, __ATOMIC_RELAXED,
                                      __ATOMIC_RELAXED);
  }
}

static void mlx5_aso_mtr_completion_handle(struct mlx5_aso_sq *sq) {
  struct mlx5_aso_cq *cq = &sq->cq;
  volatile struct mlx5_cqe *restrict cqe;
  const unsigned int cq_size = 1 << cq->log_desc_n;
  const unsigned int mask = cq_size - 1;
...
int mlx5_aso_meter_update_by_wqe(struct mlx5_dev_ctx_shared *sh,
  ...
  DRV_LOG(ERR, "Fail to send WQE for ASO meter offset %d", mtr->offset);
  return -1;
}
...
int mlx5_aso_mtr_wait(struct mlx5_dev_ctx_shared *sh,
  ...
  if (__atomic_load_n(&mtr->state, __ATOMIC_RELAXED) == ASO_METER_READY)
    return 0;
  do {
    mlx5_aso_mtr_completion_handle(sq);
    if (__atomic_load_n(&mtr->state, __ATOMIC_RELAXED) == ASO_METER_READY)
  ...
  DRV_LOG(ERR, "Fail to poll CQE ready for ASO meter offset %d", mtr->offset);
  return -1;
}
...
mlx5_aso_ct_sq_enqueue_single(struct mlx5_dev_ctx_shared *sh,
  ...
  wqe->general_cseg.misc = rte_cpu_to_be_32(pool->devx_obj->id + ct->offset);
  wqe->general_cseg.opcode = rte_cpu_to_be_32(
      MLX5_OPCODE_ACCESS_ASO |
      (ASO_OPC_MOD_CONNECTION_TRACKING << WQE_CSEG_OPC_MOD_OFFSET) |
      sq->pi << WQE_CSEG_WQE_INDEX_OFFSET);
  wqe->aso_cseg.operand_masks =
      rte_cpu_to_be_32(0u | (ASO_OPER_LOGICAL_OR << ASO_CSEG_COND_OPER_OFFSET) |
                       (ASO_OP_ALWAYS_TRUE << ASO_CSEG_COND_1_OPER_OFFSET) |
                       (ASO_OP_ALWAYS_TRUE << ASO_CSEG_COND_0_OPER_OFFSET) |
                       (BYTEWISE_64BYTE << ASO_CSEG_DATA_MASK_MODE_OFFSET));
  ...
  MLX5_SET(conn_track_aso, desg, connection_assured, profile->live_connection);
  ...
  MLX5_SET(conn_track_aso, desg, max_ack_window, profile->max_ack_window);
  ...
  MLX5_SET(tcp_window_params, orig_dir, max_win, profile->original_dir.max_win);
  MLX5_SET(tcp_window_params, orig_dir, max_ack, profile->original_dir.max_ack);
  reply_dir = MLX5_ADDR_OF(conn_track_aso, desg, reply_dir);
  MLX5_SET(tcp_window_params, reply_dir, sent_end, profile->reply_dir.sent_end);
  MLX5_SET(tcp_window_params, reply_dir, reply_end,
           profile->reply_dir.reply_end);
  MLX5_SET(tcp_window_params, reply_dir, max_win, profile->reply_dir.max_win);
  MLX5_SET(tcp_window_params, reply_dir, max_ack, profile->reply_dir.max_ack);
  sq->head++;
  sq->pi += 2; /* Each WQE contains 2 WQEBB's. */
  mlx5_doorbell_ring(&sh->tx_uar.bf_db, *(volatile uint64_t *)wqe, sq->pi,
                     &sq->sq_obj.db_rec[MLX5_SND_DBR], !sh->tx_uar.dbnc);
 ...
 */
static void mlx5_aso_ct_status_update(struct mlx5_aso_sq *sq, uint16_t num) {
  uint16_t size = 1 << sq->log_desc_n;
  uint16_t mask = size - 1;
  uint16_t i;
  ...
  for (i = 0; i < num; i++) {
    ...
    if (sq->elts[idx].query_data)
      rte_memcpy(sq->elts[idx].query_data,
                 (char *)((uintptr_t)sq->mr.addr + idx * 64), 64);
  }
}
 ...
 *   1 on success (WQE number), 0 on failure.
 */
static int mlx5_aso_ct_sq_query_single(struct mlx5_dev_ctx_shared *sh,
                                       struct mlx5_aso_ct_action *ct,
                                       char *data) {
  volatile struct mlx5_aso_wqe *wqe = NULL;
  struct mlx5_aso_sq *sq = &sh->ct_mng->aso_sq;
   ...
   */
  wqe->aso_cseg.operand_masks =
      rte_cpu_to_be_32(BYTEWISE_64BYTE << ASO_CSEG_DATA_MASK_MODE_OFFSET);
 ...
 */
static void mlx5_aso_ct_completion_handle(struct mlx5_aso_ct_pools_mng *mng) {
  struct mlx5_aso_sq *sq = &mng->aso_sq;
  struct mlx5_aso_cq *cq = &sq->cq;
  volatile struct mlx5_cqe *restrict cqe;
...
int mlx5_aso_ct_update_by_wqe(struct mlx5_dev_ctx_shared *sh,
  ...
  pool = container_of(ct, struct mlx5_aso_ct_pool, actions[ct->offset]);
  DRV_LOG(ERR, "Fail to send WQE for ASO CT %d in pool %d", ct->offset,
          pool->index);
  return -1;
}
...
int mlx5_aso_ct_wait_ready(struct mlx5_dev_ctx_shared *sh,
  ...
  if (__atomic_load_n(&ct->state, __ATOMIC_RELAXED) == ASO_CONNTRACK_READY)
    return 0;
  do {
  ...
  pool = container_of(ct, struct mlx5_aso_ct_pool, actions[ct->offset]);
  DRV_LOG(ERR, "Fail to poll CQE for ASO CT %d in pool %d", ct->offset,
          pool->index);
  return -1;
}
...
mlx5_aso_ct_obj_analyze(struct rte_flow_action_conntrack *profile,
  ...
  profile->state =
      (enum rte_flow_conntrack_state)MLX5_GET(conn_track_aso, wdata, state);
  profile->enable = !MLX5_GET(conn_track_aso, wdata, freeze_track);
  profile->selective_ack = MLX5_GET(conn_track_aso, wdata, sack_permitted);
  profile->live_connection =
      MLX5_GET(conn_track_aso, wdata, connection_assured);
  profile->challenge_ack_passed =
      MLX5_GET(conn_track_aso, wdata, challenged_acked);
  profile->max_ack_window = MLX5_GET(conn_track_aso, wdata, max_ack_window);
  profile->retransmission_limit =
      MLX5_GET(conn_track_aso, wdata, retranmission_limit);
  profile->last_window = MLX5_GET(conn_track_aso, wdata, last_win);
  profile->last_direction = MLX5_GET(conn_track_aso, wdata, last_dir);
  profile->last_index = (enum rte_flow_conntrack_tcp_last_index)MLX5_GET(
      conn_track_aso, wdata, last_index);
  ...
  profile->liberal_mode =
      MLX5_GET(conn_track_aso, wdata, reply_direction_tcp_liberal_enabled) |
      MLX5_GET(conn_track_aso, wdata, original_direction_tcp_liberal_enabled);
  /* No liberal in the RTE structure profile. */
  profile->reply_dir.scale =
      MLX5_GET(conn_track_aso, wdata, reply_direction_tcp_scale);
  profile->reply_dir.close_initiated =
      MLX5_GET(conn_track_aso, wdata, reply_direction_tcp_close_initiated);
  profile->reply_dir.data_unacked =
      MLX5_GET(conn_track_aso, wdata, reply_direction_tcp_data_unacked);
  profile->reply_dir.last_ack_seen =
      MLX5_GET(conn_track_aso, wdata, reply_direction_tcp_max_ack);
  profile->reply_dir.sent_end = MLX5_GET(tcp_window_params, r_dir, sent_end);
  profile->reply_dir.reply_end = MLX5_GET(tcp_window_params, r_dir, reply_end);
  profile->reply_dir.max_win = MLX5_GET(tcp_window_params, r_dir, max_win);
  profile->reply_dir.max_ack = MLX5_GET(tcp_window_params, r_dir, max_ack);
  profile->original_dir.scale =
      MLX5_GET(conn_track_aso, wdata, original_direction_tcp_scale);
  profile->original_dir.close_initiated =
      MLX5_GET(conn_track_aso, wdata, original_direction_tcp_close_initiated);
  profile->original_dir.data_unacked =
      MLX5_GET(conn_track_aso, wdata, original_direction_tcp_data_unacked);
  profile->original_dir.last_ack_seen =
      MLX5_GET(conn_track_aso, wdata, original_direction_tcp_max_ack);
  profile->original_dir.sent_end = MLX5_GET(tcp_window_params, o_dir, sent_end);
  profile->original_dir.reply_end =
      MLX5_GET(tcp_window_params, o_dir, reply_end);
  profile->original_dir.max_win = MLX5_GET(tcp_window_params, o_dir, max_win);
  profile->original_dir.max_ack = MLX5_GET(tcp_window_params, o_dir, max_ack);
}
...
int mlx5_aso_ct_available(struct mlx5_dev_ctx_shared *sh,
  ...
  do {
    ...
    if (state == ASO_CONNTRACK_READY || state == ASO_CONNTRACK_QUERY)
      return 0;
    /* Waiting for CQE ready, consider should block or sleep. */
    rte_delay_us_sleep(MLX5_ASO_WQE_CQE_RESPONSE_DELAY);
  } while (--poll_cqe_times);
