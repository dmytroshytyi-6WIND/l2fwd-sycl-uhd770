...
static inline void mlx5_ipool_lock(struct mlx5_indexed_pool *pool) {
  if (pool->cfg.need_lock)
    rte_spinlock_lock(&pool->rsz_lock);
}

static inline void mlx5_ipool_unlock(struct mlx5_indexed_pool *pool) {
  if (pool->cfg.need_lock)
    rte_spinlock_unlock(&pool->rsz_lock);
}

static inline uint32_t mlx5_trunk_idx_get(struct mlx5_indexed_pool *pool,
                                          uint32_t entry_idx) {
  ...
  if (entry_idx >= pool->grow_tbl[cfg->grow_trunk - 1]) {
    trunk_idx = (entry_idx - pool->grow_tbl[cfg->grow_trunk - 1]) /
                    (cfg->trunk_size << (cfg->grow_shift * cfg->grow_trunk)) +
                cfg->grow_trunk;
  } else {
...
static inline uint32_t mlx5_trunk_size_get(struct mlx5_indexed_pool *pool,
                                           uint32_t trunk_idx) {
  struct mlx5_indexed_pool_config *cfg = &pool->cfg;

  return cfg->trunk_size << (cfg->grow_shift * (trunk_idx > cfg->grow_trunk
                                                    ? cfg->grow_trunk
                                                    : trunk_idx));
}

static inline uint32_t mlx5_trunk_idx_offset_get(struct mlx5_indexed_pool *pool,
                                                 uint32_t trunk_idx) {
  struct mlx5_indexed_pool_config *cfg = &pool->cfg;
  uint32_t offset = 0;
...
mlx5_ipool_create(struct mlx5_indexed_pool_config *cfg) {
  ...
  if (!cfg || (!cfg->malloc ^ !cfg->free) ||
      ...
      (cfg->trunk_size &&
       ((cfg->trunk_size & (cfg->trunk_size - 1)) ||
        ((__builtin_ffs(cfg->trunk_size) + TRUNK_IDX_BITS) > 32))))
    return NULL;
  pool =
      mlx5_malloc(MLX5_MEM_ZERO,
                  sizeof(*pool) + cfg->grow_trunk * sizeof(pool->grow_tbl[0]),
  ...
  if (!pool->cfg.max_idx)
    pool->cfg.max_idx = mlx5_trunk_idx_offset_get(pool, TRUNK_MAX_IDX + 1);
...
static int mlx5_ipool_grow(struct mlx5_indexed_pool *pool) {
  ...
  if (pool->n_trunk_valid == TRUNK_MAX_IDX || cur_max_idx >= pool->cfg.max_idx)
    return -ENOMEM;
  if (pool->n_trunk_valid == pool->n_trunk) {
    ...
    p = pool->cfg.malloc(
        0, (pool->n_trunk_valid + n_grow) * sizeof(struct mlx5_indexed_trunk *),
        RTE_CACHE_LINE_SIZE, rte_socket_id());
    if (!p)
      return -ENOMEM;
    if (pool->trunks)
      memcpy(p, pool->trunks,
             pool->n_trunk_valid * sizeof(struct mlx5_indexed_trunk *));
  ...
  trunk_size += bmp_size;
  trunk = pool->cfg.malloc(0, trunk_size, RTE_CACHE_LINE_SIZE, rte_socket_id());
  if (!trunk)
    return -ENOMEM;
  ...
  trunk->bmp = rte_bitmap_init_with_all_set(
      data_size,
      &trunk->data[RTE_CACHE_LINE_ROUNDUP(data_size * pool->cfg.size)],
      bmp_size);
...
static uint32_t mlx5_ipool_allocate_from_global(struct mlx5_indexed_pool *pool,
                                                int cidx) {
  struct mlx5_indexed_trunk *trunk;
...
check_again:
  ...
  if (trunk_idx == trunk_n) {
    ...
    if (lc)
      memcpy(p->trunks, lc->trunks,
             trunk_idx * sizeof(struct mlx5_indexed_trunk *));
   ...
   */
  if (pool->gc &&
      (lc != pool->gc || lc->n_trunk_valid != trunk_idx || pool->gc->len)) {
    mlx5_ipool_unlock(pool);
    if (p)
      pool->cfg.free(p);
    pool->cfg.free(trunk);
...
static void *_mlx5_ipool_get_cache(struct mlx5_indexed_pool *pool, int cidx,
                                   uint32_t idx) {
  struct mlx5_indexed_trunk *trunk;
...
static void *mlx5_ipool_get_cache(struct mlx5_indexed_pool *pool,
                                  uint32_t idx) {
  void *entry;
  int cidx;
...
static void *_mlx5_ipool_malloc_cache(struct mlx5_indexed_pool *pool, int cidx,
                                      uint32_t *idx) {
  if (unlikely(!pool->cache[cidx])) {
    pool->cache[cidx] =
        pool->cfg.malloc(MLX5_MEM_ZERO,
...
static void *mlx5_ipool_malloc_cache(struct mlx5_indexed_pool *pool,
                                     uint32_t *idx) {
  void *entry;
  int cidx;
...
static void _mlx5_ipool_free_cache(struct mlx5_indexed_pool *pool, int cidx,
                                   uint32_t idx) {
...
static void mlx5_ipool_free_cache(struct mlx5_indexed_pool *pool,
                                  uint32_t idx) {
  int cidx;
...
void mlx5_ipool_free(struct mlx5_indexed_pool *pool, uint32_t idx) {
  ...
  if (trunk_idx != trunk->idx || rte_bitmap_get(trunk->bmp, entry_idx))
    goto out;
  rte_bitmap_set(trunk->bmp, entry_idx);
  trunk->free++;
  if (pool->cfg.release_mem_en &&
      trunk->free == mlx5_trunk_size_get(pool, trunk->idx)) {
...
int mlx5_ipool_destroy(struct mlx5_indexed_pool *pool) {
  ...
  if (pool->cfg.per_core_cache) {
    for (i = 0; i <= RTE_MAX_LCORE; i++) {
       ...
       */
      if (pool->cache[i]) {
        if (pool->cache[i]->lc && pool->cache[i]->lc != pool->gc &&
...
void mlx5_ipool_flush_cache(struct mlx5_indexed_pool *pool) {
  ...
  pool->bmp_mem = pool->cfg.malloc(MLX5_MEM_ZERO, mem_size, RTE_CACHE_LINE_SIZE,
                                   rte_socket_id());
  if (!pool->bmp_mem) {
...
static void *mlx5_ipool_get_next_cache(struct mlx5_indexed_pool *pool,
                                       uint32_t *pos) {
  struct rte_bitmap *ibmp;
  uint64_t slab = 0;
  uint32_t iidx = *pos;
...
struct mlx5_l3t_tbl *mlx5_l3t_create(enum mlx5_l3t_type type) {
  ...
  tbl =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(struct mlx5_l3t_tbl), 1, SOCKET_ID_ANY);
  if (!tbl) {
    rte_errno = ENOMEM;
    return NULL;
  }
  ...
  if (g_tbl) {
    for (i = 0; i < MLX5_L3T_GT_SIZE; i++) {
      ...
      for (j = 0; j < MLX5_L3T_MT_SIZE; j++) {
        if (!m_tbl->tbl[j])
          continue;
        MLX5_ASSERT(!((struct mlx5_l3t_entry_word *)m_tbl->tbl[j])->ref_cnt);
        mlx5_ipool_free(tbl->eip,
                        ((struct mlx5_l3t_entry_word *)m_tbl->tbl[j])->idx);
        m_tbl->tbl[j] = 0;
        if (!(--m_tbl->ref_cnt))
          break;
      }
...
static int32_t __l3t_get_entry(struct mlx5_l3t_tbl *tbl, uint32_t idx,
...
int32_t mlx5_l3t_clear_entry(struct mlx5_l3t_tbl *tbl, uint32_t idx) {
  ...
  if (!ref_cnt) {
    mlx5_ipool_free(tbl->eip, ((struct mlx5_l3t_entry_word *)e_tbl)->idx);
    m_tbl->tbl[(idx >> MLX5_L3T_MT_OFFSET) & MLX5_L3T_MT_MASK] = NULL;
    if (!(--m_tbl->ref_cnt)) {
      mlx5_free(m_tbl);
      g_tbl->tbl[(idx >> MLX5_L3T_GT_OFFSET) & MLX5_L3T_GT_MASK] = NULL;
      if (!(--g_tbl->ref_cnt)) {
        mlx5_free(g_tbl);
        tbl->tbl = 0;
      }
...
static int32_t __l3t_set_entry(struct mlx5_l3t_tbl *tbl, uint32_t idx,
                               union mlx5_l3t_data *data) {
  struct mlx5_l3t_level_tbl *g_tbl, *m_tbl;
  struct mlx5_l3t_entry_word *w_e_tbl;
  struct mlx5_l3t_entry_dword *dw_e_tbl;
  ...
  if (!m_tbl) {
    m_tbl = mlx5_malloc(MLX5_MEM_ZERO,
                        sizeof(struct mlx5_l3t_level_tbl) +
                            sizeof(void *) * MLX5_L3T_MT_SIZE,
                        1, SOCKET_ID_ANY);
    if (!m_tbl) {
      rte_errno = ENOMEM;
      return -1;
    }
    g_tbl->tbl[(idx >> MLX5_L3T_GT_OFFSET) & MLX5_L3T_GT_MASK] = m_tbl;
    g_tbl->ref_cnt++;
  }
  /*
  ...
  if (!e_tbl) {
    ...
    m_tbl->tbl[(idx >> MLX5_L3T_MT_OFFSET) & MLX5_L3T_MT_MASK] = e_tbl;
    m_tbl->ref_cnt++;
  }
  entry_idx = idx & MLX5_L3T_ET_MASK;
  switch (tbl->type) {
