...
int mlx5_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on) {
  ...
  DRV_LOG(DEBUG, "port %u %s VLAN filter ID %" PRIu16, dev->data->port_id,
          (on ? "enable" : "disable"), vlan_id);
  ...
  if (i < priv->vlan_filter_n) {
    ...
    --priv->vlan_filter_n;
    memmove(&priv->vlan_filter[i], &priv->vlan_filter[i + 1],
            sizeof(priv->vlan_filter[i]) * (priv->vlan_filter_n - i));
 ...
 */
void mlx5_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue,
                               int on) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (!priv->sh->dev_cap.hw_vlan_strip) {
    DRV_LOG(ERR, "port %u VLAN stripping is not supported", dev->data->port_id);
    return;
  }
  ...
  if (ret) {
    DRV_LOG(ERR,
            "Port %u failed to modify object stripping mode:"
            " %s",
...
int mlx5_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  ...
  if (mask & RTE_ETH_VLAN_STRIP_MASK) {
    int hw_vlan_strip =
        !!(dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_VLAN_STRIP);
