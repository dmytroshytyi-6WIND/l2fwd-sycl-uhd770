...
static int flow_tunnel_add_default_miss(
    struct rte_eth_dev *dev, struct rte_flow *flow,
    const struct rte_flow_attr *attr, const struct rte_flow_action *app_actions,
    uint32_t flow_idx, const struct mlx5_flow_tunnel *tunnel,
    struct tunnel_default_miss_ctx *ctx, struct rte_flow_error *error);
static struct mlx5_flow_tunnel *mlx5_find_tunnel_id(struct rte_eth_dev *dev,
                                                    uint32_t id);
static void mlx5_flow_tunnel_free(struct rte_eth_dev *dev,
                                  struct mlx5_flow_tunnel *tunnel);
static uint32_t tunnel_flow_group_to_flow_table(
    struct rte_eth_dev *dev, const struct mlx5_flow_tunnel *tunnel,
    uint32_t group, uint32_t *table, struct rte_flow_error *error);
...
static void mlx5_dbg__print_pattern(const struct rte_flow_item *item);
...
static bool mlx5_flow_is_rss_expandable_item(const struct rte_flow_item *item) {
  switch (item->type) {
 ...
 */
static enum rte_flow_item_type mlx5_nsh_proto_to_item_type(uint8_t proto_spec,
                                                           uint8_t proto_mask) {
...
static enum rte_flow_item_type mlx5_ethertype_to_item_type(rte_be16_t type_spec,
  ...
  case RTE_ETHER_TYPE_TEB:
    type = is_tunnel ? RTE_FLOW_ITEM_TYPE_ETH : RTE_FLOW_ITEM_TYPE_END;
    break;
  case RTE_ETHER_TYPE_VLAN:
    type = !is_tunnel ? RTE_FLOW_ITEM_TYPE_VLAN : RTE_FLOW_ITEM_TYPE_END;
    break;
...
#define MLX5_XSET_ITEM_MASK_SPEC(type, fld)                                    \
  do {                                                                         \
    ...
    mask = m ? ((const struct rte_flow_item_##type *)m)->fld                   \
 ...
 */
static int mlx5_flow_expand_rss(struct mlx5_flow_expand_rss *buf, size_t size,
  ...
  for (item = pattern, item_idx = 0; item->type != RTE_FLOW_ITEM_TYPE_END;
       item++, item_idx++) {
  ...
  if (next && missed) {
    ...
    if (node->rss_types & types) {
      ...
      buf->entries++;
      rte_memcpy(addr, buf->entry[0].pattern, user_pattern_size);
      addr = (void *)(((uintptr_t)addr) + user_pattern_size);
      rte_memcpy(addr, flow_items, elt * sizeof(*item));
      addr = (void *)(((uintptr_t)addr) + elt * sizeof(*item));
    }
  } else if (last_expand_item != NULL) {
    node = mlx5_flow_expand_rss_adjust_node(pattern, last_expand_item_idx,
                                            graph, node);
  }
  memset(flow_items, 0, sizeof(flow_items));
  next_node = mlx5_flow_expand_rss_skip_explicit(graph, node->next);
  ...
  while (node) {
    ...
    if (node->rss_types & types) {
      ...
      if (lsize > size)
        return -EINVAL;
      n = elt * sizeof(*item);
      buf->entry[buf->entries].priority = stack_pos + 1 + missed;
      buf->entry[buf->entries].pattern = addr;
      buf->entries++;
      rte_memcpy(addr, buf->entry[0].pattern, user_pattern_size);
      addr = (void *)(((uintptr_t)addr) + user_pattern_size);
      rte_memcpy(addr, &missed_item, missed * sizeof(*item));
      addr = (void *)(((uintptr_t)addr) + missed * sizeof(*item));
      rte_memcpy(addr, flow_items, n);
      addr = (void *)(((uintptr_t)addr) + n);
    }
    /* Go deeper. */
    if (!(node->node_flags & MLX5_EXPANSION_NODE_OPTIONAL) && node->next) {
      next_node = mlx5_flow_expand_rss_skip_explicit(graph, node->next);
      ...
      next_node = mlx5_flow_expand_rss_skip_explicit(graph, ++next_node);
    } else if (!stack_pos) {
      /*
...
static const struct mlx5_flow_expand_node mlx5_support_expansion[] = {
    [MLX5_EXPANSION_ROOT] =
        {
            .next = MLX5_FLOW_EXPAND_RSS_NEXT(
                MLX5_EXPANSION_ETH, MLX5_EXPANSION_IPV4, MLX5_EXPANSION_IPV6),
            .type = RTE_FLOW_ITEM_TYPE_END,
        },
    [MLX5_EXPANSION_ROOT_OUTER] =
        {
    ...
    [MLX5_EXPANSION_OUTER_IPV4_UDP] =
        {
            .next = MLX5_FLOW_EXPAND_RSS_NEXT(
                MLX5_EXPANSION_VXLAN, MLX5_EXPANSION_VXLAN_GPE,
                MLX5_EXPANSION_MPLS, MLX5_EXPANSION_GENEVE, MLX5_EXPANSION_GTP),
            .type = RTE_FLOW_ITEM_TYPE_UDP,
    ...
    [MLX5_EXPANSION_OUTER_IPV6] =
        {
            .next = MLX5_FLOW_EXPAND_RSS_NEXT(
                MLX5_EXPANSION_OUTER_IPV6_UDP, MLX5_EXPANSION_OUTER_IPV6_TCP,
                MLX5_EXPANSION_IPV4, MLX5_EXPANSION_IPV6, MLX5_EXPANSION_GRE,
                MLX5_EXPANSION_NVGRE),
            .type = RTE_FLOW_ITEM_TYPE_IPV6,
    ...
    [MLX5_EXPANSION_GRE] =
        {
            .next = MLX5_FLOW_EXPAND_RSS_NEXT(
                MLX5_EXPANSION_ETH, MLX5_EXPANSION_IPV4, MLX5_EXPANSION_IPV6,
                MLX5_EXPANSION_GRE_KEY, MLX5_EXPANSION_MPLS),
            .type = RTE_FLOW_ITEM_TYPE_GRE,
        },
    [MLX5_EXPANSION_GRE_KEY] =
        {
            .next = MLX5_FLOW_EXPAND_RSS_NEXT(
                MLX5_EXPANSION_IPV4, MLX5_EXPANSION_IPV6, MLX5_EXPANSION_MPLS),
    ...
    [MLX5_EXPANSION_MPLS] =
        {
            .next = MLX5_FLOW_EXPAND_RSS_NEXT(
                MLX5_EXPANSION_IPV4, MLX5_EXPANSION_IPV6, MLX5_EXPANSION_ETH),
...
static struct rte_flow_action_handle *mlx5_action_handle_create(
...
static int mlx5_action_handle_destroy(struct rte_eth_dev *dev,
                                      struct rte_flow_action_handle *handle,
                                      struct rte_flow_error *error);
static int mlx5_action_handle_update(struct rte_eth_dev *dev,
                                     struct rte_flow_action_handle *handle,
                                     const void *update,
                                     struct rte_flow_error *error);
static int mlx5_action_handle_query(struct rte_eth_dev *dev,
                                    const struct rte_flow_action_handle *handle,
                                    void *data, struct rte_flow_error *error);
static int mlx5_flow_tunnel_decap_set(struct rte_eth_dev *dev,
                                      struct rte_flow_tunnel *app_tunnel,
                                      struct rte_flow_action **actions,
                                      uint32_t *num_of_actions,
                                      struct rte_flow_error *error);
static int mlx5_flow_tunnel_match(struct rte_eth_dev *dev,
                                  struct rte_flow_tunnel *app_tunnel,
                                  struct rte_flow_item **items,
                                  uint32_t *num_of_items,
                                  struct rte_flow_error *error);
static int mlx5_flow_tunnel_item_release(struct rte_eth_dev *dev,
                                         struct rte_flow_item *pmd_items,
                                         uint32_t num_items,
                                         struct rte_flow_error *err);
static int mlx5_flow_tunnel_action_release(struct rte_eth_dev *dev,
                                           struct rte_flow_action *pmd_actions,
                                           uint32_t num_actions,
                                           struct rte_flow_error *err);
static int mlx5_flow_tunnel_get_restore_info(struct rte_eth_dev *dev,
                                             struct rte_mbuf *m,
...
mlx5_flow_flex_item_release(struct rte_eth_dev *dev,
                            ...
                            struct rte_flow_error *error);
static int mlx5_flow_info_get(struct rte_eth_dev *dev,
                              struct rte_flow_port_info *port_info,
                              struct rte_flow_queue_info *queue_info,
                              struct rte_flow_error *error);
static int mlx5_flow_port_configure(
...
static struct rte_flow_pattern_template *mlx5_flow_pattern_template_create(
    struct rte_eth_dev *dev, const struct rte_flow_pattern_template_attr *attr,
    const struct rte_flow_item items[], struct rte_flow_error *error);

static int
mlx5_flow_pattern_template_destroy(struct rte_eth_dev *dev,
                                   struct rte_flow_pattern_template *template,
                                   struct rte_flow_error *error);
static struct rte_flow_actions_template *mlx5_flow_actions_template_create(
    struct rte_eth_dev *dev, const struct rte_flow_actions_template_attr *attr,
...
static struct rte_flow_template_table *mlx5_flow_table_create(
    struct rte_eth_dev *dev, const struct rte_flow_template_table_attr *attr,
    ...
    uint8_t nb_action_templates, struct rte_flow_error *error);
static int mlx5_flow_table_destroy(struct rte_eth_dev *dev,
                                   struct rte_flow_template_table *table,
                                   struct rte_flow_error *error);
static struct rte_flow *mlx5_flow_async_flow_create(
...
static int mlx5_flow_async_flow_destroy(struct rte_eth_dev *dev, uint32_t queue,
                                        const struct rte_flow_op_attr *attr,
                                        struct rte_flow *flow, void *user_data,
                                        struct rte_flow_error *error);
static int mlx5_flow_pull(struct rte_eth_dev *dev, uint32_t queue,
                          struct rte_flow_op_result res[], uint16_t n_res,
                          struct rte_flow_error *error);
static int mlx5_flow_push(struct rte_eth_dev *dev, uint32_t queue,
                          struct rte_flow_error *error);

static struct rte_flow_action_handle *mlx5_flow_async_action_handle_create(
    struct rte_eth_dev *dev, uint32_t queue,
    const struct rte_flow_op_attr *attr,
    const struct rte_flow_indir_action_conf *conf,
    const struct rte_flow_action *action, void *user_data,
    struct rte_flow_error *error);

static int mlx5_flow_async_action_handle_update(
    struct rte_eth_dev *dev, uint32_t queue,
    const struct rte_flow_op_attr *attr, struct rte_flow_action_handle *handle,
    const void *update, void *user_data, struct rte_flow_error *error);

static int mlx5_flow_async_action_handle_destroy(
    struct rte_eth_dev *dev, uint32_t queue,
    const struct rte_flow_op_attr *attr, struct rte_flow_action_handle *handle,
    void *user_data, struct rte_flow_error *error);
...
int mlx5_flow_get_reg_id(struct rte_eth_dev *dev,
                         enum mlx5_feature_name feature, uint32_t id,
                         struct rte_flow_error *error) {
  ...
  case MLX5_APP_TAG:
     ...
     */
    start_reg = priv->mtr_color_reg != REG_C_2
                    ? REG_C_2
     ...
     * color register.
     */
    if (skip_mtr_reg && priv->sh->flow_mreg_c[id + start_reg - REG_C_0] >=
                            priv->mtr_color_reg) {
      if (id >= (uint32_t)(REG_C_7 - start_reg))
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                                  "invalid tag id");
      if (priv->sh->flow_mreg_c[id + 1 + start_reg - REG_C_0] != REG_NON)
        return priv->sh->flow_mreg_c[id + 1 + start_reg - REG_C_0];
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                                "unsupported tag id");
    }
...
int mlx5_flow_item_acceptable(const struct rte_flow_item *item,
                              const uint8_t *mask, const uint8_t *nic_mask,
                              unsigned int size, bool range_accepted,
                              struct rte_flow_error *error) {
  unsigned int i;
 ...
 *   Rx queue to update.
 */
static void flow_rxq_tunnel_ptype_update(struct mlx5_rxq_ctrl *rxq_ctrl) {
  unsigned int i;
...
void flow_drv_rxq_flags_set(struct rte_eth_dev *dev,
  ...
  if (dev_handle->fate_action == MLX5_FLOW_FATE_QUEUE) {
    ...
    hrxq =
        mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_HRXQ], dev_handle->rix_hrxq);
    if (hrxq)
  ...
  for (i = 0; i != ind_tbl->queues_n; ++i) {
    ...
    if (tunnel) {
      ...
      for (j = 0; j != MLX5_FLOW_TUNNEL; ++j) {
        if ((tunnels_info[j].tunnel & dev_handle->layers) ==
...
static void flow_rxq_mark_flag_set(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_rxq_ctrl *rxq_ctrl;

  if (priv->mark_enabled)
    return;
  LIST_FOREACH(rxq_ctrl, &priv->rxqsctrl, next) { rxq_ctrl->rxq.mark = 1; }
  priv->mark_enabled = 1;
}
 ...
 */
static void flow_rxq_flags_set(struct rte_eth_dev *dev, struct rte_flow *flow) {
  struct mlx5_priv *priv = dev->data->dev_private;
  uint32_t handle_idx;
  struct mlx5_flow_handle *dev_handle;
 ...
 */
static void flow_drv_rxq_flags_trim(struct rte_eth_dev *dev,
                                    struct mlx5_flow_handle *dev_handle) {
  struct mlx5_priv *priv = dev->data->dev_private;
  const int tunnel = !!(dev_handle->layers & MLX5_FLOW_LAYER_TUNNEL);
  struct mlx5_ind_table_obj *ind_tbl = NULL;
 ...
 *   Pointer to the flow.
 */
static void flow_rxq_flags_trim(struct rte_eth_dev *dev,
 ...
 *   Pointer to Ethernet device.
 */
static void flow_rxq_flags_clear(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  unsigned int i;
...
int mlx5_flow_validate_action_mark(const struct rte_flow_action *action,
  ...
  if (mark->id >= MLX5_FLOW_MARK_MAX)
    return rte_flow_error_set(
        error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF, &mark->id,
        "mark id must in 0 <= id < " RTE_STR(MLX5_FLOW_MARK_MAX));
  if (action_flags & MLX5_FLOW_ACTION_FLAG)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_flow_validate_action_drop(uint64_t action_flags __rte_unused,
                                   const struct rte_flow_attr *attr,
                                   struct rte_flow_error *error) {
  if (attr->egress)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS,
...
int mlx5_flow_validate_action_queue(const struct rte_flow_action *action,
  ...
  if (attr->egress)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS,
                              NULL, "queue action not supported for egress.");
  ...
  if (queue->index >= priv->rxqs_n)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &queue->index, "queue index out of range");
  if (mlx5_rxq_get(dev, queue->index) == NULL)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &queue->index, "queue is not configured");
  return 0;
}
 ...
 */
static int mlx5_validate_rss_queues(struct rte_eth_dev *dev,
...
int mlx5_validate_action_rss(struct rte_eth_dev *dev,
  ...
  if (rss->func != RTE_ETH_HASH_FUNCTION_DEFAULT &&
    ...
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &rss->func, "RSS hash function not supported");
#ifdef HAVE_IBV_DEVICE_TUNNEL_SUPPORT
  if (rss->level > 2)
#else
  if (rss->level > 1)
#endif
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &rss->level, "tunnel RSS is not supported");
  ...
  if (rss->key_len > 0 && rss->key_len < MLX5_RSS_HASH_KEY_LEN)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &rss->key_len, "RSS hash key too small");
  if (rss->key_len > MLX5_RSS_HASH_KEY_LEN)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &rss->key_len, "RSS hash key too large");
  if (rss->queue_num > priv->sh->dev_cap.ind_table_max_size)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              &rss->queue_num, "number of queues too large");
  ...
  ret = mlx5_validate_rss_queues(dev, rss->queue, rss->queue_num, &message,
                                 &queue_idx);
  if (ret != 0) {
    return rte_flow_error_set(error, -ret, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
...
int mlx5_flow_validate_action_rss(const struct rte_flow_action *action,
  ...
  if ((item_flags & MLX5_FLOW_LAYER_ECPRI) &&
    ...
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              NULL, "RSS on eCPRI is not supported now");
  }
  if ((item_flags & MLX5_FLOW_LAYER_MPLS) &&
      !(item_flags & (MLX5_FLOW_LAYER_INNER_L2 | MLX5_FLOW_LAYER_INNER_L3)) &&
      rss->level > 1)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                              "MPLS inner RSS needs to specify inner L2/L3 "
                              "items after MPLS in pattern");
  return 0;
}
...
int mlx5_flow_validate_action_default_miss(uint64_t action_flags,
  ...
  if (attr->group)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_GROUP,
                              NULL, "only group 0 is supported");
  if (attr->transfer)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_TRANSFER,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_flow_validate_action_count(struct rte_eth_dev *dev __rte_unused,
                                    const struct rte_flow_attr *attr,
                                    struct rte_flow_error *error) {
  if (attr->egress)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS,
...
int mlx5_flow_validate_attributes(struct rte_eth_dev *dev,
  ...
  if (!attributes->ingress)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS,
                              NULL, "ingress attribute is mandatory");
  return 0;
}
...
int mlx5_flow_validate_item_icmp6(const struct rte_flow_item *item,
                                  uint64_t item_flags, uint8_t target_protocol,
                                  struct rte_flow_error *error) {
  const struct rte_flow_item_icmp6 *mask = item->mask;
  const int tunnel = !!(item_flags & MLX5_FLOW_LAYER_TUNNEL);
  const uint64_t l3m =
      tunnel ? MLX5_FLOW_LAYER_INNER_L3_IPV6 : MLX5_FLOW_LAYER_OUTER_L3_IPV6;
  const uint64_t l4m =
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&rte_flow_item_icmp6_mask,
      sizeof(struct rte_flow_item_icmp6), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  return 0;
}
...
int mlx5_flow_validate_item_icmp(const struct rte_flow_item *item,
  ...
  const int tunnel = !!(item_flags & MLX5_FLOW_LAYER_TUNNEL);
  const uint64_t l3m =
      tunnel ? MLX5_FLOW_LAYER_INNER_L3_IPV4 : MLX5_FLOW_LAYER_OUTER_L3_IPV4;
  const uint64_t l4m =
      tunnel ? MLX5_FLOW_LAYER_INNER_L4 : MLX5_FLOW_LAYER_OUTER_L4;
  int ret;
  ...
  ret = mlx5_flow_item_acceptable(
      ...
      sizeof(struct rte_flow_item_icmp), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  return 0;
}
...
int mlx5_flow_validate_item_eth(const struct rte_flow_item *item,
  ...
  int tunnel = !!(item_flags & MLX5_FLOW_LAYER_TUNNEL);
  const uint64_t ethm =
      tunnel ? MLX5_FLOW_LAYER_INNER_L2 : MLX5_FLOW_LAYER_OUTER_L2;
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_eth), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  return ret;
}
...
int mlx5_flow_validate_item_vlan(const struct rte_flow_item *item,
                                 uint64_t item_flags, struct rte_eth_dev *dev,
                                 struct rte_flow_error *error) {
  ...
  if (!vlan_tag)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_SPEC,
                              item->spec, "VLAN cannot be empty");
  return 0;
}
...
int mlx5_flow_validate_item_ipv4(const struct rte_flow_item *item,
  ...
  const int tunnel = !!(item_flags & MLX5_FLOW_LAYER_TUNNEL);
  const uint64_t l3m =
      tunnel ? MLX5_FLOW_LAYER_INNER_L3 : MLX5_FLOW_LAYER_OUTER_L3;
  const uint64_t l4m =
      tunnel ? MLX5_FLOW_LAYER_INNER_L4 : MLX5_FLOW_LAYER_OUTER_L4;
  int ret;
  uint8_t next_proto = 0xFF;
  const uint64_t l2_vlan = (MLX5_FLOW_LAYER_L2 | MLX5_FLOW_LAYER_OUTER_VLAN |
                            MLX5_FLOW_LAYER_INNER_VLAN);

  if ((last_item & l2_vlan) && ether_type && ether_type != RTE_ETHER_TYPE_IPV4)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                              "IPv4 cannot follow L2/VLAN layer "
                              "which ether type is not IPv4");
  if (item_flags & MLX5_FLOW_LAYER_TUNNEL) {
    if (mask && spec)
      next_proto = mask->hdr.next_proto_id & spec->hdr.next_proto_id;
  ...
  else if (mask->hdr.next_proto_id != 0 && mask->hdr.next_proto_id != 0xff)
  ...
  ret = mlx5_flow_item_acceptable(
      ...
      acc_mask ? (const uint8_t *)acc_mask : (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_ipv4), range_accepted, error);
  if (ret < 0)
    return ret;
  return 0;
}
...
int mlx5_flow_validate_item_ipv6(const struct rte_flow_item *item,
  ...
  if ((last_item & l2_vlan) && ether_type && ether_type != RTE_ETHER_TYPE_IPV6)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
  ...
  if (next_proto == IPPROTO_HOPOPTS || next_proto == IPPROTO_ROUTING ||
      next_proto == IPPROTO_FRAGMENT || next_proto == IPPROTO_ESP ||
      next_proto == IPPROTO_AH || next_proto == IPPROTO_DSTOPTS)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask,
      acc_mask ? (const uint8_t *)acc_mask : (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_ipv6), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  return 0;
}
...
int mlx5_flow_validate_item_udp(const struct rte_flow_item *item,
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&rte_flow_item_udp_mask,
      sizeof(struct rte_flow_item_udp), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  return 0;
}
...
int mlx5_flow_validate_item_tcp(const struct rte_flow_item *item,
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)flow_mask,
      sizeof(struct rte_flow_item_tcp), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_flow_validate_item_vxlan(struct rte_eth_dev *dev, uint16_t udp_dport,
                                  const struct rte_flow_item *item,
                                  uint64_t item_flags,
                                  const struct rte_flow_attr *attr,
                                  struct rte_flow_error *error) {
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)valid_mask,
      sizeof(struct rte_flow_item_vxlan), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  if (spec) {
...
int mlx5_flow_validate_item_vxlan_gpe(const struct rte_flow_item *item,
  ...
  ret =
      mlx5_flow_item_acceptable(item, (const uint8_t *)mask,
...
int mlx5_flow_validate_item_gre_key(const struct rte_flow_item *item,
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&gre_key_default_mask,
...
int mlx5_flow_validate_item_gre_option(struct rte_eth_dev *dev,
  ...
  if (!spec || !mask)
    return rte_flow_error_set(
        ...
        "At least one field gre_option(checksum/key/sequence) must be "
        "specified");
  if (!gre_mask)
    gre_mask = &rte_flow_item_gre_mask;
...
int mlx5_flow_validate_item_gre(const struct rte_flow_item *item,
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_gre), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
...
int mlx5_flow_validate_item_geneve(const struct rte_flow_item *item,
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_geneve), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret)
    return ret;
  if (spec) {
    gbhdr = rte_be_to_cpu_16(spec->ver_opt_len_o_c_rsvd0);
    if (MLX5_GENEVE_VER_VAL(gbhdr) || MLX5_GENEVE_CRITO_VAL(gbhdr) ||
...
int mlx5_flow_validate_item_geneve_opt(const struct rte_flow_item *item,
  ...
  uint8_t data_max_supported = hca_attr->max_geneve_tlv_option_data_len * 4;
  ...
  if ((uint32_t)spec->option_len > hca_attr->max_geneve_tlv_option_data_len)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item,
                              "Geneve TLV opt length not supported");
  if (hca_attr->max_geneve_tlv_options > 1)
    DRV_LOG(DEBUG, "max_geneve_tlv_options supports more than 1 option");
  ...
  geneve_mask =
      geneve_item->mask ? geneve_item->mask : &rte_flow_item_geneve_mask;
    ...
    for (i = data_max_supported; i < data_len; i++)
      if (mask->data[i])
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM,
                                  item, "Data mask is of unsupported size");
  }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_flow_validate_item_mpls(struct rte_eth_dev *dev __rte_unused,
  ...
  if (!(prev_layer & (MLX5_FLOW_LAYER_OUTER_L4_UDP | MLX5_FLOW_LAYER_GRE |
  ...
  ret = mlx5_flow_item_acceptable(
      ...
      sizeof(struct rte_flow_item_mpls), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  return 0;
#else
...
int mlx5_flow_validate_item_nvgre(const struct rte_flow_item *item,
  ...
  ret = mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask, (const uint8_t *)&rte_flow_item_nvgre_mask,
      sizeof(struct rte_flow_item_nvgre), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
  if (ret < 0)
    return ret;
  return 0;
}
...
int mlx5_flow_validate_item_ecpri(const struct rte_flow_item *item,
  ...
  const uint64_t outer_l2_vlan =
      (MLX5_FLOW_LAYER_OUTER_L2 | MLX5_FLOW_LAYER_OUTER_VLAN);
  struct rte_flow_item_ecpri mask_lo;

  if (!(last_item & outer_l2_vlan) && last_item != MLX5_FLOW_LAYER_OUTER_L4_UDP)
    return rte_flow_error_set(
        error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
  ...
  return mlx5_flow_item_acceptable(
      item, (const uint8_t *)mask,
      acc_mask ? (const uint8_t *)acc_mask : (const uint8_t *)&nic_mask,
      sizeof(struct rte_flow_item_ecpri), MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
}
...
static int flow_null_apply(struct rte_eth_dev *dev __rte_unused,
...
static void flow_null_remove(struct rte_eth_dev *dev __rte_unused,
                             struct rte_flow *flow __rte_unused) {}

static void flow_null_destroy(struct rte_eth_dev *dev __rte_unused,
                              struct rte_flow *flow __rte_unused) {}
...
static int flow_null_sync_domain(struct rte_eth_dev *dev __rte_unused,
...
flow_get_drv_type(struct rte_eth_dev *dev, const struct rte_flow_attr *attr) {
  ...
  if (!attr->transfer)
    type =
        priv->sh->config.dv_flow_en ? MLX5_FLOW_TYPE_DV : MLX5_FLOW_TYPE_VERBS;
  return type;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static inline int flow_drv_validate(struct rte_eth_dev *dev,
                                    const struct rte_flow_attr *attr,
  ...
  return fops->validate(dev, attr, items, actions, external, hairpin, error);
}
...
flow_drv_prepare(struct rte_eth_dev *dev, const struct rte_flow *flow,
                 const struct rte_flow_attr *attr,
                 const struct rte_flow_item items[],
                 const struct rte_flow_action actions[], uint32_t flow_idx,
                 struct rte_flow_error *error) {
  const struct mlx5_flow_driver_ops *fops;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static inline int flow_drv_translate(struct rte_eth_dev *dev,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static inline int flow_drv_apply(struct rte_eth_dev *dev, struct rte_flow *flow,
                                 struct rte_flow_error *error) {
  const struct mlx5_flow_driver_ops *fops;
  enum mlx5_flow_drv_type type = flow->drv_type;
 ...
 *   Pointer to flow structure.
 */
static inline void flow_drv_destroy(struct rte_eth_dev *dev,
                                    struct rte_flow *flow) {
  const struct mlx5_flow_driver_ops *fops;
  enum mlx5_flow_drv_type type = flow->drv_type;
 ...
 *   Pointer to rss_desc
 */
static struct mlx5_flow_meter_sub_policy *flow_drv_meter_sub_policy_rss_prepare(
    struct rte_eth_dev *dev, struct rte_flow *flow,
 ...
 *   Pointer to error structure.
 */
static int flow_drv_mtr_hierarchy_rule_create(struct rte_eth_dev *dev,
                                              struct rte_flow *flow,
  ...
  MLX5_ASSERT(type > MLX5_FLOW_TYPE_MIN && type < MLX5_FLOW_TYPE_MAX);
  fops = flow_get_drv_ops(type);
  return fops->meter_hierarchy_rule_create(dev, fm, src_port, item, error);
}
    ...
    case RTE_FLOW_ACTION_TYPE_METER: {
      ...
      if (fm && !fm->def_policy) {
        ...
        if (policy->is_hierarchy) {
          policy = mlx5_flow_meter_hierarchy_get_final_policy(dev, policy);
          if (!policy)
            return NULL;
        }
        if (policy->is_rss) {
          acg = &policy->act_cnt[RTE_COLOR_GREEN];
          acy = &policy->act_cnt[RTE_COLOR_YELLOW];
          if (acg->fate_action == MLX5_FLOW_FATE_SHARED_RSS)
            rss = acg->rss->conf;
          else if (acy->fate_action == MLX5_FLOW_FATE_SHARED_RSS)
            rss = acy->rss->conf;
        }
      }
      break;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_action_handles_translate(
    struct rte_eth_dev *dev, const struct rte_flow_action actions[],
    struct mlx5_translated_action_handle *handle, int *indir_n,
    struct rte_flow_action **translated_actions, struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  for (n = 0; actions[n].type != RTE_FLOW_ACTION_TYPE_END; n++) {
    if (actions[n].type != RTE_FLOW_ACTION_TYPE_INDIRECT)
      continue;
    if (copied_n == *indir_n) {
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_NUM,
  ...
  *indir_n = copied_n;
  ...
  for (handle_end = handle + copied_n; handle < handle_end; handle++) {
    ...
    case MLX5_INDIRECT_ACTION_TYPE_RSS:
      ...
      translated[handle->index].type = RTE_FLOW_ACTION_TYPE_RSS;
      translated[handle->index].conf = &shared_rss->origin;
      break;
    ...
    case MLX5_INDIRECT_ACTION_TYPE_AGE:
      if (priv->sh->flow_hit_aso_en) {
        translated[handle->index].type =
            (enum rte_flow_action_type)MLX5_RTE_FLOW_ACTION_TYPE_AGE;
        translated[handle->index].conf = (void *)(uintptr_t)idx;
        break;
      }
      /* Fall-through */
    case MLX5_INDIRECT_ACTION_TYPE_CT:
      if (priv->sh->ct_aso_en) {
        translated[handle->index].type = RTE_FLOW_ACTION_TYPE_CONNTRACK;
        translated[handle->index].conf = (void *)(uintptr_t)idx;
        break;
      }
      /* Fall-through */
...
static unsigned int find_graph_root(uint32_t rss_level) {
  return rss_level < 2 ? MLX5_EXPANSION_ROOT : MLX5_EXPANSION_ROOT_OUTER;
}
 ...
 */
static inline uint64_t flow_get_prefix_layer_flags(struct mlx5_flow *dev_flow) {
  uint64_t layers = 0;
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_RAW_ENCAP:
      ...
      if (raw_encap->size > MLX5_ENCAPSULATION_DECISION_SIZE)
        *encap_idx = raw_decap_idx != -1 ? raw_decap_idx : actions_n;
      break;
 ...
 *   Total number of actions.
 */
static int flow_check_meter_action(struct rte_eth_dev *dev,
                                   const struct rte_flow_action actions[],
                                   bool *has_mtr, bool *has_modify,
                                   uint32_t *meter_id) {
  ...
  *has_mtr = false;
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    if (!*has_mtr)
      *has_modify |= flow_check_modify_action_type(dev, actions->type);
    actions_n++;
  }
  /* Count RTE_FLOW_ACTION_TYPE_END. */
  return actions_n + 1;
 ...
 */
static int flow_check_hairpin_split(struct rte_eth_dev *dev,
                                    const struct rte_flow_attr *attr,
                                    const struct rte_flow_action actions[]) {
  int queue_action = 0;
  int action_n = 0;
...
static uint32_t flow_list_create(struct rte_eth_dev *dev,
...
static void flow_list_destroy(struct rte_eth_dev *dev, enum mlx5_flow_type type,
                              uint32_t flow_idx);

int flow_dv_mreg_match_cb(void *tool_ctx __rte_unused,
...
struct mlx5_list_entry *flow_dv_mreg_create_cb(void *tool_ctx, void *cb_ctx) {
  ...
  if (mark_id != MLX5_DEFAULT_COPY_ID) {
    ...
    actions[0] = (struct rte_flow_action){
        .type = (enum rte_flow_action_type)MLX5_RTE_FLOW_ACTION_TYPE_MARK,
        .conf = &ftag,
    };
   ...
   * by list traversing.
   */
  mcp_res->rix_flow = flow_list_create(dev, MLX5_FLOW_TYPE_MCP, &attr, items,
                                       actions, false, error);
  if (!mcp_res->rix_flow) {
 ...
 */
static void flow_mreg_del_copy_action(struct rte_eth_dev *dev,
                                      struct rte_flow *flow) {
  struct mlx5_flow_mreg_copy_resource *mcp_res;
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (!flow->rix_mreg_copy)
    return;
  mcp_res =
      mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_MCP], flow->rix_mreg_copy);
 ...
 *   Pointer to Ethernet device.
 */
static void flow_mreg_del_default_copy_action(struct rte_eth_dev *dev) {
  struct mlx5_list_entry *entry;
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_cb_ctx ctx;
  uint32_t mark_id;
 ...
 */
static int flow_mreg_add_default_copy_action(struct rte_eth_dev *dev,
                                             struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_mreg_copy_resource *mcp_res;
  struct mlx5_flow_cb_ctx ctx;
  ...
  if (!priv->sh->config.dv_flow_en ||
      ...
      !mlx5_flow_ext_mreg_supported(dev) || !priv->sh->dv_regc0_mask)
    return 0;
  /*
 ...
 */
static int flow_mreg_update_copy_table(struct rte_eth_dev *dev,
                                       struct rte_flow *flow,
                                       const struct rte_flow_action *actions,
                                       struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  /* Check whether extensive metadata feature is engaged. */
  if (!config->dv_flow_en || config->dv_xmeta_en == MLX5_XMETA_MODE_LEGACY ||
      !mlx5_flow_ext_mreg_supported(dev) || !priv->sh->dv_regc0_mask)
    return 0;
  /* Find MARK action. */
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    switch (actions->type) {
    case RTE_FLOW_ACTION_TYPE_FLAG:
      mcp_res = flow_mreg_add_copy_action(dev, MLX5_FLOW_MARK_DEFAULT, error);
      if (!mcp_res)
        return -rte_errno;
    ...
    case RTE_FLOW_ACTION_TYPE_MARK:
      ...
      mcp_res = flow_mreg_add_copy_action(dev, mark->id, error);
      if (!mcp_res)
        return -rte_errno;
      flow->rix_mreg_copy = mcp_res->idx;
      return 0;
 ...
 *   0 on success.
 */
static int flow_hairpin_split(struct rte_eth_dev *dev,
                              const struct rte_flow_action actions[],
  ...
  tag_action->type = (enum rte_flow_action_type)MLX5_RTE_FLOW_ACTION_TYPE_TAG;
  actions_rx++;
  rte_memcpy(actions_rx, actions, sizeof(struct rte_flow_action));
  actions_rx++;
  ...
  *set_tag = (struct mlx5_rte_flow_action_set_tag){
  ...
  item = pattern_tx;
  item->type = (enum rte_flow_item_type)MLX5_RTE_FLOW_ITEM_TYPE_TAG;
  tag_item = (void *)addr;
 ...
 */
static int flow_create_split_inner(struct rte_eth_dev *dev,
                                   struct rte_flow *flow,
  ...
  SILIST_INSERT(&flow->dev_handles, dev_flow->handle_idx, dev_flow->handle,
                next);
  /*
 ...
 */
static struct mlx5_flow_meter_sub_policy *get_meter_sub_policy(
    struct rte_eth_dev *dev, struct rte_flow *flow,
    struct mlx5_flow_workspace *wks, const struct rte_flow_attr *attr,
    const struct rte_flow_item items[], struct rte_flow_error *error) {
  struct mlx5_flow_meter_policy *policy;
  ...
  if (final_policy->is_rss || final_policy->is_queue) {
    ...
    for (i = 0; i < MLX5_MTR_RTE_COLORS; i++) {
      ...
      if (fate == MLX5_FLOW_FATE_SHARED_RSS) {
         ...
         */
        if (flow_drv_translate(dev, &dev_flow, attr, items, rss_actions, error))
          goto exit;
        ...
        rss_desc_v[i].queue_num =
            rss_desc_v[i].hash_fields ? rss_desc_v[i].queue_num : 1;
        rss_desc_v[i].tunnel =
            !!(dev_flow.handle->layers & MLX5_FLOW_LAYER_TUNNEL);
        /* Use the RSS queues in the containers. */
        rss_desc_v[i].queue = (uint16_t *)(uintptr_t)rss_act->queue;
        ...
        rss_desc_v[i].queue = &final_policy->act_cnt[i].queue;
    ...
    sub_policy =
        flow_drv_meter_sub_policy_rss_prepare(dev, flow, policy, rss_desc);
  } else {
    enum mlx5_meter_domain mtr_domain =
        attr->transfer
            ? MLX5_MTR_DOMAIN_TRANSFER
            : (attr->egress ? MLX5_MTR_DOMAIN_EGRESS : MLX5_MTR_DOMAIN_INGRESS);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_meter_split_prep(
    struct rte_eth_dev *dev, struct rte_flow *flow,
    struct mlx5_flow_workspace *wks, const struct rte_flow_attr *attr,
    const struct rte_flow_item items[], struct rte_flow_item sfx_items[],
    const struct rte_flow_action actions[],
    struct rte_flow_action actions_sfx[], struct rte_flow_action actions_pre[],
    uint32_t *mtr_flow_id, struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  uint8_t mtr_reg_bits =
      priv->mtr_reg_share ? MLX5_MTR_IDLE_BITS_IN_COLOR_REG : MLX5_REG_BITS;
  uint32_t flow_id = 0;
    ...
    case RTE_FLOW_ITEM_TYPE_PORT_ID:
      ...
      if (!port_priv)
        return rte_flow_error_set(error, rte_errno,
                                  RTE_FLOW_ERROR_TYPE_ITEM_SPEC, pid_v,
      ...
      if (!fm->def_policy && wks->policy->is_hierarchy &&
        ...
        if (flow_drv_mtr_hierarchy_rule_create(dev, flow, fm, flow_src_port,
                                               items, error))
          return -rte_errno;
      }
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_VID:
      if (vlan_item_dst && vlan_item_src) {
        memcpy(vlan_item_dst, vlan_item_src, sizeof(*vlan_item_dst));
        /*
         * Convert to internal match item, it is used
         * for vlan push and set vid.
         */
        vlan_item_dst->type =
            (enum rte_flow_item_type)MLX5_RTE_FLOW_ITEM_TYPE_VLAN;
      }
      break;
    default:
      break;
    ...
    if (!action_cur)
      action_cur = (fm->def_policy) ? actions_sfx++ : actions_pre++;
  ...
  if (priv->sh->meter_aso_en) {
    ...
    if (!fm->def_policy) {
      sub_policy =
          get_meter_sub_policy(dev, flow, wks, attr, orig_items, error);
      if (!sub_policy)
        return -rte_errno;
    } else {
      enum mlx5_meter_domain mtr_domain =
      ...
      sub_policy = &priv->sh->mtrmng->def_policy[mtr_domain]->sub_policy;
    }
    tbl_data =
        container_of(sub_policy->tbl_rsc, struct mlx5_flow_tbl_data_entry, tbl);
    hw_mtr_action = actions_pre++;
    hw_mtr_action->type =
        (enum rte_flow_action_type)MLX5_RTE_FLOW_ACTION_TYPE_JUMP;
  ...
  if (fm->def_policy) {
    ...
    if (!tag_id)
      return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Failed to allocate meter flow id.");
    flow_id = tag_id - 1;
    flow_id_bits = (!flow_id) ? 1 : (MLX5_REG_BITS - __builtin_clz(flow_id));
    if ((flow_id_bits + priv->sh->mtrmng->max_mtr_bits) > mtr_reg_bits) {
      mlx5_ipool_free(fm->flow_ipool, tag_id);
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Meter flow id exceeds max limit.");
    }
  ...
  *set_tag = (struct mlx5_rte_flow_action_set_tag){
      .id = (enum modify_reg)mlx5_flow_get_reg_id(dev, MLX5_MTR_ID, 0, error),
  ...
  for (shift = 0; shift < flow_id_bits; shift++)
    flow_id_reversed = (flow_id_reversed << 1) | ((flow_id >> shift) & 0x1);
 ...
 */
static uint32_t flow_mreg_split_qrss_prep(struct rte_eth_dev *dev,
 ...
 *   0 on success, negative value otherwise
 */
static int flow_mreg_tx_copy_prep(struct rte_eth_dev *dev,
                                  struct rte_flow_action *ext_actions,
                                  const struct rte_flow_action *actions,
                                  int actions_n, struct rte_flow_error *error,
                                  int encap_idx) {
  struct mlx5_flow_action_copy_mreg *cp_mreg =
      (struct mlx5_flow_action_copy_mreg *)(ext_actions + actions_n + 1);
  int ret;
 ...
 */
static int flow_check_match_action(const struct rte_flow_action actions[],
                                   const struct rte_flow_attr *attr,
                                   enum rte_flow_action_type action,
  ...
  *qrss_action_pos = -1;
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_SAMPLE:
      sample = actions->conf;
      ratio = sample->ratio;
      sub_type = ((const struct rte_flow_action *)(sample->actions))->type;
    ...
    case RTE_FLOW_ACTION_TYPE_RAW_DECAP:
      ...
      if (actions->type == RTE_FLOW_ACTION_TYPE_RAW_ENCAP) {
        const struct rte_flow_action_raw_encap *encap = actions->conf;
 ...
 */
static int flow_sample_split_prep(struct rte_eth_dev *dev, int add_tag,
                                  struct rte_flow_item sfx_items[],
                                  const struct rte_flow_action actions[],
                                  struct rte_flow_action actions_sfx[],
                                  struct rte_flow_action actions_pre[],
                                  int actions_n, int sample_action_pos,
                                  int qrss_action_pos, int jump_table,
                                  struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_rte_flow_action_set_tag *set_tag;
  struct mlx5_rte_flow_item_tag *tag_spec;
  ...
  if (qrss_action_pos >= 0 && qrss_action_pos < sample_action_pos) {
    ...
    if (index != 0)
      memcpy(actions_pre, actions, sizeof(struct rte_flow_action) * index);
    ...
    if (sample_action_pos > index + 1)
      memcpy(actions_pre + index, actions + index + 1,
             sizeof(struct rte_flow_action) * (sample_action_pos - index - 1));
  ...
  if (add_tag) {
    ...
    *set_tag = (struct mlx5_rte_flow_action_set_tag){
    ...
    sfx_items[1] = (struct rte_flow_item){
        .type = (enum rte_flow_item_type)RTE_FLOW_ITEM_TYPE_END,
    };
  ...
  if (jump_table) {
    ...
    actions_pre[index++] = (struct rte_flow_action){
        .type = (enum rte_flow_action_type)RTE_FLOW_ACTION_TYPE_JUMP,
        .conf = jump_action,
    };
  }
  actions_pre[index] = (struct rte_flow_action){
      .type = (enum rte_flow_action_type)RTE_FLOW_ACTION_TYPE_END,
  };
  /* Put the actions after sample into Suffix flow. */
  memcpy(actions_sfx, actions + sample_action_pos + 1,
         sizeof(struct rte_flow_action) * (actions_n - sample_action_pos - 1));
  return tag_id;
}
 ...
 */
static int
flow_create_split_metadata(struct rte_eth_dev *dev, struct rte_flow *flow,
                           const struct rte_flow_attr *attr,
                           const struct rte_flow_item items[],
                           const struct rte_flow_action actions[],
                           struct mlx5_flow_split_info *flow_split_info,
  ...
  if (!config->dv_flow_en || config->dv_xmeta_en == MLX5_XMETA_MODE_LEGACY ||
    ...
    return flow_create_split_inner(dev, flow, NULL, attr, items, actions,
                                   flow_split_info, error);
  actions_n =
      flow_parse_metadata_split_actions_info(actions, &qrss, &encap_idx);
  if (qrss) {
  ...
  if (qrss) {
    /* Check if it is in meter suffix table. */
    mtr_sfx = attr->group == (attr->transfer ? (MLX5_FLOW_TABLE_LEVEL_METER - 1)
    ...
    ext_actions = mlx5_malloc(MLX5_MEM_ZERO, act_size, 0, SOCKET_ID_ANY);
    if (!ext_actions)
      return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                                "no memory to split "
                                "metadata flow");
     ...
     */
    qrss_id = flow_mreg_split_qrss_prep(dev, ext_actions, actions, qrss,
                                        actions_n, mtr_sfx, error);
    if (!mtr_sfx && !qrss_id) {
      ret = -rte_errno;
      goto exit;
    }
    ...
    ret = flow_mreg_tx_copy_prep(dev, ext_actions, actions, actions_n, error,
                                 encap_idx);
    if (ret < 0)
      goto exit;
  }
  ...
  if (qrss) {
    ...
    ret = flow_create_split_inner(dev, flow, &dev_flow, &q_attr,
                                  mtr_sfx ? items : q_items, q_actions,
                                  flow_split_info, error);
    if (ret < 0)
      goto exit;
...
exit:
   ...
   * by flow_drv_destroy.
   */
  mlx5_ipool_free(priv->sh->ipool[MLX5_IPOOL_RSS_EXPANTION_FLOW_ID], qrss_id);
 ...
 *   0 on success, negative value otherwise
 */
static uint32_t flow_meter_create_drop_flow_with_org_pattern(
    struct rte_eth_dev *dev, struct rte_flow *flow,
    const struct rte_flow_attr *attr, const struct rte_flow_item items[],
    struct mlx5_flow_split_info *flow_split_info,
    struct mlx5_flow_meter_info *fm, struct rte_flow_error *error) {
  struct mlx5_flow *dev_flow = NULL;
  ...
  return flow_create_split_inner(dev, flow, &dev_flow, &drop_attr, items,
                                 drop_actions, &drop_split_info, error);
}
 ...
 *   0 on success, negative value otherwise
 */
static int flow_create_split_meter(struct rte_eth_dev *dev,
                                   struct rte_flow *flow,
                                   const struct rte_flow_attr *attr,
                                   const struct rte_flow_item items[],
                                   const struct rte_flow_action actions[],
  ...
  if (priv->mtr_en)
    actions_n =
        flow_check_meter_action(dev, actions, &has_mtr, &has_modify, &meter_id);
  if (has_mtr) {
    if (flow->meter) {
      ...
      if (!fm)
        return rte_flow_error_set(error, EINVAL,
                                  RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                                  "Meter not found.");
      ret = mlx5_flow_meter_attach(priv, fm, &sfx_attr, error);
      if (ret)
        return -rte_errno;
      flow->meter = mtr_idx;
    }
    ...
    wks->fm = fm;
    if (!fm->def_policy) {
      wks->policy = mlx5_flow_meter_policy_find(dev, fm->policy_id, NULL);
      MLX5_ASSERT(wks->policy);
      if (wks->policy->mark)
        wks->mark = 1;
      if (wks->policy->is_hierarchy) {
        wks->final_policy =
            mlx5_flow_meter_hierarchy_get_final_policy(dev, wks->policy);
        if (!wks->final_policy)
          return rte_flow_error_set(
     ...
     */
    if (!fm->def_policy && !is_mtr_hierarchy && (!has_modify || !fm->drop_cnt))
      set_mtr_reg = false;
...
#define METER_SUFFIX_ITEM 4
    ...
    sfx_actions =
        mlx5_malloc(MLX5_MEM_ZERO, (act_size + item_size), 0, SOCKET_ID_ANY);
    if (!sfx_actions)
      return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                                "no memory to split "
                                "meter flow");
    sfx_items = (struct rte_flow_item *)((char *)sfx_actions + act_size);
    /* There's no suffix flow for meter of non-default policy. */
    if (!fm->def_policy)
      pre_actions = sfx_actions + 1;
    else
      pre_actions = sfx_actions + actions_n;
    ret = flow_meter_split_prep(dev, flow, wks, &sfx_attr, items, sfx_items,
                                actions, sfx_actions, pre_actions,
                                (set_mtr_reg ? &mtr_flow_id : NULL), error);
    if (ret) {
      ret = -rte_errno;
      goto exit;
    }
    ...
    flow_split_info->skip_scale |= 1 << MLX5_SCALE_JUMP_FLOW_GROUP_BIT;
    ret = flow_create_split_inner(dev, flow, &dev_flow, attr, items,
                                  pre_actions, flow_split_info, error);
    ...
    if (!fm->def_policy) {
      if (!set_mtr_reg && fm->drop_cnt)
        ret = flow_meter_create_drop_flow_with_org_pattern(
            dev, flow, &sfx_attr, items, flow_split_info, fm, error);
      goto exit;
    }
    /* Setting the sfx group atrr. */
    sfx_attr.group = sfx_attr.transfer ? (MLX5_FLOW_TABLE_LEVEL_METER - 1)
                                       : MLX5_FLOW_TABLE_LEVEL_METER;
    flow_split_info->prefix_layers = flow_get_prefix_layer_flags(dev_flow);
    flow_split_info->prefix_mark |= wks->mark;
    flow_split_info->table_id = MLX5_MTR_TABLE_ID_SUFFIX;
  }
  /* Add the prefix subflow. */
  ret = flow_create_split_metadata(
      dev, flow, &sfx_attr, sfx_items ? sfx_items : items,
      sfx_actions ? sfx_actions : actions, flow_split_info, error);
exit:
  if (sfx_actions)
 ...
 */
static int
flow_create_split_sample(struct rte_eth_dev *dev, struct rte_flow *flow,
                         const struct rte_flow_attr *attr,
                         const struct rte_flow_item items[],
                         const struct rte_flow_action actions[],
                         struct mlx5_flow_split_info *flow_split_info,
  ...
  if (priv->sampler_en)
    actions_n = flow_check_match_action(
        actions, attr, RTE_FLOW_ACTION_TYPE_SAMPLE, &sample_action_pos,
        &qrss_action_pos, &modify_after_mirror);
  if (actions_n) {
     ...
     */
    add_tag = (!fdb_tx || priv->sh->cdev->config.hca_attr.reg_c_preserve);
    if (add_tag)
      sfx_items = (struct rte_flow_item *)((char *)sfx_actions + act_size);
    if (modify_after_mirror)
      jump_table = attr->group * MLX5_FLOW_TABLE_FACTOR + next_ft_step;
    pre_actions = sfx_actions + actions_n;
    tag_id = flow_sample_split_prep(
        dev, add_tag, sfx_items, actions, sfx_actions, pre_actions, actions_n,
        sample_action_pos, qrss_action_pos, jump_table, error);
    ...
    if (!modify_after_mirror) {
      ...
      sample_res =
          (struct mlx5_flow_dv_sample_resource *)dev_flow->dv.sample_res;
      sfx_tbl = (struct mlx5_flow_tbl_resource *)sample_res->normal_path_tbl;
      sfx_tbl_data =
          container_of(sfx_tbl, struct mlx5_flow_tbl_data_entry, tbl);
      sfx_attr.group =
          sfx_attr.transfer ? (sfx_tbl_data->level - 1) : sfx_tbl_data->level;
    } else {
 ...
 *   0 on success, negative value otherwise
 */
static int flow_create_split_outer(struct rte_eth_dev *dev,
                                   struct rte_flow *flow,
                                   const struct rte_flow_attr *attr,
                                   const struct rte_flow_item items[],
                                   const struct rte_flow_action actions[],
  ...
  ret = flow_create_split_sample(dev, flow, attr, items, actions,
                                 flow_split_info, error);
  MLX5_ASSERT(ret <= 0);
  return ret;
}
 ...
 */
static int flow_rss_workspace_adjust(struct mlx5_flow_workspace *wks,
...
flow_list_create(struct rte_eth_dev *dev, enum mlx5_flow_type type,
                 const struct rte_flow_attr *attr,
                 const struct rte_flow_item items[],
                 const struct rte_flow_action original_actions[], bool external,
                 struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct rte_flow *flow = NULL;
  struct mlx5_flow *dev_flow;
  const struct rte_flow_action_rss *rss = NULL;
  struct mlx5_translated_action_handle indir_actions[MLX5_MAX_INDIRECT_ACTIONS];
  ...
  struct mlx5_flow_tunnel *tunnel;
  struct tunnel_default_miss_ctx default_miss_ctx = {
      0,
  };
  struct mlx5_flow_workspace *wks = mlx5_flow_push_thread_workspace();
  struct mlx5_flow_split_info flow_split_info = {.external = !!external,
                                                 .skip_scale = 0,
                                                 .flow_idx = 0,
                                                 .prefix_mark = 0,
                                                 .prefix_layers = 0,
  ...
  ret = flow_action_handles_translate(dev, original_actions, indir_actions,
                                      &indir_actions_n, &translated_actions,
                                      error);
  if (ret < 0) {
  ...
  if (hairpin_flow > 0) {
    ...
    flow_hairpin_split(dev, actions, actions_rx.actions,
                       actions_hairpin_tx.actions, items_tx.items, idx);
  ...
  if (rss && rss->types) {
    ...
    ret = mlx5_flow_expand_rss(buf, sizeof(expand_buffer.buffer), items,
                               rss->types, mlx5_support_expansion, graph_root);
    MLX5_ASSERT(ret > 0 && (unsigned int)ret < sizeof(expand_buffer.buffer));
  ...
  rss_desc->shared_rss =
      flow_get_shared_rss_action(dev, indir_actions, indir_actions_n);
  for (i = 0; i < buf->entries; ++i) {
     ...
     */
    ret = flow_create_split_outer(dev, flow, attr, buf->entry[i].pattern,
                                  p_actions_rx, &flow_split_info, error);
    if (ret < 0)
      goto error;
    if (is_flow_tunnel_steer_rule(wks->flows[0].tof_type)) {
      ret = flow_tunnel_add_default_miss(dev, flow, attr, p_actions_rx, idx,
                                         wks->flows[0].tunnel,
                                         &default_miss_ctx, error);
      if (ret < 0) {
  ...
  if (hairpin_flow) {
    ...
    dev_flow = flow_drv_prepare(dev, flow, &attr_tx, items_tx.items,
                                actions_hairpin_tx.actions, idx, error);
    if (!dev_flow)
      goto error;
    dev_flow->flow = flow;
    dev_flow->external = 0;
    SILIST_INSERT(&flow->dev_handles, dev_flow->handle_idx, dev_flow->handle,
                  next);
    ret = flow_drv_translate(dev, dev_flow, &attr_tx, items_tx.items,
...
error:
  ...
  if (rss_desc->shared_rss)
    __atomic_sub_fetch(&((struct mlx5_shared_action_rss *)mlx5_ipool_get(
...
struct rte_flow *mlx5_flow_create_esw_table_zero_flow(struct rte_eth_dev *dev) {
  ...
  return (void *)(uintptr_t)flow_list_create(dev, MLX5_FLOW_TYPE_CTL, &attr,
                                             &pattern, actions, false, &error);
}
...
uint32_t mlx5_flow_create_devx_sq_miss_flow(struct rte_eth_dev *dev,
  ...
  struct rte_flow_item pattern[] = {
      ...
      {
          .type = (enum rte_flow_item_type)MLX5_RTE_FLOW_ITEM_TYPE_TX_QUEUE,
          .spec = &txq_spec,
      },
      {
          .type = RTE_FLOW_ITEM_TYPE_END,
   ...
   */
  if (flow_list_create(dev, MLX5_FLOW_TYPE_CTL, &attr, pattern, actions, false,
                       &error) == 0)
    return 0;
  ...
  return flow_list_create(dev, MLX5_FLOW_TYPE_CTL, &attr, pattern, actions,
                          false, &error);
}
...
int mlx5_flow_validate(struct rte_eth_dev *dev,
  ...
  const struct rte_flow_action *actions;
  struct rte_flow_action *translated_actions = NULL;
  int ret = flow_action_handles_translate(dev, original_actions, indir_actions,
                                          &indir_actions_n, &translated_actions,
                                          error);
  ...
  actions = translated_actions ? translated_actions : original_actions;
  hairpin_flow = flow_check_hairpin_split(dev, attr, actions);
  ret = flow_drv_validate(dev, attr, items, actions, true, hairpin_flow, error);
  rte_free(translated_actions);
  return ret;
}
...
struct rte_flow *mlx5_flow_create(struct rte_eth_dev *dev,
  ...
  return (void *)(uintptr_t)flow_list_create(dev, MLX5_FLOW_TYPE_GEN, attr,
                                             items, actions, true, error);
}
...
#ifdef HAVE_IBV_FLOW_DV_SUPPORT
  if (priv->sh->config.dv_flow_en == 2 && type == MLX5_FLOW_TYPE_GEN) {
 ...
 */
static struct mlx5_flow_workspace *flow_alloc_thread_workspace(void) {
  ...
  data->rss_desc.queue = calloc(1, sizeof(uint16_t) * MLX5_RSSQ_DEFAULT_NUM);
 ...
 */
int mlx5_flow_verify(struct rte_eth_dev *dev __rte_unused) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct rte_flow *flow;
  uint32_t idx = 0;
  int ret = 0, i;
  ...
  for (i = 0; i < MLX5_FLOW_TYPE_MAXI; i++) {
    MLX5_IPOOL_FOREACH(priv->flows[i], idx, flow) {
      DRV_LOG(DEBUG, "port %u flow %p still referenced", dev->data->port_id,
              (void *)flow);
      ret++;
    }
  }
...
int mlx5_ctrl_flow_source_queue(struct rte_eth_dev *dev, uint32_t queue) {
  ...
  flow_idx = flow_list_create(dev, MLX5_FLOW_TYPE_CTL, &attr, items, actions,
                              false, &error);
  if (!flow_idx) {
    DRV_LOG(DEBUG,
...
int mlx5_ctrl_flow_vlan(struct rte_eth_dev *dev,
  ...
  struct rte_flow_item items[] = {
      ...
      {
          .type =
              (vlan_spec) ? RTE_FLOW_ITEM_TYPE_VLAN : RTE_FLOW_ITEM_TYPE_END,
          .spec = vlan_spec,
          .last = NULL,
          .mask = vlan_mask,
      },
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_ctrl_flow(struct rte_eth_dev *dev, struct rte_flow_item_eth *eth_spec,
                   struct rte_flow_item_eth *eth_mask) {
...
int mlx5_flow_lacp_miss(struct rte_eth_dev *dev) {
  ...
  struct rte_flow_action actions[] = {
      {
          .type =
              (enum rte_flow_action_type)MLX5_RTE_FLOW_ACTION_TYPE_DEFAULT_MISS,
      },
      {
          .type = RTE_FLOW_ACTION_TYPE_END,
      },
  ...
  struct rte_flow_error error;
  uint32_t flow_idx = flow_list_create(dev, MLX5_FLOW_TYPE_CTL, &attr, items,
                                       actions, false, &error);
...
int mlx5_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  if (priv->sh->config.dv_flow_en == 2)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Flow non-Q destruction not supported");
  flow_list_destroy(dev, MLX5_FLOW_TYPE_GEN, (uintptr_t)(void *)flow);
  return 0;
}
 ...
 * @see rte_flow_ops
 */
int mlx5_flow_flush(struct rte_eth_dev *dev,
                    struct rte_flow_error *error __rte_unused) {
...
int mlx5_flow_isolate(struct rte_eth_dev *dev, int enable,
  ...
  if (dev->data->dev_started) {
    rte_flow_error_set(error, EBUSY, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
 ...
 * @see rte_flow_ops
 */
static int flow_drv_query(struct rte_eth_dev *dev, uint32_t flow_idx,
                          const struct rte_flow_action *actions, void *data,
                          struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  const struct mlx5_flow_driver_ops *fops;
  struct rte_flow *flow =
      mlx5_ipool_get(priv->flows[MLX5_FLOW_TYPE_GEN], flow_idx);
  enum mlx5_flow_drv_type ftype;

  if (!flow) {
    return rte_flow_error_set(error, ENOENT, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "invalid flow handle");
  }
  ftype = flow->drv_type;
...
int mlx5_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  if (priv->sh->config.dv_flow_en == 2)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Flow non-Q query not supported");
  ret = flow_drv_query(dev, (uintptr_t)(void *)flow, actions, data, error);
  if (ret < 0)
    return ret;
  return 0;
}
 ...
 */
int mlx5_flow_validate_mtr_acts(
  ...
  return fops->validate_mtr_acts(dev, actions, attr, is_rss, domain_bitmap,
                                 policy_mode, error);
}
...
int mlx5_flow_create_mtr_tbls(struct rte_eth_dev *dev,
                              struct mlx5_flow_meter_info *fm, uint32_t mtr_idx,
 ...
 *   Pointer to meter policy table.
 */
void mlx5_flow_destroy_sub_policy_with_rxq(
...
void mlx5_counter_free(struct rte_eth_dev *dev, uint32_t cnt) {
  ...
  DRV_LOG(ERR, "port %u counter free is not supported.", dev->data->port_id);
}
 ...
 */
int mlx5_counter_query(struct rte_eth_dev *dev, uint32_t cnt, bool clear,
  ...
  if (flow_get_drv_type(dev, &attr) == MLX5_FLOW_TYPE_DV) {
    fops = flow_get_drv_ops(MLX5_FLOW_TYPE_DV);
    return fops->counter_query(dev, cnt, clear, pkts, bytes, action);
  }
  DRV_LOG(ERR, "port %u counter query is not supported.", dev->data->port_id);
  return -ENOTSUP;
}
...
static int mlx5_flow_info_get(struct rte_eth_dev *dev,
  ...
  if (flow_get_drv_type(dev, NULL) != MLX5_FLOW_TYPE_HW)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "info get with incorrect steering mode");
...
static struct rte_flow_template_table *mlx5_flow_table_create(
  ...
  fops = flow_get_drv_ops(MLX5_FLOW_TYPE_HW);
  return fops->template_table_create(dev, attr, item_templates,
                                     nb_item_templates, action_templates,
                                     nb_action_templates, error);
}
...
static struct rte_flow *mlx5_flow_async_flow_create(
  ...
  fops = flow_get_drv_ops(MLX5_FLOW_TYPE_HW);
  return fops->async_flow_create(dev, queue_id, attr, table, items,
                                 pattern_template_index, actions,
                                 action_template_index, user_data, error);
}
...
static int mlx5_flow_async_flow_destroy(struct rte_eth_dev *dev, uint32_t queue,
  ...
  fops = flow_get_drv_ops(MLX5_FLOW_TYPE_HW);
  return fops->async_flow_destroy(dev, queue, attr, flow, user_data, error);
}
...
static int mlx5_flow_pull(struct rte_eth_dev *dev, uint32_t queue,
  ...
  if (flow_get_drv_type(dev, NULL) != MLX5_FLOW_TYPE_HW)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "flow_q pull with incorrect steering mode");
  fops = flow_get_drv_ops(MLX5_FLOW_TYPE_HW);
...
static int mlx5_flow_push(struct rte_eth_dev *dev, uint32_t queue,
  ...
  if (flow_get_drv_type(dev, NULL) != MLX5_FLOW_TYPE_HW)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, "flow_q push with incorrect steering mode");
  fops = flow_get_drv_ops(MLX5_FLOW_TYPE_HW);
...
static struct rte_flow_action_handle *mlx5_flow_async_action_handle_create(
    ...
    const struct rte_flow_action *action, void *user_data,
    struct rte_flow_error *error) {
  const struct mlx5_flow_driver_ops *fops = flow_get_drv_ops(MLX5_FLOW_TYPE_HW);

  return fops->async_action_create(dev, queue, attr, conf, action, user_data,
                                   error);
}
...
static int mlx5_flow_async_action_handle_update(
  ...
  return fops->async_action_update(dev, queue, attr, handle, update, user_data,
                                   error);
}
...
static int mlx5_flow_async_action_handle_destroy(
  ...
  return fops->async_action_destroy(dev, queue, attr, handle, user_data, error);
}
...
mlx5_flow_create_counter_stat_mem_mng(struct mlx5_dev_ctx_shared *sh) {
  ...
  int size = (sizeof(struct flow_counter_stats) * MLX5_COUNTERS_PER_POOL +
  ...
  ret = mlx5_os_wrapped_mkey_create(sh->cdev->ctx, sh->cdev->pd, sh->cdev->pdn,
                                    mem, size, &mem_mng->wm);
  if (ret) {
    rte_errno = errno;
    mlx5_free(mem);
  ...
  for (i = 0; i < MLX5_MAX_PENDING_QUERIES; ++i)
    LIST_INSERT_HEAD(&sh->cmng.free_stat_raws,
                     mem_mng->raws + MLX5_CNT_CONTAINER_RESIZE + i, next);
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int mlx5_flow_set_counter_stat_mem(struct mlx5_dev_ctx_shared *sh,
  ...
  pool->raw = cmng->mem_mng->raws + pool->index % MLX5_CNT_CONTAINER_RESIZE;
...
void mlx5_flow_query_alarm(void *arg) {
  ...
  if (pool->raw_hw)
    ...
    goto set_alarm;
  pool->raw_hw = LIST_FIRST(&sh->cmng.free_stat_raws);
  if (!pool->raw_hw)
   ...
   */
  pool->query_gen++;
  ret = mlx5_devx_cmd_flow_counter_query(
      pool->min_dcs, 0, MLX5_COUNTERS_PER_POOL, NULL, NULL,
      pool->raw_hw->mem_mng->wm.lkey, (void *)(uintptr_t)pool->raw_hw->data,
      sh->devx_comp, (uint64_t)(uintptr_t)pool);
  if (ret) {
    DRV_LOG(ERR,
            "Failed to trigger asynchronous query for dcs ID"
            " %d",
            pool->min_dcs->id);
    pool->raw_hw = NULL;
    goto set_alarm;
 ...
 */
static void mlx5_flow_aging_check(struct mlx5_dev_ctx_shared *sh,
                                  struct mlx5_flow_counter_pool *pool) {
  ...
  for (i = 0; i < MLX5_COUNTERS_PER_POOL; ++i) {
    ...
    if (__atomic_load_n(&age_param->state, __ATOMIC_RELAXED) != AGE_CANDIDATE)
      continue;
    if (cur->data[i].hits != prev->data[i].hits) {
      __atomic_store_n(&age_param->sec_since_last_hit, 0, __ATOMIC_RELAXED);
      continue;
    }
    if (__atomic_add_fetch(&age_param->sec_since_last_hit, time_delta,
    ...
    if (__atomic_compare_exchange_n(&age_param->state, &expected, AGE_TMOUT,
...
void mlx5_flow_async_pool_query_handle(struct mlx5_dev_ctx_shared *sh,
  ...
  enum mlx5_counter_type cnt_type =
      pool->is_aged ? MLX5_COUNTER_TYPE_AGE : MLX5_COUNTER_TYPE_ORIGIN;
  ...
  if (unlikely(status)) {
    ...
    if (!TAILQ_EMPTY(&pool->counters[query_gen])) {
      ...
      TAILQ_CONCAT(&cmng->counters[cnt_type], &pool->counters[query_gen], next);
...
static int flow_group_to_table(uint32_t port_id, uint32_t group,
                               uint32_t *table,
...
int mlx5_flow_group_to_table(struct rte_eth_dev *dev,
  ...
  if (is_tunnel_offload_active(dev)) {
    standard_translation = !grp_info->external || grp_info->std_tbl_fix;
  } else {
  ...
  DRV_LOG(
      ...
      dev->data->port_id, group, grp_info->transfer, grp_info->external,
      grp_info->fdb_def_rule, standard_translation ? "STANDARD" : "TUNNEL");
  if (standard_translation)
    ret =
        flow_group_to_table(dev->data->port_id, group, table, grp_info, error);
  else
    ret = tunnel_flow_group_to_flow_table(dev, tunnel, group, table, error);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_flow_discover_mreg_c(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  enum modify_reg idx;
  int n = 0;
  ...
  for (idx = REG_C_2; idx <= REG_C_7; ++idx) {
    ...
    flow_idx = flow_list_create(dev, MLX5_FLOW_TYPE_GEN, &attr, items, actions,
                                false, &error);
    flow = mlx5_ipool_get(priv->flows[MLX5_FLOW_TYPE_GEN], flow_idx);
    if (!flow)
      continue;
...
int save_dump_file(const uint8_t *data, uint32_t size, uint32_t type,
                   uint64_t id, void *arg, FILE *file) {
  char line[BUF_SIZE];
  uint32_t out = 0;
  uint32_t k;
  ...
  case DR_DUMP_REC_TYPE_PMD_MODIFY_HDR:
    ...
    out += snprintf(line + out, BUF_SIZE - out, "%d,0x%" PRIx64 ",%d,", type,
                    id, actions_num);
    break;
  case DR_DUMP_REC_TYPE_PMD_PKT_REFORMAT:
    out += snprintf(line + out, BUF_SIZE - out, "%d,0x%" PRIx64 ",", type, id);
    break;
  case DR_DUMP_REC_TYPE_PMD_COUNTER:
    count = (struct rte_flow_query_count *)arg;
    fprintf(file, "%d,0x%" PRIx64 ",%" PRIu64 ",%" PRIu64 "\n", type, id,
  ...
  for (k = 0; k < size; k++) {
    ...
    out += snprintf(line + out, BUF_SIZE - out, "%02x", (data[k]) & 0xff);
  }
  fprintf(file, "%s\n", line);
  return 0;
}
...
int mlx5_flow_dev_dump_ipool(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  /* query counter */
  if (flow->counter &&
      (!mlx5_counter_query(dev, flow->counter, false, &count.hits, &count.bytes,
                           &action)) &&
      action) {
    id = (uint64_t)(uintptr_t)action;
    type = DR_DUMP_REC_TYPE_PMD_COUNTER;
    save_dump_file(NULL, 0, type, id, (void *)&count, file);
  }
  ...
  while (handle_idx) {
    ...
    if (dh->dvh.rix_encap_decap) {
      encap_decap = mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_DECAP_ENCAP],
    ...
    if (modify_hdr) {
      ...
      save_dump_file(data, size, type, id, (void *)(&actions_num), file);
    }
    if (encap_decap) {
      ...
      save_dump_file(data, size, type, id, NULL, file);
    }
  }
  return 0;
}
 ...
 */
static int
mlx5_flow_dev_dump_sh_all(struct rte_eth_dev *dev, FILE *file,
                          struct rte_flow_error *error __rte_unused) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_dev_ctx_shared *sh = priv->sh;
  struct mlx5_hlist *h;
  ...
  if (h) {
    for (j = 0; j <= h->mask; j++) {
      ...
      while (e) {
        encap_decap = (struct mlx5_flow_dv_encap_decap_resource *)e;
        data = encap_decap->buf;
        size = encap_decap->size;
        id = (uint64_t)(uintptr_t)encap_decap->action;
        type = DR_DUMP_REC_TYPE_PMD_PKT_REFORMAT;
  ...
  if (h) {
    ...
    for (j = 0; j <= h->mask; j++) {
      ...
      e = LIST_FIRST(&l_inconst->cache[i]->h);
      while (e) {
        modify_hdr = (struct mlx5_flow_dv_modify_hdr_resource *)e;
        data = (const uint8_t *)modify_hdr->actions;
        size = (size_t)(modify_hdr->actions_num) * 8;
        actions_num = modify_hdr->actions_num;
        id = (uint64_t)(uintptr_t)modify_hdr->action;
  ...
  for (j = 1; j <= max; j++) {
    action = NULL;
    if ((!mlx5_counter_query(dev, j, false, &count.hits, &count.bytes,
                             &action)) &&
        action) {
      id = (uint64_t)(uintptr_t)action;
      type = DR_DUMP_REC_TYPE_PMD_COUNTER;
 ...
 *   0 on success, a negative value otherwise.
 */
int mlx5_flow_dev_dump(struct rte_eth_dev *dev, struct rte_flow *flow_idx,
  ...
  if (!flow_idx) {
    ...
    if (mlx5_flow_dev_dump_sh_all(dev, file, error))
      return -EINVAL;
#endif
    return mlx5_devx_cmd_flow_dump(sh->fdb_domain, sh->rx_domain, sh->tx_domain,
                                   file);
  }
  /* dump one */
  ...
  while (handle_idx) {
    ...
    if (!dh)
      return -ENOENT;
    if (dh->drv_flow) {
      ret = mlx5_devx_cmd_flow_single_dump(dh->drv_flow, file);
      if (ret)
        return -ENOENT;
    }
    handle_idx = dh->next.next;
...
int mlx5_flow_get_aged_flows(struct rte_eth_dev *dev, void **contexts,
  ...
  DRV_LOG(ERR, "port %u get aged flows is not supported.", dev->data->port_id);
  return -ENOTSUP;
}

/* Wrapper for driver action_validate op callback */
static int flow_drv_action_validate(
    struct rte_eth_dev *dev, const struct rte_flow_indir_action_conf *conf,
    const struct rte_flow_action *action,
    const struct mlx5_flow_driver_ops *fops, struct rte_flow_error *error) {
...
static int flow_drv_action_update(struct rte_eth_dev *dev,
                                  struct rte_flow_action_handle *handle,
                                  const void *update,
                                  const struct mlx5_flow_driver_ops *fops,
                                  struct rte_flow_error *error) {
...
/* Wrapper for driver action_destroy op callback */
static int flow_drv_action_query(struct rte_eth_dev *dev,
                                 const struct rte_flow_action_handle *handle,
                                 void *data,
                                 const struct mlx5_flow_driver_ops *fops,
                                 struct rte_flow_error *error) {
...
static int mlx5_action_handle_update(struct rte_eth_dev *dev,
  ...
  ret = flow_drv_action_validate(
      dev, NULL, (const struct rte_flow_action *)update, fops, error);
  if (ret)
    return ret;
  return flow_drv_action_update(dev, handle, update, fops, error);
}
...
int mlx5_action_handle_flush(struct rte_eth_dev *dev) {
  ...
  ILIST_FOREACH(priv->sh->ipool[MLX5_IPOOL_RSS_SHARED_ACTIONS],
    ...
    ret |= mlx5_action_handle_destroy(
        dev, (struct rte_flow_action_handle *)(uintptr_t)idx, &error);
  }
  return ret;
}
...
int mlx5_action_handle_attach(struct rte_eth_dev *dev) {
  ...
  LIST_FOREACH(ind_tbl, &priv->standalone_ind_tbls, next) {
    ...
    ret = mlx5_validate_rss_queues(dev, ind_tbl->queues, ind_tbl->queues_n,
                                   &message, &queue_idx);
    if (ret != 0) {
      DRV_LOG(ERR, "Port %u cannot use queue %u in RSS: %s", dev->data->port_id,
              ind_tbl->queues[queue_idx], message);
      break;
    }
  }
  if (ret != 0)
...
mlx5_get_tof(const struct rte_flow_item *item,
  ...
  for (; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    if (item->type == (typeof(item->type))MLX5_RTE_FLOW_ITEM_TYPE_TUNNEL) {
  ...
  for (; action->conf != RTE_FLOW_ACTION_TYPE_END; action++) {
    if (action->type ==
        (typeof(action->type))MLX5_RTE_FLOW_ACTION_TYPE_TUNNEL_SET) {
 ...
 */
#ifdef HAVE_IBV_FLOW_DV_SUPPORT
__extension__ union tunnel_offload_mark {
  uint32_t val;
  struct {
...
static bool mlx5_access_tunnel_offload_db(
    struct rte_eth_dev *dev,
    bool (*match)(struct rte_eth_dev *, struct mlx5_flow_tunnel *,
                  const void *),
    void (*hit)(struct rte_eth_dev *, struct mlx5_flow_tunnel *, void *),
    void (*miss)(struct rte_eth_dev *, void *), void *ctx, bool lock_op);
...
static int flow_tunnel_add_default_miss(
  ...
  const struct rte_flow_item miss_items[2] = {{.type = RTE_FLOW_ITEM_TYPE_ETH,
                                               .spec = NULL,
                                               .last = NULL,
                                               .mask = NULL},
                                              {.type = RTE_FLOW_ITEM_TYPE_END,
  ...
  if (!attr->transfer) {
    ...
    ctx->queue =
        mlx5_malloc(MLX5_MEM_SYS | MLX5_MEM_ZERO, q_size, 0, SOCKET_ID_ANY);
    if (!ctx->queue)
      return rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                                NULL, "invalid default miss RSS");
    ctx->action_rss.func = RTE_ETH_HASH_FUNCTION_DEFAULT,
    ctx->action_rss.level = 0, ctx->action_rss.types = priv->rss_conf.rss_hf,
  ...
  ret = mlx5_flow_group_to_table(dev, tunnel, jump_data->group, &flow_table,
                                 &grp_info, error);
  if (ret)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
  ...
  dev_flow = flow_drv_prepare(dev, flow, &miss_attr, miss_items, miss_actions,
                              flow_idx, error);
  if (!dev_flow)
    return -rte_errno;
  dev_flow->flow = flow;
  ...
  SILIST_INSERT(&flow->dev_handles, dev_flow->handle_idx, dev_flow->handle,
                next);
  DRV_LOG(DEBUG, "port %u tunnel type=%d id=%u miss rule priority=%u group=%u",
          dev->data->port_id, tunnel->app_tunnel.type, tunnel->tunnel_id,
          miss_attr.priority, miss_attr.group);
  ret = flow_drv_translate(dev, dev_flow, &miss_attr, miss_items, miss_actions,
                           error);
  if (!ret)
    ret = flow_mreg_update_copy_table(dev, flow, miss_actions, error);
...
tunnel_mark_decode(struct rte_eth_dev *dev, uint32_t mark) {
  ...
  return he ? container_of(he, struct mlx5_flow_tbl_data_entry, entry) : NULL;
}

static void mlx5_flow_tunnel_grp2tbl_remove_cb(void *tool_ctx,
                                               struct mlx5_list_entry *entry) {
...
static int mlx5_flow_tunnel_grp2tbl_match_cb(void *tool_ctx __rte_unused,
                                             struct mlx5_list_entry *entry,
                                             void *cb_ctx) {
  struct mlx5_flow_cb_ctx *ctx = cb_ctx;
...
mlx5_flow_tunnel_grp2tbl_create_cb(void *tool_ctx, void *cb_ctx) {
  ...
  tte =
      mlx5_malloc(MLX5_MEM_SYS | MLX5_MEM_ZERO, sizeof(*tte), 0, SOCKET_ID_ANY);
  if (!tte)
    goto err;
  mlx5_ipool_malloc(sh->ipool[MLX5_IPOOL_TNL_TBL_ID], &tte->flow_table);
  if (tte->flow_table >= MLX5_MAX_TABLES) {
    DRV_LOG(ERR, "Tunnel TBL ID %d exceed max limit.", tte->flow_table);
    mlx5_ipool_free(sh->ipool[MLX5_IPOOL_TNL_TBL_ID], tte->flow_table);
    goto err;
...
mlx5_flow_tunnel_grp2tbl_clone_cb(void *tool_ctx __rte_unused,
                                  ...
                                  void *cb_ctx __rte_unused) {
  struct tunnel_tbl_entry *tte =
      mlx5_malloc(MLX5_MEM_SYS, sizeof(*tte), 0, SOCKET_ID_ANY);
...
static uint32_t tunnel_flow_group_to_flow_table(
  ...
  struct mlx5_list_entry *he;
  struct tunnel_tbl_entry *tte;
  union tunnel_tbl_key key = {.tunnel_id = tunnel ? tunnel->tunnel_id : 0,
                              .group = group};
  ...
  *table = tte->flow_table;
  DRV_LOG(DEBUG, "port %u tunnel %u group=%#x table=%#x", dev->data->port_id,
          key.tunnel_id, group, *table);
  return 0;
}
...
static void mlx5_flow_tunnel_free(struct rte_eth_dev *dev,
  ...
  DRV_LOG(DEBUG, "port %u release pmd tunnel id=0x%x", dev->data->port_id,
          tunnel->tunnel_id);
...
static bool find_tunnel_id_match(struct rte_eth_dev *dev,
                                 struct mlx5_flow_tunnel *tunnel,
                                 const void *x) {
...
static void find_tunnel_id_hit(struct rte_eth_dev *dev,
...
static struct mlx5_flow_tunnel *mlx5_find_tunnel_id(struct rte_eth_dev *dev,
  ...
  mlx5_access_tunnel_offload_db(dev, find_tunnel_id_match, find_tunnel_id_hit,
                                NULL, &ctx, true);
...
mlx5_flow_tunnel_allocate(struct rte_eth_dev *dev,
  ...
  tunnel->groups = mlx5_hlist_create(
      "tunnel groups", 64, false, true, priv->sh,
      mlx5_flow_tunnel_grp2tbl_create_cb, mlx5_flow_tunnel_grp2tbl_match_cb,
      mlx5_flow_tunnel_grp2tbl_remove_cb, mlx5_flow_tunnel_grp2tbl_clone_cb,
  ...
  tunnel->tunnel_id = id;
  tunnel->action.type =
      (typeof(tunnel->action.type))MLX5_RTE_FLOW_ACTION_TYPE_TUNNEL_SET;
  tunnel->action.conf = tunnel;
  tunnel->item.type = (typeof(tunnel->item.type))MLX5_RTE_FLOW_ITEM_TYPE_TUNNEL;
  tunnel->item.spec = tunnel;
  tunnel->item.last = NULL;
  tunnel->item.mask = NULL;

  DRV_LOG(DEBUG, "port %u new pmd tunnel id=0x%x", dev->data->port_id,
          tunnel->tunnel_id);
...
static int mlx5_get_flow_tunnel(struct rte_eth_dev *dev,
                                const struct rte_flow_tunnel *app_tunnel,
                                struct mlx5_flow_tunnel **tunnel) {
...
int mlx5_alloc_tunnel_hub(struct mlx5_dev_ctx_shared *sh) {
  ...
  thub = mlx5_malloc(MLX5_MEM_SYS | MLX5_MEM_ZERO, sizeof(*thub), 0,
                     SOCKET_ID_ANY);
  if (!thub)
    return -ENOMEM;
  LIST_INIT(&thub->tunnels);
  rte_spinlock_init(&thub->sl);
  thub->groups = mlx5_hlist_create(
      "flow groups", 64, false, true, sh, mlx5_flow_tunnel_grp2tbl_create_cb,
      mlx5_flow_tunnel_grp2tbl_match_cb, mlx5_flow_tunnel_grp2tbl_remove_cb,
...
static inline int mlx5_flow_tunnel_validate(struct rte_eth_dev *dev,
  ...
  if (!priv->sh->config.dv_flow_en)
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              NULL, "flow DV interface is off");
  if (!is_tunnel_offload_active(dev))
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              NULL, "tunnel offload was not activated");
  if (!tunnel)
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
                              NULL, "no application tunnel");
  switch (tunnel->type) {
  default:
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
...
static int mlx5_flow_tunnel_decap_set(struct rte_eth_dev *dev,
  ...
  if (ret < 0) {
    return rte_flow_error_set(error, ret, RTE_FLOW_ERROR_TYPE_ACTION_CONF, NULL,
...
static int mlx5_flow_tunnel_match(struct rte_eth_dev *dev,
  ...
  ret = mlx5_get_flow_tunnel(dev, app_tunnel, &tunnel);
  if (ret < 0) {
    return rte_flow_error_set(error, ret, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
                              "failed to initialize pmd tunnel");
  }
  *items = &tunnel->item;
  *num_of_items = 1;
...
static bool tunnel_element_release_match(struct rte_eth_dev *dev,
                                         struct mlx5_flow_tunnel *tunnel,
                                         const void *x) {
...
static void tunnel_element_release_hit(struct rte_eth_dev *dev,
                                       struct mlx5_flow_tunnel *tunnel,
                                       void *x) {
...
static void tunnel_element_release_miss(struct rte_eth_dev *dev, void *x) {
  struct tunnel_db_element_release_ctx *ctx = x;
  RTE_SET_USED(dev);
  ctx->ret = rte_flow_error_set(ctx->error, EINVAL, RTE_FLOW_ERROR_TYPE_HANDLE,
...
static int mlx5_flow_tunnel_get_restore_info(struct rte_eth_dev *dev,
  ...
  if (!tble) {
    DRV_LOG(DEBUG, "port %u invalid miss tunnel mark %#x", dev->data->port_id,
            m->hash.fdir.hi);
    goto err;
  }
  ...
  info->flags = RTE_FLOW_RESTORE_INFO_TUNNEL | RTE_FLOW_RESTORE_INFO_GROUP_ID |
                RTE_FLOW_RESTORE_INFO_ENCAPSULATED;

  return 0;

err:
  return rte_flow_error_set(err, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
...
static uint32_t tunnel_flow_group_to_flow_table(
    ...
    __rte_unused const struct mlx5_flow_tunnel *t, __rte_unused uint32_t group,
    __rte_unused uint32_t *table, struct rte_flow_error *error) {
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, "tunnel offload requires DV support");
}
...
static void mlx5_dbg__print_pattern(const struct rte_flow_item *item) {
  ...
  for (; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
    char *item_name;
    ret =
        rte_flow_conv(RTE_FLOW_CONV_OP_ITEM_NAME_PTR, &item_name,
                      sizeof(item_name), (void *)(uintptr_t)item->type, &error);
    if (ret > 0)
      printf("%s ", item_name);
    else
      printf("%d\n", (int)item->type);
...
static int mlx5_flow_is_std_vxlan_port(const struct rte_flow_item *udp_item) {
...
mlx5_flow_expand_rss_adjust_node(const struct rte_flow_item *pattern,
  ...
  if (item->type == RTE_FLOW_ITEM_TYPE_VXLAN && node != NULL &&
...
int mlx5_flow_discover_priorities(struct rte_eth_dev *dev) {
  ...
  default:
    rte_errno = ENOTSUP;
    DRV_LOG(ERR, "port %u maximum priority: %d expected 8/16",
            dev->data->port_id, ret);
    return -rte_errno;
  }
  DRV_LOG(INFO,
          "port %u supported flow priorities:"
          " 0-%d for ingress or egress root table,"
          " 0-%d for non-root table or transfer root table.",
          dev->data->port_id, ret - 2, MLX5_NON_ROOT_FLOW_MAX_PRIO - 1);
  return ret;
}
