...
static inline uint16_t rxq_cq_decompress_v(struct mlx5_rxq_data *rxq,
  ...
  const uint64x2_t rearm = vld1q_u64((void *)&t_pkt->rearm_data);
  ...
  for (pos = 0; pos < mcqe_n;) {
    ...
    uint16x4_t invalid_mask =
        vcreate_u16(mcqe_n - pos < MLX5_VPMD_DESCS_PER_LOOP
                        ? -1UL << ((mcqe_n - pos) * sizeof(uint16_t) * 8)
                        : 0);
#endif
...
#endif
        : [byte_cnt] "=&w"(byte_cnt)
        : [mcq] "r"(p), [rxdf] "w"(rxdf), [rearm] "w"(rearm), [e3] "r"(e3),
          [e2] "r"(e2), [e1] "r"(e1), [e0] "r"(e0),
          [mcqe_shuf_m1] "w"(mcqe_shuf_m1), [mcqe_shuf_m2] "w"(mcqe_shuf_m2),
    ...
    if (rxq->mark) {
      if (rxq->mcqe_format != MLX5_CQE_RESP_FORMAT_FTAG_STRIDX) {
        ...
        const uint32x4_t flow_mark_adj = {-1, -1, -1, -1};
        const uint8x16_t flow_mark_shuf = {28, 24, 25, -1, 20, 16, 17, -1,
                                           12, 8,  9,  -1, 4,  0,  1,  -1};
        /* Extract flow_tag field. */
        const uint32x4_t ft_mask = vdupq_n_u32(MLX5_FLOW_MARK_DEFAULT);
        const uint32x4_t fdir_flags = vdupq_n_u32(RTE_MBUF_F_RX_FDIR);
        const uint32x4_t fdir_all_flags =
            vdupq_n_u32(RTE_MBUF_F_RX_FDIR | RTE_MBUF_F_RX_FDIR_ID);
        ...
        __asm__ volatile
            ...
            ("ld1 {v16.16b - v17.16b}, [%[mcq]]\n\t"
             ...
             : [ftag] "=&w"(ftag)
             : [mcq] "r"(p), [flow_mark_shuf] "w"(flow_mark_shuf)
             : "memory", "v16", "v17");
        invalid_mask = vceqzq_u32(ftag);
        ol_flags_mask = vorrq_u32(ol_flags_mask, fdir_all_flags);
        /* Set RTE_MBUF_F_RX_FDIR if flow tag is non-zero. */
        ol_flags = vorrq_u32(ol_flags, vbicq_u32(fdir_flags, invalid_mask));
        /* Mask out invalid entries. */
        fdir_id_flags = vbicq_u32(fdir_id_flags, invalid_mask);
        /* Check if flow tag MLX5_FLOW_MARK_DEFAULT. */
        ol_flags = vorrq_u32(
            ol_flags, vbicq_u32(fdir_id_flags, vceqq_u32(ftag, ft_mask)));
        ftag = vaddq_u32(ftag, flow_mark_adj);
        elts[pos]->hash.fdir.hi = vgetq_lane_u32(ftag, 3);
        elts[pos + 1]->hash.fdir.hi = vgetq_lane_u32(ftag, 2);
        elts[pos + 2]->hash.fdir.hi = vgetq_lane_u32(ftag, 1);
        elts[pos + 3]->hash.fdir.hi = vgetq_lane_u32(ftag, 0);
      }
    }
    if (unlikely(rxq->mcqe_format != MLX5_CQE_RESP_FORMAT_HASH)) {
      if (rxq->mcqe_format == MLX5_CQE_RESP_FORMAT_L34H_STRIDX) {
        ...
        const uint32x4_t vlan_mask =
            vdupq_n_u32(RTE_MBUF_F_RX_VLAN | RTE_MBUF_F_RX_VLAN_STRIPPED);
        const uint32x4_t cv_mask = vdupq_n_u32(MLX5_CQE_VLAN_STRIPPED);
        const uint32x4_t pkt_cv = {pkt_hdr0 & 0x1, pkt_hdr1 & 0x1,
                                   pkt_hdr2 & 0x1, pkt_hdr3 & 0x1};

        ol_flags_mask = vorrq_u32(ol_flags_mask, vlan_mask);
        ol_flags = vorrq_u32(ol_flags,
                             vandq_u32(vlan_mask, vceqq_u32(pkt_cv, cv_mask)));
      ...
      const uint32x4_t hash_flags = vdupq_n_u32(RTE_MBUF_F_RX_RSS_HASH);
      const uint32x4_t rearm_flags = vdupq_n_u32((uint32_t)t_pkt->ol_flags);

      ol_flags_mask = vorrq_u32(ol_flags_mask, hash_flags);
      ol_flags = vorrq_u32(ol_flags, vbicq_u32(rearm_flags, ol_flags_mask));
...
static inline void rxq_cq_to_ptype_oflags_v(struct mlx5_rxq_data *rxq,
                                            uint32x4_t ptype_info,
                                            uint32x4_t flow_tag,
                                            uint16x4_t op_err,
                                            struct rte_mbuf **pkts) {
  uint16x4_t ptype;
  uint32x4_t pinfo, cv_flags;
  uint32x4_t ol_flags = vdupq_n_u32(rxq->rss_hash * RTE_MBUF_F_RX_RSS_HASH |
  ...
  const uint64x2_t mbuf_init =
      vld1q_u64((const uint64_t *)&rxq->mbuf_initializer);
  ...
  if (rxq->mark) {
    ...
    /* Check if flow tag MLX5_FLOW_MARK_DEFAULT. */
    ol_flags = vorrq_u32(ol_flags,
                         vbicq_u32(fdir_id_flags, vceqq_u32(flow_tag, ft_def)));
  }
  /*
  ...
  rearm0 = vreinterpretq_u64_u32(vsetq_lane_u32(
      vgetq_lane_u32(ol_flags, 3), vreinterpretq_u32_u64(mbuf_init), 2));
  rearm1 = vreinterpretq_u64_u32(vsetq_lane_u32(
      vgetq_lane_u32(ol_flags, 2), vreinterpretq_u32_u64(mbuf_init), 2));
  rearm2 = vreinterpretq_u64_u32(vsetq_lane_u32(
      vgetq_lane_u32(ol_flags, 1), vreinterpretq_u32_u64(mbuf_init), 2));
  rearm3 = vreinterpretq_u64_u32(vsetq_lane_u32(
      vgetq_lane_u32(ol_flags, 0), vreinterpretq_u32_u64(mbuf_init), 2));
...
static inline uint16_t rxq_cq_process_v(struct mlx5_rxq_data *rxq,
  ...
  for (pos = 0; pos < pkts_n; pos += MLX5_VPMD_DESCS_PER_LOOP) {
    ...
    mask = vcreate_u16(pkts_n - pos < MLX5_VPMD_DESCS_PER_LOOP
                           ? -1UL >> ((pkts_n - pos) * sizeof(uint16_t) * 8)
                           : 0);
    ...
    __asm__ volatile(
        ...
        : [op_own] "=&w"(op_own), [byte_cnt] "=&w"(byte_cnt),
          [ptype_info] "=&w"(ptype_info), [flow_tag] "=&w"(flow_tag)
        : [p3] "r"(p3), [p2] "r"(p2), [p1] "r"(p1), [p0] "r"(p0), [e3] "r"(e3),
          [e2] "r"(e2), [e1] "r"(e1), [e0] "r"(e0), [c3] "w"(c3), [c2] "w"(c2),
          [c1] "w"(c1), [c0] "w"(c0), [elts_p] "r"(elts_p),
          [pkts_p] "r"(pkts_p), [cqe_shuf_m] "w"(cqe_shuf_m),
          [mb_shuf_m] "w"(mb_shuf_m), [owner_shuf_m] "w"(owner_shuf_m),
          [len_shuf_m] "w"(len_shuf_m), [ptype_shuf_m] "w"(ptype_shuf_m),
          [ftag_shuf_m] "w"(ftag_shuf_m), [crc_adj] "w"(crc_adj),
          [flow_mark_adj] "w"(flow_mark_adj)
        : "memory", "v12", "v13", "v14", "v15", "v16", "v17", "v18", "v19",
    ...
    comp_idx =
        __builtin_clzl(vget_lane_u64(vreinterpret_u64_u16(comp_mask), 0)) /
        (sizeof(uint16_t) * 8);
    invalid_mask = vorr_u16(invalid_mask, comp_mask);
    /* D.7 count non-compressed valid CQEs. */
    n = __builtin_clzl(vget_lane_u64(vreinterpret_u64_u16(invalid_mask), 0)) /
        (sizeof(uint16_t) * 8);
    nocmp_n += n;
    /*
     ...
     */
    mask = vcreate_u16(
        n < MLX5_VPMD_DESCS_PER_LOOP ? -1UL >> (n * sizeof(uint16_t) * 8) : 0);
    invalid_mask = vorr_u16(invalid_mask, mask);
    ...
    *err |= vget_lane_u64(vreinterpret_u64_u16(opcode), 0);
    ...
    rxq_cq_to_ptype_oflags_v(rxq, ptype_info, flow_tag, opcode, &elts[pos]);
    if (unlikely(rxq->shared)) {
      elts[pos]->port =
          container_of(p0, struct mlx5_cqe, pkt_info)->user_index_low;
      elts[pos + 1]->port =
          container_of(p1, struct mlx5_cqe, pkt_info)->user_index_low;
      elts[pos + 2]->port =
          container_of(p2, struct mlx5_cqe, pkt_info)->user_index_low;
      elts[pos + 3]->port =
          container_of(p3, struct mlx5_cqe, pkt_info)->user_index_low;
    }
    if (unlikely(rxq->hw_timestamp)) {
      ...
      if (rxq->rt_timestamp) {
        ...
        mlx5_timestamp_set(elts[pos], offset, mlx5_txpp_convert_rx_ts(sh, ts));
        ts = rte_be_to_cpu_64(
    ...
    if (rxq->dynf_meta) {
      ...
      *RTE_MBUF_DYNFIELD(pkts[pos], offs, uint32_t *) =
          rte_be_to_cpu_32(container_of(p0, struct mlx5_cqe, pkt_info)
                               ->flow_table_metadata) &
          mask;
      *RTE_MBUF_DYNFIELD(pkts[pos + 1], offs, uint32_t *) =
          rte_be_to_cpu_32(container_of(p1, struct mlx5_cqe, pkt_info)
                               ->flow_table_metadata) &
          mask;
      *RTE_MBUF_DYNFIELD(pkts[pos + 2], offs, uint32_t *) =
          rte_be_to_cpu_32(container_of(p2, struct mlx5_cqe, pkt_info)
                               ->flow_table_metadata) &
          mask;
      *RTE_MBUF_DYNFIELD(pkts[pos + 3], offs, uint32_t *) =
          rte_be_to_cpu_32(container_of(p3, struct mlx5_cqe, pkt_info)
                               ->flow_table_metadata) &
          mask;
