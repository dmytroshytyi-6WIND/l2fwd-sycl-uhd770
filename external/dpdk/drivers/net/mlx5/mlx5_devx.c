 ...
 */
static int mlx5_rxq_obj_modify_rq_vlan_strip(struct mlx5_rxq_priv *rxq,
                                             int on) {
 ...
 */
static int mlx5_devx_modify_rq(struct mlx5_rxq_priv *rxq, uint8_t type) {
  struct mlx5_devx_modify_rq_attr rq_attr;
...
int mlx5_txq_devx_modify(struct mlx5_txq_obj *obj,
  ...
  if (type != MLX5_TXQ_MOD_RST2RDY) {
    ...
    if (ret) {
      DRV_LOG(ERR,
              "Cannot change the Tx SQ state to RESET"
              " %s",
              strerror(errno));
      rte_errno = errno;
      return ret;
  ...
  if (type != MLX5_TXQ_MOD_RDY2RST) {
    ...
    if (ret) {
      DRV_LOG(ERR,
              "Cannot change the Tx SQ state to READY"
              " %s",
              strerror(errno));
      rte_errno = errno;
      return ret;
 ...
 *   DevX Rx queue.
 */
static void mlx5_rxq_devx_obj_release(struct mlx5_rxq_priv *rxq) {
  ...
  if (rxq_obj->rxq_ctrl->is_hairpin) {
    ...
    if (rxq_obj->devx_channel) {
      mlx5_os_devx_destroy_event_channel(rxq_obj->devx_channel);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rx_devx_get_event(struct mlx5_rxq_obj *rxq_obj) {
#ifdef HAVE_IBV_DEVX_EVENT
  union {
    struct mlx5dv_devx_async_event_hdr event_resp;
    uint8_t buf[sizeof(struct mlx5dv_devx_async_event_hdr) + 128];
  } out;
  int ret = mlx5_glue->devx_get_event(rxq_obj->devx_channel, &out.event_resp,
                                      sizeof(out.buf));
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rxq_create_devx_rq_resources(struct mlx5_rxq_priv *rxq) {
  ...
  rq_attr.ts_format = mlx5_ts_format_conv(cdev->config.hca_attr.rq_ts_format);
  ...
  if (mlx5_rxq_mprq_enabled(rxq_data)) {
    ...
    rq_attr.wq_attr.single_wqe_log_num_of_strides =
        rxq_data->log_strd_num - MLX5_MIN_SINGLE_WQE_LOG_NUM_STRIDES;
    /* Stride size = (2^single_stride_log_num_of_bytes)*64B. */
    rq_attr.wq_attr.single_stride_log_num_of_bytes =
        rxq_data->log_strd_sz - MLX5_MIN_SINGLE_STRIDE_LOG_NUM_BYTES;
  ...
  return mlx5_devx_rq_create(cdev->ctx, &rxq->devx_rq, wqe_size, log_desc_n,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rxq_create_devx_cq_resources(struct mlx5_rxq_priv *rxq) {
  ...
  if (rxq_ctrl->started)
    return 0;
  if (priv->config.cqe_comp && !rxq_data->hw_timestamp && !rxq_data->lro) {
    ...
    case MLX5_CQE_RESP_FORMAT_CSUM:
      ...
      if (mlx5_rxq_check_vec_support(rxq_data) < 0 &&
        ...
        cq_attr.mini_cqe_res_format = MLX5_CQE_RESP_FORMAT_CSUM_STRIDX;
      else
        cq_attr.mini_cqe_res_format = MLX5_CQE_RESP_FORMAT_HASH;
    ...
    DRV_LOG(DEBUG, "Port %u Rx CQE compression is enabled, format %d.", port_id,
            priv->config.cqe_comp_fmt);
    /*
    ...
    DRV_LOG(DEBUG, "Port %u Rx CQE compression is disabled for HW timestamp.",
            port_id);
  } else if (priv->config.cqe_comp && rxq_data->lro) {
    DRV_LOG(DEBUG, "Port %u Rx CQE compression is disabled for LRO.", port_id);
  }
  cq_attr.uar_page_id = mlx5_os_get_devx_uar_page_id(sh->rx_uar.obj);
  log_cqe_n = log2above(cqe_n);
  /* Create CQ using DevX API. */
  ret = mlx5_devx_cq_create(sh->cdev->ctx, &rxq_ctrl->obj->cq_obj, log_cqe_n,
                            &cq_attr, sh->numa_node);
  if (ret)
    return ret;
  cq_obj = &rxq_ctrl->obj->cq_obj;
  rxq_data->cqes = (volatile struct mlx5_cqe(*)[])(uintptr_t)cq_obj->cqes;
  ...
  if (rxq_ctrl->obj->devx_channel) {
    ret = mlx5_os_devx_subscribe_devx_event(
        rxq_ctrl->obj->devx_channel, cq_obj->cq->obj, sizeof(event_nums),
        event_nums, (uint64_t)(uintptr_t)cq_obj->cq);
    if (ret) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rxq_obj_hairpin_new(struct mlx5_rxq_priv *rxq) {
  ...
  attr.hairpin = 1;
  max_wq_data = priv->sh->cdev->config.hca_attr.log_max_hairpin_wq_data_sz;
  ...
  attr.wq_attr.log_hairpin_num_packets =
      attr.wq_attr.log_hairpin_data_sz - MLX5_HAIRPIN_QUEUE_STRIDE;
  ...
  tmpl->rq =
      mlx5_devx_cmd_create_rq(priv->sh->cdev->ctx, &attr, rxq_ctrl->socket);
  if (!tmpl->rq) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rxq_devx_obj_new(struct mlx5_rxq_priv *rxq) {
  struct mlx5_priv *priv = rxq->priv;
  struct mlx5_rxq_ctrl *rxq_ctrl = rxq->ctrl;
  struct mlx5_rxq_data *rxq_data = &rxq_ctrl->rxq;
  struct mlx5_rxq_obj *tmpl = rxq_ctrl->obj;
  ...
  if (rxq_ctrl->irq && !rxq_ctrl->started) {
    int devx_ev_flag = MLX5DV_DEVX_CREATE_EVENT_CHANNEL_FLAGS_OMIT_EV_DATA;

    tmpl->devx_channel =
        mlx5_os_devx_create_event_channel(priv->sh->cdev->ctx, devx_ev_flag);
  ...
  if (!rxq_data->shared) {
    ...
    rxq_data->rq_db = (uint32_t *)(uintptr_t)tmpl->devx_rmp.wq.db_rec;
  }
  if (!rxq_ctrl->started) {
 ...
 */
static struct mlx5_devx_rqt_attr *mlx5_devx_ind_table_create_rqt_attr(
  ...
  rqt_attr =
      mlx5_malloc(MLX5_MEM_ZERO, sizeof(*rqt_attr) + rqt_n * sizeof(uint32_t),
                  0, SOCKET_ID_ANY);
  if (!rqt_attr) {
    DRV_LOG(ERR, "Port %u cannot allocate RQT resources.", dev->data->port_id);
    rte_errno = ENOMEM;
    return NULL;
  }
  ...
  if (queues == NULL) {
    for (i = 0; i < rqt_n; i++)
      rqt_attr->rq_list[i] = priv->drop_queue.rxq->devx_rq.rq->id;
    return rqt_attr;
  }
  for (i = 0; i != queues_n; ++i) {
    if (mlx5_is_external_rxq(dev, queues[i])) {
      struct mlx5_external_rxq *ext_rxq = mlx5_ext_rxq_get(dev, queues[i]);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_devx_ind_table_new(struct rte_eth_dev *dev,
                                   const unsigned int log_n,
                                   struct mlx5_ind_table_obj *ind_tbl) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_devx_rqt_attr *rqt_attr = NULL;
  const uint16_t *queues = dev->data->dev_started ? ind_tbl->queues : NULL;
  ...
  mlx5_free(rqt_attr);
  if (!ind_tbl->rqt) {
    DRV_LOG(ERR, "Port %u cannot create DevX RQT.", dev->data->port_id);
    rte_errno = errno;
    return -rte_errno;
  }
  return 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_devx_ind_table_modify(struct rte_eth_dev *dev,
                                      const unsigned int log_n,
  ...
  MLX5_ASSERT(ind_tbl);
  rqt_attr = mlx5_devx_ind_table_create_rqt_attr(dev, log_n, queues, queues_n);
  if (!rqt_attr)
    return -rte_errno;
  ret = mlx5_devx_cmd_modify_rqt(ind_tbl->rqt, rqt_attr);
  mlx5_free(rqt_attr);
  if (ret)
    DRV_LOG(ERR, "Port %u cannot modify DevX RQT.", dev->data->port_id);
  return ret;
}
 ...
 */
static void mlx5_devx_ind_table_destroy(struct mlx5_ind_table_obj *ind_tbl) {
 ...
 */
static void mlx5_devx_tir_attr_set(struct rte_eth_dev *dev,
                                   const uint8_t *rss_key, uint64_t hash_fields,
  ...
  if (ind_tbl->queues == NULL) {
    ...
    for (i = 0; i < ind_tbl->queues_n; ++i) {
      struct mlx5_rxq_data *rxq_i = mlx5_rxq_data_get(dev, ind_tbl->queues[i]);
  ...
  if (hash_fields) {
    ...
    /* 1 bit: 0: TCP, 1: UDP. */
    rx_hash_field_select->l4_prot_type = !!(hash_fields & MLX5_UDP_IBV_RX_HASH);
  ...
  if (lro) {
    ...
    tir_attr->lro_enable_mask =
        MLX5_TIRC_LRO_ENABLE_MASK_IPV4_LRO | MLX5_TIRC_LRO_ENABLE_MASK_IPV6_LRO;
  }
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_devx_hrxq_new(struct rte_eth_dev *dev, struct mlx5_hrxq *hrxq,
                              int tunnel __rte_unused) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_devx_tir_attr tir_attr = {0};
  int err;

  mlx5_devx_tir_attr_set(dev, hrxq->rss_key, hrxq->hash_fields, hrxq->ind_table,
                         tunnel, &tir_attr);
  hrxq->tir = mlx5_devx_cmd_create_tir(priv->sh->cdev->ctx, &tir_attr);
  if (!hrxq->tir) {
    DRV_LOG(ERR, "Port %u cannot create DevX TIR.", dev->data->port_id);
    rte_errno = errno;
    goto error;
  }
  ...
  if (hrxq->hws_flags) {
    hrxq->action = mlx5dr_action_create_dest_tir(
        priv->dr_ctx, (struct mlx5dr_devx_obj *)hrxq->tir, hrxq->hws_flags);
    if (!hrxq->action)
      goto error;
    return 0;
  }
  if (mlx5_flow_os_create_flow_action_dest_devx_tir(hrxq->tir, &hrxq->action)) {
    rte_errno = errno;
    goto error;
  }
#endif
 ...
 */
static void mlx5_devx_tir_destroy(struct mlx5_hrxq *hrxq) {
  claim_zero(mlx5_devx_cmd_destroy(hrxq->tir));
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_devx_hrxq_modify(struct rte_eth_dev *dev,
                                 struct mlx5_hrxq *hrxq, const uint8_t *rss_key,
  ...
  if (hash_fields != hrxq->hash_fields ||
    ...
    modify_tir.modify_bitmask |= MLX5_MODIFY_TIR_IN_MODIFY_BITMASK_HASH;
  ...
  if (mlx5_devx_cmd_modify_tir(hrxq->tir, &modify_tir)) {
    DRV_LOG(ERR, "port %u cannot modify DevX TIR", dev->data->port_id);
    rte_errno = errno;
    return -rte_errno;
  }
  return 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_rxq_devx_obj_drop_create(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  rxq_ctrl = mlx5_malloc(MLX5_MEM_ZERO, sizeof(*rxq_ctrl), 0, socket_id);
  if (rxq_ctrl == NULL) {
  ...
  ret = mlx5_rxq_create_devx_cq_resources(rxq);
  if (ret != 0) {
    DRV_LOG(ERR, "Port %u drop queue CQ creation failed.", dev->data->port_id);
    goto error;
  }
  ...
  ret = mlx5_rxq_create_devx_rq_resources(rxq);
  if (ret != 0) {
    DRV_LOG(ERR, "Port %u drop queue RQ creation failed.", dev->data->port_id);
    rte_errno = ENOMEM;
    goto error;
  }
  /* Change queue state to ready. */
 ...
 *   Pointer to Ethernet device.
 */
static void mlx5_rxq_devx_obj_drop_release(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
 ...
 *   Pointer to Ethernet device.
 */
static void mlx5_devx_drop_action_destroy(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_devx_drop_action_create(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_hrxq *hrxq = priv->drop_queue.hrxq;
  int ret;
 ...
 */
static uint32_t mlx5_get_txq_tis_num(struct rte_eth_dev *dev,
                                     uint16_t queue_idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  int tis_idx;

  if (priv->sh->bond.n_port &&
      priv->sh->lag.affinity_mode == MLX5_LAG_MODE_TIS) {
    tis_idx = (priv->lag_affinity_idx + queue_idx) % priv->sh->bond.n_port;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_txq_obj_hairpin_new(struct rte_eth_dev *dev, uint16_t idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (!tmpl->sq) {
    DRV_LOG(ERR, "Port %u tx hairpin queue %u can't create SQ object.",
 ...
 */
static void mlx5_txq_release_devx_resources(struct mlx5_txq_obj *txq_obj) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_txq_create_devx_sq_resources(struct rte_eth_dev *dev,
                                             uint16_t idx,
                                             uint16_t log_desc_n) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_common_device *cdev = priv->sh->cdev;
  ...
  return mlx5_devx_sq_create(cdev->ctx, &txq_obj->sq_obj, log_desc_n, &sq_attr,
                             priv->sh->numa_node);
}
#endif
...
int mlx5_txq_devx_obj_new(struct rte_eth_dev *dev, uint16_t idx) {
  ...
  txq_obj->dev = dev;
  cqe_n = (1UL << txq_data->elts_n) / MLX5_TX_COMP_THRESH + 1 +
  ...
  if (ret) {
    DRV_LOG(ERR, "Port %u Tx queue %u CQ creation failure.", dev->data->port_id,
            idx);
    goto error;
  }
  ...
  *txq_data->cq_db = 0;
  ...
  wqe_size = txq_data->tso_en
                 ...
                 : 0;
  wqe_size += sizeof(struct mlx5_wqe_cseg) + sizeof(struct mlx5_wqe_eseg) +
              sizeof(struct mlx5_wqe_dseg);
  if (txq_data->inlen_send)
    wqe_size = RTE_MAX(
        wqe_size,
        sizeof(struct mlx5_wqe_cseg) + sizeof(struct mlx5_wqe_eseg) +
            RTE_ALIGN(txq_data->inlen_send + sizeof(uint32_t), MLX5_WSEG_SIZE));
  ...
  if (ret) {
    DRV_LOG(ERR, "Port %u Tx queue %u SQ creation failure.", dev->data->port_id,
            idx);
    rte_errno = errno;
    goto error;
  }
  ...
  *txq_data->qp_db = 0;
  ...
  txq_data->wait_on_time =
      !!(!sh->config.tx_pp && sh->cdev->config.hca_attr.wait_on_time);
  /* Change Send Queue state to Ready-to-Send. */
  ret = mlx5_txq_devx_modify(txq_obj, MLX5_TXQ_MOD_RST2RDY, 0);
  if (ret) {
    rte_errno = errno;
    DRV_LOG(ERR, "Port %u Tx queue %u SQ state to SQC_STATE_RDY failed.",
            dev->data->port_id, idx);
    goto error;
  }
...
#endif
  txq_ctrl->uar_mmap_offset = mlx5_os_get_devx_uar_mmap_offset(sh->tx_uar.obj);
