...
static int mlx5_flow_meter_disable(struct rte_eth_dev *dev, uint32_t meter_id,
...
static void *mlx5_flow_meter_action_create(struct mlx5_priv *priv,
  ...
  MLX5_SET(flow_meter_parameters, fmp, start_color, MLX5_FLOW_COLOR_GREEN);
  ...
  mtr_init.flow_meter_parameter_sz = MLX5_ST_SZ_BYTES(flow_meter_parameters);
...
mlx5_flow_meter_profile_find(struct mlx5_priv *priv,
                             uint32_t meter_profile_id) {
  struct mlx5_flow_meter_profile *fmp;
  union mlx5_l3t_data data;
  int32_t ret;

  if (mlx5_l3t_get_entry(priv->mtr_profile_tbl, meter_profile_id, &data) ||
      !data.ptr)
    return NULL;
  fmp = data.ptr;
  /* Remove reference taken by the mlx5_l3t_get_entry. */
  ret = mlx5_l3t_clear_entry(priv->mtr_profile_tbl, meter_profile_id);
  if (!ret || ret == -1)
    return NULL;
  return fmp;
}
 ...
 */
static int mlx5_flow_meter_profile_validate(
  ...
  if (profile == NULL)
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_PROFILE,
  ...
  if (!priv->sh->meter_aso_en) {
    ...
    if (profile->alg != RTE_MTR_SRTCM_RFC2697) {
      return -rte_mtr_error_set(error, ENOTSUP,
                                RTE_MTR_ERROR_TYPE_METER_PROFILE, NULL,
                                "Metering algorithm is not supported.");
    ...
    if (profile->packet_mode)
      return -rte_mtr_error_set(
          error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_PROFILE, NULL,
  ...
  case RTE_MTR_SRTCM_RFC2697:
    ...
    if (cir > 0 && cir <= MLX5_SRTCM_XIR_MAX && cbs > 0 &&
        cbs <= MLX5_SRTCM_XBS_MAX && ebs <= MLX5_SRTCM_XBS_MAX) {
      ret = 0;
    } else {
      ret = -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_MTR_PARAMS,
                               NULL, "Profile values out of range.");
    }
    break;
  case RTE_MTR_TRTCM_RFC2698:
    ...
    if (cir > 0 && cir <= MLX5_SRTCM_XIR_MAX && cbs > 0 &&
        cbs <= MLX5_SRTCM_XBS_MAX && pir >= cir &&
        pir <= (MLX5_SRTCM_XIR_MAX * 2) && pbs >= cbs &&
        pbs <= (MLX5_SRTCM_XBS_MAX * 2)) {
      ret = 0;
    } else {
      ret = -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_MTR_PARAMS,
                               NULL, "Profile values out of range.");
  ...
  case RTE_MTR_TRTCM_RFC4115:
    ...
    if (cir > 0 && cir <= MLX5_SRTCM_XIR_MAX && cbs > 0 &&
        cbs <= MLX5_SRTCM_XBS_MAX && eir <= MLX5_SRTCM_XIR_MAX &&
 ...
 *   Pointer to the exp.
 */
static inline void mlx5_flow_meter_xir_man_exp_calc(int64_t xir, uint8_t *man,
                                                    uint8_t *exp) {
  int64_t _xir;
 ...
 *   Pointer to the exp.
 */
static inline void mlx5_flow_meter_xbs_man_exp_calc(uint64_t xbs, uint8_t *man,
                                                    uint8_t *exp) {
  int _exp;
  double _man;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_param_fill(struct mlx5_flow_meter_profile *fmp,
                                      struct rte_mtr_error *error) {
  ...
  case RTE_MTR_TRTCM_RFC2698:
    MLX5_ASSERT(
        fmp->profile.trtcm_rfc2698.pir > fmp->profile.trtcm_rfc2698.cir &&
        fmp->profile.trtcm_rfc2698.pbs > fmp->profile.trtcm_rfc2698.cbs);
  ...
  default:
    return -rte_mtr_error_set(error, EINVAL, RTE_MTR_ERROR_TYPE_METER_PROFILE,
                              NULL, "Metering algorithm mode is invalid");
  }
  ...
  cbs_man = man;
  cbs_exp = exp;
  srtcm->cbs_cir = rte_cpu_to_be_32(
      cbs_exp << ASO_DSEG_CBS_EXP_OFFSET | cbs_man << ASO_DSEG_CBS_MAN_OFFSET |
      cir_exp << ASO_DSEG_XIR_EXP_OFFSET | cir_man);
  ...
  ebs_man = man;
  ebs_exp = exp;
  srtcm->ebs_eir = rte_cpu_to_be_32(
      ebs_exp << ASO_DSEG_EBS_EXP_OFFSET | ebs_man << ASO_DSEG_EBS_MAN_OFFSET |
      eir_exp << ASO_DSEG_XIR_EXP_OFFSET | eir_man);
  if (srtcm->cbs_cir)
    fmp->g_support = 1;
  if (srtcm->ebs_eir)
    fmp->y_support = 1;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_mtr_cap_get(struct rte_eth_dev *dev,
  ...
  if (!priv->mtr_en)
    return -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Meter is not supported");
  ...
  cap->stats_mask =
      RTE_MTR_STATS_N_BYTES_DROPPED | RTE_MTR_STATS_N_PKTS_DROPPED;
  return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_profile_add(struct rte_eth_dev *dev,
                                       uint32_t meter_profile_id,
  ...
  if (!priv->mtr_en)
    return -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                              NULL, "Meter is not supported");
  /* Check input params. */
  ret = mlx5_flow_meter_profile_validate(dev, meter_profile_id, profile, error);
  if (ret)
    return ret;
  ...
  if (fmp == NULL)
    return -rte_mtr_error_set(error, ENOMEM, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                              NULL,
  ...
  if (ret)
    goto error;
  data.ptr = fmp;
  ret = mlx5_l3t_set_entry(priv->mtr_profile_tbl, meter_profile_id, &data);
  if (ret)
    return -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_profile_delete(struct rte_eth_dev *dev,
                                          uint32_t meter_profile_id,
                                          struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_meter_profile *fmp;
  ...
  if (fmp == NULL)
    return -rte_mtr_error_set(
        error, ENOENT, RTE_MTR_ERROR_TYPE_METER_PROFILE_ID, &meter_profile_id,
        "Meter profile id is invalid.");
  /* Check profile is unused. */
  if (fmp->ref_cnt)
    return -rte_mtr_error_set(error, EBUSY, RTE_MTR_ERROR_TYPE_METER_PROFILE_ID,
...
mlx5_flow_meter_policy_find(struct rte_eth_dev *dev, uint32_t policy_id,
                            uint32_t *policy_idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (policy_id > MLX5_MAX_SUB_POLICY_TBL_NUM || !priv->policy_idx_tbl)
    return NULL;
  if (mlx5_l3t_get_entry(priv->policy_idx_tbl, policy_id, &data) || !data.dword)
    return NULL;
  if (policy_idx)
    *policy_idx = data.dword;
  sub_policy =
      mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_MTR_POLICY], data.dword);
  /* Remove reference taken by the mlx5_l3t_get_entry. */
 ...
 */
struct mlx5_flow_meter_policy *mlx5_flow_meter_hierarchy_get_final_policy(
    struct rte_eth_dev *dev, struct mlx5_flow_meter_policy *policy) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  while (next_policy->is_hierarchy) {
    next_fm = mlx5_flow_meter_find(
        priv, next_policy->act_cnt[RTE_COLOR_GREEN].next_mtr_id, NULL);
...
mlx5_flow_meter_policy_validate(struct rte_eth_dev *dev,
                                ...
                                struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct rte_flow_attr attr = {.transfer = priv->sh->config.dv_esw_en ? 1 : 0};
  bool is_rss = false;
  uint8_t policy_mode;
  ...
  ret = mlx5_flow_validate_mtr_acts(dev, policy->actions, &attr, &is_rss,
...
static int
__mlx5_flow_meter_policy_delete(struct rte_eth_dev *dev, uint32_t policy_id,
                                struct mlx5_flow_meter_policy *mtr_policy,
                                struct rte_mtr_error *error, bool clear_l3t) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  if (mtr_policy->ref_cnt) {
    ...
    return -rte_mtr_error_set(error, EBUSY, RTE_MTR_ERROR_TYPE_METER_POLICY_ID,
                              NULL, "Meter policy object is being used.");
  }
 ...
 */
static int
mlx5_flow_meter_policy_add(struct rte_eth_dev *dev, uint32_t policy_id,
                           struct rte_mtr_meter_policy_params *policy,
                           struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct rte_flow_attr attr = {.transfer = priv->sh->config.dv_esw_en ? 1 : 0};
  ...
  if (policy_id == priv->sh->mtrmng->def_policy_id)
    return -rte_mtr_error_set(error, EEXIST, RTE_MTR_ERROR_TYPE_METER_POLICY_ID,
  ...
  if (policy_mode == MLX5_MTR_POLICY_MODE_DEF) {
    if (priv->sh->mtrmng->def_policy_id != MLX5_INVALID_POLICY_ID)
      return -rte_mtr_error_set(error, EEXIST,
                                RTE_MTR_ERROR_TYPE_METER_POLICY_ID, NULL,
                                "a policy with similar actions "
                                "is already configured");
    if (mlx5_flow_create_def_policy(dev))
      return -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY,
                                NULL, "fail to create non-terminated policy.");
    priv->sh->mtrmng->def_policy_id = policy_id;
    return 0;
  }
  if (!priv->sh->meter_aso_en)
    return -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_METER_POLICY,
                              NULL, "no ASO capability to support the policy ");
  for (i = 0; i < MLX5_MTR_DOMAIN_MAX; i++) {
  ...
  mtr_policy = mlx5_malloc(MLX5_MEM_ZERO, policy_size, RTE_CACHE_LINE_SIZE,
                           SOCKET_ID_ANY);
  if (!mtr_policy)
    return -rte_mtr_error_set(error, ENOMEM, RTE_MTR_ERROR_TYPE_METER_POLICY,
                              NULL, "Memory alloc failed for meter policy.");
  ...
  for (i = 0; i < MLX5_MTR_DOMAIN_MAX; i++) {
    ...
    mtr_policy->sub_policys[i] =
        (struct mlx5_flow_meter_sub_policy **)((uint8_t *)mtr_policy +
                                               policy_size);
    ...
    mtr_policy->sub_policy_num &=
        ~(MLX5_MTR_SUB_POLICY_NUM_MASK << (MLX5_MTR_SUB_POLICY_NUM_SHIFT * i));
    mtr_policy->sub_policy_num |=
  ...
  ret = mlx5_flow_create_mtr_acts(dev, mtr_policy, policy->actions, error);
  if (ret)
    goto policy_add_err;
  if (mtr_policy->is_hierarchy) {
    struct mlx5_flow_meter_policy *final_policy;

    final_policy = mlx5_flow_meter_hierarchy_get_final_policy(dev, mtr_policy);
    if (!final_policy)
      goto policy_add_err;
...
policy_add_err:
  if (mtr_policy) {
    ret = __mlx5_flow_meter_policy_delete(dev, policy_id, mtr_policy, error,
                                          false);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_policy_delete(struct rte_eth_dev *dev,
                                         uint32_t policy_id,
                                         struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_meter_policy *mtr_policy;
  ...
  ret =
      __mlx5_flow_meter_policy_delete(dev, policy_id, mtr_policy, error, true);
  if (ret)
    return ret;
  mlx5_free(mtr_policy);
  return 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_validate(struct mlx5_priv *priv, uint32_t meter_id,
                                    struct rte_mtr_params *params,
                                    struct rte_mtr_error *error) {
  /* Meter must use global drop action. */
  if (!priv->sh->dr_drop_action)
    return -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_MTR_PARAMS,
                              NULL, "No drop action ready for meter.");
  ...
  if (params->meter_policy_id == MLX5_INVALID_POLICY_ID)
    return -rte_mtr_error_set(error, ENOENT, RTE_MTR_ERROR_TYPE_METER_POLICY_ID,
 ...
 */
static int mlx5_flow_meter_action_modify(
    struct mlx5_priv *priv, struct mlx5_flow_meter_info *fm,
    const struct mlx5_flow_meter_srtcm_rfc2697_prm *srtcm, uint64_t modify_bits,
    uint32_t active_state, uint32_t is_enable) {
#ifdef HAVE_MLX5_DR_CREATE_ACTION_FLOW_METER
  ...
  if (priv->sh->meter_aso_en) {
    ...
    mod_attr.flow_meter_parameter_sz = MLX5_ST_SZ_BYTES(flow_meter_parameters);
    ...
    if (fm->meter_action) {
      ret = mlx5_glue->dv_modify_flow_action_meter(
          fm->meter_action, &mod_attr, rte_cpu_to_be_64(modify_bits));
      if (ret)
        return ret;
    }
...
static int mlx5_flow_meter_stats_enable_update(struct rte_eth_dev *dev,
                                               struct mlx5_flow_meter_info *fm,
                                               uint64_t stats_mask) {
  fm->bytes_dropped = (stats_mask & RTE_MTR_STATS_N_BYTES_DROPPED) ? 1 : 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_create(struct rte_eth_dev *dev, uint32_t meter_id,
  ...
  if (params->meter_policy_id == priv->sh->mtrmng->def_policy_id) {
    __atomic_add_fetch(&priv->sh->mtrmng->def_policy_ref_cnt, 1,
                       __ATOMIC_RELAXED);
    ...
    mtr_policy =
        mlx5_flow_meter_policy_find(dev, params->meter_policy_id, &policy_idx);
    if (!mtr_policy)
      return -rte_mtr_error_set(error, ENOENT,
                                RTE_MTR_ERROR_TYPE_METER_POLICY_ID, NULL,
                                "Meter policy id not valid.");
    domain_bitmap = (mtr_policy->ingress ? MLX5_MTR_DOMAIN_INGRESS_BIT : 0) |
                    (mtr_policy->egress ? MLX5_MTR_DOMAIN_EGRESS_BIT : 0) |
                    (mtr_policy->transfer ? MLX5_MTR_DOMAIN_TRANSFER_BIT : 0);
    if (fmp->g_support && mtr_policy->skip_g)
      return -rte_mtr_error_set(error, ENOTSUP,
                                RTE_MTR_ERROR_TYPE_METER_POLICY_ID, NULL,
                                "Meter green policy is empty.");
    if (fmp->y_support && mtr_policy->skip_y)
      return -rte_mtr_error_set(error, ENOTSUP,
                                RTE_MTR_ERROR_TYPE_METER_POLICY_ID, NULL,
                                "Meter yellow policy is empty.");
  }
  ...
  if (priv->sh->meter_aso_en) {
    ...
    if (!mtr_idx)
      return -rte_mtr_error_set(error, ENOMEM, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Memory alloc failed for meter.");
    aso_mtr = mlx5_aso_meter_by_idx(priv, mtr_idx);
    fm = &aso_mtr->fm;
  } else {
    if (fmp->y_support)
      return -rte_mtr_error_set(error, ENOMEM, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Unsupported profile with yellow.");
    legacy_fm = mlx5_ipool_zmalloc(priv->sh->ipool[MLX5_IPOOL_MTR], &mtr_idx);
    if (legacy_fm == NULL)
      return -rte_mtr_error_set(error, ENOMEM, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Memory alloc failed for meter.");
  ...
  if ((mtr_id_bits + priv->sh->mtrmng->max_mtr_flow_bits) > mtr_reg_bits) {
  ...
  if (priv->sh->meter_aso_en) {
    ...
    if (mlx5_l3t_set_entry(priv->mtr_idx_tbl, meter_id, &data))
      goto error;
  } else if (!params->meter_enable &&
             mlx5_flow_meter_disable(dev, meter_id, error)) {
    goto error;
  }
...
static int mlx5_flow_meter_params_flush(struct rte_eth_dev *dev,
                                        struct mlx5_flow_meter_info *fm,
                                        uint32_t mtr_idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_legacy_flow_meters *fms = &priv->flow_meters;
  ...
  mlx5_flow_destroy_mtr_tbls(dev, fm);
  if (fm->def_policy)
    __atomic_sub_fetch(&priv->sh->mtrmng->def_policy_ref_cnt, 1,
                       __ATOMIC_RELAXED);
  if (priv->sh->meter_aso_en) {
    if (!fm->def_policy) {
      mtr_policy = mlx5_flow_meter_policy_find(dev, fm->policy_id, NULL);
      if (mtr_policy)
        __atomic_sub_fetch(&mtr_policy->ref_cnt, 1, __ATOMIC_RELAXED);
      fm->policy_id = 0;
    }
    fm->def_policy = 0;
    ...
    mlx5_ipool_free(priv->sh->ipool[MLX5_IPOOL_MTR], legacy_fm->idx);
  }
  return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_destroy(struct rte_eth_dev *dev, uint32_t meter_id,
                                   struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_meter_info *fm;
  uint32_t mtr_idx = 0;
  ...
  if (fm == NULL)
    return -rte_mtr_error_set(error, ENOENT, RTE_MTR_ERROR_TYPE_MTR_ID, NULL,
                              "Meter object id not valid.");
  /* Meter object must not have any owner. */
  if (fm->ref_cnt > 0)
    return -rte_mtr_error_set(error, EBUSY, RTE_MTR_ERROR_TYPE_UNSPECIFIED,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_modify_state(struct mlx5_priv *priv,
                                        struct mlx5_flow_meter_info *fm,
                                        uint32_t new_state,
                                        struct rte_mtr_error *error) {
  ...
  if (new_state == MLX5_FLOW_METER_DISABLE)
    ret = mlx5_flow_meter_action_modify(priv, fm, &srtcm, modify_bits, 0, 0);
  else
    ret = mlx5_flow_meter_action_modify(priv, fm, &fm->profile->srtcm_prm,
                                        modify_bits, 0, 1);
  if (ret)
    return -rte_mtr_error_set(error, -ret, RTE_MTR_ERROR_TYPE_MTR_PARAMS, NULL,
                              new_state ? "Failed to enable meter."
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_enable(struct rte_eth_dev *dev, uint32_t meter_id,
                                  struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_meter_info *fm;
  int ret;
  ...
  if (fm->active_state == MLX5_FLOW_METER_ENABLE)
    return 0;
  ret = mlx5_flow_meter_modify_state(priv, fm, MLX5_FLOW_METER_ENABLE, error);
  if (!ret)
...
static int mlx5_flow_meter_disable(struct rte_eth_dev *dev, uint32_t meter_id,
  ...
  if (fm->active_state == MLX5_FLOW_METER_DISABLE)
    return 0;
  ret = mlx5_flow_meter_modify_state(priv, fm, MLX5_FLOW_METER_DISABLE, error);
  if (!ret)
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_profile_update(struct rte_eth_dev *dev,
                                          uint32_t meter_id,
                                          uint32_t meter_profile_id,
                                          struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_stats_update(struct rte_eth_dev *dev,
                                        uint32_t meter_id, uint64_t stats_mask,
                                        struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_meter_info *fm;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_stats_read(struct rte_eth_dev *dev,
                                      uint32_t meter_id,
  ...
  *stats_mask = 0;
  ...
  if (fm->drop_cnt) {
    ret = mlx5_counter_query(dev, fm->drop_cnt, clear, &pkts, &bytes, NULL);
    if (ret)
      goto error;
 ...
 *   Always 0.
 */
int mlx5_flow_meter_ops_get(struct rte_eth_dev *dev __rte_unused, void *arg) {
...
struct mlx5_flow_meter_info *mlx5_flow_meter_find(struct mlx5_priv *priv,
  ...
  if (priv->sh->meter_aso_en) {
    ...
    if (!n_valid || !priv->mtr_idx_tbl ||
        (mlx5_l3t_get_entry(priv->mtr_idx_tbl, meter_id, &data) || !data.dword))
      return NULL;
    if (mtr_idx)
      *mtr_idx = data.dword;
 ...
 *   Pointer to the meter info found on success, NULL otherwise.
 */
struct mlx5_flow_meter_info *flow_dv_meter_find_by_idx(struct mlx5_priv *priv,
                                                       uint32_t idx) {
  struct mlx5_aso_mtr *aso_mtr;
  ...
  if (priv->sh->meter_aso_en) {
    ...
    if (mlx5_aso_mtr_wait(priv->sh, aso_mtr)) {
      return rte_flow_error_set(error, ENOENT, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "Timeout in meter configuration");
    }
    ...
    rte_spinlock_lock(&fm->sl);
    if (fm->meter_action) {
      if (fm->shared && attr->transfer == fm->transfer &&
          attr->ingress == fm->ingress && attr->egress == fm->egress) {
        fm->ref_cnt++;
      } else {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                           fm->shared ? "Meter attr not match."
                                      : "Meter cannot be shared.");
        ret = -1;
      }
    } else {
      ...
      fm->meter_action = mlx5_flow_meter_action_create(priv, fm);
      if (!fm->meter_action) {
        fm->ref_cnt = 0;
        fm->ingress = 0;
        fm->egress = 0;
...
void mlx5_flow_meter_rxq_flush(struct rte_eth_dev *dev) {
  ...
  if (priv->policy_idx_tbl) {
    MLX5_L3T_FOREACH(priv->policy_idx_tbl, i, entry) {
      ...
      sub_policy =
          mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_MTR_POLICY], policy_idx);
      if (!sub_policy || !sub_policy->main_policy)
        continue;
      mtr_policy = sub_policy->main_policy;
      if (mtr_policy->is_queue || mtr_policy->is_rss)
        mlx5_flow_destroy_sub_policy_with_rxq(dev, mtr_policy);
    }
  }
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_flush_hierarchy(struct rte_eth_dev *dev,
                                           struct mlx5_flow_meter_info *fm,
                                           uint32_t mtr_idx,
                                           struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  while (!fm->ref_cnt && policy->is_hierarchy) {
    ...
    next_fm = mlx5_flow_meter_find(
        priv, policy->act_cnt[RTE_COLOR_GREEN].next_mtr_id, &next_mtr_idx);
    if (next_fm) {
      next_policy = mlx5_flow_meter_policy_find(dev, next_fm->policy_id, NULL);
      MLX5_ASSERT(next_policy);
    }
    if (mlx5_flow_meter_params_flush(dev, fm, mtr_idx))
      return -rte_mtr_error_set(error, ENOTSUP, RTE_MTR_ERROR_TYPE_MTR_ID, NULL,
                                "Failed to flush meter.");
    if (policy->ref_cnt)
      break;
    if (__mlx5_flow_meter_policy_delete(dev, policy_id, policy, error, true))
      return -rte_errno;
    mlx5_free(policy);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_flow_meter_flush_all_hierarchies(struct rte_eth_dev *dev,
                                                 struct rte_mtr_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_meter_info *fm;
  struct mlx5_flow_meter_policy *policy;
  ...
  MLX5_L3T_FOREACH(priv->policy_idx_tbl, i, entry) {
    ...
    if (!policy || !policy->is_hierarchy || policy->ref_cnt)
      continue;
    next_fm = mlx5_flow_meter_find(
        priv, policy->act_cnt[RTE_COLOR_GREEN].next_mtr_id, &mtr_idx);
    if (__mlx5_flow_meter_policy_delete(dev, i, policy, error, true))
      return -rte_mtr_error_set(error, EINVAL,
                                RTE_MTR_ERROR_TYPE_METER_POLICY_ID, NULL,
                                "Meter policy invalid.");
    mlx5_free(policy);
    if (!next_fm || next_fm->ref_cnt || next_fm->def_policy)
      continue;
    if (mlx5_flow_meter_flush_hierarchy(dev, next_fm, mtr_idx, error))
      return -rte_errno;
  }
  return 0;
}
...
int mlx5_flow_meter_flush(struct rte_eth_dev *dev,
    ...
    if (priv->mtr_idx_tbl) {
      MLX5_L3T_FOREACH(priv->mtr_idx_tbl, i, entry) {
        ...
        if (mtr_idx) {
          aso_mtr = mlx5_aso_meter_by_idx(priv, mtr_idx);
          fm = &aso_mtr->fm;
          (void)mlx5_flow_meter_params_flush(dev, fm, mtr_idx);
        }
      }
    ...
    MLX5_L3T_FOREACH(priv->policy_idx_tbl, i, entry) {
      ...
      if (!sub_policy)
        return -rte_mtr_error_set(error, EINVAL,
                                  ...
                                  "MTR object "
                                  "meter policy invalid.");
      if (__mlx5_flow_meter_policy_delete(dev, i, sub_policy->main_policy,
                                          error, true))
        return -rte_mtr_error_set(error, EINVAL,
                                  RTE_MTR_ERROR_TYPE_METER_POLICY_ID, NULL,
                                  "MTR object "
  ...
  if (priv->mtr_profile_tbl) {
    MLX5_L3T_FOREACH(priv->mtr_profile_tbl, i, entry) {
      fmp = entry;
      if (mlx5_flow_meter_profile_delete(dev, fmp->id, error))
        return -rte_mtr_error_set(error, EINVAL,
                                  RTE_MTR_ERROR_TYPE_METER_POLICY_ID, NULL,
                                  "Fail to destroy "
                                  "meter profile.");
