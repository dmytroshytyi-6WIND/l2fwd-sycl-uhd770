 ...
 */
static int flow_verbs_discover_priorities(struct rte_eth_dev *dev,
...
out:
  ...
  DRV_LOG(INFO,
          ...
          dev->data->port_id, priority - 2, MLX5_NON_ROOT_FLOW_MAX_PRIO - 1);
  return priority;
}
...
flow_verbs_counter_get_by_idx(struct rte_eth_dev *dev, uint32_t idx,
 ...
 */
static int flow_verbs_counter_create(struct rte_eth_dev *dev,
                                     struct mlx5_flow_counter *counter) {
#if defined(HAVE_IBV_DEVICE_COUNTERS_SET_V42)
  struct mlx5_priv *priv = dev->data->dev_private;
  struct ibv_context *ctx = priv->sh->cdev->ctx;
  struct ibv_counter_set_init_attr init = {.counter_set_id =
                                               counter->shared_info.id};
 ...
 */
static uint32_t flow_verbs_counter_new(struct rte_eth_dev *dev,
                                       uint32_t id __rte_unused) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_counter_mng *cmng = &priv->sh->cmng;
  ...
  if (!cnt) {
    ...
    if (n_valid == cmng->n) {
      ...
      pools = mlx5_malloc(MLX5_MEM_ZERO, size, 0, SOCKET_ID_ANY);
      if (!pools)
        return 0;
      if (n_valid) {
        memcpy(pools, cmng->pools,
               sizeof(struct mlx5_flow_counter_pool *) * n_valid);
 ...
 */
static void flow_verbs_counter_release(struct rte_eth_dev *dev,
                                       uint32_t counter) {
  struct mlx5_flow_counter_pool *pool;
...
#elif defined(HAVE_IBV_DEVICE_COUNTERS_SET_V45)
  claim_zero(
      mlx5_glue->destroy_counters((struct ibv_counters *)cnt->dcs_when_active));
#endif
  TAILQ_INSERT_HEAD(&pool->counters[0], cnt, next);
}
...
static int flow_verbs_counter_query(struct rte_eth_dev *dev __rte_unused,
  ...
  if (flow->counter) {
    struct mlx5_flow_counter_pool *pool;
    struct mlx5_flow_counter *cnt =
        flow_verbs_counter_get_by_idx(dev, flow->counter, &pool);
    struct rte_flow_query_count *qc = data;
    uint64_t counters[2] = {0, 0};
#if defined(HAVE_IBV_DEVICE_COUNTERS_SET_V42)
    struct ibv_query_counter_set_attr query_cs_attr = {
        .dcs_when_free = (struct ibv_counter_set *)cnt->dcs_when_active,
    ...
    int err = mlx5_glue->query_counters(
        ...
        RTE_DIM(counters), IBV_READ_COUNTERS_ATTR_PREFER_CACHED);
#endif
    if (err)
      return rte_flow_error_set(error, err, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                                NULL, "cannot read counter");
    qc->hits_set = 1;
    qc->bytes_set = 1;
  ...
  return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, "flow does not have counter");
#else
  (void)flow;
  (void)data;
 ...
 */
static void flow_verbs_spec_add(struct mlx5_flow_verbs_workspace *verbs,
 ...
 *   Parsed item flags.
 */
static void flow_verbs_translate_item_eth(struct mlx5_flow *dev_flow,
 ...
 */
static void flow_verbs_item_vlan_update(struct ibv_flow_attr *attr,
                                        struct ibv_flow_spec_eth *eth) {
  unsigned int i;
  const enum ibv_flow_spec_type search = eth->type;
  struct ibv_spec_header *hdr =
      (struct ibv_spec_header *)((uint8_t *)attr +
                                 sizeof(struct ibv_flow_attr));

  for (i = 0; i != attr->num_of_specs; ++i) {
    if (hdr->type == search) {
      struct ibv_flow_spec_eth *e = (struct ibv_flow_spec_eth *)hdr;
 ...
 *   Parsed item flags.
 */
static void flow_verbs_translate_item_vlan(struct mlx5_flow *dev_flow,
                                           const struct rte_flow_item *item,
                                           uint64_t item_flags) {
  ...
  if (!tunnel)
    dev_flow->handle->vf_vlan.tag = rte_be_to_cpu_16(spec->tci) & 0x0fff;
}
 ...
 *   Parsed item flags.
 */
static void flow_verbs_translate_item_ipv4(struct mlx5_flow *dev_flow,
                                           const struct rte_flow_item *item,
                                           uint64_t item_flags) {
 ...
 *   Parsed item flags.
 */
static void flow_verbs_translate_item_ipv6(struct mlx5_flow *dev_flow,
                                           const struct rte_flow_item *item,
                                           uint64_t item_flags) {
  ...
  if (spec) {
    ...
    memcpy(&ipv6.val.src_ip, spec->hdr.src_addr, RTE_DIM(ipv6.val.src_ip));
    memcpy(&ipv6.val.dst_ip, spec->hdr.dst_addr, RTE_DIM(ipv6.val.dst_ip));
    memcpy(&ipv6.mask.src_ip, mask->hdr.src_addr, RTE_DIM(ipv6.mask.src_ip));
    memcpy(&ipv6.mask.dst_ip, mask->hdr.dst_addr, RTE_DIM(ipv6.mask.dst_ip));
    vtc_flow_val = rte_be_to_cpu_32(spec->hdr.vtc_flow);
    vtc_flow_mask = rte_be_to_cpu_32(mask->hdr.vtc_flow);
    ipv6.val.flow_label = rte_cpu_to_be_32(
        (vtc_flow_val & RTE_IPV6_HDR_FL_MASK) >> RTE_IPV6_HDR_FL_SHIFT);
    ipv6.val.traffic_class =
        (vtc_flow_val & RTE_IPV6_HDR_TC_MASK) >> RTE_IPV6_HDR_TC_SHIFT;
    ipv6.val.next_hdr = spec->hdr.proto;
    ipv6.mask.flow_label = rte_cpu_to_be_32(
        (vtc_flow_mask & RTE_IPV6_HDR_FL_MASK) >> RTE_IPV6_HDR_FL_SHIFT);
    ipv6.mask.traffic_class =
        (vtc_flow_mask & RTE_IPV6_HDR_TC_MASK) >> RTE_IPV6_HDR_TC_SHIFT;
 ...
 *   Parsed item flags.
 */
static void flow_verbs_translate_item_tcp(struct mlx5_flow *dev_flow,
                                          const struct rte_flow_item *item,
 ...
 *   Parsed item flags.
 */
static void flow_verbs_translate_item_udp(struct mlx5_flow *dev_flow,
                                          const struct rte_flow_item *item,
                                          uint64_t item_flags __rte_unused) {
 ...
 *   Parsed item flags.
 */
static void flow_verbs_translate_item_vxlan(struct mlx5_flow *dev_flow,
                                            const struct rte_flow_item *item,
                                            uint64_t item_flags __rte_unused) {
 ...
 */
static uint8_t *flow_verbs_reserve_gre(struct mlx5_flow *dev_flow) {
  uint8_t *buffer;
 ...
 */
static void
flow_verbs_translate_item_gre(struct mlx5_flow *dev_flow, uint8_t *gre_spec,
  ...
  if (tunnel.mask.protocol) {
    ...
    if (tunnel.val.protocol) {
      ...
      tunnel.val.protocol = rte_cpu_to_be_16(tunnel.val.protocol);
    }
  }
#endif
  if (item_flags & MLX5_FLOW_LAYER_OUTER_L3_IPV4)
    flow_verbs_item_gre_ip_protocol_update(&verbs->attr, IBV_FLOW_SPEC_IPV4_EXT,
                                           IPPROTO_GRE);
  else
    flow_verbs_item_gre_ip_protocol_update(&verbs->attr, IBV_FLOW_SPEC_IPV6,
                                           IPPROTO_GRE);
 ...
 */
static void flow_verbs_translate_action_drop(
 ...
 *   Action configuration.
 */
static void flow_verbs_translate_action_flag(
    struct mlx5_flow *dev_flow,
    const struct rte_flow_action *action __rte_unused) {
 ...
 */
static int flow_verbs_translate_action_count(
    struct mlx5_flow *dev_flow, const struct rte_flow_action *action,
    struct rte_eth_dev *dev, struct rte_flow_error *error) {
 ...
 */
static int flow_verbs_validate(struct rte_eth_dev *dev,
  ...
  for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
    ...
    case RTE_FLOW_ITEM_TYPE_ETH:
      ret = mlx5_flow_validate_item_eth(items, item_flags, false, error);
      if (ret < 0)
        return ret;
    ...
    case RTE_FLOW_ITEM_TYPE_VLAN:
      ret = mlx5_flow_validate_item_vlan(items, item_flags, dev, error);
      if (ret < 0)
        return ret;
      last_item = tunnel
    ...
    case RTE_FLOW_ITEM_TYPE_IPV4:
      ret = mlx5_flow_validate_item_ipv4(items, item_flags, last_item,
                                         ether_type, NULL,
  ...
  if (is_empty_vlan)
    return rte_flow_error_set(
        error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_FLAG:
      ret = mlx5_flow_validate_action_flag(action_flags, attr, error);
      if (ret < 0)
        return ret;
      action_flags |= MLX5_FLOW_ACTION_FLAG;
      break;
    case RTE_FLOW_ACTION_TYPE_MARK:
      ret = mlx5_flow_validate_action_mark(actions, action_flags, attr, error);
      if (ret < 0)
        return ret;
 ...
 */
static int flow_verbs_get_actions_size(const struct rte_flow_action actions[]) {
  int size = 0;
 ...
 */
static int flow_verbs_get_items_size(const struct rte_flow_item items[]) {
  int size = 0;
 ...
 *   is set.
 */
static struct mlx5_flow *flow_verbs_prepare(
    struct rte_eth_dev *dev, const struct rte_flow_attr *attr __rte_unused,
 ...
 */
static int flow_verbs_translate(struct rte_eth_dev *dev,
                                struct mlx5_flow *dev_flow,
                                const struct rte_flow_attr *attr,
                                const struct rte_flow_item items[],
                                const struct rte_flow_action actions[],
  ...
  for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
    ...
    case RTE_FLOW_ACTION_TYPE_RSS:
      ...
      break;
    case RTE_FLOW_ACTION_TYPE_COUNT:
      ret = flow_verbs_translate_action_count(dev_flow, actions, dev, error);
      if (ret < 0)
        return ret;
      action_flags |= MLX5_FLOW_ACTION_COUNT;
      break;
  ...
  for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
    ...
    case RTE_FLOW_ITEM_TYPE_VOID:
      break;
    case RTE_FLOW_ITEM_TYPE_ETH:
      flow_verbs_translate_item_eth(dev_flow, items, item_flags);
      subpriority = MLX5_PRIORITY_MAP_L2;
      item_flags |=
          tunnel ? MLX5_FLOW_LAYER_INNER_L2 : MLX5_FLOW_LAYER_OUTER_L2;
      break;
    case RTE_FLOW_ITEM_TYPE_VLAN:
      flow_verbs_translate_item_vlan(dev_flow, items, item_flags);
      subpriority = MLX5_PRIORITY_MAP_L2;
      item_flags |=
          tunnel ? (MLX5_FLOW_LAYER_INNER_L2 | MLX5_FLOW_LAYER_INNER_VLAN)
                 : (MLX5_FLOW_LAYER_OUTER_L2 | MLX5_FLOW_LAYER_OUTER_VLAN);
      break;
    case RTE_FLOW_ITEM_TYPE_IPV4:
      flow_verbs_translate_item_ipv4(dev_flow, items, item_flags);
      subpriority = MLX5_PRIORITY_MAP_L3;
      dev_flow->hash_fields |= mlx5_flow_hashfields_adjust(
          rss_desc, tunnel, MLX5_IPV4_LAYER_TYPES, MLX5_IPV4_IBV_RX_HASH);
      item_flags |= tunnel ? MLX5_FLOW_LAYER_INNER_L3_IPV4
                           : MLX5_FLOW_LAYER_OUTER_L3_IPV4;
      break;
    case RTE_FLOW_ITEM_TYPE_IPV6:
      flow_verbs_translate_item_ipv6(dev_flow, items, item_flags);
      subpriority = MLX5_PRIORITY_MAP_L3;
      dev_flow->hash_fields |= mlx5_flow_hashfields_adjust(
          rss_desc, tunnel, MLX5_IPV6_LAYER_TYPES, MLX5_IPV6_IBV_RX_HASH);
      item_flags |= tunnel ? MLX5_FLOW_LAYER_INNER_L3_IPV6
                           : MLX5_FLOW_LAYER_OUTER_L3_IPV6;
      break;
    case RTE_FLOW_ITEM_TYPE_TCP:
      flow_verbs_translate_item_tcp(dev_flow, items, item_flags);
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_UDP:
      flow_verbs_translate_item_udp(dev_flow, items, item_flags);
      subpriority = MLX5_PRIORITY_MAP_L4;
      if (dev_flow->hash_fields != 0)
        dev_flow->hash_fields |= mlx5_flow_hashfields_adjust(
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_VXLAN:
      flow_verbs_translate_item_vxlan(dev_flow, items, item_flags);
      subpriority = MLX5_TUNNEL_PRIO_GET(rss_desc);
      item_flags |= MLX5_FLOW_LAYER_VXLAN;
      break;
    case RTE_FLOW_ITEM_TYPE_VXLAN_GPE:
      flow_verbs_translate_item_vxlan_gpe(dev_flow, items, item_flags);
      subpriority = MLX5_TUNNEL_PRIO_GET(rss_desc);
    ...
    case RTE_FLOW_ITEM_TYPE_GRE:
      ...
      break;
    case RTE_FLOW_ITEM_TYPE_MPLS:
      flow_verbs_translate_item_mpls(dev_flow, items, item_flags);
      subpriority = MLX5_TUNNEL_PRIO_GET(rss_desc);
  ...
  if (item_flags & MLX5_FLOW_LAYER_GRE)
    flow_verbs_translate_item_gre(dev_flow, gre_spec, tunnel_item, item_flags);
 ...
 */
static void flow_verbs_remove(struct rte_eth_dev *dev, struct rte_flow *flow) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  SILIST_FOREACH(priv->sh->ipool[MLX5_IPOOL_MLX5_FLOW], flow->dev_handles,
    ...
    if (handle->rix_hrxq && handle->fate_action == MLX5_FLOW_FATE_QUEUE) {
 ...
 *   Pointer to flow structure.
 */
static void flow_verbs_destroy(struct rte_eth_dev *dev, struct rte_flow *flow) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_handle *handle;
  ...
  while (flow->dev_handles) {
    ...
    handle = mlx5_ipool_get(priv->sh->ipool[MLX5_IPOOL_MLX5_FLOW], tmp_idx);
    if (!handle)
      return;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_verbs_apply(struct rte_eth_dev *dev, struct rte_flow *flow,
                            struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flow_handle *handle;
  struct mlx5_flow *dev_flow;
  ...
  for (idx = wks->flow_idx - 1; idx >= 0; idx--) {
    ...
    if (handle->fate_action == MLX5_FLOW_FATE_DROP) {
      ...
      rss_desc->tunnel = !!(handle->layers & MLX5_FLOW_LAYER_TUNNEL);
    ...
    MLX5_ASSERT(hrxq);
    handle->drv_flow = mlx5_glue->create_flow(hrxq->qp, &dev_flow->verbs.attr);
    if (!handle->drv_flow) {
      rte_flow_error_set(error, errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                         "hardware refuses to create flow");
      goto error;
    }
    if (priv->vmwa_context && handle->vf_vlan.tag && !handle->vf_vlan.created) {
      /*
 ...
 * @see rte_flow_ops
 */
static int flow_verbs_query(struct rte_eth_dev *dev, struct rte_flow *flow,
...
static int flow_verbs_sync_domain(struct rte_eth_dev *dev, uint32_t domains,
                                  uint32_t flags) {
  RTE_SET_USED(dev);
