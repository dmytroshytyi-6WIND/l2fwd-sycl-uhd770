...
#define mlx5_mprq_buf_addr(ptr, strd_n)                                        \
  (RTE_PTR_ADD((ptr), sizeof(struct mlx5_mprq_buf) +                           \
                          (strd_n) * sizeof(struct rte_mbuf_ext_shared_info) + \
 ...
 */
static __rte_always_inline uint32_t mlx5_rx_addr2mr(struct mlx5_rxq_data *rxq,
                                                    uintptr_t addr) {
  ...
  lkey =
      mlx5_mr_lookup_lkey(mr_ctrl->cache, &mr_ctrl->mru, MLX5_MR_CACHE_N, addr);
 ...
 *   This function always succeeds on valid input.
 */
static __rte_always_inline uint32_t mlx5_rx_mb2mr(struct mlx5_rxq_data *rxq,
                                                  struct rte_mbuf *mb) {
  struct mlx5_mr_ctrl *mr_ctrl = &rxq->mr_ctrl;
 ...
 *   RQ index to replace.
 */
static __rte_always_inline void mprq_buf_replace(struct mlx5_rxq_data *rxq,
                                                 uint16_t rq_idx) {
...
mprq_buf_to_pkt(struct mlx5_rxq_data *rxq, struct rte_mbuf *pkt, uint32_t len,
                struct mlx5_mprq_buf *buf, uint16_t strd_idx,
                uint16_t strd_cnt) {
  const uint32_t strd_n = RTE_BIT32(rxq->log_strd_num);
  const uint16_t strd_sz = RTE_BIT32(rxq->log_strd_sz);
  const uint16_t strd_shift = MLX5_MPRQ_STRIDE_SHIFT_BYTE * rxq->strd_shift_en;
  const int32_t hdrm_overlap = len + RTE_PKTMBUF_HEADROOM - strd_cnt * strd_sz;
  ...
  if (len <= rxq->mprq_max_memcpy_len || rxq->mprq_repl == NULL ||
    ...
    if (likely(len <= (uint32_t)(pkt->buf_len - RTE_PKTMBUF_HEADROOM))) {
      rte_memcpy(rte_pktmbuf_mtod(pkt, void *), addr, len);
      DATA_LEN(pkt) = len;
    } else if (rxq->strd_scatter_en) {
      struct rte_mbuf *prev = pkt;
      uint32_t seg_len =
          RTE_MIN(len, (uint32_t)(pkt->buf_len - RTE_PKTMBUF_HEADROOM));
      ...
      while (rem_len) {
        ...
        seg_len =
            RTE_MIN(rem_len, (uint32_t)(next->buf_len - RTE_PKTMBUF_HEADROOM));
        rte_memcpy(rte_pktmbuf_mtod(next, void *), addr, seg_len);
    ...
    MLX5_ASSERT(__atomic_load_n(&buf->refcnt, __ATOMIC_RELAXED) <= strd_n + 1);
     ...
     * different PMD.
     */
    buf_iova = rte_mempool_virt2iova(buf) + RTE_PTR_DIFF(buf_addr, buf);
    ...
    if (hdrm_overlap > 0) {
      ...
      rte_memcpy(rte_pktmbuf_mtod(seg, void *),
                 RTE_PTR_ADD(addr, len - hdrm_overlap), hdrm_overlap);
 ...
 *   0 if disabled, otherwise enabled.
 */
static __rte_always_inline int mlx5_mprq_enabled(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  uint32_t i;
  uint16_t n = 0;
  uint16_t n_ibv = 0;
 ...
 */
static __rte_always_inline bool mlx5_is_external_rxq(struct rte_eth_dev *dev,
                                                     uint16_t queue_idx) {
  struct mlx5_priv *priv = dev->data->dev_private;
