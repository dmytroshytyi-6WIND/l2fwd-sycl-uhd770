...
int mlx5_dev_configure(struct rte_eth_dev *dev) {
  ...
  priv->rss_conf.rss_key =
      mlx5_realloc(priv->rss_conf.rss_key, MLX5_MEM_RTE, MLX5_RSS_HASH_KEY_LEN,
                   0, SOCKET_ID_ANY);
  ...
  memcpy(priv->rss_conf.rss_key,
         use_app_rss_key ? dev->data->dev_conf.rx_adv_conf.rss_conf.rss_key
                         : rss_hash_default_key,
         MLX5_RSS_HASH_KEY_LEN);
  priv->rss_conf.rss_key_len = MLX5_RSS_HASH_KEY_LEN;
  priv->rss_conf.rss_hf = dev->data->dev_conf.rx_adv_conf.rss_conf.rss_hf;
  priv->rxq_privs = mlx5_realloc(priv->rxq_privs, MLX5_MEM_RTE | MLX5_MEM_ZERO,
                                 sizeof(void *) * rxqs_n, 0, SOCKET_ID_ANY);
  ...
  if (priv->ext_rxqs && rxqs_n >= MLX5_EXTERNAL_RX_QUEUE_ID_MIN) {
    DRV_LOG(ERR,
            "port %u cannot handle this many Rx queues (%u), "
            "the maximal number of internal Rx queues is %u",
            dev->data->port_id, rxqs_n, MLX5_EXTERNAL_RX_QUEUE_ID_MIN - 1);
    rte_errno = EINVAL;
    return -rte_errno;
  }
...
int mlx5_dev_configure_rss_reta(struct rte_eth_dev *dev) {
  ...
  if (priv->skip_default_rss_reta)
    return ret;
  rss_queue_arr =
      mlx5_malloc(0, rxqs_n * sizeof(unsigned int), 0, SOCKET_ID_ANY);
  if (!rss_queue_arr) {
  ...
  DRV_LOG(INFO, "port %u Rx queues number update: %u -> %u", dev->data->port_id,
          priv->rxqs_n, rxqs_n);
  priv->rxqs_n = rxqs_n;
  /*
 ...
 */
static void mlx5_set_default_params(struct rte_eth_dev *dev,
 ...
 *   Info structure output buffer.
 */
static void mlx5_set_txlimit_params(struct rte_eth_dev *dev,
                                    struct rte_eth_dev_info *info) {
  struct mlx5_priv *priv = dev->data->dev_private;
  ...
  inlen = RTE_MIN(inlen, MLX5_WQE_SIZE_MAX + MLX5_ESEG_MIN_INLINE_SIZE -
                             MLX5_WQE_CSEG_SIZE - MLX5_WQE_ESEG_SIZE -
                             MLX5_WQE_DSEG_SIZE * 2);
  nb_max = (MLX5_WQE_SIZE_MAX + MLX5_ESEG_MIN_INLINE_SIZE - MLX5_WQE_CSEG_SIZE -
            MLX5_WQE_ESEG_SIZE - MLX5_WQE_DSEG_SIZE - inlen) /
           MLX5_WSEG_SIZE;
...
int mlx5_dev_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *info) {
  ...
  info->reta_size = priv->reta_idx_n ? priv->reta_idx_n
                                     : priv->sh->dev_cap.ind_table_max_size;
  ...
  if (priv->representor) {
    ...
    MLX5_ETH_FOREACH_DEV(port_id, dev->device) {
      ...
      if (!opriv || opriv->representor || opriv->sh != priv->sh ||
...
int mlx5_representor_info_get(struct rte_eth_dev *dev,
  ...
  for (pf = 0; pf < n_pf; ++pf) {
    ...
    info->ranges[i].pf = pf;
    info->ranges[i].vf = 0;
    info->ranges[i].id_base = MLX5_REPRESENTOR_ID(pf, info->ranges[i].type, 0);
    info->ranges[i].id_end = MLX5_REPRESENTOR_ID(pf, info->ranges[i].type, -1);
    snprintf(info->ranges[i].name, sizeof(info->ranges[i].name), "pf%dvf", pf);
    i++;
    if (i == n_entries)
      break;
    ...
    info->ranges[i].id_base = MLX5_REPRESENTOR_ID(pf, info->ranges[i].type, -1);
    info->ranges[i].id_end = MLX5_REPRESENTOR_ID(pf, info->ranges[i].type, -1);
    snprintf(info->ranges[i].name, sizeof(info->ranges[i].name), "pf%dvf", pf);
    i++;
    if (i == n_entries)
    ...
    info->ranges[i].id_base = MLX5_REPRESENTOR_ID(pf, info->ranges[i].type, 0);
    info->ranges[i].id_end = MLX5_REPRESENTOR_ID(pf, info->ranges[i].type, -1);
    snprintf(info->ranges[i].name, sizeof(info->ranges[i].name), "pf%dsf", pf);
    i++;
    if (i == n_entries)
      break;
...
const uint32_t *mlx5_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
  static const uint32_t ptypes[] = {/* refers to rxq_cq_to_pkt_type() */
                                    RTE_PTYPE_L2_ETHER,
...
int mlx5_dev_set_mtu(struct rte_eth_dev *dev, uint16_t mtu) {
  ...
  if (kern_mtu == mtu) {
    priv->mtu = mtu;
    DRV_LOG(DEBUG, "port %u adapter MTU set to %u", dev->data->port_id, mtu);
    return 0;
  }
  rte_errno = EAGAIN;
  return -rte_errno;
...
eth_rx_burst_t mlx5_select_rx_function(struct rte_eth_dev *dev) {
  ...
  if (mlx5_check_vec_rx_support(dev) > 0) {
    ...
    DRV_LOG(DEBUG, "port %u selected MPRQ Rx function", dev->data->port_id);
  } else {
    DRV_LOG(DEBUG, "port %u selected SPRQ Rx function", dev->data->port_id);
  }
  return rx_pkt_burst;
}
