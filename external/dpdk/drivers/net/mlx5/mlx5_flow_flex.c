...
void mlx5_flex_item_port_cleanup(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < MLX5_PORT_FLEX_ITEM_NUM && priv->flex_item_map; i++) {
    if (priv->flex_item_map & (1 << i)) {
      ...
      claim_zero(mlx5_list_unregister(priv->sh->flex_parsers_dv,
                                      &flex->devx_fp->entry));
      flex->devx_fp = NULL;
      flex->refcnt = 0;
...
static int mlx5_flex_index(struct mlx5_priv *priv,
  ...
  if (entry < start || idx >= MLX5_PORT_FLEX_ITEM_NUM ||
...
static struct mlx5_flex_item *mlx5_flex_alloc(struct mlx5_priv *priv) {
...
static void mlx5_flex_free(struct mlx5_priv *priv,
                           struct mlx5_flex_item *item) {
  int idx = mlx5_flex_index(priv, item);

  MLX5_ASSERT(idx >= 0 && idx < MLX5_PORT_FLEX_ITEM_NUM &&
              (priv->flex_item_map & (1u << idx)));
  if (idx >= 0) {
    rte_spinlock_lock(&priv->flex_item_sl);
    MLX5_ASSERT(!item->refcnt);
...
static uint32_t mlx5_flex_get_bitfield(const struct rte_flow_item_flex *item,
...
#define SET_FP_MATCH_SAMPLE_ID(x, def, msk, val, sid)                          \
  do {                                                                         \
    ...
    tmp = MLX5_GET(fte_match_set_misc4, misc4_v, prog_sample_field_value_##x); \
    tmp = (tmp & ~out) | (val);                                                \
    MLX5_SET(fte_match_set_misc4, misc4_v, prog_sample_field_value_##x, tmp);  \
    tmp = MLX5_GET(fte_match_set_misc4, misc4_m, prog_sample_field_value_##x); \
    tmp = (tmp & ~out) | (msk);                                                \
    MLX5_SET(fte_match_set_misc4, misc4_m, prog_sample_field_value_##x, tmp);  \
    tmp = tmp ? (sid) : 0;                                                     \
    MLX5_SET(fte_match_set_misc4, misc4_v, prog_sample_field_id_##x, tmp);     \
    MLX5_SET(fte_match_set_misc4, misc4_m, prog_sample_field_id_##x, tmp);     \
  } while (0)

__rte_always_inline static void
mlx5_flex_set_match_sample(void *misc4_m, void *misc4_v, uint32_t def,
                           uint32_t mask, uint32_t value, uint32_t sample_id,
                           uint32_t id) {
  switch (id) {
  case 0:
...
void mlx5_flex_flow_translate_item(struct rte_eth_dev *dev, void *matcher,
  ...
  for (i = 0; i < tp->mapnum; i++) {
    ...
    mlx5_flex_set_match_sample(misc4_m, misc4_v, def, msk & def,
                               val & msk & def, tp->devx_fp->sample_ids[id],
                               id);
    pos += map->width;
  }
}
 ...
 *    5     b000111  0x07
 */
static uint8_t mlx5_flex_hdr_len_mask(uint8_t shift,
  ...
  return diff == 0  ? base_mask
         : diff < 0 ? (base_mask << -diff) & base_mask
                    : base_mask >> diff;
}

static int mlx5_flex_translate_length(
    struct mlx5_hca_flex_attr *attr, const struct rte_flow_item_flex_conf *conf,
    struct mlx5_flex_parser_devx *devx, struct rte_flow_error *error) {
  ...
  case FIELD_MODE_FIXED:
    if (!(attr->header_length_mode & RTE_BIT32(MLX5_GRAPH_NODE_LEN_FIXED)))
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                                "unsupported header length field mode (FIXED)");
    if (field->field_size || field->offset_mask || field->offset_shift)
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
  ...
  case FIELD_MODE_OFFSET:
    if (!(attr->header_length_mode & RTE_BIT32(MLX5_GRAPH_NODE_LEN_FIELD)))
    ...
    if (field->offset_mask == 0 || !rte_is_power_of_2(field->offset_mask + 1))
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
    ...
    node->header_length_field_mask = RTE_MIN(mask, field->offset_mask);
    break;
  case FIELD_MODE_BITMASK:
    if (!(attr->header_length_mode & RTE_BIT32(MLX5_GRAPH_NODE_LEN_BITMASK)))
      return rte_flow_error_set(
          error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
...
static int mlx5_flex_translate_next(struct mlx5_hca_flex_attr *attr,
...
static void mlx5_flex_insert_field(struct mlx5_flex_field_cover *cover,
...
static void mlx5_flex_merge_field(struct mlx5_flex_field_cover *cover,
                                  uint16_t num) {
  uint32_t i, del = 0;
  int32_t end;
  ...
  if (del) {
    ...
    if ((cover->num - num) > 1) {
      memmove(&cover->start[num + 1], &cover->start[num + 1 + del],
              (cover->num - num - 1) * sizeof(int32_t));
      memmove(&cover->end[num + 1], &cover->end[num + 1 + del],
              (cover->num - num - 1) * sizeof(int32_t));
    }
  }
}
 ...
 */
static int mlx5_flex_cover_sample(struct mlx5_flex_field_cover *cover,
  ...
  case FIELD_MODE_DUMMY:
    return 0;
  case FIELD_MODE_FIXED:
    if (!(attr->sample_offset_mode & RTE_BIT32(MLX5_GRAPH_SAMPLE_OFFSET_FIXED)))
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
    ...
    break;
  case FIELD_MODE_OFFSET:
    if (!(attr->sample_offset_mode & RTE_BIT32(MLX5_GRAPH_SAMPLE_OFFSET_FIELD)))
      return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
  ...
  for (i = 0; i < cover->num; i++) {
    if (start > cover->end[i]) {
      if (i >= (cover->num - 1u)) {
        mlx5_flex_insert_field(cover, cover->num, start, end);
        break;
      }
      continue;
    }
...
mlx5_flex_config_sample(struct mlx5_devx_match_sample_attr *na,
  ...
  switch (field->field_mode) {
  case FIELD_MODE_FIXED:
    na->flow_match_sample_offset_mode = MLX5_GRAPH_SAMPLE_OFFSET_FIXED;
    break;
  case FIELD_MODE_OFFSET:
    na->flow_match_sample_offset_mode = MLX5_GRAPH_SAMPLE_OFFSET_FIELD;
    ...
    break;
  case FIELD_MODE_BITMASK:
    na->flow_match_sample_offset_mode = MLX5_GRAPH_SAMPLE_OFFSET_BITMASK;
    na->flow_match_sample_field_offset = field->offset_base;
    na->flow_match_sample_field_offset_mask = field->offset_mask;
    na->flow_match_sample_field_offset_shift = field->offset_shift;
    break;
  ...
  case FLEX_TUNNEL_MODE_TUNNEL:
    na->flow_match_sample_tunnel_mode = MLX5_GRAPH_SAMPLE_TUNNEL_FIRST;
    break;
  case FLEX_TUNNEL_MODE_MULTI:
    /* Fallthrough */
  case FLEX_TUNNEL_MODE_OUTER:
    na->flow_match_sample_tunnel_mode = MLX5_GRAPH_SAMPLE_TUNNEL_OUTER;
    break;
  case FLEX_TUNNEL_MODE_INNER:
    na->flow_match_sample_tunnel_mode = MLX5_GRAPH_SAMPLE_TUNNEL_INNER;
    break;
  default:
    MLX5_ASSERT(false);
    break;
...
static int mlx5_flex_map_sample(struct rte_flow_item_flex_field *field,
  ...
  if (field->field_mode == FIELD_MODE_DUMMY) {
    ...
    while (done_bits) {
      uint32_t part = RTE_MIN(done_bits, sizeof(uint32_t) * CHAR_BIT);
  ...
  for (i = 0; i < parser->num_samples; i++) {
    struct mlx5_devx_match_sample_attr *sample = &parser->devx_conf.sample[i];
...
static int mlx5_flex_alloc_sample(struct mlx5_flex_field_cover *cover,
                                  struct mlx5_flex_parser_devx *parser,
                                  struct mlx5_flex_item *item,
                                  struct rte_flow_item_flex_field *field,
                                  struct mlx5_hca_flex_attr *attr,
  ...
  while (idx < cover->num) {
     ...
     */
    start = RTE_ALIGN_FLOOR(cover->start[idx], sizeof(uint32_t) * CHAR_BIT);
    node.flow_match_sample_field_base_offset = (start / CHAR_BIT) & 0xFF;
...
static int mlx5_flex_translate_sample(
    struct mlx5_hca_flex_attr *attr, const struct rte_flow_item_flex_conf *conf,
    struct mlx5_flex_parser_devx *parser, struct mlx5_flex_item *item,
    struct rte_flow_error *error) {
  ...
  for (i = 0; i < conf->nb_samples; i++) {
    ...
    for (j = i + 1; j < conf->nb_samples; j++) {
      ...
      ret = mlx5_flex_cover_sample(&cover, ft, fl, attr, error);
      if (ret < 0)
        return ret;
      if (!ret)
        continue;
    ...
    ret = mlx5_flex_alloc_sample(&cover, parser, item, fl, attr, error);
    if (ret)
      return ret;
  }
  ...
  if (conf->tunnel == FLEX_TUNNEL_MODE_MULTI) {
    ...
    rte_memcpy(parser->devx_conf.sample + parser->num_samples,
               ...
               parser->num_samples * sizeof(parser->devx_conf.sample[0]));
    for (i = 0; i < parser->num_samples; i++) {
      struct mlx5_devx_match_sample_attr *sm =
          i + parser->devx_conf.sample + parser->num_samples;

      sm->flow_match_sample_tunnel_mode = MLX5_GRAPH_SAMPLE_TUNNEL_INNER;
    }
...
static int mlx5_flex_arc_type(enum rte_flow_item_type type, int in) {
  switch (type) {
...
static int mlx5_flex_arc_in_eth(const struct rte_flow_item *item,
                                struct rte_flow_error *error) {
...
static int mlx5_flex_arc_in_udp(const struct rte_flow_item *item,
                                struct rte_flow_error *error) {
...
static int mlx5_flex_translate_arc_in(
    struct mlx5_hca_flex_attr *attr, const struct rte_flow_item_flex_conf *conf,
    struct mlx5_flex_parser_devx *devx, struct mlx5_flex_item *item,
    struct rte_flow_error *error) {
  struct mlx5_devx_graph_node_attr *node = &devx->devx_conf;
  uint32_t i;
...
static int mlx5_flex_translate_arc_out(
    struct mlx5_hca_flex_attr *attr, const struct rte_flow_item_flex_conf *conf,
    struct mlx5_flex_parser_devx *devx, struct mlx5_flex_item *item,
    struct rte_flow_error *error) {
  struct mlx5_devx_graph_node_attr *node = &devx->devx_conf;
...
static int mlx5_flex_translate_conf(struct rte_eth_dev *dev,
                                    const struct rte_flow_item_flex_conf *conf,
                                    struct mlx5_flex_parser_devx *devx,
                                    struct mlx5_flex_item *item,
                                    struct rte_flow_error *error) {
...
int flow_dv_item_release(struct rte_eth_dev *dev,
                         ...
                         struct rte_flow_error *error) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_flex_item *flex = (struct mlx5_flex_item *)(uintptr_t)handle;
  ...
  if (!__atomic_compare_exchange_n(&flex->refcnt, &old_refcnt, 0, 0,
    ...
    rte_spinlock_unlock(&priv->flex_item_sl);
    return rte_flow_error_set(error, EBUSY, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
                              "flex item has flow references");
  }
  /* Flex item is marked as invalid, we can leave locked section. */
  rte_spinlock_unlock(&priv->flex_item_sl);
  MLX5_ASSERT(flex->devx_fp);
  rc = mlx5_list_unregister(priv->sh->flex_parsers_dv, &flex->devx_fp->entry);
  flex->devx_fp = NULL;
  mlx5_flex_free(priv, flex);
  if (rc < 0)
    return rte_flow_error_set(error, EBUSY, RTE_FLOW_ERROR_TYPE_ITEM, NULL,
...
struct mlx5_list_entry *mlx5_flex_parser_create_cb(void *list_ctx, void *ctx) {
  ...
  fp = mlx5_malloc(MLX5_MEM_ZERO, sizeof(struct mlx5_flex_parser_devx), 0,
                   SOCKET_ID_ANY);
  if (!fp)
    return NULL;
  ...
  fp->devx_obj =
      mlx5_devx_cmd_create_flex_parser(sh->cdev->ctx, &fp->devx_conf);
  if (!fp->devx_obj)
    goto error;
  /* Query the firmware assigned sample ids. */
  ret = mlx5_devx_cmd_query_parse_samples(fp->devx_obj, fp->sample_ids,
                                          fp->num_samples);
  if (ret)
    goto error;
...
int mlx5_flex_parser_match_cb(void *list_ctx, struct mlx5_list_entry *iter,
                              void *ctx) {
  ...
  return !iter || !ctx ||
         memcmp(&fp->devx_conf, &org->devx_conf, sizeof(fp->devx_conf));
}
...
struct mlx5_list_entry *mlx5_flex_parser_clone_cb(void *list_ctx,
  ...
  RTE_SET_USED(list_ctx);
  RTE_SET_USED(entry);
  fp = mlx5_malloc(0, sizeof(struct mlx5_flex_parser_devx), 0, SOCKET_ID_ANY);
  if (!fp)
    return NULL;
...
void mlx5_flex_parser_clone_free_cb(void *list_ctx,
                                    struct mlx5_list_entry *entry) {
  struct mlx5_flex_parser_devx *fp =
      container_of(entry, struct mlx5_flex_parser_devx, entry);
  RTE_SET_USED(list_ctx);
