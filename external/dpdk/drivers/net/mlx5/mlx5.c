...
static LIST_HEAD(, mlx5_dev_ctx_shared)
    mlx5_dev_ctx_list = LIST_HEAD_INITIALIZER();
 ...
 *   Pointer to mlx5_dev_ctx_shared object to free.
 */
static void mlx5_flow_aso_age_mng_close(struct mlx5_dev_ctx_shared *sh) {
  int i, j;
  ...
  if (sh->aso_age_mng->pools) {
    ...
    for (i = 0; i < sh->aso_age_mng->next; ++i) {
      ...
      claim_zero(mlx5_devx_cmd_destroy(pool->flow_hit_aso_obj));
      for (j = 0; j < MLX5_COUNTERS_PER_POOL; ++j)
        if (pool->actions[j].dr_action)
          claim_zero(
              mlx5_flow_os_destroy_flow_action(pool->actions[j].dr_action));
      mlx5_free(pool);
    }
 ...
 */
static void mlx5_flow_aging_init(struct mlx5_dev_ctx_shared *sh) {
  uint32_t i;
 ...
 *
 */
void mlx5_flow_counter_mode_config(struct rte_eth_dev *dev __rte_unused) {
  ...
  if (fallback)
    DRV_LOG(INFO,
            "Use fall-back DV counter management. Flow "
  ...
  if (sh->refcnt == 1)
    ...
    DRV_LOG(WARNING,
            "Port %d in sh has different fallback mode "
            "with others:%d.",
            PORT_ID(priv), fallback);
#endif
}
 ...
 *   Pointer to mlx5_dev_ctx_shared object to free
 */
static void mlx5_flow_counters_mng_init(struct mlx5_dev_ctx_shared *sh) {
  int i;
 ...
 *   Pointer to mlx5_dev_ctx_shared object to free.
 */
static void mlx5_flow_counters_mng_close(struct mlx5_dev_ctx_shared *sh) {
  ...
  if (sh->cmng.pools) {
    ...
    for (i = 0; i < n_valid; ++i) {
      ...
      if (!fallback && pool->min_dcs)
        claim_zero(mlx5_devx_cmd_destroy(pool->min_dcs));
      for (j = 0; j < MLX5_COUNTERS_PER_POOL; ++j) {
        struct mlx5_flow_counter *cnt = MLX5_POOL_GET_CNT(pool, j);

        if (cnt->action)
          claim_zero(mlx5_flow_os_destroy_flow_action(cnt->action));
        if (fallback && MLX5_POOL_GET_CNT(pool, j)->dcs_when_free)
          claim_zero(mlx5_devx_cmd_destroy(cnt->dcs_when_free));
      }
      mlx5_free(pool);
    }
...
int mlx5_aso_flow_mtrs_mng_init(struct mlx5_dev_ctx_shared *sh) {
  if (!sh->mtrmng) {
    sh->mtrmng = mlx5_malloc(MLX5_MEM_ZERO, sizeof(*sh->mtrmng),
                             RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
    if (!sh->mtrmng) {
      DRV_LOG(ERR, "meter management allocation was failed.");
      rte_errno = ENOMEM;
      return -ENOMEM;
    }
    if (sh->meter_aso_en) {
 ...
 *   Pointer to mlx5_dev_ctx_shared object to free.
 */
static void mlx5_aso_flow_mtrs_mng_close(struct mlx5_dev_ctx_shared *sh) {
    ...
    while (idx--) {
      ...
      for (i = 0; i < MLX5_ASO_MTRS_PER_POOL; i++) {
        ...
        if (aso_mtr->fm.meter_action)
          claim_zero(mlx5_glue->destroy_flow_action(aso_mtr->fm.meter_action));
      }
#endif /* HAVE_MLX5_DR_CREATE_ACTION_ASO */
      claim_zero(mlx5_devx_cmd_destroy(mtr_pool->devx_obj));
 ...
 *   Pointer to mlx5_dev_ctx_shared object to free.
 */
static void mlx5_flow_aso_ct_mng_close(struct mlx5_dev_ctx_shared *sh) {
  ...
  while (idx--) {
    ...
    for (i = 0; i < MLX5_ASO_CT_ACTIONS_PER_POOL; i++) {
      ...
      val = __atomic_fetch_sub(&ct->refcnt, 1, __ATOMIC_RELAXED);
      MLX5_ASSERT(val == 1);
      if (val > 1)
        cnt++;
    ...
    if (cnt) {
      DRV_LOG(DEBUG, "%u ASO CT objects are being used in the pool %u", cnt, i);
    }
    mlx5_free(ct_pool);
    /* in case of failure. */
    mng->next--;
 ...
 *   Pointer to mlx5_dev_ctx_shared object.
 */
static void mlx5_flow_ipool_create(struct mlx5_dev_ctx_shared *sh) {
  uint8_t i;
  ...
  for (i = 0; i < MLX5_IPOOL_MAX; ++i) {
    ...
    case MLX5_IPOOL_MLX5_FLOW:
      cfg.size = sh->config.dv_flow_en ? sizeof(struct mlx5_flow_handle)
 ...
 *   Pointer to mlx5_dev_ctx_shared object.
 */
static void mlx5_flow_ipool_destroy(struct mlx5_dev_ctx_shared *sh) {
  uint8_t i;
...
int mlx5_flex_parser_ecpri_alloc(struct rte_eth_dev *dev) {
  ...
  if (!priv->sh->cdev->config.hca_attr.parse_graph_flex_node) {
    DRV_LOG(ERR,
            "Dynamic flex parser is not supported "
            "for device %s.",
            priv->dev_data->name);
    return -ENOTSUP;
  }
 ...
 *   Pointer to Ethernet device structure.
 */
static void mlx5_flex_parser_ecpri_release(struct rte_eth_dev *dev) {
  struct mlx5_priv *priv = dev->data->dev_private;
  struct mlx5_ecpri_parser_profile *prf = &priv->sh->ecpri_parser;
...
static int mlx5_rxtx_uars_prepare(struct mlx5_dev_ctx_shared *sh) {
  int ret;
...
static void mlx5_rxtx_uars_release(struct mlx5_dev_ctx_shared *sh) {
 ...
 */
static void mlx5_dev_ctx_shared_rx_mempool_unregister_cb(struct rte_mempool *mp,
                                                         void *arg) {
 ...
 */
static int mlx5_setup_tis(struct mlx5_dev_ctx_shared *sh) {
  int i;
  ...
  if (sh->bond.n_port) {
    if (!mlx5_devx_cmd_query_lag(sh->cdev->ctx, &lag_ctx)) {
      sh->lag.tx_remap_affinity[0] = lag_ctx.tx_remap_affinity_1;
      sh->lag.tx_remap_affinity[1] = lag_ctx.tx_remap_affinity_2;
    ...
    if (sh->lag.affinity_mode == MLX5_LAG_MODE_TIS) {
      for (i = 0; i < sh->bond.n_port; i++) {
        tis_attr.lag_tx_port_affinity =
            MLX5_IFC_LAG_MAP_TIS_AFFINITY(i, sh->bond.n_port);
        sh->tis[i] = mlx5_devx_cmd_create_tis(sh->cdev->ctx, &tis_attr);
        if (!sh->tis[i]) {
          DRV_LOG(ERR,
                  "Failed to TIS %d/%d for bonding device"
                  " %s.",
                  i, sh->bond.n_port, sh->ibdev_name);
          return -1;
        }
      }
    ...
    if (sh->lag.affinity_mode == MLX5_LAG_MODE_HASH)
      DRV_LOG(INFO, "Device %s enabled HW hash based LAG.", sh->ibdev_name);
  }
  tis_attr.lag_tx_port_affinity = 0;
  sh->tis[0] = mlx5_devx_cmd_create_tis(sh->cdev->ctx, &tis_attr);
  if (!sh->tis[0]) {
    DRV_LOG(ERR,
            "Failed to TIS 0 for bonding device"
            " %s.",
            sh->ibdev_name);
    return -1;
  }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_dev_args_check_handler(const char *key, const char *val,
                                       void *opaque) {
  ...
  if (strcmp(MLX5_TX_PP, key) == 0) {
    ...
    if (tmp != MLX5_XMETA_MODE_LEGACY && tmp != MLX5_XMETA_MODE_META16 &&
        tmp != MLX5_XMETA_MODE_META32 && tmp != MLX5_XMETA_MODE_MISS_INFO) {
    ...
    if (tmp != MLX5_RCM_NONE && tmp != MLX5_RCM_LIGHT && tmp != MLX5_RCM_AGGR) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_shared_dev_ctx_args_config(struct mlx5_dev_ctx_shared *sh,
  ...
  if (mkvlist != NULL) {
    /* Process parameters. */
    ret = mlx5_kvargs_process(mkvlist, params, mlx5_dev_args_check_handler,
                              config);
    if (ret) {
  ...
  if (!config->dv_esw_en && config->dv_xmeta_en != MLX5_XMETA_MODE_LEGACY) {
    DRV_LOG(WARNING, "Metadata mode %u is not supported (no E-Switch).",
            config->dv_xmeta_en);
...
void mlx5_rt_timestamp_config(struct mlx5_dev_ctx_shared *sh,
  ...
  if (hca_attr->access_register_user)
    ret = mlx5_devx_cmd_register_read(sh->cdev->ctx, MLX5_REGISTER_ID_MTUTC, 0,
                                      reg, dw_cnt);
  if (!ret) {
    uint32_t ts_mode;
...
mlx5_alloc_shared_dev_ctx(const struct mlx5_dev_spawn_data *spawn,
  ...
  if (err) {
    DRV_LOG(ERR, "Failed to process device configure: %s", strerror(rte_errno));
    goto error;
  }
  sh->refcnt = 1;
...
void mlx5_free_shared_dev_ctx(struct mlx5_dev_ctx_shared *sh) {
  ...
  if (!sh->cdev->config.mr_mempool_reg_en) {
    ret = rte_mempool_event_callback_unregister(
        mlx5_dev_ctx_shared_rx_mempool_event_cb, sh);
    if (ret == 0)
      rte_mempool_walk(mlx5_dev_ctx_shared_rx_mempool_unregister_cb, sh);
  }
...
void mlx5_free_table_hash_list(struct mlx5_priv *priv) {
  struct mlx5_dev_ctx_shared *sh = priv->sh;
  struct mlx5_hlist **tbls =
      (priv->sh->config.dv_flow_en == 2) ? &sh->groups : &sh->flow_tbls;
  if (*tbls == NULL)
    return;
 ...
 *   Pointer to the private device data structure.
 */
static int mlx5_alloc_hw_group_hash_list(struct mlx5_priv *priv) {
  int err = 0;
  struct mlx5_dev_ctx_shared *sh = priv->sh;
  ...
  sh->groups = mlx5_hlist_create(
      s, MLX5_FLOW_TABLE_HLIST_ARRAY_SIZE, false, true, sh,
      flow_hw_grp_create_cb, flow_hw_grp_match_cb, flow_hw_grp_remove_cb,
      flow_hw_grp_clone_cb, flow_hw_grp_clone_free_cb);
  if (!sh->groups) {
 ...
 */
int mlx5_alloc_table_hash_list(struct mlx5_priv *priv __rte_unused) {
  int err = 0;
  ...
  sh->flow_tbls = mlx5_hlist_create(
      s, MLX5_FLOW_TABLE_HLIST_ARRAY_SIZE, false, true, sh,
      flow_dv_tbl_create_cb, flow_dv_tbl_match_cb, flow_dv_tbl_remove_cb,
      flow_dv_tbl_clone_cb, flow_dv_tbl_clone_free_cb);
  if (!sh->flow_tbls) {
   ...
   * RDMA-CORE.
   */
  if (!flow_dv_tbl_resource_get(dev, 0, 0, 0, 0, NULL, 0, 1, 0, &error) ||
      !flow_dv_tbl_resource_get(dev, 0, 1, 0, 0, NULL, 0, 1, 0, &error) ||
      !flow_dv_tbl_resource_get(dev, 0, 0, 1, 0, NULL, 0, 1, 0, &error)) {
    err = ENOMEM;
    goto error;
  }
  return err;
 ...
 */
int mlx5_getenv_int(const char *name) {
 ...
 */
int mlx5_udp_tunnel_port_add(struct rte_eth_dev *dev __rte_unused,
...
int mlx5_dev_close(struct rte_eth_dev *dev) {
  ...
  if (ret)
    DRV_LOG(WARNING, "port %u some Rx queues still remain", dev->data->port_id);
  ...
  if (ret)
    DRV_LOG(WARNING, "port %u some Tx queues still remain", dev->data->port_id);
  ret = mlx5_flow_verify(dev);
  if (ret)
    DRV_LOG(WARNING, "port %u some flows still remain", dev->data->port_id);
  if (priv->hrxqs)
  ...
  if (priv->domain_id != RTE_ETH_DEV_SWITCH_DOMAIN_ID_INVALID) {
    ...
    MLX5_ETH_FOREACH_DEV(port_id, dev->device) {
      ...
      if (!opriv || opriv->domain_id != priv->domain_id ||
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_port_args_check_handler(const char *key, const char *val,
                                        void *opaque) {
...
int mlx5_port_args_config(struct mlx5_priv *priv,
  ...
  if (mkvlist != NULL) {
    /* Process parameters. */
    ret = mlx5_kvargs_process(mkvlist, params, mlx5_port_args_check_handler,
                              config);
    if (ret) {
      DRV_LOG(ERR, "Failed to process port arguments: %s", strerror(rte_errno));
      return -rte_errno;
    }
  }
  /* Adjust parameters according to device capabilities. */
  ...
  if (dev_cap->lro_supported) {
    ...
    if (!config->lro_timeout)
      config->lro_timeout = hca_attr->lro_timer_supported_periods[0];
    DRV_LOG(DEBUG, "LRO session timeout set to %d usec.", config->lro_timeout);
  }
  ...
  if (config->cqe_comp_fmt == MLX5_CQE_RESP_FORMAT_FTAG_STRIDX &&
    ...
    DRV_LOG(WARNING, "Flow Tag CQE compression format isn't supported.");
    config->cqe_comp = 0;
  }
  if (config->cqe_comp_fmt == MLX5_CQE_RESP_FORMAT_L34H_STRIDX &&
      (!devx || !hca_attr->mini_cqe_resp_l3_l4_tag)) {
    DRV_LOG(WARNING, "L3/L4 Header CQE compression format isn't supported.");
    config->cqe_comp = 0;
  }
  ...
  DRV_LOG(DEBUG, "Standard \"delay_drop\" is %u.", config->std_delay_drop);
  DRV_LOG(DEBUG, "Hairpin \"delay_drop\" is %u.", config->hp_delay_drop);
  DRV_LOG(DEBUG, "\"max_dump_files_num\" is %u.", config->max_dump_files_num);
  DRV_LOG(DEBUG, "\"log_hp_size\" is %u.", config->log_hp_size);
  DRV_LOG(DEBUG, "\"mprq_en\" is %u.", config->mprq.enabled);
  DRV_LOG(DEBUG, "\"mprq_log_stride_num\" is %u.", config->mprq.log_stride_num);
  DRV_LOG(DEBUG, "\"mprq_log_stride_size\" is %u.",
          config->mprq.log_stride_size);
  DRV_LOG(DEBUG, "\"mprq_max_memcpy_len\" is %u.", config->mprq.max_memcpy_len);
 ...
 */
static int mlx5_dummy_handler(const char *key, const char *val, void *opaque) {
...
void mlx5_port_args_set_used(const char *name, uint16_t port_id,
  ...
  DRV_LOG(DEBUG,
          "Ethernet device \"%s\" for port %u "
          "already exists, set devargs as used:",
          name, port_id);
...
int mlx5_probe_again_args_validate(struct mlx5_common_device *cdev,
  ...
  if (sh == NULL)
    return 0;
  config =
      mlx5_malloc(MLX5_MEM_ZERO | MLX5_MEM_RTE, sizeof(struct mlx5_sh_config),
                  RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
  if (config == NULL) {
    rte_errno = -ENOMEM;
    return -rte_errno;
  ...
  if (sh->config.allow_duplicate_pattern ^ config->allow_duplicate_pattern) {
    DRV_LOG(ERR,
...
void mlx5_set_min_inline(struct mlx5_priv *priv) {
  ...
  if (config->txq_inline_min != MLX5_ARG_UNSET) {
    ...
    if (priv->pci_dev != NULL) {
      ...
      case PCI_DEVICE_ID_MELLANOX_CONNECTX4VF:
        if (config->txq_inline_min < (int)MLX5_INLINE_HSIZE_L2) {
          DRV_LOG(
              DEBUG,
  ...
  if (hca_attr->eth_net_offloads) {
    ...
    case MLX5_CAP_INLINE_MODE_NOT_REQUIRED:
      ...
      config->hw_vlan_insert = hca_attr->wqe_vlan_insert;
    ...
    case MLX5_CAP_INLINE_MODE_VPORT_CONTEXT:
      ...
      case MLX5_INLINE_MODE_IP:
        config->txq_inline_min = MLX5_INLINE_HSIZE_L3;
        goto exit;
      case MLX5_INLINE_MODE_TCP_UDP:
        config->txq_inline_min = MLX5_INLINE_HSIZE_L4;
        goto exit;
      case MLX5_INLINE_MODE_INNER_L2:
        config->txq_inline_min = MLX5_INLINE_HSIZE_INNER_L2;
        goto exit;
      case MLX5_INLINE_MODE_INNER_IP:
        config->txq_inline_min = MLX5_INLINE_HSIZE_INNER_L3;
        goto exit;
      case MLX5_INLINE_MODE_INNER_TCP_UDP:
        config->txq_inline_min = MLX5_INLINE_HSIZE_INNER_L4;
        goto exit;
      }
    }
  }
...
void mlx5_set_metadata_mask(struct rte_eth_dev *dev) {
  ...
  if (sh->dv_mark_mask && sh->dv_mark_mask != mark)
    DRV_LOG(WARNING, "metadata MARK mask mismatch %08X:%08X", sh->dv_mark_mask,
            mark);
  else
    sh->dv_mark_mask = mark;
  if (sh->dv_meta_mask && sh->dv_meta_mask != meta)
    DRV_LOG(WARNING, "metadata META mask mismatch %08X:%08X", sh->dv_meta_mask,
            meta);
  else
...
int rte_pmd_mlx5_get_dyn_flag_names(char *names[], unsigned int n) {
  static const char *const dynf_names[] = {RTE_PMD_MLX5_FINE_GRANULARITY_INLINE,
...
uint16_t mlx5_eth_find_next(uint16_t port_id, struct rte_device *odev) {
  while (port_id < RTE_MAX_ETHPORTS) {
    ...
    if (dev->state != RTE_ETH_DEV_UNUSED && dev->device &&
        (dev->device == odev ||
         (dev->device->driver && dev->device->driver->name &&
          ((strcmp(dev->device->driver->name, MLX5_PCI_DRIVER_NAME) == 0) ||
           (strcmp(dev->device->driver->name, MLX5_AUXILIARY_DRIVER_NAME) ==
            0)))))
      break;
    port_id++;
  }
...
static const struct rte_pci_id mlx5_pci_id_map[] = {
    ...
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX, PCI_DEVICE_ID_MELLANOX_CONNECTX5)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX,
    ...
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX, PCI_DEVICE_ID_MELLANOX_CONNECTX6)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX,
                    PCI_DEVICE_ID_MELLANOX_CONNECTX6VF)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX,
                    PCI_DEVICE_ID_MELLANOX_CONNECTX6DX)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX, PCI_DEVICE_ID_MELLANOX_CONNECTXVF)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX,
...
RTE_PMD_REGISTER_KMOD_DEP(MLX5_ETH_DRIVER_NAME,
