...
static void mlx5_txpp_destroy_event_channel(struct mlx5_dev_ctx_shared *sh) {
  if (sh->txpp.echan) {
...
static int mlx5_txpp_create_event_channel(struct mlx5_dev_ctx_shared *sh) {
  MLX5_ASSERT(!sh->txpp.echan);
  sh->txpp.echan = mlx5_os_devx_create_event_channel(
      sh->cdev->ctx, MLX5DV_DEVX_CREATE_EVENT_CHANNEL_FLAGS_OMIT_EV_DATA);
  if (!sh->txpp.echan) {
    rte_errno = errno;
...
static void mlx5_txpp_free_pp_index(struct mlx5_dev_ctx_shared *sh) {
...
static int mlx5_txpp_alloc_pp_index(struct mlx5_dev_ctx_shared *sh) {
#ifdef HAVE_MLX5DV_PP_ALLOC
  ...
  if (sh->txpp.test) {
    ...
    len = RTE_MAX(MLX5_TXPP_TEST_PKT_SIZE, (size_t)RTE_ETHER_MIN_LEN);
    MLX5_SET(set_pp_rate_limit_context, &pp, burst_upper_bound, len);
    MLX5_SET(set_pp_rate_limit_context, &pp, typical_packet_size, len);
  ...
  sh->txpp.pp = mlx5_glue->dv_alloc_pp(sh->cdev->ctx, sizeof(pp), &pp,
...
static void mlx5_txpp_destroy_send_queue(struct mlx5_txpp_wq *wq) {
  mlx5_devx_sq_destroy(&wq->sq_obj);
  mlx5_devx_cq_destroy(&wq->cq_obj);
  memset(wq, 0, sizeof(*wq));
}

static void mlx5_txpp_destroy_rearm_queue(struct mlx5_dev_ctx_shared *sh) {
  struct mlx5_txpp_wq *wq = &sh->txpp.rearm_queue;

  mlx5_txpp_destroy_send_queue(wq);
}

static void mlx5_txpp_destroy_clock_queue(struct mlx5_dev_ctx_shared *sh) {
...
static void mlx5_txpp_doorbell_rearm_queue(struct mlx5_dev_ctx_shared *sh,
                                           uint16_t ci) {
  struct mlx5_txpp_wq *wq = &sh->txpp.rearm_queue;
  ...
  wq->sq_ci = ci + 1;
  cs.w32[0] = rte_cpu_to_be_32(
      rte_be_to_cpu_32(wqe[ci & (wq->sq_size - 1)].ctrl[0]) | (ci - 1) << 8);
  cs.w32[1] = wqe[ci & (wq->sq_size - 1)].ctrl[1];
  /* Update SQ doorbell record with new SQ ci. */
  mlx5_doorbell_ring(&sh->tx_uar.bf_db, cs.w64, wq->sq_ci, wq->sq_obj.db_rec,
                     !sh->tx_uar.dbnc);
}

static void mlx5_txpp_fill_wqe_rearm_queue(struct mlx5_dev_ctx_shared *sh) {
  struct mlx5_txpp_wq *wq = &sh->txpp.rearm_queue;
  struct mlx5_wqe *wqe = (struct mlx5_wqe *)(uintptr_t)wq->sq_obj.wqes;
  uint32_t i;
  ...
  for (i = 0; i < wq->sq_size; i += 2) {
    ...
    qs->qpn_cqn = rte_cpu_to_be_32(sh->txpp.clock_queue.sq_obj.sq->id);
    /* Build WAIT request with slave CQE index. */
    cs = &wqe[i + 1].cseg;
    cs->opcode = RTE_BE32(MLX5_OPCODE_WAIT | 0);
    cs->sq_ds = rte_cpu_to_be_32((wq->sq_obj.sq->id << 8) | 2);
    cs->flags = RTE_BE32(MLX5_COMP_ONLY_ERR << MLX5_COMP_MODE_OFFSET);
    cs->misc = RTE_BE32(0);
    qs = RTE_PTR_ADD(cs, sizeof(struct mlx5_wqe_cseg));
    index = (i * MLX5_TXPP_REARM / 2 + MLX5_TXPP_REARM / 2) &
            ((1 << MLX5_CQ_INDEX_WIDTH) - 1);
    qs->max_index = rte_cpu_to_be_32(index);
    qs->qpn_cqn = rte_cpu_to_be_32(sh->txpp.clock_queue.cq_obj.cq->id);
  }
}

/* Creates the Rearm Queue to fire the requests to Clock Queue in realtime. */
static int mlx5_txpp_create_rearm_queue(struct mlx5_dev_ctx_shared *sh) {
  struct mlx5_devx_create_sq_attr sq_attr = {
      ...
      .wq_attr =
          (struct mlx5_devx_wq_attr){
              .pd = sh->cdev->pdn,
              .uar_page = mlx5_os_get_devx_uar_page_id(sh->tx_uar.obj),
          },
      .ts_format = mlx5_ts_format_conv(sh->cdev->config.hca_attr.sq_ts_format),
  };
...
static void mlx5_txpp_fill_wqe_clock_queue(struct mlx5_dev_ctx_shared *sh) {
  struct mlx5_txpp_wq *wq = &sh->txpp.clock_queue;
  struct mlx5_wqe *wqe = (struct mlx5_wqe *)(uintptr_t)wq->sq_obj.wqes;
  ...
  if (sh->txpp.test) {
    wqe_size = RTE_ALIGN(MLX5_TXPP_TEST_PKT_SIZE + MLX5_WQE_CSEG_SIZE +
                             2 * MLX5_WQE_ESEG_SIZE - MLX5_ESEG_MIN_INLINE_SIZE,
                         MLX5_WSEG_SIZE);
  ...
  cs->sq_ds =
      rte_cpu_to_be_32((wq->sq_obj.sq->id << 8) | (wqe_size / MLX5_WSEG_SIZE));
  cs->flags = RTE_BE32(MLX5_COMP_ALWAYS << MLX5_COMP_MODE_OFFSET);
  cs->misc = RTE_BE32(0);
  ...
  if (sh->txpp.test) {
    ...
    ip_hdr->packet_id = 0;
    ip_hdr->total_length =
        RTE_BE16(MLX5_TXPP_TEST_PKT_SIZE - sizeof(struct rte_ether_hdr));
    /* use RFC5735 / RFC2544 reserved network test addresses */
    ip_hdr->src_addr = RTE_BE32((198U << 24) | (18 << 16) | (0 << 8) | 1);
    ip_hdr->dst_addr = RTE_BE32((198U << 24) | (18 << 16) | (0 << 8) | 2);
    ...
    for (i = sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) +
             sizeof(struct rte_udp_hdr);
...
wcopy:
  ...
  for (i = 1; i < MLX5_TXPP_CLKQ_SIZE; i++) {
    dst += wqe_size;
    rte_memcpy(dst, (void *)(uintptr_t)wq->sq_obj.umem_buf, wqe_size);
  }
}

/* Creates the Clock Queue for packet pacing, returns zero on success. */
static int mlx5_txpp_create_clock_queue(struct mlx5_dev_ctx_shared *sh) {
  ...
  sh->txpp.tsa = mlx5_malloc(
      MLX5_MEM_RTE | MLX5_MEM_ZERO,
      MLX5_TXPP_REARM_SQ_SIZE * sizeof(struct mlx5_txpp_ts), 0, sh->numa_node);
  if (!sh->txpp.tsa) {
  ...
  if (sh->txpp.test) {
    wq->sq_size =
        RTE_ALIGN(MLX5_TXPP_TEST_PKT_SIZE + MLX5_WQE_CSEG_SIZE +
                      2 * MLX5_WQE_ESEG_SIZE - MLX5_ESEG_MIN_INLINE_SIZE,
                  MLX5_WQE_SIZE) /
        MLX5_WQE_SIZE;
  ...
  ret = mlx5_devx_sq_create(sh->cdev->ctx, &wq->sq_obj, log2above(wq->sq_size),
...
static inline void mlx5_txpp_cq_arm(struct mlx5_dev_ctx_shared *sh) {
...
static inline int mlx5_atomic128_compare_exchange(rte_int128_t *dst,
                                                  rte_int128_t *exp,
                                                  const rte_int128_t *src) {
  uint8_t res;

  asm volatile(MPLOCKED "cmpxchg16b %[dst];"
                        " sete %[res]"
               : [dst] "=m"(dst->val[0]), "=a"(exp->val[0]),
                 "=d"(exp->val[1]), [res] "=r"(res)
               : "b"(src->val[0]), "c"(src->val[1]), "a"(exp->val[0]),
                 "d"(exp->val[1]), "m"(dst->val[0])
               : "memory");
...
static inline void mlx5_atomic_read_cqe(rte_int128_t *from, rte_int128_t *ts) {
  /*
...
static inline void mlx5_txpp_cache_timestamp(struct mlx5_dev_ctx_shared *sh,
...
static inline void mlx5_txpp_update_timestamp(struct mlx5_dev_ctx_shared *sh) {
  struct mlx5_txpp_wq *wq = &sh->txpp.clock_queue;
  ...
  if (opcode) {
    if (opcode != MLX5_CQE_INVALID) {
       ...
       */
      DRV_LOG(DEBUG, "Clock Queue error sync lost (%X).", opcode);
      __atomic_fetch_add(&sh->txpp.err_clock_queue, 1, __ATOMIC_RELAXED);
      sh->txpp.sync_lost = 1;
    }
    return;
  }
...
static inline void mlx5_txpp_init_timestamp(struct mlx5_dev_ctx_shared *sh) {
  struct mlx5_txpp_wq *wq = &sh->txpp.clock_queue;
  uint32_t wait;
...
static inline void mlx5_txpp_gather_timestamp(struct mlx5_dev_ctx_shared *sh) {
  ...
  __atomic_store_n(&sh->txpp.tsa[sh->txpp.ts_p].ts, sh->txpp.ts.ts,
                   __ATOMIC_RELAXED);
  __atomic_store_n(&sh->txpp.tsa[sh->txpp.ts_p].ci_ts, sh->txpp.ts.ci_ts,
                   __ATOMIC_RELAXED);
...
mlx5_txpp_handle_rearm_queue(struct mlx5_dev_ctx_shared *sh) {
  ...
  if (likely(cq_ci != wq->cq_ci)) {
    ...
    if (cq_ci - wq->cq_ci != 1) {
      ...
      __atomic_fetch_add(&sh->txpp.err_miss_int, 1, __ATOMIC_RELAXED);
      /* Check sync lost on wqe index. */
      if (cq_ci - wq->cq_ci >=
          (((1UL << MLX5_WQ_INDEX_WIDTH) / MLX5_TXPP_REARM) - 1))
        error = 1;
    }
    ...
    *wq->cq_obj.db_rec = rte_cpu_to_be_32(cq_ci);
    ...
    if (error) {
      ...
      __atomic_fetch_add(&sh->txpp.err_rearm_queue, 1, __ATOMIC_RELAXED);
      sh->txpp.sync_lost = 1;
    }
  }
}
...
void mlx5_txpp_interrupt_handler(void *cb_arg) {
  ...
  while (mlx5_glue->devx_get_event(sh->txpp.echan, &out.event_resp,
                                   sizeof(out.buf)) >=
         (ssize_t)sizeof(out.event_resp.cookie)) {
    mlx5_txpp_handle_rearm_queue(sh);
    mlx5_txpp_handle_clock_queue(sh);
    mlx5_txpp_cq_arm(sh);
    mlx5_txpp_doorbell_rearm_queue(sh, sh->txpp.rearm_queue.sq_ci - 1);
  }
#endif /* HAVE_IBV_DEVX_ASYNC */
}

static void mlx5_txpp_stop_service(struct mlx5_dev_ctx_shared *sh) {
...
static int mlx5_txpp_start_service(struct mlx5_dev_ctx_shared *sh) {
  ...
  sh->txpp.intr_handle = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_SHARED);
 ...
 */
static int mlx5_txpp_create(struct mlx5_dev_ctx_shared *sh) {
 ...
 * - PP context
 */
static void mlx5_txpp_destroy(struct mlx5_dev_ctx_shared *sh) {
  mlx5_txpp_stop_service(sh);
  mlx5_txpp_destroy_rearm_queue(sh);
  mlx5_txpp_destroy_clock_queue(sh);
  mlx5_txpp_free_pp_index(sh);
...
int mlx5_txpp_start(struct rte_eth_dev *dev) {
  ...
  if (sh->config.tx_pp > 0) {
    err = rte_mbuf_dynflag_lookup(RTE_MBUF_DYNFLAG_TX_TIMESTAMP_NAME, NULL);
...
int mlx5_txpp_xstats_get_names(struct rte_eth_dev *dev __rte_unused,
  ...
  if (n >= n_used + n_txpp && xstats_names) {
    for (i = 0; i < n_txpp; ++i) {
      strncpy(xstats_names[i + n_used].name, mlx5_txpp_stat_names[i],
...
static inline void mlx5_txpp_read_tsa(struct mlx5_dev_txpp *txpp,
  ...
  do {
    ...
    if ((ci ^ ts) << MLX5_CQ_INDEX_WIDTH != 0)
      continue;
    if (__atomic_load_n(&txpp->tsa[idx].ts, __ATOMIC_RELAXED) != ts)
      continue;
    if (__atomic_load_n(&txpp->tsa[idx].ci_ts, __ATOMIC_RELAXED) != ci)
      continue;
    tsa->ts = ts;
    tsa->ci_ts = ci;
    return;
 ...
 */
static uint64_t mlx5_txpp_xstats_jitter(struct mlx5_dev_txpp *txpp) {
 ...
 */
static uint64_t mlx5_txpp_xstats_wander(struct mlx5_dev_txpp *txpp) {
  struct mlx5_txpp_ts tsa0, tsa1;
  int64_t dts, dci;
  uint16_t ts_p;
 ...
 */
int mlx5_txpp_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *stats,
                         unsigned int n, unsigned int n_used) {
  unsigned int n_txpp = RTE_DIM(mlx5_txpp_stat_names);
  ...
  if (n >= n_used + n_txpp && stats) {
    ...
    stats[n_used + 0].value =
        __atomic_load_n(&sh->txpp.err_miss_int, __ATOMIC_RELAXED);
    stats[n_used + 1].value =
        __atomic_load_n(&sh->txpp.err_rearm_queue, __ATOMIC_RELAXED);
    stats[n_used + 2].value =
        __atomic_load_n(&sh->txpp.err_clock_queue, __ATOMIC_RELAXED);
    stats[n_used + 3].value =
        __atomic_load_n(&sh->txpp.err_ts_past, __ATOMIC_RELAXED);
    stats[n_used + 4].value =
        __atomic_load_n(&sh->txpp.err_ts_future, __ATOMIC_RELAXED);
