...
int cxgbe_init_hash_filter(struct adapter *adap) {
  ...
  n_user_filters = mult_frac(adap->tids.nftids, user_filter_perc, 100);
...
#define U(_mask, _field) (!(fconf & (_mask)) && S(_field))

  if (U(F_PORT, iport) || U(F_ETHERTYPE, ethtype) || U(F_PROTOCOL, proto) ||
      U(F_MACMATCH, macidx) || U(F_VLAN, ivlan_vld) ||
      U(F_VNIC_ID, ovlan_vld) || U(F_TOS, tos) || U(F_VNIC_ID, pfvf_vld))
    return -EOPNOTSUPP;

  /* Either OVLAN or PFVF match is enabled in hardware, but not both */
  if ((S(pfvf_vld) && !(iconf & F_VNIC)) || (S(ovlan_vld) && (iconf & F_VNIC)))
    return -EOPNOTSUPP;
...
#undef U
   ...
   */
  if (fs->action == FILTER_SWITCH && fs->eport >= adapter->params.nports)
    return -ERANGE;
...
static unsigned int get_filter_steerq(struct rte_eth_dev *dev,
  ...
  if (!fs->dirsteer) {
    ...
    if (fs->iq < pi->n_rx_qsets)
      iq = adapter->sge.ethrxq[pi->first_rxqset + fs->iq].rspq.abs_id;
    else
      iq = fs->iq;
  }
 ...
 */
static void set_tcb_field(struct adapter *adapter, unsigned int ftid, u16 word,
  ...
  req->reply_ctrl =
      cpu_to_be16(V_REPLY_CHAN(0) | V_QUEUENO(adapter->sge.fw_evtq.abs_id) |
                  V_NO_REPLY(no_reply));
...
static void set_tcb_tflag(struct adapter *adap, unsigned int ftid,
                          unsigned int bit_pos, unsigned int val,
                          int no_reply) {
...
static inline void mk_set_tcb_field_ulp(struct filter_entry *f,
  ...
  txpkt->cmd_dest = cpu_to_be32(V_ULPTX_CMD(ULP_TX_PKT) | V_ULP_TXPKT_DEST(0));
  ...
  req->reply_ctrl =
      cpu_to_be16(V_NO_REPLY(no_reply) | V_REPLY_CHAN(0) | V_QUEUENO(0));
...
int cxgbe_alloc_ftid(struct adapter *adap, u8 nentries) {
  ...
  t4_os_lock(&t->ftid_lock);
  if (nentries > 1)
    pos = cxgbe_bitmap_find_free_region(t->ftid_bmap, size, nentries);
  else
...
static u64 hash_filter_ntuple(const struct filter_entry *f) {
  ...
  if (tp->vlan_shift >= 0 && f->fs.mask.ivlan)
    ntuple |= (u64)(F_FT_VLAN_VLD | f->fs.val.ivlan) << tp->vlan_shift;
  if (tp->vnic_shift >= 0) {
    if ((adap->params.tp.ingress_config & F_VNIC) && f->fs.mask.pfvf_vld)
      ntuple |=
          (u64)(f->fs.val.pfvf_vld << 16 | f->fs.val.pf << 13 | f->fs.val.vf)
          << tp->vnic_shift;
    else if (!(adap->params.tp.ingress_config & F_VNIC) && f->fs.mask.ovlan_vld)
      ntuple |= (u64)(f->fs.val.ovlan_vld << 16 | f->fs.val.ovlan)
                << tp->vnic_shift;
  }
...
static void mk_abort_req_ulp(struct cpl_abort_req *abort_req,
  ...
  sc->cmd_more = cpu_to_be32(V_ULPTX_CMD(ULP_TX_SC_IMM));
  sc->len = cpu_to_be32(sizeof(*abort_req) - sizeof(struct work_request_hdr));
...
static void mk_abort_rpl_ulp(struct cpl_abort_rpl *abort_rpl,
  ...
  sc->cmd_more = cpu_to_be32(V_ULPTX_CMD(ULP_TX_SC_IMM));
  sc->len = cpu_to_be32(sizeof(*abort_rpl) - sizeof(struct work_request_hdr));
...
static int cxgbe_del_hash_filter(struct rte_eth_dev *dev,
  ...
  if (f->valid) {
    ...
    wrlen = cxgbe_roundup(sizeof(*wr) + (sizeof(*req) + sizeof(*aligner)) +
    ...
    mk_set_tcb_field_ulp(f, req, W_TCB_RSS_INFO, V_TCB_RSS_INFO(M_TCB_RSS_INFO),
                         V_TCB_RSS_INFO(adapter->sge.fw_evtq.abs_id), 0, 1);
...
static void mk_act_open_req6(struct filter_entry *f, struct rte_mbuf *mbuf,
  ...
  OPCODE_TID(req) = cpu_to_be32(MK_OPCODE_TID(CPL_ACT_OPEN_REQ6, qid_filterid));
  ...
  req->opt0 = cpu_to_be64(
      V_NAGLE(f->fs.newvlan == VLAN_REMOVE || f->fs.newvlan == VLAN_REWRITE) |
      V_DELACK(f->fs.hitcnts) | V_L2T_IDX(f->l2t ? f->l2t->idx : 0) |
      V_SMAC_SEL((cxgbe_port_viid(f->dev) & 0x7F) << 1) |
      V_TX_CHAN(f->fs.eport) | V_ULP_MODE(ULP_MODE_NONE) | F_TCAM_BYPASS |
      F_NON_OFFLOAD);
  req->params = cpu_to_be64(V_FILTER_TUPLE(hash_filter_ntuple(f)));
  req->opt2 = cpu_to_be32(
      F_RSS_QUEUE_VALID | V_RSS_QUEUE(f->fs.iq) | F_T5_OPT_2_VALID |
...
static void mk_act_open_req(struct filter_entry *f, struct rte_mbuf *mbuf,
  ...
  OPCODE_TID(req) = cpu_to_be32(MK_OPCODE_TID(CPL_ACT_OPEN_REQ, qid_filterid));
  req->local_port = cpu_to_be16(f->fs.val.lport);
  req->peer_port = cpu_to_be16(f->fs.val.fport);
...
static int cxgbe_set_hash_filter(struct rte_eth_dev *dev,
  ...
  if (f->fs.newdmac || f->fs.newvlan == VLAN_INSERT ||
    ...
    f->l2t =
        cxgbe_l2t_alloc_switching(dev, f->fs.vlan, f->fs.eport, f->fs.dmac);
    if (!f->l2t) {
      ret = -ENOMEM;
      goto out_err;
    }
  ...
  if (f->fs.type == FILTER_TYPE_IPV6) {
    ...
    mk_act_open_req6(f, mbuf, ((adapter->sge.fw_evtq.abs_id << 14) | atid),
                     adapter);
  } else {
    /* IPv4 hash filter */
    ...
    mk_act_open_req(f, mbuf, ((adapter->sge.fw_evtq.abs_id << 14) | atid),
                    adapter);
  }
...
static void t4_mk_filtdelwr(struct adapter *adap, unsigned int ftid,
  ...
  wr->tid_to_iq =
      cpu_to_be32(V_FW_FILTER_WR_TID(ftid) | V_FW_FILTER_WR_NOREPLY(qid < 0));
  wr->del_filter_to_l2tix = cpu_to_be32(F_FW_FILTER_WR_DEL_FILTER);
  if (qid >= 0)
    wr->rx_chan_rx_rpl_iq = cpu_to_be16(V_FW_FILTER_WR_RX_RPL_IQ(qid));
}
...
static int set_filter_wr(struct rte_eth_dev *dev, unsigned int fidx) {
  ...
  fwr->tid_to_iq = cpu_to_be32(
      V_FW_FILTER_WR_TID(f->tid) | V_FW_FILTER_WR_RQTYPE(f->fs.type) |
      V_FW_FILTER_WR_NOREPLY(0) | V_FW_FILTER_WR_IQ(f->fs.iq));
  fwr->del_filter_to_l2tix = cpu_to_be32(
      V_FW_FILTER_WR_DROP(f->fs.action == FILTER_DROP) |
      V_FW_FILTER_WR_DIRSTEER(f->fs.dirsteer) |
      V_FW_FILTER_WR_LPBK(f->fs.action == FILTER_SWITCH) |
      V_FW_FILTER_WR_SMAC(f->fs.newsmac) | V_FW_FILTER_WR_DMAC(f->fs.newdmac) |
      V_FW_FILTER_WR_INSVLAN(f->fs.newvlan == VLAN_INSERT ||
                             f->fs.newvlan == VLAN_REWRITE) |
      V_FW_FILTER_WR_RMVLAN(f->fs.newvlan == VLAN_REMOVE ||
                            f->fs.newvlan == VLAN_REWRITE) |
      V_FW_FILTER_WR_HITCNTS(f->fs.hitcnts) |
      V_FW_FILTER_WR_TXCHAN(f->fs.eport) | V_FW_FILTER_WR_PRIO(f->fs.prio) |
      V_FW_FILTER_WR_L2TIX(f->l2t ? f->l2t->idx : 0));
  fwr->ethtype = cpu_to_be16(f->fs.val.ethtype);
  fwr->ethtypem = cpu_to_be16(f->fs.mask.ethtype);
  fwr->frag_to_ovlan_vldm = (V_FW_FILTER_WR_IVLAN_VLD(f->fs.val.ivlan_vld) |
  ...
  if (adapter->params.filter2_wr_support) {
    fwr->filter_type_swapmac = V_FW_FILTER2_WR_SWAPMAC(f->fs.swapmac);
    fwr->natmode_to_ulp_type =
        V_FW_FILTER2_WR_ULP_TYPE(f->fs.nat_mode ? ULP_MODE_TCPDDP
                                                : ULP_MODE_NONE) |
...
int cxgbe_del_filter(struct rte_eth_dev *dev, unsigned int filter_id,
  ...
  if (f->valid) {
    f->ctx = ctx;
    cxgbe_clear_ftid(&adapter->tids, f->tid - adapter->tids.ftid_base,
                     nentries);
...
int cxgbe_set_filter(struct rte_eth_dev *dev, unsigned int filter_id,
  ...
  if (f->fs.newvlan || f->fs.newdmac) {
    f->l2t =
        cxgbe_l2t_alloc_switching(f->dev, f->fs.vlan, f->fs.eport, f->fs.dmac);
    if (!f->l2t) {
      ret = -ENOMEM;
      goto free_tid;
    }
...
void cxgbe_hash_filter_rpl(struct adapter *adap,
  ...
  unsigned int ftid = G_TID_TID(G_AOPEN_ATID(be32_to_cpu(rpl->atid_status)));
  ...
  case CPL_ERR_NONE: {
    ...
    if (f->fs.hitcnts)
      set_tcb_field(adap, tid, W_TCB_TIMESTAMP,
                    V_TCB_TIMESTAMP(M_TCB_TIMESTAMP) |
                        V_TCB_T_RTT_TS_RECENT_AGE(M_TCB_T_RTT_TS_RECENT_AGE),
                    V_TCB_TIMESTAMP(0ULL) | V_TCB_T_RTT_TS_RECENT_AGE(0ULL), 1);
    if (f->fs.newdmac)
    ...
    if (f->fs.newsmac) {
      ...
      set_tcb_field(adap, tid, W_TCB_SMAC_SEL, V_TCB_SMAC_SEL(M_TCB_SMAC_SEL),
 ...
 */
int cxgbe_get_filter_count(struct adapter *adapter, unsigned int fidx, u64 *c,
                           int hash, bool get_byte) {
  struct filter_entry *f;
  ...
  if (is_t5(adapter->params.chip) || is_t6(adapter->params.chip)) {
    ...
    if (get_byte) {
      ...
      *c = be64_to_cpu(be64_byte_count);
      ...
      ret = t4_memory_rw(adapter, MEMWIN_NIC, MEM_EDC0,
                         tcbaddr + (word_offset * sizeof(__be32)),
                         sizeof(be32_count), &be32_count, T4_MEMORY_READ);
      t4_os_unlock(&adapter->win0_lock);
      if (ret < 0)
        return ret;
...
int cxgbe_clear_filter_count(struct adapter *adapter, unsigned int fidx,
  ...
  if (clear_byte) {
    ...
    tcb_mask = V_TCB_T_RTT_TS_RECENT_AGE(M_TCB_T_RTT_TS_RECENT_AGE) |
               V_TCB_T_RTSEQ_RECENT(M_TCB_T_RTSEQ_RECENT);
    tcb_val = V_TCB_T_RTT_TS_RECENT_AGE(0ULL) | V_TCB_T_RTSEQ_RECENT(0ULL);
