...
static inline bool match_entry(struct mps_tcam_entry *entry, const u8 *eth_addr,
                               const u8 *mask) {
...
static struct mps_tcam_entry *cxgbe_mpstcam_lookup(struct mpstcam_table *t,
                                                   const u8 *eth_addr,
                                                   const u8 *mask) {
  struct mps_tcam_entry *entry = t->entry;
  int i;
  ...
  for (i = 0; i < t->size; i++) {
    if (entry[i].state == MPS_ENTRY_UNUSED || entry[i].state == MPS_ENTRY_RAWF)
...
int cxgbe_mpstcam_alloc(struct port_info *pi, const u8 *eth_addr,
  ...
  ret = t4_alloc_raw_mac_filt(adap, pi->viid, eth_addr, mask, mpstcam->free_idx,
                              0, pi->port_id, false);
  if (ret <= 0) {
    t4_os_write_unlock(&mpstcam->lock);
    return ret;
...
int cxgbe_mpstcam_remove(struct port_info *pi, u16 idx) {
  ...
  if (__atomic_load_n(&entry->refcnt, __ATOMIC_RELAXED) == 1)
    ret = t4_free_raw_mac_filt(adap, pi->viid, entry->eth_addr, entry->mask,
                               idx, 1, pi->port_id, false);
  else
...
int cxgbe_mpstcam_rawf_enable(struct port_info *pi) {
  ...
  ret = t4_alloc_raw_mac_filt(adap, pi->viid, entry->eth_addr, entry->mask,
                              rawf_idx, 0, pi->port_id, false);
  if (ret < 0)
    goto out_unlock;
