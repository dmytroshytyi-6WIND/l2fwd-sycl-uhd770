...
static void size_nports_qsets(struct adapter *adapter) {
  ...
  if (adapter->params.nports > MAX_NPORTS) {
    dev_warn(adapter->pdev_dev,
             ...
             MAX_NPORTS, adapter->params.nports);
  ...
  if (pmask_nports < adapter->params.nports) {
    dev_warn(adapter->pdev_dev,
             ...
             " mask %#x\n",
             pmask_nports, adapter->params.nports, adapter->params.vfres.pmask);
...
static int adap_init0vf(struct adapter *adapter) {
  ...
  if (adapter->params.rss.mode != FW_RSS_GLB_CONFIG_CMD_MODE_BASICVIRTUAL) {
    dev_err(adapter->pdev_dev,
...
int cxgbevf_probe(struct adapter *adapter) {
  ...
  if (!adapter->mpstcam)
    dev_warn(adapter, "VF could not allocate mps tcam table. Continuing\n");
