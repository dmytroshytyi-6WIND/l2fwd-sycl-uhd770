  ...
  if (q->pend_cred >= 64) {
    ...
    if (unlikely(!q->bar2_addr)) {
      u32 reg = is_pf4(adap) ? MYPF_REG(A_SGE_PF_KDOORBELL)
                             : T4VF_SGE_BASE_ADDR + A_SGE_VF_KDOORBELL;

      t4_write_reg_relaxed(adap, reg, val | V_QID(q->cntxt_id));
    } else {
...
static unsigned int refill_fl_usembufs(struct adapter *adap, struct sge_fl *q,
  ...
  for (i = 0; i < n; i++) {
    ...
    mbuf->data_off = (uint16_t)(
        (char *)RTE_PTR_ALIGN((char *)mbuf->buf_addr + RTE_PKTMBUF_HEADROOM,
                              adap->sge.fl_align) -
        (char *)mbuf->buf_addr);
    mbuf->next = NULL;
    mbuf->nb_segs = 1;
...
static inline unsigned int calc_tx_flits(const struct rte_mbuf *m,
  ...
  if (m->tso_segsz)
    ...
    flits += (wr_size + sizeof(struct cpl_tx_pkt_core)) / sizeof(__be64);
  return flits;
}
...
static void write_sgl(struct rte_mbuf *mbuf, struct sge_txq *q,
  ...
  sgl->cmd_nsge = htonl(V_ULPTX_CMD(ULP_TX_SC_DSGL) | V_ULPTX_NSGE(nfrags));
  if (likely(--nfrags == 0))
    return;
  /*
  ...
  if (unlikely((u8 *)end > (u8 *)q->stat)) {
    unsigned int part0 = RTE_PTR_DIFF((u8 *)q->stat, (u8 *)sgl->sge);
    unsigned int part1;
...
static inline void ship_tx_pkt_coalesce_wr(struct adapter *adap,
  ...
  const size_t fw_hdr_copy_len =
      (sizeof(vmwr->ethmacdst) + sizeof(vmwr->ethmacsrc) +
       sizeof(vmwr->ethtype) + sizeof(vmwr->vlantci));
...
static inline int should_tx_packet_coalesce(struct sge_eth_txq *txq,
  ...
  const size_t fw_hdr_copy_len =
      (sizeof(wr->ethmacdst) + sizeof(wr->ethmacsrc) + sizeof(wr->ethtype) +
       sizeof(wr->vlantci));
  struct sge_txq *q = &txq->q;
  ...
  if (!is_pf4(adap)) {
    ...
    if (q->coalesce.idx &&
        memcmp((void *)q->coalesce.ethmacdst, rte_pktmbuf_mtod(mbuf, void *),
               fw_hdr_copy_len))
      ship_tx_pkt_coalesce_wr(adap, txq);
  }
  ...
  if (type == 0)
    flits += (sizeof(struct ulp_txpkt) + sizeof(struct ulptx_idata)) /
             sizeof(__be64);
  ...
  *nflits = flits;
  ...
  if (q->coalesce.idx) {
     ...
     */
    if (unlikely((q->coalesce.len + mbuf->pkt_len > MAX_COALESCE_LEN) ||
                 (q->coalesce.flits + flits > q->coalesce.max))) {
      ship_tx_pkt_coalesce_wr(adap, txq);
      goto new;
    }
    return 1;
  ...
  q->coalesce.ptr =
      (unsigned char *)&q->desc[q->pidx] + q->coalesce.flits * sizeof(__be64);
  if (!is_pf4(adap))
    memcpy((void *)q->coalesce.ethmacdst, rte_pktmbuf_mtod(mbuf, void *),
           fw_hdr_copy_len);
  return 1;
}
...
static inline int tx_do_packet_coalesce(struct sge_eth_txq *txq,
                                        struct rte_mbuf *mbuf, int flits,
                                        struct adapter *adap,
  ...
  if (q->coalesce.type == 0) {
    ...
    mc->cmd_dest =
        htonl(V_ULPTX_CMD(4) | V_ULP_TXPKT_DEST(0) |
              V_ULP_TXPKT_FID(adap->sge.fw_evtq.cntxt_id) | F_ULP_TXPKT_RO);
  ...
  if (mbuf->ol_flags & RTE_MBUF_F_TX_IP_CKSUM) {
    cntrl = hwcsum(adap->params.chip, mbuf) | F_TXPKT_IPCSUM_DIS;
    txq->stats.tx_cso++;
  } else {
  ...
  if (is_pf4(adap))
    cpl->ctrl0 |= htonl(V_TXPKT_INTF(pi->tx_chan) | V_TXPKT_PF(adap->pf));
  else
...
int t4_eth_xmit(struct sge_eth_txq *txq, struct rte_mbuf *mbuf,
  ...
  if (!((m->ol_flags & RTE_MBUF_F_TX_TCP_SEG) ||
    ...
    if (should_tx_packet_coalesce(txq, mbuf, &cflits, adap)) {
      ...
      return tx_do_packet_coalesce(txq, mbuf, cflits, adap, pi, addr, nb_pkts);
    } else {
      return -EBUSY;
    }
  }
  ...
  if (!(m->ol_flags & RTE_MBUF_F_TX_TCP_SEG)) {
    wr->op_immdlen = htonl(
        V_FW_WR_OP(is_pf4(adap) ? FW_ETH_TX_PKT_WR : FW_ETH_TX_PKT_VM_WR) |
        V_FW_WR_IMMDLEN(len));
    if (is_pf4(adap))
      cpl = (void *)(wr + 1);
    else
      cpl = (void *)(vmwr + 1);
    if (m->ol_flags & RTE_MBUF_F_TX_IP_CKSUM) {
      cntrl = hwcsum(adap->params.chip, m) | F_TXPKT_IPCSUM_DIS;
      txq->stats.tx_cso++;
    }
  } else {
    if (is_pf4(adap))
    ...
    wr->op_immdlen = htonl(
        ...
        V_FW_WR_IMMDLEN(len));
    lso->lso_ctrl = htonl(
        V_LSO_OPCODE(CPL_TX_PKT_LSO) | F_LSO_FIRST_SLICE | F_LSO_LAST_SLICE |
        V_LSO_IPV6(v6) | V_LSO_ETHHDR_LEN(eth_xtra_len / 4) |
        V_LSO_IPHDR_LEN(l3hdr_len / 4) | V_LSO_TCPHDR_LEN(l4hdr_len / 4));
  ...
  if (is_pf4(adap))
    ...
    cpl->ctrl0 |= htonl(V_TXPKT_INTF(pi->port_id) | V_TXPKT_PF(0));
  ...
  write_sgl(m, &txq->q, (struct ulptx_sgl *)(cpl + 1), end, 0, addr);
...
static int ctrl_xmit(struct sge_ctrl_txq *q, struct rte_mbuf *mbuf) {
  ...
  dst = (void *)wr;
  inline_tx_mbuf(&q->q, rte_pktmbuf_mtod(mbuf, caddr_t), &dst, mbuf->data_len);
...
static void *alloc_ring(struct rte_eth_dev *dev, const char *z_name,
  ...
  dev_debug(adapter,
            ...
            " z_name_sw = %s\n",
            __func__, nelem, elem_size, sw_size, stat_size, queue_id, socket_id,
            z_name, z_name_sw);
   ...
   */
  tz = rte_eth_dma_zone_reserve(dev, z_name, queue_id, len, 4096, socket_id);
  if (!tz)
    return NULL;

  memset(tz->addr, 0, len);
  if (sw_size) {
    s = rte_zmalloc_socket(z_name_sw, nelem * sw_size, RTE_CACHE_LINE_SIZE,
                           socket_id);
...
static int process_responses(struct sge_rspq *q, int budget,
  ...
  while (likely(budget_left)) {
    ...
    rc = (const struct rsp_ctrl *)((const char *)q->cur_desc +
                                   (q->iqe_len - sizeof(*rc)));
    ...
    if (likely(rsp_type == X_RSPD_TYPE_FLBUF)) {
      ...
      while (stat_pidx_diff && budget_left) {
        ...
        const struct rss_header *rss_hdr = (const void *)q->cur_desc;
        const struct cpl_rx_pkt *cpl = (const void *)&q->cur_desc[1];
  ...
  *work_done = process_responses(q, budget, rx_pkts);
  ...
  if (*work_done) {
    ...
    if (unlikely(!q->bar2_addr)) {
      ...
      t4_write_reg(q->adapter, reg, val | V_INGRESSQID((u32)q->cntxt_id));
    } else {
...
int t4_sge_alloc_rxq(struct adapter *adap, struct sge_rspq *iq, bool fwevtq,
  ...
  iq->desc =
      alloc_ring(eth_dev, fwevtq ? "fwq_ring" : "rx_ring", queue_id, socket_id,
                 iq->size, iq->iqe_len, 0, 0, &iq->phys_addr, NULL);
  if (!iq->desc)
    return -ENOMEM;
  ...
  if (is_pf4(adap)) {
    pciechan = pi->tx_chan;
    c.op_to_vfn |= htonl(V_FW_IQ_CMD_PFN(adap->pf) | V_FW_IQ_CMD_VFN(0));
    if (cong >= 0)
      c.iqns_to_fl0congen = htonl(
          F_FW_IQ_CMD_IQFLINTCONGEN |
          V_FW_IQ_CMD_IQTYPE(cong ? FW_IQ_IQTYPE_NIC : FW_IQ_IQTYPE_OFLD) |
          F_FW_IQ_CMD_IQRO);
  } else {
    pciechan = pi->port_id;
  }
  ...
  c.alloc_to_len16 =
      htonl(F_FW_IQ_CMD_ALLOC | F_FW_IQ_CMD_IQSTART | (sizeof(c) / 16));
  c.type_to_iqandstindex = htonl(
      V_FW_IQ_CMD_TYPE(FW_IQ_TYPE_FL_INT_CAP) | V_FW_IQ_CMD_IQASYNCH(fwevtq) |
      V_FW_IQ_CMD_VIID(pi->viid) | V_FW_IQ_CMD_IQANDST(intr_idx < 0) |
      V_FW_IQ_CMD_IQANUD(X_UPDATEDELIVERY_STATUS_PAGE) |
      V_FW_IQ_CMD_IQANDSTINDEX(intr_idx >= 0 ? intr_idx : -intr_idx - 1));
  c.iqdroprss_to_iqesize =
      htons(V_FW_IQ_CMD_IQPCIECH(pciechan) | F_FW_IQ_CMD_IQGTSMODE |
  ...
  if (fl) {
    struct sge_eth_rxq *rxq = container_of(fl, struct sge_eth_rxq, fl);
    ...
    fl->desc = alloc_ring(eth_dev, "fl_ring", queue_id, socket_id, fl->size,
                          sizeof(__be64), s->stat_len,
                          sizeof(struct rx_sw_desc), &fl->addr, &fl->sdesc);
    if (!fl->desc) {
      ret = -ENOMEM;
      goto err;
    }
    ...
    c.iqns_to_fl0congen |= htonl(
        ...
        (unlikely(rxq->usembufs) ? 0 : F_FW_IQ_CMD_FL0PACKEN) |
        F_FW_IQ_CMD_FL0FETCHRO | F_FW_IQ_CMD_FL0DATARO | F_FW_IQ_CMD_FL0PADEN);
    if (is_pf4(adap) && cong >= 0)
      c.iqns_to_fl0congen |=
          htonl(V_FW_IQ_CMD_FL0CNGCHMAP(cong) | F_FW_IQ_CMD_FL0CONGCIF |
    ...
    c.fl0dcaen_to_fl0cidxfthresh = htons(
        V_FW_IQ_CMD_FL0FBMIN(chip_ver <= CHELSIO_T5 ? X_FETCHBURSTMIN_128B
                                                    : X_FETCHBURSTMIN_64B) |
        V_FW_IQ_CMD_FL0FBMAX(chip_ver <= CHELSIO_T5 ? X_FETCHBURSTMAX_512B
  ...
  iq->bar2_addr =
      bar2_address(adap, iq->cntxt_id, T4_BAR2_QTYPE_INGRESS, &iq->bar2_qid);
  ...
  if (fl) {
     ...
     */
    fl->bar2_addr =
        bar2_address(adap, fl->cntxt_id, T4_BAR2_QTYPE_EGRESS, &fl->bar2_qid);
...
refill_fl_err:
  t4_iq_free(adap, adap->mbox, adap->pf, 0, FW_IQ_TYPE_FL_INT_CAP, iq->cntxt_id,
             fl->cntxt_id, 0xffff);
err:
  iq->cntxt_id = 0;
  iq->abs_id = 0;
...
static void init_txq(struct adapter *adap, struct sge_txq *q, unsigned int id,
  ...
  q->abs_id = abs_id;
  q->bar2_addr =
      bar2_address(adap, q->cntxt_id, T4_BAR2_QTYPE_EGRESS, &q->bar2_qid);
  q->cidx = 0;
  q->pidx = 0;
  q->dbidx = 0;
  q->in_use = 0;
...
int t4_sge_alloc_eth_txq(struct adapter *adap, struct sge_eth_txq *txq,
  ...
  txq->q.desc =
      alloc_ring(eth_dev, "tx_ring", queue_id, socket_id, txq->q.size,
                 sizeof(struct tx_desc), s->stat_len, sizeof(struct tx_sw_desc),
  ...
  if (is_pf4(adap)) {
    pciechan = pi->tx_chan;
    c.op_to_vfn |=
        htonl(V_FW_EQ_ETH_CMD_PFN(adap->pf) | V_FW_EQ_ETH_CMD_VFN(0));
  } else {
    pciechan = pi->port_id;
  }

  c.alloc_to_len16 =
      htonl(F_FW_EQ_ETH_CMD_ALLOC | F_FW_EQ_ETH_CMD_EQSTART | (sizeof(c) / 16));
  c.autoequiqe_to_viid =
      htonl(F_FW_EQ_ETH_CMD_AUTOEQUEQE | V_FW_EQ_ETH_CMD_VIID(pi->viid));
  c.fetchszm_to_iqid =
      htonl(V_FW_EQ_ETH_CMD_HOSTFCMODE(X_HOSTFCMODE_NONE) |
            V_FW_EQ_ETH_CMD_PCIECHN(pciechan) | F_FW_EQ_ETH_CMD_FETCHRO |
            V_FW_EQ_ETH_CMD_IQID(iqid));
  c.dcaen_to_eqsize = htonl(V_FW_EQ_ETH_CMD_FBMIN(X_FETCHBURSTMIN_64B) |
...
int t4_sge_alloc_ctrl_txq(struct adapter *adap, struct sge_ctrl_txq *txq,
  ...
  txq->q.desc =
      alloc_ring(eth_dev, "ctrl_tx_ring", queue_id, socket_id, txq->q.size,
                 sizeof(struct tx_desc), 0, 0, &txq->q.phys_addr, NULL);
  if (!txq->q.desc)
    return -ENOMEM;
  ...
  c.op_to_vfn = htonl(V_FW_CMD_OP(FW_EQ_CTRL_CMD) | F_FW_CMD_REQUEST |
                      F_FW_CMD_WRITE | F_FW_CMD_EXEC |
                      V_FW_EQ_CTRL_CMD_PFN(adap->pf) | V_FW_EQ_CTRL_CMD_VFN(0));
  c.alloc_to_len16 = htonl(F_FW_EQ_CTRL_CMD_ALLOC | F_FW_EQ_CTRL_CMD_EQSTART |
                           (sizeof(c) / 16));
  c.cmpliqid_eqid = htonl(V_FW_EQ_CTRL_CMD_CMPLIQID(0));
  c.physeqid_pkd = htonl(0);
  c.fetchszm_to_iqid =
      htonl(V_FW_EQ_CTRL_CMD_HOSTFCMODE(X_HOSTFCMODE_NONE) |
            V_FW_EQ_CTRL_CMD_PCIECHN(pi->tx_chan) | F_FW_EQ_CTRL_CMD_FETCHRO |
            V_FW_EQ_CTRL_CMD_IQID(iqid));
  c.dcaen_to_eqsize = htonl(V_FW_EQ_CTRL_CMD_FBMIN(X_FETCHBURSTMIN_64B) |
...
static void free_rspq_fl(struct adapter *adap, struct sge_rspq *rq,
  ...
  t4_iq_free(adap, adap->mbox, adap->pf, 0, FW_IQ_TYPE_FL_INT_CAP, rq->cntxt_id,
             fl_id, 0xffff);
  rq->cntxt_id = 0;
  rq->abs_id = 0;
  rq->desc = NULL;
...
void t4_free_sge_resources(struct adapter *adap) {
  ...
  for (i = 0; i < ARRAY_SIZE(adap->sge.ctrlq); i++) {
    ...
    if (cq->q.desc) {
      ...
      t4_ctrl_eq_free(adap, adap->mbox, adap->pf, 0, cq->q.cntxt_id);
...
#undef READ_FL_BUF
  ...
  s->timer_val[0] = core_ticks_to_us(adap, G_TIMERVALUE0(timer_value_0_and_1));
  s->timer_val[1] = core_ticks_to_us(adap, G_TIMERVALUE1(timer_value_0_and_1));
  s->timer_val[2] = core_ticks_to_us(adap, G_TIMERVALUE2(timer_value_2_and_3));
  s->timer_val[3] = core_ticks_to_us(adap, G_TIMERVALUE3(timer_value_2_and_3));
  s->timer_val[4] = core_ticks_to_us(adap, G_TIMERVALUE4(timer_value_4_and_5));
  s->timer_val[5] = core_ticks_to_us(adap, G_TIMERVALUE5(timer_value_4_and_5));
...
int t4vf_sge_init(struct adapter *adap) {
  ...
  if ((sge_control & F_RXPKTCPLMODE) != V_RXPKTCPLMODE(X_RXPKTCPLMODE_SPLIT)) {
   ...
   */
  s_hps =
      (S_HOSTPAGESIZEPF0 + (S_HOSTPAGESIZEPF1 - S_HOSTPAGESIZEPF0) * adap->pf);
  sge_params->hps = ((sge_host_page_size >> s_hps) & M_HOSTPAGESIZEPF0);

  s_qpp = (S_QUEUESPERPAGEPF0 +
           (S_QUEUESPERPAGEPF1 - S_QUEUESPERPAGEPF0) * adap->pf);
  sge_params->eq_qpp =
      ((sge_egress_queues_per_page >> s_qpp) & M_QUEUESPERPAGEPF0);
  sge_params->iq_qpp =
      ((sge_ingress_queues_per_page >> s_qpp) & M_QUEUESPERPAGEPF0);
  ...
  case CHELSIO_T5:
    s->fl_starve_thres = G_EGRTHRESHOLDPACKING(sge_congestion_control);
    break;
  case CHELSIO_T6:
  default:
    s->fl_starve_thres = G_T6_EGRTHRESHOLDPACKING(sge_congestion_control);
    break;
  }
   ...
   */
  s->timer_val[0] =
      core_ticks_to_us(adap, G_TIMERVALUE0(sge_timer_value_0_and_1));
  s->timer_val[1] =
      core_ticks_to_us(adap, G_TIMERVALUE1(sge_timer_value_0_and_1));
  s->timer_val[2] =
      core_ticks_to_us(adap, G_TIMERVALUE2(sge_timer_value_2_and_3));
  s->timer_val[3] =
      core_ticks_to_us(adap, G_TIMERVALUE3(sge_timer_value_2_and_3));
  s->timer_val[4] =
      core_ticks_to_us(adap, G_TIMERVALUE4(sge_timer_value_4_and_5));
  s->timer_val[5] =
      core_ticks_to_us(adap, G_TIMERVALUE5(sge_timer_value_4_and_5));
