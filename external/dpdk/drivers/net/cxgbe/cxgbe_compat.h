...
#define dev_printf(level, logtype, fmt, ...)                                   \
  rte_log(RTE_LOG_##level, logtype, "rte_cxgbe_pmd: " fmt, ##__VA_ARGS__)

#define dev_err(x, fmt, ...) dev_printf(ERR, cxgbe_logtype, fmt, ##__VA_ARGS__)
...
static inline void writel_relaxed(unsigned int val,
