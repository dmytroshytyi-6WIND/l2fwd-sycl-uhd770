...
#define __CXGBE_FILL_FS(__v, __m, fs, elem, e)                                 \
  do {                                                                         \
    if ((fs)->mask.elem && ((fs)->val.elem != (__v)))                          \
      return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL,     \
                                "Redefined match item with"                    \
...
#define CXGBE_FILL_FS(v, m, elem) __CXGBE_FILL_FS(v, m, fs, elem, e)

#define CXGBE_FILL_FS_MEMCPY(v, m, elem) __CXGBE_FILL_FS_MEMCPY(v, m, fs, elem)

static int cxgbe_validate_item(const struct rte_flow_item *i,
                               struct rte_flow_error *e) {
  /* rte_flow specification does not allow it. */
  if (!i->spec && (i->mask || i->last))
    return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, i,
   ...
   */
  if (i->last)
    return rte_flow_error_set(e, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, i,
 ...
 */
static void cxgbe_tweak_filter_spec(struct adapter *adap,
...
static void cxgbe_fill_filter_region(struct adapter *adap,
                                     struct ch_filter_specification *fs) {
  ...
  if (fs->type) {
    uint8_t biton[16] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
  ...
  if (tp->vlan_shift >= 0 && fs->mask.ivlan_vld)
    ntuple_mask |= (u64)(F_FT_VLAN_VLD | fs->mask.ivlan) << tp->vlan_shift;
  if (tp->vnic_shift >= 0) {
    if (fs->mask.ovlan_vld)
      ntuple_mask |= (u64)(fs->val.ovlan_vld << 16 | fs->mask.ovlan)
                     << tp->vnic_shift;
    else if (fs->mask.pfvf_vld)
      ntuple_mask |=
          (u64)(fs->mask.pfvf_vld << 16 | fs->mask.pf << 13 | fs->mask.vf)
          << tp->vnic_shift;
  }
  if (tp->tos_shift >= 0)
...
static int ch_rte_parsetype_eth(const void *dmask,
  ...
  if (!rte_is_zero_ether_addr(&spec->src) ||
    ...
    return rte_flow_error_set(e, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item,
  ...
  if (!rte_is_zero_ether_addr(&spec->dst) ||
    ...
    CXGBE_FILL_FS_MEMCPY(spec->dst.addr_bytes, mask->dst.addr_bytes, dmac);
  }

  if (spec->type || (umask && umask->type))
    CXGBE_FILL_FS(be16_to_cpu(spec->type), be16_to_cpu(mask->type), ethtype);

  return 0;
}

static int ch_rte_parsetype_port(const void *dmask,
                                 const struct rte_flow_item *item,
                                 struct ch_filter_specification *fs,
                                 struct rte_flow_error *e) {
  ...
  if (val->index > 0x7)
    return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
...
static int ch_rte_parsetype_vlan(const void *dmask,
                                 const struct rte_flow_item *item,
                                 struct ch_filter_specification *fs,
                                 struct rte_flow_error *e) {
  ...
  if (fs->val.ethtype == RTE_ETHER_TYPE_QINQ) {
    ...
    if (spec) {
      if (spec->tci || (umask && umask->tci))
        CXGBE_FILL_FS(be16_to_cpu(spec->tci), be16_to_cpu(mask->tci), ovlan);
      fs->mask.ethtype = 0;
      fs->val.ethtype = 0;
    }
  } else {
  ...
  if (spec && (spec->inner_type || (umask && umask->inner_type)))
    CXGBE_FILL_FS(be16_to_cpu(spec->inner_type), be16_to_cpu(mask->inner_type),
                  ethtype);

  return 0;
}

static int ch_rte_parsetype_pf(const void *dmask __rte_unused,
...
static int ch_rte_parsetype_vf(const void *dmask,
                               const struct rte_flow_item *item,
                               struct ch_filter_specification *fs,
                               struct rte_flow_error *e) {
...
static int ch_rte_parsetype_udp(const void *dmask,
                                const struct rte_flow_item *item,
                                struct ch_filter_specification *fs,
                                struct rte_flow_error *e) {
...
static int ch_rte_parsetype_tcp(const void *dmask,
                                const struct rte_flow_item *item,
                                struct ch_filter_specification *fs,
                                struct rte_flow_error *e) {
...
static int ch_rte_parsetype_ipv4(const void *dmask,
                                 const struct rte_flow_item *item,
                                 struct ch_filter_specification *fs,
                                 struct rte_flow_error *e) {
  ...
  if (fs->mask.ethtype && (fs->val.ethtype != RTE_ETHER_TYPE_IPV4))
    return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
...
static int ch_rte_parsetype_ipv6(const void *dmask,
                                 const struct rte_flow_item *item,
                                 struct ch_filter_specification *fs,
                                 struct rte_flow_error *e) {
  ...
  if (vtc_flow_mask & RTE_IPV6_HDR_FL_MASK || mask->hdr.payload_len ||
      mask->hdr.hop_limits)
    return rte_flow_error_set(e, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item,
                              "flow/hop are not supported");

  if (fs->mask.ethtype && (fs->val.ethtype != RTE_ETHER_TYPE_IPV6))
    return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
  ...
  if (val->hdr.vtc_flow || (umask && umask->hdr.vtc_flow))
    CXGBE_FILL_FS(
        (vtc_flow & RTE_IPV6_HDR_TC_MASK) >> RTE_IPV6_HDR_TC_SHIFT,
        (vtc_flow_mask & RTE_IPV6_HDR_TC_MASK) >> RTE_IPV6_HDR_TC_SHIFT, tos);

  if (memcmp(val->hdr.dst_addr, z, sizeof(val->hdr.dst_addr)) ||
      (umask && memcmp(umask->hdr.dst_addr, z, sizeof(umask->hdr.dst_addr))))
    CXGBE_FILL_FS_MEMCPY(val->hdr.dst_addr, mask->hdr.dst_addr, lip);

  if (memcmp(val->hdr.src_addr, z, sizeof(val->hdr.src_addr)) ||
      (umask && memcmp(umask->hdr.src_addr, z, sizeof(umask->hdr.src_addr))))
    CXGBE_FILL_FS_MEMCPY(val->hdr.src_addr, mask->hdr.src_addr, fip);

  return 0;
}

static int cxgbe_rtef_parse_attr(struct rte_flow *flow,
                                 const struct rte_flow_attr *attr,
                                 struct rte_flow_error *e) {
  if (attr->egress)
    return rte_flow_error_set(e, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR, attr,
                              "attribute:<egress> is"
                              " not supported !");
  if (attr->group > 0)
    return rte_flow_error_set(e, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR, attr,
...
static int cxgbe_validate_fidxonadd(struct ch_filter_specification *fs,
...
static int cxgbe_verify_fidx(struct rte_flow *flow, unsigned int fidx,
                             uint8_t del) {
  if (flow->fs.cap)
    return 0; /* Hash filters */
  return del ? cxgbe_validate_fidxondel(flow->f, fidx)
             : cxgbe_validate_fidxonadd(&flow->fs, ethdev2adap(flow->dev),
                                        fidx);
}
...
static int cxgbe_get_flow_item_index(const struct rte_flow_item items[],
                                     u32 type) {
...
static int ch_rte_parse_nat(uint8_t nmode, struct ch_filter_specification *fs) {
  /* nmode:
...
static int ch_rte_parse_atype_switch(const struct rte_flow_action *a,
  ...
  case RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_VID:
    vlanid = (const struct rte_flow_action_of_set_vlan_vid *)a->conf;
  ...
  case RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_PCP:
    vlanpcp = (const struct rte_flow_action_of_set_vlan_pcp *)a->conf;
    /* If explicitly asked to push a new VLAN header,
     * then don't set rewrite mode. Otherwise, the
     * incoming VLAN packets will get their VLAN fields
     * rewritten, instead of adding an additional outer
  ...
  case RTE_FLOW_ACTION_TYPE_OF_PUSH_VLAN:
    pushvlan = (const struct rte_flow_action_of_push_vlan *)a->conf;
    if (be16_to_cpu(pushvlan->ethertype) != RTE_ETHER_TYPE_VLAN)
      return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, a,
  ...
  case RTE_FLOW_ACTION_TYPE_SET_IPV4_SRC:
    item_index = cxgbe_get_flow_item_index(items, RTE_FLOW_ITEM_TYPE_IPV4);
    if (item_index < 0)
      return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, a,
  ...
  case RTE_FLOW_ACTION_TYPE_SET_IPV6_SRC:
    item_index = cxgbe_get_flow_item_index(items, RTE_FLOW_ITEM_TYPE_IPV6);
    if (item_index < 0)
      return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, a,
  ...
  case RTE_FLOW_ACTION_TYPE_SET_TP_SRC:
    item_index = cxgbe_get_flow_item_index(items, RTE_FLOW_ITEM_TYPE_TCP);
    if (item_index < 0) {
      item_index = cxgbe_get_flow_item_index(items, RTE_FLOW_ITEM_TYPE_UDP);
      if (item_index < 0)
        return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, a,
...
static int cxgbe_rtef_parse_actions(struct rte_flow *flow,
                                    const struct rte_flow_item items[],
  ...
  for (a = action; a->type != RTE_FLOW_ACTION_TYPE_END; a++) {
    ...
    case RTE_FLOW_ACTION_TYPE_QUEUE:
      ...
      if (!q)
        return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, q,
    ...
    action_switch:
      ...
      ret = ch_rte_parse_atype_switch(a, items, &nmode, fs, e);
      if (ret)
        return ret;
    ...
    default:
      ...
      return rte_flow_error_set(e, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, a,
                                "Action not supported");
    }
  }
...
static struct chrte_fparse parseitem[] = {
    [RTE_FLOW_ITEM_TYPE_ETH] = {.fptr = ch_rte_parsetype_eth,
                                .dmask =
    ...
    [RTE_FLOW_ITEM_TYPE_PHY_PORT] = {.fptr = ch_rte_parsetype_port,
                                     .dmask =
                                         &(const struct rte_flow_item_phy_port){
                                             .index = 0x7,
                                         }},

    [RTE_FLOW_ITEM_TYPE_VLAN] = {.fptr = ch_rte_parsetype_vlan,
                                 .dmask =
    ...
    [RTE_FLOW_ITEM_TYPE_VF] = {.fptr = ch_rte_parsetype_vf,
                               .dmask =
...
static int cxgbe_rtef_parse_items(struct rte_flow *flow,
                                  const struct rte_flow_item items[],
                                  struct rte_flow_error *e) {
  struct adapter *adap = ethdev2adap(flow->dev);
  const struct rte_flow_item *i;
  ...
  for (i = items; i->type != RTE_FLOW_ITEM_TYPE_END; i++) {
    ...
    default:
      ...
      if (repeat[i->type] && i->type != RTE_FLOW_ITEM_TYPE_VLAN)
        return rte_flow_error_set(
            e, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, i,
...
static int cxgbe_flow_parse(struct rte_flow *flow,
                            const struct rte_flow_attr *attr,
                            const struct rte_flow_item item[],
                            const struct rte_flow_action action[],
                            struct rte_flow_error *e) {
...
static int __cxgbe_flow_create(struct rte_eth_dev *dev, struct rte_flow *flow) {
  ...
  err = cxgbe_poll_for_completion(&adap->sge.fw_evtq, CXGBE_FLOW_POLL_MS,
                                  CXGBE_FLOW_POLL_CNT, &ctx.completion);
  if (err) {
...
static struct rte_flow *cxgbe_flow_create(struct rte_eth_dev *dev,
                                          const struct rte_flow_attr *attr,
                                          const struct rte_flow_item item[],
                                          const struct rte_flow_action action[],
                                          struct rte_flow_error *e) {
  ...
  if (!flow) {
    rte_flow_error_set(e, ENOMEM, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
  ...
  if (ret) {
    rte_flow_error_set(e, ret, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
...
static int __cxgbe_flow_destroy(struct rte_eth_dev *dev,
                                struct rte_flow *flow) {
  struct adapter *adap = ethdev2adap(dev);
...
static int cxgbe_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow,
                              struct rte_flow_error *e) {
  struct adapter *adap = ethdev2adap(dev);
  int ret;
  ...
  t4_os_unlock(&adap->flow_lock);
  if (ret)
    return rte_flow_error_set(e, ret, RTE_FLOW_ERROR_TYPE_HANDLE, flow,
...
static int cxgbe_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow,
  ...
  if (action->type != RTE_FLOW_ACTION_TYPE_COUNT)
    return rte_flow_error_set(e, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
   ...
   */
  if (!fs.hitcnts)
    return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, &fs,
  ...
  if (ret) {
    rte_flow_error_set(e, -ret, RTE_FLOW_ERROR_TYPE_ACTION, f,
                       "cxgbe pmd failed to perform query");
    goto out;
  }
...
static int cxgbe_flow_validate(struct rte_eth_dev *dev,
                               const struct rte_flow_attr *attr,
                               const struct rte_flow_item item[],
                               const struct rte_flow_action action[],
                               struct rte_flow_error *e) {
  ...
  flow = t4_os_alloc(sizeof(struct rte_flow));
  if (!flow)
    return rte_flow_error_set(e, ENOMEM, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
  ...
  if (cxgbe_validate_filter(adap, &flow->fs)) {
    t4_os_free(flow);
    return rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
                              "validation failed. Check f/w config file.");
  }

  t4_os_lock(&adap->flow_lock);
  if (cxgbe_get_fidx(flow, &fidx)) {
    ret = rte_flow_error_set(e, ENOMEM, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
                             "no memory in tcam.");
    goto out;
  }

  if (cxgbe_verify_fidx(flow, fidx, 0)) {
    ret = rte_flow_error_set(e, EINVAL, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
                             "validation failed");
    goto out;
  }
 ...
 */
static int cxgbe_check_n_destroy(struct filter_entry *f,
...
static int cxgbe_flow_flush(struct rte_eth_dev *dev, struct rte_flow_error *e) {
  ...
  if (adap->tids.ftid_tab) {
    ...
    for (i = 0; i < adap->tids.nftids; i++, f++) {
      ...
      if (ret < 0) {
        rte_flow_error_set(e, ret, RTE_FLOW_ERROR_TYPE_HANDLE, f->private,
...
int cxgbe_dev_flow_ops_get(struct rte_eth_dev *dev __rte_unused,
