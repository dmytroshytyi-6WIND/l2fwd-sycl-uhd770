...
static const u16 cxgbe_filter_mode_features[] = {
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_ETHERTYPE | F_PROTOCOL |
     F_PORT),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_ETHERTYPE | F_PROTOCOL |
     F_FCOE),
    ...
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_ETHERTYPE | F_PORT |
     F_FCOE),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_PROTOCOL | F_TOS | F_PORT |
     F_FCOE),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_PROTOCOL | F_VLAN |
     F_FCOE),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_PROTOCOL | F_VNIC_ID |
     F_FCOE),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_TOS | F_VLAN | F_FCOE),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_TOS | F_VNIC_ID | F_FCOE),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_VLAN | F_PORT | F_FCOE),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_MACMATCH | F_VNIC_ID | F_PORT | F_FCOE),
    ...
    (F_FRAGMENTATION | F_MPSHITTYPE | F_PROTOCOL | F_TOS | F_VNIC_ID | F_PORT),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_PROTOCOL | F_TOS | F_VNIC_ID | F_FCOE),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_PROTOCOL | F_VLAN | F_PORT | F_FCOE),
    (F_FRAGMENTATION | F_MPSHITTYPE | F_PROTOCOL | F_VNIC_ID | F_PORT | F_FCOE),
    ...
    (F_FRAGMENTATION | F_MACMATCH | F_ETHERTYPE | F_PROTOCOL | F_PORT | F_FCOE),
    (F_FRAGMENTATION | F_MACMATCH | F_ETHERTYPE | F_TOS | F_PORT | F_FCOE),
    (F_FRAGMENTATION | F_MACMATCH | F_PROTOCOL | F_VLAN | F_PORT | F_FCOE),
    (F_FRAGMENTATION | F_MACMATCH | F_PROTOCOL | F_VNIC_ID | F_PORT | F_FCOE),
    ...
    (F_MPSHITTYPE | F_MACMATCH | F_ETHERTYPE | F_PROTOCOL | F_PORT | F_FCOE),
 ...
 */
void *t4_alloc_mem(size_t size) { return rte_zmalloc(NULL, size, 0); }

/**
 * Free memory allocated through t4_alloc_mem().
 */
void t4_free_mem(void *addr) { rte_free(addr); }
...
int cxgbe_setup_sge_ctrl_txq(struct adapter *adapter) {
  ...
  for_each_port(adapter, i) {
    ...
    q->q.size = 1024;
    err = t4_sge_alloc_ctrl_txq(adapter, q, adapter->eth_dev, i,
                                s->fw_evtq.cntxt_id, rte_socket_id());
    if (err) {
      dev_err(adapter, "Failed to alloc ctrl txq. Err: %d", err);
      goto out;
    }
    snprintf(name, sizeof(name), "%s_ctrl_pool_%d",
             pi->eth_dev->device->driver->name, pi->eth_dev->data->port_id);
    q->mb_pool = rte_pktmbuf_pool_create(
        name, s->ctrlq[i].q.size, RTE_CACHE_LINE_SIZE, RTE_MBUF_PRIV_ALIGN,
        RTE_MBUF_DEFAULT_BUF_SIZE, SOCKET_ID_ANY);
    if (!q->mb_pool) {
      err = -rte_errno;
      dev_err(adapter, "Can't create ctrl pool for port %d. Err: %d\n",
...
int cxgbe_setup_sge_fwevtq(struct adapter *adapter) {
  ...
  err = t4_sge_alloc_rxq(adapter, &s->fw_evtq, true, adapter->eth_dev, msi_idx,
                         NULL, fwevtq_handler, -1, NULL, 0, rte_socket_id());
  return err;
}
...
int cxgb4_set_rspq_intr_params(struct sge_rspq *q, unsigned int us,
  ...
  if (cnt) {
    ...
    if (q->desc && q->pktcnt_idx != new_idx) {
      ...
      err = t4_set_params(adap, adap->mbox, adap->pf, 0, 1, &v, &new_idx);
      if (err)
        return err;
    }
  ...
  timer_val =
      (us == 0) ? X_TIMERREG_RESTART_COUNTER : closest_timer(&adap->sge, us);

  if ((us | cnt) == 0)
    q->intr_params = V_QINTR_TIMER_IDX(X_TIMERREG_UPDATE_CIDX);
  else
    q->intr_params = V_QINTR_TIMER_IDX(timer_val) | V_QINTR_CNT_EN(cnt > 0);
  return 0;
}
  ...
  if (t->tid_tab[tid]) {
    ...
    if (t->hash_base && tid >= t->hash_base) {
      if (family == FILTER_TYPE_IPV4)
        __atomic_sub_fetch(&t->hash_tids_in_use, 1, __ATOMIC_RELAXED);
    } else {
      if (family == FILTER_TYPE_IPV4)
        __atomic_sub_fetch(&t->tids_in_use, 1, __ATOMIC_RELAXED);
    }
  }
...
void cxgbe_insert_tid(struct tid_info *t, void *data, unsigned int tid,
  ...
  if (t->hash_base && tid >= t->hash_base) {
    if (family == FILTER_TYPE_IPV4)
      __atomic_add_fetch(&t->hash_tids_in_use, 1, __ATOMIC_RELAXED);
  } else {
    if (family == FILTER_TYPE_IPV4)
      __atomic_add_fetch(&t->tids_in_use, 1, __ATOMIC_RELAXED);
  }
...
static int tid_init(struct tid_info *t) {
  ...
  size = t->ntids * sizeof(*t->tid_tab) + max_ftids * sizeof(*t->ftid_tab) +
  ...
  t->ftid_bmap = rte_bitmap_init(t->nftids, t->ftid_bmap_array, ftid_bmap_size);
  if (!t->ftid_bmap) {
    tid_free(t);
    return -ENOMEM;
  }
...
int cxgbe_cfg_queue_count(struct rte_eth_dev *eth_dev) {
  ...
  for_each_port(adap, i) {
    ...
    if (max_rxqs <= temp_pi->n_rx_qsets || max_txqs <= temp_pi->n_tx_qsets)
      return -ENOMEM;
  ...
  if ((eth_dev->data->nb_rx_queues < 1) || (eth_dev->data->nb_tx_queues < 1))
    return -EINVAL;
...
int cxgbe_cfg_queues(struct rte_eth_dev *eth_dev) {
  ...
  if (!(adap->flags & CFG_QUEUES)) {
    s->ethrxq = rte_calloc_socket(
        NULL, s->max_ethqsets, sizeof(struct sge_eth_rxq), 0, rte_socket_id());
    if (!s->ethrxq)
      return -ENOMEM;

    s->ethtxq = rte_calloc_socket(
        NULL, s->max_ethqsets, sizeof(struct sge_eth_txq), 0, rte_socket_id());
    if (!s->ethtxq) {
      rte_free(s->ethrxq);
      s->ethrxq = NULL;
      return -ENOMEM;
...
void cxgbe_stats_get(struct port_info *pi, struct port_stats *stats) {
  t4_get_port_stats_offset(pi->adapter, pi->tx_chan, stats, &pi->stats_base);
}
...
static void setup_memwin(struct adapter *adap) {
   ...
   */
  t4_write_reg(adap,
               PCIE_MEM_ACCESS_REG(A_PCIE_MEM_ACCESS_BASE_WIN, MEMWIN_NIC),
...
void cxgbe_print_port_info(struct adapter *adap) {
  ...
  for_each_port(adap, i) {
    ...
    dev_info(adap, " " PCI_PRI_FMT " Chelsio rev %d %s %s\n", loc->domain,
...
static void configure_vlan_types(struct adapter *adapter) {
  ...
  for_each_port(adapter, i) {
    ...
    t4_set_reg_field(adapter, MPS_PORT_RX_OVLAN_REG(i, A_RX_OVLAN0),
                     V_OVLAN_MASK(M_OVLAN_MASK) | V_OVLAN_ETYPE(M_OVLAN_ETYPE),
                     V_OVLAN_MASK(M_OVLAN_MASK) | V_OVLAN_ETYPE(0x88a8));
    /* OVLAN Type 0x9100 */
    t4_set_reg_field(adapter, MPS_PORT_RX_OVLAN_REG(i, A_RX_OVLAN1),
                     V_OVLAN_MASK(M_OVLAN_MASK) | V_OVLAN_ETYPE(M_OVLAN_ETYPE),
                     V_OVLAN_MASK(M_OVLAN_MASK) | V_OVLAN_ETYPE(0x9100));

    /* IVLAN 0X8100 */
    t4_set_reg_field(adapter, MPS_PORT_RX_IVLAN(i),
                     V_IVLAN_ETYPE(M_IVLAN_ETYPE), V_IVLAN_ETYPE(0x8100));

    t4_set_reg_field(adapter, MPS_PORT_RX_CTL(i),
                     F_OVLAN_EN0 | F_OVLAN_EN1 | F_IVLAN_EN,
                     F_OVLAN_EN0 | F_OVLAN_EN1 | F_IVLAN_EN);
  }
...
static int configure_filter_mode_mask(struct adapter *adap) {
  ...
  if (adap->devargs.filtermask & ~adap->devargs.filtermode) {
    pr_err(
        "Unsupported, filtermask (0x%x) must be subset of filtermode (0x%x)\n",
  ...
  ret = cxgbe_get_filter_mode_from_devargs(adap->devargs.filtermode, true);
  if (ret < 0) {
  ...
  ret = cxgbe_get_filter_mode_from_devargs(adap->devargs.filtermask, false);
  if (ret < 0) {
  ...
  return t4_set_params(adap, adap->mbox, adap->pf, 0, nparams, params, val);
}
...
static void configure_pcie_ext_tag(struct adapter *adapter) {
  ...
  if (pos > 0) {
    ...
    if (is_t6(adapter->params.chip)) {
      t4_set_reg_field(adapter, A_PCIE_CFG2, V_T6_TOTMAXTAG(M_T6_TOTMAXTAG),
                       V_T6_TOTMAXTAG(7));
      t4_set_reg_field(adapter, A_PCIE_CMD_CFG, V_T6_MINTAG(M_T6_MINTAG),
                       V_T6_MINTAG(8));
    } else {
      t4_set_reg_field(adapter, A_PCIE_CFG2, V_TOTMAXTAG(M_TOTMAXTAG),
                       V_TOTMAXTAG(3));
      t4_set_reg_field(adapter, A_PCIE_CMD_CFG, V_MINTAG(M_MINTAG),
                       V_MINTAG(8));
    }
  }
}
...
static int adap_init0_tweaks(struct adapter *adapter) {
   ...
   */
  t4_tp_wr_bits_indirect(adapter, A_TP_INGRESS_CONFIG, F_CSUM_HAS_PSEUDO_HDR,
                         0);
...
static int adap_init0_config(struct adapter *adapter, int reset) {
   ...
   */
  if (reset) {
    ret = t4_fw_reset(adapter, adapter->mbox, F_PIORSTMODE | F_PIORST);
    if (ret < 0) {
  ...
  if (cfg_addr < 0) {
    ...
    dev_warn(
        adapter,
        "Finding address for firmware config file in flash failed, error %d\n",
        -ret);
    goto bye;
  }
  ...
  t4_set_params(adapter, adapter->mbox, adapter->pf, 0, 1, &param, &val);
  ...
  caps_cmd.cfvalid_to_len16 = cpu_to_be32(
      F_FW_CAPS_CONFIG_CMD_CFVALID | V_FW_CAPS_CONFIG_CMD_MEMTYPE_CF(mtype) |
      V_FW_CAPS_CONFIG_CMD_MEMADDR64K_CF(maddr >> 16) | FW_LEN16(caps_cmd));
  ...
  if (finicsum != cfcsum)
    dev_warn(adapter,
             "Configuration File checksum mismatch: [fini] csum=%#x, computed "
             "csum=%#x\n",
             finicsum, cfcsum);
   ...
   */
  dev_info(adapter,
           "Successfully configured using Firmware Configuration File \"%s\", "
           "version %#x, computed checksum %#x\n",
...
static int adap_init0(struct adapter *adap) {
  ...
  if (state == DEV_STATE_INIT) {
    ...
    if (ret == -ENOENT) {
      dev_err(adap, "No Configuration File present on adapter. Using "
  ...
  if (ret) {
    dev_err(adap->pdev_dev, "Unable to retrieve resource provisioning info\n");
    goto bye;
  }
  ...
  caps_cmd.op_to_write =
      htonl(V_FW_CMD_OP(FW_CAPS_CONFIG_CMD) | F_FW_CMD_REQUEST | F_FW_CMD_READ);
  caps_cmd.cfvalid_to_len16 = htonl(FW_LEN16(caps_cmd));
  ret = t4_wr_mbox(adap, adap->mbox, &caps_cmd, sizeof(caps_cmd), &caps_cmd);
  if (ret < 0)
    goto bye;
...
void t4_os_portmod_changed(const struct adapter *adap, int port_id) {
  static const char *const mod_str[] = {NULL,         "LR",        "SR", "ER",
                                        "passive DA", "active DA", "LRM"};
...
void t4_os_link_changed(struct adapter *adap, int port_id) {
   ...
   * then return.
   */
  if (adap->params.vi_enable_rx == 0 || pi->vi_en_rx == pi->link_cfg.link_ok)
    return;
...
int cxgbe_link_start(struct port_info *pi) {
  ...
  ret = t4_set_rxmode(adapter, adapter->mbox, pi->viid, mtu, -1, -1, -1,
                      !!(conf_offloads & RTE_ETH_RX_OFFLOAD_VLAN_STRIP), true);
  if (ret == 0) {
  ...
  if (ret == 0) {
    ...
    pi->vi_en_tx = 1;
    ret = t4_enable_vi_params(adapter, adapter->mbox, pi->viid, pi->vi_en_rx,
                              pi->vi_en_tx, false);
  }
...
int cxgbe_write_rss_conf(const struct port_info *pi, uint64_t rss_hf) {
  ...
  if (rss_hf & RTE_ETH_RSS_NONFRAG_IPV4_UDP)
    flags |= F_FW_RSS_VI_CONFIG_CMD_IP4FOURTUPEN | F_FW_RSS_VI_CONFIG_CMD_UDPEN;
  ...
  if (rss_hf & CXGBE_RSS_HF_UDP_IPV6_MASK)
    flags |= F_FW_RSS_VI_CONFIG_CMD_IP6TWOTUPEN |
             F_FW_RSS_VI_CONFIG_CMD_IP6FOURTUPEN | F_FW_RSS_VI_CONFIG_CMD_UDPEN;
   ...
   */
  err = t4_config_vi_rss(adapter, adapter->mbox, pi->viid, flags, rss);
  return err;
}
...
int cxgbe_write_rss(const struct port_info *pi, const u16 *queues) {
  ...
  err = t4_config_rss_range(adapter, adapter->pf, pi->viid, 0, pi->rss_size,
                            rss, pi->rss_size);
  rte_free(rss);
  return err;
}
...
static void enable_rx(struct adapter *adap, struct sge_rspq *q) {
  ...
  t4_write_reg(adap,
               is_pf4(adap) ? MYPF_REG(A_SGE_PF_GTS)
...
static void fw_caps_to_speed_caps(enum fw_port_type port_type,
                                  unsigned int fw_caps, u32 *speed_caps) {
...
void cxgbe_get_speed_caps(struct port_info *pi, u32 *speed_caps) {
  *speed_caps = 0;
  ...
  fw_caps_to_speed_caps(pi->link_cfg.port_type, pi->link_cfg.pcaps, speed_caps);
...
static void adap_smt_index(struct adapter *adapter, u32 *smt_start_idx,
  ...
  ret = t4_query_params(adapter, adapter->mbox, adapter->pf, 0, 2, params,
                        smt_val);
...
int cxgbe_probe(struct adapter *adapter) {
  ...
  func = CHELSIO_CHIP_VERSION(chip) <= CHELSIO_T5 ? G_SOURCEPF(whoami)
  ...
  if (!is_t4(adapter->params.chip)) {
    ...
    qpp =
        1 << ((t4_read_reg(adapter, A_SGE_EGRESS_QUEUES_PER_PAGE_PF) >> s_qpp) &
              M_QUEUESPERPAGEPF0);
    num_seg = CXGBE_PAGE_SIZE / UDBS_SEG_SIZE;
    if (qpp > num_seg)
      dev_warn(adapter, "Incorrect SGE EGRESS QUEUES_PER_PAGE configuration, "
    ...
    t4_write_reg(adapter, A_SGE_STAT_CFG, V_STATSOURCE_T5(7) | V_STATMODE(0));
  }
  ...
  for_each_port(adapter, i) {
    ...
    snprintf(name, sizeof(name), "%s_%d", adapter->pdev->device.name, i);
    ...
    eth_dev->data->dev_private = rte_zmalloc_socket(
        name, sizeof(struct port_info), RTE_CACHE_LINE_SIZE, numa_node);
  ...
  allocate_mac:
    ...
    pi->eth_dev->data->mac_addrs = rte_zmalloc(name, RTE_ETHER_ADDR_LEN, 0);
    if (!pi->eth_dev->data->mac_addrs) {
      dev_err(adapter,
              "%s: Mem allocation failed for storing mac addr, aborting\n",
              __func__);
      err = -1;
      goto out_free;
    }
  ...
  if (is_hashfilter(adapter)) {
    ...
    dev_warn(adapter, "Maskless filter support disabled. Continuing\n");
  }
