...
void cxgbe_do_l2t_write_rpl(struct adapter *adap,
  ...
  if (unlikely(rpl->status != CPL_ERR_NONE)) {
    dev_err(adap, "Unexpected L2T_WRITE_RPL status %u for entry %u\n",
            rpl->status, l2t_idx);
    return;
  }
...
static int write_l2e(struct rte_eth_dev *dev, struct l2t_entry *e, int sync,
  ...
  OPCODE_TID(req) = cpu_to_be32(MK_OPCODE_TID(
      CPL_L2T_WRITE_REQ,
      l2t_idx | V_SYNC_WR(sync) | V_TID_QID(adap->sge.fw_evtq.abs_id)));
  req->params = cpu_to_be16(V_L2T_W_PORT(e->lport) | V_L2T_W_LPBK(loopback) |
...
static struct l2t_entry *find_or_alloc_l2e(struct l2t_data *d, u16 vlan,
  ...
  for (e = &d->l2tab[0], end = &d->l2tab[d->l2t_size]; e != end; ++e) {
    if (__atomic_load_n(&e->refcnt, __ATOMIC_RELAXED) == 0) {
      ...
      if (e->state == L2T_STATE_SWITCHING) {
        if ((!memcmp(e->dmac, dmac, RTE_ETHER_ADDR_LEN)) && e->vlan == vlan &&
            e->lport == port)
          goto exists;
      }
    }
...
static struct l2t_entry *t4_l2t_alloc_switching(struct rte_eth_dev *dev,
  ...
  if (e) {
    ...
    if (__atomic_load_n(&e->refcnt, __ATOMIC_RELAXED) == 0) {
      ...
      if (ret < 0)
        dev_debug(adap, "Failed to write L2T entry: %d", ret);
    } else {
