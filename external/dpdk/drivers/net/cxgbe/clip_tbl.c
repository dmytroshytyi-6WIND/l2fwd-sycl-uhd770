...
static int clip6_get_mbox(const struct rte_eth_dev *dev, const u32 *lip) {
  ...
  memset(&c, 0, sizeof(c));
  c.op_to_write =
      cpu_to_be32(V_FW_CMD_OP(FW_CLIP_CMD) | F_FW_CMD_REQUEST | F_FW_CMD_WRITE);
...
static int clip6_release_mbox(const struct rte_eth_dev *dev, const u32 *lip) {
  ...
  memset(&c, 0, sizeof(c));
  c.op_to_write =
      cpu_to_be32(V_FW_CMD_OP(FW_CLIP_CMD) | F_FW_CMD_REQUEST | F_FW_CMD_READ);
...
static struct clip_entry *t4_clip_alloc(struct rte_eth_dev *dev, u32 *lip,
                                        u8 v6) {
  struct adapter *adap = ethdev2adap(dev);
  ...
  if (ce) {
    ...
    if (__atomic_load_n(&ce->refcnt, __ATOMIC_RELAXED) == 0) {
      ...
      if (v6) {
        ...
        __atomic_store_n(&ce->refcnt, 1, __ATOMIC_RELAXED);
        ret = clip6_get_mbox(dev, lip);
        if (ret)
          dev_debug(adap, "CLIP FW ADD CMD failed: %d", ret);
      } else {
...
struct clip_tbl *t4_init_clip_tbl(unsigned int clipt_start,
  ...
  ctbl = t4_os_alloc(sizeof(*ctbl) + clipt_size * sizeof(struct clip_entry));
  if (!ctbl)
    return NULL;
