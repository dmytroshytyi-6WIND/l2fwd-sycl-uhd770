 ...
 */
static inline struct port_info *adap2pinfo(const struct adapter *adap,
                                           int idx) {
...
static inline void t4_os_pci_write_cfg4(struct adapter *adapter, size_t addr,
  ...
  if (rte_pci_write_config(adapter->pdev, &val32, sizeof(val32), addr) < 0)
...
static inline void t4_os_pci_read_cfg4(struct adapter *adapter, size_t addr,
                                       u32 *val) {
  if (rte_pci_read_config(adapter->pdev, val, sizeof(*val), addr) < 0)
...
static inline void t4_os_pci_write_cfg2(struct adapter *adapter, size_t addr,
  ...
  if (rte_pci_write_config(adapter->pdev, &val16, sizeof(val16), addr) < 0)
    dev_err(adapter, "Can't write to PCI config space\n");
}
 ...
 * @lock: the spinlock
 */
static inline void t4_os_lock(rte_spinlock_t *lock) { rte_spinlock_lock(lock); }
...
static inline struct mbox_entry *
t4_os_list_first_entry(struct mbox_list *head) {
...
int t4_sge_alloc_rxq(struct adapter *adap, struct sge_rspq *rspq, bool fwevtq,
                     ...
                     struct sge_fl *fl, rspq_handler_t handler, int cong,
