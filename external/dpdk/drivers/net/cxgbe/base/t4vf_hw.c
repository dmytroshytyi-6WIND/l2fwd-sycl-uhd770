...
int t4vf_wr_mbox_core(struct adapter *adapter,
   ...
   */
  if ((size % 16) != 0 || size > NUM_CIM_VF_MAILBOX_DATA_INSTANCES * 4)
    return -EINVAL;
  ...
  t4_write_reg(adapter, mbox_ctl, F_MBMSGVALID | V_MBOWNER(X_MBOWNER_FW));
  ...
  for (i = 0; i < FW_CMD_MAX_TIMEOUT; i++) {
    ...
    if (G_MBOWNER(v) == X_MBOWNER_PL) {
       ...
       */
      if ((v & F_MBMSGVALID) == 0) {
        t4_write_reg(adapter, mbox_ctl, V_MBOWNER(X_MBOWNER_NONE));
        continue;
      }
      ...
      if (rpl) {
        ...
        WARN_ON((be32_to_cpu(*(const u32 *)cmd) & F_FW_CMD_REQUEST) == 0);
...
int t4vf_fw_reset(struct adapter *adapter) {
  ...
  cmd.op_to_write = cpu_to_be32(V_FW_CMD_OP(FW_RESET_CMD) | F_FW_CMD_WRITE);
...
int t4vf_prep_adapter(struct adapter *adapter) {
  ...
  switch (ver) {
  case CHELSIO_T5:
    adapter->params.chip |= CHELSIO_CHIP_CODE(CHELSIO_T5, pl_vf_rev);
    adapter->params.arch.sge_fl_db = F_DBPRIO | F_DBTYPE;
    adapter->params.arch.mps_tcam_size = NUM_MPS_T5_CLS_SRAM_L_INSTANCES;
    break;
  case CHELSIO_T6:
    adapter->params.chip |= CHELSIO_CHIP_CODE(CHELSIO_T6, pl_vf_rev);
...
int t4vf_query_params(struct adapter *adapter, unsigned int nparams,
  ...
  memset(&cmd, 0, sizeof(cmd));
  cmd.op_to_vfn = cpu_to_be32(V_FW_CMD_OP(FW_PARAMS_CMD) | F_FW_CMD_REQUEST |
                              F_FW_CMD_READ);
  len16 = DIV_ROUND_UP(offsetof(struct fw_params_cmd, param[nparams]), 16);
...
int t4vf_get_rss_glb_config(struct adapter *adapter) {
   ...
   * VF Drivers ...
   */
  rss->mode = G_FW_RSS_GLB_CONFIG_CMD_MODE(be32_to_cpu(rpl.u.manual.mode_pkd));
  switch (rss->mode) {
  case FW_RSS_GLB_CONFIG_CMD_MODE_BASICVIRTUAL: {
    u32 word = be32_to_cpu(rpl.u.basicvirtual.synmapen_to_hashtoeplitz);
...
static int t4vf_get_port_stats_fw(struct adapter *adapter, int pidx,
  ...
  while (rem) {
    ...
    size_t len =
        (offsetof(struct fw_vi_stats_cmd, u) + sizeof(struct fw_vi_stats_ctl));
    ...
    cmd.op_to_viid = cpu_to_be32(V_FW_CMD_OP(FW_VI_STATS_CMD) |
                                 ...
                                 F_FW_CMD_REQUEST | F_FW_CMD_READ);
    cmd.retval_len16 = cpu_to_be32(V_FW_CMD_LEN16(len16));
    cmd.u.ctl.nstats_ix = cpu_to_be16(V_FW_VI_STATS_CMD_IX(ix) |
...
#define GET_STAT(name)                                                         \
  t4_read_reg64(adapter, T4VF_MPS_BASE_ADDR + A_MPS_VF_STAT_##name##_L)
  p->tx_octets = GET_STAT(TX_VF_BCAST_BYTES) + GET_STAT(TX_VF_MCAST_BYTES) +
...
static int t4vf_alloc_vi(struct adapter *adapter, int port_id) {
   ...
   */
  memset(&cmd, 0, sizeof(cmd));
  cmd.op_to_vfn = cpu_to_be32(V_FW_CMD_OP(FW_VI_CMD) | F_FW_CMD_REQUEST |
                              F_FW_CMD_WRITE | F_FW_CMD_EXEC);
  cmd.alloc_to_len16 = cpu_to_be32(FW_LEN16(cmd) | F_FW_VI_CMD_ALLOC);
...
int t4vf_port_init(struct adapter *adapter) {
  ...
  for_each_port(adapter, i) {
    ...
    vi_cmd.op_to_vfn =
        cpu_to_be32(V_FW_CMD_OP(FW_VI_CMD) | F_FW_CMD_REQUEST | F_FW_CMD_READ);
    ...
    p->rss_size = G_FW_VI_CMD_RSSSIZE(be16_to_cpu(vi_rpl.norss_rsssize));
    ...
    port_cmd.op_to_portid =
        cpu_to_be32(V_FW_CMD_OP(FW_PORT_CMD) | F_FW_CMD_REQUEST |
                    F_FW_CMD_READ | V_FW_PORT_CMD_PORTID(p->port_id));
    val = FW_PORT_ACTION_GET_PORT_INFO32;
    port_cmd.action_to_len16 =
        cpu_to_be32(V_FW_PORT_CMD_ACTION(val) | FW_LEN16(port_cmd));
