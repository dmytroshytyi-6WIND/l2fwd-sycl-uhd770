...
void cxgbe_do_smt_write_rpl(struct adapter *adap,
  ...
  if (unlikely(rpl->status != CPL_ERR_NONE)) {
    ...
    dev_err(adap, "Unexpected SMT_WRITE_RPL status %u for entry %u\n",
            rpl->status, smtidx);
    t4_os_lock(&e->lock);
...
static int write_smt_entry(struct rte_eth_dev *dev, struct smt_entry *e) {
  ...
  if (CHELSIO_CHIP_VERSION(adap->params.chip) <= CHELSIO_T5) {
    ...
    if (e->idx & 1) {
      req->pfvf1 = 0x0;
      rte_memcpy(req->src_mac1, e->src_mac, RTE_ETHER_ADDR_LEN);
      ...
      req->pfvf0 = 0x0;
      rte_memcpy(req->src_mac0, e->src_mac, RTE_ETHER_ADDR_LEN);
    ...
    t6req->pfvf0 = 0x0;
    rte_memcpy(t6req->src_mac0, s->smtab[e->idx].src_mac, RTE_ETHER_ADDR_LEN);
    row = e->hw_idx;
    req = (struct cpl_smt_write_req *)t6req;
  }

  OPCODE_TID(req) = cpu_to_be32(MK_OPCODE_TID(
      CPL_SMT_WRITE_REQ, e->hw_idx | V_TID_QID(adap->sge.fw_evtq.abs_id)));

  req->params =
      cpu_to_be32(V_SMTW_NORPL(0) | V_SMTW_IDX(row) | V_SMTW_OVLAN_IDX(0));
  ...
  for (e = &s->smtab[0], end = &s->smtab[s->smt_size]; e != end; ++e) {
    if (__atomic_load_n(&e->refcnt, __ATOMIC_RELAXED) == 0) {
      ...
      if (e->state == SMT_STATE_SWITCHING) {
         ...
         * re-use it ?
         */
        if (!memcmp(e->src_mac, smac, RTE_ETHER_ADDR_LEN))
          goto found;
      }
    }
  }
