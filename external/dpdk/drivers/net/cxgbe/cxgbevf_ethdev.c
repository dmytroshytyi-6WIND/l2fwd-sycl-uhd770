...
static int cxgbevf_dev_stats_get(struct rte_eth_dev *eth_dev,
  ...
  /* TX Stats */
  eth_stats->opackets =
      ps.tx_bcast_frames + ps.tx_mcast_frames + ps.tx_ucast_frames;
...
static int eth_cxgbevf_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  snprintf(name, sizeof(name), "cxgbevfadapter%d", eth_dev->data->port_id);
...
RTE_PMD_REGISTER_PARAM_STRING(net_cxgbevf, CXGBE_DEVARG_CMN_KEEP_OVLAN
                              "=<0|1> " CXGBE_DEVARG_CMN_TX_MODE_LATENCY
                              "=<0|1> " CXGBE_DEVARG_VF_FORCE_LINK_UP
                              "=<0|1> ");
