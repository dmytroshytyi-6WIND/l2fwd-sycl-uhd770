...
static int rxp_create_mkey(struct mlx5_regex_priv *priv, void *ptr, size_t size,
                           uint32_t access, struct mlx5_regex_mkey *mkey);
static inline void rxp_destroy_mkey(struct mlx5_regex_mkey *mkey);

int mlx5_regex_info_get(struct rte_regexdev *dev __rte_unused,
  ...
  info->regexdev_capa =
      RTE_REGEXDEV_SUPP_PCRE_GREEDY_F | RTE_REGEXDEV_CAPA_QUEUE_PAIR_OOS_F;
  info->rule_flags = 0;
...
int mlx5_regex_rules_db_import(struct rte_regexdev *dev, const char *rule_db,
  ...
  for (id = 0; id < priv->nb_engines; id++) {
    ret = mlx5_devx_regex_rules_program(priv->cdev->ctx, id, mkey.mkey->id,
                                        rule_db_len, (uintptr_t)ptr);
    if (ret < 0) {
...
int mlx5_regex_configure(struct rte_regexdev *dev,
  ...
  priv->qps =
      rte_zmalloc(NULL, sizeof(struct mlx5_regex_qp) * priv->nb_queues, 0);
  if (!priv->qps) {
