...
int mlx5_regex_close(struct rte_regexdev *dev __rte_unused) { return 0; }

static void mlx5_regex_get_name(char *name, struct rte_device *dev) {
  sprintf(name, "mlx5_regex_%s", dev->name);
}

static int mlx5_regex_dev_probe(struct mlx5_common_device *cdev,
  ...
  if ((!attr->regexp_params && !attr->mmo_regex_sq_en &&
...
static int mlx5_regex_dev_remove(struct mlx5_common_device *cdev) {
  char name[RTE_REGEXDEV_NAME_MAX_LEN];
...
RTE_PMD_REGISTER_KMOD_DEP(MLX5_REGEX_DRIVER_NAME,
