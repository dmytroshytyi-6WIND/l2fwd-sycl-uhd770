 ...
 */
static uint16_t regex_ctrl_get_nb_obj(uint16_t nb_desc) {
 ...
 */
static int regex_ctrl_destroy_cq(struct mlx5_regex_cq *cq) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int regex_ctrl_create_cq(struct mlx5_regex_priv *priv,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int regex_ctrl_destroy_hw_qp(struct mlx5_regex_qp *qp, uint16_t q_ind) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int regex_ctrl_create_hw_qp(struct mlx5_regex_priv *priv,
                                   struct mlx5_regex_qp *qp, uint16_t q_ind,
                                   uint16_t log_nb_desc) {
  ...
  attr.num_of_send_wqbbs =
      RTE_BIT32(MLX5_REGEX_WQE_LOG_NUM(priv->has_umr, log_nb_desc));
...
int mlx5_regex_qp_setup(struct rte_regexdev *dev, uint16_t qp_ind,
  ...
  if (qp->flags & RTE_REGEX_QUEUE_PAIR_CFG_OOS_F)
    ...
    qp->nb_obj = 1;
  qp->qps = rte_malloc(NULL, qp->nb_obj * sizeof(struct mlx5_regex_hw_qp), 64);
  if (!qp->qps) {
