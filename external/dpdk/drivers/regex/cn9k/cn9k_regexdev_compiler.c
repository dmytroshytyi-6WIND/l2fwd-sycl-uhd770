...
static int ree_rule_db_compile(const struct rte_regexdev_rule *rules,
                               uint16_t nb_rules, struct rxp_rof **rof,
                               struct rxp_rof **rofi,
  ...
  enum rxp_virtual_prefix_mode virtual_prefix_mode = RXP_VIRTUAL_PREFIX_MODE_0;
  ...
  ruleset.rules =
      rte_malloc("rxp_rule_entry", nb_rules * sizeof(struct rxp_rule_entry), 0);
  ...
  for (i = 0; i < nb_rules; i++) {
    ...
    ruleset.rules[i].rule_direction_type = RXP_RULE_DIRECTION_TYPE_NONE;
  }

  ret = rxp_compile_advanced(
      /*INPUT*/
      &ruleset, prefix_selection_control_list, rof_for_incremental_compile,
      roff_for_incremental_compile, rofi_for_incremental_compile,
      rule_ids_to_remove, blacklist_sample_data,

      /*OPTIONS*/
      compiler_options, prefix_capacity, global_regex_options,
      set_auto_blacklist, set_max, objective, ptpb_threshold, max_rep_max,
      threads, set_rxp_version, verbose, tpe_data_width, virtual_prefix_mode,
      compiler_output_flags, divide_ruleset,

      /*OUTPUT*/
      &compilation_statistics, &compiled_rules_log, &critical_rules_rank,
      &rule_direction_analysis, &uncompiled_rules_log, rof, &roff, rofi,
      &generated_pscl);
...
int cn9k_ree_rule_db_compile_prog(struct rte_regexdev *dev) {
  ...
  if (rule_db_len > 0) {
    ...
    ret = roc_ree_rule_db_get(vf, rule_db, rule_db_len, (char *)rule_dbi,
                              rule_dbi_len);
    if (ret) {
    ...
    rof_inc.number_of_entries = (rule_db_len / sizeof(struct rxp_rof_entry));
    ...
    ret = ree_rule_db_compile(data->rules, data->nb_rules, &rof, &rofi,
                              &rof_inc, rofi_inc_p);
    ...
    rule_dbi_len = rofi->number_of_entries * sizeof(struct rxp_rof_entry);
    rofi_rof_entries = (char *)rofi->rof_entries;
  } else {
    /* full compilation */
    ret = ree_rule_db_compile(data->rules, data->nb_rules, &rof, &rofi, NULL,
                              NULL);
  }
  if (ret != 0) {
...
free_structs:
  rxp_free_structs(NULL, NULL, NULL, NULL, NULL, &rof, NULL, &rofi, NULL, 1);
