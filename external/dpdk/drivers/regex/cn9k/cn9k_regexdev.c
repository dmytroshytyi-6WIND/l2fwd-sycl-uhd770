...
static void qp_memzone_name_get(char *name, int size, int dev_id, int qp_id) {
  snprintf(name, size, "cn9k_ree_lf_mem_%u:%u", dev_id, qp_id);
}

static struct roc_ree_qp *ree_qp_create(const struct rte_regexdev *dev,
                                        uint16_t qp_id) {
  ...
  /* Allocate queue pair */
  qp = rte_zmalloc("CN9K Regex PMD Queue Pair", sizeof(*qp), ROC_ALIGN);
  if (qp == NULL) {
  ...
  lf_mem = rte_memzone_reserve_aligned(
      name, len, rte_socket_id(),
...
static int ree_qp_destroy(const struct rte_regexdev *dev,
                          struct roc_ree_qp *qp) {
  const struct rte_memzone *lf_mem;
  char name[RTE_MEMZONE_NAMESIZE];
  int ret;
...
static int ree_queue_pair_release(struct rte_regexdev *dev, uint16_t qp_id) {
  struct cn9k_ree_data *data = dev->data->dev_private;
...
static struct rte_regexdev *ree_dev_register(const char *name) {
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    dev->data->dev_private = rte_zmalloc_socket(
        "regexdev device private", sizeof(struct cn9k_ree_data),
        RTE_CACHE_LINE_SIZE, rte_socket_id());

    if (dev->data->dev_private == NULL) {
      cn9k_err("Cannot allocate memory for dev %s private data", name);
...
static int ree_dev_unregister(struct rte_regexdev *dev) {
...
static int ree_dev_fini(struct rte_regexdev *dev) {
  struct cn9k_ree_data *data = dev->data->dev_private;
  struct roc_ree_vf *vf = &data->vf;
  int i, ret;
...
static inline int ree_enqueue(struct roc_ree_qp *qp, struct rte_regex_ops *op,
...
static uint16_t cn9k_ree_enqueue_burst(struct rte_regexdev *dev, uint16_t qp_id,
...
static inline void ree_dequeue_post_process(struct rte_regex_ops *ops) {
  ...
  if (ops->nb_matches > 0) {
    ...
    off = ((ops->nb_matches < REE_NUM_MATCHES_ALIGN) ? ops->nb_matches
                                                     : REE_NUM_MATCHES_ALIGN);
    match = (uint64_t)ops + REE_MATCH_OFFSET;
    match += (ops->nb_matches - off) * sizeof(union ree_match);
    memcpy((void *)ops->matches, (void *)match, off * sizeof(union ree_match));
  }
}

static uint16_t cn9k_ree_dequeue_burst(struct rte_regexdev *dev, uint16_t qp_id,
                                       struct rte_regex_ops **ops,
                                       uint16_t nb_ops) {
  struct cn9k_ree_data *data = dev->data->dev_private;
  struct roc_ree_qp *qp = data->queue_pairs[qp_id];
...
static int cn9k_ree_dev_info_get(struct rte_regexdev *dev,
...
static int cn9k_ree_dev_config(struct rte_regexdev *dev,
  ...
  if (data->queue_pairs == NULL) { /* first time configuration */
    data->queue_pairs =
        rte_zmalloc("regexdev->queue_pairs",
                    sizeof(data->queue_pairs[0]) * cfg->nb_queue_pairs,
                    RTE_CACHE_LINE_SIZE);
  ...
  if (cfg->rule_db && cfg->rule_db_len) {
    ...
    rule_db_len = rule_db->number_of_entries * sizeof(struct ree_rule_db_entry);
    cn9k_ree_dbg("rule_db number of entries %d", rule_db->number_of_entries);
    if (rule_db_len > cfg->rule_db_len) {
      cn9k_err("Could not program rule db");
      ret = -EINVAL;
      goto queues_detach;
    }
    ret = roc_ree_rule_db_prog(vf, (const char *)rule_db->entries, rule_db_len,
                               NULL, REE_NON_INC_PROG);
    if (ret) {
      cn9k_err("Could not program rule db");
      goto queues_detach;
    }
...
static int cn9k_ree_stop(struct rte_regexdev *dev) {
  RTE_SET_USED(dev);
...
static int cn9k_ree_start(struct rte_regexdev *dev) {
  struct cn9k_ree_data *data = dev->data->dev_private;
  struct roc_ree_vf *vf = &data->vf;
...
static int cn9k_ree_close(struct rte_regexdev *dev) {
...
cn9k_ree_queue_pair_setup(struct rte_regexdev *dev, uint16_t qp_id,
  ...
  if (qp_conf->nb_desc > REE_DEFAULT_CMD_QLEN) {
    cn9k_err("Could not setup queue pair for %u descriptors", qp_conf->nb_desc);
    return -EINVAL;
  }
...
static int cn9k_ree_rule_db_compile_activate(struct rte_regexdev *dev) {
  return cn9k_ree_rule_db_compile_prog(dev);
}

static int cn9k_ree_rule_db_update(struct rte_regexdev *dev,
  ...
  if (data->nb_rules == 0) {
    ...
    memcpy(data->rules, rules, nb_rules * sizeof(struct rte_regexdev_rule));
    data->nb_rules = nb_rules;
  } else {

    old_ptr = data->rules;
    sum_nb_rules = data->nb_rules + nb_rules;
    data->rules = rte_realloc(
        data->rules, sum_nb_rules * sizeof(struct rte_regexdev_rule), 0);
    if (data->rules == NULL) {
      data->rules = old_ptr;
      return -ENOMEM;
    }
...
static int cn9k_ree_rule_db_import(struct rte_regexdev *dev,
  ...
  ree_rule_db_len =
      ree_rule_db->number_of_entries * sizeof(struct ree_rule_db_entry);
...
static int cn9k_ree_rule_db_export(struct rte_regexdev *dev, char *rule_db) {
  struct cn9k_ree_data *data = dev->data->dev_private;
  struct roc_ree_vf *vf = &data->vf;
  ...
  ret = roc_ree_rule_db_get(vf, (char *)ree_rule_db->entries, rule_db_len, NULL,
                            0);
  if (ret) {
...
static struct rte_regexdev_ops cn9k_ree_ops = {
    ...
    .dev_rule_db_compile_activate = cn9k_ree_rule_db_compile_activate,
...
static int cn9k_ree_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  data->rule_flags =
      RTE_REGEX_PCRE_RULE_ALLOW_EMPTY_F | RTE_REGEX_PCRE_RULE_ANCHORED_F;
...
static int cn9k_ree_pci_remove(struct rte_pci_device *pci_dev) {
  char name[RTE_REGEXDEV_NAME_MAX_LEN];
...
static struct rte_pci_id pci_id_ree_table[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CAVIUM, PCI_DEVID_CNXK_RVU_REE_PF)},
    {
        .vendor_id = 0,
    }};
