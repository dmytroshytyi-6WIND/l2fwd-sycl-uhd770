...
ipsec_po_out_rlen_get(struct cn9k_ipsec_sa *sa, uint32_t plen) {
  ...
  enc_payload_len =
      RTE_ALIGN_CEIL(plen + sa->rlens.roundup_len, sa->rlens.roundup_byte);

  return sa->rlens.partial_len + enc_payload_len;
}

static __rte_always_inline int ipsec_antireplay_check(struct cn9k_ipsec_sa *sa,
                                                      uint32_t win_sz,
                                                      struct rte_mbuf *m) {
  ...
  if (!esn) {
    ...
    seqh = cnxk_on_anti_replay_get_seqh(win_sz, seql, esn_hi, esn_low);
...
static __rte_always_inline int process_outb_sa(struct rte_crypto_op *cop,
  ...
  if (unlikely(extend_tail > rte_pktmbuf_tailroom(m_src))) {
    plt_dp_err("Not enough tail room (required: %d, available: %d", extend_tail,
...
static __rte_always_inline int process_inb_sa(struct rte_crypto_op *cop,
                                              struct cn9k_ipsec_sa *sa,
                                              struct cpt_inst_s *inst) {
  struct rte_crypto_sym_op *sym_op = cop->sym;
  struct rte_mbuf *m_src = sym_op->m_src;
  ...
  if (sa->replay_win_sz) {
    ...
    if (unlikely(ret)) {
      ...
      inst->w4.s.opcode_minor = ROC_SE_MISC_MINOR_OP_PASSTHROUGH;
      inst->w4.s.param1 = 1;
      /* Send out completion code only */
      inst->w4.s.param2 = (ROC_IE_ON_SWCC_ANTI_REPLAY << 8) | 0x1;
      inst->w4.s.dlen = 1;
      inst->dptr = rte_pktmbuf_iova(m_src);
      inst->rptr = inst->dptr;
      inst->w7.u64 = sa->inst.w7;
