...
static struct rte_pci_id pci_id_cpt_table[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CAVIUM, PCI_DEVID_CN10K_RVU_CPT_VF)},
    /* sentinel */
    {.device_id = 0},
};

static int cn10k_cpt_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
...
static int cn10k_cpt_pci_remove(struct rte_pci_device *pci_dev) {
  char name[RTE_CRYPTODEV_NAME_MAX_LEN];
  struct rte_cryptodev *dev;
  struct cnxk_cpt_vf *vf;
  int ret;
