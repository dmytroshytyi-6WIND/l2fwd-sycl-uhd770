...
static int cnxk_cpt_get_mlen(void) {
  uint32_t len;
  ...
  len += RTE_ALIGN_CEIL((ROC_SE_SG_LIST_HDR_SIZE +
                         (RTE_ALIGN_CEIL(ROC_SE_MAX_SG_IN_OUT_CNT, 4) >> 2) *
                             ROC_SE_SG_ENTRY_SIZE),
                        8);

  return len;
}

static int cnxk_cpt_asym_get_mlen(void) {
  uint32_t len;
...
void cnxk_cpt_dev_info_get(struct rte_cryptodev *dev,
  ...
  info->max_nb_queue_pairs = RTE_MIN(roc_cpt->nb_lf_avail, vf->max_qps_limit);
...
static int cnxk_cpt_metabuf_mempool_create(const struct rte_cryptodev *dev,
  ...
  pool = rte_mempool_create(mempool_name, mb_pool_sz, mlen, cache_sz, 0, NULL,
...
static void cnxk_cpt_metabuf_mempool_destroy(struct cnxk_cpt_qp *qp) {
...
static struct cnxk_cpt_qp *cnxk_cpt_qp_create(const struct rte_cryptodev *dev,
                                              uint16_t qp_id, uint32_t iq_len) {
  ...
  /* Allocate queue pair */
  qp = rte_zmalloc_socket("CNXK Crypto PMD Queue Pair", sizeof(*qp), ROC_ALIGN,
                          0);
  if (qp == NULL) {
  ...
  pq_mem = rte_memzone_reserve_aligned(
...
static int cnxk_cpt_qp_destroy(const struct rte_cryptodev *dev,
                               struct cnxk_cpt_qp *qp) {
  const struct rte_memzone *pq_mem;
  char name[RTE_MEMZONE_NAMESIZE];
  int ret;
...
static int cnxk_sess_fill(struct rte_crypto_sym_xform *xform,
...
static uint64_t cnxk_cpt_inst_w7_get(struct cnxk_se_sess *sess,
...
int cnxk_ae_session_cfg(struct rte_cryptodev *dev,
                        ...
                        struct rte_cryptodev_asym_session *sess) {
  struct cnxk_ae_sess *priv = (struct cnxk_ae_sess *)sess->sess_private_data;
  struct cnxk_cpt_vf *vf = dev->data->dev_private;
  struct roc_cpt *roc_cpt = &vf->cpt;
...
void cnxk_cpt_dump_on_err(struct cnxk_cpt_qp *qp) {
  ...
  plt_print(
      "\tInflight count: %" PRIu64 "",
      pending_queue_infl_cnt(pend_q->head, pend_q->tail, pend_q->pq_mask));
