...
static inline void cpt_pack_iv(uint8_t *iv_src, uint8_t *iv_dst) {
  iv_dst[16] = iv_src[16];
...
static inline void pdcp_iv_copy(uint8_t *iv_d, uint8_t *iv_s,
                                const uint8_t pdcp_alg_type, uint8_t pack_iv) {
...
static __rte_always_inline void cpt_fc_salt_update(struct roc_se_ctx *se_ctx,
                                                   uint8_t *salt) {
...
static __rte_always_inline uint32_t fill_sg_comp_from_buf(
    struct roc_se_sglist_comp *list, uint32_t i, struct roc_se_buf_ptr *from) {
  struct roc_se_sglist_comp *to = &list[i >> 2];
...
static __rte_always_inline uint32_t fill_sg_comp_from_iov(
    struct roc_se_sglist_comp *list, uint32_t i, struct roc_se_iov_ptr *from,
...
cpt_digest_gen_prep(uint32_t flags, uint64_t d_lens,
  ...
  if (ctx->hmac) {
    cpt_inst_w4.s.opcode_major = ROC_SE_MAJOR_OP_HMAC | ROC_SE_DMA_MODE;
    cpt_inst_w4.s.param1 = key_len;
    cpt_inst_w4.s.dlen = data_len + RTE_ALIGN_CEIL(key_len, 8);
  } else {
    cpt_inst_w4.s.opcode_major = ROC_SE_MAJOR_OP_HASH | ROC_SE_DMA_MODE;
  ...
  if (ctx->hmac) {
    ...
    /* Key */
    i = fill_sg_comp(gather_comp, i, k_vaddr, RTE_ALIGN_CEIL(key_len, 8));
  }
  ...
  i = 0;
  scatter_comp =
      (struct roc_se_sglist_comp *)((uint8_t *)gather_comp + g_size_bytes);
...
cpt_enc_hmac_prep(uint32_t flags, uint64_t d_offs, uint64_t d_lens,
  ...
  if (unlikely(encr_data_len & 0xf)) {
    if ((cipher_type == ROC_SE_DES3_CBC) || (cipher_type == ROC_SE_DES3_ECB))
...
cpt_pdcp_alg_prep(uint32_t req_flags, uint64_t d_offs, uint64_t d_lens,
  ...
  if (likely((req_flags & ROC_SE_SINGLE_BUF_INPLACE) &&
    ...
    *offset_vaddr = offset_ctrl;
    ...
    m_vaddr =
        (uint8_t *)m_vaddr + ROC_SE_OFF_CTRL_LEN + RTE_ALIGN_CEIL(iv_len, 8);
...
cpt_kasumi_dec_prep(uint64_t d_offs, uint64_t d_lens,
  ...
  *offset_vaddr = rte_cpu_to_be_64((uint64_t)encr_offset << 16);
  ...
  /* IV */
  memcpy((uint8_t *)offset_vaddr + ROC_SE_OFF_CTRL_LEN, params->iv_buf, iv_len);
...
fill_sess_aead(struct rte_crypto_sym_xform *xform, struct cnxk_se_sess *sess) {
  ...
  case RTE_CRYPTO_AEAD_AES_CCM:
    plt_dp_err("Crypto: Unsupported cipher algo %u", aead_form->algo);
    return -1;
  case RTE_CRYPTO_AEAD_CHACHA20_POLY1305:
    ...
    break;
  default:
    plt_dp_err("Crypto: Undefined cipher algo %u specified", aead_form->algo);
    return -1;
  }
  if (aead_form->key.length < cipher_key_len) {
    plt_dp_err("Invalid cipher params keylen %u", aead_form->key.length);
    return -1;
  }
  sess->zsk_flag = 0;
  ...
  if (unlikely(roc_se_ciph_key_set(&sess->roc_se_ctx, enc_type,
                                   aead_form->key.data, aead_form->key.length,
                                   NULL)))
    return -1;
...
fill_sess_auth(struct rte_crypto_sym_xform *xform, struct cnxk_se_sess *sess) {
  ...
  case RTE_CRYPTO_AUTH_AES_CBC_MAC:
    ...
    return -1;
  default:
    plt_dp_err("Crypto: Undefined Hash algo %u specified", a_form->algo);
    return -1;
  }
  ...
  if ((auth_type >= ROC_SE_ZUC_EIA3) && (auth_type <= ROC_SE_AES_CMAC_EIA2))
    roc_se_ctx_swap(&sess->roc_se_ctx);
...
fill_sess_gmac(struct rte_crypto_sym_xform *xform, struct cnxk_se_sess *sess) {
  ...
  case RTE_CRYPTO_AUTH_AES_GMAC:
    ...
    break;
  default:
    plt_dp_err("Crypto: Undefined cipher algo %u specified", a_form->algo);
    return -1;
  }
  ...
  if (unlikely(roc_se_ciph_key_set(&sess->roc_se_ctx, enc_type,
                                   a_form->key.data, a_form->key.length, NULL)))
    return -1;
...
static __rte_always_inline uint32_t prepare_iov_from_pkt(
    struct rte_mbuf *pkt, struct roc_se_iov_ptr *iovec, uint32_t start_offset) {
  uint16_t index = 0;
  void *seg_data = NULL;
  int32_t seg_size = 0;
...
static __rte_always_inline uint32_t prepare_iov_from_pkt_inplace(
    struct rte_mbuf *pkt, struct roc_se_fc_params *param, uint32_t *flags) {
  uint16_t index = 0;
  void *seg_data = NULL;
  uint32_t seg_size = 0;
...
static __rte_always_inline int fill_fc_params(struct rte_crypto_op *cop,
  ...
  if (sess->aes_gcm || sess->chacha_poly) {
    ...
    if (likely(sess->mac_len)) {
      struct rte_mbuf *m = (cpt_op & ROC_SE_OP_ENCODE) ? m_dst : m_src;
  ...
  if (cpt_op & ROC_SE_OP_ENCODE)
    ...
    ret = cpt_fc_dec_hmac_prep(flags, d_offs, d_lens, &fc_params, inst);
...
fill_digest_params(struct rte_crypto_op *cop, struct cnxk_se_sess *sess,
  ...
  mdata = alloc_op_meta(&params.meta_buf, m_info->mlen, m_info->pool, infl_req);
  if (mdata == NULL) {
    ret = -ENOMEM;
    goto err_exit;
  }
