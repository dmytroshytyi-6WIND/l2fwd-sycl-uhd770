...
static uint64_t ipsec_cpt_inst_w7_get(struct roc_cpt *roc_cpt, void *sa) {
...
cn10k_ipsec_inb_sa_create(struct roc_cpt *roc_cpt, struct roc_cpt_lf *lf,
  ...
  /* Translate security parameters to SA */
  ret = cnxk_ot_ipsec_inb_sa_fill(sa_dptr, ipsec_xfrm, crypto_xfrm, false);
  if (ret) {
  ...
  /* Write session using microcode opcode */
  ret =
      roc_cpt_ctx_write(lf, sa_dptr, in_sa, sizeof(struct roc_ot_ipsec_inb_sa));
  if (ret) {
...
static int cn10k_sec_session_create(void *device,
  ...
  ret = cn10k_ipsec_session_create(device, &conf->ipsec, conf->crypto_xform,
                                   sess);
  if (ret)
    goto mempool_put;
...
static int cn10k_sec_session_destroy(void *dev,
                                     struct rte_security_session *sec_sess) {
  struct rte_cryptodev *crypto_dev = dev;
...
static unsigned int cn10k_sec_session_get_size(void *device __rte_unused) {
  return sizeof(struct cn10k_sec_session);
}

static int cn10k_sec_session_stats_get(void *device,
                                       struct rte_security_session *sess,
...
static int cn10k_sec_session_update(void *device,
                                    struct rte_security_session *sess,
                                    struct rte_security_session_conf *conf) {
  struct rte_cryptodev *crypto_dev = device;
  struct cn10k_sec_session *priv;
