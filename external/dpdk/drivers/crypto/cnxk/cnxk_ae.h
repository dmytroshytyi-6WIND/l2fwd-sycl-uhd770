...
static __rte_always_inline void cnxk_ae_modex_param_normalize(uint8_t **data,
                                                              size_t *len) {
  size_t i;
...
static inline void cnxk_ae_free_session_parameters(struct cnxk_ae_sess *sess) {
...
cnxk_ae_rsa_prep(struct rte_crypto_op *op, struct roc_ae_buf_ptr *meta_buf,
  ...
  if (rsa_op.pad == RTE_CRYPTO_RSA_PADDING_NONE) {
    ...
    if (rsa_op.op_type == RTE_CRYPTO_ASYM_OP_ENCRYPT) {
      ...
      w4.s.param2 = ROC_AE_CPT_BLOCK_TYPE2 | ((uint16_t)(exp_len) << 1);
...
static __rte_always_inline int __rte_hot cnxk_ae_enqueue_rsa_op(
  ...
  case RTE_CRYPTO_ASYM_OP_VERIFY:
    cnxk_ae_rsa_prep(op, meta_buf, &sess->rsa_ctx, &rsa->sign, inst);
    break;
  case RTE_CRYPTO_ASYM_OP_ENCRYPT:
    cnxk_ae_rsa_prep(op, meta_buf, &sess->rsa_ctx, &rsa->message, inst);
    break;
  case RTE_CRYPTO_ASYM_OP_SIGN:
    cnxk_ae_rsa_crt_prep(op, meta_buf, &sess->rsa_ctx, &rsa->message, inst);
    break;
  case RTE_CRYPTO_ASYM_OP_DECRYPT:
    cnxk_ae_rsa_crt_prep(op, meta_buf, &sess->rsa_ctx, &rsa->cipher, inst);
    break;
  default:
    op->status = RTE_CRYPTO_OP_STATUS_INVALID_ARGS;
    return -EINVAL;
...
static __rte_always_inline void cnxk_ae_ecdsa_verify_prep(
    struct rte_crypto_ecdsa_op_param *ecdsa, struct roc_ae_buf_ptr *meta_buf,
    uint64_t fpm_table_iova, struct roc_ae_ec_group *ec_grp, uint8_t curveid,
    struct cpt_inst_s *inst) {
...
static __rte_always_inline int __rte_hot cnxk_ae_enqueue_ecdsa_op(
    struct rte_crypto_op *op, struct roc_ae_buf_ptr *meta_buf,
  ...
  if (ecdsa->op_type == RTE_CRYPTO_ASYM_OP_SIGN)
    cnxk_ae_ecdsa_sign_prep(ecdsa, meta_buf, fpm_iova[curveid], ec_grp[curveid],
                            curveid, inst);
...
static __rte_always_inline int cnxk_ae_ecpm_prep(
    struct rte_crypto_ecpm_op_param *ecpm, struct roc_ae_buf_ptr *meta_buf,
...
cnxk_ae_dequeue_rsa_op(struct rte_crypto_op *cop, uint8_t *rptr,
  ...
  case RTE_CRYPTO_ASYM_OP_DECRYPT:
    if (rsa->pad == RTE_CRYPTO_RSA_PADDING_NONE) {
      ...
      rsa->message.length = rte_cpu_to_be_16(*((uint16_t *)rptr));
      /*
       * Offset output data pointer by length field
       * (2 bytes) and copy decrypted data.
       */
      memcpy(rsa->message.data, rptr + 2, rsa->message.length);
    }
    break;
  case RTE_CRYPTO_ASYM_OP_SIGN:
...
cnxk_ae_enqueue(struct cnxk_cpt_qp *qp, struct rte_crypto_op *op,
  ...
  case RTE_CRYPTO_ASYM_XFORM_RSA:
    ...
    break;
  case RTE_CRYPTO_ASYM_XFORM_ECDSA:
    ret = cnxk_ae_enqueue_ecdsa_op(op, &meta_buf, sess, sess->cnxk_fpm_iova,
                                   sess->ec_grp, inst);
    if (unlikely(ret))
      goto req_fail;
    break;
...
static __rte_always_inline void cnxk_ae_post_process(struct rte_crypto_op *cop,
                                                     struct cnxk_ae_sess *sess,
                                                     uint8_t *rptr) {
  ...
  case RTE_CRYPTO_ASYM_XFORM_MODEX:
    ...
    break;
  case RTE_CRYPTO_ASYM_XFORM_ECDSA:
    cnxk_ae_dequeue_ecdsa_op(&op->ecdsa, rptr, &sess->ec_ctx, sess->ec_grp);
    break;
  case RTE_CRYPTO_ASYM_XFORM_ECPM:
    cnxk_ae_dequeue_ecpm_op(&op->ecpm, rptr, &sess->ec_ctx, sess->ec_grp);
    break;
  default:
    cop->status = RTE_CRYPTO_OP_STATUS_INVALID_ARGS;
    break;
