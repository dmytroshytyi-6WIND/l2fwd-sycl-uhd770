...
static __rte_always_inline int __rte_hot cn9k_cpt_sym_inst_fill(
    struct cnxk_cpt_qp *qp, struct rte_crypto_op *op, struct cnxk_se_sess *sess,
...
static __rte_always_inline int __rte_hot cn9k_cpt_sec_inst_fill(
    struct rte_crypto_op *op, struct cpt_inflight_req *infl_req,
...
cn9k_cpt_sym_temp_sess_create(struct cnxk_cpt_qp *qp,
  ...
  ret = sym_session_configure(qp->lf.roc_cpt, driver_id, sym_op->xform, sess,
                              qp->sess_mp_priv);
  if (ret)
    goto sess_put;
...
static inline int cn9k_cpt_inst_prep(struct cnxk_cpt_qp *qp,
                                     struct rte_crypto_op *op,
  ...
  if (op->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC) {
    ...
    if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
      ...
      ret = cn9k_cpt_sym_inst_fill(qp, op, sess, infl_req, inst);
      ...
      ret = cn9k_cpt_sym_inst_fill(qp, op, sess, infl_req, inst);
      if (unlikely(ret)) {
        sym_session_clear(cn9k_cryptodev_driver_id, op->sym->session);
    ...
    if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
      asym_op = op->asym;
      sess = (struct cnxk_ae_sess *)asym_op->session->sess_private_data;
...
static inline void cn9k_cpt_inst_submit(struct cpt_inst_s *inst,
                                        uint64_t lmtline, uint64_t io_addr) {
  uint64_t lmt_status;
...
static uint16_t cn9k_cpt_enqueue_burst(void *qptr, struct rte_crypto_op **ops,
                                       uint16_t nb_ops) {
  ...
  while (count < nb_ops) {
    ...
    __atomic_store_n(&infl_req_1->res.u64[0], res.u64[0], __ATOMIC_RELAXED);
    inst[0].res_addr = (uint64_t)&infl_req_1->res;

    __atomic_store_n(&infl_req_2->res.u64[0], res.u64[0], __ATOMIC_RELAXED);
...
uint16_t cn9k_cpt_crypto_adapter_enqueue(uintptr_t tag_op,
  ...
  inst.w2.u64 =
      CNXK_CPT_INST_W2((RTE_EVENT_TYPE_CRYPTODEV << 28) | rsp_info->flow_id,
...
cn9k_cpt_dequeue_post_process(struct cnxk_cpt_qp *qp, struct rte_crypto_op *cop,
  ...
  if (likely(res->compcode == CPT_COMP_GOOD)) {
    ...
    if (cop->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC) {
      ...
      sess = (struct cnxk_ae_sess *)op->session->sess_private_data;
...
temp_sess_free:
  if (unlikely(cop->sess_type == RTE_CRYPTO_OP_SESSIONLESS)) {
    if (cop->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC) {
      sym_session_clear(cn9k_cryptodev_driver_id, cop->sym->session);
      sz =
...
static uint16_t cn9k_cpt_dequeue_burst(void *qptr, struct rte_crypto_op **ops,
                                       uint16_t nb_ops) {
  struct cpt_inflight_req *infl_req;
  struct cnxk_cpt_qp *qp = qptr;
  struct pending_queue *pend_q;
...
static void cn9k_cpt_dev_info_get(struct rte_cryptodev *dev,
