...
cn10k_cpt_sym_temp_sess_create(struct cnxk_cpt_qp *qp,
...
static __rte_always_inline int __rte_hot cpt_sym_inst_fill(
...
static inline int cn10k_cpt_fill_inst(struct cnxk_cpt_qp *qp,
  ...
  if (op->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC) {
    if (op->sess_type == RTE_CRYPTO_OP_SECURITY_SESSION) {
      sec_sess = get_sec_session_private_data(sym_op->sec_session);
      ...
      ret = cpt_sym_inst_fill(qp, op, sess, infl_req, &inst[0]);
      if (unlikely(ret))
        return 0;
      w7 = sess->cpt_inst_w7;
    } else {
      ...
      ret = cpt_sym_inst_fill(qp, op, sess, infl_req, &inst[0]);
      if (unlikely(ret)) {
        sym_session_clear(cn10k_cryptodev_driver_id, op->sym->session);
    ...
    if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
      asym_op = op->asym;
      ae_sess = (struct cnxk_ae_sess *)asym_op->session->sess_private_data;
...
static uint16_t cn10k_cpt_enqueue_burst(void *qptr, struct rte_crypto_op **ops,
                                        uint16_t nb_ops) {
...
again:
  ...
  if (i > PKTS_PER_STEORL) {
    lmt_arg =
        ROC_CN10K_CPT_LMT_ARG | (PKTS_PER_STEORL - 1) << 12 | (uint64_t)lmt_id;
...
static inline void cn10k_cpt_sec_post_process(struct rte_crypto_op *cop,
...
static inline void cn10k_cpt_dequeue_post_process(
    struct cnxk_cpt_qp *qp, struct rte_crypto_op *cop,
...
temp_sess_free:
  if (unlikely(cop->sess_type == RTE_CRYPTO_OP_SESSIONLESS)) {
    if (cop->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC) {
      sym_session_clear(cn10k_cryptodev_driver_id, cop->sym->session);
      sz =
...
static uint16_t cn10k_cpt_dequeue_burst(void *qptr, struct rte_crypto_op **ops,
                                        uint16_t nb_ops) {
  struct cpt_inflight_req *infl_req;
  struct cnxk_cpt_qp *qp = qptr;
  struct pending_queue *pend_q;
...
static void cn10k_cpt_dev_info_get(struct rte_cryptodev *dev,
