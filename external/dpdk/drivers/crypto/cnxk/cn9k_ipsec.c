...
static inline int cn9k_cpt_enq_sa_write(struct cn9k_ipsec_sa *sa,
                                        struct cnxk_cpt_qp *qp, uint8_t opcode,
                                        size_t ctx_len) {
...
static inline int ipsec_sa_ctl_set(struct rte_security_ipsec_xform *ipsec,
...
static int cn9k_ipsec_outb_sa_create(struct cnxk_cpt_qp *qp,
  ...
  if (ipsec->mode == RTE_SECURITY_IPSEC_SA_MODE_TUNNEL) {
    if (ipsec->tunnel.type == RTE_SECURITY_IPSEC_TUNNEL_IPV4) {
      ...
      ip6->vtc_flow = rte_cpu_to_be_32(
          0x60000000 |
          ((ipsec->tunnel.ipv6.dscp << RTE_IPV6_HDR_TC_SHIFT) &
           RTE_IPV6_HDR_TC_MASK) |
          ((ipsec->tunnel.ipv6.flabel << RTE_IPV6_HDR_FL_SHIFT) &
  ...
  sa->custom_hdr_len = sizeof(struct roc_ie_on_outb_hdr) - ROC_IE_ON_MAX_IV_LEN;
  ...
  return cn9k_cpt_enq_sa_write(sa, qp, ROC_IE_ON_MAJOR_OP_WRITE_IPSEC_OUTBOUND,
                               ctx_len);
}

static int cn9k_ipsec_inb_sa_create(struct cnxk_cpt_qp *qp,
                                    struct rte_security_ipsec_xform *ipsec,
                                    struct rte_crypto_sym_xform *crypto_xform,
                                    struct rte_security_session *sec_sess) {
  ...
  if (sa->replay_win_sz) {
    if (sa->replay_win_sz > CNXK_ON_AR_WIN_SIZE_MAX) {
      plt_err("Replay window size:%u is not supported", sa->replay_win_sz);
      return -ENOTSUP;
    }
  ...
  return cn9k_cpt_enq_sa_write(sa, qp, ROC_IE_ON_MAJOR_OP_WRITE_IPSEC_INBOUND,
                               ctx_len);
}
...
cn9k_ipsec_xform_verify(struct rte_security_ipsec_xform *ipsec,
  ...
  if (ipsec->life.bytes_hard_limit != 0 || ipsec->life.bytes_soft_limit != 0 ||
...
cn9k_ipsec_session_create(void *dev,
  ...
  if (ipsec_xform->direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS)
    return cn9k_ipsec_inb_sa_create(qp, ipsec_xform, crypto_xform, sess);
  else
    return cn9k_ipsec_outb_sa_create(qp, ipsec_xform, crypto_xform, sess);
}

static int cn9k_sec_session_create(void *device,
  ...
  ret =
      cn9k_ipsec_session_create(device, &conf->ipsec, conf->crypto_xform, sess);
  if (ret)
    goto mempool_put;
...
static int cn9k_sec_session_destroy(void *device __rte_unused,
                                    struct rte_security_session *sess) {
  struct roc_ie_on_outb_sa *out_sa;
  struct cn9k_sec_session *priv;
...
static unsigned int cn9k_sec_session_get_size(void *device __rte_unused) {
  return sizeof(struct cn9k_sec_session);
}

static int cn9k_sec_session_update(void *device,
                                   struct rte_security_session *sec_sess,
                                   struct rte_security_session_conf *conf) {
