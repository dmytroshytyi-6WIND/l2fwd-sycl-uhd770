...
static int parse_max_qps_limit(const char *key, const char *value,
                               void *extra_args) {
  RTE_SET_USED(key);
  uint32_t val;
...
int cnxk_cpt_parse_devargs(struct rte_devargs *devargs,
  ...
  rc = rte_kvargs_process(kvlist, CNXK_MAX_QPS_LIMIT, &parse_max_qps_limit,
                          &max_qps_limit);
  if (rc < 0) {
    plt_err("max_qps_limit should in the range <%d-%d>", CNXK_MAX_QPS_LIMIT_MIN,
            CNXK_MAX_QPS_LIMIT_MAX);
