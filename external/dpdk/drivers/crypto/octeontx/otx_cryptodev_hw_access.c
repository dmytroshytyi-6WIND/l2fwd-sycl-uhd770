...
static int otx_cpt_vf_init(struct cpt_vf *cptvf) {
  int ret = 0;
  ...
  if (ret) {
    CPT_LOG_ERR("%s: PF not responding to READY msg", cptvf->dev_name);
    ret = -EBUSY;
    goto exit;
  }
 ...
 */
static uint64_t otx_cpt_read_vf_misc_intr_status(struct cpt_vf *cptvf) {
 ...
 * @param   cptvf	cptvf structure
 */
static void otx_cpt_clear_mbox_intr(struct cpt_vf *cptvf) {
  cptx_vqx_misc_int_t vqx_misc_int;

  vqx_misc_int.u =
      CPT_READ_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_MISC_INT(0, 0));
  /* W1C for the VF */
  vqx_misc_int.s.mbox = 1;
  CPT_WRITE_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_MISC_INT(0, 0),
                vqx_misc_int.u);
}
 ...
 * @param   cptvf	cptvf structure
 */
static void otx_cpt_clear_irde_intr(struct cpt_vf *cptvf) {
  cptx_vqx_misc_int_t vqx_misc_int;
 ...
 * @param   cptvf	cptvf structure
 */
static void otx_cpt_clear_nwrp_intr(struct cpt_vf *cptvf) {
  cptx_vqx_misc_int_t vqx_misc_int;
 ...
 * @param   cptvf	cptvf structure
 */
static void otx_cpt_clear_swerr_intr(struct cpt_vf *cptvf) {
  cptx_vqx_misc_int_t vqx_misc_int;
 ...
 * @param   cptvf	cptvf structure
 */
static void otx_cpt_clear_hwerr_intr(struct cpt_vf *cptvf) {
  cptx_vqx_misc_int_t vqx_misc_int;
 ...
 * @param   cptvf	cptvf structure
 */
static void otx_cpt_clear_fault_intr(struct cpt_vf *cptvf) {
  cptx_vqx_misc_int_t vqx_misc_int;
 ...
 * @param   cptvf	cptvf structure
 */
static void otx_cpt_clear_dovf_intr(struct cpt_vf *cptvf) {
  cptx_vqx_misc_int_t vqx_misc_int;
 ...
 */
static void otx_cpt_write_vq_ctl(struct cpt_vf *cptvf, bool val) {
  cptx_vqx_ctl_t vqx_ctl;

  vqx_ctl.u = CPT_READ_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_CTL(0, 0));
  vqx_ctl.s.ena = val;
  CPT_WRITE_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_CTL(0, 0), vqx_ctl.u);
}

/* Write to VQX_INPROG register
 */
static void otx_cpt_write_vq_inprog(struct cpt_vf *cptvf, uint8_t val) {
  cptx_vqx_inprog_t vqx_inprg;

  vqx_inprg.u = CPT_READ_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_INPROG(0, 0));
  vqx_inprg.s.inflight = val;
  CPT_WRITE_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_INPROG(0, 0), vqx_inprg.u);
}

/* Write to VQX_DONE_WAIT NUMWAIT register
 */
static void otx_cpt_write_vq_done_numwait(struct cpt_vf *cptvf, uint32_t val) {
  cptx_vqx_done_wait_t vqx_dwait;

  vqx_dwait.u = CPT_READ_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_DONE_WAIT(0, 0));
  vqx_dwait.s.num_wait = val;
  CPT_WRITE_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_DONE_WAIT(0, 0), vqx_dwait.u);
}

/* Write to VQX_DONE_WAIT NUM_WAIT register
 */
static void otx_cpt_write_vq_done_timewait(struct cpt_vf *cptvf, uint16_t val) {
  cptx_vqx_done_wait_t vqx_dwait;
 ...
 */
static void otx_cpt_write_vq_saddr(struct cpt_vf *cptvf, uint64_t val) {
  cptx_vqx_saddr_t vqx_saddr;

  vqx_saddr.u = val;
  CPT_WRITE_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_SADDR(0, 0), vqx_saddr.u);
}

static void otx_cpt_vfvq_init(struct cpt_vf *cptvf) {
...
static int cpt_vq_init(struct cpt_vf *cptvf, uint8_t group) {
  ...
  if (err) {
    CPT_LOG_ERR("%s: PF not responding to QLEN msg", cptvf->dev_name);
    err = -EBUSY;
    goto cleanup;
  }
  ...
  if (err) {
    CPT_LOG_ERR("%s: PF not responding to VF_GRP msg", cptvf->dev_name);
    err = -EBUSY;
    goto cleanup;
  }
...
void otx_cpt_poll_misc(struct cpt_vf *cptvf) {
  uint64_t intr;
...
int otx_cpt_hw_init(struct cpt_vf *cptvf, void *pdev, void *reg_base,
                    char *name) {
...
int otx_cpt_deinit_device(void *dev) {
...
static int otx_cpt_metabuf_mempool_create(const struct rte_cryptodev *dev,
  ...
  mb_pool_sz =
      RTE_MAX(nb_elements, (METABUF_POOL_CACHE_SIZE * rte_lcore_count()));

  pool =
      rte_mempool_create_empty(mempool_name, mb_pool_sz, max_mlen,
                               METABUF_POOL_CACHE_SIZE, 0, rte_socket_id(), 0);
  ...
  ret = rte_mempool_set_ops_byname(pool, RTE_MBUF_DEFAULT_MEMPOOL_OPS, NULL);
  if (ret) {
...
static void otx_cpt_metabuf_mempool_destroy(struct cpt_instance *instance) {
...
int otx_cpt_get_resource(const struct rte_cryptodev *dev, uint8_t group,
  ...
  rz = rte_memzone_reserve_aligned(
      cptvf->dev_name, len, cptvf->node,
      RTE_MEMZONE_SIZE_HINT_ONLY | RTE_MEMZONE_256MB, RTE_CACHE_LINE_SIZE);
  if (!rz) {
    ret = rte_errno;
    goto exit;
  }
  ...
  *next_ptr = (uint64_t)chunk_head->dma_addr;
  ...
  if (cpt_vq_init(cptvf, group)) {
    CPT_LOG_ERR("Failed to initialize CPT VQ of device %s", cptvf->dev_name);
    ret = -EBUSY;
...
int otx_cpt_put_resource(struct cpt_instance *instance) {
...
int otx_cpt_start_device(void *dev) {
  int rc;
  struct cpt_vf *cptvf = (struct cpt_vf *)dev;

  rc = otx_cpt_send_vf_up(cptvf);
  if (rc) {
    CPT_LOG_ERR("Failed to mark CPT VF device %s UP, rc = %d", cptvf->dev_name,
                rc);
    return -EFAULT;
  }
...
void otx_cpt_stop_device(void *dev) {
  int rc;
  ...
  if (!retries && pending) {
    CPT_LOG_ERR("%s: Timeout waiting for commands(%u)", cptvf->dev_name,
                pending);
    return;
  }
  ...
  if (rc) {
    CPT_LOG_ERR("Failed to bring down vf %s, rc %d", cptvf->dev_name, rc);
    return;
  }
}
