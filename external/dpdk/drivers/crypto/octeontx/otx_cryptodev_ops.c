...
static int otx_cpt_que_pair_release(struct rte_cryptodev *dev,
                                    uint16_t que_pair_id);

/* Alarm routines */

static void otx_cpt_alarm_cb(void *arg) {
  struct cpt_vf *cptvf = arg;
  otx_cpt_poll_misc(cptvf);
  rte_eal_alarm_set(CPT_INTR_POLL_INTERVAL_MS * 1000, otx_cpt_alarm_cb, cptvf);
}

static int otx_cpt_periodic_alarm_start(void *arg) {
  return rte_eal_alarm_set(CPT_INTR_POLL_INTERVAL_MS * 1000, otx_cpt_alarm_cb,
                           arg);
}

static int otx_cpt_periodic_alarm_stop(void *arg) {
...
static int otx_cpt_dev_start(struct rte_cryptodev *c_dev) {
...
static void otx_cpt_dev_stop(struct rte_cryptodev *c_dev) {
  void *cptvf = c_dev->data->dev_private;
...
static int otx_cpt_dev_close(struct rte_cryptodev *c_dev) {
  void *cptvf = c_dev->data->dev_private;
  int i, ret;
...
static void otx_cpt_dev_info_get(struct rte_cryptodev *dev,
...
static int otx_cpt_que_pair_setup(struct rte_cryptodev *dev,
                                  uint16_t que_pair_id,
...
static int sym_xform_verify(struct rte_crypto_sym_xform *xform) {
  if (xform->next) {
...
static int sym_session_configure(int driver_id,
  ...
  memset(priv, 0,
         sizeof(struct cpt_sess_misc) + offsetof(struct cpt_ctx, mc_ctx));
  ...
  misc->ctx_dma_addr =
      rte_mempool_virt2iova(misc) + sizeof(struct cpt_sess_misc);

  vq_cmd_w3.u64 = 0;
  vq_cmd_w3.s.grp = 0;
  vq_cmd_w3.s.cptr = misc->ctx_dma_addr + offsetof(struct cpt_ctx, mc_ctx);
...
static void sym_session_clear(int driver_id,
                              struct rte_cryptodev_sym_session *sess) {
...
static int otx_cpt_session_cfg(struct rte_cryptodev *dev,
                               struct rte_crypto_sym_xform *xform,
                               struct rte_cryptodev_sym_session *sess,
                               struct rte_mempool *pool) {
  CPT_PMD_INIT_FUNC_TRACE();
...
static void otx_cpt_session_clear(struct rte_cryptodev *dev,
                                  struct rte_cryptodev_sym_session *sess) {
  CPT_PMD_INIT_FUNC_TRACE();
...
otx_cpt_asym_session_cfg(struct rte_cryptodev *dev __rte_unused,
  ...
  struct cpt_asym_sess_misc *priv =
      (struct cpt_asym_sess_misc *)sess->sess_private_data;
  int ret;
...
static __rte_always_inline void *__rte_hot otx_cpt_request_enqueue(
    struct cpt_instance *instance, void *req, uint64_t cpt_inst_w7) {
...
static __rte_always_inline void *__rte_hot otx_cpt_enq_single_asym(
    struct cpt_instance *instance, struct rte_crypto_op *op) {
  ...
  sess = (struct cpt_asym_sess_misc *)asym_op->session->sess_private_data;
  ...
  params.meta_buf += (4 * sizeof(uintptr_t)) + sizeof(struct cpt_request_info);
  ...
  case RTE_CRYPTO_ASYM_XFORM_ECPM:
    ret = cpt_ecpm_prep(&asym_op->ecpm, &params, sess->ec_ctx.curveid);
    if (unlikely(ret))
      goto req_fail;
    break;
...
static __rte_always_inline void *__rte_hot otx_cpt_enq_single_sym(
    struct cpt_instance *instance, struct rte_crypto_op *op) {
  ...
  sess = (struct cpt_sess_misc *)get_sym_session_private_data(
      sym_op->session, otx_cryptodev_driver_id);
  ...
  if (likely(cpt_op & CPT_OP_CIPHER_MASK))
    ...
    ret = fill_digest_params(op, sess, &instance->meta_info, &mdata,
                             (void **)&prep_req);
...
static __rte_always_inline void *__rte_hot otx_cpt_enq_single_sym_sessless(
    struct cpt_instance *instance, struct rte_crypto_op *op) {
...
otx_cpt_enq_single(struct cpt_instance *inst, struct rte_crypto_op *op,
...
static uint16_t otx_cpt_enqueue_asym(void *qptr, struct rte_crypto_op **ops,
                                     uint16_t nb_ops) {
  return otx_cpt_pkt_enqueue(qptr, ops, nb_ops, OP_TYPE_ASYM);
}

static uint16_t otx_cpt_enqueue_sym(void *qptr, struct rte_crypto_op **ops,
                                    uint16_t nb_ops) {
...
get_event_crypto_mdata(struct rte_crypto_op *op) {
  ...
  if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION)
    ec_mdata = rte_cryptodev_sym_session_get_user_data(op->sym->session);
...
uint16_t __rte_hot otx_crypto_adapter_enqueue(void *port,
  ...
  op_type =
      op->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC ? OP_TYPE_SYM : OP_TYPE_ASYM;
...
static inline void otx_cpt_asym_rsa_op(struct rte_crypto_op *cop,
                                       struct cpt_request_info *req,
...

{
  ...
  case RTE_CRYPTO_ASYM_OP_DECRYPT:
    if (rsa->pad == RTE_CRYPTO_RSA_PADDING_NONE)
      ...
      rsa->message.length = rte_cpu_to_be_16(*((uint16_t *)req->rptr));
  ...
  case RTE_CRYPTO_ASYM_OP_VERIFY:
    if (rsa->pad == RTE_CRYPTO_RSA_PADDING_NONE)
      ...
      /* Get length of decrypted output */
      rsa->sign.length = rte_cpu_to_be_16(*((uint16_t *)req->rptr));
    ...
    if (memcmp(rsa->sign.data, rsa->message.data, rsa->message.length)) {
...

{
  ...
  memcpy(ecdsa->s.data, req->rptr + RTE_ALIGN_CEIL(prime_len, 8), prime_len);
...
otx_cpt_asym_dequeue_ecpm_op(struct rte_crypto_ecpm_op_param *ecpm,
  ...
  memcpy(ecpm->r.y.data, req->rptr + RTE_ALIGN_CEIL(prime_len, 8), prime_len);
  ecpm->r.x.length = prime_len;
  ecpm->r.y.length = prime_len;
}

static __rte_always_inline void __rte_hot otx_cpt_asym_post_process(
    struct rte_crypto_op *cop, struct cpt_request_info *req) {
  ...
  case RTE_CRYPTO_ASYM_XFORM_MODEX:
    ...
    memcpy(op->modex.result.data, req->rptr, op->modex.result.length);
    break;
  case RTE_CRYPTO_ASYM_XFORM_ECDSA:
...
static __rte_always_inline void __rte_hot otx_cpt_dequeue_post_process(
    struct rte_crypto_op *cop, uintptr_t *rsp, const uint8_t op_type) {
  /* H/w has returned success */
  cop->status = RTE_CRYPTO_OP_STATUS_SUCCESS;

  /* Perform further post processing */

  if ((op_type == OP_TYPE_SYM) && (cop->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC)) {
...
static inline void free_sym_session_data(const struct cpt_instance *instance,
                                         struct rte_crypto_op *cop) {
  void *sess_private_data_t =
      get_sym_session_private_data(cop->sym->session, otx_cryptodev_driver_id);
  memset(sess_private_data_t, 0, cpt_get_session_size());
  memset(cop->sym->session, 0,
         rte_cryptodev_sym_get_existing_header_session_size(cop->sym->session));
...
otx_cpt_pkt_dequeue(void *qptr, struct rte_crypto_op **ops, uint16_t nb_ops,
  ...
  for (i = 0; i < count; i++) {
    pending_queue_peek(pqueue, (void **)&user_req, DEFAULT_CMD_QLEN,
                       i + 1 < count);
    ...
    CPT_LOG_DP_DEBUG("Request %p Op %p completed with code %d", user_req,
                     user_req->op, ret);
  ...
  for (i = 0; i < nb_completed; i++) {
    ...
    ops[i] = otx_cpt_process_response(instance, (void *)ops[i], cc[i], op_type);
  }

  return nb_completed;
}

static uint16_t otx_cpt_dequeue_asym(void *qptr, struct rte_crypto_op **ops,
                                     uint16_t nb_ops) {
  return otx_cpt_pkt_dequeue(qptr, ops, nb_ops, OP_TYPE_ASYM);
}

static uint16_t otx_cpt_dequeue_sym(void *qptr, struct rte_crypto_op **ops,
                                    uint16_t nb_ops) {
  return otx_cpt_pkt_dequeue(qptr, ops, nb_ops, OP_TYPE_SYM);
}

uintptr_t __rte_hot otx_crypto_adapter_dequeue(uintptr_t get_work1) {
  ...
  cop = (void *)rsp[1];
  op_type =
      cop->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC ? OP_TYPE_SYM : OP_TYPE_ASYM;

  do {
    cc = check_nb_command_id(req, (struct cpt_instance *)(uintptr_t)instance);
...
int otx_cpt_dev_create(struct rte_cryptodev *c_dev) {
  ...
  cptvf = rte_zmalloc_socket("otx_cryptodev_private_mem", sizeof(struct cpt_vf),
  ...
  snprintf(dev_name, 32, "%02x:%02x.%x", pdev->addr.bus, pdev->addr.devid,
           pdev->addr.function);
