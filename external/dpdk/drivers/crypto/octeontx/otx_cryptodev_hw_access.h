...
#define DEFAULT_CMD_QCHUNK_SIZE                                                \
  ((DEFAULT_CMD_QLEN - DEFAULT_CMD_QRSVD_SLOTS) / DEFAULT_CMD_QCHUNKS)
...
#define CPTX_VQX_CTL(a, b)                                                     \
  (0x0000100ll + 0x1000000000ll * ((a)&0x0) + 0x100000ll * (b))
#define CPTX_VQX_SADDR(a, b)                                                   \
  (0x0000200ll + 0x1000000000ll * ((a)&0x0) + 0x100000ll * (b))
#define CPTX_VQX_DONE_WAIT(a, b)                                               \
  (0x0000400ll + 0x1000000000ll * ((a)&0x0) + 0x100000ll * (b))
#define CPTX_VQX_INPROG(a, b)                                                  \
  (0x0000410ll + 0x1000000000ll * ((a)&0x0) + 0x100000ll * (b))
#define CPTX_VQX_DONE(a, b)                                                    \
  (0x0000420ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_DONE_ACK(a, b)                                                \
  (0x0000440ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_DONE_INT_W1S(a, b)                                            \
  (0x0000460ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_DONE_INT_W1C(a, b)                                            \
  (0x0000468ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_DONE_ENA_W1S(a, b)                                            \
  (0x0000470ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_DONE_ENA_W1C(a, b)                                            \
  (0x0000478ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_MISC_INT(a, b)                                                \
  (0x0000500ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_MISC_INT_W1S(a, b)                                            \
  (0x0000508ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_MISC_ENA_W1S(a, b)                                            \
  (0x0000510ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_MISC_ENA_W1C(a, b)                                            \
  (0x0000518ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VQX_DOORBELL(a, b)                                                \
  (0x0000600ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b))
#define CPTX_VFX_PF_MBOXX(a, b, c)                                             \
  (0x0001000ll + 0x1000000000ll * ((a)&0x1) + 0x100000ll * (b) +               \
   8ll * ((c)&0x1))
...
int otx_cpt_start_device(void *cptvf);

void otx_cpt_stop_device(void *cptvf);

/* Write to VQX_DOORBELL register
 */
static __rte_always_inline void otx_cpt_write_vq_doorbell(struct cpt_vf *cptvf,
                                                          uint32_t val) {
  cptx_vqx_doorbell_t vqx_dbell;

  vqx_dbell.u = 0;
  vqx_dbell.s.dbell_cnt = val * 8; /* Num of Instructions * 8 words */
  CPT_WRITE_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_DOORBELL(0, 0), vqx_dbell.u);
}
...
otx_cpt_read_vq_doorbell(struct cpt_vf *cptvf) {
  ...
  vqx_dbell.u = CPT_READ_CSR(CPT_CSR_REG_BASE(cptvf), CPTX_VQX_DOORBELL(0, 0));
...
static __rte_always_inline void *get_cpt_inst(struct command_queue *cqueue) {
  CPT_LOG_DP_DEBUG("CPT queue idx %u\n", cqueue->idx);
  return &cqueue->qhead[cqueue->idx * CPT_INST_SIZE];
}

static __rte_always_inline void fill_cpt_inst(struct cpt_instance *instance,
                                              void *req, uint64_t ucmd_w3) {
  struct command_queue *cqueue;
...
static __rte_always_inline void mark_cpt_inst(struct cpt_instance *instance) {
  struct cpt_vf *cptvf = (struct cpt_vf *)instance;
...
static __rte_always_inline uint8_t check_nb_command_id(
    struct cpt_request_info *user_req, struct cpt_instance *instance) {
...
complete:
  if (likely(cptres->s8x.compcode == CPT_8X_COMP_E_GOOD)) {
    ...
    CPT_LOG_DP_DEBUG("Request %p : failed with %s : err code :%x", user_req,
                     (cptres->s8x.compcode == CPT_8X_COMP_E_FAULT)
                         ? "DMA Fault"
                         : "Software error",
                     ret);
  } else {
    CPT_LOG_DP_ERR("Request %p : unexpected completion code %d", user_req,
                   cptres->s8x.compcode);
    ret = (uint8_t)*user_req->alternate_caddr;
  }
