...
static uint8_t bcmfs5_is_next_table_desc(void *desc_ptr) {
...
static int bcmfs5_enqueue_desc(uint32_t nhpos, uint32_t nhcnt, uint32_t reqid,
                               uint64_t desc, void **desc_ptr, void *start_desc,
                               void *end_desc) {
  uint64_t d;
  ...
  if ((nhpos % HEADER_BDCOUNT_MAX == 0) && (nhcnt - nhpos)) {
    ...
    d = bcmfs5_header_desc(_startpkt, _endpkt, _bdcount, 0x0, reqid);
...
static uint64_t bcmfs5_src_desc(uint64_t addr, unsigned int len) {
...
static uint64_t bcmfs5_msrc_desc(uint64_t addr, unsigned int len_div_16) {
...
static uint64_t bcmfs5_dst_desc(uint64_t addr, unsigned int len) {
...
static uint64_t bcmfs5_mdst_desc(uint64_t addr, unsigned int len_div_16) {
...
static bool bcmfs5_sanity_check(struct bcmfs_qp_message *msg) {
  unsigned int i = 0;
...
static void *bcmfs5_enqueue_msg(struct bcmfs_queue *txq,
                                struct bcmfs_qp_message *msg, uint32_t reqid,
  ...
  for (src = 0; src < msg->srcs_count; src++) {
    if (msg->srcs_len[src] & 0xf)
      d = bcmfs5_src_desc(msg->srcs_addr[src], msg->srcs_len[src]);
    else
      d = bcmfs5_msrc_desc(msg->srcs_addr[src], msg->srcs_len[src] / 16);

    nxt_page = bcmfs5_enqueue_desc(nhpos, nhcnt, reqid, d, &desc_ptr,
                                   start_desc, end_desc);
    if (nxt_page)
      txq->descs_inflight++;
    nhpos++;
  ...
  for (dst = 0; dst < msg->dsts_count; dst++) {
    if (msg->dsts_len[dst] & 0xf)
      d = bcmfs5_dst_desc(msg->dsts_addr[dst], msg->dsts_len[dst]);
    else
      d = bcmfs5_mdst_desc(msg->dsts_addr[dst], msg->dsts_len[dst] / 16);
...
static int bcmfs5_enqueue_single_request_qp(struct bcmfs_qp *qp, void *op) {
  void *next;
  int reqid;
  int ret = 0;
  uint64_t slab = 0;
  ...
  next = bcmfs5_enqueue_msg(
      txq, msg, reqid, (uint8_t *)txq->base_addr + txq->tx_write_ptr,
      txq->base_addr, (uint8_t *)txq->base_addr + txq->queue_size);
  if (next == NULL) {
...
static uint16_t bcmfs5_dequeue_qp(struct bcmfs_qp *qp, void **ops,
                                  uint16_t budget) {
  int err;
  uint16_t reqid;
  uint64_t desc;
...
static int bcmfs5_start_qp(struct bcmfs_qp *qp) {
  uint32_t val, off;
  ...
  FS_MMIO_WRITE32(bd_low, (uint8_t *)qp->ioreg + RING_BD_START_ADDRESS_LSB);
  FS_MMIO_WRITE32(bd_high, (uint8_t *)qp->ioreg + RING_BD_START_ADDRESS_MSB);
  ...
  FS_MMIO_WRITE32(cmpl_low, (uint8_t *)qp->ioreg + RING_CMPL_START_ADDR_LSB);
  FS_MMIO_WRITE32(cmpl_high, (uint8_t *)qp->ioreg + RING_CMPL_START_ADDR_MSB);
...
static void bcmfs5_shutdown_qp(struct bcmfs_qp *qp) {
  /* Disable/deactivate ring */
  FS_MMIO_WRITE32(0x0, (uint8_t *)qp->ioreg + RING_CONTROL);
}
