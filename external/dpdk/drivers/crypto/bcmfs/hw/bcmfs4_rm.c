...
#define RING_BD_TOGGLE_VALID(offset) (!RING_BD_TOGGLE_INVALID(offset))
...
static uint8_t bcmfs4_is_next_table_desc(void *desc_ptr) {
...
static uint64_t bcmfs4_next_table_desc(uint32_t toggle, uint64_t next_addr) {
...
static uint64_t bcmfs4_header_desc(uint32_t toggle, uint32_t startpkt,
  ...
  return (rm_build_desc(HEADER_TYPE, DESC_TYPE_SHIFT, DESC_TYPE_MASK) |
          ...
          rm_build_desc(startpkt, HEADER_STARTPKT_SHIFT, HEADER_STARTPKT_MASK) |
...
static void bcmfs4_enqueue_desc(uint32_t nhpos, uint32_t nhcnt, uint32_t reqid,
                                uint64_t desc, void **desc_ptr,
                                uint32_t *toggle, void *start_desc,
                                void *end_desc) {
  uint64_t d;
  ...
  if ((nhpos % HEADER_BDCOUNT_MAX == 0) && (nhcnt - nhpos)) {
    ...
    _bdcount = (nhavail <= HEADER_BDCOUNT_MAX) ? nhavail : HEADER_BDCOUNT_MAX;
    if (nhavail <= HEADER_BDCOUNT_MAX)
      _bdcount = nhavail;
    else
      _bdcount = HEADER_BDCOUNT_MAX;
    d = bcmfs4_header_desc(_toggle, _startpkt, _endpkt, _bdcount, 0x0, reqid);
...
static uint64_t bcmfs4_src_desc(uint64_t addr, unsigned int length) {
...
static uint64_t bcmfs4_msrc_desc(uint64_t addr, unsigned int length_div_16) {
...
static uint64_t bcmfs4_dst_desc(uint64_t addr, unsigned int length) {
...
static uint64_t bcmfs4_mdst_desc(uint64_t addr, unsigned int length_div_16) {
...
static bool bcmfs4_sanity_check(struct bcmfs_qp_message *msg) {
  unsigned int i = 0;
...
static uint32_t estimate_nonheader_desc_count(struct bcmfs_qp_message *msg) {
  uint32_t cnt = 0;
  unsigned int src = 0;
  unsigned int dst = 0;
...
static void *bcmfs4_enqueue_msg(struct bcmfs_qp_message *msg, uint32_t nhcnt,
                                uint32_t reqid, void *desc_ptr, uint32_t toggle,
  ...
  while (src < msg->srcs_count || dst < msg->dsts_count) {
    if (src < msg->srcs_count) {
      if (msg->srcs_len[src] & 0xf) {
        d = bcmfs4_src_desc(msg->srcs_addr[src], msg->srcs_len[src]);
      } else {
        d = bcmfs4_msrc_desc(msg->srcs_addr[src], msg->srcs_len[src] / 16);
      }
      bcmfs4_enqueue_desc(nhpos, nhcnt, reqid, d, &desc_ptr, &toggle,
                          start_desc, end_desc);
      nhpos++;
      dst_target = msg->srcs_len[src];
      src++;
    ...
    while (dst_target && (dst < msg->dsts_count)) {
      if (msg->dsts_len[dst] & 0xf) {
        d = bcmfs4_dst_desc(msg->dsts_addr[dst], msg->dsts_len[dst]);
      } else {
        d = bcmfs4_mdst_desc(msg->dsts_addr[dst], msg->dsts_len[dst] / 16);
      }
      bcmfs4_enqueue_desc(nhpos, nhcnt, reqid, d, &desc_ptr, &toggle,
                          start_desc, end_desc);
      nhpos++;
...
static int bcmfs4_enqueue_single_request_qp(struct bcmfs_qp *qp, void *op) {
  int reqid;
  void *next;
  uint32_t nhcnt;
  int ret = 0;
  ...
  next = bcmfs4_enqueue_msg(
      msg, nhcnt, reqid, (uint8_t *)txq->base_addr + txq->tx_write_ptr,
      RING_BD_TOGGLE_VALID(txq->tx_write_ptr), txq->base_addr,
      (uint8_t *)txq->base_addr + txq->queue_size);
  if (next == NULL) {
    BCMFS_DP_LOG(ERR, "Enqueue for desc failed on queue %d", qp->qpair_id);
    ret = -EINVAL;
    exit_cleanup = true;
    goto exit;
  }
  ...
  txq->tx_write_ptr = (uint32_t)((uint8_t *)next - (uint8_t *)txq->base_addr);
...
static void bcmfs4_ring_doorbell_qp(struct bcmfs_qp *qp __rte_unused) {
  /* no door bell method supported */
}

static uint16_t bcmfs4_dequeue_qp(struct bcmfs_qp *qp, void **ops,
                                  uint16_t budget) {
  int err;
  uint16_t reqid;
  uint64_t desc;
  ...
  while ((cmpl_read_offset != cmpl_write_offset) && (budget > 0)) {
    ...
    desc = *((uint64_t *)((uint8_t *)hwq->base_addr + cmpl_read_offset));
...
static int bcmfs4_start_qp(struct bcmfs_qp *qp) {
  int timeout;
  uint32_t val, off;
  ...
  for (off = 0; off < tx_queue->queue_size; off += FS_RING_DESC_SIZE) {
    ...
    if (FS_RING_BD_ALIGN_CHECK(next_addr))
      d = bcmfs4_next_table_desc(RING_BD_TOGGLE_VALID(off), next_addr);
    else
  ...
  if (!timeout) {
    BCMFS_DP_LOG(ERR, "Ring flush timeout hw-queue %d", qp->qpair_id);
  }
  ...
  if (!timeout) {
    BCMFS_DP_LOG(ERR, "Ring clear flush timeout hw-queue %d", qp->qpair_id);
  }
  ...
  tx_queue->tx_write_ptr =
      FS_MMIO_READ32((uint8_t *)qp->ioreg + RING_BD_WRITE_PTR);
  ...
  FS_MMIO_WRITE32((msi & 0xFFFFFFFF), (uint8_t *)qp->ioreg + RING_MSI_ADDR_LS);
  FS_MMIO_WRITE32(((msi >> 32) & 0xFFFFFFFF),
                  (uint8_t *)qp->ioreg + RING_MSI_ADDR_MS);
  FS_MMIO_WRITE32(qp->qpair_id, (uint8_t *)qp->ioreg + RING_MSI_DATA_VALUE);
...
static void bcmfs4_shutdown_qp(struct bcmfs_qp *qp) {
  /* Disable/deactivate ring */
  FS_MMIO_WRITE32(0x0, (uint8_t *)qp->ioreg + RING_CONTROL);
}
