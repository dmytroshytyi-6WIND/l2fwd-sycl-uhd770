...
#ifdef VFIO_PRESENT
static int vfio_map_dev_obj(const char *path, const char *dev_obj,
...
int bcmfs_attach_vfio(struct bcmfs_device *dev) {
  int ret;
  int vfio_dev_fd;
  void *v_addr = NULL;
  uint32_t size = 0;

  ret = vfio_map_dev_obj(dev->dirname, dev->name, &size, &v_addr, &vfio_dev_fd);
  if (ret)
    return -1;
...
void bcmfs_release_vfio(struct bcmfs_device *dev) {
  int ret;
  ...
  /* release the device */
  ret = rte_vfio_release_device(dev->dirname, dev->name, dev->vfio_dev_fd);
  if (ret < 0) {
...
#else
int bcmfs_attach_vfio(struct bcmfs_device *dev __rte_unused) { return -1; }

void bcmfs_release_vfio(struct bcmfs_device *dev __rte_unused) {}
#endif
