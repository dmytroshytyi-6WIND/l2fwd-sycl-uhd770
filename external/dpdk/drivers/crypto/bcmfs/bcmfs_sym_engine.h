...
#define BCMFS_CRYPTO_SHA3_224_BLOCK_SIZE                                       \
  (200 - 2 * BCMFS_CRYPTO_SHA3_224_DIGEST_SIZE)

#define BCMFS_CRYPTO_SHA3_256_DIGEST_SIZE (256 / 8)
#define BCMFS_CRYPTO_SHA3_256_BLOCK_SIZE                                       \
  (200 - 2 * BCMFS_CRYPTO_SHA3_256_DIGEST_SIZE)

#define BCMFS_CRYPTO_SHA3_384_DIGEST_SIZE (384 / 8)
#define BCMFS_CRYPTO_SHA3_384_BLOCK_SIZE                                       \
  (200 - 2 * BCMFS_CRYPTO_SHA3_384_DIGEST_SIZE)

#define BCMFS_CRYPTO_SHA3_512_DIGEST_SIZE (512 / 8)
#define BCMFS_CRYPTO_SHA3_512_BLOCK_SIZE                                       \
  (200 - 2 * BCMFS_CRYPTO_SHA3_512_DIGEST_SIZE)
...
int bcmfs_crypto_build_cipher_req(struct bcmfs_sym_request *req,
...
int bcmfs_crypto_build_auth_req(struct bcmfs_sym_request *req,
...
int bcmfs_crypto_build_chain_request(
    struct bcmfs_sym_request *req, enum rte_crypto_cipher_algorithm c_algo,
    enum rte_crypto_cipher_operation cop, enum rte_crypto_auth_algorithm a_algo,
    enum rte_crypto_auth_operation aop, struct fsattr *src, struct fsattr *dst,
    struct fsattr *cipher_key, struct fsattr *auth_key, struct fsattr *iv,
    struct fsattr *aad, struct fsattr *digest, bool cipher_first);

int bcmfs_crypto_build_aead_request(struct bcmfs_sym_request *req,
