...
struct bcmfs_hw_queue_pair_ops_table bcmfs_hw_queue_pair_ops_table = {
    .tl = RTE_SPINLOCK_INITIALIZER, .num_ops = 0};
...
int bcmfs_hw_queue_pair_register_ops(const struct bcmfs_hw_queue_pair_ops *h) {
  ...
  if (h->enq_one_req == NULL || h->dequeue == NULL || h->ring_db == NULL ||
      h->startq == NULL || h->stopq == NULL) {
    rte_spinlock_unlock(&bcmfs_hw_queue_pair_ops_table.tl);
    BCMFS_LOG(ERR, "Missing callback while registering device ops");
    return -EINVAL;
  }

  if (strlen(h->name) >= sizeof(ops->name) - 1) {
    rte_spinlock_unlock(&bcmfs_hw_queue_pair_ops_table.tl);
    BCMFS_LOG(ERR, "%s(): fs device_ops <%s>: name too long", __func__,
              h->name);
    return -EEXIST;
  }
...
fsdev_allocate_one_dev(struct rte_vdev_device *vdev, char *dirpath,
                       char *devname,
  ...
  for (i = 0; i < bcmfs_hw_queue_pair_ops_table.num_ops; i++)
    if (strstr(dirpath, bcmfs_hw_queue_pair_ops_table.qp_ops[i].name))
      fsdev->sym_hw_qp_ops = &bcmfs_hw_queue_pair_ops_table.qp_ops[i];
...
static struct bcmfs_device *find_fsdev(struct rte_vdev_device *vdev) {
  struct bcmfs_device *fsdev;
...
static void fsdev_release(struct bcmfs_device *fsdev) {
  if (fsdev == NULL)
    return;
...
static int cmprator(const void *a, const void *b) {
  return (*(const unsigned int *)a - *(const unsigned int *)b);
}

static int fsdev_find_all_devs(const char *path, const char *search,
                               uint32_t *devs) {
  DIR *dir;
  struct dirent *entry;
  int count = 0;
  ...
  while ((entry = readdir(dir)) != NULL) {
    if (strstr(entry->d_name, search)) {
      strlcpy(addr[count], entry->d_name, BCMFS_MAX_PATH_LEN);
      count++;
    }
  }
...
static bool fsdev_find_sub_dir(char *path, const char *search, char *output) {
  DIR *dir;
  struct dirent *entry;
...
static int bcmfs_vdev_probe(struct rte_vdev_device *vdev) {
  struct bcmfs_device *fsdev;
  ...
  while (strlen(dev_table[i].name)) {
    found = fsdev_find_sub_dir(top_dirpath, dev_table[i].name, sub_dirpath);
    if (found)
      break;
    i++;
  }
  ...
  snprintf(out_dirpath, sizeof(out_dirpath), "%s/%s", top_dirpath, sub_dirpath);
  count =
      fsdev_find_all_devs(out_dirpath, dev_table[dev_idx].suffix, fsdev_dev);
  if (!count) {
    BCMFS_LOG(ERR, "No supported bcmfs dev found");
    return -ENODEV;
  }
  ...
  while (count) {
    ...
    snprintf(out_dirname, sizeof(out_dirname), "%x.%s", fsdev_dev[i],
             dev_table[dev_idx].suffix);
    fsdev = fsdev_allocate_one_dev(vdev, out_dirpath, out_dirname, dtype);
    if (!fsdev) {
      count--;
      i++;
      continue;
  ...
  if (err) {
    BCMFS_LOG(WARNING, "Failed to create BCMFS SYM PMD for device %s",
              fsdev->name);
...
static int bcmfs_vdev_remove(struct rte_vdev_device *vdev) {
  struct bcmfs_device *fsdev;
...
/* Register with vdev */
static struct rte_vdev_driver rte_bcmfs_pmd = {.probe = bcmfs_vdev_probe,
                                               .remove = bcmfs_vdev_remove};

RTE_PMD_REGISTER_VDEV(bcmfs_pmd, rte_bcmfs_pmd);
