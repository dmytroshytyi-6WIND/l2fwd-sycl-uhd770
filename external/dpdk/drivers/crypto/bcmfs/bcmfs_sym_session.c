...
static void get_key(const uint8_t *input_key, int keylen,
                    uint8_t *session_key) {
  memcpy(session_key, input_key, keylen);
}

/* Set session cipher parameters */
static int crypto_set_session_cipher_parameters(
  ...
  get_key(cipher_xform->key.data, sess->cipher.key.length,
...
static int crypto_set_session_auth_parameters(
    struct bcmfs_sym_session *sess,
  ...
  get_key(auth_xform->key.data, auth_xform->key.length, sess->auth.key.data);

  return 0;
}

/* Set session aead parameters */
static int crypto_set_session_aead_parameters(
    struct bcmfs_sym_session *sess,
  ...
  get_key(aead_xform->aead.key.data, aead_xform->aead.key.length,
          sess->aead.key.data);
...
static int crypto_set_session_parameters(struct bcmfs_sym_session *sess,
                                         struct rte_crypto_sym_xform *xform) {
  int rc = 0;
  struct rte_crypto_cipher_xform *cipher_xform = crypto_get_cipher_xform(xform);
  struct rte_crypto_auth_xform *auth_xform = crypto_get_auth_xform(xform);
...
struct bcmfs_sym_session *bcmfs_sym_get_session(struct rte_crypto_op *op) {
  ...
  if (unlikely(op->sess_type == RTE_CRYPTO_OP_SESSIONLESS)) {
    ...
    sess = (struct bcmfs_sym_session *)get_sym_session_private_data(
        op->sym->session, cryptodev_bcmfs_driver_id);
  }
...
int bcmfs_sym_session_configure(struct rte_cryptodev *dev,
                                struct rte_crypto_sym_xform *xform,
  ...
  if (rte_mempool_get(mempool, &sess_private_data)) {
    BCMFS_DP_LOG(ERR, "Couldn't get object from session mempool");
    return -ENOMEM;
  }
...
void bcmfs_sym_session_clear(struct rte_cryptodev *dev,
                             struct rte_cryptodev_sym_session *sess) {
