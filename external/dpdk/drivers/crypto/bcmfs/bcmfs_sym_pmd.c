...
static int bcmfs_sym_dev_start(__rte_unused struct rte_cryptodev *dev) {
  return 0;
}

static void bcmfs_sym_dev_stop(__rte_unused struct rte_cryptodev *dev) {}

static int bcmfs_sym_dev_close(struct rte_cryptodev *dev) {
  int i, ret;
...
static void bcmfs_sym_dev_info_get(struct rte_cryptodev *dev,
...
static void bcmfs_sym_stats_get(struct rte_cryptodev *dev,
...
static void bcmfs_sym_stats_reset(struct rte_cryptodev *dev) {
  struct bcmfs_sym_dev_private *bcmfs_priv;
  struct bcmfs_device *fsdev;
...
static int bcmfs_sym_qp_release(struct rte_cryptodev *dev,
  ...
  struct bcmfs_qp *qp =
      (struct bcmfs_qp *)(dev->data->queue_pairs[queue_pair_id]);

  BCMFS_LOG(DEBUG, "Release sym qp %u on device %d", queue_pair_id,
            dev->data->dev_id);
  ...
  return bcmfs_qp_release(
      (struct bcmfs_qp **)&dev->data->queue_pairs[queue_pair_id]);
}

static void spu_req_init(struct bcmfs_sym_request *sr,
                         rte_iova_t iova __rte_unused) {
...
bcmfs_sym_req_pool_create(struct rte_cryptodev *cdev __rte_unused,
                          uint32_t nobjs, uint16_t qp_id, int socket_id) {
  char softreq_pool_name[RTE_RING_NAMESIZE];
  struct rte_mempool *mp;

  snprintf(softreq_pool_name, RTE_RING_NAMESIZE, "%s_%d", "bcm_sym", qp_id);

  mp = rte_mempool_create(softreq_pool_name, RTE_ALIGN_MUL_CEIL(nobjs, 64),
                          sizeof(struct bcmfs_sym_request), 64, 0, NULL, NULL,
                          req_pool_obj_init, NULL, socket_id, 0);
  if (mp == NULL)
    BCMFS_LOG(ERR, "Failed to create req pool, qid %d, err %d", qp_id,
              rte_errno);

  return mp;
}

static int bcmfs_sym_qp_setup(struct rte_cryptodev *cdev, uint16_t qp_id,
  ...
  qp->sr_mp = bcmfs_sym_req_pool_create(cdev, qp_conf->nb_descriptors, qp_id,
                                        socket_id);
  if (qp->sr_mp == NULL)
    return -ENOMEM;
...
/** Enqueue burst */
static uint16_t bcmfs_sym_pmd_enqueue_op_burst(void *queue_pair,
...
static uint16_t bcmfs_sym_pmd_dequeue_op_burst(void *queue_pair,
                                               struct rte_crypto_op **ops,
                                               uint16_t nb_ops) {
  int i;
  uint16_t deq = 0;
...
static const struct rte_driver cryptodev_bcmfs_sym_driver = {
    .name = bcmfs_sym_drv_name, .alias = bcmfs_sym_drv_name};

int bcmfs_sym_dev_create(struct bcmfs_device *fsdev) {
  ...
  snprintf(name, RTE_CRYPTODEV_NAME_MAX_LEN, "%s_%s", fsdev->name, "sym");
  ...
  cryptodev = rte_cryptodev_pmd_create(name, &fsdev->sym_rte_dev, &init_params);
  if (cryptodev == NULL)
    return -ENODEV;
...
int bcmfs_sym_dev_destroy(struct bcmfs_device *fsdev) {
  struct rte_cryptodev *cryptodev;
...
RTE_PMD_REGISTER_CRYPTO_DRIVER(bcmfs_crypto_drv, cryptodev_bcmfs_sym_driver,
