...
/* Helper function */
static int bcmfs_qp_check_queue_alignment(uint64_t phys_addr, uint32_t align) {
  if (((align - 1) & phys_addr) != 0)
    return -EINVAL;
  return 0;
}

static void bcmfs_queue_delete(struct bcmfs_queue *queue,
  ...
  BCMFS_LOG(DEBUG, "Free ring %d type %d, memzone: %s", queue_pair_id,
            queue->q_type, queue->memz_name);
  ...
  if (mz != NULL) {
    ...
    if (status != 0)
      BCMFS_LOG(ERR, "Error %d on freeing queue %s", status, queue->memz_name);
  } else {
    BCMFS_LOG(DEBUG, "queue %s doesn't exist", queue->memz_name);
  }
}
...
static const struct rte_memzone *queue_dma_zone_reserve(const char *queue_name,
  ...
  BCMFS_LOG(DEBUG, "Allocate memzone for %s, size %u on socket %u", queue_name,
            queue_size, socket_id);
  return rte_memzone_reserve_aligned(queue_name, queue_size, socket_id,
                                     RTE_MEMZONE_IOVA_CONTIG, align);
}

static int bcmfs_queue_create(struct bcmfs_queue *queue,
  ...
  if (qtype == BCMFS_RM_TXQ) {
    ...
    queue_size_bytes =
        qp_conf->nb_descriptors * qp_conf->max_descs_req * FS_RING_DESC_SIZE;
    queue_size_bytes = RTE_ALIGN_MUL_CEIL(queue_size_bytes, FS_RING_PAGE_SIZE);
   ...
   */
  snprintf(queue->memz_name, sizeof(queue->memz_name), "%s_%d_%s_%d_%s",
           "bcmfs", qtype, "qp_mem", queue_pair_id, q_name);
  qp_mz = queue_dma_zone_reserve(queue->memz_name, queue_size_bytes, 0, align);
  if (qp_mz == NULL) {
...
int bcmfs_qp_release(struct bcmfs_qp **qp_addr) {
...
int bcmfs_qp_setup(struct bcmfs_qp **qp_addr, uint16_t queue_pair_id,
                   struct bcmfs_qp_config *qp_conf) {
  struct bcmfs_qp *qp;
  uint32_t bmp_size;
  ...
  if (nb_descriptors < FS_RM_MIN_REQS) {
    BCMFS_LOG(ERR, "Can't create qp for %u descriptors", nb_descriptors);
    return -EINVAL;
  }
  ...
  qp = rte_zmalloc_socket("BCM FS PMD qp metadata", sizeof(*qp),
                          RTE_CACHE_LINE_SIZE, qp_conf->socket_id);
  if (qp == NULL) {
  ...
  rc = bcmfs_queue_create(&qp->tx_q, qp_conf, qp->qpair_id, BCMFS_RM_TXQ);
  if (rc) {
    BCMFS_LOG(ERR, "Tx queue create failed queue_pair_id %u", queue_pair_id);
    goto create_err;
  }

  rc = bcmfs_queue_create(&qp->cmpl_q, qp_conf, qp->qpair_id, BCMFS_RM_CPLQ);
  if (rc) {
    BCMFS_LOG(ERR, "Cmpl queue create failed queue_pair_id= %u", queue_pair_id);
    goto q_create_err;
  }
  ...
  qp->ctx_bmp_mem = rte_zmalloc("ctx_bmp_mem", bmp_size, RTE_CACHE_LINE_SIZE);
  ...
  qp->ctx_bmp = rte_bitmap_init(nb_descriptors, qp->ctx_bmp_mem, bmp_size);
  if (qp->ctx_bmp == NULL) {
    rc = -EINVAL;
    goto bmap_mem_free;
  }
  ...
  qp->ctx_pool =
      rte_zmalloc("qp_ctx_pool", sizeof(unsigned long) * nb_descriptors, 0);
  if (qp->ctx_pool == NULL) {
...
uint16_t bcmfs_enqueue_op_burst(void *qp, void **ops, uint16_t nb_ops) {
...
uint16_t bcmfs_dequeue_op_burst(void *qp, void **ops, uint16_t nb_ops) {
  struct bcmfs_qp *tmp_qp = (struct bcmfs_qp *)qp;
...
void bcmfs_qp_stats_get(struct bcmfs_qp **qp, int num_qp,
  ...
  if (stats == NULL) {
    BCMFS_LOG(ERR, "invalid param: stats %p", stats);
    return;
  }
