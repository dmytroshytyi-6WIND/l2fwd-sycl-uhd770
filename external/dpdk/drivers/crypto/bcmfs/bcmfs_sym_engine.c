...
#define SPU2_HASH_FIRST                                                        \
  0x1000000                        /* 1: hash input is input pkt               \
                                    * data                                     \
                                    */
...
#define SPU2_CIPH_PAD_EN                                                       \
  0x100000000000                       /* 1: Add pad to end of payload for     \
                                        *    enc                               \
                                        */
...
#define SPU2_HASH_KEY_LEN                                                      \
  0xFF00 /* len of hash key in bytes.                                          \
          * HMAC only.                                                         \
          */
...
#define SPU2_RET_IV                                                            \
  0x40000000 /* 1: return IV in output msg                                     \
              *    b4 payload                                                  \
              */
#define SPU2_RET_IV_LEN                                                        \
  0xF00000000 /* length in bytes of IV returned.                               \
               * 0 = 16 bytes                                                  \
               */
...
#define SPU2_TLS_LEN                                                           \
  0xFFFF00000000 /* TLS encrypt: cipher len                                    \
...
spu2_cipher_type_xlate(enum rte_crypto_cipher_algorithm cipher_alg,
                       enum spu2_cipher_type *spu2_type, struct fsattr *key) {
  int ret = 0;
...
static int spu2_hash_xlate(enum rte_crypto_auth_algorithm auth_alg,
                           struct fsattr *key, enum spu2_hash_type *spu2_type,
...
static int spu2_cipher_xlate(enum rte_crypto_cipher_algorithm cipher_alg,
                             struct fsattr *key,
...
static void spu2_fmd_ctrl0_write(struct spu2_fmd *fmd, bool is_inbound,
                                 bool auth_first, enum spu2_proto_sel protocol,
...
static void spu2_fmd_ctrl1_write(struct spu2_fmd *fmd, bool is_inbound,
                                 ...
                                 uint64_t cipher_key_len, bool gen_iv,
                                 bool hash_iv, bool return_iv,
                                 uint64_t ret_iv_len, uint64_t ret_iv_offset,
                                 uint64_t cipher_iv_len, uint64_t digest_size,
  ...
  if (auth_key_len != 0)
    ctrl1 |= ((auth_key_len << SPU2_HASH_KEY_LEN_SHIFT) & SPU2_HASH_KEY_LEN);

  if (cipher_key_len != 0)
    ctrl1 |= ((cipher_key_len << SPU2_CIPH_KEY_LEN_SHIFT) & SPU2_CIPH_KEY_LEN);
  ...
  if (digest_size != 0) {
    ctrl1 |= ((digest_size << SPU2_HASH_TAG_LEN_SHIFT) & SPU2_HASH_TAG_LEN);
  }
...
static void spu2_fmd_ctrl2_write(struct spu2_fmd *fmd, uint64_t cipher_offset,
  ...
  fmd->ctrl2 = aad1_offset | (aad1_len << SPU2_AAD1_LEN_SHIFT) |
...
static void spu2_fmd_ctrl3_write(struct spu2_fmd *fmd, uint64_t payload_len) {
...
int bcmfs_crypto_build_auth_req(struct bcmfs_sym_request *sreq,
  ...
  ret = spu2_hash_xlate(a_alg, auth_key, &spu2_auth_type, &spu2_auth_mode);
  if (ret)
    return -EINVAL;

  fmd = &sreq->fmd;

  spu2_fmd_ctrl0_write(fmd, is_inbound, SPU2_VAL_NONE, SPU2_PROTO_RESV,
                       SPU2_VAL_NONE, SPU2_VAL_NONE, spu2_auth_type,
                       spu2_auth_mode);

  spu2_fmd_ctrl1_write(fmd, is_inbound, SPU2_VAL_NONE, auth_ksize,
                       SPU2_VAL_NONE, false, false, SPU2_VAL_NONE,
                       SPU2_VAL_NONE, SPU2_VAL_NONE, iv_size, dst_size,
  ...
  /* Start of OMD */
  if (auth_ksize != 0) {
    memcpy((uint8_t *)fmd + src_msg_len, fsattr_va(auth_key), auth_ksize);
    src_msg_len += auth_ksize;
  }

  if (iv_size != 0) {
    memcpy((uint8_t *)fmd + src_msg_len, fsattr_va(iv), iv_size);
    src_msg_len += iv_size;
  } /* End of OMD */
...
int bcmfs_crypto_build_cipher_req(struct bcmfs_sym_request *sreq,
  ...
  ret = spu2_cipher_xlate(calgo, cipher_key, &spu2_ciph_type, &spu2_ciph_mode);
  if (ret)
    return -EINVAL;

  spu2_fmd_ctrl0_write(fmd, is_inbound, SPU2_VAL_NONE, SPU2_PROTO_RESV,
                       spu2_ciph_type, spu2_ciph_mode, SPU2_VAL_NONE,
                       SPU2_VAL_NONE);

  spu2_fmd_ctrl1_write(fmd, SPU2_VAL_NONE, SPU2_VAL_NONE, SPU2_VAL_NONE,
                       fsattr_sz(cipher_key), false, false, SPU2_VAL_NONE,
                       SPU2_VAL_NONE, SPU2_VAL_NONE, fsattr_sz(iv),
  ...
  if (cipher_key != NULL && fsattr_sz(cipher_key) != 0) {
    ...
    if (calgo == RTE_CRYPTO_CIPHER_AES_XTS) {
      ...
      memcpy(cipher_buf, (uint8_t *)fsattr_va(cipher_key) + xts_keylen,
             xts_keylen);
      memcpy(cipher_buf + xts_keylen, fsattr_va(cipher_key), xts_keylen);
    } else {
      memcpy(cipher_buf, fsattr_va(cipher_key), fsattr_sz(cipher_key));
    }
  ...
  if (iv != NULL && fsattr_sz(iv) != 0) {
    memcpy((uint8_t *)fmd + src_msg_len, fsattr_va(iv), fsattr_sz(iv));
...
int bcmfs_crypto_build_chain_request(
    struct bcmfs_sym_request *sreq, enum rte_crypto_cipher_algorithm cipher_alg,
    enum rte_crypto_cipher_operation cipher_op __rte_unused,
    enum rte_crypto_auth_algorithm auth_alg,
    enum rte_crypto_auth_operation auth_op, struct fsattr *src,
    struct fsattr *dst, struct fsattr *cipher_key, struct fsattr *auth_key,
    struct fsattr *iv, struct fsattr *aad, struct fsattr *digest,
    bool cipher_first) {
  int ret = 0;
  int src_index = 0;
  int dst_index = 0;
  ...
  enum spu2_cipher_type spu2_ciph_type = SPU2_CIPHER_TYPE_NONE;
  uint64_t auth_ksize = (auth_key != NULL) ? fsattr_sz(auth_key) : 0;
  uint64_t cipher_ksize = (cipher_key != NULL) ? fsattr_sz(cipher_key) : 0;
  uint64_t iv_size = (iv != NULL) ? fsattr_sz(iv) : 0;
  uint64_t digest_size = (digest != NULL) ? fsattr_sz(digest) : 0;
  ...
  /* spu2 hash algorithm and hash algorithm mode */
  ret = spu2_hash_xlate(auth_alg, auth_key, &spu2_auth_type, &spu2_auth_mode);
  if (ret)
    return -EINVAL;
  ...
  spu2_fmd_ctrl0_write(fmd, is_inbound, auth_first, SPU2_PROTO_RESV,
                       spu2_ciph_type, spu2_ciph_mode, spu2_auth_type,
                       spu2_auth_mode);

  spu2_fmd_ctrl1_write(fmd, is_inbound, aad_size, auth_ksize, cipher_ksize,
                       false, false, SPU2_VAL_NONE, SPU2_VAL_NONE,
                       SPU2_VAL_NONE, iv_size, digest_size, false,
                       SPU2_VAL_NONE);

  spu2_fmd_ctrl2_write(fmd, aad_size, auth_ksize, 0, cipher_ksize, iv_size);
  ...
  if (auth_ksize != 0) {
    ...
    src_msg_len += auth_ksize;
#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
    BCMFS_DP_HEXDUMP_LOG(DEBUG, "auth key:", fsattr_va(auth_key), auth_ksize);
#endif
  }

  if (cipher_ksize != 0) {
    memcpy((uint8_t *)fmd + src_msg_len, fsattr_va(cipher_key), cipher_ksize);
  ...
  if (iv_size != 0) {
    ...
    src_msg_len += iv_size;
#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
    BCMFS_DP_HEXDUMP_LOG(DEBUG, "iv key:", fsattr_va(iv), iv_size);
#endif
  } /* End of OMD */
...
#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
    BCMFS_DP_HEXDUMP_LOG(DEBUG, "aad :", fsattr_va(aad), aad_size);
#endif
  }
  ...
  if (auth_op == RTE_CRYPTO_AUTH_OP_VERIFY) {
     ...
     */
    if (digest_size != 0) {
      sreq->msgs.dsts_addr[dst_index] = sreq->dptr;
      sreq->msgs.dsts_len[dst_index] = fsattr_sz(digest);
      dst_index++;
    }
  } else {
    if (digest_size != 0) {
      sreq->msgs.dsts_addr[dst_index] = fsattr_pa(digest);
      sreq->msgs.dsts_len[dst_index] = fsattr_sz(digest);
      dst_index++;
    }
  }
...
static void bcmfs_crypto_ccm_update_iv(uint8_t *ivbuf, uint64_t *ivlen,
                                       bool is_esp) {
  ...
  if (is_esp)
    ...
    L = ((ivbuf[0] & CCM_B0_L_PRIME) >> CCM_B0_L_PRIME_SHIFT) + 1;
...
int bcmfs_crypto_build_aead_request(struct bcmfs_sym_request *sreq,
  ...
  if (ae_algo == RTE_CRYPTO_AEAD_AES_GCM) {
    ...
    auth_first = (aeop == RTE_CRYPTO_AEAD_OP_ENCRYPT) ? 0 : 1;
  }
  ...
  spu2_fmd_ctrl1_write(fmd, is_inbound, aad_size, 0, ksize, false, false,
                       SPU2_VAL_NONE, SPU2_VAL_NONE, SPU2_VAL_NONE, iv_size,
                       digest_size, false, SPU2_VAL_NONE);

  spu2_fmd_ctrl2_write(fmd, aad_size, 0, 0, ksize, iv_size);
  ...
  if (ksize) {
    memcpy((uint8_t *)fmd + src_msg_len, fsattr_va(key), ksize);
    src_msg_len += ksize;

#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
    BCMFS_DP_HEXDUMP_LOG(DEBUG, "cipher key:", fsattr_va(key), ksize);
#endif
  }
...
#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
    BCMFS_DP_HEXDUMP_LOG(DEBUG, "iv key:", fsattr_va(iv), fsattr_sz(iv));
#endif
  } /* End of OMD */
  ...
  if (aeop == RTE_CRYPTO_AEAD_OP_DECRYPT) {
    ...
    if (digest_size != 0) {
      sreq->msgs.dsts_addr[dst_index] = sreq->dptr;
      sreq->msgs.dsts_len[dst_index] = digest_size;
      dst_index++;
    }
  } else {
    if (digest_size != 0) {
