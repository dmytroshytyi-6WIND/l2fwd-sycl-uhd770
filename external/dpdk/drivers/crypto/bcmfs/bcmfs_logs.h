...
#define BCMFS_LOG(level, fmt, args...)                                         \
  rte_log(RTE_LOG_##level, bcmfs_conf_logtype, "%s(): " fmt "\n", __func__,    \
          ##args)

#define BCMFS_DP_LOG(level, fmt, args...)                                      \
  rte_log(RTE_LOG_##level, bcmfs_dp_logtype, "%s(): " fmt "\n", __func__,      \
          ##args)
 ...
 */
int bcmfs_hexdump_log(uint32_t level, uint32_t logtype, const char *heading,
