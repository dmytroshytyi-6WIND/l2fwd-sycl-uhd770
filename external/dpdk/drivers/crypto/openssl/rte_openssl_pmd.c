...
static void get_cipher_key(const uint8_t *input_key, int keylen,
                           uint8_t *session_key) {
  memcpy(session_key, input_key, keylen);
}

/** Get key ede 24 bytes standard from input key */
static int get_cipher_key_ede(const uint8_t *key, int keylen,
                              uint8_t *key_ede) {
  int res = 0;
...
static uint8_t get_cipher_algo(enum rte_crypto_cipher_algorithm sess_algo,
                               size_t keylen, const EVP_CIPHER **algo) {
  int res = 0;
...
static uint8_t get_auth_algo(enum rte_crypto_auth_algorithm sessalgo,
                             const EVP_MD **algo) {
  int res = 0;
...
/** Get adequate openssl function for input cipher algorithm */
static uint8_t get_aead_algo(enum rte_crypto_aead_algorithm sess_algo,
                             size_t keylen, const EVP_CIPHER **algo) {
  int res = 0;
...
static int openssl_set_sess_aead_enc_param(struct openssl_session *sess,
  ...
  if (get_aead_algo(algo, sess->cipher.key.length, &sess->cipher.evp_algo) != 0)
    return -EINVAL;
  ...
  if (EVP_EncryptInit_ex(sess->cipher.ctx, sess->cipher.evp_algo, NULL, NULL,
                         NULL) <= 0)
    return -EINVAL;

  if (EVP_CIPHER_CTX_ctrl(sess->cipher.ctx, iv_type, sess->iv.length, NULL) <=
      0)
    return -EINVAL;

  if (do_ccm)
    EVP_CIPHER_CTX_ctrl(sess->cipher.ctx, EVP_CTRL_CCM_SET_TAG, tag_len, NULL);
...
static int openssl_set_sess_aead_dec_param(struct openssl_session *sess,
                                           enum rte_crypto_aead_algorithm algo,
                                           uint8_t tag_len,
                                           const uint8_t *key) {
  int iv_type = 0;
  ...
  if (EVP_DecryptInit_ex(sess->cipher.ctx, sess->cipher.evp_algo, NULL, NULL,
                         NULL) <= 0)
    return -EINVAL;
...
static int openssl_set_session_cipher_parameters(
    struct openssl_session *sess, const struct rte_crypto_sym_xform *xform) {
  ...
  case RTE_CRYPTO_CIPHER_AES_CTR:
    ...
    if (sess->cipher.direction == RTE_CRYPTO_CIPHER_OP_ENCRYPT) {
      if (EVP_EncryptInit_ex(sess->cipher.ctx, sess->cipher.evp_algo, NULL,
                             xform->cipher.key.data, NULL) != 1) {
        return -EINVAL;
      }
    } else if (sess->cipher.direction == RTE_CRYPTO_CIPHER_OP_DECRYPT) {
      if (EVP_DecryptInit_ex(sess->cipher.ctx, sess->cipher.evp_algo, NULL,
                             xform->cipher.key.data, NULL) != 1) {
        return -EINVAL;
      }
    }
  ...
  case RTE_CRYPTO_CIPHER_3DES_CTR:
    ...
    if (get_cipher_key_ede(xform->cipher.key.data, sess->cipher.key.length,
  ...
  case RTE_CRYPTO_CIPHER_DES_DOCSISBPI:
    ...
    if (EVP_EncryptInit_ex(sess->cipher.bpi_ctx, EVP_des_ecb(), NULL,
                           xform->cipher.key.data, 0) != 1)
      return -EINVAL;
...
openssl_set_session_auth_parameters(struct openssl_session *sess,
  ...
  case RTE_CRYPTO_AUTH_AES_GMAC:
    ...
    if (sess->auth.operation == RTE_CRYPTO_AUTH_OP_GENERATE)
      return openssl_set_sess_aead_enc_param(sess, RTE_CRYPTO_AEAD_AES_GCM,
                                             xform->auth.digest_length,
                                             xform->auth.key.data);
    else
      return openssl_set_sess_aead_dec_param(sess, RTE_CRYPTO_AEAD_AES_GCM,
                                             xform->auth.digest_length,
                                             xform->auth.key.data);
    break;
  ...
  case RTE_CRYPTO_AUTH_SHA512_HMAC:
    ...
    if (HMAC_Init_ex(sess->auth.hmac.ctx, xform->auth.key.data,
                     xform->auth.key.length, sess->auth.hmac.evp_algo,
                     NULL) != 1)
      return -EINVAL;
    break;
...
int openssl_set_session_parameters(struct openssl_session *sess,
                                   const struct rte_crypto_sym_xform *xform) {
  ...
  if (cipher_xform) {
    ret = openssl_set_session_cipher_parameters(sess, cipher_xform);
    if (ret != 0) {
      OPENSSL_LOG(ERR, "Invalid/unsupported cipher parameters");
      return ret;
    }
  }
  ...
  if (auth_xform) {
    ...
    if (ret != 0) {
      OPENSSL_LOG(ERR, "Invalid/unsupported auth parameters");
      return ret;
    }
  }
  ...
  if (aead_xform) {
    ...
    if (ret != 0) {
      OPENSSL_LOG(ERR, "Invalid/unsupported AEAD parameters");
      return ret;
    }
  }
...
void openssl_reset_session(struct openssl_session *sess) {
...
static void *get_session(struct openssl_qp *qp, struct rte_crypto_op *op) {
  ...
  if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
    if (op->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC) {
      ...
      if (likely(op->sym->session != NULL))
        sess = (struct openssl_session *)get_sym_session_private_data(
            op->sym->session, cryptodev_driver_id);
    } else {
      if (likely(op->asym->session != NULL))
        asym_sess =
            (struct openssl_asym_session *)op->asym->session->sess_private_data;
      if (asym_sess == NULL)
    ...
    if (unlikely(openssl_set_session_parameters(sess, op->sym->xform) != 0)) {
      rte_mempool_put(qp->sess_mp, _sess);
      rte_mempool_put(qp->sess_mp_priv, _sess_private_data);
      sess = NULL;
    }
    op->sym->session = (struct rte_cryptodev_sym_session *)_sess;
    set_sym_session_private_data(op->sym->session, cryptodev_driver_id,
                                 _sess_private_data);
  }
 ...
 *------------------------------------------------------------------------------
 */
static inline int process_openssl_encryption_update(struct rte_mbuf *mbuf_src,
                                                    int offset, uint8_t **dst,
                                                    int srclen,
                                                    EVP_CIPHER_CTX *ctx,
                                                    uint8_t inplace) {
  struct rte_mbuf *m;
  ...
  for (m = mbuf_src; m != NULL && offset > rte_pktmbuf_data_len(m); m = m->next)
  ...
  *dst += dstlen;
  ...
  for (m = m->next; (m != NULL) && (n > 0); m = m->next) {
    ...
    if (diff && inplace) {
      rem = RTE_MIN(l, (EVP_CIPHER_CTX_block_size(ctx) - diff));
...
static inline int process_openssl_decryption_update(struct rte_mbuf *mbuf_src,
                                                    int offset, uint8_t **dst,
                                                    int srclen,
                                                    EVP_CIPHER_CTX *ctx,
                                                    uint8_t inplace) {
  ...
  *dst += dstlen;
  ...
  for (m = m->next; (m != NULL) && (n > 0); m = m->next) {
    ...
    if (diff && inplace) {
      rem = RTE_MIN(l, (EVP_CIPHER_CTX_block_size(ctx) - diff));
      if (EVP_DecryptUpdate(ctx, temp, &dstlen, src, rem) <= 0)
        return -1;
      n -= rem;
      rte_memcpy(*dst, temp, diff);
      rte_memcpy(src, temp + diff, rem);
...
static int process_openssl_cipher_encrypt(struct rte_mbuf *mbuf_src,
                                          uint8_t *dst, int offset, uint8_t *iv,
                                          int srclen, EVP_CIPHER_CTX *ctx,
                                          uint8_t inplace) {
  int totlen;
  ...
  if (process_openssl_encryption_update(mbuf_src, offset, &dst, srclen, ctx,
                                        inplace))
    goto process_cipher_encrypt_err;
...
/** Process standard openssl cipher encryption */
static int process_openssl_cipher_bpi_encrypt(uint8_t *src, uint8_t *dst,
  ...
  if (EVP_EncryptUpdate(ctx, encrypted_iv, &encrypted_ivlen, iv,
                        DES_BLOCK_SIZE) <= 0)
    goto process_cipher_encrypt_err;
...
static int process_openssl_cipher_decrypt(struct rte_mbuf *mbuf_src,
                                          uint8_t *dst, int offset, uint8_t *iv,
                                          int srclen, EVP_CIPHER_CTX *ctx,
                                          uint8_t inplace) {
  int totlen;
  ...
  if (process_openssl_decryption_update(mbuf_src, offset, &dst, srclen, ctx,
                                        inplace))
    goto process_cipher_decrypt_err;
...
static int process_openssl_cipher_des3ctr(struct rte_mbuf *mbuf_src,
                                          uint8_t *dst, int offset, uint8_t *iv,
                                          uint8_t *key, int srclen,
                                          EVP_CIPHER_CTX *ctx) {
...
static int process_openssl_auth_encryption_gcm(
    struct rte_mbuf *mbuf_src, int offset, int srclen, uint8_t *aad, int aadlen,
    uint8_t *iv, uint8_t *dst, uint8_t *tag, EVP_CIPHER_CTX *ctx) {
  int len = 0, unused = 0;
  uint8_t empty[] = {};
...
static int process_openssl_auth_encryption_ccm(struct rte_mbuf *mbuf_src,
                                               int offset, int srclen,
...
static int process_openssl_auth_decryption_gcm(
    struct rte_mbuf *mbuf_src, int offset, int srclen, uint8_t *aad, int aadlen,
    uint8_t *iv, uint8_t *dst, uint8_t *tag, EVP_CIPHER_CTX *ctx) {
  int len = 0, unused = 0;
  uint8_t empty[] = {};
...
static int process_openssl_auth_decryption_ccm(struct rte_mbuf *mbuf_src,
                                               int offset, int srclen,
                                               uint8_t *aad, int aadlen,
                                               uint8_t *iv, uint8_t *dst,
...
static int process_openssl_auth(struct rte_mbuf *mbuf_src, uint8_t *dst,
                                int offset, __rte_unused uint8_t *iv,
                                __rte_unused EVP_PKEY *pkey, int srclen,
                                EVP_MD_CTX *ctx, const EVP_MD *algo) {
  size_t dstlen;
  struct rte_mbuf *m;
  int l, n = srclen;
  uint8_t *src;
...
static int process_openssl_auth_hmac(struct rte_mbuf *mbuf_src, uint8_t *dst,
                                     int offset, int srclen, HMAC_CTX *ctx) {
  unsigned int dstlen;
  struct rte_mbuf *m;
  int l, n = srclen;
  uint8_t *src;
...
static void process_openssl_combined_op(struct rte_crypto_op *op,
  ...
  iv = rte_crypto_op_ctod_offset(op, uint8_t *, sess->iv.offset);
  if (sess->auth.algo == RTE_CRYPTO_AUTH_AES_GMAC) {
    srclen = 0;
    offset = op->sym->auth.data.offset;
    aadlen = op->sym->auth.data.length;
    aad =
        rte_pktmbuf_mtod_offset(mbuf_src, uint8_t *, op->sym->auth.data.offset);
    tag = op->sym->auth.digest.data;
    if (tag == NULL)
      tag = rte_pktmbuf_mtod_offset(mbuf_dst, uint8_t *, offset + aadlen);
  } else {
    srclen = op->sym->aead.data.length;
    dst =
        rte_pktmbuf_mtod_offset(mbuf_dst, uint8_t *, op->sym->aead.data.offset);
    offset = op->sym->aead.data.offset;
    aad = op->sym->aead.aad.data;
    aadlen = sess->auth.aad_length;
    tag = op->sym->aead.digest.data;
    if (tag == NULL)
      tag = rte_pktmbuf_mtod_offset(mbuf_dst, uint8_t *, offset + srclen);
  }
  ...
  if (sess->cipher.direction == RTE_CRYPTO_CIPHER_OP_ENCRYPT) {
    if (sess->auth.algo == RTE_CRYPTO_AUTH_AES_GMAC ||
      ...
      status = process_openssl_auth_encryption_gcm(
          mbuf_src, offset, srclen, aad, aadlen, iv, dst, tag, ctx_copy);
    else
      status = process_openssl_auth_encryption_ccm(mbuf_src, offset, srclen,
                                                   aad, aadlen, iv, dst, tag,
                                                   taglen, ctx_copy);
    ...
    if (sess->auth.algo == RTE_CRYPTO_AUTH_AES_GMAC ||
      ...
      status = process_openssl_auth_decryption_ccm(mbuf_src, offset, srclen,
                                                   aad, aadlen, iv, dst, tag,
                                                   taglen, ctx_copy);
  }
...
static void process_openssl_cipher_op(struct rte_crypto_op *op,
                                      struct openssl_session *sess,
                                      struct rte_mbuf *mbuf_src,
                                      struct rte_mbuf *mbuf_dst) {
  uint8_t *dst, *iv;
  ...
  dst =
      rte_pktmbuf_mtod_offset(mbuf_dst, uint8_t *, op->sym->cipher.data.offset);
  ...
  if (sess->cipher.mode == OPENSSL_CIPHER_LIB)
    ...
    status = process_openssl_cipher_des3ctr(
        mbuf_src, dst, op->sym->cipher.data.offset, iv, sess->cipher.key.data,
        srclen, ctx_copy);
...
/** Process cipher operation */
static void process_openssl_docsis_bpi_op(struct rte_crypto_op *op,
                                          struct openssl_session *sess,
                                          struct rte_mbuf *mbuf_src,
                                          struct rte_mbuf *mbuf_dst) {
  uint8_t *src, *dst, *iv;
  ...
  srclen = op->sym->cipher.data.length;
  src =
      rte_pktmbuf_mtod_offset(mbuf_src, uint8_t *, op->sym->cipher.data.offset);
  dst =
      rte_pktmbuf_mtod_offset(mbuf_dst, uint8_t *, op->sym->cipher.data.offset);
  ...
  if (sess->cipher.direction == RTE_CRYPTO_CIPHER_OP_ENCRYPT) {
    ...
    if (srclen < block_size) {
      status = process_openssl_cipher_bpi_encrypt(src, dst, iv, srclen,
                                                  sess->cipher.bpi_ctx);
    } else {
    ...
    if (srclen < block_size) {
      ...
      if (last_block_len) {
        ...
        status = process_openssl_cipher_bpi_encrypt(
            src, dst, iv, last_block_len, sess->cipher.bpi_ctx);
...
static void process_openssl_auth_op(struct openssl_qp *qp,
  ...
  case OPENSSL_AUTH_AS_AUTH:
    ...
    status =
        process_openssl_auth(mbuf_src, dst, op->sym->auth.data.offset, NULL,
                             NULL, srclen, ctx_a, sess->auth.auth.evp_algo);
    EVP_MD_CTX_destroy(ctx_a);
    break;
  case OPENSSL_AUTH_AS_HMAC:
    ctx_h = HMAC_CTX_new();
    HMAC_CTX_copy(ctx_h, sess->auth.hmac.ctx);
    status = process_openssl_auth_hmac(mbuf_src, dst, op->sym->auth.data.offset,
                                       srclen, ctx_h);
    HMAC_CTX_free(ctx_h);
    break;
  default:
...
static int process_openssl_dsa_sign_op(struct rte_crypto_op *cop,
                                       struct openssl_asym_session *sess) {
  struct rte_crypto_dsa_op_param *op = &cop->asym->dsa;
  DSA *dsa = sess->u.s.dsa;
  DSA_SIG *sign = NULL;

  sign = DSA_do_sign(op->message.data, op->message.length, dsa);
...
static int process_openssl_dsa_verify_op(struct rte_crypto_op *cop,
                                         struct openssl_asym_session *sess) {
  struct rte_crypto_dsa_op_param *op = &cop->asym->dsa;
  DSA *dsa = sess->u.s.dsa;
  int ret;
  ...
  ret = DSA_do_verify(op->message.data, op->message.length, sign, dsa);
...
static int process_openssl_dh_op(struct rte_crypto_op *cop,
                                 struct openssl_asym_session *sess) {
  ...
  if (sess->u.dh.key_op & (1 << RTE_CRYPTO_ASYM_OP_SHARED_SECRET_COMPUTE)) {
    ...
    peer_key = BN_bin2bn(op->pub_key.data, op->pub_key.length, peer_key);
    if (peer_key == NULL) {
      cop->status = RTE_CRYPTO_OP_STATUS_NOT_PROCESSED;
      return -1;
    }
    priv_key = BN_bin2bn(op->priv_key.data, op->priv_key.length, priv_key);
    if (priv_key == NULL) {
      BN_free(peer_key);
      cop->status = RTE_CRYPTO_OP_STATUS_NOT_PROCESSED;
      return -1;
    ...
    ret = DH_compute_key(op->shared_secret.data, peer_key, dh_key);
    if (ret < 0) {
      cop->status = RTE_CRYPTO_OP_STATUS_ERROR;
      BN_free(peer_key);
  ...
  if ((sess->u.dh.key_op & (1 << RTE_CRYPTO_ASYM_OP_PUBLIC_KEY_GENERATE)) &&
      !(sess->u.dh.key_op & (1 << RTE_CRYPTO_ASYM_OP_PRIVATE_KEY_GENERATE))) {
...
static int process_openssl_modexp_op(struct rte_crypto_op *cop,
  ...
  if (BN_mod_exp(res, base, sess->u.e.exp, sess->u.e.mod, sess->u.e.ctx)) {
...
static int process_openssl_rsa_op(struct rte_crypto_op *cop,
  ...
  default:
    ...
    OPENSSL_LOG(ERR, "rsa pad type not supported %d\n", pad);
    return 0;
  }

  switch (op->rsa.op_type) {
  case RTE_CRYPTO_ASYM_OP_ENCRYPT:
    ret = RSA_public_encrypt(op->rsa.message.length, op->rsa.message.data,
                             op->rsa.cipher.data, rsa, pad);

    if (ret > 0)
      op->rsa.cipher.length = ret;
    OPENSSL_LOG(DEBUG, "length of encrypted text %d\n", ret);
    break;

  case RTE_CRYPTO_ASYM_OP_DECRYPT:
    ret = RSA_private_decrypt(op->rsa.cipher.length, op->rsa.cipher.data,
                              op->rsa.message.data, rsa, pad);
    if (ret > 0)
      op->rsa.message.length = ret;
    break;

  case RTE_CRYPTO_ASYM_OP_SIGN:
    ret = RSA_private_encrypt(op->rsa.message.length, op->rsa.message.data,
                              op->rsa.sign.data, rsa, pad);
    if (ret > 0)
  ...
  case RTE_CRYPTO_ASYM_OP_VERIFY:
    ...
    ret = RSA_public_decrypt(op->rsa.sign.length, op->rsa.sign.data, tmp, rsa,
                             pad);
    ...
    if ((ret <= 0) ||
        (CRYPTO_memcmp(tmp, op->rsa.message.data, op->rsa.message.length))) {
...
static int process_asym_op(struct openssl_qp *qp, struct rte_crypto_op *op,
                           struct openssl_asym_session *sess) {
  int retval = 0;
...
static void copy_plaintext(struct rte_mbuf *m_src, struct rte_mbuf *m_dst,
                           struct rte_crypto_op *op) {
  uint8_t *p_src, *p_dst;
  ...
  if (op->sym->cipher.data.offset > op->sym->auth.data.offset)
    memcpy(p_dst + op->sym->auth.data.offset, p_src + op->sym->auth.data.offset,
           op->sym->cipher.data.offset - op->sym->auth.data.offset);
}

/** Process crypto operation for mbuf */
static int process_op(struct openssl_qp *qp, struct rte_crypto_op *op,
                      struct openssl_session *sess) {
  ...
  if (op->sess_type == RTE_CRYPTO_OP_SESSIONLESS) {
    ...
    memset(
        op->sym->session, 0,
        rte_cryptodev_sym_get_existing_header_session_size(op->sym->session));
...
/** Enqueue burst */
static uint16_t openssl_pmd_enqueue_burst(void *queue_pair,
  ...
  for (i = 0; i < nb_ops; i++) {
    ...
    if (ops[i]->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC)
      retval = process_op(qp, ops[i], (struct openssl_session *)sess);
    else
      retval = process_asym_op(qp, ops[i], (struct openssl_asym_session *)sess);
...
/** Dequeue burst */
static uint16_t openssl_pmd_dequeue_burst(void *queue_pair,
                                          struct rte_crypto_op **ops,
                                          uint16_t nb_ops) {
  struct openssl_qp *qp = queue_pair;
  ...
  nb_dequeued =
      rte_ring_dequeue_burst(qp->processed_ops, (void **)ops, nb_ops, NULL);
...
static int
cryptodev_openssl_create(const char *name, struct rte_vdev_device *vdev,
  ...
  dev->feature_flags =
      ...
      RTE_CRYPTODEV_FF_SYM_OPERATION_CHAINING | RTE_CRYPTODEV_FF_CPU_AESNI |
      RTE_CRYPTODEV_FF_IN_PLACE_SGL | RTE_CRYPTODEV_FF_OOP_SGL_IN_LB_OUT |
      RTE_CRYPTODEV_FF_OOP_LB_IN_LB_OUT | RTE_CRYPTODEV_FF_ASYMMETRIC_CRYPTO |
      RTE_CRYPTODEV_FF_RSA_PRIV_OP_KEY_EXP |
      RTE_CRYPTODEV_FF_RSA_PRIV_OP_KEY_QT | RTE_CRYPTODEV_FF_SYM_SESSIONLESS;
...
init_error:
  OPENSSL_LOG(ERR, "driver %s: create failed", init_params->name);
...
static int cryptodev_openssl_probe(struct rte_vdev_device *vdev) {
  struct rte_cryptodev_pmd_init_params init_params = {
      "", sizeof(struct openssl_private), rte_socket_id(),
...
static struct rte_vdev_driver cryptodev_openssl_pmd_drv = {
    .probe = cryptodev_openssl_probe, .remove = cryptodev_openssl_remove};

static struct cryptodev_driver openssl_crypto_drv;

RTE_PMD_REGISTER_VDEV(CRYPTODEV_NAME_OPENSSL_PMD, cryptodev_openssl_pmd_drv);
RTE_PMD_REGISTER_PARAM_STRING(CRYPTODEV_NAME_OPENSSL_PMD,
                              "max_nb_queue_pairs=<int> "
                              "socket_id=<int>");
RTE_PMD_REGISTER_CRYPTO_DRIVER(openssl_crypto_drv,
                               cryptodev_openssl_pmd_drv.driver,
                               cryptodev_driver_id);
