...
#define MLX5_CRYPTO_FEATURE_FLAGS                                              \
  (RTE_CRYPTODEV_FF_SYMMETRIC_CRYPTO | RTE_CRYPTODEV_FF_HW_ACCELERATED |       \
   ...
   RTE_CRYPTODEV_FF_OOP_LB_IN_LB_OUT | RTE_CRYPTODEV_FF_CIPHER_WRAPPED_KEY |   \
   RTE_CRYPTODEV_FF_CIPHER_MULTIPLE_DATA_UNITS)

TAILQ_HEAD(mlx5_crypto_privs, mlx5_crypto_priv)
mlx5_crypto_priv_list = TAILQ_HEAD_INITIALIZER(mlx5_crypto_priv_list);
...
static const struct rte_driver mlx5_drv = {.name = mlx5_crypto_drv_name,
...
static void mlx5_crypto_dev_infos_get(struct rte_cryptodev *dev,
...
static int mlx5_crypto_dev_configure(struct rte_cryptodev *dev,
  ...
  if ((config->ff_disable & RTE_CRYPTODEV_FF_SYMMETRIC_CRYPTO) != 0) {
    DRV_LOG(ERR, "Disabled symmetric crypto feature is not supported.");
    return -ENOTSUP;
  }
...
static void mlx5_crypto_dev_stop(struct rte_cryptodev *dev) {
  RTE_SET_USED(dev);
}

static int mlx5_crypto_dev_start(struct rte_cryptodev *dev) {
  struct mlx5_crypto_priv *priv = dev->data->dev_private;

  return mlx5_dev_mempool_subscribe(priv->cdev);
}

static int mlx5_crypto_dev_close(struct rte_cryptodev *dev) {
  struct mlx5_crypto_priv *priv = dev->data->dev_private;
...
static int mlx5_crypto_sym_session_configure(
  ...
  if (ret != 0) {
    DRV_LOG(ERR, "Failed to get session %p private data from mempool.",
            sess_private_data);
    return -ENOMEM;
  }
  cipher = &xform->cipher;
  ...
  sess_private_data->bs_bpt_eo_es =
      rte_cpu_to_be_32(MLX5_BSF_SIZE_64B << MLX5_BSF_SIZE_OFFSET |
  ...
  case 0:
    ...
    break;
  case 512:
    sess_private_data->bsp_res = rte_cpu_to_be_32((uint32_t)MLX5_BLOCK_SIZE_512B
                                                  << MLX5_BLOCK_SIZE_OFFSET);
    break;
  case 4096:
    sess_private_data->bsp_res = rte_cpu_to_be_32(
        (uint32_t)MLX5_BLOCK_SIZE_4096B << MLX5_BLOCK_SIZE_OFFSET);
    break;
  case 1048576:
    sess_private_data->bsp_res = rte_cpu_to_be_32((uint32_t)MLX5_BLOCK_SIZE_1MB
                                                  << MLX5_BLOCK_SIZE_OFFSET);
    break;
  default:
  ...
  sess_private_data->dek_id =
      rte_cpu_to_be_32(sess_private_data->dek->obj->id & 0xffffff);
  set_sym_session_private_data(session, dev->driver_id, sess_private_data);
...
static void mlx5_crypto_indirect_mkeys_release(struct mlx5_crypto_qp *qp,
                                               uint16_t n) {
  uint16_t i;
...
static void mlx5_crypto_qp_release(struct mlx5_crypto_qp *qp) {
  if (qp == NULL)
    return;
...
static int mlx5_crypto_queue_pair_release(struct rte_cryptodev *dev,
                                          uint16_t qp_id) {
...
mlx5_crypto_get_block_size(struct rte_crypto_op *op) {
  ...
  case (1 << 12):
    return RTE_BE32(MLX5_BLOCK_SIZE_4096B << MLX5_BLOCK_SIZE_OFFSET);
  case (1 << 9):
...
static __rte_always_inline uint32_t mlx5_crypto_klm_set(
    struct mlx5_crypto_qp *qp, struct rte_mbuf *mbuf, struct mlx5_wqe_dseg *klm,
...
mlx5_crypto_klms_set(struct mlx5_crypto_qp *qp, struct rte_crypto_op *op,
  ...
  if (unlikely(mlx5_crypto_klm_set(qp, mbuf, klm, op->sym->cipher.data.offset,
  ...
  while (remain_len) {
    ...
    if (unlikely(mlx5_crypto_klm_set(qp, mbuf, ++klm, 0, &remain_len) ==
                 UINT32_MAX)) {
      op->status = RTE_CRYPTO_OP_STATUS_ERROR;
      return 0;
    }
...
mlx5_crypto_wqe_set(struct mlx5_crypto_priv *priv, struct mlx5_crypto_qp *qp,
                    struct rte_crypto_op *op, struct mlx5_umr_wqe *umr) {
  struct mlx5_crypto_session *sess =
      get_sym_session_private_data(op->sym->session, mlx5_crypto_driver_id);
  ...
  struct mlx5_wqe_umr_bsf_seg *bsf =
      ((struct mlx5_wqe_umr_bsf_seg *)RTE_PTR_ADD(umr, priv->umr_wqe_size)) - 1;
  uint32_t ds;
  bool ipl = op->sym->m_dst == NULL || op->sym->m_dst == op->sym->m_src;
  /* Set UMR WQE. */
  uint32_t klm_n =
      mlx5_crypto_klms_set(qp, op, ipl ? op->sym->m_src : op->sym->m_dst, klms);
  ...
  cseg->opcode = rte_cpu_to_be_32((qp->db_pi << 8) | MLX5_OPCODE_RDMA_WRITE);
  ds = RTE_ALIGN(ds, 4);
  qp->db_pi += ds >> 2;
  ...
  if (priv->max_rdmar_ds > ds) {
    ...
    cseg->sq_ds = rte_cpu_to_be_32((qp->qp_obj.qp->id << 8) | ds);
    cseg->opcode = rte_cpu_to_be_32((qp->db_pi << 8) | MLX5_OPCODE_NOP);
...
static uint16_t mlx5_crypto_enqueue_burst(void *queue_pair,
  ...
  do {
    idx = qp->pi & mask;
    op = *ops++;
    umr = RTE_PTR_ADD(qp->qp_obj.umem_buf, priv->wqe_set_size * idx);
  ...
  mlx5_doorbell_ring(&priv->uar.bf_db, *(volatile uint64_t *)qp->wqe, qp->db_pi,
                     &qp->qp_obj.db_rec[MLX5_SND_DBR], !priv->uar.dbnc);
  return nb_ops;
}

static __rte_noinline void
mlx5_crypto_cqe_err_handle(struct mlx5_crypto_qp *qp,
                           struct rte_crypto_op *op) {
  const uint32_t idx = qp->ci & (qp->entries_n - 1);
  volatile struct mlx5_err_cqe *cqe =
      (volatile struct mlx5_err_cqe *)&qp->cq_obj.cqes[idx];
...
static uint16_t mlx5_crypto_dequeue_burst(void *queue_pair,
                                          struct rte_crypto_op **ops,
                                          uint16_t nb_ops) {
  struct mlx5_crypto_qp *qp = queue_pair;
...
static void mlx5_crypto_qp_init(struct mlx5_crypto_priv *priv,
                                struct mlx5_crypto_qp *qp) {
  uint32_t i;

  for (i = 0; i < qp->entries_n; i++) {
    struct mlx5_wqe_cseg *cseg =
        RTE_PTR_ADD(qp->qp_obj.umem_buf, i * priv->wqe_set_size);
    struct mlx5_wqe_umr_cseg *ucseg = (struct mlx5_wqe_umr_cseg *)(cseg + 1);
    struct mlx5_wqe_umr_bsf_seg *bsf =
        (struct mlx5_wqe_umr_bsf_seg *)(RTE_PTR_ADD(cseg, priv->umr_wqe_size)) -
        1;
    ...
    cseg->flags = RTE_BE32(MLX5_COMP_ONLY_FIRST_ERR << MLX5_COMP_MODE_OFFSET);
    ...
    ucseg->ko_to_bs = rte_cpu_to_be_32(
        (MLX5_CRYPTO_KLM_SEGS_NUM(priv->umr_wqe_size) << MLX5_UMRC_KO_OFFSET) |
        (4 << MLX5_UMRC_TO_BS_OFFSET));
    bsf->keytag = priv->keytag;
    /* Init RDMA WRITE WQE. */
    cseg = RTE_PTR_ADD(cseg, priv->umr_wqe_size);
    cseg->flags = RTE_BE32((MLX5_COMP_ALWAYS << MLX5_COMP_MODE_OFFSET) |
...
static int mlx5_crypto_indirect_mkeys_prepare(struct mlx5_crypto_priv *priv,
                                              struct mlx5_crypto_qp *qp) {
  struct mlx5_umr_wqe *umr;
  uint32_t i;
...
mlx5_crypto_queue_pair_setup(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  alloc_size +=
      (sizeof(struct rte_crypto_op *) + sizeof(struct mlx5_devx_obj *)) *
      RTE_BIT32(log_nb_desc);
  qp = rte_zmalloc_socket(__func__, alloc_size, RTE_CACHE_LINE_SIZE, socket_id);
  if (qp == NULL) {
    DRV_LOG(ERR, "Failed to allocate QP memory.");
    rte_errno = ENOMEM;
    return -rte_errno;
  }
  if (mlx5_devx_cq_create(priv->cdev->ctx, &qp->cq_obj, log_nb_desc, &cq_attr,
                          socket_id) != 0) {
  ...
  if (mlx5_mr_ctrl_init(&qp->mr_ctrl, &priv->cdev->mr_scache.dev_gen,
    ...
    DRV_LOG(ERR, "Cannot allocate MR Btree for qp %u.", (uint32_t)qp_id);
    rte_errno = ENOMEM;
    goto error;
  }
  /*
...
static void mlx5_crypto_stats_get(struct rte_cryptodev *dev,
...
static void mlx5_crypto_stats_reset(struct rte_cryptodev *dev) {
  int qp_id;
...
static int mlx5_crypto_args_check_handler(const char *key, const char *val,
                                          void *opaque) {
...
mlx5_crypto_parse_devargs(struct mlx5_kvargs_ctrl *mkvlist,
  ...
  const char **params = (const char *[]){
      "credential_id", "import_kek_id", "keytag",
      "max_segs_num",  "wcs_file",      NULL,
  };
  ...
  if (devarg_prms->login_devarg == false) {
    DRV_LOG(ERR, "No login credential devarg in order to enable crypto "
 ...
 */
static void mlx5_crypto_get_wqe_sizes(uint32_t segs_num, uint32_t *umr_size,
                                      uint32_t *rdmaw_size) {
  uint32_t diff, wqe_set_size;

  *umr_size = MLX5_CRYPTO_UMR_WQE_STATIC_SIZE +
              RTE_ALIGN(segs_num, 4) * sizeof(struct mlx5_wqe_dseg);
  ...
  *rdmaw_size = sizeof(struct mlx5_rdma_write_wqe) +
                sizeof(struct mlx5_wqe_dseg) *
                    (segs_num <= 2 ? 2 : 2 + RTE_ALIGN(segs_num - 2, 4));
...
static uint8_t mlx5_crypto_max_segs_num(uint16_t max_wqe_size) {
  ...
  while (max_segs_cap) {
    ...
    mlx5_crypto_get_wqe_sizes(max_segs_cap, &umr_wqe_size, &rdmw_wqe_size);
    if (umr_wqe_size <= max_wqe_size && rdmw_wqe_size <= max_wqe_size)
      break;
    max_segs_cap -= 4;
  }
  return max_segs_cap;
...
static int mlx5_crypto_configure_wqe_size(struct mlx5_crypto_priv *priv,
                                          uint16_t max_wqe_size,
                                          uint32_t max_segs_num) {
  uint32_t rdmw_wqe_size, umr_wqe_size;

  mlx5_crypto_get_wqe_sizes(max_segs_num, &umr_wqe_size, &rdmw_wqe_size);
  priv->wqe_set_size = rdmw_wqe_size + umr_wqe_size;
  if (umr_wqe_size > max_wqe_size || rdmw_wqe_size > max_wqe_size) {
    DRV_LOG(ERR, "Invalid max_segs_num: %u. should be %u or lower.",
            max_segs_num, mlx5_crypto_max_segs_num(max_wqe_size));
    rte_errno = EINVAL;
    return -EINVAL;
  }
...
static int mlx5_crypto_dev_probe(struct mlx5_common_device *cdev,
  ...
  struct rte_cryptodev_pmd_init_params init_params = {
      ...
      .max_nb_queue_pairs = RTE_CRYPTODEV_PMD_DEFAULT_MAX_NB_QUEUE_PAIRS,
  };
  ...
  crypto_dev = rte_cryptodev_pmd_create(ibdev_name, cdev->dev, &init_params);
  if (crypto_dev == NULL) {
  ...
  login =
      mlx5_devx_cmd_create_crypto_login_obj(cdev->ctx, &devarg_prms.login_attr);
  if (login == NULL) {
  ...
  ret = mlx5_crypto_configure_wqe_size(
      priv, cdev->config.hca_attr.max_wqe_sz_sq, devarg_prms.max_segs_num);
  if (ret) {
    mlx5_devx_uar_release(&priv->uar);
    rte_cryptodev_pmd_destroy(priv->crypto_dev);
    return -1;
  ...
  DRV_LOG(INFO, "Max number of segments: %u.",
          (unsigned int)RTE_MIN(MLX5_CRYPTO_KLM_SEGS_NUM(priv->umr_wqe_size),
...
static int mlx5_crypto_dev_remove(struct mlx5_common_device *cdev) {
...
RTE_PMD_REGISTER_KMOD_DEP(MLX5_CRYPTO_DRIVER_NAME,
