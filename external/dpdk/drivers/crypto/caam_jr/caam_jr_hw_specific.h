...
#define hw_get_inp_queue_base(jr)                                              \
  ((((dma_addr_t)GET_JR_REG(IRBA, (jr))) << 32) | (GET_JR_REG_LO(IRBA, (jr))))

/* Read pointer to job ring output ring start address */
#define hw_get_out_queue_base(jr)                                              \
  (((dma_addr_t)(GET_JR_REG(ORBA, (jr))) << 32) | (GET_JR_REG_LO(ORBA, (jr))))
#else
...
#define hw_set_input_ring_size(job_ring, size)                                 \
  SET_JR_REG(IRSR, job_ring, (size))

#define hw_set_output_ring_size(job_ring, size)                                \
  SET_JR_REG(ORSR, job_ring, (size))
...
#define sec_read_64(addr)                                                      \
  (((u64)sec_read_32((u32 *)(addr) + 1) << 32) | (sec_read_32((u32 *)(addr))))
...
#define sec_read_64(addr)                                                      \
  (((u64)sec_read_32((u32 *)(addr)) << 32) | (sec_read_32((u32 *)(addr) + 1)))
...
#define SET_JR_REG(name, jr, value) (sec_write_32(JR_REG(name, (jr)), value))
