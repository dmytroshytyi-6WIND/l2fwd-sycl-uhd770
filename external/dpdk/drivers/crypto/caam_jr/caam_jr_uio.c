 ...
 */
static bool file_name_match_extract(const char filename[], const char match[],
                                    int *number) {
  char *substr = NULL;
 ...
 */
static int file_read_first_line(const char root[], const char subdir[],
  ...
  snprintf(absolute_file_name, SEC_UIO_MAX_ATTR_FILE_NAME, "%s/%s/%s", root,
           subdir, filename);
 ...
 */
static int sec_uio_send_command(int uio_fd, int32_t uio_command) {
  int ret;
...
static void *uio_map_registers(int uio_device_fd, int uio_device_id,
   ...
   */
  ret = file_read_first_line(uio_sys_root, uio_sys_map_subdir, "size",
                             uio_map_size_str);
  ...
  mapped_address =
      mmap(0, /*dynamically choose virtual address */
           uio_map_size, PROT_READ | PROT_WRITE, MAP_SHARED, uio_device_fd, 0);
  /* offset = 0 because UIO device has only one mapping
   * for the entire SEC register memory
   */
  if (mapped_address == MAP_FAILED) {
    CAAM_JR_ERR("Failed to map registers! errno = %d job ring fd  = %d,"
...
void free_job_ring(int uio_fd) {
  ...
  /* Open device file */
  CAAM_JR_INFO("Closed device file for job ring %d , fd = %d", job_ring->jr_id,
               job_ring->uio_fd);
  ...
  if (munmap(job_ring->register_base_addr, job_ring->map_size)) {
    CAAM_JR_INFO("cannot munmap(%p, 0x%lx): %s", job_ring->register_base_addr,
...
struct uio_job_ring *config_job_ring(void) {
  ...
  SEC_ASSERT(job_ring->uio_fd >= 0, NULL,
             "Failed to open UIO device file for job ring %d", job_ring->jr_id);
  ...
  job_ring->register_base_addr =
      uio_map_registers(job_ring->uio_fd, job_ring->uio_minor_number,
...
int sec_configure(void) {
  ...
  while ((dir = readdir(d)) != NULL) {
    ...
    if (file_name_match_extract(dir->d_name, "uio", &uio_minor_number)) {
      /*
      ...
      ret = file_read_first_line(SEC_UIO_DEVICE_SYS_ATTR_PATH, dir->d_name,
                                 "name", uio_name);
      ...
      if (file_name_match_extract(uio_name, SEC_UIO_DEVICE_NAME, &jr_id)) {
        g_uio_job_ring[config_jr_no].jr_id = jr_id;
        g_uio_job_ring[config_jr_no].uio_minor_number = uio_minor_number;
