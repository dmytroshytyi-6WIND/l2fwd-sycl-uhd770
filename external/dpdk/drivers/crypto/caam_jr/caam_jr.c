...
static inline phys_addr_t caam_jr_vtop_ctx(struct caam_jr_op_ctx *ctx,
                                           void *vaddr) {
  return (size_t)vaddr - ctx->vtop_offset;
}

static inline void caam_jr_op_ending(struct caam_jr_op_ctx *ctx) {
...
static void caam_jr_stats_get(struct rte_cryptodev *dev,
                              struct rte_cryptodev_stats *stats) {
  struct caam_jr_qp **qp = (struct caam_jr_qp **)dev->data->queue_pairs;
  int i;
...
static void caam_jr_stats_reset(struct rte_cryptodev *dev) {
  int i;
  struct caam_jr_qp **qp = (struct caam_jr_qp **)(dev->data->queue_pairs);
...
static inline void caam_auth_alg(struct caam_jr_session *ses,
                                 struct alginfo *alginfo_a) {
  switch (ses->auth_alg) {
  case RTE_CRYPTO_AUTH_NULL:
    ses->digest_length = 0;
    break;
  case RTE_CRYPTO_AUTH_MD5_HMAC:
    alginfo_a->algtype = (ses->proto_alg == RTE_SECURITY_PROTOCOL_IPSEC)
...
static inline void caam_cipher_alg(struct caam_jr_session *ses,
                                   struct alginfo *alginfo_c) {
  switch (ses->cipher_alg) {
  case RTE_CRYPTO_CIPHER_NULL:
    break;
  case RTE_CRYPTO_CIPHER_AES_CBC:
    alginfo_c->algtype = (ses->proto_alg == RTE_SECURITY_PROTOCOL_IPSEC)
...
static inline void caam_aead_alg(struct caam_jr_session *ses,
...
static int caam_jr_prep_cdb(struct caam_jr_session *ses) {
  ...
  if (is_cipher_only(ses)) {
    ...
    err = rta_inline_query(IPSEC_AUTH_VAR_AES_DEC_BASE_DESC_LEN,
                           MIN_JOB_DESC_SIZE, (unsigned int *)cdb->sh_desc,
                           &cdb->sh_desc[2], 2);
    ...
    if (cdb->sh_desc[2] & 1)
      ...
      alginfo_c.key = (size_t)caam_jr_mem_vtop((void *)(size_t)alginfo_c.key);
      alginfo_c.key_type = RTA_DATA_PTR;
    }
    if (cdb->sh_desc[2] & (1 << 1))
      alginfo_a.key_type = RTA_DATA_IMM;
    else {
      alginfo_a.key = (size_t)caam_jr_mem_vtop((void *)(size_t)alginfo_a.key);
    ...
    if (is_proto_ipsec(ses)) {
      if (ses->dir == DIR_ENC) {
        shared_desc_len = cnstr_shdsc_ipsec_new_encap(
            ...
            (uint8_t *)&ses->ip4_hdr, &alginfo_c, &alginfo_a);
      } else if (ses->dir == DIR_DEC) {
        shared_desc_len = cnstr_shdsc_ipsec_new_decap(
            cdb->sh_desc, true, swap, SHR_SERIAL, &ses->decap_pdb, &alginfo_c,
            &alginfo_a);
      }
    } else {
...
static void hw_flush_job_ring(struct sec_job_ring_t *job_ring,
                              uint32_t do_notify, uint32_t *notified_descs) {
  int32_t jobs_no_to_discard = 0;
  int32_t discarded_descs_no = 0;

  CAAM_JR_DEBUG("Jr[%p] pi[%d] ci[%d].Flushing jr notify desc=[%d]", job_ring,
                job_ring->pidx, job_ring->cidx, do_notify);

  jobs_no_to_discard = hw_get_no_finished_jobs(job_ring);

  /* Discard all jobs */
  CAAM_JR_DEBUG("Jr[%p] pi[%d] ci[%d].Discarding %d descs", job_ring,
                job_ring->pidx, job_ring->cidx, jobs_no_to_discard);
 ...
 * @retval -1 for error
 */
static int hw_poll_job_ring(struct sec_job_ring_t *job_ring,
  ...
  if (JR_REG_JRINT_JRE_EXTRACT(GET_JR_REG(JRINT, job_ring))) {
    ...
    sec_error_code = JR_REG_JRINT_ERR_TYPE_EXTRACT(GET_JR_REG(JRINT, job_ring));
   ...
   */
  jobs_no_to_notify =
      (limit > number_of_jobs_available) ? number_of_jobs_available : limit;
  CAAM_JR_DP_DEBUG(
      "Jr[%p] pi[%d] ci[%d].limit =%d Available=%d.Jobs to notify=%d", job_ring,
      job_ring->pidx, job_ring->cidx, limit, number_of_jobs_available,
      jobs_no_to_notify);
  ...
  while (jobs_no_to_notify > notified_descs_no) {
    ...
    if (!current_desc_addr) {
      ...
      printf("false alarm %" PRIu64 "real %" PRIu64
             ...
             false_alarm, real_poll, sec_error_code, job_ring->cidx);
      rte_panic("CAAM JR descriptor NULL");
      return notified_descs_no;
    }
    current_desc = (struct job_descriptor *)caam_jr_dma_ptov(current_desc_addr);
    /* now increment the consumer index for the current job ring,
     * AFTER saving job in temporary location!
     */
    job_ring->cidx = SEC_CIRCULAR_COUNTER(job_ring->cidx, SEC_JOB_RING_SIZE);
    ...
    if (ctx->op->sess_type == RTE_CRYPTO_OP_SECURITY_SESSION) {
      ...
      if (ctx->op->sym->m_dst) {
        ...
        ctx->op->sym->m_dst->pkt_len = rte_be_to_cpu_16(ip4_hdr->ip_len);
        ctx->op->sym->m_dst->data_len = rte_be_to_cpu_16(ip4_hdr->ip_len);
      } else {
        ip4_hdr = (struct ip *)rte_pktmbuf_mtod(ctx->op->sym->m_src, char *);
        ctx->op->sym->m_src->pkt_len = rte_be_to_cpu_16(ip4_hdr->ip_len);
        ctx->op->sym->m_src->data_len = rte_be_to_cpu_16(ip4_hdr->ip_len);
      }
    }
    *ops = ctx->op;
...
static uint16_t caam_jr_dequeue_burst(void *qp, struct rte_crypto_op **ops,
                                      uint16_t nb_ops) {
  ...
  if (ring->jr_mode == SEC_NOTIFICATION_TYPE_NAPI) {
    if (num_rx < nb_ops) {
      ...
      SEC_ASSERT(ret == 0, ret, "Failed to enable irqs for job ring %p", ring);
    }
...
build_auth_only_sg(struct rte_crypto_op *op, struct caam_jr_session *ses) {
  ...
  if ((mbuf->nb_segs + extra_segs) > MAX_SG_ENTRIES) {
    CAAM_JR_DP_ERR("Auth: Max sec segs supported is %d", MAX_SG_ENTRIES);
    return NULL;
  }
  ...
  SEC_JD_SET_SD(jobdescr, (phys_addr_t)(caam_jr_dma_vtop(cdb)) + sdesc_offset,
                cdb->sh_hdr.hi.field.idlen);

  /* output */
  SEC_JD_SET_OUT_PTR(jobdescr, (uint64_t)sym->auth.digest.phys_addr, 0,
                     ses->digest_length);
  ...
  if (is_decode(ses)) {
    ...
    rte_memcpy(ctx->digest, sym->auth.digest.data, ses->digest_length);
#if CAAM_JR_DBG
  ...
  SEC_JD_SET_IN_PTR(jobdescr, (uint64_t)caam_jr_vtop_ctx(ctx, &ctx->sg[0]), 0,
                    length);
  /* enabling sg list */
...
build_auth_only(struct rte_crypto_op *op, struct caam_jr_session *ses) {
  ...
  if (is_decode(ses)) {
    ...
    SEC_JD_SET_IN_PTR(jobdescr, (uint64_t)start_addr, sym->auth.data.offset,
...
build_cipher_only_sg(struct rte_crypto_op *op, struct caam_jr_session *ses) {
  ...
  if (reg_segs > MAX_SG_ENTRIES) {
    CAAM_JR_DP_ERR("Cipher: Max sec segs supported is %d", MAX_SG_ENTRIES);
    return NULL;
  }
  ...
  sg->ptr = cpu_to_caam64(rte_pktmbuf_iova(mbuf) + sym->cipher.data.offset);
  ...
  SEC_JD_SET_OUT_PTR(jobdescr, (uint64_t)caam_jr_vtop_ctx(ctx, &ctx->sg[0]), 0,
                     length);
  /*enabling sg bit */
...
build_cipher_auth_sg(struct rte_crypto_op *op, struct caam_jr_session *ses) {
  ...
  uint8_t *IV_ptr = rte_crypto_op_ctod_offset(op, uint8_t *, ses->iv.offset);
  struct sec_job_descriptor_t *jobdescr;
  uint16_t auth_hdr_len = sym->cipher.data.offset - sym->auth.data.offset;
  uint16_t auth_tail_len =
      sym->auth.data.length - sym->cipher.data.length - auth_hdr_len;
  ...
  if (req_segs > MAX_SG_ENTRIES) {
    CAAM_JR_DP_ERR("Cipher-Auth: Max sec segs supported is %d", MAX_SG_ENTRIES);
    return NULL;
  }
  ...
  if (is_encode(ses))
    ...
    length = ses->iv.length + sym->auth.data.length + ses->digest_length;
...
build_cipher_auth(struct rte_crypto_op *op, struct caam_jr_session *ses) {
  ...
  SEC_JD_SET_OUT_PTR(jobdescr, (uint64_t)caam_jr_dma_vtop(&ctx->sg[6]), 0,
                     length);
  /* set sg bit */
  (jobdescr)->seq_out.command.word |= 0x01000000;
...
static inline struct caam_jr_op_ctx *build_proto(struct rte_crypto_op *op,
  ...
  /* input */
  SEC_JD_SET_IN_PTR(jobdescr, (uint64_t)src_start_addr, 0, sym->m_src->pkt_len);
  sym->m_src->packet_type &= ~RTE_PTYPE_L4_MASK;

  return ctx;
}

static int caam_jr_enqueue_op(struct rte_crypto_op *op, struct caam_jr_qp *qp) {
  ...
  case RTE_CRYPTO_OP_WITH_SESSION:
    ses = (struct caam_jr_session *)get_sym_session_private_data(
        op->sym->session, cryptodev_driver_id);
    break;
  case RTE_CRYPTO_OP_SECURITY_SESSION:
    ses = (struct caam_jr_session *)get_sec_session_private_data(
        op->sym->sec_session);
    break;
  default:
...
err1:
...
#if CAAM_JR_DBG
  if (is_decode(ses))
    rte_hexdump(stdout, "DECODE", rte_pktmbuf_mtod(op->sym->m_src, void *),
                rte_pktmbuf_data_len(op->sym->m_src));
  else
    rte_hexdump(stdout, "ENCODE", rte_pktmbuf_mtod(op->sym->m_src, void *),
                rte_pktmbuf_data_len(op->sym->m_src));
  ...
  CAAM_JR_DP_DEBUG("Jr[%p] pi[%d] ci[%d].Before sending desc", ring, ring->pidx,
                   ring->cidx);

  /* todo - do we want to retry */
  if (SEC_JOB_RING_IS_FULL(ring->pidx, ring->cidx, SEC_JOB_RING_SIZE,
                           SEC_JOB_RING_SIZE)) {
    CAAM_JR_DP_DEBUG("Ring FULL Jr[%p] pi[%d] ci[%d].Size = %d", ring,
                     ring->pidx, ring->cidx, SEC_JOB_RING_SIZE);
    caam_jr_op_ending(ctx);
    qp->tx_ring_full++;
    return -EBUSY;
  }
  ...
  jobdescr->seq_in.command.word = cpu_to_caam32(jobdescr->seq_in.command.word);
...
static uint16_t caam_jr_enqueue_burst(void *qp, struct rte_crypto_op **ops,
                                      uint16_t nb_ops) {
...
/* Release queue pair */
static int caam_jr_queue_pair_release(struct rte_cryptodev *dev,
                                      uint16_t qp_id) {
  ...
  if (qp_id >= internals->max_nb_queue_pairs) {
    CAAM_JR_ERR("Max supported qpid %d", internals->max_nb_queue_pairs);
    return -EINVAL;
  }
...
/* Setup a queue pair */
static int caam_jr_queue_pair_setup(
...
static int caam_jr_cipher_init(struct rte_cryptodev *dev __rte_unused,
...
static int caam_jr_auth_init(struct rte_cryptodev *dev __rte_unused,
                             struct rte_crypto_sym_xform *xform,
                             struct caam_jr_session *session) {
...
static int caam_jr_aead_init(struct rte_cryptodev *dev __rte_unused,
                             struct rte_crypto_sym_xform *xform,
                             struct caam_jr_session *session) {
...
static int caam_jr_set_session_parameters(struct rte_cryptodev *dev,
                                          struct rte_crypto_sym_xform *xform,
                                          void *sess) {
...
static int caam_jr_sym_session_configure(struct rte_cryptodev *dev,
                                         struct rte_crypto_sym_xform *xform,
...
static void caam_jr_sym_session_clear(struct rte_cryptodev *dev,
                                      struct rte_cryptodev_sym_session *sess) {
...
static int caam_jr_set_ipsec_session(__rte_unused struct rte_cryptodev *dev,
  ...
  if (ipsec_xform->direction == RTE_SECURITY_IPSEC_SA_DIR_EGRESS) {
    memset(&session->encap_pdb, 0,
           sizeof(struct ipsec_encap_pdb) + sizeof(session->ip4_hdr));
...
static int caam_jr_security_session_create(
  ...
  case RTE_SECURITY_PROTOCOL_IPSEC:
    ret = caam_jr_set_ipsec_session(cdev, conf, sess_private_data);
    break;
...
/* Clear the memory of session so it doesn't leave key material behind */
static int caam_jr_security_session_destroy(void *dev __rte_unused,
...
caam_jr_dev_configure(struct rte_cryptodev *dev,
  ...
  if (!internals->ctx_pool) {
    internals->ctx_pool = rte_mempool_create(
        (const char *)str, CTX_POOL_NUM_BUFS, sizeof(struct caam_jr_op_ctx),
        CTX_POOL_CACHE_SIZE, 0, NULL, NULL, NULL, NULL, SOCKET_ID_ANY, 0);
    if (!internals->ctx_pool) {
      CAAM_JR_ERR("%s create failed\n", str);
      return -ENOMEM;
    }
  } else
    CAAM_JR_INFO("mempool already created for dev_id : %d", dev->data->dev_id);

  return 0;
}

static int caam_jr_dev_start(struct rte_cryptodev *dev __rte_unused) {
  PMD_INIT_FUNC_TRACE();
  return 0;
}

static void caam_jr_dev_stop(struct rte_cryptodev *dev __rte_unused) {
  PMD_INIT_FUNC_TRACE();
}

static int caam_jr_dev_close(struct rte_cryptodev *dev) {
  struct sec_job_ring_t *internals;
...
static void caam_jr_dev_infos_get(struct rte_cryptodev *dev,
...
static int shutdown_job_ring(struct sec_job_ring_t *job_ring) {
  ...
  SEC_ASSERT(ret == 0, ret, "Failed to shutdown hardware job ring %p",
             job_ring);
  ...
  if (job_ring->jr_mode != SEC_NOTIFICATION_TYPE_POLL) {
    ...
    SEC_ASSERT(ret == 0, ret, "Failed to disable irqs for job ring %p",
               job_ring);
  }
 ...
 */
static int caam_jr_dev_uninit(struct rte_cryptodev *dev) {
  struct sec_job_ring_t *internals;
 ...
 *
 */
static void *init_job_ring(void *reg_base_addr, int irq_id) {
  ...
  job_ring->input_ring =
      caam_jr_dma_mem_alloc(L1_CACHE_BYTES, SEC_DMA_MEM_INPUT_RING_SIZE);
  memset(job_ring->input_ring, 0, SEC_DMA_MEM_INPUT_RING_SIZE);

  /* Allocate memory for output ring */
  job_ring->output_ring =
      caam_jr_dma_mem_alloc(L1_CACHE_BYTES, SEC_DMA_MEM_OUTPUT_RING_SIZE);
  ...
  if (jr_mode == SEC_NOTIFICATION_TYPE_NAPI) {
    ...
    if (napi_mode == SEC_STARTUP_INTERRUPT_MODE) {
      CAAM_JR_INFO("Enabling DONE IRQ generationon job ring - %p", job_ring);
     ...
     */
    CAAM_JR_INFO("Enabling DONE IRQ generation on job ring - %p", job_ring);
    ret = caam_jr_enable_irqs(job_ring->irq_fd);
    if (ret != 0) {
      CAAM_JR_ERR("Failed to enable irqs for job ring");
      goto cleanup;
...
static int caam_jr_dev_init(const char *name, struct rte_vdev_device *vdev,
  ...
  if (g_job_rings_no >= g_job_rings_max) {
    CAAM_JR_ERR("No more job rings available max=%d!!!!", g_job_rings_max);
    return -1;
  }
  ...
  dev->data->dev_private =
      init_job_ring(job_ring->register_base_addr, job_ring->uio_fd);
  ...
  dev->feature_flags =
      ...
      RTE_CRYPTODEV_FF_OOP_SGL_IN_LB_OUT | RTE_CRYPTODEV_FF_OOP_LB_IN_SGL_OUT |
  ...
  /*TODO free it during teardown*/
  security_instance = rte_malloc("caam_jr", sizeof(struct rte_security_ctx), 0);
...
init_error:
  CAAM_JR_ERR("driver %s: cryptodev_caam_jr_create failed", init_params->name);

  return -ENXIO;
}

/** Initialise CAAM JR crypto device */
static int cryptodev_caam_jr_probe(struct rte_vdev_device *vdev) {
  int ret;

  struct rte_cryptodev_pmd_init_params init_params = {
      "", sizeof(struct sec_job_ring_t), rte_socket_id(),
  ...
  ret = of_init();
  if (ret) {
    RTE_LOG(ERR, PMD, "of_init failed\n");
    return -EINVAL;
  }
  ...
  if (!rta_get_sec_era()) {
    ...
    for_each_compatible_node(caam_node, NULL, "fsl,sec-v4.0") {
      ...
      if (prop) {
        rta_set_sec_era(INTL_SEC_ERA(rte_be_to_cpu_32(*prop)));
        break;
      }
    }
  }
...
static int cryptodev_caam_jr_remove(struct rte_vdev_device *vdev) {
...
static void sec_job_rings_init(void) {
  int i;
...
static struct rte_vdev_driver cryptodev_caam_jr_drv = {
    .probe = cryptodev_caam_jr_probe, .remove = cryptodev_caam_jr_remove};
