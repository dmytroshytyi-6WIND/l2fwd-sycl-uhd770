...
int hw_shutdown_job_ring(struct sec_job_ring_t *job_ring) {
  ...
  if (job_ring->register_base_addr == NULL) {
    CAAM_JR_ERR("Jr[%p] has reg base addr as NULL.driver not init", job_ring);
    return 0;
  }
  ...
  if ((tmp & JRINT_ERR_HALT_MASK) != JRINT_ERR_HALT_COMPLETE || timeout == 0) {
...
void hw_job_ring_error_print(struct sec_job_ring_t *job_ring, int code) {
  ...
  case JRINT_ERR_BAD_INPUT_BASE:
    CAAM_JR_ERR("Bad Input Ring Base (%p) (not on a 4-byte boundary) ",
                (void *)job_ring);
    break;
  case JRINT_ERR_BAD_OUTPUT_BASE:
    CAAM_JR_ERR("Bad Output Ring Base (%p) (not on a 4-byte boundary) ",
                (void *)job_ring);
    break;
  case JRINT_ERR_WRITE_2_IRBA:
    CAAM_JR_ERR("Invalid write to Input Ring Base Address Register ");
    break;
  case JRINT_ERR_WRITE_2_ORBA:
    CAAM_JR_ERR("Invalid write to Output Ring Base Address Register ");
    break;
  case JRINT_ERR_RES_B4_HALT:
    CAAM_JR_ERR("Job Ring [%p] released before Job Ring is halted",
                (void *)job_ring);
    break;
  case JRINT_ERR_REM_TOO_MANY:
    CAAM_JR_ERR("Removed too many jobs from job ring [%p]", (void *)job_ring);
    break;
  case JRINT_ERR_ADD_TOO_MANY:
    CAAM_JR_ERR("Added too many jobs on job ring [%p]", job_ring);
    break;
  default:
    CAAM_JR_ERR(" Unknown SEC JR Error :%d", code);
    break;
  }
}
...
int hw_job_ring_enable_coalescing(struct sec_job_ring_t *job_ring) {
  ...
  CAAM_JR_DEBUG("Enabled coalescing on jr %p ", job_ring);
