...
int null_crypto_set_session_parameters(
...
static int process_op(const struct null_crypto_qp *qp, struct rte_crypto_op *op,
                      struct null_crypto_session *sess __rte_unused) {
  ...
  if (op->sess_type == RTE_CRYPTO_OP_SESSIONLESS) {
    memset(op->sym->session, 0, sizeof(struct null_crypto_session));
...
static struct null_crypto_session *get_session(struct null_crypto_qp *qp,
  ...
  if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
    if (likely(sym_op->session != NULL))
      sess = (struct null_crypto_session *)get_sym_session_private_data(
    ...
    if (unlikely(null_crypto_set_session_parameters(sess, sym_op->xform) !=
                 0)) {
...
/** Enqueue burst */
static uint16_t null_crypto_pmd_enqueue_burst(void *queue_pair,
...
/** Dequeue burst */
static uint16_t null_crypto_pmd_dequeue_burst(void *queue_pair,
                                              struct rte_crypto_op **ops,
                                              uint16_t nb_ops) {
  struct null_crypto_qp *qp = queue_pair;
...
static int
cryptodev_null_create(const char *name, struct rte_vdev_device *vdev,
...
static int cryptodev_null_probe(struct rte_vdev_device *dev) {
  struct rte_cryptodev_pmd_init_params init_params = {
      "", sizeof(struct null_crypto_private), rte_socket_id(),
  ...
  if (retval) {
    NULL_LOG(ERR, "Failed to parse initialisation arguments[%s]", args);
    return -EINVAL;
  }
...
static int cryptodev_null_remove_dev(struct rte_vdev_device *vdev) {
