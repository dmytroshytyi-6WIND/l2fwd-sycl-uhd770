...
/** Start device */
static int null_crypto_pmd_start(__rte_unused struct rte_cryptodev *dev) {
  return 0;
}

/** Stop device */
static void null_crypto_pmd_stop(__rte_unused struct rte_cryptodev *dev) {}

/** Close device */
static int null_crypto_pmd_close(__rte_unused struct rte_cryptodev *dev) {
  return 0;
}

/** Get device statistics */
static void null_crypto_pmd_stats_get(struct rte_cryptodev *dev,
...
static void null_crypto_pmd_stats_reset(struct rte_cryptodev *dev) {
  int qp_id;
...
/** Get device info */
static void null_crypto_pmd_info_get(struct rte_cryptodev *dev,
...
/** Release queue pair */
static int null_crypto_pmd_qp_release(struct rte_cryptodev *dev,
                                      uint16_t qp_id) {
...
static int null_crypto_pmd_qp_set_unique_name(struct rte_cryptodev *dev,
                                              struct null_crypto_qp *qp) {
  unsigned n = snprintf(qp->name, sizeof(qp->name), "null_crypto_pmd_%u_qp_%u",
...
static struct rte_ring *null_crypto_pmd_qp_create_processed_pkts_ring(
    struct null_crypto_qp *qp, unsigned ring_size, int socket_id) {
  struct rte_ring *r;
...
/** Setup a queue pair */
static int null_crypto_pmd_qp_setup(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  if (qp_id >= internals->max_nb_qpairs) {
    NULL_LOG(ERR,
             "Invalid qp_id %u, greater than maximum "
  ...
  qp->processed_pkts = null_crypto_pmd_qp_create_processed_pkts_ring(
...
static int null_crypto_pmd_sym_session_configure(
  ...
  if (rte_mempool_get(mp, &sess_private_data)) {
    NULL_LOG(ERR, "Couldn't get object from session mempool");
    return -ENOMEM;
  }
