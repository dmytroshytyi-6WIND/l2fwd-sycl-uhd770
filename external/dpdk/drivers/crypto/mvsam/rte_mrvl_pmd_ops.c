 ...
 */
static const struct rte_cryptodev_capabilities mrvl_crypto_pmd_capabilities[] =
    {{/* MD5 HMAC */
              ...
              {.xform_type = RTE_CRYPTO_SYM_XFORM_AEAD,
               {
                   .aead =
                       {.algo = RTE_CRYPTO_AEAD_AES_GCM,
                        ...
                        .aad_size = {.min = 0, .max = 64, .increment = 1},
              ...
              {.xform_type = RTE_CRYPTO_SYM_XFORM_AUTH,
               {
                   .auth =
                       {.algo = RTE_CRYPTO_AUTH_AES_GMAC,
                        ...
                        .key_size = {.min = 16, .max = 32, .increment = 8},
                        .digest_size = {.min = 16, .max = 16, .increment = 0},
                        .iv_size = {.min = 8, .max = 65532, .increment = 4}},
               }},
      }},
     {/* 3DES CBC */
      .op = RTE_CRYPTO_OP_TYPE_SYMMETRIC,
 ...
 * @returns 0. Always.
 */
static int mrvl_crypto_pmd_start(__rte_unused struct rte_cryptodev *dev) {
  return 0;
}
 ...
 * @returns 0. Always.
 */
static void mrvl_crypto_pmd_stop(__rte_unused struct rte_cryptodev *dev) {}
 ...
 */
static void mrvl_crypto_pmd_stats_get(struct rte_cryptodev *dev,
 ...
 * @param dev Pointer to the device structure.
 */
static void mrvl_crypto_pmd_stats_reset(struct rte_cryptodev *dev) {
  int qp_id;
 ...
 */
static void mrvl_crypto_pmd_info_get(struct rte_cryptodev *dev,
 ...
 * @returns 0. Always.
 */
static int mrvl_crypto_pmd_qp_release(struct rte_cryptodev *dev,
                                      uint16_t qp_id) {
 ...
 * @returns 0. Always.
 */
static int mrvl_crypto_pmd_close(struct rte_cryptodev *dev) {
  int qp_id;
 ...
 */
static int mrvl_crypto_pmd_qp_setup(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  do { /* Error handling block */
     ...
     */
    n = snprintf(match, sizeof(match), "cio-%u:%u", qp_id % num, qp_id / num);
 ...
 * @returns 0 upon success, negative value otherwise.
 */
static int mrvl_crypto_pmd_sym_session_configure(
    __rte_unused struct rte_cryptodev *dev, struct rte_crypto_sym_xform *xform,
    struct rte_cryptodev_sym_session *sess, struct rte_mempool *mp) {
  ...
  if (sam_session_create(&mrvl_sess->sam_sess_params, &mrvl_sess->sam_sess) <
      0) {
...
mrvl_crypto_pmd_sym_session_clear(struct rte_cryptodev *dev,
  ...
  if (sess_priv) {
    ...
    if (mrvl_sess->sam_sess && sam_session_destroy(mrvl_sess->sam_sess) < 0) {
 ...
 * @returns 0 upon success, negative value otherwise.
 */
static int mrvl_crypto_pmd_security_session_create(
    __rte_unused void *dev, struct rte_security_session_conf *conf,
  ...
  case RTE_SECURITY_PROTOCOL_IPSEC:
    ...
    ret =
        mrvl_ipsec_set_session_parameters(mrvl_sess, ipsec_xform, crypto_xform);
    if (ret != 0) {
      MRVL_LOG(ERR, "Failed to configure session parameters.");
    ...
    if (mrvl_sess->sam_sess_params.cipher_mode == SAM_CIPHER_GCM) {
      ...
      mrvl_sess->sam_sess_params.cipher_iv = (uint8_t *)&(conf->ipsec.salt);
    }

    ret = sam_session_create(&mrvl_sess->sam_sess_params, &mrvl_sess->sam_sess);
    if (ret < 0) {
...
static const struct rte_security_capability mrvl_crypto_pmd_sec_security_cap[] =
