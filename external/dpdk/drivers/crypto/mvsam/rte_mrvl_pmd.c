...
const char *mrvl_pmd_valid_params[] = {
    RTE_CRYPTODEV_PMD_NAME_ARG, RTE_CRYPTODEV_PMD_MAX_NB_QP_ARG,
    RTE_CRYPTODEV_PMD_SOCKET_ID_ARG, MRVL_PMD_MAX_NB_SESS_ARG};
 ...
 */
static const struct cipher_params_mapping cipher_map[] = {
    [RTE_CRYPTO_CIPHER_NULL] = {.supported = ALGO_SUPPORTED,
                                .cipher_alg = SAM_CIPHER_NONE},
    [RTE_CRYPTO_CIPHER_3DES_CBC] = {.supported = ALGO_SUPPORTED,
                                    .cipher_alg = SAM_CIPHER_3DES,
                                    .cipher_mode = SAM_CIPHER_CBC,
                                    .max_key_len = BITS2BYTES(192)},
    [RTE_CRYPTO_CIPHER_3DES_CTR] = {.supported = ALGO_SUPPORTED,
                                    .cipher_alg = SAM_CIPHER_3DES,
                                    .cipher_mode = SAM_CIPHER_CTR,
                                    .max_key_len = BITS2BYTES(192)},
    [RTE_CRYPTO_CIPHER_3DES_ECB] = {.supported = ALGO_SUPPORTED,
                                    .cipher_alg = SAM_CIPHER_3DES,
                                    .cipher_mode = SAM_CIPHER_ECB,
                                    .max_key_len = BITS2BYTES(192)},
    [RTE_CRYPTO_CIPHER_AES_CBC] = {.supported = ALGO_SUPPORTED,
                                   .cipher_alg = SAM_CIPHER_AES,
                                   .cipher_mode = SAM_CIPHER_CBC,
                                   .max_key_len = BITS2BYTES(256)},
    [RTE_CRYPTO_CIPHER_AES_CTR] = {.supported = ALGO_SUPPORTED,
                                   .cipher_alg = SAM_CIPHER_AES,
                                   .cipher_mode = SAM_CIPHER_CTR,
                                   .max_key_len = BITS2BYTES(256)},
    [RTE_CRYPTO_CIPHER_AES_ECB] = {.supported = ALGO_SUPPORTED,
                                   .cipher_alg = SAM_CIPHER_AES,
                                   .cipher_mode = SAM_CIPHER_ECB,
                                   .max_key_len = BITS2BYTES(256)},
};
 ...
 */
static const struct auth_params_mapping auth_map[] = {
    [RTE_CRYPTO_AUTH_NULL] = {.supported = ALGO_SUPPORTED,
                              .auth_alg = SAM_AUTH_NONE},
    [RTE_CRYPTO_AUTH_MD5_HMAC] = {.supported = ALGO_SUPPORTED,
                                  .auth_alg = SAM_AUTH_HMAC_MD5},
    [RTE_CRYPTO_AUTH_MD5] = {.supported = ALGO_SUPPORTED,
                             .auth_alg = SAM_AUTH_HASH_MD5},
    [RTE_CRYPTO_AUTH_SHA1_HMAC] = {.supported = ALGO_SUPPORTED,
                                   .auth_alg = SAM_AUTH_HMAC_SHA1},
    [RTE_CRYPTO_AUTH_SHA1] = {.supported = ALGO_SUPPORTED,
                              .auth_alg = SAM_AUTH_HASH_SHA1},
    [RTE_CRYPTO_AUTH_SHA224_HMAC] = {.supported = ALGO_SUPPORTED,
                                     .auth_alg = SAM_AUTH_HMAC_SHA2_224},
    [RTE_CRYPTO_AUTH_SHA224] = {.supported = ALGO_SUPPORTED,
                                .auth_alg = SAM_AUTH_HASH_SHA2_224},
    [RTE_CRYPTO_AUTH_SHA256_HMAC] = {.supported = ALGO_SUPPORTED,
                                     .auth_alg = SAM_AUTH_HMAC_SHA2_256},
    [RTE_CRYPTO_AUTH_SHA256] = {.supported = ALGO_SUPPORTED,
                                .auth_alg = SAM_AUTH_HASH_SHA2_256},
    [RTE_CRYPTO_AUTH_SHA384_HMAC] = {.supported = ALGO_SUPPORTED,
                                     .auth_alg = SAM_AUTH_HMAC_SHA2_384},
    [RTE_CRYPTO_AUTH_SHA384] = {.supported = ALGO_SUPPORTED,
                                .auth_alg = SAM_AUTH_HASH_SHA2_384},
    [RTE_CRYPTO_AUTH_SHA512_HMAC] = {.supported = ALGO_SUPPORTED,
                                     .auth_alg = SAM_AUTH_HMAC_SHA2_512},
    [RTE_CRYPTO_AUTH_SHA512] = {.supported = ALGO_SUPPORTED,
                                .auth_alg = SAM_AUTH_HASH_SHA2_512},
    [RTE_CRYPTO_AUTH_AES_GMAC] = {.supported = ALGO_SUPPORTED,
 ...
 */
static const struct cipher_params_mapping aead_map[] = {
    [RTE_CRYPTO_AEAD_AES_GCM] = {.supported = ALGO_SUPPORTED,
                                 .cipher_alg = SAM_CIPHER_AES,
 ...
 */
static int mrvl_crypto_set_cipher_session_parameters(
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_crypto_set_auth_session_parameters(
    struct mrvl_crypto_session *sess,
  ...
  sess->sam_sess_params.dir =
      (auth_xform->auth.op == RTE_CRYPTO_AUTH_OP_GENERATE) ? SAM_DIR_ENCRYPT
                                                           : SAM_DIR_DECRYPT;
  ...
  if (auth_xform->auth.key.length > 0) {
    ...
    memcpy(auth_key, auth_xform->auth.key.data, auth_xform->auth.key.length);
  }
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int mrvl_crypto_set_aead_session_parameters(
    struct mrvl_crypto_session *sess,
  ...
  if ((aead_xform->aead.algo > RTE_DIM(aead_map)) ||
      (aead_map[aead_xform->aead.algo].supported != ALGO_SUPPORTED)) {
    MRVL_LOG(ERR, "AEAD algorithm not supported!");
    return -EINVAL;
  }

  sess->sam_sess_params.dir =
      (aead_xform->aead.op == RTE_CRYPTO_AEAD_OP_ENCRYPT) ? SAM_DIR_ENCRYPT
                                                          : SAM_DIR_DECRYPT;
  sess->sam_sess_params.cipher_alg = aead_map[aead_xform->aead.algo].cipher_alg;
  sess->sam_sess_params.cipher_mode =
  ...
  memcpy(aead_key, aead_xform->aead.key.data, aead_xform->aead.key.length);
  ...
  sess->sam_sess_params.u.basic.auth_icv_len = aead_xform->aead.digest_length;

  sess->sam_sess_params.u.basic.auth_aad_len = aead_xform->aead.aad_length;
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
int mrvl_crypto_set_session_parameters(
    struct mrvl_crypto_session *sess,
  ...
  if ((aead_xform != NULL) &&
      (mrvl_crypto_set_aead_session_parameters(sess, aead_xform) < 0)) {
...
static int replay_wsz_to_mask(uint32_t replay_win_sz) {
  int mask = 0;
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
int mrvl_ipsec_set_session_parameters(
    struct mrvl_crypto_session *sess,
  ...
  sess->sam_sess_params.u.ipsec.is_esp =
      (ipsec_xform->proto == RTE_SECURITY_IPSEC_SA_PROTO_ESP) ? 1 : 0;
  sess->sam_sess_params.u.ipsec.is_ip6 = 0;
  sess->sam_sess_params.u.ipsec.is_tunnel =
      (ipsec_xform->mode == RTE_SECURITY_IPSEC_SA_MODE_TUNNEL) ? 1 : 0;
  ...
  sess->sam_sess_params.u.ipsec.tunnel.u.ipv4.df = ipsec_xform->tunnel.ipv4.df;
  ...
  sess->sam_sess_params.u.ipsec.tunnel.copy_df = ipsec_xform->options.copy_df;
 ...
 */
static inline int mrvl_request_prepare_crp(struct sam_cio_op_params *request,
  ...
  if (unlikely(op->sess_type == RTE_CRYPTO_OP_SESSIONLESS)) {
    MRVL_LOG(ERR,
             "MRVL CRYPTO PMD only supports session "
  ...
  if (rte_pktmbuf_data_len(dst_mbuf) == 0) {
    ...
    if (rte_pktmbuf_append(dst_mbuf, rte_pktmbuf_data_len(op->sym->m_src)) ==
        NULL) {
  ...
  if (sess->chain_order == MRVL_CRYPTO_CHAIN_COMBINED) {
    ...
    request->cipher_iv =
        rte_crypto_op_ctod_offset(op, uint8_t *, sess->cipher_iv_offset);
  ...
  if (sess->sam_sess_params.dir == SAM_DIR_ENCRYPT) {
     ...
     */
    if (rte_pktmbuf_mtod_offset(dst_mbuf, uint8_t *,
    ...
    while ((last_seg->next != NULL) && (d_offset >= last_seg->data_len)) {
      d_offset -= last_seg->data_len;
      last_seg = last_seg->next;
    }

    if (rte_pktmbuf_mtod_offset(last_seg, uint8_t *, d_offset) == digest)
      return 0;

    /* copy digest to last segment */
    if (last_seg->buf_len >= (d_size + d_offset)) {
      d_ptr = (unsigned char *)last_seg->buf_addr + d_offset;
 ...
 * @param op Pointer to DPDK crypto operation struct [In].
 */
static inline int mrvl_request_prepare_sec(struct sam_cio_ipsec_params *request,
                                           struct sam_buf_info *src_bd,
                                           struct sam_buf_info *dst_bd,
                                           struct rte_crypto_op *op) {
  struct mrvl_crypto_session *sess;
 ...
 */
static uint16_t mrvl_crypto_pmd_enqueue_burst(void *queue_pair,
  ...
  for (; iter_ops < nb_ops; ++iter_ops) {
    ...
    if (ops[iter_ops]->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
      ...
      if (mrvl_request_prepare_crp(&requests_crp[to_enq_crp],
                                   src_bd[iter_ops].src_bd, &dst_bd[iter_ops],
                                   ops[iter_ops]) < 0) {
  ...
  if (to_enq_crp > 0) {
    ...
    if (ret < 0) {
      ...
      for (iter = 0; iter < to_enq_crp; ++iter)
        ops[indx_map_crp[iter]]->status = RTE_CRYPTO_OP_STATUS_ERROR;
    }
  }
  ...
  if (to_enq_sec > 0) {
    ...
    if (ret < 0) {
      ...
      for (iter = 0; iter < to_enq_crp; ++iter)
        ops[indx_map_sec[iter]]->status = RTE_CRYPTO_OP_STATUS_ERROR;
    }
  }
 ...
 */
static uint16_t mrvl_crypto_pmd_dequeue_burst(void *queue_pair,
                                              struct rte_crypto_op **ops,
                                              uint16_t nb_ops) {
  int ret;
  ...
  for (i = 0; i < nb_ops; ++i) {
    ...
    case SAM_CIO_OK:
      ...
      if (ops[i]->sess_type == RTE_CRYPTO_OP_SECURITY_SESSION) {
    ...
    case SAM_CIO_ERR_ICV:
      ...
      break;
    default:
      MRVL_LOG(DEBUG, "CIO returned Error: %d.", results[i].status);
 ...
 */
static int
cryptodev_mrvl_crypto_create(const char *name, struct rte_vdev_device *vdev,
  ...
  dev->feature_flags =
      ...
      RTE_CRYPTODEV_FF_HW_ACCELERATED | RTE_CRYPTODEV_FF_OOP_SGL_IN_LB_OUT |
      RTE_CRYPTODEV_FF_OOP_LB_IN_LB_OUT | RTE_CRYPTODEV_FF_SECURITY;
...
init_error:
  MRVL_LOG(ERR, "Driver %s: %s failed!", init_params->common.name, __func__);
...
static int parse_name_arg(const char *key __rte_unused, const char *value,
  ...
  if (strlen(value) >= RTE_CRYPTODEV_NAME_MAX_LEN - 1) {
    MRVL_LOG(ERR, "Invalid name %s, should be less than %u bytes!", value,
...
static int mrvl_pmd_parse_input_args(struct mrvl_pmd_init_params *params,
  ...
  if (input_args) {
    kvlist = rte_kvargs_parse(input_args, mrvl_pmd_valid_params);
    if (kvlist == NULL)
      return -1;

    /* Common VDEV parameters */
    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_PMD_MAX_NB_QP_ARG,
                             &parse_integer_arg,
                             &params->common.max_nb_queue_pairs);
    if (ret < 0)
      goto free_kvlist;

    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_PMD_SOCKET_ID_ARG,
                             &parse_integer_arg, &params->common.socket_id);
    if (ret < 0)
      goto free_kvlist;

    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_PMD_NAME_ARG,
                             &parse_name_arg, &params->common.name);
    if (ret < 0)
      goto free_kvlist;

    ret = rte_kvargs_process(kvlist, MRVL_PMD_MAX_NB_SESS_ARG,
 ...
 * @returns 0 in case of success, negative value otherwise.
 */
static int cryptodev_mrvl_crypto_init(struct rte_vdev_device *vdev) {
  struct mrvl_pmd_init_params init_params = {
      .common = {.name = "",
                 .private_data_size = sizeof(struct mrvl_crypto_private),
                 .max_nb_queue_pairs = sam_get_num_inst() * sam_get_num_cios(0),
  ...
  if (ret) {
    MRVL_LOG(ERR, "Failed to parse initialisation arguments[%s]!", args);
    return -EINVAL;
  }
...
static int cryptodev_mrvl_crypto_uninit(struct rte_vdev_device *vdev) {
  ...
  MRVL_LOG(INFO, "Closing Marvell crypto device %s on numa socket %u.", name,
           rte_socket_id());
