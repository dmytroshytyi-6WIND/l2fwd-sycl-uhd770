 ...
 *  ----------------------
 */
static void create_se_instr(struct nitrox_softreq *sr, uint8_t qno) {
...
static void softreq_copy_iv(struct nitrox_softreq *sr, uint8_t salt_size) {
...
static void fill_sglist(struct nitrox_sgtable *sgtbl, uint16_t len,
                        rte_iova_t iova, void *virt) {
...
static int create_sglist_from_mbuf(struct nitrox_sgtable *sgtbl,
  ...
  for (m = m->next; m && datalen; m = m->next) {
    mlen =
        rte_pktmbuf_data_len(m) < datalen ? rte_pktmbuf_data_len(m) : datalen;
    sglist[cnt].len = mlen;
...
static void create_sgcomp(struct nitrox_sgtable *sgtbl) {
  int i, j, nr_sgcomp;
...
static int create_cipher_inbuf(struct nitrox_softreq *sr) {
  int err;
...
static int create_cipher_outbuf(struct nitrox_softreq *sr) {
  struct rte_crypto_op *op = sr->op;
  int err, cnt = 0;
  struct rte_mbuf *m_dst = op->sym->m_dst ? op->sym->m_dst : op->sym->m_src;

  sr->resp.orh = PENDING_SIG;
  sr->out.sglist[cnt].len = sizeof(sr->resp.orh);
  sr->out.sglist[cnt].iova =
      sr->iova + offsetof(struct nitrox_softreq, resp.orh);
  sr->out.sglist[cnt].virt = &sr->resp.orh;
  cnt++;

  sr->out.map_bufs_cnt = cnt;
  fill_sglist(&sr->out, sr->iv.len, sr->iv.iova, sr->iv.virt);
  err = create_sglist_from_mbuf(&sr->out, m_dst, op->sym->cipher.data.offset,
                                op->sym->cipher.data.length);
  if (unlikely(err))
    return err;
  ...
  sr->out.sglist[cnt].iova =
      sr->iova + offsetof(struct nitrox_softreq, resp.completion);
...
static void create_cipher_gph(uint32_t cryptlen, uint16_t ivlen,
                              struct gphdr *gph) {
...
static int process_cipher_data(struct nitrox_softreq *sr) {
  struct rte_crypto_op *op = sr->op;
  int err;
...
static int extract_cipher_auth_digest(struct nitrox_softreq *sr,
                                      struct nitrox_sglist *digest) {
  struct rte_crypto_op *op = sr->op;
  struct rte_mbuf *mdst = op->sym->m_dst ? op->sym->m_dst : op->sym->m_src;
  ...
  digest->iova = rte_pktmbuf_iova_offset(mdst, op->sym->auth.data.offset +
                                                   op->sym->auth.data.length);
  digest->virt = rte_pktmbuf_mtod_offset(
      mdst, uint8_t *, op->sym->auth.data.offset + op->sym->auth.data.length);
  return 0;
}

static int create_cipher_auth_sglist(struct nitrox_softreq *sr,
                                     struct nitrox_sgtable *sgtbl,
  ...
  if (unlikely(op->sym->cipher.data.offset + op->sym->cipher.data.length !=
...
static int create_combined_sglist(struct nitrox_softreq *sr,
                                  struct nitrox_sgtable *sgtbl,
                                  struct rte_mbuf *mbuf) {
  struct rte_crypto_op *op = sr->op;
...
static int create_aead_sglist(struct nitrox_softreq *sr,
                              struct nitrox_sgtable *sgtbl,
                              struct rte_mbuf *mbuf) {
  int err;
...
static int create_aead_inbuf(struct nitrox_softreq *sr,
                             struct nitrox_sglist *digest) {
  int err;
  struct nitrox_crypto_ctx *ctx = sr->ctx;
...
static int create_aead_oop_outbuf(struct nitrox_softreq *sr,
                                  struct nitrox_sglist *digest) {
  int err;
  struct nitrox_crypto_ctx *ctx = sr->ctx;
...
static void create_aead_inplace_outbuf(struct nitrox_softreq *sr,
                                       struct nitrox_sglist *digest) {
  int i, cnt;
  struct nitrox_crypto_ctx *ctx = sr->ctx;
...
static int create_aead_outbuf(struct nitrox_softreq *sr,
                              struct nitrox_sglist *digest) {
  struct rte_crypto_op *op = sr->op;
  int cnt = 0;
...
static void create_aead_gph(uint32_t cryptlen, uint16_t ivlen, uint32_t authlen,
                            struct gphdr *gph) {
  int auth_only_len;
  union {
    struct {
...
static int process_cipher_auth_data(struct nitrox_softreq *sr) {
  struct rte_crypto_op *op = sr->op;
  int err;
  struct nitrox_sglist digest;
...
static int softreq_copy_salt(struct nitrox_softreq *sr) {
  struct nitrox_crypto_ctx *ctx = sr->ctx;
  uint8_t *addr;
...
static int extract_combined_digest(struct nitrox_softreq *sr,
                                   struct nitrox_sglist *digest) {
  struct rte_crypto_op *op = sr->op;
  struct rte_mbuf *mdst = op->sym->m_dst ? op->sym->m_dst : op->sym->m_src;
  ...
  digest->iova = rte_pktmbuf_iova_offset(mdst, op->sym->aead.data.offset +
                                                   op->sym->aead.data.length);
  digest->virt = rte_pktmbuf_mtod_offset(
      mdst, uint8_t *, op->sym->aead.data.offset + op->sym->aead.data.length);

  return 0;
}

static int process_combined_data(struct nitrox_softreq *sr) {
  int err;
  struct nitrox_sglist digest;
  struct rte_crypto_op *op = sr->op;
  ...
  create_aead_gph(op->sym->aead.data.length, sr->iv.len,
                  op->sym->aead.data.length + sr->ctx->aad_length, &sr->gph);

  return 0;
}

static int process_softreq(struct nitrox_softreq *sr) {
  struct nitrox_crypto_ctx *ctx = sr->ctx;
  int err = 0;
...
void *nitrox_sym_instr_addr(struct nitrox_softreq *sr) { return &sr->instr; }

static void req_pool_obj_init(__rte_unused struct rte_mempool *mp,
...
struct rte_mempool *nitrox_sym_req_pool_create(struct rte_cryptodev *cdev,
  ...
  snprintf(softreq_pool_name, RTE_RING_NAMESIZE, "%s_sr_%d", cdev->data->name,
           qp_id);
  mp = rte_mempool_create(softreq_pool_name, RTE_ALIGN_MUL_CEIL(nobjs, 64),
                          sizeof(struct nitrox_softreq), 64, 0, NULL, NULL,
                          req_pool_obj_init, NULL, socket_id, 0);
  if (unlikely(!mp))
...
void nitrox_sym_req_pool_free(struct rte_mempool *mp) { rte_mempool_free(mp); }
