...
static struct nitrox_device *ndev_allocate(struct rte_pci_device *pdev) {
  struct nitrox_device *ndev;

  ndev = rte_zmalloc_socket("nitrox device", sizeof(*ndev), RTE_CACHE_LINE_SIZE,
...
static void ndev_init(struct nitrox_device *ndev, struct rte_pci_device *pdev) {
...
static struct nitrox_device *find_ndev(struct rte_pci_device *pdev) {
  struct nitrox_device *ndev;
...
static void ndev_release(struct nitrox_device *ndev) {
  if (!ndev)
    return;
...
static int nitrox_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
...
static int nitrox_pci_remove(struct rte_pci_device *pdev) {
  struct nitrox_device *ndev;
  int err;
