...
static const struct rte_driver nitrox_rte_sym_drv = {
    .name = nitrox_sym_drv_name, .alias = nitrox_sym_drv_name};

static int nitrox_sym_dev_qp_release(struct rte_cryptodev *cdev,
                                     uint16_t qp_id);

static int nitrox_sym_dev_config(struct rte_cryptodev *cdev,
...
static int nitrox_sym_dev_start(struct rte_cryptodev *cdev) {
  /* SE cores initialization is done in PF */
  RTE_SET_USED(cdev);
  return 0;
}

static void nitrox_sym_dev_stop(struct rte_cryptodev *cdev) {
  /* SE cores cleanup is done in PF */
  RTE_SET_USED(cdev);
}

static int nitrox_sym_dev_close(struct rte_cryptodev *cdev) {
  int i, ret;
...
static void nitrox_sym_dev_info_get(struct rte_cryptodev *cdev,
...
static void nitrox_sym_dev_stats_get(struct rte_cryptodev *cdev,
...
static void nitrox_sym_dev_stats_reset(struct rte_cryptodev *cdev) {
  int qp_id;
...
static int nitrox_sym_dev_qp_setup(struct rte_cryptodev *cdev, uint16_t qp_id,
  ...
  qp = rte_zmalloc_socket("nitrox PMD qp", sizeof(*qp), RTE_CACHE_LINE_SIZE,
                          socket_id);
  if (!qp) {
  ...
  qp->sr_mp = nitrox_sym_req_pool_create(cdev, qp->count, qp_id, socket_id);
...
static int flexi_aes_keylen(size_t keylen, bool is_aes) {
  int aes_keylen;
...
static bool crypto_key_is_valid(struct rte_crypto_cipher_xform *xform,
...
static int configure_cipher_ctx(struct rte_crypto_cipher_xform *xform,
  ...
  ctx->req_op = (xform->op == RTE_CRYPTO_CIPHER_OP_ENCRYPT) ? NITROX_OP_ENCRYPT
                                                            : NITROX_OP_DECRYPT;
...
static bool auth_key_is_valid(const uint8_t *data, uint16_t length,
                              struct flexi_crypto_context *fctx) {
...
static int configure_auth_ctx(struct rte_crypto_auth_xform *xform,
                              struct nitrox_crypto_ctx *ctx) {
  enum flexi_auth type;
  struct flexi_crypto_context *fctx = &ctx->fctx;
  ...
  if (unlikely(!auth_key_is_valid(xform->key.data, xform->key.length, fctx)))
    return -EINVAL;
...
static int configure_aead_ctx(struct rte_crypto_aead_xform *xform,
                              struct nitrox_crypto_ctx *ctx) {
  int aes_keylen;
  struct flexi_crypto_context *fctx = &ctx->fctx;
  ...
  ctx->opcode = FLEXI_CRYPTO_ENCRYPT_HMAC;
  ctx->req_op = (xform->op == RTE_CRYPTO_AEAD_OP_ENCRYPT) ? NITROX_OP_ENCRYPT
                                                          : NITROX_OP_DECRYPT;
  ctx->iv.offset = xform->iv.offset;
  ctx->iv.length = xform->iv.length;
  ctx->digest_length = xform->digest_length;
...
static int nitrox_sym_dev_sess_configure(struct rte_cryptodev *cdev,
...
static void nitrox_sym_dev_sess_clear(struct rte_cryptodev *cdev,
                                      struct rte_cryptodev_sym_session *sess) {
  struct nitrox_crypto_ctx *ctx =
      get_sym_session_private_data(sess, cdev->driver_id);
...
static struct nitrox_crypto_ctx *get_crypto_ctx(struct rte_crypto_op *op) {
  if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
    if (likely(op->sym->session))
      return get_sym_session_private_data(op->sym->session, nitrox_sym_drv_id);
  }

  return NULL;
}

static int nitrox_enq_single_op(struct nitrox_qp *qp,
...
static uint16_t nitrox_sym_dev_enq_burst(void *queue_pair,
...
static uint16_t nitrox_sym_dev_deq_burst(void *queue_pair,
                                         struct rte_crypto_op **ops,
                                         uint16_t nb_ops) {
  struct nitrox_qp *qp = queue_pair;
...
int nitrox_sym_pmd_create(struct nitrox_device *ndev) {
  ...
  cdev = rte_cryptodev_pmd_create(name, &ndev->rte_sym_dev, &init_params);
  if (!cdev) {
...
RTE_PMD_REGISTER_CRYPTO_DRIVER(nitrox_crypto_drv, nitrox_rte_sym_drv,
                               nitrox_sym_drv_id);
