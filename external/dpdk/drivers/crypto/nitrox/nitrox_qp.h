...
static inline uint16_t nitrox_qp_free_count(struct nitrox_qp *qp) {
...
static inline bool nitrox_qp_is_empty(struct nitrox_qp *qp) {
  return (rte_atomic16_read(&qp->pending_count) == 0);
}

static inline uint16_t nitrox_qp_used_count(struct nitrox_qp *qp) {
...
static inline void nitrox_ring_dbell(struct nitrox_qp *qp, uint16_t cnt) {
...
static inline void nitrox_qp_enqueue(struct nitrox_qp *qp, void *instr,
                                     struct nitrox_softreq *sr) {
  uint32_t head = qp->head % qp->count;

  qp->head++;
  memcpy(&qp->cmdq.ring[head * qp->cmdq.instr_size], instr,
         qp->cmdq.instr_size);
  qp->ridq[head].sr = sr;
  rte_smp_wmb();
...
static inline void nitrox_qp_dequeue(struct nitrox_qp *qp) {
  qp->tail++;
