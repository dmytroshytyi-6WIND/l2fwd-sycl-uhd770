...
static int nitrox_setup_cmdq(struct nitrox_qp *qp, uint8_t *bar_addr,
                             const char *dev_name, uint8_t instr_size,
                             int socket_id) {
  ...
  mz = rte_memzone_reserve_aligned(
      mz_name, cmdq_size, socket_id,
      RTE_MEMZONE_SIZE_HINT_ONLY | RTE_MEMZONE_256MB, CMDQ_PKT_IN_ALIGN);
  if (!mz) {
...
static int nitrox_setup_ridq(struct nitrox_qp *qp, int socket_id) {
  size_t ridq_size = qp->count * sizeof(*qp->ridq);

  qp->ridq = rte_zmalloc_socket("nitrox ridq", ridq_size, RTE_CACHE_LINE_SIZE,
                                socket_id);
  if (!qp->ridq) {
...
static int nitrox_release_cmdq(struct nitrox_qp *qp, uint8_t *bar_addr) {
...
int nitrox_qp_setup(struct nitrox_qp *qp, uint8_t *bar_addr,
                    ...
                    uint8_t instr_size, int socket_id) {
  int err;
  uint32_t count;
  ...
  if (count > MAX_CMD_QLEN) {
    NITROX_LOG(ERR,
               "%s: Number of descriptors too big %d,"
               " greater than max queue length %d\n",
               dev_name, count, MAX_CMD_QLEN);
    return -EINVAL;
  }
...
static void nitrox_release_ridq(struct nitrox_qp *qp) { rte_free(qp->ridq); }
