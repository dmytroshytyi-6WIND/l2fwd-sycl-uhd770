...
static inline int build_proto_compound_sg_fd(dpaa2_sec_session *sess,
  ...
  fle = (struct qbman_fle *)rte_malloc(
      NULL, FLE_SG_MEM_SIZE(mbuf->nb_segs + sym_op->m_src->nb_segs),
      RTE_CACHE_LINE_SIZE);
  if (unlikely(!fle)) {
  ...
  if (sess->ctxt_type == DPAA2_SEC_PDCP && sess->pdcp.hfn_ovd) {
    uint32_t hfn_ovd = *(uint32_t *)((uint8_t *)op + sess->pdcp.hfn_ovd_offset);
...
static inline int build_proto_compound_fd(dpaa2_sec_session *sess,
                                          struct rte_crypto_op *op,
...
static inline int build_proto_fd(dpaa2_sec_session *sess,
                                 struct rte_crypto_op *op, struct qbman_fd *fd,
                                 uint16_t bpid) {
  struct rte_crypto_sym_op *sym_op = op->sym;
  if (sym_op->m_dst)
...
static inline int build_authenc_gcm_sg_fd(dpaa2_sec_session *sess,
                                          struct rte_crypto_op *op,
                                          struct qbman_fd *fd,
  ...
  uint8_t *old_icv;
  struct rte_mbuf *mbuf;
  uint8_t *IV_ptr = rte_crypto_op_ctod_offset(op, uint8_t *, sess->iv.offset);
  ...
  DPAA2_SEC_DP_DEBUG("GCM SG: auth_off: 0x%x/length %d, digest-len=%d\n"
                     ...
                     sess->digest_length, sess->iv.length,
  ...
  op_fle->length = (sess->dir == DIR_ENC) ? (sym_op->aead.data.length + icv_len)
  ...
  if (sess->dir == DIR_ENC) {
    sge++;
    DPAA2_SET_FLE_ADDR(sge, DPAA2_VADDR_TO_IOVA(sym_op->aead.digest.data));
    sge->length = icv_len;
  }
  DPAA2_SET_FLE_FIN(sge);
  ...
  sge++;
  if (auth_only_len) {
    DPAA2_SET_FLE_ADDR(sge, DPAA2_VADDR_TO_IOVA(sym_op->aead.aad.data));
    sge->length = auth_only_len;
    sge++;
  }

  DPAA2_SET_FLE_ADDR(sge, DPAA2_MBUF_VADDR_TO_IOVA(mbuf));
  DPAA2_SET_FLE_OFFSET(sge, sym_op->aead.data.offset + mbuf->data_off);
  sge->length = mbuf->data_len - sym_op->aead.data.offset;
...
static inline int build_authenc_gcm_fd(dpaa2_sec_session *sess,
  ...
  fle->length = (sess->dir == DIR_ENC) ? (sym_op->aead.data.length + icv_len)
                                       : sym_op->aead.data.length;
  ...
  DPAA2_SET_FLE_OFFSET(sge, sym_op->aead.data.offset + sym_op->m_src->data_off);
  sge->length = sym_op->aead.data.length;
  if (sess->dir == DIR_DEC) {
    sge++;
    old_icv = (uint8_t *)(sge + 1);
    memcpy(old_icv, sym_op->aead.digest.data, sess->digest_length);
    DPAA2_SET_FLE_ADDR(sge, DPAA2_VADDR_TO_IOVA(old_icv));
    sge->length = sess->digest_length;
  }
  DPAA2_SET_FLE_FIN(sge);
...
static inline int build_authenc_sg_fd(dpaa2_sec_session *sess,
                                      struct rte_crypto_op *op,
                                      struct qbman_fd *fd,
                                      __rte_unused uint16_t bpid) {
  struct rte_crypto_sym_op *sym_op = op->sym;
  ...
  struct qbman_fle *fle, *sge, *ip_fle, *op_fle;
  struct sec_flow_context *flc;
  uint16_t auth_hdr_len = sym_op->cipher.data.offset - sym_op->auth.data.offset;
  uint16_t auth_tail_len =
      sym_op->auth.data.length - sym_op->cipher.data.length - auth_hdr_len;
  ...
  DPAA2_SEC_DP_DEBUG("AUTHENC SG: auth_off: 0x%x/length %d, digest-len=%d\n"
                     ...
                     sess->digest_length, sym_op->cipher.data.offset,
                     sym_op->cipher.data.length, sess->iv.length,
                     sym_op->m_src->data_off);
  ...
  ip_fle->length = (sess->dir == DIR_ENC)
                       ...
                       : (sym_op->auth.data.length + sess->iv.length + icv_len);
  ...
  if (sess->dir == DIR_DEC) {
    sge++;
    old_icv = (uint8_t *)(sge + 1);
    memcpy(old_icv, sym_op->auth.digest.data, icv_len);
    DPAA2_SET_FLE_ADDR(sge, DPAA2_VADDR_TO_IOVA(old_icv));
    sge->length = icv_len;
  }
...
static inline int build_authenc_fd(dpaa2_sec_session *sess,
  ...
  if (auth_only_len)
    DPAA2_SET_FLE_INTERNAL_JD(fle, auth_only_len);
  fle->length = (sess->dir == DIR_ENC) ? (sym_op->cipher.data.length + icv_len)
                                       : sym_op->cipher.data.length;
  ...
  /* Configure Output SGE for Encap/Decap */
  DPAA2_SET_FLE_ADDR(sge, DPAA2_MBUF_VADDR_TO_IOVA(dst));
  DPAA2_SET_FLE_OFFSET(sge, sym_op->cipher.data.offset + dst->data_off);
  ...
  if (sess->dir == DIR_ENC) {
    ...
    DPAA2_SET_FLE_ADDR(sge, DPAA2_VADDR_TO_IOVA(sym_op->auth.digest.data));
    sge->length = sess->digest_length;
    DPAA2_SET_FD_LEN(fd, (sym_op->auth.data.length + sess->iv.length));
  }
  DPAA2_SET_FLE_FIN(sge);
  ...
  fle->length =
      (sess->dir == DIR_ENC)
          ? (sym_op->auth.data.length + sess->iv.length)
          : (sym_op->auth.data.length + sess->iv.length + sess->digest_length);
  ...
  if (sess->dir == DIR_DEC) {
    sge++;
    old_icv = (uint8_t *)(sge + 1);
    memcpy(old_icv, sym_op->auth.digest.data, sess->digest_length);
    DPAA2_SET_FLE_ADDR(sge, DPAA2_VADDR_TO_IOVA(old_icv));
    sge->length = sess->digest_length;
    DPAA2_SET_FD_LEN(
        fd, (sym_op->auth.data.length + sess->digest_length + sess->iv.length));
  }
  DPAA2_SET_FLE_FIN(sge);
  if (auth_only_len) {
    DPAA2_SET_FLE_INTERNAL_JD(fle, auth_only_len);
...
static inline int build_auth_sg_fd(dpaa2_sec_session *sess,
                                   struct rte_crypto_op *op,
                                   struct qbman_fd *fd,
                                   __rte_unused uint16_t bpid) {
  struct rte_crypto_sym_op *sym_op = op->sym;
  ...
  mbuf = sym_op->m_src;
  fle = (struct qbman_fle *)rte_malloc(NULL, FLE_SG_MEM_SIZE(mbuf->nb_segs),
                                       RTE_CACHE_LINE_SIZE);
  if (unlikely(!fle)) {
  ...
  /* o/p fle */
  DPAA2_SET_FLE_ADDR(op_fle, DPAA2_VADDR_TO_IOVA(sym_op->auth.digest.data));
  ...
  if (sess->iv.length) {
    ...
    iv_ptr = rte_crypto_op_ctod_offset(op, uint8_t *, sess->iv.offset);
  ...
  if (data_len <= (mbuf->data_len - data_offset)) {
    ...
    /* remaining i/p segs */
    while ((data_len = data_len - sge->length) && (mbuf = mbuf->next)) {
      sge++;
      DPAA2_SET_FLE_ADDR(sge, DPAA2_MBUF_VADDR_TO_IOVA(mbuf));
      DPAA2_SET_FLE_OFFSET(sge, mbuf->data_off);
  ...
  if (sess->dir == DIR_DEC) {
    ...
    rte_memcpy(old_digest, sym_op->auth.digest.data, sess->digest_length);
...
static inline int build_auth_fd(dpaa2_sec_session *sess,
                                struct rte_crypto_op *op, struct qbman_fd *fd,
                                uint16_t bpid) {
  struct rte_crypto_sym_op *sym_op = op->sym;
  struct qbman_fle *fle, *sge;
...
static int build_cipher_sg_fd(dpaa2_sec_session *sess, struct rte_crypto_op *op,
  ...
  DPAA2_SEC_DP_DEBUG("CIPHER SG: cipher_off: 0x%x/length %d, ivlen=%d"
                     " data_off: 0x%x\n",
  ...
  DPAA2_SEC_DP_DEBUG("CIPHER SG: fdaddr =%" PRIx64 " bpid =%d meta =%d"
                     " off =%d, len =%d\n",
...
static int build_cipher_fd(dpaa2_sec_session *sess, struct rte_crypto_op *op,
  ...
  DPAA2_SEC_DP_DEBUG("CIPHER: cipher_off: 0x%x/length %d, ivlen=%d,"
                     " data_off: 0x%x\n",
                     data_offset, data_len, sess->iv.length,
                     sym_op->m_src->data_off);
  ...
  DPAA2_SEC_DP_DEBUG("CIPHER: fdaddr =%" PRIx64 " bpid =%d meta =%d"
                     " off =%d, len =%d\n",
                     DPAA2_GET_FD_ADDR(fd), DPAA2_GET_FD_BPID(fd),
                     rte_dpaa2_bpid_info[bpid].meta_data_size,
                     DPAA2_GET_FD_OFFSET(fd), DPAA2_GET_FD_LEN(fd));
...
static inline int build_sec_fd(struct rte_crypto_op *op, struct qbman_fd *fd,
                               uint16_t bpid) {
  int ret = -1;
  ...
  if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION)
    ...
    sess =
        (dpaa2_sec_session *)get_sec_session_private_data(op->sym->sec_session);
#endif
  else
    return -ENOTSUP;
...
static uint16_t dpaa2_sec_enqueue_burst(void *qp, struct rte_crypto_op **ops,
                                        uint16_t nb_ops) {
  ...
  if (!DPAA2_PER_LCORE_DPIO) {
    ...
    if (ret) {
      DPAA2_SEC_ERR("Failed to allocate IO portal, tid: %d\n", rte_gettid());
      return 0;
    }
  }
  ...
  while (nb_ops) {
    frames_to_send = (nb_ops > dpaa2_eqcr_size) ? dpaa2_eqcr_size : nb_ops;
      ...
      if (*dpaa2_seqn((*ops)->sym->m_src)) {
        if (*dpaa2_seqn((*ops)->sym->m_src) & QBMAN_ENQUEUE_FLAG_DCA) {
          ...
          DPAA2_PER_LCORE_DQRR_HELD &=
              ~(1 << *dpaa2_seqn((*ops)->sym->m_src) & QBMAN_EQCR_DCA_IDXMASK);
        }
    ...
    while (loop < frames_to_send) {
      ret = qbman_swp_enqueue_multiple(swp, &eqdesc, &fd_arr[loop],
...
static inline struct rte_crypto_op *sec_fd_to_mbuf(const struct qbman_fd *fd) {
  ...
  DPAA2_SEC_DP_DEBUG("FLE addr = %x - %x, offset = %x\n", fle->addr_hi,
                     fle->addr_lo, fle->fin_bpid_offset);
  ...
  DPAA2_SEC_DP_DEBUG(
      ...
      (void *)dst, dst->buf_addr, DPAA2_GET_FD_ADDR(fd), DPAA2_GET_FD_BPID(fd),
      rte_dpaa2_bpid_info[DPAA2_GET_FD_BPID(fd)].meta_data_size,
      DPAA2_GET_FD_OFFSET(fd), DPAA2_GET_FD_LEN(fd));
...
static void dpaa2_sec_dump(struct rte_crypto_op *op) {
  int i;
  ...
  printf("PDCP session params:\n"
         ...
         "\t%d\n\tsn_size:\t%d\n\thfn_ovd_offset:\t%d\n\thfn:\t\t%d\n"
         "\thfn_threshold:\t0x%x\n",
         sess->pdcp.domain, sess->pdcp.bearer, sess->pdcp.pkt_dir,
         sess->pdcp.hfn_ovd, sess->pdcp.sn_size, sess->pdcp.hfn_ovd_offset,
...
mbuf_dump:
  ...
  printf("Session address = %p\ncipher offset: %d, length: %d\n"
         ...
         sym_op->cipher.data.length, sym_op->auth.data.offset,
         sym_op->auth.data.length, sym_op->aead.data.offset,
         sym_op->aead.data.length);
  printf("\n");
}

static void dpaa2_sec_free_eqresp_buf(uint16_t eqresp_ci) {
...
static void dpaa2_sec_set_enqueue_descriptor(struct dpaa2_queue *dpaa2_q,
                                             struct rte_mbuf *m,
                                             struct qbman_eq_desc *eqdesc) {
  struct dpaa2_dpio_dev *dpio_dev = DPAA2_PER_LCORE_DPIO;
  ...
  if (*dpaa2_seqn(m) & DPAA2_ENQUEUE_FLAG_ORP) {
    orpid = (*dpaa2_seqn(m) & DPAA2_EQCR_OPRID_MASK) >> DPAA2_EQCR_OPRID_SHIFT;
    seqnum =
        (*dpaa2_seqn(m) & DPAA2_EQCR_SEQNUM_MASK) >> DPAA2_EQCR_SEQNUM_SHIFT;

    if (!priv->en_loose_ordered) {
      qbman_eq_desc_set_orp(eqdesc, 1, orpid, seqnum, 0);
      qbman_eq_desc_set_response(
          eqdesc,
          (uint64_t)DPAA2_VADDR_TO_IOVA(&dpio_dev->eqresp[dpio_dev->eqresp_pi]),
          1);
      ...
      dpio_dev->eqresp_pi + 1 < MAX_EQ_RESP_ENTRIES ? dpio_dev->eqresp_pi++
...
static uint16_t dpaa2_sec_enqueue_burst_ordered(void *qp,
  ...
  uint16_t bpid;
  struct rte_mempool *mb_pool;
  struct dpaa2_sec_dev_private *priv = dpaa2_qp->tx_vq.crypto_data->dev_private;
  ...
  while (nb_ops) {
    ...
    for (loop = 0; loop < frames_to_send; loop++) {
      ...
      if (*dpaa2_seqn((*ops)->sym->m_src))
        dpaa2_sec_set_enqueue_descriptor(&dpaa2_qp->tx_vq, (*ops)->sym->m_src,
                                         &eqdesc[loop]);
      else
    ...
    retry_count = 0;
    while (loop < frames_to_send) {
      ret = qbman_swp_enqueue_multiple_desc(swp, &eqdesc[loop], &fd_arr[loop],
                                            frames_to_send - loop);
      if (unlikely(ret < 0)) {
        retry_count++;
        if (retry_count > DPAA2_MAX_TX_RETRY_COUNT) {
...
static uint16_t dpaa2_sec_dequeue_burst(void *qp, struct rte_crypto_op **ops,
                                        uint16_t nb_ops) {
  ...
  qbman_pull_desc_set_numframes(
      &pulldesc, (nb_ops > dpaa2_dqrr_size) ? dpaa2_dqrr_size : nb_ops);
  ...
  while (1) {
    if (qbman_swp_pull(swp, &pulldesc)) {
      DPAA2_SEC_WARN("SEC VDQ command is not issued : QBMAN busy");
...
/** Release queue pair */
static int dpaa2_sec_queue_pair_release(struct rte_cryptodev *dev,
...
/** Setup a queue pair */
static int dpaa2_sec_queue_pair_setup(
  ...
  DPAA2_SEC_DEBUG("dev =%p, queue =%d, conf =%p", dev, qp_id, qp_conf);

  memset(&cfg, 0, sizeof(struct dpseci_rx_queue_cfg));

  qp = rte_malloc(NULL, sizeof(struct dpaa2_sec_qp), RTE_CACHE_LINE_SIZE);
  if (!qp) {
  ...
  qp->rx_vq.q_storage =
      rte_malloc("sec dq storage", sizeof(struct queue_storage_info_t),
                 RTE_CACHE_LINE_SIZE);
  if (!qp->rx_vq.q_storage) {
  ...
  retcode = dpseci_set_rx_queue(dpseci, CMD_PRI_LOW, priv->token, qp_id, &cfg);
  return retcode;
}
...
static int dpaa2_sec_cipher_init(struct rte_cryptodev *dev,
  ...
  priv = (struct ctxt_priv *)rte_zmalloc(
      NULL, sizeof(struct ctxt_priv) + sizeof(struct sec_flc_desc),
      RTE_CACHE_LINE_SIZE);
  if (priv == NULL) {
  ...
  session->cipher_key.data =
      rte_zmalloc(NULL, xform->cipher.key.length, RTE_CACHE_LINE_SIZE);
  ...
  session->dir =
      (xform->cipher.op == RTE_CRYPTO_CIPHER_OP_ENCRYPT) ? DIR_ENC : DIR_DEC;
  ...
  case RTE_CRYPTO_CIPHER_AES_CBC:
    ...
    bufsize =
        cnstr_shdsc_blkcipher(priv->flc_desc[0].desc, 1, 0, SHR_NEVER,
                              &cipherdata, session->iv.length, session->dir);
    break;
  ...
  case RTE_CRYPTO_CIPHER_SNOW3G_UEA2:
    ...
    bufsize = cnstr_shdsc_snow_f8(priv->flc_desc[0].desc, 1, 0, &cipherdata,
                                  session->dir);
    break;
  case RTE_CRYPTO_CIPHER_ZUC_EEA3:
    cipherdata.algtype = OP_ALG_ALGSEL_ZUCE;
    session->cipher_alg = RTE_CRYPTO_CIPHER_ZUC_EEA3;
    bufsize = cnstr_shdsc_zuce(priv->flc_desc[0].desc, 1, 0, &cipherdata,
                               session->dir);
    break;
  ...
  case RTE_CRYPTO_CIPHER_NULL:
    DPAA2_SEC_ERR("Crypto: Unsupported Cipher alg %u", xform->cipher.algo);
    ret = -ENOTSUP;
    goto error_out;
  default:
    DPAA2_SEC_ERR("Crypto: Undefined Cipher specified %u", xform->cipher.algo);
    ret = -ENOTSUP;
    goto error_out;
  }
...
static int dpaa2_sec_auth_init(struct rte_cryptodev *dev,
  ...
  priv = (struct ctxt_priv *)rte_zmalloc(
      NULL, sizeof(struct ctxt_priv) + 3 * sizeof(struct sec_flc_desc),
      RTE_CACHE_LINE_SIZE);
  if (priv == NULL) {
    DPAA2_SEC_ERR("No Memory for priv CTXT");
    return -ENOMEM;
  ...
  if (xform->auth.key.length) {
    session->auth_key.data =
        rte_zmalloc(NULL, xform->auth.key.length, RTE_CACHE_LINE_SIZE);
  ...
  session->dir =
      (xform->auth.op == RTE_CRYPTO_AUTH_OP_GENERATE) ? DIR_ENC : DIR_DEC;
  ...
  case RTE_CRYPTO_AUTH_SHA1_HMAC:
    ...
    bufsize =
        cnstr_shdsc_hmac(priv->flc_desc[DESC_INITFINAL].desc, 1, 0, SHR_NEVER,
                         &authdata, !session->dir, session->digest_length);
    break;
  ...
  case RTE_CRYPTO_AUTH_SNOW3G_UIA2:
    ...
    bufsize =
        cnstr_shdsc_snow_f9(priv->flc_desc[DESC_INITFINAL].desc, 1, 0,
                            &authdata, !session->dir, session->digest_length);
    break;
  case RTE_CRYPTO_AUTH_ZUC_EIA3:
    ...
    session->iv.length = xform->auth.iv.length;
    bufsize =
        cnstr_shdsc_zuca(priv->flc_desc[DESC_INITFINAL].desc, 1, 0, &authdata,
                         !session->dir, session->digest_length);
    break;
  case RTE_CRYPTO_AUTH_SHA1:
    ...
    bufsize =
        cnstr_shdsc_hash(priv->flc_desc[DESC_INITFINAL].desc, 1, 0, SHR_NEVER,
                         &authdata, !session->dir, session->digest_length);
    break;
  ...
  case RTE_CRYPTO_AUTH_AES_XCBC_MAC:
    ...
    bufsize = cnstr_shdsc_aes_mac(priv->flc_desc[DESC_INITFINAL].desc, 1, 0,
                                  SHR_NEVER, &authdata, !session->dir,
  ...
  case RTE_CRYPTO_AUTH_NULL:
    DPAA2_SEC_ERR("Crypto: Unsupported auth alg %un", xform->auth.algo);
    ret = -ENOTSUP;
    goto error_out;
  default:
    DPAA2_SEC_ERR("Crypto: Undefined Auth specified %u", xform->auth.algo);
    ret = -ENOTSUP;
    goto error_out;
  }
  ...
  int i;
  for (i = 0; i < bufsize; i++)
    DPAA2_SEC_DEBUG("DESC[%d]:0x%x", i, priv->flc_desc[DESC_INITFINAL].desc[i]);
#endif
...
static int dpaa2_sec_aead_init(struct rte_cryptodev *dev,
  ...
  session->aead_key.data =
      rte_zmalloc(NULL, aead_xform->key.length, RTE_CACHE_LINE_SIZE);
  if (session->aead_key.data == NULL && aead_xform->key.length > 0) {
    DPAA2_SEC_ERR("No Memory for aead key");
    rte_free(priv);
    return -ENOMEM;
  }
  memcpy(session->aead_key.data, aead_xform->key.data, aead_xform->key.length);
  ...
  case RTE_CRYPTO_AEAD_AES_CCM:
    DPAA2_SEC_ERR("Crypto: Unsupported AEAD alg %u", aead_xform->algo);
    ret = -ENOTSUP;
    goto error_out;
  default:
    DPAA2_SEC_ERR("Crypto: Undefined AEAD specified %u", aead_xform->algo);
    ret = -ENOTSUP;
    goto error_out;
  }
  session->dir =
      (aead_xform->op == RTE_CRYPTO_AEAD_OP_ENCRYPT) ? DIR_ENC : DIR_DEC;

  priv->flc_desc[0].desc[0] = aeaddata.keylen;
  err = rta_inline_query(IPSEC_AUTH_VAR_AES_DEC_BASE_DESC_LEN, DESC_JOB_IO_LEN,
  ...
  if (session->dir == DIR_ENC)
    bufsize = cnstr_shdsc_gcm_encap(priv->flc_desc[0].desc, 1, 0, SHR_NEVER,
                                    &aeaddata, session->iv.length,
                                    session->digest_length);
  else
    bufsize = cnstr_shdsc_gcm_decap(priv->flc_desc[0].desc, 1, 0, SHR_NEVER,
                                    &aeaddata, session->iv.length,
                                    session->digest_length);
  if (bufsize < 0) {
    DPAA2_SEC_ERR("Crypto: Invalid buffer length");
...
static int dpaa2_sec_aead_chain_init(struct rte_cryptodev *dev,
                                     struct rte_crypto_sym_xform *xform,
                                     dpaa2_sec_session *session) {
  struct dpaa2_sec_dev_private *dev_priv = dev->data->dev_private;
  ...
  if (session->ext_params.aead_ctxt.auth_cipher_text) {
    ...
    session->ctxt_type = (cipher_xform->op == RTE_CRYPTO_CIPHER_OP_ENCRYPT)
  ...
  session->cipher_key.data =
      rte_zmalloc(NULL, cipher_xform->key.length, RTE_CACHE_LINE_SIZE);
  ...
  session->auth_key.data =
      rte_zmalloc(NULL, auth_xform->key.length, RTE_CACHE_LINE_SIZE);
  ...
  case RTE_CRYPTO_AUTH_KASUMI_F9:
  case RTE_CRYPTO_AUTH_ZUC_EIA3:
    DPAA2_SEC_ERR("Crypto: Unsupported auth alg %u", auth_xform->algo);
    ret = -ENOTSUP;
    goto error_out;
  default:
    DPAA2_SEC_ERR("Crypto: Undefined Auth specified %u", auth_xform->algo);
    ret = -ENOTSUP;
    goto error_out;
  }
  cipherdata.key = (size_t)session->cipher_key.data;
  ...
  case RTE_CRYPTO_CIPHER_AES_ECB:
  case RTE_CRYPTO_CIPHER_KASUMI_F8:
    DPAA2_SEC_ERR("Crypto: Unsupported Cipher alg %u", cipher_xform->algo);
    ret = -ENOTSUP;
    goto error_out;
  default:
    DPAA2_SEC_ERR("Crypto: Undefined Cipher specified %u", cipher_xform->algo);
    ret = -ENOTSUP;
    goto error_out;
  }
  session->dir =
      (cipher_xform->op == RTE_CRYPTO_CIPHER_OP_ENCRYPT) ? DIR_ENC : DIR_DEC;
  ...
  if (session->ctxt_type == DPAA2_SEC_CIPHER_HASH) {
    bufsize = cnstr_shdsc_authenc(priv->flc_desc[0].desc, 1, 0, SHR_SERIAL,
                                  &cipherdata, &authdata, session->iv.length,
                                  session->digest_length, session->dir);
    if (bufsize < 0) {
      DPAA2_SEC_ERR("Crypto: Invalid buffer length");
      ret = -EINVAL;
      goto error_out;
...
static int dpaa2_sec_set_session_parameters(struct rte_cryptodev *dev,
                                            struct rte_crypto_sym_xform *xform,
                                            void *sess) {
  ...
  } else if (xform->type == RTE_CRYPTO_SYM_XFORM_AEAD && xform->next == NULL) {
...
#ifdef RTE_LIB_SECURITY
static int dpaa2_sec_ipsec_aead_init(struct rte_crypto_aead_xform *aead_xform,
                                     dpaa2_sec_session *session,
                                     struct alginfo *aeaddata) {
  PMD_INIT_FUNC_TRACE();
  ...
  case RTE_CRYPTO_AEAD_AES_GCM:
    ...
    case 16:
      ...
      break;
    default:
      DPAA2_SEC_ERR("Crypto: Undefined GCM digest %d", session->digest_length);
      return -EINVAL;
    }
    aeaddata->algmode = OP_ALG_AAI_GCM;
    session->aead_alg = RTE_CRYPTO_AEAD_AES_GCM;
  ...
  case RTE_CRYPTO_AEAD_AES_CCM:
    ...
    case 16:
      ...
      break;
    default:
      DPAA2_SEC_ERR("Crypto: Undefined CCM digest %d", session->digest_length);
      return -EINVAL;
    }
...
static int dpaa2_sec_ipsec_proto_init(
    struct rte_crypto_cipher_xform *cipher_xform,
    struct rte_crypto_auth_xform *auth_xform, dpaa2_sec_session *session,
  ...
  case RTE_CRYPTO_AUTH_SHA256_HMAC:
    ...
    if (session->digest_length != 16)
      DPAA2_SEC_WARN("+++Using sha256-hmac truncated len is non-standard,"
  ...
  case RTE_CRYPTO_AUTH_AES_CBC_MAC:
  case RTE_CRYPTO_AUTH_ZUC_EIA3:
    DPAA2_SEC_ERR("Crypto: Unsupported auth alg %u", session->auth_alg);
    return -ENOTSUP;
  default:
    DPAA2_SEC_ERR("Crypto: Undefined Auth specified %u", session->auth_alg);
    return -ENOTSUP;
  }
  cipherdata->key = (size_t)session->cipher_key.data;
  cipherdata->keylen = session->cipher_key.length;
  ...
  case RTE_CRYPTO_CIPHER_AES_ECB:
  case RTE_CRYPTO_CIPHER_KASUMI_F8:
    DPAA2_SEC_ERR("Crypto: Unsupported Cipher alg %u", session->cipher_alg);
    return -ENOTSUP;
  default:
    DPAA2_SEC_ERR("Crypto: Undefined Cipher specified %u", session->cipher_alg);
    return -ENOTSUP;
  }
...
static int dpaa2_sec_set_ipsec_session(struct rte_cryptodev *dev,
  ...
  if (conf->crypto_xform->type == RTE_CRYPTO_SYM_XFORM_CIPHER) {
    ...
    ret = dpaa2_sec_ipsec_proto_init(cipher_xform, auth_xform, session,
                                     &cipherdata, &authdata);
    ...
    ret = dpaa2_sec_ipsec_aead_init(aead_xform, session, &cipherdata);
    authdata.keylen = 0;
    authdata.algtype = 0;
  } else {
  ...
  if (ipsec_xform->direction == RTE_SECURITY_IPSEC_SA_DIR_EGRESS) {
    ...
    case OP_PCL_IPSEC_AES_GCM16:
      memcpy(encap_pdb.gcm.salt, (uint8_t *)&(ipsec_xform->salt), 4);
      break;
    }
    ...
    if (ipsec_xform->tunnel.type == RTE_SECURITY_IPSEC_TUNNEL_IPV4) {
      ...
      ip4_hdr.ip_sum =
          calc_chksum((uint16_t *)(void *)&ip4_hdr, sizeof(struct ip));
      hdr = (uint8_t *)&ip4_hdr;
    } else if (ipsec_xform->tunnel.type == RTE_SECURITY_IPSEC_TUNNEL_IPV6) {
      ip6_hdr.vtc_flow = rte_cpu_to_be_32(
          DPAA2_IPv6_DEFAULT_VTC_FLOW |
          ((ipsec_xform->tunnel.ipv6.dscp << RTE_IPV6_HDR_TC_SHIFT) &
           RTE_IPV6_HDR_TC_MASK) |
          ((ipsec_xform->tunnel.ipv6.flabel << RTE_IPV6_HDR_FL_SHIFT) &
           RTE_IPV6_HDR_FL_MASK));
      /* Payload length will be updated by HW */
      ip6_hdr.payload_len = 0;
      ip6_hdr.hop_limits = ipsec_xform->tunnel.ipv6.hlimit;
      ip6_hdr.proto = (ipsec_xform->proto == RTE_SECURITY_IPSEC_SA_PROTO_ESP)
                          ? IPPROTO_ESP
                          : IPPROTO_AH;
      memcpy(&ip6_hdr.src_addr, &ipsec_xform->tunnel.ipv6.src_addr, 16);
      memcpy(&ip6_hdr.dst_addr, &ipsec_xform->tunnel.ipv6.dst_addr, 16);
      encap_pdb.ip_hdr_len = sizeof(struct rte_ipv6_hdr);
      hdr = (uint8_t *)&ip6_hdr;
    }

    bufsize = cnstr_shdsc_ipsec_new_encap(
        priv->flc_desc[0].desc, 1, 0,
        (rta_sec_era >= RTA_SEC_ERA_10) ? SHR_WAIT : SHR_SERIAL, &encap_pdb,
        hdr, &cipherdata, &authdata);
  } else if (ipsec_xform->direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS) {
    ...
    case OP_PCL_IPSEC_AES_GCM12:
    case OP_PCL_IPSEC_AES_GCM16:
      memcpy(decap_pdb.gcm.salt, (uint8_t *)&(ipsec_xform->salt), 4);
      break;
    }
    ...
    session->dir = DIR_DEC;
    bufsize = cnstr_shdsc_ipsec_new_decap(
        priv->flc_desc[0].desc, 1, 0,
        (rta_sec_era >= RTA_SEC_ERA_10) ? SHR_WAIT : SHR_SERIAL, &decap_pdb,
        &cipherdata, &authdata);
  } else
    goto out;
  ...
  flc->word2_rflc_31_0 = lower_32_bits(
      (size_t) & (((struct dpaa2_sec_qp *)dev->data->queue_pairs[0])->rx_vq) |
      0x14);
...
static int dpaa2_sec_set_pdcp_session(struct rte_cryptodev *dev,
                                      struct rte_security_session_conf *conf,
                                      void *sess) {
  ...
  if (xform->type == RTE_CRYPTO_SYM_XFORM_CIPHER) {
    cipher_xform = &xform->cipher;
    if (xform->next != NULL && xform->next->type == RTE_CRYPTO_SYM_XFORM_AUTH) {
      session->ext_params.aead_ctxt.auth_cipher_text = true;
      auth_xform = &xform->next->auth;
    }
  } else if (xform->type == RTE_CRYPTO_SYM_XFORM_AUTH) {
  ...
  if (auth_xform) {
    session->auth_key.data =
        rte_zmalloc(NULL, auth_xform->key.length, RTE_CACHE_LINE_SIZE);
    if (!session->auth_key.data && auth_xform->key.length > 0) {
      DPAA2_SEC_ERR("No Memory for auth key");
      rte_free(session->cipher_key.data);
      rte_free(priv);
      return -ENOMEM;
  ...
  if (pdcp_xform->sdap_enabled) {
    int nb_keys_to_inline = rta_inline_pdcp_sdap_query(
        authdata.algtype, cipherdata.algtype, session->pdcp.sn_size,
    ...
    if (rta_inline_pdcp_query(authdata.algtype, cipherdata.algtype,
                              session->pdcp.sn_size, session->pdcp.hfn_ovd)) {
      cipherdata.key = DPAA2_VADDR_TO_IOVA(cipherdata.key);
      cipherdata.key_type = RTA_DATA_PTR;
    }
  }
  ...
  if (pdcp_xform->domain == RTE_SECURITY_PDCP_MODE_CONTROL) {
    if (session->dir == DIR_ENC)
      bufsize = cnstr_shdsc_pdcp_c_plane_encap(
          priv->flc_desc[0].desc, 1, swap, pdcp_xform->hfn,
          session->pdcp.sn_size, pdcp_xform->bearer, pdcp_xform->pkt_dir,
          pdcp_xform->hfn_threshold, &cipherdata, &authdata);
    ...
    bufsize =
        cnstr_shdsc_pdcp_short_mac(priv->flc_desc[0].desc, 1, swap, &authdata);
  } else {
    if (session->dir == DIR_ENC) {
      if (pdcp_xform->sdap_enabled)
        bufsize = cnstr_shdsc_pdcp_sdap_u_plane_encap(
            priv->flc_desc[0].desc, 1, swap, session->pdcp.sn_size,
            pdcp_xform->hfn, pdcp_xform->bearer, pdcp_xform->pkt_dir,
            pdcp_xform->hfn_threshold, &cipherdata, p_authdata);
      else
...
static int dpaa2_sec_security_session_create(
    void *dev, struct rte_security_session_conf *conf,
  ...
  case RTE_SECURITY_PROTOCOL_IPSEC:
    ret = dpaa2_sec_set_ipsec_session(cdev, conf, sess_private_data);
    break;
  case RTE_SECURITY_PROTOCOL_MACSEC:
    return -ENOTSUP;
  case RTE_SECURITY_PROTOCOL_PDCP:
    ret = dpaa2_sec_set_pdcp_session(cdev, conf, sess_private_data);
    break;
  default:
    return -EINVAL;
  }
...
#endif
static int dpaa2_sec_sym_session_configure(
...
static int dpaa2_sec_dev_start(struct rte_cryptodev *dev) {
  struct dpaa2_sec_dev_private *priv = dev->data->dev_private;
  struct fsl_mc_io *dpseci = (struct fsl_mc_io *)priv->hw;
  ...
  if (ret) {
    DPAA2_SEC_ERR("DPSECI with HW_ID = %d ENABLE FAILED", priv->hw_id);
  ...
  for (i = 0; i < attr.num_rx_queues && qp[i]; i++) {
    dpaa2_q = &qp[i]->rx_vq;
    dpseci_get_rx_queue(dpseci, CMD_PRI_LOW, priv->token, i, &rx_attr);
    dpaa2_q->fqid = rx_attr.fqid;
    DPAA2_SEC_DEBUG("rx_fqid: %d", dpaa2_q->fqid);
  }
  for (i = 0; i < attr.num_tx_queues && qp[i]; i++) {
    dpaa2_q = &qp[i]->tx_vq;
    dpseci_get_tx_queue(dpseci, CMD_PRI_LOW, priv->token, i, &tx_attr);
...
static void dpaa2_sec_dev_stop(struct rte_cryptodev *dev) {
  struct dpaa2_sec_dev_private *priv = dev->data->dev_private;
  struct fsl_mc_io *dpseci = (struct fsl_mc_io *)priv->hw;
  int ret;
  ...
  if (ret) {
    DPAA2_SEC_ERR("Failure in disabling dpseci %d device", priv->hw_id);
    return;
  }
...
static int dpaa2_sec_dev_close(struct rte_cryptodev *dev __rte_unused) {
  PMD_INIT_FUNC_TRACE();

  return 0;
}

static void dpaa2_sec_dev_infos_get(struct rte_cryptodev *dev,
...
static void dpaa2_sec_stats_get(struct rte_cryptodev *dev,
  ...
  if (ret) {
    ...
    DPAA2_SEC_INFO("dpseci hardware stats:"
                   ...
                   counters.dequeued_requests, counters.ob_enc_requests,
                   counters.ib_dec_requests, counters.ob_enc_bytes,
                   counters.ob_prot_bytes, counters.ib_dec_bytes,
                   counters.ib_valid_bytes);
  }
}

static void dpaa2_sec_stats_reset(struct rte_cryptodev *dev) {
  int i;
  struct dpaa2_sec_qp **qp = (struct dpaa2_sec_qp **)(dev->data->queue_pairs);
...
static void __rte_hot dpaa2_sec_process_parallel_event(
    struct qbman_swp *swp, const struct qbman_fd *fd,
    const struct qbman_result *dq, struct dpaa2_queue *rxq,
    struct rte_event *ev) {
  /* Prefetching mbuf */
...
static void dpaa2_sec_process_atomic_event(struct qbman_swp *swp __rte_unused,
                                           const struct qbman_fd *fd,
...
static void __rte_hot dpaa2_sec_process_ordered_event(
    struct qbman_swp *swp, const struct qbman_fd *fd,
    const struct qbman_result *dq, struct dpaa2_queue *rxq,
    struct rte_event *ev) {
  struct rte_crypto_op *crypto_op = (struct rte_crypto_op *)ev->event_ptr;
...
int dpaa2_sec_eventq_attach(const struct rte_cryptodev *dev, int qp_id,
  ...
  if (event->sched_type == RTE_SCHED_TYPE_ORDERED) {
    ...
    ret = dpseci_set_opr(dpseci, CMD_PRI_LOW, priv->token, qp_id,
                         OPR_OPT_CREATE, &ocfg);
    if (ret) {
  ...
  ret = dpseci_set_rx_queue(dpseci, CMD_PRI_LOW, priv->token, qp_id, &cfg);
  if (ret) {
...
int dpaa2_sec_eventq_detach(const struct rte_cryptodev *dev, int qp_id) {
  struct dpaa2_sec_dev_private *priv = dev->data->dev_private;
  struct fsl_mc_io *dpseci = (struct fsl_mc_io *)priv->hw;
  struct dpseci_rx_queue_cfg cfg;
  int ret;
...
static int dpaa2_sec_uninit(const struct rte_cryptodev *dev) {
  struct dpaa2_sec_dev_private *priv = dev->data->dev_private;
  struct fsl_mc_io *dpseci = (struct fsl_mc_io *)priv->hw;
  int ret;
...
static int check_devargs_handler(const char *key, const char *value,
                                 void *opaque) {
...
static void dpaa2_sec_get_devargs(struct rte_cryptodev *cryptodev,
                                  const char *key) {
  ...
  rte_kvargs_process(kvlist, key, check_devargs_handler, (void *)cryptodev);
  rte_kvargs_free(kvlist);
}

static int dpaa2_sec_dev_init(struct rte_cryptodev *cryptodev) {
  ...
  cryptodev->feature_flags =
      RTE_CRYPTODEV_FF_SYMMETRIC_CRYPTO | RTE_CRYPTODEV_FF_HW_ACCELERATED |
      RTE_CRYPTODEV_FF_SYM_OPERATION_CHAINING | RTE_CRYPTODEV_FF_SECURITY |
      RTE_CRYPTODEV_FF_SYM_RAW_DP | RTE_CRYPTODEV_FF_IN_PLACE_SGL |
      RTE_CRYPTODEV_FF_OOP_SGL_IN_SGL_OUT | RTE_CRYPTODEV_FF_OOP_SGL_IN_LB_OUT |
      RTE_CRYPTODEV_FF_OOP_LB_IN_SGL_OUT | RTE_CRYPTODEV_FF_OOP_LB_IN_LB_OUT;
  ...
  dpseci = (struct fsl_mc_io *)rte_calloc(NULL, 1, sizeof(struct fsl_mc_io), 0);
  if (!dpseci) {
    DPAA2_SEC_ERR("Error in allocating the memory for dpsec object");
    return -ENOMEM;
  }
  dpseci->regs = dpaa2_get_mcp_ptr(MC_PORTAL_INDEX);
  ...
  if (retcode != 0) {
    DPAA2_SEC_ERR("Cannot open the dpsec device: Error = %x", retcode);
    goto init_error;
  }
  retcode = dpseci_get_attributes(dpseci, CMD_PRI_LOW, token, &attr);
  if (retcode != 0) {
    DPAA2_SEC_ERR("Cannot get dpsec device attributed: Error = %x", retcode);
    goto init_error;
  }
  snprintf(cryptodev->data->name, sizeof(cryptodev->data->name), "dpsec-%u",
           hw_id);
  ...
  snprintf(str, sizeof(str), "sec_fle_pool_p%d_%d", getpid(),
           cryptodev->data->dev_id);
  internals->fle_pool = rte_mempool_create(
      (const char *)str, FLE_POOL_NUM_BUFS, FLE_POOL_BUF_SIZE,
      FLE_POOL_CACHE_SIZE, 0, NULL, NULL, NULL, NULL, SOCKET_ID_ANY, 0);
...
cryptodev_dpaa2_sec_probe(struct rte_dpaa2_driver *dpaa2_drv __rte_unused,
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    cryptodev->data->dev_private = rte_zmalloc_socket(
        "cryptodev private structure", sizeof(struct dpaa2_sec_dev_private),
...
static int cryptodev_dpaa2_sec_remove(struct rte_dpaa2_device *dpaa2_dev) {
  struct rte_cryptodev *cryptodev;
  int ret;
...
RTE_PMD_REGISTER_CRYPTO_DRIVER(dpaa2_sec_crypto_drv,
                               rte_dpaa2_sec_driver.driver,
                               cryptodev_driver_id);
RTE_PMD_REGISTER_PARAM_STRING(CRYPTODEV_NAME_DPAA2_SEC_PMD, DRIVER_STRICT_ORDER
                              "=<int>" DRIVER_DUMP_MODE "=<int>");
