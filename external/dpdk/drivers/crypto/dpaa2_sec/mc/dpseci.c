 ...
 */
int dpseci_open(struct fsl_mc_io *mc_io, uint32_t cmd_flags, int dpseci_id,
                uint16_t *token) {
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_close(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_create(struct fsl_mc_io *mc_io, uint16_t dprc_token,
                  uint32_t cmd_flags, const struct dpseci_cfg *cfg,
                  uint32_t *obj_id) {
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPSECI_CMDID_CREATE, cmd_flags, dprc_token);
 ...
 * Return:	'0' on Success; error code otherwise.
 */
int dpseci_destroy(struct fsl_mc_io *mc_io, uint16_t dprc_token,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPSECI_CMDID_DESTROY, cmd_flags, dprc_token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_enable(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_disable(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                   uint16_t token) {
  struct mc_command cmd = {0};
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_is_enabled(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_reset(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token) {
  struct mc_command cmd = {0};
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_get_attributes(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                          uint16_t token, struct dpseci_attr *attr) {
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_set_rx_queue(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                        uint16_t token, uint8_t queue,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPSECI_CMDID_SET_RX_QUEUE, cmd_flags, token);
  ...
  cmd_params->options = cpu_to_le32(cfg->options);
  dpseci_set_field(cmd_params->dest_type, DEST_TYPE, cfg->dest_cfg.dest_type);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_get_rx_queue(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                        uint16_t token, uint8_t queue,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPSECI_CMDID_GET_RX_QUEUE, cmd_flags, token);
  ...
  attr->dest_cfg.dest_type = dpseci_get_field(rsp_params->dest_type, DEST_TYPE);
  attr->order_preservation_en =
      dpseci_get_field(rsp_params->order_preservation_en, ORDER_PRESERVATION);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_get_tx_queue(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                        uint16_t token, uint8_t queue,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPSECI_CMDID_GET_TX_QUEUE, cmd_flags, token);
  cmd_params = (struct dpseci_cmd_get_queue *)cmd.params;
  cmd_params->queue = queue;
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_get_sec_attr(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                        uint16_t token, struct dpseci_sec_attr *attr) {
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPSECI_CMDID_GET_SEC_ATTR, cmd_flags, token);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_get_sec_counters(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                            uint16_t token,
  ...
  /* prepare command */
  cmd.header =
      mc_encode_cmd_header(DPSECI_CMDID_GET_SEC_COUNTERS, cmd_flags, token);
 ...
 * Return:  '0' on Success; Error code otherwise.
 */
int dpseci_get_api_version(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
                           uint16_t *major_ver, uint16_t *minor_ver) {
  struct dpseci_rsp_get_api_version *rsp_params;
  struct mc_command cmd = {0};
  int err;

  cmd.header = mc_encode_cmd_header(DPSECI_CMDID_GET_API_VERSION, cmd_flags, 0);
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpseci_set_opr(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token,
                   uint8_t index, uint8_t options, struct opr_cfg *cfg) {
  struct dpseci_cmd_set_opr *cmd_params;
  struct mc_command cmd = {0};

  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPSECI_CMDID_SET_OPR, cmd_flags, token);
 ...
 * Return:     '0' on Success; Error code otherwise.
 */
int dpseci_get_opr(struct fsl_mc_io *mc_io, uint32_t cmd_flags, uint16_t token,
                   uint8_t index, struct opr_cfg *cfg, struct opr_qry *qry) {
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPSECI_CMDID_GET_OPR, cmd_flags, token);
...
int dpseci_set_congestion_notification(
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPSECI_CMDID_SET_CONGESTION_NOTIFICATION,
                                    cmd_flags, token);

  cmd_params = (struct dpseci_cmd_set_congestion_notification *)cmd.params;
  ...
  dpseci_set_field(cmd_params->type_units, DEST_TYPE, cfg->dest_cfg.dest_type);
  dpseci_set_field(cmd_params->type_units, CG_UNITS, cfg->units);
...
int dpseci_get_congestion_notification(
  ...
  /* prepare command */
  cmd.header = mc_encode_cmd_header(DPSECI_CMDID_GET_CONGESTION_NOTIFICATION,
                                    cmd_flags, token);
  ...
  rsp_params = (struct dpseci_cmd_set_congestion_notification *)cmd.params;
  ...
  cfg->dest_cfg.dest_type = dpseci_get_field(rsp_params->type_units, DEST_TYPE);
