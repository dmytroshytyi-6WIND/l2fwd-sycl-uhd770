...
static int build_raw_dp_chain_fd(uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
                                 struct rte_crypto_sgl *dest_sgl,
                                 struct rte_crypto_va_iova_ptr *iv,
                                 struct rte_crypto_va_iova_ptr *digest,
                                 struct rte_crypto_va_iova_ptr *auth_iv,
                                 union rte_crypto_sym_ofs ofs, void *userdata,
                                 struct qbman_fd *fd) {
  RTE_SET_USED(auth_iv);

  dpaa2_sec_session *sess = ((struct dpaa2_sec_raw_dp_ctx *)drv_ctx)->session;
  ...
  fle = (struct qbman_fle *)rte_malloc(NULL, FLE_SG_MEM_SIZE(2 * sgl->num),
                                       RTE_CACHE_LINE_SIZE);
  if (unlikely(!fle)) {
  ...
  op_fle->length = (sess->dir == DIR_ENC) ? (cipher_len + icv_len) : cipher_len;
  ...
  ip_fle->length = (sess->dir == DIR_ENC)
                       ...
                       : (auth_len + sess->iv.length + icv_len);
...
static int build_raw_dp_aead_fd(uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
                                struct rte_crypto_sgl *dest_sgl,
                                struct rte_crypto_va_iova_ptr *iv,
                                struct rte_crypto_va_iova_ptr *digest,
                                struct rte_crypto_va_iova_ptr *auth_iv,
  ...
  op_fle->length = (sess->dir == DIR_ENC) ? (aead_len + icv_len) : aead_len;
  ...
  ip_fle->length = (sess->dir == DIR_ENC)
                       ...
                       : (aead_len + sess->iv.length + auth_only_len + icv_len);
...
static int build_raw_dp_auth_fd(uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
                                struct rte_crypto_sgl *dest_sgl,
                                struct rte_crypto_va_iova_ptr *iv,
                                struct rte_crypto_va_iova_ptr *digest,
                                struct rte_crypto_va_iova_ptr *auth_iv,
  ...
  /* o/p fle */
  DPAA2_SET_FLE_ADDR(op_fle, DPAA2_VADDR_TO_IOVA(digest->va));
  ...
  if (sess->iv.length) {
    ...
    iv_ptr = rte_crypto_op_ctod_offset(userdata, uint8_t *, sess->iv.offset);
  ...
  if (sess->dir == DIR_DEC) {
    ...
    rte_memcpy(old_digest, digest->va, sess->digest_length);
...
static int build_raw_dp_proto_fd(uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
                                 struct rte_crypto_sgl *dest_sgl,
                                 struct rte_crypto_va_iova_ptr *iv,
                                 struct rte_crypto_va_iova_ptr *digest,
                                 struct rte_crypto_va_iova_ptr *auth_iv,
  ...
  if (sess->ctxt_type == DPAA2_SEC_PDCP && sess->pdcp.hfn_ovd) {
    uint32_t hfn_ovd =
        *(uint32_t *)((uint8_t *)userdata + sess->pdcp.hfn_ovd_offset);
    /* enable HFN override */
...
static int build_raw_dp_cipher_fd(uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
                                  struct rte_crypto_sgl *dest_sgl,
                                  struct rte_crypto_va_iova_ptr *iv,
                                  struct rte_crypto_va_iova_ptr *digest,
                                  struct rte_crypto_va_iova_ptr *auth_iv,
  ...
  DPAA2_SEC_DP_DEBUG(
      ...
      fle, fle->addr_hi, fle->addr_lo, fle->length);
...
static __rte_always_inline uint32_t dpaa2_sec_raw_enqueue_burst(
  ...
  while (vec->num) {
    frames_to_send = (vec->num > dpaa2_eqcr_size) ? dpaa2_eqcr_size : vec->num;

    for (loop = 0; loop < frames_to_send; loop++) {
      /*Clear the unused FD fields before sending*/
      memset(&fd_arr[loop], 0, sizeof(struct qbman_fd));
      ret = sess->build_raw_dp_fd(drv_ctx, &vec->src_sgl[loop],
                                  &vec->dest_sgl[loop], &vec->iv[loop],
                                  &vec->digest[loop], &vec->auth_iv[loop], ofs,
                                  user_data[loop], &fd_arr[loop]);
      if (ret) {
...
static __rte_always_inline int dpaa2_sec_raw_enqueue(
...
static inline void *sec_fd_to_userdata(const struct qbman_fd *fd) {
...
static __rte_always_inline uint32_t dpaa2_sec_raw_dequeue_burst(
  ...
  while (!is_last) {
    ...
    if (unlikely(fd->simple.frc)) {
      ...
      DPAA2_SEC_ERR("SEC returned Error - %x", fd->simple.frc);
      is_success = false;
    } else {
      is_success = true;
    }
...
int dpaa2_sec_configure_raw_dp_ctx(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  if (sess_type == RTE_CRYPTO_OP_SECURITY_SESSION)
    sess =
        (dpaa2_sec_session *)get_sec_session_private_data(session_ctx.sec_sess);
...
int dpaa2_sec_get_dp_ctx_size(__rte_unused struct rte_cryptodev *dev) {
