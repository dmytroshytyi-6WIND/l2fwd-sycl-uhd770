...
#define IPSEC_MB_LOG(level, fmt, ...)                                          \
  rte_log(RTE_LOG_##level, ipsec_mb_logtype_driver, "%s() line %u: " fmt "\n", \
...
static __rte_always_inline IMB_MGR *alloc_init_mb_mgr(void) {
...
static __rte_always_inline IMB_MGR *get_per_thread_mb_mgr(void) {
...
int ipsec_mb_create(struct rte_vdev_device *vdev,
                    enum ipsec_mb_pmd_types pmd_type);

/** Device remove function */
int ipsec_mb_remove(struct rte_vdev_device *vdev);
...
typedef int (*ipsec_mb_session_configure_t)(
    IMB_MGR *mbr_mgr, void *session_private,
...
int ipsec_mb_config(struct rte_cryptodev *dev,
                    struct rte_cryptodev_config *config);

int ipsec_mb_start(struct rte_cryptodev *dev);

void ipsec_mb_stop(struct rte_cryptodev *dev);

int ipsec_mb_close(struct rte_cryptodev *dev);
...
ipsec_mb_get_session_private(struct ipsec_mb_qp *qp, struct rte_crypto_op *op) {
  ...
  case RTE_CRYPTO_OP_WITH_SESSION:
    if (likely(sym_op->session != NULL))
      sess = get_sym_session_private_data(sym_op->session, driver_id);
    break;
  case RTE_CRYPTO_OP_SESSIONLESS:
    if (!qp->sess_mp || rte_mempool_get(qp->sess_mp, (void **)&_sess))
      return NULL;

    if (!qp->sess_mp_priv ||
        rte_mempool_get(qp->sess_mp_priv, (void **)&_sess_private_data))
      return NULL;

    sess = _sess_private_data;
    if (unlikely(pmd_data->session_configure(qp->mb_mgr, sess, sym_op->xform) !=
                 0)) {
