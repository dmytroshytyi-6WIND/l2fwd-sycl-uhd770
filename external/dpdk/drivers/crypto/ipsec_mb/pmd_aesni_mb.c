 ...
 */
static void calculate_auth_precomputes(hash_one_block_t one_block_hash,
...
static inline int is_aead_algo(IMB_HASH_ALG hash_alg,
                               IMB_CIPHER_MODE cipher_mode) {
  return (hash_alg == IMB_AUTH_CHACHA20_POLY1305 ||
          hash_alg == IMB_AUTH_AES_CCM ||
          (hash_alg == IMB_AUTH_AES_GMAC && cipher_mode == IMB_CIPHER_GCM));
}
...
aesni_mb_set_session_auth_parameters(const IMB_MGR *mb_mgr,
  ...
  if (xform->auth.algo == RTE_CRYPTO_AUTH_AES_XCBC_MAC) {
    ...
    IMB_AES_XCBC_KEYEXP(mb_mgr, xform->auth.key.data,
                        sess->auth.xcbc.k1_expanded, sess->auth.xcbc.k2,
                        sess->auth.xcbc.k3);
    return 0;
  }
  ...
  if (xform->auth.algo == RTE_CRYPTO_AUTH_AES_CMAC) {
    ...
    IMB_AES_KEYEXP_128(mb_mgr, xform->auth.key.data, sess->auth.cmac.expkey,
                       dust);
  ...
  if (xform->auth.algo == RTE_CRYPTO_AUTH_AES_GMAC) {
    ...
    if (sess->auth.req_digest_len > get_digest_byte_length(IMB_AUTH_AES_GMAC)) {
      IPSEC_MB_LOG(ERR, "Invalid digest size\n");
      return -EINVAL;
    }
    sess->auth.gen_digest_len = sess->auth.req_digest_len;
    ...
    case IMB_KEY_128_BYTES:
      IMB_AES128_GCM_PRE(mb_mgr, xform->auth.key.data, &sess->cipher.gcm_key);
      sess->cipher.key_length_in_bytes = IMB_KEY_128_BYTES;
      break;
    case IMB_KEY_192_BYTES:
      IMB_AES192_GCM_PRE(mb_mgr, xform->auth.key.data, &sess->cipher.gcm_key);
      sess->cipher.key_length_in_bytes = IMB_KEY_192_BYTES;
      break;
    case IMB_KEY_256_BYTES:
      IMB_AES256_GCM_PRE(mb_mgr, xform->auth.key.data, &sess->cipher.gcm_key);
  ...
  case RTE_CRYPTO_AUTH_SHA1_HMAC:
    ...
    if (xform->auth.key.length > get_auth_algo_blocksize(IMB_AUTH_HMAC_SHA_1)) {
      IMB_SHA1(mb_mgr, xform->auth.key.data, xform->auth.key.length,
               hashed_key);
  ...
  case RTE_CRYPTO_AUTH_SHA224_HMAC:
    ...
    if (xform->auth.key.length >
        get_auth_algo_blocksize(IMB_AUTH_HMAC_SHA_224)) {
      IMB_SHA224(mb_mgr, xform->auth.key.data, xform->auth.key.length,
                 hashed_key);
      key_larger_block_size = 1;
    }
    break;
  ...
  case RTE_CRYPTO_AUTH_SHA256_HMAC:
    ...
    if (xform->auth.key.length >
        get_auth_algo_blocksize(IMB_AUTH_HMAC_SHA_256)) {
      IMB_SHA256(mb_mgr, xform->auth.key.data, xform->auth.key.length,
                 hashed_key);
      key_larger_block_size = 1;
    }
    break;
  ...
  case RTE_CRYPTO_AUTH_SHA384_HMAC:
    ...
    if (xform->auth.key.length >
        get_auth_algo_blocksize(IMB_AUTH_HMAC_SHA_384)) {
      IMB_SHA384(mb_mgr, xform->auth.key.data, xform->auth.key.length,
                 hashed_key);
      key_larger_block_size = 1;
    }
    break;
  ...
  case RTE_CRYPTO_AUTH_SHA512_HMAC:
    ...
    if (xform->auth.key.length >
        get_auth_algo_blocksize(IMB_AUTH_HMAC_SHA_512)) {
      IMB_SHA512(mb_mgr, xform->auth.key.data, xform->auth.key.length,
                 hashed_key);
      key_larger_block_size = 1;
    }
    break;
  case RTE_CRYPTO_AUTH_SHA512:
    ...
    break;
  default:
    IPSEC_MB_LOG(ERR, "Unsupported authentication algorithm selection");
    return -ENOTSUP;
  }
  uint16_t trunc_digest_size =
      get_truncated_digest_byte_length(sess->auth.algo);
  uint16_t full_digest_size = get_digest_byte_length(sess->auth.algo);
  ...
  if (key_larger_block_size) {
    calculate_auth_precomputes(hash_oneblock_fn, sess->auth.pads.inner,
                               sess->auth.pads.outer, hashed_key,
                               xform->auth.key.length,
                               get_auth_algo_blocksize(sess->auth.algo));
  } else {
    calculate_auth_precomputes(hash_oneblock_fn, sess->auth.pads.inner,
                               sess->auth.pads.outer, xform->auth.key.data,
                               xform->auth.key.length,
                               get_auth_algo_blocksize(sess->auth.algo));
  }
...
static int aesni_mb_set_session_cipher_parameters(
    const IMB_MGR *mb_mgr, struct aesni_mb_session *sess,
    const struct rte_crypto_sym_xform *xform) {
  uint8_t is_aes = 0;
  uint8_t is_3DES = 0;
  uint8_t is_docsis = 0;
  ...
  if (is_aes) {
    ...
    switch (xform->cipher.key.length) {
    case 24:
      IMB_DES_KEYSCHED(mb_mgr, keys[0], xform->cipher.key.data);
      IMB_DES_KEYSCHED(mb_mgr, keys[1], xform->cipher.key.data + 8);
      IMB_DES_KEYSCHED(mb_mgr, keys[2], xform->cipher.key.data + 16);
    ...
    if (xform->cipher.key.length != 16 && xform->cipher.key.length != 32) {
      IPSEC_MB_LOG(ERR, "Invalid cipher key length");
      return -EINVAL;
    }
    ...
    IMB_DES_KEYSCHED(mb_mgr, (uint64_t *)sess->cipher.expanded_aes_keys.encode,
                     xform->cipher.key.data);
    IMB_DES_KEYSCHED(mb_mgr, (uint64_t *)sess->cipher.expanded_aes_keys.decode,
                     xform->cipher.key.data);
  }
...
aesni_mb_set_session_aead_parameters(const IMB_MGR *mb_mgr,
  ...
  case RTE_CRYPTO_AEAD_AES_GCM:
    ...
    case IMB_KEY_128_BYTES:
      sess->cipher.key_length_in_bytes = IMB_KEY_128_BYTES;
      IMB_AES128_GCM_PRE(mb_mgr, xform->aead.key.data, &sess->cipher.gcm_key);
      break;
    case IMB_KEY_192_BYTES:
      sess->cipher.key_length_in_bytes = IMB_KEY_192_BYTES;
      IMB_AES192_GCM_PRE(mb_mgr, xform->aead.key.data, &sess->cipher.gcm_key);
      break;
    case IMB_KEY_256_BYTES:
      sess->cipher.key_length_in_bytes = IMB_KEY_256_BYTES;
      IMB_AES256_GCM_PRE(mb_mgr, xform->aead.key.data, &sess->cipher.gcm_key);
      break;
    default:
      IPSEC_MB_LOG(ERR, "Invalid cipher key length");
      return -EINVAL;
    ...
    if (sess->auth.req_digest_len == 0 || sess->auth.req_digest_len > 16) {
      IPSEC_MB_LOG(ERR, "Invalid digest size\n");
      return -EINVAL;
    }
    break;
  ...
  case RTE_CRYPTO_AEAD_CHACHA20_POLY1305:
    ...
    memcpy(sess->cipher.expanded_aes_keys.encode, xform->aead.key.data, 32);
...
static int
aesni_mb_session_configure(IMB_MGR *mb_mgr, void *priv_sess,
                           const struct rte_crypto_sym_xform *xform) {
  ...
  ret = ipsec_mb_parse_xform(xform, &mode, &auth_xform, &cipher_xform,
                             &aead_xform);
  if (ret)
    return ret;
  ...
  default:
    IPSEC_MB_LOG(ERR, "Unsupported operation chain order parameter");
    return -ENOTSUP;
  }
  ...
  ret = aesni_mb_set_session_cipher_parameters(mb_mgr, sess, cipher_xform);
  if (ret != 0) {
  ...
  if (aead_xform) {
    ret = aesni_mb_set_session_aead_parameters(mb_mgr, sess, aead_xform);
    if (ret != 0) {
      IPSEC_MB_LOG(ERR, "Invalid/unsupported aead parameters");
      return ret;
    }
  }
...
static int check_docsis_sec_session(struct rte_security_session_conf *conf) {
  ...
  if (docsis->direction == RTE_SECURITY_DOCSIS_DOWNLINK) {
    ...
    if (crypto_sym != NULL && crypto_sym->type == RTE_CRYPTO_SYM_XFORM_CIPHER &&
        crypto_sym->cipher.op == RTE_CRYPTO_CIPHER_OP_ENCRYPT &&
        crypto_sym->cipher.algo == RTE_CRYPTO_CIPHER_AES_DOCSISBPI &&
...
static int aesni_mb_set_docsis_sec_session_auth_parameters(
    struct aesni_mb_session *sess, struct rte_security_docsis_xform *xform) {
  if (xform == NULL) {
 ...
 * parameters
 */
static int aesni_mb_set_docsis_sec_session_parameters(
    __rte_unused struct rte_cryptodev *dev,
    struct rte_security_session_conf *conf, void *sess) {
  ...
  ret =
      aesni_mb_set_docsis_sec_session_auth_parameters(ipsec_sess, docsis_xform);
  if (ret != 0) {
  ...
  ret =
      aesni_mb_set_session_cipher_parameters(mb_mgr, ipsec_sess, cipher_xform);
...
auth_start_offset(struct rte_crypto_op *op, struct aesni_mb_session *session,
  ...
  if (cipher_offset > auth_offset)
    memcpy(p_dst + auth_offset, p_src + auth_offset,
           cipher_offset - auth_offset);
  ...
  if (cipher_end < auth_end)
    memcpy(p_dst + cipher_end, p_src + cipher_end, auth_end - cipher_end);
  ...
  return u_src < u_dst ? (u_dst - u_src) : (UINT64_MAX - u_src + u_dst + 1);
}

static inline void set_cpu_mb_job_params(IMB_JOB *job,
                                         struct aesni_mb_session *session,
                                         union rte_crypto_sym_ofs sofs,
                                         void *buf, uint32_t len,
                                         struct rte_crypto_va_iova_ptr *iv,
                                         struct rte_crypto_va_iova_ptr *aad,
                                         void *digest, void *udata) {
  ...
  if (job->hash_alg == IMB_AUTH_AES_GMAC &&
    ...
    job->msg_len_to_hash_in_bytes =
        len - sofs.ofs.auth.head - sofs.ofs.auth.tail;
    job->msg_len_to_cipher_in_bytes =
        len - sofs.ofs.cipher.head - sofs.ofs.cipher.tail;
  }
 ...
 */
static inline int set_mb_job_params(IMB_JOB *job, struct ipsec_mb_qp *qp,
  ...
  case IMB_AUTH_AES_GMAC:
    if (session->cipher.mode == IMB_CIPHER_GCM) {
      ...
      /* For GMAC */
      job->u.GCM.aad =
          rte_pktmbuf_mtod_offset(m_src, uint8_t *, op->sym->auth.data.offset);
  ...
  case IMB_AUTH_ZUC256_EIA3_BITLEN:
    ...
    job->u.ZUC_EIA3._iv =
        rte_crypto_op_ctod_offset(op, uint8_t *, session->auth_iv.offset);
    break;
  ...
  case IMB_AUTH_AES_CCM:
    ...
    job->iv = rte_crypto_op_ctod_offset(op, uint8_t *, session->iv.offset + 1);
    break;
  ...
  case IMB_AUTH_AES_GMAC:
    ...
    job->iv = rte_crypto_op_ctod_offset(op, uint8_t *, session->iv.offset);
    break;
  ...
  case IMB_AUTH_SNOW3G_UIA2_BITLEN:
    ...
    job->hash_start_src_offset_in_bytes = auth_start_offset(
        op, session, oop, auth_off_in_bytes, ciph_off_in_bytes,
        auth_len_in_bytes, ciph_len_in_bytes);
  ...
  default:
    job->hash_start_src_offset_in_bytes = auth_start_offset(
        op, session, oop, op->sym->auth.data.offset,
        op->sym->cipher.data.offset, op->sym->auth.data.length,
  ...
  case IMB_CIPHER_ZUC_EEA3:
    job->cipher_start_src_offset_in_bytes = op->sym->cipher.data.offset >> 3;
    job->msg_len_to_cipher_in_bytes = op->sym->cipher.data.length >> 3;
    break;
  /* ZUC and SNOW3G require length and offset in bits */
  case IMB_CIPHER_SNOW3G_UEA2_BITLEN:
  case IMB_CIPHER_KASUMI_UEA1_BITLEN:
    job->cipher_start_src_offset_in_bits = op->sym->cipher.data.offset;
 ...
 * processing.
 */
static inline int set_sec_mb_job_params(IMB_JOB *job, struct ipsec_mb_qp *qp,
                                        struct rte_crypto_op *op,
                                        uint8_t *digest_idx) {
  struct aesni_mb_qp_data *qp_data = ipsec_mb_get_qp_private_data(qp);
  struct rte_mbuf *m_src, *m_dst;
  ...
  session = (struct aesni_mb_session *)get_sec_session_private_data(
      op->sym->sec_session);
  ...
  *digest_idx = (*digest_idx + 1) % IMB_MAX_JOBS;
  ...
  /* Set data parameters */
  job->src = rte_pktmbuf_mtod(m_src, uint8_t *);
  job->dst = rte_pktmbuf_mtod_offset(m_dst, uint8_t *, sym->cipher.data.offset);
...
static inline void verify_docsis_sec_crc(IMB_JOB *job, uint8_t *status) {
  uint16_t crc_offset;
  uint8_t *crc;
...
static inline void verify_digest(IMB_JOB *job, void *digest, uint16_t len,
                                 uint8_t *status) {
...
static inline void generate_digest(IMB_JOB *job, struct rte_crypto_op *op,
                                   struct aesni_mb_session *sess) {
 ...
 */
static inline struct rte_crypto_op *post_process_mb_job(struct ipsec_mb_qp *qp,
                                                        IMB_JOB *job) {
  struct rte_crypto_op *op = (struct rte_crypto_op *)job->user_data;
  struct aesni_mb_session *sess = NULL;
  uint32_t driver_id = ipsec_mb_get_driver_id(IPSEC_MB_PMD_TYPE_AESNI_MB);
...
#endif
  {
    sess = get_sym_session_private_data(op->sym->session, driver_id);
  }
    ...
    case IMB_STATUS_COMPLETED:
      ...
      if (sess->auth.operation == RTE_CRYPTO_AUTH_OP_VERIFY) {
        if (is_aead_algo(job->hash_alg, sess->cipher.mode))
          verify_digest(job, op->sym->aead.digest.data,
                        sess->auth.req_digest_len, &op->status);
#ifdef AESNI_MB_DOCSIS_SEC_ENABLED
        else if (is_docsis_sec)
          verify_docsis_sec_crc(job, &op->status);
#endif
        else
          verify_digest(job, op->sym->auth.digest.data,
                        sess->auth.req_digest_len, &op->status);
      } else
...
static inline void post_process_mb_sync_job(IMB_JOB *job) {
  uint32_t *st;
 ...
 */
static unsigned handle_completed_jobs(struct ipsec_mb_qp *qp, IMB_MGR *mb_mgr,
...
static inline uint32_t handle_completed_sync_jobs(IMB_JOB *job,
                                                  IMB_MGR *mb_mgr) {
  uint32_t i;
...
static inline uint32_t flush_mb_sync_mgr(IMB_MGR *mb_mgr) {
  IMB_JOB *job;
...
static inline uint16_t flush_mb_mgr(struct ipsec_mb_qp *qp, IMB_MGR *mb_mgr,
  ...
  if (job)
    processed_ops += handle_completed_jobs(qp, mb_mgr, job, &ops[processed_ops],
                                           nb_ops - processed_ops);

  return processed_ops;
}

static inline IMB_JOB *set_job_null_op(IMB_JOB *job, struct rte_crypto_op *op) {
...
static uint16_t aesni_mb_dequeue_burst(void *queue_pair,
                                       struct rte_crypto_op **ops,
                                       uint16_t nb_ops) {
  ...
  do {
    ...
    if (unlikely(job == NULL)) {
      ...
      processed_jobs += flush_mb_mgr(qp, mb_mgr, &ops[processed_jobs],
...
#ifdef AESNI_MB_DOCSIS_SEC_ENABLED
    if (op->sess_type == RTE_CRYPTO_OP_SECURITY_SESSION)
      retval = set_sec_mb_job_params(job, qp, op, &digest_idx);
    else
#endif
     ...
     */
    if (job)
      processed_jobs += handle_completed_jobs(
          qp, mb_mgr, job, &ops[processed_jobs], nb_ops - processed_jobs);
  ...
  if (processed_jobs < 1)
    processed_jobs +=
        flush_mb_mgr(qp, mb_mgr, &ops[processed_jobs], nb_ops - processed_jobs);

  return processed_jobs;
}

static inline void ipsec_mb_fill_error_code(struct rte_crypto_sym_vec *vec,
                                            int32_t err) {
  uint32_t i;
...
static inline int check_crypto_sgl(union rte_crypto_sym_ofs so,
                                   const struct rte_crypto_sgl *sgl) {
...
static inline IMB_JOB *submit_sync_job(IMB_MGR *mb_mgr) {
#ifdef RTE_LIBRTE_PMD_AESNI_MB_DEBUG
...
static inline uint32_t verify_sync_dgst(struct rte_crypto_sym_vec *vec,
                                        const uint8_t dgst[][DIGEST_LENGTH_MAX],
                                        uint32_t len) {
  uint32_t i, k;
...
static uint32_t aesni_mb_process_bulk(struct rte_cryptodev *dev,
  ...
  for (i = 0, j = 0, k = 0; i != vec->num; i++) {
    ...
    set_cpu_mb_job_params(job, s, sofs, buf, len, &vec->iv[i], &vec->aad[i],
                          tmp_dgst[i], &vec->status[i]);
  ...
  if (k != 0) {
    if (s->auth.operation == RTE_CRYPTO_AUTH_OP_VERIFY)
      k = verify_sync_dgst(vec, (const uint8_t(*)[DIGEST_LENGTH_MAX])tmp_dgst,
                           s->auth.req_digest_len);
    else
      k = generate_sync_dgst(vec, (const uint8_t(*)[DIGEST_LENGTH_MAX])tmp_dgst,
                             s->auth.req_digest_len);
  }
 ...
 * configuration
 */
static int aesni_mb_pmd_sec_sess_create(void *dev,
...
static int aesni_mb_pmd_sec_sess_destroy(void *dev __rte_unused,
                                         struct rte_security_session *sess) {
...
static int aesni_mb_configure_dev(struct rte_cryptodev *dev) {
  struct rte_security_ctx *security_instance;

  security_instance = rte_malloc(
      "aesni_mb_sec", sizeof(struct rte_security_ctx), RTE_CACHE_LINE_SIZE);
...
static int aesni_mb_probe(struct rte_vdev_device *vdev) {
  return ipsec_mb_create(vdev, IPSEC_MB_PMD_TYPE_AESNI_MB);
}

static struct rte_vdev_driver cryptodev_aesni_mb_pmd_drv = {
    .probe = aesni_mb_probe, .remove = ipsec_mb_remove};
...
RTE_INIT(ipsec_mb_register_aesni_mb) {
  ...
  aesni_mb_data->feature_flags =
      RTE_CRYPTODEV_FF_SYMMETRIC_CRYPTO |
      RTE_CRYPTODEV_FF_SYM_OPERATION_CHAINING |
      RTE_CRYPTODEV_FF_OOP_LB_IN_LB_OUT | RTE_CRYPTODEV_FF_SYM_CPU_CRYPTO |
