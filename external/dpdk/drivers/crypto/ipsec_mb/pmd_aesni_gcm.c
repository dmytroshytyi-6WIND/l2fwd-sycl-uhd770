...
static void aesni_gcm_set_ops(struct aesni_gcm_ops *ops, IMB_MGR *mb_mgr) {
...
aesni_gcm_session_configure(IMB_MGR *mb_mgr, void *session,
  ...
  case IPSEC_MB_OP_AEAD_AUTHENTICATED_DECRYPT:
    ...
    if (aead_xform->aead.algo != RTE_CRYPTO_AEAD_AES_GCM) {
      IPSEC_MB_LOG(ERR, "The only combined operation supported is AES GCM");
      ret = -ENOTSUP;
      goto error_exit;
    }
    /* Set IV parameters */
    ...
    break;
  default:
    IPSEC_MB_LOG(ERR, "Wrong xform type, has to be AEAD or authentication");
    ret = -ENOTSUP;
    goto error_exit;
  }
 ...
 */
static void post_process_gcm_crypto_op(struct ipsec_mb_qp *qp,
  ...
  if (session->op == IPSEC_MB_OP_AEAD_AUTHENTICATED_DECRYPT ||
    ...
    rte_hexdump(stdout, "auth tag (orig):", digest, session->req_digest_length);
    rte_hexdump(stdout, "auth tag (calc):", tag, session->req_digest_length);
#endif
    ...
    if (session->req_digest_length != session->gen_digest_length) {
      if (session->op == IPSEC_MB_OP_AEAD_AUTHENTICATED_ENCRYPT)
        memcpy(op->sym->aead.digest.data, qp_data->temp_digest,
               session->req_digest_length);
      else
        memcpy(op->sym->auth.digest.data, qp_data->temp_digest,
               session->req_digest_length);
    }
  }
}
 ...
 *
 */
static void handle_completed_gcm_crypto_op(struct ipsec_mb_qp *qp,
                                           struct rte_crypto_op *op,
 ...
 *  0 on success
 */
static int process_gcm_crypto_op(struct ipsec_mb_qp *qp,
                                 struct rte_crypto_op *op,
                                 struct aesni_gcm_session *session) {
  struct aesni_gcm_qp_data *qp_data = ipsec_mb_get_qp_private_data(qp);
  uint8_t *src, *dst;
  ...
  part_len = (data_len < data_length) ? data_len : data_length;

  RTE_ASSERT(
      (sym_op->m_dst == NULL) ||
      ((sym_op->m_dst != NULL) && rte_pktmbuf_is_contiguous(sym_op->m_dst)));
  ...
  /* Out-of-place */
    ...
    dst = rte_pktmbuf_mtod_offset(sym_op->m_dst, uint8_t *, data_offset);
  }

  iv_ptr = rte_crypto_op_ctod_offset(op, uint8_t *, session->iv.offset);

  if (session->op == IPSEC_MB_OP_AEAD_AUTHENTICATED_ENCRYPT) {
    ops->init(&session->gdata_key, &qp_data->gcm_ctx_data, iv_ptr,
              sym_op->aead.aad.data, (uint64_t)session->aad_length);

    ops->update_enc(&session->gdata_key, &qp_data->gcm_ctx_data, dst, src,
                    (uint64_t)part_len);
    ...
    while (total_len) {
      ...
      if (oop)
        ...
        dst = src;
      part_len = (m_src->data_len < total_len) ? m_src->data_len : total_len;
    ...
    ops->finalize_enc(&session->gdata_key, &qp_data->gcm_ctx_data, tag,
                      session->gen_digest_length);
  } else if (session->op == IPSEC_MB_OP_AEAD_AUTHENTICATED_DECRYPT) {
    ops->init(&session->gdata_key, &qp_data->gcm_ctx_data, iv_ptr,
              sym_op->aead.aad.data, (uint64_t)session->aad_length);

    ops->update_dec(&session->gdata_key, &qp_data->gcm_ctx_data, dst, src,
                    (uint64_t)part_len);
    total_len = data_length - part_len;
    ...
    tag = qp_data->temp_digest;
    ops->finalize_dec(&session->gdata_key, &qp_data->gcm_ctx_data, tag,
                      session->gen_digest_length);
  } else if (session->op == IPSEC_MB_OP_HASH_GEN_ONLY) {
    ops->gmac_init(&session->gdata_key, &qp_data->gcm_ctx_data, iv_ptr,
                   session->iv.length);

    ops->gmac_update(&session->gdata_key, &qp_data->gcm_ctx_data, src,
                     (uint64_t)part_len);
    total_len = data_length - part_len;
    ...
    ops->gmac_finalize(&session->gdata_key, &qp_data->gcm_ctx_data, tag,
                       session->gen_digest_length);
...
aesni_gcm_get_session(struct ipsec_mb_qp *qp, struct rte_crypto_op *op) {
  struct aesni_gcm_session *sess = NULL;
  uint32_t driver_id = ipsec_mb_get_driver_id(IPSEC_MB_PMD_TYPE_AESNI_GCM);
  struct rte_crypto_sym_op *sym_op = op->sym;

  if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
    if (likely(sym_op->session != NULL))
      sess = (struct aesni_gcm_session *)get_sym_session_private_data(
    ...
    if (unlikely(aesni_gcm_session_configure(qp->mb_mgr, _sess_private_data,
                                             sym_op->xform) != 0)) {
...
static uint16_t aesni_gcm_pmd_dequeue_burst(void *queue_pair,
  ...
  nb_dequeued =
      rte_ring_dequeue_burst(qp->ingress_queue, (void **)ops, nb_ops, NULL);
...
static inline void aesni_gcm_fill_error_code(struct rte_crypto_sym_vec *vec,
                                             int32_t errnum) {
  uint32_t i;
...
static inline int32_t aesni_gcm_sgl_op_finalize_encryption(
    const struct aesni_gcm_session *s, struct gcm_context_data *gdata_ctx,
  ...
  if (s->req_digest_length != s->gen_digest_length) {
    ...
    ops.finalize_enc(&s->gdata_key, gdata_ctx, digest, s->gen_digest_length);
  }

  return 0;
}

static inline int32_t aesni_gcm_sgl_op_finalize_decryption(
    const struct aesni_gcm_session *s, struct gcm_context_data *gdata_ctx,
    uint8_t *digest, struct aesni_gcm_ops ops) {
  uint8_t tmpdigest[s->gen_digest_length];
  ...
  return memcmp(digest, tmpdigest, s->req_digest_length) == 0 ? 0 : EBADMSG;
}

static inline void aesni_gcm_process_gcm_sgl_op(
    const struct aesni_gcm_session *s, struct gcm_context_data *gdata_ctx,
    struct rte_crypto_sgl *sgl, void *iv, void *aad, struct aesni_gcm_ops ops) {
  uint32_t i;

  /* init crypto operation */
  ops.init(&s->gdata_key, gdata_ctx, iv, aad, (uint64_t)s->aad_length);
  ...
  for (i = 0; i < sgl->num; i++) {
    ...
    switch (s->op) {
    case IPSEC_MB_OP_AEAD_AUTHENTICATED_ENCRYPT:
      ops.update_enc(&s->gdata_key, gdata_ctx, vec->base, vec->base, vec->len);
      break;
    case IPSEC_MB_OP_AEAD_AUTHENTICATED_DECRYPT:
      ops.update_dec(&s->gdata_key, gdata_ctx, vec->base, vec->base, vec->len);
      break;
    default:
...
static inline void aesni_gcm_process_gmac_sgl_op(
    const struct aesni_gcm_session *s, struct gcm_context_data *gdata_ctx,
    struct rte_crypto_sgl *sgl, void *iv, struct aesni_gcm_ops ops) {
  ops.init(&s->gdata_key, gdata_ctx, iv, sgl->vec[0].base, sgl->vec[0].len);
}

static inline uint32_t aesni_gcm_sgl_encrypt(struct aesni_gcm_session *s,
  ...
  processed = 0;
  for (i = 0; i < vec->num; ++i) {
    aesni_gcm_process_gcm_sgl_op(s, gdata_ctx, &vec->src_sgl[i], vec->iv[i].va,
                                 vec->aad[i].va, ops);
...
static inline uint32_t aesni_gcm_sgl_decrypt(struct aesni_gcm_session *s,
                                             struct gcm_context_data *gdata_ctx,
                                             struct rte_crypto_sym_vec *vec,
                                             struct aesni_gcm_ops ops) {
  uint32_t i, processed;
...
static inline uint32_t aesni_gmac_sgl_generate(
    struct aesni_gcm_session *s, struct gcm_context_data *gdata_ctx,
    struct rte_crypto_sym_vec *vec, struct aesni_gcm_ops ops) {
  uint32_t i, processed;
  ...
  for (i = 0; i < vec->num; ++i) {
    ...
    aesni_gcm_process_gmac_sgl_op(s, gdata_ctx, &vec->src_sgl[i], vec->iv[i].va,
                                  ops);
    vec->status[i] = aesni_gcm_sgl_op_finalize_encryption(
        s, gdata_ctx, vec->digest[i].va, ops);
    processed += (vec->status[i] == 0);
...
static inline uint32_t aesni_gmac_sgl_verify(struct aesni_gcm_session *s,
                                             struct gcm_context_data *gdata_ctx,
                                             struct rte_crypto_sym_vec *vec,
                                             struct aesni_gcm_ops ops) {
  uint32_t i, processed;
...
static uint32_t aesni_gcm_process_bulk(
    struct rte_cryptodev *dev, struct rte_cryptodev_sym_session *sess,
    __rte_unused union rte_crypto_sym_ofs ofs, struct rte_crypto_sym_vec *vec) {
...
static int aesni_gcm_qp_setup(struct rte_cryptodev *dev, uint16_t qp_id,
