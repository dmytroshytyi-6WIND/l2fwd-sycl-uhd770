...
static int zuc_session_configure(__rte_unused IMB_MGR *mgr, void *zuc_sess,
  ...
  if (cipher_xform) {
    ...
    /* Copy the key */
    memcpy(sess->pKey_cipher, cipher_xform->cipher.key.data, ZUC_IV_KEY_LENGTH);
  }
  ...
  if (auth_xform) {
    ...
    /* Copy the key */
    memcpy(sess->pKey_hash, auth_xform->auth.key.data, ZUC_IV_KEY_LENGTH);
  }
...
static uint8_t process_zuc_cipher_op(struct ipsec_mb_qp *qp,
  ...
  for (i = 0; i < num_ops; i++) {
    ...
    dst[i] = ops[i]->sym->m_dst
                 ...
                 : rte_pktmbuf_mtod(ops[i]->sym->m_src, uint8_t *) +
                       (ops[i]->sym->cipher.data.offset >> 3);
    iv[i] =
        rte_crypto_op_ctod_offset(ops[i], uint8_t *, sess->cipher_iv_offset);
...
static int process_zuc_hash_op(struct ipsec_mb_qp *qp,
                               struct rte_crypto_op **ops,
                               struct zuc_session **sessions, uint8_t num_ops) {
  unsigned int i;
  uint8_t processed_ops = 0;
  ...
  for (i = 0; i < num_ops; i++) {
    ...
    iv[i] = rte_crypto_op_ctod_offset(ops[i], uint8_t *, sess->auth_iv_offset);
  ...
  for (i = 0; i < processed_ops; i++)
    if (sessions[i]->auth_op == RTE_CRYPTO_AUTH_OP_VERIFY)
      if (memcmp(dst[i], ops[i]->sym->auth.digest.data, ZUC_DIGEST_LENGTH) != 0)
...
static int process_ops(struct rte_crypto_op **ops,
                       enum ipsec_mb_operation op_type,
                       struct zuc_session **sessions, struct ipsec_mb_qp *qp,
                       uint8_t num_ops) {
  unsigned int i;
  ...
  case IPSEC_MB_OP_DECRYPT_ONLY:
    processed_ops = process_zuc_cipher_op(qp, ops, sessions, num_ops);
    break;
  case IPSEC_MB_OP_HASH_GEN_ONLY:
  case IPSEC_MB_OP_HASH_VERIFY_ONLY:
    processed_ops = process_zuc_hash_op(qp, ops, sessions, num_ops);
    break;
...
static uint16_t zuc_pmd_dequeue_burst(void *queue_pair,
                                      struct rte_crypto_op **c_ops,
                                      uint16_t nb_ops) {
  ...
  nb_dequeued =
      rte_ring_dequeue_burst(qp->ingress_queue, (void **)c_ops, nb_ops, NULL);

  for (i = 0; i < nb_dequeued; i++) {
    curr_c_op = c_ops[i];

    curr_sess =
        (struct zuc_session *)ipsec_mb_get_session_private(qp, curr_c_op);
    ...
    if (burst_size == 0) {
      ...
      if (burst_size == ZUC_MAX_BURST) {
        processed_ops =
            process_ops(int_c_ops, curr_zuc_op, sessions, qp, burst_size);
       ...
       */
      processed_ops =
          process_ops(int_c_ops, prev_zuc_op, sessions, qp, burst_size);
      if (processed_ops < burst_size) {
        burst_size = 0;
        break;
      }
...
static int zuc_probe(struct rte_vdev_device *vdev) {
  return ipsec_mb_create(vdev, IPSEC_MB_PMD_TYPE_ZUC);
}

static struct rte_vdev_driver cryptodev_zuc_pmd_drv = {.probe = zuc_probe,
...
RTE_INIT(ipsec_mb_register_zuc) {
  struct ipsec_mb_internals *zuc_data = &ipsec_mb_pmds[IPSEC_MB_PMD_TYPE_ZUC];
