...
static int snow3g_session_configure(IMB_MGR *mgr, void *priv_sess,
  ...
  int ret =
      ipsec_mb_parse_xform(xform, &mode, &auth_xform, &cipher_xform, NULL);
  if (ret)
    return ret;
...
static uint8_t *snow3g_digest_appended_in_src(struct rte_crypto_op *op) {
  unsigned int auth_size, cipher_size;

  auth_size =
      (op->sym->auth.data.offset >> 3) + (op->sym->auth.data.length >> 3);
  cipher_size =
      (op->sym->cipher.data.offset >> 3) + (op->sym->cipher.data.length >> 3);
...
static uint8_t process_snow3g_cipher_op(struct ipsec_mb_qp *qp,
  ...
  for (i = 0; i < num_ops; i++) {
    ...
    if (ops[i]->sym->m_dst && ops[i]->sym->m_src != ops[i]->sym->m_dst) {
      dst[i] =
          rte_pktmbuf_mtod_offset(ops[i]->sym->m_dst, uint8_t *, cipher_off);
      ...
      if (session->op == IPSEC_MB_OP_HASH_VERIFY_THEN_DECRYPT ||
        ...
        unencrypted_bytes = (ops[i]->sym->auth.data.offset >> 3) +
                            (ops[i]->sym->auth.data.length >> 3) +
                            (SNOW3G_DIGEST_LENGTH)-cipher_off - cipher_len;
      if (unencrypted_bytes > 0)
        rte_memcpy(rte_pktmbuf_mtod_offset(ops[i]->sym->m_dst, uint8_t *,
      ...
      dst[i] =
          rte_pktmbuf_mtod_offset(ops[i]->sym->m_src, uint8_t *, cipher_off);

    iv[i] =
        rte_crypto_op_ctod_offset(ops[i], uint8_t *, session->cipher_iv_offset);
    num_bytes[i] = cipher_len;
    processed_ops++;
  }

  IMB_SNOW3G_F8_N_BUFFER(qp->mb_mgr, &session->pKeySched_cipher, iv, src, dst,
  ...
  for (i = 0; i < num_ops; i++) {
    if ((session->op == IPSEC_MB_OP_HASH_VERIFY_THEN_DECRYPT ||
      ...
      digest_appended[i] = snow3g_digest_appended_in_src(ops[i]);
      /* Clear unencrypted digest from
       * the src buffer
       */
      if (digest_appended[i] != NULL)
        memset(digest_appended[i], 0, SNOW3G_DIGEST_LENGTH);
    }
  }
...
static uint8_t process_snow3g_cipher_op_bit(struct ipsec_mb_qp *qp,
  ...
  if (session->op == IPSEC_MB_OP_HASH_VERIFY_THEN_DECRYPT ||
    ...
    unencrypted_bytes =
        (op->sym->auth.data.offset >> 3) + (op->sym->auth.data.length >> 3) +
        (SNOW3G_DIGEST_LENGTH) - (offset_in_bits >> 3) - (length_in_bits >> 3);
  if (unencrypted_bytes > 0)
    rte_memcpy(rte_pktmbuf_mtod_offset(op->sym->m_dst, uint8_t *,
                                       (length_in_bits >> 3)),
               rte_pktmbuf_mtod_offset(op->sym->m_src, uint8_t *,
                                       (length_in_bits >> 3)),
               unencrypted_bytes);

  iv = rte_crypto_op_ctod_offset(op, uint8_t *, session->cipher_iv_offset);

  IMB_SNOW3G_F8_1_BUFFER_BIT(qp->mb_mgr, &session->pKeySched_cipher, iv, src,
                             dst, length_in_bits, offset_in_bits);

  return 1;
}

/** Generate/verify hash from mbufs with same hash key. */
static int process_snow3g_hash_op(struct ipsec_mb_qp *qp,
                                  struct rte_crypto_op **ops,
                                  struct snow3g_session *session,
                                  uint8_t num_ops) {
  uint32_t i;
  ...
  for (i = 0; i < num_ops; i++) {
    ...
    iv = rte_crypto_op_ctod_offset(ops[i], uint8_t *, session->auth_iv_offset);

    if (session->auth_op == RTE_CRYPTO_AUTH_OP_VERIFY) {
      dst = qp_data->temp_digest;
      /* Handle auth cipher verify oop case*/
      if ((session->op == IPSEC_MB_OP_ENCRYPT_THEN_HASH_GEN ||
           session->op == IPSEC_MB_OP_DECRYPT_THEN_HASH_VERIFY) &&
          ops[i]->sym->m_dst != NULL)
        src = rte_pktmbuf_mtod_offset(ops[i]->sym->m_dst, uint8_t *,
                                      ops[i]->sym->auth.data.offset >> 3);

      IMB_SNOW3G_F9_1_BUFFER(qp->mb_mgr, &session->pKeySched_hash, iv, src,
                             length_in_bits, dst);
      /* Verify digest. */
      if (memcmp(dst, ops[i]->sym->auth.digest.data, SNOW3G_DIGEST_LENGTH) != 0)
      ...
      if (digest_appended)
        rte_memcpy(ops[i]->sym->auth.digest.data, dst, SNOW3G_DIGEST_LENGTH);
    }
    processed_ops++;
  }
...
static int process_ops(struct rte_crypto_op **ops,
                       struct snow3g_session *session, struct ipsec_mb_qp *qp,
                       uint8_t num_ops) {
  uint32_t i;
  ...
  for (i = 0; i < num_ops; i++) {
    if (!rte_pktmbuf_is_contiguous(ops[i]->sym->m_src) ||
        (ops[i]->sym->m_dst != NULL &&
         !rte_pktmbuf_is_contiguous(ops[i]->sym->m_dst))) {
      IPSEC_MB_LOG(ERR,
  ...
  case IPSEC_MB_OP_DECRYPT_ONLY:
    processed_ops = process_snow3g_cipher_op(qp, ops, session, num_ops);
    break;
  case IPSEC_MB_OP_HASH_GEN_ONLY:
  case IPSEC_MB_OP_HASH_VERIFY_ONLY:
    processed_ops = process_snow3g_hash_op(qp, ops, session, num_ops);
    break;
...
static int process_op_bit(struct rte_crypto_op *op,
                          struct snow3g_session *session,
  ...
  enqueued_op = rte_ring_enqueue_burst(qp->ingress_queue, (void **)&op,
                                       processed_op, NULL);
...
static uint16_t snow3g_pmd_dequeue_burst(void *queue_pair,
                                         struct rte_crypto_op **ops,
                                         uint16_t nb_ops) {
  ...
  for (i = 0; i < nb_dequeued; i++) {
    ...
    if (unlikely(curr_sess == NULL ||
      ...
      curr_c_op->status = RTE_CRYPTO_OP_STATUS_INVALID_SESSION;
      break;
    }
    ...
    if (((curr_c_op->sym->cipher.data.length % BYTE_LEN) != 0) ||
        ((curr_c_op->sym->cipher.data.offset % BYTE_LEN) != 0)) {
      ...
      processed_ops = process_op_bit(curr_c_op, curr_sess, qp, &enqueued_ops);
      if (processed_ops != 1)
        break;
...
static int snow3g_probe(struct rte_vdev_device *vdev) {
  return ipsec_mb_create(vdev, IPSEC_MB_PMD_TYPE_SNOW3G);
}

static struct rte_vdev_driver cryptodev_snow3g_pmd_drv = {
    .probe = snow3g_probe, .remove = ipsec_mb_remove};
