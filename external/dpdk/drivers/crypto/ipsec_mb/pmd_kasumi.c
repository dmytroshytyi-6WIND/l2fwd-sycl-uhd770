...
static int kasumi_session_configure(IMB_MGR *mgr, void *priv_sess,
...
static uint8_t process_kasumi_cipher_op(struct ipsec_mb_qp *qp,
  ...
  if (processed_ops != 0)
    IMB_KASUMI_F8_N_BUFFER(qp->mb_mgr, &session->pKeySched_cipher, iv, src, dst,
...
static uint8_t process_kasumi_cipher_op_bit(struct ipsec_mb_qp *qp,
  ...
  iv_ptr = rte_crypto_op_ctod_offset(op, uint8_t *, session->cipher_iv_offset);
  iv = *((uint64_t *)(iv_ptr));
  length_in_bits = op->sym->cipher.data.length;

  IMB_KASUMI_F8_1_BUFFER_BIT(qp->mb_mgr, &session->pKeySched_cipher, iv, src,
...
static int process_kasumi_hash_op(struct ipsec_mb_qp *qp,
                                  struct rte_crypto_op **ops,
                                  struct kasumi_session *session,
                                  uint8_t num_ops) {
  unsigned int i;
  ...
  for (i = 0; i < num_ops; i++) {
    ...
    if (session->auth_op == RTE_CRYPTO_AUTH_OP_VERIFY) {
      ...
      IMB_KASUMI_F9_1_BUFFER(qp->mb_mgr, &session->pKeySched_hash, src,
                             num_bytes, dst);

      /* Verify digest. */
      if (memcmp(dst, ops[i]->sym->auth.digest.data, KASUMI_DIGEST_LENGTH) != 0)
...
static int process_ops(struct rte_crypto_op **ops,
                       struct kasumi_session *session, struct ipsec_mb_qp *qp,
                       uint8_t num_ops) {
  unsigned int i;
  ...
  case IPSEC_MB_OP_DECRYPT_ONLY:
    processed_ops = process_kasumi_cipher_op(qp, ops, session, num_ops);
    break;
  case IPSEC_MB_OP_HASH_GEN_ONLY:
  case IPSEC_MB_OP_HASH_VERIFY_ONLY:
    processed_ops = process_kasumi_hash_op(qp, ops, session, num_ops);
    break;
...
static uint16_t kasumi_pmd_dequeue_burst(void *queue_pair,
                                         struct rte_crypto_op **ops,
                                         uint16_t nb_ops) {
  ...
  for (i = 0; i < nb_dequeued; i++) {
    ...
    if (!rte_pktmbuf_is_contiguous(curr_c_op->sym->m_src) ||
        (curr_c_op->sym->m_dst != NULL &&
         !rte_pktmbuf_is_contiguous(curr_c_op->sym->m_dst))) {
      IPSEC_MB_LOG(ERR,
                   "PMD supports only contiguous mbufs, op (%p) provides "
                   "noncontiguous mbuf as source/destination buffer.",
                   curr_c_op);
    ...
    curr_sess =
        (struct kasumi_session *)ipsec_mb_get_session_private(qp, curr_c_op);
...
static int kasumi_probe(struct rte_vdev_device *vdev) {
  return ipsec_mb_create(vdev, IPSEC_MB_PMD_TYPE_KASUMI);
}

static struct rte_vdev_driver cryptodev_kasumi_pmd_drv = {
    .probe = kasumi_probe, .remove = ipsec_mb_remove};
