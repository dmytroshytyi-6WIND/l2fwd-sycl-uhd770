...
/** Configure device */
int ipsec_mb_config(__rte_unused struct rte_cryptodev *dev,
                    __rte_unused struct rte_cryptodev_config *config) {
  return 0;
}

/** Start device */
int ipsec_mb_start(__rte_unused struct rte_cryptodev *dev) { return 0; }

/** Stop device */
void ipsec_mb_stop(__rte_unused struct rte_cryptodev *dev) {}

/** Close device */
int ipsec_mb_close(__rte_unused struct rte_cryptodev *dev) { return 0; }

/** Get device statistics */
void ipsec_mb_stats_get(struct rte_cryptodev *dev,
...
void ipsec_mb_stats_reset(struct rte_cryptodev *dev) {
  int qp_id;
...
/** Get device info */
void ipsec_mb_info_get(struct rte_cryptodev *dev,
                       struct rte_cryptodev_info *dev_info) {
  struct ipsec_mb_dev_private *internals = dev->data->dev_private;
  struct ipsec_mb_internals *pmd_info = &ipsec_mb_pmds[internals->pmd_type];
...
/** Release queue pair */
int ipsec_mb_qp_release(struct rte_cryptodev *dev, uint16_t qp_id) {
  struct ipsec_mb_qp *qp = dev->data->queue_pairs[qp_id];
...
int ipsec_mb_qp_set_unique_name(struct rte_cryptodev *dev,
                                struct ipsec_mb_qp *qp) {
  uint32_t n = snprintf(qp->name, sizeof(qp->name), "ipsec_mb_pmd_%u_qp_%u",
...
ipsec_mb_qp_create_processed_ops_ring(struct ipsec_mb_qp *qp,
  ...
  if (r) {
    if (rte_ring_get_size(r) >= ring_size) {
      IPSEC_MB_LOG(INFO, "Reusing existing ring %s for processed ops",
                   ring_name);
      return r;
    }
    IPSEC_MB_LOG(ERR, "Unable to reuse existing ring %s for processed ops",
                 ring_name);
    return NULL;
  }
...
ipsec_mb_alloc_mgr_from_memzone(const struct rte_memzone **mb_mgr_mz,
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    *mb_mgr_mz = rte_memzone_lookup(mb_mgr_mz_name);
    if (*mb_mgr_mz == NULL) {
      *mb_mgr_mz = rte_memzone_reserve(mb_mgr_mz_name, imb_get_mb_mgr_size(),
                                       rte_socket_id(), 0);
    }
    if (*mb_mgr_mz == NULL) {
      IPSEC_MB_LOG(DEBUG, "Error allocating memzone for %s", mb_mgr_mz_name);
      return NULL;
    }
    ...
    *mb_mgr_mz = rte_memzone_lookup(mb_mgr_mz_name);
    if (*mb_mgr_mz == NULL) {
      IPSEC_MB_LOG(ERR, "Secondary can't find %s mz, did primary create it?",
                   mb_mgr_mz_name);
      return NULL;
    }
...
/** Setup a queue pair */
int ipsec_mb_qp_setup(struct rte_cryptodev *dev, uint16_t qp_id,
                      const struct rte_cryptodev_qp_conf *qp_conf,
                      int socket_id) {
  struct ipsec_mb_qp *qp = NULL;
  struct ipsec_mb_dev_private *internals = dev->data->dev_private;
  struct ipsec_mb_internals *pmd_data = &ipsec_mb_pmds[internals->pmd_type];
  uint32_t qp_size;
  int ret;
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ...
    IPSEC_MB_LOG(
        ERR,
        "The intel-ipsec-mb version (%s) does not support multiprocess,"
  ...
  snprintf(mz_name, sizeof(mz_name), "IMB_MGR_DEV_%d_QP_%d", dev->data->dev_id,
           qp_id);
  qp->mb_mgr = ipsec_mb_alloc_mgr_from_memzone(&(qp->mb_mgr_mz), mz_name);
#endif
  if (qp->mb_mgr == NULL) {
    ret = -ENOMEM;
  ...
  qp->ingress_queue = ipsec_mb_qp_create_processed_ops_ring(
...
unsigned ipsec_mb_sym_session_get_size(struct rte_cryptodev *dev) {
  struct ipsec_mb_dev_private *internals = dev->data->dev_private;
  struct ipsec_mb_internals *pmd_data = &ipsec_mb_pmds[internals->pmd_type];
...
int ipsec_mb_sym_session_configure(struct rte_cryptodev *dev,
...
void ipsec_mb_sym_session_clear(struct rte_cryptodev *dev,
                                struct rte_cryptodev_sym_session *sess) {
