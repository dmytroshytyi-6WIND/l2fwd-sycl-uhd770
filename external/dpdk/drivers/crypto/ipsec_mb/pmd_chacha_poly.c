...
chacha20_poly1305_session_configure(IMB_MGR *mb_mgr __rte_unused,
  ...
  case IPSEC_MB_OP_AEAD_AUTHENTICATED_DECRYPT:
    if (aead_xform->aead.algo != RTE_CRYPTO_AEAD_CHACHA20_POLY1305) {
      IPSEC_MB_LOG(
          ERR, "The only combined operation supported is CHACHA20 POLY1305");
      ret = -ENOTSUP;
      goto error_exit;
    }
    /* Set IV parameters */
  ...
  /* IV check */
  if (sess->iv.length != CHACHA20_POLY1305_IV_LENGTH && sess->iv.length != 0) {
...
chacha20_poly1305_crypto_op(struct ipsec_mb_qp *qp, struct rte_crypto_op *op,
  ...
  struct chacha20_poly1305_qp_data *qp_data = ipsec_mb_get_qp_private_data(qp);
  uint8_t *src, *dst;
  uint8_t *iv_ptr;
  ...
  IMB_CHACHA20_POLY1305_INIT(
      qp->mb_mgr, session->key, &qp_data->chacha20_poly1305_ctx_data, iv_ptr,
      sym_op->aead.aad.data, (uint64_t)session->aad_length);

  if (session->op == IPSEC_MB_OP_AEAD_AUTHENTICATED_ENCRYPT) {
    IMB_CHACHA20_POLY1305_ENC_UPDATE(qp->mb_mgr, session->key,
                                     &qp_data->chacha20_poly1305_ctx_data, dst,
    ...
    IMB_CHACHA20_POLY1305_DEC_UPDATE(qp->mb_mgr, session->key,
                                     &qp_data->chacha20_poly1305_ctx_data, dst,
                                     src, (uint64_t)part_len);
 ...
 * - void
 */
static void post_process_chacha20_poly1305_crypto_op(
    struct ipsec_mb_qp *qp, struct rte_crypto_op *op,
    struct chacha20_poly1305_session *session) {
  struct chacha20_poly1305_qp_data *qp_data = ipsec_mb_get_qp_private_data(qp);
 ...
 * - void
 */
static void handle_completed_chacha20_poly1305_crypto_op(
    struct ipsec_mb_qp *qp, struct rte_crypto_op *op,
...
static uint16_t chacha20_poly1305_pmd_dequeue_burst(void *queue_pair,
...
static int chacha20_poly1305_probe(struct rte_vdev_device *vdev) {
...
static struct rte_vdev_driver cryptodev_chacha20_poly1305_pmd_drv = {
    .probe = chacha20_poly1305_probe, .remove = ipsec_mb_remove};
...
RTE_INIT(ipsec_mb_register_chacha20_poly1305) {
  ...
  chacha_poly_data->qp_priv_size = sizeof(struct chacha20_poly1305_qp_data);
