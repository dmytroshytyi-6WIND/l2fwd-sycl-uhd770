 ...
 */
static uint16_t ipsec_mb_enqueue_burst(void *__qp, struct rte_crypto_op **ops,
                                       uint16_t nb_ops) {
  ...
  nb_enqueued =
      rte_ring_enqueue_burst(qp->ingress_queue, (void **)ops, nb_ops, NULL);
...
int ipsec_mb_create(struct rte_vdev_device *vdev,
  ...
  init_params.private_data_size =
      sizeof(struct ipsec_mb_dev_private) + pmd_data->internals_priv_size;
  init_params.max_nb_queue_pairs = RTE_CRYPTODEV_PMD_DEFAULT_MAX_NB_QUEUE_PAIRS;
  ...
  if (retval) {
    IPSEC_MB_LOG(ERR, "Failed to parse initialisation arguments[%s]", args);
    return -EINVAL;
  }

  dev = rte_cryptodev_pmd_create(name, &vdev->device, &init_params);
  if (dev == NULL) {
    IPSEC_MB_LOG(ERR, "driver %s: create failed", init_params.name);
    return -ENODEV;
  }
  ...
  if (pmd_data->dev_config) {
    ...
    if (retval < 0) {
      IPSEC_MB_LOG(ERR, "Failed to configure device %s", name);
