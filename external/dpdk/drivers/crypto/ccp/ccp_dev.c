...
struct ccp_queue *ccp_allot_queue(struct rte_cryptodev *cdev, int slot_req) {
  int i, ret = 0;
...
int ccp_read_hwrng(uint32_t *value) {
  struct ccp_device *dev;
...
ccp_queue_dma_zone_reserve(const char *queue_name, uint32_t queue_size,
                           int socket_id) {
  ...
  CCP_LOG_INFO("Allocate memzone for %s, size %u on socket %u", queue_name,
...
/* bitmap support apis */
static inline void ccp_set_bit(unsigned long *bitmap, int n) {
  __sync_fetch_and_or(&bitmap[WORD_OFFSET(n)], (1UL << BIT_OFFSET(n)));
}

static inline void ccp_clear_bit(unsigned long *bitmap, int n) {
  __sync_fetch_and_and(&bitmap[WORD_OFFSET(n)], ~(1UL << BIT_OFFSET(n)));
}

static inline uint32_t ccp_get_bit(unsigned long *bitmap, int n) {
  return ((bitmap[WORD_OFFSET(n)] & (1 << BIT_OFFSET(n))) != 0);
}

static inline uint32_t ccp_ffz(unsigned long word) {
  unsigned long first_zero;

  first_zero = __builtin_ffsl(~word);
  return first_zero ? (first_zero - 1) : BITS_PER_WORD;
}

static inline uint32_t ccp_find_first_zero_bit(unsigned long *addr,
                                               uint32_t limit) {
  uint32_t i;
  uint32_t nwords = 0;
...
static void ccp_bitmap_set(unsigned long *map, unsigned int start, int len) {
...
static void ccp_bitmap_clear(unsigned long *map, unsigned int start, int len) {
  unsigned long *p = map + WORD_OFFSET(start);
  const unsigned int size = start + len;
...
static unsigned long _ccp_find_next_bit(const unsigned long *addr,
                                        unsigned long nbits,
                                        unsigned long start,
...
static unsigned long ccp_find_next_bit(const unsigned long *addr,
                                       unsigned long size,
                                       unsigned long offset) {
  return _ccp_find_next_bit(addr, size, offset, 0UL);
}

static unsigned long ccp_find_next_zero_bit(const unsigned long *addr,
                                            unsigned long size,
                                            unsigned long offset) {
  return _ccp_find_next_bit(addr, size, offset, ~0UL);
}
 ...
 */
static unsigned long ccp_bitmap_find_next_zero_area(unsigned long *map,
                                                    unsigned long size,
                                                    unsigned long start,
                                                    unsigned int nr) {
...
static uint32_t ccp_lsb_alloc(struct ccp_queue *cmd_q, unsigned int count) {
  struct ccp_device *ccp;
  int start;

  /* First look at the map for the queue */
  if (cmd_q->lsb >= 0) {
    start = (uint32_t)ccp_bitmap_find_next_zero_area(cmd_q->lsbmap, LSB_SIZE, 0,
                                                     count);
    if (start < LSB_SIZE) {
  ...
  start = (uint32_t)ccp_bitmap_find_next_zero_area(
      ccp->lsbmap, MAX_LSB_CNT * LSB_SIZE, 0, count);
...
static void __rte_unused ccp_lsb_free(struct ccp_queue *cmd_q,
...
static int ccp_find_lsb_regions(struct ccp_queue *cmd_q, uint64_t status) {
...
static int ccp_assign_lsbs(struct ccp_device *ccp) {
  ...
  if (n_lsbs >= ccp->cmd_q_count) {
     ...
     */
    for (lsb_cnt = 1; n_lsbs && (lsb_cnt <= MAX_LSB_CNT); lsb_cnt++) {
      rc = ccp_find_and_assign_lsb_to_q(ccp, lsb_cnt, n_lsbs, &lsb_pub);
      if (rc < 0)
        return -EINVAL;
      n_lsbs = rc;
    }
...
static int ccp_add_device(struct ccp_device *dev, int type) {
  int i;
  ...
  for (i = 0; i < MAX_HW_QUEUES; i++) {
    ...
    cmd_q->reg_base = (uint8_t *)vaddr + CMD_Q_STATUS_INCR * (i + 1);

    /* CCP queue memory */
    snprintf(cmd_q->memz_name, sizeof(cmd_q->memz_name), "%s_%d_%s_%d_%s",
             "ccp_dev", (int)dev->id, "queue", (int)cmd_q->id, "mem");
    q_mz = ccp_queue_dma_zone_reserve(cmd_q->memz_name, cmd_q->qsize,
                                      SOCKET_ID_ANY);
    ...
    CCP_WRITE_REG(cmd_q->reg_base, CMD_Q_CONTROL_BASE, cmd_q->qcontrol);
    ...
    CCP_WRITE_REG(cmd_q->reg_base, CMD_Q_INTERRUPT_STATUS_BASE, ALL_INTERRUPTS);
    ...
    CCP_WRITE_REG(cmd_q->reg_base, CMD_Q_TAIL_LO_BASE, (uint32_t)dma_addr_lo);
    CCP_WRITE_REG(cmd_q->reg_base, CMD_Q_HEAD_LO_BASE, (uint32_t)dma_addr_lo);
  ...
  for (i = 0; i < dev->cmd_q_count; i++) {
    dev->cmd_q[i].sb_key = ccp_lsb_alloc(&dev->cmd_q[i], 1);
    dev->cmd_q[i].sb_iv = ccp_lsb_alloc(&dev->cmd_q[i], 1);
    dev->cmd_q[i].sb_sha = ccp_lsb_alloc(&dev->cmd_q[i], 2);
    dev->cmd_q[i].sb_hmac = ccp_lsb_alloc(&dev->cmd_q[i], 2);
  }
...
static void ccp_remove_device(struct ccp_device *dev) {
  if (dev == NULL)
    return;
...
static int is_ccp_device(const char *dirname, const struct rte_pci_id *ccp_id,
                         int *type) {
  ...
  for (id = ccp_id, i = 0; id->vendor_id != 0; id++, i++) {
    if (vendor == id->vendor_id && device_id == id->device_id) {
      *type = i;
...
static int ccp_probe_device(int ccp_type, struct rte_pci_device *pci_dev) {
  struct ccp_device *ccp_dev = NULL;
  int uio_fd = -1;

  ccp_dev = rte_zmalloc("ccp_device", sizeof(*ccp_dev), RTE_CACHE_LINE_SIZE);
  if (ccp_dev == NULL)
    goto fail;
...
int ccp_probe_devices(struct rte_pci_device *pci_dev,
  ...
  while ((d = readdir(dir)) != NULL) {
    if (d->d_name[0] == '.')
      continue;
    if (ccp_parse_pci_addr_format(d->d_name, sizeof(d->d_name), &domain, &bus,
                                  &devid, &function) != 0)
      continue;
    snprintf(dirname, sizeof(dirname), "%s/%s", SYSFS_PCI_DEVICES, d->d_name);
