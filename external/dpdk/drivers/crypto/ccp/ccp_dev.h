...
#define QUEUE_SIZE_VAL ((ffs(COMMANDS_PER_QUEUE) - 2) & CMD_Q_SIZE)
...
#define ALL_INTERRUPTS (INT_COMPLETION | INT_ERROR | INT_QUEUE_STOPPED)
...
/* bitmap */
enum { BITS_PER_WORD = sizeof(unsigned long) * CHAR_BIT };
...
static inline void ccp_pci_reg_write(void *base, int offset, uint32_t value) {
...
#define CCP_READ_REG(hw_addr, reg_offset) ccp_pci_reg_read(hw_addr, reg_offset)
...
static inline uint32_t low32_value(unsigned long addr) {
  return ((uint64_t)addr) & 0x0ffffffff;
}

static inline uint32_t high32_value(unsigned long addr) {
