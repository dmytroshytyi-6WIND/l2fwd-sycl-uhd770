...
uint32_t ccp_sha1_init[SHA_COMMON_DIGEST_SIZE / sizeof(uint32_t)] = {
    SHA1_H4, SHA1_H3, SHA1_H2, SHA1_H1, SHA1_H0, 0x0U, 0x0U, 0x0U,
};
...
static int generate_partial_hash(struct ccp_session *sess) {
  ...
  case CCP_AUTH_ALGO_SHA1_HMAC:
    ...
    hash_value_be32 =
        (uint32_t *)((uint8_t *)sess->auth.pre_compute + sess->auth.ctx_len);
  ...
  case CCP_AUTH_ALGO_SHA3_224_HMAC:
    ...
    hash_value_sha3 = (uint8_t *)(sess->auth.pre_compute + sess->auth.ctx_len);
  ...
  case CCP_AUTH_ALGO_SHA384_HMAC:
    ...
    hash_value_be64 =
        (uint64_t *)((uint8_t *)sess->auth.pre_compute + sess->auth.ctx_len);
...
static int generate_cmac_subkeys(struct ccp_session *sess) {
  ...
  if (EVP_CIPHER_CTX_set_padding(ctx, 0) <= 0)
    goto key_generate_err;
  if (EVP_EncryptUpdate(ctx, dst, &dstlen, zero_iv, AES_BLOCK_SIZE) <= 0)
    goto key_generate_err;
  ...
  ccp_ctx = (unsigned char *)(sess->auth.pre_compute + (2 * CCP_SB_BYTES) - 1);
...
ccp_configure_session_cipher(struct ccp_session *sess,
  ...
  case CCP_ENGINE_3DES:
    ...
    for (j = 0, x = 0; j < sess->cipher.key_length / 8; j++, x += 8)
      for (i = 0; i < 8; i++)
        sess->cipher.key_ccp[(8 + x) - i - 1] = sess->cipher.key[i + x];
    break;
  default:
...
ccp_configure_session_auth(struct ccp_session *sess,
  ...
  case RTE_CRYPTO_AUTH_MD5_HMAC:
    if (sess->auth_opt) {
      ...
      rte_memcpy(sess->auth.key, auth_xform->key.data, auth_xform->key.length);
    } else
  ...
  case RTE_CRYPTO_AUTH_AES_CMAC:
    ...
    rte_memcpy(sess->auth.key, auth_xform->key.data, sess->auth.key_length);
    for (i = 0; i < sess->auth.key_length; i++)
      sess->auth.key_ccp[sess->auth.key_length - i - 1] = sess->auth.key[i];
...
ccp_configure_session_aead(struct ccp_session *sess,
  ...
  rte_memcpy(sess->cipher.key, aead_xform->key.data, aead_xform->key.length);
...
static inline int ccp_cipher_slot(struct ccp_session *session) {
  int count = 0;
  ...
  case CCP_CIPHER_ALGO_3DES_CBC:
    ...
    break;
  default:
    CCP_LOG_ERR("Unsupported cipher algo %d", session->cipher.algo);
  }
  return count;
}

static inline int ccp_auth_slot(struct ccp_session *session) {
  int count = 0;
  ...
  case CCP_AUTH_ALGO_AES_CMAC:
    ...
    break;
  default:
    CCP_LOG_ERR("Unsupported auth algo %d", session->auth.algo);
  }

  return count;
}

static int ccp_aead_slot(struct ccp_session *session) {
  int count = 0;
  ...
  case RTE_CRYPTO_AEAD_AES_GCM:
    break;
  default:
    CCP_LOG_ERR("Unsupported aead algo %d", session->aead_algo);
  }
  switch (session->auth.algo) {
  case CCP_AUTH_ALGO_AES_GCM:
    ...
    break;
  default:
    CCP_LOG_ERR("Unsupported combined auth ALGO %d", session->auth.algo);
  }
  return count;
}
...
static uint8_t algo_select(int sessalgo, const EVP_MD **algo) {
  int res = 0;
...
static int process_cpu_auth_hmac(uint8_t *src, uint8_t *dst,
                                 __rte_unused uint8_t *iv, EVP_PKEY *pkey,
                                 int srclen, EVP_MD_CTX *ctx,
                                 const EVP_MD *algo, uint16_t d_len) {
  size_t dstlen;
...
static int cpu_crypto_auth(struct ccp_qp *qp, struct rte_crypto_op *op,
                           struct ccp_session *sess, EVP_MD_CTX *ctx) {
  uint8_t *src, *dst;
  int srclen, status;
  ...
  src = rte_pktmbuf_mtod_offset(mbuf_src, uint8_t *, op->sym->auth.data.offset);
  ...
  status = process_cpu_auth_hmac(src, dst, NULL, pkey, srclen, ctx, algo,
                                 sess->auth.digest_length);
  if (status) {
  ...
  if (sess->auth.op == CCP_AUTH_OP_VERIFY) {
    if (memcmp(dst, op->sym->auth.digest.data, sess->auth.digest_length) != 0) {
...
static void ccp_perform_passthru(struct ccp_passthru *pst,
...
static int ccp_perform_hmac(struct rte_crypto_op *op, struct ccp_queue *cmd_q) {
  ...
  session = (struct ccp_session *)get_sym_session_private_data(
      op->sym->session, ccp_cryptodev_driver_id);
  addr = session->auth.pre_compute;

  src_addr = rte_pktmbuf_iova_offset(op->sym->m_src, op->sym->auth.data.offset);
  append_ptr =
      (void *)rte_pktmbuf_append(op->sym->m_src, session->auth.ctx_len);
  if (iommu_mode == 2) {
  ...
  auth_msg_bits = (op->sym->auth.data.length + session->auth.block_size) * 8;
  ...
  if ((session->auth.ut.sha_type == CCP_SHA_TYPE_384) ||
    ...
    pst.src_addr = (phys_addr_t)((cmd_q->sb_sha + 1) * CCP_SB_BYTES);
  ...
  CCP_CMD_LEN(desc) = (session->auth.ctx_len - session->auth.offset);
  auth_msg_bits = (session->auth.block_size + session->auth.ctx_len -
                   session->auth.offset) *
                  8;
...
static int ccp_perform_sha(struct rte_crypto_op *op, struct ccp_queue *cmd_q) {
  struct ccp_session *session;
  union ccp_function function;
  struct ccp_desc *desc;
  uint32_t tail;
  ...
  if (iommu_mode == 2) {
    ...
    dest_addr = (phys_addr_t)rte_mem_virt2phy(append_ptr);
    pst.src_addr = (phys_addr_t)rte_mem_virt2phy((void *)session->auth.ctx);
  }
...
static int ccp_perform_sha3_hmac(struct rte_crypto_op *op,
                                 struct ccp_queue *cmd_q) {
  struct ccp_session *session;
  struct ccp_passthru pst;
  union ccp_function function;
  ...
  src_addr = rte_pktmbuf_iova_offset(op->sym->m_src, op->sym->auth.data.offset);
  append_ptr =
      (uint8_t *)rte_pktmbuf_append(op->sym->m_src, session->auth.ctx_len);
  if (!append_ptr) {
  ...
  if (iommu_mode == 2) {
    ...
    ctx_paddr = (phys_addr_t)rte_mem_virt2phy(session->auth.pre_compute);
  }
...
static int ccp_perform_sha3(struct rte_crypto_op *op, struct ccp_queue *cmd_q) {
  struct ccp_session *session;
  union ccp_function function;
  struct ccp_desc *desc;
...
static int ccp_perform_aes_cmac(struct rte_crypto_op *op,
                                struct ccp_queue *cmd_q) {
  struct ccp_session *session;
  union ccp_function function;
  struct ccp_passthru pst;
  ...
  if (op->sym->auth.data.length % session->auth.block_size == 0) {
    ...
    src_tb = rte_pktmbuf_mtod_offset(op->sym->m_src, uint8_t *,
                                     op->sym->auth.data.offset + length);
...
static int ccp_perform_aes(struct rte_crypto_op *op, struct ccp_queue *cmd_q,
  ...
  if (session->cipher.um.aes_mode != CCP_AES_MODE_ECB) {
    if (session->cipher.um.aes_mode == CCP_AES_MODE_CTR) {
      ...
      rte_memcpy(lsb_buf + (CCP_SB_BYTES - session->iv.length), iv,
                 session->iv.length);
      pst.src_addr =
          b_info->lsb_buf_phys + (b_info->lsb_buf_idx * CCP_SB_BYTES);
      b_info->lsb_buf_idx++;
    }
  ...
  src_addr =
      rte_pktmbuf_iova_offset(op->sym->m_src, op->sym->cipher.data.offset);
  if (likely(op->sym->m_dst != NULL))
    dest_addr =
        rte_pktmbuf_iova_offset(op->sym->m_dst, op->sym->cipher.data.offset);
  else
    dest_addr = src_addr;
...
static int ccp_perform_3des(struct rte_crypto_op *op, struct ccp_queue *cmd_q,
                            struct ccp_batch_info *b_info) {
  struct ccp_session *session;
  union ccp_function function;
...
static int ccp_perform_aes_gcm(struct rte_crypto_op *op,
                               struct ccp_queue *cmd_q) {
  struct ccp_session *session;
  union ccp_function function;
  uint8_t *iv;
  ...
  src_addr = rte_pktmbuf_iova_offset(op->sym->m_src, op->sym->aead.data.offset);
  if (unlikely(op->sym->m_dst != NULL))
    dest_addr =
        rte_pktmbuf_iova_offset(op->sym->m_dst, op->sym->aead.data.offset);
  else
    dest_addr = src_addr;
  ...
  *temp = rte_bswap64(op->sym->aead.data.length << 3);
  ...
  /* CMD1 IV Passthru */
  rte_memcpy(session->cipher.nonce + AES_BLOCK_SIZE, iv, session->iv.length);
...
static inline int ccp_crypto_cipher(struct rte_crypto_op *op,
                                    struct ccp_queue *cmd_q,
                                    struct ccp_batch_info *b_info) {
  int result = 0;
  struct ccp_session *session;
...
static inline int ccp_crypto_auth(struct rte_crypto_op *op,
                                  struct ccp_queue *cmd_q,
                                  struct ccp_batch_info *b_info) {
...
static inline int ccp_crypto_aead(struct rte_crypto_op *op,
                                  struct ccp_queue *cmd_q,
                                  struct ccp_batch_info *b_info) {
  int result = 0;
  struct ccp_session *session;
...
int process_ops_to_enqueue(struct ccp_qp *qp, struct rte_crypto_op **op,
                           struct ccp_queue *cmd_q, uint16_t nb_ops,
                           uint16_t total_nb_ops, int slots_req,
                           uint16_t b_idx) {
  int i, result = 0;
  struct ccp_batch_info *b_info;
  struct ccp_session *session;
  ...
  for (i = b_idx; i < (nb_ops + b_idx); i++) {
    session = (struct ccp_session *)get_sym_session_private_data(
        op[i]->sym->session, ccp_cryptodev_driver_id);
    switch (session->cmd_id) {
    case CCP_CMD_CIPHER:
    ...
    case CCP_CMD_AUTH:
      if (session->auth_opt) {
        b_info->auth_ctr++;
        result = cpu_crypto_auth(qp, op[i], session, auth_ctx);
      } else
  ...
  rte_wmb();
  /* Write the new tail address back to the queue register */
  CCP_WRITE_REG(cmd_q->reg_base, CMD_Q_TAIL_LO_BASE, b_info->tail_offset);
  /* Turn the queue back on using our cached control register */
  CCP_WRITE_REG(cmd_q->reg_base, CMD_Q_CONTROL_BASE,
                cmd_q->qcontrol | CMD_Q_RUN);
...
static inline void ccp_auth_dq_prepare(struct rte_crypto_op *op) {
  ...
  if (session->cmd_id == CCP_CMD_COMBINED) {
    ...
    digest_offset = op->sym->aead.data.offset + op->sym->aead.data.length;
  } else {
    digest_data = op->sym->auth.digest.data;
    digest_offset = op->sym->auth.data.offset + op->sym->auth.data.length;
  }
  ...
  if (session->auth.engine == CCP_ENGINE_SHA)
    if ((session->auth.ut.sha_type != CCP_SHA_TYPE_1) &&
      ...
      offset = session->auth.ctx_len - session->auth.offset - 1;
  ...
  if (session->auth.op == CCP_AUTH_OP_VERIFY) {
    if (memcmp(addr + offset, digest_data, session->auth.digest_length) != 0)
      op->status = RTE_CRYPTO_OP_STATUS_AUTH_FAILED;

  } else {
    if (unlikely(digest_data == 0))
      digest_data =
          rte_pktmbuf_mtod_offset(op->sym->m_dst, uint8_t *, digest_offset);
    rte_memcpy(digest_data, addr + offset, session->auth.digest_length);
  }
  /* Trim area used for digest from mbuf. */
  rte_pktmbuf_trim(op->sym->m_src, session->auth.ctx_len);
}

static int ccp_prepare_ops(struct ccp_qp *qp, struct rte_crypto_op **op_d,
                           struct ccp_batch_info *b_info, uint16_t nb_ops) {
  int i, min_ops;
  struct ccp_session *session;
  ...
  for (i = b_info->b_idx; i < min_ops; i++) {
    ...
    session = (struct ccp_session *)get_sym_session_private_data(
        op_d[i]->sym->session, ccp_cryptodev_driver_id);
    switch (session->cmd_id) {
    case CCP_CMD_CIPHER:
    ...
    case CCP_CMD_CIPHER_HASH:
      if (session->auth_opt)
        cpu_crypto_auth(qp, op_d[i], session, auth_ctx);
      else
        ccp_auth_dq_prepare(op_d[i]);
      break;
    case CCP_CMD_HASH_CIPHER:
...
int process_ops_to_dequeue(struct ccp_qp *qp, struct rte_crypto_op **op,
                           uint16_t nb_ops, uint16_t *total_nb_ops) {
  struct ccp_batch_info *b_info;
  ...
  if (qp->b_info != NULL) {
    ...
    if (unlikely(b_info->op_idx > 0))
      goto success;
  } else if (rte_ring_dequeue(qp->processed_pkts, (void **)&b_info))
    return 0;
