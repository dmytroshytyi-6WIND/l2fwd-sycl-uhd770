...
static struct ccp_session *get_ccp_session(struct ccp_qp *qp,
  ...
  if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
    ...
    sess = (struct ccp_session *)get_sym_session_private_data(
    ...
    if (unlikely(ccp_set_session_parameters(sess, op->sym->xform, internals) !=
                 0)) {
    ...
    set_sym_session_private_data(op->sym->session, ccp_cryptodev_driver_id,
                                 _sess_private_data);
  }
...
static uint16_t ccp_pmd_enqueue_burst(void *queue_pair,
  ...
  while (tmp_ops) {
    ...
    if (unlikely(cmd_q == NULL))
      return 0;
    enq_cnt += process_ops_to_enqueue(qp, ops, cmd_q, cur_ops, nb_ops,
                                      slots_req, b_idx);
    i++;
  }
...
static uint16_t ccp_pmd_dequeue_burst(void *queue_pair,
                                      struct rte_crypto_op **ops,
                                      uint16_t nb_ops) {
  struct ccp_qp *qp = queue_pair;
  ...
  for (i = 0; i < nb_dequeued; i++)
    if (unlikely(ops[i]->sess_type == RTE_CRYPTO_OP_SESSIONLESS)) {
      struct ccp_session *sess =
          (struct ccp_session *)get_sym_session_private_data(
              ops[i]->sym->session, ccp_cryptodev_driver_id);
...
/** Remove ccp pmd */
static int cryptodev_ccp_remove(struct rte_pci_device *pci_dev) {
...
static int cryptodev_ccp_create(const char *name,
  ...
  if (init_params->def_p.name[0] == '\0')
    strlcpy(init_params->def_p.name, name, sizeof(init_params->def_p.name));

  dev = rte_cryptodev_pmd_create(init_params->def_p.name, &pci_dev->device,
                                 &init_params->def_p);
  if (dev == NULL) {
...
init_error:
  CCP_LOG_ERR("driver %s: %s() failed", init_params->def_p.name, __func__);
...
/** Probe ccp pmd */
static int cryptodev_ccp_probe(struct rte_pci_driver *pci_drv __rte_unused,
                               struct rte_pci_device *pci_dev) {
  int rc = 0;
  char name[RTE_CRYPTODEV_NAME_MAX_LEN];
  struct ccp_pmd_init_params init_params = {
      .def_p = {"", sizeof(struct ccp_private), rte_socket_id(),
...
RTE_PMD_REGISTER_KMOD_DEP(CRYPTODEV_NAME_CCP_PMD,
