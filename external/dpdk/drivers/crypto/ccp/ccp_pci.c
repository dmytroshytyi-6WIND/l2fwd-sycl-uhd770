...
static const char *const uio_module_names[] = {"igb_uio", "uio_pci_generic",
                                               "vfio_pci"};
...
int ccp_check_pci_uio_module(void) {
  ...
  while (uio_module_names[i] != NULL) {
    while (fgets(buf, sizeof(buf), fp) != NULL) {
      if (!strncmp(buf, uio_module_names[i], strlen(uio_module_names[i]))) {
        fclose(fp);
        return i;
      }
    }
 ...
 * string is modified)
 */
static int ccp_pci_parse_one_sysfs_resource(char *line, size_t len,
                                            uint64_t *phys_addr,
                                            uint64_t *end_addr,
                                            uint64_t *flags) {
...
int ccp_pci_parse_sysfs_resource(const char *filename,
  ...
  for (i = 0; i < PCI_MAX_RESOURCE; i++) {
    if (fgets(buf, sizeof(buf), fp) == NULL)
      goto error;
    if (ccp_pci_parse_one_sysfs_resource(buf, sizeof(buf), &phys_addr,
                                         &end_addr, &flags) < 0)
      goto error;
