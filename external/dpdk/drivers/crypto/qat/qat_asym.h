...
#define QAT_ASYM_CAP(n, o, l, r, i)                                            \
  {                                                                            \
    .op = RTE_CRYPTO_OP_TYPE_ASYMMETRIC, {                                     \
      .asym = {                                                                \
          .xform_capa = {.xform_type = RTE_CRYPTO_ASYM_XFORM_##n,              \
                         .op_types = o,                                        \
                         {                                                     \
                             .modlen = {.min = l, .max = r, .increment = i},   \
                         }}},                                                  \
    }                                                                          \
  }
...
unsigned int qat_asym_session_get_private_size(struct rte_cryptodev *dev);

void qat_asym_session_clear(struct rte_cryptodev *dev,
                            struct rte_cryptodev_asym_session *sess);

void qat_asym_init_op_cookie(void *cookie);
