...
static __rte_always_inline void enqueue_one_aead_job_gen3(
...
static __rte_always_inline void enqueue_one_auth_job_gen3(
    struct qat_sym_session *ctx, struct qat_sym_op_cookie *cookie,
    struct icp_qat_fw_la_bulk_req *req, struct rte_crypto_va_iova_ptr *digest,
    struct rte_crypto_va_iova_ptr *auth_iv, union rte_crypto_sym_ofs ofs,
    uint32_t data_len) {
  struct icp_qat_fw_cipher_cd_ctrl_hdr *cipher_cd_ctrl;
  struct icp_qat_fw_la_cipher_req_params *cipher_param;
  uint32_t ver_key_offset;
  uint32_t auth_data_len = data_len - ofs.ofs.auth.head - ofs.ofs.auth.tail;

  if (!ctx->is_single_pass_gmac ||
      (auth_data_len > QAT_AES_GMAC_SPC_MAX_SIZE)) {
    enqueue_one_auth_job_gen1(ctx, req, digest, auth_iv, ofs, data_len);
    return;
  }
  ...
  if (ctx->qat_hash_alg == ICP_QAT_HW_AUTH_ALGO_GALOIS_128 ||
    ...
    /* AES-GMAC */
    qat_set_cipher_iv(cipher_param, auth_iv, ctx->auth_iv.length, req);
  }
  ...
  cookie->opt.spc_gmac.cd_cipher.cipher_config.val =
      ICP_QAT_HW_CIPHER_CONFIG_BUILD(ICP_QAT_HW_CIPHER_AEAD_MODE,
                                     ctx->qat_cipher_alg,
  ...
  QAT_FIELD_SET(cookie->opt.spc_gmac.cd_cipher.cipher_config.val,
                ctx->digest_length, QAT_CIPHER_AEAD_HASH_CMP_LEN_BITPOS,
  ...
  /* Update the request */
  req->cd_pars.u.s.content_desc_addr = cookie->opt.spc_gmac.cd_phys_addr;
  req->cd_pars.u.s.content_desc_params_sz =
      RTE_ALIGN_CEIL(
          sizeof(struct icp_qat_hw_cipher_config) + ctx->auth_key_length, 8) >>
      3;
  ...
  ICP_QAT_FW_LA_SINGLE_PASS_PROTO_FLAG_SET(req->comn_hdr.serv_specif_flags,
                                           ICP_QAT_FW_LA_SINGLE_PASS_PROTO);
  ICP_QAT_FW_LA_PROTO_SET(req->comn_hdr.serv_specif_flags,
                          ICP_QAT_FW_LA_NO_PROTO);
}

static int qat_sym_build_op_aead_gen3(void *in_op, struct qat_sym_session *ctx,
...
static int qat_sym_build_op_auth_gen3(void *in_op, struct qat_sym_session *ctx,
                                      uint8_t *out_msg, void *op_cookie) {
  register struct icp_qat_fw_la_bulk_req *req;
  struct rte_crypto_op *op = in_op;
  struct qat_sym_op_cookie *cookie = op_cookie;
...
static int qat_sym_crypto_set_session_gen3(void *cdev __rte_unused,
                                           void *session) {
...
static int qat_sym_dp_enqueue_single_aead_gen3(
  ...
  enqueue_one_aead_job_gen3(ctx, req, iv, digest, aad, ofs, (uint32_t)data_len);
...
static uint32_t qat_sym_dp_enqueue_aead_jobs_gen3(
    void *qp_data, uint8_t *drv_ctx, struct rte_crypto_sym_vec *vec,
    union rte_crypto_sym_ofs ofs, void *user_data[], int *status) {
  struct qat_qp *qp = qp_data;
  struct qat_sym_dp_ctx *dp_ctx = (void *)drv_ctx;
  struct qat_queue *tx_queue = &qp->tx_q;
  struct qat_sym_session *ctx = dp_ctx->session;
  ...
  for (i = 0; i < n; i++) {
    ...
    enqueue_one_aead_job_gen3(ctx, req, &vec->iv[i], &vec->digest[i],
...
static int qat_sym_dp_enqueue_single_auth_gen3(
    void *qp_data, uint8_t *drv_ctx, struct rte_crypto_vec *data,
    uint16_t n_data_vecs, union rte_crypto_sym_ofs ofs,
...
static uint32_t qat_sym_dp_enqueue_auth_jobs_gen3(
    void *qp_data, uint8_t *drv_ctx, struct rte_crypto_sym_vec *vec,
    union rte_crypto_sym_ofs ofs, void *user_data[], int *status) {
  struct qat_qp *qp = qp_data;
  struct qat_sym_dp_ctx *dp_ctx = (void *)drv_ctx;
...
static int qat_sym_configure_raw_dp_ctx_gen3(void *_raw_dp_ctx, void *_ctx) {
...
RTE_INIT(qat_sym_crypto_gen3_init) {
  ...
  qat_sym_gen_dev_ops[QAT_GEN3].get_capabilities = qat_sym_crypto_cap_get_gen3;
  qat_sym_gen_dev_ops[QAT_GEN3].get_feature_flags =
      qat_sym_crypto_feature_flags_get_gen1;
  qat_sym_gen_dev_ops[QAT_GEN3].set_session = qat_sym_crypto_set_session_gen3;
