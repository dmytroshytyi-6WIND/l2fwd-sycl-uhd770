...
static __rte_always_inline void enqueue_one_aead_job_gen4(
...
static int qat_sym_build_op_aead_gen4(void *in_op, struct qat_sym_session *ctx,
  ...
  total_len = qat_sym_build_req_set_data(qat_req, in_op, cookie, in_sgl.vec,
  ...
  qat_sym_debug_log_dump(qat_req, ctx, in_sgl.vec, in_sgl.num, &cipher_iv, NULL,
                         &aad, &digest);
#endif
...
static int qat_sym_crypto_set_session_gen4(void *cdev, void *session) {
...
static int qat_sym_dp_enqueue_single_aead_gen4(
  ...
  enqueue_one_aead_job_gen4(ctx, req, iv, digest, aad, ofs, (uint32_t)data_len);
...
static uint32_t qat_sym_dp_enqueue_aead_jobs_gen4(
  ...
  for (i = 0; i < n; i++) {
    ...
    enqueue_one_aead_job_gen4(ctx, req, &vec->iv[i], &vec->digest[i],
...
static int qat_sym_configure_raw_dp_ctx_gen4(void *_raw_dp_ctx, void *_ctx) {
...
RTE_INIT(qat_sym_crypto_gen4_init) {
  ...
  qat_sym_gen_dev_ops[QAT_GEN4].get_capabilities = qat_sym_crypto_cap_get_gen4;
  qat_sym_gen_dev_ops[QAT_GEN4].set_session = qat_sym_crypto_set_session_gen4;
