...
qat_sym_crypto_qp_setup_gen2(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  if (ret != 0)
    QAT_LOG(DEBUG, "QAT firmware version: %d.%d.%d", (ret >> 24) & 0xff,
            (ret >> 16) & 0xff, (ret >> 8) & 0xff);
  else
    QAT_LOG(DEBUG, "unknown QAT firmware version");

  /* set capabilities based on the fw version */
  qat_sym_private->internal_capabilities =
      QAT_SYM_CAP_VALID |
      ((ret >= MIXED_CRYPTO_MIN_FW_VER) ? QAT_SYM_CAP_MIXED_CRYPTO : 0);
  return 0;
}
...
void qat_sym_session_set_ext_hash_flags_gen2(struct qat_sym_session *session,
  ...
  QAT_FIELD_SET(header->comn_req_flags, QAT_COMN_EXT_FLAGS_USED,
                QAT_COMN_EXT_FLAGS_BITPOS, QAT_COMN_EXT_FLAGS_MASK);
  ...
  case ICP_QAT_HW_CIPHER_ALGO_SNOW_3G_UEA2:
    ...
    ICP_QAT_FW_LA_ZUC_3G_PROTO_FLAG_SET(header->serv_specif_flags, 0);
    break;
...
static int qat_sym_crypto_set_session_gen2(void *cdev, void *session) {
  struct rte_cryptodev *dev = cdev;
  struct qat_sym_session *ctx = session;
  const struct qat_cryptodev_private *qat_private = dev->data->dev_private;
  int ret;
  ...
  if (ret == -ENOTSUP) {
     ...
     */
    if ((qat_private->internal_capabilities & QAT_SYM_CAP_MIXED_CRYPTO) == 0)
      return -ENOTSUP;
...
RTE_INIT(qat_sym_crypto_gen2_init) {
  ...
  qat_sym_gen_dev_ops[QAT_GEN2].get_capabilities = qat_sym_crypto_cap_get_gen2;
  qat_sym_gen_dev_ops[QAT_GEN2].set_session = qat_sym_crypto_set_session_gen2;
...
RTE_INIT(qat_asym_crypto_gen2_init) {
  qat_asym_gen_dev_ops[QAT_GEN2].cryptodev_ops = &qat_asym_crypto_ops_gen1;
  ...
  qat_asym_gen_dev_ops[QAT_GEN2].set_session = qat_asym_crypto_set_session_gen1;
}
