...
static __rte_always_inline int op_bpi_cipher_decrypt(uint8_t *src, uint8_t *dst,
  ...
  if (EVP_EncryptUpdate(ctx, encrypted_iv, &encrypted_ivlen, iv, ivlen) <= 0)
...
static __rte_always_inline uint32_t qat_bpicipher_preprocess(
    struct qat_sym_session *ctx, struct rte_crypto_op *op) {
  int block_len = qat_cipher_get_block_size(ctx->qat_cipher_alg);
  struct rte_crypto_sym_op *sym_op = op->sym;
  uint8_t last_block_len =
      block_len > 0 ? sym_op->cipher.data.length % block_len : 0;
  ...
  if (last_block_len && ctx->qat_dir == ICP_QAT_HW_CIPHER_DECRYPT) {
    ...
    last_block = (uint8_t *)rte_pktmbuf_mtod_offset(sym_op->m_src, uint8_t *,
                                                    last_block_offset);

    if (unlikely((sym_op->m_dst != NULL) && (sym_op->m_dst != sym_op->m_src)))
      /* out-of-place operation (OOP) */
      dst = (uint8_t *)rte_pktmbuf_mtod_offset(sym_op->m_dst, uint8_t *,
                                               last_block_offset);
    else
      dst = last_block;
    ...
    if (last_block_len < sym_op->cipher.data.length)
      ...
      iv = rte_crypto_op_ctod_offset(op, uint8_t *, ctx->cipher_iv.offset);

#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
    QAT_DP_HEXDUMP_LOG(DEBUG, "BPI: src before pre-process:", last_block,
                       last_block_len);
    if (sym_op->m_dst != NULL)
      QAT_DP_HEXDUMP_LOG(DEBUG, "BPI: dst before pre-process:", dst,
                         last_block_len);
#endif
    op_bpi_cipher_decrypt(last_block, dst, iv, block_len, last_block_len,
                          ctx->bpi_ctx);
#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
    QAT_DP_HEXDUMP_LOG(DEBUG, "BPI: src after pre-process:", last_block,
                       last_block_len);
    if (sym_op->m_dst != NULL)
      QAT_DP_HEXDUMP_LOG(DEBUG, "BPI: dst after pre-process:", dst,
                         last_block_len);
#endif
  }
...
qat_auth_is_len_in_bits(struct qat_sym_session *ctx, struct rte_crypto_op *op) {
...
static __rte_always_inline int32_t qat_sym_build_req_set_data(
    struct icp_qat_fw_la_bulk_req *req, void *opaque,
    struct qat_sym_op_cookie *cookie, struct rte_crypto_vec *src_vec,
    uint16_t n_src, struct rte_crypto_vec *dst_vec, uint16_t n_dst) {
  struct qat_sgl *list;
  uint32_t i;
  ...
  if (likely(!is_sgl)) {
    ...
    tl_src = total_len_src = src_vec[0].len;
...
static __rte_always_inline uint64_t qat_sym_convert_op_to_vec_cipher(
    struct rte_crypto_op *op, struct qat_sym_session *ctx,
  ...
  default:
    QAT_DP_LOG(
        ERR, "SNOW3G/KASUMI/ZUC in QAT PMD only supports byte aligned values");
    op->status = RTE_CRYPTO_OP_STATUS_INVALID_ARGS;
    return UINT64_MAX;
  }

  cipher_iv->va = rte_crypto_op_ctod_offset(op, void *, ctx->cipher_iv.offset);
  cipher_iv->iova = rte_crypto_op_ctophys_offset(op, ctx->cipher_iv.offset);
  ...
  if (unlikely((op->sym->m_dst != NULL) &&
    ...
    int n_dst = rte_crypto_mbuf_to_vec(op->sym->m_dst, cipher_ofs, cipher_len,
                                       out_sgl->vec, QAT_SYM_SGL_MAX_NUMBER);
...
static __rte_always_inline uint64_t qat_sym_convert_op_to_vec_auth(
    struct rte_crypto_op *op, struct qat_sym_session *ctx,
    struct rte_crypto_sgl *in_sgl, struct rte_crypto_sgl *out_sgl,
  ...
  case 1:
    ...
    auth_iv->va = rte_crypto_op_ctod_offset(op, void *, ctx->auth_iv.offset);
    auth_iv->iova = rte_crypto_op_ctophys_offset(op, ctx->auth_iv.offset);
    break;
  case 0:
  ...
  n_src = rte_crypto_mbuf_to_vec(op->sym->m_src, auth_ofs, auth_ofs + auth_len,
                                 in_sgl->vec, QAT_SYM_SGL_MAX_NUMBER);
  if (n_src < 0 || n_src > op->sym->m_src->nb_segs) {
    op->status = RTE_CRYPTO_OP_STATUS_ERROR;
    return UINT64_MAX;
  ...
  if (unlikely((op->sym->m_dst != NULL) &&
               (op->sym->m_dst != op->sym->m_src))) {
    int n_dst =
        rte_crypto_mbuf_to_vec(op->sym->m_dst, auth_ofs, auth_ofs + auth_len,
                               out_sgl->vec, QAT_SYM_SGL_MAX_NUMBER);
...
static __rte_always_inline uint64_t qat_sym_convert_op_to_vec_chain(
    struct rte_crypto_op *op, struct qat_sym_session *ctx,
    struct rte_crypto_sgl *in_sgl, struct rte_crypto_sgl *out_sgl,
    struct rte_crypto_va_iova_ptr *cipher_iv,
  ...
  int is_oop = (op->sym->m_dst != NULL) && (op->sym->m_dst != op->sym->m_src);
  ...
  cipher_iv->iova = rte_crypto_op_ctophys_offset(op, ctx->cipher_iv.offset);
  auth_iv_or_aad->va =
      rte_crypto_op_ctod_offset(op, void *, ctx->auth_iv.offset);
  auth_iv_or_aad->iova = rte_crypto_op_ctophys_offset(op, ctx->auth_iv.offset);
  digest->va = (void *)op->sym->auth.digest.data;
  digest->iova = op->sym->auth.digest.phys_addr;
  ...
  if (ret == 1) {
    ...
    if (unlikely(is_sgl)) {
      ...
      struct rte_mbuf *sgl_buf = (is_oop ? op->sym->m_dst : op->sym->m_src);
      ...
      auth_end_iova = (is_oop ? rte_pktmbuf_iova(op->sym->m_dst)
                              : rte_pktmbuf_iova(op->sym->m_src)) +
                      auth_ofs + auth_len;

    /* Then check if digest-encrypted conditions are met */
    if ((auth_ofs + auth_len < cipher_ofs + cipher_len) &&
        (digest->iova == auth_end_iova))
      max_len = RTE_MAX(max_len, auth_ofs + auth_len + ctx->digest_length);
  }

  n_src = rte_crypto_mbuf_to_vec(op->sym->m_src, min_ofs, max_len, in_sgl->vec,
  ...
  if (unlikely((op->sym->m_dst != NULL) &&
               (op->sym->m_dst != op->sym->m_src))) {
    int n_dst = rte_crypto_mbuf_to_vec(op->sym->m_dst, min_ofs, max_len,
                                       out_sgl->vec, QAT_SYM_SGL_MAX_NUMBER);
...
static __rte_always_inline uint64_t qat_sym_convert_op_to_vec_aead(
    struct rte_crypto_op *op, struct qat_sym_session *ctx,
    struct rte_crypto_sgl *in_sgl, struct rte_crypto_sgl *out_sgl,
    struct rte_crypto_va_iova_ptr *cipher_iv,
    struct rte_crypto_va_iova_ptr *auth_iv_or_aad,
...
qat_set_cipher_iv(struct icp_qat_fw_la_cipher_req_params *cipher_param,
  ...
  if (iv_len <= sizeof(cipher_param->u.cipher_IV_array))
    rte_memcpy(cipher_param->u.cipher_IV_array, iv_ptr->va, iv_len);
  else {
...
enqueue_one_cipher_job_gen1(struct qat_sym_session *ctx,
  ...
  cipher_param->cipher_length =
      data_len - ofs.ofs.cipher.head - ofs.ofs.cipher.tail;
}
...
enqueue_one_auth_job_gen1(struct qat_sym_session *ctx,
  ...
  case ICP_QAT_HW_AUTH_ALGO_GALOIS_64:
    ICP_QAT_FW_LA_GCM_IV_LEN_FLAG_SET(req->comn_hdr.serv_specif_flags,
...
enqueue_one_chain_job_gen1(struct qat_sym_session *ctx,
                           struct icp_qat_fw_la_bulk_req *req,
                           struct rte_crypto_vec *src_vec, uint16_t n_src_vecs,
                           struct rte_crypto_vec *dst_vec, uint16_t n_dst_vecs,
                           struct rte_crypto_va_iova_ptr *cipher_iv,
                           struct rte_crypto_va_iova_ptr *digest,
                           struct rte_crypto_va_iova_ptr *auth_iv,
                           union rte_crypto_sym_ofs ofs, uint32_t data_len) {
  struct icp_qat_fw_la_cipher_req_params *cipher_param;
  struct icp_qat_fw_la_auth_req_params *auth_param;
  struct rte_crypto_vec *cvec = n_dst_vecs > 0 ? dst_vec : src_vec;
  ...
  if (unlikely(is_sgl)) {
    ...
    auth_iova_end = cvec[0].iova + auth_param->auth_off + auth_param->auth_len;
  ...
  if ((auth_param->auth_off + auth_param->auth_len <
    ...
    if (cipher_param->cipher_offset + cipher_param->cipher_length <
            auth_param->auth_off + auth_param->auth_len + ctx->digest_length &&
        !is_sgl)
...
static __rte_always_inline void enqueue_one_aead_job_gen1(
    struct qat_sym_session *ctx, struct icp_qat_fw_la_bulk_req *req,
    struct rte_crypto_va_iova_ptr *iv, struct rte_crypto_va_iova_ptr *digest,
    struct rte_crypto_va_iova_ptr *aad, union rte_crypto_sym_ofs ofs,
    uint32_t data_len) {
  struct icp_qat_fw_la_cipher_req_params *cipher_param =
  ...
  case ICP_QAT_HW_AUTH_ALGO_GALOIS_64:
    ICP_QAT_FW_LA_GCM_IV_LEN_FLAG_SET(req->comn_hdr.serv_specif_flags,
                                      ICP_QAT_FW_LA_GCM_IV_LEN_12_OCTETS);
    rte_memcpy(cipher_param->u.cipher_IV_array, iv->va, ctx->cipher_iv.length);
    aad_iova = aad->iova;
    break;
  case ICP_QAT_HW_AUTH_ALGO_AES_CBC_MAC:
    ...
    aad_ccm_real_len = 0;
    aad_len_field_sz = 0;
    msg_len_be = rte_bswap32((uint32_t)data_len - ofs.ofs.cipher.head);
    ...
    aad_data[0] =
        ICP_QAT_HW_CCM_BUILD_B0_FLAGS(aad_len_field_sz, ctx->digest_length, q);
    if (q > ICP_QAT_HW_CCM_MSG_LEN_MAX_FIELD_SIZE) {
      memcpy(aad_data + ctx->cipher_iv.length + ICP_QAT_HW_CCM_NONCE_OFFSET +
                 (q - ICP_QAT_HW_CCM_MSG_LEN_MAX_FIELD_SIZE),
             (uint8_t *)&msg_len_be, ICP_QAT_HW_CCM_MSG_LEN_MAX_FIELD_SIZE);
    } else {
      memcpy(aad_data + ctx->cipher_iv.length + ICP_QAT_HW_CCM_NONCE_OFFSET,
             (uint8_t *)&msg_len_be +
                 (ICP_QAT_HW_CCM_MSG_LEN_MAX_FIELD_SIZE - q),
             q);
    }
    ...
    if (aad_len_field_sz > 0) {
      *(uint16_t *)&aad_data[ICP_QAT_HW_CCM_AAD_B0_LEN] =
      ...
      if ((aad_ccm_real_len + aad_len_field_sz) % ICP_QAT_HW_CCM_AAD_B0_LEN) {
        uint8_t pad_len = 0;
        uint8_t pad_idx = 0;

        pad_len =
            ICP_QAT_HW_CCM_AAD_B0_LEN -
            ((aad_ccm_real_len + aad_len_field_sz) % ICP_QAT_HW_CCM_AAD_B0_LEN);
        pad_idx =
            ICP_QAT_HW_CCM_AAD_B0_LEN + aad_ccm_real_len + aad_len_field_sz;
    ...
    *(uint8_t *)&cipher_param->u.cipher_IV_array[0] =
    ...
    rte_memcpy((uint8_t *)aad->va + ICP_QAT_HW_CCM_NONCE_OFFSET,
               (uint8_t *)iv->va + ICP_QAT_HW_CCM_NONCE_OFFSET,
               ctx->cipher_iv.length);
    break;
  default:
...
uint64_t qat_sym_crypto_feature_flags_get_gen1(struct qat_pci_device *qat_dev);

int qat_sym_crypto_set_session_gen1(void *cryptodev, void *session);

void qat_sym_session_set_ext_hash_flags_gen2(struct qat_sym_session *session,
                                             uint8_t hash_flag);

struct qat_capabilities_info
qat_asym_crypto_cap_get_gen1(struct qat_pci_device *qat_dev);

uint64_t qat_asym_crypto_feature_flags_get_gen1(struct qat_pci_device *qat_dev);

int qat_asym_crypto_set_session_gen1(void *cryptodev, void *session);
