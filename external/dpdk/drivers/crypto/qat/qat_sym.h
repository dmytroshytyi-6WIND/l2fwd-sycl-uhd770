  ...
  {                                                                            \
    .op = RTE_CRYPTO_OP_TYPE_SYMMETRIC, {                                      \
      .sym = {.xform_type = RTE_CRYPTO_SYM_XFORM_AUTH,                         \
              {                                                                \
                  .auth = {.algo = RTE_CRYPTO_AUTH_##n, b, d},                 \
              }},                                                              \
    }                                                                          \
  }
...
#define QAT_SYM_AUTH_CAP(n, b, k, d, a, i)                                     \
  {                                                                            \
    .op = RTE_CRYPTO_OP_TYPE_SYMMETRIC, {                                      \
      .sym = {.xform_type = RTE_CRYPTO_SYM_XFORM_AUTH,                         \
              {                                                                \
                  .auth = {.algo = RTE_CRYPTO_AUTH_##n, b, k, d, a, i},        \
              }},                                                              \
    }                                                                          \
  }
...
#define QAT_SYM_AEAD_CAP(n, b, k, d, a, i)                                     \
  {                                                                            \
    .op = RTE_CRYPTO_OP_TYPE_SYMMETRIC, {                                      \
      .sym = {.xform_type = RTE_CRYPTO_SYM_XFORM_AEAD,                         \
              {                                                                \
                  .aead = {.algo = RTE_CRYPTO_AEAD_##n, b, k, d, a, i},        \
              }},                                                              \
    }                                                                          \
  }
  ...
  {                                                                            \
    .op = RTE_CRYPTO_OP_TYPE_SYMMETRIC, {                                      \
      .sym = {.xform_type = RTE_CRYPTO_SYM_XFORM_CIPHER,                       \
              {                                                                \
                  .cipher = {.algo = RTE_CRYPTO_CIPHER_##n, b, k, i},          \
              }},                                                              \
    }                                                                          \
  }
...
struct qat_sym_op_cookie {
  ...
  union {
    ...
    struct {
      struct icp_qat_hw_cipher_algo_blk cd_cipher __rte_packed
          __rte_cache_aligned;
...
uint16_t qat_sym_enqueue_burst(void *qp, struct rte_crypto_op **ops,
                               uint16_t nb_ops);

uint16_t qat_sym_dequeue_burst(void *qp, struct rte_crypto_op **ops,
                               uint16_t nb_ops);
 ...
 */
static inline int bpi_cipher_encrypt(uint8_t *src, uint8_t *dst, uint8_t *iv,
                                     int ivlen, int srclen, void *bpi_ctx) {
...
static inline uint32_t qat_bpicipher_postprocess(struct qat_sym_session *ctx,
  ...
  if (last_block_len > 0 && ctx->qat_dir == ICP_QAT_HW_CIPHER_ENCRYPT) {
    ...
    QAT_DP_HEXDUMP_LOG(DEBUG, "BPI: src before post-process:", last_block,
                       last_block_len);
    if (sym_op->m_dst != NULL)
      QAT_DP_HEXDUMP_LOG(DEBUG, "BPI: dst before post-process:", dst,
                         last_block_len);
#endif
    bpi_cipher_encrypt(last_block, dst, iv, block_len, last_block_len,
                       ctx->bpi_ctx);
#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
    QAT_DP_HEXDUMP_LOG(DEBUG, "BPI: src after post-process:", last_block,
                       last_block_len);
    if (sym_op->m_dst != NULL)
      QAT_DP_HEXDUMP_LOG(DEBUG, "BPI: dst after post-process:", dst,
                         last_block_len);
#endif
  }
...
#ifdef RTE_LIB_SECURITY
static inline void qat_crc_verify(struct qat_sym_session *ctx,
                                  struct rte_crypto_op *op) {
  struct rte_crypto_sym_op *sym_op = op->sym;
  ...
  if (ctx->qat_dir == ICP_QAT_HW_CIPHER_DECRYPT &&
    ...
    crc_data = rte_pktmbuf_mtod_offset(sym_op->m_src, uint8_t *, crc_data_ofs);

    crc = rte_net_crc_calc(crc_data, crc_data_len, RTE_NET_CRC32_ETH);
...
static inline void qat_crc_generate(struct qat_sym_session *ctx,
                                    struct rte_crypto_op *op) {
  struct rte_crypto_sym_op *sym_op = op->sym;
  ...
  if (ctx->qat_dir == ICP_QAT_HW_CIPHER_ENCRYPT &&
      sym_op->auth.data.length != 0 && sym_op->m_src->nb_segs == 1) {
...
static inline void qat_sym_preprocess_requests(void **ops, uint16_t nb_ops) {
  struct rte_crypto_op *op;
...
qat_sym_process_response(void **op, uint8_t *resp, void *op_cookie,
  ...
  struct rte_crypto_op *rx_op =
      (struct rte_crypto_op *)(uintptr_t)(resp_msg->opaque_data);
  ...
  if (rx_op->sess_type == RTE_CRYPTO_OP_SECURITY_SESSION) {
     ...
     */
    sess = (struct qat_sym_session *)get_sec_session_private_data(
        rx_op->sym->sec_session);
    is_docsis_sec = 1;
  } else
#endif
  {
    sess = (struct qat_sym_session *)get_sym_session_private_data(
        rx_op->sym->session, qat_sym_driver_id);
    is_docsis_sec = 0;
  }

  if (ICP_QAT_FW_COMN_STATUS_FLAG_OK !=
      ICP_QAT_FW_COMN_RESP_CRYPTO_STAT_GET(resp_msg->comn_hdr.comn_status)) {
  ...
  if (sess->is_single_pass_gmac) {
    struct qat_sym_op_cookie *cookie = (struct qat_sym_op_cookie *)op_cookie;
    memset(cookie->opt.spc_gmac.cd_cipher.key, 0, sess->auth_key_length);
  }
...
int qat_sym_configure_dp_ctx(struct rte_cryptodev *dev, uint16_t qp_id,
                             struct rte_crypto_raw_dp_ctx *raw_dp_ctx,
                             enum rte_crypto_op_sess_type sess_type,
                             union rte_cryptodev_session_ctx session_ctx,
                             uint8_t is_update);

int qat_sym_get_dp_ctx_size(struct rte_cryptodev *dev);

void qat_sym_init_op_cookie(void *cookie);

#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
static __rte_always_inline void qat_sym_debug_log_dump(
    struct icp_qat_fw_la_bulk_req *qat_req, struct qat_sym_session *ctx,
    struct rte_crypto_vec *vec, uint32_t vec_len,
    struct rte_crypto_va_iova_ptr *cipher_iv,
    struct rte_crypto_va_iova_ptr *auth_iv, struct rte_crypto_va_iova_ptr *aad,
  ...
  if (auth_iv && ctx->auth_iv.length > 0)
    QAT_DP_HEXDUMP_LOG(DEBUG, "auth iv:", auth_iv->va, ctx->auth_iv.length);
  if (aad && ctx->aad_len > 0)
    QAT_DP_HEXDUMP_LOG(DEBUG, "aad:", aad->va, ctx->aad_len);
  if (digest && ctx->digest_length > 0)
    QAT_DP_HEXDUMP_LOG(DEBUG, "digest:", digest->va, ctx->digest_length);
}
#else
...
static inline void qat_sym_preprocess_requests(void **ops __rte_unused,
                                               uint16_t nb_ops __rte_unused) {}

static inline void qat_sym_process_response(void **op __rte_unused,
