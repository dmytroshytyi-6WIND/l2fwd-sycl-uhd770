...
int qat_cryptodev_config(__rte_unused struct rte_cryptodev *dev,
...
void qat_cryptodev_stop(__rte_unused struct rte_cryptodev *dev) {}

int qat_cryptodev_close(struct rte_cryptodev *dev) {
  int i, ret;
...
void qat_cryptodev_info_get(struct rte_cryptodev *dev,
  ...
  if (info != NULL) {
    info->max_nb_queue_pairs = qat_qps_per_service(qat_dev, service_type);
...
void qat_cryptodev_stats_get(struct rte_cryptodev *dev,
...
void qat_cryptodev_stats_reset(struct rte_cryptodev *dev) {
  struct qat_cryptodev_private *qat_priv;
...
int qat_cryptodev_qp_release(struct rte_cryptodev *dev,
  ...
  QAT_LOG(DEBUG, "Release %s qp %u on device %d",
          qat_service_get_str(service_type), queue_pair_id, dev->data->dev_id);
...
int qat_cryptodev_qp_setup(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  qat_qp_conf.hw = qat_qp_get_hw_data(qat_dev, service_type, qp_id);
