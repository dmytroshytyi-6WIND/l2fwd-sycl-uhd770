...
static const struct rte_driver cryptodev_qat_sym_driver = {
    .name = qat_sym_drv_name, .alias = qat_sym_drv_name};

void qat_sym_init_op_cookie(void *op_cookie) {
  struct qat_sym_op_cookie *cookie = op_cookie;

  cookie->qat_sgl_src_phys_addr =
      rte_mempool_virt2iova(cookie) +
      offsetof(struct qat_sym_op_cookie, qat_sgl_src);

  cookie->qat_sgl_dst_phys_addr =
      rte_mempool_virt2iova(cookie) +
      offsetof(struct qat_sym_op_cookie, qat_sgl_dst);

  cookie->opt.spc_gmac.cd_phys_addr =
      rte_mempool_virt2iova(cookie) +
      offsetof(struct qat_sym_op_cookie, opt.spc_gmac.cd_cipher);
}

static __rte_always_inline int
qat_sym_build_request(void *in_op, uint8_t *out_msg, void *op_cookie,
                      uint64_t *opaque, enum qat_device_gen dev_gen) {
  ...
  if (likely(op->sess_type == RTE_CRYPTO_OP_WITH_SESSION)) {
    ctx = get_sym_session_private_data(op->sym->session, qat_sym_driver_id);
    if (unlikely(!ctx)) {
    ...
    if (sess != (uintptr_t)ctx) {
      ...
      if (unlikely(ctx->build_request[proc_type] == NULL)) {
        int ret = qat_sym_gen_dev_ops[dev_gen].set_session((void *)cdev,
                                                           (void *)sess);
        if (ret < 0) {
          op->status = RTE_CRYPTO_OP_STATUS_INVALID_SESSION;
          return -EINVAL;
    ...
    if ((void *)sess != (void *)op->sym->sec_session) {
      ...
      if (unlikely(ctx->bpi_ctx == NULL)) {
        QAT_DP_LOG(ERR,
                   "QAT PMD only supports security"
...
uint16_t qat_sym_enqueue_burst(void *qp, struct rte_crypto_op **ops,
                               uint16_t nb_ops) {
  return qat_enqueue_op_burst(qp, qat_sym_build_request, (void **)ops, nb_ops);
}

uint16_t qat_sym_dequeue_burst(void *qp, struct rte_crypto_op **ops,
                               uint16_t nb_ops) {
  return qat_dequeue_op_burst(qp, (void **)ops, qat_sym_process_response,
                              nb_ops);
}

int qat_sym_dev_create(struct qat_pci_device *qat_pci_dev,
  ...
  snprintf(name, RTE_CRYPTODEV_NAME_MAX_LEN, "%s_%s", qat_pci_dev->name, "sym");
  QAT_LOG(DEBUG, "Creating QAT SYM device %s", name);

  if (gen_dev_ops->cryptodev_ops == NULL) {
    QAT_LOG(ERR, "Device %s does not support symmetric crypto", name);
    return -(EFAULT);
  }
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    qat_pci_dev->qat_sym_driver_id = qat_sym_driver_id;
  } else if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    if (qat_pci_dev->qat_sym_driver_id != qat_sym_driver_id) {
      QAT_LOG(ERR,
  ...
  cryptodev = rte_cryptodev_pmd_create(name, &(qat_dev_instance->sym_rte_dev),
                                       &init_params);
...
#endif
  snprintf(capa_memz_name, RTE_CRYPTODEV_NAME_MAX_LEN, "QAT_SYM_CAPA_GEN_%d",
...
int qat_sym_dev_destroy(struct qat_pci_device *qat_pci_dev) {
  struct rte_cryptodev *cryptodev;
...
int qat_sym_configure_dp_ctx(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  if (!gen_dev_ops->set_raw_dp_ctx) {
    QAT_LOG(ERR, "Device GEN %u does not support raw data path", qat_dev_gen);
    return -ENOTSUP;
  }
  ...
  if (!is_update) {
    memset(raw_dp_ctx, 0, sizeof(*raw_dp_ctx) + sizeof(struct qat_sym_dp_ctx));
...
int qat_sym_get_dp_ctx_size(struct rte_cryptodev *dev __rte_unused) {
