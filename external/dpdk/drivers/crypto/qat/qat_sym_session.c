...
static int qat_sym_cd_cipher_set(struct qat_sym_session *cd,
                                 const uint8_t *enckey, uint32_t enckeylen);

static int qat_sym_cd_auth_set(struct qat_sym_session *cdesc,
                               const uint8_t *authkey, uint32_t authkeylen,
                               uint32_t aad_length, uint32_t digestsize,
                               unsigned int operation);
static void qat_sym_session_init_common_hdr(struct qat_sym_session *session);
 ...
 */
static void bpi_cipher_ctx_free(void *bpi_ctx) {
  if (bpi_ctx != NULL)
...
bpi_cipher_ctx_init(enum rte_crypto_cipher_algorithm cryptodev_algo,
  ...
  *ctx = EVP_CIPHER_CTX_new();
  ...
  if (cryptodev_algo == RTE_CRYPTO_CIPHER_DES_DOCSISBPI)
    algo = EVP_des_ecb();
  else if (key_length == ICP_QAT_HW_AES_128_KEY_SZ)
...
static int qat_is_auth_alg_supported(enum rte_crypto_auth_algorithm algo,
                                     struct qat_cryptodev_private *internals) {
  int i = 0;
  const struct rte_cryptodev_capabilities *capability;
...
void qat_sym_session_clear(struct rte_cryptodev *dev,
...
static int qat_get_cmd_id(const struct rte_crypto_sym_xform *xform) {
  /* Cipher Only */
  ...
  if (xform->type == RTE_CRYPTO_SYM_XFORM_AEAD) {
    ...
    else if (xform->aead.algo == RTE_CRYPTO_AEAD_AES_GCM)
      return ICP_QAT_FW_LA_CMD_HASH_CIPHER;
    else
      return ICP_QAT_FW_LA_CMD_CIPHER_HASH;
  }
...
int qat_sym_session_configure_cipher(struct rte_cryptodev *dev,
  ...
  case RTE_CRYPTO_CIPHER_DES_DOCSISBPI:
    ret = bpi_cipher_ctx_init(cipher_xform->algo, cipher_xform->op,
                              cipher_xform->key.data, cipher_xform->key.length,
                              &session->bpi_ctx);
    if (ret != 0) {
  ...
  case RTE_CRYPTO_CIPHER_ZUC_EEA3:
    if (!qat_is_cipher_alg_supported(cipher_xform->algo, internals)) {
  ...
  if (qat_sym_cd_cipher_set(session, cipher_xform->key.data,
...
int qat_sym_session_configure(struct rte_cryptodev *dev,
                              struct rte_crypto_sym_xform *xform,
                              struct rte_cryptodev_sym_session *sess,
  ...
  if (ret != 0) {
    QAT_LOG(ERR, "Crypto QAT PMD: failed to configure session parameters");
...
int qat_sym_session_set_parameters(struct rte_cryptodev *dev,
                                   struct rte_crypto_sym_xform *xform,
  ...
  if (session_paddr == 0 || session_paddr == RTE_BAD_IOVA) {
    QAT_LOG(ERR, "Session physical address unknown. Bad memory pool.");
    return -EINVAL;
  }

  memset(session, 0, sizeof(*session));
  /* Set context descriptor physical address */
  session->cd_paddr = session_paddr + offsetof(struct qat_sym_session, cd);
  ...
  case ICP_QAT_FW_LA_CMD_CIPHER_HASH:
    if (xform->type == RTE_CRYPTO_SYM_XFORM_AEAD) {
      ret = qat_sym_session_configure_aead(dev, xform, session);
      if (ret < 0)
        return ret;
    } else {
      ret = qat_sym_session_configure_cipher(dev, xform, session);
...
static int qat_sym_session_handle_single_pass(
    struct qat_sym_session *session,
...
int qat_sym_session_configure_auth(struct rte_cryptodev *dev,
                                   struct rte_crypto_sym_xform *xform,
                                   struct qat_sym_session *session) {
  ...
  case RTE_CRYPTO_AUTH_AES_CBC_MAC:
    QAT_LOG(ERR, "Crypto: Unsupported hash alg %u", auth_xform->algo);
    return -ENOTSUP;
  default:
    QAT_LOG(ERR, "Crypto: Undefined Hash algo %u specified", auth_xform->algo);
    return -EINVAL;
  }
  ...
  if (auth_xform->algo == RTE_CRYPTO_AUTH_AES_GMAC) {
    ...
    if (auth_xform->op == RTE_CRYPTO_AUTH_OP_GENERATE) {
       ...
       * then authentication
       */
      if (qat_sym_cd_cipher_set(session, auth_xform->key.data,
                                auth_xform->key.length))
        return -EINVAL;

      if (qat_sym_cd_auth_set(session, key_data, key_length, 0,
                              auth_xform->digest_length, auth_xform->op))
        return -EINVAL;
    } else {
...
int qat_sym_session_configure_aead(struct rte_cryptodev *dev,
                                   struct rte_crypto_sym_xform *xform,
                                   struct qat_sym_session *session) {
  ...
  case RTE_CRYPTO_AEAD_AES_GCM:
    ...
    if (qat_dev_gen < QAT_GEN3)
      break;
    qat_sym_session_handle_single_pass(session, aead_xform);
    break;
  ...
  if (session->is_single_pass) {
    if (qat_sym_cd_cipher_set(session, aead_xform->key.data,
    ...
    if (qat_sym_cd_auth_set(session, aead_xform->key.data,
                            aead_xform->key.length, aead_xform->aad_length,
                            aead_xform->digest_length, crypto_operation))
      return -EINVAL;
  } else {
    session->qat_dir = ICP_QAT_HW_CIPHER_DECRYPT;
    /*
...
unsigned int
qat_sym_session_get_private_size(struct rte_cryptodev *dev __rte_unused) {
...
static int qat_hash_get_state1_size(enum icp_qat_hw_auth_algo qat_hash_alg) {
  ...
  case ICP_QAT_HW_AUTH_ALGO_SHA1:
    return QAT_HW_ROUND_UP(ICP_QAT_HW_SHA1_STATE1_SZ, QAT_HW_DEFAULT_ALIGNMENT);
  ...
  case ICP_QAT_HW_AUTH_ALGO_MD5:
    return QAT_HW_ROUND_UP(ICP_QAT_HW_MD5_STATE1_SZ, QAT_HW_DEFAULT_ALIGNMENT);
  ...
  case ICP_QAT_HW_AUTH_ALGO_NULL:
    return QAT_HW_ROUND_UP(ICP_QAT_HW_NULL_STATE1_SZ, QAT_HW_DEFAULT_ALIGNMENT);
...
static int partial_hash_compute(enum icp_qat_hw_auth_algo hash_alg,
  ...
  case ICP_QAT_HW_AUTH_ALGO_SHA1:
    ...
    for (i = 0; i < digest_size >> 2; i++, hash_state_out_be32++)
      *hash_state_out_be32 = rte_bswap32(*(((uint32_t *)digest) + i));
    break;
  case ICP_QAT_HW_AUTH_ALGO_SHA224:
  ...
  case ICP_QAT_HW_AUTH_ALGO_SHA384:
    ...
    for (i = 0; i < digest_size >> 3; i++, hash_state_out_be64++)
      *hash_state_out_be64 = rte_bswap64(*(((uint64_t *)digest) + i));
    break;
  case ICP_QAT_HW_AUTH_ALGO_SHA512:
...
static int qat_sym_do_precomputes(enum icp_qat_hw_auth_algo hash_alg,
                                  ...
                                  uint8_t *p_state_buf, uint16_t *p_state_len,
                                  uint8_t aes_cmac) {
  int block_size;
  ...
  if (hash_alg == ICP_QAT_HW_AUTH_ALGO_AES_XCBC_MAC) {
    ...
    if (aes_cmac) {
      ...
      in = rte_zmalloc("AES CMAC K1", ICP_QAT_HW_AES_128_KEY_SZ, 16);
      ...
      rte_memcpy(in, AES_CMAC_SEED, ICP_QAT_HW_AES_128_KEY_SZ);
      rte_memcpy(p_state_buf, auth_key, auth_keylen);

      if (AES_set_encrypt_key(auth_key, auth_keylen << 3, &enc_key) != 0) {
        rte_free(in);
        return -EFAULT;
      }
      ...
      *p_state_len = ICP_QAT_HW_AES_XCBC_MAC_STATE2_SZ;
      ...
      in = rte_zmalloc("working mem for key", ICP_QAT_HW_AES_XCBC_MAC_STATE2_SZ,
                       16);
      if (in == NULL) {
        QAT_LOG(ERR, "Failed to alloc memory");
        return -ENOMEM;
      ...
      rte_memcpy(in, qat_aes_xcbc_key_seed, ICP_QAT_HW_AES_XCBC_MAC_STATE2_SZ);
    ...
    memset(p_state_buf, 0,
           ICP_QAT_HW_GALOIS_H_SZ + ICP_QAT_HW_GALOIS_LEN_A_SZ +
               ICP_QAT_HW_GALOIS_E_CTR0_SZ);
    in = rte_zmalloc("working mem for key", ICP_QAT_HW_GALOIS_H_SZ, 16);
    if (in == NULL) {
      QAT_LOG(ERR, "Failed to alloc memory");
      return -ENOMEM;
    }
...
static void qat_sym_session_init_common_hdr(struct qat_sym_session *session) {
  ...
  case QAT_CRYPTO_PROTO_FLAG_CCM:
    ICP_QAT_FW_LA_PROTO_SET(header->serv_specif_flags, ICP_QAT_FW_LA_CCM_PROTO);
    break;
  case QAT_CRYPTO_PROTO_FLAG_GCM:
    ICP_QAT_FW_LA_PROTO_SET(header->serv_specif_flags, ICP_QAT_FW_LA_GCM_PROTO);
    break;
  ...
  if (QAT_SESSION_IS_SLICE_SET(slice_flags, QAT_CRYPTO_SLICE_SPC)) {
    ICP_QAT_FW_LA_SINGLE_PASS_PROTO_FLAG_SET(header->serv_specif_flags,
...
int qat_sym_cd_cipher_set(struct qat_sym_session *cdesc,
                          const uint8_t *cipherkey, uint32_t cipherkeylen) {
  ...
  struct icp_qat_fw_la_cipher_20_req_params *req_ucs =
      (struct icp_qat_fw_la_cipher_20_req_params *)&cdesc->fw_req
          .serv_specif_rqpars;
  struct icp_qat_fw_la_cipher_req_params *req_cipher =
      (struct icp_qat_fw_la_cipher_req_params *)&cdesc->fw_req
          .serv_specif_rqpars;
  ...
  if (cdesc->qat_mode == ICP_QAT_HW_CIPHER_CTR_MODE) {
    ...
    if (cdesc->qat_cipher_alg == ICP_QAT_HW_CIPHER_ALGO_CHACHA20_POLY1305 &&
  ...
  if (cdesc->qat_cipher_alg == ICP_QAT_HW_CIPHER_ALGO_SNOW_3G_UEA2) {
    ...
    cipher_cd_ctrl->cipher_state_sz = ICP_QAT_HW_SNOW_3G_UEA2_IV_SZ >> 3;
    ...
    cipher_cd_ctrl->cipher_padding_sz = (2 * ICP_QAT_HW_KASUMI_BLK_SZ) >> 3;
    ...
    cipher_cd_ctrl->cipher_state_sz = ICP_QAT_HW_ZUC_3G_EEA3_IV_SZ >> 3;
  ...
  cipher->cipher_config.val = ICP_QAT_HW_CIPHER_CONFIG_BUILD(
      cdesc->qat_mode, cdesc->qat_cipher_alg, key_convert, cdesc->qat_dir);

  if (cdesc->qat_cipher_alg == ICP_QAT_HW_CIPHER_ALGO_KASUMI) {
    temp_key =
        (uint32_t *)(cdesc->cd_cur_ptr +
                     sizeof(struct icp_qat_hw_cipher_config) + cipherkeylen);
    ...
    for (wordIndex = 0; wordIndex < (cipherkeylen >> 2); wordIndex++)
      temp_key[wordIndex] ^= KASUMI_F8_KEY_MODIFIER_4_BYTES;

    cdesc->cd_cur_ptr +=
        sizeof(struct icp_qat_hw_cipher_config) + cipherkeylen + cipherkeylen;
  } else if (cdesc->is_ucs) {
    const uint8_t *final_key = cipherkey;

    cdesc->slice_types |= QAT_CRYPTO_SLICE_UCS;
    total_key_size = RTE_ALIGN_CEIL(cipherkeylen, ICP_QAT_HW_AES_128_KEY_SZ);
    ...
    cdesc->cd_cur_ptr +=
        sizeof(struct icp_qat_hw_ucs_cipher_config) + cipherkeylen;
  } else {
    memcpy(cipher->key, cipherkey, cipherkeylen);
    cdesc->cd_cur_ptr += sizeof(struct icp_qat_hw_cipher_config) + cipherkeylen;
  }

  if (cdesc->is_single_pass) {
    QAT_FIELD_SET(cipher->cipher_config.val, cdesc->digest_length,
    ...
    cdesc->cd.cipher.cipher_config.reserved =
        ICP_QAT_HW_CIPHER_CONFIG_BUILD_UPPER(cdesc->aad_len);
  ...
  if (total_key_size > cipherkeylen) {
    ...
    if ((cdesc->qat_cipher_alg == ICP_QAT_HW_CIPHER_ALGO_3DES) &&
      ...
      memcpy(cdesc->cd_cur_ptr, cipherkey, cipherkeylen);
      memcpy(cdesc->cd_cur_ptr + cipherkeylen, cipherkey, cipherkeylen);
    } else
...
int qat_sym_cd_auth_set(struct qat_sym_session *cdesc, const uint8_t *authkey,
                        uint32_t authkeylen, uint32_t aad_length,
                        uint32_t digestsize, unsigned int operation) {
  ...
  hash->auth_config.config = ICP_QAT_HW_AUTH_CONFIG_BUILD(
      cdesc->auth_mode, cdesc->qat_hash_alg, digestsize);
  ...
  if (cdesc->auth_mode == ICP_QAT_HW_AUTH_MODE0 ||
      ...
      cdesc->qat_hash_alg == ICP_QAT_HW_AUTH_ALGO_NULL || cdesc->is_cnt_zero)
  ...
  case ICP_QAT_HW_AUTH_ALGO_SHA1:
    if (cdesc->auth_mode == ICP_QAT_HW_AUTH_MODE0) {
      /* Plain SHA-1 */
      rte_memcpy(cdesc->cd_cur_ptr, sha1InitialState, sizeof(sha1InitialState));
      state1_size = qat_hash_get_state1_size(cdesc->qat_hash_alg);
      break;
    }
    /* SHA-1 HMAC */
    if (qat_sym_do_precomputes(ICP_QAT_HW_AUTH_ALGO_SHA1, authkey, authkeylen,
                               cdesc->cd_cur_ptr, &state1_size,
                               cdesc->aes_cmac)) {
  ...
  case ICP_QAT_HW_AUTH_ALGO_SHA224:
    ...
    /* SHA-224 HMAC */
    if (qat_sym_do_precomputes(ICP_QAT_HW_AUTH_ALGO_SHA224, authkey, authkeylen,
                               cdesc->cd_cur_ptr, &state1_size,
                               cdesc->aes_cmac)) {
      QAT_LOG(ERR, "(SHA)precompute failed");
      return -EFAULT;
  ...
  case ICP_QAT_HW_AUTH_ALGO_SHA256:
    ...
    /* SHA-256 HMAC */
    if (qat_sym_do_precomputes(ICP_QAT_HW_AUTH_ALGO_SHA256, authkey, authkeylen,
                               cdesc->cd_cur_ptr, &state1_size,
                               cdesc->aes_cmac)) {
      QAT_LOG(ERR, "(SHA)precompute failed");
      return -EFAULT;
  ...
  case ICP_QAT_HW_AUTH_ALGO_SHA384:
    ...
    /* SHA-384 HMAC */
    if (qat_sym_do_precomputes(ICP_QAT_HW_AUTH_ALGO_SHA384, authkey, authkeylen,
                               cdesc->cd_cur_ptr, &state1_size,
                               cdesc->aes_cmac)) {
      QAT_LOG(ERR, "(SHA)precompute failed");
      return -EFAULT;
  ...
  case ICP_QAT_HW_AUTH_ALGO_SHA512:
    ...
    /* SHA-512 HMAC */
    if (qat_sym_do_precomputes(ICP_QAT_HW_AUTH_ALGO_SHA512, authkey, authkeylen,
                               cdesc->cd_cur_ptr, &state1_size,
                               cdesc->aes_cmac)) {
      QAT_LOG(ERR, "(SHA)precompute failed");
      return -EFAULT;
  ...
  case ICP_QAT_HW_AUTH_ALGO_AES_XCBC_MAC:
    ...
    if (qat_sym_do_precomputes(ICP_QAT_HW_AUTH_ALGO_AES_XCBC_MAC, authkey,
                               authkeylen, cdesc->cd_cur_ptr + state1_size,
                               &state2_size, cdesc->aes_cmac)) {
      cdesc->aes_cmac ? QAT_LOG(ERR, "(CMAC)precompute failed")
  ...
  case ICP_QAT_HW_AUTH_ALGO_GALOIS_64:
    ...
    if (qat_sym_do_precomputes(cdesc->qat_hash_alg, authkey, authkeylen,
     ...
     */
    auth_param->u2.aad_sz = RTE_ALIGN_CEIL(aad_length, 16);
    auth_param->hash_state_sz = (auth_param->u2.aad_sz) >> 3;

    aad_len = (uint32_t *)(cdesc->cd_cur_ptr + ICP_QAT_HW_GALOIS_128_STATE1_SZ +
  ...
  case ICP_QAT_HW_AUTH_ALGO_SNOW_3G_UIA2:
    cdesc->qat_proto_flag = QAT_CRYPTO_PROTO_FLAG_SNOW3G;
    state1_size = qat_hash_get_state1_size(ICP_QAT_HW_AUTH_ALGO_SNOW_3G_UIA2);
    state2_size = ICP_QAT_HW_SNOW_3G_UIA2_STATE2_SZ;
    memset(cdesc->cd_cur_ptr, 0, state1_size + state2_size);

    cipherconfig =
        (struct icp_qat_hw_cipher_algo_blk *)(cdesc->cd_cur_ptr + state1_size +
                                              state2_size);
    cipherconfig->cipher_config.val = ICP_QAT_HW_CIPHER_CONFIG_BUILD(
        ICP_QAT_HW_CIPHER_ECB_MODE, ICP_QAT_HW_CIPHER_ALGO_SNOW_3G_UEA2,
        ICP_QAT_HW_CIPHER_KEY_CONVERT, ICP_QAT_HW_CIPHER_ENCRYPT);
    memcpy(cipherconfig->key, authkey, authkeylen);
    memset(cipherconfig->key + authkeylen, 0, ICP_QAT_HW_SNOW_3G_UEA2_IV_SZ);
    cd_extra_size += sizeof(struct icp_qat_hw_cipher_config) + authkeylen +
                     ICP_QAT_HW_SNOW_3G_UEA2_IV_SZ;
    auth_param->hash_state_sz = ICP_QAT_HW_SNOW_3G_UEA2_IV_SZ >> 3;
    break;
  case ICP_QAT_HW_AUTH_ALGO_ZUC_3G_128_EIA3:
    hash->auth_config.config = ICP_QAT_HW_AUTH_CONFIG_BUILD(
        ICP_QAT_HW_AUTH_MODE0, cdesc->qat_hash_alg, digestsize);
    cdesc->qat_proto_flag = QAT_CRYPTO_PROTO_FLAG_ZUC;
    state1_size =
        qat_hash_get_state1_size(ICP_QAT_HW_AUTH_ALGO_ZUC_3G_128_EIA3);
    state2_size = ICP_QAT_HW_ZUC_3G_EIA3_STATE2_SZ;
    memset(cdesc->cd_cur_ptr, 0,
           state1_size + state2_size + ICP_QAT_HW_ZUC_3G_EEA3_IV_SZ);
    ...
    break;
  case ICP_QAT_HW_AUTH_ALGO_MD5:
    if (qat_sym_do_precomputes(ICP_QAT_HW_AUTH_ALGO_MD5, authkey, authkeylen,
                               cdesc->cd_cur_ptr, &state1_size,
                               cdesc->aes_cmac)) {
    ...
    break;
  case ICP_QAT_HW_AUTH_ALGO_NULL:
    state1_size = qat_hash_get_state1_size(ICP_QAT_HW_AUTH_ALGO_NULL);
    state2_size = ICP_QAT_HW_NULL_STATE2_SZ;
    break;
  case ICP_QAT_HW_AUTH_ALGO_AES_CBC_MAC:
    cdesc->qat_proto_flag = QAT_CRYPTO_PROTO_FLAG_CCM;
    state1_size = qat_hash_get_state1_size(ICP_QAT_HW_AUTH_ALGO_AES_CBC_MAC);
    state2_size =
        ICP_QAT_HW_AES_CBC_MAC_KEY_SZ + ICP_QAT_HW_AES_CCM_CBC_E_CTR0_SZ;

    if (aad_length > 0) {
      aad_length += ICP_QAT_HW_CCM_AAD_B0_LEN + ICP_QAT_HW_CCM_AAD_LEN_INFO;
      auth_param->u2.aad_sz =
          RTE_ALIGN_CEIL(aad_length, ICP_QAT_HW_CCM_AAD_ALIGNMENT);
    } else {
    ...
    break;
  case ICP_QAT_HW_AUTH_ALGO_KASUMI_F9:
    state1_size = qat_hash_get_state1_size(ICP_QAT_HW_AUTH_ALGO_KASUMI_F9);
    state2_size = ICP_QAT_HW_KASUMI_F9_STATE2_SZ;
    memset(cdesc->cd_cur_ptr, 0, state1_size + state2_size);
    pTempKey = (uint32_t *)(cdesc->cd_cur_ptr + state1_size + authkeylen);
    /*
...
qat_sec_session_check_docsis(struct rte_security_session_conf *conf) {
  ...
  if (docsis->direction == RTE_SECURITY_DOCSIS_DOWNLINK) {
    ...
    if (crypto_sym != NULL && crypto_sym->type == RTE_CRYPTO_SYM_XFORM_CIPHER &&
        ...
        (crypto_sym->cipher.key.length == ICP_QAT_HW_AES_128_KEY_SZ ||
         crypto_sym->cipher.key.length == ICP_QAT_HW_AES_256_KEY_SZ) &&
...
int qat_security_session_create(void *dev,
                                struct rte_security_session_conf *conf,
  ...
  ret = qat_sec_session_set_docsis_parameters(cdev, conf, sess_private_data);
  if (ret != 0) {
...
int qat_security_session_destroy(void *dev __rte_unused,
                                 struct rte_security_session *sess) {
