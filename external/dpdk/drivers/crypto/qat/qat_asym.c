...
static const struct rte_driver cryptodev_qat_asym_driver = {
    .name = qat_asym_drv_name, .alias = qat_asym_drv_name};
...
#define HEXDUMP(name, where, size) QAT_DP_HEXDUMP_LOG(DEBUG, name, where, size)
#define HEXDUMP_OFF(name, where, size, idx)                                    \
  QAT_DP_HEXDUMP_LOG(DEBUG, name, &where[idx * size], size)

#define HEXDUMP_OFF_F(name, idx)                                               \
  QAT_DP_HEXDUMP_LOG(DEBUG, name,                                              \
...
#define CHECK_IF_NOT_EMPTY(param, name, pname, status)                         \
  do {                                                                         \
    if (param.length == 0) {                                                   \
      QAT_LOG(ERR, "Invalid " name " input parameter, zero length " pname);    \
      status = -EINVAL;                                                        \
    } else if (check_zero(param)) {                                            \
      QAT_LOG(ERR,                                                             \
              "Invalid " name " input parameter, empty " pname                 \
              ", length = %d",                                                 \
              (int)param.length);                                              \
      status = -EINVAL;                                                        \
    }                                                                          \
...
#define SET_PKE_LN(where, what, how, idx)                                      \
  rte_memcpy(where[idx] + how - what.length, what.data, what.length)

#define SET_PKE_LN_9A(where, what, how, idx)                                   \
  rte_memcpy(&where[idx * RTE_ALIGN_CEIL(how, 8)] + RTE_ALIGN_CEIL(how, 8) -   \
                 what.length,                                                  \
             what.data, what.length)

#define SET_PKE_LN_EC(where, what, how, idx)                                   \
  rte_memcpy(where[idx] + RTE_ALIGN_CEIL(how, 8) - how, what.data, how)
...
#define SET_PKE_LN_EC_F(what, how, idx)                                        \
  rte_memcpy(&cookie->input_buffer[idx * RTE_ALIGN_CEIL(how, 8)] +             \
                 RTE_ALIGN_CEIL(how, 8) - how,                                 \
             what.data, how)

static void request_init(struct icp_qat_fw_pke_request *qat_req) {
...
static void cleanup_arrays(struct qat_asym_op_cookie *cookie, int in_count,
                           int out_count, int alg_size) {
  int i;
...
static void cleanup_crt(struct qat_asym_op_cookie *cookie, int alg_size) {
  int i;
...
static void cleanup(struct qat_asym_op_cookie *cookie,
...
static int check_zero(rte_crypto_param n) {
  int i, len = n.length;
...
static int modexp_set_input(struct rte_crypto_asym_op *asym_op,
  ...
  CHECK_IF_NOT_EMPTY(xform->modex.modulus, "mod exp", "modulus", status);
  CHECK_IF_NOT_EMPTY(xform->modex.exponent, "mod exp", "exponent", status);
  if (status)
    return status;
  ...
  SET_PKE_LN(cookie->input_array, asym_op->modex.base, alg_bytesize, 0);
  SET_PKE_LN(cookie->input_array, xform->modex.exponent, alg_bytesize, 1);
  SET_PKE_LN(cookie->input_array, xform->modex.modulus, alg_bytesize, 2);
...
static uint8_t modexp_collect(struct rte_crypto_asym_op *asym_op,
                              struct qat_asym_op_cookie *cookie,
                              struct rte_crypto_asym_xform *xform) {
  ...
  rte_memcpy(modexp_result, cookie->output_array[0] + alg_bytesize - n.length,
             n.length);
  HEXDUMP("ModExp result", cookie->output_array[0], alg_bytesize);
  return RTE_CRYPTO_OP_STATUS_SUCCESS;
}

static int modinv_set_input(struct rte_crypto_asym_op *asym_op,
                            struct icp_qat_fw_pke_request *qat_req,
                            struct qat_asym_op_cookie *cookie,
                            struct rte_crypto_asym_xform *xform) {
  struct qat_asym_function qat_function;
  ...
  CHECK_IF_NOT_EMPTY(xform->modex.modulus, "mod inv", "modulus", status);
  if (status)
    return status;
  ...
  SET_PKE_LN(cookie->input_array, asym_op->modinv.base, alg_bytesize, 0);
  SET_PKE_LN(cookie->input_array, xform->modinv.modulus, alg_bytesize, 1);

  cookie->alg_bytesize = alg_bytesize;
  qat_req->pke_hdr.cd_pars.func_id = func_id;
  qat_req->input_param_count = QAT_ASYM_MODINV_NUM_IN_PARAMS;
...
static uint8_t modinv_collect(struct rte_crypto_asym_op *asym_op,
                              struct qat_asym_op_cookie *cookie,
                              struct rte_crypto_asym_xform *xform) {
  ...
  rte_memcpy(modinv_result + (asym_op->modinv.result.length - n.length),
             cookie->output_array[0] + alg_bytesize - n.length, n.length);
  HEXDUMP("ModInv result", cookie->output_array[0], alg_bytesize);
  return RTE_CRYPTO_OP_STATUS_SUCCESS;
}

static int rsa_set_pub_input(struct rte_crypto_asym_op *asym_op,
                             struct icp_qat_fw_pke_request *qat_req,
                             struct qat_asym_op_cookie *cookie,
                             struct rte_crypto_asym_xform *xform) {
  struct qat_asym_function qat_function;
  ...
  if (asym_op->rsa.op_type == RTE_CRYPTO_ASYM_OP_ENCRYPT) {
    ...
    case RTE_CRYPTO_RSA_PADDING_NONE:
      SET_PKE_LN(cookie->input_array, asym_op->rsa.message, alg_bytesize, 0);
      break;
    default:
      QAT_LOG(ERR, "Invalid RSA padding (Encryption)");
      return -EINVAL;
    }
    ...
    switch (asym_op->rsa.pad) {
    case RTE_CRYPTO_RSA_PADDING_NONE:
      SET_PKE_LN(cookie->input_array, asym_op->rsa.sign, alg_bytesize, 0);
      break;
    default:
      QAT_LOG(ERR, "Invalid RSA padding (Verify)");
      return -EINVAL;
    }
    HEXDUMP("RSA Signature", cookie->input_array[0], alg_bytesize);
  }

  SET_PKE_LN(cookie->input_array, xform->rsa.e, alg_bytesize, 1);
  SET_PKE_LN(cookie->input_array, xform->rsa.n, alg_bytesize, 2);
...
static int rsa_set_priv_input(struct rte_crypto_asym_op *asym_op,
                              struct icp_qat_fw_pke_request *qat_req,
                              struct qat_asym_op_cookie *cookie,
                              struct rte_crypto_asym_xform *xform) {
  struct qat_asym_function qat_function;
  ...
  if (xform->rsa.key_type == RTE_RSA_KEY_TYPE_QT) {
    ...
    alg_bytesize = qat_function.bytesize;
    qat_req->input_param_count = QAT_ASYM_RSA_QT_NUM_IN_PARAMS;

    SET_PKE_LN(cookie->input_array, xform->rsa.qt.p, (alg_bytesize >> 1), 1);
    SET_PKE_LN(cookie->input_array, xform->rsa.qt.q, (alg_bytesize >> 1), 2);
    SET_PKE_LN(cookie->input_array, xform->rsa.qt.dP, (alg_bytesize >> 1), 3);
    SET_PKE_LN(cookie->input_array, xform->rsa.qt.dQ, (alg_bytesize >> 1), 4);
    SET_PKE_LN(cookie->input_array, xform->rsa.qt.qInv, (alg_bytesize >> 1), 5);

    HEXDUMP("RSA p", cookie->input_array[1], alg_bytesize);
    HEXDUMP("RSA q", cookie->input_array[2], alg_bytesize);
    HEXDUMP("RSA dP", cookie->input_array[3], alg_bytesize);
    HEXDUMP("RSA dQ", cookie->input_array[4], alg_bytesize);
    HEXDUMP("RSA qInv", cookie->input_array[5], alg_bytesize);
  } else if (xform->rsa.key_type == RTE_RSA_KEY_TYPE_EXP) {
    ...
    SET_PKE_LN(cookie->input_array, xform->rsa.d, alg_bytesize, 1);
    SET_PKE_LN(cookie->input_array, xform->rsa.n, alg_bytesize, 2);

    HEXDUMP("RSA d", cookie->input_array[1], alg_bytesize);
    HEXDUMP("RSA n", cookie->input_array[2], alg_bytesize);
  } else {
  ...
  if (asym_op->rsa.op_type == RTE_CRYPTO_ASYM_OP_DECRYPT) {
    switch (asym_op->rsa.pad) {
    case RTE_CRYPTO_RSA_PADDING_NONE:
      SET_PKE_LN(cookie->input_array, asym_op->rsa.cipher, alg_bytesize, 0);
      HEXDUMP("RSA ciphertext", cookie->input_array[0], alg_bytesize);
      break;
    default:
      QAT_LOG(ERR, "Invalid padding of RSA (Decrypt)");
      return -(EINVAL);
    }
    ...
    case RTE_CRYPTO_RSA_PADDING_NONE:
      SET_PKE_LN(cookie->input_array, asym_op->rsa.message, alg_bytesize, 0);
      HEXDUMP("RSA text to be signed", cookie->input_array[0], alg_bytesize);
      break;
    default:
      QAT_LOG(ERR, "Invalid padding of RSA (Signature)");
      return -(EINVAL);
    }
  }
...
static int rsa_set_input(struct rte_crypto_asym_op *asym_op,
                         struct icp_qat_fw_pke_request *qat_req,
                         struct qat_asym_op_cookie *cookie,
                         struct rte_crypto_asym_xform *xform) {
  qat_req->input_param_count = QAT_ASYM_RSA_NUM_IN_PARAMS;
  qat_req->output_param_count = QAT_ASYM_RSA_NUM_OUT_PARAMS;
...
static uint8_t rsa_collect(struct rte_crypto_asym_op *asym_op,
                           struct qat_asym_op_cookie *cookie) {
  uint32_t alg_bytesize = cookie->alg_bytesize;
  ...
  if (asym_op->rsa.op_type == RTE_CRYPTO_ASYM_OP_ENCRYPT ||
    ...
    if (asym_op->rsa.op_type == RTE_CRYPTO_ASYM_OP_ENCRYPT) {
      ...
      rte_memcpy(rsa_result, cookie->output_array[0], alg_bytesize);
      HEXDUMP("RSA Encrypted data", cookie->output_array[0], alg_bytesize);
    } else {
      uint8_t *rsa_result = asym_op->rsa.cipher.data;
      ...
      case RTE_CRYPTO_RSA_PADDING_NONE:
        rte_memcpy(rsa_result, cookie->output_array[0], alg_bytesize);
        HEXDUMP("RSA signature", cookie->output_array[0], alg_bytesize);
        break;
      default:
    ...
    if (asym_op->rsa.op_type == RTE_CRYPTO_ASYM_OP_DECRYPT) {
      ...
      case RTE_CRYPTO_RSA_PADDING_NONE:
        rte_memcpy(rsa_result, cookie->output_array[0], alg_bytesize);
        HEXDUMP("RSA Decrypted Message", cookie->output_array[0], alg_bytesize);
        break;
      default:
        QAT_LOG(ERR, "Padding not supported");
        return RTE_CRYPTO_OP_STATUS_ERROR;
...
static int ecdsa_set_input(struct rte_crypto_asym_op *asym_op,
                           struct icp_qat_fw_pke_request *qat_req,
                           struct qat_asym_op_cookie *cookie,
                           struct rte_crypto_asym_xform *xform) {
  struct qat_asym_function qat_function;
  ...
  case RTE_CRYPTO_ASYM_OP_SIGN:
    ...
    cookie->alg_bytesize = alg_bytesize;
    qat_req->pke_hdr.cd_pars.func_id = func_id;
    qat_req->input_param_count = QAT_ASYM_ECDSA_RS_SIGN_IN_PARAMS;
    qat_req->output_param_count = QAT_ASYM_ECDSA_RS_SIGN_OUT_PARAMS;
  ...
  case RTE_CRYPTO_ASYM_OP_VERIFY:
    ...
    cookie->alg_bytesize = alg_bytesize;
    qat_req->pke_hdr.cd_pars.func_id = func_id;
    qat_req->input_param_count = QAT_ASYM_ECDSA_RS_VERIFY_IN_PARAMS;
    qat_req->output_param_count = QAT_ASYM_ECDSA_RS_VERIFY_OUT_PARAMS;
...
static uint8_t ecdsa_collect(struct rte_crypto_asym_op *asym_op,
                             struct qat_asym_op_cookie *cookie) {
  uint32_t alg_bytesize = cookie->alg_bytesize;
  ...
  if (asym_op->rsa.op_type == RTE_CRYPTO_ASYM_OP_SIGN) {
    ...
    HEXDUMP("R", cookie->output_array[0], alg_bytesize);
    HEXDUMP("S", cookie->output_array[1], alg_bytesize);
  }
  return RTE_CRYPTO_OP_STATUS_SUCCESS;
}

static int ecpm_set_input(struct rte_crypto_asym_op *asym_op,
                          struct icp_qat_fw_pke_request *qat_req,
                          struct qat_asym_op_cookie *cookie,
                          struct rte_crypto_asym_xform *xform) {
  struct qat_asym_function qat_function;
  ...
  SET_PKE_LN_EC(cookie->input_array, curve[curve_id].a, alg_bytesize, 3);
  SET_PKE_LN_EC(cookie->input_array, curve[curve_id].b, alg_bytesize, 4);
  SET_PKE_LN_EC(cookie->input_array, curve[curve_id].p, alg_bytesize, 5);
  SET_PKE_LN_EC(cookie->input_array, curve[curve_id].h, alg_bytesize, 6);

  cookie->alg_bytesize = alg_bytesize;
  qat_req->pke_hdr.cd_pars.func_id = func_id;
  qat_req->input_param_count = QAT_ASYM_ECPM_IN_PARAMS;
  qat_req->output_param_count = QAT_ASYM_ECPM_OUT_PARAMS;
...
static uint8_t ecpm_collect(struct rte_crypto_asym_op *asym_op,
                            struct qat_asym_op_cookie *cookie) {
  ...
  HEXDUMP("rX", cookie->output_array[0], alg_bytesize);
  HEXDUMP("rY", cookie->output_array[1], alg_bytesize);
  return RTE_CRYPTO_OP_STATUS_SUCCESS;
}

static int asym_set_input(struct rte_crypto_asym_op *asym_op,
                          struct icp_qat_fw_pke_request *qat_req,
                          struct qat_asym_op_cookie *cookie,
                          struct rte_crypto_asym_xform *xform) {
  switch (xform->xform_type) {
  case RTE_CRYPTO_ASYM_XFORM_MODEX:
    return modexp_set_input(asym_op, qat_req, cookie, xform);
  case RTE_CRYPTO_ASYM_XFORM_MODINV:
    return modinv_set_input(asym_op, qat_req, cookie, xform);
  case RTE_CRYPTO_ASYM_XFORM_RSA:
    return rsa_set_input(asym_op, qat_req, cookie, xform);
  case RTE_CRYPTO_ASYM_XFORM_ECDSA:
    return ecdsa_set_input(asym_op, qat_req, cookie, xform);
  case RTE_CRYPTO_ASYM_XFORM_ECPM:
    return ecpm_set_input(asym_op, qat_req, cookie, xform);
  default:
...
qat_asym_build_request(void *in_op, uint8_t *out_msg, void *op_cookie,
  ...
  struct qat_asym_op_cookie *cookie = (struct qat_asym_op_cookie *)op_cookie;
  struct rte_crypto_asym_xform *xform;
  struct qat_asym_session *qat_session =
      (struct qat_asym_session *)op->asym->session->sess_private_data;
  int err = 0;
  ...
  err = asym_set_input(asym_op, qat_req, cookie, xform);
  if (err) {
...
static uint8_t qat_asym_collect_response(struct rte_crypto_op *op,
                                         struct qat_asym_op_cookie *cookie,
                                         struct rte_crypto_asym_xform *xform) {
  struct rte_crypto_asym_op *asym_op = op->asym;
...
static int qat_asym_process_response(void **out_op, uint8_t *resp,
                                     void *op_cookie,
                                     __rte_unused uint64_t *dequeue_err_count) {
  struct icp_qat_fw_pke_resp *resp_msg = (struct icp_qat_fw_pke_resp *)resp;
  struct rte_crypto_op *op =
      (struct rte_crypto_op *)(uintptr_t)(resp_msg->opaque);
  ...
  case RTE_CRYPTO_OP_SESSIONLESS:
    ...
    break;
  default:
    QAT_DP_LOG(ERR, "Invalid session/xform settings in response ring!");
    op->status = RTE_CRYPTO_OP_STATUS_INVALID_SESSION;
  }
...
static int session_set_modexp(struct qat_asym_session *qat_session,
                              struct rte_crypto_asym_xform *xform) {
  ...
  qat_session->xform.modex.exponent.data =
      rte_malloc(NULL, xform->modex.exponent.length, 0);
...
static int session_set_modinv(struct qat_asym_session *qat_session,
                              struct rte_crypto_asym_xform *xform) {
...
static int session_set_rsa(struct qat_asym_session *qat_session,
                           struct rte_crypto_asym_xform *xform) {
  ...
  qat_session->xform.rsa.n.data = rte_malloc(NULL, xform->rsa.n.length, 0);
  if (qat_session->xform.rsa.n.data == NULL)
    return -ENOMEM;
  qat_session->xform.rsa.n.length = xform->rsa.n.length;

  qat_session->xform.rsa.e.data = rte_malloc(NULL, xform->rsa.e.length, 0);
  if (qat_session->xform.rsa.e.data == NULL) {
    ret = -ENOMEM;
    goto err;
  }
  qat_session->xform.rsa.e.length = xform->rsa.e.length;
  ...
  if (xform->rsa.key_type == RTE_RSA_KEY_TYPE_QT) {
    ...
    qat_session->xform.rsa.qt.p.length = xform->rsa.qt.p.length;
    ...
    qat_session->xform.rsa.qt.q.length = xform->rsa.qt.q.length;
    ...
    qat_session->xform.rsa.qt.dP.length = xform->rsa.qt.dP.length;
    ...
    qat_session->xform.rsa.qt.dQ.length = xform->rsa.qt.dQ.length;
    ...
    qat_session->xform.rsa.qt.qInv.length = xform->rsa.qt.qInv.length;

    rte_memcpy(qat_session->xform.rsa.qt.p.data, p, xform->rsa.qt.p.length);
    rte_memcpy(qat_session->xform.rsa.qt.q.data, q, xform->rsa.qt.q.length);
    rte_memcpy(qat_session->xform.rsa.qt.dP.data, dP, xform->rsa.qt.dP.length);
    rte_memcpy(qat_session->xform.rsa.qt.dQ.data, dQ, xform->rsa.qt.dQ.length);
    ...
    qat_session->xform.rsa.d.data = rte_malloc(NULL, xform->rsa.d.length, 0);
    if (qat_session->xform.rsa.d.data == NULL) {
      ret = -ENOMEM;
      goto err;
    }
    qat_session->xform.rsa.d.length = xform->rsa.d.length;
    rte_memcpy(qat_session->xform.rsa.d.data, d, xform->rsa.d.length);
  }

  rte_memcpy(qat_session->xform.rsa.n.data, n, xform->rsa.n.length);
  rte_memcpy(qat_session->xform.rsa.e.data, e, xform->rsa.e.length);
...
static void session_set_ecdsa(struct qat_asym_session *qat_session,
                              struct rte_crypto_asym_xform *xform) {
  qat_session->xform.ec.curve_id = xform->ec.curve_id;
}

int qat_asym_session_configure(struct rte_cryptodev *dev __rte_unused,
                               struct rte_crypto_asym_xform *xform,
...
static void session_clear_modexp(struct rte_crypto_modex_xform *modex) {
...
static void session_clear_modinv(struct rte_crypto_modinv_xform *modinv) {
  memset(modinv->modulus.data, 0, modinv->modulus.length);
  rte_free(modinv->modulus.data);
}

static void session_clear_rsa(struct rte_crypto_rsa_xform *rsa) {
...
static void session_clear_xform(struct qat_asym_session *qat_session) {
...
void qat_asym_session_clear(struct rte_cryptodev *dev,
                            struct rte_cryptodev_asym_session *session) {
  void *sess_priv = session->sess_private_data;
  struct qat_asym_session *qat_session = (struct qat_asym_session *)sess_priv;
...
static uint16_t qat_asym_crypto_enqueue_op_burst(void *qp,
                                                 struct rte_crypto_op **ops,
                                                 uint16_t nb_ops) {
  return qat_enqueue_op_burst(qp, qat_asym_build_request, (void **)ops, nb_ops);
}

static uint16_t qat_asym_crypto_dequeue_op_burst(void *qp,
                                                 struct rte_crypto_op **ops,
                                                 uint16_t nb_ops) {
...
int qat_asym_dev_create(struct qat_pci_device *qat_pci_dev,
  ...
  snprintf(name, RTE_CRYPTODEV_NAME_MAX_LEN, "%s_%s", qat_pci_dev->name,
           "asym");
  QAT_LOG(DEBUG, "Creating QAT ASYM device %s\n", name);

  if (gen_dev_ops->cryptodev_ops == NULL) {
    QAT_LOG(ERR, "Device %s does not support asymmetric crypto", name);
    return -(EFAULT);
  }

  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    qat_pci_dev->qat_asym_driver_id = qat_asym_driver_id;
  } else if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    if (qat_pci_dev->qat_asym_driver_id != qat_asym_driver_id) {
      QAT_LOG(ERR,
              "Device %s have different driver id than corresponding device in "
              "primary process",
              name);
      return -(EFAULT);
    }
  ...
  cryptodev = rte_cryptodev_pmd_create(name, &(qat_dev_instance->asym_rte_dev),
                                       &init_params);
  ...
  snprintf(capa_memz_name, RTE_CRYPTODEV_NAME_MAX_LEN, "QAT_ASYM_CAPA_GEN_%d",
  ...
  if (internals->capa_mz == NULL) {
    internals->capa_mz =
        rte_memzone_reserve(capa_memz_name, capa_size, rte_socket_id(), 0);
    if (internals->capa_mz == NULL) {
      QAT_LOG(DEBUG,
...
int qat_asym_dev_destroy(struct qat_pci_device *qat_pci_dev) {
  struct rte_cryptodev *cryptodev;
  ...
  /* free crypto device */
  cryptodev = rte_cryptodev_pmd_get_dev(qat_pci_dev->asym_dev->dev_id);
  rte_cryptodev_pmd_destroy(cryptodev);
...
RTE_PMD_REGISTER_CRYPTO_DRIVER(qat_crypto_drv, cryptodev_qat_asym_driver,
                               qat_asym_driver_id);
