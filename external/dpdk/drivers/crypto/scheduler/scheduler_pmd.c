...
static const char *const scheduler_valid_params[] = {
    RTE_CRYPTODEV_VDEV_NAME,      RTE_CRYPTODEV_VDEV_WORKER,
    RTE_CRYPTODEV_VDEV_MODE,      RTE_CRYPTODEV_VDEV_MODE_PARAM,
    RTE_CRYPTODEV_VDEV_ORDERING,  RTE_CRYPTODEV_VDEV_MAX_NB_QP_ARG,
    RTE_CRYPTODEV_VDEV_SOCKET_ID, RTE_CRYPTODEV_VDEV_COREMASK,
...
const struct scheduler_parse_map scheduler_mode_map[] = {
    {RTE_STR(SCHEDULER_MODE_NAME_ROUND_ROBIN), CDEV_SCHED_MODE_ROUNDROBIN},
    {RTE_STR(SCHEDULER_MODE_NAME_PKT_SIZE_DISTR),
     CDEV_SCHED_MODE_PKT_SIZE_DISTR},
    {RTE_STR(SCHEDULER_MODE_NAME_FAIL_OVER), CDEV_SCHED_MODE_FAILOVER},
    {RTE_STR(SCHEDULER_MODE_NAME_MULTI_CORE), CDEV_SCHED_MODE_MULTICORE}};

const struct scheduler_parse_map scheduler_ordering_map[] = {{"enable", 1},
                                                             {"disable", 0}};

#define CDEV_SCHED_MODE_PARAM_SEP_CHAR ':'

static int
cryptodev_scheduler_create(const char *name, struct rte_vdev_device *vdev,
  ...
  dev = rte_cryptodev_pmd_create(name, &vdev->device, &init_params->def_p);
  if (dev == NULL) {
    CR_SCHED_LOG(ERR, "driver %s: failed to create cryptodev vdev", name);
    return -EFAULT;
  }
  ...
  sched_ctx->max_nb_queue_pairs = init_params->def_p.max_nb_queue_pairs;
  ...
  if (init_params->mode == CDEV_SCHED_MODE_MULTICORE) {
    ...
    for (i = 0; i < sched_ctx->nb_wc; i++) {
      ...
      CR_SCHED_LOG(INFO, "  Worker core[%u]=%u added", i,
                   sched_ctx->wc_pool[i]);
    }
  }
  ...
  if (init_params->mode > CDEV_SCHED_MODE_USERDEFINED &&
    ...
    ret =
        rte_cryptodev_scheduler_mode_set(dev->data->dev_id, init_params->mode);
    if (ret < 0) {
    ...
    for (i = 0; i < RTE_DIM(scheduler_mode_map); i++) {
      ...
      CR_SCHED_LOG(INFO, "  Scheduling mode = %s", scheduler_mode_map[i].name);
      break;
    }

    if (strlen(init_params->mode_param_str) > 0) {
      s = strchr(init_params->mode_param_str, CDEV_SCHED_MODE_PARAM_SEP_CHAR);
      if (s == NULL) {
      ...
      s++;
      strlcpy(param_val, s, RTE_CRYPTODEV_SCHEDULER_NAME_MAX_LEN);

      switch (init_params->mode) {
      case CDEV_SCHED_MODE_PKT_SIZE_DISTR:
        if (strcmp(param_name, RTE_CRYPTODEV_SCHEDULER_PARAM_THRES) != 0) {
          CR_SCHED_LOG(ERR, "Invalid mode param");
          return -EINVAL;
        }
        option_type = CDEV_SCHED_OPTION_THRESHOLD;

        option.threshold_option.threshold = strtoul(param_val, &end, 0);
        break;
      default:
        CR_SCHED_LOG(ERR, "Invalid mode param");
        return -EINVAL;
      ...
      if (sched_ctx->ops.option_set(dev, option_type, (void *)&option) < 0) {
        CR_SCHED_LOG(ERR, "Invalid mode param");
        return -EINVAL;
      }
  ...
  for (i = 0; i < RTE_DIM(scheduler_ordering_map); i++) {
    if (scheduler_ordering_map[i].val != sched_ctx->reordering_enabled)
      continue;
  ...
  for (i = 0; i < init_params->nb_workers; i++) {
    sched_ctx->init_worker_names[sched_ctx->nb_init_workers] =
        rte_zmalloc_socket(NULL, RTE_CRYPTODEV_SCHEDULER_NAME_MAX_LEN, 0,
                           SOCKET_ID_ANY);

    if (!sched_ctx->init_worker_names[sched_ctx->nb_init_workers]) {
      CR_SCHED_LOG(ERR, "driver %s: Insufficient memory", name);
      return -ENOMEM;
    }

    strncpy(sched_ctx->init_worker_names[sched_ctx->nb_init_workers],
   ...
   */
  sched_ctx->capabilities = rte_zmalloc_socket(
      NULL, sizeof(struct rte_cryptodev_capabilities), 0, SOCKET_ID_ANY);
...
static int cryptodev_scheduler_remove(struct rte_vdev_device *vdev) {
  const char *name;
  struct rte_cryptodev *dev;
  struct scheduler_ctx *sched_ctx;
...
static int parse_coremask_arg(const char *key __rte_unused, const char *value,
                              void *extra_args) {
  int i, j, val;
  uint16_t idx = 0;
  char c;
...
static int parse_corelist_arg(const char *key __rte_unused, const char *value,
                              void *extra_args) {
  struct scheduler_init_params *params = extra_args;
  ...
  while (isdigit(token[0])) {
    ...
    if (core >= RTE_MAX_LCORE) {
      CR_SCHED_LOG(ERR,
                   "Invalid worker core %u, should be smaller "
                   "than %u.",
                   core, RTE_MAX_LCORE);
    }
...
/** Parse worker */
static int parse_worker_arg(const char *key __rte_unused, const char *value,
                            void *extra_args) {
...
static int parse_mode_arg(const char *key __rte_unused, const char *value,
                          void *extra_args) {
  struct scheduler_init_params *param = extra_args;
  uint32_t i;
  ...
  for (i = 0; i < RTE_DIM(scheduler_mode_map); i++) {
    if (strcmp(value, scheduler_mode_map[i].name) == 0) {
      param->mode =
          (enum rte_cryptodev_scheduler_mode)scheduler_mode_map[i].val;
...
static int parse_mode_param_arg(const char *key __rte_unused, const char *value,
                                void *extra_args) {
  struct scheduler_init_params *param = extra_args;

  strlcpy(param->mode_param_str, value, RTE_CRYPTODEV_SCHEDULER_NAME_MAX_LEN);

  return 0;
}

static int parse_ordering_arg(const char *key __rte_unused, const char *value,
                              void *extra_args) {
  struct scheduler_init_params *param = extra_args;
  uint32_t i;
  ...
  for (i = 0; i < RTE_DIM(scheduler_ordering_map); i++) {
    if (strcmp(value, scheduler_ordering_map[i].name) == 0) {
      param->enable_ordering = scheduler_ordering_map[i].val;
      break;
    }
  }
...
static int scheduler_parse_init_params(struct scheduler_init_params *params,
  ...
  if (input_args) {
    kvlist = rte_kvargs_parse(input_args, scheduler_valid_params);
    if (kvlist == NULL)
      return -1;

    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_VDEV_MAX_NB_QP_ARG,
                             &parse_integer_arg,
    ...
    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_VDEV_SOCKET_ID,
                             &parse_integer_arg, &params->def_p.socket_id);
    if (ret < 0)
      goto free_kvlist;

    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_VDEV_COREMASK,
                             &parse_coremask_arg, params);
    if (ret < 0)
      goto free_kvlist;

    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_VDEV_CORELIST,
                             &parse_corelist_arg, params);
    if (ret < 0)
      goto free_kvlist;

    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_VDEV_NAME, &parse_name_arg,
                             &params->def_p);
    if (ret < 0)
      goto free_kvlist;
    ...
    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_VDEV_MODE, &parse_mode_arg,
                             params);
    if (ret < 0)
      goto free_kvlist;
...
static int cryptodev_scheduler_probe(struct rte_vdev_device *vdev) {
  struct scheduler_init_params init_params = {
      .def_p = {"", sizeof(struct scheduler_ctx), rte_socket_id(),
  ...
  scheduler_parse_init_params(&init_params, rte_vdev_device_args(vdev));

  return cryptodev_scheduler_create(name, vdev, &init_params);
}

static struct rte_vdev_driver cryptodev_scheduler_pmd_drv = {
    .probe = cryptodev_scheduler_probe, .remove = cryptodev_scheduler_remove};
