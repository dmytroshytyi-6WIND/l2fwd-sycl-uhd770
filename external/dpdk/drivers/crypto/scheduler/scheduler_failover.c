...
failover_worker_enqueue(struct scheduler_worker *worker,
  ...
  processed_ops =
      rte_cryptodev_enqueue_burst(worker->dev_id, worker->qp_id, ops, nb_ops);
...
static uint16_t schedule_enqueue(void *qp, struct rte_crypto_op **ops,
  ...
  enqueued_ops = failover_worker_enqueue(&qp_ctx->primary_worker, ops, nb_ops);

  if (enqueued_ops < nb_ops)
    enqueued_ops += failover_worker_enqueue(
        &qp_ctx->secondary_worker, &ops[enqueued_ops], nb_ops - enqueued_ops);
...
static uint16_t schedule_dequeue(void *qp, struct rte_crypto_op **ops,
  ...
  if (worker->nb_inflight_cops) {
    nb_deq_ops2 = rte_cryptodev_dequeue_burst(
        worker->dev_id, worker->qp_id, &ops[nb_deq_ops], nb_ops - nb_deq_ops);
...
static int scheduler_start(struct rte_cryptodev *dev) {
  ...
  for (i = 0; i < dev->data->nb_queue_pairs; i++) {
    struct fo_scheduler_qp_ctx *qp_ctx =
        ((struct scheduler_qp_ctx *)dev->data->queue_pairs[i])->private_qp_ctx;

    rte_memcpy(&qp_ctx->primary_worker, &sched_ctx->workers[PRIMARY_WORKER_IDX],
...
static int scheduler_stop(__rte_unused struct rte_cryptodev *dev) { return 0; }

static int scheduler_config_qp(struct rte_cryptodev *dev, uint16_t qp_id) {
  struct scheduler_qp_ctx *qp_ctx = dev->data->queue_pairs[qp_id];
  struct fo_scheduler_qp_ctx *fo_qp_ctx;

  fo_qp_ctx = rte_zmalloc_socket(NULL, sizeof(*fo_qp_ctx), 0, rte_socket_id());
  if (!fo_qp_ctx) {
