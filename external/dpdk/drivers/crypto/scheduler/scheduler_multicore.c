...
static uint16_t schedule_enqueue(void *qp, struct rte_crypto_op **ops,
                                 uint16_t nb_ops) {
  ...
  for (i = 0; i < mc_ctx->num_workers && nb_ops != 0; i++) {
    ...
    uint16_t nb_queue_ops = rte_ring_enqueue_burst(
        enq_ring, (void *)(&ops[processed_ops]), nb_ops, NULL);
...
static uint16_t schedule_enqueue_ordering(void *qp, struct rte_crypto_op **ops,
                                          uint16_t nb_ops) {
  struct rte_ring *order_ring = ((struct scheduler_qp_ctx *)qp)->order_ring;
  uint16_t nb_ops_to_enq = get_max_enqueue_order_count(order_ring, nb_ops);
  uint16_t nb_ops_enqd = schedule_enqueue(qp, ops, nb_ops_to_enq);
...
static uint16_t schedule_dequeue(void *qp, struct rte_crypto_op **ops,
                                 uint16_t nb_ops) {
  struct mc_scheduler_qp_ctx *mc_qp_ctx =
      ((struct scheduler_qp_ctx *)qp)->private_qp_ctx;
  struct mc_scheduler_ctx *mc_ctx = mc_qp_ctx->mc_private_ctx;
  ...
  for (i = 0; i < mc_ctx->num_workers && nb_ops != 0; i++) {
    ...
    uint16_t nb_deq_ops = rte_ring_dequeue_burst(
        deq_ring, (void *)(&ops[processed_ops]), nb_ops, NULL);
...
static uint16_t schedule_dequeue_ordering(void *qp, struct rte_crypto_op **ops,
                                          uint16_t nb_ops) {
  struct rte_ring *order_ring = ((struct scheduler_qp_ctx *)qp)->order_ring;
  ...
  nb_objs =
      rte_ring_dequeue_burst_start(order_ring, (void **)ops, nb_ops, NULL);
  if (nb_objs == 0)
    return 0;
...
static int worker_attach(__rte_unused struct rte_cryptodev *dev,
                         __rte_unused uint8_t worker_id) {
  return 0;
}

static int worker_detach(__rte_unused struct rte_cryptodev *dev,
                         __rte_unused uint8_t worker_id) {
  return 0;
}

static int mc_scheduler_worker(struct rte_cryptodev *dev) {
  ...
  if (worker_idx == -1) {
    CR_SCHED_LOG(ERR, "worker on core %u:cannot find worker index!", core_id);
    return -1;
  }
  ...
  while (!mc_ctx->stop_signal) {
    if (pending_enq_ops) {
      processed_ops = rte_cryptodev_enqueue_burst(worker->dev_id, worker->qp_id,
      ...
      if (processed_ops) {
        pending_enq_ops_idx = rte_cryptodev_enqueue_burst(
            worker->dev_id, worker->qp_id, enq_ops, processed_ops);
    ...
    if (pending_deq_ops) {
      processed_ops = rte_ring_enqueue_burst(
          deq_ring, (void *)&deq_ops[pending_deq_ops_idx], pending_deq_ops,
          NULL);
      ...
      processed_ops = rte_cryptodev_dequeue_burst(
          worker->dev_id, worker->qp_id, deq_ops, MC_SCHED_BUFFER_SIZE);
      if (processed_ops) {
        inflight_ops -= processed_ops;
        if (reordering_enabled) {
          ...
          for (j = 0; j < processed_ops; j++) {
            deq_ops[j]->status |= CRYPTO_OP_STATUS_BIT_COMPLETE;
          }
        } else {
          pending_deq_ops_idx = rte_ring_enqueue_burst(
              deq_ring, (void *)deq_ops, processed_ops, NULL);
          pending_deq_ops = processed_ops - pending_deq_ops_idx;
        }
      }
    }
...
static int scheduler_start(struct rte_cryptodev *dev) {
  struct scheduler_ctx *sched_ctx = dev->data->dev_private;
  struct mc_scheduler_ctx *mc_ctx = sched_ctx->private_ctx;
  uint16_t i;
  ...
  for (i = 0; i < sched_ctx->nb_wc; i++)
    rte_eal_remote_launch((lcore_function_t *)mc_scheduler_worker, dev,
                          sched_ctx->wc_pool[i]);
  ...
  for (i = 0; i < dev->data->nb_queue_pairs; i++) {
    ...
    struct mc_scheduler_qp_ctx *mc_qp_ctx = qp_ctx->private_qp_ctx;
    uint32_t j;
    ...
    for (j = 0; j < sched_ctx->nb_workers; j++) {
      mc_qp_ctx->workers[j].dev_id = sched_ctx->workers[j].dev_id;
...
static int scheduler_stop(struct rte_cryptodev *dev) {
  struct scheduler_ctx *sched_ctx = dev->data->dev_private;
  struct mc_scheduler_ctx *mc_ctx = sched_ctx->private_ctx;
  uint16_t i;
...
static int scheduler_config_qp(struct rte_cryptodev *dev, uint16_t qp_id) {
  struct scheduler_qp_ctx *qp_ctx = dev->data->queue_pairs[qp_id];
  struct mc_scheduler_qp_ctx *mc_qp_ctx;
  struct scheduler_ctx *sched_ctx = dev->data->dev_private;
  struct mc_scheduler_ctx *mc_ctx = sched_ctx->private_ctx;

  mc_qp_ctx = rte_zmalloc_socket(NULL, sizeof(*mc_qp_ctx), 0, rte_socket_id());
  if (!mc_qp_ctx) {
...
static int scheduler_create_private_ctx(struct rte_cryptodev *dev) {
  struct scheduler_ctx *sched_ctx = dev->data->dev_private;
  ...
  for (i = 0; i < sched_ctx->nb_wc; i++) {
    ...
    snprintf(r_name, sizeof(r_name), MC_SCHED_ENQ_RING_NAME_PREFIX "%u_%u",
             dev->data->dev_id, i);
    mc_ctx->sched_enq_ring[i] = rte_ring_lookup(r_name);
    if (!mc_ctx->sched_enq_ring[i]) {
      mc_ctx->sched_enq_ring[i] =
          rte_ring_create(r_name, PER_WORKER_BUFF_SIZE, rte_socket_id(),
                          RING_F_SC_DEQ | RING_F_SP_ENQ);
      if (!mc_ctx->sched_enq_ring[i]) {
        CR_SCHED_LOG(ERR, "Cannot create ring for worker %u", i);
        goto exit;
      }
    }
    snprintf(r_name, sizeof(r_name), MC_SCHED_DEQ_RING_NAME_PREFIX "%u_%u",
             dev->data->dev_id, i);
