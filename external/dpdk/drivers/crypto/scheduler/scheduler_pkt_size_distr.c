...
static uint16_t schedule_enqueue(void *qp, struct rte_crypto_op **ops,
  ...
  for (i = 0; (i < (nb_ops - 8)) && (nb_ops > 8); i += 4) {
    ...
    job_len +=
        (ops[i]->sym->cipher.data.length == 0) * ops[i]->sym->auth.data.length;
  ...
  processed_ops_pri = rte_cryptodev_enqueue_burst(
      psd_qp_ctx->primary_worker.dev_id, psd_qp_ctx->primary_worker.qp_id,
      sched_ops[PRIMARY_WORKER_IDX], enq_ops[PRIMARY_WORKER_IDX].pos);
  ...
  processed_ops_sec = rte_cryptodev_enqueue_burst(
      psd_qp_ctx->secondary_worker.dev_id, psd_qp_ctx->secondary_worker.qp_id,
      sched_ops[SECONDARY_WORKER_IDX], enq_ops[SECONDARY_WORKER_IDX].pos);
...
static uint16_t schedule_dequeue(void *qp, struct rte_crypto_op **ops,
  ...
  if (worker->nb_inflight_cops) {
    nb_deq_ops_pri =
        rte_cryptodev_dequeue_burst(worker->dev_id, worker->qp_id, ops, nb_ops);
  ...
  if (worker->nb_inflight_cops) {
    nb_deq_ops_sec = rte_cryptodev_dequeue_burst(worker->dev_id, worker->qp_id,
                                                 &ops[nb_deq_ops_pri],
...
static int scheduler_config_qp(struct rte_cryptodev *dev, uint16_t qp_id) {
  ...
  ps_qp_ctx = rte_zmalloc_socket(NULL, sizeof(*ps_qp_ctx), 0, rte_socket_id());
  if (!ps_qp_ctx) {
...
static int scheduler_option_set(struct rte_cryptodev *dev, uint32_t option_type,
                                void *option) {
  struct psd_scheduler_ctx *psd_ctx =
      ((struct scheduler_ctx *)dev->data->dev_private)->private_ctx;
  uint32_t threshold;
  ...
  threshold =
      ((struct rte_cryptodev_scheduler_threshold_option *)option)->threshold;
...
static int scheduler_option_get(struct rte_cryptodev *dev, uint32_t option_type,
                                void *option) {
  struct psd_scheduler_ctx *psd_ctx =
      ((struct scheduler_ctx *)dev->data->dev_private)->private_ctx;
...
static struct rte_cryptodev_scheduler_ops scheduler_ps_ops = {
    worker_attach,        worker_detach,       scheduler_start,
    scheduler_stop,       scheduler_config_qp, scheduler_create_private_ctx,
    scheduler_option_set, scheduler_option_get};
