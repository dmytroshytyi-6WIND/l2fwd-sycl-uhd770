...
static int scheduler_attach_init_worker(struct rte_cryptodev *dev) {
  ...
  for (i = sched_ctx->nb_init_workers - 1; i >= 0; i--) {
    ...
    if (!worker_dev) {
      CR_SCHED_LOG(ERR, "Failed to locate worker dev %s", dev_name);
      return -EINVAL;
    }

    status = rte_cryptodev_scheduler_worker_attach(scheduler_id,
    ...
    CR_SCHED_LOG(INFO, "Scheduler %s attached worker %s", dev->data->name,
...
/** Configure device */
static int scheduler_pmd_config(struct rte_cryptodev *dev,
...
static int update_order_ring(struct rte_cryptodev *dev, uint16_t qp_id) {
  struct scheduler_ctx *sched_ctx = dev->data->dev_private;
  ...
  if (sched_ctx->reordering_enabled) {
    ...
    uint32_t buff_size =
        rte_align32pow2(sched_ctx->nb_workers * PER_WORKER_BUFF_SIZE);
    ...
    if (snprintf(order_ring_name, RTE_CRYPTODEV_NAME_MAX_LEN, "%s_rb_%u_%u",
                 RTE_STR(CRYPTODEV_NAME_SCHEDULER_PMD), dev->data->dev_id,
                 qp_id) < 0) {
    ...
    qp_ctx->order_ring =
        rte_ring_create(order_ring_name, buff_size, rte_socket_id(),
...
/** Start device */
static int scheduler_pmd_start(struct rte_cryptodev *dev) {
  struct scheduler_ctx *sched_ctx = dev->data->dev_private;
  uint32_t i;
  int ret;
  ...
  for (i = 0; i < sched_ctx->nb_workers; i++) {
    ...
    if (ret < 0) {
      CR_SCHED_LOG(ERR, "Failed to start worker dev %u", worker_dev_id);
      return ret;
    }
  }
...
/** Stop device */
static void scheduler_pmd_stop(struct rte_cryptodev *dev) {
  struct scheduler_ctx *sched_ctx = dev->data->dev_private;
  uint32_t i;
...
/** Close device */
static int scheduler_pmd_close(struct rte_cryptodev *dev) {
  struct scheduler_ctx *sched_ctx = dev->data->dev_private;
  uint32_t i;
  int ret;
  ...
  for (i = 0; i < sched_ctx->nb_workers; i++) {
    uint8_t worker_dev_id = sched_ctx->workers[i].dev_id;
    struct rte_cryptodev *worker_dev = rte_cryptodev_pmd_get_dev(worker_dev_id);
...
static void scheduler_pmd_stats_get(struct rte_cryptodev *dev,
...
static void scheduler_pmd_stats_reset(struct rte_cryptodev *dev) {
  struct scheduler_ctx *sched_ctx = dev->data->dev_private;
  uint32_t i;
...
/** Get device info */
static void scheduler_pmd_info_get(struct rte_cryptodev *dev,
  ...
  for (i = 0; i < sched_ctx->nb_workers; i++) {
    ...
    headroom_sz = worker_info.min_mbuf_headroom_req > headroom_sz
                      ? worker_info.min_mbuf_headroom_req
                      : headroom_sz;

    /* Get the max tailroom requirement among worker PMDs */
    tailroom_sz = worker_info.min_mbuf_tailroom_req > tailroom_sz
...
/** Release queue pair */
static int scheduler_pmd_qp_release(struct rte_cryptodev *dev, uint16_t qp_id) {
  struct scheduler_qp_ctx *qp_ctx = dev->data->queue_pairs[qp_id];
...
/** Setup a queue pair */
static int scheduler_pmd_qp_setup(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  if (snprintf(name, RTE_CRYPTODEV_NAME_MAX_LEN, "CRYTO_SCHE PMD %u QP %u",
  ...
  for (i = 0; i < sched_ctx->nb_workers; i++) {
     ...
     */
    ret = rte_cryptodev_queue_pair_setup(worker_id, qp_id, qp_conf, socket_id);
    if (ret < 0)
      return ret;
  }
  ...
  qp_ctx =
      rte_zmalloc_socket(name, sizeof(*qp_ctx), RTE_CACHE_LINE_SIZE, socket_id);
  if (qp_ctx == NULL)
    return -ENOMEM;
...
static int scheduler_pmd_sym_session_configure(
  ...
  for (i = 0; i < sched_ctx->nb_workers; i++) {
    ...
    ret = rte_cryptodev_sym_session_init(worker->dev_id, sess, xform, mempool);
    if (ret < 0) {
