 ...
 **/
static uint32_t
sync_caps(struct rte_cryptodev_capabilities *caps, uint32_t nb_caps,
  ...
  for (i = 0; i < sync_nb_caps; i++) {
    ...
    for (j = 0; j < nb_worker_caps; j++) {
      const struct rte_cryptodev_capabilities *s_cap = &worker_caps[j];

      if (s_cap->op != cap->op || s_cap->sym.xform_type != cap->sym.xform_type)
        continue;

      if (s_cap->sym.xform_type == RTE_CRYPTO_SYM_XFORM_AUTH) {
        if (s_cap->sym.auth.algo != cap->sym.auth.algo)
          continue;

        cap->sym.auth.digest_size.min =
            s_cap->sym.auth.digest_size.min < cap->sym.auth.digest_size.min
                ? s_cap->sym.auth.digest_size.min
                : cap->sym.auth.digest_size.min;
        cap->sym.auth.digest_size.max =
            s_cap->sym.auth.digest_size.max < cap->sym.auth.digest_size.max
                ? s_cap->sym.auth.digest_size.max
                : cap->sym.auth.digest_size.max;
      }

      if (s_cap->sym.xform_type == RTE_CRYPTO_SYM_XFORM_CIPHER)
        if (s_cap->sym.cipher.algo != cap->sym.cipher.algo)
          continue;
...
static int update_scheduler_capability(struct scheduler_ctx *sched_ctx) {
  ...
  sched_ctx->capabilities = rte_zmalloc_socket(
      NULL, sizeof(struct rte_cryptodev_capabilities) * (nb_caps + 1), 0,
      SOCKET_ID_ANY);
  if (!sched_ctx->capabilities)
    return -ENOMEM;
...
static void update_scheduler_feature_flag(struct rte_cryptodev *dev) {
...
static void update_max_nb_qp(struct scheduler_ctx *sched_ctx) {
  uint32_t i;
  uint32_t max_nb_qp;
...
int rte_cryptodev_scheduler_worker_attach(uint8_t scheduler_id,
  ...
  if (sched_ctx->nb_workers >= RTE_CRYPTODEV_SCHEDULER_MAX_NB_WORKERS) {
...
int rte_cryptodev_scheduler_mode_set(uint8_t scheduler_id,
  ...
  case CDEV_SCHED_MODE_ROUNDROBIN:
    if (rte_cryptodev_scheduler_load_user_scheduler(
            scheduler_id, crypto_scheduler_roundrobin) < 0) {
  ...
  case CDEV_SCHED_MODE_PKT_SIZE_DISTR:
    if (rte_cryptodev_scheduler_load_user_scheduler(
            scheduler_id, crypto_scheduler_pkt_size_based_distr) < 0) {
      CR_SCHED_LOG(ERR, "Failed to load scheduler");
      return -1;
    }
    break;
  case CDEV_SCHED_MODE_FAILOVER:
    if (rte_cryptodev_scheduler_load_user_scheduler(
            scheduler_id, crypto_scheduler_failover) < 0) {
      CR_SCHED_LOG(ERR, "Failed to load scheduler");
      return -1;
    }
    break;
  case CDEV_SCHED_MODE_MULTICORE:
    if (rte_cryptodev_scheduler_load_user_scheduler(
            scheduler_id, crypto_scheduler_multicore) < 0) {
      CR_SCHED_LOG(ERR, "Failed to load scheduler");
      return -1;
    }
    break;
...
int rte_cryptodev_scheduler_load_user_scheduler(
    uint8_t scheduler_id, struct rte_cryptodev_scheduler *scheduler) {
  ...
  if (strlen(scheduler->name) > RTE_CRYPTODEV_NAME_MAX_LEN - 1) {
    CR_SCHED_LOG(ERR,
                 ...
                 "%u bytes.",
                 scheduler->name, RTE_CRYPTODEV_NAME_MAX_LEN);
    return -EINVAL;
  }
...
int rte_cryptodev_scheduler_option_set(
    uint8_t scheduler_id, enum rte_cryptodev_schedule_option_type option_type,
    void *option) {
  struct rte_cryptodev *dev = rte_cryptodev_pmd_get_dev(scheduler_id);
  struct scheduler_ctx *sched_ctx;
...
int rte_cryptodev_scheduler_option_get(
    uint8_t scheduler_id, enum rte_cryptodev_schedule_option_type option_type,
    void *option) {
  struct rte_cryptodev *dev = rte_cryptodev_pmd_get_dev(scheduler_id);
  struct scheduler_ctx *sched_ctx;
