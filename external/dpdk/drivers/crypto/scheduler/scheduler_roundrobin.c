...
static uint16_t schedule_dequeue(void *qp, struct rte_crypto_op **ops,
  ...
  if (unlikely(rr_qp_ctx->workers[last_worker_idx].nb_inflight_cops == 0)) {
    do {
      last_worker_idx += 1;
      ...
      if (last_worker_idx == rr_qp_ctx->last_deq_worker_idx)
        return 0;
    } while (rr_qp_ctx->workers[last_worker_idx].nb_inflight_cops == 0);
  }
...
static int scheduler_start(struct rte_cryptodev *dev) {
  ...
  for (i = 0; i < dev->data->nb_queue_pairs; i++) {
    ...
    for (j = 0; j < sched_ctx->nb_workers; j++) {
      rr_qp_ctx->workers[j].dev_id = sched_ctx->workers[j].dev_id;
...
static int scheduler_config_qp(struct rte_cryptodev *dev, uint16_t qp_id) {
  ...
  rr_qp_ctx = rte_zmalloc_socket(NULL, sizeof(*rr_qp_ctx), 0, rte_socket_id());
  if (!rr_qp_ctx) {
