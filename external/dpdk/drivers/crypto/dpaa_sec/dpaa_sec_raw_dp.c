...
static struct dpaa_sec_job *build_dpaa_raw_dp_auth_fd(
    uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
    struct rte_crypto_sgl *dest_sgl, struct rte_crypto_va_iova_ptr *iv,
    struct rte_crypto_va_iova_ptr *digest,
    struct rte_crypto_va_iova_ptr *auth_iv, union rte_crypto_sym_ofs ofs,
    void *userdata, struct qm_fd *fd) {
  ...
  if (ses->iv.length) {
    ...
    iv_ptr = rte_crypto_op_ctod_offset(userdata, uint8_t *, ses->iv.offset);
  ...
  if (is_decode(ses)) {
    ...
    cpu_to_hw_sg(sg);
    sg++;
    rte_memcpy(old_digest, digest->va, ses->digest_length);
...
static inline struct dpaa_sec_job *build_raw_cipher_auth_gcm_sg(
    uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
    struct rte_crypto_sgl *dest_sgl, struct rte_crypto_va_iova_ptr *iv,
    struct rte_crypto_va_iova_ptr *digest,
    struct rte_crypto_va_iova_ptr *auth_iv, union rte_crypto_sym_ofs ofs,
  ...
  in_sg->final = 1;
  if (is_encode(ses))
    in_sg->length = ses->iv.length + aead_len + ses->auth_only_len;
  else
    in_sg->length =
        ses->iv.length + aead_len + ses->auth_only_len + ses->digest_length;
  ...
  if (is_decode(ses)) {
    cpu_to_hw_sg(sg);
    sg++;
    memcpy(ctx->digest, digest->va, ses->digest_length);
...
static inline struct dpaa_sec_job *build_dpaa_raw_dp_chain_fd(
    uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
    struct rte_crypto_sgl *dest_sgl, struct rte_crypto_va_iova_ptr *iv,
    struct rte_crypto_va_iova_ptr *digest,
    struct rte_crypto_va_iova_ptr *auth_iv, union rte_crypto_sym_ofs ofs,
  ...
  uint8_t *IV_ptr = iv->va;
  unsigned int i;
  uint16_t auth_hdr_len = ofs.ofs.cipher.head - ofs.ofs.auth.head;
  ...
  if (is_encode(ses))
    ...
    in_sg->length = ses->iv.length + auth_len + ses->digest_length;
...
static struct dpaa_sec_job *build_dpaa_raw_dp_cipher_fd(
    uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
    struct rte_crypto_sgl *dest_sgl, struct rte_crypto_va_iova_ptr *iv,
    struct rte_crypto_va_iova_ptr *digest,
    struct rte_crypto_va_iova_ptr *auth_iv, union rte_crypto_sym_ofs ofs,
...
static inline struct dpaa_sec_job *build_dpaa_raw_proto_sg(
    uint8_t *drv_ctx, struct rte_crypto_sgl *sgl,
    struct rte_crypto_sgl *dest_sgl, struct rte_crypto_va_iova_ptr *iv,
    struct rte_crypto_va_iova_ptr *digest,
    struct rte_crypto_va_iova_ptr *auth_iv, union rte_crypto_sym_ofs ofs,
...
static uint32_t dpaa_sec_raw_enqueue_burst(void *qp_data, uint8_t *drv_ctx,
  ...
  while (vec->num) {
    frames_to_send = (vec->num > DPAA_SEC_BURST) ? DPAA_SEC_BURST : vec->num;
    for (loop = 0; loop < frames_to_send; loop++) {
      ...
      } else if (unlikely(ses->qp[rte_lcore_id() % MAX_DPAA_CORES] !=
                          dpaa_qp)) {
        DPAA_SEC_DP_ERR("Old:sess->qp = %p"
                        " New qp = %p\n",
                        ses->qp[rte_lcore_id() % MAX_DPAA_CORES], dpaa_qp);
        frames_to_send = loop;
        goto send_pkts;
      }
      ...
      cf = ses->build_raw_dp_fd(
          drv_ctx, &vec->src_sgl[loop], &vec->dest_sgl[loop], &vec->iv[loop],
          &vec->digest[loop], &vec->auth_iv[loop], ofs, user_data[loop], fd);
      if (!cf) {
...
static int dpaa_sec_deq_raw(struct dpaa_sec_qp *qp, void **out_user_data,
...
static __rte_always_inline uint32_t dpaa_sec_raw_dequeue_burst(
  ...
  num_rx = dpaa_sec_deq_raw(dpaa_qp, out_user_data, is_user_data_array,
                            post_dequeue, nb_ops);
...
static __rte_always_inline int dpaa_sec_raw_enqueue(
    void *qp_data, uint8_t *drv_ctx, struct rte_crypto_vec *data_vec,
...
int dpaa_sec_configure_raw_dp_ctx(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  if (sess_type == RTE_CRYPTO_OP_SECURITY_SESSION)
    sess =
        (dpaa_sec_session *)get_sec_session_private_data(session_ctx.sec_sess);
...
#ifdef RTE_LIBRTE_SECURITY
  else if (sess->ctxt == DPAA_SEC_IPSEC || sess->ctxt == DPAA_SEC_PDCP)
...
int dpaa_sec_get_dp_ctx_size(__rte_unused struct rte_cryptodev *dev) {
