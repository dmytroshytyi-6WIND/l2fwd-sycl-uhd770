...
static inline struct dpaa_sec_op_ctx *dpaa_sec_alloc_ctx(dpaa_sec_session *ses,
                                                         int sg_count) {
  struct dpaa_sec_op_ctx *ctx;
  int i, retval;

  retval = rte_mempool_get(ses->qp[rte_lcore_id() % MAX_DPAA_CORES]->ctx_pool,
                           (void **)(&ctx));
  if (!ctx || retval) {
 ...
 */
static int dpaa_sec_init_rx(struct qman_fq *fq_in, rte_iova_t hwdesc,
                            uint32_t fqid_out) {
  ...
  fq_opts.we_mask =
      QM_INITFQ_WE_DESTWQ | QM_INITFQ_WE_CONTEXTA | QM_INITFQ_WE_CONTEXTB;
...
dqrr_out_fq_cb_rx(struct qman_portal *qm __always_unused,
  ...
  if (ctx->op->sess_type == RTE_CRYPTO_OP_SECURITY_SESSION) {
    ...
    uint32_t len;
    struct rte_mbuf *mbuf = (ctx->op->sym->m_dst == NULL) ? ctx->op->sym->m_src
                                                          : ctx->op->sym->m_dst;
...
static int dpaa_sec_init_tx(struct qman_fq *fq) {
  ...
  flags =
      QMAN_FQ_FLAG_NO_ENQUEUE | QMAN_FQ_FLAG_LOCKED | QMAN_FQ_FLAG_DYNAMIC_FQID;
...
static inline int is_aead(dpaa_sec_session *ses) {
  return ((ses->cipher_alg == 0) && (ses->auth_alg == 0) &&
          (ses->aead_alg != 0));
}
...
#ifdef RTE_LIB_SECURITY
static int dpaa_sec_prep_pdcp_cdb(dpaa_sec_session *ses) {
  ...
  if (ses->pdcp.sdap_enabled) {
    int nb_keys_to_inline =
        rta_inline_pdcp_sdap_query(authdata.algtype, cipherdata.algtype,
                                   ses->pdcp.sn_size, ses->pdcp.hfn_ovd);
  ...
  if (ses->pdcp.domain == RTE_SECURITY_PDCP_MODE_CONTROL) {
    if (ses->dir == DIR_ENC)
      shared_desc_len = cnstr_shdsc_pdcp_c_plane_encap(
          cdb->sh_desc, 1, swap, ses->pdcp.hfn, ses->pdcp.sn_size,
          ses->pdcp.bearer, ses->pdcp.pkt_dir, ses->pdcp.hfn_threshold,
          &cipherdata, &authdata);
    ...
    shared_desc_len =
        cnstr_shdsc_pdcp_short_mac(cdb->sh_desc, 1, swap, &authdata);
  } else {
    if (ses->dir == DIR_ENC) {
      if (ses->pdcp.sdap_enabled)
        shared_desc_len = cnstr_shdsc_pdcp_sdap_u_plane_encap(
            cdb->sh_desc, 1, swap, ses->pdcp.sn_size, ses->pdcp.hfn,
            ses->pdcp.bearer, ses->pdcp.pkt_dir, ses->pdcp.hfn_threshold,
      ...
      if (ses->pdcp.sdap_enabled)
        shared_desc_len = cnstr_shdsc_pdcp_sdap_u_plane_decap(
            cdb->sh_desc, 1, swap, ses->pdcp.sn_size, ses->pdcp.hfn,
            ses->pdcp.bearer, ses->pdcp.pkt_dir, ses->pdcp.hfn_threshold,
            &cipherdata, p_authdata);
      else
...
static int dpaa_sec_prep_ipsec_cdb(dpaa_sec_session *ses) {
  ...
  err = rta_inline_query(IPSEC_AUTH_VAR_AES_DEC_BASE_DESC_LEN, DESC_JOB_IO_LEN,
                         (unsigned int *)cdb->sh_desc, &cdb->sh_desc[2], 2);
  ...
  if (ses->dir == DIR_ENC) {
    shared_desc_len = cnstr_shdsc_ipsec_new_encap(
        cdb->sh_desc, true, swap, SHR_SERIAL, &ses->encap_pdb,
        (uint8_t *)&ses->ip4_hdr, &cipherdata, &authdata);
  } else if (ses->dir == DIR_DEC) {
    shared_desc_len =
        cnstr_shdsc_ipsec_new_decap(cdb->sh_desc, true, swap, SHR_SERIAL,
                                    &ses->decap_pdb, &cipherdata, &authdata);
  }
  return shared_desc_len;
}
#endif
/* prepare command block of the session */
static int dpaa_sec_prep_cdb(dpaa_sec_session *ses) {
  ...
  case DPAA_SEC_CIPHER:
    ...
    case RTE_CRYPTO_CIPHER_3DES_CTR:
      shared_desc_len =
          cnstr_shdsc_blkcipher(cdb->sh_desc, true, swap, SHR_NEVER, &alginfo_c,
                                ses->iv.length, ses->dir);
      break;
    case RTE_CRYPTO_CIPHER_SNOW3G_UEA2:
      shared_desc_len =
          cnstr_shdsc_snow_f8(cdb->sh_desc, true, swap, &alginfo_c, ses->dir);
      break;
    case RTE_CRYPTO_CIPHER_ZUC_EEA3:
      shared_desc_len =
          cnstr_shdsc_zuce(cdb->sh_desc, true, swap, &alginfo_c, ses->dir);
      break;
    default:
  ...
  case DPAA_SEC_AUTH:
    ...
    case RTE_CRYPTO_AUTH_SHA512:
      shared_desc_len =
          cnstr_shdsc_hash(cdb->sh_desc, true, swap, SHR_NEVER, &alginfo_a,
                           !ses->dir, ses->digest_length);
      break;
    ...
    case RTE_CRYPTO_AUTH_SHA512_HMAC:
      shared_desc_len =
          cnstr_shdsc_hmac(cdb->sh_desc, true, swap, SHR_NEVER, &alginfo_a,
                           !ses->dir, ses->digest_length);
      break;
    case RTE_CRYPTO_AUTH_SNOW3G_UIA2:
      shared_desc_len = cnstr_shdsc_snow_f9(
          cdb->sh_desc, true, swap, &alginfo_a, !ses->dir, ses->digest_length);
      break;
    case RTE_CRYPTO_AUTH_ZUC_EIA3:
      shared_desc_len = cnstr_shdsc_zuca(cdb->sh_desc, true, swap, &alginfo_a,
                                         !ses->dir, ses->digest_length);
      break;
    case RTE_CRYPTO_AUTH_AES_XCBC_MAC:
    case RTE_CRYPTO_AUTH_AES_CMAC:
      shared_desc_len =
          cnstr_shdsc_aes_mac(cdb->sh_desc, true, swap, SHR_NEVER, &alginfo_a,
                              !ses->dir, ses->digest_length);
      break;
    default:
  ...
  case DPAA_SEC_AEAD:
    ...
    if (ses->dir == DIR_ENC)
      shared_desc_len =
          cnstr_shdsc_gcm_encap(cdb->sh_desc, true, swap, SHR_NEVER, &alginfo,
                                ses->iv.length, ses->digest_length);
    else
      shared_desc_len =
          cnstr_shdsc_gcm_decap(cdb->sh_desc, true, swap, SHR_NEVER, &alginfo,
                                ses->iv.length, ses->digest_length);
    break;
  case DPAA_SEC_CIPHER_HASH:
    ...
    err =
        rta_inline_query(IPSEC_AUTH_VAR_AES_DEC_BASE_DESC_LEN, DESC_JOB_IO_LEN,
                         (unsigned int *)cdb->sh_desc, &cdb->sh_desc[2], 2);
    ...
    if (cdb->sh_desc[2] & 1)
      ...
      alginfo_c.key = (size_t)rte_dpaa_mem_vtop((void *)(size_t)alginfo_c.key);
      alginfo_c.key_type = RTA_DATA_PTR;
    }
    if (cdb->sh_desc[2] & (1 << 1))
      alginfo_a.key_type = RTA_DATA_IMM;
    else {
      alginfo_a.key = (size_t)rte_dpaa_mem_vtop((void *)(size_t)alginfo_a.key);
    ...
    shared_desc_len = cnstr_shdsc_authenc(
        cdb->sh_desc, true, swap, SHR_SERIAL, &alginfo_c, &alginfo_a,
        ses->iv.length, ses->digest_length, ses->dir);
    break;
...
static void dpaa_sec_dump(struct dpaa_sec_op_ctx *ctx, struct dpaa_sec_qp *qp) {
  ...
  if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION)
    sess = (dpaa_sec_session *)get_sym_session_private_data(
        op->sym->session, dpaa_cryptodev_driver_id);
#ifdef RTE_LIBRTE_SECURITY
  else if (op->sess_type == RTE_CRYPTO_OP_SECURITY_SESSION)
    sess =
        (dpaa_sec_session *)get_sec_session_private_data(op->sym->sec_session);
#endif
  if (sess == NULL) {
  ...
  printf("\n****************************************\n"
         ...
         "\tCipher algmode:\t%d\n",
         sess->ctxt, (sess->dir == DIR_ENC) ? "DIR_ENC" : "DIR_DEC",
  ...
  printf("\tAuth key len:\t%" PRIu64 "\n\tAuth alg:\t%d\n"
         ...
         sess->auth_key.algmode, sess->iv.length, sess->iv.offset,
         sess->digest_length, sess->auth_only_len, sess->auth_cipher_text);
#ifdef RTE_LIBRTE_SECURITY
  printf("PDCP session params:\n"
         ...
         sess->pdcp.hfn_ovd, sess->pdcp.sn_size, sess->pdcp.sdap_enabled,
         sess->pdcp.hfn_ovd_offset, sess->pdcp.hfn, sess->pdcp.hfn_threshold);
#endif
...
mbuf_dump:
  ...
  printf("\taddr = %" PRIx64 ",\n\tlen = %d,\n\tfinal = %d,\n\textention = %d"
         "\n\tbpid = %d\n\toffset = %d\n",
         (uint64_t)sg[0].addr, sg[0].length, sg[0].final, sg[0].extension,
         sg[0].bpid, sg[0].offset);
  printf("\njob->sg[1] input info:\n");
  memcpy(&sg[1], &job->sg[1], sizeof(sg[1]));
  hw_sg_to_cpu(&sg[1]);
  printf("\taddr = %" PRIx64 ",\n\tlen = %d,\n\tfinal = %d,\n\textention = %d"
         "\n\tbpid = %d\n\toffset = %d\n",
         (uint64_t)sg[1].addr, sg[1].length, sg[1].final, sg[1].extension,
         sg[1].bpid, sg[1].offset);
  ...
  printf("\tFQID = 0x%x\n\tstate = %d\n\tnb_desc = %d\n"
         "\tctx_pool = %p\n\trx_pkts = %d\n\ttx_pkts"
         "= %d\n\trx_errs = %d\n\ttx_errs = %d\n\n",
         qp->outq.fqid, qp->outq.state, qp->outq.nb_desc, qp->ctx_pool,
         qp->rx_pkts, qp->tx_pkts, qp->rx_errs, qp->tx_errs);
}
  ...
  do {
    ...
    if (op->sess_type == RTE_CRYPTO_OP_SECURITY_SESSION) {
      ...
      struct rte_mbuf *mbuf =
          (op->sym->m_dst == NULL) ? op->sym->m_src : op->sym->m_dst;
...
static inline struct dpaa_sec_job *build_auth_only_sg(struct rte_crypto_op *op,
  ...
  if (mbuf->nb_segs > MAX_SG_ENTRIES) {
    DPAA_SEC_DP_ERR("Auth: Max sec segs supported is %d", MAX_SG_ENTRIES);
    return NULL;
  }
  ...
  if (ses->iv.length) {
    ...
    iv_ptr = rte_crypto_op_ctod_offset(op, uint8_t *, ses->iv.offset);
  ...
  if (data_len <= (mbuf->data_len - data_offset)) {
    ...
    /* remaining i/p segs */
    while ((data_len = data_len - sg->length) && (mbuf = mbuf->next)) {
      cpu_to_hw_sg(sg);
      sg++;
      qm_sg_entry_set64(sg, rte_pktmbuf_iova(mbuf));
  ...
  if (is_decode(ses)) {
    ...
    cpu_to_hw_sg(sg);
    sg++;
    rte_memcpy(old_digest, sym->auth.digest.data, ses->digest_length);
...
build_cipher_only_sg(struct rte_crypto_op *op, dpaa_sec_session *ses) {
  ...
  struct rte_mbuf *mbuf;
  uint8_t req_segs;
  uint8_t *IV_ptr = rte_crypto_op_ctod_offset(op, uint8_t *, ses->iv.offset);
  int data_len, data_offset;
  ...
  if (mbuf->nb_segs > MAX_SG_ENTRIES) {
    DPAA_SEC_DP_ERR("Cipher: Max sec segs supported is %d", MAX_SG_ENTRIES);
    return NULL;
  }
...
build_cipher_auth_gcm_sg(struct rte_crypto_op *op, dpaa_sec_session *ses) {
  ...
  if (mbuf->nb_segs > MAX_SG_ENTRIES) {
    DPAA_SEC_DP_ERR("AEAD: Max sec segs supported is %d", MAX_SG_ENTRIES);
    return NULL;
  }
  ...
  in_sg->final = 1;
  if (is_encode(ses))
    in_sg->length = ses->iv.length + sym->aead.data.length + ses->auth_only_len;
  else
  ...
  if (is_decode(ses)) {
    cpu_to_hw_sg(sg);
    sg++;
    memcpy(ctx->digest, sym->aead.digest.data, ses->digest_length);
...
build_cipher_auth_gcm(struct rte_crypto_op *op, dpaa_sec_session *ses) {
  ...
  qm_sg_entry_set64(sg, dst_start_addr + sym->aead.data.offset);
  sg->length = sym->aead.data.length;
  length = sg->length;
  if (is_encode(ses)) {
    cpu_to_hw_sg(sg);
...
build_cipher_auth_sg(struct rte_crypto_op *op, dpaa_sec_session *ses) {
  ...
  if (is_encode(ses))
    ...
    in_sg->length = ses->iv.length + sym->auth.data.length + ses->digest_length;
  ...
  if (is_decode(ses)) {
    cpu_to_hw_sg(sg);
    sg++;
    memcpy(ctx->digest, sym->auth.digest.data, ses->digest_length);
    qm_sg_entry_set64(sg, rte_dpaa_mem_vtop(ctx->digest));
    sg->length = ses->digest_length;
  }
  sg->final = 1;
...
#ifdef RTE_LIB_SECURITY
static inline struct dpaa_sec_job *build_proto(struct rte_crypto_op *op,
                                               dpaa_sec_session *ses) {
  struct rte_crypto_sym_op *sym = op->sym;
  struct dpaa_sec_job *cf;
  struct dpaa_sec_op_ctx *ctx;
...
static inline struct dpaa_sec_job *build_proto_sg(struct rte_crypto_op *op,
                                                  dpaa_sec_session *ses) {
  struct rte_crypto_sym_op *sym = op->sym;
  struct dpaa_sec_job *cf;
  struct dpaa_sec_op_ctx *ctx;
  ...
  if (mbuf->nb_segs > MAX_SG_ENTRIES) {
    DPAA_SEC_DP_ERR("Proto: Max sec segs supported is %d", MAX_SG_ENTRIES);
    return NULL;
  }
...
static uint16_t dpaa_sec_enqueue_burst(void *qp, struct rte_crypto_op **ops,
                                       uint16_t nb_ops) {
  ...
  while (nb_ops) {
    frames_to_send = (nb_ops > DPAA_SEC_BURST) ? DPAA_SEC_BURST : nb_ops;
    for (loop = 0; loop < frames_to_send; loop++) {
      ...
      case RTE_CRYPTO_OP_WITH_SESSION:
        ses = (dpaa_sec_session *)get_sym_session_private_data(
            op->sym->session, dpaa_cryptodev_driver_id);
        break;
#ifdef RTE_LIB_SECURITY
      ...
      } else if (unlikely(ses->qp[rte_lcore_id() % MAX_DPAA_CORES] != qp)) {
        DPAA_SEC_DP_ERR("Old:sess->qp = %p"
                        " New qp = %p\n",
                        ses->qp[rte_lcore_id() % MAX_DPAA_CORES], qp);
        frames_to_send = loop;
        nb_ops = loop;
        goto send_pkts;
      }

      auth_hdr_len = op->sym->auth.data.length - op->sym->cipher.data.length;
      auth_tail_len = 0;
      ...
      if (rte_pktmbuf_is_contiguous(op->sym->m_src) &&
        ...
        case DPAA_SEC_CIPHER_HASH:
          auth_hdr_len =
              op->sym->cipher.data.offset - op->sym->auth.data.offset;
          auth_tail_len = op->sym->auth.data.length -
                          op->sym->cipher.data.length - auth_hdr_len;
      ...
      if ((ses->ctxt == DPAA_SEC_PDCP) && ses->pdcp.hfn_ovd) {
        fd->cmd = 0x80000000 |
                  *((uint32_t *)((uint8_t *)op + ses->pdcp.hfn_ovd_offset));
        DPAA_SEC_DP_DEBUG(
  ...
  send_pkts:
    ...
    while (loop < frames_to_send) {
      loop += qman_enqueue_multi_fq(&inq[loop], &fds[loop], &flags[loop],
                                    frames_to_send - loop);
    }
...
static uint16_t dpaa_sec_dequeue_burst(void *qp, struct rte_crypto_op **ops,
                                       uint16_t nb_ops) {
  uint16_t num_rx;
  struct dpaa_sec_qp *dpaa_qp = (struct dpaa_sec_qp *)qp;
...
/** Release queue pair */
static int dpaa_sec_queue_pair_release(struct rte_cryptodev *dev,
                                       uint16_t qp_id) {
  ...
  if (qp_id >= internals->max_nb_queue_pairs) {
    DPAA_SEC_ERR("Max supported qpid %d", internals->max_nb_queue_pairs);
    return -EINVAL;
  }
...
/** Setup a queue pair */
static int dpaa_sec_queue_pair_setup(
  ...
  snprintf(str, sizeof(str), "ctx_pool_d%d_qp%d", dev->data->dev_id, qp_id);
  if (!qp->ctx_pool) {
    qp->ctx_pool = rte_mempool_create((const char *)str, CTX_POOL_NUM_BUFS,
                                      CTX_POOL_BUF_SIZE, CTX_POOL_CACHE_SIZE, 0,
...
static int dpaa_sec_cipher_init(struct rte_cryptodev *dev __rte_unused,
  ...
  case RTE_CRYPTO_CIPHER_ZUC_EEA3:
    ...
    break;
  default:
    DPAA_SEC_ERR("Crypto: Undefined Cipher specified %u", xform->cipher.algo);
    return -ENOTSUP;
  }
  session->dir =
...
static int dpaa_sec_auth_init(struct rte_cryptodev *dev __rte_unused,
                              struct rte_crypto_sym_xform *xform,
                              dpaa_sec_session *session) {
  ...
  case RTE_CRYPTO_AUTH_AES_CMAC:
    ...
    break;
  default:
    DPAA_SEC_ERR("Crypto: Unsupported Auth specified %u", xform->auth.algo);
    return -ENOTSUP;
  }
...
static int dpaa_sec_chain_init(struct rte_cryptodev *dev __rte_unused,
                               struct rte_crypto_sym_xform *xform,
                               dpaa_sec_session *session) {
  ...
  case RTE_CRYPTO_AUTH_AES_CMAC:
    ...
    break;
  default:
    DPAA_SEC_ERR("Crypto: Unsupported Auth specified %u", auth_xform->algo);
    return -ENOTSUP;
  }
  ...
  case RTE_CRYPTO_CIPHER_AES_CTR:
    ...
    break;
  default:
    DPAA_SEC_ERR("Crypto: Undefined Cipher specified %u", cipher_xform->algo);
    return -ENOTSUP;
  }
  session->dir =
...
static int dpaa_sec_aead_init(struct rte_cryptodev *dev __rte_unused,
                              struct rte_crypto_sym_xform *xform,
                              dpaa_sec_session *session) {
  ...
  session->aead_key.data =
      rte_zmalloc(NULL, xform->aead.key.length, RTE_CACHE_LINE_SIZE);
  ...
  memcpy(session->aead_key.data, xform->aead.key.data, xform->aead.key.length);
  ...
  session->dir =
      (xform->aead.op == RTE_CRYPTO_AEAD_OP_ENCRYPT) ? DIR_ENC : DIR_DEC;

  return 0;
}

static struct qman_fq *dpaa_sec_attach_rxq(struct dpaa_sec_dev_private *qi) {
  unsigned int i;
...
static int dpaa_sec_detach_rxq(struct dpaa_sec_dev_private *qi,
                               struct qman_fq *fq) {
  unsigned int i;
...
int dpaa_sec_attach_sess_q(struct dpaa_sec_qp *qp, dpaa_sec_session *sess) {
  int ret;
  ...
  ret =
      dpaa_sec_init_rx(sess->inq[rte_lcore_id() % MAX_DPAA_CORES],
                       rte_dpaa_mem_vtop(&sess->cdb), qman_fq_fqid(&qp->outq));
  if (ret)
...
static inline void free_session_data(dpaa_sec_session *s) {
  if (is_aead(s))
...
static int dpaa_sec_set_session_parameters(struct rte_cryptodev *dev,
                                           struct rte_crypto_sym_xform *xform,
                                           void *sess) {
...
static int dpaa_sec_sym_session_configure(
...
static inline void free_session_memory(struct rte_cryptodev *dev,
                                       dpaa_sec_session *s) {
...
static void dpaa_sec_sym_session_clear(struct rte_cryptodev *dev,
...
dpaa_sec_ipsec_aead_init(struct rte_crypto_aead_xform *aead_xform,
  ...
  case RTE_CRYPTO_AEAD_AES_GCM:
    ...
    case 16:
      ...
      break;
    default:
      DPAA_SEC_ERR("Crypto: Undefined GCM digest %d", session->digest_length);
      return -EINVAL;
    }
    if (session->dir == DIR_ENC) {
      memcpy(session->encap_pdb.gcm.salt, (uint8_t *)&(ipsec_xform->salt), 4);
    } else {
      memcpy(session->decap_pdb.gcm.salt, (uint8_t *)&(ipsec_xform->salt), 4);
    }
    session->aead_key.algmode = OP_ALG_AAI_GCM;
    session->aead_alg = RTE_CRYPTO_AEAD_AES_GCM;
    break;
  default:
    DPAA_SEC_ERR("Crypto: Undefined AEAD specified %u", aead_xform->algo);
    return -ENOTSUP;
  }
  return 0;
}
...
dpaa_sec_ipsec_proto_init(struct rte_crypto_cipher_xform *cipher_xform,
  ...
  case RTE_CRYPTO_AUTH_SHA256_HMAC:
    ...
    if (session->digest_length != 16)
      DPAA_SEC_WARN("+++Using sha256-hmac truncated len is non-standard,"
  ...
  case RTE_CRYPTO_AUTH_ZUC_EIA3:
    DPAA_SEC_ERR("Crypto: Unsupported auth alg %u", session->auth_alg);
    return -ENOTSUP;
  default:
    DPAA_SEC_ERR("Crypto: Undefined Auth specified %u", session->auth_alg);
    return -ENOTSUP;
  }
  ...
  case RTE_CRYPTO_CIPHER_KASUMI_F8:
    DPAA_SEC_ERR("Crypto: Unsupported Cipher alg %u", session->cipher_alg);
    return -ENOTSUP;
  default:
    DPAA_SEC_ERR("Crypto: Undefined Cipher specified %u", session->cipher_alg);
    return -ENOTSUP;
  }
...
static int dpaa_sec_set_ipsec_session(__rte_unused struct rte_cryptodev *dev,
  ...
  if (conf->crypto_xform->type == RTE_CRYPTO_SYM_XFORM_CIPHER) {
    ...
    ret = dpaa_sec_ipsec_proto_init(cipher_xform, auth_xform, ipsec_xform,
                                    session);
    ...
    ret = dpaa_sec_ipsec_aead_init(aead_xform, ipsec_xform, session);
  } else {
  ...
  if (ipsec_xform->direction == RTE_SECURITY_IPSEC_SA_DIR_EGRESS) {
    if (ipsec_xform->tunnel.type == RTE_SECURITY_IPSEC_TUNNEL_IPV4) {
      ...
      session->ip4_hdr.ip_len = rte_cpu_to_be_16(sizeof(session->ip4_hdr));
      ...
      session->ip4_hdr.ip_p =
          (ipsec_xform->proto == RTE_SECURITY_IPSEC_SA_PROTO_ESP) ? IPPROTO_ESP
                                                                  : IPPROTO_AH;
      ...
      session->ip4_hdr.ip_sum =
          calc_chksum((uint16_t *)(void *)&session->ip4_hdr, sizeof(struct ip));
      ...
      session->ip6_hdr.hop_limits = ipsec_xform->tunnel.ipv6.hlimit;
      session->ip6_hdr.proto =
          (ipsec_xform->proto == RTE_SECURITY_IPSEC_SA_PROTO_ESP) ? IPPROTO_ESP
                                                                  : IPPROTO_AH;
      memcpy(&session->ip6_hdr.src_addr, &ipsec_xform->tunnel.ipv6.src_addr,
             16);
      memcpy(&session->ip6_hdr.dst_addr, &ipsec_xform->tunnel.ipv6.dst_addr,
             16);
      session->encap_pdb.ip_hdr_len = sizeof(struct rte_ipv6_hdr);
    }

    session->encap_pdb.options = (IPVERSION << PDBNH_ESP_ENCAP_SHIFT) |
    ...
    if (ipsec_xform->replay_win_sz) {
      ...
      case 64:
        ...
        break;
      default:
        session->decap_pdb.options |= PDBOPTS_ESP_ARS128;
      }
    }
  } else
    goto out;
...
static int dpaa_sec_set_pdcp_session(struct rte_cryptodev *dev,
                                     struct rte_security_session_conf *conf,
                                     void *sess) {
  ...
  if (xform->type == RTE_CRYPTO_SYM_XFORM_CIPHER) {
    ...
    auth_xform = &xform->auth;
    if (xform->next != NULL && xform->next->type == RTE_CRYPTO_SYM_XFORM_CIPHER)
      cipher_xform = &xform->next->cipher;
  } else {
    DPAA_SEC_ERR("Invalid crypto type");
    return -EINVAL;
  ...
  if (pdcp_xform->domain == RTE_SECURITY_PDCP_MODE_CONTROL) {
    if (pdcp_xform->sn_size != RTE_SECURITY_PDCP_SN_SIZE_5 &&
      ...
      DPAA_SEC_ERR("PDCP Seq Num size should be 5/12 bits for cmode");
      ret = -EINVAL;
      goto out;
    }
  }
...
static int dpaa_sec_security_session_create(
  ...
  case RTE_SECURITY_PROTOCOL_IPSEC:
    ret = dpaa_sec_set_ipsec_session(cdev, conf, sess_private_data);
    break;
  case RTE_SECURITY_PROTOCOL_PDCP:
    ret = dpaa_sec_set_pdcp_session(cdev, conf, sess_private_data);
    break;
...
static int dpaa_sec_dev_start(struct rte_cryptodev *dev __rte_unused) {
  PMD_INIT_FUNC_TRACE();
  return 0;
}

static void dpaa_sec_dev_stop(struct rte_cryptodev *dev __rte_unused) {
  PMD_INIT_FUNC_TRACE();
}

static int dpaa_sec_dev_close(struct rte_cryptodev *dev) {
  PMD_INIT_FUNC_TRACE();
...
static void dpaa_sec_dev_infos_get(struct rte_cryptodev *dev,
...
static enum qman_cb_dqrr_result dpaa_sec_process_parallel_event(
    void *event, struct qman_portal *qm __always_unused, struct qman_fq *outq,
...
static enum qman_cb_dqrr_result
dpaa_sec_process_atomic_event(void *event, struct qman_portal *qm __rte_unused,
                              struct qman_fq *outq,
                              const struct qm_dqrr_entry *dqrr, void **bufs) {
  u8 index;
  const struct qm_fd *fd;
...
int dpaa_sec_eventq_attach(const struct rte_cryptodev *dev, int qp_id,
                           uint16_t ch_id, const struct rte_event *event) {
...
int dpaa_sec_eventq_detach(const struct rte_cryptodev *dev, int qp_id) {
  struct qm_mcc_initfq opts = {0};
  int ret;
  struct dpaa_sec_qp *qp = dev->data->queue_pairs[qp_id];
...
static int dpaa_sec_uninit(struct rte_cryptodev *dev) {
  ...
  DPAA_SEC_INFO("Closing DPAA_SEC device %s on numa socket %u", dev->data->name,
                rte_socket_id());

  return 0;
}

static int check_devargs_handler(__rte_unused const char *key,
...
static void dpaa_sec_get_devargs(struct rte_devargs *devargs, const char *key) {
  ...
  rte_kvargs_process(kvlist, key, check_devargs_handler, NULL);
  rte_kvargs_free(kvlist);
}

static int dpaa_sec_dev_init(struct rte_cryptodev *cryptodev) {
  struct dpaa_sec_dev_private *internals;
#ifdef RTE_LIB_SECURITY
...
cryptodev_dpaa_sec_probe(struct rte_dpaa_driver *dpaa_drv __rte_unused,
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    cryptodev->data->dev_private = rte_zmalloc_socket(
        "cryptodev private structure", sizeof(struct dpaa_sec_dev_private),
  ...
  if (!rta_get_sec_era()) {
    ...
    for_each_compatible_node(caam_node, NULL, "fsl,sec-v4.0") {
      const uint32_t *prop = of_get_property(caam_node, "fsl,sec-era", NULL);
      if (prop) {
...
static int cryptodev_dpaa_sec_remove(struct rte_dpaa_device *dpaa_dev) {
  struct rte_cryptodev *cryptodev;
  int ret;
