...
static uint16_t virtqueue_dequeue_burst_rx(struct virtqueue *vq,
  ...
  for (i = 0; i < num; i++) {
    ...
    cop = (struct rte_crypto_op *)vq->vq_descx[desc_idx].crypto_op;
    ...
    op_cookie = (struct virtio_crypto_op_cookie *)vq->vq_descx[desc_idx].cookie;
...
static int
virtqueue_crypto_sym_pkt_header_arrange(struct rte_crypto_op *cop,
  ...
  case VIRTIO_CRYPTO_SYM_OP_CIPHER:
    ...
    req_data->u.sym_req.u.cipher.para.iv_len = session->iv.length;
  ...
  case VIRTIO_CRYPTO_SYM_OP_ALGORITHM_CHAINING:
    ...
    req_data->u.sym_req.u.chain.para.len_to_cipher = sym_op->cipher.data.length;
    req_data->u.sym_req.u.chain.para.hash_start_src_offset =
        sym_op->auth.data.offset;
    req_data->u.sym_req.u.chain.para.len_to_hash = sym_op->auth.data.length;
    req_data->u.sym_req.u.chain.para.aad_len = chain_para->aad_len;
...
static int virtqueue_crypto_sym_enqueue_xmit(struct virtqueue *txvq,
                                             struct rte_crypto_op *cop) {
  uint16_t idx = 0;
  uint16_t num_entry;
  uint16_t needed = 1;
  ...
  uint32_t indirect_vring_addr_offset =
  ...
  indirect_op_data_req_phys_addr = rte_mempool_virt2iova(crypto_op_cookie);
  ...
  ((struct virtio_crypto_inhdr *)((uint8_t *)op_data_req + req_data_len))
      ->status = VIRTIO_CRYPTO_ERR;

  /* point to indirect vring entry */
  desc = (struct vring_desc *)((uint8_t *)op_data_req +
  ...
  if (session->iv.length) {
    if (cop->phys_addr)
      ...
      rte_memcpy(crypto_op_cookie->iv,
                 rte_crypto_op_ctod_offset(cop, uint8_t *, session->iv.offset),
                 session->iv.length);
      desc[idx].addr = indirect_op_data_req_phys_addr + indirect_iv_addr_offset;
    }
  ...
  start_dp[head_idx].addr =
      indirect_op_data_req_phys_addr + indirect_vring_addr_offset;
...
static int virtqueue_crypto_enqueue_xmit(struct virtqueue *txvq,
                                         struct rte_crypto_op *cop) {
  int ret;
  ...
  case RTE_CRYPTO_OP_TYPE_SYMMETRIC:
    ...
    break;
  default:
    VIRTIO_CRYPTO_TX_LOG_ERR("invalid crypto op type %u", cop->type);
    ret = -EFAULT;
    break;
  }
...
static int virtio_crypto_vring_start(struct virtqueue *vq) {
...
uint16_t virtio_crypto_pkt_tx_burst(void *tx_queue,
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (unlikely(error)) {
      if (error == ENOSPC)
        VIRTIO_CRYPTO_TX_LOG_ERR("virtqueue_enqueue Free count = 0");
      else if (error == EMSGSIZE)
        VIRTIO_CRYPTO_TX_LOG_ERR("virtqueue_enqueue Free count < 1");
      else
        VIRTIO_CRYPTO_TX_LOG_ERR("virtqueue_enqueue error: %d", error);
