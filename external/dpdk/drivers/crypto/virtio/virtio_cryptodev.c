...
static unsigned int
virtio_crypto_sym_get_session_private_size(struct rte_cryptodev *dev);
static void
virtio_crypto_sym_clear_session(struct rte_cryptodev *dev,
                                struct rte_cryptodev_sym_session *sess);
static int virtio_crypto_sym_configure_session(
    struct rte_cryptodev *dev, struct rte_crypto_sym_xform *xform,
    struct rte_cryptodev_sym_session *session, struct rte_mempool *mp);

/*
 * The set of PCI devices this driver supports
 */
static const struct rte_pci_id pci_id_virtio_crypto_map[] = {
    {RTE_PCI_DEVICE(VIRTIO_CRYPTO_PCI_VENDORID, VIRTIO_CRYPTO_PCI_DEVICEID)},
    {.vendor_id = 0, /* sentinel */},
};

static const struct rte_cryptodev_capabilities virtio_capabilities[] = {
    VIRTIO_SYM_CAPABILITIES, RTE_CRYPTODEV_END_OF_CAPABILITIES_LIST()};

uint8_t cryptodev_virtio_driver_id;

#define NUM_ENTRY_SYM_CREATE_SESSION 4

static int virtio_crypto_send_command(struct virtqueue *vq,
  ...
  VIRTIO_CRYPTO_INIT_LOG_DBG("vq->vq_desc_head_idx = %d, vq = %p", head, vq);
  ...
  if (auth_key) {
    len_auth_key =
        ctrl->u.sym_create_session.u.chain.para.u.mac_param.auth_key_len;
  }
   ...
   */
  desc_offset =
      len_ctrl_req + len_cipher_key + len_auth_key + len_session_input;
  ...
  desc = (struct vring_desc *)((uint8_t *)virt_addr_started + desc_offset);
  ...
  /* cipher key part */
  if (len_cipher_key > 0) {
    memcpy((uint8_t *)virt_addr_started + len_total, cipher_key,
           len_cipher_key);
  ...
  /* auth key part */
  if (len_auth_key > 0) {
    memcpy((uint8_t *)virt_addr_started + len_total, auth_key, len_auth_key);
  ...
  /* input part */
  input = (struct virtio_crypto_session_input *)((uint8_t *)virt_addr_started +
                                                 input_offset);
  ...
  VIRTIO_CRYPTO_INIT_LOG_DBG("vq->vq_queue_index = %d", vq->vq_queue_index);
...
int virtio_crypto_queue_setup(struct rte_cryptodev *dev, int queue_type,
                              uint16_t vtpci_queue_idx, uint16_t nb_desc,
                              int socket_id, struct virtqueue **pvq) {
  ...
  if (queue_type == VTCRYPTO_DATAQ) {
    snprintf(vq_name, sizeof(vq_name), "dev%d_dataqueue%d", dev->data->dev_id,
             vtpci_queue_idx);
    ...
    snprintf(vq_name, sizeof(vq_name), "dev%d_controlqueue", dev->data->dev_id);
  ...
  if (queue_type == VTCRYPTO_DATAQ) {
    ...
    if (vq->mpool == NULL)
      vq->mpool = rte_mempool_create(
          mpool_name, vq_size, sizeof(struct virtio_crypto_op_cookie),
          RTE_CACHE_LINE_SIZE, 0, NULL, NULL, NULL, NULL, socket_id, 0);
    if (!vq->mpool) {
    ...
    for (i = 0; i < vq_size; i++) {
      vq->vq_descx[i].cookie = rte_zmalloc(
          "crypto PMD op cookie pointer",
          sizeof(struct virtio_crypto_op_cookie), RTE_CACHE_LINE_SIZE);
  ...
  mz = rte_memzone_reserve_aligned(vq_name, vq->vq_ring_size, socket_id, 0,
   ...
   */
  if ((mz->iova + vq->vq_ring_size - 1) >> (VIRTIO_PCI_QUEUE_ADDR_SHIFT + 32)) {
...
static int virtio_crypto_ctrlq_setup(struct rte_cryptodev *dev,
                                     uint16_t queue_idx) {
  int ret;
  struct virtqueue *vq;
  struct virtio_crypto_hw *hw = dev->data->dev_private;
  ...
  ret = virtio_crypto_queue_setup(dev, VTCRYPTO_CTRLQ, queue_idx, 0,
                                  SOCKET_ID_ANY, &vq);
  if (ret < 0) {
...
static void virtio_crypto_free_queues(struct rte_cryptodev *dev) {
  unsigned int i;
  struct virtio_crypto_hw *hw = dev->data->dev_private;
...
static int virtio_crypto_dev_close(struct rte_cryptodev *dev __rte_unused) {
  return 0;
}
...
static void virtio_crypto_update_stats(struct rte_cryptodev *dev,
                                       struct rte_cryptodev_stats *stats) {
  unsigned int i;
  struct virtio_crypto_hw *hw = dev->data->dev_private;
  ...
  for (i = 0; i < hw->max_dataqueues; i++) {
    const struct virtqueue *data_queue = dev->data->queue_pairs[i];
    if (data_queue == NULL)
      continue;
    ...
    stats->dequeue_err_count += data_queue->packets_received_failed;
  }
}
...
static int virtio_negotiate_features(struct virtio_crypto_hw *hw,
  ...
  hw->guest_features = vtpci_cryptodev_negotiate_features(hw, host_features);
  VIRTIO_CRYPTO_INIT_LOG_DBG("features after negotiate = %" PRIx64,
                             hw->guest_features);

  if (hw->modern) {
    if (!vtpci_with_feature(hw, VIRTIO_F_VERSION_1)) {
      VIRTIO_CRYPTO_INIT_LOG_ERR("VIRTIO_F_VERSION_1 features is not enabled.");
      return -1;
    }
    vtpci_cryptodev_set_status(hw, VIRTIO_CONFIG_STATUS_FEATURES_OK);
    if (!(vtpci_cryptodev_get_status(hw) & VIRTIO_CONFIG_STATUS_FEATURES_OK)) {
...
static int virtio_crypto_init_device(struct rte_cryptodev *cryptodev,
                                     uint64_t req_features) {
  ...
  vtpci_read_cryptodev_config(hw, offsetof(struct virtio_crypto_config, status),
  ...
  vtpci_read_cryptodev_config(
      hw, offsetof(struct virtio_crypto_config, max_dataqueues),
      &config->max_dataqueues, sizeof(config->max_dataqueues));
  hw->max_dataqueues = config->max_dataqueues;

  VIRTIO_CRYPTO_INIT_LOG_DBG("hw->max_dataqueues=%d", hw->max_dataqueues);
...
crypto_virtio_create(const char *name, struct rte_pci_device *pci_dev,
  ...
  cryptodev = rte_cryptodev_pmd_create(name, &pci_dev->device, init_params);
  if (cryptodev == NULL)
    return -ENODEV;
  ...
  if (virtio_crypto_init_device(cryptodev, VIRTIO_CRYPTO_PMD_GUEST_FEATURES) <
      0)
    return -1;
...
static int virtio_crypto_dev_uninit(struct rte_cryptodev *cryptodev) {
  struct virtio_crypto_hw *hw = cryptodev->data->dev_private;
...
static void virtio_crypto_dev_free_mbufs(struct rte_cryptodev *dev) {
  ...
  for (i = 0; i < hw->max_dataqueues; i++) {
    ...
    VIRTIO_CRYPTO_INIT_LOG_DBG("queue_pairs[%d]=%p", i,
...
static int virtio_crypto_check_sym_session_paras(struct rte_cryptodev *dev) {
  struct virtio_crypto_hw *hw;
...
virtio_crypto_sym_clear_session(struct rte_cryptodev *dev,
  ...
  VIRTIO_CRYPTO_SESSION_LOG_INFO("vq->vq_desc_head_idx = %d, "
                                 "vq = %p",
                                 vq->vq_desc_head_idx, vq);

  if (vq->vq_free_cnt < needed) {
    VIRTIO_CRYPTO_SESSION_LOG_ERR("vq->vq_free_cnt = %d is less than %d, "
                                  "not enough",
  ...
  malloc_virt_addr =
      rte_malloc(NULL,
                 len_op_ctrl_req + len_inhdr +
                     NUM_ENTRY_SYM_CLEAR_SESSION * sizeof(struct vring_desc),
                 RTE_CACHE_LINE_SIZE);
  ...
  /* status part */
  status = &(((struct virtio_crypto_inhdr *)((uint8_t *)malloc_virt_addr +
                                             len_op_ctrl_req))
                 ->status);
  *status = VIRTIO_CRYPTO_ERR;

  /* indirect desc vring part */
  desc = (struct vring_desc *)((uint8_t *)malloc_virt_addr + desc_offset);
...
static int virtio_crypto_get_chain_order(struct rte_crypto_sym_xform *xform) {
  if (xform == NULL)
    return -1;
...
static int
virtio_crypto_sym_pad_auth_param(struct virtio_crypto_op_ctrl_req *ctrl,
  ...
  case RTE_CRYPTO_AUTH_SHA1_HMAC:
    *algo = VIRTIO_CRYPTO_MAC_HMAC_SHA1;
    break;
  default:
    VIRTIO_CRYPTO_SESSION_LOG_ERR("Crypto: Undefined Hash algo %u specified",
                                  auth_xform->algo);
    return -1;
  }
...
static int virtio_crypto_sym_pad_op_ctrl_req(
    struct virtio_crypto_op_ctrl_req *ctrl, struct rte_crypto_sym_xform *xform,
    bool is_chainned, uint8_t *cipher_key_data, uint8_t *auth_key_data,
    struct virtio_crypto_session *session) {
  int ret;
  ...
  if (cipher_xform) {
    if (cipher_xform->key.length > VIRTIO_CRYPTO_MAX_KEY_SIZE) {
      VIRTIO_CRYPTO_SESSION_LOG_ERR("cipher key size cannot be longer than %u",
                                    VIRTIO_CRYPTO_MAX_KEY_SIZE);
      return -1;
    }
    if (cipher_xform->iv.length > VIRTIO_CRYPTO_MAX_IV_SIZE) {
      VIRTIO_CRYPTO_SESSION_LOG_ERR("cipher IV size cannot be longer than %u",
                                    VIRTIO_CRYPTO_MAX_IV_SIZE);
      return -1;
    }
    if (is_chainned)
      ret = virtio_crypto_sym_pad_cipher_param(
          &ctrl->u.sym_create_session.u.chain.para.cipher_param, cipher_xform);
    else
      ret = virtio_crypto_sym_pad_cipher_param(
          &ctrl->u.sym_create_session.u.cipher.para, cipher_xform);

    if (ret < 0) {
      VIRTIO_CRYPTO_SESSION_LOG_ERR("pad cipher parameter failed");
      return -1;
    }

    memcpy(cipher_key_data, cipher_xform->key.data, cipher_xform->key.length);
  ...
  if (auth_xform) {
    ...
    if (auth_xform->key.length) {
      if (auth_xform->key.length > VIRTIO_CRYPTO_MAX_KEY_SIZE) {
        VIRTIO_CRYPTO_SESSION_LOG_ERR("auth key size cannot be longer than %u",
                                      VIRTIO_CRYPTO_MAX_KEY_SIZE);
        return -1;
      }
      para->hash_mode = VIRTIO_CRYPTO_SYM_HASH_MODE_AUTH;
      para->u.mac_param.auth_key_len = (uint32_t)auth_xform->key.length;
      para->u.mac_param.hash_result_len = auth_xform->digest_length;
      memcpy(auth_key_data, auth_xform->key.data, auth_xform->key.length);
    } else {
      para->hash_mode = VIRTIO_CRYPTO_SYM_HASH_MODE_PLAIN;
      para->u.hash_param.hash_result_len = auth_xform->digest_length;
    }
...
static int virtio_crypto_check_sym_configure_session_paras(
    struct rte_cryptodev *dev, struct rte_crypto_sym_xform *xform,
    struct rte_cryptodev_sym_session *sym_sess, struct rte_mempool *mempool) {
...
static int virtio_crypto_sym_configure_session(
  ...
  ret = virtio_crypto_check_sym_configure_session_paras(dev, xform, sess,
                                                        mempool);
  if (ret < 0) {
  ...
  if (rte_mempool_get(mempool, &session_private)) {
    VIRTIO_CRYPTO_SESSION_LOG_ERR("Couldn't get object from session mempool");
    return -ENOMEM;
  }
  ...
  case VIRTIO_CRYPTO_CMD_HASH_CIPHER:
    ...
    ret = virtio_crypto_sym_pad_op_ctrl_req(
        ctrl_req, xform, true, cipher_key_data, auth_key_data, session);
    if (ret < 0) {
      VIRTIO_CRYPTO_SESSION_LOG_ERR("padding sym op ctrl req failed");
      goto error_out;
    }
    ret = virtio_crypto_send_command(control_vq, ctrl_req, cipher_key_data,
  ...
  case VIRTIO_CRYPTO_CMD_CIPHER:
    ...
    ret = virtio_crypto_sym_pad_op_ctrl_req(
        ctrl_req, xform, false, cipher_key_data, auth_key_data, session);
    if (ret < 0) {
      VIRTIO_CRYPTO_SESSION_LOG_ERR("padding sym op ctrl req failed");
      goto error_out;
    }
  ...
  set_sym_session_private_data(sess, dev->driver_id, session_private);
...
static int crypto_virtio_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
...
static int
crypto_virtio_pci_remove(struct rte_pci_device *pci_dev __rte_unused) {
  struct rte_cryptodev *cryptodev;
  ...
  rte_pci_device_name(&pci_dev->addr, cryptodev_name, sizeof(cryptodev_name));
