...
static void modern_read_dev_config(struct virtio_crypto_hw *hw, size_t offset,
                                   void *dst, int length) {
  int i;
  uint8_t *p;
...
static void modern_write_dev_config(struct virtio_crypto_hw *hw, size_t offset,
...
static uint64_t modern_get_features(struct virtio_crypto_hw *hw) {
...
static void modern_set_features(struct virtio_crypto_hw *hw,
                                uint64_t features) {
...
static uint8_t modern_get_status(struct virtio_crypto_hw *hw) {
...
static void modern_reset(struct virtio_crypto_hw *hw) {
  modern_set_status(hw, VIRTIO_CONFIG_STATUS_RESET);
  modern_get_status(hw);
}

static uint8_t modern_get_isr(struct virtio_crypto_hw *hw) {
  return rte_read8(hw->isr);
}

static uint16_t modern_set_config_irq(struct virtio_crypto_hw *hw,
                                      uint16_t vec) {
  rte_write16(vec, &hw->common_cfg->msix_config);
  return rte_read16(&hw->common_cfg->msix_config);
}

static uint16_t modern_set_queue_irq(struct virtio_crypto_hw *hw,
...
static uint16_t modern_get_queue_num(struct virtio_crypto_hw *hw,
                                     uint16_t queue_id) {
  rte_write16(queue_id, &hw->common_cfg->queue_select);
  return rte_read16(&hw->common_cfg->queue_size);
}

static int modern_setup_queue(struct virtio_crypto_hw *hw,
                              struct virtqueue *vq) {
...
static void modern_del_queue(struct virtio_crypto_hw *hw,
                             struct virtqueue *vq) {
  rte_write16(vq->vq_queue_index, &hw->common_cfg->queue_select);
...
static void modern_notify_queue(struct virtio_crypto_hw *hw __rte_unused,
                                struct virtqueue *vq) {
...
static void *get_cfg_addr(struct rte_pci_device *dev,
  ...
  if (offset + length < offset) {
    VIRTIO_CRYPTO_INIT_LOG_ERR("offset(%u) + length(%u) overflows", offset,
                               length);
    return NULL;
  }
  ...
  if (offset + length > dev->mem_resource[bar].len) {
    VIRTIO_CRYPTO_INIT_LOG_ERR(
        "invalid cap: overflows bar space: %u > %" PRIu64, offset + length,
        dev->mem_resource[bar].len);
    return NULL;
  }
...
static int virtio_read_caps(struct rte_pci_device *dev,
  ...
  while (pos) {
    ...
    if (ret < 0) {
      VIRTIO_CRYPTO_INIT_LOG_ERR("failed to read pci cap at pos: %x", pos);
      break;
    }
    ...
    if (cap.cap_vndr != PCI_CAP_ID_VNDR) {
      VIRTIO_CRYPTO_INIT_LOG_DBG("[%2x] skipping non VNDR cap id: %02x", pos,
                                 cap.cap_vndr);
      goto next;
    }

    VIRTIO_CRYPTO_INIT_LOG_DBG(
        "[%2x] cfg type: %u, bar: %u, offset: %04x, len: %u", pos, cap.cfg_type,
    ...
    case VIRTIO_PCI_CAP_NOTIFY_CFG:
      ret = rte_pci_read_config(dev, &hw->notify_off_multiplier, 4,
                                pos + sizeof(cap));
      if (ret != 4)
        VIRTIO_CRYPTO_INIT_LOG_ERR(
...
int vtpci_cryptodev_init(struct rte_pci_device *dev,
  ...
  if (virtio_read_caps(dev, hw) == 0) {
    ...
    crypto_virtio_hw_internal[hw->dev_id].vtpci_ops = &virtio_crypto_modern_ops;
    hw->modern = 1;
    return 0;
  }
