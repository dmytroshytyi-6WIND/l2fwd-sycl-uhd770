 ...
 */
typedef const crypto_func_t crypto_func_tbl_t[CRYPTO_CIPHER_MAX]
                                             [CRYPTO_AUTH_MAX]
 ...
 */
static const crypto_func_tbl_t crypto_op_ca_encrypt = {
    /* [cipher alg][auth alg][key length] = crypto_function, */
    [CIPH_AES_CBC][AUTH_SHA1_HMAC][KEYL(128)] = armv8_enc_aes_cbc_sha1_128,
    [CIPH_AES_CBC][AUTH_SHA256_HMAC][KEYL(128)] = armv8_enc_aes_cbc_sha256_128,
};

static const crypto_func_tbl_t crypto_op_ca_decrypt = {{{NULL}}};

static const crypto_func_tbl_t crypto_op_ac_encrypt = {{{NULL}}};

static const crypto_func_tbl_t crypto_op_ac_decrypt = {
    /* [cipher alg][auth alg][key length] = crypto_function, */
    [CIPH_AES_CBC][AUTH_SHA1_HMAC][KEYL(128)] = armv8_dec_aes_cbc_sha1_128,
    [CIPH_AES_CBC][AUTH_SHA256_HMAC][KEYL(128)] = armv8_dec_aes_cbc_sha256_128,
};
 ...
 */
static const crypto_func_tbl_t *crypto_cipher_auth[] = {
    &crypto_op_ca_encrypt, &crypto_op_ca_decrypt, NULL};

static const crypto_func_tbl_t *crypto_auth_cipher[] = {
    &crypto_op_ac_encrypt, &crypto_op_ac_decrypt, NULL};
 ...
 */
static const crypto_func_tbl_t **crypto_chain_order[] = {
    crypto_cipher_auth, crypto_auth_cipher, NULL};
...
#define CRYPTO_GET_ALGO(order, cop, calg, aalg, keyl)                          \
  ({                                                                           \
    crypto_func_tbl_t *func_tbl = (crypto_chain_order[(order)])[(cop)];        \
                                                                               \
...
static const crypto_key_sched_tbl_t crypto_key_sched_encrypt = {
    /* [cipher alg][key length] = key_expand_func, */
    [CIPH_AES_CBC][KEYL(128)] = armv8_expandkeys_enc_aes_cbc_128,
};

static const crypto_key_sched_tbl_t crypto_key_sched_decrypt = {
    /* [cipher alg][key length] = key_expand_func, */
 ...
 */
static const crypto_key_sched_tbl_t *crypto_key_sched_dir[] = {
    &crypto_key_sched_encrypt, &crypto_key_sched_decrypt, NULL};
...
#define CRYPTO_GET_KEY_SCHED(cop, calg, keyl)                                  \
  ({                                                                           \
    crypto_key_sched_tbl_t *ks_tbl = crypto_key_sched_dir[(cop)];              \
                                                                               \
    (calg >= CRYPTO_CIPHER_MAX) ? NULL : ((*ks_tbl)[(calg)][KEYL(keyl)]);      \
  })
...
auth_set_prerequisites(struct armv8_crypto_session *sess,
  ...
  case RTE_CRYPTO_AUTH_SHA1_HMAC:
     ...
     */
    error = armv8_sha1_block_partial(NULL, sess->auth.hmac.i_key_pad, partial,
                                     SHA1_BLOCK_SIZE);
    if (error != 0)
      return -1;
    memcpy(sess->auth.hmac.i_key_pad, partial, SHA1_BLOCK_SIZE);

    error = armv8_sha1_block_partial(NULL, sess->auth.hmac.o_key_pad, partial,
                                     SHA1_BLOCK_SIZE);
    if (error != 0)
      return -1;
  ...
  case RTE_CRYPTO_AUTH_SHA256_HMAC:
     ...
     * Will be used as initialization state for final HMAC.
     */
    error = armv8_sha256_block_partial(NULL, sess->auth.hmac.i_key_pad, partial,
                                       SHA256_BLOCK_SIZE);
    if (error != 0)
      return -1;
    memcpy(sess->auth.hmac.i_key_pad, partial, SHA256_BLOCK_SIZE);

    error = armv8_sha256_block_partial(NULL, sess->auth.hmac.o_key_pad, partial,
                                       SHA256_BLOCK_SIZE);
    if (error != 0)
      return -1;
...
static int armv8_crypto_set_session_chained_parameters(
  ...
  case 128:
    sess->crypto_func = CRYPTO_GET_ALGO(order, cop, calg, aalg, 128);
    sess->cipher.key_sched = CRYPTO_GET_KEY_SCHED(cop, calg, 128);
    break;
  case 192:
  case 256:
  ...
  if (unlikely(sess->crypto_func == NULL || sess->cipher.key_sched == NULL)) {
    /*
     ...
     */
    ARMV8_CRYPTO_LOG_ERR("No appropriate crypto function for given parameters");
    return -EINVAL;
  }
...
int armv8_crypto_set_session_parameters(
    struct armv8_crypto_session *sess,
    const struct rte_crypto_sym_xform *xform) {
  ...
  if (is_chained_op) {
    ret = armv8_crypto_set_session_chained_parameters(sess, cipher_xform,
                                                      auth_xform);
    if (unlikely(ret != 0)) {
...
get_session(struct armv8_crypto_qp *qp, struct rte_crypto_op *op) {
  ...
  if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
    ...
    if (likely(op->sym->session != NULL)) {
      sess = (struct armv8_crypto_session *)get_sym_session_private_data(
    ...
    if (unlikely(armv8_crypto_set_session_parameters(sess, op->sym->xform) !=
                 0)) {
...
static inline void process_armv8_chained_op(struct armv8_crypto_qp *qp,
  ...
  asrc = rte_pktmbuf_mtod_offset(m_asrc, uint8_t *, op->sym->auth.data.offset);
  ...
  if (sess->auth.operation == RTE_CRYPTO_AUTH_OP_GENERATE) {
    ...
    if (adst == NULL) {
      adst = rte_pktmbuf_mtod_offset(m_adst, uint8_t *,
  ...
  arg.cipher.iv =
      rte_crypto_op_ctod_offset(op, uint8_t *, sess->cipher.iv.offset);
  ...
  if (sess->auth.operation == RTE_CRYPTO_AUTH_OP_VERIFY) {
    if (memcmp(adst, op->sym->auth.digest.data, sess->auth.digest_length) !=
        0) {
...
static inline int process_op(struct armv8_crypto_qp *qp,
                             struct rte_crypto_op *op,
                             struct armv8_crypto_session *sess) {
...
/** Enqueue burst */
static uint16_t armv8_crypto_pmd_enqueue_burst(void *queue_pair,
  ...
  retval = rte_ring_enqueue_burst(qp->processed_ops, (void *)ops, i, NULL);
...
/** Dequeue burst */
static uint16_t armv8_crypto_pmd_dequeue_burst(void *queue_pair,
                                               struct rte_crypto_op **ops,
                                               uint16_t nb_ops) {
  struct armv8_crypto_qp *qp = queue_pair;
...
static int cryptodev_armv8_crypto_create(
    const char *name, struct rte_vdev_device *vdev,
  ...
  if (!rte_cpu_get_flag_enabled(RTE_CPUFLAG_AES)) {
    ARMV8_CRYPTO_LOG_ERR("AES instructions not supported by CPU");
    return -EFAULT;
  }
  ...
  if (!rte_cpu_get_flag_enabled(RTE_CPUFLAG_SHA1) ||
    ...
    ARMV8_CRYPTO_LOG_ERR("SHA1/SHA2 instructions not supported by CPU");
    return -EFAULT;
  }

  /* Check CPU for support for Advance SIMD instruction set */
  if (!rte_cpu_get_flag_enabled(RTE_CPUFLAG_NEON)) {
    ARMV8_CRYPTO_LOG_ERR("Advanced SIMD instructions not supported by CPU");
    return -EFAULT;
  }
  ...
  dev->feature_flags = RTE_CRYPTODEV_FF_SYMMETRIC_CRYPTO |
                       ...
                       RTE_CRYPTODEV_FF_CPU_NEON | RTE_CRYPTODEV_FF_CPU_ARM_CE |
...
init_error:
  ARMV8_CRYPTO_LOG_ERR("driver %s: cryptodev_armv8_crypto_create failed",
                       init_params->name);
...
static int cryptodev_armv8_crypto_init(struct rte_vdev_device *vdev) {
  struct rte_cryptodev_pmd_init_params init_params = {
      "", sizeof(struct armv8_crypto_private), rte_socket_id(),
...
static int cryptodev_armv8_crypto_uninit(struct rte_vdev_device *vdev) {
  ...
  RTE_LOG(INFO, PMD, "Closing ARMv8 crypto device %s on numa socket %u\n", name,
          rte_socket_id());
