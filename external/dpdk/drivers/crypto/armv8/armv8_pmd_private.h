...
#define ARMV8_CRYPTO_LOG_ERR(fmt, args...)                                     \
  rte_log(RTE_LOG_ERR, crypto_armv8_log_type, "[%s] %s() line %u: " fmt "\n",  \
          RTE_STR(CRYPTODEV_NAME_ARMV8_PMD), __func__, __LINE__, ##args)

#define ARMV8_CRYPTO_LOG_INFO(fmt, args...)                                    \
  rte_log(RTE_LOG_INFO, crypto_armv8_log_type, "[%s] %s() line %u: " fmt "\n", \
          RTE_STR(CRYPTODEV_NAME_ARMV8_PMD), __func__, __LINE__, ##args)
...
enum armv8_crypto_cipher_keylen {
  ...
  ARMV8_CRYPTO_CIPHER_KEYLEN_LIST_END = ARMV8_CRYPTO_CIPHER_KEYLEN_NOT_SUPPORTED
};
...
typedef int (*crypto_func_t)(uint8_t *, uint8_t *, uint64_t, uint8_t *,
                             uint8_t *, uint64_t, armv8_cipher_digest_t *);
