...
static const struct rte_cryptodev_capabilities armv8_crypto_pmd_capabilities[] =
    {{/* SHA1 HMAC */
...
/** Start device */
static int armv8_crypto_pmd_start(__rte_unused struct rte_cryptodev *dev) {
  return 0;
}

/** Stop device */
static void armv8_crypto_pmd_stop(__rte_unused struct rte_cryptodev *dev) {}

/** Close device */
static int armv8_crypto_pmd_close(__rte_unused struct rte_cryptodev *dev) {
  return 0;
}

/** Get device statistics */
static void armv8_crypto_pmd_stats_get(struct rte_cryptodev *dev,
...
static void armv8_crypto_pmd_stats_reset(struct rte_cryptodev *dev) {
  int qp_id;
...
/** Get device info */
static void armv8_crypto_pmd_info_get(struct rte_cryptodev *dev,
...
/** Release queue pair */
static int armv8_crypto_pmd_qp_release(struct rte_cryptodev *dev,
                                       uint16_t qp_id) {
...
static int armv8_crypto_pmd_qp_set_unique_name(struct rte_cryptodev *dev,
...
static struct rte_ring *armv8_crypto_pmd_qp_create_processed_ops_ring(
    struct armv8_crypto_qp *qp, unsigned int ring_size, int socket_id) {
  struct rte_ring *r;

  r = rte_ring_lookup(qp->name);
  if (r) {
    if (rte_ring_get_size(r) >= ring_size) {
      ARMV8_CRYPTO_LOG_INFO("Reusing existing ring %s for processed ops",
                            qp->name);
      return r;
    }

    ARMV8_CRYPTO_LOG_ERR("Unable to reuse existing ring %s for processed ops",
                         qp->name);
    return NULL;
  }
...
armv8_crypto_pmd_qp_setup(struct rte_cryptodev *dev, uint16_t qp_id,
  ...
  qp->processed_ops = armv8_crypto_pmd_qp_create_processed_ops_ring(
...
static int armv8_crypto_pmd_sym_session_configure(
