...
int __rte_hot cnxk_mempool_enq(struct rte_mempool *mp, void *const *obj_table,
  ...
  for (index = 0; index < n; index++)
    roc_npa_aura_op_free(mp->pool_id, 0, (uint64_t)obj_table[index]);
...
int cnxk_mempool_alloc(struct rte_mempool *mp) {
  ...
  rc = roc_npa_pool_create(&aura_handle, block_size, block_count, &aura, &pool);
  if (rc) {
  ...
  plt_npa_dbg("block_sz=%lu block_count=%d aura_handle=0x%" PRIx64, block_size,
              block_count, aura_handle);
...
int cnxk_mempool_populate(struct rte_mempool *mp, unsigned int max_objs,
  ...
  plt_npa_dbg("iova %" PRIx64 ", aligned iova %" PRIx64 "", iova - off, iova);
  ...
  roc_npa_aura_op_range_set(mp->pool_id, iova, iova + num_elts * total_elt_sz);
...
static int cnxk_mempool_plt_init(void) {
  int rc = 0;
