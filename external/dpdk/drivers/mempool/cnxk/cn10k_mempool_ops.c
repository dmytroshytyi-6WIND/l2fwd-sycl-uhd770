...
static int batch_op_data_table_create(void) {
  ...
  if (mz == NULL) {
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
      ...
      mz = rte_memzone_reserve_aligned(BATCH_OP_DATA_TABLE_MZ_NAME, sz,
                                       SOCKET_ID_ANY, 0, ROC_ALIGN);
    }
    if (mz == NULL) {
...
static inline struct batch_op_data *batch_op_data_get(uint64_t pool_id) {
  uint64_t aura = roc_npa_aura_handle_to_aura(pool_id);

  return batch_op_data_tbl[aura];
}

static inline void batch_op_data_set(uint64_t pool_id,
...
static int batch_op_init(struct rte_mempool *mp) {
  struct batch_op_data *op_data;
  int i;
...
static void batch_op_fini(struct rte_mempool *mp) {
  struct batch_op_data *op_data;
  int i;
  ...
  for (i = 0; i < RTE_MAX_LCORE; i++) {
    ...
    if (mem->status == BATCH_ALLOC_OP_ISSUED) {
      mem->sz = roc_npa_aura_batch_alloc_extract(mem->objs, mem->objs,
                                                 BATCH_ALLOC_SZ);
      mem->status = BATCH_ALLOC_OP_DONE;
    }
    if (mem->status == BATCH_ALLOC_OP_DONE) {
      roc_npa_aura_op_bulk_free(mp->pool_id, mem->objs, mem->sz, 1);
...
static int __rte_hot cn10k_mempool_enq(struct rte_mempool *mp,
...
static unsigned int cn10k_mempool_get_count(const struct rte_mempool *mp) {
  struct batch_op_data *op_data;
  ...
  for (i = 0; i < RTE_MAX_LCORE; i++) {
    ...
    if (mem->status == BATCH_ALLOC_OP_ISSUED)
      count += roc_npa_aura_batch_alloc_count(mem->objs, BATCH_ALLOC_SZ);
...
static int __rte_hot cn10k_mempool_deq(struct rte_mempool *mp, void **obj_table,
                                       unsigned int n) {
  struct batch_op_data *op_data;
  ...
  if (mem->status == BATCH_ALLOC_OP_NOT_ISSUED) {
    rc = roc_npa_aura_batch_alloc_issue(mp->pool_id, mem->objs, BATCH_ALLOC_SZ,
                                        0, 1);
...
static int cn10k_mempool_alloc(struct rte_mempool *mp) {
  uint32_t block_size;
  size_t padding;
  int rc;
  ...
  if (mp->header_size % ROC_ALIGN != 0) {
    padding = RTE_ALIGN_CEIL(mp->header_size, ROC_ALIGN) - mp->header_size;
...
static void cn10k_mempool_free(struct rte_mempool *mp) {
  batch_op_fini(mp);
  cnxk_mempool_free(mp);
}

int cn10k_mempool_plt_init(void) { return batch_op_data_table_create(); }
