...
static inline uint32_t npa_aura_size_to_u32(uint8_t val) {
...
static int parse_max_pools_handler(const char *key, const char *value,
                                   void *extra_args) {
  RTE_SET_USED(key);
  uint32_t val;
...
static inline uint32_t parse_max_pools(struct rte_devargs *devargs) {
  ...
  rte_kvargs_process(kvlist, CNXK_NPA_MAX_POOLS_PARAM, &parse_max_pools_handler,
                     &max_pools);
...
static int cnxk_mempool_plt_parse_devargs(struct rte_pci_device *pci_dev) {
  roc_idev_npa_maxpools_set(parse_max_pools(pci_dev->device.devargs));
  return 0;
}

static inline char *npa_dev_to_name(struct rte_pci_device *pci_dev,
                                    char *name) {
...
static int npa_init(struct rte_pci_device *pci_dev) {
  char name[CNXK_NPA_DEV_NAME_LEN];
  const struct rte_memzone *mz;
  struct roc_npa *dev;
  int rc = -ENOMEM;

  mz = rte_memzone_reserve_aligned(npa_dev_to_name(pci_dev, name), sizeof(*dev),
...
static int npa_fini(struct rte_pci_device *pci_dev) {
  char name[CNXK_NPA_DEV_NAME_LEN];
  const struct rte_memzone *mz;
  int rc;
...
static int npa_remove(struct rte_pci_device *pci_dev) {
...
static int npa_probe(struct rte_pci_driver *pci_drv,
