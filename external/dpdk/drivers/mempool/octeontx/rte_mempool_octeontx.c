...
static int octeontx_fpavf_alloc(struct rte_mempool *mp) {
  uintptr_t pool;
  ...
  pool = octeontx_fpa_bufpool_create(object_size, memseg_count,
                                     OCTEONTX_FPAVF_BUF_OFFSET, mp->socket_id);
...
static void octeontx_fpavf_free(struct rte_mempool *mp) {
  uintptr_t pool;
...
static __rte_always_inline void *octeontx_fpa_bufpool_alloc(uintptr_t handle) {
  return (void *)(uintptr_t)fpavf_read64(
      (void *)(handle + FPA_VF_VHAURA_OP_ALLOC(0)));
}

static __rte_always_inline void octeontx_fpa_bufpool_free(uintptr_t handle,
                                                          void *buf) {
  uint64_t free_addr =
      FPA_VF_FREE_ADDRS_S(FPA_VF_VHAURA_OP_FREE(0), 0 /* DWB */, 1 /* FABS */);

  fpavf_write64((uintptr_t)buf, (void *)(uintptr_t)(handle + free_addr));
}

static int octeontx_fpavf_enqueue(struct rte_mempool *mp,
...
static int octeontx_fpavf_dequeue(struct rte_mempool *mp, void **obj_table,
                                  unsigned int n) {
  unsigned int index;
  uintptr_t pool;
  void *obj;
...
static unsigned int octeontx_fpavf_get_count(const struct rte_mempool *mp) {
  uintptr_t pool;
...
static ssize_t octeontx_fpavf_calc_mem_size(const struct rte_mempool *mp,
  ...
  mem_size = rte_mempool_op_calc_mem_size_helper(
...
static int octeontx_fpavf_populate(struct rte_mempool *mp,
  ...
  return rte_mempool_op_populate_helper(mp, RTE_MEMPOOL_POPULATE_F_ALIGN_OBJ,
                                        max_objs, vaddr, iova, len, obj_cb,
                                        obj_cb_arg);
}
