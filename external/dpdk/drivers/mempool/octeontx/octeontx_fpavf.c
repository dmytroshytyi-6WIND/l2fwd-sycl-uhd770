...
static int octeontx_fpa_gpool_alloc(unsigned int object_size) {
  ...
  for (i = 0; i < FPA_VF_MAX; i++) {
    ...
    if ((fpadev.pool[i].bar0 == NULL) || (fpadev.pool[i].is_inuse == true) ||
...
/* lock is taken by caller */
static __rte_always_inline uintptr_t octeontx_fpa_gpool2handle(uint16_t gpool) {
  struct fpavf_res *res = NULL;
...
static __rte_always_inline bool octeontx_fpa_handle_valid(uintptr_t handle) {
  struct fpavf_res *res = NULL;
  uint8_t gpool;
  int i;
  bool ret = false;
...
static int octeontx_fpapf_pool_setup(unsigned int gpool, unsigned int buf_size,
                                     signed short buf_offset,
  ...
  ret = octeontx_mbox_send(&hdr, &cfg, sizeof(struct octeontx_mbox_fpa_cfg),
                           &resp, sizeof(resp));
  if (ret < 0) {
    ret = -EACCES;
    goto err;
  ...
  fpavf_log_dbg(
      " vfid %d gpool %d aid %d pool_cfg 0x%x pool_stack_base %" PRIx64
      " pool_stack_end %" PRIx64 " aura_cfg %" PRIx64 "\n",
...
static int octeontx_fpapf_pool_destroy(unsigned int gpool_index) {
  struct octeontx_mbox_hdr hdr;
  struct dcfg_resp resp;
  struct octeontx_mbox_fpa_cfg cfg;
  struct fpavf_res *fpa = NULL;
...
static int octeontx_fpapf_aura_attach(unsigned int gpool_index) {
  struct octeontx_mbox_hdr hdr;
  struct dcfg_resp resp;
  struct octeontx_mbox_fpa_cfg cfg;
  int ret = 0;
  ...
  if (ret < 0) {
    ...
    fpavf_log_err("aura %d to pool %d. Err=%d. FuncErr=%d\n",
                  FPA_AURA_IDX(gpool_index), gpool_index, ret, hdr.res_code);
    ret = -EACCES;
    goto err;
  }
err:
...
static int octeontx_fpapf_aura_detach(unsigned int gpool_index) {
  ...
  if (ret < 0) {
    fpavf_log_err("Couldn't detach FPA aura %d Err=%d FuncErr=%d\n",
                  FPA_AURA_IDX(gpool_index), ret, hdr.res_code);
    ret = -EINVAL;
  }
...
int octeontx_fpavf_pool_set_range(uintptr_t handle, unsigned long memsz,
  ...
  fpavf_write64((uintptr_t)memva,
                (void *)((uintptr_t)handle + FPA_VF_VHPOOL_START_ADDR(gpool)));
  fpavf_write64(va_end,
                (void *)((uintptr_t)handle + FPA_VF_VHPOOL_END_ADDR(gpool)));
  return 0;
}

static int octeontx_fpapf_start_count(uint16_t gpool_index) {
  int ret = 0;
  struct octeontx_mbox_hdr hdr = {0};
...
static __rte_always_inline int octeontx_fpavf_free(unsigned int gpool) {
...
static __rte_always_inline int octeontx_gpool_free(uint16_t gpool) {
  struct fpavf_res *res = octeontx_get_fpavf(gpool);
 ...
 */
int octeontx_fpa_bufpool_block_size(uintptr_t handle) {
  struct fpavf_res *res = NULL;
  uint8_t gpool;
...
int octeontx_fpa_bufpool_free_count(uintptr_t handle) {
  ...
  cnt = fpavf_read64((void *)((uintptr_t)pool_bar + FPA_VF_VHAURA_CNT(gaura)));
  limit = fpavf_read64(
      (void *)((uintptr_t)pool_bar + FPA_VF_VHAURA_CNT_LIMIT(gaura)));

  avail = fpavf_read64(
      (void *)((uintptr_t)pool_bar + FPA_VF_VHPOOL_AVAILABLE(gpool)));

  return RTE_MIN(avail, (limit - cnt));
}

uintptr_t octeontx_fpa_bufpool_create(unsigned int object_size,
  ...
  res = octeontx_fpapf_pool_setup(gpool, object_size, buf_offset, object_count);
  if (res < 0) {
    errno = res;
    goto error_gpool_free;
  }
  ...
  fpavf_write64(object_count,
                (void *)((uintptr_t)pool_bar + FPA_VF_VHAURA_CNT(gaura)));
  fpavf_write64(object_count,
                (void *)((uintptr_t)pool_bar + FPA_VF_VHAURA_CNT_LIMIT(gaura)));
 ...
 */
int octeontx_fpa_bufpool_destroy(uintptr_t handle, int node_id) {
  ...
  while (avail--) {
    ...
    node = (void *)(uintptr_t)fpavf_read64(
        (void *)(pool_bar + FPA_VF_VHAURA_OP_ALLOC(gaura)));
  ...
  for (curr = head; curr != NULL && curr[0] != NULL; curr = curr[0]) {
    if (curr == curr[0] || ((uintptr_t)curr != ((uintptr_t)curr[0] - sz))) {
  ...
  fpavf_write64(
      ~0ul, (void *)((uintptr_t)pool_bar + FPA_VF_VHPOOL_START_ADDR(gpool)));
  fpavf_write64(~0ul,
                (void *)((uintptr_t)pool_bar + FPA_VF_VHPOOL_END_ADDR(gpool)));
  ...
  fpavf_write64(0,
                (void *)((uintptr_t)pool_bar + FPA_VF_VHAURA_CNT_LIMIT(gaura)));
  fpavf_write64(
      0, (void *)((uintptr_t)pool_bar + FPA_VF_VHAURA_CNT_THRESHOLD(gaura)));
...
static void octeontx_fpavf_setup(void) {
  uint8_t i;
...
static int octeontx_fpavf_identify(void *bar0) {
  uint64_t val;
  uint16_t domain_id;
  uint16_t vf_id;
  uint64_t stack_ln_ptr;
  ...
  val =
      fpavf_read64((void *)((uintptr_t)bar0 + FPA_VF_VHAURA_CNT_THRESHOLD(0)));

  domain_id = (val >> 8) & 0xffff;
  vf_id = (val >> 24) & 0xffff;

  stack_ln_ptr =
      fpavf_read64((void *)((uintptr_t)bar0 + FPA_VF_VHPOOL_THRESHOLD(0)));
...
static int fpavf_probe(struct rte_pci_driver *pci_drv,
  ...
  fpavf_log_dbg("total_fpavfs %d bar0 %p domain %d vf %d stk_ln_ptr 0x%x",
                fpadev.total_gpool_cnt, fpa->bar0, fpa->domain_id, fpa->vf_id,
                (unsigned int)fpa->stack_ln_ptr);

  return 0;
}

static const struct rte_pci_id pci_fpavf_map[] = {
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CAVIUM, PCI_DEVICE_ID_OCTEONTX_FPA_VF)},
    {
        .vendor_id = 0,
    },
};
