...
static struct bucket_stack *bucket_stack_create(const struct rte_mempool *mp,
                                                unsigned int n_elts) {
  struct bucket_stack *stack;

  stack = rte_zmalloc_socket(
      "bucket_stack", sizeof(struct bucket_stack) + n_elts * sizeof(void *),
...
static void bucket_stack_push(struct bucket_stack *stack, void *obj) {
  RTE_ASSERT(stack->top < stack->limit);
  stack->objects[stack->top++] = obj;
}

static void *bucket_stack_pop_unsafe(struct bucket_stack *stack) {
  RTE_ASSERT(stack->top > 0);
  return stack->objects[--stack->top];
}

static void *bucket_stack_pop(struct bucket_stack *stack) {
  if (stack->top == 0)
    return NULL;
  return bucket_stack_pop_unsafe(stack);
}

static int bucket_enqueue_single(struct bucket_data *bd, void *obj) {
  int rc = 0;
  ...
  if (likely(hdr->lcore_id == lcore_id)) {
    ...
    struct rte_ring *adopt_ring = bd->adoption_buffer_rings[hdr->lcore_id];
...
static int bucket_enqueue(struct rte_mempool *mp, void *const *obj_table,
                          unsigned int n) {
  ...
  if (local_stack->top > bd->bucket_stack_thresh) {
    rte_ring_enqueue_bulk(bd->shared_bucket_ring,
                          &local_stack->objects[bd->bucket_stack_thresh],
                          local_stack->top - bd->bucket_stack_thresh, NULL);
    local_stack->top = bd->bucket_stack_thresh;
  }
  return rc;
}

static void **bucket_fill_obj_table(const struct bucket_data *bd, void **pstart,
...
static int bucket_dequeue_orphans(struct bucket_data *bd, void **obj_table,
  ...
  rc =
      rte_ring_dequeue_bulk(bd->shared_orphan_ring, obj_table, n_orphans, NULL);
  if (unlikely(rc != (int)n_orphans)) {
    ...
    if (objptr == NULL) {
      rc = rte_ring_dequeue(bd->shared_bucket_ring, (void **)&objptr);
      if (rc != 0) {
        rte_errno = ENOBUFS;
        return -rte_errno;
      }
    ...
    hdr->fill_cnt = 0;
    bucket_fill_obj_table(bd, (void **)&objptr, obj_table, n_orphans);
    for (i = n_orphans; i < bd->obj_per_bucket;
         i++, objptr += bd->total_elt_size) {
      rc = rte_ring_enqueue(bd->shared_orphan_ring, objptr);
      if (rc != 0) {
        RTE_ASSERT(0);
        rte_errno = -rc;
        return rc;
...
static int bucket_dequeue_buckets(struct bucket_data *bd, void **obj_table,
  ...
  while (n_buckets_from_stack-- > 0) {
    ...
    obj_table = bucket_fill_obj_table(bd, &obj, obj_table, bd->obj_per_bucket);
  }
  while (n_buckets-- > 0) {
    struct bucket_header *hdr;

    if (unlikely(rte_ring_dequeue(bd->shared_bucket_ring, (void **)&hdr) !=
                 0)) {
      /*
       * Return the already-dequeued buffers
       * back to the mempool
       */
      bucket_enqueue(bd->pool, obj_table_base, obj_table - obj_table_base);
      rte_errno = ENOBUFS;
      return -rte_errno;
    }
    hdr->lcore_id = rte_lcore_id();
    obj_table =
        bucket_fill_obj_table(bd, (void **)&hdr, obj_table, bd->obj_per_bucket);
  }

  return 0;
}

static int bucket_adopt_orphans(struct bucket_data *bd) {
  int rc = 0;
  struct rte_ring *adopt_ring = bd->adoption_buffer_rings[rte_lcore_id()];
...
static int bucket_dequeue(struct rte_mempool *mp, void **obj_table,
                          unsigned int n) {
  struct bucket_data *bd = mp->pool_data;
  ...
  if (unlikely(n_orphans > 0)) {
    rc = bucket_dequeue_orphans(
        bd, obj_table + (n_buckets * bd->obj_per_bucket), n_orphans);
    if (rc != 0)
      return rc;
  }
  ...
  if (likely(n_buckets > 0)) {
    ...
    if (unlikely(rc != 0) && n_orphans > 0) {
      rte_ring_enqueue_bulk(bd->shared_orphan_ring,
                            obj_table + (n_buckets * bd->obj_per_bucket),
                            n_orphans, NULL);
    }
  }
...
static int bucket_dequeue_contig_blocks(struct rte_mempool *mp,
  ...
  if (n > 0) {
    if (unlikely(rte_ring_dequeue_bulk(bd->shared_bucket_ring, first_objp, n,
                                       NULL) != n)) {
      /* Return the already dequeued buckets */
      while (first_objp-- != first_obj_table) {
        bucket_stack_push(cur_stack, (uint8_t *)*first_objp - header_size);
      }
      rte_errno = ENOBUFS;
      return -rte_errno;
    }
...
static int bucket_count_per_lcore(unsigned int lcore_id, void *arg) {
  struct bucket_count_per_lcore_ctx *bplc = arg;

  bplc->count += bplc->bd->obj_per_bucket * bplc->bd->buckets[lcore_id]->top;
  bplc->count += rte_ring_count(bplc->bd->adoption_buffer_rings[lcore_id]);
  return 0;
}

static void count_underfilled_buckets(struct rte_mempool *mp, void *opaque,
                                      struct rte_mempool_memhdr *memhdr,
                                      __rte_unused unsigned int mem_idx) {
  unsigned int *pcount = opaque;
  const struct bucket_data *bd = mp->pool_data;
  unsigned int bucket_page_sz = (unsigned int)(~bd->bucket_page_mask + 1);
  uintptr_t align;
  uint8_t *iter;
  ...
  for (iter = (uint8_t *)memhdr->addr + align;
       iter < (uint8_t *)memhdr->addr + memhdr->len; iter += bucket_page_sz) {
...
static unsigned int bucket_get_count(const struct rte_mempool *mp) {
...
static int bucket_init_per_lcore(unsigned int lcore_id, void *arg) {
  ...
  bd->adoption_buffer_rings[lcore_id] = rte_ring_create(
...
static void bucket_uninit_per_lcore(unsigned int lcore_id, void *arg) {
  struct bucket_data *bd = arg;
...
static int bucket_alloc(struct rte_mempool *mp) {
  int rg_flags = 0;
  int rc = 0;
  char rg_name[RTE_RING_NAMESIZE];
  ...
  bd = rte_zmalloc_socket("bucket_pool", sizeof(*bd), RTE_CACHE_LINE_SIZE,
                          mp->socket_id);
  if (bd == NULL) {
    rc = -ENOMEM;
  ...
  bd->bucket_mem_size =
      RTE_MIN(pg_sz, (size_t)(RTE_DRIVER_MEMPOOL_BUCKET_SIZE_KB * 1024));
  bd->obj_per_bucket =
      (bd->bucket_mem_size - bucket_header_size) / bd->total_elt_size;
  ...
  bd->lcore_callback_handle = rte_lcore_callback_register(
      "bucket", bucket_init_per_lcore, bucket_uninit_per_lcore, bd);
  ...
  if (mp->flags & RTE_MEMPOOL_F_SC_GET)
    rg_flags |= RING_F_SC_DEQ;
  rc = snprintf(rg_name, sizeof(rg_name), RTE_MEMPOOL_MZ_FORMAT ".0", mp->name);
  if (rc < 0 || rc >= (int)sizeof(rg_name)) {
    rc = -ENAMETOOLONG;
    goto invalid_shared_orphan_ring;
  }
  bd->shared_orphan_ring = rte_ring_create(
      rg_name, rte_align32pow2(mp->size + 1), mp->socket_id, rg_flags);
  ...
  rc = snprintf(rg_name, sizeof(rg_name), RTE_MEMPOOL_MZ_FORMAT ".1", mp->name);
  if (rc < 0 || rc >= (int)sizeof(rg_name)) {
    rc = -ENAMETOOLONG;
    goto invalid_shared_bucket_ring;
  }
  bd->shared_bucket_ring = rte_ring_create(
      rg_name, rte_align32pow2((mp->size + 1) / bd->obj_per_bucket),
...
static void bucket_free(struct rte_mempool *mp) {
  struct bucket_data *bd = mp->pool_data;
...
static ssize_t bucket_calc_mem_size(const struct rte_mempool *mp,
                                    uint32_t obj_num,
                                    __rte_unused uint32_t pg_shift,
                                    size_t *min_total_elt_size, size_t *align) {
  struct bucket_data *bd = mp->pool_data;
  ...
  *min_total_elt_size = bucket_page_sz;
   ...
   */
  return ((obj_num + bd->obj_per_bucket - 1) / bd->obj_per_bucket) *
         bucket_page_sz;
}

static int bucket_populate(struct rte_mempool *mp, unsigned int max_objs,
  ...
  bucket_page_sz = rte_align32pow2(bd->bucket_mem_size);
  align =
      RTE_PTR_ALIGN_CEIL((uintptr_t)vaddr, bucket_page_sz) - (uintptr_t)vaddr;
  ...
  for (iter = (uint8_t *)vaddr + align, n_objs = 0;
    ...
    rc = rte_mempool_op_populate_helper(
        mp, 0, RTE_MIN(bd->obj_per_bucket, max_objs - n_objs),
        iter + bucket_header_sz, iova, chunk_len, obj_cb, obj_cb_arg);
    if (rc < 0)
      return rc;
    n_objs += rc;
    if (iova != RTE_BAD_IOVA)
...
static int bucket_get_info(const struct rte_mempool *mp,
