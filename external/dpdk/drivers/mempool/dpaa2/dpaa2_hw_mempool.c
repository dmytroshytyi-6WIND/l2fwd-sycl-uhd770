...
static int rte_hw_mbuf_create_pool(struct rte_mempool *mp) {
  ...
  if (rte_dpaa2_bpid_info == NULL) {
    rte_dpaa2_bpid_info = (struct dpaa2_bp_info *)rte_malloc(
        NULL, sizeof(struct dpaa2_bp_info) * MAX_BPID, RTE_CACHE_LINE_SIZE);
    if (rte_dpaa2_bpid_info == NULL)
      return -ENOMEM;
  ...
  if (ret != 0) {
    DPAA2_MEMPOOL_ERR("Resource enable failure with err code: %d", ret);
    goto err1;
  }

  ret = dpbp_get_attributes(&avail_dpbp->dpbp, CMD_PRI_LOW, avail_dpbp->token,
                            &dpbp_attr);
  if (ret != 0) {
    DPAA2_MEMPOOL_ERR("Resource read failure with err code: %d", ret);
    goto err2;
  }
  ...
  bp_list = rte_malloc(NULL, sizeof(struct dpaa2_bp_list), RTE_CACHE_LINE_SIZE);
  if (!bp_list) {
    DPAA2_MEMPOOL_ERR("Unable to allocate buffer pool memory");
    ret = -ENOMEM;
    goto err3;
  ...
  bp_list->buf_pool.size =
      mp->elt_size - sizeof(struct rte_mbuf) - rte_pktmbuf_priv_size(mp);
...
static void rte_hw_mbuf_free_pool(struct rte_mempool *mp) {
...
static void rte_dpaa2_mbuf_release(struct rte_mempool *pool __rte_unused,
                                   void *const *obj_table, uint32_t bpid,
  ...
  for (i = 0; i < n; i++) {
    ...
    bufs[i] = (uint64_t)rte_mempool_virt2iova(obj_table[i]) + meta_data_size;
#else
  ...
  /* feed them to bman */
  retry_count = 0;
  while ((ret = qbman_swp_release(swp, &releasedesc, bufs, n)) == -EBUSY) {
    retry_count++;
...
#ifdef RTE_LIBRTE_DPAA2_USE_PHYS_IOVA
      bufs[i] =
          (uint64_t)rte_mempool_virt2iova(obj_table[n + i]) + meta_data_size;
#else
...
struct rte_mbuf *rte_dpaa2_mbuf_from_buf_addr(struct rte_mempool *mp,
  ...
  return (struct rte_mbuf *)((uint8_t *)buf_addr - bp_info->meta_data_size);
}

int rte_dpaa2_mbuf_alloc_bulk(struct rte_mempool *pool, void **obj_table,
                              unsigned int count) {
  ...
  while (n < count) {
    ...
    if ((count - n) > DPAA2_MBUF_MAX_ACQ_REL) {
      ret = qbman_swp_acquire(swp, bpid, (void *)bufs, DPAA2_MBUF_MAX_ACQ_REL);
    } else {
      ret = qbman_swp_acquire(swp, bpid, (void *)bufs, count - n);
    }
    /* In case of less than requested number of buffers available
     * in pool, qbman_swp_acquire returns 0
     */
    if (ret <= 0) {
      DPAA2_MEMPOOL_DP_DEBUG("Buffer acquire failed with err code: %d", ret);
      /* The API expect the exact number of requested bufs */
      /* Releasing all buffers allocated */
      rte_dpaa2_mbuf_release(pool, obj_table, bpid, bp_info->meta_data_size, n);
      return -ENOBUFS;
    }
    /* assigning mbuf from the acquired objects */
    for (i = 0; (i < ret) && bufs[i]; i++) {
      DPAA2_MODIFY_IOVA_TO_VADDR(bufs[i], size_t);
      obj_table[n] = (struct rte_mbuf *)(bufs[i] - bp_info->meta_data_size);
      DPAA2_MEMPOOL_DP_DEBUG("Acquired %p address %p from BMAN\n",
...
static int rte_hw_mbuf_free_bulk(struct rte_mempool *pool,
...
static unsigned int rte_hw_mbuf_get_count(const struct rte_mempool *mp) {
  int ret;
  ...
  ret = dpbp_get_num_free_bufs(&mc_io, CMD_PRI_LOW, dpbp_node->token,
                               &num_of_bufs);
  if (ret) {
    DPAA2_MEMPOOL_ERR("Unable to obtain free buf count (err=%d)", ret);
    return 0;
  }
...
static int dpaa2_populate(struct rte_mempool *mp, unsigned int max_objs,
  ...
  if (!msl) {
    ...
    rte_fslmc_vfio_mem_dmamap((size_t)vaddr, (size_t)paddr, (size_t)len);
  }
  /* Insert entry into the PA->VA Table */
  dpaax_iova_table_update(paddr, vaddr, len);

  return rte_mempool_op_populate_helper(mp, 0, max_objs, vaddr, paddr, len,
                                        obj_cb, obj_cb_arg);
}
