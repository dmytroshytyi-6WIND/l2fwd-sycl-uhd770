...
static int __stack_alloc(struct rte_mempool *mp, uint32_t flags) {
...
static int stack_alloc(struct rte_mempool *mp) { return __stack_alloc(mp, 0); }

static int lf_stack_alloc(struct rte_mempool *mp) {
...
static unsigned stack_get_count(const struct rte_mempool *mp) {
  struct rte_stack *s = mp->pool_data;

  return rte_stack_count(s);
}

static void stack_free(struct rte_mempool *mp) {
  struct rte_stack *s = mp->pool_data;

  rte_stack_free(s);
}

static struct rte_mempool_ops ops_stack = {.name = "stack",
                                           .alloc = stack_alloc,
                                           .free = stack_free,
...
static struct rte_mempool_ops ops_lf_stack = {.name = "lf_stack",
