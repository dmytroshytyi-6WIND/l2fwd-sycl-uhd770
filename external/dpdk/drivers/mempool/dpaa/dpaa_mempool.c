...
static int dpaa_mbuf_create_pool(struct rte_mempool *mp) {
  ...
  struct bman_pool_params params = {.flags = BMAN_POOL_FLAG_DYNAMIC_BPID};
  ...
  if (unlikely(!DPAA_PER_LCORE_PORTAL)) {
    ...
    if (ret) {
      DPAA_MEMPOOL_ERR("rte_dpaa_portal_init failed with ret: %d", ret);
      return -1;
    }
  }
  ...
  if (num_bufs)
    DPAA_MEMPOOL_WARN("drained %u bufs from BPID %d", num_bufs, bpid);

  if (rte_dpaa_bpid_info == NULL) {
    rte_dpaa_bpid_info = (struct dpaa_bp_info *)rte_zmalloc(
        NULL, sizeof(struct dpaa_bp_info) * DPAA_MAX_BPOOLS,
        RTE_CACHE_LINE_SIZE);
    if (rte_dpaa_bpid_info == NULL) {
      bman_free_pool(bp);
      return -ENOMEM;
  ...
  bp_info = rte_malloc(NULL, sizeof(struct dpaa_bp_info), RTE_CACHE_LINE_SIZE);
  if (!bp_info) {
...
static void dpaa_mbuf_free_pool(struct rte_mempool *mp) {
...
static void dpaa_buf_free(struct dpaa_bp_info *bp_info, uint64_t addr) {
  struct bm_buffer buf;
  int ret;

  DPAA_MEMPOOL_DPDEBUG("Free 0x%" PRIx64 " to bpid: %d", addr, bp_info->bpid);
...
static int dpaa_mbuf_free_bulk(struct rte_mempool *pool, void *const *obj_table,
                               unsigned int n) {
  struct dpaa_bp_info *bp_info = DPAA_MEMPOOL_TO_POOL_INFO(pool);
  int ret;
  unsigned int i = 0;

  DPAA_MEMPOOL_DPDEBUG("Request to free %d buffers in bpid = %d", n,
                       bp_info->bpid);
  ...
  while (i < n) {
    ...
    if (unlikely(!bp_info->ptov_off)) {
      ...
      if (bp_info->flags & DPAA_MPOOL_SINGLE_SEGMENT) {
        ...
        rte_dpaa_bpid_info[bp_info->bpid].ptov_off = bp_info->ptov_off;
      }
    }

    dpaa_buf_free(bp_info, (uint64_t)phy + bp_info->meta_data_size);
    i = i + 1;
  }

  DPAA_MEMPOOL_DPDEBUG("freed %d buffers in bpid =%d", n, bp_info->bpid);

  return 0;
}

static int dpaa_mbuf_alloc_bulk(struct rte_mempool *pool, void **obj_table,
                                unsigned int count) {
  ...
  DPAA_MEMPOOL_DPDEBUG("Request to alloc %d buffers in bpid = %d", count,
                       bp_info->bpid);

  if (unlikely(count >= (RTE_MEMPOOL_CACHE_MAX_SIZE * 2))) {
    DPAA_MEMPOOL_ERR("Unable to allocate requested (%u) buffers", count);
    return -1;
  }
  ...
  while (n < count) {
    ...
    if ((count - n) > DPAA_MBUF_MAX_ACQ_REL) {
      ret = bman_acquire(bp_info->bp, bufs, DPAA_MBUF_MAX_ACQ_REL, 0);
    } else {
     ...
     */
    if (ret <= 0) {
      DPAA_MEMPOOL_DPDEBUG("Buffer acquire failed (%d)", ret);
    ...
    for (i = 0; (i < ret) && bufs[i].addr; i++) {
      ...
      m[n] = (struct rte_mbuf *)((char *)bufaddr - bp_info->meta_data_size);
      DPAA_MEMPOOL_DPDEBUG("Paddr (%p), FD (%p) from BMAN", (void *)bufaddr,
                           (void *)m[n]);
      n++;
    }
  }

  DPAA_MEMPOOL_DPDEBUG("Allocated %d buffers from bpid=%d", n, bp_info->bpid);
  return 0;
}

static unsigned int dpaa_mbuf_get_count(const struct rte_mempool *mp) {
  struct dpaa_bp_info *bp_info;
...
static int dpaa_populate(struct rte_mempool *mp, unsigned int max_objs,
  ...
  if (!ms) {
    ...
    DPAA_MEMPOOL_WARN(
        "Fast Physical to Virtual Addr translation would not be available.");
