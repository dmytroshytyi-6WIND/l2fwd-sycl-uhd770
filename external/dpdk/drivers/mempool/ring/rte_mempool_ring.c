...
static int common_ring_mp_enqueue(struct rte_mempool *mp,
                                  void *const *obj_table, unsigned n) {
  return rte_ring_mp_enqueue_bulk(mp->pool_data, obj_table, n, NULL) == 0
             ? -ENOBUFS
             : 0;
}

static int common_ring_sp_enqueue(struct rte_mempool *mp,
                                  void *const *obj_table, unsigned n) {
...
static int rts_ring_mp_enqueue(struct rte_mempool *mp, void *const *obj_table,
                               unsigned int n) {
  return rte_ring_mp_rts_enqueue_bulk(mp->pool_data, obj_table, n, NULL) == 0
             ? -ENOBUFS
             : 0;
}

static int hts_ring_mp_enqueue(struct rte_mempool *mp, void *const *obj_table,
                               unsigned int n) {
  return rte_ring_mp_hts_enqueue_bulk(mp->pool_data, obj_table, n, NULL) == 0
             ? -ENOBUFS
             : 0;
}

static int common_ring_mc_dequeue(struct rte_mempool *mp, void **obj_table,
                                  unsigned n) {
  return rte_ring_mc_dequeue_bulk(mp->pool_data, obj_table, n, NULL) == 0
             ? -ENOBUFS
             : 0;
}

static int common_ring_sc_dequeue(struct rte_mempool *mp, void **obj_table,
                                  unsigned n) {
...
static int rts_ring_mc_dequeue(struct rte_mempool *mp, void **obj_table,
                               unsigned int n) {
  return rte_ring_mc_rts_dequeue_bulk(mp->pool_data, obj_table, n, NULL) == 0
             ? -ENOBUFS
             : 0;
}

static int hts_ring_mc_dequeue(struct rte_mempool *mp, void **obj_table,
                               unsigned int n) {
  return rte_ring_mc_hts_dequeue_bulk(mp->pool_data, obj_table, n, NULL) == 0
             ? -ENOBUFS
             : 0;
}

static unsigned common_ring_get_count(const struct rte_mempool *mp) {
  return rte_ring_count(mp->pool_data);
}

static int ring_alloc(struct rte_mempool *mp, uint32_t rg_flags) {
  int ret;
...
static int common_ring_alloc(struct rte_mempool *mp) {
  uint32_t rg_flags = 0;
...
static int rts_ring_alloc(struct rte_mempool *mp) {
  return ring_alloc(mp, RING_F_MP_RTS_ENQ | RING_F_MC_RTS_DEQ);
}

static int hts_ring_alloc(struct rte_mempool *mp) {
  return ring_alloc(mp, RING_F_MP_HTS_ENQ | RING_F_MC_HTS_DEQ);
}

static void common_ring_free(struct rte_mempool *mp) {
