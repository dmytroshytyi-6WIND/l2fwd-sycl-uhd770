...
static inline int sfc_vdpa_get_eth_addr(const char *key __rte_unused,
...
static int sfc_vdpa_set_mac_filter(efx_nic_t *nic, efx_filter_spec_t *spec,
...
int sfc_vdpa_filter_config(struct sfc_vdpa_ops_data *ops_data) {
  ...
  rc = rte_kvargs_process(sva->kvargs, SFC_VDPA_MAC_ADDR,
                          &sfc_vdpa_get_eth_addr, &ucast_eth_addr);
  if (rc < 0)
    return -1;
  ...
  rc = sfc_vdpa_set_mac_filter(nic, spec, qid, bcast_eth_addr.addr_bytes);
  if (rc != 0)
    sfc_vdpa_err(ops_data->dev_handle,
                 "broadcast MAC filter insertion failed: %s", rte_strerror(rc));
  else
  ...
  rc = sfc_vdpa_set_mac_filter(nic, spec, qid, ucast_eth_addr.addr_bytes);
  if (rc != 0)
  ...
  if (rc != 0)
    sfc_vdpa_err(sva, "mcast filter insertion failed: %s", rte_strerror(rc));
  else
    sva->filters.filter_cnt++;
...
int sfc_vdpa_filter_remove(struct sfc_vdpa_ops_data *ops_data) {
  ...
  for (i = 0; i < sva->filters.filter_cnt; i++) {
    ...
    if (rc != 0)
      sfc_vdpa_err(sva, "remove HW filter failed for entry %d: %s", i,
                   rte_strerror(rc));
  }
