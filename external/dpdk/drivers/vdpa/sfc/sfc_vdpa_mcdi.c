...
static int sfc_vdpa_mcdi_dma_alloc(void *cookie, const char *name, size_t len,
                                   efsys_mem_t *esmp) {
...
static void sfc_vdpa_mcdi_dma_free(void *cookie, efsys_mem_t *esmp) {
  struct sfc_vdpa_adapter *sva = cookie;
...
static void sfc_vdpa_mcdi_sched_restart(void *cookie) { RTE_SET_USED(cookie); }

static sfc_efx_mcdi_mgmt_evq_poll_cb sfc_vdpa_mcdi_mgmt_evq_poll;
static void sfc_vdpa_mcdi_mgmt_evq_poll(void *cookie) { RTE_SET_USED(cookie); }
...
int sfc_vdpa_mcdi_init(struct sfc_vdpa_adapter *sva) {
  ...
  logtype = sfc_vdpa_register_logtype(
      &(sva->pdev->addr), SFC_VDPA_LOGTYPE_MCDI_STR, RTE_LOG_NOTICE);

  return sfc_efx_mcdi_init(&sva->mcdi, logtype, sva->log_prefix, sva->nic,
