 ...
 */
#define SFC_VDPA_DEFAULT_FEATURES (1ULL << VHOST_USER_F_PROTOCOL_FEATURES)
...
static int sfc_vdpa_get_device_features(struct sfc_vdpa_ops_data *ops_data) {
  int rc;
  ...
  rc = efx_virtio_get_features(nic, EFX_VIRTIO_DEVICE_TYPE_NET, &dev_features);
  if (rc != 0) {
    sfc_vdpa_err(ops_data->dev_handle, "could not read device feature: %s",
                 rte_strerror(rc));
    return rc;
  }
...
static int sfc_vdpa_enable_vfio_intr(struct sfc_vdpa_ops_data *ops_data) {
  int rc;
  int *irq_fd_ptr;
  int vfio_dev_fd;
  ...
  if (rc) {
    sfc_vdpa_err(ops_data->dev_handle, "error enabling MSI-X interrupts: %s",
                 strerror(errno));
    return -1;
  }
...
static int sfc_vdpa_disable_vfio_intr(struct sfc_vdpa_ops_data *ops_data) {
  int rc;
  int vfio_dev_fd;
  struct vfio_irq_set irq_set;
  void *dev;
  ...
  rc = ioctl(vfio_dev_fd, VFIO_DEVICE_SET_IRQS, &irq_set);
  if (rc) {
    sfc_vdpa_err(ops_data->dev_handle, "error disabling MSI-X interrupts: %s",
                 strerror(errno));
    return -1;
  }
...
static int sfc_vdpa_get_vring_info(struct sfc_vdpa_ops_data *ops_data,
                                   int vq_num,
  ...
  if (rc < 0) {
    sfc_vdpa_err(ops_data->dev_handle, "get vhost vring failed: %s",
                 rte_strerror(rc));
    return rc;
  }
  ...
  if (gpa == 0) {
    sfc_vdpa_err(ops_data->dev_handle, "fail to get GPA for descriptor ring.");
    return -1;
  }
  vring->desc = gpa;
  ...
  if (gpa == 0) {
    sfc_vdpa_err(ops_data->dev_handle, "fail to get GPA for available ring.");
    return -1;
  }
  vring->avail = gpa;
  ...
  if (gpa == 0) {
    sfc_vdpa_err(ops_data->dev_handle, "fail to get GPA for used ring.");
    return -1;
  }
  vring->used = gpa;
  ...
  rc = rte_vhost_get_vring_base(ops_data->vid, vq_num, &vring->last_avail_idx,
                                &vring->last_used_idx);

  return rc;
}

static int sfc_vdpa_virtq_start(struct sfc_vdpa_ops_data *ops_data,
                                int vq_num) {
  int rc;
  efx_virtio_vq_t *vq;
  struct sfc_vdpa_vring_info vring;
  ...
  if (rc < 0) {
    sfc_vdpa_err(ops_data->dev_handle, "get vring info failed: %s",
                 rte_strerror(rc));
  ...
  if (vq_num % 2) {
    ...
    sfc_vdpa_info(ops_data->dev_handle, "configure virtqueue # %d (TXQ)",
                  vq_num);
  } else {
    vq_cfg.evvc_type = EFX_VIRTIO_VQ_TYPE_NET_RXQ;
    sfc_vdpa_info(ops_data->dev_handle, "configure virtqueue # %d (RXQ)",
                  vq_num);
  }
  ...
  vq_cfg.evcc_features = ops_data->dev_features & ops_data->req_features;

  /* Start virtqueue */
  rc = efx_virtio_qstart(vq, &vq_cfg, &vq_dyncfg);
  if (rc != 0) {
    /* destroy virtqueue */
    sfc_vdpa_err(ops_data->dev_handle, "virtqueue start failed: %s",
                 rte_strerror(rc));
...
static int sfc_vdpa_virtq_stop(struct sfc_vdpa_ops_data *ops_data, int vq_num) {
  int rc;
...
static int sfc_vdpa_configure(struct sfc_vdpa_ops_data *ops_data) {
  int rc, i;
  int nr_vring;
  int max_vring_cnt;
  efx_virtio_vq_t *vq;
  ...
  max_vring_cnt = (sfc_vdpa_adapter_by_dev_handle(dev)->max_queue_count * 2);

  /* number of vring should not be more than supported max vq count */
  if (nr_vring > max_vring_cnt) {
    sfc_vdpa_err(dev, "nr_vring (%d) is > max vring count (%d)", nr_vring,
                 max_vring_cnt);
    goto fail_vring_num;
  }
  ...
  if (rc) {
    sfc_vdpa_err(dev, "DMA map failed: %s", rte_strerror(rc));
    goto fail_dma_map;
  }
  ...
  for (i = 0; i < nr_vring; i++) {
    ...
    if ((rc != 0) || (vq == NULL)) {
      sfc_vdpa_err(dev, "virtqueue create failed: %s", rte_strerror(rc));
      goto fail_vq_create;
    }
...
static void sfc_vdpa_close(struct sfc_vdpa_ops_data *ops_data) {
  int i;
...
static void sfc_vdpa_stop(struct sfc_vdpa_ops_data *ops_data) {
  int i;
  int rc;
...
static int sfc_vdpa_start(struct sfc_vdpa_ops_data *ops_data) {
  int i, j;
  int rc;
  ...
  if (rc < 0) {
    sfc_vdpa_err(ops_data->dev_handle, "vfio intr allocation failed: %s",
                 rte_strerror(rc));
    goto fail_enable_vfio_intr;
  }

  rte_vhost_get_negotiated_features(ops_data->vid, &ops_data->req_features);

  sfc_vdpa_info(ops_data->dev_handle, "negotiated feature : 0x%" PRIx64,
                ops_data->req_features);

  for (i = 0; i < ops_data->vq_count; i++) {
    sfc_vdpa_log_init(ops_data->dev_handle, "starting vq# %d", i);
  ...
  sfc_vdpa_log_init(ops_data->dev_handle, "configure MAC filters");
  rc = sfc_vdpa_filter_config(ops_data);
  if (rc != 0) {
    sfc_vdpa_err(ops_data->dev_handle, "MAC filter config failed: %s",
                 rte_strerror(rc));
...
static int sfc_vdpa_get_queue_num(struct rte_vdpa_device *vdpa_dev,
                                  uint32_t *queue_num) {
...
static int sfc_vdpa_get_features(struct rte_vdpa_device *vdpa_dev,
                                 uint64_t *features) {
  struct sfc_vdpa_ops_data *ops_data;
  ...
  *features = ops_data->drv_features;
  ...
  sfc_vdpa_info(ops_data->dev_handle,
                "vDPA ops get_feature :: features : 0x%" PRIx64, *features);

  return 0;
}

static int sfc_vdpa_get_protocol_features(struct rte_vdpa_device *vdpa_dev,
                                          uint64_t *features) {
  struct sfc_vdpa_ops_data *ops_data;
...
static void *sfc_vdpa_notify_ctrl(void *arg) {
  struct sfc_vdpa_ops_data *ops_data;
  int vid;
...
static int sfc_vdpa_setup_notify_ctrl(struct sfc_vdpa_ops_data *ops_data) {
  int ret;
   ...
   */
  ret = pthread_create(&ops_data->notify_tid, NULL, sfc_vdpa_notify_ctrl,
                       ops_data);
  if (ret != 0) {
    sfc_vdpa_err(ops_data->dev_handle,
                 "failed to create notify_ctrl thread: %s", rte_strerror(ret));
    return -1;
  }
...
static int sfc_vdpa_dev_config(int vid) {
  ...
  ops_data = sfc_vdpa_get_data_by_dev(vdpa_dev);
  if (ops_data == NULL) {
    SFC_VDPA_GENERIC_LOG(ERR, "invalid vDPA device : %p, vid : %d", vdpa_dev,
                         vid);
    return -1;
  }
...
static int sfc_vdpa_dev_close(int vid) {
  int ret;
  struct rte_vdpa_device *vdpa_dev;
  struct sfc_vdpa_ops_data *ops_data;
...
static int sfc_vdpa_set_vring_state(int vid, int vring, int state) {
  struct sfc_vdpa_ops_data *ops_data;
  struct rte_vdpa_device *vdpa_dev;
  efx_rc_t rc;
  int vring_max;
  ...
  sfc_vdpa_info(dev, "vDPA ops set_vring_state: vid: %d, vring: %d, state:%d",
                vid, vring, state);

  vring_max = (sfc_vdpa_adapter_by_dev_handle(dev)->max_queue_count * 2);

  if (vring < 0 || vring > vring_max) {
    sfc_vdpa_err(dev, "received invalid vring id : %d to set state", vring);
    return -1;
  }
  ...
  if (state == 0) {
    ...
    if (rc != 0) {
      sfc_vdpa_err(dev, "virtqueue stop failed: %s", rte_strerror(rc));
    }
  } else {
    rc = sfc_vdpa_virtq_start(ops_data, vring);
    if (rc != 0) {
      sfc_vdpa_err(dev, "virtqueue start failed: %s", rte_strerror(rc));
    }
  }
...
static int sfc_vdpa_set_features(int vid) {
  RTE_SET_USED(vid);

  return -1;
}

static int sfc_vdpa_get_vfio_device_fd(int vid) {
  struct rte_vdpa_device *vdpa_dev;
  struct sfc_vdpa_ops_data *ops_data;
  int vfio_dev_fd;
  void *dev;
...
static int sfc_vdpa_get_notify_area(int vid, int qid, uint64_t *offset,
                                    uint64_t *size) {
  int ret;
  efx_nic_t *nic;
  int vfio_dev_fd;
  ...
  rc = efx_virtio_get_doorbell_offset(ops_data->vq_cxt[qid].vq, &bar_offset);
  if (rc != 0) {
    sfc_vdpa_err(dev, "failed to get doorbell offset: %s", rte_strerror(rc));
    return rc;
  }
  ...
  if (ret != 0) {
    sfc_vdpa_err(dev, "could not get device region info: %s", strerror(errno));
    return ret;
  }
...
struct sfc_vdpa_ops_data *sfc_vdpa_device_init(void *dev_handle,
                                               enum sfc_vdpa_context context) {
  struct sfc_vdpa_ops_data *ops_data;
  struct rte_pci_device *pci_dev;
  int rc;
   ...
   */
  ops_data->drv_features = ops_data->dev_features | SFC_VDPA_DEFAULT_FEATURES;
...
void sfc_vdpa_device_fini(struct sfc_vdpa_ops_data *ops_data) {
  rte_vdpa_unregister_device(ops_data->vdpa_dev);
