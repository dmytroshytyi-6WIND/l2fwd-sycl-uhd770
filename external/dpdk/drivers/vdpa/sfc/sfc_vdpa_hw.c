...
int sfc_vdpa_dma_alloc(struct sfc_vdpa_adapter *sva, const char *name,
  ...
  ret = snprintf(mz_name, RTE_MEMZONE_NAMESIZE, "%s_%s", sva->pdev->name, name);
  if (ret < 0 || ret >= RTE_MEMZONE_NAMESIZE) {
    sfc_vdpa_err(sva, "%s_%s too long to fit in mz_name", sva->pdev->name,
                 name);
    return -EINVAL;
  }
  ...
  mz = rte_memzone_reserve_aligned(mz_name, mcdi_buff_size, numa_node,
                                   RTE_MEMZONE_IOVA_CONTIG, PAGE_SIZE);
  if (mz == NULL) {
    sfc_vdpa_err(sva, "cannot reserve memory for %s: len=%#x: %s", mz_name,
                 (unsigned int)len, rte_strerror(rte_errno));
    return -ENOMEM;
  }
  ...
  for (;;) {
    ret = rte_vfio_container_dma_map(sva->vfio_container_fd, (uint64_t)mz->addr,
                                     mcdi_iova, mcdi_buff_size);
    if (ret == 0)
      break;

    mcdi_iova = mcdi_iova >> 1;
    if (mcdi_iova < mcdi_buff_size) {
      sfc_vdpa_err(sva, "DMA mapping failed for MCDI : %s",
  ...
  sfc_vdpa_info(sva, "DMA name=%s len=%zu => virt=%p iova=0x%" PRIx64, name,
                len, esmp->esm_base, esmp->esm_addr);

  return 0;
}

void sfc_vdpa_dma_free(struct sfc_vdpa_adapter *sva, efsys_mem_t *esmp) {
  int ret;

  sfc_vdpa_log_init(sva, "name=%s", esmp->esm_mz->name);

  ret = rte_vfio_container_dma_unmap(sva->vfio_container_fd,
                                     (uint64_t)esmp->esm_base, esmp->esm_addr,
                                     sva->mcdi_buff_size);
  if (ret < 0)
    sfc_vdpa_err(sva, "DMA unmap failed for MCDI : %s",
                 rte_strerror(rte_errno));

  sfc_vdpa_info(sva, "DMA free name=%s => virt=%p iova=0x%" PRIx64,
...
int sfc_vdpa_dma_map(struct sfc_vdpa_ops_data *ops_data, bool do_map) {
  uint32_t i, j;
  int rc;
  ...
  for (i = 0; i < vhost_mem->nregions; i++) {
    ...
    if (do_map) {
      rc =
          rte_vfio_container_dma_map(vfio_container_fd, mem_reg->host_user_addr,
                                     mem_reg->guest_phys_addr, mem_reg->size);
      if (rc < 0) {
        sfc_vdpa_err(dev, "DMA map failed : %s", rte_strerror(rte_errno));
        goto failed_vfio_dma_map;
      }
    } else {
      rc = rte_vfio_container_dma_unmap(
          vfio_container_fd, mem_reg->host_user_addr, mem_reg->guest_phys_addr,
          mem_reg->size);
      if (rc < 0) {
        sfc_vdpa_err(dev, "DMA unmap failed : %s", rte_strerror(rte_errno));
        goto error;
      }
    }
  }
...
failed_vfio_dma_map:
  for (j = 0; j < i; j++) {
    ...
    rte_vfio_container_dma_unmap(vfio_container_fd, mem_reg->host_user_addr,
                                 mem_reg->guest_phys_addr, mem_reg->size);
  }
...
static int sfc_vdpa_mem_bar_init(struct sfc_vdpa_adapter *sva,
...
static void sfc_vdpa_mem_bar_fini(struct sfc_vdpa_adapter *sva) {
  efsys_bar_t *ebp = &sva->mem_bar;
...
static int sfc_vdpa_nic_probe(struct sfc_vdpa_adapter *sva) {
...
static int sfc_vdpa_estimate_resource_limits(struct sfc_vdpa_adapter *sva) {
...
int sfc_vdpa_hw_init(struct sfc_vdpa_adapter *sva) {
  ...
  rc = efx_nic_create(sva->family, (efsys_identifier_t *)sva, &sva->mem_bar,
                      mem_ebr.ebr_offset, &sva->nic_lock, &enp);
  if (rc != 0) {
...
void sfc_vdpa_hw_fini(struct sfc_vdpa_adapter *sva) {
  efx_nic_t *enp = sva->nic;
