...
int sfc_vdpa_mcdi_init(struct sfc_vdpa_adapter *sva);
void sfc_vdpa_mcdi_fini(struct sfc_vdpa_adapter *sva);
...
int sfc_vdpa_dma_map(struct sfc_vdpa_ops_data *vdpa_data, bool do_map);
 ...
 */
static inline void sfc_vdpa_adapter_lock_init(struct sfc_vdpa_adapter *sva) {
  rte_spinlock_init(&sva->lock);
}

static inline int sfc_vdpa_adapter_is_locked(struct sfc_vdpa_adapter *sva) {
  return rte_spinlock_is_locked(&sva->lock);
}

static inline void sfc_vdpa_adapter_lock(struct sfc_vdpa_adapter *sva) {
  rte_spinlock_lock(&sva->lock);
}

static inline int sfc_vdpa_adapter_trylock(struct sfc_vdpa_adapter *sva) {
  return rte_spinlock_trylock(&sva->lock);
}

static inline void sfc_vdpa_adapter_unlock(struct sfc_vdpa_adapter *sva) {
