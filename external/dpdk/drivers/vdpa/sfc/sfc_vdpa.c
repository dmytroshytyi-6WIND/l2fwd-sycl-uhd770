...
static int sfc_vdpa_vfio_setup(struct sfc_vdpa_adapter *sva) {
  ...
  rc = rte_vfio_get_group_num(rte_pci_get_sysfs_path(), dev_name,
                              &sva->iommu_group_num);
  if (rc <= 0) {
    sfc_vdpa_err(sva, "failed to get IOMMU group for %s : %s", dev_name,
                 rte_strerror(-rc));
    goto fail_get_group_num;
  }

  sva->vfio_group_fd = rte_vfio_container_group_bind(sva->vfio_container_fd,
                                                     sva->iommu_group_num);
  if (sva->vfio_group_fd < 0) {
    sfc_vdpa_err(sva, "failed to bind IOMMU group %d to container %d",
                 sva->iommu_group_num, sva->vfio_container_fd);
    goto fail_group_bind;
  }

  if (rte_pci_map_device(dev) != 0) {
    sfc_vdpa_err(sva, "failed to map PCI device %s : %s", dev_name,
...
fail_pci_map_device:
  if (rte_vfio_container_group_unbind(sva->vfio_container_fd,
    ...
    sfc_vdpa_err(sva, "failed to unbind IOMMU group %d from container %d",
                 sva->iommu_group_num, sva->vfio_container_fd);
  }
...
fail_get_group_num:
  if (rte_vfio_container_destroy(sva->vfio_container_fd) != 0) {
    sfc_vdpa_err(sva, "failed to destroy container %d", sva->vfio_container_fd);
  }
...
static void sfc_vdpa_vfio_teardown(struct sfc_vdpa_adapter *sva) {
...
static int sfc_vdpa_set_log_prefix(struct sfc_vdpa_adapter *sva) {
  struct rte_pci_device *pci_dev = sva->pdev;
  int ret;

  ret =
      snprintf(sva->log_prefix, sizeof(sva->log_prefix),
               "PMD: sfc_vdpa " PCI_PRI_FMT " : ", pci_dev->addr.domain,
...
uint32_t sfc_vdpa_register_logtype(const struct rte_pci_addr *pci_addr,
...
static int sfc_vdpa_kvargs_parse(struct sfc_vdpa_adapter *sva) {
  struct rte_pci_device *pci_dev = sva->pdev;
...
static int sfc_vdpa_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  if (sfc_efx_dev_class_get(pci_dev->device.devargs) !=
    ...
    SFC_VDPA_GENERIC_LOG(INFO, "Incompatible device class: skip probing, "
                               "should be probed by other sfc driver.");
    return 1;
  }
  ...
  logtype_main = sfc_vdpa_register_logtype(
      &pci_dev->addr, SFC_VDPA_LOGTYPE_MAIN_STR, RTE_LOG_NOTICE);
...
static int sfc_vdpa_pci_remove(struct rte_pci_device *pci_dev) {
  ...
  if (sva == NULL) {
    SFC_VDPA_GENERIC_LOG(INFO, "Invalid device: %s.", pci_dev->name);
    return -1;
  }
