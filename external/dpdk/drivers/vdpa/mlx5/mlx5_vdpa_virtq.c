...
static void mlx5_vdpa_virtq_handler(void *cb_arg) {
  ...
  do {
    ...
    if (nbytes < 0) {
      if (errno == EINTR || errno == EWOULDBLOCK || errno == EAGAIN)
        continue;
      DRV_LOG(ERR, "Failed to read kickfd of virtq %d: %s", virtq->index,
              strerror(errno));
    }
    break;
  } while (1);
  ...
  if (virtq->notifier_state == MLX5_VDPA_NOTIFIER_STATE_DISABLED) {
    if (rte_vhost_host_notifier_ctrl(priv->vid, virtq->index, true))
      ...
      virtq->notifier_state = MLX5_VDPA_NOTIFIER_STATE_ENABLED;
...
static int mlx5_vdpa_virtq_unset(struct mlx5_vdpa_virtq *virtq) {
  unsigned int i;
    ...
    while (retries-- && ret == -EAGAIN) {
      ...
      if (ret == -EAGAIN) {
        DRV_LOG(DEBUG,
                ...
                rte_intr_fd_get(virtq->intr_handle), (int)virtq->index,
                retries);
  ...
  if (virtq->virtq) {
    ...
    if (ret)
      DRV_LOG(WARNING, "Failed to stop virtq %d.", virtq->index);
    claim_zero(mlx5_devx_cmd_destroy(virtq->virtq));
  }
  virtq->virtq = NULL;
  for (i = 0; i < RTE_DIM(virtq->umems); ++i) {
    if (virtq->umems[i].obj)
      claim_zero(mlx5_glue->devx_umem_dereg(virtq->umems[i].obj));
...
int mlx5_vdpa_virtq_modify(struct mlx5_vdpa_virtq *virtq, int state) {
  struct mlx5_devx_virtq_attr attr = {
      ...
      .state = state ? MLX5_VIRTQ_STATE_RDY : MLX5_VIRTQ_STATE_SUSPEND,
...
int mlx5_vdpa_virtq_query(struct mlx5_vdpa_priv *priv, int index) {
  ...
  DRV_LOG(INFO,
          ...
          "hw_used_index=%d",
          priv->vid, index, attr.hw_available_index, attr.hw_used_index);
  ret = rte_vhost_set_vring_base(priv->vid, index, attr.hw_available_index,
                                 attr.hw_used_index);
  if (ret) {
  ...
  if (attr.state == MLX5_VIRTQ_STATE_ERROR)
    DRV_LOG(WARNING, "vid %d vring %d hw error=%hhu", priv->vid, index,
            attr.error_type);
  return 0;
}

static uint64_t mlx5_vdpa_hva_to_gpa(struct rte_vhost_memory *mem,
                                     uint64_t hva) {
...
static int mlx5_vdpa_virtq_setup(struct mlx5_vdpa_priv *priv, int index) {
  struct mlx5_vdpa_virtq *virtq = &priv->virtqs[index];
  ...
  attr.virtio_version_1_0 = !!(priv->features & (1ULL << VIRTIO_F_VERSION_1));
   ...
   */
  attr.event_mode = vq.callfd != -1 || !(priv->caps.event_mode &
  ...
  if (attr.event_mode == MLX5_VIRTQ_EVENT_MODE_QP) {
    ret = mlx5_vdpa_event_qp_create(priv, vq.size, vq.callfd, &virtq->eqp);
    if (ret) {
      DRV_LOG(ERR, "Failed to create event QPs for virtq %d.", index);
      return -1;
    }
    ...
    DRV_LOG(INFO,
            ...
            " need event QPs and event mechanism.",
            index);
  }
  if (priv->caps.queue_counters_valid) {
    if (!virtq->counters)
      virtq->counters = mlx5_devx_cmd_create_virtio_q_counters(priv->cdev->ctx);
    if (!virtq->counters) {
      DRV_LOG(ERR,
              "Failed to create virtq couners for virtq"
              " %d.",
              index);
      goto error;
    }
  ...
  for (i = 0; i < RTE_DIM(virtq->umems); ++i) {
    ...
    virtq->umems[i].buf = rte_zmalloc(__func__, virtq->umems[i].size, 4096);
    ...
    virtq->umems[i].obj =
        mlx5_glue->devx_umem_reg(priv->cdev->ctx, virtq->umems[i].buf,
                                 virtq->umems[i].size, IBV_ACCESS_LOCAL_WRITE);
    if (!virtq->umems[i].obj) {
      DRV_LOG(ERR, "Failed to register umem %d for virtq %u.", i, index);
      goto error;
    }
  ...
  if (ret) {
    ...
    DRV_LOG(INFO,
            ...
            "virtq %d.",
            priv->vid, last_avail_idx, last_used_idx, index);
  }
  ...
  virtq->intr_handle = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_SHARED);
  ...
  if (rte_intr_fd_get(virtq->intr_handle) == -1) {
    ...
    if (rte_intr_callback_register(virtq->intr_handle, mlx5_vdpa_virtq_handler,
                                   virtq)) {
      rte_intr_fd_set(virtq->intr_handle, -1);
  ...
  ret = mlx5_glue->devx_subscribe_devx_event(
      priv->err_chnl, virtq->virtq->obj, sizeof(event_num), &event_num, cookie);
  if (ret) {
  ...
  DRV_LOG(DEBUG, "vid %u virtq %u was created successfully.", priv->vid, index);
  return 0;
error:
  mlx5_vdpa_virtq_unset(virtq);
  return -1;
...
static int mlx5_vdpa_features_validate(struct mlx5_vdpa_priv *priv) {
  if (priv->features & (1ULL << VIRTIO_F_RING_PACKED)) {
    if (!(priv->caps.virtio_queue_type & (1 << MLX5_VIRTQ_TYPE_PACKED))) {
      DRV_LOG(ERR,
              "Failed to configure PACKED mode for vdev "
...
int mlx5_vdpa_virtqs_prepare(struct mlx5_vdpa_priv *priv) {
  ...
  if (nr_vring > priv->caps.max_num_virtio_queues * 2) {
    DRV_LOG(ERR, "Do not support more than %d virtqs(%d).",
            (int)priv->caps.max_num_virtio_queues * 2, (int)nr_vring);
    return -1;
  }
  /* Always map the entire page. */
  priv->virtq_db_addr = mmap(NULL, priv->var->length, PROT_READ | PROT_WRITE,
                             MAP_SHARED, ctx->cmd_fd, priv->var->mmap_off);
...
static int mlx5_vdpa_virtq_is_modified(struct mlx5_vdpa_priv *priv,
                                       struct mlx5_vdpa_virtq *virtq) {
  struct rte_vhost_vring vq;
  ...
  if (vq.size != virtq->vq_size ||
      vq.kickfd != rte_intr_fd_get(virtq->intr_handle))
    return 1;
...
int mlx5_vdpa_virtq_stats_get(struct mlx5_vdpa_priv *priv, int qid,
  ...
  if (!virtq->counters) {
    DRV_LOG(ERR,
            "Failed to read virtq %d statistics - virtq "
            "is invalid.",
            qid);
    return -EINVAL;
  }
...
int mlx5_vdpa_virtq_stats_reset(struct mlx5_vdpa_priv *priv, int qid) {
  struct mlx5_vdpa_virtq *virtq = &priv->virtqs[qid];
  int ret;
  ...
  if (ret)
    DRV_LOG(ERR, "Failed to read virtq %d reset stats from HW.", qid);
  return ret;
}
