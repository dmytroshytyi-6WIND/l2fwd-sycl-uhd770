...
int mlx5_vdpa_logging_enable(struct mlx5_vdpa_priv *priv, int enable) {
  ...
  for (i = 0; i < priv->nr_virtqs; ++i) {
    ...
    if (!priv->virtqs[i].virtq) {
      DRV_LOG(DEBUG,
              "virtq %d is invalid for dirty bitmap "
              "enabling.",
              i);
    } else if (mlx5_devx_cmd_modify_virtq(priv->virtqs[i].virtq, &attr)) {
      DRV_LOG(ERR,
              "Failed to modify virtq %d for dirty "
              "bitmap enabling.",
              i);
      return -1;
    }
...
int mlx5_vdpa_dirty_bitmap_set(struct mlx5_vdpa_priv *priv, uint64_t log_base,
  ...
  int i;
  int ret = mlx5_os_wrapped_mkey_create(
      priv->cdev->ctx, priv->cdev->pd, priv->cdev->pdn,
      (void *)(uintptr_t)log_base, log_size, &priv->lm_mr);
