...
static int mlx5_vdpa_event_qp_global_prepare(struct mlx5_vdpa_priv *priv) {
  if (priv->eventc)
    return 0;
  priv->eventc = mlx5_os_devx_create_event_channel(
      priv->cdev->ctx, MLX5DV_DEVX_CREATE_EVENT_CHANNEL_FLAGS_OMIT_EV_DATA);
  if (!priv->eventc) {
    rte_errno = errno;
...
static void mlx5_vdpa_cq_destroy(struct mlx5_vdpa_cq *cq) {
  mlx5_devx_cq_destroy(&cq->cq_obj);
  memset(cq, 0, sizeof(*cq));
}

static inline void __rte_unused mlx5_vdpa_cq_arm(struct mlx5_vdpa_priv *priv,
...
static int mlx5_vdpa_cq_create(struct mlx5_vdpa_priv *priv, uint16_t log_desc_n,
  ...
  ret = mlx5_devx_cq_create(priv->cdev->ctx, &cq->cq_obj, log_desc_n, &attr,
                            SOCKET_ID_ANY);
  if (ret)
    goto error;
  cq->cq_ci = 0;
  ...
  ret = mlx5_os_devx_subscribe_devx_event(priv->eventc, cq->cq_obj.cq->obj,
...
static inline uint32_t mlx5_vdpa_cq_poll(struct mlx5_vdpa_cq *cq) {
  ...
  if (comp) {
    cq->cq_ci += comp;
    MLX5_ASSERT(MLX5_CQE_OPCODE(last_word.op_own) != MLX5_CQE_INVALID);
    if (unlikely(!(MLX5_CQE_OPCODE(last_word.op_own) == MLX5_CQE_RESP_ERR ||
                   MLX5_CQE_OPCODE(last_word.op_own) == MLX5_CQE_REQ_ERR)))
      cq->errors++;
    rte_io_wmb();
...
static void mlx5_vdpa_arm_all_cqs(struct mlx5_vdpa_priv *priv) {
  struct mlx5_vdpa_cq *cq;
  int i;
...
static void mlx5_vdpa_timer_sleep(struct mlx5_vdpa_priv *priv, uint32_t max) {
...
static uint32_t mlx5_vdpa_queue_complete(struct mlx5_vdpa_cq *cq) {
  uint32_t comp = 0;
...
static uint32_t mlx5_vdpa_queues_complete(struct mlx5_vdpa_priv *priv) {
  int i;
  uint32_t max = 0;
...
mlx5_vdpa_event_wait(struct mlx5_vdpa_priv *priv __rte_unused) {
  ...
  DRV_LOG(INFO, "Got error in devx_get_event, ret = %d, errno = %d.", ret,
          errno);
#endif
  return NULL;
}

static void *mlx5_vdpa_event_handle(void *arg) {
  ...
  case MLX5_VDPA_EVENT_MODE_FIXED_TIMER:
    ...
    while (1) {
      ...
      if (max == 0 && priv->no_traffic_counter++ >= priv->no_traffic_max) {
        ...
        do {
          ...
          pthread_mutex_lock(&priv->vq_config_lock);
          if (cq == NULL || mlx5_vdpa_queue_complete(cq) > 0)
            break;
        } while (1);
        priv->timer_delay_us = priv->event_us;
...
static void mlx5_vdpa_err_interrupt_handler(void *cb_arg __rte_unused) {
#ifdef HAVE_IBV_DEVX_EVENT
  ...
  while (mlx5_glue->devx_get_event(priv->err_chnl, &out.event_resp,
    ...
    if (sec > MLX5_VDPA_ERROR_TIME_SEC) {
      ...
      if (mlx5_vdpa_virtq_enable(priv, vq_index, 1))
        DRV_LOG(ERR, "Failed to enable virtq %d.", vq_index);
      else
        DRV_LOG(WARNING, "Recover virtq %d: %u.", vq_index, ++virtq->n_retry);
    } else {
...
int mlx5_vdpa_err_event_setup(struct mlx5_vdpa_priv *priv) {
  ...
  if (!priv->err_chnl) {
    rte_errno = errno;
    DRV_LOG(ERR, "Failed to create device event channel %d.", rte_errno);
    goto error;
  }
  ...
  if (rte_intr_callback_register(priv->err_intr_handle,
                                 mlx5_vdpa_err_interrupt_handler, priv)) {
    rte_intr_fd_set(priv->err_intr_handle, 0);
    DRV_LOG(ERR, "Failed to register error interrupt for device %d.",
            priv->vid);
    goto error;
  } else {
    DRV_LOG(DEBUG, "Registered error interrupt for device%d.", priv->vid);
  }
  return 0;
error:
  ...
  while (retries-- && ret == -EAGAIN) {
    ...
    if (ret == -EAGAIN) {
      DRV_LOG(DEBUG,
              ...
              rte_intr_fd_get(priv->err_intr_handle), retries);
      rte_pause();
    }
  }
  if (priv->err_chnl) {
...
static int mlx5_vdpa_qps2rts(struct mlx5_vdpa_event_qp *eqp) {
  if (mlx5_devx_cmd_modify_qp_state(eqp->fw_qp, MLX5_CMD_OP_RST2INIT_QP,
                                    eqp->sw_qp.qp->id)) {
    DRV_LOG(ERR, "Failed to modify FW QP to INIT state(%u).", rte_errno);
    return -1;
  }
  if (mlx5_devx_cmd_modify_qp_state(eqp->sw_qp.qp, MLX5_CMD_OP_RST2INIT_QP,
                                    eqp->fw_qp->id)) {
    DRV_LOG(ERR, "Failed to modify SW QP to INIT state(%u).", rte_errno);
    return -1;
  }
  if (mlx5_devx_cmd_modify_qp_state(eqp->fw_qp, MLX5_CMD_OP_INIT2RTR_QP,
                                    eqp->sw_qp.qp->id)) {
    DRV_LOG(ERR, "Failed to modify FW QP to RTR state(%u).", rte_errno);
    return -1;
  }
  if (mlx5_devx_cmd_modify_qp_state(eqp->sw_qp.qp, MLX5_CMD_OP_INIT2RTR_QP,
                                    eqp->fw_qp->id)) {
    DRV_LOG(ERR, "Failed to modify SW QP to RTR state(%u).", rte_errno);
    return -1;
  }
  if (mlx5_devx_cmd_modify_qp_state(eqp->fw_qp, MLX5_CMD_OP_RTR2RTS_QP,
                                    eqp->sw_qp.qp->id)) {
    DRV_LOG(ERR, "Failed to modify FW QP to RTS state(%u).", rte_errno);
    return -1;
  }
  if (mlx5_devx_cmd_modify_qp_state(eqp->sw_qp.qp, MLX5_CMD_OP_RTR2RTS_QP,
                                    eqp->fw_qp->id)) {
    DRV_LOG(ERR, "Failed to modify SW QP to RTS state(%u).", rte_errno);
    return -1;
  }
  return 0;
}
...
int mlx5_vdpa_event_qp_create(struct mlx5_vdpa_priv *priv, uint16_t desc_n,
  ...
  ret = mlx5_devx_qp_create(priv->cdev->ctx, &(eqp->sw_qp),
                            attr.num_of_receive_wqes * MLX5_WSEG_SIZE, &attr,
                            SOCKET_ID_ANY);
  if (ret) {
