...
static int mlx5_vdpa_regions_addr_cmp(const void *a, const void *b) {
...
#define KLM_NUM_MAX_ALIGN(sz)                                                  \
  (RTE_ALIGN_CEIL(sz, MLX5_MAX_KLM_BYTE_COUNT) / MLX5_MAX_KLM_BYTE_COUNT)
...
mlx5_vdpa_vhost_mem_regions_prepare(int vid, uint8_t *mode, uint64_t *mem_size,
  ...
  *gcd = 0;
  for (i = 0; i < mem->nregions; ++i) {
    DRV_LOG(INFO,
            "Region %u: HVA 0x%" PRIx64 ", GPA 0x%" PRIx64 ", size 0x%" PRIx64
            ".",
            i, mem->regions[i].host_user_addr, mem->regions[i].guest_phys_addr,
            mem->regions[i].size);
    if (i > 0) {
      /* Hole handle. */
      size = mem->regions[i].guest_phys_addr -
             (mem->regions[i - 1].guest_phys_addr + mem->regions[i - 1].size);
  ...
  if (!RTE_IS_POWER_OF_2(*gcd)) {
    ...
    while (candidate_gcd > 1 && (*gcd % candidate_gcd))
      candidate_gcd /= 2;
    DRV_LOG(DEBUG,
            "GCD 0x%" PRIx64 " is not power of 2. Adjusted "
            "GCD is 0x%" PRIx64 ".",
            *gcd, candidate_gcd);
    *gcd = candidate_gcd;
  }
  klm_fbs_entries_num = *mem_size / *gcd;
  if (*gcd < MLX5_MIN_KLM_FIXED_BUFFER_SIZE ||
      klm_fbs_entries_num > MLX5_DEVX_MAX_KLM_ENTRIES ||
      ((klm_entries_num * sizeof(struct mlx5_klm)) <= RTE_CACHE_LINE_SIZE &&
       (klm_fbs_entries_num * sizeof(struct mlx5_klm)) > RTE_CACHE_LINE_SIZE)) {
  ...
  DRV_LOG(DEBUG,
          ...
          ", klm_fbs_entries_num = 0x%" PRIx64 ", klm_entries_num = 0x%" PRIx64
          ".",
          mem->nregions, *mem_size, *gcd, klm_fbs_entries_num, klm_entries_num);
...
#define KLM_SIZE_MAX_ALIGN(sz)                                                 \
  ((sz) > MLX5_MAX_KLM_BYTE_COUNT ? MLX5_MAX_KLM_BYTE_COUNT : (sz))
...
int mlx5_vdpa_mem_register(struct mlx5_vdpa_priv *priv) {
  ...
  for (i = 0; i < mem->nregions; i++) {
    ...
    entry->mr = mlx5_glue->reg_mr_iova(
        priv->cdev->pd, (void *)(uintptr_t)(reg->host_user_addr), reg->size,
        reg->guest_phys_addr, IBV_ACCESS_LOCAL_WRITE);
    if (!entry->mr) {
    ...
    if (i > 0) {
      uint64_t sadd;
      uint64_t empty_region_sz =
          reg->guest_phys_addr -
          (mem->regions[i - 1].guest_phys_addr + mem->regions[i - 1].size);

      if (empty_region_sz > 0) {
        sadd = mem->regions[i - 1].guest_phys_addr + mem->regions[i - 1].size;
        klm_size = mode == MLX5_MKC_ACCESS_MODE_KLM
                       ? KLM_SIZE_MAX_ALIGN(empty_region_sz)
                       : gcd;
        for (k = 0; k < empty_region_sz; k += klm_size) {
          klm_array[klm_index].byte_count =
              k + klm_size > empty_region_sz ? empty_region_sz - k : klm_size;
          klm_array[klm_index].mkey = priv->null_mr->lkey;
    ...
    klm_size =
        mode == MLX5_MKC_ACCESS_MODE_KLM ? KLM_SIZE_MAX_ALIGN(reg->size) : gcd;
    for (k = 0; k < reg->size; k += klm_size) {
      klm_array[klm_index].byte_count =
          k + klm_size > reg->size ? reg->size - k : klm_size;
  ...
  mkey_attr.log_entity_size =
      mode == MLX5_MKC_ACCESS_MODE_KLM_FBS ? rte_log2_u64(gcd) : 0;
