...
#define MLX5_VDPA_DEFAULT_FEATURES                                             \
  ((1ULL << VHOST_USER_F_PROTOCOL_FEATURES) | (1ULL << VIRTIO_F_ANY_LAYOUT) |  \
   (1ULL << VIRTIO_NET_F_MQ) | (1ULL << VIRTIO_NET_F_GUEST_ANNOUNCE) |         \
   (1ULL << VIRTIO_F_ORDER_PLATFORM) | (1ULL << VHOST_F_LOG_ALL) |             \
...
TAILQ_HEAD(mlx5_vdpa_privs, mlx5_vdpa_priv)
priv_list = TAILQ_HEAD_INITIALIZER(priv_list);
...
static int mlx5_vdpa_get_queue_num(struct rte_vdpa_device *vdev,
                                   uint32_t *queue_num) {
  struct mlx5_vdpa_priv *priv = mlx5_vdpa_find_priv_resource_by_vdev(vdev);
...
static int mlx5_vdpa_get_vdpa_features(struct rte_vdpa_device *vdev,
                                       uint64_t *features) {
  struct mlx5_vdpa_priv *priv = mlx5_vdpa_find_priv_resource_by_vdev(vdev);
...
static int mlx5_vdpa_get_protocol_features(struct rte_vdpa_device *vdev,
                                           uint64_t *features) {
  struct mlx5_vdpa_priv *priv = mlx5_vdpa_find_priv_resource_by_vdev(vdev);
...
static int mlx5_vdpa_set_vring_state(int vid, int vring, int state) {
...
static int mlx5_vdpa_features_set(int vid) {
  struct rte_vdpa_device *vdev = rte_vhost_get_vdpa_device(vid);
  struct mlx5_vdpa_priv *priv = mlx5_vdpa_find_priv_resource_by_vdev(vdev);
...
static int mlx5_vdpa_mtu_set(struct mlx5_vdpa_priv *priv) {
  ...
  ret = mlx5_get_ifname_sysfs(mlx5_os_get_ctx_device_name(priv->cdev->ctx),
                              request.ifr_name);
  if (ret) {
...
static int mlx5_vdpa_dev_config(int vid) {
  struct rte_vdpa_device *vdev = rte_vhost_get_vdpa_device(vid);
  struct mlx5_vdpa_priv *priv = mlx5_vdpa_find_priv_resource_by_vdev(vdev);
  ...
  if (mlx5_vdpa_mtu_set(priv))
    DRV_LOG(WARNING, "MTU cannot be set on device %s.", vdev->device->name);
...
static int mlx5_vdpa_get_device_fd(int vid) {
  struct rte_vdpa_device *vdev = rte_vhost_get_vdpa_device(vid);
  struct mlx5_vdpa_priv *priv = mlx5_vdpa_find_priv_resource_by_vdev(vdev);
...
static int mlx5_vdpa_get_notify_area(int vid, int qid, uint64_t *offset,
                                     uint64_t *size) {
  struct rte_vdpa_device *vdev = rte_vhost_get_vdpa_device(vid);
  struct mlx5_vdpa_priv *priv = mlx5_vdpa_find_priv_resource_by_vdev(vdev);
  ...
  if (!priv->var) {
    DRV_LOG(ERR,
            "VAR was not created for device %s, is the device"
            " configured?.",
            vdev->device->name);
    return -EINVAL;
  }
...
static int mlx5_vdpa_get_stats_names(struct rte_vdpa_device *vdev,
                                     struct rte_vdpa_stat_name *stats_names,
                                     unsigned int size) {
  static const char *mlx5_vdpa_stats_names[MLX5_VDPA_STATS_MAX] = {
      "received_descriptors", "completed_descriptors", "bad descriptor errors",
      "exceed max chain",     "invalid buffer",        "completion errors",
  };
  struct mlx5_vdpa_priv *priv = mlx5_vdpa_find_priv_resource_by_vdev(vdev);
  unsigned int i;
...
static int mlx5_vdpa_get_stats(struct rte_vdpa_device *vdev, int qid,
  ...
  if (!priv->configured) {
    DRV_LOG(ERR, "Device %s was not configured.", vdev->device->name);
    return -ENODATA;
  }
...
static int mlx5_vdpa_reset_stats(struct rte_vdpa_device *vdev, int qid) {
  struct mlx5_vdpa_priv *priv = mlx5_vdpa_find_priv_resource_by_vdev(vdev);
...
static int mlx5_vdpa_args_check_handler(const char *key, const char *val,
                                        void *opaque) {
...
static void mlx5_vdpa_config_get(struct mlx5_kvargs_ctrl *mkvlist,
                                 struct mlx5_vdpa_priv *priv) {
  const char **params = (const char *[]){
      "event_core",        "event_mode",
      "event_us",          "hw_latency_mode",
      "hw_max_latency_us", "hw_max_pending_comp",
      "no_traffic_time",   NULL,
  };
  ...
  if (!priv->event_us && priv->event_mode == MLX5_VDPA_EVENT_MODE_DYNAMIC_TIMER)
...
static int mlx5_vdpa_dev_probe(struct mlx5_common_device *cdev,
  ...
  priv = rte_zmalloc("mlx5 vDPA device private",
                     sizeof(*priv) + sizeof(struct mlx5_vdpa_virtq) *
...
static int mlx5_vdpa_dev_remove(struct mlx5_common_device *cdev) {
  struct mlx5_vdpa_priv *priv = NULL;
  int found = 0;
...
RTE_PMD_REGISTER_KMOD_DEP(MLX5_VDPA_DRIVER_NAME,
