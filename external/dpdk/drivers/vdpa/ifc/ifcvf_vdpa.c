...
#define DRV_LOG(level, fmt, args...)                                           \
  rte_log(RTE_LOG_##level, ifcvf_vdpa_logtype, "IFCVF %s(): " fmt "\n",        \
          __func__, ##args)
...
static const char *const ifcvf_valid_arguments[] = {IFCVF_VDPA_MODE,
                                                    IFCVF_SW_FALLBACK_LM, NULL};
...
find_internal_resource_by_dev(struct rte_pci_device *pdev) {
  ...
  TAILQ_FOREACH(list, &internal_list, next) {
    if (!rte_pci_addr_cmp(&pdev->addr, &list->internal->pdev->addr)) {
      found = 1;
      break;
    }
  }
...
static int ifcvf_vfio_setup(struct ifcvf_internal *internal) {
  ...
  for (i = 0; i < RTE_MIN(PCI_MAX_RESOURCE, IFCVF_PCI_MAX_RESOURCE); i++) {
...
static int ifcvf_dma_map(struct ifcvf_internal *internal, bool do_map) {
  uint32_t i;
  int ret;
  ...
  for (i = 0; i < mem->nregions; i++) {
    ...
    reg = &mem->regions[i];
    DRV_LOG(INFO,
            "%s, region %u: HVA 0x%" PRIx64 ", "
            "GPA 0x%" PRIx64 ", size 0x%" PRIx64 ".",
            do_map ? "DMA map" : "DMA unmap", i, reg->host_user_addr,
            reg->guest_phys_addr, reg->size);

    if (do_map) {
      ret = rte_vfio_container_dma_map(vfio_container_fd, reg->host_user_addr,
                                       reg->guest_phys_addr, reg->size);
      if (ret < 0) {
      ...
      ret = rte_vfio_container_dma_unmap(vfio_container_fd, reg->host_user_addr,
                                         reg->guest_phys_addr, reg->size);
      if (ret < 0) {
...
static uint64_t hva_to_gpa(int vid, uint64_t hva) {
  ...
  for (i = 0; i < mem->nregions; i++) {
    ...
    if (hva >= reg->host_user_addr && hva < reg->host_user_addr + reg->size) {
...
static int vdpa_ifcvf_start(struct ifcvf_internal *internal) {
...
static void vdpa_ifcvf_stop(struct ifcvf_internal *internal) {
  struct ifcvf_hw *hw = &internal->hw;
  uint32_t i;
  int vid;
  uint64_t features = 0;
  ...
  if (RTE_VHOST_NEED_LOG(features)) {
    ...
    rte_vfio_container_dma_unmap(internal->vfio_container_fd, log_base,
...
#define MSIX_IRQ_SET_BUF_LEN                                                   \
  (sizeof(struct vfio_irq_set) + sizeof(int) * (IFCVF_MAX_QUEUES * 2 + 1))
static int vdpa_enable_vfio_intr(struct ifcvf_internal *internal, bool m_rx) {
  int ret;
  uint32_t i, nr_vring;
  ...
  for (i = 0; i < nr_vring; i++) {
    ...
    if ((i & 1) == 0 && m_rx == true) {
      ...
      if (fd < 0) {
        DRV_LOG(ERR, "can't setup eventfd: %s", strerror(errno));
        return -1;
      }
  ...
  if (ret) {
    DRV_LOG(ERR, "Error enabling MSI-X interrupts: %s", strerror(errno));
    return -1;
  }
...
static int vdpa_disable_vfio_intr(struct ifcvf_internal *internal) {
  int ret;
  uint32_t i, nr_vring;
  char irq_set_buf[MSIX_IRQ_SET_BUF_LEN];
  struct vfio_irq_set *irq_set;
  ...
  ret = ioctl(internal->vfio_dev_fd, VFIO_DEVICE_SET_IRQS, irq_set);
  if (ret) {
    DRV_LOG(ERR, "Error disabling MSI-X interrupts: %s", strerror(errno));
    return -1;
  }
...
static int setup_notify_relay(struct ifcvf_internal *internal) {
...
static int unset_notify_relay(struct ifcvf_internal *internal) {
  void *status;
...
static int update_datapath(struct ifcvf_internal *internal) {
  int ret;
...
static int m_ifcvf_start(struct ifcvf_internal *internal) {
  struct ifcvf_hw *hw = &internal->hw;
  uint32_t i, nr_vring;
  int vid, ret;
  struct rte_vhost_vring vq;
  ...
  for (i = 0; i < nr_vring; i++) {
    ...
    vring_init(&internal->m_vring[i], vq.size, vring_buf, rte_mem_page_size());
    ...
    if (i & 1) {
      ...
      hw->vring[i].used = m_vring_iova + (char *)internal->m_vring[i].used -
                          (char *)internal->m_vring[i].desc;
    }

    hw->vring[i].size = vq.size;

    rte_vhost_get_vring_base(vid, i, &internal->m_vring[i].avail->idx,
...
static int m_ifcvf_stop(struct ifcvf_internal *internal) {
  int vid;
  uint32_t i;
  struct rte_vhost_vring vq;
  struct ifcvf_hw *hw = &internal->hw;
...
static void *vring_relay(void *arg) {
  ...
  for (qid = 0; qid < q_num; qid += 2) {
    ...
    ev.data.u64 = 1 | qid << 1 | (uint64_t)internal->intr_fd[qid] << 32;
    if (epoll_ctl(epfd, EPOLL_CTL_ADD, internal->intr_fd[qid], &ev) < 0) {
      DRV_LOG(ERR, "epoll add error: %s", strerror(errno));
      return NULL;
    }
...
static int setup_vring_relay(struct ifcvf_internal *internal) {
  char name[THREAD_NAME_LEN];
  int ret;
...
static int unset_vring_relay(struct ifcvf_internal *internal) {
  void *status;
...
static int ifcvf_sw_fallback_switchover(struct ifcvf_internal *internal) {
  int ret;
...
static int ifcvf_dev_config(int vid) {
  struct rte_vdpa_device *vdev;
  struct internal_list *list;
  struct ifcvf_internal *internal;
  ...
  if (rte_vhost_host_notifier_ctrl(vid, RTE_VHOST_QUEUE_ALL, true) != 0)
    DRV_LOG(NOTICE, "vDPA (%s): software relay is used.", vdev->device->name);

  internal->configured = 1;
  return 0;
}

static int ifcvf_dev_close(int vid) {
  struct rte_vdpa_device *vdev;
  struct internal_list *list;
  struct ifcvf_internal *internal;
...
static int ifcvf_set_features(int vid) {
  uint64_t features = 0;
  struct rte_vdpa_device *vdev;
  struct internal_list *list;
  struct ifcvf_internal *internal;
  ...
  if (internal->sw_lm) {
    ...
    rte_vfio_container_dma_map(internal->vfio_container_fd, log_base,
                               IFCVF_LOG_BASE, log_size);
...
static int ifcvf_get_vfio_group_fd(int vid) {
  struct rte_vdpa_device *vdev;
  struct internal_list *list;
...
static int ifcvf_get_vfio_device_fd(int vid) {
  struct rte_vdpa_device *vdev;
  struct internal_list *list;
...
static int ifcvf_get_notify_area(int vid, int qid, uint64_t *offset,
                                 uint64_t *size) {
  struct rte_vdpa_device *vdev;
  struct internal_list *list;
  struct ifcvf_internal *internal;
  ...
  if (ret) {
    DRV_LOG(ERR, "Get not get device region info: %s", strerror(errno));
    return -1;
  }
...
static int ifcvf_get_queue_num(struct rte_vdpa_device *vdev,
                               uint32_t *queue_num) {
  struct internal_list *list;
...
static int ifcvf_get_vdpa_features(struct rte_vdpa_device *vdev,
                                   uint64_t *features) {
  struct internal_list *list;
...
static int ifcvf_get_protocol_features(struct rte_vdpa_device *vdev,
                                       uint64_t *features) {
  RTE_SET_USED(vdev);
...
static int ifcvf_set_vring_state(int vid, int vring, int state) {
  struct rte_vdpa_device *vdev;
  struct internal_list *list;
  struct ifcvf_internal *internal;
  struct ifcvf_hw *hw;
...
static int ifcvf_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  kvlist =
      rte_kvargs_parse(pci_dev->device.devargs->args, ifcvf_valid_arguments);
  if (kvlist == NULL)
    return 1;
  ...
  ret = rte_kvargs_process(kvlist, IFCVF_VDPA_MODE, &open_int, &vdpa_mode);
  ...
  internal->features =
      ...
      (1ULL << VIRTIO_NET_F_GUEST_ANNOUNCE) | (1ULL << VIRTIO_NET_F_CTRL_VQ) |
      (1ULL << VIRTIO_NET_F_STATUS) | (1ULL << VHOST_USER_F_PROTOCOL_FEATURES) |
      (1ULL << VHOST_F_LOG_ALL);

  list->internal = internal;

  if (rte_kvargs_count(kvlist, IFCVF_SW_FALLBACK_LM)) {
    ret = rte_kvargs_process(kvlist, IFCVF_SW_FALLBACK_LM, &open_int,
                             &sw_fallback_lm);
    if (ret < 0)
      goto error;
  }
...
static int ifcvf_pci_remove(struct rte_pci_device *pci_dev) {
  struct ifcvf_internal *internal;
  struct internal_list *list;
