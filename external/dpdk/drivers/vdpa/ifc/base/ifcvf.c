...
STATIC void *get_cap_addr(struct ifcvf_hw *hw, struct ifcvf_pci_cap *cap) {
  u8 bar = cap->bar;
  ...
  if (offset + length > hw->mem_resource[cap->bar].len) {
    DEBUGOUT("offset(%u) + length(%u) overflows bar length(%u)", offset, length,
             (u32)hw->mem_resource[cap->bar].len);
    return NULL;
  }
...
int ifcvf_init_hw(struct ifcvf_hw *hw, PCI_DEV *dev) {
  int ret;
  u8 pos;
  ...
  while (pos) {
    ret = PCI_READ_CONFIG_RANGE(dev, (u32 *)&cap, sizeof(cap), pos);
    if (ret < 0) {
    ...
    case IFCVF_PCI_CAP_NOTIFY_CFG:
      ret = PCI_READ_CONFIG_DWORD(dev, &hw->notify_off_multiplier,
                                  pos + sizeof(cap));
      if (ret < 0) {
  ...
  if (hw->common_cfg == NULL || hw->notify_base == NULL || hw->isr == NULL ||
      hw->dev_cfg == NULL) {
...
STATIC int ifcvf_hw_enable(struct ifcvf_hw *hw) {
  ...
  for (i = 0; i < hw->nr_vring; i++) {
    ...
    *(u32 *)(lm_cfg + IFCVF_LM_RING_STATE_OFFSET + (i / 2) * IFCVF_LM_CFG_SIZE +
             (i % 2) * 4) = (u32)hw->vring[i].last_avail_idx |
                            ((u32)hw->vring[i].last_used_idx << 16);

    IFCVF_WRITE_REG16(i + 1, &cfg->queue_msix_vector);
    if (IFCVF_READ_REG16(&cfg->queue_msix_vector) == IFCVF_MSI_NO_VECTOR) {
...
void ifcvf_enable_logging(struct ifcvf_hw *hw, u64 log_base, u64 log_size) {
  ...
  *(u32 *)(lm_cfg + IFCVF_LM_BASE_ADDR_LOW) = log_base & IFCVF_32_BIT_MASK;
...
u8 ifcvf_get_notify_region(struct ifcvf_hw *hw) { return hw->notify_region; }
