...
static inline int order_producer(void *arg) {
...
int order_test_setup(struct evt_test *test, struct evt_options *opt) {
  ...
  t->seqn_dynfield_offset = rte_mbuf_dynfield_register(&seqn_dynfield_desc);
  if (t->seqn_dynfield_offset < 0) {
    evt_err("failed to register mbuf field");
    return -rte_errno;
  }

  t->producer_flow_seq = rte_zmalloc_socket(
      "test_producer_flow_seq", sizeof(*t->producer_flow_seq) * opt->nb_flows,
      RTE_CACHE_LINE_SIZE, opt->socket_id);
  ...
  t->expected_flow_seq = rte_zmalloc_socket(
      "test_expected_flow_seq", sizeof(*t->expected_flow_seq) * opt->nb_flows,
      RTE_CACHE_LINE_SIZE, opt->socket_id);
...
int order_mempool_setup(struct evt_test *test, struct evt_options *opt) {
  ...
  t->pool = rte_pktmbuf_pool_create(test->name, opt->pool_sz, 256 /* Cache */,
                                    0, 512, /* Use very small mbufs */
                                    opt->socket_id);
  if (t->pool == NULL) {
