...
static int order_queue_worker(void *arg, const bool flow_id_cap) {
  ORDER_WORKER_INIT;
  struct rte_event ev;
  ...
  while (t->err == false) {
    ...
    if (ev.queue_id == 0) { /* from ordered queue */
      ...
      order_process_stage_1(t, &ev, nb_flows, expected_flow_seq, outstand_pkts);
    } else {
...
static int order_queue_worker_burst(void *arg, const bool flow_id_cap) {
  ORDER_WORKER_INIT;
  ...
  while (t->err == false) {
    uint16_t const nb_rx =
        rte_event_dequeue_burst(dev_id, port, ev, BURST_SIZE, 0);
    ...
    for (i = 0; i < nb_rx; i++) {
      ...
      if (ev[i].queue_id == 0) { /* from ordered queue */
        ...
        order_process_stage_1(t, &ev[i], nb_flows, expected_flow_seq,
                              outstand_pkts);
    ...
    while (enq < nb_rx) {
      enq += rte_event_enqueue_burst(dev_id, port, ev + enq, nb_rx - enq);
    }
  }
  return 0;
}
...
static int order_queue_launch_lcores(struct evt_test *test,
                                     struct evt_options *opt) {
  return order_launch_lcores(test, opt, worker_wrapper);
}

#define NB_QUEUES 2
static int order_queue_eventdev_setup(struct evt_test *test,
                                      struct evt_options *opt) {
  int ret;
...
static void order_queue_opt_dump(struct evt_options *opt) {
  order_opt_dump(opt);
  evt_dump("nb_evdev_queues", "%d", NB_QUEUES);
}

static bool order_queue_capability_check(struct evt_options *opt) {
