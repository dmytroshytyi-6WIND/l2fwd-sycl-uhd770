...
int pipeline_test_result(struct evt_test *test, struct evt_options *opt) {
  ...
  for (i = 0; i < t->nb_workers; i++)
    evt_info("Worker %d packets: " CLGRN "%" PRIx64 "" CLNRM
             " percentage:" CLGRN " %3.2f" CLNRM,
...
static inline uint64_t processed_pkts(struct test_pipeline *t) {
  uint8_t i;
  uint64_t total = 0;
...
int pipeline_ethdev_setup(struct evt_test *test, struct evt_options *opt) {
  ...
  if (opt->max_pkt_sz < RTE_ETHER_MIN_LEN) {
    evt_err("max_pkt_sz can not be less than %d", RTE_ETHER_MIN_LEN);
    return -EINVAL;
  }

  port_conf.rxmode.mtu =
      opt->max_pkt_sz - RTE_ETHER_HDR_LEN - RTE_ETHER_CRC_LEN;
  ...
  RTE_ETH_FOREACH_DEV(i) {
    ...
    if (rte_eth_dev_configure(i, opt->eth_queues, nb_queues, &local_port_conf) <
        0) {
    ...
    for (j = 0; j < opt->eth_queues; j++) {
      if (rte_eth_rx_queue_setup(i, j, NB_RX_DESC, rte_socket_id(), &rx_conf,
...
int pipeline_event_port_setup(struct evt_test *test, struct evt_options *opt,
  ...
  for (port = 0; port < evt_nr_active_lcores(opt->wlcores); port++) {
    ...
    if (rte_event_port_link(opt->dev_id, port, queue_arr, NULL, nb_queues) !=
        nb_queues)
      goto link_fail;
  }
...
int pipeline_event_rx_adapter_setup(struct evt_options *opt, uint8_t stride,
  ...
  if (opt->ena_vector) {
    ...
    vector_pool = rte_event_vector_pool_create(
        "vector_pool", nb_elem, 0, opt->vector_size, opt->socket_id);
    if (vector_pool == NULL) {
  ...
  RTE_ETH_FOREACH_DEV(prod) {
    ...
    if (opt->ena_vector) {
      ...
      ret = rte_event_eth_rx_adapter_vector_limits_get(opt->dev_id, prod,
                                                       &limits);
      if (ret) {
      ...
      if (opt->vector_size < limits.min_sz ||
        ...
        evt_err("Vector size [%d] not within limits max[%d] min[%d]",
                opt->vector_size, limits.min_sz, limits.max_sz);
        return -EINVAL;
      }
      ...
      if (opt->vector_tmo_nsec > limits.max_timeout_ns ||
        ...
        evt_err("Vector timeout [%" PRIu64 "] not within limits max[%" PRIu64
                "] min[%" PRIu64 "]",
      ...
      if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_EVENT_VECTOR) {
        ...
        queue_conf.vector_timeout_ns = opt->vector_tmo_nsec;
    ...
    evt_info("Port[%d] using Rx adapter[%d] configured", prod, prod);
  }
...
int pipeline_event_tx_adapter_setup(struct evt_options *opt,
  ...
  RTE_ETH_FOREACH_DEV(consm) {
    ...
    ret = rte_event_eth_tx_adapter_caps_get(opt->dev_id, consm, &cap);
    if (ret) {
      evt_err("failed to get event tx adapter[%d] caps", consm);
      return ret;
    }

    if (opt->ena_vector) {
      if (!(cap & RTE_EVENT_ETH_TX_ADAPTER_CAP_EVENT_VECTOR)) {
        evt_err("Tx adapter doesn't support event vector");
        return -EINVAL;
      }
    }

    ret = rte_event_eth_tx_adapter_create(consm, opt->dev_id, &port_conf);
    if (ret) {
    ...
    if (ret) {
      evt_err("failed to add tx queues to adapter[%d]", consm);
      return ret;
    }
    ...
    if (!(cap & RTE_EVENT_ETH_TX_ADAPTER_CAP_INTERNAL_PORT)) {
      ...
      ret = rte_event_eth_tx_adapter_service_id_get(consm, &service_id);
      if (ret != -ESRCH && ret != 0) {
    ...
    evt_info("Port[%d] using Tx adapter[%d] Configured", consm, consm);
  }
...
int pipeline_mempool_setup(struct evt_test *test, struct evt_options *opt) {
  ...
  RTE_ETH_FOREACH_DEV(i) {
    ...
    if (dev_info.rx_desc_lim.nb_mtu_seg_max != UINT16_MAX &&
      ...
      data_size = opt->max_pkt_sz / dev_info.rx_desc_lim.nb_mtu_seg_max;
    ...
    if (opt->per_port_pool) {
      ...
      snprintf(name, RTE_MEMPOOL_NAMESIZE, "%s-%d", test->name, i);
      t->pool[i] =
          rte_pktmbuf_pool_create(name,         /* mempool name */
  ...
  if (!opt->per_port_pool) {
    t->pool[0] =
        rte_pktmbuf_pool_create(test->name,   /* mempool name */
                                opt->pool_sz, /* number of elements*/
                                0,            /* cache size*/
                                0, opt->mbuf_sz, opt->socket_id); /* flags */
...
int pipeline_test_setup(struct evt_test *test, struct evt_options *opt) {
  ...
  test_pipeline = rte_zmalloc_socket(test->name, sizeof(struct test_pipeline),
