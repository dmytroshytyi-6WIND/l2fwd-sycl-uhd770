...
static __rte_noinline int pipeline_queue_worker_single_stage_tx(void *arg) {
...
static __rte_noinline int pipeline_queue_worker_single_stage_fwd(void *arg) {
  PIPELINE_WORKER_SINGLE_STAGE_INIT;
  ...
  while (!t->done) {
    ...
    for (i = 0; i < nb_rx; i++) {
      if (ev[i].sched_type == RTE_SCHED_TYPE_ATOMIC) {
        ...
        ev[i].queue_id++;
        pipeline_fwd_event_vector(&ev[i], RTE_SCHED_TYPE_ATOMIC);
      }
    }
...
static __rte_noinline int pipeline_queue_worker_multi_stage_tx(void *arg) {
  ...
  while (t->done == false) {
    ...
    ev.queue_id++;
    pipeline_fwd_event(&ev, cq_id != last_queue ? sched_type_list[cq_id]
...
static __rte_noinline int pipeline_queue_worker_multi_stage_fwd(void *arg) {
  PIPELINE_WORKER_MULTI_STAGE_INIT;
  const uint8_t *tx_queue = t->tx_evqueue_id;
...
pipeline_queue_worker_multi_stage_burst_tx(void *arg) {
  ...
  while (t->done == false) {
    ...
    for (i = 0; i < nb_rx; i++) {
      ...
      ev[i].queue_id++;
      pipeline_fwd_event(&ev[i], cq_id != last_queue ? sched_type_list[cq_id]
                                                     : RTE_SCHED_TYPE_ATOMIC);
      pipeline_event_enqueue_burst(dev, port, ev, nb_rx);
    }
  }
...
pipeline_queue_worker_multi_stage_tx_vector(void *arg) {
  ...
  while (!t->done) {
    ...
    ev.queue_id++;
    pipeline_fwd_event_vector(&ev, cq_id != last_queue ? sched_type_list[cq_id]
                                                       : RTE_SCHED_TYPE_ATOMIC);
    pipeline_event_enqueue(dev, port, &ev);
  }
...
pipeline_queue_worker_multi_stage_burst_tx_vector(void *arg) {
  ...
  while (!t->done) {
    ...
    for (i = 0; i < nb_rx; i++) {
      ...
      ev[i].queue_id++;
      pipeline_fwd_event_vector(&ev[i], cq_id != last_queue
  ...
  while (!t->done) {
    ...
    for (i = 0; i < nb_rx; i++) {
      ...
      if (cq_id == last_queue) {
        ...
        ev[i].queue_id++;
        pipeline_fwd_event_vector(&ev[i], sched_type_list[cq_id]);
      }
    }
...
static int worker_wrapper(void *arg) {
  ...
  const pipeline_queue_worker_t pipeline_queue_worker_single_stage[2][2][2] = {
  ...
  const pipeline_queue_worker_t pipeline_queue_worker_multi_stage[2][2][2] = {
  ...
  if (nb_stages == 1)
    return (pipeline_queue_worker_single_stage[opt->ena_vector][burst]
                                              [internal_port])(arg);
  else
    return (pipeline_queue_worker_multi_stage[opt->ena_vector][burst]
                                             [internal_port])(arg);

  rte_panic("invalid worker\n");
}

static int pipeline_queue_launch_lcores(struct evt_test *test,
                                        struct evt_options *opt) {
  return pipeline_launch_lcores(test, opt, worker_wrapper);
}

static int pipeline_queue_eventdev_setup(struct evt_test *test,
                                         struct evt_options *opt) {
  int ret;
  int nb_ports;
  int nb_queues;
  ...
  if (!t->internal_port) {
    ret = pipeline_event_port_setup(test, opt, queue_arr, nb_worker_queues,
                                    p_conf);
    if (ret)
      return ret;
  } else
    ret = pipeline_event_port_setup(test, opt, NULL, nb_queues, p_conf);
  ...
  if (!t->internal_port) {
    RTE_ETH_FOREACH_DEV(prod) {
      ret = rte_event_eth_tx_adapter_event_port_get(prod, &tx_evport_id);
      if (ret) {
        evt_err("Unable to get Tx adptr[%d] evprt[%d]", prod, tx_evport_id);
        return ret;
      }

      if (rte_event_port_link(opt->dev_id, tx_evport_id, &tx_evqueue_id[prod],
                              NULL, 1) != 1) {
        evt_err("Unable to link Tx adptr[%d] evprt[%d]", prod, tx_evport_id);
        return ret;
      }
    }
  }
  ...
  memcpy(t->tx_evqueue_id, tx_evqueue_id, sizeof(uint8_t) * RTE_MAX_ETHPORTS);

  return 0;
}

static void pipeline_queue_opt_dump(struct evt_options *opt) {
  pipeline_opt_dump(opt, pipeline_queue_nb_event_queues(opt));
}

static int pipeline_queue_opt_check(struct evt_options *opt) {
  return pipeline_opt_check(opt, pipeline_queue_nb_event_queues(opt));
}

static bool pipeline_queue_capability_check(struct evt_options *opt) {
  struct rte_event_dev_info dev_info;

  rte_event_dev_info_get(opt->dev_id, &dev_info);
  if (dev_info.max_event_queues < pipeline_queue_nb_event_queues(opt) ||
      dev_info.max_event_ports < evt_nr_active_lcores(opt->wlcores)) {
    evt_err("not enough eventdev queues=%d/%d or ports=%d/%d",
            pipeline_queue_nb_event_queues(opt), dev_info.max_event_queues,
            evt_nr_active_lcores(opt->wlcores), dev_info.max_event_ports);
  }
