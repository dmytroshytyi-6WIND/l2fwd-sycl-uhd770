...
static inline int atq_nb_event_queues(struct evt_options *opt) {
...
static __rte_always_inline void atq_fwd_event(struct rte_event *const ev,
...
static int perf_atq_worker(void *arg, const int enable_fwd_latency) {
  PERF_WORKER_INIT;
  struct rte_event ev;
...
static int perf_atq_worker_burst(void *arg, const int enable_fwd_latency) {
  PERF_WORKER_INIT;
  uint16_t i;
  ...
  while (t->done == false) {
    ...
    for (i = 0; i < nb_rx; i++) {
      ...
      /* last stage in pipeline */
      if (unlikely((ev[i].sub_event_type % nb_stages) == laststage)) {
        if (enable_fwd_latency)
        ...
        atq_fwd_event(&ev[i], sched_type_list, nb_stages);
      }
    }
...
static int perf_atq_launch_lcores(struct evt_test *test,
                                  struct evt_options *opt) {
  return perf_launch_lcores(test, opt, worker_wrapper);
}

static int perf_atq_eventdev_setup(struct evt_test *test,
                                   struct evt_options *opt) {
  int ret;
  uint8_t queue;
  uint8_t nb_queues;
  ...
  ret =
      perf_event_dev_port_setup(test, opt, 1 /* stride */, nb_queues, &p_conf);
  if (ret)
    return ret;
...
static void perf_atq_opt_dump(struct evt_options *opt) {
  perf_opt_dump(opt, atq_nb_event_queues(opt));
}

static int perf_atq_opt_check(struct evt_options *opt) {
  return perf_opt_check(opt, atq_nb_event_queues(opt));
}

static bool perf_atq_capability_check(struct evt_options *opt) {
  struct rte_event_dev_info dev_info;
