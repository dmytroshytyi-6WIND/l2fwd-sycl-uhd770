...
static __rte_noinline int pipeline_atq_worker_single_stage_tx(void *arg) {
...
static __rte_noinline int pipeline_atq_worker_single_stage_fwd(void *arg) {
  PIPELINE_WORKER_SINGLE_STAGE_INIT;
...
static __rte_noinline int pipeline_atq_worker_single_stage_burst_tx(void *arg) {
...
static __rte_noinline int pipeline_atq_worker_multi_stage_tx(void *arg) {
...
static __rte_noinline int pipeline_atq_worker_multi_stage_fwd(void *arg) {
  PIPELINE_WORKER_MULTI_STAGE_INIT;
  const uint8_t *tx_queue = t->tx_evqueue_id;
...
static __rte_noinline int pipeline_atq_worker_multi_stage_burst_tx(void *arg) {
...
static __rte_noinline int pipeline_atq_worker_multi_stage_burst_fwd(void *arg) {
  PIPELINE_WORKER_MULTI_STAGE_BURST_INIT;
  const uint8_t *tx_queue = t->tx_evqueue_id;
...
static __rte_noinline int pipeline_atq_worker_multi_stage_tx_vector(void *arg) {
  PIPELINE_WORKER_MULTI_STAGE_INIT;
  uint16_t vector_sz;
...
static int worker_wrapper(void *arg) {
  ...
  const pipeline_atq_worker_t pipeline_atq_worker_multi_stage[2][2][2] = {
  ...
  if (nb_stages == 1)
    ...
    return (
        pipeline_atq_worker_multi_stage[opt->ena_vector][burst][internal_port])(
        arg);

  rte_panic("invalid worker\n");
}

static int pipeline_atq_launch_lcores(struct evt_test *test,
                                      struct evt_options *opt) {
  return pipeline_launch_lcores(test, opt, worker_wrapper);
}

static int pipeline_atq_eventdev_setup(struct evt_test *test,
                                       struct evt_options *opt) {
  int ret;
  int nb_ports;
  int nb_queues;
...
static void pipeline_atq_opt_dump(struct evt_options *opt) {
  pipeline_opt_dump(opt, pipeline_atq_nb_event_queues(opt));
}

static int pipeline_atq_opt_check(struct evt_options *opt) {
  return pipeline_opt_check(opt, pipeline_atq_nb_event_queues(opt));
}

static bool pipeline_atq_capability_check(struct evt_options *opt) {
  ...
  if (dev_info.max_event_queues < pipeline_atq_nb_event_queues(opt) ||
    ...
    evt_err("not enough eventdev queues=%d/%d or ports=%d/%d",
            pipeline_atq_nb_event_queues(opt), dev_info.max_event_queues,
