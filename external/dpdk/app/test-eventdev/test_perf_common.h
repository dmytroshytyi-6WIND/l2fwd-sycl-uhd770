...
struct test_perf {
  ...
  struct rte_event_timer_adapter
      *timer_adptr[RTE_EVENT_TIMER_ADAPTER_NUM_MAX] __rte_cache_aligned;
...
static __rte_always_inline uint8_t perf_process_last_stage_latency(
    struct rte_mempool *const pool, struct rte_event *const ev,
    struct worker_data *const w, void *bufs[], int const buf_sz,
    uint8_t count) {
  uint64_t latency;
...
static inline int perf_nb_event_ports(struct evt_options *opt) {
