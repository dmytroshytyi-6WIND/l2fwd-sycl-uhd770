...
static inline int perf_queue_nb_event_queues(struct evt_options *opt) {
...
static __rte_always_inline void mark_fwd_latency(struct rte_event *const ev,
...
static __rte_always_inline void fwd_event(struct rte_event *const ev,
...
static int perf_queue_worker(void *arg, const int enable_fwd_latency) {
  PERF_WORKER_INIT;
  struct rte_event ev;
  ...
  while (t->done == false) {
    ...
    if (unlikely((ev.queue_id % nb_stages) == laststage)) {
      if (enable_fwd_latency)
        cnt = perf_process_last_stage_latency(pool, &ev, w, bufs, sz, cnt);
      else
        cnt = perf_process_last_stage(pool, &ev, w, bufs, sz, cnt);
    } else {
...
static int perf_queue_worker_burst(void *arg, const int enable_fwd_latency) {
  PERF_WORKER_INIT;
  uint16_t i;
  ...
  while (t->done == false) {
    ...
    for (i = 0; i < nb_rx; i++) {
      if (prod_crypto_type && (ev[i].event_type == RTE_EVENT_TYPE_CRYPTODEV)) {
        ...
        if (op->status == RTE_CRYPTO_OP_STATUS_SUCCESS) {
          if (op->sym->m_dst == NULL)
            ev[i].event_ptr = op->sym->m_src;
          else
            ev[i].event_ptr = op->sym->m_dst;
          rte_crypto_op_free(op);
        } else {
          rte_crypto_op_free(op);
          continue;
      ...
      /* last stage in pipeline */
      if (unlikely((ev[i].queue_id % nb_stages) == laststage)) {
        if (enable_fwd_latency)
          cnt = perf_process_last_stage_latency(pool, &ev[i], w, bufs, sz, cnt);
        else
          cnt = perf_process_last_stage(pool, &ev[i], w, bufs, sz, cnt);
...
static int perf_queue_launch_lcores(struct evt_test *test,
                                    struct evt_options *opt) {
  return perf_launch_lcores(test, opt, worker_wrapper);
}

static int perf_queue_eventdev_setup(struct evt_test *test,
                                     struct evt_options *opt) {
  uint8_t queue;
  ...
  for (queue = 0; queue < nb_queues; queue++) {
    q_conf.schedule_type = (opt->sched_type_list[queue % nb_stages]);
    ...
    if (opt->q_priority) {
       ...
       */
      uint8_t step = RTE_EVENT_DEV_PRIORITY_LOWEST / (nb_stages - 1);
      /* Higher prio for the queues closer to last stage */
      q_conf.priority = RTE_EVENT_DEV_PRIORITY_LOWEST - (step * stage_pos);
    }
  ...
  ret = perf_event_dev_port_setup(test, opt, nb_stages /* stride */, nb_queues,
                                  &p_conf);
  if (ret)
    return ret;
  ...
  if (opt->prod_type == EVT_PROD_TYPE_ETH_RX_ADPTR) {
    ...
    for (prod = 0; prod < opt->nb_timer_adptrs; prod++) {
      ret = rte_event_timer_adapter_start(t->timer_adptr[prod]);
      if (ret) {
        evt_err("failed to Start event timer adapter %d", prod);
        return ret;
      }
    }
    ...
    for (cdev_id = 0; cdev_id < cdev_count; cdev_id++) {
      ...
      if (ret) {
        evt_err("Failed to start cryptodev %u", cdev_id);
        return ret;
      }
    }
  }
...
static void perf_queue_opt_dump(struct evt_options *opt) {
  evt_dump_fwd_latency(opt);
  perf_opt_dump(opt, perf_queue_nb_event_queues(opt));
}

static int perf_queue_opt_check(struct evt_options *opt) {
  return perf_opt_check(opt, perf_queue_nb_event_queues(opt));
}

static bool perf_queue_capability_check(struct evt_options *opt) {
  struct rte_event_dev_info dev_info;
  ...
  if (dev_info.max_event_queues < perf_queue_nb_event_queues(opt) ||
    ...
    evt_err("not enough eventdev queues=%d/%d or ports=%d/%d",
            perf_queue_nb_event_queues(opt), dev_info.max_event_queues,
