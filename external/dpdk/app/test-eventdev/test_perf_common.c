...
int perf_test_result(struct evt_test *test, struct evt_options *opt) {
  ...
  for (i = 0; i < t->nb_workers; i++)
    printf("Worker %d packets: " CLGRN "%" PRIx64 " " CLNRM "percentage:" CLGRN
           ...
           (((double)t->worker[i].processed_pkts) / total) * 100);

  return t->result;
}

static inline int perf_producer(void *arg) {
  int i;
...
static inline int perf_producer_burst(void *arg) {
  uint32_t i;
  uint64_t timestamp;
  ...
  while (count < nb_pkts && t->done == false) {
    ...
    while (enq < burst_size) {
      enq += rte_event_enqueue_burst(dev_id, port, ev + enq, burst_size - enq);
      if (t->done)
        break;
      rte_pause();
      timestamp = rte_get_timer_cycles();
...
static inline int perf_event_timer_producer(void *arg) {
  int i;
  struct prod_data *p = arg;
  struct test_perf *t = p->t;
  struct evt_options *opt = t->opt;
  ...
  while (count < nb_timers && t->done == false) {
    ...
    for (i = 0; i < BURST_SIZE; i++) {
      ...
      m[i]->timestamp = rte_get_timer_cycles();
      while (rte_event_timer_arm_burst(adptr[flow_counter % nb_timer_adptrs],
                                       (struct rte_event_timer **)&m[i],
                                       1) != 1) {
        if (t->done)
          break;
        m[i]->timestamp = rte_get_timer_cycles();
  ...
  printf("%s(): lcore %d Average event timer arm latency = %.3f us\n", __func__,
         rte_lcore_id(),
         count ? (float)(arm_latency / count) / (rte_get_timer_hz() / 1000000)
               : 0);
  return 0;
}

static inline int perf_event_timer_producer_burst(void *arg) {
  int i;
  struct prod_data *p = arg;
  struct test_perf *t = p->t;
  struct evt_options *opt = t->opt;
  ...
  while (count < nb_timers && t->done == false) {
    ...
    rte_event_timer_arm_tmo_tick_burst(adptr[flow_counter % nb_timer_adptrs],
                                       (struct rte_event_timer **)m,
                                       tim.timeout_ticks, BURST_SIZE);
...
static inline void crypto_adapter_enq_op_new(struct prod_data *p) {
  ...
  if (opt->verbose_level > 1)
    printf("%s(): lcore %d queue %d cdev_id %u cdev_qp_id %u\n", __func__,
           rte_lcore_id(), p->queue_id, p->ca.cdev_id, p->ca.cdev_qp_id);
  ...
  while (count < nb_pkts && t->done == false) {
    ...
    op = rte_crypto_op_alloc(t->ca_op_pool, RTE_CRYPTO_OP_TYPE_SYMMETRIC);
    sym_op = op->sym;
    sym_op->m_src = m;
    sym_op->cipher.data.offset = 0;
    sym_op->cipher.data.length = len;
    rte_crypto_op_attach_sym_session(op,
                                     crypto_sess[flow_counter++ % nb_flows]);
...
static inline void crypto_adapter_enq_op_fwd(struct prod_data *p) {
  ...
  if (opt->verbose_level > 1)
    printf("%s(): lcore %d port %d queue %d cdev_id %u cdev_qp_id %u\n",
           __func__, rte_lcore_id(), port, p->queue_id, p->ca.cdev_id,
           p->ca.cdev_qp_id);
...
static inline int perf_event_crypto_producer(void *arg) {
  struct prod_data *p = arg;
...
static int perf_producer_wrapper(void *arg) {
  struct prod_data *p = arg;
  struct test_perf *t = p->t;
   ...
   */
  if (t->opt->prod_type == EVT_PROD_TYPE_SYNT && t->opt->prod_enq_burst_sz == 1)
...
static inline uint64_t processed_pkts(struct test_perf *t) {
  uint8_t i;
  uint64_t total = 0;
...
static inline uint64_t total_latency(struct test_perf *t) {
  uint8_t i;
  uint64_t total = 0;
...
int perf_launch_lcores(struct evt_test *test, struct evt_options *opt,
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    ret = rte_eal_remote_launch(perf_producer_wrapper, &t->prod[port_idx],
                                lcore_id);
    if (ret) {
  ...
  while (t->done == false) {
    ...
    if ((new_cycles - perf_cycles) > perf_sample) {
      ...
      if (opt->fwd_latency && pkts > 0) {
        printf(CLGRN
               "\r%.3f mpps avg %.3f mpps [avg fwd latency %.3f us] " CLNRM,
...
static int perf_event_rx_adapter_setup(struct evt_options *opt, uint8_t stride,
  ...
  RTE_ETH_FOREACH_DEV(prod) {
    ...
    ret = rte_event_eth_rx_adapter_caps_get(opt->dev_id, prod, &cap);
    if (ret) {
    ...
    ret = rte_event_eth_rx_adapter_create(prod, opt->dev_id, &prod_conf);
    if (ret) {
      evt_err("failed to create rx adapter[%d]", prod);
      return ret;
    }
    ret = rte_event_eth_rx_adapter_queue_add(prod, prod, -1, &queue_conf);
    if (ret) {
    ...
    if (!(cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT)) {
      ...
      rte_event_eth_rx_adapter_service_id_get(prod, &service_id);
...
static int perf_event_timer_adapter_setup(struct test_perf *t) {
  int i;
  int ret;
  ...
  for (i = 0; i < t->opt->nb_timer_adptrs; i++) {
    ...
    memset(&adapter_info, 0, sizeof(struct rte_event_timer_adapter_info));
    rte_event_timer_adapter_get_info(wl, &adapter_info);
    t->opt->optm_timer_tick_nsec = adapter_info.min_resolution_ns;

    if (!(adapter_info.caps & RTE_EVENT_TIMER_ADAPTER_CAP_INTERNAL_PORT)) {
      uint32_t service_id = -1U;

      rte_event_timer_adapter_service_id_get(wl, &service_id);
      ret = evt_service_setup(service_id);
      if (ret) {
        evt_err("Failed to setup service core"
                " for timer adapter\n");
...
static int perf_event_crypto_adapter_setup(struct test_perf *t,
                                           struct prod_data *p) {
  struct evt_options *opt = t->opt;
  uint32_t cap;
  int ret;
  ...
  if (cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_QP_EV_BIND) {
    ...
    ret = rte_event_crypto_adapter_queue_pair_add(
        TEST_PERF_CA_ID, p->ca.cdev_id, p->ca.cdev_qp_id, &response_info);
  } else {
    ret = rte_event_crypto_adapter_queue_pair_add(
...
int perf_event_dev_port_setup(struct evt_test *test, struct evt_options *opt,
  ...
  if (opt->prod_type == EVT_PROD_TYPE_ETH_RX_ADPTR) {
    ...
    ret =
        rte_event_crypto_adapter_create(TEST_PERF_CA_ID, opt->dev_id, &conf, 0);
    if (ret) {
    ...
    for (; port < perf_nb_event_ports(opt); port++) {
      ...
      p->ca.crypto_sess =
          rte_zmalloc_socket(NULL, sizeof(crypto_sess) * t->nb_flows,
      ...
      for (flow_id = 0; flow_id < t->nb_flows; flow_id++) {
        ...
        rte_cryptodev_sym_session_set_user_data(crypto_sess, &m_data,
                                                sizeof(m_data));
        p->ca.crypto_sess[flow_id] = crypto_sess;
      }

      conf.event_port_cfg |=
          RTE_EVENT_PORT_CFG_HINT_PRODUCER | RTE_EVENT_PORT_CFG_HINT_CONSUMER;
...
int perf_opt_check(struct evt_options *opt, uint64_t nb_queues) {
  ...
  if (opt->prod_type == EVT_PROD_TYPE_SYNT ||
    ...
    if (evt_lcores_has_overlap(opt->plcores, rte_get_main_lcore())) {
...
static inline void perf_elt_init(struct rte_mempool *mp, void *arg __rte_unused,
  ...
  RTE_ETH_FOREACH_DEV(i) {
    ...
    if (local_port_conf.rx_adv_conf.rss_conf.rss_hf !=
      ...
      evt_info("Port %u modified RSS hash function based on hardware support,"
               ...
               i, port_conf.rx_adv_conf.rss_conf.rss_hf,
               local_port_conf.rx_adv_conf.rss_conf.rss_hf);
    }
    ...
    if (rte_eth_rx_queue_setup(i, 0, NB_RX_DESC, rte_socket_id(), NULL,
                               t->pool) < 0) {
      evt_err("Failed to setup eth port [%d] rx_queue: %d.", i, 0);
      return -EINVAL;
    }

    if (rte_eth_tx_queue_setup(i, 0, NB_TX_DESC, rte_socket_id(), NULL) < 0) {
      evt_err("Failed to setup eth port [%d] tx_queue: %d.", i, 0);
      return -EINVAL;
    }

    ret = rte_eth_promiscuous_enable(i);
    if (ret != 0) {
      evt_err("Failed to enable promiscuous mode for eth port [%d]: %s", i,
              rte_strerror(-ret));
      return ret;
    }
  }
...
int perf_cryptodev_setup(struct evt_test *test, struct evt_options *opt) {
  ...
  t->ca_op_pool =
      rte_crypto_op_pool_create("crypto_op_pool", RTE_CRYPTO_OP_TYPE_SYMMETRIC,
                                opt->pool_sz, 128, 0, rte_socket_id());
  ...
  t->ca_sess_priv_pool =
      rte_mempool_create("ca_sess_priv_pool", nb_sessions, max_session_size, 0,
  ...
  for (cdev_id = 0; cdev_id < cdev_count; cdev_id++) {
    ...
    if (nb_qps > info.max_nb_queue_pairs) {
      evt_err("Not enough queue pairs per cryptodev (%u)", nb_qps);
      ret = -EINVAL;
      goto err;
    }
    ...
    for (qp_id = 0; qp_id < conf.nb_queue_pairs; qp_id++) {
      ret = rte_cryptodev_queue_pair_setup(cdev_id, qp_id, &qp_conf,
...
void perf_cryptodev_destroy(struct evt_test *test, struct evt_options *opt) {
  ...
  for (port = t->nb_workers; port < perf_nb_event_ports(opt); port++) {
    ...
    rte_event_crypto_adapter_queue_pair_del(TEST_PERF_CA_ID, p->ca.cdev_id,
                                            p->ca.cdev_qp_id);
  }
...
int perf_mempool_setup(struct evt_test *test, struct evt_options *opt) {
  ...
  if (opt->prod_type == EVT_PROD_TYPE_SYNT ||
      opt->prod_type == EVT_PROD_TYPE_EVENT_TIMER_ADPTR) {
    t->pool =
        rte_mempool_create(test->name,                   /* mempool name */
                           opt->pool_sz,                 /* number of elements*/
                           sizeof(struct perf_elt),      /* element size*/
                           512,                          /* cache size*/
                           0, NULL, NULL, perf_elt_init, /* obj constructor */
...
int perf_test_setup(struct evt_test *test, struct evt_options *opt) {
  ...
  if (opt->prod_type == EVT_PROD_TYPE_EVENT_TIMER_ADPTR) {
    t->outstand_pkts = opt->nb_timers * evt_nr_active_lcores(opt->plcores);
