...
static inline void order_flow_id_copy_from_mbuf(struct test_order *t,
                                                struct rte_event *event) {
  event->flow_id =
      *RTE_MBUF_DYNFIELD(event->mbuf, t->flow_id_dynfield_offset, flow_id_t *);
}

static inline void order_flow_id_save(struct test_order *t, flow_id_t flow_id,
                                      struct rte_mbuf *mbuf,
                                      struct rte_event *event) {
  *RTE_MBUF_DYNFIELD(mbuf, t->flow_id_dynfield_offset, flow_id_t *) = flow_id;
  event->flow_id = flow_id;
  event->mbuf = mbuf;
}

static inline seqn_t *order_mbuf_seqn(struct test_order *t,
                                      struct rte_mbuf *mbuf) {
  return RTE_MBUF_DYNFIELD(mbuf, t->seqn_dynfield_offset, seqn_t *);
}

static inline int order_nb_event_ports(struct evt_options *opt) {
  return evt_nr_active_lcores(opt->wlcores) + 1 /* producer */;
}

static __rte_always_inline void
order_process_stage_1(struct test_order *const t, struct rte_event *const ev,
                      const uint32_t nb_flows,
