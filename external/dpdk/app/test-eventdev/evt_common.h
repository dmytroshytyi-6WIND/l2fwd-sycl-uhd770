...
#define evt_info(fmt, args...) fprintf(stdout, CLYEL "" fmt CLNRM "\n", ##args)
...
static inline bool evt_has_distributed_sched(uint8_t dev_id) {
  struct rte_event_dev_info dev_info;

  rte_event_dev_info_get(dev_id, &dev_info);
  return (dev_info.event_dev_cap & RTE_EVENT_DEV_CAP_DISTRIBUTED_SCHED) ? true
                                                                        : false;
}

static inline bool evt_has_burst_mode(uint8_t dev_id) {
  struct rte_event_dev_info dev_info;
...
static inline bool evt_has_all_types_queue(uint8_t dev_id) {
  struct rte_event_dev_info dev_info;

  rte_event_dev_info_get(dev_id, &dev_info);
  return (dev_info.event_dev_cap & RTE_EVENT_DEV_CAP_QUEUE_ALL_TYPES) ? true
                                                                      : false;
}

static inline bool evt_has_flow_id(uint8_t dev_id) {
  struct rte_event_dev_info dev_info;

  rte_event_dev_info_get(dev_id, &dev_info);
  return (dev_info.event_dev_cap & RTE_EVENT_DEV_CAP_CARRY_FLOW_ID) ? true
                                                                    : false;
}

static inline int evt_service_setup(uint32_t service_id) {
  int32_t core_cnt;
  ...
  core_cnt = rte_service_lcore_list(core_array, RTE_MAX_LCORE);
  if (core_cnt < 0)
    return -ENOENT;
  /* Get the core which has least number of services running. */
  while (core_cnt--) {
    /* Reset default mapping */
    rte_service_map_lcore_set(service_id, core_array[core_cnt], 0);
    cnt = rte_service_lcore_count_services(core_array[core_cnt]);
    if (cnt < min_cnt) {
...
static inline int evt_configure_eventdev(struct evt_options *opt,
                                         uint8_t nb_queues, uint8_t nb_ports) {
  ...
  if (opt->deq_tmo_nsec) {
    if (opt->deq_tmo_nsec < info.min_dequeue_timeout_ns) {
      ...
      evt_info("dequeue_timeout_ns too low, using %d", opt->deq_tmo_nsec);
    }
    if (opt->deq_tmo_nsec > info.max_dequeue_timeout_ns) {
      opt->deq_tmo_nsec = info.max_dequeue_timeout_ns;
      evt_info("dequeue_timeout_ns too high, using %d", opt->deq_tmo_nsec);
    }
  }
