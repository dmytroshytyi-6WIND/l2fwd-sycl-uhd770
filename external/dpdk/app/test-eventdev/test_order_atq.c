...
static int order_atq_worker(void *arg, const bool flow_id_cap) {
  ORDER_WORKER_INIT;
  struct rte_event ev;
...
static int order_atq_worker_burst(void *arg, const bool flow_id_cap) {
  ORDER_WORKER_INIT;
...
static int order_atq_launch_lcores(struct evt_test *test,
                                   struct evt_options *opt) {
  return order_launch_lcores(test, opt, worker_wrapper);
}

#define NB_QUEUES 1
static int order_atq_eventdev_setup(struct evt_test *test,
                                    struct evt_options *opt) {
  int ret;
...
static void order_atq_opt_dump(struct evt_options *opt) {
  order_opt_dump(opt);
  evt_dump("nb_evdev_queues", "%d", NB_QUEUES);
}

static bool order_atq_capability_check(struct evt_options *opt) {
