...
static inline bool evt_lcores_has_overlap(bool lcores[], int lcore) {
...
static inline bool evt_lcores_has_overlap_multi(bool lcoresx[],
                                                bool lcoresy[]) {
  int i;
...
static inline bool evt_has_active_lcore(bool lcores[]) {
  int i;
...
static inline int evt_nr_active_lcores(bool lcores[]) {
  int i;
  int c = 0;
...
static inline int evt_get_first_active_lcore(bool lcores[]) {
  int i;
...
static inline bool evt_has_disabled_lcore(bool lcores[]) {
  int i;
...
static inline bool evt_has_invalid_stage(struct evt_options *opt) {
  if (!opt->nb_stages) {
    evt_err("need minimum one stage, check --stlist");
    return true;
  }
  if (opt->nb_stages > EVT_MAX_STAGES) {
    evt_err("requested changes are beyond EVT_MAX_STAGES=%d", EVT_MAX_STAGES);
    return true;
  }
  return false;
}

static inline bool evt_has_invalid_sched_type(struct evt_options *opt) {
  int i;

  for (i = 0; i < opt->nb_stages; i++) {
    if (opt->sched_type_list[i] > RTE_SCHED_TYPE_PARALLEL) {
      evt_err("invalid sched_type %d at %d", opt->sched_type_list[i], i);
      return true;
    }
  }
  return false;
...
/* option dump helpers */
static inline void evt_dump_worker_lcores(struct evt_options *opt) {
  int c;
...
static inline void evt_dump_producer_lcores(struct evt_options *opt) {
  int c;
...
static inline void evt_dump_nb_flows(struct evt_options *opt) {
  evt_dump("nb_flows", "%d", opt->nb_flows);
}

static inline void evt_dump_worker_dequeue_depth(struct evt_options *opt) {
  evt_dump("worker deq depth", "%d", opt->wkr_deq_dep);
}

static inline void evt_dump_nb_stages(struct evt_options *opt) {
  evt_dump("nb_stages", "%d", opt->nb_stages);
}

static inline void evt_dump_fwd_latency(struct evt_options *opt) {
  evt_dump("fwd_latency", "%s", EVT_BOOL_FMT(opt->fwd_latency));
}

static inline void evt_dump_queue_priority(struct evt_options *opt) {
  evt_dump("queue_priority", "%s", EVT_BOOL_FMT(opt->q_priority));
}

static inline const char *evt_sched_type_2_str(uint8_t sched_type) {
...
static inline void evt_dump_sched_type_list(struct evt_options *opt) {
  int i;
...
static inline const char *evt_prod_id_to_name(enum evt_prod_type prod_type) {
  switch (prod_type) {
  default:
...
static inline void evt_dump_producer_type(struct evt_options *opt) {
  char name[EVT_PROD_MAX_NAME_LEN];

  switch (opt->prod_type) {
  default:
  case EVT_PROD_TYPE_SYNT:
    snprintf(name, EVT_PROD_MAX_NAME_LEN, "Synthetic producer lcores");
    break;
  case EVT_PROD_TYPE_ETH_RX_ADPTR:
    snprintf(name, EVT_PROD_MAX_NAME_LEN, "Ethdev Rx Adapter producers");
  ...
  case EVT_PROD_TYPE_EVENT_TIMER_ADPTR:
    if (opt->timdev_use_burst)
      ...
      snprintf(name, EVT_PROD_MAX_NAME_LEN, "Event timer adapter producer");
    ...
    if (opt->optm_timer_tick_nsec)
      ...
      evt_dump("timer_tick_nsec", "%" PRIu64 "", opt->timer_tick_nsec);
    break;
  case EVT_PROD_TYPE_EVENT_CRYPTO_ADPTR:
    snprintf(name, EVT_PROD_MAX_NAME_LEN, "Event crypto adapter producers");
