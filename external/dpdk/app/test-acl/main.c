...
enum { DUMP_NONE, DUMP_SEARCH, DUMP_PKT, DUMP_MAX };
...
static void tracef_init(void) {
  ...
  config.traces =
      rte_zmalloc_socket(name, sz, RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
...
static int parse_cb_ipv6_rule(char *str, struct acl_rule *v) {
  ...
  if (rc != 0) {
    RTE_LOG(ERR, TESTACL, "failed to read source address/mask: %s\n",
            in[CB_FLD_SRC_ADDR]);
    return rc;
  }
  ...
  if (rc != 0) {
    RTE_LOG(ERR, TESTACL, "failed to read destination address/mask: %s\n",
            in[CB_FLD_DST_ADDR]);
    return rc;
  }
...
static uint32_t search_ip5tuples_once(uint32_t categories, uint32_t step,
  ...
  for (i = 0; i != config.used_traces; i += n) {
    ...
    ret = rte_acl_classify(config.acx, data, results, n, categories);
    ...
    for (r = 0, j = 0; j != n; j++) {
      for (k = 0; k != categories; k++, r++) {
        dump_verbose(DUMP_PKT, stdout,
                     ...
                     "result: %u\n",
                     config.ipv6 ? '6' : '4', i + j + 1, k, results[r] - 1);
      }
    }
  }
...
static int search_ip5tuples(__rte_unused void *arg) {
  ...
  for (i = 0; i != config.iter_num; i++) {
    pkt += search_ip5tuples_once(config.run_categories, config.trace_step,
                                 config.alg.name);
  }
  ...
  dump_verbose(DUMP_NONE, stdout,
               ...
               " pkts, %" PRIu32 " categories, %" PRIu64 " cycles (%.2Lf sec), "
...
static void print_usage(const char *prgname) {
  ...
  fprintf(stdout,
          ...
          prgname, RTE_ACL_RESULTS_MULTIPLIER, (uint32_t)RTE_ACL_MAX_CATEGORIES,
          buf);
}
...
static void dump_config(FILE *f) {
  ...
  fprintf(f, "%s:%u(%s)\n", OPT_SEARCH_ALG, config.alg.alg, config.alg.name);
...
static void get_input_opts(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, "", lgopts, &opt_idx)) != EOF) {
    ...
    if (strcmp(lgopts[opt_idx].name, OPT_RULE_FILE) == 0) {
      ...
      config.nb_rules =
          get_ulong_opt(optarg, lgopts[opt_idx].name, 1, RTE_ACL_MAX_INDEX + 1);
    } else if (strcmp(lgopts[opt_idx].name, OPT_MAX_SIZE) == 0) {
      config.max_size =
          get_ulong_opt(optarg, lgopts[opt_idx].name, 0, SIZE_MAX);
    } else if (strcmp(lgopts[opt_idx].name, OPT_TRACE_NUM) == 0) {
      config.nb_traces =
          get_ulong_opt(optarg, lgopts[opt_idx].name, 1, UINT32_MAX);
    } else if (strcmp(lgopts[opt_idx].name, OPT_TRACE_STEP) == 0) {
      config.trace_step =
          get_ulong_opt(optarg, lgopts[opt_idx].name, 1, TRACE_STEP_MAX);
    } else if (strcmp(lgopts[opt_idx].name, OPT_BLD_CATEGORIES) == 0) {
      config.bld_categories = get_ulong_opt(optarg, lgopts[opt_idx].name, 1,
                                            RTE_ACL_MAX_CATEGORIES);
    } else if (strcmp(lgopts[opt_idx].name, OPT_RUN_CATEGORIES) == 0) {
      config.run_categories = get_ulong_opt(optarg, lgopts[opt_idx].name, 1,
                                            RTE_ACL_MAX_CATEGORIES);
    } else if (strcmp(lgopts[opt_idx].name, OPT_ITER_NUM) == 0) {
      config.iter_num =
          get_ulong_opt(optarg, lgopts[opt_idx].name, 1, INT32_MAX);
    } else if (strcmp(lgopts[opt_idx].name, OPT_VERBOSE) == 0) {
      config.verbose =
          get_ulong_opt(optarg, lgopts[opt_idx].name, DUMP_NONE, DUMP_MAX);
  ...
  config.trace_sz =
      config.ipv6 ? sizeof(struct ipv6_5tuple) : sizeof(struct ipv4_5tuple);
}
