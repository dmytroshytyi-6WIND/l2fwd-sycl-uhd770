...
enum app_args { ARG_HELP, ARG_MEMPOOL };
...
static void args_parse(int argc, char **argv) {
  ...
  static struct option lgopts[] = {{"help", 0, 0, ARG_HELP},
                                   /* End of options */
                                   {0, 0, 0, 0}};
...
static int alloc_gpu_memory(uint16_t gpu_id) {
  void *ptr_1 = NULL;
  void *ptr_2 = NULL;
  ...
  if (ret < 0) {
    printf("GPU memory 0x%p NOT freed: GPU driver didn't find this memory "
           "address internally.\n",
           (uint8_t *)(ptr_1) + 0x700);
  } else {
...
static int register_cpu_memory(uint16_t gpu_id) {
  void *ptr = NULL;
  size_t buf_bytes = 1024;
  int ret;
  ...
  if (ret < 0) {
    printf("CPU memory 0x%p NOT unregistered: GPU driver didn't find this "
           "memory address internally\n",
           (uint8_t *)(ptr) + 0x700);
  } else {
    fprintf(stderr,
            "ERROR: rte_gpu_mem_unregister unregistered GPU memory 0x%p\n",
            (uint8_t *)(ptr) + 0x700);
    goto error;
  }
...
static int gpu_mem_cpu_map(uint16_t gpu_id) {
  void *ptr_gpu = NULL;
  void *ptr_cpu = NULL;
  size_t buf_bytes = 1024;
  unsigned int align = 4096;
  ...
  printf("GPU memory first 3 bytes set from CPU: %x %x %x\n",
         ((uint8_t *)ptr_cpu)[0], ((uint8_t *)ptr_cpu)[1],
         ((uint8_t *)ptr_cpu)[2]);
...
static int create_update_comm_flag(uint16_t gpu_id) {
  ...
  printf("Communication flag value at 0x%p was set to %d and current value is "
         "%d\n",
...
static int simulate_gpu_task(struct rte_gpu_comm_list *comm_list_item,
                             int num_pkts) {
  int idx;
...
static int create_update_comm_list(uint16_t gpu_id) {
  int ret = 0;
  int i = 0;
  ...
  if (ret == 0) {
    fprintf(stderr, "rte_gpu_comm_cleanup_list erroneously cleaned the list "
                    "even if packets have not been consumed yet\n");
    goto error;
  }
  printf("Communication list not cleaned because packets have not been "
         "consumed yet.\n");
...
int main(int argc, char **argv) {
  ...
  RTE_GPU_FOREACH(gpu_id) {
    ...
    printf("\tGPU ID %d\n\t\tparent ID %d GPU Bus ID %s NUMA node %d Tot "
           "memory %.02f MB, Tot processors %d\n",
           ginfo.dev_id, ginfo.parent, ginfo.name, ginfo.numa_node,
