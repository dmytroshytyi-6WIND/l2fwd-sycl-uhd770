...
static const struct option_dict {
  ...
  uint8_t *map_idx;
    ...
    {.str = "set-src-mac",
     ...
     .map_idx = &actions_idx},
    {.str = "set-dst-mac",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_SET_MAC_DST),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "set-src-ipv4",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_SET_IPV4_SRC),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "set-dst-ipv4",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_SET_IPV4_DST),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "set-src-ipv6",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_SET_IPV6_SRC),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "set-dst-ipv6",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_SET_IPV6_DST),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "set-src-tp",
     ...
     .map_idx = &actions_idx},
    {.str = "set-dst-tp",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_SET_TP_DST),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "inc-tcp-ack",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_INC_TCP_ACK),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "dec-tcp-ack",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_DEC_TCP_ACK),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "inc-tcp-seq",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_INC_TCP_SEQ),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "dec-tcp-seq",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_DEC_TCP_SEQ),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "set-ttl",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_SET_TTL),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "dec-ttl",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_DEC_TTL),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "set-ipv4-dscp",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_SET_IPV4_DSCP),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "set-ipv6-dscp",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_SET_IPV6_DSCP),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "flag",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_FLAG),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "meter",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_METER),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "vxlan-encap",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
    {.str = "vxlan-decap",
     .mask = FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_VXLAN_DECAP),
     .map = &flow_actions[0],
     .map_idx = &actions_idx},
};
...
static void usage(char *progname) {
  ...
  printf(
      "  --unique-data: flag to set using unique data for all"
...
static void read_meter_policy(char *prog, char *arg) {
  char *token;
  size_t i, j, k;
...
static void args_parse(int argc, char **argv) {
    ...
    case 0:
      ...
      if (strcmp(lgopts[opt_idx].name, "group") == 0) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "flow group should be >= 0\n");
        printf("group %d / ", flow_group);
      }

      for (i = 0; i < RTE_DIM(flow_options); i++)
        if (strcmp(lgopts[opt_idx].name, flow_options[i].str) == 0) {
          flow_options[i].map[(*flow_options[i].map_idx)++] =
              flow_options[i].mask;
          printf("%s / ", flow_options[i].str);
        }

      if (strcmp(lgopts[opt_idx].name, "hairpin-rss") == 0) {
        n = atoi(optarg);
        if (n > 0)
          hairpin_queues_num = n;
        else
          rte_exit(EXIT_FAILURE, "Hairpin queues should be > 0\n");

        flow_actions[actions_idx++] = HAIRPIN_RSS_ACTION;
        printf("hairpin-rss / ");
      }
      if (strcmp(lgopts[opt_idx].name, "hairpin-queue") == 0) {
        n = atoi(optarg);
        if (n > 0)
          hairpin_queues_num = n;
        else
        ...
        flow_actions[actions_idx++] = HAIRPIN_QUEUE_ACTION;
      ...
      if (strcmp(lgopts[opt_idx].name, "raw-encap") == 0) {
        ...
        flow_actions[actions_idx++] =
            FLOW_ITEM_MASK(RTE_FLOW_ACTION_TYPE_RAW_ENCAP);
        ...
        while (token != NULL) {
          for (i = 0; i < RTE_DIM(flow_options); i++) {
            ...
            if (i == (RTE_DIM(flow_options) - 1))
              rte_exit(EXIT_FAILURE, "Invalid encap item: %s\n", token);
          }
          token = strtok(NULL, ",");
        }
        printf(" / ");
      ...
      if (strcmp(lgopts[opt_idx].name, "raw-decap") == 0) {
        printf("raw-decap ");
        flow_actions[actions_idx++] =
            FLOW_ITEM_MASK(RTE_FLOW_ACTION_TYPE_RAW_DECAP);
        ...
        while (token != NULL) {
          for (i = 0; i < RTE_DIM(flow_options); i++) {
            ...
            /* Reached last item with no match */
            if (i == (RTE_DIM(flow_options) - 1))
              rte_exit(EXIT_FAILURE, "Invalid decap item %s\n", token);
          }
          token = strtok(NULL, ",");
        }
        printf(" / ");
      ...
      /* Control */
      if (strcmp(lgopts[opt_idx].name, "rules-batch") == 0) {
      ...
      if (strcmp(lgopts[opt_idx].name, "random-priority") == 0) {
        ...
        if ((optarg[0] == '\0') || (end == NULL))
          rte_exit(EXIT_FAILURE, "Invalid value for random-priority\n");
        max_priority = prio;
        token = end + 1;
      ...
      if (strcmp(lgopts[opt_idx].name, "dump-iterations") == 0)
        dump_iterations = true;
      if (strcmp(lgopts[opt_idx].name, "unique-data") == 0)
        unique_data = true;
      if (strcmp(lgopts[opt_idx].name, "deletion-rate") == 0)
        delete_flag = true;
      if (strcmp(lgopts[opt_idx].name, "dump-socket-mem") == 0)
        dump_socket_mem_flag = true;
      if (strcmp(lgopts[opt_idx].name, "enable-fwd") == 0)
        enable_fwd = true;
      if (strcmp(lgopts[opt_idx].name, "portmask") == 0) {
      ...
      if (strcmp(lgopts[opt_idx].name, "cores") == 0) {
        ...
        if (n <= RTE_MAX_LCORE && n > 0)
          ...
          rte_exit(EXIT_FAILURE, "Error: cores count must be > 0 and < %d\n",
                   RTE_MAX_LCORE);
        }
      }
      ...
      if (strcmp(lgopts[opt_idx].name, "meter-profile") == 0) {
        ...
        while (token != NULL &&
               i < sizeof(meter_profile_values) / sizeof(uint64_t)) {
  ...
  if (rules_count % rules_batch != 0) {
    rte_exit(EXIT_FAILURE, "rules_count %% rules_batch should be 0\n");
  }
...
static size_t dump_socket_mem(FILE *f) {
  ...
  for (i = 0; i < RTE_MAX_NUMA_NODES; i++) {
    ...
    if (dump_socket_mem_flag) {
      ...
      fprintf(f,
              ...
              "\nmax: %.6lf"
              "\ncount alloc: %u\nfree: %u\n",
              i, socket_stats.heap_totalsz_bytes / 1.0e6,
              ...
              socket_stats.greatest_free_size / 1.0e6, socket_stats.alloc_count,
  ...
  if (dump_socket_mem_flag && active_nodes) {
    fprintf(f,
            ...
            "\nalloc: %.6lf(%.3lf%%)\nfree: %.6lf"
            "\ncount alloc: %u\nfree: %u\n",
            total / 1.0e6, alloc / 1.0e6, (double)alloc * 100 / (double)total,
            free / 1.0e6, n_alloc, n_free);
    fprintf(f, "::::::::::::::::::::::::::::::::::::::::\n");
  }
  return alloc;
}

static void print_flow_error(struct rte_flow_error error) {
  printf("Flow can't be created %d message: %s\n", error.type,
         error.message ? error.message : "(no stated reason)");
}

static inline void print_rules_batches(double *cpu_time_per_batch) {
  uint8_t idx;
  double delta;
  double rate;
  ...
  for (idx = 0; idx < MAX_BATCHES_COUNT; idx++) {
    ...
    printf(":: Rules batch #%d: %d rules "
           ...
           idx, rules_batch, cpu_time_per_batch[idx], rate);
  }
}

static inline int has_meter(void) {
  int i;

  for (i = 0; i < MAX_ACTIONS_NUM; i++) {
    if (flow_actions[i] == 0)
      break;
    if (flow_actions[i] & FLOW_ACTION_MASK(RTE_FLOW_ACTION_TYPE_METER))
      return 1;
  }
  return 0;
}

static void create_meter_policy(void) {
  ...
  for (port_id = 0; port_id < nr_ports; port_id++) {
    for (i = 0; i < RTE_COLORS; i++)
      fill_actions(actions[i], all_actions[i], 0, 0, 0, 0, 0, 0, unique_data,
                   rx_queues_count, dst_ports[port_id]);
    ...
    ret =
        rte_mtr_meter_policy_add(port_id, policy_id[port_id], &policy, &error);
    if (ret) {
...
static void destroy_meter_policy(void) {
  struct rte_mtr_error error;
  uint16_t nr_ports;
  int port_id;
  ...
  for (port_id = 0; port_id < nr_ports; port_id++) {
    ...
    if (rte_mtr_meter_policy_delete(port_id, policy_id[port_id], &error)) {
...
static void create_meter_rule(int port_id, uint32_t counter) {
  int ret;
...
static void destroy_meter_rule(int port_id, uint32_t counter) {
  struct rte_mtr_error error;
...
static void meters_handler(int port_id, uint8_t core_id, uint8_t ops) {
  ...
  for (counter = start_counter; counter < end_counter; counter++) {
    ...
    if (!((counter + 1) % rules_batch)) {
      ...
      cpu_time_per_batch[rules_batch_idx] =
          ((double)(rte_get_timer_cycles() - start_batch)) / rte_get_timer_hz();
  ...
  printf(":: Port %d :: Core %d Meter %s :: start @[%d] - end @[%d],"
         ...
         start_counter, end_counter - 1, cpu_time_used, insertion_rate);

  if (ops == METER_CREATE)
    mc_pool.meters_record.insertion[port_id][core_id] = cpu_time_used;
  else
    mc_pool.meters_record.deletion[port_id][core_id] = cpu_time_used;
}

static void destroy_meter_profile(void) {
  struct rte_mtr_error error;
  uint16_t nr_ports;
  int port_id;
  ...
  for (port_id = 0; port_id < nr_ports; port_id++) {
    ...
    if (rte_mtr_meter_profile_delete(port_id, DEFAULT_METER_PROF_ID, &error)) {
...
static void create_meter_profile(void) {
  uint16_t nr_ports;
  int ret, port_id;
  ...
  for (port_id = 0; port_id < nr_ports; port_id++) {
    ...
    mp.srtcm_rfc2697.cir =
        meter_profile_values[0] ? meter_profile_values[0] : METER_CIR;
    mp.srtcm_rfc2697.cbs =
        meter_profile_values[1] ? meter_profile_values[1] : METER_CIR / 8;
    mp.srtcm_rfc2697.ebs = meter_profile_values[2];
    mp.packet_mode = packet_mode;
    ret =
        rte_mtr_meter_profile_add(port_id, DEFAULT_METER_PROF_ID, &mp, &error);
    if (ret != 0) {
...
static inline void destroy_flows(int port_id, uint8_t core_id,
  ...
  printf(
      ":: Port %d :: Core %d :: The time for deleting %d rules is %f seconds\n",
      port_id, core_id, rules_count_per_core, cpu_time_used);

  mc_pool.flows_record.deletion[port_id][core_id] = cpu_time_used;
}

static struct rte_flow **insert_flows(int port_id, uint8_t core_id,
  ...
  flows_list = rte_zmalloc(
      "flows_list", (sizeof(struct rte_flow *) * rules_count_per_core) + 1, 0);
  if (flows_list == NULL)
  ...
  for (counter = start_counter; counter < end_counter; counter++) {
    flow =
        generate_flow(port_id, flow_group, flow_attrs, flow_items, flow_actions,
  ...
  printf(
      ":: Port %d :: Core %d :: The time for creating %d in rules %f seconds\n",
      port_id, core_id, rules_count_per_core, cpu_time_used);
...
static void flows_handler(uint8_t core_id) {
  struct rte_flow **flows_list;
  uint16_t port_idx = 0;
  uint16_t nr_ports;
  int port_id;
...
static void dump_used_cpu_time(const char *item, uint16_t port,
                               struct used_cpu_time *used_time) {
  uint32_t i;
  ...
  insertion_latency =
      ((double)(mc_pool.rules_count / insertion_latency_time) / 1000);
  deletion_latency =
      ((double)(mc_pool.rules_count / deletion_latency_time) / 1000);

  insertion_throughput_time /= mc_pool.cores_count;
  deletion_throughput_time /= mc_pool.cores_count;
  insertion_throughput =
      ((double)(mc_pool.rules_count / insertion_throughput_time) / 1000);
  deletion_throughput =
      ((double)(mc_pool.rules_count / deletion_throughput_time) / 1000);
...
static void dump_used_mem(uint16_t port) {
  uint32_t i;
...
static int run_rte_flow_handler_cores(void *data __rte_unused) {
  uint16_t port;
  int lcore_counter = 0;
  ...
  RTE_ETH_FOREACH_DEV(port) {
    ...
    if (!((ports_mask >> port) & 0x1))
      continue;
    if (has_meter())
      dump_used_cpu_time("Meters:", port, &mc_pool.meters_record);
    dump_used_cpu_time("Flows:", port, &mc_pool.flows_record);
    dump_used_mem(port);
  }
...
static inline uint16_t do_rx(struct lcore_info *li, uint16_t rx_port,
                             uint16_t rx_queue) {
  uint16_t cnt = 0;
...
static inline void do_tx(struct lcore_info *li, uint16_t cnt, uint16_t tx_port,
                         uint16_t tx_queue) {
  uint16_t nr_tx = 0;
  uint16_t i;
 ...
 */
static char *pretty_number(uint64_t n, char *buf) {
  char p[6][4];
  int i = 0;
  int off = 0;
...
static void packet_per_second_stats(void) {
  struct lcore_info *old;
  struct lcore_info *li, *oli;
  int nr_lines = 0;
  int i;

  old = rte_zmalloc("old", sizeof(struct lcore_info) * RTE_MAX_LCORE, 0);
  if (old == NULL)
    rte_exit(EXIT_FAILURE, "No Memory available!\n");

  memcpy(old, lcore_infos, sizeof(struct lcore_info) * RTE_MAX_LCORE);
  ...
  while (!force_quit) {
    ...
    for (i = 0; i < RTE_MAX_LCORE; i++) {
      ...
      printf("%6d %16s %16s %16s\n", i, pretty_number(tx_delta, buf[0]),
...
static int start_forwarding(void *data __rte_unused) {
  ...
  while (!force_quit)
    for (stream_id = 0; stream_id < MAX_STREAMS; stream_id++) {
      ...
      cnt = do_rx(li, li->streams[stream_id].rx_port,
                  li->streams[stream_id].rx_queue);
      if (cnt)
        do_tx(li, cnt, li->streams[stream_id].tx_port,
...
static void init_port(void) {
  ...
  struct rte_eth_conf port_conf = {.rx_adv_conf = {
  ...
  mbuf_mp =
      rte_pktmbuf_pool_create("mbuf_pool", total_mbuf_num, mbuf_cache_size, 0,
  ...
  for (port_id = 0; port_id < nr_ports; port_id++) {
    ...
    if (ret == 0) {
      ...
      rte_exit(EXIT_FAILURE,
               "Error when negotiating Rx meta features on port=%u: %s\n",
    ...
    if (hairpin_queues_num != 0) {
      ...
      for (hairpin_queue = rx_queues_count, std_queue = 0;
           hairpin_queue < nr_queues; hairpin_queue++, std_queue++) {
        hairpin_conf.peers[0].port = port_id;
        hairpin_conf.peers[0].queue = std_queue + tx_queues_count;
        ret = rte_eth_rx_hairpin_queue_setup(port_id, hairpin_queue, rxd_count,
                                             &hairpin_conf);
        if (ret != 0)
          rte_exit(EXIT_FAILURE,
      ...
      for (hairpin_queue = tx_queues_count, std_queue = 0;
           hairpin_queue < nr_queues; hairpin_queue++, std_queue++) {
        hairpin_conf.peers[0].port = port_id;
        hairpin_conf.peers[0].queue = std_queue + rx_queues_count;
        ret = rte_eth_tx_hairpin_queue_setup(port_id, hairpin_queue, txd_count,
                                             &hairpin_conf);
        if (ret != 0)
          rte_exit(EXIT_FAILURE,
