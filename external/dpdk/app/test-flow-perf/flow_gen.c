...
static void fill_attributes(struct rte_flow_attr *attr, uint64_t *flow_attrs,
                            uint16_t group, uint8_t max_priority) {
  uint8_t i;
...
struct rte_flow *
generate_flow(uint16_t port_id, uint16_t group, uint64_t *flow_attrs,
  ...
  fill_actions(actions, flow_actions, outer_ip_src, next_table, hairpinq,
               encap_data, decap_data, core_idx, unique_data, rx_queues_count,
               dst_port);
