...
static int test_hash_readwrite_worker(__rte_unused void *arg) {
  uint64_t i, offset;
  uint32_t lcore_id = rte_lcore_id();
  uint64_t begin, cycles;
  int *ret;

  ret = rte_malloc(NULL, sizeof(int) * tbl_rw_test_param.num_insert, 0);
  ...
  printf("Core #%d inserting and reading %d: %'" PRId64 " - %'" PRId64 "\n",
         ...
         offset + tbl_rw_test_param.num_insert - 1);

  begin = rte_rdtsc_precise();

  for (i = offset; i < offset + tbl_rw_test_param.num_insert; i++) {

    if (rte_hash_lookup(tbl_rw_test_param.h, tbl_rw_test_param.keys + i) > 0)
      break;

    ret[i - offset] =
        rte_hash_add_key(tbl_rw_test_param.h, tbl_rw_test_param.keys + i);
    if (ret[i - offset] < 0)
      break;
    ...
    if (rte_hash_lookup(tbl_rw_test_param.h, tbl_rw_test_param.keys + rand) !=
        ret[rand])
      break;

    if (rte_hash_del_key(tbl_rw_test_param.h, tbl_rw_test_param.keys + rand) !=
        ret[rand])
      break;

    ret[rand] =
        rte_hash_add_key(tbl_rw_test_param.h, tbl_rw_test_param.keys + rand);
    if (ret[rand] < 0)
      break;
...
static int init_params(int use_ext, int use_htm, int rw_lf, int use_jhash) {
  unsigned int i;
...
static int test_hash_readwrite_functional(int use_htm, int use_rw_lf,
                                          int use_ext) {
  unsigned int i;
  const void *next_key;
  void *next_data;
  ...
  tbl_rw_test_param.num_insert = tot_insert / worker_cnt;
  ...
  /* Fire all threads. */
  rte_eal_mp_remote_launch(test_hash_readwrite_worker, NULL, SKIP_MAIN);
  rte_eal_mp_wait_lcore();

  while (rte_hash_iterate(tbl_rw_test_param.h, &next_key, &next_data, &iter) >=
         0) {
...
static int test_rw_reader(void *arg) {
  uint64_t i;
  uint64_t begin, cycles;
  ...
  for (i = 0; i < read_cnt; i++) {
    void *data = arg;
    rte_hash_lookup_data(tbl_rw_test_param.h, tbl_rw_test_param.keys + i,
                         &data);
    if (i != (uint64_t)(uintptr_t)data) {
      printf("lookup find wrong value %" PRIu64 ","
             "%" PRIu64 "\n",
             i, (uint64_t)(uintptr_t)data);
      break;
    }
  }
...
static int test_rw_writer(void *arg) {
  uint64_t i;
  uint32_t lcore_id = rte_lcore_id();
  uint64_t begin, cycles;
  int ret;
  ...
  offset =
      TOTAL_INSERT / 2 + (i - (start_coreid)) * tbl_rw_test_param.num_insert;
  begin = rte_rdtsc_precise();
  for (i = offset; i < offset + tbl_rw_test_param.num_insert; i++) {
    ret = rte_hash_add_key_data(tbl_rw_test_param.h, tbl_rw_test_param.keys + i,
                                (void *)((uintptr_t)i));
    if (ret < 0) {
  ...
  __atomic_fetch_add(&gwrites, tbl_rw_test_param.num_insert, __ATOMIC_RELAXED);
  return 0;
}

static int test_hash_readwrite_perf(struct perf *perf_results, int use_htm,
                                    int reader_faster) {
  unsigned int n;
  int ret;
  int start_coreid;
  ...
  for (n = 0; n < NUM_TEST; n++) {
    ...
    tbl_rw_test_param.rounded_tot_insert =
        TOTAL_INSERT / 2 + tbl_rw_test_param.num_insert * core_cnt[n];

    for (i = 0; i < TOTAL_INSERT / 2; i++) {
      ret =
          rte_hash_add_key_data(tbl_rw_test_param.h, tbl_rw_test_param.keys + i,
                                (void *)((uintptr_t)i));
      if (ret < 0) {
        printf("Failed to insert half of keys\n");
        goto err_free;
    ...
    for (i = 0; i < core_cnt[n]; i++)
      rte_eal_remote_launch(test_rw_reader, (void *)(uintptr_t)read_cnt,
                            worker_core_ids[i]);
    ...
    for (; i < core_cnt[n] * 2; i++)
      rte_eal_remote_launch(test_rw_writer, (void *)((uintptr_t)start_coreid),
                            worker_core_ids[i]);
    ...
    while (rte_hash_iterate(tbl_rw_test_param.h, &next_key, &next_data,
                            &iter) >= 0) {
      /* Search for the key in the list of keys added .*/
      i = *(const uint32_t *)next_key;
      tbl_rw_test_param.found[i]++;
...
static int test_hash_rw_perf_main(void) {
  /*
  ...
  if (rte_lcore_count() < 3) {
    printf(
        "Not enough cores for hash_readwrite_autotest, expecting at least 3\n");
    return TEST_SKIPPED;
  }
  ...
  if (rte_tm_supported()) {
    ...
    reader_faster = 1;
    if (test_hash_readwrite_perf(&htm_results, use_htm, reader_faster) < 0)
      return -1;
  ...
  reader_faster = 1;
  if (test_hash_readwrite_perf(&non_htm_results, use_htm, reader_faster) < 0)
    return -1;
  reader_faster = 0;
  if (test_hash_readwrite_perf(&non_htm_results, use_htm, reader_faster) < 0)
    return -1;
...
static int test_hash_rw_func_main(void) {
  /*
   * Variables used to choose different tests.
   * use_htm indicates if hardware transactional memory should be used.
   * reader_faster indicates if the reader threads should finish earlier
