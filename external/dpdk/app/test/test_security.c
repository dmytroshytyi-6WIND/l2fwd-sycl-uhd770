 ...
 */
#define MOCK_TEST_ASSERT(fail_counter, cond, msg, ...)                         \
  do {                                                                         \
    if (!(cond)) {                                                             \
      fail_counter++;                                                          \
      RTE_LOG(DEBUG, EAL, "Test assert %s line %d failed: " msg "\n",          \
 ...
 */
#define TEST_ASSERT_MOCK_CALLS(mock_data, exp_calls)                           \
  do {                                                                         \
...
#define TEST_ASSERT_MOCK_FUNCTION_CALL_RET(f_name, f_ret, exp_ret, fmt)        \
  TEST_ASSERT_EQUAL(exp_ret, f_ret,                                            \
                    "Expecting " RTE_STR(f_name) " to return " fmt             \
                                                 ", but it returned " fmt      \
                                                 "\n",                         \
                    exp_ret, f_ret)
...
#define TEST_ASSERT_MOCK_FUNCTION_CALL_NOT_NULL(f_name, f_ret)                 \
  TEST_ASSERT_NOT_NULL(f_ret,                                                  \
                       "Expecting " RTE_STR(f_name) " to return not NULL\n")
 ...
 */
#define TEST_ASSERT_SESSION_COUNT(expected_sessions_count)                     \
  do {                                                                         \
 ...
 */
#define TEST_ASSERT_MEMPOOL_USAGE(expected_mempool_usage)                      \
  do {                                                                         \
 ...
 */
#define TEST_ASSERT_PRIV_MP_USAGE(expected_priv_mp_usage)                      \
  do {                                                                         \
    struct security_testsuite_params *ts_params = &testsuite_params;           \
    unsigned int priv_mp_usage;                                                \
    priv_mp_usage = rte_mempool_in_use_count(ts_params->session_priv_mpool);   \
...
static int mock_session_create(void *device,
                               struct rte_security_session_conf *conf,
                               struct rte_security_session *sess,
                               struct rte_mempool *priv_mp) {
  void *sess_priv;
  ...
  if (mock_session_create_exp.ret == 0) {
    ...
    TEST_ASSERT_EQUAL(0, ret, "priv mempool does not have enough objects");
...
static int mock_session_update(void *device, struct rte_security_session *sess,
                               struct rte_security_session_conf *conf) {
...
static unsigned int mock_session_get_size(void *device) {
...
static int mock_session_stats_get(void *device,
                                  struct rte_security_session *sess,
                                  struct rte_security_stats *stats) {
...
static int mock_session_destroy(void *device,
                                struct rte_security_session *sess) {
...
static int mock_set_pkt_metadata(void *device,
                                 struct rte_security_session *sess,
...
static int mock_get_userdata(void *device, uint64_t md, void **userdata) {
  ...
  MOCK_TEST_ASSERT_NOT_NULL(
      mock_get_userdata_exp.failed, userdata,
      "Expecting parameter userdata not to be NULL but it's %p", userdata);
...
static int testsuite_setup(void) {
  ...
  ts_params->session_mpool =
      rte_mempool_create(SECURITY_TEST_MEMPOOL_NAME, SECURITY_TEST_MEMPOOL_SIZE,
                         SECURITY_TEST_SESSION_OBJ_SZ, 0, 0, NULL, NULL, NULL,
                         NULL, SOCKET_ID_ANY, 0);
  TEST_ASSERT_NOT_NULL(ts_params->session_mpool, "Cannot create mempool %s\n",
                       rte_strerror(rte_errno));

  ts_params->session_priv_mpool = rte_mempool_create(
      SECURITY_TEST_PRIV_MEMPOOL_NAME, SECURITY_TEST_MEMPOOL_SIZE,
      SECURITY_TEST_SESSION_PRIV_OBJ_SZ, 0, 0, NULL, NULL, NULL, NULL,
      SOCKET_ID_ANY, 0);
  if (ts_params->session_priv_mpool == NULL) {
    RTE_LOG(ERR, USER1,
            "TestCase %s() line %d failed (null): "
 ...
 */
static int destroy_session_with_check(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  if (ut_params->sess != NULL) {
    ...
    TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_destroy, ret, 0,
                                       "%d");
 ...
 */
static void ut_teardown(void) { destroy_session_with_check(); }
 ...
 */
static int ut_setup_with_session(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct rte_security_session *sess;
 ...
 */
static int test_session_create_inv_context(void) {
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_session *sess;
  ...
  sess = rte_security_session_create(NULL, &ut_params->conf,
                                     ts_params->session_mpool,
                                     ts_params->session_priv_mpool);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_create, sess, NULL,
                                     "%p");
 ...
 */
static int test_session_create_inv_context_ops(void) {
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_session *sess;
 ...
 * security operations
 */
static int test_session_create_inv_context_ops_fun(void) {
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_session *sess;
 ...
 */
static int test_session_create_inv_configuration(void) {
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_session *sess;
 ...
 * mempool
 */
static int test_session_create_inv_mempool(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct rte_security_session *sess;
 ...
 * priv mempool
 */
static int test_session_create_inv_sess_priv_mempool(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct rte_security_session *sess;
 ...
 */
static int test_session_create_mempool_empty(void) {
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct security_unittest_params *ut_params = &unittest_params;
  ...
  for (i = 0; i < SECURITY_TEST_MEMPOOL_SIZE; ++i) {
    ...
    TEST_ASSERT_EQUAL(0, ret,
                      ...
                      " to succeed",
                      i);
    ret = rte_mempool_get(ts_params->session_priv_mpool, (void **)(&tmp1[i]));
    TEST_ASSERT_EQUAL(0, ret,
  ...
  for (i = 0; i < SECURITY_TEST_MEMPOOL_SIZE; ++i) {
    ...
    rte_mempool_put(ts_params->session_priv_mpool, (tmp1[i]));
  }
  TEST_ASSERT_MEMPOOL_USAGE(0);
  TEST_ASSERT_PRIV_MP_USAGE(0);
 ...
 */
static int test_session_create_ops_failure(void) {
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_session *sess;
 ...
 */
static int test_session_create_success(void) {
  struct security_testsuite_params *ts_params = &testsuite_params;
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_session *sess;
 ...
 */
static int test_session_update_inv_context(void) {
  struct security_unittest_params *ut_params = &unittest_params;

  int ret =
      rte_security_session_update(NULL, ut_params->sess, &ut_params->conf);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_update, ret, -EINVAL,
                                     "%d");
 ...
 * security operations structure (NULL)
 */
static int test_session_update_inv_context_ops(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ut_params->ctx.ops = NULL;
 ...
 * security operations
 */
static int test_session_update_inv_context_ops_fun(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ut_params->ctx.ops = &empty_ops;

  int ret = rte_security_session_update(&ut_params->ctx, ut_params->sess,
                                        &ut_params->conf);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_update, ret, -ENOTSUP,
                                     "%d");
  TEST_ASSERT_MOCK_CALLS(mock_session_update_exp, 0);
 ...
 */
static int test_session_update_inv_configuration(void) {
  struct security_unittest_params *ut_params = &unittest_params;

  int ret = rte_security_session_update(&ut_params->ctx, ut_params->sess, NULL);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_update, ret, -EINVAL,
                                     "%d");
  TEST_ASSERT_MOCK_CALLS(mock_session_update_exp, 0);
 ...
 */
static int test_session_update_inv_session(void) {
  struct security_unittest_params *ut_params = &unittest_params;

  int ret =
      rte_security_session_update(&ut_params->ctx, NULL, &ut_params->conf);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_update, ret, -EINVAL,
                                     "%d");
  TEST_ASSERT_MOCK_CALLS(mock_session_update_exp, 0);
 ...
 * security operation fails
 */
static int test_session_update_ops_failure(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ...
  int ret = rte_security_session_update(&ut_params->ctx, ut_params->sess,
                                        &ut_params->conf);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_update, ret, -1,
                                     "%d");
 ...
 */
static int test_session_update_success(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ...
  int ret = rte_security_session_update(&ut_params->ctx, ut_params->sess,
                                        &ut_params->conf);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_update, ret, 0, "%d");
  TEST_ASSERT_MOCK_CALLS(mock_session_update_exp, 1);
 ...
 */
static int test_session_get_size_inv_context(void) {
  unsigned int ret = rte_security_session_get_size(NULL);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_get_size, ret, 0,
                                     "%u");
 ...
 * security operations structure (NULL)
 */
static int test_session_get_size_inv_context_ops(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ut_params->ctx.ops = NULL;
 ...
 * security operations
 */
static int test_session_get_size_inv_context_ops_fun(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ut_params->ctx.ops = &empty_ops;
 ...
 * security operation fails
 */
static int test_session_get_size_ops_failure(void) {
  struct security_unittest_params *ut_params = &unittest_params;
 ...
 */
static int test_session_get_size_success(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ...
  unsigned int ret = rte_security_session_get_size(&ut_params->ctx);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_get_size, ret, 1024U,
                                     "%u");
  TEST_ASSERT_MOCK_CALLS(mock_session_get_size_exp, 1);
 ...
 */
static int test_session_stats_get_inv_context(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_stats stats;

  int ret = rte_security_session_stats_get(NULL, ut_params->sess, &stats);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_stats_get, ret,
                                     -EINVAL, "%d");
 ...
 * security operations structure (NULL)
 */
static int test_session_stats_get_inv_context_ops(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_stats stats;
  ut_params->ctx.ops = NULL;

  int ret =
      rte_security_session_stats_get(&ut_params->ctx, ut_params->sess, &stats);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_stats_get, ret,
                                     -EINVAL, "%d");
  TEST_ASSERT_MOCK_CALLS(mock_session_stats_get_exp, 0);
 ...
 * security operations
 */
static int test_session_stats_get_inv_context_ops_fun(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_stats stats;
  ut_params->ctx.ops = &empty_ops;
 ...
 */
static int test_session_stats_get_inv_stats(void) {
  struct security_unittest_params *ut_params = &unittest_params;

  int ret =
      rte_security_session_stats_get(&ut_params->ctx, ut_params->sess, NULL);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_stats_get, ret,
                                     -EINVAL, "%d");
  TEST_ASSERT_MOCK_CALLS(mock_session_stats_get_exp, 0);
 ...
 * security operation fails
 */
static int test_session_stats_get_ops_failure(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_stats stats;
  ...
  int ret =
      rte_security_session_stats_get(&ut_params->ctx, ut_params->sess, &stats);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_stats_get, ret, -1,
                                     "%d");
 ...
 * path
 */
static int test_session_stats_get_success(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_stats stats;
  ...
  int ret =
      rte_security_session_stats_get(&ut_params->ctx, ut_params->sess, &stats);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_stats_get, ret, 0,
                                     "%d");
  TEST_ASSERT_MOCK_CALLS(mock_session_stats_get_exp, 1);
 ...
 */
static int test_session_destroy_inv_context(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ...
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_destroy, ret, -EINVAL,
                                     "%d");
 ...
 * security operations structure (NULL)
 */
static int test_session_destroy_inv_context_ops(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ut_params->ctx.ops = NULL;
 ...
 * security operations
 */
static int test_session_destroy_inv_context_ops_fun(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ut_params->ctx.ops = &empty_ops;
  ...
  int ret = rte_security_session_destroy(&ut_params->ctx, ut_params->sess);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_destroy, ret,
                                     -ENOTSUP, "%d");
  TEST_ASSERT_MOCK_CALLS(mock_session_destroy_exp, 0);
  TEST_ASSERT_MEMPOOL_USAGE(1);
  TEST_ASSERT_PRIV_MP_USAGE(1);
 ...
 */
static int test_session_destroy_inv_session(void) {
  struct security_unittest_params *ut_params = &unittest_params;
 ...
 * security operation fails
 */
static int test_session_destroy_ops_failure(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ...
  int ret = rte_security_session_destroy(&ut_params->ctx, ut_params->sess);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_session_destroy, ret, -1,
                                     "%d");
  TEST_ASSERT_MOCK_CALLS(mock_session_destroy_exp, 1);
  TEST_ASSERT_MEMPOOL_USAGE(1);
  TEST_ASSERT_PRIV_MP_USAGE(1);
 ...
 */
static int test_session_destroy_success(void) {
  struct security_unittest_params *ut_params = &unittest_params;
 ...
 */
static int test_set_pkt_metadata_inv_context(void) {
#ifdef RTE_DEBUG
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_mbuf m;
  int params;

  int ret = rte_security_set_pkt_metadata(NULL, ut_params->sess, &m, &params);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_set_pkt_metadata, ret,
                                     -EINVAL, "%d");
 ...
 * security operations structure (NULL)
 */
static int test_set_pkt_metadata_inv_context_ops(void) {
#ifdef RTE_DEBUG
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_mbuf m;
  int params;
  ...
  int ret = rte_security_set_pkt_metadata(&ut_params->ctx, ut_params->sess, &m,
                                          &params);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_set_pkt_metadata, ret,
                                     -EINVAL, "%d");
  TEST_ASSERT_MOCK_CALLS(mock_set_pkt_metadata_exp, 0);
 ...
 * security operations
 */
static int test_set_pkt_metadata_inv_context_ops_fun(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_mbuf m;
  int params;
  ut_params->ctx.ops = &empty_ops;
 ...
 */
static int test_set_pkt_metadata_inv_session(void) {
#ifdef RTE_DEBUG
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_mbuf m;
  int params;

  int ret = rte_security_set_pkt_metadata(&ut_params->ctx, NULL, &m, &params);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_set_pkt_metadata, ret,
                                     -EINVAL, "%d");
  TEST_ASSERT_MOCK_CALLS(mock_set_pkt_metadata_exp, 0);
 ...
 * security operation fails
 */
static int test_set_pkt_metadata_ops_failure(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_mbuf m;
  int params;
  ...
  int ret = rte_security_set_pkt_metadata(&ut_params->ctx, ut_params->sess, &m,
                                          &params);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_set_pkt_metadata, ret, -1,
                                     "%d");
 ...
 */
static int test_set_pkt_metadata_success(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_mbuf m;
  int params;
  ...
  int ret = rte_security_set_pkt_metadata(&ut_params->ctx, ut_params->sess, &m,
                                          &params);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_set_pkt_metadata, ret, 0,
                                     "%d");
  TEST_ASSERT_MOCK_CALLS(mock_set_pkt_metadata_exp, 1);
 ...
 */
static int test_get_userdata_inv_context(void) {
#ifdef RTE_DEBUG
  uint64_t md = 0xDEADBEEF;

  void *ret = rte_security_get_userdata(NULL, md);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_get_userdata, ret, NULL,
                                     "%p");
 ...
 * security operations structure (NULL)
 */
static int test_get_userdata_inv_context_ops(void) {
#ifdef RTE_DEBUG
  struct security_unittest_params *ut_params = &unittest_params;
  uint64_t md = 0xDEADBEEF;
  ut_params->ctx.ops = NULL;
 ...
 * security operations
 */
static int test_get_userdata_inv_context_ops_fun(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  uint64_t md = 0xDEADBEEF;
  ut_params->ctx.ops = &empty_ops;
 ...
 * security operation fails
 */
static int test_get_userdata_ops_failure(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  uint64_t md = 0xDEADBEEF;
 ...
 */
static int test_get_userdata_success(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  uint64_t md = 0xDEADBEEF;
  void *userdata = (void *)0x7E577E57;
  ...
  void *ret = rte_security_get_userdata(&ut_params->ctx, md);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_get_userdata, ret, userdata,
                                     "%p");
  TEST_ASSERT_MOCK_CALLS(mock_get_userdata_exp, 1);
 ...
 */
static int test_capabilities_get_inv_context(void) {
  const struct rte_security_capability *ret;
  ret = rte_security_capabilities_get(NULL);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_capabilities_get, ret, NULL,
                                     "%p");
 ...
 * security operations structure (NULL)
 */
static int test_capabilities_get_inv_context_ops(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ut_params->ctx.ops = NULL;
 ...
 * security operations
 */
static int test_capabilities_get_inv_context_ops_fun(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ut_params->ctx.ops = &empty_ops;
 ...
 * security operation fails
 */
static int test_capabilities_get_ops_failure(void) {
  struct security_unittest_params *ut_params = &unittest_params;
 ...
 */
static int test_capabilities_get_success(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  ...
  const struct rte_security_capability *ret;
  ret = rte_security_capabilities_get(&ut_params->ctx);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_capabilities_get, ret,
                                     &capabilities, "%p");
  TEST_ASSERT_MOCK_CALLS(mock_capabilities_get_exp, 1);
 ...
 */
static int test_capability_get_inv_context(void) {
  struct rte_security_capability_idx idx;

  const struct rte_security_capability *ret;
  ret = rte_security_capability_get(NULL, &idx);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_capability_get, ret, NULL,
                                     "%p");
  TEST_ASSERT_MOCK_CALLS(mock_capabilities_get_exp, 0);
 ...
 * security operations structure (NULL)
 */
static int test_capability_get_inv_context_ops(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx;
  ut_params->ctx.ops = NULL;
 ...
 * security operations
 */
static int test_capability_get_inv_context_ops_fun(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx;
  ut_params->ctx.ops = &empty_ops;
 ...
 */
static int test_capability_get_inv_idx(void) {
  struct security_unittest_params *ut_params = &unittest_params;
 ...
 * security operation fails
 */
static int test_capability_get_ops_failure(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx;
 ...
 */
static int test_capability_get_empty_table(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx;
  struct rte_security_capability capabilities[] = {
      {
 ...
 */
static int test_capability_get_no_matching_action(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
 ...
 */
static int test_capability_get_no_matching_protocol(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
 ...
 */
static int test_capability_get_no_support_for_macsec(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
      .protocol = RTE_SECURITY_PROTOCOL_MACSEC,
 ...
 */
static int test_capability_get_ipsec_mismatch_proto(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
      .protocol = RTE_SECURITY_PROTOCOL_IPSEC,
 ...
 */
static int test_capability_get_ipsec_mismatch_mode(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
      .protocol = RTE_SECURITY_PROTOCOL_IPSEC,
 ...
 */
static int test_capability_get_ipsec_mismatch_dir(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
      .protocol = RTE_SECURITY_PROTOCOL_IPSEC,
 ...
 */
static int test_capability_get_ipsec_match(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
      .protocol = RTE_SECURITY_PROTOCOL_IPSEC,
  ...
  const struct rte_security_capability *ret;
  ret = rte_security_capability_get(&ut_params->ctx, &idx);
  TEST_ASSERT_MOCK_FUNCTION_CALL_RET(rte_security_capability_get, ret,
                                     &capabilities[1], "%p");
  TEST_ASSERT_MOCK_CALLS(mock_capabilities_get_exp, 1);
 ...
 */
static int test_capability_get_pdcp_mismatch_domain(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
      .protocol = RTE_SECURITY_PROTOCOL_PDCP,
 ...
 */
static int test_capability_get_pdcp_match(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
      .protocol = RTE_SECURITY_PROTOCOL_PDCP,
 ...
 */
static int test_capability_get_docsis_mismatch_direction(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
      .protocol = RTE_SECURITY_PROTOCOL_DOCSIS,
      .docsis = {.direction = RTE_SECURITY_DOCSIS_DOWNLINK},
  };
  struct rte_security_capability capabilities[] = {
      {
          .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
          .protocol = RTE_SECURITY_PROTOCOL_DOCSIS,
          .docsis = {.direction = RTE_SECURITY_DOCSIS_UPLINK},
      },
      {
          .action = RTE_SECURITY_ACTION_TYPE_NONE,
      },
 ...
 */
static int test_capability_get_docsis_match(void) {
  struct security_unittest_params *ut_params = &unittest_params;
  struct rte_security_capability_idx idx = {
      .action = RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
      .protocol = RTE_SECURITY_PROTOCOL_DOCSIS,
...
static struct unit_test_suite security_testsuite = {
    ...
    .unit_test_cases = {
        TEST_CASE_ST(ut_setup, ut_teardown, test_session_create_inv_context),
        ...
        TEST_CASE_ST(ut_setup, ut_teardown, test_session_create_inv_mempool),
        TEST_CASE_ST(ut_setup, ut_teardown,
                     test_session_create_inv_sess_priv_mempool),
        TEST_CASE_ST(ut_setup, ut_teardown, test_session_create_mempool_empty),
        TEST_CASE_ST(ut_setup, ut_teardown, test_session_create_ops_failure),
