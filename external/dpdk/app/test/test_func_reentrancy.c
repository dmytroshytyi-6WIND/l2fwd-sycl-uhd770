 ...
 */
static int test_eal_init_once(__rte_unused void *arg) {
  unsigned lcore_self = rte_lcore_id();

  WAIT_SYNCHRO_FOR_WORKERS();

  __atomic_store_n(&obj_count, 1,
                   __ATOMIC_RELAXED); /* silent the check in the caller */
 ...
 */
static void ring_clean(unsigned int lcore_id) {
  struct rte_ring *rp;
  ...
  for (i = 0; i < MAX_ITER_MULTI; i++) {
    snprintf(ring_name, sizeof(ring_name), "fr_test_%d_%d", lcore_id, i);
    rp = rte_ring_lookup(ring_name);
    rte_ring_free(rp);
  }
}

static int ring_create_lookup(__rte_unused void *arg) {
  unsigned lcore_self = rte_lcore_id();
  struct rte_ring *rp;
  char ring_name[MAX_STRING_SIZE];
  int i;
...
static void mempool_clean(unsigned int lcore_id) {
  ...
  for (i = 0; i < MAX_ITER_MULTI; i++) {
    snprintf(mempool_name, sizeof(mempool_name), "fr_test_%d_%d", lcore_id, i);
    mp = rte_mempool_lookup(mempool_name);
    rte_mempool_free(mp);
  }
}

static int mempool_create_lookup(__rte_unused void *arg) {
  unsigned lcore_self = rte_lcore_id();
  struct rte_mempool *mp;
  char mempool_name[MAX_STRING_SIZE];
  int i;
  ...
  for (i = 0; i < MAX_ITER_ONCE; i++) {
    mp = rte_mempool_create("fr_test_once", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE, 0,
                            0, NULL, NULL, my_obj_init, NULL, SOCKET_ID_ANY, 0);
    if (mp != NULL)
      __atomic_fetch_add(&obj_count, 1, __ATOMIC_RELAXED);
  }
  ...
  /* create/lookup new ring several times */
  for (i = 0; i < MAX_ITER_MULTI; i++) {
    snprintf(mempool_name, sizeof(mempool_name), "fr_test_%d_%d", lcore_self,
             i);
    mp = rte_mempool_create(mempool_name, MEMPOOL_SIZE, MEMPOOL_ELT_SIZE, 0, 0,
...
#ifdef RTE_LIB_HASH
static void hash_clean(unsigned lcore_id) {
...
static int hash_create_free(__rte_unused void *arg) {
  unsigned lcore_self = rte_lcore_id();
  struct rte_hash *handle;
  char hash_name[MAX_STRING_SIZE];
  int i;
...
static void fbk_clean(unsigned lcore_id) {
...
static int fbk_create_free(__rte_unused void *arg) {
  unsigned lcore_self = rte_lcore_id();
  struct rte_fbk_hash_table *handle;
  char fbk_name[MAX_STRING_SIZE];
  int i;
...
#ifdef RTE_LIB_LPM
static void lpm_clean(unsigned int lcore_id) {
...
static int lpm_create_free(__rte_unused void *arg) {
  unsigned lcore_self = rte_lcore_id();
  struct rte_lpm *lpm;
...
struct test_case test_cases[] = {
    ...
    {mempool_create_lookup, NULL, mempool_clean, "mempool create/lookup"},
#ifdef RTE_LIB_HASH
 ...
 */
static int launch_test(struct test_case *pt_case) {
 ...
 */
static int test_func_reentrancy(void) {
  uint32_t case_id;
  ...
  if (rte_lcore_count() < 2) {
    printf("Not enough cores for func_reentrancy_autotest, expecting at least "
           "2\n");
    return TEST_SKIPPED;
