...
static int test_add_delete(void) {
  ...
  handle =
      rte_efd_create("test_add_delete", TABLE_SIZE, sizeof(struct flow_key),
  ...
  TEST_ASSERT_SUCCESS(rte_efd_update(handle, test_socket_id, &keys[0], data[0]),
                      "Error inserting the key");
  print_key_info("Add", &keys[0], data[0]);

  TEST_ASSERT_EQUAL(rte_efd_lookup(handle, test_socket_id, &keys[0]), data[0],
                    "failed to find key");

  TEST_ASSERT_SUCCESS(
      rte_efd_delete(handle, test_socket_id, &keys[0], &prev_value),
...
static int test_add_update_delete(void) {
  ...
  TEST_ASSERT_SUCCESS(rte_efd_update(handle, test_socket_id, &keys[1], data[1]),
                      "Error inserting the key");
  print_key_info("Add", &keys[1], data[1]);

  TEST_ASSERT_EQUAL(rte_efd_lookup(handle, test_socket_id, &keys[1]), data[1],
                    "failed to find key");
  print_key_info("Lkp", &keys[1], data[1]);

  data[1] = data[1] + 1;
  TEST_ASSERT_SUCCESS(rte_efd_update(handle, test_socket_id, &keys[1], data[1]),
                      "Error re-inserting the key");
  print_key_info("Add", &keys[1], data[1]);
  ...
  TEST_ASSERT_SUCCESS(
      rte_efd_delete(handle, test_socket_id, &keys[1], &prev_value),
      "failed to delete key");
...
static int test_efd_find_existing(void) {
  ...
  /* Create EFD table. */
  handle =
      rte_efd_create("efd_find_existing", TABLE_SIZE, sizeof(struct flow_key),
                     efd_get_all_sockets_bitmask(), test_socket_id);
  TEST_ASSERT_NOT_NULL(handle, "Error creating the efd table\n");
...
static int test_five_keys(void) {
  ...
  handle = rte_efd_create("test_five_keys", TABLE_SIZE, sizeof(struct flow_key),
                          efd_get_all_sockets_bitmask(), test_socket_id);
  TEST_ASSERT_NOT_NULL(handle, "Error creating the efd table\n");
  ...
  for (i = 0; i < 5; i++) {
    TEST_ASSERT_SUCCESS(
        rte_efd_update(handle, test_socket_id, &keys[i], data[i]),
        "Error inserting the key");
  ...
  rte_efd_lookup_bulk(handle, test_socket_id, 5, (void *)&key_array, result);

  for (i = 0; i < 5; i++) {
    TEST_ASSERT_EQUAL(result[i], data[i],
                      "bulk: failed to find key. Expected %d, got %d", data[i],
                      result[i]);
  ...
  /* Lookup */
  for (i = 0; i < 5; i++) {
    TEST_ASSERT_EQUAL(rte_efd_lookup(handle, test_socket_id, &keys[i]), data[i],
                      "failed to find key");
    print_key_info("Lkp", &keys[i], data[i]);
  }
  ...
  for (i = 0; i < 5; i++) {
    TEST_ASSERT_SUCCESS(
        rte_efd_delete(handle, test_socket_id, &keys[i], &prev_value),
        "failed to delete key");
...
static int test_average_table_utilization(void) {
  ...
  for (j = 0; j < ITERATIONS; j++) {
    handle = rte_efd_create("test_efd", num_rules_in, EFD_TEST_KEY_LEN,
                            efd_get_all_sockets_bitmask(), test_socket_id);
    if (handle == NULL) {
    ...
    for (added_keys = 0; added_keys < num_rules_in; added_keys++) {
      ...
      if (rte_efd_update(handle, test_socket_id, simple_key, val))
        break; /* continue;*/
      if (rte_efd_lookup(handle, test_socket_id, simple_key) != val)
        lost_keys++;
      else
        succeeded++;
    }
  ...
  printf("\nAverage table utilization = %.2f%% (%u/%u)\n",
         ((double)average_keys_added / num_rules_in * 100), average_keys_added,
         num_rules_in);
...
static int test_efd_creation_with_bad_parameters(void) {
  ...
  handle = rte_efd_create("creation_with_bad_parameters_2", TABLE_SIZE,
                          sizeof(struct flow_key),
                          efd_get_all_sockets_bitmask(), 255);
  if (handle != NULL) {
    rte_efd_free(handle);
    printf("Impossible creating EFD table successfully "
           "with invalid socket\n");
  ...
  handle = rte_efd_create("same_name", TABLE_SIZE, sizeof(struct flow_key),
