...
#define M_MATCH_CNT (1 + (M_MATCH_E - M_MATCH_S) / M_MATCH_STEP)
 ...
 *
 */
static int test_member_find_existing(void) {
 ...
 */
static int test_member_create_bad_param(void) {
...
static int test_member_insert(void) {
  ...
  for (i = 0; i < NUM_SAMPLES; i++) {
    ...
    ret_cache = rte_member_add(setsum_cache, &keys[i], test_set[i]);
    ret_vbf = rte_member_add(setsum_vbf, &keys[i], test_set[i]);
    TEST_ASSERT(ret_ht >= 0 && ret_cache >= 0 && ret_vbf >= 0, "insert error");
  }
...
static int test_member_lookup(void) {
  ...
  for (i = 0; i < NUM_SAMPLES; i++) {
    ...
    ret_cache = rte_member_lookup(setsum_cache, &keys[i], &set_cache);
    ...
    TEST_ASSERT(set_ht == test_set[i] && set_cache == test_set[i] &&
                    set_vbf == test_set[i],
  ...
  ret_ht = rte_member_lookup_bulk(setsum_ht, key_array, num_key_ht, set_ids_ht);

  ret_cache = rte_member_lookup_bulk(setsum_cache, key_array, num_key_cache,
                                     set_ids_cache);

  ret_vbf =
      rte_member_lookup_bulk(setsum_vbf, key_array, num_key_vbf, set_ids_vbf);
...
static int test_member_delete(void) {
  ...
  for (i = 0; i < NUM_SAMPLES / 2; i++) {
    ...
    ret_cache = rte_member_delete(setsum_cache, &keys[i], test_set[i]);
    ret_vbf = rte_member_delete(setsum_vbf, &keys[i], test_set[i]);
    /* VBF does not support delete yet, so return error code */
    TEST_ASSERT(ret_ht >= 0 && ret_cache >= 0, "key deletion function error");
    TEST_ASSERT(ret_vbf < 0, "vbf does not support deletion, error");
  }
  ...
  for (i = 0; i < NUM_SAMPLES; i++) {
    ...
    ret_cache = rte_member_lookup(setsum_cache, &keys[i], &set_cache);
    ret_vbf = rte_member_lookup(setsum_vbf, &keys[i], &set_vbf);
    TEST_ASSERT(ret_ht >= 0 && ret_cache >= 0, "key lookup function error");
...
static int test_member_multimatch(void) {
  ...
  for (i = M_MATCH_S; i <= M_MATCH_E; i += M_MATCH_STEP) {
    for (j = 0; j < NUM_SAMPLES; j++) {
      ...
      TEST_ASSERT(ret_ht >= 0 && ret_vbf >= 0 && ret_cache >= 0,
  ...
  for (i = 0; i < NUM_SAMPLES; i++) {
    ret_vbf =
        rte_member_lookup_multi(setsum_vbf, &keys[i], MAX_MATCH, set_ids_vbf);
    ret_ht =
        rte_member_lookup_multi(setsum_ht, &keys[i], MAX_MATCH, set_ids_ht);
    ret_cache = rte_member_lookup_multi(setsum_cache, &keys[i], MAX_MATCH,
                                        set_ids_cache);
    /*
    ...
    for (j = 1; j <= M_MATCH_CNT; j++) {
      TEST_ASSERT(set_ids_ht[j - 1] == j * M_MATCH_STEP - 1 &&
                      set_ids_vbf[j - 1] == j * M_MATCH_STEP - 1,
  ...
  for (i = 0; i < NUM_SAMPLES; i++)
    key_array[i] = &keys[i];
  ret_vbf = rte_member_lookup_multi_bulk(setsum_vbf, &key_array[0], num_key_ht,
                                         MAX_MATCH, match_count_vbf,
                                         (member_set_t *)set_ids_vbf_m);

  ret_ht = rte_member_lookup_multi_bulk(setsum_ht, &key_array[0], num_key_vbf,
                                        MAX_MATCH, match_count_ht,
                                        (member_set_t *)set_ids_ht_m);

  ret_cache = rte_member_lookup_multi_bulk(
      setsum_cache, &key_array[0], num_key_cache, MAX_MATCH, match_count_cache,
  ...
  for (j = 0; j < NUM_SAMPLES; j++) {
    ...
    for (i = 1; i <= M_MATCH_CNT; i++) {
      TEST_ASSERT(set_ids_ht_m[j][i - 1] == i * M_MATCH_STEP - 1,
                  "bulk multimatch lookup HT set value error");
      TEST_ASSERT(set_ids_vbf_m[j][i - 1] == i * M_MATCH_STEP - 1,
...
static void setup_keys_and_data(void) {
  unsigned int i, j;
  int num_duplicates;
  ...
  do {
    ...
    for (i = 0; i < MAX_ENTRIES - 1; i++) {
      if (memcmp(generated_keys[i], generated_keys[i + 1], KEY_SIZE) == 0) {
...
static inline int add_generated_keys(struct rte_member_setsum *setsum,
...
static inline int add_generated_keys_cache(struct rte_member_setsum *setsum,
                                           unsigned int *added_keys) {
  int ret = 0;
...
static int test_member_loadfactor(void) {
  unsigned int j;
