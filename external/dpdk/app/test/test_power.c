...
static int check_function_ptrs(void) {
  ...
  if ((rte_power_freqs == NULL) == not_null_expected) {
    printf("rte_power_freqs should%s be NULL, environment has%s been "
           "initialised\n",
           inject_not_string1, inject_not_string2);
    return -1;
  }
  ...
  if ((rte_power_freq_enable_turbo == NULL) == not_null_expected) {
    printf(
        "rte_power_freq_enable_turbo should%s be NULL, environment has%s been "
        "initialised\n",
        inject_not_string1, inject_not_string2);
    return -1;
  }
  if ((rte_power_freq_disable_turbo == NULL) == not_null_expected) {
    printf(
        "rte_power_freq_disable_turbo should%s be NULL, environment has%s been "
        "initialised\n",
        inject_not_string1, inject_not_string2);
    return -1;
  }
  if ((rte_power_get_capabilities == NULL) == not_null_expected) {
    printf(
        "rte_power_get_capabilities should%s be NULL, environment has%s been "
        "initialised\n",
        inject_not_string1, inject_not_string2);
    return -1;
  }
...
static int test_power(void) {
  ...
  const enum power_management_env envs[] = {PM_ENV_ACPI_CPUFREQ, PM_ENV_KVM_VM,
