...
static int test_bitmap_scan_operations(struct rte_bitmap *bmp) {
...
static int test_bitmap_slab_set_get(struct rte_bitmap *bmp) {
  uint32_t pos = 0;
...
static int test_bitmap_set_get_clear(struct rte_bitmap *bmp) {
  uint64_t val;
  int i;
...
static int test_bitmap_all_clear(void) {
  void *mem;
  uint32_t bmp_size;
