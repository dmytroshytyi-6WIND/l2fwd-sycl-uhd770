...
static int test_stats_bitrate_create(void) {
...
static int test_stats_bitrate_free(void) {
  int ret = 0;
...
static int test_stats_bitrate_reg(void) {
  int ret = 0;

  /* Test to register bit rate without metrics init */
  ret = rte_stats_bitrate_reg(bitrate_data);
  TEST_ASSERT(ret < 0,
              "Test Failed: rte_stats_bitrate_reg succeeded "
              "without metrics init, ret:%d",
              ret);
...
static int test_stats_bitrate_reg_invalidpointer(void) {
  int ret = 0;

  ret = rte_stats_bitrate_reg(NULL);
  TEST_ASSERT(ret < 0,
              "Test Failed: Expected failure < 0 but "
              "got %d",
              ret);
...
static int test_stats_bitrate_calc_invalid_bitrate_data(void) {
  int ret = 0;

  ret = rte_stats_bitrate_calc(NULL, portid);
  TEST_ASSERT(ret < 0,
              "Test Failed: rte_stats_bitrate_calc "
              "ret:%d",
              ret);
 ...
 */
static int test_stats_bitrate_calc_invalid_portid_1(void) {
  int ret = 0;

  ret = rte_stats_bitrate_calc(bitrate_data, 33);
  TEST_ASSERT(ret == -ENODEV,
              "Test Failed: Expected -%d for higher "
              "portid rte_stats_bitrate_calc ret:%d",
              ENODEV, ret);
...
static int test_stats_bitrate_calc_invalid_portid_2(void) {
  int ret = 0;

  ret = rte_stats_bitrate_calc(bitrate_data, -1);
  TEST_ASSERT(ret == -ENODEV,
              "Test Failed: Expected -%d for invalid "
              "portid rte_stats_bitrate_calc ret:%d",
              ENODEV, ret);
...
static int test_stats_bitrate_calc_non_existing_portid(void) {
  int ret = 0;

  ret = rte_stats_bitrate_calc(bitrate_data, 31);
  TEST_ASSERT(ret == -ENODEV,
              "Test Failed: Expected -%d for "
...
static int test_stats_bitrate_calc(void) {
  int ret = 0;

  ret = rte_stats_bitrate_calc(bitrate_data, portid);
  TEST_ASSERT(ret >= 0,
              "Test Failed: Expected >=0 for valid portid "
              "rte_stats_bitrate_calc ret:%d",
              ret);

  return TEST_SUCCESS;
}

static int test_bit_packet_forward(void) {
  int ret;
...
static int test_bit_ring_setup(void) {
...
static void test_bit_ring_free(void) {
  test_ring_free(ring);
  test_vdev_uninit("net_ring_net_ringa");
  rte_memzone_free(rte_memzone_lookup("RTE_METRICS"));
}

static struct unit_test_suite bitratestats_testsuite = {
    ...
    .unit_test_cases = {
         ...
         */
        TEST_CASE_ST(test_bit_packet_forward, NULL, test_stats_bitrate_calc),
        /* TEST CASE 9: Test to do the cleanup w.r.t create */
        TEST_CASE(test_stats_bitrate_free), TEST_CASES_END()}};

static int test_bitratestats(void) {
