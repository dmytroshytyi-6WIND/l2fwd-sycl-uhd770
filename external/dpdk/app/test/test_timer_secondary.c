...
static int timer_secondary_spawn_wait(unsigned int lcore) {
  char coremask[10];
  ...
  const char *prefix = "";
#endif
  char const *argv[] = {prgname, "-c", coremask, "--proc-type=secondary",
                        prefix};
...
static void handle_expired_timer(struct rte_timer *tim) {
...
static int timer_manage_loop(void *arg) {
#define TICK_MSECS 1
  ...
  while (!test_info->exit_flag) {
    ...
    if (diff_tsc > tick_cycles) {
      ...
      rte_timer_alt_manage(test_info->timer_data_id, NULL, 0,
...
int test_timer_secondary(void) {
  ...
  if (proc_type == RTE_PROC_PRIMARY) {
    if (rte_lcore_count() < NUM_LCORES_NEEDED) {
      printf(
          "Not enough cores for test_timer_secondary, expecting at least %u\n",
          NUM_LCORES_NEEDED);
      return TEST_SKIPPED;
    }
    ...
    test_info->tim_mempool = rte_mempool_create(
        "test_timer_mp", NUM_TIMERS, sizeof(struct rte_timer), 0, 0, NULL, NULL,
    ...
    *sec_lcorep = rte_get_next_lcore(*mgr_lcorep, 1, 1);
    ...
    ret = rte_eal_remote_launch(timer_manage_loop, (void *)test_info,
                                *mgr_lcorep);
    ...
    return test_info->expected_count == test_info->expired_count ? TEST_SUCCESS
                                                                 : TEST_FAILED;
    ...
    for (i = 0; i < NUM_TIMERS; i++) {
      ...
      ret = rte_timer_alt_reset(test_info->timer_data_id, tim, ticks, SINGLE,
                                test_info->mgr_lcore, NULL, test_info);
      if (ret < 0)
        return TEST_FAILED;
      ...
      ret = rte_timer_alt_stop(test_info->timer_data_id, tim);
      if (ret == 0) {
        test_info->expected_count--;
        rte_mempool_put(test_info->tim_mempool, (void *)tim);
      }
    }
