...
static const char *rand_type_desc(enum rand_type rand_type) {
  switch (rand_type) {
  case rand_type_64:
...
static __rte_always_inline void test_rand_perf_type(enum rand_type rand_type) {
  uint64_t start;
  uint32_t i;
  uint64_t end;
  uint64_t sum = 0;
...
static int test_rand_perf(void) {
  rte_srand(42);
