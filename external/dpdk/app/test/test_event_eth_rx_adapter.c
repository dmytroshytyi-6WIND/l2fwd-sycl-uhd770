...
static int test_event_eth_rx_adapter_common(void) {
  printf("event_eth_rx_adapter not supported on Windows, skipping test\n");
  return TEST_SKIPPED;
}

static int test_event_eth_rx_intr_adapter_common(void) {
...
static inline int port_init_common(uint16_t port,
  ...
  default_params.rx_rings = RTE_MIN(dev_info.max_rx_queues, MAX_NUM_RX_QUEUE);
...
static inline int port_init_rx_intr(uint16_t port, struct rte_mempool *mp) {
...
static inline int port_init(uint16_t port, struct rte_mempool *mp) {
  static const struct rte_eth_conf port_conf_default = {
      .rxmode =
          {
              .mq_mode = RTE_ETH_MQ_RX_NONE,
...
static int init_port_rx_intr(int num_ports) {
  int retval;
  uint16_t portid;
  int err;

  default_params.mp = rte_pktmbuf_pool_create(
      "packet_pool", NB_MBUFS, MBUF_CACHE_SIZE, MBUF_PRIV_SIZE,
  ...
  RTE_ETH_FOREACH_DEV(portid) {
    ...
    if (err)
      continue;
    if (!(default_params.caps & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT)) {
...
static int init_ports(int num_ports) {
  uint16_t portid;
  int retval;
...
static int testsuite_setup(void) {
  ...
  if (!count) {
    ...
    TEST_ASSERT(err == 0, "Failed to create event_skeleton. err=%d", err);
  ...
  if (!count) {
    ...
    TEST_ASSERT(err == 0, "Failed to create net_null. err=%d", err);
...
static int testsuite_setup_rx_intr(void) {
  int err;
  uint8_t count;
  struct rte_event_dev_info dev_info;
  ...
  err = rte_event_eth_rx_adapter_caps_get(
      TEST_DEV_ID, default_params.rx_intr_port, &default_params.caps);
  TEST_ASSERT(err == 0, "Failed to get adapter cap err %d\n", err);
...
static int adapter_create(void) {
  ...
  err = rte_event_eth_rx_adapter_create(TEST_INST_ID, TEST_DEV_ID, &rx_p_conf);
  TEST_ASSERT(err == 0, "Expected 0 got %d", err);

  return err;
}

static int adapter_create_with_params(void) {
  int err;
  struct rte_event_dev_info dev_info;
  struct rte_event_port_conf rx_p_conf;
  ...
  err = rte_event_eth_rx_adapter_create_with_params(TEST_INST_ID, TEST_DEV_ID,
                                                    &rx_p_conf, &rxa_params);
...
static int adapter_queue_event_buf_test(void) {
  int err;
  struct rte_event ev;
  uint32_t cap;
  ...
  if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_OVERRIDE_FLOW_ID) {
    ev.flow_id = 1;
    queue_config.rx_queue_flags = RTE_EVENT_ETH_RX_ADAPTER_QUEUE_FLOW_ID_VALID;
  }
  queue_config.ev = ev;
  queue_config.servicing_weight = 1;
  queue_config.event_buf_size = 0;

  err = rte_event_eth_rx_adapter_queue_add(TEST_INST_ID, TEST_ETHDEV_ID, 0,
                                           &queue_config);
  TEST_ASSERT(err == -EINVAL, "Expected -EINVAL got %d", err);
...
static int adapter_queue_stats_test(void) {
  int err;
  struct rte_event ev;
  uint32_t cap;
  struct rte_event_eth_rx_adapter_queue_conf queue_config = {0};
  ...
  err = rte_event_eth_rx_adapter_queue_stats_get(TEST_INST_ID, TEST_ETHDEV_ID,
                                                 0, &q_stats);
  TEST_ASSERT(err == -EINVAL, "Expected -EINVAL got %d", err);

  err = rte_event_eth_rx_adapter_queue_stats_reset(TEST_INST_ID, TEST_ETHDEV_ID,
                                                   0);
  TEST_ASSERT(err == -EINVAL, "Expected -EINVAL got %d", err);
...
static void adapter_free(void) { rte_event_eth_rx_adapter_free(TEST_INST_ID); }

static int adapter_create_free(void) {
  int err;
  ...
  err = rte_event_eth_rx_adapter_create(TEST_INST_ID, TEST_DEV_ID, NULL);
  TEST_ASSERT(err == -EINVAL, "Expected -EINVAL got %d", err);
...
static int adapter_create_free_with_params(void) {
  int err;
  ...
  struct rte_event_eth_rx_adapter_params rxa_params = {.event_buf_size = 1024};
...
static int adapter_queue_add_del(void) {
  int err;
  struct rte_event ev;
  uint32_t cap;
  ...
  err = rte_event_eth_rx_adapter_queue_add(
      TEST_INST_ID, rte_eth_dev_count_total(), -1, &queue_config);
  TEST_ASSERT(err == -EINVAL, "Expected -EINVAL got %d", err);
  ...
  if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_MULTI_EVENTQ) {
    ...
    err = rte_event_eth_rx_adapter_queue_add(TEST_INST_ID, TEST_ETHDEV_ID, -1,
                                             &queue_config);
    TEST_ASSERT(err == 0, "Expected 0 got %d", err);

    err = rte_event_eth_rx_adapter_queue_del(TEST_INST_ID, TEST_ETHDEV_ID, -1);
    TEST_ASSERT(err == 0, "Expected 0 got %d", err);
  } else {
    err = rte_event_eth_rx_adapter_queue_add(TEST_INST_ID, TEST_ETHDEV_ID, 0,
                                             &queue_config);
  ...
  err =
      rte_event_eth_rx_adapter_queue_add(1, TEST_ETHDEV_ID, -1, &queue_config);
  TEST_ASSERT(err == -EINVAL, "Expected -EINVAL got %d", err);
...
static int adapter_multi_eth_add_del(void) {
  int err;
  struct rte_event ev;
  ...
  for (; port_index < RTE_MAX_ETHPORTS; port_index += 1) {
    snprintf(driver_name, sizeof(driver_name), "%s%u", "net_null", drv_id);
    err = rte_vdev_init(driver_name, NULL);
    TEST_ASSERT(err == 0, "Failed driver %s got %d", driver_name, err);
    drv_id += 1;
  }
  ...
  port_index = 0;
  for (; port_index < rte_eth_dev_count_total(); port_index += 1) {
    err = rte_event_eth_rx_adapter_queue_add(TEST_INST_ID, port_index, -1,
                                             &queue_config);
    TEST_ASSERT(err == 0, "Expected 0 got %d", err);
  }
  ...
  port_index = 0;
  for (; port_index < rte_eth_dev_count_total(); port_index += 1) {
    err = rte_event_eth_rx_adapter_queue_del(TEST_INST_ID, port_index, -1);
    TEST_ASSERT(err == 0, "Expected 0 got %d", err);
  }

  /* delete vdev ports */
  for (drv_id = 0, port_index = port_index_base; port_index < RTE_MAX_ETHPORTS;
...
static int adapter_intr_queue_add_del(void) {
  int err;
  struct rte_event ev;
  uint32_t cap;
  uint16_t eth_port;
...
static int adapter_start_stop(void) {
  int err;
  struct rte_event ev;
  ...
  queue_config.rx_queue_flags = 0;
  if (default_params.caps & RTE_EVENT_ETH_RX_ADAPTER_CAP_OVERRIDE_FLOW_ID) {
    ev.flow_id = 1;
    queue_config.rx_queue_flags = RTE_EVENT_ETH_RX_ADAPTER_QUEUE_FLOW_ID_VALID;
  }
...
static int adapter_queue_conf(void) {
  ...
  err = rte_event_eth_rx_adapter_queue_conf_get(TEST_INST_ID, TEST_ETHDEV_ID,
                                                -1, &queue_conf);
  TEST_ASSERT(err == -EINVAL, "Expected -EINVAL got %d", err);
...
static struct unit_test_suite event_eth_rx_tests = {
    ...
    .unit_test_cases = {
        ...
        TEST_CASE_ST(adapter_create, adapter_free, adapter_multi_eth_add_del),
...
static struct unit_test_suite event_eth_rx_intr_tests = {
    ...
    .unit_test_cases = {
        TEST_CASE_ST(adapter_create, adapter_free, adapter_intr_queue_add_del),
        TEST_CASES_END() /**< NULL terminate unit test array */
    }};
