...
static int test_eventdev_common(void) {
...
static int test_eventdev_count(void) {
  uint8_t count;
  count = rte_event_dev_count();
...
static int test_eventdev_get_dev_id(void) {
  int ret;
...
static int test_eventdev_socket_id(void) {
  int socket_id;
  socket_id = rte_event_dev_socket_id(TEST_DEV_ID);
  TEST_ASSERT(socket_id != -EINVAL, "Failed to get socket_id %d", socket_id);
...
static int test_eventdev_info_get(void) {
  int ret;
  ...
  TEST_ASSERT(info.max_event_ports > 0, "Not enough event ports %d",
              info.max_event_ports);
  TEST_ASSERT(info.max_event_queues > 0, "Not enough event queues %d",
...
static void max_dequeue_limit(struct rte_event_dev_config *dev_conf,
                              struct rte_event_dev_info *info) {
  dev_conf->dequeue_timeout_ns = info->max_dequeue_timeout_ns + 1;
}

static void max_events_limit(struct rte_event_dev_config *dev_conf,
                             struct rte_event_dev_info *info) {
  dev_conf->nb_events_limit = info->max_num_events + 1;
}

static void max_event_ports(struct rte_event_dev_config *dev_conf,
                            struct rte_event_dev_info *info) {
  dev_conf->nb_event_ports = info->max_event_ports + 1;
}

static void max_event_queues(struct rte_event_dev_config *dev_conf,
                             struct rte_event_dev_info *info) {
  dev_conf->nb_event_queues = info->max_event_queues + 1;
}

static void max_event_queue_flows(struct rte_event_dev_config *dev_conf,
                                  struct rte_event_dev_info *info) {
  dev_conf->nb_event_queue_flows = info->max_event_queue_flows + 1;
}

static void max_event_port_dequeue_depth(struct rte_event_dev_config *dev_conf,
                                         struct rte_event_dev_info *info) {
  dev_conf->nb_event_port_dequeue_depth =
      info->max_event_port_dequeue_depth + 1;
}

static void max_event_port_enqueue_depth(struct rte_event_dev_config *dev_conf,
                                         struct rte_event_dev_info *info) {
  dev_conf->nb_event_port_enqueue_depth =
      info->max_event_port_enqueue_depth + 1;
}

static int test_eventdev_configure(void) {
  int ret;
  ...
  TEST_ASSERT_EQUAL(-EINVAL,
                    ...
                    "Config negative test failed");
  TEST_ASSERT_EQUAL(
      -EINVAL, test_ethdev_config_run(&dev_conf, &info, max_event_queue_flows),
      "Config negative test failed");

  if (info.event_dev_cap & RTE_EVENT_DEV_CAP_BURST_MODE) {
    TEST_ASSERT_EQUAL(
        -EINVAL,
        test_ethdev_config_run(&dev_conf, &info, max_event_port_dequeue_depth),
        "Config negative test failed");
    TEST_ASSERT_EQUAL(
        -EINVAL,
        test_ethdev_config_run(&dev_conf, &info, max_event_port_enqueue_depth),
        "Config negative test failed");
  }
...
static int eventdev_configure_setup(void) {
  int ret;
  struct rte_event_dev_config dev_conf;
  struct rte_event_dev_info info;
...
static int test_eventdev_queue_default_conf_get(void) {
  int i, ret;
...
static int test_eventdev_queue_setup(void) {
  int i, ret;
  struct rte_event_dev_info info;
  struct rte_event_queue_conf qconf;
  ...
  ret = rte_event_queue_setup(TEST_DEV_ID, info.max_event_queues, &qconf);
  TEST_ASSERT(ret == -EINVAL, "Expected -EINVAL, %d", ret);
...
static int test_eventdev_queue_count(void) {
  int ret;
  struct rte_event_dev_info info;
  ...
  TEST_ASSERT_SUCCESS(rte_event_dev_attr_get(TEST_DEV_ID,
                                             ...
                                             &queue_count),
                      "Queue count get failed");
  TEST_ASSERT_EQUAL(queue_count, info.max_event_queues, "Wrong queue count");

  return TEST_SUCCESS;
}

static int test_eventdev_queue_attr_priority(void) {
  int i, ret;
  struct rte_event_dev_info info;
  struct rte_event_queue_conf qconf;
  uint8_t priority;
  ...
  for (i = 0; i < (int)queue_count; i++) {
    ...
    if (info.event_dev_cap & RTE_EVENT_DEV_CAP_QUEUE_QOS)
      TEST_ASSERT_EQUAL(priority, i % RTE_EVENT_DEV_PRIORITY_LOWEST,
                        "Wrong priority value for queue%d", i);
    else
      TEST_ASSERT_EQUAL(priority, RTE_EVENT_DEV_PRIORITY_NORMAL,
                        "Wrong priority value for queue%d", i);
  }
...
static int test_eventdev_queue_attr_nb_atomic_flows(void) {
  int i, ret;
  struct rte_event_dev_info info;
  struct rte_event_queue_conf qconf;
...
static int test_eventdev_queue_attr_nb_atomic_order_sequences(void) {
  int i, ret;
  struct rte_event_dev_info info;
  struct rte_event_queue_conf qconf;
  ...
  for (i = 0; i < (int)queue_count; i++) {
    ...
    TEST_ASSERT_EQUAL(nb_atomic_order_sequences,
                      ...
                      "Wrong atomic order sequences value for queue%d", i);
  }

  return TEST_SUCCESS;
}

static int test_eventdev_queue_attr_event_queue_cfg(void) {
  int i, ret;
  struct rte_event_dev_info info;
  struct rte_event_queue_conf qconf;
  ...
  for (i = 0; i < (int)queue_count; i++) {
    ...
    TEST_ASSERT_EQUAL(event_queue_cfg, qconf.event_queue_cfg,
                      "Wrong event_queue_cfg value for queue%d", i);
  }

  return TEST_SUCCESS;
}

static int test_eventdev_port_default_conf_get(void) {
  int i, ret;
  ...
  ret = rte_event_port_default_conf_get(TEST_DEV_ID, port_count + 1, NULL);
  TEST_ASSERT(ret == -EINVAL, "Expected -EINVAL, %d", ret);

  for (i = 0; i < (int)port_count; i++) {
    ret = rte_event_port_default_conf_get(TEST_DEV_ID, i, &pconf);
...
static int test_eventdev_port_setup(void) {
  int i, ret;
  struct rte_event_dev_info info;
  struct rte_event_port_conf pconf;
  ...
  if (!(info.event_dev_cap & RTE_EVENT_DEV_CAP_IMPLICIT_RELEASE_DISABLE)) {
  ...
  ret = rte_event_port_setup(TEST_DEV_ID, info.max_event_ports, &pconf);
  TEST_ASSERT(ret == -EINVAL, "Expected -EINVAL, %d", ret);
...
static int test_eventdev_port_attr_dequeue_depth(void) {
  int ret;
  struct rte_event_dev_info info;
  struct rte_event_port_conf pconf;
  ...
  uint32_t value;
  TEST_ASSERT_EQUAL(rte_event_port_attr_get(
                        TEST_DEV_ID, 0, RTE_EVENT_PORT_ATTR_DEQ_DEPTH, &value),
                    0, "Call to get port dequeue depth failed");
  TEST_ASSERT_EQUAL(value, pconf.dequeue_depth, "Wrong port dequeue depth");

  return TEST_SUCCESS;
}

static int test_eventdev_port_attr_enqueue_depth(void) {
  int ret;
  struct rte_event_dev_info info;
  struct rte_event_port_conf pconf;
  ...
  uint32_t value;
  TEST_ASSERT_EQUAL(rte_event_port_attr_get(
                        TEST_DEV_ID, 0, RTE_EVENT_PORT_ATTR_ENQ_DEPTH, &value),
                    0, "Call to get port enqueue depth failed");
  TEST_ASSERT_EQUAL(value, pconf.enqueue_depth, "Wrong port enqueue depth");

  return TEST_SUCCESS;
}

static int test_eventdev_port_attr_new_event_threshold(void) {
  int ret;
  struct rte_event_dev_info info;
  struct rte_event_port_conf pconf;
  ...
  uint32_t value;
  TEST_ASSERT_EQUAL(
      rte_event_port_attr_get(TEST_DEV_ID, 0,
...
static int test_eventdev_port_count(void) {
  int ret;
  struct rte_event_dev_info info;
...
static int test_eventdev_timeout_ticks(void) {
  int ret;
...
static int test_eventdev_start_stop(void) {
  int i, ret;
...
static int eventdev_setup_device(void) {
  int i, ret;
...
static void eventdev_stop_device(void) { rte_event_dev_stop(TEST_DEV_ID); }

static int test_eventdev_link(void) {
  int ret, nb_queues, i;
  uint8_t queues[RTE_EVENT_MAX_QUEUES_PER_DEV];
  uint8_t priorities[RTE_EVENT_MAX_QUEUES_PER_DEV];

  ret = rte_event_port_link(TEST_DEV_ID, 0, NULL, NULL, 0);
  TEST_ASSERT(ret >= 0, "Failed to link with NULL device%d", TEST_DEV_ID);
  ...
  ret = rte_event_port_link(TEST_DEV_ID, 0, queues, priorities, nb_queues);
  TEST_ASSERT(ret == nb_queues, "Failed to link(device%d) ret=%d", TEST_DEV_ID,
              ret);
  return TEST_SUCCESS;
}

static int test_eventdev_unlink(void) {
  int ret, nb_queues, i;
  uint8_t queues[RTE_EVENT_MAX_QUEUES_PER_DEV];

  ret = rte_event_port_unlink(TEST_DEV_ID, 0, NULL, 0);
  TEST_ASSERT(ret >= 0, "Failed to unlink with NULL device%d", TEST_DEV_ID);
...
static int test_eventdev_link_get(void) {
  int ret, i;
  uint8_t queues[RTE_EVENT_MAX_QUEUES_PER_DEV];
  uint8_t priorities[RTE_EVENT_MAX_QUEUES_PER_DEV];
  ...
  TEST_ASSERT(ret == 1, "Failed to link(device%d) ret=%d", TEST_DEV_ID, ret);
  ret = rte_event_port_links_get(TEST_DEV_ID, 0, queues, priorities);
  TEST_ASSERT(ret == 1, "(%d)Wrong link get ret=%d expected=%d", TEST_DEV_ID,
              ret, 1);
  /* unlink the queue */
  ret = rte_event_port_unlink(TEST_DEV_ID, 0, NULL, 0);
  TEST_ASSERT(ret == 1, "Failed to unlink(device%d) ret=%d", TEST_DEV_ID, ret);
  ...
  if (nb_queues >= 4) {
    ...
    ret = rte_event_port_link(TEST_DEV_ID, 0, queues, priorities, 4);
    TEST_ASSERT(ret == 4, "Failed to link(device%d) ret=%d", TEST_DEV_ID, ret);

    for (i = 0; i < 2; i++)
      queues[i] = i;

    ret = rte_event_port_unlink(TEST_DEV_ID, 0, queues, 2);
    TEST_ASSERT(ret == 2, "Failed to unlink(device%d) ret=%d", TEST_DEV_ID,
                ret);
    ret = rte_event_port_links_get(TEST_DEV_ID, 0, queues, priorities);
    TEST_ASSERT(ret == 2, "(%d)Wrong link get ret=%d expected=%d", TEST_DEV_ID,
                ret, 2);
    TEST_ASSERT(queues[0] == 2, "ret=%d expected=%d", ret, 2);
    TEST_ASSERT(priorities[0] == 0x40, "ret=%d expected=%d", ret, 0x40);
    TEST_ASSERT(queues[1] == 3, "ret=%d expected=%d", ret, 3);
    TEST_ASSERT(priorities[1] == 0x40, "ret=%d expected=%d", ret, 0x40);
  }

  return TEST_SUCCESS;
}

static int test_eventdev_close(void) {
  rte_event_dev_stop(TEST_DEV_ID);
...
static struct unit_test_suite eventdev_common_testsuite = {
    ...
    .unit_test_cases = {
        TEST_CASE_ST(NULL, NULL, test_eventdev_count),
        TEST_CASE_ST(NULL, NULL, test_eventdev_get_dev_id),
        TEST_CASE_ST(NULL, NULL, test_eventdev_socket_id),
        TEST_CASE_ST(NULL, NULL, test_eventdev_info_get),
        ...
        TEST_CASE_ST(eventdev_configure_setup, NULL, test_eventdev_queue_setup),
        ...
        TEST_CASE_ST(eventdev_configure_setup, NULL, test_eventdev_port_setup),
        TEST_CASE_ST(eventdev_configure_setup, NULL,
        ...
        TEST_CASE_ST(eventdev_configure_setup, NULL, test_eventdev_port_count),
        TEST_CASE_ST(eventdev_configure_setup, NULL,
                     test_eventdev_timeout_ticks),
        TEST_CASE_ST(NULL, NULL, test_eventdev_start_stop),
        ...
        TEST_CASE_ST(eventdev_setup_device, NULL, test_eventdev_close),
...
static int test_eventdev_selftest_impl(const char *pmd, const char *opts) {
  int ret = 0;
...
static int test_eventdev_selftest_sw(void) {
  return test_eventdev_selftest_impl("event_sw", "");
}

static int test_eventdev_selftest_octeontx(void) {
  return test_eventdev_selftest_impl("event_octeontx", "");
}

static int test_eventdev_selftest_dpaa2(void) {
  return test_eventdev_selftest_impl("event_dpaa2", "");
}

static int test_eventdev_selftest_dlb2(void) {
  return test_eventdev_selftest_impl("dlb2_event", "");
}

static int test_eventdev_selftest_cn9k(void) {
  return test_eventdev_selftest_impl("event_cn9k", "");
}

static int test_eventdev_selftest_cn10k(void) {
