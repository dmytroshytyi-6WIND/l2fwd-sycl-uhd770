...
static int test_table_type(struct rte_table_ops *table_ops, void *table_args,
  ...
  if (rte_pipeline_port_out_create(pipeline, &ring_out_params, &ring_out_id) !=
      0) {
    rte_pipeline_free(pipeline);
    return -CHECK_TABLE_PORT_CONFIG;
  }
  ...
  if (rte_pipeline_table_default_entry_add(pipeline, table_id, &default_entry,
                                           &default_entry_ptr) != 0) {
    rte_pipeline_free(pipeline);
    return -CHECK_TABLE_DEFAULT_ENTRY_ADD;
  }

  if (rte_pipeline_table_entry_add(pipeline, table_id, key ? key : &table_entry,
                                   &table_entry, &key_found, &entry_ptr) != 0) {
    rte_pipeline_free(pipeline);
    return -CHECK_TABLE_ENTRY_ADD;
  }

  /* Create connections and check consistency */
  if (rte_pipeline_port_in_connect_to_table(pipeline, ring_in_id, table_id) !=
      0) {
    rte_pipeline_free(pipeline);
  ...
  printf("delete entry\n");
  /* Delete table entry */
  rte_pipeline_table_entry_delete(pipeline, table_id, key ? key : &table_entry,
                                  &key_found, NULL);
...
int test_table_stub_combined(void) {
  ...
  status =
      test_table_type(&rte_table_stub_ops, NULL, NULL, &table_packets, NULL, 1);
...
int test_table_lpm_ipv6_combined(void) {
  ...
  status = test_table_type(&rte_table_lpm_ipv6_ops, (void *)&lpm_ipv6_params,
...
int test_table_hash8lru(void) {
  ...
  status =
      test_table_type(&rte_table_hash_key8_lru_ops, (void *)&key8lru_params,
                      (void *)key8lru, &table_packets, NULL, 0);
  VERIFY(status, CHECK_TABLE_OK);
...
int test_table_hash16lru(void) {
  ...
  status =
      test_table_type(&rte_table_hash_key16_lru_ops, (void *)&key16lru_params,
                      (void *)key16lru, &table_packets, NULL, 0);
  VERIFY(status, CHECK_TABLE_OK);
...
int test_table_hash32lru(void) {
  ...
  status =
      test_table_type(&rte_table_hash_key32_lru_ops, (void *)&key32lru_params,
                      (void *)key32lru, &table_packets, NULL, 0);
  VERIFY(status, CHECK_TABLE_OK);
...
int test_table_hash8ext(void) {
  ...
  status =
      test_table_type(&rte_table_hash_key8_ext_ops, (void *)&key8ext_params,
                      (void *)key8ext, &table_packets, NULL, 0);
  VERIFY(status, CHECK_TABLE_OK);
...
int test_table_hash16ext(void) {
  ...
  status =
      test_table_type(&rte_table_hash_key16_ext_ops, (void *)&key16ext_params,
                      (void *)key16ext, &table_packets, NULL, 0);
  VERIFY(status, CHECK_TABLE_OK);
...
int test_table_hash32ext(void) {
  ...
  status =
      test_table_type(&rte_table_hash_key32_ext_ops, (void *)&key32ext_params,
                      (void *)key32ext, &table_packets, NULL, 0);
  VERIFY(status, CHECK_TABLE_OK);
...
int test_table_hash_cuckoo_combined(void) {
  ...
  status = test_table_type(&rte_table_hash_cuckoo_ops, (void *)&cuckoo_params,
                           (void *)key_cuckoo, &table_packets, NULL, 0);
  VERIFY(status, CHECK_TABLE_OK);
