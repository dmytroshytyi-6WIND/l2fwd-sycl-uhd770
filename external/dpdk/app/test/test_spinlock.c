...
static int test_spinlock_per_core(__rte_unused void *arg) {
...
static int test_spinlock_recursive_per_core(__rte_unused void *arg) {
...
static int test_spinlock_perf(void) {
  unsigned int i;
  uint64_t total = 0;
  int lock = 0;
  const unsigned lcore = rte_lcore_id();
 ...
 */
static int test_spinlock_try(__rte_unused void *arg) {
...
static int test_spinlock(void) {
  ...
  if (rte_spinlock_recursive_trylock(&slr) == 0) {
    printf("It failed to do the first spinlock_recursive_trylock but it should "
           "able to do\n");
    return -1;
  }
  if (rte_spinlock_recursive_trylock(&slr) == 0) {
    printf("It failed to do the second spinlock_recursive_trylock but it "
           "should able to do\n");
    return -1;
  }
  rte_spinlock_recursive_unlock(&slr);
