...
static int test_member_perf(void) {
...
#define NUM_LOOKUPS                                                            \
  (KEYS_TO_ADD * 5) /* Loop among keys added, several times                    \
                     */
...
enum sstype { HT = 0, CACHE, VBF, NUM_TYPE };
...
static void shuffle_input_keys(struct member_perf_params *params) {
...
static int setup_keys_and_data(struct member_perf_params *params,
                               unsigned int cycle, int miss) {
  unsigned int i, j;
  int num_duplicates;
...
static int timed_adds(struct member_perf_params *params, int type) {
  const uint64_t start_tsc = rte_rdtsc();
  unsigned int i, a;
  int32_t ret;

  for (i = 0; i < KEYS_TO_ADD; i++) {
    ret = rte_member_add(params->setsum[type], &keys[i], data[type][i]);
    if (ret < 0) {
...
static int timed_lookups(struct member_perf_params *params, int type) {
  unsigned int i, j;
  ...
  for (i = 0; i < NUM_LOOKUPS / KEYS_TO_ADD; i++) {
    for (j = 0; j < KEYS_TO_ADD; j++) {
      ret = rte_member_lookup(params->setsum[type], &keys[j], &result);
      if (ret < 0) {
...
static int timed_lookups_bulk(struct member_perf_params *params, int type) {
  unsigned int i, j, k;
  ...
  for (i = 0; i < NUM_LOOKUPS / KEYS_TO_ADD; i++) {
    for (j = 0; j < KEYS_TO_ADD / BURST_SIZE; j++) {
      ...
      ret = rte_member_lookup_bulk(params->setsum[type], keys_burst, BURST_SIZE,
                                   result);
      if (ret <= 0) {
      ...
      for (k = 0; k < BURST_SIZE; k++) {
        ...
        if (type == HT && result[k] == RTE_MEMBER_NO_MATCH) {
...
static int timed_lookups_multimatch(struct member_perf_params *params,
                                    int type) {
  unsigned int i, j;
  ...
  for (i = 0; i < NUM_LOOKUPS / KEYS_TO_ADD; i++) {
    for (j = 0; j < KEYS_TO_ADD; j++) {
      ret = rte_member_lookup_multi(params->setsum[type], &keys[j],
                                    RTE_MEMBER_BUCKET_ENTRIES, result);
      if (type != CACHE && ret <= 0) {
...
static int timed_lookups_multimatch_bulk(struct member_perf_params *params,
                                         int type) {
  unsigned int i, j, k;
  ...
  for (i = 0; i < NUM_LOOKUPS / KEYS_TO_ADD; i++) {
    for (j = 0; j < KEYS_TO_ADD / BURST_SIZE; j++) {
      ...
      ret = rte_member_lookup_multi_bulk(params->setsum[type], keys_burst,
                                         BURST_SIZE, RTE_MEMBER_BUCKET_ENTRIES,
                                         match_count, (member_set_t *)result);
      if (ret < 0) {
  ...
  cycles[type][params->cycle][LOOKUP_MULTI_BULK] = time_taken / NUM_LOOKUPS;

  return 0;
}

static int timed_deletes(struct member_perf_params *params, int type) {
  unsigned int i;
  int32_t ret;
  ...
  const uint64_t start_tsc = rte_rdtsc();
  for (i = 0; i < KEYS_TO_ADD; i++) {
    ret = rte_member_delete(params->setsum[type], &keys[i], data[type][i]);
    if (type != CACHE && ret < 0) {
      printf("delete error\n");
      return -1;
    }
...
static int timed_miss_lookup(struct member_perf_params *params, int type) {
  unsigned int i, j;
  int ret;
...
static void perform_frees(struct member_perf_params *params) {
  int i;
  for (i = 0; i < NUM_TYPE; i++) {
    if (params->setsum[i] != NULL) {
...
static int exit_with_fail(const char *testname,
                          struct member_perf_params *params, unsigned int i,
                          unsigned int j) {
...
static int run_all_tbl_perf_tests(void) {
  ...
  for (i = 0; i < NUM_KEYSIZES; i++) {
    ...
    for (j = 0; j < NUM_TYPE; j++) {
      ...
      if (timed_adds(&params, j) < 0)
        return exit_with_fail("timed_adds", &params, i, j);

      for (k = 0; k < NUM_SHUFFLES; k++)
        shuffle_input_keys(&params);

      if (timed_lookups(&params, j) < 0)
        return exit_with_fail("timed_lookups", &params, i, j);

      if (timed_lookups_bulk(&params, j) < 0)
        return exit_with_fail("timed_lookups_bulk", &params, i, j);

      if (timed_lookups_multimatch(&params, j) < 0)
        return exit_with_fail("timed_lookups_multi", &params, i, j);

      if (timed_lookups_multimatch_bulk(&params, j) < 0)
        return exit_with_fail("timed_lookups_multi_bulk", &params, i, j);

      if (timed_deletes(&params, j) < 0)
        return exit_with_fail("timed_deletes", &params, i, j);
  ...
  for (i = 0; i < NUM_KEYSIZES; i++) {
    ...
    for (j = 0; j < NUM_TYPE; j++) {
      if (timed_miss_lookup(&params, j) < 0)
        return exit_with_fail("timed_miss_lookup", &params, i, j);
    }
    perform_frees(&params);
  }
  ...
  printf("\n%-18s%-18s%-18s%-18s%-18s%-18s%-18s%-18s%-18s\n", "Keysize", "type",
         "Add", "Lookup", "Lookup_bulk", "lookup_multi", "lookup_multi_bulk",
         "Delete", "miss_lookup");
  for (i = 0; i < NUM_KEYSIZES; i++) {
    for (j = 0; j < NUM_TYPE; j++) {
  ...
  printf("\n%-18s%-18s%-18s%-18s%-18s%-18s%-18s\n", "Keysize", "type",
         "fr_single", "fr_bulk", "fr_multi", "fr_multi_bulk",
         "false_positive_rate");
  ...
  for (i = 0; i < 1; i++) {
    for (j = 0; j < NUM_TYPE; j++) {
      ...
      printf("%-18f", (float)false_data_bulk[j][i] / NUM_LOOKUPS);
      printf("%-18f", (float)false_data_multi[j][i] / NUM_LOOKUPS);
      printf("%-18f", (float)false_data_multi_bulk[j][i] / NUM_LOOKUPS);
      printf("%-18f", (float)false_hit[j][i] / NUM_LOOKUPS);
      printf("\n");
    }
  }
  return 0;
