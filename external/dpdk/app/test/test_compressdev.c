...
const char *huffman_type_strings[] = {[RTE_COMP_HUFFMAN_DEFAULT] =
                                          "PMD default",
                                      [RTE_COMP_HUFFMAN_FIXED] = "Fixed",
                                      [RTE_COMP_HUFFMAN_DYNAMIC] = "Dynamic"};

enum zlib_direction { ZLIB_NONE, ZLIB_COMPRESS, ZLIB_DECOMPRESS, ZLIB_ALL };
...
enum overflow_test { OVERFLOW_DISABLED, OVERFLOW_ENABLED };

enum ratio_switch { RATIO_DISABLED, RATIO_ENABLED };

enum operation_type { OPERATION_COMPRESSION, OPERATION_DECOMPRESSION };
...
static int testsuite_setup(void) {
  ...
  for (i = 0; i < RTE_DIM(compress_test_bufs); i++)
    max_buf_size = RTE_MAX(max_buf_size, strlen(compress_test_bufs[i]) + 1);
  ...
  ts_params->large_mbuf_pool = rte_pktmbuf_pool_create(
      "large_mbuf_pool", NUM_LARGE_MBUFS, CACHE_SIZE, 0,
      max_buf_size + RTE_PKTMBUF_HEADROOM, rte_socket_id());
  ...
  ts_params->small_mbuf_pool = rte_pktmbuf_pool_create(
      "small_mbuf_pool", NUM_LARGE_MBUFS * MAX_SEGS, CACHE_SIZE, 0,
      SMALL_SEG_SIZE + RTE_PKTMBUF_HEADROOM, rte_socket_id());
  ...
  ts_params->big_mbuf_pool =
      rte_pktmbuf_pool_create("big_mbuf_pool", NUM_BIG_MBUFS + 1, CACHE_SIZE, 0,
                              MAX_MBUF_SEGMENT_SIZE, rte_socket_id());
  if (ts_params->big_mbuf_pool == NULL) {
    RTE_LOG(ERR, USER1, "Big mbuf pool could not be created\n");
    goto exit;
  }

  ts_params->op_pool = rte_comp_op_pool_create(
      "op_pool", NUM_OPS, 0, sizeof(struct priv_op_data), rte_socket_id());
  ...
  if (ts_params->def_comp_xform == NULL) {
    RTE_LOG(ERR, USER1, "Default compress xform could not be created\n");
    goto exit;
  }
  ts_params->def_decomp_xform =
      rte_malloc(NULL, sizeof(struct rte_comp_xform), 0);
  if (ts_params->def_decomp_xform == NULL) {
    RTE_LOG(ERR, USER1, "Default decompress xform could not be created\n");
    goto exit;
  }
...
static int generic_ut_setup(void) {
  /* Configure compressdev (one device, one queue pair) */
  struct rte_compressdev_config config = {.socket_id = rte_socket_id(),
                                          .nb_queue_pairs = 1,
...
static void generic_ut_teardown(void) {
  rte_compressdev_stop(0);
  if (rte_compressdev_close(0) < 0)
    RTE_LOG(ERR, USER1, "Device could not be closed\n");
}

static int test_compressdev_invalid_configuration(void) {
  struct rte_compressdev_config invalid_config;
  struct rte_compressdev_config valid_config = {.socket_id = rte_socket_id(),
                                                .nb_queue_pairs = 1,
                                                .max_nb_priv_xforms =
                                                    NUM_MAX_XFORMS,
                                                .max_nb_streams = 1};
  ...
  memcpy(&invalid_config, &valid_config, sizeof(struct rte_compressdev_config));
  ...
  TEST_ASSERT_FAIL(rte_compressdev_queue_pair_setup(0, 0, NUM_MAX_INFLIGHT_OPS,
                                                    rte_socket_id()),
...
static int compare_buffers(const char *buffer1, uint32_t buffer1_len,
 ...
 */
static int map_zlib_flush_flag(enum rte_comp_flush_flag flag) {
  switch (flag) {
...
static int compress_zlib(struct rte_comp_op *op,
  ...
  if (comp_level != RTE_COMP_LEVEL_NONE)
    ret = deflateInit2(&stream, comp_level, Z_DEFLATED, window_bits, mem_level,
                       strategy);
  else
  ...
  if (op->m_src->nb_segs > 1) {
    single_src_buf = rte_malloc(NULL, rte_pktmbuf_pkt_len(op->m_src), 0);
    ...
    if (rte_pktmbuf_read(op->m_src, op->src.offset,
                         rte_pktmbuf_pkt_len(op->m_src) - op->src.offset,
                         single_src_buf) == NULL) {
    ...
    stream.avail_in = op->src.length;
    stream.next_in =
        rte_pktmbuf_mtod_offset(op->m_src, uint8_t *, op->src.offset);
  }
  /* SGL output */
  if (op->m_dst->nb_segs > 1) {

    single_dst_buf = rte_malloc(NULL, rte_pktmbuf_pkt_len(op->m_dst), 0);
    ...
    stream.next_out =
        rte_pktmbuf_mtod_offset(op->m_dst, uint8_t *, op->dst.offset);
  }
  ...
  if (op->m_dst->nb_segs > 1) {
    ...
    while (remaining_data > 0) {
      uint8_t *dst_data =
          rte_pktmbuf_mtod_offset(dst_buf, uint8_t *, op->dst.offset);
      /* Last segment */
  ...
  if (xform->compress.chksum == RTE_COMP_CHECKSUM_ADLER32) {
    ...
    op->produced = stream.total_out - (ZLIB_HEADER_SIZE + ZLIB_TRAILER_SIZE);
  } else if (xform->compress.chksum == RTE_COMP_CHECKSUM_CRC32) {
    rte_pktmbuf_adj(op->m_dst, GZIP_HEADER_SIZE);
    rte_pktmbuf_trim(op->m_dst, GZIP_TRAILER_SIZE);
    op->produced = stream.total_out - (GZIP_HEADER_SIZE + GZIP_TRAILER_SIZE);
  } else
...
static int decompress_zlib(struct rte_comp_op *op,
  ...
  if (op->m_src->nb_segs > 1) {
    ...
    if (rte_pktmbuf_read(op->m_src, 0, rte_pktmbuf_pkt_len(op->m_src),
                         single_src_buf) == NULL) {
      RTE_LOG(ERR, USER1, "Buffer could not be read entirely\n");
      goto exit;
    }
  ...
  if (op->m_src->nb_segs > 1) {
    ...
    while (remaining_data > 0) {
      uint8_t *dst_data = rte_pktmbuf_mtod(dst_buf, uint8_t *);
      /* Last segment */
      if (remaining_data < dst_buf->data_len) {
        memcpy(dst_data, src_data, remaining_data);
        remaining_data = 0;
...
static int prepare_sgl_bufs(const char *test_buf, struct rte_mbuf *head_buf,
                            uint32_t total_data_size,
                            struct rte_mempool *small_mbuf_pool,
                            struct rte_mempool *large_mbuf_pool,
                            uint8_t limit_segs_in_sgl, uint16_t seg_size) {
  ...
  if (buf_ptr == NULL) {
    RTE_LOG(ERR, USER1, "Not enough space in the 1st buffer\n");
    return -1;
  }
  ...
  for (i = 0; i < num_remaining_segs; i++) {
    ...
    if (buf_ptr == NULL) {
      RTE_LOG(ERR, USER1, "Not enough space in the buffer\n");
    ...
    if (ret != 0) {
      rte_pktmbuf_free(next_seg);
      RTE_LOG(ERR, USER1, "Segment could not chained\n");
      return -1;
    }
  }
...
static void extbuf_free_callback(void *addr __rte_unused,
                                 void *opaque __rte_unused) {}

static int test_run_enqueue_dequeue(struct rte_comp_op **ops,
                                    struct rte_comp_op **ops_processed,
                                    unsigned int num_bufs) {
  ...
  if (num_enqd < num_bufs) {
    RTE_LOG(ERR, USER1, "Some operations could not be enqueued\n");
    res = -1;
  }
  ...
  while (num_total_deqd < num_enqd) {
    ...
    if (deqd_retries != 0) {
      ...
      if (deqd_retries == MAX_DEQD_RETRIES) {
        RTE_LOG(ERR, USER1, "Not all operations could be dequeued\n");
        res = -1;
        break;
      }
      usleep(DEQUEUE_WAIT_TIME);
    }
    num_deqd = rte_compressdev_dequeue_burst(
        0, 0, &ops_processed[num_total_deqd], num_bufs);
...
test_setup_com_bufs(const struct interim_data_params *int_data,
  ...
  memset(test_priv_data->compressed_data_size, 0, sizeof(uint32_t) * num_bufs);

  if (test_data->decompress_state == RTE_COMP_OP_STATEFUL) {
    data_size = strlen(test_bufs[0]) + 1;
    *all_decomp_data = rte_malloc(NULL, data_size, RTE_CACHE_LINE_SIZE);
  }
  ...
  if (test_data->use_external_mbufs) {
    ...
    for (i = 0; i < num_bufs; i++) {
      rte_pktmbuf_attach_extbuf(uncomp_bufs[i], test_data->inbuf_memzone->addr,
                                test_data->inbuf_memzone->iova,
                                test_data->inbuf_data_size, &inbuf_info);
      buf_ptr = rte_pktmbuf_append(uncomp_bufs[i], test_data->inbuf_data_size);
      if (buf_ptr == NULL) {
        RTE_LOG(ERR, USER1, "Append extra bytes to the source mbuf failed\n");
        return -1;
      }
    }
    ...
    for (i = 0; i < num_bufs; i++) {
      ...
      if (prepare_sgl_bufs(test_bufs[i], uncomp_bufs[i], data_size,
 ...
 *   data size
 */
static inline uint32_t test_mbufs_calculate_data_size(
    enum operation_type op_type, unsigned int out_of_space_and_zlib,
    const struct test_private_arrays *test_priv_data,
    const struct interim_data_params *int_data,
    const struct test_data_params *test_data, unsigned int i) {
  /* local variables: */
  uint32_t data_size;
  ...
  if (out_of_space_and_zlib)
    ...
    if (op_type == OPERATION_COMPRESSION) {
      ...
      ratio_val = (ratio == RATIO_ENABLED) ? COMPRESS_BUF_SIZE_RATIO
                                           : COMPRESS_BUF_SIZE_RATIO_DISABLED;

      ratio_val = (not_zlib_compr && (overflow == OVERFLOW_ENABLED))
...
test_setup_output_bufs(enum operation_type op_type,
  ...
  if (test_data->use_external_mbufs) {
    ...
    for (i = 0; i < num_bufs; i++) {
      rte_pktmbuf_attach_extbuf(current_bufs[i], current_memzone->addr,
                                current_memzone->iova, current_memzone->len,
                                current_extbuf_info);
      rte_pktmbuf_append(current_bufs[i], current_memzone->len);
    }
  } else {
    for (i = 0; i < num_bufs; i++) {
      ...
      data_size = test_mbufs_calculate_data_size(op_type, out_of_space_and_zlib,
                                                 test_priv_data, int_data,
                                                 test_data, i);
      ...
      if (buff_type == SGL_BOTH || buff_type == LB_TO_SGL) {
        ret = prepare_sgl_bufs(NULL, current_bufs[i], data_size,
                               big_data ? buf_pool : ts_params->small_mbuf_pool,
                               big_data ? buf_pool : ts_params->large_mbuf_pool,
                               big_data ? 0 : MAX_SEGS,
        ...
        buf_ptr = rte_pktmbuf_append(current_bufs[i], data_size);
        if (buf_ptr == NULL) {
          RTE_LOG(ERR, USER1,
...
test_deflate_comp_run(const struct interim_data_params *int_data,
  ...
  unsigned int zlib_compress =
      (test_data->zlib_dir == ZLIB_ALL || test_data->zlib_dir == ZLIB_COMPRESS);
  ...
  if (zlib_compress) {
    for (i = 0; i < num_bufs; i++) {
      ...
      ret = compress_zlib(ops[i], compress_xform, DEFAULT_MEM_LEVEL);
      if (ret < 0) {
        ret_status = -1;
        goto exit;
      }
    ...
    for (i = 0; i < num_xforms; i++) {
      ret = rte_compressdev_private_xform_create(
          0, (const struct rte_comp_xform *)compress_xforms[i],
          &priv_xforms[i]);
      if (ret < 0) {
        RTE_LOG(ERR, USER1,
    ...
    if (capa->comp_feature_flags & RTE_COMP_FF_SHAREABLE_PRIV_XFORM) {
      ...
      for (i = num_xforms; i < num_bufs; i++) {
        ret = rte_compressdev_private_xform_create(
            0, compress_xforms[i % num_xforms], &priv_xforms[i]);
        if (ret < 0) {
          RTE_LOG(ERR, USER1,
                  "Compression private xform "
                  "could not be created\n");
  ...
  recovery_lb:
    ...
    if (ret < 0) {
      RTE_LOG(ERR, USER1, "Compression: enqueue/dequeue operation failed\n");
      ret_status = -1;
      goto exit;
    }

    for (i = 0; i < num_bufs; i++) {
      test_priv_data->compressed_data_size[i] += ops_processed[i]->produced;

      if (ops_processed[i]->status ==
          RTE_COMP_OP_STATUS_OUT_OF_SPACE_RECOVERABLE) {

        ops[i]->status = RTE_COMP_OP_STATUS_NOT_PROCESSED;
        ops[i]->src.offset += ops_processed[i]->consumed;
        ops[i]->src.length -= ops_processed[i]->consumed;
        ops[i]->dst.offset += ops_processed[i]->produced;

        buf_ptr = rte_pktmbuf_append(ops[i]->m_dst, ops_processed[i]->produced);
...
test_deflate_comp_finalize(const struct interim_data_params *int_data,
  ...
  unsigned int zlib_compress =
      (test_data->zlib_dir == ZLIB_ALL || test_data->zlib_dir == ZLIB_COMPRESS);
  unsigned int zlib_decompress = (test_data->zlib_dir == ZLIB_ALL ||
  ...
  for (i = 0; i < num_bufs; i++) {
    ...
    enum rte_comp_huffman huffman_type = compress_xform->deflate.huffman;
    char engine[] = "zlib (directly, not PMD)";
    if (zlib_decompress)
      strlcpy(engine, "PMD", sizeof(engine));

    RTE_LOG(DEBUG, USER1,
            "Buffer %u compressed by %s from %u to"
            " %u bytes (level = %d, huffman = %s)\n",
            i, engine, ops_processed[i]->consumed, ops_processed[i]->produced,
            compress_xform->level, huffman_type_strings[huffman_type]);
    RTE_LOG(DEBUG, USER1, "Compression ratio = %.2f\n",
            ops_processed[i]->consumed == 0
                ? 0
  ...
  for (i = 0; i < num_bufs; i++) {
    ...
    if (ops_processed[i]->status != RTE_COMP_OP_STATUS_SUCCESS) {
      ...
      RTE_LOG(ERR, USER1, "Comp: Some operations were not successful\n");
      return -1;
    }
    priv_data = (struct priv_op_data *)(ops_processed[i] + 1);
 ...
 *   - -1: On error.
 */
static int test_deflate_decomp_run(const struct interim_data_params *int_data,
                                   const struct test_data_params *test_data,
  ...
  if (zlib_decompress) {
    ...
    if (operation_type == RTE_COMP_OP_STATELESS) {
      ...
      for (i = 0; i < num_xforms; i++) {
        ret = rte_compressdev_private_xform_create(
            0, (const struct rte_comp_xform *)decompress_xforms[i],
            &priv_xforms[i]);
        if (ret < 0) {
          RTE_LOG(ERR, USER1,
      ...
      if (capa->comp_feature_flags & RTE_COMP_FF_SHAREABLE_PRIV_XFORM) {
        ...
        for (i = num_xforms; i < num_bufs; i++) {
          ret = rte_compressdev_private_xform_create(
              0, decompress_xforms[i % num_xforms], &priv_xforms[i]);
          if (ret < 0) {
            RTE_LOG(ERR, USER1,
                    "Decompression private xform"
                    " could not be created\n");
      ...
      ret = rte_compressdev_stream_create(0, decompress_xforms[0], stream);
      if (ret < 0) {
        RTE_LOG(ERR, USER1,
...
test_deflate_decomp_finalize(const struct interim_data_params *int_data,
  ...
  for (i = 0; i < num_bufs; i++) {
    ...
    if (zlib_compress)
      strlcpy(engine, "pmd", sizeof(engine));
    RTE_LOG(DEBUG, USER1, "Buffer %u decompressed by %s from %u to %u bytes\n",
            i, engine, ops_processed[i]->consumed, ops_processed[i]->produced);
    ops[i] = NULL;
  }
  ...
  for (i = 0; i < num_bufs; i++) {
    ...
    if (operation_type == RTE_COMP_OP_STATEFUL &&
        (ops_processed[i]->status ==
             RTE_COMP_OP_STATUS_OUT_OF_SPACE_RECOVERABLE ||
         ops_processed[i]->status == RTE_COMP_OP_STATUS_SUCCESS)) {

      RTE_LOG(DEBUG, USER1, ".............RECOVERABLE\n");

      /* collect the output into all_decomp_data */
      const void *ptr = rte_pktmbuf_read(
          ops_processed[i]->m_dst, ops_processed[i]->dst.offset,
          ops_processed[i]->produced,
          *all_decomp_data + *decomp_produced_data_size);
      if (ptr != *all_decomp_data + *decomp_produced_data_size)
        rte_memcpy(*all_decomp_data + *decomp_produced_data_size, ptr,
                   ops_processed[i]->produced);

      *decomp_produced_data_size += ops_processed[i]->produced;
      if (ops_processed[i]->src.length > ops_processed[i]->consumed) {
        if (ops_processed[i]->status == RTE_COMP_OP_STATUS_SUCCESS) {
          RTE_LOG(ERR, USER1, "Operation finished too early\n");
          return -1;
        }
        step++;
        if (step >= test_data->decompress_steps_max) {
          RTE_LOG(ERR, USER1, "Operation exceeded maximum steps\n");
          return -1;
        }
        ...
        if (compare_buffers(buf1, strlen(buf1) + 1, buf2,
                            *decomp_produced_data_size) < 0)
          return -1;
        /* Test checksums */
        if (compress_xforms[0]->compress.chksum != RTE_COMP_CHECKSUM_NONE) {
          if (ops_processed[i]->output_chksum != compress_checksum[i]) {
            RTE_LOG(ERR, USER1,
    ...
    } else if (ops_processed[i]->status != RTE_COMP_OP_STATUS_SUCCESS) {
      RTE_LOG(ERR, USER1,
...
test_results_validation(const struct interim_data_params *int_data,
  ...
  for (i = 0; i < num_bufs; i++) {
    priv_data = (struct priv_op_data *)(ops_processed[i] + 1);
    buf1 = test_data->use_external_mbufs ? test_data->inbuf_memzone->addr
                                         : test_bufs[priv_data->orig_idx];
    data_size = test_data->use_external_mbufs ? test_data->inbuf_data_size
                                              : strlen(buf1) + 1;

    contig_buf = rte_malloc(NULL, ops_processed[i]->produced, 0);
    if (contig_buf == NULL) {
      RTE_LOG(ERR, USER1,
              "Contiguous buffer could not "
              "be allocated\n");
      goto exit;
    }
    ...
    if (compare_buffers(buf1, data_size, buf2, ops_processed[i]->produced) < 0)
      goto exit;
...
static int test_deflate_comp_decomp(const struct interim_data_params *int_data,
                                    const struct test_data_params *test_data) {
  unsigned int num_bufs = int_data->num_bufs;
  unsigned int out_of_space = test_data->out_of_space;
  ...
  if (capa == NULL) {
    RTE_LOG(ERR, USER1, "Compress device does not support DEFLATE\n");
    return -1;
  }
  ...
  ret = test_setup_output_bufs(
      OPERATION_COMPRESSION, out_of_space == 1 && !zlib_compress,
      &test_priv_data, int_data, test_data, &compbuf_info);
  if (ret < 0) {
    ret_status = -1;
    goto exit;
  }
  ...
  ret = test_setup_output_bufs(
      OPERATION_DECOMPRESSION, out_of_space == 1 && !zlib_decompress,
      &test_priv_data, int_data, test_data, &decompbuf_info);
  if (ret < 0) {
    ret_status = -1;
    goto exit;
  }
  ...
  next_step: /* next step for stateful decompression only */
    ...
    if (ret < 0) {
      ret_status = -1;
      RTE_LOG(ERR, USER1, "Decompression: enqueue/dequeue operation failed\n");
    }
  }
...
static int test_compressdev_deflate_stateless_fixed(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i;
  int ret;
  ...
  struct interim_data_params int_data = {
      NULL, 1, NULL, &compress_xform, &ts_params->def_decomp_xform, 1};

  struct test_data_params test_data = {.compress_state = RTE_COMP_OP_STATELESS,
                                       .decompress_state =
...
static int test_compressdev_deflate_stateless_dynamic(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i;
  int ret;
  struct rte_comp_xform *compress_xform =
...
static int test_compressdev_deflate_stateless_multi_op(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  ...
  struct interim_data_params int_data = {compress_test_bufs,
                                         num_bufs,
                                         buf_idx,
                                         &ts_params->def_comp_xform,
                                         &ts_params->def_decomp_xform,
...
static int test_compressdev_deflate_stateless_multi_level(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  unsigned int level;
  uint16_t i;
  int ret;
  ...
  for (i = 0; i < RTE_DIM(compress_test_bufs); i++) {
    ...
    for (level = RTE_COMP_LEVEL_MIN; level <= RTE_COMP_LEVEL_MAX; level++) {
...
#define NUM_XFORMS 3
static int test_compressdev_deflate_stateless_multi_xform(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  ...
  for (i = 0; i < NUM_XFORMS; i++) {
    compress_xforms[i] = rte_malloc(NULL, sizeof(struct rte_comp_xform), 0);
    ...
    decompress_xforms[i] = rte_malloc(NULL, sizeof(struct rte_comp_xform), 0);
  ...
  struct interim_data_params int_data = {test_buffers,      num_bufs,
                                         buf_idx,           compress_xforms,
                                         decompress_xforms, NUM_XFORMS};
...
static int test_compressdev_deflate_stateless_sgl(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i;
  int ret;
  const struct rte_compressdev_capabilities *capab;
  ...
  struct interim_data_params int_data = {
      NULL, 1, NULL, &ts_params->def_comp_xform, &ts_params->def_decomp_xform,
      1};
...
static int test_compressdev_deflate_stateless_checksum(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i;
  int ret;
  const struct rte_compressdev_capabilities *capab;
  ...
  struct interim_data_params int_data = {
      NULL, 1, NULL, &compress_xform, &decompress_xform, 1};
  ...
  if ((capab->comp_feature_flags & RTE_COMP_FF_CRC32_ADLER32_CHECKSUM)) {
    compress_xform->compress.chksum = RTE_COMP_CHECKSUM_CRC32_ADLER32;
    decompress_xform->decompress.chksum = RTE_COMP_CHECKSUM_CRC32_ADLER32;
...
static int test_compressdev_out_of_space_buffer(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  int ret;
  uint16_t i;
  const struct rte_compressdev_capabilities *capab;
  ...
  struct interim_data_params int_data = {
      &compress_test_bufs[0],       1, &i, &ts_params->def_comp_xform,
      &ts_params->def_decomp_xform, 1};
...
static int test_compressdev_deflate_stateless_dynamic_big(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret;
  unsigned int j;
  ...
  if (test_buffer == NULL) {
    RTE_LOG(ERR, USER1, "Can't allocate buffer for big-data\n");
    return TEST_FAILED;
  }

  struct interim_data_params int_data = {
      (const char *const *)&test_buffer, 1, &i, &ts_params->def_comp_xform,
      &ts_params->def_decomp_xform,      1};
...
static int test_compressdev_deflate_stateful_decomp(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  int ret;
  uint16_t i;
  const struct rte_compressdev_capabilities *capab;
...
static int test_compressdev_deflate_stateful_decomp_checksum(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  int ret;
  uint16_t i;
  const struct rte_compressdev_capabilities *capab;
  ...
  struct interim_data_params int_data = {
      &compress_test_bufs[0], 1, &i, &compress_xform, &decompress_xform, 1};
...
static const struct rte_memzone *make_memzone(const char *name, size_t size) {
  ...
  if (memzone == NULL) {
    memzone = rte_memzone_reserve_aligned(
        mz_name, size, socket_id, RTE_MEMZONE_IOVA_CONTIG, RTE_CACHE_LINE_SIZE);
    if (memzone == NULL)
      RTE_LOG(ERR, USER1, "Can't allocate memory zone %s", mz_name);
  }
  return memzone;
}

static int test_compressdev_external_mbufs(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  size_t data_len = 0;
  uint16_t i;
  int ret = TEST_FAILED;
  ...
  struct test_data_params test_data = {
      ...
      .compbuf_memzone =
          make_memzone("compbuf", data_len * COMPRESS_BUF_SIZE_RATIO),
  ...
  for (i = 0; i < RTE_DIM(compress_test_bufs); i++) {
    ...
    rte_memcpy(test_data.inbuf_memzone->addr, compress_test_bufs[i], data_len);
...
static int test_compressdev_deflate_stateless_fixed_oos_recoverable(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i;
  int ret;
  int comp_result;
...
static int test_compressdev_deflate_im_buffers_LB_1op(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  ...
  if (test_buffer == NULL) {
    RTE_LOG(ERR, USER1, "Can't allocate buffer for 'im buffer' test\n");
    return TEST_FAILED;
  }
...
static int test_compressdev_deflate_im_buffers_LB_2ops_first(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
  ...
  struct interim_data_params int_data = {
      (const char *const *)test_buffers, 2, &i, &ts_params->def_comp_xform,
      &ts_params->def_decomp_xform,      1};
...
static int test_compressdev_deflate_im_buffers_LB_2ops_second(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
...
static int test_compressdev_deflate_im_buffers_LB_3ops(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
  ...
  struct interim_data_params int_data = {
      (const char *const *)test_buffers, 3, &i, &ts_params->def_comp_xform,
      &ts_params->def_decomp_xform,      1};
...
static int test_compressdev_deflate_im_buffers_LB_4ops(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
  ...
  struct interim_data_params int_data = {
      (const char *const *)test_buffers, 4, &i, &ts_params->def_comp_xform,
      &ts_params->def_decomp_xform,      1};
...
static int test_compressdev_deflate_im_buffers_SGL_1op(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
...
static int test_compressdev_deflate_im_buffers_SGL_2ops_first(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
...
static int test_compressdev_deflate_im_buffers_SGL_2ops_second(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
...
static int test_compressdev_deflate_im_buffers_SGL_3ops(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
...
static int test_compressdev_deflate_im_buffers_SGL_4ops(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
...
static int test_compressdev_deflate_im_buffers_SGL_over_1op(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
...
static int test_compressdev_deflate_im_buffers_SGL_over_2ops_first(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
...
static int test_compressdev_deflate_im_buffers_SGL_over_2ops_second(void) {
  struct comp_testsuite_params *ts_params = &testsuite_params;
  uint16_t i = 0;
  int ret = TEST_SUCCESS;
  int j;
...
static struct unit_test_suite compressdev_testsuite = {
    ...
    .unit_test_cases = {
        TEST_CASE_ST(NULL, NULL, test_compressdev_invalid_configuration),
...
static int test_compressdev(void) {
