...
enum plock_use_type { USE_MB, USE_SMP_MB, USE_NUM };
...
static inline void store_load_barrier(uint32_t utype) {
  if (utype == USE_MB)
    rte_mb();
 ...
 */
static void plock_lock(struct plock *l, uint32_t self) {
  uint32_t other;
...
static void plock_unlock(struct plock *l, uint32_t self) {
  rte_smp_wmb();
  l->flag[self] = 0;
}

static void plock_reset(struct plock *l, enum plock_use_type utype) {
  memset(l, 0, sizeof(*l));
  l->utype = utype;
}
 ...
 */
static void plock_add(struct plock_test *pt, uint32_t self, uint32_t n) {
...
static int plock_test1_lcore(void *data) {
  uint64_t tm;
  uint32_t lc, ln;
  uint64_t i, n;
  ...
  printf("%s(%u): %" PRIu64 " iterations finished, in %" PRIu64
         ...
         __func__, lc, i, tm, (long double)tm / i, lpt->sum[0], lpt->sum[1]);
  return 0;
}
 ...
 */
static int plock_test(uint64_t iter, enum plock_use_type utype) {
  int32_t rc;
  uint32_t i, lc, n;
  uint64_t *sum;
