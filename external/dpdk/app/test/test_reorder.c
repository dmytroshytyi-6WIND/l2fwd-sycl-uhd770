...
static struct reorder_unittest_params default_params = {.p = NULL, .b = NULL};

static struct reorder_unittest_params *test_params = &default_params;

static int test_reorder_create(void) {
...
static int test_reorder_init(void) {
  struct rte_reorder_buffer *b = NULL;
  unsigned int size;
  /*
...
static int test_reorder_find_existing(void) {
  struct rte_reorder_buffer *b = NULL;
...
static int test_reorder_free(void) {
...
static int test_reorder_insert(void) {
  struct rte_reorder_buffer *b = NULL;
...
static int test_reorder_drain(void) {
  struct rte_reorder_buffer *b = NULL;
  struct rte_mempool *p = test_params->p;
  const unsigned int size = 4;
...
static int test_setup(void) {
  ...
  if (test_params->b == NULL) {
    test_params->b =
        rte_reorder_create("PKT_RO1", rte_socket_id(), REORDER_BUFFER_SIZE);
    if (test_params->b == NULL) {
  ...
  if (test_params->p == NULL) {
    test_params->p =
        rte_pktmbuf_pool_create("RO_MBUF_POOL", NUM_MBUFS, BURST, 0,
...
static struct unit_test_suite reorder_test_suite = {
    ...
    .unit_test_cases = {
        TEST_CASE(test_reorder_create), TEST_CASE(test_reorder_init),
        TEST_CASE(test_reorder_find_existing), TEST_CASE(test_reorder_free),
        TEST_CASE(test_reorder_insert), TEST_CASE(test_reorder_drain),
        TEST_CASES_END()}};
