 ...
 */
static int test_cb(const char *cmd __rte_unused,
 ...
 */
static int test_output(const char *func_name, const char *expected) {
  int bytes;
...
static int test_dict_with_array_int_values(void) {
  int i;
  ...
  rte_tel_data_add_dict_container(&response_data, "dict_0", child_data, 0);
  rte_tel_data_add_dict_container(&response_data, "dict_1", child_data2, 0);

  return TEST_OUTPUT("{\"/test\":{\"dict_0\":[0,1,2,3,4],"
                     "\"dict_1\":[0,1,2,3,4]}}");
}

static int test_array_with_array_int_values(void) {
  int i;
...
static int test_case_array_int(void) {
  int i;
  memset(&response_data, 0, sizeof(response_data));
...
static int test_case_add_dict_int(void) {
  int i = 0;
...
static int test_case_array_string(void) {
  memset(&response_data, 0, sizeof(response_data));
...
static int test_case_add_dict_string(void) {
  memset(&response_data, 0, sizeof(response_data));
  rte_tel_data_start_dict(&response_data);
...
static int test_dict_with_array_string_values(void) {
  struct rte_tel_data *child_data = rte_tel_data_alloc();
...
static int test_dict_with_dict_values(void) {
  ...
  rte_tel_data_add_array_string(child_data, "aaaa");
  rte_tel_data_add_array_string(child_data2, "bbbb");
  rte_tel_data_add_dict_container(dict_of_dicts, "dict_0", child_data, 0);
  rte_tel_data_add_dict_container(dict_of_dicts, "dict_1", child_data2, 0);
...
static int test_array_with_array_string_values(void) {
  struct rte_tel_data *child_data = rte_tel_data_alloc();
  rte_tel_data_start_array(child_data, RTE_TEL_STRING_VAL);
...
static int test_case_array_u64(void) {
  int i;
  memset(&response_data, 0, sizeof(response_data));
...
static int test_case_add_dict_u64(void) {
  int i = 0;
  char name_of_value[8];
...
static int test_dict_with_array_u64_values(void) {
  int i;
...
static int test_array_with_array_u64_values(void) {
  int i;
...
static int connect_to_socket(void) {
  ...
  snprintf(telem_addr.sun_path, sizeof(telem_addr.sun_path),
           ...
           TELEMETRY_VERSION);
  if (connect(sock, (struct sockaddr *)&telem_addr, sizeof(telem_addr)) < 0) {
...
static int test_telemetry_data(void) {
