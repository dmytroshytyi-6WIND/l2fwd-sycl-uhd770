...
#define ERR_RETURN(...)                                                        \
  do {                                                                         \
    print_err(__func__, __LINE__, __VA_ARGS__);                                \
    return -1;                                                                 \
  } while (0)
...
static void __rte_format_printf(3, 4)
...
static int runtest(const char *printable,
                   int (*test_fn)(int16_t dev_id, uint16_t vchan),
                   int iterations, int16_t dev_id, uint16_t vchan,
...
static void await_hw(int16_t dev_id, uint16_t vchan) {
  ...
  /* for those that do, *max* end time is one second from now, but all should be
   * faster */
...
/* run a series of copy tests just using some different options for enqueues and
 * completions */
static int do_multi_copies(
    ...
    int use_completed_status) /* use completed or completed_status function */
{
  ...
  if (split_completions) {
    ...
    if (ret != half_len || dma_err)
      ERR_RETURN("Error with rte_dma_completed - first half. ret = %d, "
                 "expected ret = %u, dma_err = %d\n",
                 ret, half_len, dma_err);

    ret = rte_dma_completed(dev_id, vchan, half_len, NULL, &dma_err);
    if (ret != half_len || dma_err)
      ERR_RETURN("Error with rte_dma_completed - second half. ret = %d, "
                 "expected ret = %u, dma_err = %d\n",
                 ret, half_len, dma_err);
  } else {
    ...
    if (!use_completed_status) {
      ...
      if (n != RTE_DIM(srcs) || dma_err)
        ERR_RETURN(
            "Error with rte_dma_completed, %u [expected: %zu], dma_err = %d\n",
      ...
      for (j = 0; j < (uint16_t)n; j++)
        if (sc[j] != RTE_DMA_STATUS_SUCCESSFUL)
          ERR_RETURN("Error with rte_dma_completed_status, job %u reports "
                     "failure [code %u]\n",
                     j, sc[j]);
    }
  }
...
static int test_enqueue_copies(int16_t dev_id, uint16_t vchan) {
  unsigned int i;
  uint16_t id;
  ...
  do {
    ...
    id = rte_dma_copy(dev_id, vchan, rte_pktmbuf_iova(src),
                      rte_pktmbuf_iova(dst), COPY_LEN, RTE_DMA_OP_FLAG_SUBMIT);
    if (id != id_count)
  ...
  do {
    ...
    for (i = 0; i < max_ops; i++)
      if (rte_dma_copy(dev_id, vchan, rte_pktmbuf_iova(src),
                       rte_pktmbuf_iova(dst), COPY_LEN,
  ...
  return do_multi_copies(dev_id, vchan, 0, 0,
                         0) /* enqueue and complete 1 batch at a time */
...
static int test_failure_in_full_burst(int16_t dev_id, uint16_t vchan,
                                      bool fence, struct rte_mbuf **srcs,
                                      struct rte_mbuf **dsts,
                                      unsigned int fail_idx) {
  ...
  for (i = 0; i < COMP_BURST_SZ; i++) {
    int id = rte_dma_copy(
        ...
        dsts[i]->buf_iova + dsts[i]->data_off, COPY_LEN, OPT_FENCE(i));
    if (id < 0)
      ERR_RETURN("Error with rte_dma_copy for buffer %u\n", i);
    if (i == fail_idx)
      invalid_addr_id = id;
  ...
  if (count != fail_idx)
    ERR_RETURN("Error with rte_dma_completed for failure test. Got returned %u "
               "not %u.\n",
               count, fail_idx);
  if (!error)
    ERR_RETURN(
        "Error, missing expected failed copy, %u. has_error is not set\n",
        fail_idx);
  if (idx != invalid_addr_id - 1)
    ERR_RETURN(
        "Error, missing expected failed copy, %u. Got last idx %u, not %u\n",
  ...
  status_count =
      rte_dma_completed_status(dev_id, vchan, COMP_BURST_SZ, &idx, status);
  /* some HW may stop on error and be restarted after getting error status for
   * single value To handle this case, if we get just one error back, wait for
   * more completions and get status for rest of the burst
   */
  if (status_count == 1) {
    await_hw(dev_id, vchan);
  ...
  if (status_count != COMP_BURST_SZ - count)
    ERR_RETURN(
        "Error with completed_status calls for fail idx %u. Got %u not %u\n",
        fail_idx, status_count, COMP_BURST_SZ - count);
  /* now verify just one failure followed by multiple successful or skipped
   * entries */
  if (status[0] == RTE_DMA_STATUS_SUCCESSFUL)
    ERR_RETURN("Error with status returned for fail idx %u. First status was "
               "not failure\n",
               fail_idx);
  for (i = 1; i < status_count; i++)
    ...
    if (status[i] != RTE_DMA_STATUS_SUCCESSFUL &&
      ...
      ERR_RETURN("Error with status calls for fail idx %u. Status for job %u "
                 "(of %u) is not successful\n",
...
static int test_individual_status_query_with_failure(int16_t dev_id,
                                                     uint16_t vchan, bool fence,
  ...
  for (j = 0; j < COMP_BURST_SZ; j++) {
    int id = rte_dma_copy(
        ...
        dsts[j]->buf_iova + dsts[j]->data_off, COPY_LEN, OPT_FENCE(j));
    if (id < 0)
      ERR_RETURN("Error with rte_dma_copy for buffer %u\n", j);
    if (j == fail_idx)
      invalid_addr_id = id;
  ...
  while (!error) {
    ...
    if (n == 0 && !error)
      ERR_RETURN(
          "Error, unexpectedly got zero completions after %u completed\n",
          count);
  }
  if (idx != invalid_addr_id - 1)
    ERR_RETURN(
        "Error, last successful index not as expected, got %u, expected %u\n",
        idx, invalid_addr_id - 1);

  /* use completed_status until we hit end of burst */
  while (count + status_count < COMP_BURST_SZ) {
    uint16_t n =
        rte_dma_completed_status(dev_id, vchan, 1, &idx, &status[status_count]);
...
static int test_single_item_status_query_with_failure(int16_t dev_id,
                                                      uint16_t vchan,
                                                      struct rte_mbuf **srcs,
                                                      struct rte_mbuf **dsts,
                                                      unsigned int fail_idx) {
  ...
  for (j = 0; j < COMP_BURST_SZ; j++) {
    int id = rte_dma_copy(
        dev_id, vchan,
        (j == fail_idx ? 0 : (srcs[j]->buf_iova + srcs[j]->data_off)),
        dsts[j]->buf_iova + dsts[j]->data_off, COPY_LEN, 0);
    if (id < 0)
      ERR_RETURN("Error with rte_dma_copy for buffer %u\n", j);
    if (j == fail_idx)
      invalid_addr_id = id;
  ...
  if (error == true)
    ERR_RETURN("Error, got further errors post completed_status() call, for "
               "failure case %u.\n",
               fail_idx);
  if (count + status_count + count2 != COMP_BURST_SZ)
    ERR_RETURN(
        "Error, incorrect number of completions received, got %u not %u\n",
        count + status_count + count2, COMP_BURST_SZ);

  return 0;
}

static int test_multi_failure(int16_t dev_id, uint16_t vchan,
  ...
  while (count < COMP_BURST_SZ) {
    ...
    uint16_t ret = rte_dma_completed_status(
        dev_id, vchan, COMP_BURST_SZ - count, NULL, &status[count]);
    if (ret == 0)
  ...
  if (err_count != num_fail)
    ERR_RETURN("Error: Invalid number of failed completions returned, %u; "
               "expected %zu\n",
               err_count, num_fail);
  ...
  while (count + err_count < COMP_BURST_SZ) {
    ...
    if (error) {
      uint16_t ret = rte_dma_completed_status(dev_id, vchan, 1, NULL, status);
      if (ret != 1)
  ...
  if (err_count != num_fail)
    ERR_RETURN("Error: Incorrect number of failed completions received, got %u "
               "not %zu\n",
               err_count, num_fail);
...
static int test_completion_status(int16_t dev_id, uint16_t vchan, bool fence) {
  ...
  for (i = 0; i < RTE_DIM(fail); i++) {
    if (test_failure_in_full_burst(dev_id, vchan, fence, srcs, dsts, fail[i]) <
        0)
      return -1;

    if (test_individual_status_query_with_failure(dev_id, vchan, fence, srcs,
                                                  dsts, fail[i]) < 0)
      return -1;

    /* test is run the same fenced, or unfenced, but no harm in running it twice
     */
    if (test_single_item_status_query_with_failure(dev_id, vchan, srcs, dsts,
                                                   fail[i]) < 0)
      return -1;
  }
...
static int test_completion_handling(int16_t dev_id, uint16_t vchan) {
  return test_completion_status(dev_id, vchan, false)    /* without fences */
         || test_completion_status(dev_id, vchan, true); /* with fences */
}

static int test_enqueue_fill(int16_t dev_id, uint16_t vchan) {
  ...
  for (i = 0; i < RTE_DIM(lengths); i++) {
    ...
    int id = rte_dma_fill(dev_id, vchan, pattern, rte_pktmbuf_iova(dst),
                          lengths[i], RTE_DMA_OP_FLAG_SUBMIT);
    if (id < 0)
    ...
    for (j = 0; j < lengths[i]; j++) {
      ...
      if (dst_data[j] != pat_byte)
        ERR_RETURN(
            "Error with fill operation (lengths = %u): got (%x), not (%x)\n",
    ...
    for (; j < rte_pktmbuf_data_len(dst); j++)
      if (dst_data[j] != 0)
        ERR_RETURN("Error, fill operation wrote too far (lengths = %u): got "
                   "(%x), not (%x)\n",
...
static int test_burst_capacity(int16_t dev_id, uint16_t vchan) {
#define CAP_TEST_BURST_SIZE 64
  ...
  for (iter = 0; iter < 2 * (((int)UINT16_MAX + 1) / ring_space); iter++) {
    ...
    for (i = 0; i < (ring_space / (2 * CAP_TEST_BURST_SIZE)) + 1; i++) {
      ret =
          rte_dma_completed(dev_id, vchan, CAP_TEST_BURST_SIZE, NULL, &dma_err);
      if (ret != CAP_TEST_BURST_SIZE || dma_err) {
        enum rte_dma_status_code status;

        rte_dma_completed_status(dev_id, vchan, 1, NULL, &status);
        ERR_RETURN("Error with rte_dmadev_completed, %u [expected: %u], "
    ...
    cap = rte_dma_burst_capacity(dev_id, vchan);
    if (cap != ring_space)
      ERR_RETURN("Error, ring capacity has not reset to original value, got "
                 "%u, expected %u\n",
                 cap, ring_space);
  }
...
static int test_dmadev_instance(int16_t dev_id) {
...
#define CHECK_ERRS true
  ...
  if (rte_dma_burst_capacity(dev_id, vchan) < 32)
    ERR_RETURN(
        "Error: Device does not have sufficient burst capacity to run tests");
  ...
  if (rte_dma_burst_capacity(dev_id, vchan) < 64)
    printf("DMA Dev %u: insufficient burst capacity (64 required), skipping "
           "tests\n",
           dev_id);
  else if (runtest("burst capacity", test_burst_capacity, 1, dev_id, vchan,
                   CHECK_ERRS) < 0)
    goto err;

  /* to test error handling we can provide null pointers for source or dest in
   * copies. This requires VA mode in DPDK, since NULL(0) is a valid physical
   * address. We also need hardware that can report errors back.
   */
  if (rte_eal_iova_mode() != RTE_IOVA_VA)
    ...
    printf("DMA Dev %u: device does not report errors, skipping error handling "
           "tests\n",
           dev_id);
  else if (runtest("error handling", test_completion_handling, 1, dev_id, vchan,
                   !CHECK_ERRS) < 0)
    goto err;
