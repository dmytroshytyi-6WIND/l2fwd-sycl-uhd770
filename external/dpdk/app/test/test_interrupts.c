...
static int test_interrupt_init(void) {
  ...
  for (i = 0; i < TEST_INTERRUPT_HANDLE_MAX; i++) {
    intr_handles[i] = rte_intr_instance_alloc(RTE_INTR_INSTANCE_F_PRIVATE);
    if (!intr_handles[i])
      return -1;
  }
 ...
 */
static int test_interrupt_deinit(void) {
  int i;
 ...
 */
static int test_interrupt_trigger_interrupt(void) {
...
test_interrupt_handle_compare(struct rte_intr_handle *intr_handle_l,
  ...
  if (rte_intr_fd_get(intr_handle_l) != rte_intr_fd_get(intr_handle_r) ||
      rte_intr_type_get(intr_handle_l) != rte_intr_type_get(intr_handle_r))
    return -1;
...
static int test_interrupt_init(void) { return 0; }

static int test_interrupt_deinit(void) { return 0; }

static int test_interrupt_trigger_interrupt(void) { return 0; }
 ...
 */
static void test_interrupt_callback(void *arg) {
  ...
  if (rte_intr_callback_unregister(intr_handle, test_interrupt_callback, arg) >=
      0) {
 ...
 * Callback for the test interrupt.
 */
static void test_interrupt_callback_1(void *arg) {
  struct rte_intr_handle *intr_handle = arg;
  if (test_interrupt_handle_sanity_check(intr_handle) < 0) {
    printf("null or invalid intr_handle for %s\n", __func__);
    flag = -1;
 ...
 */
static int test_interrupt_disable(void) {
  struct rte_intr_handle *test_intr_handle;
...
test_interrupt_full_path_check(enum test_interrupt_handle_type intr_type) {
  ...
  if (rte_intr_callback_register(test_intr_handle, test_interrupt_callback,
                                 test_intr_handle) < 0) {
  ...
  rte_delay_ms(TEST_INTERRUPT_CHECK_INTERVAL);
  while ((count = rte_intr_callback_unregister(test_intr_handle,
...
static int test_interrupt(void) {
  ...
  if (test_interrupt_full_path_check(TEST_INTERRUPT_HANDLE_VALID_UIO) < 0) {
  ...
  if (test_interrupt_full_path_check(TEST_INTERRUPT_HANDLE_VALID_DEV_EVENT) <
      0) {
  ...
  if (test_interrupt_full_path_check(TEST_INTERRUPT_HANDLE_VALID_ALARM) < 0) {
  ...
  if (rte_intr_callback_register(NULL, test_interrupt_callback, NULL) == 0) {
  ...
  test_intr_handle = intr_handles[TEST_INTERRUPT_HANDLE_VALID];
  if (rte_intr_callback_register(test_intr_handle, NULL, test_intr_handle) ==
      0) {
  ...
  if (rte_intr_callback_unregister(NULL, test_interrupt_callback, NULL) > 0) {
  ...
  test_intr_handle = intr_handles[TEST_INTERRUPT_HANDLE_INVALID];
  if (rte_intr_callback_unregister(test_intr_handle, test_interrupt_callback,
                                   test_intr_handle) > 0) {
    printf("unexpectedly unregister successfully with "
           "invalid intr_handle\n");
    goto out;
  ...
  if (rte_intr_callback_register(test_intr_handle, test_interrupt_callback_1,
                                 test_intr_handle) < 0) {
  ...
  if (rte_intr_callback_unregister(test_intr_handle, test_interrupt_callback_1,
                                   (void *)-1) <= 0) {
...
out:
  ...
  test_intr_handle = intr_handles[TEST_INTERRUPT_HANDLE_VALID];
  rte_intr_callback_unregister(test_intr_handle, test_interrupt_callback,
                               (void *)-1);
  rte_intr_callback_unregister(test_intr_handle, test_interrupt_callback_1,
                               (void *)-1);
