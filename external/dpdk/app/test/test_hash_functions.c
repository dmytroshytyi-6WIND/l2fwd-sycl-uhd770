...
static uint32_t hash_values_jhash[2][12] = {
    {0x8ba9414b, 0xdf0d39c9, 0xe4cf1d42, 0xd4ccb93c, 0x5e84eafc, 0x21362cfe,
     0x2f4775ab, 0x9ff036cc, 0xeca51474, 0xbc9d6816, 0x12926a31, 0x1c9fa888},
    {0x5c62c303, 0x1b8cf784, 0x8270ac65, 0x05fa6668, 0x762df861, 0xda088f2f,
     0x59614cd4, 0x7a94f690, 0xdc1e4993, 0x30825494, 0x91d0e462, 0x768087fc}};
static uint32_t hash_values_crc[2][12] = {
    {0x00000000, 0xf26b8303, 0x91545164, 0x06040eb1, 0x9bb99201, 0xcc4c4fe4,
     0x14a90993, 0xf8a5dd8c, 0xcaa1ad0b, 0x7ac1e03e, 0x43f44466, 0x4a11475e},
    {0xbdfd3980, 0x70204542, 0x98cd4c70, 0xd52c702f, 0x41fc0e1c, 0x3905f65c,
     0x94bff47f, 0x1bab102d, 0xf4a2c645, 0xbf441539, 0x789c104f, 0x53028d3e}};
 ...
 */
static void run_hash_func_perf_test(uint32_t key_len, uint32_t init_val,
                                    rte_hash_function f) {
 ...
 */
static void run_hash_func_perf_tests(void) {
  unsigned i, j, k;
  ...
  for (i = 0; i < RTE_DIM(hashtest_initvals); i++) {
    for (j = 0; j < RTE_DIM(hashtest_key_lens); j++) {
      for (k = 0; k < RTE_DIM(hashtest_funcs); k++) {
        run_hash_func_perf_test(hashtest_key_lens[j], hashtest_initvals[i],
                                hashtest_funcs[k]);
      }
    }
  }
 ...
 */
static int verify_precalculated_hash_func_tests(void) {
  unsigned i, j;
  uint8_t key[64];
  uint32_t hash;
 ...
 */
static int verify_jhash_32bits(void) {
  unsigned i, j;
  uint8_t key[64];
    ...
    for (j = 0; j < RTE_DIM(hashtest_initvals); j++) {
      ...
      if ((hashtest_key_lens[i] & 0x3) == 0) {
        ...
        hash32 = rte_jhash_32b((const unaligned_uint32_t *)key,
                               hashtest_key_lens[i] >> 2, hashtest_initvals[j]);
        if (hash != hash32) {
 ...
 */
static int verify_jhash_words(void) {
  unsigned i;
  uint32_t key[3];
 ...
 */
static int run_hash_func_tests(void) {
