...
int run_pdump_client_tests(void) {
  ...
  for (itr = 0; itr < NUM_ITR; itr++) {
    ret = rte_pdump_enable(portid, QUEUE_ID, flags, ring_client, mp, NULL);
    if (ret < 0) {
    ...
    ret = rte_pdump_enable_by_deviceid(deviceid, QUEUE_ID, flags, ring_client,
                                       mp, NULL);
    if (ret < 0) {
...
int run_pdump_server_tests(void) {
  ...
  const char *const argv1[] = {prgname, "-c", coremask, "--proc-type=secondary",
                               prefix};
