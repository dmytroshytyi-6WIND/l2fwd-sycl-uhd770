...
const char test_resource_dpdk_blob[] = {'\x44', '\x50', '\x44', '\x4b', '\x00'};

REGISTER_RESOURCE(test_resource_dpdk, test_resource_dpdk_blob,
                  test_resource_dpdk_blob + 4);
...
static int test_resource_dpdk(void) {
  ...
  TEST_ASSERT(!strcmp(r->name, "test_resource_dpdk"),
              "Found resource %s, expected test_resource_dpdk", r->name);

  TEST_ASSERT(!strncmp("DPDK", r->begin, 4), "Unexpected payload: %.4s...",
              r->begin);
...
static int test_resource_tar(void) {
  ...
  TEST_ASSERT(!strcmp(r->name, "test_resource_tar"),
              "Found resource %s, expected test_resource_tar", r->name);

  TEST_ASSERT_SUCCESS(resource_untar(r), "Failed to to untar %s", r->name);

  f = fopen("test_resource.c", "r");
  TEST_ASSERT_NOT_NULL(f, "Missing extracted file test_resource.c");
  fclose(f);
