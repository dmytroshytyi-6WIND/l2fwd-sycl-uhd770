...
static int cplane_encap(uint32_t sn_size, uint8_t dir,
  ...
  switch (sn_size) {
  case 5:
    i = PDCP_CPLANE_OFFSET + enc_alg_off + auth_alg_off +
        ((dir == 0) ? UPLINK : DOWNLINK);
    break;
  case 12:
    i = PDCP_CPLANE_LONG_SN_OFFSET + enc_alg_off + auth_alg_off +
        ((dir == 0) ? UPLINK : DOWNLINK);
    break;
  default:
    printf("\nInvalid SN: %u for %s\n", sn_size, __func__);
...
static int uplane_encap_no_integrity(uint32_t sn_size, uint8_t dir,
                                     enum enc_alg_off enc_alg_off) {
  int i = PDCP_UPLANE_OFFSET + ((dir == 0) ? UPLINK : DOWNLINK) + enc_alg_off;
...
static int uplane_encap_with_integrity(uint32_t sn_size, uint8_t dir,
  ...
  switch (sn_size) {
  case 12:
    i = PDCP_UPLANE_12BIT_OFFSET + enc_alg_off + auth_alg_off +
        ((dir == 0) ? UPLINK : DOWNLINK);
    break;
  case 18:
    i = PDCP_UPLANE_18BIT_OFFSET + enc_alg_off + auth_alg_off +
        ((dir == 0) ? UPLINK : DOWNLINK);
    break;
  default:
    printf("\nInvalid SN: %u\n", sn_size);
...
int test_PDCP_PROTO_uplane_encap_all(void) {
  ...
  TEST_PDCP_COUNT(
      uplane_encap_with_integrity(12, DOWNLINK, ZUC_ENC, NULL_AUTH));
  TEST_PDCP_COUNT(uplane_encap_with_integrity(12, UPLINK, ZUC_ENC, SNOW_AUTH));
  TEST_PDCP_COUNT(
      uplane_encap_with_integrity(12, DOWNLINK, ZUC_ENC, SNOW_AUTH));
  ...
  TEST_PDCP_COUNT(
      uplane_encap_with_integrity(18, DOWNLINK, ZUC_ENC, NULL_AUTH));
  TEST_PDCP_COUNT(uplane_encap_with_integrity(18, UPLINK, ZUC_ENC, SNOW_AUTH));
  TEST_PDCP_COUNT(
      uplane_encap_with_integrity(18, DOWNLINK, ZUC_ENC, SNOW_AUTH));
...
int test_PDCP_PROTO_uplane_decap_all(void) {
  ...
  TEST_PDCP_COUNT(
      uplane_decap_with_integrity(12, DOWNLINK, ZUC_ENC, NULL_AUTH));
  TEST_PDCP_COUNT(uplane_decap_with_integrity(12, UPLINK, ZUC_ENC, SNOW_AUTH));
  TEST_PDCP_COUNT(
      uplane_decap_with_integrity(12, DOWNLINK, ZUC_ENC, SNOW_AUTH));
  ...
  TEST_PDCP_COUNT(
      uplane_decap_with_integrity(18, DOWNLINK, ZUC_ENC, NULL_AUTH));
  TEST_PDCP_COUNT(uplane_decap_with_integrity(18, UPLINK, ZUC_ENC, SNOW_AUTH));
  TEST_PDCP_COUNT(
      uplane_decap_with_integrity(18, DOWNLINK, ZUC_ENC, SNOW_AUTH));
