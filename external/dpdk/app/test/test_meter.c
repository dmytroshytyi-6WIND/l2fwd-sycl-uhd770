 ...
 */
static inline int tm_test_srtcm_config(void) {
 ...
 */
static inline int tm_test_trtcm_config(void) {
 ...
 */
static inline int tm_test_trtcm_rfc4115_config(void) {
 ...
 */
static inline int tm_test_srtcm_color_blind_check(void) {
#define SRTCM_BLIND_CHECK_MSG "srtcm_blind_check"
  ...
  if (rte_meter_srtcm_color_blind_check(
          &sm, &sp, time, TM_TEST_SRTCM_CBS_DF - 1) != RTE_COLOR_GREEN)
  ...
  time = rte_get_tsc_cycles() + hz;
  if (rte_meter_srtcm_color_blind_check(
          &sm, &sp, time, TM_TEST_SRTCM_CBS_DF + 1) != RTE_COLOR_YELLOW)
  ...
  if (rte_meter_srtcm_config(&sm, &sp) != 0)
    melog(SRTCM_BLIND_CHECK_MSG);
  time = rte_get_tsc_cycles() + hz;
  if (rte_meter_srtcm_color_blind_check(&sm, &sp, time, (uint32_t)sp.ebs - 1) !=
      RTE_COLOR_YELLOW)
    melog(SRTCM_BLIND_CHECK_MSG " YELLOW");
  ...
  time = rte_get_tsc_cycles() + hz;
  if (rte_meter_srtcm_color_blind_check(
          &sm, &sp, time, TM_TEST_SRTCM_EBS_DF + 1) != RTE_COLOR_RED)
 ...
 */
static inline int tm_test_trtcm_color_blind_check(void) {
#define TRTCM_BLIND_CHECK_MSG "trtcm_blind_check"
  ...
  if (rte_meter_trtcm_color_blind_check(
          &tm, &tp, time, TM_TEST_TRTCM_CBS_DF - 1) != RTE_COLOR_GREEN)
  ...
  time = rte_get_tsc_cycles() + hz;
  if (rte_meter_trtcm_color_blind_check(
          &tm, &tp, time, TM_TEST_TRTCM_CBS_DF + 1) != RTE_COLOR_YELLOW)
  ...
  time = rte_get_tsc_cycles() + hz;
  if (rte_meter_trtcm_color_blind_check(
          &tm, &tp, time, TM_TEST_TRTCM_PBS_DF - 1) != RTE_COLOR_YELLOW)
    melog(TRTCM_BLIND_CHECK_MSG " YELLOW");
  ...
  time = rte_get_tsc_cycles() + hz;
  if (rte_meter_trtcm_color_blind_check(
          &tm, &tp, time, TM_TEST_TRTCM_PBS_DF + 1) != RTE_COLOR_RED)
 ...
 */
static inline int tm_test_trtcm_rfc4115_color_blind_check(void) {
#define TRTCM_RFC4115_BLIND_CHECK_MSG "trtcm_rfc4115_blind_check"
  ...
  time = rte_get_tsc_cycles() + hz;
  if (rte_meter_trtcm_rfc4115_color_blind_check(
          &tm, &tp, time, TM_TEST_TRTCM_EBS_DF - 1) != RTE_COLOR_YELLOW)
    melog(TRTCM_RFC4115_BLIND_CHECK_MSG " YELLOW");
  ...
  time = rte_get_tsc_cycles() + hz;
  if (rte_meter_trtcm_rfc4115_color_blind_check(
          &tm, &tp, time, TM_TEST_TRTCM_EBS_DF + 1) != RTE_COLOR_RED)
...
static inline int tm_test_srtcm_aware_check(enum rte_color in[4],
                                            enum rte_color out[4]) {
 ...
 */
static inline int tm_test_srtcm_color_aware_check(void) {
 ...
 * aware check, expect flag out[0], out[1], out[2] and out[3]
 */
static inline int tm_test_trtcm_aware_check(enum rte_color in[4],
                                            enum rte_color out[4]) {
...
static inline int tm_test_trtcm_color_aware_check(void) {
  enum rte_color in[4], out[4];
  /**
   * test 4 points that will produce green, yellow, yellow, red flag
   * if using blind check
 ...
 * aware check, expect flag out[0], out[1], out[2] and out[3]
 */
static inline int tm_test_trtcm_rfc4115_aware_check(enum rte_color in[4],
                                                    enum rte_color out[4]) {
 ...
 */
static inline int tm_test_trtcm_rfc4115_color_aware_check(void) {
  enum rte_color in[4], out[4];
  /**
   * test 4 points that will produce green, yellow, yellow, red flag
   * if using blind check
