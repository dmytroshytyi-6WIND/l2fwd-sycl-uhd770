...
static int test_fib6(void) {
  printf("fib not supported on Windows, skipping test\n");
  return TEST_SKIPPED;
}

static int test_slow_fib6(void) {
...
lookup_and_check_asc(struct rte_fib6 *fib,
                     ...
                     uint8_t ip_missing[][RTE_FIB6_IPV6_ADDR_SIZE],
...
static int lookup_and_check_desc(
    struct rte_fib6 *fib,
    uint8_t ip_arr[RTE_FIB6_MAXDEPTH][RTE_FIB6_IPV6_ADDR_SIZE],
...
static int check_fib(struct rte_fib6 *fib) {
  uint64_t def_nh = 100;
  uint8_t ip_arr[RTE_FIB6_MAXDEPTH][RTE_FIB6_IPV6_ADDR_SIZE];
  ...
  for (i = 0; i < RTE_FIB6_MAXDEPTH; i++) {
    for (j = 0; j < RTE_FIB6_IPV6_ADDR_SIZE; j++) {
      ip_arr[i][j] = ip_add[j] | ~get_msk_part(RTE_FIB6_MAXDEPTH - i, j);
    }
  }
  ...
  for (i = 0; i < RTE_FIB6_MAXDEPTH; i++) {
    ret =
        rte_fib6_add(fib, ip_add, RTE_FIB6_MAXDEPTH - i, RTE_FIB6_MAXDEPTH - i);
    RTE_TEST_ASSERT(ret == 0, "Failed to add a route\n");
...
int32_t test_lookup(void) {
  ...
  RTE_TEST_ASSERT(fib != NULL, "Failed to create FIB\n");
  ret = check_fib(fib);
  RTE_TEST_ASSERT(ret == TEST_SUCCESS, "Check_fib fails for TRIE_2B type\n");
  rte_fib6_free(fib);
  ...
  RTE_TEST_ASSERT(fib != NULL, "Failed to create FIB\n");
  ret = check_fib(fib);
  RTE_TEST_ASSERT(ret == TEST_SUCCESS, "Check_fib fails for TRIE_4B type\n");
  rte_fib6_free(fib);
  ...
  RTE_TEST_ASSERT(fib != NULL, "Failed to create FIB\n");
  ret = check_fib(fib);
  RTE_TEST_ASSERT(ret == TEST_SUCCESS, "Check_fib fails for TRIE_8B type\n");
  rte_fib6_free(fib);
 ...
 * Do all unit tests.
 */
static int test_fib6(void) { return unit_test_suite_runner(&fib6_fast_tests); }
