...
static inline uint32_t _st_ring_dequeue_bulk(struct rte_ring *r, void **obj,
...
static inline uint32_t _st_ring_enqueue_bulk(struct rte_ring *r,
                                             void *const *obj, uint32_t n,
                                             uint32_t *free) {
  uint32_t m;
  struct rte_ring_zc_data zcd;
...
static int _st_ring_init(struct rte_ring *r, const char *name, uint32_t num) {
  return rte_ring_init(r, name, num, RING_F_MP_HTS_ENQ | RING_F_MC_HTS_DEQ);
}
