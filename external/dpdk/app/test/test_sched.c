...
static struct rte_sched_subport_profile_params subport_profile[] = {
    {
        .tb_rate = 1250000000,
        .tb_size = 1000000,
        .tc_rate = {1250000000, 1250000000, 1250000000, 1250000000, 1250000000,
...
static struct rte_mempool *create_mempool(void) {
  ...
  if (!mp)
    mp = rte_pktmbuf_pool_create("test_sched", NB_MBUF, MEMPOOL_CACHE_SZ, 0,
                                 MBUF_DATA_SZ, SOCKET);

  return mp;
}

static void prepare_pkt(struct rte_sched_port *port, struct rte_mbuf *mbuf) {
  ...
  vlan2 = (struct rte_vlan_hdr *)((uintptr_t)&eth_hdr->ether_type +
                                  sizeof(struct rte_vlan_hdr));
  eth_hdr = (struct rte_ether_hdr *)((uintptr_t)&eth_hdr->ether_type +
                                     2 * sizeof(struct rte_vlan_hdr));
  ip_hdr =
      (struct rte_ipv4_hdr *)((uintptr_t)eth_hdr + sizeof(eth_hdr->ether_type));
...
static int test_sched(void) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    rte_sched_port_pkt_read_tree_path(port, out_mbufs[i], &subport, &pipe,
                                      &traffic_class, &queue);
