...
static void rte_pie_set_active(const struct rte_pie_config *pie_cfg,
                               struct rte_pie *pie, uint16_t active) {
  /**< Flag for activating/deactivating pie */
  RTE_SET_USED(pie_cfg);
  pie->active = active;
}
 ...
 */
static enum test_result test_rte_pie_init(struct test_config *tcfg) {
  unsigned int i = 0;
 ...
 */
static int increase_qsize(struct rte_pie_config *pie_cfg, struct rte_pie *pie,
                          uint32_t *qlen, uint32_t pkt_len, uint32_t attempts) {
  uint32_t i = 0;
...
static void enqueue_dequeue_func(struct rte_pie_config *pie_cfg,
                                 struct rte_pie *pie, uint32_t *qlen,
  ...
  for (i = 0; i < num_ops; i++) {
     ...
     * enqueue
     */
    ret = rte_pie_enqueue(pie_cfg, pie, *qlen, sizeof(uint32_t), get_port_ts());
    if (ret == 0)
      (*enqueued)++;
    else
      (*dropped)++;
 ...
 */
static uint32_t ft_tlevels[] = {6,   12,  18,  24,  30,  36,  42,  48,
...
static enum test_result func_test1(struct test_config *tcfg) {
  ...
  *tcfg->tvar->dropped = 0;
  ...
  if (increase_qsize(&tcfg->tconfig->pconfig[i], tcfg->tqueue->pdata_in,
                     tcfg->tqueue->qlen, tcfg->tlevel[i],
  ...
  for (i = 0; i < RTE_DIM(ft_tlevels); i++) {
    ...
    enqueue_dequeue_func(&tcfg->tconfig->pconfig[i], tcfg->tqueue->pdata_in,
                         tcfg->tqueue->qlen, tcfg->tvar->num_ops,
    ...
    printf("%s%-16u%-12u%-12u%-12.4lf%-12.4lf%-12.4lf%-12.4lf%-8i\n", label,
           prob, *tcfg->tvar->enqueued, *tcfg->tvar->dropped, drop_prob * 100.0,
...
static uint16_t ft2_dp_update_interval[] = {10,  20,  50,   150,  300,
...
static void enqueue_dequeue_perf(struct rte_pie_config *pie_cfg,
                                 ...
                                 struct rte_pie *pie_out, uint32_t *qlen,
                                 uint32_t num_ops, uint32_t *enqueued,
                                 uint32_t *dropped, uint32_t *dequeued,
  ...
  for (i = 0; i < num_ops; i++) {
    ...
    ret = rte_pie_enqueue(pie_cfg, pie_in, *qlen, 1000 * sizeof(uint32_t), ts);
    rdtsc_prof_end(prof);
...
static struct test_var perf1_tvar = {.num_iterations = 0,
                                     .num_ops = 30000,
                                     .clk_freq = 0,
                                     .dropped = pt_dropped,
...
static enum test_result perf_test(struct test_config *tcfg) {
  ...
  *tcfg->tvar->dropped = 0;
  ...
  enqueue_dequeue_perf(tcfg->tconfig->pconfig, tcfg->tqueue->pdata_in, NULL,
                       tcfg->tqueue->qlen, tcfg->tvar->num_ops,
                       tcfg->tvar->enqueued, tcfg->tvar->dropped,
                       tcfg->tvar->dequeued, &prof);
...
static struct test_var perf2_tvar = {.num_iterations = 0,
                                     .num_ops = 30000,
                                     .clk_freq = 0,
                                     .dropped = pt_dropped,
                                     .enqueued = pt_enqueued,
...
static enum test_result perf_test2(struct test_config *tcfg) {
  ...
  *tcfg->tvar->dropped = 0;
  ...
  enqueue_dequeue_perf(tcfg->tconfig->pconfig, tcfg->tqueue->pdata_in,
                       tcfg->tqueue->pdata_out, tcfg->tqueue->qlen,
                       tcfg->tvar->num_ops, tcfg->tvar->enqueued,
                       tcfg->tvar->dropped, tcfg->tvar->dequeued, &prof);
...
static int test_pie(void) {
  ...
  run_tests(func_pie_tests_quick, RTE_DIM(func_pie_tests_quick), &num_tests,
            &num_pass);
