 ...
 */
static void my_obj_init(struct rte_mempool *mp, __rte_unused void *arg,
                        void *obj, unsigned i) {
  uint32_t *objnum = obj;
...
static int test_mempool_basic(struct rte_mempool *mp, int use_external_cache) {
  uint32_t *objnum;
  void **objtable;
  void *obj, *obj2;
  char *obj_data;
  ...
  if (use_external_cache) {
    ...
    cache = rte_mempool_cache_create(RTE_MEMPOOL_CACHE_MAX_SIZE, SOCKET_ID_ANY);
    if (cache == NULL)
      RET_ERR();
  } else {
  ...
  if (rte_mempool_get_priv(mp) !=
      (char *)mp + RTE_MEMPOOL_HEADER_SIZE(mp, mp->cache_size))
    GOTO_ERR(ret, out);
...
static int test_mempool_creation_with_exceeded_cache_size(void) {
  ...
  mp_cov =
      rte_mempool_create("test_mempool_cache_too_big", MEMPOOL_SIZE,
                         MEMPOOL_ELT_SIZE, RTE_MEMPOOL_CACHE_MAX_SIZE + 32, 0,
...
static int test_mempool_creation_with_invalid_flags(void) {
  ...
  mp_cov = rte_mempool_create("test_mempool_invalid_flags", MEMPOOL_SIZE,
                              MEMPOOL_ELT_SIZE, 0, 0, NULL, NULL, NULL, NULL,
 ...
 */
static int test_mempool_launch_single_consumer(__rte_unused void *arg) {
  return test_mempool_single_consumer();
}

static void my_mp_init(struct rte_mempool *mp, __rte_unused void *arg) {
 ...
 */
static int test_mempool_sp_sc(void) {
  int ret = 0;
  ...
  if (mp_spsc == NULL) {
    mp_spsc = rte_mempool_create(
        "test_mempool_sp_sc", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE, 0, 0, my_mp_init,
        NULL, my_obj_init, NULL, SOCKET_ID_ANY,
 ...
 */
static int test_mempool_basic_ex(struct rte_mempool *mp) {
  unsigned i;
  void **obj;
  void *err_obj;
  int ret = -1;
...
static void walk_cb(struct rte_mempool *mp, void *userdata __rte_unused) {
  printf("\t%s\n", mp->name);
}
...
static void test_mp_mem_init(struct rte_mempool *mp, __rte_unused void *opaque,
...
static void test_mempool_events_cb(enum rte_mempool_event event,
...
static int test_mempool_events(int (*populate)(struct rte_mempool *mp)) {
...
#define RTE_TEST_TRACE_FAILURE(...)                                            \
  ...
  for (i = 0; i < callback_num; i++) {
    ret = rte_mempool_event_callback_register(test_mempool_events_cb, &data[i]);
    RTE_TEST_ASSERT_EQUAL(ret, 0, "Failed to register the callback %zu: %s", i,
  ...
  ret = rte_mempool_event_callback_unregister(test_mempool_events_cb, NULL);
  RTE_TEST_ASSERT_NOT_EQUAL(
      ret, 0, "Unregistered a non-registered callback with NULL argument");

  /* Create mempool 0 that will be observed by all callbacks. */
  memset(&data, 0, sizeof(data));
  strcpy(name, "empty0");
  mp[0] = rte_mempool_create_empty(name, mempool_size, mempool_elt_size, 0, 0,
                                   SOCKET_ID_ANY, 0);
  RTE_TEST_ASSERT_NOT_NULL(mp[0], "Cannot create mempool %s: %s", name,
                           rte_strerror(rte_errno));
  for (j = 0; j < callback_num; j++)
    RTE_TEST_ASSERT_EQUAL(data[j].invoked, false,
                          "Callback %zu invoked on %s mempool creation", j,
                          name);

  rte_mempool_set_ops_byname(mp[0], rte_mbuf_best_mempool_ops(), NULL);
  ret = populate(mp[0]);
  RTE_TEST_ASSERT_EQUAL(ret, (int)mp[0]->size,
                        "Failed to populate mempool %s: %s", name,
                        rte_strerror(-ret));
  for (j = 0; j < callback_num; j++) {
    RTE_TEST_ASSERT_EQUAL(data[j].invoked, true,
                          "Callback %zu not invoked on mempool %s population",
                          j, name);
    RTE_TEST_ASSERT_EQUAL(data[j].event, RTE_MEMPOOL_EVENT_READY,
                          "Wrong callback invoked, expected READY");
    RTE_TEST_ASSERT_EQUAL(
        data[j].mp, mp[0],
        "Callback %zu invoked for a wrong mempool instead of %s", j, name);
  }

  /* Check that unregistered callback 0 observes no events. */
  ret = rte_mempool_event_callback_unregister(test_mempool_events_cb, &data[0]);
  RTE_TEST_ASSERT_EQUAL(ret, 0, "Failed to unregister callback 0: %s",
                        rte_strerror(rte_errno));
  memset(&data, 0, sizeof(data));
  strcpy(name, "empty1");
  mp[1] = rte_mempool_create_empty(name, mempool_size, mempool_elt_size, 0, 0,
                                   SOCKET_ID_ANY, 0);
  RTE_TEST_ASSERT_NOT_NULL(mp[1], "Cannot create mempool %s: %s", name,
                           rte_strerror(rte_errno));
  rte_mempool_set_ops_byname(mp[1], rte_mbuf_best_mempool_ops(), NULL);
  ret = populate(mp[1]);
  RTE_TEST_ASSERT_EQUAL(ret, (int)mp[1]->size,
                        "Failed to populate mempool %s: %s", name,
                        rte_strerror(-ret));
  RTE_TEST_ASSERT_EQUAL(
      data[0].invoked, false,
      "Unregistered callback 0 invoked on %s mempool populaton", name);
  ...
  for (i = 0; i < mempool_num; i++) {
    ...
    for (j = 1; j < callback_num; j++) {
      RTE_TEST_ASSERT_EQUAL(
          data[j].invoked, true,
          "Callback %zu not invoked on mempool %s destruction", j, name);
      RTE_TEST_ASSERT_EQUAL(data[j].event, RTE_MEMPOOL_EVENT_DESTROY,
    ...
    RTE_TEST_ASSERT_EQUAL(
        data[0].invoked, false,
        "Unregistered callback 0 invoked on %s mempool destruction", name);
  }

  for (j = 1; j < callback_num; j++) {
    ret =
        rte_mempool_event_callback_unregister(test_mempool_events_cb, &data[j]);
...
static void test_mempool_events_safety_cb(enum rte_mempool_event event,
                                          struct rte_mempool *mp,
                                          void *user_data) {
...
static int test_mempool_events_safety(void) {
#pragma push_macro("RTE_TEST_TRACE_FAILURE")
#undef RTE_TEST_TRACE_FAILURE
#define RTE_TEST_TRACE_FAILURE(...)                                            \
  do {                                                                         \
  ...
  sdata[0].ret = -1;
  rte_mempool_event_callback_register(test_mempool_events_safety_cb, &sdata[0]);
  ...
  sdata[1].ret = -1;
  rte_mempool_event_callback_register(test_mempool_events_safety_cb, &sdata[1]);

  mp = rte_mempool_create_empty("empty", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE, 0, 0,
                                SOCKET_ID_ANY, 0);
  ...
  RTE_TEST_ASSERT_EQUAL(sdata[0].ret, 0,
                        "Callback failed to unregister itself: %s",
                        rte_strerror(rte_errno));
  ...
  RTE_TEST_ASSERT_EQUAL(sdata[1].ret, -EEXIST, "New callback inserted twice");
  RTE_TEST_ASSERT_EQUAL(
      data.invoked, true,
  ...
  for (i = 0; i < RTE_DIM(sdata); i++)
    rte_mempool_event_callback_unregister(test_mempool_events_safety_cb,
                                          &sdata[i]);
  ret = TEST_SUCCESS;
...
static int test_mempool_flag_non_io_set_when_no_iova_contig_set(void) {
...
static int test_mempool_flag_non_io_unset_when_populated_with_valid_iova(void) {
  const struct rte_memzone *mz = NULL;
  void *virt;
  rte_iova_t iova;
  ...
  RTE_TEST_ASSERT(ret > 0, "Failed to populate mempool: %s",
                  rte_strerror(-ret));
  RTE_TEST_ASSERT(mp->flags & RTE_MEMPOOL_F_NON_IO,
                  "NON_IO flag is not set when mempool is populated with only "
                  "RTE_BAD_IOVA");
...
static int test_mempool(void) {
  ...
  struct mp_data cb_arg = {.ret = -1};
  const char *default_pool_ops = rte_mbuf_best_mempool_ops();

  /* create a mempool (without cache) */
  mp_nocache =
      rte_mempool_create("test_nocache", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE, 0, 0,
                         NULL, NULL, my_obj_init, NULL, SOCKET_ID_ANY, 0);
  ...
  mp_stack_anon = rte_mempool_create_empty(
      "test_stack_anon", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE,
      RTE_MEMPOOL_CACHE_MAX_SIZE, 0, SOCKET_ID_ANY, 0);
  ...
  mp_stack_mempool_iter =
      rte_mempool_create("test_iter_obj", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE,
                         RTE_MEMPOOL_CACHE_MAX_SIZE, 0, NULL, NULL, my_obj_init,
  ...
  nb_objs = rte_mempool_obj_iter(mp_stack_mempool_iter, my_obj_init, NULL);
  if (nb_objs == 0)
    GOTO_ERR(ret, err);

  nb_mem_chunks =
      rte_mempool_mem_iter(mp_stack_mempool_iter, test_mp_mem_init, &cb_arg);
  if (nb_mem_chunks == 0 || cb_arg.ret < 0)
    GOTO_ERR(ret, err);

  /* create a mempool with an external handler */
  mp_stack =
      rte_mempool_create_empty("test_stack", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE,
                               RTE_MEMPOOL_CACHE_MAX_SIZE, 0, SOCKET_ID_ANY, 0);
  ...
  default_pool =
      rte_mempool_create_empty("default_pool", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE,
                               RTE_MEMPOOL_CACHE_MAX_SIZE, 0, SOCKET_ID_ANY, 0);
  ...
  if (rte_mempool_set_ops_byname(default_pool, default_pool_ops, NULL) < 0) {
