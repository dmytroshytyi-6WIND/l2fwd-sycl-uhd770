...
int test_port_ring_reader(void) {
  ...
  expected_pkts =
      rte_ring_sp_enqueue_burst(port_ring_reader_params.ring, mbuf, 1, NULL);
  ...
  received_pkts =
      rte_port_ring_reader_ops.f_rx(port, res_mbuf, RTE_PORT_IN_BURST_SIZE_MAX);
...
int test_port_ring_writer(void) {
  ...
  expected_pkts = 1;
  received_pkts =
      rte_ring_sc_dequeue_burst(port_ring_writer_params.ring, (void **)res_mbuf,
                                port_ring_writer_params.tx_burst_sz, NULL);
