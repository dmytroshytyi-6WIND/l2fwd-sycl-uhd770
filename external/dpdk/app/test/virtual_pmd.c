...
static int virtual_ethdev_start_fail(struct rte_eth_dev *eth_dev __rte_unused) {
...
static int virtual_ethdev_close(struct rte_eth_dev *dev __rte_unused) {
  return 0;
}
...
static int virtual_ethdev_configure_fail(struct rte_eth_dev *dev __rte_unused) {
  return -1;
}

static int virtual_ethdev_info_get(struct rte_eth_dev *dev __rte_unused,
...
static int virtual_ethdev_rx_queue_setup_success(
    ...
    uint16_t nb_rx_desc __rte_unused, unsigned int socket_id,
    const struct rte_eth_rxconf *rx_conf __rte_unused,
    struct rte_mempool *mb_pool __rte_unused) {
  struct virtual_ethdev_queue *rx_q;

  rx_q = (struct virtual_ethdev_queue *)rte_zmalloc_socket(
      NULL, sizeof(struct virtual_ethdev_queue), 0, socket_id);
...
static int virtual_ethdev_rx_queue_setup_fail(
    struct rte_eth_dev *dev __rte_unused, uint16_t rx_queue_id __rte_unused,
    uint16_t nb_rx_desc __rte_unused, unsigned int socket_id __rte_unused,
    const struct rte_eth_rxconf *rx_conf __rte_unused,
    struct rte_mempool *mb_pool __rte_unused) {
  return -1;
}

static int virtual_ethdev_tx_queue_setup_success(
  ...
  tx_q = (struct virtual_ethdev_queue *)rte_zmalloc_socket(
      NULL, sizeof(struct virtual_ethdev_queue), 0, socket_id);
...
static int virtual_ethdev_tx_queue_setup_fail(
    struct rte_eth_dev *dev __rte_unused, uint16_t tx_queue_id __rte_unused,
    uint16_t nb_tx_desc __rte_unused, unsigned int socket_id __rte_unused,
    const struct rte_eth_txconf *tx_conf __rte_unused) {
  return -1;
}
...
static int virtual_ethdev_stats_get(struct rte_eth_dev *dev,
...
static int virtual_ethdev_stats_reset(struct rte_eth_dev *dev) {
  struct virtual_ethdev_private *dev_private = dev->data->dev_private;
  void *pkt = NULL;
...
void virtual_ethdev_start_fn_set_success(uint16_t port_id, uint8_t success) {
...
void virtual_ethdev_configure_fn_set_success(uint16_t port_id,
                                             uint8_t success) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  struct virtual_ethdev_private *dev_private = dev->data->dev_private;
  struct eth_dev_ops *dev_ops = &dev_private->dev_ops;
...
void virtual_ethdev_rx_queue_setup_fn_set_success(uint16_t port_id,
                                                  uint8_t success) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  struct virtual_ethdev_private *dev_private = dev->data->dev_private;
  struct eth_dev_ops *dev_ops = &dev_private->dev_ops;
...
void virtual_ethdev_tx_queue_setup_fn_set_success(uint16_t port_id,
                                                  uint8_t success) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  struct virtual_ethdev_private *dev_private = dev->data->dev_private;
  struct eth_dev_ops *dev_ops = &dev_private->dev_ops;
...
void virtual_ethdev_link_update_fn_set_success(uint16_t port_id,
                                               uint8_t success) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  struct virtual_ethdev_private *dev_private = dev->data->dev_private;
  struct eth_dev_ops *dev_ops = &dev_private->dev_ops;
...
static uint16_t virtual_ethdev_rx_burst_success(void *queue __rte_unused,
...
static uint16_t virtual_ethdev_tx_burst_success(void *queue,
                                                struct rte_mbuf **bufs,
                                                uint16_t nb_pkts) {
...
static uint16_t virtual_ethdev_tx_burst_fail(void *queue,
                                             struct rte_mbuf **bufs,
                                             uint16_t nb_pkts) {
...
void virtual_ethdev_rx_burst_fn_set_success(uint16_t port_id, uint8_t success) {
...
void virtual_ethdev_tx_burst_fn_set_success(uint16_t port_id, uint8_t success) {
  struct virtual_ethdev_private *dev_private = NULL;
  struct rte_eth_dev *vrtl_eth_dev = &rte_eth_devices[port_id];
...
void virtual_ethdev_tx_burst_fn_set_tx_pkt_fail_count(
    uint16_t port_id, uint8_t packet_fail_count) {
  struct virtual_ethdev_private *dev_private = NULL;
  struct rte_eth_dev *vrtl_eth_dev = &rte_eth_devices[port_id];
...
void virtual_ethdev_set_link_status(uint16_t port_id, uint8_t link_status) {
  struct rte_eth_dev *vrtl_eth_dev = &rte_eth_devices[port_id];

  vrtl_eth_dev->data->dev_link.link_status = link_status;
}

void virtual_ethdev_simulate_link_status_interrupt(uint16_t port_id,
                                                   uint8_t link_status) {
  struct rte_eth_dev *vrtl_eth_dev = &rte_eth_devices[port_id];

  vrtl_eth_dev->data->dev_link.link_status = link_status;

  rte_eth_dev_callback_process(vrtl_eth_dev, RTE_ETH_EVENT_INTR_LSC, NULL);
}

int virtual_ethdev_add_mbufs_to_rx_queue(uint16_t port_id,
...
int virtual_ethdev_get_mbufs_from_tx_queue(uint16_t port_id,
                                           struct rte_mbuf **pkt_burst,
                                           int burst_length) {
  struct virtual_ethdev_private *dev_private;
  struct rte_eth_dev *vrtl_eth_dev = &rte_eth_devices[port_id];
...
int virtual_ethdev_create(const char *name, struct rte_ether_addr *mac_addr,
  ...
  dev_private->rx_queue =
      rte_ring_create(name_buf, MAX_PKT_BURST, socket_id, 0);
  if (dev_private->rx_queue == NULL)
    goto err;

  snprintf(name_buf, sizeof(name_buf), "%s_txQ", name);
  dev_private->tx_queue =
      rte_ring_create(name_buf, MAX_PKT_BURST, socket_id, 0);
  ...
  memcpy(eth_dev->data->mac_addrs, mac_addr, sizeof(*eth_dev->data->mac_addrs));
