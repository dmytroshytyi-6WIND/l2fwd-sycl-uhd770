...
static void timer_cb(struct rte_timer *tim, void *arg __rte_unused) {
...
static int reload_timer(struct rte_timer *tim) {
   ...
   */
  uint64_t ticks = BILLION - (rte_get_timer_cycles() % BILLION) + (tim - timer);
  int ret;
...
static int worker_main_loop(__rte_unused void *arg) {
  ...
  if (RTE_PER_LCORE(n_reset_collisions) != 0) {
    printf("- core %u, %u reset collisions (OK)\n", lcore_id,
           RTE_PER_LCORE(n_reset_collisions));
  }
  return 0;
}

static int test_timer_racecond(void) {
  int ret;
  uint64_t hz;
  uint64_t cur_time;
  uint64_t end_time;
