...
static uint8_t src_ipv6_addr[] = {0xFF, 0xAA, 0xFF, 0xAA, 0xFF, 0xAA,
                                  0xFF, 0xAA, 0xFF, 0xAA, 0xFF, 0xAA,
                                  0xFF, 0xAA, 0xFF, 0xAA};
static uint8_t dst_ipv6_addr_0[] = {0xFF, 0xAA, 0xFF, 0xAA, 0xFF, 0xAA,
                                    0xFF, 0xAA, 0xFF, 0xAA, 0xFF, 0xAA,
                                    0xFF, 0xAA, 0xFF, 0xAA};
static uint8_t dst_ipv6_addr_1[] = {0xFF, 0xAA, 0xFF, 0xAA, 0xFF, 0xAA,
                                    0xFF, 0xAA, 0xFF, 0xAA, 0xFF, 0xAA,
                                    0xFF, 0xAA, 0xFF, 0xAB};
...
static int configure_ethdev(uint16_t port_id, uint8_t start, uint8_t en_isr) {
  int q_id;
...
static int test_setup(void) {
  ...
  if (test_params->pkt_eth_hdr == NULL) {
    test_params->pkt_eth_hdr =
        malloc(sizeof(struct rte_ether_hdr) + sizeof(struct rte_vlan_hdr));
  ...
  if (test_params->mbuf_pool == NULL) {
    test_params->mbuf_pool =
        rte_pktmbuf_pool_create("MBUF_POOL", nb_mbuf_per_pool, MBUF_CACHE_SIZE,
                                0, RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
    TEST_ASSERT_NOT_NULL(test_params->mbuf_pool, "rte_mempool_create failed");
  }
  ...
  if (!slaves_initialized) {
    for (i = 0; i < TEST_MAX_NUMBER_OF_PORTS; i++) {
      ...
      test_params->slave_port_ids[i] =
          virtual_ethdev_create(pmd_name, mac_addr, rte_socket_id(), 1);
...
static int test_create_bonded_device(void) {
  ...
  if (test_params->bonded_port_id == -1) {
    test_params->bonded_port_id = rte_eth_bond_create(
        BONDED_DEV_NAME, test_params->bonding_mode, rte_socket_id());
...
static int test_create_bonded_device_with_invalid_params(void) {
  int port_id;
  ...
  /* Invalid name */
  port_id =
      rte_eth_bond_create(NULL, test_params->bonding_mode, rte_socket_id());
...
static int test_add_slave_to_bonded_device(void) {
  int current_slave_count;
...
static int test_add_slave_to_invalid_bonded_device(void) {
  /* Invalid port ID */
  TEST_ASSERT_FAIL(
      rte_eth_bond_slave_add(
...
static int test_remove_slave_from_bonded_device(void) {
  int current_slave_count;
  ...
  TEST_ASSERT_SUCCESS(
      ...
      test_params->slave_port_ids[test_params->bonded_slave_count - 1]);
  TEST_ASSERT_SUCCESS(
      memcmp(mac_addr, &read_mac_addr, sizeof(read_mac_addr)),
      "bonded port mac address not set to that of primary port\n");

  rte_eth_stats_reset(
      test_params->slave_port_ids[test_params->bonded_slave_count - 1]);

  virtual_ethdev_simulate_link_status_interrupt(test_params->bonded_port_id, 0);
...
static int test_remove_slave_from_invalid_bonded_device(void) {
  /* Invalid port ID */
  TEST_ASSERT_FAIL(
      rte_eth_bond_slave_remove(
          test_params->bonded_port_id + 5,
...
static int test_add_already_bonded_slave_to_bonded_device(void) {
  ...
  snprintf(pmd_name, RTE_ETH_NAME_MAX_LEN, "%s_%d", BONDED_DEV_NAME,
           ++bonded_id);

  port_id =
      rte_eth_bond_create(pmd_name, test_params->bonding_mode, rte_socket_id());
  TEST_ASSERT(port_id >= 0, "Failed to create bonded device.");

  TEST_ASSERT(
      rte_eth_bond_slave_add(
          port_id,
          test_params->slave_port_ids[test_params->bonded_slave_count - 1]) < 0,
...
static int test_get_slaves_from_bonded_device(void) {
  int current_slave_count;
  uint16_t slaves[RTE_MAX_ETHPORTS];
  ...
  /* Invalid port id */
  current_slave_count =
      rte_eth_bond_slaves_get(INVALID_PORT_ID, slaves, RTE_MAX_ETHPORTS);
  TEST_ASSERT(current_slave_count < 0,
              "Invalid port id unexpectedly succeeded");

  current_slave_count =
      rte_eth_bond_active_slaves_get(INVALID_PORT_ID, slaves, RTE_MAX_ETHPORTS);
  TEST_ASSERT(current_slave_count < 0,
              "Invalid port id unexpectedly succeeded");
  ...
  /* non bonded device*/
  current_slave_count = rte_eth_bond_slaves_get(test_params->slave_port_ids[0],
                                                NULL, RTE_MAX_ETHPORTS);
  TEST_ASSERT(current_slave_count < 0,
              "Invalid port id unexpectedly succeeded");
...
static int test_add_remove_multiple_slaves_to_from_bonded_device(void) {
  int i;
...
static void enable_bonded_slaves(void) {
  int i;

  for (i = 0; i < test_params->bonded_slave_count; i++) {
    virtual_ethdev_tx_burst_fn_set_success(test_params->slave_port_ids[i], 1);
...
static int test_start_bonded_device(void) {
...
static int test_stop_bonded_device(void) {
  int current_slave_count;
  uint16_t slaves[RTE_MAX_ETHPORTS];
...
static int test_set_bonding_mode(void) {
  int i, bonding_mode;

  int bonding_modes[] = {BONDING_MODE_ROUND_ROBIN, BONDING_MODE_ACTIVE_BACKUP,
                         BONDING_MODE_BALANCE, BONDING_MODE_BROADCAST};

  /* Test supported link bonding modes */
  for (i = 0; i < (int)RTE_DIM(bonding_modes); i++) {
    /* Invalid port ID */
    TEST_ASSERT_FAIL(rte_eth_bond_mode_set(INVALID_PORT_ID, bonding_modes[i]),
                     "Expected call to failed as invalid port (%d) specified.",
                     INVALID_PORT_ID);

    /* Non bonded device */
    TEST_ASSERT_FAIL(
        rte_eth_bond_mode_set(test_params->slave_port_ids[0], bonding_modes[i]),
        "Expected call to failed as invalid port (%d) specified.",
        test_params->slave_port_ids[0]);

    TEST_ASSERT_SUCCESS(
        rte_eth_bond_mode_set(test_params->bonded_port_id, bonding_modes[i]),
    ...
    TEST_ASSERT_EQUAL(
        bonding_mode, bonding_modes[i],
        "Link bonding mode (%d) of port (%d) is not expected value (%d).",
        bonding_mode, test_params->bonded_port_id, bonding_modes[i]);
...
static int test_set_primary_slave(void) {
  int i, j, retval;
  ...
  /* Invalid port ID */
  TEST_ASSERT_FAIL(
      rte_eth_bond_primary_set(INVALID_PORT_ID, test_params->slave_port_ids[i]),
      "Expected call to failed as invalid port specified.");
  ...
  for (i = 0; i < 4; i++) {
    TEST_ASSERT_SUCCESS(
        rte_eth_bond_primary_set(test_params->bonded_port_id,
                                 test_params->slave_port_ids[i]),
    ...
    TEST_ASSERT_EQUAL(
        retval, test_params->slave_port_ids[i],
        "Bonded port (%d) primary port (%d) not expected value (%d)\n",
        test_params->bonded_port_id, retval, test_params->slave_port_ids[i]);
    ...
    TEST_ASSERT_SUCCESS(
        ...
        "Failed to get mac address (port %d)", test_params->slave_port_ids[i]);
    TEST_ASSERT_SUCCESS(
        memcmp(expected_mac_addr, &read_mac_addr, sizeof(read_mac_addr)),
        "bonded port mac address not set to that of primary port\n");

    /* Check bonded MAC */
    TEST_ASSERT_SUCCESS(
        rte_eth_macaddr_get(test_params->bonded_port_id, &read_mac_addr),
        "Failed to get mac address (port %d)", test_params->bonded_port_id);
    TEST_ASSERT_SUCCESS(
        memcmp(&read_mac_addr, &read_mac_addr, sizeof(read_mac_addr)),
        "bonded port mac address not set to that of primary port\n");
    ...
    for (j = 0; j < 4; j++) {
      if (j != i) {
        TEST_ASSERT_SUCCESS(
            rte_eth_macaddr_get(test_params->slave_port_ids[j], &read_mac_addr),
            "Failed to get mac address (port %d)",
...
static int test_set_explicit_bonded_mac(void) {
  int i;
  struct rte_ether_addr read_mac_addr;
  ...
  /* Non bonded device */
  TEST_ASSERT_FAIL(
      rte_eth_bond_mac_address_set(test_params->slave_port_ids[0], mac_addr),
      "Expected call to failed as invalid port specified.");

  /* NULL MAC address */
  TEST_ASSERT_FAIL(
      rte_eth_bond_mac_address_set(test_params->bonded_port_id, NULL),
      "Expected call to failed as NULL MAC specified");

  TEST_ASSERT_SUCCESS(
      rte_eth_bond_mac_address_set(test_params->bonded_port_id, mac_addr),
...
static int test_set_bonded_port_initialization_mac_assignment(void) {
  int i, slave_count;
  ...
  if (bonded_port_id == -1) {
    bonded_port_id =
        rte_eth_bond_create("net_bonding_mac_ass_test",
  ...
  if (!mac_slaves_initialized) {
    for (i = 0; i < BONDED_INIT_MAC_ASSIGNMENT_SLAVE_COUNT; i++) {
      ...
      slave_port_ids[i] =
          virtual_ethdev_create(pmd_name, &slave_mac_addr, rte_socket_id(), 1);

      TEST_ASSERT(slave_port_ids[i] >= 0, "Failed to create slave ethdev %s",
                  pmd_name);
  ...
  for (i = 0; i < BONDED_INIT_MAC_ASSIGNMENT_SLAVE_COUNT; i++) {
    TEST_ASSERT_SUCCESS(
        rte_eth_bond_slave_add(bonded_port_id, slave_port_ids[i]),
        "Failed to add slave (%d) to bonded port (%d).", slave_port_ids[i],
        bonded_port_id);
  }

  slave_count =
      rte_eth_bond_slaves_get(bonded_port_id, slaves, RTE_MAX_ETHPORTS);
  ...
  TEST_ASSERT_SUCCESS(
      rte_eth_bond_mac_address_set(bonded_port_id, &bonded_mac_addr),
      "Failed to set MAC address on bonded port (%d)", bonded_port_id);
  ...
  for (i = 0; i < BONDED_INIT_MAC_ASSIGNMENT_SLAVE_COUNT; i++) {
    virtual_ethdev_simulate_link_status_interrupt(slave_port_ids[i], 1);
  }

  TEST_ASSERT_SUCCESS(rte_eth_macaddr_get(bonded_port_id, &read_mac_addr),
                      "Failed to get mac address (port %d)", bonded_port_id);
  TEST_ASSERT_SUCCESS(
      memcmp(&bonded_mac_addr, &read_mac_addr, sizeof(read_mac_addr)),
      "bonded port mac address not as expected");

  TEST_ASSERT_SUCCESS(rte_eth_macaddr_get(slave_port_ids[0], &read_mac_addr),
                      "Failed to get mac address (port %d)", slave_port_ids[0]);
  TEST_ASSERT_SUCCESS(
      memcmp(&bonded_mac_addr, &read_mac_addr, sizeof(read_mac_addr)),
  ...
  TEST_ASSERT_SUCCESS(rte_eth_macaddr_get(slave_port_ids[1], &read_mac_addr),
                      "Failed to get mac address (port %d)", slave_port_ids[1]);
  TEST_ASSERT_SUCCESS(
      memcmp(&slave_mac_addr, &read_mac_addr, sizeof(read_mac_addr)),
  ...
  TEST_ASSERT_SUCCESS(rte_eth_macaddr_get(slave_port_ids[2], &read_mac_addr),
                      "Failed to get mac address (port %d)", slave_port_ids[2]);
  TEST_ASSERT_SUCCESS(
      memcmp(&slave_mac_addr, &read_mac_addr, sizeof(read_mac_addr)),
   ...
   */
  TEST_ASSERT_SUCCESS(
      rte_eth_bond_primary_set(bonded_port_id, slave_port_ids[2]),
      "failed to set primary port on bonded device.");

  TEST_ASSERT_SUCCESS(rte_eth_dev_stop(bonded_port_id),
                      "Failed to stop bonded port %u", bonded_port_id);
  ...
  for (i = 0; i < BONDED_INIT_MAC_ASSIGNMENT_SLAVE_COUNT; i++) {
    TEST_ASSERT_SUCCESS(
        rte_eth_bond_slave_remove(bonded_port_id, slave_port_ids[i]),
        "Failed to remove slave %d from bonded port (%d).", slave_port_ids[i],
        bonded_port_id);
  }
...
static int initialize_bonded_device_with_slaves(uint8_t bonding_mode,
                                                uint8_t bond_en_isr,
                                                uint16_t number_of_slaves,
                                                uint8_t enable_slave) {
  /* Configure bonded device */
  TEST_ASSERT_SUCCESS(
      configure_ethdev(test_params->bonded_port_id, 0, bond_en_isr),
  ...
  TEST_ASSERT_SUCCESS(
      rte_eth_bond_mode_set(test_params->bonded_port_id, bonding_mode),
      "Failed to set link bonding mode on port (%d) to (%d).",
...
static int test_adding_slave_after_bonded_device_started(void) {
  int i;

  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_ROUND_ROBIN, 0, 4, 0),
...
static int test_bonding_lsc_event_callback(
    uint16_t port_id __rte_unused, enum rte_eth_event_type type __rte_unused,
...
static inline int lsc_timeout(int wait_us) {
  int retval = 0;
...
static int test_status_interrupt(void) {
  int slave_count;
  uint16_t slaves[RTE_MAX_ETHPORTS];

  /* initialized bonding device with T slaves */
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(
          BONDING_MODE_ROUND_ROBIN, 1, TEST_STATUS_INTERRUPT_SLAVE_COUNT, 1),
  ...
  rte_eth_dev_callback_register(
      test_params->bonded_port_id, RTE_ETH_EVENT_INTR_LSC,
      test_bonding_lsc_event_callback, &test_params->bonded_port_id);
   ...
   * lsc interrupts */
  virtual_ethdev_simulate_link_status_interrupt(test_params->slave_port_ids[0],
                                                0);
  virtual_ethdev_simulate_link_status_interrupt(test_params->slave_port_ids[1],
                                                0);
  virtual_ethdev_simulate_link_status_interrupt(test_params->slave_port_ids[2],
                                                0);

  TEST_ASSERT_EQUAL(test_lsc_interrupt_count, 0,
                    "Received a link status change interrupt unexpectedly");

  virtual_ethdev_simulate_link_status_interrupt(test_params->slave_port_ids[3],
                                                0);
...
static int generate_test_burst(struct rte_mbuf **pkts_burst,
                               uint16_t burst_size, uint8_t vlan, uint8_t ipv4,
                               uint8_t toggle_dst_mac, uint8_t toggle_ip_addr,
  ...
  if (toggle_dst_mac)
    initialize_eth_header(
        test_params->pkt_eth_hdr, (struct rte_ether_addr *)src_mac,
        (struct rte_ether_addr *)dst_mac_1, ether_type, vlan, vlan_id);
  else
    initialize_eth_header(
        test_params->pkt_eth_hdr, (struct rte_ether_addr *)src_mac,
        (struct rte_ether_addr *)dst_mac_0, ether_type, vlan, vlan_id);
  ...
  if (ipv4) {
    ...
    if (toggle_ip_addr)
      pktlen = initialize_ipv6_header(test_params->pkt_ipv6_hdr,
                                      ...
                                      (uint8_t *)dst_ipv6_addr_1, pktlen);
    else
      pktlen = initialize_ipv6_header(test_params->pkt_ipv6_hdr,
                                      (uint8_t *)src_ipv6_addr,
                                      (uint8_t *)dst_ipv6_addr_0, pktlen);

    ip_hdr = test_params->pkt_ipv6_hdr;
  }

  /* Generate burst of packets to transmit */
  generated_burst_size = generate_packet_burst(
      test_params->mbuf_pool, pkts_burst, test_params->pkt_eth_hdr, vlan,
      ip_hdr, ipv4, test_params->pkt_udp_hdr, burst_size,
...
static int test_roundrobin_tx_burst(void) {
  int i, burst_size;
  struct rte_mbuf *pkt_burst[MAX_PKT_BURST];
  struct rte_eth_stats port_stats;

  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_ROUND_ROBIN, 0, 2, 1),
      "Failed to initialise bonded device");
  ...
  TEST_ASSERT_EQUAL(
      rte_eth_tx_burst(test_params->bonded_port_id, 0, pkt_burst, burst_size),
      burst_size, "tx burst failed");

  /* Verify bonded port tx stats */
  rte_eth_stats_get(test_params->bonded_port_id, &port_stats);
  TEST_ASSERT_EQUAL(
      port_stats.opackets, (uint64_t)burst_size,
  ...
  TEST_ASSERT_EQUAL(
      rte_eth_tx_burst(test_params->bonded_port_id, 0, pkt_burst, burst_size),
      0, "tx burst return unexpected value");

  /* Clean up and remove slaves from bonded device */
  return remove_slaves_and_stop_bonded_device();
}

static int verify_mbufs_ref_count(struct rte_mbuf **mbufs, int nb_mbufs,
                                  int val) {
  int i, refcnt;
...
static void free_mbufs(struct rte_mbuf **mbufs, int nb_mbufs) {
  int i;
...
static int test_roundrobin_tx_burst_slave_tx_fail(void) {
  struct rte_mbuf *pkt_burst[MAX_PKT_BURST];
  ...
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(
          BONDING_MODE_ROUND_ROBIN, 0, TEST_RR_SLAVE_TX_FAIL_SLAVE_COUNT, 1),
      "Failed to initialise bonded device");

  /* Generate test bursts of packets to transmit */
  TEST_ASSERT_EQUAL(
      generate_test_burst(pkt_burst, TEST_RR_SLAVE_TX_FAIL_BURST_SIZE, 0, 1, 0,
                          0, 0),
      TEST_RR_SLAVE_TX_FAIL_BURST_SIZE, "Failed to generate test packet burst");

  /* Copy references to packets which we expect not to be transmitted */
  first_fail_idx =
      (TEST_RR_SLAVE_TX_FAIL_BURST_SIZE - (TEST_RR_SLAVE_TX_FAIL_PACKETS_COUNT *
                                           TEST_RR_SLAVE_TX_FAIL_SLAVE_COUNT)) +
      TEST_RR_SLAVE_TX_FAIL_FAILING_SLAVE_IDX;

  for (i = 0; i < TEST_RR_SLAVE_TX_FAIL_PACKETS_COUNT; i++) {
    expected_tx_fail_pkts[i] =
        pkt_burst[first_fail_idx + (i * TEST_RR_SLAVE_TX_FAIL_SLAVE_COUNT)];
  }
  ...
  virtual_ethdev_tx_burst_fn_set_success(
      test_params->slave_port_ids[TEST_RR_SLAVE_TX_FAIL_FAILING_SLAVE_IDX], 0);
  ...
  for (i = 0; i < test_params->bonded_slave_count; i++) {
    ...
    slave_expected_tx_count =
        TEST_RR_SLAVE_TX_FAIL_BURST_SIZE / test_params->bonded_slave_count;

    if (i == TEST_RR_SLAVE_TX_FAIL_FAILING_SLAVE_IDX)
      slave_expected_tx_count =
          slave_expected_tx_count - TEST_RR_SLAVE_TX_FAIL_PACKETS_COUNT;

    TEST_ASSERT_EQUAL(
        port_stats.opackets, (uint64_t)slave_expected_tx_count,
        "Slave Port (%d) opackets value (%u) not as expected (%d)",
        test_params->slave_port_ids[i], (unsigned int)port_stats.opackets,
        slave_expected_tx_count);
  }
...
static int test_roundrobin_rx_burst_on_single_slave(void) {
  ...
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_ROUND_ROBIN, 0, 4, 1),
      "Failed to initialize bonded device with slaves");

  /* Generate test bursts of packets to transmit */
  TEST_ASSERT_EQUAL(
      generate_test_burst(gen_pkt_burst, burst_size, 0, 1, 0, 0, 0), burst_size,
  ...
  for (i = 0; i < test_params->bonded_slave_count; i++) {
    ...
    rte_eth_stats_get(test_params->bonded_port_id, &port_stats);
    TEST_ASSERT_EQUAL(
        port_stats.ipackets, (uint64_t)burst_size,
...
static int test_roundrobin_rx_burst_on_multiple_slaves(void) {
  struct rte_mbuf
      *gen_pkt_burst[TEST_ROUNDROBIN_TX_BURST_SLAVE_COUNT][MAX_PKT_BURST];
  ...
  for (i = 0; i < TEST_ROUNDROBIN_TX_BURST_SLAVE_COUNT; i++) {
    TEST_ASSERT_EQUAL(
        generate_test_burst(&gen_pkt_burst[i][0], burst_size[i], 0, 1, 0, 0, 0),
...
static int test_roundrobin_verify_mac_assignment(void) {
  struct rte_ether_addr read_mac_addr;
  ...
  TEST_ASSERT_SUCCESS(
      ...
      "Failed to get mac address (port %d)", test_params->slave_port_ids[0]);
  TEST_ASSERT_SUCCESS(
      rte_eth_macaddr_get(test_params->slave_port_ids[2], &expected_mac_addr_2),
      "Failed to get mac address (port %d)", test_params->slave_port_ids[2]);
  ...
  for (i = 0; i < test_params->bonded_slave_count; i++) {
    TEST_ASSERT_SUCCESS(
        ...
        "Failed to get mac address (port %d)", test_params->slave_port_ids[i]);
    TEST_ASSERT_SUCCESS(
        memcmp(&expected_mac_addr_0, &read_mac_addr, sizeof(read_mac_addr)),
  ...
  TEST_ASSERT_SUCCESS(
      rte_eth_bond_mac_address_set(test_params->bonded_port_id,
  ...
  TEST_ASSERT_SUCCESS(
      ...
      "Failed to get mac address (port %d)", test_params->bonded_port_id);
  TEST_ASSERT_SUCCESS(
      memcmp(bonded_mac, &read_mac_addr, sizeof(read_mac_addr)),
      "bonded port (%d) mac address not set to that of new primary port",
      test_params->slave_port_ids[i]);
  ...
  for (i = 0; i < test_params->bonded_slave_count; i++) {
    ...
    TEST_ASSERT_SUCCESS(
        memcmp(bonded_mac, &read_mac_addr, sizeof(read_mac_addr)),
        "slave port (%d) mac address not set to"
...
static int test_roundrobin_verify_promiscuous_enable_disable(void) {
  ...
  TEST_ASSERT_SUCCESS(ret, "Failed to enable promiscuous mode for port %d: %s",
                      test_params->bonded_port_id, rte_strerror(-ret));

  promiscuous_en = rte_eth_promiscuous_get(test_params->bonded_port_id);
  TEST_ASSERT_EQUAL(promiscuous_en, 1, "Port (%d) promiscuous mode not enabled",
                    test_params->bonded_port_id);
  ...
  TEST_ASSERT_SUCCESS(ret, "Failed to disable promiscuous mode for port %d: %s",
                      test_params->bonded_port_id, rte_strerror(-ret));
...
static int test_roundrobin_verify_slave_link_status_change_behaviour(void) {
  struct rte_mbuf *tx_pkt_burst[MAX_PKT_BURST] = {NULL};
  struct rte_mbuf
      *gen_pkt_burst[TEST_RR_LINK_STATUS_SLAVE_COUNT][MAX_PKT_BURST];
  struct rte_mbuf *rx_pkt_burst[MAX_PKT_BURST] = {NULL};
   ...
   * in round robin mode */
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_ROUND_ROBIN, 0,
                                           TEST_RR_LINK_STATUS_SLAVE_COUNT, 1),
      "Failed to initialize bonded device with slaves");
  ...
  TEST_ASSERT_EQUAL(port_stats.opackets, (uint64_t)0,
                    "Port (%d) opackets stats (%d) not expected (%d) value",
                    test_params->slave_port_ids[1], (int)port_stats.opackets,
                    0);
  ...
  TEST_ASSERT_EQUAL(port_stats.opackets, (uint64_t)0,
                    "Port (%d) opackets stats (%d) not expected (%d) value",
                    test_params->slave_port_ids[3], (int)port_stats.opackets,
                    0);
  ...
  for (i = 0; i < TEST_RR_LINK_STATUS_SLAVE_COUNT; i++) {
    TEST_ASSERT_EQUAL(
        generate_test_burst(&gen_pkt_burst[i][0], burst_size, 0, 1, 0, 0, 0),
...
static int test_roundrobin_verfiy_polling_slave_link_status_change(void) {
  ...
  for (i = 0; i < TEST_RR_POLLING_LINK_STATUS_SLAVE_COUNT; i++) {
    ...
    if (polling_test_slaves[i] == -1) {
      polling_test_slaves[i] =
          virtual_ethdev_create(slave_name, mac_addr, rte_socket_id(), 0);
      ...
      TEST_ASSERT_SUCCESS(configure_ethdev(polling_test_slaves[i], 0, 0),
                          "Failed to configure virtual ethdev %s(%d)",
...
static int test_activebackup_tx_burst(void) {
  int i, pktlen, primary_port, burst_size;
  struct rte_mbuf *pkts_burst[MAX_PKT_BURST];
  struct rte_eth_stats port_stats;

  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_ACTIVE_BACKUP, 0, 1, 1),
      "Failed to initialize bonded device with slaves");
  ...
  pktlen =
      initialize_udp_header(test_params->pkt_udp_hdr, src_port, dst_port_0, 16);
  pktlen = initialize_ipv4_header(test_params->pkt_ipv4_hdr, src_addr,
                                  dst_addr_0, pktlen);
  ...
  /* Send burst on bonded port */
  TEST_ASSERT_EQUAL(
      rte_eth_tx_burst(test_params->bonded_port_id, 0, pkts_burst, burst_size),
      burst_size, "tx burst failed");
  ...
  for (i = 0; i < test_params->bonded_slave_count; i++) {
    ...
    if (test_params->slave_port_ids[i] == primary_port) {
      ...
      TEST_ASSERT_EQUAL(
          port_stats.opackets, 0,
          "Slave Port (%d) opackets value (%u) not as expected (%d)",
          test_params->bonded_port_id, (unsigned int)port_stats.opackets, 0);
    }
  }
...
static int test_activebackup_rx_burst(void) {
  struct rte_mbuf *gen_pkt_burst[MAX_PKT_BURST] = {NULL};
  struct rte_mbuf *rx_pkt_burst[MAX_PKT_BURST] = {NULL};
  ...
  for (i = 0; i < test_params->bonded_slave_count; i++) {
    /* Generate test bursts of packets to transmit */
    TEST_ASSERT_EQUAL(
        generate_test_burst(&gen_pkt_burst[0], burst_size, 0, 1, 0, 0, 0),
    ...
    TEST_ASSERT_EQUAL(rte_eth_rx_burst(test_params->bonded_port_id, 0,
                      ...
                      burst_size, "rte_eth_rx_burst failed");
...
static int test_activebackup_verify_promiscuous_enable_disable(void) {
  int i, primary_port, promiscuous_en;
  int ret;

  /* Initialize bonded device with 4 slaves in round robin mode */
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_ACTIVE_BACKUP, 0, 4, 1),
      "Failed to initialize bonded device with slaves");
...
static int test_activebackup_verify_mac_assignment(void) {
  struct rte_ether_addr read_mac_addr;
  ...
  TEST_ASSERT_SUCCESS(
      ...
      "Failed to get mac address (port %d)", test_params->slave_port_ids[1]);
  ...
  TEST_ASSERT_SUCCESS(
      ...
      "Failed to get mac address (port %d)", test_params->slave_port_ids[1]);
  TEST_ASSERT_SUCCESS(
      memcmp(&expected_mac_addr_1, &read_mac_addr, sizeof(read_mac_addr)),
  ...
  TEST_ASSERT_EQUAL(rte_eth_bond_primary_set(test_params->bonded_port_id,
                                             test_params->slave_port_ids[1]),
                    0, "Failed to set bonded port (%d) primary port to (%d)",
                    test_params->bonded_port_id,
                    test_params->slave_port_ids[1]);
...
static int test_activebackup_verify_slave_link_status_change_failover(void) {
  struct rte_mbuf
      *pkt_burst[TEST_ACTIVE_BACKUP_RX_BURST_SLAVE_COUNT][MAX_PKT_BURST];
  struct rte_mbuf *rx_pkt_burst[MAX_PKT_BURST] = {NULL};
  struct rte_eth_stats port_stats;
  ...
  TEST_ASSERT_EQUAL(
      generate_test_burst(&pkt_burst[0][0], burst_size, 0, 1, 0, 0, 0),
  ...
  TEST_ASSERT_EQUAL(rte_eth_bond_active_slaves_get(test_params->bonded_port_id,
                                                   slaves, RTE_MAX_ETHPORTS),
                    2, "Number of active slaves (%d) is not as expected (%d).",
                    slave_count, 2);
  ...
  TEST_ASSERT_EQUAL(rte_eth_bond_active_slaves_get(test_params->bonded_port_id,
                                                   slaves, RTE_MAX_ETHPORTS),
                    3, "Number of active slaves (%d) is not as expected (%d).",
                    slave_count, 3);
  ...
  for (i = 0; i < TEST_ACTIVE_BACKUP_RX_BURST_SLAVE_COUNT; i++) {
    TEST_ASSERT_EQUAL(
        generate_test_burst(&pkt_burst[i][0], burst_size, 0, 1, 0, 0, 0),
        burst_size, "generate_test_burst failed");
...
static int test_balance_xmit_policy_configuration(void) {
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_ACTIVE_BACKUP, 0, 2, 1),
  ...
  /* Invalid port id */
  TEST_ASSERT_FAIL(
      rte_eth_bond_xmit_policy_set(INVALID_PORT_ID, BALANCE_XMIT_POLICY_LAYER2),
      "Expected call to failed as invalid port specified.");

  /* Set xmit policy on non bonded device */
  TEST_ASSERT_FAIL(rte_eth_bond_xmit_policy_set(test_params->slave_port_ids[0],
                                                BALANCE_XMIT_POLICY_LAYER2),
                   "Expected call to failed as invalid port specified.");

  TEST_ASSERT_SUCCESS(rte_eth_bond_xmit_policy_set(test_params->bonded_port_id,
                                                   BALANCE_XMIT_POLICY_LAYER2),
...
static int test_balance_l2_tx_burst(void) {
  struct rte_mbuf
      *pkts_burst[TEST_BALANCE_L2_TX_BURST_SLAVE_COUNT][MAX_PKT_BURST];
  ...
  TEST_ASSERT_EQUAL(generate_packet_burst(
                        test_params->mbuf_pool, &pkts_burst[0][0],
                        test_params->pkt_eth_hdr, 0, test_params->pkt_ipv4_hdr,
                        1, test_params->pkt_udp_hdr, burst_size[0],
                        PACKET_BURST_GEN_PKT_LEN, 1),
                    burst_size[0], "failed to generate packet burst");

  initialize_eth_header(
      test_params->pkt_eth_hdr, (struct rte_ether_addr *)src_mac,
      (struct rte_ether_addr *)dst_mac_1, RTE_ETHER_TYPE_IPV4, 0, 0);

  /* Generate a burst 2 of packets to transmit */
  TEST_ASSERT_EQUAL(generate_packet_burst(
                        test_params->mbuf_pool, &pkts_burst[1][0],
                        test_params->pkt_eth_hdr, 0, test_params->pkt_ipv4_hdr,
                        1, test_params->pkt_udp_hdr, burst_size[1],
                        PACKET_BURST_GEN_PKT_LEN, 1),
                    burst_size[1], "failed to generate packet burst");
  ...
  rte_eth_stats_get(test_params->bonded_port_id, &port_stats);
  TEST_ASSERT_EQUAL(
      port_stats.opackets, (uint64_t)(burst_size[0] + burst_size[1]),
      "Bonded Port (%d) opackets value (%u) not as expected (%d)",
      test_params->bonded_port_id, (unsigned int)port_stats.opackets,
  ...
  rte_eth_stats_get(test_params->slave_port_ids[1], &port_stats);
  TEST_ASSERT_EQUAL(
      port_stats.opackets, (uint64_t)burst_size[1],
      "Slave Port (%d) opackets value (%u) not as expected (%d)\n",
...
static int balance_l23_tx_burst(uint8_t vlan_enabled, uint8_t ipv4,
                                uint8_t toggle_mac_addr,
                                uint8_t toggle_ip_addr) {
  ...
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_BALANCE, 0, 2, 1),
      "Failed to initialize_bonded_device_with_slaves.");
  ...
  /* Generate test bursts of packets to transmit */
  TEST_ASSERT_EQUAL(generate_test_burst(pkts_burst_1, burst_size_1,
                                        vlan_enabled, ipv4, 0, 0, 0),
                    burst_size_1, "failed to generate packet burst");

  TEST_ASSERT_EQUAL(generate_test_burst(pkts_burst_2, burst_size_2,
                                        vlan_enabled, ipv4, toggle_mac_addr,
                                        toggle_ip_addr, 0),
                    burst_size_2, "failed to generate packet burst");
...
static int test_balance_l23_tx_burst_ipv4_toggle_ip_addr(void) {
  return balance_l23_tx_burst(0, 1, 0, 1);
}

static int test_balance_l23_tx_burst_vlan_ipv4_toggle_ip_addr(void) {
  return balance_l23_tx_burst(1, 1, 0, 1);
}

static int test_balance_l23_tx_burst_ipv6_toggle_ip_addr(void) {
  return balance_l23_tx_burst(0, 0, 0, 1);
}

static int test_balance_l23_tx_burst_vlan_ipv6_toggle_ip_addr(void) {
  return balance_l23_tx_burst(1, 0, 0, 1);
}

static int test_balance_l23_tx_burst_toggle_mac_addr(void) {
  return balance_l23_tx_burst(0, 0, 1, 0);
}

static int balance_l34_tx_burst(uint8_t vlan_enabled, uint8_t ipv4,
  ...
  TEST_ASSERT_EQUAL(generate_test_burst(pkts_burst_2, burst_size_2,
                                        vlan_enabled, ipv4, toggle_mac_addr,
                                        toggle_ip_addr, toggle_udp_port),
                    burst_size_2, "failed to generate burst");
...
static int test_balance_l34_tx_burst_ipv4_toggle_ip_addr(void) {
  return balance_l34_tx_burst(0, 1, 0, 1, 0);
}

static int test_balance_l34_tx_burst_ipv4_toggle_udp_port(void) {
  return balance_l34_tx_burst(0, 1, 0, 0, 1);
}

static int test_balance_l34_tx_burst_vlan_ipv4_toggle_ip_addr(void) {
  return balance_l34_tx_burst(1, 1, 0, 1, 0);
}

static int test_balance_l34_tx_burst_ipv6_toggle_ip_addr(void) {
  return balance_l34_tx_burst(0, 0, 0, 1, 0);
}

static int test_balance_l34_tx_burst_vlan_ipv6_toggle_ip_addr(void) {
  return balance_l34_tx_burst(1, 0, 0, 1, 0);
}

static int test_balance_l34_tx_burst_ipv6_toggle_udp_port(void) {
...
static int test_balance_tx_burst_slave_tx_fail(void) {
  ...
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(
          BONDING_MODE_BALANCE, 0, TEST_BAL_SLAVE_TX_FAIL_SLAVE_COUNT, 1),
      "Failed to initialise bonded device");
  ...
  TEST_ASSERT_EQUAL(generate_test_burst(pkts_burst_1,
                                        TEST_BAL_SLAVE_TX_FAIL_BURST_SIZE_1, 0,
                                        0, 0, 0, 0),
  ...
  TEST_ASSERT_EQUAL(generate_test_burst(pkts_burst_2,
                                        TEST_BAL_SLAVE_TX_FAIL_BURST_SIZE_2, 0,
                                        0, 1, 0, 0),
  ...
  virtual_ethdev_tx_burst_fn_set_success(
      test_params->slave_port_ids[TEST_BAL_SLAVE_TX_FAIL_FAILING_SLAVE_IDX], 0);
  ...
  TEST_ASSERT_EQUAL(
      ...
      TEST_BAL_SLAVE_TX_FAIL_BURST_SIZE_1 -
          TEST_BAL_SLAVE_TX_FAIL_PACKETS_COUNT,
      "Transmitted (%d) packets, expected to transmit (%d) packets", tx_count_1,
      TEST_BAL_SLAVE_TX_FAIL_BURST_SIZE_1 -
          TEST_BAL_SLAVE_TX_FAIL_PACKETS_COUNT);
  ...
  TEST_ASSERT_EQUAL(
      ...
      "Transmitted (%d) packets, expected to transmit (%d) packets", tx_count_2,
      TEST_BAL_SLAVE_TX_FAIL_BURST_SIZE_2);
  ...
  TEST_ASSERT_EQUAL(
      port_stats.opackets, (uint64_t)TEST_BAL_SLAVE_TX_FAIL_BURST_SIZE_2,
      "Slave Port (%d) opackets value (%u) not as expected (%d)",
      test_params->slave_port_ids[1], (unsigned int)port_stats.opackets,
      TEST_BAL_SLAVE_TX_FAIL_BURST_SIZE_2);
  ...
  /* Verify that all mbufs have a ref value of zero */
  TEST_ASSERT_SUCCESS(
      verify_mbufs_ref_count(&pkts_burst_1[tx_count_1],
                             TEST_BAL_SLAVE_TX_FAIL_PACKETS_COUNT, 1),
      "mbufs refcnts not as expected");

  free_mbufs(&pkts_burst_1[tx_count_1], TEST_BAL_SLAVE_TX_FAIL_PACKETS_COUNT);
...
static int test_balance_rx_burst(void) {
  struct rte_mbuf
      *gen_pkt_burst[TEST_BALANCE_RX_BURST_SLAVE_COUNT][MAX_PKT_BURST];
  ...
  /* Initialize bonded device with 4 slaves in round robin mode */
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_BALANCE, 0, 3, 1),
      "Failed to initialise bonded device");

  /* Generate test bursts of packets to transmit */
  for (i = 0; i < TEST_BALANCE_RX_BURST_SLAVE_COUNT; i++) {
    TEST_ASSERT_EQUAL(
        generate_test_burst(&gen_pkt_burst[i][0], burst_size[i], 0, 0, 1, 0, 0),
...
static int test_balance_verify_promiscuous_enable_disable(void) {
  int i;
  int ret;

  /* Initialize bonded device with 4 slaves in round robin mode */
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_BALANCE, 0, 4, 1),
      "Failed to initialise bonded device");
  ...
  for (i = 0; i < test_params->bonded_slave_count; i++) {
    TEST_ASSERT_EQUAL(rte_eth_promiscuous_get(test_params->slave_port_ids[i]),
                      1, "Port (%d) promiscuous mode not enabled",
                      test_params->slave_port_ids[i]);
  }
...
static int test_balance_verify_mac_assignment(void) {
  struct rte_ether_addr read_mac_addr;
  struct rte_ether_addr expected_mac_addr_0, expected_mac_addr_1;
...
static int test_balance_verify_slave_link_status_change_behaviour(void) {
  struct rte_mbuf
      *pkt_burst[TEST_BALANCE_LINK_STATUS_SLAVE_COUNT][MAX_PKT_BURST];
  struct rte_mbuf *rx_pkt_burst[MAX_PKT_BURST] = {NULL};
  struct rte_eth_stats port_stats;
  ...
  TEST_ASSERT_EQUAL(
      generate_test_burst(&pkt_burst[1][0], burst_size, 0, 1, 1, 0, 0),
      burst_size, "generate_test_burst failed");
  ...
  TEST_ASSERT_EQUAL(rte_eth_bond_active_slaves_get(test_params->bonded_port_id,
                                                   slaves, RTE_MAX_ETHPORTS),
                    1, "Number of active slaves (%d) is not as expected (%d).",
                    slave_count, 1);
  ...
  rte_eth_rx_burst(test_params->bonded_port_id, 0, rx_pkt_burst, MAX_PKT_BURST);
...
static int test_broadcast_tx_burst(void) {
  ...
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_BROADCAST, 0, 2, 1),
      "Failed to initialise bonded device");
  ...
  TEST_ASSERT_EQUAL(
      rte_eth_tx_burst(test_params->bonded_port_id, 0, pkts_burst, burst_size),
      0, "transmitted an unexpected number of packets");
...
static int test_broadcast_tx_burst_slave_tx_fail(void) {
  ...
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(
          BONDING_MODE_BROADCAST, 0, TEST_BCAST_SLAVE_TX_FAIL_SLAVE_COUNT, 1),
      "Failed to initialise bonded device");

  /* Generate test bursts for transmission */
  TEST_ASSERT_EQUAL(generate_test_burst(pkts_burst,
                                        TEST_BCAST_SLAVE_TX_FAIL_BURST_SIZE, 0,
                                        0, 0, 0, 0),
  ...
  /* Set virtual slave TEST_BAL_SLAVE_TX_FAIL_FAILING_SLAVE_IDX to only fail
   * transmission of TEST_BAL_SLAVE_TX_FAIL_PACKETS_COUNT packets of burst */
  virtual_ethdev_tx_burst_fn_set_success(test_params->slave_port_ids[0], 0);
  virtual_ethdev_tx_burst_fn_set_success(test_params->slave_port_ids[1], 0);
  virtual_ethdev_tx_burst_fn_set_success(test_params->slave_port_ids[2], 0);
  ...
  TEST_ASSERT_EQUAL(
      ...
      TEST_BCAST_SLAVE_TX_FAIL_BURST_SIZE -
          TEST_BCAST_SLAVE_TX_FAIL_MIN_PACKETS_COUNT,
      "Transmitted (%d) packets, expected to transmit (%d) packets", tx_count,
      TEST_BCAST_SLAVE_TX_FAIL_BURST_SIZE -
          TEST_BCAST_SLAVE_TX_FAIL_MIN_PACKETS_COUNT);
  ...
  TEST_ASSERT_SUCCESS(
      verify_mbufs_ref_count(&pkts_burst[tx_count],
                             TEST_BCAST_SLAVE_TX_FAIL_MIN_PACKETS_COUNT, 1),
      "mbufs refcnts not as expected");

  free_mbufs(&pkts_burst[tx_count], TEST_BCAST_SLAVE_TX_FAIL_MIN_PACKETS_COUNT);
...
static int test_broadcast_rx_burst(void) {
  struct rte_mbuf
      *gen_pkt_burst[BROADCAST_RX_BURST_NUM_OF_SLAVES][MAX_PKT_BURST];
  ...
  /* Initialize bonded device with 4 slaves in round robin mode */
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_BROADCAST, 0, 3, 1),
      "Failed to initialise bonded device");
...
static int test_broadcast_verify_promiscuous_enable_disable(void) {
  int i;
  int ret;

  /* Initialize bonded device with 4 slaves in round robin mode */
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_BROADCAST, 0, 4, 1),
      "Failed to initialise bonded device");
...
static int test_broadcast_verify_mac_assignment(void) {
  struct rte_ether_addr read_mac_addr;
  struct rte_ether_addr expected_mac_addr_0, expected_mac_addr_1;
...
static int test_broadcast_verify_slave_link_status_change_behaviour(void) {
  struct rte_mbuf
      *pkt_burst[BROADCAST_LINK_STATUS_NUM_OF_SLAVES][MAX_PKT_BURST];
  struct rte_mbuf *rx_pkt_burst[MAX_PKT_BURST] = {NULL};
  struct rte_eth_stats port_stats;
  ...
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(
          BONDING_MODE_BROADCAST, 0, BROADCAST_LINK_STATUS_NUM_OF_SLAVES, 1),
      "Failed to initialise bonded device");
  ...
  TEST_ASSERT_EQUAL(
      generate_test_burst(&pkt_burst[0][0], burst_size, 0, 0, 1, 0, 0),
      burst_size, "generate_test_burst failed");
  ...
  for (i = 0; i < BROADCAST_LINK_STATUS_NUM_OF_SLAVES; i++) {
    TEST_ASSERT_EQUAL(
        generate_test_burst(&pkt_burst[i][0], burst_size, 0, 0, 1, 0, 0),
        burst_size, "failed to generate packet burst");
...
static int test_reconfigure_bonded_device(void) {
  ...
  TEST_ASSERT_SUCCESS(
      configure_ethdev(test_params->bonded_port_id, 0, 0),
      "failed to reconfigure bonded device with less rx/tx queues");

  return 0;
}

static int test_close_bonded_device(void) {
...
static void free_virtualpmd_tx_queue(void) {
  ...
  for (slave_port = 0; slave_port < test_params->bonded_slave_count;
    ...
    to_free_cnt = virtual_ethdev_get_mbufs_from_tx_queue(
        test_params->slave_port_ids[slave_port], pkts_to_free, MAX_PKT_BURST);
    for (i = 0; i < to_free_cnt; i++)
      rte_pktmbuf_free(pkts_to_free[i]);
  }
}

static int test_tlb_tx_burst(void) {
  ...
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_TLB, 1, 3, 1),
      "Failed to initialise bonded device");
  ...
  for (i = 0; i < 400000; i++) {
    ...
    generate_packet_burst(test_params->mbuf_pool, pkt_burst,
                          test_params->pkt_eth_hdr, 0,
                          test_params->pkt_ipv4_hdr, 1,
                          test_params->pkt_udp_hdr, burst_size, 60, 1);
    /* Send burst on bonded port */
    nb_tx =
        rte_eth_tx_burst(test_params->bonded_port_id, 0, pkt_burst, burst_size);
    nb_tx2 += nb_tx;
  ...
  TEST_ASSERT_EQUAL(
      port_stats[0].opackets, (uint64_t)nb_tx2,
      "Bonded Port (%d) opackets value (%u) not as expected (%d)\n",
  ...
  TEST_ASSERT_EQUAL(sum_ports_opackets, (uint64_t)all_bond_opackets,
                    "Total packets sent by slaves is not equal to packets sent "
                    "by bond interface");
...
static int test_tlb_rx_burst(void) {
  struct rte_mbuf *gen_pkt_burst[MAX_PKT_BURST] = {NULL};
  struct rte_mbuf *rx_pkt_burst[MAX_PKT_BURST] = {NULL};
  ...
  for (i = 0; i < test_params->bonded_slave_count; i++) {
    ...
    /* Call rx burst on bonded device */
    nb_rx = rte_eth_rx_burst(test_params->bonded_port_id, 0, &rx_pkt_burst[0],
                             MAX_PKT_BURST);
      ...
      for (j = 0; j < test_params->bonded_slave_count; j++) {
        ...
        if (i == j) {
          TEST_ASSERT_EQUAL(
              port_stats.ipackets, (uint64_t)burst_size,
              "Slave Port (%d) ipackets value (%u) not as expected (%d)\n",
              test_params->slave_port_ids[i], (unsigned int)port_stats.ipackets,
              burst_size);
        } else {
          TEST_ASSERT_EQUAL(
              port_stats.ipackets, (uint64_t)0,
              "Slave Port (%d) ipackets value (%u) not as expected (%d)\n",
              test_params->slave_port_ids[i], (unsigned int)port_stats.ipackets,
              0);
        }
...
static int test_tlb_verify_promiscuous_enable_disable(void) {
  int i, primary_port, promiscuous_en;
  int ret;

  /* Initialize bonded device with 4 slaves in transmit load balancing mode */
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_TLB, 0, 4, 1),
      "Failed to initialize bonded device");
...
static int test_tlb_verify_mac_assignment(void) {
  struct rte_ether_addr read_mac_addr;
  struct rte_ether_addr expected_mac_addr_0, expected_mac_addr_1;
  ...
  /* Initialize bonded device with 2 slaves in active backup mode */
  TEST_ASSERT_SUCCESS(
      initialize_bonded_device_with_slaves(BONDING_MODE_TLB, 0, 2, 1),
      "Failed to initialize bonded device");
...
static int test_tlb_verify_slave_link_status_change_failover(void) {
  struct rte_mbuf
      *pkt_burst[TEST_ADAPTIVE_TRANSMIT_LOAD_BALANCING_RX_BURST_SLAVE_COUNT]
                [MAX_PKT_BURST];
  struct rte_mbuf *rx_pkt_burst[MAX_PKT_BURST] = {NULL};
  struct rte_eth_stats port_stats;
  ...
  for (i = 0; i < TEST_ADAPTIVE_TRANSMIT_LOAD_BALANCING_RX_BURST_SLAVE_COUNT;
       i++) {
...
static int test_alb_change_mac_in_reply_sent(void) {
  struct rte_mbuf *pkt;
  ...
  TEST_ASSERT_SUCCESS(initialize_bonded_device_with_slaves(
                          BONDING_MODE_ALB, 0, TEST_ALB_SLAVE_COUNT, 1),
                      "Failed to initialize_bonded_device_with_slaves.");
  ...
  rte_ether_addr_copy(
      rte_eth_devices[test_params->bonded_port_id].data->mac_addrs, &bond_mac);
  ...
  initialize_eth_header(eth_pkt, &bond_mac, &client_mac, RTE_ETHER_TYPE_ARP, 0,
                        0);
  arp_pkt =
      (struct rte_arp_hdr *)((char *)eth_pkt + sizeof(struct rte_ether_hdr));
...
static int test_alb_reply_from_client(void) {
  struct rte_ether_hdr *eth_pkt;
  struct rte_arp_hdr *arp_pkt;
...
static int test_alb_receive_vlan_reply(void) {
  struct rte_ether_hdr *eth_pkt;
...
static int test_alb_ipv4_tx(void) {
...
static int test_link_bonding(void) {
