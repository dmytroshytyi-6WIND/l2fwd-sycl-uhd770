...
enum test_rss_type { TEST_SOFTRSS, TEST_SOFTRSS_BE, TEST_RSS_GFNI };

static inline uint64_t run_rss_calc(uint32_t *tuples[BATCH_SZ],
                                    enum test_rss_type type, int len,
                                    const void *key) {
  int i, j;
  ...
  for (i = 0; i < ITERATIONS; i++) {
    for (j = 0; j < BATCH_SZ; j++) {
      if (type == TEST_SOFTRSS)
        ...
        hash ^= rte_softrss_be(tuples[j], len / sizeof(uint32_t),
                               (const uint8_t *)key);
      else
        hash ^=
            rte_thash_gfni((const uint64_t *)key, (uint8_t *)tuples[j], len);
    }
  }
...
static inline uint64_t run_rss_calc_bulk(uint32_t *tuples[BATCH_SZ], int len,
                                         const void *key) {
  int i;
  uint64_t start_tsc, end_tsc;
  ...
  start_tsc = rte_rdtsc_precise();
  for (i = 0; i < ITERATIONS; i++)
    rte_thash_gfni_bulk((const uint64_t *)key, len, (uint8_t **)tuples,
                        bulk_hash, BATCH_SZ);
...
static void run_thash_test(unsigned int tuple_len) {
  ...
  tsc_diff = run_rss_calc(tuples, TEST_SOFTRSS_BE, len, default_rss_key);
...
static int test_thash_perf(void) {
