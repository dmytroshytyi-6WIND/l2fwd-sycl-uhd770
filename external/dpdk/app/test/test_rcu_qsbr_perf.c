...
static inline uint32_t alloc_thread_id(void) {
  uint32_t tmp_thr_id;
...
static int test_rcu_qsbr_reader_perf(void *arg) {
...
static int test_rcu_qsbr_writer_perf(void *arg) {
  bool wait = (bool)arg;
  uint64_t token = 0;
  uint64_t loop_cnt = 0;
  uint64_t begin, cycles;
 ...
 */
static int test_rcu_qsbr_perf(void) {
  size_t sz;
  ...
  t[0] = (struct rte_rcu_qsbr *)rte_zmalloc("rcu0", sz, RTE_CACHE_LINE_SIZE);
  ...
  rte_eal_remote_launch(test_rcu_qsbr_writer_perf, (void *)1,
                        enabled_core_ids[i]);
 ...
 */
static int test_rcu_qsbr_rperf(void) {
  size_t sz;
  unsigned int i, tmp_num_cores;
  ...
  for (i = 0; i < num_cores; i++)
    rte_eal_remote_launch(test_rcu_qsbr_reader_perf, NULL, enabled_core_ids[i]);
 ...
 */
static int test_rcu_qsbr_wperf(void) {
  size_t sz;
  unsigned int i;
  ...
  for (i = 0; i < num_cores; i++)
    rte_eal_remote_launch(test_rcu_qsbr_writer_perf, (void *)0,
                          enabled_core_ids[i]);
 ...
 */
static int test_rcu_qsbr_hash_reader(void *arg) {
...
static struct rte_hash *init_hash(void) {
  ...
  for (i = 0; i < TOTAL_ENTRY; i++) {
    hash_data[i] = rte_zmalloc(NULL, sizeof(uint32_t) * RTE_MAX_LCORE, 0);
    if (hash_data[i] == NULL) {
      printf("No memory\n");
      return NULL;
    }
 ...
 */
static int test_rcu_qsbr_sw_sv_1qs(void) {
  ...
  /* Reader threads are launched */
  for (i = 0; i < num_cores; i++)
    rte_eal_remote_launch(test_rcu_qsbr_hash_reader, NULL, enabled_core_ids[i]);
  ...
  for (i = 0; i < TOTAL_ENTRY; i++) {
    ...
    for (j = 0; j < tmp_num_cores; j++) {
      if (hash_data[i][j] != COUNTER_VALUE && hash_data[i][j] != 0) {
 ...
 */
static int test_rcu_qsbr_sw_sv_1qs_non_blocking(void) {
  uint64_t token, begin, cycles;
  int ret;
  size_t sz;
  unsigned int i, j, tmp_num_cores;
...
static int test_rcu_qsbr_main(void) {
  ...
  if (rte_lcore_count() < 3) {
    printf(
        "Not enough cores for rcu_qsbr_perf_autotest, expecting at least 3\n");
    return TEST_SKIPPED;
  }
