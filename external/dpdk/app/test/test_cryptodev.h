...
enum cryptodev_api_test_type { CRYPTODEV_API_TEST = 0, CRYPTODEV_RAW_API_TEST };
 ...
 */
static inline void pktmbuf_write(struct rte_mbuf *mbuf, int offset, int len,
...
static inline uint8_t *pktmbuf_mtod_offset(struct rte_mbuf *mbuf, int offset) {
  struct rte_mbuf *m;
...
static inline rte_iova_t pktmbuf_iova_offset(struct rte_mbuf *mbuf,
                                             int offset) {
  struct rte_mbuf *m;
...
create_segmented_mbuf(struct rte_mempool *mbuf_pool, int pkt_len, int nb_segs,
                      uint8_t pattern) {
