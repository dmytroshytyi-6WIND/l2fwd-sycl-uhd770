...
static int test_ipsec_sad(void) {
...
int32_t test_create_invalid(void) {
  ...
  RTE_TEST_ASSERT(sad == NULL, "Call succeeded with invalid parameters\n");
 ...
 */
int32_t test_find_existing(void) {
 ...
 */
int32_t test_multiple_create(void) {
  int i;
  struct rte_ipsec_sad *sad = NULL;
  struct rte_ipsec_sad_conf config;
...
static int32_t __test_add_invalid(int ipv6, union rte_ipsec_sad_key *tuple) {
  int status;
  struct rte_ipsec_sad *sad = NULL;
  struct rte_ipsec_sad_conf config;
  uint64_t tmp;
  ...
  /* sad == NULL*/
  status = rte_ipsec_sad_add(NULL, tuple, RTE_IPSEC_SAD_SPI_DIP_SIP, sa);
  RTE_TEST_ASSERT(status < 0, "Call succeeded with invalid parameters\n");
  ...
  /* len is incorrect*/
  status = rte_ipsec_sad_add(sad, tuple, RTE_IPSEC_SAD_SPI_DIP_SIP + 1, sa);
  RTE_TEST_ASSERT(status < 0, "Call succeeded with invalid parameters\n");

  /* sa == NULL*/
  status = rte_ipsec_sad_add(sad, tuple, RTE_IPSEC_SAD_SPI_DIP_SIP, NULL);
  RTE_TEST_ASSERT(status < 0, "Call succeeded with invalid parameters\n");

  /* sa is not aligned*/
  status = rte_ipsec_sad_add(sad, tuple, RTE_IPSEC_SAD_SPI_DIP_SIP,
                             (void *)((uint8_t *)sa + 1));
  RTE_TEST_ASSERT(status < 0, "Call succeeded with invalid parameters\n");
...
static int32_t __test_delete_invalid(int ipv6, union rte_ipsec_sad_key *tuple) {
  int status;
  struct rte_ipsec_sad *sad = NULL;
  struct rte_ipsec_sad_conf config;
 ...
 * arguments
 */
int32_t test_delete_invalid(void) {
  int status;
...
static int32_t __test_lookup_invalid(int ipv6, union rte_ipsec_sad_key *tuple) {
  int status;
  struct rte_ipsec_sad *sad = NULL;
  struct rte_ipsec_sad_conf config;
 ...
 * arguments
 */
int32_t test_lookup_invalid(void) {
  int status;
  struct rte_ipsec_sadv4_key tuple_v4 = {10, 20, 30};
  struct rte_ipsec_sadv6_key tuple_v6 = {10,
                                         {
  ...
  status = __test_lookup_invalid(0, (union rte_ipsec_sad_key *)&tuple_v4);
  if (status != TEST_SUCCESS)
    return status;

  status = __test_lookup_invalid(1, (union rte_ipsec_sad_key *)&tuple_v6);

  return status;
}

static int32_t __test_lookup_basic(int ipv6, union rte_ipsec_sad_key *tuple,
 ...
 */
int32_t test_lookup_basic(void) {
  int status;
  struct rte_ipsec_sadv4_key tuple_v4 = {SPI, DIP, SIP};
...
static int32_t __test_lookup_adv(int ipv6, union rte_ipsec_sad_key *tuple,
  ...
  install_sa = &tmp1;
  status = rte_ipsec_sad_add(sad, tuple, RTE_IPSEC_SAD_SPI_ONLY, install_sa);
  RTE_TEST_ASSERT(status == 0, "Failed to add a rule\n");
  ...
  install_sa = &tmp2;
  status = rte_ipsec_sad_add(sad, tuple, RTE_IPSEC_SAD_SPI_DIP, install_sa);
  RTE_TEST_ASSERT(status == 0, "failed to add a rule\n");
  ...
  install_sa = &tmp3;
  status = rte_ipsec_sad_add(sad, tuple, RTE_IPSEC_SAD_SPI_DIP_SIP, install_sa);
  RTE_TEST_ASSERT(status == 0, "Failed to add a rule\n");
...
int32_t test_lookup_adv(void) {
  ...
  status = __test_lookup_adv(0, (union rte_ipsec_sad_key *)&tuple_v4, key_arr);
  if (status != TEST_SUCCESS)
    return status;
  ...
  status = __test_lookup_adv(1, (union rte_ipsec_sad_key *)&tuple_v6, key_arr);

  return status;
}

static int32_t __test_lookup_order(int ipv6, union rte_ipsec_sad_key *tuple,
                                   union rte_ipsec_sad_key *tuple_1,
                                   union rte_ipsec_sad_key *tuple_2) {
  int status;
  struct rte_ipsec_sad *sad = NULL;
  struct rte_ipsec_sad_conf config;
  const union rte_ipsec_sad_key *key_arr[] = {
 ...
 */
int32_t test_lookup_order(void) {
  int status;
  /* key to install*/
  struct rte_ipsec_sadv4_key tuple_v4 = {SPI, DIP, SIP};
  struct rte_ipsec_sadv4_key tuple_v4_1 = {SPI, DIP, BAD};
...
static struct unit_test_suite ipsec_sad_tests = {
    ...
    .teardown = NULL,
    .unit_test_cases = {
        TEST_CASE(test_create_invalid), TEST_CASE(test_find_existing),
        TEST_CASE(test_multiple_create), TEST_CASE(test_add_invalid),
        TEST_CASE(test_delete_invalid), TEST_CASE(test_lookup_invalid),
        TEST_CASE(test_lookup_basic), TEST_CASE(test_lookup_adv),
        TEST_CASE(test_lookup_order), TEST_CASES_END()}};
