...
static int32_t test_link_status_up_default(void) {
  int ret = 0;
  struct rte_eth_link link_status = {.link_speed = RTE_ETH_SPEED_NUM_2_5G,
  ...
  TEST_ASSERT_BUFFERS_ARE_EQUAL("Link up at 2.5 Gbps FDX Autoneg", text,
                                strlen(text),
                                "Invalid default link status string");
  ...
  RTE_TEST_ASSERT(ret > 0, "Failed to format default string\n");
  TEST_ASSERT_BUFFERS_ARE_EQUAL("Link up at 10 Mbps HDX Fixed", text,
                                strlen(text),
                                "Invalid default link status "
                                "string with HDX");
  ...
  RTE_TEST_ASSERT(ret > 0, "Failed to format default string\n");
  TEST_ASSERT_BUFFERS_ARE_EQUAL("Link up at Unknown HDX Fixed", text,
                                strlen(text),
                                "Invalid default link status "
                                "string with HDX");
  ...
  printf("Default link up #3: %s\n", text);
  RTE_TEST_ASSERT(ret > 0, "Failed to format default string\n");
  TEST_ASSERT_BUFFERS_ARE_EQUAL("Link up at None HDX Fixed", text, strlen(text),
                                "Invalid default link status "
                                "string with HDX");
...
static int32_t test_link_status_down_default(void) {
  int ret = 0;
  struct rte_eth_link link_status = {.link_speed = RTE_ETH_SPEED_NUM_2_5G,
  ...
  ret = rte_eth_link_to_str(text, sizeof(text), &link_status);
  RTE_TEST_ASSERT(ret > 0, "Failed to format default string\n");
  TEST_ASSERT_BUFFERS_ARE_EQUAL("Link down", text, strlen(text),
                                "Invalid default link status string");

  return TEST_SUCCESS;
}

static int32_t test_link_status_invalid(void) {
  int ret = 0;
  struct rte_eth_link link_status = {.link_speed = 55555,
                                     .link_status = RTE_ETH_LINK_UP,
                                     .link_autoneg = RTE_ETH_LINK_AUTONEG,
                                     .link_duplex = RTE_ETH_LINK_FULL_DUPLEX};
  char text[RTE_ETH_LINK_MAX_STR_LEN];
  ...
  TEST_ASSERT_BUFFERS_ARE_EQUAL("Link up at Invalid FDX Autoneg", text,
                                strlen(text),
                                "Incorrect invalid link status string");

  return TEST_SUCCESS;
}

static int32_t test_link_speed_all_values(void) {
  const char *speed;
  uint32_t i;
  struct link_speed_t {
    const char *value;
    uint32_t link_speed;
  } speed_str_map[] = {{"None", RTE_ETH_SPEED_NUM_NONE},
  ...
  for (i = 0; i < sizeof(speed_str_map) / sizeof(struct link_speed_t); i++) {
    speed = rte_eth_link_speed_to_str(speed_str_map[i].link_speed);
    TEST_ASSERT_BUFFERS_ARE_EQUAL(speed_str_map[i].value, speed,
                                  strlen(speed_str_map[i].value),
...
static int test_link_status(void) {
