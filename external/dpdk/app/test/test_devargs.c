...
static int test_args(const char *devargs, const char *layer, const char *args,
                     const int n) {
...
static int test_valid_devargs_cases(const struct devargs_case *list, size_t n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if ((list[i].bus_kv > 0 || list[i].bus != NULL) && da.bus == NULL) {
      printf("rte_devargs_parse(%s) bus not parsed\n", list[i].devargs);
      goto fail;
    }
    if (test_args(list[i].devargs, "bus", da.bus_str, list[i].bus_kv) != 0)
      goto fail;
    if (list[i].bus != NULL && strcmp(da.bus->name, list[i].bus) != 0) {
      printf("rte_devargs_parse(%s) bus name (%s) not expected (%s)\n",
             list[i].devargs, da.bus->name, list[i].bus);
      goto fail;
    }
    if ((list[i].class_kv > 0 || list[i].class != NULL) && da.cls == NULL) {
      printf("rte_devargs_parse(%s) class not parsed\n", list[i].devargs);
      goto fail;
    }
    if (test_args(list[i].devargs, "class", da.cls_str, list[i].class_kv) != 0)
      goto fail;
    if (list[i].class != NULL && strcmp(da.cls->name, list[i].class) != 0) {
      printf("rte_devargs_parse(%s) class name (%s) not expected (%s)\n",
             list[i].devargs, da.cls->name, list[i].class);
      goto fail;
    }
    if (test_args(list[i].devargs, "driver", da.drv_str, list[i].driver_kv) !=
        0)
      goto fail;
    if (list[i].name != NULL && strcmp(da.name, list[i].name) != 0) {
...
static int test_valid_devargs(void) {
  static const struct devargs_case list[] = {
      /* Global devargs syntax: */
      {"bus=pci", 1, 0, 0, "pci", NULL, NULL},
      {"class=eth", 0, 1, 0, NULL, NULL, "eth"},
      {"bus=pci,addr=1:2.3/class=eth/driver=abc,k0=v0", 2, 1, 2, "pci",
       "0000:01:02.3", "eth"},
      {"bus=vdev,name=/dev/file/name/class=eth", 2, 1, 0, "vdev",
       "/dev/file/name", "eth"},
      {"bus=vdev,name=/class/bus/path/class=eth", 2, 1, 0, "vdev",
       "/class/bus/path", "eth"},
      {"bus=vdev,name=///dblslsh/class=eth", 2, 1, 0, "vdev", "///dblslsh",
       "eth"},
      /* Legacy devargs syntax: */
      {"1:2.3", 0, 0, 0, "pci", "1:2.3", NULL},
      {"pci:1:2.3,k0=v0", 0, 0, 1, "pci", "1:2.3", NULL},
  };
  static const struct devargs_case legacy_ring_list[] = {
      {"net_ring0", 0, 0, 0, "vdev", "net_ring0", NULL},
      {"net_ring0,iface=test,path=/class/bus/,queues=1", 0, 0, 3, "vdev",
       "net_ring0", NULL},
  };
  struct rte_bus *vdev_bus = rte_bus_find_by_name("vdev");
  int ret;
  ...
  if (vdev_bus != NULL && vdev_bus->parse("net_ring0", NULL) == 0)
    ...
    ret |=
        test_valid_devargs_cases(legacy_ring_list, RTE_DIM(legacy_ring_list));
  return ret;
}

/* Test several invalid cases */
static int test_invalid_devargs(void) {
  static const char *const list[] = {"bus=wrong-bus", "class=wrong-class"};
  struct rte_devargs da;
  uint32_t i;
  int ret;
  int fail = 0;
