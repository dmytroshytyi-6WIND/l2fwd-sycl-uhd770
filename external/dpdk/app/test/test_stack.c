...
static int test_stack_push_pop(struct rte_stack *s, void **obj_table,
  ...
  for (i = 0; i < STACK_SIZE; i += bulk_sz) {
    ...
    if (rte_stack_free_count(s) != STACK_SIZE - i - bulk_sz) {
      printf("[%s():%u] stack free count: %u (expected %u)\n", __func__,
             __LINE__, rte_stack_count(s), STACK_SIZE - i - bulk_sz);
      rte_free(popped_objs);
      return -1;
    }
  }
  ...
  for (i = 0; i < STACK_SIZE; i += bulk_sz) {
    ...
    if (rte_stack_count(s) != STACK_SIZE - i - bulk_sz) {
      printf("[%s():%u] stack count: %u (expected %u)\n", __func__, __LINE__,
             rte_stack_count(s), STACK_SIZE - i - bulk_sz);
      rte_free(popped_objs);
      return -1;
    }
    ...
    if (rte_stack_free_count(s) != i + bulk_sz) {
      printf("[%s():%u] stack free count: %u (expected %u)\n", __func__,
             __LINE__, rte_stack_count(s), i + bulk_sz);
      rte_free(popped_objs);
      return -1;
    }
  }
...
static int test_stack_basic(uint32_t flags) {
...
static int test_stack_name_reuse(uint32_t flags) {
...
static int test_stack_name_length(uint32_t flags) {
...
static int test_lookup_null(void) {
...
static int stack_thread_push_pop(__rte_unused void *args) {
...
static int test_stack_multithreaded(uint32_t flags) {
  unsigned int lcore_id;
  struct rte_stack *s;
  int result = 0;

  if (rte_lcore_count() < 2) {
    printf("Not enough cores for test_stack_multithreaded, expecting at least "
           "2\n");
    return TEST_SKIPPED;
  }
  ...
  s = rte_stack_create("test", MAX_BULK * rte_lcore_count(), rte_socket_id(),
                       flags);
  if (s == NULL) {
    printf("[%s():%u] Failed to create a stack\n", __func__, __LINE__);
    return -1;
...
static int __test_stack(uint32_t flags) {
...
static int test_stack(void) { return __test_stack(0); }

static int test_lf_stack(void) {
