...
static int test_node_list_dump(void) {
...
static inline graph_dynfield_t *graph_field(struct rte_mbuf *mbuf) {
...
uint16_t test_node_worker_source(struct rte_graph *graph, struct rte_node *node,
...
uint16_t test_node0_worker(struct rte_graph *graph, struct rte_node *node,
                           void **objs, uint16_t nb_objs) {
  test_main_t *tm = &test_main;
  ...
  if (*(uint32_t *)node->ctx == test_node0.id) {
    ...
    for (i = 0; i < nb_objs; i++) {
      ...
      if ((*graph_field(data) >> 32) != tm->test_node[0].idx) {
        printf("Data idx miss match at node 0, expected"
               " = %u got = %u\n",
               tm->test_node[0].idx, (uint32_t)(*graph_field(data) >> 32));
        goto end;
      }
    ...
    rte_node_enqueue(graph, node, 0, (void **)&mbuf_p[1][0], obj_node0);
    ...
    rte_node_enqueue(graph, node, 1, (void **)&mbuf_p[1][obj_node0], obj_node1);
...
uint16_t test_node1_worker(struct rte_graph *graph, struct rte_node *node,
                           void **objs, uint16_t nb_objs) {
  test_main_t *tm = &test_main;
  uint8_t second_pass = 0;
  ...
  for (i = 0; i < nb_objs; i++) {
    ...
    if ((*graph_field(data) >> 32) != tm->test_node[1].idx) {
      printf("Data idx miss match at node 1, expected = %u"
             " got = %u\n",
             tm->test_node[1].idx, (uint32_t)(*graph_field(data) >> 32));
      goto end;
    }
...
uint16_t test_node2_worker(struct rte_graph *graph, struct rte_node *node,
                           void **objs, uint16_t nb_objs) {
  test_main_t *tm = &test_main;
  uint8_t second_pass = 0;
  struct rte_mbuf *data;
  ...
  for (i = 0; i < nb_objs; i++) {
    ...
    if ((*graph_field(data) >> 32) != tm->test_node[2].idx) {
      printf("Data idx miss match at node 2, expected = %u"
             " got = %u\n",
             tm->test_node[2].idx, (uint32_t)(*graph_field(data) >> 32));
      goto end;
    }
  ...
  if (!second_pass) {
    ...
    for (i = 0; i < obj_node0; i++) {
      data = &mbuf[3][i];
      *graph_field(data) = ((uint64_t)tm->test_node[3].idx << 32) | i;
      if ((i + 1) == obj_node0)
        *graph_field(data) |= (1 << 16);
    }
    rte_node_enqueue(graph, node, 0, (void **)&mbuf_p[3][0], obj_node0);
  }
...
uint16_t test_node3_worker(struct rte_graph *graph, struct rte_node *node,
                           void **objs, uint16_t nb_objs) {
  test_main_t *tm = &test_main;
  uint8_t second_pass = 0;
  struct rte_mbuf *data;
  ...
  for (i = 0; i < nb_objs; i++) {
    ...
    if ((*graph_field(data) >> 32) != tm->test_node[3].idx) {
      printf("Data idx miss match at node 3, expected = %u"
             " got = %u\n",
             tm->test_node[3].idx, (uint32_t)(*graph_field(data) >> 32));
      goto end;
    }
  ...
  if (second_pass) {
    ...
    rte_node_enqueue(graph, node, 0, (void **)&mbuf_p[4][0], obj_node0);
  }
...
static int test_lookup_functions(void) {
  ...
  for (i = 1; i < MAX_NODES; i++) {
    ...
    if (count != tm->test_node[i].node.nb_edges) {
      printf(
          "Test number of edges for node = %s failed Expected = %d, got = %d\n",
  ...
  for (i = 1; i < MAX_NODES; i++) {
    ...
    if (count != tm->test_node[i].node.nb_edges * sizeof(char *)) {
      printf("Test number of edge count for node = %s failed Expected = %d, "
             "got = %d\n",
             tm->test_node[i].node.name, tm->test_node[i].node.nb_edges, count);
      return -1;
    }
...
static int test_node_clone(void) {
  ...
  for (i = 1; i < MAX_NODES; i++) {
    tm->test_node[i].idx = rte_node_clone(node_id, tm->test_node[i].node.name);
...
static int test_update_edges(void) {
  ...
  node_id = rte_node_from_name("test_node00");
  count = rte_node_edge_update(node_id, 0, tm->test_node[0].node.next_nodes,
...
static int test_create_graph(void) {
  ...
  if (graph_id != RTE_GRAPH_ID_INVALID) {
    printf("Graph creation success with isolated node, expected graph creation "
           "fail\n");
    return -1;
  }
  ...
  for (i = 0; i < MAX_NODES + 1; i++) {
    ...
    if (id == st->id) {
      if (obj_stats[i] != st->objs) {
        printf("Obj count miss match for node = %s expected = %" PRId64
               ", got=%" PRId64 "\n",
      ...
      if (fn_calls[i] != st->calls) {
        printf("Func call miss match for node = %s expected = %" PRId64
               ", got = %" PRId64 "\n",
...
static int test_print_stats(void) {
...
static int graph_setup(void) {
  ...
  graph_dynfield_offset = rte_mbuf_dynfield_register(&graph_dynfield_desc);
...
static struct unit_test_suite graph_testsuite = {
    ...
    .unit_test_cases =
        {
            TEST_CASE(test_update_edges), TEST_CASE(test_lookup_functions),
            TEST_CASE(test_create_graph),
            TEST_CASE(test_graph_lookup_functions), TEST_CASE(test_graph_walk),
            TEST_CASE(test_print_stats),
            TEST_CASES_END(), /**< NULL terminate unit test array */
        },
};

static int graph_autotest_fn(void) {
