...
static int test_lpm6_perf(void) {
...
static int test_lpm6_perf(void) {
  ...
  for (i = 0; i < NUM_ROUTE_ENTRIES; i++) {
    if (rte_lpm6_add(lpm, large_route_table[i].ip, large_route_table[i].depth,
                     next_hop_add) == 0)
      status++;
  }
  /* End Timer. */
  ...
  for (i = 0; i < ITERATIONS; i++) {
    ...
    for (j = 0; j < NUM_IPS_ENTRIES; j++) {
      if (rte_lpm6_lookup(lpm, large_ips_table[j].ip, &next_hop_return) != 0)
        count++;
    }
