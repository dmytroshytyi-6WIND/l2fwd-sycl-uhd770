...
static int test_graph_perf_func(void) {
...
static int test_node_ctx_init(const struct rte_graph *graph,
...
static uint16_t test_perf_node_worker_source(struct rte_graph *graph,
...
static uint16_t test_perf_node_worker_source_burst_one(struct rte_graph *graph,
                                                       struct rte_node *node,
                                                       void **objs,
                                                       uint16_t nb_objs) {
  uint16_t count;
...
static uint16_t test_perf_node_worker(struct rte_graph *graph,
                                      struct rte_node *node, void **objs,
                                      uint16_t nb_objs) {
  uint16_t next = 0;
  uint16_t enq = 0;
  ...
  for (i = 0; i < node->ctx[0]; i++) {
    ...
    while (count) {
      switch (count & (4 - 1)) {
      case 0:
        rte_node_enqueue_x4(graph, node, next, objs[0], objs[1], objs[2],
                            objs[3]);
        objs += 4;
        count -= 4;
        break;
      case 1:
        ...
        break;
      case 2:
        rte_node_enqueue_x2(graph, node, next, objs[0], objs[1]);
        objs += 2;
        count -= 2;
        break;
      case 3:
...
static uint16_t test_perf_node_sink(struct rte_graph *graph,
                                    struct rte_node *node, void **objs,
                                    uint16_t nb_objs) {
  RTE_SET_USED(graph);
  RTE_SET_USED(node);
...
static inline rte_node_t graph_node_get(const char *pname, char *nname) {
...
static int graph_init(const char *gname, uint8_t nb_srcs, uint8_t nb_sinks,
  ...
  mz = rte_memzone_reserve(TEST_GRAPH_PERF_MZ, sizeof(struct test_graph_perf),
                           0, 0);
  if (mz == NULL) {
  ...
  node_patterns =
      malloc(sizeof(char *) * (nb_srcs + nb_sinks + stages * nodes_per_stage));
  ...
  /* Setup the Graph */
  for (i = 0; i < stages; i++) {
    node_map[i] = (rte_node_t *)(node_map + stages) + nodes_per_stage * i;
    for (j = 0; j < nodes_per_stage; j++) {
      ...
      snprintf(node_patterns[graph_data->nb_nodes], RTE_NODE_NAMESIZE, "%s",
  ...
  for (i = 0; i < stages - 1; i++) {
    for (j = 0; j < nodes_per_stage; j++) {
      ...
      edges = graph_node_count_edges(i, j, nodes_per_stage, edge_map, ename,
                                     node_data, node_map);
  ...
  for (i = 0; i < nb_srcs; i++) {
    ...
    src_nodes[i] = graph_node_get(
        burst_one ? TEST_GRAPH_SRC_BRST_ONE_NAME : TEST_GRAPH_SRC_NAME, nname);
    ...
    snprintf(node_patterns[graph_data->nb_nodes], RTE_NODE_NAMESIZE, "%s",
             rte_node_id_to_name(src_nodes[i]));
    node_data = &graph_data->node_data[graph_data->nb_nodes];
...
static int _graph_perf_wrapper(void *args) {
...
static inline void graph_fini(void) {
...
static inline int graph_hr_4s_1n_1src_1snk(void) { return measure_perf(); }

static inline int graph_hr_4s_1n_1src_1snk_brst_one(void) {
  return measure_perf();
}

static inline int graph_hr_4s_1n_2src_1snk(void) { return measure_perf(); }

static inline int graph_hr_4s_1n_1src_2snk(void) { return measure_perf(); }

static inline int graph_tree_4s_4n_1src_4snk(void) { return measure_perf(); }

static inline int graph_reverse_tree_3s_4n_1src_1snk(void) {
  return measure_perf();
}

static inline int graph_parallel_tree_5s_4n_4src_4snk(void) {
  return measure_perf();
}
 ...
 * sink:		1
 */
static inline int graph_init_hr(void) {
 ...
 * sink:		1
 */
static inline int graph_init_hr_brst_one(void) {
  uint8_t edge_map[][1][1] = {
      {{100}},
      {{100}},
      {{100}},
 ...
 * sink:		1
 */
static inline int graph_init_hr_multi_src(void) {
  uint8_t edge_map[][1][1] = {
      {{100}},
      {{100}},
      {{100}},
 ...
 * sink:		2
 */
static inline int graph_init_hr_multi_snk(void) {
  uint8_t edge_map[][1][1] = {
      {{100}},
      {{100}},
      {{100}},
 ...
 * sink:		4
 */
static inline int graph_init_tree(void) {
 ...
 * sink:		1
 */
static inline int graph_init_reverse_tree(void) {
  uint8_t edge_map[][4][4] = {
 ...
 * sink:		4
 */
static inline int graph_init_parallel_tree(void) {
  uint8_t edge_map[][4][4] = {
      {{100, 0, 0, 0}, {0, 100, 0, 0}, {0, 0, 100, 0}, {0, 0, 0, 100}},
      {{100, 0, 0, 0}, {0, 100, 0, 0}, {0, 0, 100, 0}, {0, 0, 0, 100}},
      {{100, 0, 0, 0}, {0, 100, 0, 0}, {0, 0, 100, 0}, {0, 0, 0, 100}},
