 ...
 */
static uint32_t _st_ring_dequeue_bulk(struct rte_ring *r, void **obj,
                                      uint32_t n, uint32_t *avail);

static uint32_t _st_ring_enqueue_bulk(struct rte_ring *r, void *const *obj,
                                      uint32_t n, uint32_t *free);

static int _st_ring_init(struct rte_ring *r, const char *name, uint32_t num);

static void lcore_stat_update(struct lcore_stat *ls, uint64_t call,
                              uint64_t obj, uint64_t tm, int32_t prcs) {
  ls->op.nb_call += call;
  ls->op.nb_obj += obj;
  ls->op.nb_cycle += tm;
  if (prcs) {
...
static void lcore_op_stat_aggr(struct lcore_stat *ms,
                               const struct lcore_stat *ls) {
...
static void lcore_stat_aggr(struct lcore_stat *ms,
                            const struct lcore_stat *ls) {
  ms->nb_cycle = RTE_MAX(ms->nb_cycle, ls->nb_cycle);
  lcore_op_stat_aggr(ms, ls);
}

static void lcore_stat_dump(FILE *f, uint32_t lc, const struct lcore_stat *ls) {
  long double st;
  ...
  if (ls->op.min_cycle != UINT64_MAX) {
    fprintf(f, "\t\tmax cycles/call=%" PRIu64 "(%.2Lf usec),\n",
            ls->op.max_cycle, (long double)ls->op.max_cycle / st);
    fprintf(f, "\t\tmin cycles/call=%" PRIu64 "(%.2Lf usec),\n",
            ls->op.min_cycle, (long double)ls->op.min_cycle / st);
  }
...
static void fill_ring_elm(struct ring_elem *elm, uint32_t fill) {
  uint32_t i;
...
static int32_t check_updt_elem(struct ring_elem *elm[], uint32_t num,
                               const struct ring_elem *check,
                               const struct ring_elem *fill) {
  uint32_t i;
...
static int check_ring_op(uint32_t exp, uint32_t res, uint32_t lc,
...
static int test_worker(void *arg, const char *fname, int32_t prcs) {
  int32_t rc;
  uint32_t lc, n, num;
  uint64_t cl, tm0, tm1;
  struct lcore_arg *la;
  ...
  do {
    ...
    rc = check_ring_op(num, n, lc, fname, RTE_STR(_st_ring_dequeue_bulk));
    if (rc == 0)
    ...
    /* check return value */
    rc = check_ring_op(num, n, lc, fname, RTE_STR(_st_ring_enqueue_bulk));
    if (rc != 0)
      break;
...
static int test_worker_prcs(void *arg) { return test_worker(arg, __func__, 1); }

static int test_worker_avg(void *arg) { return test_worker(arg, __func__, 0); }

static void mt1_fini(struct rte_ring *rng, void *data) {
  rte_free(rng);
  rte_free(data);
}

static int mt1_init(struct rte_ring **rng, void **data, uint32_t num) {
  int32_t rc;
  size_t sz;
  uint32_t i, nr;
  struct rte_ring *r;
  ...
  *data = NULL;
  ...
  if (elm == NULL) {
    printf("%s: alloc(%zu) for %u elems data failed", __func__, sz, num);
    return -ENOMEM;
  }
  ...
  *data = elm;
  ...
  if (r == NULL) {
    printf("%s: alloc(%zu) for FIFO with %u elems failed", __func__, sz, nr);
    return -ENOMEM;
  }
...
static int test_mt1(int (*test)(void *)) {
  int32_t rc;
  uint32_t lc, mc;
  struct rte_ring *r;
  void *data;
