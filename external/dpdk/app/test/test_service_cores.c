...
static void testsuite_teardown(void) { /* release service cores? */
}
...
static int32_t dummy_mt_unsafe_cb(void *args) {
  ...
  uint32_t exp = 0;
  int lock_taken = __atomic_compare_exchange_n(
      lock, &exp, 1, 0, __ATOMIC_RELAXED, __ATOMIC_RELAXED);
  if (lock_taken) {
...
static int unregister_all(void) {
  uint32_t i;
...
static void wait_slcore_inactive(uint32_t slcore_id) {
  int i;

  for (i = 0; rte_service_lcore_may_be_active(slcore_id) == 1 && i < 100; i++)
    rte_delay_ms(SERVICE_DELAY);
}

/* register a single dummy service */
static int dummy_register(void) {
  ...
  TEST_ASSERT_EQUAL(-EINVAL, rte_service_component_register(&service, NULL),
                    "Invalid callback");
...
static int service_get_by_name(void) {
  unregister_all();

  uint32_t sid;
  TEST_ASSERT_EQUAL(-ENODEV, rte_service_get_by_name(DUMMY_SERVICE_NAME, &sid),
                    "get by name with invalid name should return -ENODEV");
  TEST_ASSERT_EQUAL(-EINVAL, rte_service_get_by_name(DUMMY_SERVICE_NAME, 0x0),
...
static int service_probe_capability(void) {
  unregister_all();
...
static int service_attr_get(void) {
  ...
  TEST_ASSERT_EQUAL(-EINVAL, rte_service_attr_get(id, attr_id, &attr_value),
                    "Invalid attr_id didn't return -EINVAL");

  attr_id = RTE_SERVICE_ATTR_CYCLES;
  TEST_ASSERT_EQUAL(-EINVAL,
                    rte_service_attr_get(UINT32_MAX, attr_id, &attr_value),
...
static int service_lcore_attr_get(void) {
  /* ensure all services unregistered so cycle counts are zero */
  unregister_all();
  ...
  TEST_ASSERT_EQUAL(
      -EINVAL,
      rte_service_lcore_attr_get(UINT32_MAX, lcore_attr_id, &lcore_attr_value),
      "Invalid lcore_id didn't return -EINVAL");
  TEST_ASSERT_EQUAL(-ENOTSUP,
                    rte_service_lcore_attr_get(rte_lcore_id(), lcore_attr_id,
                                               &lcore_attr_value),
  ...
  TEST_ASSERT_EQUAL(
      0,
      rte_service_lcore_attr_get(slcore_id, lcore_attr_id, &lcore_attr_value),
...
static int service_start_stop(void) {
  const uint32_t sid = 0;
  ...
  TEST_ASSERT_EQUAL(0, ret, "Enabling service core, expected 0 got %d", ret);
...
static int service_remote_launch_func(void *arg) {
  RTE_SET_USED(arg);
...
static int service_lcore_en_dis_able(void) {
  const uint32_t sid = 0;
  ...
  int ret = rte_eal_remote_launch(service_remote_launch_func, NULL, slcore_id);
...
static int service_lcore_running_check(void) {
...
static int service_lcore_add_del(void) {
  ...
  TEST_ASSERT_EQUAL(slcore_id, service_core_ids[0],
                    "Service core list[0] lcore must equal %d", slcore_id);
 ...
 */
static int service_mt_safe_poll(void) {
  int mt_safe = 1;
 ...
 * using the CAS.
 */
static int service_mt_unsafe_poll(void) {
  int mt_safe = 0;
...
static int32_t delay_as_a_mt_safe_service(void *args) {
  RTE_SET_USED(args);
...
static int32_t delay_as_a_service(void *args) {
...
static int service_run_on_app_core_func(void *arg) {
...
static int service_app_lcore_poll_impl(const int mt_safe) {
  ...
  int app_core2_ret =
      rte_eal_remote_launch(service_run_on_app_core_func, &id, app_core2);
  ...
  if (mt_safe) {
    ...
    TEST_ASSERT_EQUAL(0, app_core1_ret, "MT Safe: App core1 didn't return 0");
  } else {
  ...
  printf("perf test for %s: %0.1f cycles per call\n",
         mt_safe ? "MT Safe" : "MT Unsafe", (end - start) / (float)perf_iters);

  unregister_all();
  return TEST_SUCCESS;
}

static int service_app_lcore_mt_safe(void) {
  const int mt_safe = 1;
  return service_app_lcore_poll_impl(mt_safe);
}

static int service_app_lcore_mt_unsafe(void) {
  const int mt_safe = 0;
  return service_app_lcore_poll_impl(mt_safe);
}

/* start and stop a service core - ensuring it goes back to sleep */
static int service_lcore_start_stop(void) {
...
static int service_may_be_active(void) {
  const uint32_t sid = 0;
  int i;
...
static int service_active_two_cores(void) {
  if (!rte_lcore_is_enabled(0) || !rte_lcore_is_enabled(1) ||
      !rte_lcore_is_enabled(2))
    return TEST_SKIPPED;
...
static int test_service_common(void) {
