...
static int test_event_eth_tx_adapter_common(void) {
...
static int init_ports(void) {
  ...
  for (i = 0; i < NUM_ETH_DEV; i++) {
    for (j = 0; j < MAX_NUM_QUEUE; j++) {
      ...
      default_params.r[i][j] = rte_ring_create(ring_name, RING_SIZE, SOCKET0,
                                               RING_F_SP_ENQ | RING_F_SC_DEQ);
      TEST_ASSERT((default_params.r[i][j] != NULL), "Failed to allocate ring");
    }
  }
  ...
  for (i = 0; i < NUM_ETH_PAIR; i++) {
    ...
    p = rte_eth_from_rings(dev_name, c1, MAX_NUM_QUEUE, c2, MAX_NUM_QUEUE,
                           SOCKET0);
    ...
    p = rte_eth_from_rings(dev_name, c2, MAX_NUM_QUEUE, c1, MAX_NUM_QUEUE,
                           SOCKET0);
    TEST_ASSERT(p > 0, "Port creation failed %s", dev_name);
    err = port_init(p, default_params.mp);
    TEST_ASSERT(err == 0, "Port init failed %s", dev_name);
...
static int tx_adapter_create(void) {
  int err;
  ...
  err = rte_event_eth_tx_adapter_create(TEST_INST_ID, TEST_DEV_ID, &tx_p_conf);
  TEST_ASSERT(err == 0, "Expected 0 got %d", err);

  return err;
}

static void tx_adapter_free(void) {
  rte_event_eth_tx_adapter_free(TEST_INST_ID);
}

static int tx_adapter_create_free(void) {
  int err;
  struct rte_event_dev_info dev_info;
  struct rte_event_port_conf tx_p_conf;
  ...
  err = rte_event_eth_tx_adapter_create(TEST_INST_ID, TEST_DEV_ID, NULL);
...
static int tx_adapter_queue_add_del(void) {
  int err;
  uint32_t cap;

  err = rte_event_eth_tx_adapter_caps_get(TEST_DEV_ID, TEST_ETHDEV_ID, &cap);
  TEST_ASSERT(err == 0, "Expected 0 got %d", err);

  err = rte_event_eth_tx_adapter_queue_add(TEST_INST_ID,
                                           rte_eth_dev_count_total(), -1);
  TEST_ASSERT(err == -EINVAL, "Expected -EINVAL got %d", err);

  err = rte_event_eth_tx_adapter_queue_add(TEST_INST_ID, TEST_ETHDEV_ID, 0);
  TEST_ASSERT(err == 0, "Expected 0 got %d", err);

  err = rte_event_eth_tx_adapter_queue_add(TEST_INST_ID, TEST_ETHDEV_ID, -1);
  TEST_ASSERT(err == 0, "Expected 0 got %d", err);

  err = rte_event_eth_tx_adapter_queue_del(TEST_INST_ID, TEST_ETHDEV_ID, 0);
  TEST_ASSERT(err == 0, "Expected 0 got %d", err);

  err = rte_event_eth_tx_adapter_queue_del(TEST_INST_ID, TEST_ETHDEV_ID, -1);
  TEST_ASSERT(err == 0, "Expected 0 got %d", err);
...
static int tx_adapter_start_stop(void) {
  int err;
...
static int tx_adapter_single(uint16_t port, uint16_t tx_queue_id,
  ...
  while (l++ < EDEV_RETRY) {
    ...
    if (rte_eth_rx_burst(TEST_ETHDEV_PAIR_ID, tx_queue_id, &r, 1)) {
      TEST_ASSERT_EQUAL(r, m,
                        "mbuf comparison failed"
...
static int tx_adapter_service(void) {
  ...
  err = rte_event_eth_tx_adapter_event_port_get(TEST_INST_ID, &ev_port);
  TEST_ASSERT_SUCCESS(err, "Failed to get event port %d", err);

  err =
      rte_event_dev_attr_get(TEST_DEV_ID, RTE_EVENT_DEV_ATTR_PORT_COUNT, &pcnt);
  TEST_ASSERT_SUCCESS(err, "Port count get failed");

  err = rte_event_dev_attr_get(TEST_DEV_ID, RTE_EVENT_DEV_ATTR_QUEUE_COUNT,
                               &qcnt);
  ...
  dev_conf.nb_event_port_dequeue_depth = dev_info.max_event_port_dequeue_depth;
  dev_conf.nb_event_port_enqueue_depth = dev_info.max_event_port_enqueue_depth;
  dev_conf.nb_events_limit = dev_info.max_num_events;
  ...
  for (i = 0; i < pcnt; i++) {
    ...
    n_links = rte_event_port_links_get(TEST_DEV_ID, i, queues, priorities);
    ...
    err = rte_event_port_link(TEST_DEV_ID, i, queues, priorities, n_links);
    TEST_ASSERT(n_links == err,
                "Failed to link all queues"
                " err %s\n",
                rte_strerror(rte_errno));
  }

  err = rte_event_port_link(TEST_DEV_ID, ev_port, &ev_qid, NULL, 1);
  TEST_ASSERT(err == 1, "Failed to link queue port %u", ev_port);
  ...
  for (q = 0; q < MAX_NUM_QUEUE; q++) {
    ...
    for (i = 0; i < RING_SIZE; i++) {
      pbufs[i] = &bufs[i];
      err = tx_adapter_single(TEST_ETHDEV_ID, q, pbufs[i], ev_qid,
  ...
  TEST_ASSERT_EQUAL(stats.tx_packets, MAX_NUM_QUEUE * RING_SIZE,
                    ...
                    MAX_NUM_QUEUE * RING_SIZE, stats.tx_packets);
  ...
  TEST_ASSERT_EQUAL(stats.tx_packets, 0,
                    "stats.tx_packets expected %u got %" PRIu64, 0,
                    stats.tx_packets);
...
static int tx_adapter_dynamic_device(void) {
  ...
  for (i = 0; i < RTE_DIM(null_dev); i++) {
    ...
    TEST_ASSERT_SUCCESS(ret, "%s Port creation failed %d", null_dev[i], ret);

    if (i == 0) {
      ret = tx_adapter_create();
      TEST_ASSERT_SUCCESS(ret, "Adapter create failed %d", ret);
    }

    ret = rte_eth_dev_configure(port_id + i, MAX_NUM_QUEUE, MAX_NUM_QUEUE,
                                &dev_conf);
    TEST_ASSERT_SUCCESS(ret, "Failed to configure device %d", ret);

    ret = rte_event_eth_tx_adapter_queue_add(TEST_INST_ID, port_id + i, 0);
    TEST_ASSERT_SUCCESS(ret, "Failed to add queues %d", ret);
  }

  for (i = 0; i < RTE_DIM(null_dev); i++) {
    ret = rte_event_eth_tx_adapter_queue_del(TEST_INST_ID, port_id + i, -1);
...
static struct unit_test_suite event_eth_tx_tests = {
    ...
    .unit_test_cases = {
        ...
        TEST_CASE_ST(tx_adapter_create, tx_adapter_free, tx_adapter_start_stop),
        TEST_CASE_ST(tx_adapter_create, tx_adapter_free, tx_adapter_service),
