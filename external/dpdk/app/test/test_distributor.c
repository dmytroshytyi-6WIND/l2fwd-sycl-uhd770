...
static int test_distributor(void) {
...
static inline seq_dynfield_t *seq_field(struct rte_mbuf *mbuf) {
 ...
 */
static inline unsigned total_packet_count(void) {
  unsigned i, count = 0;
  for (i = 0; i < worker_idx; i++)
    count +=
        __atomic_load_n(&worker_stats[i].handled_packets, __ATOMIC_RELAXED);
  return count;
}

/* resets the packet counts for a new test */
static inline void clear_packet_count(void) {
  unsigned int i;
  for (i = 0; i < RTE_MAX_LCORE; i++)
    __atomic_store_n(&worker_stats[i].handled_packets, 0, __ATOMIC_RELAXED);
}

/* this is the basic worker function for sanity test
 * it does nothing but return packets and count them.
 */
static int handle_work(void *arg) {
  ...
  __atomic_fetch_add(&worker_stats[id].handled_packets, num, __ATOMIC_RELAXED);
 ...
 */
static int sanity_test(struct worker_params *wp, struct rte_mempool *p) {
  struct rte_distributor *db = wp->dist;
  ...
  while (processed < BURST)
    processed +=
        rte_distributor_process(db, &bufs[processed], BURST - processed);

  count = 0;
  do {

    rte_distributor_flush(db);
    count += rte_distributor_returned_pkts(db, returns, BURST * 2);
  } while (count < BURST);
  ...
  for (i = 0; i < BIG_BATCH / BURST; i++) {
    rte_distributor_process(db, &many_bufs[i * BURST], BURST);
    num_being_processed += BURST;
    do {
      count = rte_distributor_returned_pkts(db, &return_bufs[num_returned],
 ...
 * of mbufs.
 */
static int handle_work_with_free_mbufs(void *arg) {
  struct rte_mbuf *buf[8] __rte_cache_aligned;
  struct worker_params *wp = arg;
 ...
 * library.
 */
static int sanity_test_with_mbuf_alloc(struct worker_params *wp,
  ...
  for (i = 0; i < ((1 << ITER_POWER)); i += BURST) {
    ...
    while (processed < BURST)
      processed +=
          rte_distributor_process(d, &bufs[processed], BURST - processed);
  }
  ...
  if (total_packet_count() < (1 << ITER_POWER)) {
    printf("Line %u: Packet count is incorrect, %u, expected %u\n", __LINE__,
           total_packet_count(), (1 << ITER_POWER));
    return -1;
  }
...
static int handle_work_for_shutdown_test(void *arg) {
  struct rte_mbuf *buf[8] __rte_cache_aligned;
  struct worker_params *wp = arg;
  struct rte_distributor *d = wp->dist;
  unsigned int num;
  ...
  const unsigned int id = __atomic_fetch_add(&worker_idx, 1, __ATOMIC_RELAXED);

  num = rte_distributor_get_pkt(d, id, buf, NULL, 0);

  if (num > 0) {
    zero_unset = RTE_MAX_LCORE;
    __atomic_compare_exchange_n(&zero_idx, &zero_unset, id, false,
 ...
 * library.
 */
static int sanity_test_with_worker_shutdown(struct worker_params *wp,
                                            struct rte_mempool *p) {
  struct rte_distributor *d = wp->dist;
  struct rte_mbuf *bufs[BURST];
 ...
 */
static int test_flush_with_worker_shutdown(struct worker_params *wp,
                                           struct rte_mempool *p) {
  struct rte_distributor *d = wp->dist;
  struct rte_mbuf *bufs[BURST];
  unsigned int i;
...
static int handle_and_mark_work(void *arg) {
  struct rte_mbuf *buf[8] __rte_cache_aligned;
  struct worker_params *wp = arg;
  struct rte_distributor *db = wp->dist;
  unsigned int num, i;
 ...
 * by proper workers.
 */
static int sanity_mark_test(struct worker_params *wp, struct rte_mempool *p) {
  ...
  for (i = 0; i < burst; i++) {
    ...
    bufs[1 * burst + i]->hash.usr = ((i < burst / 2) ? 1 : 2) << shift;
    bufs[2 * burst + i]->hash.usr = ((i < burst / 2) ? 2 : 1) << shift;
  }
  ...
  for (i = 0; i < buf_count / burst; i++) {
    processed = 0;
    while (processed < burst)
      processed += rte_distributor_process(db, &bufs[i * burst + processed],
                                           burst - processed);
    count +=
        rte_distributor_returned_pkts(db, &returns[count], buf_count - count);
  }
...
static int test_error_distributor_create_name(void) {
  struct rte_distributor *d = NULL;
  struct rte_distributor *db = NULL;
  char *name = NULL;

  d = rte_distributor_create(name, rte_socket_id(), rte_lcore_count() - 1,
                             RTE_DIST_ALG_SINGLE);
  ...
  db = rte_distributor_create(name, rte_socket_id(), rte_lcore_count() - 1,
                              RTE_DIST_ALG_BURST);
...
static int test_error_distributor_create_numworkers(void) {
  struct rte_distributor *ds = NULL;
  struct rte_distributor *db = NULL;

  ds = rte_distributor_create("test_numworkers", rte_socket_id(),
                              RTE_MAX_LCORE + 10, RTE_DIST_ALG_SINGLE);
  ...
  db = rte_distributor_create("test_numworkers", rte_socket_id(),
                              RTE_MAX_LCORE + 10, RTE_DIST_ALG_BURST);
  if (db != NULL || rte_errno != EINVAL) {
...
static void quit_workers(struct worker_params *wp, struct rte_mempool *p) {
  struct rte_distributor *d = wp->dist;
...
static int test_distributor(void) {
  ...
  seq_dynfield_offset = rte_mbuf_dynfield_register(&seq_dynfield_desc);
  ...
  if (db == NULL) {
    db = rte_distributor_create("Test_dist_burst", rte_socket_id(),
                                rte_lcore_count() - 1, RTE_DIST_ALG_BURST);
    if (db == NULL) {
  ...
  if (ds == NULL) {
    ds = rte_distributor_create("Test_dist_single", rte_socket_id(),
                                rte_lcore_count() - 1, RTE_DIST_ALG_SINGLE);
    if (ds == NULL) {
  ...
  const unsigned nb_bufs = (511 * rte_lcore_count()) < BIG_BATCH
                               ...
                               : (511 * rte_lcore_count());
  if (p == NULL) {
    p = rte_pktmbuf_pool_create("DT_MBUF_POOL", nb_bufs, BURST, 0,
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (i)
      strlcpy(worker_params.name, "burst", sizeof(worker_params.name));
    else
      strlcpy(worker_params.name, "single", sizeof(worker_params.name));

    rte_eal_mp_remote_launch(handle_work, &worker_params, SKIP_MAIN);
    ...
    if (rte_lcore_count() > 2) {
      rte_eal_mp_remote_launch(handle_work_for_shutdown_test, &worker_params,
                               SKIP_MAIN);
      ...
      rte_eal_mp_remote_launch(handle_and_mark_work, &worker_params, SKIP_MAIN);
