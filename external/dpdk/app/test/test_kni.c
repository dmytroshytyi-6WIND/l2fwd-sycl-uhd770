...
static struct rte_mempool *test_kni_create_mempool(void) {
  struct rte_mempool *mp;

  mp = rte_mempool_lookup("kni_mempool");
  if (!mp)
    mp = rte_pktmbuf_pool_create("kni_mempool", NB_MBUF, MEMPOOL_CACHE_SZ, 0,
                                 MBUF_DATA_SZ, SOCKET);

  return mp;
}

static struct rte_mempool *test_kni_lookup_mempool(void) {
...
static int test_kni_link_change(void) {
  int ret;
  int pid;
  ...
  if (pid == 0) {
    ...
    if (ret != 1) {
      printf("Failed! Previous link state should be 1, returned %d.\n", ret);
      goto error;
    }
    rte_delay_ms(1000);
    ...
    ret = rte_kni_update_link(test_kni_ctx, 1);
    if (ret != 0) {
      printf("Failed! Previous link state should be 0, returned %d.\n", ret);
      goto error;
    }
    printf("KNI: Set LINKUP, previous state=%d\n", ret);
 ...
 */
static int test_kni_loop(__rte_unused void *arg) {
  int ret = 0;
  ...
  if (lcore_id == lcore_main) {
    ...
    while (1) {
      ...
      num = rte_kni_tx_burst(test_kni_ctx, pkts_burst, nb_rx);
      stats.ingress += num;
      rte_kni_handle_request(test_kni_ctx);
      if (num < nb_rx) {
        for (i = num; i < nb_rx; i++) {
    ...
    while (1) {
      if (test_kni_processing_flag)
        break;
      num = rte_kni_rx_burst(test_kni_ctx, pkts_burst, PKT_BURST_SZ);
      stats.egress += num;
...
static int test_kni_allocate_lcores(void) {
  unsigned i, count = 0;
...
static int test_kni_register_handler_mp(void) {
...
#define TEST_KNI_MTU_STR " 1450"
  ...
  if (pid < 0) {
    ...
    rte_delay_ms(1000);
    if (system(IFCONFIG TEST_KNI_PORT " mtu" TEST_KNI_MTU_STR) == -1)
      return -1;
...
static int test_kni_processing(uint16_t port_id, struct rte_mempool *mp) {
  int ret = 0;
  unsigned i;
  struct rte_kni *kni;
...
static int test_kni(void) {
  ...
  if (fgets(buf, sizeof(buf), fd)) {
    if (!strncmp(buf, "lo_mode_fifo", strlen("lo_mode_fifo")) ||
        !strncmp(buf, "lo_mode_fifo_skb", strlen("lo_mode_fifo_skb"))) {
      ...
      printf("test_kni_processing skipped because of missing rte_kni module "
             "lo_mode argument\n");
  }
  fclose(fd);
