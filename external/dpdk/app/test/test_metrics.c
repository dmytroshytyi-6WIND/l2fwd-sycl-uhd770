 ...
 */
static int test_metrics_deinitialize(void) {
  int err = 0;
...
static int test_metrics_without_init(void) {
  int err = 0;
...
static int test_metrics_reg_name_with_validname(void) {
  int err = 0;
...
static int test_metrics_reg_names(void) {
  int err = 0;
  const char *const mnames[] = {
      "mean_bits_in",
      "mean_bits_out",
...
static int test_metrics_update_value(void) {
  int err = 0;
...
static int test_metrics_update_values(void) {
  int err = 0;
  const uint64_t value[REG_METRIC_COUNT] = {1, 2, 3, 4, 5, 6};

  /* Successful Test: update metrics with first set */
  err = rte_metrics_update_values(RTE_METRICS_GLOBAL, 0, &value[0], 1);
  TEST_ASSERT(err >= 0, "%s, %d", __func__, __LINE__);

  /* Successful Test: update metrics with second set */
  err = rte_metrics_update_values(RTE_METRICS_GLOBAL, 1, &value[1], 1);
  TEST_ASSERT(err >= 0, "%s, %d", __func__, __LINE__);

  /* Successful Test: update metrics with third set */
  err = rte_metrics_update_values(RTE_METRICS_GLOBAL, 2, &value[2], 4);
  TEST_ASSERT(err >= 0, "%s, %d", __func__, __LINE__);

  /* Failed Test: Invalid count size */
  err = rte_metrics_update_values(RTE_METRICS_GLOBAL, KEY, &value[0],
                                  RTE_DIM(value));
  TEST_ASSERT(err < 0, "%s, %d", __func__, __LINE__);
  ...
  err =
      rte_metrics_update_values(RTE_METRICS_GLOBAL, KEY, NULL, RTE_DIM(value));
  TEST_ASSERT(err == -EINVAL, "%s, %d", __func__, __LINE__);
...
static int test_metrics_get_names(void) {
  int err = 0;
...
static int test_metrics_get_values(void) {
  int i = 0;
  int err = 0;
  ...
  err = rte_metrics_get_values(RTE_METRICS_GLOBAL, getvalues, REG_METRIC_COUNT);
  TEST_ASSERT(err >= 0, "%s, %d", __func__, __LINE__);
   ...
   * capacity
   */
  err = rte_metrics_get_values(RTE_METRICS_GLOBAL, NULL, REG_METRIC_COUNT);
  TEST_ASSERT(err >= 0, "%s, %d", __func__, __LINE__);
