...
static int test_ethdev_configure_port(int port) {
  ...
  if (rte_eth_rx_queue_setup(port, 0, RING_SIZE, SOCKET0, NULL, mp) < 0) {
  ...
  if (ret < 0) {
    printf("Link get failed for port %u: %s", port, rte_strerror(-ret));
    return -1;
  }
...
static int test_send_basic_packets(void) {
...
static int test_send_basic_packets_port(int port) {
  struct rte_mbuf bufs[RING_SIZE];
  struct rte_mbuf *pbufs[RING_SIZE];
  int i;
...
static int test_get_stats(int port) {
  ...
  if (stats.ipackets != 0 || stats.opackets != 0 || stats.ibytes != 0 ||
      stats.obytes != 0 || stats.ierrors != 0 || stats.oerrors != 0) {
  ...
  rte_eth_stats_get(port, &stats);
  if (stats.ipackets != 1 || stats.opackets != 1 || stats.ibytes != 0 ||
      stats.obytes != 0 || stats.ierrors != 0 || stats.oerrors != 0) {
...
static int test_stats_reset(int port) {
  struct rte_eth_stats stats;
  struct rte_mbuf buf, *pbuf = &buf;
...
static int test_pmd_ring_pair_create_attach(void) {
  ...
  if ((rte_eth_tx_queue_setup(rxtx_portd, 0, RING_SIZE, SOCKET0, NULL) < 0) ||
      (rte_eth_tx_queue_setup(rxtx_porte, 0, RING_SIZE, SOCKET0, NULL) < 0)) {
    printf("TX queue setup failed\n");
    return TEST_FAILED;
  }

  if ((rte_eth_rx_queue_setup(rxtx_portd, 0, RING_SIZE, SOCKET0, NULL, mp) <
       0) ||
      (rte_eth_rx_queue_setup(rxtx_porte, 0, RING_SIZE, SOCKET0, NULL, mp) <
       0)) {
  ...
  if (stats2.ipackets != 0 || stats2.opackets != 0 || stats2.ibytes != 0 ||
      stats2.obytes != 0 || stats2.ierrors != 0 || stats2.oerrors != 0) {
    printf("Error: port %d stats are not zero\n", rxtx_porte);
    return TEST_FAILED;
  }
  ...
  rte_eth_stats_get(rxtx_portd, &stats);
  rte_eth_stats_get(rxtx_porte, &stats2);
  if (stats.ipackets != 0 || stats.opackets != 1 || stats.ibytes != 0 ||
      stats.obytes != 0 || stats.ierrors != 0 || stats.oerrors != 0) {
    printf("Error: port %d stats are not as expected\n", rxtx_portd);
    return TEST_FAILED;
  }

  if (stats2.ipackets != 1 || stats2.opackets != 0 || stats2.ibytes != 0 ||
      stats2.obytes != 0 || stats2.ierrors != 0 || stats2.oerrors != 0) {
    printf("Error: port %d stats are not as expected\n", rxtx_porte);
    return TEST_FAILED;
  }
  ...
  if (stats2.ipackets != 1 || stats2.opackets != 1 || stats2.ibytes != 0 ||
      stats2.obytes != 0 || stats2.ierrors != 0 || stats2.oerrors != 0) {
    printf("Error: port %d stats are not as expected\n", rxtx_porte);
    return TEST_FAILED;
  }
  ...
  rte_eth_stats_get(rxtx_portd, &stats);
  rte_eth_stats_get(rxtx_porte, &stats2);
  if (stats.ipackets != 2 || stats.opackets != 2 || stats.ibytes != 0 ||
      stats.obytes != 0 || stats.ierrors != 0 || stats.oerrors != 0) {
    printf("Error: port %d stats are not as expected\n", rxtx_portd);
    return TEST_FAILED;
  }
  ...
  if (stats2.ipackets != 2 || stats2.opackets != 2 || stats2.ibytes != 0 ||
      stats2.obytes != 0 || stats2.ierrors != 0 || stats2.oerrors != 0) {
    printf("Error: port %d stats are not as expected\n", rxtx_porte);
    return TEST_FAILED;
  }
...
static void test_cleanup_resources(void) {
  int itr, ret;
...
static int test_pmd_ringcreate_setup(void) {
  uint8_t nb_ports;
   ...
   */
  rxtx[0] =
      rte_ring_create("R0", RING_SIZE, SOCKET0, RING_F_SP_ENQ | RING_F_SC_DEQ);
  if (rxtx[0] == NULL) {
  ...
  rxtx[1] =
      rte_ring_create("R1", RING_SIZE, SOCKET0, RING_F_SP_ENQ | RING_F_SC_DEQ);
  if (rxtx[1] == NULL) {
    printf("rte_ring_create R1 failed");
    return -1;
  }

  tx_porta = rte_eth_from_rings("net_ringa", rxtx, NUM_RINGS, rxtx, NUM_RINGS,
                                SOCKET0);
  rx_portb = rte_eth_from_rings("net_ringb", rxtx, NUM_RINGS, rxtx, NUM_RINGS,
                                SOCKET0);
  rxtx_portc = rte_eth_from_rings("net_ringc", rxtx, NUM_RINGS, rxtx, NUM_RINGS,
                                  SOCKET0);
  rxtx_portd = rte_eth_from_rings("net_ringd", rxtx, NUM_RINGS, rxtx, NUM_RINGS,
                                  SOCKET0);
  rxtx_porte = rte_eth_from_rings("net_ringe", rxtx, NUM_RINGS, rxtx, NUM_RINGS,
                                  SOCKET0);
  ...
  if ((tx_porta >= RTE_MAX_ETHPORTS) || (rx_portb >= RTE_MAX_ETHPORTS) ||
      (rxtx_portc >= RTE_MAX_ETHPORTS) || (rxtx_portd >= RTE_MAX_ETHPORTS) ||
...
static int test_command_line_ring_port(void) {
  ...
  RTE_ETH_FOREACH_DEV(port) {
    ...
    TEST_ASSERT((ret == 0), "Error during getting device (port %d) info: %s\n",
                port, strerror(-ret));
...
static int test_ethdev_configure_ports(void) {
...
static int test_get_stats_for_port(void) {
  TEST_ASSERT(test_get_stats(rxtx_portc) == 0, "test get stats failed");
  return TEST_SUCCESS;
}

static int test_stats_reset_for_port(void) {
  TEST_ASSERT(test_stats_reset(rxtx_portc) == 0, "test stats reset failed");
  return TEST_SUCCESS;
}

static struct unit_test_suite test_pmd_ring_suite = {
    ...
    .unit_test_cases = {
        ...
        TEST_CASE(test_command_line_ring_port), TEST_CASES_END()}};

static int test_pmd_ring(void) {
