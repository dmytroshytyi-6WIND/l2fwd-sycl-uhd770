...
static void *thread_loop(void *arg) {
  ...
  if ((t->lcore_id_any && lcore_id != LCORE_ID_ANY) ||
    ...
    printf(
        "Error: could not register new thread, got %u while %sexpecting %u\n",
...
static int test_non_eal_lcores(unsigned int eal_threads_count) {
...
static int limit_lcores_init(unsigned int lcore_id __rte_unused, void *arg) {
...
static void limit_lcores_uninit(unsigned int lcore_id __rte_unused, void *arg) {
  struct limit_lcore_context *l = arg;

  l->uninit++;
}

static int test_lcores_callback(unsigned int eal_threads_count) {
  ...
  if (l.init != eal_threads_count) {
    printf("Error: lcore callback register failed but incorrect init calls, "
           "expected %u, got %u\n",
           eal_threads_count, l.init);
    goto error;
  }
  if (l.uninit != eal_threads_count - 1) {
    printf("Error: lcore callback register failed but incorrect uninit calls, "
           "expected %u, got %u\n",
  ...
  if (l.uninit != 0) {
    printf("Error: lcore callback register succeeded but incorrect uninit "
           "calls, expected 0, got %u\n",
           l.uninit);
    goto error;
  }
  ...
  handle = NULL;
  if (l.init != eal_threads_count) {
    printf("Error: lcore callback unregister done but incorrect init calls, "
           "expected %u, got %u\n",
           eal_threads_count, l.init);
    goto error;
  }
  if (l.uninit != eal_threads_count) {
    printf("Error: lcore callback unregister done but incorrect uninit calls, "
           "expected %u, got %u\n",
...
static int test_non_eal_lcores_callback(unsigned int eal_threads_count) {
  ...
  if (l[0].init != eal_threads_count || l[1].init != eal_threads_count) {
    printf("Error: lcore callbacks register succeeded but incorrect init "
           "calls, expected %u, %u, got %u, %u\n",
           eal_threads_count, eal_threads_count, l[0].init, l[1].init);
    goto error;
  }
  if (l[0].uninit != 0 || l[1].uninit != 0) {
    printf("Error: lcore callbacks register succeeded but incorrect uninit "
           "calls, expected 0, 1, got %u, %u\n",
  ...
  if (l[0].init != eal_threads_count + 1 ||
    ...
    printf("Error: incorrect init calls, expected %u, %u, got %u, %u\n",
           eal_threads_count + 1, eal_threads_count + 1, l[0].init, l[1].init);
    goto cleanup_threads;
  }
  if (l[0].uninit != 0 || l[1].uninit != 0) {
  ...
  if (l[0].init != eal_threads_count + 2 ||
    ...
    printf("Error: incorrect init calls, expected %u, %u, got %u, %u\n",
           eal_threads_count + 2, eal_threads_count + 2, l[0].init, l[1].init);
    goto cleanup_threads;
  }
  ...
  if (l[0].uninit != 2 || l[1].uninit != 1) {
    printf(
        "Error: threads reported having successfully registered and "
        "unregistered, but incorrect uninit calls, expected 2, 1, got %u, %u\n",
        l[0].uninit, l[1].uninit);
    goto error;
  }
...
static int test_ctrl_thread(void) {
  ...
  t->state = Thread_INIT;
  if (rte_ctrl_thread_create(&t->id, "test_ctrl_threads", NULL,
