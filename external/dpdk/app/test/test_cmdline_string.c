...
struct string_invalid_str string_invalid_strs[] = {
    ...
    {"toolong!!!toolong!!!toolong!!!toolong!!!toolong!!!toolong!!!"
     ...
     "toolong!!!",
     "multi#choice#string"},
    {"invalid", "toolong!!!toolong!!!toolong!!!toolong!!!toolong!!!toolong!!!"
                "toolong!!!toolong!!!toolong!!!toolong!!!toolong!!!toolong!!!"
                "toolong!!!"},
    {"", "invalid"}};
...
int test_parse_string_invalid_param(void) {
  ...
  /* test null token */
  if (cmdline_get_help_string(NULL, buf, 0) != -1) {
    printf("Error: function accepted null token!\n");
    return -1;
  }
  if (cmdline_complete_get_elt_string(NULL, 0, buf, 0) != -1) {
    printf("Error: function accepted null token!\n");
    return -1;
  }
  ...
  /* test null buffer */
  if (cmdline_complete_get_elt_string((cmdline_parse_token_hdr_t *)&token, 0,
                                      NULL, 0) != -1) {
    printf("Error: function accepted null buffer!\n");
    return -1;
  }
  if (cmdline_parse_string((cmdline_parse_token_hdr_t *)&token, NULL,
                           (void *)&result, sizeof(result)) != -1) {
    printf("Error: function accepted null buffer!\n");
    return -1;
  }
  if (cmdline_get_help_string((cmdline_parse_token_hdr_t *)&token, NULL, 0) !=
      -1) {
    printf("Error: function accepted null buffer!\n");
    return -1;
  }
  /* test null result */
  if (cmdline_parse_string((cmdline_parse_token_hdr_t *)&token, buf, NULL, 0) ==
      -1) {
    printf("Error: function rejected null result!\n");
    return -1;
  }
  /* test negative index */
  if (cmdline_complete_get_elt_string((cmdline_parse_token_hdr_t *)&token, -1,
                                      buf, 0) != -1) {
...
int test_parse_string_invalid_data(void) {
   ...
   */
  token.string_data.str = NULL;
  if (cmdline_get_help_string((cmdline_parse_token_hdr_t *)&token, small_buf,
                              sizeof(small_buf)) == -1) {
   ...
   */
  token.string_data.str = NULL;
  if (cmdline_complete_get_nb_string((cmdline_parse_token_hdr_t *)&token) !=
      0) {
...
int test_parse_string_valid(void) {
  ...
  for (i = 0; i < RTE_DIM(string_elt_strs); i++) {
    ...
    if (cmdline_complete_get_elt_string((cmdline_parse_token_hdr_t *)&token,
                                        string_elt_strs[i].idx, buf,
                                        sizeof(buf)) < 0) {
      printf("Error: getting string element failed!\n");
      return -1;
    }
    if (strncmp(buf, string_elt_strs[i].result, sizeof(buf)) != 0) {
      printf("Error: result mismatch!\n");
      return -1;
    }
  }
