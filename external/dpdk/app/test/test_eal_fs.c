...
static int test_parse_sysfs_value(void) {
  ...
  /* get a temporary filename to use for all tests - create temp file handle and
   * then use /proc to get the actual file that we can open */
  ...
  if (eal_parse_sysfs_value("/dev/not-quite-null", &retval) == 0) {
    printf("Error with eal_parse_sysfs_value() - returned success on reading "
           "empty file\n");
    goto error;
  }
