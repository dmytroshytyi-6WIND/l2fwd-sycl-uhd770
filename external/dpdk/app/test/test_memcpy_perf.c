...
static int init_buffers(void) {
  ...
  large_buf_read =
      rte_malloc("memcpy", LARGE_BUFFER_SIZE + ALIGNMENT_UNIT, ALIGNMENT_UNIT);
  ...
  small_buf_read =
      rte_malloc("memcpy", SMALL_BUFFER_SIZE + ALIGNMENT_UNIT, ALIGNMENT_UNIT);
...
static inline void fill_addr_arrays(size_t *dst_addr, int is_dst_cached,
                                    size_t dst_uoffset, size_t *src_addr,
                                    int is_src_cached, size_t src_uoffset) {
  unsigned int i;
 ...
 */
static void do_uncached_write(uint8_t *dst, int is_dst_cached,
                              const uint8_t *src, int is_src_cached,
                              size_t size) {
  unsigned i, j;
 ...
 */
#define SINGLE_PERF_TEST(dst, is_dst_cached, dst_uoffset, src, is_src_cached,  \
                         src_uoffset, size)                                    \
  do {                                                                         \
    unsigned int iter, t;                                                      \
    size_t dst_addrs[TEST_BATCH_SIZE], src_addrs[TEST_BATCH_SIZE];             \
    ...
    for (iter = 0; iter < (TEST_ITERATIONS / TEST_BATCH_SIZE); iter++) {       \
      fill_addr_arrays(dst_addrs, is_dst_cached, dst_uoffset, src_addrs,       \
                       is_src_cached, src_uoffset);                            \
    ...
    printf("(%6.2f%%) ",                                                       \
           ((double)total_time - total_time2) * 100 / total_time2);            \
  } while (0)
...
static int perf_test(void) {
  ...
  printf(
      ...
      "======= ================= ================= ================= "
      "=================\n"
      "   Size   Cache to cache     Cache to mem      Mem to cache        Mem "
      "to mem\n"
...
static int test_memcpy_perf(void) {
  int ret;
