...
static int test_reciprocal(void) {
  int result = 0;
  ...
  for (i = 0; i < MAX_ITERATIONS; i++) {
    ...
    rresult_u32 = rte_reciprocal_divide(dividend_u32, reci_u32);
    if (nresult_u32 != rresult_u32) {
      printf("Division failed, %" PRIu32 "/%" PRIu32 " = "
             "expected %" PRIu32 " result %" PRIu32 "\n",
             dividend_u32, divisor_u32, nresult_u32, rresult_u32);
      result = 1;
      break;
    }
  }
  ...
  for (i = 0; i < MAX_ITERATIONS; i++) {
    ...
    rresult_u64 = rte_reciprocal_divide_u64(dividend_u64, &reci_u64);
  ...
  for (i = 0; i < 32; i++) {
    ...
    for (j = 0; j < MAX_ITERATIONS >> 4; j++) {
      ...
      rresult_u32 = rte_reciprocal_divide((dividend_u64 >> 32), reci_u32);

      if (nresult_u32 != rresult_u32) {
        printf("Division 32 failed, %" PRIu64 "/%" PRIu64 " = "
               "expected %" PRIu64 " result %" PRIu64 "\n",
               dividend_u64 >> 32, divisor_u64, nresult_u64, rresult_u64);
        result = 1;
        break;
      }
    }
