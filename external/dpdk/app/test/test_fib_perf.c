...
#define IP_HEAD_BIT_NUM_C 3
  ...
  for (k = 0; k < rule_num; k++) {
    ptr_rule->ip = (start << (RTE_FIB_MAX_DEPTH - depth)) | ip_head_mask;
...
static int test_fib_perf(void) {
  ...
  for (i = 0; i < NUM_ROUTE_ENTRIES; i++) {
    if (rte_fib_add(fib, large_route_table[i].ip, large_route_table[i].depth,
                    next_hop_add) == 0)
      status++;
  }
  /* End Timer. */
  ...
  for (i = 0; i < ITERATIONS; i++) {
    ...
    for (j = 0; j < BATCH_SIZE; j += BULK_SIZE) {
      uint32_t k;
      rte_fib_lookup_bulk(fib, &ip_batch[j], next_hops, BULK_SIZE);
