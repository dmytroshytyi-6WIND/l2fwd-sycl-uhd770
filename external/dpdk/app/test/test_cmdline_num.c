...
static int can_parse_unsigned(uint64_t expected_result,
...
static int can_parse_signed(int64_t expected_result,
                            enum cmdline_numtype type) {
  switch (type) {
  case RTE_UINT8:
...
int test_parse_num_invalid_param(void) {
  ...
  /* try null result */
  ret = cmdline_parse_num((cmdline_parse_token_hdr_t *)&token, buf, NULL, 0);
  if (ret == -1) {
  ...
  /* coverage! */
  ret = cmdline_get_help_num((cmdline_parse_token_hdr_t *)&token, buf,
                             sizeof(buf));
  if (ret < 0) {
  ...
  for (type = RTE_UINT8; type <= RTE_INT64; type++) {
    ...
    for (i = 0; i < RTE_DIM(num_invalid_strs); i++) {
      ...
      if (ret != -1) {
        ...
        cmdline_get_help_num((cmdline_parse_token_hdr_t *)&token, buf,
                             sizeof(buf));
...
int test_parse_num_valid(void) {
  ...
  for (type = RTE_UINT8; type <= RTE_INT64; type++) {
    ...
    for (i = 0; i < RTE_DIM(num_valid_positive_strs); i++) {
      ...
      ret = cmdline_parse_num((cmdline_parse_token_hdr_t *)&token,
                              num_valid_positive_strs[i].str, (void *)&result,
                              sizeof(result));

      /* if it should have passed but didn't, or if it should have failed but
       * didn't */
      if ((ret < 0) ==
          (can_parse_unsigned(num_valid_positive_strs[i].result, type) > 0)) {
       ...
       * since unsigned numbers don't care about number of bits, we can just
       * convert everything to uint64_t without any worries. */
    ...
    for (i = 0; i < RTE_DIM(num_valid_negative_strs); i++) {
      ...
      ret = cmdline_parse_num((cmdline_parse_token_hdr_t *)&token,
                              num_valid_negative_strs[i].str, (void *)&result,
                              sizeof(result));

      /* if it should have passed but didn't, or if it should have failed but
       * didn't */
      if ((ret < 0) ==
          (can_parse_signed(num_valid_negative_strs[i].result, type) > 0)) {
        printf("Error: parser behaves unexpectedly when parsing %s as %s!\n",
  ...
  for (type = RTE_UINT8; type <= RTE_INT64; type++) {
    ...
    for (i = 0; i < RTE_DIM(num_garbage_positive_strs); i++) {
      ...
      ret = cmdline_parse_num((cmdline_parse_token_hdr_t *)&token,
                              num_garbage_positive_strs[i].str, (void *)&result,
                              sizeof(result));

      /* if it should have passed but didn't, or if it should have failed but
       * didn't */
      if ((ret < 0) ==
          (can_parse_unsigned(num_garbage_positive_strs[i].result, type) > 0)) {
        printf("Error: parser behaves unexpectedly when parsing %s as %s!\n",
    ...
    for (i = 0; i < RTE_DIM(num_garbage_negative_strs); i++) {
      ...
      ret = cmdline_parse_num((cmdline_parse_token_hdr_t *)&token,
                              num_garbage_negative_strs[i].str, (void *)&result,
                              sizeof(result));

      /* if it should have passed but didn't, or if it should have failed but
       * didn't */
      if ((ret < 0) ==
          (can_parse_signed(num_garbage_negative_strs[i].result, type) > 0)) {
        printf("Error: parser behaves unexpectedly when parsing %s as %s!\n",
  ...
  /* coverage! */
  cmdline_get_help_num((cmdline_parse_token_hdr_t *)&token, buf, sizeof(buf));
