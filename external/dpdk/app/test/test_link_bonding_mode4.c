...
static uint8_t lacpdu_rx_count[RTE_MAX_ETHPORTS] = {
    0,
};
...
#define FOR_EACH_PORT(_i, _port)                                               \
  FOR_EACH(_i, _port, test_params.slave_ports, RTE_DIM(test_params.slave_ports))
 ...
 */
#define FOR_EACH_PORT_IF(_i, _port, _condition)                                \
  FOR_EACH_PORT((_i), (_port))                                                 \
  if (!!(_condition))
 ...
 */
static int slave_get_pkts(struct slave_conf *slave, struct rte_mbuf **buf,
                          uint16_t size) {
  return rte_ring_dequeue_burst(slave->tx_queue, (void **)buf, size, NULL);
}
 ...
 */
static int slave_put_pkts(struct slave_conf *slave, struct rte_mbuf **buf,
                          uint16_t size) {
  return rte_ring_enqueue_burst(slave->rx_queue, (void **)buf, size, NULL);
}

static uint16_t bond_rx(struct rte_mbuf **buf, uint16_t size) {
  return rte_eth_rx_burst(test_params.bonded_port_id, 0, buf, size);
}

static uint16_t bond_tx(struct rte_mbuf **buf, uint16_t size) {
  return rte_eth_tx_burst(test_params.bonded_port_id, 0, buf, size);
}

static void free_pkts(struct rte_mbuf **pkts, uint16_t count) {
  uint16_t i;
...
static int configure_ethdev(uint16_t port_id, uint8_t start) {
  ...
  if (start) {
    TEST_ASSERT(rte_eth_dev_start(port_id) == 0, "Failed to start device (%d).",
                port_id);
  }
  return 0;
}

static int add_slave(struct slave_conf *slave, uint8_t start) {
  struct rte_ether_addr addr, addr_check;
  int retval;

  /* Some sanity check */
  RTE_VERIFY(test_params.slave_ports <= slave &&
             slave - test_params.slave_ports <
                 (int)RTE_DIM(test_params.slave_ports));
  ...
  TEST_ASSERT_SUCCESS(
      rte_eth_bond_slave_add(test_params.bonded_port_id, slave->port_id),
...
static int remove_slave(struct slave_conf *slave) {
  ...
  TEST_ASSERT_EQUAL(
      rte_eth_bond_slave_remove(test_params.bonded_port_id, slave->port_id), 0,
      "Failed to remove slave (idx=%u, id=%u) from bonding (id=%u)",
      (uint8_t)slave_idx, slave->port_id, test_params.bonded_port_id);
...
static void lacp_recv_cb(uint16_t slave_id, struct rte_mbuf *lacp_pkt) {
...
static int initialize_bonded_device_with_slaves(uint16_t slave_count,
  ...
  TEST_ASSERT_SUCCESS(ret, "Failed disable promiscuous mode for port %d: %s",
...
static int remove_slaves_and_stop_bonded_device(void) {
  ...
  FOR_EACH_PORT(i, slave) {
    TEST_ASSERT_SUCCESS(rte_eth_dev_stop(slave->port_id),
                        "Failed to stop bonded port %u", slave->port_id);

    TEST_ASSERT(slave->bonded == 0, "Port id=%u is still marked as enslaved.",
                slave->port_id);
  }
...
static int test_setup(void) {
  ...
  if (test_params.mbuf_pool == NULL) {
    nb_mbuf_per_pool =
        TEST_RX_DESC_MAX + DEF_PKT_BURST + TEST_TX_DESC_MAX + MAX_PKT_BURST;
    test_params.mbuf_pool =
        rte_pktmbuf_pool_create("TEST_MODE4", nb_mbuf_per_pool, MBUF_CACHE_SIZE,
                                0, RTE_MBUF_DEFAULT_BUF_SIZE, socket_id);

    TEST_ASSERT(test_params.mbuf_pool != NULL, "rte_mempool_create failed\n");
  }
  ...
  FOR_EACH_PORT(i, port) {
    ...
    if (port->rx_queue == NULL) {
      ...
      TEST_ASSERT(port->rx_queue != NULL, "Failed to allocate rx ring '%s': %s",
    ...
    if (port->port_id == INVALID_PORT_ID) {
      ...
      TEST_ASSERT(retval < (int)RTE_DIM(name) - 1, "Name too long");
      retval = rte_eth_from_rings(name, &port->rx_queue, 1, &port->tx_queue, 1,
                                  socket_id);
      TEST_ASSERT(retval >= 0, "Failed to create ring ethdev '%s'\n", name);
  ...
  if (test_params.bonded_port_id == INVALID_PORT_ID) {
    retval =
        rte_eth_bond_create(BONDED_DEV_NAME, BONDING_MODE_8023AD, socket_id);
 ...
 * frame but not LACP
 */
static int make_lacp_reply(struct slave_conf *slave, struct rte_mbuf *pkt) {
  struct rte_ether_hdr *hdr;
  struct slow_protocol_frame *slow_hdr;
  struct lacpdu *lacp;
  ...
  lacp->actor.state = STATE_LACP_ACTIVE | STATE_SYNCHRONIZATION |
 ...
 */
static int bond_handshake_reply(struct slave_conf *slave) {
  int retval;
  ...
  TEST_ASSERT(retval >= 0, "Getting slave %u packets failed.", slave->port_id);
 ...
 */
static int bond_handshake_done(struct slave_conf *slave) {
...
static unsigned bond_get_update_timeout_ms(void) {
  struct rte_eth_bond_8023ad_conf conf;

  if (rte_eth_bond_8023ad_conf_get(test_params.bonded_port_id, &conf) < 0) {
    RTE_LOG(DEBUG, EAL,
            "Failed to get bonding configuration: "
            "%s at %d\n",
            __func__, __LINE__);
 ...
 */
static int bond_handshake(void) {
  struct slave_conf *slave;
...
static int test_mode4_lacp(void) {
  int retval;
...
static int test_mode4_agg_mode_selection(void) {
  int retval;
  /* Test and verify for Stable mode */
  retval = initialize_bonded_device_with_slaves(TEST_LACP_SLAVE_COUT, 0);
  TEST_ASSERT_SUCCESS(retval, "Failed to initialize bonded device");

  retval = rte_eth_bond_8023ad_agg_selection_set(test_params.bonded_port_id,
                                                 AGG_STABLE);
  TEST_ASSERT_SUCCESS(retval, "Failed to initialize bond aggregation mode");
  retval = bond_handshake();
  TEST_ASSERT_SUCCESS(retval, "Initial handshake failed");

  retval = rte_eth_bond_8023ad_agg_selection_get(test_params.bonded_port_id);
...
static int generate_packets(struct rte_ether_addr *src_mac,
  ...
  initialize_eth_header(&pkt_eth_hdr, src_mac, dst_mac, ip4_type, vlan_enable,
                        vlan_id);
  ...
  retval = generate_packet_burst(test_params.mbuf_pool, buf, &pkt_eth_hdr,
                                 vlan_enable, &pkt_ip_hdr, 1, &pkt_udp_hdr,
                                 count, pktlen, 1);

  if (retval > 0 && retval != count)
    free_pkts(&buf[count - retval], retval);

  TEST_ASSERT_EQUAL(retval, count, "Failed to generate %u packets", count);

  return count;
}

static int generate_and_put_packets(struct slave_conf *slave,
  ...
  TEST_ASSERT_EQUAL(retval, count,
                    "Failed to enqueue packets into slave %u RX queue",
                    slave->port_id);

  return TEST_SUCCESS;
}

static int test_mode4_rx(void) {
  struct slave_conf *slave;
  uint16_t i, j;
  ...
  retval = rte_eth_rx_burst(test_params.bonded_port_id, 0, pkts, RTE_DIM(pkts));
  ...
  if (retval == expected_pkts_cnt) {
    ...
    for (i = 0; i < expected_pkts_cnt; i++) {
      ...
      cnt[rte_is_same_ether_addr(&hdr->dst_addr, &bonded_mac)]++;
    }
    ...
    TEST_ASSERT(
        cnt[0] == expected_pkts_cnt / 2 && cnt[1] == expected_pkts_cnt / 2,
  ...
  TEST_ASSERT_EQUAL(retval, expected_pkts_cnt,
                    "Expected %u packets but received only %d",
                    expected_pkts_cnt, retval);
  ...
  if (retval == expected_pkts_cnt) {
    ...
    for (i = 0; i < expected_pkts_cnt; i++) {
      hdr = rte_pktmbuf_mtod(pkts[i], struct rte_ether_hdr *);
      eq_cnt += rte_is_same_ether_addr(&hdr->dst_addr, &bonded_mac);
    }
  ...
  FOR_EACH_SLAVE(i, slave) {
    ...
    for (j = 0; j < 5; j++) {
      TEST_ASSERT(bond_handshake_reply(slave) >= 0, "Handshake after link up");
...
static int test_mode4_tx_burst(void) {
  struct slave_conf *slave;
  uint16_t i, j;
  ...
  FOR_EACH_SLAVE(i, slave) {
    ...
    TEST_ASSERT_NOT_EQUAL(normal_cnt, 0,
                          "slave %u did not transmitted any packets",
                          slave->port_id);
  ...
  TEST_ASSERT_EQUAL(exp_pkts_cnt, pkts_cnt,
                    "Expected %u packets but transmitted only %d", exp_pkts_cnt,
                    pkts_cnt);
  ...
  for (pkts_cnt = 0; pkts_cnt < RTE_DIM(pkts); pkts_cnt++) {
    ...
    TEST_ASSERT_EQUAL(retval, 1, "Failed to generate test packet %u", pkts_cnt);
  }
  exp_pkts_cnt = pkts_cnt;
...
static void init_marker(struct rte_mbuf *pkt, struct slave_conf *slave) {
  struct marker_header *marker_hdr =
      rte_pktmbuf_mtod(pkt, struct marker_header *);

  /* Copy multicast destination address */
  rte_ether_addr_copy(&slow_protocol_mac_addr, &marker_hdr->eth_hdr.dst_addr);

  /* Init source address */
  rte_ether_addr_copy(&parnter_mac_default, &marker_hdr->eth_hdr.src_addr);
  ...
  marker_hdr->marker.info_length = offsetof(struct marker, reserved_90) -
...
static int test_mode4_marker(void) {
  struct slave_conf *slave;
  struct rte_mbuf *pkts[MAX_PKT_BURST];
  ...
  FOR_EACH_SLAVE(i, slave) {
    ...
    TEST_ASSERT_EQUAL(retval, 1, "Failed to send marker packet to slave %u",
                      slave->port_id);

    for (j = 0; j < 20; ++j) {
      rte_delay_ms(delay);
      retval =
          rte_eth_rx_burst(test_params.bonded_port_id, 0, pkts, RTE_DIM(pkts));
...
static int test_mode4_expired(void) {
  ...
  for (j = 0; j < conf.long_timeout_ms / conf.update_timeout_ms + 2; j++) {
    ...
    TEST_ASSERT_EQUAL(retval, 0, "Unexpectedly received %d packets", retval);
...
static int test_mode4_ext_ctrl(void) {
  /*
  ...
  for (i = 0; i < SLAVE_COUNT; i++) {
    ...
    rte_memcpy(rte_pktmbuf_mtod(lacp_tx_buf[i], char *), &lacpdu,
               sizeof(lacpdu));
  ...
  FOR_EACH_SLAVE(i, slave) {
    TEST_ASSERT_FAIL(rte_eth_bond_8023ad_ext_slowtx(test_params.bonded_port_id,
                                                    slave->port_id,
                                                    lacp_tx_buf[i]),
                     "Slave should not allow manual LACP xmit");
    TEST_ASSERT_FAIL(rte_eth_bond_8023ad_ext_collect(test_params.bonded_port_id,
                                                     slave->port_id, 1),
...
static int test_mode4_ext_lacp(void) {
  int retval;
  struct slave_conf *slave = NULL;
  ...
  FOR_EACH_SLAVE(i, slave) {
    retval = rte_eth_bond_8023ad_ext_slowtx(test_params.bonded_port_id,
                                            slave->port_id, lacp_tx_buf[i]);
    TEST_ASSERT_SUCCESS(retval, "Slave should allow manual LACP xmit");
  }
  ...
  for (i = 0; i < 30 && all_slaves_done == 0; ++i) {
    ...
    rte_delay_ms(delay);
    FOR_EACH_SLAVE(s, slave) { total += lacpdu_rx_count[slave->port_id]; }
  ...
  FOR_EACH_SLAVE(i, slave) {
    TEST_ASSERT_EQUAL(lacpdu_rx_count[slave->port_id], 1,
                      ...
                      slave->port_id, lacpdu_rx_count[slave->port_id]);
  }
...
static int check_environment(void) {
  ...
  slaves_count = rte_eth_bond_slaves_get(test_params.bonded_port_id, slaves,
                                         RTE_DIM(slaves));

  if (slaves_count != 0)
    env_state |= 0x10;

  TEST_ASSERT_EQUAL(env_state, 0, "Environment not clean (port %u):%s%s%s%s%s",
                    port->port_id,
...
static int test_mode4_executor(int (*test_func)(void)) {
  struct slave_conf *port;
  int test_result;
  uint8_t i;
  void *pkt;
...
static int test_mode4_agg_mode_selection_wrapper(void) {
  return test_mode4_executor(&test_mode4_agg_mode_selection);
}

static int test_mode4_lacp_wrapper(void) {
  return test_mode4_executor(&test_mode4_lacp);
}

static int test_mode4_marker_wrapper(void) {
  return test_mode4_executor(&test_mode4_marker);
}

static int test_mode4_rx_wrapper(void) {
  return test_mode4_executor(&test_mode4_rx);
}

static int test_mode4_tx_burst_wrapper(void) {
  return test_mode4_executor(&test_mode4_tx_burst);
}

static int test_mode4_expired_wrapper(void) {
  return test_mode4_executor(&test_mode4_expired);
}

static int test_mode4_ext_ctrl_wrapper(void) {
  return test_mode4_executor(&test_mode4_ext_ctrl);
}

static int test_mode4_ext_lacp_wrapper(void) {
...
static int test_link_bonding_mode4(void) {
