...
struct test_thash_v4 v4_tbl[] = {
    {RTE_IPV4(161, 142, 100, 80), RTE_IPV4(66, 9, 149, 187), 1766, 2794,
     0x323e8fc2, 0x51ccc178},
    {RTE_IPV4(65, 69, 140, 83), RTE_IPV4(199, 92, 111, 2), 4739, 14230,
     0xd718262a, 0xc626b0ea},
    {RTE_IPV4(12, 22, 207, 184), RTE_IPV4(24, 19, 198, 95), 38024, 12898,
     0xd2d0a5de, 0x5c2b394a},
    {RTE_IPV4(209, 142, 163, 6), RTE_IPV4(38, 27, 205, 30), 2217, 48228,
     0x82989176, 0xafc7327f},
    {RTE_IPV4(202, 188, 127, 2), RTE_IPV4(153, 39, 163, 191), 1303, 44251,
...
static const uint8_t big_rss_key[] = {
    ...
    0x41, 0x67, 0x25, 0x3d, 0x43, 0xa3, 0x8f, 0xb0, 0xd0, 0xca, 0x2b, 0xcb,
    0xae, 0x7b, 0x30, 0xb4, 0x77, 0xcb, 0x2d, 0xa3, 0x80, 0x30, 0xf2, 0x0c,
    0x6a, 0x42, 0xb7, 0x3b, 0xbe, 0xac, 0x01, 0xfa, 0x6d, 0x5a, 0x56, 0xda,
    0x25, 0x5b, 0x0e, 0xc2, 0x41, 0x67, 0x25, 0x3d, 0x43, 0xa3, 0x8f, 0xb0,
...
static int test_toeplitz_hash_calc(void) {
  uint32_t i, j;
  ...
  for (i = 0; i < RTE_DIM(v4_tbl); i++) {
    ...
    rss_l3 =
        rte_softrss((uint32_t *)&tuple, RTE_THASH_V4_L3_LEN, default_rss_key);
    rss_l3l4 =
        rte_softrss((uint32_t *)&tuple, RTE_THASH_V4_L4_LEN, default_rss_key);
    if ((rss_l3 != v4_tbl[i].hash_l3) || (rss_l3l4 != v4_tbl[i].hash_l3l4))
      return -TEST_FAILED;
    /*Calculate hash with converted key*/
    rss_l3 =
        rte_softrss_be((uint32_t *)&tuple, RTE_THASH_V4_L3_LEN, rss_key_be);
    rss_l3l4 =
  ...
  for (i = 0; i < RTE_DIM(v6_tbl); i++) {
    ...
    /*Calculate hash with original key*/
    rss_l3 =
        rte_softrss((uint32_t *)&tuple, RTE_THASH_V6_L3_LEN, default_rss_key);
    rss_l3l4 =
        rte_softrss((uint32_t *)&tuple, RTE_THASH_V6_L4_LEN, default_rss_key);
    if ((rss_l3 != v6_tbl[i].hash_l3) || (rss_l3l4 != v6_tbl[i].hash_l3l4))
      return -TEST_FAILED;
    /*Calculate hash with converted key*/
    rss_l3 =
        rte_softrss_be((uint32_t *)&tuple, RTE_THASH_V6_L3_LEN, rss_key_be);
    rss_l3l4 =
        rte_softrss_be((uint32_t *)&tuple, RTE_THASH_V6_L4_LEN, rss_key_be);
    if ((rss_l3 != v6_tbl[i].hash_l3) || (rss_l3l4 != v6_tbl[i].hash_l3l4))
      return -TEST_FAILED;
  }
  return TEST_SUCCESS;
...
static int test_toeplitz_hash_gfni(void) {
  uint32_t i, j;
  union rte_thash_tuple tuple;
  uint32_t rss_l3, rss_l3l4;
...
static int test_toeplitz_hash_rand_data(void) {
  ...
  for (i = 0; i < ITER; i++) {
    ...
    hash[SCALAR_DATA_BUF_1_HASH_IDX] =
        rte_softrss(scalar_data[0], DATA_SZ, default_rss_key);
    hash[SCALAR_DATA_BUF_2_HASH_IDX] =
        rte_softrss(scalar_data[1], DATA_SZ, default_rss_key);
    ...
    hash[GFNI_DATA_BUF_2_HASH_IDX] = rte_thash_gfni(
        rss_key_matrixes, (uint8_t *)data[1], DATA_SZ * sizeof(uint32_t));
    rte_thash_gfni_bulk(rss_key_matrixes, DATA_SZ * sizeof(uint32_t), bulk_data,
                        &hash[GFNI_BULK_DATA_BUF_1_HASH_IDX], 2);

    if ((hash[SCALAR_DATA_BUF_1_HASH_IDX] != hash[GFNI_DATA_BUF_1_HASH_IDX]) ||
        (hash[SCALAR_DATA_BUF_1_HASH_IDX] !=
         hash[GFNI_BULK_DATA_BUF_1_HASH_IDX]) ||
        (hash[SCALAR_DATA_BUF_2_HASH_IDX] != hash[GFNI_DATA_BUF_2_HASH_IDX]) ||
        (hash[SCALAR_DATA_BUF_2_HASH_IDX] !=
...
enum { RSS_V4_IDX, RSS_V6_IDX };

static int test_toeplitz_hash_gfni_bulk(void) {
  uint32_t i, j;
  ...
  for (i = 0; i < RTE_MIN(RTE_DIM(v4_tbl), RTE_DIM(v6_tbl)); i++) {
    ...
    rte_thash_gfni_bulk(rss_key_matrixes, RTE_THASH_V6_L4_LEN * 4, tuples, rss,
                        2);
...
static int test_big_tuple_gfni(void) {
  uint32_t arr[16];
...
static int test_create_invalid(void) {
  ...
  RTE_TEST_ASSERT(ctx == NULL, "Call succeeded with invalid parameters\n");
...
static int test_multiple_create(void) {
  struct rte_thash_ctx *ctx;
  int key_len = 40;
  int reta_sz = 7;
  int i;
...
static int test_add_invalid_helper(void) {
  struct rte_thash_ctx *ctx;
  ...
  RTE_TEST_ASSERT(ret == -EINVAL, "Call succeeded with invalid parameters\n");
  ...
  ret = rte_thash_add_helper(ctx, "first_range", reta_sz, 0);
  RTE_TEST_ASSERT(ret == -EEXIST, "Call succeeded with duplicated name\n");
   ...
   */
  ret = rte_thash_add_helper(ctx, "second_range", reta_sz, 32 + 2 * reta_sz);
  RTE_TEST_ASSERT(ret == 0, "Can not create helper\n");
  ...
  RTE_TEST_ASSERT(ret == -EEXIST, "Call succeeded with overlapping ranges\n");
...
static int test_get_helper(void) {
  struct rte_thash_ctx *ctx;
...
static int test_period_overflow(void) {
  struct rte_thash_ctx *ctx;
  ...
  RTE_TEST_ASSERT(ret == -ENOSPC, "Call succeeded with invalid parameters\n");
...
static int test_predictable_rss_min_seq(void) {
  struct rte_thash_ctx *ctx;
  struct rte_thash_subtuple_helper *h;
  const int key_len = 40;
  int reta_sz = 6;
  ...
  orig_hash = rte_softrss((uint32_t *)&tuple, RTE_THASH_V4_L4_LEN, new_key);
  ...
  adj_hash = rte_softrss((uint32_t *)&tuple, RTE_THASH_V4_L4_LEN, new_key);
  RTE_TEST_ASSERT((adj_hash & HASH_MSK(reta_sz)) == desired_value,
                  "bad desired value\n");
...
struct range rng_arr[] = {{"one", 8, 56, 7},     {"two", 8, 64, 8},
                          {"three", 8, 120, 15}, {"four", 8, 48, 6},
                          {"six", 40, 40, 9},    {"five", 7, 57, 7},
                          {"seven", 8, 0, 0}};

static int test_predictable_rss_multirange(void) {
  struct rte_thash_ctx *ctx;
  ...
  for (i = 0; i < RTE_DIM(rng_arr); i++) {
    ret = rte_thash_add_helper(ctx, rng_arr[i].name, rng_arr[i].len,
                               rng_arr[i].offset);
  ...
  for (i = 0; i < RTE_DIM(rng_arr); i++) {
    for (k = 0; k < 100; k++) {
      ...
      adj = rte_thash_get_complement(h[i], hashes[i], desired_value);
      ...
      RTE_TEST_ASSERT((adj_hashes[i] & HASH_MSK(reta_sz)) == desired_value,
...
static int cmp_tuple_eq(void *userdata, uint8_t *tuple) {
  return memcmp(userdata, tuple, TUPLE_SZ);
}

static int test_adjust_tuple(void) {
  struct rte_thash_ctx *ctx;
  struct rte_thash_subtuple_helper *h;
  const int key_len = 40;
  const uint8_t *new_key;
   ...
   */
  ret = rte_thash_add_helper(ctx, "test", reta_sz * 2, (5 * CHAR_BIT) + 4);
  RTE_TEST_ASSERT(ret == 0, "can not add helper, ret %d\n", ret);
  ...
  ret = rte_thash_adjust_tuple(ctx, h, tuple, TUPLE_SZ, desired_value, 1, NULL,
                               NULL);
  RTE_TEST_ASSERT(ret == 0, "can not adjust tuple, ret %d\n", ret);

  for (i = 0; i < (TUPLE_SZ / 4); i++)
    tmp_tuple[i] = rte_be_to_cpu_32(*(uint32_t *)&tuple[i * 4]);

  hash = rte_softrss(tmp_tuple, TUPLE_SZ / 4, new_key);
  RTE_TEST_ASSERT((hash & HASH_MSK(reta_sz)) == desired_value,
                  "bad desired value\n");
  ...
  memset(tuple, 0xab, TUPLE_SZ);
  ret = rte_thash_adjust_tuple(ctx, h, tuple, TUPLE_SZ, desired_value, 1,
                               cmp_tuple_eq, tuple_copy);
   ...
   */
  memset(tuple, 0xab, TUPLE_SZ);
  ret = rte_thash_adjust_tuple(ctx, h, tuple, TUPLE_SZ, desired_value, 2,
                               cmp_tuple_eq, tuple_copy);
  RTE_TEST_ASSERT(ret == 0, "can not adjust tuple, ret %d\n", ret);
...
static struct unit_test_suite thash_tests = {
    ...
    .teardown = NULL,
    .unit_test_cases = {
        TEST_CASE(test_toeplitz_hash_calc), TEST_CASE(test_toeplitz_hash_gfni),
        TEST_CASE(test_toeplitz_hash_rand_data),
        TEST_CASE(test_toeplitz_hash_gfni_bulk), TEST_CASE(test_big_tuple_gfni),
        TEST_CASE(test_create_invalid), TEST_CASE(test_multiple_create),
        TEST_CASE(test_free_null), TEST_CASE(test_add_invalid_helper),
        TEST_CASE(test_find_existing), TEST_CASE(test_get_helper),
        TEST_CASE(test_period_overflow),
        TEST_CASE(test_predictable_rss_min_seq),
        TEST_CASE(test_predictable_rss_multirange),
        TEST_CASE(test_adjust_tuple), TEST_CASES_END()}};

static int test_thash(void) { return unit_test_suite_runner(&thash_tests); }
