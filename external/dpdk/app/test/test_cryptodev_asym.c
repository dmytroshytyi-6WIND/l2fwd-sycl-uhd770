...
static int queue_ops_rsa_sign_verify(void *sess) {
...
static int queue_ops_rsa_enc_dec(void *sess) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *op_mpool = ts_params->op_mpool;
  uint8_t dev_id = ts_params->valid_devs[0];
  struct rte_crypto_op *op, *result_op;
...
static int test_cryptodev_asym_ver(struct rte_crypto_op *op,
...
test_cryptodev_asym_op(struct crypto_testsuite_params_asym *ts_params,
                       union test_case_structure *data_tc, char *test_msg,
                       int sessionless, enum rte_crypto_asym_op_type type,
  ...
  op = rte_crypto_op_alloc(ts_params->op_mpool, RTE_CRYPTO_OP_TYPE_ASYMMETRIC);

  if (!op) {
    snprintf(test_msg, ASYM_TEST_MSG_LEN, "line %u FAILED: %s", __LINE__,
             "Failed to allocate asymmetric crypto "
             "operation struct");
    status = TEST_FAILED;
    goto error_exit;
  ...
  case RTE_CRYPTO_ASYM_XFORM_MODEX:
    ...
    memcpy(input, data_tc->modex.base.data, data_tc->modex.base.len);
    ...
    if (rte_cryptodev_asym_xform_capability_check_modlen(
            capability, xform_tc.modex.modulus.length)) {
      snprintf(test_msg, ASYM_TEST_MSG_LEN,
               "line %u "
               "FAILED: %s",
               __LINE__, "Invalid MODULUS length specified");
      status = TEST_FAILED;
      goto error_exit;
    }
    break;
  case RTE_CRYPTO_ASYM_XFORM_MODINV:
    ...
    memcpy(input, data_tc->modinv.base.data, data_tc->modinv.base.len);
    ...
    if (rte_cryptodev_asym_xform_capability_check_modlen(
            capability, xform_tc.modinv.modulus.length)) {
      snprintf(test_msg, ASYM_TEST_MSG_LEN,
               "line %u "
               "FAILED: %s",
               __LINE__, "Invalid MODULUS length specified");
  ...
  default:
    snprintf(test_msg, ASYM_TEST_MSG_LEN,
             "line %u "
             "FAILED: %s",
             __LINE__, "Invalid ASYM algorithm specified");
    status = TEST_FAILED;
    goto error_exit;
  }
  ...
  if (!sessionless) {
    ...
    if (ret < 0) {
      snprintf(test_msg, ASYM_TEST_MSG_LEN,
               "line %u "
               "FAILED: %s",
               __LINE__, "Session creation failed");
...
static int test_one_case(const void *test_case, int sessionless) {
  ...
  if (tc.modex.xform_type == RTE_CRYPTO_ASYM_XFORM_MODEX ||
    ...
    status = test_cryptodev_asym_op(&testsuite_params, &tc, test_msg,
                                    sessionless, 0, 0);
    printf("  %u) TestCase %s %s\n", test_index++, tc.modex.description,
           test_msg);
  } else {
      ...
      if (tc.modex.xform_type == RTE_CRYPTO_ASYM_XFORM_RSA) {
        if (tc.rsa_data.op_type_flags & (1 << i)) {
          if (tc.rsa_data.key_exp) {
            status =
                test_cryptodev_asym_op(&testsuite_params, &tc, test_msg,
                                       sessionless, i, RTE_RSA_KEY_TYPE_EXP);
          }
          if (status)
            break;
          if (tc.rsa_data.key_qt && (i == RTE_CRYPTO_ASYM_OP_DECRYPT ||
                                     i == RTE_CRYPTO_ASYM_OP_SIGN)) {
            status =
                test_cryptodev_asym_op(&testsuite_params, &tc, test_msg,
                                       sessionless, i, RTE_RSA_KEY_TYPE_QT);
          }
          if (status)
            break;
        }
...
static int test_one_by_one(void) {
  ...
  if ((dev_info.feature_flags & RTE_CRYPTODEV_FF_ASYM_SESSIONLESS)) {
    sessionless = 1;
  }
  ...
  if (sessionless) {
    for (i = 0; i < test_vector.size; i++) {
      if (test_one_case(test_vector.address[i], 1) != TEST_SUCCESS)
        status = TEST_FAILED;
    }
  }
...
static int test_rsa_sign_verify(void) {
  ...
  ret =
      rte_cryptodev_asym_session_create(dev_id, &rsa_xform, sess_mpool, &sess);
...
static int test_rsa_enc_dec(void) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *sess_mpool = ts_params->session_mpool;
  uint8_t dev_id = ts_params->valid_devs[0];
  void *sess = NULL;
  ...
  if (!(dev_info.feature_flags & RTE_CRYPTODEV_FF_RSA_PRIV_OP_KEY_EXP)) {
    RTE_LOG(INFO, USER1,
            "Device doesn't support decrypt op with "
            "exponent key type. Test skipped\n");
    return TEST_SKIPPED;
  }
...
static int test_rsa_sign_verify_crt(void) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *sess_mpool = ts_params->session_mpool;
  uint8_t dev_id = ts_params->valid_devs[0];
  void *sess = NULL;
  ...
  ret = rte_cryptodev_asym_session_create(dev_id, &rsa_xform_crt, sess_mpool,
                                          &sess);
...
static int test_rsa_enc_dec_crt(void) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *sess_mpool = ts_params->session_mpool;
  uint8_t dev_id = ts_params->valid_devs[0];
  void *sess = NULL;
...
static int testsuite_setup(void) {
  ...
  ts_params->op_mpool = rte_crypto_op_pool_create(
      "CRYPTO_ASYM_OP_POOL", RTE_CRYPTO_OP_TYPE_ASYMMETRIC, TEST_NUM_BUFS, 0, 0,
      rte_socket_id());
  ...
  nb_devs =
      rte_cryptodev_devices_get(rte_cryptodev_driver_name_get(gbl_driver_id),
  ...
  if (dev_id == -1) {
    RTE_LOG(ERR, USER1,
            "Device doesn't support asymmetric. "
            "Test skipped.\n");
    return TEST_FAILED;
  }
  ...
  ts_params->conf.ff_disable =
      RTE_CRYPTODEV_FF_SECURITY | RTE_CRYPTODEV_FF_SYMMETRIC_CRYPTO;
  ...
  for (qp_id = 0; qp_id < info.max_nb_queue_pairs; qp_id++) {
    TEST_ASSERT_SUCCESS(
        ...
        "Failed to setup queue pair %u on cryptodev %u ASYM", qp_id, dev_id);
  }

  ts_params->session_mpool = rte_cryptodev_asym_session_pool_create(
      "test_asym_sess_mp", TEST_NUM_SESSIONS, 0, 0, SOCKET_ID_ANY);

  TEST_ASSERT_NOT_NULL(ts_params->session_mpool,
                       "session mempool allocation failed");
  /* >8 End of device, op pool and session configuration for asymmetric crypto
   * section. */
  return TEST_SUCCESS;
}
...
static void ut_teardown_asym(void) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
...
static inline void
print_asym_capa(const struct rte_cryptodev_asymmetric_xform_capability *capa) {
  int i = 0;
  ...
  case RTE_CRYPTO_ASYM_XFORM_DH:
  case RTE_CRYPTO_ASYM_XFORM_DSA:
    printf(" modlen: min %d max %d increment %d", capa->modlen.min,
           capa->modlen.max, capa->modlen.increment);
    break;
...
static int test_capability(void) {
  ...
  rte_cryptodev_info_get(dev_id, &dev_info);
  if (!(dev_info.feature_flags & RTE_CRYPTODEV_FF_ASYMMETRIC_CRYPTO)) {
  ...
  for (i = 0; dev_info.capabilities[i].op != RTE_CRYPTO_OP_TYPE_UNDEFINED;
    ...
    if (dev_info.capabilities[i].op == RTE_CRYPTO_OP_TYPE_ASYMMETRIC) {
      idx.type = dev_capa->asym.xform_capa.xform_type;

      capa = rte_cryptodev_asym_capability_get(
          dev_id, (const struct rte_cryptodev_asym_capability_idx *)&idx);
...
static int test_dh_gen_shared_sec(struct rte_crypto_asym_xform *xfrm) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *op_mpool = ts_params->op_mpool;
  struct rte_mempool *sess_mpool = ts_params->session_mpool;
  uint8_t dev_id = ts_params->valid_devs[0];
  ...
  debug_hexdump(stdout, "shared secret:", asym_op->dh.shared_secret.data,
...
static int test_dh_gen_priv_key(struct rte_crypto_asym_xform *xfrm) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *op_mpool = ts_params->op_mpool;
  struct rte_mempool *sess_mpool = ts_params->session_mpool;
  uint8_t dev_id = ts_params->valid_devs[0];
  ...
  debug_hexdump(stdout, "private key:", asym_op->dh.priv_key.data,
...
static int test_dh_gen_pub_key(struct rte_crypto_asym_xform *xfrm) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *op_mpool = ts_params->op_mpool;
  struct rte_mempool *sess_mpool = ts_params->session_mpool;
  uint8_t dev_id = ts_params->valid_devs[0];
  ...
  asym_op->dh.priv_key.data =
      rte_malloc(NULL, dh_test_params.priv_key.length, 0);
  ...
  debug_hexdump(stdout, "pub key:", asym_op->dh.pub_key.data,
                asym_op->dh.pub_key.length);

  debug_hexdump(stdout, "priv key:", asym_op->dh.priv_key.data,
                asym_op->dh.priv_key.length);
...
static int test_dh_gen_kp(struct rte_crypto_asym_xform *xfrm) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *op_mpool = ts_params->op_mpool;
  struct rte_mempool *sess_mpool = ts_params->session_mpool;
  uint8_t dev_id = ts_params->valid_devs[0];
  ...
  debug_hexdump(stdout, "priv key:", out_prv_key, asym_op->dh.priv_key.length);
  debug_hexdump(stdout, "pub key:", out_pub_key, asym_op->dh.pub_key.length);
...
static int test_mod_inv(void) {
  ...
  if (rte_cryptodev_asym_get_xform_enum(&modinv_xform.xform_type, "modinv") <
      0) {
  ...
  if (capability == NULL) {
    RTE_LOG(INFO, USER1, "Device doesn't support MOD INV. Test Skipped\n");
    return TEST_SKIPPED;
  }

  if (rte_cryptodev_asym_xform_capability_check_modlen(
          capability, modinv_xform.modinv.modulus.length)) {
    RTE_LOG(ERR, USER1, "Invalid MODULUS length specified\n");
    return TEST_SKIPPED;
  }

  ret = rte_cryptodev_asym_session_create(dev_id, &modinv_xform, sess_mpool,
                                          &sess);
  if (ret < 0) {
    RTE_LOG(ERR, USER1,
            "line %u "
...
static int test_mod_exp(void) {
  ...
  if (rte_cryptodev_asym_get_xform_enum(&modex_xform.xform_type, "modexp") <
      0) {
    RTE_LOG(ERR, USER1, "Invalid ASYM algorithm specified\n");
    return -1;
  }
  ...
  ret = rte_cryptodev_asym_session_create(dev_id, &modex_xform, sess_mpool,
                                          &sess);
  if (ret < 0) {
    RTE_LOG(ERR, USER1,
            "line %u "
...
static int test_dh_keygenration(void) {
  int status;
  ...
  RTE_LOG(INFO, USER1, "Test Public and Private key pair generation\n");
  ...
  RTE_LOG(INFO, USER1, "Test Private Key Generation only\n");

  status = test_dh_gen_priv_key(&dh_xform);
  TEST_ASSERT_EQUAL(status, 0, "Test failed");

  RTE_LOG(INFO, USER1, "Test shared secret compute\n");
...
static int test_dsa_sign(void) {
  ...
  ret =
      rte_cryptodev_asym_session_create(dev_id, &dsa_xform, sess_mpool, &sess);
  if (ret < 0) {
    RTE_LOG(ERR, USER1, "line %u FAILED: %s", __LINE__,
            "Session creation failed");
    status = (ret == -ENOTSUP) ? TEST_SKIPPED : TEST_FAILED;
  ...
  debug_hexdump(stdout, "p: ", dsa_xform.dsa.p.data, dsa_xform.dsa.p.length);
  debug_hexdump(stdout, "q: ", dsa_xform.dsa.q.data, dsa_xform.dsa.q.length);
  debug_hexdump(stdout, "g: ", dsa_xform.dsa.g.data, dsa_xform.dsa.g.length);
  ...
  debug_hexdump(stdout, "r:", asym_op->dsa.r.data, asym_op->dsa.r.length);
  debug_hexdump(stdout, "s:", asym_op->dsa.s.data, asym_op->dsa.s.length);
...
static int test_ecdsa_sign_verify(enum curve curve_id) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *sess_mpool = ts_params->session_mpool;
  struct rte_mempool *op_mpool = ts_params->op_mpool;
  ...
  debug_hexdump(stdout, "r:", asym_op->ecdsa.r.data, asym_op->ecdsa.r.length);
  debug_hexdump(stdout, "s:", asym_op->ecdsa.s.data, asym_op->ecdsa.s.length);

  ret = verify_ecdsa_sign(input_params.sign_r.data, input_params.sign_s.data,
                          result_op);
  if (ret) {
    status = TEST_FAILED;
    RTE_LOG(ERR, USER1, "line %u FAILED: %s", __LINE__, "ECDSA sign failed.\n");
    goto exit;
  }
...
static int test_ecdsa_sign_verify_all_curve(void) {
...
static int test_ecpm(enum curve curve_id) {
  struct crypto_testsuite_params_asym *ts_params = &testsuite_params;
  struct rte_mempool *sess_mpool = ts_params->session_mpool;
  struct rte_mempool *op_mpool = ts_params->op_mpool;
  ...
  debug_hexdump(stdout, "r x:", asym_op->ecpm.r.x.data,
                asym_op->ecpm.r.x.length);
  debug_hexdump(stdout, "r y:", asym_op->ecpm.r.y.data,
                asym_op->ecpm.r.y.length);

  ret = verify_ecpm(input_params.pubkey_x.data, input_params.pubkey_y.data,
                    result_op);
  if (ret) {
    status = TEST_FAILED;
    RTE_LOG(ERR, USER1, "line %u FAILED: %s", __LINE__,
...
static int test_ecpm_all_curve(void) {
  int status, overall_status = TEST_SUCCESS;
  enum curve curve_id;
  int test_index = 0;
  const char *msg;
...
static struct unit_test_suite cryptodev_openssl_asym_testsuite = {
    ...
    .unit_test_cases = {
        ...
        TEST_CASE_ST(ut_setup_asym, ut_teardown_asym, test_dh_keygenration),
        TEST_CASE_ST(ut_setup_asym, ut_teardown_asym, test_rsa_enc_dec),
        TEST_CASE_ST(ut_setup_asym, ut_teardown_asym, test_rsa_sign_verify),
        TEST_CASE_ST(ut_setup_asym, ut_teardown_asym, test_rsa_enc_dec_crt),
        TEST_CASE_ST(ut_setup_asym, ut_teardown_asym, test_rsa_sign_verify_crt),
...
static struct unit_test_suite cryptodev_octeontx_asym_testsuite = {
    ...
    .unit_test_cases = {
        ...
        TEST_CASE_ST(ut_setup_asym, ut_teardown_asym, test_ecpm_all_curve),
        TEST_CASES_END() /**< NULL terminate unit test array */
    }};

static int test_cryptodev_openssl_asym(void) {
  gbl_driver_id =
      rte_cryptodev_driver_id_get(RTE_STR(CRYPTODEV_NAME_OPENSSL_PMD));
...
static int test_cryptodev_qat_asym(void) {
  gbl_driver_id =
      rte_cryptodev_driver_id_get(RTE_STR(CRYPTODEV_NAME_QAT_ASYM_PMD));
...
static int test_cryptodev_octeontx_asym(void) {
  gbl_driver_id =
      rte_cryptodev_driver_id_get(RTE_STR(CRYPTODEV_NAME_OCTEONTX_SYM_PMD));
  if (gbl_driver_id == -1) {
...
static int test_cryptodev_cn9k_asym(void) {
  gbl_driver_id = rte_cryptodev_driver_id_get(RTE_STR(CRYPTODEV_NAME_CN9K_PMD));
  if (gbl_driver_id == -1) {
...
static int test_cryptodev_cn10k_asym(void) {
  gbl_driver_id =
      rte_cryptodev_driver_id_get(RTE_STR(CRYPTODEV_NAME_CN10K_PMD));
  if (gbl_driver_id == -1) {
