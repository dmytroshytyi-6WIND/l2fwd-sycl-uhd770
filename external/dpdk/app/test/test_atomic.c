...
static int test_atomic_usual(__rte_unused void *arg) {
  unsigned i;
...
static int test_atomic_tas(__rte_unused void *arg) {
  while (rte_atomic32_read(&synchro) == 0)
    ;
...
static int test_atomic_addsub_and_return(__rte_unused void *arg) {
  uint32_t tmp16;
  uint32_t tmp32;
  uint64_t tmp64;
  unsigned i;
 ...
 * Here checks that if the 32/64 bits counter is equal to 0 after being
 * atomically increased by one. If it is, increase the variable of "count" by
 * one which would be checked as the result later.
 *
 */
static int test_atomic_inc_and_test(__rte_unused void *arg) {
  while (rte_atomic32_read(&synchro) == 0)
    ;
 ...
 */
static int test_atomic_dec_and_test(__rte_unused void *arg) {
  while (rte_atomic32_read(&synchro) == 0)
    ;
 ...
 */
static int test_atomic128_cmp_exchange(__rte_unused void *arg) {
  ...
  for (i = 0; i < N; i++) {
    do {
      ...
      success = rte_atomic128_cmp_exchange(&count128, &expected, &desired, 1,
...
static uint8_t get_crc8(uint8_t *message, int length) {
  uint8_t crc = 0;
  int i;
 ...
 * +------------+------------+
 */
static int test_atomic_exchange(__rte_unused void *arg) {
  int i;
...
static int test_atomic(void) {
  ...
  rte_atomic16_set(&a16, 0);
  rte_atomic64_set(&count, 0);
  rte_eal_mp_remote_launch(test_atomic_addsub_and_return, NULL, SKIP_MAIN);
  rte_atomic32_set(&synchro, 1);
  rte_eal_mp_wait_lcore();
  rte_atomic32_set(&synchro, 0);
  ...
  t.u64 = rte_rand();
  token16 =
      (get_crc8(&t.u8[0], sizeof(token16) - 1) << 8) | (t.u16[0] & 0x00ff);
