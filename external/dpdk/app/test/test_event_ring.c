...
static int test_event_ring(void) {
 ...
 */
static int test_event_ring_creation_with_wrong_size(void) {
  struct rte_event_ring *rp = NULL;

  /* Test if ring size is not power of 2 */
  rp = rte_event_ring_create("test_bad_ring_size", RING_SIZE + 1, SOCKET_ID_ANY,
                             0);
  if (rp != NULL)
    return -1;
 ...
 */
static int test_create_count_odd(void) {
  struct rte_event_ring *r =
      rte_event_ring_create("test_event_ring_count", 4097, SOCKET_ID_ANY, 0);
  if (r != NULL)
    return -1;
  return 0;
}
...
static int test_basic_event_enqueue_dequeue(void) {
  ...
  ret = rte_event_ring_enqueue_burst(sr, evs, RTE_DIM(evs), &free_count);
  if (ret != RTE_DIM(evs) - 1 || free_count != 0) {
    printf("Error, status after enqueue is unexpected\n");
    goto error;
  }
  ...
  ret = rte_event_ring_dequeue_burst(sr, evs, RTE_DIM(evs), &used_count);
  if (ret != RTE_DIM(evs) - 1 || used_count != 0) {
    printf("Error, status after enqueue is unexpected\n");
    goto error;
  }
...
static int test_event_ring_with_exact_size(void) {
  ...
  exact_sz_ring =
      rte_event_ring_create("exact sz", ring_sz, rte_socket_id(),
  ...
  if (rte_event_ring_get_size(std_ring) >=
    ...
    printf("%s: error, std ring (size: %u) is not smaller than exact size one "
           "(size %u)\n",
           __func__, rte_event_ring_get_size(std_ring),
           rte_event_ring_get_size(exact_sz_ring));
    return -1;
  }
  /*
  ...
  if (rte_event_ring_dequeue_burst(exact_sz_ring, ev_array, RTE_DIM(ev_array),
                                   NULL) != ring_sz) {
...
static int test_event_ring(void) {
  if (r == NULL)
    r = rte_event_ring_create("ev_test", RING_SIZE, SOCKET_ID_ANY, 0);
  if (r == NULL)
    return -1;
