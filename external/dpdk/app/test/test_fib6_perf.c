...
static int test_fib6_perf(void) {
...
static void print_route_distribution(const struct rules_tbl_entry *table,
                                     uint32_t n) {
  unsigned int i, j;
...
static int test_fib6_perf(void) {
  ...
  for (i = 0; i < NUM_ROUTE_ENTRIES; i++) {
    ...
    if (rte_fib6_add(fib, large_route_table[i].ip, large_route_table[i].depth,
                     next_hop_add) == 0)
      status++;
  }
  /* End Timer. */
