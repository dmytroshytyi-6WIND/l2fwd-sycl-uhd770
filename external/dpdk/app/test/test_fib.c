...
int32_t test_create_invalid(void) {
  ...
  RTE_TEST_ASSERT(fib == NULL, "Call succeeded with invalid parameters\n");
 ...
 */
int32_t test_add_del_invalid(void) {
  struct rte_fib *fib = NULL;
  struct rte_fib_conf config;
  uint64_t nh = 100;
  ...
  RTE_TEST_ASSERT(ret < 0, "Call succeeded with invalid parameters\n");
 ...
 * for incorrect user input arguments
 */
int32_t test_get_invalid(void) {
  void *p;

  p = rte_fib_get_dp(NULL);
  RTE_TEST_ASSERT(p == NULL, "Call succeeded with invalid parameters\n");
 ...
 */
static int lookup_and_check_asc(struct rte_fib *fib,
                                uint32_t ip_arr[RTE_FIB_MAXDEPTH],
                                uint32_t ip_missing, uint64_t def_nh,
                                uint32_t n) {
  ...
  for (; i <= RTE_FIB_MAXDEPTH - n; i++)
    RTE_TEST_ASSERT(nh_arr[i] == n, "Failed to get proper nexthop\n");
...
static int lookup_and_check_desc(struct rte_fib *fib,
                                 uint32_t ip_arr[RTE_FIB_MAXDEPTH],
                                 uint32_t ip_missing, uint64_t def_nh,
                                 uint32_t n) {
  uint64_t nh_arr[RTE_FIB_MAXDEPTH];
  ...
  for (; i < RTE_FIB_MAXDEPTH; i++)
    RTE_TEST_ASSERT(nh_arr[i] == def_nh, "Failed to get proper nexthop\n");
...
static int check_fib(struct rte_fib *fib) {
  uint64_t def_nh = 100;
  uint32_t ip_arr[RTE_FIB_MAXDEPTH];
  ...
  for (i = 0; i < RTE_FIB_MAXDEPTH; i++) {
    ret = rte_fib_add(fib, ip_add, RTE_FIB_MAXDEPTH - i, RTE_FIB_MAXDEPTH - i);
    RTE_TEST_ASSERT(ret == 0, "Failed to add a route\n");
...
int32_t test_lookup(void) {
  ...
  RTE_TEST_ASSERT(fib != NULL, "Failed to create FIB\n");
  ret = check_fib(fib);
  RTE_TEST_ASSERT(ret == TEST_SUCCESS, "Check_fib fails for DUMMY type\n");
  rte_fib_free(fib);
  ...
  RTE_TEST_ASSERT(fib != NULL, "Failed to create FIB\n");
  ret = check_fib(fib);
  RTE_TEST_ASSERT(ret == TEST_SUCCESS, "Check_fib fails for DIR24_8_1B type\n");
  rte_fib_free(fib);
  ...
  RTE_TEST_ASSERT(fib != NULL, "Failed to create FIB\n");
  ret = check_fib(fib);
  RTE_TEST_ASSERT(ret == TEST_SUCCESS, "Check_fib fails for DIR24_8_2B type\n");
  rte_fib_free(fib);
  ...
  RTE_TEST_ASSERT(fib != NULL, "Failed to create FIB\n");
  ret = check_fib(fib);
  RTE_TEST_ASSERT(ret == TEST_SUCCESS, "Check_fib fails for DIR24_8_4B type\n");
  rte_fib_free(fib);
  ...
  RTE_TEST_ASSERT(fib != NULL, "Failed to create FIB\n");
  ret = check_fib(fib);
  RTE_TEST_ASSERT(ret == TEST_SUCCESS, "Check_fib fails for DIR24_8_8B type\n");
  rte_fib_free(fib);
...
static struct unit_test_suite fib_fast_tests = {
    ...
    .teardown = NULL,
    .unit_test_cases = {
        TEST_CASE(test_create_invalid), TEST_CASE(test_free_null),
 ...
 * Do all unit tests.
 */
static int test_fib(void) { return unit_test_suite_runner(&fib_fast_tests); }
