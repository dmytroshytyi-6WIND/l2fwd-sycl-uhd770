...
static int alloc_rcu(void) {
  ...
  for (i = 0; i < RTE_MAX_LCORE; i++)
    t[i] = (struct rte_rcu_qsbr *)rte_zmalloc(NULL, sz, RTE_CACHE_LINE_SIZE);

  return 0;
}

static int free_rcu(void) {
  int i;
 ...
 */
static int test_rcu_qsbr_get_memsize(void) {
  size_t sz;
  ...
  if (RTE_CACHE_LINE_SIZE == 64)
    TEST_RCU_QSBR_RETURN_IF_ERROR((sz != 8384), "Get Memsize for 128 threads");
  else if (RTE_CACHE_LINE_SIZE == 128)
    TEST_RCU_QSBR_RETURN_IF_ERROR((sz != 16768), "Get Memsize for 128 threads");
 ...
 */
static int test_rcu_qsbr_init(void) {
  int r;
 ...
 * reporting their quiescent state on a QS variable.
 */
static int test_rcu_qsbr_thread_register(void) {
  int ret;
  ...
  TEST_RCU_QSBR_RETURN_IF_ERROR((ret == 0), "NULL variable, invalid thread id");
  ...
  ret = rte_rcu_qsbr_thread_register(t[0], enabled_core_ids[0]);
  TEST_RCU_QSBR_RETURN_IF_ERROR((ret == 1), "Already registered thread id");
 ...
 */
static int test_rcu_qsbr_thread_unregister(void) {
  ...
  TEST_RCU_QSBR_RETURN_IF_ERROR((ret == 1), "disabled thread id");
  /* Unregister already unregistered core */
  ret = rte_rcu_qsbr_thread_unregister(t[0], i);
  TEST_RCU_QSBR_RETURN_IF_ERROR((ret == 1), "Already unregistered core");

  /* Test with enabled lcore */
  ret = rte_rcu_qsbr_thread_unregister(t[0], enabled_core_ids[0]);
  TEST_RCU_QSBR_RETURN_IF_ERROR((ret == 1), "enabled thread id");
  /* Unregister already unregistered core */
  ret = rte_rcu_qsbr_thread_unregister(t[0], enabled_core_ids[0]);
  TEST_RCU_QSBR_RETURN_IF_ERROR((ret == 1), "Already unregistered core");
  ...
  for (j = 0; j < 3; j++) {
    ...
    for (i = 0; i < num_threads[j]; i++)
      rte_rcu_qsbr_thread_register(t[0], (j == 2) ? (RTE_MAX_LCORE - 1) : i);

    token = rte_rcu_qsbr_start(t[0]);
    TEST_RCU_QSBR_RETURN_IF_ERROR((token != (TEST_RCU_QSBR_CNT_INIT + 1)),
                                  "QSBR Start");
    ...
    for (i = 0; i < num_threads[j]; i++) {
      ...
      if ((j == 1) && (i == skip_thread_id))
        continue;
      rte_rcu_qsbr_quiescent(t[0], (j == 2) ? (RTE_MAX_LCORE - 1) : i);
    }
    ...
    for (i = 0; i < num_threads[j]; i++)
      rte_rcu_qsbr_thread_unregister(t[0], (j == 2) ? (RTE_MAX_LCORE - 1) : i);
 ...
 * status.
 */
static int test_rcu_qsbr_start(void) {
  uint64_t token;
  unsigned int i;
...
static int test_rcu_qsbr_check_reader(void *arg) {
  ...
  for (i = 0; i < num_cores; i++) {
    if (i % 2 == 0)
      ...
      rte_rcu_qsbr_thread_unregister(temp, enabled_core_ids[i]);
  }
  return 0;
}
 ...
 */
static int test_rcu_qsbr_check(void) {
  int ret;
  unsigned int i;
  uint64_t token;
  ...
  token = rte_rcu_qsbr_start(t[0]);
  TEST_RCU_QSBR_RETURN_IF_ERROR((token != (TEST_RCU_QSBR_CNT_INIT + 2)),
                                "QSBR Start");
  ...
  rte_eal_remote_launch(test_rcu_qsbr_check_reader, NULL, enabled_core_ids[0]);
...
static int test_rcu_qsbr_synchronize_reader(void *arg) {
 ...
 * the quiescent state.
 */
static int test_rcu_qsbr_synchronize(void) {
  unsigned int i;
  ...
  writer_done = 0;
  for (i = 0; i < num_cores; i++)
    rte_eal_remote_launch(test_rcu_qsbr_synchronize_reader, NULL,
                          enabled_core_ids[i]);
  rte_rcu_qsbr_synchronize(t[0], RTE_QSBR_THRID_INVALID);
  rte_rcu_qsbr_synchronize(t[0], RTE_QSBR_THRID_INVALID);
  rte_rcu_qsbr_synchronize(t[0], RTE_QSBR_THRID_INVALID);
 ...
 */
static int test_rcu_qsbr_thread_online(void) {
  int i, ret;
  uint64_t token;
 ...
 * the list of threads reporting their quiescent state on a QS variable.
 */
static int test_rcu_qsbr_thread_offline(void) {
  int i, ret;
  uint64_t token;
...
static void test_rcu_qsbr_free_resource1(void *p, void *e, unsigned int n) {
...
static void test_rcu_qsbr_free_resource2(void *p, void *e, unsigned int n) {
  if (p != NULL || e == NULL || n != 1) {
    printf("%s: Test failed\n", __func__);
    cb_failed = 1;
  }
 ...
 */
static int test_rcu_qsbr_dq_create(void) {
 ...
 */
static int test_rcu_qsbr_dq_enqueue(void) {
  int ret;
  uint64_t r;
  char rcu_dq_name[RTE_RCU_QSBR_DQ_NAMESIZE];
  struct rte_rcu_qsbr_dq_parameters params;
 ...
 */
static int test_rcu_qsbr_dq_reclaim(void) {
  int ret;
  char rcu_dq_name[RTE_RCU_QSBR_DQ_NAMESIZE];
  struct rte_rcu_qsbr_dq_parameters params;
  struct rte_rcu_qsbr_dq *dq;
 ...
 */
static int test_rcu_qsbr_dq_delete(void) {
  int ret;
  char rcu_dq_name[RTE_RCU_QSBR_DQ_NAMESIZE];
  struct rte_rcu_qsbr_dq_parameters params;
  struct rte_rcu_qsbr_dq *dq;
 ...
 * to be freed later after at least one grace period is over.
 */
static int test_rcu_qsbr_dq_functional(int32_t size, int32_t esize,
                                       uint32_t flags) {
  int i, j, ret;
  char rcu_dq_name[RTE_RCU_QSBR_DQ_NAMESIZE];
  struct rte_rcu_qsbr_dq_parameters params;
  ...
  for (i = 0; i < max_entries; i++) {
    ret = rte_rcu_qsbr_dq_enqueue(dq, e);
    TEST_RCU_QSBR_GOTO_IF_ERROR(
        end, (ret != 0), "dq enqueue functional, max_entries = %d, i = %d",
        max_entries, i);
    for (j = 0; j < esize / 8; j++)
      e[j] = sc++;
  }
  ...
  for (i = 0; i < max_entries; i++) {
    ret = rte_rcu_qsbr_dq_enqueue(dq, e);
    TEST_RCU_QSBR_GOTO_IF_ERROR(end, (ret != 0), "dq enqueue functional");
    for (j = 0; j < esize / 8; j++)
      e[j] = sc++;
  }
 ...
 */
static int test_rcu_qsbr_dump(void) {
  unsigned int i;
...
static int test_rcu_qsbr_reader(void *arg) {
  struct rte_rcu_qsbr *temp;
  ...
  do {
    ...
    for (i = 0; i < TOTAL_ENTRY; i++) {
      ...
      if (rte_hash_lookup_data(hash, keys + i, (void **)&pdata) != -ENOENT) {
        pdata[lcore_id] = 0;
...
static int test_rcu_qsbr_writer(void *arg) {
  uint64_t token;
  int32_t i, pos, del;
  uint32_t c;
  struct rte_rcu_qsbr *temp;
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (c != COUNTER_VALUE && c != 0) {
      printf("Reader lcore id %u did not complete = %u\t", rte_lcore_id(), c);
      return -1;
    }
  }
...
static struct rte_hash *init_hash(int hash_id) {
  int i;
  ...
  for (i = 0; i < TOTAL_ENTRY; i++) {
    if (rte_hash_add_key_data(h, keys + i,
                              (void *)((uintptr_t)hash_data[hash_id][i])) < 0) {
 ...
 */
static int test_rcu_qsbr_sw_sv_3qs(void) {
  uint64_t token[3];
  uint32_t c;
  int i, num_readers;
  int32_t pos[3];
 ...
 */
static int test_rcu_qsbr_mw_mv_mqs(void) {
  unsigned int i, j;
  ...
  for (i = 0; i < test_cores / 4; i++) {
    ...
    /* Reader threads are launched */
    rte_eal_remote_launch(test_rcu_qsbr_reader, (void *)&thread_info[i],
                          enabled_core_ids[j]);
    rte_eal_remote_launch(test_rcu_qsbr_reader, (void *)&thread_info[i],
                          enabled_core_ids[j + 1]);

    /* Writer threads are launched */
    rte_eal_remote_launch(test_rcu_qsbr_writer, (void *)&thread_info[i],
...
static int test_rcu_qsbr_main(void) {
  uint16_t core_id;
