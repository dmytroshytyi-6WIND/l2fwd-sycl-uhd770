...
static int parse_port_range(const char *in, uint16_t *port_low,
                            uint16_t *port_high) {
  uint16_t a, b;
...
static int parse_cb_ipv4_rule(char *str,
                              struct rte_table_acl_rule_add_params *v) {
  int i, rc;
  ...
  */
  ...
  printf("V=%u, mask=%u\n", v->field_value[DSTP_FIELD_IPV4].value.u16,
         v->field_value[DSTP_FIELD_IPV4].mask_range.u16);
  /* parse 0/0xnn */
  GET_CB_FIELD(in[CB_FLD_PROTO], v->field_value[PROTO_FIELD_IPV4].value.u8, 0,
               UINT8_MAX, '/');
...
static int parse_cb_ipv4_rule_del(char *str,
                                  struct rte_table_acl_rule_delete_params *v) {
  int i, rc;
  char *s, *sp, *in[CB_FLD_NUM];
  static const char *dlm = " \t\n";
...
static int setup_acl_pipeline(void) {
  int ret;
  int i;
  ...
  for (i = 0; i < N_PORTS; i++) {
    ...
    ret = rte_pipeline_table_entry_add_bulk(p, table_id[i], (void **)key_array,
                                            table_entries, 5, key_found,
                                            table_entries_ptr);
    if (ret < 0) {
  ...
  for (i = 0; i < N_PORTS; i++) {
    ...
    ret = rte_pipeline_table_entry_delete_bulk(
        p, table_id[i], (void **)key_array, 5, key_found, table_entries);
    if (ret < 0) {
  ...
  for (i = 0; i < N_PORTS; i++) {
    ...
    for (n = 1; n <= 5; n++) {
      ...
      ret = rte_pipeline_table_entry_add(p, table_id[i], &rule_params,
    ...
    for (n = 2; n <= 3; n++) {
      ...
      delete_params = (struct rte_pipeline_table_acl_rule_delete_params *)&(
          rule_params.field_value[0]);
      ret = rte_pipeline_table_entry_delete(p, table_id[i], delete_params,
                                            &key_found, NULL);
      if (ret < 0) {
        rte_panic("Add entry to table %u failed (%d)\n", table_id[i], ret);
        goto fail;
...
static int test_pipeline_single_filter(int expected_count) {
  ...
  for (i = 0; i < N_PORTS; i++) {
    for (j = 0; j < 8; j++) {
      ...
      if (mbuf == NULL)
         ...
         * counts won't match */
        break;
      memset(rte_pktmbuf_mtod(mbuf, char *), 0x00, sizeof(struct ipv4_5tuple));
  ...
  for (i = 0; i < N_PORTS; i++) {
    ...
    if (ret <= 0) {
      ...
      for (j = 0; j < ret; j++) {
        mbuf = objs[j];
        rte_hexdump(stdout, "mbuf", rte_pktmbuf_mtod(mbuf, char *), 64);
