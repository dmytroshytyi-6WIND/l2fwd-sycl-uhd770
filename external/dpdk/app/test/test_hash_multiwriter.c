...
static int test_hash_multiwriter_worker(void *arg) {
  uint64_t i, offset;
  uint16_t pos_core;
  ...
  printf("Core #%d inserting %d: %'" PRId64 " - %'" PRId64 "\n", lcore_id,
         tbl_multiwriter_test_params.nb_tsx_insertion, offset,
         offset + tbl_multiwriter_test_params.nb_tsx_insertion - 1);

  begin = rte_rdtsc_precise();

  for (i = offset; i < offset + tbl_multiwriter_test_params.nb_tsx_insertion;
       i++) {
  ...
  for (; i < offset + tbl_multiwriter_test_params.nb_tsx_insertion; i++)
    tbl_multiwriter_test_params.keys[i] = RTE_APP_TEST_HASH_MULTIWRITER_FAILED;

  return 0;
}

static int test_hash_multiwriter(void) {
  ...
  /* Fire all threads. */
  rte_eal_mp_remote_launch(test_hash_multiwriter_worker, enabled_core_ids,
                           CALL_MAIN);
...
static int test_hash_multiwriter_main(void) {
