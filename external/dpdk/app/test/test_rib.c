...
static int test_rib(void) {
  printf("rib not supported on Windows, skipping test\n");
  return TEST_SKIPPED;
}

static int test_slow_rib(void) {
...
int32_t test_basic(void) {
  ...
  RTE_TEST_ASSERT(ret == 0, "Failed to set rte_rib_node field\n");
 ...
 * Do all unit tests.
 */
static int test_rib(void) { return unit_test_suite_runner(&rib_tests); }
