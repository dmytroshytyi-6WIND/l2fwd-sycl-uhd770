 ...
 */
#define TEST_RING_VERIFY(exp, r, errst)                                        \
  do {                                                                         \
    if (!(exp)) {                                                              \
 ...
 */
static unsigned int test_ring_enqueue_zc_bulk(struct rte_ring *r,
...
static unsigned int test_ring_enqueue_zc_bulk_elem(struct rte_ring *r,
  ...
  ret = rte_ring_enqueue_zc_bulk_elem_start(r, esize, n, &zcd, free_space);
  if (ret != 0) {
    /* Copy the data to the ring */
...
static unsigned int test_ring_enqueue_zc_burst(struct rte_ring *r,
                                               void *const *obj_table,
                                               unsigned int n,
                                               unsigned int *free_space) {
  unsigned int ret;
...
static unsigned int test_ring_enqueue_zc_burst_elem(struct rte_ring *r,
                                                    const void *obj_table,
                                                    unsigned int esize,
                                                    unsigned int n,
                                                    unsigned int *free_space) {
  ...
  ret = rte_ring_enqueue_zc_burst_elem_start(r, esize, n, &zcd, free_space);
  if (ret != 0) {
    /* Copy the data to the ring */
    test_ring_copy_to(&zcd, obj_table, esize, ret);
    rte_ring_enqueue_zc_finish(r, ret);
...
static unsigned int test_ring_dequeue_zc_bulk(struct rte_ring *r,
...
static unsigned int test_ring_dequeue_zc_bulk_elem(struct rte_ring *r,
                                                   void *obj_table,
                                                   unsigned int esize,
                                                   unsigned int n,
                                                   unsigned int *available) {
  ...
  ret = rte_ring_dequeue_zc_bulk_elem_start(r, esize, n, &zcd, available);
  if (ret != 0) {
    /* Copy the data from the ring */
...
static unsigned int test_ring_dequeue_zc_burst(struct rte_ring *r,
                                               void **obj_table, unsigned int n,
                                               unsigned int *available) {
  unsigned int ret;
  struct rte_ring_zc_data zcd;
...
static unsigned int test_ring_dequeue_zc_burst_elem(struct rte_ring *r,
                                                    void *obj_table,
                                                    unsigned int esize,
                                                    unsigned int n,
                                                    unsigned int *available) {
  ...
  ret = rte_ring_dequeue_zc_burst_elem_start(r, esize, n, &zcd, available);
  if (ret != 0) {
    /* Copy the data from the ring */
    test_ring_copy_from(&zcd, obj_table, esize, ret);
    rte_ring_dequeue_zc_finish(r, ret);
...
static const struct {
  ...
  struct {
    unsigned int (*flegacy)(struct rte_ring *r, void *const *obj_table,
  ...
  struct {
    unsigned int (*flegacy)(struct rte_ring *r, void **obj_table,
...
static unsigned int test_ring_enq_impl(struct rte_ring *r, void **obj,
                                       int esize, unsigned int n,
                                       unsigned int test_idx) {
  if (esize == -1)
    return test_enqdeq_impl[test_idx].enq.flegacy(r, obj, n, NULL);
  else
    return test_enqdeq_impl[test_idx].enq.felem(r, obj, esize, n, NULL);
}

static unsigned int test_ring_deq_impl(struct rte_ring *r, void **obj,
                                       int esize, unsigned int n,
                                       unsigned int test_idx) {
  if (esize == -1)
    return test_enqdeq_impl[test_idx].deq.flegacy(r, obj, n, NULL);
  else
    return test_enqdeq_impl[test_idx].deq.felem(r, obj, esize, n, NULL);
}

static void test_ring_mem_init(void *obj, unsigned int count, int esize) {
  unsigned int i;
...
static int test_ring_mem_cmp(void *src, void *dst, unsigned int size) {
  int ret;
...
static void test_ring_print_test_string(const char *istr, unsigned int api_type,
                                        int esize) {
  printf("\n%s: ", istr);
 ...
 */
static int test_ring_negative_tests(void) {
  struct rte_ring *rp = NULL;
  struct rte_ring *rt = NULL;
  unsigned int i;

  /* Test with esize not a multiple of 4 */
  rp = test_ring_create("test_bad_element_size", 23, RING_SIZE + 1,
                        SOCKET_ID_ANY, 0);
  if (rp != NULL) {
  ...
  for (i = 0; i < RTE_DIM(esize); i++) {
    ...
    rp = test_ring_create("test_bad_ring_size", esize[i], RING_SIZE + 1,
                          SOCKET_ID_ANY, 0);
    if (rp != NULL) {
    ...
    rp = test_ring_create("test_bad_ring_size", esize[i], RTE_RING_SZ_MASK + 1,
                          SOCKET_ID_ANY, 0);
    if (rp != NULL) {
     ...
     */
    rp = test_ring_create("test_ring_count", esize[i], 4097, SOCKET_ID_ANY, 0);
    if (rp != NULL)
      goto test_fail;
 ...
 */
static int test_ring_burst_bulk_tests1(unsigned int test_idx) {
  struct rte_ring *r;
  ...
  for (i = 0; i < RTE_DIM(esize); i++) {
    ...
    /* Create the ring */
    r = test_ring_create("test_ring_burst_bulk_tests", esize[i], RING_SIZE,
                         SOCKET_ID_ANY,
    ...
    for (j = 0; j != TEST_RING_FULL_EMPTY_ITER; j++) {
      ...
      ret = test_ring_enq_impl(r, cur_src, esize[i], rand, test_idx);
      TEST_RING_VERIFY(ret != 0, r, goto fail);

      ret = test_ring_deq_impl(r, cur_dst, esize[i], rand, test_idx);
      TEST_RING_VERIFY(ret == rand, r, goto fail);

      /* fill the ring */
      ret = test_ring_enq_impl(r, cur_src, esize[i], rsz, test_idx);
      TEST_RING_VERIFY(ret != 0, r, goto fail);
      ...
      /* empty the ring */
      ret = test_ring_deq_impl(r, cur_dst, esize[i], rsz, test_idx);
      ...
      if (esize[i] != -1)
        temp_sz = rsz * esize[i];
      TEST_RING_VERIFY(test_ring_mem_cmp(src, dst, temp_sz) == 0, r, goto fail);
    }
 ...
 * dequeued data.
 */
static int test_ring_burst_bulk_tests2(unsigned int test_idx) {
  struct rte_ring *r;
  void **src = NULL, **cur_src = NULL, **dst = NULL, **cur_dst = NULL;
  int ret;
  unsigned int i;
  ...
  for (i = 0; i < RTE_DIM(esize); i++) {
    ...
    ret = test_ring_enq_impl(r, cur_src, esize[i], MAX_BULK, test_idx);
    ...
    ret = test_ring_deq_impl(r, cur_dst, esize[i], MAX_BULK, test_idx);
    TEST_RING_VERIFY(ret == MAX_BULK, r, goto fail);
    cur_dst = test_ring_inc_ptr(cur_dst, esize[i], MAX_BULK);

    /* check data */
    TEST_RING_VERIFY(test_ring_mem_cmp(src, dst, RTE_PTR_DIFF(cur_dst, dst)) ==
                         0,
                     r, goto fail);
 ...
 */
static int test_ring_burst_bulk_tests3(unsigned int test_idx) {
  struct rte_ring *r;
  void **src = NULL, **cur_src = NULL, **dst = NULL, **cur_dst = NULL;
  int ret;
  unsigned int i, j;
 ...
 */
static int test_ring_burst_bulk_tests4(unsigned int test_idx) {
  struct rte_ring *r;
  void **src = NULL, **cur_src = NULL, **dst = NULL, **cur_dst = NULL;
  int ret;
  unsigned int i, j;
  ...
  for (i = 0; i < RTE_DIM(esize); i++) {
    ...
    ret = test_ring_enq_impl(r, cur_src, esize[i], num_elems, test_idx);
    ...
    if ((api_type & TEST_RING_ELEM_BULK) == TEST_RING_ELEM_BULK)
      ...
      num_elems = MAX_BULK;
    ret = test_ring_deq_impl(r, cur_dst, esize[i], num_elems, test_idx);
    TEST_RING_VERIFY(ret == MAX_BULK - 3, r, goto fail);
 ...
 */
static int test_ring_basic_ex(void) {
  int ret = -1;
  unsigned int i, j;
  struct rte_ring *rp = NULL;
  void **src = NULL, **cur_src = NULL, **dst = NULL, **cur_dst = NULL;
 ...
 */
static int test_ring_with_exact_size(void) {
  ...
  for (i = 0; i < RTE_DIM(esize); i++) {
    test_ring_print_test_string("Test exact size ring",
                                TEST_RING_IGNORE_API_TYPE, esize[i]);

    std_r = test_ring_create("std", esize[i], ring_sz, rte_socket_id(),
     ...
     * standard ring
     */
    TEST_RING_VERIFY(rte_ring_get_size(std_r) <= rte_ring_get_size(exact_sz_r),
    ...
    TEST_RING_VERIFY(rte_ring_get_capacity(exact_sz_r) == ring_sz, exact_sz_r,
                     goto test_fail);
