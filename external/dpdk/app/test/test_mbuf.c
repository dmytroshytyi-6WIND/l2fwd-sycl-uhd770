 ...
 */
static int test_pktmbuf_with_non_ascii_data(struct rte_mempool *pktmbuf_pool) {
 ...
 */
static int test_one_pktmbuf(struct rte_mempool *pktmbuf_pool) {
  struct rte_mbuf *m = NULL;
  char *data, *data2, *hdr;
  unsigned i;
...
static uint16_t testclone_refcnt_read(struct rte_mbuf *m) {
  return RTE_MBUF_HAS_PINNED_EXTBUF(m) ? rte_mbuf_ext_refcnt_read(m->shinfo)
                                       : rte_mbuf_refcnt_read(m);
}

static int testclone_testupdate_testdetach(struct rte_mempool *pktmbuf_pool,
...
static int test_pktmbuf_copy(struct rte_mempool *pktmbuf_pool,
                             struct rte_mempool *clone_pool) {
  struct rte_mbuf *m = NULL;
  ...
  *data = MAGIC_DATA;
  ...
  if ((!RTE_MBUF_HAS_PINNED_EXTBUF(m) && !RTE_MBUF_CLONED(clone)) ||
      (RTE_MBUF_HAS_PINNED_EXTBUF(m) && !RTE_MBUF_HAS_EXTBUF(clone)))
  ...
  *data = MAGIC_DATA + 1;
  ...
  /* test offset copy */
  copy2 = rte_pktmbuf_copy(copy, pktmbuf_pool, sizeof(uint32_t), UINT32_MAX);
  if (copy2 == NULL)
  ...
  copy2 = rte_pktmbuf_copy(copy, pktmbuf_pool, 0, sizeof(uint32_t));
  if (copy2 == NULL)
    GOTO_FAIL("cannot copy the copy\n");
...
static int test_attach_from_different_pool(struct rte_mempool *pktmbuf_pool,
 ...
 */
static int test_pktmbuf_pool(struct rte_mempool *pktmbuf_pool) {
  unsigned i;
 ...
 */
static int test_pktmbuf_pool_bulk(void) {
  ...
  pool = rte_pktmbuf_pool_create("test_pktmbuf_bulk", NB_MBUF, 0, 0,
                                 MBUF_DATA_SIZE, SOCKET_ID_ANY);
  if (pool == NULL) {
    printf("rte_pktmbuf_pool_create() failed. rte_errno %d\n", rte_errno);
    goto err;
  }
  pool2 = rte_pktmbuf_pool_create("test_pktmbuf_bulk2", NB_MBUF, 0, 0,
                                  MBUF_DATA_SIZE, SOCKET_ID_ANY);
  if (pool2 == NULL) {
    printf("rte_pktmbuf_pool_create() failed. rte_errno %d\n", rte_errno);
    goto err;
  ...
  if (!(rte_mempool_avail_count(pool) == NB_MBUF &&
    ...
    printf("Test precondition failed: mempools: %u+%u != %u+%u",
           rte_mempool_avail_count(pool), rte_mempool_avail_count(pool2),
           NB_MBUF, NB_MBUF);
    goto err;
  }
 ...
 */
static int test_pktmbuf_pool_ptr(struct rte_mempool *pktmbuf_pool) {
  unsigned i;
  struct rte_mbuf *m[NB_MBUF];
  int ret = 0;
...
static int test_pktmbuf_free_segment(struct rte_mempool *pktmbuf_pool) {
  unsigned i;
  struct rte_mbuf *m[NB_MBUF];
  int ret = 0;
...
static int test_refcnt_worker(void *arg) {
  unsigned lcore, free;
  void *mp = 0;
...
static void test_refcnt_iter(unsigned int lcore, unsigned int iter,
  ...
  for (i = 0, n = rte_mempool_avail_count(refcnt_pool);
       i != n && (m = rte_pktmbuf_alloc(refcnt_pool)) != NULL; i++) {
...
static int test_refcnt_main(struct rte_mempool *refcnt_pool,
                            struct rte_ring *refcnt_mbuf_ring) {
  unsigned i, lcore;
...
static int test_refcnt_mbuf(void) {
#ifdef RTE_MBUF_REFCNT_ATOMIC
  ...
  refcnt_pool = rte_pktmbuf_pool_create(
      MAKE_STRING(refcnt_pool), REFCNT_MBUF_NUM, 0, 0, 0, SOCKET_ID_ANY);
  if (refcnt_pool == NULL) {
  ...
  refcnt_mbuf_ring =
      rte_ring_create("refcnt_mbuf_ring", rte_align32pow2(REFCNT_RING_SIZE),
...
static int test_failing_mbuf_sanity_check(struct rte_mempool *pktmbuf_pool) {
...
static int verify_mbuf_check_panics(struct rte_mbuf *buf) {
  int pid;
  int status;
...
static int test_mbuf_linearize(struct rte_mempool *pktmbuf_pool, int pkt_len,
                               int nb_segs) {
  ...
  for (seg = 0; remain > 0; seg++) {
    ...
    memset(rte_pktmbuf_mtod(m, uint8_t *), 0, rte_pktmbuf_tailroom(m));
...
static int test_mbuf_linearize_check(struct rte_mempool *pktmbuf_pool) {
  struct test_mbuf_array {
    int size;
    int nb_segs;
  } mbuf_array[] = {
      {128, 1}, {64, 64}, {512, 10}, {250, 11}, {123, 8},
  };
  unsigned int i;
 ...
 */
static inline void set_tx_offload(struct rte_mbuf *mb, uint64_t il2,
                                  uint64_t il3, uint64_t il4, uint64_t tso,
...
static int test_tx_offload(void) {
  struct rte_mbuf *mb;
  uint64_t tm, v1, v2;
  size_t sz;
  uint32_t i;
  ...
  printf("%s started, tx_offload = {\n"
         ...
         "\touter_l2_len=%#x,\n"
         "};\n",
         __func__, txof.l2, txof.l3, txof.l4, txof.tso, txof.l3, txof.l2);
  ...
  for (i = 0; i != num; i++)
    set_tx_offload(mb + i, txof.l2, txof.l3, txof.l4, txof.tso, txof.l3,
                   txof.l2);

  tm = rte_rdtsc_precise() - tm;
  printf("%s set tx_offload by bit-fields: %u iterations, %" PRIu64
         " cycles, %#Lf cycles/iter\n",
  ...
  for (i = 0; i != num; i++)
    mb[i].tx_offload = rte_mbuf_tx_offload(txof.l2, txof.l3, txof.l4, txof.tso,
                                           txof.l3, txof.l2, 0);

  tm = rte_rdtsc_precise() - tm;
  printf("%s set raw tx_offload: %u iterations, %" PRIu64
         " cycles, %#Lf cycles/iter\n",
         __func__, num, tm, (long double)tm / num);
...
static int test_get_rx_ol_flag_list(void) {
  int len = 6, ret = 0;
  char buf[256] = "";
  int buflen = 0;
  ...
  ret = rte_get_rx_ol_flag_list(RTE_MBUF_F_RX_SEC_OFFLOAD, buf, sizeof(buf));
  if (ret != 0)
    GOTO_FAIL("%s expected: 0, received = %d\n", __func__, ret);
...
static int test_get_tx_ol_flag_list(void) {
  int len = 6, ret = 0;
  char buf[256] = "";
  int buflen = 0;
  ...
  /* Test case to check with valid mask value */
  ret = rte_get_tx_ol_flag_list(RTE_MBUF_F_TX_UDP_CKSUM, buf, sizeof(buf));
  if (ret != 0)
    GOTO_FAIL("%s expected: 0, received = %d\n", __func__, ret);
...
static int test_get_rx_ol_flag_name(void) {
  uint16_t i;
...
static int test_get_tx_ol_flag_name(void) {
  uint16_t i;
  const char *flag_str = NULL;
...
static int test_mbuf_validate_tx_offload(const char *test_name,
                                         struct rte_mempool *pktmbuf_pool,
                                         uint64_t ol_flags, uint16_t segsize,
                                         int expected_retval) {
  struct rte_mbuf *m = NULL;
  int ret = 0;
...
static int test_mbuf_validate_tx_offload_one(struct rte_mempool *pktmbuf_pool) {
  ...
  if (test_mbuf_validate_tx_offload("MBUF_TEST_IP_CKSUM_IPV6_SET", pktmbuf_pool,
                                    ol_flags, 0, -EINVAL) < 0)
  ...
  if (test_mbuf_validate_tx_offload("MBUF_TEST_IP_TYPE_NOT_SET", pktmbuf_pool,
                                    ol_flags, 0, -EINVAL) < 0)
  ...
  ol_flags = RTE_MBUF_F_TX_L4_MASK;
  ol_flags |= RTE_MBUF_F_TX_IPV6;
  if (test_mbuf_validate_tx_offload("MBUF_TEST_IP_TYPE_SET", pktmbuf_pool,
                                    ol_flags, 0, 0) < 0)
  ...
  if (test_mbuf_validate_tx_offload("MBUF_TEST_NULL_TSO_SEGSZ", pktmbuf_pool,
                                    ol_flags, 0, -EINVAL) < 0)
  ...
  if (test_mbuf_validate_tx_offload("MBUF_TEST_TSO_IP_CKSUM_NOT_SET",
                                    pktmbuf_pool, ol_flags, 512, -EINVAL) < 0)
  ...
  ol_flags |= RTE_MBUF_F_TX_IP_CKSUM;
  if (test_mbuf_validate_tx_offload("MBUF_TEST_TSO_IP_CKSUM_SET", pktmbuf_pool,
                                    ol_flags, 512, 0) < 0)
    GOTO_FAIL("%s failed: tx offload flag error.\n", __func__);
  ...
  ol_flags |= RTE_MBUF_F_TX_IPV6;
  ol_flags |= RTE_MBUF_F_TX_TCP_SEG;
  if (test_mbuf_validate_tx_offload("MBUF_TEST_TSO_IPV6_SET", pktmbuf_pool,
                                    ol_flags, 512, 0) < 0)
  ...
  if (test_mbuf_validate_tx_offload("MBUF_TEST_OUTER_IPV4_SET", pktmbuf_pool,
                                    ol_flags, 512, 0) < 0)
    GOTO_FAIL("%s failed: tx offload flag error.\n", __func__);

  ol_flags = 0;
  /* test to confirm if packets with no TX_OFFLOAD_MASK are skipped */
  if (test_mbuf_validate_tx_offload("MBUF_TEST_OL_MASK_NOT_SET", pktmbuf_pool,
                                    ol_flags, 512, 0) < 0)
    GOTO_FAIL("%s failed: tx offload flag error.\n", __func__);
  return 0;
fail:
 ...
 */
static int test_pktmbuf_alloc_bulk(struct rte_mempool *pktmbuf_pool) {
  int ret = 0;
  ...
  for (idx = 0; idx < RTE_DIM(alloc_counts); idx++) {
    ret = rte_pktmbuf_alloc_bulk(pktmbuf_pool, mbufs, alloc_counts[idx]);
    if (ret == 0) {
      for (loop = 0; loop < alloc_counts[idx] && mbufs[loop] != NULL; loop++)
        rte_pktmbuf_free(mbufs[loop]);
    } else if (ret != 0) {
      printf("%s: Bulk alloc failed count(%u); ret val(%d)\n", __func__,
             alloc_counts[idx], ret);
      return -1;
    }
  }
  return 0;
 ...
 */
static int test_neg_pktmbuf_alloc_bulk(struct rte_mempool *pktmbuf_pool) {
  int ret = 0;
  unsigned int idx, loop;
  unsigned int neg_alloc_counts[] = {MEMPOOL_CACHE_SIZE - NB_MBUF, NB_MBUF + 1,
                                     NB_MBUF * 8, UINT_MAX};
  struct rte_mbuf *mbufs[NB_MBUF * 8] = {0};

  for (idx = 0; idx < RTE_DIM(neg_alloc_counts); idx++) {
    ret = rte_pktmbuf_alloc_bulk(pktmbuf_pool, mbufs, neg_alloc_counts[idx]);
    if (ret == 0) {
      printf("%s: Bulk alloc must fail! count(%u); ret(%d)\n", __func__,
             neg_alloc_counts[idx], ret);
      for (loop = 0; loop < neg_alloc_counts[idx] && mbufs[loop] != NULL;
           loop++)
        rte_pktmbuf_free(mbufs[loop]);
      return -1;
    }
 ...
 */
static int test_pktmbuf_read(struct rte_mempool *pktmbuf_pool) {
  struct rte_mbuf *m = NULL;
  char *data = NULL;
 ...
 */
static int test_pktmbuf_read_from_offset(struct rte_mempool *pktmbuf_pool) {
  struct rte_mbuf *m = NULL;
  ...
  data_copy = rte_pktmbuf_read(m, hdr_len + 5, MBUF_TEST_DATA_LEN2 - 5, NULL);
  if (data_copy == NULL)
  ...
  if (rte_pktmbuf_read(m, hdr_len, rte_pktmbuf_data_len(m) + 1, NULL) != NULL)
    GOTO_FAIL("%s: Requested len is larger than mbuf data len!\n", __func__);

  /* read length greater than mbuf pkt_len */
  if (rte_pktmbuf_read(m, hdr_len, rte_pktmbuf_pkt_len(m) + 1, NULL) != NULL)
 ...
 */
static struct rte_mbuf *create_packet(struct rte_mempool *pktmbuf_pool,
...
static int test_pktmbuf_read_from_chain(struct rte_mempool *pktmbuf_pool) {
  struct rte_mbuf *m;
  ...
  for (i = 0; i < RTE_DIM(test_cases); i++) {
    ...
    for (pos = 0; pos < test_cases[i].read_len; pos++) {
      if (data_copy[pos] != (char)((test_cases[i].read_off + pos) % 0xffff))
        GOTO_FAIL("Data corrupted at offset %u is %2X", pos, data_copy[pos]);
    }
    rte_pktmbuf_dump(stdout, m, rte_pktmbuf_pkt_len(m));
    rte_pktmbuf_free(m);
    m = NULL;
...
static void ext_buf_free_callback_fn(void *addr, void *opaque) {
  bool *freed = opaque;
...
test_pktmbuf_ext_shinfo_init_helper(struct rte_mempool *pktmbuf_pool) {
  ...
  uint16_t buf_len =
      EXT_BUF_TEST_DATA_LEN + sizeof(struct rte_mbuf_ext_shared_info);
  bool freed = false;
  ...
  ext_buf_addr = rte_malloc("External buffer", buf_len, RTE_CACHE_LINE_SIZE);
  if (ext_buf_addr == NULL)
    GOTO_FAIL("%s: External buffer allocation failed\n", __func__);

  ret_shinfo = rte_pktmbuf_ext_shinfo_init_helper(
      ext_buf_addr, &buf_len, ext_buf_free_callback_fn, &freed);
  if (ret_shinfo == NULL)
  ...
  rte_pktmbuf_attach_extbuf(m, ext_buf_addr, buf_iova, buf_len, ret_shinfo);
  ...
  rte_pktmbuf_attach_extbuf(clone, ext_buf_addr, buf_iova, buf_len, ret_shinfo);
...
static int test_pktmbuf_ext_pinned_buffer(struct rte_mempool *std_pool) {
  ...
  mz = rte_memzone_reserve("pinned_pool", NB_MBUF * MBUF_DATA_SIZE,
                           SOCKET_ID_ANY,
  ...
  pinned_pool = rte_pktmbuf_pool_create_extbuf(
      "test_pinned_pool", NB_MBUF, MEMPOOL_CACHE_SIZE, 0, MBUF_DATA_SIZE,
...
static int test_mbuf_dyn(struct rte_mempool *pktmbuf_pool) {
  ...
  if (offset == -1)
    GOTO_FAIL("failed to register dynamic field, offset=%d: %s", offset,
              strerror(errno));

  ret = rte_mbuf_dynfield_register(&dynfield);
  if (ret != offset)
    GOTO_FAIL("failed to lookup dynamic field, ret=%d: %s", ret,
              strerror(errno));

  offset2 = rte_mbuf_dynfield_register(&dynfield2);
  if (offset2 == -1 || offset2 == offset || (offset2 & 1))
    GOTO_FAIL("failed to register dynamic field 2, offset2=%d: %s", offset2,
              strerror(errno));

  offset3 = rte_mbuf_dynfield_register_offset(
      &dynfield3, offsetof(struct rte_mbuf, dynfield1[1]));
  ...
  if (flag == -1)
    GOTO_FAIL("failed to register dynamic flag, flag=%d: %s", flag,
              strerror(errno));

  ret = rte_mbuf_dynflag_register(&dynflag);
  if (ret != flag)
    GOTO_FAIL("failed to lookup dynamic flag, ret=%d: %s", ret,
              strerror(errno));

  flag2 = rte_mbuf_dynflag_register(&dynflag2);
  if (flag2 == -1 || flag2 == flag)
    GOTO_FAIL("failed to register dynamic flag 2, flag2=%d: %s", flag2,
              strerror(errno));

  flag3 = rte_mbuf_dynflag_register_bitnum(&dynflag3,
                                           rte_bsf64(RTE_MBUF_F_LAST_FREE));
  if (flag3 != rte_bsf64(RTE_MBUF_F_LAST_FREE))
    GOTO_FAIL("failed to register dynamic flag 3, flag3=%d: %s", flag3,
              strerror(errno));
...
static int test_nb_segs_and_next_reset(void) {
  struct rte_mbuf *m0 = NULL, *m1 = NULL, *m2 = NULL;
  struct rte_mempool *pool = NULL;

  pool = rte_pktmbuf_pool_create("test_mbuf_reset", 3, 0, 0, MBUF_DATA_SIZE,
                                 SOCKET_ID_ANY);
  if (pool == NULL)
  ...
  if (m0->nb_segs != 1 || m0->next != NULL || m1->nb_segs != 1 ||
      m1->next != NULL || m2->nb_segs != 1 || m2->next != NULL)
...
static int test_mbuf(void) {
  ...
  pktmbuf_pool =
      rte_pktmbuf_pool_create("test_pktmbuf_pool", NB_MBUF, MEMPOOL_CACHE_SIZE,
                              0, MBUF_DATA_SIZE, SOCKET_ID_ANY);
   ...
   * room size */
  pktmbuf_pool2 =
      rte_pktmbuf_pool_create("test_pktmbuf_pool2", NB_MBUF, MEMPOOL_CACHE_SIZE,
                              MBUF2_PRIV_SIZE, 0, SOCKET_ID_ANY);
