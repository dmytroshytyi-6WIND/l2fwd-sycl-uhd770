...
static int test_basic_array(void) {
  ...
  used =
      rte_tel_json_add_array_string(buf, sizeof(buf), used, "meaning of life");
...
static int test_basic_obj(void) {
  const char *expected = "{\"weddings\":4,\"funerals\":1}";
  char buf[1024];
  int used = 0;

  used = rte_tel_json_add_obj_u64(buf, sizeof(buf), used, "weddings", 4);
  used = rte_tel_json_add_obj_u64(buf, sizeof(buf), used, "funerals", 1);
...
static int test_overflow_array(void) {
  ...
  for (i = 0; i < (int)RTE_DIM(strs); i++)
    used = rte_tel_json_add_array_string(buf, sizeof(buf), used, strs[i]);
...
static int test_overflow_obj(void) {
  ...
  for (i = 0; i < (int)RTE_DIM(names); i++)
    used = rte_tel_json_add_obj_u64(buf, sizeof(buf), used, names[i], vals[i]);
...
static int test_large_array_element(void) {
...
static int test_large_obj_element(void) {
  static const char str[] = "A really long string to overflow buffer";
  /* buffer should be unmodified so initial value and expected are same */
...
static int test_telemetry_json(void) {
  if (test_basic_array() < 0 || test_basic_obj() < 0 ||
      test_overflow_array() < 0 || test_overflow_obj() < 0 ||
      test_large_array_element() < 0 || test_large_obj_element() < 0)
    return -1;
  return 0;
}
