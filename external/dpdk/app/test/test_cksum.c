...
/* test l3/l4 checksum api */
static int test_l4_cksum(struct rte_mempool *pktmbuf_pool, const char *pktdata,
                         size_t len) {
  ...
  l4_hdr =
      rte_pktmbuf_mtod_offset(m, void *, hdr_lens.l2_len + hdr_lens.l3_len);
...
static int test_cksum(void) {
  struct rte_mempool *pktmbuf_pool = NULL;

  /* create pktmbuf pool if it does not exist */
  pktmbuf_pool = rte_pktmbuf_pool_create("test_cksum_mbuf_pool", NB_MBUF,
