...
static double tsc_to_us(uint64_t tsc, size_t runs) {
  return (double)tsc / rte_get_tsc_hz() * US_PER_S / runs;
}

static int test_memset_perf(double *us_per_gb) {
...
static int test_alloc_perf(const char *name, alloc_t *alloc_fn, free_t *free_fn,
                           memset_t *memset_fn, double memset_gb_us,
                           size_t max_runs) {
  static const size_t SIZES[] = {1 << 6,  1 << 7,  1 << 10, 1 << 12, 1 << 16,
  ...
  for (i = 0; i < RTE_DIM(SIZES); i++) {
    ...
    memset_time = memset_fn != NULL ? tsc_to_us(tsc_memset, runs_done)
                                    : memset_gb_us * size / GB;
    TEST_LOG(INFO, "%12zu%8zu%12.2f%12.2f%12.2f%17.2f\n", size, runs_done,
             alloc_time, free_time, alloc_time + free_time, memset_time);
...
static void *memzone_alloc(const char *name __rte_unused, size_t size,
                           unsigned int align) {
...
static void memzone_free(void *addr) {
  rte_memzone_free((struct rte_memzone *)addr);
}

static int test_malloc_perf(void) {
  ...
  if (test_alloc_perf("rte_malloc", rte_malloc, rte_free, memset, memset_us_gb,
                      MAX_RUNS) < 0)
    return -1;
  ...
  if (test_alloc_perf("rte_memzone_reserve", memzone_alloc, memzone_free, NULL,
                      memset_us_gb, RTE_MAX_MEMZONE - 1) < 0)
    return -1;
