...
static __rte_always_inline int test_loop(struct rte_mempool *mp,
                                         struct rte_mempool_cache *cache,
                                         unsigned int x_keep,
                                         unsigned int x_get_bulk,
                                         unsigned int x_put_bulk) {
  ...
  for (i = 0; likely(i < (N / x_keep)); i++) {
    ...
    for (idx = 0; idx < x_keep; idx += x_get_bulk) {
      ret = rte_mempool_generic_get(mp, &obj_table[idx], x_get_bulk, cache);
      if (unlikely(ret < 0)) {
    ...
    for (idx = 0; idx < x_keep; idx += x_put_bulk) {
      rte_mempool_generic_put(mp, &obj_table[idx], x_put_bulk, cache);
    }
  }
...
static int per_lcore_mempool_test(void *arg) {
  ...
  if (use_external_cache) {
    ...
    cache = rte_mempool_cache_create(external_cache_size, SOCKET_ID_ANY);
    if (cache == NULL)
      RET_ERR();
  } else {
  ...
  while (time_diff / hz < TIME_S) {
    if (!use_constant_values)
      ...
      ret = test_loop(mp, cache, n_keep, CACHE_LINE_BURST, CACHE_LINE_BURST);
...
static int launch_cores(struct rte_mempool *mp, unsigned int cores) {
  unsigned lcore_id;
  uint64_t rate;
  int ret;
  ...
  printf("mempool_autotest cache=%u cores=%u n_get_bulk=%u "
         ...
         use_external_cache ? external_cache_size : (unsigned)mp->cache_size,
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    if (cores == 1)
      break;
    cores--;
    rte_eal_remote_launch(per_lcore_mempool_test, mp, lcore_id);
  }
...
static int do_one_mempool_test(struct rte_mempool *mp, unsigned int cores) {
...
static int test_mempool_perf(void) {
  ...
  mp_nocache =
      rte_mempool_create("perf_test_nocache", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE, 0,
  ...
  mp_cache =
      rte_mempool_create("perf_test_cache", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE,
  ...
  default_pool = rte_mempool_create_empty(
      "default_pool", MEMPOOL_SIZE, MEMPOOL_ELT_SIZE, 0, 0, SOCKET_ID_ANY, 0);
