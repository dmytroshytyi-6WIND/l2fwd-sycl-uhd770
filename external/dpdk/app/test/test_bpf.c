 ...
 */
static int cmp_res(const char *func, uint64_t exp_rc, uint64_t ret_rc,
...
static void test_store1_prepare(void *arg) {
...
static int test_store1_check(uint64_t rc, const void *arg) {
...
static void test_load1_prepare(void *arg) {
  struct dummy_offset *df;
...
static int test_load1_check(uint64_t rc, const void *arg) {
  uint64_t v;
  const struct dummy_offset *dft;
...
static int test_ldimm1_check(uint64_t rc, const void *arg) {
  uint64_t v1, v2;
...
static void test_mul1_prepare(void *arg) {
...
static int test_mul1_check(uint64_t rc, const void *arg) {
  uint64_t r2, r3, r4;
...
static void test_shift1_prepare(void *arg) {
  struct dummy_vect8 *dv;
  uint64_t v;
...
static int test_shift1_check(uint64_t rc, const void *arg) {
  uint64_t r2, r3, r4;
  const struct dummy_vect8 *dvt;
  struct dummy_vect8 dve;
...
static void test_jump1_prepare(void *arg) {
  struct dummy_vect8 *dv;
  uint64_t v1, v2;
...
static int test_jump1_check(uint64_t rc, const void *arg) {
...
static void test_jump2_prepare(void *arg) {
  struct dummy_net *dn;
   ...
   */
  rte_ether_addr_copy((struct rte_ether_addr *)dst_mac, &dn->eth_hdr.dst_addr);
  rte_ether_addr_copy((struct rte_ether_addr *)src_mac, &dn->eth_hdr.src_addr);
...
static int test_jump2_check(uint64_t rc, const void *arg) {
  ...
  if (eth_hdr->ether_type == htons(0x8100)) {
    const struct rte_vlan_hdr *vlan_hdr = (const void *)(eth_hdr + 1);
...
static int test_alu1_check(uint64_t rc, const void *arg) {
  uint64_t r2, r3, r4, r5, rv;
  const struct dummy_vect8 *dvt;
  struct dummy_vect8 dve;
...
static void test_bele1_prepare(void *arg) {
  struct dummy_vect8 *dv;
...
static int test_bele1_check(uint64_t rc, const void *arg) {
  uint64_t r2, r3, r4;
  const struct dummy_vect8 *dvt;
  struct dummy_vect8 dve;
...
static int test_xadd1_check(uint64_t rc, const void *arg) {
  uint64_t rv;
  const struct dummy_offset *dft;
  struct dummy_offset dfe;
...
static int test_div1_check(uint64_t rc, const void *arg) {
  uint64_t r2, r3, r4;
  const struct dummy_vect8 *dvt;
  struct dummy_vect8 dve;
...
static void dummy_func1(const void *p, uint32_t *v32, uint64_t *v64) {
...
static int test_call1_check(uint64_t rc, const void *arg) {
  uint32_t v32;
  uint64_t v64;
  const struct dummy_offset *dv;
...
static void dummy_func2(struct dummy_offset *a, struct dummy_offset *b) {
  uint64_t v;
...
static int test_call2_check(uint64_t rc, const void *arg) {
  uint64_t v;
...
static const struct dummy_offset *dummy_func3(const struct dummy_vect8 *p) {
  return &p->in[RTE_DIM(p->in) - 1];
}

static void test_call3_prepare(void *arg) {
...
static int test_call3_check(uint64_t rc, const void *arg) {
  uint64_t v;
...
static uint32_t dummy_func4_1(uint8_t a, uint8_t b, uint8_t c, uint8_t d) {
  return (a << 24) | (b << 16) | (c << 8) | (d << 0);
}

/* Implementation of memfrob */
static uint32_t dummy_func4_0(uint32_t *s, uint8_t n) {
  char *p = (char *)s;
  while (n-- > 0)
    *p++ ^= 42;
  return *s;
...
static int test_call4_check(uint64_t rc, const void *arg) {
  uint8_t a[4] = {1, 2, 3, 4};
  uint32_t s, v = 0;
...
static uint32_t dummy_func5(const char *s1, const char *s2) {
  while (*s1 && (*s1 == *s2)) {
    s1++;
    s2++;
  }
...
static int test_call5_check(uint64_t rc, const void *arg) {
  char a[] = "geek";
  char b[] = "week";
  uint32_t v;
...
static void dummy_mbuf_prep(struct rte_mbuf *mb, uint8_t buf[],
                            uint32_t buf_len, uint32_t data_len) {
  uint32_t i;
  uint8_t *db;
...
static void test_ld_mbuf1_prepare(void *arg) {
  ...
  dummy_mbuf_prep(&dm->mb[0], dm->buf[0], sizeof(dm->buf[0]), plen / 2 + 1);
  dummy_mbuf_prep(&dm->mb[1], dm->buf[1], sizeof(dm->buf[0]), plen / 2 - 1);
...
static uint64_t test_ld_mbuf1(const struct rte_mbuf *pkt) {
  uint64_t n, v;
  const uint8_t *p8;
  const uint16_t *p16;
  const uint32_t *p32;
  ...
  /* load IPv4 version and IHL */
  p8 = rte_pktmbuf_read(pkt, offsetof(struct rte_ipv4_hdr, version_ihl),
                        sizeof(*p8), &dof);
  if (p8 == NULL)
    return 0;
  ...
  /* load IPv4 src addr */
  p32 = rte_pktmbuf_read(pkt, offsetof(struct rte_ipv4_hdr, src_addr),
                         sizeof(*p32), &dof);
  if (p32 == NULL)
    return 0;
  ...
  /* load IPv4 total length */
  p16 = rte_pktmbuf_read(pkt, offsetof(struct rte_ipv4_hdr, total_length),
                         sizeof(*p16), &dof);
  if (p16 == NULL)
    return 0;
...
static int test_ld_mbuf1_check(uint64_t rc, const void *arg) {
 ...
 */
static void test_ld_mbuf2_prepare(void *arg) {
  struct dummy_mbuf *dm;
...
static int test_ld_mbuf2_check(uint64_t rc, const void *arg) {
...
static int run_test(const struct bpf_test *tst) {
  int32_t ret, rv;
  int64_t rc;
  struct rte_bpf *bpf;
  ...
  if (bpf == NULL) {
    printf("%s@%d: failed to load bpf code, error=%d(%s);\n", __func__,
           __LINE__, rte_errno, strerror(rte_errno));
    return -1;
  }
  ...
  if (ret != 0) {
    printf("%s@%d: check_result(%s) failed, error: %d(%s);\n", __func__,
           __LINE__, tst->name, ret, strerror(ret));
  }
  ...
  if (jit.func != NULL) {
    ...
    if (rv != 0) {
      printf("%s@%d: check_result(%s) failed, "
             "error: %d(%s);\n",
             __func__, __LINE__, tst->name, rv, strerror(rv));
    }
  }
...
static void test_bpf_dump(struct bpf_program *cbf,
...
static int test_bpf_match(pcap_t *pcap, const char *str, struct rte_mbuf *mb) {
  ...
  if (prm == NULL) {
    printf("%s@%d: bpf_convert('%s') failed,, error=%d(%s);\n", __func__,
           __LINE__, str, rte_errno, strerror(rte_errno));
    goto error;
  }
...
static int test_bpf_filter_sanity(pcap_t *pcap) {
...
static int test_bpf_filter(pcap_t *pcap, const char *s) {
  struct bpf_program fcode;
  struct rte_bpf_prm *prm = NULL;
  struct rte_bpf *bpf = NULL;
  ...
  if (prm == NULL) {
    printf("%s@%d: bpf_convert('%s') failed,, error=%d(%s);\n", __func__,
           __LINE__, s, rte_errno, strerror(rte_errno));
    goto error;
  }
...
static int test_bpf_convert(void) {
  unsigned int i;
  pcap_t *pcap;
  int rc;
