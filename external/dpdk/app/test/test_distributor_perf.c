...
static int test_distributor_perf(void) {
...
#define ITER_POWER_CL                                                          \
  25                  /* log 2 of how many iterations  for Cache Line test     \
                       */
 ...
 */
static int flip_bit(volatile uint64_t *arg) {
  uint64_t old_val = 0;
  while (old_val != 2) {
    while (!*arg)
      rte_pause();
 ...
 */
static void time_cache_line_switch(void) {
 ...
 */
static unsigned total_packet_count(void) {
  unsigned i, count = 0;
...
static void clear_packet_count(void) {
 ...
 */
static inline int perf_test(struct rte_distributor *d, struct rte_mempool *p) {
  unsigned int i;
  uint64_t start, end;
...
static void quit_workers(struct rte_distributor *d, struct rte_mempool *p) {
...
static int test_distributor_perf(void) {
  ...
  if (rte_lcore_count() < 2) {
    printf("Not enough cores for distributor_perf_autotest, expecting at least "
           "2\n");
    return TEST_SKIPPED;
  }
  ...
  const unsigned nb_bufs = (511 * rte_lcore_count()) < BIG_BATCH
                               ...
                               : (511 * rte_lcore_count());
  if (p == NULL) {
    p = rte_pktmbuf_pool_create("DPT_MBUF_POOL", nb_bufs, BURST, 0,
