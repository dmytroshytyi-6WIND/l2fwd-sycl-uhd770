...
static int configure_ethdev(uint16_t port_id, struct rte_eth_conf *eth_conf,
                            uint8_t start) {
  int rxq, txq;

  TEST_ASSERT(rte_eth_dev_configure(port_id, RXTX_QUEUE_COUNT, RXTX_QUEUE_COUNT,
                                    eth_conf) == 0,
...
static int remove_slaves(void) {
  ...
  FOR_EACH_PORT(n, port) {
    ...
    if (port->is_slave) {
      TEST_ASSERT_SUCCESS(
          rte_eth_bond_slave_remove(test_params.bond_port_id, port->port_id),
...
static int bond_slaves(void) {
  ...
  FOR_EACH_PORT(n, port) {
    ...
    if (!port->is_slave) {
      TEST_ASSERT_SUCCESS(
          rte_eth_bond_slave_add(test_params.bond_port_id, port->port_id),
          "Cannot attach slave %d to the bonding", port->port_id);
      port->is_slave = 1;
    }
  }
 ...
 */
static int reta_set(uint16_t port_id, uint8_t value, int reta_size) {
  struct rte_eth_rss_reta_entry64 reta_conf[512 / RTE_ETH_RETA_GROUP_SIZE];
  int i, j;
 ...
 */
static int reta_check_synced(struct slave_conf *port) {
  unsigned i;

  for (i = 0; i < test_params.bond_dev_info.reta_size; i++) {
 ...
 */
static int bond_reta_fetch(void) {
  unsigned j;
  ...
  TEST_ASSERT_SUCCESS(rte_eth_dev_rss_reta_query(
                          test_params.bond_port_id, test_params.bond_reta_conf,
 ...
 * Fetch slaves RETA
 */
static int slave_reta_fetch(struct slave_conf *port) {
  unsigned j;

  for (j = 0; j < port->dev_info.reta_size / RTE_ETH_RETA_GROUP_SIZE; j++)
    port->reta_conf[j].mask = ~0LL;

  TEST_ASSERT_SUCCESS(rte_eth_dev_rss_reta_query(port->port_id, port->reta_conf,
 ...
 */
static int slave_remove_and_add(void) {
 ...
 */
static int test_propagate(void) {
  unsigned i;
  uint8_t n;
  struct slave_conf *port;
  ...
  for (i = 0; i < sizeof(test_params.bond_dev_info.flow_type_rss_offloads) * 8;
    ...
    if (rss_hf) {
      ...
      retval =
          rte_eth_dev_rss_hash_update(test_params.bond_port_id, &bond_rss_conf);
      TEST_ASSERT_SUCCESS(retval, "Cannot set slaves hash function");

      FOR_EACH_PORT(n, port) {
        port = &test_params.slave_ports[n];

        retval = rte_eth_dev_rss_hash_conf_get(port->port_id, &port->rss_conf);
        TEST_ASSERT_SUCCESS(retval, "Cannot take slaves RSS configuration");

        TEST_ASSERT(port->rss_conf.rss_hf == rss_hf,
                    "Hash function not propagated for slave %d", port->port_id);
      }
  ...
  for (i = 1; i < 10; i++) {
    ...
    FOR_EACH_PORT(n, port) {
      ...
      retval = rte_eth_dev_rss_hash_update(port->port_id, &port->rss_conf);
      TEST_ASSERT_SUCCESS(retval, "Cannot set slaves RSS keys");
    }

    memset(bond_rss_key, i, sizeof(bond_rss_key));
    bond_rss_conf.rss_hf = default_rss_hf, bond_rss_conf.rss_key = bond_rss_key;
    ...
    FOR_EACH_PORT(n, port) {
      ...
      /* compare keys */
      retval =
          memcmp(port->rss_conf.rss_key, bond_rss_key, sizeof(bond_rss_key));
  ...
  for (i = 0; i < RXTX_QUEUE_COUNT; i++) {
    ...
    TEST_ASSERT_SUCCESS(reta_set(test_params.bond_port_id, i % RXTX_QUEUE_COUNT,
                                 test_params.bond_dev_info.reta_size),
...
static int test_rss(void) {
  ...
  TEST_ASSERT_SUCCESS(
      configure_ethdev(test_params.bond_port_id, &rss_pmd_conf, 0),
      "Failed to configure bonding device\n");

  ret = rte_eth_dev_info_get(test_params.bond_port_id,
                             &test_params.bond_dev_info);
  TEST_ASSERT((ret == 0), "Error during getting device (port %u) info: %s\n",
 ...
 */
static int test_rss_lazy(void) {
  int ret;

  TEST_ASSERT_SUCCESS(
      configure_ethdev(test_params.bond_port_id, &default_pmd_conf, 0),
      "Failed to configure bonding device\n");
...
static int test_setup(void) {
  ...
  if (test_params.mbuf_pool == NULL) {
    ...
    test_params.mbuf_pool =
        rte_pktmbuf_pool_create("RSS_MBUF_POOL", NUM_MBUFS * SLAVE_COUNT,
...
static int check_environment(void) { return TEST_SUCCESS; }

static int test_rssconf_executor(int (*test_func)(void)) {
  int test_result;
...
static int test_setup_wrapper(void) {
  return test_rssconf_executor(&test_setup);
}

static int test_rss_wrapper(void) { return test_rssconf_executor(&test_rss); }

static int test_rss_lazy_wrapper(void) {
...
static struct unit_test_suite link_bonding_rssconf_test_suite = {
    ...
    .unit_test_cases = {TEST_CASE_NAMED("test_setup", test_setup_wrapper),
                        TEST_CASE_NAMED("test_rss", test_rss_wrapper),
                        TEST_CASE_NAMED("test_rss_lazy", test_rss_lazy_wrapper),

                        TEST_CASES_END()}};

static int test_link_bonding_rssconf(void) {
