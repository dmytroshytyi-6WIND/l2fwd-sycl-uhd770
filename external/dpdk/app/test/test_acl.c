...
static void bswap_test_data(struct ipv4_7tuple *data, int len, int to_be) {
  int i;
...
static int acl_ipv4vlan_check_rule(const struct rte_acl_ipv4vlan_rule *rule) {
...
static void acl_ipv4vlan_convert_rule(const struct rte_acl_ipv4vlan_rule *ri,
  ...
  ro->field[RTE_ACL_IPV4VLAN_VLAN2_FIELD].mask_range.u16 = ri->domain_mask;
  ro->field[RTE_ACL_IPV4VLAN_SRC_FIELD].mask_range.u32 = ri->src_mask_len;
  ro->field[RTE_ACL_IPV4VLAN_DST_FIELD].mask_range.u32 = ri->dst_mask_len;
  ro->field[RTE_ACL_IPV4VLAN_SRCP_FIELD].mask_range.u16 = ri->src_port_high;
  ro->field[RTE_ACL_IPV4VLAN_DSTP_FIELD].mask_range.u16 = ri->dst_port_high;
}
 ...
 */
static int rte_acl_ipv4vlan_add_rules(struct rte_acl_ctx *ctx,
...
static void acl_ipv4vlan_config(struct rte_acl_config *cfg,
                                const uint32_t layout[RTE_ACL_IPV4VLAN_NUM],
                                uint32_t num_categories) {
  static const struct rte_acl_field_def ipv4_defs[RTE_ACL_IPV4VLAN_NUM_FIELDS] =
      {
          {
  ...
  cfg->defs[RTE_ACL_IPV4VLAN_SRC_FIELD].offset = layout[RTE_ACL_IPV4VLAN_SRC];
  cfg->defs[RTE_ACL_IPV4VLAN_DST_FIELD].offset = layout[RTE_ACL_IPV4VLAN_DST];
 ...
 *   - Zero if operation completed successfully.
 */
static int rte_acl_ipv4vlan_build(struct rte_acl_ctx *ctx,
                                  const uint32_t layout[RTE_ACL_IPV4VLAN_NUM],
                                  uint32_t num_categories) {
 ...
 */
static int test_classify_alg(struct rte_acl_ctx *acx,
                             struct ipv4_7tuple test_data[],
                             const uint8_t *data[], size_t dim,
  ...
  for (count = 0; count <= dim; count++) {
    ret = rte_acl_classify(acx, data, results, count, RTE_ACL_MAX_CATEGORIES);
    if (ret != 0) {
 ...
 */
static int test_classify_run(struct rte_acl_ctx *acx,
                             struct ipv4_7tuple test_data[], size_t dim) {
  int32_t ret;
  uint32_t i;
  const uint8_t *data[dim];

  static const enum rte_acl_classify_alg alg[] = {
      RTE_ACL_CLASSIFY_SCALAR,    RTE_ACL_CLASSIFY_SSE,
      RTE_ACL_CLASSIFY_AVX2,      RTE_ACL_CLASSIFY_NEON,
      RTE_ACL_CLASSIFY_ALTIVEC,   RTE_ACL_CLASSIFY_AVX512X16,
...
static int test_classify_buid(struct rte_acl_ctx *acx,
                              const struct rte_acl_ipv4vlan_rule *rules,
                              uint32_t num) {
  int ret;
  ...
  ret = rte_acl_ipv4vlan_build(acx, ipv4_7tuple_layout, RTE_ACL_MAX_CATEGORIES);
  if (ret != 0) {
...
static int test_classify(void) {
  ...
  for (i = 0; i != TEST_CLASSIFY_ITER; i++) {
    ...
    ret = test_classify_buid(acx, acl_test_rules, RTE_DIM(acl_test_rules));
    if (ret != 0) {
    ...
    ret = test_classify_run(acx, acl_test_data, RTE_DIM(acl_test_data));
    if (ret != 0) {
...
static int test_build_ports_range(void) {
  ...
  for (i = 0; i != RTE_DIM(test_rules); i++) {
    ...
    ret = rte_acl_classify(acx, data, results, RTE_DIM(data), 1);
    if (ret != 0) {
...
static void convert_rule(const struct rte_acl_ipv4vlan_rule *ri,
                         struct acl_ipv4vlan_rule *ro) {
  ro->data = ri->data;
 ...
 */
static void convert_rule_1(const struct rte_acl_ipv4vlan_rule *ri,
                           struct acl_ipv4vlan_rule *ro) {
  uint32_t v;
 ...
 */
static void convert_rule_2(const struct rte_acl_ipv4vlan_rule *ri,
                           struct acl_ipv4vlan_rule *ro) {
  uint32_t hi, lo, mask;
 ...
 */
static void convert_rule_3(const struct rte_acl_ipv4vlan_rule *ri,
                           struct acl_ipv4vlan_rule *ro) {
 ...
 */
static void convert_rule_4(const struct rte_acl_ipv4vlan_rule *ri,
                           struct acl_ipv4vlan_rule *ro) {
  ...
  ro->field[RTE_ACL_IPV4VLAN_SRC_FIELD] = ro->field[RTE_ACL_IPV4VLAN_DST_FIELD];

  ro->field[RTE_ACL_IPV4VLAN_DST_FIELD] = t;
}

static void ipv4vlan_config(struct rte_acl_config *cfg,
                            const uint32_t layout[RTE_ACL_IPV4VLAN_NUM],
                            uint32_t num_categories) {
  static const struct rte_acl_field_def ipv4_defs[RTE_ACL_IPV4VLAN_NUM_FIELDS] =
      {
...
static int convert_rules(struct rte_acl_ctx *acx,
...
static void convert_config(struct rte_acl_config *cfg) {
 ...
 */
static void convert_config_1(struct rte_acl_config *cfg) {
  ipv4vlan_config(cfg, ipv4_7tuple_layout, RTE_ACL_MAX_CATEGORIES);
 ...
 */
static void convert_config_2(struct rte_acl_config *cfg) {
  ipv4vlan_config(cfg, ipv4_7tuple_layout, RTE_ACL_MAX_CATEGORIES);
 ...
 */
static void convert_config_3(struct rte_acl_config *cfg) {
 ...
 */
static void convert_config_4(struct rte_acl_config *cfg) {
  ...
  cfg->defs[RTE_ACL_IPV4VLAN_SRC_FIELD] = cfg->defs[RTE_ACL_IPV4VLAN_DST_FIELD];
...
static int build_convert_rules(struct rte_acl_ctx *acx,
...
static int
test_convert_rules(const char *desc, void (*config)(struct rte_acl_config *),
                   void (*convert)(const struct rte_acl_ipv4vlan_rule *,
                                   struct acl_ipv4vlan_rule *)) {
  struct rte_acl_ctx *acx;
  int32_t rc;
  ...
  rc = convert_rules(acx, convert, acl_test_rules, RTE_DIM(acl_test_rules));
  if (rc != 0)
  ...
  for (i = 0; rc == 0 && i != RTE_DIM(mem_sizes); i++) {
    ...
    rc = test_classify_run(acx, acl_test_data, RTE_DIM(acl_test_data));
    if (rc != 0)
 ...
 * destination port.
 */
static int test_invalid_layout(void) {
  struct rte_acl_ctx *acx;
  int ret, i;
 ...
 */
static int test_create_find_add(void) {
 ...
 */
static int test_invalid_rules(void) {
  struct rte_acl_ctx *acx;
  int ret;
...
static int test_invalid_parameters(void) {
  ...
  /* cover zero categories in classify (should not fail) */
  result = rte_acl_classify_alg(acx, NULL, NULL, 0, 0, RTE_ACL_CLASSIFY_SCALAR);
  if (result != 0) {
...
static uint32_t get_u32_range_max(void) {
  uint32_t i, max;
...
static uint32_t get_u32_range_min(void) {
  uint32_t i, min;
...
static const struct rte_acl_ipv4vlan_rule *find_u32_range_rule(uint32_t val) {
  uint32_t i;
...
static void fill_u32_range_data(struct ipv4_7tuple tdata[], uint32_t start,
                                uint32_t num) {
  uint32_t i;
...
static int test_u32_range(void) {
  int32_t rc;
