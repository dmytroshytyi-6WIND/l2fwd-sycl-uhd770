...
enum { IP_CLASS_A, IP_CLASS_B, IP_CLASS_C };
...
#define IP_HEAD_BIT_NUM_C 3
  ...
  for (k = 0; k < rule_num; k++) {
    ptr_rule->ip = (start << (RTE_LPM_MAX_DEPTH - depth)) | ip_head_mask;
...
static void print_route_distribution(const struct route_rule *table,
                                     uint32_t n) {
  unsigned i, j;
 ...
 */
static int test_lpm_reader(void *arg) {
  int i;
 ...
 */
static int test_lpm_rcu_qsbr_reader(void *arg) {
  int i;
 ...
 */
static int test_lpm_rcu_qsbr_writer(void *arg) {
  ...
  for (i = 0; i < RCU_ITERATIONS; i++) {
    ...
    for (j = si; j < ei; j++) {
      ...
      if (rte_lpm_add(lpm, large_ldepth_route_table[j].ip,
                      large_ldepth_route_table[j].depth, next_hop_add) != 0) {
 ...
 */
static int test_lpm_rcu_perf_multi_writer(uint8_t use_rcu) {
  ...
  if (rte_lcore_count() < 3) {
    printf(
        "Not enough cores for lpm_rcu_perf_autotest, expecting at least 3\n");
    return TEST_SKIPPED;
  }
  ...
  for (j = 1; j < 3; j++) {
    ...
    if (use_rcu) {
      ...
      rv = (struct rte_rcu_qsbr *)rte_zmalloc("rcu0", sz, RTE_CACHE_LINE_SIZE);
    ...
    for (i = 0; i < j; i++)
      rte_eal_remote_launch(test_lpm_rcu_qsbr_writer, (void *)(uintptr_t)i,
                            enabled_core_ids[i]);
...
static int test_lpm_perf(void) {
  ...
  for (i = 0; i < NUM_ROUTE_ENTRIES; i++) {
    if (rte_lpm_add(lpm, large_route_table[i].ip, large_route_table[i].depth,
                    next_hop_add) == 0)
      status++;
  }
  /* End Timer. */
