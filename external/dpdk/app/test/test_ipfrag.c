...
static struct rte_mempool *pkt_pool, *direct_pool, *indirect_pool;

static int setup_buf_pool(void) {
  pkt_pool = rte_pktmbuf_pool_create("FRAG_MBUF_POOL", NUM_MBUFS, BURST, 0,
  ...
  direct_pool =
      rte_pktmbuf_pool_create("FRAG_D_MBUF_POOL", NUM_MBUFS, BURST, 0,
                              RTE_MBUF_DEFAULT_BUF_SIZE, SOCKET_ID_ANY);
  if (direct_pool == NULL) {
  ...
  indirect_pool = rte_pktmbuf_pool_create("FRAG_I_MBUF_POOL", NUM_MBUFS, BURST,
                                          0, 0, SOCKET_ID_ANY);
...
static int testsuite_setup(void) { return setup_buf_pool(); }
...
static void v4_allocate_packet_of(struct rte_mbuf *b, int fill, size_t s,
                                  int df, uint8_t mf, uint16_t off, uint8_t ttl,
...
static void v6_allocate_packet_of(struct rte_mbuf *b, int fill, size_t s,
...
static inline void test_free_fragments(struct rte_mbuf *mb[], uint32_t num) {
  uint32_t i;
  for (i = 0; i < num; i++)
    rte_pktmbuf_free(mb[i]);
}

static inline void test_get_offset(struct rte_mbuf **mb, int32_t len,
                                   uint16_t *offset, int ipv) {
  int32_t i;

  for (i = 0; i < len; i++) {
    if (ipv == 4) {
      struct rte_ipv4_hdr *iph = rte_pktmbuf_mtod(mb[i], struct rte_ipv4_hdr *);
      offset[i] = iph->fragment_offset;
    } else if (ipv == 6) {
      struct ipv6_extension_fragment *fh = rte_pktmbuf_mtod_offset(
          mb[i], struct ipv6_extension_fragment *, sizeof(struct rte_ipv6_hdr));
      offset[i] = fh->frag_data;
    }
  }
}

static int test_ip_frag(void) {
  ...
  struct test_ip_frags {
    ...
    uint16_t expected_fragment_offset[BURST];
      ...
      {4, 1280, 1400, 0, 0, 0, 64, IPPROTO_ICMP, RND_ID, 2, {0x2000, 0x009D}},
      {4, 1280, 1400, 0, 0, 0, 64, IPPROTO_ICMP, 0, 2, {0x2000, 0x009D}},
      {4,
       600,
       1400,
       0,
      ...
      {6, 1280, 1400, 0, 0, 0, 64, IPPROTO_ICMP, RND_ID, 2, {0x0001, 0x04D0}},
      {6, 1300, 1400, 0, 0, 0, 64, IPPROTO_ICMP, RND_ID, 2, {0x0001, 0x04E0}},
      {6, 4, 1400, 0, 0, 0, 64, IPPROTO_ICMP, RND_ID, -EINVAL},
      {6, 1300, 1400, 0, 0, 0, 0, IPPROTO_ICMP, RND_ID, 2, {0x0001, 0x04E0}},
  };
  ...
  for (i = 0; i < RTE_DIM(tests); i++) {
    ...
    RTE_TEST_ASSERT_NOT_EQUAL(b, NULL, "Failed to allocate pkt.");

    if (tests[i].pkt_id == RND_ID)
      pktid = rte_rand_max(UINT16_MAX);

    if (tests[i].ipv == 4) {
      v4_allocate_packet_of(b, 0x41414141, tests[i].pkt_size, tests[i].set_df,
                            tests[i].set_mf, tests[i].set_of, tests[i].ttl,
                            tests[i].proto, pktid);
    } else if (tests[i].ipv == 6) {
      v6_allocate_packet_of(b, 0x41414141, tests[i].pkt_size, tests[i].ttl,
                            tests[i].proto, pktid);
    }

    if (tests[i].ipv == 4)
      len = rte_ipv4_fragment_packet(b, pkts_out, BURST, tests[i].mtu_size,
                                     direct_pool, indirect_pool);
    else if (tests[i].ipv == 6)
      len = rte_ipv6_fragment_packet(b, pkts_out, BURST, tests[i].mtu_size,
                                     direct_pool, indirect_pool);

    rte_pktmbuf_free(b);

    if (len > 0) {
      test_get_offset(pkts_out, len, fragment_offset, tests[i].ipv);
      test_free_fragments(pkts_out, len);
    }

    printf("%zd: checking %d with %d\n", i, len, tests[i].expected_frags);
    RTE_TEST_ASSERT_EQUAL(len, tests[i].expected_frags, "Failed case %zd.\n",
                          i);

    if (len > 0) {
      for (j = 0; j < (size_t)len; j++) {
        printf("%zd-%zd: checking %d with %d\n", i, j, fragment_offset[j],
               rte_cpu_to_be_16(tests[i].expected_fragment_offset[j]));
...
static struct unit_test_suite ipfrag_testsuite = {
    ...
    .unit_test_cases = {
        TEST_CASE_ST(ut_setup, ut_teardown, test_ip_frag),
