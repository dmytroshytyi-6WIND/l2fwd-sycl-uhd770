...
static int test_power_cpufreq(void) {
  printf("Power management library not supported, skipping test\n");
  return TEST_SKIPPED;
}

static int test_power_caps(void) {
  printf("Power management library not supported, skipping test\n");
  return TEST_SKIPPED;
}
...
static int check_cur_freq(unsigned int lcore_id, uint32_t idx, bool turbo) {
...
#define MAX_LOOP 100
  ...
  if (snprintf(fullpath, sizeof(fullpath), TEST_POWER_SYSFILE_CPUINFO_FREQ,
               lcore_id) < 0) {
    return 0;
  }
  f = fopen(fullpath, "r");
  if (f == NULL) {
    if (snprintf(fullpath, sizeof(fullpath), TEST_POWER_SYSFILE_SCALING_FREQ,
                 lcore_id) < 0) {
      return 0;
    }
    f = fopen(fullpath, "r");
  ...
  for (i = 0; i < MAX_LOOP; i++) {
    ...
    if (env == PM_ENV_CPPC_CPUFREQ || env == PM_ENV_PSTATE_CPUFREQ) {
       ...
       */
      freq_conv =
          (cur_freq + TEST_FREQ_ROUNDING_DELTA) / TEST_ROUND_FREQ_TO_N_100000;
...
static int check_power_freqs(void) {
  uint32_t ret;
  ...
  ret = rte_power_freqs(TEST_POWER_LCORE_ID, NULL, TEST_POWER_FREQS_NUM_MAX);
  if (ret > 0) {
  ...
  ret = rte_power_freqs(TEST_POWER_LCORE_ID, freqs, TEST_POWER_FREQS_NUM_MAX);
...
static int check_power_get_freq(void) {
  int ret;
  uint32_t count;
...
static int check_power_set_freq(void) {
  ...
  ret = rte_power_set_freq(TEST_POWER_LCORE_ID, TEST_POWER_FREQS_NUM_MAX);
  if (ret >= 0) {
    printf("Unexpectedly set an invalid freq index (%u)"
           "successfully on lcore %u\n",
           TEST_POWER_FREQS_NUM_MAX, TEST_POWER_LCORE_ID);
    return -1;
  }
  ...
  if (ret >= 0) {
    printf("Unexpectedly set an invalid freq index (%u)"
           "successfully on lcore %u\n",
           total_freq_num, TEST_POWER_LCORE_ID);
    return -1;
  }
...
static int check_power_freq_down(void) {
  int ret;
...
static int check_power_freq_max(void) {
  int ret;
...
static int check_power_freq_min(void) {
  int ret;
...
static int check_power_turbo(void) {
  int ret;
...
static int test_power_cpufreq(void) {
  ...
  if (rte_power_freq_enable_turbo == NULL) {
    printf(
        "rte_power_freq_enable_turbo should not be NULL, environment has not "
        "been initialised\n");
    goto fail_all;
  }
  if (rte_power_freq_disable_turbo == NULL) {
    printf(
        "rte_power_freq_disable_turbo should not be NULL, environment has not "
        "been initialised\n");
    goto fail_all;
  }
