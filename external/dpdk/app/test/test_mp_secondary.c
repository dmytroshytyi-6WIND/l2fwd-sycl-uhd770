 ...
 */
static int run_secondary_instances(void) {
  int ret = 0;
  char coremask[10];
  ...
  const char *argv1[] = {prgname, "-c", coremask, "--proc-type=secondary",
                         prefix};
  /* good case, using auto */
  const char *argv2[] = {prgname, "-c", coremask, "--proc-type=auto", prefix};
  /* bad case, using invalid type */
  const char *argv3[] = {prgname, "-c", coremask, "--proc-type=ERROR", prefix};
#ifdef RTE_EXEC_ENV_LINUX
  /* bad case, using invalid file prefix */
  const char *argv4[] = {prgname, "-c", coremask, "--proc-type=secondary",
                         "--file-prefix=ERROR"};
#endif

  snprintf(coremask, sizeof(coremask), "%x", (1 << rte_get_main_lcore()));
 ...
 */
static int run_object_creation_tests(void) {
  ...
  rte_errno = 0;
  if ((rte_ring_create("test_ring", size, rte_socket_id(), flags) == NULL) &&
  ...
  rte_errno = 0;
  if ((rte_mempool_create("test_mp", size, elt_size, cache_size, priv_data_size,
