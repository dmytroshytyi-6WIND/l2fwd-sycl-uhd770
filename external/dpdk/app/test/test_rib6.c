...
static int test_rib6(void) {
  printf("rib6 not supported on Windows, skipping test\n");
  return TEST_SKIPPED;
}

static int test_slow_rib6(void) {
...
int32_t test_create_invalid(void) {
  ...
  RTE_TEST_ASSERT(rib == NULL, "Call succeeded with invalid parameters\n");
 ...
 */
int32_t test_insert_invalid(void) {
  struct rte_rib6 *rib = NULL;
  ...
  RTE_TEST_ASSERT(node == NULL, "Call succeeded with invalid parameters\n");
  ...
  RTE_TEST_ASSERT(node1 == NULL, "Call succeeded with invalid parameters\n");
...
int32_t test_get_fn(void) {
  ...
  RTE_TEST_ASSERT(ext == NULL, "Call succeeded with invalid parameters\n");
  ...
  RTE_TEST_ASSERT(ret == 0, "Failed to set rte_rib_node nexthop\n");
...
int32_t test_basic(void) {
  ...
  RTE_TEST_ASSERT(status == 0, "Failed to set rte_rib_node field\n");
  ...
  node = rte_rib6_lookup(rib, ip);
  RTE_TEST_ASSERT(node == NULL, "Lookup returns non existent rule\n");
  node = rte_rib6_lookup_exact(rib, ip, depth);
  RTE_TEST_ASSERT(node == NULL, "Lookup returns non existent rule\n");
...
int32_t test_tree_traversal(void) {
  struct rte_rib6 *rib = NULL;
  struct rte_rib6_node *node;
  struct rte_rib6_conf config;
...
static struct unit_test_suite rib6_tests = {
    ...
    .setup = NULL,
    .teardown = NULL,
    .unit_test_cases = {TEST_CASE(test_create_invalid),
                        TEST_CASE(test_free_null),
                        TEST_CASE(test_insert_invalid), TEST_CASE(test_get_fn),
                        TEST_CASE(test_basic), TEST_CASE(test_tree_traversal),
                        TEST_CASES_END()}};
 ...
 * Do all unit tests.
 */
static int test_rib6(void) { return unit_test_suite_runner(&rib6_tests); }
