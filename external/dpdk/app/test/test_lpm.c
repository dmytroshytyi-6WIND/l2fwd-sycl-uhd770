...
int32_t test14(void) {
  ...
  for (; ip <= RTE_IPV4(0, 1, 255, 0); ip += 256) {
    ...
    status = rte_lpm_lookup(lpm, ip, &next_hop_return);
    TEST_LPM_ASSERT((status == 0) && (next_hop_return == next_hop_base + ip));
...
int32_t test17(void) {
  ...
  const uint8_t d_ip_10_32 = 32, d_ip_10_24 = 24, d_ip_20_25 = 25;
  const uint32_t next_hop_ip_10_32 = 100, next_hop_ip_10_24 = 105,
  ...
  if ((status = rte_lpm_add(lpm, ip_10_32, d_ip_10_32, next_hop_ip_10_32)) < 0)
    return -1;
  ...
  if ((status = rte_lpm_add(lpm, ip_10_24, d_ip_10_24, next_hop_ip_10_24)) < 0)
    return -1;
  ...
  if ((status = rte_lpm_add(lpm, ip_20_25, d_ip_20_25, next_hop_ip_20_25)) < 0)
    return -1;
