...
static uint32_t hashtest_key_lens[] = {0,  2,  4,  5,  6,  7,  8,  10, 11,
                                       15, 16, 21, 31, 32, 33, 63, 64};
...
#define RETURN_IF_ERROR_RCU_QSBR(cond, str, ...)                               \
  do {                                                                         \
    if (cond) {                                                                \
      printf("ERROR line %d: " str "\n", __LINE__, ##__VA_ARGS__);             \
 ...
 */
static int test_crc32_hash_alg_equiv(void) {
  uint32_t hash_val;
  uint32_t init_val;
  ...
  for (i = 0; i < RTE_DIM(hashtest_funcs); i++) {
    for (j = 0; j < RTE_DIM(hashtest_initvals); j++) {
      for (k = 0; k < RTE_DIM(hashtest_key_lens); k++) {
        run_hash_func_test(hashtest_funcs[i], hashtest_initvals[j],
                           hashtest_key_lens[k]);
      }
    }
  }
...
static int test_add_delete(void) {
  ...
  pos0 = rte_hash_lookup(handle, &keys[0]);
  print_key_info("Lkp", &keys[0], pos0);
  RETURN_IF_ERROR(pos0 != -ENOENT, "fail: found key after deleting! (pos0=%d)",
                  pos0);
  ...
  pos1 = rte_hash_lookup_with_hash(handle, &keys[0], hash_value);
  print_key_info("Lkp", &keys[0], pos1);
  RETURN_IF_ERROR(pos1 != -ENOENT, "fail: found key after deleting! (pos1=%d)",
                  pos1);
...
static int test_add_update_delete(void) {
  ...
  pos0 = rte_hash_del_key(handle, &keys[0]);
  print_key_info("Del", &keys[0], pos0);
  RETURN_IF_ERROR(pos0 != -ENOENT, "fail: found non-existent key (pos0=%d)",
                  pos0);
...
static int test_add_update_delete_free(void) {
  ...
  RETURN_IF_ERROR(delPos0 != expectedPos0, "failed to delete key (pos0=%d)",
                  delPos0);
...
#define LCORE_CACHE_SIZE 64
   ...
   */
  for (i = 0;
       i < ut_params.entries + (RTE_MAX_LCORE - 1) * (LCORE_CACHE_SIZE - 1) + 1;
       i++) {
    keys[0].ip_src++;
    hash_value = rte_hash_hash(handle, &keys[0]);
    pos = rte_hash_add_key_with_hash(handle, &keys[0], hash_value);
...
static int test_hash_get_key_with_position(void) {
  ...
  RETURN_IF_ERROR(delPos != expectedPos, "failed to delete key (pos0=%d)",
                  delPos);
...
static int test_five_keys(void) {
  ...
  for (i = 0; i < 5; i++) {
    pos[i] = rte_hash_add_key(handle, &keys[i]);
    print_key_info("Add", &keys[i], pos[i]);
    RETURN_IF_ERROR(pos[i] != expected_pos[i], "failed to add key (pos[%u]=%d)",
                    i, pos[i]);
  }
  ...
  for (i = 0; i < 5; i++) {
    pos[i] = rte_hash_lookup(handle, &keys[i]);
    print_key_info("Lkp", &keys[i], pos[i]);
    RETURN_IF_ERROR(pos[i] != -ENOENT, "found non-existent key (pos[%u]=%d)", i,
                    pos[i]);
  }
  ...
  if (ret == 0)
    for (i = 0; i < 5; i++) {
      print_key_info("Lkp", key_array[i], pos[i]);
      RETURN_IF_ERROR(pos[i] != -ENOENT, "found not-existent key (pos[%u]=%d)",
                      i, pos[i]);
    }
...
static int test_full_bucket(void) {
  ...
  RETURN_IF_ERROR(pos[1] != expected_pos[1], "failed to delete key (pos[1]=%d)",
                  pos[1]);
...
/******************************************************************************/
static int fbk_hash_unit_test(void) {
  ...
  struct rte_fbk_hash_params invalid_params_7 = {
      ...
      .entries_per_bucket =
          RTE_FBK_HASH_ENTRIES_PER_BUCKET_MAX * 2, /* Entries > max allowed */
      .socket_id = 0,
  };
  ...
  uint32_t keys[5] = {0xc6e18639, 0xe67c201c, 0xd4c8cffd, 0x44728691,
                      0xd5430fa9};
  ...
  used_entries =
      rte_fbk_hash_get_load_factor(handle) * LOCAL_FBK_HASH_ENTRIES_MAX;
  RETURN_IF_ERROR_FBK(
...
static int test_fbk_hash_find_existing(void) {
  ...
  RETURN_IF_ERROR_FBK(result != handle,
                      "could not find existing fbk hash table");

  /* Try to find non-existing fbk hash table */
  result = rte_fbk_hash_find_existing("fbk_hash_find_non_existing");
  RETURN_IF_ERROR_FBK(!(result == NULL),
                      "found fbk table that shouldn't exist");
...
static int test_hash_creation_with_bad_parameters(void) {
  ...
  if (handle != NULL) {
    rte_hash_free(handle);
    printf("Impossible creating hash successfully with entries in parameter "
           "exceeded\n");
    return -1;
  }
  ...
  if (handle != NULL) {
    rte_hash_free(handle);
    printf("Impossible creating hash successfully if key_len in parameter is "
           "zero\n");
    return -1;
  }
 ...
 */
static int test_hash_creation_with_good_parameters(void) {
  struct rte_hash *handle;
  struct rte_hash_parameters params;
 ...
 */
static inline uint32_t test_jhash_32b(const void *k, uint32_t length,
                                      uint32_t initval) {
 ...
 */
static int test_hash_add_delete_jhash2(void) {
  int ret = -1;
  struct rte_hash *handle;
  int32_t pos1, pos2;
  ...
  if (pos2 < 0 || pos1 != pos2) {
    printf(
        "test_hash_add_delete_jhash2 delete different key from being added\n");
    goto fail_jhash2;
  }
  ret = 0;
 ...
 */
static int test_hash_add_delete_2_jhash2(void) {
  int ret = -1;
  struct rte_hash *handle;
  int32_t pos1, pos2;
...
static uint32_t test_hash_jhash_1word(const void *key, uint32_t length,
                                      uint32_t initval) {
...
static uint32_t test_hash_jhash_2word(const void *key, uint32_t length,
                                      uint32_t initval) {
  const uint32_t *k = key;
...
static uint32_t test_hash_jhash_3word(const void *key, uint32_t length,
                                      uint32_t initval) {
  const uint32_t *k = key;
 ...
 */
static int test_hash_add_delete_jhash_1word(void) {
  int ret = -1;
  struct rte_hash *handle;
  int32_t pos1, pos2;
 ...
 */
static int test_hash_add_delete_jhash_2word(void) {
  int ret = -1;
  struct rte_hash *handle;
  int32_t pos1, pos2;
 ...
 */
static int test_hash_add_delete_jhash_3word(void) {
  int ret = -1;
  struct rte_hash *handle;
  int32_t pos1, pos2;
 ...
 *  - Check returns
 */
static int test_hash_rcu_qsbr_add(void) {
  size_t sz;
  ...
  qsv2 = (struct rte_rcu_qsbr *)rte_zmalloc_socket(
      NULL, sz, RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
  RETURN_IF_ERROR_RCU_QSBR(qsv2 == NULL, "RCU QSBR variable creation failed");

  rcu_cfg.v = qsv2;
  rcu_cfg.mode = RTE_HASH_QSBR_MODE_SYNC;
  status = rte_hash_rcu_qsbr_add(g_handle, &rcu_cfg);
  rte_free(qsv2);
  RETURN_IF_ERROR_RCU_QSBR(
      status == 0, "Attach RCU QSBR to hash table succeeded where failure"
                   " is expected");
 ...
 *  - Reader lookup the entry
 */
static int test_hash_rcu_qsbr_dq_mode(uint8_t ext_bkt) {
  ...
  for (i = 0; i < total_entries; i++) {
    ...
    RETURN_IF_ERROR_RCU_QSBR(pos[i] < 0, "failed to add key (pos[%u]=%d)", i,
                             pos[i]);
    expected_pos[i] = pos[i];
  }
  ...
  RETURN_IF_ERROR_RCU_QSBR(pos[0] != expected_pos[0],
                           "failed to del correct key (pos[%u]=%d)", 0, pos[0]);

  /* Lookup */
  pos[0] = rte_hash_lookup(g_handle, &g_rand_keys[0]);
  print_key_info("Lkp", &g_rand_keys[0], pos[0]);
  RETURN_IF_ERROR_RCU_QSBR(pos[0] != -ENOENT, "found deleted key (pos[%u]=%d)",
                           0, pos[0]);
  ...
  pos[0] = rte_hash_add_key(g_handle, &g_rand_keys[0]);
  print_key_info("Add", &g_rand_keys[0], pos[0]);
  RETURN_IF_ERROR_RCU_QSBR(pos[0] < 0, "failed to add key (pos[%u]=%d)", 0,
                           pos[0]);
 ...
 */
static int test_hash_rcu_qsbr_sync_mode(uint8_t ext_bkt) {
  uint32_t total_entries = (ext_bkt == 0) ? 8 : 9;
