...
struct ipsec_unitest_params {
  ...
  struct rte_mbuf *obuf[BURST_SIZE], *ibuf[BURST_SIZE], *testbuf[BURST_SIZE];
...
static const struct ipsec_test_cfg test_cfg[] = {
    ...
    {REPLAY_WIN_0, ESN_DISABLED, 0, DATA_80_BYTES, BURST_SIZE, REORDER_PKTS},
    {REPLAY_WIN_32, ESN_ENABLED, 0, DATA_100_BYTES, 1, 0},
    {REPLAY_WIN_32, ESN_ENABLED, 0, DATA_100_BYTES, BURST_SIZE, REORDER_PKTS},
    {REPLAY_WIN_64, ESN_ENABLED, 0, DATA_64_BYTES, 1, 0},
    {REPLAY_WIN_128, ESN_ENABLED, RTE_IPSEC_SAFLAG_SQN_ATOM, DATA_80_BYTES, 1,
     0},
...
static int dummy_sec_create(void *device,
...
static int dummy_sec_destroy(void *device, struct rte_security_session *sess) {
  RTE_SET_USED(device);
  RTE_SET_USED(sess);
  return 0;
}
...
static void fill_crypto_xform(struct ipsec_unitest_params *ut_params,
  ...
  if (ut_params->ipsec_xform.direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS) {
...
static int check_cryptodev_capability(const struct ipsec_unitest_params *ut,
                                      uint8_t dev_id) {
  ...
  if (cap != NULL) {
    rc = rte_cryptodev_sym_capability_check_auth(
        cap, ut->auth_xform.auth.key.length, ut->auth_xform.auth.digest_length,
        0);
    if (rc == 0) {
      ...
      if (cap != NULL)
        rc = rte_cryptodev_sym_capability_check_cipher(
            cap, ut->cipher_xform.cipher.key.length,
...
static int testsuite_setup(void) {
  ...
  ts_params->mbuf_pool =
      rte_pktmbuf_pool_create("CRYPTO_MBUFPOOL", NUM_MBUFS, MBUF_CACHE_SIZE, 0,
  ...
  ts_params->qp_conf.mp_session_private =
      rte_mempool_create("test_priv_sess_mp", MAX_NB_SESSIONS, sess_sz, 0, 0,
                         NULL, NULL, NULL, NULL, SOCKET_ID_ANY, 0);

  TEST_ASSERT_NOT_NULL(ts_params->qp_conf.mp_session_private,
                       "private session mempool allocation failed");

  ts_params->qp_conf.mp_session = rte_cryptodev_sym_session_pool_create(
  ...
  TEST_ASSERT_SUCCESS(
      rte_cryptodev_queue_pair_setup(dev_id, 0, &ts_params->qp_conf,
                                     rte_cryptodev_socket_id(dev_id)),
      "Failed to setup queue pair %u on cryptodev %u", 0, dev_id);
...
static int ut_setup_ipsec(void) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  ...
  TEST_ASSERT_SUCCESS(rte_cryptodev_start(ts_params->valid_dev),
                      "Failed to start cryptodev %u", ts_params->valid_dev);

  return TEST_SUCCESS;
}

static void ut_teardown_ipsec(void) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  int i;
...
struct rte_ipv4_hdr ipv4_outer = {
    .version_ihl =
        IPVERSION << 4 | sizeof(ipv4_outer) / RTE_IPV4_IHL_MULTIPLIER,
...
static struct rte_mbuf *setup_test_string_tunneled(struct rte_mempool *mpool,
                                                   const char *string,
                                                   size_t len, uint32_t spi,
                                                   uint32_t seq) {
  struct rte_mbuf *m = rte_pktmbuf_alloc(mpool);
  ...
  struct rte_esp_hdr esph = {.spi = rte_cpu_to_be_32(spi),
...
static int create_dummy_sec_session(struct ipsec_unitest_params *ut,
                                    struct rte_cryptodev_qp_conf *qp,
                                    uint32_t j) {
  static struct rte_security_session_conf conf;

  ut->ss[j].security.ses = rte_security_session_create(
      &dummy_sec_ctx, &conf, qp->mp_session, qp->mp_session_private);
...
static int create_crypto_session(struct ipsec_unitest_params *ut,
                                 struct rte_cryptodev_qp_conf *qp,
                                 uint8_t dev_id, uint32_t j) {
  int32_t rc;
  ...
  rc = rte_cryptodev_sym_session_init(dev_id, s, ut->crypto_xforms,
...
static int create_session(struct ipsec_unitest_params *ut,
                          struct rte_cryptodev_qp_conf *qp, uint8_t crypto_dev,
                          uint32_t j) {
...
static int fill_ipsec_param(uint32_t replay_win_sz, uint64_t flags) {
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int create_sa(enum rte_security_session_action_type action_type,
...
static int crypto_dequeue_burst(uint16_t num_pkts) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint32_t pkt_cnt, k;
  int i;

  for (i = 0, pkt_cnt = 0; i < DEQUEUE_COUNT && pkt_cnt != num_pkts; i++) {
    k = rte_cryptodev_dequeue_burst(
        ts_params->valid_dev, 0, &ut_params->cop[pkt_cnt], num_pkts - pkt_cnt);
    pkt_cnt += k;
    rte_delay_us(1);
  }
...
static int crypto_ipsec(uint16_t num_pkts) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint32_t k, ng;
  ...
  k = rte_cryptodev_enqueue_burst(ts_params->valid_dev, 0, ut_params->cop,
                                  num_pkts);
  if (k != num_pkts) {
  ...
  ng = rte_ipsec_pkt_crypto_group(
      (const struct rte_crypto_op **)(uintptr_t)ut_params->cop, ut_params->obuf,
      grp, num_pkts);
  if (ng != 1 || grp[0].m[0] != ut_params->obuf[0] || grp[0].cnt != num_pkts ||
...
static int lksd_proto_ipsec(uint16_t num_pkts) {
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint32_t i, k, ng;
  struct rte_ipsec_group grp[1];
  ...
  for (i = 0; i != num_pkts; i++) {
    TEST_ASSERT_EQUAL(ut_params->cop[i]->type, RTE_CRYPTO_OP_TYPE_SYMMETRIC,
                      "%s: invalid crypto op type for %u-th packet\n", __func__,
                      i);
    TEST_ASSERT_EQUAL(
        ut_params->cop[i]->status, RTE_CRYPTO_OP_STATUS_NOT_PROCESSED,
        "%s: invalid crypto op status for %u-th packet\n", __func__, i);
    TEST_ASSERT_EQUAL(
        ut_params->cop[i]->sess_type, RTE_CRYPTO_OP_SECURITY_SESSION,
        "%s: invalid crypto op sess_type for %u-th packet\n", __func__, i);
    TEST_ASSERT_EQUAL(ut_params->cop[i]->sym->m_src, ut_params->ibuf[i],
...
static int crypto_ipsec_2sa(void) {
  ...
  for (i = 0; i < BURST_SIZE; i++) {
    r = i % 2;
    /* call crypto prepare */
    k = rte_ipsec_pkt_crypto_prepare(&ut_params->ss[r], ut_params->ibuf + i,
                                     ut_params->cop + i, 1);
    if (k != 1) {
      RTE_LOG(ERR, USER1, "rte_ipsec_pkt_crypto_prepare fail\n");
      return TEST_FAILED;
    }
    k = rte_cryptodev_enqueue_burst(ts_params->valid_dev, 0, ut_params->cop + i,
                                    1);
    if (k != 1) {
      RTE_LOG(ERR, USER1, "rte_cryptodev_enqueue_burst fail\n");
      return TEST_FAILED;
...
static uint32_t crypto_ipsec_4grp(uint32_t pkt_num) {
  uint32_t sa_ind;
...
static uint32_t crypto_ipsec_4grp_check_mbufs(uint32_t grp_ind,
...
static uint32_t crypto_ipsec_4grp_check_cnt(uint32_t grp_ind,
                                            struct rte_ipsec_group *grp) {
  uint32_t rc = 0;
...
static int crypto_ipsec_2sa_4grp(void) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  struct rte_ipsec_group grp[BURST_SIZE];
  uint32_t k, ng, i, j;
  ...
  for (i = 0; i < BURST_SIZE; i++) {
    ...
    /* call crypto prepare */
    k = rte_ipsec_pkt_crypto_prepare(&ut_params->ss[j], ut_params->ibuf + i,
                                     ut_params->cop + i, 1);
    if (k != 1) {
      RTE_LOG(ERR, USER1, "rte_ipsec_pkt_crypto_prepare fail\n");
      return TEST_FAILED;
  ...
  for (i = 0; i < ng; i++) {
    ...
    if (rc != 0) {
      RTE_LOG(ERR, USER1, "crypto_ipsec_4grp_check_cnt fail\n");
      return TEST_FAILED;
    }
    rc = crypto_ipsec_4grp_check_mbufs(i, grp);
    if (rc != 0) {
      RTE_LOG(ERR, USER1, "crypto_ipsec_4grp_check_mbufs fail\n");
      return TEST_FAILED;
    }
  }
  return TEST_SUCCESS;
...
static void test_ipsec_reorder_inb_pkt_burst(uint16_t num_pkts) {
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_crypto_op_alloc(uint16_t num_pkts) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  int rc = 0;
  uint16_t j;
  ...
  for (j = 0; j < num_pkts && rc == 0; j++) {
    ut_params->cop[j] =
        rte_crypto_op_alloc(ts_params->cop_mpool, RTE_CRYPTO_OP_TYPE_SYMMETRIC);
    if (ut_params->cop[j] == NULL) {
      RTE_LOG(ERR, USER1, "Failed to allocate symmetric crypto op\n");
      rc = TEST_FAILED;
    }
  }
...
static void test_ipsec_dump_buffers(struct ipsec_unitest_params *ut_params,
                                    int i) {
  ...
  if (ut_params->ibuf[j]) {
    printf("ibuf[%u] data:\n", j);
    rte_pktmbuf_dump(stdout, ut_params->ibuf[j], ut_params->ibuf[j]->data_len);
  }
  if (ut_params->obuf[j]) {
    printf("obuf[%u] data:\n", j);
    rte_pktmbuf_dump(stdout, ut_params->obuf[j], ut_params->obuf[j]->data_len);
  }
...
static void destroy_dummy_sec_session(struct ipsec_unitest_params *ut,
                                      uint32_t j) {
  rte_security_session_destroy(&dummy_sec_ctx, ut->ss[j].security.ses);
  ut->ss[j].security.ctx = NULL;
}

static void destroy_crypto_session(struct ipsec_unitest_params *ut,
...
static void destroy_session(struct ipsec_unitest_params *ut, uint8_t crypto_dev,
                            uint32_t j) {
  if (ut->ss[j].type == RTE_SECURITY_ACTION_TYPE_NONE)
...
static void destroy_sa(uint32_t j) {
  struct ipsec_unitest_params *ut = &unittest_params;
  struct ipsec_testsuite_params *ts = &testsuite_params;
...
crypto_inb_burst_null_null_check(struct ipsec_unitest_params *ut_params, int i,
  ...
  for (j = 0; j < num_pkts && num_pkts <= BURST_SIZE; j++) {
    ...
    TEST_ASSERT_BUFFERS_ARE_EQUAL(
        null_plain_data, rte_pktmbuf_mtod(ut_params->obuf[j], void *),
        test_cfg[i].pkt_sz, "input and output data does not match\n");
    TEST_ASSERT_EQUAL(ut_params->obuf[j]->data_len, ut_params->obuf[j]->pkt_len,
                      "data_len is not equal to pkt_len");
    TEST_ASSERT_EQUAL(ut_params->obuf[j]->data_len, test_cfg[i].pkt_sz,
...
static int test_ipsec_crypto_inb_burst_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  ...
  /* create rte_ipsec_sa */
  rc = create_sa(RTE_SECURITY_ACTION_TYPE_NONE, test_cfg[i].replay_win_sz,
                 test_cfg[i].flags, 0);
  if (rc != 0) {
  ...
  for (j = 0; j < num_pkts && rc == 0; j++) {
    ...
    ut_params->ibuf[j] =
        setup_test_string_tunneled(ts_params->mbuf_pool, null_encrypted_data,
  ...
  if (rc == 0) {
    ...
    if (rc == 0)
      rc = crypto_inb_burst_null_null_check(ut_params, i, num_pkts);
    else {
...
static int test_ipsec_crypto_inb_burst_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_crypto_outb_burst_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint16_t num_pkts = test_cfg[i].num_pkts;
  uint16_t j;
  ...
  for (j = 0; j < num_pkts && rc == 0; j++) {
    ut_params->ibuf[j] = setup_test_string(
        ts_params->mbuf_pool, null_plain_data, test_cfg[i].pkt_sz, 0);
    if (ut_params->ibuf[j] == NULL)
      rc = TEST_FAILED;
    else {
      /* Generate test mbuf data */
      /* packet with sequence number 0 is invalid */
      ut_params->testbuf[j] =
          setup_test_string_tunneled(ts_params->mbuf_pool, null_plain_data,
                                     test_cfg[i].pkt_sz, OUTBOUND_SPI, j + 1);
  ...
  if (rc == 0) {
    ...
    rc = crypto_ipsec(num_pkts);
    if (rc == 0)
      rc = crypto_outb_burst_null_null_check(ut_params, num_pkts);
    else
      RTE_LOG(ERR, USER1, "crypto_ipsec failed, cfg %d\n", i);
  }
...
static int test_ipsec_crypto_outb_burst_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
inline_inb_burst_null_null_check(struct ipsec_unitest_params *ut_params, int i,
  ...
  for (j = 0; j < num_pkts && num_pkts <= BURST_SIZE; j++) {
    ...
    TEST_ASSERT_EQUAL(ut_params->ibuf[j]->pkt_len, ut_params->obuf[j]->pkt_len,
                      "ibuf pkt_len is not equal to obuf pkt_len");
    TEST_ASSERT_EQUAL(ut_params->ibuf[j]->data_len, test_cfg[i].pkt_sz,
                      "data_len is not equal input data");
  }
  return 0;
}

static int test_ipsec_inline_crypto_inb_burst_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint16_t num_pkts = test_cfg[i].num_pkts;
  uint16_t j;
  ...
  if (rc == 0) {
    n = rte_ipsec_pkt_process(&ut_params->ss[0], ut_params->ibuf, num_pkts);
    if (n == num_pkts)
      rc = inline_inb_burst_null_null_check(ut_params, i, num_pkts);
    else {
      RTE_LOG(ERR, USER1, "rte_ipsec_pkt_process failed, cfg %d\n", i);
      rc = TEST_FAILED;
    }
  }
...
static int test_ipsec_inline_crypto_inb_burst_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_inline_proto_inb_burst_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint16_t num_pkts = test_cfg[i].num_pkts;
  uint16_t j;
...
static int test_ipsec_inline_proto_inb_burst_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_inline_crypto_outb_burst_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint16_t num_pkts = test_cfg[i].num_pkts;
  uint16_t j;
  ...
  if (rc == 0) {
    n = rte_ipsec_pkt_process(&ut_params->ss[0], ut_params->ibuf, num_pkts);
    if (n == num_pkts)
      rc = inline_outb_burst_null_null_check(ut_params, num_pkts);
    else {
      RTE_LOG(ERR, USER1, "rte_ipsec_pkt_process failed, cfg %d\n", i);
      rc = TEST_FAILED;
    }
...
static int test_ipsec_inline_crypto_outb_burst_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_inline_proto_outb_burst_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint16_t num_pkts = test_cfg[i].num_pkts;
  uint16_t j;
...
static int test_ipsec_inline_proto_outb_burst_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_lksd_proto_inb_burst_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint16_t num_pkts = test_cfg[i].num_pkts;
  uint16_t j;
  ...
  for (j = 0; j < num_pkts && rc == 0; j++) {
    /* packet with sequence number 0 is invalid */
    ut_params->ibuf[j] = setup_test_string(
        ts_params->mbuf_pool, null_encrypted_data, test_cfg[i].pkt_sz, 0);
    if (ut_params->ibuf[j] == NULL)
      rc = TEST_FAILED;
  }
...
static int test_ipsec_lksd_proto_inb_burst_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_lksd_proto_outb_burst_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int replay_inb_null_null_check(struct ipsec_unitest_params *ut_params,
                                      int i, int num_pkts) {
  uint16_t j;
...
static int test_ipsec_replay_inb_inside_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  int rc;
  ...
  if ((rc == 0) && (test_cfg[i].replay_win_sz != 0)) {
    ...
    ut_params->ibuf[0] = setup_test_string_tunneled(
        ts_params->mbuf_pool, null_encrypted_data, test_cfg[i].pkt_sz,
        INBOUND_SPI, test_cfg[i].replay_win_sz);
    if (ut_params->ibuf[0] == NULL)
      rc = TEST_FAILED;
    else
      rc = test_ipsec_crypto_op_alloc(1);
...
static int test_ipsec_replay_inb_inside_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_replay_inb_outside_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  int rc;
  ...
  ut_params->ibuf[0] = setup_test_string_tunneled(
      ts_params->mbuf_pool, null_encrypted_data, test_cfg[i].pkt_sz,
      INBOUND_SPI, test_cfg[i].replay_win_sz + 2);
  if (ut_params->ibuf[0] == NULL)
    rc = TEST_FAILED;
  else
    rc = test_ipsec_crypto_op_alloc(1);
  ...
  if ((rc == 0) && (test_cfg[i].replay_win_sz != 0)) {
    ...
    if (rc == 0) {
      ...
      if (rc == 0) {
        if (test_cfg[i].esn == 0) {
          RTE_LOG(ERR, USER1,
                  "packet is not outside the replay window, cfg %d pkt0_seq %u "
                  "pkt1_seq %u\n",
        ...
        RTE_LOG(ERR, USER1,
                "packet is outside the replay window, cfg %d pkt0_seq %u "
                "pkt1_seq %u\n",
                i, test_cfg[i].replay_win_sz + 2, 1);
        rc = 0;
      }
...
static int test_ipsec_replay_inb_outside_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_replay_inb_repeat_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  int rc;
...
static int test_ipsec_replay_inb_repeat_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_replay_inb_inside_burst_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint16_t num_pkts = test_cfg[i].num_pkts;
  int rc;
  ...
  if ((rc == 0) && (test_cfg[i].replay_win_sz != 0)) {
    ...
    if (rc == 0) {
      ...
      rc = crypto_ipsec(num_pkts);
      if (rc == 0)
        rc = replay_inb_null_null_check(ut_params, i, num_pkts);
      else {
        RTE_LOG(ERR, USER1, "crypto_ipsec failed\n");
        rc = TEST_FAILED;
      }
...
static int test_ipsec_replay_inb_inside_burst_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_crypto_inb_burst_2sa_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint16_t num_pkts = test_cfg[i].num_pkts;
  uint16_t j, r;
  ...
  if (rc == 0) {
    ...
    rc = crypto_ipsec_2sa();
    if (rc == 0)
      rc = crypto_inb_burst_2sa_null_null_check(ut_params, i);
    else {
      RTE_LOG(ERR, USER1, "crypto_ipsec failed, cfg %d\n", i);
      rc = TEST_FAILED;
    }
...
static int test_ipsec_crypto_inb_burst_2sa_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec_crypto_inb_burst_2sa_4grp_null_null(int i) {
  struct ipsec_testsuite_params *ts_params = &testsuite_params;
  struct ipsec_unitest_params *ut_params = &unittest_params;
  uint16_t num_pkts = test_cfg[i].num_pkts;
  uint16_t j, k;
...
static int test_ipsec_crypto_inb_burst_2sa_4grp_null_null_wrapper(void) {
  int i;
  int rc = 0;
  struct ipsec_unitest_params *ut_params = &unittest_params;
...
static int test_ipsec(void) { return unit_test_suite_runner(&ipsec_testsuite); }
