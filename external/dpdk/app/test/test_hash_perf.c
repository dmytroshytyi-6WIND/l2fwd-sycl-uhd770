...
static int create_table(unsigned int with_data, unsigned int table_index,
  ...
  if (with_locks)
    ut_params.extra_flag = RTE_HASH_EXTRA_FLAGS_TRANS_MEM_SUPPORT |
...
static void shuffle_input_keys(unsigned int table_index, unsigned int ext) {
  unsigned i;
  uint32_t swap_idx;
 ...
 */
static int get_input_keys(unsigned int with_pushes, unsigned int table_index,
                          unsigned int ext) {
  unsigned i, j;
  ...
  for (i = 0; i < keys_to_add;) {
    ...
    ret = rte_hash_add_key_with_hash(h[table_index], keys[i], signatures[i]);
    if (ret >= 0) {
...
static int timed_adds(unsigned int with_hash, unsigned int with_data,
  ...
  for (i = 0; i < keys_to_add; i++) {
    ...
    if (with_hash && with_data) {
      ret = rte_hash_add_key_with_hash_data(
          h[table_index], (const void *)keys[i], signatures[i], data);
      if (ret < 0) {
        printf("H+D: Failed to add key number %u\n", i);
        return -1;
      }
    } else if (with_hash && !with_data) {
      ret = rte_hash_add_key_with_hash(h[table_index], (const void *)keys[i],
                                       signatures[i]);
      if (ret >= 0)
        positions[i] = ret;
      else {
      ...
      ret = rte_hash_add_key_data(h[table_index], (const void *)keys[i], data);
      if (ret < 0) {
...
static int timed_lookups(unsigned int with_hash, unsigned int with_data,
                         unsigned int table_index, unsigned int ext) {
  unsigned i, j;
  const uint64_t start_tsc = rte_rdtsc();
  void *ret_data;
    ...
    for (j = 0; j < keys_to_add; j++) {
      if (with_hash && with_data) {
        ret = rte_hash_lookup_with_hash_data(
            h[table_index], (const void *)keys[j], signatures[j], &ret_data);
        if (ret < 0) {
        ...
        ret = rte_hash_lookup_with_hash(h[table_index], (const void *)keys[j],
                                        signatures[j]);
        ...
        ret = rte_hash_lookup_data(h[table_index], (const void *)keys[j],
                                   &ret_data);
        if (ret < 0) {
          printf("Key number %u was not found\n", j);
          return -1;
...
static int timed_lookups_multi(unsigned int with_hash, unsigned int with_data,
                               unsigned int table_index, unsigned int ext) {
  unsigned i, j, k;
  ...
  for (i = 0; i < num_lookups / keys_to_add; i++) {
    for (j = 0; j < keys_to_add / BURST_SIZE; j++) {
      ...
      if (!with_hash && with_data) {
        ret =
            rte_hash_lookup_bulk_data(h[table_index], (const void **)keys_burst,
        ...
        for (k = 0; k < BURST_SIZE; k++) {
          ...
          expected_data[k] =
              (void *)((uintptr_t)signatures[j * BURST_SIZE + k]);
        ...
        ret = rte_hash_lookup_with_hash_bulk_data(
            h[table_index], (const void **)keys_burst,
            &signatures[j * BURST_SIZE], BURST_SIZE, &hit_mask, ret_data);
        if (ret != BURST_SIZE) {
          printf("Expect to find %u keys,"
                 " but found %d\n",
                 BURST_SIZE, ret);
        ...
        ret = rte_hash_lookup_with_hash_bulk(
            h[table_index], (const void **)keys_burst,
            &signatures[j * BURST_SIZE], BURST_SIZE, positions_burst);
        for (k = 0; k < BURST_SIZE; k++) {
        ...
        rte_hash_lookup_bulk(h[table_index], (const void **)keys_burst,
...
static int timed_deletes(unsigned int with_hash, unsigned int with_data,
                         unsigned int table_index, unsigned int ext) {
  unsigned i;
  const uint64_t start_tsc = rte_rdtsc();
  int32_t ret;
  ...
  for (i = 0; i < keys_to_add; i++) {
    ...
    if (with_hash)
      ret = rte_hash_del_key_with_hash(h[table_index], (const void *)keys[i],
                                       signatures[i]);
    else
      ret = rte_hash_del_key(h[table_index], (const void *)keys[i]);
    if (ret >= 0)
      positions[i] = ret;
    else {
...
static void free_table(unsigned table_index) { rte_hash_free(h[table_index]); }

static void reset_table(unsigned table_index) {
  rte_hash_reset(h[table_index]);
}

static int run_all_tbl_perf_tests(unsigned int with_pushes,
                                  unsigned int with_locks, unsigned int ext) {
...
static int fbk_hash_perf_test(void) {
...
static int test_hash_perf(void) {
