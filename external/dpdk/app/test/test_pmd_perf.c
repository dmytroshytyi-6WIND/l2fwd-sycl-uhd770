...
#define NB_MBUF                                                                \
  RTE_MAX((unsigned)(nb_ports * nb_rx_queue * nb_rxd +                         \
...
static int init_traffic(struct rte_mempool *mp, struct rte_mbuf **pkts_burst,
                        uint32_t burst_size) {
  ...
  pktlen = initialize_ipv4_header(&pkt_ipv4_hdr, IPV4_ADDR(10, 0, 0, 1),
...
static int init_lcores(void) {
  unsigned lcore_id;

  for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
    lcore_conf[lcore_id].socketid = rte_lcore_to_socket_id(lcore_id);
...
static int init_mbufpool(unsigned nb_mbuf) {
  int socketid;
  unsigned lcore_id;
  char s[64];
...
static uint16_t alloc_lcore(uint16_t socketid) {
  unsigned lcore_id;
...
static void stats_display(uint16_t port_id) {
...
uint64_t (*do_measure)(struct lcore_conf *conf, struct rte_mbuf *pkts_burst[],
                       uint64_t total_pkts);

static uint64_t measure_rxtx(struct lcore_conf *conf,
...
static uint64_t measure_rxonly(struct lcore_conf *conf,
                               struct rte_mbuf *pkts_burst[],
                               uint64_t total_pkts) {
  unsigned i, portid, nb_rx, nb_tx;
...
static uint64_t measure_txonly(struct lcore_conf *conf,
                               struct rte_mbuf *pkts_burst[],
                               uint64_t total_pkts) {
  unsigned i, portid, nb_rx, nb_tx;
  uint64_t diff_tsc, cur_tsc;
...
static int main_loop(__rte_unused void *args) {
...
static inline int poll_burst(void *args) {
#define MAX_IDLE (10000)
  ...
  pkts_burst = (struct rte_mbuf **)rte_calloc_socket(
      "poll_burst", total, sizeof(void *), RTE_CACHE_LINE_SIZE, conf->socketid);
  if (!pkts_burst)
    return -1;
  ...
  while (total) {
    for (i = 0; i < conf->nb_ports; i++) {
      portid = conf->portlist[i];
      nb_rx = rte_eth_rx_burst(portid, 0, &pkts_burst[next[portid]],
...
static int exec_burst(uint32_t flags, int lcore) {
   ...
   */
  rte_eal_remote_launch(poll_burst, (void *)&pkt_per_port, lcore);
...
static int test_pmd_perf(void) {
  ...
  if (tx_burst == NULL) {
    tx_burst = (struct rte_mbuf **)rte_calloc_socket(
        "tx_buff", MAX_TRAFFIC_BURST * nb_ports, sizeof(void *),
        RTE_CACHE_LINE_SIZE, socketid);
    if (!tx_burst)
      return -1;
  }

  init_traffic(mbufpool[socketid], tx_burst, MAX_TRAFFIC_BURST * nb_ports);
  ...
  if (sc_flag == SC_CONTINUOUS) {
    ...
    if (rte_eal_wait_lcore(worker_id) < 0)
      return -1;
  } else if (sc_flag == SC_BURST_POLL_FIRST || sc_flag == SC_BURST_XMIT_FIRST)
