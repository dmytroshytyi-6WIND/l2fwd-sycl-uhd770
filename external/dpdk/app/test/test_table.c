...
static void app_free_resources(void) {
  int i;
  for (i = 0; i < N_PORTS; i++)
...
static void app_init_mbuf_pools(void) {
  ...
  if (!pool) {
    pool = rte_pktmbuf_pool_create("mempool", POOL_SIZE, POOL_CACHE_SIZE, 0,
                                   POOL_BUFFER_SIZE, 0);
    if (pool == NULL)
...
static void app_init_rings(void) {
  ...
  for (i = 0; i < N_PORTS; i++) {
    ...
    if (rings_rx[i] == NULL) {
      rings_rx[i] =
          rte_ring_create(name, RING_RX_SIZE, 0, RING_F_SP_ENQ | RING_F_SC_DEQ);
    }
    if (rings_rx[i] == NULL)
  ...
  for (i = 0; i < N_PORTS; i++) {
    ...
    if (rings_tx[i] == NULL) {
      rings_tx[i] =
          rte_ring_create(name, RING_TX_SIZE, 0, RING_F_SP_ENQ | RING_F_SC_DEQ);
    }
    if (rings_tx[i] == NULL)
