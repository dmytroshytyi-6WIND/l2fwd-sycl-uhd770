...
static int test_ticketlock_per_core(__rte_unused void *arg) {
...
static int test_ticketlock_recursive_per_core(__rte_unused void *arg) {
...
static int load_loop_fn(void *func_param) {
...
static int test_ticketlock_perf(void) {
  unsigned int i;
  uint64_t tcount = 0;
 ...
 */
static int test_ticketlock_try(__rte_unused void *arg) {
...
static int test_ticketlock(void) {
  ...
  RTE_LCORE_FOREACH_WORKER(i) {
    rte_eal_remote_launch(test_ticketlock_recursive_per_core, NULL, i);
  }
  rte_ticketlock_recursive_unlock(&tlr);
  rte_eal_mp_wait_lcore();
