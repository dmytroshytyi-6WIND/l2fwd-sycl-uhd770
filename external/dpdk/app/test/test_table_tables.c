...
table_test table_tests[] = {
    test_table_stub,        test_table_array,    test_table_lpm,
    test_table_lpm_ipv6,    test_table_hash_lru, test_table_hash_ext,
    test_table_hash_cuckoo,
};

#define PREPARE_PACKET(mbuf, value)                                            \
  do {                                                                         \
    uint32_t *k32, *signature;                                                 \
    uint8_t *key;                                                              \
    mbuf = rte_pktmbuf_alloc(pool);                                            \
    signature = RTE_MBUF_METADATA_UINT32_PTR(mbuf, APP_METADATA_OFFSET(0));    \
    key = RTE_MBUF_METADATA_UINT8_PTR(mbuf, APP_METADATA_OFFSET(32));          \
...
/* Function prototypes */
static int test_table_hash_lru_generic(struct rte_table_ops *ops,
                                       uint32_t key_size);
static int test_table_hash_ext_generic(struct rte_table_ops *ops,
                                       uint32_t key_size);
...
int test_table_stub(void) {
  ...
  expected_mask = 0;
  rte_table_stub_ops.f_lookup(table, mbufs, -1, &result_mask, (void **)entries);
...
int test_table_array(void) {
  ...
  struct rte_table_array_params array_params = {
      .n_entries = 7, .offset = APP_METADATA_OFFSET(1)};
  ...
  status = rte_table_array_ops.f_add(table, (void *)&array_key_1, &entry1,
                                     &key_found, &entry_ptr);
  if (status != 0)
    return -11;

  /* Traffic flow */
  status = rte_table_array_ops.f_add(table, (void *)&array_key_2, &entry2,
                                     &key_found, &entry_ptr);
  if (status != 0)
    return -12;
  ...
  rte_table_array_ops.f_lookup(table, mbufs, -1, &result_mask,
                               (void **)entries);

  for (i = 0; i < RTE_PORT_IN_BURST_SIZE_MAX; i++)
    if (i % 2 == 0 && *entries[i] != 'A')
      return -13;
    else if (i % 2 == 1 && *entries[i] != 'B')
      return -13;
...
int test_table_lpm(void) {
  ...
  /* Initialize params and create tables */
  struct rte_table_lpm_params lpm_params = {.name = "LPM",
                                            .n_rules = 1 << 24,
                                            .number_tbl8s = 1 << 8,
                                            .flags = 0,
  ...
  status =
      rte_table_lpm_ops.f_add(NULL, &lpm_key, &entry, &key_found, &entry_ptr);
  if (status == 0)
    return -10;
  ...
  status =
      rte_table_lpm_ops.f_add(table, &lpm_key, NULL, &key_found, &entry_ptr);
  if (status == 0)
    return -12;

  lpm_key.depth = 0;
  status =
      rte_table_lpm_ops.f_add(table, &lpm_key, &entry, &key_found, &entry_ptr);
  if (status == 0)
    return -13;
  ...
  rte_table_lpm_ops.f_lookup(table, mbufs, -1, &result_mask, (void **)entries);
  if (result_mask != expected_mask)
    return -23;
...
int test_table_lpm_ipv6(void) {
  ...
  status = rte_table_lpm_ipv6_ops.f_add(NULL, &lpm_key, &entry, &key_found,
                                        &entry_ptr);
  if (status == 0)
    return -11;

  status =
      rte_table_lpm_ipv6_ops.f_add(table, NULL, &entry, &key_found, &entry_ptr);
  if (status == 0)
    return -12;
  ...
  lpm_key.depth = 0;
  status = rte_table_lpm_ipv6_ops.f_add(table, &lpm_key, &entry, &key_found,
                                        &entry_ptr);
  if (status == 0)
    return -14;
  ...
  rte_table_lpm_ipv6_ops.f_lookup(table, mbufs, -1, &result_mask,
                                  (void **)entries);
  if (result_mask != expected_mask)
    return -24;
...
int test_table_hash_lru(void) {
  ...
  status = test_table_hash_lru_generic(&rte_table_hash_key8_lru_ops, 8);
  if (status < 0)
    return status;

  status = test_table_hash_lru_generic(&rte_table_hash_key16_lru_ops, 16);
  if (status < 0)
    return status;

  status = test_table_hash_lru_generic(&rte_table_hash_key32_lru_ops, 32);
  if (status < 0)
    return status;
...
int test_table_hash_cuckoo(void) {
  ...
  table = rte_table_hash_cuckoo_ops.f_create(&cuckoo_params, 0, entry_size);
  if (table != NULL)
    return -2;
  ...
  entry = 'A';
  status = rte_table_hash_cuckoo_ops.f_add(NULL, &key_cuckoo, &entry,
                                           &key_found, &entry_ptr);
  if (status == 0)
    return -10;

  status = rte_table_hash_cuckoo_ops.f_add(table, NULL, &entry, &key_found,
                                           &entry_ptr);
  if (status == 0)
    return -11;

  status = rte_table_hash_cuckoo_ops.f_add(table, &key_cuckoo, NULL, &key_found,
                                           &entry_ptr);
  if (status == 0)
    return -12;

  status = rte_table_hash_cuckoo_ops.f_add(table, &key_cuckoo, &entry,
                                           &key_found, &entry_ptr);
  if (status != 0)
    return -13;
  ...
  /* Delete */
  status =
      rte_table_hash_cuckoo_ops.f_delete(NULL, &key_cuckoo, &key_found, NULL);
  if (status == 0)
    return -15;

  status = rte_table_hash_cuckoo_ops.f_delete(table, NULL, &key_found, NULL);
  if (status == 0)
    return -16;

  status =
      rte_table_hash_cuckoo_ops.f_delete(table, &key_cuckoo, &key_found, NULL);
  if (status != 0)
    return -17;
  ...
  rte_table_hash_cuckoo_ops.f_lookup(table, mbufs, -1, &result_mask,
                                     (void **)entries);
  if (result_mask != expected_mask)
    return -20;
