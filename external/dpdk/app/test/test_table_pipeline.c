...
static int table_action_stub_miss(struct rte_pipeline *p,
...
table_action_0x00(__rte_unused struct rte_pipeline *p,
                  __rte_unused struct rte_mbuf **pkts, uint64_t pkts_mask,
...
char pipeline_test_names[][64] = {"Stub",
                                  "LPM",
                                  "LPMv6",
                                  "8-bit LRU Hash",
                                  "16-bit LRU Hash",
...
static int cleanup_pipeline(void) {
...
static int setup_pipeline(int test_type) {
  int ret;
  int i;
  ...
  for (i = 0; i < N_PORTS; i++) {
    ...
    ret = rte_pipeline_table_default_entry_add(p, table_id[i], &default_entry,
                                               &default_entry_ptr);
    if (ret < 0) {
    ...
    if (connect_miss_action_to_table) {
      ...
      /* Add the default action for the table. */
      ret = rte_pipeline_table_default_entry_add(
          p, table_id[i + 2], &default_entry, &default_entry_ptr);
      if (ret < 0) {
...
static int test_pipeline_single_filter(int test_type, int expected_count) {
  int i;
  int j;
  int ret;
  int tx_count;
  ...
  if (ret != -EINVAL) {
    RTE_LOG(INFO, PIPELINE, "%s: No pipeline flush error NULL pipeline (%d)\n",
            __func__, ret);
    goto fail;
  }
  ...
  for (i = 0; i < N_PORTS; i++) {
    ...
    if (ret <= 0)
      ...
      for (j = 0; j < ret; j++) {
        mbuf = objs[j];
        rte_hexdump(stdout, "Object:", rte_pktmbuf_mtod(mbuf, char *),
                    mbuf->data_len);
