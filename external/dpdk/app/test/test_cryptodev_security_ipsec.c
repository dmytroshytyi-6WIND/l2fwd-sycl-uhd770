...
struct crypto_param_comb
    alg_list[RTE_DIM(aead_list) + (RTE_DIM(cipher_list) * RTE_DIM(auth_list))];

static bool is_valid_ipv4_pkt(const struct rte_ipv4_hdr *pkt) {
...
static bool is_valid_ipv6_pkt(const struct rte_ipv6_hdr *pkt) {
...
int test_ipsec_sec_caps_verify(struct rte_security_ipsec_xform *ipsec_xform,
  ...
  if (ipsec_xform->options.stats == 1 && sec_cap->ipsec.options.stats == 0) {
    if (!silent)
  ...
  if ((ipsec_xform->direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS) &&
    ...
    if (!silent)
      RTE_LOG(INFO, USER1, "Tunnel header verify is not supported\n");
    return -ENOTSUP;
  }
  ...
  if (ipsec_xform->options.l4_csum_enable == 1 &&
    ...
    if (!silent)
      RTE_LOG(INFO, USER1, "Inner L4 checksum is not supported\n");
    return -ENOTSUP;
  }
  ...
  while ((crypto_cap = &sec_cap->crypto_capabilities[j++])->op !=
    ...
    if (crypto_cap->op == RTE_CRYPTO_OP_TYPE_SYMMETRIC &&
      ...
      if (rte_cryptodev_sym_capability_check_aead(
              sym_cap, aead->aead.key.length, aead->aead.digest_length,
              aead->aead.aad_length, aead->aead.iv.length) == 0)
        return 0;
    }
  }
...
int test_ipsec_crypto_caps_cipher_verify(
  ...
  while ((cap = &sec_cap->crypto_capabilities[j++])->op !=
    ...
    if (cap->op == RTE_CRYPTO_OP_TYPE_SYMMETRIC &&
      ...
      sym_cap = &cap->sym;
      if (rte_cryptodev_sym_capability_check_cipher(
              sym_cap, cipher->cipher.key.length, cipher->cipher.iv.length) ==
          0)
        return 0;
    }
  }
...
static void test_ipsec_csum_init(void *ip, bool l3, bool l4) {
...
void test_ipsec_td_prepare(const struct crypto_param *param1,
                           ...
                           struct ipsec_test_data *td_array, int nb_td)

{
  ...
  for (i = 0; i < nb_td; i++) {
    ...
    if (param1->type == RTE_CRYPTO_SYM_XFORM_AEAD) {
      ...
      /* Copy template for packet & key fields */
      if (flags->ipv6)
        memcpy(td, &pkt_aes_128_cbc_hmac_sha256_v6, sizeof(*td));
      else
    ...
    if (flags->sa_expiry_pkts_soft)
      td->ipsec_xform.life.packets_soft_limit = IPSEC_TEST_PACKETS_MAX - 1;
    ...
    if (flags->transport) {
      ...
      td->ipsec_xform.mode = RTE_SECURITY_IPSEC_SA_MODE_TUNNEL;

      if (flags->tunnel_ipv6)
        td->ipsec_xform.tunnel.type = RTE_SECURITY_IPSEC_TUNNEL_IPV6;
      else
        td->ipsec_xform.tunnel.type = RTE_SECURITY_IPSEC_TUNNEL_IPV4;
    }
...
void test_ipsec_td_update(struct ipsec_test_data td_inb[],
                          const struct ipsec_test_data td_outb[], int nb_td,
                          const struct ipsec_test_flags *flags) {
  int i;
  ...
  for (i = 0; i < nb_td; i++) {
    ...
    td_inb[i].ipsec_xform.options.tunnel_hdr_verify = flags->tunnel_hdr_verify;
...
void test_ipsec_display_alg(const struct crypto_param *param1,
                            const struct crypto_param *param2) {
  if (param1->type == RTE_CRYPTO_SYM_XFORM_AEAD) {
    printf("\t%s [%d]", rte_crypto_aead_algorithm_strings[param1->alg.aead],
           param1->key_length * 8);
  } else {
    printf("\t%s", rte_crypto_cipher_algorithm_strings[param1->alg.cipher]);
    if (param1->alg.cipher != RTE_CRYPTO_CIPHER_NULL)
      printf(" [%d]", param1->key_length * 8);
    printf(" %s", rte_crypto_auth_algorithm_strings[param2->alg.auth]);
...
static int test_ipsec_iv_verify_push(struct rte_mbuf *m,
...
static int test_ipsec_l3_csum_verify(struct rte_mbuf *m) {
...
static int test_ipsec_l4_csum_verify(struct rte_mbuf *m) {
...
static int test_ipsec_ttl_or_hop_decrement_verify(void *received,
                                                  void *expected) {
...
static int test_ipsec_td_verify(struct rte_mbuf *m,
                                const struct ipsec_test_data *td, bool silent,
                                const struct ipsec_test_flags *flags) {
  ...
  if (td->ipsec_xform.direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS &&
      (flags->icv_corrupt || flags->sa_expiry_pkts_hard ||
       flags->tunnel_hdr_verify || td->ar_packet))
    return TEST_SUCCESS;
...
static int test_ipsec_res_d_prepare(struct rte_mbuf *m,
                                    const struct ipsec_test_data *td,
...
static int test_ipsec_iph4_hdr_validate(const struct rte_ipv4_hdr *iph4,
                                        const struct ipsec_test_flags *flags) {
  uint8_t tos, dscp;
  uint16_t f_off;
...
static int test_ipsec_iph6_hdr_validate(const struct rte_ipv6_hdr *iph6,
                                        const struct ipsec_test_flags *flags) {
  uint32_t vtc_flow;
  uint8_t dscp;
  ...
  dscp = (vtc_flow & RTE_IPV6_HDR_DSCP_MASK) >> (RTE_IPV6_HDR_TC_SHIFT + 2);
...
int test_ipsec_post_process(struct rte_mbuf *m,
  ...
  if (td->ipsec_xform.direction == RTE_SECURITY_IPSEC_SA_DIR_EGRESS) {
    ...
    if (td->ipsec_xform.mode == RTE_SECURITY_IPSEC_SA_MODE_TRANSPORT) {
      if (flags->ipv6) {
        iph6 = (const struct rte_ipv6_hdr *)output_text;
...
int test_ipsec_status_check(const struct ipsec_test_data *td,
  ...
  if ((dir == RTE_SECURITY_IPSEC_SA_DIR_INGRESS) && td->ar_packet) {
  ...
  if (dir == RTE_SECURITY_IPSEC_SA_DIR_INGRESS && flags->sa_expiry_pkts_hard &&
  ...
  if ((dir == RTE_SECURITY_IPSEC_SA_DIR_INGRESS) && flags->tunnel_hdr_verify) {
    if (op->status != RTE_CRYPTO_OP_STATUS_ERROR) {
  ...
  if (flags->sa_expiry_pkts_soft && pkt_num == IPSEC_TEST_PACKETS_MAX) {
    if (!(op->aux_flags & RTE_CRYPTO_OP_AUX_FLAGS_IPSEC_SOFT_EXPIRY)) {
...
int test_ipsec_stats_verify(struct rte_security_ctx *ctx,
  ...
  if (flags->stats_success) {
    ...
    if (dir == RTE_SECURITY_IPSEC_SA_DIR_EGRESS) {
      if (stats.ipsec.opackets != 1 || stats.ipsec.oerrors != 0)
        ret = TEST_FAILED;
    } else {
      if (stats.ipsec.ipackets != 1 || stats.ipsec.ierrors != 0)
        ret = TEST_FAILED;
    }
  }
...
int test_ipsec_pkt_update(uint8_t *pkt, const struct ipsec_test_flags *flags) {
  ...
  if (flags->dscp == TEST_IPSEC_COPY_DSCP_INNER_1 ||
    ...
    if (is_ipv4(iph4)) {
      ...
      if (flags->dscp == TEST_IPSEC_COPY_DSCP_INNER_1 ||
          flags->dscp == TEST_IPSEC_SET_DSCP_0_INNER_1)
        tos |= (RTE_IPV4_HDR_DSCP_MASK & (TEST_IPSEC_DSCP_VAL << 2));
      else
