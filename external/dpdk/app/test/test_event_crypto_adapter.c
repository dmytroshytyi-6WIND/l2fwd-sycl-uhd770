...
static int test_event_crypto_adapter(void) {
...
#define MBUF_SIZE                                                              \
  (sizeof(struct rte_mbuf) + RTE_PKTMBUF_HEADROOM + PACKET_LENGTH)
#define IV_OFFSET                                                              \
...
static const uint8_t text_64B[] = {
    0x05, 0x15, 0x77, 0x32, 0xc9, 0x66, 0x91, 0x50, 0x93, 0x9f, 0xbb,
    0x4e, 0x2e, 0x5a, 0x02, 0xd0, 0x2d, 0x9d, 0x31, 0x5d, 0xc8, 0x9e,
    0x86, 0x36, 0x54, 0x5c, 0x50, 0xe8, 0x75, 0x54, 0x74, 0x5e, 0xd5,
    0xa2, 0x84, 0x21, 0x2d, 0xc5, 0xf8, 0x1c, 0x55, 0x1a, 0xba, 0x91,
    0xce, 0xb5, 0xa3, 0x1e, 0x31, 0xbf, 0xe9, 0xa1, 0x97, 0x5c, 0x2b,
    0xd6, 0x57, 0xa5, 0x9f, 0xab, 0xbd, 0xb0, 0x9b, 0x9c};
...
struct rte_event response_info = {.queue_id = TEST_APP_EV_QUEUE_ID,
...
static struct rte_mbuf *alloc_fill_mbuf(struct rte_mempool *mpool,
...
static int send_recv_ev(struct rte_event *ev) {
  struct rte_crypto_op *op;
  struct rte_event recv_ev;
  int ret;

  if (params.internal_port_op_fwd)
    ret = rte_event_crypto_adapter_enqueue(evdev, TEST_APP_PORT_ID, ev, NUM);
  else
  ...
  while (rte_event_dequeue_burst(evdev, TEST_APP_PORT_ID, &recv_ev, NUM, 0) ==
         0)
    rte_pause();
...
static int test_crypto_adapter_stats(void) {
...
static int test_op_forward_mode(uint8_t session_less) {
  ...
  op = rte_crypto_op_alloc(params.op_mpool, RTE_CRYPTO_OP_TYPE_SYMMETRIC);
  TEST_ASSERT_NOT_NULL(
      op, "Failed to allocate symmetric crypto operation struct\n");
  ...
  if (!session_less) {
    ...
    ret = rte_cryptodev_sym_session_init(TEST_CDEV_ID, sess, &cipher_xform,
...
static int map_adapter_service_core(void) {
  ...
  if (rte_event_crypto_adapter_service_id_get(TEST_ADAPTER_ID,
    ...
    TEST_ASSERT_SUCCESS(
        rte_service_map_lcore_set(adapter_service_id, slcore_id, 1),
...
static int test_sessionless_with_op_forward_mode(void) {
  uint32_t cap;
  int ret;
...
static int test_session_with_op_forward_mode(void) {
  uint32_t cap;
  int ret;
...
static int send_op_recv_ev(struct rte_crypto_op *op) {
  struct rte_crypto_op *recv_op;
  struct rte_event ev;
  int ret;

  ret = rte_cryptodev_enqueue_burst(TEST_CDEV_ID, TEST_CDEV_QP_ID, &op, NUM);
  TEST_ASSERT_EQUAL(ret, NUM, "Failed to enqueue to cryptodev\n");
  memset(&ev, 0, sizeof(ev));

  while (rte_event_dequeue_burst(evdev, TEST_APP_PORT_ID, &ev, NUM, 0) == 0)
    rte_pause();
...
static int test_op_new_mode(uint8_t session_less) {
  struct rte_crypto_sym_xform cipher_xform;
  struct rte_cryptodev_sym_session *sess;
  union rte_event_crypto_metadata m_data;
  struct rte_crypto_sym_op *sym_op;
...
static int test_sessionless_with_op_new_mode(void) {
  uint32_t cap;
  int ret;
...
static int test_session_with_op_new_mode(void) {
  uint32_t cap;
  int ret;
...
static int configure_cryptodev(void) {
  ...
  params.mbuf_pool =
      rte_pktmbuf_pool_create("CRYPTO_ADAPTER_MBUFPOOL", NUM_MBUFS,
  ...
  params.op_mpool = rte_crypto_op_pool_create(
      "EVENT_CRYPTO_SYM_OP_POOL", RTE_CRYPTO_OP_TYPE_SYMMETRIC, NUM_MBUFS,
      MBUF_CACHE_SIZE,
      DEFAULT_NUM_XFORMS * sizeof(struct rte_crypto_sym_xform) +
          MAXIMUM_IV_LENGTH + sizeof(union rte_event_crypto_metadata),
      rte_socket_id());
  ...
  params.session_mpool = rte_cryptodev_sym_session_pool_create(
      "CRYPTO_ADAPTER_SESSION_MP", MAX_NB_SESSIONS, 0, 0,
      sizeof(union rte_event_crypto_metadata), SOCKET_ID_ANY);
  TEST_ASSERT_NOT_NULL(params.session_mpool,
                       "session mempool allocation failed\n");

  params.session_priv_mpool = rte_mempool_create(
      "CRYPTO_AD_SESS_MP_PRIV", MAX_NB_SESSIONS, session_size, 0, 0, NULL, NULL,
  ...
  TEST_ASSERT_SUCCESS(
      rte_cryptodev_queue_pair_setup(TEST_CDEV_ID, TEST_CDEV_QP_ID, &qp_conf,
...
static inline void evdev_set_conf_values(struct rte_event_dev_config *dev_conf,
...
static int configure_eventdev(void) {
...
static void test_crypto_adapter_free(void) {
  rte_event_crypto_adapter_free(TEST_ADAPTER_ID);
}

static int test_crypto_adapter_create(void) {
  ...
  ret = rte_event_crypto_adapter_create(TEST_ADAPTER_ID, evdev, &conf, 0);
  TEST_ASSERT_SUCCESS(ret, "Failed to create event crypto adapter\n");

  return TEST_SUCCESS;
}

static int test_crypto_adapter_qp_add_del(void) {
  uint32_t cap;
  int ret;
  ...
  if (cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_QP_EV_BIND) {
    ret = rte_event_crypto_adapter_queue_pair_add(
        TEST_ADAPTER_ID, TEST_CDEV_ID, TEST_CDEV_QP_ID, &response_info);
  } else
    ret = rte_event_crypto_adapter_queue_pair_add(TEST_ADAPTER_ID, TEST_CDEV_ID,
                                                  TEST_CDEV_QP_ID, NULL);

  TEST_ASSERT_SUCCESS(ret, "Failed to create add queue pair\n");

  ret = rte_event_crypto_adapter_queue_pair_del(TEST_ADAPTER_ID, TEST_CDEV_ID,
                                                TEST_CDEV_QP_ID);
...
adapter_create:
  /* Create adapter with default port creation callback */
  ret = rte_event_crypto_adapter_create(TEST_ADAPTER_ID, evdev, &conf, mode);
  TEST_ASSERT_SUCCESS(ret, "Failed to create event crypto adapter\n");
...
static void test_crypto_adapter_stop(void) {
...
static int test_crypto_adapter_conf(enum rte_event_crypto_adapter_mode mode) {
  ...
  if (!crypto_adapter_setup_done) {
    ...
    if (!params.internal_port_op_fwd) {
      qid = TEST_CRYPTO_EV_QUEUE_ID;
      ret = rte_event_port_link(evdev, params.crypto_event_port_id, &qid, NULL,
                                1);
      TEST_ASSERT(ret >= 0,
                  "Failed to link queue %d "
                  "port=%u\n",
                  qid, params.crypto_event_port_id);
    }
  ...
  if (rte_event_dev_service_id_get(evdev, &evdev_service_id) == 0) {
    ...
    /* map services to it */
    TEST_ASSERT_SUCCESS(
        rte_service_map_lcore_set(evdev_service_id, slcore_id, 1),
...
static int test_crypto_adapter_conf_op_forward_mode(void) {
...
static int test_crypto_adapter_conf_op_new_mode(void) {
  enum rte_event_crypto_adapter_mode mode;
...
static int testsuite_setup(void) {
  ...
  TEST_ASSERT_NOT_EQUAL(slcore_id, RTE_MAX_LCORE,
                        "At least 2 lcores "
...
static void crypto_adapter_teardown(void) {
  int ret;
...
static void crypto_teardown(void) {
  /* Free mbuf mempool */
  if (params.mbuf_pool != NULL) {
...
static void eventdev_teardown(void) { rte_event_dev_stop(evdev); }
...
static struct unit_test_suite functional_testsuite = {
    ...
    .unit_test_cases = {
        ...
        TEST_CASE_ST(test_crypto_adapter_create, test_crypto_adapter_free,
        ...
        TEST_CASE_ST(test_crypto_adapter_conf_op_new_mode,
                     test_crypto_adapter_stop, test_session_with_op_new_mode),
...
REGISTER_TEST_COMMAND(event_crypto_adapter_autotest, test_event_crypto_adapter);
