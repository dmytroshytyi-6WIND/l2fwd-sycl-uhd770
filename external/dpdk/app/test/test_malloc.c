...
static int is_mem_on_socket(int32_t socket);

static int32_t addr_to_socket(void *addr);
...
static int is_memory_overlap(void *p1, size_t len1, void *p2, size_t len2) {
...
static int is_aligned(void *p, int align) {
...
static int test_align_overlap_per_lcore(__rte_unused void *arg) {
  const unsigned align1 = 8, align2 = 64, align3 = 2048;
  unsigned i, j;
...
static int test_reordered_free_per_lcore(__rte_unused void *arg) {
  const unsigned align1 = 8, align2 = 64, align3 = 2048;
  unsigned i, j;
  void *p1, *p2, *p3;
  int ret = 0;
...
static int test_str_to_size(void) {
  struct {
    const char *str;
    uint64_t value;
  } test_values[] = {
...
static int test_realloc(void) { return TEST_SKIPPED; }
#else

static int test_realloc_socket(int socket) {
  ...
  char *ptr1 = rte_zmalloc_socket(NULL, size1, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr1) {
    printf("NULL pointer returned from rte_zmalloc\n");
    return -1;
  }
  strlcpy(ptr1, hello_str, size1);
  char *ptr2 = rte_realloc_socket(ptr1, size2, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr2) {
    rte_free(ptr1);
   ...
   */
  char *ptr3 = rte_zmalloc_socket(NULL, size3, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr3) {
    printf("NULL pointer returned from rte_zmalloc\n");
    rte_free(ptr2);
    return -1;
  ...
  char *ptr4 = rte_realloc_socket(ptr3, size4, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr4) {
    printf("NULL pointer returned from rte_realloc\n");
    rte_free(ptr3);
    return -1;
  ...
  ptr4 = rte_realloc_socket(ptr3, size3 + size2 + size1, RTE_CACHE_LINE_SIZE,
                            socket);
  if (ptr3 != ptr4) {
  ...
  char *ptr5 = rte_malloc_socket(NULL, size5, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr5) {
    printf("NULL pointer returned from rte_malloc\n");
    return -1;
  }
  char *ptr6 = rte_realloc_socket(ptr5, size6, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr6) {
    printf("NULL pointer returned from rte_realloc\n");
    rte_free(ptr5);
    return -1;
  ...
  char *ptr9 = rte_malloc_socket(NULL, size9, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr9) {
    printf("NULL pointer returned from rte_malloc\n");
    return -1;
  }
  char *ptr10 = rte_malloc_socket(NULL, size10, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr10) {
    printf("NULL pointer returned from rte_malloc\n");
    return -1;
  }
  rte_free(ptr9);
  char *ptr11 = rte_realloc_socket(ptr10, size11, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr11) {
    printf("NULL pointer returned from rte_realloc\n");
    rte_free(ptr10);
    return -1;
  ...
  size_t size12_check;
  char *ptr12 = rte_realloc_socket(NULL, size12, RTE_CACHE_LINE_SIZE, socket);
  if (!ptr12) {
    printf("NULL pointer returned from rte_realloc\n");
    return -1;
  }
  if (rte_malloc_validate(ptr12, &size12_check) < 0 || size12_check != size12) {
    rte_free(ptr12);
    return -1;
  }
  rte_free(ptr12);
...
static int test_realloc_numa(void) {
  ...
  for (socket = 0; socket < RTE_MAX_NUMA_NODES; socket++) {
    if (is_mem_on_socket(socket)) {
      ...
      while (j--) {
        /* j == 1 -> resizing */
        ptr2 = rte_realloc_socket(ptr1, size, RTE_CACHE_LINE_SIZE, socket);
        if (ptr2 == NULL) {
...
static int test_realloc(void) {
  ...
  mem = mmap(NULL, mem_sz, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS,
             -1, 0);
  if (mem == MAP_FAILED) {
  ...
  if (rte_malloc_heap_memory_add(heap_name, mem, mem_sz, NULL, 0, page_sz) !=
      0) {
...
static int test_random_alloc_free(void *_ __rte_unused) {
  struct mem_list {
  ...
  for (i = 0; i < N; i++) {
    ...
    while (!free_mem) {
      const unsigned mem_size =
          sizeof(struct mem_list) + rte_rand() % (64 * 1024);
      const unsigned align = 1 << (rte_rand() % 12); /* up to 4k alignment */
      struct mem_list *entry = rte_malloc(NULL, mem_size, align);
      if (entry == NULL)
        return -1;
      ...
      if (rte_malloc_validate(entry, &allocated_size) == -1 ||
        ...
        return -1;
      memset(entry->data, rte_lcore_id(), mem_size - sizeof(*entry));
...
static int test_rte_malloc_validate(void) {
...
static int test_zero_aligned_alloc(void) {
...
static int test_malloc_bad_params(void) {
...
static int check_socket_mem(const struct rte_memseg_list *msl, void *arg) {
  int32_t *socket = arg;
...
static int test_alloc_single_socket(int32_t socket) {
  const char *type = NULL;
  const size_t size = 10;
  const unsigned align = 0;
  char *mem = NULL;
  int32_t desired_socket =
      (socket == SOCKET_ID_ANY) ? (int32_t)rte_socket_id() : socket;
...
static int test_alloc_socket(void) {
...
static int test_malloc(void) {
  ...
  if (ret < 0) {
    ...
    printf("test_align_overlap_per_lcore() passed\n");
  ...
  if (ret < 0) {
    ...
    printf("test_reordered_free_per_lcore() passed\n");
  ...
  if (ret < 0) {
    ...
    printf("test_random_alloc_free() passed\n");
  ...
  if (ret < 0) {
    ...
    printf("test_rte_malloc_validate() passed\n");
  ...
  if (ret < 0) {
    ...
    printf("test_alloc_socket() passed\n");
