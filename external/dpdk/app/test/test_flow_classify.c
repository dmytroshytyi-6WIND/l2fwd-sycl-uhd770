...
static int test_flow_classify(void) {
 ...
 */
static struct rte_flow_item_ipv4 ipv4_udp_spec_1 = {{{.version_ihl = 0},
                                                     0,
                                                     0,
                                                     0,
                                                     0,
...
static struct rte_flow_item_udp udp_spec_1 = {{32, 33, 0, 0}};

static struct rte_flow_item eth_item = {RTE_FLOW_ITEM_TYPE_ETH, 0, 0, 0};
static struct rte_flow_item eth_item_bad = {-1, 0, 0, 0};

static struct rte_flow_item ipv4_udp_item_1 = {
    RTE_FLOW_ITEM_TYPE_IPV4, &ipv4_udp_spec_1, 0, &ipv4_mask_24};
static struct rte_flow_item ipv4_udp_item_bad = {RTE_FLOW_ITEM_TYPE_IPV4, NULL,
                                                 0, NULL};

static struct rte_flow_item udp_item_1 = {RTE_FLOW_ITEM_TYPE_UDP, &udp_spec_1,
                                          0, &rte_flow_item_udp_mask};
static struct rte_flow_item udp_item_bad = {RTE_FLOW_ITEM_TYPE_UDP, NULL, 0,
                                            NULL};
 ...
 */
static struct rte_flow_item_ipv4 ipv4_tcp_spec_1 = {{{.version_ihl = 0},
                                                     0,
                                                     0,
                                                     0,
                                                     0,
...
static struct rte_flow_item_tcp tcp_spec_1 = {{16, 17, 0, 0, 0, 0, 0, 0, 0}};

static struct rte_flow_item ipv4_tcp_item_1 = {
    RTE_FLOW_ITEM_TYPE_IPV4, &ipv4_tcp_spec_1, 0, &ipv4_mask_24};

static struct rte_flow_item tcp_item_1 = {RTE_FLOW_ITEM_TYPE_TCP, &tcp_spec_1,
...
static struct rte_flow_item_ipv4 ipv4_sctp_spec_1 = {
    {{.version_ihl = 0},
     ...
     IPPROTO_SCTP,
     0,
     RTE_IPV4(11, 12, 13, 14),
     RTE_IPV4(15, 16, 17, 18)}};

static struct rte_flow_item_sctp sctp_spec_1 = {{10, 11, 0, 0}};

static struct rte_flow_item ipv4_sctp_item_1 = {
    RTE_FLOW_ITEM_TYPE_IPV4, &ipv4_sctp_spec_1, 0, &ipv4_mask_24};

static struct rte_flow_item sctp_item_1 = {
    RTE_FLOW_ITEM_TYPE_SCTP, &sctp_spec_1, 0, &rte_flow_item_sctp_mask};
...
static int test_invalid_parameters(void) {
  ...
  rule = rte_flow_classify_table_entry_add(NULL, NULL, NULL, NULL, NULL, NULL);
  if (rule) {
  ...
  if (!ret) {
    printf("Line %i: rte_flow_classify_table_entry_delete", __LINE__);
    printf(" with NULL param should have failed!\n");
    return -1;
  }
  ...
  rule =
      rte_flow_classify_table_entry_add(NULL, NULL, NULL, NULL, NULL, &error);
  if (rule) {
...
static int test_valid_parameters(void) {
  struct rte_flow_classify_rule *rule;
  int ret;
  int key_found;
  ...
  ret = rte_flow_classify_validate(cls->cls, &attr, pattern, actions, &error);
  if (ret) {
    printf("Line %i: rte_flow_classify_validate", __LINE__);
    printf(" should not have failed!\n");
    return -1;
  }
  rule = rte_flow_classify_table_entry_add(cls->cls, &attr, pattern, actions,
                                           &key_found, &error);
...
static int test_invalid_patterns(void) {
  struct rte_flow_classify_rule *rule;
  int ret;
  int key_found;
...
static int test_invalid_actions(void) {
  struct rte_flow_classify_rule *rule;
  int ret;
  int key_found;
...
static int init_ipv4_udp_traffic(struct rte_mempool *mp,
  ...
  initialize_eth_header(&pkt_eth_hdr, (struct rte_ether_addr *)src_mac,
                        (struct rte_ether_addr *)dst_mac, RTE_ETHER_TYPE_IPV4,
                        0, 0);
  pktlen = (uint16_t)(sizeof(struct rte_ether_hdr));
  printf("ETH  pktlen %u\n", pktlen);

  pktlen = initialize_ipv4_header(&pkt_ipv4_hdr, src_addr, dst_addr, pktlen);
  printf("ETH + IPv4 pktlen %u\n", pktlen);

  pktlen = initialize_udp_header(&pkt_udp_hdr, src_port, dst_port, pktlen);
  printf("ETH + IPv4 + UDP pktlen %u\n\n", pktlen);

  return generate_packet_burst(mp, pkts_burst, &pkt_eth_hdr, 0, &pkt_ipv4_hdr,
                               1, &pkt_udp_hdr, burst_size,
                               PACKET_BURST_GEN_PKT_LEN, 1);
}

static int init_ipv4_tcp_traffic(struct rte_mempool *mp,
                                 struct rte_mbuf **pkts_burst,
                                 uint32_t burst_size) {
  struct rte_ether_hdr pkt_eth_hdr;
  struct rte_ipv4_hdr pkt_ipv4_hdr;
  ...
  pktlen = initialize_ipv4_header_proto(&pkt_ipv4_hdr, src_addr, dst_addr,
                                        pktlen, IPPROTO_TCP);
  printf("ETH + IPv4 pktlen %u\n", pktlen);
  ...
  return generate_packet_burst_proto(
      mp, pkts_burst, &pkt_eth_hdr, 0, &pkt_ipv4_hdr, 1, IPPROTO_TCP,
      &pkt_tcp_hdr, burst_size, PACKET_BURST_GEN_PKT_LEN, 1);
}

static int init_ipv4_sctp_traffic(struct rte_mempool *mp,
                                  struct rte_mbuf **pkts_burst,
                                  uint32_t burst_size) {
  struct rte_ether_hdr pkt_eth_hdr;
  struct rte_ipv4_hdr pkt_ipv4_hdr;
  ...
  pktlen = initialize_sctp_header(&pkt_sctp_hdr, src_port, dst_port, pktlen);
  printf("ETH + IPv4 + SCTP pktlen %u\n\n", pktlen);

  return generate_packet_burst_proto(
      mp, pkts_burst, &pkt_eth_hdr, 0, &pkt_ipv4_hdr, 1, IPPROTO_SCTP,
      &pkt_sctp_hdr, burst_size, PACKET_BURST_GEN_PKT_LEN, 1);
}

static int init_mbufpool(void) {
  int socketid;
  int ret = 0;
  ...
  for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
    ...
    if (mbufpool[socketid] == NULL) {
      ...
      mbufpool[socketid] = rte_pktmbuf_pool_create(
          s, NB_MBUF, MEMPOOL_CACHE_SIZE, 0, MBUF_SIZE, socketid);
...
static int test_query_udp(void) {
  ...
  ret = rte_flow_classifier_query(cls->cls, bufs, MAX_PKT_BURST, rule,
                                  &udp_classify_stats);
  if (ret) {
    printf("Line %i: flow_classifier_query", __LINE__);
    printf(" should not have failed!\n");
...
static int test_query_tcp(void) {
  struct rte_flow_classify_rule *rule;
  int ret;
  int i;
  int key_found;
...
static int test_query_sctp(void) {
  struct rte_flow_classify_rule *rule;
  int ret;
  int i;
  int key_found;
