...
static void mbuf1_prepare(struct dummy_mbuf *dm, uint32_t plen) {
  struct {
...
static int test_setup(void) {
  ...
  mp = rte_pktmbuf_pool_create_by_ops("pcapng_test_pool", NUM_PACKETS, 0, 0,
...
static int test_write_packets(void) {
  ...
  for (i = 0; i < NUM_PACKETS; i++) {
    ...
    mc =
        rte_pcapng_copy(port_id, 0, orig, mp, pkt_len, rte_get_tsc_cycles(), 0);
    if (mc == NULL) {
...
static int test_write_stats(void) {
  ssize_t len;

  /* write a statistics block */
  len = rte_pcapng_write_stats(pcapng, port_id, NULL, 0, 0, NUM_PACKETS, 0);
  if (len <= 0) {
...
static void pkt_print(u_char *user, const struct pcap_pkthdr *h,
                      const u_char *bytes) {
  ...
  printf("%s.%06lu: %s -> %s type %x length %u\n", tbuf,
         (unsigned long)h->ts.tv_usec, src, dst,
         rte_be_to_cpu_16(eh->ether_type), h->len);
...
static struct unit_test_suite test_pcapng_suite = {
    .setup = test_setup,
    .teardown = test_cleanup,
    .suite_name = "Test Pcapng Unit Test Suite",
    .unit_test_cases = {TEST_CASE(test_write_packets),
                        TEST_CASE(test_write_stats), TEST_CASE(test_validate),
                        TEST_CASES_END()}};

static int test_pcapng(void) {
