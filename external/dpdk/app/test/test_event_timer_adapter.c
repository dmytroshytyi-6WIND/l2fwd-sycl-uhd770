...
static int test_event_timer_adapter_func(void) {
...
static inline int eventdev_setup(void) {
  int ret;
  ...
  TEST_ASSERT(info.max_num_events < 0 ||
              ...
              "ERROR max_num_events=%d < max_events=%d", info.max_num_events,
              MAX_TIMERS);
  ...
  if (rte_event_dev_service_id_get(evdev, &service_id) == 0) {
    ...
    TEST_ASSERT_SUCCESS(rte_service_lcore_start(sw_evdev_slcore),
                        "Failed to start service core");
    TEST_ASSERT_SUCCESS(
        rte_service_map_lcore_set(service_id, sw_evdev_slcore, 1),
...
static int testsuite_setup(void) {
  ...
  if (rte_lcore_count() < required_lcore_count) {
    printf("Not enough cores for event_timer_adapter_test, expecting at least "
           "%u\n",
...
static int setup_adapter_service(struct rte_event_timer_adapter *adptr) {
  uint32_t adapter_service_id;
  int ret;

  /* retrieve service ids */
  TEST_ASSERT_SUCCESS(
      rte_event_timer_adapter_service_id_get(adptr, &adapter_service_id),
      "Failed to get event timer "
      "adapter service id");
  /* add a service core and start it */
  ret = rte_service_lcore_add(sw_adptr_slcore);
  TEST_ASSERT(ret == 0 || ret == -EALREADY, "Failed to add service core");
  ret = rte_service_lcore_start(sw_adptr_slcore);
  TEST_ASSERT(ret == 0 || ret == -EALREADY, "Failed to start service core");

  /* map services to it */
  TEST_ASSERT_SUCCESS(
      rte_service_map_lcore_set(adapter_service_id, sw_adptr_slcore, 1),
...
static int test_port_conf_cb(uint16_t id, uint8_t event_dev_id,
  ...
  if (conf_arg != NULL)
    ...
    ret = rte_event_port_default_conf_get(event_dev_id, port_id, port_conf);
    if (ret < 0)
      return ret;
  }
...
static int _timdev_setup(uint64_t max_tmo_ns, uint64_t bkt_tck_ns,
                         uint64_t flags) {
  ...
  if (!(caps & RTE_EVENT_TIMER_ADAPTER_CAP_INTERNAL_PORT)) {
    timdev =
        rte_event_timer_adapter_create_ext(&config, test_port_conf_cb, NULL);
  ...
  eventdev_test_mempool =
      rte_mempool_create(pool_name, MAX_TIMERS * 2,
...
static int timdev_setup_usec(void) {
...
static int timdev_setup_usec_multicore(void) {
  uint64_t flags = RTE_EVENT_TIMER_ADAPTER_F_ADJUST_RES;
...
static int timdev_setup_msec(void) {
  uint64_t flags = RTE_EVENT_TIMER_ADAPTER_F_ADJUST_RES;
...
static int timdev_setup_msec_periodic(void) {
  uint64_t flags =
      RTE_EVENT_TIMER_ADAPTER_F_ADJUST_RES | RTE_EVENT_TIMER_ADAPTER_F_PERIODIC;

  /* Periodic mode with 100 ms resolution */
  return _timdev_setup(0, NSECPERSEC / 10, flags);
}

static int timdev_setup_sec(void) {
  uint64_t flags = RTE_EVENT_TIMER_ADAPTER_F_ADJUST_RES;
...
static int timdev_setup_sec_periodic(void) {
  uint64_t flags =
      RTE_EVENT_TIMER_ADAPTER_F_ADJUST_RES | RTE_EVENT_TIMER_ADAPTER_F_PERIODIC;
...
static int timdev_setup_sec_multicore(void) {
  uint64_t flags = RTE_EVENT_TIMER_ADAPTER_F_ADJUST_RES;
...
static void timdev_teardown(void) {
...
static inline int test_timer_state(void) {
  ...
  *ev_tim = tim;
  ...
  TEST_ASSERT_EQUAL(rte_event_timer_arm_burst(timdev, &ev_tim, 1), 1,
                    "Failed to arm timer with proper timeout.");
  TEST_ASSERT_EQUAL(rte_event_timer_cancel_burst(timdev, &ev_tim, 1), 1,
                    "Failed to cancel armed timer");
...
static inline int _arm_timers(uint64_t timeout_tcks, uint64_t timers) {
  uint64_t i;
  struct rte_event_timer *ev_tim;
  const struct rte_event_timer tim = {
      .ev.op = RTE_EVENT_OP_NEW,
  ...
  for (i = 0; i < timers; i++) {
    ...
    *ev_tim = tim;
    ...
    TEST_ASSERT_EQUAL(rte_event_timer_arm_burst(timdev, &ev_tim, 1), 1,
                      "Failed to arm timer %d", rte_errno);
  }

  return TEST_SUCCESS;
}

static inline int _wait_timer_triggers(uint64_t wait_sec, uint64_t arm_count,
  ...
  while (1) {
    if (rte_get_timer_cycles() - wait_start > max_wait) {
      if (events + cancel_count != arm_count)
        TEST_ASSERT_SUCCESS(max_wait, "Max time limit for timers exceeded.");
      break;
    }
...
static inline int test_timer_arm(void) {
  TEST_ASSERT_SUCCESS(_arm_timers(20, MAX_TIMERS), "Failed to arm timers");
  TEST_ASSERT_SUCCESS(_wait_timer_triggers(10, MAX_TIMERS, 0),
                      "Timer triggered count doesn't match arm count");
  return TEST_SUCCESS;
}

static inline int test_timer_arm_periodic(void) {
  TEST_ASSERT_SUCCESS(_arm_timers(1, MAX_TIMERS), "Failed to arm timers");
...
static int _arm_wrapper(void *arg) {
  RTE_SET_USED(arg);
...
static inline int test_timer_arm_multicore(void) {
...
#define MAX_BURST 16
static inline int _arm_timers_burst(uint64_t timeout_tcks, uint64_t timers) {
  uint64_t i;
  int j;
  ...
  for (i = 0; i < timers / MAX_BURST; i++) {
    TEST_ASSERT_SUCCESS(
        rte_mempool_get_bulk(eventdev_test_mempool, (void **)ev_tim, MAX_BURST),
        "mempool alloc failed");
    ...
    TEST_ASSERT_EQUAL(rte_event_timer_arm_tmo_tick_burst(
                          timdev, ev_tim, tim.timeout_ticks, MAX_BURST),
...
static inline int test_timer_arm_burst(void) {
...
static inline int test_timer_arm_burst_periodic(void) {
  TEST_ASSERT_SUCCESS(_arm_timers_burst(1, MAX_TIMERS), "Failed to arm timers");
  /* With a resolution of 100ms and wait time of 1sec,
   * there will be 10 * MAX_TIMERS periodic timer triggers.
   */
  TEST_ASSERT_SUCCESS(_wait_timer_triggers(1, 10 * MAX_TIMERS, 0),
...
static int _arm_wrapper_burst(void *arg) {
  RTE_SET_USED(arg);
...
static inline int test_timer_arm_burst_multicore(void) {
...
static inline int test_timer_cancel_periodic(void) {
  uint64_t i;
  struct rte_event_timer *ev_tim;
  const struct rte_event_timer tim = {
      .ev.op = RTE_EVENT_OP_NEW,
  ...
  TEST_ASSERT_SUCCESS(_wait_timer_triggers(30, MAX_TIMERS, MAX_TIMERS),
                      "Timer triggered count doesn't match arm, cancel count");

  return TEST_SUCCESS;
}

static inline int test_timer_cancel(void) {
  uint64_t i;
  struct rte_event_timer *ev_tim;
  const struct rte_event_timer tim = {
      .ev.op = RTE_EVENT_OP_NEW,
...
static int _cancel_producer(uint64_t timeout_tcks, uint64_t timers) {
  uint64_t i;
  struct rte_event_timer *ev_tim;
  const struct rte_event_timer tim = {
      .ev.op = RTE_EVENT_OP_NEW,
...
static int _cancel_producer_burst(uint64_t timeout_tcks, uint64_t timers) {
  ...
  for (i = 0; i < timers / MAX_BURST; i++) {
    ...
    for (j = 0; j < MAX_BURST; j++)
      TEST_ASSERT_EQUAL(ev_tim[j]->state, RTE_EVENT_TIMER_ARMED,
                        "Event timer not armed, state = %d", ev_tim[j]->state);

    ret = rte_ring_enqueue_bulk(timer_producer_ring, (void **)ev_tim, MAX_BURST,
                                NULL);
    TEST_ASSERT_EQUAL(ret, MAX_BURST, "Failed to enqueue event timers to ring");
    arm_count += ret;
  }
...
static int _cancel_producer_wrapper(void *args) {
  RTE_SET_USED(args);

  return _cancel_producer(20, MAX_TIMERS);
}

static int _cancel_producer_burst_wrapper(void *args) {
  RTE_SET_USED(args);

  return _cancel_producer_burst(100, MAX_TIMERS);
}

static int _cancel_thread(void *args) {
  RTE_SET_USED(args);
...
static int _cancel_burst_thread(void *args) {
  RTE_SET_USED(args);
  ...
  while (!arm_done || rte_ring_count(timer_producer_ring) > 0) {
    n = rte_ring_dequeue_burst(timer_producer_ring, (void **)ev_tim, MAX_BURST,
                               NULL);
    if (!n)
      continue;
    ...
    for (i = 0; i < n; i++)
      TEST_ASSERT_EQUAL(ev_tim[i]->state, RTE_EVENT_TIMER_ARMED,
                        "Event timer not armed, state = %d", ev_tim[i]->state);

    ret = rte_event_timer_cancel_burst(timdev, ev_tim, n);
    TEST_ASSERT_EQUAL(n, ret,
                      "Failed to cancel complete burst of "
                      "event timers");
  ...
  TEST_ASSERT_EQUAL(cancel_count, MAX_TIMERS,
                    ...
                    MAX_TIMERS, cancel_count, dequeue_count);

  return TEST_SUCCESS;
}

static inline int test_timer_cancel_multicore(void) {
  arm_done = 0;
  timer_producer_ring =
      rte_ring_create("timer_cancel_queue", MAX_TIMERS * 2, rte_socket_id(), 0);
  ...
  TEST_ASSERT_SUCCESS(_wait_timer_triggers(30, MAX_TIMERS * 2, MAX_TIMERS * 2),
                      "Timer triggered count doesn't match arm count");

  return TEST_SUCCESS;
}

static inline int test_timer_cancel_burst_multicore(void) {
  arm_done = 0;
  timer_producer_ring =
      rte_ring_create("timer_cancel_queue", MAX_TIMERS * 2, rte_socket_id(), 0);
  TEST_ASSERT_NOT_NULL(timer_producer_ring,
  ...
  rte_eal_remote_launch(_cancel_producer_burst_wrapper, NULL, test_lcore1);
...
static inline int test_timer_cancel_random(void) {
  uint64_t i;
  ...
  TEST_ASSERT_SUCCESS(_wait_timer_triggers(30, MAX_TIMERS, events_canceled),
                      "Timer triggered count doesn't match arm, cancel count");
...
static int adapter_create(void) {
  ...
  if (!(caps & RTE_EVENT_TIMER_ADAPTER_CAP_INTERNAL_PORT))
    adapter =
        rte_event_timer_adapter_create_ext(&conf, test_port_conf_cb, NULL);
  else
    adapter = rte_event_timer_adapter_create(&conf);
...
static int adapter_free(void) {
  ...
  timdev = NULL;
  TEST_ASSERT_FAIL(rte_event_timer_adapter_free(timdev), "Freed null adapter");
...
static int adapter_get_info(void) {
  struct rte_event_timer_adapter_info info;
...
static int adapter_lookup(void) {
...
static int stat_inc_reset_ev_enq(void) {
  int ret, i, n;
  ...
  ret =
      rte_mempool_get_bulk(eventdev_test_mempool, (void **)evtims, num_evtims);
  TEST_ASSERT_EQUAL(ret, 0, "Failed to get array of timer objs: ret = %d", ret);
...
#define MAX_TRIES num_evtims
  ...
  bool done = false;
  while (!done) {
    sum += rte_event_dequeue_burst(evdev, TEST_PORT_ID, evs, RTE_DIM(evs), 10);
  ...
  TEST_ASSERT_EQUAL(sum, num_evtims,
                    "Expected %d timer expiry events, "
                    "got %d",
                    num_evtims, sum);
  ...
  n = rte_event_dequeue_burst(evdev, TEST_PORT_ID, evs, RTE_DIM(evs), 10);

  TEST_ASSERT_EQUAL(n, 0,
                    "Dequeued unexpected number of timer expiry "
  ...
  rte_mempool_put_bulk(eventdev_test_mempool, (void **)evtims, num_evtims);

  return TEST_SUCCESS;
}

/* Test various cases in arming timers */
static int event_timer_arm(void) {
  uint16_t n;
  int ret;
  ...
  *evtim = init_tim;
  ...
  TEST_ASSERT_EQUAL(ret, 0,
                    "expected return value from "
                    "rte_event_timer_arm_burst: 0, got: %d",
                    ret);
  TEST_ASSERT_EQUAL(rte_errno, EALREADY,
                    "Unexpected rte_errno value "
  ...
  TEST_ASSERT_EQUAL(n, 1,
                    "Failed to dequeue expected number of expiry "
                    "events from event device");
 ...
 */
static int event_timer_arm_double(void) {
  uint16_t n;
  int ret;
  struct rte_event_timer_adapter *adapter = timdev;
  struct rte_event_timer *evtim = NULL;
  ...
  *evtim = init_tim;
  ...
  n = rte_event_dequeue_burst(evdev, TEST_PORT_ID, evs, RTE_DIM(evs), 0);
  TEST_ASSERT_EQUAL(n, 1,
                    "Dequeued incorrect number of expiry events - "
                    "expected: 1, actual: %d",
                    n);
...
static int event_timer_arm_expiry(void) {
  uint16_t n;
  int ret;
  struct rte_event_timer_adapter *adapter = timdev;
  struct rte_event_timer *evtim = NULL;
  ...
  *evtim = init_tim;
  ...
  n = rte_event_dequeue_burst(evdev, TEST_PORT_ID, evs, RTE_DIM(evs), 0);
  TEST_ASSERT_EQUAL(n, 1,
                    "Dequeued incorrect number (%d) of timer "
                    "expiry events",
                    n);
...
static int event_timer_arm_rearm(void) {
  uint16_t n;
  int ret;
  struct rte_event_timer *evtim = NULL;
  struct rte_event_timer *evtim2 = NULL;
 ...
 * expected.
 */
static int event_timer_arm_max(void) {
  int ret, i, n;
  int num_evtims = MAX_TIMERS;
  struct rte_event_timer *evtims[num_evtims];
  struct rte_event evs[BATCH_SIZE];
...
static int event_timer_arm_invalid_sched_type(void) {
  int ret;
  struct rte_event_timer *evtim = NULL;
  const struct rte_event_timer init_tim = {
      .ev.op = RTE_EVENT_OP_NEW,
  ...
  *evtim = init_tim;
  ...
  ret = rte_event_timer_arm_burst(timdev, &evtim, 1);
  TEST_ASSERT_EQUAL(ret, 0,
                    "Expected to fail timer arm with invalid "
                    "sched type, but didn't");
  TEST_ASSERT_EQUAL(rte_errno, EINVAL,
                    "Unexpected rte_errno value after"
 ...
 * too big fails.
 */
static int event_timer_arm_invalid_timeout(void) {
  int ret;
  struct rte_event_timer *evtim = NULL;
  const struct rte_event_timer init_tim = {
      .ev.op = RTE_EVENT_OP_NEW,
...
static int event_timer_cancel(void) {
  uint16_t n;
  int ret;
  struct rte_event_timer_adapter *adapter = timdev;
  struct rte_event_timer *evtim = NULL;
  ...
  TEST_ASSERT_EQUAL(ret, 0,
                    "Succeeded unexpectedly in canceling "
                    "uninited timer");
  TEST_ASSERT_EQUAL(rte_errno, EINVAL,
                    "Unexpected rte_errno value after "
...
static int event_timer_cancel_double(void) {
  uint16_t n;
  int ret;
  struct rte_event_timer_adapter *adapter = timdev;
  struct rte_event_timer *evtim = NULL;
  ...
  *evtim = init_tim;
   ...
   */
  struct rte_event_timer *evtim_arr[] = {evtim, evtim};
  ret = rte_event_timer_cancel_burst(adapter, evtim_arr, RTE_DIM(evtim_arr));

  /* Two requests to cancel same timer, only one should succeed */
  TEST_ASSERT_EQUAL(ret, 1,
                    "Succeeded unexpectedly in canceling timer "
                    "twice");

  TEST_ASSERT_EQUAL(rte_errno, EALREADY,
                    "Unexpected rte_errno value "
                    "after double-cancel: rte_errno = %d",
                    rte_errno);
 ...
 */
static int adapter_tick_resolution(void) {
  struct rte_event_timer_adapter_stats stats;
  ...
  TEST_ASSERT_SUCCESS(rte_event_timer_adapter_stats_get(timdev, &stats),
                      "Failed to get adapter stats");
  TEST_ASSERT_EQUAL(stats.adapter_tick_count, 0,
                    "Adapter tick count "
                    "not zeroed out");
...
static int adapter_create_max(void) {
  int i;
  uint32_t svc_start_count, svc_end_count;
  struct rte_event_timer_adapter *adapters[RTE_EVENT_TIMER_ADAPTER_NUM_MAX + 1];
  ...
  for (i = 0; i < RTE_EVENT_TIMER_ADAPTER_NUM_MAX; i++) {
    ...
    adapters[i] =
        rte_event_timer_adapter_create_ext(&conf, test_port_conf_cb, NULL);
    TEST_ASSERT_NOT_NULL(adapters[i],
  ...
  svc_end_count = rte_service_get_count();
  TEST_ASSERT_EQUAL(svc_start_count, svc_end_count,
                    "Failed to release "
...
static struct unit_test_suite event_timer_adptr_functional_testsuite = {
    ...
    .unit_test_cases = {
        TEST_CASE_ST(timdev_setup_usec, timdev_teardown, test_timer_state),
        TEST_CASE_ST(timdev_setup_usec, timdev_teardown, test_timer_arm),
        TEST_CASE_ST(timdev_setup_msec_periodic, timdev_teardown,
                     test_timer_arm_periodic),
        TEST_CASE_ST(timdev_setup_usec, timdev_teardown, test_timer_arm_burst),
        TEST_CASE_ST(timdev_setup_msec_periodic, timdev_teardown,
                     test_timer_arm_burst_periodic),
        TEST_CASE_ST(timdev_setup_sec, timdev_teardown, test_timer_cancel),
        ...
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown, adapter_get_info),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown, adapter_lookup),
        TEST_CASE_ST(NULL, timdev_teardown, adapter_start),
        TEST_CASE_ST(timdev_setup_msec, NULL, adapter_stop),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown, stat_inc_reset_ev_enq),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown, event_timer_arm),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
                     event_timer_arm_double),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
                     event_timer_arm_expiry),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown, event_timer_arm_rearm),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown, event_timer_arm_max),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
                     event_timer_arm_invalid_sched_type),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
                     event_timer_arm_invalid_timeout),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown, event_timer_cancel),
        TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
