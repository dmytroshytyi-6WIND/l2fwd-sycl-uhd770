...
static int test_red(void) {
  printf("red not supported on Windows, skipping test\n");
  return TEST_SKIPPED;
}

static int test_red_perf(void) {
...
#define TEST_NSEC_MARGIN                                                       \
  500 /**< nanosecond margin when calculating clk freq                         \
       */
...
static const uint64_t port_speed_bytes =
    (10ULL * 1000ULL * 1000ULL * 1000ULL) / 8ULL;
...
static void rdtsc_prof_print(struct rdtsc_prof *p) {
  if (p->clk_avgc > 0) {
    printf("RDTSC stats for %s: n=%" PRIu64 ", min=%" PRIu64 ", max=%" PRIu64
           ", avg=%.1f\n",
           p->name, p->clk_avgc, p->clk_min, p->clk_max,
...
static void rte_red_set_avg_int(const struct rte_red_config *red_cfg,
                                struct rte_red *red, uint32_t avg) {
  /**
...
static double calc_exp_avg_on_empty(double avg, uint32_t n,
                                    uint32_t time_diff) {
...
static double calc_drop_prob(uint32_t min_th, uint32_t max_th,
  ...
  if (avg < min_th) {
    ...
    drop_prob = (1.0 / (double)maxp_inv) *
                ((double)(avg - min_th) / (double)(max_th - min_th));
  } else {
    drop_prob = 1.0;
  }
  return drop_prob;
 ...
 */
static int check_avg(double *diff, double avg, double exp_avg,
                     double tolerance) {
  double abs_diff = 0.0;
  int ret = 1;
 ...
 */
static enum test_result test_rte_red_init(struct test_config *tcfg) {
  unsigned i = 0;
  ...
  for (i = 0; i < tcfg->tconfig->num_cfg; i++) {
    if (rte_red_config_init(
            &tcfg->tconfig->rconfig[i], (uint16_t)tcfg->tconfig->wq_log2[i],
            (uint16_t)tcfg->tconfig->min_th, (uint16_t)tcfg->tconfig->max_th,
 ...
 */
static int increase_actual_qsize(struct rte_red_config *red_cfg,
                                 struct rte_red *red, uint32_t *q,
                                 uint32_t level, uint32_t attempts) {
  uint32_t i = 0;
 ...
 */
static int increase_average_qsize(struct rte_red_config *red_cfg,
                                  struct rte_red *red, uint32_t *q,
                                  uint32_t level, uint32_t num_ops) {
  uint32_t avg = 0;
  uint32_t i = 0;
...
static void enqueue_dequeue_func(struct rte_red_config *red_cfg,
                                 struct rte_red *red, uint32_t *q,
                                 uint32_t num_ops, uint32_t *enqueued,
                                 uint32_t *dropped) {
  uint32_t i = 0;
 ...
 */
static uint32_t ft1_tlevels[] = {6,   12,  18,  24,  30,  36,  42,  48,
...
static enum test_result func_test1(struct test_config *tcfg) {
  ...
  for (i = 0; i < RTE_DIM(ft1_tlevels); i++) {
    ...
    *tcfg->tvar->dropped = 0;
    ...
    if (increase_actual_qsize(tcfg->tconfig->rconfig, tcfg->tqueue->rdata,
                              tcfg->tqueue->q, tcfg->tlevel[i],
                              tcfg->tqueue->q_ramp_up) != 0) {
      result = FAIL;
      goto out;
    }

    if (increase_average_qsize(tcfg->tconfig->rconfig, tcfg->tqueue->rdata,
                               tcfg->tqueue->q, tcfg->tlevel[i],
    ...
    enqueue_dequeue_func(tcfg->tconfig->rconfig, tcfg->tqueue->rdata,
                         tcfg->tqueue->q, tcfg->tvar->num_ops,
    ...
    printf("%s%-15u%-15u%-15u%-15.4lf%-15.4lf%-15.4lf%-15.4lf\n", label, avg,
           *tcfg->tvar->enqueued, *tcfg->tvar->dropped, drop_prob * 100.0,
           drop_rate * 100.0, diff, (double)tcfg->tqueue->drop_tolerance);
  }
out:
  return result;
}
...
static enum test_result func_test2(struct test_config *tcfg) {
  ...
  for (i = 0; i < tcfg->tconfig->num_cfg; i++) {
    ...
    *tcfg->tvar->enqueued = 0;
    ...
    enqueue_dequeue_func(&tcfg->tconfig->rconfig[i], tcfg->tqueue->rdata,
                         tcfg->tqueue->q, tcfg->tvar->num_ops,
                         tcfg->tvar->enqueued, tcfg->tvar->dropped);
    ...
    printf("%-15u%-15u%-15u%-15u%-15.4lf%-15.4lf%-15.4lf%-15.4lf\n", i, avg,
           tcfg->tconfig->min_th, tcfg->tconfig->max_th, drop_prob * 100.0,
           drop_rate * 100.0, diff, (double)tcfg->tqueue->drop_tolerance);
  }
out:
  return result;
...
static enum test_result func_test3(struct test_config *tcfg) {
  ...
  for (i = 0; i < tcfg->tvar->num_iterations; i++) {
    ...
    avg_before =
        rte_red_get_avg_float(tcfg->tconfig->rconfig, tcfg->tqueue->rdata);
    ...
    *tcfg->tqueue->q = 0;
     ...
     */
    if (rte_red_enqueue(tcfg->tconfig->rconfig, tcfg->tqueue->rdata,
                        *tcfg->tqueue->q, get_port_ts()) == 0) {
      (*tcfg->tqueue->q)++;
    } else {
    ...
    exp_avg = calc_exp_avg_on_empty(avg_before, (1 << *tcfg->tconfig->wq_log2),
                                    tcfg->tvar->wait_usec);
    avg_after =
        rte_red_get_avg_float(tcfg->tconfig->rconfig, tcfg->tqueue->rdata);
    if (!check_avg(&diff, avg_after, exp_avg,
                   (double)tcfg->tqueue->avg_tolerance))
      result = FAIL;

    printf("%-15.4lf%-15.4lf%-15.4lf%-15.4lf%-15.4lf%-15s\n", avg_before,
           avg_after, exp_avg, diff, (double)tcfg->tqueue->avg_tolerance,
...
static enum test_result func_test4(struct test_config *tcfg) {
  ...
  *tcfg->tqueue->q = 0;
   ...
   */
  time_diff = ((rte_rdtsc() - start) / tcfg->tvar->clk_freq) * MSEC_PER_SEC;
...
static struct test_config func_test5_config = {
    ...
    .msg = "functional test 5 : use several queues (each with its own run-time "
           "data),\n"
...
static enum test_result func_test5(struct test_config *tcfg) {
  ...
  for (j = 0; j < tcfg->tqueue->num_queues; j++) {
    ...
    if (increase_actual_qsize(&tcfg->tconfig->rconfig[tcfg->tqueue->qconfig[j]],
                              &tcfg->tqueue->rdata[j], &tcfg->tqueue->q[j],
                              *tcfg->tlevel, tcfg->tqueue->q_ramp_up) != 0) {
      result = FAIL;
      goto out;
    }

    if (increase_average_qsize(
            &tcfg->tconfig->rconfig[tcfg->tqueue->qconfig[j]],
            &tcfg->tqueue->rdata[j], &tcfg->tqueue->q[j], *tcfg->tlevel,
            tcfg->tqueue->avg_ramp_up) != 0) {
      result = FAIL;
      goto out;
    }
  ...
  for (j = 0; j < tcfg->tqueue->num_queues; j++) {
    ...
    enqueue_dequeue_func(&tcfg->tconfig->rconfig[tcfg->tqueue->qconfig[j]],
                         &tcfg->tqueue->rdata[j], &tcfg->tqueue->q[j],
                         tcfg->tvar->num_ops, &tcfg->tvar->enqueued[j],
                         &tcfg->tvar->dropped[j]);
    ...
    drop_prob = calc_drop_prob(
        ...
        tcfg->tconfig->maxp_inv[tcfg->tqueue->qconfig[j]], *tcfg->tlevel);
    if (!check_drop_rate(&diff, drop_rate, drop_prob,
                         (double)tcfg->tqueue->drop_tolerance))
      result = FAIL;

    printf("%-15u%-15u%-15u%-15u%-15u%-15.4lf%-15.4lf%-15.4lf%-15.4lf\n", j,
           tcfg->tqueue->qconfig[j], avg, tcfg->tconfig->min_th,
           tcfg->tconfig->max_th, drop_prob * 100.0, drop_rate * 100.0, diff,
           (double)tcfg->tqueue->drop_tolerance);
  }
out:
  return result;
...
static struct test_config func_test6_config = {
    ...
    .msg = "functional test 6 : use several queues (each with its own run-time "
           "data),\n"
           "		    use several RED configurations (such that each "
           "configuration is shared by multiple queues),\n"
           "		    increase average queue size to target level,\n"
...
static enum test_result func_test6(struct test_config *tcfg) {
  ...
  for (j = 0; j < tcfg->tqueue->num_queues; j++) {
    ...
    avg_before =
        rte_red_get_avg_float(&tcfg->tconfig->rconfig[tcfg->tqueue->qconfig[j]],
                              &tcfg->tqueue->rdata[j]);
    ...
    exp_avg = calc_exp_avg_on_empty(
        avg_before, (1 << tcfg->tconfig->wq_log2[tcfg->tqueue->qconfig[j]]),
        tcfg->tvar->wait_usec);
    avg_after =
        rte_red_get_avg_float(&tcfg->tconfig->rconfig[tcfg->tqueue->qconfig[j]],
                              &tcfg->tqueue->rdata[j]);
    ...
    printf("%-15u%-15u%-15.4lf%-15.4lf%-15.4lf%-15.4lf%-15.4lf%-15s\n", j,
           tcfg->tqueue->qconfig[j], avg_before, avg_after, exp_avg, diff,
           (double)tcfg->tqueue->avg_tolerance,
...
static void enqueue_dequeue_perf(struct rte_red_config *red_cfg,
                                 struct rte_red *red, uint32_t *q,
                                 uint32_t num_ops, uint32_t *enqueued,
                                 uint32_t *dropped, struct rdtsc_prof *prof) {
  uint32_t i = 0;
...
static struct test_var perf1_tvar = {.wait_usec = 0,
                                     .num_iterations = 15,
                                     .num_ops = 50000000,
                                     .clk_freq = 0,
                                     .dropped = pt_dropped,
...
static struct test_config perf1_test1_config = {
    ...
    .msg = "performance test 1 : use one RED configuration,\n"
           "		     set actual and average queue sizes to level below "
           "min threshold,\n"
...
static struct test_config perf1_test2_config = {
    ...
    .msg = "performance test 2 : use one RED configuration,\n"
           "		     set actual and average queue sizes to level in "
...
static struct test_config perf1_test3_config = {
    ...
    .msg = "performance test 3 : use one RED configuration,\n"
           "		     set actual and average queue sizes to level above "
           "max threshold,\n"
           "		     measure enqueue performance\n\n",
    .tconfig = &pt_tconfig,
    .tqueue = &pt_tqueue,
...
static enum test_result perf1_test(struct test_config *tcfg) {
  ...
  *tcfg->tqueue->q = *tcfg->tlevel;
   ...
   */
  rte_red_set_avg_int(tcfg->tconfig->rconfig, tcfg->tqueue->rdata,
                      *tcfg->tlevel);
  ...
  enqueue_dequeue_perf(tcfg->tconfig->rconfig, tcfg->tqueue->rdata,
                       tcfg->tqueue->q, tcfg->tvar->num_ops,
                       tcfg->tvar->enqueued, tcfg->tvar->dropped, &prof);
...
static struct test_var perf2_tvar = {.wait_usec = 500,
...
static struct test_config perf2_test4_config = {
    ...
    .msg = "performance test 4 : use one RED configuration,\n"
           ...
           "min threshold,\n"
           "		     dequeue all packets until queue is empty,\n"
           "		     measure enqueue performance when queue is "
           "empty\n\n",
    .htxt = "iteration      "
            "q avg before   "
            "q avg after    "
    ...
    *tcfg->tqueue->q = 0;
    ...
    if ((i == 0) || (i == tcfg->tvar->num_iterations - 1)) {
      ...
      avg_after =
          rte_red_get_avg_float(tcfg->tconfig->rconfig, tcfg->tqueue->rdata);
      exp_avg = calc_exp_avg_on_empty(
          avg_before, (1 << *tcfg->tconfig->wq_log2), tcfg->tvar->wait_usec);
      if (check_avg(&diff, avg_after, exp_avg,
                    (double)tcfg->tqueue->avg_tolerance))
        ok = 1;
...
static struct test_var ovfl_tvar = {.wait_usec = 10000,
                                    .num_iterations = 1,
                                    .num_ops = 10000,
                                    .clk_freq = 0,
...
static struct test_config ovfl_test1_config = {
    ...
    .msg = "overflow test 1 : use one RED configuration,\n"
           "		  increase average queue size to target level,\n"
           "		  check maximum number of bits required to represent "
           "avg_s\n\n",
    .htxt = "avg queue size  "
            "wq_log2  "
            "fraction bits  "
...
static enum test_result ovfl_test1(struct test_config *tcfg) {
  ...
  drop_rate = calc_drop_rate(*tcfg->tvar->enqueued, *tcfg->tvar->dropped);
  drop_prob = calc_drop_prob(tcfg->tconfig->min_th, tcfg->tconfig->max_th,
                             *tcfg->tconfig->maxp_inv, *tcfg->tlevel);
  if (!check_drop_rate(&diff, drop_rate, drop_prob,
                       (double)tcfg->tqueue->drop_tolerance))
    result = FAIL;
  ...
  printf("%-16u%-9u%-15u0x%08x     %-10u%-10u%-10u%-13.2lf%-13.2lf\n", avg,
         *tcfg->tconfig->wq_log2, RTE_RED_SCALING, avg_max, avg_max_bits,
         *tcfg->tvar->enqueued, *tcfg->tvar->dropped, drop_prob * 100.0,
         drop_rate * 100.0);
out:
  return result;
...
struct tests func_tests[] = {
    {&func_test1_config, func_test1}, {&func_test2_config, func_test2},
    {&func_test3_config, func_test3}, {&func_test4_config, func_test4},
    {&func_test5_config, func_test5}, {&func_test6_config, func_test6},
...
struct tests perf_tests[] = {
    {&perf1_test1_config, perf1_test}, {&perf1_test2_config, perf1_test},
    {&perf1_test3_config, perf1_test}, {&perf2_test4_config, perf2_test},
    {&perf2_test5_config, perf2_test}, {&perf2_test6_config, perf2_test},
};
...
static int test_invalid_parameters(void) {
  ...
  if (rte_red_config_init(&config, RTE_RED_WQ_LOG2_MAX + 1, 1, 2, 0) == 0) {
    printf("%i: rte_red_config_init should have failed!\n", __LINE__);
    return -1;
  }
  /* wq_log2 < RTE_RED_WQ_LOG2_MIN */
  if (rte_red_config_init(&config, RTE_RED_WQ_LOG2_MIN - 1, 1, 2, 0) == 0) {
    printf("%i: rte_red_config_init should have failed!\n", __LINE__);
    return -1;
  }
  /* maxp_inv > RTE_RED_MAXP_INV_MAX */
  if (rte_red_config_init(&config, RTE_RED_WQ_LOG2_MIN, 1, 2,
                          RTE_RED_MAXP_INV_MAX + 1) == 0) {
    printf("%i: rte_red_config_init should have failed!\n", __LINE__);
    return -1;
  }
  /* maxp_inv < RTE_RED_MAXP_INV_MIN */
  if (rte_red_config_init(&config, RTE_RED_WQ_LOG2_MIN, 1, 2,
                          RTE_RED_MAXP_INV_MIN - 1) == 0) {
    printf("%i: rte_red_config_init should have failed!\n", __LINE__);
    return -1;
  }
...
static void show_stats(const uint32_t num_tests, const uint32_t num_pass) {
...
static int tell_the_result(const uint32_t num_tests, const uint32_t num_pass) {
...
static int test_red(void) {
  ...
  if (test_invalid_parameters() < 0)
    return -1;
  run_tests(func_tests_quick, RTE_DIM(func_tests_quick), &num_tests, &num_pass);
