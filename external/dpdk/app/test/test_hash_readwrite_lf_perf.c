...
static inline int get_enabled_cores_list(void) {
  uint32_t i = 0;
  uint16_t core_id;
...
static int init_params(int rwc_lf, int use_jhash, int htm, int ext_bkt) {
...
static inline int check_bucket(uint32_t bkt_idx, uint32_t key) {
  uint32_t iter;
  uint32_t prev_iter;
  uint32_t diff;
  uint32_t count = 0;
  ...
  iter = bkt_idx * 8;
  prev_iter = iter;
  while (rte_hash_iterate(tbl_rwc_test_param.h, &next_key, &next_data, &iter) >=
         0) {
...
static int generate_keys(void) {
   ...
   */
  tbl_rwc_test_param.keys_shift_path =
      rte_malloc(NULL, sizeof(uint32_t) * TOTAL_INSERT, 0);
   ...
   */
  keys_non_shift_path = rte_malloc(NULL, sizeof(uint32_t) * TOTAL_INSERT, 0);
  ...
  for (i = 0; i < TOTAL_INSERT; i++) {
    ...
    sig = rte_hash_hash(tbl_rwc_test_param.h, tbl_rwc_test_param.keys + i);
    prim_bucket_idx = get_prim_bucket_index(tbl_rwc_test_param.h, sig);
    ...
    if (ret < 0) {
       ...
       */
      ret = rte_hash_add_key_data(tbl_rwc_test_param.h, keys + i,
                                  (void *)((uintptr_t)i));
      if (ret < 0) {
  ...
  for (i = 0; i < num_buckets; i++) {
    ...
    if (scanned_bkts[i] == 0) {
      iter = i * 8;
      while (rte_hash_iterate(tbl_rwc_test_param.h, &next_key, &next_data,
                              &iter) >= 0) {

        /* Check if key belongs to the current bucket */
        if (i >= (iter - 1) / 8)
          keys_non_shift_path[count++] = *(const uint32_t *)next_key;
        else
          break;
      }
    }
  ...
  for (i = 0; i < count_keys_ks; i++) {
    ...
    if (ret < 0) {
      ...
      sig = rte_hash_hash(tbl_rwc_test_param.h, tbl_rwc_test_param.keys_ks + i);
      prim_bucket_idx = get_prim_bucket_index(tbl_rwc_test_param.h, sig);
      short_sig = get_short_sig(sig);
      sec_bucket_idx = get_alt_bucket_index(tbl_rwc_test_param.h,
  ...
  for (i = 0; i < num_buckets; i++) {
    if (scanned_bkts[i] == 1) {
      ...
      while (rte_hash_iterate(tbl_rwc_test_param.h, &next_key, &next_data,
        ...
        /* Check if key belongs to the current bucket */
        if (i >= (iter - 1) / 8)
          keys_ks_extbkt[count++] = *(const uint32_t *)next_key;
        else
          break;
      }
    }
...
static int test_rwc_reader(__rte_unused void *arg) {
  uint32_t i, j;
  int ret;
  ...
  do {
    if (read_type & BULK_LOOKUP) {
      for (i = 0; i < (read_cnt - extra_keys); i += BULK_LOOKUP_SIZE) {
        /* Array of  pointer to the list of keys */
        for (j = 0; j < BULK_LOOKUP_SIZE; j++)
          temp_a[j] = keys + i + j;
        rte_hash_lookup_bulk(tbl_rwc_test_param.h,
                             (const void **)((uintptr_t)temp_a),
                             BULK_LOOKUP_SIZE, pos);
        /* Validate lookup result */
        for (j = 0; j < BULK_LOOKUP_SIZE; j++)
          if ((read_type & READ_FAIL && pos[j] != -ENOENT) ||
              (!(read_type & READ_FAIL) && pos[j] == -ENOENT)) {
      ...
      rte_hash_lookup_bulk(tbl_rwc_test_param.h,
                           (const void **)((uintptr_t)temp_a), extra_keys, pos);
      for (j = 0; j < extra_keys; j++)
        if ((read_type & READ_FAIL && pos[j] != -ENOENT) ||
            (!(read_type & READ_FAIL) && pos[j] == -ENOENT)) {
...
static int write_keys(uint8_t write_type) {
  uint32_t i;
  int ret;
  uint32_t key_cnt = 0;
  uint32_t *keys;
...
static int test_rwc_multi_writer(__rte_unused void *arg) {
  uint32_t i, offset;
  ...
  for (i = offset; i < offset + tbl_rwc_test_param.single_insert; i++)
    rte_hash_add_key(tbl_rwc_test_param.h, tbl_rwc_test_param.keys_ks + i);
  return 0;
}
 ...
 */
static int test_hash_add_no_ks_lookup_hit(struct rwc_perf *rwc_perf_results,
  ...
  for (m = 0; m < 2; m++) {
    ...
    for (n = 0; n < NUM_TEST; n++) {
      ...
      for (i = 1; i <= rwc_core_cnt[n]; i++)
        rte_eal_remote_launch(test_rwc_reader, (void *)(uintptr_t)read_type,
                              enabled_core_ids[i]);
      ...
      rwc_perf_results->w_no_ks_r_hit[m][n] = cycles_per_lookup;
 ...
 */
static int test_hash_add_no_ks_lookup_miss(struct rwc_perf *rwc_perf_results,
                                           int rwc_lf, int htm, int ext_bkt) {
  unsigned int n, m;
  uint64_t i;
  int use_jhash = 0;
  ...
  for (m = 0; m < 2; m++) {
    ...
    for (n = 0; n < NUM_TEST; n++) {
      ...
      unsigned long long cycles_per_lookup =
          __atomic_load_n(&gread_cycles, __ATOMIC_RELAXED) /
          __atomic_load_n(&greads, __ATOMIC_RELAXED);
      rwc_perf_results->w_no_ks_r_miss[m][n] = cycles_per_lookup;
      printf("Cycles per lookup: %llu\n", cycles_per_lookup);
    }
  }
 ...
 */
static int test_hash_add_ks_lookup_hit_non_sp(struct rwc_perf *rwc_perf_results,
                                              int rwc_lf, int htm,
                                              int ext_bkt) {
  unsigned int n, m;
  uint64_t i;
  ...
  for (m = 0; m < 2; m++) {
    ...
    for (n = 0; n < NUM_TEST; n++) {
      ...
      unsigned long long cycles_per_lookup =
          __atomic_load_n(&gread_cycles, __ATOMIC_RELAXED) /
          __atomic_load_n(&greads, __ATOMIC_RELAXED);
      rwc_perf_results->w_ks_r_hit_nsp[m][n] = cycles_per_lookup;
      printf("Cycles per lookup: %llu\n", cycles_per_lookup);
    }
  }
 ...
 */
static int test_hash_add_ks_lookup_hit_sp(struct rwc_perf *rwc_perf_results,
                                          int rwc_lf, int htm, int ext_bkt) {
  unsigned int n, m;
  uint64_t i;
  int use_jhash = 0;
  ...
  for (m = 0; m < 2; m++) {
    ...
    for (n = 0; n < NUM_TEST; n++) {
      ...
      unsigned long long cycles_per_lookup =
          __atomic_load_n(&gread_cycles, __ATOMIC_RELAXED) /
          __atomic_load_n(&greads, __ATOMIC_RELAXED);
      rwc_perf_results->w_ks_r_hit_sp[m][n] = cycles_per_lookup;
      printf("Cycles per lookup: %llu\n", cycles_per_lookup);
    }
  }
 ...
 * 'Main' thread adds keys causing key-shifts.
 */
static int test_hash_add_ks_lookup_miss(struct rwc_perf *rwc_perf_results,
                                        int rwc_lf, int htm, int ext_bkt) {
  unsigned int n, m;
  uint64_t i;
  int use_jhash = 0;
 ...
 */
static int test_hash_multi_add_lookup(struct rwc_perf *rwc_perf_results,
                                      int rwc_lf, int htm, int ext_bkt) {
  unsigned int n, m, k;
  uint64_t i;
  int use_jhash = 0;
  ...
  for (m = 1; m < NUM_TEST; m++) {
    ...
    for (k = 0; k < 2; k++) {
      ...
      for (n = 0; n < NUM_TEST; n++) {
        ...
        for (i = rwc_core_cnt[n] + 1; i <= rwc_core_cnt[m] + rwc_core_cnt[n];
             i++)
        ...
        unsigned long long cycles_per_lookup =
            __atomic_load_n(&gread_cycles, __ATOMIC_RELAXED) /
            __atomic_load_n(&greads, __ATOMIC_RELAXED);
        rwc_perf_results->multi_rw[m][k][n] = cycles_per_lookup;
        printf("Cycles per lookup: %llu\n", cycles_per_lookup);
      }
    }
  }
 ...
 */
static int test_hash_add_ks_lookup_hit_extbkt(struct rwc_perf *rwc_perf_results,
                                              int rwc_lf, int htm,
                                              int ext_bkt) {
  unsigned int n, m;
  uint64_t i;
  ...
  for (m = 0; m < 2; m++) {
    ...
    for (n = 0; n < NUM_TEST; n++) {
      ...
      for (i = 1; i <= rwc_core_cnt[n]; i++)
        rte_eal_remote_launch(test_rwc_reader, (void *)(uintptr_t)read_type,
                              enabled_core_ids[i]);
      for (i = 0; i < tbl_rwc_test_param.count_keys_ks_extbkt; i++) {
        if (rte_hash_del_key(tbl_rwc_test_param.h,
                             tbl_rwc_test_param.keys_ks_extbkt + i) < 0) {
      ...
      unsigned long long cycles_per_lookup =
          __atomic_load_n(&gread_cycles, __ATOMIC_RELAXED) /
          __atomic_load_n(&greads, __ATOMIC_RELAXED);
      rwc_perf_results->w_ks_r_hit_extbkt[m][n] = cycles_per_lookup;
      printf("Cycles per lookup: %llu\n", cycles_per_lookup);
    }
  }
 ...
 */
static int test_hash_rcu_qsbr_reader(void *arg) {
  unsigned int i, j;
  uint32_t num_keys =
      tbl_rwc_test_param.count_keys_no_ks - QSBR_REPORTING_INTERVAL;
  ...
  do {
    for (i = 0; i < num_keys; i += j) {
      for (j = 0; j < QSBR_REPORTING_INTERVAL; j++)
        rte_hash_lookup(tbl_rwc_test_param.h, keys + i + j);
 ...
 */
static int test_hash_rcu_qsbr_writer(void *arg) {
  uint32_t i, offset;
  uint64_t begin, cycles;
  ...
  for (i = offset; i < offset + tbl_rwc_test_param.single_insert; i++) {
    ...
    rte_hash_del_key(tbl_rwc_test_param.h, tbl_rwc_test_param.keys_no_ks + i);
    rte_hash_add_key(tbl_rwc_test_param.h, tbl_rwc_test_param.keys_no_ks + i);
  }
  cycles = rte_rdtsc_precise() - begin;
 ...
 */
static int test_hash_rcu_qsbr_writer_perf(struct rwc_perf *rwc_perf_results,
                                          int rwc_lf, int htm, int ext_bkt) {
  unsigned int n;
  uint64_t i;
  uint8_t write_type;
  ...
  for (n = 0; n < NUM_TEST; n++) {
    ...
    /* Calculate keys added by each writer */
    tbl_rwc_test_param.single_insert =
        tbl_rwc_test_param.count_keys_no_ks / rwc_core_cnt[n];
    printf("\nNumber of writers: %u\n", rwc_core_cnt[n]);
    ...
    for (; i <= rwc_core_cnt[n] + 2; i++) {
      rte_eal_remote_launch(test_hash_rcu_qsbr_writer,
                            (void *)(uintptr_t)pos_core, enabled_core_ids[i]);
      pos_core++;
    }
    ...
    rwc_perf_results->writer_add_del[n] = cycles_per_write_operation;
...
static int test_hash_readwrite_lf_perf_main(void) {
  /*
  ...
  if (rte_lcore_count() < 2) {
    printf("Not enough cores for hash_readwrite_lf_perf_autotest, expecting at "
           "least 2\n");
    return TEST_SKIPPED;
  }
  ...
  if (RTE_HASH_EXTRA_FLAGS_RW_CONCURRENCY_LF) {
    ...
    printf("Test lookup with read-write concurrency lock free support"
           " enabled\n");
    if (test_hash_add_no_ks_lookup_hit(&rwc_lf_results, rwc_lf, htm, ext_bkt) <
        0)
      return -1;
    if (test_hash_add_no_ks_lookup_miss(&rwc_lf_results, rwc_lf, htm, ext_bkt) <
        0)
      return -1;
    if (test_hash_add_ks_lookup_hit_non_sp(&rwc_lf_results, rwc_lf, htm,
                                           ext_bkt) < 0)
      return -1;
    if (test_hash_add_ks_lookup_hit_sp(&rwc_lf_results, rwc_lf, htm, ext_bkt) <
        0)
      return -1;
    if (test_hash_add_ks_lookup_miss(&rwc_lf_results, rwc_lf, htm, ext_bkt) < 0)
      return -1;
    if (test_hash_multi_add_lookup(&rwc_lf_results, rwc_lf, htm, ext_bkt) < 0)
      return -1;
    if (test_hash_add_ks_lookup_hit_extbkt(&rwc_lf_results, rwc_lf, htm,
                                           ext_bkt) < 0)
      return -1;
    if (test_hash_rcu_qsbr_writer_perf(&rwc_lf_results, rwc_lf, htm, ext_bkt) <
        0)
      return -1;
  }
  ...
  if (test_hash_add_no_ks_lookup_miss(&rwc_non_lf_results, rwc_lf, htm,
                                      ext_bkt) < 0)
    return -1;
  if (test_hash_add_ks_lookup_hit_non_sp(&rwc_non_lf_results, rwc_lf, htm,
                                         ext_bkt) < 0)
    return -1;
  if (test_hash_add_ks_lookup_hit_sp(&rwc_non_lf_results, rwc_lf, htm,
                                     ext_bkt) < 0)
    return -1;
  if (test_hash_add_ks_lookup_miss(&rwc_non_lf_results, rwc_lf, htm, ext_bkt) <
      0)
    return -1;
  if (test_hash_multi_add_lookup(&rwc_non_lf_results, rwc_lf, htm, ext_bkt) < 0)
    return -1;
  if (test_hash_add_ks_lookup_hit_extbkt(&rwc_non_lf_results, rwc_lf, htm,
                                         ext_bkt) < 0)
    return -1;
results:
  printf("\n\t\t\t\t\t\t********** Results summary **********\n\n");
