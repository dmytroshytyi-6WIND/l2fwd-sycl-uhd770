...
#define FOR_EACH_SUITE_TESTSUITE(iter, suite, sub_ts)                          \
  for (iter = 0,                                                               \
      sub_ts = suite->unit_test_suites ? suite->unit_test_suites[0] : NULL;    \
...
static int no_action(void) { return 0; }

static int do_recursive_call(void) {
  unsigned i;
  struct {
    const char *env_var;
    int (*action_fn)(void);
...
int main(int argc, char **argv) {
  ...
  if (extra_args != NULL && strlen(extra_args) > 0) {
    ...
    eargc = rte_strsplit(extra_args, strlen(extra_args), eargv, MAX_EXTRA_ARGS,
                         ' ');
...
static void unit_test_suite_count_tcs_on_setup_fail(
    struct unit_test_suite *suite, int test_success,
...
static void unit_test_suite_reset_counts(struct unit_test_suite *suite) {
  struct unit_test_suite *ts;
  int i;
  ...
  if (suite->setup) {
    ...
    if (test_success != 0) {
      ...
      unit_test_suite_count_tcs_on_setup_fail(
          suite, test_success, &sub_ts_failed, &sub_ts_skipped, &sub_ts_total);
      goto suite_summary;
    }
  }
...
suite_summary:
  ...
  printf(" + %s : %d/%d passed, %d/%d skipped, "
         ...
         ts->suite_name, ts->succeeded, ts->total, ts->skipped, ts->total,
