...
static int is_memory_overlap(rte_iova_t ptr1, size_t len1, rte_iova_t ptr2,
                             size_t len2) {
...
static int test_memzone_invalid_alignment(void) {
  ...
  mz = rte_memzone_reserve_aligned(TEST_MEMZONE_NAME("invalid_alignment"), 100,
                                   SOCKET_ID_ANY, 0, 100);
  if (mz != NULL) {
    printf("Zone with invalid alignment has been reserved\n");
    return -1;
...
static int test_memzone_invalid_flags(void) {
  const struct rte_memzone *mz;
  ...
  mz = rte_memzone_reserve(TEST_MEMZONE_NAME("invalid_flags"), 100,
                           SOCKET_ID_ANY, RTE_MEMZONE_IOVA_CONTIG << 1);
  if (mz != NULL) {
    printf("Zone with invalid flags has been reserved\n");
    return -1;
  }
...
static int test_memzone_reserving_zone_size_bigger_than_the_maximum(void) {
  const struct rte_memzone *mz;
  ...
  mz = rte_memzone_reserve(
      TEST_MEMZONE_NAME("zone_size_bigger_than_the_maximum"), (size_t)-1,
      SOCKET_ID_ANY, 0);
  if (mz != NULL) {
...
static int find_available_pagesz(const struct rte_memseg_list *msl, void *arg) {
...
static int test_memzone_reserve_flags(void) {
  const struct rte_memzone *mz;
  struct walk_arg wa;
   ...
   */
  if (hugepage_2MB_avail) {
    mz = rte_memzone_reserve(TEST_MEMZONE_NAME("flag_zone_2M"), size,
    ...
    mz = rte_memzone_reserve(TEST_MEMZONE_NAME("flag_zone_2M_HINT"), size,
                             SOCKET_ID_ANY,
     ...
     */
    if (!hugepage_1GB_avail) {
      mz = rte_memzone_reserve(TEST_MEMZONE_NAME("flag_zone_1G_HINT"), size,
                               SOCKET_ID_ANY,
      ...
      mz = rte_memzone_reserve(TEST_MEMZONE_NAME("flag_zone_1G"), size,
  ...
  if (hugepage_1GB_avail) {
    ...
    if (hugepage_2MB_avail && hugepage_1GB_avail) {
      mz =
          rte_memzone_reserve(TEST_MEMZONE_NAME("flag_zone_2M_HINT"), size,
                              SOCKET_ID_ANY, RTE_MEMZONE_2MB | RTE_MEMZONE_1GB);
      if (mz == NULL) {
   ...
   */
  if (hugepage_16MB_avail) {
    mz = rte_memzone_reserve(TEST_MEMZONE_NAME("flag_zone_16M"), size,
    ...
    mz = rte_memzone_reserve(TEST_MEMZONE_NAME("flag_zone_16M_HINT"), size,
                             SOCKET_ID_ANY,
     ...
     */
    if (!hugepage_16GB_avail) {
      mz = rte_memzone_reserve(TEST_MEMZONE_NAME("flag_zone_16G_HINT"), size,
                               SOCKET_ID_ANY,
      ...
      mz = rte_memzone_reserve(TEST_MEMZONE_NAME("flag_zone_16G"), size,
...
static size_t find_max_block_free_size(unsigned int align,
...
static int test_memzone_reserve_max(void) {
  unsigned int i;
  ...
  for (i = 0; i < rte_socket_count(); i++) {
    ...
    mz = rte_memzone_reserve(TEST_MEMZONE_NAME("max_zone"), 0, socket, 0);
    if (mz == NULL) {
...
static int test_memzone_reserve_max_aligned(void) {
  unsigned int i;
  ...
  for (i = 0; i < rte_socket_count(); i++) {
    ...
    mz = rte_memzone_reserve_aligned(TEST_MEMZONE_NAME("max_zone_aligned"), 0,
                                     socket, 0, align);
    if (mz == NULL) {
      printf("Failed to reserve a big chunk of memory - %s\n",
             rte_strerror(rte_errno));
...
static int test_memzone_aligned(void) {
...
static int check_memzone_bounded(const char *name, uint32_t len, uint32_t align,
                                 uint32_t bound) {
  const struct rte_memzone *mz;
  rte_iova_t bmask;
  ...
  if ((mz = rte_memzone_reserve_bounded(name, len, SOCKET_ID_ANY, 0, align,
                                        bound)) == NULL) {
  ...
  if ((mz->iova & bmask) != ((mz->iova + mz->len - 1) & bmask)) {
...
static int test_memzone_bounded(void) {
  const struct rte_memzone *memzone_err;
  int rc;

  /* should fail as boundary is not power of two */
  memzone_err =
      rte_memzone_reserve_bounded(TEST_MEMZONE_NAME("bounded_error_31"), 100,
  ...
  memzone_err = rte_memzone_reserve_bounded(
      TEST_MEMZONE_NAME("bounded_error_32"), 100, SOCKET_ID_ANY, 0, 32, 32);
  if (memzone_err != NULL) {
    printf("%s(%s)created a memzone with invalid boundary "
           "conditions\n",
           __func__, memzone_err->name);
  ...
  rc = check_memzone_bounded(TEST_MEMZONE_NAME("bounded_128"), 100, 128, 128);
  if (rc != 0)
    return rc;

  rc = check_memzone_bounded(TEST_MEMZONE_NAME("bounded_256"), 100, 256, 128);
  if (rc != 0)
    return rc;

  rc = check_memzone_bounded(TEST_MEMZONE_NAME("bounded_1K"), 100, 64, 1024);
  if (rc != 0)
    return rc;

  rc = check_memzone_bounded(TEST_MEMZONE_NAME("bounded_1K_MAX"), 0, 64, 1024);
  if (rc != 0)
    return rc;
...
static int test_memzone_free(void) {
  ...
  i = 0;
  do {
    snprintf(name, sizeof(name), TEST_MEMZONE_NAME("tempzone%u"), i);
...
static int test_memzone_basic(void) {
  ...
  memzone2 = rte_memzone_reserve(TEST_MEMZONE_NAME("testzone2"), 1000, 0, 0);

  memzone3 = rte_memzone_reserve(TEST_MEMZONE_NAME("testzone3"), 1000, 1, 0);
  ...
  memzone_cnt_expected = memzone_cnt_before + (memzone1 != NULL) +
                         (memzone2 != NULL) + (memzone3 != NULL) +
                         (memzone4 != NULL);
  ...
  if ((memzone2->len & RTE_CACHE_LINE_MASK) != 0 || memzone2->len == 0)
    return -1;
  if (memzone3 != NULL &&
      ((memzone3->len & RTE_CACHE_LINE_MASK) != 0 || memzone3->len == 0))
    return -1;
  ...
  if (is_memory_overlap(memzone1->iova, memzone1->len, memzone2->iova,
                        memzone2->len))
    return -1;
  if (memzone3 != NULL && is_memory_overlap(memzone1->iova, memzone1->len,
                                            memzone3->iova, memzone3->len))
    return -1;
  if (memzone3 != NULL && is_memory_overlap(memzone2->iova, memzone2->len,
                                            memzone3->iova, memzone3->len))
    return -1;
  ...
  mz = rte_memzone_reserve(TEST_MEMZONE_NAME("testzone1"), 100, SOCKET_ID_ANY,
                           0);
  if (mz != NULL)
    return -1;
