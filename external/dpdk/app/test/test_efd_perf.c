...
static void shuffle_input_keys(struct efd_perf_params *params) {
 ...
 */
static int setup_keys_and_data(struct efd_perf_params *params,
                               unsigned int cycle) {
  unsigned int i, j;
  int num_duplicates;
  ...
  params->efd_table =
      rte_efd_create("test_efd_perf", MAX_ENTRIES, params->key_size,
...
static int timed_adds(struct efd_perf_params *params) {
  const uint64_t start_tsc = rte_rdtsc();
  unsigned int i, a;
  int32_t ret;

  for (i = 0; i < KEYS_TO_ADD; i++) {
    ret = rte_efd_update(params->efd_table, test_socket_id, keys[i], data[i]);
    if (ret != 0) {
...
static int timed_lookups(struct efd_perf_params *params) {
  unsigned int i, j, a;
  const uint64_t start_tsc = rte_rdtsc();
  ...
  for (i = 0; i < NUM_LOOKUPS / KEYS_TO_ADD; i++) {
    for (j = 0; j < KEYS_TO_ADD; j++) {
      ret_data = rte_efd_lookup(params->efd_table, test_socket_id, keys[j]);
      if (ret_data != data[j]) {
...
static int timed_lookups_multi(struct efd_perf_params *params) {
  ...
  for (i = 0; i < NUM_LOOKUPS / KEYS_TO_ADD; i++) {
    for (j = 0; j < KEYS_TO_ADD / RTE_EFD_BURST_MAX; j++) {
      ...
      rte_efd_lookup_bulk(params->efd_table, test_socket_id, RTE_EFD_BURST_MAX,
                          keys_burst, result);
...
static int timed_deletes(struct efd_perf_params *params) {
  unsigned int i, a;
  const uint64_t start_tsc = rte_rdtsc();
  int32_t ret;

  for (i = 0; i < KEYS_TO_ADD; i++) {
    ret = rte_efd_delete(params->efd_table, test_socket_id, keys[i], NULL);
...
static void perform_frees(struct efd_perf_params *params) {
...
static int exit_with_fail(const char *testname, struct efd_perf_params *params,
                          unsigned int i) {

  printf("<<<<<Test %s failed at keysize %d iteration %d >>>>>\n", testname,
         hashtest_key_lens[params->cycle], i);
