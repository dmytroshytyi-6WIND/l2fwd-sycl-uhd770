...
static int test_libipsec_perf(void) {
...
static int fill_ipsec_param(struct ipsec_sa *sa) {
  ...
  if (sa->type == RTE_CRYPTO_SYM_XFORM_AEAD) {
    ...
    if (sa->ipsec_xform.direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS) {
    ...
    if (sa->ipsec_xform.direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS) {
      sa->cipher_xform.cipher.op = RTE_CRYPTO_CIPHER_OP_DECRYPT;
      ...
      sa->cipher_xform.cipher.op = RTE_CRYPTO_CIPHER_OP_ENCRYPT;
...
static int packet_prepare(struct rte_mbuf **buf, struct ipsec_sa *sa,
                          uint16_t num_pkts) {
  uint64_t time_stamp;
  uint16_t k = 0, i;
  ...
  for (i = 0; i < num_pkts; i++) {
    ...
    sa->cop[i] = rte_crypto_op_alloc(cop_pool, RTE_CRYPTO_OP_TYPE_SYMMETRIC);
  ...
  k = rte_ipsec_pkt_crypto_prepare(&sa->ss[0], buf, sa->cop, num_pkts);
...
static int packet_process(struct rte_mbuf **buf, struct ipsec_sa *sa,
                          uint16_t num_pkts) {
  uint64_t time_stamp;
  uint16_t k = 0;
...
static int create_traffic(struct ipsec_sa *sa, struct rte_ring *deq_ring,
  ...
  while (rte_ring_empty(deq_ring) == 0) {
    ...
    num_pkts =
        rte_ring_sc_dequeue_burst(deq_ring, (void **)mbuf, RTE_DIM(mbuf), NULL);
    ...
    num_pkts =
        rte_ring_sp_enqueue_burst(enq_ring, (void **)mbuf, num_pkts, NULL);
    if (num_pkts == 0)
      return TEST_FAILED;
  }
...
static void fill_ipsec_sa_out(const struct ipsec_test_cfg *test_cfg,
                              struct ipsec_sa *sa) {
...
static void fill_ipsec_sa_in(const struct ipsec_test_cfg *test_cfg,
                             struct ipsec_sa *sa) {
  sa->ipsec_xform.spi = DEFAULT_SPI;
...
static int init_sa_session(const struct ipsec_test_cfg *test_cfg,
...
static int testsuite_setup(void) {
  ...
  mbuf_pool =
      rte_pktmbuf_pool_create("IPSEC_PERF_MBUFPOOL", NUM_MBUFS, MBUF_CACHE_SIZE,
  ...
  ring_inb_prepare = rte_ring_create("ipsec_test_ring_inb_prepare", RING_SIZE,
                                     SOCKET_ID_ANY, 0);
  if (ring_inb_prepare == NULL)
    return TEST_FAILED;

  ring_inb_process = rte_ring_create("ipsec_test_ring_inb_process", RING_SIZE,
                                     SOCKET_ID_ANY, 0);
  if (ring_inb_process == NULL)
    return TEST_FAILED;

  ring_outb_prepare = rte_ring_create("ipsec_test_ring_outb_prepare", RING_SIZE,
                                      SOCKET_ID_ANY, 0);
  if (ring_outb_prepare == NULL)
    return TEST_FAILED;

  ring_outb_process = rte_ring_create("ipsec_test_ring_outb_process", RING_SIZE,
                                      SOCKET_ID_ANY, 0);
  ...
  for (i = 0; i < NUM_MBUF; i++) {
    ...
    if (mbuf &&
        rte_ring_sp_enqueue_bulk(ring_inb_prepare, (void **)&mbuf, 1, NULL))
      continue;
    else
      return TEST_FAILED;
  }
...
static int measure_performance(struct ipsec_sa *sa_out,
...
static void print_metrics(const struct ipsec_test_cfg *test_cfg,
                          struct ipsec_sa *sa_out, struct ipsec_sa *sa_in) {
