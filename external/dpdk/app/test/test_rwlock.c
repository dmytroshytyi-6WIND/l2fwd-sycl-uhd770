...
static int test_rwlock_per_core(__rte_unused void *arg) {
...
static int load_loop_fn(__rte_unused void *arg) {
...
static int test_rwlock_perf(void) {
  unsigned int i;
  uint64_t total = 0;
...
static int try_read(uint32_t lc) {
  int32_t rc;
  uint32_t i;
  ...
  for (i = 0; i != RTE_DIM(try_rwlock_data.data.u64); i++) {
    ...
    if (try_rwlock_data.data.u64[i] != 0) {
      ...
      rte_memdump(stdout, NULL, (void *)(uintptr_t)&try_rwlock_data.data,
...
static int try_write(uint32_t lc) {
  int32_t rc;
  uint32_t i, v;
...
static int try_read_lcore(__rte_unused void *data) {
  int32_t rc;
  uint32_t i, lc;
  uint64_t ftm, stm, tm;
...
static int try_write_lcore(__rte_unused void *data) {
  int32_t rc;
  uint32_t i, lc;
  uint64_t ftm, stm, tm;
  struct try_rwlock_lcore *lcd;
...
static void print_try_lcore_stats(const struct try_rwlock_lcore *tlc,
                                  uint32_t lc) {
  uint64_t f, s;
  ...
  printf("try_lcore_data[%u]={\n"
         ...
         "\tsuccess/fail=%#Lf,\n"
         "};\n",
         lc, tlc->rc, tlc->type == LC_TYPE_RDLOCK ? "RDLOCK" : "WRLOCK",
         tlc->stat.fail, tlc->stat.success, tlc->stat.tick,
         (long double)tlc->stat.tick / (tlc->stat.fail + tlc->stat.success),
         (long double)tlc->stat.tick / s, (long double)tlc->stat.success / f);
}

static void collect_try_lcore_stats(struct try_rwlock_lcore *tlc,
 ...
 */
static int process_try_lcore_stats(void) {
  int32_t rc;
  uint32_t lc, rd, wr;
...
static void try_test_reset(void) {
...
static int try_rwlock_test_rda(void) {
  try_test_reset();
...
static int try_rwlock_test_rds_wrm(void) {
  try_test_reset();
...
static int try_rwlock_test_rde_wro(void) {
  uint32_t lc, mlc;
  ...
  RTE_LCORE_FOREACH(lc) {
    if (lc != mlc) {
      if ((lc & 1) == 0)
        rte_eal_remote_launch(try_read_lcore, NULL, lc);
      else
        rte_eal_remote_launch(try_write_lcore, NULL, lc);
    }
  }
  try_read_lcore(NULL);
  rte_eal_mp_wait_lcore();
