...
static int test_cirbuf_string_misc(void) {
  struct cirbuf cb;
...
static int test_cirbuf_string_add_del(void) {
  struct cirbuf cb;
  char buf[CMDLINE_TEST_BUFSIZE];
  char tmp[CMDLINE_TEST_BUFSIZE];
...
static int test_cirbuf_string_add_del_reverse(void) {
  struct cirbuf cb;
  char buf[CMDLINE_TEST_BUFSIZE];
  char tmp[CMDLINE_TEST_BUFSIZE];
...
static int test_cirbuf_string_add_boundaries(void) {
  struct cirbuf cb;
  char buf[CMDLINE_TEST_BUFSIZE];
  unsigned i;
  ...
  if (cirbuf_add_buf_tail(&cb, CIRBUF_STR_TAIL, sizeof(CIRBUF_STR_TAIL)) > 0) {
    printf("Error: buffer should have been full!\n");
    return -1;
  }
  /* try adding a string to head */
  if (cirbuf_add_buf_head(&cb, CIRBUF_STR_TAIL, sizeof(CIRBUF_STR_TAIL)) > 0) {
    printf("Error: buffer should have been full!\n");
    return -1;
  }
  ...
  /* try adding a string to head */
  if (cirbuf_add_buf_head(&cb, CIRBUF_STR_HEAD, sizeof(CIRBUF_STR_HEAD)) > 0) {
    printf("Error: buffer should have been full!\n");
    return -1;
  }
  /* try adding a string to tail */
  if (cirbuf_add_buf_tail(&cb, CIRBUF_STR_HEAD, sizeof(CIRBUF_STR_HEAD)) > 0) {
    printf("Error: buffer should have been full!\n");
    return -1;
  }
...
static int test_cirbuf_string_get_del_boundaries(void) {
  struct cirbuf cb;
  char buf[CMDLINE_TEST_BUFSIZE];
  char tmp[CMDLINE_TEST_BUFSIZE];
...
static int test_cirbuf_string_get_del_partial(void) {
  struct cirbuf cb;
  char buf[CMDLINE_TEST_BUFSIZE];
  char tmp[CMDLINE_TEST_BUFSIZE];
...
static int test_cirbuf_char_add_del(void) {
  struct cirbuf cb;
  char buf[CMDLINE_TEST_BUFSIZE];
  char tmp[CMDLINE_TEST_BUFSIZE];
...
static int test_cirbuf_char_fill(void) {
  struct cirbuf cb;
  char buf[CMDLINE_TEST_BUFSIZE];
  unsigned i;
...
/* test left alignment */
static int test_cirbuf_align_left(void) {
...
#define LEN4 HALF_OFFSET + SMALL_OFFSET - 1
  ...
  /* verify result */
  if (cb.start != 0 || cb.len != LEN4 || cb.end != cb.len - 1) {
    printf("Error: buffer alignment is wrong!");
    return -1;
  }
  ...
  if (cirbuf_add_buf_head(&cb, CIRBUF_STR_HEAD, sizeof(CIRBUF_STR_HEAD)) < 0 ||
      cirbuf_add_buf_tail(&cb, CIRBUF_STR_TAIL, sizeof(CIRBUF_STR_TAIL)) < 0) {
  ...
  /* get string from head */
  if (cirbuf_get_buf_head(
          &cb, tmp, sizeof(CIRBUF_STR_HEAD) + sizeof(CIRBUF_STR_TAIL)) < 0) {
...
static int test_cirbuf_align_right(void) {
#define END_OFFSET CMDLINE_TEST_BUFSIZE - 1
  ...
  /* verify result */
  if (cb.end != END_OFFSET || cb.len != LEN3 ||
      cb.start != cb.end - cb.len + 1) {
    printf("Error: buffer alignment is wrong!");
    return -1;
  }
  ...
  /* verify result */
  if (cb.end != END_OFFSET || cb.len != LEN4 ||
      cb.start != cb.end - cb.len + 1) {
    printf("Error: buffer alignment is wrong!");
    return -1;
  }
  ...
  /* add string to tail and head */
  if (cirbuf_add_buf_tail(&cb, CIRBUF_STR_TAIL, sizeof(CIRBUF_STR_TAIL)) < 0 ||
      cirbuf_add_buf_head(&cb, CIRBUF_STR_HEAD, sizeof(CIRBUF_STR_HEAD)) < 0) {
    printf("Error: failed to add strings!\n");
    return -1;
  }
