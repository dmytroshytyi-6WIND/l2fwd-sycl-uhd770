...
static void mytimer_reset(struct mytimerinfo *timinfo, uint64_t ticks,
                          enum rte_timer_type type, unsigned tim_lcore,
                          rte_timer_cb_t fct) {
  rte_timer_reset_sync(&timinfo->tim, ticks, type, tim_lcore, fct, timinfo);
}

/* timer callback for stress tests */
static void timer_stress_cb(__rte_unused struct rte_timer *tim,
  ...
  r = rte_rand();
  if ((r & 0xff) == 0) {
    mytimer_reset(&mytiminfo[0], hz, SINGLE, lcore_id, timer_stress_cb);
  } else if ((r & 0xff) == 1) {
    mytimer_reset(&mytiminfo[0], hz, SINGLE, rte_get_next_lcore(lcore_id, 0, 1),
                  timer_stress_cb);
  } else if ((r & 0xff) == 2) {
...
static int timer_stress_main_loop(__rte_unused void *arg) {
  uint64_t hz = rte_get_timer_hz();
  unsigned lcore_id = rte_lcore_id();
  uint64_t cur_time;
  int64_t diff = 0;
...
static void main_init_workers(void) {
  unsigned i;
...
static void main_start_workers(void) {
  unsigned i;
...
static void main_wait_for_workers(void) {
  unsigned i;
...
static void worker_wait_to_start(void) {
  unsigned lcore_id = rte_lcore_id();
 ...
 * on main lcore
 */
static void timer_stress2_cb(struct rte_timer *tim __rte_unused,
...
static int timer_stress2_main_loop(__rte_unused void *arg) {
  ...
  for (i = 0; i < NB_STRESS2_TIMERS; i++)
    rte_timer_reset(&timers[i], delay, SINGLE, main_lcore, timer_stress2_cb,
                    NULL);
  ...
  for (i = 0; i < 100000; i++) {
    ...
    rte_timer_reset(&timers[r], delay, SINGLE, main_lcore, timer_stress2_cb,
                    NULL);
  }
...
static void timer_basic_cb(struct rte_timer *tim, void *arg) {
  ...
  RTE_LOG(INFO, TESTTIMER, "%" PRIu64 ": callback id=%u count=%u on core %u\n",
  ...
  if (timinfo->id == 1 && timinfo->count < 10) {
    mytimer_reset(timinfo, hz * 2, SINGLE, rte_get_next_lcore(lcore_id, 0, 1),
                  timer_basic_cb);
    return;
  }
  ...
  if (timinfo->id == 2 && timinfo->count == 25) {
    ...
    mytimer_reset(&mytiminfo[0], hz, SINGLE, lcore_id, timer_basic_cb);
  }
}

static int timer_basic_main_loop(__rte_unused void *arg) {
  uint64_t hz = rte_get_timer_hz();
  unsigned lcore_id = rte_lcore_id();
  uint64_t cur_time;
  int64_t diff = 0;
  ...
  if (lcore_id == rte_get_main_lcore()) {
    mytimer_reset(&mytiminfo[0], hz / 4, SINGLE, lcore_id, timer_basic_cb);
    mytimer_reset(&mytiminfo[1], hz / 2, SINGLE, lcore_id, timer_basic_cb);
    mytimer_reset(&mytiminfo[2], hz / 4, PERIODICAL, lcore_id, timer_basic_cb);
    mytimer_reset(&mytiminfo[3], hz / 4, PERIODICAL,
                  rte_get_next_lcore(lcore_id, 0, 1), timer_basic_cb);
  }
