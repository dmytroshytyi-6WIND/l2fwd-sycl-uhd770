...
static void test_empty_pop(struct rte_stack *s) {
...
static int bulk_push_pop(void *p) {
 ...
 */
static void run_on_core_pair(struct lcore_pair *cores, struct rte_stack *s,
                             lcore_function_t fn) {
...
static void run_on_n_cores(struct rte_stack *s, lcore_function_t fn, int n) {
  ...
  for (i = 0; i < RTE_DIM(bulk_sizes); i++) {
    ...
    RTE_LCORE_FOREACH_WORKER(lcore_id) {
      ...
      if (rte_eal_remote_launch(fn, &args[lcore_id], lcore_id))
 ...
 * lcore.
 */
static void test_single_push_pop(struct rte_stack *s) {
...
static void test_bulk_push_pop(struct rte_stack *s) {
  ...
  for (sz = 0; sz < RTE_DIM(bulk_sizes); sz++) {
    ...
    double avg = ((double)(end - start) / (iterations * bulk_sizes[sz]));
...
static int __test_stack_perf(uint32_t flags) {
...
static int test_stack_perf(void) { return __test_stack_perf(0); }

static int test_lf_stack_perf(void) {
