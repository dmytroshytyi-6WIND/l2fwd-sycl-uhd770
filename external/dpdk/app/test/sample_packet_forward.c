...
int test_dev_start(uint16_t port, struct rte_mempool *mp) {
  ...
  rc = rte_eth_rx_queue_setup(port, 0, RING_SIZE, SOCKET_ID_ANY, NULL, mp);
  if (rc != 0)
    return rc;

  rc = rte_eth_tx_queue_setup(port, 0, RING_SIZE, SOCKET_ID_ANY, NULL);
  if (rc != 0)
    return rc;
...
int test_ring_setup(struct rte_ring **ring, uint16_t *portid) {
  *ring = rte_ring_create("R0", RING_SIZE, rte_socket_id(),
  ...
  if (*ring == NULL) {
    printf("%s() line %u: rte_ring_create R0 failed", __func__, __LINE__);
    return -1;
  }
  *portid = rte_eth_from_rings("net_ringa", ring, NUM_QUEUES, ring, NUM_QUEUES,
                               rte_socket_id());
...
void test_mp_free(struct rte_mempool *mp) { rte_mempool_free(mp); }

/* Sample test to free the virtual rings */
void test_ring_free(struct rte_ring *rxtx) { rte_ring_free(rxtx); }

/* Sample test to release the vdev */
void test_vdev_uninit(const char *vdev) { rte_vdev_uninit(vdev); }
...
int test_get_mbuf_from_pool(struct rte_mempool **mp, struct rte_mbuf **pbuf,
  ...
  if (rte_pktmbuf_alloc_bulk(*mp, pbuf, NUM_PACKETS) != 0) {
    printf("%s() line %u: rte_pktmbuf_alloc_bulk failed", __func__, __LINE__);
    return -1;
  }
  return 0;
}
...
int test_packet_forward(struct rte_mbuf **pbuf, uint16_t portid,
  ...
  if (rte_eth_tx_burst(portid, queue_id, pbuf, NUM_PACKETS) < NUM_PACKETS) {
    printf("%s() line %u: Error sending packet to"
           " port %d\n",
           __func__, __LINE__, portid);
    return -1;
  }
  if (rte_eth_rx_burst(portid, queue_id, pbuf, NUM_PACKETS) < NUM_PACKETS) {
