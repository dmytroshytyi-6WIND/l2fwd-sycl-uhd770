...
#define DMA_TEST_API_RUN(test) testsuite_run_test(test, #test)
...
static int testsuite_setup(int16_t dev_id) {
  test_dev_id = dev_id;
  invalid_dev_id = -1;
...
static void testsuite_run_test(int (*test)(void), const char *name) {
  int ret = 0;
...
static int test_dma_configure(void) {
...
static int check_direction(void) {
...
static int check_port_type(struct rte_dma_info *dev_info) {
  struct rte_dma_vchan_conf vchan_conf;
  int ret;
...
static int test_dma_vchan_setup(void) {
...
static int setup_one_vchan(void) {
  struct rte_dma_vchan_conf vchan_conf = {0};
  struct rte_dma_info dev_info = {0};
  struct rte_dma_conf dev_conf = {0};
  int ret;
...
static int test_dma_start_stop(void) {
  struct rte_dma_vchan_conf vchan_conf = {0};
  struct rte_dma_conf dev_conf = {0};
  int ret;
...
static int test_dma_stats(void) {
  struct rte_dma_info dev_info = {0};
...
static int verify_memory(void) {
  ...
  for (i = 0; i < TEST_MEMCPY_SIZE; i++) {
    if (src[i] == dst[i])
      continue;
    RTE_TEST_ASSERT_EQUAL(src[i], dst[i], "Failed to copy memory, %d %d",
                          src[i], dst[i]);
  }
...
static int test_dma_completed(void) {
  uint16_t last_idx = 1;
  bool has_error = true;
  uint16_t cpl_ret;
  int ret;
...
static int test_dma_completed_status(void) {
  ...
  RTE_TEST_ASSERT_EQUAL(ret, 0, "Failed to enqueue copy, %d", ret);
  rte_delay_us_sleep(TEST_WAIT_US_VAL);
  cpl_ret = rte_dma_completed_status(test_dev_id, 0, 1, &last_idx, status);
  RTE_TEST_ASSERT_EQUAL(cpl_ret, 1, "Failed to completed status");
  RTE_TEST_ASSERT_EQUAL(last_idx, 0, "Last idx should be zero, %u", last_idx);
  for (i = 0; i < RTE_DIM(status); i++)
    RTE_TEST_ASSERT_EQUAL(status[i], 0, "Failed to completed status, %d",
                          status[i]);
