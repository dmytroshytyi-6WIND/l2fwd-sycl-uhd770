...
static void test_empty_dequeue(void) {
  ...
  for (i = 0; i < iterations; i++)
    rte_eth_rx_burst(ring_ethdev_port, 0, (void *)burst, bulk_sizes[0]);
...
static void test_bulk_enqueue_dequeue(void) {
  ...
  for (sz = 0; sz < RTE_DIM(bulk_sizes); sz++) {
    const uint64_t sc_start = rte_rdtsc();
    for (i = 0; i < iterations; i++) {
      rte_ring_sp_enqueue_bulk(r, (void *)burst, bulk_sizes[sz], NULL);
      rte_ring_sc_dequeue_bulk(r, (void *)burst, bulk_sizes[sz], NULL);
    }
    const uint64_t sc_end = rte_rdtsc();
    ...
    double sc_avg =
        ((double)(sc_end - sc_start) / (iterations * bulk_sizes[sz]));
    double eth_avg =
        ((double)(eth_end - eth_start) / (iterations * bulk_sizes[sz]));
...
static int test_ring_pmd_perf(void) {
