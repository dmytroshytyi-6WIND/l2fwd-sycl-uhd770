...
static int get_hugepage_path(char *src, int src_len, char *dst, int dst_len) {
#define NUM_TOKENS 4
 ...
 */
static int process_hugefiles(const char *prefix, enum hugepage_action action) {
  ...
  const int prefix_len =
      snprintf(hugefile_prefix, sizeof(hugefile_prefix), "%smap_", prefix);
  ...
  while (fgets(line, sizeof(line), hugedir_handle) != NULL) {
    ...
    while ((dirent = readdir(hugepage_dir)) != NULL) {
      ...
      case HUGEPAGE_DELETE: {
        ...
        snprintf(file_path, sizeof(file_path), "%s/%s", hugedir,
                 dirent->d_name);
 ...
 */
static int get_number_of_sockets(void) {
  struct dirent *dirent = NULL;
...
static int test_allow_flag(void) {
  ...
  const char *wlinval[][7] = {
      {prgname, prefix, mp_flag, allow, "error", "", ""},
      {prgname, prefix, mp_flag, allow, "0:0:0", "", ""},
      {prgname, prefix, mp_flag, allow, "0:error:0.1", "", ""},
      {prgname, prefix, mp_flag, allow, "0:0:0.1error", "", ""},
      {prgname, prefix, mp_flag, allow, "error0:0:0.1", "", ""},
      {prgname, prefix, mp_flag, allow, "0:0:0.1.2", "", ""},
  };
  /* Test with valid allow option */
  const char *wlval1[] = {prgname, prefix, mp_flag, allow, "00FF:09:0B.3"};
  const char *wlval2[] = {prgname,   prefix, mp_flag,  allow,
                          "09:0B.3", allow,  "0a:0b.1"};
  const char *wlval3[] = {
      prgname,
      prefix,
      mp_flag,
...
static int test_invalid_b_flag(void) {
  ...
  const char *blval[] = {prgname, prefix, mp_flag, "-b", "FF:09:0B.3"};
...
static int test_invalid_vdev_flag(void) {
  ...
  const char *vdevinval[] = {prgname, prefix, no_huge, vdev, "eth_dummy"};

  /* Test with valid vdev option */
  const char *vdevval1[] = {prgname, prefix, no_huge, vdev, "net_ring0"};

  const char *vdevval2[] = {prgname, prefix, no_huge, vdev,
                            "net_ring0,args=test"};

  const char *vdevval3[] = {prgname, prefix, no_huge, vdev,
...
/*
 * Test that the app doesn't run without the coremask/corelist flags. In all
 * cases should give an error and fail to run
 */
static int test_missing_c_flag(void) {
#ifdef RTE_EXEC_ENV_FREEBSD
  /* BSD target doesn't support prefixes at this point */
  ...
  /* bad coremask value */
  const char *argv3[] = {prgname, prefix, mp_flag, "-c", "error"};
  /* sanity check of tests - valid coremask value */
  const char *argv4[] = {prgname, prefix, mp_flag, "-c", "1"};
  /* -l flag but no corelist value */
  const char *argv5[] = {prgname, prefix, mp_flag, "-l"};
  const char *argv6[] = {prgname, prefix, mp_flag, "-l", " "};
  /* bad corelist values */
  const char *argv7[] = {prgname, prefix, mp_flag, "-l", "error"};
  const char *argv8[] = {prgname, prefix, mp_flag, "-l", "1-"};
  const char *argv9[] = {prgname, prefix, mp_flag, "-l", "1,"};
  const char *argv10[] = {prgname, prefix, mp_flag, "-l", "1#2"};
  /* core number is negative value */
  const char *const argv11[] = {prgname, prefix, mp_flag, "-l", "-5"};
  const char *const argv12[] = {prgname, prefix, mp_flag, "-l", "-5-7"};
  /* core number is maximum value */
  const char *const argv13[] = {prgname, prefix, mp_flag, "-l",
                                RTE_STR(RTE_MAX_LCORE)};
  const char *const argv14[] = {prgname, prefix, mp_flag, "-l",
                                "1-" RTE_STR(RTE_MAX_LCORE)};
  /* sanity check test - valid corelist value */
  const char *const argv15[] = {prgname, prefix, mp_flag, "-l", "1-2,3"};

  /* --lcores flag but no lcores value */
  const char *const argv16[] = {prgname, prefix, mp_flag, "--lcores"};
  const char *const argv17[] = {prgname, prefix, mp_flag, "--lcores", " "};
  /* bad lcores value */
  const char *const argv18[] = {prgname, prefix, mp_flag, "--lcores", "1-3-5"};
  const char *const argv19[] = {prgname, prefix, mp_flag, "--lcores", "0-1,,2"};
  const char *const argv20[] = {prgname, prefix, mp_flag, "--lcores", "0-,1"};
  const char *const argv21[] = {prgname, prefix, mp_flag, "--lcores",
                                "(0-,2-4)"};
  const char *const argv22[] = {prgname, prefix, mp_flag, "--lcores", "(-1,2)"};
  const char *const argv23[] = {prgname, prefix, mp_flag, "--lcores",
                                "(2-4)@(2-4-6)"};
  const char *const argv24[] = {prgname, prefix, mp_flag, "--lcores", "(a,2)"};
  const char *const argv25[] = {prgname, prefix, mp_flag, "--lcores",
                                "1-3@(1,3)"};
  const char *const argv26[] = {prgname, prefix, mp_flag, "--lcores",
                                "3@((1,3)"};
  const char *const argv27[] = {prgname, prefix, mp_flag, "--lcores",
                                "(4-7)=(1,3)"};
  const char *const argv28[] = {prgname, prefix, mp_flag, "--lcores",
                                "[4-7]@(1,3)"};
  /* sanity check of tests - valid lcores value */
  const char *const argv29[] = {prgname, prefix, mp_flag, "--lcores",
                                "0-1,2@(5-7),(3-5)@(0,2),(0,6),7"};
  /* check an invalid cpu value >= CPU_SETSIZE */
  const char *const argv30[] = {prgname, prefix, mp_flag, "--lcores",
                                "3@" RTE_STR(CPU_SETSIZE)};
  ...
  if (launch_proc(argv1) == 0 || launch_proc(argv3) == 0) {
    printf("Error - "
  ...
  /* start -l test */
  if (launch_proc(argv5) == 0 || launch_proc(argv6) == 0 ||
      launch_proc(argv7) == 0 || launch_proc(argv8) == 0 ||
      launch_proc(argv9) == 0 || launch_proc(argv10) == 0 ||
      launch_proc(argv11) == 0 || launch_proc(argv12) == 0 ||
      launch_proc(argv13) == 0 || launch_proc(argv14) == 0) {
    printf("Error - "
...
static int test_main_lcore_flag(void) {
  ...
  const char *argv1[] = {prgname, prefix, mp_flag, "-c", "3", "--main-lcore"};
  /* --main-lcore flag with invalid value */
  const char *argv2[] = {prgname, prefix,         mp_flag, "-c",
                         "3",     "--main-lcore", "-1"};
  const char *argv3[] = {prgname, prefix,         mp_flag, "-c",
                         "3",     "--main-lcore", "X"};
  /* main lcore not in coremask */
  const char *argv4[] = {prgname, prefix,         mp_flag, "-c",
                         "3",     "--main-lcore", "2"};
  /* valid value */
  const char *argv5[] = {prgname, prefix,         mp_flag, "-c",
                         "3",     "--main-lcore", "1"};
  /* valid value set before coremask */
  const char *argv6[] = {prgname, prefix, mp_flag, "--main-lcore",
                         "1",     "-c",   "3"};

  if (launch_proc(argv1) == 0 || launch_proc(argv2) == 0 ||
      launch_proc(argv3) == 0 || launch_proc(argv4) == 0) {
...
static int test_invalid_n_flag(void) {
  ...
  /* -n flag but no value */
  const char *argv1[] = {prgname, prefix, no_huge, no_shconf, "-n"};
  /* bad numeric value */
  const char *argv2[] = {prgname, prefix, no_huge, no_shconf, "-n", "e"};
  /* zero is invalid */
  const char *argv3[] = {prgname, prefix, no_huge, no_shconf, "-n", "0"};
  /* sanity test - check with good value */
  const char *argv4[] = {prgname, prefix, no_huge, no_shconf, "-n", "2"};
...
static int test_no_huge_flag(void) {
  ...
  /* With --no-huge and -m */
  const char *argv2[] = {prgname, prefix, no_huge, "-m", DEFAULT_MEM_SIZE};
  ...
  const char *argv4[] = {prgname,          prefix,
                         no_huge,          "-m",
                         DEFAULT_MEM_SIZE, "--socket-mem=" DEFAULT_MEM_SIZE};
  if (launch_proc(argv1) != 0) {
...
static int test_misc_flags(void) {
  ...
  /* With valid --syslog */
  const char *argv3[] = {prgname, prefix, mp_flag, "--syslog", "syslog"};
  ...
  /* With --huge-dir */
  const char *argv7[] = {prgname,          "-m",
                         DEFAULT_MEM_SIZE, "--file-prefix=hugedir",
                         "--huge-dir",     hugepath};
  ...
  const char *argv9[] = {prgname,          "-m",
                         DEFAULT_MEM_SIZE, "--file-prefix=hugedir",
                         "--huge-dir",     "invalid"};
  /* With invalid --huge-dir sub-directory */
  const char *argv10[] = {prgname,          "-m",
                          DEFAULT_MEM_SIZE, "--file-prefix=hugedir",
                          "--huge-dir",     hugepath_dir};
  /* With valid --huge-dir sub-directory */
  const char *argv11[] = {prgname,          "-m",
                          DEFAULT_MEM_SIZE, "--file-prefix=hugedir",
                          "--huge-dir",     hugepath_dir2};
  /* Secondary process with invalid --huge-dir (should run as flag has no
   * effect on secondary processes) */
  const char *argv12[] = {prgname, prefix, mp_flag, "--huge-dir", "invalid"};
  ...
  const char *argv14[] = {prgname, "--file-prefix=intr", "--vfio-intr=legacy"};

  /* try running with --vfio-intr MSI flag */
  const char *argv15[] = {prgname, "--file-prefix=intr", "--vfio-intr=msi"};

  /* try running with --vfio-intr MSI-X flag */
  const char *argv16[] = {prgname, "--file-prefix=intr", "--vfio-intr=msix"};

  /* try running with --vfio-intr invalid flag */
  const char *argv17[] = {prgname, "--file-prefix=intr", "--vfio-intr=invalid"};
  ...
  const char *const argv19[] = {prgname, "--proc-type=auto", no_shconf,
                                nosh_prefix, no_huge};
  ...
  if (launch_proc(argv12) != 0) {
    printf("Error - secondary process did not run ok with invalid --huge-dir "
           "flag\n");
    goto fail;
  }
...
static int test_file_prefix(void) {
  ...
  const char *argv0[] = {prgname, mp_flag, "-m", DEFAULT_MEM_SIZE,
                         "--file-prefix=" memtest};

  /* primary process with memtest1 and default mem mode */
  const char *argv1[] = {prgname, "-m", DEFAULT_MEM_SIZE,
                         "--file-prefix=" memtest1};

  /* primary process with memtest1 and legacy mem mode */
  const char *argv2[] = {prgname, "-m", DEFAULT_MEM_SIZE,
                         "--file-prefix=" memtest1, "--legacy-mem"};

  /* primary process with memtest2 and legacy mem mode */
  const char *argv3[] = {prgname, "-m", DEFAULT_MEM_SIZE,
                         "--file-prefix=" memtest2, "--legacy-mem"};

  /* primary process with memtest2 and default mem mode */
  const char *argv4[] = {prgname, "-m", DEFAULT_MEM_SIZE,
                         "--file-prefix=" memtest2};

  /* primary process with --in-memory mode */
  const char *const argv5[] = {prgname, "-m", DEFAULT_MEM_SIZE, "--in-memory"};

  /* primary process with memtest1 and --in-memory mode */
  const char *const argv6[] = {prgname, "-m", DEFAULT_MEM_SIZE, "--in-memory",
                               "--file-prefix=" memtest1};

  /* primary process with parent file-prefix and --in-memory mode */
  const char *const argv7[] = {
      prgname, "-m", DEFAULT_MEM_SIZE, "--in-memory", "--file-prefix", prefix};

  /* primary process with memtest1 and --single-file-segments mode */
  const char *const argv8[] = {prgname, "-m", DEFAULT_MEM_SIZE,
                               "--single-file-segments",
                               "--file-prefix=" memtest1};

  /* primary process with memtest1 and --huge-unlink=never mode */
  const char *const argv9[] = {prgname, "-m", DEFAULT_MEM_SIZE,
                               "--huge-unlink=never",
                               "--file-prefix=" memtest1};
 ...
 */
static void populate_socket_mem_param(int num_sockets, const char *mem,
  ...
  for (i = 0; i < num_sockets - 1; i++) {
    written = snprintf(&buf[offset], buf_size - offset, "%s,", mem);
    if (written < 0 || written + offset >= buf_size)
      return;
    offset += written;
  }

  written = snprintf(&buf[offset], buf_size - offset, "%s%s", mem, suffix);
  if (written < 0 || written + offset >= buf_size)
    return;
  offset += written;
}
...
static int test_memory_flags(void) {
  ...
  const char *argv0[] = {prgname, prefix, mp_flag, "-m", DEFAULT_MEM_SIZE};

  /* valid -m flag */
  const char *argv1[] = {prgname, "--file-prefix=" memtest, "-m",
                         DEFAULT_MEM_SIZE};

  /* valid (zero) --socket-mem flag */
  char arg2_socket_mem[SOCKET_MEM_STRLEN];
  const char *argv2[] = {prgname, "--file-prefix=" memtest, arg2_socket_mem};

  /* invalid (incomplete) --socket-mem flag */
  char arg3_socket_mem[SOCKET_MEM_STRLEN];
  const char *argv3[] = {prgname, "--file-prefix=" memtest, arg3_socket_mem};

  /* invalid (mixed with invalid data) --socket-mem flag */
  char arg4_socket_mem[SOCKET_MEM_STRLEN];
  const char *argv4[] = {prgname, "--file-prefix=" memtest, arg4_socket_mem};

  /* invalid (with numeric value as last character) --socket-mem flag */
  char arg5_socket_mem[SOCKET_MEM_STRLEN];
  const char *argv5[] = {prgname, "--file-prefix=" memtest, arg5_socket_mem};

  /* invalid (with empty socket) --socket-mem flag */
  char arg6_socket_mem[SOCKET_MEM_STRLEN];
  const char *argv6[] = {prgname, "--file-prefix=" memtest, arg6_socket_mem};

  /* invalid (null) --socket-mem flag */
  const char *argv7[] = {prgname, "--file-prefix=" memtest, "--socket-mem="};

  /* valid --socket-mem specified together with -m flag */
  char arg8_socket_mem[SOCKET_MEM_STRLEN];
  const char *argv8[] = {prgname, "--file-prefix=" memtest, "-m",
                         DEFAULT_MEM_SIZE, arg8_socket_mem};

#ifdef RTE_EXEC_ENV_FREEBSD
  int num_sockets = 1;
#else
  int num_sockets = RTE_MIN(get_number_of_sockets(), RTE_MAX_NUMA_NODES);
#endif
  ...
  const char *argv9[] = {prgname, "--file-prefix=" memtest, invalid_socket_mem};

  /* valid --socket-mem flag */
  char valid_socket_mem[SOCKET_MEM_STRLEN];
  const char *argv10[] = {prgname, "--file-prefix=" memtest, valid_socket_mem};
  ...
  populate_socket_mem_param(num_sockets, "0", "", arg2_socket_mem,
                            sizeof(arg2_socket_mem));
  if (launch_proc(argv2) != 0) {
  ...
  if (num_sockets > 1) {
    populate_socket_mem_param(num_sockets - 1, "2", ",", arg3_socket_mem,
                              sizeof(arg3_socket_mem));
    if (launch_proc(argv3) == 0) {
    ...
    populate_socket_mem_param(num_sockets - 1, "2", ",Fred", arg4_socket_mem,
                              sizeof(arg4_socket_mem));
    if (launch_proc(argv4) == 0) {
      printf("Error - process run ok with invalid "
    ...
    populate_socket_mem_param(num_sockets - 1, "2", ",Fred0", arg5_socket_mem,
                              sizeof(arg5_socket_mem));
    if (launch_proc(argv5) == 0) {
      printf("Error - process run ok with invalid "
  ...
  if (num_sockets > 2) {
    populate_socket_mem_param(num_sockets - 2, "2", ",,2", arg6_socket_mem,
                              sizeof(arg6_socket_mem));
    if (launch_proc(argv6) == 0) {
      printf("Error - process run ok with invalid "
  ...
  populate_socket_mem_param(num_sockets, "2", "", arg8_socket_mem,
                            sizeof(arg8_socket_mem));
  if (launch_proc(argv8) == 0) {
    printf("Error - process run ok with --socket-mem and -m specified!\n");
    return -1;
  }

  populate_socket_mem_param(num_sockets + 1, "2", "", invalid_socket_mem,
  ...
  populate_socket_mem_param(num_sockets, "2", "", valid_socket_mem,
