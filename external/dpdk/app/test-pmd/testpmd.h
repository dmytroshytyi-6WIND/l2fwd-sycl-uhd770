...
#define RTE_CACHE_LINE_SIZE_ROUNDUP(size)                                      \
  (RTE_CACHE_LINE_SIZE *                                                       \
   ((size + RTE_CACHE_LINE_SIZE - 1) / RTE_CACHE_LINE_SIZE))
...
struct rte_port {
  ...
  uint16_t
      nb_rx_desc[RTE_MAX_QUEUES_PER_PORT + 1]; /**< per queue rx desc number */
  uint16_t
      nb_tx_desc[RTE_MAX_QUEUES_PER_PORT + 1]; /**< per queue tx desc number */
  struct rte_eth_rxconf
      rx_conf[RTE_MAX_QUEUES_PER_PORT + 1]; /**< per queue rx configuration */
  struct rte_eth_txconf
      tx_conf[RTE_MAX_QUEUES_PER_PORT + 1]; /**< per queue tx configuration */
 ...
 * DCB mode enable
 */
enum dcb_mode_enable { DCB_VT_ENABLED, DCB_ENABLED };
...
extern uint32_t
    burst_tx_delay_time; /**< Burst tx delay time(us) for mac-retry. */
extern uint32_t burst_tx_retry_num; /**< Burst tx retry number for mac-retry. */

#ifdef RTE_LIB_GRO
#define GRO_DEFAULT_ITEM_NUM_PER_FLOW 32
#define GRO_DEFAULT_FLOW_NUM                                                   \
  (RTE_GRO_MAX_BURST_ITEM_NUM / GRO_DEFAULT_ITEM_NUM_PER_FLOW)
...
static inline bool is_proc_primary(void) {
  return rte_eal_process_type() == RTE_PROC_PRIMARY;
}

static inline unsigned int lcore_num(void) {
  unsigned int i;
...
static inline struct fwd_lcore *current_fwd_lcore(void) {
  return fwd_lcores[lcore_num()];
}

/* Mbuf Pools */
static inline void mbuf_poolname_build(unsigned int sock_id, char *mp_name,
                                       int name_size, uint16_t idx) {
  if (!idx)
    snprintf(mp_name, name_size, MBUF_POOL_NAME_PFX "_%u", sock_id);
  else
    snprintf(mp_name, name_size, MBUF_POOL_NAME_PFX "_%hu_%hu",
             (uint16_t)sock_id, idx);
}

static inline struct rte_mempool *mbuf_pool_find(unsigned int sock_id,
                                                 uint16_t idx) {
 ...
 */
static inline uint32_t port_pci_reg_read(struct rte_port *port,
                                         uint32_t reg_off) {
...
static inline void port_pci_reg_write(struct rte_port *port, uint32_t reg_off,
                                      uint32_t reg_v) {
  const struct rte_pci_device *pci_dev;
  const struct rte_bus *bus;
  void *reg_addr;
...
static inline void get_start_cycles(uint64_t *start_tsc) {
  if (record_core_cycles)
    *start_tsc = rte_rdtsc();
}

static inline void get_end_cycles(struct fwd_stream *fs, uint64_t start_tsc) {
  if (record_core_cycles)
    fs->core_cycles += rte_rdtsc() - start_tsc;
}

static inline void inc_rx_burst_stats(struct fwd_stream *fs, uint16_t nb_rx) {
  if (record_burst_stats)
    fs->rx_burst_stats.pkt_burst_spread[nb_rx]++;
}

static inline void inc_tx_burst_stats(struct fwd_stream *fs, uint16_t nb_tx) {
  if (record_burst_stats)
...
int port_flow_dump(portid_t port_id, bool dump_all, uint32_t rule,
...
int eth_dev_conf_get_print_err(uint16_t port_id, struct rte_eth_conf *dev_conf);
...
enum print_warning { ENABLED_WARN = 0, DISABLED_WARN };
