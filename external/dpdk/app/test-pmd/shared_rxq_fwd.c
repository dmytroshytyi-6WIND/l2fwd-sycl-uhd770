 ...
 */
static void forward_rx_only(uint16_t nb_rx, struct rte_mbuf **pkts_burst) {
 ...
 */
static struct fwd_stream *forward_stream_get(struct fwd_stream *fs,
                                             uint16_t port) {
  streamid_t sm_id;
  struct fwd_lcore *fc;
  ...
  for (sm_id = 0; sm_id < nb_fs; sm_id++) {
    if (fsm[sm_id]->rx_port == port && fsm[sm_id]->rx_queue == fs->rx_queue)
      return fsm[sm_id];
  }
  return NULL;
}
 ...
 */
static void forward_sub_burst(struct fwd_stream *src_fs, uint16_t port,
                              uint16_t nb_rx, struct rte_mbuf **pkts) {
 ...
 */
static void forward_shared_rxq(struct fwd_stream *fs, uint16_t nb_rx,
...
static void shared_rxq_fwd(struct fwd_stream *fs) {
