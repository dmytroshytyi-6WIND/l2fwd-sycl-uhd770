...
static void usage(char *progname) {
  ...
  printf("  --flowgen-clones=N: set the number of single packet clones to send "
         "in flowgen mode. Should be less than burst value.\n");
  printf("  --flowgen-flows=N: set the number of flows in flowgen mode to N (1 "
         "<= N <= INT32_MAX).\n");
  ...
  printf("  --rxfreet=N: set the free threshold of RX descriptors to N "
         "(0 <= N < value of rxd).\n");
  printf(
      "  --rxwt=N: set the write-back threshold register of RX rings to N.\n");
  ...
  printf(
      "  --txwt=N: set the write-back threshold register of TX rings to N.\n");
  ...
  printf("  --print-event "
         "<unknown|intr_lsc|queue_state|intr_reset|vf_mbox|macsec|intr_rmv|"
         "flow_aged|all>: "
  ...
  printf(
      "  --geneve-parsed-port=N: UPD port to parse GENEVE tunnel protocol\n");
#ifndef RTE_EXEC_ENV_WINDOWS
  printf("  --mlockall: lock all memory\n");
  printf("  --no-mlockall: do not lock all memory\n");
#endif
  printf(
      "  --mp-alloc <native|anon|xmem|xmemhuge>: mempool allocation method.\n"
      "    native: use regular DPDK memory to create and populate mempool\n"
      "    anon: use regular DPDK memory to create and anonymous memory to "
      "populate mempool\n"
      "    xmem: use anonymous memory to create and populate mempool\n"
      "    xmemhuge: use anonymous hugepage memory to create and populate "
      "mempool\n");
  ...
  printf("  --noisy-lkup-num-reads-writes=N: do N random reads and writes per "
         "packet\n");
...
#ifdef RTE_LIB_CMDLINE
static int init_peer_eth_addrs(const char *config_filename) {
  FILE *config_file;
  portid_t i;
  char buf[50];
 ...
 */
static void parse_fwd_coremask(const char *coremask) {
  char *end;
 ...
 * the global configuration of forwarding cores.
 */
static void parse_fwd_portmask(const char *portmask) {
  char *end;
...
static void print_invalid_socket_id_error(void) {
  unsigned int i = 0;

  fprintf(stderr, "Invalid socket id, options are: ");
  for (i = 0; i < num_sockets; i++) {
    fprintf(stderr, "%u%s", socket_ids[i], (i == num_sockets - 1) ? "\n" : ",");
  }
}

static int parse_portnuma_config(const char *q_arg) {
  char s[256];
  ...
  portid_t port_id;
  unsigned size;
  enum fieldnames { FLD_PORT = 0, FLD_SOCKET, _NUM_FLD };
...
static int parse_ringnuma_config(const char *q_arg) {
  char s[256];
  const char *p, *p0 = q_arg;
  char *end;
  ...
  portid_t port_id;
  unsigned size;
  enum fieldnames { FLD_PORT = 0, FLD_FLAG, FLD_SOCKET, _NUM_FLD };
  unsigned long int_fld[_NUM_FLD];
  char *str_fld[_NUM_FLD];
...
#define RXTX_RING 0x3
  ...
  while ((p = strchr(p0, '(')) != NULL) {
    ...
    if ((ring_flag < RX_RING_ONLY) || (ring_flag > RXTX_RING)) {
      fprintf(stderr, "Invalid ring-flag=%d config for port =%d\n", ring_flag,
              port_id);
      return -1;
    }
...
static int parse_event_printing_config(const char *optarg, int enable) {
  uint32_t mask = 0;
...
static int parse_xstats_list(const char *in_str,
...
static int parse_link_speed(int n) {
...
#define SHORTOPTS ""
#endif
  while ((opt = getopt_long(argc, argvopt, SHORTOPTS "ah", lgopts, &opt_idx)) !=
         EOF) {
    switch (opt) {
#ifdef RTE_LIB_CMDLINE
    case 'i':
    ...
    case 0: /*long options */
      ...
      if (!strcmp(lgopts[opt_idx].name, "cmdline-file")) {
        ...
        strlcpy(cmdline_filename, optarg, sizeof(cmdline_filename));
      }
      ...
      if (!strcmp(lgopts[opt_idx].name, "display-xstats")) {
        ...
        rc = parse_xstats_list(optarg, &xstats_display, &xstats_display_num);
        if (rc != 0)
          rte_exit(EXIT_FAILURE,
                   "Failed to parse display-xstats argument: %d\n", rc);
      }
      if (!strcmp(lgopts[opt_idx].name, "eth-peers-configfile")) {
        if (init_peer_eth_addrs(optarg) != 0)
          rte_exit(EXIT_FAILURE, "Cannot open logfile\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "eth-peer")) {
        ...
        if (n >= RTE_MAX_ETHPORTS)
          rte_exit(EXIT_FAILURE, "eth-peer: port %d >= RTE_MAX_ETHPORTS(%d)\n",
                   n, RTE_MAX_ETHPORTS);

        if (rte_ether_unformat_addr(port_end, &peer_eth_addrs[n]) < 0)
      ...
      if (!strcmp(lgopts[opt_idx].name, "tx-ip")) {
        ...
        if (end == optarg || !end)
          rte_exit(EXIT_FAILURE, "Invalid tx-ip: %s", optarg);
        ...
        *end++ = 0;
        ...
        if (inet_pton(AF_INET, end, &in) == 0)
          rte_exit(EXIT_FAILURE, "Invalid destination IP address: %s\n",
                   optarg);
      ...
      if (!strcmp(lgopts[opt_idx].name, "tx-udp")) {
        ...
        if (errno != 0 || end == optarg || n > UINT16_MAX ||
            !(*end == '\0' || *end == ','))
          rte_exit(EXIT_FAILURE, "Invalid UDP port: %s\n", optarg);
        tx_udp_src_port = n;
        if (*end == ',') {
          char *dst = end + 1;

          n = strtoul(dst, &end, 10);
          if (errno != 0 || end == dst || n > UINT16_MAX || *end)
            rte_exit(EXIT_FAILURE, "Invalid destination UDP port: %s\n", dst);
          tx_udp_dst_port = n;
        } else {
          tx_udp_dst_port = n;
        }
      ...
      if (!strcmp(lgopts[opt_idx].name, "nb-cores")) {
        ...
        if (n > 0 && n <= nb_lcores)
          ...
          rte_exit(EXIT_FAILURE, "nb-cores should be > 0 and <= %d\n",
                   nb_lcores);
      }
      ...
      if (!strcmp(lgopts[opt_idx].name, "port-numa-config")) {
        if (parse_portnuma_config(optarg))
          rte_exit(EXIT_FAILURE, "invalid port-numa configuration\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "ring-numa-config"))
        if (parse_ringnuma_config(optarg))
          rte_exit(EXIT_FAILURE, "invalid ring-numa configuration\n");
      if (!strcmp(lgopts[opt_idx].name, "socket-num")) {
        ...
        if (!new_socket_id((uint8_t)n)) {
          ...
          print_invalid_socket_id_error();
          rte_exit(EXIT_FAILURE, "Invalid socket id");
        }
      }
      if (!strcmp(lgopts[opt_idx].name, "mbuf-size")) {
        ...
        nb_segs = parse_item_list(optarg, "mbuf-size", MAX_SEGS_BUFFER_SPLIT,
                                  mb_sz, 0);
        if (nb_segs <= 0)
          rte_exit(EXIT_FAILURE, "bad mbuf-size\n");
        for (i = 0; i < nb_segs; i++) {
          if (mb_sz[i] <= 0 || mb_sz[i] > 0xFFFF)
            rte_exit(EXIT_FAILURE, "mbuf-size should be "
                                   "> 0 and < 65536\n");
      ...
      if (!strcmp(lgopts[opt_idx].name, "total-num-mbufs")) {
        ...
        if (n > 1024)
          ...
          rte_exit(EXIT_FAILURE, "total-num-mbufs should be > 1024\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "max-pkt-len")) {
        ...
        if (n >= RTE_ETHER_MIN_LEN)
          ...
          rte_exit(EXIT_FAILURE, "Invalid max-pkt-len=%d - should be > %d\n", n,
                   RTE_ETHER_MIN_LEN);
      }
      ...
      if (!strcmp(lgopts[opt_idx].name, "pkt-filter-report-hash")) {
      ...
      if (!strcmp(lgopts[opt_idx].name, "pkt-filter-size")) {
        if (!strcmp(optarg, "64K"))
          fdir_conf.pballoc = RTE_ETH_FDIR_PBALLOC_64K;
        else if (!strcmp(optarg, "128K"))
          fdir_conf.pballoc = RTE_ETH_FDIR_PBALLOC_128K;
        else if (!strcmp(optarg, "256K"))
          fdir_conf.pballoc = RTE_ETH_FDIR_PBALLOC_256K;
        else
          rte_exit(EXIT_FAILURE,
                   "pkt-filter-size %s invalid -"
                   " must be: 64K or 128K or 256K\n",
                   optarg);
      }
      if (!strcmp(lgopts[opt_idx].name, "pkt-filter-drop-queue")) {
        n = atoi(optarg);
        if (n >= 0)
...
#ifdef RTE_LIB_LATENCYSTATS
      if (!strcmp(lgopts[opt_idx].name, "latencystats")) {
        n = atoi(optarg);
        if (n >= 0) {
      ...
      if (!strcmp(lgopts[opt_idx].name, "enable-hw-qinq-strip"))
      ...
      if (!strcmp(lgopts[opt_idx].name, "rxq")) {
        ...
        if (n >= 0 && check_nb_rxq((queueid_t)n) == 0)
          ...
          rte_exit(EXIT_FAILURE,
                   "rxq %d invalid - must be"
                   " >= 0 && <= %u\n",
                   n, get_allowed_max_nb_rxq(&pid));
      }
      if (!strcmp(lgopts[opt_idx].name, "txq")) {
        ...
        if (n >= 0 && check_nb_txq((queueid_t)n) == 0)
          ...
          rte_exit(EXIT_FAILURE,
                   "txq %d invalid - must be"
                   " >= 0 && <= %u\n",
                   n, get_allowed_max_nb_txq(&pid));
      }
      if (!strcmp(lgopts[opt_idx].name, "hairpinq")) {
        n = atoi(optarg);
        if (n >= 0 && check_nb_hairpinq((queueid_t)n) == 0)
          nb_hairpinq = (queueid_t)n;
        else
          rte_exit(EXIT_FAILURE,
                   "txq %d invalid - must be"
                   " >= 0 && <= %u\n",
                   n, get_allowed_max_nb_hairpinq(&pid));
        if ((n + nb_txq) < 0 || check_nb_txq((queueid_t)(n + nb_txq)) != 0)
          rte_exit(EXIT_FAILURE,
                   "txq + hairpinq "
                   "%d invalid - must be"
                   " >= 0 && <= %u\n",
                   n + nb_txq, get_allowed_max_nb_txq(&pid));
        if ((n + nb_rxq) < 0 || check_nb_rxq((queueid_t)(n + nb_rxq)) != 0)
          rte_exit(EXIT_FAILURE,
                   "rxq + hairpinq "
                   "%d invalid - must be"
                   " >= 0 && <= %u\n",
                   n + nb_rxq, get_allowed_max_nb_rxq(&pid));
      }
      if (!nb_rxq && !nb_txq) {
      ...
      if (!strcmp(lgopts[opt_idx].name, "burst")) {
        ...
        if (n == 0) {
          ...
          rte_exit(EXIT_FAILURE, "burst must be between1 and %d\n",
                   MAX_PKT_BURST);
        else
      ...
      if (!strcmp(lgopts[opt_idx].name, "flowgen-clones")) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "clones must be >= 0 and <= current burst\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "flowgen-flows")) {
        ...
        if (n > 0)
          ...
          rte_exit(EXIT_FAILURE, "flows must be >= 1\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "mbcache")) {
        n = atoi(optarg);
        if ((n >= 0) && (n <= RTE_MEMPOOL_CACHE_MAX_SIZE))
      ...
      if (!strcmp(lgopts[opt_idx].name, "rxd")) {
        ...
        if (n > 0) {
          ...
          rte_exit(EXIT_FAILURE, "rxd(%d) invalid - must be > 0\n", n);
      }
      ...
      if (!strcmp(lgopts[opt_idx].name, "rxoffs")) {
        ...
        nb_offs = parse_item_list(optarg, "rxpkt offsets",
                                  MAX_SEGS_BUFFER_SPLIT, seg_off, 0);
        if (nb_offs > 0)
      ...
      if (!strcmp(lgopts[opt_idx].name, "rxpkts")) {
        ...
        nb_segs = parse_item_list(optarg, "rxpkt segments",
                                  MAX_SEGS_BUFFER_SPLIT, seg_len, 0);
        if (nb_segs > 0)
      ...
      if (!strcmp(lgopts[opt_idx].name, "tx-offloads")) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "tx-offloads must be >= 0\n");
      }
      ...
      if (!strcmp(lgopts[opt_idx].name, "rx-offloads")) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "rx-offloads must be >= 0\n");
      }
      ...
      if (!strcmp(lgopts[opt_idx].name, "vxlan-gpe-port")) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "vxlan-gpe-port must be >= 0\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "geneve-parsed-port")) {
        n = atoi(optarg);
        if (n >= 0)
          geneve_udp_port = (uint16_t)n;
        else
          rte_exit(EXIT_FAILURE, "geneve-parsed-port must be >= 0\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "print-event"))
        if (parse_event_printing_config(optarg, 1)) {
          rte_exit(EXIT_FAILURE, "invalid print-event argument\n");
        }
      if (!strcmp(lgopts[opt_idx].name, "mask-event"))
        if (parse_event_printing_config(optarg, 0)) {
          rte_exit(EXIT_FAILURE, "invalid mask-event argument\n");
        }
      ...
      if (!strcmp(lgopts[opt_idx].name, "noisy-tx-sw-buffer-size")) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "noisy-tx-sw-buffer-size must be >= 0\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "noisy-tx-sw-buffer-flushtime")) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "noisy-tx-sw-buffer-flushtime must be >= 0\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "noisy-lkup-memory")) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "noisy-lkup-memory must be >= 0\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "noisy-lkup-num-writes")) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "noisy-lkup-num-writes must be >= 0\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "noisy-lkup-num-reads")) {
        n = atoi(optarg);
        if (n >= 0)
          noisy_lkup_num_reads = n;
        else
          rte_exit(EXIT_FAILURE, "noisy-lkup-num-reads must be >= 0\n");
      }
      if (!strcmp(lgopts[opt_idx].name, "noisy-lkup-num-reads-writes")) {
        ...
        if (n >= 0)
          ...
          rte_exit(EXIT_FAILURE, "noisy-lkup-num-reads-writes must be >= 0\n");
      }
      ...
      if (!strcmp(lgopts[opt_idx].name, "rx-mq-mode")) {
        ...
        if (n >= 0 && n <= RTE_ETH_MQ_RX_VMDQ_DCB_RSS)
          ...
          rte_exit(EXIT_FAILURE, "rx-mq-mode must be >= 0 and <= %d\n",
    ...
    default:
      ...
      rte_exit(EXIT_FAILURE, "Command line is incomplete or incorrect\n");
      break;
    }
  }
  ...
  if (proc_id >= (int)num_procs)
    rte_exit(EXIT_FAILURE,
             ...
             PARAM_PROC_ID, proc_id, PARAM_NUM_PROCS, num_procs);
