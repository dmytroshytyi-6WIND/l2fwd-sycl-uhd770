...
skip_parse:
  while ((next = rte_bus_find(start, bus_match_all, NULL)) != NULL) {
    ...
    RTE_DEV_FOREACH(dev, devstr, &dev_iter) {
      ...
      if (identifier && strncmp(da.name, dev->name, strlen(dev->name)))
        continue;
      printf("\n%s Infos for device %s %s\n", info_border, dev->name,
             info_border);
      printf("Bus name: %s", dev->bus->name);
      printf("\nDriver name: %s", dev->driver->name);
      printf("\nDevargs: %s", dev->devargs ? dev->devargs->args : "");
...
static int port_reg_off_is_invalid(portid_t port_id, uint32_t reg_off) {
  ...
  if (reg_off >= pci_len) {
    fprintf(stderr,
            "Port %d: register offset %u (0x%X) out of port PCI resource "
            "(length=%" PRIu64 ")\n",
            port_id, (unsigned int)reg_off, (unsigned int)reg_off, pci_len);
    return 1;
  }
  return 0;
}

static int reg_bit_pos_is_invalid(uint8_t bit_pos) {
  if (bit_pos <= 31)
    return 0;
...
static inline void display_port_reg_value(portid_t port_id, uint32_t reg_off,
                                          uint32_t reg_v) {
...
void port_reg_bit_field_set(portid_t port_id, uint32_t reg_off,
  ...
  if (value > max_v) {
    fprintf(stderr, "Invalid value %u (0x%x) must be < %u (0x%x)\n",
            (unsigned)value, (unsigned)value, (unsigned)max_v, (unsigned)max_v);
    return;
  }
  reg_v = port_id_pci_reg_read(port_id, reg_off);
...
void port_reg_set(portid_t port_id, uint32_t reg_off, uint32_t reg_v) {
  if (port_id_is_invalid(port_id, ENABLED_WARN))
    return;
  if (port_reg_off_is_invalid(port_id, reg_off))
    return;
...
static void rss_config_display(struct rte_flow_action_rss *rss_conf) {
  ...
  for (i = 0; rss_type_table[i].str; i++) {
    if ((rss_conf->types & rss_type_table[i].rss_type) ==
...
int port_action_handle_query(portid_t port_id, uint32_t id) {
  ...
  case RTE_FLOW_ACTION_TYPE_CONNTRACK:
    printf("Conntrack Context:\n"
           ...
           query.ct.peer_port, query.ct.is_original_dir ? "Original" : "Reply",
           query.ct.enable, query.ct.live_connection, query.ct.selective_ack,
...
int port_queue_flow_create(portid_t port_id, queueid_t queue_id, bool postpone,
                           uint32_t table_id, uint32_t pattern_idx,
                           uint32_t actions_idx,
                           const struct rte_flow_item *pattern,
                           const struct rte_flow_action *actions) {
  ...
  /* Poisoning to make sure PMDs update it in case of error. */
  memset(&error, 0x11, sizeof(error));
  flow = rte_flow_async_create(port_id, queue_id, &op_attr, pt->table, pattern,
                               pattern_idx, actions, actions_idx, NULL, &error);
  if (!flow) {
...
int port_queue_flow_destroy(portid_t port_id, queueid_t queue_id, bool postpone,
  ...
  while (*tmp) {
    ...
    for (i = 0; i != n; ++i) {
       ...
       */
      memset(&error, 0x33, sizeof(error));
      if (rte_flow_async_destroy(port_id, queue_id, &op_attr, pf->flow, NULL,
                                 &error)) {
        ret = port_flow_complain(&error);
        continue;
      }
...
int port_queue_action_handle_destroy(portid_t port_id, uint32_t queue_id,
  ...
  while (*tmp) {
    ...
    for (i = 0; i != n; ++i) {
      ...
      if (pia->handle &&
          rte_flow_async_action_handle_destroy(port_id, queue_id, &attr,
                                               pia->handle, NULL, &error)) {
        ret = port_flow_complain(&error);
        continue;
      }
      *tmp = pia->next;
...
int port_queue_action_handle_update(portid_t port_id, uint32_t queue_id,
  ...
  if (rte_flow_async_action_handle_update(
          port_id, queue_id, &attr, action_handle, action, NULL, &error)) {
    return port_flow_complain(&error);
  }
...
void port_flow_list(portid_t port_id, uint32_t n, const uint32_t *group) {
  ...
  for (pf = list; pf != NULL; pf = pf->tmp) {
    ...
    printf("%" PRIu32 "\t%" PRIu32 "\t%" PRIu32 "\t%c%c%c\t", pf->id,
...
void rxtx_config_display(void) {
  ...
  printf("  %s packet forwarding%s packets/burst=%d\n",
         cur_fwd_eng->fwd_mode_name, retry_enabled == 0 ? "" : " with retry",
         nb_pkt_per_burst);
...
void pkt_fwd_config_display(struct fwd_config *cfg) {
  ...
  printf("%s packet forwarding%s - ports=%d - cores=%d - streams=%d - "
         ...
         cfg->fwd_eng->fwd_mode_name, retry_enabled == 0 ? "" : " with retry",
...
static inline void print_fdir_mask(struct rte_eth_fdir_masks *mask) {
...
void fdir_set_flex_mask(portid_t port_id, struct rte_eth_fdir_flex_mask *cfg) {
  struct rte_port *port;
  ...
  if (i >= RTE_ETH_FLOW_MAX) {
    if (flex_conf->nb_flexmasks < RTE_DIM(flex_conf->flex_mask)) {
      ...
      fprintf(stderr,
              "The flex mask table is full. Can not set flex mask for "
              "flow_type(%u).",
              cfg->flow_type);
      return;
    }
  }
  rte_memcpy(&flex_conf->flex_mask[idx], cfg,
...
void fdir_set_flex_payload(portid_t port_id,
  ...
  if (i >= RTE_ETH_PAYLOAD_MAX) {
    if (flex_conf->nb_payloads < RTE_DIM(flex_conf->flex_set)) {
      ...
      fprintf(stderr,
              "The flex payload table is full. Can not set flex payload for "
              "type(%u).",
              cfg->type);
      return;
    }
  }
  rte_memcpy(&flex_conf->flex_set[idx], cfg,
...
void port_queue_region_info_display(portid_t port_id, void *buf) {
  ...
  for (i = 0; i < info->queue_region_number; i++) {
    printf("\n	region_id: %-14u queue_number: %-14u "
           ...
           info->region[i].region_id, info->region[i].queue_num,
    ...
    printf("\n	flowtype_num is  %-14u :", info->region[i].flowtype_num);
