...
static inline void print_ether_addr(const char *what,
                                    ...
                                    char print_buf[], size_t buf_size,
                                    size_t *cur_len) {
...
static inline rte_mbuf_timestamp_t get_timestamp(const struct rte_mbuf *mbuf) {
...
static inline void dump_pkt_burst(uint16_t port_id, uint16_t queue,
  ...
  if (!nb_pkts)
    return;
  MKDUMPSTR(print_buf, buf_size, cur_len, "port %u/queue %u: %s %u packets\n",
            port_id, queue, is_rx ? "received" : "sent", (unsigned int)nb_pkts);
  for (i = 0; i < nb_pkts; i++) {
    int ret;
    ...
    if (!ret) {
      MKDUMPSTR(print_buf, buf_size, cur_len, "restore info:");
      if (info.flags & RTE_FLOW_RESTORE_INFO_TUNNEL) {
        ...
        MKDUMPSTR(print_buf, buf_size, cur_len, " - tunnel");
        if (port_tunnel)
          MKDUMPSTR(print_buf, buf_size, cur_len, " #%u", port_tunnel->id);
        else
          MKDUMPSTR(print_buf, buf_size, cur_len, " %s", "-none-");
        MKDUMPSTR(print_buf, buf_size, cur_len, " type %s",
                  port_flow_tunnel_type(&info.tunnel));
      } else {
        MKDUMPSTR(print_buf, buf_size, cur_len, " - no tunnel info");
      }
      if (info.flags & RTE_FLOW_RESTORE_INFO_ENCAPSULATED)
        MKDUMPSTR(print_buf, buf_size, cur_len, " - outer header present");
      else
        MKDUMPSTR(print_buf, buf_size, cur_len, " - no outer header");
      if (info.flags & RTE_FLOW_RESTORE_INFO_GROUP_ID)
        MKDUMPSTR(print_buf, buf_size, cur_len, " - miss group %u",
                  info.group_id);
      else
        MKDUMPSTR(print_buf, buf_size, cur_len, " - no miss group");
      MKDUMPSTR(print_buf, buf_size, cur_len, "\n");
    }
    print_ether_addr("  src=", &eth_hdr->src_addr, print_buf, buf_size,
                     &cur_len);
    print_ether_addr(" - dst=", &eth_hdr->dst_addr, print_buf, buf_size,
                     &cur_len);
    MKDUMPSTR(print_buf, buf_size, cur_len,
              " - type=0x%04x - length=%u - nb_segs=%d", eth_type,
              (unsigned int)mb->pkt_len, (int)mb->nb_segs);
    ol_flags = mb->ol_flags;
    if (ol_flags & RTE_MBUF_F_RX_RSS_HASH) {
      MKDUMPSTR(print_buf, buf_size, cur_len, " - RSS hash=0x%x",
                (unsigned int)mb->hash.rss);
      MKDUMPSTR(print_buf, buf_size, cur_len, " - RSS queue=0x%x",
                (unsigned int)queue);
    }
    if (ol_flags & RTE_MBUF_F_RX_FDIR) {
      MKDUMPSTR(print_buf, buf_size, cur_len, " - FDIR matched ");
      if (ol_flags & RTE_MBUF_F_RX_FDIR_ID)
        MKDUMPSTR(print_buf, buf_size, cur_len, "ID=0x%x", mb->hash.fdir.hi);
      else if (ol_flags & RTE_MBUF_F_RX_FDIR_FLX)
        MKDUMPSTR(print_buf, buf_size, cur_len, "flex bytes=0x%08x %08x",
                  mb->hash.fdir.hi, mb->hash.fdir.lo);
      else
        MKDUMPSTR(print_buf, buf_size, cur_len, "hash=0x%x ID=0x%x ",
                  mb->hash.fdir.hash, mb->hash.fdir.id);
    }
    if (is_timestamp_enabled(mb))
      MKDUMPSTR(print_buf, buf_size, cur_len, " - timestamp %" PRIu64 " ",
                get_timestamp(mb));
    if (ol_flags & RTE_MBUF_F_RX_QINQ)
      MKDUMPSTR(print_buf, buf_size, cur_len,
                " - QinQ VLAN tci=0x%x, VLAN tci outer=0x%x", mb->vlan_tci,
                mb->vlan_tci_outer);
    else if (ol_flags & RTE_MBUF_F_RX_VLAN)
      MKDUMPSTR(print_buf, buf_size, cur_len, " - VLAN tci=0x%x", mb->vlan_tci);
    if (!is_rx && (ol_flags & RTE_MBUF_DYNFLAG_TX_METADATA))
      MKDUMPSTR(print_buf, buf_size, cur_len, " - Tx metadata: 0x%x",
                *RTE_FLOW_DYNF_METADATA(mb));
    if (is_rx && (ol_flags & RTE_MBUF_DYNFLAG_RX_METADATA))
      MKDUMPSTR(print_buf, buf_size, cur_len, " - Rx metadata: 0x%x",
                *RTE_FLOW_DYNF_METADATA(mb));
    for (dynf_index = 0; dynf_index < 64; dynf_index++) {
      if (dynf_names[dynf_index][0] != '\0')
        MKDUMPSTR(print_buf, buf_size, cur_len, " - dynf %s: %d",
                  dynf_names[dynf_index], !!(ol_flags & (1UL << dynf_index)));
    }
    if (mb->packet_type) {
      rte_get_ptype_name(mb->packet_type, buf, sizeof(buf));
      MKDUMPSTR(print_buf, buf_size, cur_len, " - hw ptype: %s", buf);
    }
    ...
    if (sw_packet_type & RTE_PTYPE_L3_MASK)
      MKDUMPSTR(print_buf, buf_size, cur_len, " - l3_len=%d", hdr_lens.l3_len);
    if (sw_packet_type & RTE_PTYPE_L4_MASK)
      MKDUMPSTR(print_buf, buf_size, cur_len, " - l4_len=%d", hdr_lens.l4_len);
    if (sw_packet_type & RTE_PTYPE_TUNNEL_MASK)
      MKDUMPSTR(print_buf, buf_size, cur_len, " - tunnel_len=%d",
                hdr_lens.tunnel_len);
    if (sw_packet_type & RTE_PTYPE_INNER_L2_MASK)
      MKDUMPSTR(print_buf, buf_size, cur_len, " - inner_l2_len=%d",
                hdr_lens.inner_l2_len);
    if (sw_packet_type & RTE_PTYPE_INNER_L3_MASK)
      MKDUMPSTR(print_buf, buf_size, cur_len, " - inner_l3_len=%d",
                hdr_lens.inner_l3_len);
    if (sw_packet_type & RTE_PTYPE_INNER_L4_MASK)
      MKDUMPSTR(print_buf, buf_size, cur_len, " - inner_l4_len=%d",
    ...
    if (is_encapsulation) {
      ...
      if (RTE_ETH_IS_IPV4_HDR(packet_type)) {
        ...
        ipv6_hdr = rte_pktmbuf_mtod_offset(mb, struct rte_ipv6_hdr *, l2_len);
        l4_proto = ipv6_hdr->proto;
      }
      if (l4_proto == IPPROTO_UDP) {
        udp_hdr =
            rte_pktmbuf_mtod_offset(mb, struct rte_udp_hdr *, l2_len + l3_len);
        l4_len = sizeof(struct rte_udp_hdr);
        vxlan_hdr = rte_pktmbuf_mtod_offset(mb, struct rte_vxlan_hdr *,
        ...
        MKDUMPSTR(print_buf, buf_size, cur_len,
                  ...
                  "last_rsvd = %d",
                  packet_type, udp_port, vx_vni >> 8, vx_vni & 0xff);
      }
    }
    MKDUMPSTR(print_buf, buf_size, cur_len, " - %s queue=0x%x",
              is_rx ? "Receive" : "Send", (unsigned int)queue);
    MKDUMPSTR(print_buf, buf_size, cur_len, "\n");
    if (is_rx)
    ...
    MKDUMPSTR(print_buf, buf_size, cur_len, "  ol_flags: %s\n", buf);
    if (rte_mbuf_check(mb, 1, &reason) < 0)
      MKDUMPSTR(print_buf, buf_size, cur_len, "INVALID mbuf: %s\n", reason);
    if (cur_len >= buf_size)
      printf("%s ...\n", print_buf);
    else
      printf("%s", print_buf);
...
uint16_t tx_pkt_set_md(uint16_t port_id, __rte_unused uint16_t queue,
  ...
  if (rte_flow_dynf_metadata_avail())
    for (i = 0; i < nb_pkts; i++) {
      *RTE_FLOW_DYNF_METADATA(pkts[i]) = ports[port_id].tx_metadata;
  ...
  for (queue = 0; queue < dev_info.nb_tx_queues; queue++)
    if (!ports[portid].tx_set_md_cb[queue])
      ports[portid].tx_set_md_cb[queue] =
          rte_eth_add_tx_callback(portid, queue, tx_pkt_set_md, NULL);
}
  ...
  for (queue = 0; queue < dev_info.nb_tx_queues; queue++)
    if (!ports[portid].tx_set_dynf_cb[queue])
      ports[portid].tx_set_dynf_cb[queue] =
          rte_eth_add_tx_callback(portid, queue, tx_pkt_set_dynf, NULL);
}
...
int eth_dev_conf_get_print_err(uint16_t port_id,
  ...
  if (ret != 0)
    fprintf(stderr, "Error during getting device configuration (port %u): %s\n",
...
void eth_set_promisc_mode(uint16_t port, int enable) {
  ...
  if (ret != 0)
    fprintf(stderr, "Error during %s promiscuous mode for port %u: %s\n",
...
void eth_set_allmulticast_mode(uint16_t port, int enable) {
  ...
  if (ret != 0)
    fprintf(stderr, "Error during %s all-multicast mode for port %u: %s\n",
            enable ? "enabling" : "disabling", port, rte_strerror(-ret));
}
...
int eth_link_get_nowait_print_err(uint16_t port_id, struct rte_eth_link *link) {
  ...
  if (ret < 0)
    fprintf(stderr, "Device (port %u) link get (without wait) failed: %s\n",
...
int eth_macaddr_get_print_err(uint16_t port_id,
  ...
  if (ret != 0)
    fprintf(stderr, "Error getting device (port %u) mac address: %s\n", port_id,
            rte_strerror(-ret));
