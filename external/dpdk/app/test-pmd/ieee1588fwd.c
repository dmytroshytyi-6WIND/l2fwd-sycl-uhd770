...
static void port_ieee1588_rx_timestamp_check(portid_t pi, uint32_t index) {
  ...
  printf("Port %u RX timestamp value %ju s %lu ns\n", pi,
         (uintmax_t)timestamp.tv_sec, timestamp.tv_nsec);
}

#define MAX_TX_TMST_WAIT_MICROSECS 1000 /**< 1 milli-second */

static void port_ieee1588_tx_timestamp_check(portid_t pi) {
  struct timespec timestamp = {0, 0};
  unsigned wait_us = 0;
...
static void ieee1588_packet_fwd(struct fwd_stream *fs) {
  struct rte_mbuf *mb;
  ...
  if (!(mb->ol_flags & RTE_MBUF_F_RX_IEEE1588_PTP)) {
    if (eth_type == RTE_ETHER_TYPE_1588) {
      ...
      printf("Port %u Received non PTP packet type=0x%4x "
             "len=%u\n",
             fs->rx_port, eth_type, (unsigned)mb->pkt_len);
    }
    rte_pktmbuf_free(mb);
    return;
  }
...
static int port_ieee1588_fwd_begin(portid_t pi) {
  rte_eth_timesync_enable(pi);
  return 0;
}

static void port_ieee1588_fwd_end(portid_t pi) { rte_eth_timesync_disable(pi); }
