...
static const char *ip_proto_name(uint16_t ip_proto) {
  static const char *ip_proto_names[] = {
      ...
      "UNASSIGNED",  "TCP", /**< transport control protocol */
      ...
      "UNASSIGNED",  "UNASSIGNED", "UNASSIGNED", "ICMPv6", /**< ICMP6 */
      ...
      "UNASSIGNED",  "UNASSIGNED", "PIM", /**< Protocol Independent Mcast */
  };
...
static void ether_addr_dump(const char *what, const struct rte_ether_addr *ea) {
...
static uint16_t ipv4_hdr_cksum(struct rte_ipv4_hdr *ip_h) {
  uint16_t *v16_h;
  uint32_t ip_cksum;
  ...
  ip_cksum = v16_h[0] + v16_h[1] + v16_h[2] + v16_h[3] + v16_h[4] + v16_h[6] +
             v16_h[7] + v16_h[8] + v16_h[9];
 ...
 */
static void reply_to_icmp_echo_rqsts(struct fwd_stream *fs) {
  ...
  for (i = 0; i < nb_rx; i++) {
    ...
    if (eth_type == RTE_ETHER_TYPE_VLAN) {
      vlan_h =
          (struct rte_vlan_hdr *)((char *)eth_h + sizeof(struct rte_ether_hdr));
    ...
    if (eth_type == RTE_ETHER_TYPE_ARP) {
      ...
      if (verbose_level > 0) {
        printf("  ARP:  hrd=%d proto=0x%04x hln=%d "
               "pln=%d op=%u (%s)\n",
               RTE_BE_TO_CPU_16(arp_h->arp_hardware), arp_pro, arp_h->arp_hlen,
               arp_h->arp_plen, arp_op, arp_op_name(arp_op));
      }
      if ((RTE_BE_TO_CPU_16(arp_h->arp_hardware) != RTE_ARP_HRD_ETHER) ||
          (arp_pro != RTE_ETHER_TYPE_IPV4) || (arp_h->arp_hlen != 6) ||
          (arp_h->arp_plen != 4)) {
      ...
      rte_ether_addr_copy(&ports[fs->tx_port].eth_addr, &eth_h->src_addr);
     ...
     */
    icmp_h =
        (struct rte_icmp_hdr *)((char *)ip_h + sizeof(struct rte_ipv4_hdr));
  ...
  if (nb_replies > 0) {
    nb_tx = rte_eth_tx_burst(fs->tx_port, fs->tx_queue, pkts_burst, nb_replies);
    /*
     * Retry if necessary
     */
    if (unlikely(nb_tx < nb_replies) && fs->retry_enabled) {
      retry = 0;
      while (nb_tx < nb_replies && retry++ < burst_tx_retry_num) {
        rte_delay_us(burst_tx_delay_time);
        nb_tx += rte_eth_tx_burst(fs->tx_port, fs->tx_queue, &pkts_burst[nb_tx],
                                  nb_replies - nb_tx);
      }
    }
    fs->tx_packets += nb_tx;
