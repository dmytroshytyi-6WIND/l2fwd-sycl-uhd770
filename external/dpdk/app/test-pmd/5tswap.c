...
static inline void swap_ipv4(struct rte_ipv4_hdr *ipv4_hdr) {
  rte_be32_t addr;
...
static inline void swap_ipv6(struct rte_ipv6_hdr *ipv6_hdr) {
  uint8_t addr[16];
...
static inline void swap_tcp(struct rte_tcp_hdr *tcp_hdr) {
  rte_be16_t port;
...
static inline void swap_udp(struct rte_udp_hdr *udp_hdr) {
  rte_be16_t port;
 ...
 */
static void pkt_burst_5tuple_swap(struct fwd_stream *fs) {
  ...
  vlan_qinq_set(pkts_burst, nb_rx, ol_flags, txp->tx_vlan_id,
