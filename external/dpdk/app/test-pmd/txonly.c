...
static void copy_buf_to_pkt_segs(void *buf, unsigned len, struct rte_mbuf *pkt,
                                 unsigned offset) {
  struct rte_mbuf *seg;
  void *seg_buf;
  unsigned copy_len;
...
static inline void update_pkt_header(struct rte_mbuf *pkt,
...
pkt_burst_prepare(struct rte_mbuf *pkt, struct rte_mempool *mbp,
  ...
  if (unlikely(timestamp_enable)) {
    ...
    if (unlikely(!skew)) {
      ...
      phase = tx_pkt_times_inter * fs->tx_queue / (txqs_n ? txqs_n : 1);
      /*
       * Initialize the scheduling time phase shift
       * depending on queue index.
       */
      skew = timestamp_initial[fs->tx_port] + tx_pkt_times_inter + phase;
      fs->ts_skew = skew;
    }
 ...
 */
static void pkt_burst_transmit(struct fwd_stream *fs) {
  ...
  if (rte_mempool_get_bulk(mbp, (void **)pkts_burst, nb_pkt_per_burst) == 0) {
    for (nb_pkt = 0; nb_pkt < nb_pkt_per_burst; nb_pkt++) {
      if (unlikely(!pkt_burst_prepare(pkts_burst[nb_pkt], mbp, &eth_hdr,
                                      vlan_tci, vlan_tci_outer, ol_flags,
                                      nb_pkt, fs))) {
        rte_mempool_put_bulk(mbp, (void **)&pkts_burst[nb_pkt],
    ...
    for (nb_pkt = 0; nb_pkt < nb_pkt_per_burst; nb_pkt++) {
      ...
      if (unlikely(!pkt_burst_prepare(pkt, mbp, &eth_hdr, vlan_tci,
                                      vlan_tci_outer, ol_flags, nb_pkt, fs))) {
  ...
  if (unlikely(nb_tx < nb_pkt)) {
    if (verbose_level > 0 && fs->fwd_dropped == 0)
      printf("port %d tx_queue %d - drop "
             ...
             fs->tx_port, fs->tx_queue, (unsigned)nb_pkt, (unsigned)nb_tx,
...
static int tx_only_begin(portid_t pi) {
  uint16_t pkt_hdr_len, pkt_data_len;
  int dynf;

  pkt_hdr_len =
      (uint16_t)(sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) +
                 sizeof(struct rte_udp_hdr));
  ...
  if (dynf >= 0)
    timestamp_off = dynf;
  timestamp_enable = tx_pkt_times_inter && timestamp_mask &&
                     timestamp_off >= 0 &&
