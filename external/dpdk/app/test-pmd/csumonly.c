...
#define GRE_SUPPORTED_FIELDS                                                   \
  (GRE_CHECKSUM_PRESENT | GRE_KEY_PRESENT | GRE_SEQUENCE_PRESENT)
...
static uint16_t get_udptcp_checksum(struct rte_mbuf *m, void *l3_hdr,
                                    uint16_t l4_off, uint16_t ethertype) {
...
static void parse_ipv4(struct rte_ipv4_hdr *ipv4_hdr,
...
static void parse_ipv6(struct rte_ipv6_hdr *ipv6_hdr,
                       struct testpmd_offload_info *info) {
  struct rte_tcp_hdr *tcp_hdr;
 ...
 */
static void parse_ethernet(struct rte_ether_hdr *eth_hdr,
                           struct testpmd_offload_info *info) {
...
static void update_tunnel_outer(struct testpmd_offload_info *info) {
  info->is_tunnel = 1;
 ...
 */
static void parse_gtp(struct rte_udp_hdr *udp_hdr,
                      struct testpmd_offload_info *info) {
  struct rte_ipv4_hdr *ipv4_hdr;
  struct rte_ipv6_hdr *ipv6_hdr;
  ...
  gtp_hdr =
      (struct rte_gtp_hdr *)((char *)udp_hdr + sizeof(struct rte_udp_hdr));
  ...
  if (gtp_hdr->msg_type == 0xff) {
    ip_ver = *(uint8_t *)((char *)udp_hdr + sizeof(struct rte_udp_hdr) +
...
static void parse_vxlan(struct rte_udp_hdr *udp_hdr,
                        struct testpmd_offload_info *info) {
  ...
  eth_hdr =
      (struct rte_ether_hdr *)((char *)udp_hdr + sizeof(struct rte_udp_hdr) +
...
static void parse_vxlan_gpe(struct rte_udp_hdr *udp_hdr,
                            struct testpmd_offload_info *info) {
  struct rte_ether_hdr *eth_hdr;
  struct rte_ipv4_hdr *ipv4_hdr;
  struct rte_ipv6_hdr *ipv6_hdr;
  ...
  if (!vxlan_gpe_hdr->proto ||
      vxlan_gpe_hdr->proto == RTE_VXLAN_GPE_TYPE_IPV4) {
    update_tunnel_outer(info);
...
static void parse_geneve(struct rte_udp_hdr *udp_hdr,
                         struct testpmd_offload_info *info) {
  struct rte_ether_hdr *eth_hdr;
  struct rte_ipv4_hdr *ipv4_hdr;
  struct rte_ipv6_hdr *ipv6_hdr;
  ...
  geneve_hdr =
      (struct rte_geneve_hdr *)((char *)udp_hdr + sizeof(struct rte_udp_hdr));
  geneve_len = sizeof(struct rte_geneve_hdr) + geneve_hdr->opt_len * 4;
  if (!geneve_hdr->proto || geneve_hdr->proto == _htons(RTE_ETHER_TYPE_IPV4)) {
    update_tunnel_outer(info);
...
/* Parse a gre header */
static void parse_gre(struct simple_gre_hdr *gre_hdr,
                      struct testpmd_offload_info *info) {
  struct rte_ether_hdr *eth_hdr;
  struct rte_ipv4_hdr *ipv4_hdr;
  struct rte_ipv6_hdr *ipv6_hdr;
...
static void parse_encap_ip(void *encap_ip, struct testpmd_offload_info *info) {
...
static uint64_t process_inner_cksums(void *l3_hdr,
                                     const struct testpmd_offload_info *info,
  ...
  if (!info->is_tunnel) {
    max_pkt_len = info->l2_len + info->l3_len + info->l4_len + info->tso_segsz;
    if (info->tso_segsz != 0 && info->pkt_len > max_pkt_len)
      tso_segsz = info->tso_segsz;
  } else {
    max_pkt_len = info->outer_l2_len + info->outer_l3_len + info->l2_len +
                  info->l3_len + info->l4_len + info->tunnel_tso_segsz;
  ...
  if (info->l4_proto == IPPROTO_UDP) {
    ...
    if (udp_hdr->dgram_cksum != 0) {
      if (tx_offloads & RTE_ETH_TX_OFFLOAD_UDP_CKSUM) {
        ...
        if (info->is_tunnel)
          l4_off =
              info->l2_len + info->outer_l3_len + info->l2_len + info->l3_len;
        else
          l4_off = info->l2_len + info->l3_len;
        udp_hdr->dgram_cksum = 0;
        udp_hdr->dgram_cksum =
            get_udptcp_checksum(m, l3_hdr, l4_off, info->ethertype);
      }
    }
#ifdef RTE_LIB_GSO
    if (info->gso_enable)
    ...
    if (tso_segsz)
      ...
      if (info->is_tunnel)
        ...
        l4_off = info->l2_len + info->l3_len;
      tcp_hdr->cksum = 0;
      tcp_hdr->cksum = get_udptcp_checksum(m, l3_hdr, l4_off, info->ethertype);
    }
#ifdef RTE_LIB_GSO
    if (info->gso_enable)
      ol_flags |= RTE_MBUF_F_TX_TCP_SEG;
...
static uint64_t process_outer_cksums(void *outer_l3_hdr,
                                     struct testpmd_offload_info *info,
  ...
  if (udp_hdr->dgram_cksum != 0) {
    udp_hdr->dgram_cksum = 0;
    udp_hdr->dgram_cksum =
        get_udptcp_checksum(m, outer_l3_hdr, info->l2_len + info->outer_l3_len,
 ...
 */
static int mbuf_copy_split(const struct rte_mbuf *ms, struct rte_mbuf *md[],
 ...
 */
static struct rte_mbuf *pkt_copy_split(const struct rte_mbuf *pkt) {
  int32_t n, rc;
  ...
  while (i != 0) {
    ...
    if (rte_pktmbuf_tailroom(md[i]) < seglen[i]) {
      TESTPMD_LOG(ERR,
                  ...
                  mp->name, i, seglen[i], rte_pktmbuf_tailroom(md[i]));
      break;
    }
  }
 ...
 */
static void pkt_burst_checksum_forward(struct fwd_stream *fs) {
  ...
  nb_rx =
      rte_eth_rx_burst(fs->rx_port, fs->rx_queue, pkts_burst, nb_pkt_per_burst);
  ...
  for (i = 0; i < nb_rx; i++) {
    ...
    tx_ol_flags = m->ol_flags & (RTE_MBUF_F_INDIRECT | RTE_MBUF_F_EXTERNAL);
    rx_ol_flags = m->ol_flags;

    /* Update the L3/L4 checksum error packet statistics */
    if ((rx_ol_flags & RTE_MBUF_F_RX_IP_CKSUM_MASK) ==
        RTE_MBUF_F_RX_IP_CKSUM_BAD)
      rx_bad_ip_csum += 1;
    if ((rx_ol_flags & RTE_MBUF_F_RX_L4_CKSUM_MASK) ==
    ...
    if (txp->parse_tunnel) {
      if (info.l4_proto == IPPROTO_UDP) {
        ...
        if (info.is_tunnel) {
          tx_ol_flags |= RTE_MBUF_F_TX_TUNNEL_VXLAN_GPE;
          goto tunnel_update;
        }
        parse_vxlan(udp_hdr, &info);
        if (info.is_tunnel) {
          tx_ol_flags |= RTE_MBUF_F_TX_TUNNEL_VXLAN;
          goto tunnel_update;
        }
        parse_geneve(udp_hdr, &info);
        if (info.is_tunnel) {
          tx_ol_flags |= RTE_MBUF_F_TX_TUNNEL_GENEVE;
          goto tunnel_update;
        }
        /* Always keep last. */
  ...
  tunnel_update:
    ...
    tx_ol_flags |= process_inner_cksums(l3_hdr, &info, tx_offloads, m);
    ...
    if (info.is_tunnel == 1) {
      tx_ol_flags |=
          process_outer_cksums(outer_l3_hdr, &info, tx_offloads,
                               !!(tx_ol_flags & RTE_MBUF_F_TX_TCP_SEG), m);
    }
    ...
    if (info.is_tunnel == 1) {
      if (info.tunnel_tso_segsz ||
          ...
          (tx_offloads & RTE_ETH_TX_OFFLOAD_OUTER_UDP_CKSUM)) {
        ...
        /* if there is a outer UDP cksum
           ...
           hardware */
        m->l2_len = info.outer_l2_len + info.outer_l3_len + info.l2_len;
        m->l3_len = info.l3_len;
        m->l4_len = info.l4_len;
      }
    } else {
    ...
    if (verbose_level > 0) {
      ...
      printf("rx: l2_len=%d ethertype=%x l3_len=%d "
             ...
             info.l2_len, rte_be_to_cpu_16(info.ethertype), info.l3_len,
             info.l4_proto, info.l4_len, buf);
      if (rx_ol_flags & RTE_MBUF_F_RX_LRO)
        printf("rx: m->lro_segsz=%u\n", m->tso_segsz);
      if (info.is_tunnel == 1)
        printf("rx: outer_l2_len=%d outer_ethertype=%x "
               "outer_l3_len=%d\n",
               info.outer_l2_len, rte_be_to_cpu_16(info.outer_ethertype),
               info.outer_l3_len);
      /* dump tx packet info */
      if ((tx_offloads &
      ...
      if (info.is_tunnel == 1) {
        ...
        if (info.tunnel_tso_segsz != 0 && (m->ol_flags & RTE_MBUF_F_TX_TCP_SEG))
          printf("tx: m->tso_segsz=%d\n", m->tso_segsz);
      } else if (info.tso_segsz != 0 && (m->ol_flags & RTE_MBUF_F_TX_TCP_SEG))
        printf("tx: m->tso_segsz=%d\n", m->tso_segsz);
  ...
  if (unlikely(gro_enable)) {
    if (gro_flush_cycles == GRO_DEFAULT_FLUSH_CYCLES) {
      ...
      if (++fs->gro_times >= gro_flush_cycles) {
        ...
        nb_rx += rte_gro_timeout_flush(gro_ctx, 0, RTE_GRO_TCP_IPV4,
                                       &pkts_burst[nb_rx], gro_pkts_num);
        fs->gro_times = 0;
      }
    }
  }
  ...
  if (gso_ports[fs->tx_port].enable != 0) {
    ...
    for (i = 0; i < nb_rx; i++) {
      ...
      ret = rte_gso_segment(pkts_burst[i], gso_ctx, &gso_segments[nb_segments],
  ...
  nb_prep = rte_eth_tx_prepare(fs->tx_port, fs->tx_queue, tx_pkts_burst, nb_rx);
  if (nb_prep != nb_rx)
    fprintf(stderr, "Preparing packet burst to transmit failed: %s\n",
            rte_strerror(rte_errno));

  nb_tx = rte_eth_tx_burst(fs->tx_port, fs->tx_queue, tx_pkts_burst, nb_prep);
