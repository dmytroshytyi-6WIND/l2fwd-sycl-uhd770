...
#define ACTION_MODIFY_SIZE                                                     \
  (sizeof(struct rte_flow_action_modify_field) + ACTION_MODIFY_PATTERN_SIZE)
...
static const char *const modify_field_ops[] = {"set", "add", "sub", NULL};

static const char *const modify_field_ids[] = {
    "start",        "mac_dst",       "mac_src",     "vlan_type", "vlan_id",
    "mac_type",     "ipv4_dscp",     "ipv4_ttl",    "ipv4_src",  "ipv4_dst",
    "ipv6_dscp",    "ipv6_hoplimit", "ipv6_src",    "ipv6_dst",  "tcp_port_src",
    "tcp_port_dst", "tcp_seq_num",   "tcp_ack_num", "tcp_flags", "udp_port_src",
    "udp_port_dst", "vxlan_vni",     "geneve_vni",  "gtp_teid",  "tag",
    "mark",         "meta",          "pointer",     "value",     NULL};
...
struct token {
   ...
   */
  int (*call)(struct context *ctx, const struct token *token, const char *str,
   ...
   * snprintf()).
   */
  int (*comp)(struct context *ctx, const struct token *token, unsigned int ent,
...
#define NEXT(...)                                                              \
  (const enum index *const[]) { __VA_ARGS__, NULL, }

/** Static initializer for a NEXT() entry. */
#define NEXT_ENTRY(...)                                                        \
  (const enum index[]) { __VA_ARGS__, ZERO, }

/** Static initializer for the args field. */
#define ARGS(...)                                                              \
  (const struct arg *const[]) { __VA_ARGS__, NULL, }
...
static const enum index next_at_attr[] = {
    ACTIONS_TEMPLATE_CREATE_ID, ACTIONS_TEMPLATE_INGRESS,
    ACTIONS_TEMPLATE_EGRESS,    ACTIONS_TEMPLATE_TRANSFER,
    ACTIONS_TEMPLATE_SPEC,      ZERO,
};
...
static const enum index next_ia_create_attr[] = {
    INDIRECT_ACTION_CREATE_ID, INDIRECT_ACTION_INGRESS, INDIRECT_ACTION_EGRESS,
    INDIRECT_ACTION_TRANSFER,  INDIRECT_ACTION_SPEC,    ZERO,
};
...
static const enum index next_vc_attr[] = {
    VC_GROUP,      VC_PRIORITY,     VC_INGRESS,   VC_EGRESS, VC_TRANSFER,
    VC_TUNNEL_SET, VC_TUNNEL_MATCH, ITEM_PATTERN, ZERO,
};
...
static const enum index item_param[] = {
    ITEM_PARAM_IS,   ITEM_PARAM_SPEC,   ITEM_PARAM_LAST,
    ITEM_PARAM_MASK, ITEM_PARAM_PREFIX, ZERO,
};
...
static const enum index item_raw[] = {
    ITEM_RAW_RELATIVE, ITEM_RAW_SEARCH,      ITEM_RAW_OFFSET, ITEM_RAW_LIMIT,
    ITEM_RAW_PATTERN,  ITEM_RAW_PATTERN_HEX, ITEM_NEXT,       ZERO,
};

static const enum index item_eth[] = {
    ITEM_ETH_DST,      ITEM_ETH_SRC, ITEM_ETH_TYPE,
    ITEM_ETH_HAS_VLAN, ITEM_NEXT,    ZERO,
};

static const enum index item_vlan[] = {
    ITEM_VLAN_TCI,        ITEM_VLAN_PCP,           ITEM_VLAN_DEI, ITEM_VLAN_VID,
    ITEM_VLAN_INNER_TYPE, ITEM_VLAN_HAS_MORE_VLAN, ITEM_NEXT,     ZERO,
};

static const enum index item_ipv4[] = {
    ITEM_IPV4_VER_IHL, ITEM_IPV4_TOS,
    ITEM_IPV4_ID,      ITEM_IPV4_FRAGMENT_OFFSET,
    ITEM_IPV4_TTL,     ITEM_IPV4_PROTO,
    ITEM_IPV4_SRC,     ITEM_IPV4_DST,
    ITEM_NEXT,         ZERO,
};

static const enum index item_ipv6[] = {
    ITEM_IPV6_TC,           ITEM_IPV6_FLOW, ITEM_IPV6_PROTO,
    ITEM_IPV6_HOP,          ITEM_IPV6_SRC,  ITEM_IPV6_DST,
    ITEM_IPV6_HAS_FRAG_EXT, ITEM_NEXT,      ZERO,
};

static const enum index item_icmp[] = {
    ITEM_ICMP_TYPE, ITEM_ICMP_CODE, ITEM_ICMP_IDENT,
    ITEM_ICMP_SEQ,  ITEM_NEXT,      ZERO,
};
...
static const enum index item_tcp[] = {
    ITEM_TCP_SRC, ITEM_TCP_DST, ITEM_TCP_FLAGS, ITEM_NEXT, ZERO,
};

static const enum index item_sctp[] = {
    ITEM_SCTP_SRC,   ITEM_SCTP_DST, ITEM_SCTP_TAG,
    ITEM_SCTP_CKSUM, ITEM_NEXT,     ZERO,
};
...
static const enum index item_mpls[] = {
    ITEM_MPLS_LABEL, ITEM_MPLS_TC, ITEM_MPLS_S, ITEM_NEXT, ZERO,
};

static const enum index item_gre[] = {
    ITEM_GRE_PROTO, ITEM_GRE_C_RSVD0_VER, ITEM_GRE_C_BIT,
    ITEM_GRE_K_BIT, ITEM_GRE_S_BIT,       ITEM_NEXT,
    ZERO,
};
...
static const enum index item_gtp[] = {
    ITEM_GTP_FLAGS, ITEM_GTP_MSG_TYPE, ITEM_GTP_TEID, ITEM_NEXT, ZERO,
};

static const enum index item_geneve[] = {
    ITEM_GENEVE_VNI, ITEM_GENEVE_PROTO, ITEM_GENEVE_OPTLEN, ITEM_NEXT, ZERO,
};
...
static const enum index item_ppp[] = {
    ITEM_PPP_ADDR, ITEM_PPP_CTRL, ITEM_PPP_PROTO_ID, ITEM_NEXT, ZERO,
};
...
static const enum index action_rss[] = {
    ACTION_RSS_FUNC,    ACTION_RSS_LEVEL,  ACTION_RSS_TYPES, ACTION_RSS_KEY,
    ACTION_RSS_KEY_LEN, ACTION_RSS_QUEUES, ACTION_NEXT,      ZERO,
};
...
static const enum index action_sample[] = {
    ACTION_SAMPLE, ACTION_SAMPLE_RATIO, ACTION_SAMPLE_INDEX, ACTION_NEXT, ZERO,
};

static const enum index next_action_sample[] = {
    ACTION_QUEUE,       ACTION_RSS,
    ACTION_MARK,        ACTION_COUNT,
    ACTION_PORT_ID,     ACTION_RAW_ENCAP,
    ACTION_VXLAN_ENCAP, ACTION_NVGRE_ENCAP,
    ACTION_NEXT,        ZERO,
};
...
static const enum index action_modify_field_src[] = {
    ACTION_MODIFY_FIELD_SRC_LEVEL, ACTION_MODIFY_FIELD_SRC_OFFSET,
    ACTION_MODIFY_FIELD_SRC_VALUE, ACTION_MODIFY_FIELD_SRC_POINTER,
    ACTION_MODIFY_FIELD_WIDTH,     ZERO,
};
...
static int parse_set_sample_action(struct context *, const struct token *,
                                   const char *, unsigned int, void *,
                                   unsigned int);
static int parse_set_init(struct context *, const struct token *, const char *,
                          unsigned int, void *, unsigned int);
static int parse_flex_handle(struct context *, const struct token *,
                             const char *, unsigned int, void *, unsigned int);
static int parse_init(struct context *, const struct token *, const char *,
                      unsigned int, void *, unsigned int);
static int parse_vc(struct context *, const struct token *, const char *,
                    unsigned int, void *, unsigned int);
static int parse_vc_spec(struct context *, const struct token *, const char *,
                         unsigned int, void *, unsigned int);
static int parse_vc_conf(struct context *, const struct token *, const char *,
                         unsigned int, void *, unsigned int);
...
static int parse_vc_action_mplsoudp_decap(struct context *,
                                          const struct token *, const char *,
                                          unsigned int, void *, unsigned int);
static int parse_vc_action_raw_encap(struct context *, const struct token *,
                                     const char *, unsigned int, void *,
                                     unsigned int);
static int parse_vc_action_raw_decap(struct context *, const struct token *,
                                     const char *, unsigned int, void *,
                                     unsigned int);
...
static int parse_vc_action_sample(struct context *ctx,
                                  ...
                                  unsigned int len, void *buf,
                                  unsigned int size);
static int parse_vc_action_sample_index(struct context *ctx,
                                        const struct token *token,
                                        const char *str, unsigned int len,
                                        void *buf, unsigned int size);
static int parse_vc_modify_field_op(struct context *ctx,
                                    const struct token *token, const char *str,
                                    unsigned int len, void *buf,
                                    unsigned int size);
static int parse_vc_modify_field_id(struct context *ctx,
                                    const struct token *token, const char *str,
                                    unsigned int len, void *buf,
                                    unsigned int size);
static int parse_vc_action_conntrack_update(struct context *ctx,
                                            const struct token *token,
                                            const char *str, unsigned int len,
                                            void *buf, unsigned int size);
static int parse_destroy(struct context *, const struct token *, const char *,
                         unsigned int, void *, unsigned int);
static int parse_flush(struct context *, const struct token *, const char *,
                       unsigned int, void *, unsigned int);
static int parse_dump(struct context *, const struct token *, const char *,
                      unsigned int, void *, unsigned int);
static int parse_query(struct context *, const struct token *, const char *,
                       unsigned int, void *, unsigned int);
static int parse_action(struct context *, const struct token *, const char *,
                        unsigned int, void *, unsigned int);
static int parse_list(struct context *, const struct token *, const char *,
                      unsigned int, void *, unsigned int);
static int parse_aged(struct context *, const struct token *, const char *,
                      unsigned int, void *, unsigned int);
static int parse_isolate(struct context *, const struct token *, const char *,
                         unsigned int, void *, unsigned int);
static int parse_configure(struct context *, const struct token *, const char *,
                           unsigned int, void *, unsigned int);
static int parse_template(struct context *, const struct token *, const char *,
                          unsigned int, void *, unsigned int);
static int parse_template_destroy(struct context *, const struct token *,
                                  const char *, unsigned int, void *,
                                  unsigned int);
static int parse_table(struct context *, const struct token *, const char *,
                       unsigned int, void *, unsigned int);
static int parse_table_destroy(struct context *, const struct token *,
                               const char *, unsigned int, void *,
                               unsigned int);
static int parse_qo(struct context *, const struct token *, const char *,
                    unsigned int, void *, unsigned int);
static int parse_qo_destroy(struct context *, const struct token *,
                            const char *, unsigned int, void *, unsigned int);
static int parse_qia(struct context *, const struct token *, const char *,
                     unsigned int, void *, unsigned int);
static int parse_qia_destroy(struct context *, const struct token *,
                             const char *, unsigned int, void *, unsigned int);
static int parse_push(struct context *, const struct token *, const char *,
                      unsigned int, void *, unsigned int);
static int parse_pull(struct context *, const struct token *, const char *,
                      unsigned int, void *, unsigned int);
static int parse_tunnel(struct context *, const struct token *, const char *,
                        unsigned int, void *, unsigned int);
static int parse_flex(struct context *, const struct token *, const char *,
                      unsigned int, void *, unsigned int);
static int parse_int(struct context *, const struct token *, const char *,
                     unsigned int, void *, unsigned int);
static int parse_prefix(struct context *, const struct token *, const char *,
                        unsigned int, void *, unsigned int);
static int parse_boolean(struct context *, const struct token *, const char *,
                         unsigned int, void *, unsigned int);
static int parse_string(struct context *, const struct token *, const char *,
                        unsigned int, void *, unsigned int);
static int parse_hex(struct context *ctx, const struct token *token,
                     const char *str, unsigned int len, void *buf,
                     unsigned int size);
static int parse_string0(struct context *, const struct token *, const char *,
                         unsigned int, void *, unsigned int);
static int parse_mac_addr(struct context *, const struct token *, const char *,
                          unsigned int, void *, unsigned int);
static int parse_ipv4_addr(struct context *, const struct token *, const char *,
                           unsigned int, void *, unsigned int);
static int parse_ipv6_addr(struct context *, const struct token *, const char *,
                           unsigned int, void *, unsigned int);
static int parse_port(struct context *, const struct token *, const char *,
                      unsigned int, void *, unsigned int);
static int parse_ia(struct context *, const struct token *, const char *,
                    unsigned int, void *, unsigned int);
...
static int parse_ia_id2ptr(struct context *ctx, const struct token *token,
                           const char *str, unsigned int len, void *buf,
                           unsigned int size);
static int parse_mp(struct context *, const struct token *, const char *,
                    unsigned int, void *, unsigned int);
static int comp_none(struct context *, const struct token *, unsigned int,
                     char *, unsigned int);
static int comp_boolean(struct context *, const struct token *, unsigned int,
                        char *, unsigned int);
static int comp_action(struct context *, const struct token *, unsigned int,
                       char *, unsigned int);
static int comp_port(struct context *, const struct token *, unsigned int,
                     char *, unsigned int);
static int comp_rule_id(struct context *, const struct token *, unsigned int,
                        char *, unsigned int);
...
static int comp_actions_template_id(struct context *, const struct token *,
                                    unsigned int, char *, unsigned int);
static int comp_table_id(struct context *, const struct token *, unsigned int,
                         char *, unsigned int);
static int comp_queue_id(struct context *, const struct token *, unsigned int,
                         char *, unsigned int);
...
static const struct token token_list[] =
    {
        ...
        [FLOW] =
            {
                ...
                .next = NEXT(NEXT_ENTRY(
                    INFO, CONFIGURE, PATTERN_TEMPLATE, ACTIONS_TEMPLATE, TABLE,
                    INDIRECT_ACTION, VALIDATE, CREATE, DESTROY, FLUSH, DUMP,
                    LIST, AGED, QUERY, ISOLATE, TUNNEL, FLEX, QUEUE, PUSH,
                    PULL)),
                .call = parse_init,
            },
        /* Top-level command. */
        [INFO] =
            {
                ...
                .next = NEXT(NEXT_ENTRY(END), NEXT_ENTRY(COMMON_PORT_ID)),
        ...
        [CONFIGURE] =
            {
                ...
                .next = NEXT(next_config_attr, NEXT_ENTRY(COMMON_PORT_ID)),
                .args = ARGS(ARGS_ENTRY(struct buffer, port)),
                .call = parse_configure,
            },
        /* Configure arguments. */
        [CONFIG_QUEUES_NUMBER] =
            {
                ...
                .next = NEXT(next_config_attr, NEXT_ENTRY(COMMON_UNSIGNED)),
                .args = ARGS(
                    ARGS_ENTRY(struct buffer, args.configure.nb_queue)),
            },
        [CONFIG_QUEUES_SIZE] =
            {
                .name = "queues_size",
                ...
                .next = NEXT(next_config_attr, NEXT_ENTRY(COMMON_UNSIGNED)),
                .args = ARGS(
                    ARGS_ENTRY(struct buffer, args.configure.queue_attr.size)),
            },
        [CONFIG_COUNTERS_NUMBER] =
            {
        ...
        [PATTERN_TEMPLATE_RELAXED_MATCHING] =
            {
                ...
                .next = NEXT(next_pt_attr, NEXT_ENTRY(COMMON_BOOLEAN)),
                .args = ARGS(
                    ARGS_ENTRY_BF(struct buffer, args.vc.attr.reserved, 1)),
            },
        [PATTERN_TEMPLATE_INGRESS] =
            {
                .name = "ingress",
        ...
        [TABLE_CREATE_ID] =
            {
                ...
                .next = NEXT(next_table_attr, NEXT_ENTRY(COMMON_TABLE_ID)),
        ...
        [TABLE_DESTROY_ID] =
            {
                ...
                .args = ARGS(
                    ARGS_ENTRY_PTR(struct buffer, args.table_destroy.table_id)),
                .call = parse_table_destroy,
            },
        [TABLE_GROUP] =
            {
                ...
                .args = ARGS(
                    ARGS_ENTRY(struct buffer, args.table.attr.flow_attr.group)),
            },
        [TABLE_PRIORITY] =
            {
                .name = "priority",
                ...
                .next = NEXT(
                    next_table_attr, NEXT_ENTRY(COMMON_PRIORITY_LEVEL)),
                .args = ARGS(
                    ARGS_ENTRY(
                        struct buffer, args.table.attr.flow_attr.priority)),
            },
        [TABLE_EGRESS] =
            {
                .name = "egress",
        ...
        [TABLE_RULES_NUMBER] =
            {
                ...
                .next = NEXT(next_table_attr, NEXT_ENTRY(COMMON_UNSIGNED)),
                .args = ARGS(
                    ARGS_ENTRY(struct buffer, args.table.attr.nb_flows)),
            },
        [TABLE_PATTERN_TEMPLATE] =
            {
                .name = "pattern_template",
                ...
                .next = NEXT(
                    next_table_attr, NEXT_ENTRY(COMMON_PATTERN_TEMPLATE_ID)),
                .args = ARGS(
                    ARGS_ENTRY_PTR(struct buffer, args.table.pat_templ_id)),
                .call = parse_table,
            },
        [TABLE_ACTIONS_TEMPLATE] =
            {
                ...
                .next = NEXT(
                    next_table_attr, NEXT_ENTRY(COMMON_ACTIONS_TEMPLATE_ID)),
                .args = ARGS(
                    ARGS_ENTRY_PTR(struct buffer, args.table.act_templ_id)),
                .call = parse_table,
            },
        /* Top-level command. */
        [QUEUE] =
        ...
        [QUEUE_TEMPLATE_TABLE] =
            {
                ...
                .next = NEXT(NEXT_ENTRY(QUEUE_PATTERN_TEMPLATE),
                             NEXT_ENTRY(COMMON_TABLE_ID)),
                .args = ARGS(ARGS_ENTRY(struct buffer, args.vc.table_id)),
                .call = parse_qo,
            },
        [QUEUE_PATTERN_TEMPLATE] =
            {
        ...
        [QUEUE_INDIRECT_ACTION_DESTROY_POSTPONE] =
            {
                .name = "postpone",
                .help = "postpone destroy operation",
                .next = NEXT(next_qia_destroy_attr, NEXT_ENTRY(COMMON_BOOLEAN)),
                .args = ARGS(ARGS_ENTRY(struct buffer, postpone)),
            },
        [QUEUE_INDIRECT_ACTION_DESTROY_ID] =
            {
                ...
                .next = NEXT(next_qia_destroy_attr,
                             NEXT_ENTRY(COMMON_INDIRECT_ACTION_ID)),
                .args = ARGS(
                    ARGS_ENTRY_PTR(struct buffer, args.ia_destroy.action_id)),
                .call = parse_qia_destroy,
            },
        ...
        [QUEUE_INDIRECT_ACTION_UPDATE_POSTPONE] =
            {
                ...
                .next = NEXT(next_qia_update_attr, NEXT_ENTRY(COMMON_BOOLEAN)),
                .args = ARGS(ARGS_ENTRY(struct buffer, postpone)),
            },
        /* Indirect action create arguments. */
        [QUEUE_INDIRECT_ACTION_CREATE_ID] =
        ...
        [QUEUE_INDIRECT_ACTION_CREATE_POSTPONE] =
            {
                .name = "postpone",
                .help = "postpone create operation",
                .next = NEXT(next_qia_create_attr, NEXT_ENTRY(COMMON_BOOLEAN)),
                .args = ARGS(ARGS_ENTRY(struct buffer, postpone)),
            },
        [QUEUE_INDIRECT_ACTION_SPEC] =
            {
        ...
        [PULL] =
            {
                ...
                .next = NEXT(NEXT_ENTRY(PULL_QUEUE),
                             NEXT_ENTRY(COMMON_PORT_ID)),
                .args = ARGS(ARGS_ENTRY(struct buffer, port)),
                .call = parse_pull,
            },
        ...
        [INDIRECT_ACTION_QUERY] =
            {
                ...
                .next = NEXT(
                    NEXT_ENTRY(END), NEXT_ENTRY(COMMON_INDIRECT_ACTION_ID)),
        ...
        [TUNNEL] =
            {
                ...
                .help = "new tunnel API",
                .next = NEXT(
                    NEXT_ENTRY(TUNNEL_CREATE, TUNNEL_LIST, TUNNEL_DESTROY)),
                .call = parse_tunnel,
            },
        /* Tunnel arguments. */
        [TUNNEL_CREATE] =
        ...
        [ITEM_RAW_RELATIVE] =
            {
                ...
                .args = ARGS(
                    ARGS_ENTRY_BF(struct rte_flow_item_raw, relative, 1)),
            },
        [ITEM_RAW_SEARCH] =
            {
                .name = "search",
                ...
                .next = NEXT(item_raw, NEXT_ENTRY(COMMON_BOOLEAN), item_param),
                .args = ARGS(
                    ARGS_ENTRY_BF(struct rte_flow_item_raw, search, 1)),
            },
        [ITEM_RAW_OFFSET] =
            {
                .name = "offset",
        ...
        [ITEM_RAW_PATTERN] =
            {
                ...
                .next = NEXT(
                    item_raw, NEXT_ENTRY(COMMON_STRING),
                    NEXT_ENTRY(ITEM_PARAM_IS, ITEM_PARAM_SPEC,
                               ITEM_PARAM_MASK)),
        ...
        [ITEM_RAW_PATTERN_HEX] =
            {
                ...
                .next = NEXT(
                    item_raw, NEXT_ENTRY(COMMON_HEX),
                    NEXT_ENTRY(ITEM_PARAM_IS, ITEM_PARAM_SPEC,
                               ITEM_PARAM_MASK)),
                .args = ARGS(ARGS_ENTRY(struct rte_flow_item_raw, pattern),
                             ARGS_ENTRY(struct rte_flow_item_raw, length),
        ...
        [ITEM_ETH_HAS_VLAN] =
            {
                ...
                .next = NEXT(item_eth, NEXT_ENTRY(COMMON_UNSIGNED), item_param),
                .args = ARGS(
                    ARGS_ENTRY_BF(struct rte_flow_item_eth, has_vlan, 1)),
            },
        [ITEM_VLAN] =
            {
                .name = "vlan",
        ...
        [ITEM_VLAN_INNER_TYPE] =
            {
                ...
                .next = NEXT(item_vlan, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_vlan, inner_type)),
            },
        [ITEM_VLAN_HAS_MORE_VLAN] =
            {
                .name = "has_more_vlan",
                ...
                .next = NEXT(item_vlan, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_BF(struct rte_flow_item_vlan, has_more_vlan, 1)),
            },
        [ITEM_IPV4] =
            {
                .name = "ipv4",
        ...
        [ITEM_IPV4_VER_IHL] =
            {
                ...
                .next = NEXT(item_ipv4, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY(struct rte_flow_item_ipv4, hdr.version_ihl)),
            },
        [ITEM_IPV4_TOS] =
            {
                .name = "tos",
        ...
        [ITEM_IPV4_ID] =
            {
                ...
                .next = NEXT(item_ipv4, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_ipv4, hdr.packet_id)),
            },
        [ITEM_IPV4_FRAGMENT_OFFSET] =
            {
        ...
        [ITEM_IPV4_SRC] =
            {
                ...
                .next = NEXT(item_ipv4, NEXT_ENTRY(COMMON_IPV4_ADDR),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_ipv4, hdr.src_addr)),
            },
        [ITEM_IPV4_DST] =
            {
                .name = "dst",
                ...
                .next = NEXT(item_ipv4, NEXT_ENTRY(COMMON_IPV4_ADDR),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_ipv4, hdr.dst_addr)),
            },
        [ITEM_IPV6] =
            {
                .name = "ipv6",
        ...
        [ITEM_IPV6_PROTO] =
            {
                ...
                .next = NEXT(item_ipv6, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_ipv6, hdr.proto)),
            },
        [ITEM_IPV6_HOP] =
            {
                .name = "hop",
                ...
                .next = NEXT(item_ipv6, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_ipv6, hdr.hop_limits)),
            },
        [ITEM_IPV6_SRC] =
            {
                .name = "src",
                ...
                .next = NEXT(item_ipv6, NEXT_ENTRY(COMMON_IPV6_ADDR),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_ipv6, hdr.src_addr)),
            },
        [ITEM_IPV6_DST] =
            {
                .name = "dst",
                ...
                .next = NEXT(item_ipv6, NEXT_ENTRY(COMMON_IPV6_ADDR),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_ipv6, hdr.dst_addr)),
            },
        [ITEM_IPV6_HAS_FRAG_EXT] =
            {
                .name = "has_frag_ext",
                ...
                .next = NEXT(item_ipv6, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_BF(struct rte_flow_item_ipv6, has_frag_ext, 1)),
            },
        [ITEM_ICMP] =
            {
                .name = "icmp",
        ...
        [ITEM_ICMP_TYPE] =
            {
                ...
                .next = NEXT(item_icmp, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_icmp, hdr.icmp_type)),
            },
        [ITEM_ICMP_CODE] =
            {
                .name = "code",
                ...
                .next = NEXT(item_icmp, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_icmp, hdr.icmp_code)),
            },
        [ITEM_ICMP_IDENT] =
            {
                .name = "ident",
                ...
                .next = NEXT(item_icmp, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_icmp, hdr.icmp_ident)),
            },
        [ITEM_ICMP_SEQ] =
            {
                .name = "seq",
        ...
        [ITEM_UDP_SRC] =
            {
                ...
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_udp, hdr.src_port)),
            },
        [ITEM_UDP_DST] =
            {
                .name = "dst",
                ...
                .next = NEXT(item_udp, NEXT_ENTRY(COMMON_UNSIGNED), item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_udp, hdr.dst_port)),
            },
        [ITEM_TCP] =
            {
                .name = "tcp",
        ...
        [ITEM_TCP_SRC] =
            {
                ...
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_tcp, hdr.src_port)),
            },
        [ITEM_TCP_DST] =
            {
                .name = "dst",
                ...
                .next = NEXT(item_tcp, NEXT_ENTRY(COMMON_UNSIGNED), item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_tcp, hdr.dst_port)),
            },
        [ITEM_TCP_FLAGS] =
            {
                .name = "flags",
                ...
                .next = NEXT(item_tcp, NEXT_ENTRY(COMMON_UNSIGNED), item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_tcp, hdr.tcp_flags)),
            },
        [ITEM_SCTP] =
            {
                .name = "sctp",
        ...
        [ITEM_SCTP_SRC] =
            {
                ...
                .next = NEXT(item_sctp, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_sctp, hdr.src_port)),
            },
        [ITEM_SCTP_DST] =
            {
                .name = "dst",
                ...
                .next = NEXT(item_sctp, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_sctp, hdr.dst_port)),
            },
        [ITEM_SCTP_TAG] =
            {
                .name = "tag",
                ...
                .next = NEXT(item_sctp, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_sctp, hdr.tag)),
            },
        [ITEM_SCTP_CKSUM] =
            {
                .name = "cksum",
                ...
                .next = NEXT(item_sctp, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_sctp, hdr.cksum)),
            },
        [ITEM_VXLAN] =
            {
                .name = "vxlan",
        ...
        [ITEM_VXLAN_LAST_RSVD] =
            {
                ...
                .next = NEXT(item_vxlan, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_vxlan, rsvd1)),
            },
        [ITEM_E_TAG] =
            {
                .name = "e_tag",
        ...
        [ITEM_GRE_PROTO] =
            {
                ...
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_gre, protocol)),
            },
        [ITEM_GRE_C_RSVD0_VER] =
            {
                .name = "c_rsvd0_ver",
                .help = "checksum (1b), undefined (1b), key bit (1b),"
                        " sequence number (1b), reserved 0 (9b),"
                        " version (3b)",
                .next = NEXT(item_gre, NEXT_ENTRY(COMMON_UNSIGNED), item_param),
                .args = ARGS(
                    ARGS_ENTRY_HTON(struct rte_flow_item_gre, c_rsvd0_ver)),
            },
        [ITEM_GRE_C_BIT] =
            {
                .name = "c_bit",
                ...
                .args = ARGS(
                    ARGS_ENTRY_MASK_HTON(struct rte_flow_item_gre, c_rsvd0_ver,
                                         "\x80\x00\x00\x00")),
            },
        [ITEM_GRE_S_BIT] =
            {
                ...
                .next = NEXT(item_gre, NEXT_ENTRY(COMMON_BOOLEAN), item_param),
                .args = ARGS(ARGS_ENTRY_MASK_HTON(struct rte_flow_item_gre, c_rsvd0_ver, "\x10\x00\x00\x00")),
            },
        [ITEM_GRE_K_BIT] =
            {
                .name = "k_bit",
                .help = "key bit (K)",
                .next = NEXT(item_gre, NEXT_ENTRY(COMMON_BOOLEAN), item_param),
                .args = ARGS(ARGS_ENTRY_MASK_HTON(struct rte_flow_item_gre, c_rsvd0_ver, "\x20\x00\x00\x00")),
            },
        [ITEM_FUZZY] =
            {
                .name = "fuzzy",
                .help = "fuzzy pattern match, expect faster than default",
                .priv = PRIV_ITEM(FUZZY, sizeof(struct rte_flow_item_fuzzy)),
        ...
        [ITEM_FUZZY_THRESH] =
            {
                ...
                .next = NEXT(item_fuzzy, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(ARGS_ENTRY(struct rte_flow_item_fuzzy, thresh)),
            },
        [ITEM_GTP] =
            {
                .name = "gtp",
        ...
        [ITEM_GENEVE_PROTO] =
            {
                ...
                .next = NEXT(item_geneve, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_geneve, protocol)),
            },
        [ITEM_GENEVE_OPTLEN] =
            {
                .name = "optlen",
        ...
        [ITEM_ARP_ETH_IPV4] =
            {
                ...
                .priv = PRIV_ITEM(ARP_ETH_IPV4, sizeof(
                                                    struct
        ...
        [ITEM_ICMP6_TYPE] =
            {
                ...
                .next = NEXT(item_icmp6, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_icmp6, type)),
            },
        [ITEM_ICMP6_CODE] =
            {
                .name = "code",
                ...
                .next = NEXT(item_icmp6, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_icmp6, code)),
            },
        [ITEM_ICMP6_ND_NS] =
            {
                .name = "icmp6_nd_ns",
        ...
        [ITEM_ICMP6_ND_OPT] =
            {
                ...
                .help = "match presence of any ICMPv6 neighbor discovery"
                        " option",
                .priv = PRIV_ITEM(ICMP6_ND_OPT, sizeof(
                                                    struct
        ...
        [ITEM_META_DATA] =
            {
                ...
                .next = NEXT(item_meta, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(ARGS_ENTRY_MASK(struct rte_flow_item_meta, data,
                                             "\xff\xff\xff\xff")),
            },
        [ITEM_GRE_KEY] =
            {
        ...
        [ITEM_GENEVE_OPT_LENGTH] =
            {
                ...
                .next = NEXT(item_geneve_opt, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(ARGS_ENTRY_BOUNDED(struct rte_flow_item_geneve_opt,
                                                option_len, 0, 31)),
            },
        [ITEM_GENEVE_OPT_DATA] =
            {
        ...
        [ITEM_PORT_REPRESENTOR] =
            {
                ...
                .help = "match traffic entering the embedded switch from the "
                        "given ethdev",
                .priv = PRIV_ITEM(PORT_REPRESENTOR, sizeof(
                                                        struct
        ...
        [ITEM_REPRESENTED_PORT] =
            {
                ...
                .priv = PRIV_ITEM(REPRESENTED_PORT, sizeof(
                                                        struct
                                                        rte_flow_item_ethdev)),
        ...
        [ITEM_L2TPV2_MSG_DATA_TUNNEL_ID] =
            {
                ...
                .next = NEXT(item_l2tpv2_type_data, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_l2tpv2, hdr.type7
                                                                              .tunnel_id)),
            },
        [ITEM_L2TPV2_MSG_DATA_SESSION_ID] =
            {
        ...
        [ITEM_L2TPV2_MSG_DATA_L_LENGTH] =
            {
                ...
                .next = NEXT(item_l2tpv2_type_data_l,
                             NEXT_ENTRY(COMMON_UNSIGNED), item_param),
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_l2tpv2, hdr.type6
                                                                              .length)),
            },
        [ITEM_L2TPV2_MSG_DATA_L_TUNNEL_ID] =
            {
        ...
        [ITEM_L2TPV2_MSG_DATA_S_TUNNEL_ID] =
            {
                ...
                .next = NEXT(item_l2tpv2_type_data_s,
                             NEXT_ENTRY(COMMON_UNSIGNED), item_param),
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_l2tpv2, hdr.type5
                                                                              .tunnel_id)),
            },
        [ITEM_L2TPV2_MSG_DATA_S_SESSION_ID] =
            {
        ...
        [ITEM_L2TPV2_MSG_DATA_O_TUNNEL_ID] =
            {
                ...
                .next = NEXT(item_l2tpv2_type_data_o,
                             NEXT_ENTRY(COMMON_UNSIGNED), item_param),
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_l2tpv2, hdr.type4
                                                                              .tunnel_id)),
            },
        [ITEM_L2TPV2_MSG_DATA_O_SESSION_ID] =
            {
        ...
        [ITEM_L2TPV2_MSG_DATA_L_S_LENGTH] =
            {
                ...
                .next = NEXT(item_l2tpv2_type_data_l_s,
                             NEXT_ENTRY(COMMON_UNSIGNED), item_param),
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_l2tpv2, hdr.type3
                                                                              .length)),
            },
        [ITEM_L2TPV2_MSG_DATA_L_S_TUNNEL_ID] =
            {
        ...
        [ITEM_L2TPV2_MSG_CTRL_LENGTH] =
            {
                .name = "length",
                .help = "message length",
                .next = NEXT(item_l2tpv2_type_ctrl, NEXT_ENTRY(COMMON_UNSIGNED),
                             item_param),
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_l2tpv2, hdr.type3
                                                                              .length)),
            },
        ...
        [ITEM_PPP_ADDR] =
            {
                .name = "addr",
                .help = "PPP address",
                .next = NEXT(item_ppp, NEXT_ENTRY(COMMON_UNSIGNED), item_param),
        ...
        [ACTION_RSS_KEY] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_ARB(
                                 ...
                                 sizeof(
                                     ((struct rte_flow_action_rss *)0)->key)),
                             ARGS_ENTRY_ARB(
                                 offsetof(struct action_rss_data, conf) +
        ...
        [ACTION_RSS_KEY_LEN] =
            {
                ...
                .next = NEXT(action_rss, NEXT_ENTRY(COMMON_UNSIGNED)),
                .args = ARGS(ARGS_ENTRY_ARB_BOUNDED(offsetof(
                                                        struct action_rss_data,
                                                        conf) +
                                                        offsetof(
                                                            struct rte_flow_action_rss, key_len),
                                                    sizeof(
                                                        ((struct rte_flow_action_rss *)0)
                                                            ->key_len),
                                                    0, RSS_HASH_KEY_LENGTH)),
            },
        [ACTION_RSS_QUEUES] =
            {
                .name = "queues",
        ...
        [ACTION_VF_ORIGINAL] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_BF(struct rte_flow_action_vf, original,
                                           1)),
                .call = parse_vc_conf,
            },
        [ACTION_VF_ID] =
        ...
        [ACTION_OF_SET_MPLS_TTL] =
            {
                ...
                .priv = PRIV_ACTION(OF_SET_MPLS_TTL,
                                    sizeof(
        ...
        [ACTION_OF_SET_NW_TTL] =
            {
                ...
                .priv = PRIV_ACTION(OF_SET_NW_TTL,
                                    sizeof(
        ...
        [ACTION_OF_SET_VLAN_VID] =
            {
                ...
                .priv = PRIV_ACTION(OF_SET_VLAN_VID,
                                    sizeof(
        ...
        [ACTION_OF_SET_VLAN_PCP] =
            {
                ...
                .priv = PRIV_ACTION(OF_SET_VLAN_PCP,
                                    sizeof(
        ...
        [ACTION_OF_POP_MPLS_ETHERTYPE] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_action_of_pop_mpls,
                                             ethertype)),
                .call = parse_vc_conf,
            },
        [ACTION_OF_PUSH_MPLS] =
        ...
        [ACTION_SET_IPV4_SRC_IPV4_SRC] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_action_set_ipv4,
                                             ipv4_addr)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_IPV4_DST] =
        ...
        [ACTION_SET_IPV6_SRC_IPV6_SRC] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_action_set_ipv6,
                                             ipv6_addr)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_IPV6_DST] =
        ...
        [ACTION_SET_TP_SRC_TP_SRC] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_action_set_tp,
                                             port)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_TP_DST] =
        ...
        [ACTION_SET_TTL_TTL] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_action_set_ttl,
                                             ttl_value)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_MAC_SRC] =
        ...
        [ACTION_SET_MAC_SRC_MAC_SRC] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_action_set_mac,
                                             mac_addr)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_MAC_DST] =
        ...
        [ACTION_MODIFY_FIELD_SRC_VALUE] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_ARB(0, 0), ARGS_ENTRY_ARB(0, 0),
        ...
        [SET] =
            {
                ...
                .next = NEXT(NEXT_ENTRY(SET_RAW_ENCAP, SET_RAW_DECAP,
                                        SET_SAMPLE_ACTIONS)),
                .call = parse_set_init,
            },
        /* Sub-level commands. */
        [SET_RAW_ENCAP] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_ARB_BOUNDED(offsetof(struct buffer,
                                                             port),
                                                    sizeof(((struct buffer *)0)->port), 0,
        ...
        [ACTION_SET_TAG_DATA] =
            {
                ...
                .help = "tag value",
                .next = NEXT(action_set_tag, NEXT_ENTRY(COMMON_UNSIGNED)),
                .args = ARGS(ARGS_ENTRY(struct rte_flow_action_set_tag, data)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_TAG_MASK] =
            {
                ...
                .next = NEXT(action_set_tag, NEXT_ENTRY(COMMON_UNSIGNED)),
                .args = ARGS(ARGS_ENTRY(struct rte_flow_action_set_tag, mask)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_META] =
            {
        ...
        [ACTION_SET_META_DATA] =
            {
                ...
                .args = ARGS(ARGS_ENTRY(struct rte_flow_action_set_meta, data)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_META_MASK] =
            {
                ...
                .next = NEXT(action_set_meta, NEXT_ENTRY(COMMON_UNSIGNED)),
                .args = ARGS(ARGS_ENTRY(struct rte_flow_action_set_meta, mask)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_IPV4_DSCP] =
            {
        ...
        [ACTION_SET_IPV4_DSCP_VALUE] =
            {
                ...
                .args = ARGS(ARGS_ENTRY(struct rte_flow_action_set_dscp, dscp)),
                .call = parse_vc_conf,
            },
        [ACTION_SET_IPV6_DSCP] =
            {
        ...
        [ACTION_AGE] =
            {
                ...
                .priv = PRIV_ACTION(AGE, sizeof(struct rte_flow_action_age)),
                .call = parse_vc,
            },
        [ACTION_AGE_TIMEOUT] =
            {
        ...
        [ACTION_SAMPLE] =
            {
                ...
                .priv = PRIV_ACTION(SAMPLE, sizeof(struct action_sample_data)),
        ...
        [ACTION_SAMPLE_RATIO] =
            {
                ...
                .args = ARGS(ARGS_ENTRY_ARB(offsetof(struct action_sample_data,
                                                     conf) +
                                                offsetof(struct
                                                         rte_flow_action_sample,
                                                         ratio),
                                            sizeof(((struct rte_flow_action_sample
                                                         *)0)
                                                       ->ratio))),
            },
        [ACTION_SAMPLE_INDEX] =
        ...
        [ACTION_PORT_REPRESENTOR] =
            {
                .name = "port_representor",
                .help = "at embedded switch level, send matching traffic to "
                        "the given ethdev",
        ...
        [INDIRECT_ACTION_DESTROY_ID] =
            {
                ...
                .next = NEXT(next_ia_destroy_attr,
                             NEXT_ENTRY(COMMON_INDIRECT_ACTION_ID)),
                .args = ARGS(ARGS_ENTRY_PTR(struct buffer, args.ia_destroy
                                                               .action_id)),
                .call = parse_ia_destroy,
            },
        /* Indirect action create arguments. */
        ...
        [ITEM_POL_POLICY] =
            {
                .name = "policy",
                .help = "add port meter policy",
                .next = NEXT(NEXT_ENTRY(ACTION_POL_R), NEXT_ENTRY(ACTION_POL_Y),
...
static size_t arg_entry_bf_fill(void *dst, uintmax_t val,
...
static int strcmp_partial(const char *full, const char *partial,
                          size_t partial_len) {
 ...
 */
static int parse_prefix(struct context *ctx, const struct token *token,
                        const char *str, unsigned int len, void *buf,
                        unsigned int size) {
...
static int parse_default(struct context *ctx, const struct token *token,
                         const char *str, unsigned int len, void *buf,
                         unsigned int size) {
  (void)ctx;
  (void)buf;
...
static int parse_init(struct context *ctx, const struct token *token,
                      const char *str, unsigned int len, void *buf,
                      unsigned int size) {
...
static int parse_ia(struct context *ctx, const struct token *token,
                    const char *str, unsigned int len, void *buf,
                    unsigned int size) {
  struct buffer *out = buf;
  ...
  case INDIRECT_ACTION_UPDATE:
    out->args.vc.actions =
        (void *)RTE_ALIGN_CEIL((uintptr_t)(out + 1), sizeof(double));
...
static int parse_ia_destroy(struct context *ctx, const struct token *token,
  ...
  action_id =
      out->args.ia_destroy.action_id + out->args.ia_destroy.action_id_n++;
...
/** Parse tokens for indirect action commands. */
static int parse_qia(struct context *ctx, const struct token *token,
                     const char *str, unsigned int len, void *buf,
                     unsigned int size) {
  struct buffer *out = buf;
...
/** Parse tokens for indirect action destroy command. */
static int parse_qia_destroy(struct context *ctx, const struct token *token,
                             const char *str, unsigned int len, void *buf,
                             unsigned int size) {
  struct buffer *out = buf;
  uint32_t *action_id;
...
static int parse_mp(struct context *ctx, const struct token *token,
                    const char *str, unsigned int len, void *buf,
                    unsigned int size) {
  struct buffer *out = buf;
...
static int parse_vc(struct context *ctx, const struct token *token,
                    const char *str, unsigned int len, void *buf,
                    unsigned int size) {
  struct buffer *out = buf;
  uint8_t *data;
  ...
  case ITEM_PATTERN:
    ...
    return len;
  case ITEM_END:
    if ((out->command == VALIDATE || out->command == CREATE) && ctx->last)
      return -1;
    if (out->command == PATTERN_TEMPLATE_CREATE && !ctx->last)
      return -1;
    break;
  case ACTIONS:
    out->args.vc.actions = (void *)RTE_ALIGN_CEIL(
        (uintptr_t)(out->args.vc.pattern + out->args.vc.pattern_n),
        sizeof(double));
  ...
  if (!out->args.vc.actions) {
    ...
    data = (void *)RTE_ALIGN_FLOOR((uintptr_t)(out->args.vc.data - data_size),
                                   sizeof(double));
...
static int parse_vc_spec(struct context *ctx, const struct token *token,
                         const char *str, unsigned int len, void *buf,
                         unsigned int size) {
  struct buffer *out = buf;
...
static int parse_vc_conf(struct context *ctx, const struct token *token,
                         const char *str, unsigned int len, void *buf,
                         unsigned int size) {
  struct buffer *out = buf;
...
static int parse_vc_item_ecpri_type(struct context *ctx,
                                    const struct token *token, const char *str,
                                    unsigned int len, void *buf,
                                    unsigned int size) {
  ...
  data_size = ctx->objdata / 3; /* spec, last, mask */
  ecpri_mask =
      (struct rte_flow_item_ecpri *)(out->args.vc.data + (data_size * 2));
...
static int parse_vc_item_l2tpv2_type(struct context *ctx,
                                     const struct token *token, const char *str,
                                     unsigned int len, void *buf,
                                     unsigned int size) {
  ...
  data_size = ctx->objdata / 3; /* spec, last, mask */
  l2tpv2_mask =
      (struct rte_flow_item_l2tpv2 *)(out->args.vc.data + (data_size * 2));
...
static int parse_vc_action_meter_color_type(struct context *ctx,
                                            const struct token *token,
                                            const char *str, unsigned int len,
                                            void *buf, unsigned int size) {
  ...
  conf = (struct rte_flow_action_meter_color *)(uintptr_t)(action_data->conf);
  conf->color = color;
  return len;
}

/** Parse RSS action. */
static int parse_vc_action_rss(struct context *ctx, const struct token *token,
                               const char *str, unsigned int len, void *buf,
                               unsigned int size) {
  struct buffer *out = buf;
  struct rte_flow_action *action;
 ...
 */
static int parse_vc_action_rss_func(struct context *ctx,
                                    const struct token *token, const char *str,
                                    unsigned int len, void *buf,
                                    unsigned int size) {
  struct action_rss_data *action_rss_data;
 ...
 */
static int parse_vc_action_rss_type(struct context *ctx,
                                    const struct token *token, const char *str,
                                    unsigned int len, void *buf,
                                    unsigned int size) {
 ...
 */
static int parse_vc_action_rss_queue(struct context *ctx,
                                     const struct token *token, const char *str,
                                     unsigned int len, void *buf,
                                     unsigned int size) {
...
static int parse_setup_vxlan_encap_data(
  ...
  *action_vxlan_encap_data = (struct action_vxlan_encap_data){
  ...
  if (!vxlan_encap_conf.select_ipv4) {
    memcpy(&action_vxlan_encap_data->item_ipv6.hdr.src_addr,
           &vxlan_encap_conf.ipv6_src, sizeof(vxlan_encap_conf.ipv6_src));
    memcpy(&action_vxlan_encap_data->item_ipv6.hdr.dst_addr,
           &vxlan_encap_conf.ipv6_dst, sizeof(vxlan_encap_conf.ipv6_dst));
  ...
  if (!vxlan_encap_conf.select_vlan)
    action_vxlan_encap_data->items[1].type = RTE_FLOW_ITEM_TYPE_VOID;
  if (vxlan_encap_conf.select_tos_ttl) {
    if (vxlan_encap_conf.select_ipv4) {
      static struct rte_flow_item_ipv4 ipv4_mask_tos;

      memcpy(&ipv4_mask_tos, &rte_flow_item_ipv4_mask, sizeof(ipv4_mask_tos));
      ...
      action_vxlan_encap_data->items[2].mask = &ipv4_mask_tos;
    } else {
      static struct rte_flow_item_ipv6 ipv6_mask_tos;

      memcpy(&ipv6_mask_tos, &rte_flow_item_ipv6_mask, sizeof(ipv6_mask_tos));
      ipv6_mask_tos.hdr.vtc_flow |= RTE_BE32(0xfful << RTE_IPV6_HDR_TC_SHIFT);
      ipv6_mask_tos.hdr.hop_limits = 0xff;
      action_vxlan_encap_data->item_ipv6.hdr.vtc_flow |= rte_cpu_to_be_32(
          (uint32_t)vxlan_encap_conf.ip_tos << RTE_IPV6_HDR_TC_SHIFT);
      action_vxlan_encap_data->item_ipv6.hdr.hop_limits =
          vxlan_encap_conf.ip_ttl;
      action_vxlan_encap_data->items[2].mask = &ipv6_mask_tos;
    }
  }
...
static int parse_vc_action_vxlan_encap(struct context *ctx,
                                       const struct token *token,
                                       const char *str, unsigned int len,
                                       void *buf, unsigned int size) {
  struct buffer *out = buf;
...
static int parse_setup_nvgre_encap_data(
  ...
  *action_nvgre_encap_data = (struct action_nvgre_encap_data){
  ...
  if (!nvgre_encap_conf.select_ipv4) {
    memcpy(&action_nvgre_encap_data->item_ipv6.hdr.src_addr,
           &nvgre_encap_conf.ipv6_src, sizeof(nvgre_encap_conf.ipv6_src));
    memcpy(&action_nvgre_encap_data->item_ipv6.hdr.dst_addr,
           &nvgre_encap_conf.ipv6_dst, sizeof(nvgre_encap_conf.ipv6_dst));
  ...
  if (!nvgre_encap_conf.select_vlan)
    action_nvgre_encap_data->items[1].type = RTE_FLOW_ITEM_TYPE_VOID;
...
static int parse_vc_action_nvgre_encap(struct context *ctx,
                                       const struct token *token,
                                       const char *str, unsigned int len,
                                       void *buf, unsigned int size) {
  struct buffer *out = buf;
...
static int parse_vc_action_l2_encap(struct context *ctx,
                                    const struct token *token, const char *str,
                                    unsigned int len, void *buf,
                                    unsigned int size) {
  struct buffer *out = buf;
  ...
  *action_encap_data = (struct action_raw_encap_data){
  ...
  memcpy(eth.dst.addr_bytes, l2_encap_conf.eth_dst, RTE_ETHER_ADDR_LEN);
  memcpy(eth.src.addr_bytes, l2_encap_conf.eth_src, RTE_ETHER_ADDR_LEN);
  ...
  action_encap_data->conf.size = header - action_encap_data->data;
  action->conf = &action_encap_data->conf;
  return ret;
}

/** Parse l2 decap action. */
static int parse_vc_action_l2_decap(struct context *ctx,
                                    const struct token *token, const char *str,
                                    unsigned int len, void *buf,
                                    unsigned int size) {
  struct buffer *out = buf;
  ...
  *action_decap_data = (struct action_raw_decap_data){
  ...
  action_decap_data->conf.size = header - action_decap_data->data;
...
static int parse_vc_action_mplsogre_encap(struct context *ctx,
                                          const struct token *token,
                                          const char *str, unsigned int len,
                                          void *buf, unsigned int size) {
  struct buffer *out = buf;
  ...
  *action_encap_data = (struct action_raw_encap_data){
  ...
  if (mplsogre_encap_conf.select_vlan)
    ...
    eth.type = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6);
  memcpy(eth.dst.addr_bytes, mplsogre_encap_conf.eth_dst, RTE_ETHER_ADDR_LEN);
  memcpy(eth.src.addr_bytes, mplsogre_encap_conf.eth_src, RTE_ETHER_ADDR_LEN);
  memcpy(header, &eth, sizeof(eth));
  header += sizeof(eth);
  if (mplsogre_encap_conf.select_vlan) {
  ...
  if (mplsogre_encap_conf.select_ipv4) {
    ...
    memcpy(&ipv6.hdr.src_addr, &mplsogre_encap_conf.ipv6_src,
           sizeof(mplsogre_encap_conf.ipv6_src));
    memcpy(&ipv6.hdr.dst_addr, &mplsogre_encap_conf.ipv6_dst,
...
static int parse_vc_action_mplsogre_decap(struct context *ctx,
                                          const struct token *token,
                                          const char *str, unsigned int len,
                                          void *buf, unsigned int size) {
  struct buffer *out = buf;
...
static int parse_vc_action_mplsoudp_encap(struct context *ctx,
                                          const struct token *token,
                                          const char *str, unsigned int len,
                                          void *buf, unsigned int size) {
  struct buffer *out = buf;
  ...
  *action_encap_data = (struct action_raw_encap_data){
  ...
  if (mplsoudp_encap_conf.select_vlan)
    ...
    eth.type = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6);
  memcpy(eth.dst.addr_bytes, mplsoudp_encap_conf.eth_dst, RTE_ETHER_ADDR_LEN);
  memcpy(eth.src.addr_bytes, mplsoudp_encap_conf.eth_src, RTE_ETHER_ADDR_LEN);
  memcpy(header, &eth, sizeof(eth));
  header += sizeof(eth);
  if (mplsoudp_encap_conf.select_vlan) {
  ...
  if (mplsoudp_encap_conf.select_ipv4) {
    ...
    memcpy(&ipv6.hdr.src_addr, &mplsoudp_encap_conf.ipv6_src,
           sizeof(mplsoudp_encap_conf.ipv6_src));
    memcpy(&ipv6.hdr.dst_addr, &mplsoudp_encap_conf.ipv6_dst,
...
static int parse_vc_action_mplsoudp_decap(struct context *ctx,
                                          const struct token *token,
                                          const char *str, unsigned int len,
                                          void *buf, unsigned int size) {
  struct buffer *out = buf;
...
static int parse_vc_action_raw_decap_index(struct context *ctx,
                                           const struct token *token,
                                           const char *str, unsigned int len,
                                           void *buf, unsigned int size) {
  ...
  RTE_SET_USED(buf);
  RTE_SET_USED(size);
  arg = ARGS_ENTRY_ARB_BOUNDED(offsetof(struct action_raw_decap_data, idx),
...
static int parse_vc_action_raw_encap_index(struct context *ctx,
                                           const struct token *token,
                                           const char *str, unsigned int len,
                                           void *buf, unsigned int size) {
  ...
  if (ctx->curr != ACTION_RAW_ENCAP_INDEX_VALUE)
    return -1;
  arg = ARGS_ENTRY_ARB_BOUNDED(offsetof(struct action_raw_encap_data, idx),
...
static int parse_vc_action_raw_encap(struct context *ctx,
                                     const struct token *token, const char *str,
                                     unsigned int len, void *buf,
                                     unsigned int size) {
  struct buffer *out = buf;
...
static int parse_vc_action_raw_decap(struct context *ctx,
                                     const struct token *token, const char *str,
                                     unsigned int len, void *buf,
                                     unsigned int size) {
  struct buffer *out = buf;
...
static int parse_vc_action_sample_index(struct context *ctx,
  ...
  if (ctx->curr != ACTION_SAMPLE_INDEX_VALUE)
    return -1;
  arg = ARGS_ENTRY_ARB_BOUNDED(offsetof(struct action_sample_data, idx),
                               sizeof(((struct action_sample_data *)0)->idx), 0,
                               RAW_SAMPLE_CONFS_MAX_NUM - 1);
  if (push_args(ctx, arg))
    return -1;
  ret = parse_int(ctx, token, str, len, NULL, 0);
  if (ret < 0) {
...
static int parse_vc_action_conntrack_update(struct context *ctx,
  ...
  if (ctx->curr == ACTION_CONNTRACK_UPDATE_DIR) {
    ct_modify->new_ct.is_original_dir = conntrack_context.is_original_dir;
    ct_modify->direction = 1;
  } else {
    uint32_t old_dir;

    old_dir = ct_modify->new_ct.is_original_dir;
    memcpy(&ct_modify->new_ct, &conntrack_context, sizeof(conntrack_context));
...
static int parse_destroy(struct context *ctx, const struct token *token,
                         const char *str, unsigned int len, void *buf,
                         unsigned int size) {
  struct buffer *out = buf;
...
static int parse_flush(struct context *ctx, const struct token *token,
                       const char *str, unsigned int len, void *buf,
                       unsigned int size) {
  struct buffer *out = buf;
...
static int parse_dump(struct context *ctx, const struct token *token,
                      const char *str, unsigned int len, void *buf,
                      unsigned int size) {
  struct buffer *out = buf;
...
static int parse_query(struct context *ctx, const struct token *token,
                       const char *str, unsigned int len, void *buf,
                       unsigned int size) {
  struct buffer *out = buf;
...
/** Parse action names. */
static int parse_action(struct context *ctx, const struct token *token,
                        const char *str, unsigned int len, void *buf,
                        unsigned int size) {
  struct buffer *out = buf;
  const struct arg *arg = pop_args(ctx);
  ...
  for (i = 0; next_action[i]; ++i) {
    ...
    if (!priv)
      goto error;
    if (out)
      memcpy((uint8_t *)ctx->object + arg->offset, &priv->type, arg->size);
    return len;
  }
error:
  push_args(ctx, arg);
...
static int parse_list(struct context *ctx, const struct token *token,
                      const char *str, unsigned int len, void *buf,
                      unsigned int size) {
  struct buffer *out = buf;
...
static int parse_aged(struct context *ctx, const struct token *token,
                      const char *str, unsigned int len, void *buf,
                      unsigned int size) {
  struct buffer *out = buf;
...
static int parse_isolate(struct context *ctx, const struct token *token,
                         const char *str, unsigned int len, void *buf,
                         unsigned int size) {
  struct buffer *out = buf;
...
static int parse_configure(struct context *ctx, const struct token *token,
                           const char *str, unsigned int len, void *buf,
                           unsigned int size) {
  struct buffer *out = buf;
...
static int parse_template(struct context *ctx, const struct token *token,
                          const char *str, unsigned int len, void *buf,
                          unsigned int size) {
  struct buffer *out = buf;
  ...
  if (!out)
    return len;
  if (!out->command) {
    if (ctx->curr != PATTERN_TEMPLATE && ctx->curr != ACTIONS_TEMPLATE)
      return -1;
    if (sizeof(*out) > size)
      return -1;
    out->command = ctx->curr;
  ...
  case ACTIONS_TEMPLATE_MASK:
    out->args.vc.masks = (void *)RTE_ALIGN_CEIL(
        (uintptr_t)(out->args.vc.actions + out->args.vc.actions_n),
        sizeof(double));
...
static int parse_template_destroy(struct context *ctx,
                                  const struct token *token, const char *str,
                                  unsigned int len, void *buf,
                                  unsigned int size) {
  struct buffer *out = buf;
  ...
  if (!out)
    return len;
  if (!out->command || out->command == PATTERN_TEMPLATE ||
...
static int parse_table(struct context *ctx, const struct token *token,
                       const char *str, unsigned int len, void *buf,
                       unsigned int size) {
  struct buffer *out = buf;
  uint32_t *template_id;
  ...
  case TABLE_PATTERN_TEMPLATE:
    out->args.table.pat_templ_id =
        (void *)RTE_ALIGN_CEIL((uintptr_t)(out + 1), sizeof(double));
    template_id =
        out->args.table.pat_templ_id + out->args.table.pat_templ_id_n++;
    if ((uint8_t *)template_id > (uint8_t *)out + size)
      return -1;
    ctx->objdata = 0;
    ctx->object = template_id;
  ...
  case TABLE_ACTIONS_TEMPLATE:
    out->args.table.act_templ_id =
        (void *)RTE_ALIGN_CEIL((uintptr_t)(out->args.table.pat_templ_id +
                                           out->args.table.pat_templ_id_n),
                               sizeof(double));
    template_id =
        out->args.table.act_templ_id + out->args.table.act_templ_id_n++;
    if ((uint8_t *)template_id > (uint8_t *)out + size)
      return -1;
    ctx->objdata = 0;
    ctx->object = template_id;
...
static int parse_table_destroy(struct context *ctx, const struct token *token,
                               const char *str, unsigned int len, void *buf,
                               unsigned int size) {
  struct buffer *out = buf;
  uint32_t *table_id;
  ...
  table_id =
      out->args.table_destroy.table_id + out->args.table_destroy.table_id_n++;
...
static int parse_qo(struct context *ctx, const struct token *token,
                    const char *str, unsigned int len, void *buf,
                    unsigned int size) {
  struct buffer *out = buf;
...
static int parse_qo_destroy(struct context *ctx, const struct token *token,
                            const char *str, unsigned int len, void *buf,
                            unsigned int size) {
  struct buffer *out = buf;
  uint32_t *flow_id;
  ...
  case QUEUE_DESTROY_ID:
    flow_id = out->args.destroy.rule + out->args.destroy.rule_n++;
...
static int parse_push(struct context *ctx, const struct token *token,
                      const char *str, unsigned int len, void *buf,
                      unsigned int size) {
  struct buffer *out = buf;
...
static int parse_pull(struct context *ctx, const struct token *token,
                      const char *str, unsigned int len, void *buf,
                      unsigned int size) {
  struct buffer *out = buf;
...
static int parse_flex(struct context *ctx, const struct token *token,
                      const char *str, unsigned int len, void *buf,
                      unsigned int size) {
  struct buffer *out = buf;
...
static int parse_tunnel(struct context *ctx, const struct token *token,
                        const char *str, unsigned int len, void *buf,
                        unsigned int size) {
  struct buffer *out = buf;
 ...
 * storage location.
 */
static int parse_int(struct context *ctx, const struct token *token,
                     const char *str, unsigned int len, void *buf,
                     unsigned int size) {
  const struct arg *arg = pop_args(ctx);
  uintmax_t u;
 ...
 */
static int parse_string(struct context *ctx, const struct token *token,
                        const char *str, unsigned int len, void *buf,
                        unsigned int size) {
  ...
  if (arg_addr->size) {
    memcpy((uint8_t *)ctx->object + arg_addr->offset,
           (void *[]){(uint8_t *)ctx->object + arg_data->offset},
           arg_addr->size);
    if (ctx->objmask)
      memcpy((uint8_t *)ctx->objmask + arg_addr->offset,
             (void *[]){(uint8_t *)ctx->objmask + arg_data->offset},
             arg_addr->size);
  }
  return len;
error:
...
static int parse_hex_string(const char *src, uint8_t *dst, uint32_t *size) {
...
static int parse_hex(struct context *ctx, const struct token *token,
  ...
  if (ctx->objmask)
    memset((uint8_t *)ctx->objmask + arg_data->offset, 0xff, hexlen);
  /* Save address if requested. */
  if (arg_addr->size) {
    memcpy((uint8_t *)ctx->object + arg_addr->offset,
           (void *[]){(uint8_t *)ctx->object + arg_data->offset},
 ...
 */
static int parse_string0(struct context *ctx,
                         const struct token *token __rte_unused,
                         const char *str, unsigned int len, void *buf,
                         unsigned int size) {
  const struct arg *arg_data = pop_args(ctx);
 ...
 * location.
 */
static int parse_mac_addr(struct context *ctx, const struct token *token,
                          const char *str, unsigned int len, void *buf,
                          unsigned int size) {
  const struct arg *arg = pop_args(ctx);
 ...
 * location.
 */
static int parse_ipv4_addr(struct context *ctx, const struct token *token,
                           const char *str, unsigned int len, void *buf,
                           unsigned int size) {
  const struct arg *arg = pop_args(ctx);
  char str2[len + 1];
 ...
 * location.
 */
static int parse_ipv6_addr(struct context *ctx, const struct token *token,
                           const char *str, unsigned int len, void *buf,
                           unsigned int size) {
  const struct arg *arg = pop_args(ctx);
  char str2[len + 1];
...
static const char *const boolean_name[] = {
    "0", "1", "false", "true", "no", "yes", "N", "Y", "off", "on", NULL,
};
 ...
 * location.
 */
static int parse_boolean(struct context *ctx, const struct token *token,
                         const char *str, unsigned int len, void *buf,
                         unsigned int size) {
  const struct arg *arg = pop_args(ctx);
  unsigned int i;
...
static int parse_port(struct context *ctx, const struct token *token,
                      const char *str, unsigned int len, void *buf,
                      unsigned int size) {
...
static int parse_set_raw_encap_decap(struct context *ctx,
                                     const struct token *token, const char *str,
                                     unsigned int len, void *buf,
                                     unsigned int size) {
  struct buffer *out = buf;
...
/** Parse set command, initialize output buffer for subsequent tokens. */
static int parse_set_sample_action(struct context *ctx,
                                   const struct token *token, const char *str,
                                   unsigned int len, void *buf,
                                   unsigned int size) {
  struct buffer *out = buf;
 ...
 */
static int parse_set_init(struct context *ctx, const struct token *token,
                          const char *str, unsigned int len, void *buf,
                          unsigned int size) {
  struct buffer *out = buf;
  ...
  if (!out->command) {
    ...
    out->command = ctx->curr;
    out->args.vc.data = (uint8_t *)out + size;
    ctx->object = (void *)RTE_ALIGN_CEIL((uintptr_t)(out + 1), sizeof(double));
  }
  return len;
}
 ...
 */
static int parse_flex_handle(struct context *ctx, const struct token *token,
                             const char *str, unsigned int len, void *buf,
                             unsigned int size) {
  ...
  if (offset == offsetof(struct rte_flow_item_flex, handle)) {
    ...
    handle = (uint16_t)(uintptr_t)((struct rte_flow_item_flex *)ctx->object)
                 ->pattern;
...
/** No completion. */
static int comp_none(struct context *ctx, const struct token *token,
...
static int comp_boolean(struct context *ctx, const struct token *token,
                        unsigned int ent, char *buf, unsigned int size) {
  unsigned int i;
...
static int comp_action(struct context *ctx, const struct token *token,
                       unsigned int ent, char *buf, unsigned int size) {
  unsigned int i;
  ...
  for (i = 0; next_action[i]; ++i)
    if (buf && i == ent)
      return strlcpy(buf, token_list[next_action[i]].name, size);
  if (buf)
    return -1;
  return i;
}
...
static int comp_port(struct context *ctx, const struct token *token,
                     unsigned int ent, char *buf, unsigned int size) {
  unsigned int i = 0;
  portid_t p;
...
static int comp_rule_id(struct context *ctx, const struct token *token,
                        unsigned int ent, char *buf, unsigned int size) {
  unsigned int i = 0;
...
static int comp_vc_action_rss_type(struct context *ctx,
                                   const struct token *token, unsigned int ent,
                                   char *buf, unsigned int size) {
  unsigned int i;
...
static int comp_vc_action_rss_queue(struct context *ctx,
                                    const struct token *token, unsigned int ent,
                                    char *buf, unsigned int size) {
  (void)ctx;
  (void)token;
...
static int comp_set_raw_index(struct context *ctx, const struct token *token,
                              unsigned int ent, char *buf, unsigned int size) {
  uint16_t idx = 0;
  uint16_t nb = 0;
...
/** Complete index number for set raw_encap/raw_decap commands. */
static int comp_set_sample_index(struct context *ctx, const struct token *token,
                                 unsigned int ent, char *buf,
                                 unsigned int size) {
  uint16_t idx = 0;
  uint16_t nb = 0;
...
static int comp_set_modify_field_op(struct context *ctx,
                                    const struct token *token, unsigned int ent,
                                    char *buf, unsigned int size) {
  RTE_SET_USED(ctx);
  RTE_SET_USED(token);
...
static int comp_set_modify_field_id(struct context *ctx,
                                    const struct token *token, unsigned int ent,
                                    char *buf, unsigned int size) {
  const char *name;
...
static int comp_pattern_template_id(struct context *ctx,
                                    const struct token *token, unsigned int ent,
                                    char *buf, unsigned int size) {
  unsigned int i = 0;
  struct rte_port *port;
...
static int comp_actions_template_id(struct context *ctx,
                                    const struct token *token, unsigned int ent,
                                    char *buf, unsigned int size) {
  unsigned int i = 0;
  struct rte_port *port;
...
static int comp_table_id(struct context *ctx, const struct token *token,
                         unsigned int ent, char *buf, unsigned int size) {
  unsigned int i = 0;
  struct rte_port *port;
...
static int comp_queue_id(struct context *ctx, const struct token *token,
                         unsigned int ent, char *buf, unsigned int size) {
  unsigned int i = 0;
  struct rte_port *port;
...
static int cmd_flow_parse(cmdline_parse_token_hdr_t *hdr, const char *src,
...
int flow_parse(const char *src, void *result, unsigned int size,
...
static int cmd_flow_complete_get_nb(cmdline_parse_token_hdr_t *hdr) {
  struct context *ctx = &cmd_flow_context;
...
static int cmd_flow_complete_get_elt(cmdline_parse_token_hdr_t *hdr, int index,
...
static int cmd_flow_get_help(cmdline_parse_token_hdr_t *hdr, char *dst,
                             unsigned int size) {
  struct context *ctx = &cmd_flow_context;
...
static void cmd_flow_tok(cmdline_parse_token_hdr_t **hdr,
...
static void cmd_flow_parsed(const struct buffer *in) {
  ...
  case PATTERN_TEMPLATE_CREATE:
    port_flow_pattern_template_create(
        in->port, in->args.vc.pat_templ_id,
  ...
  case ACTIONS_TEMPLATE_CREATE:
    port_flow_actions_template_create(
        in->port, in->args.vc.act_templ_id,
  ...
  case ACTIONS_TEMPLATE_DESTROY:
    ...
    break;
  case TABLE_CREATE:
    port_flow_template_table_create(
        in->port, in->args.table.id, &in->args.table.attr,
        in->args.table.pat_templ_id_n, in->args.table.pat_templ_id,
        in->args.table.act_templ_id_n, in->args.table.act_templ_id);
    break;
  case TABLE_DESTROY:
  ...
  case QUEUE_DESTROY:
    port_queue_flow_destroy(in->port, in->queue, in->postpone,
                            in->args.destroy.rule_n, in->args.destroy.rule);
    break;
  case PUSH:
  ...
  case PULL:
    ...
    break;
  case QUEUE_INDIRECT_ACTION_CREATE:
    port_queue_action_handle_create(in->port, in->queue, in->postpone,
  ...
  case QUEUE_INDIRECT_ACTION_DESTROY:
    port_queue_action_handle_destroy(in->port, in->queue, in->postpone,
                                     in->args.ia_destroy.action_id_n,
                                     in->args.ia_destroy.action_id);
    break;
  case QUEUE_INDIRECT_ACTION_UPDATE:
    port_queue_action_handle_update(in->port, in->queue, in->postpone,
                                    in->args.vc.attr.group,
                                    in->args.vc.actions);
    break;
  case INDIRECT_ACTION_CREATE:
    port_action_handle_create(in->port, in->args.vc.attr.group,
                              &((const struct rte_flow_indir_action_conf){
                                  .ingress = in->args.vc.attr.ingress,
                                  .egress = in->args.vc.attr.egress,
                                  .transfer = in->args.vc.attr.transfer,
  ...
  case INDIRECT_ACTION_DESTROY:
    port_action_handle_destroy(in->port, in->args.ia_destroy.action_id_n,
                               in->args.ia_destroy.action_id);
    break;
  case INDIRECT_ACTION_UPDATE:
  ...
  case INDIRECT_ACTION_QUERY:
    ...
    break;
  case VALIDATE:
    port_flow_validate(in->port, &in->args.vc.attr, in->args.vc.pattern,
                       in->args.vc.actions, &in->args.vc.tunnel_ops);
    break;
  case CREATE:
    port_flow_create(in->port, &in->args.vc.attr, in->args.vc.pattern,
                     in->args.vc.actions, &in->args.vc.tunnel_ops);
    break;
  case DESTROY:
    port_flow_destroy(in->port, in->args.destroy.rule_n, in->args.destroy.rule);
    break;
  case FLUSH:
  ...
  case DUMP_ONE:
  case DUMP_ALL:
    port_flow_dump(in->port, in->args.dump.mode, in->args.dump.rule,
                   in->args.dump.file);
    break;
  case QUERY:
    port_flow_query(in->port, in->args.query.rule, &in->args.query.action);
    break;
  case LIST:
    port_flow_list(in->port, in->args.list.group_n, in->args.list.group);
    break;
  case ISOLATE:
  ...
  case ACTION_POL_G:
    ...
    break;
  case FLEX_ITEM_CREATE:
    flex_item_create(in->port, in->args.flex.token, in->args.flex.filename);
    break;
  case FLEX_ITEM_DESTROY:
...
static void cmd_flow_cb(void *arg0, struct cmdline *cl, void *arg2) {
  if (cl == NULL)
...
static void update_fields(uint8_t *buf, struct rte_flow_item *item,
                          uint16_t next_proto) {
...
static const void *flow_item_default_mask(const struct rte_flow_item *item) {
...
/** Dispatch parsed buffer to function calls. */
static void cmd_set_raw_parsed_sample(const struct buffer *in) {
  ...
  uint32_t max_size =
      sizeof(struct rte_flow_action) * ACTION_SAMPLE_ACTIONS_NUM;
  ...
  for (; i <= n - 1; i++) {
    ...
    case RTE_FLOW_ACTION_TYPE_MARK:
      ...
      rte_memcpy(&sample_mark[idx], (const void *)action->conf, size);
      action->conf = &sample_mark[idx];
      break;
    case RTE_FLOW_ACTION_TYPE_COUNT:
      size = sizeof(struct rte_flow_action_count);
      rte_memcpy(&sample_count[idx], (const void *)action->conf, size);
      action->conf = &sample_count[idx];
      break;
    case RTE_FLOW_ACTION_TYPE_QUEUE:
      size = sizeof(struct rte_flow_action_queue);
      rte_memcpy(&sample_queue[idx], (const void *)action->conf, size);
      action->conf = &sample_queue[idx];
      break;
    case RTE_FLOW_ACTION_TYPE_RSS:
      size = sizeof(struct rte_flow_action_rss);
      rss = action->conf;
      rte_memcpy(&sample_rss_data[idx].conf, (const void *)rss, size);
      if (rss->key_len && rss->key) {
        sample_rss_data[idx].conf.key = sample_rss_data[idx].key;
        rte_memcpy((void *)((uintptr_t)sample_rss_data[idx].conf.key),
                   (const void *)rss->key, sizeof(uint8_t) * rss->key_len);
      }
      if (rss->queue_num && rss->queue) {
        sample_rss_data[idx].conf.queue = sample_rss_data[idx].queue;
        rte_memcpy((void *)((uintptr_t)sample_rss_data[idx].conf.queue),
                   (const void *)rss->queue, sizeof(uint16_t) * rss->queue_num);
      }
      action->conf = &sample_rss_data[idx].conf;
      break;
    case RTE_FLOW_ACTION_TYPE_RAW_ENCAP:
      size = sizeof(struct rte_flow_action_raw_encap);
      rte_memcpy(&sample_encap[idx], (const void *)action->conf, size);
      action->conf = &sample_encap[idx];
      break;
    case RTE_FLOW_ACTION_TYPE_PORT_ID:
      size = sizeof(struct rte_flow_action_port_id);
      rte_memcpy(&sample_port_id[idx], (const void *)action->conf, size);
    ...
    case RTE_FLOW_ACTION_TYPE_VF:
      ...
      rte_memcpy(&sample_vf[idx], (const void *)action->conf, size);
...
/** Dispatch parsed buffer to function calls. */
static void cmd_set_raw_parsed(const struct buffer *in) {
  ...
  RTE_ASSERT(in->command == SET_RAW_ENCAP || in->command == SET_RAW_DECAP);
  ...
  *total_size = 0;
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    case RTE_FLOW_ITEM_TYPE_GENEVE_OPT:
      ...
      if (opt->option_len && opt->data) {
        *total_size += opt->option_len * sizeof(uint32_t);
        rte_memcpy(data_tail - (*total_size), opt->data,
    ...
    case RTE_FLOW_ITEM_TYPE_GTP:
      ...
      if (gtp_psc != i + 1) {
        fprintf(stderr, "Error - GTP PSC does not follow GTP\n");
        goto error;
      }
      gtp = item->spec;
      if ((gtp->v_pt_rsv_flags & 0x07) != 0x04) {
        /* Only E flag should be set. */
        fprintf(stderr, "Error - GTP unsupported flags\n");
        goto error;
      } else {
        struct rte_gtp_hdr_ext_word ext_word = {.next_ext = 0x85};

        /* We have to add GTP header extra word. */
        *total_size += sizeof(ext_word);
        rte_memcpy(data_tail - (*total_size), &ext_word, sizeof(ext_word));
      }
      size = sizeof(struct rte_gtp_hdr);
      break;
    case RTE_FLOW_ITEM_TYPE_GTP_PSC:
      if (gtp_psc >= 0) {
        fprintf(stderr, "Error - Multiple GTP PSC items\n");
        goto error;
      } else {
        ...
        *total_size += sizeof(psc);
        rte_memcpy(data_tail - (*total_size), &psc, sizeof(psc));
        gtp_psc = i;
        size = 0;
      }
      break;
    ...
    case RTE_FLOW_ITEM_TYPE_FLEX:
      size = item->spec
                 ? ((const struct rte_flow_item_flex *)item->spec)->length
                 : 0;
      break;
      ...
      if (item->spec) {
        ...
        if (opt->checksum_rsvd.checksum) {
          *total_size += sizeof(opt->checksum_rsvd);
          rte_memcpy(data_tail - (*total_size), &opt->checksum_rsvd,
                     sizeof(opt->checksum_rsvd));
        }
        if (opt->key.key) {
          *total_size += sizeof(opt->key.key);
          rte_memcpy(data_tail - (*total_size), &opt->key.key,
                     sizeof(opt->key.key));
        }
        if (opt->sequence.sequence) {
          *total_size += sizeof(opt->sequence.sequence);
          rte_memcpy(data_tail - (*total_size), &opt->sequence.sequence,
    ...
    *total_size += size;
    ...
    update_fields((data_tail - (*total_size)), item, upper_layer);
    upper_layer = proto;
  }
...
/** Populate help strings for current token (cmdline API). */
static int cmd_set_raw_get_help(cmdline_parse_token_hdr_t *hdr, char *dst,
                                unsigned int size) {
  struct context *ctx = &cmd_flow_context;
  const struct token *token = &token_list[ctx->prev];
...
/** Populate the next dynamic token. */
static void cmd_set_raw_tok(cmdline_parse_token_hdr_t **hdr,
                            cmdline_parse_token_hdr_t **hdr_inst) {
  struct context *ctx = &cmd_flow_context;
...
/** Token generator and output processing callback (cmdline API). */
static void cmd_set_raw_cb(void *arg0, struct cmdline *cl, void *arg2) {
  if (cl == NULL)
...
static void cmd_show_set_raw_parsed(void *parsed_result, struct cmdline *cl,
                                    void *data) {
...
cmdline_parse_token_string_t cmd_show_set_raw_cmd_show =
    TOKEN_STRING_INITIALIZER(struct cmd_show_set_raw_result, cmd_show, "show");
cmdline_parse_token_string_t cmd_show_set_raw_cmd_what =
    TOKEN_STRING_INITIALIZER(struct cmd_show_set_raw_result, cmd_what,
                             "raw_encap#raw_decap");
cmdline_parse_token_num_t cmd_show_set_raw_cmd_index = TOKEN_NUM_INITIALIZER(
    struct cmd_show_set_raw_result, cmd_index, RTE_UINT16);
cmdline_parse_token_string_t cmd_show_set_raw_cmd_all =
    TOKEN_STRING_INITIALIZER(struct cmd_show_set_raw_result, cmd_all, "all");
