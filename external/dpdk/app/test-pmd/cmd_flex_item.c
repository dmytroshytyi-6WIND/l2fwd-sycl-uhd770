...
static struct flex_item *flex_parser_fetch(uint16_t port_id, uint16_t flex_id) {
...
static __rte_always_inline bool match_strkey(const char *key,
                                             const char *pattern) {
  return strncmp(key, pattern, strlen(key)) == 0;
}

static int flex_tunnel_parse(json_t *jtun,
...
static int flex_field_parse(json_t *jfld,
                            struct rte_flow_item_flex_field *fld) {
  const char *key;
  json_t *je;
...
enum flex_link_type { FLEX_LINK_IN = 0, FLEX_LINK_OUT = 1 };

static int flex_link_item_parse(const char *src, struct rte_flow_item *item) {
#define FLEX_PARSE_DATA_SIZE 1024

  int ret;
  uint8_t *ptr, data[FLEX_PARSE_DATA_SIZE] = {
                    0,
                };
  char flow_rule[256];
  ...
  src = flow_rule;
  ret = flow_parse(src, (void *)data, sizeof(data), &attr, &pattern, &actions);
  if (ret)
    return ret;
...
static int flex_link_parse(json_t *jobj, struct rte_flow_item_flex_link *link,
  ...
  json_object_foreach(jobj, key, je) {
    if (match_strkey(key, "item")) {
      if (!json_is_string(je))
        return -EINVAL;
      ret = flex_link_item_parse(json_string_value(je), &link->item);
      if (ret)
        return -EINVAL;
    ...
    if (match_strkey(key, "next")) {
      if (json_is_integer(je))
        link->next = (typeof(link->next))json_integer_value(je);
      else if (json_is_real(je))
        link->next = (typeof(link->next))json_real_value(je);
      else
        return -EINVAL;
    }
  }
...
static int flex_item_config(json_t *jroot,
  ...
  json_object_foreach(jroot, key, jobj) {
    if (match_strkey(key, "tunnel")) {
      ...
      ret = flex_field_parse(jobj, &flex_conf->next_protocol);
      if (ret) {
      ...
      for (i = 0; i < size; i++) {
        ...
        ret = flex_field_parse(ji, flex_conf->sample_data + i);
        if (ret) {
      ...
      for (i = 0; i < size; i++) {
        ji = json_array_get(jobj, i);
        ret = flex_link_parse(ji, flex_conf->input_link + i, FLEX_LINK_IN);
        if (ret) {
      ...
      for (i = 0; i < size; i++) {
        ji = json_array_get(jobj, i);
        ret = flex_link_parse(ji, flex_conf->output_link + i, FLEX_LINK_OUT);
        if (ret) {
...
static struct flex_item *flex_item_init(void) {
  ...
  samples_size =
      RTE_ALIGN(FLEX_ITEM_MAX_SAMPLES_NUM * sizeof(conf->sample_data[0]),
                sizeof(uintptr_t));
  ...
  conf->input_link =
      (typeof(conf->input_link))((uint8_t *)conf->sample_data + samples_size);
  conf->output_link =
      (typeof(conf->output_link))((uint8_t *)conf->input_link + links_size);
...
static int flex_item_build_config(struct flex_item *fp, const char *filename) {
  int ret;
...
void flex_item_create(portid_t port_id, uint16_t flex_id,
                      const char *filename) {
  ...
  if (ret)
    goto out;
  fp->flex_handle =
      rte_flow_flex_item_create(port_id, &fp->flex_conf, &flow_error);
  if (fp->flex_handle) {
...
static cmdline_parse_token_string_t flex_pattern_token =
    TOKEN_STRING_INITIALIZER(struct flex_pattern_set, flex_pattern,
                             "flex_pattern");
static cmdline_parse_token_string_t flex_pattern_is_token =
    TOKEN_STRING_INITIALIZER(struct flex_pattern_set, is_spec, "is");
static cmdline_parse_token_string_t flex_pattern_spec_token =
    TOKEN_STRING_INITIALIZER(struct flex_pattern_set, is_spec, "spec");
 ...
 */
static uint32_t flex_pattern_data(const char *str, uint8_t *data) {
...
static void flex_pattern_parsed_fn(void *parsed_result,
  ...
  if (full_spec) {
    fp->mask.length = flex_pattern_data(res->mask_data, fp->mask_pattern);
    if (!fp->mask.length) {
