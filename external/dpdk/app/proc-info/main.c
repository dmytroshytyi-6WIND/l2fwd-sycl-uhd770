...
#define STATS_BDR_STR(w, s)                                                    \
  printf("%.*s%s%.*s\n", w, STATS_BDR_FMT, s, w, STATS_BDR_FMT)
...
/**< display usage */
static void proc_info_usage(const char *prgname) {
  printf("%s [EAL options] -- -p PORTMASK\n"
         ...
         "collectd format\n"
         "  --host-id STRING: host id used to identify the system process is "
         "running on\n"
 ...
 */
static int parse_xstats_ids(char *list, uint64_t *ids, int limit) {
  int length;
  char *token;
  char *ctx = NULL;
  char *endptr;
...
static int proc_info_preparse_args(int argc, char **argv) {
  char *prgname = argv[0];
  int i;
...
static int proc_info_parse_args(int argc, char **argv) {
  int opt;
  int option_index;
  char *prgname = argv[0];
  ...
  /* Parse command line */
  while ((opt = getopt_long(argc, argv, "p:m", long_option, &option_index)) !=
         EOF) {
    switch (opt) {
    /* portmask */
    case 'p':
    ...
    case 0:
      ...
      /* Print xstats */
        ...
        enable_xstats = 1;
#ifdef RTE_LIB_METRICS
      else if (!strncmp(long_option[option_index].name, "metrics",
                        MAX_LONG_OPT_SZ))
        enable_metrics = 1;
#endif
      /* Reset stats */
      ...
      /* Reset xstats */
                        ...
                        MAX_LONG_OPT_SZ))
        reset_xstats = 1;
      else if (!strncmp(long_option[option_index].name, "show-port",
                        MAX_LONG_OPT_SZ))
        enable_shw_port = 1;
      else if (!strncmp(long_option[option_index].name, "show-tm",
                        MAX_LONG_OPT_SZ))
        enable_shw_tm = 1;
      else if (!strncmp(long_option[option_index].name, "show-crypto",
                        MAX_LONG_OPT_SZ))
        enable_shw_crypto = 1;
      else if (!strncmp(long_option[option_index].name, "show-ring",
                        MAX_LONG_OPT_SZ)) {
        enable_shw_ring = 1;
        ring_name = optarg;
      } else if (!strncmp(long_option[option_index].name, "show-mempool",
                          MAX_LONG_OPT_SZ)) {
        enable_shw_mempool = 1;
        mempool_name = optarg;
      } else if (!strncmp(long_option[option_index].name, "iter-mempool",
                          MAX_LONG_OPT_SZ)) {
        enable_iter_mempool = 1;
        mempool_iter_name = optarg;
      } else if (!strncmp(long_option[option_index].name, "dump-regs",
                          MAX_LONG_OPT_SZ)) {
        enable_dump_regs = 1;
    ...
    case 1:
      ...
      if (!strncmp(long_option[option_index].name, "xstats-name",
                   MAX_LONG_OPT_SZ)) {
        enable_xstats_name = 1;
        xstats_name = optarg;
        printf("name:%s:%s\n", long_option[option_index].name, optarg);
      } else if (!strncmp(long_option[option_index].name, "xstats-ids",
                          MAX_LONG_OPT_SZ)) {
        int ret = parse_xstats_ids(optarg, xstats_ids, MAX_NB_XSTATS_IDS);
        if (ret <= 0) {
...
static void meminfo_display(void) {
...
static void nic_stats_display(uint16_t port_id) {
  ...
  for (i = 0; i < RTE_ETHDEV_QUEUE_STAT_CNTRS; i++) {
    printf("  Stats reg %2d RX-packets: %-10" PRIu64 "  RX-errors: %-10" PRIu64
           "  RX-bytes: %-10" PRIu64 "\n",
  ...
  printf("\n");
  for (i = 0; i < RTE_ETHDEV_QUEUE_STAT_CNTRS; i++) {
    printf("  Stats reg %2d TX-packets: %-10" PRIu64 "  TX-bytes: %-10" PRIu64
           "\n",
...
static void nic_stats_clear(uint16_t port_id) {
...
static void nic_xstats_by_name_display(uint16_t port_id, char *name) {
  uint64_t id;
...
static void nic_xstats_by_ids_display(uint16_t port_id, uint64_t *ids,
                                      int len) {
  ...
  if (len != rte_eth_xstats_get_names_by_id(port_id, xstats_names, len, ids)) {
...
static void nic_xstats_display(uint16_t port_id) {
  struct rte_eth_xstat_name *xstats_names;
  uint64_t *values;
  int len, ret, i;
  static const char *nic_stats_border = "########################";
  ...
  for (i = 0; i < len; i++) {
    if (enable_collectd_format) {
      ...
      collectd_resolve_cnt_type(counter_type, sizeof(counter_type),
                                xstats_names[i].name);
      n = snprintf(buf, MAX_STRING_LEN,
                   "PUTVAL %s/dpdkstat-port.%u/%s-%s N:%" PRIu64 "\n", host_id,
                   port_id, counter_type, xstats_names[i].name, values[i]);
      if (n > sizeof(buf) - 1)
        n = sizeof(buf) - 1;
...
static void nic_xstats_clear(uint16_t port_id) {
  int ret;
...
#ifdef RTE_LIB_METRICS
static void metrics_display(int port_id) {
  ...
  metrics =
      rte_malloc("proc_info_metrics", sizeof(struct rte_metric_value) * len, 0);
  if (metrics == NULL) {
...
static void show_security_context(uint16_t portid, bool inline_offload) {
  void *p_ctx;
  ...
  if (s_cap) {
    printf("\t  -- action (0x%x), protocol (0x%x),"
           " offload flags (0x%x)\n",
           s_cap->action, s_cap->protocol, s_cap->ol_flags);
    printf("\t  -- capabilities - oper type %x\n",
           s_cap->crypto_capabilities->op);
  }
}

static void show_offloads(uint64_t offloads,
...
static void show_port(void) {
  ...
  for (i = 0; i < RTE_MAX_ETHPORTS; i++) {
    ...
    printf("\t  -- driver %s device %s socket %d\n", dev_info.driver_name,
           dev_info.device->name, rte_eth_dev_socket_id(i));
    ...
    if (ret == 0 && fc_conf.mode != RTE_ETH_FC_NONE) {
      printf("\t  -- flow control mode %s%s high %u low %u pause %u%s%s\n",
             ...
             : fc_conf.mode == RTE_ETH_FC_FULL     ? "full"
                                                   : "???",
             fc_conf.autoneg ? " auto" : "", fc_conf.high_water,
    ...
    for (j = 0; j < dev_info.nb_rx_queues; j++) {
      ...
      if (queue_info.rx_buf_size != 0)
        printf(" rx buffer size %u", queue_info.rx_buf_size);

      printf(" mempool %s socket %d", queue_info.mp->name,
    ...
    for (j = 0; j < dev_info.nb_tx_queues; j++) {
      ...
      printf("\t  -- %d descriptors %d", j, queue_info.nb_desc);

      printf(" thresh %u/%u", queue_info.conf.tx_rs_thresh,
    ...
    if (ret == 0) {
      if (rss_conf.rss_key) {
        printf("  - RSS\n");
        printf("\t  -- RSS len %u key (hex):", rss_conf.rss_key_len);
...
static void display_nodecap_info(int is_leaf,
...
static void display_levelcap_info(int is_leaf,
...
static void show_tm(void) {
  ...
  RTE_ETH_FOREACH_DEV(i) {
    ...
    printf("  - Generic for port (%u)\n"
           ...
           i, dev_info.driver_name, dev_info.max_vfs, dev_info.max_tx_queues,
    ...
    printf("  - MAX: nodes (%u) levels (%u) children (%u)\n", cap.n_nodes_max,
           cap.n_levels_max, cap.sched_n_children_max);

    printf("  - identical nodes: non leaf (%d) leaf (%d)\n",
           cap.non_leaf_nodes_identical, cap.leaf_nodes_identical);
    ...
    printf("  - Shaper MAX:\n"
           ...
           cap.shaper_private_dual_rate_n_max, cap.shaper_shared_n_max,
    ...
    printf("  - mask stats (0x%" PRIx64 ")"
           ...
           cap.stats_mask, cap.dynamic_update_mask);
    ...
    printf("  - sched MAX:\n"
           ...
           "\t  -- wfq groups (%u)\n"
           "\t  -- wfq weight (%u)\n",
           cap.sched_sp_n_priorities_max, cap.sched_sp_n_priorities_max,
           cap.sched_wfq_n_children_per_group_max, cap.sched_wfq_n_groups_max,
    ...
    printf("  - CMAN support:\n"
           ...
           cap.cman_wred_byte_mode_supported, cap.cman_head_drop_supported);
    ...
    for (j = 0; j < cap.n_nodes_max; j++) {
      ...
      ret = rte_tm_node_capabilities_get(i, j, &capnode, &error);
      if (ret)
        continue;
    ...
    for (j = 0; j < cap.n_levels_max; j++) {
      ...
      ret = rte_tm_level_capabilities_get(i, j, &caplevel, &error);
      if (ret)
        continue;

      printf("  - Level %u\n", j);
      printf("\t  -- node MAX: %u non leaf %u leaf %u\n", caplevel.n_nodes_max,
             caplevel.n_nodes_nonleaf_max, caplevel.n_nodes_leaf_max);
      ...
      for (k = 0; k < caplevel.n_nodes_max; k++) {
        ret = rte_tm_node_type_get(i, k, &is_leaf, &error);
        if (ret)
          continue;
    ...
    for (j = 0; j < n_leaf_nodes; j++) {
      ...
      ret = rte_tm_node_stats_read(i, j, &stats, &cap.stats_mask, 0, &error);
      if (ret)
        continue;
      ...
      printf("  -- leaf queued:"
             ...
             stats.leaf.n_pkts_queued, stats.leaf.n_bytes_queued);
      printf("  - dropped:\n"
             "\t  -- GREEN:"
             " pkts (%" PRIu64 ") bytes (%" PRIu64 ")\n"
             "\t  -- YELLOW:"
...
static void display_crypto_feature_info(uint64_t x) {
  if (x == 0)
    return;
...
static void show_crypto(void) {
  ...
  for (i = 0; i < crypto_dev_count; i++) {
    ...
    printf("\t  -- name (%s)\n"
           ...
           dev_info.device->numa_node, rte_cryptodev_queue_pair_count(i));
    ...
    if (rte_cryptodev_stats_get(i, &stats) == 0) {
      ...
      printf("\t\t  + enqueue count (%" PRIu64 ")"
             " error (%" PRIu64 ")\n",
             stats.enqueued_count, stats.enqueue_err_count);
      printf("\t\t  + dequeue count (%" PRIu64 ")"
             " error (%" PRIu64 ")\n",
             stats.dequeued_count, stats.dequeue_err_count);
    }
...
static void show_ring(char *name) {
...
static void show_mempool(char *name) {
  ...
  if (name != NULL) {
    ...
    if (ptr != NULL) {
      ...
      printf("  - Size %u Cache %u element %u\n"
             "  - header %u trailer %u\n"
             "  - private data size %u\n",
             ptr->size, ptr->cache_size, ptr->elt_size, ptr->header_size,
             ptr->trailer_size, ptr->private_data_size);
      printf("  - memezone - socket %d\n", ptr->mz->socket_id);
      printf("  - Count: avail (%u), in use (%u)\n",
             rte_mempool_avail_count(ptr), rte_mempool_in_use_count(ptr));
...
static void mempool_itr_obj(struct rte_mempool *mp, void *opaque, void *obj,
                            unsigned int obj_idx) {
  printf("  - obj_idx %u opaque %p obj %p\n", obj_idx, opaque, obj);

  if (obj)
    rte_hexdump(stdout, " Obj Content", obj,
                (mp->elt_size > 256) ? 256 : mp->elt_size);
}

static void iter_mempool(char *name) {
  ...
  if (name != NULL) {
    ...
    if (ptr != NULL) {
      /* iterate each object */
      uint32_t ret = rte_mempool_obj_iter(ptr, mempool_itr_obj, NULL);
      printf("\n  - iterated %u objects\n", ret);
      return;
    }
  }
...
static void dump_regs(char *file_prefix) {
#define MAX_FILE_NAME_SZ (MAX_LONG_OPT_SZ + 10)
  ...
  RTE_ETH_FOREACH_DEV(i) {
    ...
    snprintf(file_name, MAX_FILE_NAME_SZ, "%s-port%u", file_prefix, i);
    ...
    if (fp_regs == NULL) {
      ...
      if (nr_written != buf_size)
        ...
        printf("Device (%s) regs dumped successfully, "
               ...
               dev_info.device->name, dev_info.driver_name, reg_info.version);
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < RTE_MAX_ETHPORTS; i++) {
    ...
    if (enable_stats)
      ...
      nic_xstats_by_ids_display(i, xstats_ids, nb_xstats_ids);
#ifdef RTE_LIB_METRICS
