...
void app_main_loop_worker_pipeline_lpm_ipv6(void) {
  ...
  RTE_LOG(INFO, USER1, "Core %u is doing work (pipeline with IPv6 LPM table)\n",
          rte_lcore_id());
  ...
  for (i = 0; i < app.n_ports; i++) {
    ...
    ip = rte_bswap32(i << (24 - __builtin_popcount(app.n_ports - 1)));
    ...
    printf("Adding rule to IPv6 LPM table (IPv6 destination = "
           ...
           "port out = %u)\n",
           key.ip[0], key.ip[1], key.ip[2], key.ip[3], key.ip[4], key.ip[5],
           key.ip[6], key.ip[7], key.ip[8], key.ip[9], key.ip[10], key.ip[11],
           key.ip[12], key.ip[13], key.ip[14], key.ip[15], key.depth, i);
