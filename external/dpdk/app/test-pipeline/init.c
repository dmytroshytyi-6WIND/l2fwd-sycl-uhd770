...
static void app_init_rings(void) {
  ...
  for (i = 0; i < app.n_ports; i++) {
    ...
    app.rings_tx[i] = rte_ring_create(name, app.ring_tx_size, rte_socket_id(),
                                      RING_F_SP_ENQ | RING_F_SC_DEQ);
...
static void app_ports_check_link(void) {
  ...
  for (i = 0; i < app.n_ports; i++) {
    ...
    if (ret < 0) {
      RTE_LOG(INFO, USER1, "Failed to get port %u link status: %s\n", port,
              rte_strerror(-ret));
      all_ports_up = 0;
      continue;
    }
    rte_eth_link_to_str(link_status_text, sizeof(link_status_text), &link);
    RTE_LOG(INFO, USER1, "Port %u %s\n", port, link_status_text);
...
static void app_init_ports(void) {
  ...
  for (i = 0; i < app.n_ports; i++) {
    ...
    /* Init RX queues */
    ret =
        rte_eth_rx_queue_setup(port, 0, app.port_rx_ring_size,
