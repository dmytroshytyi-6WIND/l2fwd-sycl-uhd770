...
void app_main_loop_worker_pipeline_lpm(void) {
  ...
  RTE_LOG(INFO, USER1,
          "Core %u is doing work (pipeline with "
          "LPM table)\n",
          rte_lcore_id());
  ...
  for (i = 0; i < app.n_ports; i++) {
    ...
    printf("Adding rule to LPM table (IPv4 destination = %" PRIu32 ".%" PRIu32
           ".%" PRIu32 ".%" PRIu32 "/%" PRIu8 " => port out = %" PRIu32 ")\n",
           (key.ip & 0xFF000000) >> 24, (key.ip & 0x00FF0000) >> 16,
           (key.ip & 0x0000FF00) >> 8, key.ip & 0x000000FF, key.depth, i);
