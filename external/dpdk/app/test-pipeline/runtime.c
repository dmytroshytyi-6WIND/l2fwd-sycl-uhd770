...
void app_main_loop_worker(void) {
  ...
  for (i = 0;; i = ((i + 1) & (app.n_ports - 1))) {
    ...
    ret = rte_ring_sc_dequeue_bulk(app.rings_rx[i], (void **)worker_mbuf->array,
                                   app.burst_size_worker_read, NULL);

    if (ret == 0)
      continue;

    do {
      ret = rte_ring_sp_enqueue_bulk(app.rings_tx[i ^ 1],
                                     (void **)worker_mbuf->array,
                                     app.burst_size_worker_write, NULL);
    } while (ret == 0);
  }
}
...
void app_main_loop_tx(void) {
  ...
  for (i = 0;; i = ((i + 1) & (app.n_ports - 1))) {
    ...
    ret = rte_ring_sc_dequeue_bulk(app.rings_tx[i],
    ...
    n_pkts = rte_eth_tx_burst(app.ports[i], 0, app.mbuf_tx[i].array, n_mbufs);
