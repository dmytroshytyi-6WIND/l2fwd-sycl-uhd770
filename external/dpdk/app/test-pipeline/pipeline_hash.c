...
static void translate_options(uint32_t *special, uint32_t *ext,
                              uint32_t *key_size) {
...
void app_main_loop_worker_pipeline_hash(void) {
  ...
  RTE_LOG(INFO, USER1,
          "Core %u is doing work "
          "(pipeline with hash table, %s, %s, %d-byte key)\n",
          rte_lcore_id(), special ? "specialized" : "non-specialized",
          ext ? "extendible bucket" : "LRU", key_size);
  ...
  for (i = 0; i < (1 << 24); i++) {
    ...
    status = rte_pipeline_table_entry_add(p, table_id, key, &entry, &key_found,
                                          &entry_ptr);
    if (status < 0)
...
uint64_t test_hash(void *key, __rte_unused void *key_mask,
                   __rte_unused uint32_t key_size, __rte_unused uint64_t seed) {
  uint32_t *k32 = key;
...
uint32_t test_hash_cuckoo(const void *key, __rte_unused uint32_t key_size,
...
void app_main_loop_rx_metadata(void) {
  ...
  for (i = 0;; i = ((i + 1) & (app.n_ports - 1))) {
    ...
    n_mbufs = rte_eth_rx_burst(app.ports[i], 0, app.mbuf_rx.array,
    ...
    for (j = 0; j < n_mbufs; j++) {
      ...
      signature = RTE_MBUF_METADATA_UINT32_PTR(m, APP_METADATA_OFFSET(0));
      key = RTE_MBUF_METADATA_UINT8_PTR(m, APP_METADATA_OFFSET(32));

      if (RTE_ETH_IS_IPV4_HDR(m->packet_type)) {
        ip_hdr = (struct rte_ipv4_hdr *)&m_data[sizeof(struct rte_ether_hdr)];
        ip_dst = ip_hdr->dst_addr;

        k32 = (uint32_t *)key;
        k32[0] = ip_dst & 0xFFFFFF00;
      } else if (RTE_ETH_IS_IPV6_HDR(m->packet_type)) {
        ipv6_hdr = (struct rte_ipv6_hdr *)&m_data[sizeof(struct rte_ether_hdr)];
    ...
    do {
      ret = rte_ring_sp_enqueue_bulk(app.rings_rx[i],
                                     (void **)app.mbuf_rx.array, n_mbufs, NULL);
    } while (ret == 0);
  }
}
