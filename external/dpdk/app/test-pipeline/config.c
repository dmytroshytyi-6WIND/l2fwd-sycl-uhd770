...
void app_print_usage(void) { printf(usage); }
...
int app_parse_args(int argc, char **argv) {
  ...
  int option_index;
  char *prgname = argv[0];
  static struct option lgopts[] = {{"none", 0, 0, 0},
                                   {"stub", 0, 0, 0},
                                   {"hash-8-ext", 0, 0, 0},
                                   {"hash-8-lru", 0, 0, 0},
                                   {"hash-16-ext", 0, 0, 0},
    ...
    case 0: /* long options */
      if (!pipeline_type_provided) {
        ...
        for (i = 0; i < e_APP_PIPELINES; i++) {
          if (!strcmp(lgopts[option_index].name, app_args_table[i].name)) {
            app.pipeline_type = app_args_table[i].value;
