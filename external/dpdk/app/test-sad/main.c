...
static int parse_distrib(const char *in) {
  int a, b, c;
...
static void print_config(void) {
  fprintf(stdout,
          ...
          config.nb_rules, config.fract_32, config.fract_64, config.fract_96,
          config.nb_rules_32, config.nb_rules_64, config.nb_rules_96,
          config.nb_tuples, config.burst_sz, config.fract_rnd_tuples,
...
static int get_str_num(FILE *f, int num) {
  int n_lines = 0;
...
static int parse_file(FILE *f, struct rule *tbl, int rule_tbl) {
  int ret, i, j = 0;
  ...
  while (fgets(line, sizeof(line), f) != NULL) {
    ...
    GET_CB_FIELD(in[CB_RULE_SPI], tbl[j].tuple.v4.spi, 0, UINT32_MAX, 0);

    if (config.ipv6)
      ret = inet_pton(AF_INET6, in[CB_RULE_DIP], &tbl[j].tuple.v6.dip);
    else
      ret = inet_pton(AF_INET, in[CB_RULE_DIP], &tbl[j].tuple.v4.dip);
    if (ret != 1)
      return -EINVAL;
    if (config.ipv6)
      ret = inet_pton(AF_INET6, in[CB_RULE_SIP], &tbl[j].tuple.v6.sip);
    else
      ret = inet_pton(AF_INET, in[CB_RULE_SIP], &tbl[j].tuple.v4.sip);
    if (ret != 1)
      return -EINVAL;
...
static void get_random_rules(struct rule *tbl, uint32_t nb_rules,
                             int rule_tbl) {
  ...
  for (i = 0; i < nb_rules; i++, edge += step) {
    ...
    if (rule_tbl) {
      tbl[i].tuple.v4.spi =
          get_rnd_rng((uint64_t)edge, (uint64_t)(edge + step));
      if (config.ipv6) {
        for (j = 0; j < 16; j++) {
      ...
      if (rnd >= (100UL - config.fract_32)) {
        ...
        config.nb_rules_32++;
      } else if (rnd >= (100UL - (config.fract_32 + config.fract_64))) {
      ...
      if (rnd >= 100UL - config.fract_rnd_tuples) {
        ...
        tbl[i].tuple.v4.spi = rules_tbl[i % config.nb_rules].tuple.v4.spi;
        if (config.ipv6) {
          int r_idx = i % config.nb_rules;
          memcpy(tbl[i].tuple.v6.dip, rules_tbl[r_idx].tuple.v6.dip,
                 sizeof(tbl[i].tuple.v6.dip));
          memcpy(tbl[i].tuple.v6.sip, rules_tbl[r_idx].tuple.v6.sip,
                 sizeof(tbl[i].tuple.v6.sip));
        } else {
          tbl[i].tuple.v4.dip = rules_tbl[i % config.nb_rules].tuple.v4.dip;
          tbl[i].tuple.v4.sip = rules_tbl[i % config.nb_rules].tuple.v4.sip;
        }
      }
    }
  }
...
static void tbl_init(struct rule **tbl, uint32_t *n_entries,
  ...
  *n_entries = get_str_num(f, *n_entries);
  printf("%d entries\n", *n_entries);
  *tbl =
      rte_zmalloc(NULL, sizeof(struct rule) * *n_entries, RTE_CACHE_LINE_SIZE);
  if (*tbl == NULL)
...
static void parse_opts(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "f:t:n:d:l:r:6b:vpc")) != -1) {
    ...
    case 'n':
      ...
      if ((errno != 0) || (config.nb_rules == 0) || (endptr[0] != 0)) {
        print_usage();
    ...
    case 'b':
      ...
      if ((errno != 0) || (config.burst_sz == 0) ||
          (config.burst_sz > BURST_SZ_MAX) || (endptr[0] != 0)) {
        print_usage();
    ...
    case 'l':
      ...
      if ((errno != 0) || (config.nb_tuples == 0) || (endptr[0] != 0)) {
        print_usage();
    ...
    case 'r':
      ...
      if ((errno != 0) || (config.fract_rnd_tuples == 0) ||
          (config.fract_rnd_tuples >= 100) || (endptr[0] != 0)) {
        print_usage();
...
static void print_addr(int af, const void *addr) {
...
static void print_tuple(int af, uint32_t spi, const void *dip,
                        const void *sip) {
...
static void print_result(const union rte_ipsec_sad_key *key, void *res) {
...
static int lookup(void *arg) {
  ...
  for (i = 0; i < config.nb_tuples; i += burst_sz) {
    for (j = 0; j < burst_sz; j++)
      keys[j] = (union rte_ipsec_sad_key *)(&tuples_tbl[i + j].tuple);
...
static void add_rules(struct rte_ipsec_sad *sad, uint32_t fract) {
  int32_t ret;
  uint32_t i, j, f, fn, n;
  ...
  for (n = 0, j = 0; n != config.nb_rules; n = fn, j++) {
    ...
    start = rte_rdtsc_precise();
    for (i = n; i != fn; i++) {
      ret = rte_ipsec_sad_add(sad, &rules_tbl[i].tuple, rules_tbl[i].rule_type,
                              &rules_tbl[i]);
      if (ret != 0)
  ...
  for (i = 0; i != j; i++)
    printf("ADD %u rules, %.2Lf cycles/rule, %.2Lf ADD/sec\n", nm[i],
           (long double)tm[i] / nm[i],
           (long double)nm[i] * rte_get_tsc_hz() / tm[i]);
}

static void del_rules(struct rte_ipsec_sad *sad, uint32_t fract) {
  int32_t ret;
  uint32_t i, j, f, fn, n;
  uint64_t start, tm[fract + 1];
  uint32_t nm[fract + 1];
  ...
  for (n = 0, j = 0; n != config.nb_rules; n = fn, j++) {
    ...
    start = rte_rdtsc_precise();
    for (i = n; i != fn; i++) {
      ret = rte_ipsec_sad_del(sad, &rules_tbl[i].tuple, rules_tbl[i].rule_type);
      if (ret != 0 && ret != -ENOENT)
        rte_exit(ret, "%s failed @ %u-th rule\n", __func__, i);
    }
    tm[j] = rte_rdtsc_precise() - start;
