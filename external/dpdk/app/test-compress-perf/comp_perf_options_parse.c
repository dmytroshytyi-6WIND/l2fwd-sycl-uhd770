...
static void usage(char *progname) {
  printf("%s [EAL options] --\n"
         ...
         " --cc-delay-us N: delay between enqueue and dequeue operations in "
         "microseconds\n"
...
static int parse_cperf_test_type(struct comp_test_data *test_data,
                                 const char *arg) {
  struct name_id_map cperftest_namemap[] = {
      ...
      {comp_perf_test_type_strs[CPERF_TEST_TYPE_PMDCC], CPERF_TEST_TYPE_PMDCC}};
...
static int parse_range(const char *arg, uint8_t *min, uint8_t *max,
                       uint8_t *inc) {
  char *token;
  uint8_t number;
  ...
  if (token != NULL) {
    ...
    if (errno == EINVAL || errno == ERANGE || number < *min)
      goto err_range;
...
static int parse_list(const char *arg, uint8_t *list, uint8_t *min,
                      uint8_t *max) {
  char *token;
  uint32_t number;
  uint8_t count = 0;
...
static int parse_num_iter(struct comp_test_data *test_data, const char *arg) {
  ...
  if (test_data->num_iter == 0) {
    RTE_LOG(ERR, USER1, "Total number of iterations must be higher than 0\n");
    return -1;
  }
...
static int parse_pool_sz(struct comp_test_data *test_data, const char *arg) {
...
static int parse_burst_sz(struct comp_test_data *test_data, const char *arg) {
...
static int parse_extended_input_sz(struct comp_test_data *test_data,
                                   const char *arg) {
  uint32_t tmp;
  ...
  if (tmp == 0) {
    RTE_LOG(ERR, USER1, "Extended file size must be higher than 0\n");
    return -1;
  }
  return 0;
}

static int parse_seg_sz(struct comp_test_data *test_data, const char *arg) {
...
static int parse_max_num_sgl_segs(struct comp_test_data *test_data,
                                  const char *arg) {
  ...
  if (test_data->max_sgl_segs == 0) {
    RTE_LOG(ERR, USER1,
            "Max number of segments per mbuf chain "
...
static int parse_window_sz(struct comp_test_data *test_data, const char *arg) {
  uint16_t tmp;
...
static int parse_driver_name(struct comp_test_data *test_data,
                             const char *arg) {
  if (strlen(arg) > (sizeof(test_data->driver_name) - 1))
    return -1;

  strlcpy(test_data->driver_name, arg, sizeof(test_data->driver_name));

  return 0;
}

static int parse_test_file(struct comp_test_data *test_data, const char *arg) {
...
static int parse_op_type(struct comp_test_data *test_data, const char *arg) {
  struct name_id_map optype_namemap[] = {
      {"comp", COMPRESS_ONLY},
      {"decomp", DECOMPRESS_ONLY},
      {"comp_and_decomp", COMPRESS_DECOMPRESS}};
...
static int parse_huffman_enc(struct comp_test_data *test_data,
                             const char *arg) {
  struct name_id_map huffman_namemap[] = {
      {"default", RTE_COMP_HUFFMAN_DEFAULT},
      {"fixed", RTE_COMP_HUFFMAN_FIXED},
      {"dynamic", RTE_COMP_HUFFMAN_DYNAMIC}};

  int id =
      get_str_key_id_mapping(huffman_namemap, RTE_DIM(huffman_namemap), arg);
  if (id < 0) {
...
static int parse_level(struct comp_test_data *test_data, const char *arg) {
  int ret;
   ...
   * arse it as a list
   */
  if (parse_range(arg, &test_data->level_lst.min, &test_data->level_lst.max,
                  &test_data->level_lst.inc) < 0) {
    ret = parse_list(arg, test_data->level_lst.list, &test_data->level_lst.min,
                     &test_data->level_lst.max);
    if (ret < 0) {
      RTE_LOG(ERR, USER1, "Failed to parse compression level/s\n");
      return -1;
    }
...
static int parse_external_mbufs(struct comp_test_data *test_data,
                                const char *arg __rte_unused) {
  test_data->use_external_mbufs = 1;
  return 0;
}

static int parse_cyclecount_delay_us(struct comp_test_data *test_data,
                                     const char *arg) {
...
static int comp_perf_opts_parse_long(int opt_idx,
...
int comp_perf_options_parse(struct comp_test_data *test_data, int argc,
                            char **argv) {
...
void comp_perf_options_default(struct comp_test_data *test_data) {
...
int comp_perf_options_check(struct comp_test_data *test_data) {
