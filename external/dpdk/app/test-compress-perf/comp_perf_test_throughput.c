...
void cperf_throughput_test_destructor(void *arg) {
  if (arg) {
    comp_perf_free_memory(((struct cperf_benchmark_ctx *)arg)->ver.options,
                          &((struct cperf_benchmark_ctx *)arg)->ver.mem);
    rte_free(arg);
  }
}

void *cperf_throughput_test_constructor(uint8_t dev_id, uint16_t qp_id,
...
static int main_loop(struct cperf_benchmark_ctx *ctx,
  ...
  for (iter = 0; iter < num_iter; iter++) {
    ...
    while (remaining_ops > 0) {
      ...
      if (ops_needed &&
          !rte_comp_op_bulk_alloc(mem->op_pool, &ops[ops_unused], ops_needed)) {
  ...
  if (type == RTE_COMP_COMPRESS)
    ctx->comp_tsc_duration[test_data->level] = tsc_duration / num_iter;
  else
    ctx->decomp_tsc_duration[test_data->level] = tsc_duration / num_iter;
...
int cperf_throughput_test_runner(void *test_ctx) {
  ...
  ctx->comp_tsc_byte = (double)(ctx->comp_tsc_duration[test_data->level]) /
                       test_data->input_data_sz;

  ctx->decomp_tsc_byte = (double)(ctx->decomp_tsc_duration[test_data->level]) /
                         test_data->input_data_sz;

  ctx->comp_gbps = rte_get_tsc_hz() / ctx->comp_tsc_byte * 8 / 1000000000;

  ctx->decomp_gbps = rte_get_tsc_hz() / ctx->decomp_tsc_byte * 8 / 1000000000;

  exp = 0;
  if (__atomic_compare_exchange_n(&display_once, &exp, 1, 0, __ATOMIC_RELAXED,
                                  __ATOMIC_RELAXED)) {
    printf("\n%12s%6s%12s%17s%15s%16s\n", "lcore id", "Level", "Comp size",
           "Comp ratio [%]", "Comp [Gbps]", "Decomp [Gbps]");
  }

  printf("%12u%6u%12zu%17.2f%15.2f%16.2f\n", ctx->ver.mem.lcore_id,
         test_data->level, ctx->ver.comp_data_sz, ctx->ver.ratio,
         ctx->comp_gbps, ctx->decomp_gbps);
