...
__extension__ static const struct cperf_test cperf_testmap[] = {
    ...
    [CPERF_TEST_TYPE_PMDCC] = {cperf_cyclecount_test_constructor,
                               cperf_cyclecount_test_runner,
                               cperf_cyclecount_test_destructor}};

static struct comp_test_data *test_data;

static int comp_perf_check_capabilities(struct comp_test_data *test_data,
                                        uint8_t cdev_id) {
  const struct rte_compressdev_capabilities *cap;

  cap = rte_compressdev_capability_get(cdev_id, RTE_COMP_ALGO_DEFLATE);
  ...
  if (test_data->huffman_enc == RTE_COMP_HUFFMAN_FIXED &&
    ...
    RTE_LOG(ERR, USER1, "Compress device does not supported Fixed Huffman\n");
    return -1;
  }

  if (test_data->huffman_enc == RTE_COMP_HUFFMAN_DYNAMIC &&
      (comp_flags & RTE_COMP_FF_HUFFMAN_DYNAMIC) == 0) {
    RTE_LOG(ERR, USER1, "Compress device does not supported Dynamic Huffman\n");
    return -1;
  }

  /* Window size */
  if (test_data->window_sz != -1) {
    if (param_range_check(test_data->window_sz, &cap->window_size) < 0) {
      RTE_LOG(ERR, USER1,
...
static int comp_perf_initialize_compressdev(struct comp_test_data *test_data,
                                            uint8_t *enabled_cdevs) {
  uint8_t enabled_cdev_count, nb_lcores, cdev_id;
  unsigned int i, j;
  int ret;

  enabled_cdev_count = rte_compressdev_devices_get(
      test_data->driver_name, enabled_cdevs, RTE_COMPRESS_MAX_DEVS);
  ...
  if (enabled_cdev_count > nb_lcores) {
    if (nb_lcores == 0) {
      RTE_LOG(ERR, USER1,
              "Cannot run with 0 cores! Increase the number of cores\n");
      return -EINVAL;
    }
  ...
  for (i = 0; i < enabled_cdev_count && i < RTE_COMPRESS_MAX_DEVS;
       i++, nb_lcores -= test_data->nb_qps) {
    ...
    struct rte_compressdev_config config = {
        .socket_id = socket_id,
        .nb_queue_pairs =
            nb_lcores > test_data->nb_qps ? test_data->nb_qps : nb_lcores,
    ...
    for (j = 0; j < test_data->nb_qps; j++) {
      ret = rte_compressdev_queue_pair_setup(cdev_id, j, NUM_MAX_INFLIGHT_OPS,
                                             socket_id);
      if (ret < 0) {
        RTE_LOG(ERR, USER1, "Failed to setup queue pair %u on compressdev %u",
                j, cdev_id);
        return -EINVAL;
      }
    }
...
static int comp_perf_dump_input_data(struct comp_test_data *test_data) {
  ...
  if (test_data->input_data_sz > actual_file_sz)
    ...
    RTE_LOG(INFO, USER1, "%zu bytes read from file %s\n",
            test_data->input_data_sz, test_data->input_file);
...
static void comp_perf_cleanup_on_signal(int signalNumber __rte_unused) {
  test_data->perf_comp_force_stop = 1;
}

static void comp_perf_register_cleanup_on_signal(void) {
...
int main(int argc, char **argv) {
  ...
  test_data = rte_zmalloc_socket(NULL, sizeof(struct comp_test_data), 0,
                                 rte_socket_id());
  ...
  nb_compressdevs = comp_perf_initialize_compressdev(test_data, enabled_cdevs);
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    ctx[i] =
        cperf_testmap[test_data->test].constructor(cdev_id, qp_id, test_data);
    if (ctx[i] == NULL) {
  ...
  while (test_data->level <= test_data->level_lst.max) {
    ...
    RTE_LCORE_FOREACH_WORKER(lcore_id) {
      ...
      rte_eal_remote_launch(cperf_testmap[test_data->test].runner, ctx[i],
                            lcore_id);
      i++;
    }
    i = 0;
  ...
  case ST_DURING_TEST:
    ...
    RTE_LCORE_FOREACH_WORKER(lcore_id) {
      ...
      if (ctx[i] && cperf_testmap[test_data->test].destructor)
        cperf_testmap[test_data->test].destructor(ctx[i]);
      i++;
    }
    /* fallthrough */
  case ST_INPUT_DATA:
    ...
    /* fallthrough */
  case ST_COMPDEV:
    for (i = 0; i < nb_compressdevs && i < RTE_COMPRESS_MAX_DEVS; i++) {
  ...
  default:
    i = rte_eal_cleanup();
    if (i) {
      RTE_LOG(ERR, USER1, "Error from rte_eal_cleanup(), %d\n", i);
      ret = i;
    }
    break;
  }
...
__rte_weak void cperf_cyclecount_test_destructor(void *arg __rte_unused) {
  RTE_LOG(INFO, USER1, "Something wrong happened!!!\n");
}

__rte_weak int cperf_cyclecount_test_runner(void *test_ctx __rte_unused) {
  return 0;
}
...
__rte_weak void cperf_throughput_test_destructor(void *arg __rte_unused) {}

__rte_weak int cperf_throughput_test_runner(void *test_ctx __rte_unused) {
  return 0;
}
__rte_weak void *
...
__rte_weak void cperf_verify_test_destructor(void *arg __rte_unused) {}

__rte_weak int cperf_verify_test_runner(void *test_ctx __rte_unused) {
  return 0;
}
