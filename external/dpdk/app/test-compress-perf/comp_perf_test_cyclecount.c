...
void cperf_cyclecount_test_destructor(void *arg) {
...
void *cperf_cyclecount_test_constructor(uint8_t dev_id, uint16_t qp_id,
...
static int cperf_cyclecount_op_setup(struct rte_comp_op **ops,
                                     struct cperf_cyclecount_ctx *ctx,
                                     struct rte_mbuf **input_bufs,
                                     struct rte_mbuf **output_bufs,
                                     void *priv_xform, uint32_t out_seg_sz) {
  ...
  for (iter = 0; iter < num_iter; iter++) {
    ...
    while (remaining_ops > 0) {
      uint16_t num_ops = RTE_MIN(remaining_ops, test_data->burst_sz);
      ops_needed = num_ops;

      /* Allocate compression operations */
      if (ops_needed &&
          rte_mempool_get_bulk(mem->op_pool, (void **)ops, ops_needed) != 0) {
        RTE_LOG(ERR, USER1,
      ...
      for (i = 0; i < ops_needed; i++) {
        ...
        ops[op_id]->src.length = rte_pktmbuf_pkt_len(input_bufs[buf_id]);
      ...
      rte_mempool_put_bulk(mem->op_pool, (void **)ops, num_deq);
    }
  }
  return res;
end:
...
static int main_loop(struct cperf_cyclecount_ctx *ctx,
  ...
  if (ops == NULL) {
    RTE_LOG(ERR, USER1, "Can't allocate memory for ops structures\n");
    return -1;
  }
  ...
  if (type == RTE_COMP_COMPRESS) {
    xform = (struct rte_comp_xform){
        ...
        .compress = {.algo = RTE_COMP_ALGO_DEFLATE,
    ...
    xform = (struct rte_comp_xform){
        ...
        .decompress = {.algo = RTE_COMP_ALGO_DEFLATE,
                       .chksum = RTE_COMP_CHECKSUM_NONE,
                       .window_size = test_data->window_sz,
                       .hash_algo = RTE_COMP_HASH_ALGO_NONE}};
  ...
  if (rte_compressdev_private_xform_create(dev_id, &xform, &priv_xform) < 0) {
  ...
  ret = cperf_cyclecount_op_setup(ops, ctx, input_bufs, output_bufs, priv_xform,
                                  out_seg_sz);
  ...
  for (iter = 0; iter < num_iter; iter++) {
    ...
    while (remaining_ops > 0) {
      ...
      if ((ops_unused > 0) && (num_enq > 0)) {
        size_t nb_b_to_mov = ops_unused * sizeof(struct rte_comp_op *);
      ...
      /* Allocate compression operations */
      if (ops_needed &&
          rte_mempool_get_bulk(mem->op_pool, (void **)&ops[ops_unused],
                               ops_needed) != 0) {
        RTE_LOG(ERR, USER1, "Could not allocate enough operations\n");
        res = -1;
        goto end;
      }
      ...
      for (i = 0; i < ops_needed; i++) {
         ...
         */
        uint32_t buf_id = total_enq_ops + i + ops_unused;
      ...
      tsc_start = rte_rdtsc_precise();
      num_enq = rte_compressdev_enqueue_burst(dev_id, mem->qp_id, ops, num_ops);
      tsc_end = rte_rdtsc_precise();
      tsc_duration = tsc_end - tsc_start;
      ...
      tsc_start = rte_rdtsc_precise();
      num_deq =
          rte_compressdev_dequeue_burst(dev_id, mem->qp_id, deq_ops, allocated);
      tsc_end = rte_rdtsc_precise();
      tsc_duration = tsc_end - tsc_start;
      ...
      rte_mempool_put_bulk(mem->op_pool, (void **)deq_ops, num_deq);
      allocated -= num_deq;
    }
    ...
    while (total_deq_ops < total_ops) {
      ...
      tsc_start = rte_rdtsc_precise();
      num_deq = rte_compressdev_dequeue_burst(dev_id, mem->qp_id, deq_ops,
                                              test_data->burst_sz);
      tsc_end = rte_rdtsc_precise();
      tsc_duration = tsc_end - tsc_start;
      ctx->duration_deq += tsc_duration;
...
int cperf_cyclecount_test_runner(void *test_ctx) {
   ...
   */
  if (__atomic_compare_exchange_n(&ctx->ver.mem.print_info_once, &exp, 1, 0,
    ...
    printf("    lcore: %u,"
           ...
           rte_compressdev_name_get(ctx->ver.mem.dev_id), ctx->ver.mem.dev_id,
           rte_compressdev_socket_id(ctx->ver.mem.dev_id), ctx->ver.mem.qp_id);
  ...
  duration_enq_per_op_comp =
      ctx->duration_enq / (ctx->ver.mem.total_bufs * test_data->num_iter);
  duration_deq_per_op_comp =
      ctx->duration_deq / (ctx->ver.mem.total_bufs * test_data->num_iter);
  ...
  duration_setup_per_op =
      ctx->duration_op / (ctx->ver.mem.total_bufs * test_data->num_iter);
  ...
  if (display_once == 0) {
    ...
    printf(
        ...
        "  - Cycles section: number of cycles per 'op' for the following "
        "operations:\n"
        "    setup/op - memory allocation, op configuration and memory "
        "dealocation\n"
        "    [C-e] - compression enqueue\n"
        "    [C-d] - compression dequeue\n"
        "    [D-e] - decompression enqueue\n"
    ...
    printf("\n%12s%6s%12s%17s", "lcore id", "Level", "Comp size",
           "Comp ratio [%]");

    printf("  |%10s %6s %8s %6s %8s", " Retries:", "[C-e]", "[C-d]", "[D-e]",
           "[D-d]");
  ...
  printf("%12u"
         ...
         "%12zu"
         "%17.2f",
         ctx->ver.mem.lcore_id, test_data->level, ctx->ver.comp_data_sz,
         ctx->ver.ratio);

  printf("  |%10s %6u %8u %6u %8u", " ", ops_enq_retries_comp,
         ops_deq_retries_comp, ops_enq_retries_decomp, ops_deq_retries_decomp);

  printf("  |%9s %9u %9u %9u %9u %9u\n", " ", duration_setup_per_op,
         duration_enq_per_op_comp, duration_deq_per_op_comp,
         duration_enq_per_op_decomp, duration_deq_per_op_decomp);
