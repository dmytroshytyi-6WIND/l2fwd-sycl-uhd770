...
void cperf_verify_test_destructor(void *arg) {
  if (arg) {
    comp_perf_free_memory(((struct cperf_verify_ctx *)arg)->options,
                          &((struct cperf_verify_ctx *)arg)->mem);
    rte_free(arg);
  }
}

void *cperf_verify_test_constructor(uint8_t dev_id, uint16_t qp_id,
...
static int main_loop(struct cperf_verify_ctx *ctx,
  ...
  for (iter = 0; iter < num_iter; iter++) {
    ...
    while (remaining_ops > 0) {
      ...
      for (i = 0; i < num_deq; i++) {
        ...
        if (op->status == RTE_COMP_OP_STATUS_OUT_OF_SPACE_TERMINATED ||
          ...
          RTE_LOG(ERR, USER1,
                  "Out of space error occurred due to uncompressible input "
                  "data expanding to larger than destination buffer. Increase "
                  "the EXPANSE_RATIO constant to use this data.\n");
          res = -1;
          goto end;
        } else if (op->status != RTE_COMP_OP_STATUS_SUCCESS) {
          RTE_LOG(ERR, USER1, "Some operations were not successful\n");
          goto end;
        }

        const void *read_data_addr =
            rte_pktmbuf_read(op->m_dst, 0, op->produced, output_data_ptr);
        if (read_data_addr == NULL) {
          RTE_LOG(ERR, USER1, "Could not copy buffer in destination\n");
          res = -1;
          goto end;
        }

        if (read_data_addr != output_data_ptr)
          rte_memcpy(output_data_ptr, rte_pktmbuf_mtod(op->m_dst, uint8_t *),
                     op->produced);
    ...
    while (total_deq_ops < total_ops) {
      ...
      for (i = 0; i < num_deq; i++) {
        ...
        const void *read_data_addr = rte_pktmbuf_read(
            op->m_dst, op->dst.offset, op->produced, output_data_ptr);
        if (read_data_addr == NULL) {
          RTE_LOG(ERR, USER1, "Could not copy buffer in destination\n");
          res = -1;
          goto end;
...
int cperf_verify_test_runner(void *test_ctx) {
  ...
  if (ctx->decomp_data_sz != test_data->input_data_sz) {
    ...
    if (memcmp(ctx->mem.decompressed_data, test_data->input_data,
  ...
  ctx->ratio = (double)ctx->comp_data_sz / test_data->input_data_sz * 100;
  ...
  if (!ctx->silent) {
    ...
    printf("%12u%6u%12zu%17.2f\n", ctx->mem.lcore_id, test_data->level,
           ctx->comp_data_sz, ctx->ratio);
  }
