...
int param_range_check(uint16_t size, const struct rte_param_log2_range *range) {
...
static uint32_t find_buf_size(uint32_t input_size) {
  uint32_t i;
   ...
   * than input_size
   */
  uint32_t size = !input_size ? MIN_COMPRESSED_BUF_SIZE : (input_size << 1);
...
void comp_perf_free_memory(struct comp_test_data *test_data,
...
static void comp_perf_extbuf_free_cb(void *addr __rte_unused,
...
comp_perf_make_memzone(const char *name, struct cperf_mem_resources *mem,
  ...
  snprintf(mz_name, RTE_MEMZONE_NAMESIZE, "%s_s%u_d%u_q%u_%d", name, socket_id,
           mem->dev_id, mem->qp_id, number);
...
static int comp_perf_allocate_external_mbufs(struct comp_test_data *test_data,
                                             struct cperf_mem_resources *mem) {
  uint32_t i;

  mem->comp_memzones = rte_zmalloc_socket(
      NULL, test_data->total_segs * sizeof(struct rte_memzone *), 0,
      rte_socket_id());
  ...
  mem->decomp_memzones = rte_zmalloc_socket(
      NULL, test_data->total_segs * sizeof(struct rte_memzone *), 0,
      rte_socket_id());
  ...
  mem->comp_buf_infos = rte_zmalloc_socket(
      NULL, test_data->total_segs * sizeof(struct rte_mbuf_ext_shared_info), 0,
      rte_socket_id());
  ...
  mem->decomp_buf_infos = rte_zmalloc_socket(
      NULL, test_data->total_segs * sizeof(struct rte_mbuf_ext_shared_info), 0,
      rte_socket_id());
  ...
  for (i = 0; i < test_data->total_segs; i++) {
    mem->comp_memzones[i] =
        comp_perf_make_memzone("comp", mem, i, test_data->out_seg_sz);
    ...
    mem->decomp_memzones[i] =
        comp_perf_make_memzone("decomp", mem, i, test_data->seg_sz);
    ...
    mem->comp_buf_infos[i].free_cb = comp_perf_extbuf_free_cb;
    mem->comp_buf_infos[i].fcb_opaque = NULL;
    rte_mbuf_ext_refcnt_set(&mem->comp_buf_infos[i], 1);

    mem->decomp_buf_infos[i].free_cb = comp_perf_extbuf_free_cb;
...
int comp_perf_allocate_memory(struct comp_test_data *test_data,
                              struct cperf_mem_resources *mem) {
   ...
   */
  test_data->total_segs = DIV_CEIL(test_data->input_data_sz, test_data->seg_sz);
  ...
  snprintf(pool_name, sizeof(pool_name), "comp_buf_pool_%u_qp_%u", mem->dev_id,
           mem->qp_id);
  mem->comp_buf_pool = rte_pktmbuf_pool_create(
      pool_name, test_data->total_segs, 0, 0, comp_mbuf_size, rte_socket_id());
  ...
  mem->decomp_buf_pool =
      rte_pktmbuf_pool_create(pool_name, test_data->total_segs, 0, 0,
  ...
  mem->total_bufs = DIV_CEIL(test_data->total_segs, test_data->max_sgl_segs);

  snprintf(pool_name, sizeof(pool_name), "op_pool_%u_qp_%u", mem->dev_id,
           mem->qp_id);

  /* one mempool for both src and dst mbufs */
  mem->op_pool = rte_comp_op_pool_create(pool_name, mem->total_bufs * 2, 0, 0,
                                         rte_socket_id());
  if (mem->op_pool == NULL) {
   ...
   */
  mem->compressed_data = rte_zmalloc_socket(
      NULL,
      RTE_MAX((size_t)test_data->out_seg_sz * test_data->total_segs,
  ...
  mem->decompressed_data =
      rte_zmalloc_socket(NULL, test_data->input_data_sz, 0, rte_socket_id());
  ...
  mem->comp_bufs = rte_zmalloc_socket(
      NULL, mem->total_bufs * sizeof(struct rte_mbuf *), 0, rte_socket_id());
  if (mem->comp_bufs == NULL) {
    RTE_LOG(ERR, USER1,
  ...
  mem->decomp_bufs = rte_zmalloc_socket(
      NULL, mem->total_bufs * sizeof(struct rte_mbuf *), 0, rte_socket_id());
  if (mem->decomp_bufs == NULL) {
    RTE_LOG(ERR, USER1,
...
int prepare_bufs(struct comp_test_data *test_data,
                 struct cperf_mem_resources *mem) {
  ...
  for (i = 0; i < mem->total_bufs; i++) {
    ...
    mem->decomp_bufs[i] = rte_pktmbuf_alloc(mem->decomp_buf_pool);
    ...
    if (test_data->use_external_mbufs != 0) {
      rte_pktmbuf_attach_extbuf(mem->decomp_bufs[i],
                                ...
                                test_data->seg_sz, &mem->decomp_buf_infos[dmz]);
      dmz++;
    }

    data_addr = (uint8_t *)rte_pktmbuf_append(mem->decomp_bufs[i], data_sz);
    if (data_addr == NULL) {
    ...
    while (segs_per_mbuf < test_data->max_sgl_segs && remaining_data > 0) {
      struct rte_mbuf *next_seg = rte_pktmbuf_alloc(mem->decomp_buf_pool);
      ...
      if (test_data->use_external_mbufs != 0) {
        rte_pktmbuf_attach_extbuf(next_seg, mem->decomp_memzones[dmz]->addr,
                                  mem->decomp_memzones[dmz]->iova,
                                  test_data->seg_sz,
      ...
      data_addr = (uint8_t *)rte_pktmbuf_append(next_seg, data_sz);
      ...
      if (rte_pktmbuf_chain(mem->decomp_bufs[i], next_seg) < 0) {
    ...
    mem->comp_bufs[i] = rte_pktmbuf_alloc(mem->comp_buf_pool);
    ...
    if (test_data->use_external_mbufs != 0) {
      rte_pktmbuf_attach_extbuf(
          mem->comp_bufs[i], mem->comp_memzones[cmz]->addr,
          mem->comp_memzones[cmz]->iova, test_data->out_seg_sz,
          &mem->comp_buf_infos[cmz]);
      cmz++;
    }

    data_addr =
        (uint8_t *)rte_pktmbuf_append(mem->comp_bufs[i], test_data->out_seg_sz);
    if (data_addr == NULL) {
      RTE_LOG(ERR, USER1, "Could not append data\n");
      return -1;
    }
    ...
    for (j = 1; j < segs_per_mbuf; j++) {
      struct rte_mbuf *next_seg = rte_pktmbuf_alloc(mem->comp_buf_pool);
      ...
      if (test_data->use_external_mbufs != 0) {
        rte_pktmbuf_attach_extbuf(next_seg, mem->comp_memzones[cmz]->addr,
      ...
      data_addr =
          (uint8_t *)rte_pktmbuf_append(next_seg, test_data->out_seg_sz);
      if (data_addr == NULL) {
        RTE_LOG(ERR, USER1, "Could not append data\n");
        return -1;
      }

      if (rte_pktmbuf_chain(mem->comp_bufs[i], next_seg) < 0) {
        RTE_LOG(ERR, USER1, "Could not chain mbufs\n");
        return -1;
      }
    }
...
void print_test_dynamics(const struct comp_test_data *test_data) {
  uint32_t opt_total_segs = DIV_CEIL(buffer_info.input_data_sz, MAX_SEG_SIZE);
  ...
  printf("\nFor the current input parameters (segment size = %u,"
         ...
         buffer_info.segment_sz, buffer_info.segments_per_buff);
