...
static inline bool is_avail_op(struct active_device *ad,
                               enum rte_bbdev_op_type op_type) {
  return ad->supported_ops & (1 << op_type);
}

static inline bool flags_match(uint32_t flags_req, uint32_t flags_present) {
  return (flags_req & flags_present) == flags_req;
}

static void clear_soft_out_cap(uint32_t *op_flags) {
 ...
 */
static inline void convert_op_data_to_be(void) {
...
static int check_dev_cap(const struct rte_bbdev_info *dev_info) {
  unsigned int i;
  unsigned int nb_inputs, nb_soft_outputs, nb_hard_outputs, nb_harq_inputs,
      nb_harq_outputs;
  ...
  for (i = 0; op_cap->type != RTE_BBDEV_OP_NONE; ++i, ++op_cap) {
    ...
    if (op_cap->type == RTE_BBDEV_OP_TURBO_DEC) {
      const struct rte_bbdev_op_cap_turbo_dec *cap = &op_cap->cap.turbo_dec;
      /* Ignore lack of soft output capability, just skip
       * checking if soft output is valid.
       */
      if ((test_vector.turbo_dec.op_flags & RTE_BBDEV_TURBO_SOFT_OUTPUT) &&
          !(cap->capability_flags & RTE_BBDEV_TURBO_SOFT_OUTPUT)) {
        printf("INFO: Device \"%s\" does not support soft output - soft output "
               "flags will be ignored.\n",
               dev_info->dev_name);
        clear_soft_out_cap(&test_vector.turbo_dec.op_flags);
      }

      if (!flags_match(test_vector.turbo_dec.op_flags, cap->capability_flags))
        return TEST_FAILED;
      ...
      if (nb_soft_outputs > cap->num_buffers_soft_out &&
          (test_vector.turbo_dec.op_flags & RTE_BBDEV_TURBO_SOFT_OUTPUT)) {
        printf("Too many soft outputs defined: %u, max: %u\n", nb_soft_outputs,
               cap->num_buffers_soft_out);
        return TEST_FAILED;
      }
      if (nb_hard_outputs > cap->num_buffers_hard_out) {
        printf("Too many hard outputs defined: %u, max: %u\n", nb_hard_outputs,
               cap->num_buffers_hard_out);
        return TEST_FAILED;
      }
      if (intr_enabled &&
          !(cap->capability_flags & RTE_BBDEV_TURBO_DEC_INTERRUPTS)) {
        printf("Dequeue interrupts are not supported!\n");
        return TEST_FAILED;
      }

      return TEST_SUCCESS;
    } else if (op_cap->type == RTE_BBDEV_OP_TURBO_ENC) {
      const struct rte_bbdev_op_cap_turbo_enc *cap = &op_cap->cap.turbo_enc;

      if (!flags_match(test_vector.turbo_enc.op_flags, cap->capability_flags))
        return TEST_FAILED;
      if (nb_inputs > cap->num_buffers_src) {
        printf("Too many inputs defined: %u, max: %u\n", nb_inputs,
               cap->num_buffers_src);
      ...
      if (intr_enabled &&
          !(cap->capability_flags & RTE_BBDEV_TURBO_ENC_INTERRUPTS)) {
        printf("Dequeue interrupts are not supported!\n");
        return TEST_FAILED;
      }
      ...
      const struct rte_bbdev_op_cap_ldpc_enc *cap = &op_cap->cap.ldpc_enc;

      if (!flags_match(test_vector.ldpc_enc.op_flags, cap->capability_flags)) {
      ...
      if (intr_enabled &&
          !(cap->capability_flags & RTE_BBDEV_LDPC_ENC_INTERRUPTS)) {
        printf("Dequeue interrupts are not supported!\n");
        return TEST_FAILED;
      }
      ...
      if (!flags_match(test_vector.ldpc_dec.op_flags, cap->capability_flags)) {
        printf("Flag Mismatch\n");
        return TEST_FAILED;
      }
      if (nb_inputs > cap->num_buffers_src) {
      ...
      if (nb_harq_outputs > cap->num_buffers_hard_out) {
        printf("Too many HARQ outputs defined: %u, max: %u\n", nb_harq_outputs,
               cap->num_buffers_hard_out);
        return TEST_FAILED;
      }
      if (intr_enabled &&
          !(cap->capability_flags & RTE_BBDEV_LDPC_DEC_INTERRUPTS)) {
        printf("Dequeue interrupts are not supported!\n");
        return TEST_FAILED;
      }
...
static struct rte_mempool *create_mbuf_pool(struct op_data_entries *entries,
                                            uint8_t dev_id, int socket_id,
  ...
  return rte_pktmbuf_pool_create(
      pool_name, mbuf_pool_size, 0, 0,
      RTE_MAX(max_seg_sz + RTE_PKTMBUF_HEADROOM + FILLER_HEADROOM,
              (unsigned int)RTE_MBUF_DEFAULT_BUF_SIZE),
      socket_id);
}

static int create_mempools(struct active_device *ad, int socket_id,
  ...
  struct op_data_entries *hard_out = &test_vector.entries[DATA_HARD_OUTPUT];
  struct op_data_entries *soft_out = &test_vector.entries[DATA_SOFT_OUTPUT];
  struct op_data_entries *harq_in = &test_vector.entries[DATA_HARQ_INPUT];
  struct op_data_entries *harq_out = &test_vector.entries[DATA_HARQ_OUTPUT];
  ...
  mp = rte_bbdev_op_pool_create(pool_name, op_type, ops_pool_size,
                                OPS_CACHE_SIZE, socket_id);
  TEST_ASSERT_NOT_NULL(
      mp, "ERROR Failed to create %u items ops pool for dev %u on socket %u.",
  ...
  if (in->nb_segments > 0) {
    ...
    TEST_ASSERT_NOT_NULL(mp,
                         "ERROR Failed to create %u items input pktmbuf pool "
                         "for dev %u on socket %u.",
  ...
  if (hard_out->nb_segments > 0) {
    mbuf_pool_size =
        optimal_mempool_size(ops_pool_size * hard_out->nb_segments);
    mp = create_mbuf_pool(hard_out, ad->dev_id, socket_id, mbuf_pool_size,
                          "hard_out");
    TEST_ASSERT_NOT_NULL(mp,
                         "ERROR Failed to create %u items hard output pktmbuf "
                         "pool for dev %u on socket %u.",
                         mbuf_pool_size, ad->dev_id, socket_id);
  ...
  if (soft_out->nb_segments > 0) {
    mbuf_pool_size =
        optimal_mempool_size(ops_pool_size * soft_out->nb_segments);
    mp = create_mbuf_pool(soft_out, ad->dev_id, socket_id, mbuf_pool_size,
                          "soft_out");
    TEST_ASSERT_NOT_NULL(mp,
                         "ERROR Failed to create %uB soft output pktmbuf pool "
                         "for dev %u on socket %u.",
                         mbuf_pool_size, ad->dev_id, socket_id);
  ...
  if (harq_in->nb_segments > 0) {
    mbuf_pool_size = optimal_mempool_size(ops_pool_size * harq_in->nb_segments);
    mp = create_mbuf_pool(harq_in, ad->dev_id, socket_id, mbuf_pool_size,
                          "harq_in");
    TEST_ASSERT_NOT_NULL(mp,
                         "ERROR Failed to create %uB harq input pktmbuf pool "
                         "for dev %u on socket %u.",
                         mbuf_pool_size, ad->dev_id, socket_id);
  ...
  if (harq_out->nb_segments > 0) {
    mbuf_pool_size =
        optimal_mempool_size(ops_pool_size * harq_out->nb_segments);
    mp = create_mbuf_pool(harq_out, ad->dev_id, socket_id, mbuf_pool_size,
                          "harq_out");
    TEST_ASSERT_NOT_NULL(mp,
                         "ERROR Failed to create %uB harq output pktmbuf pool "
                         "for dev %u on socket %u.",
                         mbuf_pool_size, ad->dev_id, socket_id);
 ...
 */
  ...
  if ((get_init_device() == true) &&
    ...
    TEST_ASSERT_SUCCESS(ret, "Failed to configure 4G FPGA PF for bbdev %s",
                        info->dev_name);
  }
#endif
#ifdef RTE_BASEBAND_FPGA_5GNR_FEC
  if ((get_init_device() == true) &&
    ...
    TEST_ASSERT_SUCCESS(ret, "Failed to configure 5G FPGA PF for bbdev %s",
                        info->dev_name);
  }
#endif
#ifdef RTE_BASEBAND_ACC100
  if ((get_init_device() == true) &&
    ...
    TEST_ASSERT_SUCCESS(ret, "Failed to configure ACC100 PF for bbdev %s",
                        info->dev_name);
  }
#endif
  ...
  TEST_ASSERT(queue_id != 0, "ERROR Failed to configure any queues on dev %u",
              dev_id);
...
static uint8_t populate_active_devices(void) {
  int ret;
  uint8_t dev_id;
...
static int read_test_vector(void) {
  int ret;
...
static int interrupt_testsuite_setup(void) {
  TEST_ASSERT_SUCCESS(read_test_vector(), "Test suite setup failed\n");
...
static int ut_setup(void) {
  ...
  for (i = 0; i < nb_active_devs; i++) {
    ...
    /* start the device */
    TEST_ASSERT_SUCCESS(rte_bbdev_start(dev_id), "Failed to start bbdev %u",
                        dev_id);
  }
...
static int init_op_data_objs(struct rte_bbdev_op_data *bufs,
  ...
  for (i = 0; i < n; ++i) {
    ...
    TEST_ASSERT_NOT_NULL(
        ...
        op_type, n * ref_entries->nb_segments, mbuf_pool->size);
    ...
    if ((op_type == DATA_INPUT) || (op_type == DATA_HARQ_INPUT)) {
      if ((op_type == DATA_INPUT) && large_input) {
        ...
        TEST_ASSERT_NOT_NULL(data, "rte malloc failed with %u bytes",
                             seg->length);
      ...
      for (j = 1; j < ref_entries->nb_segments; ++j) {
        ...
        TEST_ASSERT_NOT_NULL(m_tail,
                             "Not enough mbufs in %d data type mbuf pool "
        ...
        TEST_ASSERT_SUCCESS(
            ret, "Couldn't chain mbufs from %d data type mbuf pool", op_type);
      }
    } else {
...
static void limit_input_llr_val_range(struct rte_bbdev_op_data *input_ops,
                                      const uint16_t n,
                                      const int8_t max_llr_modulus) {
  uint16_t i, byte_idx;
  ...
  for (i = 0; i < n; ++i) {
    ...
    while (m != NULL) {
      ...
      for (byte_idx = 0; byte_idx < rte_pktmbuf_data_len(m); ++byte_idx)
        llr[byte_idx] =
            round((double)max_llr_modulus * llr[byte_idx] / INT8_MAX);
 ...
 */
static void ldpc_add_filler(struct rte_bbdev_op_data *input_ops,
                            const uint16_t n,
                            struct test_op_params *op_params) {
  ...
  if (!(ldpc_cap_flags & RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_FILLERS))
    return;
...
static int fill_queue_buffers(
    struct test_op_params *op_params, struct rte_mempool *in_mp,
    struct rte_mempool *hard_out_mp, struct rte_mempool *soft_out_mp,
    struct rte_mempool *harq_in_mp, struct rte_mempool *harq_out_mp,
    uint16_t queue_id, const struct rte_bbdev_op_cap *capabilities,
  ...
  struct rte_mempool *mbuf_pools[DATA_NUM_TYPES] = {
      in_mp, soft_out_mp, hard_out_mp, harq_in_mp, harq_out_mp,
  };
  ...
  for (type = DATA_INPUT; type < DATA_NUM_TYPES; ++type) {
    struct op_data_entries *ref_entries = &test_vector.entries[type];
    if (ref_entries->nb_segments == 0)
      continue;

    ret = allocate_buffers_on_socket(
        queue_ops[type], n * sizeof(struct rte_bbdev_op_data), socket_id);
    TEST_ASSERT_SUCCESS(
        ret, "Couldn't allocate memory for rte_bbdev_op_data structs");

    ret = init_op_data_objs(*queue_ops[type], ref_entries, mbuf_pools[type], n,
                            type, min_alignment);
  ...
  if (test_vector.op_type == RTE_BBDEV_OP_LDPC_DEC) {
    ...
    if (!loopback && !llr_comp)
      ldpc_input_llr_scaling(*queue_ops[DATA_INPUT], n, ldpc_llr_size,
                             ldpc_llr_decimals);
    if (!loopback && !harq_comp)
      ldpc_input_llr_scaling(*queue_ops[DATA_HARQ_INPUT], n, ldpc_llr_size,
                             ldpc_llr_decimals);
    if (!loopback)
      ldpc_add_filler(*queue_ops[DATA_HARQ_INPUT], n, op_params);
  }

  return 0;
}

static void free_buffers(struct active_device *ad,
                         struct test_op_params *op_params) {
  unsigned int i, j;
...
static void copy_reference_dec_op(struct rte_bbdev_dec_op **ops, unsigned int n,
  ...
  for (i = 0; i < n; ++i) {
    if (turbo_dec->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
      ops[i]->turbo_dec.tb_params.ea = turbo_dec->tb_params.ea;
      ops[i]->turbo_dec.tb_params.eb = turbo_dec->tb_params.eb;
      ops[i]->turbo_dec.tb_params.k_pos = turbo_dec->tb_params.k_pos;
      ops[i]->turbo_dec.tb_params.k_neg = turbo_dec->tb_params.k_neg;
      ops[i]->turbo_dec.tb_params.c = turbo_dec->tb_params.c;
      ops[i]->turbo_dec.tb_params.c_neg = turbo_dec->tb_params.c_neg;
      ops[i]->turbo_dec.tb_params.cab = turbo_dec->tb_params.cab;
      ops[i]->turbo_dec.tb_params.r = turbo_dec->tb_params.r;
    } else {
    ...
    if (soft_outputs != NULL)
      ops[i]->turbo_dec.soft_output = soft_outputs[start_idx + i];
  }
}

static void copy_reference_enc_op(struct rte_bbdev_enc_op **ops, unsigned int n,
                                  unsigned int start_idx,
                                  struct rte_bbdev_op_data *inputs,
  ...
  for (i = 0; i < n; ++i) {
    if (turbo_enc->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
      ops[i]->turbo_enc.tb_params.ea = turbo_enc->tb_params.ea;
      ops[i]->turbo_enc.tb_params.eb = turbo_enc->tb_params.eb;
      ops[i]->turbo_enc.tb_params.k_pos = turbo_enc->tb_params.k_pos;
      ops[i]->turbo_enc.tb_params.k_neg = turbo_enc->tb_params.k_neg;
      ops[i]->turbo_enc.tb_params.c = turbo_enc->tb_params.c;
      ops[i]->turbo_enc.tb_params.c_neg = turbo_enc->tb_params.c_neg;
      ops[i]->turbo_enc.tb_params.cab = turbo_enc->tb_params.cab;
      ops[i]->turbo_enc.tb_params.ncb_pos = turbo_enc->tb_params.ncb_pos;
      ops[i]->turbo_enc.tb_params.ncb_neg = turbo_enc->tb_params.ncb_neg;
      ...
      ops[i]->turbo_enc.cb_params.ncb = turbo_enc->cb_params.ncb;
    }
...
static inline double maxstar(double A, double B) {
  if (fabs(A - B) > 5)
    return RTE_MAX(A, B);
  else
 ...
 */
static void gen_qm8_llr(int8_t *llrs, uint32_t i, double N0, double llr_max) {
  int qm = 8;
  int qam = 256;
  int m, k;
  ...
  const double symbols_I[256] = {
      5,   5,   7,  7,   5,   5,   7,   7,   3,   3,   1,   1,   3,   3,   1,
      1,   5,   5,  7,   7,   5,   5,   7,   7,   3,   3,   1,   1,   3,   3,
      1,   1,   11, 11,  9,   9,   11,  11,  9,   9,   13,  13,  15,  15,  13,
      13,  15,  15, 11,  11,  9,   9,   11,  11,  9,   9,   13,  13,  15,  15,
      13,  13,  15, 15,  5,   5,   7,   7,   5,   5,   7,   7,   3,   3,   1,
      1,   3,   3,  1,   1,   5,   5,   7,   7,   5,   5,   7,   7,   3,   3,
      1,   1,   3,  3,   1,   1,   11,  11,  9,   9,   11,  11,  9,   9,   13,
      13,  15,  15, 13,  13,  15,  15,  11,  11,  9,   9,   11,  11,  9,   9,
      13,  13,  15, 15,  13,  13,  15,  15,  -5,  -5,  -7,  -7,  -5,  -5,  -7,
      -7,  -3,  -3, -1,  -1,  -3,  -3,  -1,  -1,  -5,  -5,  -7,  -7,  -5,  -5,
      -7,  -7,  -3, -3,  -1,  -1,  -3,  -3,  -1,  -1,  -11, -11, -9,  -9,  -11,
      -11, -9,  -9, -13, -13, -15, -15, -13, -13, -15, -15, -11, -11, -9,  -9,
      -11, -11, -9, -9,  -13, -13, -15, -15, -13, -13, -15, -15, -5,  -5,  -7,
      -7,  -5,  -5, -7,  -7,  -3,  -3,  -1,  -1,  -3,  -3,  -1,  -1,  -5,  -5,
      -7,  -7,  -5, -5,  -7,  -7,  -3,  -3,  -1,  -1,  -3,  -3,  -1,  -1,  -11,
      -11, -9,  -9, -11, -11, -9,  -9,  -13, -13, -15, -15, -13, -13, -15, -15,
      -11, -11, -9, -9,  -11, -11, -9,  -9,  -13, -13, -15, -15, -13, -13, -15,
      -15};
  const double symbols_Q[256] = {
      5,   7,  5,   7,   3,   1,   3,   1,   5,   7,   5,   7,   3,   1,   3,
      1,   11, 9,   11,  9,   13,  15,  13,  15,  11,  9,   11,  9,   13,  15,
      13,  15, 5,   7,   5,   7,   3,   1,   3,   1,   5,   7,   5,   7,   3,
      1,   3,  1,   11,  9,   11,  9,   13,  15,  13,  15,  11,  9,   11,  9,
      13,  15, 13,  15,  -5,  -7,  -5,  -7,  -3,  -1,  -3,  -1,  -5,  -7,  -5,
      -7,  -3, -1,  -3,  -1,  -11, -9,  -11, -9,  -13, -15, -13, -15, -11, -9,
      -11, -9, -13, -15, -13, -15, -5,  -7,  -5,  -7,  -3,  -1,  -3,  -1,  -5,
      -7,  -5, -7,  -3,  -1,  -3,  -1,  -11, -9,  -11, -9,  -13, -15, -13, -15,
      -11, -9, -11, -9,  -13, -15, -13, -15, 5,   7,   5,   7,   3,   1,   3,
      1,   5,  7,   5,   7,   3,   1,   3,   1,   11,  9,   11,  9,   13,  15,
      13,  15, 11,  9,   11,  9,   13,  15,  13,  15,  5,   7,   5,   7,   3,
      1,   3,  1,   5,   7,   5,   7,   3,   1,   3,   1,   11,  9,   11,  9,
      13,  15, 13,  15,  11,  9,   11,  9,   13,  15,  13,  15,  -5,  -7,  -5,
      -7,  -3, -1,  -3,  -1,  -5,  -7,  -5,  -7,  -3,  -1,  -3,  -1,  -11, -9,
      -11, -9, -13, -15, -13, -15, -11, -9,  -11, -9,  -13, -15, -13, -15, -5,
      -7,  -5, -7,  -3,  -1,  -3,  -1,  -5,  -7,  -5,  -7,  -3,  -1,  -3,  -1,
      -11, -9, -11, -9,  -13, -15, -13, -15, -11, -9,  -11, -9,  -13, -15, -13,
      -15};
  ...
  /* 5.1.4 of TS38.211 */
  I = (1 - 2 * b[0]) *
      (8 - (1 - 2 * b[2]) * (4 - (1 - 2 * b[4]) * (2 - (1 - 2 * b[6]))));
  Q = (1 - 2 * b[1]) *
      (8 - (1 - 2 * b[3]) * (4 - (1 - 2 * b[5]) * (2 - (1 - 2 * b[7]))));
  /* AWGN channel */
  I += sqrt(N0 / 2) * randn(0);
  Q += sqrt(N0 / 2) * randn(1);
  /*
  ...
  for (m = 0; m < qam; m++)
    log_syml_prob[m] =
        -(pow(I - symbols_I[m], 2.0) + pow(Q - symbols_Q[m], 2.0)) / N0;
 ...
 * Modulation, AWGN and LLR estimation from max log development
 */
static void gen_qm6_llr(int8_t *llrs, uint32_t i, double N0, double llr_max) {
  int qm = 6;
  int qam = 64;
  int m, k;
  double I, Q, p0, p1, llr_, b[qm], log_syml_prob[qam];
 ...
 * Modulation, AWGN and LLR estimation from max log development
 */
static void gen_qm4_llr(int8_t *llrs, uint32_t i, double N0, double llr_max) {
  int qm = 4;
  int qam = 16;
  int m, k;
  double I, Q, p0, p1, llr_, b[qm], log_syml_prob[qam];
...
static void gen_qm2_llr(int8_t *llrs, uint32_t j, double N0, double llr_max) {
  double b, b1, n;
  ...
  n = randn(j % 2);
  b1 = ((llrs[j] > 0 ? 2.0 : -2.0) + coeff * n) / N0;
...
static void generate_llr_input(uint16_t n, struct rte_bbdev_op_data *inputs,
                               struct rte_bbdev_dec_op *ref_op) {
  struct rte_mbuf *m;
  uint16_t qm;
...
static void copy_reference_ldpc_dec_op(struct rte_bbdev_dec_op **ops,
                                       unsigned int n, unsigned int start_idx,
                                       struct rte_bbdev_op_data *inputs,
                                       struct rte_bbdev_op_data *hard_outputs,
                                       struct rte_bbdev_op_data *soft_outputs,
                                       struct rte_bbdev_op_data *harq_inputs,
  ...
  for (i = 0; i < n; ++i) {
    if (ldpc_dec->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
      ops[i]->ldpc_dec.tb_params.ea = ldpc_dec->tb_params.ea;
      ops[i]->ldpc_dec.tb_params.eb = ldpc_dec->tb_params.eb;
      ops[i]->ldpc_dec.tb_params.c = ldpc_dec->tb_params.c;
      ops[i]->ldpc_dec.tb_params.cab = ldpc_dec->tb_params.cab;
      ops[i]->ldpc_dec.tb_params.r = ldpc_dec->tb_params.r;
    } else {
...
static void copy_reference_ldpc_enc_op(struct rte_bbdev_enc_op **ops,
                                       unsigned int n, unsigned int start_idx,
                                       struct rte_bbdev_op_data *inputs,
                                       struct rte_bbdev_op_data *outputs,
                                       struct rte_bbdev_enc_op *ref_op) {
  ...
  for (i = 0; i < n; ++i) {
    if (ldpc_enc->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
      ...
      ops[i]->ldpc_enc.tb_params.cab = ldpc_enc->tb_params.cab;
...
static int check_dec_status_and_ordering(struct rte_bbdev_dec_op *op,
  ...
  TEST_ASSERT(status == expected_status,
              "op_status (%d) != expected_status (%d)", op->status,
              expected_status);
...
static int check_enc_status_and_ordering(struct rte_bbdev_enc_op *op,
                                         unsigned int order_idx,
                                         const int expected_status) {
...
static inline int validate_op_chain(struct rte_bbdev_op_data *op,
  ...
  for (i = 0; i < nb_dst_segments; ++i) {
    ...
    TEST_ASSERT_BUFFERS_ARE_EQUAL(
        orig_op->segments[i].addr,
        rte_pktmbuf_mtod_offset(m, uint32_t *, offset), data_len,
 ...
 */
static inline uint16_t get_k0(uint16_t n_cb, uint16_t z_c, uint8_t bg,
                              uint8_t rv_index) {
  if (rv_index == 0)
    return 0;
  ...
  if (rv_index == 1)
    return (((bg == 1 ? K0_1_1 : K0_1_2) * n_cb) / n) * z_c;
  else if (rv_index == 2)
    return (((bg == 1 ? K0_2_1 : K0_2_2) * n_cb) / n) * z_c;
  else
    return (((bg == 1 ? K0_3_1 : K0_3_2) * n_cb) / n) * z_c;
}

/* HARQ output length including the Filler bits */
static inline uint16_t compute_harq_len(struct rte_bbdev_op_ldpc_dec *ops_ld) {
  uint16_t k0 = 0;
  ...
  uint16_t parity_offset =
      (ops_ld->basegraph == 1 ? 20 : 8) * ops_ld->z_c - ops_ld->n_filler;
  uint16_t deRmOutSize = RTE_MIN(
      k0 + ops_ld->cb_params.e + ((k0 > parity_offset) ? 0 : ops_ld->n_filler),
      ops_ld->n_cb);
  uint16_t numRows = ((deRmOutSize + ops_ld->z_c - 1) / ops_ld->z_c);
  uint16_t harq_output_len = numRows * ops_ld->z_c;
  return harq_output_len;
}

static inline int validate_op_harq_chain(struct rte_bbdev_op_data *op,
                                         struct op_data_entries *orig_op,
  ...
  for (i = 0; i < nb_dst_segments; ++i) {
    ...
    TEST_ASSERT(orig_op->segments[i].length < (uint32_t)(data_len + 64),
                "Length of segment differ in original (%u) and filled (%u) op",
                orig_op->segments[i].length, data_len);
    ...
    if (!(ldpc_cap_flags & RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_FILLERS) ||
        (ops_ld->op_flags & RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_LOOPBACK)) {
      data_len -= ops_ld->z_c;
      ...
      parity_offset =
          (ops_ld->basegraph == 1 ? 20 : 8) * ops_ld->z_c - ops_ld->n_filler;
      uint16_t deRmOutSize = compute_harq_len(ops_ld) - ops_ld->n_filler;
    ...
    for (j = 0, jj = 0; j < data_len; j++, jj++) {
      if (j == parity_offset) {
        ...
        for (k = 0; k < ops_ld->n_filler; k++) {
          if (harq_out[jj] != llr_max_pre_scaling) {
            printf("HARQ Filler issue %d: %d %d\n", jj, harq_out[jj], llr_max);
            byte_error++;
          }
          jj++;
        }
      }
      if (!(ops_ld->op_flags & RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_LOOPBACK)) {
        if (ldpc_llr_decimals > 1)
          harq_out[jj] = (harq_out[jj] + 1) >> (ldpc_llr_decimals - 1);
        /* Saturated to S7 */
      ...
      if (harq_orig[j] != harq_out[jj]) {
        error = (harq_orig[j] > harq_out[jj]) ? harq_orig[j] - harq_out[jj]
                                              : harq_out[jj] - harq_orig[j];
        abs_harq_origin = harq_orig[j] > 0 ? harq_orig[j] : -harq_orig[j];
        /* Residual quantization error */
        if ((error > 8 && (abs_harq_origin < (llr_max - 16))) || (error > 16)) {
  ...
  if (byte_error)
    TEST_ASSERT(byte_error <= 1, "HARQ output mismatch (%d) %d", byte_error,
                cum_error);
...
static int validate_dec_op(struct rte_bbdev_dec_op **ops, const uint16_t n,
                           struct rte_bbdev_dec_op *ref_op,
  ...
  for (i = 0; i < n; ++i) {
    ...
    TEST_ASSERT_SUCCESS(ret, "Checking status and ordering for decoder failed");

    TEST_ASSERT_SUCCESS(validate_op_chain(hard_output, hard_data_orig),
                        "Hard output buffers (CB=%u) are not equal", i);

    if (ref_op->turbo_dec.op_flags & RTE_BBDEV_TURBO_SOFT_OUTPUT)
      TEST_ASSERT_SUCCESS(validate_op_chain(soft_output, soft_data_orig),
                          "Soft output buffers (CB=%u) are not equal", i);
  }
...
static int validate_ldpc_bler(struct rte_bbdev_dec_op **ops, const uint16_t n) {
  unsigned int i;
  struct op_data_entries *hard_data_orig =
      &test_vector.entries[DATA_HARD_OUTPUT];
...
static int validate_ldpc_dec_op(struct rte_bbdev_dec_op **ops, const uint16_t n,
                                struct rte_bbdev_dec_op *ref_op,
                                const int vector_mask) {
  unsigned int i;
  int ret;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (!check_bit(ops[i]->ldpc_dec.op_flags,
                   RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_LOOPBACK) &&
        (ops[i]->status & (1 << RTE_BBDEV_SYNDROME_ERROR)) == 0)
      TEST_ASSERT_SUCCESS(validate_op_chain(hard_output, hard_data_orig),
                          "Hard output buffers (CB=%u) are not equal", i);
    ...
    if (ref_op->ldpc_dec.op_flags & RTE_BBDEV_LDPC_SOFT_OUT_ENABLE)
      TEST_ASSERT_SUCCESS(validate_op_chain(soft_output, soft_data_orig),
                          "Soft output buffers (CB=%u) are not equal", i);
    if (ref_op->ldpc_dec.op_flags & RTE_BBDEV_LDPC_HQ_COMBINE_OUT_ENABLE) {
      TEST_ASSERT_SUCCESS(
          validate_op_harq_chain(harq_output, harq_data_orig, ops_td),
          "HARQ output buffers (CB=%u) are not equal", i);
    }
...
static int validate_enc_op(struct rte_bbdev_enc_op **ops, const uint16_t n,
                           struct rte_bbdev_enc_op *ref_op) {
  unsigned int i;
  int ret;
  struct op_data_entries *hard_data_orig =
  ...
  for (i = 0; i < n; ++i) {
    ...
    TEST_ASSERT_SUCCESS(ret, "Checking status and ordering for encoder failed");
    TEST_ASSERT_SUCCESS(
        validate_op_chain(&ops[i]->turbo_enc.output, hard_data_orig),
        "Output buffers (CB=%u) are not equal", i);
  }
...
static int validate_ldpc_enc_op(struct rte_bbdev_enc_op **ops, const uint16_t n,
                                struct rte_bbdev_enc_op *ref_op) {
  unsigned int i;
  int ret;
  struct op_data_entries *hard_data_orig =
  ...
  for (i = 0; i < n; ++i) {
    ...
    TEST_ASSERT_SUCCESS(ret, "Checking status and ordering for encoder failed");
    TEST_ASSERT_SUCCESS(
        validate_op_chain(&ops[i]->ldpc_enc.output, hard_data_orig),
        "Output buffers (CB=%u) are not equal", i);
  }
...
static void create_reference_dec_op(struct rte_bbdev_dec_op *op) {
  unsigned int i;
  ...
  for (i = 0; i < entry->nb_segments; ++i)
    op->turbo_dec.input.length += entry->segments[i].length;
}

static void create_reference_ldpc_dec_op(struct rte_bbdev_dec_op *op) {
  unsigned int i;
  struct op_data_entries *entry;
  ...
  entry = &test_vector.entries[DATA_INPUT];
  for (i = 0; i < entry->nb_segments; ++i)
    op->ldpc_dec.input.length += entry->segments[i].length;
  if (test_vector.ldpc_dec.op_flags & RTE_BBDEV_LDPC_HQ_COMBINE_IN_ENABLE) {
    entry = &test_vector.entries[DATA_HARQ_INPUT];
    for (i = 0; i < entry->nb_segments; ++i)
      op->ldpc_dec.harq_combined_input.length += entry->segments[i].length;
  }
}

static void create_reference_enc_op(struct rte_bbdev_enc_op *op) {
  unsigned int i;
  struct op_data_entries *entry;
  ...
  entry = &test_vector.entries[DATA_INPUT];
  for (i = 0; i < entry->nb_segments; ++i)
    op->turbo_enc.input.length += entry->segments[i].length;
}

static void create_reference_ldpc_enc_op(struct rte_bbdev_enc_op *op) {
  unsigned int i;
  struct op_data_entries *entry;
  ...
  entry = &test_vector.entries[DATA_INPUT];
  for (i = 0; i < entry->nb_segments; ++i)
    op->ldpc_enc.input.length += entry->segments[i].length;
}

static uint32_t calc_dec_TB_size(struct rte_bbdev_dec_op *op) {
  uint8_t i;
...
static uint32_t calc_ldpc_dec_TB_size(struct rte_bbdev_dec_op *op) {
  uint8_t i;
  uint32_t c, r, tb_size = 0;
...
static uint32_t calc_enc_TB_size(struct rte_bbdev_enc_op *op) {
  uint8_t i;
  uint32_t c, r, tb_size = 0;
...
static uint32_t calc_ldpc_enc_TB_size(struct rte_bbdev_enc_op *op) {
  uint8_t i;
  uint32_t c, r, tb_size = 0;
...
static int init_test_op_params(struct test_op_params *op_params,
                               enum rte_bbdev_op_type op_type,
                               const int expected_status, const int vector_mask,
                               struct rte_mempool *ops_mp, uint16_t burst_sz,
                               uint16_t num_to_process, uint16_t num_lcores) {
  int ret = 0;
...
static int run_test_case_on_device(test_case_function *test_case_func,
                                   uint8_t dev_id,
                                   struct test_op_params *op_params) {
  ...
  f_ret = create_mempools(ad, socket_id, op_type, get_num_ops());
  ...
  f_ret = init_test_op_params(op_params, test_vector.op_type,
                              test_vector.expected_status, test_vector.mask,
                              ad->ops_mempool, burst_sz, get_num_ops(),
                              get_num_lcores());
  if (f_ret != TEST_SUCCESS) {
  ...
  TEST_ASSERT_NOT_NULL(capabilities, "Couldn't find capabilities");
  ...
  for (i = 0; i < ad->nb_queues; ++i) {
    f_ret = fill_queue_buffers(
        op_params, ad->in_mbuf_pool, ad->hard_out_mbuf_pool,
        ad->soft_out_mbuf_pool, ad->harq_in_mbuf_pool, ad->harq_out_mbuf_pool,
        ad->queue_ids[i], capabilities, info.drv.min_alignment, socket_id);
    if (f_ret != TEST_SUCCESS) {
 ...
 * per burst size.
 */
static int run_test_case(test_case_function *test_case_func) {
  int ret = 0;
  uint8_t dev;

  /* Alloc op_params */
  struct test_op_params *op_params =
      rte_zmalloc(NULL, sizeof(struct test_op_params), RTE_CACHE_LINE_SIZE);
  TEST_ASSERT_NOT_NULL(
      op_params, "Failed to alloc %zuB for op_params",
...
static void retrieve_harq_ddr(uint16_t dev_id, uint16_t queue_id,
  ...
  for (j = 0; j < n; ++j) {
    if ((loopback && mem_out) || hc_out) {
      ...
      ops[j]->ldpc_dec.op_flags = RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_LOOPBACK +
                                  RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_IN_ENABLE;
      if (h_comp)
        ops[j]->ldpc_dec.op_flags += RTE_BBDEV_LDPC_HARQ_6BIT_COMPRESSION;
      ops[j]->ldpc_dec.harq_combined_input.offset = harq_offset;
      ...
      rte_bbdev_enqueue_ldpc_dec_ops(dev_id, queue_id, &ops[j], 1);
      ret = 0;
      while (ret == 0)
        ret = rte_bbdev_dequeue_ldpc_dec_ops(dev_id, queue_id, &ops_deq[j], 1);
 ...
 */
static void preload_harq_ddr(uint16_t dev_id, uint16_t queue_id,
                             struct rte_bbdev_dec_op **ops, const uint16_t n,
                             bool preload) {
  uint16_t j;
  int deq;
  ...
  if ((mem_in || hc_in) && preload) {
    for (j = 0; j < n; ++j) {
      ...
      if (h_comp)
        ops[j]->ldpc_dec.op_flags += RTE_BBDEV_LDPC_HARQ_6BIT_COMPRESSION;
      ops[j]->ldpc_dec.harq_combined_output.offset = harq_offset;
    ...
    deq = 0;
    while (deq != n)
      deq += rte_bbdev_dequeue_ldpc_dec_ops(dev_id, queue_id, &ops_deq[deq],
                                            n - deq);
...
static void dequeue_event_callback(uint16_t dev_id,
  ...
  if (unlikely(event != RTE_BBDEV_EVENT_DEQUEUE)) {
    ...
    printf("Dequeue interrupt handler called for incorrect event!\n");
    return;
  }
  ...
  if (test_vector.op_type == RTE_BBDEV_OP_TURBO_DEC)
    deq = rte_bbdev_dequeue_dec_ops(
        dev_id, queue_id,
        &tp->dec_ops[__atomic_load_n(&tp->nb_dequeued, __ATOMIC_RELAXED)],
        burst_sz);
  else if (test_vector.op_type == RTE_BBDEV_OP_LDPC_DEC)
    ...
    deq = rte_bbdev_dequeue_ldpc_enc_ops(
        dev_id, queue_id,
        &tp->enc_ops[__atomic_load_n(&tp->nb_dequeued, __ATOMIC_RELAXED)],
        burst_sz);
  else /*RTE_BBDEV_OP_TURBO_ENC*/
    deq = rte_bbdev_dequeue_enc_ops(
        dev_id, queue_id,
        &tp->enc_ops[__atomic_load_n(&tp->nb_dequeued, __ATOMIC_RELAXED)],
        burst_sz);

  if (deq < burst_sz) {
    printf("After receiving the interrupt all operations should be dequeued. "
           "Expected: %u, got: %u\n",
           burst_sz, deq);
    __atomic_store_n(&tp->processing_status, TEST_FAILED, __ATOMIC_RELAXED);
    return;
  ...
  if (test_vector.op_type == RTE_BBDEV_OP_TURBO_DEC) {
    ...
    for (i = 0; i < num_ops; ++i)
      tp->iter_count =
          RTE_MAX(tp->dec_ops[i]->turbo_dec.iter_count, tp->iter_count);
  ...
  tp->ops_per_sec +=
      ((double)num_ops) / ((double)total_time / (double)rte_get_tsc_hz());
...
static int throughput_intr_lcore_ldpc_dec(void *arg) {
  ...
  TEST_ASSERT_SUCCESS((burst_sz > MAX_BURST), "BURST_SIZE should be <= %u",
                      MAX_BURST);
  ...
  ret = rte_bbdev_dec_op_alloc_bulk(tp->op_params->mp, ops, num_to_process);
  ...
  for (j = 0; j < TEST_REPETITIONS; ++j) {
    ...
    for (enqueued = 0; enqueued < num_to_process;) {
      ...
      enq = 0;
      do {
        enq += rte_bbdev_enqueue_ldpc_dec_ops(tp->dev_id, queue_id,
...
static int throughput_intr_lcore_dec(void *arg) {
  struct thread_params *tp = arg;
  unsigned int enqueued;
  const uint16_t queue_id = tp->queue_id;
  const uint16_t burst_sz = tp->op_params->burst_sz;
  ...
  for (j = 0; j < TEST_REPETITIONS; ++j) {
    ...
    for (enqueued = 0; enqueued < num_to_process;) {
      ...
      enq = 0;
      do {
        enq += rte_bbdev_enqueue_dec_ops(tp->dev_id, queue_id, &ops[enqueued],
                                         num_to_enq);
      } while (unlikely(num_to_enq != enq));
      enqueued += enq;
...
static int throughput_intr_lcore_enc(void *arg) {
  struct thread_params *tp = arg;
  unsigned int enqueued;
  const uint16_t queue_id = tp->queue_id;
  const uint16_t burst_sz = tp->op_params->burst_sz;
  ...
  ret = rte_bbdev_enc_op_alloc_bulk(tp->op_params->mp, ops, num_to_process);
  TEST_ASSERT_SUCCESS(ret, "Allocation failed for %d ops", num_to_process);
  if (test_vector.op_type != RTE_BBDEV_OP_NONE)
  ...
  for (j = 0; j < TEST_REPETITIONS; ++j) {
    ...
    for (enqueued = 0; enqueued < num_to_process;) {
      ...
      enq = 0;
      do {
        enq += rte_bbdev_enqueue_enc_ops(tp->dev_id, queue_id, &ops[enqueued],
                                         num_to_enq);
...
static int throughput_intr_lcore_ldpc_enc(void *arg) {
  struct thread_params *tp = arg;
  unsigned int enqueued;
  const uint16_t queue_id = tp->queue_id;
  const uint16_t burst_sz = tp->op_params->burst_sz;
  ...
  TEST_ASSERT_SUCCESS(ret, "Allocation failed for %d ops", num_to_process);
  if (test_vector.op_type != RTE_BBDEV_OP_NONE)
    copy_reference_ldpc_enc_op(ops, num_to_process, 0, bufs->inputs,
                               bufs->hard_outputs, tp->op_params->ref_enc_op);
  ...
  for (j = 0; j < TEST_REPETITIONS; ++j) {
    ...
    for (enqueued = 0; enqueued < num_to_process;) {
      ...
      enq = 0;
      do {
        enq += rte_bbdev_enqueue_ldpc_enc_ops(tp->dev_id, queue_id,
                                              &ops[enqueued], num_to_enq);
      } while (unlikely(enq != num_to_enq));
      enqueued += enq;
...
static int throughput_pmd_lcore_dec(void *arg) {
  struct thread_params *tp = arg;
  uint16_t enq, deq;
  ...
  if (test_vector.op_type != RTE_BBDEV_OP_NONE)
    copy_reference_dec_op(ops_enq, num_ops, 0, bufs->inputs, bufs->hard_outputs,
                          bufs->soft_outputs, ref_op);
  ...
  for (i = 0; i < TEST_REPETITIONS; ++i) {
    ...
    for (enq = 0, deq = 0; enq < num_ops;) {
      ...
      enq += rte_bbdev_enqueue_dec_ops(tp->dev_id, queue_id, &ops_enq[enq],
                                       num_to_enq);

      deq += rte_bbdev_dequeue_dec_ops(tp->dev_id, queue_id, &ops_deq[deq],
                                       enq - deq);
    }
  ...
  /* get the max of iter_count for all dequeued ops */
  for (i = 0; i < num_ops; ++i) {
    tp->iter_count = RTE_MAX(ops_enq[i]->turbo_dec.iter_count, tp->iter_count);
  }

  if (test_vector.op_type != RTE_BBDEV_OP_NONE) {
    ret = validate_dec_op(ops_deq, num_ops, ref_op, tp->op_params->vector_mask);
  ...
  tp->ops_per_sec = ((double)num_ops * TEST_REPETITIONS) /
                    ((double)total_time / (double)rte_get_tsc_hz());
  tp->mbps =
      (((double)(num_ops * TEST_REPETITIONS * tb_len_bits)) / 1000000.0) /
      ((double)total_time / (double)rte_get_tsc_hz());

  return TEST_SUCCESS;
}

static int bler_pmd_lcore_ldpc_dec(void *arg) {
  struct thread_params *tp = arg;
  uint16_t enq, deq;
  uint64_t total_time = 0, start_time;
  const uint16_t queue_id = tp->queue_id;
  ...
  uint16_t num_to_enq;
  bool extDdr =
      check_bit(ldpc_cap_flags, RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_OUT_ENABLE);
  bool loopback = check_bit(ref_op->ldpc_dec.op_flags,
                            RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_LOOPBACK);
  bool hc_out = check_bit(ref_op->ldpc_dec.op_flags,
                          RTE_BBDEV_LDPC_HQ_COMBINE_OUT_ENABLE);
  ...
  for (i = 0; i < 1; ++i) { /* Could add more iterations */
    ...
    for (enq = 0, deq = 0; enq < num_ops;) {
      ...
      enq += rte_bbdev_enqueue_ldpc_dec_ops(tp->dev_id, queue_id, &ops_enq[enq],
                                            num_to_enq);

      deq += rte_bbdev_dequeue_ldpc_dec_ops(tp->dev_id, queue_id, &ops_deq[deq],
                                            enq - deq);
    }
  ...
  if (test_vector.op_type != RTE_BBDEV_OP_NONE && tp->bler == 0 &&
      parity_bler == 0 && !hc_out) {
  ...
  tp->ops_per_sec =
      ((double)num_ops * 1) / ((double)total_time / (double)rte_get_tsc_hz());
  tp->mbps = (((double)(num_ops * 1 * tb_len_bits)) / 1000000.0) /
             ((double)total_time / (double)rte_get_tsc_hz());

  return TEST_SUCCESS;
}

static int throughput_pmd_lcore_ldpc_dec(void *arg) {
  struct thread_params *tp = arg;
  uint16_t enq, deq;
  uint64_t total_time = 0, start_time;
  const uint16_t queue_id = tp->queue_id;
...
static int throughput_pmd_lcore_enc(void *arg) {
  struct thread_params *tp = arg;
  uint16_t enq, deq;
  uint64_t total_time = 0, start_time;
  const uint16_t queue_id = tp->queue_id;
  ...
  TEST_ASSERT_SUCCESS(ret, "Allocation failed for %d ops", num_ops);
  if (test_vector.op_type != RTE_BBDEV_OP_NONE)
    copy_reference_enc_op(ops_enq, num_ops, 0, bufs->inputs, bufs->hard_outputs,
                          ref_op);
  ...
  for (i = 0; i < TEST_REPETITIONS; ++i) {
    ...
    for (enq = 0, deq = 0; enq < num_ops;) {
      ...
      enq += rte_bbdev_enqueue_enc_ops(tp->dev_id, queue_id, &ops_enq[enq],
                                       num_to_enq);

      deq += rte_bbdev_dequeue_enc_ops(tp->dev_id, queue_id, &ops_deq[deq],
                                       enq - deq);
    }
...
static int throughput_pmd_lcore_ldpc_enc(void *arg) {
  struct thread_params *tp = arg;
  uint16_t enq, deq;
  uint64_t total_time = 0, start_time;
  const uint16_t queue_id = tp->queue_id;
  ...
  for (i = 0; i < TEST_REPETITIONS; ++i) {
    ...
    for (enq = 0, deq = 0; enq < num_ops;) {
      ...
      enq += rte_bbdev_enqueue_ldpc_enc_ops(tp->dev_id, queue_id, &ops_enq[enq],
                                            num_to_enq);

      deq += rte_bbdev_dequeue_ldpc_enc_ops(tp->dev_id, queue_id, &ops_deq[deq],
                                            enq - deq);
    }
...
static void print_enc_throughput(struct thread_params *t_params,
                                 unsigned int used_cores) {
  unsigned int iter = 0;
  double total_mops = 0, total_mbps = 0;

  for (iter = 0; iter < used_cores; iter++) {
    printf("Throughput for core (%u): %.8lg Ops/s, %.8lg Mbps\n",
...
static void print_dec_throughput(struct thread_params *t_params,
                                 unsigned int used_cores) {
  ...
  for (core_idx = 0; core_idx < used_cores; core_idx++) {
    printf("Throughput for core (%u): %.8lg Ops/s, %.8lg Mbps @ max %u "
           "iterations\n",
           t_params[core_idx].lcore_id, t_params[core_idx].ops_per_sec,
           t_params[core_idx].mbps, t_params[core_idx].iter_count);
  ...
  printf("\nTotal throughput for %u cores: %.8lg MOPS, %.8lg Mbps @ max %u "
         "iterations\n",
         used_cores, total_mops, total_mbps, iter_count);
}

/* Aggregate the performance results over the number of cores used */
static void print_dec_bler(struct thread_params *t_params,
                           unsigned int used_cores) {
  unsigned int core_idx = 0;
  ...
  for (core_idx = 0; core_idx < used_cores; core_idx++) {
    printf("Core%u BLER %.1f %% - Iters %.1f - Tp %.1f Mbps %s\n",
           t_params[core_idx].lcore_id, t_params[core_idx].bler * 100,
  ...
  printf("SNR %.2f BLER %.1f %% - Iterations %.1f %d - Tp %.1f Mbps %s\n", snr,
         total_bler * 100, total_iter, get_iter_max(), total_mbps,
         get_vector_filename());
}
 ...
 */
static int bler_test(struct active_device *ad,
                     struct test_op_params *op_params) {
  int ret;
  ...
  TEST_ASSERT_NOT_NULL(op_type_str, "Invalid op type: %u", test_vector.op_type);

  printf("+ ------------------------------------------------------- +\n");
  printf("== test: bler\ndev: %s, nb_queues: %u, burst size: %u, num ops: %u, "
         "num_lcores: %u, op type: %s, itr mode: %s, GHz: %lg\n",
         info.dev_name, ad->nb_queues, op_params->burst_sz,
         op_params->num_to_process, op_params->num_lcores, op_type_str,
  ...
  if ((test_vector.op_type == RTE_BBDEV_OP_LDPC_DEC) &&
      !check_bit(test_vector.ldpc_dec.op_flags,
                 RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_LOOPBACK) &&
      !check_bit(test_vector.ldpc_dec.op_flags, RTE_BBDEV_LDPC_LLR_COMPRESSION))
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    rte_eal_remote_launch(bler_function, &t_params[used_cores++], lcore_id);
  }
 ...
 */
static int throughput_test(struct active_device *ad,
                           struct test_op_params *op_params) {
  int ret;
  unsigned int lcore_id, used_cores = 0;
  ...
  printf("+ ------------------------------------------------------- +\n");
  printf("== test: throughput\ndev: %s, nb_queues: %u, burst size: %u, num "
         "ops: %u, num_lcores: %u, op type: %s, itr mode: %s, GHz: %lg\n",
         info.dev_name, ad->nb_queues, op_params->burst_sz,
         op_params->num_to_process, op_params->num_lcores, op_type_str,
         intr_enabled ? "Interrupt mode" : "PMD mode",
         (double)rte_get_tsc_hz() / 1000000000.0);
  ...
  if (intr_enabled) {
    ...
    ret = rte_bbdev_callback_register(ad->dev_id, RTE_BBDEV_EVENT_DEQUEUE,
                                      dequeue_event_callback, t_params);
    if (ret < 0) {
      rte_free(t_params);
      return ret;
    }
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    rte_eal_remote_launch(throughput_function, &t_params[used_cores++],
                          lcore_id);
  }
...
static int latency_test_dec(struct rte_mempool *mempool,
                            struct test_buffers *bufs,
                            struct rte_bbdev_dec_op *ref_op, int vector_mask,
                            uint16_t dev_id, uint16_t queue_id,
                            const uint16_t num_to_process, uint16_t burst_sz,
                            uint64_t *total_time, uint64_t *min_time,
                            uint64_t *max_time) {
  ...
  for (i = 0, dequeued = 0; dequeued < num_to_process; ++i) {
    ...
    if (test_vector.op_type != RTE_BBDEV_OP_NONE)
      copy_reference_dec_op(ops_enq, burst_sz, dequeued, bufs->inputs,
    ...
    enq = rte_bbdev_enqueue_dec_ops(dev_id, queue_id, &ops_enq[enq], burst_sz);
    TEST_ASSERT(enq == burst_sz, "Error enqueueing burst, expected %u, got %u",
                burst_sz, enq);

    /* Dequeue */
    do {
      deq += rte_bbdev_dequeue_dec_ops(dev_id, queue_id, &ops_deq[deq],
                                       burst_sz - deq);
    ...
    *total_time += last_time;
    ...
    if (test_vector.op_type != RTE_BBDEV_OP_NONE) {
      ret = validate_dec_op(ops_deq, burst_sz, ref_op, vector_mask);
      TEST_ASSERT_SUCCESS(ret, "Validation failed!");
    }
...
static int
latency_test_ldpc_dec(struct rte_mempool *mempool, struct test_buffers *bufs,
                      struct rte_bbdev_dec_op *ref_op, int vector_mask,
                      uint16_t dev_id, uint16_t queue_id,
                      const uint16_t num_to_process, uint16_t burst_sz,
                      uint64_t *total_time, uint64_t *min_time,
                      uint64_t *max_time, bool disable_et) {
  int ret = TEST_SUCCESS;
  uint16_t i, j, dequeued;
  struct rte_bbdev_dec_op *ops_enq[MAX_BURST], *ops_deq[MAX_BURST];
  uint64_t start_time = 0, last_time = 0;
  bool extDdr = ldpc_cap_flags & RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_OUT_ENABLE;
  ...
  for (i = 0, dequeued = 0; dequeued < num_to_process; ++i) {
    ...
    if (test_vector.op_type != RTE_BBDEV_OP_NONE)
      copy_reference_ldpc_dec_op(ops_enq, burst_sz, dequeued, bufs->inputs,
                                 bufs->hard_outputs, bufs->soft_outputs,
                                 bufs->harq_inputs, bufs->harq_outputs, ref_op);

    if (extDdr)
      preload_harq_ddr(dev_id, queue_id, ops_enq, burst_sz, true);
    ...
    enq = rte_bbdev_enqueue_ldpc_dec_ops(dev_id, queue_id, &ops_enq[enq],
                                         burst_sz);
    TEST_ASSERT(enq == burst_sz, "Error enqueueing burst, expected %u, got %u",
                burst_sz, enq);
...
static int latency_test_enc(struct rte_mempool *mempool,
  ...
  for (i = 0, dequeued = 0; dequeued < num_to_process; ++i) {
    ...
    if (test_vector.op_type != RTE_BBDEV_OP_NONE)
      copy_reference_enc_op(ops_enq, burst_sz, dequeued, bufs->inputs,
                            bufs->hard_outputs, ref_op);
    ...
    enq = rte_bbdev_enqueue_enc_ops(dev_id, queue_id, &ops_enq[enq], burst_sz);
    TEST_ASSERT(enq == burst_sz, "Error enqueueing burst, expected %u, got %u",
                burst_sz, enq);

    /* Dequeue */
    do {
      deq += rte_bbdev_dequeue_enc_ops(dev_id, queue_id, &ops_deq[deq],
                                       burst_sz - deq);
      if (likely(first_time && (deq > 0))) {
        last_time += rte_rdtsc_precise() - start_time;
        first_time = false;
...
static int latency_test_ldpc_enc(struct rte_mempool *mempool,
                                 struct test_buffers *bufs,
                                 struct rte_bbdev_enc_op *ref_op,
                                 uint16_t dev_id, uint16_t queue_id,
  ...
  for (i = 0, dequeued = 0; dequeued < num_to_process; ++i) {
    ...
    TEST_ASSERT_SUCCESS(ret, "rte_bbdev_enc_op_alloc_bulk() failed");
    if (test_vector.op_type != RTE_BBDEV_OP_NONE)
      copy_reference_ldpc_enc_op(ops_enq, burst_sz, dequeued, bufs->inputs,
                                 bufs->hard_outputs, ref_op);
    ...
    enq = rte_bbdev_enqueue_ldpc_enc_ops(dev_id, queue_id, &ops_enq[enq],
                                         burst_sz);
    TEST_ASSERT(enq == burst_sz, "Error enqueueing burst, expected %u, got %u",
                burst_sz, enq);
    ...
    /* Dequeue */
    do {
      deq += rte_bbdev_dequeue_ldpc_enc_ops(dev_id, queue_id, &ops_deq[deq],
                                            burst_sz - deq);
      if (likely(first_time && (deq > 0))) {
        last_time += rte_rdtsc_precise() - start_time;
        first_time = false;
...
static int validation_latency_test(struct active_device *ad,
                                   struct test_op_params *op_params,
                                   bool latency_flag) {
  int iter;
  ...
  printf("%s, burst size: %u, num ops: %u, op type: %s\n", info.dev_name,
         burst_sz, num_to_process, op_type_str);

  if (op_type == RTE_BBDEV_OP_TURBO_DEC)
    iter = latency_test_dec(op_params->mp, bufs, op_params->ref_dec_op,
                            op_params->vector_mask, ad->dev_id, queue_id,
                            num_to_process, burst_sz, &total_time, &min_time,
                            &max_time);
  else if (op_type == RTE_BBDEV_OP_LDPC_ENC)
    iter = latency_test_ldpc_enc(op_params->mp, bufs, op_params->ref_enc_op,
                                 ad->dev_id, queue_id, num_to_process, burst_sz,
                                 &total_time, &min_time, &max_time);
  else if (op_type == RTE_BBDEV_OP_LDPC_DEC)
    iter = latency_test_ldpc_dec(op_params->mp, bufs, op_params->ref_dec_op,
                                 op_params->vector_mask, ad->dev_id, queue_id,
                                 num_to_process, burst_sz, &total_time,
                                 &min_time, &max_time, latency_flag);
  else /* RTE_BBDEV_OP_TURBO_ENC */
    iter = latency_test_enc(op_params->mp, bufs, op_params->ref_enc_op,
                            ad->dev_id, queue_id, num_to_process, burst_sz,
                            &total_time, &min_time, &max_time);
  ...
  printf(
      ...
      (double)(total_time * 1000000) / (double)iter / (double)rte_get_tsc_hz(),
      (double)min_time, (double)(min_time * 1000000) / (double)rte_get_tsc_hz(),
      (double)max_time,
      (double)(max_time * 1000000) / (double)rte_get_tsc_hz());

  return TEST_SUCCESS;
}

static int latency_test(struct active_device *ad,
                        struct test_op_params *op_params) {
  return validation_latency_test(ad, op_params, true);
}

static int validation_test(struct active_device *ad,
                           struct test_op_params *op_params) {
  return validation_latency_test(ad, op_params, false);
}

#ifdef RTE_BBDEV_OFFLOAD_COST
static int get_bbdev_queue_stats(uint16_t dev_id, uint16_t queue_id,
                                 struct rte_bbdev_stats *stats) {
...
offload_latency_test_dec(struct rte_mempool *mempool, struct test_buffers *bufs,
  ...
  for (i = 0, dequeued = 0; dequeued < num_to_process; ++i) {
    ...
    enq_sw_last_time =
        rte_rdtsc_precise() - enq_start_time - stats.acc_offload_cycles;
    time_st->enq_sw_max_time =
        RTE_MAX(time_st->enq_sw_max_time, enq_sw_last_time);
    time_st->enq_sw_min_time =
        RTE_MIN(time_st->enq_sw_min_time, enq_sw_last_time);
    time_st->enq_sw_total_time += enq_sw_last_time;

    time_st->enq_acc_max_time =
        RTE_MAX(time_st->enq_acc_max_time, stats.acc_offload_cycles);
    time_st->enq_acc_min_time =
        RTE_MIN(time_st->enq_acc_min_time, stats.acc_offload_cycles);
    ...
    do {
      deq += rte_bbdev_dequeue_dec_ops(dev_id, queue_id, &ops_deq[deq], enq);
...
static int offload_latency_test_ldpc_dec(struct rte_mempool *mempool,
                                         struct test_buffers *bufs,
                                         struct rte_bbdev_dec_op *ref_op,
                                         uint16_t dev_id, uint16_t queue_id,
                                         const uint16_t num_to_process,
  ...
  for (i = 0, dequeued = 0; dequeued < num_to_process; ++i) {
    ...
    do {
      deq +=
          rte_bbdev_dequeue_ldpc_dec_ops(dev_id, queue_id, &ops_deq[deq], enq);
    } while (unlikely(deq == 0));
...
offload_latency_test_enc(struct rte_mempool *mempool, struct test_buffers *bufs,
  ...
  for (i = 0, dequeued = 0; dequeued < num_to_process; ++i) {
    ...
    /* Dequeue one operation */
    do {
      deq += rte_bbdev_dequeue_enc_ops(dev_id, queue_id, &ops_deq[deq], enq);
    } while (unlikely(deq == 0));
...
static int offload_latency_test_ldpc_enc(struct rte_mempool *mempool,
                                         struct test_buffers *bufs,
                                         struct rte_bbdev_enc_op *ref_op,
                                         uint16_t dev_id, uint16_t queue_id,
                                         const uint16_t num_to_process,
  ...
  for (i = 0, dequeued = 0; dequeued < num_to_process; ++i) {
    ...
    do {
      deq +=
          rte_bbdev_dequeue_ldpc_enc_ops(dev_id, queue_id, &ops_deq[deq], enq);
    } while (unlikely(deq == 0));
...
static int offload_cost_test(struct active_device *ad,
                             struct test_op_params *op_params) {
  ...
  printf("+ ------------------------------------------------------- +\n");
  printf("== test: offload latency test\ndev: %s, burst size: %u, num ops: %u, "
         "op type: %s\n",
         info.dev_name, burst_sz, num_to_process, op_type_str);

  if (op_type == RTE_BBDEV_OP_TURBO_DEC)
    iter = offload_latency_test_dec(op_params->mp, bufs, op_params->ref_dec_op,
                                    ad->dev_id, queue_id, num_to_process,
                                    burst_sz, &time_st);
  else if (op_type == RTE_BBDEV_OP_TURBO_ENC)
    iter = offload_latency_test_enc(op_params->mp, bufs, op_params->ref_enc_op,
                                    ad->dev_id, queue_id, num_to_process,
                                    burst_sz, &time_st);
  else if (op_type == RTE_BBDEV_OP_LDPC_ENC)
    iter = offload_latency_test_ldpc_enc(
        op_params->mp, bufs, op_params->ref_enc_op, ad->dev_id, queue_id,
        num_to_process, burst_sz, &time_st);
  else if (op_type == RTE_BBDEV_OP_LDPC_DEC)
    iter = offload_latency_test_ldpc_dec(
        op_params->mp, bufs, op_params->ref_dec_op, ad->dev_id, queue_id,
        num_to_process, burst_sz, &time_st);
  else
    iter = offload_latency_test_enc(op_params->mp, bufs, op_params->ref_enc_op,
                                    ad->dev_id, queue_id, num_to_process,
  ...
  printf("Enqueue driver offload cost latency:\n"
         ...
         (double)(time_st.enq_sw_total_time * 1000000) / (double)iter /
             (double)rte_get_tsc_hz(),
         (double)time_st.enq_sw_min_time,
         (double)(time_st.enq_sw_min_time * 1000000) / rte_get_tsc_hz(),
         (double)time_st.enq_sw_max_time,
         (double)(time_st.enq_sw_max_time * 1000000) / rte_get_tsc_hz(),
         (double)time_st.enq_acc_total_time / (double)iter,
         (double)(time_st.enq_acc_total_time * 1000000) / (double)iter /
             (double)rte_get_tsc_hz(),
         (double)time_st.enq_acc_min_time,
         (double)(time_st.enq_acc_min_time * 1000000) / rte_get_tsc_hz(),
         (double)time_st.enq_acc_max_time,
         (double)(time_st.enq_acc_max_time * 1000000) / rte_get_tsc_hz());
  ...
  printf("Dequeue offload cost latency - one op:\n"
         ...
         (double)(time_st.deq_total_time * 1000000) / (double)iter /
             (double)rte_get_tsc_hz(),
         (double)time_st.deq_min_time,
         (double)(time_st.deq_min_time * 1000000) / rte_get_tsc_hz(),
         (double)time_st.deq_max_time,
         (double)(time_st.deq_max_time * 1000000) / rte_get_tsc_hz());
...
#ifdef RTE_BBDEV_OFFLOAD_COST
static int offload_latency_empty_q_test_dec(
    uint16_t dev_id, uint16_t queue_id, const uint16_t num_to_process,
    uint16_t burst_sz, uint64_t *deq_total_time, uint64_t *deq_min_time,
  ...
  for (i = 0, deq_total = 0; deq_total < num_to_process;
    ...
    if (op_type == RTE_BBDEV_OP_LDPC_DEC)
      rte_bbdev_dequeue_ldpc_dec_ops(dev_id, queue_id, ops, burst_sz);
    else
...
static int offload_latency_empty_q_test_enc(
    uint16_t dev_id, uint16_t queue_id, const uint16_t num_to_process,
    uint16_t burst_sz, uint64_t *deq_total_time, uint64_t *deq_min_time,
    uint64_t *deq_max_time, const enum rte_bbdev_op_type op_type) {
  int i, deq_total;
  ...
  for (i = 0, deq_total = 0; deq_total < num_to_process;
    ...
    if (op_type == RTE_BBDEV_OP_LDPC_ENC)
      rte_bbdev_dequeue_ldpc_enc_ops(dev_id, queue_id, ops, burst_sz);
    else
...
static int offload_latency_empty_q_test(struct active_device *ad,
                                        struct test_op_params *op_params) {
#ifndef RTE_BBDEV_OFFLOAD_COST
  RTE_SET_USED(ad);
  RTE_SET_USED(op_params);
  ...
  if (op_type == RTE_BBDEV_OP_TURBO_DEC || op_type == RTE_BBDEV_OP_LDPC_DEC)
    iter = offload_latency_empty_q_test_dec(
        ad->dev_id, queue_id, num_to_process, burst_sz, &deq_total_time,
        &deq_min_time, &deq_max_time, op_type);
  else
    iter = offload_latency_empty_q_test_enc(
        ad->dev_id, queue_id, num_to_process, burst_sz, &deq_total_time,
        &deq_min_time, &deq_max_time, op_type);
  ...
  printf("Empty dequeue offload:\n"
         ...
         (double)deq_max_time,
         (double)(deq_max_time * 1000000) / rte_get_tsc_hz());

  return TEST_SUCCESS;
#endif
}

static int bler_tc(void) { return run_test_case(bler_test); }

static int throughput_tc(void) { return run_test_case(throughput_test); }

static int offload_cost_tc(void) { return run_test_case(offload_cost_test); }

static int offload_latency_empty_q_tc(void) {
  return run_test_case(offload_latency_empty_q_test);
}

static int latency_tc(void) { return run_test_case(latency_test); }

static int validation_tc(void) { return run_test_case(validation_test); }

static int interrupt_tc(void) { return run_test_case(throughput_test); }
