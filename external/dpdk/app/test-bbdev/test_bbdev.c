...
static int ut_setup(void) {
  ...
  num_queues = 1;
  TEST_ASSERT_SUCCESS(
      rte_bbdev_setup_queues(null_dev_id, num_queues, SOCKET_ID_ANY),
      "Failed to setup queues for bbdev %u", 0);

  /* Start the device */
  TEST_ASSERT_SUCCESS(rte_bbdev_start(null_dev_id), "Failed to start bbdev %u",
                      0);

  return TEST_SUCCESS;
}

static void ut_teardown(void) { rte_bbdev_close(null_dev_id); }

static int test_bbdev_configure_invalid_dev_id(void) {
  uint8_t dev_id;
  uint8_t num_queues;
  ...
  for (dev_id = 0; dev_id < RTE_BBDEV_MAX_DEVS; dev_id++) {
    if (!rte_bbdev_is_valid(dev_id)) {
      TEST_ASSERT_FAIL(
          rte_bbdev_setup_queues(dev_id, num_queues, SOCKET_ID_ANY),
...
static int test_bbdev_configure_invalid_num_queues(void) {
  ...
  TEST_ASSERT_FAIL(rte_bbdev_setup_queues(dev_id, 0, SOCKET_ID_ANY),
                   "Failed test for rte_bbdev_setup_queues: "
                   "invalid num_queues %d",
                   0);

  TEST_ASSERT_SUCCESS(rte_bbdev_setup_queues(dev_id, num_queues, SOCKET_ID_ANY),
                      "Failed test for rte_bbdev_setup_queues: "
                      "invalid dev_num %u",
                      dev_id);
  ...
  TEST_ASSERT_SUCCESS(return_value = rte_bbdev_info_get(dev_id, &info),
                      "Failed test for rte_bbdev_info_get: "
                      "invalid return value:%i",
                      return_value);

  TEST_ASSERT(info.num_queues == num_queues,
              "Failed test for rte_bbdev_info_get: "
              "invalid num_queues:%u",
              info.num_queues);
  ...
  num_queues++;
  TEST_ASSERT_FAIL(rte_bbdev_setup_queues(dev_id, num_queues, SOCKET_ID_ANY),
                   "Failed test for rte_bbdev_setup_queues: "
                   "invalid num_queues: %u",
                   num_queues);
...
static int test_bbdev_configure_stop_device(void) {
  struct rte_bbdev_info info;
  uint8_t dev_id;
  int return_value;
  ...
  TEST_ASSERT_SUCCESS(return_value = rte_bbdev_info_get(dev_id, &info),
                      ...
                      "rte_bbdev_info_get function: %i",
                      return_value);

  TEST_ASSERT_SUCCESS(info.started,
                      "Failed test for rte_bbdev_info_get: "
                      "started value: %u",
                      info.started);

  TEST_ASSERT_SUCCESS(
      rte_bbdev_setup_queues(dev_id, info.drv.max_num_queues, SOCKET_ID_ANY),
      "Failed test for rte_bbdev_setup_queues: "
      "device should be stopped, dev_id: %u",
      dev_id);

  return_value = rte_bbdev_intr_enable(dev_id);
  TEST_ASSERT(return_value != -EBUSY,
              "Failed test for rte_bbdev_intr_enable: device should be "
              "stopped, dev_id: %u",
              dev_id);
  ...
  TEST_ASSERT_FAIL(
      rte_bbdev_setup_queues(dev_id, info.drv.max_num_queues, SOCKET_ID_ANY),
      "Failed test for rte_bbdev_setup_queues: "
      "device should be started, dev_id: %u",
      dev_id);
...
static int test_bbdev_configure_stop_queue(void) {
  struct bbdev_testsuite_params *ts_params = &testsuite_params;
  struct rte_bbdev_info info;
  ...
  TEST_ASSERT(qinfo.conf.deferred_start == ts_params->qconf.deferred_start,
              "Failed test for rte_bbdev_queue_info_get: "
              "invalid queue_size:%u",
              qinfo.conf.deferred_start);
  ...
  TEST_ASSERT(qinfo.started == 0,
              "Failed test for rte_bbdev_queue_info_get: "
              "invalid value for qinfo.started:%u",
              qinfo.started);
...
static int test_bbdev_configure_invalid_queue_configure(void) {
  struct bbdev_testsuite_params *ts_params = &testsuite_params;
  int return_value;
  struct rte_bbdev_info info;
  uint8_t dev_id;
...
static int test_bbdev_op_pool(void) {
  ...
  TEST_ASSERT(rte_bbdev_op_pool_create(NULL, RTE_BBDEV_OP_TURBO_DEC, size,
                                       cache_size, 0) == NULL,
              "Failed test for rte_bbdev_op_pool_create: "
              "NULL name parameter");

  TEST_ASSERT((mp = rte_bbdev_op_pool_create(pool_dec, RTE_BBDEV_OP_TURBO_DEC,
                                             size, cache_size, 0)) != NULL,
              "Failed test for rte_bbdev_op_pool_create: "
  ...
  TEST_ASSERT_SUCCESS(strcmp(mp->name, pool_dec),
                      "Failed test for rte_bbdev_op_pool_create: "
                      "invalid name of mempool, mp->name: %s",
                      mp->name);
  ...
  TEST_ASSERT((mp = rte_bbdev_op_pool_create(pool_enc, RTE_BBDEV_OP_TURBO_ENC,
                                             size, cache_size, 0)) != NULL,
              "Failed test for rte_bbdev_op_pool_create: "
              "returned value is empty");
  ...
  TEST_ASSERT((mp = rte_bbdev_op_pool_create("Test_NONE", RTE_BBDEV_OP_NONE,
                                             size, cache_size, 0)) != NULL,
              "Failed test for rte_bbdev_op_pool_create: "
  ...
  TEST_ASSERT(
      (mp = rte_bbdev_op_pool_create("Test_INV", RTE_BBDEV_OP_TYPE_COUNT, size,
                                     cache_size, 0)) == NULL,
      "Failed test for rte_bbdev_op_pool_create: "
  ...
  TEST_ASSERT((mp = rte_bbdev_op_pool_create("Test_InvSize", RTE_BBDEV_OP_NONE,
                                             size, cache_size, 0)) == NULL,
              "Failed test for rte_bbdev_op_pool_create: "
 ...
 *  allocated
 */
static int test_bbdev_op_type(void) {
  ...
  mp_dec = rte_bbdev_op_pool_create(pool_dec, RTE_BBDEV_OP_TURBO_DEC,
...
static int test_bbdev_op_pool_size(void) {
  ...
  /* Add 128 RTE_BBDEV_OP_TURBO_ENC ops */
  TEST_ASSERT(rte_bbdev_enc_op_alloc_bulk(mp_none, ops_ext_arr, OPS_COUNT) == 0,
  ...
  TEST_ASSERT(rte_bbdev_enc_op_alloc_bulk(mp_none, ops_ext2_arr, OPS_COUNT) !=
                  0,
              "Failed test for allocating bbdev ops: "
...
static int test_bbdev_count(void) {
...
static int test_bbdev_stats(void) {
  ...
  TEST_ASSERT_SUCCESS(
      rte_bbdev_queue_configure(dev_id, queue_id, &ts_params->qconf),
      "Failed to configure queue %u on device %u ", queue_id, dev_id);
  ...
  TEST_ASSERT_FAIL(rte_bbdev_stats_get(RTE_BBDEV_MAX_DEVS, &stats),
                   "Failed test for rte_bbdev_stats_get on device %u ", dev_id);
...
static int test_bbdev_driver_init(void) {
  ...
  TEST_ASSERT(dev1 == NULL, "Failed initialize bbdev driver with NULL name");
  ...
  do {
    ...
    TEST_ASSERT(dev2 != NULL, "Failed to initialize bbdev driver");
    ++num_devs;
  ...
  TEST_ASSERT(dev2 == NULL,
              ...
              "more drivers than RTE_BBDEV_MAX_DEVS: %d ",
              num_devs, RTE_BBDEV_MAX_DEVS);
  ...
  while (num_devs >= num_devs_tmp) {
    ...
    TEST_ASSERT_SUCCESS(rte_bbdev_release(dev2),
                        "Failed to uninitialize bbdev driver %s ", name_tmp);
    num_devs--;
  }
  ...
  TEST_ASSERT(
      dev1->state == RTE_BBDEV_INITIALIZED,
      "Failed test rte_bbdev_allocate: "
...
static void event_callback(uint16_t dev_id, enum rte_bbdev_event_type type,
  ...
  if (type == RTE_BBDEV_EVENT_UNKNOWN || type == RTE_BBDEV_EVENT_ERROR ||
      type == RTE_BBDEV_EVENT_MAX)
    *(int *)param = type;
}

static int test_bbdev_callback(void) {
  struct rte_bbdev *dev1, *dev2;
   ...
   */
  TEST_ASSERT_SUCCESS(
      rte_bbdev_callback_register(dev1->data->dev_id, RTE_BBDEV_EVENT_UNKNOWN,
   ...
   */
  TEST_ASSERT_SUCCESS(
      rte_bbdev_callback_register(dev1->data->dev_id, RTE_BBDEV_EVENT_ERROR,
                                  event_callback, &event_status),
   ...
   */
  TEST_ASSERT_SUCCESS(
      rte_bbdev_callback_unregister(dev1->data->dev_id, RTE_BBDEV_EVENT_ERROR,
                                    event_callback, &event_status),
   ...
   */
  TEST_ASSERT_SUCCESS(
      rte_bbdev_callback_register(dev2->data->dev_id, RTE_BBDEV_EVENT_ERROR,
                                  event_callback, &event_status),
      "Failed to callback rgstr for RTE_BBDEV_EVENT_ERROR");
   ...
   */
  TEST_ASSERT_SUCCESS(
      rte_bbdev_callback_register(dev2->data->dev_id, RTE_BBDEV_EVENT_UNKNOWN,
                                  event_callback, &event_status),
  ...
  TEST_ASSERT_SUCCESS(
      rte_bbdev_callback_unregister(dev2->data->dev_id, RTE_BBDEV_EVENT_UNKNOWN,
                                    event_callback, &event_status),
  ...
  TEST_ASSERT_FAIL(rte_bbdev_callback_register(dev_id, RTE_BBDEV_EVENT_ERROR,
                                               event_callback, &event_status),
                   "Failed test for rte_bbdev_callback_register: "
                   "function called after rte_bbdev_driver_uninit .");

  TEST_ASSERT_FAIL(rte_bbdev_callback_unregister(dev_id, RTE_BBDEV_EVENT_ERROR,
                                                 event_callback, &event_status),
                   "Failed test for rte_dev_callback_unregister: "
                   "function called after rte_bbdev_driver_uninit. ");
...
static int test_bbdev_invalid_driver(void) {
  struct rte_bbdev dev1, *dev2;
  uint8_t dev_id = null_dev_id;
  uint16_t queue_id = 0;
  struct rte_bbdev_stats stats;
  ...
  TEST_ASSERT_SUCCESS(rte_bbdev_setup_queues(dev_id, 1, SOCKET_ID_ANY),
  ...
  TEST_ASSERT_FAIL(rte_bbdev_queue_configure(RTE_BBDEV_MAX_DEVS, queue_id,
                                             &ts_params->qconf),
                   "Failed to configure queue %u on device %u ", queue_id,
                   dev_id);
  ...
  TEST_ASSERT_FAIL(
      rte_bbdev_queue_info_get(RTE_BBDEV_MAX_DEVS, queue_id, &qinfo),
      "Failed test for rte_bbdev_info_get: "
      "invalid dev_id ");

  TEST_ASSERT_FAIL(
      rte_bbdev_queue_info_get(dev_id, RTE_MAX_QUEUES_PER_PORT, &qinfo),
      "Failed test for rte_bbdev_info_get: "
      "invalid queue_id ");
  ...
  TEST_ASSERT_SUCCESS(rte_bbdev_close(dev_id), "Failed to close bbdev %u ",
                      dev_id);

  return TEST_SUCCESS;
}

static int test_bbdev_get_named_dev(void) {
...
static struct unit_test_suite bbdev_null_testsuite = {
    ...
    .unit_test_cases = {
        ...
        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_configure_stop_device),

        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_configure_stop_queue),

        TEST_CASE_ST(ut_setup, ut_teardown,
                     test_bbdev_configure_invalid_queue_configure),

        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_op_pool),

        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_op_type),

        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_op_pool_size),

        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_stats),

        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_driver_init),

        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_callback),

        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_invalid_driver),

        TEST_CASE_ST(ut_setup, ut_teardown, test_bbdev_get_named_dev),
