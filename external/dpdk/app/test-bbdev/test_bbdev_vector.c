...
const char *op_data_prefixes[] = {
    "input", "soft_output", "hard_output", "harq_input", "harq_output",
};
...
static int parse_values(char *tokens, uint32_t **data, uint32_t *data_length) {
  ...
  values = (uint32_t *)rte_zmalloc(NULL, sizeof(uint32_t) * data_size, 0);
  if (values == NULL)
    return -1;
  ...
  while (tok != NULL) {
    ...
    if (n_tokens >= data_size) {
      ...
      values_resized =
          (uint32_t *)rte_realloc(values, sizeof(uint32_t) * data_size, 0);
  ...
  values_resized =
      (uint32_t *)rte_realloc(values, sizeof(uint32_t) * (n_tokens + 1), 0);
...
static int op_decoder_flag_strtoul(char *token, uint32_t *op_flag_value) {
...
static int op_ldpc_decoder_flag_strtoul(char *token, uint32_t *op_flag_value) {
  ...
  else if (!strcmp(token, "RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_IN_ENABLE"))
    *op_flag_value = RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_IN_ENABLE;
  else if (!strcmp(token, "RTE_BBDEV_LDPC_INTERNAL_HARQ_MEMORY_OUT_ENABLE"))
...
static int op_encoder_flag_strtoul(char *token, uint32_t *op_flag_value) {
...
static int op_ldpc_encoder_flag_strtoul(char *token, uint32_t *op_flag_value) {
...
static int parse_turbo_flags(char *tokens, uint32_t *op_flags,
  ...
  while (tok != NULL) {
    ...
    if (op_type == RTE_BBDEV_OP_TURBO_DEC) {
      ...
      if (op_ldpc_encoder_flag_strtoul(tok, &op_flag_value) == -1)
        return -1;
    } else if (op_type == RTE_BBDEV_OP_LDPC_DEC) {
      if (op_ldpc_decoder_flag_strtoul(tok, &op_flag_value) == -1)
        return -1;
    } else {
      return -1;
    }
...
static int op_turbo_type_strtol(char *token, enum rte_bbdev_op_type *op_type) {
  trim_space(token);
...
static int parse_expected_status(char *tokens, int *status,
                                 enum rte_bbdev_op_type op_type) {
  char *tok = NULL;
      ...
      *status = *status | (1 << RTE_BBDEV_SYNDROME_ERROR);
      ...
      if ((op_type == RTE_BBDEV_OP_TURBO_DEC) ||
        ...
        *status = *status | (1 << RTE_BBDEV_CRC_ERROR);
        ...
        printf("CRC is only a valid value for decoder\n");
        return -1;
      }
    } else {
 ...
 */
static int parse_data_entry(const char *key_token, char *token,
                            struct test_bbdev_vector *vector,
                            enum op_data_type type, const char *prefix) {
  int ret;
  ...
  if (id != *nb_ops) {
    printf("Please order data entries sequentially, i.e. %s0, %s1, ...\n",
           prefix, prefix);
    return -1;
  }
...
static int parse_decoder_params(const char *key_token, char *token,
                                struct test_bbdev_vector *vector) {
  int ret = 0, status = 0;
  uint32_t op_flags = 0;
  char *err = NULL;
...
static int parse_encoder_params(const char *key_token, char *token,
                                struct test_bbdev_vector *vector) {
  int ret = 0, status = 0;
  uint32_t op_flags = 0;
  char *err = NULL;
  ...
  if (starts_with(key_token, op_data_prefixes[DATA_INPUT]))
    ...
    ret =
        parse_data_entry(key_token, token, vector, DATA_HARD_OUTPUT, "output");
  else if (!strcmp(key_token, "e")) {
    vector->mask |= TEST_BBDEV_VF_E;
    ...
    turbo_enc->tb_params.ncb_neg = (uint16_t)strtoul(token, &err, 0);
    ret = ((err == NULL) || (*err != '\0')) ? -1 : 0;
  } else if (!strcmp(key_token, "ncb_pos")) {
    vector->mask |= TEST_BBDEV_VF_NCB_POS;
    turbo_enc->tb_params.ncb_pos = (uint16_t)strtoul(token, &err, 0);
    ret = ((err == NULL) || (*err != '\0')) ? -1 : 0;
  } else if (!strcmp(key_token, "r")) {
    vector->mask |= TEST_BBDEV_VF_R;
...
static int parse_ldpc_encoder_params(const char *key_token, char *token,
                                     struct test_bbdev_vector *vector) {
  int ret = 0, status = 0;
  uint32_t op_flags = 0;
  char *err = NULL;
...
static int parse_ldpc_decoder_params(const char *key_token, char *token,
                                     struct test_bbdev_vector *vector) {
  int ret = 0, status = 0;
  uint32_t op_flags = 0;
  char *err = NULL;
  ...
  if (starts_with(key_token, op_data_prefixes[DATA_INPUT]))
    ...
    ret = parse_data_entry(key_token, token, vector, DATA_HARQ_INPUT,
                           op_data_prefixes[DATA_HARQ_INPUT]);
  else if (starts_with(key_token, op_data_prefixes[DATA_HARQ_OUTPUT]))
    ret = parse_data_entry(key_token, token, vector, DATA_HARQ_OUTPUT,
...
static int parse_entry(char *entry, struct test_bbdev_vector *vector) {
  int ret = 0;
...
static int check_decoder_segments(struct test_bbdev_vector *vector) {
  unsigned char i;
  struct rte_bbdev_op_turbo_dec *turbo_dec = &vector->turbo_dec;
...
static int check_ldpc_decoder_segments(struct test_bbdev_vector *vector) {
  unsigned char i;
  struct rte_bbdev_op_ldpc_dec *ldpc_dec = &vector->ldpc_dec;
...
static int check_decoder_llr_spec(struct test_bbdev_vector *vector) {
  struct rte_bbdev_op_turbo_dec *turbo_dec = &vector->turbo_dec;
  ...
  if ((turbo_dec->op_flags & RTE_BBDEV_TURBO_POS_LLR_1_BIT_IN) &&
    ...
    printf("Both positive and negative LLR input flags were set!\n");
    return -1;
  }
  if (!(turbo_dec->op_flags & RTE_BBDEV_TURBO_POS_LLR_1_BIT_IN) &&
      !(turbo_dec->op_flags & RTE_BBDEV_TURBO_NEG_LLR_1_BIT_IN)) {
    printf("INFO: input LLR sign formalism was not specified and will be set "
           "to negative LLR for '1' bit\n");
    turbo_dec->op_flags |= RTE_BBDEV_TURBO_NEG_LLR_1_BIT_IN;
  }
  ...
  if (!(turbo_dec->op_flags & RTE_BBDEV_TURBO_POS_LLR_1_BIT_SOFT_OUT) &&
      !(turbo_dec->op_flags & RTE_BBDEV_TURBO_NEG_LLR_1_BIT_SOFT_OUT)) {
    printf("INFO: soft output LLR sign formalism was not specified and will be "
           "set to negative LLR for '1' bit\n");
    turbo_dec->op_flags |= RTE_BBDEV_TURBO_NEG_LLR_1_BIT_SOFT_OUT;
  }
...
static int check_decoder_op_flags(struct test_bbdev_vector *vector) {
  struct rte_bbdev_op_turbo_dec *turbo_dec = &vector->turbo_dec;

  if ((turbo_dec->op_flags & RTE_BBDEV_TURBO_DEC_TB_CRC_24B_KEEP) &&
      !(turbo_dec->op_flags & RTE_BBDEV_TURBO_CRC_TYPE_24B)) {
    printf("WARNING: RTE_BBDEV_TURBO_DEC_TB_CRC_24B_KEEP flag is missing "
...
static int check_decoder(struct test_bbdev_vector *vector) {
  struct rte_bbdev_op_turbo_dec *turbo_dec = &vector->turbo_dec;
  ...
  if (!(mask & TEST_BBDEV_VF_CODE_BLOCK_MODE)) {
    printf("WARNING: code_block_mode was not specified in vector file and will "
           "be set to 1 (0 - TB Mode, 1 - CB mode)\n");
  ...
  if (turbo_dec->code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
    if (!(mask & TEST_BBDEV_VF_EA))
      printf("WARNING: ea was not specified in vector file and will be set to "
             "0\n");
    if (!(mask & TEST_BBDEV_VF_EB))
      printf("WARNING: eb was not specified in vector file and will be set to "
             "0\n");
    if (!(mask & TEST_BBDEV_VF_K_NEG))
      printf("WARNING: k_neg was not specified in vector file and will be set "
             "to 0\n");
    if (!(mask & TEST_BBDEV_VF_K_POS))
      printf("WARNING: k_pos was not specified in vector file and will be set "
             "to 0\n");
    if (!(mask & TEST_BBDEV_VF_C_NEG))
      printf("WARNING: c_neg was not specified in vector file and will be set "
             "to 0\n");
    ...
    if (!(mask & TEST_BBDEV_VF_CAB))
      printf("WARNING: cab was not specified in vector file and will be set to "
             "0\n");
  ...
  if (!(mask & TEST_BBDEV_VF_RV_INDEX))
    printf("INFO: rv_index was not specified in vector file and will be set to "
           "0\n");
  if (!(mask & TEST_BBDEV_VF_ITER_MIN))
    printf("WARNING: iter_min was not specified in vector file and will be set "
           "to 0\n");
  if (!(mask & TEST_BBDEV_VF_ITER_MAX))
    printf("WARNING: iter_max was not specified in vector file and will be set "
           "to 0\n");
  if (!(mask & TEST_BBDEV_VF_EXPECTED_ITER_COUNT))
    printf("WARNING: expected_iter_count was not specified in vector file and "
           "iter_count will not be validated\n");
  if (!(mask & TEST_BBDEV_VF_EXT_SCALE))
    printf("WARNING: ext_scale was not specified in vector file and will be "
           "set to 0\n");
  if (!(mask & TEST_BBDEV_VF_OP_FLAGS)) {
    printf("WARNING: op_flags was not specified in vector file and "
    ...
    printf("INFO: RTE_BBDEV_TURBO_MAP_DEC was not set in vector file and "
           "num_maps will be set to 0\n");
    turbo_dec->num_maps = 0;
  }
  if (!(mask & TEST_BBDEV_VF_EXPECTED_STATUS))
    printf("WARNING: expected_status was not specified in vector file and will "
           "be set to 0\n");
  return 0;
}

/* checks LDPC decoder parameters */
static int check_ldpc_decoder(struct test_bbdev_vector *vector) {
  struct rte_bbdev_op_ldpc_dec *ldpc_dec = &vector->ldpc_dec;
  const int mask = vector->mask;
...
static int check_encoder(struct test_bbdev_vector *vector) {
  unsigned char i;
  const int mask = vector->mask;
  ...
  if (vector->turbo_enc.code_block_mode == RTE_BBDEV_TRANSPORT_BLOCK) {
    if (!(mask & TEST_BBDEV_VF_EA) &&
        (vector->turbo_enc.op_flags & RTE_BBDEV_TURBO_RATE_MATCH))
      printf("WARNING: ea was not specified in vector file and will be set to "
             "0\n");
    if (!(mask & TEST_BBDEV_VF_EB) &&
        (vector->turbo_enc.op_flags & RTE_BBDEV_TURBO_RATE_MATCH))
    ...
    if (!(mask & TEST_BBDEV_VF_NCB_NEG))
      printf("WARNING: ncb_neg was not specified in vector file and will be "
             "set to 0\n");
    if (!(mask & TEST_BBDEV_VF_NCB_POS))
      printf("WARNING: ncb_pos was not specified in vector file and will be "
             "set to 0\n");
    if (!(mask & TEST_BBDEV_VF_R))
      printf(
          "WARNING: r was not specified in vector file and will be set to 0\n");
    ...
    if (!(mask & TEST_BBDEV_VF_NCB))
      printf("WARNING: ncb was not specified in vector file and will be set to "
             "0\n");
  }
  if (!(mask & TEST_BBDEV_VF_RV_INDEX))
    printf("INFO: rv_index was not specified in vector file and will be set to "
           "0\n");
  if (!(mask & TEST_BBDEV_VF_OP_FLAGS))
    printf("INFO: op_flags was not specified in vector file and capabilities "
...
/* checks encoder parameters */
static int check_ldpc_encoder(struct test_bbdev_vector *vector) {
  unsigned char i;
  const int mask = vector->mask;
...
static int bbdev_check_vector(struct test_bbdev_vector *vector) {
  if (vector->op_type == RTE_BBDEV_OP_TURBO_DEC) {
...
int test_bbdev_vector_read(const char *filename,
                           struct test_bbdev_vector *vector) {
  int ret = 0;
  size_t len = 0;
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    if (entry[strlen(entry) - 1] == ',' || entry[strlen(entry) - 1] == '=') {
      while (getline(&line, &len, fp) != -1) {
        trim_space(line);

        /* extend entry about length of new line */
        char *entry_extended = realloc(entry, strlen(line) + strlen(entry) + 1);

        if (entry_extended == NULL) {
          printf("Fail to allocate %zu bytes\n",
                 strlen(line) + strlen(entry) + 1);
          ret = -ENOMEM;
          goto exit;
        }
