...
unsigned int get_num_ops(void) { return test_params.num_ops; }

unsigned int get_burst_sz(void) { return test_params.burst_sz; }

unsigned int get_num_lcores(void) { return test_params.num_lcores; }

double get_snr(void) { return test_params.snr; }

unsigned int get_iter_max(void) { return test_params.iter_max; }

bool get_init_device(void) { return test_params.init_device; }
...
static int parse_args(int argc, char **argv, struct test_params *tp) {
  ...
  static struct option lgopts[] = {
      {"num-ops", 1, 0, 'n'},    {"burst-size", 1, 0, 'b'},
      {"test-cases", 1, 0, 'c'}, {"test-vector", 1, 0, 'v'},
      {"lcores", 1, 0, 'l'},     {"snr", 1, 0, 's'},
      {"iter_max", 6, 0, 't'},   {"init-device", 0, 0, 'i'},
  ...
  while ((opt = getopt_long(argc, argv, "hin:b:c:v:l:s:t:", lgopts,
    ...
    switch (opt) {
    case 'n':
      TEST_ASSERT(strlen(optarg) > 0, "Num of operations is not provided");
      tp->num_ops = strtol(optarg, NULL, 10);
      break;
    case 'b':
      TEST_ASSERT(strlen(optarg) > 0, "Burst size is not provided");
      tp->burst_sz = strtol(optarg, NULL, 10);
      TEST_ASSERT(tp->burst_sz <= MAX_BURST,
                  "Burst size mustn't be greater than %u", MAX_BURST);
      break;
    case 'c':
      ...
      ret = rte_strsplit(optarg, strlen(optarg), tokens, MAX_CMDLINE_TESTCASES,
                         tc_sep);

      TEST_ASSERT(ret <= MAX_CMDLINE_TESTCASES, "Too many test cases (max=%d)",
      ...
      for (tc = 0; tc < ret; ++tc) {
        ...
        TEST_ASSERT(tp->test_to_run[num_tests] != NULL, "Unknown test case: %s",
                    tokens[tc]);
        ++num_tests;
      }
      break;
    case 'v':
      ...
      TEST_ASSERT(strlen(optarg) > 0, "Config file name is null");

      snprintf(tp->test_vector_filename, sizeof(tp->test_vector_filename), "%s",
               optarg);
      break;
    case 's':
      TEST_ASSERT(strlen(optarg) > 0, "SNR is not provided");
      tp->snr = strtod(optarg, NULL);
      break;
    case 't':
      TEST_ASSERT(strlen(optarg) > 0, "Iter_max is not provided");
      tp->iter_max = strtol(optarg, NULL, 10);
      break;
    case 'l':
      TEST_ASSERT(strlen(optarg) > 0, "Num of lcores is not provided");
      tp->num_lcores = strtol(optarg, NULL, 10);
      TEST_ASSERT(tp->num_lcores <= RTE_MAX_LCORE,
                  "Num of lcores mustn't be greater than %u", RTE_MAX_LCORE);
      break;
    case 'i':
  ...
  if (tp->num_ops == 0) {
    printf("WARNING: Num of operations was not provided or was set 0. Set to "
           "default (%u)\n",
           DEFAULT_OPS);
    tp->num_ops = DEFAULT_OPS;
  }
  if (tp->burst_sz == 0) {
    printf("WARNING: Burst size was not provided or was set 0. Set to default "
           "(%u)\n",
           DEFAULT_BURST);
    tp->burst_sz = DEFAULT_BURST;
  }
  if (tp->num_lcores == 0) {
    printf("WARNING: Num of lcores was not provided or was set 0. Set to value "
           "from RTE config (%u)\n",
           rte_lcore_count());
...
static int run_parsed_tests(struct test_params *tp) {
  int ret = TEST_SUCCESS;
  unsigned int i;
