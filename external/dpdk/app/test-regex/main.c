...
static void args_parse(int argc, char **argv, char *rules_file, char *data_file,
                       uint32_t *nb_jobs, bool *perf_mode,
                       uint32_t *nb_iterations, uint32_t *nb_qps,
                       uint32_t *nb_lcores, uint32_t *nb_segs) {
  char **argvopt;
  int opt;
  int opt_idx;
  size_t len;
  ...
  while ((opt = getopt_long(argc, argvopt, "", lgopts, &opt_idx)) != EOF) {
    ...
    case ARG_RULES_FILE_NAME:
      ...
      if (len == MAX_FILE_NAME)
        rte_exit(EXIT_FAILURE, "Rule file name to long max %d\n",
                 MAX_FILE_NAME - 1);
    ...
    case ARG_DATA_FILE_NAME:
      len = strnlen(optarg, MAX_FILE_NAME - 1);
      if (len == MAX_FILE_NAME)
        rte_exit(EXIT_FAILURE, "Data file name to long max %d\n",
                 MAX_FILE_NAME - 1);
...
static long read_file(char *file, char **buf) {
  FILE *fp;
  long buf_len = 0;
  size_t read_len;
  int res = 0;
...
static int clone_buf(char *data_buf, char **buf, long data_len) {
  char *dest_buf;
  dest_buf = rte_malloc(NULL, sizeof(char) * (data_len + 1), 4096);
  if (!dest_buf)
    return -ENOMEM;
...
static int init_port(uint16_t *nb_max_payload, char *rules_file,
...
static void extbuf_free_cb(void *addr __rte_unused,
                           void *fcb_opaque __rte_unused) {}
...
regex_create_segmented_mbuf(struct rte_mempool *mbuf_pool, int pkt_len,
  ...
  t_len = pkt_len >= nb_segs ? (pkt_len / nb_segs + !!(pkt_len % nb_segs)) : 1;
  size = pkt_len;
...
static int run_regex(void *args) {
  ...
  snprintf(mbuf_pool, sizeof(mbuf_pool), "mbuf_pool_%2u", qp_id_base);
  mbuf_mp = rte_pktmbuf_pool_create(
      mbuf_pool, rte_align32pow2(nb_jobs * nb_qps * nb_segs), 0, 0,
      (nb_segs == 1)
          ? MBUF_SIZE
          : (rte_align32pow2(job_len) / nb_segs + RTE_PKTMBUF_HEADROOM),
      rte_socket_id());
  if (mbuf_mp == NULL) {
  ...
  for (qp_id = 0; qp_id < nb_qps; qp_id++) {
    ...
    qp->jobs_ctx = jobs_ctx = rte_malloc(NULL, sizeof(*jobs_ctx) * nb_jobs, 0);
    if (!jobs_ctx) {
    ...
    for (i = 0; (pos < data_len) && (i < nb_jobs); i++) {
      ...
      ops[i] = rte_malloc(
          NULL,
          sizeof(*ops[0]) + nb_max_matches * sizeof(struct rte_regexdev_match),
          0);
      if (!ops[i]) {
      ...
      if (nb_segs > 1) {
        ops[i]->mbuf = regex_create_segmented_mbuf(mbuf_mp, act_job_len,
                                                   nb_segs, &buf[pos]);
      } else {
        ops[i]->mbuf = rte_pktmbuf_alloc(mbuf_mp);
        if (ops[i]->mbuf) {
          rte_pktmbuf_attach_extbuf(ops[i]->mbuf, &buf[pos], 0, act_job_len,
                                    &shinfo);
    ...
    do {
      ...
      for (qp_id = 0; qp_id < nb_qps; qp_id++) {
        ...
        if (qp->total_dequeue < actual_jobs) {
          ...
          struct rte_regex_ops **cur_ops_to_enqueue =
              qp->ops + qp->total_enqueue;

          if (actual_jobs - qp->total_enqueue)
            qp->total_enqueue += rte_regexdev_enqueue_burst(
                dev_id, qp_id_base + qp_id, cur_ops_to_enqueue,
                actual_jobs - qp->total_enqueue);
        }
      }
      for (qp_id = 0; qp_id < nb_qps; qp_id++) {
        qp = &qps[qp_id];
        if (qp->total_dequeue < actual_jobs) {
          struct rte_regex_ops **cur_ops_to_dequeue =
              qp->ops + qp->total_dequeue;

          qp->total_dequeue += rte_regexdev_dequeue_burst(
              dev_id, qp_id_base + qp_id, cur_ops_to_dequeue,
              qp->total_enqueue - qp->total_dequeue);
          qp->cycles += (rte_rdtsc_precise() - qp->start);
          update = true;
        }
      }
    } while (update);
  ...
  for (qp_id = 0; qp_id < nb_qps; qp_id++) {
    ...
    printf("Core=%u QP=%u Job=%ld Bytes Time=%Lf sec Perf=%Lf "
           "Gbps\n",
           rte_lcore_id(), qp_id + qp_id_base, job_len, time,
    ...
    for (d_ind = 0; d_ind < qp->total_dequeue; d_ind++) {
      ...
      for (i = 0; i < nb_matches; i++) {
        printf("match %d, rule = %d, "
               "start = %d,len = %d\n",
               i, match->rule_id, match->start_offset, match->len);
        match++;
      }
    }
...
static int distribute_qps_to_lcores(uint32_t nb_cores, uint32_t nb_qps,
  ...
  qps_per_lcore = rte_malloc(NULL, sizeof(*qps_per_lcore) * nb_cores, 0);
  if (!qps_per_lcore)
  ...
  *qpl = qps_per_lcore;
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    if (nb_qps % nb_cores > detected_lcores)
      max_qp_id++;
    qps_per_lcore[detected_lcores].nb_qps = max_qp_id - min_qp_id + 1;
  ...
  for (i = 0; i < detected_lcores; i++) {
    printf("===> Core %d: allocated queues: ", qps_per_lcore[i].lcore_id);
    min_qp_id = qps_per_lcore[i].qp_id_base;
    max_qp_id = qps_per_lcore[i].qp_id_base + qps_per_lcore[i].nb_qps;
...
int main(int argc, char **argv) {
  ...
  argv += ret;
  if (argc > 1)
    args_parse(argc, argv, rules_file, data_file, &nb_jobs, &perf_mode,
               &nb_iterations, &nb_qps, &nb_lcores, &nb_segs);
  ...
  ret = init_port(&nb_max_payload, rules_file, &nb_max_matches, nb_qps);
  if (ret < 0)
  ...
  for (i = 0; i < nb_lcores; i++) {
    ...
    rte_eal_remote_launch(run_regex, &rgxc[i], qps_per_lcore[i].lcore_id);
  }
