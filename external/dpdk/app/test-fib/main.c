...
enum { RT_PREFIX, RT_NEXTHOP, RT_NUM };
...
#define NIPQUAD6(addr)                                                         \
  ((uint8_t *)addr)[0] << 8 | ((uint8_t *)addr)[1],                            \
      ((uint8_t *)addr)[2] << 8 | ((uint8_t *)addr)[3],                        \
      ((uint8_t *)addr)[4] << 8 | ((uint8_t *)addr)[5],                        \
      ((uint8_t *)addr)[6] << 8 | ((uint8_t *)addr)[7],                        \
      ((uint8_t *)addr)[8] << 8 | ((uint8_t *)addr)[9],                        \
      ((uint8_t *)addr)[10] << 8 | ((uint8_t *)addr)[11],                      \
      ((uint8_t *)addr)[12] << 8 | ((uint8_t *)addr)[13],                      \
      ((uint8_t *)addr)[14] << 8 | ((uint8_t *)addr)[15]
#endif
...
static uint64_t get_rnd_rng(uint64_t l, uint64_t u) {
  if (l == u)
    return l;
  else
...
get_max_nh(uint8_t nh_sz) {
  ...
  return RTE_MIN(((1ULL << (bits_in_nh(nh_sz) - 1)) - 1), (1ULL << 21) - 1);
}

static int get_fib_type(void) {
...
static int complete_distrib(uint8_t depth_lim, const uint32_t n, uint8_t rpd[],
                            uint32_t nrpd[]) {
  uint8_t depth;
  uint32_t nr = 0;
  uint8_t m = 0;
  ...
  for (depth = 0; depth <= depth_lim; depth++) {
    if (rpd[depth] != 0) {
      if (rpd[depth] == UINT8_MAX)
        config.nb_routes_per_depth[depth] = nrpd[depth];
      else
        config.nb_routes_per_depth[depth] = (n * rpd[depth]) / 100;
  ...
  for (depth = 0; depth <= depth_lim; depth++) {
    if (rpd[depth] == 0) {
      ...
      uint64_t max_routes_per_depth = 1ULL << RTE_MIN(depth, 63);
      uint32_t avg_routes_left = (n - nr) / (depth_lim + 1 - m++);
...
static int parse_distrib(uint8_t depth_lim, const uint32_t n) {
...
static void shuffle_rt_4(struct rt_rule_4 *rt, int n) {
...
static void shuffle_rt_6(struct rt_rule_6 *rt, int n) {
...
static void gen_random_rt_4(struct rt_rule_4 *rt, int nh_sz) {
  uint32_t i, j, k = 0;
  ...
  for (i = 1; i <= 32; i++) {
    ...
    for (j = 0; j < config.nb_routes_per_depth[i]; j++, k++, edge += step) {
      uint64_t rnd_val = get_rnd_rng((uint64_t)edge, (uint64_t)(edge + step));
...
static void complete_v6_addr(uint32_t *addr, uint32_t rnd, int n) {
  int i;
...
static void gen_random_rt_6(struct rt_rule_6 *rt, int nh_sz) {
  uint32_t a, i, j, k = 0;
  ...
  for (a = 0; a < 4; a++) {
    for (i = 1; i <= 32; i++) {
      ...
      double edge = 0;
      double step =
          (double)(1ULL << i) / config.nb_routes_per_depth[(a * 32) + i];
      for (j = 0; j < config.nb_routes_per_depth[a * 32 + i];
           j++, k++, edge += step) {
        uint64_t rnd_val = get_rnd_rng((uint64_t)edge, (uint64_t)(edge + step));
        rnd = rte_cpu_to_be_32(rnd_val << (32 - i));
        complete_v6_addr((uint32_t *)rt[k].addr, rnd, a);
        rt[k].depth = (a * 32) + i;
        rt[k].nh = rte_rand() & get_max_nh(nh_sz);
      }
    }
...
static inline void set_rnd_ipv6(uint8_t *addr, uint8_t *route, int depth) {
  int i;
...
static void gen_rnd_lookup_tbl(int af) {
  ...
  if (af == AF_INET) {
    for (i = 0, j = 0; i < config.nb_lookup_ips;
      ...
      if ((rte_rand() % 100) < config.rnd_lookup_ips_ratio) {
        ...
        tbl4[i] =
            rt4[j].addr | (rte_rand() & ((1ULL << (32 - rt4[j].depth)) - 1));
    }
  } else {
    for (i = 0, j = 0; i < config.nb_lookup_ips;
         i++, j = (j + 1) % config.nb_routes) {
      if ((rte_rand() % 100) < config.rnd_lookup_ips_ratio) {
        ...
        set_rnd_ipv6(&tbl6[i * 16], rt6[j].addr, rt6[j].depth);
      }
    }
  }
}

static int _inet_net_pton(int af, char *prefix, void *addr) {
  const char *dlm = "/";
  char *s, *sp;
  int ret, depth;
...
static int parse_rt_4(FILE *f) {
  ...
  while (fgets(line, sizeof(line), f) != NULL) {
    ...
    GET_CB_FIELD(in[RT_NEXTHOP], rt[j].nh, 0, UINT32_MAX, 0);
    j++;
  }
  return 0;
}

static int parse_rt_6(FILE *f) {
  int ret, i, j = 0;
  char *s, *sp, *in[RT_NUM];
  static const char *dlm = " \t\n";
  int string_tok_nb = RTE_DIM(in);
...
static int parse_lookup(FILE *f, int af) {
  int ret, i = 0;
...
static void print_config(void) {
  ...
  for (i = 1; i <= depth_lim; i++) {
    fprintf(stdout, "depth /%d:%u", i, config.nb_routes_per_depth[i]);
    if (i % 4 == 0)
      dlm = '\n';
    else
      dlm = '\t';
...
static int check_config(void) {
  ...
  if (!((config.ent_sz == 1) || (config.ent_sz == 2) || (config.ent_sz == 4) ||
        (config.ent_sz == 8))) {
...
static void parse_opts(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "f:t:n:d:l:r:c6ab:e:g:w:u:sv:")) != -1) {
    switch (opt) {
    case 'f':
    ...
    case 'r':
      errno = 0;
      config.rnd_lookup_ips_ratio = strtoul(optarg, &endptr, 10);
      if ((errno != 0) || (config.rnd_lookup_ips_ratio == 0) ||
    ...
    case 'g':
      ...
      break;
    case 'v':
      if ((strcmp(optarg, "s1") == 0) || (strcmp(optarg, "s") == 0)) {
        config.lookup_fn = 1;
        break;
...
static int dump_rt_4(struct rt_rule_4 *rt) {
  FILE *f;
  uint32_t i;
...
static int run_v4(void) {
  ...
  if (conf.type == RTE_FIB_DIR24_8) {
    ...
    conf.dir24_8.num_tbl8 =
        RTE_MIN(config.tbl8, get_max_nh(conf.dir24_8.nh_sz));
  }
  ...
  if (config.lookup_fn != 0) {
    if (config.lookup_fn == 1)
      ret = rte_fib_select_lookup(fib, RTE_FIB_LOOKUP_DIR24_8_SCALAR_MACRO);
    else if (config.lookup_fn == 2)
      ret = rte_fib_select_lookup(fib, RTE_FIB_LOOKUP_DIR24_8_VECTOR_AVX512);
    else if (config.lookup_fn == 3)
      ret = rte_fib_select_lookup(fib, RTE_FIB_LOOKUP_DIR24_8_SCALAR_INLINE);
    else if (config.lookup_fn == 4)
      ret = rte_fib_select_lookup(fib, RTE_FIB_LOOKUP_DIR24_8_SCALAR_UNI);
    else
      ret = -EINVAL;
    if (ret != 0) {
  ...
  for (k = config.print_fract, i = 0; k > 0; k--) {
    ...
    for (j = 0; j < (config.nb_routes - i) / k; j++) {
      ret = rte_fib_add(fib, rt[i + j].addr, rt[i + j].depth, rt[i + j].nh);
      if (unlikely(ret != 0)) {
  ...
  if (config.flags & CMP_FLAG) {
    ...
    lpm_conf.number_tbl8s = RTE_MAX(conf.dir24_8.num_tbl8, config.tbl8);
    ...
    for (k = config.print_fract, i = 0; k > 0; k--) {
      start = rte_rdtsc_precise();
      for (j = 0; j < (config.nb_routes - i) / k; j++) {
        ret = rte_lpm_add(lpm, rt[i + j].addr, rt[i + j].depth, rt[i + j].nh);
        if (ret != 0) {
          if (rt[i + j].depth == 0)
            print_depth_err();
  ...
  if (config.flags & CMP_FLAG) {
    ...
    for (i = 0; i < config.nb_lookup_ips; i += BURST_SZ) {
      start = rte_rdtsc_precise();
      ret = rte_lpm_lookup_bulk(lpm, tbl4 + i, lpm_nh, BURST_SZ);
      acc += rte_rdtsc_precise() - start;
      if (ret != 0) {
  ...
  if (config.flags & CMP_FLAG) {
    for (k = config.print_fract, i = 0; k > 0; k--) {
      start = rte_rdtsc_precise();
      for (j = 0; j < (config.nb_routes - i) / k; j++)
        rte_lpm_delete(lpm, rt[i + j].addr, rt[i + j].depth);
...
static int dump_rt_6(struct rt_rule_6 *rt) {
  FILE *f;
  uint32_t i;
...
static int run_v6(void) {
  ...
  if (conf.type == RTE_FIB6_TRIE) {
    ...
    conf.trie.num_tbl8 = RTE_MIN(config.tbl8, get_max_nh(conf.trie.nh_sz));
  }
  ...
  if (config.lookup_fn != 0) {
    if (config.lookup_fn == 1)
      ret = rte_fib6_select_lookup(fib, RTE_FIB6_LOOKUP_TRIE_SCALAR);
    else if (config.lookup_fn == 2)
      ret = rte_fib6_select_lookup(fib, RTE_FIB6_LOOKUP_TRIE_VECTOR_AVX512);
    else
      ret = -EINVAL;
    if (ret != 0) {
      printf("Can not init lookup function\n");
  ...
  for (k = config.print_fract, i = 0; k > 0; k--) {
    start = rte_rdtsc_precise();
    for (j = 0; j < (config.nb_routes - i) / k; j++) {
      ret = rte_fib6_add(fib, rt[i + j].addr, rt[i + j].depth, rt[i + j].nh);
      if (unlikely(ret != 0)) {
        printf("Can not add a route to FIB, err %d\n", ret);
        return -ret;
      }
  ...
  if (config.flags & CMP_FLAG) {
    lpm_conf.max_rules = config.nb_routes * 2;
    lpm_conf.number_tbl8s = RTE_MAX(conf.trie.num_tbl8, config.tbl8);
    ...
    for (k = config.print_fract, i = 0; k > 0; k--) {
      start = rte_rdtsc_precise();
      for (j = 0; j < (config.nb_routes - i) / k; j++) {
        ret = rte_lpm6_add(lpm, rt[i + j].addr, rt[i + j].depth, rt[i + j].nh);
        if (ret != 0) {
          if (rt[i + j].depth == 0)
            print_depth_err();
          printf("Can not add a route to LPM, "
  ...
  for (i = 0; i < config.nb_lookup_ips; i += BURST_SZ) {
    start = rte_rdtsc_precise();
    ret = rte_fib6_lookup_bulk(fib, (uint8_t(*)[16])(tbl6 + i * 16), fib_nh,
                               BURST_SZ);
    acc += rte_rdtsc_precise() - start;
    if (ret != 0) {
      printf("FIB lookup fails, err %d\n", ret);
  ...
  if (config.flags & CMP_FLAG) {
    ...
    for (i = 0; i < config.nb_lookup_ips; i += BURST_SZ) {
      start = rte_rdtsc_precise();
      ret = rte_lpm6_lookup_bulk_func(lpm, (uint8_t(*)[16])(tbl6 + i * 16),
                                      lpm_nh, BURST_SZ);
      acc += rte_rdtsc_precise() - start;
      if (ret != 0) {
        printf("LPM lookup fails, err %d\n", ret);
    ...
    for (i = 0; i < config.nb_lookup_ips; i += BURST_SZ) {
      rte_fib6_lookup_bulk(fib, (uint8_t(*)[16])(tbl6 + i * 16), fib_nh,
                           BURST_SZ);
      rte_lpm6_lookup_bulk_func(lpm, (uint8_t(*)[16])(tbl6 + i * 16), lpm_nh,
                                BURST_SZ);
      for (j = 0; j < BURST_SZ; j++) {
        if ((fib_nh[j] != (uint32_t)lpm_nh[j]) &&
            !((lpm_nh[j] == -1) && (fib_nh[j] == def_nh))) {
          printf("FAIL\n");
          return -1;
        }
      }
  ...
  if (config.flags & CMP_FLAG) {
    for (k = config.print_fract, i = 0; k > 0; k--) {
      start = rte_rdtsc_precise();
      for (j = 0; j < (config.nb_routes - i) / k; j++)
        rte_lpm6_delete(lpm, rt[i + j].addr, rt[i + j].depth);
...
int main(int argc, char **argv) {
  ...
  rt_ent_sz =
      (af == AF_INET) ? sizeof(struct rt_rule_4) : sizeof(struct rt_rule_6);
  ...
  config.lookup_tbl = rte_malloc(NULL, lookup_ent_sz * config.nb_lookup_ips, 0);
  ...
  if (fr == NULL) {
    if (distrib_string != NULL)
      ...
      uint8_t rpd[129] = {0};
      uint32_t nrpd[129] = {0};
      ret = complete_distrib(depth_lim, config.nb_routes, rpd, nrpd);
    }
    if (ret != 0)
      rte_exit(-ret, "Bad routes distribution configuration\n");
    if (af == AF_INET) {
      gen_random_rt_4(config.rt, __builtin_ctz(config.ent_sz));
      if (config.flags & SHUFFLE_FLAG)
        shuffle_rt_4(config.rt, config.nb_routes);
    } else {
      gen_random_rt_6(config.rt, __builtin_ctz(config.ent_sz));
      if (config.flags & SHUFFLE_FLAG)
