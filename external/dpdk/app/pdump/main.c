...
enum pdump_en_dis { DISABLE = 1, ENABLE = 2 };

enum pcap_stream { IFACE = 1, PCAP = 2 };

enum pdump_by { PORT_ID = 1, DEVICE_ID = 2 };

static const char *const valid_pdump_arguments[] = {
    PDUMP_PORT_ARG,      PDUMP_PCI_ARG,
    PDUMP_QUEUE_ARG,     PDUMP_DIR_ARG,
    PDUMP_RX_DEV_ARG,    PDUMP_TX_DEV_ARG,
    PDUMP_RING_SIZE_ARG, PDUMP_MSIZE_ARG,
    PDUMP_NUM_MBUFS_ARG, NULL};
...
static int parse_device_id(const char *key __rte_unused, const char *value,
                           void *extra_args) {
...
static int parse_queue(const char *key __rte_unused, const char *value,
                       void *extra_args) {
  unsigned long n;
  struct pdump_tuples *pt = extra_args;
...
static int parse_rxtxdev(const char *key, const char *value, void *extra_args) {
...
static int parse_uint_value(const char *key, const char *value,
                            void *extra_args) {
  struct parse_val *v;
  unsigned long t;
  char *end;
...
static int parse_pdump(const char *optarg) {
  ...
  if (!((cnt1 == 1 && cnt2 == 0) || (cnt1 == 0 && cnt2 == 1))) {
    ...
    ret = rte_kvargs_process(kvlist, PDUMP_PORT_ARG, &parse_uint_value, &v);
    if (ret < 0)
      goto free_kvlist;
    pt->port = (uint16_t)v.val;
    pt->dump_by_type = PORT_ID;
  } else if (cnt2 == 1) {
    ret = rte_kvargs_process(kvlist, PDUMP_PCI_ARG, &parse_device_id, pt);
    if (ret < 0)
      goto free_kvlist;
  }
  ...
  if (cnt1 == 1) {
    ...
    ret =
        rte_kvargs_process(kvlist, PDUMP_RING_SIZE_ARG, &parse_uint_value, &v);
    if (ret < 0)
      goto free_kvlist;
  ...
  if (cnt1 == 1) {
    ...
    v.max = UINT16_MAX;
    ret =
        rte_kvargs_process(kvlist, PDUMP_NUM_MBUFS_ARG, &parse_uint_value, &v);
    if (ret < 0)
      goto free_kvlist;
...
static int launch_args_parse(int argc, char **argv, char *prgname) {
  ...
  /* Parse command line */
  while ((opt = getopt_long(argc, argv, " ", long_option, &option_index)) !=
         EOF) {
    switch (opt) {
...
static inline void disable_pdump(struct pdump_tuples *pt) {
...
static inline void pdump_rxtx(struct rte_ring *ring, uint16_t vdev_id,
                              struct pdump_stats *stats) {
  ...
  const uint16_t nb_in_deq =
      rte_ring_dequeue_burst(ring, (void *)rxtx_bufs, BURST_SIZE, NULL);
  stats->dequeue_pkts += nb_in_deq;

  if (nb_in_deq) {
    /* then sent on vdev */
    uint16_t nb_in_txd = rte_eth_tx_burst(vdev_id, 0, rxtx_bufs, nb_in_deq);
...
static void free_ring_data(struct rte_ring *ring, uint16_t vdev_id,
                           struct pdump_stats *stats) {
  ...
  for (i = 0; i < num_tuples; i++) {
    ...
    if (mbuf_pool == NULL) {
      ...
      mbuf_pool = rte_pktmbuf_pool_create_by_ops(
          mempool_name, pt->total_num_mbufs, MBUF_POOL_CACHE_SIZE, 0,
          pt->mbuf_data_size, rte_socket_id(), "ring_mp_mc");
      if (mbuf_pool == NULL) {
        cleanup_rings();
    ...
    if (pt->dir == RTE_PDUMP_FLAG_RXTX) {
      ...
      pt->rx_ring =
          rte_ring_create(ring_name, pt->ring_size, rte_socket_id(), 0);
      if (pt->rx_ring == NULL) {
        cleanup_rings();
      ...
      /* create vdevs */
      snprintf(vdev_name, sizeof(vdev_name), VDEV_NAME_FMT, RX_STR, i);
      (pt->rx_vdev_stream_type == IFACE)
          ? snprintf(vdev_args, sizeof(vdev_args), VDEV_IFACE_ARGS_FMT,
                     pt->rx_dev)
          : snprintf(vdev_args, sizeof(vdev_args), VDEV_PCAP_ARGS_FMT,
                     pt->rx_dev);
      ...
      if (pt->single_pdump_dev)
        ...
        snprintf(vdev_name, sizeof(vdev_name), VDEV_NAME_FMT, TX_STR, i);
        (pt->rx_vdev_stream_type == IFACE)
            ? snprintf(vdev_args, sizeof(vdev_args), VDEV_IFACE_ARGS_FMT,
                       pt->tx_dev)
            : snprintf(vdev_args, sizeof(vdev_args), VDEV_PCAP_ARGS_FMT,
...
static void enable_pdump(void) {
  ...
  for (i = 0; i < num_tuples; i++) {
    ...
    if (pt->dir == RTE_PDUMP_FLAG_RXTX) {
      if (pt->dump_by_type == DEVICE_ID) {
        ret = rte_pdump_enable_by_deviceid(pt->device_id, pt->queue,
                                           RTE_PDUMP_FLAG_RX, pt->rx_ring,
                                           pt->mp, NULL);
        ret1 = rte_pdump_enable_by_deviceid(pt->device_id, pt->queue,
                                            RTE_PDUMP_FLAG_TX, pt->tx_ring,
                                            pt->mp, NULL);
      } else if (pt->dump_by_type == PORT_ID) {
        ret = rte_pdump_enable(pt->port, pt->queue, RTE_PDUMP_FLAG_RX,
                               pt->rx_ring, pt->mp, NULL);
        ret1 = rte_pdump_enable(pt->port, pt->queue, RTE_PDUMP_FLAG_TX,
                                pt->tx_ring, pt->mp, NULL);
      }
    } else if (pt->dir == RTE_PDUMP_FLAG_RX) {
      if (pt->dump_by_type == DEVICE_ID)
        ret = rte_pdump_enable_by_deviceid(pt->device_id, pt->queue, pt->dir,
                                           pt->rx_ring, pt->mp, NULL);
      else if (pt->dump_by_type == PORT_ID)
        ret = rte_pdump_enable(pt->port, pt->queue, pt->dir, pt->rx_ring,
                               pt->mp, NULL);
    } else if (pt->dir == RTE_PDUMP_FLAG_TX) {
      if (pt->dump_by_type == DEVICE_ID)
        ret = rte_pdump_enable_by_deviceid(pt->device_id, pt->queue, pt->dir,
        ...
        ret = rte_pdump_enable(pt->port, pt->queue, pt->dir, pt->tx_ring,
                               pt->mp, NULL);
    }
    if (ret < 0 || ret1 < 0) {
...
static inline void pdump_packets(struct pdump_tuples *pt) {
  if (pt->dir & RTE_PDUMP_FLAG_RX)
...
static unsigned int get_next_core(unsigned int lcore) {
  lcore = rte_get_next_lcore(lcore, 1, 0);
  if (lcore == RTE_MAX_LCORE)
    rte_exit(EXIT_FAILURE, "Max core limit %u reached for packet capture",
             lcore);
  return lcore;
}
  ...
  if (!multiple_core_capture) {
    ...
    for (i = 0; i < num_tuples; i++)
      printf(" - port %u device (%s) queue %u\n", pdump_t[i].port,
             pdump_t[i].device_id, pdump_t[i].queue);
