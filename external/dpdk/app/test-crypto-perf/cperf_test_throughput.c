...
static void cperf_throughput_test_free(struct cperf_throughput_ctx *ctx) {
  if (!ctx)
    return;
  if (ctx->sess) {
    if (ctx->options->op_type == CPERF_ASYM_MODEX)
      rte_cryptodev_asym_session_free(ctx->dev_id, (void *)ctx->sess);
...
void *cperf_throughput_test_constructor(
...
int cperf_throughput_test_runner(void *test_ctx) {
  ...
  while (test_burst_size <= ctx->options->max_burst_size) {
    ...
    while (ops_enqd_total < ctx->options->total_ops) {
      ...
      uint16_t burst_size =
          ((ops_enqd_total + test_burst_size) <= ctx->options->total_ops)
              ? test_burst_size
    ...
    double ops_per_second =
        ((double)ctx->options->total_ops / tsc_duration) * rte_get_tsc_hz();
    ...
    if (!ctx->options->csv) {
      if (__atomic_compare_exchange_n(&display_once, &exp, 1, 0,
        ...
        printf("%12s%12s%12s%12s%12s%12s%12s%12s%12s%12s\n\n", "lcore id",
               "Buf Size", "Burst Size", "Enqueued", "Dequeued", "Failed Enq",
      ...
      printf("%12u%12u%12u%12" PRIu64 "%12" PRIu64 "%12" PRIu64 "%12" PRIu64
             ...
             ops_enqd_total, ops_deqd_total, ops_enqd_failed, ops_deqd_failed,
             ops_per_second / 1000000, throughput_gbps, cycles_per_packet);
    } else {
      if (__atomic_compare_exchange_n(&display_once, &exp, 1, 0,
                                      __ATOMIC_RELAXED, __ATOMIC_RELAXED))
