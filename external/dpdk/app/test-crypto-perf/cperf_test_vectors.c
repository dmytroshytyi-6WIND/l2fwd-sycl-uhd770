...
uint8_t perf_base[20] = {0xF8, 0xBA, 0x1A, 0x55, 0xD0, 0x2F, 0x85,
...
/* aad */
uint8_t aad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
...
/* cipher key */
uint8_t cipher_key[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                        0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
...
/* AEAD key */
uint8_t aead_key[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                      0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
                      0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                      0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F};
...
uint8_t ipsec_plaintext[2048] = {
    /* IP */
    0x45, 0x00, 0x00, 0x28, 0xa4, 0xad, 0x40, 0x00, 0x40, 0x06, 0x78, 0x80,
    0x0a, 0x01, 0x03, 0x8f, 0x0a, 0x01, 0x06, 0x12,

    /* TCP */
    0x80, 0x23, 0x06, 0xb8, 0xcb, 0x71, 0x26, 0x02, 0xdd, 0x6b, 0xb0, 0x3e,
    0x50, 0x10, 0x16, 0xd0, 0x75, 0x67, 0x00, 0x01};

struct cperf_test_vector *
cperf_test_vector_get_dummy(struct cperf_options *options) {
  struct cperf_test_vector *t_vec;

  t_vec = (struct cperf_test_vector *)rte_malloc(
  ...
  if (options->op_type == CPERF_PDCP || options->op_type == CPERF_IPSEC) {
    ...
    if (options->cipher_iv_sz != 0) {
      /* Set IV parameters */
      t_vec->cipher_iv.data = rte_malloc(NULL, options->cipher_iv_sz, 16);
    ...
    if (options->auth_algo == RTE_CRYPTO_AUTH_NULL) {
      ...
      t_vec->digest.data = rte_malloc(NULL, options->digest_sz, 16);
      if (t_vec->digest.data == NULL) {
        rte_free(t_vec->cipher_iv.data);
        rte_free(t_vec);
        return NULL;
      }
      t_vec->digest.phys_addr = rte_malloc_virt2iova(t_vec->digest.data);
  ...
  if (options->op_type == CPERF_AUTH_ONLY ||
    ...
    /* Set IV parameters */
    t_vec->auth_iv.data = rte_malloc(NULL, options->auth_iv_sz, 16);
  ...
  if (options->op_type == CPERF_AEAD || options->op_type == CPERF_IPSEC) {
    t_vec->aead_key.length = options->aead_key_sz;
    t_vec->aead_key.data = aead_key;

    if (options->aead_aad_sz) {
      t_vec->aad.data = rte_malloc(NULL, options->aead_aad_sz, 16);
    ...
    /* Set IV parameters */
    t_vec->aead_iv.data = rte_malloc(NULL, options->aead_iv_sz, 16);
