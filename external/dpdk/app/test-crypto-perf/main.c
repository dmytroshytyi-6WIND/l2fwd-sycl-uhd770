...
const struct cperf_test cperf_testmap[] = {
    [CPERF_TEST_TYPE_THROUGHPUT] = {cperf_throughput_test_constructor,
                                    cperf_throughput_test_runner,
                                    cperf_throughput_test_destructor},
    [CPERF_TEST_TYPE_LATENCY] = {cperf_latency_test_constructor,
    ...
    [CPERF_TEST_TYPE_PMDCC] = {cperf_pmd_cyclecount_test_constructor,
                               cperf_pmd_cyclecount_test_runner,
                               cperf_pmd_cyclecount_test_destructor}};

static int create_asym_op_pool_socket(int32_t socket_id, uint32_t nb_sessions) {
  char mp_name[RTE_MEMPOOL_NAMESIZE];
  struct rte_mempool *mpool = NULL;

  if (session_pool_socket[socket_id].sess_mp == NULL) {
    snprintf(mp_name, RTE_MEMPOOL_NAMESIZE, "perf_asym_sess_pool%u", socket_id);
    mpool = rte_cryptodev_asym_session_pool_create(mp_name, nb_sessions, 0, 0,
                                                   socket_id);
    if (mpool == NULL) {
...
static int fill_session_pool_socket(int32_t socket_id,
  ...
  if (session_pool_socket[socket_id].priv_mp == NULL) {
    ...
    sess_mp = rte_mempool_create(mp_name, nb_sessions, session_priv_size, 0, 0,
  ...
  if (session_pool_socket[socket_id].sess_mp == NULL) {
    ...
    sess_mp = rte_cryptodev_sym_session_pool_create(mp_name, nb_sessions, 0, 0,
                                                    0, socket_id);
...
static int cperf_initialize_cryptodev(struct cperf_options *opts,
  ...
  enabled_cdev_count = rte_cryptodev_devices_get(
      opts->device_type, enabled_cdevs, RTE_CRYPTO_MAX_DEVS);
  ...
  for (cdev_id = 0; cdev_id < rte_cryptodev_count(); cdev_id++) {
    sess_size =
        rte_security_session_get_size(rte_cryptodev_get_sec_ctx(cdev_id));
    if (sess_size > max_sess_size)
      max_sess_size = sess_size;
  }
#endif
  ...
  for (i = 0; i < enabled_cdev_count && i < RTE_CRYPTO_MAX_DEVS; i++) {
    ...
    if (!strcmp((const char *)opts->device_type, "crypto_scheduler") &&
        rte_cryptodev_scheduler_mode_get(cdev_id) == CDEV_SCHED_MODE_MULTICORE)
      opts->nb_qps = 1;
#endif
    ...
    if (opts->op_type == CPERF_ASYM_MODEX) {
      if ((cdev_info.feature_flags & RTE_CRYPTODEV_FF_ASYMMETRIC_CRYPTO) == 0)
        continue;
    }
    ...
    if (!strcmp((const char *)opts->device_type, "crypto_scheduler")) {
#ifdef RTE_CRYPTO_SCHEDULER
      uint32_t nb_slaves = rte_cryptodev_scheduler_workers_get(cdev_id, NULL);

      sessions_needed = enabled_cdev_count * opts->nb_qps * nb_slaves;
#endif
    } else
    ...
    if (opts->op_type == CPERF_ASYM_MODEX)
      ret = create_asym_op_pool_socket(socket_id, sessions_needed);
    else
...
static int cperf_verify_devices_capabilities(struct cperf_options *opts,
                                             uint8_t *enabled_cdevs,
  ...
  for (i = 0; i < nb_cryptodevs; i++) {
    ...
    if (opts->op_type == CPERF_ASYM_MODEX) {
      ...
      asym_capability =
          rte_cryptodev_asym_capability_get(cdev_id, &asym_cap_idx);
    ...
    if (opts->op_type == CPERF_CIPHER_ONLY ||
      ...
      ret = rte_cryptodev_sym_capability_check_cipher(
          capability, opts->cipher_key_sz, opts->cipher_iv_sz);
      if (ret != 0)
        return ret;
    }
    ...
    if (opts->op_type == CPERF_AEAD) {
      ...
      ret = rte_cryptodev_sym_capability_check_aead(
          capability, opts->aead_key_sz, opts->digest_sz, opts->aead_aad_sz,
          opts->aead_iv_sz);
      if (ret != 0)
        return ret;
    }
...
static int cperf_check_test_vector(struct cperf_options *opts,
  ...
  if (opts->op_type == CPERF_CIPHER_ONLY) {
    if (opts->cipher_algo == RTE_CRYPTO_CIPHER_NULL) {
      ...
      if (opts->cipher_iv_sz && test_vec->cipher_iv.data == NULL)
        return -1;
    ...
    if (opts->auth_algo != RTE_CRYPTO_AUTH_NULL) {
      ...
      if (opts->auth_key_sz && test_vec->auth_key.data == NULL)
        return -1;
...
int main(int argc, char **argv) {
  ...
  ret = cperf_verify_devices_capabilities(&opts, enabled_cdevs, nb_cryptodevs);
  if (ret) {
    RTE_LOG(ERR, USER1,
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    ctx[i] = cperf_testmap[opts.test].constructor(
        ...
        session_pool_socket[socket_id].priv_mp, cdev_id, qp_id, &opts, t_vec,
        &op_fns);
    if (ctx[i] == NULL) {
  ...
  if (opts.imix_distribution_count != 0) {
    ...
    opts.imix_buffer_sizes =
        rte_malloc(NULL, sizeof(uint32_t) * opts.pool_sz, 0);
    /*
    ...
    for (i = 1; i < buffer_size_count; i++)
      distribution_total[i] =
          imix_distribution_list[i] + distribution_total[i - 1];

    /* Calculate a random sequence of packet sizes, based on distribution */
    for (op_idx = 0; op_idx < opts.pool_sz; op_idx++) {
      uint16_t random_number =
          rte_rand() % distribution_total[buffer_size_count - 1];
    ...
    for (i = 0; i < buffer_size_count; i++)
      test_average_size += buffer_size_list[i] * imix_distribution_list[i];

    opts.test_buffer_size =
        test_average_size / distribution_total[buffer_size_count - 1];
    ...
    RTE_LCORE_FOREACH_WORKER(lcore_id) {
      ...
      rte_eal_remote_launch(cperf_testmap[opts.test].runner, ctx[i], lcore_id);
      i++;
    }
    i = 0;
    RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    while (opts.test_buffer_size <= opts.max_buffer_size) {
      ...
      RTE_LCORE_FOREACH_WORKER(lcore_id) {
        ...
        rte_eal_remote_launch(cperf_testmap[opts.test].runner, ctx[i],
                              lcore_id);
        i++;
      }
      i = 0;
  ...
  for (i = 0; i < nb_cryptodevs && i < RTE_CRYPTO_MAX_DEVS; i++) {
