...
static void cperf_latency_test_free(struct cperf_latency_ctx *ctx) {
  if (ctx) {
    if (ctx->sess) {
...
void *cperf_latency_test_constructor(
  ...
  if (cperf_alloc_common_memory(options, test_vector, dev_id, qp_id,
                                extra_op_priv_size, &ctx->src_buf_offset,
                                &ctx->dst_buf_offset, &ctx->pool) < 0)
    goto err;

  ctx->res = rte_malloc(
      NULL, sizeof(struct cperf_op_result) * ctx->options->total_ops, 0);
...
static inline void store_timestamp(struct rte_crypto_op *op,
                                   uint64_t timestamp) {
...
int cperf_latency_test_runner(void *arg) {
  ...
  while (test_burst_size <= ctx->options->max_burst_size) {
    ...
    while (enqd_tot < ctx->options->total_ops) {
      ...
      uint16_t burst_size =
          ((enqd_tot + test_burst_size) <= ctx->options->total_ops)
              ? test_burst_size
              : ctx->options->total_ops - enqd_tot;

      /* Allocate objects containing crypto operations and mbufs */
      if (rte_mempool_get_bulk(ctx->pool, (void **)ops, burst_size) != 0) {
        RTE_LOG(ERR, USER1,
      ...
      (ctx->populate_ops)(ops, ctx->src_buf_offset, ctx->dst_buf_offset,
                          burst_size, ctx->sess, ctx->options, ctx->test_vector,
      ...
      if (ops_enqd != burst_size)
        rte_mempool_put_bulk(ctx->pool, (void **)&ops[ops_enqd],
                             burst_size - ops_enqd);
    ...
    if (ctx->options->csv) {
      ...
      for (i = 0; i < ctx->options->total_ops; i++) {
        ...
        printf("\n%u,%u,%u,%" PRIu64 ",%" PRIu64 ",%.3f", ctx->lcore_id,
               ctx->options->test_buffer_size, test_burst_size, i + 1,
               ctx->res[i].tsc_end - ctx->res[i].tsc_start,
               tunit * (double)(ctx->res[i].tsc_end - ctx->res[i].tsc_start) /
                   tsc_hz);
      }
    } else {
      ...
      printf("\n#  enqueued\t%12" PRIu64 "\t%10" PRIu64 "\t"
             "%10" PRIu64 "\t%10" PRIu64,
             enqd_tot, enqd_avg, enqd_max, enqd_min);
      printf("\n#  dequeued\t%12" PRIu64 "\t%10" PRIu64 "\t"
             "%10" PRIu64 "\t%10" PRIu64,
             deqd_tot, deqd_avg, deqd_max, deqd_min);
...
void cperf_latency_test_destructor(void *arg) {
  struct cperf_latency_ctx *ctx = arg;
