...
static int cperf_set_ops_asym(
    struct rte_crypto_op **ops, uint32_t src_buf_offset __rte_unused,
    uint32_t dst_buf_offset __rte_unused, uint16_t nb_ops,
    struct rte_cryptodev_sym_session *sess,
    const struct cperf_options *options __rte_unused,
    const struct cperf_test_vector *test_vector __rte_unused,
    uint16_t iv_offset __rte_unused, uint32_t *imix_idx __rte_unused,
...
static int cperf_set_ops_security(
    struct rte_crypto_op **ops, uint32_t src_buf_offset __rte_unused,
    uint32_t dst_buf_offset __rte_unused, uint16_t nb_ops,
    struct rte_cryptodev_sym_session *sess, const struct cperf_options *options,
    const struct cperf_test_vector *test_vector,
    uint16_t iv_offset __rte_unused, uint32_t *imix_idx, uint64_t *tsc_start) {
  uint16_t i;

  for (i = 0; i < nb_ops; i++) {
    struct rte_crypto_sym_op *sym_op = ops[i]->sym;
    struct rte_security_session *sec_sess = (struct rte_security_session *)sess;
    uint32_t buf_sz;

    uint32_t *per_pkt_hfn =
        rte_crypto_op_ctod_offset(ops[i], uint32_t *, iv_offset);
    *per_pkt_hfn = options->pdcp_ses_hfn_en ? 0 : PDCP_DEFAULT_HFN;
    ...
    if (options->op_type == CPERF_DOCSIS) {
      ...
      sym_op->auth.data.length =
          buf_sz - sym_op->auth.data.offset - RTE_ETHER_CRC_LEN;
      /*
       * DOCSIS header and SRC and DST MAC addresses are not
       * ciphered
       */
      sym_op->cipher.data.offset =
          sym_op->auth.data.offset + RTE_ETHER_HDR_LEN - RTE_ETHER_TYPE_LEN;
      sym_op->cipher.data.length = buf_sz - sym_op->cipher.data.offset;
    }
    ...
    if (dst_buf_offset == 0)
      ...
      sym_op->m_dst = (struct rte_mbuf *)((uint8_t *)ops[i] + dst_buf_offset);
  }
...
static int cperf_set_ops_security_ipsec(
    struct rte_crypto_op **ops, uint32_t src_buf_offset __rte_unused,
    uint32_t dst_buf_offset __rte_unused, uint16_t nb_ops,
    struct rte_cryptodev_sym_session *sess, const struct cperf_options *options,
    const struct cperf_test_vector *test_vector,
...
static int cperf_set_ops_null_cipher(
    struct rte_crypto_op **ops, uint32_t src_buf_offset,
    uint32_t dst_buf_offset, uint16_t nb_ops,
    struct rte_cryptodev_sym_session *sess, const struct cperf_options *options,
    const struct cperf_test_vector *test_vector __rte_unused,
  ...
  for (i = 0; i < nb_ops; i++) {
    ...
    /* cipher parameters */
    if (options->imix_distribution_count) {
      sym_op->cipher.data.length = options->imix_buffer_sizes[*imix_idx];
      *imix_idx = (*imix_idx + 1) % options->pool_sz;
    } else
...
static int cperf_set_ops_null_auth(
    struct rte_crypto_op **ops, uint32_t src_buf_offset,
    uint32_t dst_buf_offset, uint16_t nb_ops,
    struct rte_cryptodev_sym_session *sess, const struct cperf_options *options,
    const struct cperf_test_vector *test_vector __rte_unused,
  ...
  for (i = 0; i < nb_ops; i++) {
    ...
    /* auth parameters */
    if (options->imix_distribution_count) {
      sym_op->auth.data.length = options->imix_buffer_sizes[*imix_idx];
      *imix_idx = (*imix_idx + 1) % options->pool_sz;
    } else
...
static int cperf_set_ops_cipher(struct rte_crypto_op **ops,
...
static int cperf_set_ops_auth(struct rte_crypto_op **ops,
  ...
  for (i = 0; i < nb_ops; i++) {
    ...
    if (test_vector->auth_iv.length) {
      uint8_t *iv_ptr = rte_crypto_op_ctod_offset(ops[i], uint8_t *, iv_offset);
      memcpy(iv_ptr, test_vector->auth_iv.data, test_vector->auth_iv.length);
    }

    /* authentication parameters */
    if (options->auth_op == RTE_CRYPTO_AUTH_OP_VERIFY) {
      sym_op->auth.digest.data = test_vector->digest.data;
      sym_op->auth.digest.phys_addr = test_vector->digest.phys_addr;
    } else {
      ...
      if (options->out_of_place) {
        ...
        tbuf = sym_op->m_src;
        while ((tbuf->next != NULL) && (offset >= tbuf->data_len)) {
      ...
      sym_op->auth.digest.data =
          rte_pktmbuf_mtod_offset(buf, uint8_t *, offset);
      sym_op->auth.digest.phys_addr = rte_pktmbuf_iova_offset(buf, offset);
    }
  ...
  if (options->test == CPERF_TEST_TYPE_VERIFY) {
    if (test_vector->auth_iv.length) {
      for (i = 0; i < nb_ops; i++) {
        uint8_t *iv_ptr =
            rte_crypto_op_ctod_offset(ops[i], uint8_t *, iv_offset);
...
static int cperf_set_ops_cipher_auth(
    struct rte_crypto_op **ops, uint32_t src_buf_offset,
    uint32_t dst_buf_offset, uint16_t nb_ops,
    struct rte_cryptodev_sym_session *sess, const struct cperf_options *options,
    const struct cperf_test_vector *test_vector, uint16_t iv_offset,
    uint32_t *imix_idx, uint64_t *tsc_start __rte_unused) {
  uint16_t i;
...
static int cperf_set_ops_aead(struct rte_crypto_op **ops,
                              uint32_t src_buf_offset, uint32_t dst_buf_offset,
                              uint16_t nb_ops,
                              struct rte_cryptodev_sym_session *sess,
                              const struct cperf_options *options,
  ...
  uint16_t aad_offset =
      iv_offset + RTE_ALIGN_CEIL(test_vector->aead_iv.length, 16);
  ...
  for (i = 0; i < nb_ops; i++) {
    ...
    /* AEAD parameters */
    if (options->imix_distribution_count) {
      sym_op->aead.data.length = options->imix_buffer_sizes[*imix_idx];
      *imix_idx = (*imix_idx + 1) % options->pool_sz;
    } else
    ...
    sym_op->aead.aad.data =
        rte_crypto_op_ctod_offset(ops[i], uint8_t *, aad_offset);
    sym_op->aead.aad.phys_addr =
        rte_crypto_op_ctophys_offset(ops[i], aad_offset);

    if (options->aead_op == RTE_CRYPTO_AEAD_OP_DECRYPT) {
      sym_op->aead.digest.data = test_vector->digest.data;
      sym_op->aead.digest.phys_addr = test_vector->digest.phys_addr;
    } else {

      uint32_t offset = sym_op->aead.data.length + sym_op->aead.data.offset;
      struct rte_mbuf *buf, *tbuf;
      ...
      sym_op->aead.digest.data =
          rte_pktmbuf_mtod_offset(buf, uint8_t *, offset);
      sym_op->aead.digest.phys_addr = rte_pktmbuf_iova_offset(buf, offset);
    }
  }
  ...
  if (options->test == CPERF_TEST_TYPE_VERIFY) {
    for (i = 0; i < nb_ops; i++) {
      ...
      if (options->aead_algo == RTE_CRYPTO_AEAD_AES_CCM) {
        ...
        memcpy(ops[i]->sym->aead.aad.data + 18, test_vector->aad.data,
               test_vector->aad.length);
      } else {
        memcpy(iv_ptr, test_vector->aead_iv.data, test_vector->aead_iv.length);

        memcpy(ops[i]->sym->aead.aad.data, test_vector->aad.data,
               test_vector->aad.length);
      }
    }
  }
...
create_ipsec_session(struct rte_mempool *sess_mp, struct rte_mempool *priv_mp,
...
#define CPERF_IPSEC_DEFTTL 64
               ...
               .direction =
                   ((options->cipher_op == RTE_CRYPTO_CIPHER_OP_ENCRYPT) &&
                    (options->auth_op == RTE_CRYPTO_AUTH_OP_GENERATE)) ||
                           (options->aead_op == RTE_CRYPTO_AEAD_OP_ENCRYPT)
  ...
  return (void *)rte_security_session_create(ctx, &sess_conf, sess_mp, priv_mp);
}

static struct rte_cryptodev_sym_session *
cperf_create_session(struct rte_mempool *sess_mp, struct rte_mempool *priv_mp,
                     uint8_t dev_id, const struct cperf_options *options,
                     const struct cperf_test_vector *test_vector,
                     uint16_t iv_offset) {
  ...
  if (options->op_type == CPERF_ASYM_MODEX) {
    ...
    ret =
        rte_cryptodev_asym_session_create(dev_id, &xform, sess_mp, &asym_sess);
    if (ret < 0) {
  ...
  if (options->op_type == CPERF_PDCP) {
    ...
    if (options->auth_algo != 0) {
      ...
      auth_xform.auth.algo = options->auth_algo;
      auth_xform.auth.op = options->auth_op;
      auth_xform.auth.iv.offset = iv_offset + cipher_xform.cipher.iv.length;
        ...
        {.pdcp =
             {
                 ...
                 .hfn = options->pdcp_ses_hfn_en ? PDCP_DEFAULT_HFN : 0,
  ...
  if (options->op_type == CPERF_IPSEC) {
    return create_ipsec_session(sess_mp, priv_mp, dev_id, options, test_vector,
                                iv_offset);
  }
  ...
  if (options->op_type == CPERF_CIPHER_ONLY) {
    ...
    rte_cryptodev_sym_session_init(dev_id, sess, &cipher_xform, priv_mp);
    /*
     *  auth only
     */
    ...
    /* create crypto session */
    rte_cryptodev_sym_session_init(dev_id, sess, &auth_xform, priv_mp);
    /*
     * cipher and auth
     */
    ...
    aead_xform.aead.key.data = test_vector->aead_key.data;
    aead_xform.aead.key.length = test_vector->aead_key.length;
    aead_xform.aead.iv.length = test_vector->aead_iv.length;

    aead_xform.aead.digest_length = options->digest_sz;
    aead_xform.aead.aad_length = options->aead_aad_sz;

    /* Create crypto session */
    rte_cryptodev_sym_session_init(dev_id, sess, &aead_xform, priv_mp);
  }

  return sess;
}

int cperf_get_op_functions(const struct cperf_options *options,
