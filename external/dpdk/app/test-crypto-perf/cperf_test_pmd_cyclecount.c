  ...
  if (ctx->sess) {
    ...
    if (ctx->options->op_type == CPERF_PDCP ||
      ...
      struct rte_security_ctx *sec_ctx =
          (struct rte_security_ctx *)rte_cryptodev_get_sec_ctx(ctx->dev_id);
...
void *cperf_pmd_cyclecount_test_constructor(
  ...
  size_t alloc_sz = sizeof(struct rte_crypto_op *) * options->nb_descriptors;
...
static inline int pmd_cyclecount_bench_ops(struct pmd_cyclecount_state *state,
                                           uint32_t cur_op,
  ...
  for (cur_iter_op = 0; cur_iter_op < iter_ops_needed;
    ...
    uint32_t burst_size =
        RTE_MIN(iter_ops_needed - cur_iter_op, test_burst_size);
    struct rte_crypto_op **ops = &state->ctx->ops[cur_iter_op];

    /* Allocate objects containing crypto operations and mbufs */
    if (rte_mempool_get_bulk(state->ctx->pool, (void **)ops, burst_size) != 0) {
      RTE_LOG(ERR, USER1,
    ...
    (state->ctx->populate_ops)(
        ops, state->ctx->src_buf_offset, state->ctx->dst_buf_offset, burst_size,
        state->ctx->sess, state->opts, state->ctx->test_vector, iv_offset,
        &imix_idx, NULL);
    ...
    rte_mempool_put_bulk(state->ctx->pool, (void **)ops, burst_size);
  }
...
static int pmd_cyclecount_build_ops(struct pmd_cyclecount_state *state,
                                    uint32_t iter_ops_needed,
                                    uint16_t test_burst_size) {
  uint32_t cur_iter_op;
  uint32_t imix_idx = 0;
...
static uint32_t pmd_cyclecount_bench_enq(struct pmd_cyclecount_state *state,
                                         uint32_t iter_ops_needed,
                                         uint16_t test_burst_size) {
  ...
  while (cur_iter_op < iter_ops_needed) {
    ...
    burst_enqd = rte_cryptodev_enqueue_burst(
        state->ctx->dev_id, state->ctx->qp_id, ops, burst_size);
...
/* benchmark dequeue */
static void pmd_cyclecount_bench_deq(struct pmd_cyclecount_state *state,
                                     uint32_t iter_ops_needed,
                                     uint16_t test_burst_size) {
  ...
  while (cur_iter_op < iter_ops_needed) {
    uint32_t burst_size =
        RTE_MIN(iter_ops_needed - cur_iter_op, test_burst_size);
    struct rte_crypto_op **ops_processed = &state->ctx->ops[cur_iter_op];
    uint32_t burst_deqd;

    burst_deqd = rte_cryptodev_dequeue_burst(
        state->ctx->dev_id, state->ctx->qp_id, ops_processed, burst_size);
...
pmd_cyclecount_bench_burst_sz(struct pmd_cyclecount_state *state,
  ...
  for (cur_op = 0; cur_op < state->opts->total_ops;
    ...
    if (unlikely(pmd_cyclecount_bench_ops(state, cur_op, test_burst_size)))
      return -1;
  }
  ...
  while (cur_op < state->opts->total_ops) {
    ...
    if (unlikely(
            pmd_cyclecount_build_ops(state, iter_ops_needed, test_burst_size)))
      return -1;
    ...
    /* fill up TX ring */
    iter_ops_needed =
        pmd_cyclecount_bench_enq(state, iter_ops_needed, test_burst_size);
    ...
    /* drain RX ring */
    pmd_cyclecount_bench_deq(state, iter_ops_needed, test_burst_size);
     ...
     */
    rte_mempool_put_bulk(state->ctx->pool, (void **)state->ctx->ops,
                         iter_ops_allocd);
  }
...
int cperf_pmd_cyclecount_test_runner(void *test_ctx) {
  ...
  if (opts->segments_sz < ctx->options->max_buffer_size) {
    ...
    if ((dev_info.feature_flags & RTE_CRYPTODEV_FF_MBUF_SCATTER_GATHER) == 0) {
      state.linearize = 1;
    }
  }
#endif /* CPERF_LINEARIZATION_ENABLE */
  ...
  while (test_burst_size <= opts->max_burst_size) {
    ...
    if (!opts->csv) {
      if (__atomic_compare_exchange_n(&display_once, &exp, 1, 0,
        ...
        printf(PRETTY_HDR_FMT, "lcore id", "Buf Size", "Burst Size", "Enqueued",
               "Dequeued", "Enq Retries", "Deq Retries", "Cycles/Op",
               "Cycles/Enq", "Cycles/Deq");

      printf(PRETTY_LINE_FMT, state.ctx->lcore_id, opts->test_buffer_size,
             test_burst_size, state.ops_enqd, state.ops_deqd,
             state.ops_enq_retries, state.ops_deq_retries,
             state.cycles_per_build, state.cycles_per_enq,
             state.cycles_per_deq);
    } else {
      if (__atomic_compare_exchange_n(&display_once, &exp, 1, 0,
                                      __ATOMIC_RELAXED, __ATOMIC_RELAXED))
        printf(CSV_HDR_FMT, "# lcore id", "Buf Size", "Burst Size", "Enqueued",
               "Dequeued", "Enq Retries", "Deq Retries", "Cycles/Op",
               "Cycles/Enq", "Cycles/Deq");

      printf(CSV_LINE_FMT, state.ctx->lcore_id, opts->test_buffer_size,
             test_burst_size, state.ops_enqd, state.ops_deqd,
             state.ops_enq_retries, state.ops_deq_retries,
             state.cycles_per_build, state.cycles_per_enq,
             state.cycles_per_deq);
...
void cperf_pmd_cyclecount_test_destructor(void *arg) {
