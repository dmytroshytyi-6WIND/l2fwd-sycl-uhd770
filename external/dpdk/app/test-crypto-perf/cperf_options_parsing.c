...
static int get_str_key_id_mapping(struct name_id_map *map, unsigned int map_len,
                                  const char *str_key) {
  unsigned int i;
...
static int parse_cperf_test_type(struct cperf_options *opts, const char *arg) {
  struct name_id_map cperftest_namemap[] = {
      {cperf_test_type_strs[CPERF_TEST_TYPE_THROUGHPUT],
       CPERF_TEST_TYPE_THROUGHPUT},
      {cperf_test_type_strs[CPERF_TEST_TYPE_VERIFY], CPERF_TEST_TYPE_VERIFY},
      {cperf_test_type_strs[CPERF_TEST_TYPE_LATENCY], CPERF_TEST_TYPE_LATENCY},
      {cperf_test_type_strs[CPERF_TEST_TYPE_PMDCC], CPERF_TEST_TYPE_PMDCC}};

  int id = get_str_key_id_mapping((struct name_id_map *)cperftest_namemap,
...
static int parse_uint32_t(uint32_t *value, const char *arg) {
  char *end = NULL;
...
static int parse_uint16_t(uint16_t *value, const char *arg) {
  uint32_t val = 0;
...
static int parse_range(const char *arg, uint32_t *min, uint32_t *max,
                       uint32_t *inc) {
  char *token;
  uint32_t number;
  ...
  if (token != NULL) {
    ...
    if (errno == EINVAL || errno == ERANGE || number == 0)
      goto err_range;
  ...
  if (token != NULL) {
    ...
    if (errno == EINVAL || errno == ERANGE || number == 0 || number < *min)
      goto err_range;
...
static int parse_list(const char *arg, uint32_t *list, uint32_t *min,
                      uint32_t *max) {
  char *token;
  uint32_t number;
  uint8_t count = 0;
...
static int parse_total_ops(struct cperf_options *opts, const char *arg) {
...
static int parse_pool_sz(struct cperf_options *opts, const char *arg) {
...
static int parse_burst_sz(struct cperf_options *opts, const char *arg) {
  int ret;
  ...
  if (parse_range(arg, &opts->min_burst_size, &opts->max_burst_size,
    ...
    ret = parse_list(arg, opts->burst_size_list, &opts->min_burst_size,
                     &opts->max_burst_size);
    if (ret < 0) {
...
static int parse_buffer_sz(struct cperf_options *opts, const char *arg) {
  int ret;

  /* Try parsing the argument as a range, if it fails, parse it as a list */
  if (parse_range(arg, &opts->min_buffer_size, &opts->max_buffer_size,
                  &opts->inc_buffer_size) < 0) {
    ret = parse_list(arg, opts->buffer_size_list, &opts->min_buffer_size,
...
static int parse_segment_sz(struct cperf_options *opts, const char *arg) {
...
static int parse_imix(struct cperf_options *opts, const char *arg) {
  int ret;

  ret = parse_list(arg, opts->imix_distribution_list, NULL, NULL);
  if (ret < 0) {
...
static int parse_desc_nb(struct cperf_options *opts, const char *arg) {
...
static int parse_device_type(struct cperf_options *opts, const char *arg) {
...
static int parse_op_type(struct cperf_options *opts, const char *arg) {
  struct name_id_map optype_namemap[] = {
      {cperf_op_type_strs[CPERF_CIPHER_ONLY], CPERF_CIPHER_ONLY},
      {cperf_op_type_strs[CPERF_AUTH_ONLY], CPERF_AUTH_ONLY},
      {cperf_op_type_strs[CPERF_CIPHER_THEN_AUTH], CPERF_CIPHER_THEN_AUTH},
      {cperf_op_type_strs[CPERF_AUTH_THEN_CIPHER], CPERF_AUTH_THEN_CIPHER},
      {cperf_op_type_strs[CPERF_AEAD], CPERF_AEAD},
      {cperf_op_type_strs[CPERF_PDCP], CPERF_PDCP},
      {cperf_op_type_strs[CPERF_DOCSIS], CPERF_DOCSIS},
      {cperf_op_type_strs[CPERF_IPSEC], CPERF_IPSEC},
      {cperf_op_type_strs[CPERF_ASYM_MODEX], CPERF_ASYM_MODEX}};
...
static int parse_sessionless(struct cperf_options *opts,
                             const char *arg __rte_unused) {
  opts->sessionless = 1;
  return 0;
}

static int parse_out_of_place(struct cperf_options *opts,
                              const char *arg __rte_unused) {
  opts->out_of_place = 1;
  return 0;
}

static int parse_test_file(struct cperf_options *opts, const char *arg) {
...
static int parse_test_name(struct cperf_options *opts, const char *arg) {
  char *test_name =
      (char *)rte_zmalloc(NULL, sizeof(char) * (strlen(arg) + 3), 0);
  if (test_name == NULL) {
...
static int parse_silent(struct cperf_options *opts,
                        const char *arg __rte_unused) {
  opts->silent = 1;
...
static int parse_cipher_algo(struct cperf_options *opts, const char *arg) {
...
static int parse_cipher_op(struct cperf_options *opts, const char *arg) {
  ...
  int id = get_str_key_id_mapping(cipher_op_namemap, RTE_DIM(cipher_op_namemap),
                                  arg);
  if (id < 0) {
...
static int parse_cipher_key_sz(struct cperf_options *opts, const char *arg) {
  return parse_uint16_t(&opts->cipher_key_sz, arg);
}

static int parse_cipher_iv_sz(struct cperf_options *opts, const char *arg) {
  return parse_uint16_t(&opts->cipher_iv_sz, arg);
}

static int parse_auth_algo(struct cperf_options *opts, const char *arg) {
...
static int parse_auth_op(struct cperf_options *opts, const char *arg) {
  ...
  int id =
      get_str_key_id_mapping(auth_op_namemap, RTE_DIM(auth_op_namemap), arg);
  if (id < 0) {
    RTE_LOG(ERR, USER1,
            "invalid authentication operation specified"
            "\n");
    return -1;
  }
...
static int parse_auth_key_sz(struct cperf_options *opts, const char *arg) {
  return parse_uint16_t(&opts->auth_key_sz, arg);
}

static int parse_digest_sz(struct cperf_options *opts, const char *arg) {
  return parse_uint16_t(&opts->digest_sz, arg);
}

#ifdef RTE_LIB_SECURITY
static int parse_pdcp_sn_sz(struct cperf_options *opts, const char *arg) {
  uint32_t val = 0;
  int ret = parse_uint32_t(&val, arg);
...
static int parse_pdcp_domain(struct cperf_options *opts, const char *arg) {
  struct name_id_map pdcp_domain_namemap[] = {
      {cperf_pdcp_domain_strs[RTE_SECURITY_PDCP_MODE_CONTROL],
       RTE_SECURITY_PDCP_MODE_CONTROL},
      {cperf_pdcp_domain_strs[RTE_SECURITY_PDCP_MODE_DATA],
       RTE_SECURITY_PDCP_MODE_DATA},
      {cperf_pdcp_domain_strs[RTE_SECURITY_PDCP_MODE_SHORT_MAC],
...
static int parse_pdcp_ses_hfn_en(struct cperf_options *opts,
                                 const char *arg __rte_unused) {
  opts->pdcp_ses_hfn_en = 1;
  return 0;
}

static int parse_docsis_hdr_sz(struct cperf_options *opts, const char *arg) {
  return parse_uint16_t(&opts->docsis_hdr_sz, arg);
}
#endif

static int parse_auth_iv_sz(struct cperf_options *opts, const char *arg) {
  return parse_uint16_t(&opts->auth_iv_sz, arg);
}

static int parse_aead_algo(struct cperf_options *opts, const char *arg) {
...
static int parse_aead_op(struct cperf_options *opts, const char *arg) {
  struct name_id_map aead_op_namemap[] = {
      {rte_crypto_aead_operation_strings[RTE_CRYPTO_AEAD_OP_ENCRYPT],
       RTE_CRYPTO_AEAD_OP_ENCRYPT},
      {rte_crypto_aead_operation_strings[RTE_CRYPTO_AEAD_OP_DECRYPT],
       RTE_CRYPTO_AEAD_OP_DECRYPT}};

  int id =
      get_str_key_id_mapping(aead_op_namemap, RTE_DIM(aead_op_namemap), arg);
  if (id < 0) {
    RTE_LOG(ERR, USER1,
            "invalid AEAD operation specified"
            "\n");
    return -1;
  }
...
static int parse_aead_key_sz(struct cperf_options *opts, const char *arg) {
  return parse_uint16_t(&opts->aead_key_sz, arg);
}

static int parse_aead_iv_sz(struct cperf_options *opts, const char *arg) {
  return parse_uint16_t(&opts->aead_iv_sz, arg);
}

static int parse_aead_aad_sz(struct cperf_options *opts, const char *arg) {
  return parse_uint16_t(&opts->aead_aad_sz, arg);
}

static int parse_csv_friendly(struct cperf_options *opts,
                              const char *arg __rte_unused) {
  opts->csv = 1;
  opts->silent = 1;
  return 0;
...
static int parse_pmd_cyclecount_delay_ms(struct cperf_options *opts,
                                         const char *arg) {
...
typedef int (*option_parser_t)(struct cperf_options *opts, const char *arg);
...
void cperf_options_default(struct cperf_options *opts) {
...
static int cperf_opts_parse_long(int opt_idx, struct cperf_options *opts) {
...
int cperf_options_parse(struct cperf_options *options, int argc, char **argv) {
...
static int check_cipher_buffer_length(struct cperf_options *options) {
...
#ifdef RTE_LIB_SECURITY
static int check_docsis_buffer_length(struct cperf_options *options) {
  uint32_t buffer_size, buffer_size_idx = 0;
  ...
  while (buffer_size <= options->max_buffer_size) {
    if (buffer_size < (uint32_t)(options->docsis_hdr_sz + RTE_ETHER_HDR_LEN +
                                 RTE_ETHER_CRC_LEN)) {
      RTE_LOG(ERR, USER1,
              "Some of the buffer sizes are not "
              "valid for DOCSIS\n");
      return -EINVAL;
    }
...
int cperf_options_check(struct cperf_options *options) {
  if (options->op_type == CPERF_CIPHER_ONLY || options->op_type == CPERF_DOCSIS)
    options->digest_sz = 0;

  if (options->out_of_place &&
      options->segment_sz <= options->max_buffer_size) {
    RTE_LOG(ERR, USER1,
            "Out of place mode can only work "
  ...
  if (options->segment_sz == 0) {
    options->segment_sz = options->max_buffer_size + options->digest_sz;
  ...
  if ((options->imix_distribution_count != 0) &&
      (options->imix_distribution_count != options->buffer_size_count)) {
    RTE_LOG(ERR, USER1,
            "IMIX distribution must have the same "
            "number of buffer sizes\n");
    return -EINVAL;
  }

  if (options->test == CPERF_TEST_TYPE_VERIFY && options->test_file == NULL) {
    RTE_LOG(ERR, USER1,
            "Define path to the file with test"
            " vectors.\n");
    return -EINVAL;
  }

  if (options->test == CPERF_TEST_TYPE_VERIFY &&
      options->op_type != CPERF_CIPHER_ONLY && options->test_name == NULL) {
    RTE_LOG(ERR, USER1,
            "Define test name to get the correct digest"
  ...
  if (options->test == CPERF_TEST_TYPE_VERIFY &&
      (options->inc_buffer_size != 0 || options->buffer_size_count > 1)) {
    RTE_LOG(ERR, USER1,
            "Only one buffer size is allowed when "
            "using the verify test.\n");
    return -EINVAL;
  }

  if (options->test == CPERF_TEST_TYPE_VERIFY &&
      (options->inc_burst_size != 0 || options->burst_size_count > 1)) {
    RTE_LOG(ERR, USER1,
            "Only one burst size is allowed when "
            "using the verify test.\n");
    return -EINVAL;
  }
  ...
  if (options->test == CPERF_TEST_TYPE_PMDCC &&
    ...
    RTE_LOG(ERR, USER1,
            "For pmd cyclecount benchmarks, pool size "
  ...
  if (options->op_type == CPERF_CIPHER_THEN_AUTH) {
    if (options->cipher_op != RTE_CRYPTO_CIPHER_OP_ENCRYPT &&
      ...
      RTE_LOG(ERR, USER1,
              "Option cipher then auth must use"
    ...
    if (options->cipher_op != RTE_CRYPTO_CIPHER_OP_DECRYPT &&
      ...
      RTE_LOG(ERR, USER1,
              "Option auth then cipher must use"
...
void cperf_options_dump(struct cperf_options *opts) {
  uint8_t size_idx;
