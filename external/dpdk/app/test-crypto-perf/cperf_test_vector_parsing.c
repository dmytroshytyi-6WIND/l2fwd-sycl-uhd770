...
int free_test_vector(struct cperf_test_vector *vector,
...
void show_test_vector(struct cperf_test_vector *test_vector) {
  ...
  if (test_vector->plaintext.data) {
    ...
    for (i = 0; i < test_vector->plaintext.length; ++i) {
      ...
      if (i == test_vector->plaintext.length - 1)
        printf("0x%02x", test_vector->plaintext.data[i]);
      else
        printf("0x%02x, ", test_vector->plaintext.data[i]);
    }
    printf("\n");
  ...
  if (test_vector->cipher_key.data) {
    ...
    for (i = 0; i < test_vector->cipher_key.length; ++i) {
      ...
      if (i == (uint32_t)(test_vector->cipher_key.length - 1))
        printf("0x%02x", test_vector->cipher_key.data[i]);
      else
        printf("0x%02x, ", test_vector->cipher_key.data[i]);
    }
    printf("\n");
  ...
  if (test_vector->ciphertext.data) {
    ...
    for (i = 0; i < test_vector->ciphertext.length; ++i) {
      ...
      if (i == test_vector->ciphertext.length - 1)
        printf("0x%02x", test_vector->ciphertext.data[i]);
      else
        printf("0x%02x, ", test_vector->ciphertext.data[i]);
    }
    printf("\n");
...
static int parse_values(char *tokens, uint8_t **data, uint32_t *data_length) {
  uint32_t n_tokens;
  ...
  while (tok != NULL) {
    ...
    if (n_tokens >= data_size) {
      ...
      values_resized =
          (uint8_t *)rte_realloc(values, sizeof(uint8_t) * data_size, 0);
  ...
  values_resized =
      (uint8_t *)rte_realloc(values, sizeof(uint8_t) * (n_tokens + 1), 0);
...
static int parse_entry(char *entry, struct cperf_test_vector *vector,
  ...
  if (strstr(key_token, "plaintext")) {
    ...
    vector->digest.phys_addr = rte_malloc_virt2iova(vector->digest.data);
    if (tc_found)
...
static int parse_file(struct cperf_test_vector *vector,
                      struct cperf_options *opts) {
  uint8_t tc_found = 0;
  ...
  while ((read = getline(&line, &len, fp)) != -1) {
    ...
    entry = (char *)rte_realloc(entry, sizeof(char) * strlen(line) + 1, 0);
    if (entry == NULL)
      return -1;
    ...
    if (entry[strlen(entry) - 1] == ',' || entry[strlen(entry) - 1] == '=') {
      while ((read = getline(&line, &len, fp)) != -1) {
        ...
        char *entry_extended = (char *)rte_realloc(
            entry, sizeof(char) * (strlen(line) + strlen(entry)) + 1, 0);
...
cperf_test_vector_get_from_file(struct cperf_options *opts) {
  ...
  test_vector = (struct cperf_test_vector *)rte_zmalloc(
      NULL, sizeof(struct cperf_test_vector), 0);
  if (test_vector == NULL)
    return test_vector;
