...
static void fill_single_seg_mbuf(struct rte_mbuf *m, struct rte_mempool *mp,
                                 void *obj, uint32_t mbuf_offset,
                                 uint16_t segment_sz, uint16_t headroom,
                                 uint16_t data_len) {
...
static void fill_multi_seg_mbuf(struct rte_mbuf *m, struct rte_mempool *mp,
                                void *obj, uint32_t mbuf_offset,
                                uint16_t segment_sz, uint16_t headroom,
                                uint16_t data_len, uint16_t segments_nb) {
  uint16_t mbuf_hdr_size = sizeof(struct rte_mbuf);
  uint16_t remaining_segments = segments_nb;
  struct rte_mbuf *next_mbuf;
  rte_iova_t next_seg_phys_addr =
      rte_mempool_virt2iova(obj) + mbuf_offset + mbuf_hdr_size;
...
static void mempool_asym_obj_init(struct rte_mempool *mp,
                                  __rte_unused void *opaque_arg, void *obj,
...
static void mempool_obj_init(struct rte_mempool *mp, void *opaque_arg,
                             void *obj, __rte_unused unsigned int i) {
  struct obj_params *params = opaque_arg;
  struct rte_crypto_op *op = obj;
  struct rte_mbuf *m =
      (struct rte_mbuf *)((uint8_t *)obj + params->src_buf_offset);
  /* Set crypto operation */
  ...
  if (params->segments_nb == 1)
    fill_single_seg_mbuf(m, mp, obj, params->src_buf_offset, params->segment_sz,
                         params->headroom_sz, params->data_len);
  else
    fill_multi_seg_mbuf(m, mp, obj, params->src_buf_offset, params->segment_sz,
                        params->headroom_sz, params->data_len,
                        params->segments_nb);
  ...
  if (params->dst_buf_offset) {
    ...
    fill_single_seg_mbuf(m, mp, obj, params->dst_buf_offset, params->segment_sz,
                         params->headroom_sz, params->data_len);
    op->sym->m_dst = m;
  } else
    op->sym->m_dst = NULL;
...
int cperf_alloc_common_memory(const struct cperf_options *options,
  ...
  if (options->op_type == CPERF_ASYM_MODEX) {
    ...
    *pool = rte_crypto_op_pool_create(
        pool_name, RTE_CRYPTO_OP_TYPE_ASYMMETRIC, options->pool_sz,
        RTE_MEMPOOL_CACHE_MAX_SIZE, 0, rte_socket_id());
    if (*pool == NULL) {
  ...
  if (options->aead_algo == RTE_CRYPTO_AEAD_AES_CCM) {
    ...
    crypto_op_private_size = extra_op_priv_size +
                             test_vector->cipher_iv.length +
                             test_vector->auth_iv.length +
                             test_vector->aead_iv.length + options->aead_aad_sz;
  }
  ...
  *src_buf_offset = crypto_op_total_size_padded;
  ...
  struct obj_params params = {
      ...
      .data_len =
          options->segment_sz - options->headroom_sz - options->tailroom_sz,
      .segments_nb = segments_nb,
      .src_buf_offset = crypto_op_total_size_padded,
      .dst_buf_offset = 0};

  if (options->out_of_place) {
    *dst_buf_offset = *src_buf_offset + (mbuf_size * segments_nb);
    params.dst_buf_offset = *dst_buf_offset;
    /* Destination buffer will be one segment only */
    obj_size += max_size + sizeof(struct rte_mbuf);
  }

  *pool = rte_mempool_create_empty(pool_name, options->pool_sz, obj_size, 512,
                                   0, rte_socket_id(), 0);
  if (*pool == NULL) {
    RTE_LOG(ERR, USER1, "Cannot allocate mempool for device %u\n", dev_id);
    return -1;
  ...
  ret = rte_mempool_set_ops_byname(*pool, mp_ops_name, NULL);
  if (ret != 0) {
