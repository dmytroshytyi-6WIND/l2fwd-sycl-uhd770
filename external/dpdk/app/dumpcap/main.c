...
static const char *version(void) {
  ...
  snprintf(str, sizeof(str), "%s 1.0 (%s)\n", progname, rte_version());
  return str;
}
...
static unsigned long get_uint(const char *arg, const char *name,
  ...
  if (*arg == '\0' || *endp != '\0')
    rte_exit(EXIT_FAILURE, "Specified %s \"%s\" is not a valid number\n", name,
             arg);
  if (limit && u > limit)
...
static void auto_stop(char *opt) {
  ...
  value = strchr(opt, ':');
  if (value == NULL)
    rte_exit(EXIT_FAILURE, "Missing colon in auto stop parameter\n");
  ...
  *value++ = '\0';
  if (strcmp(opt, "duration") == 0) {
    ...
    if (*value == '\0' || *endp != '\0' || interval <= 0)
      rte_exit(EXIT_FAILURE, "Invalid duration \"%s\"\n", value);
    ...
    rte_exit(EXIT_FAILURE, "Unknown autostop parameter \"%s\"\n", opt);
  }
}
...
static void compile_filter(void) {
  ...
  if (bpf_prm == NULL)
    rte_exit(EXIT_FAILURE, "bpf convert failed: %s(%d)\n",
...
static void parse_opts(int argc, char **argv) {
  ...
  for (;;) {
    c = getopt_long(argc, argv, "a:b:c:dDf:ghi:nN:pPqs:vw:", long_options,
                    &option_index);
    if (c == -1)
      break;
    ...
    case 'a':
      ...
      break;
    case 'b':
      rte_exit(EXIT_FAILURE, "multiple files not implemented\n");
      break;
    case 'c':
...
static void signal_handler(int sig_num __rte_unused) {
...
static void report_packet_stats(dumpcap_out_t out) {
  ...
  TAILQ_FOREACH(intf, &interfaces, next) {
    ...
    if (use_pcapng)
      rte_pcapng_write_stats(out.pcapng, intf->port, NULL, start_time, end_time,
                             ifrecv, ifdrop);
...
static void dpdk_init(void) {
  static const char *const args[] = {"dumpcap", "--proc-type", "secondary",
                                     "--log-level", "notice"
...
static struct rte_mempool *create_mempool(void) {
  ...
  mp = rte_pktmbuf_pool_create_by_ops(
      pool_name, num_mbufs, MBUF_POOL_CACHE_SIZE, 0,
      rte_pcapng_mbuf_size(snaplen), rte_socket_id(), "ring_mp_sc");
  if (mp == NULL)
    rte_exit(EXIT_FAILURE, "Mempool (%s) creation failed: %s\n", pool_name,
             rte_strerror(rte_errno));
...
static dumpcap_out_t create_output(void) {
  ...
  if (output_name == NULL) {
    ...
    snprintf(tmp_path, sizeof(tmp_path), "/tmp/%s_%u_%s_%s.%s", progname,
             intf->port, intf->name, ts, use_pcapng ? "pcapng" : "pcap");
  ...
  if (use_pcapng) {
    ...
    ret.pcapng = rte_pcapng_fdopen(fd, os, NULL, version(), capture_comment);
    if (ret.pcapng == NULL)
...
static void enable_pdump(struct rte_ring *r, struct rte_mempool *mp) {
  ...
  TAILQ_FOREACH(intf, &interfaces, next) {
    if (promiscuous_mode) {
      ...
      if (ret != 0)
        fprintf(stderr, "port %u set promiscuous enable failed: %d\n",
                intf->port, ret);
    }

    ret = rte_pdump_enable_bpf(intf->port, RTE_PDUMP_ALL_QUEUES, flags, snaplen,
                               r, mp, bpf_prm);
    if (ret < 0)
      rte_exit(EXIT_FAILURE, "Packet dump enable failed: %s\n",
               rte_strerror(-ret));
  }
}
...
static ssize_t pcap_write_packets(pcap_dumper_t *dumper,
...
int main(int argc, char **argv) {
  ...
  while (!__atomic_load_n(&quit_signal, __ATOMIC_RELAXED)) {
    ...
    if (stop.duration != 0 && create_timestamp() - start_time > stop.duration)
      break;
  }
