...
static struct mcast_group_params mcast_group_table[] = {
    {RTE_IPV4(224, 0, 0, 101), 0x1}, {RTE_IPV4(224, 0, 0, 102), 0x2},
    {RTE_IPV4(224, 0, 0, 103), 0x3}, {RTE_IPV4(224, 0, 0, 104), 0x4},
    {RTE_IPV4(224, 0, 0, 105), 0x5}, {RTE_IPV4(224, 0, 0, 106), 0x6},
    {RTE_IPV4(224, 0, 0, 107), 0x7}, {RTE_IPV4(224, 0, 0, 108), 0x8},
    {RTE_IPV4(224, 0, 0, 109), 0x9}, {RTE_IPV4(224, 0, 0, 110), 0xA},
    {RTE_IPV4(224, 0, 0, 111), 0xB}, {RTE_IPV4(224, 0, 0, 112), 0xC},
    {RTE_IPV4(224, 0, 0, 113), 0xD}, {RTE_IPV4(224, 0, 0, 114), 0xE},
    {RTE_IPV4(224, 0, 0, 115), 0xF},
};

/* Send burst of packets on an output interface */
static void send_burst(struct lcore_queue_conf *qconf, uint16_t port) {
...
static inline void mcast_send_pkt(struct rte_mbuf *pkt,
  ...
  ethdr = (struct rte_ether_hdr *)rte_pktmbuf_prepend(pkt,
...
static inline void mcast_forward(struct rte_mbuf *m,
                                 struct lcore_queue_conf *qconf) {
  struct rte_mbuf *mc;
  ...
  iphdr = (struct rte_ipv4_hdr *)rte_pktmbuf_adj(
  ...
  use_clone = (port_num <= MCAST_CLONE_PORTS && m->nb_segs <= MCAST_CLONE_SEGS);
...
static inline void send_timeout_burst(struct lcore_queue_conf *qconf) {
  uint64_t cur_tsc;
  uint16_t portid;
...
static uint32_t parse_portmask(const char *portmask) {
  char *end = NULL;
  unsigned long pm;
...
static int parse_nqueue(const char *q_arg) {
  ...
  if (errno != 0 || end == NULL || *end != '\0' || n == 0 ||
      n >= MAX_RX_QUEUE_PER_LCORE)
    return -1;
...
static int init_mcast_hash(void) {
  uint32_t i;
  ...
  for (i = 0; i < RTE_DIM(mcast_group_table); i++) {
    if (rte_fbk_hash_add_key(mcast_hash, mcast_group_table[i].ip,
...
int main(int argc, char **argv) {
  ...
  packet_pool = rte_pktmbuf_pool_create("packet_pool", NB_PKT_MBUF, 32, 0,
                                        PKT_MBUF_DATA_SIZE, rte_socket_id());

  if (packet_pool == NULL)
    rte_exit(EXIT_FAILURE, "Cannot init packet mbuf pool\n");

  header_pool = rte_pktmbuf_pool_create("header_pool", NB_HDR_MBUF, 32, 0,
                                        HDR_MBUF_DATA_SIZE, rte_socket_id());

  if (header_pool == NULL)
    rte_exit(EXIT_FAILURE, "Cannot init header mbuf pool\n");

  clone_pool = rte_pktmbuf_pool_create("clone_pool", NB_CLONE_MBUF, 32, 0, 0,
                                       rte_socket_id());
  ...
  RTE_ETH_FOREACH_DEV(portid) {
    ...
    if (ret < 0)
      rte_exit(EXIT_FAILURE, "rte_eth_allmulticast_enable: err=%d, port=%d\n",
               ret, portid);
    /* Start device */
