...
int main(int argc, char **argv) {
  ...
  RTE_ETH_FOREACH_DEV(portid) {
    ...
    printf("Creating queues: nb_rxq=%d nb_txq=%u... ", nb_rx_queue, n_tx_queue);
  ...
  node_patterns =
      malloc((MAX_RX_QUEUE_PER_LCORE + nb_patterns) * sizeof(*node_patterns));
  if (!node_patterns)
    return -ENOMEM;
  ...
  for (i = 0; i < IPV4_L3FWD_LPM_NUM_ROUTES; i++) {
    ...
    if (ret < 0)
      rte_exit(EXIT_FAILURE, "Unable to add ip4 route %s to graph\n",
               route_str);

    memcpy(rewrite_data, val_eth + dst_port, rewrite_len);

    /* Add next hop rewrite data for id 'i' */
    ret = rte_node_ip4_rewrite_add(i, rewrite_data, rewrite_len, dst_port);
    if (ret < 0)
      rte_exit(EXIT_FAILURE,
