...
int rte_ethtool_get_drvinfo(uint16_t port_id, struct ethtool_drvinfo *drvinfo) {
  ...
  if (bus && !strcmp(bus->name, "pci")) {
    ...
    snprintf(drvinfo->bus_info, sizeof(drvinfo->bus_info), "%04x:%02x:%02x.%x",
...
int rte_ethtool_net_stop(uint16_t port_id) { return rte_eth_dev_stop(port_id); }

int rte_ethtool_net_get_mac_addr(uint16_t port_id,
...
int rte_ethtool_net_set_mac_addr(uint16_t port_id,
                                 struct rte_ether_addr *addr) {
  if (addr == NULL)
    return -EINVAL;
...
int rte_ethtool_net_validate_addr(uint16_t port_id __rte_unused,
                                  struct rte_ether_addr *addr) {
  if (addr == NULL)
    return -EINVAL;
...
#ifdef RTE_NET_IXGBE
    rte_pmd_ixgbe_set_vf_rxmode(port_id, vf, RTE_ETH_VMDQ_ACCEPT_UNTAG, 0);
#endif
  }
...
int rte_ethtool_set_ringparam(uint16_t port_id,
  ...
  stat =
      rte_eth_rx_queue_setup(port_id, 0, ring_param->rx_pending,
