...
cmdline_parse_token_string_t pcmd_ringparam_token_cmd =
    TOKEN_STRING_INITIALIZER(struct pcmd_intintint_params, cmd, "ringparam");
...
cmdline_parse_token_string_t pcmd_pause_token_opt =
    TOKEN_STRING_INITIALIZER(struct pcmd_intstr_params, opt, "all#tx#rx#none");
...
static void pcmd_quit_callback(__rte_unused void *ptr_params,
                               struct cmdline *ctx,
                               __rte_unused void *ptr_data) {
  cmdline_quit(ctx);
}

static void pcmd_drvinfo_callback(__rte_unused void *ptr_params,
...
static void pcmd_link_callback(__rte_unused void *ptr_params,
                               __rte_unused struct cmdline *ctx,
                               __rte_unused void *ptr_data) {
  uint16_t id_port;
  int stat_port;
...
static void pcmd_regs_callback(void *ptr_params,
                               __rte_unused struct cmdline *ctx,
                               __rte_unused void *ptr_data) {
  ...
  if (len_regs > 0) {
    ...
    if (!rte_ethtool_get_regs(params->port, &regs, buf_data)) {
      ...
      if (fp_regs == NULL) {
        ...
        if ((int)fwrite(buf_data, 1, len_regs, fp_regs) != len_regs)
...
static void pcmd_eeprom_callback(void *ptr_params,
                                 __rte_unused struct cmdline *ctx,
                                 __rte_unused void *ptr_data) {
  struct pcmd_intstr_params *params = ptr_params;
  ...
  if (len_eeprom > 0) {
    ...
    for (pos_eeprom = 0; pos_eeprom < len_eeprom;
      ...
      if (pos_eeprom + EEPROM_DUMP_CHUNKSIZE > len_eeprom)
        ...
        info_eeprom.len = EEPROM_DUMP_CHUNKSIZE;
      stat = rte_ethtool_get_eeprom(params->port, &info_eeprom, bytes_eeprom);
      if (stat != 0) {
        printf("EEPROM read error %i\n", stat);
        break;
      }
      if (fwrite(bytes_eeprom, 1, info_eeprom.len, fp_eeprom) !=
          info_eeprom.len) {
        printf("Error writing '%s'\n", params->opt);
        break;
      }
...
static void pcmd_module_eeprom_callback(void *ptr_params,
                                        __rte_unused struct cmdline *ctx,
                                        __rte_unused void *ptr_data) {
  struct pcmd_intstr_params *params = ptr_params;
  struct ethtool_eeprom info_eeprom;
  ...
  stat =
      rte_ethtool_get_module_eeprom(params->port, &info_eeprom, bytes_eeprom);
  if (stat != 0) {
  ...
  if (fwrite(bytes_eeprom, 1, info_eeprom.len, fp_eeprom) != info_eeprom.len) {
    printf("Error writing '%s'\n", params->opt);
  }
  fclose(fp_eeprom);
}

static void pcmd_pause_callback(void *ptr_params,
                                __rte_unused struct cmdline *ctx,
                                void *ptr_data) {
  struct pcmd_intstr_params *params = ptr_params;
...
static void pcmd_open_callback(__rte_unused void *ptr_params,
                               __rte_unused struct cmdline *ctx,
                               __rte_unused void *ptr_data) {
...
static void pcmd_stop_callback(__rte_unused void *ptr_params,
                               __rte_unused struct cmdline *ctx,
                               __rte_unused void *ptr_data) {
  struct pcmd_int_params *params = ptr_params;
  int stat;
...
static void pcmd_rxmode_callback(void *ptr_params,
                                 __rte_unused struct cmdline *ctx,
                                 __rte_unused void *ptr_data) {
  struct pcmd_intstr_params *params = ptr_params;
  int stat;
...
static void pcmd_macaddr_callback(void *ptr_params,
                                  __rte_unused struct cmdline *ctx,
                                  void *ptr_data) {
  ...
  if (ptr_data != NULL) {
    lock_port(params->port);
    stat = rte_ethtool_net_set_mac_addr(params->port, &params->mac);
    ...
    if (stat == 0) {
      printf("Port %i MAC Address: " RTE_ETHER_ADDR_PRT_FMT "\n", params->port,
             RTE_ETHER_ADDR_BYTES(&mac_addr));
      return;
    }
  }

  printf("Port %i: Error %s\n", params->port, strerror(-stat));
}

static void pcmd_mtu_callback(void *ptr_params,
                              __rte_unused struct cmdline *ctx,
                              __rte_unused void *ptr_data) {
  struct pcmd_intstr_params *params = ptr_params;
  int stat;
  ...
  if (*ptr_parse_end != '\0' || new_mtu < RTE_ETHER_MIN_MTU ||
...
static void pcmd_portstats_callback(__rte_unused void *ptr_params,
  ...
  if (stat == 0) {
    ...
    printf("   In: %" PRIu64 " (%" PRIu64 " bytes)\n"
           ...
           "  Err: %" PRIu64 "\n",
           stat_info.ipackets, stat_info.ibytes, stat_info.opackets,
           stat_info.obytes, stat_info.ierrors + stat_info.oerrors);
  } else if (stat == -ENOTSUP)
    printf("Port %i: Operation not supported\n", params->port);
  else
  ...
  if (ptr_data == NULL) {
    ...
    if (stat == 0) {
      printf("Port %i ring parameters\n"
             "  Rx Pending: %i (%i max)\n"
             "  Tx Pending: %i (%i max)\n",
             params->port, ring_data.rx_pending, ring_data.rx_max_pending,
             ring_data.tx_pending, ring_data.tx_max_pending);
    }
  } else {
...
static void pcmd_vlan_callback(__rte_unused void *ptr_params,
  ...
  if (strcasecmp("add", params->mode) == 0) {
    stat = rte_ethtool_net_vlan_rx_add_vid(params->port, params->vid);
    if (stat == 0)
      printf("VLAN vid %i added\n", params->vid);

  } else if (strcasecmp("del", params->mode) == 0) {
    stat = rte_ethtool_net_vlan_rx_kill_vid(params->port, params->vid);
    if (stat == 0)
...
cmdline_parse_inst_t pcmd_regs = {
    ...
    .tokens = {(void *)&pcmd_regs_token_cmd, (void *)&pcmd_intstr_token_port,
               (void *)&pcmd_intstr_token_opt, NULL},
};
cmdline_parse_inst_t pcmd_eeprom = {
    .f = pcmd_eeprom_callback,
    .data = NULL,
    .help_str = "eeprom <port_id> <filename>\n    Dump EEPROM to file",
    .tokens = {(void *)&pcmd_eeprom_token_cmd, (void *)&pcmd_intstr_token_port,
               (void *)&pcmd_intstr_token_opt, NULL},
};
cmdline_parse_inst_t pcmd_module_eeprom = {
    ...
    .tokens = {(void *)&pcmd_module_eeprom_token_cmd,
               (void *)&pcmd_intstr_token_port, (void *)&pcmd_intstr_token_opt,
               NULL},
};
cmdline_parse_inst_t pcmd_pause_noopt = {
    ...
    .tokens = {(void *)&pcmd_pause_token_cmd, (void *)&pcmd_pause_token_port,
               NULL},
};
cmdline_parse_inst_t pcmd_pause = {
    .f = pcmd_pause_callback,
    .data = NULL,
    .help_str = "pause <port_id> <all|tx|rx|none>\n     Pause/unpause port",
    .tokens = {(void *)&pcmd_pause_token_cmd, (void *)&pcmd_pause_token_port,
               (void *)&pcmd_pause_token_opt, NULL},
};
cmdline_parse_inst_t pcmd_open = {
    .f = pcmd_open_callback,
    .data = NULL,
    .help_str = "open <port_id>\n     Open port",
    .tokens = {(void *)&pcmd_open_token_cmd, (void *)&pcmd_int_token_port,
               NULL},
};
cmdline_parse_inst_t pcmd_stop = {
    ...
    .tokens = {(void *)&pcmd_stop_token_cmd, (void *)&pcmd_int_token_port,
               NULL},
};
cmdline_parse_inst_t pcmd_rxmode = {
    ...
    .tokens = {(void *)&pcmd_rxmode_token_cmd, (void *)&pcmd_int_token_port,
               NULL},
};
cmdline_parse_inst_t pcmd_macaddr_get = {
    ...
    .help_str = "macaddr <port_id>\n"
                "     Get MAC address",
    .tokens = {(void *)&pcmd_macaddr_token_cmd, (void *)&pcmd_intstr_token_port,
               NULL},
};
cmdline_parse_inst_t pcmd_macaddr = {
    .f = pcmd_macaddr_callback,
    .data = (void *)0x01,
    .help_str = "macaddr <port_id> <mac_addr>\n"
                "     Set MAC address",
    .tokens = {(void *)&pcmd_macaddr_token_cmd, (void *)&pcmd_intmac_token_port,
               (void *)&pcmd_intmac_token_mac, NULL},
};
cmdline_parse_inst_t pcmd_mtu = {
    ...
    .help_str = "mtu <port_id> <mtu_value>\n"
                "     Change MTU",
    .tokens = {(void *)&pcmd_mtu_token_cmd, (void *)&pcmd_intstr_token_port,
               (void *)&pcmd_intstr_token_opt, NULL},
};
cmdline_parse_inst_t pcmd_portstats = {
    ...
    .tokens = {(void *)&pcmd_portstats_token_cmd, (void *)&pcmd_int_token_port,
               NULL},
};
cmdline_parse_inst_t pcmd_ringparam = {
    ...
    .tokens = {(void *)&pcmd_ringparam_token_cmd,
               (void *)&pcmd_intintint_token_port, NULL},
};
cmdline_parse_inst_t pcmd_ringparam_set = {
    ...
    .tokens = {(void *)&pcmd_ringparam_token_cmd,
               ...
               (void *)&pcmd_intintint_token_rx, NULL},
};
cmdline_parse_inst_t pcmd_validate = {
    ...
    .tokens = {(void *)&pcmd_validate_token_cmd, (void *)&pcmd_intmac_token_mac,
               NULL},
};
cmdline_parse_inst_t pcmd_vlan = {
    ...
    .help_str = "vlan <port_id> <add|del> <vlan_id>\n"
                "     Add/remove VLAN id",
    .tokens = {(void *)&pcmd_vlan_token_cmd, (void *)&pcmd_vlan_token_port,
               (void *)&pcmd_vlan_token_mode, (void *)&pcmd_vlan_token_vid,
               NULL},
};
