...
static unsigned bits_count(uint64_t bitmask) {
  unsigned count = 0;
 ...
 */
static int parse_set(const char *input, rte_cpuset_t *cpusetp) {
  unsigned idx;
 ...
 */
static int parse_l3ca(const char *l3ca) {
  unsigned idx = 0;
  ...
  do {
    ...
    rte_memcpy(&m_config[idx].cpumask, &cpuset, sizeof(rte_cpuset_t));
...
static int check_cpus_overlapping(void) {
  unsigned i = 0;
  unsigned j = 0;
  rte_cpuset_t mask;
  ...
  for (i = 0; i < m_config_count; i++) {
    for (j = i + 1; j < m_config_count; j++) {
      RTE_CPU_AND(&mask, &m_config[i].cpumask, &m_config[j].cpumask);
...
static int check_cbm_len_and_contention(void) {
  unsigned i = 0;
  uint64_t mask = 0;
...
static int check_and_select_classes(unsigned cos_id_map[][PQOS_MAX_SOCKETS]) {
  unsigned i = 0;
  unsigned j = 0;
...
static int configure_cat(unsigned cos_id_map[][PQOS_MAX_SOCKETS]) {
  unsigned phy_pkg_id = 0;
  unsigned cpu_id = 0;
  unsigned cos_id = 0;
  unsigned i = 0;
...
static int parse_args(int argc, char **argv) {
  ...
  static struct option lgopts[] = {{"l3ca", required_argument, 0, 0},
                                   {NULL, 0, 0, 0}};
...
static void print_cmd_line_config(void) {
  ...
  for (i = 0; i < m_config_count; i++) {
    ...
    for (j = 0; j < PQOS_MAX_CORES; j++) {
      ...
      len += snprintf(cpustr + len, sizeof(cpustr) - len - 1, "%u,", j);
    ...
    if (m_config[i].cdp == 1) {
      printf("PQOS: CPUs: %s cMASK: 0x%llx, dMASK: "
             "0x%llx\n",
             cpustr, (unsigned long long)m_config[i].code_mask,
 ...
 */
static void print_cat_config(void) {
...
#if PQOS_VERSION <= 103
    ret = pqos_cpu_get_cores(m_cpu, m_sockets[i], PQOS_MAX_SOCKET_CORES,
                             &lcount, &lcores[0]);
    if (ret != PQOS_RETVAL_OK) {
#else
    lcores = pqos_cpu_get_cores(m_cpu, m_sockets[i], &lcount);
...
#if PQOS_VERSION <= 103
  ret = pqos_cpu_get_sockets(m_cpu, PQOS_MAX_SOCKETS, &m_sock_count, m_sockets);
  if (ret != PQOS_RETVAL_OK) {
#else
