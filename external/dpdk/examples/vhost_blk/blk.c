...
static void vhost_strcpy_pad(void *dst, const char *src, size_t size, int pad) {
  size_t len;
...
static int vhost_bdev_blk_readwrite(struct vhost_block_dev *bdev,
                                    struct vhost_blk_task *task,
                                    uint64_t lba_512,
                                    __rte_unused uint32_t xfer_len) {
  uint32_t i;
  uint64_t offset;
  uint32_t nbytes = 0;
...
int vhost_bdev_process_blk_commands(struct vhost_block_dev *bdev,
  ...
  case VIRTIO_BLK_T_IN:
    if (unlikely(task->data_len == 0 || (task->data_len & (512 - 1)) != 0)) {
      fprintf(stderr,
              ...
              "(req_idx = %" PRIu16 ").\n",
              task->req->type ? "WRITE" : "READ", task->req_idx);
      return VIRTIO_BLK_S_UNSUPP;
    }

    task->dxfer_dir = BLK_DIR_FROM_DEV;
    vhost_bdev_blk_readwrite(bdev, task, task->req->sector, task->data_len);
    break;
  ...
  case VIRTIO_BLK_T_GET_ID:
    ...
    vhost_strcpy_pad(task->iovs[0].iov_base, bdev->product_name, used_len, ' ');
    break;
  default:
