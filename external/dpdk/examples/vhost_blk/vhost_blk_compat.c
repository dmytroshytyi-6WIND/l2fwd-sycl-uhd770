...
static int vhost_blk_get_config(struct vhost_block_dev *bdev, uint8_t *config,
                                uint32_t len) {
...
static enum rte_vhost_msg_result extern_vhost_pre_msg_handler(int vid,
                                                              void *_msg) {
  char path[PATH_MAX];
  ...
  case VHOST_USER_GET_CONFIG: {
    ...
    rc = vhost_blk_get_config(ctrlr->bdev, msg->payload.cfg.region,
...
static enum rte_vhost_msg_result extern_vhost_post_msg_handler(int vid,
                                                               void *_msg) {
  char path[PATH_MAX];
  struct vhost_blk_ctrlr *ctrlr;
  struct vhost_user_msg *msg = _msg;
