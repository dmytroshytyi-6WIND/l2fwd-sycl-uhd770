...
static uint64_t gpa_to_vva(struct vhost_blk_ctrlr *ctrlr, uint64_t gpa,
                           uint64_t *len) {
...
static void enqueue_task(struct vhost_blk_task *task) {
  struct vhost_blk_queue *vq = task->vq;
  struct vring_used *used = vq->vring.used;

  rte_vhost_set_last_inflight_io_split(task->ctrlr->vid, vq->id, task->req_idx);
  ...
  rte_vhost_clr_inflight_desc_split(task->ctrlr->vid, vq->id, used->idx,
                                    task->req_idx);
...
static void enqueue_task_packed(struct vhost_blk_task *task) {
  struct vhost_blk_queue *vq = task->vq;
...
static bool descriptor_has_next_packed(struct vring_packed_desc *cur_desc) {
  return !!(cur_desc->flags & VRING_DESC_F_NEXT);
}

static bool descriptor_has_next_split(struct vring_desc *cur_desc) {
  return !!(cur_desc->flags & VRING_DESC_F_NEXT);
}

static int desc_payload_to_iovs(struct vhost_blk_ctrlr *ctrlr,
                                struct iovec *iovs, uint32_t *iov_index,
                                uintptr_t payload, uint64_t remaining) {
  void *vva;
  uint64_t len;
  ...
  do {
    ...
    len = remaining;
    vva = (void *)(uintptr_t)gpa_to_vva(ctrlr, payload, &len);
    if (!vva || !len) {
...
static struct vring_desc *vring_get_next_desc(struct vhost_blk_queue *vq,
...
static int setup_iovs_from_descs_split(struct vhost_blk_ctrlr *ctrlr,
                                       struct vhost_blk_queue *vq,
                                       uint16_t req_idx, struct iovec *iovs,
                                       uint32_t *iovs_idx, uint32_t *payload) {
  ...
  do {
    ...
    if (desc_payload_to_iovs(ctrlr, iovs, iovs_idx, desc->addr, desc->len) !=
        0) {
...
static int setup_iovs_from_descs_packed(struct vhost_blk_ctrlr *ctrlr,
                                        struct vhost_blk_queue *vq,
                                        uint16_t req_idx, struct iovec *iovs,
                                        uint32_t *iovs_idx, uint32_t *payload) {
...
static int setup_iovs_from_inflight_desc(struct vhost_blk_ctrlr *ctrlr,
                                         struct vhost_blk_queue *vq,
                                         uint16_t req_idx, struct iovec *iovs,
                                         uint32_t *iovs_idx,
                                         uint32_t *payload) {
...
static void process_blk_task(struct vhost_blk_task *task) {
  uint32_t payload = 0;
  ...
  if (task->vq->packed_ring) {
    ...
    if (resubmit_inflight != NULL && resubmit_inflight->resubmit_list != NULL) {
      if (setup_iovs_from_inflight_desc(task->ctrlr, task->vq, task->req_idx,
                                        task->iovs, &task->iovs_cnt,
                                        &payload)) {
        fprintf(stderr, "Failed to setup iovs\n");
        return;
      }
    } else {
      if (setup_iovs_from_descs_packed(task->ctrlr, task->vq, task->req_idx,
                                       task->iovs, &task->iovs_cnt, &payload)) {
        fprintf(stderr, "Failed to setup iovs\n");
        return;
      }
    }
  } else {
    if (setup_iovs_from_descs_split(task->ctrlr, task->vq, task->req_idx,
                                    task->iovs, &task->iovs_cnt, &payload)) {
      fprintf(stderr, "Failed to setup iovs\n");
      return;
    }
  ...
  /* Transport data len */
  task->data_len =
      payload - task->iovs[0].iov_len - task->iovs[task->iovs_cnt - 1].iov_len;
...
static void blk_task_init(struct vhost_blk_task *task) {
  task->iovs_cnt = 0;
  task->data_len = 0;
  task->req = NULL;
  task->status = NULL;
...
static void submit_inflight_vq(struct vhost_blk_queue *vq) {
  struct rte_vhost_ring_inflight *inflight_ring;
  struct rte_vhost_resubmit_info *resubmit_inflight;
  ...
  if (resubmit_inflight == NULL || resubmit_inflight->resubmit_num == 0)
    return;
  ...
  while (resubmit_inflight->resubmit_num-- > 0) {
    ...
    desc_idx =
        resubmit_inflight->resubmit_list[resubmit_inflight->resubmit_num].index;
...
vhost_blk_vq_get_desc_chain_buffer_id(struct vhost_blk_queue *vq,
  ...
  struct vring_packed_desc *desc = &vq->vring.desc_packed[vq->last_avail_idx];
...
static uint16_t vq_get_desc_idx(struct vhost_blk_queue *vq) {
  uint16_t desc_idx;
...
static int vhost_blk_vq_is_avail(struct vhost_blk_queue *vq) {
  if (vq->packed_ring) {
    uint16_t flags = vq->vring.desc_packed[vq->last_avail_idx].flags;
...
static void process_vq(struct vhost_blk_queue *vq) {
  struct vhost_blk_task *task;
  ...
  if (vq->packed_ring) {
    while (vhost_blk_vq_is_avail(vq)) {
      ...
      task_idx =
          vhost_blk_vq_get_desc_chain_buffer_id(vq, &req_idx, &chain_num);
      task = &vq->tasks[task_idx];
      ...
      rte_vhost_set_inflight_desc_packed(task->ctrlr->vid, vq->id,
                                         task->req_idx, last_idx,
                                         &task->inflight_idx);
    ...
    while (vhost_blk_vq_is_avail(vq)) {
      ...
      blk_task_init(task);
      rte_vhost_set_inflight_desc_split(task->ctrlr->vid, vq->id,
                                        task->req_idx);
      process_blk_task(task);
    }
  }
...
static void *ctrlr_worker(void *arg) {
...
static int alloc_task_pool(struct vhost_blk_ctrlr *ctrlr) {
  struct vhost_blk_queue *vq;
  int i, j;
  ...
  for (i = 0; i < NUM_OF_BLK_QUEUES; i++) {
    ...
    vq->tasks =
        rte_zmalloc(NULL, sizeof(struct vhost_blk_task) * vq->vring.size, 0);
    if (!vq->tasks) {
...
static void free_task_pool(struct vhost_blk_ctrlr *ctrlr) {
  int i;
...
static int new_device(int vid) {
  ...
  ret = rte_vhost_get_negotiated_protocol_features(vid, &protocol_features);
  if (ret) {
    fprintf(stderr, "Failed to get the negotiated protocol features\n");
    return -1;
  }
  inflight_shmfd =
  ...
  for (i = 0; i < NUM_OF_BLK_QUEUES; i++) {
    ...
    assert(rte_vhost_get_vhost_vring(ctrlr->vid, i, &vq->vring) == 0);
    assert(rte_vhost_get_vring_base(ctrlr->vid, i, &vq->last_avail_idx,
                                    &vq->last_used_idx) == 0);
    ...
    if (packed_ring && inflight_shmfd) {
      ...
      assert(rte_vhost_get_vring_base_from_inflight(
                 ctrlr->vid, i, &vq->last_avail_idx, &vq->last_used_idx) == 0);

      vq->avail_wrap_counter = vq->last_avail_idx & (1 << 15);
      vq->last_avail_idx = vq->last_avail_idx & 0x7fff;
      vq->used_wrap_counter = vq->last_used_idx & (1 << 15);
      vq->last_used_idx = vq->last_used_idx & 0x7fff;
    }
  ...
  if (rte_ctrl_thread_create(&tid, "vhostblk-ctrlr", NULL, &ctrlr_worker,
                             ctrlr) != 0) {
...
static void destroy_device(int vid) {
  ...
  for (i = 0; i < NUM_OF_BLK_QUEUES; i++) {
    vq = &ctrlr->queues[i];
    if (vq->packed_ring) {
      vq->last_avail_idx |= (vq->avail_wrap_counter << 15);
      vq->last_used_idx |= (vq->used_wrap_counter << 15);
    }

    rte_vhost_set_vring_base(ctrlr->vid, i, vq->last_avail_idx,
                             vq->last_used_idx);
  }
...
vhost_blk_bdev_construct(const char *bdev_name, const char *bdev_serial,
                         uint32_t blk_size, uint64_t blk_cnt, bool wce_enable) {
  ...
  snprintf(bdev->product_name, sizeof(bdev->product_name), "%s", bdev_serial);
...
vhost_blk_ctrlr_construct(const char *ctrlr_name) {
  ...
  ctrlr->bdev =
      vhost_blk_bdev_construct("malloc0", "vhost_blk_malloc0", 4096, 32768, 0);
  if (!ctrlr->bdev) {
    rte_free(ctrlr);
    rte_vhost_driver_unregister(dev_pathname);
    return NULL;
...
static void vhost_blk_ctrlr_destroy(struct vhost_blk_ctrlr *ctrlr) {
  if (ctrlr->bdev != NULL) {
