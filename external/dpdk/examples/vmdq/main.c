 ...
 */
#define NUM_MBUFS_PER_PORT                                                     \
  (MAX_QUEUES * RTE_MAX(RTE_TEST_RX_DESC_DEFAULT, RTE_TEST_TX_DESC_DEFAULT))
...
static inline int port_init(uint16_t port, struct rte_mempool *mbuf_pool) {
  ...
  retval = rte_eth_dev_adjust_nb_rx_tx_desc(port, &rxRingSize, &txRingSize);
  if (retval != 0)
    return retval;
  ...
  for (q = 0; q < rxRings; q++) {
    retval = rte_eth_rx_queue_setup(
        port, q, rxRingSize, rte_eth_dev_socket_id(port), rxconf, mbuf_pool);
    if (retval < 0) {
  ...
  printf("Port %u MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8
         " %02" PRIx8 " %02" PRIx8 "\n",
         (unsigned)port, RTE_ETHER_ADDR_BYTES(&vmdq_ports_eth_addr[port]));
  ...
  for (q = 0; q < num_pools; q++) {
    ...
    retval = rte_eth_dev_mac_addr_add(port, &mac, q + vmdq_pool_base);
    if (retval) {
...
static int vmdq_parse_num_pools(const char *q_arg) {
  char *end = NULL;
  int n;
...
/* Display usage */
static void vmdq_usage(const char *prgname) {
 ...
 */
static int vmdq_parse_args(int argc, char **argv) {
  int opt;
  int option_index;
  unsigned i;
...
static void update_mac_address(struct rte_mbuf *m, unsigned dst_port) {
...
static void sighup_handler(int signum) {
  ...
  for (; q < num_queues; q++) {
    if ((q - vmdq_queue_base) % (num_vmdq_queues / num_pools) == 0)
      printf("\nPool %u: ",
             (q - vmdq_queue_base) / (num_vmdq_queues / num_pools));
...
static int lcore_main(__rte_unused void *dummy) {
  ...
  if (remainder != 0) {
    if (core_id < remainder) {
      startQueue = (uint16_t)(core_id * (num_vmdq_queues / num_cores + 1));
      endQueue = (uint16_t)(startQueue + (num_vmdq_queues / num_cores) + 1);
    } else {
      startQueue =
          (uint16_t)(core_id * (num_vmdq_queues / num_cores) + remainder);
      endQueue = (uint16_t)(startQueue + (num_vmdq_queues / num_cores));
    }
  } else {
    startQueue = (uint16_t)(core_id * (num_vmdq_queues / num_cores));
    endQueue = (uint16_t)(startQueue + (num_vmdq_queues / num_cores));
  }
  ...
  for (;;) {
    ...
    for (p = 0; p < num_ports; p++) {
      ...
      for (q = startQueue; q < endQueue; q++) {
        const uint16_t rxCount = rte_eth_rx_burst(sport, q, buf, buf_size);
        ...
        const uint16_t txCount =
            rte_eth_tx_burst(dport, vmdq_queue_base + core_id, buf, rxCount);
...
int main(int argc, char *argv[]) {
  ...
  mbuf_pool = rte_pktmbuf_pool_create(
      "MBUF_POOL", NUM_MBUFS_PER_PORT * nb_ports, MBUF_CACHE_SIZE, 0,
