...
int port_in_action_profile_init(void) {
  TAILQ_INIT(&port_in_action_profile_list);

  return 0;
}

struct port_in_action_profile *port_in_action_profile_find(const char *name) {
...
port_in_action_profile_create(const char *name,
  ...
  /* Check input params */
  if ((name == NULL) || port_in_action_profile_find(name) || (params == NULL))
    return NULL;
...
int table_action_profile_init(void) {
  TAILQ_INIT(&table_action_profile_list);

  return 0;
}

struct table_action_profile *table_action_profile_find(const char *name) {
...
table_action_profile_create(const char *name,
  ...
  /* Check input params */
  if ((name == NULL) || table_action_profile_find(name) || (params == NULL) ||
