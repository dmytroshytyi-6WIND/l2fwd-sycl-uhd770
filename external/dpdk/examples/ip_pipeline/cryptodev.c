...
struct cryptodev *cryptodev_find(const char *name) {
...
struct cryptodev *cryptodev_next(struct cryptodev *cryptodev) {
  return (cryptodev == NULL) ? TAILQ_FIRST(&cryptodev_list)
                             : TAILQ_NEXT(cryptodev, node);
}

struct cryptodev *cryptodev_create(const char *name,
  ...
  /* Check input params */
  if ((name == NULL) || cryptodev_find(name) || (params->n_queues == 0) ||
      (params->queue_size == 0) || (params->session_pool_size == 0))
    return NULL;
  ...
  cache_size =
      (params->session_pool_size / 2 < PIPELINE_CRYPTO_SESSION_CACHE_SIZE)
  ...
  cryptodev->mp_init =
      rte_mempool_create(NULL, params->session_pool_size,
