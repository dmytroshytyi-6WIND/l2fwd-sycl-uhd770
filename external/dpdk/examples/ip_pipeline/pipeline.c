...
struct pipeline *pipeline_create(const char *name,
  ...
  /* Check input params */
  if ((name == NULL) || pipeline_find(name) || (params == NULL) ||
  ...
  msgq_req = rte_ring_create(msgq_name, PIPELINE_MSGQ_SIZE, params->cpu_id,
  ...
  msgq_rsp = rte_ring_create(msgq_name, PIPELINE_MSGQ_SIZE, params->cpu_id,
                             RING_F_SP_ENQ | RING_F_SC_DEQ);
  if (msgq_rsp == NULL) {
...
int pipeline_port_in_create(const char *pipeline_name,
  ...
  if (ap) {
    action = rte_port_in_action_create(ap->ap, pipeline->cpu_id);
    if (action == NULL)
      return -1;
...
int pipeline_port_out_create(const char *pipeline_name,
  ...
  case PORT_OUT_CRYPTODEV: {
    ...
    pp_nodrop.sym_crypto.crypto_op_offset = params->cryptodev.op_offset;
...
int pipeline_table_create(const char *pipeline_name,
  ...
  snprintf(name, NAME_MAX, "%s_table%u", pipeline_name, pipeline->n_tables);
  ...
  if (ap) {
    action = rte_table_action_create(ap->ap, pipeline->cpu_id);
    if (action == NULL)
      return -1;
...
void table_rule_add(struct table *table, struct table_rule *new_rule) {
