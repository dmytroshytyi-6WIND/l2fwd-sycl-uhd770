...
static const char cmd_mempool_help[] = "mempool <mempool_name>\n"
                                       "   buffer <buffer_size>\n"
                                       "   pool <pool_size>\n"
                                       "   cache <cache_size>\n"
                                       "   cpu <cpu_id>\n";

static void cmd_mempool(char **tokens, uint32_t n_tokens, char *out,
                        size_t out_size) {
...
static void cmd_link(char **tokens, uint32_t n_tokens, char *out,
                     size_t out_size) {
...
static void print_link_info(struct link *link, char *out, size_t out_size) {
  ...
  if (ret != 0) {
    snprintf(out, out_size, "\n%s: MAC address get failed: %s", link->name,
             rte_strerror(-ret));
    return;
  }
  ...
  snprintf(out, out_size,
           ...
           "\tTX errors %" PRIu64 "\n",
           link->name, eth_link.link_status == 0 ? "DOWN" : "UP", mtu,
           RTE_ETHER_ADDR_BYTES(&mac_addr), link->n_rxq, link->n_txq,
           link->port_id, rte_eth_link_speed_to_str(eth_link.link_speed),
           stats.ipackets, stats.ibytes, stats.ierrors, stats.imissed,
           stats.rx_nombuf, stats.opackets, stats.obytes, stats.oerrors);
}

/*
 * link show [<link_name>]
 */
static void cmd_link_show(char **tokens, uint32_t n_tokens, char *out,
                          size_t out_size) {
  struct link *link;
  char *link_name;
  ...
  if (n_tokens == 2) {
    ...
    if (link == NULL) {
      snprintf(out, out_size, MSG_ARG_INVALID, "Link does not exist");
      return;
    }
    print_link_info(link, out, out_size);
  }
...
static const char cmd_swq_help[] = "swq <swq_name>\n"
                                   "   size <size>\n"
                                   "   cpu <cpu_id>\n";

static void cmd_swq(char **tokens, uint32_t n_tokens, char *out,
                    size_t out_size) {
  struct swq_params p;
  char *name;
  struct swq *swq;
...
static void cmd_tmgr_subport_profile(char **tokens, uint32_t n_tokens,
  ...
  for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++)
    if (parser_read_uint64(&subport_profile.tc_rate[i], tokens[5 + i]) != 0) {
...
static void cmd_tmgr_pipe_profile(char **tokens, uint32_t n_tokens, char *out,
                                  size_t out_size) {
...
static const char cmd_tmgr_help[] = "tmgr <tmgr_name>\n"
                                    "   rate <rate>\n"
...
static void cmd_tmgr(char **tokens, uint32_t n_tokens, char *out,
                     size_t out_size) {
...
static void cmd_tmgr_subport(char **tokens, uint32_t n_tokens, char *out,
                             size_t out_size) {
...
static void cmd_tmgr_subport_pipe(char **tokens, uint32_t n_tokens, char *out,
                                  size_t out_size) {
  ...
  status = tmgr_pipe_config(name, subport_id, pipe_id_first, pipe_id_last,
                            pipe_profile_id);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
...
static const char cmd_tap_help[] = "tap <tap_name>\n";

static void cmd_tap(char **tokens, uint32_t n_tokens, char *out,
                    size_t out_size) {
  char *name;
  struct tap *tap;
...
static const char cmd_kni_help[] = "kni <kni_name>\n"
                                   "   link <link_name>\n"
                                   "   mempool <mempool_name>\n"
                                   "   [thread <thread_id>]\n";

static void cmd_kni(char **tokens, uint32_t n_tokens, char *out,
                    size_t out_size) {
  struct kni_params p;
  char *name;
  struct kni *kni;
...
static void cmd_cryptodev(char **tokens, uint32_t n_tokens, char *out,
                          size_t out_size) {
  ...
  if (strcmp(tokens[2], "dev") == 0)
    ...
    if (parser_read_uint32(&params.dev_id, tokens[3]) < 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "dev_id");
      return;
    }
  } else {
    snprintf(out, out_size, MSG_ARG_INVALID, "cryptodev");
    return;
  }
  ...
  if (parser_read_uint32(&params.n_queues, tokens[5]) < 0) {
    snprintf(out, out_size, MSG_ARG_INVALID, "q");
    return;
  }
  ...
  if (strcmp(tokens[7], "max_sessions")) {
    snprintf(out, out_size, MSG_ARG_NOT_FOUND, "max_sessions");
    return;
  }
...
static void cmd_port_in_action_profile(char **tokens, uint32_t n_tokens,
                                       char *out, size_t out_size) {
...
static void cmd_table_action_profile(char **tokens, uint32_t n_tokens,
                                     char *out, size_t out_size) {
  ...
  if ((t0 < n_tokens) && (strcmp(tokens[t0], "meter") == 0)) {
    if (n_tokens < t0 + 6) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "table action profile meter");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 1], "srtcm") == 0)
      ...
      snprintf(out, out_size, MSG_ARG_NOT_FOUND, "srtcm or trtcm");
      return;
    }
  ...
  if ((t0 < n_tokens) && (strcmp(tokens[t0], "tm") == 0)) {
    if (n_tokens < t0 + 5) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "table action profile tm");
      return;
    }
    ...
    if (parser_read_uint32(&p.tm.n_subports_per_port, tokens[t0 + 2]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_subports_per_port");
      return;
    }
    ...
    if (parser_read_uint32(&p.tm.n_pipes_per_subport, tokens[t0 + 4]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_pipes_per_subport");
      return;
    }
  ...
  if ((t0 < n_tokens) && (strcmp(tokens[t0], "encap") == 0)) {
    ...
    if (n_tokens < t0 + 2) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "action profile encap");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 1], "ether") == 0)
      ...
      if (n_tokens < t0 + 2 + 5) {
        snprintf(out, out_size, MSG_ARG_MISMATCH, "action profile encap vxlan");
        return;
      }

      if (strcmp(tokens[t0 + 2], "offset") != 0) {
        snprintf(out, out_size, MSG_ARG_NOT_FOUND, "vxlan: offset");
        return;
      }

      if (parser_read_uint32(&p.encap.vxlan.data_offset, tokens[t0 + 2 + 1]) !=
          0) {
        snprintf(out, out_size, MSG_ARG_INVALID, "vxlan: ether_offset");
        return;
      }
      ...
      if (strcmp(tokens[t0 + 2 + 2], "ipv4") == 0)
        ...
        snprintf(out, out_size, MSG_ARG_INVALID, "vxlan: ipv4 or ipv6");
        return;
      }

      if (strcmp(tokens[t0 + 2 + 3], "vlan") != 0) {
        snprintf(out, out_size, MSG_ARG_NOT_FOUND, "vxlan: vlan");
        return;
      }
      ...
      if (strcmp(tokens[t0 + 2 + 4], "on") == 0)
        ...
        snprintf(out, out_size, MSG_ARG_INVALID, "vxlan: on or off");
        return;
      }
      ...
      p.encap.encap_mask = 1LLU << RTE_TABLE_ACTION_ENCAP_QINQ_PPPOE;
    else {
  ...
  if ((t0 < n_tokens) && (strcmp(tokens[t0], "nat") == 0)) {
    if (n_tokens < t0 + 4) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "table action profile nat");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 1], "src") == 0)
      ...
      snprintf(out, out_size, MSG_ARG_NOT_FOUND, "src or dst");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 3], "tcp") == 0)
      ...
      snprintf(out, out_size, MSG_ARG_NOT_FOUND, "tcp or udp");
      return;
    }
  ...
  if ((t0 < n_tokens) && (strcmp(tokens[t0], "ttl") == 0)) {
    if (n_tokens < t0 + 4) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "table action profile ttl");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 1], "drop") == 0)
      ...
      snprintf(out, out_size, MSG_ARG_NOT_FOUND, "drop or fwd");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 3], "none") == 0)
      ...
      snprintf(out, out_size, MSG_ARG_NOT_FOUND, "none or pkts");
      return;
    }
  ...
  if ((t0 < n_tokens) && (strcmp(tokens[t0], "stats") == 0)) {
    if (n_tokens < t0 + 2) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "table action profile stats");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 1], "pkts") == 0) {
      ...
      snprintf(out, out_size, MSG_ARG_NOT_FOUND, "pkts or bytes or both");
      return;
    }
  ...
  if ((t0 < n_tokens) && (strcmp(tokens[t0], "sym_crypto") == 0)) {
    ...
    if (n_tokens < t0 + 5 || strcmp(tokens[t0 + 1], "dev") ||
    ...
    if (parser_read_uint32(&p.sym_crypto.op_offset, tokens[t0 + 4]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID,
               "table action profile sym_crypto");
      return;
    }
...
static const char cmd_pipeline_help[] = "pipeline <pipeline_name>\n"
                                        "   period <timer_period_ms>\n"
                                        "   offset_port_id <offset_port_id>\n"
                                        "   cpu <cpu_id>\n";

static void cmd_pipeline(char **tokens, uint32_t n_tokens, char *out,
                         size_t out_size) {
...
static void cmd_pipeline_port_in(char **tokens, uint32_t n_tokens, char *out,
                                 size_t out_size) {
  ...
  if (strcmp(tokens[t0], "link") == 0) {
    if (n_tokens < t0 + 4) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port in link");
      return;
    }
    ...
    if (n_tokens < t0 + 2) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port in swq");
      return;
    }
    ...
    if (n_tokens < t0 + 2) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port in tmgr");
      return;
    }
    ...
    if (n_tokens < t0 + 6) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port in tap");
      return;
    }
    ...
    if (n_tokens < t0 + 2) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port in kni");
      return;
    }
    ...
    if (n_tokens < t0 + 6) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port in source");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 5], "bpp") != 0) {
      snprintf(out, out_size, MSG_ARG_NOT_FOUND, "bpp");
      return;
    }

    if (parser_read_uint32(&p.source.n_bytes_per_pkt, tokens[t0 + 6]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_bytes_per_pkt");
      return;
    }
    ...
    if (n_tokens < t0 + 3) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port in cryptodev");
      return;
    }
    ...
    p.dev_name = tokens[t0 + 1];
    if (parser_read_uint16(&p.rxq.queue_id, tokens[t0 + 3]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "rxq");
      return;
    }
  ...
  enabled = 1;
  if ((n_tokens > t0) && (strcmp(tokens[t0], "disabled") == 0)) {
    enabled = 0;
  ...
  status = pipeline_port_in_create(pipeline_name, &p, enabled);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static void cmd_pipeline_port_out(char **tokens, uint32_t n_tokens, char *out,
                                  size_t out_size) {
  ...
  if (strcmp(tokens[6], "link") == 0) {
    if (n_tokens != 10) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port out link");
      return;
    }
    ...
    if (n_tokens != 8) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port out swq");
      return;
    }
    ...
    if (n_tokens != 8) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port out tmgr");
      return;
    }
    ...
    if (n_tokens != 8) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port out tap");
      return;
    }
    ...
    if (n_tokens != 8) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port out kni");
      return;
    }
    ...
    if ((n_tokens != 7) && (n_tokens != 11)) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port out sink");
      return;
    }
    ...
    if (n_tokens != 12) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port out cryptodev");
      return;
    }
    ...
    if (parser_read_uint16(&p.cryptodev.queue_id, tokens[9]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "queue_id");
      return;
    }
    ...
    if (parser_read_uint32(&p.cryptodev.op_offset, tokens[11]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "queue_id");
      return;
    }
  } else {
...
static void cmd_pipeline_table(char **tokens, uint32_t n_tokens, char *out,
                               size_t out_size) {
  ...
  if (strcmp(tokens[t0], "acl") == 0) {
    if (n_tokens < t0 + 6) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline table acl");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 1], "ipv4") == 0)
      ...
      snprintf(out, out_size, MSG_ARG_NOT_FOUND, "ipv4 or ipv6");
      return;
    }
    ...
    if (parser_read_uint32(&p.match.acl.ip_header_offset, tokens[t0 + 3]) !=
        0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "ip_header_offset");
      return;
    }
    ...
    if (parser_read_uint32(&p.match.acl.n_rules, tokens[t0 + 5]) != 0) {
    ...
    if (n_tokens < t0 + 5) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline table array");
      return;
    }
    ...
    if (parser_read_uint32(&p.match.array.key_offset, tokens[t0 + 2]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "key_offset");
      return;
    }
    ...
    if (parser_read_uint32(&p.match.array.n_keys, tokens[t0 + 4]) != 0) {
    ...
    if (n_tokens < t0 + 12) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline table hash");
      return;
    }
    ...
    if (strcmp(tokens[t0 + 1], "ext") == 0)
      ...
      snprintf(out, out_size, MSG_ARG_NOT_FOUND, "ext or lru");
      return;
    }
    ...
    if ((parser_read_uint32(&p.match.hash.key_size, tokens[t0 + 3]) != 0) ||
    ...
    if ((parse_hex_string(tokens[t0 + 5], key_mask, &key_mask_size) != 0) ||
    ...
    if (parser_read_uint32(&p.match.hash.key_offset, tokens[t0 + 7]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "key_offset");
      return;
    }
    ...
    if (parser_read_uint32(&p.match.hash.n_buckets, tokens[t0 + 9]) != 0) {
    ...
    if (parser_read_uint32(&p.match.hash.n_keys, tokens[t0 + 11]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_keys");
      return;
    }
    ...
    if (n_tokens < t0 + 6) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline table lpm");
      return;
    }
    ...
    if (parser_read_uint32(&p.match.lpm.key_offset, tokens[t0 + 3]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "key_offset");
      return;
    }
    ...
    if (parser_read_uint32(&p.match.lpm.n_rules, tokens[t0 + 5]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_rules");
      return;
    }
...
static void cmd_pipeline_port_in_table(char **tokens, uint32_t n_tokens,
                                       char *out, size_t out_size) {
  char *pipeline_name;
  ...
  status = pipeline_port_in_connect_to_table(pipeline_name, port_id, table_id);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static void cmd_pipeline_port_in_stats(char **tokens, uint32_t n_tokens,
                                       char *out, size_t out_size) {
  ...
  status = pipeline_port_in_stats_read(pipeline_name, port_id, &stats, clear);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }

  snprintf(out, out_size, MSG_PIPELINE_PORT_IN_STATS, stats.stats.n_pkts_in,
           stats.n_pkts_dropped_by_ah, stats.stats.n_pkts_drop);
}

static const char cmd_pipeline_port_in_enable_help[] =
    "pipeline <pipeline_name> port in <port_id> enable\n";

static void cmd_pipeline_port_in_enable(char **tokens, uint32_t n_tokens,
                                        char *out, size_t out_size) {
  char *pipeline_name;
  uint32_t port_id;
  int status;
...
static void cmd_pipeline_port_in_disable(char **tokens, uint32_t n_tokens,
                                         char *out, size_t out_size) {
  char *pipeline_name;
  uint32_t port_id;
  int status;
...
static void cmd_pipeline_port_out_stats(char **tokens, uint32_t n_tokens,
                                        char *out, size_t out_size) {
  ...
  status = pipeline_port_out_stats_read(pipeline_name, port_id, &stats, clear);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }

  snprintf(out, out_size, MSG_PIPELINE_PORT_OUT_STATS, stats.stats.n_pkts_in,
           stats.n_pkts_dropped_by_ah, stats.stats.n_pkts_drop);
}
...
static void cmd_pipeline_table_stats(char **tokens, uint32_t n_tokens,
                                     char *out, size_t out_size) {
  ...
  status = pipeline_table_stats_read(pipeline_name, table_id, &stats, clear);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }

  snprintf(out, out_size, MSG_PIPELINE_TABLE_STATS, stats.stats.n_pkts_in,
           stats.stats.n_pkts_lookup_miss, stats.n_pkts_dropped_by_lkp_hit_ah,
           stats.n_pkts_dropped_lkp_hit, stats.n_pkts_dropped_by_lkp_miss_ah,
...
static uint32_t parse_match(char **tokens, uint32_t n_tokens, char *out,
                            size_t out_size, struct table_rule_match *m) {
  memset(m, 0, sizeof(*m));
  ...
  if (strcmp(tokens[1], "acl") == 0) {
    ...
    if (parser_read_uint32(&m->match.acl.priority, tokens[3]) != 0) {
    ...
    if (parser_read_uint32(&m->match.acl.sa_depth, tokens[6]) != 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "sa_depth");
      return 0;
    }

    if (parser_read_uint32(&m->match.acl.da_depth, tokens[8]) != 0) {
  ...
  if (strcmp(tokens[1], "hash") == 0) {
    ...
    if (strcmp(tokens[2], "raw") == 0) {
      ...
      if (parse_hex_string(tokens[3], m->match.hash.key, &key_size) != 0) {
    ...
    if (strcmp(tokens[2], "ipv4_addr") == 0) {
      ...
      if (parse_ipv4_addr(tokens[3], &addr) != 0) {
        snprintf(out, out_size, MSG_ARG_INVALID, "addr");
        return 0;
      }
    ...
    if (strcmp(tokens[2], "qinq") == 0) {
      struct pkt_key_qinq *qinq = (struct pkt_key_qinq *)m->match.hash.key;
      ...
      if ((parser_read_uint16(&svlan, tokens[3]) != 0) || (svlan > 0xFFF)) {
        snprintf(out, out_size, MSG_ARG_INVALID, "svlan");
        return 0;
      }

      if ((parser_read_uint16(&cvlan, tokens[4]) != 0) || (cvlan > 0xFFF)) {
        snprintf(out, out_size, MSG_ARG_INVALID, "cvlan");
        return 0;
      }
  ...
  if (strcmp(tokens[1], "lpm") == 0) {
    ...
    if (strcmp(tokens[2], "ipv4") == 0) {
      ...
      snprintf(out, out_size, MSG_ARG_MISMATCH, "ipv4 or ipv6");
      return 0;
    }
  ...
  snprintf(out, out_size, MSG_ARG_MISMATCH, "acl or array or hash or lpm");
  return 0;
}
 ...
 *    <pa> ::= g | y | r | drop
 */
static uint32_t parse_table_action_fwd(char **tokens, uint32_t n_tokens,
  ...
  if (n_tokens && (strcmp(tokens[0], "port") == 0)) {
    ...
    if ((n_tokens < 2) || parser_read_uint32(&id, tokens[1]))
      return 0;
...
static uint32_t parse_table_action_balance(char **tokens, uint32_t n_tokens,
                                           struct table_rule_action *a) {
  uint32_t i;
...
static int parse_policer_action(char *token, enum rte_table_action_policer *a) {
  if (strcmp(token, "g") == 0) {
...
static uint32_t
parse_table_action_meter_tc(char **tokens, uint32_t n_tokens,
                            struct rte_table_action_mtr_tc_params *mtr) {
  if ((n_tokens < 9) || strcmp(tokens[0], "meter") ||
      parser_read_uint32(&mtr->meter_profile_id, tokens[1]) ||
      strcmp(tokens[2], "policer") || strcmp(tokens[3], "g") ||
...
static uint32_t parse_table_action_meter(char **tokens, uint32_t n_tokens,
                                         struct table_rule_action *a) {
  ...
  if ((n_tokens < 10) || strcmp(tokens[0], "tc0") ||
      (parse_table_action_meter_tc(tokens + 1, n_tokens - 1, &a->mtr.mtr[0]) ==
       0))
    return 0;
  ...
  if ((n_tokens < 30) ||
      (parse_table_action_meter_tc(tokens + 1, n_tokens - 1, &a->mtr.mtr[1]) ==
       0) ||
      strcmp(tokens[10], "tc2") ||
      (parse_table_action_meter_tc(tokens + 11, n_tokens - 11,
                                   &a->mtr.mtr[2]) == 0) ||
      strcmp(tokens[20], "tc3") ||
      (parse_table_action_meter_tc(tokens + 21, n_tokens - 21,
                                   &a->mtr.mtr[3]) == 0))
    return 0;
...
static uint32_t parse_table_action_tm(char **tokens, uint32_t n_tokens,
                                      struct table_rule_action *a) {
  uint32_t subport_id, pipe_id;

  if ((n_tokens < 5) || strcmp(tokens[0], "tm") ||
      strcmp(tokens[1], "subport") ||
      parser_read_uint32(&subport_id, tokens[2]) || strcmp(tokens[3], "pipe") ||
...
static uint32_t parse_table_action_encap(char **tokens, uint32_t n_tokens,
                                         struct table_rule_action *a) {
  ...
  if (n_tokens && (strcmp(tokens[0], "ether") == 0)) {
    if ((n_tokens < 3) || parse_mac_addr(tokens[1], &a->encap.ether.ether.da) ||
  ...
  if (n_tokens && (strcmp(tokens[0], "vlan") == 0)) {
    ...
    if ((n_tokens < 6) || parse_mac_addr(tokens[1], &a->encap.vlan.ether.da) ||
        parse_mac_addr(tokens[2], &a->encap.vlan.ether.sa) ||
        parser_read_uint32(&pcp, tokens[3]) || (pcp > 0x7) ||
        parser_read_uint32(&dei, tokens[4]) || (dei > 0x1) ||
        parser_read_uint32(&vid, tokens[5]) || (vid > 0xFFF))
      return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "qinq") == 0)) {
    ...
    if ((n_tokens < 9) || parse_mac_addr(tokens[1], &a->encap.qinq.ether.da) ||
        parse_mac_addr(tokens[2], &a->encap.qinq.ether.sa) ||
        parser_read_uint32(&svlan_pcp, tokens[3]) || (svlan_pcp > 0x7) ||
        parser_read_uint32(&svlan_dei, tokens[4]) || (svlan_dei > 0x1) ||
        parser_read_uint32(&svlan_vid, tokens[5]) || (svlan_vid > 0xFFF) ||
        parser_read_uint32(&cvlan_pcp, tokens[6]) || (cvlan_pcp > 0x7) ||
        parser_read_uint32(&cvlan_dei, tokens[7]) || (cvlan_dei > 0x1) ||
        parser_read_uint32(&cvlan_vid, tokens[8]) || (cvlan_vid > 0xFFF))
      return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "qinq_pppoe") == 0)) {
    ...
    if ((n_tokens < 10) ||
        parse_mac_addr(tokens[1], &a->encap.qinq_pppoe.ether.da) ||
        parse_mac_addr(tokens[2], &a->encap.qinq_pppoe.ether.sa) ||
        parser_read_uint32(&svlan_pcp, tokens[3]) || (svlan_pcp > 0x7) ||
        parser_read_uint32(&svlan_dei, tokens[4]) || (svlan_dei > 0x1) ||
        parser_read_uint32(&svlan_vid, tokens[5]) || (svlan_vid > 0xFFF) ||
        parser_read_uint32(&cvlan_pcp, tokens[6]) || (cvlan_pcp > 0x7) ||
        parser_read_uint32(&cvlan_dei, tokens[7]) || (cvlan_dei > 0x1) ||
        parser_read_uint32(&cvlan_vid, tokens[8]) || (cvlan_vid > 0xFFF) ||
        parser_read_uint16(&a->encap.qinq_pppoe.pppoe.session_id, tokens[9]))
      return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "mpls") == 0)) {
    ...
    if (parse_mac_addr(tokens[2], &a->encap.mpls.ether.da) ||
        ...
        strcmp(tokens[4], "label0") || parser_read_uint32(&label, tokens[5]) ||
        (label > 0xFFFFF) || parser_read_uint32(&tc, tokens[6]) || (tc > 0x7) ||
        parser_read_uint32(&ttl, tokens[7]) || (ttl > 0x3F))
      return 0;
    ...
    if ((n_tokens < 4) || parser_read_uint32(&label, tokens[1]) ||
        (label > 0xFFFFF) || parser_read_uint32(&tc, tokens[2]) || (tc > 0x7) ||
        parser_read_uint32(&ttl, tokens[3]) || (ttl > 0x3F))
      return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "pppoe") == 0)) {
    if ((n_tokens < 4) || parse_mac_addr(tokens[1], &a->encap.pppoe.ether.da) ||
        parse_mac_addr(tokens[2], &a->encap.pppoe.ether.sa) ||
        parser_read_uint16(&a->encap.pppoe.pppoe.session_id, tokens[3]))
      return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "vxlan") == 0)) {
    ...
    /* ether <da> <sa> */
    if ((n_tokens < 3) || strcmp(tokens[0], "ether") ||
    ...
    if (strcmp(tokens[0], "vlan") == 0) {
      ...
      if ((n_tokens < 4) || parser_read_uint32(&pcp, tokens[1]) || (pcp > 7) ||
          parser_read_uint32(&dei, tokens[2]) || (dei > 1) ||
          parser_read_uint32(&vid, tokens[3]) || (vid > 0xFFF))
        return 0;
    ...
    if (strcmp(tokens[0], "ipv4") == 0) {
      ...
      if ((n_tokens < 5) || parse_ipv4_addr(tokens[1], &sa) ||
          parse_ipv4_addr(tokens[2], &da) ||
          parser_read_uint8(&dscp, tokens[3]) || (dscp > 64) ||
      ...
      if ((n_tokens < 6) || parse_ipv6_addr(tokens[1], &sa) ||
          parse_ipv6_addr(tokens[2], &da) ||
          parser_read_uint32(&flow_label, tokens[3]) ||
          parser_read_uint8(&dscp, tokens[4]) || (dscp > 64) ||
    ...
    /* udp <sp> <dp> */
    if ((n_tokens < 3) || strcmp(tokens[0], "udp") ||
    ...
    /* vxlan <vni> */
    if ((n_tokens < 2) || strcmp(tokens[0], "vxlan") ||
...
static uint32_t parse_table_action_nat(char **tokens, uint32_t n_tokens,
                                       struct table_rule_action *a) {
  if ((n_tokens < 4) || strcmp(tokens[0], "nat"))
    return 0;
...
static uint32_t parse_table_action_ttl(char **tokens, uint32_t n_tokens,
                                       struct table_rule_action *a) {
  if ((n_tokens < 2) || strcmp(tokens[0], "ttl"))
    return 0;
...
static uint32_t parse_table_action_stats(char **tokens, uint32_t n_tokens,
                                         struct table_rule_action *a) {
  if ((n_tokens < 1) || strcmp(tokens[0], "stats"))
    return 0;
...
static uint32_t parse_table_action_time(char **tokens, uint32_t n_tokens,
                                        struct table_rule_action *a) {
  if ((n_tokens < 1) || strcmp(tokens[0], "time"))
    return 0;
...
parse_table_action_cipher(struct rte_table_action_sym_crypto_params *p,
                          ...
                          uint32_t n_tokens, uint32_t encrypt,
  ...
  if (n_tokens < 7 || strcmp(tokens[1], "cipher_algo") ||
      strcmp(tokens[3], "cipher_key") || strcmp(tokens[5], "cipher_iv"))
    return NULL;
  ...
  xform_cipher->cipher.op =
      encrypt ? RTE_CRYPTO_CIPHER_OP_ENCRYPT : RTE_CRYPTO_CIPHER_OP_DECRYPT;

  /* cipher_algo */
  status =
      rte_cryptodev_get_cipher_algo_enum(&xform_cipher->cipher.algo, tokens[2]);
  if (status < 0)
    goto error_exit;
  ...
  status = parse_hex_string(tokens[6], p->cipher_auth.cipher_iv.val,
                            (uint32_t *)&len);
  if (status < 0)
    goto error_exit;
...
parse_table_action_cipher_auth(struct rte_table_action_sym_crypto_params *p,
                               ...
                               uint32_t encrypt, uint32_t *used_n_tokens) {
  struct rte_crypto_sym_xform *xform_cipher;
  struct rte_crypto_sym_xform *xform_auth;
  int status;
  size_t len;

  if (n_tokens < 13 || strcmp(tokens[7], "auth_algo") ||
      strcmp(tokens[9], "auth_key") || strcmp(tokens[11], "digest_size"))
    return NULL;
  ...
  xform_auth->auth.op =
      encrypt ? RTE_CRYPTO_AUTH_OP_GENERATE : RTE_CRYPTO_AUTH_OP_VERIFY;

  /* auth_algo */
  status = rte_cryptodev_get_auth_algo_enum(&xform_auth->auth.algo, tokens[8]);
  if (status < 0)
    goto error_exit;
  ...
  status = parser_read_uint16(&xform_auth->auth.digest_length, tokens[12]);
  if (status < 0)
    goto error_exit;
...
parse_table_action_aead(struct rte_table_action_sym_crypto_params *p,
  ...
  if (n_tokens < 11 || strcmp(tokens[1], "aead_algo") ||
      strcmp(tokens[3], "aead_key") || strcmp(tokens[5], "aead_iv") ||
      strcmp(tokens[7], "aead_aad") || strcmp(tokens[9], "digest_size"))
    return NULL;
  ...
  xform_aead->aead.op =
      encrypt ? RTE_CRYPTO_AEAD_OP_ENCRYPT : RTE_CRYPTO_AEAD_OP_DECRYPT;

  /* aead_algo */
  status = rte_cryptodev_get_aead_algo_enum(&xform_aead->aead.algo, tokens[2]);
  if (status < 0)
    goto error_exit;
  ...
  status = parse_hex_string(tokens[6], p->aead.iv.val, (uint32_t *)&len);
  if (status < 0)
    goto error_exit;
  ...
  /* digest_size */
  status = parser_read_uint16(&xform_aead->aead.digest_length, tokens[10]);
  if (status < 0)
    goto error_exit;
...
static uint32_t parse_table_action_sym_crypto(char **tokens, uint32_t n_tokens,
                                              struct table_rule_action *a) {
  ...
  if ((n_tokens < 12) || strcmp(tokens[0], "sym_crypto") ||
  ...
  if (strcmp(tokens[3], "cipher") == 0) {
    ...
    xform = parse_table_action_cipher(p, key, max_key_len, tokens, n_tokens,
                                      encrypt, &used_n_tokens);
  } else if (strcmp(tokens[3], "cipher_auth") == 0) {
    tokens += 3;
    n_tokens -= 3;

    xform = parse_table_action_cipher_auth(p, key, max_key_len, tokens,
                                           n_tokens, encrypt, &used_n_tokens);
  } else if (strcmp(tokens[3], "aead") == 0) {
    tokens += 3;
    n_tokens -= 3;

    xform = parse_table_action_aead(p, key, max_key_len, tokens, n_tokens,
                                    encrypt, &used_n_tokens);
  }
...
static uint32_t parse_table_action_tag(char **tokens, uint32_t n_tokens,
                                       struct table_rule_action *a) {
  if ((n_tokens < 2) || strcmp(tokens[0], "tag"))
    return 0;
...
static uint32_t parse_table_action_decap(char **tokens, uint32_t n_tokens,
                                         struct table_rule_action *a) {
  if ((n_tokens < 2) || strcmp(tokens[0], "decap"))
    return 0;
...
static uint32_t parse_table_action(char **tokens, uint32_t n_tokens, char *out,
                                   size_t out_size,
                                   struct table_rule_action *a) {
  ...
  if ((n_tokens < 2) || strcmp(tokens[0], "action"))
    return 0;
  ...
  if (n_tokens && (strcmp(tokens[0], "fwd") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action fwd");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "balance") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action balance");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "meter") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action meter");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "tm") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action tm");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "encap") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action encap");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "nat") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action nat");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "ttl") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action ttl");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "stats") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action stats");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "time") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action time");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "sym_crypto") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action sym_crypto");
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "tag") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action tag");
      return 0;
    }
  ...
  if (n_tokens && (strcmp(tokens[0], "decap") == 0)) {
    ...
    if (n == 0) {
      snprintf(out, out_size, MSG_ARG_INVALID, "action decap");
      return 0;
    }
...
static void cmd_pipeline_table_rule_add(char **tokens, uint32_t n_tokens,
                                        char *out, size_t out_size) {
  ...
  /* match */
  n_tokens_parsed = parse_match(tokens + t0, n_tokens - t0, out, out_size, &m);
  ...
  /* action */
  n_tokens_parsed =
      parse_table_action(tokens + t0, n_tokens - t0, out, out_size, &a);
  if (n_tokens_parsed == 0)
    return;
  t0 += n_tokens_parsed;
...
static void cmd_pipeline_table_rule_add_default(char **tokens,
                                                uint32_t n_tokens, char *out,
                                                size_t out_size) {
  ...
  if (strcmp(tokens[10], "drop") == 0) {
    ...
    snprintf(out, out_size, MSG_ARG_INVALID, "drop or port or meta or table");
    return;
  }

  status = pipeline_table_rule_add_default(pipeline_name, table_id, &action);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static int cli_rule_file_process(const char *file_name, size_t line_len_max,
                                 struct table_rule_list **rule_list,
                                 uint32_t *n_rules, uint32_t *line_number,
                                 char *out, size_t out_size);

static void cmd_pipeline_table_rule_add_bulk(char **tokens, uint32_t n_tokens,
                                             char *out, size_t out_size) {
  ...
  status = cli_rule_file_process(file_name, 1024, &list, &n_rules, &line_number,
                                 out, out_size);
  if (status) {
  ...
  /* Rule bulk add */
  status = pipeline_table_rule_add_bulk(pipeline_name, table_id, list,
                                        &n_rules_added, &n_rules_not_added);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static void cmd_pipeline_table_rule_delete(char **tokens, uint32_t n_tokens,
                                           char *out, size_t out_size) {
  struct table_rule_match m;
  char *pipeline_name;
  ...
  status = pipeline_table_rule_delete(pipeline_name, table_id, &m);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static void cmd_pipeline_table_rule_delete_default(char **tokens,
                                                   uint32_t n_tokens, char *out,
                                                   size_t out_size) {
  char *pipeline_name;
  uint32_t table_id;
  ...
  status = pipeline_table_rule_delete_default(pipeline_name, table_id);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static void ether_addr_show(FILE *f, struct rte_ether_addr *addr) {
  fprintf(f, RTE_ETHER_ADDR_PRT_FMT, RTE_ETHER_ADDR_BYTES(addr));
}

static void ipv4_addr_show(FILE *f, uint32_t addr) {
  fprintf(f, "%u.%u.%u.%u", addr >> 24, (addr >> 16) & 0xFF, (addr >> 8) & 0xFF,
          addr & 0xFF);
}

static void ipv6_addr_show(FILE *f, uint8_t *addr) {
  fprintf(f,
          "%02x%02x:%02x%02x:%02x%02x:%02x%02x:"
          "%02x%02x:%02x%02x:%02x%02x:%02x%02x:",
          (uint32_t)addr[0], (uint32_t)addr[1], (uint32_t)addr[2],
          (uint32_t)addr[3], (uint32_t)addr[4], (uint32_t)addr[5],
          (uint32_t)addr[6], (uint32_t)addr[7], (uint32_t)addr[8],
          (uint32_t)addr[9], (uint32_t)addr[10], (uint32_t)addr[11],
          (uint32_t)addr[12], (uint32_t)addr[13], (uint32_t)addr[14],
          (uint32_t)addr[15]);
}

static const char *policer_action_string(enum rte_table_action_policer action) {
  switch (action) {
  case RTE_TABLE_ACTION_POLICER_COLOR_GREEN:
    return "G";
  case RTE_TABLE_ACTION_POLICER_COLOR_YELLOW:
    return "Y";
  case RTE_TABLE_ACTION_POLICER_COLOR_RED:
    return "R";
  case RTE_TABLE_ACTION_POLICER_DROP:
    return "D";
  default:
    return "?";
  }
...
static int table_rule_show(const char *pipeline_name, uint32_t table_id,
  ...
  /* Check input params. */
  if ((pipeline_name == NULL) || (file_name == NULL))
    return -1;

  p = pipeline_find(pipeline_name);
  if ((p == NULL) || (table_id >= p->n_tables))
    return -1;
  ...
  TAILQ_FOREACH(rule, &table->rules, node) {
    ...
    switch (m->match_type) {
    case TABLE_ACL:
      fprintf(f, "acl priority %u ", m->match.acl.priority);
      ...
      fprintf(f, "%u %u %u %u %u ", (uint32_t)m->match.acl.sp0,
              (uint32_t)m->match.acl.sp1, (uint32_t)m->match.acl.dp0,
              (uint32_t)m->match.acl.dp1, (uint32_t)m->match.acl.proto);
      break;

    case TABLE_ARRAY:
      fprintf(f, "array %u ", m->match.array.pos);
      break;
    ...
    case TABLE_LPM:
      ...
      fprintf(f, "%u ", (uint32_t)m->match.lpm.depth);
      break;
    ...
    if (a->action_mask & (1LLU << RTE_TABLE_ACTION_MTR)) {
      ...
      for (i = 0; i < RTE_TABLE_ACTION_TC_MAX; i++)
        if (a->mtr.tc_mask & (1 << i)) {
          struct rte_table_action_mtr_tc_params *p = &a->mtr.mtr[i];
          enum rte_table_action_policer ga = p->policer[RTE_COLOR_GREEN];
          enum rte_table_action_policer ya = p->policer[RTE_COLOR_YELLOW];
          enum rte_table_action_policer ra = p->policer[RTE_COLOR_RED];

          fprintf(f, "tc%u meter %u policer g %s y %s r %s ", i,
                  a->mtr.mtr[i].meter_profile_id, policer_action_string(ga),
                  policer_action_string(ya), policer_action_string(ra));
        }
    }

    if (a->action_mask & (1LLU << RTE_TABLE_ACTION_TM))
      fprintf(f, "tm subport %u pipe %u ", a->tm.subport_id, a->tm.pipe_id);
    ...
    if (a->action_mask & (1LLU << RTE_TABLE_ACTION_ENCAP)) {
      ...
      case RTE_TABLE_ACTION_ENCAP_VLAN:
        ...
        fprintf(f, " pcp %u dei %u vid %u ", a->encap.vlan.vlan.pcp,
                a->encap.vlan.vlan.dei, a->encap.vlan.vlan.vid);
        break;
      ...
      case RTE_TABLE_ACTION_ENCAP_QINQ:
        ...
        fprintf(f, " pcp %u dei %u vid %u pcp %u dei %u vid %u ",
                a->encap.qinq.svlan.pcp, a->encap.qinq.svlan.dei,
                a->encap.qinq.svlan.vid, a->encap.qinq.cvlan.pcp,
                a->encap.qinq.cvlan.dei, a->encap.qinq.cvlan.vid);
        break;

      case RTE_TABLE_ACTION_ENCAP_MPLS:
        fprintf(f, "mpls %s ",
                (a->encap.mpls.unicast) ? "unicast " : "multicast ");
        ether_addr_show(f, &a->encap.mpls.ether.da);
        fprintf(f, " ");
        ether_addr_show(f, &a->encap.mpls.ether.sa);
        fprintf(f, " ");
        for (i = 0; i < a->encap.mpls.mpls_count; i++) {
          struct rte_table_action_mpls_hdr *l = &a->encap.mpls.mpls[i];

          fprintf(f, "label%u %u %u %u ", i, l->label, l->tc, l->ttl);
        }
        break;
      ...
      case RTE_TABLE_ACTION_ENCAP_VXLAN:
        ...
        if (table->ap->params.encap.vxlan.vlan)
          fprintf(f, " vlan pcp %u dei %u vid %u ", a->encap.vxlan.vlan.pcp,
                  a->encap.vxlan.vlan.dei, a->encap.vxlan.vlan.vid);
        if (table->ap->params.encap.vxlan.ip_version) {
          fprintf(f, " ipv4 ");
          ipv4_addr_show(f, a->encap.vxlan.ipv4.sa);
          fprintf(f, " ");
          ipv4_addr_show(f, a->encap.vxlan.ipv4.da);
          fprintf(f, " %u %u ", (uint32_t)a->encap.vxlan.ipv4.dscp,
          ...
          fprintf(f, " %u %u %u ", a->encap.vxlan.ipv6.flow_label,
                  (uint32_t)a->encap.vxlan.ipv6.dscp,
                  (uint32_t)a->encap.vxlan.ipv6.hop_limit);
          fprintf(f, " udp %u %u vxlan %u ", a->encap.vxlan.udp.sp,
                  a->encap.vxlan.udp.dp, a->encap.vxlan.vxlan.vni);
        }
        break;
...
static void cmd_pipeline_table_rule_show(char **tokens, uint32_t n_tokens,
                                         char *out, size_t out_size) {
...
static void cmd_pipeline_table_rule_stats_read(char **tokens, uint32_t n_tokens,
                                               char *out, size_t out_size) {
  struct table_rule_match m;
  ...
  n_tokens_parsed = parse_match(tokens, n_tokens, out, out_size, &m);
  if (n_tokens_parsed == 0)
    return;
  ...
  status = pipeline_table_rule_stats_read(pipeline_name, table_id, &m, &stats,
                                          clear);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
...
static const char cmd_pipeline_table_meter_profile_add_help[] =
    "pipeline <pipeline_name> table <table_id> meter profile "
    "<meter_profile_id>\n"
    "   add srtcm cir <cir> cbs <cbs> ebs <ebs>\n"
    "   | trtcm cir <cir> pir <pir> cbs <cbs> pbs <pbs>\n";

static void cmd_pipeline_table_meter_profile_add(char **tokens,
                                                 uint32_t n_tokens, char *out,
                                                 size_t out_size) {
  ...
  status = pipeline_table_mtr_profile_add(pipeline_name, table_id,
                                          meter_profile_id, &p);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
...
static void cmd_pipeline_table_meter_profile_delete(char **tokens,
                                                    uint32_t n_tokens,
                                                    char *out,
                                                    size_t out_size) {
  char *pipeline_name;
  ...
  status = pipeline_table_mtr_profile_delete(pipeline_name, table_id,
                                             meter_profile_id);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
...
static void cmd_pipeline_table_rule_meter_read(char **tokens, uint32_t n_tokens,
                                               char *out, size_t out_size) {
  struct table_rule_match m;
  ...
  status =
      pipeline_table_rule_mtr_read(pipeline_name, table_id, &m, &stats, clear);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static const char cmd_pipeline_table_dscp_help[] =
    ...
    " File <file_name>:\n"
    "   - exactly 64 lines\n"
    "   - line format: <tc_id> <tc_queue_id> <color>, with <color> as: g | y | "
    "r\n";

static int load_dscp_table(struct rte_table_action_dscp_table *dscp_table,
                           const char *file_name, uint32_t *line_number) {
  FILE *f = NULL;
  uint32_t dscp, l;

  /* Check input arguments */
  if ((dscp_table == NULL) || (file_name == NULL) || (line_number == NULL)) {
    if (line_number)
      *line_number = 0;
    return -EINVAL;
  }
  ...
  for (dscp = 0, l = 1;; l++) {
    ...
    if ((dscp >= RTE_DIM(dscp_table->entry)) || (n_tokens != RTE_DIM(tokens)) ||
...
static void cmd_pipeline_table_dscp(char **tokens, uint32_t n_tokens, char *out,
                                    size_t out_size) {
  ...
  status = pipeline_table_dscp_table_update(pipeline_name, table_id, UINT64_MAX,
                                            &dscp_table);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
...
static void cmd_pipeline_table_rule_ttl_read(char **tokens, uint32_t n_tokens,
                                             char *out, size_t out_size) {
  struct table_rule_match m;
  ...
  status =
      pipeline_table_rule_ttl_read(pipeline_name, table_id, &m, &stats, clear);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static void cmd_pipeline_table_rule_time_read(char **tokens, uint32_t n_tokens,
                                              char *out, size_t out_size) {
  struct table_rule_match m;
  char *pipeline_name;
  uint64_t timestamp;
  ...
  status =
      pipeline_table_rule_time_read(pipeline_name, table_id, &m, &timestamp);
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
    return;
  }
...
static void cmd_thread_pipeline_enable(char **tokens, uint32_t n_tokens,
                                       char *out, size_t out_size) {
  char *pipeline_name;
  uint32_t thread_id;
  int status;
...
static void cmd_thread_pipeline_disable(char **tokens, uint32_t n_tokens,
                                        char *out, size_t out_size) {
  char *pipeline_name;
  uint32_t thread_id;
  int status;
  ...
  if (status) {
    snprintf(out, out_size, MSG_CMD_FAIL, "thread pipeline disable");
    return;
  }
}

static void cmd_help(char **tokens, uint32_t n_tokens, char *out,
                     size_t out_size) {
  tokens++;
  n_tokens--;
  ...
  if (strcmp(tokens[0], "tmgr") == 0) {
    ...
    if ((n_tokens == 2) && (strcmp(tokens[1], "subport")) == 0) {
      snprintf(out, out_size, "\n%s\n", cmd_tmgr_subport_help);
      return;
    }

    if ((n_tokens == 3) && (strcmp(tokens[1], "subport") == 0) &&
    ...
    if ((n_tokens == 3) && (strcmp(tokens[1], "pipe") == 0) &&
        (strcmp(tokens[2], "profile") == 0)) {
  ...
  if ((n_tokens == 4) && (strcmp(tokens[0], "port") == 0) &&
      (strcmp(tokens[1], "in") == 0) && (strcmp(tokens[2], "action") == 0) &&
      (strcmp(tokens[3], "profile") == 0)) {
    snprintf(out, out_size, "\n%s\n", cmd_port_in_action_profile_help);
    return;
  }

  if ((n_tokens == 3) && (strcmp(tokens[0], "table") == 0) &&
  ...
  if ((strcmp(tokens[0], "pipeline") == 0) &&
    ...
    if ((n_tokens == 4) && (strcmp(tokens[2], "in") == 0) &&
    ...
    if ((n_tokens == 4) && (strcmp(tokens[2], "out") == 0) &&
        (strcmp(tokens[3], "stats") == 0)) {
  ...
  if ((strcmp(tokens[0], "pipeline") == 0) &&
    ...
    if ((n_tokens == 4) && (strcmp(tokens[2], "rule") == 0) &&
        (strcmp(tokens[3], "add") == 0)) {
      snprintf(out, out_size, "\n%s\n", cmd_pipeline_table_rule_add_help);
      return;
    }

    if ((n_tokens == 5) && (strcmp(tokens[2], "rule") == 0) &&
        (strcmp(tokens[3], "add") == 0) &&
    ...
    if ((n_tokens == 5) && (strcmp(tokens[2], "rule") == 0) &&
        (strcmp(tokens[3], "add") == 0) && (strcmp(tokens[4], "bulk") == 0)) {
    ...
    if ((n_tokens == 5) && (strcmp(tokens[2], "rule") == 0) &&
        (strcmp(tokens[3], "stats") == 0) && (strcmp(tokens[4], "read") == 0)) {
      snprintf(out, out_size, "\n%s\n",
               cmd_pipeline_table_rule_stats_read_help);
      return;
    }

    if ((n_tokens == 5) && (strcmp(tokens[2], "meter") == 0) &&
        (strcmp(tokens[3], "profile") == 0) &&
    ...
    if ((n_tokens == 5) && (strcmp(tokens[2], "rule") == 0) &&
        (strcmp(tokens[3], "meter") == 0) && (strcmp(tokens[4], "read") == 0)) {
      snprintf(out, out_size, "\n%s\n",
    ...
    if ((n_tokens == 5) && (strcmp(tokens[2], "rule") == 0) &&
        (strcmp(tokens[3], "ttl") == 0) && (strcmp(tokens[4], "read") == 0)) {
      snprintf(out, out_size, "\n%s\n", cmd_pipeline_table_rule_ttl_read_help);
      return;
    }

    if ((n_tokens == 5) && (strcmp(tokens[2], "rule") == 0) &&
        (strcmp(tokens[3], "time") == 0) && (strcmp(tokens[4], "read") == 0)) {
      snprintf(out, out_size, "\n%s\n", cmd_pipeline_table_rule_time_read_help);
      return;
    }
  }

  if ((n_tokens == 3) && (strcmp(tokens[0], "thread") == 0) &&
...
void cli_process(char *in, char *out, size_t out_size) {
  ...
  if (strcmp(tokens[0], "tmgr") == 0) {
    if ((n_tokens >= 3) && (strcmp(tokens[1], "subport") == 0) &&
        (strcmp(tokens[2], "profile") == 0)) {
      cmd_tmgr_subport_profile(tokens, n_tokens, out, out_size);
      return;
    }
    ...
    if ((n_tokens >= 5) && (strcmp(tokens[2], "subport") == 0) &&
  ...
  if (strcmp(tokens[0], "pipeline") == 0) {
    if ((n_tokens >= 3) && (strcmp(tokens[2], "period") == 0)) {
      cmd_pipeline(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "in") == 0) && (strcmp(tokens[4], "bsz") == 0)) {
      cmd_pipeline_port_in(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "out") == 0) && (strcmp(tokens[4], "bsz") == 0)) {
      cmd_pipeline_port_out(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 4) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[3], "match") == 0)) {
      cmd_pipeline_table(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 6) && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "in") == 0) && (strcmp(tokens[5], "table") == 0)) {
      cmd_pipeline_port_in_table(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 6) && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "in") == 0) && (strcmp(tokens[5], "stats") == 0)) {
      cmd_pipeline_port_in_stats(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 6) && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "in") == 0) && (strcmp(tokens[5], "enable") == 0)) {
      cmd_pipeline_port_in_enable(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 6) && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "in") == 0) && (strcmp(tokens[5], "disable") == 0)) {
      cmd_pipeline_port_in_disable(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 6) && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "out") == 0) && (strcmp(tokens[5], "stats") == 0)) {
      cmd_pipeline_port_out_stats(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "stats") == 0)) {
      cmd_pipeline_table_stats(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 7) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "rule") == 0) && (strcmp(tokens[5], "add") == 0) &&
        (strcmp(tokens[6], "match") == 0)) {
      if ((n_tokens >= 8) && (strcmp(tokens[7], "default") == 0)) {
        cmd_pipeline_table_rule_add_default(tokens, n_tokens, out, out_size);
        return;
      }

      cmd_pipeline_table_rule_add(tokens, n_tokens, out, out_size);
      return;
    }
    ...
    if ((n_tokens >= 7) && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_pipeline_table_rule_add_bulk(tokens, n_tokens, out, out_size);
      return;
    }
    ...
    if ((n_tokens >= 7) && (strcmp(tokens[2], "table") == 0) &&
        ...
        (strcmp(tokens[6], "match") == 0)) {
      if ((n_tokens >= 8) && (strcmp(tokens[7], "default") == 0)) {
        cmd_pipeline_table_rule_delete_default(tokens, n_tokens, out, out_size);
        return;
      }

      cmd_pipeline_table_rule_delete(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 6) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "rule") == 0) && (strcmp(tokens[5], "show") == 0)) {
      cmd_pipeline_table_rule_show(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 7) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "rule") == 0) && (strcmp(tokens[5], "read") == 0) &&
        (strcmp(tokens[6], "stats") == 0)) {
      cmd_pipeline_table_rule_stats_read(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 8) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "meter") == 0) &&
        (strcmp(tokens[5], "profile") == 0) &&
        (strcmp(tokens[7], "add") == 0)) {
      cmd_pipeline_table_meter_profile_add(tokens, n_tokens, out, out_size);
      return;
    }
    ...
    if ((n_tokens >= 8) && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_pipeline_table_meter_profile_delete(tokens, n_tokens, out, out_size);
      return;
    }
    ...
    if ((n_tokens >= 7) && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_pipeline_table_rule_meter_read(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "dscp") == 0)) {
      cmd_pipeline_table_dscp(tokens, n_tokens, out, out_size);
      return;
    }
    ...
    if ((n_tokens >= 7) && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_pipeline_table_rule_ttl_read(tokens, n_tokens, out, out_size);
      return;
    }
    ...
    if ((n_tokens >= 7) && (strcmp(tokens[2], "table") == 0) &&
      ...
      cmd_pipeline_table_rule_time_read(tokens, n_tokens, out, out_size);
      return;
    }
  }

  if (strcmp(tokens[0], "thread") == 0) {
    if ((n_tokens >= 5) && (strcmp(tokens[4], "enable") == 0)) {
      cmd_thread_pipeline_enable(tokens, n_tokens, out, out_size);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[4], "disable") == 0)) {
      cmd_thread_pipeline_disable(tokens, n_tokens, out, out_size);
      return;
    }
  }
...
int cli_script_process(const char *file_name, size_t msg_in_len_max,
  ...
  if ((file_name == NULL) || (strlen(file_name) == 0) ||
      (msg_in_len_max == 0) || (msg_out_len_max == 0))
    return -EINVAL;
  ...
  for (;;) {
    ...
    cli_process(msg_in, msg_out, msg_out_len_max);
...
static int cli_rule_file_process(const char *file_name, size_t line_len_max,
  ...
  /* Check input arguments */
  if ((file_name == NULL) || (strlen(file_name) == 0) || (line_len_max == 0) ||
      (rule_list == NULL) || (n_rules == NULL) || (line_number == NULL) ||
      (out == NULL)) {
    status = -EINVAL;
  ...
  for (line_id = 1, rule_id = 0;; line_id++) {
    ...
    /* Rule match. */
    n_tokens_parsed =
        parse_match(tokens + t0, n_tokens - t0, out, out_size, &rule->match);
    if (n_tokens_parsed == 0) {
      status = -EINVAL;
      goto cli_rule_file_process_free;
    }
    ...
    /* Rule action. */
    n_tokens_parsed = parse_table_action(tokens + t0, n_tokens - t0, out,
                                         out_size, &rule->action);
    if (n_tokens_parsed == 0) {
      status = -EINVAL;
      goto cli_rule_file_process_free;
    }
