...
static struct rte_sched_pipe_params pipe_profile[TMGR_PIPE_PROFILE_MAX];
        ...
        .red_params =
            {
                ...
                [0][1] =
                    {.min_th = 40, .max_th = 64, .maxp_inv = 10, .wq_log2 = 9},
                [0][2] =
                    {.min_th = 32, .max_th = 64, .maxp_inv = 10, .wq_log2 = 9},
...
struct tmgr_port *tmgr_port_find(const char *name) {
...
int tmgr_subport_profile_add(struct rte_sched_subport_profile_params *params) {
  /* Check input params */
  if (params == NULL)
    return -1;

  /* Save profile */
  memcpy(&subport_profile[n_subport_profiles], params, sizeof(*params));
...
int tmgr_pipe_profile_add(struct rte_sched_pipe_params *p) {
  /* Check input params */
  if (p == NULL)
    return -1;

  /* Save profile */
  memcpy(&pipe_profile[n_pipe_profiles], p, sizeof(*p));
...
struct tmgr_port *tmgr_port_create(const char *name,
                                   struct tmgr_port_params *params) {
  ...
  /* Check input params */
  if ((name == NULL) || tmgr_port_find(name) || (params == NULL) ||
      (params->n_subports_per_port == 0) ||
      (params->n_pipes_per_subport == 0) ||
      (params->cpu_id >= RTE_MAX_NUMA_NODES) || (n_subport_profiles == 0) ||
      (n_pipe_profiles == 0))
    return NULL;
  ...
  subport_params.n_pipes_per_subport_enabled = params->n_pipes_per_subport;

  for (i = 0; i < params->n_subports_per_port; i++) {
    int status;

    status = rte_sched_subport_config(s, i, &subport_params, 0);
    ...
    for (j = 0; j < params->n_pipes_per_subport; j++) {
      ...
      status = rte_sched_pipe_config(s, i, j, 0);
...
int tmgr_subport_config(const char *port_name, uint32_t subport_id,
  ...
  /* Resource config */
  status =
      rte_sched_subport_config(port->s, subport_id, NULL, subport_profile_id);

  return status;
}

int tmgr_pipe_config(const char *port_name, uint32_t subport_id,
                     uint32_t pipe_id_first, uint32_t pipe_id_last,
  ...
  if ((port == NULL) || (subport_id >= port->n_subports_per_port) ||
      ...
      (pipe_id_first > pipe_id_last) || (pipe_profile_id >= n_pipe_profiles))
    return -1;
  ...
  for (i = pipe_id_first; i <= pipe_id_last; i++) {
    ...
    status =
        rte_sched_pipe_config(port->s, subport_id, i, (int)pipe_profile_id);
