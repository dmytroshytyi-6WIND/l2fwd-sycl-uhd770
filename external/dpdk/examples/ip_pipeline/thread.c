...
int thread_init(void) {
  ...
  RTE_LCORE_FOREACH_WORKER(i) {
    ...
    msgq_req = rte_ring_create(name, THREAD_MSGQ_SIZE, cpu_id,
    ...
    msgq_rsp = rte_ring_create(name, THREAD_MSGQ_SIZE, cpu_id,
                               RING_F_SP_ENQ | RING_F_SC_DEQ);
...
static struct thread_msg_rsp *thread_msg_send_recv(uint32_t thread_id,
...
int thread_pipeline_enable(uint32_t thread_id, const char *pipeline_name) {
  ...
  /* Check input params */
  if ((thread_id >= RTE_MAX_LCORE) || (p == NULL) || (p->n_ports_in == 0) ||
      (p->n_ports_out == 0) || (p->n_tables == 0))
    return -1;

  t = &thread[thread_id];
  if ((t->enabled == 0) || p->enabled)
    return -1;
...
int thread_pipeline_disable(uint32_t thread_id, const char *pipeline_name) {
  ...
  /* Check input params */
  if ((thread_id >= RTE_MAX_LCORE) || (p == NULL))
    return -1;
...
int pipeline_port_in_stats_read(const char *pipeline_name, uint32_t port_id,
                                struct rte_pipeline_port_in_stats *stats,
                                int clear) {
  struct pipeline *p;
...
int pipeline_port_in_enable(const char *pipeline_name, uint32_t port_id) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  int status;
...
int pipeline_port_in_disable(const char *pipeline_name, uint32_t port_id) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  int status;
...
int pipeline_port_out_stats_read(const char *pipeline_name, uint32_t port_id,
                                 struct rte_pipeline_port_out_stats *stats,
                                 int clear) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
...
int pipeline_table_stats_read(const char *pipeline_name, uint32_t table_id,
                              struct rte_pipeline_table_stats *stats,
                              int clear) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
...
static int match_check(struct table_rule_match *match, struct pipeline *p,
                       uint32_t table_id) {
  struct table *table;
...
static int action_check(struct table_rule_action *action, struct pipeline *p,
                        uint32_t table_id) {
...
static int action_default_check(struct table_rule_action *action,
                                struct pipeline *p, uint32_t table_id) {
  if ((action == NULL) ||
      (action->action_mask != (1LLU << RTE_TABLE_ACTION_FWD)) || (p == NULL) ||
      (table_id >= p->n_tables))
    return -1;
...
static int match_convert(struct table_rule_match *mh,
...
static int table_rule_add_bulk_ll(struct table_ll *table,
                                  struct table_rule_list *list,
                                  uint32_t *n_rules) {
  ...
  if (match_ll == NULL || action_ll == NULL || match_ll_ptr == NULL ||
      action_ll_ptr == NULL || entries_ptr == NULL || found == NULL) {
    status = -ENOMEM;
  ...
  if (table->bulk_supported) {
    status = rte_pipeline_table_entry_add_bulk(table->p, table->table_id,
                                               match_ll_ptr, action_ll_ptr, n,
                                               found, entries_ptr);
    if (status)
      goto table_rule_add_bulk_ll_free;
  } else
    for (i = 0; i < n; i++) {
      status = rte_pipeline_table_entry_add(table->p, table->table_id,
                                            match_ll_ptr[i], action_ll_ptr[i],
                                            &found[i], &entries_ptr[i]);
      if (status) {
        if (i == 0)
          goto table_rule_add_bulk_ll_free;
...
int pipeline_table_rule_add(const char *pipeline_name, uint32_t table_id,
                            struct table_rule_match *match,
                            struct table_rule_action *action) {
  struct pipeline *p;
  struct table *table;
  ...
  p = pipeline_find(pipeline_name);
  if ((p == NULL) || (table_id >= p->n_tables) ||
      match_check(match, p, table_id) || action_check(action, p, table_id))
    return -1;
...
int pipeline_table_rule_add_default(const char *pipeline_name,
                                    uint32_t table_id,
                                    struct table_rule_action *action) {
  struct pipeline *p;
  struct table *table;
  ...
  /* Check input params */
  if ((pipeline_name == NULL) || (action == NULL))
    return -1;
...
static uint32_t table_rule_list_free(struct table_rule_list *list) {
  uint32_t n = 0;
...
int pipeline_table_rule_add_bulk(const char *pipeline_name, uint32_t table_id,
                                 struct table_rule_list *list,
  ...
  /* Check input params */
  if ((pipeline_name == NULL) || (list == NULL) || TAILQ_EMPTY(list) ||
      (n_rules_added == NULL) || (n_rules_not_added == NULL)) {
...
int pipeline_table_rule_delete(const char *pipeline_name, uint32_t table_id,
                               struct table_rule_match *match) {
  struct pipeline *p;
  struct table *table;
  struct pipeline_msg_req *req;
...
int pipeline_table_rule_delete_default(const char *pipeline_name,
                                       uint32_t table_id) {
  struct pipeline *p;
  struct table *table;
  struct pipeline_msg_req *req;
...
int pipeline_table_rule_stats_read(
    const char *pipeline_name, uint32_t table_id,
    struct table_rule_match *match,
    struct rte_table_action_stats_counters *stats, int clear) {
  struct pipeline *p;
  struct table *table;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  ...
  /* Check input params */
  if ((pipeline_name == NULL) || (match == NULL) || (stats == NULL))
    return -1;
  ...
  if (!pipeline_is_running(p)) {
    status = rte_table_action_stats_read(table->a, rule->data, stats, clear);
...
int pipeline_table_mtr_profile_add(
    const char *pipeline_name, uint32_t table_id, uint32_t meter_profile_id,
    struct rte_table_action_meter_profile *profile) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
  ...
  if (!pipeline_is_running(p)) {
    ...
    status = rte_table_action_meter_profile_add(a, meter_profile_id, profile);
...
int pipeline_table_mtr_profile_delete(const char *pipeline_name,
                                      uint32_t table_id,
                                      uint32_t meter_profile_id) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
...
int pipeline_table_rule_mtr_read(const char *pipeline_name, uint32_t table_id,
                                 struct table_rule_match *match,
                                 struct rte_table_action_mtr_counters *stats,
                                 int clear) {
  struct pipeline *p;
  ...
  if (!pipeline_is_running(p)) {
    status = rte_table_action_meter_read(table->a, rule->data, tc_mask, stats,
                                         clear);
...
int pipeline_table_dscp_table_update(
    const char *pipeline_name, uint32_t table_id, uint64_t dscp_mask,
    struct rte_table_action_dscp_table *dscp_table) {
  struct pipeline *p;
  struct pipeline_msg_req *req;
  struct pipeline_msg_rsp *rsp;
...
int pipeline_table_rule_ttl_read(const char *pipeline_name, uint32_t table_id,
                                 struct table_rule_match *match,
                                 struct rte_table_action_ttl_counters *stats,
                                 int clear) {
  struct pipeline *p;
  ...
  if (!pipeline_is_running(p)) {
    status = rte_table_action_ttl_read(table->a, rule->data, stats, clear);
...
int pipeline_table_rule_time_read(const char *pipeline_name, uint32_t table_id,
                                  struct table_rule_match *match,
                                  uint64_t *timestamp) {
  struct pipeline *p;
  struct table *table;
  ...
  /* Check input params */
  if ((pipeline_name == NULL) || (match == NULL) || (timestamp == NULL))
    return -1;
  ...
  if (!pipeline_is_running(p)) {
    status = rte_table_action_time_read(table->a, rule->data, timestamp);
...
pipeline_msg_handle_table_rule_time_read(struct pipeline_data *p,
  ...
  rsp->status =
      rte_table_action_time_read(a, data, &rsp->table_rule_time_read.timestamp);
