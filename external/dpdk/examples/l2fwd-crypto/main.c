...
enum cdev_type { CDEV_TYPE_ANY, CDEV_TYPE_HW, CDEV_TYPE_SW };
...
static void print_stats(void) {
  ...
  uint64_t total_packets_enqueued, total_packets_dequeued, total_packets_errors;
  uint16_t portid;
  uint64_t cdevid;
  ...
  for (cdevid = 0; cdevid < RTE_CRYPTO_MAX_DEVS; cdevid++) {
    ...
    printf("\nStatistics for cryptodev %" PRIu64 " -------------------------"
           ...
           cdevid, crypto_statistics[cdevid].enqueued,
  ...
  printf("\nAggregate statistics ==============================="
         ...
         total_packets_rx, total_packets_enqueued, total_packets_dequeued,
         total_packets_tx, total_packets_dropped, total_packets_errors);
  printf("\n====================================================\n");
...
static int l2fwd_crypto_send_burst(struct lcore_queue_conf *qconf, unsigned n,
  ...
  ret = rte_cryptodev_enqueue_burst(cparams->dev_id, cparams->qp_id, op_buffer,
                                    (uint16_t)n);
...
/* Crypto enqueue. 8< */
static int l2fwd_crypto_enqueue(struct rte_crypto_op *op,
                                struct l2fwd_crypto_params *cparams) {
...
static int l2fwd_simple_crypto_enqueue(struct rte_mbuf *m,
  ...
  ip_hdr = (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(m, char *) + ipdata_offset);

  ipdata_offset +=
      (ip_hdr->version_ihl & RTE_IPV4_HDR_IHL_MASK) * RTE_IPV4_IHL_MULTIPLIER;
  ...
  if (cparams->do_cipher) {
    ...
    case RTE_CRYPTO_CIPHER_3DES_ECB:
      if (data_len % cparams->block_size)
        pad_len = cparams->block_size - (data_len % cparams->block_size);
      break;
  ...
  if (cparams->do_hash) {
    if (cparams->auth_iv.length) {
      uint8_t *iv_ptr = rte_crypto_op_ctod_offset(
          op, uint8_t *, IV_OFFSET + cparams->cipher_iv.length);
      /*
    ...
    op->sym->auth.digest.phys_addr = rte_pktmbuf_iova_offset(
        m, rte_pktmbuf_pkt_len(m) - cparams->digest_length);
  ...
  if (cparams->do_cipher) {
    ...
    /* Copy IV at the end of the crypto operation */
    rte_memcpy(iv_ptr, cparams->cipher_iv.data, cparams->cipher_iv.length);
  ...
  if (cparams->do_aead) {
    ...
    op->sym->aead.digest.phys_addr = rte_pktmbuf_iova_offset(
        m, rte_pktmbuf_pkt_len(m) - cparams->digest_length);
...
static int l2fwd_send_burst(struct lcore_queue_conf *qconf, unsigned n,
                            uint16_t port) {
...
static int l2fwd_send_packet(struct rte_mbuf *m, uint16_t port) {
  unsigned lcore_id, len;
  struct lcore_queue_conf *qconf;
...
static void l2fwd_mac_updating(struct rte_mbuf *m, uint16_t dest_portid) {
...
static void l2fwd_simple_forward(struct rte_mbuf *m, uint16_t portid,
  ...
  if (options->cipher_xform.cipher.op == RTE_CRYPTO_CIPHER_OP_DECRYPT) {
    pad_len =
        m->pkt_len - rte_be_to_cpu_16(ip_hdr->total_length) - ipdata_offset;
...
initialize_crypto_session(struct l2fwd_crypto_options *options,
  ...
  session =
      rte_cryptodev_sym_session_create(session_pool_socket[socket_id].sess_mp);
  if (session == NULL)
    return NULL;

  if (rte_cryptodev_sym_session_init(cdev_id, session, first_xform,
...
static void l2fwd_crypto_options_print(struct l2fwd_crypto_options *options);

/* main processing loop */
static void l2fwd_main_loop(struct l2fwd_crypto_options *options) {
  ...
  for (i = 0; i < qconf->nb_crypto_devs; i++) {
    ...
    if (port_cparams[i].do_hash) {
      ...
      port_cparams[i].digest_length = options->auth_xform.auth.digest_length;
      /* Set IV parameters */
      if (options->auth_iv.length) {
        options->auth_xform.auth.iv.offset =
            IV_OFFSET + options->cipher_iv.length;
        options->auth_xform.auth.iv.length = options->auth_iv.length;
      }
    }
    ...
    if (port_cparams[i].do_aead) {
      ...
      port_cparams[i].digest_length = options->aead_xform.aead.digest_length;
      if (options->aead_xform.aead.aad_length) {
        ...
        if (port_cparams[i].aead_algo == RTE_CRYPTO_AEAD_AES_CCM)
          memmove(port_cparams[i].aad.data + 18, port_cparams[i].aad.data,
                  port_cparams[i].aad.length);
    ...
    session = initialize_crypto_session(options, port_cparams[i].dev_id);
    if (session == NULL)
  ...
  while (1) {
    ...
    if (unlikely(diff_tsc > drain_tsc)) {
      ...
      for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
        ...
        l2fwd_send_burst(&lcore_queue_conf[lcore_id],
                         qconf->pkt_buf[portid].len, portid);
      ...
      if (timer_period > 0) {
        ...
        if (unlikely(timer_tsc >= (uint64_t)timer_period)) {
          ...
          if (lcore_id == rte_get_main_lcore() && options->refresh_period) {
            print_stats();
            timer_tsc = 0;
          }
        }
    ...
    for (i = 0; i < qconf->nb_rx_ports; i++) {
      ...
      if (nb_rx) {
         ...
         */
        if (rte_crypto_op_bulk_alloc(l2fwd_crypto_op_pool,
                                     RTE_CRYPTO_OP_TYPE_SYMMETRIC, ops_burst,
                                     nb_rx) != nb_rx) {
          for (j = 0; j < nb_rx; j++)
      ...
      do {
        nb_rx = rte_cryptodev_dequeue_burst(cparams->dev_id, cparams->qp_id,
        ...
        for (j = 0; j < nb_rx; j++) {
          ...
          l2fwd_simple_forward(m, portid, options);
        }
...
static void l2fwd_crypto_usage(const char *prgname) {
  printf(
      "%s [EAL options] --\n"
      ...
      "  --cipher_key_random_size SIZE: size of cipher key when generated "
      "randomly\n"
      "  --cipher_iv IV (bytes separated with \":\")\n"
      "  --cipher_iv_random_size SIZE: size of cipher IV when generated "
      "randomly\n"
      ...
      "  --aead_key_random_size SIZE: size of AEAD key when generated "
      "randomly\n"
      ...
      "  --sessionless\n"
      "  --cryptodev_mask MASK: hexadecimal bitmask of crypto devices to "
      "configure\n"
...
static int parse_cryptodev_type(enum cdev_type *type, char *optarg) {
...
static int parse_crypto_opt_chain(struct l2fwd_crypto_options *options,
                                  char *optarg) {
...
static int parse_cipher_algo(enum rte_crypto_cipher_algorithm *algo,
                             char *optarg) {
...
static int parse_cipher_op(enum rte_crypto_cipher_operation *op, char *optarg) {
...
static int parse_bytes(uint8_t *data, char *input_arg, uint16_t max_size) {
  ...
  for (byte_count = 0, token = strtok(input_arg, ":");
       (byte_count < max_size) && (token != NULL); token = strtok(NULL, ":")) {
...
/** Parse size param*/
static int parse_size(int *size, const char *q_arg) {
  char *end = NULL;
  unsigned long n;
...
/** Parse crypto cipher operation command line argument */
static int parse_auth_algo(enum rte_crypto_auth_algorithm *algo, char *optarg) {
...
static int parse_auth_op(enum rte_crypto_auth_operation *op, char *optarg) {
...
static int parse_aead_algo(enum rte_crypto_aead_algorithm *algo, char *optarg) {
...
static int parse_aead_op(enum rte_crypto_aead_operation *op, char *optarg) {
  if (strcmp("ENCRYPT", optarg) == 0) {
...
static int parse_cryptodev_mask(struct l2fwd_crypto_options *options,
                                const char *q_arg) {
  char *end = NULL;
  uint64_t pm;
...
l2fwd_crypto_parse_args_long_options(struct l2fwd_crypto_options *options,
  ...
  /* Cipher options */
    ...
    return parse_cipher_algo(&options->cipher_xform.cipher.algo, optarg);

  else if (strcmp(lgopts[option_index].name, "cipher_op") == 0)
    return parse_cipher_op(&options->cipher_xform.cipher.op, optarg);
    ...
    if (retval == 0 && val >= 0) {
      options->cipher_xform.cipher.dataunit_len = (uint32_t)val;
      return 0;
    } else
      return -1;
  }
  ...
  /* AEAD options */
    ...
    return parse_aead_algo(&options->aead_xform.aead.algo, optarg);
  }

  else if (strcmp(lgopts[option_index].name, "aead_op") == 0)
    return parse_aead_op(&options->aead_xform.aead.op, optarg);
    ...
    options->aad_param = 1;
    options->aad.length = parse_bytes(options->aad.data, optarg, MAX_AAD_SIZE);
...
/** Parse port mask */
static int l2fwd_crypto_parse_portmask(struct l2fwd_crypto_options *options,
                                       const char *q_arg) {
  char *end = NULL;
  unsigned long pm;
...
static int l2fwd_crypto_parse_nqueue(struct l2fwd_crypto_options *options,
                                     const char *q_arg) {
  char *end = NULL;
  unsigned long n;
...
/** Parse timer period */
static int l2fwd_crypto_parse_timer_period(struct l2fwd_crypto_options *options,
                                           const char *q_arg) {
  char *end = NULL;
  unsigned long n;
...
static void l2fwd_crypto_default_options(struct l2fwd_crypto_options *options) {
...
static void display_cipher_info(struct l2fwd_crypto_options *options) {
  printf("\n---- Cipher information ---\n");
  printf(
      "Algorithm: %s\n",
      rte_crypto_cipher_algorithm_strings[options->cipher_xform.cipher.algo]);
  rte_hexdump(stdout, "Cipher key:", options->cipher_xform.cipher.key.data,
              options->cipher_xform.cipher.key.length);
  rte_hexdump(stdout, "IV:", options->cipher_iv.data,
              options->cipher_iv.length);
}

static void display_auth_info(struct l2fwd_crypto_options *options) {
  printf("\n---- Authentication information ---\n");
  printf("Algorithm: %s\n",
         rte_crypto_auth_algorithm_strings[options->auth_xform.auth.algo]);
  rte_hexdump(stdout, "Auth key:", options->auth_xform.auth.key.data,
              options->auth_xform.auth.key.length);
  rte_hexdump(stdout, "IV:", options->auth_iv.data, options->auth_iv.length);
}

static void display_aead_info(struct l2fwd_crypto_options *options) {
  printf("\n---- AEAD information ---\n");
  printf("Algorithm: %s\n",
         rte_crypto_aead_algorithm_strings[options->aead_xform.aead.algo]);
  rte_hexdump(stdout, "AEAD key:", options->aead_xform.aead.key.data,
              options->aead_xform.aead.key.length);
...
static int l2fwd_crypto_parse_args(struct l2fwd_crypto_options *options,
                                   int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argvopt, "p:q:sT:", lgopts, &option_index)) !=
         EOF) {
    switch (opt) {
    /* long options */
    case 0:
      retval =
          l2fwd_crypto_parse_args_long_options(options, lgopts, option_index);
      if (retval < 0) {
    ...
    /* timer period */
    case 'T':
      retval = l2fwd_crypto_parse_timer_period(options, optarg);
      if (retval < 0) {
        l2fwd_crypto_usage(prgname);
        return -1;
      }
...
static int check_type(const struct l2fwd_crypto_options *options,
...
check_device_support_cipher_algo(const struct l2fwd_crypto_options *options,
  ...
  if (cap->op == RTE_CRYPTO_OP_TYPE_UNDEFINED) {
    printf("Algorithm %s not supported by cryptodev %u"
           ...
           rte_crypto_cipher_algorithm_strings[opt_cipher_algo], cdev_id,
           options->string_type);
    return NULL;
  }
...
check_device_support_auth_algo(const struct l2fwd_crypto_options *options,
  ...
  enum rte_crypto_auth_algorithm opt_auth_algo = options->auth_xform.auth.algo;
  ...
  if (cap->op == RTE_CRYPTO_OP_TYPE_UNDEFINED) {
    printf("Algorithm %s not supported by cryptodev %u"
           " or device not of preferred type (%s)\n",
           rte_crypto_auth_algorithm_strings[opt_auth_algo], cdev_id,
           options->string_type);
    return NULL;
  }
...
check_device_support_aead_algo(const struct l2fwd_crypto_options *options,
  ...
  enum rte_crypto_aead_algorithm opt_aead_algo = options->aead_xform.aead.algo;
  ...
  if (cap->op == RTE_CRYPTO_OP_TYPE_UNDEFINED) {
    printf("Algorithm %s not supported by cryptodev %u"
           " or device not of preferred type (%s)\n",
           rte_crypto_aead_algorithm_strings[opt_aead_algo], cdev_id,
           options->string_type);
    return NULL;
  }
...
static int check_cryptodev_mask(struct l2fwd_crypto_options *options,
                                uint8_t cdev_id) {
...
static inline int check_supported_size(uint16_t length, uint16_t min,
...
static int check_iv_param(const struct rte_crypto_param_range *iv_range_size,
   ...
   */
  if (iv_param) {
    if (check_supported_size(iv_length, iv_range_size->min, iv_range_size->max,
    ...
    if (check_supported_size(iv_random_size, iv_range_size->min,
                             iv_range_size->max, iv_range_size->increment) != 0)
      return -1;
  }
...
static int check_capabilities(struct l2fwd_crypto_options *options,
                              uint8_t cdev_id) {
  ...
  if (options->xform_chain == L2FWD_CRYPTO_AEAD) {
    ...
    cap = check_device_support_aead_algo(options, &dev_info, cdev_id);
    if (cap == NULL)
      return -1;

    if (check_iv_param(&cap->sym.aead.iv_size, options->aead_iv_param,
                       options->aead_iv_random_size,
                       options->aead_iv.length) != 0) {
      RTE_LOG(DEBUG, USER1, "Device %u does not support IV length\n", cdev_id);
      return -1;
    }
    ...
    if (options->aead_key_param) {
      if (check_supported_size(options->aead_xform.aead.key.length,
     ...
     */
    if (options->aad_param) {
      if (check_supported_size(options->aad.length, cap->sym.aead.aad_size.min,
  ...
  if (options->xform_chain == L2FWD_CRYPTO_CIPHER_HASH ||
    ...
    if (check_iv_param(&cap->sym.cipher.iv_size, options->cipher_iv_param,
    ...
    if (options->ckey_param) {
      if (check_supported_size(options->cipher_xform.cipher.key.length,
                               cap->sym.cipher.key_size.min,
                               cap->sym.cipher.key_size.max,
                               cap->sym.cipher.key_size.increment) != 0) {
        if (dev_info.feature_flags & RTE_CRYPTODEV_FF_CIPHER_WRAPPED_KEY) {
          RTE_LOG(DEBUG, USER1,
      ...
      if (cap->sym.cipher.dataunit_set != 0) {
        ...
        if (ret == -1) {
          RTE_LOG(DEBUG, USER1,
                  "Device %u does not support "
                  "data-unit length %u\n",
                  cdev_id, options->cipher_xform.cipher.dataunit_len);
          return -1;
        }
      }
    }
  ...
  if (options->xform_chain == L2FWD_CRYPTO_CIPHER_HASH ||
    ...
    cap = check_device_support_auth_algo(options, &dev_info, cdev_id);
    if (cap == NULL)
      return -1;

    if (check_iv_param(&cap->sym.auth.iv_size, options->auth_iv_param,
    ...
    if (options->akey_param) {
      if (check_supported_size(options->auth_xform.auth.key.length,
...
static int initialize_cryptodevs(struct l2fwd_crypto_options *options,
                                 unsigned nb_ports, uint8_t *enabled_cdevs) {
  ...
  for (cdev_id = 0; cdev_id < cdev_count; cdev_id++) {
...
#ifdef RTE_CRYPTO_SCHEDULER
      uint32_t nb_workers = rte_cryptodev_scheduler_workers_get(cdev_id, NULL);
    ...
    if (session_pool_socket[socket_id].priv_mp == NULL) {
      ...
      session_pool_socket[socket_id].priv_mp =
          rte_mempool_create(mp_name, sessions_needed, max_sess_sz, 0, 0, NULL,
    ...
    if (session_pool_socket[socket_id].sess_mp == NULL) {
      ...
      session_pool_socket[socket_id].sess_mp =
          rte_cryptodev_sym_session_pool_create(mp_name, sessions_needed, 0, 0,
                                                0, socket_id);
    ...
    if (options->xform_chain == L2FWD_CRYPTO_AEAD) {
      ...
      if (options->aead_iv_param == 0) {
        if (options->aead_iv_random_size != -1)
          options->aead_iv.length = options->aead_iv_random_size;
        /* No size provided, use minimum size. */
        else
          options->aead_iv.length = cap->sym.aead.iv_size.min;
      }

      /* Set key if not provided from command line */
      if (options->aead_key_param == 0) {
        if (options->aead_key_random_size != -1)
          options->aead_xform.aead.key.length = options->aead_key_random_size;
        /* No size provided, use minimum size. */
        else
          options->aead_xform.aead.key.length = cap->sym.aead.key_size.min;
      ...
      /* No size provided, use minimum size. */
        ...
        options->aead_xform.aead.digest_length = cap->sym.aead.digest_size.min;
    }
    ...
    if (options->xform_chain == L2FWD_CRYPTO_CIPHER_HASH ||
      ...
      if (options->cipher_iv_param == 0) {
        if (options->cipher_iv_random_size != -1)
          options->cipher_iv.length = options->cipher_iv_random_size;
        /* No size provided, use minimum size. */
        else
          options->cipher_iv.length = cap->sym.cipher.iv_size.min;
      }

      /* Set key if not provided from command line */
      if (options->ckey_param == 0) {
        if (options->ckey_random_size != -1)
          options->cipher_xform.cipher.key.length = options->ckey_random_size;
        /* No size provided, use minimum size. */
        else
          options->cipher_xform.cipher.key.length =
              cap->sym.cipher.key_size.min;
    ...
    if (options->xform_chain == L2FWD_CRYPTO_CIPHER_HASH ||
      ...
      if (options->auth_iv_param == 0) {
        if (options->auth_iv_random_size != -1)
          options->auth_iv.length = options->auth_iv_random_size;
        /* No size provided, use minimum size. */
        else
          options->auth_iv.length = cap->sym.auth.iv_size.min;
      }

      /* Set key if not provided from command line */
      if (options->akey_param == 0) {
        if (options->akey_random_size != -1)
          options->auth_xform.auth.key.length = options->akey_random_size;
        /* No size provided, use minimum size. */
        else
          options->auth_xform.auth.key.length = cap->sym.auth.key_size.min;
      ...
      /* Set digest size if not provided from command line */
      if (options->digest_size != -1)
        options->auth_xform.auth.digest_length = options->digest_size;
      /* No size provided, use minimum size. */
      else
        options->auth_xform.auth.digest_length = cap->sym.auth.digest_size.min;
    }
    ...
    qp_conf.mp_session_private = session_pool_socket[socket_id].priv_mp;

    retval = rte_cryptodev_queue_pair_setup(cdev_id, 0, &qp_conf, socket_id);
    if (retval < 0) {
      printf("Failed to setup queue pair %u on cryptodev %u", 0, cdev_id);
      return -1;
    }
...
static int initialize_ports(struct l2fwd_crypto_options *options) {
  ...
  RTE_ETH_FOREACH_DEV(portid) {
    ...
    retval = rte_eth_macaddr_get(portid, &l2fwd_ports_eth_addr[portid]);
    if (retval < 0) {
...
static void reserve_key_memory(struct l2fwd_crypto_options *options) {
...
int main(int argc, char **argv) {
  ...
  l2fwd_pktmbuf_pool = rte_pktmbuf_pool_create(
      "mbuf_pool", NB_MBUF, 512,
      RTE_ALIGN(sizeof(struct rte_crypto_op), RTE_CACHE_LINE_SIZE),
      RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
  if (l2fwd_pktmbuf_pool == NULL)
    rte_exit(EXIT_FAILURE, "Cannot create mbuf pool\n");

  /* create crypto op pool */
  l2fwd_crypto_op_pool = rte_crypto_op_pool_create(
      "crypto_op_pool", RTE_CRYPTO_OP_TYPE_SYMMETRIC, NB_MBUF, 128,
  ...
  enabled_cdevcount =
      initialize_cryptodevs(&options, enabled_portcount, enabled_cdevs);
  ...
  for (rx_lcore_id = 0, qconf = NULL, cdev_id = 0;
       cdev_id < nb_cryptodevs && enabled_cdevcount; cdev_id++) {
    ...
    printf("Lcore %u: cryptodev %u\n", rx_lcore_id, (unsigned)cdev_id);
  }

  /* launch per-lcore init on every lcore */
  rte_eal_mp_remote_launch(l2fwd_launch_one_lcore, (void *)&options, CALL_MAIN);
