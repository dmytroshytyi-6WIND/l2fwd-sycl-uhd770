...
static const char *const fwd_mode_s[] = {
    "file-trans", "rxonly", "txonly", "iofwd", NULL,
};
...
static void cmd_sendfile_parsed(void *parsed_result,
  ...
  for (i = 0; i < NTB_MAX_PKT_BURST; i++)
    pkts_send[i] =
        (struct rte_rawdev_buf *)malloc(sizeof(struct rte_rawdev_buf));
  ...
  for (i = 0; i < nb_burst; i++) {
    ...
    if (rte_mempool_get_bulk(mbuf_pool, (void **)mbuf_send, val) == 0) {
      for (nb_pkt = 0; nb_pkt < val; nb_pkt++) {
        mbuf_send[nb_pkt]->port = dev_id;
        mbuf_send[nb_pkt]->data_len = fread(
            rte_pktmbuf_mtod(mbuf_send[nb_pkt], void *), 1, buf_size, file);
        mbuf_send[nb_pkt]->pkt_len = mbuf_send[nb_pkt]->data_len;
        pkts_send[nb_pkt]->buf_addr = mbuf_send[nb_pkt];
      }
    } else {
      for (nb_pkt = 0; nb_pkt < val; nb_pkt++) {
        mbuf_send[nb_pkt] = rte_mbuf_raw_alloc(mbuf_pool);
    ...
    ret =
        rte_rawdev_enqueue_buffers(dev_id, pkts_send, nb_pkt, (void *)queue_id);
    if (ret < 0) {
    ...
    while (nb_tx != nb_pkt && retry < BURST_TX_RETRIES) {
      rte_delay_us(1);
      ret = rte_rawdev_enqueue_buffers(dev_id, &pkts_send[nb_tx],
                                       nb_pkt - nb_tx, (void *)queue_id);
      if (ret < 0) {
        printf("Enqueue failed with err %d\n", ret);
...
cmdline_parse_token_string_t cmd_send_file_send =
    TOKEN_STRING_INITIALIZER(struct cmd_sendfile_result, send_string, "send");
...
static int start_polling_recv_file(void *param) {
  ...
  while (!conf->stopped) {
    ...
    while (file_len < size && !conf->stopped) {
      ret = rte_rawdev_dequeue_buffers(dev_id, pkts_recv, pkt_burst,
                                       (void *)queue_id);
      if (ret < 0) {
      ...
      for (i = 0; i < nb_rx; i++) {
        ...
        fwrite(rte_pktmbuf_mtod(mbuf, void *), 1, mbuf->data_len, file);
...
static int start_iofwd_per_lcore(void *param) {
  ...
  for (i = 0; i < NTB_MAX_PKT_BURST; i++)
    ntb_buf[i] = (struct rte_rawdev_buf *)malloc(sizeof(struct rte_rawdev_buf));

  while (!conf->stopped) {
    for (i = 0; i < conf->nb_stream; i++) {
      fs = fwd_streams[conf->stream_id + i];
      if (fs.tx_ntb) {
        nb_rx = rte_eth_rx_burst(fs.rx_port, fs.qp_id, pkts_burst, pkt_burst);
        if (unlikely(nb_rx == 0))
          continue;
        for (j = 0; j < nb_rx; j++)
          ntb_buf[j]->buf_addr = pkts_burst[j];
        ret = rte_rawdev_enqueue_buffers(fs.tx_port, ntb_buf, nb_rx,
        ...
        ret = rte_rawdev_dequeue_buffers(fs.rx_port, ntb_buf, pkt_burst,
                                         (void *)(size_t)fs.qp_id);
        if (ret < 0) {
          printf("Dequeue failed with err %d\n", ret);
          goto clean;
        ...
        nb_tx = rte_eth_tx_burst(fs.tx_port, fs.qp_id, pkts_burst, nb_rx);
...
static int start_rxonly_per_lcore(void *param) {
  struct rte_rawdev_buf *ntb_buf[NTB_MAX_PKT_BURST];
  struct ntb_fwd_lcore_conf *conf = param;
  struct ntb_fwd_stream fs;
  uint16_t nb_rx;
...
static int start_txonly_per_lcore(void *param) {
  struct rte_rawdev_buf *ntb_buf[NTB_MAX_PKT_BURST];
  struct rte_mbuf *pkts_burst[NTB_MAX_PKT_BURST];
  struct ntb_fwd_lcore_conf *conf = param;
  struct ntb_fwd_stream fs;
  ...
  while (!conf->stopped) {
    for (i = 0; i < conf->nb_stream; i++) {
      fs = fwd_streams[conf->stream_id + i];
      if (rte_mempool_get_bulk(mbuf_pool, (void **)pkts_burst, pkt_burst) ==
          0) {
        for (nb_pkt = 0; nb_pkt < pkt_burst; nb_pkt++) {
          pkts_burst[nb_pkt]->port = dev_id;
          pkts_burst[nb_pkt]->data_len =
              pkts_burst[nb_pkt]->buf_len - RTE_PKTMBUF_HEADROOM;
          pkts_burst[nb_pkt]->pkt_len = pkts_burst[nb_pkt]->data_len;
          ntb_buf[nb_pkt]->buf_addr = pkts_burst[nb_pkt];
        }
      } else {
        for (nb_pkt = 0; nb_pkt < pkt_burst; nb_pkt++) {
          pkts_burst[nb_pkt] = rte_pktmbuf_alloc(mbuf_pool);
      ...
      ret = rte_rawdev_enqueue_buffers(fs.tx_port, ntb_buf, nb_pkt,
                                       (void *)(size_t)fs.qp_id);
      if (ret < 0) {
        printf("Enqueue failed with err %d\n", ret);
        for (j = 0; j < nb_pkt; j++)
...
static int ntb_fwd_config_setup(void) {
  uint16_t i;
...
static void assign_stream_to_lcores(void) {
  struct ntb_fwd_lcore_conf *conf;
  struct ntb_fwd_stream *fs;
    ...
    for (i = 0; i < conf->nb_stream; i++) {
      ...
      if (fwd_mode == IOFWD)
        printf(" + Stream %u : %s%u RX -> %s%u TX,"
               " Q=%u\n",
               conf->stream_id + i, fs->tx_ntb ? "Eth" : "NTB", fs->rx_port,
               fs->tx_ntb ? "NTB" : "Eth", fs->tx_port, fs->qp_id);
...
static void start_pkt_fwd(void) {
  struct ntb_fwd_lcore_conf *conf;
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    conf->stopped = 0;
    if (fwd_mode == FILE_TRANS)
      rte_eal_remote_launch(start_polling_recv_file, conf, lcore_id);
    else if (fwd_mode == IOFWD)
      rte_eal_remote_launch(start_iofwd_per_lcore, conf, lcore_id);
    else if (fwd_mode == RXONLY)
      rte_eal_remote_launch(start_rxonly_per_lcore, conf, lcore_id);
    else if (fwd_mode == TXONLY)
      rte_eal_remote_launch(start_txonly_per_lcore, conf, lcore_id);
  }
}
...
static void ntb_stats_clear(void) {
  int nb_ids, i;
  uint32_t *ids;
...
static inline void ntb_calculate_throughput(uint16_t port) {
  ...
  diff_pkts_rx = (ntb_port_stats[port].rx > prev_pkts_rx[port])
                     ? (ntb_port_stats[port].rx - prev_pkts_rx[port])
                     : 0;
  diff_pkts_tx = (ntb_port_stats[port].tx > prev_pkts_tx[port])
                     ? (ntb_port_stats[port].tx - prev_pkts_tx[port])
                     : 0;
  prev_pkts_rx[port] = ntb_port_stats[port].rx;
  prev_pkts_tx[port] = ntb_port_stats[port].tx;
  mpps_rx = diff_cycles > 0 ? diff_pkts_rx * rte_get_tsc_hz() / diff_cycles : 0;
  mpps_tx = diff_cycles > 0 ? diff_pkts_tx * rte_get_tsc_hz() / diff_cycles : 0;
  printf("  Throughput (since last show)\n");
  printf("  Rx-pps: %12" PRIu64 "\n  Tx-pps: %12" PRIu64 "\n", mpps_rx,
         mpps_tx);
}

static void ntb_stats_display(void) {
  ...
  if (nb_ids != rte_rawdev_xstats_names_get(dev_id, xstats_names, nb_ids)) {
...
static void cmd_set_fwd_mode_parsed(__rte_unused void *parsed_result,
                                    __rte_unused struct cmdline *cl,
                                    __rte_unused void *data) {
...
cmdline_parse_token_string_t cmd_setfwd_mode = TOKEN_STRING_INITIALIZER(
    struct cmd_set_fwd_mode_result, mode, "file-trans#iofwd#txonly#rxonly");
...
static const char short_options[] = "i" /* interactive mode */
    ;
...
static void ntb_usage(const char *prgname) {
  printf("%s [EAL options] -- [options]\n"
         ...
         prgname, NTB_MIN_DESC_SIZE, NTB_MAX_DESC_SIZE, NTB_MAX_PKT_BURST);
}

static void ntb_parse_args(int argc, char **argv) {
  char *prgname = argv[0], **argvopt = argv;
  int opt, opt_idx, n, i;

  while ((opt = getopt_long(argc, argvopt, short_options, lgopts, &opt_idx)) !=
         EOF) {
    switch (opt) {
    case 'i':
    ...
    case OPT_NB_DESC_NUM:
      ...
      if (n >= NTB_MIN_DESC_SIZE && n <= NTB_MAX_DESC_SIZE)
        ...
        rte_exit(EXIT_FAILURE,
                 ...
                 " [%u, %u].\n",
                 NTB_MIN_DESC_SIZE, NTB_MAX_DESC_SIZE);
      break;
...
static void ntb_mempool_mz_free(__rte_unused struct rte_mempool_memhdr *memhdr,
                                void *opaque) {
  const struct rte_memzone *mz = opaque;
  rte_memzone_free(mz);
}

static struct rte_mempool *ntb_mbuf_pool_create(uint16_t mbuf_seg_size,
                                                uint32_t nb_mbuf,
  ...
  mp = rte_mempool_create_empty(
      pool_name, nb_mbuf, (mbuf_seg_size + sizeof(struct rte_mbuf)),
      MEMPOOL_CACHE_SIZE, sizeof(struct rte_pktmbuf_pool_private), socket_id,
      0);
  if (mp == NULL)
    return NULL;
  ...
  ntb_conf->mz_list = rte_zmalloc(
      "ntb_memzone_list", sizeof(struct rte_memzone *) * ntb_info.mw_cnt, 0);
  ...
  for (mz_id = 0; mz_id < ntb_info.mw_cnt; mz_id++) {
    ...
    align =
        ntb_info.mw_size_align ? ntb_info.mw_size[mz_id] : RTE_CACHE_LINE_SIZE;
    ...
    if (mz_id)
      ret =
          rte_mempool_populate_iova(mp, mz->addr, mz->iova, mz->len,
                                    ntb_mempool_mz_free, (void *)(uintptr_t)mz);
    else
      ret = rte_mempool_populate_iova(
          mp, (void *)((size_t)mz->addr + ntb_info.ntb_hdr_size),
          mz->iova + ntb_info.ntb_hdr_size, mz->len - ntb_info.ntb_hdr_size,
          ntb_mempool_mz_free, (void *)(uintptr_t)mz);
    if (ret <= 0) {
      rte_memzone_free(mz);
      rte_mempool_free(mp);
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < RTE_RAWDEV_MAX_DEVS; i++)
    if (rte_rawdevs[i].driver_name &&
        (strncmp(rte_rawdevs[i].driver_name, "raw_ntb", NTB_DRV_NAME_LEN) ==
         0) &&
  ...
  nb_mbuf =
      nb_desc * num_queues * 2 * 2 + rte_lcore_count() * MEMPOOL_CACHE_SIZE;
  mbuf_pool = ntb_mbuf_pool_create(ntb_buf_size, nb_mbuf, ntb_info, &ntb_conf,
                                   rte_socket_id());
  if (mbuf_pool == NULL)
  ...
  for (i = 0; i < num_queues; i++) {
    /* Setup rawdev queue */
    ret = rte_rawdev_queue_setup(dev_id, i, &ntb_q_conf, sizeof(ntb_q_conf));
    if (ret < 0)
      rte_exit(EXIT_FAILURE, "Failed to setup ntb queue %u.\n", i);
  }
  ...
  if (eth_port_id < RTE_MAX_ETHPORTS) {
    ...
    eth_pconf.rx_adv_conf.rss_conf.rss_hf &= ethdev_info.flow_type_rss_offloads;
    ret =
        rte_eth_dev_configure(eth_port_id, num_queues, num_queues, &eth_pconf);
    if (ret)
      rte_exit(EXIT_FAILURE,
    ...
    for (i = 0; i < num_queues; i++) {
      ...
      if (ret < 0)
        rte_exit(EXIT_FAILURE, "Failed to setup eth rxq %u.\n", i);
      ret = rte_eth_tx_queue_setup(eth_port_id, i, nb_desc,
                                   rte_eth_dev_socket_id(eth_port_id),
                                   &eth_tx_conf);
      if (ret < 0)
        rte_exit(EXIT_FAILURE, "Failed to setup eth txq %u.\n", i);
    }
