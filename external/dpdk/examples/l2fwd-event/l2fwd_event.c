...
static inline int l2fwd_event_service_enable(uint32_t service_id) {
...
static __rte_always_inline void l2fwd_event_fwd(struct l2fwd_resources *rsrc,
                                                struct rte_event *ev,
...
l2fwd_event_loop_single(struct l2fwd_resources *rsrc, const uint32_t flags) {
  ...
  while (!rsrc->force_quit) {
    ...
    if (flags & L2FWD_EVENT_TX_ENQ) {
      while (rte_event_enqueue_burst(event_d_id, port_id, &ev, 1) &&
             !rsrc->force_quit)
        ;
    }
    ...
    if (flags & L2FWD_EVENT_TX_DIRECT) {
      while (
          !rte_event_eth_tx_adapter_enqueue(event_d_id, port_id, &ev, 1, 0) &&
          !rsrc->force_quit)
        ;
    }
  }
...
l2fwd_event_loop_burst(struct l2fwd_resources *rsrc, const uint32_t flags) {
  ...
  while (!rsrc->force_quit) {
    ...
    for (i = 0; i < nb_rx; i++) {
      l2fwd_event_fwd(rsrc, &ev[i], tx_q_id, timer_period, flags);
    }
...
static __rte_always_inline void l2fwd_event_loop(struct l2fwd_resources *rsrc,
                                                 const uint32_t flags) {
...
l2fwd_event_vector_fwd(struct l2fwd_resources *rsrc,
  ...
  if (vec->attr_valid) {
    ...
    for (i = 0, j = 1; i < vec->nb_elem; i++, j++) {
      ...
      if (flags & L2FWD_EVENT_UPDT_MAC)
        l2fwd_mac_updating(mbufs[i], vec->port, &rsrc->eth_addr[vec->port]);
    }
    ...
    for (i = 0, j = 1; i < vec->nb_elem; i++, j++) {
      ...
      if (flags & L2FWD_EVENT_UPDT_MAC)
        l2fwd_mac_updating(mbufs[i], mbufs[i]->port,
...
l2fwd_event_loop_vector(struct l2fwd_resources *rsrc, const uint32_t flags) {
  ...
  while (!rsrc->force_quit) {
    ...
    for (i = 0; i < nb_rx; i++) {
      ...
      l2fwd_event_vector_fwd(rsrc, ev[i].vec, timer_period, flags);
    }
...
l2fwd_event_main_loop_tx_d_mac_vec(struct l2fwd_resources *rsrc) {
  l2fwd_event_loop_vector(rsrc, L2FWD_EVENT_UPDT_MAC | L2FWD_EVENT_TX_DIRECT);
}
...
l2fwd_event_main_loop_tx_q_mac_vec(struct l2fwd_resources *rsrc) {
  l2fwd_event_loop_vector(rsrc, L2FWD_EVENT_UPDT_MAC | L2FWD_EVENT_TX_ENQ);
}
...
void l2fwd_event_resource_setup(struct l2fwd_resources *rsrc) {
  ...
  evt_rsrc =
      rte_zmalloc("l2fwd_event", sizeof(struct l2fwd_event_resources), 0);
  if (evt_rsrc == NULL)
