...
static inline void l2fwd_poll_simple_forward(struct l2fwd_resources *rsrc,
                                             struct rte_mbuf *m,
                                             uint32_t portid) {
  ...
  buffer =
      ((struct l2fwd_poll_resources *)rsrc->poll_rsrc)->tx_buffer[dst_port];
...
static void l2fwd_poll_main_loop(struct l2fwd_resources *rsrc) {
  ...
  while (!rsrc->force_quit) {
    ...
    if (unlikely(diff_tsc > drain_tsc)) {
      for (i = 0; i < qconf->n_rx_port; i++) {
        port_id = rsrc->dst_ports[qconf->rx_port_list[i]];
...
static void l2fwd_poll_lcore_config(struct l2fwd_resources *rsrc) {
  struct l2fwd_poll_resources *poll_rsrc = rsrc->poll_rsrc;
...
static void l2fwd_poll_init_tx_buffers(struct l2fwd_resources *rsrc) {
  struct l2fwd_poll_resources *poll_rsrc = rsrc->poll_rsrc;
  uint16_t port_id;
  int ret;
  ...
  RTE_ETH_FOREACH_DEV(port_id) {
    ...
    rte_eth_tx_buffer_init(poll_rsrc->tx_buffer[port_id], MAX_PKT_BURST);

    ret = rte_eth_tx_buffer_set_err_callback(
        poll_rsrc->tx_buffer[port_id], rte_eth_tx_buffer_count_callback,
...
void l2fwd_poll_resource_setup(struct l2fwd_resources *rsrc) {
  ...
  poll_rsrc =
      rte_zmalloc("l2fwd_poll_rsrc", sizeof(struct l2fwd_poll_resources), 0);
  if (poll_rsrc == NULL)
