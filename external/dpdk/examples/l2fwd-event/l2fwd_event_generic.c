...
static uint32_t l2fwd_event_device_setup_generic(struct l2fwd_resources *rsrc) {
...
static void l2fwd_event_port_setup_generic(struct l2fwd_resources *rsrc) {
  struct l2fwd_event_resources *evt_rsrc = rsrc->evt_rsrc;
...
static void l2fwd_event_queue_setup_generic(struct l2fwd_resources *rsrc,
...
static void l2fwd_rx_tx_adapter_setup_generic(struct l2fwd_resources *rsrc) {
  struct l2fwd_event_resources *evt_rsrc = rsrc->evt_rsrc;
  ...
  ret = rte_event_port_link(
      ...
      &evt_rsrc->evq.event_q_id[evt_rsrc->evq.nb_queues - 1], NULL, 1);
  if (ret != 1)
