...
/* display usage */
static void l2fwd_event_usage(const char *prgname) {
  printf(
...
static int l2fwd_event_parse_portmask(const char *portmask) {
  char *end = NULL;
  unsigned long pm;
...
static unsigned int l2fwd_event_parse_nqueue(const char *q_arg) {
  char *end = NULL;
  unsigned long n;
...
static int l2fwd_event_parse_timer_period(const char *q_arg) {
  char *end = NULL;
  int n;
...
static void l2fwd_event_parse_mode(const char *optarg,
...
static void l2fwd_event_parse_eventq_sched(const char *optarg,
                                           struct l2fwd_resources *rsrc) {
...
static int l2fwd_event_parse_args(int argc, char **argv,
                                  struct l2fwd_resources *rsrc) {
  int mac_updating = 1;
  ...
  while ((opt = getopt_long(argc, argvopt, short_options, lgopts,
    ...
    /* portmask */
    case 'p':
      rsrc->enabled_port_mask = l2fwd_event_parse_portmask(optarg);
    ...
    /* nqueue */
    case 'q':
      rsrc->rx_queue_per_lcore = l2fwd_event_parse_nqueue(optarg);
 ...
 */
static int check_port_pair_config(struct l2fwd_resources *rsrc) {
  ...
  for (index = 0; index < rte_eth_dev_count_avail(); index++) {
    ...
    if (port_pair_mask & (1 << portid) && rsrc->dst_ports[portid] != index) {
...
static int l2fwd_launch_one_lcore(void *args) {
...
static void check_all_ports_link_status(struct l2fwd_resources *rsrc,
                                        uint32_t port_mask) {
...
static void print_stats(struct l2fwd_resources *rsrc) {
...
static void l2fwd_event_print_stats(struct l2fwd_resources *rsrc) {
