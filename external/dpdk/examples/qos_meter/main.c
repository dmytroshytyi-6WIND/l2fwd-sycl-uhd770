...
static int app_configure_flow_table(void) {
  uint32_t i;
  int ret;
  ...
  ret = rte_meter_trtcm_profile_config(&app_trtcm_profile, &app_trtcm_params);
  if (ret)
    return ret;
...
static inline void app_set_pkt_color(uint8_t *pkt_data,
                                     enum policer_action color) {
  pkt_data[APP_PKT_COLOR_POS] = (uint8_t)color;
}

static inline int app_pkt_handle(struct rte_mbuf *pkt, uint64_t time) {
  ...
  output_color = (uint8_t)FUNC_METER(&app_flows[flow_id], &PROFILE, time,
                                     pkt_len, (enum rte_color)input_color);
...
static __rte_noreturn int main_loop(__rte_unused void *dummy) {
...
int main(int argc, char **argv) {
  ...
  if (conf.rx_adv_conf.rss_conf.rss_hf !=
    ...
    printf("Port %u modified RSS hash function based on hardware support,"
           ...
           port_rx, port_conf.rx_adv_conf.rss_conf.rss_hf,
  ...
  ret = rte_eth_rx_queue_setup(port_rx, NIC_RX_QUEUE, nb_rxd,
                               rte_eth_dev_socket_id(port_rx), &rxq_conf, pool);
  if (ret < 0)
  ...
  ret = rte_eth_tx_queue_setup(port_rx, NIC_TX_QUEUE, nb_txd,
                               rte_eth_dev_socket_id(port_rx), &txq_conf);
  if (ret < 0)
  ...
  if (conf.rx_adv_conf.rss_conf.rss_hf !=
    ...
    printf("Port %u modified RSS hash function based on hardware support,"
           "requested:%#" PRIx64 " configured:%#" PRIx64 "\n",
           port_tx, port_conf.rx_adv_conf.rss_conf.rss_hf,
           conf.rx_adv_conf.rss_conf.rss_hf);
  }
  ...
  ret = rte_eth_rx_queue_setup(port_tx, NIC_RX_QUEUE, nb_rxd,
                               rte_eth_dev_socket_id(port_tx), NULL, pool);
  if (ret < 0)
    rte_exit(EXIT_FAILURE, "Port %d RX queue setup error (%d)\n", port_tx, ret);
  ...
  tx_buffer = rte_zmalloc_socket(
      "tx_buffer", RTE_ETH_TX_BUFFER_SIZE(RTE_MBUF_F_TX_BURST_MAX), 0,
  ...
  if (ret != 0)
    rte_exit(EXIT_FAILURE, "Port %d promiscuous mode enable error (%s)\n",
