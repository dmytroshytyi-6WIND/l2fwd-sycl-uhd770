...
static inline struct rte_mbuf **mbuf_input(struct rte_mbuf *mbuf) {
  return RTE_MBUF_DYNFIELD(mbuf, input_dynfield_offset, struct rte_mbuf **);
}
...
/* display usage */
static inline void usage(const char *prgname) {
...
/* parse core mask */
static inline uint16_t bbdev_parse_mask(const char *mask) {
  char *end = NULL;
  unsigned long pm;
...
/* parse core mask */
static inline uint16_t bbdev_parse_number(const char *mask) {
  char *end = NULL;
  unsigned long pm;
...
static int bbdev_parse_args(int argc, char **argv,
                            struct app_config_params *app_params) {
  int optind = 0;
  int opt;
  int opt_indx = 0;
  ...
  static struct option lgopts[] = {{"enc_core_mask", required_argument, 0, 'e'},
  ...
  while ((opt = getopt_long(argc, argv, "e:d:p:b:", lgopts, &opt_indx)) !=
    ...
    switch (opt) {
    case 'e':
      app_params->enc_core_mask = bbdev_parse_mask(optarg);
      if (app_params->enc_core_mask == 0) {
        usage(prgname);
        return -1;
      }
      app_params->num_enc_cores = __builtin_popcount(app_params->enc_core_mask);
      break;

    case 'd':
      app_params->dec_core_mask = bbdev_parse_mask(optarg);
      if (app_params->dec_core_mask == 0) {
        usage(prgname);
        return -1;
      }
      app_params->num_dec_cores = __builtin_popcount(app_params->dec_core_mask);
      break;
...
static void print_mac(unsigned int portid,
                      struct rte_ether_addr *bbdev_ports_eth_address) {
  printf("Port %u, MAC address: " RTE_ETHER_ADDR_PRT_FMT "\n\n",
         (unsigned int)portid, RTE_ETHER_ADDR_BYTES(bbdev_ports_eth_address));
}

static inline void pktmbuf_free_bulk(struct rte_mbuf **mbufs,
...
static inline void pktmbuf_input_free_bulk(struct rte_mbuf **mbufs,
                                           unsigned int nb_to_free) {
  unsigned int i;
  for (i = 0; i < nb_to_free; ++i) {
...
#define MAX_CHECK_TIME 90  /* 9s (90 * 100ms) in total */
  ...
  for (count = 0; count <= MAX_CHECK_TIME &&
    ...
    if (link_get_err >= 0 && link.link_status) {
      const char *dp = (link.link_duplex == RTE_ETH_LINK_FULL_DUPLEX)
                           ? "full-duplex"
                           : "half-duplex";
      printf("\nPort %u Link Up - speed %s - %s\n", port_id,
             rte_eth_link_speed_to_str(link.link_speed), dp);
      return 0;
    }
    printf(".");
    fflush(stdout);
...
static inline void add_ether_hdr(struct rte_mbuf *pkt_src,
                                 struct rte_mbuf *pkt_dst) {
...
static inline void add_awgn(struct rte_mbuf **mbufs, uint16_t num_pkts) {
  RTE_SET_USED(mbufs);
 ...
 */
static inline void transform_enc_out_dec_in(struct rte_mbuf **mbufs,
                                            uint8_t *temp_buf,
  ...
  for (i = 0; i < num_pkts; ++i) {
    uint16_t pkt_data_len =
        rte_pktmbuf_data_len(mbufs[i]) - sizeof(struct rte_ether_hdr);

    /* Resize the packet if needed */
    if (pkt_data_len < ncb) {
      char *data = rte_pktmbuf_append(mbufs[i], ncb - pkt_data_len);
      if (data == NULL)
        printf("Not enough space in decoder input packet");
    }
    ...
    for (j = 0; j < 3; ++j) {
      for (l = start_bit_idx; l < start_bit_idx + d; ++l) {
        uint8_t *data = rte_pktmbuf_mtod_offset(
            mbufs[i], uint8_t *, sizeof(struct rte_ether_hdr) + (l >> 3));
        if (*data & (0x80 >> (l & 7)))
...
static inline void verify_data(struct rte_mbuf **mbufs, uint16_t num_pkts) {
  uint16_t i;
  for (i = 0; i < num_pkts; ++i) {
...
static int initialize_ports(struct app_config_params *app_params,
  ...
  for (q = 0; q < app_params->num_enc_cores; q++) {
    ret = rte_eth_rx_queue_setup(port_id, q, RTE_TEST_RX_DESC_DEFAULT,
  ...
  for (q = 0; q < app_params->num_dec_cores; q++) {
    ret = rte_eth_tx_queue_setup(port_id, q, RTE_TEST_TX_DESC_DEFAULT,
                                 rte_eth_dev_socket_id(port_id), NULL);
    if (ret < 0) {
...
static void lcore_conf_init(struct app_config_params *app_params,
  ...
  for (lcore_id = 0; lcore_id < 8 * sizeof(uint64_t); ++lcore_id) {
    ...
    if ((1ULL << lcore_id) & app_params->enc_core_mask) {
      ...
      lconf->enc_queue_id = app_params->enc_queue_ids[enc_q_id++];
    }

    if ((1ULL << lcore_id) & app_params->dec_core_mask) {
      lconf->core_type |= (1 << RTE_BBDEV_OP_TURBO_DEC);
      lconf->tx_queue_id = tx_queue_id++;
      lconf->dec_queue_id = app_params->dec_queue_ids[dec_q_id++];
    }

    lconf->bbdev_enc_op_pool = bbdev_op_pools[RTE_BBDEV_OP_TURBO_ENC];
    lconf->bbdev_dec_op_pool = bbdev_op_pools[RTE_BBDEV_OP_TURBO_DEC];
...
static void print_lcore_stats(struct lcore_statistics *lstats,
                              unsigned int lcore_id) {
...
static void print_stats(struct stats_lcore_params *stats_lcore) {
  ...
  if (ret < 0 || ret > len) {
    free(xstats);
    rte_exit(EXIT_FAILURE, "rte_eth_xstats_get(%u) len%i failed: %d", port_id,
             len, ret);
  }
  ...
  if (ret < 0) {
    free(xstats);
    free(xstats_names);
    rte_exit(EXIT_FAILURE, "ERROR(%d): Failure to get BBDEV %u statistics\n",
             ret, bbdev_id);
  }
  ...
  printf("BBDEV %u: %s enqueue error count:\t\t%" PRIu64 "\n", bbdev_id,
         stats_border, bbstats.enqueue_err_count);
  printf("BBDEV %u: %s dequeue error count:\t\t%" PRIu64 "\n\n", bbdev_id,
         stats_border, bbstats.dequeue_err_count);
...
static inline void run_encoding(struct lcore_conf *lcore_conf) {
  uint16_t i;
  ...
  nb_rx = rte_eth_rx_burst(port_id, rx_queue_id, rx_pkts_burst, MAX_PKT_BURST);
  if (!nb_rx)
    return;
  ...
  for (i = 0; i < nb_rx; i++) {
    ...
    const uint16_t pkt_data_len =
        rte_pktmbuf_data_len(rx_pkts_burst[i]) - sizeof(struct rte_ether_hdr);
    ...
    *mbuf_input(enc_out_pkts[i]) = rx_pkts_burst[i];
    ...
    data = rte_pktmbuf_append(enc_out_pkts[i], sizeof(struct rte_ether_hdr));
    if (data == NULL) {
    ...
    bbdev_ops_burst[i]->turbo_enc.input.data = rx_pkts_burst[i];
    bbdev_ops_burst[i]->turbo_enc.input.offset = sizeof(struct rte_ether_hdr);
    ...
    if (in_data_len < pkt_data_len)
      rte_pktmbuf_trim(rx_pkts_burst[i], pkt_data_len - in_data_len);
    else if (in_data_len > pkt_data_len) {
      data = rte_pktmbuf_append(rx_pkts_burst[i], in_data_len - pkt_data_len);
      if (data == NULL)
        printf("Not enough storage in mbuf to perform the encoding\n");
    }

    bbdev_ops_burst[i]->turbo_enc.output.data = enc_out_pkts[i];
    bbdev_ops_burst[i]->turbo_enc.output.offset = sizeof(struct rte_ether_hdr);
  }

  /* Enqueue packets on BBDevice */
  nb_enq =
      rte_bbdev_enqueue_enc_ops(bbdev_id, enc_queue_id, bbdev_ops_burst, nb_rx);
  if (unlikely(nb_enq < nb_rx)) {
    pktmbuf_input_free_bulk(&enc_out_pkts[nb_enq], nb_rx - nb_enq);
    rte_bbdev_enc_op_free_bulk(&bbdev_ops_burst[nb_enq], nb_rx - nb_enq);
  ...
  nb_deq = 0;
  do {
    nb_deq += rte_bbdev_dequeue_enc_ops(
        bbdev_id, enc_queue_id, &bbdev_ops_burst[nb_deq], nb_enq - nb_deq);
  ...
  if (unlikely(nb_sent < nb_deq)) {
    pktmbuf_input_free_bulk(&enc_out_pkts[nb_sent], nb_deq - nb_sent);
    lcore_stats->enc_to_dec_lost_packets += nb_deq - nb_sent;
  }
}

static void run_decoding(struct lcore_conf *lcore_conf) {
  uint16_t i;
  ...
  nb_recv = rte_ring_dequeue_burst(enc_to_dec_ring, (void **)recv_pkts_burst,
                                   MAX_PKT_BURST, NULL);
  if (!nb_recv)
    return;
  ...
  for (i = 0; i < nb_recv; i++) {
    ...
    bbdev_ops_burst[i]->turbo_dec.input.offset = sizeof(struct rte_ether_hdr);
    bbdev_ops_burst[i]->turbo_dec.input.length =
        rte_pktmbuf_data_len(recv_pkts_burst[i]) - sizeof(struct rte_ether_hdr);

    bbdev_ops_burst[i]->turbo_dec.hard_output.data = recv_pkts_burst[i];
    bbdev_ops_burst[i]->turbo_dec.hard_output.offset =
        sizeof(struct rte_ether_hdr);
  }

  /* Enqueue packets on BBDevice */
  nb_enq = rte_bbdev_enqueue_dec_ops(bbdev_id, bbdev_queue_id, bbdev_ops_burst,
                                     nb_recv);
  if (unlikely(nb_enq < nb_recv)) {
    pktmbuf_input_free_bulk(&recv_pkts_burst[nb_enq], nb_recv - nb_enq);
    rte_bbdev_dec_op_free_bulk(&bbdev_ops_burst[nb_enq], nb_recv - nb_enq);
  ...
  nb_deq = 0;
  do {
    nb_deq += rte_bbdev_dequeue_dec_ops(
        bbdev_id, bbdev_queue_id, &bbdev_ops_burst[nb_deq], nb_enq - nb_deq);
  } while (unlikely(nb_deq < nb_enq));
  ...
  if (unlikely(nb_tx < nb_deq)) {
    pktmbuf_input_free_bulk(&recv_pkts_burst[nb_tx], nb_deq - nb_tx);
...
static int processing_loop(void *arg) {
  ...
  const bool run_encoder =
      (lcore_conf->core_type & (1 << RTE_BBDEV_OP_TURBO_ENC));
  const bool run_decoder =
      (lcore_conf->core_type & (1 << RTE_BBDEV_OP_TURBO_DEC));
...
static int prepare_bbdev_device(unsigned int dev_id,
...
static inline bool check_matching_capabilities(uint64_t mask,
                                               uint64_t required_mask) {
  return (mask & required_mask) == required_mask;
}

static void enable_bbdev(struct app_config_params *app_params) {
  ...
  while (op_cap->type != RTE_BBDEV_OP_NONE) {
    if (op_cap->type == RTE_BBDEV_OP_TURBO_ENC) {
      if (check_matching_capabilities(op_cap->cap.turbo_enc.capability_flags,
                                      def_op_enc.op_flags))
        encoder_capable = true;
    }

    if (op_cap->type == RTE_BBDEV_OP_TURBO_DEC) {
      if (check_matching_capabilities(op_cap->cap.turbo_dec.capability_flags,
                                      def_op_dec.op_flags))
...
int main(int argc, char **argv) {
  ...
  bbdev_op_pools[RTE_BBDEV_OP_TURBO_DEC] =
      rte_bbdev_op_pool_create("bbdev_op_pool_dec", RTE_BBDEV_OP_TURBO_DEC,
                               NB_MBUF, 128, rte_socket_id());
  bbdev_op_pools[RTE_BBDEV_OP_TURBO_ENC] =
      rte_bbdev_op_pool_create("bbdev_op_pool_enc", RTE_BBDEV_OP_TURBO_ENC,
                               NB_MBUF, 128, rte_socket_id());
  ...
  enc_to_dec_ring = rte_ring_create("enc_to_dec_ring", rte_align32pow2(NB_MBUF),
                                    rte_socket_id(), flags);

  /* Get the number of available bbdev devices */
  nb_bbdevs = rte_bbdev_count();
  if (nb_bbdevs <= app_params.bbdev_id)
    rte_exit(EXIT_FAILURE, "%u BBDevs detected, cannot use BBDev with ID %u!\n",
             nb_bbdevs, app_params.bbdev_id);
  printf("Number of bbdevs detected: %d\n", nb_bbdevs);

  if (!rte_eth_dev_is_valid_port(app_params.port_id))
    rte_exit(EXIT_FAILURE, "cannot use port with ID %u!\n", app_params.port_id);

  /* create the mbuf mempool for ethdev pkts */
  ethdev_mbuf_mempool =
      rte_pktmbuf_pool_create("ethdev_mbuf_pool", NB_MBUF, MEMPOOL_CACHE_SIZE,
  ...
  bbdev_mbuf_mempool =
      rte_pktmbuf_pool_create("bbdev_mbuf_pool", NB_MBUF, MEMPOOL_CACHE_SIZE, 0,
                              RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
  if (bbdev_mbuf_mempool == NULL)
    rte_exit(EXIT_FAILURE, "Cannot create ethdev mbuf mempool\n");

  /* register mbuf field to store input pointer */
  input_dynfield_offset = rte_mbuf_dynfield_register(&input_dynfield_desc);
  ...
  for (lcore_id = 0; lcore_id < 8 * sizeof(uint64_t); ++lcore_id)
    if (((1ULL << lcore_id) & app_params.enc_core_mask) ||
      ...
      if (!rte_lcore_is_enabled(lcore_id))
        rte_exit(EXIT_FAILURE, "Requested lcore_id %u is not enabled!\n",
                 lcore_id);
  ...
  lcore_conf_init(&app_params, lcore_conf, bbdev_op_pools, bbdev_mbuf_mempool,
                  enc_to_dec_ring, lcore_stats);
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    if (lcore_conf[lcore_id].core_type != 0)
      ...
      rte_eal_remote_launch(processing_loop, &lcore_conf[lcore_id], lcore_id);
    else if (!stats_thread_started) {
      /* launch statistics printing loop */
      rte_eal_remote_launch(stats_loop, &stats_lcore, lcore_id);
      stats_thread_started = true;
    }
  }

  if (!stats_thread_started && lcore_conf[main_lcore_id].core_type != 0)
    rte_exit(EXIT_FAILURE,
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) { ret |= rte_eal_wait_lcore(lcore_id); }
