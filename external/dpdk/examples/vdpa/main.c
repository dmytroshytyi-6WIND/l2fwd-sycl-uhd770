...
/* display usage */
static void vdpa_usage(const char *prgname) {
  printf("Usage: %s [EAL options] -- "
         "	--interactive|-i: run in interactive mode.\n"
         "	--iface <path>: specify the path prefix of the socket files, "
         "e.g. /tmp/vhost-user-.\n"
...
static int parse_args(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, short_option, long_option, &idx)) !=
    ...
    /* long options */
    case 0:
      if (strncmp(long_option[idx].name, "iface", MAX_PATH_LEN) == 0) {
...
static int new_device(int vid) {
  ...
  for (i = 0; i < MAX_VDPA_SAMPLE_PORTS; i++) {
    ...
    if (!dev) {
      RTE_LOG(ERR, VDPA, "Failed to get generic device for port %d\n", i);
      continue;
    }
    printf("\nnew port %s, device : %s\n", ifname, dev->name);
    vports[i].vid = vid;
...
static int start_vdpa(struct vdpa_port *vport) {
  int ret;
  ...
  if (ret != 0)
    rte_exit(EXIT_FAILURE, "register driver failed: %s\n", socket_path);

  ret = rte_vhost_driver_callback_register(socket_path, &vdpa_sample_devops);
  if (ret != 0)
    rte_exit(EXIT_FAILURE, "register driver ops failed: %s\n", socket_path);

  ret = rte_vhost_driver_attach_vdpa_device(socket_path, vport->dev);
  if (ret != 0)
    rte_exit(EXIT_FAILURE, "attach vdpa device failed: %s\n", socket_path);

  if (rte_vhost_driver_start(socket_path) < 0)
    rte_exit(EXIT_FAILURE, "start vhost driver failed: %s\n", socket_path);
  return 0;
}

static void close_vdpa(struct vdpa_port *vport) {
  int ret;
  char *socket_path = vport->ifname;

  ret = rte_vhost_driver_detach_vdpa_device(socket_path);
  if (ret != 0)
    RTE_LOG(ERR, VDPA, "detach vdpa device failed: %s\n", socket_path);

  ret = rte_vhost_driver_unregister(socket_path);
  if (ret != 0)
    RTE_LOG(ERR, VDPA, "Fail to unregister vhost driver for %s.\n",
            socket_path);
...
static void vdpa_sample_quit(void) {
  int i;
...
static void cmd_help_parsed(__rte_unused void *parsed_result,
  ...
  cmdline_printf(cl, "\n"
                     ...
                     "    list                                      : list all "
                     "available vdpa devices.\n"
                     "    create <socket file> <vdev addr>          : create a "
                     "new vdpa port.\n"
                     "    stats <device ID> <virtio queue ID>       : show "
                     "statistics of virtio queue, 0xffff for all.\n"
                     "    quit                                      : exit "
                     "vdpa sample app.\n");
}
...
static void cmd_list_vdpa_devices_parsed(__rte_unused void *parsed_result,
                                         struct cmdline *cl,
...
static void cmd_create_vdpa_port_parsed(void *parsed_result, struct cmdline *cl,
                                        __rte_unused void *data) {
  struct rte_vdpa_device *dev;
...
static void cmd_device_stats_parsed(void *parsed_result, struct cmdline *cl,
  ...
  if (res->qid == 0xFFFF) {
    ...
    if (last == 0) {
      RTE_LOG(ERR, VDPA,
              "Failed to get num of actual virtqs"
              " for device %s.\n",
              res->bdf);
      return;
    }
  ...
  if (!vport->stats_names) {
    ...
    if (vport->stats_n <= 0) {
      RTE_LOG(ERR, VDPA,
              "Failed to get names number of "
              "device %s stats.\n",
              res->bdf);
      return;
    }
    vport->stats_names = rte_zmalloc(
        NULL,
        (sizeof(*vport->stats_names) + sizeof(*vport->stats)) * vport->stats_n,
        0);
    if (!vport->stats_names) {
      RTE_LOG(ERR, VDPA,
              "Failed to allocate memory for stat"
              " names of device %s.\n",
              res->bdf);
      return;
    }
    ...
    if (vport->stats_n != i) {
      RTE_LOG(ERR, VDPA,
              "Failed to get names of device %s "
              "stats.\n",
              res->bdf);
      return;
    }
    vport->stats =
        (struct rte_vdpa_stat *)(vport->stats_names + vport->stats_n);
  }
  cmdline_printf(cl, "\nDevice %s:\n", res->bdf);
  for (; first <= last; first++) {
    ...
    if (rte_vdpa_get_stats(vport->dev, (int)first, vport->stats,
                           vport->stats_n) <= 0) {
      RTE_LOG(ERR, VDPA,
              "Failed to get vdpa queue statistics"
              " for device %s qid %d.\n",
              res->bdf, (int)first);
      return;
    }
...
int main(int argc, char *argv[]) {
  ...
  if (interactive == 1) {
    ...
    RTE_DEV_FOREACH(dev, "class=vdpa", &dev_iter) {
      ...
      snprintf(vports[devcnt].ifname, MAX_PATH_LEN, "%s%d", iface, devcnt);
