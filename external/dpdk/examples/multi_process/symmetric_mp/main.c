...
static void smp_usage(const char *prgname, const char *errmsg) {
  printf("\nError: %s\n", errmsg);
...
static int smp_parse_args(int argc, char **argv) {
  int opt, ret;
  char **argvopt;
  int option_index;
  ...
  static struct option lgopts[] = {
      {PARAM_NUM_PROCS, 1, 0, 0}, {PARAM_PROC_ID, 1, 0, 0}, {NULL, 0, 0, 0}};
 ...
 */
static inline int smp_port_init(uint16_t port, struct rte_mempool *mbuf_pool,
                                uint16_t num_queues) {
  ...
  if (retval == -EINVAL) {
    printf("Port %u configuration failed. Re-attempting with HW checksum "
           "disabled.\n",
           port);
  ...
  if (retval == -ENOTSUP) {
    printf(
        "Port %u configuration failed. Re-attempting with HW RSS disabled.\n",
        port);
  ...
  for (q = 0; q < rx_rings; q++) {
    retval = rte_eth_rx_queue_setup(
        port, q, nb_rxd, rte_eth_dev_socket_id(port), &rxq_conf, mbuf_pool);
    if (retval < 0)
      return retval;
  }
 ...
 */
static void assign_ports_to_cores(void) {
...
static int lcore_main(void *arg __rte_unused) {
   ...
   */
  msgbufpos +=
      snprintf(msgbuf, sizeof(msgbuf) - msgbufpos, "Lcore %u using ports ", id);
  for (p = start_port; p < end_port; p++) {
    msgbufpos += snprintf(msgbuf + msgbufpos, sizeof(msgbuf) - msgbufpos, "%u ",
                          (unsigned)ports[p]);
  }
  printf("%s\n", msgbuf);
...
int main(int argc, char **argv) {
  ...
  mp = (proc_type == RTE_PROC_SECONDARY)
           ...
           : rte_pktmbuf_pool_create(_SMP_MBUF_POOL, NB_MBUFS, MBUF_CACHE_SIZE,
