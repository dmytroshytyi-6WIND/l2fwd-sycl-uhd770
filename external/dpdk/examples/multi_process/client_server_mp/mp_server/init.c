...
static int init_mbuf_pools(void) {
  ...
  const unsigned int num_mbufs_client =
      num_clients *
      (CLIENT_QUEUE_RINGSIZE + RTE_MP_TX_DESC_DEFAULT * ports->num_ports);
...
static int init_shm_rings(void) {
  ...
  for (i = 0; i < num_clients; i++) {
    ...
    clients[i].rx_q = rte_ring_create(
        q_name, ringsize, socket_id,
  ...
  for (count = 0; count <= MAX_CHECK_TIME; count++) {
    ...
    for (portid = 0; portid < port_num; portid++) {
      ...
      if (print_flag == 1) {
        ...
        printf("Port %d %s\n", ports->id[portid], link_status_text);
        continue;
      }
...
int init(int argc, char *argv[]) {
  ...
  mz = rte_memzone_reserve(MZ_PORT_INFO, sizeof(*ports), rte_socket_id(),
                           NO_FLAGS);
  if (mz == NULL)
