...
static const char *get_printable_mac_addr(uint16_t port) {
  ...
  if (unlikely(port >= RTE_MAX_ETHPORTS)) {
    if (err_address[0] == '\0')
      rte_ether_format_addr(err_address, sizeof(err_address), &null_mac);
    return err_address;
  }
  if (unlikely(addresses[port][0] == '\0')) {
    ...
    rte_ether_format_addr(addresses[port], sizeof(addresses[port]), &mac);
  }
...
static void do_stats_display(void) {
  ...
  for (i = 0; i < ports->num_ports; i++) {
    printf("Port %u - rx: %9" PRIu64 "\t"
           "tx: %9" PRIu64 "\n",
           (unsigned)ports->id[i], ports->rx_stats.rx[i], port_tx[i]);
  }
