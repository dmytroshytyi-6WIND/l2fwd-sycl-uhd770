...
#define BURST_RX_WAIT_US                                                       \
  15                       /* Defines how long we wait between retries on RX   \
                            */
...
/* Enable VM2VM communications. If this is disabled then the MAC address compare
 * is skipped. */
typedef enum {
  VM2VM_DISABLED = 0,
  VM2VM_SOFTWARE = 1,
...
static unsigned num_ports =
    0; /**< The number of ports specified in command line */
...
#define MBUF_TABLE_DRAIN_TSC                                                   \
  ((rte_get_tsc_hz() + US_PER_S - 1) / US_PER_S * BURST_TX_DRAIN_US)

static inline bool is_dma_configured(int16_t dev_id) {
  int i;
...
static inline int open_dma(const char *value) {
  ...
  struct rte_dma_vchan_conf qconf = {.direction = RTE_DMA_DIR_MEM_TO_MEM,
  ...
  args_nr =
      rte_strsplit(substr, strlen(substr), dma_arg, RTE_MAX_VHOST_DEVICE, ',');
  if (args_nr <= 0) {
    ret = -1;
    goto out;
  }
 ...
 */
static inline int get_eth_conf(struct rte_eth_conf *eth_conf,
...
static inline int port_init(uint16_t port) {
  ...
  /* The max pool number from dev_info will be used to validate the pool number
  ...
  for (q = 0; q < rx_rings; q++) {
    retval = rte_eth_rx_queue_setup(
        port, q, rx_ring_size, rte_eth_dev_socket_id(port), rxconf, mbuf_pool);
    if (retval < 0) {
 ...
 */
static int us_vhost_parse_socket_path(const char *q_arg) {
  char *old;
...
static void us_vhost_usage(const char *prgname) {
  RTE_LOG(
      ...
      "		--vm2vm [0|1|2]: disable/software(default)/hardware vm2vm "
      "comms\n"
      ...
      "buffers\n"
      "		--stats [0-N]: 0: Disable stats, N: Time in seconds to print "
      "stats\n"
...
static int us_vhost_parse_args(int argc, char **argv) {
  ...
  static struct option long_option[] = {
      {OPT_VM2VM, required_argument, NULL, OPT_VM2VM_NUM},
      {OPT_RX_RETRY, required_argument, NULL, OPT_RX_RETRY_NUM},
      {OPT_RX_RETRY_DELAY, required_argument, NULL, OPT_RX_RETRY_DELAY_NUM},
      {OPT_RX_RETRY_NUMB, required_argument, NULL, OPT_RX_RETRY_NUMB_NUM},
      {OPT_MERGEABLE, required_argument, NULL, OPT_MERGEABLE_NUM},
      {OPT_STATS, required_argument, NULL, OPT_STATS_NUM},
      {OPT_SOCKET_FILE, required_argument, NULL, OPT_SOCKET_FILE_NUM},
      {OPT_TX_CSUM, required_argument, NULL, OPT_TX_CSUM_NUM},
      {OPT_TSO, required_argument, NULL, OPT_TSO_NUM},
      {OPT_CLIENT, no_argument, NULL, OPT_CLIENT_NUM},
      {OPT_BUILTIN_NET_DRIVER, no_argument, NULL, OPT_BUILTIN_NET_DRIVER_NUM},
      {OPT_DMAS, required_argument, NULL, OPT_DMAS_NUM},
      {OPT_NUM_MBUFS, required_argument, NULL, OPT_NUM_MBUFS_NUM},
      {NULL, 0, 0, 0},
  };

  /* Parse command line */
  while ((opt = getopt_long(argc, argv, "p:P", long_option, &option_index)) !=
         EOF) {
    switch (opt) {
    /* Portmask */
    case 'p':
    ...
    case 'P':
      ...
      vmdq_conf_default.rx_adv_conf.vmdq_rx_conf.rx_mode =
          RTE_ETH_VMDQ_ACCEPT_BROADCAST | RTE_ETH_VMDQ_ACCEPT_MULTICAST;
      break;
...
/*
 * Update the global var NUM_PORTS and array PORTS according to system ports
 * number and return valid ports number
 */
...
static int link_vmdq(struct vhost_dev *vdev, struct rte_mbuf *m) {
  ...
  RTE_LOG(INFO, VHOST_DATA,
          ...
          vdev->vid, RTE_ETHER_ADDR_BYTES(&vdev->mac_address), vdev->vlan_tag);
...
static inline void unlink_vmdq(struct vhost_dev *vdev) {
  ...
  if (vdev->ready == DEVICE_RX) {
    ...
    rx_count = rte_eth_rx_burst(ports[0], (uint16_t)vdev->vmdq_rx_q, pkts_burst,
                                MAX_PKT_BURST);
...
static __rte_always_inline void complete_async_pkts(struct vhost_dev *vdev) {
  struct rte_mbuf *p_cpl[MAX_PKT_BURST];
  uint16_t complete_count;
  int16_t dma_id = dma_bind[vdev->vid].dmas[VIRTIO_RXQ].dev_id;

  complete_count = rte_vhost_poll_enqueue_completed(
      vdev->vid, VIRTIO_RXQ, p_cpl, MAX_PKT_BURST, dma_id, 0);
  if (complete_count) {
...
static __rte_always_inline void sync_virtio_xmit(struct vhost_dev *dst_vdev,
                                                 struct vhost_dev *src_vdev,
                                                 struct rte_mbuf *m) {
  uint16_t ret;
  ...
  if (enable_stats) {
    __atomic_add_fetch(&dst_vdev->stats.rx_total_atomic, 1, __ATOMIC_SEQ_CST);
    __atomic_add_fetch(&dst_vdev->stats.rx_atomic, ret, __ATOMIC_SEQ_CST);
    src_vdev->stats.tx_total++;
    src_vdev->stats.tx += ret;
  }
}

static __rte_always_inline void drain_vhost(struct vhost_dev *vdev) {
  uint16_t ret;
  ...
  if (enable_stats) {
    __atomic_add_fetch(&vdev->stats.rx_total_atomic, nr_xmit, __ATOMIC_SEQ_CST);
    __atomic_add_fetch(&vdev->stats.rx_atomic, ret, __ATOMIC_SEQ_CST);
  }
...
static __rte_always_inline void drain_vhost_table(void) {
 ...
 */
static __rte_always_inline int virtio_tx_local(struct vhost_dev *vdev,
                                               struct rte_mbuf *m) {
  struct rte_ether_hdr *pkt_hdr;
 ...
 */
static __rte_always_inline int find_local_dest(struct vhost_dev *vdev,
                                               struct rte_mbuf *m,
                                               uint32_t *offset,
                                               uint16_t *vlan_tag) {
  struct vhost_dev *dst_vdev;
...
static void virtio_tx_offload(struct rte_mbuf *m) {
  ...
  tcp_hdr =
      rte_pktmbuf_mtod_offset(m, struct rte_tcp_hdr *, m->l2_len + m->l3_len);
...
static __rte_always_inline void drain_eth_rx(struct vhost_dev *vdev) {
   ...
   */
  if (enable_retry &&
      unlikely(rx_count > rte_vhost_avail_entries(vdev->vid, VIRTIO_RXQ))) {
    uint32_t retry;
  ...
  if (builtin_net_driver) {
    ...
    complete_async_pkts(vdev);
    enqueue_count = rte_vhost_submit_enqueue_burst(vdev->vid, VIRTIO_RXQ, pkts,
                                                   rx_count, dma_id, 0);
    ...
    enqueue_count =
        rte_vhost_enqueue_burst(vdev->vid, VIRTIO_RXQ, pkts, rx_count);
  }

  if (enable_stats) {
    __atomic_add_fetch(&vdev->stats.rx_total_atomic, rx_count,
                       __ATOMIC_SEQ_CST);
    __atomic_add_fetch(&vdev->stats.rx_atomic, enqueue_count, __ATOMIC_SEQ_CST);
  }
...
static __rte_always_inline void drain_virtio_tx(struct vhost_dev *vdev) {
  ...
  if (builtin_net_driver) {
    ...
    count = rte_vhost_dequeue_burst(vdev->vid, VIRTIO_TXQ, mbuf_pool, pkts,
                                    MAX_PKT_BURST);
  }
...
static void destroy_device(int vid) {
  ...
  if (dma_bind[vid].dmas[VIRTIO_RXQ].async_enabled) {
    ...
    while (vdev->pkts_inflight) {
      n_pkt = rte_vhost_clear_queue_thread_unsafe(
          vid, VIRTIO_RXQ, m_cpl, vdev->pkts_inflight, dma_id, 0);
...
static int new_device(int vid) {
  ...
  for (i = 0; i < RTE_MAX_LCORE; i++) {
    vhost_txbuff[i * RTE_MAX_VHOST_DEVICE + vid] = rte_zmalloc(
        "vhost bufftable", sizeof(struct vhost_bufftable), RTE_CACHE_LINE_SIZE);

    if (vhost_txbuff[i * RTE_MAX_VHOST_DEVICE + vid] == NULL) {
      RTE_LOG(INFO, VHOST_DATA, "(%d) couldn't allocate memory for vhost TX\n",
              vid);
      return -1;
    }
  }
...
static int vring_state_changed(int vid, uint16_t queue_id, int enable) {
  ...
  if (dma_bind[vid].dmas[queue_id].async_enabled) {
    if (!enable) {
      ...
      while (vdev->pkts_inflight) {
        n_pkt = rte_vhost_clear_queue_thread_unsafe(
            vid, queue_id, m_cpl, vdev->pkts_inflight, dma_id, 0);
        free_pkts(m_cpl, n_pkt);
        __atomic_sub_fetch(&vdev->pkts_inflight, n_pkt, __ATOMIC_SEQ_CST);
      }
    }
...
static void *print_stats(__rte_unused void *arg) {
  ...
  while (1) {
    ...
    TAILQ_FOREACH(vdev, &vhost_dev_list, global_vdev_entry) {
      ...
      rx_total =
          __atomic_load_n(&vdev->stats.rx_total_atomic, __ATOMIC_SEQ_CST);
      rx = __atomic_load_n(&vdev->stats.rx_atomic, __ATOMIC_SEQ_CST);
...
int main(int argc, char *argv[]) {
   ...
   */
  mbuf_pool =
      rte_pktmbuf_pool_create("MBUF_POOL", total_num_mbufs, MBUF_CACHE_SIZE, 0,
