...
static __rte_always_inline int enqueue_pkt(struct vhost_dev *dev,
  ...
  desc_addr =
      rte_vhost_va_from_guest_pa(dev->mem, desc_gaddr, &desc_chunck_len);
  /*
...
static __rte_always_inline int dequeue_pkt(struct vhost_dev *dev,
                                           struct rte_vhost_vring *vr,
                                           struct rte_mbuf *m,
                                           uint16_t desc_idx,
  ...
  while (1) {
    ...
    rte_memcpy(rte_pktmbuf_mtod_offset(cur, void *, mbuf_offset),
               (void *)((uintptr_t)(desc_addr + desc_offset)), cpy_len);
