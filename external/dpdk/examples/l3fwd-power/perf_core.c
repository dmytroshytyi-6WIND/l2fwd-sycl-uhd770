...
static int is_hp_core(unsigned int lcore) {
  ...
  for (i = 0; i < nb_prf_lc_prms; i++) {
    ...
    if (prf_lc_prms[i].high_perf) {
      ...
      if (prf_lc_prms[i].lcore_idx < nb_non_perf_lcores)
        lcore = non_perf_lcores[prf_lc_prms[i].lcore_idx];
    }
...
int parse_perf_config(const char *q_arg) {
  ...
  while ((p = strchr(p0, '(')) != NULL) {
    ...
    prf_lc_prms[nb_prf_lc_prms].port_id = (uint8_t)int_fld[FLD_PORT];
    prf_lc_prms[nb_prf_lc_prms].queue_id = (uint8_t)int_fld[FLD_QUEUE];
    prf_lc_prms[nb_prf_lc_prms].high_perf = !!(uint8_t)int_fld[FLD_LCORE_HP];
    prf_lc_prms[nb_prf_lc_prms].lcore_idx = (uint8_t)int_fld[FLD_LCORE_IDX];
    ++nb_prf_lc_prms;
  }
