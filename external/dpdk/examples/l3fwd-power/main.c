...
const struct telstats_name telstats_strings[] = {
    {"empty_poll"}, {"full_poll"}, {"busy_percent"}};

/* core busyness in percentage */
enum busy_rate { ZERO = 0, PARTIAL = 50, FULL = 100 };
...
static struct ipv4_l3fwd_route ipv4_l3fwd_route_array[] = {
    {{RTE_IPV4(100, 10, 0, 1), RTE_IPV4(200, 10, 0, 1), 101, 11, IPPROTO_TCP},
     0},
    {{RTE_IPV4(100, 20, 0, 2), RTE_IPV4(200, 20, 0, 2), 102, 12, IPPROTO_TCP},
     1},
    {{RTE_IPV4(100, 30, 0, 3), RTE_IPV4(200, 30, 0, 3), 103, 13, IPPROTO_TCP},
     2},
    {{RTE_IPV4(100, 40, 0, 4), RTE_IPV4(200, 40, 0, 4), 104, 14, IPPROTO_TCP},
     3},
};
...
static struct ipv6_l3fwd_route ipv6_l3fwd_route_array[] = {
    {{{0xfe, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x1b, 0x21, 0xff,
       0xfe, 0x91, 0x38, 0x05},
      {0xfe, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x1e, 0x67, 0xff,
       0xfe, 0x0d, 0xb6, 0x0a},
      1,
      10,
      IPPROTO_UDP},
...
static struct ipv4_l3fwd_route ipv4_l3fwd_route_array[] = {
    {RTE_IPV4(1, 1, 1, 0), 24, 0}, {RTE_IPV4(2, 1, 1, 0), 24, 1},
    {RTE_IPV4(3, 1, 1, 0), 24, 2}, {RTE_IPV4(4, 1, 1, 0), 24, 3},
    {RTE_IPV4(5, 1, 1, 0), 24, 4}, {RTE_IPV4(6, 1, 1, 0), 24, 5},
...
power_freq_scaleup_heuristic(unsigned int lcore_id, uint16_t port_id,
                             uint16_t queue_id);
...
static int is_done(void) { return quit_signal; }
...
static void power_timer_cb(__rte_unused struct rte_timer *tim,
   ...
   */
  hz = rte_get_timer_hz();
  rte_timer_reset(&power_timers[lcore_id], hz / TIMER_NUMBER_PER_SECOND, SINGLE,
...
static inline int send_single_packet(struct rte_mbuf *m, uint16_t port) {
  uint32_t lcore_id;
...
static inline int is_valid_ipv4_pkt(struct rte_ipv4_hdr *pkt,
                                    uint32_t link_len) {
...
#if (APP_LOOKUP_METHOD == APP_LOOKUP_EXACT_MATCH)
static void print_ipv4_key(struct ipv4_5tuple key) {
  printf("IP dst = %08x, IP src = %08x, port dst = %d, port src = %d, "
         "proto = %d\n",
         (unsigned)key.ip_dst, (unsigned)key.ip_src, key.port_dst, key.port_src,
         key.proto);
}
static void print_ipv6_key(struct ipv6_5tuple key) {
  printf("IP dst = " IPv6_BYTES_FMT ", IP src = " IPv6_BYTES_FMT ", "
         "port dst = %d, port src = %d, proto = %d\n",
         IPv6_BYTES(key.ip_dst), IPv6_BYTES(key.ip_src), key.port_dst,
...
static inline void parse_ptype_one(struct rte_mbuf *m) {
...
static int add_cb_parse_ptype(uint16_t portid, uint16_t queueid) {
...
static int sleep_until_rx_interrupt(int num, int lcore) {
   ...
   */
  static struct { bool wakeup; } __rte_cache_aligned status[RTE_MAX_LCORE];
...
static int event_register(struct lcore_conf *qconf) {
  ...
  for (i = 0; i < qconf->n_rx_queue; ++i) {
    ...
    ret = rte_eth_dev_rx_intr_ctl_q(portid, queueid, RTE_EPOLL_PER_THREAD,
                                    RTE_INTR_EVENT_ADD,
                                    (void *)((uintptr_t)data));
    if (ret)
      return ret;
...
static int main_intr_loop(__rte_unused void *dummy) {
  ...
  for (i = 0; i < qconf->n_rx_queue; i++) {
    ...
    RTE_LOG(INFO, L3FWD_POWER, " -- lcoreid=%u portid=%u rxqueueid=%hhu\n",
  ...
  while (!is_done()) {
    ...
    if (unlikely(diff_tsc > drain_tsc)) {
      for (i = 0; i < qconf->n_tx_port; ++i) {
        ...
        rte_eth_tx_buffer_flush(portid, qconf->tx_queue_id[portid],
  ...
  start_rx:
    ...
    for (i = 0; i < qconf->n_rx_queue; ++i) {
      ...
      if (unlikely(nb_rx == 0)) {
        ...
        rx_queue->idle_hint =
            power_idle_heuristic(rx_queue->zero_rx_packet_count);
        lcore_rx_idle_count++;
      } else {
    ...
    if (unlikely(lcore_rx_idle_count == qconf->n_rx_queue)) {
       ...
       * less as possible.
       */
      for (i = 1, lcore_idle_hint = qconf->rx_queue_list[0].idle_hint;
      ...
      if (lcore_idle_hint < SUSPEND_THRESHOLD)
        ...
        if (intr_en) {
          ...
          sleep_until_rx_interrupt(qconf->n_rx_queue, lcore_id);
...
static int main_telemetry_loop(__rte_unused void *dummy) {
  struct rte_mbuf *pkts_burst[MAX_PKT_BURST];
  unsigned int lcore_id;
  ...
  while (!is_done()) {
    ...
    if (unlikely(poll_count >= DEFAULT_COUNT)) {
      ...
      if (diff_tsc >= MAX_CYCLES) {
        br = FULL;
      } else if (diff_tsc > MIN_CYCLES && diff_tsc < MAX_CYCLES) {
...
/* main processing loop */
static int main_empty_poll_loop(__rte_unused void *dummy) {
  struct rte_mbuf *pkts_burst[MAX_PKT_BURST];
  unsigned int lcore_id;
  uint64_t prev_tsc, diff_tsc, cur_tsc;
  int i, j, nb_rx;
...
/* main processing loop */
static int main_legacy_loop(__rte_unused void *dummy) {
  struct rte_mbuf *pkts_burst[MAX_PKT_BURST];
  unsigned lcore_id;
    ...
    for (i = 0; i < qconf->n_rx_queue; ++i) {
      ...
      if (unlikely(nb_rx == 0)) {
        ...
        rx_queue->freq_up_hint =
            power_freq_scaleup_heuristic(lcore_id, portid, queueid);
      }
...
static int parse_pmd_mgmt_config(const char *name) {
...
static int parse_args(int argc, char **argv) {
  ...
  uint32_t limit;
  char *prgname = argv[0];
  static struct option lgopts[] = {{"config", 1, 0, 0},
                                   {"perf-config", 1, 0, 0},
  ...
  while ((opt = getopt_long(argc, argvopt, "p:l:m:h:P", lgopts,
                            &option_index)) != EOF) {
    ...
    case 0:
      ...
      if (!strncmp(lgopts[option_index].name, "perf-config", 11)) {
      ...
      if (!strncmp(lgopts[option_index].name, "high-perf-cores", 15)) {
      ...
      if (!strncmp(lgopts[option_index].name, CMD_LINE_OPT_LEGACY,
      ...
      if (!strncmp(lgopts[option_index].name, CMD_LINE_OPT_EMPTY_POLL, 10)) {
        if (app_mode != APP_MODE_DEFAULT) {
      ...
      if (!strncmp(lgopts[option_index].name, CMD_LINE_OPT_TELEMETRY,
      ...
      if (!strncmp(lgopts[option_index].name, CMD_LINE_OPT_PMD_MGMT,
      ...
      if (!strncmp(lgopts[option_index].name, CMD_LINE_OPT_INTERRUPT_ONLY,
        ...
        if (app_mode != APP_MODE_DEFAULT) {
          printf(
              " interrupt-only mode is mutually exclusive with other modes\n");
          return -1;
        }
      ...
      if (!strncmp(lgopts[option_index].name, CMD_LINE_OPT_MAX_PKT_LEN,
...
static void setup_lpm(int socketid) {
  ...
  for (i = 0; i < RTE_DIM(ipv4_l3fwd_route_array); i++) {
    ret = rte_lpm_add(
        ipv4_l3fwd_lookup_struct[socketid], ipv4_l3fwd_route_array[i].ip,
...
static int deinit_power_library(void) {
  unsigned int lcore_id;
  int ret = 0;
  ...
  RTE_LCORE_FOREACH(lcore_id) {
    ...
    if (ret) {
      RTE_LOG(ERR, POWER, "Library deinitialization failed on core %u\n",
              lcore_id);
      return ret;
    }
  }
...
static void get_current_stat_values(uint64_t *values) {
...
static void update_telemetry(__rte_unused struct rte_timer *tim,
                             __rte_unused void *arg) {
  int ret;
  uint64_t values[NUM_TELSTATS] = {0};

  get_current_stat_values(values);
  ret = rte_metrics_update_values(RTE_METRICS_GLOBAL, telstats_index, values,
                                  RTE_DIM(values));
  if (ret < 0)
    RTE_LOG(WARNING, POWER, "failed to update metrics\n");
}

static int handle_app_stats(const char *cmd __rte_unused,
  ...
  for (i = 0; i < NUM_TELSTATS; i++)
    rte_tel_data_add_dict_u64(d, telstats_strings[i].name, values[i]);
  return 0;
}

static void telemetry_setup_timer(void) {
  int lcore_id = rte_lcore_id();
  uint64_t hz = rte_get_timer_hz();
  uint64_t ticks;

  ticks = hz / TELEMETRY_INTERVALS_PER_SEC;
  rte_timer_reset_sync(&telemetry_timer, ticks, PERIODICAL, lcore_id,
                       update_telemetry, NULL);
}
static void empty_poll_setup_timer(void) {
  ...
  rte_timer_reset_sync(&ep_ptr->timer0, ep_ptr->interval_ticks, PERIODICAL,
                       lcore_id, rte_empty_poll_detection, (void *)ep_ptr);
}
...
static int autodetect_mode(void) {
  RTE_LOG(
      NOTICE, L3FWD_POWER,
...
static const char *mode_to_str(enum appmode mode) {
  switch (mode) {
...
int main(int argc, char **argv) {
  ...
  RTE_ETH_FOREACH_DEV(portid) {
    ...
    bool need_intr =
        app_mode == APP_MODE_LEGACY || app_mode == APP_MODE_INTERRUPT;
  ...
  for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
    ...
    for (queue = 0; queue < qconf->n_rx_queue; ++queue) {
      ...
      ret = rte_eth_rx_queue_setup(portid, queueid, nb_rxd, socketid, &rxq_conf,
                                   pktmbuf_pool[socketid]);
      if (ret < 0)
        rte_exit(EXIT_FAILURE,
      ...
      if (app_mode == APP_MODE_PMD_MGMT && !baseline_enabled) {
        ret = rte_power_ethdev_pmgmt_queue_enable(lcore_id, portid, queueid,
                                                  pmgmt_type);
        if (ret < 0)
          rte_exit(EXIT_FAILURE,
  ...
  RTE_ETH_FOREACH_DEV(portid) {
    ...
    if (!parse_ptype)
      if (!check_ptype(portid))
        rte_exit(EXIT_FAILURE, "PMD can not provide needed ptypes\n");
  }
  ...
  if (app_mode == APP_MODE_LEGACY) {
    ...
    rte_eal_mp_remote_launch(main_empty_poll_loop, NULL, SKIP_MAIN);
    ...
    rte_telemetry_register_cmd("/l3fwd-power/stats", handle_app_stats,
                               "Returns global power stats. Parameters: None");
    rte_eal_mp_remote_launch(main_telemetry_loop, NULL, SKIP_MAIN);
