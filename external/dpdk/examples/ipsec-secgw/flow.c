...
static void ipv4_hdr_print(struct rte_ipv4_hdr *hdr) {
  char a, b, c, d;
...
static int ipv4_addr_cpy(rte_be32_t *spec, rte_be32_t *mask, char *token,
...
static void ipv6_hdr_print(struct rte_ipv6_hdr *hdr) {
  uint8_t *addr;
...
static int ipv6_addr_cpy(uint8_t *spec, uint8_t *mask, char *token,
                         struct parse_status *status) {
  struct in6_addr ip;
  uint32_t depth, i;
    ...
    if (strcmp(tokens[ti], "src") == 0) {
      ...
      if (rule->is_ipv4) {
        if (ipv4_addr_cpy(&rule->ipv4.spec.hdr.src_addr,
                          &rule->ipv4.mask.hdr.src_addr, tokens[ti], status))
          return;
      } else {
        if (ipv6_addr_cpy(rule->ipv6.spec.hdr.src_addr,
                          rule->ipv6.mask.hdr.src_addr, tokens[ti], status))
          return;
      }
    }
    if (strcmp(tokens[ti], "dst") == 0) {
      ...
      if (rule->is_ipv4) {
        if (ipv4_addr_cpy(&rule->ipv4.spec.hdr.dst_addr,
                          &rule->ipv4.mask.hdr.dst_addr, tokens[ti], status))
          return;
      } else {
        if (ipv6_addr_cpy(rule->ipv6.spec.hdr.dst_addr,
                          rule->ipv6.mask.hdr.dst_addr, tokens[ti], status))
          return;
      }
    }
...
static void flow_init_single(struct flow_rule_entry *rule) {
