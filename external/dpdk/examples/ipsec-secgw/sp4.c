...
#define IPV4_DST_MASK_FROM_SP(acr) ((acr).field[DST_FIELD_IPV4].mask_range.u32)

#define IPV4_SRC_MASK_FROM_SP(acr) ((acr).field[SRC_FIELD_IPV4].mask_range.u32)
...
static struct rte_acl_field_def ip4_defs[NUM_FIELDS_IPV4] = {
    ...
    {.type = RTE_ACL_FIELD_TYPE_RANGE,
     ...
     .input_index = RTE_ACL_IPV4_PORTS,
     .offset =
         sizeof(struct ip) - offsetof(struct ip, ip_p) + sizeof(uint16_t)},
};
...
static int extend_sp_arr(struct acl4_rules **sp_tbl, uint32_t cur_cnt,
                         uint32_t *cur_sz) {
  if (*sp_tbl == NULL) {
  ...
  if (cur_cnt >= *cur_sz) {
    *sp_tbl = realloc(*sp_tbl, *cur_sz * sizeof(struct acl4_rules) * 2);
    if (*sp_tbl == NULL)
      return -1;
    /* clean reallocated extra space */
    memset(&(*sp_tbl)[*cur_sz], 0, *cur_sz * sizeof(struct acl4_rules));
    *cur_sz *= 2;
  }
...
void parse_sp4_tokens(char **tokens, uint32_t n_tokens,
  ...
  if (strcmp(tokens[1], "in") == 0) {
    ...
    if (extend_sp_arr(&acl4_rules_in, nb_acl4_rules_in, &sp_in_sz) < 0)
      return;
    ...
    if (extend_sp_arr(&acl4_rules_out, nb_acl4_rules_out, &sp_out_sz) < 0)
      return;
  ...
  for (ti = 2; ti < n_tokens; ti++) {
    ...
    if (strcmp(tokens[ti], "src") == 0) {
      ...
      APP_CHECK(parse_ipv4_addr(tokens[ti], &ip, &depth) == 0, status,
                "unrecognized "
      ...
      rule_ipv4->field[1].value.u32 = rte_bswap32(ip.s_addr);
      rule_ipv4->field[1].mask_range.u32 = depth;
    ...
    if (strcmp(tokens[ti], "dst") == 0) {
      ...
      rule_ipv4->field[2].value.u32 = rte_bswap32(ip.s_addr);
      rule_ipv4->field[2].mask_range.u32 = depth;
...
static void print_one_ip4_rule(const struct acl4_rules *rule, int32_t extra) {
  uint8_t a, b, c, d;
...
static inline void dump_ip4_rules(const struct acl4_rules *rule, int32_t num,
                                  int32_t extra) {
  int32_t i;
...
static struct rte_acl_ctx *acl4_init(const char *name, int32_t socketid,
                                     const struct acl4_rules *rules,
                                     uint32_t rules_nb) {
  char s[PATH_MAX];
...
void sp4_init(struct socket_ctx *ctx, int32_t socket_id) {
  ...
  if (check_spi_value(ctx->sa_in, 1) < 0)
    rte_exit(EXIT_FAILURE, "Inbound IPv4 SP DB has unmatched in SAD SPIs\n");

  if (check_spi_value(ctx->sa_out, 0) < 0)
    rte_exit(EXIT_FAILURE, "Outbound IPv4 SP DB has unmatched in SAD SPIs\n");

  if (nb_acl4_rules_in > 0) {
    name = "sp_ip4_in";
    ctx->sp_ip4_in = (struct sp_ctx *)acl4_init(name, socket_id, acl4_rules_in,
                                                nb_acl4_rules_in);
  } else
  ...
  if (nb_acl4_rules_out > 0) {
    name = "sp_ip4_out";
    ctx->sp_ip4_out = (struct sp_ctx *)acl4_init(
        name, socket_id, acl4_rules_out, nb_acl4_rules_out);
  } else
    RTE_LOG(WARNING, IPSEC,
 ...
 */
int sp4_spi_present(uint32_t spi, int inbound, struct ip_addr ip_addr[2],
                    uint32_t mask[2]) {
  uint32_t num;
...
void sp4_sort_arr(void) {
  qsort(acl4_rules_in, nb_acl4_rules_in, sizeof(struct acl4_rules), sp_cmp);
  qsort(acl4_rules_out, nb_acl4_rules_out, sizeof(struct acl4_rules), sp_cmp);
}
