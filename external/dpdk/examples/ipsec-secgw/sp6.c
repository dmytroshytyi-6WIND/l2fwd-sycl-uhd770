...
#define IPV6_DST_FROM_SP(addr, acr)                                            \
  do {                                                                         \
    (addr).ip.ip6.ip6[0] =                                                     \
        rte_cpu_to_be_64(IPV6_FROM_SP((acr), IP6_DST1, IP6_DST0));             \
    (addr).ip.ip6.ip6[1] =                                                     \
        rte_cpu_to_be_64(IPV6_FROM_SP((acr), IP6_DST3, IP6_DST2));             \
  } while (0)

#define IPV6_SRC_FROM_SP(addr, acr)                                            \
  do {                                                                         \
    (addr).ip.ip6.ip6[0] =                                                     \
        rte_cpu_to_be_64(IPV6_FROM_SP((acr), IP6_SRC1, IP6_SRC0));             \
    (addr).ip.ip6.ip6[1] =                                                     \
        rte_cpu_to_be_64(IPV6_FROM_SP((acr), IP6_SRC3, IP6_SRC2));             \
  } while (0)
...
static int extend_sp_arr(struct acl6_rules **sp_tbl, uint32_t cur_cnt,
                         uint32_t *cur_sz) {
  if (*sp_tbl == NULL) {
  ...
  if (cur_cnt >= *cur_sz) {
    *sp_tbl = realloc(*sp_tbl, *cur_sz * sizeof(struct acl6_rules) * 2);
    if (*sp_tbl == NULL)
      return -1;
    /* clean reallocated extra space */
    memset(&(*sp_tbl)[*cur_sz], 0, *cur_sz * sizeof(struct acl6_rules));
    *cur_sz *= 2;
  }
...
void parse_sp6_tokens(char **tokens, uint32_t n_tokens,
  ...
  if (strcmp(tokens[1], "in") == 0) {
    ...
    if (extend_sp_arr(&acl6_rules_in, nb_acl6_rules_in, &sp_in_sz) < 0)
      return;
    ...
    if (extend_sp_arr(&acl6_rules_out, nb_acl6_rules_out, &sp_out_sz) < 0)
      return;
  ...
  for (ti = 2; ti < n_tokens; ti++) {
    ...
    if (strcmp(tokens[ti], "protect") == 0) {
      ...
      if (status->status < 0)
        return;
      APP_CHECK(bypass_p == 0, status,
                "conflict item "
                "between \"%s\" and \"%s\"",
                tokens[ti], "bypass");
      if (status->status < 0)
        return;
      APP_CHECK(discard_p == 0, status,
                "conflict item "
                "between \"%s\" and \"%s\"",
                tokens[ti], "discard");
      if (status->status < 0)
      ...
      tv = atoi(tokens[ti]);
      APP_CHECK(tv != DISCARD && tv != BYPASS, status, "invalid SPI: %s",
                tokens[ti]);
      if (status->status < 0)
        return;
    ...
    if (strcmp(tokens[ti], "src") == 0) {
      ...
      APP_CHECK(parse_ipv6_addr(tokens[ti], &ip, &depth) == 0, status,
                "unrecognized "
      ...
      rule_ipv6->field[1].value.u32 =
          (uint32_t)ip.s6_addr[0] << 24 | (uint32_t)ip.s6_addr[1] << 16 |
          (uint32_t)ip.s6_addr[2] << 8 | (uint32_t)ip.s6_addr[3];
      rule_ipv6->field[1].mask_range.u32 = (depth > 32) ? 32 : depth;
      depth = (depth > 32) ? (depth - 32) : 0;
      rule_ipv6->field[2].value.u32 =
          (uint32_t)ip.s6_addr[4] << 24 | (uint32_t)ip.s6_addr[5] << 16 |
          (uint32_t)ip.s6_addr[6] << 8 | (uint32_t)ip.s6_addr[7];
      rule_ipv6->field[2].mask_range.u32 = (depth > 32) ? 32 : depth;
      depth = (depth > 32) ? (depth - 32) : 0;
      rule_ipv6->field[3].value.u32 =
          (uint32_t)ip.s6_addr[8] << 24 | (uint32_t)ip.s6_addr[9] << 16 |
          (uint32_t)ip.s6_addr[10] << 8 | (uint32_t)ip.s6_addr[11];
      rule_ipv6->field[3].mask_range.u32 = (depth > 32) ? 32 : depth;
      depth = (depth > 32) ? (depth - 32) : 0;
      rule_ipv6->field[4].value.u32 =
          (uint32_t)ip.s6_addr[12] << 24 | (uint32_t)ip.s6_addr[13] << 16 |
          (uint32_t)ip.s6_addr[14] << 8 | (uint32_t)ip.s6_addr[15];
      rule_ipv6->field[4].mask_range.u32 = (depth > 32) ? 32 : depth;
    ...
    if (strcmp(tokens[ti], "dst") == 0) {
      ...
      rule_ipv6->field[5].value.u32 =
          (uint32_t)ip.s6_addr[0] << 24 | (uint32_t)ip.s6_addr[1] << 16 |
          (uint32_t)ip.s6_addr[2] << 8 | (uint32_t)ip.s6_addr[3];
      rule_ipv6->field[5].mask_range.u32 = (depth > 32) ? 32 : depth;
      depth = (depth > 32) ? (depth - 32) : 0;
      rule_ipv6->field[6].value.u32 =
          (uint32_t)ip.s6_addr[4] << 24 | (uint32_t)ip.s6_addr[5] << 16 |
          (uint32_t)ip.s6_addr[6] << 8 | (uint32_t)ip.s6_addr[7];
      rule_ipv6->field[6].mask_range.u32 = (depth > 32) ? 32 : depth;
      depth = (depth > 32) ? (depth - 32) : 0;
      rule_ipv6->field[7].value.u32 =
          (uint32_t)ip.s6_addr[8] << 24 | (uint32_t)ip.s6_addr[9] << 16 |
          (uint32_t)ip.s6_addr[10] << 8 | (uint32_t)ip.s6_addr[11];
      rule_ipv6->field[7].mask_range.u32 = (depth > 32) ? 32 : depth;
      depth = (depth > 32) ? (depth - 32) : 0;
      rule_ipv6->field[8].value.u32 =
          (uint32_t)ip.s6_addr[12] << 24 | (uint32_t)ip.s6_addr[13] << 16 |
          (uint32_t)ip.s6_addr[14] << 8 | (uint32_t)ip.s6_addr[15];
      rule_ipv6->field[8].mask_range.u32 = (depth > 32) ? 32 : depth;
    ...
    if (strcmp(tokens[ti], "proto") == 0) {
      ...
      APP_CHECK(parse_range(tokens[ti], &low, &high) == 0, status,
                "unrecognized input \"%s\""
                ", expect \"from:to\"",
                tokens[ti]);
      if (status->status < 0)
        return;
      APP_CHECK(low <= 0xff, status,
                "proto low "
                "over-limit");
      if (status->status < 0)
        return;
      APP_CHECK(high <= 0xff, status,
                "proto high "
                "over-limit");
      if (status->status < 0)
        return;
    ...
    if (strcmp(tokens[ti], "sport") == 0) {
      ...
      APP_CHECK(parse_range(tokens[ti], &port_low, &port_high) == 0, status,
                "unrecognized "
  ...
  APP_CHECK(protect_p | bypass_p | discard_p, status,
            "missing "
...
static inline void print_one_ip6_rule(const struct acl6_rules *rule,
                                      int32_t extra) {
  uint8_t a, b, c, d;
  ...
  uint32_t_to_char(rule->field[IP6_DST0].value.u32, &a, &b, &c, &d);
  printf("%.2x%.2x:%.2x%.2x", a, b, c, d);
  uint32_t_to_char(rule->field[IP6_DST1].value.u32, &a, &b, &c, &d);
  printf(":%.2x%.2x:%.2x%.2x", a, b, c, d);
  uint32_t_to_char(rule->field[IP6_DST2].value.u32, &a, &b, &c, &d);
  printf(":%.2x%.2x:%.2x%.2x", a, b, c, d);
  ...
  printf("%hu : %hu %hu : %hu 0x%hhx/0x%hhx ", rule->field[IP6_SRCP].value.u16,
         rule->field[IP6_SRCP].mask_range.u16, rule->field[IP6_DSTP].value.u16,
         rule->field[IP6_DSTP].mask_range.u16, rule->field[IP6_PROTO].value.u8,
         rule->field[IP6_PROTO].mask_range.u8);
  if (extra)
    printf("0x%x-0x%x-0x%x ", rule->data.category_mask, rule->data.priority,
           rule->data.userdata);
}

static inline void dump_ip6_rules(const struct acl6_rules *rule, int32_t num,
                                  int32_t extra) {
  int32_t i;
...
static struct rte_acl_ctx *acl6_init(const char *name, int32_t socketid,
                                     const struct acl6_rules *rules,
                                     uint32_t rules_nb) {
  char s[PATH_MAX];
  ...
  if (rte_acl_add_rules(ctx, (const struct rte_acl_rule *)rules, rules_nb) < 0)
 ...
 */
static int check_spi_value(struct sa_ctx *sa_ctx, int inbound) {
  uint32_t i, num, spi;
  int32_t spi_idx;
...
void sp6_init(struct socket_ctx *ctx, int32_t socket_id) {
  ...
  if (check_spi_value(ctx->sa_in, 1) < 0)
    rte_exit(EXIT_FAILURE, "Inbound IPv6 SP DB has unmatched in SAD SPIs\n");

  if (check_spi_value(ctx->sa_out, 0) < 0)
    rte_exit(EXIT_FAILURE, "Outbound IPv6 SP DB has unmatched in SAD SPIs\n");

  if (nb_acl6_rules_in > 0) {
    name = "sp_ip6_in";
    ctx->sp_ip6_in = (struct sp_ctx *)acl6_init(name, socket_id, acl6_rules_in,
                                                nb_acl6_rules_in);
  } else
  ...
  if (nb_acl6_rules_out > 0) {
    name = "sp_ip6_out";
    ctx->sp_ip6_out = (struct sp_ctx *)acl6_init(
        name, socket_id, acl6_rules_out, nb_acl6_rules_out);
  } else
    RTE_LOG(WARNING, IPSEC,
...
static int sp_cmp(const void *p, const void *q) {
 ...
 */
int sp6_spi_present(uint32_t spi, int inbound, struct ip_addr ip_addr[2],
                    uint32_t mask[2]) {
  uint32_t num;
...
void sp6_sort_arr(void) {
  qsort(acl6_rules_in, nb_acl6_rules_in, sizeof(struct acl6_rules), sp_cmp);
  qsort(acl6_rules_out, nb_acl6_rules_out, sizeof(struct acl6_rules), sp_cmp);
}
