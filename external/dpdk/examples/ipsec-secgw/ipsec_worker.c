...
static inline enum pkt_type process_ipsec_get_pkt_type(struct rte_mbuf *pkt,
  ...
  if (RTE_ETH_IS_IPV4_HDR(ptype)) {
    ...
    *nlp =
        RTE_PTR_ADD(eth, RTE_ETHER_HDR_LEN + offsetof(struct ip6_hdr, ip6_nxt));
    if ((ptype & RTE_PTYPE_TUNNEL_MASK) == RTE_PTYPE_TUNNEL_ESP)
...
static inline int process_ipsec_ev_outbound(struct ipsec_ctx *ctx,
  ...
  rte_security_set_pkt_metadata(sess->security.ctx, sess->security.ses, pkt,
                                NULL);
...
process_ipsec_ev_drv_mode_outbound_vector(struct rte_event_vector *vec,
  ...
  for (i = 0; i < vec->nb_elem; i++) {
    ...
    rte_security_set_pkt_metadata(data[port_id].ctx, data[port_id].sess, pkt,
                                  NULL);
...
ipsec_wrkr_non_burst_int_port_drv_mode(struct eh_event_link_info *links,
  ...
  while (!force_quit) {
    ...
    nb_rx = rte_event_dequeue_burst(links[0].eventdev_id,
                                    links[0].event_port_id, &ev, /* events */
                                    1,                           /* nb_events */
                                    0 /* timeout_ticks */);
