...
static inline void enqueue_cop_bulk(struct cdev_qp *cqp,
  ...
  do {
    ...
    if (len == RTE_DIM(cqp->buf)) {
      n = rte_cryptodev_enqueue_burst(cqp->id, cqp->qp, cqp->buf, len);
      cqp->in_flight += n;
...
static inline int fill_ipsec_session(struct rte_ipsec_session *ss,
 ...
 */
static uint32_t sa_group(void *sa_ptr[], struct rte_mbuf *pkts[],
 ...
 */
static inline void copy_to_trf(struct ipsec_traffic *trf, uint64_t satp,
  ...
  for (j = 0, s = out->num; j != num; j++) {
    out->data[s + j] = rte_pktmbuf_mtod_offset(mb[j], void *, ofs);
    out->pkts[s + j] = mb[j];
  }
...
static uint32_t ipsec_prepare_crypto_group(struct ipsec_ctx *ctx,
                                           struct ipsec_sa *sa,
 ...
 * Should be removed in future.
 */
static inline void prep_process_group(void *sa, struct rte_mbuf *mb[],
                                      uint32_t cnt) {
  uint32_t j;
  struct ipsec_mbuf_metadata *priv;
    ...
    if (priv->sa->mss) {
      ...
      if ((ptype & RTE_PTYPE_L4_MASK) == RTE_PTYPE_L4_TCP) {
        ...
        if ((IS_TUNNEL(priv->sa->flags))) {
          ...
          mb[j]->ol_flags |= RTE_MBUF_F_TX_TUNNEL_ESP;
          if (RTE_ETH_IS_IPV4_HDR(ptype))
            mb[j]->ol_flags |= RTE_MBUF_F_TX_OUTER_IP_CKSUM;
        }
        mb[j]->l4_len = sizeof(struct rte_tcp_hdr);
        mb[j]->ol_flags |= (RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_TCP_CKSUM);
        if (RTE_ETH_IS_IPV4_HDR(ptype))
 ...
 */
static uint32_t ipsec_process_inline_group(struct rte_ipsec_session *ips,
                                           void *sa, struct ipsec_traffic *trf,
                                           struct rte_mbuf *mb[],
                                           uint32_t cnt) {
  uint64_t satp;
  uint32_t k;
 ...
 */
static uint32_t ipsec_process_cpu_group(struct rte_ipsec_session *ips, void *sa,
...
void ipsec_process(struct ipsec_ctx *ctx, struct ipsec_traffic *trf) {
  ...
  for (i = 0; i != n; i++) {
    ...
    if (sa == NULL ||
        (ips->crypto.ses == NULL && fill_ipsec_session(ips, ctx, sa) != 0))
      k = 0;
    ...
    /* process packets inline */
      ...
      case RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL:
        k = ipsec_prepare_crypto_group(ctx, sa, ips, pg->m, pg->cnt);
        break;
      case RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO:
      case RTE_SECURITY_ACTION_TYPE_INLINE_PROTOCOL:
        k = ipsec_process_inline_group(ips, sa, trf, pg->m, pg->cnt);
        break;
      case RTE_SECURITY_ACTION_TYPE_CPU_CRYPTO:
        k = ipsec_process_cpu_group(ips, sa, trf, pg->m, pg->cnt);
        break;
      default:
        k = 0;
      }
...
static inline uint32_t cqp_dequeue(struct cdev_qp *cqp,
                                   struct rte_crypto_op *cop[], uint32_t num) {
  uint32_t n;
...
static inline uint32_t ctx_dequeue(struct ipsec_ctx *ctx,
                                   struct rte_crypto_op *cop[], uint32_t num) {
  uint32_t i, n;
...
void ipsec_cqp_process(struct ipsec_ctx *ctx, struct ipsec_traffic *trf) {
  ...
  ng = rte_ipsec_pkt_crypto_group((const struct rte_crypto_op **)(uintptr_t)cop,
                                  out->pkts, grp, n);
