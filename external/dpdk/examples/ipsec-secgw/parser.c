...
static int parse_tokenize_string(char *string, char *tokens[],
  ...
  if ((i == *n_tokens) && (NULL != strtok_r(string, PARSE_DELIMITER, &string)))
    return -E2BIG;
...
int parse_ipv4_addr(const char *token, struct in_addr *ipv4, uint32_t *mask) {
  ...
  if (pch != NULL) {
    strlcpy(ip_str, token,
            RTE_MIN((unsigned int long)(pch - token + 1), sizeof(ip_str)));
    pch += 1;
    if (is_str_num(pch) != 0)
      return -EINVAL;
    if (mask)
 ...
 */
static const char *parse_uint8x16(const char *s, uint8_t *v, uint8_t ls) {
  char *end;
  unsigned long t;
...
static void cfg_sp_add_cfg_item_parsed(void *parsed_result,
  ...
  APP_CHECK(
      (parse_tokenize_string(params->multi_string, tokens, &n_tokens) == 0),
...
static cmdline_parse_token_string_t cfg_sp_add_sp_str =
    TOKEN_STRING_INITIALIZER(struct cfg_sp_add_cfg_item, sp_keyword, "sp");
...
static void cfg_sa_add_cfg_item_parsed(void *parsed_result,
                                       __rte_unused struct cmdline *cl,
                                       void *data) {
  ...
  APP_CHECK(parse_tokenize_string(params->multi_string, tokens, &n_tokens) == 0,
            status, "too many arguments\n");
...
static cmdline_parse_token_string_t cfg_sa_add_sa_str =
    TOKEN_STRING_INITIALIZER(struct cfg_sa_add_cfg_item, sa_keyword, "sa");
...
static void cfg_rt_add_cfg_item_parsed(void *parsed_result,
                                       __rte_unused struct cmdline *cl,
                                       void *data) {
...
static cmdline_parse_token_string_t cfg_rt_add_rt_str =
    TOKEN_STRING_INITIALIZER(struct cfg_rt_add_cfg_item, rt_keyword, "rt");
...
static void cfg_flow_add_cfg_item_parsed(void *parsed_result,
                                         __rte_unused struct cmdline *cl,
                                         void *data) {
...
static cmdline_parse_token_string_t cfg_flow_add_flow_str =
    TOKEN_STRING_INITIALIZER(struct cfg_flow_add_cfg_item, flow_keyword,
                             "flow");
...
static void cfg_parse_neigh(void *parsed_result,
...
int parse_cfg_file(const char *cfg_filename) {
  ...
  do {
    ...
    if (pos != NULL) {
      ...
      *pos = '\0';
      ...
      if (strlen(oneline) + strlen(str) > 1022) {
        rte_panic("%s:%u: error: "
                  "the concatenated line contains more characters the parser "
                  "can handle\n",
                  cfg_filename, line_num);
        goto error_exit;
      }
    ...
    if (status.status < 0) {
      rte_panic("%s:%u: error: %s", cfg_filename, line_num, status.parse_msg);
      goto error_exit;
    }
