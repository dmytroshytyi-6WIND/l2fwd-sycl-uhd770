...
int ipsec_sad_create(const char *name, struct ipsec_sad *sad, int socket_id,
...
static inline int cmp_sa_key(struct ipsec_sa *sa, int is_v4,
                             struct rte_ipv4_hdr *ipv4,
                             struct rte_ipv6_hdr *ipv6) {
  int sa_type = WITHOUT_TRANSPORT_VERSION(sa->flags);
  if ((sa_type == TRANSPORT) ||
      /* IPv4 check */
      (is_v4 && (sa_type == IP4_TUNNEL) && (sa->src.ip.ip4 == ipv4->src_addr) &&
...
static inline void sa_cache_update(struct ipsec_sa **sa_cache,
                                   struct ipsec_sa *sa, uint32_t mask) {
  uint32_t cache_idx;
...
static inline void sad_lookup(struct ipsec_sad *sad, struct rte_mbuf *pkts[],
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if ((pkts[i]->packet_type & (RTE_PTYPE_TUNNEL_MASK | RTE_PTYPE_L4_MASK)) ==
    ...
    if (is_ipv4) {
      ...
      keys_v4[nb_v4] = (const union rte_ipsec_sad_key *)&v4[nb_v4];
      v4_idxes[nb_v4++] = i;
    } else {
      ...
      memcpy(v6[nb_v6].dip, ipv6->dst_addr, sizeof(ipv6->dst_addr));
      memcpy(v6[nb_v6].sip, ipv6->src_addr, sizeof(ipv6->src_addr));
      keys_v6[nb_v6] = (const union rte_ipsec_sad_key *)&v6[nb_v6];
      v6_idxes[nb_v6++] = i;
    }
  }
  ...
  for (i = 0; i < nb_v4; i++) {
    ipv4 = rte_pktmbuf_mtod(pkts[v4_idxes[i]], struct rte_ipv4_hdr *);
    if ((v4_res[i] != NULL) && (cmp_sa_key(v4_res[i], 1, ipv4, NULL))) {
      sa[v4_idxes[i]] = v4_res[i];
      sa_cache_update(cache->v4, (struct ipsec_sa *)v4_res[i], cache->mask);
    } else
      sa[v4_idxes[i]] = NULL;
  }
  for (i = 0; i < nb_v6; i++) {
    ipv6 = rte_pktmbuf_mtod(pkts[v6_idxes[i]], struct rte_ipv6_hdr *);
    if ((v6_res[i] != NULL) && (cmp_sa_key(v6_res[i], 0, NULL, ipv6))) {
      sa[v6_idxes[i]] = v6_res[i];
      sa_cache_update(cache->v6, (struct ipsec_sa *)v6_res[i], cache->mask);
    } else
      sa[v6_idxes[i]] = NULL;
  }
}
