...
int esp_inbound(struct rte_mbuf *m, struct ipsec_sa *sa,
  ...
  RTE_ASSERT(sa != NULL);
  if (ipsec_get_action_type(sa) == RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO)
    return 0;
  ...
  if (sa->aead_algo == RTE_CRYPTO_AEAD_AES_GCM) {
    ...
    sym_cop->aead.aad.phys_addr =
        rte_pktmbuf_iova_offset(m, aad - rte_pktmbuf_mtod(m, uint8_t *));

    sym_cop->aead.digest.data = rte_pktmbuf_mtod_offset(
        m, void *, rte_pktmbuf_pkt_len(m) - sa->digest_len);
    sym_cop->aead.digest.phys_addr =
        rte_pktmbuf_iova_offset(m, rte_pktmbuf_pkt_len(m) - sa->digest_len);
  } else {
    ...
    case RTE_CRYPTO_AUTH_SHA256_HMAC:
      ...
      sym_cop->auth.data.length =
          sizeof(struct rte_esp_hdr) + sa->iv_len + payload_len;
      break;
    default:
    ...
    sym_cop->auth.digest.data = rte_pktmbuf_mtod_offset(
        m, void *, rte_pktmbuf_pkt_len(m) - sa->digest_len);
...
int esp_inbound_post(struct rte_mbuf *m, struct ipsec_sa *sa,
  ...
  if (ips->type == RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO &&
    ...
    nexthdr = rte_pktmbuf_mtod_offset(
        m, uint8_t *, rte_pktmbuf_pkt_len(m) - sa->digest_len - 1);
    pad_len = nexthdr - 1;
    ...
    if (rte_pktmbuf_trim(m, *pad_len + 2 + sa->digest_len)) {
      RTE_LOG(ERR, IPSEC_ESP, "failed to remove pad_len + digest\n");
      return -EINVAL;
    }
  }
  ...
  if (unlikely(IS_TRANSPORT(sa->flags))) {
    ...
    if (likely(ip->ip_v == IPVERSION)) {
      ...
      ip6->ip6_nxt = *nexthdr;
      ip6->ip6_plen = htons(rte_pktmbuf_data_len(m) - sizeof(struct ip6_hdr));
    }
  } else
...
int esp_outbound(struct rte_mbuf *m, struct ipsec_sa *sa,
  ...
  pad_payload_len =
      RTE_ALIGN_CEIL(rte_pktmbuf_pkt_len(m) - ip_hdr_len + 2, sa->block_size);
  ...
  case IP4_TUNNEL:
    ip4 = ip4ip_outbound(m, sizeof(struct rte_esp_hdr) + sa->iv_len, &sa->src,
                         &sa->dst);
    esp = (struct rte_esp_hdr *)(ip4 + 1);
    break;
  case IP6_TUNNEL:
    ip6 = ip6ip_outbound(m, sizeof(struct rte_esp_hdr) + sa->iv_len, &sa->src,
                         &sa->dst);
    esp = (struct rte_esp_hdr *)(ip6 + 1);
    break;
  case TRANSPORT:
    new_ip = (uint8_t *)rte_pktmbuf_prepend(m, sizeof(struct rte_esp_hdr) +
                                                   sa->iv_len);
  ...
  if (ips->type == RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO) {
    if (ips->security.ol_flags & RTE_SECURITY_TX_HW_TRAILER_OFFLOAD) {
  ...
  if (sa->aead_algo == RTE_CRYPTO_AEAD_AES_GCM) {
    ...
    sym_cop->aead.digest.data = rte_pktmbuf_mtod_offset(
        m, uint8_t *, rte_pktmbuf_pkt_len(m) - sa->digest_len);
    sym_cop->aead.digest.phys_addr =
        rte_pktmbuf_iova_offset(m, rte_pktmbuf_pkt_len(m) - sa->digest_len);
  } else {
    switch (sa->cipher_algo) {
    ...
    case RTE_CRYPTO_AUTH_SHA256_HMAC:
      sym_cop->auth.data.offset = ip_hdr_len;
      sym_cop->auth.data.length =
          sizeof(struct rte_esp_hdr) + sa->iv_len + pad_payload_len;
      break;
    default:
      RTE_LOG(ERR, IPSEC_ESP, "unsupported auth algorithm %u\n", sa->auth_algo);
      return -EINVAL;
