...
static inline void set_ipsec_conf(struct ipsec_sa *sa,
                                  struct rte_security_ipsec_xform *ipsec) {
  if (ipsec->mode == RTE_SECURITY_IPSEC_SA_MODE_TUNNEL) {
    struct rte_security_ipsec_tunnel_param *tunnel = &ipsec->tunnel;
    if (IS_IP4_TUNNEL(sa->flags)) {
      tunnel->type = RTE_SECURITY_IPSEC_TUNNEL_IPV4;
      tunnel->ipv4.ttl = IPDEFTTL;

      memcpy((uint8_t *)&tunnel->ipv4.src_ip, (uint8_t *)&sa->src.ip.ip4, 4);

      memcpy((uint8_t *)&tunnel->ipv4.dst_ip, (uint8_t *)&sa->dst.ip.ip4, 4);
...
int create_lookaside_session(struct ipsec_ctx *ipsec_ctx, struct ipsec_sa *sa,
  ...
  ret = rte_hash_lookup_data(ipsec_ctx->cdev_map, &key, (void **)&cdev_id_qp);
  if (ret < 0) {
    RTE_LOG(ERR, IPSEC,
            "No cryptodev: core %u, cipher_algo %u, "
            "auth_algo %u, aead_algo %u\n",
            key.lcore_id, key.cipher_algo, key.auth_algo, key.aead_algo);
    return -1;
  }
  ...
  if (ips->type != RTE_SECURITY_ACTION_TYPE_NONE &&
    ...
    if (ips->type == RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL) {
      ...
      ips->security.ses =
          rte_security_session_create(ctx, &sess_conf, ipsec_ctx->session_pool,
    ...
    ips->crypto.ses = rte_cryptodev_sym_session_create(ipsec_ctx->session_pool);
...
int create_inline_session(struct socket_ctx *skt_ctx, struct ipsec_sa *sa,
                          struct rte_ipsec_session *ips) {
  int32_t ret = 0;
  ...
  if (IS_TRANSPORT(sa->flags)) {
    ...
    if (IS_IP4(sa->flags)) {
      ...
      sess_conf.ipsec.tunnel.ipv4.src_ip.s_addr = sa->src.ip.ip4;
      sess_conf.ipsec.tunnel.ipv4.dst_ip.s_addr = sa->dst.ip.ip4;
    } else if (IS_IP6(sa->flags)) {
      sess_conf.ipsec.tunnel.type = RTE_SECURITY_IPSEC_TUNNEL_IPV6;

      memcpy(sess_conf.ipsec.tunnel.ipv6.src_addr.s6_addr, sa->src.ip.ip6.ip6_b,
             16);
      memcpy(sess_conf.ipsec.tunnel.ipv6.dst_addr.s6_addr, sa->dst.ip.ip6.ip6_b,
             16);
    }
  ...
  if (ips->type == RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO) {
    ...
    sec_ctx = (struct rte_security_ctx *)rte_eth_dev_get_sec_ctx(sa->portid);
    if (sec_ctx == NULL) {
      RTE_LOG(ERR, IPSEC, " rte_eth_dev_get_sec_ctx failed\n");
      return -1;
    }

    ips->security.ses = rte_security_session_create(
        sec_ctx, &sess_conf, skt_ctx->session_pool, skt_ctx->session_priv_pool);
    if (ips->security.ses == NULL) {
      RTE_LOG(ERR, IPSEC, "SEC Session init failed: err: %d\n", ret);
      return -1;
    }
    ...
    while (sec_cap->action != RTE_SECURITY_ACTION_TYPE_NONE) {
      if (sec_cap->action == ips->type &&
          ...
          sec_cap->ipsec.mode == RTE_SECURITY_IPSEC_SA_MODE_TUNNEL &&
    ...
    if (sec_cap->action == RTE_SECURITY_ACTION_TYPE_NONE) {
      RTE_LOG(ERR, IPSEC, "No suitable security capability found\n");
      return -1;
    }
    ...
    if (IS_IP6(sa->flags)) {
      ...
      memcpy(sa->ipv6_spec.hdr.dst_addr, sa->dst.ip.ip6.ip6_b, 16);
      memcpy(sa->ipv6_spec.hdr.src_addr, sa->src.ip.ip6.ip6_b, 16);
    ...
    sa->attr.egress = (sa->direction == RTE_SECURITY_IPSEC_SA_DIR_EGRESS);
    sa->attr.ingress = (sa->direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS);
    if (sa->attr.ingress) {
      uint8_t rss_key[64];
      ...
      ret = rte_eth_dev_rss_hash_conf_get(sa->portid, &rss_conf);
      if (ret != 0) {
        RTE_LOG(ERR, IPSEC, "rte_eth_dev_rss_hash_conf_get:ret=%d\n", ret);
        return -1;
      }
      ...
      sa->action[1].conf = &(struct rte_flow_action_queue){
          .index = 0,
      };
      ret = rte_flow_validate(sa->portid, &sa->attr, sa->pattern, sa->action,
                              &err);
      ...
      if (ret)
        goto flow_create_failure;
               ...
               (ips->security.ol_flags & RTE_SECURITY_TX_HW_TRAILER_OFFLOAD)) {
      sa->action[1].type = RTE_FLOW_ACTION_TYPE_PASSTHRU;
      sa->action[2].type = RTE_FLOW_ACTION_TYPE_END;
    }
  flow_create:
    sa->flow =
        rte_flow_create(sa->portid, &sa->attr, sa->pattern, sa->action, &err);
    if (sa->flow == NULL) {
    flow_create_failure:
      RTE_LOG(ERR, IPSEC, "Failed to create ipsec flow msg: %s\n", err.message);
      return -1;
    }
    ...
    sec_cap = rte_security_capabilities_get(sec_ctx);
    if (sec_cap == NULL) {
      RTE_LOG(ERR, IPSEC, "No capabilities registered\n");
      return -1;
    }
...
int create_ipsec_esp_flow(struct ipsec_sa *sa) {
  int ret = 0;
  struct rte_flow_error err;
  if (sa->direction == RTE_SECURITY_IPSEC_SA_DIR_EGRESS) {
    RTE_LOG(ERR, IPSEC, "No Flow director rule for Egress traffic\n");
    return -1;
  }
  if (sa->flags == TRANSPORT) {
    RTE_LOG(ERR, IPSEC, "No Flow director rule for transport mode\n");
    return -1;
  }
  ...
  if (IS_IP6(sa->flags)) {
    ...
    sa->pattern[1].type = RTE_FLOW_ITEM_TYPE_IPV6;
    sa->pattern[1].spec = &sa->ipv6_spec;
    memcpy(sa->ipv6_spec.hdr.dst_addr, sa->dst.ip.ip6.ip6_b,
           sizeof(sa->dst.ip.ip6.ip6_b));
    memcpy(sa->ipv6_spec.hdr.src_addr, sa->src.ip.ip6.ip6_b,
           sizeof(sa->src.ip.ip6.ip6_b));
    sa->pattern[2].type = RTE_FLOW_ITEM_TYPE_ESP;
    sa->pattern[2].spec = &sa->esp_spec;
    sa->pattern[2].mask = &rte_flow_item_esp_mask;
    sa->esp_spec.hdr.spi = rte_cpu_to_be_32(sa->spi);
  ...
  ret = rte_flow_validate(sa->portid, &sa->attr, sa->pattern, sa->action, &err);
  if (ret < 0) {
 ...
 */
void enqueue_cop_burst(struct cdev_qp *cqp) {
  uint32_t i, len, ret;
...
static inline void enqueue_cop(struct cdev_qp *cqp, struct rte_crypto_op *cop) {
  cqp->buf[cqp->len++] = cop;
...
static inline void ipsec_enqueue(ipsec_xform_fn xform_func,
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    case RTE_SECURITY_ACTION_TYPE_NONE:
      ...
      rte_crypto_op_attach_sym_session(&priv->cop, ips->crypto.ses);
    ...
    case RTE_SECURITY_ACTION_TYPE_INLINE_PROTOCOL:
      ...
      if (ips->security.ol_flags & RTE_SECURITY_TX_OLOAD_NEED_MDATA)
        rte_security_set_pkt_metadata(ips->security.ctx, ips->security.ses,
                                      pkts[i], NULL);
      continue;
...
static inline int32_t ipsec_inline_dequeue(ipsec_xform_fn xform_func,
                                           struct ipsec_ctx *ipsec_ctx,
...
static inline int ipsec_dequeue(ipsec_xform_fn xform_func,
                                struct ipsec_ctx *ipsec_ctx,
  ...
  for (i = 0; i < ipsec_ctx->nb_qps && nb_pkts < max_pkts; i++) {
    ...
    nb_cops =
        rte_cryptodev_dequeue_burst(cqp->id, cqp->qp, cops, max_pkts - nb_pkts);
...
uint16_t ipsec_inbound(struct ipsec_ctx *ctx, struct rte_mbuf *pkts[],
...
uint16_t ipsec_inbound_cqp_dequeue(struct ipsec_ctx *ctx,
                                   struct rte_mbuf *pkts[], uint16_t len) {
  return ipsec_dequeue(esp_inbound_post, ctx, pkts, len);
}

uint16_t ipsec_outbound(struct ipsec_ctx *ctx, struct rte_mbuf *pkts[],
...
uint16_t ipsec_outbound_cqp_dequeue(struct ipsec_ctx *ctx,
                                    struct rte_mbuf *pkts[], uint16_t len) {
