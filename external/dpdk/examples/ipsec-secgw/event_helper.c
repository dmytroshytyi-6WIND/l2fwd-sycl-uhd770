...
static int eh_get_enabled_cores(struct rte_bitmap *eth_core_mask) {
  int i, count = 0;
...
static inline bool eh_dev_has_rx_internal_port(uint8_t eventdev_id) {
  bool flag = true;
  int j, ret;
...
static inline bool eh_dev_has_tx_internal_port(uint8_t eventdev_id) {
  bool flag = true;
  int j, ret;
...
static int eh_set_default_conf_eventdev(struct eventmode_conf *em_conf) {
...
static void eh_do_capability_check(struct eventmode_conf *em_conf) {
  struct eventdev_params *eventdev_config;
...
static int eh_set_default_conf_link(struct eventmode_conf *em_conf) {
  struct eventdev_params *eventdev_config;
  ...
  for (i = 0; i < eventdev_config->nb_eventport; i++) {
    ...
    lcore_id = eh_get_next_active_core(em_conf, lcore_id);
...
static int eh_set_default_conf_rx_adapter(struct eventmode_conf *em_conf) {
...
static int eh_set_default_conf_tx_adapter(struct eventmode_conf *em_conf) {
...
static int eh_validate_conf(struct eventmode_conf *em_conf) {
  int ret;
...
static int eh_initialize_eventdev(struct eventmode_conf *em_conf) {
  ...
  for (i = 0; i < nb_eventdev; i++) {
    ...
    memset(&evdev_default_conf, 0, sizeof(struct rte_event_dev_info));

    /* Get default conf of eventdev */
    ret = rte_event_dev_info_get(eventdev_id, &evdev_default_conf);
    if (ret < 0) {
      EH_LOG_ERR("Error in getting event device info[devID:%d]", eventdev_id);
      return ret;
    }

    memset(&eventdev_conf, 0, sizeof(struct rte_event_dev_config));
    eventdev_conf.nb_events_limit = evdev_default_conf.max_num_events;
    eventdev_conf.nb_event_queues = nb_eventqueue;
    eventdev_conf.nb_event_ports = eventdev_config->nb_eventport;
    ...
    for (j = 0; j < nb_eventqueue; j++) {
      ...
      eventq_conf.event_queue_cfg = eventdev_config->ev_queue_mode;
      /*
       ...
       */
      if (!eventdev_config->all_internal_ports && j == nb_eventqueue - 1) {
        eventq_conf.schedule_type = RTE_SCHED_TYPE_ATOMIC;
      } else {
        eventq_conf.schedule_type = em_conf->ext_params.sched_type;
      }
      ...
      /* Setup the queue */
      ret = rte_event_queue_setup(eventdev_id, j, &eventq_conf);
      if (ret < 0) {
        EH_LOG_ERR("Failed to setup event queue %d", ret);
        return ret;
      }
    }
    ...
    for (j = 0; j < eventdev_config->nb_eventport; j++) {
      ...
      if (ret < 0) {
        EH_LOG_ERR("Failed to setup event port %d", ret);
        return ret;
      }
    }
  }
  ...
  for (j = 0; j < em_conf->nb_link; j++) {
    ...
    /* Link queue to port */
    ret = rte_event_port_link(eventdev_id, link->event_port_id, queue, NULL, 1);
    if (ret < 0) {
...
static int eh_start_eventdev(struct eventmode_conf *em_conf) {
  struct eventdev_params *eventdev_config;
  int nb_eventdev = em_conf->nb_eventdev;
  int i, ret;
  ...
  for (i = 0; i < nb_eventdev; i++) {
    ...
    if (ret < 0) {
      EH_LOG_ERR("Failed to start event device %d, %d", i, ret);
      return ret;
    }
  }
  return 0;
...
static int eh_event_vector_limits_validate(struct eventmode_conf *em_conf,
                                           uint8_t ev_dev_id,
                                           uint8_t ethdev_id) {
  ...
  ret =
      rte_event_eth_rx_adapter_vector_limits_get(ev_dev_id, ethdev_id, &limits);
  if (ret) {
  ...
  if (em_conf->vector_tmo_ns > limits.max_timeout_ns ||
    ...
    EH_LOG_ERR("Vector timeout [%" PRIu64 "] not within limits max[%" PRIu64
               "] min[%" PRIu64 "]",
               em_conf->vector_tmo_ns, limits.max_timeout_ns,
...
static int eh_rx_adapter_configure(struct eventmode_conf *em_conf,
                                   struct rx_adapter_conf *adapter) {
  ...
  if (em_conf->ext_params.event_vector) {
    ...
    if (em_conf->vector_pool_sz) {
      ...
      nb_elem = (nb_bufs_in_pool / em_conf->ext_params.vector_size) + 1;
      if (per_port_pool)
        nb_elem = nb_ports * nb_elem;
    }

    vector_pool = rte_event_vector_pool_create(
        "vector_pool", nb_elem, 0, em_conf->ext_params.vector_size, socket_id);
    if (vector_pool == NULL) {
  ...
  port_conf.dequeue_depth = evdev_default_conf.max_event_port_dequeue_depth;
  port_conf.enqueue_depth = evdev_default_conf.max_event_port_enqueue_depth;
  ...
  for (j = 0; j < adapter->nb_connections; j++) {
    ...
    if (em_conf->ext_params.event_vector) {
      ret = eh_event_vector_limits_validate(em_conf, eventdev_id,
                                            conn->ethdev_id);
      if (ret)
        return ret;
    ...
    ret = rte_event_eth_rx_adapter_queue_add(
        adapter->adapter_id, conn->ethdev_id, conn->ethdev_rx_qid, &queue_conf);
    if (ret < 0) {
      EH_LOG_ERR("Failed to add eth queue to rx adapter %d", ret);
      return ret;
    }
  }
  ...
  ret =
      rte_event_eth_rx_adapter_service_id_get(adapter->adapter_id, &service_id);
  if (ret != -ESRCH && ret < 0) {
    EH_LOG_ERR("Failed to get service id used by rx adapter %d", ret);
    return ret;
  }
...
static int eh_initialize_rx_adapter(struct eventmode_conf *em_conf) {
  struct rx_adapter_conf *adapter;
  int i, ret;
...
static int32_t eh_start_worker_eth_core(struct eventmode_conf *conf,
                                        uint32_t lcore_id) {
  ...
  for (i = 0; i < conf->nb_rx_adapter; i++) {
    ...
    if (service_count > EVENT_MODE_MAX_ADAPTERS_PER_RX_CORE) {
      EH_LOG_ERR("Exceeded the max allowed adapters per rx core");
      break;
    }
    ...
    if (ret != -ESRCH && ret < 0) {
      EH_LOG_ERR("Failed to get service id used by rx adapter");
      return ret;
    }
  ...
  for (i = 0; i < conf->nb_tx_adapter; i++) {
    ...
    if (service_count > EVENT_MODE_MAX_ADAPTERS_PER_TX_CORE) {
      EH_LOG_ERR("Exceeded the max allowed adapters per tx core");
      break;
    }
    ...
    if (ret != -ESRCH && ret < 0) {
      EH_LOG_ERR("Failed to get service id used by tx adapter");
      return ret;
    }
...
static int32_t eh_stop_worker_eth_core(void) {
  if (eth_core_running) {
...
static int eh_verify_match_worker(struct eh_app_worker_params *match_wrkr) {
...
static uint8_t eh_get_event_lcore_links(uint32_t lcore_id, struct eh_conf *conf,
...
static int eh_tx_adapter_configure(struct eventmode_conf *em_conf,
  ...
  /* Setup port conf */
  port_conf.new_event_threshold = evdev_default_conf.max_num_events;
  port_conf.dequeue_depth = evdev_default_conf.max_event_port_dequeue_depth;
  port_conf.enqueue_depth = evdev_default_conf.max_event_port_enqueue_depth;
  ...
  for (j = 0; j < adapter->nb_connections; j++) {
    ...
    ret = rte_event_eth_tx_adapter_queue_add(
        adapter->adapter_id, conn->ethdev_id, conn->ethdev_tx_qid);
    if (ret < 0) {
      EH_LOG_ERR("Failed to add eth queue to tx adapter %d", ret);
      return ret;
    }
  }
  ...
  ret =
      rte_event_eth_tx_adapter_event_port_get(adapter->adapter_id, &tx_port_id);
  if (ret) {
   ...
   */
  for (j = 0; j < eventdev_config->nb_eventport; j++) {
    rte_event_port_unlink(eventdev_id, j, &(adapter->tx_ev_queue), 1);
  }

  /* Link Tx event queue to Tx port */
  ret = rte_event_port_link(eventdev_id, tx_port_id, &(adapter->tx_ev_queue),
                            NULL, 1);
  if (ret != 1) {
  ...
  ret =
      rte_event_eth_tx_adapter_service_id_get(adapter->adapter_id, &service_id);
  if (ret != -ESRCH && ret < 0) {
    EH_LOG_ERR("Failed to get service id used by tx adapter %d", ret);
    return ret;
  }
...
static int eh_initialize_tx_adapter(struct eventmode_conf *em_conf) {
  struct tx_adapter_conf *adapter;
  int i, ret;
...
static void eh_display_operating_mode(struct eventmode_conf *em_conf) {
  char sched_types[][32] = {
...
static void eh_display_event_dev_conf(struct eventmode_conf *em_conf) {
  char queue_mode[][32] = {
      "",
      "ATQ (ALL TYPE QUEUE)",
      "SINGLE LINK",
  ...
  for (i = 0; i < em_conf->nb_eventdev; i++) {
    sprintf(print_buf, "\tDev ID: %-2d \tQueues: %-2d \tPorts: %-2d",
            em_conf->eventdev_config[i].eventdev_id,
            em_conf->eventdev_config[i].nb_eventqueue,
            em_conf->eventdev_config[i].nb_eventport);
    sprintf(print_buf + strlen(print_buf), "\tQueue mode: %s",
...
static void eh_display_rx_adapter_conf(struct eventmode_conf *em_conf) {
  ...
  for (i = 0; i < nb_rx_adapter; i++) {
    ...
    sprintf(print_buf,
            ...
            adapter->adapter_id, adapter->nb_connections, adapter->eventdev_id);
    if (adapter->rx_core_id == (uint32_t)-1)
      sprintf(print_buf + strlen(print_buf), "\tRx core: %-2s",
              "[INTERNAL PORT]");
    else if (adapter->rx_core_id == RTE_MAX_LCORE)
      sprintf(print_buf + strlen(print_buf), "\tRx core: %-2s", "[NONE]");
    else
      sprintf(print_buf + strlen(print_buf), "\tRx core: %-2d",
              adapter->rx_core_id);
    ...
    for (j = 0; j < adapter->nb_connections; j++) {
      ...
      sprintf(print_buf, "\t\tEthdev ID: %-2d", conn->ethdev_id);

      if (conn->ethdev_rx_qid == -1)
        sprintf(print_buf + strlen(print_buf), "\tEth rx queue: %-2s", "ALL");
      else
        sprintf(print_buf + strlen(print_buf), "\tEth rx queue: %-2d",
                conn->ethdev_rx_qid);

      sprintf(print_buf + strlen(print_buf), "\tEvent queue: %-2d",
              conn->eventq_id);
      EH_LOG_INFO("%s", print_buf);
    }
  }
...
static void eh_display_tx_adapter_conf(struct eventmode_conf *em_conf) {
  ...
  for (i = 0; i < nb_tx_adapter; i++) {
    ...
    sprintf(print_buf,
            ...
            adapter->adapter_id, adapter->nb_connections, adapter->eventdev_id);
    if (adapter->tx_core_id == (uint32_t)-1)
      sprintf(print_buf + strlen(print_buf), "\tTx core: %-2s",
              "[INTERNAL PORT]");
    else if (adapter->tx_core_id == RTE_MAX_LCORE)
      sprintf(print_buf + strlen(print_buf), "\tTx core: %-2s", "[NONE]");
    else
      sprintf(print_buf + strlen(print_buf),
              "\tTx core: %-2d,\tInput event queue: %-2d", adapter->tx_core_id,
              adapter->tx_ev_queue);
    ...
    for (j = 0; j < adapter->nb_connections; j++) {
      ...
      if (conn->ethdev_tx_qid == -1)
        sprintf(print_buf + strlen(print_buf), "\tEth tx queue: %-2s", "ALL");
      else
        sprintf(print_buf + strlen(print_buf), "\tEth tx queue: %-2d",
                conn->ethdev_tx_qid);
      EH_LOG_INFO("%s", print_buf);
    }
  }
...
static void eh_display_link_conf(struct eventmode_conf *em_conf) {
  struct eh_event_link_info *link;
  char print_buf[256] = {0};
  int i;
  ...
  for (i = 0; i < em_conf->nb_link; i++) {
    ...
    sprintf(print_buf, "\tEvent dev ID: %-2d\tEvent port: %-2d",
            link->eventdev_id, link->event_port_id);

    if (em_conf->ext_params.all_ev_queue_to_ev_port)
      sprintf(print_buf + strlen(print_buf), "Event queue: %-2s\t", "ALL");
    else
      sprintf(print_buf + strlen(print_buf), "Event queue: %-2d\t",
              link->eventq_id);

    sprintf(print_buf + strlen(print_buf), "Lcore: %-2d", link->lcore_id);
    EH_LOG_INFO("%s", print_buf);
  }
  EH_LOG_INFO("");
}

struct eh_conf *eh_conf_init(void) {
  struct eventmode_conf *em_conf = NULL;
  ...
  bitmap =
      rte_zmalloc("event-helper-ethcore-bitmap", nb_bytes, RTE_CACHE_LINE_SIZE);
  if (!bitmap) {
    EH_LOG_ERR("Failed to allocate memory for eth cores bitmap\n");
    goto free_em_conf;
  }

  em_conf->eth_core_mask = rte_bitmap_init(RTE_MAX_LCORE, bitmap, nb_bytes);
...
void eh_conf_uninit(struct eh_conf *conf) {
  struct eventmode_conf *em_conf = NULL;
...
void eh_display_conf(struct eh_conf *conf) {
  struct eventmode_conf *em_conf;
...
int32_t eh_devs_init(struct eh_conf *conf) {
  struct eventmode_conf *em_conf;
  uint16_t port_id;
  int ret;
  ...
  RTE_ETH_FOREACH_DEV(port_id) {
    ...
    if (ret != 0) {
      EH_LOG_ERR("Failed to stop port %u, err: %d", port_id, ret);
      return ret;
    }
  }
  ...
  RTE_ETH_FOREACH_DEV(port_id) {
    ...
    if (ret < 0) {
      EH_LOG_ERR("Failed to start eth dev %d, %d", port_id, ret);
      return ret;
    }
  }
...
int32_t eh_devs_uninit(struct eh_conf *conf) {
  struct eventmode_conf *em_conf;
  int ret, i, j;
  uint16_t id;
  ...
  for (i = 0; i < em_conf->nb_rx_adapter; i++) {
    ...
    for (j = 0; j < em_conf->rx_adapter[i].nb_connections; j++) {
      ...
      ret = rte_event_eth_rx_adapter_queue_del(
          id, em_conf->rx_adapter[i].conn[j].ethdev_id, -1);
      if (ret < 0) {
        EH_LOG_ERR("Failed to remove rx adapter queues %d", ret);
        return ret;
      }
    }
  ...
  for (i = 0; i < em_conf->nb_tx_adapter; i++) {
    ...
    for (j = 0; j < em_conf->tx_adapter[i].nb_connections; j++) {
      ...
      ret = rte_event_eth_tx_adapter_queue_del(
          id, em_conf->tx_adapter[i].conn[j].ethdev_id, -1);
      if (ret < 0) {
        EH_LOG_ERR("Failed to remove tx adapter queues %d", ret);
        return ret;
      }
    }
...
void eh_launch_worker(struct eh_conf *conf,
                      struct eh_app_worker_params *app_wrkr,
  ...
  if (match_wrkr == NULL) {
    EH_LOG_ERR("Failed to match worker registered for lcore %d", lcore_id);
    goto clean_and_exit;
  }
