...
static int extend_sa_arr(struct ipsec_sa **sa_tbl, uint32_t cur_cnt,
                         uint32_t *cur_sz) {
  if (*sa_tbl == NULL) {
...
void parse_sa_tokens(char **tokens, uint32_t n_tokens,
  ...
  if (!type_p ||
      (!portid_p && ips->type != RTE_SECURITY_ACTION_TYPE_CPU_CRYPTO)) {
    ips->type = RTE_SECURITY_ACTION_TYPE_NONE;
  }
...
static struct sa_ctx *sa_create(const char *name, int32_t socket_id,
                                uint32_t nb_sa) {
  char s[PATH_MAX];
  ...
  sa_ctx =
      rte_zmalloc(NULL, sizeof(struct sa_ctx) + sizeof(struct ipsec_sa) * nb_sa,
                  RTE_CACHE_LINE_SIZE);
...
static int get_spi_proto(uint32_t spi, enum rte_security_ipsec_sa_direction dir,
  ...
  rc4 = sp4_spi_present(spi, dir == RTE_SECURITY_IPSEC_SA_DIR_INGRESS, ip_addr,
                        mask);
  rc6 = sp6_spi_present(spi, dir == RTE_SECURITY_IPSEC_SA_DIR_INGRESS, ip_addr,
                        mask);
...
static int sa_add_address_inline_crypto(struct ipsec_sa *sa) {
  ...
  if (protocol < 0)
    ...
    if (mask[0] == IP4_FULL_MASK && mask[1] == IP4_FULL_MASK &&
        ip_addr[0].ip.ip4 != 0 && ip_addr[1].ip.ip4 != 0) {
...
static int sa_add_rules(struct sa_ctx *sa_ctx, const struct ipsec_sa entries[],
...
static inline int sa_out_add_rules(struct sa_ctx *sa_ctx,
...
static inline int sa_in_add_rules(struct sa_ctx *sa_ctx,
                                  const struct ipsec_sa entries[],
                                  uint32_t nb_entries,
                                  struct socket_ctx *skt_ctx) {
 ...
 */
static void fill_ipsec_app_sa_prm(struct rte_ipsec_sa_prm *prm,
...
static int fill_ipsec_sa_prm(struct rte_ipsec_sa_prm *prm,
                             const struct ipsec_sa *ss,
                             const struct rte_ipv4_hdr *v4,
...
static int fill_ipsec_session(struct rte_ipsec_session *ss,
 ...
 */
static int ipsec_sa_init(struct ipsec_sa *lsa, struct rte_ipsec_sa *sa,
                         uint32_t sa_size) {
  int rc;
 ...
 * one per session.
 */
static int ipsec_satbl_init(struct sa_ctx *ctx, uint32_t nb_ent,
                            int32_t socket) {
  int32_t rc, sz;
  uint32_t i, idx;
  size_t tsz;
