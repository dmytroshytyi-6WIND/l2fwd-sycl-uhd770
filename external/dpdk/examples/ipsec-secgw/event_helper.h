 ...
 */
enum eh_rx_types { EH_RX_TYPE_NON_BURST = 0, EH_RX_TYPE_BURST };

/**
 * Event mode packet tx types
 */
enum eh_tx_types { EH_TX_TYPE_INTERNAL_PORT = 0, EH_TX_TYPE_NO_INTERNAL_PORT };
...
struct eventmode_conf {
  ...
  uint8_t nb_link;
  /**< No of links */
  struct eh_event_link_info link[EVENT_MODE_MAX_LCORE_LINKS];
  /**< Per link conf */
...
struct eh_app_worker_params {
  ...
  void (*worker_thread)(struct eh_event_link_info *links, uint8_t nb_links);
  /**< Worker thread */
};
