...
static inline uint32_t send_fragment_packet(struct lcore_conf *qconf,
  ...
  if (proto == IPPROTO_IP)
    rc = rte_ipv4_fragment_packet(m, tbl->m_table + len, n, mtu_size, m->pool,
                                  qconf->frag.pool_indir);
  else
    rc = rte_ipv6_fragment_packet(m, tbl->m_table + len, n, mtu_size, m->pool,
                                  qconf->frag.pool_indir);
...
static void pool_init(struct socket_ctx *ctx, int32_t socket_id, int portid,
                      uint32_t nb_mbuf) {
  char s[64];
  int32_t ms;
...
int32_t main(int32_t argc, char **argv) {
  ...
  if (unprotected_port_mask && !nb_sa_in)
    rte_exit(EXIT_FAILURE,
             "Cannot use unprotected portmask without configured SA inbound\n");
  ...
  for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
    ...
    if (per_port_pool) {
      RTE_ETH_FOREACH_DEV(portid) {
        ...
        pool_init(&socket_ctx[socket_id], socket_id, portid, nb_bufs_in_pool);
      }
    } else {
      pool_init(&socket_ctx[socket_id], socket_id, 0, nb_bufs_in_pool);
    }
