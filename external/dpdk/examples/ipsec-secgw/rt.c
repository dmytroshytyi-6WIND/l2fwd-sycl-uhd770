  ...
  for (ti = 1; ti < n_tokens; ti++) {
    if (strcmp(tokens[ti], "dst") == 0) {
      ...
      if (route_ipv4 != NULL) {
        ...
        if (status->status < 0)
          return;
        route_ipv4->ip = rte_bswap32((uint32_t)ip.s_addr);
...
void rt_init(struct socket_ctx *ctx, int32_t socket_id) {
  ...
  for (i = 0; i < nb_rt_ip4; i++) {
    ret = rte_lpm_add(lpm, rt_ip4[i].ip, rt_ip4[i].depth, rt_ip4[i].if_out);
    if (ret < 0)
      rte_exit(EXIT_FAILURE,
    ...
    printf("LPM: Adding route %hhu.%hhu.%hhu.%hhu/%hhu (%hhu)\n", a, b, c, d,
           rt_ip4[i].depth, rt_ip4[i].if_out);
  }
  ...
  for (i = 0; i < nb_rt_ip6; i++) {
    ret = rte_lpm6_add(lpm6, rt_ip6[i].ip, rt_ip6[i].depth, rt_ip6[i].if_out);
    if (ret < 0)
      rte_exit(EXIT_FAILURE,
               "Fail to add entry num %u to %s "
               "LPM table on socket %d\n",
