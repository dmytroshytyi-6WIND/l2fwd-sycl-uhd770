...
static void cmd_quit_parsed(__rte_unused void *parsed_result,
  ...
  RTE_LCORE_FOREACH(lcore_id) { rte_power_exit(lcore_id); }
  cmdline_quit(cl);
}
...
static int query_data(struct rte_power_channel_packet *pkt,
...
static void cmd_query_freq_list_parsed(void *parsed_result,
                                       __rte_unused struct cmdline *cl,
                                       __rte_unused void *data) {
...
cmdline_parse_token_string_t cmd_query_freq_token = TOKEN_STRING_INITIALIZER(
    struct cmd_freq_list_result, query_freq, "query_cpu_freq");
...
static void cmd_query_caps_list_parsed(void *parsed_result,
                                       __rte_unused struct cmdline *cl,
                                       __rte_unused void *data) {
  ...
  if (query_list) {
    ...
    for (i = 0; i < pkt_caps_list.num_vcpu; ++i)
      cmdline_printf(cl,
                     "Capabilities of [%d] vcore are:"
                     " turbo possibility: %" PRId64 ", "
                     "is priority core: %" PRId64 ".\n",
                     i, pkt_caps_list.turbo[i], pkt_caps_list.priority[i]);
  } else {
    cmdline_printf(cl,
                   "Capabilities of [%d] vcore are:"
                   " turbo possibility: %" PRId64 ", "
                   "is priority core: %" PRId64 ".\n",
                   lcore_id, pkt_caps_list.turbo[lcore_id],
                   pkt_caps_list.priority[lcore_id]);
  }
}

cmdline_parse_token_string_t cmd_query_caps_token = TOKEN_STRING_INITIALIZER(
    struct cmd_query_caps_result, query_caps, "query_cpu_caps");
...
static int check_response_cmd(unsigned int lcore_id, int *result) {
  struct rte_power_channel_packet pkt;
  int ret;
...
cmdline_parse_token_num_t cmd_set_cpu_freq_core_num =
    TOKEN_NUM_INITIALIZER(struct cmd_set_cpu_freq_result, lcore_id, RTE_UINT8);
...
static inline int send_policy(struct rte_power_channel_packet *pkt,
                              struct cmdline *cl) {
  int ret;
...
static void cmd_send_policy_parsed(void *parsed_result, struct cmdline *cl,
                                   __rte_unused void *data) {
  int ret = -1;
...
cmdline_parse_token_string_t cmd_send_policy = TOKEN_STRING_INITIALIZER(
    struct cmd_send_policy_result, send_policy, "send_policy");
cmdline_parse_token_string_t cmd_send_policy_cmd_cmd =
    TOKEN_STRING_INITIALIZER(struct cmd_send_policy_result, cmd, "now");
