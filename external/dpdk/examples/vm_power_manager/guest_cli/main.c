...
static int parse_args(int argc, char **argv) {
  ...
  int option_index;
  char *prgname = argv[0];
  const struct option lgopts[] = {{"vm-name", required_argument, 0, 'n'},
  ...
  while ((opt = getopt_long(argc, argvopt, "n:b:q:p:", lgopts,
                            &option_index)) != EOF) {

    switch (opt) {
    /* portmask */
    case 'n':
      strlcpy(policy->vm_name, optarg, RTE_POWER_VM_MAX_NAME_SZ);
    ...
    case 'q':
      ...
      for (i = 0; i < MAX_HOURS; i++) {
        if (hours[i]) {
          if (opt == 'b') {
            printf("***Busy Hour %d\n", i);
            policy->timer_policy.busy_hours[idx++] = i;
          } else {
            printf("***Quiet Hour %d\n", i);
            policy->timer_policy.quiet_hours[idx++] = i;
          }
        }
      }
      break;
    case 'l':
      cnt = parse_set(optarg, cores, RTE_POWER_MAX_VCPU_PER_VM);
      if (cnt < 0) {
      ...
      break;
    case 'p':
      cnt = parse_set(optarg, ports, RTE_POWER_MAX_VCPU_PER_VM);
      if (cnt < 0) {
    ...
    case 'o':
      if (!strcmp(optarg, "TRAFFIC"))
        policy->policy_to_use = RTE_POWER_POLICY_TRAFFIC;
      else if (!strcmp(optarg, "TIME"))
        policy->policy_to_use = RTE_POWER_POLICY_TIME;
      else if (!strcmp(optarg, "WORKLOAD"))
        policy->policy_to_use = RTE_POWER_POLICY_WORKLOAD;
      else if (!strcmp(optarg, "BRANCH_RATIO"))
        policy->policy_to_use = RTE_POWER_POLICY_BRANCH_RATIO;
      else {
...
int main(int argc, char **argv) {
  ...
  RTE_LCORE_FOREACH(lcore_id) { rte_power_init(lcore_id); }
  run_cli(NULL);
