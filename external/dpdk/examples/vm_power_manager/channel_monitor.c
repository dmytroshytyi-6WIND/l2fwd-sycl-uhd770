...
static int str_to_ether_addr(const char *a, struct rte_ether_addr *ether_addr) {
  int i;
  char *end;
...
static int set_policy_mac(struct rte_power_channel_packet *pkt, int idx,
                          char *mac) {
  union PFID pfid;
  int ret;
  ...
  if (ret != 0) {
    RTE_LOG(ERR, CHANNEL_MONITOR, "Invalid mac address received in JSON\n");
    pkt->vfid[idx] = 0;
    return -1;
  }
...
static char *get_resource_name_from_chn_path(const char *channel_path) {
  char *substr = NULL;
...
static int get_resource_id_from_vmname(const char *vm_name) {
  int result = -1;
  int off = 0;
...
static int parse_json_to_pkt(json_t *element,
  ...
  if (vm_name == NULL) {
    RTE_LOG(ERR, CHANNEL_MONITOR, "vm_name is NULL, request rejected !\n");
    return -1;
  }
  ...
  json_object_foreach(element, key, value) {
    if (!strcmp(key, "policy")) {
      ...
      if (!strcmp(command, "power")) {
        ...
        RTE_LOG(ERR, CHANNEL_MONITOR, "Invalid command received in JSON\n");
        return -1;
      }
      ...
      if (!strcmp(command, "TIME")) {
        ...
        pkt->policy_to_use = RTE_POWER_POLICY_TRAFFIC;
      } else if (!strcmp(command, "WORKLOAD")) {
        pkt->policy_to_use = RTE_POWER_POLICY_WORKLOAD;
      } else if (!strcmp(command, "BRANCH_RATIO")) {
        pkt->policy_to_use = RTE_POWER_POLICY_BRANCH_RATIO;
      } else {
        RTE_LOG(ERR, CHANNEL_MONITOR, "Wrong policy_type received in JSON\n");
        return -1;
      }
      ...
      if (!strcmp(command, "HIGH")) {
        ...
        RTE_LOG(ERR, CHANNEL_MONITOR, "Wrong workload received in JSON\n");
        return -1;
      }
      ...
      for (i = 0; i < size; i++) {
        int hour = (int)json_integer_value(json_array_get(value, i));
      ...
      for (i = 0; i < size; i++) {
        char mac[32];
        strlcpy(mac, json_string_value(json_array_get(value, i)), 32);
      ...
      RTE_LOG(ERR, CHANNEL_MONITOR, "Unknown key received in JSON string: %s\n",
              key);
    }
...
static void core_share(int pNo, int z, int x, int t) {
  if (policies[pNo].core_share[z].pcpu == lvm_info[x].pcpus[t]) {
    if (strcmp(policies[pNo].pkt.vm_name, lvm_info[x].vm_name) != 0) {
      policies[pNo].core_share[z].status = 1;
      power_manager_scale_core_max(policies[pNo].core_share[z].pcpu);
    }
  }
}

static void core_share_status(int pNo) {
...
static int pcpu_monitor(struct policy *pol, struct core_info *ci, int pcpu,
                        int count) {
  int ret = 0;
  ...
  if (pol->pkt.policy_to_use == RTE_POWER_POLICY_BRANCH_RATIO) {
    ...
    if (ret == 0)
      RTE_LOG(INFO, CHANNEL_MONITOR, "Monitoring pcpu %d OOB for %s\n", pcpu,
              pol->pkt.vm_name);
    else
      RTE_LOG(ERR, CHANNEL_MONITOR, "Error monitoring pcpu %d OOB for %s\n",
              pcpu, pol->pkt.vm_name);

  } else {
    pol->core_share[count].pcpu = pcpu;
    RTE_LOG(INFO, CHANNEL_MONITOR, "Monitoring pcpu %d for %s\n", pcpu,
            pol->pkt.vm_name);
  }
  return ret;
}

static void get_pcpu_to_control(struct policy *pol) {
  ...
  RTE_LOG(DEBUG, CHANNEL_MONITOR, "Looking for pcpu for %s\n",
          pol->pkt.vm_name);
...
static int get_pfid(struct policy *pol) {
...
#ifdef RTE_NET_I40E
      ret = rte_pmd_i40e_query_vfid_by_mac(
          x, (struct rte_ether_addr *)&(pol->pkt.vfid[i]));
#else
      ret = -ENOTSUP;
#endif
      if (ret != -EINVAL) {
...
static int update_policy(struct rte_power_channel_packet *pkt) {

  unsigned int updated = 0;
  unsigned int i;

  RTE_LOG(INFO, CHANNEL_MONITOR, "Applying policy for %s\n", pkt->vm_name);
  ...
  for (i = 0; i < RTE_DIM(policies); i++) {
    if (strcmp(policies[i].pkt.vm_name, pkt->vm_name) == 0) {
      ...
      if (policies[i].pkt.policy_to_use == RTE_POWER_POLICY_TRAFFIC) {
...
static int remove_policy(struct rte_power_channel_packet *pkt __rte_unused) {
  unsigned int i;
...
static uint64_t get_pkt_diff(struct policy *pol) {

  uint64_t vsi_pkt_count, vsi_pkt_total = 0, vsi_pkt_count_prev_total = 0;
...
static void apply_traffic_profile(struct policy *pol) {
  ...
  if (diff >= (pol->pkt.traffic_policy.max_max_packet_thresh)) {
    for (count = 0; count < pol->pkt.num_vcpu; count++) {
      if (pol->core_share[count].status != 1)
        power_manager_scale_core_max(pol->core_share[count].pcpu);
    }
  } else if (diff >= (pol->pkt.traffic_policy.avg_max_packet_thresh)) {
    for (count = 0; count < pol->pkt.num_vcpu; count++) {
      if (pol->core_share[count].status != 1)
        power_manager_scale_core_med(pol->core_share[count].pcpu);
    }
  } else if (diff < (pol->pkt.traffic_policy.avg_max_packet_thresh)) {
    for (count = 0; count < pol->pkt.num_vcpu; count++) {
      if (pol->core_share[count].status != 1)
        power_manager_scale_core_min(pol->core_share[count].pcpu);
    }
  }
}

static void apply_time_profile(struct policy *pol) {
  ...
  for (x = 0; x < RTE_POWER_HOURS_PER_DAY; x++) {
    ...
    if (ptm->tm_hour == pol->pkt.timer_policy.busy_hours[x]) {
      ...
      break;
    } else if (ptm->tm_hour == pol->pkt.timer_policy.quiet_hours[x]) {
      for (count = 0; count < pol->pkt.num_vcpu; count++) {
        if (pol->core_share[count].status != 1) {
          power_manager_scale_core_min(pol->core_share[count].pcpu);
        }
...
static void apply_workload_profile(struct policy *pol) {
...
static void apply_policy(struct policy *pol) {
...
static int write_binary_packet(void *buffer, size_t buffer_len,
...
static int send_freq(struct rte_power_channel_packet *pkt,
                     struct channel_info *chan_info, bool freq_list) {
  ...
  return write_binary_packet(&channel_pkt_freq_list,
                             sizeof(channel_pkt_freq_list), chan_info);
}

static int send_capabilities(struct rte_power_channel_packet *pkt,
                             struct channel_info *chan_info,
                             bool list_requested) {
  unsigned int vcore_id = pkt->resource_id;
  ...
  if (list_requested) {
    unsigned int i;
    for (i = 0; i < info.num_vcpus; i++) {
      ret = rte_power_get_capabilities(info.pcpu_map[i], &caps);
      if (ret == 0) {
        channel_pkt_caps_list.turbo[i] = caps.turbo;
        channel_pkt_caps_list.priority[i] = caps.priority;
      } else
        return -1;
    }
  } else {
    ret = rte_power_get_capabilities(info.pcpu_map[vcore_id], &caps);
    if (ret == 0) {
      channel_pkt_caps_list.turbo[vcore_id] = caps.turbo;
      channel_pkt_caps_list.priority[vcore_id] = caps.priority;
    } else
      return -1;
  }

  return write_binary_packet(&channel_pkt_caps_list,
                             sizeof(channel_pkt_caps_list), chan_info);
}

static int send_ack_for_received_cmd(struct rte_power_channel_packet *pkt,
                                     struct channel_info *chan_info,
                                     uint32_t command) {
  pkt->command = command;
  return write_binary_packet(pkt, sizeof(*pkt), chan_info);
}

static int process_request(struct rte_power_channel_packet *pkt,
                           struct channel_info *chan_info) {
  int ret;
  ...
  if (pkt->command == RTE_POWER_CPU_POWER) {
    ...
    if (valid_unit) {
      ret = send_ack_for_received_cmd(pkt, chan_info,
                                      scale_res >= 0 ? RTE_POWER_CMD_ACK
                                                     : RTE_POWER_CMD_NACK);
      if (ret < 0)
  ...
  if (pkt->command == RTE_POWER_PKT_POLICY) {
    RTE_LOG(INFO, CHANNEL_MONITOR, "Processing policy request %s\n",
            pkt->vm_name);
    int ret = send_ack_for_received_cmd(pkt, chan_info, RTE_POWER_CMD_ACK);
    if (ret < 0)
      RTE_LOG(ERR, CHANNEL_MONITOR, "Error during sending ack command.\n");
    update_policy(pkt);
    policy_is_set = 1;
  ...
  if (pkt->command == RTE_POWER_PKT_POLICY_REMOVE) {
    ...
    if (ret == 0)
      RTE_LOG(INFO, CHANNEL_MONITOR, "Removed policy %s\n", pkt->vm_name);
    else
      RTE_LOG(INFO, CHANNEL_MONITOR, "Policy %s does not exist\n",
              pkt->vm_name);
  }
  ...
  if (pkt->command == RTE_POWER_QUERY_FREQ_LIST ||
    ...
    RTE_LOG(INFO, CHANNEL_MONITOR, "Frequency for %s requested.\n",
            pkt->vm_name);
    int ret =
        send_freq(pkt, chan_info, pkt->command == RTE_POWER_QUERY_FREQ_LIST);
    if (ret < 0)
  ...
  if (pkt->command == RTE_POWER_QUERY_CAPS_LIST ||
    ...
    RTE_LOG(INFO, CHANNEL_MONITOR, "Capabilities for %s requested.\n",
            pkt->vm_name);
    int ret = send_capabilities(pkt, chan_info,
  ...
  __atomic_compare_exchange_n(&(chan_info->status), &channel_processing,
                              CHANNEL_MGR_CHANNEL_CONNECTED, 0,
...
int channel_monitor_init(void) {
  ...
  global_events_list =
      rte_malloc("epoll_events", sizeof(*global_events_list) * MAX_EVENTS,
                 RTE_CACHE_LINE_SIZE);
...
static void read_binary_packet(struct channel_info *chan_info) {
  struct rte_power_channel_packet pkt;
  void *buffer = &pkt;
...
#ifdef USE_JANSSON
static void read_json_packet(struct channel_info *chan_info) {
  struct rte_power_channel_packet pkt;
  int n_bytes, ret;
  json_t *root;
  json_error_t error;
  ...
  do {
    ...
    if (root) {
      resource_name = get_resource_name_from_chn_path(chan_info->channel_path);
      /*
      ...
      json_decref(root);
      if (ret) {
        RTE_LOG(ERR, CHANNEL_MONITOR, "Error validating JSON profile data\n");
        break;
      }
      start = strstr(pkt.vm_name, CHANNEL_MGR_FIFO_PATTERN_NAME);
      if (start != NULL) {
      ...
      RTE_LOG(ERR, CHANNEL_MONITOR, "JSON error on line %d: %s\n", error.line,
              error.text);
    }
  } while (n_bytes > 0);
}
...
void run_channel_monitor(void) {
  while (run_loop) {
    ...
    n_events = epoll_wait(global_event_fd, global_events_list, MAX_EVENTS, 1);
    if (!run_loop)
      break;
    for (i = 0; i < n_events; i++) {
      struct channel_info *chan_info =
          (struct channel_info *)global_events_list[i].data.ptr;
