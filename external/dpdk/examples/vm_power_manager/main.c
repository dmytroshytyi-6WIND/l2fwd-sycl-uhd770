...
static int parse_args(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argvopt, "p:q:T:b:", lgopts,
                            &option_index)) != EOF) {
...
static int run_monitor(__rte_unused void *arg) {
...
static int run_core_monitor(__rte_unused void *arg) {
...
int main(int argc, char **argv) {
  ...
  if (nb_ports > 0) {
    ...
    RTE_ETH_FOREACH_DEV(portid) {
      ...
      for (w = 0; w < RTE_POWER_MAX_VFS; w++) {
        ...
        ret = -ENOTSUP;
#ifdef RTE_NET_IXGBE
        ret = rte_pmd_ixgbe_set_vf_mac_addr(portid, w, &eth);
#endif
#ifdef RTE_NET_I40E
        if (ret == -ENOTSUP)
          ret = rte_pmd_i40e_set_vf_mac_addr(portid, w, &eth);
#endif
#ifdef RTE_NET_BNXT
        if (ret == -ENOTSUP)
          ret = rte_pmd_bnxt_set_vf_mac_addr(portid, w, &eth);
#endif

        switch (ret) {
        case 0:
          printf("Port %d VF %d MAC: ", portid, w);
          for (j = 0; j < 5; j++) {
  ...
  if (lcore_id == RTE_MAX_LCORE) {
    RTE_LOG(ERR, EAL,
            "A minimum of three cores are required to run "
            "application\n");
    return 0;
  }
