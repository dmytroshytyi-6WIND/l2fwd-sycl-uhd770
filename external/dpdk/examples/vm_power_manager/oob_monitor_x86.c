...
void branch_monitor_exit(void) { run_loop = 0; }
...
static float apply_policy(int core) {
  struct core_info *ci;
  uint64_t counter = 0;
  ...
  ret = pread(ci->cd[core].msr_fd, &counter, sizeof(counter), IA32_PERFCTR0);
  if (ret < 0)
    RTE_LOG(ERR, POWER_MANAGER, "unable to read counter for core %u\n", core);
  branches = counter;

  counter = 0;
  ret = pread(ci->cd[core].msr_fd, &counter, sizeof(counter), IA32_PERFCTR1);
  if (ret < 0)
    RTE_LOG(ERR, POWER_MANAGER, "unable to read counter for core %u\n", core);
...
int add_core_to_monitor(int core) {
  ...
  if (core < ci->core_count) {
    ...
    ret = pwrite(ci->cd[core].msr_fd, &setup, sizeof(setup), IA32_PERFEVTSEL0);
    if (ret < 0) {
      RTE_LOG(ERR, POWER_MANAGER, "unable to set counter for core %u\n", core);
      return ret;
    }
    setup = IA32_PERFEVT_BRANCH_MISS;
    ret = pwrite(ci->cd[core].msr_fd, &setup, sizeof(setup), IA32_PERFEVTSEL1);
    if (ret < 0) {
      RTE_LOG(ERR, POWER_MANAGER, "unable to set counter for core %u\n", core);
      return ret;
    }
    ...
    for (j = 0; j < ci->core_count; j++) {
      if (ci->cd[j].oob_enabled) {
        ...
        if ((print > PRINT_LOOP_COUNT) && (g_active)) {
          printf("  %d: %.4f {%lu} {%d}", j, ratio, g_branches, reads);
          printed = 1;
          reads = 0;
        } else {
          reads++;
