...
static struct virtual_machine_info *find_domain_by_name(const char *name) {
...
static int update_pcpus_mask(struct virtual_machine_info *vm_info) {
  ...
  if (!virDomainIsActive(vm_info->domainPtr)) {
    n_vcpus = virDomainGetVcpuPinInfo(vm_info->domainPtr,
                                      vm_info->info.nrVirtCpu, global_cpumaps,
                                      global_maplen, VIR_DOMAIN_AFFECT_CONFIG);
    if (n_vcpus < 0) {
  ...
  memset(global_vircpuinfo, 0, sizeof(*global_vircpuinfo) * RTE_MAX_LCORE);

  cpuinfo = global_vircpuinfo;

  n_vcpus = virDomainGetVcpus(vm_info->domainPtr, cpuinfo, RTE_MAX_LCORE,
                              global_cpumaps, global_maplen);
  if (n_vcpus < 0) {
    RTE_LOG(ERR, CHANNEL_MANAGER,
            "Error getting vCPU info for "
            "active VM '%s'\n",
...
update_pcpus:
  ...
  if (n_vcpus != vm_info->info.nrVirtCpu) {
    RTE_LOG(INFO, CHANNEL_MANAGER,
            "Updating the number of vCPUs for VM '%s"
            " from %d -> %d\n",
            vm_info->name, vm_info->info.nrVirtCpu, n_vcpus);
...
static inline int channel_exists(struct virtual_machine_info *vm_info,
...
static int open_non_blocking_channel(struct channel_info *info) {
  int ret, flags;
  ...
  ret = connect(info->fd, (struct sockaddr *)&sock_addr, sizeof(sock_addr));
  if (ret < 0) {
...
static int open_host_channel(struct channel_info *info) {
  int flags;
...
static int setup_channel_info(struct virtual_machine_info **vm_info_dptr,
                              struct channel_info **chan_info_dptr,
                              unsigned channel_num) {
...
static int fifo_path(char *dst, unsigned int len, unsigned int id) {
  int cnt;
...
static int setup_host_channel_info(struct channel_info **chan_info_dptr,
...
int add_all_channels(const char *vm_name) {
  ...
  while ((dir = readdir(d)) != NULL) {
    ...
    if ((errno != 0) || (remaining[0] == '\0') || tail_ptr == NULL ||
        (*tail_ptr != '\0')) {
      RTE_LOG(WARNING, CHANNEL_MANAGER,
    ...
    if (channel_num >= RTE_MAX_LCORE) {
      RTE_LOG(WARNING, CHANNEL_MANAGER,
              ...
              channel_num, RTE_MAX_LCORE - 1, CHANNEL_MGR_SOCKET_PATH,
              dir->d_name);
      continue;
    }
    ...
    chan_info = rte_malloc(NULL, sizeof(*chan_info), RTE_CACHE_LINE_SIZE);
    if (chan_info == NULL) {
      RTE_LOG(ERR, CHANNEL_MANAGER,
    ...
    if ((size_t)snprintf(chan_info->channel_path,
                         ...
                         dir->d_name) >= sizeof(chan_info->channel_path)) {
  ...
  for (i = 0; i < len_channel_list; i++) {
    if (channel_list[i] >= RTE_MAX_LCORE) {
      RTE_LOG(INFO, CHANNEL_MANAGER,
              ...
              "0...%d\n",
              channel_list[i], RTE_MAX_LCORE - 1);
      continue;
    }
    ...
    snprintf(chan_info->channel_path, sizeof(chan_info->channel_path),
             "%s%s.%u", CHANNEL_MGR_SOCKET_PATH, vm_name, channel_list[i]);
...
int set_channel_status_all(const char *vm_name, enum channel_status status) {
  struct virtual_machine_info *vm_info;
  unsigned i;
...
void get_all_vm(int *num_vm, int *num_vcpu) {
  ...
  *num_vm =
      virConnectListAllDomains(global_vir_conn_ptr, &domptr, domain_flags);
  if (*num_vm <= 0) {
  ...
  for (i = 0; i < *num_vm; i++) {
    ...
    n_vcpus = virDomainGetVcpuPinInfo(domptr[i], numVcpus[i], global_cpumaps,
    ...
    for (ii = 0; ii < n_vcpus; ii++) {
      for (jj = 0; jj < global_n_host_cpus; jj++) {
        if (VIR_CPU_USABLE(global_cpumaps, global_maplen, ii, jj) > 0) {
...
int add_vm(const char *vm_name) {
  ...
  if (new_domain->info.nrVirtCpu > RTE_MAX_LCORE) {
    RTE_LOG(ERR, CHANNEL_MANAGER,
            ...
            new_domain->info.nrVirtCpu, RTE_MAX_LCORE);
    rte_free(new_domain);
    return -1;
  }
...
int remove_vm(const char *vm_name) {
...
static void disconnect_hypervisor(void) {
  if (global_vir_conn_ptr != NULL) {
...
static int connect_hypervisor(const char *path) {
  if (global_vir_conn_ptr != NULL) {
    RTE_LOG(ERR, CHANNEL_MANAGER,
...
int channel_manager_init(const char *path __rte_unused) {
  virNodeInfo info;
  ...
  if (connect_hypervisor(path) < 0) {
    ...
    global_vircpuinfo = rte_zmalloc(
        NULL, sizeof(*global_vircpuinfo) * RTE_MAX_LCORE, RTE_CACHE_LINE_SIZE);
    if (global_vircpuinfo == NULL) {
      RTE_LOG(ERR, CHANNEL_MANAGER, "Error allocating memory for CPU Info\n");
      goto error;
    }
    global_cpumaps =
        rte_zmalloc(NULL, RTE_MAX_LCORE * global_maplen, RTE_CACHE_LINE_SIZE);
  ...
  if (global_n_host_cpus > RTE_MAX_LCORE) {
    RTE_LOG(WARNING, CHANNEL_MANAGER,
            ...
            global_n_host_cpus, RTE_MAX_LCORE, RTE_MAX_LCORE - 1);
