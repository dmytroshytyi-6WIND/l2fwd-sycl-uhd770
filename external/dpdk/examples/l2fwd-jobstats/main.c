...
static void show_lcore_stats(unsigned lcore_id) {
...
#define STAT_FMT "\n%-18s %'14.0f %6.1f%% %'10.0f %'10.0f %'10.0f"
  ...
  printf("\n----------------"
         ...
         "\n%-18s %'14.0f"
         "\n%-18s %'14" PRIu64 STAT_FMT /* Exec */
             STAT_FMT                   /* Management */
                 STAT_FMT               /* Busy */
                     STAT_FMT,          /* Idle  */
         lcore_id, cycles_to_ns(collection_time), "Stat type", "total",
         "%total", "avg", "min", "max",
         "Stats duration:", cycles_to_ns(stats_period),
         "Loop count:", loop_count, "Exec time", cycles_to_ns(exec),
         exec * 100.0 / stats_period,
         cycles_to_ns(loop_count ? exec / loop_count : 0),
         cycles_to_ns(exec_min), cycles_to_ns(exec_max), "Management time",
         cycles_to_ns(management), management * 100.0 / stats_period,
         cycles_to_ns(loop_count ? management / loop_count : 0),
         cycles_to_ns(management_min), cycles_to_ns(management_max),
         "Exec + management", cycles_to_ns(busy), busy * 100.0 / stats_period,
         cycles_to_ns(loop_count ? busy / loop_count : 0),
         cycles_to_ns(busy_min), cycles_to_ns(busy_max), "Idle (job)",
         cycles_to_ns(idle_exec), idle_exec * 100.0 / stats_period,
         cycles_to_ns(idle_exec_cnt ? idle_exec / idle_exec_cnt : 0),
         cycles_to_ns(idle_exec_min), cycles_to_ns(idle_exec_max));

  for (i = 0; i < qconf->n_rx_port; i++) {
    job = &qconf->port_fwd_jobs[i];
    printf("\n\nJob %" PRIu32 ": %-20s "
           "\n%-18s %'14" PRIu64 "\n%-18s %'14.0f" STAT_FMT,
           i, job->name, "Exec count:", jobs_exec_cnt[i],
           "Exec period: ", cycles_to_ns(jobs_period[i]), "Exec time",
           cycles_to_ns(jobs_exec[i]), jobs_exec[i] * 100.0 / stats_period,
           cycles_to_ns(jobs_exec_cnt[i] ? jobs_exec[i] / jobs_exec_cnt[i] : 0),
           cycles_to_ns(jobs_exec_min[i]), cycles_to_ns(jobs_exec_max[i]));
  }
  ...
  if (qconf->n_rx_port > 0) {
    ...
    printf("\n\nJob %" PRIu32 ": %-20s "
           "\n%-18s %'14" PRIu64 "\n%-18s %'14.0f" STAT_FMT,
           i, job->name, "Exec count:", flush_exec_cnt,
           "Exec period: ", cycles_to_ns(flush_period), "Exec time",
           cycles_to_ns(flush_exec), flush_exec * 100.0 / stats_period,
           cycles_to_ns(flush_exec_cnt ? flush_exec / flush_exec_cnt : 0),
           cycles_to_ns(flush_exec_min), cycles_to_ns(flush_exec_max));
  }
}

/* Print out statistics on packets dropped */
static void show_stats_cb(__rte_unused void *param) {
...
static void l2fwd_job_update_cb(struct rte_jobstats *job, int64_t result) {
...
static void l2fwd_fwd_job(__rte_unused struct rte_timer *timer, void *arg) {
...
static void l2fwd_flush_job(__rte_unused struct rte_timer *timer,
  ...
  for (;;) {
    ...
    do {
      ...
      do {
        ...
        stats_read_pending =
            __atomic_load_n(&qconf->stats_read_pending, __ATOMIC_RELAXED);
...
static void l2fwd_usage(const char *prgname) {
  printf("%s [EAL options] -- -p PORTMASK [-q NQ]\n"
         ...
         "  -l set system default locale instead of default (\"C\" locale) for "
         "thousands separator in stats.",
         prgname);
}
...
static int l2fwd_parse_args(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argvopt, "p:q:T:l", lgopts, &option_index)) !=
         EOF) {
...
int main(int argc, char **argv) {
  ...
  RTE_LCORE_FOREACH(lcore_id) {
    ...
    ret = rte_timer_reset(&qconf->flush_timer, drain_tsc, PERIODICAL, lcore_id,
                          &l2fwd_flush_job, NULL);

    if (ret < 0) {
      rte_exit(1, "Failed to reset flush job timer for lcore %u: %s", lcore_id,
               rte_strerror(-ret));
    }
    ...
    for (i = 0; i < qconf->n_rx_port; i++) {
      ...
      if (ret < 0) {
        rte_exit(1, "Failed to reset lcore %u port %u job timer: %s", lcore_id,
