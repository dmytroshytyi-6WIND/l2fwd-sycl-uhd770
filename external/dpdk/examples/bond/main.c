...
#define RX_FTHRESH                                                             \
  (MAX_PKT_BURST * 2) /**< Default values of RX free threshold reg. */
...
static void slave_port_init(uint16_t portid, struct rte_mempool *mbuf_pool) {
  int retval;
  ...
  if (retval != 0)
    rte_exit(EXIT_FAILURE,
             "port %u: rte_eth_dev_adjust_nb_rx_tx_desc "
             "failed (res=%d)\n",
             portid, retval);
  ...
  retval = rte_eth_rx_queue_setup(
      portid, 0, nb_rxd, rte_eth_dev_socket_id(portid), &rxq_conf, mbuf_pool);
  if (retval < 0)
    rte_exit(retval, " port %u: RX queue 0 setup failed (res=%d)", portid,
             retval);
  ...
  if (retval < 0)
    rte_exit(retval, "port %u: TX queue 0 setup failed (res=%d)", portid,
             retval);

  retval = rte_eth_dev_start(portid);
  if (retval < 0)
    rte_exit(retval, "Start port %d failed (res=%d)", portid, retval);

  struct rte_ether_addr addr;

  retval = rte_eth_macaddr_get(portid, &addr);
  if (retval != 0)
    rte_exit(retval, "Mac address get port %d failed (res=%d)", portid, retval);
...
static void bond_port_init(struct rte_mempool *mbuf_pool) {
  int retval;
  uint8_t i;
  uint16_t nb_rxd = RTE_RX_DESC_DEFAULT;
  uint16_t nb_txd = RTE_TX_DESC_DEFAULT;
  ...
  retval = rte_eth_bond_create("net_bonding0", BONDING_MODE_ALB,
                               0 /*SOCKET_ID_ANY*/);
  if (retval < 0)
    rte_exit(EXIT_FAILURE, "Failed to create bond port\n");
  ...
  retval = rte_eth_rx_queue_setup(BOND_PORT, 0, nb_rxd,
                                  rte_eth_dev_socket_id(BOND_PORT), &rxq_conf,
                                  mbuf_pool);
  if (retval < 0)
    rte_exit(retval, " port %u: RX queue 0 setup failed (res=%d)", BOND_PORT,
             retval);
  ...
  if (retval < 0)
    rte_exit(retval, "port %u: TX queue 0 setup failed (res=%d)", BOND_PORT,
             retval);
  ...
  if (retval != 0) {
    rte_exit(EXIT_FAILURE, "port %u: promiscuous mode enable failed: %s\n",
  ...
  if (retval != 0)
    rte_exit(retval, "port %u: Mac address get failed (res=%d)", BOND_PORT,
             retval);
...
static inline void get_string(struct cmd_obj_send_result *res, char *buf,
                              uint8_t size) {
...
static void cmd_obj_send_parsed(void *parsed_result,
  ...
  ret = rte_eth_macaddr_get(BOND_PORT, &bond_mac_addr);
  if (ret != 0) {
    cmdline_printf(cl, "Failed to get bond (port %u) MAC address: %s\n",
  ...
  arp_hdr->arp_data.arp_tip = ((unsigned char *)&res->ip.addr.ipv4)[0] |
...
static void cmd_start_parsed(__rte_unused void *parsed_result,
  ...
  if (global_flag_stru_p->LcoreMainIsRunning == 0) {
    if (rte_eal_get_lcore_state(global_flag_stru_p->LcoreMainCore) != WAIT) {
      rte_spinlock_unlock(&global_flag_stru_p->lock);
      return;
    }
    rte_spinlock_unlock(&global_flag_stru_p->lock);
  ...
  cmdline_printf(cl,
                 ...
                 BOND_IP_1, BOND_IP_2, BOND_IP_3, BOND_IP_4);
}
...
static void cmd_help_parsed(__rte_unused void *parsed_result,
                            struct cmdline *cl, __rte_unused void *data) {
  cmdline_printf(
      cl, "ALB - link bonding mode 6 example\n"
...
static void cmd_stop_parsed(__rte_unused void *parsed_result,
  ...
  rte_spinlock_trylock(&global_flag_stru_p->lock);
  if (global_flag_stru_p->LcoreMainIsRunning == 0) {
    cmdline_printf(cl, "lcore_main not running on core:%d\n",
                   global_flag_stru_p->LcoreMainCore);
    rte_spinlock_unlock(&global_flag_stru_p->lock);
    return;
  }
  ...
  if (rte_eal_wait_lcore(global_flag_stru_p->LcoreMainCore) < 0)
    cmdline_printf(cl, "error: lcore_main can not stop on core:%d\n",
                   global_flag_stru_p->LcoreMainCore);
  else
    cmdline_printf(cl, "lcore_main stopped on core:%d\n",
                   global_flag_stru_p->LcoreMainCore);
  rte_spinlock_unlock(&global_flag_stru_p->lock);
}
...
static void cmd_show_parsed(__rte_unused void *parsed_result,
  ...
  for (i = 0; i < slaves_count; i++) {
    ...
    if (ret != 0) {
      cmdline_printf(cl, "Failed to get port %u MAC address: %s\n", i,
                     strerror(-ret));
      continue;
    }
...
int main(int argc, char *argv[]) {
  ...
  printf("Starting lcore_main on core %d:%d Our IP:%d.%d.%d.%d\n",
         worker_core_id,
         rte_eal_remote_launch((lcore_function_t *)lcore_main, NULL,
                               worker_core_id),
         BOND_IP_1, BOND_IP_2, BOND_IP_3, BOND_IP_4);
