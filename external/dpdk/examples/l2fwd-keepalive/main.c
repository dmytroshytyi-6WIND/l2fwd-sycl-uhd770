...
static void print_stats(__rte_unused struct rte_timer *ptr_timer,
...
static int l2fwd_parse_check_period(const char *q_arg) {
  char *end = NULL;
  int n;
...
static int l2fwd_parse_args(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argvopt, "p:q:T:K:", lgopts,
                            &option_index)) != EOF) {
...
static void dead_core(__rte_unused void *ptr_data, const int id_core) {
...
static void relay_core_state(void *ptr_data, const int id_core,
                             const enum rte_keepalive_state core_state,
                             uint64_t last_alive) {
  rte_keepalive_relayed_state((struct rte_keepalive_shm *)ptr_data, id_core,
...
int main(int argc, char **argv) {
  ...
  l2fwd_pktmbuf_pool =
      rte_pktmbuf_pool_create("mbuf_pool", total_nb_mbufs, 32, 0,
  ...
  RTE_ETH_FOREACH_DEV(portid) {
    ...
    if (ret < 0)
      rte_exit(EXIT_FAILURE, "Cannot mac address: err=%d, port=%u\n", ret,
               portid);
  ...
  if (check_period > 0) {
    ...
    if (ka_shm == NULL)
      rte_exit(EXIT_FAILURE, "rte_keepalive_shm_create() failed");
    /* Initialize keepalive functionality. 8< */
    rte_global_keepalive_info = rte_keepalive_create(&dead_core, ka_shm);
    ...
    if (rte_timer_reset(&hb_timer, (check_period * rte_get_timer_hz()) / 1000,
                        PERIODICAL, rte_lcore_id(),
                        (void (*)(struct rte_timer *, void *)) &
                            rte_keepalive_dispatch_pings,
                        rte_global_keepalive_info) != 0)
      rte_exit(EXIT_FAILURE, "Keepalive setup failure.\n");
  }
  if (timer_period > 0) {
    /* Issues the pings keepalive_dispatch_pings(). 8< */
    if (rte_timer_reset(&stats_timer,
                        (timer_period * rte_get_timer_hz()) / 1000, PERIODICAL,
                        rte_lcore_id(), &print_stats, NULL) != 0)
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    if (qconf->n_rx_port == 0)
      ...
      rte_keepalive_register_core(rte_global_keepalive_info, lcore_id);
    }
  }
  while (!terminate_signal_received) {
    rte_timer_manage();
