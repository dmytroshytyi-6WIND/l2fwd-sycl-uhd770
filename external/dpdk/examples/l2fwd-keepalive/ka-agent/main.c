...
static struct rte_keepalive_shm *ka_shm_create(void) {
  ...
  if (fd < 0)
    ...
    shm = (struct rte_keepalive_shm *)mmap(0, size, PROT_READ | PROT_WRITE,
                                           MAP_SHARED, fd, 0);
    close(fd);
    if (shm == MAP_FAILED)
...
int main(void) {
  ...
  while (1) {
    ...
    for (idx_core = 0; idx_core < RTE_KEEPALIVE_MAXCORES; idx_core++)
      if (shm->core_last_seen_times[idx_core] > most_recent_alive_time)
        most_recent_alive_time = shm->core_last_seen_times[idx_core];
    ...
    if (sem_timedwait(&shm->core_died, &timeout) == -1) {
      ...
      if (sem_errno == ETIMEDOUT) {
        if (last_seen_alive_time == most_recent_alive_time &&
            cnt_timeouts++ > MAX_TIMEOUTS) {
