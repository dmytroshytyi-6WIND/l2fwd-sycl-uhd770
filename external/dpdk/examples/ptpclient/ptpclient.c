...
static void print_clock_info(struct ptpv2_data_slave_ordinary *ptp_data) {
  int64_t nsec;
  struct timespec net_time, sys_time;

  printf("Master Clock id: %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
         ptp_data->master_clock_id.id[0], ptp_data->master_clock_id.id[1],
         ptp_data->master_clock_id.id[2], ptp_data->master_clock_id.id[3],
         ptp_data->master_clock_id.id[4], ptp_data->master_clock_id.id[5],
         ptp_data->master_clock_id.id[6], ptp_data->master_clock_id.id[7]);

  printf("\nT2 - Slave  Clock.  %lds %ldns", (ptp_data->tstamp2.tv_sec),
         (ptp_data->tstamp2.tv_nsec));

  printf("\nT1 - Master Clock.  %lds %ldns ", ptp_data->tstamp1.tv_sec,
         (ptp_data->tstamp1.tv_nsec));

  printf("\nT3 - Slave  Clock.  %lds %ldns", ptp_data->tstamp3.tv_sec,
         (ptp_data->tstamp3.tv_nsec));

  printf("\nT4 - Master Clock.  %lds %ldns ", ptp_data->tstamp4.tv_sec,
  ...
  printf("\nCurrent PTP Time: %.24s %.9ld ns", ctime(&ts), net_time.tv_nsec);
  ...
  printf("\nCurrent SYS Time: %.24s %.6ld ns", ctime(&tp),
         ptp_data->new_adj.tv_usec);
 ...
 */
static void parse_sync(struct ptpv2_data_slave_ordinary *ptp_data,
  ...
  if (ptp_data->ptpset == 0) {
    rte_memcpy(&ptp_data->master_clock_id, &ptp_hdr->source_port_id.clock_id,
               sizeof(struct clock_id));
    ptp_data->ptpset = 1;
  }

  if (memcmp(&ptp_hdr->source_port_id.clock_id,
             &ptp_hdr->source_port_id.clock_id, sizeof(struct clock_id)) == 0) {

    if (ptp_data->ptpset == 1)
      rte_eth_timesync_read_rx_timestamp(ptp_data->portid, &ptp_data->tstamp2,
                                         rx_tstamp_idx);
  }
}
 ...
 */
static void parse_fup(struct ptpv2_data_slave_ordinary *ptp_data) {
  ...
  ptp_data->tstamp1.tv_sec = ((uint64_t)ntohl(origin_tstamp->sec_lsb)) |
  ...
  if (ptp_data->seqID_FOLLOWUP == ptp_data->seqID_SYNC) {
    ...
    if (ret != 0) {
      printf("\nCore %u: port %u failed to get MAC address: %s\n",
             rte_lcore_id(), ptp_data->portid, rte_strerror(-ret));
      return;
    }

    created_pkt = rte_pktmbuf_alloc(mbuf_pool);
    pkt_size = sizeof(struct rte_ether_hdr) + sizeof(struct delay_req_msg);
    ...
    req_msg = rte_pktmbuf_mtod_offset(created_pkt, struct delay_req_msg *,
                                      sizeof(struct rte_ether_hdr));
    ...
    req_msg->hdr.message_length = htons(sizeof(struct delay_req_msg));
    req_msg->hdr.domain_number = ptp_hdr->domain_number;

    /* Set up clock id. */
    client_clkid = &req_msg->hdr.source_port_id.clock_id;
    ...
    rte_memcpy(&ptp_data->client_clock_id, client_clkid,
               sizeof(struct clock_id));
    ...
    rte_eth_timesync_read_tx_timestamp(ptp_data->portid, &ptp_data->tstamp3);
 ...
 * time.
 */
static inline void update_kernel_time(void) {
  int64_t nsec;
  struct timespec net_time, sys_time;
...
static void parse_drsp(struct ptpv2_data_slave_ordinary *ptp_data) {
  ...
  if (memcmp(&ptp_data->client_clock_id,
    ...
    if (seq_id == ptp_data->seqID_FOLLOWUP) {
      ...
      ptp_data->tstamp4.tv_sec = ((uint64_t)ntohl(rx_tstamp->sec_lsb)) |
...
static int ptp_parse_portmask(const char *portmask) {
  char *end = NULL;
  unsigned long pm;
...
static int parse_ptp_kernel(const char *param) {
  char *end = NULL;
  unsigned long pm;
...
static int ptp_parse_args(int argc, char **argv) {
  int opt, ret;
  char **argvopt;
  int option_index;
  char *prgname = argv[0];
  ...
  while ((opt = getopt_long(argc, argvopt, "p:T:", lgopts, &option_index)) !=
         EOF) {
