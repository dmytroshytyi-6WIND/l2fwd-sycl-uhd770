...
static int64_t timer_period =
    10 * TIMER_MILLISECOND * 1000; /* default period is 10 seconds */
...
static void print_stats(void) {
  ...
  for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
    ...
    printf("\nStatistics for port %u ------------------------------"
           ...
           link_get_err < 0 ? "0" : rte_eth_link_speed_to_str(link.link_speed),
           link_get_err < 0
               ? "Link get failed"
...
static void lsi_simple_forward(struct rte_mbuf *m, unsigned portid) {
...
static void lsi_main_loop(void) {
...
static int lsi_launch_one_lcore(__rte_unused void *dummy) {
  lsi_main_loop();
  return 0;
}

/* display usage */
static void lsi_usage(const char *prgname) {
...
static int lsi_parse_portmask(const char *portmask) {
  char *end = NULL;
  unsigned long pm;
...
static unsigned int lsi_parse_nqueue(const char *q_arg) {
  char *end = NULL;
  unsigned long n;
...
static int lsi_parse_timer_period(const char *q_arg) {
  char *end = NULL;
  int n;
...
static int lsi_parse_args(int argc, char **argv) {
  int opt, ret;
  char **argvopt;
  int option_index;
  char *prgname = argv[0];
  ...
  while ((opt = getopt_long(argc, argvopt, "p:q:T:", lgopts, &option_index)) !=
         EOF) {
...
static int lsi_event_callback(uint16_t port_id, enum rte_eth_event_type type,
...
int main(int argc, char **argv) {
  ...
  lsi_pktmbuf_pool = rte_pktmbuf_pool_create(
  ...
  for (portid = 0; portid < nb_ports; portid++) {
    ...
    while (rte_lcore_is_enabled(rx_lcore_id) == 0 ||
           lcore_queue_conf[rx_lcore_id].n_rx_port == lsi_rx_queue_per_lcore) {
  ...
  for (portid = 0; portid < nb_ports; portid++) {
    ...
    if (ret < 0)
      rte_exit(EXIT_FAILURE, "rte_eth_macaddr_get: err=%d, port=%u\n", ret,
               (unsigned int)portid);
    ...
    printf("Port %u, MAC address: " RTE_ETHER_ADDR_PRT_FMT "\n\n",
           (unsigned)portid, RTE_ETHER_ADDR_BYTES(&lsi_ports_eth_addr[portid]));
