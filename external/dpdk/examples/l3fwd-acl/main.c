...
static inline int is_valid_ipv4_pkt(struct rte_ipv4_hdr *pkt,
                                    uint32_t link_len);
#endif
static inline void send_single_packet(struct rte_mbuf *m, uint16_t port);
...
struct rte_acl_field_def ipv6_defs[NUM_FIELDS_IPV6] = {
    ...
    },
    {
        ...
        .offset =
            sizeof(struct rte_ipv6_hdr) - offsetof(struct rte_ipv6_hdr, proto),
    },
    {
        .type = RTE_ACL_FIELD_TYPE_RANGE,
        .size = sizeof(uint16_t),
...
static inline void dump_acl4_rule(struct rte_mbuf *m, uint32_t sig) {
  ...
  printf("Src port:%hu,Dst port:%hu ", rte_bswap16(*(uint16_t *)(ipv4_hdr + 1)),
...
static inline void dump_acl6_rule(struct rte_mbuf *m, uint32_t sig) {
  ...
  struct rte_ipv6_hdr *ipv6_hdr = rte_pktmbuf_mtod_offset(
      m, struct rte_ipv6_hdr *, sizeof(struct rte_ether_hdr));
...
#ifdef DO_RFC_1812_CHECKS
static inline void prepare_one_packet(struct rte_mbuf **pkts_in,
...
static inline void prepare_acl_parameter(struct rte_mbuf **pkts_in,
...
static inline void send_packets(struct rte_mbuf **m, uint32_t *res, int num) {
  int i;
...
static int parse_cb_ipv6_rule(char *str, struct rte_acl_rule *v,
  ...
  /* source port. */
  GET_CB_FIELD(in[CB_FLD_SRC_PORT_LOW], v->field[SRCP_FIELD_IPV6].value.u16, 0,
               UINT16_MAX, 0);
  ...
  /* destination port. */
  GET_CB_FIELD(in[CB_FLD_DST_PORT_LOW], v->field[DSTP_FIELD_IPV6].value.u16, 0,
               UINT16_MAX, 0);
...
static int parse_cb_ipv4vlan_rule(char *str, struct rte_acl_rule *v,
  ...
  rc = parse_ipv4_net(in[CB_FLD_SRC_ADDR], &v->field[SRC_FIELD_IPV4].value.u32,
  ...
  rc = parse_ipv4_net(in[CB_FLD_DST_ADDR], &v->field[DST_FIELD_IPV4].value.u32,
  ...
  GET_CB_FIELD(in[CB_FLD_SRC_PORT_LOW], v->field[SRCP_FIELD_IPV4].value.u16, 0,
               UINT16_MAX, 0);
  GET_CB_FIELD(in[CB_FLD_SRC_PORT_HIGH],
  ...
  GET_CB_FIELD(in[CB_FLD_DST_PORT_LOW], v->field[DSTP_FIELD_IPV4].value.u16, 0,
               UINT16_MAX, 0);
  GET_CB_FIELD(in[CB_FLD_DST_PORT_HIGH],
...
static int add_rules(const char *rule_path, struct rte_acl_rule **proute_base,
...
static int usage_acl_alg(char *buf, size_t sz) {
  uint32_t i, n, rc, tn;

  n = 0;
  tn = 0;
  for (i = 0; i < RTE_DIM(acl_alg); i++) {
    rc = snprintf(buf + n, sz - n, i == RTE_DIM(acl_alg) - 1 ? "%s" : "%s|",
                  acl_alg[i].name);
    tn += rc;
    if (rc < sz - n)
      n += rc;
...
static const char *str_acl_alg(enum rte_acl_classify_alg alg) {
  uint32_t i;
...
static enum rte_acl_classify_alg parse_acl_alg(const char *alg) {
  uint32_t i;
...
static struct rte_acl_ctx *setup_acl(struct rte_acl_rule *route_base,
...
static int app_acl_init(void) {
  ...
  for (i = 0; i < NB_SOCKETS; i++) {
    if (acl_config.mapped[i]) {
      acl_config.acx_ipv4[i] = setup_acl(route_base_ipv4, acl_base_ipv4,
                                         route_num_ipv4, acl_num_ipv4, 0, i);

      acl_config.acx_ipv6[i] = setup_acl(route_base_ipv6, acl_base_ipv6,
    ...
    for (i = 0; i < qconf->n_rx_queue; ++i) {
      ...
      if (nb_rx > 0) {
        ...
        if (acl_search.num_ipv4) {
          rte_acl_classify(acl_config.acx_ipv4[socketid], acl_search.data_ipv4,
                           acl_search.res_ipv4, acl_search.num_ipv4,
                           DEFAULT_MAX_CATEGORIES);

          send_packets(acl_search.m_ipv4, acl_search.res_ipv4,
                       acl_search.num_ipv4);
        }

        if (acl_search.num_ipv6) {
          rte_acl_classify(acl_config.acx_ipv6[socketid], acl_search.data_ipv6,
                           acl_search.res_ipv6, acl_search.num_ipv6,
                           DEFAULT_MAX_CATEGORIES);

          send_packets(acl_search.m_ipv6, acl_search.res_ipv6,
                       acl_search.num_ipv6);
        }
      }
    }
...
static void print_usage(const char *prgname) {
  ...
  printf("%s [EAL options] -- -p PORTMASK -P"
         ...
         "  --" OPT_MAX_PKT_LEN
         " PKTLEN: maximum packet length in decimal (64-9600)\n"
...
static const char *parse_eth_dest(const char *optarg) {
  ...
  if (cmdline_parse_etheraddr(NULL, port_end, &port_l2hdr[portid].dst_addr,
...
static int parse_args(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argvopt, "p:P", lgopts, &option_index)) !=
         EOF) {
 ...
 */
static void set_default_dest_mac(void) {
  uint32_t i;
