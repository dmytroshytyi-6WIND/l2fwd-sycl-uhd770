...
static struct { const char *rule_ipv4_name; } parm_config;
...
static __rte_noreturn void lcore_main(struct flow_classifier *cls_app) {
  uint16_t port;
  int ret;
  int i = 0;

  ret = rte_flow_classify_table_entry_delete(cls_app->cls, rules[7]);
  if (ret)
  ...
  for (;;) {
    ...
    RTE_ETH_FOREACH_DEV(port) {
      ...
      for (i = 0; i < MAX_NUM_CLASSIFY; i++) {
        if (rules[i]) {
          ret = rte_flow_classifier_query(cls_app->cls, bufs, nb_rx, rules[i],
                                          &classify_stats);
          if (ret)
            printf("rule [%d] query failed ret [%d]\n\n", i, ret);
          else {
            printf("rule[%d] count=%" PRIu64 "\n", i, ntuple_stats.counter1);
...
static int get_cb_field(char **in, uint32_t *fd, int base, unsigned long lim,
                        char dlm) {
  unsigned long val;
  char *end;
...
static int parse_ipv4_5tuple_rule(char *str,
  ...
  ret = parse_ipv4_net(in[CB_FLD_SRC_ADDR], &ntuple_filter->src_ip,
  ...
  ret = parse_ipv4_net(in[CB_FLD_DST_ADDR], &ntuple_filter->dst_ip,
...
static int add_classify_rule(struct rte_eth_ntuple_filter *ntuple_filter,
  ...
  ipv4_mask.hdr.src_addr = convert_depth_to_bitmask(ipv4_mask.hdr.src_addr);
  ipv4_mask.hdr.dst_addr = ntuple_filter->dst_ip_mask;
  ipv4_mask.hdr.dst_addr = convert_depth_to_bitmask(ipv4_mask.hdr.dst_addr);
  ...
  ret = rte_flow_classify_validate(cls_app->cls, &attr, pattern_ipv4_5tuple,
                                   actions, &error);
  if (ret) {
  ...
  rule = rte_flow_classify_table_entry_add(
      cls_app->cls, &attr, pattern_ipv4_5tuple, actions, &key_found, &error);
  if (rule == NULL) {
...
static int parse_args(int argc, char **argv) {
  ...
  int option_index;
  char *prgname = argv[0];
  static struct option lgopts[] = {{OPTION_RULE_IPV4, 1, 0, 0},
                                   {NULL, 0, 0, 0}};
  ...
  while ((opt = getopt_long(argc, argvopt, "", lgopts, &option_index)) != EOF) {
    ...
    /* long options */
    case 0:
      if (!strncmp(lgopts[option_index].name, OPTION_RULE_IPV4,
