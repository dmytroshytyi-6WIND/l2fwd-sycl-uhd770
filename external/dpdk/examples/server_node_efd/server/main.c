...
static void do_stats_display(void) {
  ...
  for (i = 0; i < info->num_ports; i++) {
    printf("Port %u - rx: %9" PRIu64 "\t"
           "tx: %9" PRIu64 "\n",
           (unsigned int)info->id[i], info->rx_stats.rx[i], port_tx[i]);
  }
  ...
  for (i = 0; i < num_nodes; i++) {
    ...
    printf("Node %2u - rx: %9llu, rx_drop: %9llu\n"
           ...
           i, rx, rx_drop, node_tx[i], node_tx_drop[i], filter->passed,
           filter->drop);
  }
 ...
 * repeatedly sleeps.
 */
static int sleep_lcore(__rte_unused void *dummy) {
...
/* Flush rx queue. 8< */
static void flush_rx_queue(uint16_t node) {
  uint16_t j;
  struct node *cl;
 ...
 */
static inline void enqueue_rx_packet(uint8_t node, struct rte_mbuf *buf) {
...
static void process_packets(uint32_t port_num __rte_unused,
  ...
  rte_efd_lookup_bulk(efd_table, socket_id, rx_count, (const void **)key_ptrs,
                      data);
  for (i = 0; i < rx_count; i++) {
...
static void do_packet_forwarding(void) {
  ...
  for (;;) {
    ...
    /* read a port */
    rx_count = rte_eth_rx_burst(info->id[port_num], 0, buf, PACKET_READ_SIZE);
