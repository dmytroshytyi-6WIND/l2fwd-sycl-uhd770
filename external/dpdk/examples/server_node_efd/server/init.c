...
static int init_mbuf_pools(void) {
  const unsigned int num_mbufs =
      (num_nodes * MBUFS_PER_NODE) + (info->num_ports * MBUFS_PER_PORT);
  ...
  printf("Creating mbuf pool '%s' [%u mbufs] ...\n", PKTMBUF_POOL_NAME,
         num_mbufs);
  pktmbuf_pool =
      rte_pktmbuf_pool_create(PKTMBUF_POOL_NAME, num_mbufs, MBUF_CACHE_SIZE, 0,
...
static int init_port(uint16_t port_num) {
  ...
  for (q = 0; q < tx_rings; q++) {
    retval = rte_eth_tx_queue_setup(port_num, q, tx_ring_size,
                                    rte_eth_dev_socket_id(port_num), &txconf);
    if (retval < 0)
      return retval;
  }
...
static int init_shm_rings(void) {
  ...
  nodes = rte_malloc("node details", sizeof(*nodes) * num_nodes, 0);
  if (nodes == NULL)
  ...
  for (i = 0; i < num_nodes; i++) {
    ...
    nodes[i].rx_q = rte_ring_create(q_name, ringsize, socket_id,
...
/* Create EFD table. 8< */
static void create_efd_table(void) {
...
static void populate_efd_table(void) {
  unsigned int i;
  int32_t ret;
  uint32_t ip_dst;
  uint8_t socket_id = rte_socket_id();
  ...
  for (i = 0; i < num_flows; i++) {
    ...
    ret = rte_efd_update(efd_table, socket_id, (void *)&ip_dst,
                         (efd_value_t)node_id);
    if (ret < 0)
      rte_exit(EXIT_FAILURE,
  ...
  for (count = 0; count <= MAX_CHECK_TIME; count++) {
    ...
    for (portid = 0; portid < port_num; portid++) {
      ...
      if (print_flag == 1) {
        ...
        printf("Port %d %s\n", info->id[portid], link_status_text);
        continue;
      }
...
int init(int argc, char *argv[]) {
  ...
  mz = rte_memzone_reserve(MZ_SHARED_INFO, sizeof(*info), rte_socket_id(),
                           NO_FLAGS);
  if (mz == NULL)
