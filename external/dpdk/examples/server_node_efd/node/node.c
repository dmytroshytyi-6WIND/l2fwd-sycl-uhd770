 ...
 */
static int parse_node_num(const char *node) {
  char *end = NULL;
  unsigned long temp;
...
static int parse_app_args(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argvopt, "n:", lgopts, &option_index)) !=
         EOF) {
    switch (opt) {
    case 'n':
...
static void configure_tx_buffer(uint16_t port_id, uint16_t size) {
  ...
  tx_buffer[port_id] =
      rte_zmalloc_socket("tx_buffer", RTE_ETH_TX_BUFFER_SIZE(size), 0,
  ...
  ret = rte_eth_tx_buffer_set_err_callback(
      tx_buffer[port_id], flush_tx_error_callback, (void *)(intptr_t)port_id);
  if (ret < 0)
 ...
 */
static void configure_output_ports(const struct shared_info *info) {
  int i;
...
static struct rte_hash *create_hash_table(const struct shared_info *info) {
  ...
  if (h == NULL)
    rte_exit(EXIT_FAILURE, "Problem creating the hash table for node %d\n",
             node_id);
  return h;
}

static void populate_hash_table(const struct rte_hash *h,
 ...
 * port it arrived on.
 */
static inline void transmit_packet(struct rte_mbuf *buf) {
  int sent;
...
static inline void handle_packets(struct rte_hash *h, struct rte_mbuf **bufs,
...
int main(int argc, char *argv[]) {
  ...
  for (;;) {
     ...
     */
    while (rx_pkts > 0 &&
           unlikely(rte_ring_dequeue_bulk(rx_ring, pkts, rx_pkts, NULL) == 0))
    ...
    if (unlikely(rx_pkts == 0)) {
      if (need_flush)
        for (port = 0; port < info->num_ports; port++) {
          sent =
              rte_eth_tx_buffer_flush(info->id[port], node_id, tx_buffer[port]);
          if (unlikely(sent))
