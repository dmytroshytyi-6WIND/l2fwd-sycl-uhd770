...
static uint32_t find_lo(uint32_t lcore_id) {
  uint32_t i;
...
static int parse_socket_arg(char *arg) {
  uint32_t nb_sockets;
  uint32_t lcore_id;
...
static int parse_config(char *q_arg) {
  struct lcore_option *lo;
  char s[256];
  const char *p, *p0 = q_arg;
  char *end;
  enum fieldnames { FLD_LCORE = 0, FLD_CID, FLD_QID, _NUM_FLD };
...
static void vhost_crypto_usage(const char *prgname) {
  printf("%s [EAL options] --\n"
         "  --%s <lcore>,SOCKET-FILE-PATH\n"
         "  --%s (lcore,cdev_id,queue_id)[,(lcore,cdev_id,queue_id)]\n"
         "  --%s: zero copy\n"
         "  --%s: guest polling\n",
         prgname, OPT_SOCKET_FILE, OPT_CONFIG, OPT_ZERO_COPY, OPT_POLLING);
}

static int vhost_crypto_parse_args(int argc, char **argv) {
  int opt, ret;
  char *prgname = argv[0];
  char **argvopt;
  int option_index;
  struct option lgopts[] = {
      ...
      {OPT_CONFIG, required_argument, NULL, OPT_CONFIG_NUM},
      {OPT_ZERO_COPY, no_argument, NULL, OPT_ZERO_COPY_NUM},
      {OPT_POLLING, no_argument, NULL, OPT_POLLING_NUM},
      {NULL, 0, 0, 0}};
  ...
  while ((opt = getopt_long(argc, argvopt, "", lgopts, &option_index)) != EOF) {
    ...
    case OPT_ZERO_COPY_NUM:
      options.zero_copy = RTE_VHOST_CRYPTO_ZERO_COPY_ENABLE;
      break;
...
static int new_device(int vid) {
  ...
  ret = rte_vhost_crypto_create(
      vid, info->cid, info->sess_pool, info->sess_priv_pool,
...
static int vhost_crypto_worker(void *arg) {
  ...
  for (i = 0; i < NB_VIRTIO_QUEUES; i++) {
    if (rte_crypto_op_bulk_alloc(info->cop_pool, RTE_CRYPTO_OP_TYPE_SYMMETRIC,
                                 ops[i], burst_size) < burst_size) {
  ...
  while (1) {
    for (i = 0; i < info->nb_vids; i++) {
      ...
      for (j = 0; j < NB_VIRTIO_QUEUES; j++) {
        ...
        fetched =
            rte_vhost_crypto_fetch_requests(info->vids[i], j, ops[j], to_fetch);
        info->nb_inflight_ops +=
            rte_cryptodev_enqueue_burst(info->cid, info->qid, ops[j], fetched);
        ...
        fetched = rte_cryptodev_dequeue_burst(
            ...
            RTE_MIN(burst_size, info->nb_inflight_ops));
        fetched = rte_vhost_crypto_finalize_requests(ops_deq[j], fetched,
                                                     callfds, &nb_callfds);
        ...
        rte_mempool_put_bulk(info->cop_pool, (void **)ops_deq[j], fetched);
      }
    }
  }
exit:
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < options.nb_los; i++) {
    ...
    info = rte_zmalloc_socket(NULL, sizeof(*info), RTE_CACHE_LINE_SIZE,
...
#define VHOST_CRYPTO_CDEV_NAME_AESNI_GCM_PMD crypto_aesni_gcm
    ...
    if (ret < 0) {
      RTE_LOG(ERR, USER1, "Failed to configure cryptodev %u", info->cid);
      goto error_exit;
    }

    snprintf(name, 127, "SESS_POOL_%u", lo->lcore_id);
    info->sess_pool = rte_cryptodev_sym_session_pool_create(
        name, SESSION_MAP_ENTRIES, 0, 0, 0,
        rte_lcore_to_socket_id(lo->lcore_id));

    snprintf(name, 127, "SESS_POOL_PRIV_%u", lo->lcore_id);
    info->sess_priv_pool = rte_mempool_create(
        name, SESSION_MAP_ENTRIES,
        rte_cryptodev_sym_get_private_session_size(info->cid), 64, 0, NULL,
        NULL, NULL, NULL, rte_lcore_to_socket_id(lo->lcore_id), 0);
    ...
    info->cop_pool = rte_crypto_op_pool_create(
        name, RTE_CRYPTO_OP_TYPE_SYMMETRIC, NB_MEMPOOL_OBJS, NB_CACHE_OBJS,
        VHOST_CRYPTO_MAX_IV_LEN, rte_lcore_to_socket_id(lo->lcore_id));
    ...
    for (j = 0; j < dev_info.max_nb_queue_pairs; j++) {
      ret = rte_cryptodev_queue_pair_setup(
          info->cid, j, &qp_conf, rte_lcore_to_socket_id(lo->lcore_id));
      if (ret < 0) {
  ...
  for (i = 0; i < options.nb_los; i++) {
    ...
    if (rte_eal_remote_launch(vhost_crypto_worker, info, lo->lcore_id) < 0) {
    ...
    for (j = 0; j < lo->nb_sockets; j++) {
      ...
      ret = rte_vhost_crypto_driver_start(lo->socket_files[j]);
      if (ret < 0) {
