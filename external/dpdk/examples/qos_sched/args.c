...
static const char usage[] = "                                                  "
                            ...
                            "                             \n"
                            "    --pfc \"RX PORT, TX PORT, RX LCORE, WT "
                            "LCORE\" : Packet flow configuration \n"
                            "           multiple pfc can be configured in "
                            "command line                      \n"
                            "                                                  "
                            "                             \n"
                            ...
                            "value is %u)                    \n"
                            "    --mnc I : main core index (default value is "
                            "%u)                            \n"
                            "    --rsz \"A, B, C\" :   Ring sizes              "
                            "                             \n"
                            "           A = Size (in number of buffer "
                            "descriptors) of each of the NIC RX    \n"
                            "               rings read by the I/O RX lcores "
                            "(default value is %u)           \n"
                            "           B = Size (in number of elements) of "
                            "each of the SW rings used by the\n"
                            "               I/O RX lcores to send packets to "
                            "worker lcores (default value is\n"
                            "               %u)                                "
                            "                             \n"
                            "           C = Size (in number of buffer "
                            "descriptors) of each of the NIC TX    \n"
                            "               rings written by worker lcores "
                            "(default value is %u)            \n"
                            "    --bsz \"A, B, C, D\": Burst sizes             "
                            "                             \n"
                            "           A = I/O RX lcore read burst size from "
                            "NIC RX (default value is %u)  \n"
                            "           B = I/O RX lcore write burst size to "
                            "output SW rings,               \n"
                            "               Worker lcore read burst size from "
                            "input SW rings,               \n"
                            "               QoS enqueue size (default value is "
                            "%u)                          \n"
                            "           C = QoS dequeue size (default value is "
                            "%u)                          \n"
                            "           D = Worker lcore write burst size to "
                            "NIC TX (default value is %u)   \n"
                            "    --msz M : Mempool size (in number of mbufs) "
                            "for each pfc (default %u)      \n"
                            "    --rth \"A, B, C\" :   RX queue threshold "
                            "parameters                        \n"
                            "           A = RX prefetch threshold (default "
                            "value is %u)                     \n"
                            "           B = RX host threshold (default value "
                            "is %u)                         \n"
                            "           C = RX write-back threshold (default "
                            "value is %u)                   \n"
                            "    --tth \"A, B, C\" :   TX queue threshold "
                            "parameters                        \n"
                            "           A = TX prefetch threshold (default "
                            "value is %u)                     \n"
                            "           B = TX host threshold (default value "
                            "is %u)                         \n"
                            "           C = TX write-back threshold (default "
                            "value is %u)                   \n"
...
static void app_usage(const char *prgname) {
  printf(usage, prgname, APP_INTERACTIVE_DEFAULT, app_main_core,
         ...
         MAX_PKT_RX_BURST, PKT_ENQUEUE, PKT_DEQUEUE, MAX_PKT_TX_BURST, NB_MBUF,
         RX_PTHRESH, RX_HTHRESH, RX_WTHRESH, TX_PTHRESH, TX_HTHRESH,
         TX_WTHRESH);
}
...
/* returns:
        ...
        -1 in case of error
*/
static int app_parse_opt_vals(const char *conf_str, char separator,
                              uint32_t n_vals, uint32_t *opt_vals) {
  char *string;
  int i, n_tokens;
  char *tokens[MAX_OPT_VALUES];

  if (conf_str == NULL || opt_vals == NULL || n_vals == 0 ||
      n_vals > MAX_OPT_VALUES)
    return -1;
  ...
  n_tokens =
      rte_strsplit(string, strnlen(string, 32), tokens, n_vals, separator);
...
int app_parse_args(int argc, char **argv) {
  ...
  static struct option lgopts[] = {{OPT_PFC, 1, NULL, OPT_PFC_NUM},
