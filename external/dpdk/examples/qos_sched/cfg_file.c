...
int cfg_load_pipe(struct rte_cfgfile *cfg,
  ...
  profiles =
      rte_cfgfile_num_sections(cfg, "pipe profile", sizeof("pipe profile") - 1);
  ...
  for (j = 0; j < profiles; j++) {
    ...
    entry =
        rte_cfgfile_get_entry(cfg, pipe_name, "tc 12 oversubscription weight");
    if (entry)
    ...
    if (entry) {
      for (i = 0; i < RTE_SCHED_BE_QUEUES_PER_PIPE; i++) {
        pipe_params[j].wrr_weights[i] = (uint8_t)strtol(entry, &next, 10);
        if (next == NULL)
          break;
        entry = next;
      }
...
int cfg_load_subport(struct rte_cfgfile *cfg,
  ...
  if (rte_cfgfile_has_section(cfg, "pie")) {
    ...
    for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++) {
      ...
      if (entry)
        cman_params.pie_params[i].qdelay_ref = (uint16_t)atoi(entry);

      /* Parse Max Burst value */
      snprintf(str, sizeof(str), "tc %d max burst", i);
      entry = rte_cfgfile_get_entry(cfg, "pie", str);
      if (entry)
        cman_params.pie_params[i].max_burst = (uint16_t)atoi(entry);

      /* Parse Update Interval Value */
      snprintf(str, sizeof(str), "tc %d update interval", i);
      entry = rte_cfgfile_get_entry(cfg, "pie", str);
      if (entry)
        cman_params.pie_params[i].dp_update_interval = (uint16_t)atoi(entry);

      /* Parse Tailq Threshold Value */
      snprintf(str, sizeof(str), "tc %d tailq th", i);
      entry = rte_cfgfile_get_entry(cfg, "pie", str);
      if (entry)
        cman_params.pie_params[i].tailq_th = (uint16_t)atoi(entry);
    }
  }
  ...
  for (i = 0; i < MAX_SCHED_SUBPORTS; i++) {
    ...
    if (rte_cfgfile_has_section(cfg, sec_name)) {
      entry =
          rte_cfgfile_get_entry(cfg, sec_name, "number of pipes per subport");
      if (entry)
        subport_params[i].n_pipes_per_subport_enabled = (uint32_t)atoi(entry);
      ...
      if (entry) {
        ...
        for (j = 0; j < RTE_SCHED_TRAFFIC_CLASS_BE; j++) {
          subport_params[i].qsize[j] = (uint16_t)strtol(entry, &next, 10);
        ...
        for (j = 0; j < RTE_SCHED_BE_QUEUES_PER_PIPE; j++) {
          active_queues[n_active_queues] = RTE_SCHED_TRAFFIC_CLASS_BE + j;
          n_active_queues++;
        }
      }
      ...
      for (j = 0; j < n_entries; j++) {
        if (strncmp("pipe", entries[j].name, sizeof("pipe") - 1) == 0) {
          ...
          n_tokens = rte_strsplit(&entries[j].name[sizeof("pipe")],
                                  strnlen(entries[j].name, CFG_NAME_LEN),
                                  tokens, 2, '-');
          ...
          for (k = begin; k <= end; k++) {
            ...
            snprintf(profile_name, sizeof(profile_name), "pipe profile %d",
                     profile);
