...
static int cryptodev_fips_validate_app_int(void) {
  struct rte_cryptodev_config conf = {rte_socket_id(), 1, 0};
  struct rte_cryptodev_qp_conf qp_conf = {128, NULL, NULL};
  struct rte_cryptodev_info dev_info;
  uint32_t sess_sz = rte_cryptodev_sym_get_private_session_size(env.dev_id);
  ...
  env.mpool = rte_pktmbuf_pool_create(
      "FIPS_MEMPOOL", nb_mbufs, 0, 0,
      sizeof(struct rte_mbuf) + RTE_PKTMBUF_HEADROOM + env.mbuf_data_room,
      rte_socket_id());
  if (!env.mpool)
    return ret;

  ret =
      rte_cryptodev_queue_pair_setup(env.dev_id, 0, &qp_conf, rte_socket_id());
  if (ret < 0)
    return ret;
  ...
  env.sess_priv_mpool =
      rte_mempool_create("FIPS_SESS_PRIV_MEMPOOL", 16, sess_sz, 0, 0, NULL,
                         NULL, NULL, NULL, rte_socket_id(), 0);
  if (!env.sess_priv_mpool)
    goto error_exit;

  env.op_pool = rte_crypto_op_pool_create(
      "FIPS_OP_POOL", RTE_CRYPTO_OP_TYPE_SYMMETRIC, 1, 0, 16, rte_socket_id());
  if (!env.op_pool)
    goto error_exit;
...
static void cryptodev_fips_validate_app_uninit(void) {
...
static int fips_test_one_file(void);

static int parse_cryptodev_arg(char *arg) {
...
static int parse_cryptodev_id_arg(char *arg) {
...
static void cryptodev_fips_validate_usage(const char *prgname) {
  ...
  printf("%s [EAL options] --\n"
         ...
         prgname, OPT_REQ_FILE_PATH, OPT_RSP_FILE_PATH, OPT_FOLDER,
         OPT_MBUF_DATAROOM, def_mbuf_seg_size, OPT_CRYPTODEV, OPT_CRYPTODEV_ID,
         OPT_CRYPTODEV_ST, OPT_CRYPTODEV_BK_ID, OPT_CRYPTODEV_BK_DIR_KEY);
}

static int cryptodev_fips_validate_parse_args(int argc, char **argv) {
  int opt, ret;
  char *prgname = argv[0];
  char **argvopt;
  int option_index;
  struct option lgopts[] = {
      {OPT_REQ_FILE_PATH, required_argument, NULL, OPT_REQ_FILE_PATH_NUM},
      {OPT_RSP_FILE_PATH, required_argument, NULL, OPT_RSP_FILE_PATH_NUM},
      {OPT_FOLDER, no_argument, NULL, OPT_FOLDER_NUM},
      {OPT_MBUF_DATAROOM, required_argument, NULL, OPT_MBUF_DATAROOM_NUM},
      {OPT_CRYPTODEV, required_argument, NULL, OPT_CRYPTODEV_NUM},
      {OPT_CRYPTODEV_ID, required_argument, NULL, OPT_CRYPTODEV_ID_NUM},
      {OPT_CRYPTODEV_ST, no_argument, NULL, OPT_CRYPTODEV_ST_NUM},
      {OPT_CRYPTODEV_BK_ID, required_argument, NULL, OPT_CRYPTODEV_BK_ID_NUM},
      {OPT_CRYPTODEV_BK_DIR_KEY, required_argument, NULL,
       OPT_CRYPTODEV_BK_DIR_KEY_NUM},
      {NULL, 0, 0, 0}};
  ...
  while ((opt = getopt_long(argc, argvopt, "s:", lgopts, &option_index)) !=
         EOF) {
    ...
    case OPT_CRYPTODEV_BK_ID_NUM:
      if (!env.broken_test_config) {
        env.broken_test_config =
            rte_malloc(NULL, sizeof(*env.broken_test_config), 0);
        if (!env.broken_test_config)
          return -ENOMEM;

        env.broken_test_config->expect_fail_dir = self_test_dir_enc_auth_gen;
      }

      if (parser_read_uint32(&env.broken_test_config->expect_fail_test_idx,
                             optarg) < 0) {
    ...
    case OPT_CRYPTODEV_BK_DIR_KEY_NUM:
      if (!env.broken_test_config) {
        ...
        env.broken_test_config->expect_fail_test_idx = 0;
      }

      if (strcmp(optarg, "enc") == 0)
        env.broken_test_config->expect_fail_dir = self_test_dir_enc_auth_gen;
      else if (strcmp(optarg, "dec") == 0)
        env.broken_test_config->expect_fail_dir = self_test_dir_dec_auth_verify;
      else {
        rte_free(env.broken_test_config);
        cryptodev_fips_validate_usage(prgname);
        return -EINVAL;
    ...
    case OPT_MBUF_DATAROOM_NUM:
      if (parser_read_uint16(&env.mbuf_data_room, optarg) < 0) {
        cryptodev_fips_validate_usage(prgname);
        return -EINVAL;
      }
...
int main(int argc, char *argv[]) {
  ...
  if (!env.is_path_folder) {
    ...
    while ((dir = readdir(d_req)) != NULL) {
      ...
      snprintf(req_path, 1023, "%s/%s", env.req_path, dir->d_name);
      snprintf(rsp_path, 1023, "%s/%s", env.rsp_path, dir->d_name);
...
static int prepare_data_mbufs(struct fips_val *val) {
...
static int prepare_cipher_op(void) {
...
int prepare_auth_op(void) {
  ...
  env.digest =
      rte_zmalloc(NULL, vec.cipher_auth.digest.len, RTE_CACHE_LINE_SIZE);
  if (!env.digest) {
  ...
  if (info.op == FIPS_TEST_DEC_AUTH_VERIF)
    memcpy(env.digest, vec.cipher_auth.digest.val, vec.cipher_auth.digest.len);
...
int prepare_aead_op(void) {
  ...
  if (info.op == FIPS_TEST_ENC_AUTH_GEN) {
    ...
    rte_free(env.digest);
    env.digest = rte_zmalloc(NULL, vec.aead.digest.len, RTE_CACHE_LINE_SIZE);
    if (!env.digest) {
      RTE_LOG(ERR, USER1, "Not enough memory\n");
      return -ENOMEM;
    }
    ...
    sym->aead.digest.phys_addr = rte_malloc_virt2iova(sym->aead.digest.data);
  }
...
static int prepare_aes_xform(struct rte_crypto_sym_xform *xform) {
  ...
  if (rte_cryptodev_sym_capability_check_cipher(cap, cipher_xform->key.length,
    ...
    RTE_LOG(ERR, USER1, "PMD %s key length %u IV length %u\n", info.device_name,
            cipher_xform->key.length, cipher_xform->iv.length);
    return -EPERM;
  }
...
static int prepare_tdes_xform(struct rte_crypto_sym_xform *xform) {
  const struct rte_cryptodev_symmetric_capability *cap;
  struct rte_cryptodev_sym_capability_idx cap_idx;
  struct rte_crypto_cipher_xform *cipher_xform = &xform->cipher;
...
static int prepare_hmac_xform(struct rte_crypto_sym_xform *xform) {
  const struct rte_cryptodev_symmetric_capability *cap;
  struct rte_cryptodev_sym_capability_idx cap_idx;
  ...
  if (rte_cryptodev_sym_capability_check_auth(
    ...
    RTE_LOG(ERR, USER1, "PMD %s key length %u IV length %u\n", info.device_name,
            auth_xform->key.length, auth_xform->digest_length);
    return -EPERM;
  }
...
int prepare_gcm_xform(struct rte_crypto_sym_xform *xform) {
  ...
  if (rte_cryptodev_sym_capability_check_aead(
    ...
    RTE_LOG(ERR, USER1, "PMD %s key_len %u tag_len %u aad_len %u iv_len %u\n",
            info.device_name, aead_xform->key.length, aead_xform->digest_length,
            aead_xform->aad_length, aead_xform->iv.length);
    return -EPERM;
  }
...
int prepare_gmac_xform(struct rte_crypto_sym_xform *xform) {
  ...
  if (rte_cryptodev_sym_capability_check_auth(cap, auth_xform->key.length,
                                              auth_xform->digest_length,
                                              auth_xform->iv.length) != 0) {

    RTE_LOG(ERR, USER1, "PMD %s key length %u Digest length %u IV length %u\n",
            info.device_name, auth_xform->key.length, auth_xform->digest_length,
...
static int prepare_cmac_xform(struct rte_crypto_sym_xform *xform) {
  const struct rte_cryptodev_symmetric_capability *cap;
  struct rte_cryptodev_sym_capability_idx cap_idx;
  struct rte_crypto_auth_xform *auth_xform = &xform->auth;
...
static int prepare_ccm_xform(struct rte_crypto_sym_xform *xform) {
  const struct rte_cryptodev_symmetric_capability *cap;
  struct rte_cryptodev_sym_capability_idx cap_idx;
  struct rte_crypto_aead_xform *aead_xform = &xform->aead;
...
static int prepare_sha_xform(struct rte_crypto_sym_xform *xform) {
  const struct rte_cryptodev_symmetric_capability *cap;
  struct rte_cryptodev_sym_capability_idx cap_idx;
  struct rte_crypto_auth_xform *auth_xform = &xform->auth;
...
static int prepare_xts_xform(struct rte_crypto_sym_xform *xform) {
  const struct rte_cryptodev_symmetric_capability *cap;
  struct rte_cryptodev_sym_capability_idx cap_idx;
  struct rte_crypto_cipher_xform *cipher_xform = &xform->cipher;
...
static int get_writeback_data(struct fips_val *val) {
...
static int fips_generic_test(void) {
...
static int fips_mct_tdes_test(void) {
...
#define TDES_INTERN_ITER 10000
  ...
  for (i = 0; i < TDES_EXTERN_ITER; i++) {
      ...
      if (j == 0) {
        ...
        if (info.op == FIPS_TEST_ENC_AUTH_GEN) {
          if (test_mode == TDES_MODE_ECB) {
            ...
            memcpy(vec.pt.val, vec.iv.val, TDES_BLOCK_SIZE);
    ...
    for (k = 0; k < 24; k++)
      val_key.val[k] = (__builtin_popcount(val_key.val[k]) & 0x1)
                           ? val_key.val[k]
                           : (val_key.val[k] ^ 0x1);
...
static int fips_mct_aes_ecb_test(void) {
...
static int fips_mct_aes_test(void) {
#define AES_BLOCK_SIZE 16
#define AES_EXTERN_ITER 100
#define AES_INTERN_ITER 1000
  struct fips_val val = {NULL, 0}, val_key;
...
static int fips_mct_sha_test(void) {
...
#define SHA_MD_BLOCK 3
  ...
  for (j = 0; j < SHA_EXTERN_ITER; j++) {
    ...
    for (i = 0; i < (SHA_INTERN_ITER); i++) {
      ...
      memcpy((vec.pt.val + md[0].len + md[1].len), md[2].val,
             (size_t)md[2].len);
...
static int init_test_ops(void) {
  ...
  case FIPS_TEST_ALGO_AES_XTS:
    ...
    break;
  default:
    if (strstr(info.file_name, "TECB") || strstr(info.file_name, "TCBC")) {
...
static void print_test_block(void) {
  uint32_t i;
