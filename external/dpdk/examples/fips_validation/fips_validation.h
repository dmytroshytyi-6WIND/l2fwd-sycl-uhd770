...
#define DEF_MBUF_SEG_SIZE                                                      \
  (UINT16_MAX - sizeof(struct rte_mbuf) - RTE_PKTMBUF_HEADROOM)
...
enum file_types { FIPS_TYPE_REQ = 1, FIPS_TYPE_FAX, FIPS_TYPE_RSP };
...
enum fips_tdes_test_mode { TDES_MODE_CBC = 0, TDES_MODE_ECB };
...
enum fips_sha_test_types { SHA_KAT = 0, SHA_MCT };
...
int parse_test_aes_init(void);

int parse_test_tdes_init(void);

int parse_test_hmac_init(void);

int parse_test_gcm_init(void);

int parse_test_cmac_init(void);

int parse_test_ccm_init(void);

int parse_test_sha_init(void);
