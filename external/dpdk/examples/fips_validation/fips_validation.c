...
int fips_test_fetch_one_block(void) {
  size_t size;
  int ret = 0;
  uint32_t i;
...
static void fips_test_parse_version(void) {
...
static int fips_test_parse_header(void) {
  uint32_t i;
  char *tmp;
  int ret;
  int algo_parsed = 0;
  ...
  for (i = 0; i < info.nb_vec_lines; i++) {
    ...
    if (tmp != NULL) {
      ...
      fprintf(info.fp_wr, "%s%s\n",
              "# Config Info for DPDK Cryptodev : ", info.device_name);
      continue;
    }
    ...
    if (tmp != NULL) {
      ...
      fprintf(info.fp_wr, "%s%s%s\n", tmp_output,
              "information for DPDK Cryptodev ", info.device_name);
      continue;
    }
    ...
    if (tmp != NULL) {
      ...
      fprintf(info.fp_wr, "%s%s%s\n", tmp_output,
              "test information for DPDK Cryptodev ", info.device_name);
      continue;
    }
...
static int parse_file_type(const char *path) {
...
int fips_test_init(const char *req_file_path, const char *rsp_file_path,
  ...
  if (rte_strscpy(info.file_name, req_file_path, sizeof(info.file_name)) < 0) {
  ...
  if (rte_strscpy(info.device_name, device_name, sizeof(info.device_name)) <
      0) {
...
int fips_test_parse_one_case(void) {
  uint32_t i, j = 0;
  uint32_t is_interim;
  ...
  if (info.interim_callbacks) {
    for (i = 0; i < info.nb_vec_lines; i++) {
      ...
      for (j = 0; info.interim_callbacks[j].key != NULL; j++)
        if (strstr(info.vec[i], info.interim_callbacks[j].key)) {
          is_interim = 1;

          ret = info.interim_callbacks[j].cb(info.interim_callbacks[j].key,
                                             info.vec[i],
  ...
  for (i = info.vec_start_off; i < info.nb_vec_lines; i++) {
    for (j = 0; info.callbacks[j].key != NULL; j++)
      if (strstr(info.vec[i], info.callbacks[j].key)) {
        ret = info.callbacks[j].cb(info.callbacks[j].key, info.vec[i],
                                   info.callbacks[j].val);
        if (ret < 0)
          return ret;
        break;
      }
...
void fips_test_write_one_case(void) {
  uint32_t i;
...
static int parser_read_uint64_hex(uint64_t *value, const char *p) {
  char *next;
  uint64_t val;
...
int parse_uint8_known_len_hex_str(const char *key, char *src,
                                  struct fips_val *val) {
...
int parse_uint8_hex_str(const char *key, char *src, struct fips_val *val) {
  uint32_t len, j;
...
int parser_read_uint32_val(const char *key, char *src, struct fips_val *val) {
...
int parser_read_uint32_bit_val(const char *key, char *src,
                               struct fips_val *val) {
  int ret;
...
int writeback_hex_str(const char *key, char *dst, struct fips_val *val) {
  char *str = dst;
  uint32_t len;
...
int parser_read_uint32(uint32_t *value, char *p) {
  uint64_t val = 0;
...
int update_info_vec(uint32_t count) {
