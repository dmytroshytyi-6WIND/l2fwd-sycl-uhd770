...
static int writeback_tdes_hex_str(const char *key, char *dst,
                                  struct fips_val *val);

static int parse_tdes_uint8_hex_str(const char *key, char *src,
                                    struct fips_val *val);

static int parse_tdes_interim(const char *key, char *text,
                              struct fips_val *val);
...
static int parse_test_tdes_writeback(struct fips_val *val) {
...
static int rsp_test_tdes_check(struct fips_val *val) {
  ...
  for (i = 0; i < info.nb_vec_lines; i++) {
    ...
    for (j = 0; j < RTE_DIM(test_types); j++)
      if (strstr(line, test_types[j].desc)) {
        info.interim_info.tdes_data.test_type = test_types[j].type;
        if (strstr(line, TEST_TYPE_ECB_KEY))
          info.interim_info.tdes_data.test_mode = TDES_MODE_ECB;
        else
          info.interim_info.tdes_data.test_mode = TDES_MODE_CBC;
        break;
      }
  }
