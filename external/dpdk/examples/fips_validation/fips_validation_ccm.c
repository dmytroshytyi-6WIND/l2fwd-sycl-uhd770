...
static int parser_dvpt_interim(const char *key, char *src,
                               struct fips_val *val) {
  char *tmp, c, value[10];
...
static int parse_dvpt_ct_hex_str(const char *key, char *src,
                                 struct fips_val *val) {
  int ret;
...
static int parse_uint8_ccm_aad_str(const char *key, char *src,
                                   struct fips_val *val) {
...
static int parse_test_ccm_writeback(struct fips_val *val) {
  ...
  for (i = 0; i < info.nb_vec_lines; i++) {
    ...
    for (j = 0; j < RTE_DIM(ctt); j++)
      if (strstr(line, ctt[j].str)) {
        info.interim_info.ccm_data.test_type = ctt[j].type;
