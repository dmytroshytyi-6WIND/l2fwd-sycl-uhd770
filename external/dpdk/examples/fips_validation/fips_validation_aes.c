...
static int parse_interim_enc_dec(const char *key, __rte_unused char *text,
...
static int parse_test_aes_writeback(struct fips_val *val) {
...
static int rsp_test_aes_check(struct fips_val *val) {
...
int parse_test_aes_init(void) {
  ...
  for (i = 0; i < info.nb_vec_lines; i++) {
    ...
    if (tmp) {
      for (j = 0; j < RTE_DIM(aes_test_types); j++)
        if (strstr(line, aes_test_types[j].desc)) {
          info.interim_info.aes_data.test_type = aes_test_types[j].type;
          break;
        }
      ...
      for (j = 0; j < RTE_DIM(algo_con); j++)
        if (strcmp(algo_con[j].name, tmp) == 0) {
          info.interim_info.aes_data.cipher_algo = (uint32_t)algo_con[j].algo;
          break;
        }
    ...
    if (tmp) {
      ...
      if (parser_read_uint32(&info.interim_info.aes_data.key_len, tmp) < 0)
        return -EINVAL;
