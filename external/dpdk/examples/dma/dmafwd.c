...
static void print_port_stats(uint16_t port_id) {
  printf("\nStatistics for port %u ------------------------------"
         ...
         port_id, port_statistics.tx[port_id], port_statistics.rx[port_id],
         port_statistics.tx_dropped[port_id],
         port_statistics.copy_dropped[port_id]);
}

/* Print out statistics for one dmadev device. */
static void print_dmadev_stats(uint32_t dev_id, struct rte_dma_stats stats) {
...
static void print_total_stats(struct total_statistics *ts) {
...
static void print_stats(char *prgname) {
  ...
  status_strlen =
      snprintf(status_string, sizeof(status_string), "%s, ", prgname);
  status_strlen += snprintf(
      status_string + status_strlen, sizeof(status_string) - status_strlen,
      "Worker Threads = %d, ", rte_lcore_count() > 2 ? 2 : 1);
  status_strlen +=
      snprintf(status_string + status_strlen,
               sizeof(status_string) - status_strlen, "Copy Mode = %s,\n",
               copy_mode == COPY_MODE_SW_NUM ? COPY_MODE_SW : COPY_MODE_DMA);
  status_strlen += snprintf(
      status_string + status_strlen, sizeof(status_string) - status_strlen,
      "Updating MAC = %s, ", mac_updating ? "enabled" : "disabled");
  ...
  while (!force_quit) {
    ...
    for (i = 0; i < cfg.nb_ports; i++) {
      ...
      delta_ts.total_packets_dropped += port_statistics.tx_dropped[port_id] +
                                        port_statistics.copy_dropped[port_id];
      delta_ts.total_packets_tx += port_statistics.tx[port_id];
      delta_ts.total_packets_rx += port_statistics.rx[port_id];
...
static void update_mac_addrs(struct rte_mbuf *m, uint32_t dest_portid) {
...
static inline void pktmbuf_metadata_copy(const struct rte_mbuf *src,
                                         struct rte_mbuf *dst) {
...
/* Copy packet data */
static inline void pktmbuf_sw_copy(struct rte_mbuf *src, struct rte_mbuf *dst) {
  rte_memcpy(rte_pktmbuf_mtod(dst, char *), rte_pktmbuf_mtod(src, char *),
             src->data_len);
}
/* >8 End of perform packet copy there is a user-defined function. */

static uint32_t dma_enqueue_packets(struct rte_mbuf *pkts[],
                                    struct rte_mbuf *pkts_copy[],
  ...
  for (i = 0; i < nb_rx; i++) {
    /* Perform data copy */
    ret = rte_dma_copy(dev_id, 0, rte_pktmbuf_iova(pkts[i]),
...
static inline uint32_t dma_enqueue(struct rte_mbuf *pkts[],
                                   struct rte_mbuf *pkts_copy[], uint32_t num,
                                   uint32_t step, uint16_t dev_id) {
  uint32_t i, k, m, n;
...
static inline uint32_t dma_dequeue(struct rte_mbuf *src[],
                                   struct rte_mbuf *dst[], uint32_t num,
                                   uint16_t dev_id) {
  struct dma_bufs *dma = &dma_bufs[dev_id];
...
static void dma_rx_port(struct rxtx_port_config *rx_config) {
  int32_t ret;
  ...
  for (i = 0; i < rx_config->nb_queues; i++) {
    ...
    nb_rx =
        rte_eth_rx_burst(rx_config->rxtx_port, i, pkts_burst, MAX_PKT_BURST);
    ...
    ret =
        rte_mempool_get_bulk(dma_pktmbuf_pool, (void *)pkts_burst_copy, nb_rx);

    if (unlikely(ret < 0))
      rte_exit(EXIT_FAILURE, "Unable to allocate memory.\n");

    for (j = 0; j < nb_rx; j++)
      pktmbuf_metadata_copy(pkts_burst[j], pkts_burst_copy[j]);

    if (copy_mode == COPY_MODE_DMA_NUM) {
      /* enqueue packets for  hardware copy */
      nb_enq = dma_enqueue(pkts_burst, pkts_burst_copy, nb_rx, dma_batch_sz,
                           rx_config->dmadev_ids[i]);

      /* free any not enqueued packets. */
      rte_mempool_put_bulk(dma_pktmbuf_pool, (void *)&pkts_burst[nb_enq],
                           nb_rx - nb_enq);
      rte_mempool_put_bulk(dma_pktmbuf_pool, (void *)&pkts_burst_copy[nb_enq],
                           nb_rx - nb_enq);

      port_statistics.copy_dropped[rx_config->rxtx_port] += (nb_rx - nb_enq);

      /* get completed copies */
      nb_rx = dma_dequeue(pkts_burst, pkts_burst_copy, MAX_PKT_BURST,
                          rx_config->dmadev_ids[i]);
    } else {
      /* Perform packet software copy, free source packets */
      for (j = 0; j < nb_rx; j++)
        pktmbuf_sw_copy(pkts_burst[j], pkts_burst_copy[j]);
    }
...
static void dma_tx_port(struct rxtx_port_config *tx_config) {
  ...
  for (i = 0; i < tx_config->nb_queues; i++) {
    ...
    nb_dq = rte_ring_dequeue_burst(tx_config->rx_to_tx_ring, (void *)mbufs,
                                   MAX_PKT_BURST, NULL);
    if (nb_dq == 0)
      continue;
    ...
    if (mac_updating) {
      for (j = 0; j < nb_dq; j++)
        update_mac_addrs(mbufs[j], tx_config->rxtx_port);
    }

    nb_tx = rte_eth_tx_burst(tx_config->rxtx_port, 0, (void *)mbufs, nb_dq);

    port_statistics.tx[tx_config->rxtx_port] += nb_tx;

    /* Free any unsent packets. */
    if (unlikely(nb_tx < nb_dq))
      rte_mempool_put_bulk(dma_pktmbuf_pool, (void *)&mbufs[nb_tx],
                           nb_dq - nb_tx);
  }
}
...
static void rxtx_main_loop(void) {
  uint16_t i;
  uint16_t nb_ports = cfg.nb_ports;
...
/* Display usage */
static void dma_usage(const char *prgname) {
  printf(
      ...
      "  -s --ring-size RS: size of dmadev descriptor ring for hardware copy "
      "mode or rte_ring for software copy mode\n"
      "  -i --stats-interval SI: interval, in seconds, between stats prints "
      "(default is 1)\n",
      prgname);
}

static int dma_parse_portmask(const char *portmask) {
  char *end = NULL;
  unsigned long pm;
...
static copy_mode_t dma_parse_copy_mode(const char *copy_mode) {
...
static int dma_parse_args(int argc, char **argv, unsigned int nb_ports) {
  ...
  while ((opt = getopt_long(argc, argvopt, short_options, lgopts,
    ...
    case 's':
      ...
      if (ring_size > MBUF_RING_SIZE) {
        printf("Max ring_size is %d, setting ring_size to max", MBUF_RING_SIZE);
...
static void configure_dmadev_queue(uint32_t dev_id) {
...
static void assign_dmadevs(void) {
  ...
  if (nb_dmadev < cfg.nb_ports * cfg.ports[0].nb_queues)
    rte_exit(EXIT_FAILURE, "Not enough dmadevs (%u) for all queues (%u).\n",
...
static void assign_rings(void) {
  ...
  for (i = 0; i < cfg.nb_ports; i++) {
    ...
    cfg.ports[i].rx_to_tx_ring = rte_ring_create(
        ring_name, ring_size, rte_socket_id(), RING_F_SP_ENQ | RING_F_SC_DEQ);
 ...
 */
static inline void port_init(uint16_t portid, struct rte_mempool *mbuf_pool,
                             uint16_t nb_queues) {
  ...
  for (i = 0; i < nb_queues; i++) {
    ret = rte_eth_rx_queue_setup(
        portid, i, nb_rxd, rte_eth_dev_socket_id(portid), &rxq_conf, mbuf_pool);
    if (ret < 0)
      rte_exit(EXIT_FAILURE,
  ...
  tx_buffer[portid] =
      rte_zmalloc_socket("tx_buffer", RTE_ETH_TX_BUFFER_SIZE(MAX_PKT_BURST), 0,
...
int main(int argc, char **argv) {
  ...
  nb_mbufs = RTE_MAX(
      nb_ports *
          (nb_queues * (nb_rxd + nb_txd + 4 * MAX_PKT_BURST + ring_size) +
           ring_size + rte_lcore_count() * MEMPOOL_CACHE_SIZE),
      MIN_POOL_SIZE);

  /* Create the mbuf pool */
  sz = max_frame_size + RTE_PKTMBUF_HEADROOM;
  sz = RTE_MAX(sz, (size_t)RTE_MBUF_DEFAULT_BUF_SIZE);
  dma_pktmbuf_pool = rte_pktmbuf_pool_create(
      "mbuf_pool", nb_mbufs, MEMPOOL_CACHE_SIZE, 0, sz, rte_socket_id());
  ...
  if (cfg.nb_lcores < 1)
    rte_exit(EXIT_FAILURE, "There should be at least one worker lcore.\n");
