 ...
 */
static unsigned int get_previous_lcore_id(unsigned int id) {
  int i;
...
static inline void pktmbuf_free_bulk(struct rte_mbuf *mbuf_table[],
                                     unsigned n) {
  unsigned int i;
...
static inline int free_tx_buffers(struct rte_eth_dev_tx_buffer *tx_buffer[]) {
  uint16_t port_id;
...
configure_tx_buffers(struct rte_eth_dev_tx_buffer *tx_buffer[]) {
  ...
  RTE_ETH_FOREACH_DEV(port_id) {
    ...
    tx_buffer[port_id] =
        rte_zmalloc_socket("tx_buffer", RTE_ETH_TX_BUFFER_SIZE(MAX_PKTS_BURST),
...
static inline int configure_eth_port(uint16_t port_id) {
...
static void print_stats(void) {
  ...
  for (lcore_id = 0; lcore_id <= end_w_lcore_id; lcore_id++) {
    if (insight_worker && rte_lcore_is_enabled(lcore_id) &&
    ...
    app_stats.wkr.enqueue_failed_pkts += wkr_stats[lcore_id].enq_failed_pkts;
  }
 ...
 * ring.
 */
static int rx_thread(struct rte_ring *ring_out) {
  uint32_t seqn = 0;
  uint16_t i, ret = 0;
  uint16_t nb_rx_pkts;
  uint16_t port_id;
  ...
  while (!quit_signal) {
    ...
    RTE_ETH_FOREACH_DEV(port_id) {
      if ((portmask & (1 << port_id)) != 0) {
        ...
        if (nb_rx_pkts == 0) {
          RTE_LOG_DP(DEBUG, REORDERAPP, "%s():Received zero packets\n",
                     __func__);
          continue;
        }
        ...
        if (unlikely(ret < nb_rx_pkts)) {
          app_stats.rx.enqueue_failed_pkts += (nb_rx_pkts - ret);
 ...
 * transmitting.
 */
static int send_thread(struct send_thread_args *args) {
  int ret;
  unsigned int i, dret;
  ...
  while (!quit_signal) {
    ...
    nb_dq_mbufs = rte_ring_dequeue_burst(args->ring_in, (void *)mbufs,
                                         MAX_PKTS_BURST, NULL);
 ...
 */
static int tx_thread(struct rte_ring *ring_in) {
  uint32_t i, dqnum;
  uint8_t outp;
  unsigned sent;
  struct rte_mbuf *mbufs[MAX_PKTS_BURST];
  ...
  while (!quit_signal) {
    ...
    /* deque the mbufs from workers_to_tx ring */
    dqnum =
        rte_ring_dequeue_burst(ring_in, (void *)mbufs, MAX_PKTS_BURST, NULL);
...
int main(int argc, char **argv) {
  ...
  mbuf_pool =
      rte_pktmbuf_pool_create("mbuf_pool", MBUF_PER_POOL, MBUF_POOL_CACHE_SIZE,
  ...
  if (!disable_reorder) {
    send_args.buffer =
        rte_reorder_create("PKT_RO", rte_socket_id(), REORDER_BUFFER_SIZE);
  ...
  for (lcore_id = 0; lcore_id <= get_previous_lcore_id(last_lcore_id);
    ...
    if (rte_lcore_is_enabled(lcore_id) && lcore_id != main_lcore_id)
      rte_eal_remote_launch(worker_thread, (void *)&worker_args, lcore_id);
  ...
  if (disable_reorder) {
    ...
    rte_eal_remote_launch((lcore_function_t *)send_thread, (void *)&send_args,
                          last_lcore_id);
  }
