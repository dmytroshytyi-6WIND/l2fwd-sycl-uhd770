...
static inline int get_eth_conf(struct rte_eth_conf *eth_conf) {
  ...
  (void)(rte_memcpy(&eth_conf->rx_adv_conf.vmdq_dcb_conf, &conf, sizeof(conf)));
...
static inline int port_init(uint16_t port, struct rte_mempool *mbuf_pool) {
  ...
  for (q = 0; q < num_queues; q++) {
    retval = rte_eth_rx_queue_setup(
        port, q, rxRingSize, rte_eth_dev_socket_id(port), NULL, mbuf_pool);
    if (retval < 0) {
...
static int vmdq_parse_num_tcs(const char *q_arg) {
  char *end = NULL;
  int n;
  ...
  for (;;) {
    ...
    for (p = 0; p < num_ports; p++) {
      ...
      for (q = startQueue; q < endQueue; q++) {
        const uint16_t rxCount = rte_eth_rx_burst(src, q, buf, buf_size);
        ...
        const uint16_t txCount = rte_eth_tx_burst(dst, q, buf, rxCount);
        if (txCount != rxCount) {
