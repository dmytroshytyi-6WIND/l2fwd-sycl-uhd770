...
static __rte_always_inline void worker_fwd_event(struct rte_event *ev,
                                                 uint8_t sched) {
...
static __rte_always_inline void worker_event_enqueue(const uint8_t dev,
                                                     const uint8_t port,
                                                     struct rte_event *ev) {
...
worker_event_enqueue_burst(const uint8_t dev, const uint8_t port,
  ...
  while (enq < nb_rx) {
    enq += rte_event_enqueue_burst(dev, port, ev + enq, nb_rx - enq);
  }
}
...
static int worker_do_tx_single(void *arg) {
...
static int worker_do_tx_single_atq(void *arg) {
  struct worker_data *data = (struct worker_data *)arg;
  const uint8_t dev = data->dev_id;
  const uint8_t port = data->port_id;
  size_t fwd = 0, received = 0, tx = 0;
...
static int worker_do_tx_single_burst(void *arg) {
  ...
  while (!fdata->done) {
    uint16_t i;
    uint16_t nb_rx = rte_event_dequeue_burst(dev, port, ev, BATCH_SIZE, 0);
...
static int worker_do_tx_single_burst_atq(void *arg) {
  struct rte_event ev[BATCH_SIZE + 1];
...
static int worker_do_tx(void *arg) {
  struct rte_event ev;
  ...
  while (!fdata->done) {
    ...
    if (cq_id >= lst_qid) {
      ...
      worker_fwd_event(&ev, RTE_SCHED_TYPE_ATOMIC);
      ev.queue_id =
          (cq_id == lst_qid) ? cdata.next_qid[ev.queue_id] : ev.queue_id;
    } else {
...
static int worker_do_tx_atq(void *arg) {
  struct rte_event ev;
...
static int worker_do_tx_burst(void *arg) {
  ...
  while (!fdata->done) {
    ...
    const uint16_t nb_rx =
        rte_event_dequeue_burst(dev, port, ev, BATCH_SIZE, 0);
    ...
    for (i = 0; i < nb_rx; i++) {
      ...
      if (cq_id >= lst_qid) {
        ...
        ev[i].queue_id = (cq_id == lst_qid) ? cdata.next_qid[ev[i].queue_id]
                                            : ev[i].queue_id;
...
static int worker_do_tx_burst_atq(void *arg) {
  struct rte_event ev[BATCH_SIZE];
  ...
  while (!fdata->done) {
    ...
    for (i = 0; i < nb_rx; i++) {
      const uint8_t cq_id = ev[i].sub_event_type % cdata.num_stages;
...
static int setup_eventdev_worker_tx_enq(struct worker_data *worker_data) {
  uint8_t i;
  ...
  if (dev_info.max_event_port_dequeue_depth <
    ...
    config.nb_event_port_dequeue_depth = dev_info.max_event_port_dequeue_depth;
  if (dev_info.max_event_port_enqueue_depth <
      config.nb_event_port_enqueue_depth)
    config.nb_event_port_enqueue_depth = dev_info.max_event_port_enqueue_depth;
  ...
  for (i = 0; i < nb_queues; i++) {
    ...
    if (atq) {
      ...
      wkr_q_conf.event_queue_cfg = RTE_EVENT_QUEUE_CFG_ALL_TYPES;
    } else {
      uint8_t slot;

      nb_slots = cdata.num_stages + 1;
      slot = i % nb_slots;
      wkr_q_conf.schedule_type =
          slot == cdata.num_stages ? RTE_SCHED_TYPE_ATOMIC : cdata.queue_type;
    }
    ...
    if (cdata.enable_queue_priorities) {
      const uint32_t prio_delta = (RTE_EVENT_DEV_PRIORITY_LOWEST) / nb_slots;

      /* higher priority for queues closer to tx */
      wkr_q_conf.priority =
          RTE_EVENT_DEV_PRIORITY_LOWEST - prio_delta * (i % nb_slots);
    }
  ...
  ret = rte_event_dev_service_id_get(dev_id, &fdata->evdev_service_id);
  if (ret != -ESRCH && ret != 0) {
...
static int32_t service_rx_adapter(void *arg) {
  int i;
  struct rx_adptr_services *adptr_services = arg;

  for (i = 0; i < adptr_services->nb_rx_adptrs; i++)
    rte_service_run_iter_on_app_lcore(adptr_services->rx_adpt_arr[i], 1);
  return 0;
}
...
static int init_ports(uint16_t num_ports) {
  uint16_t portid;
  ...
  struct rte_mempool *mp = rte_pktmbuf_pool_create(
      ...
      /* cache_size */ 512,
      /* priv_size*/ 0,
      /* data_room_size */ RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
...
static void init_adapters(uint16_t nb_ports) {
  int i;
  int ret;
  uint8_t evdev_id = 0;
  ...
  if (adptr_p_conf.dequeue_depth > dev_info.max_event_port_dequeue_depth)
    adptr_p_conf.dequeue_depth = dev_info.max_event_port_dequeue_depth;
  if (adptr_p_conf.enqueue_depth > dev_info.max_event_port_enqueue_depth)
    adptr_p_conf.enqueue_depth = dev_info.max_event_port_enqueue_depth;
  ...
  for (i = 0; i < nb_ports; i++) {
    ...
    ret = rte_event_eth_rx_adapter_create(i, evdev_id, &adptr_p_conf);
    if (ret)
      rte_exit(EXIT_FAILURE, "failed to create rx adapter[%d]", i);

    ret = rte_event_eth_rx_adapter_caps_get(evdev_id, i, &cap);
    if (ret)
      rte_exit(EXIT_FAILURE, "failed to get event rx adapter "
                             "capabilities");

    queue_conf.ev.queue_id =
        cdata.rx_stride ? (i * cdata.rx_stride) : (uint8_t)cdata.qid[0];

    ret = rte_event_eth_rx_adapter_queue_add(i, i, -1, &queue_conf);
    if (ret)
      rte_exit(EXIT_FAILURE, "Failed to add queues to Rx adapter");

    /* Producer needs to be scheduled. */
    if (!(cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT)) {
      ret = rte_event_eth_rx_adapter_service_id_get(i, &service_id);
      if (ret != -ESRCH && ret != 0) {
        rte_exit(EXIT_FAILURE, "Error getting the service ID for rx adptr\n");
      }
      ...
      adptr_services->rx_adpt_arr =
          rte_realloc(adptr_services->rx_adpt_arr,
                      adptr_services->nb_rx_adptrs * sizeof(uint32_t), 0);
    ...
    if (ret)
      rte_exit(EXIT_FAILURE, "Rx adapter[%d] start failed", i);
  }
  ...
  for (i = 0; i < nb_ports; i++) {
    ret = rte_event_eth_tx_adapter_queue_add(cdata.tx_adapter_id, i, -1);
    if (ret)
      rte_exit(EXIT_FAILURE, "Failed to add queues to Tx adapter");
  }

  ret = rte_event_eth_tx_adapter_start(cdata.tx_adapter_id);
  if (ret)
    rte_exit(EXIT_FAILURE, "Tx adapter[%d] start failed", cdata.tx_adapter_id);
  ...
  if (adptr_services->nb_rx_adptrs) {
    ...
    int32_t ret =
        rte_service_component_register(&service, &fdata->rxadptr_service_id);
    if (ret)
      rte_exit(EXIT_FAILURE, "Rx adapter service register failed");

    rte_service_runstate_set(fdata->rxadptr_service_id, 1);
    rte_service_component_runstate_set(fdata->rxadptr_service_id, 1);
...
static void worker_tx_enq_opt_check(void) {
  int i;
  int ret;
  uint32_t cap = 0;
  uint8_t rx_needed = 0;
  ...
  if (cdata.all_type_queues &&
      !(eventdev_info.event_dev_cap & RTE_EVENT_DEV_CAP_QUEUE_ALL_TYPES))
    rte_exit(EXIT_FAILURE, "Event dev doesn't support all type queues\n");
  sched_needed =
      !(eventdev_info.event_dev_cap & RTE_EVENT_DEV_CAP_DISTRIBUTED_SCHED);

  RTE_ETH_FOREACH_DEV(i) {
    ret = rte_event_eth_rx_adapter_caps_get(0, i, &cap);
    if (ret)
      rte_exit(EXIT_FAILURE, "failed to get event rx adapter capabilities");
    rx_needed |= !(cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT);
  }

  if (cdata.worker_lcore_mask == 0 || (rx_needed && cdata.rx_lcore_mask == 0) ||
    ...
    printf("Core part of pipeline was not assigned any cores. "
           ...
           "\trx: %" PRIu64 "\n\tsched: %" PRIu64 "\n\tworkers: %" PRIu64 "\n",
           cdata.rx_lcore_mask, cdata.sched_lcore_mask,
           cdata.worker_lcore_mask);
    rte_exit(-1, "Fix core masks\n");
  }

  if (!sched_needed)
    memset(fdata->sched_core, 0, sizeof(unsigned int) * MAX_NUM_CORE);
  if (!rx_needed)
    memset(fdata->rx_core, 0, sizeof(unsigned int) * MAX_NUM_CORE);
...
static worker_loop get_worker_loop_single_burst(uint8_t atq) {
  if (atq)
...
static worker_loop get_worker_loop_single_non_burst(uint8_t atq) {
  if (atq)
...
static worker_loop get_worker_loop_burst(uint8_t atq) {
  if (atq)
...
static worker_loop get_worker_loop_non_burst(uint8_t atq) {
  if (atq)
...
static worker_loop get_worker_single_stage(bool burst) {
...
static worker_loop get_worker_multi_stage(bool burst) {
  uint8_t atq = cdata.all_type_queues ? 1 : 0;
