...
struct config_data cdata = {.num_packets = (1L << 25), /* do ~32M packets */
                            .num_fids = 512,
...
static void dump_core_info(unsigned int lcore_id, struct worker_data *data,
                           unsigned int worker_idx) {
  if (fdata->rx_core[lcore_id])
    printf("[%s()] lcore %d executing NIC Rx\n", __func__, lcore_id);

  if (fdata->tx_core[lcore_id])
    printf("[%s()] lcore %d executing NIC Tx\n", __func__, lcore_id);

  if (fdata->sched_core[lcore_id])
    printf("[%s()] lcore %d executing scheduler\n", __func__, lcore_id);

  if (fdata->worker_core[lcore_id])
    printf("[%s()] lcore %d executing worker, using eventdev port %u\n",
           __func__, lcore_id, data[worker_idx].port_id);
}

static bool core_in_use(unsigned int lcore_id) {
...
static uint64_t parse_coremask(const char *coremask) {
  int i, j, idx = 0;
...
static void usage(void) {
  const char *usage_str =
      ...
      "  Options:\n"
      "  -n, --packets=N              Send N packets (default ~32M), 0 implies "
      "no limit\n"
      "  -f, --atomic-flows=N         Use N random flows from 1 to N (default "
      "16)\n"
...
static void parse_app_args(int argc, char **argv) {
  ...
  for (;;) {
    c = getopt_long(argc, argv, "r:t:e:c:w:n:f:s:m:paoPqDW:", long_options,
                    &option_index);
    if (c == -1)
      break;
...
static void do_capability_setup(uint8_t eventdev_id) {
  int ret;
  uint16_t i;
  ...
  RTE_ETH_FOREACH_DEV(i) {
    ...
    if (ret)
      rte_exit(EXIT_FAILURE, "Invalid capability for Tx adptr port %d\n", i);
    generic_pipeline |= !(caps & RTE_EVENT_ETH_TX_ADAPTER_CAP_INTERNAL_PORT);
  }
...
static inline uint64_t port_stat(int dev_id, int32_t p) {
  char statname[64];
...
int main(int argc, char **argv) {
  ...
  worker_data = rte_calloc(0, cdata.num_workers, sizeof(worker_data[0]), 0);
  if (worker_data == NULL)
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) {
    ...
    if (!fdata->rx_core[lcore_id] && !fdata->worker_core[lcore_id] &&
        !fdata->tx_core[lcore_id] && !fdata->sched_core[lcore_id])
      continue;

    dump_core_info(lcore_id, worker_data, worker_idx);

    err = rte_eal_remote_launch(fdata->cap.worker, &worker_data[worker_idx],
                                lcore_id);
    if (err) {
  ...
  if (!cdata.quiet &&
      (port_stat(dev_id, worker_data[0].port_id) != (uint64_t)-ENOTSUP)) {
    printf("\nPort Workload distribution:\n");
    uint32_t i;
    uint64_t tot_pkts = 0;
    uint64_t pkts_per_wkr[RTE_MAX_LCORE] = {0};
    for (i = 0; i < cdata.num_workers; i++) {
      pkts_per_wkr[i] = port_stat(dev_id, worker_data[i].port_id);
      tot_pkts += pkts_per_wkr[i];
    }
    for (i = 0; i < cdata.num_workers; i++) {
      float pc = pkts_per_wkr[i] * 100 / ((float)tot_pkts);
