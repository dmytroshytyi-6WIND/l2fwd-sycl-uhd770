...
static __rte_always_inline int worker_generic(void *arg) {
  struct rte_event ev;
  ...
  while (!fdata->done) {
    ...
    const uint16_t nb_rx = rte_event_dequeue_burst(dev_id, port_id, &ev, 1, 0);
...
static int worker_generic_burst(void *arg) {
  ...
  while (!fdata->done) {
    ...
    const uint16_t nb_rx =
        rte_event_dequeue_burst(dev_id, port_id, events, RTE_DIM(events), 0);
    ...
    while (nb_tx < nb_rx && !fdata->done)
      nb_tx += rte_event_enqueue_burst(dev_id, port_id, events + nb_tx,
                                       nb_rx - nb_tx);
    sent += nb_tx;
  }
...
static int setup_eventdev_generic(struct worker_data *worker_data) {
  ...
  for (i = 0; i < cdata.num_stages; i++) {
    ...
    if (cdata.enable_queue_priorities) {
      ...
      wkr_q_conf.priority = RTE_EVENT_DEV_PRIORITY_LOWEST - prio_delta * i;
    }
  ...
  for (i = 0; i < cdata.num_workers; i++) {
    ...
    for (s = 0; s < cdata.num_stages; s++) {
      if (rte_event_port_link(dev_id, i, &worker_queues[s].queue_id,
                              &worker_queues[s].priority, 1) != 1) {
...
static void init_adapters(uint16_t nb_ports) {
  ...
  for (i = 0; i < nb_ports; i++) {
    ret = rte_event_eth_rx_adapter_queue_add(cdata.rx_adapter_id, i, -1,
                                             &queue_conf);
    if (ret)
  ...
  ret =
      rte_event_eth_tx_adapter_event_port_get(cdata.tx_adapter_id, &tx_port_id);
  if (ret)
    rte_exit(EXIT_FAILURE, "Failed to get Tx adapter port id");
  ret = rte_event_port_link(evdev_id, tx_port_id, &cdata.tx_queue_id, NULL, 1);
  if (ret != 1)
    rte_exit(EXIT_FAILURE, "Unable to link Tx adapter port to Tx queue");

  ret = rte_event_eth_rx_adapter_service_id_get(cdata.rx_adapter_id,
                                                &fdata->rxadptr_service_id);
  if (ret != -ESRCH && ret != 0) {
    rte_exit(EXIT_FAILURE, "Error getting the service ID for Rx adapter\n");
  }
  ...
  if (ret != -ESRCH && ret != 0) {
    rte_exit(EXIT_FAILURE, "Error getting the service ID for Tx adapter\n");
  }
  ...
  if (ret)
    rte_exit(EXIT_FAILURE, "Rx adapter[%d] start failed", cdata.rx_adapter_id);
...
static void generic_opt_check(void) {
  int i;
  int ret;
  uint32_t cap = 0;
  uint8_t rx_needed = 0;
  ...
  if (cdata.worker_lcore_mask == 0 || (rx_needed && cdata.rx_lcore_mask == 0) ||
    ...
    printf("Core part of pipeline was not assigned any cores. "
           ...
           "\trx: %" PRIu64 "\n\ttx: %" PRIu64 "\n\tsched: %" PRIu64
           "\n\tworkers: %" PRIu64 "\n",
           cdata.rx_lcore_mask, cdata.tx_lcore_mask, cdata.sched_lcore_mask,
