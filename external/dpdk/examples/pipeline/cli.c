...
static int parser_read_uint64(uint64_t *value, const char *p) {
  char *next;
  uint64_t val;
...
static int parser_read_uint32(uint32_t *value, const char *p) {
  uint64_t val = 0;
...
static int parser_read_uint16(uint16_t *value, const char *p) {
  uint64_t val = 0;
  int ret = parser_read_uint64(&val, p);
...
static int parse_tokenize_string(char *string, char *tokens[],
                                 uint32_t *n_tokens) {
  uint32_t i;
...
static void cmd_ring(char **tokens, uint32_t n_tokens, char *out,
                     size_t out_size, void *obj) {
...
static void cmd_pipeline_create(char **tokens, uint32_t n_tokens, char *out,
                                size_t out_size, void *obj) {
  struct pipeline *p;
  char *name;
  uint32_t numa_node;
...
static void cmd_pipeline_port_in(char **tokens, uint32_t n_tokens, char *out,
  ...
  if (strcmp(tokens[t0], "link") == 0) {
    ...
    if (!link) {
      snprintf(out, out_size, MSG_ARG_INVALID, "link_name");
      return;
    }
    ...
    status = rte_swx_pipeline_port_in_config(p->p, port_id, "ethdev", &params);
  } else if (strcmp(tokens[t0], "ring") == 0) {
    struct rte_swx_port_ring_reader_params params;
    struct ring *ring;

    if (n_tokens < t0 + 4) {
      snprintf(out, out_size, MSG_ARG_MISMATCH, "pipeline port in ring");
      return;
    }

    ring = ring_find(obj, tokens[t0 + 1]);
    if (!ring) {
      snprintf(out, out_size, MSG_ARG_INVALID, "ring_name");
      return;
    }
    ...
    status = rte_swx_pipeline_port_in_config(p->p, port_id, "ring", &params);
    ...
    if (!mp) {
      snprintf(out, out_size, MSG_ARG_INVALID, "mempool_name");
      return;
    }
    params.pool = mp->m;
    ...
    if (parser_read_uint64(&params.n_loops, tokens[t0 + 4])) {
      snprintf(out, out_size, MSG_ARG_INVALID, "n_loops");
      return;
    }

    t0 += 5;

    status = rte_swx_pipeline_port_in_config(p->p, port_id, "source", &params);
    ...
    if (!tap) {
      snprintf(out, out_size, MSG_ARG_INVALID, "tap_name");
      return;
    }
    params.fd = tap->fd;
...
static void cmd_pipeline_port_out(char **tokens, uint32_t n_tokens, char *out,
  ...
  if (strcmp(tokens[t0], "link") == 0) {
    ...
    status = rte_swx_pipeline_port_out_config(p->p, port_id, "ethdev", &params);
  } else if (strcmp(tokens[t0], "ring") == 0) {
    ...
    status = rte_swx_pipeline_port_out_config(p->p, port_id, "ring", &params);
  } else if (strcmp(tokens[t0], "sink") == 0) {
    struct rte_swx_port_sink_params params;

    params.file_name = strcmp(tokens[t0 + 1], "none") ? tokens[t0 + 1] : NULL;

    t0 += 2;

    status = rte_swx_pipeline_port_out_config(p->p, port_id, "sink", &params);
  } else if (strcmp(tokens[t0], "tap") == 0) {
...
static void cmd_pipeline_build(char **tokens, uint32_t n_tokens, char *out,
                               size_t out_size, void *obj) {
  ...
  status = rte_swx_pipeline_build_from_spec(p->p, spec, &err_line, &err_msg);
  fclose(spec);
  if (status) {
...
static void table_entry_free(struct rte_swx_table_entry *entry) {
  if (!entry)
    return;
...
static int pipeline_table_entries_add(struct rte_swx_ctl_pipeline *p,
                                      const char *table_name, FILE *file,
  ...
  for (line_id = 1;; line_id++) {
    ...
    status = rte_swx_ctl_pipeline_table_entry_add(p, table_name, entry);
...
static void cmd_pipeline_table_add(char **tokens, uint32_t n_tokens, char *out,
                                   size_t out_size, void *obj) {
  struct pipeline *p;
  ...
  status =
      pipeline_table_entries_add(p->ctl, table_name, file, &file_line_number);
  if (status)
...
static int pipeline_table_entries_delete(struct rte_swx_ctl_pipeline *p,
                                         const char *table_name, FILE *file,
                                         uint32_t *file_line_number) {
  char *line = NULL;
  uint32_t line_id = 0;
  ...
  for (line_id = 1;; line_id++) {
    ...
    status = rte_swx_ctl_pipeline_table_entry_delete(p, table_name, entry);
    table_entry_free(entry);
    if (status)
      goto error;
  }
...
static void cmd_pipeline_table_delete(char **tokens, uint32_t n_tokens,
                                      char *out, size_t out_size, void *obj) {
  struct pipeline *p;
  char *pipeline_name, *table_name, *file_name;
  FILE *file = NULL;
  uint32_t file_line_number = 0;
  ...
  status = pipeline_table_entries_delete(p->ctl, table_name, file,
                                         &file_line_number);
  if (status)
    snprintf(out, out_size, "Invalid entry in file %s at line %u\n", file_name,
             file_line_number);
...
static int pipeline_table_default_entry_add(struct rte_swx_ctl_pipeline *p,
                                            const char *table_name, FILE *file,
                                            uint32_t *file_line_number) {
  char *line = NULL;
  uint32_t line_id = 0;
  ...
  for (line_id = 1;; line_id++) {
    ...
    status = rte_swx_ctl_pipeline_table_default_entry_add(p, table_name, entry);
    table_entry_free(entry);
    if (status)
      goto error;
  }
...
static void cmd_pipeline_table_default(char **tokens, uint32_t n_tokens,
                                       char *out, size_t out_size, void *obj) {
  struct pipeline *p;
  char *pipeline_name, *table_name, *file_name;
  FILE *file = NULL;
  ...
  status = pipeline_table_default_entry_add(p->ctl, table_name, file,
                                            &file_line_number);
  if (status)
    snprintf(out, out_size, "Invalid entry in file %s at line %u\n", file_name,
             file_line_number);
...
static void cmd_pipeline_table_show(char **tokens, uint32_t n_tokens, char *out,
                                    size_t out_size, void *obj) {
  struct pipeline *p;
...
static void cmd_pipeline_selector_group_add(char **tokens, uint32_t n_tokens,
  ...
  if (strcmp(tokens[4], "group") || strcmp(tokens[5], "add")) {
    snprintf(out, out_size, MSG_ARG_NOT_FOUND, "group add");
    return;
  }

  status =
      rte_swx_ctl_pipeline_selector_group_add(p->ctl, selector_name, &group_id);
  if (status)
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
  else
...
static const char cmd_pipeline_selector_group_delete_help[] =
    "pipeline <pipeline_name> selector <selector_name> group delete "
    "<group_id>\n";

static void cmd_pipeline_selector_group_delete(char **tokens, uint32_t n_tokens,
                                               char *out, size_t out_size,
                                               void *obj) {
  struct pipeline *p;
  char *pipeline_name, *selector_name;
  ...
  if (strcmp(tokens[4], "group") || strcmp(tokens[5], "delete")) {
  ...
  status = rte_swx_ctl_pipeline_selector_group_delete(p->ctl, selector_name,
                                                      group_id);
  if (status)
    snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
}
...
static int token_is_comment(const char *token) {
  if ((token[0] == '#') || (token[0] == ';') ||
      ((token[0] == '/') && (token[1] == '/')))
    return 1; /* TRUE. */
...
static int pipeline_selector_group_member_read(const char *string,
                                               uint32_t *group_id,
                                               uint32_t *member_id,
                                               uint32_t *weight,
                                               int *is_blank_or_comment) {
  ...
  if (n_tokens < 4 || strcmp(tokens[0], "group") || strcmp(tokens[2], "member"))
    goto error;
...
static int pipeline_selector_group_members_add(struct rte_swx_ctl_pipeline *p,
  ...
  for (line_id = 1;; line_id++) {
    ...
    status = pipeline_selector_group_member_read(line, &group_id, &member_id,
                                                 &weight, &is_blank_or_comment);
    if (status) {
      if (is_blank_or_comment)
        continue;
    ...
    status = rte_swx_ctl_pipeline_selector_group_member_add(
        p, selector_name, group_id, member_id, weight);
    if (status)
      goto error;
  }
...
static const char cmd_pipeline_selector_group_member_add_help[] =
    "pipeline <pipeline_name> selector <selector_name> group member add "
    "<file_name>";

static void cmd_pipeline_selector_group_member_add(char **tokens,
  ...
  if (strcmp(tokens[4], "group") || strcmp(tokens[5], "member") ||
      strcmp(tokens[6], "add")) {
  ...
  status = pipeline_selector_group_members_add(p->ctl, selector_name, file,
                                               &file_line_number);
  if (status)
    snprintf(out, out_size, "Invalid entry in file %s at line %u\n", file_name,
             file_line_number);
...
pipeline_selector_group_members_delete(struct rte_swx_ctl_pipeline *p,
                                       const char *selector_name, FILE *file,
                                       uint32_t *file_line_number) {
  char *line = NULL;
  uint32_t line_id = 0;
  int status = 0;
  ...
  for (line_id = 1;; line_id++) {
    ...
    status = rte_swx_ctl_pipeline_selector_group_member_delete(
        p, selector_name, group_id, member_id);
    if (status)
      goto error;
  }
...
static const char cmd_pipeline_selector_group_member_delete_help[] =
    "pipeline <pipeline_name> selector <selector_name> group member delete "
    "<file_name>";

static void cmd_pipeline_selector_group_member_delete(
    char **tokens, uint32_t n_tokens, char *out, size_t out_size, void *obj) {
  struct pipeline *p;
  char *pipeline_name, *selector_name, *file_name;
  FILE *file = NULL;
  uint32_t file_line_number = 0;
  ...
  status = pipeline_selector_group_members_delete(p->ctl, selector_name, file,
                                                  &file_line_number);
  if (status)
    snprintf(out, out_size, "Invalid entry in file %s at line %u\n", file_name,
             file_line_number);
...
static void cmd_pipeline_selector_show(char **tokens, uint32_t n_tokens,
                                       char *out, size_t out_size, void *obj) {
  struct pipeline *p;
  char *pipeline_name, *selector_name;
  FILE *file = NULL;
...
static int pipeline_learner_default_entry_add(struct rte_swx_ctl_pipeline *p,
  ...
  for (line_id = 1;; line_id++) {
    ...
    entry = rte_swx_ctl_pipeline_learner_default_entry_read(
        p, learner_name, line, &is_blank_or_comment);
    if (!entry) {
      if (is_blank_or_comment)
        continue;
    ...
    status =
        rte_swx_ctl_pipeline_learner_default_entry_add(p, learner_name, entry);
    table_entry_free(entry);
    if (status)
      goto error;
  }
...
static void cmd_pipeline_learner_default(char **tokens, uint32_t n_tokens,
                                         char *out, size_t out_size,
                                         void *obj) {
  struct pipeline *p;
  ...
  status = pipeline_learner_default_entry_add(p->ctl, learner_name, file,
                                              &file_line_number);
  if (status)
    snprintf(out, out_size, "Invalid entry in file %s at line %u\n", file_name,
             file_line_number);
...
static void cmd_pipeline_commit(char **tokens, uint32_t n_tokens, char *out,
                                size_t out_size, void *obj) {
  struct pipeline *p;
  char *pipeline_name;
  int status;
...
static void cmd_pipeline_abort(char **tokens, uint32_t n_tokens, char *out,
                               size_t out_size, void *obj) {
  struct pipeline *p;
  char *pipeline_name;
...
static void cmd_pipeline_regrd(char **tokens, uint32_t n_tokens, char *out,
                               size_t out_size, void *obj) {
  struct pipeline *p;
  const char *name;
  uint64_t value;
...
static void cmd_pipeline_regwr(char **tokens, uint32_t n_tokens, char *out,
                               size_t out_size, void *obj) {
  struct pipeline *p;
  const char *name;
  uint64_t value;
...
static void cmd_pipeline_meter_profile_add(char **tokens, uint32_t n_tokens,
                                           char *out, size_t out_size,
                                           void *obj) {
...
static void cmd_pipeline_meter_profile_delete(char **tokens, uint32_t n_tokens,
                                              char *out, size_t out_size,
                                              void *obj) {
  struct pipeline *p;
  const char *profile_name;
...
static const char cmd_pipeline_meter_reset_help[] =
    "pipeline <pipeline_name> meter <meter_array_name> from <index0> to "
    "<index1> "
    "reset\n";

static void cmd_pipeline_meter_reset(char **tokens, uint32_t n_tokens,
                                     char *out, size_t out_size, void *obj) {
  struct pipeline *p;
  const char *name;
...
static void cmd_pipeline_meter_set(char **tokens, uint32_t n_tokens, char *out,
                                   size_t out_size, void *obj) {
  struct pipeline *p;
...
static void cmd_pipeline_meter_stats(char **tokens, uint32_t n_tokens,
                                     char *out, size_t out_size, void *obj) {
  ...
  for (; idx0 <= idx1; idx0++) {
    ...
    snprintf(out, out_size,
             "| %7d | %16" PRIx64 " | %16" PRIx64 " | %16" PRIx64
             " | %16" PRIx64 " | %16" PRIx64 " | %16" PRIx64 " |\n",
             idx0, stats.n_pkts[RTE_COLOR_GREEN],
             stats.n_pkts[RTE_COLOR_YELLOW], stats.n_pkts[RTE_COLOR_RED],
             stats.n_bytes[RTE_COLOR_GREEN], stats.n_bytes[RTE_COLOR_YELLOW],
...
static void cmd_pipeline_stats(char **tokens, uint32_t n_tokens, char *out,
                               size_t out_size, void *obj) {
  ...
  for (i = 0; i < info.n_ports_in; i++) {
    ...
    snprintf(out, out_size,
             "\tPort %u:"
             " packets %" PRIu64 " bytes %" PRIu64 " empty %" PRIu64 "\n",
  ...
  for (i = 0; i < info.n_tables; i++) {
    ...
    status =
        rte_swx_ctl_pipeline_table_stats_read(p->p, table_info.name, &stats);
    if (status) {
    ...
    snprintf(out, out_size,
             ...
             "\t\tHit (packets): %" PRIu64 "\n"
             "\t\tMiss (packets): %" PRIu64 "\n",
             table_info.name, stats.n_pkts_hit, stats.n_pkts_miss);
    out_size -= strlen(out);
    out += strlen(out);
    ...
    for (j = 0; j < info.n_actions; j++) {
      ...
      snprintf(out, out_size, "\t\tAction %s (packets): %" PRIu64 "\n",
               action_info.name, stats.n_pkts_action[j]);
      out_size -= strlen(out);
      out += strlen(out);
    }
  }
  ...
  for (i = 0; i < info.n_learners; i++) {
    ...
    status = rte_swx_ctl_pipeline_learner_stats_read(p->p, learner_info.name,
                                                     &stats);
    if (status) {
    ...
    snprintf(out, out_size,
             ...
             learner_info.name, stats.n_pkts_hit, stats.n_pkts_miss,
             stats.n_pkts_learn_ok, stats.n_pkts_learn_err,
             stats.n_pkts_forget);
    out_size -= strlen(out);
    out += strlen(out);
...
static void cmd_help(char **tokens, uint32_t n_tokens, char *out,
                     size_t out_size, void *arg __rte_unused) {
  tokens++;
  n_tokens--;
  ...
  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 2) &&
      (strcmp(tokens[1], "create") == 0)) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_create_help);
    return;
  }

  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 3) &&
  ...
  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 2) &&
      (strcmp(tokens[1], "build") == 0)) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_build_help);
    return;
  }

  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 3) &&
      (strcmp(tokens[1], "table") == 0) && (strcmp(tokens[2], "add") == 0)) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_table_add_help);
    return;
  }

  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 3) &&
      (strcmp(tokens[1], "table") == 0) && (strcmp(tokens[2], "delete") == 0)) {
  ...
  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 3) &&
      (strcmp(tokens[1], "table") == 0) && (strcmp(tokens[2], "show") == 0)) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_table_show_help);
    return;
  }

  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 4) &&
      (strcmp(tokens[1], "selector") == 0) &&
      (strcmp(tokens[2], "group") == 0) && (strcmp(tokens[3], "add") == 0)) {
  ...
  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 4) &&
      (strcmp(tokens[1], "selector") == 0) &&
      (strcmp(tokens[2], "group") == 0) && (strcmp(tokens[3], "delete") == 0)) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_selector_group_delete_help);
    return;
  }

  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 5) &&
      (strcmp(tokens[1], "selector") == 0) &&
      (strcmp(tokens[2], "group") == 0) && (strcmp(tokens[3], "member") == 0) &&
  ...
  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 2) &&
      (strcmp(tokens[1], "regrd") == 0)) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_regrd_help);
    return;
  }

  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 2) &&
      (strcmp(tokens[1], "regwr") == 0)) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_regwr_help);
    return;
  }

  if (!strcmp(tokens[0], "pipeline") && (n_tokens == 4) &&
      !strcmp(tokens[1], "meter") && !strcmp(tokens[2], "profile") &&
      !strcmp(tokens[3], "add")) {
  ...
  if (!strcmp(tokens[0], "pipeline") && (n_tokens == 3) &&
      !strcmp(tokens[1], "meter") && !strcmp(tokens[2], "reset")) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_meter_reset_help);
    return;
  }

  if (!strcmp(tokens[0], "pipeline") && (n_tokens == 3) &&
      !strcmp(tokens[1], "meter") && !strcmp(tokens[2], "set")) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_meter_set_help);
    return;
  }

  if (!strcmp(tokens[0], "pipeline") && (n_tokens == 3) &&
      !strcmp(tokens[1], "meter") && !strcmp(tokens[2], "stats")) {
    snprintf(out, out_size, "\n%s\n", cmd_pipeline_meter_stats_help);
    return;
  }

  if ((strcmp(tokens[0], "pipeline") == 0) && (n_tokens == 2) &&
      (strcmp(tokens[1], "stats") == 0)) {
...
void cli_process(char *in, char *out, size_t out_size, void *obj) {
  ...
  if (strcmp(tokens[0], "pipeline") == 0) {
    if ((n_tokens >= 3) && (strcmp(tokens[2], "create") == 0)) {
      cmd_pipeline_create(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 4) && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "in") == 0)) {
      cmd_pipeline_port_in(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 4) && (strcmp(tokens[2], "port") == 0) &&
        (strcmp(tokens[3], "out") == 0)) {
      cmd_pipeline_port_out(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 3) && (strcmp(tokens[2], "build") == 0)) {
      cmd_pipeline_build(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "add") == 0)) {
      cmd_pipeline_table_add(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "delete") == 0)) {
      cmd_pipeline_table_delete(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "default") == 0)) {
      cmd_pipeline_table_default(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "table") == 0) &&
        (strcmp(tokens[4], "show") == 0)) {
      cmd_pipeline_table_show(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 6) && (strcmp(tokens[2], "selector") == 0) &&
        (strcmp(tokens[4], "group") == 0) && (strcmp(tokens[5], "add") == 0)) {
      cmd_pipeline_selector_group_add(tokens, n_tokens, out, out_size, obj);
      return;
    }
    ...
    if ((n_tokens >= 6) && (strcmp(tokens[2], "selector") == 0) &&
      ...
      cmd_pipeline_selector_group_delete(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 7) && (strcmp(tokens[2], "selector") == 0) &&
        (strcmp(tokens[4], "group") == 0) &&
        (strcmp(tokens[5], "member") == 0) && (strcmp(tokens[6], "add") == 0)) {
      cmd_pipeline_selector_group_member_add(tokens, n_tokens, out, out_size,
                                             obj);
      return;
    }
    ...
    if ((n_tokens >= 7) && (strcmp(tokens[2], "selector") == 0) &&
      ...
      cmd_pipeline_selector_group_member_delete(tokens, n_tokens, out, out_size,
                                                obj);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "selector") == 0) &&
        (strcmp(tokens[4], "show") == 0)) {
      cmd_pipeline_selector_show(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[2], "learner") == 0) &&
        (strcmp(tokens[4], "default") == 0)) {
      cmd_pipeline_learner_default(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 3) && (strcmp(tokens[2], "commit") == 0)) {
      cmd_pipeline_commit(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 3) && (strcmp(tokens[2], "abort") == 0)) {
      cmd_pipeline_abort(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 3) && (strcmp(tokens[2], "regrd") == 0)) {
      cmd_pipeline_regrd(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 3) && (strcmp(tokens[2], "regwr") == 0)) {
      cmd_pipeline_regwr(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 6) && (strcmp(tokens[2], "meter") == 0) &&
    ...
    if ((n_tokens >= 9) && (strcmp(tokens[2], "meter") == 0) &&
    ...
    if ((n_tokens >= 3) && (strcmp(tokens[2], "stats") == 0)) {
      cmd_pipeline_stats(tokens, n_tokens, out, out_size, obj);
      return;
    }
  }
  ...
  if (strcmp(tokens[0], "thread") == 0) {
    if ((n_tokens >= 5) && (strcmp(tokens[4], "enable") == 0)) {
      cmd_thread_pipeline_enable(tokens, n_tokens, out, out_size, obj);
      return;
    }

    if ((n_tokens >= 5) && (strcmp(tokens[4], "disable") == 0)) {
      cmd_thread_pipeline_disable(tokens, n_tokens, out, out_size, obj);
      return;
    }
  }
