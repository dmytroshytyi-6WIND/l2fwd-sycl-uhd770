...
struct mempool *mempool_create(struct obj *obj, const char *name,
                               struct mempool_params *params) {
  struct mempool *mempool;
  struct rte_mempool *m;

  /* Check input params */
  if ((name == NULL) || mempool_find(obj, name) || (params == NULL) ||
      (params->buffer_size < BUFFER_SIZE_MIN) || (params->pool_size == 0))
    return NULL;

  /* Resource create */
  m = rte_pktmbuf_pool_create(name, params->pool_size, params->cache_size, 0,
...
struct mempool *mempool_find(struct obj *obj, const char *name) {
  struct mempool *mempool;
    ...
    .rxmode =
        {
            ...
            .mtu = 9000 - (RTE_ETHER_HDR_LEN +
                           RTE_ETHER_CRC_LEN), /* Jumbo frame MTU */
...
static int rss_setup(uint16_t port_id, uint16_t reta_size,
  ...
  for (i = 0; i < reta_size; i++) {
    ...
    reta_conf[reta_id].reta[reta_pos] = (uint16_t)rss->queue_id[rss_qs_pos];
  }
...
struct link *link_create(struct obj *obj, const char *name,
  ...
  /* Check input params */
  if ((name == NULL) || link_find(obj, name) || (params == NULL) ||
      (params->rx.n_queues == 0) || (params->rx.queue_size == 0) ||
      (params->tx.n_queues == 0) || (params->tx.queue_size == 0))
    return NULL;
  ...
  if (params->dev_name) {
    ...
    if (status)
      return NULL;
  } else if (!rte_eth_dev_is_valid_port(port_id))
    return NULL;
  ...
  status = rte_eth_dev_configure(port_id, params->rx.n_queues,
                                 params->tx.n_queues, &port_conf);
  ...
  for (i = 0; i < params->rx.n_queues; i++) {
    status = rte_eth_rx_queue_setup(port_id, i, params->rx.queue_size, cpu_id,
                                    NULL, mempool->m);
  ...
  for (i = 0; i < params->tx.n_queues; i++) {
    status =
        rte_eth_tx_queue_setup(port_id, i, params->tx.queue_size, cpu_id, NULL);
...
int link_is_up(struct obj *obj, const char *name) {
...
struct link *link_find(struct obj *obj, const char *name) {
  struct link *link;
...
struct link *link_next(struct obj *obj, struct link *link) {
  return (link == NULL) ? TAILQ_FIRST(&obj->link_list) : TAILQ_NEXT(link, node);
}

/*
 * ring
 */
struct ring *ring_create(struct obj *obj, const char *name,
   ...
   * Resource create
   */
  r = rte_ring_create(name, params->size, params->numa_node, flags);
  if (!r)
    return NULL;
...
struct ring *ring_find(struct obj *obj, const char *name) {
  struct ring *ring;
...
struct tap *tap_find(struct obj *obj, const char *name) {
  struct tap *tap;
...
struct tap *tap_next(struct obj *obj, struct tap *tap) {
  return (tap == NULL) ? TAILQ_FIRST(&obj->tap_list) : TAILQ_NEXT(tap, node);
}

#ifndef RTE_EXEC_ENV_LINUX

struct tap *tap_create(struct obj *obj __rte_unused,
...
struct tap *tap_create(struct obj *obj, const char *name) {
  struct tap *tap;
  struct ifreq ifr;
  int fd, status;

  /* Check input params */
  if ((name == NULL) || tap_find(obj, name))
    return NULL;
...
struct pipeline *pipeline_create(struct obj *obj, const char *name,
                                 int numa_node) {
  ...
  /* Check input params */
  if ((name == NULL) || pipeline_find(obj, name))
    return NULL;
...
struct pipeline *pipeline_find(struct obj *obj, const char *name) {
  struct pipeline *pipeline;
