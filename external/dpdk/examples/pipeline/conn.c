...
struct conn *conn_init(struct conn_params *p) {
  ...
  if ((p == NULL) || (p->welcome == NULL) || (p->prompt == NULL) ||
      (p->addr == NULL) || (p->buf_size == 0) || (p->msg_in_len_max == 0) ||
      (p->msg_out_len_max == 0) || (p->msg_handle == NULL))
    return NULL;
  ...
  if ((conn->welcome == NULL) || (conn->prompt == NULL) ||
      (conn->buf == NULL) || (conn->msg_in == NULL) ||
      (conn->msg_out == NULL)) {
    conn_free(conn);
    return NULL;
  }
...
int conn_poll_for_conn(struct conn *conn) {
...
static int data_event_handle(struct conn *conn, int fd_client) {
...
static int control_event_handle(struct conn *conn, int fd_client) {
  int status;
