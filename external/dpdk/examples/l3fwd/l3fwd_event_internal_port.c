...
static uint32_t l3fwd_event_device_setup_internal_port(void) {
...
static void l3fwd_event_port_setup_internal_port(void) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
static void l3fwd_event_queue_setup_internal_port(uint32_t event_queue_cfg) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
  uint8_t event_d_id = evt_rsrc->event_d_id;
...
static void l3fwd_rx_tx_adapter_setup_internal_port(void) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
  ...
  RTE_ETH_FOREACH_DEV(port_id) {
    ...
    if (evt_rsrc->vector_enabled) {
      ...
      if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_EVENT_VECTOR) {
        ...
        eth_q_conf.vector_timeout_ns = evt_rsrc->vector_tmo_ns;
