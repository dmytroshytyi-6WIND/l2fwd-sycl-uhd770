...
static inline void fib_parse_packet(struct rte_mbuf *mbuf, uint32_t *ipv4,
                                    uint32_t *ipv4_cnt,
...
static inline void fib_send_single(int nb_tx, struct lcore_conf *qconf,
  ...
  for (j = 0; j < nb_tx; j++) {
    ...
    rfc1812_process(
        (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(pkts_burst[j],
    ...
    *(uint64_t *)&eth_hdr->dst_addr = dest_eth_addr[hops[j]];
    rte_ether_addr_copy(&ports_eth_addr[hops[j]], &eth_hdr->src_addr);
...
static inline void fib_send_packets(int nb_rx, struct rte_mbuf **pkts_burst,
  ...
  for (i = 0; i < (nb_rx - FIB_PREFETCH_OFFSET); i++) {
    /* Prefetch packet. */
    rte_prefetch0(
        rte_pktmbuf_mtod(pkts_burst[i + FIB_PREFETCH_OFFSET], void *));
    fib_parse_packet(pkts_burst[i], &ipv4_arr[ipv4_cnt], &ipv4_cnt,
  ...
  if (likely(ipv4_cnt > 0))
    rte_fib_lookup_bulk(qconf->ipv4_lookup_struct, ipv4_arr, hopsv4, ipv4_cnt);

  /* Lookup IPv6 hops if IPv6 packets are present. */
  if (ipv6_cnt > 0)
    rte_fib6_lookup_bulk(qconf->ipv6_lookup_struct, ipv6_arr, hopsv6, ipv6_cnt);
...
int fib_main_loop(__rte_unused void *dummy) {
...
fib_event_loop(struct l3fwd_event_resources *evt_rsrc, const uint8_t flags) {
  ...
  while (!force_quit) {
    ...
    for (i = 0; i < (nb_deq - FIB_PREFETCH_OFFSET); i++) {
      ...
      /* Prefetch packet. */
      rte_prefetch0(
          rte_pktmbuf_mtod(events[i + FIB_PREFETCH_OFFSET].mbuf, void *));

      fib_parse_packet(events[i].mbuf, &ipv4_arr[ipv4_cnt], &ipv4_cnt,
                       ipv6_arr[ipv6_cnt], &ipv6_cnt, &type_arr[i]);
    }
    ...
    /* Lookup IPv6 hops if IPv6 packets are present. */
    if (ipv6_cnt > 0)
      rte_fib6_lookup_bulk(lconf->ipv6_lookup_struct, ipv6_arr, hopsv6,
                           ipv6_cnt);
...
int __rte_noinline fib_event_main_loop_tx_d(__rte_unused void *dummy) {
...
int __rte_noinline fib_event_main_loop_tx_d_burst(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
int __rte_noinline fib_event_main_loop_tx_q(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
int __rte_noinline fib_event_main_loop_tx_q_burst(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
fib_process_event_vector(struct rte_event_vector *vec) {
  ...
  for (i = 0; i < (vec->nb_elem - FIB_PREFETCH_OFFSET); i++) {
    rte_prefetch0(rte_pktmbuf_mtod(mbufs[i + FIB_PREFETCH_OFFSET], void *));
  ...
  /* Lookup IPv4 hops if IPv4 packets are present. */
  if (likely(ipv4_cnt > 0))
    rte_fib_lookup_bulk(lconf->ipv4_lookup_struct, ipv4_arr, hopsv4, ipv4_cnt);

  /* Lookup IPv6 hops if IPv6 packets are present. */
  if (ipv6_cnt > 0)
    rte_fib6_lookup_bulk(lconf->ipv6_lookup_struct, ipv6_arr, hopsv6, ipv6_cnt);
...
int __rte_noinline fib_event_main_loop_tx_d_vector(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
int __rte_noinline fib_event_main_loop_tx_q_vector(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
void setup_fib(const int socketid) {
  ...
  for (i = 0; i < route_num_v4; i++) {
    ...
    rte_eth_dev_info_get(route_base_v4[i].if_out, &dev_info);
    ret =
        rte_fib_add(ipv4_l3fwd_fib_lookup_struct[socketid], route_base_v4[i].ip,
                    route_base_v4[i].depth, route_base_v4[i].if_out);
  ...
  ipv6_l3fwd_fib_lookup_struct[socketid] =
      rte_fib6_create(s, socketid, &config);
  ...
  for (i = 0; i < route_num_v6; i++) {
    ...
    /* Skip unused ports. */
    if ((1 << route_base_v6[i].if_out & enabled_port_mask) == 0)
      continue;

    rte_eth_dev_info_get(route_base_v6[i].if_out, &dev_info);
    ret = rte_fib6_add(ipv6_l3fwd_fib_lookup_struct[socketid],
                       route_base_v6[i].ip_8, route_base_v6[i].depth,
    ...
    if (inet_ntop(AF_INET6, route_base_v6[i].ip_8, abuf, sizeof(abuf)) !=
        NULL) {
      printf("FIB: Adding route %s / %d (%d) [%s]\n", abuf,
             route_base_v6[i].depth, route_base_v6[i].if_out,
             dev_info.device->name);
    } else {
...
void *fib_get_ipv4_l3fwd_lookup_struct(const int socketid) {
