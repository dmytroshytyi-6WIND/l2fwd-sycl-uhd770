...
#endif
  dst_port = em_get_ipv4_dst_port(ipv4_hdr, portid, qconf->ipv4_lookup_struct);
...
l3fwd_em_handle_ipv6(struct rte_mbuf *m, uint16_t portid,
  ...
  dst_port = em_get_ipv6_dst_port(ipv6_hdr, portid, qconf->ipv6_lookup_struct);
 ...
 * from main_loop.
 */
static inline void l3fwd_em_no_opt_send_packets(int nb_rx,
 ...
 * from main_loop.
 */
static inline void l3fwd_em_no_opt_process_events(int nb_rx,
   ...
   */
  for (j = 0; j < (nb_rx - PREFETCH_OFFSET); j++) {
    rte_prefetch0(rte_pktmbuf_mtod(events[j + PREFETCH_OFFSET]->mbuf, void *));
