...
static int em_parse_v6_net(const char *in, uint8_t *v) {
  int32_t rc;
...
static int em_parse_v6_rule(char *str, struct em_rule *v) {
  int i, rc;
...
static int em_parse_v4_rule(char *str, struct em_rule *v) {
  int i, rc;
  char *s, *sp, *in[CB_FLD_MAX];
  static const char *dlm = " \t\n";
  int dim = CB_FLD_MAX;
...
static int em_add_rules(const char *rule_path, struct em_rule **proute_base,
  ...
  *proute_base = NULL;
  ...
  while (fgets(buff, LINE_MAX, fh) != NULL) {
    ...
    if (parser(buff + 1, next) != 0) {
      RTE_LOG(ERR, L3FWD, "%s Line %u: parse rules error\n", rule_path, i);
      fclose(fh);
      free(route_rules);
      return -EINVAL;
    }
...
static int em_add_default_v4_rules(void) {
  ...
  for (i = 0; i < (unsigned int)route_num_v4; i++) {
    ...
    em_route_base_v4[i].v4_key.port_dst =
        ipv4_l3fwd_em_route_array[i].key.port_dst;
    em_route_base_v4[i].v4_key.port_src =
        ipv4_l3fwd_em_route_array[i].key.port_src;
...
static int em_add_default_v6_rules(void) {
  ...
  for (i = 0; i < (unsigned int)route_num_v6; i++) {
    memcpy(em_route_base_v6[i].v6_key.ip_dst,
           ipv6_l3fwd_em_route_array[i].key.ip_dst,
           sizeof(em_route_base_v6[i].v6_key.ip_dst));
    memcpy(em_route_base_v6[i].v6_key.ip_src,
           ipv6_l3fwd_em_route_array[i].key.ip_src,
           sizeof(em_route_base_v6[i].v6_key.ip_src));
    em_route_base_v6[i].v6_key.port_dst =
        ipv6_l3fwd_em_route_array[i].key.port_dst;
    em_route_base_v6[i].v6_key.port_src =
        ipv6_l3fwd_em_route_array[i].key.port_src;
...
void read_config_files_em(void) {
  /* ipv4 check */
  if (parm_config.rule_ipv4_name != NULL &&
      parm_config.rule_ipv6_name != NULL) {
    /* ipv4 check */
    route_num_v4 = em_add_rules(parm_config.rule_ipv4_name, &em_route_base_v4,
                                &em_parse_v4_rule);
    if (route_num_v4 < 0) {
      em_free_routes();
    ...
    /* ipv6 check */
    route_num_v6 = em_add_rules(parm_config.rule_ipv6_name, &em_route_base_v6,
                                &em_parse_v6_rule);
    if (route_num_v6 < 0) {
      em_free_routes();
