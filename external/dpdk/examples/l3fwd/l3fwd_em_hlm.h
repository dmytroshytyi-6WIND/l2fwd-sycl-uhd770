...
static __rte_always_inline void em_get_dst_port_ipv4xN(struct lcore_conf *qconf,
                                                       struct rte_mbuf *m[],
                                                       uint16_t portid,
                                                       uint16_t dst_port[]) {
  int i;
  ...
  for (i = 0; i < EM_HASH_LOOKUP_COUNT; i++) {
    dst_port[i] = ((ret[i] < 0) ? portid : ipv4_l3fwd_out_if[ret[i]]);
...
static __rte_always_inline void em_get_dst_port_ipv6xN(struct lcore_conf *qconf,
                                                       struct rte_mbuf *m[],
                                                       uint16_t portid,
                                                       uint16_t dst_port[]) {
  int i;
  ...
  for (i = 0; i < EM_HASH_LOOKUP_COUNT; i++) {
    dst_port[i] = ((ret[i] < 0) ? portid : ipv6_l3fwd_out_if[ret[i]]);
...
em_get_dst_port_ipv4xN_events(struct lcore_conf *qconf, struct rte_mbuf *m[],
  ...
  for (i = 0; i < EM_HASH_LOOKUP_COUNT; i++) {
    dst_port[i] = ((ret[i] < 0) ? m[i]->port : ipv4_l3fwd_out_if[ret[i]]);
...
em_get_dst_port_ipv6xN_events(struct lcore_conf *qconf, struct rte_mbuf *m[],
  ...
  for (i = 0; i < EM_HASH_LOOKUP_COUNT; i++) {
    dst_port[i] = ((ret[i] < 0) ? m[i]->port : ipv6_l3fwd_out_if[ret[i]]);
...
static __rte_always_inline uint16_t em_get_dst_port(
  ...
  if (tcp_or_udp && (l3_ptypes == RTE_PTYPE_L3_IPV4)) {
    ...
    next_hop =
        em_get_ipv4_dst_port(ipv4_hdr, portid, qconf->ipv4_lookup_struct);
    ...
    next_hop =
        em_get_ipv6_dst_port(ipv6_hdr, portid, qconf->ipv6_lookup_struct);
 ...
 * from main_loop.
 */
static inline void l3fwd_em_send_packets(int nb_rx,
  ...
  for (j = 0; j < EM_HASH_LOOKUP_COUNT && j < nb_rx; j++) {
    rte_prefetch0(rte_pktmbuf_mtod(pkts_burst[j], struct rte_ether_hdr *) + 1);
  }

  for (j = 0; j < n; j += EM_HASH_LOOKUP_COUNT) {

    uint32_t pkt_type = RTE_PTYPE_L3_MASK | RTE_PTYPE_L4_TCP | RTE_PTYPE_L4_UDP;
    ...
    for (i = 0, pos = j + EM_HASH_LOOKUP_COUNT;
      ...
      rte_prefetch0(rte_pktmbuf_mtod(pkts_burst[pos], struct rte_ether_hdr *) +
                    1);
    }

    if (tcp_or_udp && (l3_type == RTE_PTYPE_L3_IPV4)) {

      em_get_dst_port_ipv4xN(qconf, &pkts_burst[j], portid, &dst_port[j]);

    } else if (tcp_or_udp && (l3_type == RTE_PTYPE_L3_IPV6)) {

      em_get_dst_port_ipv6xN(qconf, &pkts_burst[j], portid, &dst_port[j]);

    } else {
      for (i = 0; i < EM_HASH_LOOKUP_COUNT; i++)
        dst_port[j + i] = em_get_dst_port(qconf, pkts_burst[j + i], portid);
    }
  }
 ...
 * from main_loop.
 */
static inline void l3fwd_em_process_events(int nb_rx, struct rte_event **ev,
                                           struct lcore_conf *qconf) {
  int32_t i, j, pos;
  uint16_t dst_port[MAX_PKT_BURST];
  ...
  for (j = 0; j < n; j += EM_HASH_LOOKUP_COUNT) {
    ...
    if (tcp_or_udp && (l3_type == RTE_PTYPE_L3_IPV4)) {
      ...
      em_get_dst_port_ipv4xN_events(qconf, &pkts_burst[j], &dst_port[j]);

    } else if (tcp_or_udp && (l3_type == RTE_PTYPE_L3_IPV6)) {

      em_get_dst_port_ipv6xN_events(qconf, &pkts_burst[j], &dst_port[j]);

    } else {
      for (i = 0; i < EM_HASH_LOOKUP_COUNT; i++) {
        pkts_burst[j + i]->port =
            em_get_dst_port(qconf, pkts_burst[j + i], pkts_burst[j + i]->port);
        process_packet(pkts_burst[j + i], &pkts_burst[j + i]->port);
      }
      continue;
    }
  ...
  for (; j < nb_rx; j++) {
    pkts_burst[j]->port =
        em_get_dst_port(qconf, pkts_burst[j], pkts_burst[j]->port);
    process_packet(pkts_burst[j], &pkts_burst[j]->port);
  }
}

static inline void l3fwd_em_process_event_vector(struct rte_event_vector *vec,
                                                 struct lcore_conf *qconf) {
  ...
  for (j = 0; j < EM_HASH_LOOKUP_COUNT && j < vec->nb_elem; j++)
    rte_prefetch0(rte_pktmbuf_mtod(mbufs[j], struct rte_ether_hdr *) + 1);
  ...
  for (j = 0; j < n; j += EM_HASH_LOOKUP_COUNT) {
    ...
    for (i = 0, pos = j + EM_HASH_LOOKUP_COUNT;
         i < EM_HASH_LOOKUP_COUNT && pos < vec->nb_elem; i++, pos++) {
      rte_prefetch0(rte_pktmbuf_mtod(mbufs[pos], struct rte_ether_hdr *) + 1);
    }

    if (tcp_or_udp && (l3_type == RTE_PTYPE_L3_IPV4)) {
      em_get_dst_port_ipv4xN_events(qconf, &mbufs[j], &dst_port[j]);
    } else if (tcp_or_udp && (l3_type == RTE_PTYPE_L3_IPV6)) {
      em_get_dst_port_ipv6xN_events(qconf, &mbufs[j], &dst_port[j]);
    } else {
      for (i = 0; i < EM_HASH_LOOKUP_COUNT; i++) {
        mbufs[j + i]->port =
            em_get_dst_port(qconf, mbufs[j + i], mbufs[j + i]->port);
        process_packet(mbufs[j + i], &mbufs[j + i]->port);
  ...
  for (; j < vec->nb_elem; j++) {
    mbufs[j]->port = em_get_dst_port(qconf, mbufs[j], mbufs[j]->port);
