...
l3fwd_lpm_simple_forward(struct rte_mbuf *m, uint16_t portid,
  ...
  if (RTE_ETH_IS_IPV4_HDR(m->packet_type)) {
...
#endif
    dst_port =
        lpm_get_ipv4_dst_port(ipv4_hdr, portid, qconf->ipv4_lookup_struct);
    ...
    *(uint64_t *)&eth_hdr->dst_addr = dest_eth_addr[dst_port];
    ...
    dst_port =
        lpm_get_ipv6_dst_port(ipv6_hdr, portid, qconf->ipv6_lookup_struct);
...
static inline void l3fwd_lpm_no_opt_send_packets(int nb_rx,
