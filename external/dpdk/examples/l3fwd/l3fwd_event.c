...
static void l3fwd_eth_dev_port_setup(struct rte_eth_conf *port_conf) {
  ...
  RTE_ETH_FOREACH_DEV(port_id) {
    ...
    ret = rte_eth_dev_configure(port_id, evt_rsrc->eth_rx_queues, 1,
                                &local_port_conf);
    if (ret < 0)
    ...
    if (!evt_rsrc->per_port_pool) {
       ...
       */
      nb_mbuf = RTE_MAX(nb_ports * nb_rxd + nb_ports * nb_txd +
                            nb_ports * nb_lcores * MAX_PKT_BURST +
                            nb_lcores * MEMPOOL_CACHE_SIZE,
                        8192u);
      ret = init_mem(0, nb_mbuf);
    } else {
      nb_mbuf = RTE_MAX(nb_rxd + nb_rxd + nb_lcores * MAX_PKT_BURST +
                            nb_lcores * MEMPOOL_CACHE_SIZE,
                        8192u);
    ...
    for (eth_qid = 0; eth_qid < evt_rsrc->eth_rx_queues; eth_qid++) {
      if (!evt_rsrc->per_port_pool)
        ret = rte_eth_rx_queue_setup(port_id, eth_qid, nb_rxd, 0, &rxconf,
...
static void l3fwd_event_capability_setup(void) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
  uint32_t caps = 0;
  uint16_t i;
  int ret;
...
int l3fwd_get_free_event_port(struct l3fwd_event_resources *evt_rsrc) {
  static int index;
  int port_id;
