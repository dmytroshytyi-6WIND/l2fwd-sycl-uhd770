...
enum { CB_FLD_DST_ADDR, CB_FLD_IF_OUT, CB_FLD_MAX };
...
int is_bypass_line(const char *buff) {
  int i = 0;
...
static int parse_ipv6_addr_mask(char *token, uint32_t *ipv6, uint8_t *mask) {
  char *sa, *sm, *sv;
  const char *dlm = "/";
...
static int parse_ipv4_addr_mask(char *token, uint32_t *ipv4, uint8_t *mask) {
  char *sa, *sm, *sv;
  const char *dlm = "/";
...
static int lpm_parse_v6_net(char *in, uint32_t *v, uint8_t *mask_len) {
  int32_t rc;
...
static int lpm_parse_v6_rule(char *str, struct lpm_route_rule *v) {
  int i, rc;
...
static int lpm_parse_v4_rule(char *str, struct lpm_route_rule *v) {
  int i, rc;
  char *s, *sp, *in[CB_FLD_MAX];
  static const char *dlm = " \t\n";
  int dim = CB_FLD_MAX;
...
static int lpm_add_default_v4_rules(void) {
...
static int lpm_add_default_v6_rules(void) {
...
static int lpm_add_rules(const char *rule_path,
...
void read_config_files_lpm(void) {
  if (parm_config.rule_ipv4_name != NULL &&
      parm_config.rule_ipv6_name != NULL) {
    /* ipv4 check */
    route_num_v4 = lpm_add_rules(parm_config.rule_ipv4_name, &route_base_v4,
                                 &lpm_parse_v4_rule);
    if (route_num_v4 < 0) {
      lpm_free_routes();
    ...
    /* ipv6 check */
    route_num_v6 = lpm_add_rules(parm_config.rule_ipv6_name, &route_base_v6,
                                 &lpm_parse_v6_rule);
    if (route_num_v6 < 0) {
      lpm_free_routes();
