...
lpm_get_ipv4_dst_port(const struct rte_ipv4_hdr *ipv4_hdr, uint16_t portid,
...
static inline uint16_t
lpm_get_ipv6_dst_port(const struct rte_ipv6_hdr *ipv6_hdr, uint16_t portid,
...
static __rte_always_inline uint16_t lpm_get_dst_port(
    const struct lcore_conf *qconf, struct rte_mbuf *pkt, uint16_t portid) {
  ...
  if (RTE_ETH_IS_IPV4_HDR(pkt->packet_type)) {
    ...
    return lpm_get_ipv4_dst_port(ipv4_hdr, portid, qconf->ipv4_lookup_struct);
  } else if (RTE_ETH_IS_IPV6_HDR(pkt->packet_type)) {

    eth_hdr = rte_pktmbuf_mtod(pkt, struct rte_ether_hdr *);
    ipv6_hdr = (struct rte_ipv6_hdr *)(eth_hdr + 1);

    return lpm_get_ipv6_dst_port(ipv6_hdr, portid, qconf->ipv6_lookup_struct);
  }
...
lpm_get_dst_port_with_ipv4(const struct lcore_conf *qconf, struct rte_mbuf *pkt,
  ...
  if (RTE_ETH_IS_IPV4_HDR(pkt->packet_type)) {
    return (uint16_t)(
        (rte_lpm_lookup(qconf->ipv4_lookup_struct, dst_ipv4, &next_hop) == 0)
            ? next_hop
            : portid);
...
int lpm_main_loop(__rte_unused void *dummy) {
...
lpm_process_event_pkt(const struct lcore_conf *lconf, struct rte_mbuf *mbuf) {
  ...
  if (RTE_ETH_IS_IPV4_HDR(mbuf->packet_type)) {
    ...
    if (is_valid_ipv4_pkt(ipv4_hdr, mbuf->pkt_len) < 0) {
      mbuf->port = BAD_PORT;
      continue;
    }
  ...
  *(uint64_t *)&eth_hdr->dst_addr = dest_eth_addr[mbuf->port];
  ...
  /* src addr */
  rte_ether_addr_copy(&ports_eth_addr[mbuf->port], &eth_hdr->src_addr);
#endif
  return mbuf->port;
}
...
static __rte_always_inline void
lpm_event_loop(struct l3fwd_event_resources *evt_rsrc, const uint8_t flags) {
...
int __rte_noinline lpm_event_main_loop_tx_d(__rte_unused void *dummy) {
...
int __rte_noinline lpm_event_main_loop_tx_d_burst(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
int __rte_noinline lpm_event_main_loop_tx_q(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
int __rte_noinline lpm_event_main_loop_tx_q_burst(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
static __rte_always_inline void
lpm_process_event_vector(struct rte_event_vector *vec,
                         struct lcore_conf *lconf) {
...
int __rte_noinline lpm_event_main_loop_tx_d_vector(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
int __rte_noinline lpm_event_main_loop_tx_q_vector(__rte_unused void *dummy) {
  struct l3fwd_event_resources *evt_rsrc = l3fwd_get_eventdev_rsrc();
...
void setup_lpm(const int socketid) {
  ...
  for (i = 0; i < route_num_v4; i++) {
    ...
    ret =
        rte_lpm_add(ipv4_l3fwd_lpm_lookup_struct[socketid], route_base_v4[i].ip,
    ...
    printf("LPM: Adding route %s / %d (%d) [%s]\n",
           inet_ntop(AF_INET, &in, abuf, sizeof(abuf)), route_base_v4[i].depth,
  ...
  for (i = 0; i < route_num_v6; i++) {
    ...
    printf("LPM: Adding route %s / %d (%d) [%s]\n",
           inet_ntop(AF_INET6, route_base_v6[i].ip_8, abuf, sizeof(abuf)),
