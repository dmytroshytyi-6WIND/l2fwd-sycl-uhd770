...
static inline void l3fwd_em_send_packets(int nb_rx,
  ...
  if (nb_rx > 0) {
    rte_prefetch0(rte_pktmbuf_mtod(pkts_burst[0], struct rte_ether_hdr *) + 1);
  }
 ...
 * from main_loop.
 */
static inline void l3fwd_em_process_events(int nb_rx, struct rte_event **events,
                                           struct lcore_conf *qconf) {
  int32_t i, j;

  rte_prefetch0(rte_pktmbuf_mtod(events[0]->mbuf, struct rte_ether_hdr *) + 1);
...
static inline void l3fwd_em_process_event_vector(struct rte_event_vector *vec,
  ...
  for (i = 0, j = 1; i < vec->nb_elem; i++, j++) {
    ...
    mbufs[i]->port = em_get_dst_port(qconf, mbufs[i], mbufs[i]->port);
