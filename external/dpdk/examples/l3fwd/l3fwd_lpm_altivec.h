 ...
 */
static inline void processx4_step1(struct rte_mbuf *pkt[FWDSTEP],
 ...
 */
static inline void processx4_step2(const struct lcore_conf *qconf,
                                   vector unsigned int dip, uint32_t ipv4_flag,
                                   uint8_t portid,
  ...
  if (likely(ipv4_flag)) {
    rte_lpm_lookupx4(qconf->ipv4_lookup_struct, (xmm_t)dip, (uint32_t *)&dst,
                     portid);
 ...
 * from main_loop.
 */
static inline void l3fwd_lpm_send_packets(int nb_rx,
  ...
  for (j = 0; j != k; j += FWDSTEP)
    processx4_step1(&pkts_burst[j], &dip[j / FWDSTEP], &ipv4_flag[j / FWDSTEP]);

  for (j = 0; j != k; j += FWDSTEP)
    processx4_step2(qconf, dip[j / FWDSTEP], ipv4_flag[j / FWDSTEP], portid,
