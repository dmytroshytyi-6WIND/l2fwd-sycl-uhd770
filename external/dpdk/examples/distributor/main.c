 ...
 */
    ...
    uint16_t sent =
        rte_ring_enqueue_burst(out_ring, (void *)bufs, nb_ret, NULL);
#endif
...
int main(int argc, char *argv[]) {
  ...
  d = rte_distributor_create("PKT_DIST", rte_socket_id(), rte_lcore_count() - 4,
                             RTE_DIST_ALG_BURST);
  if (d == NULL)
  ...
  struct lcore_params *pd = rte_malloc(NULL, sizeof(*pd), 0);
  if (!pd)
    rte_panic("malloc failure\n");
