...
static const char *const compressdev_pmd_valid_params[] = {
    RTE_COMPRESSDEV_PMD_NAME_ARG, RTE_COMPRESSDEV_PMD_SOCKET_ID_ARG};
 ...
 */
typedef int (*compressdev_private_xform_create_t)(
    struct rte_compressdev *dev, const struct rte_comp_xform *xform,
    void **private_xform);
 ...
 *  - errno on failure
 */
__rte_experimental int rte_compressdev_pmd_parse_input_args(
 ...
 *  - errno on failure
 */
__rte_experimental int rte_compressdev_pmd_destroy(struct rte_compressdev *dev);
