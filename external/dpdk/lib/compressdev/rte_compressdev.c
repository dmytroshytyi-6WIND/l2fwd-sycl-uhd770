...
rte_compressdev_capability_get(uint8_t dev_id, enum rte_comp_algorithm algo) {
...
static struct rte_compressdev *rte_compressdev_get_dev(uint8_t dev_id) {
...
static unsigned int rte_compressdev_is_valid_dev(uint8_t dev_id) {
...
int rte_compressdev_get_dev_id(const char *name) {
  ...
  for (i = 0; i < compressdev_globals.nb_devs; i++)
    if ((strcmp(compressdev_globals.devs[i].data->name, name) == 0) &&
        (compressdev_globals.devs[i].attached == RTE_COMPRESSDEV_ATTACHED))
      return i;

  return -1;
}

uint8_t rte_compressdev_count(void) { return compressdev_globals.nb_devs; }

uint8_t rte_compressdev_devices_get(const char *driver_name, uint8_t *devices,
                                    uint8_t nb_devices) {
  uint8_t i, count = 0;
...
int rte_compressdev_socket_id(uint8_t dev_id) {
  struct rte_compressdev *dev;
...
static inline int rte_compressdev_data_alloc(uint8_t dev_id,
  ...
  n = snprintf(mz_name, sizeof(mz_name), "rte_compressdev_data_%u", dev_id);
  if (n >= (int)sizeof(mz_name))
    return -EINVAL;

  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    mz = rte_memzone_reserve(mz_name, sizeof(struct rte_compressdev_data),
                             socket_id, 0);
  } else
...
static uint8_t rte_compressdev_find_free_device_index(void) {
  uint8_t dev_id;

  for (dev_id = 0; dev_id < RTE_COMPRESS_MAX_DEVS; dev_id++) {
    if (rte_comp_devices[dev_id].attached == RTE_COMPRESSDEV_DETACHED)
      return dev_id;
  }
  return RTE_COMPRESS_MAX_DEVS;
}

struct rte_compressdev *rte_compressdev_pmd_allocate(const char *name,
                                                     int socket_id) {
  struct rte_compressdev *compressdev;
  uint8_t dev_id;

  if (rte_compressdev_pmd_get_named_dev(name) != NULL) {
    COMPRESSDEV_LOG(ERR, "comp device with name %s already allocated!", name);
    return NULL;
  }
  ...
  if (compressdev->data == NULL) {
    ...
    int retval =
        rte_compressdev_data_alloc(dev_id, &compressdev_data, socket_id);
    ...
    strlcpy(compressdev->data->name, name, RTE_COMPRESSDEV_NAME_MAX_LEN);
...
int rte_compressdev_pmd_release_device(struct rte_compressdev *compressdev) {
  int ret;
...
static int rte_compressdev_queue_pairs_config(struct rte_compressdev *dev,
                                              uint16_t nb_qpairs,
                                              int socket_id) {
  struct rte_compressdev_info dev_info;
  void **qp;
  ...
  if ((dev == NULL) || (nb_qpairs < 1)) {
    COMPRESSDEV_LOG(ERR, "invalid param: dev %p, nb_queues %u", dev, nb_qpairs);
    return -EINVAL;
  }

  COMPRESSDEV_LOG(DEBUG, "Setup %d queues pairs on device %u", nb_qpairs,
                  dev->data->dev_id);
  ...
  if ((dev_info.max_nb_queue_pairs != 0) &&
    ...
    COMPRESSDEV_LOG(ERR, "Invalid num queue_pairs (%u) for dev %u", nb_qpairs,
                    dev->data->dev_id);
    return -EINVAL;
  }
  ...
  if (dev->data->queue_pairs == NULL) { /* first time configuration */
    dev->data->queue_pairs =
        rte_zmalloc_socket("compressdev->queue_pairs",
    ...
    qp = rte_realloc(qp, sizeof(qp[0]) * nb_qpairs, RTE_CACHE_LINE_SIZE);
    if (qp == NULL) {
      COMPRESSDEV_LOG(ERR, "failed to realloc qp meta data, nb_queues %u",
                      nb_qpairs);
      return -(ENOMEM);
    }
...
static int rte_compressdev_queue_pairs_release(struct rte_compressdev *dev) {
  uint16_t num_qps, i;
  int ret;
...
int rte_compressdev_configure(uint8_t dev_id,
  ...
  if (dev->data->dev_started) {
    COMPRESSDEV_LOG(ERR, "device %d must be stopped to allow configuration",
                    dev_id);
    return -EBUSY;
  }
  ...
  diag = rte_compressdev_queue_pairs_config(dev, config->nb_queue_pairs,
                                            config->socket_id);
  if (diag != 0) {
    COMPRESSDEV_LOG(ERR, "dev%d rte_comp_dev_queue_pairs_config = %d", dev_id,
                    diag);
    return diag;
  }
...
int rte_compressdev_start(uint8_t dev_id) {
  ...
  if (dev->data->dev_started != 0) {
    COMPRESSDEV_LOG(ERR, "Device with dev_id=%" PRIu8 " already started",
                    dev_id);
    return 0;
  }
...
void rte_compressdev_stop(uint8_t dev_id) {
  ...
  if (dev->data->dev_started == 0) {
    COMPRESSDEV_LOG(ERR, "Device with dev_id=%" PRIu8 " already stopped",
                    dev_id);
    return;
  }
...
int rte_compressdev_close(uint8_t dev_id) {
  ...
  if (dev->data->dev_started == 1) {
    COMPRESSDEV_LOG(ERR, "Device %u must be stopped before closing", dev_id);
    return -EBUSY;
  }
...
int rte_compressdev_queue_pair_setup(uint8_t dev_id, uint16_t queue_pair_id,
  ...
  if (max_inflight_ops == 0) {
    COMPRESSDEV_LOG(ERR, "Invalid maximum number of inflight operations");
    return -EINVAL;
  }

  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->queue_pair_setup, -ENOTSUP);

  return (*dev->dev_ops->queue_pair_setup)(dev, queue_pair_id, max_inflight_ops,
                                           socket_id);
}
...
uint16_t rte_compressdev_dequeue_burst(uint8_t dev_id, uint16_t qp_id,
  ...
  nb_ops = (*dev->dequeue_burst)(dev->data->queue_pairs[qp_id], ops, nb_ops);
...
int rte_compressdev_stats_get(uint8_t dev_id,
...
int rte_compressdev_private_xform_create(uint8_t dev_id,
  ...
  if (ret < 0) {
    COMPRESSDEV_LOG(ERR, "dev_id %d failed to create private_xform: err=%d",
                    dev_id, ret);
    return ret;
  };
...
int rte_compressdev_private_xform_free(uint8_t dev_id, void *priv_xform) {
  struct rte_compressdev *dev;
  int ret;
  ...
  if (ret < 0) {
    COMPRESSDEV_LOG(ERR, "dev_id %d failed to free private xform: err=%d",
                    dev_id, ret);
    return ret;
  };
...
int rte_compressdev_stream_create(uint8_t dev_id,
                                  const struct rte_comp_xform *xform,
                                  void **stream) {
  struct rte_compressdev *dev;
  int ret;
  ...
  if (ret < 0) {
    COMPRESSDEV_LOG(ERR, "dev_id %d failed to create stream: err=%d", dev_id,
                    ret);
    return ret;
  };
...
int rte_compressdev_stream_free(uint8_t dev_id, void *stream) {
  ...
  if (ret < 0) {
    COMPRESSDEV_LOG(ERR, "dev_id %d failed to free stream: err=%d", dev_id,
                    ret);
    return ret;
  };
