 ...
 */
static int rte_compressdev_pmd_parse_name_arg(const char *key __rte_unused,
                                              const char *value,
                                              void *extra_args) {
 ...
 */
static int rte_compressdev_pmd_parse_uint_arg(const char *key __rte_unused,
                                              const char *value,
                                              void *extra_args) {
  int i;
  char *end;
...
int rte_compressdev_pmd_parse_input_args(
    struct rte_compressdev_pmd_init_params *params, const char *args) {
  ...
  if (args) {
    ...
    ret = rte_kvargs_process(kvlist, RTE_COMPRESSDEV_PMD_SOCKET_ID_ARG,
                             &rte_compressdev_pmd_parse_uint_arg,
                             &params->socket_id);
    if (ret < 0)
      goto free_kvlist;

    ret = rte_kvargs_process(kvlist, RTE_COMPRESSDEV_PMD_NAME_ARG,
                             &rte_compressdev_pmd_parse_name_arg, params);
    if (ret < 0)
      goto free_kvlist;
  }
...
rte_compressdev_pmd_create(const char *name, struct rte_device *device,
  ...
  COMPRESSDEV_LOG(INFO, "Init parameters - name: %s, socket id: %d", name,
                  params->socket_id);
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    compressdev->data->dev_private =
        rte_zmalloc_socket("compressdev device private", private_data_size,
...
int rte_compressdev_pmd_destroy(struct rte_compressdev *compressdev) {
  int retval;

  COMPRESSDEV_LOG(INFO, "Closing comp device %s", compressdev->device->name);
