 ...
 */
static inline void rte_comp_op_reset(struct rte_comp_op *op) {
 ...
 */
static inline int rte_comp_op_raw_bulk_alloc(struct rte_mempool *mempool,
...
static void rte_comp_op_init(struct rte_mempool *mempool,
                             __rte_unused void *opaque_arg, void *_op_data,
...
struct rte_mempool *rte_comp_op_pool_create(const char *name,
  ...
  if (mp != NULL) {
    priv = (struct rte_comp_op_pool_private *)rte_mempool_get_priv(mp);

    if (mp->elt_size != elt_size || mp->cache_size < cache_size ||
        mp->size < nb_elts || priv->user_size < user_size) {
      mp = NULL;
      COMPRESSDEV_LOG(
          ERR, "Mempool %s already exists but with incompatible parameters",
          name);
      return NULL;
    }
    return mp;
  ...
  mp = rte_mempool_create(name, nb_elts, elt_size, cache_size,
                          sizeof(struct rte_comp_op_pool_private), NULL, NULL,
                          rte_comp_op_init, NULL, socket_id, 0);
