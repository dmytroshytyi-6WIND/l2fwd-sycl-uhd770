...
#define RTE_HASH_EXTRA_FLAGS_MASK                                              \
...
static inline int rte_hash_cmp_eq(const void *key1, const void *key2,
 ...
 */
static inline uint16_t get_short_sig(const hash_sig_t hash) {
  return hash >> 16;
}

static inline uint32_t get_prim_bucket_index(const struct rte_hash *h,
                                             const hash_sig_t hash) {
  return hash & h->bucket_bitmask;
}

static inline uint32_t get_alt_bucket_index(const struct rte_hash *h,
...
struct rte_hash *rte_hash_create(const struct rte_hash_parameters *params) {
  ...
  if ((params->entries > RTE_HASH_ENTRIES_MAX) ||
      (params->entries < RTE_HASH_BUCKET_ENTRIES) || (params->key_len == 0)) {
    rte_errno = EINVAL;
  ...
  if ((params->extra_flag & RTE_HASH_EXTRA_FLAGS_RW_CONCURRENCY) &&
    ...
    rte_errno = EINVAL;
    RTE_LOG(ERR, HASH,
            "rte_hash_create: choose rw concurrency or "
  ...
  if (use_local_cache)
     ...
     */
    num_key_slots =
        params->entries + (RTE_MAX_LCORE - 1) * (LCORE_CACHE_SIZE - 1) + 1;
  else
  ...
  r = rte_ring_create_elem(ring_name, sizeof(uint32_t),
                           rte_align32pow2(num_key_slots), params->socket_id,
                           0);
  if (r == NULL) {
  ...
  const uint32_t num_buckets =
      rte_align32pow2(params->entries) / RTE_HASH_BUCKET_ENTRIES;
  ...
  buckets =
      rte_zmalloc_socket(NULL, num_buckets * sizeof(struct rte_hash_bucket),
  ...
  if (ext_table_support) {
    ...
    if (readwrite_concur_lf_support) {
      ...
      if (ext_bkt_to_free == NULL) {
        RTE_LOG(ERR, HASH,
                "ext bkt to free memory allocation "
                "failed\n");
        goto err_unlock;
      }
    }
  ...
  k = rte_zmalloc_socket(NULL, key_tbl_size, RTE_CACHE_LINE_SIZE,
                         params->socket_id);
 ...
 */
  ...
  if (use_local_cache) {
    local_free_slots =
        rte_zmalloc_socket(NULL, sizeof(struct lcore_cache) * RTE_MAX_LCORE,
                           RTE_CACHE_LINE_SIZE, params->socket_id);
...
int32_t rte_hash_max_key_id(const struct rte_hash *h) {
  ...
  if (h->use_local_cache)
     ...
     * that can be stored in the lcore caches
     */
    return (h->entries + ((RTE_MAX_LCORE - 1) * (LCORE_CACHE_SIZE - 1)));
  else
    return h->entries;
}
...
int32_t rte_hash_count(const struct rte_hash *h) {
  ...
  if (h->use_local_cache) {
    tot_ring_cnt = h->entries + (RTE_MAX_LCORE - 1) * (LCORE_CACHE_SIZE - 1);
    for (i = 0; i < RTE_MAX_LCORE; i++)
      cached_cnt += h->local_free_slots[i].len;

    ret = tot_ring_cnt - rte_ring_count(h->free_slots) - cached_cnt;
  } else {
...
static inline void __hash_rw_writer_lock(const struct rte_hash *h) {
...
static inline void __hash_rw_reader_lock(const struct rte_hash *h) {
...
static inline void __hash_rw_writer_unlock(const struct rte_hash *h) {
  if (h->writer_takes_lock && h->hw_trans_mem_support)
...
static inline void __hash_rw_reader_unlock(const struct rte_hash *h) {
  if (h->readwrite_concur_support && h->hw_trans_mem_support)
...
void rte_hash_reset(struct rte_hash *h) {
  ...
  *h->tbl_chng_cnt = 0;
  ...
  if (h->ext_table_support) {
    memset(h->buckets_ext, 0, h->num_buckets * sizeof(struct rte_hash_bucket));
 ...
 */
static inline void enqueue_slot_back(const struct rte_hash *h,
  ...
  if (h->use_local_cache) {
    ...
    rte_ring_sp_enqueue_elem(h->free_slots, &slot_id, sizeof(uint32_t));
}

/* Search a key from bucket and update its data.
 * Writer holds the lock before calling this.
 */
static inline int32_t search_and_update(const struct rte_hash *h, void *data,
                                        const void *key,
 ...
 * empty entry.
 */
static inline int32_t rte_hash_cuckoo_insert_mw(
    const struct rte_hash *h, struct rte_hash_bucket *prim_bkt,
    struct rte_hash_bucket *sec_bkt, const struct rte_hash_key *key, void *data,
 ...
 */
static inline int rte_hash_cuckoo_move_insert_mw(
    const struct rte_hash *h, struct rte_hash_bucket *bkt,
    struct rte_hash_bucket *alt_bkt, const struct rte_hash_key *key, void *data,
    struct queue_node *leaf, uint32_t leaf_slot, uint16_t sig, uint32_t new_idx,
    int32_t *ret_val) {
  ...
  while (likely(curr_node->prev != NULL)) {
    ...
    prev_alt_bkt_idx = get_alt_bucket_index(h, prev_node->cur_bkt_idx,
                                            prev_bkt->sig_current[prev_slot]);

    if (unlikely(&h->buckets[prev_alt_bkt_idx] != curr_bkt)) {
      /* revert it to empty, otherwise duplicated keys */
      __atomic_store_n(&curr_bkt->key_idx[curr_slot], EMPTY_SLOT,
                       __ATOMIC_RELEASE);
      __hash_rw_writer_unlock(h);
      return -1;
    }
    ...
    __atomic_store_n(&curr_bkt->key_idx[curr_slot],
                     prev_bkt->key_idx[prev_slot], __ATOMIC_RELEASE);
 ...
 * Cuckoo
 */
static inline int rte_hash_cuckoo_make_space_mw(
    const struct rte_hash *h, struct rte_hash_bucket *bkt,
    struct rte_hash_bucket *sec_bkt, const struct rte_hash_key *key, void *data,
    uint16_t sig, uint32_t bucket_idx, uint32_t new_idx, int32_t *ret_val) {
  unsigned int i;
  ...
  /* Cuckoo bfs Search */
  while (likely(tail != head && head < queue + RTE_HASH_BFS_QUEUE_MAX_LEN -
                                           RTE_HASH_BUCKET_ENTRIES)) {
    curr_bkt = tail->bkt;
    cur_idx = tail->cur_bkt_idx;
    for (i = 0; i < RTE_HASH_BUCKET_ENTRIES; i++) {
      if (curr_bkt->key_idx[i] == EMPTY_SLOT) {
        int32_t ret = rte_hash_cuckoo_move_insert_mw(
            h, bkt, sec_bkt, key, data, tail, i, sig, new_idx, ret_val);
        if (likely(ret != -1))
          return ret;
      }
...
static inline uint32_t alloc_slot(const struct rte_hash *h,
                                  struct lcore_cache *cached_free_slots) {
  ...
  if (h->use_local_cache) {
    ...
    if (cached_free_slots->len == 0) {
      ...
      n_slots = rte_ring_mc_dequeue_burst_elem(
          h->free_slots, cached_free_slots->objs, sizeof(uint32_t),
    ...
    if (rte_ring_sc_dequeue_elem(h->free_slots, &slot_id, sizeof(uint32_t)) !=
        0)
      return EMPTY_SLOT;
  }
...
static inline int32_t __rte_hash_add_key_with_hash(const struct rte_hash *h,
                                                   const void *key,
  ...
  ret = rte_hash_cuckoo_insert_mw(h, prim_bkt, sec_bkt, key, data, short_sig,
                                  slot_id, &ret_val);
  if (ret == 0)
    return slot_id - 1;
  else if (ret == 1) {
  ...
  ret =
      rte_hash_cuckoo_make_space_mw(h, prim_bkt, sec_bkt, key, data, short_sig,
                                    prim_bucket_idx, slot_id, &ret_val);
  if (ret == 0)
    return slot_id - 1;
  else if (ret == 1) {
    enqueue_slot_back(h, cached_free_slots, slot_id);
  ...
  ret =
      rte_hash_cuckoo_make_space_mw(h, sec_bkt, prim_bkt, key, data, short_sig,
                                    sec_bucket_idx, slot_id, &ret_val);
  ...
  if (rte_ring_sc_dequeue_elem(h->free_ext_bkts, &ext_bkt_id,
      ...
      ext_bkt_id == 0) {
    if (h->dq) {
      if (rte_rcu_qsbr_dq_reclaim(h->dq, h->hash_rcu_cfg->max_reclaim_size,
                                  NULL, NULL, NULL) == 0) {
        rte_ring_sc_dequeue_elem(h->free_ext_bkts, &ext_bkt_id,
                                 sizeof(uint32_t));
      }
    }
    if (ext_bkt_id == 0) {
   ...
   */
  __atomic_store_n(&(h->buckets_ext[ext_bkt_id - 1]).key_idx[0], slot_id,
                   __ATOMIC_RELEASE);
...
int32_t rte_hash_add_key_with_hash(const struct rte_hash *h, const void *key,
                                   hash_sig_t sig) {
...
int rte_hash_add_key_with_hash_data(const struct rte_hash *h, const void *key,
                                    hash_sig_t sig, void *data) {
  int ret;
...
static inline int32_t search_one_bucket_l(const struct rte_hash *h,
...
static inline int32_t search_one_bucket_lf(const struct rte_hash *h,
                                           const void *key, uint16_t sig,
                                           void **data,
                                           const struct rte_hash_bucket *bkt) {
  int i;
...
static inline int32_t __rte_hash_lookup_with_hash_l(const struct rte_hash *h,
                                                    const void *key,
                                                    hash_sig_t sig,
                                                    void **data) {
  uint32_t prim_bucket_idx, sec_bucket_idx;
...
static inline int32_t __rte_hash_lookup_with_hash_lf(const struct rte_hash *h,
                                                     const void *key,
                                                     hash_sig_t sig,
                                                     void **data) {
  uint32_t prim_bucket_idx, sec_bucket_idx;
...
static inline int32_t __rte_hash_lookup_with_hash(const struct rte_hash *h,
                                                  const void *key,
                                                  hash_sig_t sig, void **data) {
  if (h->readwrite_concur_lf_support)
...
int32_t rte_hash_lookup_with_hash(const struct rte_hash *h, const void *key,
                                  hash_sig_t sig) {
  RETURN_IF_TRUE(((h == NULL) || (key == NULL)), -EINVAL);
...
static int free_slot(const struct rte_hash *h, uint32_t slot_id) {
  ...
  if (h->use_local_cache) {
    ...
    if (cached_free_slots->len == LCORE_CACHE_SIZE) {
      ...
      n_slots = rte_ring_mp_enqueue_burst_elem(
          h->free_slots, cached_free_slots->objs, sizeof(uint32_t),
          LCORE_CACHE_SIZE, NULL);
...
static void __hash_rcu_qsbr_free_resource(void *p, void *e, unsigned int n) {
  void *key_data = NULL;
  int ret;
  ...
  if (h->ext_table_support && rcu_dq_entry.ext_bkt_idx != EMPTY_SLOT)
    ...
    rte_ring_sp_enqueue_elem(h->free_ext_bkts, &rcu_dq_entry.ext_bkt_idx,
                             sizeof(uint32_t));

  /* Return key indexes to free slot ring */
  ret = free_slot(h, rcu_dq_entry.key_idx);
  if (ret < 0) {
    RTE_LOG(ERR, HASH, "%s: could not enqueue free slots in global ring\n",
            __func__);
  }
}
...
int rte_hash_rcu_qsbr_add(struct rte_hash *h, struct rte_hash_rcu_config *cfg) {
  ...
  if (cfg->mode == RTE_HASH_QSBR_MODE_SYNC) {
    ...
    snprintf(rcu_dq_name, sizeof(rcu_dq_name), "HASH_RCU_%s", h->name);
...
static inline void remove_entry(const struct rte_hash *h,
                                struct rte_hash_bucket *bkt, unsigned int i) {
 ...
 * empty slot.
 */
static inline void __rte_hash_compact_ll(const struct rte_hash *h,
                                         struct rte_hash_bucket *cur_bkt,
                                         int pos) {
  int i;
  ...
  for (i = RTE_HASH_BUCKET_ENTRIES - 1; i >= 0; i--) {
    if (last_bkt->key_idx[i] != EMPTY_SLOT) {
      ...
      __atomic_store_n(&cur_bkt->key_idx[pos], last_bkt->key_idx[i],
                       __ATOMIC_RELEASE);
      if (h->readwrite_concur_lf_support) {
         ...
         */
        __atomic_store_n(h->tbl_chng_cnt, *h->tbl_chng_cnt + 1,
                         __ATOMIC_RELEASE);
 ...
 * function.
 */
static inline int32_t search_and_remove(const struct rte_hash *h,
                                        const void *key,
                                        struct rte_hash_bucket *bkt,
...
static inline int32_t __rte_hash_del_key_with_hash(const struct rte_hash *h,
                                                   const void *key,
                                                   hash_sig_t sig) {
  uint32_t prim_bucket_idx, sec_bucket_idx;
...
return_bkt:
  ...
  if (i == RTE_HASH_BUCKET_ENTRIES) {
    ...
    if (h->no_free_on_del) {
      ...
      rte_ring_sp_enqueue_elem(h->free_ext_bkts, &index, sizeof(uint32_t));
  }
...
return_key:
  ...
  if (h->hash_rcu_cfg) {
    ...
    if (h->dq == NULL) {
      ...
      __hash_rcu_qsbr_free_resource((void *)((uintptr_t)h), &rcu_dq_entry, 1);
    } else if (h->dq)
...
int32_t rte_hash_del_key_with_hash(const struct rte_hash *h, const void *key,
                                   hash_sig_t sig) {
  RETURN_IF_TRUE(((h == NULL) || (key == NULL)), -EINVAL);
...
compare_signatures(uint32_t *prim_hash_matches, uint32_t *sec_hash_matches,
                   ...
                   const struct rte_hash_bucket *sec_bkt, uint16_t sig,
  ...
  case RTE_HASH_COMPARE_SSE:
    ...
    *prim_hash_matches = _mm_movemask_epi8(
        _mm_cmpeq_epi16(_mm_load_si128((__m128i const *)prim_bkt->sig_current),
                        _mm_set1_epi16(sig)));
    /* Compare all signatures in the bucket */
    *sec_hash_matches = _mm_movemask_epi8(
        _mm_cmpeq_epi16(_mm_load_si128((__m128i const *)sec_bkt->sig_current),
                        _mm_set1_epi16(sig)));
    break;
...
static inline void __bulk_lookup_l(const struct rte_hash *h, const void **keys,
                                   const struct rte_hash_bucket **primary_bkt,
                                   const struct rte_hash_bucket **secondary_bkt,
                                   uint16_t *sig, int32_t num_keys,
                                   int32_t *positions, uint64_t *hit_mask,
                                   void *data[]) {
  uint64_t hits = 0;
  int32_t i;
  int32_t ret;
  ...
  for (i = 0; i < num_keys; i++) {
    compare_signatures(&prim_hitmask[i], &sec_hitmask[i], primary_bkt[i],
                       secondary_bkt[i], sig[i], h->sig_cmp_fn);

    if (prim_hitmask[i]) {
      uint32_t first_hit = __builtin_ctzl(prim_hitmask[i]) >> 1;
      uint32_t key_idx = primary_bkt[i]->key_idx[first_hit];
      const struct rte_hash_key *key_slot =
          (const struct rte_hash_key *)((const char *)h->key_store +
...
__bulk_lookup_lf(const struct rte_hash *h, const void **keys,
                 const struct rte_hash_bucket **primary_bkt,
                 const struct rte_hash_bucket **secondary_bkt, uint16_t *sig,
  ...
  do {
    ...
    for (i = 0; i < num_keys; i++) {
      while (prim_hitmask[i]) {
        uint32_t hit_index = __builtin_ctzl(prim_hitmask[i]) >> 1;
        uint32_t key_idx = __atomic_load_n(&primary_bkt[i]->key_idx[hit_index],
                                           __ATOMIC_ACQUIRE);
        const struct rte_hash_key *key_slot =
            (const struct rte_hash_key *)((const char *)h->key_store +
                                          key_idx * h->key_entry_size);
      ...
      while (sec_hitmask[i]) {
        uint32_t hit_index = __builtin_ctzl(sec_hitmask[i]) >> 1;
        uint32_t key_idx = __atomic_load_n(
            &secondary_bkt[i]->key_idx[hit_index], __ATOMIC_ACQUIRE);
        const struct rte_hash_key *key_slot =
            (const struct rte_hash_key *)((const char *)h->key_store +
                                          key_idx * h->key_entry_size);
...
static inline void
__bulk_lookup_prefetching_loop(const struct rte_hash *h, const void **keys,
                               int32_t num_keys, uint16_t *sig,
                               const struct rte_hash_bucket **primary_bkt,
                               const struct rte_hash_bucket **secondary_bkt) {
  int32_t i;
...
static inline void __rte_hash_lookup_bulk_l(const struct rte_hash *h,
  ...
  __bulk_lookup_prefetching_loop(h, keys, num_keys, sig, primary_bkt,
                                 secondary_bkt);

  __bulk_lookup_l(h, keys, primary_bkt, secondary_bkt, sig, num_keys, positions,
                  hit_mask, data);
}

static inline void __rte_hash_lookup_bulk_lf(const struct rte_hash *h,
                                             const void **keys,
                                             int32_t num_keys,
                                             int32_t *positions,
                                             uint64_t *hit_mask, void *data[]) {
...
static inline void __rte_hash_lookup_bulk(const struct rte_hash *h,
                                          const void **keys, int32_t num_keys,
                                          int32_t *positions,
                                          uint64_t *hit_mask, void *data[]) {
  if (h->readwrite_concur_lf_support)
    __rte_hash_lookup_bulk_lf(h, keys, num_keys, positions, hit_mask, data);
  else
    __rte_hash_lookup_bulk_l(h, keys, num_keys, positions, hit_mask, data);
}

int rte_hash_lookup_bulk(const struct rte_hash *h, const void **keys,
                         uint32_t num_keys, int32_t *positions) {
  RETURN_IF_TRUE(((h == NULL) || (keys == NULL) || (num_keys == 0) ||
                  (num_keys > RTE_HASH_LOOKUP_BULK_MAX) || (positions == NULL)),
                 -EINVAL);
...
int rte_hash_lookup_bulk_data(const struct rte_hash *h, const void **keys,
                              ...
                              void *data[]) {
  RETURN_IF_TRUE(((h == NULL) || (keys == NULL) || (num_keys == 0) ||
                  (num_keys > RTE_HASH_LOOKUP_BULK_MAX) || (hit_mask == NULL)),
                 -EINVAL);
...
static inline void __rte_hash_lookup_with_hash_bulk_l(
    const struct rte_hash *h, const void **keys, hash_sig_t *prim_hash,
    int32_t num_keys, int32_t *positions, uint64_t *hit_mask, void *data[]) {
  int32_t i;
  uint32_t prim_index[RTE_HASH_LOOKUP_BULK_MAX];
  uint32_t sec_index[RTE_HASH_LOOKUP_BULK_MAX];
  uint16_t sig[RTE_HASH_LOOKUP_BULK_MAX];
...
static inline void __rte_hash_lookup_with_hash_bulk_lf(
    const struct rte_hash *h, const void **keys, hash_sig_t *prim_hash,
    int32_t num_keys, int32_t *positions, uint64_t *hit_mask, void *data[]) {
  int32_t i;
  uint32_t prim_index[RTE_HASH_LOOKUP_BULK_MAX];
...
static inline void __rte_hash_lookup_with_hash_bulk(
    const struct rte_hash *h, const void **keys, hash_sig_t *prim_hash,
    int32_t num_keys, int32_t *positions, uint64_t *hit_mask, void *data[]) {
  if (h->readwrite_concur_lf_support)
    __rte_hash_lookup_with_hash_bulk_lf(h, keys, prim_hash, num_keys, positions,
                                        hit_mask, data);
  else
    __rte_hash_lookup_with_hash_bulk_l(h, keys, prim_hash, num_keys, positions,
                                       hit_mask, data);
}

int rte_hash_lookup_with_hash_bulk(const struct rte_hash *h, const void **keys,
                                   hash_sig_t *sig, uint32_t num_keys,
                                   int32_t *positions) {
  RETURN_IF_TRUE(((h == NULL) || (keys == NULL) || (sig == NULL) ||
                  (num_keys == 0) || (num_keys > RTE_HASH_LOOKUP_BULK_MAX) ||
                  (positions == NULL)),
                 -EINVAL);

  __rte_hash_lookup_with_hash_bulk(h, keys, sig, num_keys, positions, NULL,
                                   NULL);
  return 0;
}
...
int rte_hash_lookup_with_hash_bulk_data(const struct rte_hash *h,
  ...
  __rte_hash_lookup_with_hash_bulk(h, keys, sig, num_keys, positions, hit_mask,
                                   data);
...
int32_t rte_hash_iterate(const struct rte_hash *h, const void **key,
  ...
  const uint32_t total_entries_main = h->num_buckets * RTE_HASH_BUCKET_ENTRIES;
