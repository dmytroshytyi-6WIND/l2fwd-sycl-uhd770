...
__rte_internal static inline __m512i
__rte_thash_gfni(const uint64_t *mtrx, const uint8_t *tuple,
                 const uint8_t *secondary_tuple, int len) {
  __m512i permute_idx = _mm512_set_epi32(
      0x07060504, 0x07060504, 0x06050403, 0x06050403, 0x05040302, 0x05040302,
      0x04030201, 0x04030201, 0x03020100, 0x03020100, 0x020100FF, 0x020100FF,
      0x0100FFFE, 0x0100FFFE, 0x00FFFEFD, 0x00FFFEFD);
  const __m512i rewind_idx = _mm512_set_epi32(
      0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
      0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x0000003B, 0x0000003B,
      0x00003B3A, 0x00003B3A, 0x003B3A39, 0x003B3A39);
  ...
  for (; len > 0; len -= 64, tuple += 64) {
    if (i == 8)
      perm_bytes =
          _mm512_maskz_permutexvar_epi8(rewind_mask, rewind_idx, perm_bytes);
    ...
    if (secondary_tuple) {
      ...
      tuple_bytes_2 = _mm512_maskz_loadu_epi8(load_mask, secondary_tuple);
    }

    chunk_len = __builtin_popcountll(load_mask);
    for (i = 0; i < ((chunk_len + prepend) / 8); i++, mtrx += 8) {
      perm_bytes = _mm512_mask_permutexvar_epi8(perm_bytes, permute_mask,
                                                permute_idx, tuple_bytes);

      if (secondary_tuple)
        perm_bytes = _mm512_mask_permutexvar_epi8(perm_bytes, permute_mask_2,
