...
static inline uint32_t crc32c_sse42_u64_mimic(uint64_t data,
                                              uint64_t init_val) {
  union {
    uint32_t u32[2];
    uint64_t u64;
...
#if defined(RTE_ARCH_X86)
  if (alg == CRC32_SSE42_x64 && !rte_cpu_get_flag_enabled(RTE_CPUFLAG_EM64T))
    alg = CRC32_SSE42;
#endif
  crc32_alg = alg;
}
...
RTE_INIT(rte_hash_crc_init_alg) { rte_hash_crc_set_alg(CRC32_SSE42_x64); }
