 ...
 */
static const __m128i rte_thash_ipv6_bswap_mask = {0x0405060700010203ULL,
 ...
 */
static inline void rte_thash_load_v6_addrs(const struct rte_ipv6_hdr *orig,
 ...
 */
static inline uint32_t rte_softrss(uint32_t *input_tuple, uint32_t input_len,
  ...
  for (j = 0; j < input_len; j++) {
    for (map = input_tuple[j]; map; map &= (map - 1)) {
      ...
      ret ^= rte_cpu_to_be_32(((const uint32_t *)rss_key)[j]) << (31 - i) |
             (uint32_t)((uint64_t)(rte_cpu_to_be_32(
 ...
 *   Calculated hash value.
 */
static inline uint32_t rte_softrss_be(uint32_t *input_tuple, uint32_t input_len,
                                      const uint8_t *rss_key) {
  uint32_t i, j, map, ret = 0;
 ...
 *  NULL otherwise
 */
__rte_experimental struct rte_thash_ctx *
 ...
 *  negative on error
 */
__rte_experimental int rte_thash_add_helper(struct rte_thash_ctx *ctx,
 ...
 */
__rte_experimental struct rte_thash_subtuple_helper *
...
__rte_experimental uint32_t rte_thash_get_complement(
    struct rte_thash_subtuple_helper *h, uint32_t hash, uint32_t desired_hash);
 ...
 */
__rte_experimental const uint64_t *
...
__rte_experimental int rte_thash_adjust_tuple(
    struct rte_thash_ctx *ctx, struct rte_thash_subtuple_helper *h,
    uint8_t *tuple, unsigned int tuple_len, uint32_t desired_value,
    unsigned int attempts, rte_thash_check_tuple_t fn, void *userdata);
