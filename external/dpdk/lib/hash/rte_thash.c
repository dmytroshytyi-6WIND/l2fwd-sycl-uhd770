...
#define RETA_SZ_IN_RANGE(reta_sz)                                              \
  ((reta_sz >= RTE_THASH_RETA_SZ_MIN) && (reta_sz <= RTE_THASH_RETA_SZ_MAX))
...
void rte_thash_complete_matrix(uint64_t *matrixes, const uint8_t *rss_key,
                               int size) {
  int i, j;
  ...
  for (i = 0; i < size; i++) {
    for (j = 0; j < 8; j++) {
      ...
      right_part = (uint16_t)(rss_key[(i + 1) % size]) >> (8 - j);
      m[i * 8 + j] = left_part | right_part;
    }
  }
}

static inline uint32_t get_bit_lfsr(struct thash_lfsr *lfsr) {
  uint32_t bit, ret;
  ...
  ret = lfsr->state & 0x1;
  lfsr->state =
      ((lfsr->state >> 1) | (bit << (lfsr->deg - 1))) & ((1 << lfsr->deg) - 1);

  lfsr->bits_cnt++;
  return ret;
}

static inline uint32_t get_rev_bit_lfsr(struct thash_lfsr *lfsr) {
  uint32_t bit, ret;

  bit = __builtin_popcount(lfsr->rev_state & lfsr->rev_poly) & 0x1;
  ret = lfsr->rev_state & (1 << (lfsr->deg - 1));
  lfsr->rev_state = ((lfsr->rev_state << 1) | bit) & ((1 << lfsr->deg) - 1);

  lfsr->bits_cnt++;
  return ret;
}

static inline uint32_t thash_get_rand_poly(uint32_t poly_degree) {
  return irreducible_poly_table[poly_degree]
                               [rte_rand() %
                                RTE_DIM(irreducible_poly_table[poly_degree])];
}

static struct thash_lfsr *alloc_lfsr(struct rte_thash_ctx *ctx) {
  struct thash_lfsr *lfsr;
  uint32_t i;
...
static void attach_lfsr(struct rte_thash_subtuple_helper *h,
                        struct thash_lfsr *lfsr) {
  lfsr->ref_cnt++;
  h->lfsr = lfsr;
}

static void free_lfsr(struct thash_lfsr *lfsr) {
  lfsr->ref_cnt--;
  if (lfsr->ref_cnt == 0)
    rte_free(lfsr);
}

struct rte_thash_ctx *rte_thash_init_ctx(const char *name, uint32_t key_len,
                                         uint32_t reta_sz, uint8_t *key,
                                         uint32_t flags) {
  ...
  if (te == NULL) {
    RTE_LOG(ERR, HASH, "Can not allocate tailq entry for thash context %s\n",
            name);
    rte_errno = ENOMEM;
    goto exit;
  }
  ...
  if (rte_thash_gfni_supported()) {
    ctx->matrices =
        rte_zmalloc(NULL, key_len * sizeof(uint64_t), RTE_CACHE_LINE_SIZE);
    ...
    rte_thash_complete_matrix(ctx->matrices, ctx->hash_key, key_len);
  }
...
static inline void set_bit(uint8_t *ptr, uint32_t bit, uint32_t pos) {
 ...
 */
static int generate_subkey(struct rte_thash_ctx *ctx, struct thash_lfsr *lfsr,
  ...
  if (((lfsr->bits_cnt + req_bits) > (1ULL << lfsr->deg) - 1) &&
    ...
    RTE_LOG(ERR, HASH, "Can't generate m-sequence due to period overflow\n");
    return -ENOSPC;
  }
  ...
  if (ctx->matrices != NULL)
    rte_thash_complete_matrix(ctx->matrices, ctx->hash_key, ctx->key_len);

  return 0;
}

static inline uint32_t get_subvalue(struct rte_thash_ctx *ctx,
                                    uint32_t offset) {
  uint32_t *tmp, val;
...
generate_complement_table(struct rte_thash_ctx *ctx,
  ...
  for (i = 1; i < (1 << ctx->reta_sz_log); i++) {
    ...
    for (j = i; j; j &= (j - 1)) {
      k = rte_bsf32(j);
      val ^= get_subvalue(ctx, start - k + ctx->reta_sz_log - 1);
    }
    h->compl_table[val] = i;
  }
}

static inline int insert_before(struct rte_thash_ctx *ctx,
...
static inline int insert_after(struct rte_thash_ctx *ctx,
                               struct rte_thash_subtuple_helper *ent,
                               struct rte_thash_subtuple_helper *cur_ent,
                               struct rte_thash_subtuple_helper *next_ent,
...
int rte_thash_add_helper(struct rte_thash_ctx *ctx, const char *name,
  ...
  if ((ctx == NULL) || (name == NULL) || (len < ctx->reta_sz_log) ||
      ((offset + len + TOEPLITZ_HASH_LEN - 1) > ctx->key_len * CHAR_BIT))
    return -EINVAL;
  ...
  start = ((ctx->flags & RTE_THASH_MINIMAL_SEQ) == RTE_THASH_MINIMAL_SEQ)
              ? (end - (2 * ctx->reta_sz_log - 1))
              : offset;

  ent = rte_zmalloc(NULL,
                    sizeof(struct rte_thash_subtuple_helper) +
  ...
  while (cur_ent) {
    ...
    while ((next_ent != NULL) && (next_ent->offset < range_end)) {
      range_end = RTE_MAX(next_ent->offset + next_ent->len, range_end);
    ...
    if (start < cur_ent->offset)
      return insert_before(ctx, ent, cur_ent, next_ent, start, end, range_end);
    else if (start < range_end)
      return insert_after(ctx, ent, cur_ent, next_ent, prev_ent, end,
                          range_end);
...
static inline uint8_t read_unaligned_byte(uint8_t *ptr, unsigned int len,
                                          unsigned int offset) {
  uint8_t ret = 0;
  ...
  if (offset % CHAR_BIT) {
    ret <<= (offset % CHAR_BIT);
    ret |= ptr[(offset / CHAR_BIT) + 1] >> (CHAR_BIT - (offset % CHAR_BIT));
  }

  return ret >> (CHAR_BIT - len);
}

static inline uint32_t read_unaligned_bits(uint8_t *ptr, int len, int offset) {
  uint32_t ret = 0;
  ...
  while (len > 0) {
    ...
    ret |= read_unaligned_byte(ptr, RTE_MIN(len, CHAR_BIT), offset);
    offset += CHAR_BIT;
    len -= CHAR_BIT;
  }
...
static inline uint8_t get_bits_mask(unsigned int len, unsigned int offset) {
  ...
  return ((1 << (CHAR_BIT - offset)) - 1) ^ ((1 << (CHAR_BIT - last_bit)) - 1);
}

static inline void write_unaligned_byte(uint8_t *ptr, unsigned int len,
...
static inline void write_unaligned_bits(uint8_t *ptr, int len, int offset,
                                        uint32_t val) {
  uint8_t tmp;
  ...
  while (len > 0) {
    ...
    write_unaligned_byte(ptr, part_len, offset + len - part_len, tmp);
    len -= CHAR_BIT;
    val >>= CHAR_BIT;
  }
}

int rte_thash_adjust_tuple(struct rte_thash_ctx *ctx,
                           struct rte_thash_subtuple_helper *h, uint8_t *tuple,
                           unsigned int tuple_len, uint32_t desired_value,
                           unsigned int attempts, rte_thash_check_tuple_t fn,
                           void *userdata) {
  ...
  for (i = 0; i < attempts; i++) {
    ...
    if (fn != NULL) {
      ...
      if (ret == 0)
        ...
        offset -= tmp_len;
        tmp = read_unaligned_bits(tuple, tmp_len, offset);
        tmp++;
        tmp &= (1 << tmp_len) - 1;
        write_unaligned_bits(tuple, tmp_len, offset, tmp);
      }
    } else
      return 0;
  }
