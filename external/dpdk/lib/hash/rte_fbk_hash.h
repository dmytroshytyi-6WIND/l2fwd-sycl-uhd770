 ...
 */
static inline int
rte_fbk_hash_add_key_with_bucket(struct rte_fbk_hash_table *ht, uint32_t key,
                                 uint16_t value, uint32_t bucket) {
  /*
 ...
 *   0 if ok, or negative value on error.
 */
static inline int rte_fbk_hash_add_key(struct rte_fbk_hash_table *ht,
                                       uint32_t key, uint16_t value) {
  return rte_fbk_hash_add_key_with_bucket(ht, key, value,
                                          rte_fbk_hash_get_bucket(ht, key));
}
 ...
 */
static inline int
rte_fbk_hash_delete_key_with_bucket(struct rte_fbk_hash_table *ht, uint32_t key,
                                    uint32_t bucket) {
  ...
  for (i = 0; i < ht->entries_per_bucket; i++) {
    if (ht->t[bucket + i].entry.key == key) {
       ...
       * it doesn't matter.
       */
      ht->t[bucket + i].whole_entry = ht->t[bucket + last_entry].whole_entry;
 ...
 *   0 if ok, or negative value on error.
 */
static inline int rte_fbk_hash_delete_key(struct rte_fbk_hash_table *ht,
                                          uint32_t key) {
  return rte_fbk_hash_delete_key_with_bucket(ht, key,
                                             rte_fbk_hash_get_bucket(ht, key));
}
 ...
 *   The value that was associated with the key, or negative value on error.
 */
static inline int rte_fbk_hash_lookup(const struct rte_fbk_hash_table *ht,
                                      uint32_t key) {
  return rte_fbk_hash_lookup_with_bucket(ht, key,
                                         rte_fbk_hash_get_bucket(ht, key));
}
 ...
 */
static inline void rte_fbk_hash_clear_all(struct rte_fbk_hash_table *ht) {
