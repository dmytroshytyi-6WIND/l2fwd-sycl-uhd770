...
static uint64_t latencystat_cycles_per_ns(void) {
...
static inline rte_mbuf_timestamp_t *timestamp_dynfield(struct rte_mbuf *mbuf) {
...
#define NUM_LATENCY_STATS                                                      \
  (sizeof(lat_stats_strings) / sizeof(lat_stats_strings[0]))
...
int32_t rte_latencystats_update(void) {
  ...
  for (i = 0; i < NUM_LATENCY_STATS; i++) {
    ...
    values[i] = (uint64_t)floor((*stats_ptr) / latencystat_cycles_per_ns());
  }

  ret = rte_metrics_update_values(RTE_METRICS_GLOBAL, latency_stats_index,
...
static void rte_latencystats_fill_values(struct rte_metric_value *values) {
  unsigned int i;
  float *stats_ptr = NULL;
  ...
  for (i = 0; i < NUM_LATENCY_STATS; i++) {
    ...
    values[i].key = i;
    values[i].value =
        (uint64_t)floor((*stats_ptr) / latencystat_cycles_per_ns());
  }
}

static uint16_t add_time_stamps(uint16_t pid __rte_unused,
...
static uint16_t calc_latency(uint16_t pid __rte_unused,
                             uint16_t qid __rte_unused, struct rte_mbuf **pkts,
  ...
  for (i = 0; i < cnt; i++) {
     ...
     */
    glob_stats->jitter +=
        (fabsf(prev_latency - latency[i]) - glob_stats->jitter) / 16;
     ...
     */
    glob_stats->avg_latency += alpha * (latency[i] - glob_stats->avg_latency);
...
int rte_latencystats_init(uint64_t app_samp_intvl,
  ...
  latency_stats_index = rte_metrics_reg_names(ptr_strings, NUM_LATENCY_STATS);
  if (latency_stats_index < 0) {
    RTE_LOG(DEBUG, LATENCY_STATS, "Failed to register latency stats names\n");
    return -1;
  }
  ...
  RTE_ETH_FOREACH_DEV(pid) {
    ...
    for (qid = 0; qid < dev_info.nb_rx_queues; qid++) {
      cbs = &rx_cbs[pid][qid];
      cbs->cb =
          rte_eth_add_first_rx_callback(pid, qid, add_time_stamps, user_cb);
      if (!cbs->cb)
        RTE_LOG(INFO, LATENCY_STATS,
                "Failed to "
    ...
    for (qid = 0; qid < dev_info.nb_tx_queues; qid++) {
      cbs = &tx_cbs[pid][qid];
      cbs->cb = rte_eth_add_tx_callback(pid, qid, calc_latency, user_cb);
      if (!cbs->cb)
        RTE_LOG(INFO, LATENCY_STATS,
                "Failed to "
...
int rte_latencystats_get_names(struct rte_metric_name *names, uint16_t size) {
  ...
  for (i = 0; i < NUM_LATENCY_STATS; i++)
    strlcpy(names[i].name, lat_stats_strings[i].name, sizeof(names[i].name));
...
int rte_latencystats_get(struct rte_metric_value *values, uint16_t size) {
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ...
    mz = rte_memzone_lookup(MZ_RTE_LATENCY_STATS);
    if (mz == NULL) {
      RTE_LOG(ERR, LATENCY_STATS, "Latency stats memzone not found\n");
      return -ENOMEM;
    }
    glob_stats = mz->addr;
  }
