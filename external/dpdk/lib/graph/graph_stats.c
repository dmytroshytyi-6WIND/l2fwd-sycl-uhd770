...
static inline void print_banner(FILE *f) {
  boarder();
...
static inline void print_node(FILE *f,
  ...
  cycles_per_call =
      call_delta ? (double)((cycles - stat->prev_cycles) / call_delta) : 0;
...
static int stats_mem_populate(struct rte_graph_cluster_stats **stats_in,
...
static void stats_mem_fini(struct rte_graph_cluster_stats *stats) {
  free(stats);
}

static void cluster_init(struct cluster *cluster) {
  memset(cluster, 0, sizeof(*cluster));
}

static int cluster_add(struct cluster *cluster, struct graph *graph) {
  rte_graph_t count;
  size_t sz;
...
static void cluster_fini(struct cluster *cluster) { free(cluster->graphs); }

static int expand_pattern_to_cluster(struct cluster *cluster,
                                     const char *pattern) {
...
static inline void cluster_node_arregate_stats(struct cluster_node *cluster) {
...
static inline void cluster_node_store_prev_stats(struct cluster_node *cluster) {
  struct rte_graph_cluster_node_stats *stat = &cluster->stat;
...
void rte_graph_cluster_stats_get(struct rte_graph_cluster_stats *stat,
  ...
  for (count = 0; count < stat->max_nodes; count++) {
    ...
    if (!skip_cb)
      rc = stat->fn(!count, (count == stat->max_nodes - 1), stat->cookie,
                    &cluster->stat);
