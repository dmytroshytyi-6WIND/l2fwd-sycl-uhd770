...
static size_t graph_fp_mem_calc_size(struct graph *graph) {
...
static void graph_header_popluate(struct graph *_graph) {
...
static void graph_nodes_populate(struct graph *_graph) {
  ...
  STAILQ_FOREACH(graph_node, &_graph->node_list, next) {
    ...
    for (count = 0; count < nb_edges; count++)
      node->nodes[count] =
          (struct rte_node *)&graph_node->adjacency_list[count]->node->name[0];
...
struct rte_node *graph_node_id_to_ptr(const struct rte_graph *graph,
  ...
  rte_graph_foreach_node(count, off, graph,
                         node) if (unlikely(node->id == id)) return node;
...
struct rte_node *graph_node_name_to_ptr(const struct rte_graph *graph,
  ...
  rte_graph_foreach_node(
      count, off, graph,
      node) if (strncmp(name, node->name, RTE_NODE_NAMESIZE) == 0) return node;

  return NULL;
}

static int graph_node_nexts_populate(struct graph *_graph) {
...
static int graph_src_nodes_populate(struct graph *_graph) {
  struct rte_graph *graph = _graph->graph;
  struct graph_node *graph_node;
  struct rte_node *node;
  int32_t head = -1;
...
static int graph_fp_mem_populate(struct graph *graph) {
  int rc;
...
static void graph_nodes_mem_destroy(struct rte_graph *graph) {
  rte_node_t count;
  rte_graph_off_t off;
  struct rte_node *node;
  ...
  rte_graph_foreach_node(count, off, graph, node) rte_free(node->objs);
}
