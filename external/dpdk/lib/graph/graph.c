...
void graph_spinlock_lock(void) { rte_spinlock_lock(&graph_lock); }

void graph_spinlock_unlock(void) { rte_spinlock_unlock(&graph_lock); }

static int graph_node_add(struct graph *graph, struct node *node) {
  struct graph_node *graph_node;
  size_t sz;

  /* Skip the duplicate nodes */
  STAILQ_FOREACH(graph_node, &graph->node_list, next)
  if (strncmp(node->name, graph_node->node->name, RTE_NODE_NAMESIZE) == 0)
    return 0;
...
static struct graph_node *node_to_graph_node(struct graph *graph,
                                             struct node *node) {
  struct graph_node *graph_node;
...
static int graph_node_edges_add(struct graph *graph) {
  struct graph_node *graph_node;
...
static int graph_adjacency_list_update(struct graph *graph) {
...
static int expand_pattern_to_node(struct graph *graph, const char *pattern) {
...
static int graph_node_init(struct graph *graph) {
  struct graph_node *graph_node;
  const char *name;
  int rc;
...
static void graph_node_fini(struct graph *graph) {
  struct graph_node *graph_node;
...
static struct rte_graph *graph_mem_fixup_node_ctx(struct rte_graph *graph) {
...
static struct rte_graph *graph_mem_fixup_secondary(struct rte_graph *graph) {
...
void __rte_noinline __rte_node_stream_alloc(struct rte_graph *graph,
                                            struct rte_node *node) {
...
void __rte_noinline __rte_node_stream_alloc_size(struct rte_graph *graph,
                                                 struct rte_node *node,
                                                 uint16_t req_size) {
  uint16_t size = node->size;
...
static int graph_to_dot(FILE *f, struct graph *graph) {
  ...
  STAILQ_FOREACH(graph_node, &graph->node_list, next) {
    ...
    for (i = 0; i < graph_node->node->nb_edges; i++) {
      rc = fprintf(f, "\t\"%s\"->\"%s\"%s", node_name,
                   ...
                   graph_node->node->flags & RTE_NODE_SOURCE_F ? src_edge_color
                                                               : edge_color);
      if (rc < 0)
        goto end;
    }
...
static void graph_scan_dump(FILE *f, rte_graph_t id, bool all) {
  struct graph *graph;
...
void rte_graph_dump(FILE *f, rte_graph_t id) { graph_scan_dump(f, id, false); }

void rte_graph_list_dump(FILE *f) { graph_scan_dump(f, 0, true); }

rte_graph_t rte_graph_max_count(void) { return graph_id; }
