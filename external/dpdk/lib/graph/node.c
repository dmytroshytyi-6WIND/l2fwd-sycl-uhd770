...
static bool node_has_duplicate_entry(const char *name) {
  struct node *node;
...
/* Public functions */
rte_node_t __rte_node_register(const struct rte_node_register *reg) {
  struct node *node;
  rte_edge_t i;
  size_t sz;
  ...
  RTE_BUILD_BUG_ON((offsetof(struct rte_node, nodes) -
                    offsetof(struct rte_node, ctx)) != RTE_CACHE_LINE_MIN_SIZE);
...
static int clone_name(struct rte_node_register *reg, struct node *node,
                      const char *name) {
  ssize_t sz, rc;
...
static rte_node_t node_clone(struct node *node, const char *name) {
...
static rte_edge_t edge_update(struct node *node, struct node *prev,
                              rte_edge_t from, const char **next_nodes,
                              rte_edge_t nb_edges) {
  ...
  for (i = from; i < max_edges; i++, count++) {
    if (rte_strscpy(node->next_nodes[i], next_nodes[count], RTE_NODE_NAMESIZE) <
        0)
      goto restore;
  }
restore:
...
static rte_node_t node_copy_edges(struct node *node, char *next_nodes[]) {
  rte_edge_t i;
...
static void node_scan_dump(FILE *f, rte_node_t id, bool all) {
  struct node *node;
...
void rte_node_dump(FILE *f, rte_node_t id) { node_scan_dump(f, id, false); }

void rte_node_list_dump(FILE *f) { node_scan_dump(f, 0, true); }

rte_node_t rte_node_max_count(void) { return node_id; }
