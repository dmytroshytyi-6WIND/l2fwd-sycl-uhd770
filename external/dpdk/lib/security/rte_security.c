...
#define RTE_PTR_OR_ERR_RET(ptr, retval)                                        \
  do {                                                                         \
    if ((ptr) == NULL)                                                         \
      return retval;                                                           \
  } while (0)
...
#define RTE_PTR_CHAIN3_OR_ERR_RET(p1, p2, p3, retval, last_retval)             \
  do {                                                                         \
...
int rte_security_dynfield_register(void) {
  ...
  rte_security_dynfield_offset = rte_mbuf_dynfield_register(&dynfield_desc);
  return rte_security_dynfield_offset;
}

struct rte_security_session *rte_security_session_create(
    struct rte_security_ctx *instance, struct rte_security_session_conf *conf,
    struct rte_mempool *mp, struct rte_mempool *priv_mp) {
...
int rte_security_session_update(struct rte_security_ctx *instance,
  ...
  RTE_PTR_CHAIN3_OR_ERR_RET(instance, ops, session_update, -EINVAL, -ENOTSUP);
...
int rte_security_session_destroy(struct rte_security_ctx *instance,
  ...
  RTE_PTR_CHAIN3_OR_ERR_RET(instance, ops, session_destroy, -EINVAL, -ENOTSUP);
  RTE_PTR_OR_ERR_RET(sess, -EINVAL);
...
void *__rte_security_get_userdata(struct rte_security_ctx *instance,
                                  uint64_t md) {
  void *userdata = NULL;
  ...
  while ((capability = &capabilities[i++])->action !=
    ...
    if (capability->action == idx->action &&
      ...
      if (idx->protocol == RTE_SECURITY_PROTOCOL_IPSEC) {
        if (capability->ipsec.proto == idx->ipsec.proto &&
            capability->ipsec.mode == idx->ipsec.mode &&
            capability->ipsec.direction == idx->ipsec.direction)
          return capability;
      } else if (idx->protocol == RTE_SECURITY_PROTOCOL_PDCP) {
        if (capability->pdcp.domain == idx->pdcp.domain)
          return capability;
      } else if (idx->protocol == RTE_SECURITY_PROTOCOL_DOCSIS) {
        if (capability->docsis.direction == idx->docsis.direction)
          return capability;
      }
    }
  }
...
static int security_handle_cryptodev_list(const char *cmd __rte_unused,
  ...
  for (dev_id = 0; dev_id < RTE_CRYPTO_MAX_DEVS; dev_id++)
    if (rte_cryptodev_is_valid_dev(dev_id) && rte_cryptodev_get_sec_ctx(dev_id))
...
#define SEC_CAPS_SZ                                                            \
  (RTE_ALIGN_CEIL(sizeof(struct rte_security_capability), sizeof(uint64_t)) /  \
   sizeof(uint64_t))

static int sec_caps_array(struct rte_tel_data *d,
                          const struct rte_security_capability *capabilities) {
...
static int security_capabilities_from_dev_id(int dev_id, const void **caps) {
  const struct rte_security_capability *capabilities;
...
static int security_handle_cryptodev_sec_caps(const char *cmd __rte_unused,
                                              const char *params,
                                              struct rte_tel_data *d) {
  const struct rte_security_capability *capabilities;
...
static int security_handle_cryptodev_crypto_caps(const char *cmd __rte_unused,
                                                 const char *params,
                                                 struct rte_tel_data *d) {
  const struct rte_security_capability *capabilities;
  ...
  rte_tel_data_start_dict(d);
  crypto_caps_n =
      crypto_caps_array(crypto_caps, capabilities->crypto_capabilities);
...
RTE_INIT(security_init_telemetry) {
  rte_telemetry_register_cmd(
      "/security/cryptodev/list", security_handle_cryptodev_list,
      "Returns list of available crypto devices by IDs. No parameters.");

  rte_telemetry_register_cmd(
      "/security/cryptodev/sec_caps", security_handle_cryptodev_sec_caps,
  ...
  rte_telemetry_register_cmd("/security/cryptodev/crypto_caps",
                             ...
                             "Returns crypto capabilities for a security "
                             "capability. Parameters: int dev_id, sec_cap_id");
}
