 ...
 * If the specified key was never inserted, a pseudorandom answer will be
 * returned. There is no way to know based on the lookup if the key was ever
 * inserted originally, so this must be tracked elsewhere.
 *
 * @param table
 *   EFD table to reference
 * @param socket_id
...
efd_value_t rte_efd_lookup(const struct rte_efd_table *table,
                           unsigned int socket_id, const void *key);
...
void rte_efd_lookup_bulk(const struct rte_efd_table *table,
                         unsigned int socket_id, int num_keys,
                         const void **key_list, efd_value_t *value_list);
