...
static struct pcapng_option *pcapng_add_option(struct pcapng_option *popt,
                                               uint16_t code, const void *data,
                                               uint16_t len) {
  popt->code = code;
  popt->length = len;
 ...
 */
static int pcapng_section_block(rte_pcapng_t *self, const char *os,
                                const char *hw, const char *app,
                                const char *comment) {
...
static int pcapng_add_interface(rte_pcapng_t *self, uint16_t port) {
  ...
  dev = dev_info.device;
  if (dev)
    snprintf(ifhw, sizeof(ifhw), "%s-%s", dev->bus->name, dev->name);

  /* DPDK reports in units of Mbps */
  if (rte_eth_link_get(port, &link) == 0 && link.link_status == RTE_ETH_LINK_UP)
  ...
  *hdr = (struct pcapng_interface_block){
  ...
  opt = pcapng_add_option(opt, PCAPNG_IFB_NAME, ifname, strlen(ifname));
  if (ea)
    opt = pcapng_add_option(opt, PCAPNG_IFB_MACADDR, ea, RTE_ETHER_ADDR_LEN);
  if (speed != 0)
    opt = pcapng_add_option(opt, PCAPNG_IFB_SPEED, &speed, sizeof(uint64_t));
  if (dev)
    opt = pcapng_add_option(opt, PCAPNG_IFB_HARDWARE, ifhw, strlen(ifhw));
  opt = pcapng_add_option(opt, PCAPNG_OPT_END, NULL, 0);
 ...
 * of the file.
 */
static int pcapng_interfaces(rte_pcapng_t *self) {
  uint16_t port_id;
  uint16_t index = 0;
 ...
 */
ssize_t rte_pcapng_write_stats(rte_pcapng_t *self, uint16_t port_id,
                               const char *comment, uint64_t start_time,
                               uint64_t end_time, uint64_t ifrecv,
                               uint64_t ifdrop) {
  ...
  if (comment)
    opt = pcapng_add_option(opt, PCAPNG_OPT_COMMENT, comment, strlen(comment));
  if (start_time != 0)
    opt = pcapng_add_option(opt, PCAPNG_ISB_STARTTIME, &start_time,
                            sizeof(start_time));
  if (end_time != 0)
    opt =
        pcapng_add_option(opt, PCAPNG_ISB_ENDTIME, &end_time, sizeof(end_time));
  if (ifrecv != UINT64_MAX)
    opt = pcapng_add_option(opt, PCAPNG_ISB_IFRECV, &ifrecv, sizeof(ifrecv));
  if (ifdrop != UINT64_MAX)
    opt = pcapng_add_option(opt, PCAPNG_ISB_IFDROP, &ifdrop, sizeof(ifdrop));
  if (optlen != 0)
    opt = pcapng_add_option(opt, PCAPNG_OPT_END, NULL, 0);
...
uint32_t rte_pcapng_mbuf_size(uint32_t length) {
  ...
  return sizeof(struct rte_mbuf) + RTE_ALIGN(length, sizeof(uint32_t)) +
...
static int pcapng_vlan_insert(struct rte_mbuf *m, uint16_t ether_type,
                              uint16_t tci) {
  ...
  nh = (struct rte_ether_hdr *)rte_pktmbuf_prepend(m,
                                                   sizeof(struct rte_vlan_hdr));
  if (nh == NULL)
    return -ENOSPC;
...
struct rte_mbuf *rte_pcapng_copy(uint16_t port_id, uint32_t queue,
  ...
  struct pcapng_option *opt;
  const uint16_t optlen =
      pcapng_optlen(sizeof(flags)) + pcapng_optlen(sizeof(queue));
  struct rte_mbuf *mc;
  uint64_t ns;
  ...
  if ((direction == RTE_PCAPNG_DIRECTION_IN &&
    ...
    if (pcapng_vlan_insert(mc, RTE_ETHER_TYPE_VLAN, md->vlan_tci) != 0)
      goto fail;
  }
  ...
  if ((direction == RTE_PCAPNG_DIRECTION_IN &&
    ...
    if (pcapng_vlan_insert(mc, RTE_ETHER_TYPE_QINQ, md->vlan_tci_outer) != 0)
      goto fail;
  }
  ...
  opt =
      (struct pcapng_option *)rte_pktmbuf_append(mc, optlen + sizeof(uint32_t));
  ...
  epb = (struct pcapng_enhance_packet_block *)rte_pktmbuf_prepend(mc,
                                                                  sizeof(*epb));
...
ssize_t rte_pcapng_write_packets(rte_pcapng_t *self, struct rte_mbuf *pkts[],
                                 uint16_t nb_pkts) {
...
rte_pcapng_t *rte_pcapng_fdopen(int fd, const char *osname,
                                const char *hardware, const char *appname,
                                const char *comment) {
  rte_pcapng_t *self;
