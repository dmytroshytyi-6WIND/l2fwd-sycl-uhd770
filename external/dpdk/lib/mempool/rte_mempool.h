...
#define RTE_MEMPOOL_HEADER_COOKIE1                                             \
  0xbadbadbadadd2e55ULL /**< Header cookie.                                    \
                         */
#define RTE_MEMPOOL_HEADER_COOKIE2                                             \
  0xf2eef2eedadd2e55ULL /**< Header cookie.                                    \
                         */
#define RTE_MEMPOOL_TRAILER_COOKIE                                             \
  0xadd2e55badbadbadULL /**< Trailer                                           \
                           cookie.*/
...
struct rte_mempool_debug_stats {
  ...
  uint64_t get_success_blks; /**< Successful allocation number of contiguous
                                blocks. */
...
#define RTE_MEMPOOL_NAMESIZE                                                   \
  (RTE_RING_NAMESIZE - sizeof(RTE_MEMPOOL_MZ_PREFIX) + 1)
...
#define RTE_MEMPOOL_VALID_USER_FLAGS                                           \
  (RTE_MEMPOOL_F_NO_SPREAD | RTE_MEMPOOL_F_NO_CACHE_ALIGN |                    \
   RTE_MEMPOOL_F_SP_PUT | RTE_MEMPOOL_F_SC_GET | RTE_MEMPOOL_F_NO_IOVA_CONTIG)
/**
 ...
 */
#ifdef RTE_LIBRTE_MEMPOOL_DEBUG
#define RTE_MEMPOOL_STAT_ADD(mp, name, n)                                      \
  do {                                                                         \
...
#define RTE_MEMPOOL_HEADER_SIZE(mp, cs)                                        \
  (sizeof(*(mp)) +                                                             \
   (((cs) == 0) ? 0 : (sizeof(struct rte_mempool_cache) * RTE_MAX_LCORE)))
...
static inline struct rte_mempool_objhdr *rte_mempool_get_header(void *obj) {
  return (struct rte_mempool_objhdr *)RTE_PTR_SUB(
      obj, sizeof(struct rte_mempool_objhdr));
}
...
#define RTE_MEMPOOL_CONTIG_BLOCKS_CHECK_COOKIES(mp, first_obj_table_const, n,  \
                                                free)                          \
  rte_mempool_contig_blocks_check_cookies(mp, first_obj_table_const, n, free)
#else
#define RTE_MEMPOOL_CONTIG_BLOCKS_CHECK_COOKIES(mp, first_obj_table_const, n,  \
                                                free)                          \
  do {                                                                         \
 ...
 */
typedef int (*rte_mempool_dequeue_t)(struct rte_mempool *mp, void **obj_table,
                                     unsigned int n);
 ...
 */
int rte_mempool_op_populate_helper(struct rte_mempool *mp, unsigned int flags,
                                   unsigned int max_objs, void *vaddr,
                                   rte_iova_t iova, size_t len,
                                   rte_mempool_populate_obj_cb_t *obj_cb,
                                   void *obj_cb_arg);
 ...
 */
static inline struct rte_mempool_ops *rte_mempool_get_ops(int ops_index) {
 ...
 */
static inline int rte_mempool_ops_dequeue_bulk(struct rte_mempool *mp,
 ...
 *   - <0: Error; code of dequeue function.
 */
static inline int rte_mempool_ops_dequeue_contig_blocks(struct rte_mempool *mp,
                                                        void **first_obj_table,
                                                        unsigned int n) {
  struct rte_mempool_ops *ops;
 ...
 */
static inline int rte_mempool_ops_enqueue_bulk(struct rte_mempool *mp,
...
#define RTE_MEMPOOL_REGISTER_OPS(ops)                                          \
  RTE_INIT(mp_hdlr_init_##ops) { rte_mempool_register_ops(&ops); }
...
rte_mempool_do_generic_get(struct rte_mempool *mp, void **obj_table,
  ...
  if (cache->len < n) {
    ...
    ret = rte_mempool_ops_dequeue_bulk(mp, &cache->objs[cache->len], req);
    if (unlikely(ret < 0)) {
      /*
 ...
 */
static __rte_always_inline int
rte_mempool_get_contig_blocks(struct rte_mempool *mp, void **first_obj_table,
                              unsigned int n) {
  int ret;
  ...
  if (ret == 0) {
    ...
    RTE_MEMPOOL_CONTIG_BLOCKS_CHECK_COOKIES(mp, first_obj_table, n, 1);
  } else {
    RTE_MEMPOOL_STAT_ADD(mp, get_fail_bulk, 1);
...
static inline rte_iova_t rte_mempool_virt2iova(const void *elt) {
  ...
  hdr = (const struct rte_mempool_objhdr *)RTE_PTR_SUB(elt, sizeof(*hdr));
  return hdr->iova;
}
...
static inline void *rte_mempool_get_priv(struct rte_mempool *mp) {
  return (char *)mp + RTE_MEMPOOL_HEADER_SIZE(mp, mp->cache_size);
}
