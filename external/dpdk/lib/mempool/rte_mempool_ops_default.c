...
ssize_t rte_mempool_op_calc_mem_size_helper(const struct rte_mempool *mp,
  ...
  if (total_elt_sz == 0) {
    ...
    if (obj_per_page == 0) {
       ...
       * at least one object.
       */
      mem_size = RTE_ALIGN_CEIL(total_elt_sz + chunk_reserve, pg_sz) * obj_num;
    } else {
      ...
      mem_size += ((obj_num - objs_in_last_page) / obj_per_page) << pg_shift;
...
ssize_t rte_mempool_op_calc_mem_size_default(const struct rte_mempool *mp,
                                             ...
                                             size_t *min_chunk_size,
                                             size_t *align) {
  return rte_mempool_op_calc_mem_size_helper(mp, obj_num, pg_shift, 0,
...
int rte_mempool_op_populate_default(struct rte_mempool *mp,
                                    ...
                                    rte_mempool_populate_obj_cb_t *obj_cb,
                                    void *obj_cb_arg) {
  return rte_mempool_op_populate_helper(mp, 0, max_objs, vaddr, iova, len,
                                        obj_cb, obj_cb_arg);
}
