...
static void mempool_event_callback_invoke(enum rte_mempool_event event,
...
static size_t get_min_page_size(int socket_id) {
...
uint32_t rte_mempool_calc_obj_size(uint32_t elt_size, uint32_t flags,
  ...
  if ((flags & RTE_MEMPOOL_F_NO_CACHE_ALIGN) == 0)
    sz->header_size = RTE_ALIGN_CEIL(sz->header_size, RTE_MEMPOOL_ALIGN);
  ...
  if ((flags & RTE_MEMPOOL_F_NO_CACHE_ALIGN) == 0) {
    ...
    sz->trailer_size +=
        ((RTE_MEMPOOL_ALIGN - (sz->total_size & RTE_MEMPOOL_ALIGN_MASK)) &
  ...
  if ((flags & RTE_MEMPOOL_F_NO_SPREAD) == 0) {
    unsigned new_size;
    new_size = arch_mem_object_align(sz->header_size + sz->elt_size +
                                     sz->trailer_size);
...
static void rte_mempool_free_memchunks(struct rte_mempool *mp) {
...
static int mempool_ops_alloc_once(struct rte_mempool *mp) {
  int ret;
...
int rte_mempool_populate_iova(struct rte_mempool *mp, char *vaddr,
  ...
  i = rte_mempool_ops_populate(
      mp, mp->size - mp->populated_size, (char *)vaddr + off,
      (iova == RTE_BAD_IOVA) ? RTE_BAD_IOVA : (iova + off), len - off,
      mempool_add_elem, NULL);
...
static rte_iova_t get_iova(void *addr) {
 ...
 */
int rte_mempool_populate_virt(struct rte_mempool *mp, char *addr, size_t len,
                              size_t pg_sz,
                              rte_mempool_memchunk_free_cb_t *free_cb,
                              void *opaque) {
  rte_iova_t iova;
  ...
  if (mp->flags & RTE_MEMPOOL_F_NO_IOVA_CONTIG)
    return rte_mempool_populate_iova(mp, addr, RTE_BAD_IOVA, len, free_cb,
                                     opaque);
  ...
  for (off = 0; off < len && mp->populated_size < mp->size; off += phys_len) {
    ...
    for (phys_len = RTE_MIN(
             (size_t)(RTE_PTR_ALIGN_CEIL(addr + off + 1, pg_sz) - (addr + off)),
             len - off);
         off + phys_len < len;
    ...
    ret = rte_mempool_populate_iova(mp, addr + off, iova, phys_len, free_cb,
                                    opaque);
    if (ret == 0)
      continue;
    if (ret < 0)
...
int rte_mempool_populate_default(struct rte_mempool *mp) {
  ...
  for (mz_id = 0, n = mp->size; n > 0; mz_id++, n -= ret) {
    ...
    mem_size =
        rte_mempool_ops_calc_mem_size(mp, n, pg_shift, &min_chunk_size, &align);
    ...
    ret = snprintf(mz_name, sizeof(mz_name), RTE_MEMPOOL_MZ_FORMAT "_%d",
                   mp->name, mz_id);
    ...
    do {
      mz = rte_memzone_reserve_aligned(
          mz_name, RTE_MIN((size_t)mem_size, max_alloc_size), mp->socket_id,
          mz_flags, align);

      if (mz != NULL || rte_errno != ENOMEM)
        break;

      max_alloc_size = RTE_MIN(max_alloc_size, (size_t)mem_size) / 2;
    ...
    if (pg_sz == 0 || (mz_flags & RTE_MEMZONE_IOVA_CONTIG))
      ret = rte_mempool_populate_iova(mp, mz->addr, iova, mz->len,
                                      rte_mempool_memchunk_mz_free,
                                      (void *)(uintptr_t)mz);
    else
      ret = rte_mempool_populate_virt(mp, mz->addr, mz->len, pg_sz,
                                      rte_mempool_memchunk_mz_free,
                                      (void *)(uintptr_t)mz);
...
static ssize_t get_anon_size(const struct rte_mempool *mp) {
  ssize_t size;
  ...
  pg_sz = rte_mem_page_size();
  pg_shift = rte_bsf32(pg_sz);
  size = rte_mempool_ops_calc_mem_size(mp, mp->size, pg_shift, &min_chunk_size,
                                       &align);
...
static void rte_mempool_memchunk_anon_free(struct rte_mempool_memhdr *memhdr,
                                           void *opaque) {
  ssize_t size;
...
struct rte_mempool *rte_mempool_create_empty(const char *name, unsigned n,
  ...
  RTE_BUILD_BUG_ON((sizeof(struct rte_mempool_cache) & RTE_CACHE_LINE_MASK) !=
                   0);
#ifdef RTE_LIBRTE_MEMPOOL_DEBUG
  RTE_BUILD_BUG_ON(
      (sizeof(struct rte_mempool_debug_stats) & RTE_CACHE_LINE_MASK) != 0);
  RTE_BUILD_BUG_ON(
      (offsetof(struct rte_mempool, stats) & RTE_CACHE_LINE_MASK) != 0);
#endif
   ...
   */
  private_data_size =
      (private_data_size + RTE_MEMPOOL_ALIGN_MASK) & (~RTE_MEMPOOL_ALIGN_MASK);
   ...
   */
  mp->local_cache = (struct rte_mempool_cache *)RTE_PTR_ADD(
      mp, RTE_MEMPOOL_HEADER_SIZE(mp, 0));
...
rte_mempool_create(const char *name, unsigned n, unsigned elt_size,
  ...
  rte_mempool_trace_create(name, n, elt_size, cache_size, private_data_size,
                           mp_init, mp_init_arg, obj_init, obj_init_arg, flags,
                           mp);
  return mp;
...
static unsigned rte_mempool_dump_cache(FILE *f, const struct rte_mempool *mp) {
  unsigned lcore_id;
  unsigned count = 0;
  ...
  while (n--) {
    ...
    if (free == 0) {
      if (cookie != RTE_MEMPOOL_HEADER_COOKIE1) {
        RTE_LOG(CRIT, MEMPOOL, "obj=%p, mempool=%p, cookie=%" PRIx64 "\n", obj,
...
void rte_mempool_contig_blocks_check_cookies(const struct rte_mempool *mp,
  ...
  const size_t total_elt_sz = mp->header_size + mp->elt_size + mp->trailer_size;
  unsigned int i, j;
...
#ifdef RTE_LIBRTE_MEMPOOL_DEBUG
static void mempool_obj_audit(struct rte_mempool *mp, __rte_unused void *opaque,
                              void *obj, __rte_unused unsigned idx) {
  RTE_MEMPOOL_CHECK_COOKIES(mp, &obj, 1, 2);
}

static void mempool_audit_cookies(struct rte_mempool *mp) {
  unsigned num;
...
#else
#define mempool_audit_cookies(mp)                                              \
  do {                                                                         \
  } while (0)
#endif
...
void rte_mempool_walk(void (*func)(struct rte_mempool *, void *), void *arg) {
  struct rte_tailq_entry *te = NULL;
  struct rte_mempool_list *mempool_list;
  void *tmp_te;
...
int rte_mempool_event_callback_register(rte_mempool_event_callback *func,
                                        void *user_data) {
  struct mempool_callback_list *list;
  struct rte_tailq_entry *te = NULL;
  ...
  if (te == NULL) {
    RTE_LOG(ERR, MEMPOOL, "Cannot allocate event callback tailq entry!\n");
    ret = -ENOMEM;
    goto exit;
  }
...
int rte_mempool_event_callback_unregister(rte_mempool_event_callback *func,
                                          void *user_data) {
  struct mempool_callback_list *list;
  struct rte_tailq_entry *te = NULL;
  struct mempool_callback_data *cb;
...
static void mempool_list_cb(struct rte_mempool *mp, void *arg) {
  struct rte_tel_data *d = (struct rte_tel_data *)arg;

  rte_tel_data_add_array_string(d, mp->name);
}

static int mempool_handle_list(const char *cmd __rte_unused,
...
static void mempool_info_cb(struct rte_mempool *mp, void *arg) {
...
RTE_INIT(mempool_init_telemetry) {
  rte_telemetry_register_cmd(
      "/mempool/list", mempool_handle_list,
