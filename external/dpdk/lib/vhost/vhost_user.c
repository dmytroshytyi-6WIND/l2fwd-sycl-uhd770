...
static int send_vhost_reply(struct virtio_net *dev, int sockfd,
                            struct vhu_msg_context *ctx);
static int read_vhost_message(struct virtio_net *dev, int sockfd,
                              struct vhu_msg_context *ctx);
 ...
 */
static int validate_msg_fds(struct virtio_net *dev, struct vhu_msg_context *ctx,
                            int expected_fds) {
  ...
  VHOST_LOG_CONFIG(ERR, "(%s) expect %d FDs for request %s, received %d\n",
                   ...
                   vhost_message_str[ctx->msg.request.master], ctx->fd_num);
  ...
  if (do_map) {
    for (i = 0; i < dev->nr_guest_pages; i++) {
      ...
      ret = rte_vfio_container_dma_map(RTE_VFIO_DEFAULT_CONTAINER_FD,
                                       page->host_user_addr, page->host_iova,
                                       page->size);
      if (ret) {
        /*
...
static void vhost_user_notify_queue_state(struct virtio_net *dev,
                                          uint16_t index, int enable) {
 ...
 */
static int vhost_user_set_owner(struct virtio_net **pdev,
                                struct vhu_msg_context *ctx,
...
static int vhost_user_reset_owner(struct virtio_net **pdev,
                                  struct vhu_msg_context *ctx,
                                  int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
 ...
 */
static int vhost_user_get_features(struct virtio_net **pdev,
                                   struct vhu_msg_context *ctx,
                                   int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  uint64_t features = 0;
 ...
 */
static int vhost_user_get_queue_num(struct virtio_net **pdev,
                                    struct vhu_msg_context *ctx,
                                    int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
 ...
 */
static int vhost_user_set_features(struct virtio_net **pdev,
                                   struct vhu_msg_context *ctx,
                                   int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  ...
  if (dev->features &
      ((1ULL << VIRTIO_NET_F_MRG_RXBUF) | (1ULL << VIRTIO_F_VERSION_1) |
  ...
  VHOST_LOG_CONFIG(
      DEBUG, "(%s) mergeable RX buffers %s, virtio 1 %s\n", dev->ifname,
 ...
 */
static int vhost_user_set_vring_num(struct virtio_net **pdev,
                                    struct vhu_msg_context *ctx,
                                    int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  ...
  if (vq_is_packed(dev)) {
    ...
    vq->shadow_used_packed = rte_malloc_socket(
        NULL, vq->size * sizeof(struct vring_used_elem_packed),
    ...
    vq->shadow_used_split =
        rte_malloc_socket(NULL, vq->size * sizeof(struct vring_used_elem),
                          RTE_CACHE_LINE_SIZE, vq->numa_node);
  ...
  vq->batch_copy_elems =
      rte_malloc_socket(NULL, vq->size * sizeof(struct batch_copy_elem),
                        RTE_CACHE_LINE_SIZE, vq->numa_node);
...
static struct virtio_net *numa_realloc(struct virtio_net *dev, int index) {
  ...
  if (vq->log_cache) {
    ...
    lc = rte_realloc_socket(vq->log_cache, sizeof(*lc) * VHOST_LOG_CACHE_NR, 0,
                            node);
    if (!lc) {
  ...
  if (vq->resubmit_inflight) {
    ...
    if (!ri) {
      VHOST_LOG_CONFIG(ERR,
                       "(%s) failed to realloc resubmit inflight on node %d\n",
                       dev->ifname, node);
      return dev;
    }
    ...
    if (ri->resubmit_list) {
      ...
      if (!rd) {
        VHOST_LOG_CONFIG(ERR,
                         "(%s) failed to realloc resubmit list on node %d\n",
                         dev->ifname, node);
        return dev;
      }
      ri->resubmit_list = rd;
...
#else
static struct virtio_net *numa_realloc(struct virtio_net *dev,
...
static uint64_t qva_to_vva(struct virtio_net *dev, uint64_t qva,
                           uint64_t *len) {
 ...
 */
static uint64_t ring_addr_to_vva(struct virtio_net *dev,
                                 struct vhost_virtqueue *vq, uint64_t ra,
                                 uint64_t *size) {
...
static uint64_t log_addr_to_gpa(struct virtio_net *dev,
                                struct vhost_virtqueue *vq) {
  uint64_t log_gpa;
...
static struct virtio_net *translate_ring_addresses(struct virtio_net *dev,
  ...
  if (vq_is_packed(dev)) {
    ...
    vq->desc_packed = (struct vring_packed_desc *)(uintptr_t)ring_addr_to_vva(
    ...
    vq->driver_event =
        (struct vring_packed_desc_event *)(uintptr_t)ring_addr_to_vva(
            dev, vq, addr->avail_user_addr, &len);
    ...
    vq->device_event =
        (struct vring_packed_desc_event *)(uintptr_t)ring_addr_to_vva(
            dev, vq, addr->used_user_addr, &len);
  ...
  vq->desc = (struct vring_desc *)(uintptr_t)ring_addr_to_vva(
      dev, vq, addr->desc_user_addr, &len);
  ...
  if (dev->features & (1ULL << VIRTIO_RING_F_EVENT_IDX))
    len += sizeof(uint16_t);
  expected_len = len;
  vq->avail = (struct vring_avail *)(uintptr_t)ring_addr_to_vva(
      dev, vq, addr->avail_user_addr, &len);
  ...
  if (dev->features & (1ULL << VIRTIO_RING_F_EVENT_IDX))
    len += sizeof(uint16_t);
  expected_len = len;
  vq->used = (struct vring_used *)(uintptr_t)ring_addr_to_vva(
      dev, vq, addr->used_user_addr, &len);
  ...
  if (vq->last_used_idx != vq->used->idx) {
    VHOST_LOG_CONFIG(
        WARNING, "(%s) last_used_idx (%u) and vq->used->idx (%u) mismatches;\n",
        dev->ifname, vq->last_used_idx, vq->used->idx);
    vq->last_used_idx = vq->used->idx;
    vq->last_avail_idx = vq->used->idx;
    VHOST_LOG_CONFIG(
        WARNING, "(%s) some packets maybe resent for Tx and dropped for Rx\n",
        dev->ifname);
  }
 ...
 */
static int vhost_user_set_vring_addr(struct virtio_net **pdev,
                                     struct vhu_msg_context *ctx,
                                     int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  struct vhost_virtqueue *vq;
 ...
 */
static int vhost_user_set_vring_base(struct virtio_net **pdev,
                                     struct vhu_msg_context *ctx,
                                     int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  struct vhost_virtqueue *vq = dev->virtqueue[ctx->msg.payload.state.index];
  ...
  VHOST_LOG_CONFIG(
      INFO, "(%s) vring base idx:%u last_used_idx:%u last_avail_idx:%u.\n",
...
static int add_one_guest_page(struct virtio_net *dev, uint64_t guest_phys_addr,
                              uint64_t host_iova, uint64_t host_user_addr,
                              uint64_t size) {
  ...
  if (dev->nr_guest_pages == dev->max_guest_pages) {
    ...
    dev->guest_pages =
        rte_realloc(dev->guest_pages, dev->max_guest_pages * sizeof(*page),
                    RTE_CACHE_LINE_SIZE);
...
static int add_guest_pages(struct virtio_net *dev,
                           struct rte_vhost_mem_region *reg,
                           uint64_t page_size) {
  ...
  if (add_one_guest_page(dev, guest_phys_addr, host_iova, host_user_addr,
                         size) < 0)
    return -1;
...
vhost_user_postcopy_region_register(struct virtio_net *dev,
  ...
  if (ioctl(dev->postcopy_ufd, UFFDIO_REGISTER, &reg_struct)) {
    VHOST_LOG_CONFIG(ERR,
                     "(%s) failed to register ufd for region "
                     "%" PRIx64 " - %" PRIx64 " (ufd = %d) %s\n",
                     dev->ifname, (uint64_t)reg_struct.range.start,
  ...
  VHOST_LOG_CONFIG(
      ...
      dev->ifname, (uint64_t)reg_struct.range.start,
      (uint64_t)reg_struct.range.start + (uint64_t)reg_struct.range.len - 1);

  return 0;
}
#else
static int vhost_user_postcopy_region_register(
...
static int vhost_user_postcopy_register(struct virtio_net *dev, int main_fd,
                                        struct vhu_msg_context *ctx) {
  ...
  if (read_vhost_message(dev, main_fd, &ack_ctx) <= 0) {
    VHOST_LOG_CONFIG(ERR,
                     "(%s) failed to read qemu ack on postcopy set-mem-table\n",
                     dev->ifname);
    return -1;
  }
...
static int vhost_user_mmap_region(struct virtio_net *dev,
  ...
  if (mmap_offset >= -region->size) {
    VHOST_LOG_CONFIG(ERR,
                     "(%s) mmap_offset (%#" PRIx64
                     ") and memory_size (%#" PRIx64 ") overflow\n",
  ...
  if (mmap_size == 0) {
     ...
     */
    VHOST_LOG_CONFIG(ERR,
                     "(%s) mmap size (0x%" PRIx64 ") or alignment (0x%" PRIx64
                     ") is invalid\n",
...
static int vhost_user_set_mem_table(struct virtio_net **pdev,
                                    struct vhu_msg_context *ctx, int main_fd) {
  struct virtio_net *dev = *pdev;
  ...
  if (dev->guest_pages == NULL) {
    ...
    dev->guest_pages = rte_zmalloc_socket(
        NULL, dev->max_guest_pages * sizeof(struct guest_page),
  ...
  dev->mem = rte_zmalloc_socket("vhost-mem-table",
                                sizeof(struct rte_vhost_memory) +
...
static bool vq_is_ready(struct virtio_net *dev, struct vhost_virtqueue *vq) {
  ...
  return rings_ok && vq->kickfd != VIRTIO_UNINITIALIZED_EVENTFD &&
         vq->callfd != VIRTIO_UNINITIALIZED_EVENTFD && vq->enabled;
}
...
static void *inflight_mem_alloc(struct virtio_net *dev, const char *name,
                                size_t size, int *fd) {
  void *ptr;
  int mfd = -1;
...
static uint32_t get_pervq_shm_size_packed(uint16_t queue_size) {
  return RTE_ALIGN_MUL_CEIL(
      sizeof(struct rte_vhost_inflight_desc_packed) * queue_size +
...
static int vhost_user_get_inflight_fd(struct virtio_net **pdev,
  ...
  if (dev->inflight_info == NULL) {
    dev->inflight_info = rte_zmalloc_socket(
        "inflight_info", sizeof(struct inflight_mem_info), 0, numa_node);
    if (!dev->inflight_info) {
...
static int vhost_user_set_inflight_fd(struct virtio_net **pdev,
                                      struct vhu_msg_context *ctx,
                                      int main_fd __rte_unused) {
  ...
  addr =
      mmap(0, mmap_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, mmap_offset);
  if (addr == MAP_FAILED) {
...
static int vhost_user_set_vring_call(struct virtio_net **pdev,
                                     struct vhu_msg_context *ctx,
                                     int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
...
static int vhost_check_queue_inflights_split(struct virtio_net *dev,
  ...
  if (resubmit_num) {
    resubmit = rte_zmalloc_socket(
        "resubmit", sizeof(struct rte_vhost_resubmit_info), 0, vq->numa_node);
    if (!resubmit) {
      VHOST_LOG_CONFIG(ERR,
    ...
    resubmit->resubmit_list = rte_zmalloc_socket(
        "resubmit_list", resubmit_num * sizeof(struct rte_vhost_resubmit_desc),
        0, vq->numa_node);
    ...
    if (resubmit->resubmit_num > 1)
      qsort(resubmit->resubmit_list, resubmit->resubmit_num,
            sizeof(struct rte_vhost_resubmit_desc), resubmit_desc_compare);
...
static int vhost_check_queue_inflights_packed(struct virtio_net *dev,
                                              struct vhost_virtqueue *vq) {
  uint16_t i;
...
static int vhost_user_set_vring_kick(struct virtio_net **pdev,
                                     struct vhu_msg_context *ctx,
                                     int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  struct vhost_vring_file file;
 ...
 */
static int vhost_user_get_vring_base(struct virtio_net **pdev,
                                     struct vhu_msg_context *ctx,
                                     int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  struct vhost_virtqueue *vq = dev->virtqueue[ctx->msg.payload.state.index];
 ...
 */
static int vhost_user_set_vring_enable(struct virtio_net **pdev,
                                       struct vhu_msg_context *ctx,
                                       int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
...
static int vhost_user_get_protocol_features(struct virtio_net **pdev,
                                            struct vhu_msg_context *ctx,
                                            int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
...
static int vhost_user_set_protocol_features(struct virtio_net **pdev,
                                            struct vhu_msg_context *ctx,
                                            int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  ...
  VHOST_LOG_CONFIG(
      INFO, "(%s) negotiated Vhost-user protocol features: 0x%" PRIx64 "\n",
      dev->ifname, dev->protocol_features);

  return RTE_VHOST_MSG_RESULT_OK;
}

static int vhost_user_set_log_base(struct virtio_net **pdev,
                                   struct vhu_msg_context *ctx,
                                   int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  int fd = ctx->fds[0];
  ...
  if (off >= -size) {
    VHOST_LOG_CONFIG(ERR,
                     "(%s) log offset %#" PRIx64 " and log size %#" PRIx64
                     " overflow\n",
                     dev->ifname, off, size);
    goto close_msg_fds;
  }
  ...
  VHOST_LOG_CONFIG(INFO,
                   "(%s) log mmap size: %" PRId64 ", offset: %" PRId64 "\n",
                   dev->ifname, size, off);
  ...
  for (i = 0; i < dev->nr_vring; i++) {
    ...
    vq->log_cache = rte_malloc_socket(
        "vq log cache", sizeof(struct log_cache_entry) * VHOST_LOG_CACHE_NR, 0,
        vq->numa_node);
    /*
 ...
 */
static int vhost_user_send_rarp(struct virtio_net **pdev,
                                struct vhu_msg_context *ctx,
                                int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
...
static int vhost_user_net_set_mtu(struct virtio_net **pdev,
                                  struct vhu_msg_context *ctx,
                                  int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
...
static int vhost_user_set_req_fd(struct virtio_net **pdev,
                                 struct vhu_msg_context *ctx,
                                 int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  int fd = ctx->fds[0];
  ...
  if (fd < 0) {
    VHOST_LOG_CONFIG(ERR,
                     "(%s) invalid file descriptor for slave channel (%d)\n",
                     dev->ifname, fd);
    return RTE_VHOST_MSG_RESULT_ERR;
  }
...
static int is_vring_iotlb_split(struct vhost_virtqueue *vq,
...
static int is_vring_iotlb_packed(struct vhost_virtqueue *vq,
                                 struct vhost_iotlb_msg *imsg) {
  struct vhost_vring_addr *ra;
  uint64_t start, end, len;
...
static int is_vring_iotlb(struct virtio_net *dev, struct vhost_virtqueue *vq,
                          struct vhost_iotlb_msg *imsg) {
  if (vq_is_packed(dev))
...
static int vhost_user_iotlb_msg(struct virtio_net **pdev,
                                struct vhu_msg_context *ctx,
                                int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
...
static int vhost_user_set_postcopy_advise(struct virtio_net **pdev,
                                          struct vhu_msg_context *ctx,
                                          int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
#ifdef RTE_LIBRTE_VHOST_POSTCOPY
...
vhost_user_set_postcopy_listen(struct virtio_net **pdev,
  ...
  if (dev->mem && dev->mem->nregions) {
    VHOST_LOG_CONFIG(ERR,
                     "(%s) regions already registered at postcopy-listen\n",
                     dev->ifname);
    return RTE_VHOST_MSG_RESULT_ERR;
  }
...
static int vhost_user_postcopy_end(struct virtio_net **pdev,
                                   struct vhu_msg_context *ctx,
                                   int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
...
static int vhost_user_get_status(struct virtio_net **pdev,
                                 struct vhu_msg_context *ctx,
                                 int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
...
static int vhost_user_set_status(struct virtio_net **pdev,
                                 struct vhu_msg_context *ctx,
                                 int main_fd __rte_unused) {
  struct virtio_net *dev = *pdev;
  ...
  if ((dev->status & VIRTIO_DEVICE_STATUS_FEATURES_OK) &&
      (dev->flags & VIRTIO_DEV_FEATURES_FAILED)) {
    VHOST_LOG_CONFIG(
        ERR, "(%s) FEATURES_OK bit is set but feature negotiation failed\n",
        dev->ifname);
    /*
...
static int read_vhost_message(struct virtio_net *dev, int sockfd,
  ...
  ret = read_fd_message(dev->ifname, sockfd, (char *)&ctx->msg,
                        VHOST_USER_HDR_SIZE, ctx->fds,
                        VHOST_MEMORY_MAX_NREGIONS, &ctx->fd_num);
  if (ret <= 0) {
    return ret;
...
static int send_vhost_message(struct virtio_net *dev, int sockfd,
                              struct vhu_msg_context *ctx) {
  if (!ctx)
    return 0;

  return send_fd_message(dev->ifname, sockfd, (char *)&ctx->msg,
                         VHOST_USER_HDR_SIZE + ctx->msg.size, ctx->fds,
                         ctx->fd_num);
}
...
static int send_vhost_slave_message(struct virtio_net *dev,
                                    struct vhu_msg_context *ctx) {
  int ret;
 ...
 */
static int vhost_user_check_and_alloc_queue_pair(struct virtio_net *dev,
                                                 struct vhu_msg_context *ctx) {
  uint32_t vring_idx;
...
static int process_slave_message_reply(struct virtio_net *dev,
  ...
  if (msg_reply.msg.request.slave != ctx->msg.request.slave) {
    VHOST_LOG_CONFIG(
        ERR, "(%s) received unexpected msg type (%u), expected %u\n",
...
static int vhost_user_slave_config_change(struct virtio_net *dev,
                                          bool need_reply) {
  int ret;
  struct vhu_msg_context ctx = {
      .msg = {
...
int rte_vhost_host_notifier_ctrl(int vid, uint16_t qid, bool enable) {
  ...
  if (!(dev->features & (1ULL << VIRTIO_F_VERSION_1)) ||
      !(dev->features & (1ULL << VHOST_USER_F_PROTOCOL_FEATURES)) ||
      !(dev->protocol_features & (1ULL << VHOST_USER_PROTOCOL_F_SLAVE_REQ)) ||
      !(dev->protocol_features &
        (1ULL << VHOST_USER_PROTOCOL_F_SLAVE_SEND_FD)) ||
      !(dev->protocol_features & (1ULL << VHOST_USER_PROTOCOL_F_HOST_NOTIFIER)))
    return -ENOTSUP;
  ...
  if (enable) {
    for (i = q_start; i <= q_last; i++) {
      ...
      if (vhost_user_slave_set_vring_host_notifier(dev, i, vfio_device_fd,
                                                   offset, size) < 0) {
        ret = -EFAULT;
        goto disable;
      }
