...
#define VHOST_DMA_MAX_COPY_COMPLETE                                            \
  ((VIRTIO_MAX_RX_PKTLEN / RTE_MBUF_DEFAULT_DATAROOM) * MAX_PKT_BURST)
...
#define PACKED_DESC_SINGLE_DEQUEUE_FLAG                                        \
  (VRING_DESC_F_NEXT | VRING_DESC_F_INDIRECT)

#define PACKED_BATCH_SIZE                                                      \
  (RTE_CACHE_LINE_SIZE / sizeof(struct vring_packed_desc))
#define PACKED_BATCH_MASK (PACKED_BATCH_SIZE - 1)

#ifdef VHOST_GCC_UNROLL_PRAGMA
#define vhost_for_each_try_unroll(iter, val, size)                             \
  _Pragma("GCC unroll 4") for (iter = val; iter < size; iter++)
#endif

#ifdef VHOST_CLANG_UNROLL_PRAGMA
#define vhost_for_each_try_unroll(iter, val, size)                             \
  _Pragma("unroll 4") for (iter = val; iter < size; iter++)
#endif
...
#define VIRTIO_NET_SUPPORTED_FEATURES                                          \
  ((1ULL << VIRTIO_NET_F_MRG_RXBUF) | (1ULL << VIRTIO_F_ANY_LAYOUT) |          \
   (1ULL << VIRTIO_NET_F_CTRL_VQ) | (1ULL << VIRTIO_NET_F_CTRL_RX) |           \
   (1ULL << VIRTIO_NET_F_GUEST_ANNOUNCE) | (1ULL << VIRTIO_NET_F_MQ) |         \
   (1ULL << VIRTIO_F_VERSION_1) | (1ULL << VHOST_F_LOG_ALL) |                  \
   (1ULL << VHOST_USER_F_PROTOCOL_FEATURES) | (1ULL << VIRTIO_NET_F_GSO) |     \
   (1ULL << VIRTIO_NET_F_HOST_TSO4) | (1ULL << VIRTIO_NET_F_HOST_TSO6) |       \
   (1ULL << VIRTIO_NET_F_HOST_UFO) | (1ULL << VIRTIO_NET_F_HOST_ECN) |         \
   (1ULL << VIRTIO_NET_F_CSUM) | (1ULL << VIRTIO_NET_F_GUEST_CSUM) |           \
   (1ULL << VIRTIO_NET_F_GUEST_TSO4) | (1ULL << VIRTIO_NET_F_GUEST_TSO6) |     \
   (1ULL << VIRTIO_NET_F_GUEST_UFO) | (1ULL << VIRTIO_NET_F_GUEST_ECN) |       \
   (1ULL << VIRTIO_RING_F_INDIRECT_DESC) | (1ULL << VIRTIO_RING_F_EVENT_IDX) | \
   (1ULL << VIRTIO_NET_F_MTU) | (1ULL << VIRTIO_F_IN_ORDER) |                  \
   (1ULL << VIRTIO_F_IOMMU_PLATFORM) | (1ULL << VIRTIO_F_RING_PACKED))
...
static __rte_always_inline bool vq_is_packed(struct virtio_net *dev) {
  return dev->features & (1ull << VIRTIO_F_RING_PACKED);
}

static inline bool desc_is_avail(struct vring_packed_desc *desc,
                                 bool wrap_counter) {
...
static inline void vq_inc_last_used_packed(struct vhost_virtqueue *vq,
                                           uint16_t num) {
...
static inline void vq_inc_last_avail_packed(struct vhost_virtqueue *vq,
                                            uint16_t num) {
...
void __vhost_log_cache_write_iova(struct virtio_net *dev,
                                  ...
                                  uint64_t len);
void __vhost_log_cache_sync(struct virtio_net *dev, struct vhost_virtqueue *vq);
...
static __rte_always_inline void vhost_log_write(struct virtio_net *dev,
                                                uint64_t addr, uint64_t len) {
...
vhost_log_cache_used_vring(struct virtio_net *dev, struct vhost_virtqueue *vq,
  ...
  if (unlikely(dev->features & (1ULL << VHOST_F_LOG_ALL))) {
    if (unlikely(vq->log_guest_addr == 0))
      return;
    __vhost_log_cache_write(dev, vq, vq->log_guest_addr + offset, len);
  }
}

static __rte_always_inline void vhost_log_used_vring(struct virtio_net *dev,
...
static __rte_always_inline void vhost_log_write_iova(struct virtio_net *dev,
                                                     struct vhost_virtqueue *vq,
                                                     uint64_t iova,
                                                     uint64_t len) {
  if (likely(!(dev->features & (1ULL << VHOST_F_LOG_ALL))))
...
#define VHOST_LOG_CONFIG(level, fmt, args...)                                  \
  rte_log(RTE_LOG_##level, vhost_config_log_level, "VHOST_CONFIG: " fmt, ##args)
...
#define PRINT_PACKET(device, addr, size, header)                               \
  do {                                                                         \
    ...
    if ((header))                                                              \
      snprintf(packet, VHOST_MAX_PRINT_BUFF,                                   \
               "(%d) Header size %d: ", (device->vid), (size));                \
    else                                                                       \
      snprintf(packet, VHOST_MAX_PRINT_BUFF,                                   \
               "(%d) Packet size %d: ", (device->vid), (size));                \
...
static __rte_always_inline rte_iova_t gpa_to_first_hpa(struct virtio_net *dev,
                                                       uint64_t gpa,
                                                       uint64_t gpa_size,
                                                       uint64_t *hpa_size) {
  uint32_t i;
  ...
  *hpa_size = gpa_size;
  if (dev->nr_guest_pages >= VHOST_BINARY_SEARCH_THRESH) {
    ...
    if (page) {
      if (gpa + gpa_size <= page->guest_phys_addr + page->size) {
        return gpa - page->guest_phys_addr + page->host_iova;
      } else if (gpa < page->guest_phys_addr + page->size) {
        *hpa_size = page->guest_phys_addr + page->size - gpa;
        return gpa - page->guest_phys_addr + page->host_iova;
      }
    }
  } else {
...
static __rte_always_inline rte_iova_t gpa_to_hpa(struct virtio_net *dev,
...
static __rte_always_inline uint64_t hva_to_gpa(struct virtio_net *dev,
                                               uint64_t vva, uint64_t len) {
  ...
  for (i = 0; i < dev->mem->nregions; i++) {
    ...
    if (vva >= r->host_user_addr && vva + len < r->host_user_addr + r->size) {
...
static __rte_always_inline struct virtio_net *get_device(int vid) {
  struct virtio_net *dev = vhost_devices[vid];

  if (unlikely(!dev)) {
    VHOST_LOG_CONFIG(ERR, "(%d) device not found.\n", vid);
  }
...
static __rte_always_inline void free_ind_table(void *idesc) { rte_free(idesc); }

static __rte_always_inline void restore_mbuf(struct rte_mbuf *m) {
...
static __rte_always_inline bool mbuf_is_consumed(struct rte_mbuf *m) {
  while (m) {
