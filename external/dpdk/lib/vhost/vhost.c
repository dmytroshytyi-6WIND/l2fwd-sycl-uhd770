 ...
 */
static __rte_always_inline void vhost_set_bit(unsigned int nr,
...
static __rte_always_inline void vhost_log_page(uint8_t *log_base,
                                               uint64_t page) {
...
void __vhost_log_write_iova(struct virtio_net *dev, struct vhost_virtqueue *vq,
  ...
  if (map_len != len) {
    VHOST_LOG_DATA(ERR,
                   "(%s) failed to write log for IOVA 0x%" PRIx64
...
#else
    __atomic_fetch_or(log_base + elem->offset, elem->val, __ATOMIC_RELAXED);
#endif
  }
...
static __rte_always_inline void vhost_log_cache_page(struct virtio_net *dev,
                                                     struct vhost_virtqueue *vq,
                                                     uint64_t page) {
...
void __vhost_log_cache_write(struct virtio_net *dev, struct vhost_virtqueue *vq,
...
void __vhost_log_cache_write_iova(struct virtio_net *dev,
                                  struct vhost_virtqueue *vq, uint64_t iova,
                                  uint64_t len) {
  uint64_t hva, gpa, map_len;
  map_len = len;
...
void *vhost_alloc_copy_ind_table(struct virtio_net *dev,
                                 struct vhost_virtqueue *vq, uint64_t desc_addr,
                                 uint64_t desc_len) {
  void *idesc;
  uint64_t src, dst;
  ...
  while (remain) {
    len = remain;
    src = vhost_iova_to_vva(dev, vq, desc_addr, &len, VHOST_ACCESS_RO);
    if (unlikely(!src || !len)) {
      rte_free(idesc);
      return NULL;
    }
 ...
 */
static void free_device(struct virtio_net *dev) {
  uint32_t i;
...
static __rte_always_inline int log_translate(struct virtio_net *dev,
                                             struct vhost_virtqueue *vq) {
  if (likely(!(vq->ring_addrs.flags & (1 << VHOST_VRING_F_LOG))))
    return 0;

  vq->log_guest_addr =
      translate_log_addr(dev, vq, vq->ring_addrs.log_guest_addr);
...
uint64_t translate_log_addr(struct virtio_net *dev, struct vhost_virtqueue *vq,
  ...
  if (dev->features & (1ULL << VIRTIO_F_IOMMU_PLATFORM)) {
    ...
    hva = vhost_iova_to_vva(dev, vq, log_addr, &size, VHOST_ACCESS_RW);
    ...
    if (!gpa) {
      VHOST_LOG_DATA(ERR,
                     "(%s) failed to find GPA for log_addr: 0x%" PRIx64
                     " hva: 0x%" PRIx64 "\n",
...
/* Caller should have iotlb_lock read-locked */
static int vring_translate_split(struct virtio_net *dev,
                                 struct vhost_virtqueue *vq) {
  uint64_t req_size, size;

  req_size = sizeof(struct vring_desc) * vq->size;
  size = req_size;
  vq->desc = (struct vring_desc *)(uintptr_t)vhost_iova_to_vva(
      dev, vq, vq->ring_addrs.desc_user_addr, &size, VHOST_ACCESS_RW);
  ...
  size = req_size;
  vq->avail = (struct vring_avail *)(uintptr_t)vhost_iova_to_vva(
      dev, vq, vq->ring_addrs.avail_user_addr, &size, VHOST_ACCESS_RW);
  ...
  if (dev->features & (1ULL << VIRTIO_RING_F_EVENT_IDX))
    req_size += sizeof(uint16_t);
  size = req_size;
  vq->used = (struct vring_used *)(uintptr_t)vhost_iova_to_vva(
      dev, vq, vq->ring_addrs.used_user_addr, &size, VHOST_ACCESS_RW);
...
/* Caller should have iotlb_lock read-locked */
static int vring_translate_packed(struct virtio_net *dev,
                                  struct vhost_virtqueue *vq) {
  uint64_t req_size, size;

  req_size = sizeof(struct vring_packed_desc) * vq->size;
  size = req_size;
  vq->desc_packed = (struct vring_packed_desc *)(uintptr_t)vhost_iova_to_vva(
      dev, vq, vq->ring_addrs.desc_user_addr, &size, VHOST_ACCESS_RW);
  ...
  size = req_size;
  vq->driver_event =
      (struct vring_packed_desc_event *)(uintptr_t)vhost_iova_to_vva(
          dev, vq, vq->ring_addrs.avail_user_addr, &size, VHOST_ACCESS_RW);
...
static void init_vring_queue(struct virtio_net *dev, uint32_t vring_idx) {
  struct vhost_virtqueue *vq;
...
static void reset_vring_queue(struct virtio_net *dev, uint32_t vring_idx) {
  struct vhost_virtqueue *vq;
  int callfd;

  if (vring_idx >= VHOST_MAX_VRING) {
    VHOST_LOG_CONFIG(ERR, "(%s) failed to reset vring, out of bound (%d)\n",
                     dev->ifname, vring_idx);
    return;
  }
  ...
  if (!vq) {
    VHOST_LOG_CONFIG(
        ERR, "(%s) failed to reset vring, virtqueue not allocated (%d)\n",
        dev->ifname, vring_idx);
    return;
  }
...
void vhost_attach_vdpa_device(int vid, struct rte_vdpa_device *vdpa_dev) {
  struct virtio_net *dev = get_device(vid);
...
void vhost_setup_virtio_net(int vid, bool enable, bool compliant_ol_flags) {
  struct virtio_net *dev = get_device(vid);
...
int rte_vhost_get_numa_node(int vid) {
  ...
  ret = get_mempolicy(&numa_node, NULL, 0, dev, MPOL_F_NODE | MPOL_F_ADDR);
  if (ret < 0) {
    VHOST_LOG_CONFIG(ERR, "(%s) failed to query numa node: %s\n", dev->ifname,
                     rte_strerror(errno));
    return -1;
...
int rte_vhost_set_last_inflight_io_packed(int vid, uint16_t vring_idx,
  ...
  if (inflight_info->used_idx >= inflight_info->desc_num) {
    ...
    inflight_info->used_wrap_counter = !inflight_info->used_wrap_counter;
  }
...
static inline int vhost_enable_notify_split(struct virtio_net *dev,
                                            struct vhost_virtqueue *vq,
                                            int enable) {
  if (vq->used == NULL)
    return -1;
...
static inline int vhost_enable_notify_packed(struct virtio_net *dev,
                                             struct vhost_virtqueue *vq,
                                             int enable) {
  uint16_t flags;
  ...
  if (dev->features & (1ULL << VIRTIO_RING_F_EVENT_IDX)) {
    ...
    vq->device_event->off_wrap = vq->last_avail_idx | vq->avail_wrap_counter
                                                          << 15;
  }
...
int rte_vhost_enable_guest_notification(int vid, uint16_t queue_id,
                                        int enable) {
  struct virtio_net *dev = get_device(vid);
  struct vhost_virtqueue *vq;
  int ret;
...
void rte_vhost_log_used_vring(int vid, uint16_t vring_idx, uint64_t offset,
                              uint64_t len) {
  struct virtio_net *dev;
  struct vhost_virtqueue *vq;
...
int rte_vhost_get_vring_base(int vid, uint16_t queue_id,
  ...
  if (vq_is_packed(dev)) {
    *last_avail_idx = (vq->avail_wrap_counter << 15) | vq->last_avail_idx;
    *last_used_idx = (vq->used_wrap_counter << 15) | vq->last_used_idx;
  } else {
...
int rte_vhost_extern_callback_register(
    int vid, struct rte_vhost_user_extern_ops const *const ops, void *ctx) {
  struct virtio_net *dev = get_device(vid);
...
static __rte_always_inline int async_channel_register(int vid,
                                                      uint16_t queue_id) {
  struct virtio_net *dev = get_device(vid);
  ...
  if (unlikely(vq->async)) {
    VHOST_LOG_CONFIG(
        ERR, "(%s) async register failed: already registered (qid: %d)\n",
        dev->ifname, queue_id);
    return -1;
  }
  ...
  async->pkts_info =
      rte_malloc_socket(NULL, vq->size * sizeof(struct async_inflight_info),
  ...
  if (!async->pkts_cmpl_flag) {
    VHOST_LOG_CONFIG(ERR,
                     "(%s) failed to allocate async pkts_cmpl_flag (qid: %d)\n",
                     dev->ifname, queue_id);
    goto out_free_async;
  }

  if (vq_is_packed(dev)) {
    async->buffers_packed = rte_malloc_socket(
...
int rte_vhost_async_channel_unregister(int vid, uint16_t queue_id) {
  ...
  if (!rte_spinlock_trylock(&vq->access_lock)) {
    VHOST_LOG_CONFIG(
        ERR, "(%s) failed to unregister async channel, virtqueue busy.\n",
        dev->ifname);
    return -1;
  }
  ...
  if (vq->async->pkts_inflight_n) {
    VHOST_LOG_CONFIG(ERR, "(%s) failed to unregister async channel.\n",
                     dev->ifname);
    VHOST_LOG_CONFIG(
        ERR, "(%s) inflight packets must be completed before unregistration.\n",
        dev->ifname);
    ret = -1;
    goto out;
  }
...
int rte_vhost_async_dma_configure(int16_t dma_id, uint16_t vchan_id) {
  ...
  pkts_cmpl_flag_addr =
      rte_zmalloc(NULL, sizeof(bool *) * max_desc, RTE_CACHE_LINE_SIZE);
  if (!pkts_cmpl_flag_addr) {
    VHOST_LOG_CONFIG(ERR,
                     "Failed to allocate pkts_cmpl_flag_addr for DMA %d "
                     "vChannel %u.\n",
                     dma_id, vchan_id);
...
int rte_vhost_async_get_inflight(int vid, uint16_t queue_id) {
  ...
  if (!rte_spinlock_trylock(&vq->access_lock)) {
    VHOST_LOG_CONFIG(
        DEBUG, "(%s) failed to check in-flight packets. virtqueue busy.\n",
        dev->ifname);
    return ret;
  }
...
int rte_vhost_get_monitor_addr(int vid, uint16_t queue_id,
