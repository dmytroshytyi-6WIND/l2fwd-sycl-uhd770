...
static int get_last_valid_idx(struct fdset *pfdset, int last_valid_idx) {
  int i;
...
static void fdset_move(struct fdset *pfdset, int dst, int src) {
  pfdset->fd[dst] = pfdset->fd[src];
  pfdset->rwfds[dst] = pfdset->rwfds[src];
}

static void fdset_shrink_nolock(struct fdset *pfdset) {
  int i;
 ...
 */
static void fdset_shrink(struct fdset *pfdset) {
 ...
 */
static int fdset_find_fd(struct fdset *pfdset, int fd) {
  int i;
...
static void fdset_add_fd(struct fdset *pfdset, int idx, int fd, fd_cb rcb,
                         fd_cb wcb, void *dat) {
 ...
 */
int fdset_add(struct fdset *pfdset, int fd, fd_cb rcb, fd_cb wcb, void *dat) {
  int i;
...
static void fdset_pipe_read_cb(int readfd, void *dat __rte_unused,
...
int fdset_pipe_init(struct fdset *fdset) {
  ...
  if (pipe(fdset->u.pipefd) < 0) {
    RTE_LOG(ERR, VHOST_FDMAN, "failed to create pipe for vhost fdset\n");
    return -1;
  }

  ret = fdset_add(fdset, fdset->u.readfd, fdset_pipe_read_cb, NULL, NULL);
