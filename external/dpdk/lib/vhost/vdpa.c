...
static struct rte_vdpa_device *__vdpa_find_device_by_name(const char *name) {
...
struct rte_vdpa_device *rte_vdpa_register_device(struct rte_device *rte_dev,
  ...
  if (!ops->get_queue_num || !ops->get_features ||
      !ops->get_protocol_features || !ops->dev_conf || !ops->dev_close ||
      !ops->set_vring_state || !ops->set_features) {
...
int rte_vdpa_unregister_device(struct rte_vdpa_device *dev) {
...
int rte_vdpa_relay_vring_used(int vid, uint16_t qid, void *vring_m) {
  ...
  while (idx != idx_m) {
    ...
    if (vq->desc[desc_id].flags & VRING_DESC_F_INDIRECT) {
      ...
      desc_ring = (struct vring_desc *)(uintptr_t)vhost_iova_to_vva(
          dev, vq, vq->desc[desc_id].addr, &dlen, VHOST_ACCESS_RO);
      if (unlikely(!desc_ring))
        return -1;

      if (unlikely(dlen < vq->desc[desc_id].len)) {
        idesc = vhost_alloc_copy_ind_table(dev, vq, vq->desc[desc_id].addr,
                                           vq->desc[desc_id].len);
        if (unlikely(!idesc))
          return -1;
    ...
    do {
      ...
      if (desc.flags & VRING_DESC_F_WRITE)
        vhost_log_write_iova(dev, vq, desc.addr, desc.len);
      desc_id = desc.next;
...
static int vdpa_dev_match(struct rte_vdpa_device *dev,
...
static void *vdpa_dev_iterate(const void *start, const char *str,
