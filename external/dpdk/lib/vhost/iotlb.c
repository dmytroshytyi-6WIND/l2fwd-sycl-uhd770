...
static void vhost_user_iotlb_cache_random_evict(struct vhost_virtqueue *vq);
...
void vhost_user_iotlb_pending_insert(struct virtio_net *dev,
  ...
  if (ret) {
    ...
    if (ret) {
      VHOST_LOG_CONFIG(
          ERR, "(%s) IOTLB pool %s still empty, pending insertion failure\n",
          dev->ifname, vq->iotlb_pool->name);
      return;
    }
  }
...
void vhost_user_iotlb_cache_insert(struct virtio_net *dev,
                                   struct vhost_virtqueue *vq, uint64_t iova,
  ...
  if (ret) {
    VHOST_LOG_CONFIG(
        DEBUG, "(%s) IOTLB pool %s empty, clear entries for cache insertion\n",
        dev->ifname, vq->iotlb_pool->name);
    ...
    if (ret) {
      VHOST_LOG_CONFIG(
          ERR, "(%s) IOTLB pool %s still empty, cache insertion failed\n",
          dev->ifname, vq->iotlb_pool->name);
      return;
    }
  }
...
void vhost_user_iotlb_cache_remove(struct vhost_virtqueue *vq, uint64_t iova,
                                   uint64_t size) {
  struct vhost_iotlb_entry *node, *temp_node;
...
int vhost_user_iotlb_init(struct virtio_net *dev, int vq_index) {
  ...
  snprintf(pool_name, sizeof(pool_name), "iotlb_%u_%d_%d", getpid(), dev->vid,
           vq_index);
  ...
  vq->iotlb_pool = rte_mempool_create(
      pool_name, IOTLB_CACHE_SIZE, sizeof(struct vhost_iotlb_entry), 0, 0, 0,
