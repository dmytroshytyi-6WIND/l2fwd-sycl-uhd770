...
#define VIRTIO_CRYPTO_FEATURES                                                 \
  ((1ULL << VIRTIO_F_NOTIFY_ON_EMPTY) |                                        \
   (1ULL << VIRTIO_RING_F_INDIRECT_DESC) | (1ULL << VIRTIO_RING_F_EVENT_IDX) | \
   (1ULL << VIRTIO_NET_F_CTRL_VQ) | (1ULL << VIRTIO_F_VERSION_1) |             \
...
static int cipher_algo_transform(uint32_t virtio_cipher_algo,
...
static int auth_algo_transform(uint32_t virtio_auth_algo,
...
static int transform_cipher_param(struct rte_crypto_sym_xform *xform,
...
static int transform_chain_param(struct rte_crypto_sym_xform *xforms,
                                 VhostUserCryptoSessionParam *param) {
...
static void vhost_crypto_create_sess(struct vhost_crypto *vcrypto,
  ...
  /* insert hash to map */
  if (rte_hash_add_key_data(vcrypto->session_map, &vcrypto->last_session_id,
                            session) < 0) {
...
static int vhost_crypto_close_sess(struct vhost_crypto *vcrypto,
                                   uint64_t session_id) {
  struct rte_cryptodev_sym_session *session;
...
static enum rte_vhost_msg_result vhost_crypto_msg_post_handler(int vid,
                                                               void *msg) {
...
reach_inhdr(struct vhost_crypto_data_req *vc_req,
            struct vhost_crypto_desc *head, uint32_t max_n_descs) {
  struct virtio_crypto_inhdr *inhdr;
  ...
  inhdr = IOVA_TO_VVA(struct virtio_crypto_inhdr *, vc_req, last->addr, &dlen,
                      VHOST_ACCESS_WO);
...
static __rte_always_inline int move_desc(struct vhost_crypto_desc *head,
  ...
  while (desc->flags & VRING_DESC_F_NEXT && left > 0 && desc >= head &&
...
get_data_ptr(struct vhost_crypto_data_req *vc_req,
             struct vhost_crypto_desc *cur_desc, uint8_t perm) {
  void *data;
...
static __rte_always_inline int copy_data(void *dst_data,
...
static void free_wb_data(struct vhost_crypto_writeback_data *wb_data,
...
prepare_write_back_data(struct vhost_crypto_data_req *vc_req,
                        ...
                        uint8_t *src, uint32_t offset, uint64_t write_back_len,
                        uint32_t max_n_descs) {
  ...
  while (write_back_len && desc >= head_desc &&
...
prepare_sym_cipher_op(struct vhost_crypto *vcrypto, struct rte_crypto_op *op,
  ...
  /* prepare */
  /* iv */
  if (unlikely(copy_data(iv_data, vc_req, head, &desc, cipher->para.iv_len,
                         max_n_descs))) {
  ...
  case RTE_VHOST_CRYPTO_ZERO_COPY_DISABLE:
    ...
    m_src->data_len = cipher->para.src_data_len;
    if (unlikely(copy_data(rte_pktmbuf_mtod(m_src, uint8_t *), vc_req, head,
                           &desc, cipher->para.src_data_len,
                           max_n_descs) < 0)) {
      ret = VIRTIO_CRYPTO_BADMSG;
      goto error_exit;
    }
  ...
  case RTE_VHOST_CRYPTO_ZERO_COPY_ENABLE:
    ...
    break;
  case RTE_VHOST_CRYPTO_ZERO_COPY_DISABLE:
    vc_req->wb = prepare_write_back_data(
        vc_req, head, &desc, &ewb, rte_pktmbuf_mtod(m_src, uint8_t *), 0,
...
vhost_crypto_check_chain_request(struct virtio_crypto_alg_chain_data_req *req) {
  if (likely((req->para.iv_len <= VHOST_CRYPTO_MAX_IV_LEN) &&
             ...
             (req->para.cipher_start_src_offset < VHOST_CRYPTO_MAX_DATA_SIZE) &&
             (req->para.len_to_cipher <= VHOST_CRYPTO_MAX_DATA_SIZE) &&
             (req->para.hash_start_src_offset < VHOST_CRYPTO_MAX_DATA_SIZE) &&
...
prepare_sym_chain_op(struct vhost_crypto *vcrypto, struct rte_crypto_op *op,
  ...
  /* prepare */
  /* iv */
  if (unlikely(copy_data(iv_data, vc_req, head, &desc, chain->para.iv_len,
                         max_n_descs) < 0)) {
    ret = VIRTIO_CRYPTO_BADMSG;
    goto error_exit;
  }
  ...
  case RTE_VHOST_CRYPTO_ZERO_COPY_ENABLE:
    ...
    if (unlikely(move_desc(head, &desc, chain->para.src_data_len, max_n_descs) <
                 0)) {
      VC_LOG_ERR("Incorrect descriptor");
      ret = VIRTIO_CRYPTO_ERR;
      goto error_exit;
  ...
  case RTE_VHOST_CRYPTO_ZERO_COPY_DISABLE:
    ...
    m_src->data_len = chain->para.src_data_len;
    if (unlikely(copy_data(rte_pktmbuf_mtod(m_src, uint8_t *), vc_req, head,
                           &desc, chain->para.src_data_len, max_n_descs) < 0)) {
      ret = VIRTIO_CRYPTO_BADMSG;
      goto error_exit;
    }
  ...
  case RTE_VHOST_CRYPTO_ZERO_COPY_ENABLE:
    ...
    if (unlikely(move_desc(vc_req->head, &desc, chain->para.dst_data_len,
                           max_n_descs) < 0)) {
      VC_LOG_ERR("Incorrect descriptor");
      ret = VIRTIO_CRYPTO_ERR;
      goto error_exit;
    ...
    op->sym->auth.digest.phys_addr =
        gpa_to_hpa(vcrypto->dev, desc->addr, chain->para.hash_result_len);
    ...
    if (unlikely(move_desc(head, &desc, chain->para.hash_result_len,
                           max_n_descs) < 0)) {
      VC_LOG_ERR("Incorrect descriptor");
      ret = VIRTIO_CRYPTO_ERR;
      goto error_exit;
  ...
  case RTE_VHOST_CRYPTO_ZERO_COPY_DISABLE:
    vc_req->wb = prepare_write_back_data(
        vc_req, head, &desc, &ewb, rte_pktmbuf_mtod(m_src, uint8_t *),
    ...
    ewb->next =
        prepare_write_back_data(vc_req, head, &desc, &ewb2, digest_addr, 0,
    ...
    if (unlikely(copy_data(digest_addr, vc_req, head, &digest_desc,
                           chain->para.hash_result_len, max_n_descs) < 0)) {
      ret = VIRTIO_CRYPTO_BADMSG;
      goto error_exit;
    }
    ...
    op->sym->auth.digest.phys_addr =
        rte_pktmbuf_iova_offset(m_src, digest_offset);
    break;
  default:
    ret = VIRTIO_CRYPTO_BADMSG;
    goto error_exit;
  ...
  op->sym->cipher.data.length =
      chain->para.src_data_len - chain->para.cipher_start_src_offset;

  op->sym->auth.data.offset = chain->para.hash_start_src_offset;
  op->sym->auth.data.length = chain->para.len_to_hash;

  vc_req->len =
      chain->para.dst_data_len + chain->para.hash_result_len + INHDR_LEN;
  return 0;
 ...
 */
static __rte_always_inline int vhost_crypto_process_one_req(
    struct vhost_crypto *vcrypto, struct vhost_virtqueue *vq,
    struct rte_crypto_op *op, struct vring_desc *head,
    struct vhost_crypto_desc *descs, uint16_t desc_idx) {
  ...
  dlen = head->len;
  src_desc = IOVA_TO_VVA(struct vring_desc *, vc_req, head->addr, &dlen,
                         VHOST_ACCESS_RO);
  ...
  if (unlikely(nb_descs > VHOST_CRYPTO_MAX_N_DESC || nb_descs == 0)) {
    ...
    if (nb_descs > 0) {
      ...
      if (inhdr_desc->len != sizeof(*inhdr))
        return -1;
      inhdr = IOVA_TO_VVA(struct virtio_crypto_inhdr *, vc_req,
                          inhdr_desc->addr, &dlen, VHOST_ACCESS_WO);
  ...
  if (unlikely(copy_data(&req, vc_req, descs, &desc, sizeof(req), max_n_descs) <
               0)) {
    err = VIRTIO_CRYPTO_BADMSG;
    VC_LOG_ERR("Invalid descriptor");
    goto error_exit;
  ...
  case VIRTIO_CRYPTO_CIPHER_DECRYPT:
    ...
    if (vcrypto->cache_session_id != session_id) {
      err = rte_hash_lookup_data(vcrypto->session_map, &session_id,
                                 (void **)&session);
      if (unlikely(err < 0)) {
        err = VIRTIO_CRYPTO_ERR;
    ...
    case VIRTIO_CRYPTO_SYM_OP_NONE:
      ...
      break;
    case VIRTIO_CRYPTO_SYM_OP_CIPHER:
      err = prepare_sym_cipher_op(vcrypto, op, vc_req, &req.u.sym_req.u.cipher,
                                  desc, max_n_descs);
      break;
    case VIRTIO_CRYPTO_SYM_OP_ALGORITHM_CHAINING:
      err = prepare_sym_chain_op(vcrypto, op, vc_req, &req.u.sym_req.u.chain,
                                 desc, max_n_descs);
      break;
    }
    if (unlikely(err != 0)) {
...
static __rte_always_inline uint16_t vhost_crypto_complete_one_vm_requests(
    struct rte_crypto_op **ops, uint16_t nb_ops, int *callfd) {
...
int rte_vhost_crypto_driver_start(const char *path) {
...
int rte_vhost_crypto_create(int vid, uint8_t cryptodev_id,
                            struct rte_mempool *sess_pool,
                            struct rte_mempool *sess_priv_pool, int socket_id) {
  struct virtio_net *dev = get_device(vid);
  ...
  vcrypto = rte_zmalloc_socket(NULL, sizeof(*vcrypto), RTE_CACHE_LINE_SIZE,
                               socket_id);
  if (!vcrypto) {
  ...
  vcrypto->mbuf_pool = rte_pktmbuf_pool_create(
      name, VHOST_CRYPTO_MBUF_POOL_SIZE, 512,
      sizeof(struct vhost_crypto_data_req),
      VHOST_CRYPTO_MAX_DATA_SIZE + RTE_PKTMBUF_HEADROOM, rte_socket_id());
  ...
  vcrypto->wb_pool =
      rte_mempool_create(name, VHOST_CRYPTO_MBUF_POOL_SIZE,
                         sizeof(struct vhost_crypto_writeback_data), 128, 0,
...
int rte_vhost_crypto_set_zero_copy(int vid,
...
uint16_t rte_vhost_crypto_fetch_requests(int vid, uint32_t qid,
  ...
  case RTE_VHOST_CRYPTO_ZERO_COPY_ENABLE:
    ...
    for (i = 0; i < count; i++) {
      ...
      if (unlikely(vhost_crypto_process_one_req(vcrypto, vq, op, head, descs,
                                                used_idx) < 0))
        break;
    }

    if (unlikely(i < count))
      rte_mempool_put_bulk(vcrypto->mbuf_pool, (void **)&mbufs[i * 2],
                           (count - i) * 2);
...
uint16_t rte_vhost_crypto_finalize_requests(struct rte_crypto_op **ops,
                                            uint16_t nb_ops, int *callfds,
                                            uint16_t *nb_callfds) {
