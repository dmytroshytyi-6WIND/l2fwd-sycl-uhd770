...
static struct vhost_user vhost_user = {
    .fdset = {.fd = {[0 ... MAX_FDS - 1] = {-1, NULL, NULL, NULL, 0}},
 ...
 */
int read_fd_message(char *ifname, int sockfd, char *buf, int buflen, int *fds,
...
int send_fd_message(char *ifname, int sockfd, char *buf, int buflen, int *fds,
                    int fd_num) {
...
static void vhost_user_add_connection(int fd,
                                      struct vhost_user_socket *vsocket) {
  int vid;
  size_t size;
...
static void vhost_user_server_new_connection(int fd, void *dat,
...
static int vhost_user_start_server(struct vhost_user_socket *vsocket) {
  ...
  ret = fdset_add(&vhost_user.fdset, fd, vhost_user_server_new_connection, NULL,
                  vsocket);
  if (ret < 0) {
    VHOST_LOG_CONFIG(ERR,
...
static int vhost_user_connect_nonblock(char *path, int fd, struct sockaddr *un,
                                       size_t sz) {
  int ret, flags;
...
static void *vhost_user_client_reconnect(void *arg __rte_unused) {
  int ret;
  ...
  while (1) {
     ...
     */
    for (reconn = TAILQ_FIRST(&reconn_list.head); reconn != NULL;
         reconn = next) {
...
static int vhost_user_reconnect_init(void) {
  int ret;
...
static int vhost_user_start_client(struct vhost_user_socket *vsocket) {
  ...
  ret = vhost_user_connect_nonblock(
      vsocket->path, fd, (struct sockaddr *)&vsocket->un, sizeof(vsocket->un));
  if (ret == 0) {
    vhost_user_add_connection(fd, vsocket);
    return 0;
  }
...
static struct vhost_user_socket *find_vhost_user_socket(const char *path) {
  int i;
...
int rte_vhost_driver_attach_vdpa_device(const char *path,
...
int rte_vhost_driver_get_protocol_features(const char *path,
  ...
  if (vdpa_dev->ops->get_protocol_features(vdpa_dev, &vdpa_protocol_features) <
      0) {
  ...
  *protocol_features = vsocket->protocol_features & vdpa_protocol_features;
...
static void vhost_user_socket_mem_free(struct vhost_user_socket *vsocket) {
...
int rte_vhost_driver_register(const char *path, uint64_t flags) {
  ...
  if (vsocket->async_copy && (flags & (RTE_VHOST_USER_IOMMU_SUPPORT |
                                       RTE_VHOST_USER_POSTCOPY_SUPPORT))) {
    VHOST_LOG_CONFIG(
        ERR, "(%s) async copy with IOMMU or post-copy not supported\n", path);
    goto out_mutex;
  }
  ...
  if (vsocket->async_copy) {
    ...
    VHOST_LOG_CONFIG(
        INFO, "(%s) logging feature is disabled in async copy mode\n", path);
  }
  ...
  if (vsocket->linearbuf && !vsocket->extbuf) {
    uint64_t seg_offload_features = (1ULL << VIRTIO_NET_F_HOST_TSO4) |
                                    (1ULL << VIRTIO_NET_F_HOST_TSO6) |
                                    (1ULL << VIRTIO_NET_F_HOST_UFO);

    VHOST_LOG_CONFIG(
        INFO, "(%s) Linear buffers requested without external buffers,\n",
        path);
    VHOST_LOG_CONFIG(
        INFO, "(%s) disabling host segmentation offloading support\n", path);
  ...
  if (!(flags & RTE_VHOST_USER_POSTCOPY_SUPPORT)) {
    vsocket->protocol_features &= ~(1ULL << VHOST_USER_PROTOCOL_F_PAGEFAULT);
  } else {
...
static bool vhost_user_remove_reconnect(struct vhost_user_socket *vsocket) {
  int found = false;
  struct vhost_user_reconnect *reconn, *next;

  pthread_mutex_lock(&reconn_list.mutex);

  for (reconn = TAILQ_FIRST(&reconn_list.head); reconn != NULL; reconn = next) {
    next = TAILQ_NEXT(reconn, next);
...
again:
  ...
  for (i = 0; i < vhost_user.vsocket_cnt; i++) {
    ...
    pthread_mutex_lock(&vsocket->conn_mutex);
    for (conn = TAILQ_FIRST(&vsocket->conn_list); conn != NULL; conn = next) {
       ...
       */
      if (fdset_try_del(&vhost_user.fdset, conn->connfd) == -1) {
        pthread_mutex_unlock(&vsocket->conn_mutex);
        pthread_mutex_unlock(&vhost_user.mutex);
        goto again;
      }
 ...
 */
int rte_vhost_driver_callback_register(
    const char *path, struct rte_vhost_device_ops const *const ops) {
  struct vhost_user_socket *vsocket;
...
int rte_vhost_driver_start(const char *path) {
  ...
  if (fdset_tid == 0) {
    ...
    int ret = rte_ctrl_thread_create(&fdset_tid, "vhost-events", NULL,
