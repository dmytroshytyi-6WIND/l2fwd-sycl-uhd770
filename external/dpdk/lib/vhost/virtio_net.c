...
static bool is_valid_virt_queue_idx(uint32_t idx, int is_tx,
                                    uint32_t nr_vring) {
  return (is_tx ^ (idx & 1)) == 0 && idx < nr_vring;
}

static __rte_always_inline int64_t vhost_async_dma_transfer_one(
    struct virtio_net *dev, struct vhost_virtqueue *vq, int16_t dma_id,
    uint16_t vchan_id, uint16_t flag_idx, struct vhost_iov_iter *pkt) {
...
vhost_async_dma_check_completed(struct virtio_net *dev, int16_t dma_id,
                                uint16_t vchan_id, uint16_t max_pkts) {
  struct async_dma_vchan_info *dma_info =
      &dma_copy_track[dma_id].vchans[vchan_id];
  uint16_t ring_mask = dma_info->ring_mask;
  uint16_t last_idx = 0;
   ...
   */
  nr_copies =
      rte_dma_completed(dma_id, vchan_id, max_pkts, &last_idx, &has_error);
...
static inline void do_data_copy_enqueue(struct virtio_net *dev,
...
do_flush_shadow_used_ring_split(struct virtio_net *dev,
                                struct vhost_virtqueue *vq, uint16_t to,
                                uint16_t from, uint16_t size) {
  rte_memcpy(&vq->used->ring[to], &vq->shadow_used_split[from],
             size * sizeof(struct vring_used_elem));
  vhost_log_cache_used_vring(dev, vq, offsetof(struct vring_used, ring[to]),
                             size * sizeof(struct vring_used_elem));
}

static __rte_always_inline void
flush_shadow_used_ring_split(struct virtio_net *dev,
                             struct vhost_virtqueue *vq) {
...
static __rte_always_inline void
update_shadow_used_ring_split(struct vhost_virtqueue *vq, uint16_t desc_idx,
                              uint32_t len) {
...
vhost_flush_enqueue_shadow_packed(struct virtio_net *dev,
  ...
  for (i = 0; i < vq->shadow_used_idx; i++) {
    ...
    if (i > 0) {
      ...
      vhost_log_cache_used_vring(
          dev, vq, vq->last_used_idx * sizeof(struct vring_packed_desc),
  ...
  vhost_log_cache_used_vring(dev, vq,
                             head_idx * sizeof(struct vring_packed_desc),
                             sizeof(struct vring_packed_desc));
...
vhost_flush_dequeue_shadow_packed(struct virtio_net *dev,
  ...
  vhost_log_cache_used_vring(
      dev, vq, vq->shadow_last_used_idx * sizeof(struct vring_packed_desc),
      sizeof(struct vring_packed_desc));
  vq->shadow_used_idx = 0;
  vhost_log_cache_sync(dev, vq);
}
...
vhost_flush_enqueue_batch_packed(struct virtio_net *dev,
                                 struct vhost_virtqueue *vq, uint64_t *lens,
                                 uint16_t *ids) {
  uint16_t i;
  uint16_t flags;
  ...
  vhost_log_cache_used_vring(
      dev, vq, last_used_idx * sizeof(struct vring_packed_desc),
      sizeof(struct vring_packed_desc) * PACKED_BATCH_SIZE);
  vhost_log_cache_sync(dev, vq);
...
vhost_shadow_dequeue_batch_packed(struct virtio_net *dev,
                                  struct vhost_virtqueue *vq, uint16_t *ids) {
  uint16_t flags;
  uint16_t i;
  uint16_t begin;
  ...
  rte_atomic_thread_fence(__ATOMIC_RELEASE);
  vhost_for_each_try_unroll(i, begin, PACKED_BATCH_SIZE)
      vq->desc_packed[vq->last_used_idx + i]
          .flags = flags;
...
static __rte_always_inline void
vhost_shadow_dequeue_single_packed(struct vhost_virtqueue *vq, uint16_t buf_id,
                                   uint16_t count) {
  uint16_t flags;
...
static __rte_always_inline void
vhost_shadow_enqueue_packed(struct vhost_virtqueue *vq, uint32_t *len,
                            uint16_t *id, uint16_t *count,
...
static __rte_always_inline void vhost_shadow_enqueue_single_packed(
    struct virtio_net *dev, struct vhost_virtqueue *vq, uint32_t *len,
...
#define ASSIGN_UNLESS_EQUAL(var, val)                                          \
  do {                                                                         \
    if ((var) != (val))                                                        \
      (var) = (val);                                                           \
  } while (0)
...
virtio_enqueue_offload(struct rte_mbuf *m_buf, struct virtio_net_hdr *net_hdr) {
  ...
  if (m_buf->ol_flags & RTE_MBUF_F_TX_IP_CKSUM) {
    ...
    ipv4_hdr =
        rte_pktmbuf_mtod_offset(m_buf, struct rte_ipv4_hdr *, m_buf->l2_len);
...
map_one_desc(struct virtio_net *dev, struct vhost_virtqueue *vq,
             struct buf_vector *buf_vec, uint16_t *vec_idx, uint64_t desc_iova,
             uint64_t desc_len, uint8_t perm) {
...
fill_vec_buf_split(struct virtio_net *dev, struct vhost_virtqueue *vq,
  ...
  *desc_chain_head = idx;
  ...
  if (vq->desc[idx].flags & VRING_DESC_F_INDIRECT) {
    ...
    descs = (struct vring_desc *)(uintptr_t)vhost_iova_to_vva(
        dev, vq, vq->desc[idx].addr, &dlen, VHOST_ACCESS_RO);
    if (unlikely(!descs))
      return -1;
    ...
    if (unlikely(dlen < vq->desc[idx].len)) {
       ...
       */
      idesc = vhost_alloc_copy_ind_table(dev, vq, vq->desc[idx].addr,
                                         vq->desc[idx].len);
      if (unlikely(!idesc))
        return -1;
  ...
  while (1) {
    ...
    if (unlikely(map_one_desc(dev, vq, buf_vec, &vec_id, descs[idx].addr, dlen,
                              perm))) {
      free_ind_table(idesc);
      return -1;
    }
...
reserve_avail_buf_split(struct virtio_net *dev, struct vhost_virtqueue *vq,
  ...
  *num_buffers = 0;
  ...
  while (size > 0) {
    ...
    if (unlikely(fill_vec_buf_split(dev, vq, cur_idx, &vec_idx, buf_vec,
                                    &head_idx, &len, VHOST_ACCESS_RW) < 0))
      return -1;
...
static __rte_always_inline int
fill_vec_buf_packed_indirect(struct virtio_net *dev, struct vhost_virtqueue *vq,
                             struct vring_packed_desc *desc, uint16_t *vec_idx,
                             struct buf_vector *buf_vec, uint32_t *len,
                             uint8_t perm) {
  uint16_t i;
  uint32_t nr_descs;
  uint16_t vec_id = *vec_idx;
  ...
  dlen = desc->len;
  descs = (struct vring_packed_desc *)(uintptr_t)vhost_iova_to_vva(
      dev, vq, desc->addr, &dlen, VHOST_ACCESS_RO);
  if (unlikely(!descs))
    return -1;
  ...
  for (i = 0; i < nr_descs; i++) {
    ...
    *len += dlen;
    if (unlikely(
            map_one_desc(dev, vq, buf_vec, &vec_id, descs[i].addr, dlen, perm)))
      return -1;
  }
  *vec_idx = vec_id;
  ...
  while (1) {
    ...
    *buf_id = descs[avail_idx].id;
    ...
    if (descs[avail_idx].flags & VRING_DESC_F_INDIRECT) {
      if (unlikely(fill_vec_buf_packed_indirect(dev, vq, &descs[avail_idx],
                                                &vec_id, buf_vec, len,
                                                perm) < 0))
        return -1;
    } else {
      ...
      *len += dlen;
      ...
      if (unlikely(map_one_desc(dev, vq, buf_vec, &vec_id,
                                descs[avail_idx].addr, dlen, perm)))
        return -1;
    }
...
static __rte_always_inline int async_iter_add_iovec(struct virtio_net *dev,
...
static __rte_always_inline void async_iter_finalize(struct vhost_async *async) {
  async->iter_idx++;
}

static __rte_always_inline void async_iter_cancel(struct vhost_async *async) {
  struct vhost_iov_iter *iter;
...
static __rte_always_inline void async_iter_reset(struct vhost_async *async) {
  async->iter_idx = 0;
  async->iovec_idx = 0;
}
...
async_mbuf_to_desc_seg(struct virtio_net *dev, struct vhost_virtqueue *vq,
  ...
  while (cpy_len) {
    host_iova = (void *)(uintptr_t)gpa_to_first_hpa(dev, buf_iova + buf_offset,
                                                    cpy_len, &mapped_len);
    ...
    if (unlikely(async_iter_add_iovec(
            dev, async,
...
sync_mbuf_to_desc_seg(struct virtio_net *dev, struct vhost_virtqueue *vq,
  ...
  if (likely(cpy_len > MAX_BATCH_LEN || vq->batch_copy_nb_elems >= vq->size)) {
    ...
    batch_copy[vq->batch_copy_nb_elems].dst = (void *)((uintptr_t)(buf_addr));
...
mbuf_to_desc(struct virtio_net *dev, struct vhost_virtqueue *vq,
             struct rte_mbuf *m, struct buf_vector *buf_vec, uint16_t nr_vec,
             uint16_t num_buffers, bool is_async) {
  uint32_t vec_idx = 0;
  ...
  while (mbuf_avail != 0 || m->next != NULL) {
    ...
    if (hdr_addr) {
      ...
      if (unlikely(hdr == &tmp_hdr)) {
        ...
        vhost_log_cache_write_iova(dev, vq, buf_vec[0].buf_iova,
                                   dev->vhost_hlen);
      }
    ...
    if (is_async) {
      if (async_mbuf_to_desc_seg(dev, vq, m, mbuf_offset, buf_iova + buf_offset,
                                 cpy_len) < 0)
        goto error;
    } else {
      sync_mbuf_to_desc_seg(dev, vq, m, mbuf_offset, buf_addr + buf_offset,
...
vhost_enqueue_single_packed(struct virtio_net *dev, struct vhost_virtqueue *vq,
  ...
  while (size > 0) {
    ...
    if (unlikely(fill_vec_buf_packed(dev, vq, avail_idx, &desc_count, buf_vec,
                                     &nr_vec, &buf_id, &len,
                                     VHOST_ACCESS_RW) < 0))
      return -1;
...
static __rte_noinline uint32_t virtio_dev_rx_split(struct virtio_net *dev,
                                                   struct vhost_virtqueue *vq,
  ...
  for (pkt_idx = 0; pkt_idx < count; pkt_idx++) {
    ...
    if (mbuf_to_desc(dev, vq, pkts[pkt_idx], buf_vec, nr_vec, num_buffers,
                     false) < 0) {
      vq->shadow_used_idx -= num_buffers;
      break;
    }
...
static __rte_always_inline int virtio_dev_rx_sync_batch_check(
    struct virtio_net *dev, struct vhost_virtqueue *vq, struct rte_mbuf **pkts,
    uint64_t *desc_addrs, uint64_t *lens) {
  bool wrap_counter = vq->avail_wrap_counter;
  struct vring_packed_desc *descs = vq->desc_packed;
  uint16_t avail_idx = vq->last_avail_idx;
  ...
  vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) lens[i] =
  ...
  vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) desc_addrs[i] =
      vhost_iova_to_vva(dev, vq, descs[avail_idx + i].addr, &lens[i],
                        VHOST_ACCESS_RW);
...
static __rte_always_inline void virtio_dev_rx_batch_packed_copy(
    struct virtio_net *dev, struct vhost_virtqueue *vq, struct rte_mbuf **pkts,
    uint64_t *desc_addrs, uint64_t *lens) {
  uint32_t buf_offset = sizeof(struct virtio_net_hdr_mrg_rxbuf);
  ...
  vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) {
    ...
    hdrs[i] = (struct virtio_net_hdr_mrg_rxbuf *)(uintptr_t)desc_addrs[i];
    lens[i] = pkts[i]->pkt_len + sizeof(struct virtio_net_hdr_mrg_rxbuf);
  }
  ...
  vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) {
    rte_memcpy((void *)(uintptr_t)(desc_addrs[i] + buf_offset),
               rte_pktmbuf_mtod_offset(pkts[i], void *, 0), pkts[i]->pkt_len);
  }

  vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE)
      vhost_log_cache_write_iova(dev, vq, descs[avail_idx + i].addr, lens[i]);

  vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) ids[i] =
      descs[avail_idx + i].id;
...
static __rte_always_inline int16_t virtio_dev_rx_single_packed(
    struct virtio_net *dev, struct vhost_virtqueue *vq, struct rte_mbuf *pkt) {
  struct buf_vector buf_vec[BUF_VECTOR_MAX];
  uint16_t nr_descs = 0;

  if (unlikely(vhost_enqueue_single_packed(dev, vq, pkt, buf_vec, &nr_descs) <
               0)) {
    VHOST_LOG_DATA(DEBUG, "(%s) failed to get enough desc from vring\n",
                   dev->ifname);
    return -1;
...
static __rte_noinline uint32_t virtio_dev_rx_packed(
    struct virtio_net *dev, struct vhost_virtqueue *__rte_restrict vq,
  ...
  do {
    ...
    if (count - pkt_idx >= PACKED_BATCH_SIZE) {
      if (!virtio_dev_rx_sync_batch_packed(dev, vq, &pkts[pkt_idx])) {
...
static __rte_always_inline uint32_t virtio_dev_rx(struct virtio_net *dev,
                                                  uint16_t queue_id,
                                                  struct rte_mbuf **pkts,
                                                  uint32_t count) {
  struct vhost_virtqueue *vq;
...
static __rte_always_inline void
store_dma_desc_info_split(struct vring_used_elem *s_ring,
                          struct vring_used_elem *d_ring, uint16_t ring_size,
                          uint16_t s_idx, uint16_t d_idx, uint16_t count) {
...
store_dma_desc_info_packed(struct vring_used_elem_packed *s_ring,
                           ...
                           uint16_t ring_size, uint16_t s_idx, uint16_t d_idx,
                           uint16_t count) {
...
static __rte_noinline uint32_t virtio_dev_rx_async_submit_split(
    struct virtio_net *dev, struct vhost_virtqueue *vq, uint16_t queue_id,
  ...
  if (unlikely(pkt_err)) {
    ...
    VHOST_LOG_DATA(DEBUG,
                   "(%s) %s: failed to transfer %u packets for queue %u.\n",
  ...
  if (likely(vq->shadow_used_idx)) {
    ...
    store_dma_desc_info_split(vq->shadow_used_split, async->descs_split,
                              vq->size, 0, to, vq->shadow_used_idx);
...
static __rte_always_inline int
vhost_enqueue_async_packed(struct virtio_net *dev, struct vhost_virtqueue *vq,
                           struct rte_mbuf *pkt, struct buf_vector *buf_vec,
  ...
  if (unlikely(mbuf_to_desc(dev, vq, pkt, buf_vec, nr_vec, *nr_buffers, true) <
               0))
    return -1;

  vhost_shadow_enqueue_packed(vq, buffer_len, buffer_buf_id, buffer_desc_count,
                              *nr_buffers);

  return 0;
}

static __rte_always_inline int16_t virtio_dev_rx_async_packed(
    struct virtio_net *dev, struct vhost_virtqueue *vq, struct rte_mbuf *pkt,
    uint16_t *nr_descs, uint16_t *nr_buffers) {
  struct buf_vector buf_vec[BUF_VECTOR_MAX];
...
static __rte_noinline uint32_t virtio_dev_rx_async_submit_packed(
    struct virtio_net *dev, struct vhost_virtqueue *vq, uint16_t queue_id,
    struct rte_mbuf **pkts, uint32_t count, int16_t dma_id, uint16_t vchan_id) {
  uint32_t pkt_idx = 0;
  ...
  do {
    ...
    num_buffers = 0;
    num_descs = 0;
    if (unlikely(virtio_dev_rx_async_packed(dev, vq, pkts[pkt_idx], &num_descs,
                                            &num_buffers) < 0))
      break;
...
static __rte_always_inline uint16_t vhost_poll_enqueue_completed(
    struct virtio_net *dev, uint16_t queue_id, struct rte_mbuf **pkts,
...
uint16_t rte_vhost_poll_enqueue_completed(int vid, uint16_t queue_id,
  ...
  n_pkts_cpl = vhost_poll_enqueue_completed(dev, queue_id, pkts, count, dma_id,
                                            vchan_id);
...
static __rte_always_inline uint32_t virtio_dev_rx_async_submit(
    struct virtio_net *dev, uint16_t queue_id, struct rte_mbuf **pkts,
    uint32_t count, int16_t dma_id, uint16_t vchan_id) {
  struct vhost_virtqueue *vq;
  uint32_t nb_tx = 0;
  ...
  if (vq_is_packed(dev))
    nb_tx = virtio_dev_rx_async_submit_packed(dev, vq, queue_id, pkts, count,
                                              dma_id, vchan_id);
  else
    nb_tx = virtio_dev_rx_async_submit_split(dev, vq, queue_id, pkts, count,
                                             dma_id, vchan_id);
...
static inline bool virtio_net_with_host_offload(struct virtio_net *dev) {
  if (dev->features &
      ((1ULL << VIRTIO_NET_F_CSUM) | (1ULL << VIRTIO_NET_F_HOST_ECN) |
...
static int parse_headers(struct rte_mbuf *m, uint8_t *l4_proto) {
  struct rte_ipv4_hdr *ipv4_hdr;
  ...
  if (ethertype == RTE_ETHER_TYPE_VLAN) {
    if (data_len < sizeof(struct rte_ether_hdr) + sizeof(struct rte_vlan_hdr))
      goto error;
  ...
  switch (*l4_proto) {
  case IPPROTO_TCP:
    if (data_len < m->l2_len + m->l3_len + sizeof(struct rte_tcp_hdr))
      goto error;
    break;
  case IPPROTO_UDP:
    if (data_len < m->l2_len + m->l3_len + sizeof(struct rte_udp_hdr))
      goto error;
    break;
  case IPPROTO_SCTP:
    if (data_len < m->l2_len + m->l3_len + sizeof(struct rte_sctp_hdr))
      goto error;
    break;
  default:
    goto error;
...
static __rte_noinline void copy_vnet_hdr_from_desc(struct virtio_net_hdr *hdr,
                                                   struct buf_vector *buf_vec) {
  uint64_t len;
  ...
  while (1) {
    ...
    if (likely(cpy_len > MAX_BATCH_LEN || vq->batch_copy_nb_elems >= vq->size ||
      ...
      rte_memcpy(rte_pktmbuf_mtod_offset(cur, void *, mbuf_offset),
                 (void *)((uintptr_t)(buf_addr + buf_offset)), cpy_len);
    } else {
      batch_copy[vq->batch_copy_nb_elems].dst =
          rte_pktmbuf_mtod_offset(cur, void *, mbuf_offset);
      batch_copy[vq->batch_copy_nb_elems].src =
...
static int virtio_dev_extbuf_alloc(struct virtio_net *dev, struct rte_mbuf *pkt,
                                   uint32_t size) {
 ...
 */
static __rte_always_inline int virtio_dev_pktmbuf_prep(struct virtio_net *dev,
                                                       struct rte_mbuf *pkt,
                                                       uint32_t data_len) {
...
__rte_always_inline static uint16_t
virtio_dev_tx_split(struct virtio_net *dev, struct vhost_virtqueue *vq,
   ...
   */
  free_entries =
      __atomic_load_n(&vq->avail->idx, __ATOMIC_ACQUIRE) - vq->last_avail_idx;
  if (free_entries == 0)
    return 0;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (unlikely(fill_vec_buf_split(dev, vq, vq->last_avail_idx + i, &nr_vec,
                                    buf_vec, &head_idx, &buf_len,
                                    VHOST_ACCESS_RO) < 0))
      break;
...
__rte_noinline static uint16_t
virtio_dev_tx_split_legacy(struct virtio_net *dev, struct vhost_virtqueue *vq,
                           struct rte_mempool *mbuf_pool,
                           struct rte_mbuf **pkts, uint16_t count) {
  return virtio_dev_tx_split(dev, vq, mbuf_pool, pkts, count, true);
}

__rte_noinline static uint16_t virtio_dev_tx_split_compliant(
    struct virtio_net *dev, struct vhost_virtqueue *vq,
    struct rte_mempool *mbuf_pool, struct rte_mbuf **pkts, uint16_t count) {
  return virtio_dev_tx_split(dev, vq, mbuf_pool, pkts, count, false);
}

static __rte_always_inline int vhost_reserve_avail_batch_packed(
    struct virtio_net *dev, struct vhost_virtqueue *vq, struct rte_mbuf **pkts,
    uint16_t avail_idx, uintptr_t *desc_addrs, uint16_t *ids) {
  ...
  vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) {
    desc_addrs[i] = vhost_iova_to_vva(dev, vq, descs[avail_idx + i].addr,
  ...
  vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) buf_lens[i] =
      pkts[i]->buf_len - pkts[i]->data_off;
...
virtio_dev_tx_batch_packed(struct virtio_net *dev, struct vhost_virtqueue *vq,
                           struct rte_mbuf **pkts, bool legacy_ol_flags) {
  uint16_t avail_idx = vq->last_avail_idx;
  uint32_t buf_offset = sizeof(struct virtio_net_hdr_mrg_rxbuf);
  ...
  if (vhost_reserve_avail_batch_packed(dev, vq, pkts, avail_idx, desc_addrs,
                                       ids))
    return -1;
  ...
  vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) rte_memcpy(
      rte_pktmbuf_mtod_offset(pkts[i], void *, 0),
      (void *)(uintptr_t)(desc_addrs[i] + buf_offset), pkts[i]->pkt_len);
...
vhost_dequeue_single_packed(struct virtio_net *dev, struct vhost_virtqueue *vq,
                            struct rte_mempool *mbuf_pool,
                            struct rte_mbuf *pkts, uint16_t *buf_id,
                            uint16_t *desc_count, bool legacy_ol_flags) {
  struct buf_vector buf_vec[BUF_VECTOR_MAX];
  uint32_t buf_len;
  uint16_t nr_vec = 0;
  int err;
...
__rte_always_inline static uint16_t virtio_dev_tx_packed(
    struct virtio_net *dev, struct vhost_virtqueue *__rte_restrict vq,
    struct rte_mempool *mbuf_pool, struct rte_mbuf **__rte_restrict pkts,
    uint32_t count, bool legacy_ol_flags) {
  uint32_t pkt_idx = 0;
  ...
  do {
    ...
    if (count - pkt_idx >= PACKED_BATCH_SIZE) {
      if (!virtio_dev_tx_batch_packed(dev, vq, &pkts[pkt_idx],
                                      legacy_ol_flags)) {
        pkt_idx += PACKED_BATCH_SIZE;
        continue;
      }
...
__rte_noinline static uint16_t virtio_dev_tx_packed_legacy(
    struct virtio_net *dev, struct vhost_virtqueue *__rte_restrict vq,
    struct rte_mempool *mbuf_pool, struct rte_mbuf **__rte_restrict pkts,
    uint32_t count) {
...
uint16_t rte_vhost_dequeue_burst(int vid, uint16_t queue_id,
  ...
  if (unlikely(__atomic_load_n(&dev->broadcast_rarp, __ATOMIC_ACQUIRE) &&
               __atomic_compare_exchange_n(&dev->broadcast_rarp, &success, 0, 0,
                                           __ATOMIC_RELEASE,
                                           __ATOMIC_RELAXED))) {
