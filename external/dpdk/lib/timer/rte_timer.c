...
#ifdef RTE_LIBRTE_TIMER_DEBUG
#define __TIMER_STAT_ADD(priv_timer, name, n)                                  \
  do {                                                                         \
...
static inline int timer_data_valid(uint32_t id) {
...
#define TIMER_DATA_VALID_GET_OR_ERR_RET(id, timer_data, retval)                \
  do {                                                                         \
...
int rte_timer_subsystem_init(void) {
  ...
  const size_t data_arr_size = RTE_MAX_DATA_ELS * sizeof(*rte_timer_data_arr);
  ...
  if (mz == NULL) {
    mz = rte_memzone_reserve_aligned(mz_name, mem_size, SOCKET_ID_ANY, 0,
                                     RTE_CACHE_LINE_SIZE);
    if (mz == NULL) {
      rte_mcfg_timer_unlock();
      return -ENOMEM;
 ...
 * status of the timer
 */
static int timer_set_config_state(struct rte_timer *tim,
  ...
  while (success == 0) {
     ...
     */
    success = __atomic_compare_exchange_n(&tim->status.u32, &prev_status.u32,
                                          status.u32, 0, __ATOMIC_ACQUIRE,
                                          __ATOMIC_RELAXED);
  }
 ...
 */
static int timer_set_running_state(struct rte_timer *tim) {
  union rte_timer_status prev_status, status;
 ...
 * all skiplist levels.
 */
static void timer_get_prev_entries_for_node(struct rte_timer *tim,
                                            unsigned tim_lcore,
                                            struct rte_timer **prev,
                                            struct priv_timer *priv_timer) {
  int i;
 ...
 */
static void timer_add(struct rte_timer *tim, unsigned int tim_lcore,
                      struct priv_timer *priv_timer) {
  unsigned lvl;
  ...
  const unsigned tim_level =
      timer_get_skiplist_level(priv_timer[tim_lcore].curr_skiplist_depth);
 ...
 */
static void timer_del(struct rte_timer *tim, union rte_timer_status prev_status,
...
static int __rte_timer_reset(struct rte_timer *tim, uint64_t expire,
                             uint64_t period, unsigned tim_lcore,
                             rte_timer_cb_t fct, void *arg, int local_is_locked,
                             struct rte_timer_data *timer_data) {
  union rte_timer_status prev_status, status;
  int ret;
  unsigned lcore_id = rte_lcore_id();
  ...
  if (tim_lcore == (unsigned)LCORE_ID_ANY) {
    if (lcore_id < RTE_MAX_LCORE) {
      ...
      tim_lcore = rte_get_next_lcore(priv_timer[lcore_id].prev_lcore, 0, 1);
  ...
  if (prev_status.state == RTE_TIMER_RUNNING && lcore_id < RTE_MAX_LCORE) {
...
int rte_timer_reset(struct rte_timer *tim, uint64_t ticks,
  ...
  return rte_timer_alt_reset(default_data_id, tim, ticks, type, tim_lcore, fct,
                             arg);
}

int rte_timer_alt_reset(uint32_t timer_data_id, struct rte_timer *tim,
  ...
  return __rte_timer_reset(tim, cur_time + ticks, period, tim_lcore, fct, arg,
                           0, timer_data);
}

/* loop until rte_timer_reset() succeed */
void rte_timer_reset_sync(struct rte_timer *tim, uint64_t ticks,
                          enum rte_timer_type type, unsigned tim_lcore,
                          rte_timer_cb_t fct, void *arg) {
  while (rte_timer_reset(tim, ticks, type, tim_lcore, fct, arg) != 0)
    rte_pause();
}

static int __rte_timer_stop(struct rte_timer *tim, int local_is_locked,
                            struct rte_timer_data *timer_data) {
  union rte_timer_status prev_status, status;
  unsigned lcore_id = rte_lcore_id();
  int ret;
...
int rte_timer_pending(struct rte_timer *tim) {
  return __atomic_load_n(&tim->status.state, __ATOMIC_RELAXED) ==
         RTE_TIMER_PENDING;
}

/* must be called periodically, run all timer that expired */
static void __rte_timer_manage(struct rte_timer_data *timer_data) {
  union rte_timer_status status;
  ...
  for (tim = run_first_tim; tim != NULL; tim = next_tim) {
    ...
    if (tim->period == 0) {
       ...
       */
      __atomic_store_n(&tim->status.u32, status.u32, __ATOMIC_RELEASE);
      __rte_timer_reset(tim, tim->expire + tim->period, tim->period, lcore_id,
                        tim->f, tim->arg, 1, timer_data);
      rte_spinlock_unlock(&priv_timer[lcore_id].list_lock);
    }
  }
...
int rte_timer_alt_manage(uint32_t timer_data_id, unsigned int *poll_lcores,
                         int nb_poll_lcores, rte_timer_alt_manage_cb_t f) {
  ...
  for (i = 0; i < nb_poll_lcores; i++) {
    ...
    /* break the existing list at current time point */
    timer_get_prev_entries(cur_time, poll_lcore, prev, data->priv_timer);
    for (j = privp->curr_skiplist_depth - 1; j >= 0; j--) {
      if (prev[j] == &privp->pending_head)
        continue;
      privp->pending_head.sl_next[j] = prev[j]->sl_next[j];
    ...
    /* update the next to expire timer value */
    privp->pending_head.expire = (privp->pending_head.sl_next[0] == NULL)
                                     ? 0
  ...
  while (1) {
    ...
    if (tim->period == 0) {
      ...
      /* keep it in list and mark timer as pending */
      rte_spinlock_lock(&data->priv_timer[this_lcore].list_lock);
      status.state = RTE_TIMER_PENDING;
      __TIMER_STAT_ADD(data->priv_timer, pending, 1);
       ...
       */
      __atomic_store_n(&tim->status.u32, status.u32, __ATOMIC_RELEASE);
      __rte_timer_reset(tim, tim->expire + tim->period, tim->period, this_lcore,
                        tim->f, tim->arg, 1, data);
      rte_spinlock_unlock(&data->priv_timer[this_lcore].list_lock);
    }
...
int rte_timer_stop_all(uint32_t timer_data_id, unsigned int *walk_lcores,
                       int nb_walk_lcores, rte_timer_stop_all_cb_t f,
                       void *f_arg) {
  int i;
  struct priv_timer *priv_timer;
  uint32_t walk_lcore;
  ...
  for (i = 0; i < nb_walk_lcores; i++) {
    ...
    for (tim = priv_timer->pending_head.sl_next[0]; tim != NULL;
         tim = next_tim) {
      next_tim = tim->sl_next[0];
...
int rte_timer_alt_dump_stats(uint32_t timer_data_id __rte_unused, FILE *f) {
  struct rte_timer_data *timer_data;
