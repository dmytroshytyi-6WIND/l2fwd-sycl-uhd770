...
#define PDUMP_LOG(level, fmt, args...)                                         \
  rte_log(RTE_LOG_##level, pdump_logtype, "%s(): " fmt, __func__, ##args)
...
static void pdump_copy(uint16_t port_id, uint16_t queue,
  ...
  for (i = 0; i < nb_pkts; i++) {
     ...
     */
    if (cbs->ver == V2)
      p = rte_pcapng_copy(port_id, queue, pkts[i], mp, cbs->snaplen, ts,
                          direction);
    else
...
static uint16_t pdump_rx(uint16_t port, uint16_t queue, struct rte_mbuf **pkts,
  ...
  pdump_copy(port, queue, RTE_PCAPNG_DIRECTION_IN, pkts, nb_pkts, cbs, stats);
  return nb_pkts;
}

static uint16_t pdump_tx(uint16_t port, uint16_t queue, struct rte_mbuf **pkts,
                         uint16_t nb_pkts, void *user_params) {
  const struct pdump_rxtx_cbs *cbs = user_params;
  struct rte_pdump_stats *stats = &pdump_stats->tx[port][queue];

  pdump_copy(port, queue, RTE_PCAPNG_DIRECTION_OUT, pkts, nb_pkts, cbs, stats);
  return nb_pkts;
}

static int pdump_register_rx_callbacks(enum pdump_version ver, uint16_t end_q,
  ...
  for (; qid < end_q; qid++) {
    ...
    if (operation == ENABLE) {
      if (cbs->cb) {
        PDUMP_LOG(ERR, "rx callback for port=%d queue=%d, already exists\n",
                  port, qid);
        return -EEXIST;
      }
      cbs->ver = ver;
      ...
      if (cbs->cb == NULL) {
        PDUMP_LOG(ERR, "failed to add rx callback, errno=%d\n", rte_errno);
        return rte_errno;
      }
      ...
      if (cbs->cb == NULL) {
        PDUMP_LOG(ERR, "no existing rx callback for port=%d queue=%d\n", port,
                  qid);
        return -EINVAL;
      }
      ret = rte_eth_remove_rx_callback(port, qid, cbs->cb);
      if (ret < 0) {
        PDUMP_LOG(ERR, "failed to remove rx callback, errno=%d\n", -ret);
        return ret;
      }
      cbs->cb = NULL;
    }
...
static int pdump_register_tx_callbacks(enum pdump_version ver, uint16_t end_q,
                                       uint16_t port, uint16_t queue,
                                       struct rte_ring *ring,
                                       struct rte_mempool *mp,
                                       struct rte_bpf *filter,
  ...
  for (; qid < end_q; qid++) {
    ...
    if (operation == ENABLE) {
      if (cbs->cb) {
        PDUMP_LOG(ERR, "tx callback for port=%d queue=%d, already exists\n",
                  port, qid);
        return -EEXIST;
      }
      cbs->ver = ver;
      ...
      cbs->cb = rte_eth_add_tx_callback(port, qid, pdump_tx, cbs);
      if (cbs->cb == NULL) {
        PDUMP_LOG(ERR, "failed to add tx callback, errno=%d\n", rte_errno);
        return rte_errno;
      }
    } else if (operation == DISABLE) {
      int ret;
      ...
      if (cbs->cb == NULL) {
        PDUMP_LOG(ERR, "no existing tx callback for port=%d queue=%d\n", port,
                  qid);
        return -EINVAL;
      }
      ret = rte_eth_remove_tx_callback(port, qid, cbs->cb);
      if (ret < 0) {
        PDUMP_LOG(ERR, "failed to remove tx callback, errno=%d\n", -ret);
        return ret;
      }
      cbs->cb = NULL;
    }
...
static int set_pdump_rxtx_cbs(const struct pdump_request *p) {
  ...
  if (!(p->ver == V1 || p->ver == V2)) {
    PDUMP_LOG(ERR, "incorrect client version %u\n", p->ver);
    return -EINVAL;
  }

  if (p->prm) {
    if (p->prm->prog_arg.type != RTE_BPF_ARG_PTR_MBUF) {
      PDUMP_LOG(ERR, "invalid BPF program type: %u\n", p->prm->prog_arg.type);
      return -EINVAL;
    }
  ...
  if (ret < 0) {
    PDUMP_LOG(ERR, "failed to get port id for device id=%s\n", p->device);
    return -EINVAL;
  }
  ...
  if (queue == RTE_PDUMP_ALL_QUEUES) {
    ...
    if (ret != 0) {
      PDUMP_LOG(ERR, "Error during getting device (port %u) info: %s\n", port,
                strerror(-ret));
      return ret;
    }
    ...
    if (nb_rx_q == 0 && flags & RTE_PDUMP_FLAG_RX) {
      PDUMP_LOG(ERR, "number of rx queues cannot be 0\n");
      return -EINVAL;
    }
    if (nb_tx_q == 0 && flags & RTE_PDUMP_FLAG_TX) {
      PDUMP_LOG(ERR, "number of tx queues cannot be 0\n");
      return -EINVAL;
    }
    if ((nb_tx_q == 0 || nb_rx_q == 0) && flags == RTE_PDUMP_FLAG_RXTX) {
      PDUMP_LOG(ERR, "both tx&rx queues must be non zero\n");
      return -EINVAL;
    }
  }
...
static int pdump_validate_ring_mp(struct rte_ring *ring,
                                  struct rte_mempool *mp) {
  ...
  if (mp->flags & RTE_MEMPOOL_F_SP_PUT || mp->flags & RTE_MEMPOOL_F_SC_GET) {
    PDUMP_LOG(ERR, "mempool with SP or SC set not valid for pdump,"
                   "must have MP and MC set\n");
    rte_errno = EINVAL;
    return -1;
  }
  if (rte_ring_is_prod_single(ring) || rte_ring_is_cons_single(ring)) {
    PDUMP_LOG(ERR, "ring with SP or SC set is not valid for pdump,"
                   "must have MP and MC set\n");
    rte_errno = EINVAL;
    return -1;
  }
...
static int pdump_validate_flags(uint32_t flags) {
  if ((flags & RTE_PDUMP_FLAG_RXTX) == 0) {
    PDUMP_LOG(ERR, "invalid flags, should be either rx/tx/rxtx\n");
    rte_errno = EINVAL;
    return -1;
  }
...
static int pdump_validate_port(uint16_t port, char *name) {
  int ret = 0;
...
static int pdump_prepare_client_request(const char *device, uint16_t queue,
  ...
  if (ret < 0)
    PDUMP_LOG(ERR, "client request for pdump enable/disable failed\n");
  return ret;
}
 ...
 * bogus value.
 */
static int pdump_enable(uint16_t port, uint16_t queue, uint32_t flags,
                        uint32_t snaplen, struct rte_ring *ring,
                        struct rte_mempool *mp, const struct rte_bpf_prm *prm) {
  int ret;
  ...
  return pdump_prepare_client_request(name, queue, flags, snaplen, ENABLE, ring,
                                      mp, prm);
}
...
int rte_pdump_enable(uint16_t port, uint16_t queue, uint32_t flags,
  ...
  return pdump_enable(port, queue, flags, 0, ring, mp, NULL);
}

int rte_pdump_enable_bpf(uint16_t port, uint16_t queue, uint32_t flags,
                         uint32_t snaplen, struct rte_ring *ring,
                         struct rte_mempool *mp,
                         const struct rte_bpf_prm *prm) {
  return pdump_enable(port, queue, flags, snaplen, ring, mp, prm);
}

static int pdump_enable_by_deviceid(const char *device_id, uint16_t queue,
                                    uint32_t flags, uint32_t snaplen,
                                    struct rte_ring *ring,
                                    struct rte_mempool *mp,
                                    const struct rte_bpf_prm *prm) {
  ...
  return pdump_prepare_client_request(device_id, queue, flags, snaplen, ENABLE,
                                      ring, mp, prm);
}
...
int rte_pdump_enable_by_deviceid(char *device_id, uint16_t queue,
                                 ...
                                 struct rte_mempool *mp,
                                 void *filter __rte_unused) {
  return pdump_enable_by_deviceid(device_id, queue, flags, 0, ring, mp, NULL);
}

int rte_pdump_enable_bpf_by_deviceid(const char *device_id, uint16_t queue,
                                     uint32_t flags, uint32_t snaplen,
                                     struct rte_ring *ring,
                                     struct rte_mempool *mp,
                                     const struct rte_bpf_prm *prm) {
  return pdump_enable_by_deviceid(device_id, queue, flags, snaplen, ring, mp,
                                  prm);
}
...
int rte_pdump_disable(uint16_t port, uint16_t queue, uint32_t flags) {
  ...
  ret = pdump_prepare_client_request(name, queue, flags, 0, DISABLE, NULL, NULL,
                                     NULL);
...
int rte_pdump_disable_by_deviceid(char *device_id, uint16_t queue,
  ...
  ret = pdump_prepare_client_request(device_id, queue, flags, 0, DISABLE, NULL,
                                     NULL, NULL);

  return ret;
}

static void pdump_sum_stats(
    uint16_t port, uint16_t nq,
...
int rte_pdump_stats(uint16_t port, struct rte_pdump_stats *stats) {
  struct rte_eth_dev_info dev_info;
  const struct rte_memzone *mz;
  int ret;
