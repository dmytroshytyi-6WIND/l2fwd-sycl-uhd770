 ...
 */
int rte_member_create_vbf(struct rte_member_setsum *ss,
                          const struct rte_member_parameters *params) {

  if (params->num_set > RTE_MEMBER_MAX_BF ||
      !rte_is_power_of_2(params->num_set) || params->num_keys == 0 ||
      params->false_positive_rate == 0 || params->false_positive_rate > 1) {
    rte_errno = EINVAL;
  ...
  float fp_one_bf =
      1 - pow((1 - params->false_positive_rate), 1.0 / ss->num_set);
  ...
  uint32_t bits = ceil((num_keys_per_bf * log(fp_one_bf)) /
   ...
   */
  float new_fp =
      pow((1 - pow((1 - 1.0 / ss->bits), num_keys_per_bf * ss->num_hashes)),
          ss->num_hashes);
  ...
  while (tmp_num_hash > 1) {
    ...
    tmp_num_hash--;
    new_fp =
        pow((1 - pow((1 - 1.0 / ss->bits), num_keys_per_bf * tmp_num_hash)),
            tmp_num_hash);
    new_fp = 1 - pow((1 - new_fp), ss->num_set);
...
static inline uint32_t test_bit(uint32_t bit_loc,
...
static inline void set_bit(uint32_t bit_loc, const struct rte_member_setsum *ss,
                           int32_t set) {
  uint32_t *vbf = ss->table;
  uint32_t div_shift = ss->div_shift;
  uint32_t mul_shift = ss->mul_shift;
...
int rte_member_lookup_vbf(const struct rte_member_setsum *ss, const void *key,
                          member_set_t *set_id) {
  uint32_t j;
  uint32_t h1 = MEMBER_HASH_FUNC(key, ss->key_len, ss->prim_hash_seed);
  uint32_t h2 = MEMBER_HASH_FUNC(&h1, sizeof(uint32_t), ss->sec_hash_seed);
  uint32_t mask = ~0;
  uint32_t bit_loc;
...
uint32_t rte_member_lookup_bulk_vbf(const struct rte_member_setsum *ss,
  ...
  for (i = 0; i < num_keys; i++)
    h1[i] = MEMBER_HASH_FUNC(keys[i], ss->key_len, ss->prim_hash_seed);
  for (i = 0; i < num_keys; i++)
    h2[i] = MEMBER_HASH_FUNC(&h1[i], sizeof(uint32_t), ss->sec_hash_seed);
  for (i = 0; i < num_keys; i++) {
    mask[i] = ~0;
...
uint32_t rte_member_lookup_multi_vbf(const struct rte_member_setsum *ss,
...
uint32_t rte_member_lookup_multi_bulk_vbf(const struct rte_member_setsum *ss,
                                          const void **keys, uint32_t num_keys,
...
int rte_member_add_vbf(const struct rte_member_setsum *ss, const void *key,
                       member_set_t set_id) {
  uint32_t i, h1, h2;
  uint32_t bit_loc;
...
void rte_member_free_vbf(struct rte_member_setsum *ss) { rte_free(ss->table); }

void rte_member_reset_vbf(const struct rte_member_setsum *ss) {
  uint32_t *vbf = ss->table;
