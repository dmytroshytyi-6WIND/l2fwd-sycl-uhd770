...
rte_member_create(const struct rte_member_parameters *params) {
  ...
  if (params->key_len == 0 || params->prim_hash_seed == params->sec_hash_seed) {
    rte_errno = EINVAL;
  ...
  TAILQ_FOREACH(te, member_list, next) {
    setsum = te->data;
    if (strncmp(params->name, setsum->name, RTE_MEMBER_NAMESIZE) == 0)
      break;
  }
  setsum = NULL;
  if (te != NULL) {
  ...
  setsum = rte_zmalloc_socket(params->name, sizeof(struct rte_member_setsum),
...
int rte_member_add(const struct rte_member_setsum *setsum, const void *key,
                   member_set_t set_id) {
...
int rte_member_lookup(const struct rte_member_setsum *setsum, const void *key,
                      member_set_t *set_id) {
...
int rte_member_lookup_bulk(const struct rte_member_setsum *setsum,
  ...
  switch (setsum->type) {
  case RTE_MEMBER_TYPE_HT:
    return rte_member_lookup_bulk_ht(setsum, keys, num_keys, set_ids);
  case RTE_MEMBER_TYPE_VBF:
    return rte_member_lookup_bulk_vbf(setsum, keys, num_keys, set_ids);
  default:
    return -EINVAL;
  }
}
...
int rte_member_lookup_multi(const struct rte_member_setsum *setsum,
  ...
  switch (setsum->type) {
  case RTE_MEMBER_TYPE_HT:
    return rte_member_lookup_multi_ht(setsum, key, match_per_key, set_id);
  case RTE_MEMBER_TYPE_VBF:
    return rte_member_lookup_multi_vbf(setsum, key, match_per_key, set_id);
  default:
    return -EINVAL;
  }
}
...
int rte_member_lookup_multi_bulk(const struct rte_member_setsum *setsum,
  ...
  if (setsum == NULL || keys == NULL || set_ids == NULL || match_count == NULL)
    return -EINVAL;

  switch (setsum->type) {
  case RTE_MEMBER_TYPE_HT:
    return rte_member_lookup_multi_bulk_ht(
        setsum, keys, num_keys, max_match_per_key, match_count, set_ids);
  case RTE_MEMBER_TYPE_VBF:
    return rte_member_lookup_multi_bulk_vbf(
        setsum, keys, num_keys, max_match_per_key, match_count, set_ids);
  default:
    return -EINVAL;
  }
...
int rte_member_delete(const struct rte_member_setsum *setsum, const void *key,
                      member_set_t set_id) {
  if (setsum == NULL || key == NULL)
    return -EINVAL;
