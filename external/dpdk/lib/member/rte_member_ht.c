...
static inline int update_entry_search(uint32_t bucket_id, member_sig_t tmp_sig,
...
static inline int search_bucket_single(uint32_t bucket_id, member_sig_t tmp_sig,
                                       struct member_ht_bucket *buckets,
                                       member_set_t *set_id) {
  uint32_t iter;
  ...
  for (iter = 0; iter < RTE_MEMBER_BUCKET_ENTRIES; iter++) {
    if (tmp_sig == buckets[bucket_id].sigs[iter] &&
        buckets[bucket_id].sets[iter] != RTE_MEMBER_NO_MATCH) {
...
static inline void search_bucket_multi(uint32_t bucket_id, member_sig_t tmp_sig,
                                       struct member_ht_bucket *buckets,
                                       uint32_t *counter,
...
int rte_member_create_ht(struct rte_member_setsum *ss,
  ...
  if ((num_entries > RTE_MEMBER_ENTRIES_MAX) ||
    ...
    rte_errno = EINVAL;
    RTE_MEMBER_LOG(ERR, "Membership HT create with invalid parameters\n");
    return -EINVAL;
  }
  ...
  struct member_ht_bucket *buckets = rte_zmalloc_socket(
      NULL, num_buckets * size_bucket_t, RTE_CACHE_LINE_SIZE, ss->socket_id);
  ...
  RTE_MEMBER_LOG(DEBUG,
                 "Hash table based filter created, "
                 "the table has %u entries, %u buckets\n",
                 num_entries, num_buckets);
  return 0;
}

static inline void get_buckets_index(const struct rte_member_setsum *ss,
                                     const void *key, uint32_t *prim_bkt,
                                     uint32_t *sec_bkt, member_sig_t *sig) {
  uint32_t first_hash = MEMBER_HASH_FUNC(key, ss->key_len, ss->prim_hash_seed);
  uint32_t sec_hash =
      MEMBER_HASH_FUNC(&first_hash, sizeof(uint32_t), ss->sec_hash_seed);
  /*
...
int rte_member_lookup_ht(const struct rte_member_setsum *ss, const void *key,
                         member_set_t *set_id) {
  ...
  *set_id = RTE_MEMBER_NO_MATCH;
  ...
  case RTE_MEMBER_COMPARE_AVX2:
    if (search_bucket_single_avx(prim_bucket, tmp_sig, buckets, set_id) ||
        search_bucket_single_avx(sec_bucket, tmp_sig, buckets, set_id))
      return 1;
    break;
#endif
  default:
    if (search_bucket_single(prim_bucket, tmp_sig, buckets, set_id) ||
        search_bucket_single(sec_bucket, tmp_sig, buckets, set_id))
      return 1;
  }
...
uint32_t rte_member_lookup_bulk_ht(const struct rte_member_setsum *ss,
  ...
  for (i = 0; i < num_keys; i++) {
    get_buckets_index(ss, keys[i], &prim_buckets[i], &sec_buckets[i],
                      &tmp_sig[i]);
...
#if defined(RTE_ARCH_X86) && defined(__AVX2__)
    case RTE_MEMBER_COMPARE_AVX2:
      if (search_bucket_single_avx(prim_buckets[i], tmp_sig[i], buckets,
                                   &set_id[i]) ||
          search_bucket_single_avx(sec_buckets[i], tmp_sig[i], buckets,
                                   &set_id[i]))
        num_matches++;
      else
...
#endif
    default:
      if (search_bucket_single(prim_buckets[i], tmp_sig[i], buckets,
                               &set_id[i]) ||
          search_bucket_single(sec_buckets[i], tmp_sig[i], buckets, &set_id[i]))
        num_matches++;
      else
        set_id[i] = RTE_MEMBER_NO_MATCH;
    }
...
uint32_t rte_member_lookup_multi_ht(const struct rte_member_setsum *ss,
...
#if defined(RTE_ARCH_X86) && defined(__AVX2__)
  case RTE_MEMBER_COMPARE_AVX2:
    search_bucket_multi_avx(prim_bucket, tmp_sig, buckets, &num_matches,
                            match_per_key, set_id);
    if (num_matches < match_per_key)
      search_bucket_multi_avx(sec_bucket, tmp_sig, buckets, &num_matches,
                              match_per_key, set_id);
    return num_matches;
#endif
  default:
    search_bucket_multi(prim_bucket, tmp_sig, buckets, &num_matches,
                        match_per_key, set_id);
    if (num_matches < match_per_key)
      search_bucket_multi(sec_bucket, tmp_sig, buckets, &num_matches,
                          match_per_key, set_id);
    return num_matches;
  }
}

uint32_t rte_member_lookup_multi_bulk_ht(const struct rte_member_setsum *ss,
                                         const void **keys, uint32_t num_keys,
...
#if defined(RTE_ARCH_X86) && defined(__AVX2__)
    case RTE_MEMBER_COMPARE_AVX2:
      search_bucket_multi_avx(prim_buckets[i], tmp_sig[i], buckets,
                              &match_cnt_tmp, match_per_key,
                              &set_ids[i * match_per_key]);
      if (match_cnt_tmp < match_per_key)
        search_bucket_multi_avx(sec_buckets[i], tmp_sig[i], buckets,
                                &match_cnt_tmp, match_per_key,
                                &set_ids[i * match_per_key]);
...
#endif
    default:
      search_bucket_multi(prim_buckets[i], tmp_sig[i], buckets, &match_cnt_tmp,
                          match_per_key, &set_ids[i * match_per_key]);
      if (match_cnt_tmp < match_per_key)
        search_bucket_multi(sec_buckets[i], tmp_sig[i], buckets, &match_cnt_tmp,
                            match_per_key, &set_ids[i * match_per_key]);
      match_count[i] = match_cnt_tmp;
      if (match_cnt_tmp != 0)
        num_matches++;
...
static inline int try_insert(struct member_ht_bucket *buckets, uint32_t prim,
                             uint32_t sec, member_sig_t sig,
                             member_set_t set_id) {
  int i;
...
static inline int try_update(struct member_ht_bucket *buckets, uint32_t prim,
                             uint32_t sec, member_sig_t sig,
                             member_set_t set_id,
  ...
  case RTE_MEMBER_COMPARE_AVX2:
    if (update_entry_search_avx(prim, sig, buckets, set_id) ||
        update_entry_search_avx(sec, sig, buckets, set_id))
      return 0;
    break;
#endif
  default:
...
static inline int evict_from_bucket(void) {
 ...
 * library
 */
static inline int make_space_bucket(const struct rte_member_setsum *ss,
  ...
  if (i == RTE_MEMBER_BUCKET_ENTRIES || ++(*nr_pushes) > RTE_MEMBER_MAX_PUSHES)
    return -ENOSPC;
...
int rte_member_add_ht(const struct rte_member_setsum *ss, const void *key,
                      member_set_t set_id) {
  int ret;
   ...
   */
  if (ss->cache) {
    ret = try_update(buckets, prim_bucket, sec_bucket, tmp_sig, set_id,
                     ss->sig_cmp_fn);
    if (ret != -1)
      return ret;
  }
...
void rte_member_free_ht(struct rte_member_setsum *ss) { rte_free(ss->table); }

int rte_member_delete_ht(const struct rte_member_setsum *ss, const void *key,
                         member_set_t set_id) {
  int i;
  uint32_t prim_bucket, sec_bucket;
  member_sig_t tmp_sig;
...
void rte_member_reset_ht(const struct rte_member_setsum *ss) {
  uint32_t i, j;
  struct member_ht_bucket *buckets = ss->table;
