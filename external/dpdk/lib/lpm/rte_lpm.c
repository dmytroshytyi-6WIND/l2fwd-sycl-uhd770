...
#define VERIFY_DEPTH(depth)                                                    \
  do {                                                                         \
    if ((depth == 0) || (depth > RTE_LPM_MAX_DEPTH))                           \
      rte_panic("LPM: Invalid depth (%u) at line %d", (unsigned)(depth),       \
                __LINE__);                                                     \
  } while (0)
#else
#define VERIFY_DEPTH(depth)
 ...
 * mask   (OUT)		: 32bit mask
 */
static uint32_t __attribute__((pure)) depth_to_mask(uint8_t depth) {
  VERIFY_DEPTH(depth);
 ...
 */
static uint32_t __attribute__((pure)) depth_to_range(uint8_t depth) {
  VERIFY_DEPTH(depth);
...
struct rte_lpm *rte_lpm_create(const char *name, int socket_id,
  ...
  i_lpm =
      rte_zmalloc_socket(mem_name, mem_size, RTE_CACHE_LINE_SIZE, socket_id);
  if (i_lpm == NULL) {
  ...
  i_lpm->rules_tbl = rte_zmalloc_socket(NULL, (size_t)rules_size,
  ...
  i_lpm->lpm.tbl8 = rte_zmalloc_socket(NULL, (size_t)tbl8s_size,
                                       RTE_CACHE_LINE_SIZE, socket_id);
...
static void __lpm_rcu_qsbr_free_resource(void *p, void *data, unsigned int n) {
  ...
  __atomic_store(&tbl8[tbl8_group_index], &zero_tbl8_entry, __ATOMIC_RELAXED);
}
...
int rte_lpm_rcu_qsbr_add(struct rte_lpm *lpm, struct rte_lpm_rcu_config *cfg) {
  ...
  if (cfg->mode == RTE_LPM_QSBR_MODE_SYNC) {
    ...
    snprintf(rcu_dq_name, sizeof(rcu_dq_name), "LPM_RCU_%s", i_lpm->name);
 ...
 */
static int32_t rule_add(struct __rte_lpm *i_lpm, uint32_t ip_masked,
  ...
  if (i_lpm->rule_info[depth - 1].used_rules > 0) {
    ...
    for (; rule_index < last_rule; rule_index++) {
      ...
      if (i_lpm->rules_tbl[rule_index].ip == ip_masked) {
        ...
        if (i_lpm->rules_tbl[rule_index].next_hop == next_hop)
          return -EEXIST;
 ...
 * NOTE: Valid range for depth parameter is 1 .. 32 inclusive.
 */
static void rule_delete(struct __rte_lpm *i_lpm, int32_t rule_index,
                        uint8_t depth) {
  int i;
 ...
 * NOTE: Valid range for depth parameter is 1 .. 32 inclusive.
 */
static int32_t rule_find(struct __rte_lpm *i_lpm, uint32_t ip_masked,
                         uint8_t depth) {
 ...
 */
static int32_t _tbl8_alloc(struct __rte_lpm *i_lpm) {
  uint32_t group_idx; /* tbl8 group index. */
  struct rte_lpm_tbl_entry *tbl8_entry;

  /* Scan through tbl8 to find a free (i.e. INVALID) tbl8 group. */
  for (group_idx = 0; group_idx < i_lpm->number_tbl8s; group_idx++) {
    tbl8_entry = &i_lpm->lpm.tbl8[group_idx * RTE_LPM_TBL8_GROUP_NUM_ENTRIES];
...
static int32_t tbl8_free(struct __rte_lpm *i_lpm, uint32_t tbl8_group_start) {
  struct rte_lpm_tbl_entry zero_tbl8_entry = {0};
  int status;
  ...
  if (i_lpm->v == NULL) {
    ...
    rte_rcu_qsbr_synchronize(i_lpm->v, RTE_QSBR_THRID_INVALID);
    /* Set tbl8 group invalid*/
    __atomic_store(&i_lpm->lpm.tbl8[tbl8_group_start], &zero_tbl8_entry,
                   __ATOMIC_RELAXED);
  } else if (i_lpm->rcu_mode == RTE_LPM_QSBR_MODE_DQ) {
    /* Push into QSBR defer queue. */
    status = rte_rcu_qsbr_dq_enqueue(i_lpm->dq, (void *)&tbl8_group_start);
    if (status == 1) {
...
static __rte_noinline int32_t add_depth_small(struct __rte_lpm *i_lpm,
...
#define group_idx next_hop
  ...
  for (i = tbl24_index; i < (tbl24_index + tbl24_range); i++) {
    ...
    if (!i_lpm->lpm.tbl24[i].valid || (i_lpm->lpm.tbl24[i].valid_group == 0 &&
       ...
       * conditions
       */
      __atomic_store(&i_lpm->lpm.tbl24[i], &new_tbl24_entry, __ATOMIC_RELEASE);
    ...
    if (i_lpm->lpm.tbl24[i].valid_group == 1) {
       ...
       */
      tbl8_index =
          i_lpm->lpm.tbl24[i].group_idx * RTE_LPM_TBL8_GROUP_NUM_ENTRIES;
      tbl8_group_end = tbl8_index + RTE_LPM_TBL8_GROUP_NUM_ENTRIES;

      for (j = tbl8_index; j < tbl8_group_end; j++) {
        if (!i_lpm->lpm.tbl8[j].valid || i_lpm->lpm.tbl8[j].depth <= depth) {
          struct rte_lpm_tbl_entry new_tbl8_entry = {
              .valid = VALID,
              .valid_group = VALID,
              .depth = depth,
           ...
           * race conditions
           */
          __atomic_store(&i_lpm->lpm.tbl8[j], &new_tbl8_entry,
                         __ATOMIC_RELAXED);
...
static __rte_noinline int32_t add_depth_big(struct __rte_lpm *i_lpm,
...
#define group_idx next_hop
  ...
  if (!i_lpm->lpm.tbl24[tbl24_index].valid) {
    ...
    for (i = tbl8_index; i < (tbl8_index + tbl8_range); i++) {
      ...
      __atomic_store(&i_lpm->lpm.tbl8[i], &new_tbl8_entry, __ATOMIC_RELAXED);
    }
    ...
    for (i = tbl8_index; i < (tbl8_index + tbl8_range); i++) {
      ...
      if (!i_lpm->lpm.tbl8[i].valid || i_lpm->lpm.tbl8[i].depth <= depth) {
        struct rte_lpm_tbl_entry new_tbl8_entry = {
            .valid = VALID,
            .depth = depth,
            .next_hop = next_hop,
 ...
 */
int rte_lpm_is_rule_present(struct rte_lpm *lpm, uint32_t ip, uint8_t depth,
                            uint32_t *next_hop) {
  struct __rte_lpm *i_lpm;
  uint32_t ip_masked;
  int32_t rule_index;
...
static int32_t find_previous_rule(struct __rte_lpm *i_lpm, uint32_t ip,
...
static int32_t delete_depth_small(struct __rte_lpm *i_lpm, uint32_t ip_masked,
  ...
  if (sub_rule_index < 0) {
    ...
    for (i = tbl24_index; i < (tbl24_index + tbl24_range); i++) {
      ...
      if (i_lpm->lpm.tbl24[i].valid_group == 0 &&
          i_lpm->lpm.tbl24[i].depth <= depth) {
        __atomic_store(&i_lpm->lpm.tbl24[i], &zero_tbl24_entry,
                       __ATOMIC_RELEASE);
        ...
        for (j = tbl8_index; j < (tbl8_index + RTE_LPM_TBL8_GROUP_NUM_ENTRIES);
             j++) {
 ...
 */
static int32_t tbl8_recycle_check(struct rte_lpm_tbl_entry *tbl8,
...
static int32_t delete_depth_big(struct __rte_lpm *i_lpm, uint32_t ip_masked,
                                uint8_t depth, int32_t sub_rule_index,
                                uint8_t sub_rule_depth) {
#define group_idx next_hop
...
int rte_lpm_delete(struct rte_lpm *lpm, uint32_t ip, uint8_t depth) {
   ...
   */
  if (depth <= MAX_DEPTH_TBL24) {
    return delete_depth_small(i_lpm, ip_masked, depth, sub_rule_index,
                              sub_rule_depth);
...
void rte_lpm_delete_all(struct rte_lpm *lpm) {
  ...
  /* Zero tbl8. */
  memset(i_lpm->lpm.tbl8, 0,
         sizeof(i_lpm->lpm.tbl8[0]) * RTE_LPM_TBL8_GROUP_NUM_ENTRIES *
             i_lpm->number_tbl8s);
