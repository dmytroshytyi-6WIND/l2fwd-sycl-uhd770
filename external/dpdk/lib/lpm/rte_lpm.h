...
__extension__ struct rte_lpm_tbl_entry {
  /**
...
struct rte_lpm {
  ...
  struct rte_lpm_tbl_entry
      tbl24[RTE_LPM_TBL24_NUM_ENTRIES] __rte_cache_aligned; /**< LPM tbl24
                                                               table. */
...
static inline int rte_lpm_lookup(struct rte_lpm *lpm, uint32_t ip,
  ...
  if (unlikely((tbl_entry & RTE_LPM_VALID_EXT_ENTRY_BITMASK) ==
    ...
    unsigned tbl8_index = (uint8_t)ip + (((uint32_t)tbl_entry & 0x00FFFFFF) *
...
static inline int rte_lpm_lookup_bulk_func(const struct rte_lpm *lpm,
                                           const uint32_t *ips,
                                           uint32_t *next_hops,
                                           const unsigned n) {
  unsigned i;
  ...
  RTE_LPM_RETURN_IF_TRUE(
      ((lpm == NULL) || (ips == NULL) || (next_hops == NULL)), -EINVAL);
  ...
  for (i = 0; i < n; i++) {
    ...
    if (unlikely((next_hops[i] & RTE_LPM_VALID_EXT_ENTRY_BITMASK) ==
      ...
      unsigned tbl8_index =
          (uint8_t)ips[i] + (((uint32_t)next_hops[i] & 0x00FFFFFF) *
                             RTE_LPM_TBL8_GROUP_NUM_ENTRIES);
 ...
 */
static inline void rte_lpm_lookupx4(const struct rte_lpm *lpm, xmm_t ip,
