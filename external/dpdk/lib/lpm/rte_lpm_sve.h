...
__rte_lpm_lookup_vec(const struct rte_lpm *lpm, const uint32_t *ips,
  ...
  do {
    ...
    v_tbl24 = svld1_gather_index(pg, (const uint32_t *)lpm->tbl24, v_idx);
    ...
    v_idx = svadd_x(pv, svand_x(pv, v_ip, svdup_u32_z(pv, 0xff)), v_idx);
    /* Extract values from tbl8[] */
    v_tbl8 = svld1_gather_index(pv, (const uint32_t *)lpm->tbl8, v_idx);
