...
enum valid_flag { INVALID = 0, VALID };
...
struct rte_lpm6 {
  ...
  struct rte_lpm6_tbl_entry
      tbl24[RTE_LPM6_TBL24_NUM_ENTRIES] __rte_cache_aligned; /**< LPM tbl24
                                                                table. */
  ...
  struct rte_lpm6_tbl_entry tbl8[0] __rte_cache_aligned; /**< LPM tbl8 table. */
};
 ...
 */
static inline void ip6_mask_addr(uint8_t *ip, uint8_t depth) {
...
/* copy ipv6 address */
static inline void ip6_copy_addr(uint8_t *dst, const uint8_t *src) {
...
rule_hash(const void *data, __rte_unused uint32_t data_len, uint32_t init_val) {
 ...
 */
static void tbl8_pool_init(struct rte_lpm6 *lpm) {
  uint32_t i;
 ...
 */
static inline uint32_t tbl8_get(struct rte_lpm6 *lpm, uint32_t *tbl8_ind) {
 ...
 */
static inline uint32_t tbl8_put(struct rte_lpm6 *lpm, uint32_t tbl8_ind) {
 ...
 */
static inline uint32_t tbl8_available(struct rte_lpm6 *lpm) {
 ...
 */
static inline void rule_key_init(struct rte_lpm6_rule_key *key, uint8_t *ip,
                                 uint8_t depth) {
 ...
 */
static void rebuild_lpm(struct rte_lpm6 *lpm) {
  uint64_t next_hop;
  struct rte_lpm6_rule_key *rule_key;
  uint32_t iter = 0;

  while (rte_hash_iterate(lpm->rules_tbl, (void *)&rule_key, (void **)&next_hop,
                          &iter) >= 0)
    rte_lpm6_add(lpm, rule_key->ip, rule_key->depth, (uint32_t)next_hop);
}

/*
 * Allocates memory for LPM object
 */
struct rte_lpm6 *rte_lpm6_create(const char *name, int socket_id,
  ...
  RTE_BUILD_BUG_ON(sizeof(struct rte_lpm6_rule_key) % sizeof(uint32_t) != 0);
  ...
  tbl8_pool = rte_malloc(NULL, sizeof(uint32_t) * config->number_tbl8s,
                         RTE_CACHE_LINE_SIZE);
  if (tbl8_pool == NULL) {
  ...
  tbl8_hdrs =
      rte_malloc(NULL, sizeof(struct rte_lpm_tbl8_hdr) * config->number_tbl8s,
                 RTE_CACHE_LINE_SIZE);
  if (tbl8_hdrs == NULL) {
  ...
  mem_size =
      sizeof(*lpm) + (sizeof(lpm->tbl8[0]) * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES *
                      config->number_tbl8s);
  ...
  lpm = rte_zmalloc_socket(mem_name, (size_t)mem_size, RTE_CACHE_LINE_SIZE,
                           socket_id);
...
/* Find a rule */
static inline int rule_find_with_key(struct rte_lpm6 *lpm,
...
/* Find a rule */
static int rule_find(struct rte_lpm6 *lpm, uint8_t *ip, uint8_t depth,
                     uint32_t *next_hop) {
  struct rte_lpm6_rule_key rule_key;
 ...
 *   <0 - error
 */
static inline int rule_add(struct rte_lpm6 *lpm, uint8_t *ip, uint8_t depth,
                           uint32_t next_hop) {
  int ret, rule_exist;
  struct rte_lpm6_rule_key rule_key;
  uint32_t unused;
 ...
 */
static void expand_rule(struct rte_lpm6 *lpm, uint32_t tbl8_gindex,
                        uint8_t old_depth, uint8_t new_depth, uint32_t next_hop,
                        uint8_t valid) {
  ...
  for (j = tbl8_gindex; j < tbl8_group_end; j++) {
    if (!lpm->tbl8[j].valid ||
        (lpm->tbl8[j].ext_entry == 0 && lpm->tbl8[j].depth <= old_depth)) {
      ...
      tbl8_gindex_next =
          lpm->tbl8[j].lpm6_tbl8_gindex * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES;
      expand_rule(lpm, tbl8_gindex_next, old_depth, new_depth, next_hop, valid);
    }
  }
}
 ...
 * Init a tbl8 header
 */
static inline void init_tbl8_header(struct rte_lpm6 *lpm, uint32_t tbl_ind,
                                    uint32_t owner_tbl_ind,
                                    uint32_t owner_entry_ind) {
 ...
 */
static uint32_t get_bitshift(const uint8_t *ip, uint8_t first_byte,
                             uint8_t bytes) {
 ...
 */
static inline int simulate_add_step(struct rte_lpm6 *lpm,
                                    struct rte_lpm6_tbl_entry *tbl,
                                    struct rte_lpm6_tbl_entry **next_tbl,
                                    const uint8_t *ip, uint8_t bytes,
                                    uint8_t first_byte, uint8_t depth,
  ...
  *next_tbl = &(lpm->tbl8[next_tbl_ind * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES]);
  *need_tbl_nb = 0;
  return 1;
}
 ...
 * the process needs to be continued by calling the function again.
 */
static inline int add_step(struct rte_lpm6 *lpm, struct rte_lpm6_tbl_entry *tbl,
                           uint32_t tbl_ind,
                           struct rte_lpm6_tbl_entry **next_tbl,
  ...
  if (depth <= bits_covered) {
    ...
    for (i = entry_ind; i < (entry_ind + tbl_range); i++) {
      if (!tbl[i].valid || (tbl[i].ext_entry == 0 && tbl[i].depth <= depth)) {
         ...
         */
        tbl8_gindex = tbl[i].lpm6_tbl8_gindex * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES;
        expand_rule(lpm, tbl8_gindex, depth, depth, next_hop, VALID);
      }
    }
   ...
   */
    ...
    if (!tbl[entry_ind].valid) {
      ...
      tbl8_group_start = tbl8_gindex * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES;
     ...
     */
      ...
      tbl8_group_start = tbl8_gindex * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES;
      tbl8_group_end = tbl8_group_start + RTE_LPM6_TBL8_GROUP_NUM_ENTRIES;

      struct rte_lpm6_tbl_entry tbl_entry = {.next_hop =
                                                 tbl[entry_ind].next_hop,
 ...
 */
static int simulate_add(struct rte_lpm6 *lpm, const uint8_t *masked_ip,
                        uint8_t depth) {
  struct rte_lpm6_tbl_entry *tbl;
  ...
  ret = simulate_add_step(lpm, lpm->tbl24, &tbl_next, masked_ip, ADD_FIRST_BYTE,
                          1, depth, &need_tbl_nb);
  ...
  for (i = ADD_FIRST_BYTE; i < RTE_LPM6_IPV6_ADDR_SIZE && ret == 1; i++) {
    tbl = tbl_next;
    ret = simulate_add_step(lpm, tbl, &tbl_next, masked_ip, 1, (uint8_t)(i + 1),
                            depth, &need_tbl_nb);
    total_need_tbl_nb += need_tbl_nb;
  }
 ...
 * Add a route
 */
int rte_lpm6_add(struct rte_lpm6 *lpm, const uint8_t *ip, uint8_t depth,
                 uint32_t next_hop) {
  struct rte_lpm6_tbl_entry *tbl;
  struct rte_lpm6_tbl_entry *tbl_next = NULL;
  ...
  /* Inspect the first three bytes through tbl24 on the first step. */
  tbl = lpm->tbl24;
  status = add_step(lpm, tbl, TBL24_IND, &tbl_next, &tbl_next_num, masked_ip,
                    ADD_FIRST_BYTE, 1, depth, next_hop, is_new_rule);
  assert(status >= 0);
  ...
  for (i = ADD_FIRST_BYTE; i < RTE_LPM6_IPV6_ADDR_SIZE && status == 1; i++) {
    tbl = tbl_next;
    status =
        add_step(lpm, tbl, tbl_next_num, &tbl_next, &tbl_next_num, masked_ip, 1,
                 (uint8_t)(i + 1), depth, next_hop, is_new_rule);
    assert(status >= 0);
  }
 ...
 */
static inline int lookup_step(const struct rte_lpm6 *lpm,
                              const struct rte_lpm6_tbl_entry *tbl,
                              const struct rte_lpm6_tbl_entry **tbl_next,
                              const uint8_t *ip, uint8_t first_byte,
                              uint32_t *next_hop) {
  ...
  if ((tbl_entry & RTE_LPM6_VALID_EXT_ENTRY_BITMASK) ==
    ...
    tbl8_index = ip[first_byte - 1] + ((tbl_entry & RTE_LPM6_TBL8_BITMASK) *
                                       RTE_LPM6_TBL8_GROUP_NUM_ENTRIES);
...
int rte_lpm6_lookup_bulk_func(const struct rte_lpm6 *lpm,
  ...
  for (i = 0; i < n; i++) {
    first_byte = LOOKUP_FIRST_BYTE;
    tbl24_index =
        (ips[i][0] << BYTES2_SIZE) | (ips[i][1] << BYTE_SIZE) | ips[i][2];
    ...
    do {
       ...
       */
      status =
          lookup_step(lpm, tbl, &tbl_next, ips[i], first_byte++, &next_hop);
      tbl = tbl_next;
    } while (status == 1);
...
int rte_lpm6_is_rule_present(struct rte_lpm6 *lpm, const uint8_t *ip,
  ...
  /* Check user arguments. */
  if ((lpm == NULL) || next_hop == NULL || ip == NULL || (depth < 1) ||
      (depth > RTE_LPM6_MAX_DEPTH))
    return -EINVAL;
 ...
 *   <0 on failure
 */
static inline int rule_delete(struct rte_lpm6 *lpm, uint8_t *ip,
                              uint8_t depth) {
  int ret;
  struct rte_lpm6_rule_key rule_key;
...
int rte_lpm6_delete_bulk_func(struct rte_lpm6 *lpm,
  ...
  memset(lpm->tbl8, 0,
         sizeof(lpm->tbl8[0]) * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES *
             lpm->number_tbl8s);
  tbl8_pool_init(lpm);
 ...
 */
static uint8_t __attribute__((pure)) depth_to_mask_1b(uint8_t depth) {
 ...
 */
static int rule_find_less_specific(struct rte_lpm6 *lpm, uint8_t *ip,
                                   uint8_t depth, struct rte_lpm6_rule *rule) {
  int ret;
  uint32_t next_hop;
  uint8_t mask;
  struct rte_lpm6_rule_key rule_key;
 ...
 */
static void rule_find_range(struct rte_lpm6 *lpm, const uint8_t *ip,
                            uint8_t depth, struct rte_lpm6_tbl_entry **from,
  ...
  if (depth <= 24) {
    ...
    *out_tbl_ind = TBL24_IND;
    ...
    tbl = &lpm->tbl8[tbl_ind * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES];
 ...
 */
static void remove_tbl(struct rte_lpm6 *lpm, struct rte_lpm_tbl8_hdr *tbl_hdr,
  ...
  if (tbl_hdr->owner_tbl_ind == TBL24_IND)
    ...
    owner_entry = &lpm->tbl8[owner_tbl_ind * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES +
                             tbl_hdr->owner_entry_ind];

    struct rte_lpm_tbl8_hdr *owner_tbl_hdr = &lpm->tbl8_hdrs[owner_tbl_ind];
  ...
  /* unlink the table */
  if (lsp_rule != NULL) {
    struct rte_lpm6_tbl_entry new_tbl_entry = {.next_hop = lsp_rule->next_hop,
    ...
    *owner_entry = new_tbl_entry;
    ...
    struct rte_lpm6_tbl_entry new_tbl_entry = {.next_hop = 0,
                                               .depth = 0,
                                               .valid = INVALID,
                                               .valid_group = INVALID,
                                               .ext_entry = 0};
...
int rte_lpm6_delete(struct rte_lpm6 *lpm, const uint8_t *ip, uint8_t depth) {
   ...
   */
  ret = rule_find_less_specific(lpm, masked_ip, depth, &lsp_rule_obj);
  ...
  for (; from <= to; from++)
    if (from->ext_entry == 1) {
      ...
      if (lsp_rule != NULL)
        expand_rule(lpm,
                    from->lpm6_tbl8_gindex * RTE_LPM6_TBL8_GROUP_NUM_ENTRIES,
                    depth, lsp_rule->depth, lsp_rule->next_hop, VALID);
      else
      ...
      if (lsp_rule != NULL) {
        struct rte_lpm6_tbl_entry new_tbl_entry = {.next_hop =
                                                       lsp_rule->next_hop,
                                                   .depth = lsp_rule->depth,
                                                   .valid = VALID,
                                                   .valid_group = VALID,
