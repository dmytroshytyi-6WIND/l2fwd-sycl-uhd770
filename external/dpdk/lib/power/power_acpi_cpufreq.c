 ...
 */
static int set_freq_internal(struct acpi_power_info *pi, uint32_t idx) {
 ...
 */
static int power_set_governor_userspace(struct acpi_power_info *pi) {
 ...
 */
static int power_set_governor_original(struct acpi_power_info *pi) {
 ...
 * sys file.
 */
static int power_get_available_freqs(struct acpi_power_info *pi) {
  FILE *f;
  int ret = -1, i, count;
  char *p;
  char buf[BUFSIZ];
  ...
  count = rte_strsplit(buf, sizeof(buf), freqs, RTE_MAX_LCORE_FREQS, ' ');
  if (count <= 0) {
    RTE_LOG(ERR, POWER,
            "No available frequency in "
            "" POWER_SYSFILE_AVAIL_FREQ "\n",
            pi->lcore_id);
    goto out;
  }
  ...
  for (i = 0, pi->nb_freqs = 0; i < count; i++) {
    POWER_DEBUG_TRACE("Lcore %u frequency[%d]: %s\n", pi->lcore_id, i,
                      freqs[i]);
    pi->freqs[pi->nb_freqs++] = strtoul(freqs[i], &p, POWER_CONVERT_TO_DECIMAL);
  }
 ...
 */
static int power_init_for_setting_freq(struct acpi_power_info *pi) {
  FILE *f;
  char buf[BUFSIZ];
  uint32_t i, freq;
  int ret;
