 ...
 */
static int set_freq_internal(struct cppc_power_info *pi, uint32_t idx) {
 ...
 */
static int power_set_governor_userspace(struct cppc_power_info *pi) {
  return power_set_governor(pi->lcore_id, POWER_GOVERNOR_USERSPACE,
                            pi->governor_ori, sizeof(pi->governor_ori));
}

static int power_check_turbo(struct cppc_power_info *pi) {
  FILE *f_nom = NULL, *f_max = NULL, *f_cmax = NULL;
  int ret = -1;
  uint32_t nominal_perf = 0, highest_perf = 0, cpuinfo_max_freq = 0;

  open_core_sysfs_file(&f_max, "r", POWER_SYSFILE_HIGHEST_PERF, pi->lcore_id);
  if (f_max == NULL) {
    RTE_LOG(ERR, POWER, "failed to open %s\n", POWER_SYSFILE_HIGHEST_PERF);
    goto err;
  }

  open_core_sysfs_file(&f_nom, "r", POWER_SYSFILE_NOMINAL_PERF, pi->lcore_id);
  if (f_nom == NULL) {
    RTE_LOG(ERR, POWER, "failed to open %s\n", POWER_SYSFILE_NOMINAL_PERF);
    goto err;
  }

  open_core_sysfs_file(&f_cmax, "r", POWER_SYSFILE_SYS_MAX, pi->lcore_id);
  if (f_cmax == NULL) {
 ...
 * sys file.
 */
static int power_get_available_freqs(struct cppc_power_info *pi) {
   ...
   */
  nominal_perf = (pi->nominal_perf < UNIT_DIFF) ? pi->nominal_perf * UNIT_DIFF
                                                : pi->nominal_perf;
  num_freqs =
      (nominal_perf - scaling_min_freq) / BUS_FREQ + 1 + pi->turbo_available;
 ...
 */
static int power_init_for_setting_freq(struct cppc_power_info *pi) {
  FILE *f = NULL;
  char buf[BUFSIZ];
  uint32_t i, freq;
  int ret;
  ...
  freq_conv = (freq + FREQ_ROUNDING_DELTA) / ROUND_FREQ_TO_N_100000;
 ...
 */
static int power_set_governor_original(struct cppc_power_info *pi) {
