...
int guest_channel_host_connect(const char *path, unsigned int lcore_id) {
  ...
  if (fd < 0) {
    RTE_LOG(ERR, GUEST_CHANNEL,
            "Unable to to connect to '%s' with error "
            "%s\n",
  ...
  if (fcntl(fd, F_SETFL, flags) < 0) {
    RTE_LOG(ERR, GUEST_CHANNEL,
            "Failed on setting non-blocking mode for "
            "file %s",
            fd_path);
    goto error;
  }
...
int power_guest_channel_read_msg(void *pkt, size_t pkt_len,
                                 unsigned int lcore_id) {
  int ret;
  struct pollfd fds;
  ...
  while (pkt_len > 0) {
    ret = read(global_fds[lcore_id], pkt, pkt_len);
    ...
    if (ret == 0) {
      RTE_LOG(ERR, GUEST_CHANNEL,
              "Expected more data, but connection has been closed.\n");
      return -1;
    }
    pkt = (char *)pkt + ret;
    pkt_len -= ret;
...
int rte_power_guest_channel_receive_msg(void *pkt, size_t pkt_len,
                                        unsigned int lcore_id) {
