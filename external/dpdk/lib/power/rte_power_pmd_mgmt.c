...
static inline bool queue_equal(const union queue *l, const union queue *r) {
  return l->val == r->val;
}

static inline void queue_copy(union queue *dst, const union queue *src) {
  dst->val = src->val;
}

static struct queue_list_entry *queue_list_find(const struct pmd_core_cfg *cfg,
                                                const union queue *q) {
...
static int queue_list_add(struct pmd_core_cfg *cfg, const union queue *q) {
...
static struct queue_list_entry *queue_list_take(struct pmd_core_cfg *cfg,
                                                const union queue *q) {
...
static inline int get_monitor_addresses(struct pmd_core_cfg *cfg,
...
static inline void queue_reset(struct pmd_core_cfg *cfg,
                               struct queue_list_entry *qcfg) {
...
static inline bool queue_can_sleep(struct pmd_core_cfg *cfg,
                                   struct queue_list_entry *qcfg) {
...
static inline bool lcore_can_sleep(struct pmd_core_cfg *cfg) {
...
static uint16_t clb_multiwait(uint16_t port_id __rte_unused,
  ...
  if (likely(!empty))
    ...
    ret = get_monitor_addresses(lcore_conf, pmc, lcore_conf->n_queues);
    if (ret < 0)
      return nb_rx;
...
static uint16_t clb_umwait(uint16_t port_id, uint16_t qidx,
...
static uint16_t clb_pause(uint16_t port_id __rte_unused,
                          uint16_t qidx __rte_unused,
                          struct rte_mbuf **pkts __rte_unused, uint16_t nb_rx,
                          uint16_t max_pkts __rte_unused, void *arg) {
  const unsigned int lcore = rte_lcore_id();
  ...
  if (likely(!empty))
    ...
    if (global_data.intrinsics_support.power_pause) {
      ...
      const uint64_t wait_tsc = cur + global_data.tsc_per_us;
...
static uint16_t clb_scale_freq(uint16_t port_id __rte_unused,
                               uint16_t qidx __rte_unused,
                               struct rte_mbuf **pkts __rte_unused,
                               uint16_t nb_rx, uint16_t max_pkts __rte_unused,
                               void *arg) {
...
static int queue_stopped(const uint16_t port_id, const uint16_t queue_id) {
...
static int cfg_queues_stopped(struct pmd_core_cfg *queue_cfg) {
...
static int check_scale(unsigned int lcore) {
...
static int check_monitor(struct pmd_core_cfg *cfg, const union queue *qdata) {
  ...
  multimonitor_supported = global_data.intrinsics_support.power_monitor_multi;
  ...
  if (rte_eth_get_monitor_addr(qdata->portid, qdata->qid, &dummy) == -ENOTSUP) {
    RTE_LOG(DEBUG, POWER,
            "The device does not support rte_eth_get_monitor_addr\n");
    return -ENOTSUP;
  }
...
static inline rte_rx_callback_fn get_monitor_callback(void) {
  return global_data.intrinsics_support.power_monitor_multi ? clb_multiwait
                                                            : clb_umwait;
}

int rte_power_ethdev_pmgmt_queue_enable(unsigned int lcore_id, uint16_t port_id,
                                        uint16_t queue_id,
  ...
  queue_cfg->cb = rte_eth_add_rx_callback(port_id, queue_id, clb, queue_cfg);
