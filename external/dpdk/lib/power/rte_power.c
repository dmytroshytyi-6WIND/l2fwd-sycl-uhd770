...
static void reset_power_function_ptrs(void) {
...
enum power_management_env rte_power_get_env(void) { return global_default_env; }
...
int rte_power_init(unsigned int lcore_id) {
  ...
  RTE_LOG(ERR, POWER,
          "Unable to set Power Management Environment for lcore "
          "%u\n",
          lcore_id);
out:
  return ret;
