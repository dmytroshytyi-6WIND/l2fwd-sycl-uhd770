...
int cpufreq_check_scaling_driver(const char *driver_name) {
   ...
   */
  open_core_sysfs_file(&f, "r", POWER_SYSFILE_SCALING_DRIVER, lcore_id);
...
int power_set_governor(unsigned int lcore_id, const char *new_governor,
  ...
  open_core_sysfs_file(&f_governor, "rw+", POWER_SYSFILE_GOVERNOR, lcore_id);
  if (f_governor == NULL) {
  ...
  ret = 0;
  RTE_LOG(INFO, POWER,
          "Power management governor of lcore %u has been "
