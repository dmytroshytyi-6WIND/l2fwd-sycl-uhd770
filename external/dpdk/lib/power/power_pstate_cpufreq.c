...
enum power_state { POWER_IDLE = 0, POWER_ONGOING, POWER_USED, POWER_UNKNOWN };
...
static int32_t power_rdmsr(int msr, uint64_t *val, unsigned int lcore_id) {
  int fd, ret;
 ...
 */
static int power_init_for_setting_freq(struct pstate_power_info *pi) {
  ...
  open_core_sysfs_file(&f_min, "rw+", POWER_SYSFILE_MIN_FREQ, pi->lcore_id);
  if (f_min == NULL) {
    RTE_LOG(ERR, POWER, "failed to open %s\n", POWER_SYSFILE_MIN_FREQ);
    goto err;
  }

  open_core_sysfs_file(&f_max, "rw+", POWER_SYSFILE_MAX_FREQ, pi->lcore_id);
  if (f_max == NULL) {
...
static int set_freq_internal(struct pstate_power_info *pi, uint32_t idx) {
  uint32_t target_freq = 0;

  if (idx >= RTE_MAX_LCORE_FREQS || idx >= pi->nb_freqs) {
    RTE_LOG(ERR, POWER,
            "Invalid frequency index %u, which "
  ...
  if (fseek(pi->f_cur_min, 0, SEEK_SET) < 0) {
    RTE_LOG(ERR, POWER,
            "Fail to set file position indicator to 0 "
  ...
  if (idx > pi->curr_idx) {
    ...
    if (fprintf(pi->f_cur_min, "%u", target_freq) < 0) {
      RTE_LOG(ERR, POWER,
              "Fail to write new frequency for "
              "lcore %u\n",
              pi->lcore_id);
      return -1;
    }
 ...
 */
static int power_set_governor_performance(struct pstate_power_info *pi) {
  return power_set_governor(pi->lcore_id, POWER_GOVERNOR_PERF, pi->governor_ori,
 ...
 */
static int power_set_governor_original(struct pstate_power_info *pi) {
 ...
 * sys file.
 */
static int power_get_available_freqs(struct pstate_power_info *pi) {
  ...
  /* open all files */
  open_core_sysfs_file(&f_max, "r", POWER_SYSFILE_BASE_MAX_FREQ, pi->lcore_id);
  if (f_max == NULL) {
    RTE_LOG(ERR, POWER, "failed to open %s\n", POWER_SYSFILE_BASE_MAX_FREQ);
    goto out;
  }

  open_core_sysfs_file(&f_min, "r", POWER_SYSFILE_BASE_MIN_FREQ, pi->lcore_id);
  if (f_min == NULL) {
   ...
   */
  num_freqs =
      (base_max_freq - sys_min_freq) / BUS_FREQ + 1 + pi->turbo_available;
...
static int power_get_cur_idx(struct pstate_power_info *pi) {
  FILE *f_cur;
  int ret = -1;
  uint32_t sys_cur_freq = 0;
  unsigned int i;

  open_core_sysfs_file(&f_cur, "r", POWER_SYSFILE_CUR_FREQ, pi->lcore_id);
  if (f_cur == NULL) {
  ...
  freq_conv = (sys_cur_freq + FREQ_ROUNDING_DELTA) / ROUND_FREQ_TO_N_100000;
...
int power_pstate_cpufreq_init(unsigned int lcore_id) {
   ...
   */
  if (!__atomic_compare_exchange_n(&(pi->state), &exp_state, POWER_ONGOING, 0,
                                   __ATOMIC_ACQUIRE, __ATOMIC_RELAXED)) {
    RTE_LOG(INFO, POWER,
            "Power management of lcore %u is "
            "in use\n",
            lcore_id);
    return -1;
  }
  ...
  if (power_set_governor_performance(pi) < 0) {
    RTE_LOG(ERR, POWER,
            "Cannot set governor of lcore %u to "
            "performance\n",
            lcore_id);
    goto fail;
  }
  ...
  if (power_init_for_setting_freq(pi) < 0) {
    RTE_LOG(ERR, POWER,
            "Cannot init for setting frequency for "
            "lcore %u\n",
            lcore_id);
    goto fail;
  }
  ...
  if (power_get_available_freqs(pi) < 0) {
    RTE_LOG(ERR, POWER,
            "Cannot get available frequencies of "
            "lcore %u\n",
            lcore_id);
    goto fail;
  }
  ...
  if (power_pstate_cpufreq_freq_max(lcore_id) < 0) {
    RTE_LOG(ERR, POWER,
            "Cannot set frequency of lcore %u "
            "to max\n",
            lcore_id);
    goto fail;
  }
  ...
  RTE_LOG(INFO, POWER,
          "Initialized successfully for lcore %u "
          "power management\n",
          lcore_id);
  exp_state = POWER_ONGOING;
  __atomic_compare_exchange_n(&(pi->state), &exp_state, POWER_USED, 0,
...
fail:
  exp_state = POWER_ONGOING;
  __atomic_compare_exchange_n(&(pi->state), &exp_state, POWER_UNKNOWN, 0,
                              __ATOMIC_RELEASE, __ATOMIC_RELAXED);
...
int power_pstate_cpufreq_exit(unsigned int lcore_id) {
  ...
  if (power_set_governor_original(pi) < 0) {
    RTE_LOG(ERR, POWER,
            "Cannot set the governor of %u back "
            "to the original\n",
            lcore_id);
    goto fail;
  }
  ...
  RTE_LOG(INFO, POWER,
          "Power management of lcore %u has exited from "
          "'performance' mode and been set back to the "
          "original\n",
          lcore_id);
  exp_state = POWER_ONGOING;
  __atomic_compare_exchange_n(&(pi->state), &exp_state, POWER_IDLE, 0,
                              __ATOMIC_RELEASE, __ATOMIC_RELAXED);
...
int power_pstate_cpufreq_freq_max(unsigned int lcore_id) {
  ...
  if (lcore_power_info[lcore_id].turbo_available) {
    if (lcore_power_info[lcore_id].turbo_enable)
      ...
      return set_freq_internal(&lcore_power_info[lcore_id], 1);
  } else
    return set_freq_internal(&lcore_power_info[lcore_id], 0);
}
...
int power_pstate_enable_turbo(unsigned int lcore_id) {
  ...
  if (pi->turbo_available)
    ...
    pi->turbo_enable = 0;
    RTE_LOG(ERR, POWER, "Failed to enable turbo on lcore %u\n", lcore_id);
    return -1;
  }
