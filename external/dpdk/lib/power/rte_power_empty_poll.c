...
static uint32_t get_freq_index(enum freq_val index) {
  return freq_index[index];
}

static int set_power_freq(int lcore_id, enum freq_val freq,
                          bool specific_freq) {
  int err = 0;
...
static __rte_always_inline void set_state(struct priority_worker *poll_stats,
...
static __rte_always_inline void set_policy(struct priority_worker *poll_stats,
...
static void update_training_stats(struct priority_worker *poll_stats,
                                  uint32_t freq, bool specific_freq,
  ...
  if (poll_stats->cur_freq == freq &&
    ...
    if (poll_stats->thresh[freq].cur_train_iter == 0) {
      ...
      set_power_freq(poll_stats->lcore_id, freq, specific_freq);

      poll_stats->empty_dequeues_prev = poll_stats->empty_dequeues;

      poll_stats->thresh[freq].cur_train_iter++;

      return;
    } else if (poll_stats->thresh[freq].cur_train_iter <= max_train_iter) {

      p0_empty_deq =
          poll_stats->empty_dequeues - poll_stats->empty_dequeues_prev;
      ...
      if (poll_stats->thresh[freq].trained == false) {
        poll_stats->thresh[freq].base_edpi =
            poll_stats->thresh[freq].base_edpi / max_train_iter;
...
update_stats(struct priority_worker *poll_stats) {
  ...
  percent =
      100 -
      (uint32_t)(((float)tot_edpi / (float)s->thresh[s->cur_freq].base_edpi) *
                 100);
...
update_stats_normal(struct priority_worker *poll_stats) {
  ...
  if (poll_stats->cur_freq == LOW)
    ...
    if (percent > poll_stats->thresh[MED].threshold_percent) {
    ...
    if (percent < poll_stats->thresh[HGH].threshold_percent) {
...
static int empty_poll_training(struct priority_worker *poll_stats,
                               uint32_t max_train_iter) {
  ...
  update_training_stats(poll_stats, LOW, false, max_train_iter);

  update_training_stats(poll_stats, MED, false, max_train_iter);

  update_training_stats(poll_stats, HGH, false, max_train_iter);
...
void rte_empty_poll_detection(struct rte_timer *tim, void *arg) {
  ...
  for (i = 0; i < NUM_NODES; i++) {
    ...
    case (TRAINING):
      empty_poll_training(poll_stats, ep_params->max_train_iter);
      break;
...
int rte_power_empty_poll_stat_init(struct ep_params **eptr, uint8_t *freq_tlb,
                                   struct ep_policy *policy) {
  uint32_t i;
  /* Allocate the ep_params structure */
  ep_params =
      rte_zmalloc_socket(NULL, sizeof(struct ep_params), 0, rte_socket_id());
  ...
  *eptr = ep_params;
  ...
  for (i = 0; i < NUM_NODES; i++) {
    ...
    total_avail_freqs[i] = rte_power_freqs(i, avail_freqs[i], NUM_FREQS);

    RTE_LOG(INFO, POWER, "total avail freq is %d , lcoreid %d\n",
            total_avail_freqs[i], i);
