...
#define RTE_DMA_LOG(level, ...)                                                \
  rte_log(RTE_LOG_##level, rte_dma_logtype,                                    \
          RTE_FMT("dma: " RTE_FMT_HEAD(__VA_ARGS__, ) "\n",                    \
...
static int dma_check_name(const char *name) {
  size_t name_len;
...
static int16_t dma_find_free_id(void) {
  int16_t i;
...
static struct rte_dma_dev *dma_find_by_name(const char *name) {
  int16_t i;
...
static int dma_fp_data_prepare(void) {
  size_t size;
  void *ptr;
  int i;
   ...
   */
  size =
      dma_devices_max * sizeof(struct rte_dma_fp_object) + RTE_CACHE_LINE_SIZE;
  ptr = malloc(size);
  if (ptr == NULL)
    return -ENOMEM;
  memset(ptr, 0, size);
...
static int dma_dev_data_prepare(void) {
  size_t size;
...
static int dma_shared_data_prepare(void) {
...
static int dma_data_prepare(void) {
  int ret;
...
static struct rte_dma_dev *dma_allocate_primary(const char *name, int numa_node,
  ...
  dev_private = rte_zmalloc_socket(name, private_data_size, RTE_CACHE_LINE_SIZE,
                                   numa_node);
  if (dev_private == NULL) {
...
static struct rte_dma_dev *dma_attach_secondary(const char *name) {
  struct rte_dma_dev *dev;
  int16_t i;
  int ret;
  ...
  if (i == dma_devices_max) {
    RTE_DMA_LOG(ERR, "Device %s is not driven by the primary process", name);
    return NULL;
  }
...
static struct rte_dma_dev *dma_allocate(const char *name, int numa_node,
                                        size_t private_data_size) {
  struct rte_dma_dev *dev;
...
static void dma_release(struct rte_dma_dev *dev) {
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
...
int rte_dma_info_get(int16_t dev_id, struct rte_dma_info *dev_info) {
  ...
  ret =
      (*dev->dev_ops->dev_info_get)(dev, dev_info, sizeof(struct rte_dma_info));
  if (ret != 0)
    return ret;
...
int rte_dma_configure(int16_t dev_id, const struct rte_dma_conf *dev_conf) {
  ...
  if (dev->data->dev_started != 0) {
    RTE_DMA_LOG(ERR, "Device %d must be stopped to allow configuration",
                dev_id);
    return -EBUSY;
  }
  ...
  if (dev_conf->nb_vchans == 0) {
    RTE_DMA_LOG(ERR, "Device %d configure zero vchans", dev_id);
    return -EINVAL;
  }
  if (dev_conf->nb_vchans > dev_info.max_vchans) {
    RTE_DMA_LOG(ERR, "Device %d configure too many vchans", dev_id);
    return -EINVAL;
  }
  if (dev_conf->enable_silent && !(dev_info.dev_capa & RTE_DMA_CAPA_SILENT)) {
  ...
  if (ret == 0)
    memcpy(&dev->data->dev_conf, dev_conf, sizeof(struct rte_dma_conf));
...
int rte_dma_close(int16_t dev_id) {
  ...
  if (dev->data->dev_started == 1) {
    RTE_DMA_LOG(ERR, "Device %d must be stopped before closing", dev_id);
    return -EBUSY;
  }
...
int rte_dma_vchan_setup(int16_t dev_id, uint16_t vchan,
  ...
  if (conf->direction == RTE_DMA_DIR_MEM_TO_MEM &&
    ...
    RTE_DMA_LOG(ERR, "Device %d don't support mem2mem transfer", dev_id);
    return -EINVAL;
  }
  if (conf->direction == RTE_DMA_DIR_MEM_TO_DEV &&
      !(dev_info.dev_capa & RTE_DMA_CAPA_MEM_TO_DEV)) {
    RTE_DMA_LOG(ERR, "Device %d don't support mem2dev transfer", dev_id);
    return -EINVAL;
  }
  if (conf->direction == RTE_DMA_DIR_DEV_TO_MEM &&
      !(dev_info.dev_capa & RTE_DMA_CAPA_DEV_TO_MEM)) {
    RTE_DMA_LOG(ERR, "Device %d don't support dev2mem transfer", dev_id);
    return -EINVAL;
  }
  if (conf->direction == RTE_DMA_DIR_DEV_TO_DEV &&
      !(dev_info.dev_capa & RTE_DMA_CAPA_DEV_TO_DEV)) {
    RTE_DMA_LOG(ERR, "Device %d don't support dev2dev transfer", dev_id);
    return -EINVAL;
  }
  if (conf->nb_desc < dev_info.min_desc || conf->nb_desc > dev_info.max_desc) {
    RTE_DMA_LOG(ERR, "Device %d number of descriptors invalid", dev_id);
    return -EINVAL;
  }
  ...
  if ((conf->dst_port.port_type == RTE_DMA_PORT_NONE && dst_is_dev) ||
    ...
    RTE_DMA_LOG(ERR, "Device %d destination port type invalid", dev_id);
    return -EINVAL;
  }
...
int rte_dma_stats_get(int16_t dev_id, uint16_t vchan,
  ...
  if (vchan >= dev->data->dev_conf.nb_vchans && vchan != RTE_DMA_ALL_VCHAN) {
    RTE_DMA_LOG(ERR, "Device %d vchan %u out of range", dev_id, vchan);
    return -EINVAL;
  }
...
int rte_dma_vchan_status(int16_t dev_id, uint16_t vchan,
...
static const char *dma_capability_name(uint64_t capability) {
  static const struct {
    uint64_t capability;
    const char *name;
  } capa_names[] = {
...
static void dma_dump_capability(FILE *f, uint64_t dev_capa) {
  uint64_t capa;
...
static int dummy_copy(__rte_unused void *dev_private,
                      __rte_unused uint16_t vchan, __rte_unused rte_iova_t src,
                      __rte_unused rte_iova_t dst, __rte_unused uint32_t length,
                      __rte_unused uint64_t flags) {
  RTE_DMA_LOG(ERR, "copy is not configured or not supported.");
  return -EINVAL;
}

static int dummy_copy_sg(__rte_unused void *dev_private,
                         __rte_unused uint16_t vchan,
                         __rte_unused const struct rte_dma_sge *src,
                         __rte_unused const struct rte_dma_sge *dst,
                         __rte_unused uint16_t nb_src,
                         __rte_unused uint16_t nb_dst,
                         __rte_unused uint64_t flags) {
  RTE_DMA_LOG(ERR, "copy_sg is not configured or not supported.");
  return -EINVAL;
}

static int dummy_fill(__rte_unused void *dev_private,
                      __rte_unused uint16_t vchan,
                      __rte_unused uint64_t pattern,
                      __rte_unused rte_iova_t dst, __rte_unused uint32_t length,
                      __rte_unused uint64_t flags) {
...
static int dummy_submit(__rte_unused void *dev_private,
                        __rte_unused uint16_t vchan) {
  RTE_DMA_LOG(ERR, "submit is not configured or not supported.");
  return -EINVAL;
}

static uint16_t dummy_completed(__rte_unused void *dev_private,
                                __rte_unused uint16_t vchan,
                                __rte_unused const uint16_t nb_cpls,
                                __rte_unused uint16_t *last_idx,
                                __rte_unused bool *has_error) {
  RTE_DMA_LOG(ERR, "completed is not configured or not supported.");
  return 0;
}

static uint16_t dummy_completed_status(
    __rte_unused void *dev_private, __rte_unused uint16_t vchan,
    __rte_unused const uint16_t nb_cpls, __rte_unused uint16_t *last_idx,
    __rte_unused enum rte_dma_status_code *status) {
  RTE_DMA_LOG(ERR, "completed_status is not configured or not supported.");
  return 0;
}

static uint16_t dummy_burst_capacity(__rte_unused const void *dev_private,
                                     __rte_unused uint16_t vchan) {
