...
#define RTE_DMA_FOREACH_DEV(p)                                                 \
  for (p = rte_dma_next_dev(0); p != -1; p = rte_dma_next_dev(p + 1))
 ...
 * failures to the user but other operations can continue. Without this bit set,
 * invalid data is not handled by either HW or driver, so user must ensure that
 * all memory addresses are valid and accessible by HW.
 */
 ...
 *   0 on success. Otherwise negative value is returned.
 */
__rte_experimental int rte_dma_stats_get(int16_t dev_id, uint16_t vchan,
 ...
 *
 * Enum with the options for the channel status, either idle, active or halted
 * due to error
...
enum rte_dma_vchan_status {
  ...
  RTE_DMA_VCHAN_HALTED_ERROR, /**< not processing due to error, cannot accept
                                 new ops */
};
 ...
 * to check if all jobs are completed, without actually gathering completions
 * from those jobs.
 *
 * @param dev_id
 *   The identifier of the device.
 ...
 */
__rte_experimental int rte_dma_vchan_status(int16_t dev_id, uint16_t vchan,
...
#ifdef RTE_DMADEV_DEBUG
  if (!rte_dma_is_valid(dev_id) || src == NULL || dst == NULL || nb_src == 0 ||
      nb_dst == 0)
    return -EINVAL;
  RTE_FUNC_PTR_OR_ERR_RET(*obj->copy_sg, -ENOTSUP);
#endif

  return (*obj->copy_sg)(obj->dev_private, vchan, src, dst, nb_src, nb_dst,
                         flags);
}
 ...
 */
__rte_experimental static inline int
rte_dma_fill(int16_t dev_id, uint16_t vchan, uint64_t pattern, rte_iova_t dst,
             uint32_t length, uint64_t flags) {
  struct rte_dma_fp_object *obj = &rte_dma_fp_objs[dev_id];
  ...
  return (*obj->fill)(obj->dev_private, vchan, pattern, dst, length, flags);
}
 ...
 *   0 on success. Otherwise negative value is returned.
 */
__rte_experimental static inline int rte_dma_submit(int16_t dev_id,
                                                    uint16_t vchan) {
  struct rte_dma_fp_object *obj = &rte_dma_fp_objs[dev_id];
 ...
 */
__rte_experimental static inline uint16_t
rte_dma_completed_status(int16_t dev_id, uint16_t vchan, const uint16_t nb_cpls,
  ...
  return (*obj->completed_status)(obj->dev_private, vchan, nb_cpls, last_idx,
                                  status);
}
