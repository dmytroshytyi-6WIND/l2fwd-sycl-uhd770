...
__rte_stack_lf_count(struct rte_stack *s) {
   ...
   */
  return (unsigned int)__atomic_load_n(&s->stack_lf.used.len, __ATOMIC_RELAXED);
}

static __rte_always_inline void
__rte_stack_lf_push_elems(struct rte_stack_lf_list *list,
                          struct rte_stack_lf_elem *first,
                          struct rte_stack_lf_elem *last, unsigned int num) {
  ...
  do {
    ...
    success = rte_atomic128_cmp_exchange(
        (rte_int128_t *)&list->head, (rte_int128_t *)&old_head,
        (rte_int128_t *)&new_head, 1, __ATOMIC_RELEASE, __ATOMIC_RELAXED);
  } while (success == 0);
...
__rte_stack_lf_pop_elems(struct rte_stack_lf_list *list, unsigned int num,
                         void **obj_table, struct rte_stack_lf_elem **last) {
  struct rte_stack_lf_head old_head;
  uint64_t len;
  int success;
  ...
  while (1) {
    ...
    if (__atomic_compare_exchange_n(&list->len, &len, len - num, 1,
  ...
  do {
    ...
    success = rte_atomic128_cmp_exchange(
        (rte_int128_t *)&list->head, (rte_int128_t *)&old_head,
        (rte_int128_t *)&new_head, 0, __ATOMIC_RELAXED, __ATOMIC_RELAXED);
  } while (success == 0);
