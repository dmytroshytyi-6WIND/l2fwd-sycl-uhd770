...
__rte_stack_lf_push(struct rte_stack *s, void *const *obj_table,
                    unsigned int n) {
...
__rte_stack_lf_pop(struct rte_stack *s, void **obj_table, unsigned int n) {
  ...
  /* Pop n used elements */
  first = __rte_stack_lf_pop_elems(&s->stack_lf.used, n, obj_table, &last);
  if (unlikely(first == NULL))
    return 0;
 ...
 */
void rte_stack_lf_init(struct rte_stack *s, unsigned int count);
 ...
 */
ssize_t rte_stack_lf_get_memsize(unsigned int count);
