...
static void rte_stack_init(struct rte_stack *s, unsigned int count,
                           uint32_t flags) {
  memset(s, 0, sizeof(*s));
...
static ssize_t rte_stack_get_memsize(unsigned int count, uint32_t flags) {
  if (flags & RTE_STACK_F_LF)
...
struct rte_stack *rte_stack_create(const char *name, unsigned int count,
  ...
  ret = snprintf(mz_name, sizeof(mz_name), "%s%s", RTE_STACK_MZ_PREFIX, name);
  ...
  mz = rte_memzone_reserve_aligned(mz_name, sz, socket_id, 0, __alignof__(*s));
  if (mz == NULL) {
