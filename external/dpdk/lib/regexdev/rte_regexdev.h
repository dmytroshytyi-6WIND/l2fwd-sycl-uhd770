...
#define RTE_REGEXDEV_VALID_DEV_ID_OR_ERR_RET(dev_id, retval)                   \
  do {                                                                         \
...
#define RTE_REGEXDEV_VALID_DEV_ID_OR_RET(dev_id)                               \
  do {                                                                         \
    if (!rte_regexdev_is_valid_dev(dev_id)) {                                  \
      RTE_REGEXDEV_LOG(ERR, "Invalid dev_id=%u\n", dev_id);                    \
      return;                                                                  \
 ...
 *   rte_regexdev_rule_db_compile_activate()
 */
__rte_experimental int rte_regexdev_rule_db_update(
    uint8_t dev_id, const struct rte_regexdev_rule *rules, uint32_t nb_rules);
 ...
 */
__rte_experimental int
rte_regexdev_xstats_reset(uint8_t dev_id, const uint16_t *ids, uint16_t nb_ids);
