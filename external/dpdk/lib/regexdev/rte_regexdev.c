...
static uint16_t regexdev_find_free_dev(void) {
  uint16_t i;
...
static struct rte_regexdev *regexdev_allocated(const char *name) {
  uint16_t i;
...
static int regexdev_shared_data_prepare(void) {
  ...
  if (rte_regexdev_shared_data == NULL) {
    ...
    mz = rte_memzone_reserve(MZ_RTE_REGEXDEV_DATA,
                             sizeof(*rte_regexdev_shared_data), rte_socket_id(),
                             flags);
    if (mz == NULL)
      return -ENOMEM;
...
static int regexdev_check_name(const char *name) {
  size_t name_len;
...
struct rte_regexdev *rte_regexdev_register(const char *name) {
  ...
  if (dev_id == RTE_MAX_REGEXDEV_DEVS) {
    RTE_REGEXDEV_LOG(ERR, "Reached maximum number of RegEx devices\n");
    return NULL;
  }
...
uint8_t rte_regexdev_count(void) {
  int i;
  int count = 0;
...
int rte_regexdev_get_dev_id(const char *name) {
  int i;
  int id = -EINVAL;
...
static int regexdev_info_get(uint8_t dev_id,
...
int rte_regexdev_info_get(uint8_t dev_id, struct rte_regexdev_info *dev_info) {
...
int rte_regexdev_configure(uint8_t dev_id,
  ...
  if (dev->data->dev_started) {
    RTE_REGEXDEV_LOG(ERR, "Dev %u must be stopped to allow configuration\n",
                     dev_id);
    return -EBUSY;
  }
  ...
  if (cfg->nb_queue_pairs > dev_info.max_queue_pairs) {
    RTE_REGEXDEV_LOG(ERR, "Dev %u num of queues %d > %d\n", dev_id,
                     cfg->nb_queue_pairs, dev_info.max_queue_pairs);
    return -EINVAL;
  }
  if (cfg->nb_rules_per_group == 0) {
    RTE_REGEXDEV_LOG(ERR, "Dev %u num of rules per group must be > 0\n",
                     dev_id);
    return -EINVAL;
  }
  if (cfg->nb_rules_per_group > dev_info.max_rules_per_group) {
    RTE_REGEXDEV_LOG(ERR, "Dev %u num of rules per group %d > %d\n", dev_id,
...
int rte_regexdev_queue_pair_setup(uint8_t dev_id, uint16_t queue_pair_id,
  ...
  if (queue_pair_id >= dev->data->dev_conf.nb_queue_pairs) {
    RTE_REGEXDEV_LOG(ERR, "Dev %u invalid queue %d > %d\n", dev_id,
                     queue_pair_id, dev->data->dev_conf.nb_queue_pairs);
    return -EINVAL;
  }
  if (dev->data->dev_started) {
    RTE_REGEXDEV_LOG(ERR, "Dev %u must be stopped to allow configuration\n",
...
int rte_regexdev_start(uint8_t dev_id) {
  struct rte_regexdev *dev;
  int ret;
...
int rte_regexdev_stop(uint8_t dev_id) {
  struct rte_regexdev *dev;
...
int rte_regexdev_close(uint8_t dev_id) {
  struct rte_regexdev *dev;
...
int rte_regexdev_attr_get(uint8_t dev_id, enum rte_regexdev_attr_id attr_id,
                          void *attr_value) {
  struct rte_regexdev *dev;
...
int rte_regexdev_attr_set(uint8_t dev_id, enum rte_regexdev_attr_id attr_id,
...
int rte_regexdev_rule_db_update(uint8_t dev_id,
...
int rte_regexdev_rule_db_compile_activate(uint8_t dev_id) {
  struct rte_regexdev *dev;
...
int rte_regexdev_rule_db_import(uint8_t dev_id, const char *rule_db,
                                uint32_t rule_db_len) {
  struct rte_regexdev *dev;

  RTE_REGEXDEV_VALID_DEV_ID_OR_ERR_RET(dev_id, -EINVAL);
  dev = &rte_regex_devices[dev_id];
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->dev_db_import, -ENOTSUP);
  if (rule_db == NULL) {
    RTE_REGEXDEV_LOG(ERR, "Dev %d rules can't be NULL\n", dev_id);
    return -EINVAL;
  }
...
int rte_regexdev_rule_db_export(uint8_t dev_id, char *rule_db) {
  struct rte_regexdev *dev;

  RTE_REGEXDEV_VALID_DEV_ID_OR_ERR_RET(dev_id, -EINVAL);
  dev = &rte_regex_devices[dev_id];
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->dev_db_export, -ENOTSUP);
  return (*dev->dev_ops->dev_db_export)(dev, rule_db);
}

int rte_regexdev_xstats_names_get(uint8_t dev_id,
                                  struct rte_regexdev_xstats_map *xstats_map) {
  struct rte_regexdev *dev;

  RTE_REGEXDEV_VALID_DEV_ID_OR_ERR_RET(dev_id, -EINVAL);
  dev = &rte_regex_devices[dev_id];
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->dev_xstats_names_get, -ENOTSUP);
  if (xstats_map == NULL) {
...
int rte_regexdev_xstats_get(uint8_t dev_id, const uint16_t *ids,
...
int rte_regexdev_xstats_by_name_get(uint8_t dev_id, const char *name,
                                    uint16_t *id, uint64_t *value) {
  struct rte_regexdev *dev;

  RTE_REGEXDEV_VALID_DEV_ID_OR_ERR_RET(dev_id, -EINVAL);
  dev = &rte_regex_devices[dev_id];
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->dev_xstats_by_name_get, -ENOTSUP);
  if (name == NULL) {
...
int rte_regexdev_xstats_reset(uint8_t dev_id, const uint16_t *ids,
                              uint16_t nb_ids) {
  struct rte_regexdev *dev;
...
int rte_regexdev_selftest(uint8_t dev_id) {
  struct rte_regexdev *dev;
...
int rte_regexdev_dump(uint8_t dev_id, FILE *f) {
  struct rte_regexdev *dev;
