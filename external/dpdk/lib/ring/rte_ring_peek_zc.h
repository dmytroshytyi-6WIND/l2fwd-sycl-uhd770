...
__rte_ring_get_elem_addr(struct rte_ring *r, uint32_t head, uint32_t esize,
 ...
 */
static __rte_always_inline unsigned int __rte_ring_do_enqueue_zc_elem_start(
    struct rte_ring *r, unsigned int esize, uint32_t n,
    enum rte_ring_queue_behavior behavior, struct rte_ring_zc_data *zcd,
    unsigned int *free_space) {
  uint32_t free, head, next;

  switch (r->prod.sync_type) {
  case RTE_RING_SYNC_ST:
    n = __rte_ring_move_prod_head(r, RTE_RING_SYNC_ST, n, behavior, &head,
                                  &next, &free);
    break;
  ...
  __rte_ring_get_elem_addr(r, head, esize, n, &zcd->ptr1, &zcd->n1, &zcd->ptr2);
 ...
 */
static __rte_always_inline unsigned int rte_ring_enqueue_zc_bulk_elem_start(
    struct rte_ring *r, unsigned int esize, unsigned int n,
    struct rte_ring_zc_data *zcd, unsigned int *free_space) {
  return __rte_ring_do_enqueue_zc_elem_start(r, esize, n, RTE_RING_QUEUE_FIXED,
                                             zcd, free_space);
}
...
rte_ring_enqueue_zc_bulk_start(struct rte_ring *r, unsigned int n,
                               ...
                               unsigned int *free_space) {
  return rte_ring_enqueue_zc_bulk_elem_start(r, sizeof(uintptr_t), n, zcd,
                                             free_space);
}
 ...
 *   The number of objects that can be enqueued, either 0 or n
 */
static __rte_always_inline unsigned int rte_ring_enqueue_zc_burst_elem_start(
    struct rte_ring *r, unsigned int esize, unsigned int n,
    struct rte_ring_zc_data *zcd, unsigned int *free_space) {
  return __rte_ring_do_enqueue_zc_elem_start(
      r, esize, n, RTE_RING_QUEUE_VARIABLE, zcd, free_space);
}
...
rte_ring_enqueue_zc_burst_start(struct rte_ring *r, unsigned int n,
                                struct rte_ring_zc_data *zcd,
                                unsigned int *free_space) {
  return rte_ring_enqueue_zc_burst_elem_start(r, sizeof(uintptr_t), n, zcd,
                                              free_space);
}
 ...
 */
static __rte_always_inline void rte_ring_enqueue_zc_finish(struct rte_ring *r,
                                                           unsigned int n) {
 ...
 */
static __rte_always_inline unsigned int __rte_ring_do_dequeue_zc_elem_start(
    struct rte_ring *r, uint32_t esize, uint32_t n,
    enum rte_ring_queue_behavior behavior, struct rte_ring_zc_data *zcd,
  ...
  case RTE_RING_SYNC_ST:
    n = __rte_ring_move_cons_head(r, RTE_RING_SYNC_ST, n, behavior, &head,
                                  &next, &avail);
    break;
  case RTE_RING_SYNC_MT_HTS:
 ...
 */
static __rte_always_inline unsigned int rte_ring_dequeue_zc_bulk_elem_start(
    struct rte_ring *r, unsigned int esize, unsigned int n,
    struct rte_ring_zc_data *zcd, unsigned int *available) {
  return __rte_ring_do_dequeue_zc_elem_start(r, esize, n, RTE_RING_QUEUE_FIXED,
                                             zcd, available);
}
...
rte_ring_dequeue_zc_bulk_start(struct rte_ring *r, unsigned int n,
                               struct rte_ring_zc_data *zcd,
                               unsigned int *available) {
  return rte_ring_dequeue_zc_bulk_elem_start(r, sizeof(uintptr_t), n, zcd,
                                             available);
}
 ...
 *   The number of objects that can be dequeued, either 0 or n.
 */
static __rte_always_inline unsigned int rte_ring_dequeue_zc_burst_elem_start(
    struct rte_ring *r, unsigned int esize, unsigned int n,
    struct rte_ring_zc_data *zcd, unsigned int *available) {
  return __rte_ring_do_dequeue_zc_elem_start(
      r, esize, n, RTE_RING_QUEUE_VARIABLE, zcd, available);
}
...
rte_ring_dequeue_zc_burst_start(struct rte_ring *r, unsigned int n,
                                struct rte_ring_zc_data *zcd,
                                unsigned int *available) {
  return rte_ring_dequeue_zc_burst_elem_start(r, sizeof(uintptr_t), n, zcd,
                                              available);
}
 ...
 *   The number of objects to remove from the ring.
 */
static __rte_always_inline void rte_ring_dequeue_zc_finish(struct rte_ring *r,
                                                           unsigned int n) {
