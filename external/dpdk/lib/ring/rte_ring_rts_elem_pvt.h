...
__rte_ring_rts_update_tail(struct rte_ring_rts_headtail *ht) {
  ...
  } while (__atomic_compare_exchange_n(&ht->tail.raw, &ot.raw, nt.raw, 0,
                                       __ATOMIC_RELEASE,
                                       __ATOMIC_ACQUIRE) == 0);
}
 ...
 */
static __rte_always_inline uint32_t __rte_ring_rts_move_prod_head(
    struct rte_ring *r, uint32_t num, enum rte_ring_queue_behavior behavior,
  ...
  do {
    ...
    *free_entries = capacity + r->cons.tail - oh.val.pos;
     ...
     */
  } while (__atomic_compare_exchange_n(&r->rts_prod.head.raw, &oh.raw, nh.raw,
                                       0, __ATOMIC_ACQUIRE,
                                       __ATOMIC_ACQUIRE) == 0);
...
__rte_ring_rts_move_cons_head(struct rte_ring *r, uint32_t num,
  ...
  do {
    ...
    *entries = r->prod.tail - oh.val.pos;
     ...
     */
  } while (__atomic_compare_exchange_n(&r->rts_cons.head.raw, &oh.raw, nh.raw,
                                       0, __ATOMIC_ACQUIRE,
                                       __ATOMIC_ACQUIRE) == 0);
 ...
 */
static __rte_always_inline unsigned int __rte_ring_do_rts_enqueue_elem(
 ...
 *     If behavior == RTE_RING_QUEUE_FIXED, this will be 0 or n only.
 */
static __rte_always_inline unsigned int __rte_ring_do_rts_dequeue_elem(
