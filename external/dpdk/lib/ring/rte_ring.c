...
ssize_t rte_ring_get_memsize_elem(unsigned int esize, unsigned int count) {
  ...
  if ((!POWEROF2(count)) || (count > RTE_RING_SZ_MASK)) {
    RTE_LOG(ERR, RING,
            "Requested number of elements is invalid, must be power of 2, and "
            "not exceed %u\n",
            RTE_RING_SZ_MASK);
...
int rte_ring_init(struct rte_ring *r, const char *name, unsigned int count,
  ...
  RTE_BUILD_BUG_ON((offsetof(struct rte_ring, cons) & RTE_CACHE_LINE_MASK) !=
                   0);
  RTE_BUILD_BUG_ON((offsetof(struct rte_ring, prod) & RTE_CACHE_LINE_MASK) !=
                   0);
  ...
  if (flags & RING_F_EXACT_SZ) {
    ...
    if ((!POWEROF2(count)) || (count > RTE_RING_SZ_MASK)) {
      RTE_LOG(ERR, RING,
              "Requested size is invalid, must be power of 2, and not exceed "
              "the size limit %u\n",
              RTE_RING_SZ_MASK);
      return -EINVAL;
    }
...
struct rte_ring *rte_ring_create_elem(const char *name, unsigned int esize,
  ...
  mz = rte_memzone_reserve_aligned(mz_name, ring_size, socket_id, mz_flags,
                                   __alignof__(*r));
  if (mz != NULL) {
    r = mz->addr;
...
struct rte_ring *rte_ring_create(const char *name, unsigned int count,
  ...
  return rte_ring_create_elem(name, sizeof(void *), count, socket_id, flags);
}
