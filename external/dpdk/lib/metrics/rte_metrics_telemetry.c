...
#define METRICS_LOG_ERR(fmt, args...) METRICS_LOG(ERR, fmt, ##args)

#define METRICS_LOG_WARN(fmt, args...) METRICS_LOG(WARNING, fmt, ##args)

static int32_t rte_metrics_tel_reg_port_ethdev_to_metrics(uint16_t port_id) {
  ...
  if (num_xstats < 0) {
    METRICS_LOG_ERR("rte_eth_xstats_get(%u) failed: %d", port_id, num_xstats);
    return -EPERM;
  }
  ...
  if (rte_eth_xstats_get_names(port_id, eth_xstats_names, num_xstats) !=
      num_xstats) {
    METRICS_LOG_ERR("rte_eth_xstats_get_names(%u) len %d failed", port_id,
                    num_xstats);
    ret = -EPERM;
    goto free_xstats;
  }
  ...
  if (ret < 0)
    METRICS_LOG_ERR(
        "rte_metrics_reg_names failed - metrics may already be registered");
...
static int32_t rte_metrics_tel_update_metrics_ethdev(uint16_t port_id,
                                                     int reg_start_index) {
  int ret, num_xstats, i;
  ...
  if (ret < 0 || ret > num_xstats) {
    free(eth_xstats);
    METRICS_LOG_ERR("rte_eth_xstats_get(%u) len%i failed: %d", port_id,
                    num_xstats, ret);
    return -EPERM;
  }
...
static int32_t rte_metrics_tel_format_port(uint32_t pid, json_t *ports,
                                           uint32_t *metric_ids,
                                           int num_metric_ids) {
  ...
  if (rte_metrics_get_names(names, num_metrics) != num_metrics ||
      rte_metrics_get_values(pid, metrics, num_metrics) != num_metrics) {
  ...
  for (i = 0; i < num_metrics; i++) {
    ...
    json_t *stat = json_pack("{s,s,s,I}", "name", names[metrics[i].key].name,
                             "value", metrics[i].value);
    if (stat == NULL || json_array_append_new(stats, stat) < 0) {
      METRICS_LOG_ERR("Format stat with id: %u failed", metrics[i].key);
      goto fail;
    }
  }
...
int32_t rte_metrics_tel_encode_json_format(struct telemetry_encode_param *ep,
                                           char **json_buffer) {
  json_t *root, *ports;
  int ret, i;
  ...
  if (ep->type == PORT_STATS) {
    ...
    for (i = 0; i < ep->pp.num_port_ids; i++) {
      ret = rte_metrics_tel_format_port(ep->pp.port_ids[i], ports,
                                        &ep->pp.metric_ids[0],
    ...
    ret = rte_metrics_tel_format_port(RTE_METRICS_GLOBAL, ports, NULL, 0);
    if (ret < 0) {
  ...
  root = json_pack("{s,s,s,o}", "status_code", "Status OK: 200", "data", ports);
  if (root == NULL) {
...
int32_t rte_metrics_tel_get_ports_stats_json(struct telemetry_encode_param *ep,
                                             int *reg_index,
                                             char **json_buffer) {
  int ret, i;
  uint32_t port_id;
  ...
  for (i = 0; i < ep->pp.num_port_ids; i++) {
    ...
    ret = rte_metrics_tel_update_metrics_ethdev(port_id, reg_index[i]);
    if (ret < 0) {
...
int32_t rte_metrics_tel_get_port_stats_ids(struct telemetry_encode_param *ep) {
...
static int32_t rte_metrics_tel_stat_names_to_ids(const char *const *stat_names,
                                                 uint32_t *stat_ids,
                                                 int num_stat_names) {
...
int32_t rte_metrics_tel_extract_data(struct telemetry_encode_param *ep,
                                     json_t *data) {
  int ret;
...
static int rte_metrics_tel_initial_metrics_setup(void) {
  int ret;
  rte_metrics_init(rte_socket_id());
...
static int handle_ports_all_stats_values(const char *cmd __rte_unused,
...
static int handle_global_stats_values(const char *cmd __rte_unused,
                                      const char *params __rte_unused,
                                      char *buffer, int buf_len) {
  char *json_buffer = NULL;
...
static int handle_ports_stats_values_by_name(const char *cmd __rte_unused,
                                             const char *params, char *buffer,
                                             int buf_len) {
  char *json_buffer = NULL;
  struct telemetry_encode_param ep;
  int ret, used = 0;
...
int32_t rte_metrics_tel_get_global_stats(struct telemetry_encode_param *ep) {
  RTE_SET_USED(ep);
