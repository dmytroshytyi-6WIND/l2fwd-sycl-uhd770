...
void rte_metrics_init(int socket_id) {
  ...
  if (memzone != NULL)
    return;
  memzone =
      rte_memzone_reserve(RTE_METRICS_MEMZONE_NAME,
...
int rte_metrics_update_values(int port_id, uint16_t key, const uint64_t *values,
                              uint32_t count) {
  ...
  if (port_id == RTE_METRICS_GLOBAL)
    for (idx_value = 0; idx_value < count; idx_value++) {
      ...
      stats->metadata[idx_metric].global_value = values[idx_value];
    }
  else
    for (idx_value = 0; idx_value < count; idx_value++) {
      idx_metric = key + idx_value;
...
int rte_metrics_get_names(struct rte_metric_name *names, uint16_t capacity) {
  ...
  if (names != NULL) {
    ...
    for (idx_name = 0; idx_name < stats->cnt_stats; idx_name++)
      strlcpy(names[idx_name].name, stats->metadata[idx_name].name,
...
int rte_metrics_get_values(int port_id, struct rte_metric_value *values,
                           uint16_t capacity) {
  struct rte_metrics_meta_s *entry;
  struct rte_metrics_data_s *stats;
  const struct rte_memzone *memzone;
