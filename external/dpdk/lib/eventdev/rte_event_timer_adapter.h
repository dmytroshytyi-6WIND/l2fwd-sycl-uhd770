...
#define ADAPTER_VALID_OR_ERR_RET(adapter, retval)                              \
  do {                                                                         \
    if (adapter == NULL || !adapter->allocated)                                \
      return retval;                                                           \
  } while (0)

#define FUNC_PTR_OR_ERR_RET(func, errval)                                      \
  do {                                                                         \
    if ((func) == NULL)                                                        \
      return errval;                                                           \
  } while (0)

#define FUNC_PTR_OR_NULL_RET_WITH_ERRNO(func, errval)                          \
  do {                                                                         \
    if ((func) == NULL) {                                                      \
      rte_errno = errval;                                                      \
      return NULL;                                                             \
...
#endif
  rte_eventdev_trace_timer_arm_burst(adapter, (void **)evtims, nb_evtims);
 ...
 *   - EALREADY A timer was encountered that was already armed
 */
static inline uint16_t rte_event_timer_arm_tmo_tick_burst(
    const struct rte_event_timer_adapter *adapter,
    struct rte_event_timer **evtims, const uint64_t timeout_ticks,
  ...
  return adapter->arm_tmo_tick_burst(adapter, evtims, timeout_ticks, nb_evtims);
}
...
#endif
  rte_eventdev_trace_timer_cancel_burst(adapter, (void **)evtims, nb_evtims);
