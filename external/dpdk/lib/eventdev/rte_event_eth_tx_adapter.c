...
#define TXA_CHECK_TXQ(dev, queue)                                              \
  do {                                                                         \
    ...
    if ((queue) != -1 && (uint16_t)(queue) >= (dev)->data->nb_tx_queues) {     \
      RTE_EDEV_LOG_ERR("Invalid tx queue_id %" PRIu16, (uint16_t)(queue));     \
      return -EINVAL;                                                          \
    }                                                                          \
  } while (0)
...
txa_service_adapter_create_ext(uint8_t id, struct rte_eventdev *dev,
                               rte_event_eth_tx_adapter_conf_cb conf_cb,
                               void *conf_arg);
static int txa_service_queue_del(uint8_t id, const struct rte_eth_dev *dev,
                                 int32_t tx_queue_id);

static int txa_adapter_exist(uint8_t id) {
  return txa_dev_id_array[id] != TXA_INVALID_DEV_ID;
}

static inline int txa_valid_id(uint8_t id) {
  return id < RTE_EVENT_ETH_TX_ADAPTER_MAX_INSTANCE;
}

static void *txa_memzone_array_get(const char *name, unsigned int elt_size,
                                   int nb_elems) {
...
static int txa_dev_id_array_init(void) {
  if (txa_dev_id_array == NULL) {
    int i;

    txa_dev_id_array =
        txa_memzone_array_get("txa_adapter_array", sizeof(int),
...
static int txa_init(void) { return txa_dev_id_array_init(); }

static int txa_service_data_init(void) {
  if (txa_service_data_array == NULL) {
    txa_service_data_array =
        txa_memzone_array_get("txa_service_data_array", sizeof(int),
                              RTE_EVENT_ETH_TX_ADAPTER_MAX_INSTANCE);
    if (txa_service_data_array == NULL)
      return -ENOMEM;
  }
...
static inline struct txa_service_data *txa_service_id_to_data(uint8_t id) {
...
static int txa_service_conf_cb(uint8_t __rte_unused id, uint8_t dev_id,
...
static int txa_service_ethdev_alloc(struct txa_service_data *txa) {
  struct txa_service_ethdev *txa_ethdev;
  ...
  txa_ethdev = rte_zmalloc_socket(
      txa->mem_name, dev_count * sizeof(*txa_ethdev), 0, txa->socket_id);
  if (txa_ethdev == NULL) {
  ...
  if (txa->dev_count)
    memcpy(txa_ethdev, txa->txa_ethdev, txa->dev_count * sizeof(*txa_ethdev));
...
static int txa_service_queue_array_alloc(struct txa_service_data *txa,
                                         uint16_t port_id) {
  struct txa_service_queue_info *tqi;
  uint16_t nb_queue;
  int ret;
  ...
  tqi = rte_zmalloc_socket(txa->mem_name,
                           nb_queue * sizeof(struct txa_service_queue_info), 0,
                           txa->socket_id);
  if (tqi == NULL)
    return -ENOMEM;
...
static void txa_service_queue_array_free(struct txa_service_data *txa,
                                         uint16_t port_id) {
  struct txa_service_ethdev *txa_ethdev;
  struct txa_service_queue_info *tqi;
...
static void txa_service_unregister(struct txa_service_data *txa) {
...
static int txa_service_register(struct txa_service_data *txa) {
  int ret;
  ...
  ret = rte_service_component_register(&service, (uint32_t *)&txa->service_id);
  if (ret) {
...
txa_service_tx_buf_alloc(struct txa_service_data *txa,
  ...
  tb = rte_zmalloc_socket(txa->mem_name, RTE_ETH_TX_BUFFER_SIZE(TXA_BATCH_SIZE),
...
static int txa_service_is_queue_added(struct txa_service_data *txa,
                                      const struct rte_eth_dev *dev,
                                      uint16_t tx_queue_id) {
  struct txa_service_queue_info *tqi;
...
static int txa_service_ctrl(uint8_t id, int start) {
  int ret;
...
static void txa_service_buffer_retry(struct rte_mbuf **pkts, uint16_t unsent,
                                     void *userdata) {
  struct txa_retry *tr;
  ...
  do {
    n = rte_eth_tx_burst(tr->port_id, tr->tx_queue, &pkts[sent], unsent - sent);
...
static uint16_t txa_process_event_vector(struct txa_service_data *txa,
  ...
  if (vec->attr_valid) {
    ...
    for (i = 0; i < vec->nb_elem; i++) {
      nb_tx += rte_eth_tx_buffer(port, queue, tqi->tx_buf, mbufs[i]);
    }
  } else {
    for (i = 0; i < vec->nb_elem; i++) {
      port = mbufs[i]->port;
...
static void txa_service_tx(struct txa_service_data *txa, struct rte_event *ev,
                           uint32_t n) {
  uint32_t i;
  uint16_t nb_tx;
  struct rte_event_eth_tx_adapter_stats *stats;
  ...
  if ((txa->loop_cnt++ & (TXA_FLUSH_THRESHOLD - 1)) == 0) {
    ...
    RTE_ETH_FOREACH_DEV(i) {
      ...
      for (q = 0; q < dev->data->nb_tx_queues; q++) {
        ...
        nb_tx += rte_eth_tx_buffer_flush(i, q, tqi->tx_buf);
      }
    }
...
static int txa_service_adapter_create(uint8_t id, struct rte_eventdev *dev,
  ...
  *cb_conf = *port_conf;
  ret = txa_service_adapter_create_ext(id, dev, txa_service_conf_cb, cb_conf);
  if (ret) {
    rte_free(cb_conf);
    return ret;
  }
...
txa_service_adapter_create_ext(uint8_t id, struct rte_eventdev *dev,
  ...
  snprintf(mem_name, TXA_MEM_NAME_LEN, "rte_event_eth_txa_%d", id);

  ret = txa_service_data_init();
  if (ret != 0)
    return ret;

  txa = rte_zmalloc_socket(mem_name, sizeof(*txa), RTE_CACHE_LINE_SIZE,
                           socket_id);
  if (txa == NULL) {
...
static int txa_service_event_port_get(uint8_t id, uint8_t *port) {
  struct txa_service_data *txa;
...
static int txa_service_adapter_free(uint8_t id) {
  struct txa_service_data *txa;

  txa = txa_service_id_to_data(id);
  if (txa->nb_queues) {
    RTE_EDEV_LOG_ERR("%" PRIu16 " Tx queues not deleted", txa->nb_queues);
    return -EBUSY;
  }
...
static int txa_service_queue_add(uint8_t id,
  ...
  if (tx_queue_id == -1) {
    ...
    qdone = rte_zmalloc(txa->mem_name, nb_queues * sizeof(*qdone), 0);
    if (qdone == NULL)
      return -ENOMEM;
    j = 0;
  ...
  rte_eth_tx_buffer_set_err_callback(tb, txa_service_buffer_retry, txa_retry);
...
err_unlock:
  if (txa->nb_queues == 0) {
    txa_service_queue_array_free(txa, eth_dev->data->port_id);
    txa_service_unregister(txa);
  }
...
static int txa_service_id_get(uint8_t id, uint32_t *service_id) {
  struct txa_service_data *txa;
...
static int txa_service_start(uint8_t id) { return txa_service_ctrl(id, 1); }

static int txa_service_stats_get(uint8_t id,
                                 struct rte_event_eth_tx_adapter_stats *stats) {
  struct txa_service_data *txa;
...
static int txa_service_stats_reset(uint8_t id) {
  struct txa_service_data *txa;
...
static int txa_service_stop(uint8_t id) { return txa_service_ctrl(id, 0); }
...
int rte_event_eth_tx_adapter_create(uint8_t id, uint8_t dev_id,
  ...
  rte_eventdev_trace_eth_tx_adapter_create(id, dev_id, NULL, port_conf, ret);
  txa_dev_id_array[id] = dev_id;
  return 0;
}

int rte_event_eth_tx_adapter_create_ext(
    uint8_t id, uint8_t dev_id, rte_event_eth_tx_adapter_conf_cb conf_cb,
    void *conf_arg) {
  struct rte_eventdev *dev;
  int ret;
  ...
  rte_eventdev_trace_eth_tx_adapter_create(id, dev_id, conf_cb, conf_arg, ret);
  txa_dev_id_array[id] = dev_id;
  return 0;
}
...
int rte_event_eth_tx_adapter_free(uint8_t id) {
  ...
  ret = txa_dev_adapter_free(id) ? txa_dev_adapter_free(id)(id, txa_evdev(id))
                                 : 0;
...
int rte_event_eth_tx_adapter_queue_add(uint8_t id, uint16_t eth_dev_id,
                                       int32_t queue) {
  ...
  if (caps & RTE_EVENT_ETH_TX_ADAPTER_CAP_INTERNAL_PORT)
    ret = txa_dev_queue_add(id)
              ? txa_dev_queue_add(id)(id, txa_evdev(id), eth_dev, queue)
              : 0;
  else
    ret = txa_service_queue_add(id, txa_evdev(id), eth_dev, queue);

  rte_eventdev_trace_eth_tx_adapter_queue_add(id, eth_dev_id, queue, ret);
  return ret;
}

int rte_event_eth_tx_adapter_queue_del(uint8_t id, uint16_t eth_dev_id,
                                       int32_t queue) {
  struct rte_eth_dev *eth_dev;
  int ret;
  uint32_t caps;
  ...
  if (caps & RTE_EVENT_ETH_TX_ADAPTER_CAP_INTERNAL_PORT)
    ret = txa_dev_queue_del(id)
              ? txa_dev_queue_del(id)(id, txa_evdev(id), eth_dev, queue)
              : 0;
  else
    ret = txa_service_queue_del(id, eth_dev, queue);

  rte_eventdev_trace_eth_tx_adapter_queue_del(id, eth_dev_id, queue, ret);
  return ret;
}
...
int rte_event_eth_tx_adapter_stats_get(
    uint8_t id, struct rte_event_eth_tx_adapter_stats *stats) {
  int ret;
  ...
  *stats = (struct rte_event_eth_tx_adapter_stats){0};
  ...
  ret = txa_dev_stats_get(id) ? txa_dev_stats_get(id)(id, txa_evdev(id), stats)
                              : 0;
...
int rte_event_eth_tx_adapter_stats_reset(uint8_t id) {
  ...
  ret =
      txa_dev_stats_reset(id) ? txa_dev_stats_reset(id)(id, txa_evdev(id)) : 0;
  if (ret == 0)
