...
static struct rte_eventdev_global eventdev_globals = {.nb_devs = 0};
...
uint8_t rte_event_dev_count(void) { return eventdev_globals.nb_devs; }
...
int rte_event_dev_get_dev_id(const char *name) {
  ...
  for (i = 0; i < eventdev_globals.nb_devs; i++) {
    cmp = (strncmp(rte_event_devices[i].data->name, name,
          ...
          (rte_event_devices[i].dev
               ? (strncmp(rte_event_devices[i].dev->driver->name, name,
                          RTE_EVENTDEV_NAME_MAX_LEN) == 0)
               : 0);
    if (cmp && (rte_event_devices[i].attached == RTE_EVENTDEV_ATTACHED))
      return i;
  }
  return -ENODEV;
}
...
int rte_event_eth_rx_adapter_caps_get(uint8_t dev_id, uint16_t eth_port_id,
  ...
  return dev->dev_ops->eth_rx_adapter_caps_get
             ? (*dev->dev_ops->eth_rx_adapter_caps_get)(
                   dev, &rte_eth_devices[eth_port_id], caps)
             : 0;
}
...
int rte_event_crypto_adapter_caps_get(uint8_t dev_id, uint8_t cdev_id,
  ...
  return dev->dev_ops->crypto_adapter_caps_get
             ? (*dev->dev_ops->crypto_adapter_caps_get)(dev, cdev, caps)
             : 0;
}
...
static inline int event_dev_queue_config(struct rte_eventdev *dev,
                                         uint8_t nb_queues) {
  ...
  if (nb_queues != 0) {
    ...
    if (nb_queues > old_nb_queues) {
      ...
      memset(queues_cfg + old_nb_queues, 0, sizeof(queues_cfg[0]) * new_qs);
    }
  } else {
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->queue_release, -ENOTSUP);
...
static inline int event_dev_port_config(struct rte_eventdev *dev,
                                        uint8_t nb_ports) {
  ...
  if (nb_ports != 0) { /* re-config */
    ...
    if (nb_ports > old_nb_ports) {
      ...
      unsigned int links_map_end = nb_ports * RTE_EVENT_MAX_QUEUES_PER_DEV;

      memset(ports + old_nb_ports, 0, sizeof(ports[0]) * new_ps);
      memset(ports_cfg + old_nb_ports, 0, sizeof(ports_cfg[0]) * new_ps);
      for (i = old_links_map_end; i < links_map_end; i++)
        links_map[i] = EVENT_QUEUE_SERVICE_PRIORITY_INVALID;
    }
  } else {
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->port_release, -ENOTSUP);
...
int rte_event_dev_configure(uint8_t dev_id,
  ...
  if (dev->data->dev_started) {
    RTE_EDEV_LOG_ERR("device %d must be stopped to allow configuration",
                     dev_id);
    return -EBUSY;
  }
  ...
  if (!(dev_conf->event_dev_cfg & RTE_EVENT_DEV_CFG_PER_DEQUEUE_TIMEOUT)) {
    if (dev_conf->dequeue_timeout_ns &&
        (dev_conf->dequeue_timeout_ns < info.min_dequeue_timeout_ns ||
         dev_conf->dequeue_timeout_ns > info.max_dequeue_timeout_ns)) {
  ...
  if (dev_conf->nb_events_limit > info.max_num_events) {
    RTE_EDEV_LOG_ERR("dev%d nb_events_limit=%d > max_num_events=%d", dev_id,
  ...
  if (!dev_conf->nb_event_queues) {
    RTE_EDEV_LOG_ERR("dev%d nb_event_queues cannot be zero", dev_id);
    return -EINVAL;
  }
  if (dev_conf->nb_event_queues >
  ...
  if (dev_conf->nb_event_queues - dev_conf->nb_single_link_event_port_queues >
      info.max_event_queues) {
    RTE_EDEV_LOG_ERR(
        "id%d nb_event_queues=%d - nb_single_link_event_port_queues=%d > "
        "max_event_queues=%d",
        dev_id, dev_conf->nb_event_queues,
        dev_conf->nb_single_link_event_port_queues, info.max_event_queues);
    return -EINVAL;
  }
  if (dev_conf->nb_single_link_event_port_queues > dev_conf->nb_event_queues) {
    RTE_EDEV_LOG_ERR(
        "dev%d nb_single_link_event_port_queues=%d > nb_event_queues=%d",
        dev_id, dev_conf->nb_single_link_event_port_queues,
  ...
  if (dev_conf->nb_event_ports >
      info.max_event_ports + info.max_single_link_event_port_queue_pairs) {
  ...
  if (dev_conf->nb_event_ports - dev_conf->nb_single_link_event_port_queues >
  ...
  if (dev_conf->nb_single_link_event_port_queues > dev_conf->nb_event_ports) {
    RTE_EDEV_LOG_ERR(
        "dev%d nb_single_link_event_port_queues=%d > nb_event_ports=%d", dev_id,
        dev_conf->nb_single_link_event_port_queues, dev_conf->nb_event_ports);
    return -EINVAL;
  }
  ...
  if (dev_conf->nb_event_queue_flows > info.max_event_queue_flows) {
    RTE_EDEV_LOG_ERR("dev%d nb_flows=%x > max_flows=%x", dev_id,
  ...
  if (!dev_conf->nb_event_port_dequeue_depth) {
    RTE_EDEV_LOG_ERR("dev%d nb_dequeue_depth cannot be zero", dev_id);
    return -EINVAL;
  }
  if ((info.event_dev_cap & RTE_EVENT_DEV_CAP_BURST_MODE) &&
      (dev_conf->nb_event_port_dequeue_depth >
       info.max_event_port_dequeue_depth)) {
    RTE_EDEV_LOG_ERR("dev%d nb_dq_depth=%d > max_dq_depth=%d", dev_id,
                     dev_conf->nb_event_port_dequeue_depth,
                     info.max_event_port_dequeue_depth);
    return -EINVAL;
  }
  ...
  if (!dev_conf->nb_event_port_enqueue_depth) {
    RTE_EDEV_LOG_ERR("dev%d nb_enqueue_depth cannot be zero", dev_id);
    return -EINVAL;
  }
  if ((info.event_dev_cap & RTE_EVENT_DEV_CAP_BURST_MODE) &&
      (dev_conf->nb_event_port_enqueue_depth >
       info.max_event_port_enqueue_depth)) {
    RTE_EDEV_LOG_ERR("dev%d nb_enq_depth=%d > max_enq_depth=%d", dev_id,
                     dev_conf->nb_event_port_enqueue_depth,
                     info.max_event_port_enqueue_depth);
    return -EINVAL;
  }
  ...
  if (diag != 0) {
    RTE_EDEV_LOG_ERR("dev%d event_dev_queue_config = %d", dev_id, diag);
    return diag;
  }
  ...
  if (diag != 0) {
    ...
    RTE_EDEV_LOG_ERR("dev%d event_dev_port_config = %d", dev_id, diag);
    return diag;
  }
...
static inline int is_valid_queue(struct rte_eventdev *dev, uint8_t queue_id) {
  if (queue_id < dev->data->nb_queues &&
      queue_id < RTE_EVENT_MAX_QUEUES_PER_DEV)
    return 1;
  else
    return 0;
}
...
int rte_event_queue_setup(uint8_t dev_id, uint8_t queue_id,
  ...
  if (is_valid_atomic_queue_conf(queue_conf)) {
    if (queue_conf->nb_atomic_flows == 0 ||
      ...
      RTE_EDEV_LOG_ERR("dev%d queue%d Invalid nb_atomic_flows=%d max_flows=%d",
  ...
  if (is_valid_ordered_queue_conf(queue_conf)) {
    if (queue_conf->nb_atomic_order_sequences == 0 ||
        queue_conf->nb_atomic_order_sequences >
            dev->data->dev_conf.nb_event_queue_flows) {
      RTE_EDEV_LOG_ERR(
          "dev%d queue%d Invalid nb_atomic_order_seq=%d max_flows=%d", dev_id,
          queue_id, queue_conf->nb_atomic_order_sequences,
          dev->data->dev_conf.nb_event_queue_flows);
      return -EINVAL;
    }
  }
  ...
  if (dev->data->dev_started) {
    RTE_EDEV_LOG_ERR("device %d must be stopped to allow queue setup", dev_id);
    return -EBUSY;
  }
...
static inline int is_valid_port(struct rte_eventdev *dev, uint8_t port_id) {
...
int rte_event_port_setup(uint8_t dev_id, uint8_t port_id,
  ...
  if ((port_conf && !port_conf->new_event_threshold) ||
      (port_conf &&
       port_conf->new_event_threshold > dev->data->dev_conf.nb_events_limit)) {
    RTE_EDEV_LOG_ERR(
        "dev%d port%d Invalid event_threshold=%d nb_events_limit=%d", dev_id,
        port_id, port_conf->new_event_threshold,
  ...
  if ((port_conf && !port_conf->dequeue_depth) ||
    ...
    RTE_EDEV_LOG_ERR(
        "dev%d port%d Invalid dequeue depth=%d max_dequeue_depth=%d", dev_id,
        port_id, port_conf->dequeue_depth,
        dev->data->dev_conf.nb_event_port_dequeue_depth);
    return -EINVAL;
  }
  ...
  if ((port_conf && !port_conf->enqueue_depth) ||
    ...
    RTE_EDEV_LOG_ERR(
        "dev%d port%d Invalid enqueue depth=%d max_enqueue_depth=%d", dev_id,
        port_id, port_conf->enqueue_depth,
        dev->data->dev_conf.nb_event_port_enqueue_depth);
    return -EINVAL;
  }
  ...
  if (port_conf &&
    ...
    RTE_EDEV_LOG_ERR("dev%d port%d Implicit release disable not supported",
                     dev_id, port_id);
    return -EINVAL;
  }

  if (dev->data->dev_started) {
    RTE_EDEV_LOG_ERR("device %d must be stopped to allow port setup", dev_id);
    return -EBUSY;
  }
...
int rte_event_port_unlink(uint8_t dev_id, uint8_t port_id, uint8_t queues[],
  ...
  if (queues == NULL) {
    ...
    for (j = 0; j < nb_unlinks; j++) {
      if (links_map[queues[j]] == EVENT_QUEUE_SERVICE_PRIORITY_INVALID)
        break;
    }
  }
...
static int xstats_get_count(uint8_t dev_id, enum rte_event_dev_xstats_mode mode,
                            uint8_t queue_port_id) {
  struct rte_eventdev *dev = &rte_eventdevs[dev_id];
  if (dev->dev_ops->xstats_get_names != NULL)
    return (*dev->dev_ops->xstats_get_names)(dev, mode, queue_port_id, NULL,
                                             NULL, 0);
  return 0;
}

int rte_event_dev_xstats_names_get(
    uint8_t dev_id, enum rte_event_dev_xstats_mode mode, uint8_t queue_port_id,
    struct rte_event_dev_xstats_name *xstats_names, unsigned int *ids,
    unsigned int size) {
  RTE_EVENTDEV_VALID_DEVID_OR_ERR_RET(dev_id, -ENODEV);
  const int cnt_expected_entries =
      xstats_get_count(dev_id, mode, queue_port_id);
...
int rte_event_dev_xstats_get(uint8_t dev_id,
  ...
  if (dev->dev_ops->xstats_get != NULL)
    return (*dev->dev_ops->xstats_get)(dev, mode, queue_port_id, ids, values,
                                       n);
  return -ENOTSUP;
}
...
struct rte_mempool *rte_event_vector_pool_create(const char *name,
  ...
  if (!nb_elem) {
    RTE_LOG(ERR, EVENTDEV, "Invalid number of elements=%d requested\n",
            nb_elem);
    rte_errno = EINVAL;
    return NULL;
  }

  elt_sz = sizeof(struct rte_event_vector) + (nb_elem * sizeof(uintptr_t));
  mp = rte_mempool_create_empty(name, n, elt_sz, cache_size, 0, socket_id, 0);
  if (mp == NULL)
    return NULL;
...
int rte_event_dev_start(uint8_t dev_id) {
  ...
  if (dev->data->dev_started != 0) {
    RTE_EDEV_LOG_ERR("Device with dev_id=%" PRIu8 "already started", dev_id);
    return 0;
  }
...
void rte_event_dev_stop(uint8_t dev_id) {
  ...
  if (dev->data->dev_started == 0) {
    RTE_EDEV_LOG_ERR("Device with dev_id=%" PRIu8 "already stopped", dev_id);
    return;
  }
...
int rte_event_dev_close(uint8_t dev_id) {
  ...
  if (dev->data->dev_started == 1) {
    RTE_EDEV_LOG_ERR("Device %u must be stopped before closing", dev_id);
    return -EBUSY;
  }
...
static inline int eventdev_data_alloc(uint8_t dev_id,
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    mz = rte_memzone_reserve(mz_name, sizeof(struct rte_eventdev_data),
                             socket_id, 0);
  } else
  ...
  *data = mz->addr;
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    ...
    for (n = 0; n < RTE_EVENT_MAX_PORTS_PER_DEV * RTE_EVENT_MAX_QUEUES_PER_DEV;
         n++)
      (*data)->links_map[n] = EVENT_QUEUE_SERVICE_PRIORITY_INVALID;
  }

  return 0;
}

static inline uint8_t eventdev_find_free_device_index(void) {
  uint8_t dev_id;
...
void event_dev_probing_finish(struct rte_eventdev *eventdev) {
  if (eventdev == NULL)
    return;

  event_dev_fp_ops_set(rte_event_fp_ops + eventdev->data->dev_id, eventdev);
}

static int handle_dev_list(const char *cmd __rte_unused,
  ...
  for (dev_id = 0; dev_id < RTE_EVENT_MAX_DEVS; dev_id++) {
    if (rte_eventdevs[dev_id].attached == RTE_EVENTDEV_ATTACHED)
...
static int handle_port_list(const char *cmd __rte_unused, const char *params,
                            struct rte_tel_data *d) {
  int i;
  uint8_t dev_id;
  struct rte_eventdev *dev;
...
static int handle_queue_list(const char *cmd __rte_unused, const char *params,
                             struct rte_tel_data *d) {
  int i;
  uint8_t dev_id;
  struct rte_eventdev *dev;
...
static int handle_queue_links(const char *cmd __rte_unused, const char *params,
                              struct rte_tel_data *d) {
  int i, ret, port_id = 0;
  char *end_param;
  uint8_t dev_id;
...
static int eventdev_build_telemetry_data(int dev_id,
                                         enum rte_event_dev_xstats_mode mode,
                                         int port_queue_id,
                                         struct rte_tel_data *d) {
  ...
  num_xstats = rte_event_dev_xstats_names_get(dev_id, mode, port_queue_id, NULL,
                                              NULL, 0);
  ...
  xstat_names = malloc((sizeof(struct rte_event_dev_xstats_name)) * num_xstats);
  if (xstat_names == NULL)
    return -1;
  ...
  ret = rte_event_dev_xstats_names_get(dev_id, mode, port_queue_id, xstat_names,
                                       ids, num_xstats);
  ...
  ret = rte_event_dev_xstats_get(dev_id, mode, port_queue_id, ids, values,
                                 num_xstats);
  if (ret < 0 || ret > num_xstats) {
    free(xstat_names);
    free(ids);
  ...
  for (i = 0; i < num_xstats; i++)
    rte_tel_data_add_dict_u64(d, xstat_names[i].name, values[i]);
...
static int handle_dev_xstats(const char *cmd __rte_unused, const char *params,
                             struct rte_tel_data *d) {
  int dev_id;
  enum rte_event_dev_xstats_mode mode;
  char *end_param;
...
static int handle_port_xstats(const char *cmd __rte_unused, const char *params,
                              struct rte_tel_data *d) {
  int dev_id;
  int port_queue_id = 0;
  enum rte_event_dev_xstats_mode mode;
...
static int handle_queue_xstats(const char *cmd __rte_unused, const char *params,
                               struct rte_tel_data *d) {
  int dev_id;
  int port_queue_id = 0;
  enum rte_event_dev_xstats_mode mode;
...
RTE_INIT(eventdev_init_telemetry) {
  rte_telemetry_register_cmd(
      "/eventdev/dev_list", handle_dev_list,
      "Returns list of available eventdevs. Takes no parameters");
  rte_telemetry_register_cmd(
      "/eventdev/port_list", handle_port_list,
      "Returns list of available ports. Parameter: DevID");
  rte_telemetry_register_cmd(
      "/eventdev/queue_list", handle_queue_list,
  ...
  rte_telemetry_register_cmd(
      "/eventdev/port_xstats", handle_port_xstats,
      "Returns stats for an eventdev port. Params: DevID,PortID");
  rte_telemetry_register_cmd(
      "/eventdev/queue_xstats", handle_queue_xstats,
      "Returns stats for an eventdev queue. Params: DevID,QueueID");
  rte_telemetry_register_cmd(
      "/eventdev/queue_links", handle_queue_links,
