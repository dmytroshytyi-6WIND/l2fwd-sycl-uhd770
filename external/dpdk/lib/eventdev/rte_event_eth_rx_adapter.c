...
rxa_timestamp_dynfield(struct rte_mbuf *mbuf) {
  return RTE_MBUF_DYNFIELD(mbuf, event_eth_rx_timestamp_dynfield_offset,
                           rte_mbuf_timestamp_t *);
}

static inline int rxa_validate_id(uint8_t id) {
...
#define RTE_EVENT_ETH_RX_ADAPTER_ID_VALID_OR_ERR_RET(id, retval)               \
  do {                                                                         \
...
#define RTE_EVENT_ETH_RX_ADAPTER_ID_VALID_OR_GOTO_ERR_RET(id, retval)          \
  do {                                                                         \
    if (!rxa_validate_id(id)) {                                                \
      RTE_EDEV_LOG_ERR("Invalid eth Rx adapter id = %d\n", id);                \
      ret = retval;                                                            \
...
#define RTE_EVENT_ETH_RX_ADAPTER_TOKEN_VALID_OR_GOTO_ERR_RET(token, retval)    \
  do {                                                                         \
...
#define RTE_ETH_VALID_PORTID_OR_GOTO_ERR_RET(port_id, retval)                  \
  do {                                                                         \
 ...
 */
static int rxa_wrr_next(struct event_eth_rx_adapter *rx_adapter, unsigned int n,
                        int *cw, struct eth_rx_poll_entry *eth_rx_poll,
                        uint16_t max_wt, uint16_t gcd, int prev) {
  int i = prev;
  uint16_t w;
...
static inline int rxa_shared_intr(struct eth_device_info *dev_info,
                                  int rx_queue_id) {
  int multi_intr_cap;
  ...
  return !multi_intr_cap || rx_queue_id >= RTE_MAX_RXTX_INTR_VEC_ID - 1;
}

static inline int rxa_intr_queue(struct eth_device_info *dev_info,
                                 int rx_queue_id) {
  struct eth_rx_queue_info *queue_info;

  queue_info = &dev_info->rx_queue[rx_queue_id];
  return dev_info->rx_queue && !dev_info->internal_event_port &&
         queue_info->queue_enabled && queue_info->wt == 0;
}

static inline int rxa_polled_queue(struct eth_device_info *dev_info,
                                   int rx_queue_id) {
  struct eth_rx_queue_info *queue_info;

  queue_info = &dev_info->rx_queue[rx_queue_id];
  return !dev_info->internal_event_port && dev_info->rx_queue &&
         queue_info->queue_enabled && queue_info->wt != 0;
}

/* Calculate change in number of vectors after Rx queue ID is add/deleted */
static int rxa_nb_intr_vect(struct eth_device_info *dev_info, int rx_queue_id,
                            int add) {
  uint16_t i;
  int n, s;
  uint16_t nbq;
  ...
  if (rx_queue_id == -1) {
    for (i = 0; i < nbq; i++) {
      if (!rxa_shared_intr(dev_info, i))
        n += add ? !rxa_intr_queue(dev_info, i) : rxa_intr_queue(dev_info, i);
      else
        s += add ? !rxa_intr_queue(dev_info, i) : rxa_intr_queue(dev_info, i);
    }
    ...
    if (!rxa_shared_intr(dev_info, rx_queue_id))
      ...
      n = add ? !dev_info->nb_shared_intr : dev_info->nb_shared_intr == 1;
  }
 ...
 */
static void rxa_calc_nb_post_intr_del(struct event_eth_rx_adapter *rx_adapter,
                                      struct eth_device_info *dev_info,
                                      int rx_queue_id, uint32_t *nb_rx_intr) {
  uint32_t intr_diff;
 ...
 */
static void rxa_calc_nb_post_add_intr(struct event_eth_rx_adapter *rx_adapter,
                                      struct eth_device_info *dev_info,
                                      int rx_queue_id, uint32_t *nb_rx_poll,
  ...
  if (rx_queue_id == -1) {
    intr_diff = dev_info->dev->data->nb_rx_queues - dev_info->nb_rx_intr;
    poll_diff = dev_info->nb_rx_poll;
    wrr_len_diff = dev_info->wrr_len;
  } else {
    intr_diff = !rxa_intr_queue(dev_info, rx_queue_id);
    poll_diff = rxa_polled_queue(dev_info, rx_queue_id);
    wrr_len_diff = poll_diff ? dev_info->rx_queue[rx_queue_id].wt : 0;
  }
 ...
 */
static void rxa_calc_nb_post_poll_del(struct event_eth_rx_adapter *rx_adapter,
                                      struct eth_device_info *dev_info,
                                      int rx_queue_id, uint32_t *nb_rx_poll,
                                      uint32_t *nb_wrr) {
  uint32_t poll_diff;
 ...
 */
static void rxa_calc_nb_post_add_poll(struct event_eth_rx_adapter *rx_adapter,
                                      struct eth_device_info *dev_info,
                                      int rx_queue_id, uint16_t wt,
                                      uint32_t *nb_rx_poll,
                                      uint32_t *nb_rx_intr, uint32_t *nb_wrr) {
  uint32_t intr_diff;
  uint32_t poll_diff;
  ...
  if (rx_queue_id == -1) {
    intr_diff = dev_info->nb_rx_intr;
    poll_diff = dev_info->dev->data->nb_rx_queues - dev_info->nb_rx_poll;
    wrr_len_diff = wt * dev_info->dev->data->nb_rx_queues - dev_info->wrr_len;
  } else {
    intr_diff = rxa_intr_queue(dev_info, rx_queue_id);
    poll_diff = !rxa_polled_queue(dev_info, rx_queue_id);
...
static void rxa_calc_nb_post_add(struct event_eth_rx_adapter *rx_adapter,
                                 struct eth_device_info *dev_info,
                                 int rx_queue_id, uint16_t wt,
                                 uint32_t *nb_rx_poll, uint32_t *nb_rx_intr,
                                 uint32_t *nb_wrr) {
  if (wt != 0)
    rxa_calc_nb_post_add_poll(rx_adapter, dev_info, rx_queue_id, wt, nb_rx_poll,
                              nb_rx_intr, nb_wrr);
  else
    rxa_calc_nb_post_add_intr(rx_adapter, dev_info, rx_queue_id, nb_rx_poll,
                              nb_rx_intr, nb_wrr);
}

/* Calculate nb_rx_* after deleting rx_queue_id */
static void rxa_calc_nb_post_del(struct event_eth_rx_adapter *rx_adapter,
                                 struct eth_device_info *dev_info,
                                 int rx_queue_id, uint32_t *nb_rx_poll,
                                 uint32_t *nb_rx_intr, uint32_t *nb_wrr) {
  rxa_calc_nb_post_poll_del(rx_adapter, dev_info, rx_queue_id, nb_rx_poll,
                            nb_wrr);
  rxa_calc_nb_post_intr_del(rx_adapter, dev_info, rx_queue_id, nb_rx_intr);
}
...
rxa_alloc_poll(struct event_eth_rx_adapter *rx_adapter,
  ...
  len = RTE_ALIGN(num_rx_polled * sizeof(*rx_adapter->eth_rx_poll),
                  RTE_CACHE_LINE_SIZE);
  return rte_zmalloc_socket(rx_adapter->mem_name, len, RTE_CACHE_LINE_SIZE,
 ...
 */
static uint32_t *rxa_alloc_wrr(struct event_eth_rx_adapter *rx_adapter,
                               int nb_wrr) {
  size_t len;

  len = RTE_ALIGN(nb_wrr * sizeof(*rx_adapter->wrr_sched), RTE_CACHE_LINE_SIZE);
  return rte_zmalloc_socket(rx_adapter->mem_name, len, RTE_CACHE_LINE_SIZE,
                            rx_adapter->socket_id);
}

static int rxa_alloc_poll_arrays(struct event_eth_rx_adapter *rx_adapter,
                                 uint32_t nb_poll, uint32_t nb_wrr,
...
static void rxa_calc_wrr_sequence(struct event_eth_rx_adapter *rx_adapter,
                                  struct eth_rx_poll_entry *rx_poll,
                                  uint32_t *rx_wrr) {
  uint16_t d;
  uint16_t q;
  ...
  RTE_ETH_FOREACH_DEV(d) {
    ...
    struct eth_device_info *dev_info = &rx_adapter->eth_devices[d];
    ...
    for (q = 0; q < nb_rx_queues; q++) {
      struct eth_rx_queue_info *queue_info = &dev_info->rx_queue[q];
      uint16_t wt;
  ...
  for (i = 0; i < max_wrr_pos; i++) {
    rx_wrr[i] =
        rxa_wrr_next(rx_adapter, poll_q, &cw, rx_poll, max_wt, gcd, prev);
    prev = rx_wrr[i];
  }
}

static inline void rxa_mtoip(struct rte_mbuf *m, struct rte_ipv4_hdr **ipv4_hdr,
...
static inline uint32_t rxa_do_softrss(struct rte_mbuf *m,
                                      const uint8_t *rss_key_be) {
  uint32_t input_len;
  void *tuple;
...
static inline int rxa_enq_blocked(struct event_eth_rx_adapter *rx_adapter) {
...
rxa_enq_block_end_ts(struct event_eth_rx_adapter *rx_adapter,
  ...
  if (rx_adapter->rx_enq_block_start_ts) {
    ...
    stats->rx_enq_block_cycles +=
        stats->rx_enq_end_ts - rx_adapter->rx_enq_block_start_ts;
...
rxa_flush_event_buffer(struct event_eth_rx_adapter *rx_adapter,
  ...
  if (buf->last && n == count) {
    ...
    n1 = rte_event_enqueue_new_burst(rx_adapter->eventdev_id,
                                     rx_adapter->event_port_id, &buf->events[0],
                                     buf->tail);
...
static inline void rxa_init_vector(struct event_eth_rx_adapter *rx_adapter,
...
rxa_create_event_vector(struct event_eth_rx_adapter *rx_adapter,
  ...
  if (vec->vector_ev == NULL) {
    if (rte_mempool_get(vec->vector_pool, (void **)&vec->vector_ev) < 0) {
...
rxa_buffer_mbufs(struct event_eth_rx_adapter *rx_adapter, uint16_t eth_dev_id,
  ...
  if (!eth_rx_queue_info->ena_vector) {
    ts =
        m->ol_flags & event_eth_rx_timestamp_dynflag ? 0 : rte_get_tsc_cycles();

    /* 0xffff ffff ffff ffff if RTE_MBUF_F_RX_TIMESTAMP is set,
     * otherwise 0
     */
    ts_mask =
        (uint64_t)(!(m->ol_flags & event_eth_rx_timestamp_dynflag)) - 1ULL;
    ...
    for (i = 0; i < num; i++) {
      ...
      m = mbufs[i];
      *rxa_timestamp_dynfield(m) = ts | (*rxa_timestamp_dynfield(m) & ts_mask);

      ev = &buf->events[new_tail];

      rss = do_rss ? rxa_do_softrss(m, rx_adapter->rss_key_be) : m->hash.rss;
      ev->event = event;
      ev->flow_id = (rss & ~flow_id_mask) | (ev->flow_id & flow_id_mask);
      ev->mbuf = m;
      new_tail++;
    }
  } else {
    num =
        rxa_create_event_vector(rx_adapter, eth_rx_queue_info, buf, mbufs, num);
  }
  ...
  if (num && dev_info->cb_fn) {
    ...
    nb_cb = dev_info->cb_fn(
        ...
        buf->last | (buf->events_size & ~buf->last_mask),
        buf->count >= BATCH_SIZE ? buf->count - BATCH_SIZE : 0,
        &buf->events[buf->tail], num, dev_info->cb_arg, &dropped);
    if (unlikely(nb_cb > num))
      RTE_EDEV_LOG_ERR("Rx CB returned %d (> %d) events", nb_cb, num);
    else
      num = nb_cb;
    if (dropped)
...
static inline bool rxa_pkt_buf_available(struct eth_event_enqueue_buffer *buf) {
...
rxa_eth_rx(struct event_eth_rx_adapter *rx_adapter, uint16_t port_id,
  ...
  while (rxa_pkt_buf_available(buf)) {
    ...
    rxa_buffer_mbufs(rx_adapter, port_id, queue_id, mbufs, n, buf, stats);
    nb_rx += n;
  ...
  if (nb_flushed == 0)
    rte_event_maintain(rx_adapter->eventdev_id, rx_adapter->event_port_id, 0);
...
static int rxa_intr_ring_check_avail(struct event_eth_rx_adapter *rx_adapter,
                                     uint32_t num_intr_vec) {
  if (rx_adapter->num_intr_vec + num_intr_vec > RTE_EVENT_ETH_INTR_RING_SIZE) {
...
static void rxa_intr_ring_del_entries(struct event_eth_rx_adapter *rx_adapter,
                                      struct eth_device_info *dev_info,
                                      uint16_t rx_queue_id) {
  int i, n;
  union queue_data qd;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!rxa_shared_intr(dev_info, rx_queue_id)) {
      if (qd.port == dev_info->dev->data->port_id && qd.queue == rx_queue_id)
        continue;
    } else {
 ...
 */
static void *rxa_intr_thread(void *arg) {
...
rxa_intr_ring_dequeue(struct event_eth_rx_adapter *rx_adapter) {
  ...
  if (rte_ring_count(rx_adapter->intr_ring) == 0 && !rx_adapter->qd_valid)
    return;
    ...
    if (rxa_shared_intr(dev_info, queue)) {
      ...
      for (i = dev_info->next_q_idx; i < nb_queues; i++) {
        ...
        if (!rxa_intr_queue(dev_info, i))
          continue;
        n = rxa_eth_rx(rx_adapter, port, i, nb_rx, rx_adapter->max_nb_rx,
                       &rxq_empty, buf, stats);
        nb_rx += n;
      ...
      dev_info->next_q_idx =
          dev_info->multi_intr_cap ? RTE_MAX_RXTX_INTR_VEC_ID - 1 : 0;
    } else {
      n = rxa_eth_rx(rx_adapter, port, queue, nb_rx, rx_adapter->max_nb_rx,
                     &rxq_empty, buf, stats);
...
static inline void rxa_poll(struct event_eth_rx_adapter *rx_adapter) {
  ...
  for (num_queue = 0; num_queue < rx_adapter->wrr_len; num_queue++) {
    ...
    nb_rx += rxa_eth_rx(rx_adapter, d, qid, nb_rx, max_nb_rx, NULL, buf, stats);
    if (nb_rx > max_nb_rx) {
      rx_adapter->wrr_pos = (wrr_pos + 1) % rx_adapter->wrr_len;
      break;
    }
...
static void rxa_vector_expire(struct eth_rx_vector_data *vec, void *arg) {
  struct event_eth_rx_adapter *rx_adapter = arg;
  struct eth_event_enqueue_buffer *buf = NULL;
  struct rte_event_eth_rx_adapter_stats *stats = NULL;
  struct rte_event *ev;
...
static int rxa_service_func(void *args) {
...
static int rte_event_eth_rx_adapter_init(void) {
  const char *name = RXA_ADAPTER_ARRAY;
  const struct rte_memzone *mz;
  unsigned int sz;

  sz = sizeof(*event_eth_rx_adapter) * RTE_EVENT_ETH_RX_ADAPTER_MAX_INSTANCE;
  ...
  if (mz == NULL) {
    mz = rte_memzone_reserve_aligned(name, sz, rte_socket_id(), 0,
                                     RTE_CACHE_LINE_SIZE);
    if (mz == NULL) {
      RTE_EDEV_LOG_ERR("failed to reserve memzone err = %" PRId32, rte_errno);
      return -rte_errno;
    }
  }
...
static int rxa_memzone_lookup(void) {
  const struct rte_memzone *mz;
...
static inline struct event_eth_rx_adapter *rxa_id_to_adapter(uint8_t id) {
  return event_eth_rx_adapter ? event_eth_rx_adapter[id] : NULL;
}

static int rxa_default_conf_cb(uint8_t id, uint8_t dev_id,
...
static int rxa_epoll_create1(void) {
#if defined(LINUX)
  int fd;
...
static int rxa_init_epd(struct event_eth_rx_adapter *rx_adapter) {
...
static int rxa_create_intr_thread(struct event_eth_rx_adapter *rx_adapter) {
  int err;
  ...
  rx_adapter->intr_ring = rte_ring_create(
      "intr_ring", RTE_EVENT_ETH_INTR_RING_SIZE, rte_socket_id(), 0);
  if (!rx_adapter->intr_ring)
    return -ENOMEM;

  rx_adapter->epoll_events = rte_zmalloc_socket(
      rx_adapter->mem_name,
      RTE_EVENT_ETH_INTR_RING_SIZE * sizeof(struct rte_epoll_event),
      RTE_CACHE_LINE_SIZE, rx_adapter->socket_id);
  ...
  snprintf(thread_name, RTE_MAX_THREAD_NAME_LEN, "rx-intr-thread-%d",
           rx_adapter->id);

  err = rte_ctrl_thread_create(&rx_adapter->rx_intr_thread, thread_name, NULL,
...
static int rxa_destroy_intr_thread(struct event_eth_rx_adapter *rx_adapter) {
  int err;
...
static int rxa_free_intr_resources(struct event_eth_rx_adapter *rx_adapter) {
  int ret;
...
static int rxa_disable_intr(struct event_eth_rx_adapter *rx_adapter,
                            struct eth_device_info *dev_info,
                            uint16_t rx_queue_id) {
  int err;
  ...
  err = rte_eth_dev_rx_intr_ctl_q(eth_dev_id, rx_queue_id, rx_adapter->epd,
                                  RTE_INTR_EVENT_DEL, 0);
  if (err)
...
static int rxa_del_intr_queue(struct event_eth_rx_adapter *rx_adapter,
                              struct eth_device_info *dev_info,
                              int rx_queue_id) {
  int err;
  int i;
  ...
  if (rx_queue_id == -1) {
    ...
    for (i = 0; i < dev_info->nb_rx_intr; i++) {
      ...
      if (!sintr || s == 0) {
        ...
        err = rxa_disable_intr(rx_adapter, dev_info, q);
        if (err)
          return err;
        rxa_intr_ring_del_entries(rx_adapter, dev_info, q);
      }
    }
  } else {
    ...
    if (!rxa_shared_intr(dev_info, rx_queue_id) ||
      ...
      err = rxa_disable_intr(rx_adapter, dev_info, rx_queue_id);
      if (err)
        return err;
      rxa_intr_ring_del_entries(rx_adapter, dev_info, rx_queue_id);
    }

    for (i = 0; i < dev_info->nb_rx_intr; i++) {
      if (dev_info->intr_queue[i] == rx_queue_id) {
        for (; i < dev_info->nb_rx_intr - 1; i++)
          dev_info->intr_queue[i] = dev_info->intr_queue[i + 1];
        break;
      }
    }
  }
...
static int rxa_config_intr(struct event_eth_rx_adapter *rx_adapter,
                           struct eth_device_info *dev_info,
                           uint16_t rx_queue_id) {
  int err, err1;
  uint16_t eth_dev_id = dev_info->dev->data->port_id;
  ...
  if (dev_info->intr_queue == NULL) {
    size_t len = dev_info->dev->data->nb_rx_queues * sizeof(uint16_t);
    dev_info->intr_queue =
        rte_zmalloc_socket(rx_adapter->mem_name, len, 0, rx_adapter->socket_id);
    if (dev_info->intr_queue == NULL)
      return -ENOMEM;
  }
  ...
  if (err)
    RTE_EDEV_LOG_ERR("Could not disable interrupt for"
                     ...
                     rx_queue_id, err);
err_del_event:
  err1 = rte_eth_dev_rx_intr_ctl_q(eth_dev_id, rx_queue_id, rx_adapter->epd,
                                   RTE_INTR_EVENT_DEL, 0);
  if (err1) {
...
static int rxa_add_intr_queue(struct event_eth_rx_adapter *rx_adapter,
...
static int rxa_init_service(struct event_eth_rx_adapter *rx_adapter,
                            uint8_t id) {
  int ret;
  ...
  ret = rx_adapter->conf_cb(id, rx_adapter->eventdev_id, &rx_adapter_conf,
                            rx_adapter->conf_arg);
  if (ret) {
...
static void rxa_update_queue(struct event_eth_rx_adapter *rx_adapter,
                             struct eth_device_info *dev_info,
                             int32_t rx_queue_id, uint8_t add) {
  struct eth_rx_queue_info *queue_info;
  int enabled;
  uint16_t i;
...
static void rxa_set_vector_data(struct eth_rx_queue_info *queue_info,
                                uint16_t vector_count, uint64_t vector_ns,
                                struct rte_mempool *mp, uint32_t qid,
                                uint16_t port_id) {
#define NSEC2TICK(__ns, __freq) (((__ns) * (__freq)) / 1E9)
  ...
  vector_data->vector_timeout_ticks = NSEC2TICK(vector_ns, rte_get_timer_hz());
  vector_data->ts = 0;
  flow_id = queue_info->event & 0xFFFFF;
  flow_id = flow_id == 0 ? (qid & 0xFFF) | (port_id & 0xFF) << 12 : flow_id;
  vector_data->event = (queue_info->event & ~0xFFFFF) | flow_id;
}

static void rxa_sw_del(struct event_eth_rx_adapter *rx_adapter,
...
rxa_add_queue(struct event_eth_rx_adapter *rx_adapter,
  ...
  if (conf->rx_queue_flags & RTE_EVENT_ETH_RX_ADAPTER_QUEUE_FLOW_ID_VALID) {
  ...
  if (conf->rx_queue_flags & RTE_EVENT_ETH_RX_ADAPTER_QUEUE_EVENT_VECTOR) {
    ...
    rxa_set_vector_data(queue_info, conf->vector_sz, conf->vector_timeout_ns,
                        conf->vector_mp, rx_queue_id,
                        dev_info->dev->data->port_id);
    rx_adapter->ena_vector = 1;
    rx_adapter->vector_tmo_ticks =
        rx_adapter->vector_tmo_ticks
            ? RTE_MIN(queue_info->vector_data.vector_timeout_ticks >> 1,
                      rx_adapter->vector_tmo_ticks)
            : queue_info->vector_data.vector_timeout_ticks >> 1;
  }
  ...
  if (rxa_intr_queue(dev_info, rx_queue_id)) {
    ...
    if (dev_info->nb_shared_intr == 1) {
      if (dev_info->multi_intr_cap)
        dev_info->next_q_idx = RTE_MAX_RXTX_INTR_VEC_ID - 1;
      else
  ...
  new_rx_buf = rte_zmalloc_socket("rx_buffer_meta", sizeof(*new_rx_buf), 0,
  ...
  new_rx_buf->events = rte_zmalloc_socket(
      "rx_buffer", sizeof(struct rte_event) * new_rx_buf->events_size, 0,
      rte_eth_dev_socket_id(eth_dev_id));
  ...
  stats = rte_zmalloc_socket("rx_queue_stats", sizeof(*stats), 0,
                             rte_eth_dev_socket_id(eth_dev_id));
  if (stats == NULL) {
...
rxa_sw_add(struct event_eth_rx_adapter *rx_adapter, uint16_t eth_dev_id,
  ...
  if (queue_conf->servicing_weight == 0) {
    ...
    if (queue_conf->servicing_weight == 0 && rx_adapter->use_queue_event_buf) {
  ...
  if (dev_info->rx_queue == NULL) {
    dev_info->rx_queue = rte_zmalloc_socket(
        rx_adapter->mem_name, nb_rx_queues * sizeof(struct eth_rx_queue_info),
  ...
  rxa_calc_nb_post_add(rx_adapter, dev_info, rx_queue_id,
                       queue_conf->servicing_weight, &nb_rx_poll, &nb_rx_intr,
                       &nb_wrr);
  ...
  ret =
      rxa_alloc_poll_arrays(rx_adapter, nb_rx_poll, nb_wrr, &rx_poll, &rx_wrr);
  if (ret)
  ...
  if (wt == 0) {
    ...
    if (rx_adapter->num_rx_intr > nb_rx_intr) {
      num_intr_vec = rxa_nb_intr_vect(dev_info, rx_queue_id, 0);
      /* interrupt based queues are being converted to
       * poll mode queues, delete the interrupt configuration
       * for those.
       */
      ret = rxa_del_intr_queue(rx_adapter, dev_info, rx_queue_id);
      if (ret)
        goto err_free_rxqueue;
    }
  }
  ...
  if (wt == 0) {
    ...
    if (rx_queue_id == -1) {
      ...
      if (!rxa_intr_queue(dev_info, rx_queue_id))
        dev_info->intr_queue[nb_rx_intr - 1] = rx_queue_id;
    }
  }
...
static int rxa_ctrl(uint8_t id, int start) {
...
static int rxa_create(uint8_t id, uint8_t dev_id,
  ...
  snprintf(mem_name, ETH_RX_ADAPTER_MEM_NAME_LEN, "rte_event_eth_rx_adapter_%d",
           id);
  ...
  rx_adapter->eth_devices = rte_zmalloc_socket(
      rx_adapter->mem_name, RTE_MAX_ETHPORTS * sizeof(struct eth_device_info),
      0, socket_id);
  ...
  if (!rx_adapter->use_queue_event_buf) {
    ...
    events = rte_zmalloc_socket(
        rx_adapter->mem_name, buf->events_size * sizeof(*events), 0, socket_id);
    if (events == NULL) {
  ...
  rte_eventdev_trace_eth_rx_adapter_create(id, dev_id, conf_cb, conf_arg);
  return 0;
}

int rte_event_eth_rx_adapter_create_ext(
...
int rte_event_eth_rx_adapter_create_with_params(
    uint8_t id, uint8_t dev_id, struct rte_event_port_conf *port_config,
    struct rte_event_eth_rx_adapter_params *rxa_params) {
...
int rte_event_eth_rx_adapter_create(uint8_t id, uint8_t dev_id,
  ...
  *pc = *port_config;
  ...
  ret =
      rte_event_eth_rx_adapter_create_ext(id, dev_id, rxa_default_conf_cb, pc);
  if (ret)
    rte_free(pc);
  return ret;
}
...
int rte_event_eth_rx_adapter_queue_add(
  ...
  dev = &rte_eventdevs[rx_adapter->eventdev_id];
  ret = rte_event_eth_rx_adapter_caps_get(rx_adapter->eventdev_id, eth_dev_id,
                                          &cap);
  if (ret) {
  ...
  if ((cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_MULTI_EVENTQ) == 0 &&
    ...
    RTE_EDEV_LOG_ERR("Rx queues can only be connected to single "
                     "event queue, eth port: %" PRIu16 " adapter id: %" PRIu8,
                     eth_dev_id, id);
    return -EINVAL;
  }

  if (rx_queue_id != -1 &&
      (uint16_t)rx_queue_id >= rte_eth_devices[eth_dev_id].data->nb_rx_queues) {
    RTE_EDEV_LOG_ERR("Invalid rx queue_id %" PRIu16, (uint16_t)rx_queue_id);
    return -EINVAL;
  }

  if ((rx_adapter->use_queue_event_buf && queue_conf->event_buf_size == 0) ||
      (!rx_adapter->use_queue_event_buf && queue_conf->event_buf_size != 0)) {
  ...
  if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT) {
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->eth_rx_adapter_queue_add, -ENOTSUP);
    if (dev_info->rx_queue == NULL) {
      dev_info->rx_queue = rte_zmalloc_socket(
          rx_adapter->mem_name,
          dev_info->dev->data->nb_rx_queues * sizeof(struct eth_rx_queue_info),
          0, rx_adapter->socket_id);
      if (dev_info->rx_queue == NULL)
        return -ENOMEM;
    }

    ret = (*dev->dev_ops->eth_rx_adapter_queue_add)(
        dev, &rte_eth_devices[eth_dev_id], rx_queue_id, queue_conf);
    if (ret == 0) {
      dev_info->internal_event_port = 1;
      rxa_update_queue(rx_adapter, &rx_adapter->eth_devices[eth_dev_id],
                       rx_queue_id, 1);
    }
  } else {
    rte_spinlock_lock(&rx_adapter->rx_lock);
    ...
    if (ret == 0) {
      ...
      ret = rxa_sw_add(rx_adapter, eth_dev_id, rx_queue_id, queue_conf);
      rte_service_component_runstate_set(
          service_id, rxa_sw_adapter_queue_count(rx_adapter));
    }
    rte_spinlock_unlock(&rx_adapter->rx_lock);
  }

  rte_eventdev_trace_eth_rx_adapter_queue_add(id, eth_dev_id, rx_queue_id,
                                              queue_conf, ret);
  if (ret)
    return ret;
...
int rte_event_eth_rx_adapter_queue_del(uint8_t id, uint16_t eth_dev_id,
  ...
  if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT) {
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->eth_rx_adapter_queue_del, -ENOTSUP);
    ret = (*dev->dev_ops->eth_rx_adapter_queue_del)(
    ...
    rxa_calc_nb_post_del(rx_adapter, dev_info, rx_queue_id, &nb_rx_poll,
                         &nb_rx_intr, &nb_wrr);

    ret = rxa_alloc_poll_arrays(rx_adapter, nb_rx_poll, nb_wrr, &rx_poll,
                                &rx_wrr);
    if (ret)
      return ret;
  ...
  rte_eventdev_trace_eth_rx_adapter_queue_del(id, eth_dev_id, rx_queue_id, ret);
  return ret;
}
...
int rte_event_eth_rx_adapter_vector_limits_get(
  ...
  if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT) {
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->eth_rx_adapter_vector_limits_get,
                            -ENOTSUP);
...
static inline void rxa_queue_stats_reset(struct eth_rx_queue_info *queue_info) {
...
int rte_event_eth_rx_adapter_stats_get(
    uint8_t id, struct rte_event_eth_rx_adapter_stats *stats) {
  struct event_eth_rx_adapter *rx_adapter;
  struct eth_event_enqueue_buffer *buf;
  ...
  RTE_ETH_FOREACH_DEV(i) {
    ...
    if (rx_adapter->use_queue_event_buf && dev_info->rx_queue) {
      ...
      for (j = 0; j < dev_info->dev->data->nb_rx_queues; j++) {
        ...
        stats->rx_enq_block_cycles += q_stats->rx_enq_block_cycles;
      }
    }
...
int rte_event_eth_rx_adapter_queue_stats_get(
    uint8_t id, uint16_t eth_dev_id, uint16_t rx_queue_id,
  ...
  if (dev->dev_ops->eth_rx_adapter_queue_stats_get != NULL) {
    return (*dev->dev_ops->eth_rx_adapter_queue_stats_get)(
        dev, &rte_eth_devices[eth_dev_id], rx_queue_id, stats);
  }
...
int rte_event_eth_rx_adapter_queue_stats_reset(uint8_t id, uint16_t eth_dev_id,
                                               uint16_t rx_queue_id) {
  struct event_eth_rx_adapter *rx_adapter;
  struct eth_device_info *dev_info;
  struct eth_rx_queue_info *queue_info;
  ...
  if (dev->dev_ops->eth_rx_adapter_queue_stats_reset != NULL) {
    return (*dev->dev_ops->eth_rx_adapter_queue_stats_reset)(
        dev, &rte_eth_devices[eth_dev_id], rx_queue_id);
  }
...
int rte_event_eth_rx_adapter_event_port_get(uint8_t id,
                                            uint8_t *event_port_id) {
  struct event_eth_rx_adapter *rx_adapter;
...
int rte_event_eth_rx_adapter_cb_register(uint8_t id, uint16_t eth_dev_id,
  ...
  if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT) {
    RTE_EDEV_LOG_ERR("Rx callback not supported for eth port %" PRIu16,
                     eth_dev_id);
    return -EINVAL;
  }
...
int rte_event_eth_rx_adapter_queue_conf_get(
    uint8_t id, uint16_t eth_dev_id, uint16_t rx_queue_id,
...
#define TICK2NSEC(_ticks, _freq) (((_ticks) * (1E9)) / (_freq))
  ...
  if (queue_info->flow_id_mask != 0)
    queue_conf->rx_queue_flags |= RTE_EVENT_ETH_RX_ADAPTER_QUEUE_FLOW_ID_VALID;
  ...
  if (dev->dev_ops->eth_rx_adapter_queue_conf_get != NULL) {
    ret = (*dev->dev_ops->eth_rx_adapter_queue_conf_get)(
        dev, &rte_eth_devices[eth_dev_id], rx_queue_id, queue_conf);
    return ret;
  }
...
static int handle_rxa_stats(const char *cmd __rte_unused, const char *params,
  ...
  if (rte_event_eth_rx_adapter_stats_get(rx_adapter_id, &rx_adptr_stats)) {
...
static int handle_rxa_stats_reset(const char *cmd __rte_unused,
                                  const char *params,
...
static int handle_rxa_get_queue_conf(const char *cmd __rte_unused,
                                     const char *params,
                                     struct rte_tel_data *d) {
  uint8_t rx_adapter_id;
  uint16_t rx_queue_id;
...
static int handle_rxa_get_queue_stats(const char *cmd __rte_unused,
                                      const char *params,
                                      struct rte_tel_data *d) {
  uint8_t rx_adapter_id;
  uint16_t rx_queue_id;
...
static int handle_rxa_queue_stats_reset(const char *cmd __rte_unused,
                                        const char *params,
                                        struct rte_tel_data *d __rte_unused) {
  uint8_t rx_adapter_id;
  uint16_t rx_queue_id;
...
RTE_INIT(rxa_init_telemetry) {
  rte_telemetry_register_cmd("/eventdev/rxa_stats", handle_rxa_stats,
  ...
  rte_telemetry_register_cmd(
      "/eventdev/rxa_queue_conf", handle_rxa_get_queue_conf,
      "Returns Rx queue config. Parameter: rxa_id, dev_id, queue_id");

  rte_telemetry_register_cmd(
      "/eventdev/rxa_queue_stats", handle_rxa_get_queue_stats,
      "Returns Rx queue stats. Parameter: rxa_id, dev_id, queue_id");

  rte_telemetry_register_cmd(
      "/eventdev/rxa_queue_stats_reset", handle_rxa_queue_stats_reset,
