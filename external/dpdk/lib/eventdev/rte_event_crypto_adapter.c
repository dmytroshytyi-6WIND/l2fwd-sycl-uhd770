...
#define EVENT_CRYPTO_ADAPTER_ID_VALID_OR_ERR_RET(id, retval)                   \
  do {                                                                         \
    if (!eca_valid_id(id)) {                                                   \
...
static inline int eca_valid_id(uint8_t id) {
  return id < RTE_EVENT_CRYPTO_ADAPTER_MAX_INSTANCE;
}

static int eca_init(void) {
  const char *name = "crypto_adapter_array";
  const struct rte_memzone *mz;
  unsigned int sz;

  sz = sizeof(*event_crypto_adapter) * RTE_EVENT_CRYPTO_ADAPTER_MAX_INSTANCE;
...
eca_circular_buffer_init(const char *name,
                         struct crypto_ops_circular_buffer *bufp, uint16_t sz) {
  bufp->op_buffer = rte_zmalloc(name, sizeof(struct rte_crypto_op *) * sz, 0);
...
eca_circular_buffer_flush_to_cdev(struct crypto_ops_circular_buffer *bufp,
  ...
  *nb_ops_flushed =
      rte_cryptodev_enqueue_burst(cdev_id, qp_id, &ops[*headp], n);
...
static inline struct event_crypto_adapter *eca_id_to_adapter(uint8_t id) {
  return event_crypto_adapter ? event_crypto_adapter[id] : NULL;
}

static int eca_default_config_cb(uint8_t id, uint8_t dev_id,
...
int rte_event_crypto_adapter_create_ext(
    uint8_t id, uint8_t dev_id, rte_event_crypto_adapter_conf_cb conf_cb,
    enum rte_event_crypto_adapter_mode mode, void *conf_arg) {
  ...
  snprintf(mem_name, CRYPTO_ADAPTER_MEM_NAME_LEN, "rte_event_crypto_adapter_%d",
           id);

  adapter = rte_zmalloc_socket(mem_name, sizeof(*adapter), RTE_CACHE_LINE_SIZE,
                               socket_id);
  if (adapter == NULL) {
  ...
  if (eca_circular_buffer_init("eca_edev_circular_buffer", &adapter->ebuf,
  ...
  if (ret < 0) {
    RTE_EDEV_LOG_ERR("Failed to get info for eventdev %d: %s!", dev_id,
  ...
  adapter->implicit_release_disabled =
      (dev_info.event_dev_cap & RTE_EVENT_DEV_CAP_IMPLICIT_RELEASE_DISABLE);
  ...
  adapter->cdevs = rte_zmalloc_socket(
      adapter->mem_name,
      rte_cryptodev_count() * sizeof(struct crypto_device_info), 0, socket_id);
  ...
  rte_eventdev_trace_crypto_adapter_create(id, dev_id, adapter, conf_arg, mode);
  return 0;
}
...
int rte_event_crypto_adapter_create(uint8_t id, uint8_t dev_id,
  ...
  if (pc == NULL)
    return -ENOMEM;
  *pc = *port_config;
  ret = rte_event_crypto_adapter_create_ext(id, dev_id, eca_default_config_cb,
                                            mode, pc);
  if (ret)
    rte_free(pc);
...
int rte_event_crypto_adapter_free(uint8_t id) {
  ...
  if (adapter->nb_qps) {
    RTE_EDEV_LOG_ERR("%" PRIu16 "Queue pairs not deleted", adapter->nb_qps);
    return -EBUSY;
  }
...
eca_enq_to_cryptodev(struct event_crypto_adapter *adapter, struct rte_event *ev,
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (crypto_op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
      m_data = rte_cryptodev_sym_session_get_user_data(crypto_op->sym->session);
      if (m_data == NULL) {
      ...
      m_data =
          (union rte_event_crypto_metadata *)((uint8_t *)crypto_op +
                                              crypto_op->private_data_offset);
      cdev_id = m_data->request_info.cdev_id;
      qp_id = m_data->request_info.queue_pair_id;
      qp_info = &adapter->cdevs[cdev_id].qpairs[qp_id];
    ...
    if (eca_circular_buffer_batch_ready(&qp_info->cbuf)) {
      ret = eca_circular_buffer_flush_to_cdev(&qp_info->cbuf, cdev_id, qp_id,
                                              &nb_enqueued);
      /**
       ...
       */
      if (unlikely(ret < 0 &&
                   !eca_circular_buffer_space_for_batch(&qp_info->cbuf)))
...
static unsigned int eca_crypto_cdev_flush(struct event_crypto_adapter *adapter,
                                          uint8_t cdev_id,
                                          uint16_t *nb_ops_flushed) {
  ...
  for (qp = 0; qp < dev->data->nb_queue_pairs; qp++) {
    ...
    eca_circular_buffer_flush_to_cdev(&curr_queue->cbuf, cdev_id, qp,
                                      &nb_enqueued);
...
static unsigned int eca_crypto_enq_flush(struct event_crypto_adapter *adapter) {
  struct rte_event_crypto_adapter_stats *stats = &adapter->crypto_stats;
  uint8_t cdev_id;
  ...
  for (cdev_id = 0; cdev_id < num_cdev; cdev_id++)
    nb_enqueued += eca_crypto_cdev_flush(adapter, cdev_id, &nb_ops_flushed);
  /**
...
static int eca_crypto_adapter_enq_run(struct event_crypto_adapter *adapter,
  ...
  for (nb_enq = 0; nb_enq < max_enq; nb_enq += n) {
    ...
    n = rte_event_dequeue_burst(event_dev_id, event_port_id, ev, BATCH_SIZE, 0);
...
skip_event_dequeue_burst:
  ...
  if ((++adapter->transmit_loop_count & (CRYPTO_ENQ_FLUSH_THRESHOLD - 1)) ==
      0) {
    nb_enqueued += eca_crypto_enq_flush(adapter);
  }
...
eca_ops_enqueue_burst(struct event_crypto_adapter *adapter,
  ...
  for (i = 0; i < num; i++) {
    ...
    if (ops[i]->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
      m_data = rte_cryptodev_sym_session_get_user_data(ops[i]->sym->session);
    } else if (ops[i]->sess_type == RTE_CRYPTO_OP_SESSIONLESS &&
               ops[i]->private_data_offset) {
      m_data = (union rte_event_crypto_metadata *)((uint8_t *)ops[i] +
                                                   ops[i]->private_data_offset);
    }
  ...
  do {
    nb_enqueued += rte_event_enqueue_burst(
        event_dev_id, event_port_id, &events[nb_enqueued], nb_ev - nb_enqueued);

  } while (retry++ < CRYPTO_ADAPTER_MAX_EV_ENQ_RETRIES && nb_enqueued < nb_ev);
...
static void eca_ops_buffer_flush(struct event_crypto_adapter *adapter) {
  if (likely(adapter->ebuf.count == 0))
    return;

  while (eca_circular_buffer_flush_to_evdev(adapter, &adapter->ebuf))
    ;
}
static inline unsigned int
eca_crypto_adapter_deq_run(struct event_crypto_adapter *adapter,
  ...
  do {
    ...
    for (cdev_id = adapter->next_cdev_id; cdev_id < num_cdev; cdev_id++) {
      uint16_t queues = 0;
      ...
      for (qp = curr_dev->next_queue_pair_id; queues < dev_qps;
        ...
        n = rte_cryptodev_dequeue_burst(cdev_id, qp, ops, BATCH_SIZE);
        if (!n)
          continue;
        ...
        if (unlikely(!adapter->ebuf.count))
          nb_enqueued = eca_ops_enqueue_burst(adapter, ops, n);
        ...
        for (i = nb_enqueued; i < n; i++)
          eca_circular_buffer_add(&adapter->ebuf, ops[nb_enqueued]);
      ...
      check:
        ...
        if (nb_deq >= max_deq) {
          if ((qp + 1) == dev_qps) {
            adapter->next_cdev_id = (cdev_id + 1) % num_cdev;
          }
          curr_dev->next_queue_pair_id = (qp + 1) % dev->data->nb_queue_pairs;
...
static void eca_crypto_adapter_run(struct event_crypto_adapter *adapter,
...
static int eca_service_func(void *args) {
...
static int eca_init_service(struct event_crypto_adapter *adapter, uint8_t id) {
  ...
  snprintf(service.name, CRYPTO_ADAPTER_NAME_LEN, "rte_event_crypto_adapter_%d",
           id);
...
static void eca_update_qp_info(struct event_crypto_adapter *adapter,
...
static int eca_add_queue_pair(struct event_crypto_adapter *adapter,
                              uint8_t cdev_id, int queue_pair_id) {
  ...
  if (dev_info->qpairs == NULL) {
    ...
    if (eca_circular_buffer_init("eca_cdev_circular_buffer", &qpairs->cbuf,
  ...
  if (queue_pair_id == -1) {
    ...
    eca_update_qp_info(adapter, dev_info, (uint16_t)queue_pair_id, 1);
...
int rte_event_crypto_adapter_queue_pair_add(uint8_t id, uint8_t cdev_id,
  ...
  dev = &rte_eventdevs[adapter->eventdev_id];
  ret = rte_event_crypto_adapter_caps_get(adapter->eventdev_id, cdev_id, &cap);
  if (ret) {
  ...
  if ((cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_QP_EV_BIND) &&
      (event == NULL)) {
    RTE_EDEV_LOG_ERR("Conf value can not be NULL for dev_id=%u", cdev_id);
    return -EINVAL;
  }
  ...
  if ((cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_OP_FWD) ||
      ...
      (cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_OP_NEW &&
       adapter->mode == RTE_EVENT_CRYPTO_ADAPTER_OP_NEW)) {
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->crypto_adapter_queue_pair_add,
                            -ENOTSUP);
    if (dev_info->qpairs == NULL) {
      dev_info->qpairs =
          rte_zmalloc_socket(adapter->mem_name,
    ...
    if (ret)
      ...
      eca_update_qp_info(adapter, &adapter->cdevs[cdev_id], queue_pair_id, 1);
  }
  ...
  if ((cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_OP_NEW &&
    ...
    if (ret == 0)
      ret = eca_add_queue_pair(adapter, cdev_id, queue_pair_id);
    rte_spinlock_unlock(&adapter->lock);
...
int rte_event_crypto_adapter_queue_pair_del(uint8_t id, uint8_t cdev_id,
  ...
  if ((cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_OP_FWD) ||
    ...
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->crypto_adapter_queue_pair_del,
                            -ENOTSUP);
    ret = (*dev->dev_ops->crypto_adapter_queue_pair_del)(dev, dev_info->dev,
                                                         queue_pair_id);
    if (ret == 0) {
      eca_update_qp_info(adapter, &adapter->cdevs[cdev_id], queue_pair_id, 0);
    ...
    if (queue_pair_id == -1) {
      for (i = 0; i < dev_info->dev->data->nb_queue_pairs; i++)
        eca_update_qp_info(adapter, dev_info, queue_pair_id, 0);
    } else {
      eca_update_qp_info(adapter, dev_info, (uint16_t)queue_pair_id, 0);
    }
  ...
  rte_eventdev_trace_crypto_adapter_queue_pair_del(id, cdev_id, queue_pair_id,
                                                   ret);
  return ret;
}

static int eca_adapter_ctrl(uint8_t id, int start) {
  struct event_crypto_adapter *adapter;
  struct crypto_device_info *dev_info;
  struct rte_eventdev *dev;
  uint32_t i;
...
int rte_event_crypto_adapter_stats_get(
    uint8_t id, struct rte_event_crypto_adapter_stats *stats) {
  struct event_crypto_adapter *adapter;
  ...
  for (i = 0; i < rte_cryptodev_count(); i++) {
    ...
    if (dev_info->internal_event_port == 0 ||
      ...
      continue;
    ret = (*dev->dev_ops->crypto_adapter_stats_get)(dev, dev_info->dev,
                                                    &dev_stats);
    if (ret)
      continue;
...
int rte_event_crypto_adapter_event_port_get(uint8_t id,
                                            uint8_t *event_port_id) {
  struct event_crypto_adapter *adapter;
