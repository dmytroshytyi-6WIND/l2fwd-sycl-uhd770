...
#define EVTIM_LOG_DBG(...) EVTIM_LOG(DEBUG, evtim_logtype, __VA_ARGS__)
#define EVTIM_BUF_LOG_DBG(...)                                                 \
  EVTIM_LOG(DEBUG, evtim_buffer_logtype, __VA_ARGS__)
#define EVTIM_SVC_LOG_DBG(...) EVTIM_LOG(DEBUG, evtim_svc_logtype, __VA_ARGS__)
#else
...
static int default_port_conf_cb(uint16_t id, uint8_t event_dev_id,
  ...
  if (conf_arg != NULL)
    ...
    ret = rte_event_port_default_conf_get(dev_id, port_id, port_conf);
    if (ret < 0)
      return ret;
  }
...
struct rte_event_timer_adapter *rte_event_timer_adapter_create(
  ...
  return rte_event_timer_adapter_create_ext(conf, default_port_conf_cb, NULL);
}

struct rte_event_timer_adapter *rte_event_timer_adapter_create_ext(
    const struct rte_event_timer_adapter_conf *conf,
    rte_event_timer_adapter_port_conf_cb_t conf_cb, void *conf_arg) {
  uint16_t adapter_id;
  struct rte_event_timer_adapter *adapter;
  ...
  mz = rte_memzone_reserve(mz_name, sizeof(struct rte_event_timer_adapter_data),
                           conf->socket_id, 0);
  if (mz == NULL)
  ...
  ret = dev->dev_ops->timer_adapter_caps_get(
      dev, adapter->data->conf.flags, &adapter->data->caps, &adapter->ops);
  if (ret < 0) {
    rte_errno = -ret;
    goto free_memzone;
  }

  if (!(adapter->data->caps & RTE_EVENT_TIMER_ADAPTER_CAP_INTERNAL_PORT)) {
  ...
  rte_eventdev_trace_timer_adapter_create(adapter_id, adapter, conf, conf_cb);
  return adapter;
...
int rte_event_timer_adapter_stop(
    const struct rte_event_timer_adapter *adapter) {
  int ret;
...
int rte_event_timer_adapter_service_id_get(
    struct rte_event_timer_adapter *adapter, uint32_t *service_id) {
  ADAPTER_VALID_OR_ERR_RET(adapter, -EINVAL);
...
int rte_event_timer_adapter_stats_get(
    struct rte_event_timer_adapter *adapter,
...
static inline bool event_buffer_full(struct event_buffer *bufp) {
  return (bufp->head - bufp->tail) == EVENT_BUFFER_SZ;
}

static inline bool event_buffer_batch_ready(struct event_buffer *bufp) {
  return (bufp->head - bufp->tail) >= EVENT_BUFFER_BATCHSZ;
}

static void event_buffer_init(struct event_buffer *bufp) {
  bufp->head = bufp->tail = 0;
  memset(&bufp->events, 0, sizeof(struct rte_event) * EVENT_BUFFER_SZ);
}

static int event_buffer_add(struct event_buffer *bufp,
                            struct rte_event *eventp) {
  size_t head_idx;
...
static void event_buffer_flush(struct event_buffer *bufp, uint8_t dev_id,
                               uint8_t port_id, uint16_t *nb_events_flushed,
  ...
  *nb_events_flushed =
      rte_event_enqueue_burst(dev_id, port_id, &events[tail_idx], n);
...
static void swtim_callback(struct rte_timer *tim) {
  ...
  if (ret < 0) {
     ...
     * next iteration.
     */
    ret = rte_timer_alt_reset(sw->timer_data_id, tim, 0, SINGLE, lcore, NULL,
                              evtim);
    if (ret < 0) {
    ...
    if (unlikely(sw->in_use[lcore].v == 0)) {
      sw->in_use[lcore].v = 1;
      n_lcores = __atomic_fetch_add(&sw->n_poll_lcores, 1, __ATOMIC_RELAXED);
      __atomic_store_n(&sw->poll_lcores[n_lcores], lcore, __ATOMIC_RELAXED);
    }
  } else {
  ...
  if (event_buffer_batch_ready(&sw->buffer)) {
    event_buffer_flush(&sw->buffer, adapter->data->event_dev_id,
 ...
 */
static inline bool swtim_did_tick(struct swtim *sw) {
  ...
  cycles_per_adapter_tick =
      sw->timer_tick_ns * (rte_get_timer_hz() / NSECPERSEC);
...
check_destination_event_queue(struct rte_event_timer *evtim,
  ...
  ret =
      rte_event_queue_attr_get(adapter->data->event_dev_id, evtim->ev.queue_id,
                               RTE_EVENT_QUEUE_ATTR_SCHEDULE_TYPE, &sched_type);

  if ((ret == 0 && evtim->ev.sched_type == sched_type) || ret == -EOVERFLOW)
    return 0;

  return -1;
}

static int swtim_service_func(void *arg) {
  ...
  if (swtim_did_tick(sw)) {
    rte_timer_alt_manage(sw->timer_data_id, sw->poll_lcores, sw->n_poll_lcores,
                         swtim_callback);
  ...
  rte_event_maintain(adapter->data->event_dev_id, adapter->data->event_port_id,
                     0);
 ...
 */
static int compute_msg_mempool_cache_size(uint64_t nb_requested,
                                          uint64_t nb_actual) {
  int i;
  int size;
  int cache_size = 0;
  ...
  for (i = 0;; i++) {
    ...
    if (RTE_MAX_LCORE * size < (int)(nb_actual - nb_requested) &&
        size < RTE_MEMPOOL_CACHE_MAX_SIZE && size <= nb_actual / 1.5)
      cache_size = size;
    else
      break;
  }
...
static int swtim_init(struct rte_event_timer_adapter *adapter) {
  int i, ret;
  struct swtim *sw;
  unsigned int flags;
...
#define SWTIM_NAMESIZE 32
  ...
  snprintf(swtim_name, SWTIM_NAMESIZE, "swtim_%" PRIu8, adapter->data->id);
  ...
  snprintf(pool_name, SWTIM_NAMESIZE, "swtim_pool_%" PRIu8, adapter->data->id);
  ...
  int cache_size =
      compute_msg_mempool_cache_size(adapter->data->conf.nb_timers, nb_timers);
  flags = 0; /* pool is multi-producer, multi-consumer */
  sw->tim_pool = rte_mempool_create(
      pool_name, pool_size, sizeof(struct rte_timer), cache_size, 0, NULL, NULL,
  ...
  snprintf(service.name, RTE_SERVICE_NAME_MAX, "swtim_svc_%" PRIu8,
           adapter->data->id);
  ...
  if (ret < 0) {
    EVTIM_LOG_ERR("failed to register service %s with id %" PRIu32 ": err = %d",
                  service.name, sw->service_id, ret);
...
static void swtim_free_tim(struct rte_timer *tim, void *arg) {
  struct swtim *sw = arg;
 ...
 */
static int swtim_uninit(struct rte_event_timer_adapter *adapter) {
  int ret;
  struct swtim *sw = swtim_pmd_priv(adapter);

  /* Free outstanding timers */
  rte_timer_stop_all(sw->timer_data_id, sw->poll_lcores, sw->n_poll_lcores,
                     swtim_free_tim, sw);
...
static inline int32_t get_mapped_count_for_service(uint32_t service_id) {
...
static int swtim_start(const struct rte_event_timer_adapter *adapter) {
  int mapped_count;
  struct swtim *sw = swtim_pmd_priv(adapter);
...
static int swtim_stop(const struct rte_event_timer_adapter *adapter) {
  int ret;
  struct swtim *sw = swtim_pmd_priv(adapter);
...
static void swtim_get_info(const struct rte_event_timer_adapter *adapter,
                           struct rte_event_timer_adapter_info *adapter_info) {
  struct swtim *sw = swtim_pmd_priv(adapter);
...
static int swtim_stats_get(const struct rte_event_timer_adapter *adapter,
                           struct rte_event_timer_adapter_stats *stats) {
  struct swtim *sw = swtim_pmd_priv(adapter);
...
static int swtim_stats_reset(const struct rte_event_timer_adapter *adapter) {
  struct swtim *sw = swtim_pmd_priv(adapter);
  memset(&sw->stats, 0, sizeof(sw->stats));
  return 0;
}

static uint16_t __swtim_arm_burst(const struct rte_event_timer_adapter *adapter,
   ...
   */
  if (unlikely(__atomic_compare_exchange_n(&sw->in_use[lcore_id].v, &exp_state,
                                           1, 0, __ATOMIC_RELAXED,
                                           __ATOMIC_RELAXED))) {
    EVTIM_LOG_DBG("Adding lcore id = %u to list of lcores to poll", lcore_id);
    n_lcores = __atomic_fetch_add(&sw->n_poll_lcores, 1, __ATOMIC_RELAXED);
    __atomic_store_n(&sw->poll_lcores[n_lcores], lcore_id, __ATOMIC_RELAXED);
  }

  ret = rte_mempool_get_bulk(sw->tim_pool, (void **)tims, nb_evtims);
  if (ret < 0) {
    rte_errno = ENOSPC;
    return 0;
  }
  ...
  for (i = 0; i < nb_evtims; i++) {
    ...
    if (unlikely(ret == -1)) {
      __atomic_store_n(&evtims[i]->state, RTE_EVENT_TIMER_ERROR_TOOLATE,
                       __ATOMIC_RELAXED);
      rte_errno = EINVAL;
      break;
    } else if (unlikely(ret == -2)) {
      __atomic_store_n(&evtims[i]->state, RTE_EVENT_TIMER_ERROR_TOOEARLY,
                       __ATOMIC_RELAXED);
      rte_errno = EINVAL;
      break;
    }

    if (unlikely(check_destination_event_queue(evtims[i], adapter) < 0)) {
    ...
    ret = rte_timer_alt_reset(sw->timer_data_id, tim, cycles, SINGLE, lcore_id,
                              NULL, evtims[i]);
    if (ret < 0) {
  ...
  if (i < nb_evtims)
    rte_mempool_put_bulk(sw->tim_pool, (void **)&tims[i], nb_evtims - i);

  return i;
}

static uint16_t swtim_arm_burst(const struct rte_event_timer_adapter *adapter,
                                struct rte_event_timer **evtims,
                                uint16_t nb_evtims) {
...
swtim_cancel_burst(const struct rte_event_timer_adapter *adapter,
                   struct rte_event_timer **evtims, uint16_t nb_evtims) {
  int i, ret;
...
swtim_arm_tmo_tick_burst(const struct rte_event_timer_adapter *adapter,
                         struct rte_event_timer **evtims,
                         uint64_t timeout_ticks, uint16_t nb_evtims) {
  int i;
