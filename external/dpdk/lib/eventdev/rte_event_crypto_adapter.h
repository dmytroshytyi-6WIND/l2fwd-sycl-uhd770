 ...
 *  rte_event_crypto_adapter_create_ext().
 */
typedef int (*rte_event_crypto_adapter_conf_cb)(
    uint8_t id, uint8_t dev_id, struct rte_event_crypto_adapter_conf *conf,
    void *arg);
 ...
 *              closed systems.
 */
static inline uint16_t rte_event_crypto_adapter_enqueue(uint8_t dev_id,
                                                        uint8_t port_id,
                                                        struct rte_event ev[],
                                                        uint16_t nb_events) {
...
#endif
  rte_eventdev_trace_crypto_adapter_enqueue(dev_id, port_id, ev, nb_events);
