...
static uint16_t dummy_event_enqueue(__rte_unused void *port,
                                    __rte_unused const struct rte_event *ev) {
  RTE_EDEV_LOG_ERR("event enqueue requested for unconfigured event device");
  return 0;
}
...
static uint16_t dummy_event_dequeue(__rte_unused void *port,
                                    __rte_unused struct rte_event *ev,
                                    __rte_unused uint64_t timeout_ticks) {
  RTE_EDEV_LOG_ERR("event dequeue requested for unconfigured event device");
  return 0;
}

static uint16_t dummy_event_dequeue_burst(__rte_unused void *port,
                                          __rte_unused struct rte_event ev[],
                                          __rte_unused uint16_t nb_events,
                                          __rte_unused uint64_t timeout_ticks) {
  RTE_EDEV_LOG_ERR(
...
static void dummy_event_maintain(__rte_unused void *port, __rte_unused int op) {
  RTE_EDEV_LOG_ERR("maintenance requested for unconfigured event device");
}
...
dummy_event_tx_adapter_enqueue_same_dest(__rte_unused void *port,
                                         __rte_unused struct rte_event ev[],
                                         __rte_unused uint16_t nb_events) {
  RTE_EDEV_LOG_ERR("event Tx adapter enqueue same destination requested for "
...
void event_dev_fp_ops_reset(struct rte_event_fp_ops *fp_op) {
...
void event_dev_fp_ops_set(struct rte_event_fp_ops *fp_op,
