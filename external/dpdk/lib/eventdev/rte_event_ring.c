...
int rte_event_ring_init(struct rte_event_ring *r, const char *name,
  ...
  RTE_BUILD_BUG_ON((sizeof(struct rte_event_ring) & RTE_CACHE_LINE_MASK) != 0);
...
struct rte_event_ring *rte_event_ring_create(const char *name,
                                             ...
                                             unsigned int flags) {
  return (struct rte_event_ring *)rte_ring_create_elem(
      name, sizeof(struct rte_event), count, socket_id, flags);
}
