...
rte_event_ring_enqueue_burst(struct rte_event_ring *r,
                             const struct rte_event *events, unsigned int n,
                             uint16_t *free_space) {
  unsigned int num;
  uint32_t space;

  num = rte_ring_enqueue_burst_elem(&r->r, events, sizeof(struct rte_event), n,
                                    &space);
 ...
 */
static __rte_always_inline unsigned int
rte_event_ring_dequeue_burst(struct rte_event_ring *r, struct rte_event *events,
                             unsigned int n, uint16_t *available) {
  unsigned int num;
  uint32_t remaining;

  num = rte_ring_dequeue_burst_elem(&r->r, events, sizeof(struct rte_event), n,
                                    &remaining);
