 ...
 * the name.
 */
__rte_internal static inline int rte_event_pmd_pci_probe_named(
    struct rte_pci_driver *pci_drv, struct rte_pci_device *pci_dev,
    size_t private_data_size, eventdev_pmd_pci_callback_t devinit,
    const char *name) {
  ...
  eventdev = rte_event_pmd_allocate(name, pci_dev->device.numa_node);
  if (eventdev == NULL)
    return -ENOMEM;

  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    eventdev->data->dev_private =
        rte_zmalloc_socket("eventdev private structure", private_data_size,
  ...
  RTE_EDEV_LOG_ERR("driver %s: (vendor_id=0x%x device_id=0x%x)"
                   " failed",
                   pci_drv->driver.name, (unsigned int)pci_dev->id.vendor_id,
 ...
 * interface.
 */
__rte_internal static inline int rte_event_pmd_pci_probe(
    struct rte_pci_driver *pci_drv, struct rte_pci_device *pci_dev,
    size_t private_data_size, eventdev_pmd_pci_callback_t devinit) {
  char eventdev_name[RTE_EVENTDEV_NAME_MAX_LEN];

  rte_pci_device_name(&pci_dev->addr, eventdev_name, sizeof(eventdev_name));

  return rte_event_pmd_pci_probe_named(pci_drv, pci_dev, private_data_size,
                                       devinit, eventdev_name);
}
 ...
 * interface.
 */
__rte_internal static inline int
