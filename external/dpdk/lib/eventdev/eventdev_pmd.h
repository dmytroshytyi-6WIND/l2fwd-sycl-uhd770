...
#define RTE_EVENTDEV_VALID_DEVID_OR_ERRNO_RET(dev_id, errno, retval)           \
  do {                                                                         \
    if (!rte_event_pmd_is_valid_dev((dev_id))) {                               \
      RTE_EDEV_LOG_ERR("Invalid dev_id=%d\n", dev_id);                         \
      rte_errno = errno;                                                       \
...
struct rte_eventdev_data {
  ...
  uint16_t
      links_map[RTE_EVENT_MAX_PORTS_PER_DEV * RTE_EVENT_MAX_QUEUES_PER_DEV];
 ...
 */
__rte_internal static inline struct rte_eventdev *
 ...
 */
__rte_internal static inline unsigned
...
typedef void (*eventdev_queue_default_conf_get_t)(
    struct rte_eventdev *dev, uint8_t queue_id,
 ...
 *   Returns 0 on success.
 */
typedef int (*eventdev_queue_setup_t)(
    struct rte_eventdev *dev, uint8_t queue_id,
 ...
 *
 */
typedef void (*eventdev_port_default_conf_get_t)(
    struct rte_eventdev *dev, uint8_t port_id,
 ...
 *   Returns 0 on success.
 */
typedef int (*eventdev_port_setup_t)(
    struct rte_eventdev *dev, uint8_t port_id,
 ...
 */
typedef int (*eventdev_xstats_get_names_t)(
...
typedef int (*eventdev_eth_rx_adapter_q_stats_get)(
    const struct rte_eventdev *dev, const struct rte_eth_dev *eth_dev,
    uint16_t rx_queue_id, struct rte_event_eth_rx_adapter_queue_stats *q_stats);
 ...
 */
__rte_internal static inline rte_event_pmd_selftest_seqn_t *
rte_event_pmd_selftest_seqn(struct rte_mbuf *mbuf) {
  return RTE_MBUF_DYNFIELD(mbuf, rte_event_pmd_selftest_seqn_dynfield_offset,
 ...
 *   - <0: Error code returned by the driver function.
 */
typedef int (*eventdev_crypto_adapter_stop_t)(const struct rte_eventdev *dev,
...
typedef int (*eventdev_eth_tx_adapter_queue_add_t)(
    uint8_t id, const struct rte_eventdev *dev,
...
struct eventdev_ops {
  ...
  eventdev_eth_rx_adapter_vector_limits_get_t eth_rx_adapter_vector_limits_get;
 ...
 */
__rte_internal struct rte_eventdev *rte_event_pmd_allocate(const char *name,
                                                           int socket_id);
 ...
 */
__rte_internal int rte_event_pmd_release(struct rte_eventdev *eventdev);
 ...
 *  New event device.
 */
__rte_internal void event_dev_probing_finish(struct rte_eventdev *eventdev);
 ...
 */
__rte_internal void event_dev_fp_ops_reset(struct rte_event_fp_ops *fp_op);
 ...
 */
__rte_internal void event_dev_fp_ops_set(struct rte_event_fp_ops *fp_ops,
                                         const struct rte_eventdev *dev);
