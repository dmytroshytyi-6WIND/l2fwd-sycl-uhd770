...
void rte_distributor_request_pkt(struct rte_distributor *d,
                                 ...
                                 struct rte_mbuf **oldpkt, unsigned int count) {
  ...
  if (unlikely(d->alg_type == RTE_DIST_ALG_SINGLE)) {
    rte_distributor_request_pkt_single(d->d_single, worker_id,
                                       count ? oldpkt[0] : NULL);
    return;
  }
   ...
   */
  __atomic_store_n(retptr64, *retptr64 | RTE_DISTRIB_GET_BUF, __ATOMIC_RELEASE);
}

int rte_distributor_poll_pkt(struct rte_distributor *d, unsigned int worker_id,
  ...
  if (unlikely(d->alg_type == RTE_DIST_ALG_SINGLE)) {
    pkts[0] = rte_distributor_poll_pkt_single(d->d_single, worker_id);
    return (pkts[0]) ? 1 : 0;
  }
   ...
   */
  __atomic_store_n(&(buf->bufptr64[0]), buf->bufptr64[0] | RTE_DISTRIB_GET_BUF,
                   __ATOMIC_RELEASE);

  return count;
}

int rte_distributor_get_pkt(struct rte_distributor *d, unsigned int worker_id,
                            struct rte_mbuf **pkts, struct rte_mbuf **oldpkt,
                            unsigned int return_count) {
  int count;

  if (unlikely(d->alg_type == RTE_DIST_ALG_SINGLE)) {
    if (return_count <= 1) {
      pkts[0] = rte_distributor_get_pkt_single(d->d_single, worker_id,
...
int rte_distributor_return_pkt(struct rte_distributor *d,
  ...
  if (unlikely(d->alg_type == RTE_DIST_ALG_SINGLE)) {
    if (num == 1)
      return rte_distributor_return_pkt_single(d->d_single, worker_id,
                                               oldpkt[0]);
    else if (num == 0)
      return rte_distributor_return_pkt_single(d->d_single, worker_id, NULL);
    else
      return -EINVAL;
  }
...
static inline void store_return(uintptr_t oldbuf, struct rte_distributor *d,
                                unsigned int *ret_start,
 ...
 */
void find_match_scalar(struct rte_distributor *d, uint16_t *data_ptr,
                       uint16_t *output_ptr) {
 ...
 */
static void handle_worker_shutdown(struct rte_distributor *d,
                                   unsigned int wkr) {
  ...
  if (!(__atomic_load_n(&(buf->bufptr64[0]), __ATOMIC_ACQUIRE) &
    ...
    for (i = 0; i < RTE_DIST_BURST_SIZE; i++)
      if (buf->bufptr64[i] & RTE_DISTRIB_VALID_BUF)
        pkts[pkts_count++] =
            (void *)((uintptr_t)(buf->bufptr64[i] >> RTE_DISTRIB_FLAG_BITS));
   ...
   */
  __atomic_store_n(&(buf->bufptr64[0]), RTE_DISTRIB_GET_BUF, __ATOMIC_RELEASE);

  /* Collect backlog packets from worker */
  for (i = 0; i < d->backlog[wkr].count; i++)
    pkts[pkts_count++] =
        (void *)((uintptr_t)(d->backlog[wkr].pkts[i] >> RTE_DISTRIB_FLAG_BITS));
 ...
 */
static unsigned int handle_returns(struct rte_distributor *d,
                                   unsigned int wkr) {
  struct rte_distributor_buffer *buf = &(d->bufs[wkr]);
  uintptr_t oldbuf;
  unsigned int ret_start = d->returns.start, ret_count = d->returns.count;
  ...
  if (__atomic_load_n(&(buf->retptr64[0]), __ATOMIC_ACQUIRE) &
    ...
    for (i = 0; i < RTE_DIST_BURST_SIZE; i++) {
      if (buf->retptr64[i] & RTE_DISTRIB_VALID_BUF) {
        oldbuf = ((uintptr_t)(buf->retptr64[i] >> RTE_DISTRIB_FLAG_BITS));
        /* store returns in a circular buffer */
 ...
 */
static unsigned int release(struct rte_distributor *d, unsigned int wkr) {
  struct rte_distributor_buffer *buf = &(d->bufs[wkr]);
  unsigned int i;
  ...
  for (i = 0; i < d->backlog[wkr].count; i++) {
    d->bufs[wkr].bufptr64[i] =
        d->backlog[wkr].pkts[i] | RTE_DISTRIB_GET_BUF | RTE_DISTRIB_VALID_BUF;
...
int rte_distributor_process(struct rte_distributor *d, struct rte_mbuf **mbufs,
  ...
  if (d->alg_type == RTE_DIST_ALG_SINGLE) {
    /* Call the old API */
    return rte_distributor_process_single(d->d_single, mbufs, num_mbufs);
  }
  ...
  if (unlikely(num_mbufs == 0)) {
    ...
    for (wid = 0; wid < d->num_workers; wid++) {
      /* Sync with worker on GET_BUF flag. */
      if (__atomic_load_n(&(d->bufs[wid].bufptr64[0]), __ATOMIC_ACQUIRE) &
          RTE_DISTRIB_GET_BUF) {
        d->bufs[wid].count = 0;
        release(d, wid);
        handle_returns(d, wid);
  ...
  while (next_idx < num_mbufs) {
    ...
    for (j = 0; j < pkts; j++) {
      ...
      if (matches[j] && d->active[matches[j] - 1]) {
        struct rte_distributor_backlog *bl = &d->backlog[matches[j] - 1];
...
int rte_distributor_returned_pkts(struct rte_distributor *d,
  ...
  unsigned int retval =
      (max_mbufs < returns->count) ? max_mbufs : returns->count;
  unsigned int i;

  if (d->alg_type == RTE_DIST_ALG_SINGLE) {
    /* Call the old API */
    return rte_distributor_returned_pkts_single(d->d_single, mbufs, max_mbufs);
  }

  for (i = 0; i < retval; i++) {
    unsigned int idx = (returns->start + i) & RTE_DISTRIB_RETURNS_MASK;
 ...
 */
static inline unsigned int total_outstanding(const struct rte_distributor *d) {
...
void rte_distributor_clear_returns(struct rte_distributor *d) {
  ...
  for (wkr = 0; wkr < d->num_workers; wkr++)
    ...
    __atomic_store_n(&(d->bufs[wkr].retptr64[0]), 0, __ATOMIC_RELEASE);
...
struct rte_distributor *rte_distributor_create(const char *name,
  ...
  if (alg_type == RTE_DIST_ALG_SINGLE) {
    ...
    d->d_single = rte_distributor_create_single(name, socket_id, num_workers);
    if (d->d_single == NULL) {
      free(d);
  ...
  dist_burst_list =
      RTE_TAILQ_CAST(rte_dist_burst_tailq.head, rte_dist_burst_list);
