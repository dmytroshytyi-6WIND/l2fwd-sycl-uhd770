...
void find_match_vec(struct rte_distributor *d, uint16_t *data_ptr,
  ...
  for (i = 0; i < d->num_workers; i++) {
    ...
    inflight_fids = _mm_load_si128((__m128i *)&(d->in_flight_tags[i]));
    preflight_fids = _mm_load_si128((__m128i *)(bl->tags));
    ...
    mask1 =
        _mm_cmpestrm(inflight_fids, 8, incoming_fids, 8,
                     _SIDD_UWORD_OPS | _SIDD_CMP_EQUAL_ANY | _SIDD_UNIT_MASK);
    mask2 =
        _mm_cmpestrm(preflight_fids, 8, incoming_fids, 8,
                     _SIDD_UWORD_OPS | _SIDD_CMP_EQUAL_ANY | _SIDD_UNIT_MASK);
