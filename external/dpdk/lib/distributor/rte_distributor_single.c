...
void rte_distributor_request_pkt_single(struct rte_distributor_single *d,
                                        unsigned worker_id,
                                        struct rte_mbuf *oldpkt) {
  union rte_distributor_buffer_single *buf = &d->bufs[worker_id];
  int64_t req = (((int64_t)(uintptr_t)oldpkt) << RTE_DISTRIB_FLAG_BITS) |
                RTE_DISTRIB_GET_BUF;
  RTE_WAIT_UNTIL_MASKED(&buf->bufptr64, RTE_DISTRIB_FLAGS_MASK, ==, 0,
                        __ATOMIC_RELAXED);
...
rte_distributor_poll_pkt_single(struct rte_distributor_single *d,
  ...
  if (__atomic_load_n(&buf->bufptr64, __ATOMIC_ACQUIRE) & RTE_DISTRIB_GET_BUF)
    return NULL;
...
int rte_distributor_return_pkt_single(struct rte_distributor_single *d,
                                      unsigned worker_id,
                                      struct rte_mbuf *oldpkt) {
  union rte_distributor_buffer_single *buf = &d->bufs[worker_id];
...
static int add_to_backlog(struct rte_distributor_backlog *bl, int64_t item) {
...
static int64_t backlog_pop(struct rte_distributor_backlog *bl) {
  bl->count--;
  return bl->pkts[bl->start++ & RTE_DISTRIB_BACKLOG_MASK];
}

/* stores a packet returned from a worker inside the returns array */
static inline void store_return(uintptr_t oldbuf,
...
static inline void handle_worker_shutdown(struct rte_distributor_single *d,
                                          unsigned int wkr) {
  ...
  if (unlikely(d->backlog[wkr].count != 0)) {
    ...
    for (i = 0; i < bl->count; i++) {
      ...
      pkts[i] = (void *)((uintptr_t)(bl->pkts[idx] >> RTE_DISTRIB_FLAG_BITS));
    }
    /* recursive call.
 ...
 */
static int process_returns(struct rte_distributor_single *d) {
  unsigned wkr;
  unsigned flushed = 0;
  unsigned ret_start = d->returns.start, ret_count = d->returns.count;

  for (wkr = 0; wkr < d->num_workers; wkr++) {
    uintptr_t oldbuf = 0;
    /* Sync with worker. Acquire bufptr64. */
    const int64_t data =
        __atomic_load_n(&(d->bufs[wkr].bufptr64), __ATOMIC_ACQUIRE);
    ...
    if (data & RTE_DISTRIB_GET_BUF) {
      ...
      if (d->backlog[wkr].count)
        ...
        __atomic_store_n(&(d->bufs[wkr].bufptr64),
                         backlog_pop(&d->backlog[wkr]), __ATOMIC_RELEASE);
      else {
        /* Sync with worker on GET_BUF flag. */
        __atomic_store_n(&(d->bufs[wkr].bufptr64), RTE_DISTRIB_GET_BUF,
                         __ATOMIC_RELEASE);
        d->in_flight_tags[wkr] = 0;
        d->in_flight_bitmask &= ~(1UL << wkr);
      }
...
int rte_distributor_process_single(struct rte_distributor_single *d,
  ...
  while (next_idx < num_mbufs || next_mb != NULL) {
    uintptr_t oldbuf = 0;
    /* Sync with worker. Acquire bufptr64. */
    int64_t data = __atomic_load_n(&(d->bufs[wkr].bufptr64), __ATOMIC_ACQUIRE);
    ...
    if ((data & RTE_DISTRIB_GET_BUF) && (d->backlog[wkr].count || next_mb)) {
      ...
      if (d->backlog[wkr].count)
        ...
        /* Sync with worker. Release bufptr64.  */
        __atomic_store_n(&(d->bufs[wkr].bufptr64), next_value,
                         __ATOMIC_RELEASE);
  ...
  for (wkr = 0; wkr < d->num_workers; wkr++)
    if (d->backlog[wkr].count &&
      ...
      int64_t oldbuf = d->bufs[wkr].bufptr64 >> RTE_DISTRIB_FLAG_BITS;

      store_return(oldbuf, d, &ret_start, &ret_count);

      /* Sync with worker. Release bufptr64. */
      __atomic_store_n(&(d->bufs[wkr].bufptr64), backlog_pop(&d->backlog[wkr]),
                       __ATOMIC_RELEASE);
    }
...
int rte_distributor_returned_pkts_single(struct rte_distributor_single *d,
                                         struct rte_mbuf **mbufs,
                                         unsigned max_mbufs) {
...
rte_distributor_create_single(const char *name, unsigned socket_id,
  ...
  distributor_list =
      RTE_TAILQ_CAST(rte_distributor_tailq.head, rte_distributor_list);
