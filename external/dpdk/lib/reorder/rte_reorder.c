...
static void rte_reorder_free_mbufs(struct rte_reorder_buffer *b);

struct rte_reorder_buffer *rte_reorder_init(struct rte_reorder_buffer *b,
                                            unsigned int bufsize,
                                            const char *name,
                                            unsigned int size) {
  const unsigned int min_bufsize =
      sizeof(*b) + (2 * size * sizeof(struct rte_mbuf *));
  ...
  b->order_buf.entries =
      RTE_PTR_ADD(&b[1], size * sizeof(b->ready_buf.entries[0]));
...
static unsigned rte_reorder_fill_overflow(struct rte_reorder_buffer *b,
                                          unsigned n) {
  /*
  ...
  struct cir_buffer *order_buf = &b->order_buf, *ready_buf = &b->ready_buf;
  ...
  while (order_head_adv < n &&
    ...
    while (order_buf->entries[order_buf->head] != NULL) {
      ready_buf->entries[ready_buf->head] = order_buf->entries[order_buf->head];
