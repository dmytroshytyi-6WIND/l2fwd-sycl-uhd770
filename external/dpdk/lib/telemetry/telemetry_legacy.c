...
static int register_client(const char *cmd __rte_unused,
                           const char *params __rte_unused, char *buffer,
                           int buf_len);
...
int rte_telemetry_legacy_register(const char *cmd,
  ...
  snprintf(callbacks[num_legacy_callbacks].cmd, MAX_LEN, "\"command\":\"%s\"",
           cmd);
  snprintf(callbacks[num_legacy_callbacks].data, MAX_LEN,
           data_req ? "%s{\"" : "%snull", DATA_REQ_LABEL);
...
static int register_client(const char *cmd __rte_unused, const char *params,
                           char *buffer __rte_unused,
                           int buf_len __rte_unused) {
#ifndef RTE_EXEC_ENV_WINDOWS
  pthread_t th;
  char data[BUF_SIZE];
  int fd;
  ...
  *strchr(data, '\"') = 0;
  ...
  rc = pthread_create(&th, NULL, &legacy_client_handler, (void *)(uintptr_t)fd);
  if (rc != 0) {
...
static int send_error_response(int s, int err) {
  const char *desc;
  char out_buf[100000];
  ...
  int used = snprintf(out_buf, sizeof(out_buf),
                      "{\"status_code\": "
                      "\"Status Error: %s\", \"data\": null}",
                      desc);
...
static void perform_command(telemetry_legacy_cb fn, const char *param, int s) {
  char out_buf[100000];
  int ret, used = 0;
...
static int parse_client_request(char *buffer, int buf_len, int s) {
  int i;
  ...
  for (i = 0; i < num_legacy_callbacks; i++) {
    ...
    if (!(strchr(valid_sep, action_sep) && strchr(valid_sep, cmd_sep)))
      return -EPERM;
    char data_sep;
