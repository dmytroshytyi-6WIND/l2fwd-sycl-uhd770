...
#ifndef RTE_EXEC_ENV_WINDOWS
static void *client_handler(void *socket);
#endif /* !RTE_EXEC_ENV_WINDOWS */
...
static int list_commands(const char *cmd __rte_unused,
...
static int json_info(const char *cmd __rte_unused,
...
static int command_help(const char *cmd __rte_unused, const char *params,
                        struct rte_tel_data *d) {
  int i;
...
static int container_to_json(const struct rte_tel_data *d, char *out_buf,
                             size_t buf_len) {
  size_t used = 0;
  unsigned int i;
  ...
  if (d->type == RTE_TEL_ARRAY_U64)
    for (i = 0; i < d->data_len; i++)
      used = rte_tel_json_add_array_u64(out_buf, buf_len, used,
                                        d->data.array[i].u64val);
  if (d->type == RTE_TEL_ARRAY_INT)
    for (i = 0; i < d->data_len; i++)
      used = rte_tel_json_add_array_int(out_buf, buf_len, used,
                                        d->data.array[i].ival);
  if (d->type == RTE_TEL_ARRAY_STRING)
    for (i = 0; i < d->data_len; i++)
      used = rte_tel_json_add_array_string(out_buf, buf_len, used,
                                           d->data.array[i].sval);
  if (d->type == RTE_TEL_DICT)
    for (i = 0; i < d->data_len; i++) {
      ...
      case RTE_TEL_STRING_VAL:
        used = rte_tel_json_add_obj_str(out_buf, buf_len, used, v->name,
                                        v->value.sval);
        break;
      case RTE_TEL_INT_VAL:
        used = rte_tel_json_add_obj_int(out_buf, buf_len, used, v->name,
                                        v->value.ival);
        break;
      case RTE_TEL_U64_VAL:
        used = rte_tel_json_add_obj_u64(out_buf, buf_len, used, v->name,
                                        v->value.u64val);
        break;
      case RTE_TEL_CONTAINER: {
        char temp[buf_len];
        const struct container *cont = &v->value.container;
        if (container_to_json(cont->data, temp, buf_len) != 0)
          used =
              rte_tel_json_add_obj_json(out_buf, buf_len, used, v->name, temp);
        if (!cont->keep)
...
static void output_json(const char *cmd, const struct rte_tel_data *d, int s) {
  ...
  RTE_BUILD_BUG_ON(sizeof(out_buf) <
                   MAX_CMD_LEN + RTE_TEL_MAX_SINGLE_STRING_LEN + 10);
  switch (d->type) {
  case RTE_TEL_NULL:
  ...
  case RTE_TEL_STRING:
    used =
        snprintf(out_buf, sizeof(out_buf), "{\"%.*s\":\"%.*s\"}", MAX_CMD_LEN,
                 cmd, RTE_TEL_MAX_SINGLE_STRING_LEN, d->data.str);
    break;
  case RTE_TEL_DICT:
    prefix_used =
        snprintf(out_buf, sizeof(out_buf), "{\"%.*s\":", MAX_CMD_LEN, cmd);
    ...
    for (i = 0; i < d->data_len; i++) {
      ...
      switch (v->type) {
      case RTE_TEL_STRING_VAL:
        used = rte_tel_json_add_obj_str(cb_data_buf, buf_len, used, v->name,
                                        v->value.sval);
        break;
      case RTE_TEL_INT_VAL:
        used = rte_tel_json_add_obj_int(cb_data_buf, buf_len, used, v->name,
                                        v->value.ival);
        break;
      case RTE_TEL_U64_VAL:
        used = rte_tel_json_add_obj_u64(cb_data_buf, buf_len, used, v->name,
                                        v->value.u64val);
        break;
      case RTE_TEL_CONTAINER: {
        char temp[buf_len];
        const struct container *cont = &v->value.container;
        if (container_to_json(cont->data, temp, buf_len) != 0)
          used = rte_tel_json_add_obj_json(cb_data_buf, buf_len, used, v->name,
                                           temp);
        if (!cont->keep)
          rte_tel_data_free(cont->data);
      }
  ...
  case RTE_TEL_ARRAY_CONTAINER:
    ...
    for (i = 0; i < d->data_len; i++)
      if (d->type == RTE_TEL_ARRAY_STRING)
        used = rte_tel_json_add_array_string(cb_data_buf, buf_len, used,
                                             d->data.array[i].sval);
      else if (d->type == RTE_TEL_ARRAY_INT)
        used = rte_tel_json_add_array_int(cb_data_buf, buf_len, used,
                                          d->data.array[i].ival);
      else if (d->type == RTE_TEL_ARRAY_U64)
        used = rte_tel_json_add_array_u64(cb_data_buf, buf_len, used,
                                          d->data.array[i].u64val);
      else if (d->type == RTE_TEL_ARRAY_CONTAINER) {
        char temp[buf_len];
        const struct container *rec_data = &d->data.array[i].container;
        if (container_to_json(rec_data->data, temp, buf_len) != 0)
          used = rte_tel_json_add_array_json(cb_data_buf, buf_len, used, temp);
        if (!rec_data->keep)
...
static void perform_command(telemetry_cb fn, const char *cmd, const char *param,
                            int s) {
...
static int unknown_command(const char *cmd __rte_unused,
                           const char *params __rte_unused,
                           struct rte_tel_data *d) {
...
static inline char *get_socket_path(const char *runtime_dir,
                                    const int version) {
...
static void set_thread_name(pthread_t id __rte_unused,
...
static int telemetry_legacy_init(void) {
  pthread_t t_old;
  int rc;
  ...
  if ((size_t)snprintf(v1_socket.path, sizeof(v1_socket.path), "%s/telemetry",
                       socket_dir) >= sizeof(v1_socket.path)) {
...
static int telemetry_v2_init(void) {
  ...
  while (v2_socket.sock < 0) {
    ...
    if (snprintf(v2_socket.path, sizeof(v2_socket.path), "%s:%d", spath,
                 ++suffix) >= (int)sizeof(v2_socket.path)) {
      TMTY_LOG(ERR, "Error with socket binding, path too long\n");
      return -1;
    }
    v2_socket.sock = create_socket(v2_socket.path);
...
int32_t rte_telemetry_init(const char *runtime_dir, const char *rte_version,
                           rte_cpuset_t *cpuset, rte_log_fn log_fn,
                           uint32_t registered_logtype) {
