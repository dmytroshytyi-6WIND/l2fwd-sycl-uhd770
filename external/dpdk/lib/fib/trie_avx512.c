...
transpose_x16(uint8_t ips[16][RTE_FIB6_IPV6_ADDR_SIZE], __m512i *first,
              __m512i *second, __m512i *third, __m512i *fourth) {
  __m512i tmp1, tmp2, tmp3, tmp4;
  __m512i tmp5, tmp6, tmp7, tmp8;
  const __rte_x86_zmm_t perm_idxes = {
      .u32 = {0, 4, 8, 12, 2, 6, 10, 14, 1, 5, 9, 13, 3, 7, 11, 15},
  };
...
static __rte_always_inline void
transpose_x8(uint8_t ips[8][RTE_FIB6_IPV6_ADDR_SIZE], __m512i *first,
             __m512i *second) {
  __m512i tmp1, tmp2, tmp3, tmp4;
  const __rte_x86_zmm_t perm_idxes = {
      .u64 = {0, 2, 4, 6, 1, 3, 5, 7},
...
trie_vec_lookup_x16x2(void *p, uint8_t ips[32][RTE_FIB6_IPV6_ADDR_SIZE],
  ...
  const __rte_x86_zmm_t bswap = {
      .u8 = {2, 1, 0, 255, 6, 5, 4, 255, 10, 9, 8, 255, 14, 13, 12, 255,
  ...
  if (size == sizeof(uint16_t)) {
    ...
    res_2 = _mm512_i32gather_epi32(idxes_2, (const int *)dp->tbl24, 2);
    res_1 = _mm512_and_epi32(res_1, res_msk);
    res_2 = _mm512_and_epi32(res_2, res_msk);
  } else {
    res_1 = _mm512_i32gather_epi32(idxes_1, (const int *)dp->tbl24, 4);
    res_2 = _mm512_i32gather_epi32(idxes_2, (const int *)dp->tbl24, 4);
  }
  ...
  shuf_idxes = _mm512_setr_epi32(3, 7, 11, 15, 19, 23, 27, 31, 35, 39, 43, 47,
                                 51, 55, 59, 63);

  base_idxes = _mm512_setr_epi32(0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44,
                                 48, 52, 56, 60);
  ...
  while (msk_ext_1 || msk_ext_2) {
    ...
    byte_chunk_1 = (i < 8) ? ((i >= 4) ? second_1 : first_1)
                           : ((i >= 12) ? fourth_1 : third_1);
    byte_chunk_2 = (i < 8) ? ((i >= 4) ? second_2 : first_2)
                           : ((i >= 12) ? fourth_2 : third_2);
    bytes_1 = _mm512_maskz_shuffle_epi8(k, byte_chunk_1, shuf_idxes);
    bytes_2 = _mm512_maskz_shuffle_epi8(k, byte_chunk_2, shuf_idxes);
    idxes_1 = _mm512_maskz_add_epi32(msk_ext_1, idxes_1, bytes_1);
    idxes_2 = _mm512_maskz_add_epi32(msk_ext_2, idxes_2, bytes_2);
    if (size == sizeof(uint16_t)) {
      tmp_1 = _mm512_mask_i32gather_epi32(zero, msk_ext_1, idxes_1,
                                          (const int *)dp->tbl8, 2);
      tmp_2 = _mm512_mask_i32gather_epi32(zero, msk_ext_2, idxes_2,
                                          (const int *)dp->tbl8, 2);
    ...
    res_1 = _mm512_mask_blend_epi32(msk_ext_1 ^ new_msk_1, res_1, tmp_1);
    res_2 = _mm512_mask_blend_epi32(msk_ext_2 ^ new_msk_2, res_2, tmp_2);
  ...
  tmp2_1 = _mm512_maskz_expand_epi32(exp_msk, _mm512_castsi256_si512(tmp256_1));
  tmp2_2 = _mm512_maskz_expand_epi32(exp_msk, _mm512_castsi256_si512(tmp256_2));
...
static void trie_vec_lookup_x8x2_8b(void *p,
  ...
  const __rte_x86_zmm_t bswap = {
      .u8 = {2,   1,   0,   255, 255, 255, 255, 255, 10,  9,   8,   255, 255,
             255, 255, 255, 2,   1,   0,   255, 255, 255, 255, 255, 10,  9,
             8,   255, 255, 255, 255, 255, 2,   1,   0,   255, 255, 255, 255,
             255, 10,  9,   8,   255, 255, 255, 255, 255, 2,   1,   0,   255,
             255, 255, 255, 255, 10,  9,   8,   255, 255, 255, 255, 255},
  };
  ...
  while (msk_ext_1 || msk_ext_2) {
    ...
    tmp_1 = _mm512_mask_i64gather_epi64(zero, msk_ext_1, idxes_1,
                                        (const void *)dp->tbl8, 8);
    tmp_2 = _mm512_mask_i64gather_epi64(zero, msk_ext_2, idxes_2,
                                        (const void *)dp->tbl8, 8);
    new_msk_1 = _mm512_test_epi64_mask(tmp_1, lsb);
    new_msk_2 = _mm512_test_epi64_mask(tmp_2, lsb);
    res_1 = _mm512_mask_blend_epi64(msk_ext_1 ^ new_msk_1, res_1, tmp_1);
    res_2 = _mm512_mask_blend_epi64(msk_ext_2 ^ new_msk_2, res_2, tmp_2);
...
void rte_trie_vec_lookup_bulk_2b(void *p,
...
void rte_trie_vec_lookup_bulk_4b(void *p,
                                 uint8_t ips[][RTE_FIB6_IPV6_ADDR_SIZE],
                                 uint64_t *next_hops, const unsigned int n) {
  uint32_t i;
  for (i = 0; i < (n / 32); i++) {
...
void rte_trie_vec_lookup_bulk_8b(void *p,
                                 uint8_t ips[][RTE_FIB6_IPV6_ADDR_SIZE],
                                 uint64_t *next_hops, const unsigned int n) {
  uint32_t i;
  for (i = 0; i < (n / 16); i++) {
