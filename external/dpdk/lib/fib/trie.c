...
rte_fib6_lookup_fn_t trie_get_lookup_fn(void *p,
...
static void write_to_dp(void *ptr, uint64_t val, enum rte_fib_trie_nh_sz size,
                        int n) {
  int i;
...
static void tbl8_pool_init(struct rte_trie_tbl *dp) {
  uint32_t i;
 ...
 */
static inline int32_t tbl8_get(struct rte_trie_tbl *dp) {
 ...
 */
static inline void tbl8_put(struct rte_trie_tbl *dp, uint32_t tbl8_ind) {
  dp->tbl8_pool[--dp->tbl8_pool_pos] = tbl8_ind;
}

static int tbl8_alloc(struct rte_trie_tbl *dp, uint64_t nh) {
  int64_t tbl8_idx;
  uint8_t *tbl8_ptr;
  ...
  if (tbl8_idx < 0)
    return tbl8_idx;
  tbl8_ptr =
      get_tbl_p_by_idx(dp->tbl8, tbl8_idx * TRIE_TBL8_GRP_NUM_ENT, dp->nh_sz);
  /*Init tbl8 entries with nexthop from tbl24*/
  write_to_dp((void *)tbl8_ptr, nh, dp->nh_sz, TRIE_TBL8_GRP_NUM_ENT);
  return tbl8_idx;
}

static void tbl8_recycle(struct rte_trie_tbl *dp, void *par,
                         uint64_t tbl8_idx) {
  uint32_t i;
  uint64_t nh;
  uint16_t *ptr16;
...
#define BYTE_SIZE 8
static inline uint32_t get_idx(const uint8_t *ip, uint32_t prev_idx, int bytes,
                               int first_byte) {
  int i;
  uint32_t idx = 0;
  uint8_t bitshift;
...
static inline uint64_t get_val_by_p(void *p, uint8_t nh_sz) {
  uint64_t val = 0;
 ...
 */
static void recycle_root_path(struct rte_trie_tbl *dp, const uint8_t *ip_part,
  ...
  if (common_tbl8 != 0) {
    p = get_tbl_p_by_idx(
        dp->tbl8, (val >> 1) * TRIE_TBL8_GRP_NUM_ENT + *ip_part, dp->nh_sz);
    recycle_root_path(dp, ip_part + 1, common_tbl8 - 1, p);
  }
  tbl8_recycle(dp, prev, val >> 1);
}

static inline int build_common_root(struct rte_trie_tbl *dp, const uint8_t *ip,
...
static int write_edge(struct rte_trie_tbl *dp, const uint8_t *ip_part,
                      uint64_t next_hop, int len, enum edge edge, void *ent) {
  ...
  if (len != 0) {
    ...
    p = get_tbl_p_by_idx(
        dp->tbl8, (tbl8_idx * TRIE_TBL8_GRP_NUM_ENT) + *ip_part, dp->nh_sz);
    ...
    if (edge == LEDGE) {
      ...
      write_to_dp(get_tbl_p_by_idx(dp->tbl8, tbl8_idx * TRIE_TBL8_GRP_NUM_ENT,
                                   dp->nh_sz),
...
static int install_to_dp(struct rte_trie_tbl *dp, const uint8_t *ledge,
                         const uint8_t *r, uint64_t next_hop) {
  ...
  ret = write_edge(dp, &ledge[first_tbl8_byte + !(common_bytes < 3)], next_hop,
                   llen, LEDGE, ent);
  if (ret < 0)
    return ret;
  ...
  uint8_t common_tbl8 =
      (common_bytes < TBL24_BYTES) ? 0 : common_bytes - (TBL24_BYTES - 1);
  ent = get_tbl24_p(dp, ledge, dp->nh_sz);
  recycle_root_path(dp, ledge + TBL24_BYTES, common_tbl8, ent);
  return 0;
}

static void get_nxt_net(uint8_t *ip, uint8_t depth) {
  int i;
  uint8_t part_depth;
  uint8_t prev_byte;
...
static int modify_dp(struct rte_trie_tbl *dp, struct rte_rib6 *rib,
  ...
  do {
    tmp = rte_rib6_get_nxt(rib, ip, depth, tmp, RTE_RIB6_GET_NXT_COVER);
    if (tmp != NULL) {
...
int trie_modify(struct rte_fib6 *fib, const uint8_t ip[RTE_FIB6_IPV6_ADDR_SIZE],
  ...
  if (depth > 24) {
    tmp = rte_rib6_get_nxt(rib, ip_masked, RTE_ALIGN_FLOOR(depth, 8), NULL,
    ...
    if (tmp == NULL) {
      ...
      depth_diff = RTE_ALIGN_CEIL(depth, 8) - RTE_ALIGN_CEIL(parent_depth, 8);
...
void *trie_create(const char *name, int socket_id, struct rte_fib6_conf *conf) {
  ...
  if ((name == NULL) || (conf == NULL) ||
      ...
      (conf->trie.num_tbl8 > get_max_nh(conf->trie.nh_sz)) ||
  ...
  dp = rte_zmalloc_socket(
      name, sizeof(struct rte_trie_tbl) + TRIE_TBL24_NUM_ENT * (1 << nh_sz),
  ...
  dp->tbl8 = rte_zmalloc_socket(
      mem_name, TRIE_TBL8_GRP_NUM_ENT * (1ll << nh_sz) * (num_tbl8 + 1),
      RTE_CACHE_LINE_SIZE, socket_id);
  if (dp->tbl8 == NULL) {
    rte_errno = ENOMEM;
    rte_free(dp);
  ...
  dp->tbl8_pool =
      rte_zmalloc_socket(mem_name, sizeof(uint32_t) * dp->number_tbl8s,
                         RTE_CACHE_LINE_SIZE, socket_id);
