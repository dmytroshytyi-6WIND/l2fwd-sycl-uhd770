...
static __rte_always_inline void dir24_8_vec_lookup_x16(void *p,
                                                       const uint32_t *ips,
                                                       uint64_t *next_hops,
                                                       int size) {
  ...
  if (msk_ext != 0) {
    ...
    if (size == sizeof(uint8_t)) {
      idxes = _mm512_mask_i32gather_epi32(zero, msk_ext, idxes,
                                          (const int *)dp->tbl8, 1);
  ...
  tmp2 = _mm512_maskz_expand_epi32(exp_msk, _mm512_castsi256_si512(tmp256));
  _mm512_storeu_si512(next_hops, tmp1);
  _mm512_storeu_si512(next_hops + 8, tmp2);
}

static __rte_always_inline void
dir24_8_vec_lookup_x8_8b(void *p, const uint32_t *ips, uint64_t *next_hops) {
  struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
  const __m512i zero = _mm512_set1_epi32(0);
...
void rte_dir24_8_vec_lookup_bulk_1b(void *p, const uint32_t *ips,
  ...
  dir24_8_lookup_bulk_1b(p, ips + i * 16, next_hops + i * 16, n - i * 16);
}

void rte_dir24_8_vec_lookup_bulk_2b(void *p, const uint32_t *ips,
                                    uint64_t *next_hops, const unsigned int n) {
  uint32_t i;
  for (i = 0; i < (n / 16); i++)
    dir24_8_vec_lookup_x16(p, ips + i * 16, next_hops + i * 16,
  ...
  dir24_8_lookup_bulk_2b(p, ips + i * 16, next_hops + i * 16, n - i * 16);
}

void rte_dir24_8_vec_lookup_bulk_4b(void *p, const uint32_t *ips,
                                    uint64_t *next_hops, const unsigned int n) {
  uint32_t i;
  for (i = 0; i < (n / 16); i++)
    dir24_8_vec_lookup_x16(p, ips + i * 16, next_hops + i * 16,
  ...
  dir24_8_lookup_bulk_4b(p, ips + i * 16, next_hops + i * 16, n - i * 16);
}

void rte_dir24_8_vec_lookup_bulk_8b(void *p, const uint32_t *ips,
                                    uint64_t *next_hops, const unsigned int n) {
  uint32_t i;
  for (i = 0; i < (n / 8); i++)
