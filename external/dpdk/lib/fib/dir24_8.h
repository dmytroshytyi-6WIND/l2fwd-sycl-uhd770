...
static inline void *get_tbl24_p(struct dir24_8_tbl *dp, uint32_t ip,
                                uint8_t nh_sz) {
  return (void *)&(
      (uint8_t *)dp->tbl24)[(ip & DIR24_8_TBL24_MASK) >> (8 - nh_sz)];
}

static inline uint8_t bits_in_nh(uint8_t nh_sz) { return 8 * (1 << nh_sz); }

static inline uint64_t get_max_nh(uint8_t nh_sz) {
  return ((1ULL << (bits_in_nh(nh_sz) - 1)) - 1);
}

static inline uint32_t get_tbl24_idx(uint32_t ip) { return ip >> 8; }

static inline uint32_t get_tbl8_idx(uint32_t res, uint32_t ip) {
  return (res >> 1) * DIR24_8_TBL8_GRP_NUM_ENT + (uint8_t)ip;
}

static inline uint64_t lookup_msk(uint8_t nh_sz) {
  return ((1ULL << ((1 << (nh_sz + 3)) - 1)) << 1) - 1;
}

static inline uint8_t get_psd_idx(uint32_t val, uint8_t nh_sz) {
  return val & ((1 << (3 - nh_sz)) - 1);
}

static inline uint32_t get_tbl_idx(uint32_t val, uint8_t nh_sz) {
  return val >> (3 - nh_sz);
}

static inline uint64_t get_tbl24(struct dir24_8_tbl *dp, uint32_t ip,
                                 uint8_t nh_sz) {
  return ((dp->tbl24[get_tbl_idx(get_tbl24_idx(ip), nh_sz)] >>
           (get_psd_idx(get_tbl24_idx(ip), nh_sz) * bits_in_nh(nh_sz))) &
          lookup_msk(nh_sz));
}

static inline uint64_t get_tbl8(struct dir24_8_tbl *dp, uint32_t res,
                                uint32_t ip, uint8_t nh_sz) {
  return ((dp->tbl8[get_tbl_idx(get_tbl8_idx(res, ip), nh_sz)] >>
           (get_psd_idx(get_tbl8_idx(res, ip), nh_sz) * bits_in_nh(nh_sz))) &
          lookup_msk(nh_sz));
}

static inline int is_entry_extended(uint64_t ent) {
  return (ent & DIR24_8_EXT_ENT) == DIR24_8_EXT_ENT;
}

#define LOOKUP_FUNC(suffix, type, bulk_prefetch, nh_sz)                        \
  static inline void dir24_8_lookup_bulk_##suffix(                             \
      void *p, const uint32_t *ips, uint64_t *next_hops,                       \
    ...
    for (i = 0; i < (n - prefetch_offset); i++) {                              \
      rte_prefetch0(get_tbl24_p(dp, ips[i + prefetch_offset], nh_sz));         \
...
static inline void dir24_8_lookup_bulk(struct dir24_8_tbl *dp,
                                       const uint32_t *ips, uint64_t *next_hops,
                                       const unsigned int n, uint8_t nh_sz) {
  uint64_t tmp;
  uint32_t i;
...
static inline void dir24_8_lookup_bulk_0(void *p, const uint32_t *ips,
                                         uint64_t *next_hops,
                                         const unsigned int n) {
  struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
...
static inline void dir24_8_lookup_bulk_1(void *p, const uint32_t *ips,
                                         uint64_t *next_hops,
                                         const unsigned int n) {
  struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
...
static inline void dir24_8_lookup_bulk_2(void *p, const uint32_t *ips,
                                         uint64_t *next_hops,
                                         const unsigned int n) {
  struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
...
static inline void dir24_8_lookup_bulk_3(void *p, const uint32_t *ips,
                                         uint64_t *next_hops,
                                         const unsigned int n) {
  struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
...
static inline void dir24_8_lookup_bulk_uni(void *p, const uint32_t *ips,
                                           uint64_t *next_hops,
                                           const unsigned int n) {
  struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
  uint64_t tmp;
...
void *dir24_8_create(const char *name, int socket_id,
                     struct rte_fib_conf *conf);

void dir24_8_free(void *p);

rte_fib_lookup_fn_t dir24_8_get_lookup_fn(void *p,
                                          enum rte_fib_lookup_type type);

int dir24_8_modify(struct rte_fib *fib, uint32_t ip, uint8_t depth,
