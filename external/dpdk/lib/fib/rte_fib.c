...
static void dummy_lookup(void *fib_p, const uint32_t *ips, uint64_t *next_hops,
...
static int dummy_modify(struct rte_fib *fib, uint32_t ip, uint8_t depth,
...
static int init_dataplane(struct rte_fib *fib, __rte_unused int socket_id,
  ...
  case RTE_FIB_DIR24_8:
    ...
    if (fib->dp == NULL)
      return -rte_errno;
    fib->lookup = dir24_8_get_lookup_fn(fib->dp, RTE_FIB_LOOKUP_DEFAULT);
...
int rte_fib_add(struct rte_fib *fib, uint32_t ip, uint8_t depth,
                uint64_t next_hop) {
  if ((fib == NULL) || (fib->modify == NULL) || (depth > RTE_FIB_MAXDEPTH))
    return -EINVAL;
...
int rte_fib_lookup_bulk(struct rte_fib *fib, uint32_t *ips, uint64_t *next_hops,
                        int n) {
  FIB_RETURN_IF_TRUE(((fib == NULL) || (ips == NULL) || (next_hops == NULL) ||
                      (fib->lookup == NULL)),
                     -EINVAL);
...
struct rte_fib *rte_fib_create(const char *name, int socket_id,
                               struct rte_fib_conf *conf) {
  ...
  if (rib == NULL) {
    RTE_LOG(ERR, LPM, "Can not allocate RIB %s\n", name);
    return NULL;
  }
  ...
  if (te == NULL) {
    RTE_LOG(ERR, LPM, "Can not allocate tailq entry for FIB %s\n", name);
    rte_errno = ENOMEM;
    goto exit;
  }

  /* Allocate memory to store the FIB data structures. */
  fib = rte_zmalloc_socket(mem_name, sizeof(struct rte_fib),
...
static void free_dataplane(struct rte_fib *fib) {
  switch (fib->type) {
  case RTE_FIB_DUMMY:
    return;
  case RTE_FIB_DIR24_8:
