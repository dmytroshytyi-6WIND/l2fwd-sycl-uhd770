...
static void write_to_fib(void *ptr, uint64_t val,
                         enum rte_fib_dir24_8_nh_sz size, int n) {
  int i;
...
static int tbl8_get_idx(struct dir24_8_tbl *dp) {
  uint32_t i;
  int bit_idx;

  for (i = 0; (i < (dp->number_tbl8s >> BITMAP_SLAB_BIT_SIZE_LOG2)) &&
              (dp->tbl8_idxes[i] == UINT64_MAX);
       i++)
    ;
...
static inline void tbl8_free_idx(struct dir24_8_tbl *dp, int idx) {
  dp->tbl8_idxes[idx >> BITMAP_SLAB_BIT_SIZE_LOG2] &=
      ~(1ULL << (idx & BITMAP_SLAB_BITMASK));
}

static int tbl8_alloc(struct dir24_8_tbl *dp, uint64_t nh) {
  int64_t tbl8_idx;
  uint8_t *tbl8_ptr;
  ...
  write_to_fib((void *)tbl8_ptr, nh | DIR24_8_EXT_ENT, dp->nh_sz,
               DIR24_8_TBL8_GRP_NUM_ENT);
  dp->cur_tbl8s++;
  return tbl8_idx;
}

static void tbl8_recycle(struct dir24_8_tbl *dp, uint32_t ip,
                         uint64_t tbl8_idx) {
  uint32_t i;
  uint64_t nh;
  uint8_t *ptr8;
...
static int install_to_fib(struct dir24_8_tbl *dp, uint32_t ledge,
                          uint32_t redge, uint64_t next_hop) {
  uint64_t tbl24_tmp;
  int tbl8_idx;
  int tmp_tbl8_idx;
  uint8_t *tbl8_ptr;
    ...
    if ((ROUNDUP(ledge, 24) - ledge) != 0) {
      ...
      if ((tbl24_tmp & DIR24_8_EXT_ENT) != DIR24_8_EXT_ENT) {
        ...
        write_to_fib(get_tbl24_p(dp, ledge, dp->nh_sz),
                     (tbl8_idx << 1) | DIR24_8_EXT_ENT, dp->nh_sz, 1);
      } else
      ...
      write_to_fib((void *)tbl8_ptr, (next_hop << 1) | DIR24_8_EXT_ENT,
                   dp->nh_sz, ROUNDUP(ledge, 24) - ledge);
      tbl8_recycle(dp, ledge, tbl8_idx);
    }
    write_to_fib(get_tbl24_p(dp, ROUNDUP(ledge, 24), dp->nh_sz), next_hop << 1,
                 dp->nh_sz, len);
    if (redge & ~DIR24_8_TBL24_MASK) {
      ...
      if ((tbl24_tmp & DIR24_8_EXT_ENT) != DIR24_8_EXT_ENT) {
        ...
        if (tbl8_idx < 0)
          return -ENOSPC;
        /*update dir24 entry with tbl8 index*/
        write_to_fib(get_tbl24_p(dp, redge, dp->nh_sz),
                     (tbl8_idx << 1) | DIR24_8_EXT_ENT, dp->nh_sz, 1);
      } else
        tbl8_idx = tbl24_tmp >> 1;
      tbl8_ptr = (uint8_t *)dp->tbl8 +
...
static int modify_fib(struct dir24_8_tbl *dp, struct rte_rib *rib, uint32_t ip,
  ...
  ledge = ip;
  do {
    tmp = rte_rib_get_nxt(rib, ip, depth, tmp, RTE_RIB_GET_NXT_COVER);
    if (tmp != NULL) {
...
int dir24_8_modify(struct rte_fib *fib, uint32_t ip, uint8_t depth,
  ...
  case RTE_FIB_ADD:
    ...
    if (depth > 24) {
      tmp = rte_rib_get_nxt(rib, ip, 24, NULL, RTE_RIB_GET_NXT_COVER);
...
void *dir24_8_create(const char *name, int socket_id,
  ...
  num_tbl8 = RTE_ALIGN_CEIL(fib_conf->dir24_8.num_tbl8, BITMAP_SLAB_BIT_SIZE);

  snprintf(mem_name, sizeof(mem_name), "DP_%s", name);
  dp = rte_zmalloc_socket(
      name, sizeof(struct dir24_8_tbl) + DIR24_8_TBL24_NUM_ENT * (1 << nh_sz),
  ...
  uint64_t tbl8_sz =
      DIR24_8_TBL8_GRP_NUM_ENT * (1ULL << nh_sz) * (num_tbl8 + 1);
  dp->tbl8 =
      rte_zmalloc_socket(mem_name, tbl8_sz, RTE_CACHE_LINE_SIZE, socket_id);
  if (dp->tbl8 == NULL) {
    rte_errno = ENOMEM;
    rte_free(dp);
    return NULL;
  ...
  dp->tbl8_idxes =
      rte_zmalloc_socket(mem_name, RTE_ALIGN_CEIL(dp->number_tbl8s, 64) >> 3,
                         RTE_CACHE_LINE_SIZE, socket_id);
