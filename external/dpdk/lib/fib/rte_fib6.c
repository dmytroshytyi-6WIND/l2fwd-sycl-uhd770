...
static void dummy_lookup(void *fib_p, uint8_t ips[][RTE_FIB6_IPV6_ADDR_SIZE],
...
static int dummy_modify(struct rte_fib6 *fib,
...
static int init_dataplane(struct rte_fib6 *fib, __rte_unused int socket_id,
...
int rte_fib6_lookup_bulk(struct rte_fib6 *fib,
  ...
  FIB6_RETURN_IF_TRUE((fib == NULL) || (ips == NULL) || (next_hops == NULL) ||
                          (fib->lookup == NULL),
                      -EINVAL);
...
struct rte_fib6 *rte_fib6_create(const char *name, int socket_id,
  ...
  fib = rte_zmalloc_socket(mem_name, sizeof(struct rte_fib6),
  ...
  if (ret < 0) {
    RTE_LOG(ERR, LPM, "FIB dataplane struct %s memory allocation failed\n",
            name);
    rte_errno = -ret;
    goto free_fib;
  }
...
static void free_dataplane(struct rte_fib6 *fib) {
  switch (fib->type) {
  case RTE_FIB6_DUMMY:
    return;
  case RTE_FIB6_TRIE:
