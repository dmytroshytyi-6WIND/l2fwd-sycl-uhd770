...
static uint16_t ethdev_tx_node_process(struct rte_graph *graph,
  ...
  if (count != nb_objs) {
    rte_node_enqueue(graph, node, ETHDEV_TX_NEXT_PKT_DROP, &objs[count],
                     nb_objs - count);
  }
...
static int ethdev_tx_node_init(const struct rte_graph *graph,
