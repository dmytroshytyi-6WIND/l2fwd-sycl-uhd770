...
static struct ethdev_ctrl { uint16_t nb_graphs; } ctrl;

int rte_node_eth_config(struct rte_node_ethdev_config *conf, uint16_t nb_confs,
                        uint16_t nb_graphs) {
  ...
  for (i = 0; i < nb_confs; i++) {
    ...
    for (j = 0; j < rx_q_used; j++) {
      ...
      node_dbg("ethdev", "Rx node %s-%s: is at %u", rx_node->name, name, id);
    }
    ...
    rte_node_edge_update(ip4_rewrite_node->id, RTE_EDGE_ID_INVALID, &next_nodes,
                         1);
