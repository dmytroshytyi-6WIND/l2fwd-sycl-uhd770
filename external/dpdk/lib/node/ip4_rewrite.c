...
static uint16_t ip4_rewrite_node_process(struct rte_graph *graph,
...
static int ip4_rewrite_node_init(const struct rte_graph *graph,
...
int ip4_rewrite_set_next(uint16_t port_id, uint16_t next_index) {
  if (ip4_rewrite_nm == NULL) {
    ip4_rewrite_nm =
        rte_zmalloc("ip4_rewrite", sizeof(struct ip4_rewrite_node_main),
                    RTE_CACHE_LINE_SIZE);
    if (ip4_rewrite_nm == NULL)
      return -ENOMEM;
  }
