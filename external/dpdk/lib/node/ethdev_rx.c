...
static __rte_always_inline uint16_t ethdev_rx_node_process_inline(
    struct rte_graph *graph, struct rte_node *node, ethdev_rx_node_ctx_t *ctx) {
...
static __rte_always_inline uint16_t ethdev_rx_node_process(
...
static inline uint32_t l3_ptype(uint16_t etype, uint32_t ptype) {
...
static uint16_t eth_pkt_parse_cb(uint16_t port, uint16_t queue,
  ...
  while (n_left >= 12) {
    ...
    rte_prefetch0(rte_pktmbuf_mtod(pkts[4], struct rte_ether_hdr *));
    rte_prefetch0(rte_pktmbuf_mtod(pkts[5], struct rte_ether_hdr *));
    rte_prefetch0(rte_pktmbuf_mtod(pkts[6], struct rte_ether_hdr *));
    rte_prefetch0(rte_pktmbuf_mtod(pkts[7], struct rte_ether_hdr *));
...
static int ethdev_ptype_setup(uint16_t port, uint16_t queue) {
  ...
  if (!l3_ipv4 || !l3_ipv6) {
    ...
    if (!rte_eth_add_rx_callback(port, queue, eth_pkt_parse_cb, NULL)) {
      node_err("ethdev_rx", "Failed to add rx ptype cb: port=%d, queue=%d\n",
               port, queue);
      return -EINVAL;
    }
  }
...
static int ethdev_rx_node_init(const struct rte_graph *graph,
