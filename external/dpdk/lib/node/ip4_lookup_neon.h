...
/* ARM64 NEON */
static uint16_t ip4_lookup_node_process_vec(struct rte_graph *graph,
  ...
  while (n_left_from >= 4) {
    ...
    rte_edge_t fix_spec =
        ((next_index == result.u16[1]) && (result.u16[1] == result.u16[3]) &&
         (result.u16[3] == result.u16[5]) && (result.u16[5] == result.u16[7]));
    ...
    if (unlikely(fix_spec == 0)) {
      ...
      if (next_index == result.u16[1]) {
        ...
        rte_node_enqueue_x1(graph, node, result.u16[1], from[0]);
      }
      ...
      if (next_index == result.u16[3]) {
        ...
        rte_node_enqueue_x1(graph, node, result.u16[3], from[1]);
      }
      ...
      if (next_index == result.u16[5]) {
        ...
        rte_node_enqueue_x1(graph, node, result.u16[5], from[2]);
      }
      ...
      if (next_index == result.u16[7]) {
        ...
        rte_node_enqueue_x1(graph, node, result.u16[7], from[3]);
      }
