...
static uint16_t ip4_lookup_node_process_scalar(struct rte_graph *graph,
...
int rte_node_ip4_route_add(uint32_t ip, uint8_t depth, uint16_t next_hop,
  ...
  node_dbg("ip4_lookup", "LPM: Adding route %s / %d nh (0x%x)", abuf, depth,
           val);
  ...
  for (socket = 0; socket < RTE_MAX_NUMA_NODES; socket++) {
    ...
    if (ret < 0) {
      node_err("ip4_lookup",
               "Unable to add entry %s / %d nh (%x) to LPM table on sock %d, "
               "rc=%d\n",
...
static int setup_lpm(struct ip4_lookup_node_main *nm, int socket) {
...
static int ip4_lookup_node_init(const struct rte_graph *graph,
                                struct rte_node *node) {
