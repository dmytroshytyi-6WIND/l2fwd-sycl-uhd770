...
struct rte_flow_classifier {
  ...
  uint16_t nb_pkts;
  struct rte_flow_classify_table_entry *entries[RTE_PORT_IN_BURST_SIZE_MAX];
} __rte_cache_aligned;
...
int rte_flow_classify_validate(struct rte_flow_classifier *cls,
  ...
  if (cls == NULL) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: rte_flow_classifier parameter is NULL\n",
                          __func__);
    return -EINVAL;
  }
...
print_acl_ipv4_key_add(struct rte_table_acl_rule_add_params *key) {
  ...
  uint32_t_to_char(key->field_value[SRC_FIELD_IPV4].value.u32, &a, &b, &c, &d);
  printf(" %hhu.%hhu.%hhu.%hhu/0x%x ", a, b, c, d,
         key->field_value[SRC_FIELD_IPV4].mask_range.u32);

  uint32_t_to_char(key->field_value[DST_FIELD_IPV4].value.u32, &a, &b, &c, &d);
  printf("%hhu.%hhu.%hhu.%hhu/0x%x ", a, b, c, d,
         key->field_value[DST_FIELD_IPV4].mask_range.u32);

  printf("%hu : 0x%x %hu : 0x%x", key->field_value[SRCP_FIELD_IPV4].value.u16,
...
rte_flow_classifier_check_params(struct rte_flow_classifier_params *params) {
  if (params == NULL) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: Incorrect value for parameter params\n",
                          __func__);
    return -EINVAL;
  }
  ...
  /* name */
  if (params->name == NULL) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: Incorrect value for parameter name\n",
                          __func__);
    return -EINVAL;
  }
  ...
  if (params->socket_id < 0) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: Incorrect value for parameter socket_id\n",
                          __func__);
    return -EINVAL;
  }
...
rte_flow_classifier_create(struct rte_flow_classifier_params *params) {
  ...
  if (ret != 0) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: flow classifier params check failed (%d)\n",
                          __func__, ret);
    return NULL;
  }
  ...
  cls =
      rte_zmalloc_socket("FLOW_CLASSIFIER", sizeof(struct rte_flow_classifier),
                         RTE_CACHE_LINE_SIZE, params->socket_id);

  if (cls == NULL) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: flow classifier memory allocation failed\n",
                          __func__);
    return NULL;
  }
...
static void rte_flow_classify_table_free(struct rte_cls_table *table) {
...
rte_table_check_params(struct rte_flow_classifier *cls,
  ...
  if (cls == NULL) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: flow classifier parameter is NULL\n",
                          __func__);
    return -EINVAL;
  }
  if (params == NULL) {
  ...
  if (params->ops->f_create == NULL) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: f_create function pointer is NULL\n",
                          __func__);
    return -EINVAL;
  }

  if (params->ops->f_lookup == NULL) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: f_lookup function pointer is NULL\n",
                          __func__);
    return -EINVAL;
  }
  ...
  if (cls->num_tables == RTE_FLOW_CLASSIFY_TABLE_MAX) {
    RTE_FLOW_CLASSIFY_LOG(ERR, "%s: Incorrect value for num_tables parameter\n",
                          __func__);
    return -EINVAL;
  }
...
int rte_flow_classify_table_create(
    struct rte_flow_classifier *cls,
    struct rte_flow_classify_table_params *params) {
  ...
  /* Create the table */
  h_table =
      params->ops->f_create(params->arg_create, cls->socket_id, entry_size);
  if (h_table == NULL) {
...
allocate_acl_ipv4_5tuple_rule(struct rte_flow_classifier *cls) {
  ...
  rule->rules.u.ipv4_5tuple.src_port_mask = cls->ntuple_filter.src_port_mask;
  ...
  rule->rules.u.ipv4_5tuple.dst_port_mask = cls->ntuple_filter.dst_port_mask;
...
struct rte_flow_classify_rule *rte_flow_classify_table_entry_add(
    struct rte_flow_classifier *cls, const struct rte_flow_attr *attr,
    const struct rte_flow_item pattern[],
    const struct rte_flow_action actions[], int *key_found,
    struct rte_flow_error *error) {
  struct rte_flow_classify_rule *rule;
  ...
  for (i = 0; i < cls->num_tables; i++) {
    ...
    if (table->type == table_type) {
      if (table->ops.f_add != NULL) {
        ret =
            table->ops.f_add(table->h_table, &rule->u.key.key_add, &rule->entry,
                             &rule->key_found, &rule->entry_ptr);
        if (ret) {
          free(rule);
          return NULL;
        }
  ...
  for (i = 0; i < cls->num_tables; i++) {
    ...
    if (table->type == tbl_type) {
      if (table->ops.f_delete != NULL) {
        ret = table->ops.f_delete(table->h_table, &rule->u.key.key_del,
                                  &rule->key_found, &rule->entry);
        if (ret == 0)
          free(rule);
        return ret;
      }
...
static int flow_classifier_lookup(struct rte_flow_classifier *cls,
                                  struct rte_cls_table *table,
  ...
  ret = table->ops.f_lookup(table->h_table, pkts, pkts_mask, &lookup_hit_mask,
                            (void **)cls->entries);
...
static int action_apply(struct rte_flow_classifier *cls,
                        struct rte_flow_classify_rule *rule,
...
int rte_flow_classifier_query(struct rte_flow_classifier *cls,
  ...
  for (i = 0; i < cls->num_tables; i++) {
    ...
    if (table->type == tbl_type) {
      ret = flow_classifier_lookup(cls, table, pkts, nb_pkts);
      if (!ret) {
