...
static int classify_parse_ntuple_filter(const struct rte_flow_attr *attr,
...
struct classify_action *classify_get_flow_action(void) {
  return &action;
}
...
void classify_pattern_skip_void_item(struct rte_flow_item *items,
                                     const struct rte_flow_item *pattern) {
...
static bool classify_match_pattern(enum rte_flow_item_type *item_array,
...
parse_filter_t classify_find_parse_filter_func(struct rte_flow_item *pattern) {
  parse_filter_t parse_filter = NULL;
  uint8_t i = 0;

  for (; i < RTE_DIM(classify_supported_patterns); i++) {
    if (classify_match_pattern(classify_supported_patterns[i].items, pattern)) {
      parse_filter = classify_supported_patterns[i].parse_filter;
      break;
    }
  }
