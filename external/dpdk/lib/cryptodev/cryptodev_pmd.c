 ...
 */
static int rte_cryptodev_pmd_parse_name_arg(const char *key __rte_unused,
                                            const char *value,
                                            void *extra_args) {
 ...
 */
static int rte_cryptodev_pmd_parse_uint_arg(const char *key __rte_unused,
                                            const char *value,
                                            void *extra_args) {
  int i;
  char *end;
...
int rte_cryptodev_pmd_parse_input_args(
    struct rte_cryptodev_pmd_init_params *params, const char *args) {
  ...
  if (args) {
    ...
    ret = rte_kvargs_process(kvlist, RTE_CRYPTODEV_PMD_NAME_ARG,
                             &rte_cryptodev_pmd_parse_name_arg, params);
    if (ret < 0)
      goto free_kvlist;
  }
...
rte_cryptodev_pmd_create(const char *name, struct rte_device *device,
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    cryptodev->data->dev_private = rte_zmalloc_socket(
        "cryptodev device private", params->private_data_size,
...
void rte_cryptodev_pmd_probing_finish(struct rte_cryptodev *cryptodev) {
  if (cryptodev == NULL)
    return;
  /*
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY)
    cryptodev_fp_ops_set(rte_crypto_fp_ops + cryptodev->data->dev_id,
                         cryptodev);
}
...
dummy_crypto_enqueue_burst(__rte_unused void *qp,
  ...
  CDEV_LOG_ERR("crypto enqueue burst requested for unconfigured device");
  rte_errno = ENOTSUP;
  return 0;
}
...
dummy_crypto_dequeue_burst(__rte_unused void *qp,
                           __rte_unused struct rte_crypto_op **ops,
                           __rte_unused uint16_t nb_ops) {
  CDEV_LOG_ERR("crypto dequeue burst requested for unconfigured device");
  rte_errno = ENOTSUP;
  return 0;
}

void cryptodev_fp_ops_reset(struct rte_crypto_fp_ops *fp_ops) {
...
void cryptodev_fp_ops_set(struct rte_crypto_fp_ops *fp_ops,
