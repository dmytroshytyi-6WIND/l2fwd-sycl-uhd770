 ...
 */
__rte_experimental int rte_cryptodev_asym_xform_capability_check_optype(
 ...
 *   - Return -1 if the parameters are out of range of the capability.
 */
__rte_experimental int rte_cryptodev_asym_xform_capability_check_modlen(
    const struct rte_cryptodev_asymmetric_xform_capability *capability,
    uint16_t modlen);
...
extern const char *rte_cryptodev_get_feature_name(uint64_t flag);
 ...
 */
extern int rte_cryptodev_get_dev_id(const char *name);
 ...
 */
extern const char *rte_cryptodev_name_get(uint8_t dev_id);
 ...
 */
extern uint8_t rte_cryptodev_count(void);
 ...
 */
extern uint8_t rte_cryptodev_device_count_by_driver(uint8_t driver_id);
 ...
 */
extern int rte_cryptodev_socket_id(uint8_t dev_id);
 ...
 */
extern int rte_cryptodev_configure(uint8_t dev_id,
 ...
 */
extern int rte_cryptodev_start(uint8_t dev_id);
 ...
 * @param	dev_id		The identifier of the device.
 */
extern void rte_cryptodev_stop(uint8_t dev_id);
 ...
 */
extern int rte_cryptodev_close(uint8_t dev_id);
 ...
 */
extern uint16_t rte_cryptodev_queue_pair_count(uint8_t dev_id);
 ...
 *   - Non-zero otherwise.
 */
extern int rte_cryptodev_stats_get(uint8_t dev_id,
 ...
 * @param	dev_id		The identifier of the device.
 */
extern void rte_cryptodev_stats_reset(uint8_t dev_id);
 ...
 * RTE_CRYPTO_OP_TYPE_UNDEFINED.
 */
extern void rte_cryptodev_info_get(uint8_t dev_id,
 ...
 */
extern int rte_cryptodev_callback_register(uint8_t dev_id,
                                           enum rte_cryptodev_event_type event,
 ...
 */
__rte_experimental int
rte_cryptodev_sym_session_set_user_data(struct rte_cryptodev_sym_session *sess,
 ...
 */
__rte_experimental uint32_t rte_cryptodev_sym_cpu_crypto_process(
 ...
 */
typedef uint32_t (*cryptodev_sym_raw_dequeue_burst_t)(
    void *qp, uint8_t *drv_ctx,
    rte_cryptodev_raw_get_dequeue_count_t get_dequeue_count,
    uint32_t max_nb_to_dequeue, rte_cryptodev_raw_post_dequeue_t post_dequeue,
    void **out_user_data, uint8_t is_user_data_array, uint32_t *n_success,
    int *dequeue_status);
 ...
 *   - On failure return negative integer.
 */
__rte_experimental int rte_cryptodev_configure_raw_dp_ctx(
 ...
 *     "enqueue_status" value.
 */
__rte_experimental uint32_t rte_cryptodev_raw_enqueue_burst(
    struct rte_crypto_raw_dp_ctx *ctx, struct rte_crypto_sym_vec *vec,
    union rte_crypto_sym_ofs ofs, void **user_data, int *enqueue_status);
 ...
 *   - negative integer: failure.
 */
__rte_experimental static __rte_always_inline int rte_cryptodev_raw_enqueue(
    struct rte_crypto_raw_dp_ctx *ctx, struct rte_crypto_vec *data_vec,
    uint16_t n_data_vecs, union rte_crypto_sym_ofs ofs,
    struct rte_crypto_va_iova_ptr *iv, struct rte_crypto_va_iova_ptr *digest,
    struct rte_crypto_va_iova_ptr *aad_or_auth_iv, void *user_data) {
  return (*ctx->enqueue)(ctx->qp_data, ctx->drv_ctx_data, data_vec, n_data_vecs,
                         ofs, iv, digest, aad_or_auth_iv, user_data);
}
 ...
 */
__rte_experimental int
rte_cryptodev_raw_enqueue_done(struct rte_crypto_raw_dp_ctx *ctx, uint32_t n);
 ...
 *     queue, depends on "dequeue_status" value.
 */
__rte_experimental uint32_t rte_cryptodev_raw_dequeue_burst(
 ...
 */
__rte_experimental int
rte_cryptodev_raw_dequeue_done(struct rte_crypto_raw_dp_ctx *ctx, uint32_t n);
...
__rte_experimental int
rte_cryptodev_remove_enq_callback(uint8_t dev_id, uint16_t qp_id,
 ...
 */
__rte_experimental int
rte_cryptodev_remove_deq_callback(uint8_t dev_id, uint16_t qp_id,
                                  struct rte_cryptodev_cb *cb);
 ...
 *   *ops* array.
 */
static inline uint16_t rte_cryptodev_dequeue_burst(uint8_t dev_id,
                                                   uint16_t qp_id,
                                                   struct rte_crypto_op **ops,
                                                   uint16_t nb_ops) {
  ...
  if (unlikely(fp_ops->qp.deq_cb != NULL)) {
    ...
    while (cb != NULL) {
      nb_ops = cb->fn(dev_id, qp_id, ops, nb_ops, cb->arg);
      cb = cb->next;
    };
 ...
 * a *rte_crypto_op*.
 */
static inline uint16_t rte_cryptodev_enqueue_burst(uint8_t dev_id,
                                                   uint16_t qp_id,
                                                   struct rte_crypto_op **ops,
                                                   uint16_t nb_ops) {
  const struct rte_crypto_fp_ops *fp_ops;
