...
static struct rte_cryptodev_global cryptodev_globals = {
    .devs = rte_crypto_devices, .data = {NULL}, .nb_devs = 0};
...
int rte_cryptodev_get_cipher_algo_enum(
    enum rte_crypto_cipher_algorithm *algo_enum, const char *algo_string) {
  unsigned int i;
...
int rte_cryptodev_asym_get_xform_enum(
    enum rte_crypto_asym_xform_type *xform_enum, const char *xform_string) {
  unsigned int i;

  for (i = 1; i < RTE_DIM(rte_crypto_asym_xform_strings); i++) {
    if (strcmp(xform_string, rte_crypto_asym_xform_strings[i]) == 0) {
...
rte_cryptodev_sym_capability_get(
    uint8_t dev_id, const struct rte_cryptodev_sym_capability_idx *idx) {
...
static int param_range_check(uint16_t size,
...
rte_cryptodev_asym_capability_get(
    uint8_t dev_id, const struct rte_cryptodev_asym_capability_idx *idx) {
  const struct rte_cryptodev_capabilities *capability;
  struct rte_cryptodev_info dev_info;
  unsigned int i = 0;
...
int rte_cryptodev_sym_capability_check_cipher(
...
int rte_cryptodev_sym_capability_check_auth(
    const struct rte_cryptodev_symmetric_capability *capability,
...
int rte_cryptodev_sym_capability_check_aead(
    const struct rte_cryptodev_symmetric_capability *capability,
...
int rte_cryptodev_asym_xform_capability_check_optype(
...
int rte_cryptodev_asym_xform_capability_check_modlen(
    const struct rte_cryptodev_asymmetric_xform_capability *capability,
    uint16_t modlen) {
...
static void cryptodev_cb_cleanup(struct rte_cryptodev *dev) {
...
static int cryptodev_cb_init(struct rte_cryptodev *dev) {
  struct rte_cryptodev_cb_rcu *list;
  ...
  dev->enq_cbs = rte_zmalloc(
      NULL, sizeof(struct rte_cryptodev_cb_rcu) * dev->data->nb_queue_pairs, 0);
  if (dev->enq_cbs == NULL) {
    CDEV_LOG_ERR("Failed to allocate memory for enq callbacks");
    return -ENOMEM;
  }

  dev->deq_cbs = rte_zmalloc(
      NULL, sizeof(struct rte_cryptodev_cb_rcu) * dev->data->nb_queue_pairs, 0);
  if (dev->deq_cbs == NULL) {
...
static inline uint8_t rte_cryptodev_is_valid_device_data(uint8_t dev_id) {
  if (dev_id >= RTE_CRYPTO_MAX_DEVS || rte_crypto_devices[dev_id].data == NULL)
    return 0;
...
int rte_cryptodev_get_dev_id(const char *name) {
  ...
  for (i = 0; i < RTE_CRYPTO_MAX_DEVS; i++) {
    if (!rte_cryptodev_is_valid_device_data(i))
      continue;
    if ((strcmp(cryptodev_globals.devs[i].data->name, name) == 0) &&
        (cryptodev_globals.devs[i].attached == RTE_CRYPTODEV_ATTACHED))
      return i;
  }
...
uint8_t rte_cryptodev_count(void) { return cryptodev_globals.nb_devs; }
...
uint8_t rte_cryptodev_devices_get(const char *driver_name, uint8_t *devices,
                                  uint8_t nb_devices) {
  uint8_t i, count = 0;
...
void *rte_cryptodev_get_sec_ctx(uint8_t dev_id) {
  if (dev_id < RTE_CRYPTO_MAX_DEVS &&
      (rte_crypto_devices[dev_id].feature_flags & RTE_CRYPTODEV_FF_SECURITY))
    return rte_crypto_devices[dev_id].security_ctx;

  return NULL;
}

int rte_cryptodev_socket_id(uint8_t dev_id) {
  struct rte_cryptodev *dev;
...
static inline int rte_cryptodev_data_alloc(uint8_t dev_id,
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    mz = rte_memzone_reserve(mz_name, sizeof(struct rte_cryptodev_data),
                             socket_id, 0);
    CDEV_LOG_DEBUG("PRIMARY:reserved memzone for %s (%p)", mz_name, mz);
  } else {
    mz = rte_memzone_lookup(mz_name);
    CDEV_LOG_DEBUG("SECONDARY:looked up memzone for %s (%p)", mz_name, mz);
  }
...
static inline int rte_cryptodev_data_free(uint8_t dev_id,
                                          struct rte_cryptodev_data **data) {
  char mz_name[RTE_MEMZONE_NAMESIZE];
  const struct rte_memzone *mz;
  int n;
  ...
  *data = NULL;
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    CDEV_LOG_DEBUG("PRIMARY:free memzone of %s (%p)", mz_name, mz);
    return rte_memzone_free(mz);
  } else {
    CDEV_LOG_DEBUG("SECONDARY:don't free memzone of %s (%p)", mz_name, mz);
  }

  return 0;
}

static uint8_t rte_cryptodev_find_free_device_index(void) {
  uint8_t dev_id;
...
struct rte_cryptodev *rte_cryptodev_pmd_allocate(const char *name,
  ...
  if (cryptodev->data == NULL) {
    ...
    int retval = rte_cryptodev_data_alloc(dev_id, cryptodev_data, socket_id);
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
      strlcpy(cryptodev->data->name, name, RTE_CRYPTODEV_NAME_MAX_LEN);
    ...
    CDEV_LOG_DEBUG("Data for %s: dev_id %d, socket %d, started %d",
                   cryptodev->data->name, cryptodev->data->dev_id,
                   cryptodev->data->socket_id, cryptodev->data->dev_started);
...
static int rte_cryptodev_queue_pairs_config(struct rte_cryptodev *dev,
  ...
  if ((dev == NULL) || (nb_qpairs < 1)) {
    CDEV_LOG_ERR("invalid param: dev %p, nb_queues %u", dev, nb_qpairs);
    return -EINVAL;
  }

  CDEV_LOG_DEBUG("Setup %d queues pairs on device %u", nb_qpairs,
                 dev->data->dev_id);
  ...
  if (nb_qpairs > (dev_info.max_nb_queue_pairs)) {
    CDEV_LOG_ERR("Invalid num queue_pairs (%u) for dev %u", nb_qpairs,
                 dev->data->dev_id);
    return -EINVAL;
  }
  ...
  if (dev->data->queue_pairs == NULL) { /* first time configuration */
    dev->data->queue_pairs = rte_zmalloc_socket(
        ...
        sizeof(dev->data->queue_pairs[0]) * dev_info.max_nb_queue_pairs,
    ...
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->queue_pair_release, -ENOTSUP);
...
int rte_cryptodev_configure(uint8_t dev_id,
  ...
  if (dev->data->dev_started) {
    CDEV_LOG_ERR("device %d must be stopped to allow configuration", dev_id);
    return -EBUSY;
  }
  ...
  diag = rte_cryptodev_queue_pairs_config(dev, config->nb_queue_pairs,
                                          config->socket_id);
  if (diag != 0) {
    CDEV_LOG_ERR("dev%d rte_crypto_dev_queue_pairs_config = %d", dev_id, diag);
    return diag;
  }
...
int rte_cryptodev_start(uint8_t dev_id) {
  ...
  if (dev->data->dev_started != 0) {
    CDEV_LOG_ERR("Device with dev_id=%" PRIu8 " already started", dev_id);
    return 0;
  }
...
void rte_cryptodev_stop(uint8_t dev_id) {
  ...
  if (dev->data->dev_started == 0) {
    CDEV_LOG_ERR("Device with dev_id=%" PRIu8 " already stopped", dev_id);
    return;
  }
...
int rte_cryptodev_close(uint8_t dev_id) {
  ...
  if (dev->data->dev_started == 1) {
    CDEV_LOG_ERR("Device %u must be stopped before closing", dev_id);
    return -EBUSY;
  }
...
int rte_cryptodev_get_qp_status(uint8_t dev_id, uint16_t queue_pair_id) {
  ...
  if (qps[queue_pair_id]) {
    CDEV_LOG_DEBUG("qp %d on dev %d is initialised", queue_pair_id, dev_id);
    return 1;
  }

  CDEV_LOG_DEBUG("qp %d on dev %d is not initialised", queue_pair_id, dev_id);
...

{
  ...
  if (qp_conf->mp_session) {
    ...
    if (!pool_priv ||
        qp_conf->mp_session->private_data_size < sizeof(*pool_priv)) {
    ...
    if ((rte_cryptodev_sym_get_existing_header_session_size(&s) > obj_size) ||
        (s.nb_drivers <= dev->driver_id) ||
        rte_cryptodev_sym_get_private_session_size(dev_id) > obj_priv_size) {
      CDEV_LOG_ERR("Invalid mempool\n");
      return -EINVAL;
    }
  }
...
rte_cryptodev_add_enq_callback(uint8_t dev_id, uint16_t qp_id,
                               rte_cryptodev_callback_fn cb_fn, void *cb_arg) {
  struct rte_cryptodev *dev;
  struct rte_cryptodev_cb_rcu *list;
...
int rte_cryptodev_remove_enq_callback(uint8_t dev_id, uint16_t qp_id,
  ...
  for (; *prev_cb != NULL; prev_cb = &curr_cb->next) {
    ...
    if (curr_cb == cb) {
      ...
      __atomic_store_n(prev_cb, curr_cb->next, __ATOMIC_RELAXED);
      ret = 0;
      break;
    }
  }
...
void rte_cryptodev_pmd_callback_process(struct rte_cryptodev *dev,
  ...
  TAILQ_FOREACH(cb_lst, &(dev->link_intr_cbs), next) {
    ...
    cb_lst->active = 1;
    rte_spinlock_unlock(&rte_cryptodev_cb_lock);
    dev_cb.cb_fn(dev->data->dev_id, dev_cb.event, dev_cb.cb_arg);
    rte_spinlock_lock(&rte_cryptodev_cb_lock);
    cb_lst->active = 0;
  }
  rte_spinlock_unlock(&rte_cryptodev_cb_lock);
...
int rte_cryptodev_sym_session_init(uint8_t dev_id,
  ...
  struct rte_cryptodev *dev;
  uint32_t sess_priv_sz = rte_cryptodev_sym_get_private_session_size(dev_id);
  uint8_t index;
  int ret;
  ...
  if (sess->sess_data[index].refcnt == 0) {
    ret = dev->dev_ops->sym_session_configure(dev, xforms, sess, mp);
    if (ret < 0) {
      CDEV_LOG_ERR("dev_id %d failed to configure session details", dev_id);
      return ret;
    }
  }
...
rte_cryptodev_sym_session_pool_create(const char *name, uint32_t nb_elts,
                                      ...
                                      uint16_t user_data_size, int socket_id) {
  struct rte_mempool *mp;
  struct rte_cryptodev_sym_session_pool_private_data *pool_priv;
  uint32_t obj_sz;
  ...
  mp = rte_mempool_create(name, nb_elts, obj_sz, cache_size,
                          (uint32_t)(sizeof(*pool_priv)), NULL, NULL, NULL,
                          NULL, socket_id, 0);
  if (mp == NULL) {
  ...
  rte_cryptodev_trace_sym_session_pool_create(name, nb_elts, elt_size,
                                              cache_size, user_data_size, mp);
  return mp;
}
...
rte_cryptodev_asym_session_pool_create(const char *name, uint32_t nb_elts,
  ...
  rte_cryptodev_trace_asym_session_pool_create(name, nb_elts, user_data_size,
                                               cache_size, mp);
  return mp;
}
...
rte_cryptodev_sym_session_data_size(struct rte_cryptodev_sym_session *sess) {
  return (sizeof(sess->sess_data[0]) * sess->nb_drivers) + sess->user_data_sz;
}

static uint8_t rte_cryptodev_sym_is_valid_session_pool(struct rte_mempool *mp) {
  struct rte_cryptodev_sym_session_pool_private_data *pool_priv;
  ...
  if (!pool_priv || mp->private_data_size < sizeof(*pool_priv) ||
      pool_priv->nb_drivers != nb_drivers ||
      mp->elt_size <
          rte_cryptodev_sym_get_header_session_size() + pool_priv->user_data_sz)
    return 0;
...
int rte_cryptodev_asym_session_create(uint8_t dev_id,
                                      ...
                                      struct rte_mempool *mp, void **session) {
  ...
  session_priv_data_sz = rte_cryptodev_asym_get_private_session_size(dev_id);
  pool_priv = rte_mempool_get_priv(mp);

  if (pool_priv->max_priv_session_sz < session_priv_data_sz) {
    CDEV_LOG_DEBUG(
        "The private session data size used when creating the mempool is "
        "smaller than this device's private session data.");
    return -EINVAL;
  }

  /* Verify if provided mempool can hold elements big enough. */
  if (mp->elt_size < session_header_size + session_priv_data_sz) {
    CDEV_LOG_ERR("mempool elements too small to hold session objects");
    return -EINVAL;
  }
...
unsigned int rte_cryptodev_sym_get_header_session_size(void) {
  ...
  return (unsigned int)(sizeof(s) + rte_cryptodev_sym_session_data_size(&s));
}

unsigned int rte_cryptodev_sym_get_existing_header_session_size(
    struct rte_cryptodev_sym_session *sess) {
  if (!sess)
    return 0;
  else
...
int rte_cryptodev_asym_session_set_user_data(void *session, void *data,
                                             uint16_t size) {
  ...
  rte_memcpy(sess->sess_private_data + sess->max_priv_data_sz, data, size);
  return 0;
}

void *rte_cryptodev_asym_session_get_user_data(void *session) {
  struct rte_cryptodev_asym_session *sess = session;
  if (sess == NULL || sess->user_data_sz == 0)
    return NULL;

  return (void *)(sess->sess_private_data + sess->max_priv_data_sz);
}

static inline void sym_crypto_fill_status(struct rte_crypto_sym_vec *vec,
                                          int32_t errnum) {
  uint32_t i;
  for (i = 0; i < vec->num; i++)
    vec->status[i] = errnum;
...
uint32_t rte_cryptodev_sym_cpu_crypto_process(
    uint8_t dev_id, struct rte_cryptodev_sym_session *sess,
...
int rte_cryptodev_configure_raw_dp_ctx(
    uint8_t dev_id, uint16_t qp_id, struct rte_crypto_raw_dp_ctx *ctx,
  ...
  return (*dev->dev_ops->sym_configure_raw_dp_ctx)(dev, qp_id, ctx, sess_type,
                                                   session_ctx, is_update);
}
...
uint32_t rte_cryptodev_raw_enqueue_burst(struct rte_crypto_raw_dp_ctx *ctx,
                                         ...
                                         void **user_data,
                                         int *enqueue_status) {
  return (*ctx->enqueue_burst)(ctx->qp_data, ctx->drv_ctx_data, vec, ofs,
                               user_data, enqueue_status);
}
...
uint32_t rte_cryptodev_raw_dequeue_burst(
    ...
    int *status) {
  return (*ctx->dequeue_burst)(
      ctx->qp_data, ctx->drv_ctx_data, get_dequeue_count, max_nb_to_dequeue,
      post_dequeue, out_user_data, is_user_data_array, n_success_jobs, status);
}
...
static void rte_crypto_op_init(struct rte_mempool *mempool, void *opaque_arg,
                               void *_op_data, __rte_unused unsigned i) {
...
rte_crypto_op_pool_create(const char *name, enum rte_crypto_op_type type,
  ...
  if (mp != NULL) {
    priv = (struct rte_crypto_op_pool_private *)rte_mempool_get_priv(mp);

    if (mp->elt_size != elt_size || mp->cache_size < cache_size ||
        mp->size < nb_elts || priv->priv_size < priv_size) {
      mp = NULL;
  ...
  mp = rte_mempool_create(name, nb_elts, elt_size, cache_size,
                          ...
                          rte_crypto_op_init, &type, socket_id, 0);
...
int rte_cryptodev_pmd_create_dev_name(char *name, const char *dev_name_prefix) {
  ...
  for (i = 0; i < RTE_CRYPTO_MAX_DEVS; i++) {
    int ret =
        snprintf(name, RTE_CRYPTODEV_NAME_MAX_LEN, "%s_%u", dev_name_prefix, i);
...
static int cryptodev_handle_dev_list(const char *cmd __rte_unused,
...
static int cryptodev_handle_dev_info(const char *cmd __rte_unused,
                                     const char *params,
                                     struct rte_tel_data *d) {
  ...
  rte_tel_data_add_dict_string(d, "device_name", cryptodev_info.device->name);
...
static int cryptodev_handle_dev_stats(const char *cmd __rte_unused,
                                      const char *params,
                                      struct rte_tel_data *d) {
...
crypto_caps_array(struct rte_tel_data *d,
  ...
  while ((dev_caps = &capabilities[i++])->op != RTE_CRYPTO_OP_TYPE_UNDEFINED) {
...
static int cryptodev_handle_dev_caps(const char *cmd __rte_unused,
                                     const char *params,
                                     struct rte_tel_data *d) {
  struct rte_cryptodev_info dev_info;
...
RTE_INIT(cryptodev_init_telemetry) {
  ...
  rte_telemetry_register_cmd(
      "/cryptodev/list", cryptodev_handle_dev_list,
  ...
  rte_telemetry_register_cmd(
      "/cryptodev/caps", cryptodev_handle_dev_caps,
