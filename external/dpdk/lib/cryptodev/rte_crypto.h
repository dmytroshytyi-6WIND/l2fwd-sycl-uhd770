 ...
 */
static inline void __rte_crypto_op_reset(struct rte_crypto_op *op,
 ...
 */
static inline int __rte_crypto_op_raw_bulk_alloc(struct rte_mempool *mempool,
                                                 enum rte_crypto_op_type type,
...
static inline unsigned rte_crypto_op_bulk_alloc(struct rte_mempool *mempool,
                                                enum rte_crypto_op_type type,
                                                struct rte_crypto_op **ops,
                                                uint16_t nb_ops) {
  int i;
 ...
 */
static inline void *__rte_crypto_op_get_priv_data(struct rte_crypto_op *op,
                                                  uint32_t size) {
  uint32_t priv_size;
  ...
  if (likely(op->mempool != NULL)) {
    ...
    if (likely(priv_size >= size)) {
      if (op->type == RTE_CRYPTO_OP_TYPE_SYMMETRIC)
        return (void *)((uint8_t *)(op + 1) + sizeof(struct rte_crypto_sym_op));
 ...
 */
static inline void rte_crypto_op_free(struct rte_crypto_op *op) {
