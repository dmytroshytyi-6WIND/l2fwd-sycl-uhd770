 ...
 */
__rte_internal struct rte_cryptodev *rte_cryptodev_pmd_get_dev(uint8_t dev_id);
 ...
 *   - The rte_cryptodev structure pointer for the given device ID.
 */
__rte_internal struct rte_cryptodev *
 ...
 *  - Returns -ENOMEM if the private session could not be allocated.
 */
typedef int (*cryptodev_asym_configure_session_t)(
 ...
 */
typedef void (*cryptodev_sym_free_session_t)(
 ...
 * @param	sess		Cryptodev session structure
 */
typedef void (*cryptodev_asym_clear_session_t)(
    struct rte_cryptodev *dev, struct rte_cryptodev_asym_session *sess);
/**
...
typedef int (*cryptodev_sym_configure_raw_dp_ctx_t)(
    struct rte_cryptodev *dev, uint16_t qp_id,
    struct rte_crypto_raw_dp_ctx *ctx, enum rte_crypto_op_sess_type sess_type,
 ...
 */
__rte_internal int
rte_cryptodev_pmd_parse_input_args(struct rte_cryptodev_pmd_init_params *params,
                                   const char *args);
 ...
 */
__rte_internal struct rte_cryptodev *
rte_cryptodev_pmd_create(const char *name, struct rte_device *device,
 ...
 *  - errno on failure
 */
__rte_internal int rte_cryptodev_pmd_destroy(struct rte_cryptodev *cryptodev);
 ...
 */
__rte_internal uint8_t rte_cryptodev_allocate_driver(
    struct cryptodev_driver *crypto_drv, const struct rte_driver *drv);
 ...
 *  void
 */
__rte_internal void rte_cryptodev_pmd_probing_finish(struct rte_cryptodev *dev);
...
__rte_internal void cryptodev_fp_ops_reset(struct rte_crypto_fp_ops *fp_ops);

/* Setup crypto device fastpath APIs. */
__rte_internal void cryptodev_fp_ops_set(struct rte_crypto_fp_ops *fp_ops,
