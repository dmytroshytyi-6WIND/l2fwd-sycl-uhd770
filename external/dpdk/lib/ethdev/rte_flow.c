 ...
 */
static inline size_t rte_flow_conv_copy(void *buf, const void *data,
                                        const size_t size,
                                        const struct rte_flow_desc_data *desc,
                                        int type) {
  /**
...
static size_t rte_flow_item_flex_conv(void *buf, const void *data) {
  struct rte_flow_item_flex *dst = buf;
  const struct rte_flow_item_flex *src = data;
  if (buf) {
    dst->pattern =
        rte_memcpy((void *)((uintptr_t)(dst + 1)), src->pattern, src->length);
  }
  return src->length;
}
...
static const struct rte_flow_desc_data rte_flow_desc_item[] = {
    ...
    MK_FLOW_ITEM(PPPOE_PROTO_ID, sizeof(struct rte_flow_item_pppoe_proto_id)),
...
static const struct rte_flow_desc_data rte_flow_desc_action[] = {
    ...
    MK_FLOW_ACTION(OF_SET_NW_TTL, sizeof(struct rte_flow_action_of_set_nw_ttl)),
    ...
    MK_FLOW_ACTION(OF_PUSH_VLAN, sizeof(struct rte_flow_action_of_push_vlan)),
    MK_FLOW_ACTION(OF_SET_VLAN_VID,
                   sizeof(struct rte_flow_action_of_set_vlan_vid)),
    MK_FLOW_ACTION(OF_SET_VLAN_PCP,
                   sizeof(struct rte_flow_action_of_set_vlan_pcp)),
    MK_FLOW_ACTION(OF_POP_MPLS, sizeof(struct rte_flow_action_of_pop_mpls)),
    MK_FLOW_ACTION(OF_PUSH_MPLS, sizeof(struct rte_flow_action_of_push_mpls)),
    ...
    MK_FLOW_ACTION(SET_IPV4_SRC, sizeof(struct rte_flow_action_set_ipv4)),
    MK_FLOW_ACTION(SET_IPV4_DST, sizeof(struct rte_flow_action_set_ipv4)),
    MK_FLOW_ACTION(SET_IPV6_SRC, sizeof(struct rte_flow_action_set_ipv6)),
    MK_FLOW_ACTION(SET_IPV6_DST, sizeof(struct rte_flow_action_set_ipv6)),
    MK_FLOW_ACTION(SET_TP_SRC, sizeof(struct rte_flow_action_set_tp)),
    MK_FLOW_ACTION(SET_TP_DST, sizeof(struct rte_flow_action_set_tp)),
    ...
    MK_FLOW_ACTION(MODIFY_FIELD, sizeof(struct rte_flow_action_modify_field)),
    /**
...
static inline void fts_enter(struct rte_eth_dev *dev) {
  if (!(dev->data->dev_flags & RTE_ETH_DEV_FLOW_OPS_THREAD_SAFE))
    pthread_mutex_lock(&dev->data->flow_ops_mutex);
}

static inline void fts_exit(struct rte_eth_dev *dev) {
  if (!(dev->data->dev_flags & RTE_ETH_DEV_FLOW_OPS_THREAD_SAFE))
    pthread_mutex_unlock(&dev->data->flow_ops_mutex);
}

static int flow_err(uint16_t port_id, int ret, struct rte_flow_error *error) {
  if (ret == 0)
    return 0;
...
const struct rte_flow_ops *rte_flow_ops_get(uint16_t port_id,
  ...
  if (code != 0) {
    rte_flow_error_set(error, code, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                       rte_strerror(code));
    return NULL;
  }
  return ops;
...
int rte_flow_validate(uint16_t port_id, const struct rte_flow_attr *attr,
  ...
  return rte_flow_error_set(error, ENOSYS, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
...
int rte_flow_destroy(uint16_t port_id, struct rte_flow *flow,
                     struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
  int ret;
...
int rte_flow_query(uint16_t port_id, struct rte_flow *flow,
...
int rte_flow_error_set(struct rte_flow_error *error, int code,
                       enum rte_flow_error_type type, const void *cause,
                       const char *message) {
  if (error) {
 ...
 *   enough.
 */
static size_t rte_flow_conv_item_spec(void *buf, const size_t size,
                                      const struct rte_flow_item *item,
                                      enum rte_flow_conv_item_spec_type type) {
  size_t off;
  const void *data = type == RTE_FLOW_CONV_ITEM_SPEC   ? item->spec
  ...
  case RTE_FLOW_ITEM_TYPE_RAW:
    ...
    if (tmp) {
      ...
      if (size >= off + tmp)
        dst.raw->pattern = rte_memcpy((void *)((uintptr_t)dst.raw + off),
                                      src.raw->pattern, tmp);
      off += tmp;
    }
    break;
  default:
    off = rte_flow_conv_copy(buf, data, size, rte_flow_desc_item, item->type);
    break;
  }
  return off;
}
 ...
 *   enough.
 */
static size_t rte_flow_conv_action_conf(void *buf, const size_t size,
  ...
  case RTE_FLOW_ACTION_TYPE_RSS:
    ...
    if (src.rss->key_len && src.rss->key) {
      ...
      if (size >= off + tmp)
        dst.rss->key =
            rte_memcpy((void *)((uintptr_t)dst.rss + off), src.rss->key, tmp);
      off += tmp;
    }
    if (src.rss->queue_num) {
      ...
      if (size >= off + tmp)
        dst.rss->queue =
            rte_memcpy((void *)((uintptr_t)dst.rss + off), src.rss->queue, tmp);
      off += tmp;
    }
    break;
  ...
  case RTE_FLOW_ACTION_TYPE_NVGRE_ENCAP:
    ...
    dst.vxlan_encap = buf;
    RTE_BUILD_BUG_ON(
        sizeof(*src.vxlan_encap) != sizeof(*src.nvgre_encap) ||
        offsetof(struct rte_flow_action_vxlan_encap, definition) !=
            offsetof(struct rte_flow_action_nvgre_encap, definition));
    off = sizeof(*dst.vxlan_encap);
    if (src.vxlan_encap->definition) {
      off = RTE_ALIGN_CEIL(off, sizeof(*dst.vxlan_encap->definition));
      ret = rte_flow_conv(
          RTE_FLOW_CONV_OP_PATTERN, (void *)((uintptr_t)dst.vxlan_encap + off),
          size > off ? size - off : 0, src.vxlan_encap->definition, NULL);
      if (ret < 0)
        return 0;
      if (size >= off + ret)
    ...
    break;
  default:
    off = rte_flow_conv_copy(buf, action->conf, size, rte_flow_desc_action,
                             action->type);
    break;
  }
  return off;
 ...
 */
static int rte_flow_conv_pattern(struct rte_flow_item *dst, const size_t size,
  ...
  for (i = 0, off = 0; !num || i != num; ++i, ++src, ++dst) {
    ...
    if (((int)src->type >= 0) &&
      ...
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, src,
  ...
  do {
    if (src->spec) {
      ...
      ret = rte_flow_conv_item_spec((void *)(data + off),
 ...
 *   rte_errno is set.
 */
static int rte_flow_conv_actions(struct rte_flow_action *dst, const size_t size,
  ...
  for (i = 0, off = 0; !num || i != num; ++i, ++src, ++dst) {
    ...
    if (((int)src->type >= 0) &&
      ...
      return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, src,
  ...
  do {
    if (src->conf) {
      off = RTE_ALIGN_CEIL(off, sizeof(double));
      ret = rte_flow_conv_action_conf((void *)(data + off),
                                      size > off ? size - off : 0, src);
      if (size && size >= off + ret)
 ...
 */
static int rte_flow_conv_rule(struct rte_flow_conv_rule *dst, const size_t size,
  ...
  if (src->attr_ro) {
    ...
    if (size && size >= off + sizeof(*dst->attr))
      dst->attr = rte_memcpy((void *)((uintptr_t)dst + off), src->attr_ro,
                             sizeof(*dst->attr));
    off += sizeof(*dst->attr);
  }
  if (src->pattern_ro) {
    ...
    ret = rte_flow_conv_pattern((void *)((uintptr_t)dst + off),
                                size > off ? size - off : 0, src->pattern_ro, 0,
                                error);
    if (ret < 0)
      return ret;
  ...
  if (src->actions_ro) {
    ...
    ret = rte_flow_conv_actions((void *)((uintptr_t)dst + off),
                                size > off ? size - off : 0, src->actions_ro, 0,
                                error);
    if (ret < 0)
      return ret;
 ...
 *   otherwise and rte_errno is set.
 */
static int rte_flow_conv_name(int is_action, int is_ptr, char *dst,
                              const size_t size, const void *src,
                              struct rte_flow_error *error) {
  struct desc_info {
...
int rte_flow_conv(enum rte_flow_conv_op op, void *dst, size_t size,
  ...
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, "unknown object conversion operation");
}

/** Store a full rte_flow description. */
size_t rte_flow_copy(struct rte_flow_desc *desc, size_t len,
                     const struct rte_flow_attr *attr,
  ...
  if (dst_size &&
      (&dst->pattern != &desc->items || &dst->actions != &desc->actions ||
...
int rte_flow_dev_dump(uint16_t port_id, struct rte_flow *flow, FILE *file,
                      struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
  int ret;
...
int rte_flow_get_aged_flows(uint16_t port_id, void **contexts,
...
struct rte_flow_action_handle *rte_flow_action_handle_create(
    uint16_t port_id, const struct rte_flow_indir_action_conf *conf,
  ...
  handle =
      ops->action_handle_create(&rte_eth_devices[port_id], conf, action, error);
  if (handle == NULL)
    flow_err(port_id, -rte_errno, error);
  return handle;
}

int rte_flow_action_handle_destroy(uint16_t port_id,
                                   struct rte_flow_action_handle *handle,
                                   struct rte_flow_error *error) {
  int ret;
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
  ...
  if (unlikely(!ops->action_handle_destroy))
    return rte_flow_error_set(error, ENOSYS, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, rte_strerror(ENOSYS));
  ret = ops->action_handle_destroy(&rte_eth_devices[port_id], handle, error);
  return flow_err(port_id, ret, error);
}

int rte_flow_action_handle_update(uint16_t port_id,
                                  struct rte_flow_action_handle *handle,
                                  const void *update,
                                  struct rte_flow_error *error) {
  int ret;
  ...
  if (unlikely(!ops->action_handle_update))
    return rte_flow_error_set(error, ENOSYS, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, rte_strerror(ENOSYS));
  ret = ops->action_handle_update(&rte_eth_devices[port_id], handle, update,
                                  error);
  return flow_err(port_id, ret, error);
}

int rte_flow_action_handle_query(uint16_t port_id,
  ...
  if (unlikely(!ops->action_handle_query))
    return rte_flow_error_set(error, ENOSYS, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                              NULL, rte_strerror(ENOSYS));
  ret =
      ops->action_handle_query(&rte_eth_devices[port_id], handle, data, error);
  return flow_err(port_id, ret, error);
}

int rte_flow_tunnel_decap_set(uint16_t port_id, struct rte_flow_tunnel *tunnel,
  ...
  if (likely(!!ops->tunnel_decap_set)) {
    return flow_err(
        port_id,
        ops->tunnel_decap_set(dev, tunnel, actions, num_of_actions, error),
        error);
  }
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, rte_strerror(ENOTSUP));
...
int rte_flow_tunnel_match(uint16_t port_id, struct rte_flow_tunnel *tunnel,
                          struct rte_flow_item **items, uint32_t *num_of_items,
                          struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
  ...
  if (likely(!!ops->tunnel_match)) {
    return flow_err(port_id,
                    ops->tunnel_match(dev, tunnel, items, num_of_items, error),
                    error);
  }
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, rte_strerror(ENOTSUP));
...
int rte_flow_get_restore_info(uint16_t port_id, struct rte_mbuf *m,
  ...
  if (likely(!!ops->get_restore_info)) {
    return flow_err(port_id, ops->get_restore_info(dev, m, restore_info, error),
                    error);
  }
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, rte_strerror(ENOTSUP));
...
int rte_flow_tunnel_action_decap_release(uint16_t port_id,
                                         struct rte_flow_action *actions,
                                         uint32_t num_of_actions,
                                         struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  ...
  if (likely(!!ops->tunnel_action_decap_release)) {
    return flow_err(
        port_id,
        ops->tunnel_action_decap_release(dev, actions, num_of_actions, error),
        error);
  }
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, rte_strerror(ENOTSUP));
...
int rte_flow_tunnel_item_release(uint16_t port_id, struct rte_flow_item *items,
  ...
  if (likely(!!ops->tunnel_item_release)) {
    return flow_err(port_id,
                    ops->tunnel_item_release(dev, items, num_of_items, error),
                    error);
  }
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, rte_strerror(ENOTSUP));
...
int rte_flow_pick_transfer_proxy(uint16_t port_id, uint16_t *proxy_port_id,
                                 struct rte_flow_error *error) {
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
  ...
  return flow_err(port_id, ops->pick_transfer_proxy(dev, proxy_port_id, error),
                  error);
}
...
int rte_flow_flex_item_release(uint16_t port_id,
...
int rte_flow_info_get(uint16_t port_id, struct rte_flow_port_info *port_info,
  ...
  if (dev->data->dev_configured == 0) {
    RTE_FLOW_LOG(INFO, "Device with port_id=%" PRIu16 " is not configured.\n",
                 port_id);
    return -EINVAL;
  }
  if (port_info == NULL) {
  ...
  if (likely(!!ops->info_get)) {
    return flow_err(port_id, ops->info_get(dev, port_info, queue_info, error),
                    error);
  }
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, rte_strerror(ENOTSUP));
...
int rte_flow_configure(uint16_t port_id,
  ...
  if (dev->data->dev_started != 0) {
    RTE_FLOW_LOG(INFO, "Device with port_id=%" PRIu16 " already started.\n",
                 port_id);
    return -EINVAL;
  }
  if (port_attr == NULL) {
...
struct rte_flow_pattern_template *rte_flow_pattern_template_create(
    uint16_t port_id,
  ...
  if (dev->data->flow_configured == 0) {
    RTE_FLOW_LOG(INFO,
                 ...
                 port_id);
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_STATE, NULL,
                       rte_strerror(EINVAL));
    return NULL;
  }
  if (template_attr == NULL) {
    RTE_FLOW_LOG(ERR, "Port %" PRIu16 " template attr is NULL.\n", port_id);
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, NULL,
                       rte_strerror(EINVAL));
    return NULL;
  }
  if (pattern == NULL) {
    RTE_FLOW_LOG(ERR, "Port %" PRIu16 " pattern is NULL.\n", port_id);
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, NULL,
                       rte_strerror(EINVAL));
    return NULL;
  }
  if (likely(!!ops->pattern_template_create)) {
    template = ops->pattern_template_create(dev, template_attr, pattern, error);
    if (template == NULL)
      flow_err(port_id, -rte_errno, error);
    return template;
  }
...
int rte_flow_pattern_template_destroy(
    uint16_t port_id, struct rte_flow_pattern_template *pattern_template,
    struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
...
struct rte_flow_actions_template *rte_flow_actions_template_create(
    uint16_t port_id,
  ...
  if (actions == NULL) {
    RTE_FLOW_LOG(ERR, "Port %" PRIu16 " actions is NULL.\n", port_id);
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, NULL,
                       rte_strerror(EINVAL));
    return NULL;
  }
  if (masks == NULL) {
    RTE_FLOW_LOG(ERR, "Port %" PRIu16 " masks is NULL.\n", port_id);
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, NULL,
                       rte_strerror(EINVAL));
  }
  if (likely(!!ops->actions_template_create)) {
    template =
        ops->actions_template_create(dev, template_attr, actions, masks, error);
    if (template == NULL)
      flow_err(port_id, -rte_errno, error);
    return template;
  }
...
int rte_flow_actions_template_destroy(
    uint16_t port_id, struct rte_flow_actions_template *actions_template,
    struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
...
struct rte_flow_template_table *rte_flow_template_table_create(
    uint16_t port_id, const struct rte_flow_template_table_attr *table_attr,
  ...
  if (table_attr == NULL) {
    RTE_FLOW_LOG(ERR, "Port %" PRIu16 " table attr is NULL.\n", port_id);
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, NULL,
                       rte_strerror(EINVAL));
    return NULL;
  }
  if (pattern_templates == NULL) {
    RTE_FLOW_LOG(ERR, "Port %" PRIu16 " pattern templates is NULL.\n", port_id);
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, NULL,
                       rte_strerror(EINVAL));
    return NULL;
  }
  if (actions_templates == NULL) {
    RTE_FLOW_LOG(ERR, "Port %" PRIu16 " actions templates is NULL.\n", port_id);
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, NULL,
                       rte_strerror(EINVAL));
    return NULL;
  }
  if (likely(!!ops->template_table_create)) {
    table = ops->template_table_create(dev, table_attr, pattern_templates,
                                       nb_pattern_templates, actions_templates,
...
int rte_flow_template_table_destroy(
    uint16_t port_id, struct rte_flow_template_table *template_table,
    struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
  ...
  if (likely(!!ops->template_table_destroy)) {
    return flow_err(port_id,
                    ops->template_table_destroy(dev, template_table, error),
                    error);
  }
  return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
                            NULL, rte_strerror(ENOTSUP));
...
struct rte_flow *rte_flow_async_create(
    uint16_t port_id, uint32_t queue_id, const struct rte_flow_op_attr *op_attr,
  ...
  flow = ops->async_create(dev, queue_id, op_attr, template_table, pattern,
...
int rte_flow_async_destroy(uint16_t port_id, uint32_t queue_id,
  ...
  return flow_err(
      port_id,
      ops->async_destroy(dev, queue_id, op_attr, flow, user_data, error),
      error);
}

int rte_flow_push(uint16_t port_id, uint32_t queue_id,
                  struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);

  return flow_err(port_id, ops->push(dev, queue_id, error), error);
}

int rte_flow_pull(uint16_t port_id, uint32_t queue_id,
...
struct rte_flow_action_handle *rte_flow_async_action_handle_create(
    uint16_t port_id, uint32_t queue_id, const struct rte_flow_op_attr *op_attr,
  ...
  handle = ops->async_action_handle_create(
      dev, queue_id, op_attr, indir_action_conf, action, user_data, error);
  if (handle == NULL)
    flow_err(port_id, -rte_errno, error);
  return handle;
}

int rte_flow_async_action_handle_destroy(
    uint16_t port_id, uint32_t queue_id, const struct rte_flow_op_attr *op_attr,
    struct rte_flow_action_handle *action_handle, void *user_data,
    struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
  int ret;

  ret = ops->async_action_handle_destroy(dev, queue_id, op_attr, action_handle,
                                         user_data, error);
  return flow_err(port_id, ret, error);
}

int rte_flow_async_action_handle_update(
    uint16_t port_id, uint32_t queue_id, const struct rte_flow_op_attr *op_attr,
    struct rte_flow_action_handle *action_handle, const void *update,
    void *user_data, struct rte_flow_error *error) {
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
  int ret;

  ret = ops->async_action_handle_update(dev, queue_id, op_attr, action_handle,
