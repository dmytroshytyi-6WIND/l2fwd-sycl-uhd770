...
static int eth_mac_cmp(const char *key __rte_unused, const char *value,
                       void *opaque) {
...
static int eth_representor_cmp(const char *key __rte_unused, const char *value,
                               void *opaque) {
  int ret;
  char *values;
  ...
  for (i = 0; i < nc * np * nf; ++i) {
    ...
    p = (i / nf) % np;
    f = i % nf;
    if (rte_eth_representor_id_get(
            edev->data->backer_port_id, eth_da.type,
            eth_da.nb_mh_controllers == 0 ? -1 : eth_da.mh_controllers[c],
            eth_da.nb_ports == 0 ? -1 : eth_da.ports[p],
            eth_da.nb_representor_ports == 0 ? -1 : eth_da.representor_ports[f],
            &id) < 0)
      continue;
...
static int eth_dev_match(const struct rte_eth_dev *edev, const void *_arg) {
  int ret;
  ...
  ret = rte_kvargs_process(kvlist, eth_params_keys[RTE_ETH_PARAM_MAC],
                           eth_mac_cmp, edev->data);
  if (ret != 0)
    return -1;

  ret = rte_kvargs_process(kvlist, eth_params_keys[RTE_ETH_PARAM_REPRESENTOR],
...
static void *eth_dev_iterate(const void *start, const char *str,
