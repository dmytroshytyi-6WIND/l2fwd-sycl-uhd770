 ...
 */
uint16_t profile_hook_rx_burst_cb(__rte_unused uint16_t port_id,
 ...
 */
static inline int vtune_profile_rx_init(uint16_t port_id,
                                        uint8_t rx_queue_num) {
  uint16_t q_id;

  for (q_id = 0; q_id < rx_queue_num; ++q_id) {
    if (!rte_eth_add_rx_callback(port_id, q_id, profile_hook_rx_burst_cb,
                                 NULL)) {
      return -rte_errno;
    }
  }
...
int __rte_eth_dev_profile_init(__rte_unused uint16_t port_id,
