...
#define iter_anybus_str "class=eth,"
  if (strncmp(devargs_str, iter_anybus_str, strlen(iter_anybus_str)) == 0) {
  ...
  ret = snprintf(bus_str, str_size, "%s=%s", bus_param_key, devargs.name);
  if (ret != str_size - 1) {
    ret = -EINVAL;
    goto error;
  }
...
uint16_t rte_eth_iterator_next(struct rte_dev_iterator *iter) {
  if (iter == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get next device from NULL iterator\n");
  ...
  do { /* loop to try all matching rte_device */
    ...
    if (iter->bus != NULL &&
      ...
      iter->device = iter->bus->dev_iterate(iter->device, iter->bus_str, iter);
      if (iter->device == NULL)
        break; /* no more rte_device candidate */
    }
    /* A device is matching bus part, need to check ethdev part. */
    iter->class_device =
        iter->cls->dev_iterate(iter->class_device, iter->cls_str, iter);
...
uint16_t rte_eth_find_next_of(uint16_t port_id,
...
uint16_t rte_eth_find_next_sibling(uint16_t port_id, uint16_t ref_port_id) {
  RTE_ETH_VALID_PORTID_OR_ERR_RET(ref_port_id, RTE_MAX_ETHPORTS);
  return rte_eth_find_next_of(port_id, rte_eth_devices[ref_port_id].device);
}

static bool eth_dev_is_allocated(const struct rte_eth_dev *ethdev) {
...
static int eth_is_valid_owner_id(uint64_t owner_id) {
...
static int eth_dev_owner_set(const uint16_t port_id,
  ...
  if (new_owner == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot set ethdev port %u owner from NULL owner\n",
                   port_id);
    return -EINVAL;
  }
  ...
  if (!eth_is_valid_owner_id(new_owner->id) &&
    ...
    RTE_ETHDEV_LOG(
        ERR, "Invalid owner old_id=%016" PRIx64 " new_id=%016" PRIx64 "\n",
  ...
  if (port_owner->id != old_owner_id) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot set owner to port %u already owned by %s_%016" PRIX64 "\n",
...
int rte_eth_dev_owner_unset(const uint16_t port_id, const uint64_t owner_id) {
  const struct rte_eth_dev_owner new_owner =
      (struct rte_eth_dev_owner){.id = RTE_ETH_DEV_NO_OWNER, .name = ""};
  int ret;
  ...
  if (eth_is_valid_owner_id(owner_id)) {
    for (port_id = 0; port_id < RTE_MAX_ETHPORTS; port_id++) {
      ...
      if (data != NULL && data->owner.id == owner_id)
        memset(&data->owner, 0, sizeof(struct rte_eth_dev_owner));
    }
    RTE_ETHDEV_LOG(NOTICE,
                   "All port owners owned by %016" PRIx64
                   " identifier have removed\n",
                   owner_id);
  } else {
    RTE_ETHDEV_LOG(ERR, "Invalid owner ID=%016" PRIx64 "\n", owner_id);
    ret = -EINVAL;
  }
...
int rte_eth_dev_get_port_by_name(const char *name, uint16_t *port_id) {
  ...
  if (port_id == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get port ID to NULL for %s\n", name);
    return -EINVAL;
  }
...
static int eth_err(uint16_t port_id, int ret) {
  if (ret == 0)
    return 0;
...
static int eth_dev_validate_rx_queue(const struct rte_eth_dev *dev,
                                     uint16_t rx_queue_id) {
  uint16_t port_id;

  if (rx_queue_id >= dev->data->nb_rx_queues) {
    port_id = dev->data->port_id;
    RTE_ETHDEV_LOG(ERR, "Invalid Rx queue_id=%u of device with port_id=%u\n",
                   rx_queue_id, port_id);
    return -EINVAL;
  }
...
static int eth_dev_validate_tx_queue(const struct rte_eth_dev *dev,
                                     uint16_t tx_queue_id) {
  uint16_t port_id;

  if (tx_queue_id >= dev->data->nb_tx_queues) {
    port_id = dev->data->port_id;
    RTE_ETHDEV_LOG(ERR, "Invalid Tx queue_id=%u of device with port_id=%u\n",
                   tx_queue_id, port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_rx_queue_start(uint16_t port_id, uint16_t rx_queue_id) {
  ...
  if (!dev->data->dev_started) {
    RTE_ETHDEV_LOG(ERR, "Port %u must be started before start any queue\n",
                   port_id);
    return -EINVAL;
  }
  ...
  if (rte_eth_dev_is_rx_hairpin_queue(dev, rx_queue_id)) {
    RTE_ETHDEV_LOG(INFO,
                   "Can't start Rx hairpin queue %" PRIu16
...
int rte_eth_dev_rx_queue_stop(uint16_t port_id, uint16_t rx_queue_id) {
  ...
  if (rte_eth_dev_is_rx_hairpin_queue(dev, rx_queue_id)) {
    RTE_ETHDEV_LOG(INFO,
                   "Can't stop Rx hairpin queue %" PRIu16
                   " of device with port_id=%" PRIu16 "\n",
                   rx_queue_id, port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_tx_queue_start(uint16_t port_id, uint16_t tx_queue_id) {
  ...
  if (rte_eth_dev_is_tx_hairpin_queue(dev, tx_queue_id)) {
    RTE_ETHDEV_LOG(INFO,
                   "Can't start Tx hairpin queue %" PRIu16
                   " of device with port_id=%" PRIu16 "\n",
                   tx_queue_id, port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_tx_queue_stop(uint16_t port_id, uint16_t tx_queue_id) {
  ...
  if (rte_eth_dev_is_tx_hairpin_queue(dev, tx_queue_id)) {
    RTE_ETHDEV_LOG(INFO,
                   "Can't stop Tx hairpin queue %" PRIu16
                   " of device with port_id=%" PRIu16 "\n",
                   tx_queue_id, port_id);
    return -EINVAL;
  }
...
static inline int eth_dev_check_lro_pkt_size(uint16_t port_id,
  ...
  if (dev_info_size == 0) {
    if (config_size != max_rx_pkt_len) {
      RTE_ETHDEV_LOG(ERR,
                     "Ethdev port_id=%d max_lro_pkt_size"
                     " %u != %u is not allowed\n",
    ...
    RTE_ETHDEV_LOG(ERR,
                   "Ethdev port_id=%d max_lro_pkt_size %u "
                   "> max allowed value %u\n",
    ...
    RTE_ETHDEV_LOG(ERR,
                   "Ethdev port_id=%d max_lro_pkt_size %u "
                   "< min allowed value %u\n",
                   port_id, config_size, (unsigned int)RTE_ETHER_MIN_LEN);
    ret = -EINVAL;
  }
  return ret;
}
 ...
 *
 */
static int eth_dev_validate_offloads(uint16_t port_id, uint64_t req_offloads,
                                     uint64_t set_offloads,
                                     const char *offload_type,
  ...
  while (offloads_diff != 0) {
    ...
    if (offload & req_offloads) {
      RTE_ETHDEV_LOG(ERR, "Port %u failed to enable %s offload %s\n", port_id,
                     offload_type, offload_name(offload));
      ret = -EINVAL;
    }
...
static int eth_dev_validate_mtu(uint16_t port_id,
  ...
  if (mtu < dev_info->min_mtu) {
    RTE_ETHDEV_LOG(ERR, "MTU (%u) < device min MTU (%u) for port_id %u\n", mtu,
                   dev_info->min_mtu, port_id);
    return -EINVAL;
  }
  if (mtu > dev_info->max_mtu) {
    RTE_ETHDEV_LOG(ERR, "MTU (%u) > device max MTU (%u) for port_id %u\n", mtu,
  ...
  if (frame_size > dev_info->max_rx_pktlen) {
    RTE_ETHDEV_LOG(
        ERR, "Frame size (%u) > device max frame size (%u) for port_id %u\n",
...
int rte_eth_dev_configure(uint16_t port_id, uint16_t nb_rx_q, uint16_t nb_tx_q,
  ...
  if (dev_conf == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot configure ethdev port %u from NULL config\n",
                   port_id);
    return -EINVAL;
  }
  ...
  if (dev->data->dev_started) {
    RTE_ETHDEV_LOG(ERR, "Port %u must be stopped to allow configuration\n",
                   port_id);
    return -EBUSY;
  }
  ...
  if (nb_tx_q > dev_info.max_tx_queues) {
    RTE_ETHDEV_LOG(ERR, "Ethdev port_id=%u nb_tx_queues=%u > %u\n", port_id,
                   nb_tx_q, dev_info.max_tx_queues);
    ret = -EINVAL;
    goto rollback;
  }
  ...
  ret =
      eth_dev_validate_mtu(port_id, &dev_info, dev->data->dev_conf.rxmode.mtu);
  if (ret != 0)
    goto rollback;
  ...
  if (dev_conf->rxmode.offloads & RTE_ETH_RX_OFFLOAD_TCP_LRO) {
    ...
    ret = eth_dev_check_lro_pkt_size(
        port_id, dev->data->dev_conf.rxmode.max_lro_pkt_size, max_rx_pktlen,
  ...
  if ((dev_conf->rxmode.offloads & dev_info.rx_offload_capa) !=
    ...
    RTE_ETHDEV_LOG(ERR,
                   ...
                   port_id, dev_conf->rxmode.offloads, dev_info.rx_offload_capa,
                   __func__);
    ret = -EINVAL;
    goto rollback;
  }
  if ((dev_conf->txmode.offloads & dev_info.tx_offload_capa) !=
    ...
    RTE_ETHDEV_LOG(ERR,
                   ...
                   "capabilities 0x%" PRIx64 " in %s()\n",
                   port_id, dev_conf->txmode.offloads, dev_info.tx_offload_capa,
                   __func__);
    ret = -EINVAL;
    goto rollback;
  }
  ...
  if ((dev_info.flow_type_rss_offloads |
    ...
    RTE_ETHDEV_LOG(ERR,
                   "Ethdev port_id=%u invalid rss_hf: 0x%" PRIx64
  ...
  if (((dev_conf->rxmode.mq_mode & RTE_ETH_MQ_RX_RSS_FLAG) == 0) &&
    ...
    RTE_ETHDEV_LOG(ERR,
                   "Ethdev port_id=%u config invalid Rx mq_mode without RSS "
                   "but %s offload is requested\n",
                   port_id,
  ...
  if (diag != 0) {
    RTE_ETHDEV_LOG(ERR, "Port%u eth_dev_rx_queue_config = %d\n", port_id, diag);
    ret = diag;
    goto rollback;
  }
  ...
  if (diag != 0) {
    RTE_ETHDEV_LOG(ERR, "Port%u eth_dev_tx_queue_config = %d\n", port_id, diag);
  ...
  diag = eth_dev_validate_offloads(port_id, dev_conf->rxmode.offloads,
  ...
  diag = eth_dev_validate_offloads(port_id, dev_conf->txmode.offloads,
...
static void eth_dev_mac_restore(struct rte_eth_dev *dev,
                                struct rte_eth_dev_info *dev_info) {
...
static int eth_dev_config_restore(struct rte_eth_dev *dev,
                                  struct rte_eth_dev_info *dev_info,
                                  uint16_t port_id) {
  int ret;
  ...
  if (rte_eth_promiscuous_get(port_id) == 1 &&
    ...
    if (ret != 0 && ret != -ENOTSUP) {
      RTE_ETHDEV_LOG(
          ERR, "Failed to enable promiscuous mode for device (port %u): %s\n",
    ...
    if (ret != 0 && ret != -ENOTSUP) {
      RTE_ETHDEV_LOG(
          ERR, "Failed to disable promiscuous mode for device (port %u): %s\n",
          port_id, rte_strerror(-ret));
      return ret;
    }
  }
  ...
  if (rte_eth_allmulticast_get(port_id) == 1 &&
    ...
    if (ret != 0 && ret != -ENOTSUP) {
      RTE_ETHDEV_LOG(
          ERR, "Failed to enable allmulticast mode for device (port %u): %s\n",
          port_id, rte_strerror(-ret));
      return ret;
    }
    ...
    if (ret != 0 && ret != -ENOTSUP) {
      RTE_ETHDEV_LOG(
          ERR, "Failed to disable allmulticast mode for device (port %u): %s\n",
          port_id, rte_strerror(-ret));
      return ret;
    }
  }
...
int rte_eth_dev_start(uint16_t port_id) {
  ...
  if (dev->data->dev_configured == 0) {
    RTE_ETHDEV_LOG(INFO, "Device with port_id=%" PRIu16 " is not configured.\n",
                   port_id);
    return -EINVAL;
  }
  ...
  if (ret != 0) {
    RTE_ETHDEV_LOG(
        ERR, "Error during restoring configuration for device (port %u): %s\n",
        port_id, rte_strerror(-ret));
    ret_stop = rte_eth_dev_stop(port_id);
    if (ret_stop != 0) {
      RTE_ETHDEV_LOG(ERR, "Failed to stop device (port %u): %s\n", port_id,
                     rte_strerror(-ret_stop));
    }
...
int rte_eth_dev_reset(uint16_t port_id) {
  ...
  if (ret != 0) {
    RTE_ETHDEV_LOG(
        ERR, "Failed to stop device (port %u) before reset: %s - ignore\n",
        port_id, rte_strerror(-ret));
  }
...
rte_eth_rx_queue_check_split(const struct rte_eth_rxseg_split *rx_seg,
  ...
  if (n_seg > seg_capa->max_nseg) {
    RTE_ETHDEV_LOG(ERR, "Requested Rx segments %u exceed supported %u\n", n_seg,
                   seg_capa->max_nseg);
    return -EINVAL;
  }
  /*
  ...
  for (seg_idx = 0; seg_idx < n_seg; seg_idx++) {
    ...
    if (seg_idx != 0 && mp_first != mpl && seg_capa->multi_pools == 0) {
    ...
    if (offset != 0) {
      ...
      if (offset & offset_mask) {
        RTE_ETHDEV_LOG(ERR, "Rx segmentation invalid offset alignment %u, %u\n",
                       offset, seg_capa->offset_align_log2);
        return -EINVAL;
      }
    }
    if (mpl->private_data_size < sizeof(struct rte_pktmbuf_pool_private)) {
      RTE_ETHDEV_LOG(ERR, "%s private_data_size %u < %u\n", mpl->name,
                     mpl->private_data_size,
                     (unsigned int)sizeof(struct rte_pktmbuf_pool_private));
      return -ENOSPC;
    }
    ...
    *mbp_buf_size = rte_pktmbuf_data_room_size(mpl);
    ...
    if (*mbp_buf_size < length + offset) {
      RTE_ETHDEV_LOG(ERR,
                     "%s mbuf_data_room_size %u < %u (segment length=%u + "
                     "segment offset=%u)\n",
                     mpl->name, *mbp_buf_size, length + offset, length, offset);
      return -EINVAL;
    }
  }
  return 0;
...
int rte_eth_rx_queue_setup(uint16_t port_id, uint16_t rx_queue_id,
  ...
  if (mp != NULL) {
    ...
    if (rx_conf != NULL && rx_conf->rx_nseg != 0) {
      RTE_ETHDEV_LOG(ERR, "Ambiguous segment configuration\n");
      return -EINVAL;
    }
    /*
    ...
    if (mbp_buf_size < dev_info.min_rx_bufsize + RTE_PKTMBUF_HEADROOM) {
      RTE_ETHDEV_LOG(ERR,
                     "%s mbuf_data_room_size %u < %u (RTE_PKTMBUF_HEADROOM=%u "
    ...
    if (rx_conf == NULL || rx_conf->rx_seg == NULL || rx_conf->rx_nseg == 0) {
      RTE_ETHDEV_LOG(
          ERR, "Memory pool is null and no extended configuration provided\n");
      return -EINVAL;
    }
    ...
    if (rx_conf->offloads & RTE_ETH_RX_OFFLOAD_BUFFER_SPLIT) {
      ret =
          rte_eth_rx_queue_check_split(rx_seg, n_seg, &mbp_buf_size, &dev_info);
      if (ret != 0)
        return ret;
    } else {
  ...
  if ((local_conf.offloads & dev_info.rx_queue_offload_capa) !=
    ...
    RTE_ETHDEV_LOG(
        ERR,
        "Ethdev port_id=%d rx_queue_id=%d, new added offloads 0x%" PRIx64
        " must be "
  ...
  if (local_conf.share_group > 0 &&
    ...
    RTE_ETHDEV_LOG(ERR,
                   "Ethdev port_id=%d rx_queue_id=%d, enabled share_group=%hu "
                   "while device doesn't support Rx queue share\n",
  ...
  ret = (*dev->dev_ops->rx_queue_setup)(dev, rx_queue_id, nb_rx_desc, socket_id,
                                        &local_conf, mp);
  if (!ret) {
  ...
  rte_ethdev_trace_rxq_setup(port_id, rx_queue_id, nb_rx_desc, mp, rx_conf,
                             ret);
  return eth_err(port_id, ret);
}

int rte_eth_rx_hairpin_queue_setup(uint16_t port_id, uint16_t rx_queue_id,
                                   uint16_t nb_rx_desc,
  ...
  if (conf == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot setup ethdev port %u Rx hairpin queue from NULL config\n",
        port_id);
    return -EINVAL;
  }
  ...
  if (ret != 0)
    return ret;
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->rx_hairpin_queue_setup, -ENOTSUP);
  /* if nb_rx_desc is zero use max number of desc from the driver. */
  if (nb_rx_desc == 0)
    nb_rx_desc = cap.max_nb_desc;
  if (nb_rx_desc > cap.max_nb_desc) {
    RTE_ETHDEV_LOG(ERR, "Invalid value for nb_rx_desc(=%hu), should be: <= %hu",
                   nb_rx_desc, cap.max_nb_desc);
    return -EINVAL;
  }
  ...
  for (i = 0, count = 0;
       i < dev->data->nb_rx_queues && cap.max_nb_queues != UINT16_MAX; i++) {
  ...
  if (dev->data->dev_started)
    return -EBUSY;
  eth_dev_rxq_release(dev, rx_queue_id);
  ret = (*dev->dev_ops->rx_hairpin_queue_setup)(dev, rx_queue_id, nb_rx_desc,
                                                conf);
  if (ret == 0)
    dev->data->rx_queue_state[rx_queue_id] = RTE_ETH_QUEUE_STATE_HAIRPIN;
  return eth_err(port_id, ret);
}
...
int rte_eth_tx_queue_setup(uint16_t port_id, uint16_t tx_queue_id,
  ...
  if ((local_conf.offloads & dev_info.tx_queue_offload_capa) !=
    ...
    RTE_ETHDEV_LOG(
        ERR,
        "Ethdev port_id=%d tx_queue_id=%d, new added offloads 0x%" PRIx64
        " must be "
        "within per-queue offload capabilities 0x%" PRIx64 " in %s()\n",
  ...
  return eth_err(port_id,
                 (*dev->dev_ops->tx_queue_setup)(dev, tx_queue_id, nb_tx_desc,
                                                 socket_id, &local_conf));
}

int rte_eth_tx_hairpin_queue_setup(uint16_t port_id, uint16_t tx_queue_id,
                                   uint16_t nb_tx_desc,
                                   const struct rte_eth_hairpin_conf *conf) {
  struct rte_eth_dev *dev;
  struct rte_eth_hairpin_cap cap;
  ...
  if (conf == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot setup ethdev port %u Tx hairpin queue from NULL config\n",
        port_id);
    return -EINVAL;
  }
  ...
  if (ret != 0)
    return ret;
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->tx_hairpin_queue_setup, -ENOTSUP);
  /* if nb_rx_desc is zero use max number of desc from the driver. */
  if (nb_tx_desc == 0)
    nb_tx_desc = cap.max_nb_desc;
  if (nb_tx_desc > cap.max_nb_desc) {
    RTE_ETHDEV_LOG(ERR, "Invalid value for nb_tx_desc(=%hu), should be: <= %hu",
                   nb_tx_desc, cap.max_nb_desc);
    return -EINVAL;
  }
  ...
  for (i = 0, count = 0;
       i < dev->data->nb_tx_queues && cap.max_nb_queues != UINT16_MAX; i++) {
  ...
  if (dev->data->dev_started)
    return -EBUSY;
  eth_dev_txq_release(dev, tx_queue_id);
  ret = (*dev->dev_ops->tx_hairpin_queue_setup)(dev, tx_queue_id, nb_tx_desc,
                                                conf);
  if (ret == 0)
    dev->data->tx_queue_state[tx_queue_id] = RTE_ETH_QUEUE_STATE_HAIRPIN;
  return eth_err(port_id, ret);
}
...
int rte_eth_hairpin_get_peer_ports(uint16_t port_id, uint16_t *peer_ports,
  ...
  if (peer_ports == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot get ethdev port %u hairpin peer ports to NULL\n", port_id);
    return -EINVAL;
  }

  if (len == 0) {
    RTE_ETHDEV_LOG(ERR,
                   "Cannot get ethdev port %u hairpin peer ports to array with "
                   "zero size\n",
                   port_id);
    return -EINVAL;
  }

  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->hairpin_get_peer_ports, -ENOTSUP);

  ret =
      (*dev->dev_ops->hairpin_get_peer_ports)(dev, peer_ports, len, direction);
  if (ret < 0)
...
int rte_eth_tx_buffer_set_err_callback(struct rte_eth_dev_tx_buffer *buffer,
                                       ...
                                       void *userdata) {
  if (buffer == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot set Tx buffer error callback to NULL buffer\n");
    return -EINVAL;
  }
...
int rte_eth_link_get(uint16_t port_id, struct rte_eth_link *eth_link) {
  struct rte_eth_dev *dev;
...
int rte_eth_link_get_nowait(uint16_t port_id, struct rte_eth_link *eth_link) {
  struct rte_eth_dev *dev;
...
int rte_eth_link_to_str(char *str, size_t len,
  ...
  if (len == 0) {
    RTE_ETHDEV_LOG(ERR, "Cannot convert link to string with zero size\n");
    return -EINVAL;
  }
  ...
  if (eth_link->link_status == RTE_ETH_LINK_DOWN)
    ...
    return snprintf(
        str, len, "Link up at %s %s %s",
        rte_eth_link_speed_to_str(eth_link->link_speed),
        (eth_link->link_duplex == RTE_ETH_LINK_FULL_DUPLEX) ? "FDX" : "HDX",
        (eth_link->link_autoneg == RTE_ETH_LINK_AUTONEG) ? "Autoneg" : "Fixed");
}
...
static inline int eth_dev_get_xstats_basic_count(struct rte_eth_dev *dev) {
...
static int eth_dev_get_xstats_count(uint16_t port_id) {
  struct rte_eth_dev *dev;
  int count;
...
int rte_eth_xstats_get_id_by_name(uint16_t port_id, const char *xstat_name,
  ...
  if (id == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get ethdev port %u xstats ID to NULL\n",
                   port_id);
    return -ENOMEM;
  }
  ...
  if (cnt_xstats !=
      rte_eth_xstats_get_names_by_id(port_id, xstats_names, cnt_xstats, NULL)) {
...
static int eth_basic_stats_get_names(struct rte_eth_dev *dev,
  ...
  for (idx = 0; idx < RTE_NB_STATS; idx++) {
    strlcpy(xstats_names[cnt_used_entries].name,
            eth_dev_stats_strings[idx].name, sizeof(xstats_names[0].name));
    cnt_used_entries++;
  }
...
int rte_eth_xstats_get_names_by_id(uint16_t port_id,
  ...
  if (ids && dev->dev_ops->xstats_get_names_by_id != NULL && size > 0) {
    ...
    if (no_basic_stat_requested)
      return (*dev->dev_ops->xstats_get_names_by_id)(dev, ids_copy,
                                                     xstats_names, size);
  }
  ...
  if (!ids) {
    int num_stats =
        rte_eth_xstats_get_names(port_id, xstats_names, expected_entries);
  ...
  xstats_names_copy =
      calloc(expected_entries, sizeof(struct rte_eth_xstat_name));
...
int rte_eth_xstats_get_names(uint16_t port_id,
  ...
  if (dev->dev_ops->xstats_get_names != NULL) {
    ...
    cnt_driver_entries = (*dev->dev_ops->xstats_get_names)(
        dev, xstats_names + cnt_used_entries, size - cnt_used_entries);
...
static int eth_basic_stats_get(uint16_t port_id, struct rte_eth_xstat *xstats) {
  struct rte_eth_dev *dev;
  ...
  for (i = 0; i < RTE_NB_STATS; i++) {
    stats_ptr = RTE_PTR_ADD(&eth_stats, eth_dev_stats_strings[i].offset);
    val = *stats_ptr;
  ...
  for (q = 0; q < nb_rxqs; q++) {
    for (i = 0; i < RTE_NB_RXQ_STATS; i++) {
      stats_ptr = RTE_PTR_ADD(&eth_stats, eth_dev_rxq_stats_strings[i].offset +
                                              q * sizeof(uint64_t));
      val = *stats_ptr;
      xstats[count++].value = val;
    }
  ...
  for (q = 0; q < nb_txqs; q++) {
    for (i = 0; i < RTE_NB_TXQ_STATS; i++) {
      stats_ptr = RTE_PTR_ADD(&eth_stats, eth_dev_txq_stats_strings[i].offset +
                                              q * sizeof(uint64_t));
      val = *stats_ptr;
      xstats[count++].value = val;
    }
...
int rte_eth_xstats_get_by_id(uint16_t port_id, const uint64_t *ids,
  ...
  if (ids && dev->dev_ops->xstats_get_by_id != NULL && size) {
    ...
    if (no_basic_stat_requested)
      return (*dev->dev_ops->xstats_get_by_id)(dev, ids_copy, values, size);
  }
...
static int eth_dev_set_queue_stats_mapping(uint16_t port_id, uint16_t queue_id,
...
int rte_eth_dev_set_tx_queue_stats_mapping(uint16_t port_id,
                                           uint16_t tx_queue_id,
                                           uint8_t stat_idx) {
  return eth_err(port_id, eth_dev_set_queue_stats_mapping(
...
int rte_eth_dev_set_rx_queue_stats_mapping(uint16_t port_id,
                                           ...
                                           uint8_t stat_idx) {
  return eth_err(port_id, eth_dev_set_queue_stats_mapping(
                              port_id, rx_queue_id, stat_idx, STAT_QMAP_RX));
}
...
int rte_eth_dev_fw_version_get(uint16_t port_id, char *fw_version,
  ...
  if (fw_version == NULL && fw_size > 0) {
    RTE_ETHDEV_LOG(ERR,
                   "Cannot get ethdev port %u FW version to NULL when string "
                   "size is non zero\n",
                   port_id);
    return -EINVAL;
  }
  ...
  return eth_err(port_id,
                 (*dev->dev_ops->fw_version_get)(dev, fw_version, fw_size));
}
...
int rte_eth_dev_info_get(uint16_t port_id, struct rte_eth_dev_info *dev_info) {
  ...
  dev_info->max_rx_queues =
      RTE_MIN(dev_info->max_rx_queues, RTE_MAX_QUEUES_PER_PORT);
  dev_info->max_tx_queues =
      RTE_MIN(dev_info->max_tx_queues, RTE_MAX_QUEUES_PER_PORT);
...
int rte_eth_dev_conf_get(uint16_t port_id, struct rte_eth_conf *dev_conf) {
  ...
  if (dev_conf == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get ethdev port %u configuration to NULL\n",
                   port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_set_ptypes(uint16_t port_id, uint32_t ptype_mask,
  ...
  if (num > 0 && set_ptypes == NULL) {
    RTE_ETHDEV_LOG(ERR,
                   "Cannot get ethdev port %u set packet types to NULL when "
   ...
   */
  for (i = 0, j = 0; set_ptypes != NULL && (all_ptypes[i] != RTE_PTYPE_UNKNOWN);
       ++i) {
...
int rte_eth_macaddr_get(uint16_t port_id, struct rte_ether_addr *mac_addr) {
  ...
  if (mac_addr == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get ethdev port %u MAC address to NULL\n",
                   port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_set_mtu(uint16_t port_id, uint16_t mtu) {
  ...
  if (dev->data->dev_configured == 0) {
    RTE_ETHDEV_LOG(ERR, "Port %u must be configured before MTU set\n", port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_set_vlan_ether_type(uint16_t port_id,
  ...
  return eth_err(port_id, (*dev->dev_ops->vlan_tpid_set)(dev, vlan_type, tpid));
}
...
int rte_eth_dev_set_vlan_offload(uint16_t port_id, int offload_mask) {
  ...
  if ((dev_offloads & dev_info.rx_offload_capa) != dev_offloads) {
    ...
    RTE_ETHDEV_LOG(ERR,
                   ...
                   "0x%" PRIx64 " in %s()\n",
                   port_id, new_offloads, dev_info.rx_offload_capa, __func__);
    return -EINVAL;
  }
...
int rte_eth_dev_priority_flow_ctrl_set(uint16_t port_id,
  ...
  if (*dev->dev_ops->priority_flow_ctrl_set)
    return eth_err(port_id,
                   (*dev->dev_ops->priority_flow_ctrl_set)(dev, pfc_conf));
  return -ENOTSUP;
}
  ...
  if ((pfc_queue_conf->mode == RTE_ETH_FC_RX_PAUSE) ||
    ...
    if (pfc_queue_conf->rx_pause.tx_qid >= dev_info->nb_tx_queues) {
      RTE_ETHDEV_LOG(
          ...
          pfc_queue_conf->rx_pause.tx_qid, dev_info->nb_tx_queues);
      return -EINVAL;
    }
  ...
  if ((pfc_queue_conf->mode == RTE_ETH_FC_TX_PAUSE) ||
    ...
    if (pfc_queue_conf->tx_pause.rx_qid >= dev_info->nb_rx_queues) {
      RTE_ETHDEV_LOG(
          ...
          pfc_queue_conf->tx_pause.rx_qid, dev_info->nb_rx_queues);
      return -EINVAL;
    }
...
int rte_eth_dev_priority_flow_ctrl_queue_info_get(
...
int rte_eth_dev_priority_flow_ctrl_queue_configure(
  ...
  if (pfc_info.mode_capa == RTE_ETH_FC_FULL ||
    ...
    ret = validate_rx_pause_config(&dev_info, pfc_info.tc_max, pfc_queue_conf);
    if (ret != 0)
      return ret;
  }
  ...
  if (pfc_info.mode_capa == RTE_ETH_FC_FULL ||
    ...
    ret = validate_tx_pause_config(&dev_info, pfc_info.tc_max, pfc_queue_conf);
    if (ret != 0)
      return ret;
  }

  if (*dev->dev_ops->priority_flow_ctrl_queue_config)
    return eth_err(port_id, (*dev->dev_ops->priority_flow_ctrl_queue_config)(
                                dev, pfc_queue_conf));
  return -ENOTSUP;
}

static int eth_check_reta_mask(struct rte_eth_rss_reta_entry64 *reta_conf,
                               uint16_t reta_size) {
  uint16_t i, num;
...
static int eth_check_reta_entry(struct rte_eth_rss_reta_entry64 *reta_conf,
                                uint16_t reta_size, uint16_t max_rxq) {
...
int rte_eth_dev_rss_reta_update(uint16_t port_id,
  ...
  if (reta_conf == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot update ethdev port %u RSS RETA to NULL\n",
                   port_id);
    return -EINVAL;
  }
  ...
  if (reta_size == 0) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot update ethdev port %u RSS RETA with zero size\n", port_id);
    return -EINVAL;
  }
  ...
  /* Check entry value */
  ret = eth_check_reta_entry(reta_conf, reta_size, dev->data->nb_rx_queues);
  if (ret < 0)
    return ret;

  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->reta_update, -ENOTSUP);
  return eth_err(port_id,
                 (*dev->dev_ops->reta_update)(dev, reta_conf, reta_size));
}
...
int rte_eth_dev_rss_reta_query(uint16_t port_id,
  ...
  return eth_err(port_id,
                 (*dev->dev_ops->reta_query)(dev, reta_conf, reta_size));
}
...
int rte_eth_dev_rss_hash_update(uint16_t port_id,
  ...
  if ((dev_info.flow_type_rss_offloads | rss_conf->rss_hf) !=
    ...
    RTE_ETHDEV_LOG(ERR,
                   "Ethdev port_id=%u invalid rss_hf: 0x%" PRIx64
                   ", valid value: 0x%" PRIx64 "\n",
                   port_id, rss_conf->rss_hf, dev_info.flow_type_rss_offloads);
    return -EINVAL;
  }
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->rss_hash_update, -ENOTSUP);
  return eth_err(port_id, (*dev->dev_ops->rss_hash_update)(dev, rss_conf));
}
...
int rte_eth_dev_rss_hash_conf_get(uint16_t port_id,
  ...
  if (rss_conf == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get ethdev port %u RSS hash config to NULL\n",
                   port_id);
    return -EINVAL;
  }

  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->rss_hash_conf_get, -ENOTSUP);
  return eth_err(port_id, (*dev->dev_ops->rss_hash_conf_get)(dev, rss_conf));
}
...
int rte_eth_dev_udp_tunnel_port_add(uint16_t port_id,
  ...
  if (udp_tunnel == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot add ethdev port %u UDP tunnel port from NULL UDP tunnel\n",
        port_id);
    return -EINVAL;
  }
  ...
  return eth_err(port_id,
                 (*dev->dev_ops->udp_tunnel_port_add)(dev, udp_tunnel));
}
...
int rte_eth_dev_udp_tunnel_port_delete(uint16_t port_id,
  ...
  return eth_err(port_id,
                 (*dev->dev_ops->udp_tunnel_port_del)(dev, udp_tunnel));
}
...
int rte_eth_fec_get_capability(uint16_t port_id,
  ...
  if (speed_fec_capa == NULL && num > 0) {
    RTE_ETHDEV_LOG(ERR,
                   "Cannot get ethdev port %u FEC capability to NULL when "
                   "array size is non zero\n",
                   port_id);
    return -EINVAL;
  }
...
int rte_eth_fec_get(uint16_t port_id, uint32_t *fec_capa) {
  ...
  if (fec_capa == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get ethdev port %u current FEC mode to NULL\n",
                   port_id);
    return -EINVAL;
  }
 ...
 * an empty spot.
 */
static int eth_dev_get_mac_addr_index(uint16_t port_id,
...
int rte_eth_dev_mac_addr_add(uint16_t port_id, struct rte_ether_addr *addr,
                             uint32_t pool) {
  struct rte_eth_dev *dev;
  int index;
  uint64_t pool_mask;
...
int rte_eth_dev_mac_addr_remove(uint16_t port_id, struct rte_ether_addr *addr) {
  struct rte_eth_dev *dev;
  int index;
  ...
  if (addr == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot remove ethdev port %u MAC address from NULL address\n",
        port_id);
    return -EINVAL;
  }
 ...
 * an empty spot.
 */
static int eth_dev_get_hash_mac_addr_index(uint16_t port_id,
                                           const struct rte_ether_addr *addr) {
  struct rte_eth_dev_info dev_info;
  struct rte_eth_dev *dev = &rte_eth_devices[port_id];
  unsigned i;
  ...
  for (i = 0; i < dev_info.max_hash_mac_addrs; i++)
    if (memcmp(addr, &dev->data->hash_mac_addrs[i], RTE_ETHER_ADDR_LEN) == 0)
      return i;
...
int rte_eth_dev_uc_hash_table_set(uint16_t port_id, struct rte_ether_addr *addr,
  ...
  if (addr == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot set ethdev port %u unicast hash table from NULL address\n",
        port_id);
    return -EINVAL;
  }
  ...
  if (ret == 0) {
    /* Update address in NIC data structure */
    if (on)
      rte_ether_addr_copy(addr, &dev->data->hash_mac_addrs[index]);
    else
      rte_ether_addr_copy(&null_mac_addr, &dev->data->hash_mac_addrs[index]);
  }
...
int rte_eth_dev_uc_all_hash_table_set(uint16_t port_id, uint8_t on) {
  ...
  return eth_err(port_id, (*dev->dev_ops->uc_all_hash_table_set)(dev, on));
}
...
int rte_eth_set_queue_rate_limit(uint16_t port_id, uint16_t queue_idx,
  ...
  return eth_err(
      port_id, (*dev->dev_ops->set_queue_rate_limit)(dev, queue_idx, tx_rate));
}
...
int rte_eth_dev_callback_register(uint16_t port_id,
  ...
  if (cb_fn == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot register ethdev port %u callback from NULL\n",
                   port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_callback_unregister(uint16_t port_id,
  ...
  if (cb_fn == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot unregister ethdev port %u callback from NULL\n",
                   port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_rx_intr_ctl(uint16_t port_id, int epfd, int op, void *data) {
  ...
  for (qid = 0; qid < dev->data->nb_rx_queues; qid++) {
    ...
    if (rc && rc != -EEXIST) {
      RTE_ETHDEV_LOG(ERR, "p %u q %u Rx ctl error op %d epfd %d vec %u\n",
...
int rte_eth_dev_rx_intr_ctl_q_get_fd(uint16_t port_id, uint16_t queue_id) {
  ...
  efd_idx = (vec >= RTE_INTR_VEC_RXTX_OFFSET) ? (vec - RTE_INTR_VEC_RXTX_OFFSET)
                                              : vec;
...
rte_eth_add_rx_callback(uint16_t port_id, uint16_t queue_id,
  ...
  if (!tail) {
     ...
     */
    __atomic_store_n(&rte_eth_devices[port_id].post_rx_burst_cbs[queue_id], cb,
                     __ATOMIC_RELEASE);
...
rte_eth_add_tx_callback(uint16_t port_id, uint16_t queue_id,
  ...
  if (!tail) {
     ...
     * cb is visible to data plane.
     */
    __atomic_store_n(&rte_eth_devices[port_id].pre_tx_burst_cbs[queue_id], cb,
                     __ATOMIC_RELEASE);
...
int rte_eth_rx_queue_info_get(uint16_t port_id, uint16_t queue_id,
  ...
  if (dev->data->rx_queues == NULL || dev->data->rx_queues[queue_id] == NULL) {
    RTE_ETHDEV_LOG(ERR,
                   "Rx queue %" PRIu16 " of device with port_id=%" PRIu16
                   " has not been setup\n",
                   queue_id, port_id);
    return -EINVAL;
  }
  ...
  if (rte_eth_dev_is_rx_hairpin_queue(dev, queue_id)) {
    RTE_ETHDEV_LOG(INFO,
                   "Can't get hairpin Rx queue %" PRIu16
                   " info of device with port_id=%" PRIu16 "\n",
                   queue_id, port_id);
    return -EINVAL;
  }
...
int rte_eth_tx_queue_info_get(uint16_t port_id, uint16_t queue_id,
  ...
  if (dev->data->tx_queues == NULL || dev->data->tx_queues[queue_id] == NULL) {
    RTE_ETHDEV_LOG(ERR,
                   "Tx queue %" PRIu16 " of device with port_id=%" PRIu16
                   " has not been setup\n",
                   queue_id, port_id);
    return -EINVAL;
  }
  ...
  if (rte_eth_dev_is_tx_hairpin_queue(dev, queue_id)) {
    RTE_ETHDEV_LOG(INFO,
                   "Can't get hairpin Tx queue %" PRIu16
                   " info of device with port_id=%" PRIu16 "\n",
                   queue_id, port_id);
    return -EINVAL;
  }
...
int rte_eth_rx_burst_mode_get(uint16_t port_id, uint16_t queue_id,
  ...
  return eth_err(port_id, dev->dev_ops->rx_burst_mode_get(dev, queue_id, mode));
}
...
int rte_eth_tx_burst_mode_get(uint16_t port_id, uint16_t queue_id,
  ...
  memset(mode, 0, sizeof(*mode));
  return eth_err(port_id, dev->dev_ops->tx_burst_mode_get(dev, queue_id, mode));
}
...
int rte_eth_get_monitor_addr(uint16_t port_id, uint16_t queue_id,
  ...
  if (pmc == NULL) {
    RTE_ETHDEV_LOG(ERR,
                   "Cannot get ethdev port %u Rx queue %u power monitor "
                   "condition to NULL\n",
                   port_id, queue_id);
    return -EINVAL;
  }
  ...
  return eth_err(port_id, dev->dev_ops->get_monitor_addr(
                              dev->data->rx_queues[queue_id], pmc));
}
...
int rte_eth_dev_set_mc_addr_list(uint16_t port_id,
  ...
  return eth_err(port_id,
                 dev->dev_ops->set_mc_addr_list(dev, mc_addr_set, nb_mc_addr));
}
...
int rte_eth_timesync_read_rx_timestamp(uint16_t port_id,
  ...
  if (timestamp == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot read ethdev port %u Rx timestamp to NULL\n",
                   port_id);
    return -EINVAL;
  }
...
int rte_eth_timesync_read_tx_timestamp(uint16_t port_id,
  ...
  if (timestamp == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot read ethdev port %u Tx timestamp to NULL\n",
                   port_id);
    return -EINVAL;
  }
  ...
  return eth_err(port_id,
                 (*dev->dev_ops->timesync_read_tx_timestamp)(dev, timestamp));
}
...
int rte_eth_timesync_read_time(uint16_t port_id, struct timespec *timestamp) {
  ...
  if (timestamp == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot read ethdev port %u timesync time to NULL\n",
                   port_id);
    return -EINVAL;
  }

  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->timesync_read_time, -ENOTSUP);
  return eth_err(port_id, (*dev->dev_ops->timesync_read_time)(dev, timestamp));
}
...
int rte_eth_timesync_write_time(uint16_t port_id,
  ...
  if (timestamp == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot write ethdev port %u timesync from NULL time\n",
                   port_id);
    return -EINVAL;
  }

  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->timesync_write_time, -ENOTSUP);
  return eth_err(port_id, (*dev->dev_ops->timesync_write_time)(dev, timestamp));
}
...
int rte_eth_dev_get_reg_info(uint16_t port_id, struct rte_dev_reg_info *info) {
  ...
  if (info == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get ethdev port %u register info to NULL\n",
                   port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_get_eeprom(uint16_t port_id, struct rte_dev_eeprom_info *info) {
  ...
  if (info == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get ethdev port %u EEPROM info to NULL\n",
                   port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_set_eeprom(uint16_t port_id, struct rte_dev_eeprom_info *info) {
  ...
  if (info == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot set ethdev port %u EEPROM from NULL info\n",
                   port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_get_module_info(uint16_t port_id,
  ...
  if (modinfo == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot get ethdev port %u EEPROM module info to NULL\n", port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_get_module_eeprom(uint16_t port_id,
  ...
  if (info == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot get ethdev port %u module EEPROM info to NULL\n", port_id);
    return -EINVAL;
  }

  if (info->data == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot get ethdev port %u module EEPROM data to NULL\n", port_id);
    return -EINVAL;
  }

  if (info->length == 0) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot get ethdev port %u module EEPROM to data with zero size\n",
        port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_get_dcb_info(uint16_t port_id,
  ...
  if (dcb_info == NULL) {
    RTE_ETHDEV_LOG(ERR, "Cannot get ethdev port %u DCB info to NULL\n",
                   port_id);
    return -EINVAL;
  }
...
static void eth_dev_adjust_nb_desc(uint16_t *nb_desc,
...
int rte_eth_dev_hairpin_capability_get(uint16_t port_id,
  ...
  if (cap == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot get ethdev port %u hairpin capability to NULL\n", port_id);
    return -EINVAL;
  }
...
int rte_eth_dev_pool_ops_supported(uint16_t port_id, const char *pool) {
  ...
  if (pool == NULL) {
    RTE_ETHDEV_LOG(
        ERR, "Cannot test ethdev port %u mempool operation from NULL pool\n",
        port_id);
    return -EINVAL;
  }
...
static int eth_dev_handle_port_list(const char *cmd __rte_unused,
...
static void eth_dev_add_port_queue_stats(struct rte_tel_data *d,
                                         uint64_t *q_stats,
...
static int eth_dev_handle_port_stats(const char *cmd __rte_unused,
                                     const char *params,
                                     struct rte_tel_data *d) {
...
static int eth_dev_handle_port_xstats(const char *cmd __rte_unused,
                                      const char *params,
                                      struct rte_tel_data *d) {
  ...
  for (i = 0; i < num_xstats; i++)
    rte_tel_data_add_dict_u64(d, xstat_names[i].name, eth_xstats[i].value);
  return 0;
}

static int eth_dev_handle_port_link_status(const char *cmd __rte_unused,
                                           const char *params,
                                           struct rte_tel_data *d) {
...
static int eth_dev_handle_port_info(const char *cmd __rte_unused,
                                    const char *params,
                                    struct rte_tel_data *d) {
  ...
  rte_tel_data_add_dict_int(d, "nb_rx_queues", eth_dev->data->nb_rx_queues);
  rte_tel_data_add_dict_int(d, "nb_tx_queues", eth_dev->data->nb_tx_queues);
  ...
  rte_ether_format_addr(mac_addr, sizeof(mac_addr), eth_dev->data->mac_addrs);
  rte_tel_data_add_dict_string(d, "mac_addr", mac_addr);
  rte_tel_data_add_dict_int(d, "promiscuous", eth_dev->data->promiscuous);
  rte_tel_data_add_dict_int(d, "scattered_rx", eth_dev->data->scattered_rx);
  rte_tel_data_add_dict_int(d, "all_multicast", eth_dev->data->all_multicast);
  rte_tel_data_add_dict_int(d, "dev_started", eth_dev->data->dev_started);
  rte_tel_data_add_dict_int(d, "lro", eth_dev->data->lro);
  rte_tel_data_add_dict_int(d, "dev_configured", eth_dev->data->dev_configured);

  rte_tel_data_start_array(rxq_state, RTE_TEL_INT_VAL);
  for (i = 0; i < eth_dev->data->nb_rx_queues; i++)
    rte_tel_data_add_array_int(rxq_state, eth_dev->data->rx_queue_state[i]);

  rte_tel_data_start_array(txq_state, RTE_TEL_INT_VAL);
  for (i = 0; i < eth_dev->data->nb_tx_queues; i++)
    rte_tel_data_add_array_int(txq_state, eth_dev->data->tx_queue_state[i]);
  ...
  rte_tel_data_add_dict_int(
      d, "ethdev_rss_hf", eth_dev->data->dev_conf.rx_adv_conf.rss_conf.rss_hf);
...
int rte_eth_rx_metadata_negotiate(uint16_t port_id, uint64_t *features) {
  ...
  if (dev->data->dev_configured != 0) {
    RTE_ETHDEV_LOG(ERR, "The port (ID=%" PRIu16 ") is already configured\n",
                   port_id);
    return -EBUSY;
  }
...
int rte_eth_ip_reassembly_capability_get(
...
int rte_eth_ip_reassembly_conf_get(uint16_t port_id,
  ...
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->ip_reassembly_conf_get, -ENOTSUP);
  memset(conf, 0, sizeof(struct rte_eth_ip_reassembly_params));
  return eth_err(port_id, (*dev->dev_ops->ip_reassembly_conf_get)(dev, conf));
}

int rte_eth_ip_reassembly_conf_set(
    uint16_t port_id, const struct rte_eth_ip_reassembly_params *conf) {
  struct rte_eth_dev *dev;
  ...
  if (conf == NULL) {
    RTE_ETHDEV_LOG(ERR, "Invalid IP reassembly configuration (NULL)\n");
    return -EINVAL;
  }

  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->ip_reassembly_conf_set, -ENOTSUP);
  return eth_err(port_id, (*dev->dev_ops->ip_reassembly_conf_set)(dev, conf));
}
...
RTE_INIT(ethdev_init_telemetry) {
  ...
  rte_telemetry_register_cmd(
      "/ethdev/link_status", eth_dev_handle_port_link_status,
      "Returns the link status for a port. Parameters: int port_id");
  rte_telemetry_register_cmd(
      "/ethdev/info", eth_dev_handle_port_info,
