...
struct rte_eth_dev *eth_find_device(const struct rte_eth_dev *start,
                                    rte_eth_cmp_t cmp, const void *data) {
  struct rte_eth_dev *edev;
  ptrdiff_t idx;

  /* Avoid Undefined Behaviour */
  if (start != NULL && (start < &rte_eth_devices[0] ||
...
static int rte_eth_devargs_enlist(uint16_t *list, uint16_t *len_list,
...
static char *rte_eth_devargs_process_range(char *str, uint16_t *list,
                                           uint16_t *len_list,
  ...
  if (result == 1) {
    ...
    for (val = lo; val <= hi; val++) {
      if (rte_eth_devargs_enlist(list, len_list, max_list, val) != 0)
        return NULL;
    }
  } else
    return NULL;
 ...
 */
static char *rte_eth_devargs_process_list(char *str, uint16_t *list,
                                          uint16_t *len_list,
                                          const uint16_t max_list) {
  char *pos = str;
  ...
  if (*pos == '[')
    pos++;
  while (1) {
    pos = rte_eth_devargs_process_range(pos, list, len_list, max_list);
    if (pos == NULL)
      return NULL;
    if (*pos != ',') /* end of list */
      break;
...
int rte_eth_devargs_parse_representor_ports(char *str, void *data) {
  ...
  if (str[0] == 'p' && str[1] == 'f') {
    ...
    str += 2;
    str = rte_eth_devargs_process_list(str, eth_da->ports, &eth_da->nb_ports,
...
static struct dummy_queue
    *dummy_queues_array[RTE_MAX_ETHPORTS][RTE_MAX_QUEUES_PER_PORT];
...
static uint16_t dummy_eth_rx_burst(void *rxq,
  ...
  if (port_id < RTE_DIM(per_port_queues) && !queue->rx_warn_once) {
    RTE_ETHDEV_LOG(
        ERR, "lcore %u called rx_pkt_burst for not ready port %" PRIuPTR "\n",
...
static uint16_t dummy_eth_tx_burst(void *txq,
  ...
  if (port_id < RTE_DIM(per_port_queues) && !queue->tx_warn_once) {
    RTE_ETHDEV_LOG(
        ERR, "lcore %u called tx_pkt_burst for not ready port %" PRIuPTR "\n",
        rte_lcore_id(), port_id);
    rte_dump_stack();
...
uint16_t rte_eth_call_tx_callbacks(uint16_t port_id, uint16_t queue_id,
  ...
  while (cb != NULL) {
    nb_pkts = cb->fn.tx(port_id, queue_id, tx_pkts, nb_pkts, cb->param);
    cb = cb->next;
  }
...
void eth_dev_shared_data_prepare(void) {
  ...
  if (eth_dev_shared_data == NULL) {
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
      ...
      mz =
          rte_memzone_reserve(MZ_RTE_ETH_DEV_DATA, sizeof(*eth_dev_shared_data),
                              rte_socket_id(), flags);
    } else
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
      eth_dev_shared_data->next_owner_id = RTE_ETH_DEV_NO_OWNER + 1;
      rte_spinlock_init(&eth_dev_shared_data->ownership_lock);
      memset(eth_dev_shared_data->data, 0, sizeof(eth_dev_shared_data->data));
    }
  }
...
int eth_dev_rx_queue_config(struct rte_eth_dev *dev, uint16_t nb_queues) {
  ...
  if (dev->data->rx_queues == NULL &&
      nb_queues != 0) { /* first time configuration */
    dev->data->rx_queues =
        rte_zmalloc("ethdev->rx_queues",
                    sizeof(dev->data->rx_queues[0]) * RTE_MAX_QUEUES_PER_PORT,
                    RTE_CACHE_LINE_SIZE);
    if (dev->data->rx_queues == NULL) {
      dev->data->nb_rx_queues = 0;
      return -(ENOMEM);
    }
  } else if (dev->data->rx_queues != NULL &&
             nb_queues != 0) { /* re-configure */
  ...
  if (dev->data->tx_queues == NULL &&
    ...
    dev->data->tx_queues =
        rte_zmalloc("ethdev->tx_queues",
                    sizeof(dev->data->tx_queues[0]) * RTE_MAX_QUEUES_PER_PORT,
                    RTE_CACHE_LINE_SIZE);
    if (dev->data->tx_queues == NULL) {
      dev->data->nb_tx_queues = 0;
      return -(ENOMEM);
    }
  } else if (dev->data->tx_queues != NULL &&
             nb_queues != 0) { /* re-configure */
    for (i = nb_queues; i < old_nb_queues; i++)
