...
static struct rte_eth_dev *eth_dev_allocated(const char *name) {
  uint16_t i;
...
static uint16_t eth_dev_find_free_port(void) {
  uint16_t i;
...
int rte_eth_dev_callback_process(struct rte_eth_dev *dev,
  ...
  TAILQ_FOREACH(cb_lst, &(dev->link_intr_cbs), next) {
    ...
    rc = dev_cb.cb_fn(dev->data->port_id, dev_cb.event, dev_cb.cb_arg,
                      dev_cb.ret_param);
    rte_spinlock_lock(&eth_dev_cb_lock);
    cb_lst->active = 0;
  }
...
int rte_eth_dev_release_port(struct rte_eth_dev *eth_dev) {
  ...
  if (eth_dev->state != RTE_ETH_DEV_UNUSED)
    rte_eth_dev_callback_process(eth_dev, RTE_ETH_EVENT_DESTROY, NULL);
...
int rte_eth_dev_create(struct rte_device *device, const char *name,
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    ...
    if (priv_data_size) {
      ethdev->data->dev_private = rte_zmalloc_socket(
          name, priv_data_size, RTE_CACHE_LINE_SIZE, device->numa_node);
  ...
  if (ethdev_bus_specific_init) {
    ...
    if (retval) {
      RTE_ETHDEV_LOG(ERR, "ethdev bus specific initialisation failed\n");
      goto probe_failed;
    }
  }
...
int rte_eth_dev_destroy(struct rte_eth_dev *ethdev,
...
struct rte_eth_dev *rte_eth_dev_get_by_name(const char *name) {
  uint16_t pid;
...
int rte_eth_dev_is_rx_hairpin_queue(struct rte_eth_dev *dev,
                                    uint16_t queue_id) {
...
int rte_eth_dev_is_tx_hairpin_queue(struct rte_eth_dev *dev,
                                    uint16_t queue_id) {
...
static int eth_dev_devargs_tokenise(struct rte_kvargs *arglist,
                                    const char *str_in) {
  int state;
...
int rte_eth_devargs_parse(const char *dargs, struct rte_eth_devargs *eth_da) {
  ...
  for (i = 0; i < args.count; i++) {
    ...
    if (strcmp("representor", pair->key) == 0) {
      ...
      result = rte_eth_devargs_parse_representor_ports(pair->value, eth_da);
      if (result < 0)
        goto parse_cleanup;
    }
  }
...
static inline int eth_dev_dma_mzone_name(char *name, size_t len,
                                         uint16_t port_id, uint16_t queue_id,
                                         const char *ring_name) {
  return snprintf(name, len, "eth_p%d_q%d_%s", port_id, queue_id, ring_name);
}
...
rte_eth_dma_zone_reserve(const struct rte_eth_dev *dev, const char *ring_name,
  ...
  if (mz) {
    if ((socket_id != SOCKET_ID_ANY && socket_id != mz->socket_id) ||
        size > mz->len || ((uintptr_t)mz->addr & (align - 1)) != 0) {
      RTE_ETHDEV_LOG(ERR,
...
int rte_eth_hairpin_queue_peer_bind(uint16_t cur_port, uint16_t cur_queue,
  ...
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->hairpin_queue_peer_bind, -ENOTSUP);

  return (*dev->dev_ops->hairpin_queue_peer_bind)(dev, cur_queue, peer_info,
                                                  direction);
}

int rte_eth_hairpin_queue_peer_unbind(uint16_t cur_port, uint16_t cur_queue,
                                      uint32_t direction) {
  struct rte_eth_dev *dev;

  /* No need to check the validity again. */
  dev = &rte_eth_devices[cur_port];
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->hairpin_queue_peer_unbind, -ENOTSUP);
...
int rte_eth_hairpin_queue_peer_update(uint16_t peer_port, uint16_t peer_queue,
  ...
  RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->hairpin_queue_peer_update, -ENOTSUP);

  return (*dev->dev_ops->hairpin_queue_peer_update)(dev, peer_queue, cur_info,
                                                    peer_info, direction);
}

int rte_eth_ip_reassembly_dynfield_register(int *field_offset,
                                            int *flag_offset) {
...
uint16_t rte_eth_pkt_burst_dummy(void *queue __rte_unused,
                                 struct rte_mbuf **pkts __rte_unused,
                                 uint16_t nb_pkts __rte_unused) {
  return 0;
}

int rte_eth_representor_id_get(uint16_t port_id,
  ...
  if (ret == -ENOTSUP && type == RTE_ETH_REPRESENTOR_VF && controller == -1 &&
      pf == -1) {
  ...
  for (i = 0; i < info->nb_ranges; ++i) {
    ...
    if (info->ranges[i].id_end < info->ranges[i].id_base) {
      RTE_LOG(WARNING, EAL,
              "Port %hu invalid representor ID Range %u - %u, entry %d\n",
              port_id, info->ranges[i].id_base, info->ranges[i].id_end, i);
      continue;
    }
    count = info->ranges[i].id_end - info->ranges[i].id_base + 1;
    switch (info->ranges[i].type) {
    case RTE_ETH_REPRESENTOR_PF:
      if (pf < info->ranges[i].pf || pf >= info->ranges[i].pf + count)
        continue;
      *repr_id = info->ranges[i].id_base + (pf - info->ranges[i].pf);
      ret = 0;
      goto out;
    case RTE_ETH_REPRESENTOR_VF:
      ...
      if (representor_port < info->ranges[i].vf ||
        ...
        continue;
      *repr_id =
          info->ranges[i].id_base + (representor_port - info->ranges[i].vf);
      ret = 0;
      goto out;
    case RTE_ETH_REPRESENTOR_SF:
      ...
      if (representor_port < info->ranges[i].sf ||
        ...
        continue;
      *repr_id =
          info->ranges[i].id_base + (representor_port - info->ranges[i].sf);
      ret = 0;
      goto out;
    default:
      break;
...
int rte_eth_switch_domain_alloc(uint16_t *domain_id) {
  ...
  *domain_id = RTE_ETH_DEV_SWITCH_DOMAIN_ID_INVALID;
  ...
  for (i = 0; i < RTE_MAX_ETHPORTS; i++) {
    if (eth_dev_switch_domains[i].state == RTE_ETH_SWITCH_DOMAIN_UNUSED) {
      eth_dev_switch_domains[i].state = RTE_ETH_SWITCH_DOMAIN_ALLOCATED;
      *domain_id = i;
      return 0;
    }
  }
