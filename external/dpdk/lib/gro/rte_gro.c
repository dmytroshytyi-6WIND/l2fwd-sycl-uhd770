...
typedef void *(*gro_tbl_create_fn)(uint16_t socket_id, uint16_t max_flow_num,
                                   uint16_t max_item_per_flow);
typedef void (*gro_tbl_destroy_fn)(void *tbl);
typedef uint32_t (*gro_tbl_pkt_count_fn)(void *tbl);

static gro_tbl_create_fn tbl_create_fn[RTE_GRO_TYPE_MAX_NUM] = {
    gro_tcp4_tbl_create, gro_vxlan_tcp4_tbl_create, gro_udp4_tbl_create,
    gro_vxlan_udp4_tbl_create, NULL};
static gro_tbl_destroy_fn tbl_destroy_fn[RTE_GRO_TYPE_MAX_NUM] = {
    gro_tcp4_tbl_destroy, gro_vxlan_tcp4_tbl_destroy, gro_udp4_tbl_destroy,
    gro_vxlan_udp4_tbl_destroy, NULL};
static gro_tbl_pkt_count_fn tbl_pkt_count_fn[RTE_GRO_TYPE_MAX_NUM] = {
    gro_tcp4_tbl_pkt_count, gro_vxlan_tcp4_tbl_pkt_count,
    gro_udp4_tbl_pkt_count, gro_vxlan_udp4_tbl_pkt_count, NULL};

#define IS_IPV4_TCP_PKT(ptype)                                                 \
  (RTE_ETH_IS_IPV4_HDR(ptype) &&                                               \
   ((ptype & RTE_PTYPE_L4_TCP) == RTE_PTYPE_L4_TCP) &&                         \
   (RTE_ETH_IS_TUNNEL_PKT(ptype) == 0))

#define IS_IPV4_UDP_PKT(ptype)                                                 \
  (RTE_ETH_IS_IPV4_HDR(ptype) &&                                               \
   ((ptype & RTE_PTYPE_L4_UDP) == RTE_PTYPE_L4_UDP) &&                         \
   (RTE_ETH_IS_TUNNEL_PKT(ptype) == 0))

#define IS_IPV4_VXLAN_TCP4_PKT(ptype)                                          \
  (RTE_ETH_IS_IPV4_HDR(ptype) &&                                               \
   ((ptype & RTE_PTYPE_L4_UDP) == RTE_PTYPE_L4_UDP) &&                         \
   ((ptype & RTE_PTYPE_TUNNEL_VXLAN) == RTE_PTYPE_TUNNEL_VXLAN) &&             \
   ((ptype & RTE_PTYPE_INNER_L4_TCP) == RTE_PTYPE_INNER_L4_TCP) &&             \
   (((ptype & RTE_PTYPE_INNER_L3_MASK) == RTE_PTYPE_INNER_L3_IPV4) ||          \
    ((ptype & RTE_PTYPE_INNER_L3_MASK) == RTE_PTYPE_INNER_L3_IPV4_EXT) ||      \
    ((ptype & RTE_PTYPE_INNER_L3_MASK) ==                                      \
     RTE_PTYPE_INNER_L3_IPV4_EXT_UNKNOWN)))

#define IS_IPV4_VXLAN_UDP4_PKT(ptype)                                          \
  (RTE_ETH_IS_IPV4_HDR(ptype) &&                                               \
   ((ptype & RTE_PTYPE_L4_UDP) == RTE_PTYPE_L4_UDP) &&                         \
   ((ptype & RTE_PTYPE_TUNNEL_VXLAN) == RTE_PTYPE_TUNNEL_VXLAN) &&             \
...
void *rte_gro_ctx_create(const struct rte_gro_param *param) {
  ...
  gro_ctx = rte_zmalloc_socket(__func__, sizeof(struct gro_ctx),
  ...
  for (i = 0; i < RTE_GRO_TYPE_MAX_NUM; i++) {
    ...
    gro_ctx->tbls[i] = create_tbl_fn(param->socket_id, param->max_flow_num,
...
uint16_t rte_gro_reassemble_burst(struct rte_mbuf **pkts, uint16_t nb_pkts,
  ...
  if (unlikely((param->gro_types &
                (RTE_GRO_IPV4_VXLAN_TCP_IPV4 | RTE_GRO_TCP_IPV4 |
                 RTE_GRO_IPV4_VXLAN_UDP_IPV4 | RTE_GRO_UDP_IPV4)) == 0))
    return nb_pkts;

  /* Get the maximum number of packets */
  item_num = RTE_MIN(nb_pkts, (param->max_flow_num * param->max_item_per_flow));
  ...
  for (i = 0; i < nb_pkts; i++) {
     ...
     */
    if (IS_IPV4_VXLAN_TCP4_PKT(pkts[i]->packet_type) && do_vxlan_tcp_gro) {
      ret = gro_vxlan_tcp4_reassemble(pkts[i], &vxlan_tcp_tbl, 0);
      if (ret > 0)
        /* Merge successfully */
        nb_after_gro--;
      else if (ret < 0)
        unprocess_pkts[unprocess_num++] = pkts[i];
               ...
               do_vxlan_udp_gro) {
      ret = gro_vxlan_udp4_reassemble(pkts[i], &vxlan_udp_tbl, 0);
      if (ret > 0)
        /* Merge successfully */
        nb_after_gro--;
      else if (ret < 0)
        unprocess_pkts[unprocess_num++] = pkts[i];
    } else if (IS_IPV4_TCP_PKT(pkts[i]->packet_type) && do_tcp4_gro) {
      ...
      if (ret > 0)
        ...
        unprocess_pkts[unprocess_num++] = pkts[i];
    } else if (IS_IPV4_UDP_PKT(pkts[i]->packet_type) && do_udp4_gro) {
  ...
  if ((nb_after_gro < nb_pkts) || (unprocess_num < nb_pkts)) {
    i = 0;
    /* Flush all packets from the tables */
    if (do_vxlan_tcp_gro) {
      i = gro_vxlan_tcp4_tbl_timeout_flush(&vxlan_tcp_tbl, 0, pkts, nb_pkts);
    }

    if (do_vxlan_udp_gro) {
      i += gro_vxlan_udp4_tbl_timeout_flush(&vxlan_udp_tbl, 0, &pkts[i],
                                            nb_pkts - i);
    }

    if (do_tcp4_gro) {
      i += gro_tcp4_tbl_timeout_flush(&tcp_tbl, 0, &pkts[i], nb_pkts - i);
    }

    if (do_udp4_gro) {
      i += gro_udp4_tbl_timeout_flush(&udp_tbl, 0, &pkts[i], nb_pkts - i);
    }
...
uint16_t rte_gro_reassemble(struct rte_mbuf **pkts, uint16_t nb_pkts,
  ...
  do_tcp4_gro = (gro_ctx->gro_types & RTE_GRO_TCP_IPV4) == RTE_GRO_TCP_IPV4;
  do_vxlan_tcp_gro = (gro_ctx->gro_types & RTE_GRO_IPV4_VXLAN_TCP_IPV4) ==
                     RTE_GRO_IPV4_VXLAN_TCP_IPV4;
  do_udp4_gro = (gro_ctx->gro_types & RTE_GRO_UDP_IPV4) == RTE_GRO_UDP_IPV4;
  ...
  for (i = 0; i < nb_pkts; i++) {
    if (IS_IPV4_VXLAN_TCP4_PKT(pkts[i]->packet_type) && do_vxlan_tcp_gro) {
      if (gro_vxlan_tcp4_reassemble(pkts[i], vxlan_tcp_tbl, current_time) < 0)
        unprocess_pkts[unprocess_num++] = pkts[i];
    } else if (IS_IPV4_VXLAN_UDP4_PKT(pkts[i]->packet_type) &&
               do_vxlan_udp_gro) {
      if (gro_vxlan_udp4_reassemble(pkts[i], vxlan_udp_tbl, current_time) < 0)
        unprocess_pkts[unprocess_num++] = pkts[i];
    } else if (IS_IPV4_TCP_PKT(pkts[i]->packet_type) && do_tcp4_gro) {
      if (gro_tcp4_reassemble(pkts[i], tcp_tbl, current_time) < 0)
        unprocess_pkts[unprocess_num++] = pkts[i];
    } else if (IS_IPV4_UDP_PKT(pkts[i]->packet_type) && do_udp4_gro) {
      if (gro_udp4_reassemble(pkts[i], udp_tbl, current_time) < 0)
        unprocess_pkts[unprocess_num++] = pkts[i];
    } else
      unprocess_pkts[unprocess_num++] = pkts[i];
  }
  if (unprocess_num > 0) {
    memcpy(pkts, unprocess_pkts, sizeof(struct rte_mbuf *) * unprocess_num);
  }
...
uint16_t rte_gro_timeout_flush(void *ctx, uint64_t timeout_cycles,
  ...
  if (gro_types & RTE_GRO_IPV4_VXLAN_TCP_IPV4) {
    num = gro_vxlan_tcp4_tbl_timeout_flush(
        gro_ctx->tbls[RTE_GRO_IPV4_VXLAN_TCP_IPV4_INDEX], flush_timestamp, out,
        left_nb_out);
    left_nb_out = max_nb_out - num;
  }

  if ((gro_types & RTE_GRO_IPV4_VXLAN_UDP_IPV4) && left_nb_out > 0) {
    num += gro_vxlan_udp4_tbl_timeout_flush(
        gro_ctx->tbls[RTE_GRO_IPV4_VXLAN_UDP_IPV4_INDEX], flush_timestamp,
        &out[num], left_nb_out);
    left_nb_out = max_nb_out - num;
  }
  ...
  if ((gro_types & RTE_GRO_TCP_IPV4) && left_nb_out > 0) {
    num += gro_tcp4_tbl_timeout_flush(gro_ctx->tbls[RTE_GRO_TCP_IPV4_INDEX],
  ...
  if ((gro_types & RTE_GRO_UDP_IPV4) && left_nb_out > 0) {
    num += gro_udp4_tbl_timeout_flush(gro_ctx->tbls[RTE_GRO_UDP_IPV4_INDEX],
                                      flush_timestamp, &out[num], left_nb_out);
  }
