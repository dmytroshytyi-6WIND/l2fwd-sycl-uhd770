 ...
 */
static inline int is_same_tcp4_flow(struct tcp4_flow_key k1,
                                    struct tcp4_flow_key k2) {
  return (rte_is_same_ether_addr(&k1.eth_saddr, &k2.eth_saddr) &&
          ...
          (k1.ip_dst_addr == k2.ip_dst_addr) && (k1.recv_ack == k2.recv_ack) &&
 ...
 * the original packet.
 */
static inline int merge_two_tcp4_packets(struct gro_tcp4_item *item,
  ...
  hdr_len = l2_offset + pkt_head->l2_len + pkt_head->l3_len + pkt_head->l4_len;
 ...
 */
static inline int check_seq_option(struct gro_tcp4_item *item,
                                   struct rte_tcp_hdr *tcph, uint32_t sent_seq,
                                   uint16_t ip_id, uint16_t tcp_hl,
                                   uint16_t tcp_dl, uint16_t l2_offset,
                                   uint8_t is_atomic) {
  ...
  if ((tcp_hl != tcp_hl_orig) ||
      ((len > 0) && (memcmp(tcph + 1, tcph_orig + 1, len) != 0)))
    return 0;
  ...
  if ((sent_seq == item->sent_seq + len) &&
      (is_atomic || (ip_id == item->ip_id + 1)))
    /* append the new packet */
    return 1;
  else if ((sent_seq + tcp_dl == item->sent_seq) &&
           (is_atomic || (ip_id + item->nb_merged == item->ip_id)))
