...
void *gro_vxlan_udp4_tbl_create(uint16_t socket_id, uint16_t max_flow_num,
  ...
  tbl = rte_zmalloc_socket(__func__, sizeof(struct gro_vxlan_udp4_tbl),
...
static inline uint32_t find_an_empty_item(struct gro_vxlan_udp4_tbl *tbl) {
...
static inline uint32_t find_an_empty_flow(struct gro_vxlan_udp4_tbl *tbl) {
...
static inline uint32_t insert_new_item(struct gro_vxlan_udp4_tbl *tbl,
                                       struct rte_mbuf *pkt,
...
static inline uint32_t delete_item(struct gro_vxlan_udp4_tbl *tbl,
...
static inline uint32_t insert_new_flow(struct gro_vxlan_udp4_tbl *tbl,
...
static inline int is_same_vxlan_udp4_flow(struct vxlan_udp4_flow_key k1,
...
static inline int udp4_check_vxlan_neighbor(struct gro_vxlan_udp4_item *item,
                                            uint16_t frag_offset,
                                            uint16_t ip_dl) {
  ...
  cmp = udp4_check_neighbor(&item->inner_item, frag_offset, ip_dl, l2_offset);
  if (cmp > 0)
...
static inline int merge_two_vxlan_udp4_packets(struct gro_vxlan_udp4_item *item,
...
static inline void update_vxlan_header(struct gro_vxlan_udp4_item *item) {
...
int32_t gro_vxlan_udp4_reassemble(struct rte_mbuf *pkt,
  ...
  prev_idx = cur_idx;
  do {
    cmp = udp4_check_vxlan_neighbor(&(tbl->items[cur_idx]), frag_offset, ip_dl);
    if (cmp) {
      if (merge_two_vxlan_udp4_packets(&(tbl->items[cur_idx]), pkt, cmp,
                                       frag_offset, is_last_frag)) {
        return 1;
      }
      /*
       ...
       */
      if (insert_new_item(tbl, pkt, start_time, prev_idx, frag_offset,
                          is_last_frag) == INVALID_ARRAY_INDEX)
        return -1;
      return 0;
    }

    /* Ensure inserted items are ordered by frag_offset */
    if (frag_offset < tbl->items[cur_idx].inner_item.frag_offset) {
      break;
    }
...
static int gro_vxlan_udp4_merge_items(struct gro_vxlan_udp4_tbl *tbl,
                                      uint32_t start_idx) {
  uint16_t frag_offset;
  uint8_t is_last_frag;
  int16_t ip_dl;
  ...
  while (item_idx != INVALID_ARRAY_INDEX) {
    ...
    cmp =
        udp4_check_vxlan_neighbor(&(tbl->items[start_idx]), frag_offset, ip_dl);
    if (cmp) {
      if (merge_two_vxlan_udp4_packets(&(tbl->items[start_idx]), pkt, cmp,
                                       frag_offset, is_last_frag)) {
        item_idx = delete_item(tbl, item_idx, INVALID_ARRAY_INDEX);
        tbl->items[start_idx].inner_item.next_pkt_idx = item_idx;
      } else
        return 0;
    } else
      return 0;
