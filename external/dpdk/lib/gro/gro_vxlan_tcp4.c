...
void *gro_vxlan_tcp4_tbl_create(uint16_t socket_id, uint16_t max_flow_num,
  ...
  tbl = rte_zmalloc_socket(__func__, sizeof(struct gro_vxlan_tcp4_tbl),
...
static inline uint32_t find_an_empty_item(struct gro_vxlan_tcp4_tbl *tbl) {
...
static inline uint32_t find_an_empty_flow(struct gro_vxlan_tcp4_tbl *tbl) {
...
static inline uint32_t insert_new_item(struct gro_vxlan_tcp4_tbl *tbl,
                                       struct rte_mbuf *pkt,
                                       uint64_t start_time, uint32_t prev_idx,
                                       uint32_t sent_seq, uint16_t outer_ip_id,
                                       uint16_t ip_id, uint8_t outer_is_atomic,
                                       uint8_t is_atomic) {
  uint32_t item_idx;
...
static inline uint32_t delete_item(struct gro_vxlan_tcp4_tbl *tbl,
                                   uint32_t item_idx, uint32_t prev_item_idx) {
...
static inline uint32_t insert_new_flow(struct gro_vxlan_tcp4_tbl *tbl,
  ...
  rte_ether_addr_copy(&(src->inner_key.eth_saddr), &(dst->inner_key.eth_saddr));
  rte_ether_addr_copy(&(src->inner_key.eth_daddr), &(dst->inner_key.eth_daddr));
...
static inline int is_same_vxlan_tcp4_flow(struct vxlan_tcp4_flow_key k1,
                                          struct vxlan_tcp4_flow_key k2) {
  return (rte_is_same_ether_addr(&k1.outer_eth_saddr, &k2.outer_eth_saddr) &&
          rte_is_same_ether_addr(&k1.outer_eth_daddr, &k2.outer_eth_daddr) &&
...
static inline int check_vxlan_seq_option(
    struct gro_vxlan_tcp4_item *item, struct rte_tcp_hdr *tcp_hdr,
    uint32_t sent_seq, uint16_t outer_ip_id, uint16_t ip_id, uint16_t tcp_hl,
    uint16_t tcp_dl, uint8_t outer_is_atomic, uint8_t is_atomic) {
  ...
  cmp = check_seq_option(&item->inner_item, tcp_hdr, sent_seq, ip_id, tcp_hl,
                         tcp_dl, l2_offset, is_atomic);
  if ((cmp > 0) && (outer_is_atomic || (outer_ip_id == item->outer_ip_id + 1)))
    /* Append the new packet. */
    return 1;
  else if ((cmp < 0) &&
           (outer_is_atomic ||
            (outer_ip_id + item->inner_item.nb_merged == item->outer_ip_id)))
...
static inline int merge_two_vxlan_tcp4_packets(struct gro_vxlan_tcp4_item *item,
                                               struct rte_mbuf *pkt, int cmp,
                                               uint32_t sent_seq,
                                               uint16_t outer_ip_id,
                                               uint16_t ip_id) {
  if (merge_two_tcp4_packets(&item->inner_item, pkt, cmp, sent_seq, ip_id,
...
static inline void update_vxlan_header(struct gro_vxlan_tcp4_item *item) {
...
int32_t gro_vxlan_tcp4_reassemble(struct rte_mbuf *pkt,
  ...
  outer_ipv4_hdr =
      (struct rte_ipv4_hdr *)((char *)outer_eth_hdr + pkt->outer_l2_len);
  udp_hdr = (struct rte_udp_hdr *)((char *)outer_ipv4_hdr + pkt->outer_l3_len);
  vxlan_hdr =
      (struct rte_vxlan_hdr *)((char *)udp_hdr + sizeof(struct rte_udp_hdr));
  ...
  hdr_len = pkt->outer_l2_len + pkt->outer_l3_len + pkt->l2_len + pkt->l3_len +
            pkt->l4_len;
  /*
  ...
  outer_ip_id =
      outer_is_atomic ? 0 : rte_be_to_cpu_16(outer_ipv4_hdr->packet_id);
  ...
  if (find == 0) {
    item_idx =
        insert_new_item(tbl, pkt, start_time, INVALID_ARRAY_INDEX, sent_seq,
                        outer_ip_id, ip_id, outer_is_atomic, is_atomic);
  ...
  prev_idx = cur_idx;
  do {
    cmp = check_vxlan_seq_option(&(tbl->items[cur_idx]), tcp_hdr, sent_seq,
                                 outer_ip_id, ip_id, pkt->l4_len, tcp_dl,
                                 outer_is_atomic, is_atomic);
    if (cmp) {
      if (merge_two_vxlan_tcp4_packets(&(tbl->items[cur_idx]), pkt, cmp,
                                       sent_seq, outer_ip_id, ip_id))
        return 1;
      /*
       ...
       */
      if (insert_new_item(tbl, pkt, start_time, prev_idx, sent_seq, outer_ip_id,
  ...
  if (insert_new_item(tbl, pkt, start_time, prev_idx, sent_seq, outer_ip_id,
                      ip_id, outer_is_atomic, is_atomic) == INVALID_ARRAY_INDEX)
    return -1;
...
uint16_t gro_vxlan_tcp4_tbl_timeout_flush(struct gro_vxlan_tcp4_tbl *tbl,
  ...
  for (i = 0; i < max_flow_num; i++) {
    ...
    while (j != INVALID_ARRAY_INDEX) {
      if (tbl->items[j].inner_item.start_time <= flush_timestamp) {
