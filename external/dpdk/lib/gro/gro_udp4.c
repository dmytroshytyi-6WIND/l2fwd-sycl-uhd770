...
void *gro_udp4_tbl_create(uint16_t socket_id, uint16_t max_flow_num,
  ...
  tbl = rte_zmalloc_socket(__func__, sizeof(struct gro_udp4_tbl),
...
static inline uint32_t find_an_empty_item(struct gro_udp4_tbl *tbl) {
  uint32_t i;
...
static inline uint32_t find_an_empty_flow(struct gro_udp4_tbl *tbl) {
  uint32_t i;
...
static inline uint32_t insert_new_item(struct gro_udp4_tbl *tbl,
                                       struct rte_mbuf *pkt,
...
static inline uint32_t delete_item(struct gro_udp4_tbl *tbl, uint32_t item_idx,
...
static inline uint32_t insert_new_flow(struct gro_udp4_tbl *tbl,
 ...
 */
static inline void update_header(struct gro_udp4_item *item) {
  struct rte_ipv4_hdr *ipv4_hdr;
  struct rte_mbuf *pkt = item->firstseg;
  uint16_t frag_offset;

  ipv4_hdr =
      (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(pkt, char *) + pkt->l2_len);
...
int32_t gro_udp4_reassemble(struct rte_mbuf *pkt, struct gro_udp4_tbl *tbl,
                            uint64_t start_time) {
  ...
  prev_idx = cur_idx;
  do {
    cmp = udp4_check_neighbor(&(tbl->items[cur_idx]), frag_offset, ip_dl, 0);
    if (cmp) {
      if (merge_two_udp4_packets(&(tbl->items[cur_idx]), pkt, cmp, frag_offset,
                                 is_last_frag, 0))
        return 1;
      /*
    ...
    if (frag_offset < tbl->items[cur_idx].frag_offset) {
      break;
    }
...
static int gro_udp4_merge_items(struct gro_udp4_tbl *tbl, uint32_t start_idx) {
  uint16_t frag_offset;
  uint8_t is_last_frag;
  int16_t ip_dl;
  struct rte_mbuf *pkt;
  ...
  while (item_idx != INVALID_ARRAY_INDEX) {
    ...
    cmp = udp4_check_neighbor(&(tbl->items[start_idx]), frag_offset, ip_dl, 0);
    if (cmp) {
      if (merge_two_udp4_packets(&(tbl->items[start_idx]), pkt, cmp,
                                 frag_offset, is_last_frag, 0)) {
        item_idx = delete_item(tbl, item_idx, INVALID_ARRAY_INDEX);
        tbl->items[start_idx].next_pkt_idx = item_idx;
      } else
        return 0;
    } else
      return 0;
