 ...
 */
static inline int is_same_udp4_flow(struct udp4_flow_key k1,
                                    struct udp4_flow_key k2) {
  return (rte_is_same_ether_addr(&k1.eth_saddr, &k2.eth_saddr) &&
          ...
          (k1.ip_dst_addr == k2.ip_dst_addr) && (k1.ip_id == k2.ip_id));
}
 ...
 * the original packet.
 */
static inline int merge_two_udp4_packets(struct gro_udp4_item *item,
  ...
  ip_len = pkt_head->pkt_len - l2_len + pkt_tail->pkt_len - hdr_len;
 ...
 */
static inline int udp4_check_neighbor(struct gro_udp4_item *item,
                                      uint16_t frag_offset, uint16_t ip_dl,
                                      uint16_t l2_offset) {
  struct rte_mbuf *pkt_orig = item->firstseg;
  uint16_t len;

  /* check if the two packets are neighbors */
  len = pkt_orig->pkt_len - l2_offset - pkt_orig->l2_len - pkt_orig->l3_len;
...
static inline int is_ipv4_fragment(const struct rte_ipv4_hdr *hdr) {
