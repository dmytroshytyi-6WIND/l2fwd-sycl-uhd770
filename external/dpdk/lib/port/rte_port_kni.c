...
static void *rte_port_kni_reader_create(void *params, int socket_id) {
  struct rte_port_kni_reader_params *conf = params;
...
static int rte_port_kni_reader_rx(void *port, struct rte_mbuf **pkts,
                                  uint32_t n_pkts) {
  struct rte_port_kni_reader *p = port;
  uint16_t rx_pkt_cnt;
...
static int rte_port_kni_reader_free(void *port) {
  if (port == NULL) {
...
static void *rte_port_kni_writer_create(void *params, int socket_id) {
  struct rte_port_kni_writer_params *conf = params;
...
static inline void send_burst(struct rte_port_kni_writer *p) {
  uint32_t nb_tx;
...
static int rte_port_kni_writer_tx(void *port, struct rte_mbuf *pkt) {
  struct rte_port_kni_writer *p = port;
...
static int rte_port_kni_writer_tx_bulk(void *port, struct rte_mbuf **pkts,
                                       uint64_t pkts_mask) {
  struct rte_port_kni_writer *p = port;
...
static int rte_port_kni_writer_flush(void *port) {
  struct rte_port_kni_writer *p = port;
...
static int rte_port_kni_writer_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: Port is NULL\n", __func__);
    return -EINVAL;
  }
...
static void *rte_port_kni_writer_nodrop_create(void *params, int socket_id) {
  struct rte_port_kni_writer_nodrop_params *conf = params;
...
static inline void send_burst_nodrop(struct rte_port_kni_writer_nodrop *p) {
  uint32_t nb_tx = 0, i;
  ...
  for (i = 0; i < p->n_retries; i++) {
    nb_tx +=
        rte_kni_tx_burst(p->kni, p->tx_buf + nb_tx, p->tx_buf_count - nb_tx);
...
static int rte_port_kni_writer_nodrop_tx(void *port, struct rte_mbuf *pkt) {
  struct rte_port_kni_writer_nodrop *p = port;
...
static int rte_port_kni_writer_nodrop_tx_bulk(void *port,
...
static int rte_port_kni_writer_nodrop_flush(void *port) {
  struct rte_port_kni_writer_nodrop *p = port;
...
static int rte_port_kni_writer_nodrop_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: Port is NULL\n", __func__);
    return -EINVAL;
  }
...
static int rte_port_kni_writer_nodrop_stats_read(
