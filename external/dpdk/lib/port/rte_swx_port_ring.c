...
static int reader_pkt_rx(void *port, struct rte_swx_pkt *pkt) {
  ...
  if (p->pos == p->n_pkts) {
    ...
    n_pkts = rte_ring_sc_dequeue_burst(p->params.ring, (void **)p->pkts,
                                       p->params.burst_size, NULL);
    if (!n_pkts) {
      p->stats.n_empty++;
      return 0;
    }
...
static void __writer_flush(struct writer *p) {
  ...
  for (n_pkts = 0;;) {
    n_pkts += rte_ring_sp_enqueue_burst(
        p->params.ring, (void **)p->pkts + n_pkts, p->n_pkts - n_pkts, NULL);
