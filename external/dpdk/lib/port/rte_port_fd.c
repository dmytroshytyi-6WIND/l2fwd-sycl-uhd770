...
static void *rte_port_fd_reader_create(void *params, int socket_id) {
  struct rte_port_fd_reader_params *conf = params;
...
static int rte_port_fd_reader_rx(void *port, struct rte_mbuf **pkts,
                                 uint32_t n_pkts) {
...
static int rte_port_fd_reader_free(void *port) {
  if (port == NULL) {
...
static void *rte_port_fd_writer_create(void *params, int socket_id) {
  struct rte_port_fd_writer_params *conf = params;
...
static inline void send_burst(struct rte_port_fd_writer *p) {
  uint32_t i;
...
static int rte_port_fd_writer_tx(void *port, struct rte_mbuf *pkt) {
  struct rte_port_fd_writer *p = port;
...
static int rte_port_fd_writer_tx_bulk(void *port, struct rte_mbuf **pkts,
                                      uint64_t pkts_mask) {
  struct rte_port_fd_writer *p = port;
...
static int rte_port_fd_writer_flush(void *port) {
  struct rte_port_fd_writer *p = port;
...
static int rte_port_fd_writer_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: Port is NULL\n", __func__);
    return -EINVAL;
  }
...
static void *rte_port_fd_writer_nodrop_create(void *params, int socket_id) {
  struct rte_port_fd_writer_nodrop_params *conf = params;
  struct rte_port_fd_writer_nodrop *port;

  /* Check input parameters */
  if ((conf == NULL) || (conf->fd < 0) || (conf->tx_burst_sz == 0) ||
      (conf->tx_burst_sz > RTE_PORT_IN_BURST_SIZE_MAX) ||
      (!rte_is_power_of_2(conf->tx_burst_sz))) {
    RTE_LOG(ERR, PORT, "%s: Invalid input parameters\n", __func__);
    return NULL;
...
static inline void send_burst_nodrop(struct rte_port_fd_writer_nodrop *p) {
  uint64_t n_retries;
  uint32_t i;
...
static int rte_port_fd_writer_nodrop_tx(void *port, struct rte_mbuf *pkt) {
  struct rte_port_fd_writer_nodrop *p = port;
...
static int rte_port_fd_writer_nodrop_tx_bulk(void *port, struct rte_mbuf **pkts,
                                             uint64_t pkts_mask) {
  struct rte_port_fd_writer_nodrop *p = port;
  uint32_t tx_buf_count = p->tx_buf_count;
...
static int rte_port_fd_writer_nodrop_flush(void *port) {
  struct rte_port_fd_writer_nodrop *p = port;
...
static int rte_port_fd_writer_nodrop_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: Port is NULL\n", __func__);
    return -EINVAL;
  }
...
static int rte_port_fd_writer_nodrop_stats_read(
