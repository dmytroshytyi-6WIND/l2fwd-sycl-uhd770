...
static void *rte_port_sched_reader_create(void *params, int socket_id) {
  struct rte_port_sched_reader_params *conf = params;
  struct rte_port_sched_reader *port;

  /* Check input parameters */
  if ((conf == NULL) || (conf->sched == NULL)) {
...
static int rte_port_sched_reader_rx(void *port, struct rte_mbuf **pkts,
                                    uint32_t n_pkts) {
...
static int rte_port_sched_reader_free(void *port) {
  if (port == NULL) {
...
static int rte_port_sched_reader_stats_read(void *port,
                                            struct rte_port_in_stats *stats,
                                            int clear) {
  struct rte_port_sched_reader *p = port;
...
static void *rte_port_sched_writer_create(void *params, int socket_id) {
  struct rte_port_sched_writer_params *conf = params;
  struct rte_port_sched_writer *port;

  /* Check input parameters */
  if ((conf == NULL) || (conf->sched == NULL) || (conf->tx_burst_sz == 0) ||
...
static int rte_port_sched_writer_tx(void *port, struct rte_mbuf *pkt) {
...
static int rte_port_sched_writer_tx_bulk(void *port, struct rte_mbuf **pkts,
                                         uint64_t pkts_mask) {
  struct rte_port_sched_writer *p = (struct rte_port_sched_writer *)port;
  ...
  if (expr == 0) {
    ...
    if (tx_buf_count) {
      nb_tx = rte_sched_port_enqueue(p->sched, p->tx_buf, tx_buf_count);
...
static int rte_port_sched_writer_flush(void *port) {
  struct rte_port_sched_writer *p = (struct rte_port_sched_writer *)port;
...
static int rte_port_sched_writer_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: port is NULL\n", __func__);
    return -EINVAL;
  }
...
static int rte_port_sched_writer_stats_read(void *port,
                                            struct rte_port_out_stats *stats,
                                            int clear) {
  struct rte_port_sched_writer *p = port;
