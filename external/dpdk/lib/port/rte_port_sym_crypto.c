...
static void *rte_port_sym_crypto_reader_create(void *params, int socket_id) {
  struct rte_port_sym_crypto_reader_params *conf = params;
...
static int rte_port_sym_crypto_reader_rx(void *port, struct rte_mbuf **pkts,
                                         uint32_t n_pkts) {
  struct rte_port_sym_crypto_reader *p = port;
  uint16_t rx_ops_cnt, i, n = 0;

  rx_ops_cnt =
      rte_cryptodev_dequeue_burst(p->cryptodev_id, p->queue_id, p->ops, n_pkts);
...
static int rte_port_sym_crypto_reader_free(void *port) {
  if (port == NULL) {
...
static int rte_port_sym_crypto_reader_stats_read(
    void *port, struct rte_port_in_stats *stats, int clear) {
  struct rte_port_sym_crypto_reader *p = port;
...
static void *rte_port_sym_crypto_writer_create(void *params, int socket_id) {
  struct rte_port_sym_crypto_writer_params *conf = params;
  struct rte_port_sym_crypto_writer *port;

  /* Check input parameters */
  if ((conf == NULL) || (conf->tx_burst_sz == 0) ||
...
static inline void send_burst(struct rte_port_sym_crypto_writer *p) {
  uint32_t nb_tx;

  nb_tx = rte_cryptodev_enqueue_burst(p->cryptodev_id, p->queue_id, p->tx_buf,
                                      p->tx_buf_count);

  RTE_PORT_SYM_CRYPTO_WRITER_STATS_PKTS_DROP_ADD(p, p->tx_buf_count - nb_tx);
...
static int rte_port_sym_crypto_writer_tx(void *port, struct rte_mbuf *pkt) {
  struct rte_port_sym_crypto_writer *p = port;

  p->tx_buf[p->tx_buf_count++] =
      (struct rte_crypto_op *)RTE_MBUF_METADATA_UINT8_PTR(pkt,
                                                          p->crypto_op_offset);
...
static int rte_port_sym_crypto_writer_tx_bulk(void *port,
  ...
  if (expr == 0) {
    ...
    for (i = 0; i < n_pkts; i++)
      p->tx_buf[p->tx_buf_count++] =
          (struct rte_crypto_op *)RTE_MBUF_METADATA_UINT8_PTR(
              pkts[i], p->crypto_op_offset);
    ...
    for (; pkts_mask;) {
      ...
      p->tx_buf[tx_buf_count++] =
          (struct rte_crypto_op *)RTE_MBUF_METADATA_UINT8_PTR(
              pkt, p->crypto_op_offset);
...
static int rte_port_sym_crypto_writer_flush(void *port) {
  struct rte_port_sym_crypto_writer *p = port;
...
static int rte_port_sym_crypto_writer_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: Port is NULL\n", __func__);
    return -EINVAL;
  }
...
static int rte_port_sym_crypto_writer_stats_read(
...
static void *rte_port_sym_crypto_writer_nodrop_create(void *params,
                                                      int socket_id) {
  struct rte_port_sym_crypto_writer_nodrop_params *conf = params;
...
send_burst_nodrop(struct rte_port_sym_crypto_writer_nodrop *p) {
  ...
  for (i = 0; i < p->n_retries; i++) {
    nb_tx +=
        rte_cryptodev_enqueue_burst(p->cryptodev_id, p->queue_id,
  ...
  RTE_PORT_SYM_CRYPTO_WRITER_NODROP_STATS_PKTS_DROP_ADD(p, p->tx_buf_count -
                                                               nb_tx);
  for (; nb_tx < p->tx_buf_count; nb_tx++)
    rte_pktmbuf_free(p->tx_buf[nb_tx]->sym->m_src);
...
static int rte_port_sym_crypto_writer_nodrop_tx(void *port,
                                                struct rte_mbuf *pkt) {
  struct rte_port_sym_crypto_writer_nodrop *p = port;
...
static int rte_port_sym_crypto_writer_nodrop_tx_bulk(void *port,
                                                     struct rte_mbuf **pkts,
                                                     uint64_t pkts_mask) {
  struct rte_port_sym_crypto_writer_nodrop *p = port;
  ...
  if (expr == 0) {
    ...
    for (; pkts_mask;) {
      ...
      p->tx_buf[tx_buf_count++] =
          (struct rte_crypto_op *)RTE_MBUF_METADATA_UINT8_PTR(
              pkt, p->crypto_op_offset);
      RTE_PORT_SYM_CRYPTO_WRITER_NODROP_STATS_PKTS_IN_ADD(p, 1);
      pkts_mask &= ~pkt_mask;
    }
...
static int rte_port_sym_crypto_writer_nodrop_flush(void *port) {
  struct rte_port_sym_crypto_writer_nodrop *p = port;
...
static int rte_port_sym_crypto_writer_nodrop_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: Port is NULL\n", __func__);
    return -EINVAL;
  }
...
static int rte_port_sym_crypto_writer_nodrop_stats_read(
    void *port, struct rte_port_out_stats *stats, int clear) {
  struct rte_port_sym_crypto_writer_nodrop *p = port;
