...
static void *rte_port_ring_reader_create_internal(void *params, int socket_id,
                                                  uint32_t is_multi) {
  struct rte_port_ring_reader_params *conf = params;
  struct rte_port_ring_reader *port;

  /* Check input parameters */
  if ((conf == NULL) || (conf->ring == NULL) ||
  ...
  /* Memory allocation */
  port =
      rte_zmalloc_socket("PORT", sizeof(*port), RTE_CACHE_LINE_SIZE, socket_id);
  if (port == NULL) {
...
static void *rte_port_ring_reader_create(void *params, int socket_id) {
  return rte_port_ring_reader_create_internal(params, socket_id, 0);
}

static void *rte_port_ring_multi_reader_create(void *params, int socket_id) {
  return rte_port_ring_reader_create_internal(params, socket_id, 1);
}

static int rte_port_ring_reader_rx(void *port, struct rte_mbuf **pkts,
                                   uint32_t n_pkts) {
  struct rte_port_ring_reader *p = port;
  uint32_t nb_rx;

  nb_rx = rte_ring_sc_dequeue_burst(p->ring, (void **)pkts, n_pkts, NULL);
  RTE_PORT_RING_READER_STATS_PKTS_IN_ADD(p, nb_rx);

  return nb_rx;
}

static int rte_port_ring_multi_reader_rx(void *port, struct rte_mbuf **pkts,
                                         uint32_t n_pkts) {
  struct rte_port_ring_reader *p = port;
  uint32_t nb_rx;

  nb_rx = rte_ring_mc_dequeue_burst(p->ring, (void **)pkts, n_pkts, NULL);
  RTE_PORT_RING_READER_STATS_PKTS_IN_ADD(p, nb_rx);

  return nb_rx;
}

static int rte_port_ring_reader_free(void *port) {
  if (port == NULL) {
...
static int rte_port_ring_reader_stats_read(void *port,
                                           struct rte_port_in_stats *stats,
                                           int clear) {
  struct rte_port_ring_reader *p = port;
...
static void *rte_port_ring_writer_create_internal(void *params, int socket_id,
                                                  uint32_t is_multi) {
  struct rte_port_ring_writer_params *conf = params;
...
static void *rte_port_ring_writer_create(void *params, int socket_id) {
  return rte_port_ring_writer_create_internal(params, socket_id, 0);
}

static void *rte_port_ring_multi_writer_create(void *params, int socket_id) {
  return rte_port_ring_writer_create_internal(params, socket_id, 1);
}

static inline void send_burst(struct rte_port_ring_writer *p) {
  uint32_t nb_tx;
...
static inline void send_burst_mp(struct rte_port_ring_writer *p) {
  uint32_t nb_tx;
...
static int rte_port_ring_writer_tx(void *port, struct rte_mbuf *pkt) {
...
static int rte_port_ring_multi_writer_tx(void *port, struct rte_mbuf *pkt) {
  struct rte_port_ring_writer *p = port;
...
rte_port_ring_writer_tx_bulk_internal(void *port, struct rte_mbuf **pkts,
                                      uint64_t pkts_mask, uint32_t is_multi) {
  struct rte_port_ring_writer *p = port;

  uint64_t bsz_mask = p->bsz_mask;
  uint32_t tx_buf_count = p->tx_buf_count;
  uint64_t expr =
      (pkts_mask & (pkts_mask + 1)) | ((pkts_mask & bsz_mask) ^ bsz_mask);
  ...
  if (expr == 0) {
    ...
    if (is_multi)
      n_pkts_ok =
          rte_ring_mp_enqueue_burst(p->ring, (void **)pkts, n_pkts, NULL);
    else
      n_pkts_ok =
          rte_ring_sp_enqueue_burst(p->ring, (void **)pkts, n_pkts, NULL);
...
static int rte_port_ring_writer_tx_bulk(void *port, struct rte_mbuf **pkts,
                                        uint64_t pkts_mask) {
  return rte_port_ring_writer_tx_bulk_internal(port, pkts, pkts_mask, 0);
}

static int rte_port_ring_multi_writer_tx_bulk(void *port,
                                              struct rte_mbuf **pkts,
                                              uint64_t pkts_mask) {
  return rte_port_ring_writer_tx_bulk_internal(port, pkts, pkts_mask, 1);
}

static int rte_port_ring_writer_flush(void *port) {
  struct rte_port_ring_writer *p = port;
...
static int rte_port_ring_multi_writer_flush(void *port) {
  struct rte_port_ring_writer *p = port;
...
static int rte_port_ring_writer_free(void *port) {
  struct rte_port_ring_writer *p = port;
...
static int rte_port_ring_writer_stats_read(void *port,
                                           struct rte_port_out_stats *stats,
                                           int clear) {
  struct rte_port_ring_writer *p = port;
...
static void *rte_port_ring_writer_nodrop_create_internal(void *params,
                                                         int socket_id,
                                                         uint32_t is_multi) {
  struct rte_port_ring_writer_nodrop_params *conf = params;
...
static void *rte_port_ring_writer_nodrop_create(void *params, int socket_id) {
  return rte_port_ring_writer_nodrop_create_internal(params, socket_id, 0);
}

static void *rte_port_ring_multi_writer_nodrop_create(void *params,
                                                      int socket_id) {
  return rte_port_ring_writer_nodrop_create_internal(params, socket_id, 1);
}

static inline void send_burst_nodrop(struct rte_port_ring_writer_nodrop *p) {
  uint32_t nb_tx = 0, i;
  ...
  for (i = 0; i < p->n_retries; i++) {
    nb_tx += rte_ring_sp_enqueue_burst(p->ring, (void **)(p->tx_buf + nb_tx),
...
static inline void send_burst_mp_nodrop(struct rte_port_ring_writer_nodrop *p) {
  uint32_t nb_tx = 0, i;
  ...
  for (i = 0; i < p->n_retries; i++) {
    nb_tx += rte_ring_mp_enqueue_burst(p->ring, (void **)(p->tx_buf + nb_tx),
                                       p->tx_buf_count - nb_tx, NULL);
...
static int rte_port_ring_writer_nodrop_tx(void *port, struct rte_mbuf *pkt) {
  struct rte_port_ring_writer_nodrop *p = port;
...
static int rte_port_ring_multi_writer_nodrop_tx(void *port,
                                                struct rte_mbuf *pkt) {
  struct rte_port_ring_writer_nodrop *p = port;
...
static __rte_always_inline int rte_port_ring_writer_nodrop_tx_bulk_internal(
    void *port, struct rte_mbuf **pkts, uint64_t pkts_mask, uint32_t is_multi) {
  struct rte_port_ring_writer_nodrop *p = port;
...
static int rte_port_ring_writer_nodrop_tx_bulk(void *port,
                                               struct rte_mbuf **pkts,
                                               uint64_t pkts_mask) {
  return rte_port_ring_writer_nodrop_tx_bulk_internal(port, pkts, pkts_mask, 0);
}

static int rte_port_ring_multi_writer_nodrop_tx_bulk(void *port,
                                                     struct rte_mbuf **pkts,
                                                     uint64_t pkts_mask) {
  return rte_port_ring_writer_nodrop_tx_bulk_internal(port, pkts, pkts_mask, 1);
}

static int rte_port_ring_writer_nodrop_flush(void *port) {
  struct rte_port_ring_writer_nodrop *p = port;
...
static int rte_port_ring_multi_writer_nodrop_flush(void *port) {
  struct rte_port_ring_writer_nodrop *p = port;
...
static int rte_port_ring_writer_nodrop_free(void *port) {
  struct rte_port_ring_writer_nodrop *p = port;
...
static int rte_port_ring_writer_nodrop_stats_read(
    void *port, struct rte_port_out_stats *stats, int clear) {
  struct rte_port_ring_writer_nodrop *p = port;
