...
static int pcap_source_load(struct rte_port_source *port, const char *file_name,
                            uint32_t n_bytes_per_pkt, int socket_id) {
  uint32_t n_pkts = 0;
  uint32_t i;
  ...
  port->pkt_len = rte_zmalloc_socket("PCAP", (sizeof(*port->pkt_len) * n_pkts),
                                     0, socket_id);
  ...
  pkt_len_aligns = rte_malloc("PCAP", (sizeof(*pkt_len_aligns) * n_pkts), 0);
  ...
  port->pkts =
      rte_zmalloc_socket("PCAP", (sizeof(*port->pkts) * n_pkts), 0, socket_id);
  if (port->pkts == NULL) {
    RTE_LOG(ERR, PORT, "No enough memory\n");
    goto error_exit;
  }
  ...
  buff = rte_zmalloc_socket("PCAP", total_buff_len, 0, socket_id);
  if (buff == NULL) {
    RTE_LOG(ERR, PORT, "No enough memory\n");
    goto error_exit;
  }
...
static void *rte_port_source_create(void *params, int socket_id) {
  struct rte_port_source_params *p = params;
  ...
  if (p->file_name) {
    int status =
        PCAP_SOURCE_LOAD(port, p->file_name, p->n_bytes_per_pkt, socket_id);
...
static int rte_port_source_free(void *port) {
...
static int rte_port_source_rx(void *port, struct rte_mbuf **pkts,
                              uint32_t n_pkts) {
  struct rte_port_source *p = port;
  uint32_t i;
  ...
  if (p->pkt_buff != NULL) {
    for (i = 0; i < n_pkts; i++) {
      ...
      rte_memcpy(pkt_data, p->pkts[p->pkt_index], p->pkt_len[p->pkt_index]);
...
static int rte_port_source_stats_read(void *port,
                                      struct rte_port_in_stats *stats,
                                      int clear) {
  struct rte_port_source *p = port;
...
static int pcap_sink_open(struct rte_port_sink *port, const char *file_name,
                          uint32_t max_n_pkts) {
  pcap_t *tx_pcap;
...
static void pcap_sink_write_pkt(struct rte_port_sink *port,
  ...
  if (mbuf->nb_segs > 1) {
    ...
    for (jumbo_mbuf = mbuf; jumbo_mbuf != NULL; jumbo_mbuf = jumbo_mbuf->next) {
      rte_memcpy(&jumbo_pkt_buf[pkt_index],
                 rte_pktmbuf_mtod(jumbo_mbuf, uint8_t *), jumbo_mbuf->data_len);
...
#define PCAP_SINK_WRITE_PKT(port, mbuf) pcap_sink_write_pkt(port, mbuf)
...
static void *rte_port_sink_create(void *params, int socket_id) {
  ...
  if (p->file_name) {
    int status = PCAP_SINK_OPEN(port, p->file_name, p->max_n_pkts);
...
static int rte_port_sink_tx(void *port, struct rte_mbuf *pkt) {
...
static int rte_port_sink_tx_bulk(void *port, struct rte_mbuf **pkts,
                                 uint64_t pkts_mask) {
  struct rte_port_sink *p = port;
  ...
  if ((pkts_mask & (pkts_mask + 1)) == 0) {
    ...
    if (p->dumper) {
      ...
      for (; dump_pkts_mask;) {
        pkt_index = __builtin_ctzll(dump_pkts_mask);
...
static int rte_port_sink_flush(void *port) {
  struct rte_port_sink *p = port;
...
static int rte_port_sink_free(void *port) {
  struct rte_port_sink *p = port;
...
static int rte_port_sink_stats_read(void *port,
                                    struct rte_port_out_stats *stats,
                                    int clear) {
  struct rte_port_sink *p = port;
