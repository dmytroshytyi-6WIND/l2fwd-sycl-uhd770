...
typedef int32_t (*frag_op)(struct rte_mbuf *pkt_in, struct rte_mbuf **pkts_out,
...
static void *rte_port_ring_reader_frag_create(void *params, int socket_id,
                                              int is_ipv4) {
  struct rte_port_ring_reader_frag_params *conf = params;
  ...
  port->f_frag =
      (is_ipv4) ? rte_ipv4_fragment_packet : rte_ipv6_fragment_packet;

  return port;
}

static void *rte_port_ring_reader_ipv4_frag_create(void *params,
                                                   int socket_id) {
  return rte_port_ring_reader_frag_create(params, socket_id, 1);
}

static void *rte_port_ring_reader_ipv6_frag_create(void *params,
                                                   int socket_id) {
  return rte_port_ring_reader_frag_create(params, socket_id, 0);
}

static int rte_port_ring_reader_frag_rx(void *port, struct rte_mbuf **pkts,
                                        uint32_t n_pkts) {
  struct rte_port_ring_reader_frag *p = port;
  uint32_t n_pkts_out;
  ...
  for (;;) {
    ...
    if (p->n_pkts == 0) {
      p->n_pkts = rte_ring_sc_dequeue_burst(p->ring, (void **)p->pkts,
                                            RTE_PORT_IN_BURST_SIZE_MAX, NULL);
    ...
    status = p->f_frag(pkt, p->frags, RTE_PORT_FRAG_MAX_FRAGS_PER_PACKET,
                       p->mtu, p->pool_direct, p->pool_indirect);
    ...
    for (i = 0; i < p->n_frags; i++) {
      uint8_t *src = RTE_MBUF_METADATA_UINT8_PTR(pkt, sizeof(struct rte_mbuf));
      uint8_t *dst =
...
static int rte_port_ring_reader_frag_free(void *port) {
  if (port == NULL) {
...
static int rte_port_frag_reader_stats_read(void *port,
                                           struct rte_port_in_stats *stats,
                                           int clear) {
  struct rte_port_ring_reader_frag *p = port;
