...
static void *rte_port_eventdev_reader_create(void *params, int socket_id) {
  struct rte_port_eventdev_reader_params *conf = params;
...
static int rte_port_eventdev_reader_rx(void *port, struct rte_mbuf **pkts,
                                       uint32_t n_pkts) {
  struct rte_port_eventdev_reader *p = port;
  uint16_t rx_evts_cnt, i;

  rx_evts_cnt =
      rte_event_dequeue_burst(p->eventdev_id, p->port_id, p->ev, n_pkts, 0);
...
static int rte_port_eventdev_reader_free(void *port) {
  if (port == NULL) {
...
static void *rte_port_eventdev_writer_create(void *params, int socket_id) {
  struct rte_port_eventdev_writer_params *conf = params;
  struct rte_port_eventdev_writer *port;
  unsigned int i;

  /* Check input parameters */
  if ((conf == NULL) || (conf->enq_burst_sz == 0) ||
...
static inline void send_burst(struct rte_port_eventdev_writer *p) {
  uint32_t nb_enq;

  nb_enq = rte_event_enqueue_burst(p->eventdev_id, p->port_id, p->ev,
                                   p->enq_buf_count);

  RTE_PORT_EVENTDEV_WRITER_STATS_PKTS_DROP_ADD(p, p->enq_buf_count - nb_enq);
...
static int rte_port_eventdev_writer_tx(void *port, struct rte_mbuf *pkt) {
...
static int rte_port_eventdev_writer_tx_bulk(void *port, struct rte_mbuf **pkts,
                                            uint64_t pkts_mask) {
  struct rte_port_eventdev_writer *p = port;
  ...
  if (expr == 0) {
    ...
    n_enq_ok =
        rte_event_enqueue_burst(p->eventdev_id, p->port_id, events, n_pkts);

    RTE_PORT_EVENTDEV_WRITER_STATS_PKTS_DROP_ADD(p, n_pkts - n_enq_ok);
...
static int rte_port_eventdev_writer_flush(void *port) {
  struct rte_port_eventdev_writer *p = port;
...
static int rte_port_eventdev_writer_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: Port is NULL\n", __func__);
    return -EINVAL;
  }
...
static void *rte_port_eventdev_writer_nodrop_create(void *params,
                                                    int socket_id) {
  struct rte_port_eventdev_writer_nodrop_params *conf = params;
...
send_burst_nodrop(struct rte_port_eventdev_writer_nodrop *p) {
  ...
  for (i = 0; i < p->n_retries; i++) {
    nb_enq += rte_event_enqueue_burst(
        p->eventdev_id, p->port_id, p->ev + nb_enq, p->enq_buf_count - nb_enq);
  ...
  RTE_PORT_EVENTDEV_WRITER_NODROP_STATS_PKTS_DROP_ADD(p, p->enq_buf_count -
                                                             nb_enq);
  for (; nb_enq < p->enq_buf_count; nb_enq++)
    rte_pktmbuf_free(p->ev[nb_enq].mbuf);
...
static int rte_port_eventdev_writer_nodrop_tx(void *port,
                                              struct rte_mbuf *pkt) {
...
static int rte_port_eventdev_writer_nodrop_tx_bulk(void *port,
...
static int rte_port_eventdev_writer_nodrop_flush(void *port) {
  struct rte_port_eventdev_writer_nodrop *p = port;
...
static int rte_port_eventdev_writer_nodrop_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: Port is NULL\n", __func__);
    return -EINVAL;
  }
...
static int rte_port_eventdev_writer_nodrop_stats_read(
