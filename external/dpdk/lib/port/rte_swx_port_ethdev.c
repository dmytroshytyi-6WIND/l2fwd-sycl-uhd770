...
static void *reader_create(void *args) {
...
static int reader_pkt_rx(void *port, struct rte_swx_pkt *pkt) {
  ...
  if (p->pos == p->n_pkts) {
    ...
    n_pkts = rte_eth_rx_burst(p->params.port_id, p->params.queue_id, p->pkts,
                              p->params.burst_size);
    if (!n_pkts) {
      p->stats.n_empty++;
      return 0;
    ...
    TRACE("[Ethdev RX port %u queue %u] %d packets in\n",
          (uint32_t)p->params.port_id, (uint32_t)p->params.queue_id, n_pkts);
  ...
  TRACE("[Ethdev RX port %u queue %u] Pkt %d (%u bytes at offset %u)\n",
        (uint32_t)p->params.port_id, (uint32_t)p->params.queue_id, p->pos - 1,
        pkt->length, pkt->offset);
  if (TRACE_LEVEL)
    rte_hexdump(stdout, NULL, &((uint8_t *)m->buf_addr)[m->data_off],
                m->data_len);
...
static void reader_free(void *port) {
  struct reader *p = port;
  int i;
...
static void reader_stats_read(void *port, struct rte_swx_port_in_stats *stats) {
  struct reader *p = port;
...
static void *writer_create(void *args) {
  struct rte_eth_dev_info info;
...
static void __writer_flush(struct writer *p) {
  int n_pkts;

  for (n_pkts = 0;;) {
    n_pkts += rte_eth_tx_burst(p->params.port_id, p->params.queue_id,
                               p->pkts + n_pkts, p->n_pkts - n_pkts);
...
static void writer_pkt_tx(void *port, struct rte_swx_pkt *pkt) {
  struct writer *p = port;
  struct rte_mbuf *m = pkt->handle;

  TRACE("[Ethdev TX port %u queue %u] Pkt %d (%u bytes at offset %u)\n",
        (uint32_t)p->params.port_id, (uint32_t)p->params.queue_id,
        p->n_pkts - 1, pkt->length, pkt->offset);
  if (TRACE_LEVEL)
...
static void writer_flush(void *port) {
  struct writer *p = port;
...
static void writer_free(void *port) {
  struct writer *p = port;
...
static void writer_stats_read(void *port,
                              struct rte_swx_port_out_stats *stats) {
  struct writer *p = port;
