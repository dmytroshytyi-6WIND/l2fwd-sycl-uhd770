...
static int handle_telemetry_cmd_ipsec_sa_list(const char *cmd __rte_unused,
 ...
 *
 */
static int handle_telemetry_cmd_ipsec_sa_stats(const char *cmd __rte_unused,
                                               const char *params,
                                               struct rte_tel_data *data) {
  struct ipsec_telemetry_entry *entry;
  const struct rte_ipsec_sa *sa;
  ...
  LIST_FOREACH(entry, &ipsec_telemetry_list, next) {
    ...
    rte_tel_data_add_dict_u64(sa_data, name_pkt_cnt, sa->statistics.count);
    ...
    snprintf(sa_name, sizeof(sa_name), "SA_SPI_%i", rte_be_to_cpu_32(sa->spi));
...
static int handle_telemetry_cmd_ipsec_sa_details(const char *cmd __rte_unused,
                                                 const char *params,
                                                 struct rte_tel_data *data) {
  struct ipsec_telemetry_entry *entry;
  const struct rte_ipsec_sa *sa;
  ...
  LIST_FOREACH(entry, &ipsec_telemetry_list, next) {
    ...
    if (mode == RTE_IPSEC_SATP_MODE_TRANS) {
      ...
      if ((sa->type & RTE_IPSEC_SATP_NATT_MASK) == RTE_IPSEC_SATP_NATT_ENABLE) {
        if (sa->type & RTE_IPSEC_SATP_MODE_TUNLV4) {
          rte_tel_data_add_dict_string(data, "Tunnel-Type", "IPv4-UDP");
        } else if (sa->type & RTE_IPSEC_SATP_MODE_TUNLV6) {
          rte_tel_data_add_dict_string(data, "Tunnel-Type", "IPv6-UDP");
        }
      } else {
        if (sa->type & RTE_IPSEC_SATP_MODE_TUNLV4) {
          rte_tel_data_add_dict_string(data, "Tunnel-Type", "IPv4");
        } else if (sa->type & RTE_IPSEC_SATP_MODE_TUNLV6) {
          rte_tel_data_add_dict_string(data, "Tunnel-Type", "IPv6");
        }
      }
    }

    rte_tel_data_add_dict_string(data, "extended-sequence-number",
    ...
    if ((sa->type & RTE_IPSEC_SATP_DIR_MASK) == RTE_IPSEC_SATP_DIR_IB)
      ...
      if (sa->sqn.inb.rsn[sa->sqn.inb.rdidx])
        ...
        rte_tel_data_add_dict_u64(data, "sequence-number", 0);
    else
      rte_tel_data_add_dict_u64(data, "sequence-number", sa->sqn.outb);

    rte_tel_data_add_dict_string(data, "explicit-congestion-notification",
                                 (sa->type & RTE_IPSEC_SATP_ECN_MASK) ==
                                         RTE_IPSEC_SATP_ECN_ENABLE
                                     ? "enabled"
                                     : "disabled");

    rte_tel_data_add_dict_string(data, "copy-DSCP",
...
int rte_ipsec_telemetry_sa_add(const struct rte_ipsec_sa *sa) {
  struct ipsec_telemetry_entry *entry =
      rte_zmalloc(NULL, sizeof(struct ipsec_telemetry_entry), 0);
  if (entry == NULL)
    return -ENOMEM;
  entry->sa = sa;
...
RTE_INIT(rte_ipsec_telemetry_init) {
  rte_telemetry_register_cmd(
      "/ipsec/sa/list", handle_telemetry_cmd_ipsec_sa_list,
      "Return list of IPsec SAs with telemetry enabled.");
  rte_telemetry_register_cmd(
      "/ipsec/sa/stats", handle_telemetry_cmd_ipsec_sa_stats,
      "Returns IPsec SA statistics. Parameters: int sa_spi");
  rte_telemetry_register_cmd(
      "/ipsec/sa/details", handle_telemetry_cmd_ipsec_sa_details,
