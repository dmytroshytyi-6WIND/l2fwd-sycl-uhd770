 ...
 */
static inline rte_be32_t sqn_hi32(rte_be64_t sqn) {
 ...
 */
static inline rte_be32_t sqn_low32(rte_be64_t sqn) {
#if RTE_BYTE_ORDER == RTE_BIG_ENDIAN
  return sqn;
#else
  return (sqn >> 32);
 ...
 */
static inline rte_be16_t sqn_low16(rte_be64_t sqn) {
#if RTE_BYTE_ORDER == RTE_BIG_ENDIAN
  return sqn;
#else
  return (sqn >> 48);
 ...
 */
static inline uint64_t reconstruct_esn(uint64_t t, uint32_t sqn, uint32_t w) {
  uint32_t th, tl, bl;
 ...
 */
static inline int32_t esn_inb_check_sqn(const struct replay_sqn *rsn,
 ...
 */
static inline uint64_t esn_outb_update_sqn(struct rte_ipsec_sa *sa,
                                           uint32_t *num) {
  uint64_t n, s, sqn;
 ...
 */
static inline int32_t esn_inb_update_sqn(struct replay_sqn *rsn,
                                         const struct rte_ipsec_sa *sa,
                                         uint64_t sqn) {
  ...
  if (sqn > rsn->sqn) {
    ...
    for (i = 0; i != diff; i++) {
      new_bucket = (i + last_bucket + 1) & sa->replay.bucket_index_mask;
 ...
 */
static inline void rsn_copy(const struct rte_ipsec_sa *sa, uint32_t dst,
                            uint32_t src) {
  uint32_t i, n;
  struct replay_sqn *d;
 ...
 */
static inline struct replay_sqn *rsn_acquire(struct rte_ipsec_sa *sa) {
  uint32_t n;
 ...
 */
static inline void rsn_release(struct rte_ipsec_sa *sa,
                               struct replay_sqn *rsn) {
  if (SQN_ATOMIC(sa))
 ...
 * Start RSN update.
 */
static inline struct replay_sqn *rsn_update_start(struct rte_ipsec_sa *sa) {
  uint32_t k, n;
  struct replay_sqn *rsn;
 ...
 * Finish RSN update.
 */
static inline void rsn_update_finish(struct rte_ipsec_sa *sa,
                                     struct replay_sqn *rsn) {
  uint32_t n;
