 ...
 */
static inline void remove_esph(char *np, char *op, uint32_t hlen) {
  uint32_t i;
 ...
 */
static inline void insert_esph(char *np, char *op, uint32_t hlen) {
  uint32_t i;
...
static inline int update_trs_l3hdr(const struct rte_ipsec_sa *sa, void *p,
                                   uint32_t plen, uint32_t l2len,
                                   uint32_t l3len, uint8_t proto) {
  int32_t rc;
  ...
  if ((sa->type & RTE_IPSEC_SATP_IPV_MASK) == RTE_IPSEC_SATP_IPV4) {
    ...
    *p_nh = proto;
    ...
    /* fix packet length */
    v6h->payload_len = rte_cpu_to_be_16(plen - l2len - sizeof(*v6h));
  }
 ...
 */
static inline uint8_t get_ipv6_tc(rte_be32_t vtc_flow) {
  uint32_t v;
...
static inline rte_be32_t set_ipv6_tc(rte_be32_t vtc_flow, uint32_t tos) {
  uint32_t v;
 ...
 */
static inline void update_outb_tun_tos(const void *ref_h, void *update_h,
                                       uint32_t tos_mask, uint8_t is_outh_ipv4,
                                       uint8_t is_inh_ipv4) {
  ...
  case 0: /*outh ipv6, inh ipv6 */
    ...
    itp =
        get_ipv6_tc(((const struct rte_ipv6_hdr *)ref_h)->vtc_flow) & tos_mask;
    v6out_h->vtc_flow = set_ipv6_tc(v6out_h->vtc_flow, otp | itp);
    break;
  case 1: /*outh ipv6, inh ipv4 */
    v6out_h = update_h;
    otp = get_ipv6_tc(v6out_h->vtc_flow) & ~tos_mask;
    itp = ((const struct rte_ipv4_hdr *)ref_h)->type_of_service & tos_mask;
    v6out_h->vtc_flow = set_ipv6_tc(v6out_h->vtc_flow, otp | itp);
    break;
 ...
 * @param is_inner_ipv4: 1 if inner header is ipv4, 0 if it is ipv6.
 */
static inline void update_inb_tun_tos(const void *ref_h, void *update_h,
  ...
  case 0: /* outh ipv6, inh ipv6 */
    ...
    ecn_v6in = v6in_h->vtc_flow & rte_cpu_to_be_32(RTE_IPV6_HDR_ECN_MASK);
    if ((ecn_v6out == rte_cpu_to_be_32(RTE_IPV6_HDR_ECN_CE)) && (ecn_v6in != 0))
      v6in_h->vtc_flow |= rte_cpu_to_be_32(RTE_IPV6_HDR_ECN_CE);
    break;
  case 1: /* outh ipv6, inh ipv4 */
    v4in_h = update_h;
    ecn_v6out = ((const struct rte_ipv6_hdr *)ref_h)->vtc_flow &
    ...
    if ((ecn_v6out == rte_cpu_to_be_32(RTE_IPV6_HDR_ECN_CE)) && (ecn_v4in != 0))
      v4in_h->type_of_service |= RTE_IPV4_HDR_ECN_CE;
    break;
  case 2: /* outh ipv4, inh ipv6 */
    v6in_h = update_h;
    ecn_v4out = ((const struct rte_ipv4_hdr *)ref_h)->type_of_service &
                RTE_IPV4_HDR_ECN_MASK;
    ecn_v6in = v6in_h->vtc_flow & rte_cpu_to_be_32(RTE_IPV6_HDR_ECN_MASK);
...
static inline void update_tun_outb_l3hdr(const struct rte_ipsec_sa *sa,
                                         void *outh, const void *inh,
                                         uint32_t plen, uint32_t l2len,
                                         rte_be16_t pid) {
  struct rte_ipv4_hdr *v4h;
  struct rte_ipv6_hdr *v6h;
  ...
  if (sa->type & TUN_HDR_MSK)
    update_outb_tun_tos(
        inh, outh, sa->tos_mask, is_outh_ipv4,
        ((sa->type & RTE_IPSEC_SATP_IPV_MASK) == RTE_IPSEC_SATP_IPV4));
}

static inline void update_tun_inb_l3hdr(const struct rte_ipsec_sa *sa,
                                        const void *outh, void *inh) {
  if (sa->type & TUN_HDR_MSK)
    update_inb_tun_tos(
        outh, inh, ((sa->type & RTE_IPSEC_SATP_MODE_TUNLV4) != 0),
        ((sa->type & RTE_IPSEC_SATP_IPV_MASK) == RTE_IPSEC_SATP_IPV4));
}
