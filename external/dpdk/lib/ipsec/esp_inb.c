...
typedef uint16_t (*esp_inb_process_t)(struct rte_ipsec_sa *sa,
                                      struct rte_mbuf *mb[], uint32_t sqn[],
 ...
 */
static inline void sop_ciph_auth_prepare(struct rte_crypto_sym_op *sop,
 ...
 * used by inb_cop_prepare(), see below.
 */
static inline void sop_aead_prepare(struct rte_crypto_sym_op *sop,
                                    const struct rte_ipsec_sa *sa,
                                    const union sym_op_data *icv, uint32_t pofs,
                                    uint32_t plen) {
 ...
 */
static inline void inb_cop_prepare(struct rte_crypto_op *cop,
                                   const struct rte_ipsec_sa *sa,
                                   struct rte_mbuf *mb,
                                   const union sym_op_data *icv, uint32_t pofs,
                                   uint32_t plen) {
  ...
  case ALGO_TYPE_AES_GCM:
    ...
    gcm = rte_crypto_op_ctod_offset(cop, struct aead_gcm_iv *, sa->iv_ofs);
  ...
  case ALGO_TYPE_AES_CCM:
    ...
    /* fill AAD IV (located inside crypto op) */
    ccm = rte_crypto_op_ctod_offset(cop, struct aead_ccm_iv *, sa->iv_ofs);
  ...
  case ALGO_TYPE_CHACHA20_POLY1305:
    ...
    /* fill AAD IV (located inside crypto op) */
    chacha20_poly1305 = rte_crypto_op_ctod_offset(
        cop, struct aead_chacha20_poly1305_iv *, sa->iv_ofs);
    aead_chacha20_poly1305_iv_fill(chacha20_poly1305, ivp[0], sa->salt);
    break;
  ...
  case ALGO_TYPE_AES_CTR:
    ...
    ctr = rte_crypto_op_ctod_offset(cop, struct aesctr_cnt_blk *, sa->iv_ofs);
...
static inline uint32_t inb_cpu_crypto_prepare(const struct rte_ipsec_sa *sa,
                                              struct rte_mbuf *mb,
                                              uint32_t *pofs, uint32_t plen,
                                              void *iv) {
  struct aead_gcm_iv *gcm;
 ...
 * last segment.
 */
static struct rte_mbuf *move_icv(struct rte_mbuf *ml, uint32_t ofs) {
  uint32_t n;
  struct rte_mbuf *ms;
  const void *prev;
  void *new;
 ...
 */
static inline void inb_pkt_xprepare(const struct rte_ipsec_sa *sa,
                                    rte_be64_t sqc,
                                    const union sym_op_data *icv) {
  ...
  case ALGO_TYPE_CHACHA20_POLY1305:
    if (sa->aad_len != 0) {
      chacha_aad = (struct aead_chacha20_poly1305_aad *)(icv->va + sa->icv_len);
      aead_chacha20_poly1305_aad_fill(chacha_aad, sa->spi, sqc, IS_ESN(sa));
    }
    break;
  }
}

static inline int inb_get_sqn(const struct rte_ipsec_sa *sa,
                              const struct replay_sqn *rsn, struct rte_mbuf *mb,
                              uint32_t hlen, rte_be64_t *sqc) {
  int32_t rc;
  uint64_t sqn;
...
static inline int32_t inb_prepare(const struct rte_ipsec_sa *sa,
                                  struct rte_mbuf *mb, uint32_t hlen,
...
static inline int32_t inb_pkt_prepare(const struct rte_ipsec_sa *sa,
                                      const struct replay_sqn *rsn,
                                      struct rte_mbuf *mb, uint32_t hlen,
                                      union sym_op_data *icv) {
  int rc;
  rte_be64_t sqn;
 ...
 */
uint16_t esp_inb_pkt_prepare(const struct rte_ipsec_session *ss,
 ...
 * - esp tail structure
 */
static inline void process_step1(struct rte_mbuf *mb, uint32_t tlen,
                                 struct rte_mbuf **ml,
                                 struct rte_esp_tail *espt, uint32_t *hlen,
                                 uint32_t *tofs) {
 ...
 */
static inline int check_pad_bytes(struct rte_mbuf *mb, uint32_t ofs,
                                  uint32_t len) {
  const uint8_t *pd;
  uint32_t k, n;
 ...
 */
static inline int32_t trs_process_check(struct rte_mbuf *mb,
                                        struct rte_mbuf **ml, uint32_t *tofs,
                                        struct rte_esp_tail espt, uint32_t hlen,
                                        uint32_t tlen) {
 ...
 */
static inline int32_t tun_process_check(struct rte_mbuf *mb,
                                        struct rte_mbuf **ml, uint32_t *tofs,
                                        struct rte_esp_tail espt, uint32_t hlen,
                                        uint32_t tlen, uint8_t proto) {
 ...
 */
static inline void *tun_process_step2(struct rte_mbuf *mb, struct rte_mbuf *ml,
                                      uint32_t hlen, uint32_t adj,
                                      uint32_t tofs, uint32_t tlen,
                                      uint32_t *sqn) {
 ...
 */
static inline void *trs_process_step2(struct rte_mbuf *mb, struct rte_mbuf *ml,
                                      uint32_t hlen, uint32_t adj,
                                      uint32_t tofs, uint32_t tlen,
                                      uint32_t *sqn) {
  char *np, *op;
 ...
 * - ol_flags
 */
static inline void trs_process_step3(struct rte_mbuf *mb) {
 ...
 * - tx_offload
 */
static inline void tun_process_step3(struct rte_mbuf *mb, uint64_t txof_msk,
                                     uint64_t txof_val) {
 ...
 */
static inline uint16_t tun_process(struct rte_ipsec_sa *sa,
                                   struct rte_mbuf *mb[], uint32_t sqn[],
                                   uint32_t dr[], uint16_t num,
                                   uint8_t sqh_len) {
  ...
  for (i = 0; i != num; i++) {
    ...
    if (tun_process_check(mb[i], &ml[i], &to[i], espt[i], adj, tl, sa->proto) ==
        0) {

      outh = rte_pktmbuf_mtod_offset(mb[i], uint8_t *, mb[i]->l2_len);

      /* modify packet's layout */
      inh = tun_process_step2(mb[i], ml[i], hl[i], adj, to[i], tl, sqn + k);

      /* update inner ip header */
      update_tun_inb_l3hdr(sa, outh, inh);

      /* update mbuf's metadata */
      tun_process_step3(mb[i], sa->tx_offload.msk, sa->tx_offload.val);
      k++;
      bytes += mb[i]->pkt_len;
    } else
      dr[i - k] = i;
 ...
 * *process* function for tunnel packets
 */
static inline uint16_t trs_process(struct rte_ipsec_sa *sa,
                                   struct rte_mbuf *mb[], uint32_t sqn[],
                                   uint32_t dr[], uint16_t num,
                                   uint8_t sqh_len) {
  char *np;
  ...
  for (i = 0; i != num; i++) {
    ...
    /* check that packet is valid */
    if (trs_process_check(mb[i], &ml[i], &to[i], espt[i], hl[i] + cofs, tl) ==
        0) {

      /* modify packet's layout */
      np = trs_process_step2(mb[i], ml[i], hl[i], cofs, to[i], tl, sqn + k);
      update_trs_l3hdr(sa, np + l2, mb[i]->pkt_len, l2, hl[i] - l2,
                       espt[i].next_proto);
 ...
 */
static inline uint16_t esp_inb_rsn_update(struct rte_ipsec_sa *sa,
                                          const uint32_t sqn[], uint32_t dr[],
                                          uint16_t num) {
  uint32_t i, k;
  struct replay_sqn *rsn;
 ...
 */
static inline uint16_t esp_inb_pkt_process(struct rte_ipsec_sa *sa,
 ...
 * (synchronous mode).
 */
uint16_t cpu_inb_pkt_prepare(const struct rte_ipsec_session *ss,
                             struct rte_mbuf *mb[], uint16_t num) {
  int32_t rc;
  uint32_t i, k;
  struct rte_ipsec_sa *sa;
  ...
  for (i = 0, k = 0; i != num; i++) {
    ...
    if (rc >= 0) {
      ...
      clen[k] = inb_cpu_crypto_prepare(sa, mb[i], l4ofs + k, rc, ivbuf[k]);
 ...
 */
uint16_t esp_inb_tun_pkt_process(const struct rte_ipsec_session *ss,
                                 struct rte_mbuf *mb[], uint16_t num) {
...
uint16_t inline_inb_tun_pkt_process(const struct rte_ipsec_session *ss,
                                    struct rte_mbuf *mb[], uint16_t num) {
 ...
 */
uint16_t esp_inb_trs_pkt_process(const struct rte_ipsec_session *ss,
                                 struct rte_mbuf *mb[], uint16_t num) {
  struct rte_ipsec_sa *sa = ss->sa;
...
uint16_t inline_inb_trs_pkt_process(const struct rte_ipsec_session *ss,
                                    struct rte_mbuf *mb[], uint16_t num) {
