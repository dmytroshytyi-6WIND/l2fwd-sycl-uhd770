...
static inline void aes_ctr_cnt_blk_fill(struct aesctr_cnt_blk *ctr, uint64_t iv,
                                        uint32_t nonce) {
  ctr->nonce = nonce;
  ctr->iv = iv;
...
static inline void aead_chacha20_poly1305_iv_fill(
    struct aead_chacha20_poly1305_iv *chacha20_poly1305, uint64_t iv,
    uint32_t salt) {
...
static inline void aead_gcm_iv_fill(struct aead_gcm_iv *gcm, uint64_t iv,
                                    uint32_t salt) {
  gcm->salt = salt;
  gcm->iv = iv;
...
static inline void aead_ccm_iv_fill(struct aead_ccm_iv *ccm, uint64_t iv,
                                    uint32_t salt) {
 ...
 */
static inline void aead_gcm_aad_fill(struct aead_gcm_aad *aad, rte_be32_t spi,
                                     rte_be64_t sqn, int esn) {
  aad->spi = spi;
  if (esn)
    aad->sqn.u64 = sqn;
  else {
 ...
 * Make sure that not used bytes are zeroed.
 */
static inline void aead_ccm_aad_fill(struct aead_ccm_aad *aad, rte_be32_t spi,
                                     rte_be64_t sqn, int esn) {
  aad->spi = spi;
  if (esn)
    aad->sqn.u64 = sqn;
...
static inline void gen_iv(uint64_t iv[IPSEC_MAX_IV_QWORD], rte_be64_t sqn) {
  iv[0] = sqn;
  iv[1] = 0;
}
...
aead_chacha20_poly1305_aad_fill(struct aead_chacha20_poly1305_aad *aad,
                                rte_be32_t spi, rte_be64_t sqn, int esn) {
  aad->spi = spi;
  if (esn)
    aad->sqn.u64 = sqn;
  else {
...
static inline void copy_iv(uint64_t dst[IPSEC_MAX_IV_QWORD],
                           const uint64_t src[IPSEC_MAX_IV_QWORD],
                           uint32_t len) {
 ...
 */
static inline void insert_sqh(uint32_t sqh, void *picv, uint32_t icv_len) {
  uint32_t *icv;
  int32_t i;
 ...
 * icv parameter points to the new start of ICV.
 */
static inline void remove_sqh(void *picv, uint32_t icv_len) {
  uint32_t i, *icv;
 ...
 */
static inline void lksd_none_cop_prepare(struct rte_crypto_op *cop,
