 ...
 */
static inline int add_specific(struct rte_ipsec_sad *sad, const void *key,
  ...
  ret = rte_hash_lookup_with_hash_data(
      sad->hash[RTE_IPSEC_SAD_SPI_ONLY], key,
      rte_hash_crc(key, sad->keysize[RTE_IPSEC_SAD_SPI_ONLY], sad->init_val),
      &tmp_val);
  if (ret < 0)
    tmp_val = NULL;
...
int rte_ipsec_sad_add(struct rte_ipsec_sad *sad,
                      const union rte_ipsec_sad_key *key, int key_type,
                      void *sa) {
  void *tmp_val;
  int ret;
  ...
  case (RTE_IPSEC_SAD_SPI_ONLY):
    ret = rte_hash_lookup_with_hash_data(
        sad->hash[key_type], key,
        rte_hash_crc(key, sad->keysize[key_type], sad->init_val), &tmp_val);
    if (ret >= 0)
      tmp_val = SET_BIT(sa, GET_BIT(tmp_val, RTE_IPSEC_SAD_KEY_TYPE_MASK));
    else
      tmp_val = sa;
    ret = rte_hash_add_key_with_hash_data(
        sad->hash[key_type], key,
 ...
 */
static inline int del_specific(struct rte_ipsec_sad *sad, const void *key,
                               int key_type) {
  void *tmp_val;
  int ret;
  uint32_t *cnt;
  ...
  if (ret < 0)
    return ret;
  cnt = (key_type == RTE_IPSEC_SAD_SPI_DIP) ? &sad->cnt_arr[ret].cnt_dip
...
int rte_ipsec_sad_del(struct rte_ipsec_sad *sad,
  ...
  case (RTE_IPSEC_SAD_SPI_ONLY):
    ...
    if (GET_BIT(tmp_val, RTE_IPSEC_SAD_KEY_TYPE_MASK) == 0) {
      ...
      tmp_val = GET_BIT(tmp_val, RTE_IPSEC_SAD_KEY_TYPE_MASK);
      ret = rte_hash_add_key_with_hash_data(
          sad->hash[key_type], key,
          rte_hash_crc(key, sad->keysize[key_type], sad->init_val), tmp_val);
    }
...
rte_ipsec_sad_create(const char *name, const struct rte_ipsec_sad_conf *conf) {
  ...
  /** Init SAD*/
  sa_sum = RTE_MAX(MIN_HASH_ENTRIES, conf->max_sa[RTE_IPSEC_SAD_SPI_ONLY]) +
           RTE_MAX(MIN_HASH_ENTRIES, conf->max_sa[RTE_IPSEC_SAD_SPI_DIP]) +
           RTE_MAX(MIN_HASH_ENTRIES, conf->max_sa[RTE_IPSEC_SAD_SPI_DIP_SIP]);
  sad = rte_zmalloc_socket(NULL,
                           sizeof(*sad) + (sizeof(struct hash_cnt) * sa_sum),
  ...
  if (conf->flags & RTE_IPSEC_SAD_FLAG_IPV6)
    hash_params.key_len += sizeof(((struct rte_ipsec_sadv6_key *)0)->dip);
  else
    hash_params.key_len += sizeof(((struct rte_ipsec_sadv4_key *)0)->dip);
  ...
  if (conf->flags & RTE_IPSEC_SAD_FLAG_IPV6)
    hash_params.key_len += sizeof(((struct rte_ipsec_sadv6_key *)0)->sip);
  else
    hash_params.key_len += sizeof(((struct rte_ipsec_sadv4_key *)0)->sip);
  ...
  sad_list = RTE_TAILQ_CAST(rte_ipsec_sad_tailq.head, rte_ipsec_sad_list);
  rte_mcfg_tailq_write_lock();
  /* guarantee there's no existing */
  TAILQ_FOREACH(te, sad_list, next) {
    tmp_sad = (struct rte_ipsec_sad *)te->data;
    if (strncmp(sad_name, tmp_sad->name, RTE_IPSEC_SAD_NAMESIZE) == 0)
      break;
  }
  if (te != NULL) {
 ...
 */
static int __ipsec_sad_lookup(const struct rte_ipsec_sad *sad,
                              const union rte_ipsec_sad_key *keys[], void *sa[],
                              uint32_t n) {
  ...
  for (i = 0; i < n; i++) {
    sa[i] = NULL;
    hash_sig[i] = rte_hash_crc_4byte(keys[i]->v4.spi, sad->init_val);
  }
  ...
  rte_hash_lookup_with_hash_bulk_data(sad->hash[RTE_IPSEC_SAD_SPI_ONLY],
                                      (const void **)keys, hash_sig, n, &mask_1,
                                      sa);
  for (map = mask_1; map; map &= (map - 1)) {
    ...
    if ((uintptr_t)sa[i] & RTE_IPSEC_SAD_SPI_DIP_SIP) {
      idx_3[n_3] = i;
      hash_sig_3[n_3] = rte_hash_crc(
          keys[i], sad->keysize[RTE_IPSEC_SAD_SPI_DIP_SIP], sad->init_val);
      keys_3[n_3++] = keys[i];
    }
    if ((uintptr_t)sa[i] & RTE_IPSEC_SAD_SPI_DIP) {
      idx_2[n_2] = i;
      hash_sig_2[n_2] = rte_hash_crc(
          keys[i], sad->keysize[RTE_IPSEC_SAD_SPI_DIP], sad->init_val);
      keys_2[n_2++] = keys[i];
    }
  ...
  if (n_2 != 0) {
    rte_hash_lookup_with_hash_bulk_data(sad->hash[RTE_IPSEC_SAD_SPI_DIP],
                                        keys_2, hash_sig_2, n_2, &mask_2,
                                        vals_2);
  ...
  if (n_3 != 0) {
    rte_hash_lookup_with_hash_bulk_data(sad->hash[RTE_IPSEC_SAD_SPI_DIP_SIP],
                                        keys_3, hash_sig_3, n_3, &mask_3,
                                        vals_3);
...
int rte_ipsec_sad_lookup(const struct rte_ipsec_sad *sad,
  ...
  do {
    ...
    found += __ipsec_sad_lookup(sad, &keys[i], &sa[i], num);
    i += num;
  } while (i != n);
