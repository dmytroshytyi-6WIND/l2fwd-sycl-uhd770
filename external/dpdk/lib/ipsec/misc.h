 ...
 */
static inline void move_bad_mbufs(struct rte_mbuf *mb[],
                                  const uint32_t bad_idx[], uint32_t nb_mb,
                                  uint32_t nb_bad) {
  uint32_t i, j, k;
 ...
 */
static inline struct rte_mbuf *mbuf_get_seg_ofs(struct rte_mbuf *mb,
                                                uint32_t *ofs) {
  uint32_t k, n, plen;
  struct rte_mbuf *ms;
  ...
  if (n == plen) {
    ...
    ms = mb;
    for (k = rte_pktmbuf_data_len(ms); n >= k; k = rte_pktmbuf_data_len(ms)) {
      ms = ms->next;
      n -= k;
    }
  }
 ...
 */
static inline void mbuf_cut_seg_ofs(struct rte_mbuf *mb, struct rte_mbuf *ms,
 ...
 */
static inline void cpu_crypto_bulk(
    const struct rte_ipsec_session *ss, union rte_crypto_sym_ofs ofs,
    struct rte_mbuf *mb[], struct rte_crypto_va_iova_ptr iv[],
    struct rte_crypto_va_iova_ptr aad[], struct rte_crypto_va_iova_ptr dgst[],
    uint32_t l4ofs[], uint32_t clen[], uint32_t num) {
  uint32_t i, j, n;
  int32_t vcnt, vofs;
  int32_t st[num];
  ...
  for (i = 0; i != num; i++) {
    ...
    vcnt = rte_crypto_mbuf_to_vec(mb[i], l4ofs[i], clen[i], &vec[vofs],
                                  vnum - vofs);
    ...
    if (vcnt < 0) {
      ...
      vofs = 0;
      vcnt = rte_crypto_mbuf_to_vec(mb[i], l4ofs[i], clen[i], vec, vnum);
      RTE_ASSERT(vcnt > 0);
      j = i;
    }
  ...
  n += rte_cryptodev_sym_cpu_crypto_process(ss->crypto.dev_id, ss->crypto.ses,
                                            ofs, &symvec);
