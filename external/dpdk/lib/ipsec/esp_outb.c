 ...
 */
static inline void outb_cop_prepare(struct rte_crypto_op *cop,
                                    const struct rte_ipsec_sa *sa,
                                    const uint64_t ivp[IPSEC_MAX_IV_QWORD],
                                    const union sym_op_data *icv, uint32_t hlen,
                                    uint32_t plen) {
...
outb_tun_pkt_prepare(struct rte_ipsec_sa *sa, rte_be64_t sqc,
                     const uint64_t ivp[IPSEC_MAX_IV_QWORD],
                     struct rte_mbuf *mb, union sym_op_data *icv,
                     uint8_t sqh_len, uint8_t tso) {
  ...
  mb->tx_offload = (mb->tx_offload & sa->tx_offload.msk) | sa->tx_offload.val;
  ...
  if (sa->type & RTE_IPSEC_SATP_NATT_ENABLE) {
    struct rte_udp_hdr *udph =
        (struct rte_udp_hdr *)(ph - sizeof(struct rte_udp_hdr));
    udph->dgram_len =
 ...
 */
static inline void outb_pkt_xprepare(const struct rte_ipsec_sa *sa,
                                     rte_be64_t sqc,
                                     const union sym_op_data *icv) {
  uint32_t *psqh;
  ...
  case ALGO_TYPE_CHACHA20_POLY1305:
    if (sa->aad_len != 0) {
      chacha20_poly1305_aad =
          (struct aead_chacha20_poly1305_aad *)(icv->va + sa->icv_len);
      aead_chacha20_poly1305_aad_fill(chacha20_poly1305_aad, sa->spi, sqc,
                                      IS_ESN(sa));
    }
    break;
  default:
 ...
 */
uint16_t esp_outb_tun_prepare(const struct rte_ipsec_session *ss,
                              struct rte_mbuf *mb[],
  ...
  for (i = 0; i != n; i++) {
    ...
    rc = outb_tun_pkt_prepare(sa, sqc, iv, mb[i], &icv, sa->sqh_len, 0);
...
outb_trs_pkt_prepare(struct rte_ipsec_sa *sa, rte_be64_t sqc,
  ...
  np = update_trs_l3hdr(sa, ph + l2len, mb->pkt_len - sqh_len, l2len, l3len,
                        IPPROTO_ESP);
 ...
 */
uint16_t esp_outb_trs_prepare(const struct rte_ipsec_session *ss,
                              struct rte_mbuf *mb[],
                              struct rte_crypto_op *cop[], uint16_t num) {
  int32_t rc;
  ...
  for (i = 0; i != n; i++) {
    ...
    /* try to update the packet itself */
    rc = outb_trs_pkt_prepare(sa, sqc, iv, mb[i], &icv, sa->sqh_len, 0);
    /* success, setup crypto op */
    if (rc >= 0) {
      outb_pkt_xprepare(sa, sqc, &icv);
      lksd_none_cop_prepare(cop[k], cs, mb[i]);
...
static inline uint32_t outb_cpu_crypto_prepare(const struct rte_ipsec_sa *sa,
                                               uint32_t *pofs, uint32_t plen,
                                               void *iv) {
  uint64_t *ivp = iv;
  struct aead_gcm_iv *gcm;
...
static uint16_t cpu_outb_pkt_prepare(const struct rte_ipsec_session *ss,
                                     struct rte_mbuf *mb[], uint16_t num,
                                     esp_outb_prepare_t prepare,
                                     uint32_t cofs_mask) {
  int32_t rc;
  uint64_t sqn;
  rte_be64_t sqc;
  ...
  for (i = 0, k = 0; i != n; i++) {
    ...
    if (rc >= 0) {
      ...
      clen[k] = outb_cpu_crypto_prepare(sa, l4ofs + k, rc, ivbuf[k]);
...
uint16_t cpu_outb_trs_pkt_prepare(const struct rte_ipsec_session *ss,
                                  struct rte_mbuf *mb[], uint16_t num) {
  return cpu_outb_pkt_prepare(ss, mb, num, outb_trs_pkt_prepare, UINT32_MAX);
}
 ...
 */
uint16_t esp_outb_sqh_process(const struct rte_ipsec_session *ss,
                              struct rte_mbuf *mb[], uint16_t num) {
  ...
  for (i = 0; i != num; i++) {
    if ((mb[i]->ol_flags & RTE_MBUF_F_RX_SEC_OFFLOAD_FAILED) == 0) {
      ...
      icv = rte_pktmbuf_mtod_offset(ml, void *, ml->data_len - icv_len);
 ...
 */
static inline void inline_outb_mbuf_prepare(const struct rte_ipsec_session *ss,
                                            struct rte_mbuf *mb[],
                                            uint16_t num) {
  ...
  for (i = 0; i != num; i++) {
    ...
    bytes += mb[i]->pkt_len;
    if (ol_flags != 0)
      rte_security_set_pkt_metadata(ss->security.ctx, ss->security.ses, mb[i],
                                    NULL);
  }
...
static inline int esn_outb_nb_segments(struct rte_mbuf *m) {
  if (m->ol_flags & (RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_UDP_SEG)) {
    uint16_t pkt_l3len = m->pkt_len - m->l2_len;
    uint16_t segments = (m->tso_segsz > 0 && pkt_l3len > m->tso_segsz)
                            ? (pkt_l3len + m->tso_segsz - 1) / m->tso_segsz
                            : 1;
    return segments;
  }
  return 1; /* no TSO */
...
static inline uint16_t esn_outb_nb_valid_packets(uint16_t num, uint32_t n_sqn,
                                                 uint16_t nb_segs[]) {
  uint16_t i;
  uint32_t seg_cnt = 0;
 ...
 */
uint16_t inline_outb_tun_pkt_process(const struct rte_ipsec_session *ss,
                                     struct rte_mbuf *mb[], uint16_t num) {
  int32_t rc;
  ...
  for (i = 0; i != num; i++) {
    ...
    rc = outb_tun_pkt_prepare(sa, sqc, iv, mb[i], &icv, 0,
                              (mb[i]->ol_flags & (RTE_MBUF_F_TX_TCP_SEG |
 ...
 * INLINE_CRYPTO type of device.
 */
uint16_t inline_outb_trs_pkt_process(const struct rte_ipsec_session *ss,
                                     struct rte_mbuf *mb[], uint16_t num) {
  int32_t rc;
  uint32_t i, k, nb_segs_total, n_sqn;
  uint64_t sqn;
 ...
 */
uint16_t inline_proto_outb_pkt_process(const struct rte_ipsec_session *ss,
                                       struct rte_mbuf *mb[], uint16_t num) {
