 ...
 */
static int fill_crypto_xform(struct crypto_xform *xform, uint64_t type,
...
uint64_t rte_ipsec_sa_type(const struct rte_ipsec_sa *sa) { return sa->type; }
 ...
 */
static size_t rsn_size(uint32_t nb_bucket) {
  size_t sz;
 ...
 */
static uint32_t replay_num_bucket(uint32_t wsz) {
  uint32_t nb;
...
static int32_t ipsec_sa_size(uint64_t type, uint32_t *wnd_sz,
                             uint32_t *nb_bucket) {
  uint32_t n, sz, wsz;
  ...
  if ((type & RTE_IPSEC_SATP_DIR_MASK) == RTE_IPSEC_SATP_DIR_IB) {
     ...
     */
    wsz = ((type & RTE_IPSEC_SATP_ESN_MASK) == RTE_IPSEC_SATP_ESN_DISABLE)
              ? wsz
              : RTE_MAX(wsz, (uint32_t)WINDOW_BUCKET_SIZE);
    if (wsz != 0)
...
void rte_ipsec_sa_fini(struct rte_ipsec_sa *sa) { memset(sa, 0, sa->size); }

/*
 * Determine expected SA type based on input parameters.
 */
static int fill_sa_type(const struct rte_ipsec_sa_prm *prm, uint64_t *type) {
  uint64_t tp;
  ...
  else if (prm->ipsec_xform.direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS)
    tp |= RTE_IPSEC_SATP_DIR_IB;
  else
    return -EINVAL;

  if (prm->ipsec_xform.mode == RTE_SECURITY_IPSEC_SA_MODE_TUNNEL) {
    if (prm->ipsec_xform.tunnel.type == RTE_SECURITY_IPSEC_TUNNEL_IPV4)
      tp |= RTE_IPSEC_SATP_MODE_TUNLV4;
    else if (prm->ipsec_xform.tunnel.type == RTE_SECURITY_IPSEC_TUNNEL_IPV6)
    ...
    if (prm->tun.next_proto == IPPROTO_IPIP)
      ...
      return -EINVAL;
  } else if (prm->ipsec_xform.mode == RTE_SECURITY_IPSEC_SA_MODE_TRANSPORT) {
 ...
 */
static void esp_inb_init(struct rte_ipsec_sa *sa) {
 ...
 */
static void esp_inb_tun_init(struct rte_ipsec_sa *sa,
                             const struct rte_ipsec_sa_prm *prm) {
 ...
 */
static void esp_outb_init(struct rte_ipsec_sa *sa, uint32_t hlen,
                          uint64_t sqn) {
  uint8_t algo_type;
  ...
  case ALGO_TYPE_NULL:
    sa->ctp.cipher.offset = hlen + sizeof(struct rte_esp_hdr) + sa->iv_len;
 ...
 */
static void esp_outb_tun_init(struct rte_ipsec_sa *sa,
                              const struct rte_ipsec_sa_prm *prm) {
  sa->proto = prm->tun.next_proto;
  ...
  if (sa->type & RTE_IPSEC_SATP_NATT_ENABLE) {
    struct rte_udp_hdr *udph = (struct rte_udp_hdr *)&sa->hdr[prm->tun.hdr_len];
  ...
  sa->tx_offload.val = rte_mbuf_tx_offload(
      sa->hdr_l3_off, sa->hdr_len - sa->hdr_l3_off, 0, 0, 0, 0, 0);
 ...
 */
static int esp_sa_init(struct rte_ipsec_sa *sa,
                       const struct rte_ipsec_sa_prm *prm,
  ...
  sa->tx_offload.msk =
      ~rte_mbuf_tx_offload(MBUF_MAX_L2_LEN, MBUF_MAX_L3_LEN, 0, 0, 0, 0, 0);
 ...
 */
static void fill_sa_replay(struct rte_ipsec_sa *sa, uint32_t wnd_sz,
                           uint32_t nb_bucket, uint64_t sqn) {
  ...
  if ((sa->type & RTE_IPSEC_SATP_SQN_MASK) == RTE_IPSEC_SATP_SQN_ATOM) {
    sa->sqn.inb.rsn[1] = (struct replay_sqn *)((uintptr_t)sa->sqn.inb.rsn[0] +
                                               rsn_size(nb_bucket));
...
int rte_ipsec_sa_init(struct rte_ipsec_sa *sa,
                      const struct rte_ipsec_sa_prm *prm, uint32_t size) {
  int32_t rc, sz;
  uint32_t nb, wsz;
  uint64_t type;
  ...
  /* check for ESN flag */
  sa->sqn_mask = (prm->ipsec_xform.options.esn == 0) ? UINT32_MAX : UINT64_MAX;
 ...
 */
static inline void lksd_proto_cop_prepare(const struct rte_ipsec_session *ss,
                                          struct rte_mbuf *mb[],
 ...
 */
static uint16_t lksd_proto_prepare(const struct rte_ipsec_session *ss,
                                   struct rte_mbuf *mb[],
 ...
 */
uint16_t pkt_flag_process(const struct rte_ipsec_session *ss,
 ...
 * type of device.
 */
static int lksd_none_pkt_func_select(const struct rte_ipsec_sa *sa,
                                     struct rte_ipsec_sa_pkt_func *pf) {
  int32_t rc;

  static const uint64_t msk =
      RTE_IPSEC_SATP_DIR_MASK | RTE_IPSEC_SATP_MODE_MASK;
  ...
  case (RTE_IPSEC_SATP_DIR_OB | RTE_IPSEC_SATP_MODE_TUNLV6):
    ...
    pf->process = (sa->sqh_len != 0) ? esp_outb_sqh_process : pkt_flag_process;
    break;
  case (RTE_IPSEC_SATP_DIR_OB | RTE_IPSEC_SATP_MODE_TRANS):
...
static int cpu_crypto_pkt_func_select(const struct rte_ipsec_sa *sa,
                                      struct rte_ipsec_sa_pkt_func *pf) {
  int32_t rc;
 ...
 * type of device.
 */
static int inline_crypto_pkt_func_select(const struct rte_ipsec_sa *sa,
                                         struct rte_ipsec_sa_pkt_func *pf) {
  int32_t rc;
 ...
 */
int ipsec_sa_pkt_func_select(const struct rte_ipsec_session *ss,
