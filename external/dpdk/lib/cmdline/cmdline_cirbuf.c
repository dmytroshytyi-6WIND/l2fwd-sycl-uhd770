...
int cirbuf_init(struct cirbuf *cbuf, char *buf, unsigned int start,
                unsigned int maxlen) {
  if (!cbuf || !buf)
    return -EINVAL;
  cbuf->maxlen = maxlen;
...
void cirbuf_del_head(struct cirbuf *cbuf) { __cirbuf_del_head(cbuf); }
...
void cirbuf_del_tail(struct cirbuf *cbuf) { __cirbuf_del_tail(cbuf); }
...
char cirbuf_get_head(struct cirbuf *cbuf) { return cbuf->buf[cbuf->start]; }

/* get head or get tail */

char cirbuf_get_tail(struct cirbuf *cbuf) { return cbuf->buf[cbuf->end]; }
