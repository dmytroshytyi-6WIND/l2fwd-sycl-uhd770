...
static void rdline_miniprintf(struct rdline *rdl, const char *buf,
                              unsigned int val);
...
int rdline_init(struct rdline *rdl, rdline_write_char_t *write_char,
...
struct rdline *rdline_new(rdline_write_char_t *write_char,
                          ...
                          rdline_complete_t *complete, void *opaque) {
  struct rdline *rdl;
...
void rdline_free(struct rdline *rdl) { free(rdl); }
...
static void display_right_buffer(struct rdline *rdl, int force) {
  unsigned int i;
  char tmp;
  ...
  CIRBUF_FOREACH(&rdl->right, i, tmp) { rdl->write_char(rdl, tmp); }
  if (!CIRBUF_IS_EMPTY(&rdl->right))
    rdline_miniprintf(rdl, vt100_multi_left, CIRBUF_GET_LEN(&rdl->right));
}
...
void rdline_redisplay(struct rdline *rdl) {
  ...
  for (i = 0; i < rdl->prompt_size; i++)
    rdl->write_char(rdl, rdl->prompt[i]);
  CIRBUF_FOREACH(&rdl->left, i, tmp) { rdl->write_char(rdl, tmp); }
...
int rdline_char_in(struct rdline *rdl, char c) {
  ...
  if (cmd >= 0) {
    ...
    case CMDLINE_KEY_WLEFT:
      while (!CIRBUF_IS_EMPTY(&rdl->left) &&
             (tmp = cirbuf_get_tail(&rdl->left)) && isblank2(tmp)) {
        rdline_puts(rdl, vt100_left_arr);
        cirbuf_del_tail(&rdl->left);
        cirbuf_add_head(&rdl->right, tmp);
      }
    ...
    case CMDLINE_KEY_WRIGHT:
      while (!CIRBUF_IS_EMPTY(&rdl->right) &&
             (tmp = cirbuf_get_head(&rdl->right)) && isblank2(tmp)) {
        rdline_puts(rdl, vt100_right_arr);
        cirbuf_del_head(&rdl->right);
        cirbuf_add_tail(&rdl->left, tmp);
      }
    ...
    case CMDLINE_KEY_CTRL_A:
      if (CIRBUF_IS_EMPTY(&rdl->left))
        break;
      rdline_miniprintf(rdl, vt100_multi_left, CIRBUF_GET_LEN(&rdl->left));
      while (!CIRBUF_IS_EMPTY(&rdl->left)) {
        tmp = cirbuf_get_tail(&rdl->left);
        cirbuf_del_tail(&rdl->left);
        cirbuf_add_head(&rdl->right, tmp);
    ...
    case CMDLINE_KEY_CTRL_E:
      if (CIRBUF_IS_EMPTY(&rdl->right))
        break;
      rdline_miniprintf(rdl, vt100_multi_right, CIRBUF_GET_LEN(&rdl->right));
      while (!CIRBUF_IS_EMPTY(&rdl->right)) {
        tmp = cirbuf_get_head(&rdl->right);
        cirbuf_del_head(&rdl->right);
        cirbuf_add_tail(&rdl->left, tmp);
    ...
    case CMDLINE_KEY_CTRL_D:
      if (cmd == CMDLINE_KEY_CTRL_D && CIRBUF_IS_EMPTY(&rdl->left) &&
    ...
    case CMDLINE_KEY_HELP:
      ...
      if (rdl->complete) {
        ...
        ret = rdl->complete(rdl, rdl->left_buf, tmp_buf, sizeof(tmp_buf),
                            &complete_state);
  ...
  /* standard chars */
  if (CIRBUF_GET_LEN(&rdl->left) + CIRBUF_GET_LEN(&rdl->right) >=
      RDLINE_BUF_SIZE)
    return RDLINE_RES_SUCCESS;
...
char *rdline_get_history_item(struct rdline *rdl, unsigned int idx) {
