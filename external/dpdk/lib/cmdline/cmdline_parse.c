    ...
    if (resbuf == NULL) {
      ...
      if (token_hdr.offset > resbuf_size) {
        printf("Parse error(%s:%d): Token offset(%u) "
               ...
               __FILE__, __LINE__, token_hdr.offset, resbuf_size);
        return -ENOBUFS;
      }
      rb_sz = resbuf_size - token_hdr.offset;

      n = token_hdr.ops->parse(token_p, buf, (char *)resbuf + token_hdr.offset,
                               rb_sz);
    }
...
int cmdline_parse(struct cmdline *cl, const char *buf) {
  ...
  while (inst) {
    ...
    /* fully parsed */
    tok = match_inst(inst, buf, 0, result_buf, CMDLINE_PARSE_RESULT_BUFSIZE);
...
int cmdline_complete(struct cmdline *cl, const char *buf, int *state, char *dst,
    ...
    while (inst) {
      ...
      for (i = 0; i < n; i++) {
        ...
        if (!strncmp(partial_tok, tmpbuf, partial_tok_len)) {
          if (comp_len == -1) {
            strlcpy(comp_buf, tmpbuf + partial_tok_len, sizeof(comp_buf));
            comp_len = strnlen(tmpbuf + partial_tok_len,
                               sizeof(tmpbuf) - partial_tok_len);

          } else {
            comp_len = nb_common_chars(comp_buf, tmpbuf + partial_tok_len);
            comp_buf[comp_len] = 0;
          }
          nb_completable++;
        }
  ...
  while (inst) {
    ...
    /* several choices */
    for (i = 0; i < n; i++) {
      if (token_hdr.ops->complete_get_elt(token_p, i, tmpbuf, sizeof(tmpbuf)) <
          0)
        continue;
      /* we have at least room for one char */
      tmp_len = strnlen(tmpbuf, sizeof(tmpbuf));
