...
static void cmdline_valid_buffer(struct rdline *rdl, const char *buf,
...
static int cmdline_complete_buffer(struct rdline *rdl, const char *buf,
  ...
  for (i = 0; i < size; i++) {
    ...
    if (ret == RDLINE_RES_VALIDATED) {
      ...
      if (history) {
        ...
        same = !memcmp(buffer, history, histlen) && buffer[histlen] == '\n';
      } else
        same = 0;
