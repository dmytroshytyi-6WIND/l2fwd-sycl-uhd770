...
void terminal_adjust(struct cmdline *cl) {
  ...
  if (GetConsoleMode(handle, &mode)) {
    ...
    mode &= ~(ENABLE_LINE_INPUT |      /* no line buffering */
...
static int cmdline_is_key_down(const INPUT_RECORD *record) {
  return (record->EventType == KEY_EVENT) && record->Event.KeyEvent.bKeyDown;
}

static int cmdline_poll_char_console(HANDLE handle) {
  INPUT_RECORD record;
  DWORD events;
...
static int cmdline_poll_char_file(struct cmdline *cl, HANDLE handle) {
...
int cmdline_poll_char(struct cmdline *cl) {
  ...
  return cl->oldterm.is_console_input ? cmdline_poll_char_console(handle)
