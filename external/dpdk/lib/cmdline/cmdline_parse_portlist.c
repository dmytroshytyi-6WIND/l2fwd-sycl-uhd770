...
static void parse_set_list(cmdline_portlist_t *pl, size_t low, size_t high) {
  do {
    pl->map |= (1 << low++);
  } while (low <= high);
}

static int parse_ports(cmdline_portlist_t *pl, const char *str) {
  size_t ps, pe;
  const char *first, *last;
  char *end;

  for (first = str, last = first; first != NULL && last != NULL;
       first = last + 1) {
    ...
    if (end[0] == '-') {
      ...
      if (errno != 0 || end == first || (end[0] != 0 && end != last))
        return -1;
    } else {
      pe = ps;
    }
...
int cmdline_parse_portlist(__rte_unused cmdline_parse_token_hdr_t *tk,
...
int cmdline_get_help_portlist(__rte_unused cmdline_parse_token_hdr_t *tk,
