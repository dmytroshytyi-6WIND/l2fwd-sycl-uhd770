...
static inline void update_ipv4_udp_headers(struct rte_mbuf *pkt,
  ...
  for (i = 0; i < nb_segs; i++) {
    ipv4_hdr =
        rte_pktmbuf_mtod_offset(segs[i], struct rte_ipv4_hdr *, l2_hdrlen);
    length = segs[i]->pkt_len - l2_hdrlen;
    ipv4_hdr->total_length = rte_cpu_to_be_16(length);

    is_mf = i < tail_idx ? IPV4_HDR_MF_BIT : 0;
    ipv4_hdr->fragment_offset = rte_cpu_to_be_16(frag_offset | is_mf);
    frag_offset += ((length - l3_hdrlen) >> 3);
  }
}

int gso_udp4_segment(struct rte_mbuf *pkt, uint16_t gso_size,
  ...
  ipv4_hdr = rte_pktmbuf_mtod_offset(pkt, struct rte_ipv4_hdr *, pkt->l2_len);
