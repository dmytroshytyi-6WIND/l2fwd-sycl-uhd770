...
static inline void hdr_segment_init(struct rte_mbuf *hdr_segment,
                                    struct rte_mbuf *pkt,
  ...
  rte_memcpy(rte_pktmbuf_mtod(hdr_segment, char *),
             rte_pktmbuf_mtod(pkt, char *), pkt_hdr_offset);
}

static inline void free_gso_segment(struct rte_mbuf **pkts, uint16_t nb_pkts) {
  uint16_t i;
...
int gso_do_segment(struct rte_mbuf *pkt, uint16_t pkt_hdr_offset,
  ...
  while (more_in_pkt) {
    ...
    while (more_out_segs && more_in_pkt) {
      ...
      pyld_segment->data_off = pkt_in_data_pos + pkt_in->data_off;
