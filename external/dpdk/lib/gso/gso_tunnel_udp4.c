...
static void update_tunnel_ipv4_udp_headers(struct rte_mbuf *pkt,
  ...
  for (i = 0; i < nb_segs; i++) {
     ...
     */
    ipv4_hdr = (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(segs[i], char *) +
                                       inner_ipv4_offset);
...
int gso_tunnel_udp4_segment(struct rte_mbuf *pkt, uint16_t gso_size,
  ...
  inner_ipv4_hdr =
      (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(pkt, char *) + hdr_offset);
  /*
