 ...
 */
static inline void update_udp_header(struct rte_mbuf *pkt,
                                     uint16_t udp_offset) {
  struct rte_udp_hdr *udp_hdr;

  udp_hdr = (struct rte_udp_hdr *)(rte_pktmbuf_mtod(pkt, char *) + udp_offset);
 ...
 */
static inline void update_tcp_header(struct rte_mbuf *pkt, uint16_t l4_offset,
                                     uint32_t sent_seq, uint8_t non_tail) {
  struct rte_tcp_hdr *tcp_hdr;

  tcp_hdr = (struct rte_tcp_hdr *)(rte_pktmbuf_mtod(pkt, char *) + l4_offset);
 ...
 */
static inline void update_ipv4_header(struct rte_mbuf *pkt, uint16_t l3_offset,
                                      uint16_t id) {
  struct rte_ipv4_hdr *ipv4_hdr;

  ipv4_hdr = (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(pkt, char *) + l3_offset);
 ...
 */
int gso_do_segment(struct rte_mbuf *pkt, uint16_t pkt_hdr_offset,
                   uint16_t pyld_unit_size, struct rte_mempool *direct_pool,
                   struct rte_mempool *indirect_pool,
                   struct rte_mbuf **pkts_out, uint16_t nb_pkts_out);
#endif
