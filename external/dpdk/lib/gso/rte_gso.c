...
int rte_gso_segment(struct rte_mbuf *pkt, const struct rte_gso_ctx *gso_ctx,
  ...
  if (pkt == NULL || pkts_out == NULL || gso_ctx == NULL || nb_pkts_out < 1 ||
  ...
  if ((IS_IPV4_VXLAN_TCP4(pkt->ol_flags) &&
    ...
    ret = gso_tunnel_tcp4_segment(pkt, gso_size, ipid_delta, direct_pool,
    ...
    ret = gso_tunnel_udp4_segment(pkt, gso_size, direct_pool, indirect_pool,
                                  pkts_out, nb_pkts_out);
  } else if (IS_IPV4_TCP(pkt->ol_flags) &&
             (gso_ctx->gso_types & RTE_ETH_TX_OFFLOAD_TCP_TSO)) {
    pkt->ol_flags &= (~RTE_MBUF_F_TX_TCP_SEG);
    ret = gso_tcp4_segment(pkt, gso_size, ipid_delta, direct_pool,
                           indirect_pool, pkts_out, nb_pkts_out);
  } else if (IS_IPV4_UDP(pkt->ol_flags) &&
             (gso_ctx->gso_types & RTE_ETH_TX_OFFLOAD_UDP_TSO)) {
    pkt->ol_flags &= (~RTE_MBUF_F_TX_UDP_SEG);
    ret = gso_udp4_segment(pkt, gso_size, direct_pool, indirect_pool, pkts_out,
                           nb_pkts_out);
  } else {
