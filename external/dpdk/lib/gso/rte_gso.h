...
#define RTE_GSO_UDP_SEG_SIZE_MIN                                               \
  (sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) +                \
   sizeof(struct rte_udp_hdr) + 1)
 ...
 * For example, set RTE_MBUF_F_TX_TCP_SEG and RTE_MBUF_F_TX_IPV4 in ol_flags to
 * segment a TCP/IPv4 packet. If rte_gso_segment() succeeds, the
 * RTE_MBUF_F_TX_TCP_SEG flag is removed for all GSO segments and the input
 * packet.
 *
 ...
 */
int rte_gso_segment(struct rte_mbuf *pkt, const struct rte_gso_ctx *ctx,
