...
typedef void(rte_gpu_callback_t)(int16_t dev_id, enum rte_gpu_event event,
                                 void *user_data);
 ...
 */
__rte_experimental int16_t rte_gpu_add_child(const char *name, int16_t parent,
...
#define RTE_GPU_FOREACH_CHILD(dev_id, parent)                                  \
  for (dev_id = rte_gpu_find_next(0, parent); dev_id >= 0;                     \
 ...
 *   - EPERM if driver error
 */
__rte_experimental void *rte_gpu_mem_alloc(int16_t dev_id, size_t size,
                                           unsigned int align)
    __rte_alloc_size(2);
 ...
 *   - EPERM if driver error
 */
__rte_experimental int rte_gpu_mem_register(int16_t dev_id, size_t size,
                                            void *ptr);
 ...
 * @b EXPERIMENTAL: this API may change without prior notice.
 *
 * Deregister a chunk of memory previously registered with
 ...
 * @return
 *   A pointer to the mapped GPU memory usable by the CPU, otherwise NULL and
 rte_errno is set:
 *   - ENODEV if invalid dev_id
 *   - ENOTSUP if operation not supported by the driver
 *   - E2BIG if size is higher than limit
 ...
 *   - EPERM if driver error
 */
__rte_experimental void *rte_gpu_mem_cpu_map(int16_t dev_id, size_t size,
                                             void *ptr);
