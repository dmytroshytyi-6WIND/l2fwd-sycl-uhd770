...
/* Logging */
RTE_LOG_REGISTER_DEFAULT(gpu_logtype, DEBUG);
#define GPU_LOG(level, ...)                                                    \
  rte_log(RTE_LOG_##level, gpu_logtype,                                        \
          RTE_FMT("gpu: " RTE_FMT_HEAD(__VA_ARGS__, ) "\n",                    \
...
uint16_t rte_gpu_count_avail(void) { return gpu_count; }
...
static bool gpu_match_parent(int16_t dev_id, int16_t parent) {
...
static int16_t gpu_find_free_id(void) {
  int16_t dev_id;
...
static struct rte_gpu *gpu_get_by_id(int16_t dev_id) {
...
static int gpu_shared_mem_init(void) {
...
struct rte_gpu *rte_gpu_allocate(const char *name) {
  ...
  gpu_count++;
  GPU_LOG(DEBUG, "new device %s (id %d) of total %d", name, dev_id, gpu_count);
  return dev;
}
...
struct rte_gpu *rte_gpu_attach(const char *name) {
  ...
  gpu_count++;
  GPU_LOG(DEBUG, "attached device %s (id %d) of total %d", name, dev_id,
          gpu_count);
  return dev;
}

int16_t rte_gpu_add_child(const char *name, int16_t parent,
...
int rte_gpu_release(struct rte_gpu *dev) {
  ...
  RTE_GPU_FOREACH_CHILD(child, dev_id) {
    GPU_LOG(ERR, "cannot release device %d with child %d", dev_id, child);
    rte_errno = EBUSY;
    return -rte_errno;
  }

  GPU_LOG(DEBUG, "free device %s (id %d)", dev->mpshared->info.name,
          dev->mpshared->info.dev_id);
...
int rte_gpu_callback_register(int16_t dev_id, enum rte_gpu_event event,
  ...
  do {
    ...
    TAILQ_FOREACH(callback, callbacks, next) {
      if (callback->event == event && callback->function == function &&
...
int rte_gpu_callback_unregister(int16_t dev_id, enum rte_gpu_event event,
  ...
  do {
    ...
    RTE_TAILQ_FOREACH_SAFE(callback, callbacks, next, nextcb) {
      if (callback->event != event || callback->function != function ||
          (callback->user_data != user_data && user_data != (void *)-1))
        continue;
...
void rte_gpu_notify(struct rte_gpu *dev, enum rte_gpu_event event) {
  int16_t dev_id;
  struct rte_gpu_callback *callback;
...
int rte_gpu_comm_set_flag(struct rte_gpu_comm_flag *devflag, uint32_t val) {
  if (devflag == NULL) {
    rte_errno = EINVAL;
    return -rte_errno;
  }
...
struct rte_gpu_comm_list *rte_gpu_comm_create_list(uint16_t dev_id,
  ...
  comm_list =
      rte_zmalloc(NULL, sizeof(struct rte_gpu_comm_list) * num_comm_items, 0);
  if (comm_list == NULL) {
    rte_errno = ENOMEM;
    return NULL;
  }
  printf("5");
  ret = rte_gpu_mem_register(
      dev_id, sizeof(struct rte_gpu_comm_list) * num_comm_items, comm_list);
  if (ret < 0) {
    rte_errno = ENOMEM;
    return NULL;
  }
   ...
   */
  comm_list[0].status_d = rte_gpu_mem_alloc(
      dev_id, sizeof(enum rte_gpu_comm_list_status) * num_comm_items,
      info.page_size);
  if (ret < 0) {
    rte_errno = ENOMEM;
    return NULL;
  ...
  if (comm_list[0].status_h == NULL) {
     ...
     */
    comm_list[0].status_h = rte_zmalloc(
        NULL, sizeof(enum rte_gpu_comm_list_status) * num_comm_items, 0);
    if (comm_list[0].status_h == NULL) {
      rte_errno = ENOMEM;
      printf("======8");
      return NULL;
  ...
  for (idx_l = 0; idx_l < num_comm_items; idx_l++) {
    comm_list[idx_l].pkt_list = rte_zmalloc(
    ...
    ret = rte_gpu_mem_register(
        dev_id, sizeof(struct rte_gpu_comm_pkt) * RTE_GPU_COMM_LIST_PKTS_MAX,
    ...
    comm_list[idx_l].mbufs = rte_zmalloc(
        NULL,
        // sizeof(struct rte_mbuf *) * RTE_GPU_COMM_LIST_PKTS_MAX, 0);
        sizeof(struct rte_mbuf *) * 128, 0);
