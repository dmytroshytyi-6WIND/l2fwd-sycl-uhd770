...
enum { WHITE, GREY, BLACK, MAX_NODE_COLOUR };

/* possible edge types */
enum { UNKNOWN_EDGE, TREE_EDGE, BACK_EDGE, CROSS_EDGE, MAX_EDGE_TYPE };
...
static const char *check_alu_bele(const struct ebpf_insn *ins) {
  if (ins->imm != 16 && ins->imm != 32 && ins->imm != 64)
    return "invalid imm field";
  return NULL;
}

static const char *eval_exit(struct bpf_verifier *bvf,
...
static void eval_umax_bound(struct bpf_reg_val *rv, uint64_t mask) {
  rv->u.max = mask;
  rv->u.min = 0;
}

static void eval_smax_bound(struct bpf_reg_val *rv, uint64_t mask) {
  rv->s.max = mask >> 1;
  rv->s.min = rv->s.max ^ UINT64_MAX;
}

static void eval_max_bound(struct bpf_reg_val *rv, uint64_t mask) {
  eval_umax_bound(rv, mask);
  eval_smax_bound(rv, mask);
}

static void eval_fill_max_bound(struct bpf_reg_val *rv, uint64_t mask) {
  eval_max_bound(rv, mask);
  rv->v.type = RTE_BPF_ARG_RAW;
  rv->mask = mask;
}

static void eval_fill_imm64(struct bpf_reg_val *rv, uint64_t mask,
                            uint64_t val) {
  rv->mask = mask;
  rv->s.min = val;
  rv->s.max = val;
...
static void eval_fill_imm(struct bpf_reg_val *rv, uint64_t mask, int32_t imm) {
  uint64_t v;
...
static const char *eval_ld_imm64(struct bpf_verifier *bvf,
                                 const struct ebpf_insn *ins) {
  uint32_t i;
  uint64_t val;
...
static void eval_apply_mask(struct bpf_reg_val *rv, uint64_t mask) {
...
static void eval_add(struct bpf_reg_val *rd, const struct bpf_reg_val *rs,
                     uint64_t msk) {
  ...
  if ((rd->s.min != rd->s.max || rs->s.min != rs->s.max) &&
      (((rs->s.min < 0 && rv.s.min > rd->s.min) || rv.s.min < rd->s.min) ||
       ((rs->s.max < 0 && rv.s.max > rd->s.max) || rv.s.max < rd->s.max)))
...
static void eval_sub(struct bpf_reg_val *rd, const struct bpf_reg_val *rs,
                     uint64_t msk) {
  struct bpf_reg_val rv;
...
static void eval_lsh(struct bpf_reg_val *rd, const struct bpf_reg_val *rs,
  ...
  if ((uint64_t)rd->s.min >> (opsz - 1) != 0 ||
      rd->s.max >= RTE_LEN2MASK(opsz - rs->u.max - 1, int64_t))
...
static void eval_rsh(struct bpf_reg_val *rd, const struct bpf_reg_val *rs,
                     size_t opsz, uint64_t msk) {
  /* check if shift value is less then max result bits */
  if (rs->u.max >= opsz) {
    eval_max_bound(rd, msk);
...
static void eval_arsh(struct bpf_reg_val *rd, const struct bpf_reg_val *rs,
                      size_t opsz, uint64_t msk) {
  uint32_t shv;
...
static uint64_t eval_umax_bits(uint64_t v, size_t opsz) {
  if (v == 0)
    return 0;
...
static uint64_t eval_uand_max(uint64_t v1, uint64_t v2, size_t opsz) {
...
/* estimate max possible value for (v1 | v2) */
static uint64_t eval_uor_max(uint64_t v1, uint64_t v2, size_t opsz) {
  v1 = eval_umax_bits(v1, opsz);
  v2 = eval_umax_bits(v2, opsz);
  return (v1 | v2);
}

static void eval_and(struct bpf_reg_val *rd, const struct bpf_reg_val *rs,
                     size_t opsz, uint64_t msk) {
  ...
  if (rd->s.min == rd->s.max && rs->s.min == rs->s.max) {
    ...
    rd->s.max =
        eval_uand_max(rd->s.max & (msk >> 1), rs->s.max & (msk >> 1), opsz);
    rd->s.min &= rs->s.min;
  } else
    eval_smax_bound(rd, msk);
}

static void eval_or(struct bpf_reg_val *rd, const struct bpf_reg_val *rs,
                    size_t opsz, uint64_t msk) {
  /* both operands are constants */
  if (rd->u.min == rd->u.max && rs->u.min == rs->u.max) {
    rd->u.min |= rs->u.min;
...
static void eval_xor(struct bpf_reg_val *rd, const struct bpf_reg_val *rs,
                     size_t opsz, uint64_t msk) {
  /* both operands are constants */
  if (rd->u.min == rd->u.max && rs->u.min == rs->u.max) {
    rd->u.min ^= rs->u.min;
...
static void eval_mul(struct bpf_reg_val *rd, const struct bpf_reg_val *rs,
                     size_t opsz, uint64_t msk) {
  /* both operands are constants */
  if (rd->u.min == rd->u.max && rs->u.min == rs->u.max) {
...
static const char *eval_divmod(uint32_t op, struct bpf_reg_val *rd,
                               struct bpf_reg_val *rs, size_t opsz,
                               uint64_t msk) {
  /* both operands are constants */
  if (rd->u.min == rd->u.max && rs->u.min == rs->u.max) {
    if (rs->u.max == 0)
...
static void eval_neg(struct bpf_reg_val *rd, size_t opsz, uint64_t msk) {
  uint64_t ux, uy;
  int64_t sx, sy;
...
static const char *eval_ld_mbuf(struct bpf_verifier *bvf,
                                const struct ebpf_insn *ins) {
  uint32_t i, mode;
 ...
 */
static const char *eval_defined(const struct bpf_reg_val *dst,
...
static const char *eval_alu(struct bpf_verifier *bvf,
                            const struct ebpf_insn *ins) {
  uint64_t msk;
  uint32_t op;
  size_t opsz;
  ...
  opsz =
      (BPF_CLASS(ins->code) == BPF_ALU) ? sizeof(uint32_t) : sizeof(uint64_t);
  opsz = opsz * CHAR_BIT;
  ...
  err =
      eval_defined((op != EBPF_MOV) ? rd : NULL, (op != BPF_NEG) ? &rs : NULL);
  if (err != NULL)
    return err;
...
static const char *eval_bele(struct bpf_verifier *bvf,
                             const struct ebpf_insn *ins) {
  uint64_t msk;
  struct bpf_eval_state *st;
  struct bpf_reg_val *rd;
...
static const char *eval_ptr(struct bpf_verifier *bvf, struct bpf_reg_val *rm,
                            uint32_t opsz, uint32_t align, int16_t off) {
  struct bpf_reg_val rv;
  ...
  if (rm->u.max + opsz > rm->v.size ||
      (uint64_t)rm->s.max + opsz > rm->v.size || rm->s.min < 0)
...
static void eval_max_load(struct bpf_reg_val *rv, uint64_t mask) {
  eval_umax_bound(rv, mask);
...
static const char *eval_load(struct bpf_verifier *bvf,
                             const struct ebpf_insn *ins) {
  uint32_t opsz;
  uint64_t msk;
  const char *err;
...
static const char *eval_mbuf_store(const struct bpf_reg_val *rv,
                                   uint32_t opsz) {
  uint32_t i;
  ...
  for (i = 0; i != RTE_DIM(mbuf_ro_fileds) &&
              (mbuf_ro_fileds[i].off + mbuf_ro_fileds[i].sz <= rv->u.max ||
               rv->u.max + opsz <= mbuf_ro_fileds[i].off);
       i++)
    ;
...
static const char *eval_store(struct bpf_verifier *bvf,
                              const struct ebpf_insn *ins) {
  uint32_t opsz;
  uint64_t msk;
  const char *err;
  ...
  if (rd.v.type == BPF_ARG_PTR_STACK) {
    ...
    if (BPF_CLASS(ins->code) == BPF_STX && BPF_MODE(ins->code) == EBPF_XADD)
...
static const char *eval_func_arg(struct bpf_verifier *bvf,
                                 const struct rte_bpf_arg *arg,
                                 struct bpf_reg_val *rv) {
  uint32_t i, n;
  struct bpf_eval_state *st;
  const char *err;
  ...
  if (arg->type != rv->v.type && arg->type != RTE_BPF_ARG_RAW &&
      (arg->type != RTE_BPF_ARG_PTR || RTE_BPF_ARG_PTR_TYPE(rv->v.type) == 0))
...
static const char *eval_call(struct bpf_verifier *bvf,
                             const struct ebpf_insn *ins) {
  uint32_t i, idx;
  struct bpf_reg_val *rv;
  ...
  for (i = 0; i != xsym->func.nb_args && err == NULL; i++) {
    err =
        eval_func_arg(bvf, xsym->func.args + i, bvf->evst->rv + EBPF_REG_1 + i);
  }
...
static void eval_jeq_jne(struct bpf_reg_val *trd, struct bpf_reg_val *trs) {
  /* sreg is constant */
...
static void eval_jgt_jle(struct bpf_reg_val *trd, struct bpf_reg_val *trs,
...
static void eval_jlt_jge(struct bpf_reg_val *trd, struct bpf_reg_val *trs,
                         struct bpf_reg_val *frd, struct bpf_reg_val *frs) {
  frd->u.min = RTE_MAX(frd->u.min, frs->u.min);
  trd->u.max = RTE_MIN(trd->u.max, trs->u.max - 1);
}

static void eval_jsgt_jsle(struct bpf_reg_val *trd, struct bpf_reg_val *trs,
                           struct bpf_reg_val *frd, struct bpf_reg_val *frs) {
  frd->s.max = RTE_MIN(frd->s.max, frs->s.min);
  trd->s.min = RTE_MAX(trd->s.min, trs->s.min + 1);
}

static void eval_jslt_jsge(struct bpf_reg_val *trd, struct bpf_reg_val *trs,
                           struct bpf_reg_val *frd, struct bpf_reg_val *frs) {
  frd->s.min = RTE_MAX(frd->s.min, frs->s.min);
  trd->s.max = RTE_MIN(trd->s.max, trs->s.max - 1);
}

static const char *eval_jcc(struct bpf_verifier *bvf,
                            const struct ebpf_insn *ins) {
  uint32_t op;
  const char *err;
 ...
 */
static const struct bpf_ins_check
    ins_chk[UINT8_MAX + 1] =
        {
 ...
 */
static const char *check_syntax(const struct ebpf_insn *ins) {
 ...
 */
static uint32_t get_node_idx(const struct bpf_verifier *bvf,
                             const struct inst_node *node) {
  return node - bvf->in;
}
 ...
 */
static struct inst_node *get_next_node(struct bpf_verifier *bvf,
...
static void set_node_colour(struct bpf_verifier *bvf, struct inst_node *node,
                            uint32_t new) {
  uint32_t prev;
 ...
 */
static int add_edge(struct bpf_verifier *bvf, struct inst_node *node,
                    uint32_t nidx) {
  uint32_t ne;

  if (nidx > bvf->prm->nb_ins) {
    RTE_BPF_LOG(ERR,
                "%s: program boundary violation at pc: %u, "
                "next pc: %u\n",
 ...
 */
static void set_edge_type(struct bpf_verifier *bvf, struct inst_node *node,
...
static struct inst_node *get_prev_node(struct bpf_verifier *bvf,
                                       struct inst_node *node) {
 ...
 */
static void dfs(struct bpf_verifier *bvf) {
 ...
 */
static void log_unreachable(const struct bpf_verifier *bvf) {
  uint32_t i;
  struct inst_node *node;
  const struct ebpf_insn *ins;
  ...
  for (i = 0; i != bvf->prm->nb_ins; i++) {
    ...
    if (node->colour == WHITE && ins->code != (BPF_LD | BPF_IMM | EBPF_DW))
 ...
 */
static void log_loop(const struct bpf_verifier *bvf) {
  uint32_t i, j;
  struct inst_node *node;
  ...
  for (i = 0; i != bvf->prm->nb_ins; i++) {
    ...
    for (j = 0; j != node->nb_edge; j++) {
      if (node->edge_type[j] == BACK_EDGE)
        RTE_BPF_LOG(ERR, "loop at pc:%u --> pc:%u;\n", i, node->edge_dest[j]);
    }
  }
}
 ...
 */
static int validate(struct bpf_verifier *bvf) {
  int32_t rc;
  uint32_t i;
  struct inst_node *node;
  const struct ebpf_insn *ins;
  ...
  RTE_BPF_LOG(DEBUG,
              ...
              __func__, bvf, bvf->nb_nodes, bvf->nb_jcc_nodes,
              bvf->node_colour[WHITE], bvf->node_colour[GREY],
              bvf->node_colour[BLACK], bvf->edge_type[UNKNOWN_EDGE],
              bvf->edge_type[TREE_EDGE], bvf->edge_type[BACK_EDGE],
              bvf->edge_type[CROSS_EDGE]);
 ...
 */
static struct bpf_eval_state *pull_eval_state(struct bpf_verifier *bvf) {
  uint32_t n;
...
static void push_eval_state(struct bpf_verifier *bvf) { bvf->evst_pool.cur--; }

static void evst_pool_fini(struct bpf_verifier *bvf) {
  bvf->evst = NULL;
  free(bvf->evst_pool.ent);
  memset(&bvf->evst_pool, 0, sizeof(bvf->evst_pool));
}

static int evst_pool_init(struct bpf_verifier *bvf) {
  uint32_t n;
 ...
 */
static int save_eval_state(struct bpf_verifier *bvf, struct inst_node *node) {
  struct bpf_eval_state *st;

  /* get new eval_state for this node */
  st = pull_eval_state(bvf);
  if (st == NULL) {
    RTE_BPF_LOG(ERR, "%s: internal error (out of space) at pc: %u\n", __func__,
                get_node_idx(bvf, node));
    return -ENOMEM;
  }
  ...
  RTE_BPF_LOG(DEBUG, "%s(bvf=%p,node=%u) old/new states: %p/%p;\n", __func__,
              bvf, get_node_idx(bvf, node), node->evst, bvf->evst);
 ...
 */
static void restore_eval_state(struct bpf_verifier *bvf,
                               struct inst_node *node) {
  RTE_BPF_LOG(DEBUG, "%s(bvf=%p,node=%u) old/new states: %p/%p;\n", __func__,
              bvf, get_node_idx(bvf, node), bvf->evst, node->evst);
...
static void log_eval_state(const struct bpf_verifier *bvf,
                           const struct ebpf_insn *ins, uint32_t pc,
                           int32_t loglvl) {
  ...
  rte_log(loglvl, rte_bpf_logtype,
          ...
          "};\n",
          ins->dst_reg, rv->v.type, rv->v.size, rv->mask, rv->u.min, rv->u.max,
          rv->s.min, rv->s.max);
}
 ...
 */
static int evaluate(struct bpf_verifier *bvf) {
  int32_t rc;
  uint32_t idx, op;
  const char *err;
  const struct ebpf_insn *ins;
  ...
  while (node != NULL && rc == 0) {
    ...
    if (next != NULL) {
      ...
      /* proceed with next child */
      if (node->cur_edge == node->nb_edge && node->evst != NULL)
...
int bpf_validate(struct rte_bpf *bpf) {
  int32_t rc;
  ...
  if (rc == 0) {
    ...
    if (bvf.nb_ldmb_nodes != 0)
      bpf->stack_sz =
          RTE_ALIGN_CEIL(bpf->stack_sz + sizeof(uint64_t), sizeof(uint64_t));
  }
