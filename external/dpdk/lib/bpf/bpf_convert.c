...
#define EBPF_ALU64_REG(OP, DST, SRC)                                           \
  ((struct ebpf_insn){.code = EBPF_ALU64 | BPF_OP(OP) | BPF_X,                 \
                      .dst_reg = DST,                                          \
                      .src_reg = SRC,                                          \
                      .off = 0,                                                \
                      .imm = 0})
...
#define BPF_ALU32_IMM(OP, DST, IMM)                                            \
  ((struct ebpf_insn){.code = BPF_ALU | BPF_OP(OP) | BPF_K,                    \
                      .dst_reg = DST,                                          \
                      .src_reg = 0,                                            \
                      .off = 0,                                                \
                      .imm = IMM})
...
#define BPF_MOV32_REG(DST, SRC)                                                \
  ((struct ebpf_insn){.code = BPF_ALU | EBPF_MOV | BPF_X,                      \
                      .dst_reg = DST,                                          \
                      .src_reg = SRC,                                          \
                      .off = 0,                                                \
                      .imm = 0})
...
#define BPF_MOV32_IMM(DST, IMM)                                                \
  ((struct ebpf_insn){.code = BPF_ALU | EBPF_MOV | BPF_K,                      \
                      .dst_reg = DST,                                          \
                      .src_reg = 0,                                            \
                      .off = 0,                                                \
                      .imm = IMM})
...
#define BPF_MOV32_RAW(TYPE, DST, SRC, IMM)                                     \
  ((struct ebpf_insn){.code = BPF_ALU | EBPF_MOV | BPF_SRC(TYPE),              \
                      .dst_reg = DST,                                          \
                      .src_reg = SRC,                                          \
                      .off = 0,                                                \
                      .imm = IMM})
...
#define BPF_LD_ABS(SIZE, IMM)                                                  \
  ((struct ebpf_insn){.code = BPF_LD | BPF_SIZE(SIZE) | BPF_ABS,               \
                      .dst_reg = 0,                                            \
                      .src_reg = 0,                                            \
                      .off = 0,                                                \
                      .imm = IMM})
...
#define BPF_LDX_MEM(SIZE, DST, SRC, OFF)                                       \
  ((struct ebpf_insn){.code = BPF_LDX | BPF_SIZE(SIZE) | BPF_MEM,              \
                      .dst_reg = DST,                                          \
                      .src_reg = SRC,                                          \
                      .off = OFF,                                              \
                      .imm = 0})
...
#define BPF_STX_MEM(SIZE, DST, SRC, OFF)                                       \
  ((struct ebpf_insn){.code = BPF_STX | BPF_SIZE(SIZE) | BPF_MEM,              \
                      .dst_reg = DST,                                          \
                      .src_reg = SRC,                                          \
                      .off = OFF,                                              \
                      .imm = 0})
...
#define BPF_JMP_IMM(OP, DST, IMM, OFF)                                         \
  ((struct ebpf_insn){.code = BPF_JMP | BPF_OP(OP) | BPF_K,                    \
                      .dst_reg = DST,                                          \
                      .src_reg = 0,                                            \
                      .off = OFF,                                              \
                      .imm = IMM})
...
#define BPF_EXIT_INSN()                                                        \
  ((struct ebpf_insn){.code = BPF_JMP | EBPF_EXIT,                             \
                      .dst_reg = 0,                                            \
                      .src_reg = 0,                                            \
                      .off = 0,                                                \
                      .imm = 0})
...
static bool convert_bpf_load(const struct bpf_insn *fp,
  ...
  case SKF_AD_OFF + SKF_AD_ALU_XOR_X:
    ...
    RTE_BPF_LOG(ERR, "rte_bpf_convert: socket offset %d not supported\n",
                fp->k - SKF_AD_OFF);
    return true;
  default:
    return false;
  ...
  do {                                                                         \
    ...
    case BPF_RET | BPF_K:
      if (BPF_RVAL(fp->code) == BPF_K) {
        *insn++ = BPF_MOV32_RAW(BPF_K, EBPF_REG_0, 0, fp->k);
      }
      *insn = BPF_EXIT_INSN();
      break;
    ...
    case BPF_LDX | BPF_MEM:
      *insn = BPF_LDX_MEM(BPF_W,
                          ...
                          BPF_REG_FP, -(BPF_MEMWORDS - fp->k) * 4);
      break;
    ...
    case BPF_LDX | BPF_IMM:
      *insn = BPF_MOV32_IMM(
          BPF_CLASS(fp->code) == BPF_LD ? BPF_REG_A : BPF_REG_X, fp->k);
      break;
    ...
    case BPF_LDX | BPF_W | BPF_LEN:
      ...
      *insn = BPF_LDX_MEM(BPF_W,
                          BPF_CLASS(fp->code) == BPF_LD ? BPF_REG_A : BPF_REG_X,
                          BPF_REG_CTX, offsetof(struct rte_mbuf, pkt_len));
      break;
...
struct rte_bpf_prm *rte_bpf_convert(const struct bpf_program *prog) {
  ...
  prm = rte_zmalloc("bpf_filter", sizeof(*prm) + ebpf_len * sizeof(*ebpf), 0);
  if (prm == NULL) {
    rte_errno = ENOMEM;
    return NULL;
  }
