...
static struct rte_bpf *bpf_load(const struct rte_bpf_prm *prm) {
  uint8_t *buf;
  struct rte_bpf *bpf;
  ...
  buf = mmap(NULL, sz, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1,
             0);
  if (buf == MAP_FAILED)
    return NULL;
 ...
 */
static int bpf_check_xsym(const struct rte_bpf_xsym *xsym) {
  uint32_t i;
  ...
  if (xsym->type == RTE_BPF_XTYPE_VAR) {
    ...
    if (xsym->func.ret.type != RTE_BPF_ARG_UNDEF && xsym->func.ret.size == 0)
      return -EINVAL;
  } else
    return -EINVAL;
