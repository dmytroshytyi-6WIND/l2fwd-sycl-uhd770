...
static const uint32_t ebpf2x86[] = {
    [EBPF_REG_0] = RAX, [EBPF_REG_1] = RDI,  [EBPF_REG_2] = RSI,
    [EBPF_REG_3] = RDX, [EBPF_REG_4] = RCX,  [EBPF_REG_5] = R8,
    [EBPF_REG_6] = RBX, [EBPF_REG_7] = R13,  [EBPF_REG_8] = R14,
    [EBPF_REG_9] = R15, [EBPF_REG_10] = RBP,
};
 ...
 */
static size_t imm_size(int32_t v) {
  if (v == (int8_t)v)
    return sizeof(int8_t);
  return sizeof(int32_t);
}

static void emit_bytes(struct bpf_jit_state *st, const uint8_t ins[],
                       uint32_t sz) {
  uint32_t i;
...
static void emit_imm(struct bpf_jit_state *st, const uint32_t imm,
                     uint32_t sz) {
  union bpf_jit_imm v;
 ...
 * emit REX byte
 */
static void emit_rex(struct bpf_jit_state *st, uint32_t op, uint32_t reg,
                     uint32_t rm) {
  uint8_t rex;
  ...
  rex = 0;
  if (BPF_CLASS(op) == EBPF_ALU64 || op == (BPF_ST | BPF_MEM | EBPF_DW) ||
      op == (BPF_STX | BPF_MEM | EBPF_DW) ||
      op == (BPF_STX | EBPF_XADD | EBPF_DW) ||
      op == (BPF_LD | BPF_IMM | EBPF_DW) ||
      (BPF_CLASS(op) == BPF_LDX && BPF_MODE(op) == BPF_MEM &&
       BPF_SIZE(op) != BPF_W))
    rex |= REX_W;
 ...
 * emit MODRegRM byte
 */
static void emit_modregrm(struct bpf_jit_state *st, uint32_t mod, uint32_t reg,
                          uint32_t rm) {
  uint8_t v;
 ...
 * emit SIB byte
 */
static void emit_sib(struct bpf_jit_state *st, uint32_t scale, uint32_t idx,
                     uint32_t base) {
  uint8_t v;
 ...
 */
static void emit_opcode(struct bpf_jit_state *st, uint8_t ops, uint32_t reg) {
  uint8_t v;
 ...
 * emit xchg %<sreg>, %<dreg>
 */
static void emit_xchg_reg(struct bpf_jit_state *st, uint32_t sreg,
                          uint32_t dreg) {
 ...
 * emit neg %<dreg>
 */
static void emit_neg(struct bpf_jit_state *st, uint32_t op, uint32_t dreg) {
 ...
 * emit mov %<sreg>, %<dreg>
 */
static void emit_mov_reg(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                         uint32_t dreg) {
 ...
 */
static void emit_movzwl(struct bpf_jit_state *st, uint32_t sreg,
                        uint32_t dreg) {
 ...
 * emit ror <imm8>, %<dreg>
 */
static void emit_ror_imm(struct bpf_jit_state *st, uint32_t dreg,
                         uint32_t imm) {
 ...
 * emit bswap %<dreg>
 */
static void emit_be2le_48(struct bpf_jit_state *st, uint32_t dreg,
                          uint32_t imm) {
  uint32_t rop;
...
static void emit_be2le(struct bpf_jit_state *st, uint32_t dreg, uint32_t imm) {
  if (imm == 16) {
 ...
 */
static void emit_le2be(struct bpf_jit_state *st, uint32_t dreg, uint32_t imm) {
  if (imm == 16)
    emit_movzwl(st, dreg, dreg);
  else if (imm == 32)
 ...
 *   xor <imm>, %<dreg>
 */
static void emit_alu_imm(struct bpf_jit_state *st, uint32_t op, uint32_t dreg,
                         uint32_t imm) {
  uint8_t mod, opcode;
  uint32_t bop, imsz;
  ...
  static const uint8_t mods[] = {
      [GET_BPF_OP(BPF_ADD)] = 0, [GET_BPF_OP(BPF_AND)] = 4,
      [GET_BPF_OP(BPF_OR)] = 1,  [GET_BPF_OP(BPF_SUB)] = 5,
 ...
 *   xor %<sreg>, %<dreg>
 */
static void emit_alu_reg(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                         uint32_t dreg) {
  uint32_t bop;

  static const uint8_t ops[] = {
      [GET_BPF_OP(BPF_ADD)] = 0x01, [GET_BPF_OP(BPF_AND)] = 0x21,
      [GET_BPF_OP(BPF_OR)] = 0x09,  [GET_BPF_OP(BPF_SUB)] = 0x29,
...
static void emit_shift(struct bpf_jit_state *st, uint32_t op, uint32_t dreg) {
  uint8_t mod;
  uint32_t bop, opx;
 ...
 *   sar <imm>, %<dreg>
 */
static void emit_shift_imm(struct bpf_jit_state *st, uint32_t op, uint32_t dreg,
                           uint32_t imm) {
 ...
 */
static void emit_shift_reg(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                           uint32_t dreg) {
  if (sreg != RCX)
 ...
 * emit mov <imm>, %<dreg>
 */
static void emit_mov_imm(struct bpf_jit_state *st, uint32_t op, uint32_t dreg,
                         uint32_t imm) {
 ...
 * emit mov <imm64>, %<dreg>
 */
static void emit_ld_imm64(struct bpf_jit_state *st, uint32_t dreg,
 ...
 * mov %r11, %rax
 */
static void emit_mul(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
 ...
 */
static void emit_ld_reg(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
 ...
 *   mov <imm>, <ofs>(%<dreg>)
 */
static void emit_st_common(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
...
static void emit_st_imm(struct bpf_jit_state *st, uint32_t op, uint32_t dreg,
                        uint32_t imm, int32_t ofs) {
  emit_st_common(st, op, 0, dreg, imm, ofs);
}

static void emit_st_reg(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                        uint32_t dreg, int32_t ofs) {
 ...
 */
static void emit_st_xadd(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                         uint32_t dreg, int32_t ofs) {
  uint32_t imsz, mods;
 ...
 *    call *%rax
 */
static void emit_call(struct bpf_jit_state *st, uintptr_t trg) {
 ...
 */
static void emit_abs_jmp(struct bpf_jit_state *st, int32_t ofs) {
  int32_t joff;
  uint32_t imsz;
 ...
 */
static void emit_jmp(struct bpf_jit_state *st, int32_t ofs) {
 ...
 *    cmovle %<sreg>, <%dreg>
 */
static void emit_movcc_reg(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                           uint32_t dreg) {
  uint32_t bop;
 ...
 * where 'ofs' is the target offset for the native code.
 */
static void emit_abs_jcc(struct bpf_jit_state *st, uint32_t op, int32_t ofs) {
  uint32_t bop, imsz;
  int32_t joff;
 ...
 * where 'ofs' is the target offset for the BPF bytecode.
 */
static void emit_jcc(struct bpf_jit_state *st, uint32_t op, int32_t ofs) {
 ...
 * emit cmp <imm>, %<dreg>
 */
static void emit_cmp_imm(struct bpf_jit_state *st, uint32_t op, uint32_t dreg,
                         uint32_t imm) {
  uint8_t ops;
  uint32_t imsz;
 ...
 * emit test <imm>, %<dreg>
 */
static void emit_tst_imm(struct bpf_jit_state *st, uint32_t op, uint32_t dreg,
                         uint32_t imm) {
  const uint8_t ops = 0xF7;
...
static void emit_jcc_imm(struct bpf_jit_state *st, uint32_t op, uint32_t dreg,
                         uint32_t imm, int32_t ofs) {
 ...
 * emit test %<sreg>, %<dreg>
 */
static void emit_tst_reg(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                         uint32_t dreg) {
 ...
 * emit cmp %<sreg>, %<dreg>
 */
static void emit_cmp_reg(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                         uint32_t dreg) {
...
static void emit_jcc_reg(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                         uint32_t dreg, int32_t ofs) {
  if (BPF_OP(op) == BPF_JSET)
 ...
 * mov %r10, %rdx
 */
static void emit_div(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                     uint32_t dreg, uint32_t imm) {
  uint32_t sr;
 ...
 */
static void emit_ldmb_fast_path(struct bpf_jit_state *st,
                                const uint32_t rg[EBPF_REG_7], uint32_t sreg,
                                uint32_t mode, uint32_t sz, uint32_t imm,
                                const int32_t ofs[LDMB_OFS_NUM]) {
  /* make R2 contain *off* value */

  if (sreg != rg[EBPF_REG_2]) {
    emit_mov_imm(st, EBPF_ALU64 | EBPF_MOV | BPF_K, rg[EBPF_REG_2], imm);
    if (mode == BPF_IND)
      emit_alu_reg(st, EBPF_ALU64 | BPF_ADD | BPF_X, sreg, rg[EBPF_REG_2]);
  } else
    /* BPF_IND with sreg == R2 */
    emit_alu_imm(st, EBPF_ALU64 | BPF_ADD | BPF_K, rg[EBPF_REG_2], imm);

  /* R3 = mbuf->data_len */
  emit_ld_reg(st, BPF_LDX | BPF_MEM | BPF_H, rg[EBPF_REG_6], rg[EBPF_REG_3],
              offsetof(struct rte_mbuf, data_len));

  /* R3 = R3 - R2 */
  emit_alu_reg(st, EBPF_ALU64 | BPF_SUB | BPF_X, rg[EBPF_REG_2],
               rg[EBPF_REG_3]);
  ...
  /* R0 = mbuf->buf_addr */
  emit_ld_reg(st, BPF_LDX | BPF_MEM | EBPF_DW, rg[EBPF_REG_6], rg[EBPF_REG_0],
              offsetof(struct rte_mbuf, buf_addr));

  /* R0 = R0 + R3 */
  emit_alu_reg(st, EBPF_ALU64 | BPF_ADD | BPF_X, rg[EBPF_REG_3],
               rg[EBPF_REG_0]);

  /* R0 = R0 + R2 */
  emit_alu_reg(st, EBPF_ALU64 | BPF_ADD | BPF_X, rg[EBPF_REG_2],
               rg[EBPF_REG_0]);
 ...
 */
static void emit_ldmb_slow_path(struct bpf_jit_state *st,
                                const uint32_t rg[EBPF_REG_7], uint32_t sz) {
  ...
  emit_mov_reg(st, EBPF_ALU64 | EBPF_MOV | BPF_X, rg[EBPF_REG_6],
               rg[EBPF_REG_1]);
 ...
 */
static void emit_ldmb_fin(struct bpf_jit_state *st, uint32_t dreg,
                          uint32_t opsz, uint32_t sz) {
 ...
 *   res = bswap(res);
 */
static void emit_ld_mbuf(struct bpf_jit_state *st, uint32_t op, uint32_t sreg,
                         uint32_t imm) {
...
static void emit_prolog(struct bpf_jit_state *st, int32_t stack_size) {
  uint32_t i;
  int32_t spil, ofs;
  ...
  emit_alu_imm(st, EBPF_ALU64 | BPF_SUB | BPF_K, RSP, spil * sizeof(uint64_t));

  ofs = 0;
  for (i = 0; i != RTE_DIM(save_regs); i++) {
    if (INUSE(st->reguse, save_regs[i]) != 0) {
      emit_st_reg(st, BPF_STX | BPF_MEM | EBPF_DW, save_regs[i], RSP, ofs);
      ofs += sizeof(uint64_t);
    }
  }
 ...
 * emit ret
 */
static void emit_ret(struct bpf_jit_state *st) {
  const uint8_t ops = 0xC3;

  emit_bytes(st, &ops, sizeof(ops));
}

static void emit_epilog(struct bpf_jit_state *st) {
  uint32_t i;
  int32_t spil, ofs;
  ...
  if (spil != 0) {
    ...
    if (INUSE(st->reguse, RBP) != 0)
      emit_mov_reg(st, EBPF_ALU64 | EBPF_MOV | BPF_X, RBP, RSP);

    ofs = 0;
    for (i = 0; i != RTE_DIM(save_regs); i++) {
      if (INUSE(st->reguse, save_regs[i]) != 0) {
        emit_ld_reg(st, BPF_LDX | BPF_MEM | EBPF_DW, RSP, save_regs[i], ofs);
        ofs += sizeof(uint64_t);
      }
    }
 ...
 */
static int emit(struct bpf_jit_state *st, const struct rte_bpf *bpf) {
  uint32_t i, dr, op, sr;
  ...
  for (i = 0; i != bpf->prm.nb_ins; i++) {
    ...
    case (BPF_JMP | EBPF_CALL):
      emit_call(st, (uintptr_t)bpf->prm.xsym[ins->imm].func.val);
      break;
    /* return instruction */
 ...
 */
int bpf_jit_x86(struct rte_bpf *bpf) {
  int32_t rc;
  uint32_t i;
  size_t sz;
