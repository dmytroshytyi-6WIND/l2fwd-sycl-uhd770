 ...
 */
static __rte_always_inline void bpf_eth_cbi_inuse(struct bpf_eth_cbi *cbi) {
  cbi->use++;
 ...
 */
static __rte_always_inline void bpf_eth_cbi_unuse(struct bpf_eth_cbi *cbi) {
...
static void bpf_eth_cbi_cleanup(struct bpf_eth_cbi *bc) {
  bc->bpf = NULL;
  memset(&bc->jit, 0, sizeof(bc->jit));
}

static struct bpf_eth_cbi *bpf_eth_cbh_find(struct bpf_eth_cbh *cbh,
...
static struct bpf_eth_cbi *bpf_eth_cbh_add(struct bpf_eth_cbh *cbh,
                                           uint16_t port, uint16_t queue) {
  struct bpf_eth_cbi *cbi;
...
static inline uint32_t apply_filter(struct rte_mbuf *mb[], const uint64_t rc[],
...
static inline uint32_t pkt_filter_vm(const struct rte_bpf *bpf,
...
static inline uint32_t pkt_filter_jit(const struct rte_bpf_jit *jit,
                                      struct rte_mbuf *mb[], uint32_t num,
                                      uint32_t drop) {
  uint32_t i, n;
  void *dp;
...
static inline uint32_t pkt_filter_mb_vm(const struct rte_bpf *bpf,
                                        struct rte_mbuf *mb[], uint32_t num,
                                        uint32_t drop) {
  uint64_t rc[num];
...
static inline uint32_t pkt_filter_mb_jit(const struct rte_bpf_jit *jit,
                                         struct rte_mbuf *mb[], uint32_t num,
                                         uint32_t drop) {
  uint32_t i, n;
  uint64_t rc[num];
...
static uint16_t bpf_rx_callback_vm(__rte_unused uint16_t port,
  ...
  rc = (cbi->cb != NULL) ? pkt_filter_vm(cbi->bpf, pkt, nb_pkts, 1) : nb_pkts;
  bpf_eth_cbi_unuse(cbi);
  return rc;
}

static uint16_t bpf_rx_callback_jit(__rte_unused uint16_t port,
                                    __rte_unused uint16_t queue,
                                    struct rte_mbuf *pkt[], uint16_t nb_pkts,
                                    __rte_unused uint16_t max_pkts,
                                    void *user_param) {
  ...
  cbi = user_param;
  bpf_eth_cbi_inuse(cbi);
  rc = (cbi->cb != NULL) ? pkt_filter_jit(&cbi->jit, pkt, nb_pkts, 1) : nb_pkts;
  bpf_eth_cbi_unuse(cbi);
  return rc;
}

static uint16_t bpf_tx_callback_vm(__rte_unused uint16_t port,
                                   __rte_unused uint16_t queue,
                                   struct rte_mbuf *pkt[], uint16_t nb_pkts,
                                   void *user_param) {
  struct bpf_eth_cbi *cbi;
  ...
  cbi = user_param;
  bpf_eth_cbi_inuse(cbi);
  rc = (cbi->cb != NULL) ? pkt_filter_vm(cbi->bpf, pkt, nb_pkts, 0) : nb_pkts;
  bpf_eth_cbi_unuse(cbi);
  return rc;
}

static uint16_t bpf_tx_callback_jit(__rte_unused uint16_t port,
                                    __rte_unused uint16_t queue,
                                    struct rte_mbuf *pkt[], uint16_t nb_pkts,
                                    void *user_param) {
  struct bpf_eth_cbi *cbi;
  ...
  cbi = user_param;
  bpf_eth_cbi_inuse(cbi);
  rc = (cbi->cb != NULL) ? pkt_filter_jit(&cbi->jit, pkt, nb_pkts, 0) : nb_pkts;
  bpf_eth_cbi_unuse(cbi);
  return rc;
}
...
static uint16_t bpf_rx_callback_mb_vm(__rte_unused uint16_t port,
                                      __rte_unused uint16_t queue,
                                      struct rte_mbuf *pkt[], uint16_t nb_pkts,
                                      __rte_unused uint16_t max_pkts,
                                      void *user_param) {
  ...
  bpf_eth_cbi_inuse(cbi);
  rc =
      (cbi->cb != NULL) ? pkt_filter_mb_vm(cbi->bpf, pkt, nb_pkts, 1) : nb_pkts;
  bpf_eth_cbi_unuse(cbi);
  return rc;
}

static uint16_t bpf_rx_callback_mb_jit(__rte_unused uint16_t port,
                                       __rte_unused uint16_t queue,
                                       struct rte_mbuf *pkt[], uint16_t nb_pkts,
                                       __rte_unused uint16_t max_pkts,
                                       void *user_param) {
  ...
  cbi = user_param;
  bpf_eth_cbi_inuse(cbi);
  rc = (cbi->cb != NULL) ? pkt_filter_mb_jit(&cbi->jit, pkt, nb_pkts, 1)
                         : nb_pkts;
  bpf_eth_cbi_unuse(cbi);
  return rc;
}

static uint16_t bpf_tx_callback_mb_vm(__rte_unused uint16_t port,
                                      __rte_unused uint16_t queue,
                                      struct rte_mbuf *pkt[], uint16_t nb_pkts,
                                      void *user_param) {
  struct bpf_eth_cbi *cbi;
  ...
  bpf_eth_cbi_inuse(cbi);
  rc =
      (cbi->cb != NULL) ? pkt_filter_mb_vm(cbi->bpf, pkt, nb_pkts, 0) : nb_pkts;
  bpf_eth_cbi_unuse(cbi);
  return rc;
}

static uint16_t bpf_tx_callback_mb_jit(__rte_unused uint16_t port,
                                       __rte_unused uint16_t queue,
                                       struct rte_mbuf *pkt[], uint16_t nb_pkts,
                                       void *user_param) {
  struct bpf_eth_cbi *cbi;
  ...
  cbi = user_param;
  bpf_eth_cbi_inuse(cbi);
  rc = (cbi->cb != NULL) ? pkt_filter_mb_jit(&cbi->jit, pkt, nb_pkts, 0)
                         : nb_pkts;
  bpf_eth_cbi_unuse(cbi);
  return rc;
}

static rte_rx_callback_fn select_rx_callback(enum rte_bpf_arg_type type,
                                             uint32_t flags) {
...
static rte_tx_callback_fn select_tx_callback(enum rte_bpf_arg_type type,
                                             uint32_t flags) {
  if (flags & RTE_BPF_ETH_F_JIT) {
    if (type == RTE_BPF_ARG_PTR)
 ...
 */
static void bpf_eth_cbi_unload(struct bpf_eth_cbi *bc) {
...
static void bpf_eth_unload(struct bpf_eth_cbh *cbh, uint16_t port,
                           uint16_t queue) {
...
static int bpf_eth_elf_load(struct bpf_eth_cbh *cbh, uint16_t port,
                            uint16_t queue, const struct rte_bpf_prm *prm,
...
int rte_bpf_eth_rx_elf_load(uint16_t port, uint16_t queue,
                            const struct rte_bpf_prm *prm, const char *fname,
