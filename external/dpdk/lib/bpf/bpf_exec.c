...
#define BPF_JMP_CND_REG(reg, ins, op, type)                                    \
  ((ins) += ((type)(reg)[(ins)->dst_reg] op(type)(reg)[(ins)->src_reg])        \
                ? (ins)->off                                                   \
                : 0)

#define BPF_JMP_CND_IMM(reg, ins, op, type)                                    \
  ((ins) += ((type)(reg)[(ins)->dst_reg] op(type)(ins)->imm) ? (ins)->off : 0)
...
#define BPF_OP_ALU_IMM(reg, ins, op, type)                                     \
  ((reg)[(ins)->dst_reg] = (type)(reg)[(ins)->dst_reg] op(type)(ins)->imm)

#define BPF_DIV_ZERO_CHECK(bpf, reg, ins, type)                                \
  do {                                                                         \
    if ((type)(reg)[(ins)->src_reg] == 0) {                                    \
      RTE_BPF_LOG(ERR, "%s(%p): division by 0 at pc: %#zx;\n", __func__, bpf,  \
...
#define BPF_ST_IMM(reg, ins, type)                                             \
  (*(type *)(uintptr_t)((reg)[(ins)->dst_reg] + (ins)->off) = (type)(ins)->imm)
...
#define BPF_ST_XADD_REG(reg, ins, tp)                                          \
  (rte_atomic##tp##_add(                                                       \
      (rte_atomic##tp##_t *)(uintptr_t)((reg)[(ins)->dst_reg] + (ins)->off),   \
      reg[ins->src_reg]))
...
#define BPF_LD_ABS(bpf, reg, ins, type, op)                                    \
  do {                                                                         \
...
#define BPF_LD_IND(bpf, reg, ins, type, op)                                    \
  do {                                                                         \
...
static inline void bpf_alu_be(uint64_t reg[EBPF_REG_NUM],
...
static inline void bpf_alu_le(uint64_t reg[EBPF_REG_NUM],
                              const struct ebpf_insn *ins) {
  uint64_t *v;
...
static inline const void *bpf_ld_mbuf(const struct rte_bpf *bpf,
                                      uint64_t reg[EBPF_REG_NUM],
                                      const struct ebpf_insn *ins, uint32_t off,
                                      uint32_t len) {
...
static inline uint64_t bpf_exec(const struct rte_bpf *bpf,
                                uint64_t reg[EBPF_REG_NUM]) {
  const struct ebpf_insn *ins;
  ...
  for (ins = bpf->prm.ins;; ins++) {
    ...
    case (BPF_LD | BPF_IMM | EBPF_DW):
      reg[ins->dst_reg] = (uint32_t)ins[0].imm | (uint64_t)(uint32_t)ins[1].imm
                                                     << 32;
      ins++;
      break;
    ...
    case (BPF_JMP | EBPF_CALL):
      reg[EBPF_REG_0] = bpf->prm.xsym[ins->imm].func.val(
          reg[EBPF_REG_1], reg[EBPF_REG_2], reg[EBPF_REG_3], reg[EBPF_REG_4],
          reg[EBPF_REG_5]);
      break;
    /* return instruction */
    ...
    default:
      RTE_BPF_LOG(ERR, "%s(%p): invalid opcode %#x at pc: %#zx;\n", __func__,
                  bpf, ins->code, (uintptr_t)ins - (uintptr_t)bpf->prm.ins);
      return 0;
    }
  }
