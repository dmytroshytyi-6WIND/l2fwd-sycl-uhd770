...
static uint32_t bpf_find_xsym(const char *sn, enum rte_bpf_xtype type,
 ...
 * symbol *sn*
 */
static int resolve_xsym(const char *sn, size_t ofs, struct ebpf_insn *ins,
                        size_t ins_sz, const struct rte_bpf_prm *prm) {
  uint32_t idx, fidx;
  ...
  if (type == RTE_BPF_XTYPE_FUNC) {
    ...
    ins[idx + 1].imm = (uint64_t)(uintptr_t)prm->xsym[fidx].var.val >> 32;
  }

  return 0;
}

static int check_elf_header(const Elf64_Ehdr *eh) {
  const char *err;
 ...
 */
static int find_elf_code(Elf *elf, const char *section, Elf_Data **psd,
                         size_t *pidx) {
  Elf_Scn *sc;
  const Elf64_Ehdr *eh;
  const Elf64_Shdr *sh;
  ...
  for (sc = elf_nextscn(elf, NULL); sc != NULL; sc = elf_nextscn(elf, sc)) {
    sh = elf64_getshdr(sc);
    sn = elf_strptr(elf, eh->e_shstrndx, sh->sh_name);
    if (sn != NULL && strcmp(section, sn) == 0 && sh->sh_type == SHT_PROGBITS &&
 ...
 */
static int process_reloc(Elf *elf, size_t sym_idx, Elf64_Rel *re, size_t re_sz,
                         struct ebpf_insn *ins, size_t ins_sz,
  ...
  for (i = 0; i != n; i++) {
    ...
    if (rc != 0) {
      RTE_BPF_LOG(ERR, "resolve_xsym(%s, %zu) error code: %d\n", sn, ofs, rc);
      return rc;
    }
  }
 ...
 * and update bpf code.
 */
static int elf_reloc_code(Elf *elf, Elf_Data *ed, size_t sidx,
                          const struct rte_bpf_prm *prm) {
  Elf64_Rel *re;
  Elf_Scn *sc;
  const Elf64_Shdr *sh;
  ...
  for (sc = elf_nextscn(elf, NULL); sc != NULL && rc == 0;
    ...
    if (sh->sh_type == SHT_REL && sh->sh_info == sidx) {
      sd = elf_getdata(sc, NULL);
      if (sd == NULL || sd->d_size == 0 || sd->d_size % sizeof(re[0]) != 0)
        return -EINVAL;
      rc = process_reloc(elf, sh->sh_link, sd->d_buf, sd->d_size, ed->d_buf,
                         ed->d_size, prm);
    }
  }
...
static struct rte_bpf *bpf_load_elf(const struct rte_bpf_prm *prm, int32_t fd,
                                    const char *section) {
  Elf *elf;
  Elf_Data *sd;
  size_t sidx;
