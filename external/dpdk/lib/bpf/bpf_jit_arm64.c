...
static int check_immr_imms(bool is64, uint8_t immr, uint8_t imms) {
...
static int check_mov_hw(bool is64, const uint8_t val) {
  if (val == 16 || val == 0)
    return 0;
...
static int check_ls_sz(uint8_t sz) {
...
static int check_reg(uint8_t r) { return (r > 31) ? 1 : 0; }

static int is_first_pass(struct a64_jit_ctx *ctx) { return (ctx->ins == NULL); }

static int check_invalid_args(struct a64_jit_ctx *ctx, uint32_t limit) {
  uint32_t idx;
  ...
  for (idx = 0; idx < limit; idx++) {
    if (rte_le_to_cpu_32(ctx->ins[idx]) == A64_INVALID_OP_CODE) {
      RTE_BPF_LOG(ERR, "%s: invalid opcode at %u;\n", __func__, idx);
      return -EINVAL;
    }
  }
  return 0;
...
static int jump_offset_init(struct a64_jit_ctx *ctx, struct rte_bpf *bpf) {
  uint32_t i;
...
static void jump_offset_fini(struct a64_jit_ctx *ctx) { free(ctx->map); }

static void jump_offset_update(struct a64_jit_ctx *ctx, uint32_t ebpf_idx) {
  if (is_first_pass(ctx))
    ctx->map[ebpf_idx].off = ctx->idx;
}

static void jump_offset_to_branch_update(struct a64_jit_ctx *ctx,
                                         uint32_t ebpf_idx) {
  if (is_first_pass(ctx))
    ctx->map[ebpf_idx].off_to_b = ctx->idx - ctx->map[ebpf_idx].off;
}

static int32_t jump_offset_get(struct a64_jit_ctx *ctx, uint32_t from,
                               int16_t offset) {
...
static int check_cond(uint8_t cond) { return (cond >= A64_AL) ? 1 : 0; }

static uint8_t ebpf_to_a64_cond(uint8_t op) {
  switch (BPF_OP(op)) {
  case BPF_JEQ:
    return A64_EQ;
  case BPF_JGT:
...
/* Emit an instruction */
static inline void emit_insn(struct a64_jit_ctx *ctx, uint32_t insn,
                             int error) {
  if (error)
...
static void emit_ret(struct a64_jit_ctx *ctx) { emit_insn(ctx, 0xd65f03c0, 0); }

static void emit_add_sub_imm(struct a64_jit_ctx *ctx, bool is64, bool sub,
                             uint8_t rd, uint8_t rn, int16_t imm12) {
  uint32_t insn, imm;
  ...
  emit_insn(ctx, insn, check_reg(rd) || check_reg(rn) || check_imm(12, imm12));
}

static void emit_add_imm_64(struct a64_jit_ctx *ctx, uint8_t rd, uint8_t rn,
                            uint16_t imm12) {
  emit_add_sub_imm(ctx, 1, 0, rd, rn, imm12);
}

static void emit_sub_imm_64(struct a64_jit_ctx *ctx, uint8_t rd, uint8_t rn,
                            uint16_t imm12) {
  emit_add_sub_imm(ctx, 1, 1, rd, rn, imm12);
}

static void emit_mov(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t rn) {
  emit_add_sub_imm(ctx, is64, 0, rd, rn, 0);
}

static void emit_mov_64(struct a64_jit_ctx *ctx, uint8_t rd, uint8_t rn) {
  emit_mov(ctx, 1, rd, rn);
}

static void emit_ls_pair_64(struct a64_jit_ctx *ctx, uint8_t rt, uint8_t rt2,
                            uint8_t rn, bool push, bool load, bool pre_index) {
  uint32_t insn;
...
static void emit_stack_push(struct a64_jit_ctx *ctx, uint8_t rt, uint8_t rt2) {
  emit_ls_pair_64(ctx, rt, rt2, A64_SP, 1, 0, 1);
}

/* Emit ldp rt, rt2, [sp, #16] */
static void emit_stack_pop(struct a64_jit_ctx *ctx, uint8_t rt, uint8_t rt2) {
...
#define A64_MOVZ 2
#define A64_MOVK 3
static void mov_imm(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                    uint8_t type, uint16_t imm16, uint8_t shift) {
  uint32_t insn;
...
static void emit_mov_imm32(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                           uint32_t val) {
...
static int u16_blocks_weight(const uint64_t val, bool one) {
...
static void emit_mov_imm(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                         uint64_t val) {
  uint64_t nval = ~val;
  int movn, sr;
...
static void emit_ls(struct a64_jit_ctx *ctx, uint8_t sz, uint8_t rt, uint8_t rn,
                    uint8_t rm, bool load) {
  uint32_t insn;
  ...
  emit_insn(ctx, insn,
            check_reg(rt) || check_reg(rn) || check_reg(rm) || check_ls_sz(sz));
}

static void emit_str(struct a64_jit_ctx *ctx, uint8_t sz, uint8_t rt,
                     uint8_t rn, uint8_t rm) {
  emit_ls(ctx, sz, rt, rn, rm, 0);
}

static void emit_ldr(struct a64_jit_ctx *ctx, uint8_t sz, uint8_t rt,
                     uint8_t rn, uint8_t rm) {
...
static void emit_add_sub(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
...
static void emit_add(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t rm) {
  emit_add_sub(ctx, is64, rd, rd, rm, A64_ADD);
}

static void emit_sub(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t rm) {
  emit_add_sub(ctx, is64, rd, rd, rm, A64_SUB);
}

static void emit_neg(struct a64_jit_ctx *ctx, bool is64, uint8_t rd) {
  emit_add_sub(ctx, is64, rd, A64_ZR, rd, A64_SUB);
}

static void emit_mul(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t rm) {
  uint32_t insn;
...
#define A64_LSRV 0x9
#define A64_ASRV 0xA
static void emit_data_process_two_src(struct a64_jit_ctx *ctx, bool is64,
...
static void emit_div(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t rm) {
  emit_data_process_two_src(ctx, is64, rd, rd, rm, A64_UDIV);
}

static void emit_lslv(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                      uint8_t rm) {
  emit_data_process_two_src(ctx, is64, rd, rd, rm, A64_LSLV);
}

static void emit_lsrv(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                      uint8_t rm) {
  emit_data_process_two_src(ctx, is64, rd, rd, rm, A64_LSRV);
}

static void emit_asrv(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                      uint8_t rm) {
...
#define A64_UBFM 0x2
#define A64_SBFM 0x0
static void emit_bitfield(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                          uint8_t rn, uint8_t immr, uint8_t imms, uint16_t op)
...
static void emit_lsl(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t imm) {
  const unsigned int width = is64 ? 64 : 32;
  uint8_t imms, immr;
...
static void emit_lsr(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t imm) {
  emit_bitfield(ctx, is64, rd, rd, imm, is64 ? 63 : 31, A64_UBFM);
}

static void emit_asr(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t imm) {
...
#define A64_OR 1
#define A64_XOR 2
static void emit_logical(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
...
static void emit_or(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                    uint8_t rm) {
  emit_logical(ctx, is64, rd, rm, A64_OR);
}

static void emit_and(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t rm) {
  emit_logical(ctx, is64, rd, rm, A64_AND);
}

static void emit_xor(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
                     uint8_t rm) {
  emit_logical(ctx, is64, rd, rm, A64_XOR);
}

static void emit_msub(struct a64_jit_ctx *ctx, bool is64, uint8_t rd,
  ...
  emit_insn(ctx, insn,
            check_reg(rd) || check_reg(rn) || check_reg(rm) || check_reg(ra));
}

static void emit_mod(struct a64_jit_ctx *ctx, bool is64, uint8_t tmp,
                     uint8_t rd, uint8_t rm) {
  emit_data_process_two_src(ctx, is64, tmp, rd, rm, A64_UDIV);
  emit_msub(ctx, is64, rd, tmp, rm, rd);
}

static void emit_blr(struct a64_jit_ctx *ctx, uint8_t rn) {
  uint32_t insn;
...
static void emit_zero_extend(struct a64_jit_ctx *ctx, uint8_t rd, int32_t imm) {
  switch (imm) {
  case 16:
...
static void emit_rev(struct a64_jit_ctx *ctx, uint8_t rd, int32_t imm) {
  uint32_t insn;
...
static void emit_be(struct a64_jit_ctx *ctx, uint8_t rd, int32_t imm) {
  if (is_be())
...
static void emit_le(struct a64_jit_ctx *ctx, uint8_t rd, int32_t imm) {
  if (is_be())
    emit_rev(ctx, rd, imm);
  else
    emit_zero_extend(ctx, rd, imm);
...
static uint8_t ebpf_to_a64_reg(struct a64_jit_ctx *ctx, uint8_t reg) {
 ...
 * R31     - Stack pointer
 */
static void emit_prologue_has_call(struct a64_jit_ctx *ctx) {
...
static void emit_epilogue_has_call(struct a64_jit_ctx *ctx) {
...
static void emit_prologue_no_call(struct a64_jit_ctx *ctx) {
  /*
...
static void emit_epilogue_no_call(struct a64_jit_ctx *ctx) {
  if (ctx->stack_sz)
    emit_add_imm_64(ctx, A64_SP, A64_SP, ctx->stack_sz);
...
static void emit_prologue(struct a64_jit_ctx *ctx) {
  if (ctx->foundcall)
...
static void emit_epilogue(struct a64_jit_ctx *ctx) {
...
static void emit_call(struct a64_jit_ctx *ctx, uint8_t tmp, void *func) {
...
static void emit_cbnz(struct a64_jit_ctx *ctx, bool is64, uint8_t rt,
                      int32_t imm19) {
  uint32_t insn, imm;
...
static void emit_b(struct a64_jit_ctx *ctx, int32_t imm26) {
  uint32_t insn, imm;
...
static void emit_return_zero_if_src_zero(struct a64_jit_ctx *ctx, bool is64,
                                         uint8_t src) {
  uint8_t r0 = ebpf_to_a64_reg(ctx, EBPF_REG_0);
...
static void emit_stadd(struct a64_jit_ctx *ctx, bool is64, uint8_t rs,
                       uint8_t rn) {
  uint32_t insn;
...
static void emit_ldxr(struct a64_jit_ctx *ctx, bool is64, uint8_t rt,
                      uint8_t rn) {
  uint32_t insn;
...
static void emit_stxr(struct a64_jit_ctx *ctx, bool is64, uint8_t rs,
...
static void emit_xadd(struct a64_jit_ctx *ctx, uint8_t op, uint8_t tmp1,
                      uint8_t tmp2, uint8_t tmp3, uint8_t dst, int16_t off,
                      uint8_t src) {
...
static void emit_cmp_tst(struct a64_jit_ctx *ctx, bool is64, uint8_t rn,
                         uint8_t rm, uint32_t opc) {
  uint32_t insn;
...
static void emit_cmp(struct a64_jit_ctx *ctx, bool is64, uint8_t rn,
                     uint8_t rm) {
  emit_cmp_tst(ctx, is64, rn, rm, A64_CMP);
}

static void emit_tst(struct a64_jit_ctx *ctx, bool is64, uint8_t rn,
                     uint8_t rm) {
  emit_cmp_tst(ctx, is64, rn, rm, A64_TST);
}

static void emit_b_cond(struct a64_jit_ctx *ctx, uint8_t cond, int32_t imm19) {
  uint32_t insn, imm;
...
static void emit_branch(struct a64_jit_ctx *ctx, uint8_t op, uint32_t i,
                        int16_t off) {
  jump_offset_to_branch_update(ctx, i);
  emit_b_cond(ctx, ebpf_to_a64_cond(op), jump_offset_get(ctx, i, off));
}

static void check_program_has_call(struct a64_jit_ctx *ctx,
                                   struct rte_bpf *bpf) {
 ...
 */
static int emit(struct a64_jit_ctx *ctx, struct rte_bpf *bpf) {
  ...
  for (i = 0; i != bpf->prm.nb_ins; i++) {
    ...
    case (BPF_JMP | EBPF_EXIT):
      ...
      break;
    default:
      RTE_BPF_LOG(ERR, "%s(%p): invalid opcode %#x at pc: %u;\n", __func__, bpf,
                  ins->code, i);
      return -EINVAL;
    }
  }
 ...
 */
int bpf_jit_arm64(struct rte_bpf *bpf) {
