...
static inline const char *get_u8_pciaddr_field(const char *in, void *_u8,
                                               char dlm) {
  unsigned long val;
  uint8_t *u8 = _u8;
  char *end;
...
static int pci_bdf_parse(const char *input, struct rte_pci_addr *dev_addr) {
  const char *in = input;
...
static int pci_dbdf_parse(const char *input, struct rte_pci_addr *dev_addr) {
  const char *in = input;
  unsigned long val;
  char *end;
...
int rte_pci_addr_cmp(const struct rte_pci_addr *addr,
  ...
  dev_addr = ((uint64_t)addr->domain << 24) | (addr->bus << 16) |
             (addr->devid << 8) | addr->function;
  dev_addr2 = ((uint64_t)addr2->domain << 24) | (addr2->bus << 16) |
...
int rte_pci_addr_parse(const char *str, struct rte_pci_addr *addr) {
  if (pci_bdf_parse(str, addr) == 0 || pci_dbdf_parse(str, addr) == 0)
    return 0;
  return -1;
}
