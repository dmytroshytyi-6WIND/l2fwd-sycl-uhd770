...
static int acl_merge_trie(struct acl_build_context *context,
                          ...
                          struct rte_acl_node *node_b, uint32_t level,
                          struct rte_acl_node **node_c);

static void acl_deref_ptr(struct acl_build_context *context,
                          struct rte_acl_node *node, int index);

static void *acl_build_alloc(struct acl_build_context *context, size_t n,
                             size_t s) {
  uint32_t m;
  void *p;
  ...
  for (m = 0; m < RTE_DIM(context->blocks); m++) {
    if (context->blocks[m].block_size == alloc_size &&
        context->blocks[m].mem_ptr != NULL) {
 ...
 */
static void acl_build_free(struct acl_build_context *context, size_t s,
                           void *p) {
  uint32_t n;
 ...
 */
static struct rte_acl_node *acl_alloc_node(struct acl_build_context *context,
                                           int level) {
 ...
 */
static void acl_free_node(struct acl_build_context *context,
                          struct rte_acl_node *node) {
  uint32_t n;
  ...
  if (node->mrt != NULL) {
    acl_build_free(context, sizeof(struct rte_acl_match_results), node->mrt);
    node->mrt = NULL;
  }

  /* free transitions to other nodes */
  if (node->ptrs != NULL) {
    acl_build_free(context, node->max_ptrs * sizeof(struct rte_acl_ptr_set),
                   node->ptrs);
    node->ptrs = NULL;
  }
 ...
 */
static void acl_include(struct rte_acl_bitset *dst, struct rte_acl_bitset *src,
                        bits_t mask) {
  uint32_t n;
 ...
 */
static int acl_exclude(struct rte_acl_bitset *dst, struct rte_acl_bitset *src1,
 ...
 */
static int acl_add_ptr(struct acl_build_context *context,
                       struct rte_acl_node *node, struct rte_acl_node *ptr,
  ...
  if (node->num_ptrs >= node->max_ptrs) {
    ...
    if (node->ptrs != NULL) {
      memcpy(ptrs, node->ptrs, node->num_ptrs * sizeof(*ptrs));
      acl_build_free(context, node->max_ptrs * sizeof(*ptrs), node->ptrs);
    }
    node->ptrs = ptrs;
 ...
 */
static int acl_add_ptr_range(struct acl_build_context *context,
                             struct rte_acl_node *root,
                             struct rte_acl_node *node, uint8_t low,
                             uint8_t high) {
  uint32_t n;
 ...
 */
static int acl_gen_mask(struct rte_acl_bitset *bitset, uint32_t value,
                        uint32_t mask) {
  int range = 0;
  uint32_t n;
 ...
 */
static int acl_intersect_type(const struct rte_acl_bitset *a_bits,
 ...
 * Duplicate a node
 */
static struct rte_acl_node *acl_dup_node(struct acl_build_context *context,
                                         struct rte_acl_node *node) {
  uint32_t n;
  ...
  if (node->num_ptrs > 0) {
    next->ptrs = acl_build_alloc(context, node->max_ptrs,
  ...
  for (n = 0; n < node->num_ptrs; n++) {
    if (node->ptrs[n].ptr != NULL) {
      ...
      acl_include(&next->ptrs[n].values, &node->ptrs[n].values, -1);
    }
  }
 ...
 */
static int acl_copy_ptr(struct acl_build_context *context,
                        struct rte_acl_node *dst, struct rte_acl_node *src,
                        int index, struct rte_acl_bitset *b_bits) {
  int rc;
  struct rte_acl_bitset bits;
 ...
 */
static void acl_compact_node_ptrs(struct rte_acl_node *node_a) {
  uint32_t n;
  int min_add = node_a->min_add;

  while (node_a->num_ptrs > 0 && node_a->ptrs[node_a->num_ptrs - 1].ptr == NULL)
    node_a->num_ptrs--;
  ...
  for (n = min_add; n + 1 < node_a->num_ptrs; n++) {
    ...
    if (node_a->ptrs[n].ptr == NULL) {
      ...
      acl_include(&node_a->ptrs[n].values,
                  &node_a->ptrs[node_a->num_ptrs - 1].values, 0);
      node_a->ptrs[n].ptr = node_a->ptrs[node_a->num_ptrs - 1].ptr;
...
static int acl_resolve_leaf(struct acl_build_context *context,
                            struct rte_acl_node *node_a,
  ...
  for (n = 0; n < context->cfg.num_categories; n++) {
    if (node_a->mrt->priority[n] != node_b->mrt->priority[n]) {
      combined_priority |= (node_a->mrt->priority[n] > node_b->mrt->priority[n])
                               ? ACL_PRIORITY_NODE_A
                               : ACL_PRIORITY_NODE_B;
    }
  }
...
static int acl_merge_trie(struct acl_build_context *context,
   ...
   */
  node_intersect_type =
      acl_intersect_type(&node_c->values, &node_b->values, &node_intersect);
    ...
    for (n = 0; n < ptrs_c; n++) {
      ...
      for (m = 0; m < ptrs_b; m++) {
        ...
        if (node_b->ptrs[m].ptr == NULL ||
            node_c->ptrs[n].ptr == node_b->ptrs[m].ptr)
          continue;

        child_intersect_type = acl_intersect_type(
            &node_c->ptrs[n].values, &node_b->ptrs[m].values, &child_intersect);

        if ((child_intersect_type & ACL_INTERSECT) != 0) {
          if (acl_merge_trie(context, node_c->ptrs[n].ptr, node_b->ptrs[m].ptr,
                             level + 1, &child_node_c))
            return 1;

          if (child_node_c != NULL && child_node_c != node_c->ptrs[n].ptr) {
             ...
             * in the intersection.
             */
            acl_add_ptr(context, node_c, child_node_c, &child_intersect);

            /*
             * inc refs if pointer is not
             * to node b.
             */
            node_a_refs += (child_node_c != node_b->ptrs[m].ptr);

            /*
             * Remove intersection from C
             * pointer.
             */
            if (!acl_exclude(&node_c->ptrs[n].values, &node_c->ptrs[n].values,
                             &child_intersect)) {
  ...
  if ((node_intersect_type & ACL_INTERSECT_B) != 0) {
    ...
    for (m = 0; m < node_b->num_ptrs; m++)
      if (node_b->ptrs[m].ptr != NULL)
        acl_copy_ptr(context, node_c, node_b, m, &node_intersect);
  }
 ...
 */
static void acl_build_reset(struct rte_acl_ctx *ctx) {
  rte_free(ctx->mem);
  memset(&ctx->num_categories, 0,
         sizeof(*ctx) - offsetof(struct rte_acl_ctx, num_categories));
}

static void acl_gen_full_range(struct acl_build_context *context,
                               struct rte_acl_node *root,
...
static void acl_gen_range_mdl(struct acl_build_context *context,
                              struct rte_acl_node *root,
                              struct rte_acl_node *end, uint8_t lo, uint8_t hi,
                              int size, int level) {
  struct rte_acl_node *node;
...
static void acl_gen_range_low(struct acl_build_context *context,
                              struct rte_acl_node *root,
                              struct rte_acl_node *end, const uint8_t *lo,
                              int size, int level) {
  struct rte_acl_node *node;
  uint32_t n;
  ...
  if (n > 1 && lo[n - 1] != UINT8_MAX)
    acl_gen_range_mdl(context, node, end, lo[n - 1] + 1, UINT8_MAX, n, level);
}

static void acl_gen_range_high(struct acl_build_context *context,
                               struct rte_acl_node *root,
                               struct rte_acl_node *end, const uint8_t *hi,
                               int size, int level) {
  struct rte_acl_node *node;
  uint32_t n;
  ...
  /* generate middle sub-trie */
  if (n > 1 && hi[n - 1] != 0)
    acl_gen_range_mdl(context, node, end, 0, hi[n - 1] - 1, n, level);
}

static struct rte_acl_node *
acl_gen_range_trie(struct acl_build_context *context, const void *min,
                   const void *max, int size, int level,
  ...
  *pend = acl_alloc_node(context, level + size);
  ...
  if (lo[n] + 1 != hi[n] || lo_00 == 0 || hi_ff == UINT8_MAX) {
    ...
    acl_gen_range_mdl(context, prev, *pend, lo_00, hi_ff, n + 1, level);
  }

  return root;
}

static struct rte_acl_node *acl_gen_mask_trie(struct acl_build_context *context,
                                              const void *value,
...
static struct rte_acl_node *build_trie(struct acl_build_context *context,
  ...
  while (rule != NULL) {
    ...
    for (n = 0; n < rule->config->num_fields; n++) {
      ...
      case RTE_ACL_FIELD_TYPE_BITMASK:
        merge =
            acl_gen_mask_trie(context, &fld->value, &fld->mask_range,
                              rule->config->defs[n].size, end->level + 1, &end);
        break;
      ...
      case RTE_ACL_FIELD_TYPE_MASK: {
         ...
         */
        uint64_t mask;
        mask = RTE_ACL_MASKLEN_TO_BITMASK(fld->mask_range.u32,
                                          rule->config->defs[n].size);

        /* gen a mini-trie for this field */
        merge =
            acl_gen_mask_trie(context, &fld->value, (char *)&mask,
                              rule->config->defs[n].size, end->level + 1, &end);
      } break;

      case RTE_ACL_FIELD_TYPE_RANGE:
        merge = acl_gen_range_trie(context, &rule->f->field[field_index].value,
                                   &rule->f->field[field_index].mask_range,
                                   rule->config->defs[n].size, end->level + 1,
                                   &end);
        break;

      default:
        RTE_LOG(ERR, ACL, "Error in rule[%u] type - %hhu\n",
                rule->f->data.userdata, rule->config->defs[n].type);
        return NULL;
      }

      /* merge this field on to the end of the rule */
      if (acl_merge_trie(context, end_prev, merge, 0, NULL) != 0) {
        return NULL;
      }
    }
     ...
     */
    if (end->mrt == NULL)
      end->mrt = acl_build_alloc(context, 1, sizeof(*end->mrt));
...
static void acl_calc_wildness(struct rte_acl_build_rule *head,
  ...
  for (rule = head; rule != NULL; rule = rule->next) {
    ...
    for (n = 0; n < config->num_fields; n++) {
      ...
      uint64_t msk_val = RTE_LEN2MASK(bit_len, typeof(msk_val));
      double size = bit_len;
      int field_index = config->defs[n].field_index;
      const struct rte_acl_field *fld = rule->f->field + field_index;

      switch (rule->config->defs[n].type) {
      case RTE_ACL_FIELD_TYPE_BITMASK:
        wild =
            (size - __builtin_popcountll(fld->mask_range.u64 & msk_val)) / size;
        break;
      ...
      case RTE_ACL_FIELD_TYPE_RANGE:
        wild = (fld->mask_range.u64 & msk_val) - (fld->value.u64 & msk_val);
        wild = wild / msk_val;
        break;
      }
...
static void acl_rule_stats(struct rte_acl_build_rule *head,
                           struct rte_acl_config *config) {
  struct rte_acl_build_rule *rule;
  ...
  for (rule = head; rule != NULL; rule = rule->next) {
    ...
    for (n = 0; n < config->num_fields; n++) {
      ...
      for (m = 1; m < RTE_DIM(wild_limits); m++) {
        if (rule->wildness[field_index] >= wild_limits[m])
          tally[n][m]++;
      }
    }
  ...
  if (fields_deactivated) {
    ...
    for (k = 0; k < config->num_fields; k++) {
      if (tally[k][TALLY_DEACTIVATED] == 0) {
        memmove(&tally[l][0], &tally[k][0], TALLY_NUM * sizeof(tally[0][0]));
        memmove(&config->defs[l++], &config->defs[k],
...
static int rule_cmp_wildness(struct rte_acl_build_rule *r1,
                             struct rte_acl_build_rule *r2) {
  uint32_t n;
  ...
  for (n = 1; n < r1->config->num_fields; n++) {
    ...
    if (r1->wildness[field_index] != r2->wildness[field_index])
      return r1->wildness[field_index] - r2->wildness[field_index];
  }
  return 0;
}
 ...
 */
static void rule_list_split(struct rte_acl_build_rule *source,
 ...
 */
static struct rte_acl_build_rule *sort_rules(struct rte_acl_build_rule *head) {
...
static uint32_t acl_build_index(const struct rte_acl_config *config,
                                uint32_t *data_index) {
  uint32_t n, m;
  int32_t last_header;
...
build_one_trie(struct acl_build_context *context,
  ...
  context->tries[n].num_data_indexes =
      acl_build_index(config, context->data_indexes[n]);
  context->tries[n].data_index = context->data_indexes[n];

  context->cur_node_max = node_max;

  context->bld_tries[n].trie =
      build_trie(context, rule_sets[n], &last, &context->tries[n].count);

  return last;
}

static int acl_build_tries(struct acl_build_context *context,
                           struct rte_acl_build_rule *head) {
  ...
  for (n = 0;; n = num_tries) {
    ...
    for (head = rule_sets[num_tries]; head != NULL; head = head->next)
      head->config = config;
...
static void acl_build_log(const struct acl_build_context *ctx) {
  uint32_t n;
  ...
  RTE_LOG(DEBUG, ACL,
          ...
          "nodes created: %u\n"
          "memory consumed: %zu\n",
          ctx->acx->name, ctx->node_max, ctx->num_nodes, ctx->pool.alloc);

  for (n = 0; n < RTE_DIM(ctx->tries); n++) {
    if (ctx->tries[n].count != 0)
      RTE_LOG(DEBUG, ACL, "trie %u: number of rules: %u, indexes: %u\n", n,
              ctx->tries[n].count, ctx->tries[n].num_data_indexes);
  }
}

static int acl_build_rules(struct acl_build_context *bcx) {
  ...
  for (i = 0; i != n; i++) {
    rule = (const struct rte_acl_rule *)((uintptr_t)bcx->acx->rules +
                                         bcx->acx->rule_sz * i);
 ...
 */
static void acl_set_data_indexes(struct rte_acl_ctx *ctx) {
  uint32_t i, n, ofs;
 ...
 */
static int acl_bld(struct acl_build_context *bcx, struct rte_acl_ctx *ctx,
  ...
  bcx->cfg = *cfg;
  bcx->category_mask =
      RTE_LEN2MASK(bcx->cfg.num_categories, typeof(bcx->category_mask));
  ...
  if (rc != 0) {
    RTE_LOG(ERR, ACL, "ACL context: %s, %s() failed with error code: %d\n",
 ...
 */
static int acl_check_bld_param(struct rte_acl_ctx *ctx,
  ...
  if (ctx == NULL || cfg == NULL || cfg->num_categories == 0 ||
      cfg->num_categories > RTE_ACL_MAX_CATEGORIES || cfg->num_fields == 0 ||
      cfg->num_fields > RTE_ACL_MAX_FIELDS)
    return -EINVAL;

  for (i = 0; i != cfg->num_fields; i++) {
    if (cfg->defs[i].type > RTE_ACL_FIELD_TYPE_BITMASK) {
      RTE_LOG(ERR, ACL, "ACL context: %s, invalid type: %hhu for %u-th field\n",
              ctx->name, cfg->defs[i].type, i);
      return -EINVAL;
    }
    for (j = 0;
         j != RTE_DIM(field_sizes) && cfg->defs[i].size != field_sizes[j]; j++)
      ;

    if (j == RTE_DIM(field_sizes)) {
      RTE_LOG(ERR, ACL, "ACL context: %s, invalid size: %hhu for %u-th field\n",
 ...
 */
static uint32_t get_first_load_size(const struct rte_acl_config *cfg) {
...
int rte_acl_build(struct rte_acl_ctx *ctx, const struct rte_acl_config *cfg) {
  ...
  for (rc = -ERANGE; n >= NODE_MIN && rc == -ERANGE; n /= 2) {
    ...
    if (rc == 0) {
      ...
      rc = rte_acl_gen(ctx, bcx.tries, bcx.bld_tries, bcx.num_tries,
                       bcx.cfg.num_categories,
