 ...
 */
static inline void resolve_priority_sse(uint64_t transition, int n,
 ...
 */
static void acl_process_matches(xmm_t *indices, int slot,
                                const struct rte_acl_ctx *ctx,
                                struct parms *parms,
...
acl_match_check_x4(int slot, const struct rte_acl_ctx *ctx, struct parms *parms,
                   struct acl_flow_data *flows, xmm_t *indices1,
 ...
 */
static __rte_always_inline xmm_t transition4(xmm_t next_input,
  ...
  ACL_TR_CALC_ADDR(mm, 128, addr, xmm_index_mask.x, next_input,
                   xmm_shuffle_input.x, xmm_ones_16.x, xmm_range_base.x, tr_lo,
                   tr_hi);
 ...
 */
static inline int search_sse_8(const struct rte_acl_ctx *ctx,
  ...
  acl_match_check_x4(0, ctx, parms, &flows, &indices1, &indices2,
                     xmm_match_mask.x);
  acl_match_check_x4(4, ctx, parms, &flows, &indices3, &indices4,
                     xmm_match_mask.x);
 ...
 */
static inline int search_sse_4(const struct rte_acl_ctx *ctx,
                               const uint8_t **data, uint32_t *results,
