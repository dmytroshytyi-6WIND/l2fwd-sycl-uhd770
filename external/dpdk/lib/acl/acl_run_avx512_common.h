 ...
 */
static __rte_always_inline _T_simd _F_(calc_addr)(
    _T_simd index_mask, _T_simd next_input, _T_simd shuffle_input,
 ...
 */
static __rte_always_inline _T_simd _F_(trans)(_T_simd next_input,
 ...
 */
static inline _T_simd _F_(get_next_bytes)(const struct acl_flow_avx512 *flow,
                                          _T_simd pdata[2], uint32_t msk,
                                          _T_simd *di, uint32_t bnum) {
  const int32_t *div;
  uint32_t m[2];
 ...
 */
static inline void _F_(start_flow)(struct acl_flow_avx512 *flow, uint32_t num,
                                   uint32_t msk, _T_simd pdata[2], _T_simd *idx,
                                   _T_simd *di) {
  uint32_t n, m[2], nm[2];
  _T_simd ni, nd[2];
 ...
 * tr_hi contains high 32 bits for up to _N_ transitions.
 */
static inline uint32_t _F_(match_process)(struct acl_flow_avx512 *flow,
                                          uint32_t *fmsk, uint32_t *rmsk,
                                          _T_simd pdata[2], _T_simd *di,
                                          _T_simd *idx, _T_simd *tr_lo,
                                          _T_simd *tr_hi) {
  uint32_t n;
  _T_simd res;
 ...
 */
static inline void _F_(match_check_process)(struct acl_flow_avx512 *flow,
                                            uint32_t fm[2], _T_simd pdata[4],
                                            _T_simd di[2], _T_simd idx[2],
                                            _T_simd inp[2], _T_simd tr_lo[2],
                                            _T_simd tr_hi[2]) {
  uint32_t n[2];
  uint32_t rm[2];
  ...
  while ((rm[0] | rm[1]) != 0) {
    ...
    n[0] = _F_(match_process)(flow, &fm[0], &rm[0], &pdata[0], &di[0], &idx[0],
                              &tr_lo[0], &tr_hi[0]);
    n[1] = _F_(match_process)(flow, &fm[1], &rm[1], &pdata[2], &di[1], &idx[1],
                              &tr_lo[1], &tr_hi[1]);

    /* execute first transition for new flows, if any */

    if (n[0] != 0) {
      inp[0] = _F_(get_next_bytes)(flow, &pdata[0], rm[0], &di[0],
                                   flow->first_load_sz);
    ...
    if (n[1] != 0) {
      inp[1] = _F_(get_next_bytes)(flow, &pdata[2], rm[1], &di[1],
                                   flow->first_load_sz);
...
static inline void _F_(reset_flow_vars)(_T_simd di[2], _T_simd idx[2],
                                        _T_simd pdata[4], _T_simd tr_lo[2],
                                        _T_simd tr_hi[2]) {
  di[0] = _M_SI_(setzero)();
  di[1] = _M_SI_(setzero)();
 ...
 */
static inline void _F_(search_trie)(struct acl_flow_avx512 *flow) {
  uint32_t fm[2];
  ...
  /* first 1B load */
  _F_(start_flow)
  (flow, _SIMD_MASK_BIT_, _SIMD_MASK_MAX_, &pdata[0], &idx[0], &di[0]);
  _F_(start_flow)
  (flow, _SIMD_MASK_BIT_, _SIMD_MASK_MAX_, &pdata[2], &idx[1], &di[1]);
  ...
  while ((fm[0] | fm[1]) != 0) {
    ...
    in[0] =
        _F_(get_next_bytes)(flow, &pdata[0], fm[0], &di[0], sizeof(uint32_t));
    in[1] =
        _F_(get_next_bytes)(flow, &pdata[2], fm[1], &di[1], sizeof(uint32_t));
 ...
 */
static inline _T_simd _F_(resolve_match_idx)(_T_simd mi) {
 ...
 */
static inline _T_simd _F_(resolve_pri)(const int32_t res[], const int32_t pri[],
 ...
 */
static inline void _F_(resolve_sc)(uint32_t result[], const int32_t res[],
                                   const int32_t pri[], const uint32_t match[],
 ...
 */
static inline void _F_(resolve_single_cat)(
    uint32_t result[], const struct rte_acl_match_results pr[],
  ...
  for (k = 0; k != (nb_pkt & ~_SIMD_FLOW_MSK_); k += _SIMD_FLOW_NUM_) {
    ...
    cr[0] =
        _F_(resolve_pri)(res, pri, match + k, _SIMD_MASK_MAX_, nb_trie, nb_pkt);
    cr[1] =
        _F_(resolve_pri)(res, pri, match + j, _SIMD_MASK_MAX_, nb_trie, nb_pkt);
  ...
  if (n != 0) {
    if (n > _SIMD_MASK_BIT_) {
      _F_(resolve_sc)
      (result + k, res, pri, match + k, _SIMD_MASK_BIT_, nb_trie, nb_pkt);
      k += _SIMD_MASK_BIT_;
      n -= _SIMD_MASK_BIT_;
    }
