...
struct _altivec_acl_const {
  ...
  rte_xmm_t range_base;
    ...
    {.u32 = {RTE_ACL_NODE_INDEX, RTE_ACL_NODE_INDEX, RTE_ACL_NODE_INDEX,
             RTE_ACL_NODE_INDEX}},
    {.u16 = {1, 1, 1, 1, 1, 1, 1, 1}},
...
resolve_priority_altivec(uint64_t transition, int n,
  ...
  for (x = 0; x < categories; x += RTE_ACL_RESULTS_MULTIPLIER) {
    ...
    if (parms[n].cmplt->count != ctx->num_tries) {
      ...
      priority = vec_sel(priority, priority1, selector);
    }
 ...
 */
static __rte_always_inline uint32_t check_any_match_x4(uint64_t val[]) {
...
acl_match_check_x4(int slot, const struct rte_acl_ctx *ctx, struct parms *parms,
  ...
  while (check_any_match_x4(transitions)) {
    transitions[0] = acl_match_check(transitions[0], slot, ctx, parms, flows,
                                     resolve_priority_altivec);
    transitions[1] = acl_match_check(transitions[1], slot + 1, ctx, parms,
                                     flows, resolve_priority_altivec);
    transitions[2] = acl_match_check(transitions[2], slot + 2, ctx, parms,
                                     flows, resolve_priority_altivec);
    transitions[3] = acl_match_check(transitions[3], slot + 3, ctx, parms,
                                     flows, resolve_priority_altivec);
  }
}
...
transition4(xmm_t next_input, const uint64_t *trans, xmm_t *indices1,
            xmm_t *indices2) {
  ...
  tr_lo =
      (xmm_t){(*indices1)[0], (*indices1)[2], (*indices2)[0], (*indices2)[2]};
  tr_hi =
      (xmm_t){(*indices1)[1], (*indices1)[3], (*indices2)[1], (*indices2)[3]};
  ...
  r = vec_perm(tr_hi, (xmm_t){(uint16_t)0 << 16}, (vector unsigned char)r);
  ...
  t = (xmm_t)vec_sel(
      vec_sel((vector signed char)vec_sub(zeroes, (vector signed char)t),
              (vector signed char)t, vec_cmpgt((vector signed char)t, zeroes)),
      zeroes, vec_cmpeq((vector signed char)t, zeroes));

  t = (xmm_t)vec_msum((vector signed char)t, (vector unsigned char)t,
                      (xmm_t){});
  quad_ofs = (xmm_t)vec_msum(
      (vector signed short)t,
      *(vector signed short *)&altivec_acl_const.xmm_ones_16.u16, (xmm_t){});
 ...
 */
static inline int search_altivec_8(const struct rte_acl_ctx *ctx,
  ...
  acl_set_flow(&flows, cmplt, RTE_DIM(cmplt), data, results, total_packets,
  ...
  while (flows.started > 0) {
    ...
    input0 = (xmm_t){GET_NEXT_4BYTES(parms, 0), GET_NEXT_4BYTES(parms, 1),
                     GET_NEXT_4BYTES(parms, 2), GET_NEXT_4BYTES(parms, 3)};

    input1 = (xmm_t){GET_NEXT_4BYTES(parms, 4), GET_NEXT_4BYTES(parms, 5),
                     GET_NEXT_4BYTES(parms, 6), GET_NEXT_4BYTES(parms, 7)};

    /* Process the 4 bytes of input on each stream. */

    input0 = transition4(input0, flows.trans, (xmm_t *)&index_array[0],
                         (xmm_t *)&index_array[2]);
    input1 = transition4(input1, flows.trans, (xmm_t *)&index_array[4],
                         (xmm_t *)&index_array[6]);
 ...
 */
static inline int search_altivec_4(const struct rte_acl_ctx *ctx,
                                   const uint8_t **data, uint32_t *results,
  ...
  while (flows.started > 0) {
    ...
    /* Gather 4 bytes of input data for each stream. */
    input = (xmm_t){GET_NEXT_4BYTES(parms, 0), GET_NEXT_4BYTES(parms, 1),
                    GET_NEXT_4BYTES(parms, 2), GET_NEXT_4BYTES(parms, 3)};

    /* Process the 4 bytes of input on each stream. */
    input = transition4(input, flows.trans, (xmm_t *)&index_array[0],
                        (xmm_t *)&index_array[2]);
    input = transition4(input, flows.trans, (xmm_t *)&index_array[0],
                        (xmm_t *)&index_array[2]);
    input = transition4(input, flows.trans, (xmm_t *)&index_array[0],
