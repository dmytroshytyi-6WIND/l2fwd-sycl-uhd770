 ...
 */
static __rte_always_inline ymm_t transition8(ymm_t next_input,
  ...
  ACL_TR_CALC_ADDR(mm256, 256, addr, ymm_index_mask.y, next_input,
                   ymm_shuffle_input.y, ymm_ones_16.y, ymm_range_base.y, *tr_lo,
                   *tr_hi);
 ...
 * tr_hi contains high 32 bits for 8 transition.
 */
static inline void acl_process_matches_avx2x8(const struct rte_acl_ctx *ctx,
                                              struct parms *parms,
                                              struct acl_flow_data *flows,
                                              uint32_t slot, ymm_t matches,
                                              ymm_t *tr_lo, ymm_t *tr_hi) {
  ymm_t t0, t1;
  ymm_t lo, hi;
  xmm_t l0, l1;
  ...
  for (i = 0; i != RTE_DIM(tr) / 2; i++) {
    ...
    tr[i] = acl_match_check(tr[i], slot + i, ctx, parms, flows,
                            resolve_priority_sse);
    tr[i + 4] = acl_match_check(tr[i + 4], slot + i + 4, ctx, parms, flows,
                                resolve_priority_sse);
  }
...
static inline void acl_match_check_avx2x8(const struct rte_acl_ctx *ctx,
                                          struct parms *parms,
                                          struct acl_flow_data *flows,
                                          uint32_t slot, ymm_t *tr_lo,
                                          ymm_t *tr_hi, ymm_t match_mask) {
  uint32_t msk;
  ymm_t matches, temp;
 ...
 */
static inline int search_avx2x16(const struct rte_acl_ctx *ctx,
  ...
  t0 = _mm256_set_epi64x(index_array[5], index_array[4], index_array[1],
                         index_array[0]);
  t1 = _mm256_set_epi64x(index_array[7], index_array[6], index_array[3],
                         index_array[2]);

  ACL_TR_HILO(mm256, __m256, t0, t1, tr_lo[0], tr_hi[0]);

  t0 = _mm256_set_epi64x(index_array[13], index_array[12], index_array[9],
                         index_array[8]);
  t1 = _mm256_set_epi64x(index_array[15], index_array[14], index_array[11],
                         index_array[10]);
  ...
  while (flows.started > 0) {
    ...
    input[0] = _mm256_set_epi32(in[7], in[6], in[5], in[4], in[3], in[2], in[1],
                                in[0]);
    ...
    input[1] = _mm256_set_epi32(in[7], in[6], in[5], in[4], in[3], in[2], in[1],
                                in[0]);
