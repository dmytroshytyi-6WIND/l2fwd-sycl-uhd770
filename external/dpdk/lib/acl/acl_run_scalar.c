...
static inline uint32_t scan_forward(uint32_t input, uint32_t max) {
  return (input == 0) ? max : rte_bsf32(input);
}

static inline uint64_t scalar_transition(const uint64_t *trans_table,
                                         uint64_t transition, uint8_t input) {
  ...
  if (index != RTE_ACL_NODE_DFA) {
    ...
    x = ranges >> (input / RTE_ACL_DFA_GR64_SIZE * RTE_ACL_DFA_GR64_BIT);
    x &= UINT8_MAX;
    x = input - x;
  }
...
int rte_acl_classify_scalar(const struct rte_acl_ctx *ctx, const uint8_t **data,
  ...
  acl_set_flow(&flows, cmplt, RTE_DIM(cmplt), data, results, num, categories,
               ctx->trans_table);
  ...
  while ((transition0 | transition1) & RTE_ACL_NODE_MATCH) {
    transition0 = acl_match_check(transition0, 0, ctx, parms, &flows,
                                  resolve_priority_scalar);
    transition1 = acl_match_check(transition1, 1, ctx, parms, &flows,
                                  resolve_priority_scalar);
  }
  ...
  while (flows.started > 0) {
    ...
    for (n = 0; n < 4; n++) {
      ...
      transition0 =
          scalar_transition(flows.trans, transition0, (uint8_t)input0);
      input0 >>= CHAR_BIT;

      transition1 =
          scalar_transition(flows.trans, transition1, (uint8_t)input1);
      input1 >>= CHAR_BIT;
    }
