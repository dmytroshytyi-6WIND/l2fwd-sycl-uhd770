...
static void acl_gen_log_stats(const struct rte_acl_ctx *ctx,
  ...
  RTE_LOG(DEBUG, ACL,
          ...
          "total: %zu bytes\n"
          "max limit: %zu bytes\n",
          ctx->name, ctx->socket_id, counts->single,
          counts->single * sizeof(uint64_t), counts->quad, counts->quad_vectors,
          (indices->quad_index - indices->dfa_index) * sizeof(uint64_t),
          counts->dfa, counts->dfa_gr64, indices->dfa_index * sizeof(uint64_t),
          counts->match, counts->match * sizeof(struct rte_acl_match_results),
          ctx->mem_sz, max_size);
}

static uint64_t acl_dfa_gen_idx(const struct rte_acl_node *node,
                                uint32_t index) {
  uint64_t idx;
  uint32_t i;
  ...
  for (i = 0; i != RTE_DIM(node->dfa_gr64); i++) {
    ...
    idx |= (i - node->dfa_gr64[i]) << (6 + RTE_ACL_DFA_GR64_BIT * i);
  }

  return idx << (CHAR_BIT * sizeof(index)) | index | node->node_type;
}

static void acl_dfa_fill_gr64(const struct rte_acl_node *node,
                              const uint64_t src[RTE_ACL_DFA_SIZE],
                              uint64_t dst[RTE_ACL_DFA_SIZE]) {
  uint32_t i;
...
static uint32_t acl_dfa_count_gr64(const uint64_t array_ptr[RTE_ACL_DFA_SIZE],
  ...
  for (i = 0; i != RTE_ACL_DFA_GR64_NUM; i++) {
    ...
    for (j = 0; j != i; j++) {
      if (memcmp(array_ptr + i * RTE_ACL_DFA_GR64_SIZE,
                 ...
                 RTE_ACL_DFA_GR64_SIZE * sizeof(array_ptr[0])) == 0)
        break;
    }
    gr64[i] = (j != i) ? gr64[j] : k++;
  }
...
static uint32_t acl_node_fill_dfa(const struct rte_acl_node *node,
 ...
 *  node.
 */
static int acl_count_sequential_groups(struct rte_acl_bitset *bits,
                                       int zero_one) {
 ...
 */
static int acl_count_fanout(struct rte_acl_node *node) {
  uint32_t n;
  int ranges;
 ...
 */
static void acl_count_trie_types(struct acl_node_counters *counts,
                                 struct rte_acl_node *node, uint64_t no_match,
                                 int force_dfa) {
  uint32_t n;
  int num_ptrs;
  uint64_t dfa[RTE_ACL_DFA_SIZE];
...
static void acl_add_ptrs(struct rte_acl_node *node, uint64_t *node_array,
                         uint64_t no_match, int resolved) {
  uint32_t x;
  int32_t m;
 ...
 */
static void acl_gen_node(struct rte_acl_node *node, uint64_t *node_array,
                         uint64_t no_match, struct rte_acl_indices *index,
                         int num_categories) {
  uint32_t n, sz, *qtrp;
  uint64_t *array_ptr;
  ...
  case RTE_ACL_NODE_SINGLE:
    node->node_index =
        RTE_ACL_QUAD_SINGLE | index->single_index | node->node_type;
  ...
  case RTE_ACL_NODE_MATCH:
    match = ((struct rte_acl_match_results *)(node_array + index->match_start));
  ...
  case RTE_ACL_NODE_UNDEFINED:
    RTE_ACL_VERIFY(node->node_type != (uint32_t)RTE_ACL_NODE_UNDEFINED);
    break;
  }
  ...
  for (n = 0; n < node->num_ptrs; n++) {
    if (node->ptrs[n].ptr != NULL)
      acl_gen_node(node->ptrs[n].ptr, node_array, no_match, index,
                   num_categories);
  }
...
static void acl_calc_counts_indices(struct acl_node_counters *counts,
                                    struct rte_acl_indices *indices,
                                    struct rte_acl_bld_trie *node_bld_trie,
                                    uint32_t num_tries, uint64_t no_match) {
  uint32_t n;
...
int rte_acl_gen(struct rte_acl_ctx *ctx, struct rte_acl_trie *trie,
  ...
  acl_calc_counts_indices(&counts, &indices, node_bld_trie, num_tries,
                          no_match);
  ...
  mem = rte_zmalloc_socket(ctx->name, total_size, RTE_CACHE_LINE_SIZE,
                           ctx->socket_id);
  if (mem == NULL) {
    RTE_LOG(ERR, ACL, "allocation of %zu bytes on socket %d for %s failed\n",
  ...
  for (n = 0; n < num_tries; n++) {
    ...
    acl_gen_node(node_bld_trie[n].trie, node_array, no_match, &indices,
                 num_categories);
