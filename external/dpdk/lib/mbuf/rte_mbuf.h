 ...
 */
static inline void rte_mbuf_prefetch_part1(struct rte_mbuf *m) {
 ...
 *   The pointer to the mbuf.
 */
static inline void rte_mbuf_prefetch_part2(struct rte_mbuf *m) {
 ...
 */
static inline rte_iova_t rte_mbuf_data_iova(const struct rte_mbuf *mb) {
 ...
 *   The IO address of the beginning of the mbuf data
 */
static inline rte_iova_t rte_mbuf_data_iova_default(const struct rte_mbuf *mb) {
 ...
 */
static inline struct rte_mbuf *rte_mbuf_from_indirect(struct rte_mbuf *mi) {
 ...
 */
static inline char *rte_mbuf_buf_addr(struct rte_mbuf *mb,
 ...
 */
static inline char *rte_mbuf_data_addr_default(struct rte_mbuf *mb) {
 ...
 */
static inline char *rte_mbuf_to_baddr(struct rte_mbuf *md) {
 ...
 */
static inline uint32_t rte_pktmbuf_priv_flags(struct rte_mempool *mp) {
...
/* internal */
static inline uint16_t __rte_mbuf_refcnt_update(struct rte_mbuf *m,
                                                int16_t value) {
 ...
 *   Updated value
 */
static inline uint16_t rte_mbuf_refcnt_update(struct rte_mbuf *m,
                                              int16_t value) {
  /*
...
rte_mbuf_ext_refcnt_update(struct rte_mbuf_ext_shared_info *shinfo,
  ...
  return __atomic_add_fetch(&shinfo->refcnt, (uint16_t)value, __ATOMIC_ACQ_REL);
}
 ...
 */
void rte_pktmbuf_init(struct rte_mempool *mp, void *opaque_arg, void *m,
                      unsigned i);
 ...
 *    - ENOMEM - no appropriate memory area found in which to create memzone
 */
__rte_experimental struct rte_mempool *rte_pktmbuf_pool_create_extbuf(
 ...
 *   The source mbuf.
 */
static inline void rte_mbuf_dynfield_copy(struct rte_mbuf *mdst,
                                          const struct rte_mbuf *msrc) {
  memcpy(&mdst->dynfield1, msrc->dynfield1, sizeof(mdst->dynfield1));
}

/* internal */
static inline void __rte_pktmbuf_copy_hdr(struct rte_mbuf *mdst,
                                          const struct rte_mbuf *msrc) {
  mdst->port = msrc->port;
...
static inline void rte_pktmbuf_attach(struct rte_mbuf *mi, struct rte_mbuf *m) {
  RTE_ASSERT(RTE_MBUF_DIRECT(mi) && rte_mbuf_refcnt_read(mi) == 1);
 ...
 * callback.
 */
static inline void __rte_pktmbuf_free_extbuf(struct rte_mbuf *m) {
 ...
 */
static inline void __rte_pktmbuf_free_direct(struct rte_mbuf *m) {
  struct rte_mbuf *md;
...
static inline int __rte_pktmbuf_pinned_extbuf_decref(struct rte_mbuf *m) {
   ...
   */
  if (likely(
          __atomic_add_fetch(&shinfo->refcnt, (uint16_t)-1, __ATOMIC_ACQ_REL)))
    return 1;
 ...
 */
static inline int rte_pktmbuf_chain(struct rte_mbuf *head,
...
rte_mbuf_tx_offload(uint64_t il2, uint64_t il3, uint64_t il4, uint64_t tso,
  ...
  return il2 << RTE_MBUF_L2_LEN_OFS | il3 << RTE_MBUF_L3_LEN_OFS |
         il4 << RTE_MBUF_L4_LEN_OFS | tso << RTE_MBUF_TSO_SEGSZ_OFS |
         ol3 << RTE_MBUF_OUTL3_LEN_OFS | ol2 << RTE_MBUF_OUTL2_LEN_OFS |
 ...
 */
static inline int rte_validate_tx_offload(const struct rte_mbuf *m) {
  ...
  if (ol_flags & RTE_MBUF_F_TX_TCP_SEG)
    if ((m->tso_segsz == 0) || ((ol_flags & RTE_MBUF_F_TX_IPV4) &&
 ...
 *   - -1, on error
 */
static inline int rte_pktmbuf_linearize(struct rte_mbuf *mbuf) {
 ...
 */
static inline uint32_t rte_mbuf_sched_queue_get(const struct rte_mbuf *m) {
 ...
 */
static inline uint8_t rte_mbuf_sched_color_get(const struct rte_mbuf *m) {
 ...
 */
static inline void rte_mbuf_sched_get(const struct rte_mbuf *m,
                                      uint32_t *queue_id,
 ...
 */
static inline void rte_mbuf_sched_queue_set(struct rte_mbuf *m,
                                            uint32_t queue_id) {
 ...
 */
static inline void rte_mbuf_sched_traffic_class_set(struct rte_mbuf *m,
 ...
 */
static inline void rte_mbuf_sched_color_set(struct rte_mbuf *m, uint8_t color) {
 ...
 *  Color id to be set
 */
static inline void rte_mbuf_sched_set(struct rte_mbuf *m, uint32_t queue_id,
                                      uint8_t traffic_class, uint8_t color) {
