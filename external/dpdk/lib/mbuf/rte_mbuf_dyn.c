 ...
 */
static void process_score(void) {
  ...
  while (off < sizeof(struct rte_mbuf)) {
    ...
    for (size = 0;
         (off + size) < sizeof(struct rte_mbuf) && shm->free_space[off + size];
         size++)
      ;
    if (size == 0) {
      off++;
     ...
     */
    for (align = 1; (off % (align << 1)) == 0 && (align << 1) <= size;
         align <<= 1)
      ;
...
#define mark_free(field)                                                       \
  memset(&shm->free_space[offsetof(struct rte_mbuf, field)], 1,                \
...
static int init_shared_mem(void) {
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    mz = rte_memzone_reserve_aligned(RTE_MBUF_DYN_MZNAME,
                                     sizeof(struct mbuf_dyn_shm), SOCKET_ID_ANY,
...
static int check_offset(size_t offset, size_t size, size_t align) {
  size_t i;
...
static struct mbuf_dynfield_elt *__mbuf_dynfield_lookup(const char *name) {
  struct mbuf_dynfield_list *mbuf_dynfield_list;
  struct mbuf_dynfield_elt *mbuf_dynfield;
  struct rte_tailq_entry *te;

  mbuf_dynfield_list =
      RTE_TAILQ_CAST(mbuf_dynfield_tailq.head, mbuf_dynfield_list);
...
__rte_mbuf_dynfield_register_offset(const struct rte_mbuf_dynfield *params,
  ...
  if (req == SIZE_MAX) {
     ...
     */
    for (offset = 0; offset < sizeof(struct rte_mbuf); offset++) {
      if (check_offset(offset, params->size, params->align) == 0 &&
  ...
  RTE_LOG(DEBUG, MBUF,
          ...
          params->name, params->size, params->align, params->flags, offset);
...
/* assume tailq is locked */
static struct mbuf_dynflag_elt *__mbuf_dynflag_lookup(const char *name) {
  struct mbuf_dynflag_list *mbuf_dynflag_list;
  struct mbuf_dynflag_elt *mbuf_dynflag;
  struct rte_tailq_entry *te;

  mbuf_dynflag_list =
      RTE_TAILQ_CAST(mbuf_dynflag_tailq.head, mbuf_dynflag_list);

  TAILQ_FOREACH(te, mbuf_dynflag_list, next) {
    mbuf_dynflag = (struct mbuf_dynflag_elt *)te->data;
    if (strncmp(name, mbuf_dynflag->params.name, RTE_MBUF_DYN_NAMESIZE) == 0)
      break;
  }
...
void rte_mbuf_dyn_dump(FILE *out) {
  ...
  TAILQ_FOREACH(te, mbuf_dynfield_list, next) {
    ...
    fprintf(out, "  name=%s offset=%zd size=%zd align=%zd flags=%x\n",
            dynfield->params.name, dynfield->offset, dynfield->params.size,
            dynfield->params.align, dynfield->params.flags);
  }
  ...
  TAILQ_FOREACH(te, mbuf_dynflag_list, next) {
    ...
    fprintf(out, "  name=%s bitnum=%u flags=%x\n", dynflag->params.name,
            dynflag->bitnum, dynflag->params.flags);
  }
  fprintf(out,
  ...
  for (i = 0; i < sizeof(struct rte_mbuf); i++) {
    if ((i % 8) == 0)
      fprintf(out, "  %4.4zx: ", i);
    fprintf(out, "%2.2x%s", shm->free_space[i], (i % 8 != 7) ? " " : "\n");
  }
...
static int rte_mbuf_dyn_timestamp_register(int *field_offset, uint64_t *flag,
  ...
  if (offset < 0) {
    RTE_LOG(ERR, MBUF, "Failed to register mbuf field for timestamp\n");
    return -1;
  }
  ...
  if (offset < 0) {
    RTE_LOG(ERR, MBUF, "Failed to register mbuf flag for %s timestamp\n",
            direction);
    return -1;
  }
  if (flag != NULL)
...
int rte_mbuf_dyn_rx_timestamp_register(int *field_offset, uint64_t *rx_flag) {
  return rte_mbuf_dyn_timestamp_register(field_offset, rx_flag, "Rx",
                                         RTE_MBUF_DYNFLAG_RX_TIMESTAMP_NAME);
}

int rte_mbuf_dyn_tx_timestamp_register(int *field_offset, uint64_t *tx_flag) {
  return rte_mbuf_dyn_timestamp_register(field_offset, tx_flag, "Tx",
