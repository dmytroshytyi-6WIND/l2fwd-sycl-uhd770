...
static int firmware_open(struct firmware_read_ctx *ctx, const char *name,
                         size_t blocksize) {
...
static ssize_t firmware_read_block(struct firmware_read_ctx *ctx, void *buf,
                                   size_t count) {
  return archive_read_data(ctx->a, buf, count);
}

static void firmware_close(struct firmware_read_ctx *ctx) {
  archive_read_free(ctx->a);
  ctx->a = NULL;
}
...
static int firmware_read(const char *name, void **buf, size_t *bufsz) {
...
int rte_firmware_read(const char *name, void **buf, size_t *bufsz) {
  char path[PATH_MAX];
  int ret;
  ...
  if (ret < 0) {
    ...
    if (access(path, F_OK) == 0) {
      RTE_LOG(WARNING, EAL,
              "libarchive not linked, %s cannot be decompressed\n", path);
    }
#else
