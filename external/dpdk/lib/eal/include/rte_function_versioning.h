 ...
 * When a symbol is exported from a library to provide an API, it also provides
 * a calling convention (ABI) that is embodied in its name, return type,
 ...
 * internal function name <b><e>
 */
#define VERSION_SYMBOL(b, e, n)                                                \
  __asm__(".symver " RTE_STR(b) RTE_STR(e) ", " RTE_STR(b) "@DPDK_" RTE_STR(n))

/*
 * VERSION_SYMBOL_EXPERIMENTAL
 * Creates a symbol version table entry binding the symbol <b>@EXPERIMENTAL to
 * the internal function name <b><e>. The macro is used when a symbol matures to
 * become part of the stable ABI, to provide an alias to experimental for some
 * time.
 */
#define VERSION_SYMBOL_EXPERIMENTAL(b, e)                                      \
  __asm__(".symver " RTE_STR(b) RTE_STR(e) ", " RTE_STR(b) "@EXPERIMENTAL")
