...
#define RTE_PMD_EXPORT_NAME(name, idx)                                         \
  static const char RTE_PMD_EXPORT_NAME_ARRAY(this_pmd_name, idx) __rte_used = \
      RTE_STR(name)
...
#define RTE_PMD_REGISTER_PARAM_STRING(name, str)                               \
  static const char DRV_EXP_TAG(name, param_string_export)[] __rte_used = str
...
#define RTE_DEV_FOREACH(dev, devstr, it)                                       \
  for (rte_dev_iterator_init(it, devstr), dev = rte_dev_iterator_next(it);     \
       dev != NULL; dev = rte_dev_iterator_next(it))
 ...
 */
__rte_experimental int rte_dev_dma_map(struct rte_device *dev, void *addr,
 ...
 *	Negative value and rte_errno is set otherwise.
 */
__rte_experimental int rte_dev_dma_unmap(struct rte_device *dev, void *addr,
                                         uint64_t iova, size_t len);
