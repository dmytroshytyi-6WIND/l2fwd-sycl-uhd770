 ...
 */
__rte_experimental static __rte_always_inline bool
 ...
 *
 */
__rte_experimental void __rte_trace_mem_per_thread_alloc(void);
...
__rte_trace_point_emit_field(size_t sz, const char *field, const char *type);
 ...
 *   - <0: Failure to register the tracepoint.
 */
__rte_experimental int __rte_trace_point_register(rte_trace_point_t *trace,
                                                  const char *name,
...
static __rte_always_inline void *__rte_trace_point_emit_ev_header(void *mem,
                                                                  uint64_t in) {
  uint64_t val;

  /* Event header [63:0] = id [63:48] | timestamp [47:0] */
  val =
      rte_get_tsc_cycles() & ~(0xffffULL << __RTE_TRACE_EVENT_HEADER_ID_SHIFT);
