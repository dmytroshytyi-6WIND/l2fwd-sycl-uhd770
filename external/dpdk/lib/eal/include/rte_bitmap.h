 ...
 *
 * The bitmap scan operation is optimized for 64-bit CPUs using 64/128 byte
 *cache lines. The bitmap is hierarchically organized using two arrays (array1
 *and array2), with each bit in array1 being associated with a full cache line
...
struct rte_bitmap {
  ...
  uint32_t array1_size; /**< Number of 64-bit slabs in array1 that are actually
                           used */
  ...
  uint32_t
      go2; /**< Bitmap scan: Go/stop condition for current array2 cache line */

  /* Storage space for array1 and array2 */
  uint8_t memory[];
};

static inline void __rte_bitmap_index1_inc(struct rte_bitmap *bmp) {
  bmp->index1 = (bmp->index1 + 1) & (bmp->array1_size - 1);
}

static inline uint64_t __rte_bitmap_mask1_get(struct rte_bitmap *bmp) {
  return (~1llu) << bmp->offset1;
}

static inline void __rte_bitmap_index2_set(struct rte_bitmap *bmp) {
  bmp->index2 = (((bmp->index1 << RTE_BITMAP_SLAB_BIT_SIZE_LOG2) + bmp->offset1)
                 << RTE_BITMAP_CL_SLAB_SIZE_LOG2);
}

static inline uint32_t __rte_bitmap_get_memory_footprint(
    uint32_t n_bits, uint32_t *array1_byte_offset, uint32_t *array1_slabs,
  ...
  n_cache_lines_array2 =
      (n_bits + RTE_BITMAP_CL_BIT_SIZE - 1) / RTE_BITMAP_CL_BIT_SIZE;
  ...
  n_slabs_context =
      (sizeof(struct rte_bitmap) + (RTE_BITMAP_SLAB_BIT_SIZE / 8) - 1) /
      (RTE_BITMAP_SLAB_BIT_SIZE / 8);
  n_cache_lines_context_and_array1 =
      (n_slabs_context + n_slabs_array1 + RTE_BITMAP_CL_SLAB_SIZE - 1) /
      RTE_BITMAP_CL_SLAB_SIZE;
  n_bytes_total = (n_cache_lines_context_and_array1 + n_cache_lines_array2) *
                  RTE_CACHE_LINE_SIZE;
  ...
  if (array2_byte_offset) {
    *array2_byte_offset =
        n_cache_lines_context_and_array1 * RTE_CACHE_LINE_SIZE;
  }
  if (array2_slabs) {
...
static inline void __rte_bitmap_scan_init(struct rte_bitmap *bmp) {
 ...
 */
static inline uint32_t rte_bitmap_get_memory_footprint(uint32_t n_bits) {
 ...
 */
static inline struct rte_bitmap *rte_bitmap_init(uint32_t n_bits, uint8_t *mem,
                                                 uint32_t mem_size) {
  ...
  size = __rte_bitmap_get_memory_footprint(n_bits, &array1_byte_offset,
                                           &array1_slabs, &array2_byte_offset,
                                           &array2_slabs);
  if (size > mem_size)
    return NULL;
...
__rte_bitmap_clear_slab_overhead_bits(uint64_t *slabs, uint32_t slab_size,
  ...
  if (index < slab_size)
    memset(&slabs[index], 0, sizeof(slabs[0]) * (slab_size - index));
}
 ...
 *   Handle to bitmap instance.
 */
__rte_experimental static inline struct rte_bitmap *
rte_bitmap_init_with_all_set(uint32_t n_bits, uint8_t *mem, uint32_t mem_size) {
  ...
  __rte_bitmap_clear_slab_overhead_bits(bmp->array1, bmp->array1_size,
                                        bmp->array2_size >>
                                            RTE_BITMAP_CL_SLAB_SIZE_LOG2);
  __rte_bitmap_clear_slab_overhead_bits(bmp->array2, bmp->array2_size, n_bits);
  return bmp;
}
 ...
 */
static inline int rte_bitmap_free(struct rte_bitmap *bmp) {
  /* Check input arguments */
  if (bmp == NULL) {
    return -1;
  }
 ...
 *   Handle to bitmap instance
 */
static inline void rte_bitmap_reset(struct rte_bitmap *bmp) {
 ...
 *   0 upon success, error code otherwise
 */
static inline void rte_bitmap_prefetch0(struct rte_bitmap *bmp, uint32_t pos) {
  uint64_t *slab2;
  uint32_t index2;
 ...
 */
static inline uint64_t rte_bitmap_get(struct rte_bitmap *bmp, uint32_t pos) {
  uint64_t *slab2;
 ...
 *   Bit position
 */
static inline void rte_bitmap_set(struct rte_bitmap *bmp, uint32_t pos) {
 ...
 */
static inline void rte_bitmap_set_slab(struct rte_bitmap *bmp, uint32_t pos,
                                       uint64_t slab) {
  uint64_t *slab1, *slab2;
...
static inline uint64_t __rte_bitmap_line_not_empty(uint64_t *slab2) {
 ...
 *   Bit position
 */
static inline void rte_bitmap_clear(struct rte_bitmap *bmp, uint32_t pos) {
  uint64_t *slab1, *slab2;
  uint32_t index1, index2, offset1, offset2;
...
static inline int __rte_bitmap_scan_search(struct rte_bitmap *bmp) {
  uint64_t value1;
  uint32_t i;
...
static inline void __rte_bitmap_scan_read_init(struct rte_bitmap *bmp) {
  __rte_bitmap_index2_set(bmp);
  bmp->go2 = 1;
  rte_prefetch1((void *)(bmp->array2 + bmp->index2 + 8));
}

static inline int __rte_bitmap_scan_read(struct rte_bitmap *bmp, uint32_t *pos,
                                         uint64_t *slab) {
  uint64_t *slab2;
 ...
 */
static inline int rte_bitmap_scan(struct rte_bitmap *bmp, uint32_t *pos,
                                  uint64_t *slab) {
