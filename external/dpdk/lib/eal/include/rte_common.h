 ...
 */
#if defined(RTE_CC_GCC) || defined(RTE_CC_CLANG)
#define __rte_alloc_size(...) __attribute__((alloc_size(__VA_ARGS__)))
#else
 ...
 */
static inline int rte_is_aligned(void *ptr, unsigned align) {
...
#ifndef __CHECKER__
#define RTE_BUILD_BUG_ON(condition) ((void)sizeof(char[2 - 2 * !!(condition)]))
#endif
...
__extension__ typedef void *RTE_MARKER[2];
/** Marker for 1B alignment in a structure. */
__extension__ typedef uint8_t RTE_MARKER8[2];
/** Marker for 2B alignment in a structure. */
__extension__ typedef uint16_t RTE_MARKER16[2];
/** Marker for 4B alignment in a structure. */
__extension__ typedef uint32_t RTE_MARKER32[2];
/** Marker for 8B alignment in a structure. */
__extension__ typedef uint64_t RTE_MARKER64[2];
 ...
 *    The combined value.
 */
static inline uint64_t rte_combine64ms1b(uint64_t v) {
  v |= v >> 1;
  v |= v >> 2;
  v |= v >> 4;
  v |= v >> 8;
 ...
 */
static inline uint32_t rte_align32prevpow2(uint32_t x) {
  x = rte_combine32ms1b(x);
 ...
 *   Input parameter aligned to the next power of 2
 */
static inline uint64_t rte_align64pow2(uint64_t v) {
  v--;
 ...
 */
static inline int rte_bsf32_safe(uint32_t v, uint32_t *pos) {
  if (v == 0)
    return 0;
 ...
 */
static inline int rte_fls_u32(uint32_t x) {
 ...
 *     least significant set bit in the input parameter.
 */
static inline int rte_bsf64(uint64_t v) { return (uint32_t)__builtin_ctzll(v); }
 ...
 *     Returns 0 if ``v`` was 0, otherwise returns 1.
 */
static inline int rte_bsf64_safe(uint64_t v, uint32_t *pos) {
  if (v == 0)
    return 0;
 ...
 *     The rounded-up log2 of the input, or 0 if the input is 0.
 */
static inline uint32_t rte_log2_u64(uint64_t v) {
  if (v == 0)
    return 0;
  v = rte_align64pow2(v);
 ...
 *     Number.
 */
static inline uint64_t rte_str_to_size(const char *str) {
  char *endptr;
 ...
 *     printf format characters which will be expanded using any further
