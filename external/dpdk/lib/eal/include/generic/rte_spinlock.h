...
static inline void rte_spinlock_lock(rte_spinlock_t *sl) {
  ...
  while (!__atomic_compare_exchange_n(&sl->locked, &exp, 1, 0, __ATOMIC_ACQUIRE,
                                      __ATOMIC_RELAXED)) {
    rte_wait_until_equal_32((volatile uint32_t *)&sl->locked, 0,
                            __ATOMIC_RELAXED);
    exp = 0;
  }
}
 ...
 */
static inline int rte_spinlock_trylock_tm(rte_spinlock_t *sl);
