...
static inline void rte_pause(void) { asm volatile("yield" ::: "memory"); }

#ifdef RTE_WAIT_UNTIL_EQUAL_ARCH_DEFINED

/* Send an event to quit WFE. */
#define __RTE_ARM_SEVL()                                                       \
  { asm volatile("sevl" : : : "memory"); }

/* Put processor into low power WFE(Wait For Event) state. */
#define __RTE_ARM_WFE()                                                        \
...
#define __RTE_ARM_LOAD_EXC_128(src, dst, memorder)                             \
  {                                                                            \
    ...
    if (memorder == __ATOMIC_RELAXED) {                                        \
      asm volatile(                                                            \
          "ldxp %x[tmp0], %x[tmp1], [%x[addr]]"                                \
          : [tmp0] "=&r"(dst_128->val[0]), [tmp1] "=&r"(dst_128->val[1])       \
          : [addr] "r"(src)                                                    \
          : "memory");                                                         \
    } else {                                                                   \
      asm volatile(                                                            \
          "ldaxp %x[tmp0], %x[tmp1], [%x[addr]]"                               \
          : [tmp0] "=&r"(dst_128->val[0]), [tmp1] "=&r"(dst_128->val[1])       \
          : [addr] "r"(src)                                                    \
          : "memory");                                                         \
    }                                                                          \
...
#define __RTE_ARM_LOAD_EXC(src, dst, memorder, size)                           \
  {                                                                            \
    RTE_BUILD_BUG_ON(size != 16 && size != 32 && size != 64 && size != 128);   \
    if (size == 16)                                                            \
