...
static __rte_always_inline void rte_atomic_thread_fence(int memorder) {
...
#define __ATOMIC128_CAS_OP(cas_op_name, op_string)                             \
  static __rte_noinline void cas_op_name(rte_int128_t *dst, rte_int128_t *old, \
    ...
    asm volatile(".arch armv8-a+lse\n" op_string                               \
                 " %[old0], %[old1], %[upd0], %[upd1], [%[dst]]"               \
                 : [old0] "+r"(x0), [old1] "+r"(x1)                            \
                 : [upd0] "r"(x2), [upd1] "r"(x3), [dst] "r"(dst)              \
                 : "memory");                                                  \
    old->val[0] = x0;                                                          \
    old->val[1] = x1;                                                          \
  }
#else
#define __ATOMIC128_CAS_OP(cas_op_name, op_string)                             \
  static __rte_always_inline void cas_op_name(                                 \
      rte_int128_t *dst, rte_int128_t *old, rte_int128_t updated) {            \
    asm volatile(op_string " %[old], %H[old], %[upd], %H[upd], [%[dst]]"       \
                 : [old] "+r"(old->int128)                                     \
...
rte_atomic128_cmp_exchange(rte_int128_t *dst, rte_int128_t *exp,
  ...
  RTE_ASSERT(success == __ATOMIC_RELAXED || success == __ATOMIC_ACQUIRE ||
             success == __ATOMIC_RELEASE || success == __ATOMIC_ACQ_REL ||
...
#define __LOAD_128(op_string, src, dst)                                        \
  {                                                                            \
    asm volatile(op_string " %0, %1, %2"                                       \
                 : "=&r"(dst.val[0]), "=&r"(dst.val[1])                        \
                 : "Q"(src->val[0])                                            \
                 : "memory");                                                  \
  }
...
#define __STORE_128(op_string, dst, src, ret)                                  \
  {                                                                            \
    asm volatile(op_string " %w0, %1, %2, %3"                                  \
                 : "=&r"(ret)                                                  \
                 : "r"(src.val[0]), "r"(src.val[1]), "Q"(dst->val[0])          \
                 : "memory");                                                  \
  }
