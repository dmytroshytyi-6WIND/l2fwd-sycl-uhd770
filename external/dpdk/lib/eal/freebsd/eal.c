...
static int rte_eal_config_attach(void) {
  ...
  rte_mem_cfg_addr = mmap(NULL, sizeof(*config->mem_config), PROT_READ,
...
int rte_eal_init(int argc, char **argv) {
  ...
  RTE_LOG(DEBUG, EAL, "Main lcore %u is ready (tid=%p;cpuset=[%s%s])\n",
          config->main_lcore, thread_id, cpuset, ret == 0 ? "" : "...");
...
enum rte_intr_mode rte_eal_vfio_intr_mode(void) { return RTE_INTR_MODE_NONE; }
...
int rte_vfio_is_enabled(__rte_unused const char *modname) { return 0; }
...
int rte_vfio_get_group_num(__rte_unused const char *sysfs_base,
                           __rte_unused const char *dev_addr,
...
int rte_vfio_get_group_fd(__rte_unused int iommu_group_num) {
  rte_errno = ENOTSUP;
  return -1;
}
...
int rte_vfio_container_destroy(__rte_unused int container_fd) {
  rte_errno = ENOTSUP;
  return -1;
}

int rte_vfio_container_group_bind(__rte_unused int container_fd,
                                  __rte_unused int iommu_group_num) {
  rte_errno = ENOTSUP;
  return -1;
}

int rte_vfio_container_group_unbind(__rte_unused int container_fd,
                                    __rte_unused int iommu_group_num) {
  rte_errno = ENOTSUP;
  return -1;
}

int rte_vfio_container_dma_map(__rte_unused int container_fd,
...
int rte_vfio_container_dma_unmap(__rte_unused int container_fd,
                                 __rte_unused uint64_t vaddr,
                                 __rte_unused uint64_t iova,
                                 __rte_unused uint64_t len) {
  rte_errno = ENOTSUP;
