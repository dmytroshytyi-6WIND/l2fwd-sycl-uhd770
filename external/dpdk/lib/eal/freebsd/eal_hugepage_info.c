...
int eal_hugepage_info_init(void) {
  ...
  error = sysctlbyname("hw.contigmem.num_buffers", &num_buffers, &sysctl_size,
                       NULL, 0);
  ...
  error = sysctlbyname("hw.contigmem.buffer_size", &buffer_size, &sysctl_size,
                       NULL, 0);
  ...
  if (flock(fd, LOCK_EX | LOCK_NB) < 0) {
    RTE_LOG(ERR, EAL,
            "could not lock memory. Is another DPDK process running?\n");
    return -1;
  }
