...
void rte_eal_alarm_cleanup(void) { rte_intr_instance_free(intr_handle); }
...
static inline int timespec_cmp(const struct timespec *now,
...
static inline uint64_t diff_ns(struct timespec *now, struct timespec *at) {
...
static int unregister_current_callback(void) {
  struct alarm_entry *ap;
  int ret = 0;
  ...
  if (!LIST_EMPTY(&alarm_list)) {
    ...
    do {
      ret = rte_intr_callback_unregister(intr_handle, eal_alarm_callback,
                                         &ap->time);
    } while (ret == -EAGAIN);
  }
...
static int register_first_callback(void) {
  struct alarm_entry *ap;
  int ret = 0;
  ...
  if (!LIST_EMPTY(&alarm_list)) {
    ...
    ret =
        rte_intr_callback_register(intr_handle, eal_alarm_callback, &ap->time);
  }
  return ret;
}

static void eal_alarm_callback(void *arg __rte_unused) {
  struct timespec now;
  struct alarm_entry *ap;
