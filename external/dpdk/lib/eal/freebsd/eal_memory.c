...
int rte_eal_hugepage_init(void) {
  ...
  for (i = 0; i < internal_conf->num_hugepage_sizes; i++) {
    ...
    for (j = 0; j < n_pages; j++) {
       ...
       * the previous one.
       */
      snprintf(physaddr_str, sizeof(physaddr_str), "hw.contigmem.physaddr.%d",
               j);
      error = sysctlbyname(physaddr_str, &physaddr, &sysctl_size, NULL, 0);
      if (error < 0) {
        RTE_LOG(ERR, EAL,
                "Failed to get physical addr for buffer %u "
                "from %s\n",
                j, hpi->hugedir);
        return -1;
      }
      ...
      for (msl_idx = 0; msl_idx < RTE_MAX_MEMSEG_LISTS; msl_idx++) {
        ...
        ms_idx = rte_fbarray_find_next_n_free(arr, 0, 1 + (need_hole ? 1 : 0));
      ...
      if (msl_idx == RTE_MAX_MEMSEG_LISTS) {
        RTE_LOG(ERR, EAL,
                "Could not find space for memseg. Please increase %s and/or %s "
                "in configuration.\n",
      ...
      addr = RTE_PTR_ADD(msl->base_va, (size_t)msl->page_sz * ms_idx);

      /* address is already mapped in memseg list, so using
       * MAP_FIXED here is safe.
       */
      addr = mmap(addr, page_sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED,
      ...
      RTE_LOG(INFO, EAL,
              "Mapped memory segment %u @ %p: physaddr:0x%" PRIx64
              ", len %zu\n",
  ...
  if (total_mem < internal_conf->memory) {
    RTE_LOG(ERR, EAL,
            "Couldn't reserve requested memory, "
            "requested: %" PRIu64 "M "
            "available: %" PRIu64 "M\n",
...
static int attach_segment(const struct rte_memseg_list *msl,
  ...
  addr = mmap(ms->addr, ms->len, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED,
...
static int memseg_list_alloc(struct rte_memseg_list *msl) {
  int flags = 0;
...
static int memseg_primary_init(void) {
  ...
  for (hpi_idx = 0; hpi_idx < (int)internal_conf->num_hugepage_sizes;
    ...
    max_type_mem =
        RTE_MIN(max_mem - total_mem, (uint64_t)RTE_MAX_MEM_MB_PER_TYPE << 20);
    ...
    type_msl_idx = 0;
    while (total_type_mem < max_type_mem && total_segs < max_segs) {
      ...
      if (eal_memseg_list_init(msl, hugepage_sz, n_segs, 0, type_msl_idx,
                               false))
        return -1;
...
int rte_eal_memseg_init(void) {
  return rte_eal_process_type() == RTE_PROC_PRIMARY ? memseg_primary_init()
