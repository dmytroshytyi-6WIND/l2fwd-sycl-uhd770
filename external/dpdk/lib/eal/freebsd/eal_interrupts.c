...
static int intr_source_to_kevent(const struct rte_intr_handle *ih,
                                 struct kevent *ke) {
...
static void eal_intr_process_interrupts(struct kevent *events, int nfds) {
...
static void *eal_intr_thread_main(void *arg __rte_unused) {
  ...
  for (;;) {
    ...
    if (nfds < 0) {
      if (errno == EINTR)
        continue;
      RTE_LOG(ERR, EAL, "kevent returns with fail\n");
      break;
    }
