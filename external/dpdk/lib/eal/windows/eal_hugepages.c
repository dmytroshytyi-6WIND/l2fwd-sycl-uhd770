...
static int hugepage_claim_privilege(void) {
...
static int hugepage_info_init(void) {
  ...
  for (socket_id = 0; socket_id < rte_socket_count(); socket_id++) {
    ...
    if (!GetNumaAvailableMemoryNodeEx(numa_node, &bytes)) {
      RTE_LOG_WIN32_ERR("GetNumaAvailableMemoryNodeEx(%u)", numa_node);
      continue;
    }

    hpi->num_pages[socket_id] = bytes / hpi->hugepage_sz;
    RTE_LOG(DEBUG, EAL, "Found %u hugepages of %zu bytes on socket %u\n",
...
int eal_hugepage_info_init(void) {
  if (hugepage_claim_privilege() < 0) {
    RTE_LOG(ERR, EAL,
            "Cannot claim hugepage privilege\n"
