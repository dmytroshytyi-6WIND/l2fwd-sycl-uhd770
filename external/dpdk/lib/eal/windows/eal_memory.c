 ...
 */
typedef PVOID (*VirtualAlloc2_type)(HANDLE Process, PVOID BaseAddress,
...
int eal_mem_win32api_init(void) {
  ...
  VirtualAlloc2_ptr =
      (VirtualAlloc2_type)((void *)GetProcAddress(library, function));
  if (VirtualAlloc2_ptr == NULL) {
...
int eal_mem_virt2iova_init(void) {
  ...
  list = SetupDiGetClassDevs(&GUID_DEVINTERFACE_VIRT2PHYS, NULL, NULL,
  ...
  if (!SetupDiEnumDeviceInterfaces(list, NULL, &GUID_DEVINTERFACE_VIRT2PHYS, 0,
                                   &ifdata)) {
    RTE_LOG_WIN32_ERR("SetupDiEnumDeviceInterfaces()");
    goto exit;
  }

  if (!SetupDiGetDeviceInterfaceDetail(list, &ifdata, NULL, 0, &detail_size,
                                       NULL)) {
    if (GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
      RTE_LOG_WIN32_ERR("SetupDiGetDeviceInterfaceDetail(probe)");
      goto exit;
    }
  }
  ...
  if (detail == NULL) {
    RTE_LOG(ERR, EAL,
            "Cannot allocate virt2phys "
            "device interface detail data\n");
    goto exit;
  }

  detail->cbSize = sizeof(*detail);
  if (!SetupDiGetDeviceInterfaceDetail(list, &ifdata, detail, detail_size, NULL,
                                       NULL)) {
  ...
  virt2phys_device =
      CreateFile(detail->DevicePath, 0, 0, NULL, OPEN_EXISTING, 0, NULL);
...
phys_addr_t rte_mem_virt2phy(const void *virt) {
  ...
  if (!DeviceIoControl(virt2phys_device, IOCTL_VIRT2PHYS_TRANSLATE, &virt,
                       sizeof(virt), &phys, sizeof(phys), &bytes_returned,
                       NULL)) {
...
static void set_errno_from_win32_alloc_error(DWORD code) {
  switch (code) {
  case ERROR_SUCCESS:
    rte_errno = 0;
    break;
...
void *eal_mem_commit(void *requested_addr, size_t size, int socket_id) {
  ...
  if (requested_addr != NULL) {
    ...
    if (VirtualQueryEx(process, requested_addr, &info, sizeof(info)) !=
        sizeof(info)) {
    ...
    if ((info.RegionSize > size) &&
        !VirtualFreeEx(process, requested_addr, size, flags)) {
      RTE_LOG_WIN32_ERR("VirtualFreeEx(%p, %zu, preserve placeholder)",
                        requested_addr, size);
      return NULL;
    }
    ...
    if (!VirtualFreeEx(process, requested_addr, 0, MEM_RELEASE)) {
      RTE_LOG_WIN32_ERR("VirtualFreeEx(%p, 0, release)", requested_addr);
      return NULL;
    }
  }
  ...
  flags = MEM_RESERVE | MEM_COMMIT | MEM_LARGE_PAGES;
  addr = VirtualAlloc2_ptr(process, requested_addr, size, flags, PAGE_READWRITE,
                           &param, param_count);
  if (addr == NULL) {
...
int eal_mem_decommit(void *addr, size_t size) {
  ...
  stub = VirtualAlloc2_ptr(process, addr, size, flags, PAGE_NOACCESS, NULL, 0);
  if (stub == NULL) {
 ...
 */
static int mem_free(void *addr, size_t size, bool reserved) {
  MEMORY_BASIC_INFORMATION info;
  HANDLE process;

  process = GetCurrentProcess();

  if (VirtualQueryEx(process, addr, &info, sizeof(info)) != sizeof(info)) {
  ...
  if (!VirtualFreeEx(process, addr, size,
    ...
    RTE_LOG_WIN32_ERR("VirtualFreeEx(%p, %zu, preserve placeholder)", addr,
                      size);
    return -1;
  }
...
void eal_mem_free(void *virt, size_t size) { mem_free(virt, size, false); }
...
void *rte_mem_map(void *requested_addr, size_t size, int prot, int flags,
  ...
  mapping_handle =
      CreateFileMapping(file_handle, NULL, sys_prot, size_high, size_low, NULL);
  ...
  virt = MapViewOfFileEx(mapping_handle, sys_access, offset_high, offset_low,
                         size, requested_addr);
  if (!virt) {
...
static int eal_nohuge_init(void) {
  ...
  addr = VirtualAlloc(NULL, mem_sz, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
  if (addr == NULL) {
...
int rte_eal_hugepage_init(void) {
  ...
  return internal_conf->no_hugetlbfs ? eal_nohuge_init()
