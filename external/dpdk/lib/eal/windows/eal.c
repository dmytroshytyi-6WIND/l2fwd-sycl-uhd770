...
enum rte_proc_type_t eal_proc_type_detect(void) {
   ...
   */
  errno_t err = _sopen_s(&mem_cfg_fd, pathname, _O_RDWR, _SH_DENYNO,
                         _S_IREAD | _S_IWRITE);
  if (err == 0) {
...
bool rte_mp_disable(void) { return true; }
...
static int sync_func(void *arg __rte_unused) { return 0; }
...
int rte_eal_init(int argc, char **argv) {
  ...
  if (internal_conf->in_memory == 0) {
    RTE_LOG(WARNING, EAL,
            "Multi-process support is requested, "
            "but not available.\n");
  ...
  if (eal_mem_virt2iova_init() < 0) {
    ...
    RTE_LOG(DEBUG, EAL,
            "Cannot access virt2phys driver, "
  ...
  RTE_LCORE_FOREACH_WORKER(i) {
     ...
     * and children
     */
    if (_pipe(lcore_config[i].pipe_main2worker, sizeof(char), _O_BINARY) < 0)
      rte_panic("Cannot create pipe\n");
    if (_pipe(lcore_config[i].pipe_worker2main, sizeof(char), _O_BINARY) < 0)
      rte_panic("Cannot create pipe\n");
...
int rte_firmware_read(__rte_unused const char *name, __rte_unused void **buf,
