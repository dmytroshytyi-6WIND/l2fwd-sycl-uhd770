...
static void __rte_format_printf(1, 2) log_early(const char *format, ...) {
  va_list va;
...
int eal_create_cpu_map(void) {
  ...
  infos_size = 0;
  if (!GetLogicalProcessorInformationEx(RelationNumaNode, NULL, &infos_size)) {
  ...
  if (!GetLogicalProcessorInformationEx(RelationNumaNode, infos, &infos_size)) {
  ...
  while ((uint8_t *)info - (uint8_t *)infos < infos_size) {
    ...
    info = (SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX *)((uint8_t *)info +
                                                       info->Size);
  }
