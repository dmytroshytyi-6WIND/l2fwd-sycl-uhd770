...
static void eal_intr_process(const OVERLAPPED_ENTRY *event) {
  RTE_SET_USED(event);
}

static int eal_intr_thread_handle_init(void) {
...
static void *eal_intr_thread_main(LPVOID arg __rte_unused) {
  bool finished = false;
  ...
  while (!finished) {
    ...
    result =
        GetQueuedCompletionStatusEx(intr_iocp, events, RTE_DIM(events),
                                    &event_count, INFINITE, /* no timeout */
...
int rte_intr_rx_ctl(__rte_unused struct rte_intr_handle *intr_handle,
...
int eal_intr_thread_schedule(void (*func)(void *arg), void *arg) {
  if (!QueueUserAPC((PAPCFUNC)(ULONG_PTR)func, intr_thread_handle,
                    (ULONG_PTR)arg)) {
...
void eal_intr_thread_cancel(void) {
  if (!PostQueuedCompletionStatus(intr_iocp, 0, IOCP_KEY_SHUTDOWN, NULL)) {
...
int rte_intr_callback_register(
...
int rte_intr_callback_unregister(
    __rte_unused const struct rte_intr_handle *intr_handle,
    __rte_unused rte_intr_callback_fn cb_fn, __rte_unused void *cb_arg) {
  return 0;
}
...
int rte_intr_enable(__rte_unused const struct rte_intr_handle *intr_handle) {
  return -ENOTSUP;
}

int rte_intr_ack(__rte_unused const struct rte_intr_handle *intr_handle) {
  return -ENOTSUP;
}

int rte_intr_disable(__rte_unused const struct rte_intr_handle *intr_handle) {
  return -ENOTSUP;
}
...
int rte_intr_tls_epfd(void) { return -ENOTSUP; }
