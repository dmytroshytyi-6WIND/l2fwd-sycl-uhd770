...
void rte_dump_stack(void) {
  ...
  frame_num = RtlCaptureStackBackTrace(0, BACKTRACE_SIZE, stack_trace, NULL);
  ...
  while (frame_num > 0) {
    ...
    if (!ret) {
      ...
      if (error_code == ERROR_INVALID_ADDRESS) {
        ...
        rte_log(RTE_LOG_ERR, RTE_LOGTYPE_EAL, "%d: [<missing_symbols>]\n",
                frame_num--);
        continue;
      } else {
    ...
    rte_log(RTE_LOG_ERR, RTE_LOGTYPE_EAL, "%d: [%s (%s+0x%0llx)[0x%0llX]]\n",
            frame_num, error_code ? "<unknown>" : line.FileName,
