...
static int alloc_seg(struct rte_memseg *ms, void *requested_addr, int socket_id,
  ...
  if (ms->len > 0) {
    ...
    if ((ms->addr == requested_addr) && (ms->socket_id == socket_id) &&
    ...
    RTE_LOG(DEBUG, EAL,
            "Attempted to reallocate segment %p "
            "(size %zu) on socket %d",
            ms->addr, ms->len, ms->socket_id);
    return -1;
  }
  ...
  if (requested_addr == NULL) {
    ...
    if ((addr != NULL) && (addr != requested_addr)) {
      RTE_LOG(CRIT, EAL,
              "Address %p occupied by an alien "
    ...
    if (addr == NULL) {
      RTE_LOG(DEBUG, EAL,
              "Cannot commit reserved memory %p "
              "(size %zu) on socket %d\n",
  ...
  *(volatile int *)addr = *(volatile int *)addr;
  ...
  if (iova == RTE_BAD_IOVA) {
    RTE_LOG(DEBUG, EAL, "Cannot get IOVA of allocated segment\n");
    goto error;
  }
  ...
  if (page->Node != numa_node) {
    RTE_LOG(DEBUG, EAL, "NUMA node hint %u (socket %d) not respected, got %u\n",
...
static int free_seg(struct rte_memseg *ms) {
...
static int alloc_seg_walk(const struct rte_memseg_list *msl, void *arg) {
  ...
  for (i = 0; i < need; i++, cur_idx++) {
    ...
    if (alloc_seg(cur, map_addr, wa->socket, wa->hi)) {
      RTE_LOG(DEBUG, EAL,
              "attempted to allocate %i segments, "
