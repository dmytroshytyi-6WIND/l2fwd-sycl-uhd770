...
int rte_eal_remote_launch(lcore_function_t *f, void *arg,
   ...
   */
  if (__atomic_load_n(&lcore_config[worker_id].state, __ATOMIC_ACQUIRE) != WAIT)
    return -EBUSY;
...
void *eal_thread_loop(void *arg __rte_unused) {
  char c;
  int n, ret;
  ...
  while (1) {
     ...
     * since 'state' variable is used as the guard variable.
     */
    __atomic_store_n(&lcore_config[lcore_id].state, RUNNING, __ATOMIC_RELEASE);
     ...
     */
    while ((f = __atomic_load_n(&lcore_config[lcore_id].f, __ATOMIC_ACQUIRE)) ==
           NULL)
      rte_pause();
     ...
     */
    __atomic_store_n(&lcore_config[lcore_id].state, WAIT, __ATOMIC_RELEASE);
  }
}
...
int eal_thread_create(pthread_t *thread) {
  ...
  th = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)(ULONG_PTR)eal_thread_loop,
...
int rte_sys_gettid(void) { return GetCurrentThreadId(); }

int rte_thread_setname(__rte_unused pthread_t id,
