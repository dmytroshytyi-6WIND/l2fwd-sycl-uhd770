...
enum alarm_state { ALARM_ARMED, ALARM_TRIGGERED, ALARM_CANCELLED };
...
static void alarm_remove_unsafe(struct alarm_entry *ap) {
  LIST_REMOVE(ap, next);
  CloseHandle(ap->timer);
  free(ap);
}

static void alarm_callback(void *arg, DWORD low __rte_unused,
                           DWORD high __rte_unused) {
  struct alarm_entry *ap = arg;
  unsigned int state = ALARM_ARMED;

  if (!atomic_compare_exchange_strong(&ap->state, &state, ALARM_TRIGGERED))
    return;
...
static int alarm_set(struct alarm_entry *entry, LARGE_INTEGER deadline) {
  BOOL ret = SetWaitableTimer(entry->timer, &deadline, 0, alarm_callback, entry,
                              FALSE);
  if (!ret) {
...
static void alarm_task_exec(void *arg) {
...
static bool alarm_matches(const struct alarm_entry *ap,
...
int rte_eal_alarm_cancel(rte_eal_alarm_callback cb_fn, void *cb_arg) {
  ...
  do {
    ...
    LIST_FOREACH(ap, &alarm_list, next) {
      ...
      state = ALARM_ARMED;
      if (atomic_compare_exchange_strong(&ap->state, &state, ALARM_CANCELLED)) {
        alarm_remove_unsafe(ap);
        removed++;
...
static void intr_thread_entry(void *arg) {
