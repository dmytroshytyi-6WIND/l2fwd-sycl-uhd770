...
static __rte_always_inline unsigned int rte_xbegin(void) {
...
static __rte_always_inline void rte_xend(void) {
  asm volatile(".byte 0x0f,0x01,0xd5" ::: "memory");
}

/* not an inline function to workaround a clang bug with -O0 */
#define rte_xabort(status)                                                     \
  do {                                                                         \
    asm volatile(".byte 0xc6,0xf8,%P0" ::"i"(status) : "memory");              \
  } while (0)

static __rte_always_inline int rte_xtest(void) {
  unsigned char out;
