...
#ifndef __PIC__
  asm volatile(MPLOCKED "cmpxchg8b (%[dst]);"
                        "setz %[res];"
...
static inline uint64_t rte_atomic64_exchange(volatile uint64_t *dest,
                                             uint64_t val) {
  uint64_t old;
...
static inline void rte_atomic64_add(rte_atomic64_t *v, int64_t inc) {
  ...
  while (success == 0) {
    tmp = v->cnt;
    success = rte_atomic64_cmpset((volatile uint64_t *)&v->cnt, tmp, tmp + inc);
  }
}
...
static inline void rte_atomic64_sub(rte_atomic64_t *v, int64_t dec) {
  ...
  while (success == 0) {
    tmp = v->cnt;
    success = rte_atomic64_cmpset((volatile uint64_t *)&v->cnt, tmp, tmp - dec);
  }
}
