...
static inline void __umwait_wakeup(volatile void *addr) {
  uint64_t val;
...
static inline int __check_val_size(const uint8_t sz) {
  switch (sz) {
...
int rte_power_monitor(const struct rte_power_monitor_cond *pmc,
  ...
  asm volatile(".byte 0xf3, 0x0f, 0xae, 0xf7;" : : "D"(pmc->addr));
