 ...
 */
static int huge_wrap_sigsetjmp(void) { return sigsetjmp(huge_jmpenv, 1); }

static struct sigaction huge_action_old;
static int huge_need_recover;

static void huge_register_sigbus(void) {
  sigset_t mask;
...
static void huge_recover_sigbus(void) {
...
static void prepare_numa(int *oldpolicy, struct bitmask *oldmask,
                         int socket_id) {
  ...
  RTE_LOG(DEBUG, EAL, "Setting policy MPOL_PREFERRED for socket %d\n",
          socket_id);
  numa_set_preferred(socket_id);
}

static void restore_numa(int *oldpolicy, struct bitmask *oldmask) {
  RTE_LOG(DEBUG, EAL, "Restoring previous memory policy: %d\n", *oldpolicy);
...
static int get_seg_memfd(struct hugepage_info *hi __rte_unused,
  ...
  if (internal_conf->single_file_segments) {
    ...
    if (fd < 0) {
      snprintf(segname, sizeof(segname), "seg_%i-%i", list_idx, seg_idx);
      fd = memfd_create(segname, flags);
      if (fd < 0) {
        RTE_LOG(DEBUG, EAL, "%s(): memfd create failed: %s\n", __func__,
                strerror(errno));
...
static int get_seg_fd(char *path, int buflen, struct hugepage_info *hi,
  ...
  if (!internal_conf->single_file_segments &&
      ...
      rte_eal_process_type() == RTE_PROC_PRIMARY && ret == 0) {
    /* coverity[toctou] */
    if (unlink(path) < 0) {
...
static int resize_hugefile_in_memory(int fd, uint64_t fa_offset,
                                     uint64_t page_sz, bool grow) {
  int flags = grow ? 0 : FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE;
  int ret;
...
static int resize_hugefile_in_filesystem(int fd, uint64_t fa_offset,
                                         uint64_t page_sz, bool grow,
                                         bool *dirty) {
  const struct internal_config *internal_conf =
      eal_get_internal_configuration();
  ...
  do {
    if (fallocate_supported == 0) {
      ...
      if (!grow) {
        RTE_LOG(DEBUG, EAL,
                "%s(): fallocate not supported, not freeing page back to the "
                "system\n",
                __func__);
        return -1;
      }
      ...
      if (new_size > cur_size && ftruncate(fd, new_size) < 0) {
      ...
      if (ret < 0) {
        if (fallocate_supported == -1 && errno == ENOTSUP) {
          RTE_LOG(ERR, EAL,
                  "%s(): fallocate() not supported, hugepage deallocation will "
                  "be disabled\n",
                  __func__);
          again = true;
...
static void close_hugefile(int fd, char *path, int list_idx) {
  const struct internal_config *internal_conf =
      eal_get_internal_configuration();
  /*
   ...
   */
  if (!internal_conf->in_memory && rte_eal_process_type() == RTE_PROC_PRIMARY &&
      unlink(path))
...
static int resize_hugefile(int fd, uint64_t fa_offset, uint64_t page_sz,
  ...
  if (internal_conf->in_memory) {
    if (dirty != NULL)
      *dirty = false;
    return resize_hugefile_in_memory(fd, fa_offset, page_sz, grow);
  }

  return resize_hugefile_in_filesystem(fd, fa_offset, page_sz, grow, dirty);
}

static int alloc_seg(struct rte_memseg *ms, void *addr, int socket_id,
  ...
  if (internal_conf->in_memory && !anonymous_hugepages_supported) {
    RTE_LOG(ERR, EAL,
            "Anonymous hugepages not supported, in-memory mode cannot allocate "
            "memory\n");
    return -1;
  }
  ...
  if (internal_conf->in_memory && !memfd_create_supported) {
    const int in_memory_flags =
        MAP_HUGETLB | MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS;
    int pagesz_flag;
  ...
  *(volatile int *)addr = *(volatile int *)addr;
  ...
  if (check_numa()) {
    ret =
        get_mempolicy(&cur_socket_id, NULL, 0, addr, MPOL_F_NODE | MPOL_F_ADDR);
    if (ret < 0) {
...
resized:
  ...
  if (internal_conf->single_file_segments) {
    ...
    if (!internal_conf->hugepage_file.unlink_before_mapping &&
        internal_conf->in_memory == 0 && lock(fd, LOCK_EX) == 1)
      unlink(path);
    close(fd);
...
static int free_seg(struct rte_memseg *ms, struct hugepage_info *hi,
                    unsigned int list_idx, unsigned int seg_idx) {
  uint64_t map_offset;
  char path[PATH_MAX];
  int fd, ret = 0;
...
static int alloc_seg_walk(const struct rte_memseg_list *msl, void *arg) {
  ...
  if (wa->exact) {
     ...
     */
    cur_idx = rte_fbarray_find_biggest_free(&cur_msl->memseg_arr, 0);
    if (cur_idx < 0)
      return 0;
    start_idx = cur_idx;
     ...
     */
    cur_len = rte_fbarray_find_contig_free(&cur_msl->memseg_arr, cur_idx);
  ...
  for (i = 0; i < need; i++, cur_idx++) {
    ...
    if (alloc_seg(cur, map_addr, wa->socket, wa->hi, msl_idx, cur_idx)) {
      RTE_LOG(DEBUG, EAL,
...
static int free_seg_walk(const struct rte_memseg_list *msl, void *arg) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
...
int eal_memalloc_alloc_seg_bulk(struct rte_memseg **ms, int n_segs,
  ...
  for (i = 0; i < (int)RTE_DIM(internal_conf->hugepage_info); i++) {
    if (page_sz == internal_conf->hugepage_info[i].hugepage_sz) {
...
int eal_memalloc_free_seg_bulk(struct rte_memseg **ms, int n_segs) {
  ...
  for (seg = 0; seg < n_segs; seg++) {
    ...
    /* memalloc is locked, so it's safe to use thread-unsafe version
     */
    walk_res = rte_memseg_list_walk_thread_unsafe(free_seg_walk, &wa);
    if (walk_res == 1)
      continue;
    if (walk_res == 0)
...
static int sync_chunk(struct rte_memseg_list *primary_msl,
                      struct rte_memseg_list *local_msl,
                      struct hugepage_info *hi, unsigned int msl_idx, bool used,
                      int start, int end) {
  ...
  diff_len = used ? rte_fbarray_find_contig_free(l_arr, start)
  ...
  if (!used) {
    ...
    eal_memalloc_mem_event_notify(RTE_MEM_EVENT_FREE, start_va, len);
  }
  ...
  for (i = 0; i < diff_len; i++) {
    ...
    if (used) {
      ret = alloc_seg(l_ms, p_ms->addr, p_ms->socket_id, hi, msl_idx, seg_idx);
      if (ret < 0)
        return -1;
  ...
  if (used) {
    ...
    eal_memalloc_mem_event_notify(RTE_MEM_EVENT_ALLOC, start_va, len);
  }

  /* calculate how much we can advance until next chunk */
  diff_len = used ? rte_fbarray_find_contig_used(l_arr, start)
...
static int sync_status(struct rte_memseg_list *primary_msl,
                       struct rte_memseg_list *local_msl,
                       struct hugepage_info *hi, unsigned int msl_idx,
                       bool used) {
  struct rte_fbarray *l_arr, *p_arr;
  ...
  while (p_idx >= 0) {
    ...
    if (used) {
      p_chunk_len = rte_fbarray_find_contig_used(p_arr, p_idx);
      l_chunk_len = rte_fbarray_find_contig_used(l_arr, p_idx);
    } else {
      p_chunk_len = rte_fbarray_find_contig_free(p_arr, p_idx);
      l_chunk_len = rte_fbarray_find_contig_free(l_arr, p_idx);
    }
    ...
    do {
      ret = sync_chunk(primary_msl, local_msl, hi, msl_idx, used, start, end);
      start += ret;
  ...
  next_chunk:
    ...
    if (used) {
      p_idx = rte_fbarray_find_next_used(p_arr, next_chunk_search_idx);
    } else {
      p_idx = rte_fbarray_find_next_free(p_arr, next_chunk_search_idx);
    }
  }
  return 0;
}

static int sync_existing(struct rte_memseg_list *primary_msl,
                         struct rte_memseg_list *local_msl,
                         struct hugepage_info *hi, unsigned int msl_idx) {
  int ret, dir_fd;
...
static int sync_walk(const struct rte_memseg_list *msl,
  ...
  for (i = 0; i < RTE_DIM(internal_conf->hugepage_info); i++) {
    uint64_t cur_sz = internal_conf->hugepage_info[i].hugepage_sz;
...
static int secondary_msl_create_walk(const struct rte_memseg_list *msl,
                                     void *arg __rte_unused) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  struct rte_memseg_list *primary_msl, *local_msl;
  char name[PATH_MAX];
  ...
  snprintf(name, RTE_FBARRAY_NAME_LEN, "%s_%i", primary_msl->memseg_arr.name,
           getpid());
...
static int secondary_msl_destroy_walk(const struct rte_memseg_list *msl,
                                      void *arg __rte_unused) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  struct rte_memseg_list *local_msl;
  int msl_idx, ret;
...
static int destroy_list(int list_idx) {
  const struct internal_config *internal_conf =
      eal_get_internal_configuration();
...
static int fd_list_create_walk(const struct rte_memseg_list *msl,
                               void *arg __rte_unused) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  unsigned int len;
  int msl_idx;
...
static int fd_list_destroy_walk(const struct rte_memseg_list *msl,
                                void *arg __rte_unused) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  int msl_idx;
...
int eal_memalloc_set_seg_fd(int list_idx, int seg_idx, int fd) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  const struct internal_config *internal_conf =
      eal_get_internal_configuration();
...
int eal_memalloc_cleanup(void) {
  ...
  if (rte_memseg_list_walk_thread_unsafe(secondary_msl_destroy_walk, NULL))
    return -1;
...
int eal_memalloc_init(void) {
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY)
    if (rte_memseg_list_walk(secondary_msl_create_walk, NULL) < 0)
      return -1;
  if (rte_eal_process_type() == RTE_PROC_PRIMARY && internal_conf->in_memory) {
    ...
    if (mfd_res == 1)
      ...
      RTE_LOG(INFO, EAL,
              "Using memfd is not supported, falling back to anonymous "
              "hugepages\n");
     ...
     * test if we do.
     */
    if (internal_conf->single_file_segments && mfd_res != 1) {
      RTE_LOG(
          ERR, EAL,
    ...
    if (internal_conf->hugepage_file.unlink_before_mapping &&
        !internal_conf->hugepage_file.unlink_existing) {
      RTE_LOG(ERR, EAL,
              "Unlinking existing hugepage files is prohibited, cannot unlink "
              "them before mapping.\n");
      return -1;
    }
  }
