...
static void sigbus_action_recover(void) {
...
static int dev_uev_socket_fd_create(void) {
...
static int dev_uev_parse(const char *buf, struct rte_dev_event *event,
                         int length) {
...
static void dev_delayed_unregister(void *param) {
...
static void dev_uev_handler(__rte_unused void *param) {
  ...
  ret = recv(rte_intr_fd_get(intr_handle), buf, EAL_UEV_MSG_LEN, MSG_DONTWAIT);
...
int rte_dev_event_monitor_stop(void) {
  ...
  ret = rte_intr_callback_unregister(intr_handle, dev_uev_handler, (void *)-1);
  if (ret < 0) {
