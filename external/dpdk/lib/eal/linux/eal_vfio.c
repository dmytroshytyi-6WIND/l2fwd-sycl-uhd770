...
static int is_null_map(const struct user_mem_map *map) {
 ...
 */
static int user_mem_map_cmp(const void *a, const void *b) {
 ...
 */
static int process_maps(struct user_mem_map *src, size_t src_len,
                        struct user_mem_map newmap[2], uint64_t vaddr,
                        uint64_t len) {
  ...
  if (last_off != 0) {
    ...
    struct user_mem_map *last = first_off == 0 ? dst_first : dst_last;
...
static void delete_maps(struct user_mem_maps *user_mem_maps,
                        struct user_mem_map *del_maps, size_t n_del) {
  int i;
  size_t j;
...
static void copy_maps(struct user_mem_maps *user_mem_maps,
                      struct user_mem_map *add_maps, size_t n_add) {
  int i;
  size_t j;
...
static int merge_map(struct user_mem_map *left, struct user_mem_map *right) {
...
static bool addr_is_chunk_aligned(struct user_mem_map *maps, size_t n_maps,
...
static int find_user_mem_maps(struct user_mem_maps *user_mem_maps,
                              uint64_t addr, uint64_t iova, uint64_t len,
                              struct user_mem_map *dst, size_t dst_len) {
...
static void compact_user_maps(struct user_mem_maps *user_mem_maps) {
  int i;
...
static int vfio_open_group_fd(int iommu_group_num) {
  int vfio_group_fd;
  ...
  if (internal_conf->process_type == RTE_PROC_PRIMARY) {
    ...
    snprintf(filename, sizeof(filename), VFIO_GROUP_FMT, iommu_group_num);
    ...
    if (vfio_group_fd < 0) {
      ...
      snprintf(filename, sizeof(filename), VFIO_NOIOMMU_GROUP_FMT,
               iommu_group_num);
      vfio_group_fd = open(filename, O_RDWR);
      if (vfio_group_fd < 0) {
        if (errno != ENOENT) {
...
static struct vfio_config *get_vfio_cfg_by_group_num(int iommu_group_num) {
  ...
  for (i = 0; i < VFIO_MAX_CONTAINERS; i++) {
    ...
    for (j = 0; j < VFIO_MAX_GROUPS; j++) {
      if (vfio_cfg->vfio_groups[j].group_num == iommu_group_num)
        return vfio_cfg;
    }
  }
...
static int vfio_get_group_fd(struct vfio_config *vfio_cfg,
                             int iommu_group_num) {
  int i;
  int vfio_group_fd;
...
static struct vfio_config *get_vfio_cfg_by_group_fd(int vfio_group_fd) {
  struct vfio_config *vfio_cfg;
  int i, j;
...
static struct vfio_config *get_vfio_cfg_by_container_fd(int container_fd) {
  int i;
...
static int get_vfio_group_idx(int vfio_group_fd) {
  struct vfio_config *vfio_cfg;
  int i, j;
...
static void vfio_group_device_get(int vfio_group_fd) {
  struct vfio_config *vfio_cfg;
  int i;
...
static void vfio_group_device_put(int vfio_group_fd) {
  struct vfio_config *vfio_cfg;
  int i;
...
static int vfio_group_device_count(int vfio_group_fd) {
  struct vfio_config *vfio_cfg;
  int i;
...
static void vfio_mem_event_callback(enum rte_mem_event type, const void *addr,
  ...
  if (rte_eal_iova_mode() == RTE_IOVA_VA && msl->external == 0) {
    ...
    for (; cur_len < len; cur_len += page_sz) {
      if (type == RTE_MEM_EVENT_ALLOC)
        vfio_dma_mem_map(default_vfio_cfg, vfio_va, vfio_va, page_sz, 1);
      else
        vfio_dma_mem_map(default_vfio_cfg, vfio_va, vfio_va, page_sz, 0);
      vfio_va += page_sz;
    }
  ...
  while (cur_len < len) {
    ...
    if (type == RTE_MEM_EVENT_ALLOC)
      vfio_dma_mem_map(default_vfio_cfg, ms->addr_64, ms->iova, ms->len, 1);
    else
      vfio_dma_mem_map(default_vfio_cfg, ms->addr_64, ms->iova, ms->len, 0);
  next:
    cur_len += ms->len;
    ++ms;
  }
...
static int vfio_sync_default_container(void) {
  struct rte_mp_msg mp_req, *mp_rep;
  struct rte_mp_reply mp_reply = {0};
  struct timespec ts = {.tv_sec = 5, .tv_nsec = 0};
  struct vfio_mp_param *p = (struct vfio_mp_param *)mp_req.param;
...
int rte_vfio_setup_device(const char *sysfs_base, const char *dev_addr,
  ...
  if (ret == 0) {
    RTE_LOG(NOTICE, EAL, "%s not managed by VFIO driver, skipping\n", dev_addr);
    return 1;
  }
  ...
  if (ret) {
    RTE_LOG(ERR, EAL,
            "%s cannot get VFIO group status, "
            "error %i (%s)\n",
  ...
  if (!(group_status.flags & VFIO_GROUP_FLAGS_CONTAINER_SET)) {
    ...
    if (internal_conf->process_type == RTE_PROC_PRIMARY &&
      ...
      for (i = 0; i < user_mem_maps->n_maps; i++) {
        ...
        ret = t->dma_user_map_func(vfio_container_fd, map->addr, map->iova,
                                   map->len, 1);
        if (ret) {
          RTE_LOG(ERR, EAL,
                  "Couldn't map user memory for DMA: "
                  "va: 0x%" PRIx64 " "
                  "iova: 0x%" PRIx64 " "
                  "len: 0x%" PRIu64 "\n",
    ...
    const struct vfio_iommu_type *t = default_vfio_cfg->vfio_iommu_type;
  ...
  if (!rte_uuid_is_null(vf_token)) {
    ...
    snprintf(dev, sizeof(dev), "%s vf_token=%s", dev_addr, vf_token_str);
...
int rte_vfio_release_device(const char *sysfs_base, const char *dev_addr,
                            int vfio_dev_fd) {
  struct vfio_config *vfio_cfg;
  int vfio_group_fd;
  int iommu_group_num;
...
int rte_vfio_enable(const char *modname) {
  ...
  if (internal_conf->process_type == RTE_PROC_PRIMARY) {
    ...
    default_vfio_cfg->vfio_container_fd = vfio_get_default_container_fd();
  }
...
const struct vfio_iommu_type *vfio_set_iommu_type(int vfio_container_fd) {
  unsigned idx;
  for (idx = 0; idx < RTE_DIM(iommu_types); idx++) {
    ...
    RTE_LOG(DEBUG, EAL,
            "Set IOMMU type %d (%s) failed, error "
            "%i (%s)\n",
            t->type_id, t->name, errno, strerror(errno));
  }
...
int vfio_has_supported_extensions(int vfio_container_fd) {
  ...
  for (idx = 0; idx < RTE_DIM(iommu_types); idx++) {
    ...
    if (ret < 0) {
      RTE_LOG(ERR, EAL,
              "Could not get IOMMU type, error "
              "%i (%s)\n",
  ...
  if (internal_conf->process_type == RTE_PROC_PRIMARY) {
    ...
    if (vfio_container_fd < 0) {
      RTE_LOG(ERR, EAL,
              ...
              "%i (%s)\n",
              VFIO_CONTAINER_PATH, errno, strerror(errno));
      return -1;
    }
...
int rte_vfio_get_group_num(const char *sysfs_base, const char *dev_addr,
  ...
  snprintf(linkname, sizeof(linkname), "%s/%s/iommu_group", sysfs_base,
           dev_addr);
...
static int type1_map_contig(const struct rte_memseg_list *msl,
  ...
  return vfio_type1_dma_mem_map(*vfio_container_fd, ms->addr_64, ms->iova, len,
                                1);
}

static int type1_map(const struct rte_memseg_list *msl,
...
static int vfio_type1_dma_mem_map(int vfio_container_fd, uint64_t vaddr,
                                  uint64_t iova, uint64_t len, int do_map) {
  ...
  if (do_map != 0) {
    ...
    if (ret) {
       ...
       */
      if (errno == EEXIST) {
        RTE_LOG(DEBUG, EAL, "Memory segment is already mapped, skipping");
      } else {
        RTE_LOG(ERR, EAL,
    ...
    if (ret) {
      RTE_LOG(ERR, EAL,
              "Cannot clear DMA remapping, error "
              "%i (%s)\n",
              errno, strerror(errno));
      return -1;
    } else if (dma_unmap.size != len) {
      RTE_LOG(ERR, EAL,
              "Unexpected size %" PRIu64
...
static int vfio_type1_dma_map(int vfio_container_fd) {
  if (rte_eal_iova_mode() == RTE_IOVA_VA) {
    /* with IOVA as VA mode, we can get away with mapping contiguous
     * chunks rather than going page-by-page.
     */
    int ret = rte_memseg_contig_walk(type1_map_contig, &vfio_container_fd);
    if (ret)
      return ret;
...
static int vfio_spapr_dma_do_map(int vfio_container_fd, uint64_t vaddr,
                                 uint64_t iova, uint64_t len, int do_map) {
  struct vfio_iommu_spapr_register_memory reg = {
      .argsz = sizeof(reg), .vaddr = (uintptr_t)vaddr, .size = len, .flags = 0};
  int ret;
  ...
  if (do_map != 0) {
    ...
    if (ret) {
      RTE_LOG(ERR, EAL,
              "Cannot map vaddr for IOMMU, error "
              "%i (%s)\n",
              errno, strerror(errno));
      return -1;
    }
    ...
    if (ret) {
      RTE_LOG(ERR, EAL,
              "Cannot unmap vaddr for IOMMU, error "
              "%i (%s)\n",
              errno, strerror(errno));
      return -1;
    }

    ret = ioctl(vfio_container_fd, VFIO_IOMMU_SPAPR_UNREGISTER_MEMORY, &reg);
    if (ret) {
      RTE_LOG(ERR, EAL,
...
static int vfio_spapr_map_walk(const struct rte_memseg_list *msl,
                               const struct rte_memseg *ms, void *arg) {
  int *vfio_container_fd = arg;
 ...
 */
static int vfio_spapr_size_walk(const struct rte_memseg_list *msl, void *arg) {
 ...
 */
static int find_highest_mem_addr(struct spapr_size_walk_param *param) {
  ...
  if (rte_eal_iova_mode() == RTE_IOVA_PA) {
    ...
    while (getline(&line, &line_len, fd) != -1) {
      ...
      RTE_LOG(DEBUG, EAL,
              "Found system RAM from 0x%" PRIx64 " to 0x%" PRIx64 "\n", start,
              end);
      if (end > max)
        max = end;
    }
    ...
    if (max == 0) {
      RTE_LOG(ERR, EAL,
              "Failed to find valid \"System RAM\" "
              "entry in file %s\n",
              proc_iomem);
      return -1;
    }
    ...
    RTE_LOG(DEBUG, EAL, "Highest VA address in memseg list is 0x%" PRIx64 "\n",
            param->max_va);
 ...
 */
static int spapr_dma_win_size(void) {
  ...
  if (param.is_user_managed)
    RTE_LOG(WARNING, EAL,
            "Detected user managed external memory which may not be managed by "
            "the IOMMU\n");
...
static int vfio_spapr_create_dma_window(int vfio_container_fd) {
  ...
  if (ret) {
    ...
    for (levels = create.levels + 1; ret && levels <= info.ddw.levels;
         levels++) {
  ...
  if (ret) {
    RTE_LOG(ERR, EAL,
            "Cannot create new DMA window, error "
            "%i (%s)\n",
            errno, strerror(errno));
    RTE_LOG(
        ERR, EAL,
  ...
  if (create.start_addr != 0) {
    RTE_LOG(ERR, EAL, "Received unsupported start address 0x%" PRIx64 "\n",
...
static int vfio_spapr_dma_mem_map(int vfio_container_fd, uint64_t vaddr,
                                  uint64_t iova, uint64_t len, int do_map) {
  int ret = 0;
...
static int vfio_spapr_dma_map(int vfio_container_fd) {
...
static int vfio_noiommu_dma_map(int __rte_unused vfio_container_fd) {
  /* No-IOMMU mode does not need DMA mapping */
  return 0;
}

static int vfio_noiommu_dma_mem_map(int __rte_unused vfio_container_fd,
                                    uint64_t __rte_unused vaddr,
                                    uint64_t __rte_unused iova,
                                    uint64_t __rte_unused len,
...
static int vfio_dma_mem_map(struct vfio_config *vfio_cfg, uint64_t vaddr,
  ...
  return t->dma_user_map_func(vfio_cfg->vfio_container_fd, vaddr, iova, len,
                              do_map);
}

static int container_dma_map(struct vfio_config *vfio_cfg, uint64_t vaddr,
...
static int container_dma_unmap(struct vfio_config *vfio_cfg, uint64_t vaddr,
                               uint64_t iova, uint64_t len) {
  ...
  if (!has_partial_unmap) {
    ...
    end_aligned =
        addr_is_chunk_aligned(orig_maps, n_orig, vaddr + len, iova + len);
  ...
  if (vfio_dma_mem_map(vfio_cfg, vaddr, iova, len, 0)) {
    ...
    if (rte_errno != ENODEV && rte_errno != ENOTSUP) {
      ...
      RTE_LOG(DEBUG, EAL,
              "DMA unmapping failed, but removing mappings anyway\n");
    }
  }
...
int rte_vfio_noiommu_is_enabled(void) {
  ...
  if (cnt != 1) {
    RTE_LOG(ERR, EAL,
            "Unable to read from VFIO noiommu file "
            "%i (%s)\n",
            errno, strerror(errno));
    return -1;
  }
...
int rte_vfio_container_dma_map(int container_fd, uint64_t vaddr, uint64_t iova,
                               uint64_t len) {
  struct vfio_config *vfio_cfg;
