 ...
 */
static void *map_shared_memory(const char *filename, const size_t mem_size,
                               int flags) {
  void *retval;
...
static void *open_shared_memory(const char *filename, const size_t mem_size) {
...
static int get_hp_sysfs_value(const char *subdir, const char *file,
                              unsigned long *val) {
  char path[PATH_MAX];

  snprintf(path, sizeof(path), "%s/%s/%s", sys_dir_path, subdir, file);
  return eal_parse_sysfs_value(path, val);
}

/* this function is only called from eal_hugepage_info_init which itself
 * is only called from a primary process */
static uint32_t get_num_hugepages(const char *subdir, size_t sz,
...
static uint32_t get_num_hugepages_on_node(const char *subdir,
                                          unsigned int socket, size_t sz) {
  ...
  snprintf(path, sizeof(path), "%s/%s/%s", socketpath, subdir, nr_hp_file);
...
static int get_hugepage_dir(uint64_t hugepage_sz, char *hugedir, int len) {
  ...
  while (fgets(buf, sizeof(buf), fd)) {
    ...
    if (rte_strsplit(buf, sizeof(buf), splitstr, _FIELDNAME_MAX, split_tok) !=
        _FIELDNAME_MAX) {
  ...
  if (found[0] != '\0') {
    ...
    strlcpy(hugedir,
            internal_conf->hugepage_dir != NULL ? internal_conf->hugepage_dir
                                                : found,
            len);
    return 0;
  }
 ...
 */
static int walk_hugedir(const char *hugedir, walk_hugedir_t *cb,
                        void *user_data) {
  DIR *dir;
...
static void clear_hugedir_cb(const struct walk_hugedir_data *whd) {
...
static void inspect_hugedir_cb(const struct walk_hugedir_data *whd) {
  uint64_t *total_size = whd->user_data;
  struct stat st;

  if (fstat(whd->file_fd, &st) < 0)
    RTE_LOG(DEBUG, EAL, "%s(): stat(\"%s\") failed: %s", __func__,
            whd->file_name, strerror(errno));
  else
 ...
 */
static int inspect_hugedir(const char *hugedir, uint64_t *total_size) {
  return walk_hugedir(hugedir, inspect_hugedir_cb, total_size);
}

static int compare_hpi(const void *a, const void *b) {
...
static void calc_num_pages(struct hugepage_info *hpi, struct dirent *dirent,
                           unsigned int reusable_pages) {
  ...
  if (!internal_conf->legacy_mem && reusable_pages == 0)
    for (i = 0; i < rte_socket_count(); i++) {
      ...
      unsigned int num_pages =
          get_num_hugepages_on_node(dirent->d_name, socket, hpi->hugepage_sz);
  ...
  if (total_pages == 0) {
    hpi->num_pages[0] =
        get_num_hugepages(dirent->d_name, hpi->hugepage_sz, reusable_pages);

#ifndef RTE_ARCH_64
    /* for 32-bit systems, limit number of hugepages to
     * 1GB per page size */
    hpi->num_pages[0] =
        RTE_MIN(hpi->num_pages[0], RTE_PGSIZE_1G / hpi->hugepage_sz);
#endif
  }
}
...
static int hugepage_info_init(void) {
  ...
  if (dir == NULL) {
    RTE_LOG(ERR, EAL, "Cannot open directory %s to read system hugepage info\n",
            sys_dir_path);
    return -1;
  }
  ...
  for (dirent = readdir(dir); dirent != NULL; dirent = readdir(dir)) {
    ...
    if (get_hugepage_dir(hpi->hugepage_sz, hpi->hugedir, sizeof(hpi->hugedir)) <
        0) {
      uint32_t num_pages;

      num_pages = get_num_hugepages(dirent->d_name, hpi->hugepage_sz, 0);
      if (num_pages > 0)
        RTE_LOG(NOTICE, EAL,
