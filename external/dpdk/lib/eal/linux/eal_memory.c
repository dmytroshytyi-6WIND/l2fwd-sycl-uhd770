 ...
 */
static int find_physaddrs(struct hugepage_file *hugepg_tbl,
 ...
 */
static int set_physaddrs(struct hugepage_file *hugepg_tbl,
                         struct hugepage_info *hpi) {
  unsigned int i;
...
static unsigned map_all_hugepages(struct hugepage_file *hugepg_tbl,
  ...
  if (have_numa) {
    ...
    if (get_mempolicy(&oldpolicy, oldmask->maskp, oldmask->size + 1, 0, 0) <
        0) {
      RTE_LOG(ERR, EAL,
  ...
  for (i = 0; i < hpi->num_pages[0]; i++) {
    ...
    hf->file_id = i;
    hf->size = hugepage_sz;
    eal_get_hugefile_path(hf->filepath, sizeof(hf->filepath), hpi->hugedir,
                          hf->file_id);
...
out:
  ...
  if (maxnode) {
    ...
    if (oldpolicy == MPOL_DEFAULT) {
      numa_set_localalloc();
    } else if (set_mempolicy(oldpolicy, oldmask->maskp, oldmask->size + 1) <
               0) {
 ...
 * page.
 */
static int find_numasocket(struct hugepage_file *hugepg_tbl,
                           struct hugepage_info *hpi) {
  int socket_id;
  char *end, *nodestr;
  ...
  snprintf(hugedir_str, sizeof(hugedir_str), "%s/%s", hpi->hugedir,
  ...
  while (fgets(buf, sizeof(buf), f) != NULL) {
    ...
    if (strstr(buf, " huge ") == NULL && strstr(buf, hugedir_str) == NULL)
      continue;
    ...
    for (i = 0; i < hpi->num_pages[0]; i++) {
      ...
      if (hugepg_tbl[i].orig_va == va) {
        ...
        hp_count++;
#ifdef RTE_EAL_NUMA_AWARE_HUGEPAGES
        RTE_LOG(DEBUG, EAL, "Hugepage %s is on socket %d\n",
...
static int cmp_physaddr(const void *a, const void *b) {
...
static int unlink_hugepage_files(struct hugepage_file *hugepg_tbl,
      ...
      for (page = 0; page < nrpages; page++) {
        ...
        if ((hp->size == hpi[size].hugepage_sz) &&
          ...
          if (pages_found == hpi[size].num_pages[socket]) {
            ...
            munmap(hp->orig_va, (size_t)unmap_len);
...
static int remap_segment(struct hugepage_file *hugepages, int seg_start,
                         int seg_end) {
  ...
  for (msl_idx = 0; msl_idx < RTE_MAX_MEMSEG_LISTS; msl_idx++) {
    ...
    empty = arr->count == 0;
    ms_idx = rte_fbarray_find_next_n_free(arr, 0, seg_len + (empty ? 0 : 1));
  ...
  if (msl_idx == RTE_MAX_MEMSEG_LISTS) {
    RTE_LOG(ERR, EAL,
            "Could not find space for memseg. Please increase %s and/or %s in "
            "configuration.\n",
            RTE_STR(RTE_MAX_MEMSEG_PER_TYPE), RTE_STR(RTE_MAX_MEM_MB_PER_TYPE));
    return -1;
  }

#ifdef RTE_ARCH_PPC_64
  /* for PPC64 we go through the list backwards */
  for (cur_page = seg_end - 1; cur_page >= seg_start; cur_page--, ms_idx++) {
#else
...
static uint64_t get_mem_amount(uint64_t page_sz, uint64_t max_mem) {
...
static int memseg_list_free(struct rte_memseg_list *msl) {
 ...
 */
static int __rte_unused prealloc_segments(struct hugepage_file *hugepages,
                                          int n_pages) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
   ...
   */
  for (hpi_idx = 0; hpi_idx < internal_conf->num_hugepage_sizes; hpi_idx++) {
    uint64_t page_sz = internal_conf->hugepage_info[hpi_idx].hugepage_sz;
    ...
    for (i = 0; i < rte_socket_count(); i++) {
      ...
      for (cur_page = 0; cur_page < n_pages; cur_page++) {
        ...
        if (new_memseg) {
          ...
          if (seg_start_page != -1) {
            end_seg = 1;
            prev_seg_start_page = seg_start_page;
          }
        ...
        if (end_seg) {
          if (prev_seg_start_page != -1) {
            ...
            n_contig_segs++;
            n_segs += cur_page - prev_seg_start_page;
      ...
      if (eal_memseg_list_init(msl, page_sz, n_segs, socket, msl_idx, true) < 0)
        return -1;
 ...
 */
static int remap_needed_hugepages(struct hugepage_file *hugepages,
                                  int n_pages) {
  ...
  for (cur_page = 0; cur_page < n_pages; cur_page++) {
    ...
    if (new_memseg) {
      ...
      if (cur_page != 0) {
        ret = remap_segment(hugepages, seg_start_page, cur_page);
        if (ret != 0)
          return -1;
      }
...
static int eal_legacy_hugepage_init(void) {
  ...
  if (internal_conf->no_hugetlbfs) {
    ...
    if (eal_memseg_list_init_named(msl, "nohugemem", page_sz, n_segs, 0,
                                   true)) {
      return -1;
    }
  ...
  for (i = 0; i < (int)internal_conf->num_hugepage_sizes; i++) {
    ...
    if (pages_new < pages_old) {
      RTE_LOG(DEBUG, EAL, "%d not %d hugepages of size %u MB allocated\n",
    ...
    if (rte_eal_using_phys_addrs() && rte_eal_iova_mode() != RTE_IOVA_VA) {
    ...
    qsort(&tmp_hp[hp_offset], hpi->num_pages[0], sizeof(struct hugepage_file),
          cmp_physaddr);
  ...
  for (i = 0; i < nr_hugefiles; i++) {
    ...
    const int nb_hpsizes =
        RTE_MIN(MAX_HUGEPAGE_SIZES, (int)internal_conf->num_hugepage_sizes);
    for (j = 0; j < nb_hpsizes; j++) {
      if (tmp_hp[i].size == internal_conf->hugepage_info[j].hugepage_sz) {
  ...
  nr_hugepages = eal_dynmem_calc_num_pages_per_socket(
      memory, internal_conf->hugepage_info, used_hp,
    ...
    for (j = 0; j < RTE_MAX_NUMA_NODES; j++) {
      if (used_hp[i].num_pages[j] > 0) {
        RTE_LOG(DEBUG, EAL,
                ...
                (unsigned)(used_hp[i].hugepage_sz / 0x100000), j);
      }
    }
  }
   ...
   */
  if (copy_hugepages_to_shared_mem(hugepage, nr_hugefiles, tmp_hp,
                                   nr_hugefiles) < 0) {
  ...
  if (internal_conf->legacy_mem && prealloc_segments(hugepage, nr_hugefiles)) {
 ...
 */
static int eal_legacy_hugepage_attach(void) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
...
static int eal_hugepage_attach(void) {
  if (eal_memalloc_sync_with_primary()) {
    RTE_LOG(ERR, EAL, "Could not map memory from primary process\n");
    if (aslr_enabled() > 0)
      RTE_LOG(ERR, EAL,
              "It is recommended to disable ASLR in the kernel and retry "
              "running both primary and secondary processes\n");
    return -1;
  }
  return 0;
}
...
int rte_eal_hugepage_init(void) {
  ...
  return internal_conf->legacy_mem ? eal_legacy_hugepage_init()
...
int rte_eal_hugepage_attach(void) {
  ...
  return internal_conf->legacy_mem ? eal_legacy_hugepage_attach()
                                   : eal_hugepage_attach();
}
...
static int __rte_unused memseg_primary_init_32(void) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  ...
  extra_mem_per_socket =
      active_sockets == 0 ? total_extra_mem : total_extra_mem / active_sockets;
  ...
  for (i = 0; i < rte_socket_count(); i++) {
    ...
    skip = active_sockets != 0 && internal_conf->socket_mem[socket_id] == 0;
    ...
    max_socket_mem = (active_sockets != 0 ? internal_conf->socket_mem[socket_id]
    ...
    for (hpi_idx = 0; hpi_idx < hp_sizes; hpi_idx++) {
      ...
      max_pagesz_mem = RTE_ALIGN_FLOOR(max_pagesz_mem, hugepage_sz);
      ...
      type_msl_idx = 0;
      while (cur_pagesz_mem < max_pagesz_mem && total_segs < max_segs) {
        uint64_t cur_mem;
        unsigned int n_segs;
        ...
        cur_mem = get_mem_amount(hugepage_sz, max_pagesz_mem);
        n_segs = cur_mem / hugepage_sz;

        if (eal_memseg_list_init(msl, hugepage_sz, n_segs, socket_id,
                                 type_msl_idx, true)) {
        ...
        if (eal_memseg_list_alloc(msl, 0)) {
           ...
           */
          RTE_LOG(ERR, EAL,
                  "Cannot allocate VA space for memseg list, retrying with "
                  "different page size\n");
...
static int __rte_unused memseg_primary_init(void) {
  return eal_dynmem_memseg_lists_init();
}

static int memseg_secondary_init(void) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  int msl_idx = 0;
  struct rte_memseg_list *msl;
...
int rte_eal_memseg_init(void) {
  ...
  if (getrlimit(RLIMIT_NOFILE, &lim) == 0) {
    ...
    if (setrlimit(RLIMIT_NOFILE, &lim) < 0) {
      ...
      RTE_LOG(DEBUG, EAL,
              "Setting maximum number of open files to %" PRIu64 "\n",
  ...
  if (!internal_conf->legacy_mem && rte_socket_count() > 1) {
    RTE_LOG(WARNING, EAL,
            "DPDK is running on a NUMA system, but is compiled without NUMA "
            "support.\n");
    RTE_LOG(
        WARNING, EAL,
