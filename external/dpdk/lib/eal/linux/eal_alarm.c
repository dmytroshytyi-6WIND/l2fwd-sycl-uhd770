...
int rte_eal_alarm_set(uint64_t us, rte_eal_alarm_callback cb_fn, void *cb_arg) {
  ...
  new_alarm->time.tv_sec =
      now.tv_sec + (((now.tv_nsec / NS_PER_US) + us) / US_PER_S);

  rte_spinlock_lock(&alarm_list_lk);
  if (!handler_registered) {
    /* registration can fail, callback can be registered later */
    if (rte_intr_callback_register(intr_handle, eal_alarm_callback, NULL) == 0)
...
int rte_eal_alarm_cancel(rte_eal_alarm_callback cb_fn, void *cb_arg) {
  ...
  do {
    ...
    while ((ap = LIST_FIRST(&alarm_list)) != NULL && cb_fn == ap->cb_fn &&
