...
struct rte_intr_callback {
  ...
  rte_intr_unregister_callback_fn
      ucb_fn; /**< fn to call before cb is deleted */
};
...
#define MSIX_IRQ_SET_BUF_LEN                                                   \
  (sizeof(struct vfio_irq_set) + sizeof(int) * (RTE_MAX_RXTX_INTR_VEC_ID + 1))

/* enable legacy (INTx) interrupts */
static int vfio_enable_intx(const struct rte_intr_handle *intr_handle) {
...
static int vfio_disable_intx(const struct rte_intr_handle *intr_handle) {
  struct vfio_irq_set *irq_set;
  char irq_set_buf[IRQ_SET_BUF_LEN];
  int len, ret, vfio_dev_fd;
...
static int vfio_ack_intx(const struct rte_intr_handle *intr_handle) {
  struct vfio_irq_set irq_set;
  int vfio_dev_fd;
...
static int vfio_enable_msi(const struct rte_intr_handle *intr_handle) {
  int len, ret;
  char irq_set_buf[IRQ_SET_BUF_LEN];
  struct vfio_irq_set *irq_set;
...
static int vfio_disable_msi(const struct rte_intr_handle *intr_handle) {
  struct vfio_irq_set *irq_set;
  char irq_set_buf[IRQ_SET_BUF_LEN];
  int len, ret, vfio_dev_fd;
...
static int vfio_enable_msix(const struct rte_intr_handle *intr_handle) {
  int len, ret;
  ...
  irq_set->count =
      rte_intr_max_intr_get(intr_handle)
          ? (rte_intr_max_intr_get(intr_handle) > RTE_MAX_RXTX_INTR_VEC_ID + 1
...
static int vfio_disable_msix(const struct rte_intr_handle *intr_handle) {
  struct vfio_irq_set *irq_set;
  char irq_set_buf[MSIX_IRQ_SET_BUF_LEN];
  int len, ret, vfio_dev_fd;
...
/* enable req notifier */
static int vfio_enable_req(const struct rte_intr_handle *intr_handle) {
  int len, ret;
  char irq_set_buf[IRQ_SET_BUF_LEN];
  struct vfio_irq_set *irq_set;
  int *fd_ptr, vfio_dev_fd;
...
static int vfio_disable_req(const struct rte_intr_handle *intr_handle) {
  struct vfio_irq_set *irq_set;
  char irq_set_buf[IRQ_SET_BUF_LEN];
  int len, ret, vfio_dev_fd;
...
static int uio_intx_intr_disable(const struct rte_intr_handle *intr_handle) {
  ...
  if (uio_cfg_fd < 0 || pread(uio_cfg_fd, &command_high, 1, 5) != 1) {
    RTE_LOG(ERR, EAL, "Error reading interrupts status for fd %d\n",
            uio_cfg_fd);
    return -1;
  }
  /* disable interrupts */
  ...
  if (pwrite(uio_cfg_fd, &command_high, 1, 5) != 1) {
    RTE_LOG(ERR, EAL, "Error disabling interrupts for fd %d\n", uio_cfg_fd);
    return -1;
  }
...
static int uio_intx_intr_enable(const struct rte_intr_handle *intr_handle) {
  unsigned char command_high;
  int uio_cfg_fd;
  ...
  command_high &= ~0x4;
  if (pwrite(uio_cfg_fd, &command_high, 1, 5) != 1) {
    RTE_LOG(ERR, EAL, "Error enabling interrupts for fd %d\n", uio_cfg_fd);
    return -1;
  }
...
static int uio_intr_disable(const struct rte_intr_handle *intr_handle) {
  const int value = 0;
...
int rte_intr_callback_unregister_pending(
    const struct rte_intr_handle *intr_handle, rte_intr_callback_fn cb_fn,
    void *cb_arg, rte_intr_unregister_callback_fn ucb_fn) {
  int ret;
  struct rte_intr_source *src;
 ...
 *  never return;
 */
static __rte_noreturn void *eal_intr_thread_main(__rte_unused void *arg) {
  ...
  for (;;) {
    ...
    TAILQ_FOREACH(src, &intr_sources, next) {
       ...
       * into wait list.
       */
      if (epoll_ctl(pfd, EPOLL_CTL_ADD, rte_intr_fd_get(src->intr_handle),
                    &ev) < 0) {
        rte_panic("Error adding fd %d epoll_ctl, %s\n",
                  rte_intr_fd_get(src->intr_handle), strerror(errno));
      } else
        numfds++;
    }
    rte_spinlock_unlock(&intr_lock);
...
static void eal_intr_proc_rxtx_intr(int fd,
  ...
  do {
    ...
    if (nbytes < 0) {
      if (errno == EINTR || errno == EWOULDBLOCK || errno == EAGAIN)
        continue;
      RTE_LOG(ERR, EAL, "Error reading from fd %d: %s\n", fd, strerror(errno));
    } else if (nbytes == 0)
...
static int eal_epoll_process_event(struct epoll_event *evs, unsigned int n,
  ...
  for (i = 0; i < n; i++) {
     ...
     */
    if (!rev || !__atomic_compare_exchange_n(
                    &rev->status, &valid_status, RTE_EPOLL_EXEC, 0,
     ...
     */
    __atomic_store_n(&rev->status, RTE_EPOLL_VALID, __ATOMIC_RELEASE);
    count++;
  }
  return count;
}

static inline int eal_init_tls_epfd(void) {
...
static int eal_epoll_wait(int epfd, struct rte_epoll_event *events,
...
int rte_epoll_wait(int epfd, struct rte_epoll_event *events, int maxevents,
                   int timeout) {
...
static inline void eal_epoll_data_safe_free(struct rte_epoll_event *ev) {
  ...
  while (!__atomic_compare_exchange_n(&ev->status, &valid_status,
                                      ...
                                      __ATOMIC_RELAXED)) {
    while (__atomic_load_n(&ev->status, __ATOMIC_RELAXED) != RTE_EPOLL_VALID)
      rte_pause();
    valid_status = RTE_EPOLL_VALID;
  }
...
int rte_epoll_ctl(int epfd, int op, int fd, struct rte_epoll_event *event) {
  ...
  if (op == EPOLL_CTL_ADD) {
    __atomic_store_n(&event->status, RTE_EPOLL_VALID, __ATOMIC_RELAXED);
  ...
  if (epoll_ctl(epfd, op, fd, &ev) < 0) {
    ...
    if (op == EPOLL_CTL_ADD)
      ...
      __atomic_store_n(&event->status, RTE_EPOLL_INVALID, __ATOMIC_RELAXED);
    return -1;
  }

  if (op == EPOLL_CTL_DEL &&
      __atomic_load_n(&event->status, __ATOMIC_RELAXED) != RTE_EPOLL_INVALID)
    eal_epoll_data_safe_free(event);

  return 0;
}

int rte_intr_rx_ctl(struct rte_intr_handle *intr_handle, int epfd, int op,
  ...
  case RTE_INTR_EVENT_ADD:
    ...
    if (__atomic_load_n(&rev->status, __ATOMIC_RELAXED) != RTE_EPOLL_INVALID) {
...
int rte_intr_efd_enable(struct rte_intr_handle *intr_handle, uint32_t nb_efd) {
  ...
  if (rte_intr_type_get(intr_handle) == RTE_INTR_HANDLE_VFIO_MSIX) {
    for (i = 0; i < n; i++) {
      ...
      if (fd < 0) {
        RTE_LOG(ERR, EAL, "can't setup eventfd, error %i (%s)\n", errno,
                strerror(errno));
        return -errno;
      }
