...
int eal_clean_runtime_dir(void) {
  const char *runtime_dir = rte_eal_get_runtime_dir();
  DIR *dir;
  struct dirent *dirent;
  int dir_fd, fd, lck_result;
  static const char *const filters[] = {"fbarray_*", "mp_socket_*"};
 ...
 * processes. */
static int rte_eal_config_create(void) {
  ...
  if (internal_conf->base_virtaddr != 0)
    rte_mem_cfg_addr = (void *)RTE_ALIGN_FLOOR(
        internal_conf->base_virtaddr - sizeof(struct rte_mem_config), page_sz);
  else
  ...
  rte_mem_cfg_addr =
      eal_get_virtual_area(rte_mem_cfg_addr, &cfg_len_aligned, page_sz, 0, 0);
  ...
  mapped_mem_cfg_addr =
      mmap(rte_mem_cfg_addr, cfg_len_aligned, PROT_READ | PROT_WRITE,
...
static int rte_eal_config_attach(void) {
  struct rte_config *config = rte_eal_get_configuration();
  ...
  mem_config = (struct rte_mem_config *)mmap(
      NULL, sizeof(*mem_config), PROT_READ, MAP_SHARED, mem_cfg_fd, 0);
...
static int rte_eal_config_reattach(void) {
  struct rte_config *config = rte_eal_get_configuration();
  struct rte_mem_config *mem_config;
  ...
  mem_config = (struct rte_mem_config *)mmap(
      rte_mem_cfg_addr, sizeof(*mem_config), PROT_READ | PROT_WRITE, MAP_SHARED,
      mem_cfg_fd, 0);
  ...
  if (mem_config == MAP_FAILED || mem_config != rte_mem_cfg_addr) {
    if (mem_config != MAP_FAILED) {
      ...
      RTE_LOG(ERR, EAL,
              "Cannot mmap memory for rte_config at [%p], got [%p]"
...
static int rte_config_init(void) {
  ...
  case RTE_PROC_SECONDARY:
    ...
    if (eal_mcfg_check_version() < 0) {
      RTE_LOG(ERR, EAL,
              "Primary and secondary process DPDK version mismatch\n");
      return -1;
    }
...
static void eal_hugedirs_unlock(void) {
  int i;
...
static void eal_usage(const char *prgname) {
  ...
  printf("EAL Linux options:\n"
         ...
         "  --" OPT_VFIO_VF_TOKEN
         "     VF token (UUID) shared between SR-IOV PF and VFs\n"
...
static int eal_parse_socket_arg(char *strval, volatile uint64_t *socket_arg) {
  ...
  arg_num = rte_strsplit(strval, len, arg, RTE_MAX_NUMA_NODES, ',');
...
static int eal_parse_vfio_intr(const char *mode) {
  struct internal_config *internal_conf = eal_get_internal_configuration();
  unsigned i;
  static struct {
    const char *name;
...
static int eal_parse_vfio_vf_token(const char *vf_token) {
...
static void eal_log_level_parse(int argc, char **argv) {
  int opt;
  char **argvopt;
  int option_index;
  ...
  while ((opt = getopt_long(argc, argvopt, eal_short_options, eal_long_options,
                            &option_index)) != EOF) {
    ...
    ret = (opt == OPT_LOG_LEVEL_NUM)
              ? eal_parse_common_option(opt, optarg, internal_conf)
              : 0;
...
static int eal_parse_args(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argvopt, eal_short_options, eal_long_options,
    ...
    case OPT_SOCKET_MEM_NUM:
      if (eal_parse_socket_arg(optarg, internal_conf->socket_mem) < 0) {
        RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_SOCKET_MEM "\n");
        eal_usage(prgname);
        ret = -1;
        goto out;
      }
    ...
    case OPT_SOCKET_LIMIT_NUM:
      if (eal_parse_socket_arg(optarg, internal_conf->socket_limit) < 0) {
        RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_SOCKET_LIMIT "\n");
        eal_usage(prgname);
        ret = -1;
        goto out;
      }
    ...
    case OPT_VFIO_INTR_NUM:
      if (eal_parse_vfio_intr(optarg) < 0) {
        RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_VFIO_INTR "\n");
        eal_usage(prgname);
        ret = -1;
        goto out;
      }
    ...
    case OPT_VFIO_VF_TOKEN_NUM:
      if (eal_parse_vfio_vf_token(optarg) < 0) {
        RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_VFIO_VF_TOKEN "\n");
        eal_usage(prgname);
        ret = -1;
        goto out;
      }
    ...
    case OPT_MBUF_POOL_OPS_NAME_NUM: {
      ...
      if (ops_name == NULL)
        ...
        internal_conf->user_mbuf_pool_ops_name = ops_name;
      }
      break;
    }
    ...
    default:
      if (opt < OPT_LONG_MIN_NUM && isprint(opt)) {
        RTE_LOG(ERR, EAL,
                ...
                "on Linux\n",
                opt);
      } else if (opt >= OPT_LONG_MIN_NUM && opt < OPT_LONG_MAX_NUM) {
        RTE_LOG(ERR, EAL,
...
static int check_socket(const struct rte_memseg_list *msl, void *arg) {
  int *socket_id = arg;
...
static void eal_check_mem_on_local_socket(void) {
  ...
  if (rte_memseg_list_walk(check_socket, &socket_id) == 0)
    RTE_LOG(WARNING, EAL,
            "WARNING: Main core has no memory on local socket!\n");
}

static int sync_func(__rte_unused void *arg) { return 0; }
 ...
 */
static bool is_iommu_enabled(void) {
...
int rte_eal_init(int argc, char **argv) {
  ...
  if (!__atomic_compare_exchange_n(&run_once, &has_run, 1, 0, __ATOMIC_RELAXED,
                                   __ATOMIC_RELAXED)) {
  ...
  if (internal_conf->iova_mode == RTE_IOVA_DC) {
    ...
    if (iova_mode == RTE_IOVA_DC) {
      ...
      if (!phys_addrs) {
        ...
        RTE_LOG(DEBUG, EAL,
                "KNI is loaded, selecting IOVA as PA mode for better KNI "
                "performance.\n");
#endif
        ...
        iova_mode = RTE_IOVA_PA;
        RTE_LOG(DEBUG, EAL,
                "IOMMU is not available, selecting IOVA as PA mode.\n");
      }
    }
#if defined(RTE_LIB_KNI) && LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0)
     ...
     * in kernels < 4.10
     */
    if (iova_mode == RTE_IOVA_VA && rte_eal_check_module("rte_kni") == 1) {
      if (phys_addrs) {
        iova_mode = RTE_IOVA_PA;
        RTE_LOG(WARNING, EAL,
                "Forcing IOVA as 'PA' because KNI module is loaded\n");
      } else {
        RTE_LOG(DEBUG, EAL,
                "KNI can not work since physical addresses are unavailable\n");
      }
    }
#endif
  ...
  RTE_LCORE_FOREACH_WORKER(i) {
    ...
    snprintf(thread_name, sizeof(thread_name), "lcore-worker-%d", i);
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY &&
    ...
    int tlog =
        rte_log_register_type_and_pick_level("lib.telemetry", RTE_LOG_WARNING);
    if (tlog < 0)
      tlog = RTE_LOGTYPE_EAL;
    if (rte_telemetry_init(rte_eal_get_runtime_dir(), rte_version(),
...
static int mark_freeable(const struct rte_memseg_list *msl,
