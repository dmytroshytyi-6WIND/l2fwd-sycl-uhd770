...
static void mem_free(void *addr, const bool trace_ena) {
  if (trace_ena)
...
void rte_free(void *addr) { return mem_free(addr, true); }

void eal_free_no_trace(void *addr) { return mem_free(addr, false); }

static void *malloc_socket(const char *type, size_t size, unsigned int align,
  ...
  ptr = malloc_heap_alloc(type, size, socket_arg, 0, align == 0 ? 1 : align, 0,
                          false);
 ...
 */
void *rte_malloc_socket(const char *type, size_t size, unsigned int align,
                        int socket_arg) {
  return malloc_socket(type, size, align, socket_arg, true);
}

void *eal_malloc_no_trace(const char *type, size_t size, unsigned int align) {
...
void *rte_realloc_socket(void *ptr, size_t size, unsigned int align,
  ...
  if ((socket == SOCKET_ID_ANY ||
      ...
      RTE_PTR_ALIGN(ptr, align) == ptr && malloc_heap_resize(elem, size) == 0) {
...
int rte_malloc_get_socket_stats(int socket,
  ...
  return malloc_heap_get_stats(&mcfg->malloc_heaps[heap_idx], socket_stats);
}
...
int rte_malloc_heap_get_socket(const char *name) {
  ...
  if (name == NULL || strnlen(name, RTE_HEAP_NAME_MAX_LEN) == 0 ||
      strnlen(name, RTE_HEAP_NAME_MAX_LEN) == RTE_HEAP_NAME_MAX_LEN) {
    rte_errno = EINVAL;
    return -1;
  }
 ...
 */
int rte_malloc_set_limit(__rte_unused const char *type,
...
static struct malloc_heap *find_named_heap(const char *name) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  unsigned int i;
...
int rte_malloc_heap_memory_add(const char *heap_name, void *va_addr, size_t len,
  ...
  if (heap_name == NULL || va_addr == NULL || page_sz == 0 ||
      ...
      strnlen(heap_name, RTE_HEAP_NAME_MAX_LEN) == RTE_HEAP_NAME_MAX_LEN) {
    rte_errno = EINVAL;
    return -1;
  }
...
static int sync_memory(const char *heap_name, void *va_addr, size_t len,
                       bool attach) {
  struct malloc_heap *heap = NULL;
  struct rte_memseg_list *msl;
  int ret;
  ...
  if (attach) {
    ...
    if (ret == 0) {
       ...
       * added.
       */
      eal_memalloc_mem_event_notify(RTE_MEM_EVENT_ALLOC, va_addr, len);
    } else {
      ret = -1;
      goto unlock;
    }
     ...
     * be removed.
     */
    eal_memalloc_mem_event_notify(RTE_MEM_EVENT_FREE, msl->base_va, msl->len);
...
int rte_malloc_heap_create(const char *heap_name) {
  ...
  if (heap_name == NULL || strnlen(heap_name, RTE_HEAP_NAME_MAX_LEN) == 0 ||
      strnlen(heap_name, RTE_HEAP_NAME_MAX_LEN) == RTE_HEAP_NAME_MAX_LEN) {
    rte_errno = EINVAL;
    return -1;
  }
  ...
  for (i = 0; i < RTE_MAX_HEAPS; i++) {
    ...
    /* existing heap */
    if (strncmp(heap_name, tmp->name, RTE_HEAP_NAME_MAX_LEN) == 0) {
