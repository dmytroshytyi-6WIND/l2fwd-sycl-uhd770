...
const char *trace_mode_to_string(enum rte_trace_mode mode) {
  switch (mode) {
  case RTE_TRACE_MODE_OVERWRITE:
    return "overwrite";
  case RTE_TRACE_MODE_DISCARD:
    return "discard";
  default:
    return "unknown";
  }
...
const char *trace_area_to_string(enum trace_area_e area) {
  switch (area) {
  case TRACE_AREA_HEAP:
    return "heap";
  case TRACE_AREA_HUGEPAGE:
    return "hugepage";
  default:
    return "unknown";
  }
...
static bool trace_entry_compare(const char *name) {
...
void trace_uuid_generate(void) {
  ...
  rte_uuid_t uuid = RTE_UUID_INIT(sz_total, trace->nb_trace_points, 0x4370,
                                  0x8f50, 0x222ddd514176ULL);
  rte_uuid_copy(trace->uuid, uuid);
}

static int trace_session_name_generate(char *trace_dir) {
  struct tm *tm_result;
  time_t tm;
  int rc;
  ...
  rc = rte_strscpy(trace_dir, eal_get_hugefile_prefix(), TRACE_PREFIX_LEN);
  if (rc == -E2BIG)
    rc = TRACE_PREFIX_LEN;
  trace_dir[rc++] = '-';

  rc = strftime(trace_dir + rc, TRACE_DIR_STR_LEN - rc, "%Y-%m-%d-%p-%I-%M-%S",
                tm_result);
  if (rc == 0)
    goto fail;
...
static int trace_dir_update(const char *str) {
  struct trace *trace = trace_obj_get();
  int rc, remaining;
...
int eal_trace_bufsz_args_save(char const *val) {
  struct trace *trace = trace_obj_get();
  uint64_t bufsz;
...
int eal_trace_dir_args_save(char const *val) {
  struct trace *trace = trace_obj_get();
  char *dir_path;
  int rc;
...
static int trace_dir_default_path_get(char *dir_path) {
  struct trace *trace = trace_obj_get();
...
static int trace_meta_save(struct trace *trace) {
...
static inline int trace_file_sz(struct __rte_trace_header *hdr) {
  return sizeof(struct __rte_trace_stream_header) + hdr->offset;
}

static int trace_mem_save(struct trace *trace, struct __rte_trace_header *hdr,
                          uint32_t cnt) {
  char file_name[PATH_MAX];
  FILE *f;
  int rc;
