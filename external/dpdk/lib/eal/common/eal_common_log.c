...
uint32_t rte_log_get_global_level(void) { return rte_logs.level; }
...
static void logtype_set_level(uint32_t type, uint32_t level) {
  ...
  if (current != level) {
    ...
    RTE_LOG(DEBUG, EAL, "%s log level changed from %s to %s\n",
            ...
            eal_log_level2str(current), eal_log_level2str(level));
  }
}
...
int rte_log_set_level_regexp(const char *regex, uint32_t level) {
  ...
  for (i = 0; i < rte_logs.dynamic_types_len; i++) {
    if (rte_logs.dynamic_types[i].name == NULL)
      continue;
    if (regexec(&r, rte_logs.dynamic_types[i].name, 0, NULL, 0) == 0)
 ...
 */
static int log_save_level(uint32_t priority, const char *regex,
                          const char *pattern) {
...
int rte_log_cur_msg_logtype(void) { return RTE_PER_LCORE(log_cur_msg).logtype; }

static int log_lookup(const char *name) {
  size_t i;
...
static int log_register(const char *name, uint32_t level) {
  ...
  new_dynamic_types =
      realloc(rte_logs.dynamic_types, sizeof(struct rte_log_dynamic_type) *
  ...
  memset(&rte_logs.dynamic_types[id], 0, sizeof(rte_logs.dynamic_types[id]));
...
static const struct logtype logtype_strings[] = {
    {RTE_LOGTYPE_EAL, "lib.eal"},     {RTE_LOGTYPE_MALLOC, "lib.malloc"},
    {RTE_LOGTYPE_RING, "lib.ring"},   {RTE_LOGTYPE_MEMPOOL, "lib.mempool"},
    {RTE_LOGTYPE_TIMER, "lib.timer"}, {RTE_LOGTYPE_PMD, "pmd"},
    {RTE_LOGTYPE_HASH, "lib.hash"},   {RTE_LOGTYPE_LPM, "lib.lpm"},
    {RTE_LOGTYPE_KNI, "lib.kni"},     {RTE_LOGTYPE_ACL, "lib.acl"},
    {RTE_LOGTYPE_POWER, "lib.power"}, {RTE_LOGTYPE_METER, "lib.meter"},
    {RTE_LOGTYPE_SCHED, "lib.sched"}, {RTE_LOGTYPE_PORT, "lib.port"},
    {RTE_LOGTYPE_TABLE, "lib.table"}, {RTE_LOGTYPE_PIPELINE, "lib.pipeline"},
    {RTE_LOGTYPE_MBUF, "lib.mbuf"},   {RTE_LOGTYPE_CRYPTODEV, "lib.cryptodev"},
    {RTE_LOGTYPE_EFD, "lib.efd"},     {RTE_LOGTYPE_EVENTDEV, "lib.eventdev"},
    {RTE_LOGTYPE_GSO, "lib.gso"},     {RTE_LOGTYPE_USER1, "user1"},
    {RTE_LOGTYPE_USER2, "user2"},     {RTE_LOGTYPE_USER3, "user3"},
    {RTE_LOGTYPE_USER4, "user4"},     {RTE_LOGTYPE_USER5, "user5"},
    {RTE_LOGTYPE_USER6, "user6"},     {RTE_LOGTYPE_USER7, "user7"},
...
RTE_INIT_PRIO(log_init, LOG) {
  ...
  rte_logs.dynamic_types =
      calloc(RTE_LOGTYPE_FIRST_EXT_ID, sizeof(struct rte_log_dynamic_type));
...
static int log_type_compare(const void *a, const void *b) {
