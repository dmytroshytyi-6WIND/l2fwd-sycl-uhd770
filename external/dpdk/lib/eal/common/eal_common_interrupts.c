...
#define RTE_INTR_INSTANCE_KNOWN_FLAGS                                          \
  (RTE_INTR_INSTANCE_F_PRIVATE | RTE_INTR_INSTANCE_F_SHARED)
...
struct rte_intr_handle *rte_intr_instance_alloc(uint32_t flags) {
  ...
  if (uses_rte_memory) {
    intr_handle->efds =
        rte_zmalloc(NULL, RTE_MAX_RXTX_INTR_VEC_ID * sizeof(int), 0);
  } else {
    intr_handle->efds = calloc(RTE_MAX_RXTX_INTR_VEC_ID, sizeof(int));
  }
  ...
  if (uses_rte_memory) {
    intr_handle->elist = rte_zmalloc(
        NULL, RTE_MAX_RXTX_INTR_VEC_ID * sizeof(struct rte_epoll_event), 0);
  } else {
    intr_handle->elist =
        calloc(RTE_MAX_RXTX_INTR_VEC_ID, sizeof(struct rte_epoll_event));
  }
...
int rte_intr_event_list_update(struct rte_intr_handle *intr_handle, int size) {
  ...
  uses_rte_memory = RTE_INTR_INSTANCE_USES_RTE_MEMORY(intr_handle->alloc_flags);
  if (uses_rte_memory) {
    tmp_efds = rte_realloc(intr_handle->efds, size * sizeof(int), 0);
  } else {
  ...
  if (uses_rte_memory) {
    ...
    tmp_elist =
        realloc(intr_handle->elist, size * sizeof(struct rte_epoll_event));
  }
  if (tmp_elist == NULL) {
...
int rte_intr_max_intr_set(struct rte_intr_handle *intr_handle, int max_intr) {
  CHECK_VALID_INTR_HANDLE(intr_handle);

  if (max_intr > intr_handle->nb_intr) {
    RTE_LOG(DEBUG, EAL,
            "Maximum interrupt vector ID (%d) exceeds "
...
int rte_intr_efds_index_set(struct rte_intr_handle *intr_handle, int index,
                            int fd) {
  CHECK_VALID_INTR_HANDLE(intr_handle);
...
int rte_intr_elist_index_set(struct rte_intr_handle *intr_handle, int index,
...
int rte_intr_vec_list_index_set(struct rte_intr_handle *intr_handle, int index,
                                int vec) {
  CHECK_VALID_INTR_HANDLE(intr_handle);
