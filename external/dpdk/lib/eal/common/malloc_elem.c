...
size_t malloc_elem_find_max_iova_contig(struct malloc_elem *elem,
                                        size_t align) {
  ...
  if (!elem->msl->external &&
      (rte_eal_iova_mode() == RTE_IOVA_VA ||
       (internal_conf->legacy_mem && rte_eal_has_hugepages())))
  ...
  cur = RTE_PTR_DIFF(cur_seg_end, contig_seg_start) - MALLOC_ELEM_TRAILER_LEN;
  max = cur;
 ...
 */
static bool elem_check_phys_contig(const struct rte_memseg_list *msl,
 ...
 * fit, return NULL.
 */
static void *elem_start_pt(struct malloc_elem *elem, size_t size,
  ...
  while (elem_size >= size) {
    ...
    uintptr_t end_pt = (uintptr_t)elem + elem_size - MALLOC_ELEM_TRAILER_LEN;
    ...
    if (contig) {
       ...
       */
      if (!elem_check_phys_contig(elem->msl, (void *)new_data_start,
                                  new_data_size)) {
        elem_size -= align;
        continue;
      }
 ...
 * split_pt parameter.
 */
static void split_elem(struct malloc_elem *elem, struct malloc_elem *split_pt) {
 ...
 */
static void __rte_unused remove_elem(struct malloc_elem *elem) {
...
static int next_elem_is_adjacent(struct malloc_elem *elem) {
  const struct internal_config *internal_conf =
      eal_get_internal_configuration();
...
static int prev_elem_is_adjacent(struct malloc_elem *elem) {
  const struct internal_config *internal_conf =
      eal_get_internal_configuration();
...
#define MALLOC_LOG2_INCREMENT 2
  ...
  return index <= RTE_HEAP_NUM_FREELISTS - 1 ? index
                                             : RTE_HEAP_NUM_FREELISTS - 1;
}
...
struct malloc_elem *malloc_elem_alloc(struct malloc_elem *elem, size_t size,
  ...
  struct malloc_elem *new_elem =
      elem_start_pt(elem, size, align, bound, contig);
  const size_t old_elem_size = (uintptr_t)new_elem - (uintptr_t)elem;
  const size_t trailer_size =
      elem->size - old_elem_size - size - MALLOC_ELEM_OVERHEAD;
 ...
 */
static inline void join_elem(struct malloc_elem *elem1,
                             struct malloc_elem *elem2) {
...
void malloc_elem_hide_region(struct malloc_elem *elem, void *start,
                             size_t len) {
...
static inline const char *elem_state_to_str(enum elem_state state) {
  switch (state) {
  case ELEM_PAD:
    return "PAD";
  case ELEM_BUSY:
