...
static inline int malloc_elem_cookies_ok(const struct malloc_elem *elem) {
  return elem != NULL;
}

/* dummy function - no header if malloc_debug is not enabled */
static inline void set_header(struct malloc_elem *elem __rte_unused) {}

/* dummy function - no trailer if malloc_debug is not enabled */
static inline void set_trailer(struct malloc_elem *elem __rte_unused) {}
...
static inline void set_header(struct malloc_elem *elem) {
  if (elem != NULL)
    MALLOC_ELEM_HEADER(elem) = MALLOC_HEADER_COOKIE;
}

static inline void set_trailer(struct malloc_elem *elem) {
  if (elem != NULL)
...
static inline int malloc_elem_cookies_ok(const struct malloc_elem *elem) {
  return elem != NULL && MALLOC_ELEM_HEADER(elem) == MALLOC_HEADER_COOKIE &&
         MALLOC_ELEM_TRAILER(elem) == MALLOC_TRAILER_COOKIE;
}
...
__rte_no_asan static inline void asan_set_shadow(void *addr, char val) {
  *(char *)addr = val;
}

static inline void asan_set_zone(void *ptr, size_t len, uint32_t val) {
  size_t offset, i;
  void *shadow;
 ...
 */
static inline void asan_set_freezone(void *ptr, size_t size) {
 ...
 */
static inline void asan_clear_alloczone(struct malloc_elem *elem) {
  asan_set_zone((void *)elem, elem->size, 0x0);
}

static inline void asan_clear_split_alloczone(struct malloc_elem *elem) {
 ...
 */
static inline void asan_set_redzone(struct malloc_elem *elem,
                                    size_t user_size) {
  ...
  if (elem != NULL) {
    ...
    head_redzone = (uintptr_t)RTE_PTR_ADD(elem, MALLOC_ELEM_HEADER_LEN -
                                                    ASAN_SHADOW_GRAIN_SIZE);
    front_shadow = ASAN_MEM_TO_SHADOW(head_redzone);
    asan_set_shadow(front_shadow, ASAN_MEM_REDZONE_FLAG);
    front_shadow = ASAN_MEM_TO_SHADOW(head_redzone - ASAN_SHADOW_GRAIN_SIZE);
    asan_set_shadow(front_shadow, ASAN_MEM_REDZONE_FLAG);

    /* Set mark after the end of the allocated memory */
    tail_redzone =
        (uintptr_t)RTE_PTR_ADD(elem, MALLOC_ELEM_HEADER_LEN + elem->user_size);
    ...
    tail_shadow = ASAN_MEM_TO_SHADOW(tail_redzone + ASAN_SHADOW_GRAIN_SIZE);
 ...
 * Requirement: redzone >= 16, is a power of two.
 */
static inline void asan_clear_redzone(struct malloc_elem *elem) {
  uintptr_t head_redzone;
  uintptr_t tail_redzone;
  void *head_shadow;
  void *tail_shadow;
  ...
  if (elem != NULL) {
    ...
    head_shadow = ASAN_MEM_TO_SHADOW(head_redzone - ASAN_SHADOW_GRAIN_SIZE);
    asan_set_shadow(head_shadow, 0x00);
...
static inline size_t old_malloc_size(struct malloc_elem *elem) {
  if (elem->state != ELEM_PAD)
    elem = RTE_PTR_ADD(elem, elem->pad);
...
static inline void asan_set_freezone(void *ptr __rte_unused,
                                     size_t size __rte_unused) {}

static inline void asan_clear_alloczone(struct malloc_elem *elem __rte_unused) {
}

static inline void
asan_clear_split_alloczone(struct malloc_elem *elem __rte_unused) {}

static inline void asan_set_redzone(struct malloc_elem *elem __rte_unused,
                                    size_t user_size __rte_unused) {}

static inline void asan_clear_redzone(struct malloc_elem *elem __rte_unused) {}
 ...
 */
static inline struct malloc_elem *malloc_elem_from_data(const void *data) {
  if (data == NULL)
    return NULL;
 ...
 */
void malloc_elem_init(struct malloc_elem *elem, struct malloc_heap *heap,
 ...
 */
int malloc_elem_can_hold(struct malloc_elem *elem, size_t size,
 ...
 */
struct malloc_elem *malloc_elem_alloc(struct malloc_elem *elem, size_t size,
                                      unsigned int align, size_t bound,
                                      bool contig);
...
struct malloc_elem *malloc_elem_join_adjacent_free(struct malloc_elem *elem);

/*
 * attempt to resize a malloc_elem by expanding into any free space
 * immediately after it in memory.
 */
int malloc_elem_resize(struct malloc_elem *elem, size_t size);

void malloc_elem_hide_region(struct malloc_elem *elem, void *start, size_t len);
 ...
 */
void malloc_elem_dump(const struct malloc_elem *elem, FILE *f);
 ...
 */
size_t malloc_elem_find_max_iova_contig(struct malloc_elem *elem, size_t align);
