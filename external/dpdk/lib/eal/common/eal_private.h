 ...
 */
int rte_devargs_layers_parse(struct rte_devargs *devargs, const char *devstr);
 ...
 */
int dev_sigbus_handler_register(void);
 ...
 *   - On failure, a negative value.
 */
int dev_sigbus_handler_unregister(void);
 ...
 */
int eal_file_open(const char *path, int flags);
 ...
 *   0 on success, (-1) on failure.
 */
int eal_set_runtime_dir(const char *run_dir);
 ...
 */
struct internal_config *eal_get_internal_configuration(void);
...
__rte_format_printf(2, 3) int eal_asprintf(char **buffer, const char *format,
                                           ...);
