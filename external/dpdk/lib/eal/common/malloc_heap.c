...
static unsigned check_hugepage_sz(unsigned flags, uint64_t hugepage_sz) {
...
int malloc_socket_to_heap_id(unsigned int socket_id) {
 ...
 */
static struct malloc_elem *malloc_heap_add_memory(struct malloc_heap *heap,
...
find_suitable_element(struct malloc_heap *heap, size_t size, unsigned int flags,
                      size_t align, size_t bound, bool contig) {
  size_t idx;
  struct malloc_elem *elem, *alt_elem = NULL;

  for (idx = malloc_elem_free_list_index(size); idx < RTE_HEAP_NUM_FREELISTS;
       idx++) {
    for (elem = LIST_FIRST(&heap->free_head[idx]); !!elem;
         elem = LIST_NEXT(elem, free_list)) {
      if (malloc_elem_can_hold(elem, size, align, bound, contig)) {
 ...
 * Returns null on failure, or pointer to element on success.
 */
static struct malloc_elem *find_biggest_element(struct malloc_heap *heap,
                                                size_t *size,
                                                unsigned int flags,
  ...
  for (idx = 0; idx < RTE_HEAP_NUM_FREELISTS; idx++) {
    for (elem = LIST_FIRST(&heap->free_head[idx]); !!elem;
      ...
      if ((flags & RTE_MEMZONE_SIZE_HINT_ONLY) == 0 &&
          !check_hugepage_sz(flags, elem->msl->page_sz))
        continue;
      if (contig) {
        cur_size = malloc_elem_find_max_iova_contig(elem, align);
      } else {
        void *data_start = RTE_PTR_ADD(elem, MALLOC_ELEM_HEADER_LEN);
        void *data_end =
            RTE_PTR_ADD(elem, elem->size - MALLOC_ELEM_TRAILER_LEN);
        void *aligned = RTE_PTR_ALIGN_CEIL(data_start, align);
 ...
 */
static void *heap_alloc(struct malloc_heap *heap, const char *type __rte_unused,
                        size_t size, unsigned int flags, size_t align,
...
static void *heap_alloc_biggest(struct malloc_heap *heap,
...
void rollback_expand_heap(struct rte_memseg **ms, int n_segs,
                          struct malloc_elem *elem, void *map_addr,
                          size_t map_len) {
  if (elem != NULL) {
...
/* this function is exposed in malloc_mp.h */
struct malloc_elem *alloc_pages_on_heap(struct malloc_heap *heap,
                                        uint64_t pg_sz, size_t elt_size,
                                        int socket, unsigned int flags,
                                        size_t align, size_t bound, bool contig,
                                        struct rte_memseg **ms, int n_segs) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  struct rte_memseg_list *msl;
  ...
  if (eal_memalloc_mem_alloc_validate(socket, heap->total_size + alloc_sz) <
      0) {
  ...
  if (mcfg->dma_maskbits &&
     ...
     */
    RTE_LOG(ERR, EAL,
            "%s(): couldn't allocate memory due to IOVA exceeding limits of "
            "current DMA mask\n",
            __func__);
     ...
     * problem.
     */
    if ((rte_eal_iova_mode() == RTE_IOVA_VA) && rte_eal_using_phys_addrs())
      RTE_LOG(
          ERR, EAL,
  ...
  ret = find_suitable_element(heap, elt_size, flags, align, bound, contig);
...
static int try_expand_heap_primary(struct malloc_heap *heap, uint64_t pg_sz,
                                   size_t elt_size, int socket,
                                   unsigned int flags, size_t align,
                                   size_t bound, bool contig) {
  struct malloc_elem *elem;
  ...
  alloc_sz = RTE_ALIGN_CEIL(align + elt_size + MALLOC_ELEM_TRAILER_LEN, pg_sz);
  ...
  elem = alloc_pages_on_heap(heap, pg_sz, elt_size, socket, flags, align, bound,
                             contig, ms, n_segs);
...
free_elem:
  if (callback_triggered)
    eal_memalloc_mem_event_notify(RTE_MEM_EVENT_FREE, map_addr, alloc_sz);
...
static int try_expand_heap_secondary(struct malloc_heap *heap, uint64_t pg_sz,
                                     size_t elt_size, int socket,
                                     unsigned int flags, size_t align,
                                     size_t bound, bool contig) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
...
static int try_expand_heap(struct malloc_heap *heap, uint64_t pg_sz,
                           size_t elt_size, int socket, unsigned int flags,
                           size_t align, size_t bound, bool contig) {
  int ret;
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    ret = try_expand_heap_primary(heap, pg_sz, elt_size, socket, flags, align,
                                  bound, contig);
  } else {
    ret = try_expand_heap_secondary(heap, pg_sz, elt_size, socket, flags, align,
                                    bound, contig);
  }
...
static int compare_pagesz(const void *a, const void *b) {
...
static int alloc_more_mem_on_socket(struct malloc_heap *heap, size_t size,
                                    int socket, unsigned int flags,
                                    size_t align, size_t bound, bool contig) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  ...
  for (i = 0; i < RTE_MAX_MEMSEG_LISTS; i++) {
    ...
    if (size_flags != 0 && check_hugepage_sz(size_flags, msl->page_sz))
  ...
  qsort(requested_msls, n_requested_msls, sizeof(requested_msls[0]),
        compare_pagesz);
  qsort(other_msls, n_other_msls, sizeof(other_msls[0]), compare_pagesz);
  ...
  for (i = 0; i < n_requested_pg_sz; i++) {
     ...
     */
    if (!try_expand_heap(heap, pg_sz, size, socket, size_flags, align, bound,
                         contig))
      return 0;
  }
  if (n_other_pg_sz == 0)
  ...
  for (i = 0; i < n_other_pg_sz; i++) {
    ...
    if (!try_expand_heap(heap, pg_sz, size, socket, flags, align, bound,
                         contig))
      return 0;
  }
  return -1;
...
static void *malloc_heap_alloc_on_heap_id(const char *type, size_t size,
  ...
  if (!alloc_more_mem_on_socket(heap, size, socket_id, flags, align, bound,
                                contig)) {
    ret = heap_alloc(heap, type, size, flags, align, bound, contig);
...
void *malloc_heap_alloc(const char *type, size_t size, int socket_arg,
  ...
  ret = malloc_heap_alloc_on_heap_id(type, size, heap_id, flags, align, bound,
                                     contig);
  ...
  for (i = 0; i < (int)rte_socket_count(); i++) {
    if (i == heap_id)
      continue;
    ret = malloc_heap_alloc_on_heap_id(type, size, i, flags, align, bound,
                                       contig);
    if (ret != NULL)
      return ret;
  }
...
static void *heap_alloc_biggest_on_heap_id(const char *type,
                                           unsigned int heap_id,
                                           unsigned int flags, size_t align,
                                           bool contig) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
...
void *malloc_heap_alloc_biggest(const char *type, int socket_arg,
                                unsigned int flags, size_t align, bool contig) {
  ...
  ret = heap_alloc_biggest_on_heap_id(type, heap_id, flags, align, contig);
  if (ret != NULL || socket_arg != SOCKET_ID_ANY)
    return ret;
  ...
  for (i = 0; i < (int)rte_socket_count(); i++) {
    ...
    if (cur_socket == socket)
      continue;
    ret = heap_alloc_biggest_on_heap_id(type, i, flags, align, contig);
    if (ret != NULL)
      return ret;
  }
  return NULL;
...
/* this function is exposed in malloc_mp.h */
int malloc_heap_free_pages(void *aligned_start, size_t aligned_len) {
...
int malloc_heap_free(struct malloc_elem *elem) {
  struct malloc_heap *heap;
  ...
  n_segs = aligned_len / page_sz;
  for (i = 0; i < n_segs; i++) {
    const struct rte_memseg *tmp = rte_mem_virt2memseg(aligned_start, msl);
  ...
  if (after_space != 0 && after_space < MALLOC_ELEM_OVERHEAD + MIN_DATA_SIZE) {
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    /* notify user about changes in memory map */
    eal_memalloc_mem_event_notify(RTE_MEM_EVENT_FREE, aligned_start,
                                  aligned_len);
...
int malloc_heap_resize(struct malloc_elem *elem, size_t size) {
  int ret;
 ...
 */
int malloc_heap_get_stats(struct malloc_heap *heap,
  ...
  socket_stats->heap_allocsz_bytes =
      (socket_stats->heap_totalsz_bytes - socket_stats->heap_freesz_bytes);
 ...
 * Function to retrieve data for a given heap
 */
void malloc_heap_dump(struct malloc_heap *heap, FILE *f) {
  struct malloc_elem *elem;
...
static int destroy_elem(struct malloc_elem *elem, size_t len) {
...
malloc_heap_create_external_seg(void *va_addr, rte_iova_t iova_addrs[],
                                ...
                                const char *seg_name, unsigned int socket_id) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  ...
  snprintf(fbarray_name, sizeof(fbarray_name), "%s_%p", seg_name, va_addr);
...
static int extseg_walk(const struct rte_memseg_list *msl, void *arg) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
...
struct rte_memseg_list *malloc_heap_find_external_seg(void *va_addr,
                                                      size_t len) {
...
int malloc_heap_destroy_external_seg(struct rte_memseg_list *msl) {
...
int malloc_heap_add_external_memory(struct malloc_heap *heap,
                                    struct rte_memseg_list *msl) {
  ...
  eal_memalloc_mem_event_notify(RTE_MEM_EVENT_ALLOC, msl->base_va, msl->len);

  return 0;
}

int malloc_heap_remove_external_memory(struct malloc_heap *heap, void *va_addr,
                                       size_t len) {
...
int malloc_heap_create(struct malloc_heap *heap, const char *heap_name) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
...
int malloc_heap_destroy(struct malloc_heap *heap) {
...
void rte_eal_malloc_heap_cleanup(void) { unregister_mp_requests(); }
