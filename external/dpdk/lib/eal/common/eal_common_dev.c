...
#define ITCTX(ptr) (((struct dev_next_ctx *)(intptr_t)ptr)->it)

#define BUSCTX(ptr) (((struct dev_next_ctx *)(intptr_t)ptr)->bus_str)

#define CLSCTX(ptr) (((struct dev_next_ctx *)(intptr_t)ptr)->cls_str)
...
static int build_devargs(const char *busname, const char *devname,
...
int rte_dev_probe(const char *devargs) {
  ...
  if (rte_eal_process_type() != RTE_PROC_PRIMARY) {
    ...
    if (ret != 0) {
      RTE_LOG(ERR, EAL, "Failed to send hotplug request to primary\n");
      return -ENOMSG;
    }
    if (req.result != 0)
      RTE_LOG(ERR, EAL, "Failed to hotplug add device\n");
    return req.result;
  }
  ...
  if (ret != 0) {
    RTE_LOG(ERR, EAL, "Failed to attach device on primary process\n");
  ...
  if (ret != 0) {
    RTE_LOG(ERR, EAL, "Failed to send hotplug add request to secondary\n");
    ret = -ENOMSG;
    goto rollback;
  }
   ...
   */
  if (req.result != 0) {
    RTE_LOG(ERR, EAL, "Failed to attach device on secondary process\n");
    ret = req.result;
...
int rte_dev_remove(struct rte_device *dev) {
  ...
  if (rte_eal_process_type() != RTE_PROC_PRIMARY) {
    ...
    if (req.result != 0)
      RTE_LOG(ERR, EAL, "Failed to hotplug remove device\n");
    return req.result;
  }
   ...
   */
  if (ret != 0) {
    RTE_LOG(ERR, EAL, "Failed to send device detach request to secondary\n");
    ret = -ENOMSG;
    goto rollback;
  }
   ...
   */
  if (req.result != 0) {
    RTE_LOG(ERR, EAL, "Failed to detach device on secondary process\n");
    ret = req.result;
    /**
  ...
  if (ret != 0) {
    RTE_LOG(ERR, EAL, "Failed to detach device on primary process\n");
...
void rte_dev_event_callback_process(const char *device_name,
  ...
  TAILQ_FOREACH(cb_lst, &dev_event_cbs, next) {
    ...
    cb_lst->active = 1;
    rte_spinlock_unlock(&dev_event_lock);
    cb_lst->cb_fn(device_name, event, cb_lst->cb_arg);
    rte_spinlock_lock(&dev_event_lock);
    cb_lst->active = 0;
  }
  rte_spinlock_unlock(&dev_event_lock);
...
int rte_dev_iterator_init(struct rte_dev_iterator *it, const char *dev_str) {
  ...
  if (bus == NULL && cls == NULL) {
    RTE_LOG(ERR, EAL, "Either bus or class must be specified.\n");
    rte_errno = EINVAL;
    goto get_out;
  }
...
static char *dev_str_sane_copy(const char *str) {
  size_t end;
  char *copy;
...
static int class_next_dev_cmp(const struct rte_class *cls, const void *ctx) {
  struct rte_dev_iterator *it;
...
static int bus_next_dev_cmp(const struct rte_bus *bus, const void *ctx) {
...
struct rte_device *rte_dev_iterator_next(struct rte_dev_iterator *it) {
  ...
  while (
      (bus = rte_bus_find(bus, bus_next_dev_cmp, CTX(it, bus_str, cls_str)))) {
    if (it->device != NULL) {
      it->bus = bus;
      goto out;
    }
    if (it->bus_str != NULL || rte_errno != 0)
      break;
  }
  if (rte_errno == 0)
    rte_errno = old_errno;
