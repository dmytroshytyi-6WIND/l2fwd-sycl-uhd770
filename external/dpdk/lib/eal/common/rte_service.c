...
int32_t rte_service_init(void) {
  ...
  rte_services =
      rte_calloc("rte_services", RTE_SERVICE_NUM_MAX,
                 sizeof(struct rte_service_spec_impl), RTE_CACHE_LINE_SIZE);
  if (!rte_services) {
 ...
 */
static inline int service_valid(uint32_t id) {
  return !!(rte_services[id].internal_flags & SERVICE_F_REGISTERED);
}

static struct rte_service_spec_impl *service_get(uint32_t id) {
  return &rte_services[id];
}

/* validate ID and retrieve service pointer, or return error value */
#define SERVICE_VALID_GET_OR_ERR_RET(id, service, retval)                      \
  do {                                                                         \
 ...
 */
static inline int service_stats_enabled(struct rte_service_spec_impl *impl) {
  return !!(impl->internal_flags & SERVICE_F_STATS_ENABLED);
}

static inline int service_mt_safe(struct rte_service_spec_impl *s) {
...
uint32_t rte_service_get_count(void) { return rte_service_count; }
...
int32_t rte_service_get_by_name(const char *name, uint32_t *service_id) {
  ...
  for (i = 0; i < RTE_SERVICE_NUM_MAX; i++) {
    if (service_valid(i) && strcmp(name, rte_services[i].spec.name) == 0) {
      *service_id = i;
      return 0;
    }
  }
...
int32_t rte_service_component_runstate_set(uint32_t id, uint32_t runstate) {
   ...
   */
  if (runstate)
    __atomic_store_n(&s->comp_runstate, RUNSTATE_RUNNING, __ATOMIC_RELEASE);
  else
    __atomic_store_n(&s->comp_runstate, RUNSTATE_STOPPED, __ATOMIC_RELEASE);
...
int32_t rte_service_runstate_set(uint32_t id, uint32_t runstate) {
   ...
   */
  if (runstate)
    __atomic_store_n(&s->app_runstate, RUNSTATE_RUNNING, __ATOMIC_RELEASE);
  else
    __atomic_store_n(&s->app_runstate, RUNSTATE_STOPPED, __ATOMIC_RELEASE);
...
int32_t rte_service_runstate_get(uint32_t id) {
  ...
  if (__atomic_load_n(&s->comp_runstate, __ATOMIC_ACQUIRE) ==
          RUNSTATE_RUNNING &&
      __atomic_load_n(&s->app_runstate, __ATOMIC_ACQUIRE) == RUNSTATE_RUNNING) {
    int check_disabled = !(s->internal_flags & SERVICE_F_START_CHECK);
    int lcore_mapped =
        (__atomic_load_n(&s->num_mapped_cores, __ATOMIC_RELAXED) > 0);
...
static inline void service_runner_do_callback(struct rte_service_spec_impl *s,
...
static int32_t service_run(uint32_t i, struct core_state *cs,
                           uint64_t service_mask,
                           struct rte_service_spec_impl *s,
...
static int32_t service_runner_func(void *arg) {
  RTE_SET_USED(arg);
  uint32_t i;
   ...
   */
  while (__atomic_load_n(&cs->runstate, __ATOMIC_ACQUIRE) == RUNSTATE_RUNNING) {
...
int32_t rte_service_lcore_may_be_active(uint32_t lcore) {
   ...
   */
  return __atomic_load_n(&lcore_states[lcore].thread_active, __ATOMIC_ACQUIRE);
}
...
static int32_t service_update(uint32_t sid, uint32_t lcore, uint32_t *set,
                              uint32_t *enabled) {
  ...
  if (set) {
    uint64_t lcore_mapped = lcore_states[lcore].service_mask & sid_mask;

    if (*set && !lcore_mapped) {
      lcore_states[lcore].service_mask |= sid_mask;
      __atomic_add_fetch(&rte_services[sid].num_mapped_cores, 1,
                         __ATOMIC_RELAXED);
    }
    if (!*set && lcore_mapped) {
      lcore_states[lcore].service_mask &= ~(sid_mask);
      __atomic_sub_fetch(&rte_services[sid].num_mapped_cores, 1,
                         __ATOMIC_RELAXED);
    }
  }
...
int32_t rte_service_map_lcore_set(uint32_t id, uint32_t lcore,
                                  uint32_t enabled) {
  uint32_t on = enabled > 0;
  return service_update(id, lcore, &on, 0);
}

int32_t rte_service_map_lcore_get(uint32_t id, uint32_t lcore) {
  uint32_t enabled;
...
static void set_lcore_state(uint32_t lcore, int32_t state) {
...
int32_t rte_service_lcore_reset_all(void) {
  ...
  for (i = 0; i < RTE_MAX_LCORE; i++) {
    if (lcore_states[i].is_service_core) {
       ...
       */
      __atomic_store_n(&lcore_states[i].runstate, RUNSTATE_STOPPED,
                       __ATOMIC_RELEASE);
    }
  }
  for (i = 0; i < RTE_SERVICE_NUM_MAX; i++)
    __atomic_store_n(&rte_services[i].num_mapped_cores, 0, __ATOMIC_RELAXED);
...
int32_t rte_service_lcore_del(uint32_t lcore) {
   ...
   * in runstate update functions.
   */
  if (__atomic_load_n(&cs->runstate, __ATOMIC_ACQUIRE) != RUNSTATE_STOPPED)
    return -EBUSY;
...
int32_t rte_service_lcore_start(uint32_t lcore) {
  if (lcore >= RTE_MAX_LCORE)
    return -EINVAL;
   ...
   * in runstate update functions.
   */
  if (__atomic_load_n(&cs->runstate, __ATOMIC_ACQUIRE) == RUNSTATE_RUNNING)
    return -EALREADY;
...
int32_t rte_service_lcore_stop(uint32_t lcore) {
  if (lcore >= RTE_MAX_LCORE)
    return -EINVAL;
  ...
  for (i = 0; i < RTE_SERVICE_NUM_MAX; i++) {
    ...
    int32_t only_core = (1 == __atomic_load_n(&rte_services[i].num_mapped_cores,
                                              __ATOMIC_RELAXED));
...
static void service_dump_one(FILE *f, struct rte_service_spec_impl *s) {
  ...
  if (s->calls != 0)
    calls = s->calls;
  fprintf(f,
          "  %s: stats %d\tcalls %" PRIu64 "\tcycles %" PRIu64 "\tavg: %" PRIu64
          "\n",
          s->spec.name, service_stats_enabled(s), s->calls, s->cycles_spent,
          s->cycles_spent / calls);
}

static void service_dump_calls_per_lcore(FILE *f, uint32_t lcore) {
  uint32_t i;
  struct core_state *cs = &lcore_states[lcore];
