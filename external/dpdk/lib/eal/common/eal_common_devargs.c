...
static int devargs_bus_parse_default(struct rte_devargs *devargs,
  ...
  while (s != NULL) {
    ...
    while (s != NULL) {
      ...
      for (i = 0; i < RTE_DIM(layers); i++) {
        if (strncmp(s + 1, layers[i].key, strlen(layers[i].key)) == 0) {
          *s = '\0';
          break;
        }
      }
    ...
    layers[nblayer].kvlist = rte_kvargs_parse(layers[nblayer].str, NULL);
...
static int bus_name_cmp(const struct rte_bus *bus, const void *name) {
...
struct rte_devargs *rte_devargs_next(const char *busname,
  ...
  while (da != NULL) {
    if (busname == NULL || (strcmp(busname, da->bus->name) == 0))
      return da;
