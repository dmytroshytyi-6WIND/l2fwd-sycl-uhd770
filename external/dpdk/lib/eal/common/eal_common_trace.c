...
static struct trace trace = {
    .args = STAILQ_HEAD_INITIALIZER(trace.args),
};
...
bool rte_trace_is_enabled(void) { return trace.status; }

static void trace_mode_set(rte_trace_point_t *trace, enum rte_trace_mode mode) {
...
enum rte_trace_mode rte_trace_mode_get(void) { return trace.mode; }

static bool trace_point_is_invalid(rte_trace_point_t *t) {
  return (t == NULL) || (trace_id_get(t) >= trace.nb_trace_points);
}

bool rte_trace_point_is_enabled(rte_trace_point_t *trace) {
  uint64_t val;
...
int rte_trace_point_enable(rte_trace_point_t *trace) {
  if (trace_point_is_invalid(trace))
    return -ERANGE;

  __atomic_or_fetch(trace, __RTE_TRACE_FIELD_ENABLE_MASK, __ATOMIC_RELEASE);
  return 0;
}

int rte_trace_point_disable(rte_trace_point_t *trace) {
  if (trace_point_is_invalid(trace))
    return -ERANGE;

  __atomic_and_fetch(trace, ~__RTE_TRACE_FIELD_ENABLE_MASK, __ATOMIC_RELEASE);
  return 0;
}
...
static void trace_point_dump(FILE *f, struct trace_point *tp) {
...
static void trace_lcore_mem_dump(FILE *f) {
  ...
  for (count = 0; count < trace->nb_trace_mem_list; count++) {
    ...
    fprintf(f, "\tid %d, mem=%p, area=%s, lcore_id=%d, name=%s\n", count,
            header, trace_area_to_string(trace->lcore_meta[count].area),
            header->stream_header.lcore_id, header->stream_header.thread_name);
  }
...
void __rte_trace_mem_per_thread_alloc(void) {
  ...
  trace->lcore_meta =
      realloc(trace->lcore_meta, sizeof(trace->lcore_meta[0]) * (count + 1));
...
static void trace_mem_per_thread_free_unlocked(struct thread_mem_meta *meta) {
...
void trace_mem_per_thread_free(void) {
  ...
  if (count != trace->nb_trace_mem_list) {
    ...
    if (count != trace->nb_trace_mem_list - 1) {
      memmove(meta, meta + 1,
              sizeof(*meta) * (trace->nb_trace_mem_list - count - 1));
    }
    trace->nb_trace_mem_list--;
  }
  rte_spinlock_unlock(&trace->lock);
...
void __rte_trace_point_emit_field(size_t sz, const char *in,
                                  const char *datatype) {
  char *field;
  char *fixup;
  int rc;
  ...
  if (fixup != NULL)
    in = fixup;
  rc =
      asprintf(&field, "%s        %s %s;\n",
               RTE_PER_LCORE(ctf_field) != NULL ? RTE_PER_LCORE(ctf_field) : "",
               datatype, in);
...
int __rte_trace_point_register(rte_trace_point_t *handle, const char *name,
