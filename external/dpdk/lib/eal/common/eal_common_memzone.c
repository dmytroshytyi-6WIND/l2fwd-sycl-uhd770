...
#define MEMZONE_KNOWN_FLAGS                                                    \
  (RTE_MEMZONE_2MB | RTE_MEMZONE_1GB | RTE_MEMZONE_16MB | RTE_MEMZONE_16GB |   \
   RTE_MEMZONE_256KB | RTE_MEMZONE_256MB | RTE_MEMZONE_512MB |                 \
   RTE_MEMZONE_4GB | RTE_MEMZONE_SIZE_HINT_ONLY | RTE_MEMZONE_IOVA_CONTIG)
...
memzone_reserve_aligned_thread_unsafe(const char *name, size_t len,
  ...
  if (len == 0 && bound == 0) {
    ...
    mz_addr = malloc_heap_alloc(NULL, requested_len, socket_id, flags, align,
                                bound, contig);
  }
  if (mz_addr == NULL) {
    rte_errno = ENOMEM;
  ...
  mz->addr = mz_addr;
  mz->len = requested_len == 0 ? elem->size - elem->pad - MALLOC_ELEM_OVERHEAD
                               : requested_len;
...
rte_memzone_reserve_thread_safe(const char *name, size_t len, int socket_id,
  ...
  mz = memzone_reserve_aligned_thread_unsafe(name, len, socket_id, flags, align,
                                             bound);

  rte_eal_trace_memzone_reserve(name, len, socket_id, flags, align, bound, mz);
...
rte_memzone_reserve_bounded(const char *name, size_t len, int socket_id,
  ...
  return rte_memzone_reserve_thread_safe(name, len, socket_id, flags, align,
                                         bound);
}
...
static void dump_memzone(const struct rte_memzone *mz, void *arg) {
  ...
  fprintf(f,
          "Zone %u: name:<%s>, len:0x%zx, virt:%p, "
          "socket_id:%" PRId32 ", flags:%" PRIx32 "\n",
          mz_idx, mz->name, mz->len, mz->addr, mz->socket_id, mz->flags);
...
void rte_memzone_dump(FILE *f) { rte_memzone_walk(dump_memzone, f); }
...
int rte_eal_memzone_init(void) {
  ...
  if (rte_eal_process_type() == RTE_PROC_PRIMARY &&
      rte_fbarray_init(&mcfg->memzones, "memzone", RTE_MAX_MEMZONE,
