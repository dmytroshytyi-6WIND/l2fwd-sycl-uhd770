...
void *eal_get_virtual_area(void *requested_addr, size_t *size, size_t page_sz,
                           int flags, int reserve_flags) {
  ...
  do {
    ...
    mapped_addr =
        eal_mem_reserve(requested_addr, (size_t)map_sz, reserve_flags);
  ...
  } while ((allow_shrink || addr_is_hint) && (mapped_addr == NULL) &&
           (*size > 0));

  /* align resulting address - if map failed, we will ignore the value
   * anyway, so no need to add additional checks.
   */
  aligned_addr = no_align ? mapped_addr : RTE_PTR_ALIGN(mapped_addr, page_sz);
  ...
  if (*size == 0) {
    ...
    if (internal_conf->base_virtaddr != 0) {
      RTE_LOG(WARNING, EAL,
              ...
              requested_addr, aligned_addr);
      RTE_LOG(WARNING, EAL,
              "   This may cause issues with mapping memory into secondary "
              "processes\n");
    } else {
      RTE_LOG(DEBUG, EAL,
              "WARNING! Base virtual address hint (%p != %p) not respected!\n",
...
int eal_memseg_list_init_named(struct rte_memseg_list *msl, const char *name,
                               uint64_t page_sz, int n_segs, int socket_id,
                               bool heap) {
...
int eal_memseg_list_init(struct rte_memseg_list *msl, uint64_t page_sz,
  ...
  return eal_memseg_list_init_named(msl, name, page_sz, n_segs, socket_id,
                                    heap);
}

int eal_memseg_list_alloc(struct rte_memseg_list *msl, int reserve_flags) {
...
void eal_memseg_list_populate(struct rte_memseg_list *msl, void *addr,
                              int n_segs) {
...
static struct rte_memseg *virt2memseg(const void *addr,
...
static struct rte_memseg_list *virt2memseg_list(const void *addr) {
...
static int find_virt(const struct rte_memseg_list *msl __rte_unused,
...
static int find_virt_legacy(const struct rte_memseg_list *msl __rte_unused,
...
struct rte_memseg *rte_mem_virt2memseg(const void *addr,
                                       const struct rte_memseg_list *msl) {
  return virt2memseg(addr, msl != NULL ? msl : rte_mem_virt2memseg_list(addr));
}

static int physmem_size(const struct rte_memseg_list *msl, void *arg) {
...
static int dump_memseg(const struct rte_memseg_list *msl,
  ...
  fprintf(f,
          "Segment %i-%i: IOVA:0x%" PRIx64 ", len:%zu, "
          "virt:%p, socket_id:%" PRId32 ", "
          "hugepage_sz:%" PRIu64 ", nchannel:%" PRIx32 ", "
          "nrank:%" PRIx32 " fd:%i\n",
          msl_idx, ms_idx, ms->iova, ms->len, ms->addr, ms->socket_id,
          ms->hugepage_sz, ms->nchannel, ms->nrank, fd);
...
int rte_mem_alloc_validator_register(const char *name,
  ...
  return eal_memalloc_mem_alloc_validator_register(name, clb, socket_id, limit);
}
...
void rte_dump_physmem_layout(FILE *f) { rte_memseg_walk(dump_memseg, f); }

static int check_iova(const struct rte_memseg_list *msl __rte_unused,
                      const struct rte_memseg *ms, void *arg) {
  uint64_t *mask = arg;
  rte_iova_t iova;
...
static int check_dma_mask(uint8_t maskbits, bool thread_unsafe) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
  uint64_t mask;
  int ret;
   ...
   */
  mcfg->dma_maskbits = mcfg->dma_maskbits == 0
                           ? maskbits
...
int rte_memseg_contig_walk_thread_unsafe(rte_memseg_contig_walk_t func,
  ...
  for (i = 0; i < RTE_MAX_MEMSEG_LISTS; i++) {
    ...
    while (ms_idx >= 0) {
      ...
      if (ret)
        return ret;
      ms_idx = rte_fbarray_find_next_used(arr, ms_idx + n_segs);
    }
  }
  return 0;
}
...
int rte_extmem_register(void *va_addr, size_t len, rte_iova_t iova_addrs[],
  ...
  if (va_addr == NULL || page_sz == 0 || len == 0 ||
      !rte_is_power_of_2(page_sz) || RTE_ALIGN(len, page_sz) != len ||
  ...
  n = len / page_sz;
  if (malloc_heap_create_external_seg(va_addr, iova_addrs, n, page_sz, "extmem",
                                      socket_id) == NULL) {
    ret = -1;
    goto unlock;
  }
...
static int sync_memory(void *va_addr, size_t len, bool attach) {
  struct rte_memseg_list *msl;
  int ret = 0;
...
static int handle_eal_heap_info_request(const char *cmd __rte_unused,
                                        const char *params,
  ...
  rte_tel_data_add_dict_u64(d, "Heap_size", sock_stats.heap_totalsz_bytes);
  rte_tel_data_add_dict_u64(d, "Free_size", sock_stats.heap_freesz_bytes);
  rte_tel_data_add_dict_u64(d, "Alloc_size", sock_stats.heap_allocsz_bytes);
...
static int handle_eal_heap_list_request(const char *cmd __rte_unused,
...
static int handle_eal_memzone_info_request(const char *cmd __rte_unused,
                                           const char *params,
                                           struct rte_tel_data *d) {
  struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
...
static void memzone_list_cb(const struct rte_memzone *mz __rte_unused,
...
static int handle_eal_memzone_list_request(const char *cmd __rte_unused,
                                           const char *params __rte_unused,
                                           struct rte_tel_data *d) {
  rte_tel_data_start_array(d, RTE_TEL_INT_VAL);
...
RTE_INIT(memory_telemetry) {
  ...
  rte_telemetry_register_cmd(EAL_MEMZONE_INFO_REQ,
                             handle_eal_memzone_info_request,
                             "Returns memzone info. Parameters: int mz_id");
  rte_telemetry_register_cmd(EAL_HEAP_LIST_REQ, handle_eal_heap_list_request,
