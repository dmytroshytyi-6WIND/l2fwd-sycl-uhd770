...
int eal_dynmem_memseg_lists_init(void) {
  ...
  max_mem_per_type =
      RTE_MIN((uint64_t)RTE_MAX_MEM_MB_PER_TYPE << 20, max_mem / n_memtypes);
  /*
  ...
  for (cur_type = 0; cur_type < n_memtypes; cur_type++) {
    ...
    max_segs_per_type =
        RTE_MIN(max_segs_per_type, (unsigned int)RTE_MAX_MEMSEG_PER_TYPE);
    /* limit number of segments to maximum allowed per list */
    max_segs_per_list =
        RTE_MIN(max_segs_per_type, (unsigned int)RTE_MAX_MEMSEG_PER_LIST);
    ...
    for (cur_seglist = 0; cur_seglist < n_seglists; cur_seglist++) {
      if (msl_idx >= RTE_MAX_MEMSEG_LISTS) {
        RTE_LOG(ERR, EAL, "No more space in memseg lists, please increase %s\n",
                RTE_STR(RTE_MAX_MEMSEG_LISTS));
        goto out;
      }
      msl = &mcfg->memsegs[msl_idx++];

      if (eal_memseg_list_init(msl, pagesz, n_segs, socket_id, cur_seglist,
                               true))
        goto out;
...
static int __rte_unused hugepage_count_walk(const struct rte_memseg_list *msl,
                                            void *arg) {
...
static int limits_callback(int socket_id, size_t cur_limit, size_t new_len) {
...
int eal_dynmem_hugepage_init(void) {
  ...
  for (hp_sz_idx = 0; hp_sz_idx < (int)internal_conf->num_hugepage_sizes;
       hp_sz_idx++) {
#ifndef RTE_ARCH_64
  ...
  if (eal_dynmem_calc_num_pages_per_socket(
  ...
  for (hp_sz_idx = 0; hp_sz_idx < (int)internal_conf->num_hugepage_sizes;
    ...
    for (socket_id = 0; socket_id < RTE_MAX_NUMA_NODES; socket_id++) {
      ...
      do {
        ...
        cur_pages = eal_memalloc_alloc_seg_bulk(pages, needed, hpi->hugepage_sz,
                                                socket_id, false);
        if (cur_pages <= 0) {
          free(pages);
          return -1;
  ...
  if (internal_conf->force_socket_limits) {
    ...
    for (i = 0; i < RTE_MAX_NUMA_NODES; i++) {
      ...
      if (limit == 0)
        continue;
      if (rte_mem_alloc_validator_register("socket-limit", limits_callback, i,
                                           limit))
        RTE_LOG(ERR, EAL,
...
int eal_dynmem_calc_num_pages_per_socket(uint64_t *memory,
  ...
  if (internal_conf->force_sockets == 0) {
    ...
    for (socket = 0; socket < RTE_MAX_NUMA_NODES && total_size != 0; socket++) {
      ...
      default_size =
          internal_conf->memory * cpu_per_socket[socket] / rte_lcore_count();
    ...
    for (socket = 0; socket < RTE_MAX_NUMA_NODES && total_size != 0; socket++) {
      ...
      default_size =
          RTE_MIN(get_socket_mem_size(socket) - memory[socket], total_size);
  ...
  for (socket = 0; socket < RTE_MAX_NUMA_NODES && total_mem != 0; socket++) {
    ...
    for (i = 0; i < num_hp_info && memory[socket] != 0; i++) {
      ...
      hp_used[i].num_pages[socket] =
          RTE_MIN(memory[socket] / hp_info[i].hugepage_sz,
      ...
      for (j = i + 1; j < num_hp_info; j++)
        remaining_mem += hp_info[j].hugepage_sz * hp_info[j].num_pages[socket];
    ...
    if (memory[socket] > 0 && internal_conf->socket_mem[socket] != 0) {
      /* to prevent icc errors */
      requested = (unsigned int)(internal_conf->socket_mem[socket] / 0x100000);
      available = requested - ((unsigned int)(memory[socket] / 0x100000));
      RTE_LOG(ERR, EAL,
              "Not enough memory available on "
