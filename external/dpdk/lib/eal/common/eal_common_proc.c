...
struct pending_request {
  ...
  enum { REQUEST_TYPE_SYNC, REQUEST_TYPE_ASYNC } type;
  char dst[PATH_MAX];
...
static int mp_send(struct rte_mp_msg *msg, const char *peer, int type);

/* for use with alarm callback */
static void async_reply_handle(void *arg);

/* for use with process_msg */
static struct pending_request *async_reply_handle_thread_unsafe(void *arg);

static void trigger_async_action(struct pending_request *req);

static struct pending_request *find_pending_request(const char *dst,
                                                    const char *act_name) {
  struct pending_request *r;

  TAILQ_FOREACH(r, &pending_requests.requests, next) {
    if (!strcmp(r->dst, dst) && !strcmp(r->request->name, act_name))
      break;
  }
...
static void create_socket_path(const char *name, char *buf, int len) {
...
static struct action_entry *find_action_entry_by_name(const char *name) {
...
static int validate_action_name(const char *name) {
  if (name == NULL) {
...
static enum async_action process_async_request(struct pending_request *sr,
  ...
  if (sr->reply_received == 1 && sr->reply) {
    ...
    tmp = realloc(user_msgs, sizeof(*msg) * (reply->nb_received + 1));
    if (!tmp) {
      ...
      memcpy(&user_msgs[reply->nb_received], msg, sizeof(*msg));
      reply->nb_received++;
    }
...
static void trigger_async_action(struct pending_request *sr) {
  struct async_request_param *param;
  struct rte_mp_reply *reply;
...
static int open_socket_fd(void) {
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY)
    snprintf(peer_name, sizeof(peer_name), "%d_%" PRIx64, getpid(),
             rte_rdtsc());
...
int rte_mp_channel_init(void) {
  ...
  if (rte_ctrl_thread_create(&mp_handle_tid, "rte_mp_handle", NULL, mp_handle,
                             NULL) < 0) {
 ...
 *
 */
static int send_msg(const char *dst_path, struct rte_mp_msg *msg, int type) {
  int snd;
  struct iovec iov;
  struct msghdr msgh;
  struct cmsghdr *cmsg;
  ...
  if (snd < 0) {
    ...
    if (errno == ECONNREFUSED && rte_eal_process_type() == RTE_PROC_PRIMARY) {
      unlink(dst_path);
      return 0;
    }
...
static int mp_send(struct rte_mp_msg *msg, const char *peer, int type) {
  ...
  while ((ent = readdir(mp_dir))) {
    ...
    snprintf(path, sizeof(path), "%s/%s", mp_dir_path, ent->d_name);
...
static int check_input(const struct rte_mp_msg *msg) {
  if (msg == NULL) {
...
static int mp_request_async(const char *dst, struct rte_mp_msg *req,
                            struct async_request_param *param,
...
static int mp_request_sync(const char *dst, struct rte_mp_msg *req,
                           struct rte_mp_reply *reply,
                           const struct timespec *ts) {
  int ret;
  ...
  do {
    ret = pthread_cond_timedwait(&pending_req.sync.cond, &pending_requests.lock,
                                 ts);
...
int rte_mp_request_sync(struct rte_mp_msg *req, struct rte_mp_reply *reply,
  ...
  if (clock_gettime(CLOCK_MONOTONIC, &now) < 0) {
    ...
    goto end;
      ...
      now.tv_sec + ts->tv_sec + (now.tv_nsec + ts->tv_nsec) / 1000000000;
...
static bool set_mp_status(enum mp_status status) {
  ...
  desired = status;
  if (__atomic_compare_exchange_n(&mcfg->mp_status, &expected, desired, false,
...
bool rte_mp_disable(void) { return set_mp_status(MP_STATUS_DISABLED); }
