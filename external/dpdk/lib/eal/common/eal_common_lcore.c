...
static int socket_id_cmp(const void *a, const void *b) {
...
int rte_eal_cpu_init(void) {
  ...
  for (; lcore_id < CPU_SETSIZE; lcore_id++) {
    ...
    RTE_LOG(DEBUG, EAL, "Skipped lcore %u as core %u on socket %u\n", lcore_id,
            eal_cpu_core_id(lcore_id), eal_cpu_socket_id(lcore_id));
  }

  /* Set the count of enabled logical cores of the EAL configuration */
  config->lcore_count = count;
  RTE_LOG(DEBUG, EAL, "Maximum logical cores by configuration: %u\n",
          RTE_MAX_LCORE);
...
static TAILQ_HEAD(lcore_callbacks_head, lcore_callback)
    lcore_callbacks = TAILQ_HEAD_INITIALIZER(lcore_callbacks);

static int callback_init(struct lcore_callback *callback,
...
static void callback_uninit(struct lcore_callback *callback,
                            unsigned int lcore_id) {
...
static void free_callback(struct lcore_callback *callback) {
  free(callback->name);
  free(callback);
}
...
static int lcore_dump_cb(unsigned int lcore_id, void *arg) {
  struct rte_config *cfg = rte_eal_get_configuration();
...
void rte_lcore_dump(FILE *f) { rte_lcore_iterate(lcore_dump_cb, f); }
