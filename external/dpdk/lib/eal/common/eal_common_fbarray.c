...
static size_t calc_data_size(size_t page_sz, unsigned int elt_sz,
                             unsigned int len) {
...
static struct used_mask *get_used_mask(void *data, unsigned int elt_sz,
                                       unsigned int len) {
  return (struct used_mask *)RTE_PTR_ADD(data, elt_sz * len);
}

static int resize_and_map(int fd, const char *path, void *addr, size_t len) {
  void *map_addr;
...
static int overlap(const struct mem_area *ma, const void *start, size_t len) {
...
static int find_next_n(const struct rte_fbarray *arr, unsigned int start,
...
static int find_next(const struct rte_fbarray *arr, unsigned int start,
                     bool used) {
  const struct used_mask *msk = get_used_mask(arr->data, arr->elt_sz, arr->len);
...
static int find_contig(const struct rte_fbarray *arr, unsigned int start,
                       bool used) {
  const struct used_mask *msk = get_used_mask(arr->data, arr->elt_sz, arr->len);
  unsigned int idx, first, first_mod;
  unsigned int last, last_mod;
...
static int find_prev_n(const struct rte_fbarray *arr, unsigned int start,
                       unsigned int n, bool used) {
  const struct used_mask *msk = get_used_mask(arr->data, arr->elt_sz, arr->len);
  ...
  ignore_msk = first_mod == MASK_ALIGN - 1 ? UINT64_MAX : /* prevent overflow */
  ...
  do {
    ...
    if (n <= MASK_ALIGN) {
      ...
      if (tmp_msk != 0) {
         ...
         * would have been.
         */
        run_start = MASK_ALIGN - __builtin_clzll(tmp_msk) - n;
        return MASK_GET_IDX(msk_idx, run_start);
      }
    }
...
static int find_prev(const struct rte_fbarray *arr, unsigned int start,
                     bool used) {
  const struct used_mask *msk = get_used_mask(arr->data, arr->elt_sz, arr->len);
  unsigned int idx, first, first_mod;
  uint64_t ignore_msk;
...
static int find_rev_contig(const struct rte_fbarray *arr, unsigned int start,
                           bool used) {
  const struct used_mask *msk = get_used_mask(arr->data, arr->elt_sz, arr->len);
  unsigned int idx, first, first_mod;
  unsigned int need_len, result = 0;
...
static int fully_validate(const char *name, unsigned int elt_sz,
                          unsigned int len) {
...
int rte_fbarray_init(struct rte_fbarray *arr, const char *name,
  ...
  if (internal_conf->no_shconf) {
    ...
    static const int flags =
        RTE_MAP_FORCE_ADDRESS | RTE_MAP_PRIVATE | RTE_MAP_ANONYMOUS;
    void *new_data = rte_mem_map(data, mmap_len, RTE_PROT_READ | RTE_PROT_WRITE,
                                 flags, fd, 0);
    if (new_data == NULL) {
...
static int fbarray_find(struct rte_fbarray *arr, unsigned int start, bool next,
                        bool used) {
  int ret = -1;
...
static int fbarray_find_n(struct rte_fbarray *arr, unsigned int start,
                          unsigned int n, bool next, bool used) {
  int ret = -1;
...
int rte_fbarray_find_next_n_free(struct rte_fbarray *arr, unsigned int start,
                                 unsigned int n) {
  return fbarray_find_n(arr, start, n, true, false);
}

int rte_fbarray_find_next_n_used(struct rte_fbarray *arr, unsigned int start,
                                 unsigned int n) {
  return fbarray_find_n(arr, start, n, true, true);
}

int rte_fbarray_find_prev_n_free(struct rte_fbarray *arr, unsigned int start,
                                 unsigned int n) {
  return fbarray_find_n(arr, start, n, false, false);
}

int rte_fbarray_find_prev_n_used(struct rte_fbarray *arr, unsigned int start,
                                 unsigned int n) {
  return fbarray_find_n(arr, start, n, false, true);
}

static int fbarray_find_contig(struct rte_fbarray *arr, unsigned int start,
                               bool next, bool used) {
  int ret = -1;
...
static int fbarray_find_biggest(struct rte_fbarray *arr, unsigned int start,
                                bool used, bool rev) {
