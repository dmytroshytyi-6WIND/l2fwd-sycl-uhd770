...
static struct rte_bus_list rte_bus_list = TAILQ_HEAD_INITIALIZER(rte_bus_list);
...
static int bus_dump_one(FILE *f, struct rte_bus *bus) {
  int ret;
...
static int cmp_rte_device(const struct rte_device *dev1, const void *_dev2) {
  const struct rte_device *dev2 = _dev2;

  return dev1 != dev2;
}

static int bus_find_device(const struct rte_bus *bus, const void *_dev) {
...
static int cmp_bus_name(const struct rte_bus *bus, const void *_name) {
...
static int bus_can_parse(const struct rte_bus *bus, const void *_name) {
  const char *name = _name;
...
enum rte_iova_mode rte_bus_get_iommu_class(void) {
  ...
  if (buses_want_va && !buses_want_pa) {
    ...
    if (buses_want_va) {
      RTE_LOG(WARNING, EAL,
              "Some buses want 'VA' but forcing 'DC' because other buses want "
              "'PA'.\n");
      RTE_LOG(WARNING, EAL,
              "Depending on the final decision by the EAL, not all buses may "
...
static int bus_handle_sigbus(const struct rte_bus *bus,
