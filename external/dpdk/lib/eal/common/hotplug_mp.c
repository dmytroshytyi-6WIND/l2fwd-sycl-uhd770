...
static int send_response_to_secondary(const struct eal_dev_mp_req *req,
                                      int result, const void *peer) {
  struct rte_mp_msg mp_resp;
  struct eal_dev_mp_req *resp = (struct eal_dev_mp_req *)mp_resp.param;
  int ret;
...
static void __handle_secondary_request(void *param) {
  struct mp_reply_bundle *bundle = param;
  const struct rte_mp_msg *msg = &bundle->msg;
  const struct eal_dev_mp_req *req = (const struct eal_dev_mp_req *)msg->param;
...
static int handle_primary_request(const struct rte_mp_msg *msg,
                                  const void *peer) {
  struct rte_mp_msg mp_resp;
  const struct eal_dev_mp_req *req = (const struct eal_dev_mp_req *)msg->param;
  struct eal_dev_mp_req *resp = (struct eal_dev_mp_req *)mp_resp.param;
...
int eal_dev_hotplug_request_to_secondary(struct eal_dev_mp_req *req) {
  ...
  for (i = 0; i < mp_reply.nb_received; i++) {
    ...
    if (resp->result != 0) {
      if (req->t == EAL_DEV_REQ_TYPE_ATTACH && resp->result == -EEXIST)
        continue;
      if (req->t == EAL_DEV_REQ_TYPE_DETACH && resp->result == -ENOENT)
        continue;
      req->result = resp->result;
    }
  }
