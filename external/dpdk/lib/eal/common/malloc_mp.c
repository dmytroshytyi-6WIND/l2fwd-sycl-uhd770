...
static int handle_sync_response(const struct rte_mp_msg *request,
                                const struct rte_mp_reply *reply);
static int handle_rollback_response(const struct rte_mp_msg *request,
                                    const struct rte_mp_reply *reply);
...
enum req_state { REQ_STATE_INACTIVE = 0, REQ_STATE_ACTIVE, REQ_STATE_COMPLETE };
...
} mp_request_list = {.list = TAILQ_HEAD_INITIALIZER(mp_request_list.list),
...
static struct mp_request *find_request_by_id(uint64_t id) {
...
static int handle_sync(const struct rte_mp_msg *msg, const void *peer) {
  ...
  const struct malloc_mp_req *req = (const struct malloc_mp_req *)msg->param;
  struct malloc_mp_req *resp = (struct malloc_mp_req *)reply.param;
  int ret;
...
static int handle_free_request(const struct malloc_mp_req *m) {
  ...
  eal_memalloc_mem_event_notify(RTE_MEM_EVENT_FREE, m->free_req.addr,
                                m->free_req.len);
...
static int handle_alloc_request(const struct malloc_mp_req *m,
                                struct mp_request *req) {
  ...
  alloc_sz = RTE_ALIGN_CEIL(ar->align + ar->elt_size + MALLOC_ELEM_TRAILER_LEN,
                            ar->page_sz);
...
static int handle_request(const struct rte_mp_msg *msg,
                          const void *peer __rte_unused) {
  const struct malloc_mp_req *m = (const struct malloc_mp_req *)msg->param;
  ...
  if (ret != 0) {
    ...
    struct malloc_mp_req *resp = (struct malloc_mp_req *)resp_msg.param;

    /* send failure message straight away */
    resp_msg.num_fds = 0;
    resp_msg.len_param = sizeof(*resp);
    strlcpy(resp_msg.name, MP_ACTION_RESPONSE, sizeof(resp_msg.name));
    ...
    struct malloc_mp_req *sr = (struct malloc_mp_req *)sr_msg.param;
    struct timespec ts;
...
static int handle_sync_response(const struct rte_mp_msg *request,
  ...
  for (i = 0; i < reply->nb_received; i++) {
    struct malloc_mp_req *resp = (struct malloc_mp_req *)reply->msgs[i].param;
  ...
  if (entry->user_req.t == REQ_TYPE_FREE) {
    ...
    free(entry->alloc_state.ms);
    free(entry);
  } else if (entry->user_req.t == REQ_TYPE_ALLOC && result == REQ_RESULT_FAIL) {
    struct rte_mp_msg rb_msg;
    struct malloc_mp_req *rb = (struct malloc_mp_req *)rb_msg.param;
    struct timespec ts;
    struct primary_alloc_req_state *state = &entry->alloc_state;
    int ret;

    memset(&rb_msg, 0, sizeof(rb_msg));

    /* we've failed to sync, so do a rollback */
    eal_memalloc_mem_event_notify(RTE_MEM_EVENT_FREE, state->map_addr,
                                  state->map_len);

    rollback_expand_heap(state->ms, state->ms_len, state->elem, state->map_addr,
                         state->map_len);
    ...
    /* there may be stray timeout still waiting */
    do {
      ret = rte_mp_request_async(&rb_msg, &ts, handle_rollback_response);
    } while (ret != 0 && rte_errno == EEXIST);
    if (ret != 0) {
      RTE_LOG(ERR, EAL,
...
static int handle_response(const struct rte_mp_msg *msg,
                           const void *peer __rte_unused) {
  const struct malloc_mp_req *m = (const struct malloc_mp_req *)msg->param;
  struct mp_request *entry;
...
int request_to_primary(struct malloc_mp_req *user_req) {
  ...
  ts.tv_sec = now.tv_sec + MP_TIMEOUT_S + (now.tv_usec * 1000) / 1000000000;
  ...
  do {
    ret = pthread_cond_timedwait(&entry->cond, &mp_request_list.lock, &ts);
...
int register_mp_requests(void) {
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    ...
    if (rte_mp_action_register(MP_ACTION_RESPONSE, handle_response)) {
      RTE_LOG(ERR, EAL, "Couldn't register '%s' action\n", MP_ACTION_RESPONSE);
      return -1;
    }
  }
...
void unregister_mp_requests(void) {
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
