...
bool eal_memalloc_is_contig(const struct rte_memseg_list *msl, void *start,
                            size_t len) {
  ...
  if (!rte_eal_has_hugepages()) {
    ...
    start_seg = RTE_PTR_DIFF(aligned_start, msl->base_va) / pgsz;
    end_seg = RTE_PTR_DIFF(aligned_end, msl->base_va) / pgsz;
...
int eal_memalloc_mem_event_callback_register(const char *name,
                                             rte_mem_event_callback_t clb,
                                             void *arg) {
...
int eal_memalloc_mem_event_callback_unregister(const char *name, void *arg) {
  struct mem_event_callback_entry *entry;
  int ret, len;
...
void eal_memalloc_mem_event_notify(enum rte_mem_event event, const void *start,
                                   size_t len) {
  struct mem_event_callback_entry *entry;
...
int eal_memalloc_mem_alloc_validator_register(const char *name,
                                              rte_mem_alloc_validator_t clb,
  ...
  RTE_LOG(DEBUG, EAL,
          "Mem alloc validator '%s' on socket %i with limit %zu registered\n",
...
int eal_memalloc_mem_alloc_validator_unregister(const char *name,
                                                int socket_id) {
  struct mem_alloc_validator_entry *entry;
  int ret, len;
...
int eal_memalloc_mem_alloc_validate(int socket_id, size_t new_len) {
  struct mem_alloc_validator_entry *entry;
  int ret = 0;
