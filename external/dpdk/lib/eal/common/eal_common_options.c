...
const char eal_short_options[] = "a:" /* allow */
                                 "b:" /* block */
                                 "c:" /* coremask */
                                 "s:" /* service coremask */
                                 "d:" /* driver */
...
rte_usage_hook_t eal_get_application_usage_hook(void) {
...
int eal_save_args(int argc, char **argv) {
  ...
  rte_telemetry_register_cmd(
      EAL_PARAM_REQ, handle_eal_info_request,
      "Returns EAL commandline parameters used. Takes no parameters");
  rte_telemetry_register_cmd(
      EAL_APP_PARAM_REQ, handle_eal_info_request,
...
static int eal_option_device_add(enum rte_devtype type, const char *optarg) {
...
const char *eal_get_hugefile_prefix(void) {
...
void eal_reset_internal_config(struct internal_config *internal_cfg) {
  ...
  memset(internal_cfg->vfio_vf_token, 0, sizeof(internal_cfg->vfio_vf_token));
...
static int eal_plugin_add(const char *path) {
...
int eal_plugins_init(void) { return 0; }
#else

static int eal_plugindir_init(const char *path) {
  DIR *d = NULL;
...
static int verify_perms(const char *dirpath) {
  struct stat st;
...
static void *eal_dlopen(const char *pathname) {
  void *retval = NULL;
...
static int is_shared_build(void) {
...
int eal_plugins_init(void) {
   ...
   */
  if (is_shared_build() && *default_solib_dir != '\0' &&
      stat(default_solib_dir, &sb) == 0 && S_ISDIR(sb.st_mode))
...
static int eal_parse_service_coremask(const char *coremask) {
  ...
  for (i = i - 1; i >= 0 && idx < RTE_MAX_LCORE; i--) {
    ...
    for (j = 0; j < BITS_PER_HEX && idx < RTE_MAX_LCORE; j++, idx++) {
      if ((1 << j) & val) {
        ...
        uint32_t lcore = idx;
        if (main_lcore_parsed && cfg->main_lcore == lcore) {
          RTE_LOG(ERR, EAL,
...
static int eal_service_cores_parsed(void) {
  int idx;
...
static int update_lcore_config(int *cores) {
  struct rte_config *cfg = rte_eal_get_configuration();
  unsigned int count = 0;
  unsigned int i;
  int ret = 0;
...
static int check_core_list(int *lcores, unsigned int count) {
   ...
   */
  for (i = 0; i < count; i++) {
    ret = snprintf(&lcorestr[len], sizeof(lcorestr) - len, "%d@%d,", i,
                   lcores[i]);
    if (ret > 0)
      len = len + ret;
  }
...
static int eal_parse_coremask(const char *coremask, int *cores) {
    ...
    for (j = 0; j < BITS_PER_HEX; j++, idx++) {
      if ((1 << j) & val) {
        if (count >= RTE_MAX_LCORE) {
          RTE_LOG(
              ERR, EAL,
              "Too many lcores provided. Cannot exceed RTE_MAX_LCORE (%d)\n",
              RTE_MAX_LCORE);
          return -1;
        }
        lcores[count++] = idx;
...
static int eal_parse_service_corelist(const char *corelist) {
  struct rte_config *cfg = rte_eal_get_configuration();
  int i;
  unsigned count = 0;
  char *end = NULL;
    ...
    if (*end == '-') {
      ...
      for (idx = min; idx <= max; idx++) {
        if (cfg->lcore_role[idx] != ROLE_SERVICE) {
          /* handle main lcore already parsed */
          uint32_t lcore = idx;
          if (cfg->main_lcore == lcore && main_lcore_parsed) {
            RTE_LOG(
                ERR, EAL,
...
static int eal_parse_corelist(const char *corelist, int *cores) {
...
static int eal_parse_main_lcore(const char *arg) {
  ...
  if (lcore_config[cfg->main_lcore].core_role == ROLE_SERVICE) {
    RTE_LOG(ERR, EAL, "Error: Main lcore is used as a service core\n");
    return -1;
  }
 ...
 */
static int eal_parse_set(const char *input, rte_cpuset_t *set) {
  unsigned idx;
...
static int check_cpuset(rte_cpuset_t *set) {
  unsigned int idx;
...
static int eal_parse_lcores(const char *lcores) {
  ...
  do {
    ...
    if (*lcores == '@') {
      ...
      if (offset < (end - lcore_start) && *(lcore_start + offset) != '(')
        lflags = 1;
    }
...
#ifndef RTE_EXEC_ENV_WINDOWS
static int eal_parse_syslog(const char *facility,
                            struct internal_config *conf) {
  int i;
  static const struct {
    const char *name;
    int value;
  } map[] = {{"auth", LOG_AUTH},     {"cron", LOG_CRON},
             {"daemon", LOG_DAEMON}, {"ftp", LOG_FTP},
...
static void eal_log_usage(void) {
  unsigned int level;
...
static int eal_parse_log_priority(const char *level) {
  ...
  /* check for errors */
  if (errno != 0 || end == NULL || *end != '\0' || tmp >= UINT32_MAX)
    return -1;

  return tmp;
}

static int eal_parse_log_level(const char *arg) {
...
static int eal_parse_iova_mode(const char *name) {
  int mode;
...
static int eal_parse_simd_bitwidth(const char *arg) {
  char *end;
...
static int eal_parse_base_virtaddr(const char *arg) {
  char *end;
  uint64_t addr;
  struct internal_config *internal_conf = eal_get_internal_configuration();
...
static char *available_cores(void) {
  char *str = NULL;
  int previous;
  int sequence;
  char *tmp;
...
static int eal_parse_huge_unlink(const char *arg,
                                 struct hugepage_file_discipline *out) {
  ...
  if (strcmp(arg, HUGE_UNLINK_NEVER) == 0) {
    RTE_LOG(WARNING, EAL,
            "Using --" OPT_HUGE_UNLINK "=" HUGE_UNLINK_NEVER
            " may create data leaks.\n");
...
int eal_parse_common_option(int opt, const char *optarg,
  ...
  case 'c': {
    ...
    if (eal_service_cores_parsed())
      RTE_LOG(WARNING, EAL,
              "Service cores parsed before dataplane cores. Please ensure -c "
              "is before -s or -S\n");
  ...
  case 'l': {
    ...
    if (eal_service_cores_parsed())
      RTE_LOG(WARNING, EAL,
              "Service cores parsed before dataplane cores. Please ensure -l "
              "is before -s or -S\n");
  ...
  case OPT_MAIN_LCORE_NUM:
    if (eal_parse_main_lcore(optarg) < 0) {
      RTE_LOG(ERR, EAL, "invalid parameter for --" OPT_MAIN_LCORE "\n");
      return -1;
    }
    break;

  case OPT_VDEV_NUM:
    if (eal_option_device_add(RTE_DEVTYPE_VIRTUAL, optarg) < 0) {
      return -1;
    }
    break;
  ...
  case OPT_SYSLOG_NUM:
    if (eal_parse_syslog(optarg, conf) < 0) {
      RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_SYSLOG "\n");
      return -1;
    }
    break;
#endif
  ...
  case OPT_TRACE_NUM: {
    if (eal_trace_args_save(optarg) < 0) {
      RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_TRACE "\n");
      return -1;
    }
    break;
  }
  ...
  case OPT_TRACE_DIR_NUM: {
    if (eal_trace_dir_args_save(optarg) < 0) {
      RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_TRACE_DIR "\n");
      return -1;
    }
    break;
  }
  ...
  case OPT_TRACE_BUF_SIZE_NUM: {
    if (eal_trace_bufsz_args_save(optarg) < 0) {
      RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_TRACE_BUF_SIZE "\n");
      return -1;
    }
    break;
  }
  ...
  case OPT_TRACE_MODE_NUM: {
    if (eal_trace_mode_args_save(optarg) < 0) {
      RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_TRACE_MODE "\n");
      return -1;
    }
    break;
  }
  ...
  case OPT_IOVA_MODE_NUM:
    if (eal_parse_iova_mode(optarg) < 0) {
      RTE_LOG(ERR, EAL, "invalid parameters for --" OPT_IOVA_MODE "\n");
      return -1;
    }
    break;
  ...
  case OPT_FORCE_MAX_SIMD_BITWIDTH_NUM:
    if (eal_parse_simd_bitwidth(optarg) < 0) {
      RTE_LOG(ERR, EAL,
              "invalid parameter for --" OPT_FORCE_MAX_SIMD_BITWIDTH "\n");
      return -1;
    }
    break;
...
static void eal_auto_detect_cores(struct rte_config *cfg) {
...
static void compute_ctrl_threads_cpuset(struct internal_config *internal_cfg) {
  ...
  if (!CPU_COUNT(cpuset)) {
    memcpy(cpuset, &lcore_config[rte_get_main_lcore()].cpuset, sizeof(*cpuset));
  }
}
...
int eal_check_common_options(struct internal_config *internal_cfg) {
  ...
  if (internal_cfg->user_mbuf_pool_ops_name != NULL &&
    ...
    RTE_LOG(ERR, EAL,
            "Invalid length of --" OPT_MBUF_POOL_OPS_NAME " option\n");
    return -1;
  }
  ...
  if (mem_parsed && internal_cfg->force_sockets == 1) {
    RTE_LOG(ERR, EAL,
            "Options -m and --" OPT_SOCKET_MEM " cannot "
            "be specified at the same time\n");
    return -1;
  }
  if (internal_cfg->no_hugetlbfs && internal_cfg->force_sockets == 1) {
    RTE_LOG(ERR, EAL,
            "Option --" OPT_SOCKET_MEM " cannot "
  ...
  if (internal_cfg->single_file_segments &&
      ...
      !internal_cfg->in_memory) {
    RTE_LOG(ERR, EAL,
            "Option --" OPT_SINGLE_FILE_SEGMENTS " is "
            "not compatible with --" OPT_HUGE_UNLINK "\n");
    return -1;
  }
  if (!internal_cfg->hugepage_file.unlink_existing && internal_cfg->in_memory) {
    RTE_LOG(ERR, EAL,
            "Option --" OPT_IN_MEMORY " is not compatible "
            "with --" OPT_HUGE_UNLINK "=" HUGE_UNLINK_NEVER "\n");
    return -1;
  }
  if (internal_cfg->legacy_mem && internal_cfg->in_memory) {
    RTE_LOG(ERR, EAL,
            "Option --" OPT_LEGACY_MEM " is not compatible "
            "with --" OPT_IN_MEMORY "\n");
    return -1;
  }
  ...
  if (internal_cfg->no_hugetlbfs && internal_cfg->match_allocations) {
    RTE_LOG(ERR, EAL,
            "Option --" OPT_NO_HUGE " is not compatible "
            "with --" OPT_MATCH_ALLOCATIONS "\n");
    return -1;
  }
...
int rte_vect_set_max_simd_bitwidth(uint16_t bitwidth) {
  ...
  if (internal_conf->max_simd_bitwidth.forced) {
    RTE_LOG(NOTICE, EAL,
            "Cannot set max SIMD bitwidth - user runtime override enabled");
    return -EPERM;
  }
...
void eal_common_usage(void) {
  printf(
      ...
      "                      where c1, c2, etc are core indexes between 0 and "
      "%d\n"
      ...
      "                      '@' can be omitted if cpus and lcores have the "
      "same value\n"
      ...
      "                      Only use the specified devices. The argument "
      "format\n"
      "                      for PCI devices is <[domain:]bus:devid.func>.\n"
      "                      This option can be present several times.\n"
      "                      [NOTE: " OPT_DEV_ALLOW
      " cannot be used with " OPT_DEV_BLOCK " option]\n"
      ...
      "                      Enable trace based on regular expression trace "
      "name.\n"
