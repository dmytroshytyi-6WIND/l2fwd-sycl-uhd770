...
__rte_format_printf(2, 0) static int metadata_printf(char **str,
                                                     const char *fmt, ...) {
  va_list ap;
  int rc;
...
static int meta_copy(char **meta, int *offset, char *str, int rc) {
  int count = *offset;
  char *ptr = *meta;
...
static int meta_data_type_emit(char **meta, int *offset) {
  char *str = NULL;
  int rc;
...
static int meta_header_emit(char **meta, int *offset) {
...
static int meta_env_emit(char **meta, int *offset) {
  char *str = NULL;
  int rc;
...
static int meta_clock_pass1_emit(char **meta, int *offset) {
  char *str = NULL;
  int rc;

  rc = metadata_printf(&str, "clock {\n"
                             "    name = \"dpdk\";\n"
                             "    freq = ");
  return meta_copy(meta, offset, str, rc);
}

static int meta_clock_pass2_emit(char **meta, int *offset) {
  char *str = NULL;
  int rc;
...
static int meta_clock_pass3_emit(char **meta, int *offset) {
  char *str = NULL;
  int rc;
...
static int meta_clock_pass4_emit(char **meta, int *offset) {
  char *str = NULL;
  int rc;
...
static int meta_stream_emit(char **meta, int *offset) {
  char *str = NULL;
  int rc;

  rc = metadata_printf(&str, "stream {\n"
...
static int meta_event_emit(char **meta, int *offset, struct trace_point *tp) {
  char *str = NULL;
  int rc;
...
static void meta_fix_freq(struct trace *trace, char *meta) {
  char *str;
  int rc;
...
static void meta_fix_freq_offset(struct trace *trace, char *meta) {
  ...
  uptime_ticks =
      trace->uptime_ticks & ((1ULL << __RTE_TRACE_EVENT_HEADER_ID_SHIFT) - 1);
  freq = rte_get_tsc_hz();
...
static void meta_fixup(struct trace *trace, char *meta) {
