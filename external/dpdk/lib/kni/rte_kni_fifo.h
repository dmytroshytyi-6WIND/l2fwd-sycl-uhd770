...
#define __KNI_LOAD_ACQUIRE(src) ({ __atomic_load_n((src), __ATOMIC_ACQUIRE); })
#define __KNI_STORE_RELEASE(dst, value)                                        \
  do {                                                                         \
 ...
 */
static inline unsigned kni_fifo_put(struct rte_kni_fifo *fifo, void **data,
                                    unsigned num) {
  unsigned i = 0;
 ...
 */
static inline unsigned kni_fifo_get(struct rte_kni_fifo *fifo, void **data,
                                    unsigned num) {
  unsigned i = 0;
 ...
 */
static inline uint32_t kni_fifo_count(struct rte_kni_fifo *fifo) {
  unsigned fifo_write = __KNI_LOAD_ACQUIRE(&fifo->write);
  unsigned fifo_read = __KNI_LOAD_ACQUIRE(&fifo->read);
 ...
 */
static inline uint32_t kni_fifo_free_count(struct rte_kni_fifo *fifo) {
