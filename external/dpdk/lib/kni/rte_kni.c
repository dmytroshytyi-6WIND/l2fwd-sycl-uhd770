...
#define KNI_FIFO_SIZE                                                          \
  (KNI_FIFO_COUNT_MAX * sizeof(void *) + sizeof(struct rte_kni_fifo))

#define KNI_REQUEST_MBUF_NUM_MAX 32

#define KNI_MEM_CHECK(cond, fail)                                              \
  do {                                                                         \
    if (cond)                                                                  \
      goto fail;                                                               \
  } while (0)
...
static struct rte_kni *__rte_kni_get(const char *name) {
  struct rte_kni *kni;
...
static int kni_reserve_mz(struct rte_kni *kni) {
...
static void kni_release_mz(struct rte_kni *kni) {
  rte_memzone_free(kni->m_tx_q);
  rte_memzone_free(kni->m_rx_q);
  rte_memzone_free(kni->m_alloc_q);
  rte_memzone_free(kni->m_free_q);
...
struct rte_kni *rte_kni_alloc(struct rte_mempool *pktmbuf_pool,
  ...
  if (kni_fd < 0) {
    RTE_LOG(ERR, KNI,
            "KNI subsystem has not been initialized. Invoke rte_kni_init() "
            "first\n");
    return NULL;
  }
...
static void kni_free_fifo(struct rte_kni_fifo *fifo) {
  int ret;
  struct rte_mbuf *pkt;
...
static void *va2pa(struct rte_mbuf *m) {
  return (void *)((unsigned long)m -
                  ((unsigned long)m->buf_addr - (unsigned long)m->buf_iova));
}

static void *va2pa_all(struct rte_mbuf *mbuf) {
...
static void obj_free(struct rte_mempool *mp __rte_unused, void *opaque,
...
static void kni_free_fifo_phy(struct rte_mempool *mp,
...
static int kni_config_promiscusity(uint16_t port_id, uint8_t to_on) {
  ...
  if (ret != 0)
    RTE_LOG(ERR, KNI, "Failed to %s promiscuous mode for port %u: %s\n",
            to_on ? "enable" : "disable", port_id, rte_strerror(-ret));
...
static int kni_config_allmulticast(uint16_t port_id, uint8_t to_on) {
  ...
  if (ret != 0)
    RTE_LOG(ERR, KNI, "Failed to %s allmulticast mode for port %u: %s\n",
            to_on ? "enable" : "disable", port_id, rte_strerror(-ret));
...
int rte_kni_handle_request(struct rte_kni *kni) {
  ...
  case RTE_KNI_REQ_CHANGE_MAC_ADDR: /* Change MAC Address */
    if (kni->ops.config_mac_address)
      req->result =
          kni->ops.config_mac_address(kni->ops.port_id, req->mac_addr);
    else if (kni->ops.port_id != UINT16_MAX)
      req->result = kni_config_mac_address(kni->ops.port_id, req->mac_addr);
    break;
  case RTE_KNI_REQ_CHANGE_PROMISC: /* Change PROMISCUOUS MODE */
    if (kni->ops.config_promiscusity)
      req->result =
          kni->ops.config_promiscusity(kni->ops.port_id, req->promiscusity);
    else if (kni->ops.port_id != UINT16_MAX)
      req->result =
          kni_config_promiscusity(kni->ops.port_id, req->promiscusity);
    break;
  case RTE_KNI_REQ_CHANGE_ALLMULTI: /* Change ALLMULTICAST MODE */
    if (kni->ops.config_allmulticast)
      req->result =
          kni->ops.config_allmulticast(kni->ops.port_id, req->allmulti);
    else if (kni->ops.port_id != UINT16_MAX)
      req->result = kni_config_allmulticast(kni->ops.port_id, req->allmulti);
    break;
  default:
...
static void kni_allocate_mbufs(struct rte_kni *kni) {
  ...
  allocq_free =
      (allocq_free > MAX_MBUF_BURST_NUM) ? MAX_MBUF_BURST_NUM : allocq_free;
...
const char *rte_kni_get_name(const struct rte_kni *kni) { return kni->name; }

static enum kni_ops_status kni_check_request_register(struct rte_kni_ops *ops) {
  /* check if KNI request ops has been registered*/
  if (ops == NULL)
    return KNI_REQ_NO_REGISTER;

  if (ops->change_mtu == NULL && ops->config_network_if == NULL &&
      ops->config_mac_address == NULL && ops->config_promiscusity == NULL &&
...
int rte_kni_update_link(struct rte_kni *kni, unsigned int linkup) {
  char path[64];
  char old_carrier[2];
