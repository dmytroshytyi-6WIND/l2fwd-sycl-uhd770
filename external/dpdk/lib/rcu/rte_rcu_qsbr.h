...
#define __RTE_RCU_DP_LOG(level, fmt, args...)                                  \
  rte_log(RTE_LOG_##level, rte_rcu_log_type, "%s(): " fmt "\n", __func__,      \
          ##args)
#else
#define __RTE_RCU_DP_LOG(level, fmt, args...)
#endif
...
#define __RTE_QSBR_THRID_ARRAY_SIZE(max_threads)                               \
  RTE_ALIGN(                                                                   \
      RTE_ALIGN_MUL_CEIL(max_threads, __RTE_QSBR_THRID_ARRAY_ELM_SIZE) >> 3,   \
      RTE_CACHE_LINE_SIZE)
#define __RTE_QSBR_THRID_ARRAY_ELM(v, i)                                       \
...
rte_rcu_qsbr_thread_online(struct rte_rcu_qsbr *v, unsigned int thread_id) {
   ...
   */
  __atomic_store_n(&v->qsbr_cnt[thread_id].cnt, t, __ATOMIC_RELAXED);
...
rte_rcu_qsbr_thread_offline(struct rte_rcu_qsbr *v, unsigned int thread_id) {
  ...
  __atomic_store_n(&v->qsbr_cnt[thread_id].cnt, __RTE_QSBR_CNT_THR_OFFLINE,
                   __ATOMIC_RELEASE);
}
...
rte_rcu_qsbr_lock(__rte_unused struct rte_rcu_qsbr *v,
  ...
  __atomic_fetch_add(&v->qsbr_cnt[thread_id].lock_cnt, 1, __ATOMIC_ACQUIRE);
#endif
}
...
rte_rcu_qsbr_unlock(__rte_unused struct rte_rcu_qsbr *v,
  ...
  __atomic_fetch_sub(&v->qsbr_cnt[thread_id].lock_cnt, 1, __ATOMIC_RELEASE);
 ...
 */
static __rte_always_inline void rte_rcu_qsbr_quiescent(struct rte_rcu_qsbr *v,
                                                       unsigned int thread_id) {
  uint64_t t;
  ...
  if (t != __atomic_load_n(&v->qsbr_cnt[thread_id].cnt, __ATOMIC_RELAXED))
    __atomic_store_n(&v->qsbr_cnt[thread_id].cnt, t, __ATOMIC_RELEASE);
...
__rte_rcu_qsbr_check_selective(struct rte_rcu_qsbr *v, uint64_t t, bool wait) {
  ...
  for (i = 0, reg_thread_id = __RTE_QSBR_THRID_ARRAY_ELM(v, 0);
    ...
    while (bmap) {
      ...
      __RTE_RCU_DP_LOG(DEBUG,
                       "%s: check: token = %" PRIu64
                       ", wait = %d, Bit Map = 0x%" PRIx64 ", Thread ID = %d",
                       __func__, t, wait, bmap, id + j);
      c = __atomic_load_n(&v->qsbr_cnt[id + j].cnt, __ATOMIC_ACQUIRE);
      __RTE_RCU_DP_LOG(DEBUG,
                       "%s: status: token = %" PRIu64
  ...
  if (acked_token != __RTE_QSBR_CNT_MAX)
    __atomic_store_n(&v->acked_token, acked_token, __ATOMIC_RELAXED);
 ...
 */
static __rte_always_inline int __rte_rcu_qsbr_check_all(struct rte_rcu_qsbr *v,
  ...
  for (i = 0, cnt = v->qsbr_cnt; i < v->max_threads; i++, cnt++) {
    __RTE_RCU_DP_LOG(
        DEBUG, "%s: check: token = %" PRIu64 ", wait = %d, Thread ID = %d",
        __func__, t, wait, i);
    while (1) {
 ...
 *     of quiescent states.
 */
static __rte_always_inline int rte_rcu_qsbr_check(struct rte_rcu_qsbr *v,
                                                  uint64_t t, bool wait) {
  RTE_ASSERT(v != NULL);

  /* Check if all the readers have already acknowledged this token */
  if (likely(t <= v->acked_token)) {
    __RTE_RCU_DP_LOG(DEBUG, "%s: check: token = %" PRIu64 ", wait = %d",
                     __func__, t, wait);
    __RTE_RCU_DP_LOG(DEBUG, "%s: status: least acked token = %" PRIu64,
 ...
 */
__rte_experimental struct rte_rcu_qsbr_dq *
