...
size_t rte_rcu_qsbr_get_memsize(uint32_t max_threads) {
  ...
  if (max_threads == 0) {
    rte_log(RTE_LOG_ERR, rte_rcu_log_type, "%s(): Invalid max_threads %u\n",
...
int rte_rcu_qsbr_init(struct rte_rcu_qsbr *v, uint32_t max_threads) {
  ...
  if (v == NULL) {
    rte_log(RTE_LOG_ERR, rte_rcu_log_type, "%s(): Invalid input parameter\n",
            __func__);
    rte_errno = EINVAL;
  ...
  v->num_elems =
      RTE_ALIGN_MUL_CEIL(max_threads, __RTE_QSBR_THRID_ARRAY_ELM_SIZE) /
...
int rte_rcu_qsbr_thread_register(struct rte_rcu_qsbr *v,
  ...
  old_bmap =
      __atomic_load_n(__RTE_QSBR_THRID_ARRAY_ELM(v, i), __ATOMIC_RELAXED);
  if (old_bmap & 1UL << id)
    return 0;

  do {
    new_bmap = old_bmap | (1UL << id);
    success = __atomic_compare_exchange(__RTE_QSBR_THRID_ARRAY_ELM(v, i),
                                        &old_bmap, &new_bmap, 0,
...
int rte_rcu_qsbr_thread_unregister(struct rte_rcu_qsbr *v,
  ...
  do {
    ...
    if (success)
      __atomic_fetch_sub(&v->num_threads, 1, __ATOMIC_RELAXED);
    else if (!(old_bmap & (1UL << id)))
...
int rte_rcu_qsbr_dump(FILE *f, struct rte_rcu_qsbr *v) {
  ...
  for (i = 0; i < v->num_elems; i++) {
    bmap = __atomic_load_n(__RTE_QSBR_THRID_ARRAY_ELM(v, i), __ATOMIC_ACQUIRE);
  ...
  for (i = 0; i < v->num_elems; i++) {
    ...
    while (bmap) {
      ...
      fprintf(f, "thread ID = %u, count = %" PRIu64 ", lock count = %u\n",
              id + t,
              __atomic_load_n(&v->qsbr_cnt[id + t].cnt, __ATOMIC_RELAXED),
              __atomic_load_n(&v->qsbr_cnt[id + t].lock_cnt, __ATOMIC_RELAXED));
      bmap &= ~(1UL << t);
    }
  }
...
rte_rcu_qsbr_dq_create(const struct rte_rcu_qsbr_dq_parameters *params) {
  ...
  if (params == NULL || params->free_fn == NULL || params->v == NULL ||
      params->name == NULL || params->size == 0 || params->esize == 0 ||
      (params->esize % 4 != 0)) {
    rte_log(RTE_LOG_ERR, rte_rcu_log_type, "%s(): Invalid input parameter\n",
            __func__);
    rte_errno = EINVAL;
  ...
  if ((params->trigger_reclaim_limit <= params->size) &&
    ...
    rte_log(RTE_LOG_ERR, rte_rcu_log_type,
            "%s(): Invalid input parameter, size = %u, trigger_reclaim_limit = "
            "%u, max_reclaim_size = %u\n",
  ...
  dq = rte_zmalloc(NULL, sizeof(struct rte_rcu_qsbr_dq), RTE_CACHE_LINE_SIZE);
  if (dq == NULL) {
    rte_errno = ENOMEM;
  ...
  dq->r =
      rte_ring_create_elem(params->name, __RTE_QSBR_TOKEN_SIZE + params->esize,
                           qs_fifo_size, SOCKET_ID_ANY, flags);
  if (dq->r == NULL) {
    rte_log(RTE_LOG_ERR, rte_rcu_log_type, "%s(): defer queue create failed\n",
            __func__);
    rte_free(dq);
    return NULL;
  }
...
int rte_rcu_qsbr_dq_enqueue(struct rte_rcu_qsbr_dq *dq, void *e) {
  ...
  if (cur_size > dq->trigger_reclaim_limit) {
    rte_log(RTE_LOG_INFO, rte_rcu_log_type, "%s(): Triggering reclamation\n",
            __func__);
    rte_rcu_qsbr_dq_reclaim(dq, dq->max_reclaim_size, NULL, NULL, NULL);
  }
  ...
  if (rte_ring_enqueue_elem(dq->r, data, dq->esize) != 0) {
    rte_log(RTE_LOG_ERR, rte_rcu_log_type, "%s(): Enqueue failed\n", __func__);
...
int rte_rcu_qsbr_dq_reclaim(struct rte_rcu_qsbr_dq *dq, unsigned int n,
  ...
  while (cnt < n && rte_ring_dequeue_bulk_elem_start(dq->r, &data, dq->esize, 1,
                                                     available) != 0) {
    dq_elem = (__rte_rcu_qsbr_dq_elem_t *)data;
  ...
  rte_log(RTE_LOG_INFO, rte_rcu_log_type, "%s(): Reclaimed %u resources\n",
          __func__, cnt);
...
int rte_rcu_qsbr_dq_delete(struct rte_rcu_qsbr_dq *dq) {
  ...
  if (dq == NULL) {
    rte_log(RTE_LOG_DEBUG, rte_rcu_log_type, "%s(): Invalid input parameter\n",
            __func__);
