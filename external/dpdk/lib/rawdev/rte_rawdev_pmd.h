...
#define RTE_RDEV_ERR(fmt, args...) RTE_RDEV_LOG(ERR, fmt, ##args)
#define RTE_RDEV_DEBUG(fmt, args...) RTE_RDEV_LOG(DEBUG, fmt, ##args)
#define RTE_RDEV_INFO(fmt, args...) RTE_RDEV_LOG(INFO, fmt, ##args)

/* Macros to check for valid device */
#define RTE_RAWDEV_VALID_DEVID_OR_ERR_RET(dev_id, retval)                      \
  do {                                                                         \
...
#define RTE_RAWDEV_VALID_DEVID_OR_RET(dev_id)                                  \
  do {                                                                         \
    if (!rte_rawdev_pmd_is_valid_dev((dev_id))) {                              \
      RTE_RDEV_ERR("Invalid dev_id=%d", dev_id);                               \
      return;                                                                  \
 ...
 */
static inline unsigned rte_rawdev_pmd_is_valid_dev(uint8_t dev_id) {
  struct rte_rawdev *dev;
 ...
 *   Returns 0 on success.
 */
typedef int (*rawdev_queue_setup_t)(struct rte_rawdev *dev, uint16_t queue_id,
                                    rte_rawdev_obj_t queue_conf,
                                    size_t queue_conf_size);
 ...
 */
typedef int (*rawdev_get_attr_t)(struct rte_rawdev *dev, const char *attr_name,
                                 uint64_t *attr_value);
 ...
 *  !0 Error
 */
typedef int (*rawdev_set_attr_t)(struct rte_rawdev *dev, const char *attr_name,
 ...
 */
typedef int (*rawdev_xstats_get_names_t)(
    const struct rte_rawdev *dev, struct rte_rawdev_xstats_name *xstats_names,
    unsigned int size);
...
struct rte_rawdev *rte_rawdev_pmd_init(const char *name,
                                       size_t dev_private_size, int socket_id);
