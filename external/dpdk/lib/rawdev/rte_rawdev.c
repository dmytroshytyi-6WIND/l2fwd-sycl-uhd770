...
static struct rte_rawdev_global rawdev_globals = {.nb_devs = 0};

/* Raw device, northbound API implementation */
uint8_t rte_rawdev_count(void) { return rawdev_globals.nb_devs; }
...
uint16_t rte_rawdev_get_dev_id(const char *name) {
  ...
  for (i = 0; i < rawdev_globals.nb_devs; i++)
    if ((strcmp(rte_rawdevices[i].name, name) == 0) &&
        (rte_rawdevices[i].attached == RTE_RAWDEV_ATTACHED))
      return i;
  return -ENODEV;
}
...
int rte_rawdev_info_get(uint16_t dev_id, struct rte_rawdev_info *dev_info,
  ...
  if (dev_info->dev_private != NULL) {
    ...
    ret = (*rawdev->dev_ops->dev_info_get)(rawdev, dev_info->dev_private,
                                           dev_private_size);
  }
...
int rte_rawdev_configure(uint16_t dev_id, struct rte_rawdev_info *dev_conf,
  ...
  if (dev->started) {
    RTE_RDEV_ERR("device %d must be stopped to allow configuration", dev_id);
    return -EBUSY;
  }
...
int rte_rawdev_set_attr(uint16_t dev_id, const char *attr_name,
...
int rte_rawdev_enqueue_buffers(uint16_t dev_id, struct rte_rawdev_buf **buffers,
...
int rte_rawdev_dequeue_buffers(uint16_t dev_id, struct rte_rawdev_buf **buffers,
                               unsigned int count, rte_rawdev_obj_t context) {
  struct rte_rawdev *dev;
...
static int xstats_get_count(uint16_t dev_id) {
...
uint64_t rte_rawdev_xstats_by_name_get(uint16_t dev_id, const char *name,
                                       unsigned int *id) {
...
int rte_rawdev_start(uint16_t dev_id) {
  ...
  dev = &rte_rawdevs[dev_id];
  if (dev->started != 0) {
    RTE_RDEV_ERR("Device with dev_id=%" PRIu8 "already started", dev_id);
    return 0;
  }
...
void rte_rawdev_stop(uint16_t dev_id) {
  ...
  if (dev->started == 0) {
    RTE_RDEV_ERR("Device with dev_id=%" PRIu8 "already stopped", dev_id);
    return;
  }
...
int rte_rawdev_close(uint16_t dev_id) {
  ...
  if (dev->started == 1) {
    RTE_RDEV_ERR("Device %u must be stopped before closing", dev_id);
    return -EBUSY;
  }
...
static inline uint8_t rte_rawdev_find_free_device_index(void) {
  uint16_t dev_id;

  for (dev_id = 0; dev_id < RTE_RAWDEV_MAX_DEVS; dev_id++) {
    if (rte_rawdevs[dev_id].attached == RTE_RAWDEV_DETACHED)
      return dev_id;
  }
...
rte_rawdev_pmd_allocate(const char *name, size_t dev_priv_size, int socket_id) {
  ...
  if (rte_rawdev_pmd_get_named_dev(name) != NULL) {
    RTE_RDEV_ERR("Event device with name %s already allocated!", name);
    return NULL;
  }
  ...
  if (dev_priv_size > 0) {
    rawdev->dev_private = rte_zmalloc_socket("rawdev private", dev_priv_size,
...
static int handle_dev_xstats(const char *cmd __rte_unused, const char *params,
  ...
  rawdev_xstats =
      malloc((sizeof(uint64_t) + sizeof(struct rte_rawdev_xstats_name) +
              sizeof(unsigned int)) *
             num_xstats);
  ...
  for (i = 0; i < num_xstats; i++)
    rte_tel_data_add_dict_u64(d, xstat_names[i].name, rawdev_xstats[i]);
