...
static inline bool is_valid_node(struct rte_rib6_node *node) {
  return (node->flag & RTE_RIB_VALID_NODE) == RTE_RIB_VALID_NODE;
}

static inline bool is_right_node(struct rte_rib6_node *node) {
 ...
 */
static inline bool is_covered(const uint8_t ip1[RTE_RIB6_IPV6_ADDR_SIZE],
                              const uint8_t ip2[RTE_RIB6_IPV6_ADDR_SIZE],
                              uint8_t depth) {
  int i;
...
static inline int get_dir(const uint8_t ip[RTE_RIB6_IPV6_ADDR_SIZE],
                          uint8_t depth) {
  uint8_t index, msk;
...
static struct rte_rib6_node *node_alloc(struct rte_rib6 *rib) {
...
static void node_free(struct rte_rib6 *rib, struct rte_rib6_node *ent) {
  --rib->cur_nodes;
...
rte_rib6_lookup_exact(struct rte_rib6 *rib,
  ...
  while (cur != NULL) {
    if (rte_rib6_is_equal(cur->ip, tmp_ip) && (cur->depth == depth) &&
        is_valid_node(cur))
      return cur;
...
rte_rib6_get_nxt(struct rte_rib6 *rib,
                 ...
                 struct rte_rib6_node *last, int flag) {
  ...
  if (last == NULL) {
    ...
    while ((tmp->parent != NULL) &&
           (is_right_node(tmp) || (tmp->parent->right == NULL))) {
      tmp = tmp->parent;
      if (is_valid_node(tmp) &&
          (is_covered(tmp->ip, tmp_ip, depth) && (tmp->depth > depth)))
        return tmp;
    }
...
void rte_rib6_remove(struct rte_rib6 *rib,
                     const uint8_t ip[RTE_RIB6_IPV6_ADDR_SIZE], uint8_t depth) {
...
struct rte_rib6_node *rte_rib6_insert(struct rte_rib6 *rib,
                                      const uint8_t ip[RTE_RIB6_IPV6_ADDR_SIZE],
                                      uint8_t depth) {
  struct rte_rib6_node **tmp;
  struct rte_rib6_node *prev = NULL;
  ...
  while (1) {
     ...
     */
    if (rte_rib6_is_equal(tmp_ip, (*tmp)->ip) && (depth == (*tmp)->depth)) {
  ...
  if (rte_rib6_is_equal(common_prefix, tmp_ip) && (common_depth == depth)) {
    /* insert as a parent */
...
int rte_rib6_get_ip(const struct rte_rib6_node *node,
                    uint8_t ip[RTE_RIB6_IPV6_ADDR_SIZE]) {
...
struct rte_rib6 *rte_rib6_create(const char *name, int socket_id,
  ...
  node_pool = rte_mempool_create(mem_name, conf->max_nodes,
                                 sizeof(struct rte_rib6_node) + conf->ext_sz, 0,
                                 0, NULL, NULL, NULL, NULL, socket_id, 0);

  if (node_pool == NULL) {
    RTE_LOG(ERR, LPM, "Can not allocate mempool for RIB6 %s\n", name);
    return NULL;
  }
  ...
  if (te == NULL) {
    RTE_LOG(ERR, LPM, "Can not allocate tailq entry for RIB6 %s\n", name);
    rte_errno = ENOMEM;
    goto exit;
  }

  /* Allocate memory to store the RIB6 data structures. */
  rib = rte_zmalloc_socket(mem_name, sizeof(struct rte_rib6),
...
void rte_rib6_free(struct rte_rib6 *rib) {
  ...
  while ((tmp = rte_rib6_get_nxt(rib, 0, 0, tmp, RTE_RIB6_GET_NXT_ALL)) != NULL)
