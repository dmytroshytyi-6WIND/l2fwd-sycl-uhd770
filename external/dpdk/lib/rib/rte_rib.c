...
static inline bool is_valid_node(struct rte_rib_node *node) {
  return (node->flag & RTE_RIB_VALID_NODE) == RTE_RIB_VALID_NODE;
}

static inline bool is_right_node(struct rte_rib_node *node) {
 ...
 */
static inline bool is_covered(uint32_t ip1, uint32_t ip2, uint8_t depth) {
  return ((ip1 ^ ip2) & rte_rib_depth_to_mask(depth)) == 0;
}

static inline struct rte_rib_node *get_nxt_node(struct rte_rib_node *node,
                                                uint32_t ip) {
  return (ip & (1 << (31 - node->depth))) ? node->right : node->left;
}

static struct rte_rib_node *node_alloc(struct rte_rib *rib) {
...
static void node_free(struct rte_rib *rib, struct rte_rib_node *ent) {
  --rib->cur_nodes;
...
static struct rte_rib_node *__rib_lookup_exact(struct rte_rib *rib, uint32_t ip,
                                               uint8_t depth) {
...
struct rte_rib_node *rte_rib_lookup_exact(struct rte_rib *rib, uint32_t ip,
                                          uint8_t depth) {
 ...
 */
struct rte_rib_node *rte_rib_get_nxt(struct rte_rib *rib, uint32_t ip,
                                     uint8_t depth, struct rte_rib_node *last,
                                     int flag) {
  ...
  if (last == NULL) {
    ...
    while ((tmp->parent != NULL) &&
      ...
      if (is_valid_node(tmp) &&
          (is_covered(tmp->ip, ip, depth) && (tmp->depth > depth)))
        return tmp;
    }
...
struct rte_rib *rte_rib_create(const char *name, int socket_id,
  ...
  node_pool = rte_mempool_create(mem_name, conf->max_nodes,
                                 sizeof(struct rte_rib_node) + conf->ext_sz, 0,
                                 0, NULL, NULL, NULL, NULL, socket_id, 0);

  if (node_pool == NULL) {
    RTE_LOG(ERR, LPM, "Can not allocate mempool for RIB %s\n", name);
    return NULL;
  }
  ...
  if (te == NULL) {
    RTE_LOG(ERR, LPM, "Can not allocate tailq entry for RIB %s\n", name);
    rte_errno = ENOMEM;
    goto exit;
  }

  /* Allocate memory to store the RIB data structures. */
  rib = rte_zmalloc_socket(mem_name, sizeof(struct rte_rib),
...
void rte_rib_free(struct rte_rib *rib) {
  ...
  while ((tmp = rte_rib_get_nxt(rib, 0, 0, tmp, RTE_RIB_GET_NXT_ALL)) != NULL)
