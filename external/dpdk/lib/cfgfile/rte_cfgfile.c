...
static unsigned _strip(char *str, unsigned len) {
  int newlen = len;
  if (len == 0)
    return 0;
...
static struct rte_cfgfile_section *_get_section(struct rte_cfgfile *cfg,
...
static int _add_entry(struct rte_cfgfile_section *section,
...
struct rte_cfgfile *rte_cfgfile_load(const char *filename, int flags) {
  return rte_cfgfile_load_with_params(filename, flags, &default_cfgfile_params);
}
...
rte_cfgfile_load_with_params(const char *filename, int flags,
  ...
  while (fgets(buffer, sizeof(buffer), f) != NULL) {
    ...
    if (buffer[0] == '[') {
      ...
      if (end == NULL) {
        CFG_LOG(ERR, "line %d - no terminating ']' character found\n", lineno);
        goto error1;
      }
      *end = '\0';
      ...
      if (split[1] == NULL) {
        CFG_LOG(ERR, "line %d - no '=' character found\n", lineno);
        goto error1;
      }
      *split[1] = '\0';
      split[1]++;
      ...
      if (!(flags & CFG_FLAG_EMPTY_VALUES) && (*split[1] == '\0')) {
        CFG_LOG(ERR, "line %d - cannot use empty values\n", lineno);
        goto error1;
      }
      ...
      _add_entry(&cfg->sections[cfg->num_sections - 1], split[0], split[1]);
    }
  }
  fclose(f);
  return cfg;
...
struct rte_cfgfile *rte_cfgfile_create(int flags) {
  ...
  cfg->sections =
      calloc(CFG_ALLOC_SECTION_BATCH, sizeof(struct rte_cfgfile_section));
  ...
  for (i = 0; i < CFG_ALLOC_SECTION_BATCH; i++) {
    cfg->sections[i].entries =
        calloc(CFG_ALLOC_ENTRY_BATCH, sizeof(struct rte_cfgfile_entry));
...
int rte_cfgfile_add_section(struct rte_cfgfile *cfg, const char *sectionname) {
  ...
  if (cfg->num_sections == cfg->allocated_sections) {
    ...
    for (i = 0; i < CFG_ALLOC_SECTION_BATCH; i++) {
      ...
      n_sections[i + cfg->allocated_sections].allocated_entries = 0;
...
int rte_cfgfile_add_entry(struct rte_cfgfile *cfg, const char *sectionname,
                          const char *entryname, const char *entryvalue) {
  int ret;
  ...
  struct rte_cfgfile_section *curr_section = _get_section(cfg, sectionname);
...
int rte_cfgfile_num_sections(struct rte_cfgfile *cfg, const char *sectionname,
                             size_t length) {
  int i;
  int num_sections = 0;
  for (i = 0; i < cfg->num_sections; i++) {
...
int rte_cfgfile_section_entries_by_index(struct rte_cfgfile *cfg, int index,
                                         char *sectionname,
                                         struct rte_cfgfile_entry *entries,
                                         int max_entries) {
  int i;
