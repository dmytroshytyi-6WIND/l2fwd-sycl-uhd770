...
#define ADD_TIME_MIN_MAX(obj, type, value)                                     \
  do {                                                                         \
...
#define RESET_TIME_MIN_MAX(obj, type)                                          \
  do {                                                                         \
    (obj)->type##_time = 0;                                                    \
 ...
 */
static void default_update_function(struct rte_jobstats *job, int64_t result) {
...
void rte_jobstats_context_reset(struct rte_jobstats_context *ctx) {
...
void rte_jobstats_set_target(struct rte_jobstats *job, int64_t target) {
  job->target = target;
}
...
int rte_jobstats_abort(struct rte_jobstats *job) {
...
void rte_jobstats_set_period(struct rte_jobstats *job, uint64_t period,
                             uint8_t saturate) {
  if (saturate != 0) {
...
void rte_jobstats_set_min(struct rte_jobstats *job, uint64_t period) {
  job->min_period = period;
  if (job->period < period)
    job->period = period;
}

void rte_jobstats_set_max(struct rte_jobstats *job, uint64_t period) {
...
void rte_jobstats_set_update_period_function(
    struct rte_jobstats *job, rte_job_update_period_cb_t update_period_cb) {
...
void rte_jobstats_reset(struct rte_jobstats *job) {
