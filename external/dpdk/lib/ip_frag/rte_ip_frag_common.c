...
struct rte_ip_frag_tbl *rte_ip_frag_table_create(uint32_t bucket_num,
  ...
  if (rte_is_power_of_2(bucket_entries) == 0 || nb_entries > UINT32_MAX ||
      nb_entries == 0 || nb_entries < max_entries) {
  ...
  if ((tbl = rte_zmalloc_socket(__func__, sz, RTE_CACHE_LINE_SIZE,
                                socket_id)) == NULL) {
    RTE_LOG(ERR, USER1, "%s: allocation of %zu bytes at socket %d failed do\n",
...
void rte_ip_frag_table_destroy(struct rte_ip_frag_tbl *tbl) {
  ...
  TAILQ_FOREACH(fp, &tbl->lru, lru) { ip_frag_free_immediate(fp); }
...
void rte_ip_frag_table_statistics_dump(FILE *f,
  ...
  fprintf(f,
          ...
          tbl->max_entries, tbl->use_entries, tbl->stat.find_num,
          tbl->stat.add_num, tbl->stat.del_num, tbl->stat.reuse_num, fail_total,
          fail_nospace, fail_total - fail_nospace);
}
