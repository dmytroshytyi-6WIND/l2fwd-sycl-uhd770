...
static inline void ip_frag_memmove(char *dst, char *src, int len) {
  int i;
...
struct rte_mbuf *rte_ipv6_frag_reassemble_packet(
    struct rte_ip_frag_tbl *tbl, struct rte_ip_frag_death_row *dr,
  ...
  IP_FRAG_LOG(DEBUG,
              "%s:%d:\n"
              "mbuf: %p, tms: %" PRIu64 ", key: <" IPv6_KEY_BYTES_FMT ", %#x>, "
              "ofs: %u, len: %d, padding: %d, flags: %#x\n"
              "tbl: %p, max_cycles: %" PRIu64 ", entry_mask: %#x, "
              "max_entries: %u, use_entries: %u\n\n",
              __func__, __LINE__, mb, tms, IPv6_KEY_BYTES(key.src_dst), key.id,
              ip_ofs, ip_len, trim, RTE_IPV6_GET_MF(frag_hdr->frag_data), tbl,
              tbl->max_cycles, tbl->entry_mask, tbl->max_entries,
              tbl->use_entries);
  ...
  IP_FRAG_LOG(DEBUG,
              ...
              __func__, __LINE__, tbl, tbl->max_entries, tbl->use_entries, fp,
              IPv6_KEY_BYTES(fp->key.src_dst), fp->key.id, fp->start,
  ...
  IP_FRAG_LOG(DEBUG,
              ...
              ", %#x>, start: %" PRIu64
              ", total_size: %u, frag_size: %u, last_idx: %u\n\n",
              __func__, __LINE__, mb, tbl, tbl->max_entries, tbl->use_entries,
