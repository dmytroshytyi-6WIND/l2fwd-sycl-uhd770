...
struct rte_mbuf *ipv4_frag_reassemble(struct ip_frag_pkt *fp) {
  ...
  ip_hdr->total_length =
      rte_cpu_to_be_16((uint16_t)(fp->total_size + m->l3_len));
 ...
 */
struct rte_mbuf *rte_ipv4_frag_reassemble_packet(
  ...
  IP_FRAG_LOG(DEBUG,
              ...
              __func__, __LINE__, mb, tms, key.src_dst[0], key.id, ip_ofs,
              ip_len, trim, ip_flag, tbl, tbl->max_cycles, tbl->entry_mask,
  ...
  IP_FRAG_LOG(DEBUG,
              ...
              fp->key.src_dst[0], fp->key.id, fp->start, fp->total_size,
              fp->frag_size, fp->last_idx);
  ...
  IP_FRAG_LOG(DEBUG,
              ...
              fp, fp->key.src_dst[0], fp->key.id, fp->start, fp->total_size,
              fp->frag_size, fp->last_idx);
