...
static inline void __fill_ipv4hdr_frag(struct rte_ipv4_hdr *dst,
                                       const struct rte_ipv4_hdr *src,
                                       uint16_t header_len, uint16_t len,
                                       uint16_t fofs, uint16_t dofs,
                                       uint32_t mf) {
...
int32_t rte_ipv4_fragment_packet(struct rte_mbuf *pkt_in,
  ...
  if (unlikely(pkt_in == NULL) || unlikely(pkts_out == NULL) ||
      ...
      unlikely(pool_indirect == NULL) || unlikely(mtu_size < RTE_ETHER_MIN_MTU))
    return -EINVAL;

  in_hdr = rte_pktmbuf_mtod(pkt_in, struct rte_ipv4_hdr *);
  header_len =
      (in_hdr->version_ihl & RTE_IPV4_HDR_IHL_MASK) * RTE_IPV4_IHL_MULTIPLIER;
  ...
  while (likely(more_in_segs)) {
    ...
    __fill_ipv4hdr_frag(out_hdr, in_hdr, header_len, (uint16_t)out_pkt->pkt_len,
                        flag_offset, fragment_offset, more_in_segs);

    fragment_offset =
        (uint16_t)(fragment_offset + out_pkt->pkt_len - header_len);
