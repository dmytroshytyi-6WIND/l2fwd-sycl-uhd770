...
static inline void __fill_ipv6hdr_frag(struct rte_ipv6_hdr *dst,
                                       const struct rte_ipv6_hdr *src,
                                       uint16_t len, uint16_t fofs,
                                       uint32_t mf) {
...
int32_t rte_ipv6_fragment_packet(struct rte_mbuf *pkt_in,
  ...
  if (unlikely(pkt_in == NULL) || unlikely(pkts_out == NULL) ||
      ...
      unlikely(pool_indirect == NULL) || unlikely(mtu_size < RTE_IPV6_MIN_MTU))
    return -EINVAL;
  ...
  while (likely(more_in_segs)) {
    ...
    fragment_offset = (uint16_t)(fragment_offset + out_pkt->pkt_len -
                                 sizeof(struct rte_ipv6_hdr) -
                                 sizeof(struct rte_ipv6_fragment_ext));
