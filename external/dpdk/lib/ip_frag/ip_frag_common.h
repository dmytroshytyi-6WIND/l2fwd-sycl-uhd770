...
#define IPv6_KEY_BYTES_FMT "%08" PRIx64 "%08" PRIx64 "%08" PRIx64 "%08" PRIx64

#ifdef RTE_LIBRTE_IP_FRAG_TBL_STAT
#define IP_FRAG_TBL_STAT_UPDATE(s, f, v) ((s)->f += (v))
#else
#define IP_FRAG_TBL_STAT_UPDATE(s, f, v)                                       \
  do {                                                                         \
  } while (0)
...
/* invalidate the key */
static inline void ip_frag_key_invalidate(struct ip_frag_key *key) {
  key->key_len = 0;
}

/* compare two keys */
static inline uint64_t ip_frag_key_cmp(const struct ip_frag_key *k1,
                                       const struct ip_frag_key *k2) {
  uint32_t i;
  uint64_t val;
...
static inline void ip_frag_free(struct ip_frag_pkt *fp,
                                struct rte_ip_frag_death_row *dr) {
  uint32_t i, k;
...
static inline void ip_frag_free_immediate(struct ip_frag_pkt *fp) {
  uint32_t i;
...
static inline void ip_frag_inuse(struct rte_ip_frag_tbl *tbl,
                                 const struct ip_frag_pkt *fp) {
...
/* reset the fragment */
static inline void ip_frag_reset(struct ip_frag_pkt *fp, uint64_t tms) {
...
static inline void ip_frag_tbl_del(struct rte_ip_frag_tbl *tbl,
                                   struct rte_ip_frag_death_row *dr,
