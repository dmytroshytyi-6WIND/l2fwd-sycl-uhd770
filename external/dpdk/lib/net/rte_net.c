...
static uint32_t ptype_tunnel(uint16_t *proto, const struct rte_mbuf *m,
                             uint32_t *off) {
  switch (*proto) {
  case IPPROTO_GRE: {
...
#define MAX_EXT_HDRS 5
  for (i = 0; i < MAX_EXT_HDRS; i++) {
    ...
    case IPPROTO_DSTOPTS:
      xh = rte_pktmbuf_read(m, *off, sizeof(*xh), &xh_copy);
      if (xh == NULL)
        return -1;
      *off += (xh->len + 1) * 8;
      proto = xh->next_hdr;
...
#define MAX_MPLS_HDR 5
    for (i = 0; i < MAX_MPLS_HDR; i++) {
      mh = rte_pktmbuf_read(m, off + (i * sizeof(*mh)), sizeof(*mh), &mh_copy);
      if (unlikely(mh == NULL))
        return pkt_type;
    }
    if (i == MAX_MPLS_HDR)
...
l3:
  ...
  if ((pkt_type & RTE_PTYPE_L4_MASK) == RTE_PTYPE_L4_UDP) {
    ...
    if (unlikely(th == NULL))
      return pkt_type & (RTE_PTYPE_L2_MASK | RTE_PTYPE_L3_MASK);
  ...
  if ((pkt_type & RTE_PTYPE_INNER_L4_MASK) == RTE_PTYPE_INNER_L4_UDP) {
    ...
    th = rte_pktmbuf_read(m, off, sizeof(*th), &th_copy);
    if (unlikely(th == NULL))
      return pkt_type & (RTE_PTYPE_INNER_L2_MASK | RTE_PTYPE_INNER_L3_MASK);
