...
static const uint32_t mask[4] __rte_aligned(16) = {0xffffffff, 0xffffffff,
                                                   0x00000000, 0x00000000};

static const uint32_t mask2[4] __rte_aligned(16) = {0x00000000, 0xffffffff,
                                                    0xffffffff, 0xffffffff};

static __rte_always_inline __m512i crcr32_folding_round(__m512i data_block,
                                                        __m512i precomp,
                                                        __m512i fold) {
  __m512i tmp0, tmp1;
...
static __rte_always_inline __m128i crc32_fold_128(
    __m512i fold0, __m512i fold1, const struct crc_vpclmulqdq_ctx *params) {
  __m128i res, res2;
  __m256i a;
...
last_two_xmm(const uint8_t *data, uint32_t data_len, uint32_t n, __m128i res,
  ...
  const uint32_t mask3[4] __rte_aligned(16) = {0x80808080, 0x80808080,
  ...
  pshufb_shf = _mm_loadu_si128((const __m128i *)(shf_table + (data_len - n)));

  res = _mm_shuffle_epi8(res, pshufb_shf);
  pshufb_shf =
      _mm_xor_si128(pshufb_shf, _mm_load_si128((const __m128i *)mask3));
...
barrett_reduction(__m128i data64, const struct crc_vpclmulqdq_ctx *params) {
  ...
  data64 = _mm_ternarylogic_epi64(data64, tmp1, *(const __m128i *)mask, 0x28);
...
crc32_eth_calc_vpclmulqdq(const uint8_t *data, uint32_t data_len, uint32_t crc,
  ...
  if (data_len > 255) {
    ...
    for (n = 256; (n + 256) <= data_len; n += 256) {
      ...
      qw1 = _mm512_loadu_si512((const __m512i *)&(data[n + 64]));
      qw2 = _mm512_loadu_si512((const __m512i *)&(data[n + 128]));
      qw3 = _mm512_loadu_si512((const __m512i *)&(data[n + 192]));
    ...
    while (reduction > 0)
      reduction_loop(&res, &reduction, data, &n, params);

    reduction += 16;

    if (n != data_len)
      res = last_two_xmm(data, data_len, n, res, params);
  } else {
    if (data_len > 31) {
      ...
      if (data_len > 3) {
        d = _mm_loadu_si128((const __m128i *)&shf_table[data_len]);
...
static void crc32_load_init_constants(void) {
  __m128i a;
  /* fold constants */
  ...
  crc32_eth.fold_7x128b = _mm512_setr_epi64(c4, c5, c6, c7, c8, c9, c10, c11);
  crc32_eth.fold_3x128b = _mm512_setr_epi64(c12, c13, c14, c15, c16, c17, 0, 0);
  crc32_eth.fold_1x128b =
      _mm_setr_epi64(_mm_cvtsi64_m64(c16), _mm_cvtsi64_m64(c17));

  crc32_eth.rk5_rk6 =
      _mm_setr_epi64(_mm_cvtsi64_m64(c18), _mm_cvtsi64_m64(c19));
  crc32_eth.rk7_rk8 =
      _mm_setr_epi64(_mm_cvtsi64_m64(c20), _mm_cvtsi64_m64(c21));
}

static void crc16_load_init_constants(void) {
  __m128i a;
  /* fold constants */
  ...
  crc16_ccitt.fold_7x128b = _mm512_setr_epi64(c4, c5, c6, c7, c8, c9, c10, c11);
  crc16_ccitt.fold_3x128b =
      _mm512_setr_epi64(c12, c13, c14, c15, c16, c17, 0, 0);
  crc16_ccitt.fold_1x128b =
      _mm_setr_epi64(_mm_cvtsi64_m64(c16), _mm_cvtsi64_m64(c17));
...
void rte_net_crc_avx512_init(void) {
...
uint32_t rte_crc16_ccitt_avx512_handler(const uint8_t *data,
                                        uint32_t data_len) {
  /* return 16-bit CRC value */
  return (uint16_t)~crc32_eth_calc_vpclmulqdq(data, data_len, 0xffff,
                                              &crc16_ccitt);
}

uint32_t rte_crc32_eth_avx512_handler(const uint8_t *data, uint32_t data_len) {
  /* return 32-bit CRC value */
  return ~crc32_eth_calc_vpclmulqdq(data, data_len, 0xffffffffUL, &crc32_eth);
}
