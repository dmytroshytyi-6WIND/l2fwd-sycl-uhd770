 ...
 */
static __rte_always_inline __m128i crcr32_folding_round(__m128i data_block,
                                                        __m128i precomp,
                                                        __m128i fold) {
...
static __rte_always_inline __m128i crcr32_reduce_128_to_64(__m128i data128,
                                                           __m128i precomp) {
...
static __rte_always_inline uint32_t crcr32_reduce_64_to_32(__m128i data64,
                                                           __m128i precomp) {
  static const uint32_t mask1[4] __rte_aligned(16) = {0xffffffff, 0xffffffff,
...
static __rte_always_inline __m128i xmm_shift_left(__m128i reg,
...
crc32_eth_calc_pclmulqdq(const uint8_t *data, uint32_t data_len, uint32_t crc,
...
void rte_net_crc_sse42_init(void) {
...
uint32_t rte_crc16_ccitt_sse42_handler(const uint8_t *data, uint32_t data_len) {
  /** return 16-bit CRC value */
  return (uint16_t)~crc32_eth_calc_pclmulqdq(data, data_len, 0xffff,
                                             &crc16_ccitt_pclmulqdq);
}

uint32_t rte_crc32_eth_sse42_handler(const uint8_t *data, uint32_t data_len) {
  return ~crc32_eth_calc_pclmulqdq(data, data_len, 0xffffffffUL,
                                   &crc32_eth_pclmulqdq);
}
