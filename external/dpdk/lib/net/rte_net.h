 ...
 */
static inline int rte_net_intel_cksum_flags_prepare(struct rte_mbuf *m,
                                                    uint64_t ol_flags) {
  ...
  if (!(ol_flags & (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_L4_MASK |
                    RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_OUTER_IP_CKSUM)))
    return 0;
  ...
  if (ol_flags & (RTE_MBUF_F_TX_OUTER_IPV4 | RTE_MBUF_F_TX_OUTER_IPV6)) {
    ...
    if (ol_flags & RTE_MBUF_F_TX_OUTER_IP_CKSUM) {
      ipv4_hdr =
          rte_pktmbuf_mtod_offset(m, struct rte_ipv4_hdr *, m->outer_l2_len);
  ...
  if (ol_flags & RTE_MBUF_F_TX_IPV4) {
    ipv4_hdr =
        rte_pktmbuf_mtod_offset(m, struct rte_ipv4_hdr *, inner_l3_offset);
  ...
  if ((ol_flags & RTE_MBUF_F_TX_L4_MASK) == RTE_MBUF_F_TX_UDP_CKSUM) {
    if (ol_flags & RTE_MBUF_F_TX_IPV4) {
      ...
      ipv6_hdr =
          rte_pktmbuf_mtod_offset(m, struct rte_ipv6_hdr *, inner_l3_offset);
      /* non-TSO udp */
