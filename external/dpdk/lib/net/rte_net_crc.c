...
static uint32_t rte_crc16_ccitt_default_handler(const uint8_t *data,
                                                uint32_t data_len);

static uint32_t rte_crc32_eth_default_handler(const uint8_t *data,
                                              uint32_t data_len);

static uint32_t rte_crc16_ccitt_handler(const uint8_t *data, uint32_t data_len);

static uint32_t rte_crc32_eth_handler(const uint8_t *data, uint32_t data_len);

typedef uint32_t (*rte_net_crc_handler)(const uint8_t *data, uint32_t data_len);
 ...
 *   reflected value
 */
static uint32_t reflect_32bits(uint32_t val) {
  uint32_t i, res = 0;
...
static void crc32_eth_init_lut(uint32_t poly, uint32_t *lut) {
  uint32_t i, j;
...
static __rte_always_inline uint32_t crc32_eth_calc_lut(const uint8_t *data,
                                                       uint32_t data_len,
                                                       uint32_t crc,
                                                       const uint32_t *lut) {
  while (data_len--)
...
static void rte_net_crc_scalar_init(void) {
  /* 32-bit crc init */
...
static inline uint32_t rte_crc16_ccitt_handler(const uint8_t *data,
                                               uint32_t data_len) {
  /* return 16-bit CRC value */
  return (uint16_t)~crc32_eth_calc_lut(data, data_len, 0xffff, crc16_ccitt_lut);
}

static inline uint32_t rte_crc32_eth_handler(const uint8_t *data,
                                             uint32_t data_len) {
...
static const rte_net_crc_handler *avx512_vpclmulqdq_get_handlers(void) {
#ifdef CC_X86_64_AVX512_VPCLMULQDQ_SUPPORT
  if (AVX512_VPCLMULQDQ_CPU_SUPPORTED && max_simd_bitwidth >= RTE_VECT_SIMD_512)
...
static void avx512_vpclmulqdq_init(void) {
#ifdef CC_X86_64_AVX512_VPCLMULQDQ_SUPPORT
...
static const rte_net_crc_handler *sse42_pclmulqdq_get_handlers(void) {
#ifdef CC_X86_64_SSE42_PCLMULQDQ_SUPPORT
  if (SSE42_PCLMULQDQ_CPU_SUPPORTED && max_simd_bitwidth >= RTE_VECT_SIMD_128)
...
static void sse42_pclmulqdq_init(void) {
#ifdef CC_X86_64_SSE42_PCLMULQDQ_SUPPORT
...
#define NEON_PMULL_CPU_SUPPORTED rte_cpu_get_flag_enabled(RTE_CPUFLAG_PMULL)

static const rte_net_crc_handler *neon_pmull_get_handlers(void) {
#ifdef CC_ARM64_NEON_PMULL_SUPPORT
  if (NEON_PMULL_CPU_SUPPORTED && max_simd_bitwidth >= RTE_VECT_SIMD_128)
...
static void neon_pmull_init(void) {
#ifdef CC_ARM64_NEON_PMULL_SUPPORT
...
void rte_net_crc_set_alg(enum rte_net_crc_alg alg) {
  handlers = NULL;
  if (max_simd_bitwidth == 0)
    max_simd_bitwidth = rte_vect_get_max_simd_bitwidth();
...
uint32_t rte_net_crc_calc(const void *data, uint32_t data_len,
