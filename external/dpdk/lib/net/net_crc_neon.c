 ...
 */
static inline uint64x2_t crcr32_folding_round(uint64x2_t data_block,
                                              uint64x2_t precomp,
                                              uint64x2_t fold) {
  uint64x2_t tmp0 = vreinterpretq_u64_p128(
      vmull_p64(vgetq_lane_p64(vreinterpretq_p64_u64(fold), 1),
                vgetq_lane_p64(vreinterpretq_p64_u64(precomp), 0)));

  uint64x2_t tmp1 = vreinterpretq_u64_p128(
      vmull_p64(vgetq_lane_p64(vreinterpretq_p64_u64(fold), 0),
 ...
 */
static inline uint64x2_t crcr32_reduce_128_to_64(uint64x2_t data128,
                                                 uint64x2_t precomp) {
  uint64x2_t tmp0, tmp1, tmp2;

  /* 64b fold */
  tmp0 = vreinterpretq_u64_p128(
      vmull_p64(vgetq_lane_p64(vreinterpretq_p64_u64(data128), 0),
                vgetq_lane_p64(vreinterpretq_p64_u64(precomp), 0)));
  ...
  tmp1 = vreinterpretq_u64_p128(
      vmull_p64(vgetq_lane_p64(vreinterpretq_p64_u64(tmp2), 0),
                vgetq_lane_p64(vreinterpretq_p64_u64(precomp), 1)));
 ...
 */
static inline uint32_t crcr32_reduce_64_to_32(uint64x2_t data64,
                                              uint64x2_t precomp) {
  static uint32_t mask1[4] __rte_aligned(16) = {0xffffffff, 0xffffffff,
                                                0x00000000, 0x00000000};
  static uint32_t mask2[4] __rte_aligned(16) = {0x00000000, 0xffffffff,
  ...
  tmp1 = vreinterpretq_u64_p128(
      vmull_p64(vgetq_lane_p64(vreinterpretq_p64_u64(tmp0), 0),
                vgetq_lane_p64(vreinterpretq_p64_u64(precomp), 0)));
  tmp1 = veorq_u64(tmp1, tmp0);
  tmp1 = vandq_u64(tmp1, vld1q_u64((uint64_t *)mask1));

  tmp2 = vreinterpretq_u64_p128(
      vmull_p64(vgetq_lane_p64(vreinterpretq_p64_u64(tmp1), 0),
                vgetq_lane_p64(vreinterpretq_p64_u64(precomp), 1)));
...
crc32_eth_calc_pmull(const uint8_t *data, uint32_t data_len, uint32_t crc,
...
partial_bytes:
  if (likely(n < data_len)) {
    ...
    /* k = rk1 & rk2 */
    temp = vreinterpretq_u64_p128(
        vmull_p64(vgetq_lane_p64(vreinterpretq_p64_u64(a), 1),
                  vgetq_lane_p64(vreinterpretq_p64_u64(k), 0)));
    fold = vreinterpretq_u64_p128(
        vmull_p64(vgetq_lane_p64(vreinterpretq_p64_u64(a), 0),
...
void rte_net_crc_neon_init(void) {
...
uint32_t rte_crc16_ccitt_neon_handler(const uint8_t *data, uint32_t data_len) {
  return (uint16_t)~crc32_eth_calc_pmull(data, data_len, 0xffff,
                                         &crc16_ccitt_pmull);
}

uint32_t rte_crc32_eth_neon_handler(const uint8_t *data, uint32_t data_len) {
  return ~crc32_eth_calc_pmull(data, data_len, 0xffffffffUL, &crc32_eth_pmull);
}
