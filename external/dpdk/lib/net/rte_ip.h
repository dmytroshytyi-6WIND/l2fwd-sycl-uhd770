 ...
 */
static inline int rte_raw_cksum_mbuf(const struct rte_mbuf *m, uint32_t off,
  ...
  if (off + len <= rte_pktmbuf_data_len(m)) {
    *cksum = rte_raw_cksum(rte_pktmbuf_mtod_offset(m, const char *, off), len);
    return 0;
  }
...
static inline uint16_t rte_ipv4_phdr_cksum(const struct rte_ipv4_hdr *ipv4_hdr,
  ...
  if (ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    psd_hdr.len =
        rte_cpu_to_be_16((uint16_t)(l3_len - rte_ipv4_hdr_len(ipv4_hdr)));
  }
