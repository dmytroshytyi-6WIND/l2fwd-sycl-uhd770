...
#define RTE_ETHER_MIN_MTU                                                      \
  68 /**< Minimum MTU for IPv4 packets, see RFC 791.                           \
      */
...
static inline int rte_vlan_strip(struct rte_mbuf *m) {
  ...
  memmove(rte_pktmbuf_adj(m, sizeof(struct rte_vlan_hdr)), eh,
...
static inline int rte_vlan_insert(struct rte_mbuf **m) {
  ...
  nh = (struct rte_ether_hdr *)(void *)rte_pktmbuf_prepend(
      *m, sizeof(struct rte_vlan_hdr));
  if (nh == NULL)
    return -ENOSPC;
