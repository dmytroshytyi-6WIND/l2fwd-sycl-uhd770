...
#define VALID_DEV_OR_RET_ERR(dev, dev_id)                                      \
  do {                                                                         \
    if (dev == NULL) {                                                         \
...
#define VALID_DEV_OPS_OR_RET_ERR(dev, dev_id)                                  \
  do {                                                                         \
    if (dev->dev_ops == NULL) {                                                \
      rte_bbdev_log(ERR, "NULL dev_ops structure in device %u", dev_id);       \
      return -ENODEV;                                                          \
    }                                                                          \
  } while (0)

/* Helper macro to check that driver implements required function pointer */
#define VALID_FUNC_OR_RET_ERR(func, dev_id)                                    \
  do {                                                                         \
    if (func == NULL) {                                                        \
      rte_bbdev_log(ERR, "device %u does not support %s", dev_id, #func);      \
      return -ENOTSUP;                                                         \
    }                                                                          \
  } while (0)

/* Helper macro to check that queue is valid */
#define VALID_QUEUE_OR_RET_ERR(queue_id, dev)                                  \
  do {                                                                         \
    if (queue_id >= dev->data->num_queues) {                                   \
      rte_bbdev_log(ERR, "Invalid queue_id %u for device %u", queue_id,        \
                    dev->data->dev_id);                                        \
      return -ERANGE;                                                          \
    }                                                                          \
  } while (0)
...
static struct rte_bbdev *get_dev(uint16_t dev_id) {
...
static int rte_bbdev_data_alloc(void) {
  ...
  if (mz == NULL) {
    rte_bbdev_log(CRIT, "Cannot allocate memzone for bbdev port data");
    return -ENOMEM;
  }

  rte_bbdev_data = mz->addr;
  if (rte_eal_process_type() == RTE_PROC_PRIMARY)
    memset(rte_bbdev_data, 0, RTE_BBDEV_MAX_DEVS * sizeof(*rte_bbdev_data));
  return 0;
}
 ...
 * NULL.
 */
static struct rte_bbdev_data *find_bbdev_data(const char *name) {
  uint16_t data_id;

  for (data_id = 0; data_id < RTE_BBDEV_MAX_DEVS; ++data_id) {
    if (strlen(rte_bbdev_data[data_id].name) == 0) {
      memset(&rte_bbdev_data[data_id], 0, sizeof(struct rte_bbdev_data));
...
static uint16_t find_free_dev_id(void) {
  uint16_t i;
...
struct rte_bbdev *rte_bbdev_allocate(const char *name) {
  int ret;
  ...
  rte_bbdev_log_debug("Initialised device %s (id = %u). Num devices = %u", name,
                      dev_id, num_devs);

  return bbdev;
}

int rte_bbdev_release(struct rte_bbdev *bbdev) {
  uint16_t dev_id;
  ...
  if (__atomic_sub_fetch(&bbdev->data->process_cnt, 1, __ATOMIC_RELAXED) == 0)
  ...
  rte_bbdev_log_debug("Un-initialised device id = %u. Num devices = %u", dev_id,
                      num_devs);
  return 0;
}
...
struct rte_bbdev *rte_bbdev_get_named_dev(const char *name) {
  ...
  for (i = 0; i < RTE_BBDEV_MAX_DEVS; i++) {
    ...
    if (dev && (strncmp(dev->data->name, name, RTE_BBDEV_NAME_MAX_LEN) == 0))
      return dev;
  }
...
uint16_t rte_bbdev_count(void) { return num_devs; }
...
int rte_bbdev_setup_queues(uint16_t dev_id, uint16_t num_queues,
  ...
  if (dev->data->started) {
    rte_bbdev_log(ERR, "Device %u cannot be configured when started", dev_id);
    return -EBUSY;
  }
  ...
  if ((num_queues == 0) || (num_queues > dev_info.max_num_queues)) {
    rte_bbdev_log(ERR, "Device %u supports 0 < N <= %u queues, not %u", dev_id,
                  dev_info.max_num_queues, num_queues);
    return -EINVAL;
  }
  ...
  if (dev->data->queues != NULL) {
    ...
    for (i = 0; i < dev->data->num_queues; i++) {
      ...
      if (ret < 0) {
        rte_bbdev_log(ERR, "Device %u queue %u release failed", dev_id, i);
        return ret;
      }
    }
    ...
    if (dev->dev_ops->close) {
      ...
      if (ret < 0) {
        rte_bbdev_log(ERR, "Device %u couldn't be closed", dev_id);
        return ret;
      }
    }
  ...
  dev->data->queues =
      rte_calloc_socket(DEV_NAME, num_queues, sizeof(dev->data->queues[0]),
                        RTE_CACHE_LINE_SIZE, dev->data->socket_id);
  if (dev->data->queues == NULL) {
    rte_bbdev_log(ERR, "calloc of %u queues for device %u on socket %i failed",
  ...
  if (dev->dev_ops->setup_queues) {
    ...
    if (ret < 0) {
      rte_bbdev_log(ERR, "Device %u memory configuration failed", dev_id);
      goto error;
    }
  }

  rte_bbdev_log_debug("Device %u set up with %u queues", dev_id, num_queues);
  return 0;
...
int rte_bbdev_intr_enable(uint16_t dev_id) {
  ...
  if (dev->dev_ops->intr_enable) {
    ...
    if (ret < 0) {
      rte_bbdev_log(ERR, "Device %u interrupts configuration failed", dev_id);
      return ret;
    }
...
int rte_bbdev_queue_configure(uint16_t dev_id, uint16_t queue_id,
  ...
  if (conf != NULL) {
    if ((conf->op_type == RTE_BBDEV_OP_NONE) &&
        (dev_info.capabilities[0].type == RTE_BBDEV_OP_NONE)) {
      ret = 1;
    } else {
      for (p = dev_info.capabilities; p->type != RTE_BBDEV_OP_NONE; p++) {
    ...
    if (conf->queue_size > dev_info.queue_size_lim) {
      rte_bbdev_log(ERR, "Size (%u) of queue %u of device %u must be: <= %u",
    ...
    if (conf->op_type == RTE_BBDEV_OP_TURBO_DEC &&
      ...
      rte_bbdev_log(ERR, "Priority (%u) of queue %u of bbdev %u must be <= %u",
  ...
  if (dev->data->queues[queue_id].queue_private != NULL) {
    ...
    if (ret < 0) {
      rte_bbdev_log(ERR, "Device %u queue %u release failed", dev_id, queue_id);
      return ret;
    }
  }

  /* Get driver to setup the queue */
  ret = dev->dev_ops->queue_setup(
      dev, queue_id, (conf != NULL) ? conf : &dev_info.default_queue_conf);
  if (ret < 0) {
    /* This may happen when trying different priority levels */
    rte_bbdev_log(INFO, "Device %u queue %u setup failed", dev_id, queue_id);
    return ret;
  }

  /* Store configuration */
  stored_conf = &dev->data->queues[queue_id].conf;
  memcpy(stored_conf, (conf != NULL) ? conf : &dev_info.default_queue_conf,
         sizeof(*stored_conf));
  ...
  rte_bbdev_log_debug("Configured dev%uq%u (size=%u, type=%s, prio=%u)", dev_id,
                      queue_id, stored_conf->queue_size, op_type_str,
...
int rte_bbdev_queue_start(uint16_t dev_id, uint16_t queue_id) {
  ...
  if (dev->data->queues[queue_id].started) {
    rte_bbdev_log_debug("Queue %u of device %u already started", queue_id,
                        dev_id);
    return 0;
  }
  ...
  if (dev->dev_ops->queue_start) {
    ...
    if (ret < 0) {
      rte_bbdev_log(ERR, "Device %u queue %u start failed", dev_id, queue_id);
      return ret;
    }
  }
...
int rte_bbdev_queue_stop(uint16_t dev_id, uint16_t queue_id) {
  ...
  if (!dev->data->queues[queue_id].started) {
    rte_bbdev_log_debug("Queue %u of device %u already stopped", queue_id,
                        dev_id);
    return 0;
  }
  ...
  if (dev->dev_ops->queue_stop) {
    ...
    if (ret < 0) {
      rte_bbdev_log(ERR, "Device %u queue %u stop failed", dev_id, queue_id);
      return ret;
    }
  }
...
static void get_stats_from_queues(struct rte_bbdev *dev,
                                  struct rte_bbdev_stats *stats) {
  unsigned int q_id;
  for (q_id = 0; q_id < dev->data->num_queues; q_id++) {
    struct rte_bbdev_stats *q_stats = &dev->data->queues[q_id].queue_stats;
...
static void reset_stats_in_queues(struct rte_bbdev *dev) {
  unsigned int q_id;
  for (q_id = 0; q_id < dev->data->num_queues; q_id++) {
    struct rte_bbdev_stats *q_stats = &dev->data->queues[q_id].queue_stats;
...
int rte_bbdev_queue_info_get(uint16_t dev_id, uint16_t queue_id,
  ...
  rte_bbdev_log_debug("Retrieved info of queue %u of device %u", queue_id,
                      dev_id);
  return 0;
}

/* Calculate size needed to store bbdev_op, depending on type */
static unsigned int get_bbdev_op_size(enum rte_bbdev_op_type type) {
...
static void bbdev_op_init(struct rte_mempool *mempool, void *arg, void *element,
  ...
  if (type == RTE_BBDEV_OP_TURBO_DEC || type == RTE_BBDEV_OP_LDPC_DEC) {
    ...
    op->mempool = mempool;
  } else if (type == RTE_BBDEV_OP_TURBO_ENC || type == RTE_BBDEV_OP_LDPC_ENC) {
...
struct rte_mempool *rte_bbdev_op_pool_create(const char *name,
  ...
  if (type >= RTE_BBDEV_OP_TYPE_COUNT) {
    rte_bbdev_log(ERR, "Invalid op type (%u), should be less than %u", type,
  ...
  if (mp == NULL) {
    rte_bbdev_log(
        ...
        name, num_elements, get_bbdev_op_size(type), rte_strerror(rte_errno));
    return NULL;
  }
...
int rte_bbdev_callback_register(uint16_t dev_id,
  ...
  if (event >= RTE_BBDEV_EVENT_MAX) {
    rte_bbdev_log(ERR, "Invalid event type (%u), should be less than %u", event,
                  RTE_BBDEV_EVENT_MAX);
    return -EINVAL;
  }
  ...
  if (user_cb == NULL) {
    user_cb =
        rte_zmalloc("INTR_USER_CALLBACK", sizeof(struct rte_bbdev_callback), 0);
    if (user_cb != NULL) {
      user_cb->cb_fn = cb_fn;
...
int rte_bbdev_callback_unregister(uint16_t dev_id,
  ...
  if (cb_fn == NULL) {
    rte_bbdev_log(ERR, "NULL callback function cannot be unregistered");
    return -EINVAL;
  }
...
int rte_bbdev_queue_intr_ctl(uint16_t dev_id, uint16_t queue_id, int epfd,
                             int op, void *data) {
  uint32_t vec;
  struct rte_bbdev *dev = get_dev(dev_id);
  ...
  if (ret && (ret != -EEXIST)) {
    rte_bbdev_log(ERR, "dev %u q %u int ctl error op %d epfd %d vec %u\n",
...
const char *rte_bbdev_op_type_str(enum rte_bbdev_op_type op_type) {
  static const char *const op_types[] = {
      "RTE_BBDEV_OP_NONE",      "RTE_BBDEV_OP_TURBO_DEC",
      "RTE_BBDEV_OP_TURBO_ENC", "RTE_BBDEV_OP_LDPC_DEC",
