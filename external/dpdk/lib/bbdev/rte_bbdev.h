...
enum rte_bbdev_state { RTE_BBDEV_UNUSED, RTE_BBDEV_INITIALIZED };
...
#define RTE_BBDEV_FOREACH(i)                                                   \
  for (i = rte_bbdev_find_next(-1); i < RTE_BBDEV_MAX_DEVS;                    \
...
typedef uint16_t (*rte_bbdev_enqueue_enc_ops_t)(
    struct rte_bbdev_queue_data *q_data, struct rte_bbdev_enc_op **ops,
    uint16_t num);

/** @internal Enqueue decode operations for processing on queue of a device. */
typedef uint16_t (*rte_bbdev_enqueue_dec_ops_t)(
    struct rte_bbdev_queue_data *q_data, struct rte_bbdev_dec_op **ops,
    uint16_t num);
 ...
 */
static inline uint16_t rte_bbdev_enqueue_enc_ops(uint16_t dev_id,
                                                 uint16_t queue_id,
 ...
 *   entries in the @p ops array).
 */
static inline uint16_t rte_bbdev_enqueue_dec_ops(uint16_t dev_id,
                                                 uint16_t queue_id,
 ...
 */
static inline uint16_t rte_bbdev_dequeue_enc_ops(uint16_t dev_id,
                                                 uint16_t queue_id,
                                                 struct rte_bbdev_enc_op **ops,
                                                 uint16_t num_ops) {
  struct rte_bbdev *dev = &rte_bbdev_devices[dev_id];
...
static inline uint16_t rte_bbdev_dequeue_dec_ops(uint16_t dev_id,
                                                 uint16_t queue_id,
                                                 struct rte_bbdev_dec_op **ops,
                                                 uint16_t num_ops) {
  struct rte_bbdev *dev = &rte_bbdev_devices[dev_id];
