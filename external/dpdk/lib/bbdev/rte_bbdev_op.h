 ...
 */
static inline int rte_bbdev_enc_op_alloc_bulk(struct rte_mempool *mempool,
 ...
 *   - EINVAL if invalid mempool is provided
 */
static inline int rte_bbdev_dec_op_alloc_bulk(struct rte_mempool *mempool,
 ...
 */
static inline void rte_bbdev_dec_op_free_bulk(struct rte_bbdev_dec_op **ops,
 ...
 *   Number of structures
 */
static inline void rte_bbdev_enc_op_free_bulk(struct rte_bbdev_enc_op **ops,
                                              unsigned int num_ops) {
  if (num_ops > 0)
    rte_mempool_put_bulk(ops[0]->mempool, (void **)ops, num_ops);
}
