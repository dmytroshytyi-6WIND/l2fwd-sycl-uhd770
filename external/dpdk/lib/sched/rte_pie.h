...
#define RTE_DQ_THRESHOLD                                                       \
  16384 /**< Queue length threshold (2^14)                                     \
         * to start measurement cycle (bytes)                                  \
         */
#define RTE_DQ_WEIGHT                                                          \
  0.25 /**< Weight (RTE_DQ_THRESHOLD/2^16) to compute dequeue rate */
...
struct rte_pie_config {
  ...
  uint64_t dp_update_interval; /**< Update interval for drop probability (in CPU
                                  cycles) */
...
struct rte_pie {
  ...
  uint32_t departed_bytes_count; /**< Number of bytes departed in current
                                    measurement cycle */
  uint64_t start_measurement;    /**< Time to start to measurement cycle (in cpu
                                    cycles) */
...
rte_pie_enqueue_empty(const struct rte_pie_config *pie_cfg, struct rte_pie *pie,
                      uint32_t pkt_len) {
   ...
   */
  if ((pie->active == 1) && (pie->qlen < (pie_cfg->tailq_th * 0.1))) {
    pie->active = 0;
 ...
 */
static void __rte_experimental _calc_drop_probability(
    const struct rte_pie_config *pie_cfg, struct rte_pie *pie, uint64_t time) {
...
_rte_pie_drop(const struct rte_pie_config *pie_cfg, struct rte_pie *pie) {
  uint64_t rand_value;
...
/**
 * @brief Decides if new packet should be enqueued or dropped for non-empty
 * queue
 *
 * @param pie_cfg [in] config pointer to a PIE configuration parameter structure
 * @param pie [in,out] data pointer to PIE runtime data
 ...
 */
static inline int __rte_experimental rte_pie_enqueue_nonempty(
    const struct rte_pie_config *pie_cfg, struct rte_pie *pie, uint32_t pkt_len,
    const uint64_t time) {
  ...
  if ((pie->active == 0) && (pie->qlen >= (pie_cfg->tailq_th * 0.1))) {
    pie->active = 1;
    pie->qdelay_old = 0;
    pie->drop_prob = 0;
 ...
 */
static inline int __rte_experimental rte_pie_enqueue(
    const struct rte_pie_config *pie_cfg, struct rte_pie *pie,
    const unsigned int qlen, uint32_t pkt_len, const uint64_t time) {
 ...
 */
static inline void __rte_experimental rte_pie_dequeue(struct rte_pie *pie,
                                                      uint32_t pkt_len,
                                                      uint64_t time) {
  ...
  if (pie->in_measurement) {
    ...
    if (pie->departed_bytes_count >= RTE_DQ_THRESHOLD) {
      ...
      if (pie->avg_dq_time == 0)
        ...
        pie->avg_dq_time =
            dq_time * RTE_DQ_WEIGHT + pie->avg_dq_time * (1 - RTE_DQ_WEIGHT);
