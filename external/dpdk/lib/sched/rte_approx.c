...
static inline uint32_t less(uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
  return a * d < b * c;
}

static inline uint32_t less_or_equal(uint32_t a, uint32_t b, uint32_t c,
                                     uint32_t d) {
  return a * d <= b * c;
}

/* check whether a/b is a valid approximation */
static inline uint32_t matches(uint32_t a, uint32_t b, uint32_t alpha_num,
                               uint32_t d_num, uint32_t denum) {
...
static inline void find_exact_solution_left(uint32_t p_a, uint32_t q_a,
                                            uint32_t p_b, uint32_t q_b,
                                            uint32_t alpha_num, uint32_t d_num,
                                            uint32_t denum, uint32_t *p,
                                            uint32_t *q) {
...
static inline void find_exact_solution_right(uint32_t p_a, uint32_t q_a,
                                             uint32_t p_b, uint32_t q_b,
                                             uint32_t alpha_num, uint32_t d_num,
                                             uint32_t denum, uint32_t *p,
                                             uint32_t *q) {
...
static int find_best_rational_approximation(uint32_t alpha_num, uint32_t d_num,
                                            uint32_t denum, uint32_t *p,
                                            uint32_t *q) {
  uint32_t p_a, q_a, p_b, q_b;

  /* check assumptions on the inputs */
  if (!((0 < d_num) && (d_num < alpha_num) && (alpha_num < denum) &&
        (d_num + alpha_num < denum))) {
    return -1;
  }
  ...
  while (1) {
    ...
    bb = matches(p_b + (x - 1) * p_a, q_b + (x - 1) * q_a, alpha_num, d_num,
                 denum);
    if (aa || bb) {
      find_exact_solution_left(p_a, q_a, p_b, q_b, alpha_num, d_num, denum, p,
                               q);
      return 0;
    }
    ...
    bb = matches(p_b + (x - 1) * p_a, q_b + (x - 1) * q_a, alpha_num, d_num,
                 denum);
    if (aa || bb) {
      find_exact_solution_right(p_a, q_a, p_b, q_b, alpha_num, d_num, denum, p,
                                q);
      return 0;
    }
...
static inline uint64_t less_64(uint64_t a, uint64_t b, uint64_t c, uint64_t d) {
  return a * d < b * c;
}

static inline uint64_t less_or_equal_64(uint64_t a, uint64_t b, uint64_t c,
                                        uint64_t d) {
  return a * d <= b * c;
}

/* check whether a/b is a valid approximation (64 bit version) */
static inline uint64_t matches_64(uint64_t a, uint64_t b, uint64_t alpha_num,
                                  uint64_t d_num, uint64_t denum) {
...
static inline void find_exact_solution_left_64(uint64_t p_a, uint64_t q_a,
                                               uint64_t p_b, uint64_t q_b,
                                               uint64_t alpha_num,
                                               uint64_t d_num, uint64_t denum,
...
static inline void find_exact_solution_right_64(uint64_t p_a, uint64_t q_a,
                                                uint64_t p_b, uint64_t q_b,
                                                uint64_t alpha_num,
                                                uint64_t d_num, uint64_t denum,
                                                uint64_t *p, uint64_t *q) {
...
static int find_best_rational_approximation_64(uint64_t alpha_num,
                                               uint64_t d_num, uint64_t denum,
                                               uint64_t *p, uint64_t *q) {
  uint64_t p_a, q_a, p_b, q_b;

  /* check assumptions on the inputs */
  if (!((d_num > 0) && (d_num < alpha_num) && (alpha_num < denum) &&
        (d_num + alpha_num < denum))) {
    return -1;
  }
  ...
  while (1) {
    ...
    bb = matches_64(p_b + (x - 1) * p_a, q_b + (x - 1) * q_a, alpha_num, d_num,
                    denum);
    if (aa || bb) {
      find_exact_solution_left_64(p_a, q_a, p_b, q_b, alpha_num, d_num, denum,
                                  p, q);
      return 0;
    }
    ...
    bb = matches_64(p_b + (x - 1) * p_a, q_b + (x - 1) * q_a, alpha_num, d_num,
                    denum);
    if (aa || bb) {
      find_exact_solution_right_64(p_a, q_a, p_b, q_b, alpha_num, d_num, denum,
                                   p, q);
      return 0;
    }
