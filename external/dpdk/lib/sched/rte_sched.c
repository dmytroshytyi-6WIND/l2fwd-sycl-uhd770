...
rte_sched_subport_pipe_qbase(struct rte_sched_subport *subport,
                             uint32_t qindex) {
  uint32_t pindex = qindex >> 4;
  uint32_t qpos = qindex & (RTE_SCHED_QUEUES_PER_PIPE - 1);

  return (subport->queue_array + pindex * subport->qsize_sum +
...
static inline uint16_t rte_sched_port_pipe_queue(struct rte_sched_port *port,
...
static inline uint8_t rte_sched_port_pipe_tc(struct rte_sched_port *port,
                                             uint32_t qindex) {
...
static inline uint8_t rte_sched_port_tc_queue(struct rte_sched_port *port,
                                              uint32_t qindex) {
...
static int pipe_profile_check(struct rte_sched_pipe_params *params,
                              uint64_t rate, uint16_t *qsize) {
  uint32_t i;

  /* Pipe parameters */
  if (params == NULL) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for parameter params\n", __func__);
    return -EINVAL;
  }

  /* TB rate: non-zero, not greater than port rate */
  if (params->tb_rate == 0 || params->tb_rate > rate) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for tb rate\n", __func__);
    return -EINVAL;
  }

  /* TB size: non-zero */
  if (params->tb_size == 0) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for tb size\n", __func__);
    return -EINVAL;
  }
  ...
  for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++) {
    if ((qsize[i] == 0 && params->tc_rate[i] != 0) ||
        (qsize[i] != 0 &&
         (params->tc_rate[i] == 0 || params->tc_rate[i] > params->tb_rate))) {
      RTE_LOG(ERR, SCHED, "%s: Incorrect value for qsize or tc_rate\n",
              __func__);
      return -EINVAL;
    }
  }
  ...
  if (params->tc_rate[RTE_SCHED_TRAFFIC_CLASS_BE] == 0 ||
    ...
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for be traffic class rate\n",
            __func__);
    return -EINVAL;
  }
  ...
  if (params->tc_period == 0) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for tc period\n", __func__);
    return -EINVAL;
  }

  /*  Best effort tc oversubscription weight: non-zero */
  if (params->tc_ov_weight == 0) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for tc ov weight\n", __func__);
    return -EINVAL;
  }
  ...
  for (i = 0; i < RTE_SCHED_BE_QUEUES_PER_PIPE; i++) {
    if (params->wrr_weights[i] == 0) {
      RTE_LOG(ERR, SCHED, "%s: Incorrect value for wrr weight\n", __func__);
      return -EINVAL;
    }
  }
...
static int rte_sched_port_check_params(struct rte_sched_port_params *params) {
  uint32_t i;
  ...
  /* socket */
  if (params->socket < 0) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for socket id\n", __func__);
    return -EINVAL;
  }

  /* rate */
  if (params->rate == 0) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for rate\n", __func__);
    return -EINVAL;
  }

  /* mtu */
  if (params->mtu == 0) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for mtu\n", __func__);
    return -EINVAL;
  }
  ...
  if (params->n_subports_per_port == 0 ||
    ...
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for number of subports\n",
            __func__);
    return -EINVAL;
  }

  if (params->subport_profiles == NULL || params->n_subport_profiles == 0 ||
      params->n_max_subport_profiles == 0 ||
      params->n_subport_profiles > params->n_max_subport_profiles) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for subport profiles\n", __func__);
    return -EINVAL;
  }

  for (i = 0; i < params->n_subport_profiles; i++) {
    struct rte_sched_subport_profile_params *p = params->subport_profiles + i;
    int status;

    status = subport_profile_check(p, params->rate);
    if (status != 0) {
      RTE_LOG(ERR, SCHED, "%s: subport profile check failed(%d)\n", __func__,
              status);
      return -EINVAL;
    }
  }
  ...
  if (params->n_pipes_per_subport == 0 ||
    ...
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for maximum pipes number\n",
            __func__);
    return -EINVAL;
  }
...
rte_sched_subport_get_array_base(struct rte_sched_subport_params *params,
  ...
  uint32_t size_queue = n_subport_pipe_queues * sizeof(struct rte_sched_queue);
...
static void rte_sched_subport_config_qsize(struct rte_sched_subport *subport) {
  uint32_t i;
...
static void rte_sched_port_log_pipe_profile(struct rte_sched_subport *subport,
                                            uint32_t i) {
  struct rte_sched_pipe_profile *p = subport->pipe_profiles + i;

  RTE_LOG(DEBUG, SCHED,
          "Low level config for pipe profile %u:\n"
          "	Token bucket: period = %" PRIu64
          ", credits per period = %" PRIu64 ", size = %" PRIu64 "\n"
          ...
          /* Traffic classes */
          p->tc_period, p->tc_credits_per_period[0],
          p->tc_credits_per_period[1], p->tc_credits_per_period[2],
          p->tc_credits_per_period[3], p->tc_credits_per_period[4],
          p->tc_credits_per_period[5], p->tc_credits_per_period[6],
          p->tc_credits_per_period[7], p->tc_credits_per_period[8],
          p->tc_credits_per_period[9], p->tc_credits_per_period[10],
          p->tc_credits_per_period[11], p->tc_credits_per_period[12],
...
static void rte_sched_port_log_subport_profile(struct rte_sched_port *port,
                                               uint32_t i) {
...
static inline uint64_t rte_sched_time_ms_to_bytes(uint64_t time_ms,
                                                  uint64_t rate) {
...
static void rte_sched_pipe_profile_convert(struct rte_sched_subport *subport,
  ...
  if (src->tb_rate == rate) {
    ...
    double tb_rate = (double)src->tb_rate / (double)rate;
    double d = RTE_SCHED_TB_RATE_CONFIG_ERR;

    rte_approx_64(tb_rate, d, &dst->tb_credits_per_period, &dst->tb_period);
  }
  ...
  for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++)
    if (subport->qsize[i])
      dst->tc_credits_per_period[i] =
          rte_sched_time_ms_to_bytes(src->tc_period, src->tc_rate[i]);
...
static void rte_sched_subport_config_pipe_profile_table(
    struct rte_sched_subport *subport, struct rte_sched_subport_params *params,
    uint64_t rate) {
  uint32_t i;
...
static void rte_sched_port_config_subport_profile_table(
    struct rte_sched_port *port, struct rte_sched_port_params *params,
    uint64_t rate) {
  uint32_t i;

  for (i = 0; i < port->n_subport_profiles; i++) {
    struct rte_sched_subport_profile_params *src = params->subport_profiles + i;
    struct rte_sched_subport_profile *dst = port->subport_profiles + i;
...
rte_sched_subport_check_params(struct rte_sched_subport_params *params,
  ...
  for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++) {
    ...
    if (qsize != 0 && !rte_is_power_of_2(qsize)) {
      RTE_LOG(ERR, SCHED, "%s: Incorrect value for qsize\n", __func__);
      return -EINVAL;
    }
  }
  ...
  if (params->n_pipes_per_subport_enabled == 0 ||
    ...
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for pipes number\n", __func__);
    return -EINVAL;
  }

  /* pipe_profiles and n_pipe_profiles */
  if (params->pipe_profiles == NULL || params->n_pipe_profiles == 0 ||
      params->n_max_pipe_profiles == 0 ||
      params->n_pipe_profiles > params->n_max_pipe_profiles) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for pipe profiles\n", __func__);
    return -EINVAL;
  }
  ...
  for (i = 0; i < params->n_pipe_profiles; i++) {
    ...
    if (status != 0) {
      RTE_LOG(ERR, SCHED, "%s: Pipe profile check failed(%d)\n", __func__,
              status);
      return -EINVAL;
    }
  }
...
uint32_t rte_sched_port_get_memory_footprint(
  ...
  if (status != 0) {
    RTE_LOG(ERR, SCHED, "%s: Port scheduler port params check failed (%d)\n",
            __func__, status);
  ...
  for (i = 0; i < port_params->n_subports_per_port; i++) {
    ...
    status = rte_sched_subport_check_params(
        sp, port_params->n_pipes_per_subport, port_params->rate);
    if (status != 0) {
      RTE_LOG(ERR, SCHED,
  ...
  for (i = 0; i < port_params->n_subports_per_port; i++) {
    ...
    size1 +=
        rte_sched_subport_get_array_base(sp, e_RTE_SCHED_SUBPORT_ARRAY_TOTAL);
  }
...
rte_sched_port_config(struct rte_sched_port_params *params) {
  ...
  if (status != 0) {
    RTE_LOG(ERR, SCHED, "%s: Port scheduler params check failed (%d)\n",
            __func__, status);
    return NULL;
  }
  ...
  size2 =
      params->n_max_subport_profiles * sizeof(struct rte_sched_subport_profile);
  ...
  port->subport_profiles = rte_zmalloc_socket(
      "subport_profile", size2, RTE_CACHE_LINE_SIZE, params->socket);
  ...
  port->n_pipes_per_subport_log2 = __builtin_ctz(params->n_pipes_per_subport);
  ...
  cycles_per_byte = (rte_get_tsc_hz() << RTE_SCHED_TIME_SHIFT) / params->rate;
...
static inline void rte_sched_subport_free(struct rte_sched_port *port,
                                          struct rte_sched_subport *subport) {
  uint32_t n_subport_pipe_queues;
  uint32_t qindex;
  ...
  for (qindex = 0; qindex < n_subport_pipe_queues; qindex++) {
    struct rte_mbuf **mbufs = rte_sched_subport_pipe_qbase(subport, qindex);
...
static void rte_sched_free_memory(struct rte_sched_port *port,
                                  uint32_t n_subports) {
  uint32_t i;
...
#ifdef RTE_SCHED_CMAN
static int rte_sched_red_config(struct rte_sched_port *port,
  ...
  for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++) {
    ...
    for (j = 0; j < RTE_COLORS; j++) {
      ...
      if (rte_red_config_init(&s->red_config[i][j],
                              ...
                              params->cman_params->red_params[i][j].maxp_inv) !=
          0) {
        rte_sched_free_memory(port, n_subports);

        RTE_LOG(NOTICE, SCHED, "%s: RED configuration init fails\n", __func__);
        return -EINVAL;
      }
    }
  }
...
static int rte_sched_pie_config(struct rte_sched_port *port,
                                struct rte_sched_subport *s,
                                struct rte_sched_subport_params *params,
                                uint32_t n_subports) {
  uint32_t i;
  ...
  for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++) {
    if (params->cman_params->pie_params[i].tailq_th > params->qsize[i]) {
      RTE_LOG(NOTICE, SCHED, "%s: PIE tailq threshold incorrect\n", __func__);
      return -EINVAL;
    }

    if (rte_pie_config_init(
            &s->pie_config[i], params->cman_params->pie_params[i].qdelay_ref,
      ...
      RTE_LOG(NOTICE, SCHED, "%s: PIE configuration init fails\n", __func__);
      return -EINVAL;
    }
  }
...
static int rte_sched_cman_config(struct rte_sched_port *port,
                                 struct rte_sched_subport *s,
                                 struct rte_sched_subport_params *params,
                                 uint32_t n_subports) {
...
int rte_sched_subport_config(struct rte_sched_port *port, uint32_t subport_id,
                             struct rte_sched_subport_params *params,
  ...
  /* Check user parameters */
  if (port == NULL) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for parameter port\n", __func__);
    return 0;
  }
  ...
  if (port->subports[subport_id] == NULL) {
    ...
    status = rte_sched_subport_check_params(params, port->n_pipes_per_subport,
                                            port->rate);
    if (status != 0) {
      RTE_LOG(NOTICE, SCHED, "%s: Port scheduler params check failed (%d)\n",
              __func__, status);
      ret = -EINVAL;
      goto out;
    }
    ...
    /* Allocate memory to store the data structures */
    s = rte_zmalloc_socket("subport_params", size0 + size1, RTE_CACHE_LINE_SIZE,
                           port->socket);
    if (s == NULL) {
      RTE_LOG(ERR, SCHED, "%s: Memory allocation fails\n", __func__);
      ret = -ENOMEM;
    ...
    /* Large data structures */
    s->pipe =
        (struct rte_sched_pipe *)(s->memory +
    ...
    s->bmp = rte_bitmap_init(n_subport_pipe_queues, s->bmp_array, bmp_mem_size);
    if (s->bmp == NULL) {
      RTE_LOG(ERR, SCHED, "%s: Subport bitmap init error\n", __func__);
      ret = -EINVAL;
      goto out;
    }
...

  {
    ...
    s->tc_ov_wm_max =
        rte_sched_time_ms_to_bytes(profile->tc_period, s->pipe_tc_be_rate_max);
...
int rte_sched_pipe_config(struct rte_sched_port *port, uint32_t subport_id,
                          uint32_t pipe_id, int32_t pipe_profile) {
  struct rte_sched_subport *s;
  ...
  if (pipe_id >= s->n_pipes_per_subport_enabled) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for parameter pipe id\n",
            __func__);
    ret = -EINVAL;
    goto out;
  }
  ...
  if (!deactivate && profile >= s->n_pipe_profiles) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for parameter pipe profile\n",
            __func__);
    ret = -EINVAL;
    goto out;
  }
  ...
  if (p->tb_time) {
    ...
    if (s->tc_ov != tc_be_ov) {
      RTE_LOG(DEBUG, SCHED,
              "Subport %u Best-effort TC oversubscription is OFF (%.4lf >= "
              "%.4lf)\n",
...
int rte_sched_port_subport_profile_add(
  ...
  for (i = 0; i < port->n_subport_profiles; i++)
    if (memcmp(port->subport_profiles + i, dst, sizeof(*dst)) == 0) {
      RTE_LOG(ERR, SCHED, "%s: subport profile exists\n", __func__);
      return -EINVAL;
    }
...
static inline uint32_t rte_sched_port_qindex(struct rte_sched_port *port,
                                             uint32_t subport, uint32_t pipe,
                                             uint32_t traffic_class,
                                             uint32_t queue) {
  return ((subport & (port->n_subports_per_port - 1))
          << (port->n_pipes_per_subport_log2 + 4)) |
         ((pipe & (port->subports[subport]->n_pipes_per_subport_enabled - 1))
          << 4) |
         ((rte_sched_port_pipe_queue(port, traffic_class) + queue) &
          (RTE_SCHED_QUEUES_PER_PIPE - 1));
}

void rte_sched_port_pkt_write(struct rte_sched_port *port, struct rte_mbuf *pkt,
                              uint32_t subport, uint32_t pipe,
...
enum rte_color rte_sched_port_pkt_read_color(const struct rte_mbuf *pkt) {
...
int rte_sched_subport_read_stats(struct rte_sched_port *port,
  ...
  if (stats == NULL) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for parameter stats\n", __func__);
    return -EINVAL;
  }

  if (tc_ov == NULL) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for tc_ov\n", __func__);
    return -EINVAL;
  }
...
int rte_sched_queue_read_stats(struct rte_sched_port *port, uint32_t queue_id,
                               struct rte_sched_queue_stats *stats,
                               uint16_t *qlen) {
  struct rte_sched_subport *s;
  ...
  if (queue_id >= rte_sched_port_queues_per_port(port)) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for queue id\n", __func__);
    return -EINVAL;
  }
  ...
  if (qlen == NULL) {
    RTE_LOG(ERR, SCHED, "%s: Incorrect value for parameter qlen\n", __func__);
    return -EINVAL;
  }
...
static inline void rte_sched_port_update_subport_stats_on_drop(
    struct rte_sched_port *port, struct rte_sched_subport *subport,
    uint32_t qindex, struct rte_mbuf *pkt,
...
static inline void rte_sched_port_update_queue_stats_on_drop(
    struct rte_sched_subport *subport, uint32_t qindex, struct rte_mbuf *pkt,
    __rte_unused uint32_t n_pkts_cman_dropped) {
  struct rte_sched_queue_extra *qe = subport->queue_extra + qindex;
  uint32_t pkt_len = pkt->pkt_len;
...
static inline int rte_sched_port_cman_drop(struct rte_sched_port *port,
                                           struct rte_sched_subport *subport,
                                           struct rte_mbuf *pkt,
...
static inline void rte_sched_port_pie_dequeue(struct rte_sched_subport *subport,
                                              uint32_t qindex, uint32_t pkt_len,
                                              uint64_t time) {
...
static inline int
rte_sched_port_cman_drop(struct rte_sched_port *port __rte_unused,
...
static inline void debug_check_queue_slab(struct rte_sched_subport *subport,
...
rte_sched_port_subport(struct rte_sched_port *port, struct rte_mbuf *pkt) {
  uint32_t queue_id = rte_mbuf_sched_queue_get(pkt);
...
static inline int rte_sched_port_enqueue_qwa(struct rte_sched_port *port,
                                             struct rte_sched_subport *subport,
                                             uint32_t qindex,
  ...
  if (unlikely(rte_sched_port_cman_drop(port, subport, pkt, qindex, qlen) ||
    ...
    rte_sched_port_update_subport_stats_on_drop(port, subport, qindex, pkt,
                                                qlen < qsize);
...
int rte_sched_port_enqueue(struct rte_sched_port *port, struct rte_mbuf **pkts,
                           uint32_t n_pkts) {
  struct rte_mbuf *pkt00, *pkt01, *pkt10, *pkt11, *pkt20, *pkt21, *pkt30,
      *pkt31, *pkt_last;
  struct rte_mbuf **q00_base, **q01_base, **q10_base, **q11_base, **q20_base,
      **q21_base, **q30_base, **q31_base, **q_last_base;
  ...
  if (unlikely(n_pkts < 6)) {
    ...
    for (i = 0; i < n_pkts; i++)
      q[i] = rte_sched_port_enqueue_qptrs_prefetch0(subports[i], pkts[i],
                                                    subport_qmask);
    ...
    for (i = 0; i < n_pkts; i++)
      result += rte_sched_port_enqueue_qwa(port, subports[i], q[i], q_base[i],
                                           pkts[i]);
  ...
  for (i = 6; i < (n_pkts & (~1)); i += 2) {
    ...
    subport11 = rte_sched_port_subport(port, pkt11);
    q10 =
        rte_sched_port_enqueue_qptrs_prefetch0(subport10, pkt10, subport_qmask);
    q11 =
        rte_sched_port_enqueue_qptrs_prefetch0(subport11, pkt11, subport_qmask);
  ...
  q_last = rte_sched_port_enqueue_qptrs_prefetch0(subport_last, pkt_last,
                                                  subport_qmask);
  ...
  if (n_pkts & 1) {
    r_last = rte_sched_port_enqueue_qwa(port, subport_last, q_last, q_last_base,
                                        pkt_last);
    result += r_last;
  }
...
grinder_tc_ov_credits_update(struct rte_sched_port *port,
  ...
  for (i = 0; i < RTE_SCHED_TRAFFIC_CLASS_BE; i++) {
    tc_ov_consumption[i] =
        sp->tc_credits_per_period[i] - subport->tc_credits[i];
  ...
  tc_ov_consumption_max =
      sp->tc_credits_per_period[RTE_SCHED_TRAFFIC_CLASS_BE] - tc_consumption;
...
static inline void grinder_credits_update(struct rte_sched_port *port,
                                          struct rte_sched_subport *subport,
                                          uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
...
static inline int grinder_credits_check(struct rte_sched_port *port,
                                        struct rte_sched_subport *subport,
                                        uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
  struct rte_sched_pipe *pipe = grinder->pipe;
  ...
  enough_credits =
      (pkt_len <= subport_tb_credits) && (pkt_len <= subport_tc_credits) &&
      (pkt_len <= pipe_tb_credits) && (pkt_len <= pipe_tc_credits) &&
...
static inline int grinder_schedule(struct rte_sched_port *port,
                                   struct rte_sched_subport *subport,
                                   uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
...
static inline int grinder_pipe_exists(struct rte_sched_subport *subport,
                                      uint32_t base_pipe) {
  uint32_t i;
...
static inline void grinder_pcache_populate(struct rte_sched_subport *subport,
                                           uint32_t pos, uint32_t bmp_pos,
                                           uint64_t bmp_slab) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
  uint16_t w[4];
...
static inline void grinder_tccache_populate(struct rte_sched_subport *subport,
                                            uint32_t pos, uint32_t qindex,
                                            uint16_t qmask) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
  uint8_t b, i;
  ...
  grinder->tccache_qmask[grinder->tccache_w] = b;
  grinder->tccache_qindex[grinder->tccache_w] =
      qindex + RTE_SCHED_TRAFFIC_CLASS_BE;
  grinder->tccache_w += (b != 0);
}

static inline int grinder_next_tc(struct rte_sched_port *port,
                                  struct rte_sched_subport *subport,
                                  uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
  struct rte_mbuf **qbase;
...
static inline int grinder_next_pipe(struct rte_sched_port *port,
                                    struct rte_sched_subport *subport,
                                    uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
  ...
  grinder->pipe_params =
      NULL; /* to be set after the pipe structure is prefetched */
...
static inline void grinder_wrr_load(struct rte_sched_subport *subport,
                                    uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
  struct rte_sched_pipe *pipe = grinder->pipe;
  ...
  grinder->wrr_tokens[0] = ((uint16_t)pipe->wrr_tokens[0])
                           << RTE_SCHED_WRR_SHIFT;
  grinder->wrr_tokens[1] = ((uint16_t)pipe->wrr_tokens[1])
                           << RTE_SCHED_WRR_SHIFT;
  grinder->wrr_tokens[2] = ((uint16_t)pipe->wrr_tokens[2])
                           << RTE_SCHED_WRR_SHIFT;
  grinder->wrr_tokens[3] = ((uint16_t)pipe->wrr_tokens[3])
                           << RTE_SCHED_WRR_SHIFT;
...
static inline void grinder_wrr_store(struct rte_sched_subport *subport,
                                     uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
  struct rte_sched_pipe *pipe = grinder->pipe;
...
static inline void grinder_wrr(struct rte_sched_subport *subport,
                               uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
...
static inline void grinder_prefetch_pipe(struct rte_sched_subport *subport,
                                         uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
...
static inline void
grinder_prefetch_tc_queue_arrays(struct rte_sched_subport *subport,
                                 uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
...
static inline void grinder_prefetch_mbuf(struct rte_sched_subport *subport,
                                         uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
...
static inline uint32_t grinder_handle(struct rte_sched_port *port,
                                      struct rte_sched_subport *subport,
                                      uint32_t pos) {
  struct rte_sched_grinder *grinder = subport->grinder + pos;
...
static inline void rte_sched_port_time_resync(struct rte_sched_port *port) {
...
static inline int rte_sched_port_exceptions(struct rte_sched_subport *subport,
                                            int second_pass) {
  int exceptions;
...
int rte_sched_port_dequeue(struct rte_sched_port *port, struct rte_mbuf **pkts,
  ...
  for (i = 0, count = 0;; i++) {
    ...
    count += grinder_handle(port, subport, i & (RTE_SCHED_PORT_N_GRINDERS - 1));
