...
#define RTE_RED_S                                                              \
  (1 << 22) /**< Packet size multiplied by number of leaf queues */
...
struct rte_red_params {
  ...
  uint16_t maxp_inv; /**< Inverse of packet marking probability maximum value
                        (maxp = 1 / maxp_inv) */
  uint16_t
      wq_log2; /**< Negated log2 of queue weight (wq = 1 / (2 ^ wq_log2)) */
};
...
struct rte_red_config {
  ...
  uint32_t pa_const; /**< Precomputed constant value used for pa calculation
...
int rte_red_config_init(struct rte_red_config *red_cfg, const uint16_t wq_log2,
                        const uint16_t min_th, const uint16_t max_th,
 ...
 */
static inline uint32_t rte_fast_rand(void) {
 ...
 */
static inline uint16_t __rte_red_calc_qempty_factor(uint8_t wq_log2,
                                                    uint16_t m) {
  uint32_t n = 0;
  uint32_t f = 0;
 ...
 */
static inline int rte_red_enqueue_empty(const struct rte_red_config *red_cfg,
                                        struct rte_red *red,
                                        const uint64_t time) {
   ...
   * empty. In this case we have time stamps provided by scheduler in byte units
   * (bytes transmitted on network port). Such time stamp translates into time
   * units as port speed is fixed but such approach simplifies the code.
   */
 ...
 */
static inline int __rte_red_drop(const struct rte_red_config *red_cfg,
                                 struct rte_red *red) {
...
/**
 * @brief Decides if new packet should be enqueued or dropped in queue non-empty
 * case
 *
 * @param red_cfg [in] config pointer to a RED configuration parameter structure
 * @param red [in,out] data pointer to RED runtime data
 ...
 * @retval 2 drop the packet based on mark probability criterion
 */
static inline int rte_red_enqueue_nonempty(const struct rte_red_config *red_cfg,
                                           struct rte_red *red,
                                           const unsigned q) {
  RTE_ASSERT(red_cfg != NULL);
  RTE_ASSERT(red != NULL);
...
static inline int rte_red_enqueue(const struct rte_red_config *red_cfg,
                                  struct rte_red *red, const unsigned q,
                                  const uint64_t time) {
  RTE_ASSERT(red_cfg != NULL);
  RTE_ASSERT(red != NULL);
 ...
 * @param time [in] current time stamp
 */
static inline void rte_red_mark_queue_empty(struct rte_red *red,
                                            const uint64_t time) {
  red->q_time = time;
}
