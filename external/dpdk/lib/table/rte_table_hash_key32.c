...
static int keycmp(void *a, void *b, void *b_mask) {
...
static void keycpy(void *dst, void *src, void *src_mask) {
...
static int check_params_create(struct rte_table_hash_params *params) {
  /* name */
  if (params->name == NULL) {
  ...
  /* n_buckets */
  if ((params->n_buckets == 0) || (!rte_is_power_of_2(params->n_buckets))) {
...
static void *rte_table_hash_create_key32_lru(void *params, int socket_id,
                                             uint32_t entry_size) {
   ...
   */
  n_buckets =
      rte_align32pow2((p->n_keys + KEYS_PER_BUCKET - 1) / KEYS_PER_BUCKET);
  ...
  f = rte_zmalloc_socket(p->name, (size_t)total_size, RTE_CACHE_LINE_SIZE,
                         socket_id);
  if (f == NULL) {
    RTE_LOG(ERR, TABLE,
            "%s: Cannot allocate %" PRIu64 " bytes "
  ...
  for (i = 0; i < n_buckets; i++) {
    ...
    bucket = (struct rte_bucket_4_32 *)&f->memory[i * f->bucket_size];
...
static int rte_table_hash_free_key32_lru(void *table) {
...
static int rte_table_hash_entry_add_key32_lru(void *table, void *key,
                                              void *entry, int *key_found,
                                              void **entry_ptr) {
  struct rte_table_hash *f = table;
  struct rte_bucket_4_32 *bucket;
  ...
  bucket = (struct rte_bucket_4_32 *)&f->memory[bucket_index * f->bucket_size];
...
static int rte_table_hash_entry_delete_key32_lru(void *table, void *key,
...
static void *rte_table_hash_create_key32_ext(void *params, int socket_id,
                                             uint32_t entry_size) {
  struct rte_table_hash_params *p = params;
  struct rte_table_hash *f;
  ...
  f->stack =
      (uint32_t *)&f->memory[(p->n_buckets + n_buckets_ext) * f->bucket_size];
...
static int rte_table_hash_free_key32_ext(void *table) {
  struct rte_table_hash *f = table;
...
static int rte_table_hash_entry_add_key32_ext(void *table, void *key,
                                              void *entry, int *key_found,
                                              void **entry_ptr) {
  struct rte_table_hash *f = table;
  ...
  signature = f->f_hash(key, f->key_mask, f->key_size, f->seed);
  bucket_index = signature & (f->n_buckets - 1);
  bucket0 = (struct rte_bucket_4_32 *)&f->memory[bucket_index * f->bucket_size];
  signature |= RTE_BUCKET_ENTRY_VALID;
  ...
  if (f->stack_pos > 0) {
    ...
    bucket = (struct rte_bucket_4_32 *)&f
                 ->memory[(f->n_buckets + bucket_index) * f->bucket_size];
...
static int rte_table_hash_entry_delete_key32_ext(void *table, void *key,
                                                 int *key_found, void *entry) {
  struct rte_table_hash *f = table;
  struct rte_bucket_4_32 *bucket0, *bucket, *bucket_prev;
  uint64_t signature;
    ...
    for (i = 0; i < 4; i++) {
      ...
      if ((bucket_signature == signature) &&
        ...
        *key_found = 1;
        ...
        if ((bucket->signature[0] == 0) && (bucket->signature[1] == 0) &&
            (bucket->signature[2] == 0) && (bucket->signature[3] == 0) &&
            (bucket_prev != NULL)) {
          bucket_prev->next = bucket->next;
          bucket_prev->next_valid = bucket->next_valid;

          memset(bucket, 0, sizeof(struct rte_bucket_4_32));
          bucket_index =
              (((uint8_t *)bucket - (uint8_t *)f->memory) / f->bucket_size) -
              f->n_buckets;
...
#define lookup1_stage1(mbuf1, bucket1, f)                                      \
  {                                                                            \
    ...
    bucket_index = signature & (f->n_buckets - 1);                             \
    bucket1 =                                                                  \
        (struct rte_bucket_4_32 *)&f->memory[bucket_index * f->bucket_size];   \
...
#define lookup1_stage2_lru(pkt2_index, mbuf2, bucket2, pkts_mask_out, entries, \
                           f)                                                  \
  {                                                                            \
    void *a;                                                                   \
    uint64_t pkt_mask;                                                         \
...
#define lookup_grinder(pkt_index, buckets, keys, pkts_mask_out, entries,       \
  ...
  {                                                                            \
    ...
    rte_prefetch0(                                                             \
        (void *)(((uintptr_t)bucket_next) + 2 * RTE_CACHE_LINE_SIZE));         \
    buckets[pkt_index] = bucket_next;                                          \
    keys[pkt_index] = key;                                                     \
  }

#define lookup2_stage0(pkt00_index, pkt01_index, mbuf00, mbuf01, pkts,         \
                       pkts_mask, f)                                           \
  {                                                                            \
...
#define lookup2_stage0_with_odd_support(pkt00_index, pkt01_index, mbuf00,      \
                                        mbuf01, pkts, pkts_mask, f)            \
  {                                                                            \
    uint64_t pkt00_mask, pkt01_mask;                                           \
    uint32_t key_offset = f->key_offset;                                       \
                                                                               \
...
#define lookup2_stage1(mbuf10, mbuf11, bucket10, bucket11, f)                  \
  {                                                                            \
    ...
    bucket10 =                                                                 \
        (struct rte_bucket_4_32 *)&f->memory[bucket10_index * f->bucket_size]; \
    ...
    bucket11 =                                                                 \
        (struct rte_bucket_4_32 *)&f->memory[bucket11_index * f->bucket_size]; \
...
#define lookup2_stage2_lru(pkt20_index, pkt21_index, mbuf20, mbuf21, bucket20, \
                           bucket21, pkts_mask_out, entries, f)                \
  {                                                                            \
    void *a20, *a21;                                                           \
...
#define lookup2_stage2_ext(pkt20_index, pkt21_index, mbuf20, mbuf21, bucket20, \
                           bucket21, pkts_mask_out, entries, buckets_mask,     \
                           buckets, keys, f)                                   \
  {                                                                            \
...
static int rte_table_hash_lookup_key32_lru(void *table, struct rte_mbuf **pkts,
                                           uint64_t pkts_mask,
  ...
  if (__builtin_popcountll(pkts_mask) < 5) {
    for (; pkts_mask;) {
      ...
      lookup1_stage2_lru(pkt_index, mbuf, bucket, pkts_mask_out, entries, f);
    }

    *lookup_hit_mask = pkts_mask_out;
    RTE_TABLE_HASH_KEY32_STATS_PKTS_LOOKUP_MISS(
        f, n_pkts_in - __builtin_popcountll(pkts_mask_out));
    return 0;
  }
   ...
   */
  /* Pipeline stage 0 */
  lookup2_stage0(pkt00_index, pkt01_index, mbuf00, mbuf01, pkts, pkts_mask, f);
  ...
  for (; pkts_mask;) {
    ...
    /* Pipeline stage 0 */
    lookup2_stage0_with_odd_support(pkt00_index, pkt01_index, mbuf00, mbuf01,
                                    pkts, pkts_mask, f);
    ...
    /* Pipeline stage 2 */
    lookup2_stage2_lru(pkt20_index, pkt21_index, mbuf20, mbuf21, bucket20,
                       bucket21, pkts_mask_out, entries, f);
  }
...
static int rte_table_hash_lookup_key32_ext(void *table, struct rte_mbuf **pkts,
                                           uint64_t pkts_mask,
                                           uint64_t *lookup_hit_mask,
                                           void **entries) {
  struct rte_table_hash *f = (struct rte_table_hash *)table;
  ...
  if (__builtin_popcountll(pkts_mask) < 5) {
    for (; pkts_mask;) {
      ...
      lookup1_stage0(pkt_index, mbuf, pkts, pkts_mask, f);
      lookup1_stage1(mbuf, bucket, f);
      lookup1_stage2_ext(pkt_index, mbuf, bucket, pkts_mask_out, entries,
                         buckets_mask, buckets, keys, f);
    }
  ...
  for (; pkts_mask;) {
    ...
    /* Pipeline stage 2 */
    lookup2_stage2_ext(pkt20_index, pkt21_index, mbuf20, mbuf21, bucket20,
                       bucket21, pkts_mask_out, entries, buckets_mask, buckets,
                       keys, f);
  }
...
grind_next_buckets:
  ...
  for (; buckets_mask;) {
    ...
    for (; buckets_mask;) {
      ...
      lookup_grinder(pkt_index, buckets, keys, pkts_mask_out, entries,
                     buckets_mask_next, f);
    }
...
static int rte_table_hash_key32_stats_read(void *table,
                                           struct rte_table_stats *stats,
                                           int clear) {
