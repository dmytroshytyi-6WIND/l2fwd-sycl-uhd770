...
static void *rte_table_acl_create(void *params, int socket_id,
                                  uint32_t entry_size) {
  ...
  RTE_BUILD_BUG_ON(((sizeof(struct rte_table_acl) % RTE_CACHE_LINE_SIZE) != 0));
  ...
  if ((p->n_rule_fields == 0) || (p->n_rule_fields > RTE_ACL_MAX_FIELDS)) {
  ...
  acl_rule_memory_size =
      RTE_CACHE_LINE_ROUNDUP(p->n_rules * RTE_ACL_RULE_SZ(p->n_rule_fields));
  total_size = sizeof(struct rte_table_acl) + action_table_size +
               acl_rule_list_size + acl_rule_memory_size;

  acl = rte_zmalloc_socket("TABLE", total_size, RTE_CACHE_LINE_SIZE, socket_id);
  if (acl == NULL) {
    RTE_LOG(ERR, TABLE, "%s: Cannot allocate %u bytes for ACL table\n",
            __func__, total_size);
    return NULL;
  }

  acl->action_table = &acl->memory[0];
  acl->acl_rule_list = (struct rte_acl_rule **)&acl->memory[action_table_size];
  acl->acl_rule_memory =
      (uint8_t *)&acl->memory[action_table_size + acl_rule_list_size];
...
static int rte_table_acl_free(void *table) {
...
static int rte_table_acl_build(struct rte_table_acl *acl,
  ...
  for (i = 1; i < acl->n_rules; i++) {
    if (acl->acl_rule_list[i] != NULL) {
      status = rte_acl_add_rules(ctx, acl->acl_rule_list[i], 1);
      if (status != 0) {
        RTE_LOG(ERR, TABLE, "%s: Cannot add rule to low level ACL table\n",
                __func__);
        rte_acl_free(ctx);
        return -1;
      }
  ...
  if (status != 0) {
    RTE_LOG(ERR, TABLE, "%s: Cannot build the low level ACL table\n", __func__);
    rte_acl_free(ctx);
    return -1;
  }
...
static int rte_table_acl_entry_add(void *table, void *key, void *entry,
                                   int *key_found, void **entry_ptr) {
  struct rte_table_acl *acl = table;
  struct rte_table_acl_rule_add_params *rule = key;
  ...
  memcpy(&acl_rule.field[0], &rule->field_value[0],
  ...
  for (i = 1; i < acl->n_rules; i++) {
    ...
    status = memcmp(&acl->acl_rule_list[i]->field[0], &rule->field_value[0],
                    acl->cfg.num_fields * sizeof(struct rte_acl_field));
  ...
  rule_location = (struct rte_acl_rule *)&acl
                      ->acl_rule_memory[free_pos * acl->acl_params.rule_size];
...
static int rte_table_acl_entry_delete(void *table, void *key, int *key_found,
                                      void *entry) {
  struct rte_table_acl *acl = table;
  struct rte_table_acl_rule_delete_params *rule = key;
  ...
  *key_found = 1;
  if (entry != NULL)
    memcpy(entry, &acl->memory[pos * acl->entry_size], acl->entry_size);

  return 0;
}

static int rte_table_acl_entry_add_bulk(void *table, void **keys,
                                        void **entries, uint32_t n_keys,
                                        int *key_found, void **entries_ptr) {
  struct rte_table_acl *acl = table;
  struct rte_acl_ctx *ctx;
  ...
  for (i = 0; i < n_keys; i++) {
    ...
    for (j = 1; j < acl->n_rules; j++) {
      ...
      /* Compare the key fields */
      status = memcmp(&acl->acl_rule_list[j]->field[0], &rule->field_value[0],
                      acl->cfg.num_fields * sizeof(struct rte_acl_field));
...
static int rte_table_acl_entry_delete_bulk(void *table, void **keys,
                                           uint32_t n_keys, int *key_found,
                                           void **entries) {
  struct rte_table_acl *acl = table;
  ...
  memset(rule_pos, 0, n_keys * sizeof(uint32_t));
  for (i = 0; i < n_keys; i++) {
    struct rte_table_acl_rule_delete_params *rule = keys[i];
...
static int rte_table_acl_lookup(void *table, struct rte_mbuf **pkts,
                                uint64_t pkts_mask, uint64_t *lookup_hit_mask,
                                void **entries) {
  ...
  /* Input conversion */
  for (i = 0, j = 0;
       i < (uint32_t)(RTE_PORT_IN_BURST_SIZE_MAX - __builtin_clzll(pkts_mask));
       i++) {
  ...
  for (i = 0; i < n_pkts; i++) {
    ...
    if (action_table_pos != 0) {
      ...
      entries[pkt_pos] =
          (void *)&acl->memory[action_table_pos * acl->entry_size];
      rte_prefetch0(entries[pkt_pos]);
    }
  }

  *lookup_hit_mask = pkts_out_mask;
  RTE_TABLE_ACL_STATS_PKTS_LOOKUP_MISS(
      acl, n_pkts_in - __builtin_popcountll(pkts_out_mask));

  return 0;
}

static int rte_table_acl_stats_read(void *table, struct rte_table_stats *stats,
                                    int clear) {
  struct rte_table_acl *acl = table;
