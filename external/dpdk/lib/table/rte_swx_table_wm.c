...
static uint32_t count_entries(struct rte_swx_table_entry_list *entries) {
...
static int acl_table_cfg_get(struct rte_acl_config *cfg,
...
static void acl_table_rule_field8(uint8_t *value, uint8_t *mask,
                                  uint8_t *key_mask0, uint8_t *key_mask,
                                  uint8_t *key, uint32_t offset) {
  uint8_t km0, km;
...
static void acl_table_rule_field32(uint32_t *value, uint32_t *mask,
                                   uint8_t *key_mask0, uint8_t *key_mask,
  ...
  *value = (k[0] << 24) | (k[1] << 16) | (k[2] << 8) | k[3];

  *mask = ((km[0] & km0[0]) << 24) | ((km[1] & km0[1]) << 16) |
          ((km[2] & km0[2]) << 8) | (km[3] & km0[3]);
}

RTE_ACL_RULE_DEF(acl_rule, RTE_ACL_MAX_FIELDS);

static struct rte_acl_rule *acl_table_rules_get(
    struct rte_acl_config *acl_cfg, struct rte_swx_table_params *p,
    struct rte_swx_table_entry_list *entries, uint32_t n_entries) {
  struct rte_swx_table_entry *entry;
  uint8_t *memory;
  ...
  TAILQ_FOREACH(entry, entries, node) {
    ...
    acl_rule->data.priority = RTE_ACL_MAX_PRIORITY - entry->key_priority;
    ...
    for (field_id = 0; field_id < n_fields; field_id++) {
      ...
      uint32_t offset = acl_cfg->defs[field_id].offset - p->key_offset;

      if (size == 1) {
        uint8_t value, mask;

        acl_table_rule_field8(&value, &mask, p->key_mask0, entry->key_mask,
                              entry->key, offset);
        ...
        acl_table_rule_field32(&value, &mask, p->key_mask0, entry->key_mask,
                               entry->key, p->key_size, offset);
...
static void entry_data_copy(uint8_t *data,
                            struct rte_swx_table_entry_list *entries,
                            uint32_t n_entries, uint32_t entry_data_size) {
  struct rte_swx_table_entry *entry;
  uint32_t i = 0;
...
static int table_lookup(void *table, void *mailbox __rte_unused,
                        const uint8_t **key, uint64_t *action_id,
