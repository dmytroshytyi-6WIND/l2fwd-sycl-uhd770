...
struct rte_swx_table_selector_params {
   ...
   * selector_offset* .. (*selector_offset* + *selector_size* - 1) of the
   * selector buffer in order to specify which bits from the selector buffer are
   * part of the selector and which ones are not. A bit value of 1 in the
   * *selector_mask* means the respective bit in the selector buffer is part of
   * the selector, while a bit value of 0 means the opposite. A NULL value means
   * that all the bits are part of the selector, i.e. the *selector_mask* is an
   * all-ones mask.
   */
 ...
 *   Selector table creation parameters.
 * @param[in] groups
 *   Groups to be added to the table at creation time. When NULL, it signifies
 * that all groups are invalid, otherwise it points to a pre-allocated array of
 * size *n_groups_max*, where a NULL element indicates that the associated group
 * is invalid.
 * @param[in] numa_node
...
rte_swx_table_selector_group_set(void *table, uint32_t group_id,
                                 struct rte_swx_table_selector_group *group);

/**
 * Selector table select
 *
 * This operation selects a member from the given group based on a hasing
 * scheme.
 *
 * Multiple invocations of this function may be required in order to complete a
 * single select operation for a given table and a given group ID. The
 * completion of the operation is flagged by a return value of 1; in case of a
 * return value of 0, the function must be invoked again with exactly the same
 * arguments.
 *
 * The mailbox argument is used to store the context of each on-going operation.
 * The mailbox mechanism allows for multiple concurrent select operations into
 * the same table.
 *
 * The typical reason an implementation may choose to split the operation into
 * multiple steps is to hide the latency of the inherent memory read operations:
 * before a read operation with the source data likely not in the CPU cache, the
 * source data prefetch is issued and the operation is postponed in favor of
 * some other unrelated work, which the CPU executes in parallel with the source
 * data being fetched into the CPU cache; later on, the operation is resumed,
 * this time with the source data likely to be read from the CPU cache with no
 * CPU pipeline stall, which significantly improves the operation performance.
 *
 * @param[in] table
 *   Selector table handle.
 * @param[in] mailbox
 ...
 *   Buffer where the key to select a member within the identified group is
 * located starting from offset *selector_offset*. Its size must be equal to the
 * table *selector_size*.
 * @param[in] member_id_buffer
 *   Buffer where the output member ID is to be placed at offset
 * *member_id_offset*.
 * @return
 *   0 when the operation is not yet completed, and 1 when the operation is
 * complete. No other return values are allowed.
 */
__rte_experimental int rte_swx_table_selector_select(
    void *table, void *mailbox, uint8_t **group_id_buffer,
    uint8_t **selector_buffer, uint8_t **member_id_buffer);
