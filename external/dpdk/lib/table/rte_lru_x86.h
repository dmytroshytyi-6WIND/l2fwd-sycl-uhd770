  ...
  do {                                                                         \
    ...
    static uint64_t masks[10] =                                                \
        {/* Shuffle order; Make Zero (see _mm_shuffle_epi8 manual) */          \
         0x0100070605040302, 0x8080808080808080, 0x0302070605040100,           \
         0x8080808080808080, 0x0504070603020100, 0x8080808080808080,           \
         0x0706050403020100, 0x8080808080808080, 0x0706050403020100,           \
         0x8080808080808080};                                                  \
...
static inline int f_lru_pos(uint64_t lru_list) {
...
#define lru_update(bucket, mru_val)                                            \
  do {                                                                         \
    const uint64_t orvals[] = {0xFFFFLLU, 0xFFFFLLU << 16, 0xFFFFLLU << 32,    \
                               0xFFFFLLU << 48, 0LLU};                         \
