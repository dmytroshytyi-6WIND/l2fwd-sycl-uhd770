...
uint64_t
rte_swx_table_selector_footprint_get(uint32_t n_groups_max,
...
void rte_swx_table_selector_free(void *table) {
...
static int table_create_check(struct rte_swx_table_selector_params *params) {
  if (!params)
    return -1;

  if (!params->selector_size || (params->selector_size > 64) ||
      !params->n_groups_max || (params->n_groups_max > 1U << 31) ||
...
static int table_params_copy(struct table *t,
                             struct rte_swx_table_selector_params *params) {
  ...
  t->params.n_members_per_group_max =
      rte_align32pow2(params->n_members_per_group_max);
  ...
  if (params->selector_mask)
    memcpy(t->params.selector_mask, params->selector_mask,
           params->selector_size);
  else
...
static int group_set(struct table *t, uint32_t group_id,
...
rte_swx_table_selector_create(struct rte_swx_table_selector_params *params,
  ...
  t->group_table =
      env_calloc(t->group_table_size, RTE_CACHE_LINE_SIZE, numa_node);
  if (!t->group_table)
    goto error;

  t->members =
      calloc(params->n_members_per_group_max, sizeof(struct group_member_info));
  if (!t->members)
    goto error;
...
static int group_check(struct table *t,
                       struct rte_swx_table_selector_group *group) {
...
static uint32_t members_read(struct group_member_info *members,
                             struct rte_swx_table_selector_group *group) {
  struct rte_swx_table_selector_member *elem;
  uint32_t n_members = 0;
...
static uint32_t members_min_weight_find(struct group_member_info *members,
                                        uint32_t n_members) {
...
static uint32_t members_weight_divisor_check(struct group_member_info *members,
                                             uint32_t n_members,
                                             uint32_t divisor) {
  uint32_t i;
...
static void members_weight_divisor_apply(struct group_member_info *members,
                                         uint32_t n_members, uint32_t divisor) {
  uint32_t i;
...
static uint32_t members_weight_sum(struct group_member_info *members,
                                   uint32_t n_members) {
  uint32_t result = 0, i;
...
static void members_weight_scale(struct group_member_info *members,
                                 uint32_t n_members,
                                 uint32_t n_members_per_group_max,
                                 uint32_t weight_sum) {
...
static void members_write(struct group_member_info *members, uint32_t n_members,
                          uint32_t *group_table) {
  uint32_t pos = 0, i;
...
static int group_set(struct table *t, uint32_t group_id,
  ...
  members_weight_scale(members, n_members, t->params.n_members_per_group_max,
                       weight_sum);
...
uint64_t rte_swx_table_selector_mailbox_size_get(void) {
  return sizeof(struct mailbox);
}

int rte_swx_table_selector_select(void *table, void *mailbox __rte_unused,
                                  uint8_t **group_id_buffer,
                                  uint8_t **selector_buffer,
                                  uint8_t **member_id_buffer) {
  struct table *t = table;
  uint32_t *group_id_ptr, *member_id_ptr, group_id, member_id, selector,
      group_member_index;
  ...
  selector = hash(&(*selector_buffer)[t->params.selector_offset],
                  t->params.selector_mask, t->params.selector_size, 0);
