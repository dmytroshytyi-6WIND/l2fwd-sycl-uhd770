...
static void *rte_table_stub_create(__rte_unused void *params,
  ...
  stub = rte_zmalloc_socket("TABLE", size, RTE_CACHE_LINE_SIZE, socket_id);
  if (stub == NULL) {
    RTE_LOG(ERR, TABLE, "%s: Cannot allocate %u bytes for stub table\n",
            __func__, size);
    return NULL;
  }
...
static int rte_table_stub_lookup(__rte_unused void *table,
...
static int rte_table_stub_stats_read(void *table, struct rte_table_stats *stats,
                                     int clear) {
