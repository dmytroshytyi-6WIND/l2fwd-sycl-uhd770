...
static void *env_calloc(size_t size, size_t alignment, int numa_node) {
...
static void *env_calloc(size_t size, size_t alignment __rte_unused,
                        int numa_node) {
  void *start;
...
static inline uint64_t crc32_u64_generic(uint64_t crc, uint64_t value) {
  int i;
...
static inline uint32_t hash(void *key, void *key_mask, uint32_t key_size,
                            uint32_t seed) {
  uint64_t *k = key;
  uint64_t *m = key_mask;
 ...
 */
static inline uint32_t table_keycmp(void *a, void *b, void *b_mask,
                                    uint32_t n_bytes) {
...
struct table_params {
   ...
   * generation (in increments of 8 bytes, from 8 to 64 bytes) and for run-time
   * key comparison. This is why key sizes bigger than 64 bytes are not allowed.
   */
  size_t key_size_pow2;

  /* log2(key_size_pow2). Purpose: avoid multiplication with non-power-of-2
   * numbers. */
  ...
  /* The data size, i.e. the 8-byte action_id field plus the action data size,
   * upgraded to the next power of 2.
   */
  size_t data_size_pow2;

  /* log2(data_size_pow2). Purpose: avoid multiplication with non-power of 2
   * numbers. */
  size_t data_size_log2;

  /* Number of buckets. Must be a power of 2 to avoid modulo with non-power-of-2
   * numbers. */
  size_t n_buckets;
  ...
  /* Total number of key bytes in the bucket, including the key padding bytes.
   * There are (key_size_pow2 - key_size) padding bytes for each key in the
   * bucket.
   */
  size_t bucket_key_all_size;

  /* Bucket size. Must be a power of 2 to avoid multiplication with
   * non-power-of-2 number. */
  size_t bucket_size;
...
static int table_params_get(struct table_params *p,
                            struct rte_swx_table_learner_params *params) {
  /* Check input parameters. */
  if (!params || !params->key_size || (params->key_size > 64) ||
      !params->n_keys_max || (params->n_keys_max > 1U << 31) ||
      !params->key_timeout)
    return -EINVAL;
  ...
  p->bucket_size =
      rte_align64pow2(sizeof(struct table_bucket) + p->bucket_key_all_size +
...
static inline struct table_bucket *table_bucket_get(struct table *t,
                                                    size_t bucket_id) {
  return (struct table_bucket *)&t
      ->buckets[bucket_id << t->params.bucket_size_log2];
}

static inline uint8_t *table_bucket_key_get(struct table *t,
                                            struct table_bucket *b,
                                            size_t bucket_key_pos) {
  return &b->key[bucket_key_pos << t->params.key_size_log2];
}

static inline uint64_t *table_bucket_data_get(struct table *t,
                                              struct table_bucket *b,
                                              size_t bucket_key_pos) {
...
void *rte_swx_table_learner_create(struct rte_swx_table_learner_params *params,
                                   int numa_node) {
  struct table_params p;
  struct table *t;
  int status;
...
void rte_swx_table_learner_free(void *table) {
...
uint64_t rte_swx_table_learner_mailbox_size_get(void) {
  return sizeof(struct mailbox);
}

int rte_swx_table_learner_lookup(void *table, void *mailbox,
                                 uint64_t input_time, uint8_t **key,
                                 uint64_t *action_id, uint8_t **action_data,
                                 int *hit) {
  struct table *t = table;
  ...
  case 1: {
    ...
    for (i = 0; i < TABLE_KEYS_PER_BUCKET; i++) {
      ...
      if ((time > input_time) && (sig == m->input_sig) &&
...
uint32_t rte_swx_table_learner_add(void *table, void *mailbox,
                                   uint64_t input_time, uint64_t action_id,
                                   uint8_t *action_data) {
  struct table *t = table;
  struct mailbox *m = mailbox;
  struct table_bucket *b = m->bucket;
  ...
  /* Lookup hit: The key, key signature and key time are already properly
   * configured (the key time was bumped by lookup), only the key data need to
   * be updated.
   */
  if (m->hit) {
  ...
  /* Lookup miss: Search for a free position in the current bucket and install
   * the key. */
  for (i = 0; i < TABLE_KEYS_PER_BUCKET; i++) {
    uint64_t time = b->time[i];
    ...
    /* Free position: Either there was never a key installed here, so the key
     * time is set to zero (the init value), which is always less than the
     * current time, or this position was used before, but the key expired (the
     * key time is in the past).
     */
    if (time < input_time) {
      uint8_t *key = table_bucket_key_get(t, b, i);
      uint64_t *data = table_bucket_data_get(t, b, i);
...
void rte_swx_table_learner_delete(void *table __rte_unused, void *mailbox) {
  struct mailbox *m = mailbox;
