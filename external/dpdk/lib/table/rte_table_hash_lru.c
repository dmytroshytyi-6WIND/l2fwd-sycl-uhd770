...
static void *rte_table_hash_lru_create(void *params, int socket_id,
                                       uint32_t entry_size) {
  ...
  total_size =
      table_meta_sz + key_mask_sz + bucket_sz + key_sz + key_stack_sz + data_sz;
  ...
  RTE_LOG(INFO, TABLE,
          "%s (%u-byte key): Hash table %s memory footprint"
          " is %" PRIu64 " bytes\n",
...
static int rte_table_hash_lru_free(void *table) {
...
static int rte_table_hash_lru_entry_add(void *table, void *key, void *entry,
...
static int rte_table_hash_lru_entry_delete(void *table, void *key,
...
static int rte_table_hash_lru_lookup_unoptimized(void *table,
  ...
  *lookup_hit_mask = pkts_mask_out;
  RTE_TABLE_HASH_LRU_STATS_PKTS_LOOKUP_MISS(
 ...
 *
 ***/
static int rte_table_hash_lru_lookup(void *table, struct rte_mbuf **pkts,
                                     uint64_t pkts_mask,
                                     uint64_t *lookup_hit_mask,
                                     void **entries) {
  struct rte_table_hash *t = (struct rte_table_hash *)table;
  ...
  if (__builtin_popcountll(pkts_mask) < 7)
    return rte_table_hash_lru_lookup_unoptimized(table, pkts, pkts_mask,
  ...
  if (pkts_mask_match_many) {
    ...
    status = rte_table_hash_lru_lookup_unoptimized(
...
static int rte_table_hash_lru_stats_read(void *table,
                                         struct rte_table_stats *stats,
                                         int clear) {
  struct rte_table_hash *t = table;
