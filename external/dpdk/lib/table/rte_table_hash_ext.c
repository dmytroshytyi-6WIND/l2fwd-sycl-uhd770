...
#define BUCKET_NEXT(bucket) ((void *)((bucket)->next & (~1LU)))

#define BUCKET_NEXT_VALID(bucket) ((bucket)->next & 1LU)
...
static int keycmp(void *a, void *b, void *b_mask, uint32_t n_bytes) {
...
static void *rte_table_hash_ext_create(void *params, int socket_id,
                                       uint32_t entry_size) {
  ...
  if ((check_params_create(p) != 0) || (!rte_is_power_of_2(entry_size)) ||
  ...
  bucket_ext_sz = RTE_CACHE_LINE_ROUNDUP(n_buckets_ext * sizeof(struct bucket));
  ...
  t = rte_zmalloc_socket(p->name, (size_t)total_size, RTE_CACHE_LINE_SIZE,
                         socket_id);
  if (t == NULL) {
    RTE_LOG(ERR, TABLE,
            "%s: Cannot allocate %" PRIu64 " bytes"
  ...
  RTE_LOG(INFO, TABLE,
          "%s (%u-byte key): Hash table %s memory "
...
static int rte_table_hash_ext_free(void *table) {
...
static int rte_table_hash_ext_entry_add(void *table, void *key, void *entry,
  ...
  for (bkt = bkt0; bkt != NULL; bkt = BUCKET_NEXT(bkt))
    for (i = 0; i < KEYS_PER_BUCKET; i++) {
      ...
      uint8_t *bkt_key = &t->key_mem[bkt_key_index << t->key_size_shl];
  ...
  for (bkt_prev = NULL, bkt = bkt0; bkt != NULL;
       bkt_prev = bkt, bkt = BUCKET_NEXT(bkt))
    for (i = 0; i < KEYS_PER_BUCKET; i++) {
      uint64_t bkt_sig = (uint64_t)bkt->sig[i];
...
static int rte_table_hash_ext_entry_delete(void *table, void *key,
    ...
    for (i = 0; i < KEYS_PER_BUCKET; i++) {
      ...
      if ((sig == bkt_sig) &&
        ...
        *key_found = 1;
        ...
        if ((bkt_prev != NULL) && (bkt->sig[0] == 0) && (bkt->sig[1] == 0) &&
          ...
          t->bkt_ext_stack[t->bkt_ext_stack_tos++] = bkt_index;
        }
...
static int rte_table_hash_ext_lookup_unoptimized(void *table,
...
#define lookup_cmp_key(mbuf, key, match_key, f)                                \
  {                                                                            \
    ...
    case 64: {                                                                 \
      ...
      or = xor[0] | xor[1] | xor[2] | xor[3] | xor[4] | xor[5] | xor[6] |      \
           xor[7];                                                             \
      match_key = 0;                                                           \
      if (or == 0)                                                             \
        match_key = 1;                                                         \
 ...
 *
 ***/
static int rte_table_hash_ext_lookup(void *table, struct rte_mbuf **pkts,
                                     uint64_t pkts_mask,
                                     uint64_t *lookup_hit_mask,
                                     void **entries) {
  struct rte_table_hash *t = (struct rte_table_hash *)table;
  ...
  if (__builtin_popcountll(pkts_mask) < 7) {
    status = rte_table_hash_ext_lookup_unoptimized(table, pkts, pkts_mask,
                                                   lookup_hit_mask, entries);
    RTE_TABLE_HASH_EXT_STATS_PKTS_LOOKUP_MISS(
        t, n_pkts_in - __builtin_popcountll(*lookup_hit_mask));
    return status;
  }
  ...
  for (; pkts_mask;) {
    ...
    /* Pipeline stage 0 */
    lookup2_stage0_with_odd_support(t, g, pkts, pkts_mask, pkt00_index,
                                    pkt01_index);
  ...
  /* Pipeline stage 3 */
  lookup2_stage3(t, g, pkts, pkt30_index, pkt31_index, pkts_mask_out, entries);
  ...
  if (pkts_mask_match_many) {
    ...
    status = rte_table_hash_ext_lookup_unoptimized(
        table, pkts, pkts_mask_match_many, &pkts_mask_out_slow, entries);
    pkts_mask_out |= pkts_mask_out_slow;
  }

  *lookup_hit_mask = pkts_mask_out;
  RTE_TABLE_HASH_EXT_STATS_PKTS_LOOKUP_MISS(
      t, n_pkts_in - __builtin_popcountll(pkts_mask_out));
  return status;
}

static int rte_table_hash_ext_stats_read(void *table,
                                         struct rte_table_stats *stats,
                                         int clear) {
  struct rte_table_hash *t = table;
