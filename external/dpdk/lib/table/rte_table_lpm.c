...
static void *rte_table_lpm_create(void *params, int socket_id,
                                  uint32_t entry_size) {
...
static int rte_table_lpm_free(void *table) {
...
static int nht_find_free(struct rte_table_lpm *lpm, uint32_t *pos) {
  uint32_t i;
...
static int nht_find_existing(struct rte_table_lpm *lpm, void *entry,
                             uint32_t *pos) {
  uint32_t i;
...
static int rte_table_lpm_entry_add(void *table, void *key, void *entry,
  ...
  status = rte_lpm_is_rule_present(lpm->lpm, ip_prefix->ip, ip_prefix->depth,
                                   &nht_pos0);
...
static int rte_table_lpm_entry_delete(void *table, void *key, int *key_found,
                                      void *entry) {
  struct rte_table_lpm *lpm = table;
  struct rte_table_lpm_key *ip_prefix = key;
  uint32_t nht_pos;
...
static int rte_table_lpm_lookup(void *table, struct rte_mbuf **pkts,
  ...
  *lookup_hit_mask = pkts_out_mask;
  RTE_TABLE_LPM_STATS_PKTS_LOOKUP_MISS(
      lpm, n_pkts_in - __builtin_popcountll(pkts_out_mask));
  return 0;
}

static int rte_table_lpm_stats_read(void *table, struct rte_table_stats *stats,
                                    int clear) {
