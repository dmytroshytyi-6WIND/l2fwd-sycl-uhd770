...
static void *rte_table_hash_cuckoo_create(void *params, int socket_id,
                                          uint32_t entry_size) {
  ...
  struct rte_hash_parameters hash_cuckoo_params = {.entries = p->n_keys,
                                                   .key_len = p->key_size,
                                                   .hash_func = p->f_hash,
                                                   .hash_func_init_val =
                                                       p->seed,
  ...
  if (h_table == NULL) {
    ...
    if (h_table == NULL) {
      RTE_LOG(ERR, TABLE, "%s: failed to create cuckoo hash table %s\n",
              __func__, p->name);
      rte_free(t);
      return NULL;
    }
...
static int rte_table_hash_cuckoo_free(void *table) {
...
static int rte_table_hash_cuckoo_entry_add(void *table, void *key, void *entry,
...
static int rte_table_hash_cuckoo_entry_delete(void *table, void *key,
                                              int *key_found, void *entry) {
  struct rte_table_hash *t = table;
  int pos = 0;

  /* Check input parameters */
  if ((table == NULL) || (key == NULL) || (key_found == NULL))
    return -EINVAL;
...
static int rte_table_hash_cuckoo_lookup(void *table, struct rte_mbuf **pkts,
                                        uint64_t pkts_mask,
  ...
  if ((pkts_mask & (pkts_mask + 1)) == 0) {
    ...
    /* Bulk Lookup */
    status = rte_hash_lookup_bulk(t->h_table, (const void **)keys, n_pkts_in,
                                  positions);
    if (status == 0) {
      for (i = 0; i < n_pkts_in; i++) {
    ...
    for (i = 0; i < (uint32_t)(RTE_PORT_IN_BURST_SIZE_MAX -
                               __builtin_clzll(pkts_mask));
         i++) {
      uint64_t pkt_mask = 1LLU << i;

      if (pkt_mask & pkts_mask) {
        struct rte_mbuf *pkt = pkts[i];
        uint8_t *key = RTE_MBUF_METADATA_UINT8_PTR(pkt, t->key_offset);
        int pos;
  ...
  *lookup_hit_mask = pkts_mask_out;
  RTE_TABLE_HASH_CUCKOO_STATS_PKTS_LOOKUP_MISS(
      t, n_pkts_in - __builtin_popcountll(pkts_mask_out));

  return 0;
}

static int rte_table_hash_cuckoo_stats_read(void *table,
                                            struct rte_table_stats *stats,
                                            int clear) {
  struct rte_table_hash *t = table;
