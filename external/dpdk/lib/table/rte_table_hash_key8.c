...
static void *rte_table_hash_create_key8_lru(void *params, int socket_id,
                                            uint32_t entry_size) {
  ...
  for (i = 0; i < n_buckets; i++) {
    ...
    bucket = (struct rte_bucket_4_8 *)&f->memory[i * f->bucket_size];
...
static int rte_table_hash_free_key8_lru(void *table) {
...
static int rte_table_hash_entry_add_key8_lru(void *table, void *key,
  ...
  bucket = (struct rte_bucket_4_8 *)&f->memory[bucket_index * f->bucket_size];
...
static int rte_table_hash_entry_delete_key8_lru(void *table, void *key,
...
static void *rte_table_hash_create_key8_ext(void *params, int socket_id,
                                            uint32_t entry_size) {
  struct rte_table_hash_params *p = params;
  struct rte_table_hash *f;
...
static int rte_table_hash_free_key8_ext(void *table) {
  struct rte_table_hash *f = table;
...
static int rte_table_hash_entry_add_key8_ext(void *table, void *key,
                                             void *entry, int *key_found,
                                             void **entry_ptr) {
  struct rte_table_hash *f = table;
  ...
  signature = f->f_hash(key, &f->key_mask, f->key_size, f->seed);
  bucket_index = signature & (f->n_buckets - 1);
  bucket0 = (struct rte_bucket_4_8 *)&f->memory[bucket_index * f->bucket_size];
  ...
  if (f->stack_pos > 0) {
    ...
    bucket = (struct rte_bucket_4_8 *)&f
...
static int rte_table_hash_entry_delete_key8_ext(void *table, void *key,
                                                int *key_found, void *entry) {
  struct rte_table_hash *f = table;
  struct rte_bucket_4_8 *bucket0, *bucket, *bucket_prev;
  uint64_t signature;
    ...
    for (i = 0, mask = 1LLU; i < 4; i++, mask <<= 1) {
      ...
      if ((bucket_signature & mask) &&
        ...
        *key_found = 1;
        ...
        if ((bucket->signature == 0) && (bucket_prev != NULL)) {
          bucket_prev->next = bucket->next;
          bucket_prev->next_valid = bucket->next_valid;

          memset(bucket, 0, sizeof(struct rte_bucket_4_8));
          bucket_index =
...
#define lookup1_stage1(mbuf1, bucket1, f)                                      \
  {                                                                            \
    ...
    bucket_index = signature & (f->n_buckets - 1);                             \
    bucket1 =                                                                  \
        (struct rte_bucket_4_8 *)&f->memory[bucket_index * f->bucket_size];    \
...
#define lookup2_stage1(mbuf10, mbuf11, bucket10, bucket11, f)                  \
  {                                                                            \
    ...
    bucket10 =                                                                 \
        (struct rte_bucket_4_8 *)&f->memory[bucket10_index * f->bucket_size];  \
    rte_prefetch0(bucket10);                                                   \
                                                                               \
    signature11 = f_hash(key11, &f->key_mask, KEY_SIZE, seed);                 \
    bucket11_index = signature11 & (f->n_buckets - 1);                         \
    bucket11 =                                                                 \
        (struct rte_bucket_4_8 *)&f->memory[bucket11_index * f->bucket_size];  \
...
static int rte_table_hash_lookup_key8_lru(void *table, struct rte_mbuf **pkts,
                                          uint64_t pkts_mask,
  ...
  if (__builtin_popcountll(pkts_mask) < 5) {
    ...
    *lookup_hit_mask = pkts_mask_out;
    RTE_TABLE_HASH_KEY8_STATS_PKTS_LOOKUP_MISS(
...
static int rte_table_hash_lookup_key8_ext(void *table, struct rte_mbuf **pkts,
                                          uint64_t pkts_mask,
                                          uint64_t *lookup_hit_mask,
                                          void **entries) {
  struct rte_table_hash *f = (struct rte_table_hash *)table;
...
static int rte_table_hash_key8_stats_read(void *table,
                                          struct rte_table_stats *stats,
                                          int clear) {
