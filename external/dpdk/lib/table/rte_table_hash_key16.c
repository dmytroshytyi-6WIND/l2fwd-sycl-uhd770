...
static void *rte_table_hash_create_key16_lru(void *params, int socket_id,
                                             uint32_t entry_size) {
  ...
  for (i = 0; i < n_buckets; i++) {
    ...
    bucket = (struct rte_bucket_4_16 *)&f->memory[i * f->bucket_size];
    lru_init(bucket);
  }
...
static int rte_table_hash_free_key16_lru(void *table) {
...
static int rte_table_hash_entry_add_key16_lru(void *table, void *key,
  ...
  bucket = (struct rte_bucket_4_16 *)&f->memory[bucket_index * f->bucket_size];
...
static int rte_table_hash_entry_delete_key16_lru(void *table, void *key,
...
static void *rte_table_hash_create_key16_ext(void *params, int socket_id,
                                             uint32_t entry_size) {
  struct rte_table_hash_params *p = params;
  struct rte_table_hash *f;
...
static int rte_table_hash_free_key16_ext(void *table) {
  struct rte_table_hash *f = table;
...
static int rte_table_hash_entry_add_key16_ext(void *table, void *key,
                                              void *entry, int *key_found,
                                              void **entry_ptr) {
  struct rte_table_hash *f = table;
  ...
  signature = f->f_hash(key, f->key_mask, f->key_size, f->seed);
  bucket_index = signature & (f->n_buckets - 1);
  bucket0 = (struct rte_bucket_4_16 *)&f->memory[bucket_index * f->bucket_size];
  signature |= RTE_BUCKET_ENTRY_VALID;
  ...
  if (f->stack_pos > 0) {
    ...
    bucket = (struct rte_bucket_4_16 *)&f
...
static int rte_table_hash_entry_delete_key16_ext(void *table, void *key,
                                                 int *key_found, void *entry) {
  struct rte_table_hash *f = table;
  struct rte_bucket_4_16 *bucket0, *bucket, *bucket_prev;
  uint64_t signature;
      ...
      if ((bucket_signature == signature) &&
        ...
        *key_found = 1;
        ...
        if ((bucket->signature[0] == 0) && (bucket->signature[1] == 0) &&
          ...
          memset(bucket, 0, sizeof(struct rte_bucket_4_16));
          bucket_index =
...
#define lookup1_stage1(mbuf1, bucket1, f)                                      \
  {                                                                            \
    ...
    bucket_index = signature & (f->n_buckets - 1);                             \
    bucket1 =                                                                  \
        (struct rte_bucket_4_16 *)&f->memory[bucket_index * f->bucket_size];   \
...
#define lookup2_stage1(mbuf10, mbuf11, bucket10, bucket11, f)                  \
  {                                                                            \
    ...
    bucket10 =                                                                 \
        (struct rte_bucket_4_16 *)&f->memory[bucket10_index * f->bucket_size]; \
    ...
    bucket11 =                                                                 \
        (struct rte_bucket_4_16 *)&f->memory[bucket11_index * f->bucket_size]; \
...
static int rte_table_hash_lookup_key16_lru(void *table, struct rte_mbuf **pkts,
                                           uint64_t pkts_mask,
  ...
  if (__builtin_popcountll(pkts_mask) < 5) {
    ...
    *lookup_hit_mask = pkts_mask_out;
    RTE_TABLE_HASH_KEY16_STATS_PKTS_LOOKUP_MISS(
...
static int rte_table_hash_lookup_key16_ext(void *table, struct rte_mbuf **pkts,
                                           uint64_t pkts_mask,
                                           uint64_t *lookup_hit_mask,
                                           void **entries) {
  struct rte_table_hash *f = (struct rte_table_hash *)table;
...
static int rte_table_hash_key16_stats_read(void *table,
                                           struct rte_table_stats *stats,
                                           int clear) {
