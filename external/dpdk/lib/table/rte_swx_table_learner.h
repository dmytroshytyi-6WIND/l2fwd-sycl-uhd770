 ...
 *
 * This table type is typically used for learning or connection tracking, where
 * it allows for the implementation of the "add on miss" scenario: whenever the
 * lookup key is not found in the table (lookup miss), the data plane can decide
 * to add this key to the table with a given action with no control plane
 * intervention. Likewise, the table keys expire based on a configurable timeout
 * and are automatically deleted from the table with no control plane
 * intervention.
 */
...
struct rte_swx_table_learner_params {
   ...
   * *key_offset* .. (*key_offset* + *key_size* - 1) of the key buffer in order
   * to specify which bits from the key buffer are part of the key and which
   * ones are not. A bit value of 1 in the *key_mask0* means the respective bit
   * in the key buffer is part of the key, while a bit value of 0 means the
   * opposite. A NULL value means that all the bits are part of the key, i.e.
   * the *key_mask0* is an all-ones mask.
   */
  uint8_t *key_mask0;

  /** Maximum size (in bytes) of the action data. The data stored in the table
   * for each entry is equal to *action_data_size* plus 8 bytes, which are used
   * to store the action ID.
   */
  ...
  /** Key timeout in seconds. Must be non-zero. Each table key expires and is
   * automatically deleted from the table after this many seconds.
   */
 ...
 */
__rte_experimental uint64_t rte_swx_table_learner_footprint_get(
...
rte_swx_table_learner_create(struct rte_swx_table_learner_params *params,
                             int numa_node);
 ...
 *
 * The mailbox argument is used to store the context of an on-going table key
 * lookup operation, and possibly an associated key add operation. The mailbox
 * mechanism allows for multiple concurrent table key lookup and add operations
 * into the same table.
 *
 * @param[in] table
 *   Table handle.
...
rte_swx_table_learner_lookup(void *table, void *mailbox, uint64_t time,
                             uint8_t **key, uint64_t *action_id,
                             uint8_t **action_data, int *hit);

/**
 * Learner table key add
 *
 * This operation takes the latest key that was looked up in the table and adds
 * it to the table with the given action ID and action data. Typically, this
 * operation is only invoked when the latest lookup operation in the current
 * table resulted in lookup miss.
 *
 * @param[in] table
 *   Table handle.
 * @param[in] mailbox
 ...
 *
 * This operation takes the latest key that was looked up in the table and
 * deletes it from the table. Typically, this operation is only invoked to force
 * the deletion of the key before the key expires on timeout due to inactivity.
 *
 * @param[in] table
 *   Table handle.
 * @param[in] mailbox
