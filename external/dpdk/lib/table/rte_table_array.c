...
static void *rte_table_array_create(void *params, int socket_id,
                                    uint32_t entry_size) {
  ...
  if ((p == NULL) || (p->n_entries == 0) || (!rte_is_power_of_2(p->n_entries)))
    return NULL;

  /* Memory allocation */
  total_cl_size = (sizeof(struct rte_table_array) + RTE_CACHE_LINE_SIZE) /
                  RTE_CACHE_LINE_SIZE;
  total_cl_size +=
      (p->n_entries * entry_size + RTE_CACHE_LINE_SIZE) / RTE_CACHE_LINE_SIZE;
  total_size = total_cl_size * RTE_CACHE_LINE_SIZE;
  t = rte_zmalloc_socket("TABLE", total_size, RTE_CACHE_LINE_SIZE, socket_id);
  if (t == NULL) {
    RTE_LOG(ERR, TABLE, "%s: Cannot allocate %u bytes for array table\n",
            __func__, total_size);
    return NULL;
  }
...
static int rte_table_array_free(void *table) {
...
static int rte_table_array_entry_add(void *table, void *key, void *entry,
  ...
  *lookup_hit_mask = pkts_mask;
  ...
  if ((pkts_mask & (pkts_mask + 1)) == 0) {
    ...
    for (i = 0; i < n_pkts; i++) {
      ...
      uint32_t entry_pos =
          RTE_MBUF_METADATA_UINT32(pkt, t->offset) & t->entry_pos_mask;

      entries[i] = (void *)&t->array[entry_pos * t->entry_size];
    }
  } else {
    for (; pkts_mask;) {
      ...
      entries[pkt_index] = (void *)&t->array[entry_pos * t->entry_size];
      pkts_mask &= ~pkt_mask;
    }
  }
...
static int rte_table_array_stats_read(void *table,
                                      struct rte_table_stats *stats,
                                      int clear) {
