...
static void *rte_table_lpm_ipv6_create(void *params, int socket_id,
                                       uint32_t entry_size) {
  struct rte_table_lpm_ipv6_params *p = params;
  ...
  lpm = rte_zmalloc_socket("TABLE", total_size, RTE_CACHE_LINE_SIZE, socket_id);
  if (lpm == NULL) {
    RTE_LOG(ERR, TABLE, "%s: Cannot allocate %u bytes for LPM IPv6 table\n",
            __func__, total_size);
    return NULL;
  }
  ...
  if (lpm->lpm == NULL) {
    rte_free(lpm);
    RTE_LOG(ERR, TABLE, "Unable to create low-level LPM IPv6 table\n");
    return NULL;
  }
...
static int rte_table_lpm_ipv6_free(void *table) {
...
static int nht_find_free(struct rte_table_lpm_ipv6 *lpm, uint32_t *pos) {
  uint32_t i;
...
static int nht_find_existing(struct rte_table_lpm_ipv6 *lpm, void *entry,
                             uint32_t *pos) {
  uint32_t i;
...
static int rte_table_lpm_ipv6_entry_add(void *table, void *key, void *entry,
                                        int *key_found, void **entry_ptr) {
  struct rte_table_lpm_ipv6 *lpm = table;
  struct rte_table_lpm_ipv6_key *ip_prefix = key;
  ...
  status = rte_lpm6_is_rule_present(lpm->lpm, ip_prefix->ip, ip_prefix->depth,
                                    &nht_pos0);
  ...
  if (rte_lpm6_add(lpm->lpm, ip_prefix->ip, ip_prefix->depth, nht_pos) < 0) {
...
static int rte_table_lpm_ipv6_entry_delete(void *table, void *key,
...
static int rte_table_lpm_ipv6_lookup(void *table, struct rte_mbuf **pkts,
                                     uint64_t pkts_mask,
  ...
  for (i = 0;
    ...
    if (pkt_mask & pkts_mask) {
      ...
      uint8_t *ip = RTE_MBUF_METADATA_UINT8_PTR(pkt, lpm->offset);
      int status;
      uint32_t nht_pos;
  ...
  *lookup_hit_mask = pkts_out_mask;
  RTE_TABLE_LPM_IPV6_STATS_PKTS_LOOKUP_MISS(
      lpm, n_pkts_in - __builtin_popcountll(pkts_out_mask));
  return 0;
}

static int rte_table_lpm_ipv6_stats_read(void *table,
                                         struct rte_table_stats *stats,
                                         int clear) {
