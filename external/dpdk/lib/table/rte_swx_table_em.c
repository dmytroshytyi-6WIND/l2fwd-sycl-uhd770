...
static void env_free(void *start, size_t size) { numa_free(start, size); }
...
static void keycpy(void *dst, void *src, void *src_mask, uint32_t n_bytes) {
 ...
 */
static inline uint32_t keycmp(void *a, void *b, void *b_mask,
                              uint32_t n_bytes) {
...
static inline uint8_t *table_key(struct table *t, uint32_t key_id) {
  return &t->keys[(uint64_t)key_id << t->key_size_shl];
}

static inline uint64_t *table_key_data(struct table *t, uint32_t key_id) {
  return (uint64_t *)&t->data[(uint64_t)key_id << t->data_size_shl];
}

static inline int bkt_is_empty(struct bucket_extension *bkt) {
 ...
 */
static inline int bkt_key_is_empty(struct bucket_extension *bkt,
                                   uint32_t bkt_pos) {
  return bkt->sig[bkt_pos] ? 0 : 1;
}

/* Return: 0 = Keys are NOT equal; 1 = Keys are equal. */
static inline int bkt_keycmp(struct table *t, struct bucket_extension *bkt,
                             uint8_t *input_key, uint32_t bkt_pos,
                             uint32_t input_sig) {
  uint32_t bkt_key_id;
  uint8_t *bkt_key;
...
static inline void bkt_key_install(struct table *t,
                                   struct bucket_extension *bkt,
                                   struct rte_swx_table_entry *input,
                                   uint32_t bkt_pos, uint32_t bkt_key_id,
                                   uint32_t input_sig) {
  uint8_t *bkt_key;
  uint64_t *bkt_data;
...
static inline void bkt_key_data_update(struct table *t,
                                       struct bucket_extension *bkt,
                                       struct rte_swx_table_entry *input,
                                       uint32_t bkt_pos) {
  uint32_t bkt_key_id;
...
static int __table_create(struct table **table, uint64_t *memory_footprint,
                          struct rte_swx_table_params *params,
                          const char *args __rte_unused, int numa_node) {
  struct table *t;
  uint8_t *memory;
...
static void table_free(void *table) {
...
static int table_add(void *table, struct rte_swx_table_entry *entry) {
  struct table *t = table;
...
static int table_del(void *table, struct rte_swx_table_entry *entry) {
  struct table *t = table;
  struct bucket_extension *bkt0, *bkt, *bkt_prev;
  uint32_t input_sig, bkt_id, i;
...
static uint64_t table_mailbox_size_get_unoptimized(void) { return 0; }

static int table_lookup_unoptimized(void *table, void *mailbox __rte_unused,
...
static uint64_t table_mailbox_size_get(void) { return sizeof(struct mailbox); }
...
static int table_lookup(void *table, void *mailbox, uint8_t **key,
                        uint64_t *action_id, uint8_t **action_data, int *hit) {
  struct table *t = table;
  ...
  case 2: {
    ...
    *hit = lkp_hit;
    ...
    if (!lkp_hit && (m->sig_match_many || bkt->next))
      return table_lookup_unoptimized(t, m, key, action_id, action_data, hit);
...
static void *table_create(struct rte_swx_table_params *params,
                          struct rte_swx_table_entry_list *entries,
                          const char *args, int numa_node) {
  struct table *t;
