...
enum instruction_type {
  ...
  INSTR_ALU_CKADD_STRUCT20, /* src = h.header, with sizeof(header) = 20 bytes.
                             */
  INSTR_ALU_CKADD_STRUCT,   /* src = h.header, with sizeof(header) any 4-byte
                               multiple. */
 ...
 * Action.
 */
typedef void (*action_func_t)(struct rte_swx_pipeline *p);

struct action {
  TAILQ_ENTRY(action) node;
  char name[RTE_SWX_NAME_SIZE];
  struct struct_type *st;
  int *args_endianness; /* 0 = Host Byte Order (HBO); 1 = Network Byte Order
                           (NBO). */
...
static inline uint64_t instr_operand_hbo(struct thread *t,
...
static inline uint64_t instr_operand_nbo(struct thread *t,
                                         const struct instr_operand *x) {
  uint8_t *x_struct = t->structs[x->struct_id];
  uint64_t *x64_ptr = (uint64_t *)&x_struct[x->offset];
  uint64_t x64 = *x64_ptr;
...
#define ALU_HH_FAST(thread, ip, operator)                                      \
  {                                                                            \
    ...
    uint64_t src64 = *src64_ptr;                                               \
    uint64_t src =                                                             \
        (src64 << (64 - (ip)->alu.src.n_bits)) >> (64 - (ip)->alu.dst.n_bits); \
                                                                               \
    uint64_t result = dst operator src;                                        \
                                                                               \
    *dst64_ptr = (dst64 & ~dst64_mask) | result;                               \
 ...
 * Instruction.
 */
static inline void pipeline_port_inc(struct rte_swx_pipeline *p) {
  p->port_id = (p->port_id + 1) & (p->n_ports_in - 1);
}

static inline void thread_ip_reset(struct rte_swx_pipeline *p,
                                   struct thread *t) {
  t->ip = p->instructions;
}

static inline void thread_ip_set(struct thread *t, struct instruction *ip) {
  t->ip = ip;
}

static inline void thread_ip_action_call(struct rte_swx_pipeline *p,
                                         struct thread *t, uint32_t action_id) {
  t->ret = t->ip + 1;
  t->ip = p->action_instructions[action_id];
}

static inline void thread_ip_inc(struct rte_swx_pipeline *p);
...
static inline void thread_ip_inc_cond(struct thread *t, int cond) {
  t->ip += cond;
}

static inline void thread_yield(struct rte_swx_pipeline *p) {
  p->thread_id = (p->thread_id + 1) & (RTE_SWX_PIPELINE_THREADS_MAX - 1);
}

static inline void thread_yield_cond(struct rte_swx_pipeline *p, int cond) {
 ...
 * rx.
 */
static inline int __instr_rx_exec(struct rte_swx_pipeline *p, struct thread *t,
  ...
  TRACE("[Thread %2u] rx %s from port %u\n", p->thread_id,
        pkt_received ? "1 pkt" : "0 pkts", p->port_id);
...
static inline void instr_rx_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
 ...
 * tx.
 */
static inline void emit_handler(struct thread *t) {
  ...
  if ((t->n_headers_out == 1) && (h0->ptr + h0->n_bytes == t->ptr)) {
  ...
  if ((t->n_headers_out == 2) && (h1->ptr + h1->n_bytes == t->ptr) &&
      (h0->ptr == h0->ptr0)) {
    uint32_t offset;
...
static inline void __instr_tx_exec(struct rte_swx_pipeline *p, struct thread *t,
                                   const struct instruction *ip) {
...
static inline void __instr_tx_i_exec(struct rte_swx_pipeline *p,
                                     struct thread *t,
                                     const struct instruction *ip) {
...
static inline void
__instr_drop_exec(struct rte_swx_pipeline *p, struct thread *t,
...
__instr_hdr_extract_many_exec(struct rte_swx_pipeline *p __rte_unused,
                              struct thread *t, const struct instruction *ip,
                              uint32_t n_extract) {
...
static inline void __instr_hdr_extract_exec(struct rte_swx_pipeline *p,
                                            struct thread *t,
                                            const struct instruction *ip) {
  __instr_hdr_extract_many_exec(p, t, ip, 1);
}

static inline void __instr_hdr_extract2_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
...
static inline void __instr_hdr_extract3_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
...
static inline void __instr_hdr_extract4_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
...
static inline void __instr_hdr_extract5_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
...
static inline void __instr_hdr_extract6_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
...
static inline void __instr_hdr_extract7_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
...
static inline void __instr_hdr_extract8_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
...
__instr_hdr_extract_m_exec(struct rte_swx_pipeline *p __rte_unused,
  ...
  TRACE("[Thread %2u]: extract header %u (%u + %u bytes)\n", p->thread_id,
        header_id, n_bytes, n_bytes_last);
...
static inline void __instr_hdr_emit_exec(struct rte_swx_pipeline *p,
                                         struct thread *t,
                                         const struct instruction *ip) {
  __instr_hdr_emit_many_exec(p, t, ip, 1);
}

static inline void __instr_hdr_emit_tx_exec(struct rte_swx_pipeline *p,
                                            struct thread *t,
                                            const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 2 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_hdr_emit2_tx_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 3 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_hdr_emit3_tx_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 4 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_hdr_emit4_tx_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 5 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_hdr_emit5_tx_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 6 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_hdr_emit6_tx_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 7 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_hdr_emit7_tx_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 8 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_hdr_emit8_tx_exec(struct rte_swx_pipeline *p,
                                             struct thread *t,
                                             const struct instruction *ip) {
 ...
 * learn.
 */
static inline void __instr_learn_exec(struct rte_swx_pipeline *p,
                                      struct thread *t,
                                      const struct instruction *ip) {
  ...
  /* Table. */
  status = rte_swx_table_learner_add(ts->obj, l->mailbox, t->time, action_id,
 ...
 */
static inline void
__instr_forget_exec(struct rte_swx_pipeline *p, struct thread *t,
                    const struct instruction *ip __rte_unused) {
  uint32_t learner_id = t->learner_id;
  struct rte_swx_table_state *ts =
      &t->table_state[p->n_tables + p->n_selectors + learner_id];
 ...
 * mov.
 */
static inline void __instr_mov_exec(struct rte_swx_pipeline *p __rte_unused,
                                    struct thread *t,
                                    const struct instruction *ip) {
  TRACE("[Thread %2u] mov\n", p->thread_id);
...
static inline void __instr_mov_mh_exec(struct rte_swx_pipeline *p __rte_unused,
                                       struct thread *t,
                                       const struct instruction *ip) {
...
static inline void __instr_mov_hm_exec(struct rte_swx_pipeline *p __rte_unused,
                                       struct thread *t,
                                       const struct instruction *ip) {
...
static inline void __instr_mov_hh_exec(struct rte_swx_pipeline *p __rte_unused,
                                       struct thread *t,
                                       const struct instruction *ip) {
...
static inline void __instr_mov_i_exec(struct rte_swx_pipeline *p __rte_unused,
                                      struct thread *t,
                                      const struct instruction *ip) {
...
__instr_dma_ht_many_exec(struct rte_swx_pipeline *p __rte_unused,
  ...
  for (i = 0; i < n_dma; i++) {
    ...
    void *dst = MASK64_BIT_GET(valid_headers, header_id) ? h_ptr : h_ptr0;
...
static inline void __instr_dma_ht_exec(struct rte_swx_pipeline *p,
                                       struct thread *t,
                                       const struct instruction *ip) {
  __instr_dma_ht_many_exec(p, t, ip, 1);
}

static inline void __instr_dma_ht2_exec(struct rte_swx_pipeline *p,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 2 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_dma_ht3_exec(struct rte_swx_pipeline *p,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 3 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_dma_ht4_exec(struct rte_swx_pipeline *p,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 4 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_dma_ht5_exec(struct rte_swx_pipeline *p,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 5 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_dma_ht6_exec(struct rte_swx_pipeline *p,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 6 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_dma_ht7_exec(struct rte_swx_pipeline *p,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 7 instructions are fused. ***\n",
        p->thread_id);
...
static inline void __instr_dma_ht8_exec(struct rte_swx_pipeline *p,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] *** The next 8 instructions are fused. ***\n",
        p->thread_id);
 ...
 * alu.
 */
static inline void __instr_alu_add_exec(struct rte_swx_pipeline *p __rte_unused,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] add\n", p->thread_id);
...
static inline void __instr_alu_sub_exec(struct rte_swx_pipeline *p __rte_unused,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] sub\n", p->thread_id);
...
static inline void __instr_alu_shl_exec(struct rte_swx_pipeline *p __rte_unused,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] shl\n", p->thread_id);
...
static inline void __instr_alu_shr_exec(struct rte_swx_pipeline *p __rte_unused,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] shr\n", p->thread_id);
...
static inline void __instr_alu_and_exec(struct rte_swx_pipeline *p __rte_unused,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] and\n", p->thread_id);
...
static inline void __instr_alu_or_exec(struct rte_swx_pipeline *p __rte_unused,
                                       struct thread *t,
                                       const struct instruction *ip) {
  TRACE("[Thread %2u] or\n", p->thread_id);
...
static inline void
__instr_alu_or_i_exec(struct rte_swx_pipeline *p __rte_unused, struct thread *t,
                      const struct instruction *ip) {
...
static inline void __instr_alu_xor_exec(struct rte_swx_pipeline *p __rte_unused,
                                        struct thread *t,
                                        const struct instruction *ip) {
  TRACE("[Thread %2u] xor\n", p->thread_id);
...
__instr_alu_ckadd_struct_exec(struct rte_swx_pipeline *p __rte_unused,
                              struct thread *t, const struct instruction *ip) {
  uint32_t src_header_id =
      ip->alu.src.n_bits; /* The src header ID is stored here. */
 ...
 * Register array.
 */
static inline uint64_t *instr_regarray_regarray(struct rte_swx_pipeline *p,
                                                const struct instruction *ip) {
  struct regarray_runtime *r = &p->regarray_runtime[ip->regarray.regarray_id];
  return r->regarray;
}

static inline uint64_t instr_regarray_idx_hbo(struct rte_swx_pipeline *p,
                                              struct thread *t,
                                              const struct instruction *ip) {
  struct regarray_runtime *r = &p->regarray_runtime[ip->regarray.regarray_id];
...
static inline uint64_t instr_regarray_idx_nbo(struct rte_swx_pipeline *p,
                                              struct thread *t,
                                              const struct instruction *ip) {
  struct regarray_runtime *r = &p->regarray_runtime[ip->regarray.regarray_id];
  ...
  uint64_t idx64 = *idx64_ptr;
  uint64_t idx =
      (ntoh64(idx64) >> (64 - ip->regarray.idx.n_bits)) & r->size_mask;
...
static inline uint64_t instr_regarray_idx_imm(struct rte_swx_pipeline *p,
                                              const struct instruction *ip) {
  struct regarray_runtime *r = &p->regarray_runtime[ip->regarray.regarray_id];
...
static inline uint64_t instr_regarray_src_hbo(struct thread *t,
                                              const struct instruction *ip) {
...
static inline uint64_t instr_regarray_src_nbo(struct thread *t,
                                              const struct instruction *ip) {
  uint8_t *src_struct = t->structs[ip->regarray.dstsrc.struct_id];
  uint64_t *src64_ptr = (uint64_t *)&src_struct[ip->regarray.dstsrc.offset];
  uint64_t src64 = *src64_ptr;
...
static inline void
instr_regarray_dst_hbo_src_hbo_set(struct thread *t,
...
static inline void
instr_regarray_dst_nbo_src_hbo_set(struct thread *t,
                                   const struct instruction *ip, uint64_t src) {
  uint8_t *dst_struct = t->structs[ip->regarray.dstsrc.struct_id];
  uint64_t *dst64_ptr = (uint64_t *)&dst_struct[ip->regarray.dstsrc.offset];
  uint64_t dst64 = *dst64_ptr;
...
static inline void __instr_regprefetch_rh_exec(struct rte_swx_pipeline *p,
                                               struct thread *t,
                                               const struct instruction *ip) {
...
static inline void __instr_regprefetch_rm_exec(struct rte_swx_pipeline *p,
                                               struct thread *t,
                                               const struct instruction *ip) {
  uint64_t *regarray, idx;
...
static inline void __instr_regprefetch_ri_exec(struct rte_swx_pipeline *p,
...
static inline void __instr_regrd_hrh_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx;
...
static inline void __instr_regrd_hrm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx;
...
static inline void __instr_regrd_mrh_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx;
...
static inline void __instr_regrd_mrm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx;
...
static inline void __instr_regrd_hri_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx;
...
static inline void __instr_regrd_mri_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx;
...
static inline void __instr_regwr_rhh_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
...
static inline void __instr_regwr_rhm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regwr_rmh_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regwr_rmm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regwr_rhi_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regwr_rmi_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regwr_rih_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regwr_rim_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regwr_rii_exec(struct rte_swx_pipeline *p,
                                          struct thread *t __rte_unused,
                                          const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regadd_rhh_exec(struct rte_swx_pipeline *p,
                                           struct thread *t,
                                           const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regadd_rhm_exec(struct rte_swx_pipeline *p,
                                           struct thread *t,
                                           const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regadd_rmh_exec(struct rte_swx_pipeline *p,
                                           struct thread *t,
                                           const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regadd_rmm_exec(struct rte_swx_pipeline *p,
                                           struct thread *t,
                                           const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regadd_rhi_exec(struct rte_swx_pipeline *p,
                                           struct thread *t,
                                           const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regadd_rmi_exec(struct rte_swx_pipeline *p,
                                           struct thread *t,
                                           const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regadd_rih_exec(struct rte_swx_pipeline *p,
                                           struct thread *t,
                                           const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regadd_rim_exec(struct rte_swx_pipeline *p,
                                           struct thread *t,
                                           const struct instruction *ip) {
  uint64_t *regarray, idx, src;
...
static inline void __instr_regadd_rii_exec(struct rte_swx_pipeline *p,
                                           struct thread *t __rte_unused,
                                           const struct instruction *ip) {
  uint64_t *regarray, idx, src;
 ...
 * metarray.
 */
static inline struct meter *instr_meter_idx_hbo(struct rte_swx_pipeline *p,
                                                struct thread *t,
                                                const struct instruction *ip) {
...
static inline struct meter *instr_meter_idx_nbo(struct rte_swx_pipeline *p,
                                                struct thread *t,
                                                const struct instruction *ip) {
  struct metarray_runtime *r = &p->metarray_runtime[ip->meter.metarray_id];
...
static inline struct meter *instr_meter_idx_imm(struct rte_swx_pipeline *p,
                                                const struct instruction *ip) {
  struct metarray_runtime *r = &p->metarray_runtime[ip->meter.metarray_id];
...
static inline uint32_t instr_meter_length_hbo(struct thread *t,
                                              const struct instruction *ip) {
...
static inline uint32_t instr_meter_length_nbo(struct thread *t,
                                              const struct instruction *ip) {
  uint8_t *src_struct = t->structs[ip->meter.length.struct_id];
  uint64_t *src64_ptr = (uint64_t *)&src_struct[ip->meter.length.offset];
  uint64_t src64 = *src64_ptr;
...
static inline void instr_meter_color_out_hbo_set(struct thread *t,
                                                 const struct instruction *ip,
...
static inline void __instr_metprefetch_h_exec(struct rte_swx_pipeline *p,
                                              struct thread *t,
                                              const struct instruction *ip) {
  struct meter *m;
...
static inline void __instr_metprefetch_m_exec(struct rte_swx_pipeline *p,
                                              struct thread *t,
                                              const struct instruction *ip) {
  struct meter *m;
...
static inline void __instr_metprefetch_i_exec(struct rte_swx_pipeline *p,
                                              struct thread *t __rte_unused,
                                              const struct instruction *ip) {
  struct meter *m;
...
static inline void __instr_meter_hhm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  ...
  color_out = rte_meter_trtcm_color_aware_check(&m->m, &m->profile->profile,
                                                time, length, color_in);
...
static inline void __instr_meter_hhi_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_hmm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_hmi_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_mhm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_mhi_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_mmm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_mmi_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_ihm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_ihi_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_imm_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
...
static inline void __instr_meter_imi_exec(struct rte_swx_pipeline *p,
                                          struct thread *t,
                                          const struct instruction *ip) {
  struct meter *m;
  uint64_t time, n_pkts, n_bytes;
