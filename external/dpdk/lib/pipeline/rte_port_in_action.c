 ...
 */
static int fltr_cfg_check(struct rte_port_in_action_fltr_config *cfg) {
  if (cfg == NULL)
    return -1;
...
static void fltr_init(struct fltr_data *data,
                      struct rte_port_in_action_fltr_config *cfg) {
  data->port_id = cfg->port_id;
}

static int fltr_apply(struct fltr_data *data,
 ...
 */
static int lb_cfg_check(struct rte_port_in_action_lb_config *cfg) {
  if ((cfg == NULL) || (cfg->key_size < RTE_PORT_IN_ACTION_LB_KEY_SIZE_MIN) ||
...
static void lb_init(struct lb_data *data,
 ...
 * Action profile
 */
static int action_valid(enum rte_port_in_action_type action) {
  switch (action) {
...
static size_t action_cfg_size(enum rte_port_in_action_type action) {
  switch (action) {
  case RTE_PORT_IN_ACTION_FLTR:
...
static size_t action_data_size(enum rte_port_in_action_type action,
...
static void action_data_offset_set(struct ap_data *ap_data,
  ...
  for (action = 0; action < RTE_PORT_IN_ACTION_MAX; action++)
    if (action_mask & (1LLU << action)) {
      ...
      offset +=
          action_data_size((enum rte_port_in_action_type)action, ap_config);
    }
...
rte_port_in_action_profile_create(uint32_t socket_id) {
  ...
  /* Memory allocation */
  ap = rte_zmalloc_socket(NULL, sizeof(struct rte_port_in_action_profile),
...
int rte_port_in_action_profile_action_register(
    struct rte_port_in_action_profile *profile,
    enum rte_port_in_action_type type, void *action_config) {
  int status;
...
int rte_port_in_action_profile_free(
    struct rte_port_in_action_profile *profile) {
  if (profile == NULL)
    return 0;
...
static void action_data_init(struct rte_port_in_action *action,
                             enum rte_port_in_action_type type) {
...
rte_port_in_action_create(struct rte_port_in_action_profile *profile,
  ...
  action = rte_zmalloc_socket(NULL, size, RTE_CACHE_LINE_SIZE, socket_id);
  if (action == NULL)
    return NULL;
  ...
  for (i = 0; i < RTE_PORT_IN_ACTION_MAX; i++)
    if (action->cfg.action_mask & (1LLU << i))
      action_data_init(action, (enum rte_port_in_action_type)i);

  return action;
}

int rte_port_in_action_apply(struct rte_port_in_action *action,
                             enum rte_port_in_action_type type,
                             void *action_params) {
  void *action_data;

  /* Check input arguments */
  if ((action == NULL) || (action_valid(type) == 0) ||
  ...
  switch (type) {
  case RTE_PORT_IN_ACTION_FLTR:
    return fltr_apply(action_data, action_params);
...
static int ah_filter_on_match(struct rte_pipeline *p, struct rte_mbuf **pkts,
  ...
  struct fltr_data *data = action_data_get(action, RTE_PORT_IN_ACTION_FLTR);
  uint32_t i;

  for (i = 0; i < n_pkts; i++) {
    struct rte_mbuf *pkt = pkts[i];
    uint64_t *pkt_key = RTE_MBUF_METADATA_UINT64_PTR(pkt, key_offset);
    ...
    if (or == 0) {
      ...
      rte_pipeline_port_out_packet_insert(p, data->port_id, pkt);
    }
  }
...
static int ah_filter_on_mismatch(struct rte_pipeline *p, struct rte_mbuf **pkts,
                                 uint32_t n_pkts, void *arg) {
  struct rte_port_in_action *action = arg;
  struct rte_port_in_action_fltr_config *cfg = &action->cfg.fltr;
  uint64_t *key_mask = (uint64_t *)cfg->key_mask;
...
static int ah_lb(struct rte_pipeline *p, struct rte_mbuf **pkts,
                 uint32_t n_pkts, void *arg) {
  struct rte_port_in_action *action = arg;
  ...
  for (i = 0; i < n_pkts; i++) {
    struct rte_mbuf *pkt = pkts[i];
    uint8_t *pkt_key = RTE_MBUF_METADATA_UINT8_PTR(pkt, cfg->key_offset);

    uint64_t digest =
        cfg->f_hash(pkt_key, cfg->key_mask, cfg->key_size, cfg->seed);
...
ah_selector(struct rte_port_in_action *action) {
  ...
  if (action->cfg.action_mask == 1LLU << RTE_PORT_IN_ACTION_FLTR)
    return (action->cfg.fltr.filter_on_match) ? ah_filter_on_match
...
int rte_port_in_action_params_get(struct rte_port_in_action *action,
...
int rte_port_in_action_free(struct rte_port_in_action *action) {
  if (action == NULL)
    return 0;
