...
static void extobj_spec_free(struct extobj_spec *s) {
  if (!s)
    return;
...
static int extobj_statement_parse(struct extobj_spec *s, char **tokens,
                                  uint32_t n_tokens, uint32_t n_lines,
                                  uint32_t *err_line, const char **err_msg) {
  /* Check format. */
  if (((n_tokens != 4) && (n_tokens != 6)) ||
      ((n_tokens == 4) && strcmp(tokens[2], "instanceof")) ||
      ((n_tokens == 6) &&
       (strcmp(tokens[2], "instanceof") || strcmp(tokens[4], "pragma")))) {
    if (err_line)
      *err_line = n_lines;
    if (err_msg)
  ...
  if (!s->name || !s->extern_type_name || ((n_tokens == 6) && !s->pragma)) {
    free(s->name);
    free(s->extern_type_name);
    free(s->pragma);
...
static void struct_spec_free(struct struct_spec *s) {
  uint32_t i;
...
static int struct_statement_parse(struct struct_spec *s, uint32_t *block_mask,
                                  char **tokens, uint32_t n_tokens,
                                  uint32_t n_lines, uint32_t *err_line,
                                  const char **err_msg) {
  /* Check format. */
...
static int struct_block_parse(struct struct_spec *s, uint32_t *block_mask,
                              char **tokens, uint32_t n_tokens,
                              uint32_t n_lines, uint32_t *err_line,
                              const char **err_msg) {
  ...
  if ((p[0]) || !n_bits || (n_bits % 8) || ((n_bits > 64) && !varbit)) {
    error = -EINVAL;
...
static void header_spec_free(struct header_spec *s) {
  if (!s)
    return;
...
static int header_statement_parse(struct header_spec *s, char **tokens,
                                  uint32_t n_tokens, uint32_t n_lines,
                                  uint32_t *err_line, const char **err_msg) {
  /* Check format. */
...
static void metadata_spec_free(struct metadata_spec *s) {
  if (!s)
    return;
...
static int metadata_statement_parse(struct metadata_spec *s, char **tokens,
                                    uint32_t n_tokens, uint32_t n_lines,
                                    uint32_t *err_line, const char **err_msg) {
  /* Check format. */
...
static void action_spec_free(struct action_spec *s) {
  uint32_t i;
...
static int action_statement_parse(struct action_spec *s, uint32_t *block_mask,
                                  char **tokens, uint32_t n_tokens,
                                  uint32_t n_lines, uint32_t *err_line,
                                  const char **err_msg) {
  /* Check format. */
  if (((n_tokens != 5) && (n_tokens != 6)) ||
      ((n_tokens == 5) &&
       (strcmp(tokens[2], "args") || strcmp(tokens[3], "none") ||
        strcmp(tokens[4], "{"))) ||
      ((n_tokens == 6) &&
       (strcmp(tokens[2], "args") || strcmp(tokens[3], "instanceof") ||
        strcmp(tokens[5], "{")))) {
    if (err_line)
      *err_line = n_lines;
    if (err_msg)
...
static int action_block_parse(struct action_spec *s, uint32_t *block_mask,
                              char **tokens, uint32_t n_tokens,
                              uint32_t n_lines, uint32_t *err_line,
                              const char **err_msg) {
  ...
  new_instructions =
      realloc(s->instructions, (s->n_instructions + 1) * sizeof(char *));
  if (!new_instructions) {
    free(instr);
...
static void table_spec_free(struct table_spec *s) {
...
static int table_key_statement_parse(uint32_t *block_mask, char **tokens,
                                     uint32_t n_tokens, uint32_t n_lines,
                                     uint32_t *err_line, const char **err_msg) {
  /* Check format. */
...
static int table_key_block_parse(struct table_spec *s, uint32_t *block_mask,
                                 char **tokens, uint32_t n_tokens,
                                 uint32_t n_lines, uint32_t *err_line,
                                 const char **err_msg) {
  ...
  if ((n_tokens != 2) ||
      (strcmp(tokens[1], "exact") && strcmp(tokens[1], "wildcard") &&
       strcmp(tokens[1], "lpm"))) {
    if (err_line)
      *err_line = n_lines;
    if (err_msg)
  ...
  new_fields =
      realloc(s->params.fields, (s->params.n_fields + 1) *
...
static int table_actions_statement_parse(uint32_t *block_mask, char **tokens,
                                         uint32_t n_tokens, uint32_t n_lines,
                                         uint32_t *err_line,
                                         const char **err_msg) {
  /* Check format. */
...
static int table_actions_block_parse(struct table_spec *s, uint32_t *block_mask,
                                     char **tokens, uint32_t n_tokens,
                                     uint32_t n_lines, uint32_t *err_line,
                                     const char **err_msg) {
  const char **new_action_names = NULL;
  int *new_action_is_for_table_entries = NULL,
      *new_action_is_for_default_entry = NULL;
  char *name = NULL;
  ...
  /* Check input arguments. */
  if ((n_tokens > 2) || ((n_tokens == 2) && strcmp(tokens[1], "@tableonly") &&
  ...
  s->params.action_is_for_table_entries[s->params.n_actions] =
      action_is_for_table_entries;

  s->params.action_is_for_default_entry = new_action_is_for_default_entry;
  s->params.action_is_for_default_entry[s->params.n_actions] =
...
static int table_statement_parse(struct table_spec *s, uint32_t *block_mask,
                                 char **tokens, uint32_t n_tokens,
                                 uint32_t n_lines, uint32_t *err_line,
                                 const char **err_msg) {
  /* Check format. */
...
static int table_block_parse(struct table_spec *s, uint32_t *block_mask,
                             char **tokens, uint32_t n_tokens, uint32_t n_lines,
                             uint32_t *err_line, const char **err_msg) {
  if (*block_mask & (1 << TABLE_KEY_BLOCK))
    return table_key_block_parse(s, block_mask, tokens, n_tokens, n_lines,
                                 err_line, err_msg);

  if (*block_mask & (1 << TABLE_ACTIONS_BLOCK))
    return table_actions_block_parse(s, block_mask, tokens, n_tokens, n_lines,
                                     err_line, err_msg);
  ...
  if (!strcmp(tokens[0], "key"))
    return table_key_statement_parse(block_mask, tokens, n_tokens, n_lines,
                                     err_line, err_msg);

  if (!strcmp(tokens[0], "actions"))
    return table_actions_statement_parse(block_mask, tokens, n_tokens, n_lines,
                                         err_line, err_msg);

  if (!strcmp(tokens[0], "default_action")) {
    if (((n_tokens != 4) && (n_tokens != 5)) || strcmp(tokens[2], "args") ||
...
static void selector_spec_free(struct selector_spec *s) {
...
static int selector_statement_parse(struct selector_spec *s,
                                    uint32_t *block_mask, char **tokens,
                                    uint32_t n_tokens, uint32_t n_lines,
                                    uint32_t *err_line, const char **err_msg) {
  /* Check format. */
  if ((n_tokens != 3) || strcmp(tokens[2], "{")) {
...
static int selector_selector_statement_parse(uint32_t *block_mask,
                                             char **tokens, uint32_t n_tokens,
                                             uint32_t n_lines,
                                             uint32_t *err_line,
                                             const char **err_msg) {
...
static int selector_selector_block_parse(struct selector_spec *s,
                                         uint32_t *block_mask, char **tokens,
                                         uint32_t n_tokens, uint32_t n_lines,
                                         uint32_t *err_line,
                                         const char **err_msg) {
...
static int selector_block_parse(struct selector_spec *s, uint32_t *block_mask,
                                char **tokens, uint32_t n_tokens,
                                uint32_t n_lines, uint32_t *err_line,
                                const char **err_msg) {
  if (*block_mask & (1 << SELECTOR_SELECTOR_BLOCK))
    return selector_selector_block_parse(s, block_mask, tokens, n_tokens,
                                         n_lines, err_line, err_msg);
  ...
  if (!strcmp(tokens[0], "selector"))
    return selector_selector_statement_parse(block_mask, tokens, n_tokens,
                                             n_lines, err_line, err_msg);
...
static void learner_spec_free(struct learner_spec *s) {
  uintptr_t default_action_name;
  uint32_t i;
...
static int learner_key_statement_parse(uint32_t *block_mask, char **tokens,
                                       uint32_t n_tokens, uint32_t n_lines,
                                       uint32_t *err_line,
                                       const char **err_msg) {
  /* Check format. */
...
static int learner_key_block_parse(struct learner_spec *s, uint32_t *block_mask,
                                   char **tokens, uint32_t n_tokens,
                                   uint32_t n_lines, uint32_t *err_line,
                                   const char **err_msg) {
  ...
  new_field_names =
      realloc(s->params.field_names, (s->params.n_fields + 1) * sizeof(char *));
...
static int learner_actions_statement_parse(uint32_t *block_mask, char **tokens,
                                           uint32_t n_tokens, uint32_t n_lines,
                                           uint32_t *err_line,
                                           const char **err_msg) {
  /* Check format. */
...
static int learner_actions_block_parse(struct learner_spec *s,
                                       uint32_t *block_mask, char **tokens,
                                       uint32_t n_tokens, uint32_t n_lines,
                                       uint32_t *err_line,
                                       const char **err_msg) {
...
static int learner_statement_parse(struct learner_spec *s, uint32_t *block_mask,
                                   char **tokens, uint32_t n_tokens,
                                   uint32_t n_lines, uint32_t *err_line,
                                   const char **err_msg) {
  /* Check format. */
...
static int learner_block_parse(struct learner_spec *s, uint32_t *block_mask,
                               char **tokens, uint32_t n_tokens,
                               uint32_t n_lines, uint32_t *err_line,
                               const char **err_msg) {
  if (*block_mask & (1 << LEARNER_KEY_BLOCK))
    return learner_key_block_parse(s, block_mask, tokens, n_tokens, n_lines,
                                   err_line, err_msg);

  if (*block_mask & (1 << LEARNER_ACTIONS_BLOCK))
    return learner_actions_block_parse(s, block_mask, tokens, n_tokens, n_lines,
                                       err_line, err_msg);
  ...
  if (!strcmp(tokens[0], "key"))
    return learner_key_statement_parse(block_mask, tokens, n_tokens, n_lines,
                                       err_line, err_msg);

  if (!strcmp(tokens[0], "actions"))
    return learner_actions_statement_parse(block_mask, tokens, n_tokens,
                                           n_lines, err_line, err_msg);
...
static void regarray_spec_free(struct regarray_spec *s) {
  if (!s)
    return;
...
static int regarray_statement_parse(struct regarray_spec *s, char **tokens,
                                    uint32_t n_tokens, uint32_t n_lines,
                                    uint32_t *err_line, const char **err_msg) {
  char *p;

  /* Check format. */
  if ((n_tokens != 6) || strcmp(tokens[2], "size") ||
...
static void metarray_spec_free(struct metarray_spec *s) {
  if (!s)
    return;
...
static int metarray_statement_parse(struct metarray_spec *s, char **tokens,
                                    uint32_t n_tokens, uint32_t n_lines,
                                    uint32_t *err_line, const char **err_msg) {
  char *p;
...
static void apply_spec_free(struct apply_spec *s) {
  uint32_t i;
...
static int apply_statement_parse(uint32_t *block_mask, char **tokens,
                                 uint32_t n_tokens, uint32_t n_lines,
                                 uint32_t *err_line, const char **err_msg) {
  /* Check format. */
  if ((n_tokens != 2) || strcmp(tokens[1], "{")) {
...
static int apply_block_parse(struct apply_spec *s, uint32_t *block_mask,
                             char **tokens, uint32_t n_tokens, uint32_t n_lines,
                             uint32_t *err_line, const char **err_msg) {
  char buffer[RTE_SWX_INSTRUCTION_SIZE], *instr;
  const char **new_instructions;
 ...
 * Pipeline.
 */
int rte_swx_pipeline_build_from_spec(struct rte_swx_pipeline *p, FILE *spec,
                                     uint32_t *err_line, const char **err_msg) {
  ...
  for (n_lines = 1;; n_lines++) {
    ...
    for (;;) {
      ...
      if (strnlen(token, RTE_SWX_NAME_SIZE) >= RTE_SWX_NAME_SIZE) {
        if (err_line)
          *err_line = n_lines;
        if (err_msg)
    ...
    if (block_mask & (1 << STRUCT_BLOCK)) {
      status = struct_block_parse(&struct_spec, &block_mask, tokens, n_tokens,
                                  n_lines, err_line, err_msg);
      if (status)
        goto error;
      ...
      /* End of block. */
      status = rte_swx_pipeline_struct_type_register(
          p, struct_spec.name, struct_spec.fields, struct_spec.n_fields,
          struct_spec.varbit);
      if (status) {
        if (err_line)
          *err_line = n_lines;
    ...
    if (block_mask & (1 << ACTION_BLOCK)) {
      status = action_block_parse(&action_spec, &block_mask, tokens, n_tokens,
                                  n_lines, err_line, err_msg);
      if (status)
        goto error;
      ...
      /* End of block. */
      status = rte_swx_pipeline_action_config(
          p, action_spec.name, action_spec.args_struct_type_name,
          action_spec.instructions, action_spec.n_instructions);
      if (status) {
        if (err_line)
          *err_line = n_lines;
        if (err_msg)
    ...
    if (block_mask & (1 << TABLE_BLOCK)) {
      status = table_block_parse(&table_spec, &block_mask, tokens, n_tokens,
                                 n_lines, err_line, err_msg);
      if (status)
        goto error;
      ...
      /* End of block. */
      status =
          rte_swx_pipeline_table_config(p, table_spec.name, &table_spec.params,
                                        table_spec.recommended_table_type_name,
                                        table_spec.args, table_spec.size);
      if (status) {
        if (err_line)
          *err_line = n_lines;
        if (err_msg)
    ...
    if (block_mask & (1 << SELECTOR_BLOCK)) {
      status = selector_block_parse(&selector_spec, &block_mask, tokens,
                                    n_tokens, n_lines, err_line, err_msg);
      if (status)
        goto error;
      ...
      /* End of block. */
      status = rte_swx_pipeline_selector_config(p, selector_spec.name,
    ...
    if (block_mask & (1 << LEARNER_BLOCK)) {
      status = learner_block_parse(&learner_spec, &block_mask, tokens, n_tokens,
                                   n_lines, err_line, err_msg);
      if (status)
        goto error;
      ...
      /* End of block. */
      status = rte_swx_pipeline_learner_config(
          p, learner_spec.name, &learner_spec.params, learner_spec.size,
    ...
    if (block_mask & (1 << APPLY_BLOCK)) {
      status = apply_block_parse(&apply_spec, &block_mask, tokens, n_tokens,
                                 n_lines, err_line, err_msg);
      if (status)
        goto error;
      ...
      /* End of block. */
      status = rte_swx_pipeline_instructions_config(p, apply_spec.instructions,
    ...
    if (!strcmp(tokens[0], "extobj")) {
      status = extobj_statement_parse(&extobj_spec, tokens, n_tokens, n_lines,
                                      err_line, err_msg);
      if (status)
        goto error;

      status = rte_swx_pipeline_extern_object_config(
          p, extobj_spec.name, extobj_spec.extern_type_name,
          extobj_spec.pragma);
      if (status) {
        if (err_line)
          *err_line = n_lines;
    ...
    if (!strcmp(tokens[0], "struct")) {
      status = struct_statement_parse(&struct_spec, &block_mask, tokens,
                                      n_tokens, n_lines, err_line, err_msg);
      if (status)
        goto error;
    ...
    if (!strcmp(tokens[0], "header")) {
      status = header_statement_parse(&header_spec, tokens, n_tokens, n_lines,
                                      err_line, err_msg);
      if (status)
        goto error;

      status = rte_swx_pipeline_packet_header_register(
          p, header_spec.name, header_spec.struct_type_name);
      if (status) {
        if (err_line)
          *err_line = n_lines;
        if (err_msg)
    ...
    if (!strcmp(tokens[0], "metadata")) {
      status = metadata_statement_parse(&metadata_spec, tokens, n_tokens,
                                        n_lines, err_line, err_msg);
      if (status)
        goto error;

      status = rte_swx_pipeline_packet_metadata_register(
          p, metadata_spec.struct_type_name);
      if (status) {
        if (err_line)
          *err_line = n_lines;
        if (err_msg)
    ...
    if (!strcmp(tokens[0], "action")) {
      status = action_statement_parse(&action_spec, &block_mask, tokens,
                                      n_tokens, n_lines, err_line, err_msg);
      if (status)
        goto error;
    ...
    if (!strcmp(tokens[0], "table")) {
      status = table_statement_parse(&table_spec, &block_mask, tokens, n_tokens,
                                     n_lines, err_line, err_msg);
      if (status)
        goto error;
    ...
    /* selector. */
    if (!strcmp(tokens[0], "selector")) {
      status = selector_statement_parse(&selector_spec, &block_mask, tokens,
                                        n_tokens, n_lines, err_line, err_msg);
      if (status)
        goto error;
    ...
    if (!strcmp(tokens[0], "learner")) {
      status = learner_statement_parse(&learner_spec, &block_mask, tokens,
                                       n_tokens, n_lines, err_line, err_msg);
      if (status)
        goto error;
    ...
    if (!strcmp(tokens[0], "regarray")) {
      status = regarray_statement_parse(&regarray_spec, tokens, n_tokens,
                                        n_lines, err_line, err_msg);
      if (status)
        goto error;

      status = rte_swx_pipeline_regarray_config(
          p, regarray_spec.name, regarray_spec.size, regarray_spec.init_val);
      if (status) {
        if (err_line)
          *err_line = n_lines;
        if (err_msg)
    ...
    if (!strcmp(tokens[0], "metarray")) {
      status = metarray_statement_parse(&metarray_spec, tokens, n_tokens,
                                        n_lines, err_line, err_msg);
      if (status)
        goto error;

      status = rte_swx_pipeline_metarray_config(p, metarray_spec.name,
                                                metarray_spec.size);
      if (status) {
        if (err_line)
          *err_line = n_lines;
    ...
    if (!strcmp(tokens[0], "apply")) {
      status = apply_statement_parse(&block_mask, tokens, n_tokens, n_lines,
                                     err_line, err_msg);
      if (status)
        goto error;
