...
struct selector {
   ...
   * commit. Array of info.n_groups_max elements.
   */
...
static struct action *action_find(struct rte_swx_ctl_pipeline *ctl,
...
static void action_free(struct rte_swx_ctl_pipeline *ctl) {
  uint32_t i;
...
static struct table *table_find(struct rte_swx_ctl_pipeline *ctl,
...
static int table_params_get(struct rte_swx_ctl_pipeline *ctl,
                            uint32_t table_id) {
...
static struct rte_swx_table_entry *table_entry_alloc(struct table *table) {
...
static int table_entry_key_check_em(struct table *table,
                                    struct rte_swx_table_entry *entry) {
...
static int table_entry_check(struct rte_swx_ctl_pipeline *ctl,
                             uint32_t table_id,
                             struct rte_swx_table_entry *entry, int key_check,
                             int data_check) {
  struct table *table = &ctl->tables[table_id];
  int status;
  ...
  if (data_check) {
    ...
    /* When both key_check and data_check are true, we are interested in both
     * the entry key and data, which means the operation is _regular_ table
     * entry add.
     */
    if (key_check && !tai->action_is_for_table_entries)
      return -EINVAL;

    /* When key_check is false while data_check is true, we are only interested
     * in the entry data, which means the operation is _default_ table entry
     * add.
     */
...
table_entry_duplicate(struct rte_swx_ctl_pipeline *ctl, uint32_t table_id,
                      struct rte_swx_table_entry *entry, int key_duplicate,
                      int data_duplicate) {
  struct table *table = &ctl->tables[table_id];
  ...
  if (key_duplicate && !table->is_stub) {
    ...
    if (entry->key_mask) {
      ...
      memcpy(new_entry->key_mask, entry->key_mask, table->params.key_size);
    }
...
static int table_entry_keycmp(struct table *table,
...
static void table_entries_free(struct table *table) {
  for (;;) {
    struct rte_swx_table_entry *entry;
...
static void table_pending_add_admit(struct table *table) {
  TAILQ_CONCAT(&table->entries, &table->pending_add, node);
}

static void table_pending_add_free(struct table *table) {
  for (;;) {
    struct rte_swx_table_entry *entry;
...
static void table_pending_modify0_admit(struct table *table) {
  TAILQ_CONCAT(&table->entries, &table->pending_modify0, node);
}

static void table_pending_modify0_free(struct table *table) {
  for (;;) {
    struct rte_swx_table_entry *entry;
...
static void table_pending_modify1_admit(struct table *table) {
  TAILQ_CONCAT(&table->entries, &table->pending_modify1, node);
}

static void table_pending_modify1_free(struct table *table) {
  for (;;) {
    struct rte_swx_table_entry *entry;
...
static void table_pending_delete_admit(struct table *table) {
  TAILQ_CONCAT(&table->entries, &table->pending_delete, node);
}

static void table_pending_delete_free(struct table *table) {
  for (;;) {
    struct rte_swx_table_entry *entry;
...
static void table_pending_default_free(struct table *table) {
...
static int table_is_update_pending(struct table *table,
                                   int consider_pending_default) {
  struct rte_swx_table_entry *e;
  uint32_t n = 0;
...
static void table_free(struct rte_swx_ctl_pipeline *ctl) {
  uint32_t i;
...
static void selector_group_members_free(struct selector *s, uint32_t group_id) {
...
static void selector_pending_group_members_free(struct selector *s,
                                                uint32_t group_id) {
...
static int selector_group_duplicate_to_pending(struct selector *s,
                                               uint32_t group_id) {
...
static void selector_free(struct rte_swx_ctl_pipeline *ctl) {
  uint32_t i;
...
static struct selector *selector_find(struct rte_swx_ctl_pipeline *ctl,
...
static int selector_params_get(struct rte_swx_ctl_pipeline *ctl,
...
static void learner_pending_default_free(struct learner *l) {
...
static void learner_free(struct rte_swx_ctl_pipeline *ctl) {
  uint32_t i;
...
static struct learner *learner_find(struct rte_swx_ctl_pipeline *ctl,
...
static uint32_t learner_action_data_size_get(struct rte_swx_ctl_pipeline *ctl,
                                             struct learner *l) {
...
static void table_state_free(struct rte_swx_ctl_pipeline *ctl) {
...
static int table_state_create(struct rte_swx_ctl_pipeline *ctl) {
  uint32_t table_base_index, selector_base_index, learner_base_index, i;
  int status = 0;

  ctl->ts_next =
      calloc(ctl->info.n_tables + ctl->info.n_selectors + ctl->info.n_learners,
  ...
  for (i = 0; i < ctl->info.n_tables; i++) {
    ...
    if (!table->is_stub && table->ops.add) {
      ts_next->obj = table->ops.create(&table->params, &table->entries,
                                       table->info.args, ctl->numa_node);
      if (!ts_next->obj) {
        status = -ENODEV;
        goto error;
      }
    ...
    ts_next->default_action_data = malloc(table->params.action_data_size);
    if (!ts_next->default_action_data) {
      status = -ENOMEM;
      goto error;
    }

    memcpy(ts_next->default_action_data, ts->default_action_data,
  ...
  for (i = 0; i < ctl->info.n_selectors; i++) {
    struct selector *s = &ctl->selectors[i];
    struct rte_swx_table_state *ts_next =
        &ctl->ts_next[selector_base_index + i];

    /* Table object. */
    ts_next->obj =
        rte_swx_table_selector_create(&s->params, NULL, ctl->numa_node);
    if (!ts_next->obj) {
      status = -ENODEV;
      goto error;
    }
...
void rte_swx_ctl_pipeline_free(struct rte_swx_ctl_pipeline *ctl) {
  if (!ctl)
    return;
...
rte_swx_ctl_pipeline_create(struct rte_swx_pipeline *p) {
  ...
  for (i = 0; i < ctl->info.n_actions; i++) {
    ...
    /* args. */
    a->args =
        calloc(a->info.n_args, sizeof(struct rte_swx_ctl_action_arg_info));
    if (!a->args)
      goto error;

    for (j = 0; j < a->info.n_args; j++) {
      status = rte_swx_ctl_action_arg_info_get(p, i, j, &a->args[j]);
      if (status)
        goto error;
    }
  ...
  for (i = 0; i < ctl->info.n_tables; i++) {
    ...
    for (j = 0; j < t->info.n_match_fields; j++) {
      status = rte_swx_ctl_table_match_field_info_get(p, i, j, &t->mf[j]);
      if (status)
        goto error;
    }
    ...
    /* actions. */
    t->actions =
        calloc(t->info.n_actions, sizeof(struct rte_swx_ctl_table_action_info));
    if (!t->actions)
      goto error;

    for (j = 0; j < t->info.n_actions; j++) {
      status = rte_swx_ctl_table_action_info_get(p, i, j, &t->actions[j]);
      if (status || t->actions[j].action_id >= ctl->info.n_actions)
        goto error;
    }
  ...
  for (i = 0; i < ctl->info.n_selectors; i++) {
    ...
    /* group_id field. */
    status =
        rte_swx_ctl_selector_group_id_field_info_get(p, i, &s->group_id_field);
    if (status)
      goto error;

    /* selector fields. */
    s->selector_fields =
        calloc(s->info.n_selector_fields,
               sizeof(struct rte_swx_ctl_table_match_field_info));
    ...
    for (j = 0; j < s->info.n_selector_fields; j++) {
      status =
          rte_swx_ctl_selector_field_info_get(p, i, j, &s->selector_fields[j]);
      if (status)
        goto error;
    }

    /* member_id field. */
    status = rte_swx_ctl_selector_member_id_field_info_get(p, i,
                                                           &s->member_id_field);
    if (status)
      goto error;
  ...
  for (i = 0; i < ctl->info.n_learners; i++) {
    ...
    for (j = 0; j < l->info.n_match_fields; j++) {
      status = rte_swx_ctl_learner_match_field_info_get(p, i, j, &l->mf[j]);
      if (status)
        goto error;
    }
    ...
    /* actions. */
    l->actions =
        calloc(l->info.n_actions, sizeof(struct rte_swx_ctl_table_action_info));
    if (!l->actions)
      goto error;

    for (j = 0; j < l->info.n_actions; j++) {
      status = rte_swx_ctl_learner_action_info_get(p, i, j, &l->actions[j]);
...
int rte_swx_ctl_pipeline_table_entry_add(struct rte_swx_ctl_pipeline *ctl,
                                         const char *table_name,
                                         struct rte_swx_table_entry *entry) {
  struct table *table;
  ...
  if (existing_entry) {
    TAILQ_INSERT_TAIL(&table->pending_modify1, new_entry, node);

    TAILQ_REMOVE(&table->entries, existing_entry, node);

    TAILQ_INSERT_TAIL(&table->pending_modify0, existing_entry, node);
  ...
  if (existing_entry) {
    TAILQ_INSERT_AFTER(&table->pending_add, existing_entry, new_entry, node);

    TAILQ_REMOVE(&table->pending_add, existing_entry, node);
  ...
  if (existing_entry) {
    TAILQ_INSERT_AFTER(&table->pending_modify1, existing_entry, new_entry,
                       node);

    TAILQ_REMOVE(&table->pending_modify1, existing_entry, node);
  ...
  if (existing_entry) {
    ...
    TAILQ_REMOVE(&table->pending_delete, existing_entry, node);
...
int rte_swx_ctl_pipeline_table_entry_delete(struct rte_swx_ctl_pipeline *ctl,
                                            const char *table_name,
                                            struct rte_swx_table_entry *entry) {
  struct table *table;
  ...
  if (existing_entry) {
    ...
    TAILQ_INSERT_TAIL(&table->pending_delete, existing_entry, node);
  ...
  if (existing_entry) {
    ...
    TAILQ_REMOVE(&table->pending_modify0, real_existing_entry, node);

    TAILQ_INSERT_TAIL(&table->pending_delete, real_existing_entry, node);
...
static void table_entry_list_free(struct rte_swx_table_entry_list *list) {
  for (;;) {
    struct rte_swx_table_entry *entry;
...
static int table_entry_list_duplicate(struct rte_swx_ctl_pipeline *ctl,
                                      uint32_t table_id,
 ...
 */
static int table_rollfwd0(struct rte_swx_ctl_pipeline *ctl, uint32_t table_id,
                          uint32_t after_swap) {
  struct table *table = &ctl->tables[table_id];
  ...
  if (!after_swap) {
    ...
    status =
        table_entry_list_duplicate(ctl, table_id, &list, &table->pending_add);
    if (status)
      goto error;

    status = table_entry_list_duplicate(ctl, table_id, &list,
                                        &table->pending_modify1);
    if (status)
      goto error;

    /* Create new table object with the updates included. */
    ts_next->obj = table->ops.create(&table->params, &list, table->info.args,
                                     ctl->numa_node);
    if (!ts_next->obj) {
      status = -ENODEV;
      goto error;
 ...
 */
static void table_rollfwd1(struct rte_swx_ctl_pipeline *ctl,
                           uint32_t table_id) {
  struct table *table = &ctl->tables[table_id];
  struct rte_swx_table_state *ts_next = &ctl->ts_next[table_id];
  struct action *a;
 ...
 * stage cannot fail.
 */
static void table_rollfwd2(struct rte_swx_ctl_pipeline *ctl,
                           uint32_t table_id) {
  struct table *table = &ctl->tables[table_id];
 ...
 * happened.
 */
static void table_rollback(struct rte_swx_ctl_pipeline *ctl,
                           uint32_t table_id) {
  struct table *table = &ctl->tables[table_id];
  struct rte_swx_table_state *ts_next = &ctl->ts_next[table_id];
 ...
 */
static void table_abort(struct rte_swx_ctl_pipeline *ctl, uint32_t table_id) {
  struct table *table = &ctl->tables[table_id];
...
int rte_swx_ctl_pipeline_selector_group_add(struct rte_swx_ctl_pipeline *ctl,
                                            const char *selector_name,
                                            uint32_t *group_id) {
  struct selector *s;
  uint32_t i;
...
int rte_swx_ctl_pipeline_selector_group_delete(struct rte_swx_ctl_pipeline *ctl,
                                               const char *selector_name,
                                               uint32_t group_id) {
  struct selector *s;
  ...
  s = selector_find(ctl, selector_name);
  if (!s || (group_id >= s->info.n_groups_max) || !s->groups_added[group_id])
    return -EINVAL;
...
int rte_swx_ctl_pipeline_selector_group_member_add(
    struct rte_swx_ctl_pipeline *ctl, const char *selector_name,
    uint32_t group_id, uint32_t member_id, uint32_t member_weight) {
  struct selector *s;
  struct rte_swx_table_selector_group *group;
  struct rte_swx_table_selector_member *m;
  ...
  if (!member_weight)
    return rte_swx_ctl_pipeline_selector_group_member_delete(
        ctl, selector_name, group_id, member_id);
  ...
  /* If this member is already in this group, then simply update its weight and
   * return. */
...
int rte_swx_ctl_pipeline_selector_group_member_delete(
    struct rte_swx_ctl_pipeline *ctl, const char *selector_name,
    uint32_t group_id __rte_unused, uint32_t member_id __rte_unused) {
  struct selector *s;
  struct rte_swx_table_selector_group *group;
  struct rte_swx_table_selector_member *m;
...
static int selector_rollfwd(struct rte_swx_ctl_pipeline *ctl,
                            uint32_t selector_id) {
  struct selector *s = &ctl->selectors[selector_id];
  struct rte_swx_table_state *ts_next =
      &ctl->ts_next[ctl->info.n_tables + selector_id];
  uint32_t group_id;

  /* Push pending group member changes (s->pending_groups[group_id]) to the
   * selector table mirror copy (ts_next->obj).
   */
...
static void selector_rollfwd_finalize(struct rte_swx_ctl_pipeline *ctl,
                                      uint32_t selector_id) {
  struct selector *s = &ctl->selectors[selector_id];
  uint32_t group_id;

  /* Commit pending group member changes (s->pending_groups[group_id]) to the
   * stable group records (s->groups[group_id).
   */
  for (group_id = 0; group_id < s->info.n_groups_max; group_id++) {
  ...
  /* Commit pending group validity changes (from
   * s->groups_pending_delete[group_id] to s->groups_added[group_id].
   */
  for (group_id = 0; group_id < s->info.n_groups_max; group_id++)
    if (s->groups_pending_delete[group_id]) {
...
static void selector_rollback(struct rte_swx_ctl_pipeline *ctl,
                              uint32_t selector_id) {
  struct selector *s = &ctl->selectors[selector_id];
  ...
  /* Discard any previous changes to the selector table mirror copy
   * (ts_next->obj). */
  for (group_id = 0; group_id < s->info.n_groups_max; group_id++) {
    struct rte_swx_table_selector_group *gp = s->pending_groups[group_id];
...
static void selector_abort(struct rte_swx_ctl_pipeline *ctl,
                           uint32_t selector_id) {
  struct selector *s = &ctl->selectors[selector_id];
  uint32_t group_id;
...
static int learner_default_entry_check(struct rte_swx_ctl_pipeline *ctl,
                                       uint32_t learner_id,
                                       struct rte_swx_table_entry *entry) {
...
int rte_swx_ctl_pipeline_learner_default_entry_add(
    struct rte_swx_ctl_pipeline *ctl, const char *learner_name,
    struct rte_swx_table_entry *entry) {
  struct learner *l;
  struct rte_swx_table_entry *new_entry;
  uint32_t learner_id;
...
static void learner_rollfwd(struct rte_swx_ctl_pipeline *ctl,
                            uint32_t learner_id) {
  struct learner *l = &ctl->learners[learner_id];
  struct rte_swx_table_state *ts_next =
      &ctl->ts_next[ctl->info.n_tables + ctl->info.n_selectors + learner_id];
  struct action *a;
  uint8_t *action_data;
  uint64_t action_id;
...
static void learner_rollfwd_finalize(struct rte_swx_ctl_pipeline *ctl,
                                     uint32_t learner_id) {
  struct learner *l = &ctl->learners[learner_id];
...
static void learner_abort(struct rte_swx_ctl_pipeline *ctl,
                          uint32_t learner_id) {
  struct learner *l = &ctl->learners[learner_id];

  /* Free up the pending default entry, as it is no longer going to be added to
   * the table. */
  learner_pending_default_free(l);
}

int rte_swx_ctl_pipeline_commit(struct rte_swx_ctl_pipeline *ctl,
                                int abort_on_fail) {
  struct rte_swx_table_state *ts;
  int status = 0;
  uint32_t i;
  ...
  /* Operate the changes on the current ts_next before it becomes the new ts.
   * First, operate all the changes that can fail; if no failure, then operate
   * the changes that cannot fail. We must be able to fully revert all the
   * changes that can fail as if they never happened.
   */
  for (i = 0; i < ctl->info.n_tables; i++) {
  ...
  /* Second, operate all the changes that cannot fail. Since nothing can fail
   * from this point onwards, the transaction is guaranteed to be successful.
   */
  for (i = 0; i < ctl->info.n_tables; i++)
  ...
  /* Operate the changes on the current ts_next, which is the previous ts, in
   * order to get the current ts_next in sync with the current ts. Since the
   * changes that can fail did not fail on the previous ts_next, it is
   * guaranteed that they will not fail on the current ts_next, hence no error
   * checking is needed.
   */
  for (i = 0; i < ctl->info.n_tables; i++) {
...
void rte_swx_ctl_pipeline_abort(struct rte_swx_ctl_pipeline *ctl) {
  uint32_t i;
...
static int mask_to_prefix(uint64_t mask, uint32_t mask_length,
...
struct rte_swx_table_entry *rte_swx_ctl_pipeline_table_entry_read(
    struct rte_swx_ctl_pipeline *ctl, const char *table_name,
    const char *string, int *is_blank_or_comment) {
  char *token_array[RTE_SWX_CTL_ENTRY_TOKENS_MAX], **tokens;
  struct table *table;
  struct action *action;
  struct rte_swx_table_entry *entry = NULL;
  char *s0 = NULL, *s;
  uint32_t n_tokens = 0, arg_offset = 0, lpm_prefix_length_max = 0,
           lpm_prefix_length = 0, i;
...
action:
  ...
  for (i = 0; i < action->info.n_args; i++) {
    ...
    /* Copy to entry. */
    memcpy(&entry->action_data[arg_offset], (uint8_t *)&val, arg->n_bits / 8);
...
struct rte_swx_table_entry *rte_swx_ctl_pipeline_learner_default_entry_read(
    struct rte_swx_ctl_pipeline *ctl, const char *learner_name,
    const char *string, int *is_blank_or_comment) {
  char *token_array[RTE_SWX_CTL_ENTRY_TOKENS_MAX], **tokens;
  struct learner *l;
...
static void table_entry_printf(FILE *f, struct rte_swx_ctl_pipeline *ctl,
                               struct table *table,
                               struct rte_swx_table_entry *entry) {
...
int rte_swx_ctl_pipeline_table_fprintf(FILE *f,
  ...
  fprintf(f, "# Table %s: key size %u bytes, key offset %u, key mask [",
          table->info.name, table->params.key_size, table->params.key_offset);
  ...
  fprintf(f, "# Table %s currently has %u entries.\n", table_name, n_entries);
  return 0;
}

int rte_swx_ctl_pipeline_selector_fprintf(FILE *f,
                                          struct rte_swx_ctl_pipeline *ctl,
                                          const char *selector_name) {
  struct selector *s;
  uint32_t group_id;
  ...
  fprintf(f, "# Selector %s: max groups %u, max members per group %u\n",
          s->info.name, s->info.n_groups_max, s->info.n_members_per_group_max);
