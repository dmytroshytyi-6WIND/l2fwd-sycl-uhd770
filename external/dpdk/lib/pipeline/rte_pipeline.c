...
static inline uint32_t rte_mask_get_next(uint64_t mask, uint32_t pos) {
  uint64_t mask_rot =
      (mask << ((63 - pos) & 0x3F)) | (mask >> ((pos + 1) & 0x3F));
  return (__builtin_ctzll(mask_rot) - (63 - pos)) & 0x3F;
}

static inline uint32_t rte_mask_get_prev(uint64_t mask, uint32_t pos) {
  uint64_t mask_rot = (mask >> (pos & 0x3F)) | (mask << ((64 - pos) & 0x3F));
  return ((63 - __builtin_clzll(mask_rot)) + pos) & 0x3F;
}

static void rte_pipeline_table_free(struct rte_table *table);

static void rte_pipeline_port_in_free(struct rte_port_in *port);

static void rte_pipeline_port_out_free(struct rte_port_out *port);

/*
 * Pipeline
 *
 */
static int rte_pipeline_check_params(struct rte_pipeline_params *params) {
  if (params == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: Incorrect value for parameter params\n",
            __func__);
    return -EINVAL;
  }
  ...
  /* name */
  if (params->name == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: Incorrect value for parameter name\n",
            __func__);
    return -EINVAL;
  }
  ...
  if (params->socket_id < 0) {
    RTE_LOG(ERR, PIPELINE, "%s: Incorrect value for parameter socket_id\n",
            __func__);
    return -EINVAL;
  }
...
struct rte_pipeline *rte_pipeline_create(struct rte_pipeline_params *params) {
  ...
  if (status != 0) {
    RTE_LOG(ERR, PIPELINE, "%s: Pipeline params check failed (%d)\n", __func__,
            status);
    return NULL;
  }
  ...
  if (p == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: Pipeline memory allocation failed\n", __func__);
    return NULL;
  }
 ...
 *
 */
static int rte_table_check_params(struct rte_pipeline *p,
  ...
  if (params->ops->f_create == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: f_create function pointer is NULL\n", __func__);
    return -EINVAL;
  }

  if (params->ops->f_lookup == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: f_lookup function pointer is NULL\n", __func__);
    return -EINVAL;
  }

  /* De we have room for one more table? */
  if (p->num_tables == RTE_PIPELINE_TABLE_MAX) {
    RTE_LOG(ERR, PIPELINE, "%s: Incorrect value for num_tables parameter\n",
            __func__);
    return -EINVAL;
  }
...
int rte_pipeline_table_create(struct rte_pipeline *p,
  ...
  entry_size =
      sizeof(struct rte_pipeline_table_entry) + params->action_data_size;
  default_entry = rte_zmalloc_socket("PIPELINE", entry_size,
                                     RTE_CACHE_LINE_SIZE, p->socket_id);
  if (default_entry == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: Failed to allocate default entry\n", __func__);
    return -EINVAL;
  }

  /* Create the table */
  h_table = params->ops->f_create(params->arg_create, p->socket_id, entry_size);
  if (h_table == NULL) {
...
void rte_pipeline_table_free(struct rte_table *table) {
...
int rte_pipeline_table_default_entry_add(
    struct rte_pipeline *p, uint32_t table_id,
    struct rte_pipeline_table_entry *default_entry,
  ...
  if (default_entry == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: default_entry parameter is NULL\n", __func__);
    return -EINVAL;
  }

  if (table_id >= p->num_tables) {
    RTE_LOG(ERR, PIPELINE, "%s: table_id %d out of range\n", __func__,
            table_id);
    return -EINVAL;
  }
  ...
  if ((default_entry->action == RTE_PIPELINE_ACTION_TABLE) &&
    ...
    RTE_LOG(ERR, PIPELINE, "%s: Tree-like topologies not allowed\n", __func__);
    return -EINVAL;
  }
...
int rte_pipeline_table_default_entry_delete(
    struct rte_pipeline *p, uint32_t table_id,
...
int rte_pipeline_table_entry_add(struct rte_pipeline *p, uint32_t table_id,
                                 void *key,
                                 struct rte_pipeline_table_entry *entry,
                                 int *key_found,
  ...
  if ((entry->action == RTE_PIPELINE_ACTION_TABLE) &&
      table->table_next_id_valid && (entry->table_id != table->table_next_id)) {
    RTE_LOG(ERR, PIPELINE, "%s: Tree-like topologies not allowed\n", __func__);
    return -EINVAL;
  }
  ...
  return (table->ops.f_add)(table->h_table, key, (void *)entry, key_found,
                            (void **)entry_ptr);
}

int rte_pipeline_table_entry_delete(struct rte_pipeline *p, uint32_t table_id,
                                    void *key, int *key_found,
                                    struct rte_pipeline_table_entry *entry) {
  struct rte_table *table;
  ...
  if (table->ops.f_delete == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: f_delete function pointer NULL\n", __func__);
    return -EINVAL;
  }
...
int rte_pipeline_table_entry_add_bulk(
    struct rte_pipeline *p, uint32_t table_id, void **keys,
    struct rte_pipeline_table_entry **entries, uint32_t n_keys, int *key_found,
  ...
  return (table->ops.f_add_bulk)(table->h_table, keys, (void **)entries, n_keys,
                                 key_found, (void **)entries_ptr);
}

int rte_pipeline_table_entry_delete_bulk(
    struct rte_pipeline *p, uint32_t table_id, void **keys, uint32_t n_keys,
    int *key_found, struct rte_pipeline_table_entry **entries) {
  struct rte_table *table;
...
rte_pipeline_port_in_check_params(struct rte_pipeline *p,
  ...
  if (params->ops->f_create == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: f_create function pointer NULL\n", __func__);
    return -EINVAL;
  }
  ...
  if (p->num_ports_in == RTE_PIPELINE_PORT_IN_MAX) {
    RTE_LOG(ERR, PIPELINE, "%s: invalid value for num_ports_in\n", __func__);
    return -EINVAL;
  }
...
rte_pipeline_port_out_check_params(struct rte_pipeline *p,
  ...
  if (params->ops->f_tx == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: f_tx function pointer NULL\n", __func__);
    return -EINVAL;
  }

  if (params->ops->f_tx_bulk == NULL) {
    RTE_LOG(ERR, PIPELINE, "%s: f_tx_bulk function pointer NULL\n", __func__);
    return -EINVAL;
  }

  /* Do we have room for one more port? */
  if (p->num_ports_out == RTE_PIPELINE_PORT_OUT_MAX) {
    RTE_LOG(ERR, PIPELINE, "%s: invalid value for num_ports_out\n", __func__);
    return -EINVAL;
  }
...
void rte_pipeline_port_in_free(struct rte_port_in *port) {
...
void rte_pipeline_port_out_free(struct rte_port_out *port) {
  if (port->ops.f_free != NULL)
    port->ops.f_free(port->h_port);
}
...
int rte_pipeline_port_in_connect_to_table(struct rte_pipeline *p,
  ...
  if (table_id >= p->num_tables) {
    RTE_LOG(ERR, PIPELINE, "%s: Table ID %u is out of range\n", __func__,
            table_id);
    return -EINVAL;
  }
...
static inline void rte_pipeline_compute_masks(struct rte_pipeline *p,
                                              uint64_t pkts_mask) {
...
static inline void rte_pipeline_action_handler_port_bulk(struct rte_pipeline *p,
                                                         uint64_t pkts_mask,
                                                         uint32_t port_id) {
  ...
  if (port_out->f_action != NULL) {
    ...
    RTE_PIPELINE_STATS_AH_DROP_READ(p, port_out->n_pkts_dropped_by_ah);
  }

  /* Output port TX */
  if (p->pkts_mask != 0)
    port_out->ops.f_tx_bulk(port_out->h_port, p->pkts, p->pkts_mask);
}

static inline void rte_pipeline_action_handler_port(struct rte_pipeline *p,
                                                    uint64_t pkts_mask) {
  p->pkts_mask = pkts_mask;
  ...
  if ((pkts_mask & (pkts_mask + 1)) == 0) {
    ...
    for (i = 0; i < n_pkts; i++) {
      ...
      if (port_out->f_action == NULL) /* Output port TX */
        ...
        port_out->f_action(p, p->pkts, pkt_mask, port_out->arg_ah);
...
static inline void rte_pipeline_action_handler_port_meta(struct rte_pipeline *p,
                                                         uint64_t pkts_mask) {
  p->pkts_mask = pkts_mask;
...
static inline void rte_pipeline_action_handler_drop(struct rte_pipeline *p,
                                                    uint64_t pkts_mask) {
  if ((pkts_mask & (pkts_mask + 1)) == 0) {
    uint64_t n_pkts = __builtin_popcountll(pkts_mask);
    uint32_t i;
...
int rte_pipeline_run(struct rte_pipeline *p) {
  ...
  if (port_in->f_action != NULL) {
    ...
    RTE_PIPELINE_STATS_AH_DROP_READ(p, port_in->n_pkts_dropped_by_ah);
  }
  ...
  for (table_id = port_in->table_id; p->pkts_mask != 0;) {
    ...
    /* Lookup */
    table = &p->tables[table_id];
    table->ops.f_lookup(table->h_table, p->pkts, p->pkts_mask, &lookup_hit_mask,
                        (void **)p->entries);
    lookup_miss_mask = p->pkts_mask & (~lookup_hit_mask);

    /* Lookup miss */
    if (lookup_miss_mask != 0) {
      struct rte_pipeline_table_entry *default_entry = table->default_entry;

      p->pkts_mask = lookup_miss_mask;

      /* Table user actions */
      if (table->f_action_miss != NULL) {
        table->f_action_miss(p, p->pkts, lookup_miss_mask, default_entry,
                             table->arg_ah);
      ...
      if ((default_entry->action == RTE_PIPELINE_ACTION_PORT) &&
          (p->pkts_mask != 0))
        rte_pipeline_action_handler_port_bulk(p, p->pkts_mask,
        ...
        RTE_PIPELINE_STATS_TABLE_DROP1(p, table->n_pkts_dropped_lkp_miss);
      }
    }
    ...
    if (lookup_hit_mask != 0) {
      ...
      if (table->f_action_hit != NULL) {
        table->f_action_hit(p, p->pkts, lookup_hit_mask, p->entries,
                            table->arg_ah);

        RTE_PIPELINE_STATS_AH_DROP_READ(p, table->n_pkts_dropped_by_lkp_hit_ah);
      }
      ...
      RTE_PIPELINE_STATS_TABLE_DROP1(p, table->n_pkts_dropped_lkp_hit);
    }
...
int rte_pipeline_port_out_stats_read(struct rte_pipeline *p, uint32_t port_id,
  ...
  if (port_id >= p->num_ports_out) {
    RTE_LOG(ERR, PIPELINE, "%s: port OUT ID %u is out of range\n", __func__,
            port_id);
    return -EINVAL;
  }
...
int rte_pipeline_table_stats_read(struct rte_pipeline *p, uint32_t table_id,
  ...
  if (table_id >= p->num_tables) {
    RTE_LOG(ERR, PIPELINE, "%s: table %u is out of range\n", __func__,
            table_id);
    return -EINVAL;
  }
  ...
  if (stats != NULL) {
    stats->n_pkts_dropped_by_lkp_hit_ah = table->n_pkts_dropped_by_lkp_hit_ah;
    stats->n_pkts_dropped_by_lkp_miss_ah = table->n_pkts_dropped_by_lkp_miss_ah;
