...
static void *env_malloc(size_t size, size_t alignment, int numa_node) {
  return rte_zmalloc_socket(NULL, size, alignment, numa_node);
}

static void env_free(void *start, size_t size __rte_unused) { rte_free(start); }

#else

#include <numa.h>

static void *env_malloc(size_t size, size_t alignment __rte_unused,
                        int numa_node) {
  void *start;
...
static void env_free(void *start, size_t size) {
  if (numa_available() == -1)
    return;
 ...
 * Struct.
 */
static struct struct_type *struct_type_find(struct rte_swx_pipeline *p,
                                            const char *name) {
...
static struct field *struct_type_field_find(struct struct_type *st,
                                            const char *name) {
  uint32_t i;
...
int rte_swx_pipeline_struct_type_register(struct rte_swx_pipeline *p,
                                          const char *name,
  ...
  for (i = 0; i < n_fields; i++) {
    ...
    int var_size =
        ((i == n_fields - 1) && last_field_has_variable_size) ? 1 : 0;
    uint32_t j;
...
static int struct_build(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static void struct_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static void struct_free(struct rte_swx_pipeline *p) {
  struct_build_free(p);
 ...
 * Input port.
 */
static struct port_in_type *port_in_type_find(struct rte_swx_pipeline *p,
                                              const char *name) {
...
int rte_swx_pipeline_port_in_type_register(struct rte_swx_pipeline *p,
                                           const char *name,
...
static struct port_in *port_in_find(struct rte_swx_pipeline *p,
                                    uint32_t port_id) {
  struct port_in *port;
...
int rte_swx_pipeline_port_in_config(struct rte_swx_pipeline *p,
                                    uint32_t port_id,
...
static int port_in_build(struct rte_swx_pipeline *p) {
  struct port_in *port;
  uint32_t i;
...
static void port_in_build_free(struct rte_swx_pipeline *p) {
  free(p->in);
  p->in = NULL;
}

static void port_in_free(struct rte_swx_pipeline *p) {
 ...
 * Output port.
 */
static struct port_out_type *port_out_type_find(struct rte_swx_pipeline *p,
                                                const char *name) {
...
int rte_swx_pipeline_port_out_type_register(struct rte_swx_pipeline *p,
                                            const char *name,
...
static struct port_out *port_out_find(struct rte_swx_pipeline *p,
                                      uint32_t port_id) {
...
int rte_swx_pipeline_port_out_config(struct rte_swx_pipeline *p,
                                     uint32_t port_id,
                                     const char *port_type_name, void *args) {
...
static int port_out_build(struct rte_swx_pipeline *p) {
  struct port_out *port;
  uint32_t i;
...
static void port_out_build_free(struct rte_swx_pipeline *p) {
  free(p->out);
  p->out = NULL;
}

static void port_out_free(struct rte_swx_pipeline *p) {
 ...
 * Extern object.
 */
static struct extern_type *extern_type_find(struct rte_swx_pipeline *p,
                                            const char *name) {
...
static struct extern_obj *extern_obj_find(struct rte_swx_pipeline *p,
                                          const char *name) {
...
static struct extern_type_member_func *
extern_obj_member_func_parse(struct rte_swx_pipeline *p, const char *name,
...
static struct field *
extern_obj_mailbox_field_parse(struct rte_swx_pipeline *p, const char *name,
                               struct extern_obj **object) {
  struct extern_obj *obj;
  struct field *f;
...
int rte_swx_pipeline_extern_type_register(
    struct rte_swx_pipeline *p, const char *name,
...
int rte_swx_pipeline_extern_type_member_func_register(
    struct rte_swx_pipeline *p, const char *extern_type_name, const char *name,
...
int rte_swx_pipeline_extern_object_config(struct rte_swx_pipeline *p,
...
static int extern_obj_build(struct rte_swx_pipeline *p) {
  uint32_t i;
  ...
  for (i = 0; i < RTE_SWX_PIPELINE_THREADS_MAX; i++) {
    ...
    t->extern_objs =
        calloc(p->n_extern_objs, sizeof(struct extern_obj_runtime));
    CHECK(t->extern_objs, ENOMEM);

    TAILQ_FOREACH(obj, &p->extern_objs, node) {
      struct extern_obj_runtime *r = &t->extern_objs[obj->id];
      struct extern_type_member_func *func;
      uint32_t mailbox_size = obj->type->mailbox_struct_type->n_bits / 8;
...
static void extern_obj_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static void extern_obj_free(struct rte_swx_pipeline *p) {
 ...
 * Extern function.
 */
static struct extern_func *extern_func_find(struct rte_swx_pipeline *p,
                                            const char *name) {
...
static struct extern_func *extern_func_parse(struct rte_swx_pipeline *p,
                                             const char *name) {
  if (name[0] != 'f' || name[1] != '.')
    return NULL;
...
static struct field *
extern_func_mailbox_field_parse(struct rte_swx_pipeline *p, const char *name,
...
int rte_swx_pipeline_extern_func_register(struct rte_swx_pipeline *p,
                                          const char *name,
                                          const char *mailbox_struct_type_name,
...
static int extern_func_build(struct rte_swx_pipeline *p) {
  uint32_t i;
  ...
  for (i = 0; i < RTE_SWX_PIPELINE_THREADS_MAX; i++) {
    ...
    /* Memory allocation. */
    t->extern_funcs =
        calloc(p->n_extern_funcs, sizeof(struct extern_func_runtime));
    CHECK(t->extern_funcs, ENOMEM);

    /* Extern function. */
    TAILQ_FOREACH(func, &p->extern_funcs, node) {
      struct extern_func_runtime *r = &t->extern_funcs[func->id];
      uint32_t mailbox_size = func->mailbox_struct_type->n_bits / 8;
...
static void extern_func_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static void extern_func_free(struct rte_swx_pipeline *p) {
 ...
 * Header.
 */
static struct header *header_find(struct rte_swx_pipeline *p,
                                  const char *name) {
  struct header *elem;
...
static struct header *header_find_by_struct_id(struct rte_swx_pipeline *p,
                                               uint32_t struct_id) {
  struct header *elem;
...
static struct header *header_parse(struct rte_swx_pipeline *p,
                                   const char *name) {
  if (name[0] != 'h' || name[1] != '.')
    return NULL;
...
static struct field *header_field_parse(struct rte_swx_pipeline *p,
                                        const char *name,
...
int rte_swx_pipeline_packet_header_register(struct rte_swx_pipeline *p,
                                            const char *name,
...
static int header_build(struct rte_swx_pipeline *p) {
  struct header *h;
  uint32_t n_bytes = 0, i;

  TAILQ_FOREACH(h, &p->headers, node) { n_bytes += h->st->n_bits / 8; }

  for (i = 0; i < RTE_SWX_PIPELINE_THREADS_MAX; i++) {
    struct thread *t = &p->threads[i];
    uint32_t offset = 0;

    t->headers = calloc(p->n_headers, sizeof(struct header_runtime));
    CHECK(t->headers, ENOMEM);

    t->headers_out = calloc(p->n_headers, sizeof(struct header_out_runtime));
...
static void header_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static void header_free(struct rte_swx_pipeline *p) {
  header_build_free(p);
 ...
 * Meta-data.
 */
static struct field *metadata_field_parse(struct rte_swx_pipeline *p,
                                          const char *name) {
  if (!p->metadata_st)
    return NULL;
...
int rte_swx_pipeline_packet_metadata_register(struct rte_swx_pipeline *p,
                                              const char *struct_type_name) {
...
static int metadata_build(struct rte_swx_pipeline *p) {
...
static void metadata_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static void metadata_free(struct rte_swx_pipeline *p) {
 ...
 * Instruction.
 */
static int instruction_is_tx(enum instruction_type type) {
  switch (type) {
  case INSTR_TX:
  case INSTR_TX_I:
  case INSTR_DROP:
...
static int instruction_does_tx(struct instruction *instr) {
  switch (instr->type) {
  case INSTR_TX:
  case INSTR_TX_I:
  case INSTR_DROP:
...
static int instruction_is_jmp(struct instruction *instr) {
  switch (instr->type) {
  case INSTR_JMP:
...
static int instruction_does_thread_yield(struct instruction *instr) {
  switch (instr->type) {
  case INSTR_RX:
  case INSTR_TABLE:
  case INSTR_TABLE_AF:
...
static struct field *action_field_parse(struct action *action,
                                        const char *name);

static struct field *struct_field_parse(struct rte_swx_pipeline *p,
                                        struct action *action, const char *name,
                                        uint32_t *struct_id) {
  struct field *f;
 ...
 * rx.
 */
static int instr_rx_translate(struct rte_swx_pipeline *p, struct action *action,
 ...
 * tx.
 */
static int instr_tx_translate(struct rte_swx_pipeline *p,
                              struct action *action __rte_unused, char **tokens,
                              int n_tokens, struct instruction *instr,
                              struct instruction_data *data __rte_unused) {
  char *port = tokens[1];
  struct field *f;
  uint32_t port_val;
...
static int instr_drop_translate(struct rte_swx_pipeline *p __rte_unused,
                                struct action *action __rte_unused,
                                char **tokens __rte_unused, int n_tokens,
                                struct instruction *instr,
                                struct instruction_data *data __rte_unused) {
...
static inline void instr_tx_exec(struct rte_swx_pipeline *p) {
...
static inline void instr_tx_i_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_drop_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 */
static int
instr_hdr_extract_translate(struct rte_swx_pipeline *p, struct action *action,
                            char **tokens, int n_tokens,
                            struct instruction *instr,
                            struct instruction_data *data __rte_unused) {
  struct header *h;
...
static int
instr_hdr_lookahead_translate(struct rte_swx_pipeline *p, struct action *action,
                              char **tokens, int n_tokens,
                              struct instruction *instr,
                              struct instruction_data *data __rte_unused) {
  struct header *h;
...
static inline void instr_hdr_extract_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_extract2_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_extract3_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_extract4_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_extract5_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_extract6_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_extract7_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_extract8_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_extract_m_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_lookahead_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_emit_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_emit_tx_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_emit2_tx_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_emit3_tx_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_emit4_tx_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_emit5_tx_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_emit6_tx_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_emit7_tx_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_emit8_tx_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_hdr_validate_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * invalidate.
 */
static int instr_hdr_invalidate_translate(
    struct rte_swx_pipeline *p, struct action *action __rte_unused,
    char **tokens, int n_tokens, struct instruction *instr,
    struct instruction_data *data __rte_unused) {
  struct header *h;
...
static inline void instr_hdr_invalidate_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * table.
 */
static struct table *table_find(struct rte_swx_pipeline *p, const char *name);

static struct selector *selector_find(struct rte_swx_pipeline *p,
                                      const char *name);

static struct learner *learner_find(struct rte_swx_pipeline *p,
                                    const char *name);

static int instr_table_translate(struct rte_swx_pipeline *p,
                                 struct action *action, char **tokens,
                                 int n_tokens, struct instruction *instr,
                                 struct instruction_data *data __rte_unused) {
  struct table *t;
  struct selector *s;
...
static inline void instr_table_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
  ...
  /* Table. */
  done = table->func(ts->obj, table->mailbox, table->key, &action_id,
                     &action_data, &hit);
  if (!done) {
    /* Thread. */
    TRACE("[Thread %2u] table %u (not finalized)\n", p->thread_id, table_id);
  ...
  TRACE("[Thread %2u] table %u (%s, action %u)\n", p->thread_id, table_id,
        hit ? "hit" : "miss", (uint32_t)action_id);
...
static inline void instr_table_af_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
  uint32_t table_id = ip->table.table_id;
  struct rte_swx_table_state *ts = &t->table_state[table_id];
...
static inline void instr_selector_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
  ...
  /* Table. */
  done = rte_swx_table_selector_select(
      ts->obj, selector->mailbox, selector->group_id_buffer,
      selector->selector_buffer, selector->member_id_buffer);
  if (!done) {
    /* Thread. */
...
static inline void instr_learner_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
  uint32_t learner_id = ip->table.table_id;
  struct rte_swx_table_state *ts =
      &t->table_state[p->n_tables + p->n_selectors + learner_id];
  ...
  done = rte_swx_table_learner_lookup(ts->obj, l->mailbox, time, l->key,
                                      &action_id, &action_data, &hit);
  if (!done) {
    /* Thread. */
...
static inline void instr_learner_af_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
  uint32_t learner_id = ip->table.table_id;
  struct rte_swx_table_state *ts =
 ...
 * learn.
 */
static struct action *action_find(struct rte_swx_pipeline *p, const char *name);

static int action_has_nbo_args(struct action *a);

static int learner_action_args_check(struct rte_swx_pipeline *p,
                                     struct action *a, const char *mf_name);

static int instr_learn_translate(struct rte_swx_pipeline *p,
                                 struct action *action, char **tokens,
                                 int n_tokens, struct instruction *instr,
                                 struct instruction_data *data __rte_unused) {
  struct action *a;
...
static inline void instr_learn_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * forget.
 */
static int instr_forget_translate(struct rte_swx_pipeline *p __rte_unused,
...
static inline void instr_forget_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * extern.
 */
static int instr_extern_translate(struct rte_swx_pipeline *p,
                                  struct action *action __rte_unused,
                                  char **tokens, int n_tokens,
                                  struct instruction *instr,
                                  struct instruction_data *data __rte_unused) {
...
static inline void instr_extern_obj_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
  uint32_t done;
...
static inline void instr_extern_func_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
  uint32_t done;
 ...
 * mov.
 */
static int instr_mov_translate(struct rte_swx_pipeline *p,
                               struct action *action, char **tokens,
                               int n_tokens, struct instruction *instr,
                               struct instruction_data *data __rte_unused) {
...
static inline void instr_mov_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_mov_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_mov_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_mov_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_mov_i_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * dma.
 */
static inline void instr_dma_ht_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_dma_ht2_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_dma_ht3_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_dma_ht4_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_dma_ht5_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_dma_ht6_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_dma_ht7_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_dma_ht8_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * alu.
 */
static int instr_alu_add_translate(struct rte_swx_pipeline *p,
                                   struct action *action, char **tokens,
                                   int n_tokens, struct instruction *instr,
                                   struct instruction_data *data __rte_unused) {
  char *dst = tokens[1], *src = tokens[2];
...
static int instr_alu_sub_translate(struct rte_swx_pipeline *p,
                                   struct action *action, char **tokens,
                                   int n_tokens, struct instruction *instr,
                                   struct instruction_data *data __rte_unused) {
  char *dst = tokens[1], *src = tokens[2];
...
instr_alu_ckadd_translate(struct rte_swx_pipeline *p,
  ...
  instr->alu.src.n_bits =
      (uint8_t)hsrc->id;     /* The src header ID is stored here. */
...
static int instr_alu_shl_translate(struct rte_swx_pipeline *p,
                                   struct action *action, char **tokens,
                                   int n_tokens, struct instruction *instr,
                                   struct instruction_data *data __rte_unused) {
  char *dst = tokens[1], *src = tokens[2];
...
static int instr_alu_shr_translate(struct rte_swx_pipeline *p,
                                   struct action *action, char **tokens,
                                   int n_tokens, struct instruction *instr,
                                   struct instruction_data *data __rte_unused) {
  char *dst = tokens[1], *src = tokens[2];
...
static int instr_alu_and_translate(struct rte_swx_pipeline *p,
                                   struct action *action, char **tokens,
                                   int n_tokens, struct instruction *instr,
                                   struct instruction_data *data __rte_unused) {
  char *dst = tokens[1], *src = tokens[2];
...
static int instr_alu_or_translate(struct rte_swx_pipeline *p,
                                  struct action *action, char **tokens,
                                  int n_tokens, struct instruction *instr,
                                  struct instruction_data *data __rte_unused) {
  char *dst = tokens[1], *src = tokens[2];
...
static int instr_alu_xor_translate(struct rte_swx_pipeline *p,
                                   struct action *action, char **tokens,
                                   int n_tokens, struct instruction *instr,
                                   struct instruction_data *data __rte_unused) {
  char *dst = tokens[1], *src = tokens[2];
...
static inline void instr_alu_add_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_add_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_add_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_add_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_add_mi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_add_hi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_sub_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_sub_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_sub_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_sub_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_sub_mi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_sub_hi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shl_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shl_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shl_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shl_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shl_mi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shl_hi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shr_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shr_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shr_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shr_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shr_mi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_shr_hi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_and_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_and_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_and_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_and_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_and_i_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_or_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_or_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_or_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_or_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_or_i_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_xor_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_xor_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_xor_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_xor_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_xor_i_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_ckadd_field_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_cksub_field_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_ckadd_struct20_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_alu_ckadd_struct_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * Register array.
 */
static struct regarray *regarray_find(struct rte_swx_pipeline *p,
                                      const char *name);

static int
instr_regprefetch_translate(struct rte_swx_pipeline *p, struct action *action,
                            char **tokens, int n_tokens,
                            struct instruction *instr,
                            struct instruction_data *data __rte_unused) {
...
static int instr_regrd_translate(struct rte_swx_pipeline *p,
                                 struct action *action, char **tokens,
                                 int n_tokens, struct instruction *instr,
                                 struct instruction_data *data __rte_unused) {
...
static int instr_regwr_translate(struct rte_swx_pipeline *p,
                                 struct action *action, char **tokens,
                                 int n_tokens, struct instruction *instr,
                                 struct instruction_data *data __rte_unused) {
...
static int instr_regadd_translate(struct rte_swx_pipeline *p,
                                  struct action *action, char **tokens,
                                  int n_tokens, struct instruction *instr,
                                  struct instruction_data *data __rte_unused) {
  char *regarray = tokens[1], *idx = tokens[2], *src = tokens[3];
...
static inline void instr_regprefetch_rh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regprefetch_rm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regprefetch_ri_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regrd_hrh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regrd_hrm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regrd_mrh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regrd_mrm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regrd_hri_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regrd_mri_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regwr_rhh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regwr_rhm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regwr_rmh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regwr_rmm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regwr_rhi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regwr_rmi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regwr_rih_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regwr_rim_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regwr_rii_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regadd_rhh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regadd_rhm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regadd_rmh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regadd_rmm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regadd_rhi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regadd_rmi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regadd_rih_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regadd_rim_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_regadd_rii_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * metarray.
 */
static struct metarray *metarray_find(struct rte_swx_pipeline *p,
                                      const char *name);

static int
instr_metprefetch_translate(struct rte_swx_pipeline *p, struct action *action,
                            char **tokens, int n_tokens,
                            struct instruction *instr,
                            struct instruction_data *data __rte_unused) {
...
static int instr_meter_translate(struct rte_swx_pipeline *p,
                                 struct action *action, char **tokens,
                                 int n_tokens, struct instruction *instr,
                                 struct instruction_data *data __rte_unused) {
...
static inline void instr_metprefetch_h_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_metprefetch_m_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_metprefetch_i_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_hhm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_hhi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_hmm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_hmi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_mhm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_mhi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_mmm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_mmi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_ihm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_ihi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_imm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_meter_imi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * jmp.
 */
static int instr_jmp_translate(struct rte_swx_pipeline *p __rte_unused,
                               struct action *action __rte_unused,
                               char **tokens, int n_tokens,
                               struct instruction *instr,
...
static int instr_jmp_valid_translate(struct rte_swx_pipeline *p,
                                     struct action *action __rte_unused,
                                     char **tokens, int n_tokens,
                                     struct instruction *instr,
                                     struct instruction_data *data) {
...
static int instr_jmp_invalid_translate(struct rte_swx_pipeline *p,
                                       struct action *action __rte_unused,
                                       char **tokens, int n_tokens,
                                       struct instruction *instr,
                                       struct instruction_data *data) {
...
static int instr_jmp_hit_translate(struct rte_swx_pipeline *p __rte_unused,
                                   struct action *action, char **tokens,
                                   int n_tokens, struct instruction *instr,
                                   struct instruction_data *data) {
  CHECK(!action, EINVAL);
...
static int instr_jmp_miss_translate(struct rte_swx_pipeline *p __rte_unused,
                                    struct action *action, char **tokens,
                                    int n_tokens, struct instruction *instr,
                                    struct instruction_data *data) {
  CHECK(!action, EINVAL);
...
static int instr_jmp_action_hit_translate(struct rte_swx_pipeline *p,
                                          struct action *action, char **tokens,
                                          int n_tokens,
                                          struct instruction *instr,
                                          struct instruction_data *data) {
...
static int instr_jmp_action_miss_translate(struct rte_swx_pipeline *p,
                                           struct action *action, char **tokens,
                                           int n_tokens,
                                           struct instruction *instr,
                                           struct instruction_data *data) {
...
static int instr_jmp_eq_translate(struct rte_swx_pipeline *p,
                                  struct action *action, char **tokens,
                                  int n_tokens, struct instruction *instr,
                                  struct instruction_data *data) {
...
static int instr_jmp_neq_translate(struct rte_swx_pipeline *p,
                                   struct action *action, char **tokens,
                                   int n_tokens, struct instruction *instr,
                                   struct instruction_data *data) {
  char *a = tokens[2], *b = tokens[3];
...
static int instr_jmp_lt_translate(struct rte_swx_pipeline *p,
                                  struct action *action, char **tokens,
                                  int n_tokens, struct instruction *instr,
                                  struct instruction_data *data) {
  char *a = tokens[2], *b = tokens[3];
...
static int instr_jmp_gt_translate(struct rte_swx_pipeline *p,
                                  struct action *action, char **tokens,
                                  int n_tokens, struct instruction *instr,
                                  struct instruction_data *data) {
  char *a = tokens[2], *b = tokens[3];
...
static inline void instr_jmp_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_valid_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_invalid_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
  uint32_t header_id = ip->jmp.header_id;
...
static inline void instr_jmp_hit_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_miss_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_action_hit_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_action_miss_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_eq_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_eq_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_eq_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_eq_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_eq_i_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_neq_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_neq_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_neq_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_neq_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_neq_i_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_lt_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_lt_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_lt_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_lt_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_lt_mi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_lt_hi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_gt_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_gt_mh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_gt_hm_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_gt_hh_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_gt_mi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
...
static inline void instr_jmp_gt_hi_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * return.
 */
static int instr_return_translate(struct rte_swx_pipeline *p __rte_unused,
                                  struct action *action,
                                  char **tokens __rte_unused, int n_tokens,
                                  struct instruction *instr,
                                  struct instruction_data *data __rte_unused) {
...
static inline void instr_return_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
...
static int instr_translate(struct rte_swx_pipeline *p, struct action *action,
                           char *string, struct instruction *instr,
                           struct instruction_data *data) {
  ...
  if (!strcmp(tokens[tpos], "rx"))
    return instr_rx_translate(p, action, &tokens[tpos], n_tokens - tpos, instr,
                              data);

  if (!strcmp(tokens[tpos], "tx"))
    return instr_tx_translate(p, action, &tokens[tpos], n_tokens - tpos, instr,
                              data);

  if (!strcmp(tokens[tpos], "drop"))
    return instr_drop_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                instr, data);

  if (!strcmp(tokens[tpos], "extract"))
    return instr_hdr_extract_translate(p, action, &tokens[tpos],
                                       n_tokens - tpos, instr, data);

  if (!strcmp(tokens[tpos], "lookahead"))
    return instr_hdr_lookahead_translate(p, action, &tokens[tpos],
                                         n_tokens - tpos, instr, data);

  if (!strcmp(tokens[tpos], "emit"))
    return instr_hdr_emit_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                    instr, data);

  if (!strcmp(tokens[tpos], "validate"))
    return instr_hdr_validate_translate(p, action, &tokens[tpos],
                                        n_tokens - tpos, instr, data);

  if (!strcmp(tokens[tpos], "invalidate"))
    return instr_hdr_invalidate_translate(p, action, &tokens[tpos],
                                          n_tokens - tpos, instr, data);

  if (!strcmp(tokens[tpos], "mov"))
    return instr_mov_translate(p, action, &tokens[tpos], n_tokens - tpos, instr,
                               data);

  if (!strcmp(tokens[tpos], "add"))
    return instr_alu_add_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                   instr, data);

  if (!strcmp(tokens[tpos], "sub"))
    return instr_alu_sub_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                   instr, data);

  if (!strcmp(tokens[tpos], "ckadd"))
    return instr_alu_ckadd_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                     instr, data);

  if (!strcmp(tokens[tpos], "cksub"))
    return instr_alu_cksub_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                     instr, data);

  if (!strcmp(tokens[tpos], "and"))
    return instr_alu_and_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                   instr, data);

  if (!strcmp(tokens[tpos], "or"))
    return instr_alu_or_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                  instr, data);

  if (!strcmp(tokens[tpos], "xor"))
    return instr_alu_xor_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                   instr, data);

  if (!strcmp(tokens[tpos], "shl"))
    return instr_alu_shl_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                   instr, data);

  if (!strcmp(tokens[tpos], "shr"))
    return instr_alu_shr_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                   instr, data);

  if (!strcmp(tokens[tpos], "regprefetch"))
    return instr_regprefetch_translate(p, action, &tokens[tpos],
                                       n_tokens - tpos, instr, data);

  if (!strcmp(tokens[tpos], "regrd"))
    return instr_regrd_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                 instr, data);

  if (!strcmp(tokens[tpos], "regwr"))
    return instr_regwr_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                 instr, data);

  if (!strcmp(tokens[tpos], "regadd"))
    return instr_regadd_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                  instr, data);

  if (!strcmp(tokens[tpos], "metprefetch"))
    return instr_metprefetch_translate(p, action, &tokens[tpos],
                                       n_tokens - tpos, instr, data);

  if (!strcmp(tokens[tpos], "meter"))
    return instr_meter_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                 instr, data);

  if (!strcmp(tokens[tpos], "table"))
    return instr_table_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                 instr, data);

  if (!strcmp(tokens[tpos], "learn"))
    return instr_learn_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                 instr, data);

  if (!strcmp(tokens[tpos], "forget"))
    return instr_forget_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                  instr, data);

  if (!strcmp(tokens[tpos], "extern"))
    return instr_extern_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                  instr, data);

  if (!strcmp(tokens[tpos], "jmp"))
    return instr_jmp_translate(p, action, &tokens[tpos], n_tokens - tpos, instr,
                               data);

  if (!strcmp(tokens[tpos], "jmpv"))
    return instr_jmp_valid_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                     instr, data);

  if (!strcmp(tokens[tpos], "jmpnv"))
    return instr_jmp_invalid_translate(p, action, &tokens[tpos],
                                       n_tokens - tpos, instr, data);

  if (!strcmp(tokens[tpos], "jmph"))
    return instr_jmp_hit_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                   instr, data);

  if (!strcmp(tokens[tpos], "jmpnh"))
    return instr_jmp_miss_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                    instr, data);

  if (!strcmp(tokens[tpos], "jmpa"))
    return instr_jmp_action_hit_translate(p, action, &tokens[tpos],
                                          n_tokens - tpos, instr, data);

  if (!strcmp(tokens[tpos], "jmpna"))
    return instr_jmp_action_miss_translate(p, action, &tokens[tpos],
                                           n_tokens - tpos, instr, data);

  if (!strcmp(tokens[tpos], "jmpeq"))
    return instr_jmp_eq_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                  instr, data);

  if (!strcmp(tokens[tpos], "jmpneq"))
    return instr_jmp_neq_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                   instr, data);

  if (!strcmp(tokens[tpos], "jmplt"))
    return instr_jmp_lt_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                  instr, data);

  if (!strcmp(tokens[tpos], "jmpgt"))
    return instr_jmp_gt_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                  instr, data);

  if (!strcmp(tokens[tpos], "return"))
    return instr_return_translate(p, action, &tokens[tpos], n_tokens - tpos,
                                  instr, data);

  return -EINVAL;
}

static struct instruction_data *label_find(struct instruction_data *data,
...
static uint32_t label_is_used(struct instruction_data *data, uint32_t n,
                              const char *label) {
  uint32_t count = 0, i;
...
static int instr_label_check(struct instruction_data *instruction_data,
  ...
  /* Check that no jump instruction (either conditional or not) can jump to
   * itself (loop). */
  for (i = 0; i < n_instructions; i++) {
    struct instruction_data *data = &instruction_data[i];
    char *label = data->label;
  ...
  for (i = 0; i < n_instructions; i++) {
    ...
    data->n_users = label_is_used(instruction_data, n_instructions, label);
  }

  return 0;
}

static int instr_jmp_resolve(struct instruction *instructions,
  ...
  for (i = 0; i < n_instructions; i++) {
    ...
    found = label_find(instruction_data, n_instructions, data->jmp_label);
    CHECK(found, EINVAL);
...
static int instr_verify(struct rte_swx_pipeline *p __rte_unused,
                        struct action *a, struct instruction *instr,
                        struct instruction_data *data __rte_unused,
                        uint32_t n_instructions) {
  if (!a) {
...
static uint32_t instr_compact(struct instruction *instructions,
                              struct instruction_data *instruction_data,
                              uint32_t n_instructions) {
  uint32_t i, pos = 0;
...
static int instr_pattern_extract_many_search(struct instruction *instr,
                                             struct instruction_data *data,
                                             uint32_t n_instr,
...
static void instr_pattern_extract_many_replace(struct instruction *instr,
                                               struct instruction_data *data,
                                               uint32_t n_instr) {
  uint32_t i;
...
instr_pattern_extract_many_optimize(struct instruction *instructions,
  ...
  for (i = 0; i < n_instructions;) {
    ...
    /* Extract many. */
    detected = instr_pattern_extract_many_search(instr, data,
                                                 n_instructions - i, &n_instr);
    if (detected) {
      instr_pattern_extract_many_replace(instr, data, n_instr);
      i += n_instr;
      continue;
    }
  ...
  /* Eliminate the invalid instructions that have been optimized out. */
  n_instructions =
      instr_compact(instructions, instruction_data, n_instructions);

  return n_instructions;
}

static int instr_pattern_emit_many_tx_search(struct instruction *instr,
                                             struct instruction_data *data,
                                             uint32_t n_instr,
                                             uint32_t *n_pattern_instr) {
  uint32_t i;
...
static void instr_pattern_emit_many_tx_replace(struct instruction *instr,
                                               struct instruction_data *data,
                                               uint32_t n_instr) {
  uint32_t i;
...
instr_pattern_emit_many_tx_optimize(struct instruction *instructions,
  ...
  for (i = 0; i < n_instructions;) {
    ...
    /* Emit many + TX. */
    detected = instr_pattern_emit_many_tx_search(instr, data,
                                                 n_instructions - i, &n_instr);
    if (detected) {
      instr_pattern_emit_many_tx_replace(instr, data, n_instr);
      i += n_instr;
      continue;
    }
...
static uint32_t action_arg_src_mov_count(
    struct action *a, uint32_t arg_id, struct instruction *instructions,
    struct instruction_data *instruction_data, uint32_t n_instructions);

static int instr_pattern_mov_all_validate_search(
    struct rte_swx_pipeline *p, struct action *a, struct instruction *instr,
    struct instruction_data *data, uint32_t n_instr,
    struct instruction *instructions, struct instruction_data *instruction_data,
    uint32_t n_instructions, uint32_t *n_pattern_instr) {
  struct header *h;
  ...
  for (i = 1; i < h->st->n_fields; i++)
    if (data[i].invalid || data[i].n_users || (instr[i].type != INSTR_MOV_HM) ||
        ...
        (instr[i].mov.src.offset !=
         a->st->fields[src_field_id + i].offset / 8) ||
  ...
  for (j = src_field_id; j < src_field_id + h->st->n_fields; j++) {
    ...
    n_users = action_arg_src_mov_count(a, j, instructions, instruction_data,
                                       n_instructions);
    if (n_users > 1)
      return 0;
  }
...
static void instr_pattern_mov_all_validate_replace(
    struct rte_swx_pipeline *p, struct action *a, struct instruction *instr,
    struct instruction_data *data, uint32_t n_instr) {
  struct header *h;
...
static uint32_t instr_pattern_mov_all_validate_optimize(
    struct rte_swx_pipeline *p, struct action *a,
    struct instruction *instructions, struct instruction_data *instruction_data,
    uint32_t n_instructions) {
  uint32_t i;
  ...
  for (i = 0; i < n_instructions;) {
    ...
    /* Mov all + validate. */
    detected = instr_pattern_mov_all_validate_search(
        p, a, instr, data, n_instructions - i, instructions, instruction_data,
...
static int instr_pattern_dma_many_search(struct instruction *instr,
                                         struct instruction_data *data,
                                         uint32_t n_instr,
                                         uint32_t *n_pattern_instr) {
  uint32_t i;
...
static void instr_pattern_dma_many_replace(struct instruction *instr,
                                           struct instruction_data *data,
                                           uint32_t n_instr) {
  uint32_t i;
...
instr_pattern_dma_many_optimize(struct instruction *instructions,
  ...
  for (i = 0; i < n_instructions;) {
    ...
    /* DMA many. */
    detected = instr_pattern_dma_many_search(instr, data, n_instructions - i,
                                             &n_instr);
    if (detected) {
...
static uint32_t instr_optimize(struct rte_swx_pipeline *p, struct action *a,
                               struct instruction *instructions,
                               struct instruction_data *instruction_data,
                               uint32_t n_instructions) {
  /* Extract many. */
  n_instructions = instr_pattern_extract_many_optimize(
      instructions, instruction_data, n_instructions);

  /* Emit many + TX. */
  n_instructions = instr_pattern_emit_many_tx_optimize(
      instructions, instruction_data, n_instructions);

  /* Mov all + validate. */
  n_instructions = instr_pattern_mov_all_validate_optimize(
      p, a, instructions, instruction_data, n_instructions);

  /* DMA many. */
  n_instructions = instr_pattern_dma_many_optimize(
      instructions, instruction_data, n_instructions);

  return n_instructions;
}

static int instruction_config(struct rte_swx_pipeline *p, struct action *a,
...
static int instruction_table_build(struct rte_swx_pipeline *p) {
...
static void instruction_table_build_free(struct rte_swx_pipeline *p) {
  if (!p->instruction_table)
    return;
...
static void instruction_table_free(struct rte_swx_pipeline *p) {
  instruction_table_build_free(p);
}

static inline void instr_exec(struct rte_swx_pipeline *p) {
  struct thread *t = &p->threads[p->thread_id];
  struct instruction *ip = t->ip;
 ...
 * Action.
 */
static struct action *action_find(struct rte_swx_pipeline *p,
                                  const char *name) {
  struct action *elem;
...
static struct action *action_find_by_id(struct rte_swx_pipeline *p,
                                        uint32_t id) {
...
static struct field *action_field_find(struct action *a, const char *name) {
...
static int action_does_learning(struct action *a) {
  uint32_t i;
...
int rte_swx_pipeline_action_config(struct rte_swx_pipeline *p, const char *name,
...
static int action_build(struct rte_swx_pipeline *p) {
  struct action *action;
...
static void action_build_free(struct rte_swx_pipeline *p) {
  free(p->action_funcs);
  p->action_funcs = NULL;
...
static void action_free(struct rte_swx_pipeline *p) {
  action_build_free(p);
...
static uint32_t action_arg_src_mov_count(
  ...
  if (!a->st || (arg_id >= a->st->n_fields) || !instructions ||
      !instruction_data || !n_instructions)
    return 0;
  ...
  for (i = 0; i < n_instructions; i++) {
    ...
    if (data->invalid ||
        ...
        instr->mov.src.struct_id || (instr->mov.src.offset != offset))
      continue;
 ...
 * Table.
 */
static struct table_type *table_type_find(struct rte_swx_pipeline *p,
                                          const char *name) {
...
static struct table *table_find_by_id(struct rte_swx_pipeline *p, uint32_t id) {
...
int rte_swx_pipeline_table_type_register(
    struct rte_swx_pipeline *p, const char *name,
...
static int table_match_type_resolve(struct rte_swx_match_field_params *fields,
                                    uint32_t n_fields,
                                    enum rte_swx_table_match_type *match_type) {
  ...
  if ((n_fields_lpm > 1) || (n_fields_lpm && (n_fields_em != n_fields - 1)))
    return -EINVAL;

  *match_type = (n_fields_em == n_fields) ? RTE_SWX_TABLE_MATCH_EXACT
...
int rte_swx_pipeline_table_config(struct rte_swx_pipeline *p, const char *name,
                                  struct rte_swx_pipeline_table_params *params,
                                  const char *recommended_table_type_name,
                                  const char *args, uint32_t size) {
  ...
  for (i = 0; i < p->n_actions; i++)
    if (!strcmp(params->action_names[i], params->default_action_name))
      break;
  CHECK(i < params->n_actions, EINVAL);
  CHECK(!params->action_is_for_default_entry ||
            params->action_is_for_default_entry[i],
        EINVAL);
  ...
  if (params->n_fields) {
    ...
    status =
        table_match_type_resolve(params->fields, params->n_fields, &match_type);
    if (status)
      return status;
  ...
  for (i = 0; i < params->n_fields; i++) {
    ...
    f->field = header ? header_field_parse(p, field->name, NULL)
  ...
  if (default_action->st)
    memcpy(t->default_action_data, params->default_action_data,
...
static struct rte_swx_table_params *table_params_get(struct table *table) {
...
static void table_params_free(struct rte_swx_table_params *params) {
  if (!params)
    return;
...
static int table_stub_lkp(void *table __rte_unused, void *mailbox __rte_unused,
                          uint8_t **key __rte_unused,
                          uint64_t *action_id __rte_unused,
                          uint8_t **action_data __rte_unused, int *hit) {
  *hit = 0;
  return 1; /* DONE. */
}

static int table_build(struct rte_swx_pipeline *p) {
  uint32_t i;
  ...
  for (i = 0; i < RTE_SWX_PIPELINE_THREADS_MAX; i++) {
    ...
    TAILQ_FOREACH(table, &p->tables, node) {
      ...
      if (table->type) {
        ...
        /* r->key. */
        r->key = table->header ? &t->structs[table->header->struct_id]
...
static void table_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static void table_free(struct rte_swx_pipeline *p) {
  table_build_free(p);
...
static struct selector *selector_find_by_id(struct rte_swx_pipeline *p,
                                            uint32_t id) {
...
int rte_swx_pipeline_selector_config(
    struct rte_swx_pipeline *p, const char *name,
    struct rte_swx_pipeline_selector_params *params) {
  struct selector *s;
  ...
  s->selector_fields =
      calloc(params->n_selector_fields, sizeof(struct field *));
...
static void selector_params_free(struct rte_swx_table_selector_params *params) {
  if (!params)
    return;
...
static void selector_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static int selector_build(struct rte_swx_pipeline *p) {
  uint32_t i;
  int status = 0;

  /* Per pipeline: selector statistics. */
  p->selector_stats =
      calloc(p->n_selectors, sizeof(struct selector_statistics));
  if (!p->selector_stats) {
    status = -ENOMEM;
    goto error;
  }
...
static void selector_free(struct rte_swx_pipeline *p) {
  selector_build_free(p);
...
static struct learner *learner_find_by_id(struct rte_swx_pipeline *p,
                                          uint32_t id) {
...
static int learner_action_args_check(struct rte_swx_pipeline *p,
  ...
  /* Check that there are enough meta-data fields, starting with the mf_name
   * field, to cover all the action arguments.
   */
  mf_pos = mf - mst->fields;
  CHECK(mst->n_fields - mf_pos >= ast->n_fields, EINVAL);

  /* Check that the size of each of the identified meta-data fields matches
   * exactly the size of the corresponding action argument.
   */
...
static int learner_action_learning_check(struct rte_swx_pipeline *p,
                                         struct action *action,
                                         const char **action_names,
                                         uint32_t n_actions) {
  uint32_t i;

  /* For each "learn" instruction of the current action, check that the learned
   * action (i.e. the action passed as argument to the "learn" instruction) is
   * also enabled for the current learner table.
   */
...
int rte_swx_pipeline_learner_config(
    struct rte_swx_pipeline *p, const char *name,
    struct rte_swx_pipeline_learner_params *params, uint32_t size,
    uint32_t timeout) {
  struct learner *l = NULL;
  struct action *default_action;
  struct header *header = NULL;
  ...
  for (i = 0; i < params->n_actions; i++) {
    ...
    status = learner_action_learning_check(p, a, params->action_names,
                                           params->n_actions);
    if (status)
      return status;
  ...
  for (i = 0; i < params->n_fields; i++) {
    ...
    l->fields[i] = header ? header_field_parse(p, field_name, NULL)
                          : metadata_field_parse(p, field_name);
  }
  ...
  if (default_action->st)
    memcpy(l->default_action_data, params->default_action_data,
           default_action->st->n_bits / 8);
...
static void learner_params_free(struct rte_swx_table_learner_params *params) {
  if (!params)
    return;
...
static void learner_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static int learner_build(struct rte_swx_pipeline *p) {
  uint32_t i;
  int status = 0;
  ...
  for (i = 0; i < RTE_SWX_PIPELINE_THREADS_MAX; i++) {
    ...
    TAILQ_FOREACH(l, &p->learners, node) {
      ...
      /* r->key. */
      r->key = l->header ? &t->structs[l->header->struct_id]
                         : &t->structs[p->metadata_struct_id];
    }
  }
...
static void learner_free(struct rte_swx_pipeline *p) {
  learner_build_free(p);
 ...
 * Table state.
 */
static int table_state_build(struct rte_swx_pipeline *p) {
  struct table *table;
  struct selector *s;
  struct learner *l;
  ...
  TAILQ_FOREACH(table, &p->tables, node) {
    ...
    if (table->type) {
      ...
      ts->obj =
          table->type->ops.create(params, NULL, table->args, p->numa_node);
    ...
    if (table->action_data_size_max) {
      ts->default_action_data = malloc(table->action_data_size_max);
      CHECK(ts->default_action_data, ENOMEM);

      memcpy(ts->default_action_data, table->default_action_data,
  ...
  TAILQ_FOREACH(l, &p->learners, node) {
    struct rte_swx_table_state *ts =
        &p->table_state[p->n_tables + p->n_selectors + l->id];
    ...
    if (l->action_data_size_max) {
      ...
      memcpy(ts->default_action_data, l->default_action_data,
...
static void table_state_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
  ...
  for (i = 0; i < p->n_learners; i++) {
    struct rte_swx_table_state *ts =
        &p->table_state[p->n_tables + p->n_selectors + i];
...
static void table_state_free(struct rte_swx_pipeline *p) {
...
static struct regarray *regarray_find_by_id(struct rte_swx_pipeline *p,
                                            uint32_t id) {
...
int rte_swx_pipeline_regarray_config(struct rte_swx_pipeline *p,
...
static int regarray_build(struct rte_swx_pipeline *p) {
  ...
  TAILQ_FOREACH(regarray, &p->regarrays, node) {
    ...
    r->regarray = env_malloc(regarray->size * sizeof(uint64_t),
                             RTE_CACHE_LINE_SIZE, p->numa_node);
...
static void regarray_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static void regarray_free(struct rte_swx_pipeline *p) {
 ...
 * Meter array.
 */
static struct meter_profile *meter_profile_find(struct rte_swx_pipeline *p,
                                                const char *name) {
...
static struct metarray *metarray_find_by_id(struct rte_swx_pipeline *p,
                                            uint32_t id) {
...
int rte_swx_pipeline_metarray_config(struct rte_swx_pipeline *p,
                                     const char *name, uint32_t size) {
  struct metarray *m;
...
static void meter_init(struct meter *m) {
...
static int metarray_build(struct rte_swx_pipeline *p) {
  struct metarray *m;
...
static void metarray_build_free(struct rte_swx_pipeline *p) {
  uint32_t i;
...
static void metarray_free(struct rte_swx_pipeline *p) {
 ...
 * Pipeline.
 */
void rte_swx_pipeline_free(struct rte_swx_pipeline *p) {
  void *lib;
...
static int port_in_types_register(struct rte_swx_pipeline *p) {
  int status;

  status = rte_swx_pipeline_port_in_type_register(
      p, "ethdev", &rte_swx_port_ethdev_reader_ops);
  if (status)
    return status;

  status = rte_swx_pipeline_port_in_type_register(
      p, "ring", &rte_swx_port_ring_reader_ops);
  if (status)
    return status;

#ifdef RTE_PORT_PCAP
  status = rte_swx_pipeline_port_in_type_register(p, "source",
                                                  &rte_swx_port_source_ops);
  if (status)
    return status;
#endif

  status = rte_swx_pipeline_port_in_type_register(p, "fd",
...
static int port_out_types_register(struct rte_swx_pipeline *p) {
  int status;

  status = rte_swx_pipeline_port_out_type_register(
      p, "ethdev", &rte_swx_port_ethdev_writer_ops);
  if (status)
    return status;

  status = rte_swx_pipeline_port_out_type_register(
      p, "ring", &rte_swx_port_ring_writer_ops);
  if (status)
    return status;

  status = rte_swx_pipeline_port_out_type_register(p, "sink",
                                                   &rte_swx_port_sink_ops);
  if (status)
    return status;

  status = rte_swx_pipeline_port_out_type_register(p, "fd",
...
static int table_types_register(struct rte_swx_pipeline *p) {
  int status;

  status = rte_swx_pipeline_table_type_register(
      p, "exact", RTE_SWX_TABLE_MATCH_EXACT, &rte_swx_table_exact_match_ops);
  if (status)
    return status;
...
int rte_swx_pipeline_config(struct rte_swx_pipeline **p, int numa_node) {
...
int rte_swx_pipeline_instructions_config(struct rte_swx_pipeline *p,
                                         const char **instructions,
                                         uint32_t n_instructions) {
  int err;
  uint32_t i;
...
static int pipeline_compile(struct rte_swx_pipeline *p);

int rte_swx_pipeline_build(struct rte_swx_pipeline *p) {
  ...
  /* Drop port. */
  status = rte_swx_pipeline_port_out_config(p, p->n_ports_out, "sink",
                                            &drop_port_params);
  if (status)
    goto error;
...
void rte_swx_pipeline_run(struct rte_swx_pipeline *p, uint32_t n_instructions) {
  uint32_t i;
...
void rte_swx_pipeline_flush(struct rte_swx_pipeline *p) {
  uint32_t i;
 ...
 * Control.
 */
int rte_swx_ctl_pipeline_info_get(struct rte_swx_pipeline *p,
...
int rte_swx_ctl_pipeline_numa_node_get(struct rte_swx_pipeline *p,
                                       int *numa_node) {
  if (!p || !numa_node)
    return -EINVAL;
...
int rte_swx_ctl_action_info_get(struct rte_swx_pipeline *p, uint32_t action_id,
...
int rte_swx_ctl_action_arg_info_get(
    struct rte_swx_pipeline *p, uint32_t action_id, uint32_t action_arg_id,
...
int rte_swx_ctl_table_info_get(struct rte_swx_pipeline *p, uint32_t table_id,
...
int rte_swx_ctl_table_match_field_info_get(
    struct rte_swx_pipeline *p, uint32_t table_id, uint32_t match_field_id,
...
int rte_swx_ctl_table_action_info_get(
    struct rte_swx_pipeline *p, uint32_t table_id, uint32_t table_action_id,
  ...
  table_action->action_is_for_table_entries =
      t->action_is_for_table_entries[table_action_id];
  table_action->action_is_for_default_entry =
      t->action_is_for_default_entry[table_action_id];

  return 0;
}

int rte_swx_ctl_table_ops_get(struct rte_swx_pipeline *p, uint32_t table_id,
...
int rte_swx_ctl_selector_info_get(struct rte_swx_pipeline *p,
...
int rte_swx_ctl_selector_group_id_field_info_get(
    struct rte_swx_pipeline *p, uint32_t selector_id,
...
int rte_swx_ctl_selector_field_info_get(
    struct rte_swx_pipeline *p, uint32_t selector_id,
...
int rte_swx_ctl_learner_info_get(struct rte_swx_pipeline *p,
                                 uint32_t learner_id,
...
int rte_swx_ctl_learner_match_field_info_get(
    struct rte_swx_pipeline *p, uint32_t learner_id, uint32_t match_field_id,
    struct rte_swx_ctl_table_match_field_info *match_field) {
  struct learner *l;
  struct field *f;
...
int rte_swx_ctl_learner_action_info_get(
    struct rte_swx_pipeline *p, uint32_t learner_id, uint32_t learner_action_id,
...
int rte_swx_pipeline_table_state_get(struct rte_swx_pipeline *p,
...
int rte_swx_pipeline_table_state_set(struct rte_swx_pipeline *p,
                                     struct rte_swx_table_state *table_state) {
  if (!p || !table_state || !p->build_done)
    return -EINVAL;
...
int rte_swx_ctl_pipeline_port_in_stats_read(
    struct rte_swx_pipeline *p, uint32_t port_id,
...
int rte_swx_ctl_pipeline_table_stats_read(struct rte_swx_pipeline *p,
...
int rte_swx_ctl_pipeline_selector_stats_read(
    struct rte_swx_pipeline *p, const char *selector_name,
...
int rte_swx_ctl_pipeline_learner_stats_read(
    struct rte_swx_pipeline *p, const char *learner_name,
    struct rte_swx_learner_stats *stats) {
  struct learner *l;
  struct learner_statistics *learner_stats;

  if (!p || !learner_name || !learner_name[0] || !stats ||
      !stats->n_pkts_action)
    return -EINVAL;
  ...
  memcpy(stats->n_pkts_action, learner_stats->n_pkts_action,
         p->n_actions * sizeof(uint64_t));
...
int rte_swx_ctl_regarray_info_get(struct rte_swx_pipeline *p,
...
int rte_swx_ctl_pipeline_regarray_read(struct rte_swx_pipeline *p,
...
int rte_swx_ctl_pipeline_regarray_write(struct rte_swx_pipeline *p,
                                        const char *regarray_name,
                                        uint32_t regarray_index,
                                        uint64_t value) {
  struct regarray *regarray;
...
int rte_swx_ctl_metarray_info_get(struct rte_swx_pipeline *p,
...
int rte_swx_ctl_meter_profile_add(struct rte_swx_pipeline *p, const char *name,
...
int rte_swx_ctl_meter_profile_delete(struct rte_swx_pipeline *p,
                                     const char *name) {
  struct meter_profile *mp;
...
int rte_swx_ctl_meter_reset(struct rte_swx_pipeline *p,
...
int rte_swx_ctl_meter_set(struct rte_swx_pipeline *p, const char *metarray_name,
...
int rte_swx_ctl_meter_stats_read(struct rte_swx_pipeline *p,
                                 const char *metarray_name,
                                 uint32_t metarray_index,
 ...
 */
static const char *instr_type_to_name(struct instruction *instr) {
  switch (instr->type) {
  case INSTR_RX:
    return "INSTR_RX";
  ...
  case INSTR_HDR_EXTRACT:
    return "INSTR_HDR_EXTRACT";
  case INSTR_HDR_EXTRACT2:
    return "INSTR_HDR_EXTRACT2";
  case INSTR_HDR_EXTRACT3:
    return "INSTR_HDR_EXTRACT3";
  case INSTR_HDR_EXTRACT4:
    return "INSTR_HDR_EXTRACT4";
  case INSTR_HDR_EXTRACT5:
    return "INSTR_HDR_EXTRACT5";
  case INSTR_HDR_EXTRACT6:
    return "INSTR_HDR_EXTRACT6";
  case INSTR_HDR_EXTRACT7:
    return "INSTR_HDR_EXTRACT7";
  case INSTR_HDR_EXTRACT8:
    return "INSTR_HDR_EXTRACT8";

  case INSTR_HDR_EXTRACT_M:
    return "INSTR_HDR_EXTRACT_M";

  case INSTR_HDR_LOOKAHEAD:
    return "INSTR_HDR_LOOKAHEAD";

  case INSTR_HDR_EMIT:
    return "INSTR_HDR_EMIT";
  case INSTR_HDR_EMIT_TX:
    return "INSTR_HDR_EMIT_TX";
  case INSTR_HDR_EMIT2_TX:
    return "INSTR_HDR_EMIT2_TX";
  case INSTR_HDR_EMIT3_TX:
    return "INSTR_HDR_EMIT3_TX";
  case INSTR_HDR_EMIT4_TX:
    return "INSTR_HDR_EMIT4_TX";
  case INSTR_HDR_EMIT5_TX:
    return "INSTR_HDR_EMIT5_TX";
  case INSTR_HDR_EMIT6_TX:
    return "INSTR_HDR_EMIT6_TX";
  case INSTR_HDR_EMIT7_TX:
    return "INSTR_HDR_EMIT7_TX";
  case INSTR_HDR_EMIT8_TX:
    return "INSTR_HDR_EMIT8_TX";

  case INSTR_HDR_VALIDATE:
    return "INSTR_HDR_VALIDATE";
  case INSTR_HDR_INVALIDATE:
    return "INSTR_HDR_INVALIDATE";
  ...
  case INSTR_DMA_HT:
    return "INSTR_DMA_HT";
  case INSTR_DMA_HT2:
    return "INSTR_DMA_HT2";
  case INSTR_DMA_HT3:
    return "INSTR_DMA_HT3";
  case INSTR_DMA_HT4:
    return "INSTR_DMA_HT4";
  case INSTR_DMA_HT5:
    return "INSTR_DMA_HT5";
  case INSTR_DMA_HT6:
    return "INSTR_DMA_HT6";
  case INSTR_DMA_HT7:
    return "INSTR_DMA_HT7";
  case INSTR_DMA_HT8:
    return "INSTR_DMA_HT8";

  case INSTR_ALU_ADD:
    return "INSTR_ALU_ADD";
  case INSTR_ALU_ADD_MH:
    return "INSTR_ALU_ADD_MH";
  case INSTR_ALU_ADD_HM:
    return "INSTR_ALU_ADD_HM";
  case INSTR_ALU_ADD_HH:
    return "INSTR_ALU_ADD_HH";
  case INSTR_ALU_ADD_MI:
    return "INSTR_ALU_ADD_MI";
  case INSTR_ALU_ADD_HI:
    return "INSTR_ALU_ADD_HI";

  case INSTR_ALU_SUB:
    return "INSTR_ALU_SUB";
  case INSTR_ALU_SUB_MH:
    return "INSTR_ALU_SUB_MH";
  case INSTR_ALU_SUB_HM:
    return "INSTR_ALU_SUB_HM";
  case INSTR_ALU_SUB_HH:
    return "INSTR_ALU_SUB_HH";
  case INSTR_ALU_SUB_MI:
    return "INSTR_ALU_SUB_MI";
  case INSTR_ALU_SUB_HI:
    return "INSTR_ALU_SUB_HI";

  case INSTR_ALU_CKADD_FIELD:
    return "INSTR_ALU_CKADD_FIELD";
  case INSTR_ALU_CKADD_STRUCT20:
    return "INSTR_ALU_CKADD_STRUCT20";
  case INSTR_ALU_CKADD_STRUCT:
    return "INSTR_ALU_CKADD_STRUCT";
  case INSTR_ALU_CKSUB_FIELD:
    return "INSTR_ALU_CKSUB_FIELD";

  case INSTR_ALU_AND:
    return "INSTR_ALU_AND";
  case INSTR_ALU_AND_MH:
    return "INSTR_ALU_AND_MH";
  case INSTR_ALU_AND_HM:
    return "INSTR_ALU_AND_HM";
  case INSTR_ALU_AND_HH:
    return "INSTR_ALU_AND_HH";
  case INSTR_ALU_AND_I:
    return "INSTR_ALU_AND_I";

  case INSTR_ALU_OR:
    return "INSTR_ALU_OR";
  case INSTR_ALU_OR_MH:
    return "INSTR_ALU_OR_MH";
  case INSTR_ALU_OR_HM:
    return "INSTR_ALU_OR_HM";
  case INSTR_ALU_OR_HH:
    return "INSTR_ALU_OR_HH";
  case INSTR_ALU_OR_I:
    return "INSTR_ALU_OR_I";

  case INSTR_ALU_XOR:
    return "INSTR_ALU_XOR";
  case INSTR_ALU_XOR_MH:
    return "INSTR_ALU_XOR_MH";
  case INSTR_ALU_XOR_HM:
    return "INSTR_ALU_XOR_HM";
  case INSTR_ALU_XOR_HH:
    return "INSTR_ALU_XOR_HH";
  case INSTR_ALU_XOR_I:
    return "INSTR_ALU_XOR_I";

  case INSTR_ALU_SHL:
    return "INSTR_ALU_SHL";
  case INSTR_ALU_SHL_MH:
    return "INSTR_ALU_SHL_MH";
  case INSTR_ALU_SHL_HM:
    return "INSTR_ALU_SHL_HM";
  case INSTR_ALU_SHL_HH:
    return "INSTR_ALU_SHL_HH";
  case INSTR_ALU_SHL_MI:
    return "INSTR_ALU_SHL_MI";
  case INSTR_ALU_SHL_HI:
    return "INSTR_ALU_SHL_HI";

  case INSTR_ALU_SHR:
    return "INSTR_ALU_SHR";
  case INSTR_ALU_SHR_MH:
    return "INSTR_ALU_SHR_MH";
  case INSTR_ALU_SHR_HM:
    return "INSTR_ALU_SHR_HM";
  case INSTR_ALU_SHR_HH:
    return "INSTR_ALU_SHR_HH";
  case INSTR_ALU_SHR_MI:
    return "INSTR_ALU_SHR_MI";
  case INSTR_ALU_SHR_HI:
    return "INSTR_ALU_SHR_HI";

  case INSTR_REGPREFETCH_RH:
    return "INSTR_REGPREFETCH_RH";
  case INSTR_REGPREFETCH_RM:
    return "INSTR_REGPREFETCH_RM";
  case INSTR_REGPREFETCH_RI:
    return "INSTR_REGPREFETCH_RI";

  case INSTR_REGRD_HRH:
    return "INSTR_REGRD_HRH";
  case INSTR_REGRD_HRM:
    return "INSTR_REGRD_HRM";
  case INSTR_REGRD_HRI:
    return "INSTR_REGRD_HRI";
  case INSTR_REGRD_MRH:
    return "INSTR_REGRD_MRH";
  case INSTR_REGRD_MRM:
    return "INSTR_REGRD_MRM";
  case INSTR_REGRD_MRI:
    return "INSTR_REGRD_MRI";

  case INSTR_REGWR_RHH:
    return "INSTR_REGWR_RHH";
  case INSTR_REGWR_RHM:
    return "INSTR_REGWR_RHM";
  case INSTR_REGWR_RHI:
    return "INSTR_REGWR_RHI";
  case INSTR_REGWR_RMH:
    return "INSTR_REGWR_RMH";
  case INSTR_REGWR_RMM:
    return "INSTR_REGWR_RMM";
  case INSTR_REGWR_RMI:
    return "INSTR_REGWR_RMI";
  case INSTR_REGWR_RIH:
    return "INSTR_REGWR_RIH";
  case INSTR_REGWR_RIM:
    return "INSTR_REGWR_RIM";
  case INSTR_REGWR_RII:
    return "INSTR_REGWR_RII";

  case INSTR_REGADD_RHH:
    return "INSTR_REGADD_RHH";
  case INSTR_REGADD_RHM:
    return "INSTR_REGADD_RHM";
  case INSTR_REGADD_RHI:
    return "INSTR_REGADD_RHI";
  case INSTR_REGADD_RMH:
    return "INSTR_REGADD_RMH";
  case INSTR_REGADD_RMM:
    return "INSTR_REGADD_RMM";
  case INSTR_REGADD_RMI:
    return "INSTR_REGADD_RMI";
  case INSTR_REGADD_RIH:
    return "INSTR_REGADD_RIH";
  case INSTR_REGADD_RIM:
    return "INSTR_REGADD_RIM";
  case INSTR_REGADD_RII:
    return "INSTR_REGADD_RII";

  case INSTR_METPREFETCH_H:
    return "INSTR_METPREFETCH_H";
  case INSTR_METPREFETCH_M:
    return "INSTR_METPREFETCH_M";
  case INSTR_METPREFETCH_I:
    return "INSTR_METPREFETCH_I";

  case INSTR_METER_HHM:
    return "INSTR_METER_HHM";
  case INSTR_METER_HHI:
    return "INSTR_METER_HHI";
  case INSTR_METER_HMM:
    return "INSTR_METER_HMM";
  case INSTR_METER_HMI:
    return "INSTR_METER_HMI";
  case INSTR_METER_MHM:
    return "INSTR_METER_MHM";
  case INSTR_METER_MHI:
    return "INSTR_METER_MHI";
  case INSTR_METER_MMM:
    return "INSTR_METER_MMM";
  case INSTR_METER_MMI:
    return "INSTR_METER_MMI";
  case INSTR_METER_IHM:
    return "INSTR_METER_IHM";
  case INSTR_METER_IHI:
    return "INSTR_METER_IHI";
  case INSTR_METER_IMM:
    return "INSTR_METER_IMM";
  case INSTR_METER_IMI:
    return "INSTR_METER_IMI";

  case INSTR_TABLE:
    return "INSTR_TABLE";
  case INSTR_TABLE_AF:
    return "INSTR_TABLE_AF";
  case INSTR_SELECTOR:
    return "INSTR_SELECTOR";
  case INSTR_LEARNER:
    return "INSTR_LEARNER";
  case INSTR_LEARNER_AF:
    return "INSTR_LEARNER_AF";

  case INSTR_LEARNER_LEARN:
    return "INSTR_LEARNER_LEARN";
  case INSTR_LEARNER_FORGET:
    return "INSTR_LEARNER_FORGET";

  case INSTR_EXTERN_OBJ:
    return "INSTR_EXTERN_OBJ";
  case INSTR_EXTERN_FUNC:
    return "INSTR_EXTERN_FUNC";

  case INSTR_JMP:
    return "INSTR_JMP";
  case INSTR_JMP_VALID:
    return "INSTR_JMP_VALID";
  case INSTR_JMP_INVALID:
    return "INSTR_JMP_INVALID";
  case INSTR_JMP_HIT:
    return "INSTR_JMP_HIT";
  case INSTR_JMP_MISS:
    return "INSTR_JMP_MISS";
  case INSTR_JMP_ACTION_HIT:
    return "INSTR_JMP_ACTION_HIT";
  case INSTR_JMP_ACTION_MISS:
    return "INSTR_JMP_ACTION_MISS";
  case INSTR_JMP_EQ:
    return "INSTR_JMP_EQ";
  case INSTR_JMP_EQ_MH:
    return "INSTR_JMP_EQ_MH";
  case INSTR_JMP_EQ_HM:
    return "INSTR_JMP_EQ_HM";
  case INSTR_JMP_EQ_HH:
    return "INSTR_JMP_EQ_HH";
  case INSTR_JMP_EQ_I:
    return "INSTR_JMP_EQ_I";
  case INSTR_JMP_NEQ:
    return "INSTR_JMP_NEQ";
  case INSTR_JMP_NEQ_MH:
    return "INSTR_JMP_NEQ_MH";
  case INSTR_JMP_NEQ_HM:
    return "INSTR_JMP_NEQ_HM";
  case INSTR_JMP_NEQ_HH:
    return "INSTR_JMP_NEQ_HH";
  case INSTR_JMP_NEQ_I:
    return "INSTR_JMP_NEQ_I";
  case INSTR_JMP_LT:
    return "INSTR_JMP_LT";
  case INSTR_JMP_LT_MH:
    return "INSTR_JMP_LT_MH";
  case INSTR_JMP_LT_HM:
    return "INSTR_JMP_LT_HM";
  case INSTR_JMP_LT_HH:
    return "INSTR_JMP_LT_HH";
  case INSTR_JMP_LT_MI:
    return "INSTR_JMP_LT_MI";
  case INSTR_JMP_LT_HI:
    return "INSTR_JMP_LT_HI";
  case INSTR_JMP_GT:
    return "INSTR_JMP_GT";
  case INSTR_JMP_GT_MH:
    return "INSTR_JMP_GT_MH";
  case INSTR_JMP_GT_HM:
    return "INSTR_JMP_GT_HM";
  case INSTR_JMP_GT_HH:
    return "INSTR_JMP_GT_HH";
  case INSTR_JMP_GT_MI:
    return "INSTR_JMP_GT_MI";
  case INSTR_JMP_GT_HI:
    return "INSTR_JMP_GT_HI";
...
typedef void (*instruction_export_t)(struct instruction *, FILE *);

static void instr_io_export(struct instruction *instr, FILE *f) {
  uint32_t n_io = 0, n_io_imm = 0, n_hdrs = 0, i;

  /* n_io, n_io_imm, n_hdrs. */
  if (instr->type == INSTR_RX || instr->type == INSTR_TX ||
      instr->type == INSTR_HDR_EXTRACT_M ||
  ...
  if (instr->type == INSTR_HDR_EXTRACT_M ||
      instr->type == INSTR_HDR_LOOKAHEAD || instr->type == INSTR_HDR_EMIT)
    n_hdrs = 1;
  ...
  if (n_io)
    fprintf(f,
            ...
            "\t\t\t\t.n_bits = %u,\n"
            "\t\t\t},\n",
            instr->io.io.offset, instr->io.io.n_bits);
  ...
  if (n_hdrs) {
    ...
    for (i = 0; i < n_hdrs; i++)
      fprintf(f, "%u, ", instr->io.hdr.header_id[i]);
    ...
    for (i = 0; i < n_hdrs; i++)
      fprintf(f, "%u, ", instr->io.hdr.struct_id[i]);
    ...
    for (i = 0; i < n_hdrs; i++)
      fprintf(f, "%u, ", instr->io.hdr.n_bytes[i]);
...
static void instr_hdr_validate_export(struct instruction *instr, FILE *f) {
  fprintf(f,
          "\t{\n"
          "\t\t.type = %s,\n"
          "\t\t.valid = {\n"
          ...
          "\t\t},\n"
          "\t},\n",
          instr_type_to_name(instr), instr->valid.header_id);
}

static void instr_mov_export(struct instruction *instr, FILE *f) {
  if (instr->type != INSTR_MOV_I)
    fprintf(f,
            ...
            "\t\t},\n"
            "\t},\n",
            instr_type_to_name(instr), instr->mov.dst.struct_id,
            instr->mov.dst.n_bits, instr->mov.dst.offset,
            instr->mov.src.struct_id, instr->mov.src.n_bits,
    ...
    fprintf(f,
            ...
            "\t},\n",
            instr_type_to_name(instr), instr->mov.dst.struct_id,
            instr->mov.dst.n_bits, instr->mov.dst.offset, instr->mov.src_val);
}

static void instr_dma_ht_export(struct instruction *instr, FILE *f) {
  uint32_t n_dma = 0, i;
  ...
  for (i = 0; i < n_dma; i++)
    fprintf(f, "%u, ", instr->dma.dst.header_id[i]);
  ...
  for (i = 0; i < n_dma; i++)
    fprintf(f, "%u, ", instr->dma.dst.struct_id[i]);
  ...
  for (i = 0; i < n_dma; i++)
    fprintf(f, "%u, ", instr->dma.src.offset[i]);
  ...
  for (i = 0; i < n_dma; i++)
    fprintf(f, "%u, ", instr->dma.n_bytes[i]);
...
static void instr_alu_export(struct instruction *instr, FILE *f) {
  int imm = 0;

  if (instr->type == INSTR_ALU_ADD_MI || instr->type == INSTR_ALU_ADD_HI ||
      instr->type == INSTR_ALU_SUB_MI || instr->type == INSTR_ALU_SUB_HI ||
      instr->type == INSTR_ALU_SHL_MI || instr->type == INSTR_ALU_SHL_HI ||
      instr->type == INSTR_ALU_SHR_MI || instr->type == INSTR_ALU_SHR_HI ||
      instr->type == INSTR_ALU_AND_I || instr->type == INSTR_ALU_OR_I ||
      instr->type == INSTR_ALU_XOR_I)
    imm = 1;
  ...
  if (!imm)
    fprintf(f,
            ...
            "\t\t},\n"
            "\t},\n",
            instr_type_to_name(instr), instr->alu.dst.struct_id,
            instr->alu.dst.n_bits, instr->alu.dst.offset,
            instr->alu.src.struct_id, instr->alu.src.n_bits,
    ...
    fprintf(f,
            ...
            "\t},\n",
            instr_type_to_name(instr), instr->alu.dst.struct_id,
            instr->alu.dst.n_bits, instr->alu.dst.offset, instr->alu.src_val);
}

static void instr_reg_export(struct instruction *instr __rte_unused,
                             FILE *f __rte_unused) {
   ...
   */
  if (instr->type == INSTR_REGPREFETCH_RI || instr->type == INSTR_REGRD_HRI ||
      instr->type == INSTR_REGRD_MRI || instr->type == INSTR_REGWR_RIH ||
      instr->type == INSTR_REGWR_RIM || instr->type == INSTR_REGWR_RII ||
      instr->type == INSTR_REGADD_RIH || instr->type == INSTR_REGADD_RIM ||
      instr->type == INSTR_REGADD_RII)
    idx_imm = 1;

  /* src is the 3rd operand for the regwr and regadd instructions. */
  if (instr->type == INSTR_REGWR_RHI || instr->type == INSTR_REGWR_RMI ||
      instr->type == INSTR_REGWR_RII || instr->type == INSTR_REGADD_RHI ||
      instr->type == INSTR_REGADD_RMI || instr->type == INSTR_REGADD_RII)
    src_imm = 1;
  ...
  fprintf(f,
          ...
          "\t\t.regarray = {\n"
          "\t\t\t.regarray_id = %u,\n",
          instr_type_to_name(instr), instr->regarray.regarray_id);
  ...
  if (!idx_imm)
    fprintf(f,
            ...
            "\t\t\t\t\t.offset = %u,\n"
            "\t\t\t\t},\n",
            instr->regarray.idx.struct_id, instr->regarray.idx.n_bits,
  ...
  if (!prefetch) {
    if (!src_imm)
      fprintf(f,
              ...
              "\t\t\t\t\t.offset = %u,\n"
              "\t\t\t\t},\n",
              instr->regarray.dstsrc.struct_id, instr->regarray.dstsrc.n_bits,
              instr->regarray.dstsrc.offset);
    else
      fprintf(f, "\t\t\t\t.dstsrc_val = %" PRIu64 ",\n",
...
static void instr_meter_export(struct instruction *instr __rte_unused,
                               FILE *f __rte_unused) {
  int prefetch = 0, idx_imm = 0, color_in_imm = 0;

  if (instr->type == INSTR_METPREFETCH_H ||
      instr->type == INSTR_METPREFETCH_M || instr->type == INSTR_METPREFETCH_I)
    prefetch = 1;

  /* idx_imm. */
  if (instr->type == INSTR_METPREFETCH_I || instr->type == INSTR_METER_IHM ||
      instr->type == INSTR_METER_IHI || instr->type == INSTR_METER_IMM ||
      instr->type == INSTR_METER_IMI)
    idx_imm = 1;

  /* color_in_imm. */
  if (instr->type == INSTR_METER_HHI || instr->type == INSTR_METER_HMI ||
      instr->type == INSTR_METER_MHI || instr->type == INSTR_METER_MMI ||
      instr->type == INSTR_METER_IHI || instr->type == INSTR_METER_IMI)
    color_in_imm = 1;
  ...
  fprintf(f,
          ...
          "\t\t.meter = {\n"
          "\t\t\t.metarray_id = %u,\n",
          instr_type_to_name(instr), instr->meter.metarray_id);
  ...
  if (!idx_imm)
    fprintf(f,
            ...
            "\t\t\t\t.offset = %u,\n"
            "\t\t\t},\n",
            instr->meter.idx.struct_id, instr->meter.idx.n_bits,
  ...
  if (!prefetch) {
    ...
    fprintf(f,
            ...
            "\t\t\t\t.offset = %u,\n"
            "\t\t\t},\n",
            instr->meter.length.struct_id, instr->meter.length.n_bits,
    ...
    if (!color_in_imm)
      fprintf(f,
              ...
              "\t\t\t\t.offset = %u,\n"
              "\t\t\t},\n",
              instr->meter.color_in.struct_id, instr->meter.color_in.n_bits,
              instr->meter.color_in.offset);
    else
      fprintf(f, "\t\t\t.color_in_val = %u,\n",
    ...
    fprintf(f,
            ...
            "\t\t\t\t.offset = %u,\n"
            "\t\t\t},\n",
            instr->meter.color_out.struct_id, instr->meter.color_out.n_bits,
...
static void instr_table_export(struct instruction *instr, FILE *f) {
  fprintf(f,
          "\t{\n"
          "\t\t.type = %s,\n"
          "\t\t.table = {\n"
          ...
          "\t\t},\n"
          "\t},\n",
          instr_type_to_name(instr), instr->table.table_id);
}

static void instr_learn_export(struct instruction *instr, FILE *f) {
  fprintf(f,
          "\t{\n"
          "\t\t.type = %s,\n"
          "\t\t.learn = {\n"
          ...
          "\t\t},\n"
          "\t},\n",
          instr_type_to_name(instr), instr->learn.action_id);
}

static void instr_forget_export(struct instruction *instr, FILE *f) {
  fprintf(f,
          "\t{\n"
          "\t\t.type = %s,\n"
          "\t},\n",
...
static void instr_extern_export(struct instruction *instr, FILE *f) {
  if (instr->type == INSTR_EXTERN_OBJ)
    fprintf(f,
            ...
            "\t\t},\n"
            "\t},\n",
            instr_type_to_name(instr), instr->ext_obj.ext_obj_id,
    ...
    fprintf(f,
            ...
            "\t\t},\n"
            "\t},\n",
            instr_type_to_name(instr), instr->ext_func.ext_func_id);
}

static void instr_jmp_export(struct instruction *instr, FILE *f __rte_unused) {
  fprintf(f,
          "\t{\n"
          "\t\t.type = %s,\n"
          "\t\t.jmp = {\n"
  ...
  case INSTR_JMP_VALID:
  case INSTR_JMP_INVALID:
    fprintf(f, "\t\t\t.header_id = %u,\n", instr->jmp.header_id);
    break;

  case INSTR_JMP_ACTION_HIT:
  case INSTR_JMP_ACTION_MISS:
    fprintf(f, "\t\t\t.action_id = %u,\n", instr->jmp.action_id);
    break;
  ...
  case INSTR_JMP_GT_HH:
    fprintf(f,
            ...
            "\t\t\t\t.offset = %u,\n"
            "\t\t\t},\n",
            instr->jmp.a.struct_id, instr->jmp.a.n_bits, instr->jmp.a.offset,
            instr->jmp.b.struct_id, instr->jmp.b.n_bits, instr->jmp.b.offset);
    break;
...
static void instr_return_export(struct instruction *instr, FILE *f) {
  fprintf(f,
          "\t{\n"
          "\t\t.type = %s,\n",
          instr_type_to_name(instr));
...
static void action_data_codegen(struct action *a, FILE *f) {
  uint32_t i;

  fprintf(f, "static const struct instruction action_%s_instructions[] = {\n",
          a->name);
...
static const char *instr_type_to_func(struct instruction *instr) {
  switch (instr->type) {
  case INSTR_RX:
    return NULL;

  case INSTR_TX:
    return "__instr_tx_exec";
  case INSTR_TX_I:
    return "__instr_tx_i_exec";
  case INSTR_DROP:
    return "__instr_drop_exec";

  case INSTR_HDR_EXTRACT:
    return "__instr_hdr_extract_exec";
  case INSTR_HDR_EXTRACT2:
    return "__instr_hdr_extract2_exec";
  case INSTR_HDR_EXTRACT3:
    return "__instr_hdr_extract3_exec";
  case INSTR_HDR_EXTRACT4:
    return "__instr_hdr_extract4_exec";
  case INSTR_HDR_EXTRACT5:
    return "__instr_hdr_extract5_exec";
  case INSTR_HDR_EXTRACT6:
    return "__instr_hdr_extract6_exec";
  case INSTR_HDR_EXTRACT7:
    return "__instr_hdr_extract7_exec";
  case INSTR_HDR_EXTRACT8:
    return "__instr_hdr_extract8_exec";

  case INSTR_HDR_EXTRACT_M:
    return "__instr_hdr_extract_m_exec";

  case INSTR_HDR_LOOKAHEAD:
    return "__instr_hdr_lookahead_exec";

  case INSTR_HDR_EMIT:
    return "__instr_hdr_emit_exec";
  case INSTR_HDR_EMIT_TX:
    return "__instr_hdr_emit_tx_exec";
  case INSTR_HDR_EMIT2_TX:
    return "__instr_hdr_emit2_tx_exec";
  case INSTR_HDR_EMIT3_TX:
    return "__instr_hdr_emit3_tx_exec";
  case INSTR_HDR_EMIT4_TX:
    return "__instr_hdr_emit4_tx_exec";
  case INSTR_HDR_EMIT5_TX:
    return "__instr_hdr_emit5_tx_exec";
  case INSTR_HDR_EMIT6_TX:
    return "__instr_hdr_emit6_tx_exec";
  case INSTR_HDR_EMIT7_TX:
    return "__instr_hdr_emit7_tx_exec";
  case INSTR_HDR_EMIT8_TX:
    return "__instr_hdr_emit8_tx_exec";

  case INSTR_HDR_VALIDATE:
    return "__instr_hdr_validate_exec";
  case INSTR_HDR_INVALIDATE:
    return "__instr_hdr_invalidate_exec";

  case INSTR_MOV:
    return "__instr_mov_exec";
  case INSTR_MOV_MH:
    return "__instr_mov_mh_exec";
  case INSTR_MOV_HM:
    return "__instr_mov_hm_exec";
  case INSTR_MOV_HH:
    return "__instr_mov_hh_exec";
  case INSTR_MOV_I:
    return "__instr_mov_i_exec";

  case INSTR_DMA_HT:
    return "__instr_dma_ht_exec";
  case INSTR_DMA_HT2:
    return "__instr_dma_ht2_exec";
  case INSTR_DMA_HT3:
    return "__instr_dma_ht3_exec";
  case INSTR_DMA_HT4:
    return "__instr_dma_ht4_exec";
  case INSTR_DMA_HT5:
    return "__instr_dma_ht5_exec";
  case INSTR_DMA_HT6:
    return "__instr_dma_ht6_exec";
  case INSTR_DMA_HT7:
    return "__instr_dma_ht7_exec";
  case INSTR_DMA_HT8:
    return "__instr_dma_ht8_exec";

  case INSTR_ALU_ADD:
    return "__instr_alu_add_exec";
  case INSTR_ALU_ADD_MH:
    return "__instr_alu_add_mh_exec";
  case INSTR_ALU_ADD_HM:
    return "__instr_alu_add_hm_exec";
  case INSTR_ALU_ADD_HH:
    return "__instr_alu_add_hh_exec";
  case INSTR_ALU_ADD_MI:
    return "__instr_alu_add_mi_exec";
  case INSTR_ALU_ADD_HI:
    return "__instr_alu_add_hi_exec";

  case INSTR_ALU_SUB:
    return "__instr_alu_sub_exec";
  case INSTR_ALU_SUB_MH:
    return "__instr_alu_sub_mh_exec";
  case INSTR_ALU_SUB_HM:
    return "__instr_alu_sub_hm_exec";
  case INSTR_ALU_SUB_HH:
    return "__instr_alu_sub_hh_exec";
  case INSTR_ALU_SUB_MI:
    return "__instr_alu_sub_mi_exec";
  case INSTR_ALU_SUB_HI:
    return "__instr_alu_sub_hi_exec";

  case INSTR_ALU_CKADD_FIELD:
    return "__instr_alu_ckadd_field_exec";
  case INSTR_ALU_CKADD_STRUCT20:
    return "__instr_alu_ckadd_struct20_exec";
  case INSTR_ALU_CKADD_STRUCT:
    return "__instr_alu_ckadd_struct_exec";
  case INSTR_ALU_CKSUB_FIELD:
    return "__instr_alu_cksub_field_exec";

  case INSTR_ALU_AND:
    return "__instr_alu_and_exec";
  case INSTR_ALU_AND_MH:
    return "__instr_alu_and_mh_exec";
  case INSTR_ALU_AND_HM:
    return "__instr_alu_and_hm_exec";
  case INSTR_ALU_AND_HH:
    return "__instr_alu_and_hh_exec";
  case INSTR_ALU_AND_I:
    return "__instr_alu_and_i_exec";

  case INSTR_ALU_OR:
    return "__instr_alu_or_exec";
  case INSTR_ALU_OR_MH:
    return "__instr_alu_or_mh_exec";
  case INSTR_ALU_OR_HM:
    return "__instr_alu_or_hm_exec";
  case INSTR_ALU_OR_HH:
    return "__instr_alu_or_hh_exec";
  case INSTR_ALU_OR_I:
    return "__instr_alu_or_i_exec";

  case INSTR_ALU_XOR:
    return "__instr_alu_xor_exec";
  case INSTR_ALU_XOR_MH:
    return "__instr_alu_xor_mh_exec";
  case INSTR_ALU_XOR_HM:
    return "__instr_alu_xor_hm_exec";
  case INSTR_ALU_XOR_HH:
    return "__instr_alu_xor_hh_exec";
  case INSTR_ALU_XOR_I:
    return "__instr_alu_xor_i_exec";

  case INSTR_ALU_SHL:
    return "__instr_alu_shl_exec";
  case INSTR_ALU_SHL_MH:
    return "__instr_alu_shl_mh_exec";
  case INSTR_ALU_SHL_HM:
    return "__instr_alu_shl_hm_exec";
  case INSTR_ALU_SHL_HH:
    return "__instr_alu_shl_hh_exec";
  case INSTR_ALU_SHL_MI:
    return "__instr_alu_shl_mi_exec";
  case INSTR_ALU_SHL_HI:
    return "__instr_alu_shl_hi_exec";

  case INSTR_ALU_SHR:
    return "__instr_alu_shr_exec";
  case INSTR_ALU_SHR_MH:
    return "__instr_alu_shr_mh_exec";
  case INSTR_ALU_SHR_HM:
    return "__instr_alu_shr_hm_exec";
  case INSTR_ALU_SHR_HH:
    return "__instr_alu_shr_hh_exec";
  case INSTR_ALU_SHR_MI:
    return "__instr_alu_shr_mi_exec";
  case INSTR_ALU_SHR_HI:
    return "__instr_alu_shr_hi_exec";

  case INSTR_REGPREFETCH_RH:
    return "__instr_regprefetch_rh_exec";
  case INSTR_REGPREFETCH_RM:
    return "__instr_regprefetch_rm_exec";
  case INSTR_REGPREFETCH_RI:
    return "__instr_regprefetch_ri_exec";

  case INSTR_REGRD_HRH:
    return "__instr_regrd_hrh_exec";
  case INSTR_REGRD_HRM:
    return "__instr_regrd_hrm_exec";
  case INSTR_REGRD_HRI:
    return "__instr_regrd_hri_exec";
  case INSTR_REGRD_MRH:
    return "__instr_regrd_mrh_exec";
  case INSTR_REGRD_MRM:
    return "__instr_regrd_mrm_exec";
  case INSTR_REGRD_MRI:
    return "__instr_regrd_mri_exec";

  case INSTR_REGWR_RHH:
    return "__instr_regwr_rhh_exec";
  case INSTR_REGWR_RHM:
    return "__instr_regwr_rhm_exec";
  case INSTR_REGWR_RHI:
    return "__instr_regwr_rhi_exec";
  case INSTR_REGWR_RMH:
    return "__instr_regwr_rmh_exec";
  case INSTR_REGWR_RMM:
    return "__instr_regwr_rmm_exec";
  case INSTR_REGWR_RMI:
    return "__instr_regwr_rmi_exec";
  case INSTR_REGWR_RIH:
    return "__instr_regwr_rih_exec";
  case INSTR_REGWR_RIM:
    return "__instr_regwr_rim_exec";
  case INSTR_REGWR_RII:
    return "__instr_regwr_rii_exec";

  case INSTR_REGADD_RHH:
    return "__instr_regadd_rhh_exec";
  case INSTR_REGADD_RHM:
    return "__instr_regadd_rhm_exec";
  case INSTR_REGADD_RHI:
    return "__instr_regadd_rhi_exec";
  case INSTR_REGADD_RMH:
    return "__instr_regadd_rmh_exec";
  case INSTR_REGADD_RMM:
    return "__instr_regadd_rmm_exec";
  case INSTR_REGADD_RMI:
    return "__instr_regadd_rmi_exec";
  case INSTR_REGADD_RIH:
    return "__instr_regadd_rih_exec";
  case INSTR_REGADD_RIM:
    return "__instr_regadd_rim_exec";
  case INSTR_REGADD_RII:
    return "__instr_regadd_rii_exec";

  case INSTR_METPREFETCH_H:
    return "__instr_metprefetch_h_exec";
  case INSTR_METPREFETCH_M:
    return "__instr_metprefetch_m_exec";
  case INSTR_METPREFETCH_I:
    return "__instr_metprefetch_i_exec";

  case INSTR_METER_HHM:
    return "__instr_meter_hhm_exec";
  case INSTR_METER_HHI:
    return "__instr_meter_hhi_exec";
  case INSTR_METER_HMM:
    return "__instr_meter_hmm_exec";
  case INSTR_METER_HMI:
    return "__instr_meter_hmi_exec";
  case INSTR_METER_MHM:
    return "__instr_meter_mhm_exec";
  case INSTR_METER_MHI:
    return "__instr_meter_mhi_exec";
  case INSTR_METER_MMM:
    return "__instr_meter_mmm_exec";
  case INSTR_METER_MMI:
    return "__instr_meter_mmi_exec";
  case INSTR_METER_IHM:
    return "__instr_meter_ihm_exec";
  case INSTR_METER_IHI:
    return "__instr_meter_ihi_exec";
  case INSTR_METER_IMM:
    return "__instr_meter_imm_exec";
  case INSTR_METER_IMI:
    return "__instr_meter_imi_exec";
  ...
  case INSTR_LEARNER_LEARN:
    return "__instr_learn_exec";
  case INSTR_LEARNER_FORGET:
    return "__instr_forget_exec";
...
static void action_instr_does_tx_codegen(struct action *a, uint32_t instr_pos,
  ...
  fprintf(f,
          ...
          "\tinstr_rx_exec(p);\n"
          "\treturn;\n",
          instr_type_to_func(instr), a->name, instr_pos);
}

static void action_instr_extern_obj_codegen(struct action *a,
                                            uint32_t instr_pos, FILE *f) {
  fprintf(
      f,
...
static void action_instr_extern_func_codegen(struct action *a,
                                             uint32_t instr_pos, FILE *f) {
  fprintf(
      f,
...
static void action_instr_jmp_codegen(struct action *a, uint32_t instr_pos,
                                     struct instruction *instr,
                                     struct instruction_data *data, FILE *f) {
  switch (instr->type) {
  case INSTR_JMP:
    fprintf(f, "goto %s;\n", data->jmp_label);
    return;
  ...
  case INSTR_JMP_VALID:
    fprintf(f,
            ...
            "\t\tgoto %s;\n",
            a->name, instr_pos, data->jmp_label);
    return;
  ...
  case INSTR_JMP_EQ:
    fprintf(f,
            ...
            "\t\tgoto %s;\n",
            a->name, instr_pos, a->name, instr_pos, data->jmp_label);
    return;
...
static void action_instr_return_codegen(FILE *f) { fprintf(f, "return;\n"); }

static void action_instr_codegen(struct action *a, FILE *f) {
  uint32_t i;
  ...
  for (i = 0; i < a->n_instructions; i++) {
    ...
    fprintf(f, "%s(p, t, &action_%s_instructions[%u]);\n",
            instr_type_to_func(instr), a->name, i);
  }
...
instruction_group_list_group_find(struct instruction_group_list *igl,
                                  uint32_t instruction_id) {
  struct instruction_group *g;

  TAILQ_FOREACH(g, igl, node)
  if ((g->first_instr_id <= instruction_id) &&
      (instruction_id <= g->last_instr_id))
    return g;

  return NULL;
}

static void instruction_group_list_free(struct instruction_group_list *igl) {
  if (!igl)
    return;
...
instruction_group_list_create(struct rte_swx_pipeline *p) {
  ...
  /* Iteration 1: Separate the instructions into groups based on the thread
   * yield instructions. Do not worry about the jump instructions at this point.
   */
  for (i = 0; i < p->n_instructions; i++) {
    ...
    /* If the current group contains at least one instruction, then finalize it
     * (with the previous instruction), add it to the list and allocate a new
     * group (that starts with the current instruction).
     */
     ...
     * group contains just the current thread yield instruction), add it to the
     * list and allocate a new group (that starts with the next instruction).
     */
  ...
  /* Iteration 2: Handle jumps. If the current group contains an instruction
   * which represents the destination of a jump instruction located in a
   * different group ("far jump"), then the current group has to be split, so
   * that the instruction representing the far jump destination is at the start
   * of its group.
   */
  for (;;) {
    int is_modified = 0;
    ...
    /* Re-evaluate everything, as at least one group got split, so some jumps
     * that were previously considered local (i.e. the jump destination is in
     * the same group as the jump instruction) can now be "far jumps" (i.e. the
     * jump destination is in a different group than the jump instruction). Wost
     * case scenario: each instruction that is a jump destination ends up as the
     * first instruction of its group.
     */
    if (!is_modified)
      break;
...
pipeline_instr_does_tx_codegen(struct rte_swx_pipeline *p __rte_unused,
                               uint32_t instr_pos, struct instruction *instr,
                               FILE *f) {
  fprintf(f,
          ...
          "\tinstr_rx_exec(p);\n"
          "\treturn;\n",
          instr_type_to_func(instr), instr_pos);
}

static int pipeline_instr_jmp_codegen(struct rte_swx_pipeline *p,
  ...
  case INSTR_JMP_VALID:
    fprintf(f, "if (HEADER_VALID(t, pipeline_instructions[%u].jmp.header_id))",
            jmp_instr_id);
    break;
  ...
  case INSTR_JMP_ACTION_HIT:
    fprintf(f, "if (t->action_id == pipeline_instructions[%u].jmp.action_id)",
            jmp_instr_id);
    break;
  ...
  data =
      label_find(p->instruction_data, p->n_instructions, jmp_data->jmp_label);
  if (!data)
    return -EINVAL;
  ...
  /* Code generation for "near" jump (same instruction group) or "far" jump
...
static void instruction_group_list_codegen(struct instruction_group_list *igl,
  ...
  fprintf(f, "static const struct instruction pipeline_instructions[] = {\n");
   ...
   * instruction group that contains more than one instruction.
   */
  TAILQ_FOREACH(g, igl, node) {
    ...
    for (j = g->first_instr_id; j <= g->last_instr_id; j++) {
      ...
      /* Any other instruction type. */
      fprintf(f, "%s(p, t, &pipeline_instructions[%u]);\n",
              instr_type_to_func(instr), j);
    }

    /* Finalize the generated pipeline function. For some instructions such as
     * TX, emit-many-and-TX and unconditional jump, the next instruction has
     * been already decided unconditionally and the instruction pointer of the
     * current thread set accordingly; for all the other instructions, the
     * instruction pointer must be incremented now.
     */
...
static uint32_t instruction_group_list_custom_instructions_count(
  ...
  /* Groups with a single instruction: no function is generated for this group,
   * the group keeps its current instruction. Groups with more than two
   * instructions: one function and the associated custom instruction get
   * generated for each such group.
   */
  TAILQ_FOREACH(g, igl, node) {
    if (g->first_instr_id == g->last_instr_id)
      continue;
...
static int pipeline_codegen(struct rte_swx_pipeline *p,
                            struct instruction_group_list *igl) {
  struct action *a;
  FILE *f = NULL;
...
static int pipeline_libload(struct rte_swx_pipeline *p,
                            struct instruction_group_list *igl) {
  struct action *a;
  struct instruction_group *g;
  ...
  snprintf(buffer, RTE_SWX_PIPELINE_CMD_MAX_SIZE,
           "gcc -c -O3 -fpic -Wno-deprecated-declarations -o %s/pipeline.o "
           "%s/pipeline.c "
           "-I %s/lib/pipeline "
           "-I %s/lib/eal/include "
           ...
           ">>%s/pipeline.log 2>&1",
           dir_out, dir_out, dir_in, dir_in, dir_in, dir_in, dir_in, dir_in,
           dir_in, dir_in, dir_in, dir_in, dir_in, dir_out, dir_out, dir_out,
           dir_out);
  ...
  /* Open library. */
  snprintf(buffer, RTE_SWX_PIPELINE_CMD_MAX_SIZE, "%s/libpipeline.so", dir_out);
  ...
  TAILQ_FOREACH(g, igl, node) {
    ...
    snprintf(buffer, RTE_SWX_PIPELINE_CMD_MAX_SIZE, "pipeline_func_%u",
             g->group_id);
...
static int pipeline_adjust_check(struct rte_swx_pipeline *p __rte_unused,
                                 struct instruction_group_list *igl) {
   ...
   * to store all the custom instructions.
   */
...
static void pipeline_adjust(struct rte_swx_pipeline *p,
                            struct instruction_group_list *igl) {
  struct instruction_group *g;
  uint32_t i;
  ...
  TAILQ_FOREACH(g, igl, node) {
    ...
    /* First instruction of the group: change its type to the new custom
     * instruction. */
  ...
  p->n_instructions =
      instr_compact(p->instructions, p->instruction_data, p->n_instructions);

  /* Resolve the jump destination for any "standalone" jump instructions (i.e.
   * those jump instructions that are the only instruction within their group,
   * so they were left unmodified).
   */
