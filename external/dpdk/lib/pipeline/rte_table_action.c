...
static int fwd_apply(struct fwd_data *data,
 ...
 * RTE_TABLE_ACTION_LB
 */
static int lb_cfg_check(struct rte_table_action_lb_config *cfg) {
  if ((cfg == NULL) || (cfg->key_size < RTE_TABLE_ACTION_LB_KEY_SIZE_MIN) ||
      (cfg->key_size > RTE_TABLE_ACTION_LB_KEY_SIZE_MAX) ||
      (!rte_is_power_of_2(cfg->key_size)) || (cfg->f_hash == NULL))
    return -1;
...
static int lb_apply(struct lb_data *data,
...
pkt_work_lb(struct rte_mbuf *mbuf, struct lb_data *data,
  ...
  digest = cfg->f_hash(pkt_key, cfg->key_mask, cfg->key_size, cfg->seed);
 ...
 */
static int mtr_cfg_check(struct rte_table_action_mtr_config *mtr) {
  if ((mtr->alg == RTE_TABLE_ACTION_METER_SRTCM) ||
      ((mtr->n_tc != 1) && (mtr->n_tc != 4)) || (mtr->n_bytes_enabled != 0))
    return -ENOTSUP;
  return 0;
}
...
static void mtr_trtcm_data_meter_profile_id_set(struct mtr_trtcm_data *data,
                                                uint32_t profile_id) {
...
static uint64_t mtr_trtcm_data_stats_get(struct mtr_trtcm_data *data,
                                         enum rte_color color) {
  return data->stats[color] >> 8;
}

static void mtr_trtcm_data_stats_reset(struct mtr_trtcm_data *data,
                                       enum rte_color color) {
...
static size_t mtr_data_size(struct rte_table_action_mtr_config *mtr) {
...
meter_profile_data_find(struct meter_profile_data *mp, uint32_t mp_size,
                        uint32_t profile_id) {
  uint32_t i;
...
static int mtr_apply_check(struct rte_table_action_mtr_params *p,
                           struct rte_table_action_mtr_config *cfg,
                           struct meter_profile_data *mp, uint32_t mp_size) {
  uint32_t i;
  ...
  for (i = 0; i < RTE_TABLE_ACTION_TC_MAX; i++) {
    ...
    mp_data = meter_profile_data_find(mp, mp_size, p_tc->meter_profile_id);
    if (!mp_data)
      return -EINVAL;
  }
...
static int mtr_apply(struct mtr_trtcm_data *data,
  ...
  for (i = 0; i < RTE_TABLE_ACTION_TC_MAX; i++) {
    ...
    /* Meter object */
    status = rte_meter_trtcm_config(&data_tc->trtcm, &mp_data->profile);
    if (status)
      return status;

    /* Meter profile */
    mtr_trtcm_data_meter_profile_id_set(data_tc, mp_data - mp);

    /* Policer actions */
    mtr_trtcm_data_policer_action_set(data_tc, RTE_COLOR_GREEN,
                                      p_tc->policer[RTE_COLOR_GREEN]);

    mtr_trtcm_data_policer_action_set(data_tc, RTE_COLOR_YELLOW,
                                      p_tc->policer[RTE_COLOR_YELLOW]);

    mtr_trtcm_data_policer_action_set(data_tc, RTE_COLOR_RED,
...
pkt_work_mtr(struct rte_mbuf *mbuf, struct mtr_trtcm_data *data,
             struct dscp_table_data *dscp_table, struct meter_profile_data *mp,
             uint64_t time, uint32_t dscp, uint16_t total_length) {
  uint64_t drop_mask;
  ...
  color_meter = rte_meter_trtcm_color_aware_check(
      &data->trtcm, &mp[mp_id].profile, time, total_length, color_in);
  ...
  color_policer = MTR_TRTCM_DATA_POLICER_ACTION_COLOR_GET(data, color_meter);
 ...
 * RTE_TABLE_ACTION_TM
 */
static int tm_cfg_check(struct rte_table_action_tm_config *tm) {
...
static int tm_apply_check(struct rte_table_action_tm_params *p,
...
static int tm_apply(struct tm_data *data, struct rte_table_action_tm_params *p,
                    struct rte_table_action_tm_config *cfg) {
  int status;
...
static __rte_always_inline void pkt_work_tm(struct rte_mbuf *mbuf,
                                            struct tm_data *data,
                                            struct dscp_table_data *dscp_table,
                                            uint32_t dscp) {
  struct dscp_table_entry_data *dscp_entry = &dscp_table->entry[dscp];
  uint32_t queue_id = data->queue_id | dscp_entry->tc_queue;
 ...
 */
static int encap_valid(enum rte_table_action_encap_type encap) {
  switch (encap) {
...
static int encap_cfg_check(struct rte_table_action_encap_config *encap) {
...
#define VLAN(pcp, dei, vid)                                                    \
  ((uint16_t)((((uint64_t)(pcp)) & 0x7LLU) << 13) |                            \
   ((((uint64_t)(dei)) & 0x1LLU) << 12) | (((uint64_t)(vid)) & 0xFFFLLU))
...
static size_t encap_data_size(struct rte_table_action_encap_config *encap) {
...
static int encap_apply_check(struct rte_table_action_encap_params *p,
...
static int
encap_ether_apply(void *data, struct rte_table_action_encap_params *p,
                  struct rte_table_action_common_config *common_cfg) {
  struct encap_ether_data *d = data;
  uint16_t ethertype =
      (common_cfg->ip_version) ? RTE_ETHER_TYPE_IPV4 : RTE_ETHER_TYPE_IPV6;
...
static int encap_vlan_apply(void *data, struct rte_table_action_encap_params *p,
                            struct rte_table_action_common_config *common_cfg) {
  ...
  /* VLAN */
  d->vlan.vlan_tci =
      rte_htons(VLAN(p->vlan.vlan.pcp, p->vlan.vlan.dei, p->vlan.vlan.vid));
  d->vlan.eth_proto = rte_htons(ethertype);

  return 0;
}

static int encap_qinq_apply(void *data, struct rte_table_action_encap_params *p,
                            struct rte_table_action_common_config *common_cfg) {
  ...
  /* SVLAN */
  d->svlan.vlan_tci =
      rte_htons(VLAN(p->qinq.svlan.pcp, p->qinq.svlan.dei, p->qinq.svlan.vid));
  d->svlan.eth_proto = rte_htons(RTE_ETHER_TYPE_VLAN);

  /* CVLAN */
  d->cvlan.vlan_tci =
      rte_htons(VLAN(p->qinq.cvlan.pcp, p->qinq.cvlan.dei, p->qinq.cvlan.vid));
  d->cvlan.eth_proto = rte_htons(ethertype);

  return 0;
}

static int encap_qinq_pppoe_apply(void *data,
...
static int encap_mpls_apply(void *data,
                            struct rte_table_action_encap_params *p) {
  struct encap_mpls_data *d = data;
  uint16_t ethertype =
      (p->mpls.unicast) ? ETHER_TYPE_MPLS_UNICAST : ETHER_TYPE_MPLS_MULTICAST;
  uint32_t i;
  ...
  for (i = 0; i < p->mpls.mpls_count - 1; i++)
    d->mpls[i] = rte_htonl(MPLS(p->mpls.mpls[i].label, p->mpls.mpls[i].tc, 0,
                                p->mpls.mpls[i].ttl));

  d->mpls[i] = rte_htonl(
      MPLS(p->mpls.mpls[i].label, p->mpls.mpls[i].tc, 1, p->mpls.mpls[i].ttl));

  d->mpls_count = p->mpls.mpls_count;
  return 0;
}

static int encap_pppoe_apply(void *data,
                             struct rte_table_action_encap_params *p) {
...
static int encap_vxlan_apply(void *data,
                             struct rte_table_action_encap_params *p,
                             struct rte_table_action_encap_config *cfg) {
  ...
  if (cfg->vxlan.ip_version)
    if (cfg->vxlan.vlan) {
      ...
      /* Ethernet */
      rte_ether_addr_copy(&p->vxlan.ether.da, &d->ether.dst_addr);
      rte_ether_addr_copy(&p->vxlan.ether.sa, &d->ether.src_addr);
      d->ether.ether_type = rte_htons(RTE_ETHER_TYPE_VLAN);

      /* VLAN */
      d->vlan.vlan_tci = rte_htons(
          VLAN(p->vxlan.vlan.pcp, p->vxlan.vlan.dei, p->vxlan.vlan.vid));
    ...
    /* IPv6*/
    d->ipv6.vtc_flow = rte_htonl((6 << 28) | (p->vxlan.ipv6.dscp << 22) |
    ...
    memcpy(d->ipv6.src_addr, p->vxlan.ipv6.sa, sizeof(p->vxlan.ipv6.sa));
    memcpy(d->ipv6.dst_addr, p->vxlan.ipv6.da, sizeof(p->vxlan.ipv6.da));
...
static int encap_apply(void *data, struct rte_table_action_encap_params *p,
                       struct rte_table_action_encap_config *cfg,
                       struct rte_table_action_common_config *common_cfg) {
  int status;
...
encap_vxlan_ipv4_checksum_update(uint16_t cksum0, uint16_t total_length) {
  int32_t cksum1;
...
static __rte_always_inline void *encap(void *dst, const void *src, size_t n) {
  dst = ((uint8_t *)dst) - n;
...
pkt_work_encap_vxlan_ipv4(struct rte_mbuf *mbuf,
  ...
  ipv4_total_length =
      ether_length + (sizeof(struct rte_vxlan_hdr) +
                      sizeof(struct rte_udp_hdr) + sizeof(struct rte_ipv4_hdr));
  ipv4_hdr_cksum = encap_vxlan_ipv4_checksum_update(
      vxlan_tbl->ipv4.hdr_checksum, rte_htons(ipv4_total_length));
  udp_length = ether_length +
               (sizeof(struct rte_vxlan_hdr) + sizeof(struct rte_udp_hdr));
  ...
  mbuf->data_off =
      ether_offset - (sizeof(struct rte_mbuf) + sizeof(*vxlan_pkt));
...
pkt_work_encap_vxlan_ipv6(struct rte_mbuf *mbuf,
  ...
  ether_length = (uint16_t)mbuf->pkt_len;
  ipv6_payload_length = ether_length + (sizeof(struct rte_vxlan_hdr) +
...
static __rte_always_inline void
pkt_work_encap(struct rte_mbuf *mbuf, void *data,
               struct rte_table_action_encap_config *cfg, void *ip,
               uint16_t total_length, uint32_t ip_offset) {
  switch (cfg->encap_mask) {
  case 1LLU << RTE_TABLE_ACTION_ENCAP_ETHER:
    encap(ip, data, sizeof(struct encap_ether_data));
    mbuf->data_off =
        ip_offset - (sizeof(struct rte_mbuf) + sizeof(struct encap_ether_data));
    mbuf->pkt_len = mbuf->data_len =
        total_length + sizeof(struct encap_ether_data);
    break;

  case 1LLU << RTE_TABLE_ACTION_ENCAP_VLAN:
    encap(ip, data, sizeof(struct encap_vlan_data));
    mbuf->data_off =
        ip_offset - (sizeof(struct rte_mbuf) + sizeof(struct encap_vlan_data));
    mbuf->pkt_len = mbuf->data_len =
        total_length + sizeof(struct encap_vlan_data);
    break;

  case 1LLU << RTE_TABLE_ACTION_ENCAP_QINQ:
    encap(ip, data, sizeof(struct encap_qinq_data));
    mbuf->data_off =
        ip_offset - (sizeof(struct rte_mbuf) + sizeof(struct encap_qinq_data));
    mbuf->pkt_len = mbuf->data_len =
        total_length + sizeof(struct encap_qinq_data);
    break;

  case 1LLU << RTE_TABLE_ACTION_ENCAP_MPLS: {
    struct encap_mpls_data *mpls = data;
    size_t size = sizeof(struct rte_ether_hdr) + mpls->mpls_count * 4;
  ...
  case 1LLU << RTE_TABLE_ACTION_ENCAP_PPPOE: {
    ...
    mbuf->data_off =
        ip_offset - (sizeof(struct rte_mbuf) + sizeof(struct encap_pppoe_data));
    mbuf->pkt_len = mbuf->data_len =
        total_length + sizeof(struct encap_pppoe_data);
    break;
  }
  ...
  case 1LLU << RTE_TABLE_ACTION_ENCAP_QINQ_PPPOE: {
    ...
    mbuf->pkt_len = mbuf->data_len =
        total_length + sizeof(struct encap_qinq_pppoe_data);
    break;
  }
 ...
 */
static int nat_cfg_check(struct rte_table_action_nat_config *nat) {
  if ((nat->proto != 0x06) && (nat->proto != 0x11))
    return -ENOTSUP;
...
nat_data_size(struct rte_table_action_nat_config *nat __rte_unused,
  ...
  return (ip_version) ? sizeof(struct nat_ipv4_data)
                      : sizeof(struct nat_ipv6_data);
}

static int nat_apply_check(struct rte_table_action_nat_params *p,
...
static int nat_apply(void *data, struct rte_table_action_nat_params *p,
                     struct rte_table_action_common_config *cfg) {
  int status;
...
static __rte_always_inline uint16_t nat_ipv4_checksum_update(uint16_t cksum0,
                                                             uint32_t ip0,
                                                             uint32_t ip1) {
  int32_t cksum1;
...
static __rte_always_inline uint16_t
nat_ipv4_tcp_udp_checksum_update(uint16_t cksum0, uint32_t ip0, uint32_t ip1,
...
static __rte_always_inline uint16_t
nat_ipv6_tcp_udp_checksum_update(uint16_t cksum0, uint16_t *ip0, uint16_t *ip1,
                                 uint16_t port0, uint16_t port1) {
  int32_t cksum1;
  ...
  /* Subtract ip0 and port 0 (one's complement logic) */
  cksum1 -= ip0[0] + ip0[1] + ip0[2] + ip0[3] + ip0[4] + ip0[5] + ip0[6] +
            ip0[7] + port0;
  cksum1 = (cksum1 & 0xFFFF) + (cksum1 >> 16);
  cksum1 = (cksum1 & 0xFFFF) + (cksum1 >> 16);

  /* Add ip1 and port1 (one's complement logic) */
  cksum1 += ip1[0] + ip1[1] + ip1[2] + ip1[3] + ip1[4] + ip1[5] + ip1[6] +
            ip1[7] + port1;
  cksum1 = (cksum1 & 0xFFFF) + (cksum1 >> 16);
  cksum1 = (cksum1 & 0xFFFF) + (cksum1 >> 16);
...
static __rte_always_inline void
pkt_ipv4_work_nat(struct rte_ipv4_hdr *ip, struct nat_ipv4_data *data,
  ...
  if (cfg->source_nat) {
    if (cfg->proto == 0x6) {
      ...
      ip_cksum =
          nat_ipv4_checksum_update(ip->hdr_checksum, ip->src_addr, data->addr);

      tcp_cksum = nat_ipv4_tcp_udp_checksum_update(
          tcp->cksum, ip->src_addr, data->addr, tcp->src_port, data->port);
    ...
    if (cfg->proto == 0x6) {
      ...
      ip_cksum =
          nat_ipv4_checksum_update(ip->hdr_checksum, ip->dst_addr, data->addr);

      tcp_cksum = nat_ipv4_tcp_udp_checksum_update(
          tcp->cksum, ip->dst_addr, data->addr, tcp->dst_port, data->port);
...
static __rte_always_inline void
pkt_ipv6_work_nat(struct rte_ipv6_hdr *ip, struct nat_ipv6_data *data,
                  struct rte_table_action_nat_config *cfg) {
  if (cfg->source_nat) {
    if (cfg->proto == 0x6) {
      struct rte_tcp_hdr *tcp = (struct rte_tcp_hdr *)&ip[1];
      ...
      tcp_cksum = nat_ipv6_tcp_udp_checksum_update(
          tcp->cksum, (uint16_t *)ip->src_addr, (uint16_t *)data->addr,
          tcp->src_port, data->port);
      ...
      udp_cksum = nat_ipv6_tcp_udp_checksum_update(
          udp->dgram_cksum, (uint16_t *)ip->src_addr, (uint16_t *)data->addr,
          udp->src_port, data->port);
    ...
    if (cfg->proto == 0x6) {
      ...
      tcp_cksum = nat_ipv6_tcp_udp_checksum_update(
          tcp->cksum, (uint16_t *)ip->dst_addr, (uint16_t *)data->addr,
          tcp->dst_port, data->port);
      ...
      udp_cksum = nat_ipv6_tcp_udp_checksum_update(
          udp->dgram_cksum, (uint16_t *)ip->dst_addr, (uint16_t *)data->addr,
          udp->dst_port, data->port);
 ...
 */
static int ttl_cfg_check(struct rte_table_action_ttl_config *ttl) {
  if (ttl->drop == 0)
    return -ENOTSUP;
...
#define TTL_INIT(data, decrement) ((data)->n_packets = (decrement) ? 1 : 0)

#define TTL_DEC_GET(data) ((uint8_t)((data)->n_packets & 1))

#define TTL_STATS_RESET(data) ((data)->n_packets = ((data)->n_packets & 1))

#define TTL_STATS_READ(data) ((data)->n_packets >> 1)

#define TTL_STATS_ADD(data, value)                                             \
  ((data)->n_packets = (((((data)->n_packets >> 1) + (value)) << 1) |          \
                        ((data)->n_packets & 1)))

static int ttl_apply(void *data, struct rte_table_action_ttl_params *p) {
...
static __rte_always_inline uint64_t pkt_ipv4_work_ttl(struct rte_ipv4_hdr *ip,
...
static __rte_always_inline uint64_t pkt_ipv6_work_ttl(struct rte_ipv6_hdr *ip,
                                                      struct ttl_data *data) {
  uint32_t drop;
 ...
 */
static int stats_cfg_check(struct rte_table_action_stats_config *stats) {
...
static int stats_apply(struct stats_data *data,
...
static __rte_always_inline void pkt_work_stats(struct stats_data *data,
...
static int time_apply(struct time_data *data,
                      struct rte_table_action_time_params *p) {
  data->time = p->time;
  return 0;
}

static __rte_always_inline void pkt_work_time(struct time_data *data,
                                              uint64_t time) {
  data->time = time;
}
...
struct crypto_op_sym_iv_aad {
  ...
  union {
    struct {
      uint8_t cipher_iv[RTE_TABLE_ACTION_SYM_CRYPTO_IV_SIZE_MAX];
...
static int get_block_size(const struct rte_crypto_sym_xform *xform,
                          uint8_t cdev_id) {
...
static int sym_crypto_apply(struct sym_crypto_data *data,
  ...
  while (xform) {
    if (xform->type == RTE_CRYPTO_SYM_XFORM_CIPHER) {
      ...
      if (cipher_xform->iv.length > RTE_TABLE_ACTION_SYM_CRYPTO_IV_SIZE_MAX)
        return -ENOMEM;
      if (cipher_xform->iv.offset != RTE_TABLE_ACTION_SYM_CRYPTO_IV_OFFSET)
        return -EINVAL;
      ...
      data->cipher_auth.cipher_iv_len = cipher_xform->iv.length;
      data->cipher_auth.cipher_iv_data_offset =
          (uint16_t)p->cipher_auth.cipher_iv_update.offset;
      data->cipher_auth.cipher_iv_update_len =
          (uint16_t)p->cipher_auth.cipher_iv_update.length;

      rte_memcpy(data->iv_aad_data, p->cipher_auth.cipher_iv.val,
      ...
      if (auth_xform->iv.length > RTE_TABLE_ACTION_SYM_CRYPTO_IV_SIZE_MAX)
        return -ENOMEM;
      data->op_mask |= CRYPTO_OP_MASK_AUTH;

      data->cipher_auth.auth_iv_len = auth_xform->iv.length;
      data->cipher_auth.auth_iv_data_offset =
          (uint16_t)p->cipher_auth.auth_iv_update.offset;
      data->cipher_auth.auth_iv_update_len =
          (uint16_t)p->cipher_auth.auth_iv_update.length;
      data->digest_len = auth_xform->digest_length;

      data->direction = (auth_xform->op == RTE_CRYPTO_AUTH_OP_GENERATE)
      ...
      if ((aead_xform->iv.length > RTE_TABLE_ACTION_SYM_CRYPTO_IV_SIZE_MAX) ||
          (aead_xform->aad_length > RTE_TABLE_ACTION_SYM_CRYPTO_AAD_SIZE_MAX))
        return -EINVAL;
      if (aead_xform->iv.offset != RTE_TABLE_ACTION_SYM_CRYPTO_IV_OFFSET)
        return -EINVAL;
      ...
      data->aead.iv_data_offset = (uint16_t)p->aead.iv_update.offset;
      data->aead.iv_update_len = (uint16_t)p->aead.iv_update.length;
      data->aead.aad_data_offset = (uint16_t)p->aead.aad_update.offset;
      data->aead.aad_update_len = (uint16_t)p->aead.aad_update.length;

      rte_memcpy(data->iv_aad_data, p->aead.iv.val, p->aead.iv.length);

      rte_memcpy(data->iv_aad_data + p->aead.iv.length, p->aead.aad.val,
                 p->aead.aad.length);

      data->direction = (aead_xform->op == RTE_CRYPTO_AEAD_OP_ENCRYPT)
                            ? RTE_CRYPTO_CIPHER_OP_ENCRYPT
                            : RTE_CRYPTO_CIPHER_OP_DECRYPT;
    } else
      return -EINVAL;
  ...
  if (auth_xform && auth_xform->iv.length) {
    if (cipher_xform) {
      if (auth_xform->iv.offset !=
          RTE_TABLE_ACTION_SYM_CRYPTO_IV_OFFSET + cipher_xform->iv.length)
        return -EINVAL;

      rte_memcpy(data->iv_aad_data + cipher_xform->iv.length,
                 p->cipher_auth.auth_iv.val, p->cipher_auth.auth_iv.length);
    } else {
      rte_memcpy(data->iv_aad_data, p->cipher_auth.auth_iv.val,
  ...
  ret = rte_cryptodev_sym_session_init(cfg->cryptodev_id, session, p->xform,
                                       cfg->mp_init);
  if (ret < 0) {
...
static __rte_always_inline uint64_t pkt_work_sym_crypto(
    struct rte_mbuf *mbuf, struct sym_crypto_data *data,
    struct rte_table_action_sym_crypto_config *cfg, uint16_t ip_offset) {
  struct crypto_op_sym_iv_aad *crypto_op =
      (struct crypto_op_sym_iv_aad *)RTE_MBUF_METADATA_UINT8_PTR(
          mbuf, cfg->op_offset);
  ...
  if (data->direction == RTE_CRYPTO_CIPHER_OP_ENCRYPT) {
    uint32_t append_len =
        RTE_ALIGN_CEIL(payload_len, data->block_size) - payload_len;

    if (unlikely(rte_pktmbuf_append(mbuf, append_len + data->digest_len) ==
                 NULL))
      return 1;
  ...
  if (data->op_mask & CRYPTO_OP_MASK_CIPHER) {
    ...
    if (data->cipher_auth.cipher_iv_update_len) {
      uint8_t *pkt_iv = RTE_MBUF_METADATA_UINT8_PTR(
          mbuf, data->cipher_auth.cipher_iv_data_offset + ip_offset);
    ...
    /** write iv */
    rte_memcpy(iv, data->iv_aad_data, data->cipher_auth.cipher_iv_len);
  }
  ...
  if (data->op_mask & CRYPTO_OP_MASK_AUTH) {
    ...
    sym->auth.data.length =
        mbuf->data_len - sym->auth.data.offset - data->digest_len;
    sym->auth.digest.data = rte_pktmbuf_mtod_offset(
        mbuf, uint8_t *, rte_pktmbuf_pkt_len(mbuf) - data->digest_len);
    sym->auth.digest.phys_addr = rte_pktmbuf_iova_offset(
        mbuf, rte_pktmbuf_pkt_len(mbuf) - data->digest_len);

    if (data->cipher_auth.auth_iv_update_len) {
      uint8_t *pkt_iv = RTE_MBUF_METADATA_UINT8_PTR(
          mbuf, data->cipher_auth.auth_iv_data_offset + ip_offset);
      uint8_t *data_iv = data->iv_aad_data + data->cipher_auth.cipher_iv_len;

      if (data->direction == RTE_CRYPTO_CIPHER_OP_ENCRYPT)
        rte_memcpy(pkt_iv, data_iv, data->cipher_auth.auth_iv_update_len);
      else
        rte_memcpy(data_iv, pkt_iv, data->cipher_auth.auth_iv_update_len);
    }
  ...
  if (data->op_mask & CRYPTO_OP_MASK_AEAD) {
    ...
    sym->aead.aad.phys_addr =
        rte_pktmbuf_iova_offset(mbuf, aad - rte_pktmbuf_mtod(mbuf, uint8_t *));
    sym->aead.digest.data = rte_pktmbuf_mtod_offset(
        mbuf, uint8_t *, rte_pktmbuf_pkt_len(mbuf) - data->digest_len);
    ...
    if (data->aead.iv_update_len) {
      uint8_t *pkt_iv = RTE_MBUF_METADATA_UINT8_PTR(
          mbuf, data->aead.iv_data_offset + ip_offset);
      uint8_t *data_iv = data->iv_aad_data;

      if (data->direction == RTE_CRYPTO_CIPHER_OP_ENCRYPT)
        rte_memcpy(pkt_iv, data_iv, data->aead.iv_update_len);
      else
        rte_memcpy(data_iv, pkt_iv, data->aead.iv_update_len);
    }

    rte_memcpy(iv, data->iv_aad_data, data->aead.iv_len);

    if (data->aead.aad_update_len) {
      uint8_t *pkt_aad = RTE_MBUF_METADATA_UINT8_PTR(
          mbuf, data->aead.aad_data_offset + ip_offset);
      uint8_t *data_aad = data->iv_aad_data + data->aead.iv_len;

      if (data->direction == RTE_CRYPTO_CIPHER_OP_ENCRYPT)
        rte_memcpy(pkt_aad, data_aad, data->aead.iv_update_len);
      else
        rte_memcpy(data_aad, pkt_aad, data->aead.iv_update_len);
    }

    rte_memcpy(aad, data->iv_aad_data + data->aead.iv_len, data->aead.aad_len);
  }
...
static int tag_apply(struct tag_data *data,
                     struct rte_table_action_tag_params *p) {
  data->tag = p->tag;
  return 0;
}

static __rte_always_inline void pkt_work_tag(struct rte_mbuf *mbuf,
...
static __rte_always_inline void
pkt4_work_tag(struct rte_mbuf *mbuf0, struct rte_mbuf *mbuf1,
              struct rte_mbuf *mbuf2, struct rte_mbuf *mbuf3,
              struct tag_data *data0, struct tag_data *data1,
              struct tag_data *data2, struct tag_data *data3) {
...
static int decap_apply(struct decap_data *data,
                       struct rte_table_action_decap_params *p) {
  data->n = p->n;
  return 0;
}

static __rte_always_inline void pkt_work_decap(struct rte_mbuf *mbuf,
...
static __rte_always_inline void
pkt4_work_decap(struct rte_mbuf *mbuf0, struct rte_mbuf *mbuf1,
                struct rte_mbuf *mbuf2, struct rte_mbuf *mbuf3,
                struct decap_data *data0, struct decap_data *data1,
                struct decap_data *data2, struct decap_data *data3) {
 ...
 * Action profile
 */
static int action_valid(enum rte_table_action_type action) {
  switch (action) {
...
static size_t action_cfg_size(enum rte_table_action_type action) {
  switch (action) {
  case RTE_TABLE_ACTION_LB:
...
static void *action_cfg_get(struct ap_config *ap_config,
...
static void action_cfg_set(struct ap_config *ap_config,
                           enum rte_table_action_type type, void *action_cfg) {
...
static size_t action_data_size(enum rte_table_action_type action,
  ...
  case RTE_TABLE_ACTION_NAT:
    return nat_data_size(&ap_config->nat, &ap_config->common);
...
static void action_data_offset_set(struct ap_data *ap_data,
                                   struct ap_config *ap_config) {
  ...
  for (action = 0; action < RTE_TABLE_ACTION_MAX; action++)
    if (action_mask & (1LLU << action)) {
      ...
      offset += action_data_size((enum rte_table_action_type)action, ap_config);
    }
...
int rte_table_action_profile_action_register(
    struct rte_table_action_profile *profile, enum rte_table_action_type type,
    void *action_config) {
  int status;

  /* Check input arguments */
  if ((profile == NULL) || profile->frozen || (action_valid(type) == 0) ||
...
int rte_table_action_profile_freeze(struct rte_table_action_profile *profile) {
  if (profile->frozen)
    return -EBUSY;
...
int rte_table_action_profile_free(struct rte_table_action_profile *profile) {
  if (profile == NULL)
    return 0;
...
rte_table_action_create(struct rte_table_action_profile *profile,
  ...
  /* Check input arguments */
  if ((profile == NULL) || (profile->frozen == 0))
    return NULL;

  /* Memory allocation */
  action = rte_zmalloc_socket(NULL, sizeof(struct rte_table_action),
...
static __rte_always_inline void *
action_data_get(void *data, struct rte_table_action *action,
                enum rte_table_action_type type) {
...
int rte_table_action_apply(struct rte_table_action *action, void *data,
                           enum rte_table_action_type type,
                           void *action_params) {
  void *action_data;

  /* Check input arguments */
  if ((action == NULL) || (data == NULL) || (action_valid(type) == 0) ||
  ...
  switch (type) {
  case RTE_TABLE_ACTION_FWD:
    return fwd_apply(action_data, action_params);

  case RTE_TABLE_ACTION_LB:
    return lb_apply(action_data, action_params);

  case RTE_TABLE_ACTION_MTR:
    return mtr_apply(action_data, action_params, &action->cfg.mtr, action->mp,
                     RTE_DIM(action->mp));

  case RTE_TABLE_ACTION_TM:
    return tm_apply(action_data, action_params, &action->cfg.tm);

  case RTE_TABLE_ACTION_ENCAP:
    return encap_apply(action_data, action_params, &action->cfg.encap,
                       &action->cfg.common);

  case RTE_TABLE_ACTION_NAT:
    return nat_apply(action_data, action_params, &action->cfg.common);

  case RTE_TABLE_ACTION_TTL:
    return ttl_apply(action_data, action_params);

  case RTE_TABLE_ACTION_STATS:
    return stats_apply(action_data, action_params);

  case RTE_TABLE_ACTION_TIME:
    return time_apply(action_data, action_params);

  case RTE_TABLE_ACTION_SYM_CRYPTO:
    return sym_crypto_apply(action_data, &action->cfg.sym_crypto,
                            action_params);

  case RTE_TABLE_ACTION_TAG:
    return tag_apply(action_data, action_params);

  case RTE_TABLE_ACTION_DECAP:
    return decap_apply(action_data, action_params);
...
int rte_table_action_dscp_table_update(
    struct rte_table_action *action, uint64_t dscp_mask,
  ...
  if ((action == NULL) ||
      ...
      (dscp_mask == 0) || (table == NULL))
    return -EINVAL;

  for (i = 0; i < RTE_DIM(table->entry); i++) {
    struct dscp_table_entry_data *data = &action->dscp_table.entry[i];
    struct rte_table_action_dscp_table_entry *entry = &table->entry[i];
...
int rte_table_action_meter_profile_add(
    struct rte_table_action *action, uint32_t meter_profile_id,
  ...
  mp_data = meter_profile_data_find(action->mp, RTE_DIM(action->mp),
                                    meter_profile_id);
  if (mp_data)
    return -EEXIST;

  mp_data = meter_profile_data_find_unused(action->mp, RTE_DIM(action->mp));
  if (!mp_data)
    return -ENOSPC;

  /* Install new profile */
  status = rte_meter_trtcm_profile_config(&mp_data->profile, &profile->trtcm);
  if (status)
    return status;
...
int rte_table_action_meter_read(struct rte_table_action *action, void *data,
                                uint32_t tc_mask,
  ...
  if (stats) {
    for (i = 0; i < RTE_TABLE_ACTION_TC_MAX; i++) {
      struct rte_table_action_mtr_counters_tc *dst = &stats->stats[i];
...
int rte_table_action_ttl_read(struct rte_table_action *action, void *data,
  ...
  /* Check input arguments */
  if ((action == NULL) ||
      ((action->cfg.action_mask & (1LLU << RTE_TABLE_ACTION_TTL)) == 0) ||
      (data == NULL))
    return -EINVAL;
...
int rte_table_action_stats_read(struct rte_table_action *action, void *data,
  ...
  /* Check input arguments */
  if ((action == NULL) ||
      ((action->cfg.action_mask & (1LLU << RTE_TABLE_ACTION_STATS)) == 0) ||
      (data == NULL))
    return -EINVAL;

  stats_data = action_data_get(data, action, RTE_TABLE_ACTION_STATS);
...
int rte_table_action_time_read(struct rte_table_action *action, void *data,
                               uint64_t *timestamp) {
  struct time_data *time_data;

  /* Check input arguments */
  if ((action == NULL) ||
      ((action->cfg.action_mask & (1LLU << RTE_TABLE_ACTION_TIME)) == 0) ||
...
rte_table_action_crypto_sym_session_get(struct rte_table_action *action,
  ...
  /* Check input arguments */
  if ((action == NULL) ||
      ((action->cfg.action_mask & (1LLU << RTE_TABLE_ACTION_SYM_CRYPTO)) ==
       0) ||
      (data == NULL))
    return NULL;

  sym_crypto_data = action_data_get(data, action, RTE_TABLE_ACTION_SYM_CRYPTO);

  return sym_crypto_data->session;
}

static __rte_always_inline uint64_t pkt_work(
    struct rte_mbuf *mbuf, struct rte_pipeline_table_entry *table_entry,
    uint64_t time, struct rte_table_action *action, struct ap_config *cfg) {
  ...
  if (cfg->common.ip_version) {
    ...
    total_length = rte_ntohs(hdr->payload_len) + sizeof(struct rte_ipv6_hdr);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_LB)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_LB);

    pkt_work_lb(mbuf, data, &cfg->lb);
  }
  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_MTR)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_MTR);

    drop_mask |= pkt_work_mtr(mbuf, data, &action->dscp_table, action->mp, time,
                              dscp, total_length);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_TM)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_TM);

    pkt_work_tm(mbuf, data, &action->dscp_table, dscp);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_DECAP)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_DECAP);

    pkt_work_decap(mbuf, data);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_ENCAP)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_ENCAP);

    pkt_work_encap(mbuf, data, &cfg->encap, ip, total_length, ip_offset);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_NAT)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_NAT);
  ...
  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_TTL)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_TTL);
  ...
  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_STATS)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_STATS);

    pkt_work_stats(data, total_length);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_TIME)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_TIME);
  ...
  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_SYM_CRYPTO)) {
    void *data =
        action_data_get(table_entry, action, RTE_TABLE_ACTION_SYM_CRYPTO);

    drop_mask |= pkt_work_sym_crypto(mbuf, data, &cfg->sym_crypto, ip_offset);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_TAG)) {
    void *data = action_data_get(table_entry, action, RTE_TABLE_ACTION_TAG);
...
static __rte_always_inline uint64_t pkt4_work(
    struct rte_mbuf **mbufs, struct rte_pipeline_table_entry **table_entries,
    uint64_t time, struct rte_table_action *action, struct ap_config *cfg) {
  ...
  if (cfg->common.ip_version) {
    ...
    total_length0 = rte_ntohs(hdr0->payload_len) + sizeof(struct rte_ipv6_hdr);
    total_length1 = rte_ntohs(hdr1->payload_len) + sizeof(struct rte_ipv6_hdr);
    total_length2 = rte_ntohs(hdr2->payload_len) + sizeof(struct rte_ipv6_hdr);
    total_length3 = rte_ntohs(hdr3->payload_len) + sizeof(struct rte_ipv6_hdr);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_LB)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_LB);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_LB);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_LB);
    void *data3 = action_data_get(table_entry3, action, RTE_TABLE_ACTION_LB);

    pkt_work_lb(mbuf0, data0, &cfg->lb);

    pkt_work_lb(mbuf1, data1, &cfg->lb);

    pkt_work_lb(mbuf2, data2, &cfg->lb);

    pkt_work_lb(mbuf3, data3, &cfg->lb);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_MTR)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_MTR);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_MTR);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_MTR);
    void *data3 = action_data_get(table_entry3, action, RTE_TABLE_ACTION_MTR);

    drop_mask0 |= pkt_work_mtr(mbuf0, data0, &action->dscp_table, action->mp,
                               time, dscp0, total_length0);

    drop_mask1 |= pkt_work_mtr(mbuf1, data1, &action->dscp_table, action->mp,
                               time, dscp1, total_length1);

    drop_mask2 |= pkt_work_mtr(mbuf2, data2, &action->dscp_table, action->mp,
                               time, dscp2, total_length2);

    drop_mask3 |= pkt_work_mtr(mbuf3, data3, &action->dscp_table, action->mp,
                               time, dscp3, total_length3);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_TM)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_TM);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_TM);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_TM);
    void *data3 = action_data_get(table_entry3, action, RTE_TABLE_ACTION_TM);

    pkt_work_tm(mbuf0, data0, &action->dscp_table, dscp0);

    pkt_work_tm(mbuf1, data1, &action->dscp_table, dscp1);

    pkt_work_tm(mbuf2, data2, &action->dscp_table, dscp2);

    pkt_work_tm(mbuf3, data3, &action->dscp_table, dscp3);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_DECAP)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_DECAP);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_DECAP);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_DECAP);
    void *data3 = action_data_get(table_entry3, action, RTE_TABLE_ACTION_DECAP);

    pkt4_work_decap(mbuf0, mbuf1, mbuf2, mbuf3, data0, data1, data2, data3);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_ENCAP)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_ENCAP);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_ENCAP);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_ENCAP);
    void *data3 = action_data_get(table_entry3, action, RTE_TABLE_ACTION_ENCAP);

    pkt_work_encap(mbuf0, data0, &cfg->encap, ip0, total_length0, ip_offset);

    pkt_work_encap(mbuf1, data1, &cfg->encap, ip1, total_length1, ip_offset);

    pkt_work_encap(mbuf2, data2, &cfg->encap, ip2, total_length2, ip_offset);

    pkt_work_encap(mbuf3, data3, &cfg->encap, ip3, total_length3, ip_offset);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_NAT)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_NAT);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_NAT);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_NAT);
    void *data3 = action_data_get(table_entry3, action, RTE_TABLE_ACTION_NAT);
  ...
  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_TTL)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_TTL);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_TTL);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_TTL);
    void *data3 = action_data_get(table_entry3, action, RTE_TABLE_ACTION_TTL);
  ...
  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_STATS)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_STATS);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_STATS);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_STATS);
  ...
  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_TIME)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_TIME);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_TIME);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_TIME);
    void *data3 = action_data_get(table_entry3, action, RTE_TABLE_ACTION_TIME);
  ...
  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_SYM_CRYPTO)) {
    void *data0 =
        action_data_get(table_entry0, action, RTE_TABLE_ACTION_SYM_CRYPTO);
    void *data1 =
        action_data_get(table_entry1, action, RTE_TABLE_ACTION_SYM_CRYPTO);
    void *data2 =
        action_data_get(table_entry2, action, RTE_TABLE_ACTION_SYM_CRYPTO);
    void *data3 =
        action_data_get(table_entry3, action, RTE_TABLE_ACTION_SYM_CRYPTO);

    drop_mask0 |=
        pkt_work_sym_crypto(mbuf0, data0, &cfg->sym_crypto, ip_offset);
    drop_mask1 |=
        pkt_work_sym_crypto(mbuf1, data1, &cfg->sym_crypto, ip_offset);
    drop_mask2 |=
        pkt_work_sym_crypto(mbuf2, data2, &cfg->sym_crypto, ip_offset);
    drop_mask3 |=
        pkt_work_sym_crypto(mbuf3, data3, &cfg->sym_crypto, ip_offset);
  }

  if (cfg->action_mask & (1LLU << RTE_TABLE_ACTION_TAG)) {
    void *data0 = action_data_get(table_entry0, action, RTE_TABLE_ACTION_TAG);
    void *data1 = action_data_get(table_entry1, action, RTE_TABLE_ACTION_TAG);
    void *data2 = action_data_get(table_entry2, action, RTE_TABLE_ACTION_TAG);
    void *data3 = action_data_get(table_entry3, action, RTE_TABLE_ACTION_TAG);

    pkt4_work_tag(mbuf0, mbuf1, mbuf2, mbuf3, data0, data1, data2, data3);
  }

  return drop_mask0 | (drop_mask1 << 1) | (drop_mask2 << 2) | (drop_mask3 << 3);
}

static __rte_always_inline int ah(struct rte_pipeline *p,
  ...
  if (cfg->action_mask &
      ((1LLU << RTE_TABLE_ACTION_MTR) | (1LLU << RTE_TABLE_ACTION_TIME)))
    time = rte_rdtsc();
  ...
  if ((pkts_mask & (pkts_mask + 1)) == 0) {
    ...
    for (i = 0; i < (n_pkts & (~0x3LLU)); i += 4) {
      ...
      drop_mask = pkt4_work(&pkts[i], &entries[i], time, action, cfg);
    ...
    for (; i < n_pkts; i++) {
      ...
      drop_mask = pkt_work(pkts[i], entries[i], time, action, cfg);
    ...
    for (; pkts_mask;) {
      ...
      drop_mask = pkt_work(pkts[pos], entries[pos], time, action, cfg);
...
static int ah_default(struct rte_pipeline *p, struct rte_mbuf **pkts,
                      uint64_t pkts_mask,
                      struct rte_pipeline_table_entry **entries, void *arg) {
  struct rte_table_action *action = arg;

  return ah(p, pkts, pkts_mask, entries, action, &action->cfg);
}
...
int rte_table_action_table_params_get(
    struct rte_table_action *action, struct rte_pipeline_table_params *params) {
  ...
  params->action_data_size =
      total_size - sizeof(struct rte_pipeline_table_entry);
