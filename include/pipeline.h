...
public:
  ...
  static t_ns zeroNs() { return t_ns::zero(); }
...
public:
  Pipeline(int _index, int _workload_type, int _pkt_time_ns, int _rx_queue,
           int _tx_queue, int _gpu_id);
  ~Pipeline();
  ...
  enum rte_gpu_comm_list_status pipeline_status;
};
